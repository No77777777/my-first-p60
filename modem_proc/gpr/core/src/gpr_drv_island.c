...
// clang-format off
/*
$Header: //commercial/MPSS.HA.1.1/Main/modem_proc/gpr/core/src/gpr_drv_island.c#2 $
*/
// clang-format on
...
uint32_t __gpr_cmd_async_send(gpr_packet_t *packet)
{
   ...
   {
      AR_MSG(DBG_ERROR_PRIO, "GPR send: Domain ID out of bounds %lu, Max ID supported %lu", domain_id, GPR_PL_MAX_DOMAIN_ID_V);
      return AR_EBADPARAM;
   }
...
uint32_t __gpr_cmd_alloc_v2(uint32_t alloc_size, gpr_heap_index_t heap_index, gpr_packet_t **ret_packet)
{
   ...
   if ( alloc_size > GPR_MAX_ALLOC_PKT_SIZE - GPR_PKT_HEADER_BYTE_SIZE_V )
   {
        AR_MSG(DBG_ERROR_PRIO, "alloc_error, alloc_size is out of range");
        return AR_EBADPARAM;
   }
...
uint32_t __gpr_cmd_free(gpr_packet_t *packet)
{
   ...
   if (GPR_PL_MAX_DOMAIN_ID_V < domain_id)
   {
      AR_MSG(DBG_ERROR_PRIO, "GPR free: Domain ID out of bounds %lu, Max ID supported %lu", domain_id, GPR_PL_MAX_DOMAIN_ID_V);
      return AR_EBADPARAM;
   }
