...
--------   ---     -------------------------------------------------------
2018-09-11 mtaneda Created file.
...
static const uint32_t ar_msg_domain_id = 2; // ADSP
...
/* Option to put the ar_dbg_log data in a specific section, etc. */
...
unsigned int AR_DBG_LOG_ATTR ar_dbg_log_wr_offset = 0;

uint32_t AR_DBG_LOG_ATTR ar_dbg_log_buffer[AR_DBG_LOG_BUFFER_SIZE_WORDS];

/* Exported only for the benefit of the dbg_log cosim which reads out this debug buffer */
extern unsigned int AR_DBG_LOG_ATTR ar_dbg_log_buffer_size_words;
unsigned int AR_DBG_LOG_ATTR ar_dbg_log_buffer_size_words = AR_DBG_LOG_BUFFER_SIZE_WORDS;
...
void ar_dbg_log(const ar_dbg_log_msg_t *msg_info_ptr, uint32_t time_ms, ...)
{
   ...
   // it.
   ar_dbg_log_buffer_size_words = AR_DBG_LOG_BUFFER_SIZE_WORDS;
    ...
    * starting offset.
    */
   uint32_t wr_offset = atomic_add_masked_return(&ar_dbg_log_wr_offset, num_words, AR_DBG_LOG_BUFFER_SIZE_MASK);

   wr_offset = (wr_offset - num_words) & AR_DBG_LOG_BUFFER_SIZE_MASK;

   ar_dbg_log_buffer[wr_offset] = (uintptr_t)msg_info_ptr;
   wr_offset                    = (wr_offset + 1) & AR_DBG_LOG_BUFFER_SIZE_MASK;

   ar_dbg_log_buffer[wr_offset] = time_ms;
   wr_offset                    = (wr_offset + 1) & AR_DBG_LOG_BUFFER_SIZE_MASK;
   ...
   while (n_args--)
   {
      ar_dbg_log_buffer[wr_offset] = va_arg(va_argp, uint32_t);
      wr_offset                    = (wr_offset + 1) & AR_DBG_LOG_BUFFER_SIZE_MASK;
   }
   va_end(va_argp);
}
...
/**
Function to log with userpd cosim
*/
...
bool_t is_island = FALSE;
   ...
   const ar_dbg_log_msg_t xx_mm_log_msg = { .msg_header.b.msg_line     = ln,
   ...
   ar_dbg_log(&xx_mm_log_msg, time_ms);

#ifdef DEBUG_AR_OSAL_LOG_UTIL
   static uint32_t counter = 0;
   printf("printed userpd msg %ld, fn: %s, ln: %ld string: %s \n ", counter++, fn, ln, message);
#endif
