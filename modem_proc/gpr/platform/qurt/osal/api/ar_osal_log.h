...
#define AR_DEBUG            (0x4) /**< debug message, required at minimum for debug.*/
#define AR_INFO             (0x8) /**< info message, additional info to support debug */
...
#define AR_HIGH_PRIO           (AR_DEBUG   )   /**< High priority debug message. */
#define AR_MED_PRIO            (AR_INFO    )   /**< Medium priority debug message. */
#define AR_LOW_PRIO            (AR_VERBOSE )   /**< Low priority debug message. */

#define AR_LOG_FATAL(log_tag, ...)   AR_LOG_CRITICAL(log_tag, __VA_ARGS__)
#define AR_LOG_HIGH(log_tag, ...)    AR_LOG_DEBUG(log_tag, __VA_ARGS__)
#define AR_LOG_ERROR(log_tag, ...)   AR_LOG_ERR(log_tag, __VA_ARGS__)
#define AR_LOG_MED(log_tag, ...)     AR_LOG_INFO(log_tag, __VA_ARGS__)
