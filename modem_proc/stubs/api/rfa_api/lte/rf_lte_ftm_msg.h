...
{
  uint8                           num_chains;
  rfa_rf_lte_ftm_get_rx_agc_chain_data   rfa_rf_lte_ftm_get_rx_agc_rec[RFM_MAX_DEVICES]; /* actual size = 1..8 elements*/
...
/*-----------------------------------------------------------------------*/
/* This structure is used to support overriding Alternate Path Index for PCELL or SCELL devices in Calibration mode*/
typedef PACK(struct)
{
...
/*-----------------------------------------------------------------------*/
/* This structure is used to support overriding Alternate Path Index for logical devices in FTM verification mode*/
typedef PACK(struct)
{
  ...
  uint8 alt_path_prx;
  uint8 alt_path_drx;
} rfa_rf_lte_ftm_alt_path_verif_params_s;

/*-----------------------------------------------------------------------*/
typedef struct
{ 
  msgr_hdr_struct_type hdr;
  rfa_rf_lte_ftm_alt_path_verif_params_s lte_ftm_alt_path_verif_params;
} rfa_rf_lte_ftm_alt_path_verif_ind_s;
