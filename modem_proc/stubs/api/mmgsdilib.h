...
Copyright (c) 2004 - 2017 QUALCOMM Technologies, Inc (QTI) and its licensors.  All
...
$Header: //commercial/MPSS.TA.3.0.c1/Main/modem_proc/stubs/api/mmgsdilib.h#3 $$ $DateTime: 2021/07/08 09:57:42 $ $Author: tfong $

when       who     what, where, why
--------   ---     ----------------------------------------------------------- 
04/24/17   nr      Support to store more FPLMNs in cache for MMGSDI_USIM_FPLMN
04/19/17   av      Expose public API that returns current CSIM hiding status 
...
08/19/15   ks      Exposing mmgsdi API to get number of hw slots enabled
...
12/12/14   vdc     Added support for ERA-GLONASS
...
/*=============================================================================
   ENUM:      MMGSDI_APDU_BEHAVIOR_TYPE
=============================================================================*/
/** @brief Contains information regarding the APDU behavior type
*/
typedef enum {
  MMGSDI_APDU_BEHAVIOR_NO_GET_RESPONSE_FOR_WARNING = 0,
  MMGSDI_APDU_BEHAVIOR_AUTOMATIC_GET_RESPONSE_FOR_WARNING,
} mmgsdi_apdu_behavior_type;


/* ----------------------------------------------------------------------------

   ENUM:      MMGSDI_CSIM_HIDING_STATUS_ENUM_TYPE

   DESCRIPTION:
     The MMGSDI CSIM hiding status enum type
-------------------------------------------------------------------------------*/
typedef enum {
  MMGSDI_CSIM_UNHIDDEN,
  MMGSDI_CSIM_UNHIDING,
  MMGSDI_CSIM_HIDDEN
} mmgsdi_csim_hiding_status_enum_type;
...
typedef enum {
  ...
  MMGSDI_NV_SUPPORT_EXTENDED_FPLMN_I,
...
typedef struct {
  ...
  union {
    ...
    boolean                        uim_misconfig_ruim_n5_workaround;
                                   /* NV_SUPPORT_EXTENDED_FPLMN */
    boolean                        support_extended_fplmn;
...
/* ----------------------------------------------------------------------------
   STRUCTURE:      MMGSDI_SESSIONS_TO_SLOTS_MAPPING_TYPE
-----------------------------------------------------------------------------*/
/**
  @brief Structure used to hold the session type to slot mapping.
*/
typedef struct
{
  mmgsdi_slot_id_enum_type   pri_slot;
  mmgsdi_slot_id_enum_type   sec_slot;
  mmgsdi_slot_id_enum_type   ter_slot;
...
/**
  ...
  @param[in] default_channel_streaming: Allow APDU streaming on default channel
  @param[in] response_cb_ptr            Pointer to the response callback.
  @param[in] client_ref                 User data returned upon completion of
                                        this command.
...
/**
  ...
  * This function is called again to stop hiding CSIMs from modem clients
  ...
  @param[in] hide_csim:       When TRUE, stop hinding the CSIMs by faking
                              session activation for all 1X sessions that
                              were active before this function was called
                              to hide the CSIMs. When FALSE, hide CSIMs by
                              faking that it is disabled.
  @param[in] response_cb_ptr: Response callback
  @param[in] client_ref:      User Data returned upon
                              completion of this cmd.
  @dependencies
...
/**
  ...
  @param[out] csim_hiding_status_ptr: Output pointer holding current CSIM
                                      hiding status, if the call succeeds.
  ...
  @return
  MMGSDI_SUCCESS          : If the API is able to successfully determine
                            the CSIM hiding status
  MMGSDI_INCORRECT_PARAMS : If incorrect parameters are passed to the API
  MMGSDI_ERROR            : Otherwise
