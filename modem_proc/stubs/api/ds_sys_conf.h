...
/*===========================================================================
  ...
  $Header: //commercial/MPSS.TA.3.0.c1/Main/modem_proc/stubs/api/ds_sys_conf.h#3 $ $DateTime: 2021/07/08 09:57:42 $ $Author: tfong $
...
typedef enum
{
  ...
  */
  ...
  DS_SYS_CONF_3GPP_LTE_CELL_INFO        = DS_SYS_CONF_3GPP_CONF_MIN + 3,
  /**<
       Configuration holds value for primary and secondary cell info for LTE RAT. 
       It is updated by 3GPP mode handler whenever there is change in cell 
       information carriers for LTE RAT
  */
  
  DS_SYS_CONF_3GPP_PS_DATA_AVAIL_APPL_SERVICE_INFO = DS_SYS_CONF_3GPP_CONF_MIN + 4,
  /**< 
     This configuration is set during bootup if there is default list that needs to be 
     published or when card is read and it has updated list or during OMA DM process.
     Set operation shall trigger PS_SYS_EVENT_3GPP_PS_DATA_AVAIL_CHANGE.
  */ 
  ...
  */
  ...
  DS_SYS_CONF_APN_INFO	             	   = DS_SYS_CONF_COMMON_CONF_MIN + 12,
  /**<
  ...
  */
  ...
  DS_SYS_CONF_RAT_PREFERENCE 		= DS_SYS_CONF_COMMON_CONF_MIN + 13,
  /**<
        This configuration is used by applications to configure the rat preference.
        External clients like QMI and DSS can use this to query the same.
  */

  DS_SYS_CONF_DL_THROUGHPUT_INTERVAL_STATUS_CHANGE  = DS_SYS_CONF_COMMON_CONF_MIN + 14,
   /**< 
       This configuration is used by applications like QMI/DSS to knwo the current
       throughput interval status. All interested users in knowing throughput info
       change should be registered for the event
       DS_SYS_EVENT_UPDATED_DL_THROUGHPUT_INTERVAL_STATUS_CHANGE.
   */

   DS_SYS_CONF_UL_THROUGHPUT_CONFIG      = DS_SYS_CONF_COMMON_CONF_MIN + 15,
   /**<
        This configuration shall be only used by user to let below layers like 
        Modehandlers know the interval at which uplink throughput timer and queue size 
        information should be generated. All interested users in knowing throughput info
        should be registered for event DS_SYS_EVENT_UL_THROUGHPUT_INFO.
   */

   DS_SYS_CONF_UL_THROUGHPUT_INTERVAL_STATUS_CHANGE  = DS_SYS_CONF_COMMON_CONF_MIN + 16,
   /**<
        This configuration shall be only used by MH to notify clients that uplink
        throoughput interval status has changes. Setting this configuration
        generates DS_SYS_EVENT_UL_THROUGHPUT_INTERVAL_STATUS_CHANGE
   */

   DS_SYS_CONF_UL_THROUGHPUT_INFO  = DS_SYS_CONF_COMMON_CONF_MIN + 17,
   /**<
        This configuration shall be only used by MH to notify clients that uplink
        throoughput interval status has changes. Setting this configuration
        generates DS_SYS_EVENT_UL_THROUGHPUT_INTERVAL_STATUS_CHANGE
  ...
  */  
  ...
  DS_SYS_CONF_DATA_SERVICE_SETTING      = DS_SYS_CONF_COMMON_CONF_MIN + 20,
  /**< 
        This configuration is set by the tethered application once on boot up
  ...
  */
  ...
  DS_SYS_CONF_DATA_SERVICE_ROAMING_SETTING   = DS_SYS_CONF_COMMON_CONF_MIN +  21,
  /**< 
        This configuration is set by the tethered application once on boot up
        and when there is a change in the UI settings. This conveys if the data
  ...
  */
  ...
  DS_SYS_CONF_EMERGENCY_CALL_APN_TYPE = DS_SYS_CONF_COMMON_CONF_MIN + 22,
  /**<        
       This configuration is set by modehandler for APN type for emergency call.
  */
  ...
  */
  ...
  DS_SYS_CONF_GET_WLAN_IP_ADDR         = DS_SYS_CONF_WLAN_MIN + 2,
  /**<       
       Configuration retrieves IP ADDR for WLAN. User wont be able to do
       set operation. Whenever WLAN mode handler sets the ip address and if 
       there is change in ip address then framewok would trigger 
       DS_SYS_EVENT_WLAN_IP_ADDR_CHANGE
       
       @see DS_SYS_EVENT_WLAN_IP_ADDR_CHANGE
  */  

  DS_SYS_CONF_GET_WLAN_HESSID            = DS_SYS_CONF_WLAN_MIN + 3,
  /**<       
       Configuration retrieves HESSID for WLAN. User wont be able to do
       set operation. Whenever WLAN mode handler sets the SSID and if 
       there is change in HESSID then we would trigger 
...
/**
  @brief Data type to be used with  DS_SYS_CONF_EMERGENCY_CALL_APN_TYPE.
*/
...
typedef struct
{
  ...
  /* APN type.
   */     
} ds_sys_conf_emer_call_apn_type;
/*===========================================================================
