/*
  Copyright (c) 2009-2014, 2017, 2020 Qualcomm Technologies, Inc.
  All Rights Reserved.
  Confidential and Proprietary - Qualcomm Technologies, Inc.

  $Header: //components/rel/apr.common/3.0/osal/variant/qurt/src/apr_timer.c#3 $
  $Author: pwbldsvc $
*/
...
)
{
  ...
  else
  {
    MSG_1( MSG_SSID_DFLT, MSG_LEGACY_HIGH,
              "apr_timer_interrupt_handler(): the_timer == NULL, %lu", (uint64_t)data);
  }
  return ;
 cleanup:
  return ;
...
)
{
  ...
  EC_NZERO( timer_def_osal( &the_timer->handle, 
                  ...
                  TIMER_FUNC1_CB_TYPE, 
                  apr_timer_interrupt_handler, 
                  (time_osal_notify_data)the_timer 
                ), cleanup);
...
)
{
  ...
  timer_stop(&the_timer->handle, T_USEC, NULL);
...
)
{
  ...
  EC_NZERO( timer_set_64(&the_timer->handle,
                     time_us,
                     0,
                     T_USEC), cleanup);
