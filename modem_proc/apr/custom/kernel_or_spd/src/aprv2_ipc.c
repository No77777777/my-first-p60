...
#define T APR_RPD 
#define U APR_SPD 
DalPlatformInfoPlatformInfoType platform_info_type;
bool_t is_fusion=0; //if it is fusion, this value is updated to 1.
...
APR_INTERNAL int32_t aprv2_ipc_init ( void )
{
  ...
  DalDeviceHandle *platform_info_handle = NULL;
  ...
  dal_result = DAL_DeviceAttach(DALDEVICEID_PLATFORMINFO, &platform_info_handle);
  if ((dal_result != DAL_SUCCESS) || (platform_info_handle == NULL))
  {
    APR_DBG_1(APR_DBG_ERROR,"DEVCFG PlatformInfo: DAL_DeviceAttach failed err=%d", dal_result);
    goto DETACH_RETURN;
  }
    
  dal_result = DalPlatformInfo_GetPlatformInfo(platform_info_handle, &platform_info_type);
  if (dal_result != DAL_SUCCESS)
  {
    APR_DBG_1(APR_DBG_ERROR,"DEVCFG PlatformInfo: DalPlatformInfo_GetPlatformInfo failed err=%d", dal_result);
    goto DETACH_RETURN;
  }
  
  //#We can make use of platform key type to determine if the target is fusion.
  //#Only if the target is SDM8150, then we go check for sub-type to determine if the modem is external/internal. 
  //#Else, go ahead assuming that modem is external. 
  if (platform_info_type.fusion == TRUE) 
  {
	  #if  defined (sdm855)  || defined (sm8150)
        if ( platform_info_type.subtype != 1 )
		{
			is_fusion = TRUE;
		}
	  #else
		is_fusion = TRUE; //This is a fusion with modem as external. Invoke qsockets now
	  #endif
	
    //Detach the daldevice, once the fusion configurations are completed..	
	if (platform_info_handle)
    {
      DAL_DeviceDetach(platform_info_handle);
    }  
	  
	  
  }

  APR_DBG_0(APR_DBG_HIGH, "Ipc init in progress.");
   
  if ( is_fusion ) //Exercise only for fusion
  {
	 APR_DBG_0(APR_DBG_HIGH, "Qsocket init in progress.");
	 #ifdef QSOCKET_PORTS
     rc = apr_qsocket_init();
	 #endif
	 if ( rc ) return rc;
