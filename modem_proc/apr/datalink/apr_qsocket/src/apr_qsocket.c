/*===========================================================================
  ...
  $Header: //components/rel/apr.common/3.0/datalink/apr_qsocket/src/apr_qsocket.c#12 $
  $Author: pwbldsvc $
  ...
  */
  do
  {
    ...
    if (rc != APR_EOK)
    {
	  ...
	  qclose(port->ctrl_fd);
	  qclose(port->data_fd );
	  __aprv2_cmd_free(port->thread_id, broadcast_packet);
      break;
    }
    ...
    __aprv2_cmd_free(port->thread_id, broadcast_packet); //Free the allocated packet..
