...
$Header: //components/rel/mcfg_fwk.mpss/7.8.1.1/mcfg_utils/src/mcfg_map.c#2 $ 
$DateTime: 2023/01/22 20:47:50 $ 
$Author: pwbldsvc $ 
$Change: 43002587 $ 
...
/*===========================================================================
  ...
  FUNCTION mcfg_map_get_map_size

  DESCRIPTION
    This function returns the number of mbn records stored in the map.
  ...
  RETURN VALUE
    int - Num of records stored in map.

  SIDE EFFECTS
    None

===========================================================================*/
int mcfg_map_get_map_size
(
  mcfg_config_type_e_type type
)
{
  ...
  return ordered_list_size(&(mcfg_map_record_map[type].map));
  
} /* mcfg_map_get_map_size() */
...
)
{
  ...
  MCFG_MSG_LOW_3("mcfg_map_make_room: type %d size %d top weight %d", type, required_space, mcfg_map_record_map[type].top_weight);

  map_list = &(mcfg_map_record_map[type].map);
  map_node = ordered_list_peek_back(map_list);
  while (map_node != NULL && size_of_removed < required_space)
  {
        ...
        map_node->link.weight <= mcfg_map_record_map[type].top_weight)
    {
      ...
      if (!mcfg_efs_get_comp_config_size(type,&map_node->record.id,&comp_size))
      {
        comp_size = map_node->record.size/MCFG_COMPRESS_RATIO;
      }
      if(mcfg_efs_delete_config_local(type, &map_node->record.id, map_node->record.size))
      {
        size_of_removed += comp_size;
        MCFG_MSG_MED_1("mcfg_map_make_room: remove one config with size %d", comp_size);
  ...
  if (size_of_removed < required_space)
  {
    MCFG_MSG_ERROR_2("mcfg_map_make_room: only made room of %d, not enough for %d", 
                 size_of_removed, required_space);
    return FALSE;
  }

  MCFG_MSG_MED_1("mcfg_map_make_room: made enough room of %d", size_of_removed);
  return TRUE;
} /* mcfg_map_make_room() */
...
/*-------------------------------------------------------------------------*/
  ...
  if(weight == MCFG_WEIGHT_ACTIVE)
  {
    if (!record->efs_present)
    {
...
#ifdef FEATURE_MCFG_REMOTE_STORAGE
      if (!mcfg_efs_get_comp_config_size(type,id,&comp_size))
      {
        comp_size = record->size/MCFG_COMPRESS_RATIO;
      }
      /* Empty config record used to check if space is avaialble */
      config.config_len = record->size;
      config.comp_config_len = comp_size;
      config.comp_status = MCFG_COMP_STATUS_COMPRESSED;
      if( (MCFG_ERR_NONE != mcfg_utils_space_available(type, &config, &available_space, FALSE)) &&
          (available_space < comp_size) )
      {
        required_space = comp_size - (int)available_space;
        MCFG_MSG_HIGH_2("mcfg_map_update_prio: no more local space, try to make room. Req size %d, avail size %lu", 
			             comp_size, (uint32)available_space);
      }
      else
      {
         MCFG_MSG_MED_1("mcfg_map_update_prio: Space available %lu", (uint32)available_space);
         space_available = TRUE;
      }

      if (space_available || mcfg_map_make_room(type, required_space))
      {
        if(MCFG_ERR_NONE == mcfg_efs_copy_config_from_remote(type, &record->id, record->size))
        {
      ...
      if (!result)
      {
        MCFG_MSG_ERROR_1("mcfg_map_update_prio: selected config %d not available locally", record->size);
        return result;
      }
    }
  }
...
)
{
  ...
  else
  {
    ...
    if(!mcfg_map_update_prio(type, id, MCFG_WEIGHT_ACTIVE))
    {
      MCFG_MSG_ERROR("mcfg_map_set_selected_config:failed to update prio");
      return FALSE;
    }
...
)
{
  ...
  int     size = 0;
  int     num_of_local_mbns = 0;
  uint64  space_limit;
  uint64  size64;
  uint32 current_map_size = 0;
  ...
  *curr_size = 0;
  map_list = &(mcfg_map_record_map[type].map);
  current_map_size = ordered_list_size(map_list);
  MCFG_ASSERT_COND_RET_FALSE(current_map_size <= MCFG_CONFIG_LIST_SIZE_MAX);
  ...
  /*
  num_of_local_mbns = number of /mbn files currently present in efs.
  curr_size = (current_map_size) * 512 + size of all /mbn files currently present.
  max size = curr_size + maximum size required for storing of remaining /mbn files +  size required for storing meta files for remaining mbn records.
  */
  *max_size = *curr_size + (MCFG_CONFIG_LIST_SIZE_MAX - num_of_local_mbns) * (MCFG_CONFIG_SIZE_MAX/MCFG_COMPRESS_RATIO) + (MCFG_CONFIG_LIST_SIZE_MAX - current_map_size) * 512;

  /* Check against the configured max size allowed*/
  space_limit = mcfg_setting_space_limit() *1024;
  if ((mcfg_setting_space_limit() != 0) && (*max_size > (space_limit+(MCFG_CONFIG_SIZE_MAX/MCFG_COMPRESS_RATIO))))
  {
    MCFG_MSG_LOW_2("reduce max allowed size from %d to configured %d", *max_size, space_limit);
    *max_size = space_limit + (MCFG_CONFIG_SIZE_MAX/MCFG_COMPRESS_RATIO);
  }
  ...
  if (*max_size > (MCFG_CONFIG_SIZE_MAX/MCFG_COMPRESS_RATIO))
  {
    *max_size = *max_size - (MCFG_CONFIG_SIZE_MAX/MCFG_COMPRESS_RATIO);
  }
  else
  {
    *max_size = 0;
...
)
{
        ...
        (uint8 *)&sig_id) && !terminated)
  {
    ...
    {
      MCFG_MSG_ERROR_1("mcfg_map_read_config_map: sig mismatch (0x%lx)", 
                       sig_id.sig);
      terminated = FALSE;
      break;
    }
