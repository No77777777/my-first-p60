...
$Header: //components/rel/mcfg_fwk.mpss/7.8.1.1/mcfg_utils/src/mcfg_validate.c#1 $ 
...
typedef enum mcfg_variant_type_e{
  ...
  U_CARRIER_CONFIG_ITEM,
  U_CARRIER_FEATURE_ITEM,
  U_CARRIER_LARGE_EFS_FILE,
  U_CARRIER_SLOT_LARGE_EFS_FILE,
  U_CARRIER_MAX
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch(mcfg_item_type_int)
  {
    ...
    case MCFG_INT_MUXD_SW_ONLY_LARGE_EFS_FILE:
...
/* No use cases for Indexed EFS Item for now */
  ...
  if(attributes & U_ITEM_ATTRIB_MULTISIM)
  {
    ...
    if(subs_id)
    {
      if(mcfg_item_type_int < MCFG_INT_DATA_PROFILE || 
         ((mcfg_item_type_int >= MCFG_INT_LARGE_EFS_FILE) && 
          (mcfg_item_type_int <= MCFG_INT_MUXD_SW_ONLY_LARGE_EFS_FILE)))
      {
  ...
  if(!(attributes & U_ITEM_ATTRIB_DELETE))
  {
    if (large_efs)
    {
      config_data.data_len = efs_large_item_body.hdr.mcfg_nv_item_length;
    }
    else
    {
...
#ifdef FEATURE_MCFG_FEATURETAG
    /* only process feature tag items if processing status is off */
    if((mcfg_processing_status_get() == PROCESSING_OFF) &&
      (mcfg_item_type_int != MCFG_INT_FEATURE_ITEM))
    {
#ifdef MCFG_DEBUG
          MCFG_MSG_MED_1("FeatureTag section set item type to SW_ONLY. Proc Status == False: %d", 
                         mcfg_processing_status_get() == FALSE);
#endif /* MCFG_DEBUG */
        mcfg_item_type_int = MCFG_INT_SW_ONLY;
    }
        ...
        case MCFG_INT_MUXD_LARGE_EFS_FILE:
          ...
          else 
          {
            if( mcfg_item_type_int == MCFG_INT_LARGE_EFS_FILE  ||
                ...
                mcfg_item_type_int == MCFG_INT_MUXD_LARGE_EFS_FILE )
            {
              if(!mcfg_create_efs_file(cfg_type, ram_cust_data_ptr, mcfg_item_prefix.attrib, mcfg_restore_fact_def_validate, TRUE))
              {
                mcfg_nvefs_update_status = FALSE;
              }
            }
            ...
            {
              if(!mcfg_create_efs_file(cfg_type, ram_cust_data_ptr, mcfg_item_prefix.attrib, mcfg_restore_fact_def_validate, FALSE))
              {
                mcfg_nvefs_update_status = FALSE;
              }
            }
