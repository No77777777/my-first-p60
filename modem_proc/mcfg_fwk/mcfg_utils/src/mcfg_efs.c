...
$Header: //components/rel/mcfg_fwk.mpss/7.8.1.1/mcfg_utils/src/mcfg_efs.c#1 $ 
...
/*===========================================================================
  ...
  FUNCTION mcfg_efs_get_comp_config_size

  DESCRIPTION
    This function is used to read compressed config size for a given
    type and ID of config file.
  ...
  PARAMETERS
    type [in]  type of config memory to query
    id   [in]  ID of the config to query
    size [out] compressed size of config
...
===========================================================================*/
boolean mcfg_efs_get_comp_config_size
(
  mcfg_config_type_e_type type,
  mcfg_config_id_s_type  *id,
  int                    *size
)
{
  ...
  do
  {
    ...
    {
      MCFG_MSG_ERROR("get_comp_config_size: Failed to get Path");
      break;
    }

    if (efs_stat(path, &sbuf) < 0)
    {
      MCFG_MSG_ERROR("get_comp_config_size: Failed to get comp size");
      break;
    }
  ...
  if(!status)
  {
    if(!mcfg_efs_get_config_size(type, id, size))
    {
      MCFG_MSG_ERROR("get_comp_config_size: Failed to get size");
      return status;
    }
    *size = *size/MCFG_COMPRESS_RATIO;
    status = TRUE;
  }

  return status;
} /* mcfg_efs_get_comp_config_size() */
...
)
{
  ...
  {
    MCFG_MSG_MED_1("mcfg_efs_get_config: compresses config len %d", tmp_config_len);
...
)
{
  ...
  efs_unlink(path);
  if(efs_put(path, buf, len, (O_CREAT | O_AUTODIR), 0777) < 0)
  {
    MCFG_MSG_SPRINTF_2("Unable to create/open '%s' efs_errno %d", path, efs_errno);
    return FALSE;
  }
  return TRUE;
...
===========================================================================*/
mcfg_error_e_type mcfg_efs_store_config_path
(
  mcfg_config_type_e_type  type,
  mcfg_config_id_s_type   *id,
  char                    *remote_path
)
{
  ...
  MCFG_CHECK_NULL_PTR_RET_ERROR(remote_path, MCFG_ERR_NULL_POINTER);
...
)
{
  ...
  {
    MCFG_MSG_ERROR_1("mcfg_efs_add_config_local:map limit exceeds max size %d",MCFG_CONFIG_LIST_SIZE_MAX);
...
#ifdef FEATURE_MCFG_REMOTE_STORAGE
  if (space_available || mcfg_map_make_room(type, config->comp_config_len))
#endif
  {
...
#ifdef FEATURE_MCFG_REMOTE_STORAGE
  else if (!mcfg_remote_config_exists(type, id))
  {
    MCFG_MSG_ERROR("add_config_local: no space to store config in local and remote");
    return MCFG_ERR_OUT_OF_SPACE;
  }
  else
  {
...
)
{
  ...
  if(mcfg_map_get_map_size(type) >= MCFG_CONFIG_LIST_SIZE_MAX)
  {
    MCFG_MSG_ERROR_1("mcfg_efs_add_config:map limit exceeds max size %d",MCFG_CONFIG_LIST_SIZE_MAX);
    return MCFG_ERR_NOT_SUPPORTED;
  }
...
#ifdef FEATURE_MCFG_FEATURETAG
  error = mcfg_feature_tag_walk_through(type, config);
  if (error != MCFG_ERR_NONE)
  {
