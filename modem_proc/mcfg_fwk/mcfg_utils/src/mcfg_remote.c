...
$Header: //components/rel/mcfg_fwk.mpss/7.8.1.1/mcfg_utils/src/mcfg_remote.c#1 $ 
...
)
{
  ...
  {
    MCFG_MSG_ERROR_1("mcfg_remote_add_config:map limit exceeds max size %d",MCFG_CONFIG_LIST_SIZE_MAX);
  ...
  do
  {
...
#ifdef FEATURE_MCFG_FEATURETAG
    error = mcfg_feature_tag_walk_through(type, &remote_config);
    if (error != MCFG_ERR_NONE)
    {
      MCFG_MSG_ERROR("check_and_update_config: Invalid feature sequence");
      error = MODEM_CFG_INVALID_FEATURE_TAGS;
      break;
    }
#endif

    if (id->id_len==0 &&
        !mcfg_multi_mbn_generate_id(remote_config.config_addr, remote_config.config_len, id)) 
    {
      MCFG_MSG_ERROR("fail to generate ID");
      error = MCFG_ERR_INTERNAL;
      break;
    }
    ...
    {
      MCFG_MSG_MED("MBN with same ID already exists.");
      error = MCFG_ERR_DUPLICATE_ID;
      break;
    }

    /* save to remote path */
	error = mcfg_efs_store_config_path(type, id, (char *)config->config_addr);
    if(error != MCFG_ERR_NONE)
    {
      break;
    }
    /* Try to save to local EFS */
	error = mcfg_efs_add_config_local(type, id, &remote_config, TRUE);
	if (error != MCFG_ERR_NONE)
    {
      break;
    }
...
)
{
  ...
  if (mcfg_remote_accessible() == FALSE)
  {
     MCFG_MSG_ERROR("mcfg_remote_config_exists failed...rfs not accessible!");
     return FALSE;
  }
...
)
{
  ...
  uint8 oem_hw_config_count = 0;
  boolean replace_config_based_on_version = FALSE;
  uint32 modem_default_process_status;
  ...
  if (!mcfg_setting_remote_discover())
  {
   ...
   for(current_mcfg_type=MCFG_TYPE_HW; current_mcfg_type <= MCFG_TYPE_SW; current_mcfg_type++ )
   {
     ...
     //if version is different , then delete all the remote mbn's...else just return
      if (MCFG_ERR_NONE != mcfg_utils_delete_all(current_mcfg_type, MCFG_STORAGE_MASK_REMOTE, TRUE))
      {
        MCFG_MSG_LOW_1("unable to delete existing inactive configs for cfg type %d ", current_mcfg_type);
        continue;
      }

      // Force load to ensure default is processed after remote discovery
      if (current_mcfg_type == MCFG_TYPE_SW)
      {
        for (sloti = MCFG_SLOT_INDEX_FIRST; sloti < MCFG_NUM_OF_UIM_SLOT; sloti++)
        {
          if (!mcfg_default_config_force_load(MCFG_TYPE_SW, TRUE, mcfg_uim_map_slot_index_to_sub_id(sloti)))
          {
            MCFG_MSG_HIGH_2("Force load will not be performed for def type(%d) on sub %d!", 
                            current_mcfg_type, mcfg_uim_map_slot_index_to_sub_id(sloti));
            return FALSE;
          }
        }
      }
     ...
     else
     {
        ...
        /* Do not replace any config unless OEM's HW config list file is present with a single config provided in it */
        replace_config_based_on_version = FALSE;
        if(current_mcfg_type == MCFG_TYPE_HW)
        {
          if(mcfg_remote_customized_list_file(filepath, current_mcfg_type))
          {
            oem_hw_config_count = 0;
            line_offset = 0;
            while (line_offset < stat.st_size && mcfg_utils_read_one_line(data_from_path_info, stat.st_size+1, &line_offset, line, MCFG_RFS_RECORD_FILENAME_MAX-1))
            {
              ...
              oem_hw_config_count++;
              if(oem_hw_config_count > 1)
              {
                break;
              }
            }

            MCFG_MSG_HIGH_1("mcfg_remote_discover: oem_hw_config_count %d", oem_hw_config_count);
            if(oem_hw_config_count == 1)
            {
              /* HW config can be replaced when there is only one oem specific HW config is provided */
              replace_config_based_on_version = TRUE;
            }
          }
        }
        ...
        while (line_offset < stat.st_size && mcfg_utils_read_one_line(data_from_path_info, stat.st_size+1, &line_offset, line, MCFG_RFS_RECORD_FILENAME_MAX-1))
        {
           ...
           mcfg_multi_mbn_add_config(current_mcfg_type, &config, MCFG_STORAGE_REMOTE, replace_config_based_on_version);
  ...
  else
  {
    for (sloti = MCFG_SLOT_INDEX_FIRST; sloti < MCFG_NUM_OF_UIM_SLOT; sloti++)
    {
      modem_default_process_status = 
        mcfg_default_config_process(MCFG_TYPE_SW, mcfg_uim_map_slot_index_to_sub_id(sloti));
      if (modem_default_process_status != MODEM_CFG_PROC_SUCCESS)
      {
        MCFG_MSG_ERROR_1("mcfg_remote_discover: Failed to process SW_DEFAULT on sub %d", 
