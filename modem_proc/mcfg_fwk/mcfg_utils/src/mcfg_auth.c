...
$Header: //components/rel/mcfg_fwk.mpss/7.8.1.1/mcfg_utils/src/mcfg_auth.c#2 $ 
$DateTime: 2021/07/12 02:05:02 $ 
$Author: pwbldsvc $ 
$Change: 31770219 $ 
...
==============================================================================*/
/* EFS file to be used to enable/disable err_fatal in case mcfg_auth_img_verify() fails */
#define MCFG_IMG_VERIFY_ERR_FATAL     "/nv/item_files/mcfg/img_verify_err_fatal"
...
)
{
  ...
  if(MCFG_INTERGER_OVERFLOWED(auth_img_hdr->qc_metadata_size, auth_img_hdr->oem_metadata_size))
  {
    MCFG_MSG_ERROR_2("mcfg_auth_verify_hash() invalid qc_metadata_size %lu oem_metadata_size %lu", auth_img_hdr->qc_metadata_size, auth_img_hdr->oem_metadata_size);
    return FALSE;
  }
...
/*===========================================================================
  ...
  FUNCTION mcfg_auth_img_verify_err_fatal

  DESCRIPTION
    API to check if the err_fatal is enabled or not in case mcfg_auth_img_verify() fails
...
===========================================================================*/
static boolean mcfg_auth_img_verify_err_fatal
(
  void
)
{
  struct fs_stat stat;
  if((efs_lstat(MCFG_IMG_VERIFY_ERR_FATAL, &stat) == -1) && (efs_errno == ENOENT))
  {
    return FALSE;
  } 
  return TRUE;
...
)
{
  ...
  else if(!auth_enabled)
  {
    ...
    retVal = TRUE;
                                                                  ...
                                                                  NULL))
    {
      if (mcfg_auth_img_verify_err_fatal())
      {
        MCFG_ERR_FATAL("mcfg_auth_img_verify() failed!!", 0,0,0);
      }
