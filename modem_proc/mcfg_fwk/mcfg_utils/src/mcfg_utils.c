...
$Header: //components/rel/mcfg_fwk.mpss/7.8.1.1/mcfg_utils/src/mcfg_utils.c#1 $ 
...
#define MCFG_IGNORE_MODEM_RESET       "/nv/item_files/mcfg/ignore_modem_reset"
/*==============================================================================
  Typedefs
==============================================================================*/
...
static timer_type       mcfg_utils_reset_retry_timer;  /* periodic SSR retry timer */
...
static boolean mcfg_ssreq_served = FALSE; /* denotes if SSReq initiated and got served */
#endif
...
void mcfg_utils_reset_modem(mcfg_power_down_reason_e_type res_code)
{
  ...
  struct fs_stat stat;
  if(!((efs_lstat(MCFG_IGNORE_MODEM_RESET, &stat) == -1) && (efs_errno == ENOENT)))
  {
    MCFG_MSG_HIGH("mcfg_utils_reset_modem: Reset got blocked. Reason: Ignore modem reset NV is enabled!");
    return;
  }
...
#ifdef FEATURE_MCFG_SSREQ
  MCFG_MSG_MED_1("mcfg_utils_reset_modem, mcfg_ssreq_served:%d", mcfg_ssreq_served);
  if (timer_is_active(&mcfg_utils_reset_retry_timer) && mcfg_ssreq_served)
  {
    MCFG_MSG_MED("SSR request already initiated, waiting for power down..");
    return;
  }
...
/*===========================================================================
  ...
  PARAMETERS
    type        [in]   type of the config to check
    config      [in]   details of the config for which the check is
    available   [out]  currently available space limit
    processing  [in]   is it for processing the config?
...
)
{
  ...
  *available = 0;   // initialize to '0'; so that it can be used for the validation by the caller.
  ...
  {
    MCFG_MSG_ERROR_2("Max size(%d) < curr size(%d)", (int)max_size, (int)curr_size);
    return MCFG_ERR_OUT_OF_SPACE;
  }
  ...
  *available = max_size - curr_size;
  ...
  {
    MCFG_MSG_ERROR_4("space_available: no enough space %lu for requested size %d. max_size %lu, curr_size %lu", 
                     (uint32)*available, request_size, (uint32)max_size, (uint32)curr_size);
...
)
{
  ...
  if (id->id_len>0 && !mcfg_utils_is_config_id_unique(type, id))
  {
    MCFG_MSG_ERROR("mcfg_utils_add_config:ID is not unique");
    return MCFG_ERR_DUPLICATE_ID;
  }
  if(mcfg_map_get_map_size(type) >= MCFG_CONFIG_LIST_SIZE_MAX)
  {
    MCFG_MSG_ERROR_1("mcfg_utils_add_config:map limit exceeds max size %d",MCFG_CONFIG_LIST_SIZE_MAX);
    return MCFG_ERR_NOT_SUPPORTED;
  }
  if(!mcfg_map_get_config_limits(type, &max_size, &curr_size))
  {
    MCFG_MSG_ERROR("mcfg_utils_add_config: Failed to get config limit");
    return MCFG_ERR_OUT_OF_SPACE;
  }
...
)
{
  ...
  if (id->id_len==1)
  {
    mcfg_run_policy(0, id->id[0]);
    return MCFG_ERR_NONE;
  }
  if (id->id_len==2)
  {
    mcfg_run_policy(1, id->id[0]);
    return MCFG_ERR_NONE;
  }
#endif
...
/*===========================================================================
  ...
  PARAMETERS
    type      [in]  type of the config to delete
    storage_mask [in] the storage type of the config to delete
    force_delete [in] boolean flag to enable/disable force_delete. Will be overwritten as 'FALSE', if autoselection is disabled
...
)
{
  ...
  {  
    if ((mcfg_uim_nv_autoselect_enabled(MCFG_NUM_OF_UIM_SLOT) == FALSE) || (type == MCFG_TYPE_HW))
    {
      force_delete = FALSE;
    }
      ...
      for (i = 0; i < num_of_id; i++)
      {
        ...
        /* Keep petting the dog in case deletion takes a long time,
        ** especially with remote and non-EMMC flash and a large number
        ** of MBNs */       
...
boolean mcfg_utils_get_config_by_carrier_index
(
...
)
{
  ...
  MCFG_MSG_MED_4("set_active_config sub%d %02x%02x%02x", 
  ...
  mcfg_setting_backup(mcfg_uim_map_sub_id_to_slot_index(sub), MCFG_SETTING_CURR);
  // backup current settings for later use during deactivation
  mcfg_setting_backup(mcfg_uim_map_sub_id_to_slot_index(sub), MCFG_SETTING_BKUP);
#endif
...
)
{
  ...
  for (sub = MCFG_SUB_ID_FIRST; sub < MCFG_NUM_OF_SUB_ID; sub++)
  {
...
#ifdef FEATURE_MCFG_FEATURETAG
        if (!mcfg_settings_updated(mcfg_uim_map_sub_id_to_slot_index(sub)))
#endif
      {
...

    {
      ...
      {
        MCFG_MSG_MED_2("Refresh is ongloing for slot %d, sub %d, set ACTIVATE timer", slot, sub);
  ...
  else
  {
    ...
    switch (utils_cmd_data->cmd_id)
    {
      ...
      case MCFG_UTILS_CMD_RESET_RETRY:
        timer_set(&mcfg_utils_reset_retry_timer, MCFG_UTILS_RESET_RETRY_TIMEOUT,
                  0, T_SEC);
        break;
      ...
      case MCFG_UTILS_CMD_CLEAR_RESET_TIMER:
        ...
        timer_clr(&mcfg_utils_reset_retry_timer,T_SEC);
        break;
...
)
{
  ...
  timer_def(&mcfg_utils_reset_retry_timer,
            &mcfg_utils_reset_timer_group,
            NULL,
            0,
            mcfg_utils_cmd_reset_timer_cb,
...
static mcfg_config_header_e_type mcfg_utils_check_config_header(const void *config)
{
  ...
  MCFG_MSG_LOW_2("mcfg_utils_check_config_header(): first 2 bytes of config, 0x%x, 0x%x", buf[0], buf[1]);
...
)
{
  uint8   *pTemp;
  uint32  left_size;
  MCFG_CHECK_NULL_PTR_RET(buf);

  if (msgprint)
  {
    MCFG_MSG_SPRINTF_3("%s buf %p with size of %d", msgprint, buf, size);
  }
  else
  {
    MCFG_MSG_SPRINTF_2("print buf %p with size of %d", buf, size);
  }

  pTemp = buf;
  while ((pTemp+8) <= (buf+size))
  {
    MCFG_MSG_MED_8("%02x%02x%02x%02x %02x%02x%02x%02x",
  ...
  if (pTemp <(buf+size))
  {
    ...
    switch (left_size)
    {
      case 1:
        ...
        break;
      case 2:
        MCFG_MSG_MED_2("%02x%02x",
                       pTemp[0], pTemp[1]);
        break;
      case 3:
        MCFG_MSG_MED_3("%02x%02x%02x",
                       pTemp[0], pTemp[1], pTemp[2]);
        break;

      case 4:
        MCFG_MSG_MED_4("%02x%02x%02x%02x",
                       pTemp[0], pTemp[1], pTemp[2], pTemp[3]);
        break;

      case 5:
        MCFG_MSG_MED_5("%02x%02x%02x%02x %02x",
                       pTemp[0], pTemp[1], pTemp[2], pTemp[3],
                       pTemp[4]);
        break;

      case 6:
        MCFG_MSG_MED_6("%02x%02x%02x%02x %02x%02x",
                       pTemp[0], pTemp[1], pTemp[2], pTemp[3],
                       pTemp[4], pTemp[5]);
        break;

      case 7:
        MCFG_MSG_MED_7("%02x%02x%02x%02x %02x%02x%02x",
                       pTemp[0], pTemp[1], pTemp[2], pTemp[3],
                       pTemp[4], pTemp[5], pTemp[6]);
        break;
      default:
        MCFG_MSG_ERROR_1("unexpected leftover %lu", left_size);
        break;
      }
  }
}
