...
/*===========================================================================
  ...
  $Header: //components/rel/mcfg_fwk.mpss/7.8.1.1/mcfg_utils/src/mcfg_load.c#2 $ $DateTime: 2021/07/12 02:05:02 $ $Author: pwbldsvc $ 
...
static boolean mcfg_load_read_data(mcfg_config_s_type* config_ptr, uint32 offset, uint32 size, uint8* data)
{
  ...
  MCFG_ASSERT_COND_RET_FALSE(config_ptr->config_len > 0);
       ...
       !MCFG_INTERGER_OVERFLOWED(config_ptr->config_addr, offset))
  {
    ...
    memscpy(data, size, mcfg_data_ptr, config_ptr->config_len);
    return TRUE;
  }
  else
  {
