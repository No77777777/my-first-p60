...
/*===========================================================================
  ...
  $Header: //components/rel/mcfg_fwk.mpss/7.8.1.1/mcfg_proc/src/mcfg_proc.c#1 $ $DateTime: 2021/04/08 00:04:19 $ $Author: pwbldsvc $ 
...
#define U_CARRIER_FEATURE_ITEM            0x07
#define U_CARRIER_LARGE_EFS_FILE          0x08
#define U_CARRIER_SLOT_LARGE_EFS_FILE     0x09
...
// Typedefs
typedef mcfg_trl_mcfg_header_s_type mcfg_header_type;
...
typedef enum mcfg_exclude_status_e
{
  MCFG_EXCLUDE_STATUS_UNINIT = 0,
  MCFG_EXCLUDE_STATUS_NOTFOUND,
  MCFG_EXCLUDE_STATUS_FOUND
} mcfg_exclude_status_e_type;

//list for excluded items
typedef struct                            
{
  mcfg_exclude_status_e_type	 status;
	mre_set_t 									*nv_list;
	mre_set_t 	  							*efs_list;
} mcfg_exclude_list_type;
mcfg_exclude_list_type mcfg_exclude_list;
...
static boolean mcfg_proc_exclude_list_generate();
static boolean mcfg_proc_is_excluded_nv(nv_items_enum_type eItem);
static boolean mcfg_proc_is_excluded_efs(char *efs_path, size_t len);
...
/* -------------------------------------------------------------------- */
  ...
  do
  {
    ...
    if (mcfg_utils_space_available(current_mcfg_type, &mcfg_current_buf, &available_space, TRUE) != MCFG_ERR_NONE)
    {
    ...
    if( mcfg_update_required(current_mcfg_type, ram_cust_data_ptr, &ram_mcfg_version, mcfg_subs_num, &mcfg_current_id))
    {
      ...
      if (refresh_type != MCFG_REFRESH_TYPE_SUBS)
      {
        if (!mcfg_update_nvefs(current_mcfg_type, num_mcfg_items, ram_cust_data_ptr, 
                              mcfg_subs_num, MCFG_REFRESH_TYPE_SLOT, FALSE))
        {
      ...
      {
        if (!mcfg_update_nvefs(current_mcfg_type, num_mcfg_items, ram_cust_data_ptr, 
                              mcfg_subs_num, MCFG_REFRESH_TYPE_SUBS, FALSE))
        {
          MCFG_MSG_ERROR_2("Update NVEFS Fail for cfg type, %0x! subs_id=%d", current_mcfg_type, mcfg_subs_num);
          modem_cfg_process_status = MODEM_CFG_UPDATE_NVEFS_FAIL;
          break;
...
/* -------------------------------------------------------------------- */
  ...
  for(current_mcfg_type=MCFG_TYPE_MIN; current_mcfg_type <= MCFG_TYPE_MAX; current_mcfg_type++ )
  {
    ...
    for(mcfg_subs_num = MCFG_SUB_ID_FIRST; mcfg_subs_num < mcfg_max_subs; mcfg_subs_num++)
    {
      ...
      /* Check if there's enough space to process MBN */
      if (mcfg_utils_space_available(current_mcfg_type, mcfg_current_buf.config_len, &available_space, TRUE) != MCFG_ERR_NONE)
      {
        MCFG_MSG_ERROR_2("No enough EFS space for cfg_type %d, subs_id %d",current_mcfg_type, mcfg_subs_num);
        modem_cfg_process_status = MODEM_CFG_PROC_LOAD_FAIL; 
        continue;
      ...
      if( mcfg_update_required(current_mcfg_type, ram_cust_data_ptr, &ram_mcfg_version, mcfg_subs_num, &mcfg_current_id))
      {
        ...
        if (!mcfg_update_nvefs(current_mcfg_type, num_mcfg_items, ram_cust_data_ptr, mcfg_subs_num, MCFG_REFRESH_TYPE_SLOT_N_SUBS, FALSE))
        {
          MCFG_MSG_ERROR_2("Update NVEFS Fail for cfg type, %0x! subs_id=%d", current_mcfg_type, mcfg_subs_num);
          mcfg_utils_free_config_buffer(p_mcfg_buf);
          modem_cfg_process_status = MODEM_CFG_UPDATE_NVEFS_FAIL;
        ...
        MCFG_MSG_HIGH_2("Update mcfg version to %0x, subs_id=%d", ram_mcfg_version, mcfg_subs_num);
                            ...
                            mcfg_refresh_type_e_type refresh_type)
{
  ...
  if( ( mcfg_hdr->magic1 != MCFG_MAGIC ) || 
      ((mcfg_hdr->mcfg_format_ver_num != MCFG_FORMAT_NUM) && 
       ...
       (mcfg_hdr->mcfg_format_ver_num != MCFG_FORMAT_NUM_FEATAGGED)) ||
                             ...
                             uint8 subs_id)
{
  ...
  mcfg_slot_index_type_e_type sloti;
   /* Populate the Item Prefix from the  Data Ptr*/
    ...
    if ((item_type == U_CARRIER_NV_ITEM || item_type == U_CARRIER_NV_EFS_FILE || item_type == U_CARRIER_DATA_PROFILE || item_type == U_CARRIER_LARGE_EFS_FILE) &&
        (refresh_type == MCFG_REFRESH_TYPE_SLOT))
    {
      //MCFG_MSG_LOW_1("skip item_type %d for non-slot items", item_type);
      return MCFG_INT_SW_ONLY;
    }
    else if ((item_type == U_CARRIER_SLOT_NV_ITEM || item_type == U_CARRIER_SLOT_NV_EFS_FILE || item_type == U_CARRIER_SLOT_LARGE_EFS_FILE) && 
    ...
    switch(item_type)
    {
      ...
      case U_CARRIER_LARGE_EFS_FILE:
      case U_CARRIER_SLOT_LARGE_EFS_FILE:
          if ( (mcfg_item_prefix_ptr->attrib) & U_ITEM_ATTRIB_CFG ) 
          {
            ret_val = MCFG_INT_LARGE_EFS_FILE;
            if( (mcfg_item_prefix_ptr->attrib) & U_ITEM_ATTRIB_WRITE_ONCE ) 
            {
               ret_val = MCFG_INT_STATIC_WO_LARGE_EFS_FILE;
            ...
            if((mcfg_item_prefix_ptr->attrib) & U_ITEM_ATTRIB_MUXD)
            {
              ret_val = MCFG_INT_MUXD_LARGE_EFS_FILE;
            }
          }
          else if ( (mcfg_item_prefix_ptr->attrib) & U_ITEM_ATTRIB_DELETE ) 
          {
            ret_val = MCFG_INT_DELETE_LARGE_EFS_FILE;
          }
          else
          {
            /* Muxd-SW EFS files */
            if( (mcfg_item_prefix_ptr->attrib) & U_ITEM_ATTRIB_MUXD )
            {
              ret_val = MCFG_INT_MUXD_SW_ONLY_LARGE_EFS_FILE;
            }
          }
          break;
    ...
    case U_CARRIER_CONFIG_ITEM:
      ...
      case U_CARRIER_FEATURE_ITEM:
#ifdef FEATURE_MCFG_FEATURETAG
          // NOTE - Only attribute bit currently supported for feature is multiSIM bit
          // MCFG_MSG_MED("setting ret_val to MCFG_INT_FEATURE_ITEM");
          ret_val = MCFG_INT_FEATURE_ITEM;
#else
          ret_val = MCFG_INT_SW_ONLY;
...
#ifdef FEATURE_MCFG_DATA_API_CLEANUP_SUPPORT
    mcfg_task_data_cleanup(SYS_MODEM_AS_ID_NONE);
#endif
  }
...
#ifdef FEATURE_MCFG_DATA_API_CLEANUP_SUPPORT
      mcfg_task_data_cleanup(SYS_MODEM_AS_ID_1);
#endif
    }
    else if (subs_id == 1)
    {
...
#ifdef FEATURE_MCFG_DATA_API_CLEANUP_SUPPORT
      mcfg_task_data_cleanup(SYS_MODEM_AS_ID_2);
#endif
    }
    else if (subs_id == 2)
    {
...
#ifdef FEATURE_MCFG_DATA_API_CLEANUP_SUPPORT
      mcfg_task_data_cleanup(SYS_MODEM_AS_ID_3);
#endif
    }
  }
                          ...
                          boolean skip_storage_items)
{
  ...
  uint8 item_index = 0;
  uint8 subs_id;
  uint32 feat_data_len;

  if(cfg_type == MCFG_TYPE_SW)
  {
    mcfg_proc_exclude_list_generate();
  }
  ...
  for (i = 1; i<num_mcfg_items; i++) 
  {
...
#ifdef FEATURE_MCFG_FEATURETAG
    if((mcfg_processing_status_get() == PROCESSING_OFF) || skip_storage_items)
    {
      if (mcfg_item_type_int != MCFG_INT_FEATURE_ITEM)
      {
#ifdef MCFG_DEBUG
          MCFG_MSG_MED_2("FeatureTag: item type set to SW_ONLY. Proc Status == False: %d, skip_storage_items: %d", 
                         mcfg_processing_status_get() == FALSE, skip_storage_items);
#endif /* MCFG_DEBUG */

        /* If processing state is inactive then only process feature tags. */
        mcfg_item_type_int = MCFG_INT_SW_ONLY;
      }
    }
#endif /* FEATURE_MCFG_FEATURETAG */
    ...
    switch (mcfg_item_type_int) 
    {
      ...
      case MCFG_INT_NV_ITEM:
        ...
        {    
          mcfg_update_nv_item_flag = TRUE;
				...
				if(cfg_type == MCFG_TYPE_SW && mcfg_proc_is_excluded_nv(nv_mcfg_cust_item))
				{
					MCFG_MSG_HIGH_1("Exclude nv item %d", nv_mcfg_cust_item);
					mcfg_update_nv_item_flag = FALSE;
				}
...
#ifdef FEATURE_MCFG_FEATURETAG
      case MCFG_INT_FEATURE_ITEM:
        MCFG_MSG_LOW_2("Encountered feature item type %d index %d", 
                       mcfg_item_type_int, i);

        feat_data_len = mcfg_item_prefix.item_length - sizeof(mcfg_item_prefix);

        if(!mcfg_process_feature(subs_id, ram_cust_data_ptr, feat_data_len, 
                                 MCFG_FEAT_CONTEXT_ACTIVATION))
        {
          /* Error processing featureTag*/
          MCFG_MSG_ERROR_1("MCFG: Failure processing FeatureTag at index: %lu", i);
          mcfg_nvefs_update_status = FALSE;
        }
      ...
      case MCFG_INT_MUXD_NV_ITEM:
            ...
            )
          {
            ...
            if ( mcfg_item_prefix.attrib & U_ITEM_ATTRIB_INDEXED )
            {
              ...
              index = *(dataSubID_ptr);
						...
						if(cfg_type == MCFG_TYPE_SW && mcfg_proc_is_excluded_nv(nv_mcfg_cust_item))
						{
							MCFG_MSG_HIGH_1("Exclude nv item %d for deletion", nv_mcfg_cust_item);
							delete_nv_flag = FALSE;
						}
        ...
        case MCFG_INT_LARGE_EFS_FILE:
        case MCFG_INT_STATIC_WO_LARGE_EFS_FILE:
        case MCFG_INT_MUXD_LARGE_EFS_FILE:
          MCFG_MSG_LOW_2("Encountered EFS_FILE type %d index %d", 
                         mcfg_item_type_int, i);
          if (mcfg_item_type_int == MCFG_INT_LARGE_EFS_FILE ||
              mcfg_item_type_int == MCFG_INT_STATIC_WO_LARGE_EFS_FILE ||
              mcfg_item_type_int == MCFG_INT_MUXD_LARGE_EFS_FILE )
          {
            if(!mcfg_create_efs_file(cfg_type, ram_cust_data_ptr, mcfg_item_prefix.attrib, mcfg_restore_fact_def, TRUE))
            {
              mcfg_nvefs_update_status = FALSE;
            }
          }
          ...
          {
            if(!mcfg_create_efs_file(cfg_type, ram_cust_data_ptr, mcfg_item_prefix.attrib, mcfg_restore_fact_def, FALSE))
            {
              mcfg_nvefs_update_status = FALSE;
            }
          }
        ...
        case MCFG_INT_DELETE_LARGE_EFS_FILE:
          MCFG_MSG_HIGH("Encountered delete EFS File");
          if(mcfg_item_type_int == MCFG_INT_DELETE_LARGE_EFS_FILE)
          {
            if(MCFG_NV_STATUS_OK != mcfg_delete_efs_file(cfg_type, ram_cust_data_ptr, mcfg_item_prefix.attrib, TRUE))
            {
              mcfg_nvefs_update_status = FALSE;
            }
          }
          ...
          {
            if(MCFG_NV_STATUS_OK != mcfg_delete_efs_file(cfg_type, ram_cust_data_ptr, mcfg_item_prefix.attrib, FALSE))
            {
              mcfg_nvefs_update_status = FALSE;
            }
          }
...
boolean mcfg_verify_large_efs_item_contents(mcfg_tlv* header, mcfg_large_tlv* body)
{
  // TLV1 ( 0x0001, File path Length, File path )
  if(header->hdr.mcfg_nv_item_type != MCFG_EFS_ITEM_TYPE_FILE_PATH)
  {
...
SIDE EFFECTS
  None
===========================================================================*/
boolean mcfg_create_efs_file(uint8 cfg_type, uint8* efsData, uint8 attributes, boolean restore_fact, boolean large_efs)
{
  char* filePath = 0;
  uint8* fileData = 0;
  uint8  subs_id = 0;
  ...
  mcfg_large_tlv efs_large_item_body;
  ...
  if (large_efs)
  {
    ...
    mcfg_read_large_tlv(&efs_large_item_body, efsData + efs_item_header.hdr.mcfg_nv_item_length + MCFG_TL_SIZE_BYTES);

    if(!mcfg_verify_large_efs_item_contents(&efs_item_header, &efs_large_item_body))
    {
      MCFG_MSG_ERROR("EFS File item format incorrect");
      return FALSE;
    }
    fileData = efs_large_item_body.value;
  }
  else
  {
    // read file contents data
  ...
  if(cfg_type == MCFG_TYPE_SW && mcfg_proc_is_excluded_efs(filePath, strlen(filePath)))
  {
    MCFG_MSG_SPRINTF_1("exclude EFS File: %s, skipping...", filePath);
    return TRUE;
  }
...
/* No use cases for Indexed EFS Item for now */
  ...
  if(attributes & U_ITEM_ATTRIB_MULTISIM)
  {
    ...
    /* Decrement the size by 1 to account for the subs_mask */
     ...
     if (large_efs)
     {
       efs_large_item_body.hdr.mcfg_nv_item_length--;
     }
     else
     {
  ...
  if(attributes & U_ITEM_ATTRIB_MUXD)
  {
    if (large_efs)
    {
      mcfg_result = mcfg_nv_efs_update_symlink(f_name, efs_large_item_body.hdr.mcfg_nv_item_length);
    }
    else
    {
  ...
  if (large_efs)
  {
    efs_result = efs_put(f_name, fileData, efs_large_item_body.hdr.mcfg_nv_item_length, O_CREAT | O_AUTODIR, ALLPERMS);
  }
  else
  {
...
SIDE EFFECTS
  None
===========================================================================*/
mcfg_nv_status_e_type mcfg_delete_efs_file(uint8 cfg_type, uint8* efsData, uint8 attrib, boolean large_efs )
{
  char* filePath = 0;
  uint8* fileData = 0;
  mcfg_tlv efs_item_header;
  ...
  if(cfg_type == MCFG_TYPE_SW && mcfg_proc_is_excluded_efs(filePath, strlen(filePath)))
  {
    MCFG_MSG_SPRINTF_1("exclude EFS File: %s, skip deletion...", filePath);
...
boolean mcfg_read_large_tlv(mcfg_large_tlv* dest, uint8* src)
{
  boolean success = TRUE; // here for future use in case we want to indicate errors

  memscpy(&(dest->hdr.mcfg_nv_item_type), sizeof(dest->hdr.mcfg_nv_item_type), src, sizeof(dest->hdr.mcfg_nv_item_type));
  src += sizeof(dest->hdr.mcfg_nv_item_type);
  memscpy(&(dest->hdr.mcfg_nv_item_length), sizeof(dest->hdr.mcfg_nv_item_length), src, sizeof(dest->hdr.mcfg_nv_item_length));
  src += sizeof(dest->hdr.mcfg_nv_item_length);
  dest->value = src;
...
#ifdef FEATURE_MCFG_FEATURETAG
    if (mcfg_processing_status_get() == PROCESSING_OFF)
    {
      if (mcfg_item_type_int != MCFG_INT_FEATURE_ITEM)
      {
#ifdef MCFG_DEBUG
          MCFG_MSG_MED_1("FeatureTag section set item type to SW_ONLY during cleanup. Proc Status == PROCESSING_OFF: %d", 
                         mcfg_processing_status_get() == PROCESSING_OFF);
#endif /* MCFG_DEBUG */
...
#ifdef FEATURE_MCFG_FEATURETAG
      case MCFG_INT_FEATURE_ITEM:
        MCFG_MSG_LOW_2("Encountered feature item type 0x%02x, index %d", 
                       mcfg_item_type_int, i);

        feat_data_len = mcfg_item_prefix.item_length - sizeof(mcfg_item_prefix);
        /* Control processing state based on feature tags representing active features
           Active features are expected to remain the same between MBN activation and deactivation.*/
        if(!mcfg_process_feature(subs_id, ram_cust_data_ptr, feat_data_len, 
        ...
        ram_cust_data_ptr += feat_data_len;
        break;
#endif/* FEATURE_MCFG_FEATURETAG */
        ...
        case MCFG_INT_DELETE_LARGE_EFS_FILE:
          ...
          if(mcfg_item_type_int == MCFG_INT_LARGE_EFS_FILE ||
             mcfg_item_type_int == MCFG_INT_STATIC_WO_LARGE_EFS_FILE ||
             mcfg_item_type_int == MCFG_INT_MUXD_LARGE_EFS_FILE ||
             mcfg_item_type_int == MCFG_INT_DELETE_LARGE_EFS_FILE)
          {
            if(MCFG_NV_STATUS_OK != mcfg_delete_efs_file(cfg_type, ram_cust_data_ptr, mcfg_item_prefix.attrib, TRUE))
            {
              mcfg_nvefs_update_status = FALSE;
...
FUNCTION  mcfg_nvs_are_equal() 

DESCRIPTION
  This compares two NVs are the same.
...
===========================================================================*/
static boolean mcfg_nvs_are_equal(
  const void  *e1,
  const void  *e2
)
{
  const nv_items_enum_type  *p_nv1 = (const nv_items_enum_type *) e1;
  const nv_items_enum_type  *p_nv2 = (const nv_items_enum_type *) e2;
...
FUNCTION  mcfg_efs_are_equal() 

DESCRIPTION
  This compares two EFSs have the same path.
...
===========================================================================*/
static boolean mcfg_efs_are_equal(
  const void  *e1,
  const void  *e2
)
{
  const char  *p_efs1 = (const char *) e1;
  const char  *p_efs2 = (const char *) e2;

  return strcmp(p_efs1, p_efs2) == 0;
}

/*===========================================================================

FUNCTION  mcfg_proc_add_excluded_nv() 

DESCRIPTION
  This adds NV item into the list for excluding.

DEPENDENCIES
  None

RETURN VALUE
  boolean - TRUE if added, otherwise FALSE.

SIDE EFFECTS
  None

===========================================================================*/
boolean mcfg_proc_add_excluded_nv(nv_items_enum_type *eItem)
{
	return mre_set_add(mcfg_exclude_list.nv_list, eItem);
}

/*===========================================================================

FUNCTION  mcfg_proc_is_excluded_nv() 

DESCRIPTION
  This checks NV item should be excluded from processing or not.

DEPENDENCIES
  None

RETURN VALUE
  boolean - TRUE if excluded, otherwise FALSE.

SIDE EFFECTS
  None

===========================================================================*/
boolean mcfg_proc_is_excluded_nv(nv_items_enum_type eItem)
{
  nv_items_enum_type eItem_local;

  if( mcfg_exclude_list.status == MCFG_EXCLUDE_STATUS_NOTFOUND)
	{
  	return FALSE;
	}

  eItem_local = eItem;
  return (mre_set_find(mcfg_exclude_list.nv_list, mcfg_nvs_are_equal, &eItem_local)!=NULL);
}

/*===========================================================================

FUNCTION  mcfg_proc_add_excluded_efs() 

DESCRIPTION
  This adds efs file path into the list for excluding.
...
===========================================================================*/
boolean mcfg_proc_add_excluded_efs(char *efs_path, size_t len)
{
	return mre_set_add(mcfg_exclude_list.efs_list, efs_path);
}

/*===========================================================================

FUNCTION  mcfg_proc_is_excluded_efs() 

DESCRIPTION
  This checks EFS file path should be excluded from processing or not.
...
===========================================================================*/
boolean mcfg_proc_is_excluded_efs(char *efs_path, size_t len)
{
  if( mcfg_exclude_list.status == MCFG_EXCLUDE_STATUS_NOTFOUND)
	{
  	return FALSE;
	...
	return (mre_set_find(mcfg_exclude_list.efs_list, mcfg_efs_are_equal, efs_path) != NULL);
}

/*===========================================================================

FUNCTION  mcfg_exclude_nvefs() 

DESCRIPTION
  This processed modem config data to find out the NV/EFS item marked for exclusion
...
)
{
  ...
  int i;
  mcfg_nv_item_hdr_type mcfg_nv_item_data_hdr;
  nv_items_enum_type *p_nv;
	mcfg_tlv            efs_item_header;
	char 							 *file_path;

  boolean excluding = FALSE;

  MCFG_MSG_MED_1("In mcfg_exclude_nvefs, num_mcfg_items = %d", (int)num_mcfg_items);
  ...
  {
    /* Parse the MCFG Item Prefix for further processing */
    mcfg_item_type_int = mcfg_parse_item_prefix(cfg_type, 
                                                ram_cust_data_ptr, 
                                                &mcfg_item_prefix, 
                                                MCFG_REFRESH_TYPE_SLOT_N_SUBS, 
                                                MCFG_SUB_ID_FIRST);
    ...
    if (mcfg_item_prefix.sp_ops & U_ITEM_OPS_ATTRIB_EXCLUDE)
    {
      MCFG_MSG_HIGH_2("exclude %d with sp_ops %x", i, mcfg_item_prefix.sp_ops);
			excluding = TRUE;
    }
		else
		{
    ...
    switch (mcfg_item_type_int) 
    {
      ...
      case MCFG_INT_NV_ITEM:
      case MCFG_INT_DELETE_NV_ITEM:
          /* Copy out the hdr as it may not be aligned for uint16*/
          memscpy(&mcfg_nv_item_data_hdr, sizeof(mcfg_nv_item_hdr_type),
                  ram_cust_data_ptr, sizeof(mcfg_nv_item_hdr_type)); 

			    p_nv = mcfg_malloc(sizeof(nv_items_enum_type));
				  if(p_nv)
				  {
				  	*p_nv = (nv_items_enum_type) mcfg_nv_item_data_hdr.mcfg_nv_item_type;
	          MCFG_MSG_HIGH_3("mcfg_exclude_nvefs NV item type % index_num %d item %d", 
	                          mcfg_item_type_int, i, (int)*p_nv);
						if(!mcfg_proc_add_excluded_nv(p_nv))
					  {
					    mcfg_free(p_nv);
						}
				  }
        ...
        case MCFG_INT_MUXD_EFS_FILE:
        case MCFG_INT_DELETE_EFS_FILE:
          MCFG_MSG_LOW_2("Encountered exclude EFS File type %d index %d", 
                         mcfg_item_type_int, i);
					
					mcfg_read_tlv(&efs_item_header, ram_cust_data_ptr);
												  
 					file_path = mcfg_malloc( sizeof(uint8)* MCFG_FILE_PATH_LENGTH );
					if(file_path)
					{
						memscpy(file_path, MCFG_FILE_PATH_LENGTH, 
							efs_item_header.value, efs_item_header.hdr.mcfg_nv_item_length); 
						MCFG_MSG_SPRINTF_3("mcfg_exclude_nvefs NV item type % index_num %d path %d", 
														mcfg_item_type_int, i, file_path);
						if(!mcfg_proc_add_excluded_efs(file_path, strlen(file_path)))
						{
							mcfg_free(file_path);
						}
					}
        ...
        case MCFG_INT_MUXD_SW_ONLY:
	      case MCFG_INT_SW_ONLY:
					MCFG_MSG_LOW("mcfg_exclude_nvefs MCFG_INT_SW_ONLY");
					break;

	      default:
	        /* Unknown - Err fatal should never be here */
	        MCFG_ERR_FATAL("mcfg_exclude_nvefs Parsing Error %d", 
	                       mcfg_item_type_int, 0, 0); 
	        break;
      }
...
FUNCTION  mcfg_proc_exclude_list_generate() 

DESCRIPTION
  This generates excluded list from activated HW MBN
...
===========================================================================*/
boolean mcfg_proc_exclude_list_generate()
{
  if (mcfg_exclude_list.status == MCFG_EXCLUDE_STATUS_UNINIT)
  {

    mcfg_config_id_s_type	mcfg_id;
    mcfg_config_s_type 		mcfg_current_buf;
    mcfg_config_s_type *p_mcfg_buf = NULL;
    ...
    do
    {
      MCFG_MSG_MED("Generating exclude_list...");

      /* LOAD AND AUTHENTICATE MODEM CFG IMAGE */
      if (MCFG_ERR_NONE != modem_cfg_load_and_auth_mcfg(
          MCFG_TYPE_HW, &mcfg_id, p_mcfg_buf,
          FALSE, MCFG_SUB_ID_FIRST))
      {
        MCFG_MSG_ERROR("Load & Authenticate fail for excluding HW MBN");
        break;
      }
      ...
      if (mcfg_seg_ptr == NULL)
      {
        MCFG_MSG_ERROR("Cannot find MCFG_SEG");
        break;
      }
      ...
      /* Validate the MCFG Header Information and get the number of NV items from this data */
      num_mcfg_items = validate_mcfg_header(MCFG_TYPE_HW, ram_cust_data_ptr, MCFG_SUB_ID_FIRST,
                                            MCFG_REFRESH_TYPE_SLOT_N_SUBS);
      ...
      ram_cust_data_ptr = mcfg_seg_ptr + MCFG_NV_DATA_OFFSET;
      if (mcfg_exclude_nvefs(MCFG_TYPE_HW, num_mcfg_items, ram_cust_data_ptr))
      {
        MCFG_MSG_HIGH("MCFG NVEFS exclude found!");
        mcfg_exclude_list.status = MCFG_EXCLUDE_STATUS_FOUND;
      }
      else
      {
        MCFG_MSG_HIGH("MCFG NVEFS exclude not found!");
        mcfg_exclude_list.status = MCFG_EXCLUDE_STATUS_NOTFOUND;
      }
  ...
  return (mcfg_exclude_list.status != MCFG_EXCLUDE_STATUS_UNINIT); 
}

/*===========================================================================

FUNCTION  mcfg_proc_exclude_list_init() 

DESCRIPTION
  This initializes excluded list at task starts
...
===========================================================================*/
boolean mcfg_proc_exclude_list_init()
{
  memset(&mcfg_exclude_list, 0, sizeof(mcfg_exclude_list));

  mcfg_exclude_list.status = MCFG_EXCLUDE_STATUS_UNINIT;
  mcfg_exclude_list.nv_list = mre_set_new(sizeof(uint32),
  										  5,
  										  MRE_SET_NO_MAX_SIZE,
  										  NULL,
  										  NULL );
  mcfg_exclude_list.efs_list = mre_set_new(MCFG_FILE_PATH_LENGTH,
  										  5,
  										  MRE_SET_NO_MAX_SIZE,
  										  NULL,
  										  NULL );
