...
/*===========================================================================
  ...
  $Header: //components/rel/mcfg_fwk.mpss/7.8.1.1/mcfg_proc/src/mcfg_task.c#1 $ $DateTime: 2021/04/08 00:04:19 $ $Author: pwbldsvc $
...
#ifdef FEATURE_MCFG_DATA_API_CLEANUP_SUPPORT
#define  MODEM_CFG_DATA_READY_SIG        0x0040      /* Data RCEVT signal   */
#endif

#define  MCFG_RCEVT_NAME_MMGSDI_READY    "MM_GSDI: READY"
#define  MCFG_RCEVT_NAME_CM_READY        "cm:ready"
#ifdef FEATURE_MCFG_DATA_API_CLEANUP_SUPPORT
#define  MCFG_RCEVT_NAME_DATA_READY      "DS:READY"
#endif
...
extern void     mcfg_reclaim_SBL_MBA();
...
#ifdef FEATURE_MCFG_DATA_API_CLEANUP_SUPPORT
static boolean mcfg_data_ready_for_cleanup = FALSE;
#endif

static timer_type mcfg_dog_hb_timer;
#define MCFG_DOG_HB_TIMER_TIMEOUT (2) //in seconds
  ...
  while ((cmd = (mcfg_task_cmd_s_type *) q_get(&mcfg_task_cmd_q)) != NULL)
  {
    switch (cmd->cmd_id)
    {
      ...
      case MCFG_TASK_CMD_DISCOVER:
        mcfg_remote_discover(MCFG_RFS_PATH_DISCOVER);
        break;
#endif

      case MCFG_TASK_CMD_SVC:
        mcfg_svc_cmd_proc(cmd);
        break;
...
)
{
  ...
  mcfg_refresh_init();
#endif
	mcfg_proc_exclude_list_init();

#ifdef FEATURE_MCFG_FEATURETAG
  mcfg_feat_init();
#endif

/* define timer for DOG HB */
timer_def(&mcfg_dog_hb_timer, NULL, NULL, 0, NULL, 0);
  ...
  /*Register callback with NPA to reclaim SBL. NPA will notify when APPS ip bootedup */
  npa_resource_available_cb("/ssr/smp2p/inited",mcfg_reclaim_SBL_MBA,NULL);
#endif
  mcfg_dynrec_init();
...
#ifdef FEATURE_MCFG_DATA_API_CLEANUP_SUPPORT
  /* Rcevt registration for Data ready event */ 
  rcevt_sigex.signal = modem_cfg_os_self();
  rcevt_sigex.mask = MODEM_CFG_DATA_READY_SIG;
  if (RCEVT_NULL == rcevt_register_sigex_name(
       MCFG_RCEVT_NAME_DATA_READY, MODEM_CFG_OS_RCEVT_SIGEX_TYPE, 
       &rcevt_sigex))
  {
    MCFG_MSG_ERROR("fail to register rcevt for Data ready");
    return;
  }
  sigs_mask ^= MODEM_CFG_DATA_READY_SIG;
#endif
...
#ifdef FEATURE_MCFG_DATA_API_CLEANUP_SUPPORT
    if (received_sigs & MODEM_CFG_DATA_READY_SIG)
    {
      /* Process data ready rcevt signal */
      modem_cfg_os_clr_sigs (mcfg_tcb, MODEM_CFG_DATA_READY_SIG);
      if(!rcevt_unregister_sigex_name(
         MCFG_RCEVT_NAME_DATA_READY, MODEM_CFG_OS_RCEVT_SIGEX_TYPE, 
         &rcevt_sigex))
      {
        MCFG_MSG_ERROR("fail to unregister rcevt for Data READY");
      }
      sigs_mask &= ~MODEM_CFG_DATA_READY_SIG;
      mcfg_data_ready_for_cleanup = TRUE;
    }
#endif
...
{
  if(!timer_is_active(&mcfg_dog_hb_timer))
  {
    MCFG_MSG_LOW("mcfg_task_dog_hb_report: Calling dog_hb_report");
    dog_hb_report(modem_cfg_dog_rpt_var);
    timer_set(&mcfg_dog_hb_timer, MCFG_DOG_HB_TIMER_TIMEOUT, 0, T_SEC);
  }
}
...
/*===========================================================================
  ...
  FUNCTION mcfg_task_data_cleanup

  DESCRIPTION
    This function checks task status and calls data cleanup API.
...
===========================================================================*/
void mcfg_task_data_cleanup
(
  sys_modem_as_id_e_type asid
)
{
  struct fs_stat stat;
  if(!((efs_lstat("/nv/item_files/mcfg/ignore_data_cleanup", &stat) == -1) && (efs_errno == ENOENT)))
  {
    MCFG_MSG_HIGH("Ignore data cleanup enabled");
    return;
  }

  if(!mcfg_bootup_completed)
  {
    MCFG_MSG_HIGH_1("Delete Data profile from efs before data task starts, ASID %d", asid);
    //efs profile only cleanup
    ds3g_mcfg_cleanup_efs(asid);
    
    return;
  }

  while (!mcfg_data_ready_for_cleanup)
  {
    //Wait for data ready rcevt signal
    MCFG_MSG_HIGH("Wait for data ready rcevt signal");
    if(modem_cfg_os_wait(MODEM_CFG_DATA_READY_SIG) & MODEM_CFG_DATA_READY_SIG)
    {
      //Keep the signal so it will be cleared and unregister rcevt in main routine.
      MCFG_MSG_HIGH("data ready");
      mcfg_data_ready_for_cleanup = TRUE;
    }
  }

  MCFG_MSG_HIGH_1("Delete Data profile from efs after data ready, ASID %d", asid);
  ds3g_mcfg_cleanup(asid);
}
#endif /* FEATURE_MCFG_DATA_API_CLEANUP_SUPPORT */
