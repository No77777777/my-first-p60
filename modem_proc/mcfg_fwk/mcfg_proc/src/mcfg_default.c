...
/*===========================================================================
  ...
  $Header: //components/rel/mcfg_fwk.mpss/7.8.1.1/mcfg_proc/src/mcfg_default.c#1 $ $DateTime: 2021/04/08 00:04:19 $ $Author: pwbldsvc $ 
...
/* -------------------------------------------------------------------- */
  ...
  do
  {
    ...
    if (mcfg_utils_space_available(type, &config, &available_space, TRUE) != MCFG_ERR_NONE)
    {
    ...
    if (!mcfg_update_nvefs(type, num_mcfg_items, def_mcfg_data_ptr, subs_id,
                          MCFG_REFRESH_TYPE_SLOT_N_SUBS, FALSE ) )
    {
