...
$Header: //components/rel/mcfg_fwk.mpss/7.8.1.1/mcfg_proc/src/mcfg_refresh.c#1 $
...
/*===========================================================================
  ...
  FUNCTION  mcfg_refresh_abort_all

  DESCRIPTION
    This function is used to abort all current refresh processes
  ...
  RETURN VALUE
   returns the mask of slots that refreshes get successfully aborted

  SIDE EFFECTS
    None

===========================================================================*/
mcfg_refresh_index_mask_type mcfg_refresh_abort_all
(
  void
)
{
  ...
  mcfg_refresh_index_mask_type aborted_mask = 0;

  for(slot_index = MCFG_REFRESH_INDEX_0; slot_index < MCFG_REFRESH_INDEX_MAX; slot_index++)
  {
    if(mcfg_refresh_is_ongoing(1<<slot_index))
    {
      if(mcfg_refresh_abort(slot_index))
      {
        aborted_mask += (1<<slot_index);
        MCFG_MSG_HIGH_1("refreshing for sloti %d aborted", slot_index);
      }
    }
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (cmd != NULL)
  {
    ...
    {
      MCFG_MSG_ERROR_1("invalid iccid %ld", uimdata->iccid.data_len);
      refresh_cmd_data->data.uimdata.iccid.data_len = 0;
      refresh_cmd_data->data.uimdata.iccid.data_ptr = NULL;
    }
...
/*-------------------------------------------------------------------------*/
  ...
  else
  {
    ...
    if (orig == MCFG_REFRESH_ORIG_APP)
    {
      ...
      mcfg_svc_cmd_cached_activate_config_ind(slot, MCFG_ERR_NONE);
#endif
    }
    else if (orig == MCFG_REFRESH_ORIG_PDC)
    {
      ...
      mcfg_svc_cmd_config_change_ind(MCFG_TYPE_SW, &mcfg_refresh_cache.slotdata[slot].config_id);
#endif
    }
...
/*-------------------------------------------------------------------------*/
  ...
  if (status != MCFG_REFRESH_SUCCESS)
  {
    ...
    if (orig == MCFG_REFRESH_ORIG_APP)
    {
      ...
      mcfg_svc_cmd_cached_activate_config_ind(slot, MCFG_ERR_REFRESH_FAILED);
#endif
    }
    else if (orig == MCFG_REFRESH_ORIG_PDC)
    {
...
/*-------------------------------------------------------------------------*/
     ...
     || !mcfg_uim_nv_autoselect_enabled((mcfg_slot_index_type_e_type)uimdata->slot_index))
  {
    ...
    {
      if(!mcfg_uim_update_iccid((mcfg_slot_index_type_e_type)uimdata->slot_index, card_iccid))
      {
        MCFG_MSG_MED("no update to ICCID with auto selection disabled");
      }
    }
  }
  else
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(ph_cmd_err == CM_PH_CMD_ERR_NOERR)
  {
    mcfg_refresh_set_state(slot_index, 
          ...
          ph_cmd_err == CM_PH_CMD_ERR_INVALID_SUBS_ID ||
          ph_cmd_err == CM_PH_CMD_ERR_INVALID_SUBSCRIPTION_P)
  {
    oprt_mode = cm_ph_get_oprt_mode();
    MCFG_MSG_ERROR_3("Aborting tear down, for subi %d, oprt_mode = %d, ph_cmd_err=%d", 
      sub_index, oprt_mode, ph_cmd_err);
    mmgsdi_continue_after_nv_refresh(
    ...
    case MCFG_REFRESH_CMD_CM_LPM:    
      ...
      else
      {
        ...
        for (sloti = 0; sloti < MCFG_NUM_OF_SLOT_INDEX; sloti++)
        {
          ...
          mcfg_svc_cmd_cached_activate_config_ind(sloti, MCFG_ERR_REFRESH_FAILED);
        }
      }
      break;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  while (refresh_evt_node != NULL)
  {
    ...
    refresh_evt_node = list_peek_next(&(mcfg_refresh_cache.event_client_list), 
      ...
      mcfg_svc_cmd_generate_refresh_event_ind(mcfg_event, slot, sub);
    #endif
}
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  else if (prev_state == MCFG_REFRESH_STATE_UPDATING_SLOT || prev_state == MCFG_REFRESH_STATE_UPDATING_SUBS)
  {
    mcfg_refresh_cache.slotdata[slot_index].cb_count = 0;
  }
  ...
  MCFG_MSG_MED("APP centric refreshing all");
    ...
    for (sloti = 0; sloti < MCFG_NUM_OF_SLOT_INDEX; sloti++)
    {
      ...
      else
      {
        ...
        mcfg_svc_cmd_cached_activate_config_ind(sloti, MCFG_ERR_NONE);
      }
    }
