...
$Header: //components/rel/mcfg_fwk.mpss/7.8.1.1/mcfg_proc/src/mcfg_multi_mbn.c#1 $
...
)
{
  ...
  if (mcfg_utils_get_active_config(type, &active_id, MCFG_SUB_ID_FIRST))
  {
    ...
    if(MCFG_ERR_NONE != mcfg_utils_delete_config(type, &active_id, MCFG_STORAGE_MASK_ALL))
    {
  ...
  else
  {
      ...
      for (i = 0; i < num_of_id; i++)
      {
        ...
        if ((cur_version & MCFG_VER_CARRIER_MAJOR_MASK) == (version & MCFG_VER_CARRIER_MAJOR_MASK))
        {
          ...
          if(MCFG_ERR_NONE != mcfg_utils_delete_config(type, &id_list[i], storage_mask))
          {
            MCFG_MSG_ERROR("Fail to delete config");
          }
        }
...
)
{
  ...
  if(!replace_config_based_on_version)
  {
    return MCFG_ERR_NONE;
  }
  ...
  if (type == MCFG_TYPE_HW)
  {
    /* Activate new HW MBN, deactivate/delete existing HW MBN if any  */
    return mcfg_multi_mbn_replace_config(type, &id);
  }
  ...
  return mcfg_multi_mbn_replace_config_by_version(type, version, (1 << storage), &id);
...
)
{
  ...
  mcfg_large_tlv mbn_large_item_body;
  ...
  for (i = 1; i < num_of_mbns; i++) 
  {
    ...
    switch (mcfg_item_type_int) 
    {
      case MCFG_INT_EFS_FILE:
        ...
        else
        {
          ...
          else
          {
            ...
            {
              /* Replace only HW MBN at boot-up time */
              if (pHeaderInfo->mcfg_type == MCFG_TYPE_HW)
              {
            ...
            {
              mcfg_multi_mbn_add_config(pHeaderInfo->mcfg_type, &config, MCFG_STORAGE_LOCAL, FALSE);
            } 
      ...
      case MCFG_INT_LARGE_EFS_FILE:
        ...
        mcfg_read_tlv(&mbn_item_header, pData);
        // read mbn contents data
        mcfg_read_large_tlv(&mbn_large_item_body, pData + mbn_item_header.hdr.mcfg_nv_item_length + MCFG_TL_SIZE_BYTES);

        if (!mcfg_verify_large_efs_item_contents(&mbn_item_header, &mbn_large_item_body)) 
        {
          MCFG_MSG_ERROR("MBN File item format incorrect");
        }
        else
        {
          config.config_len = mbn_large_item_body.hdr.mcfg_nv_item_length;
          if (config.config_len < 0 || config.config_len > MCFG_CONFIG_SIZE_MAX)
          {
            MCFG_MSG_ERROR_1("Invalid config size %x", config.config_len);
          }
          ...
          {
            config.config_addr = mbn_large_item_body.value;
            config.comp_status = MCFG_COMP_STATUS_NONE;
            ...
            if (!pHeaderInfo)
            {
              MCFG_MSG_ERROR_1("Invalid header %p", mbn_large_item_body.value);
            }
            else if (bootup)
            {
              /* Replace only HW MBN at boot-up time */
            ...
            else
            {
               mcfg_multi_mbn_add_config(pHeaderInfo->mcfg_type, &config, 
                                     MCFG_STORAGE_LOCAL, (pHeaderInfo->mcfg_type == MCFG_TYPE_HW));
            } 
  ...
  do
  {
    ...
    if (mulit_mbn_digest_available)
    {
                                          ...
                                          0, MCFG_SUB_ID_FIRST, prev_multi_mbn_digest, MCFG_DIGEST_SIZE))
       {
         ...
         {
           MCFG_MSG_ERROR_1("prev and curr multi mbn digest version is same for cfg_type : (%d)!", type);
           processing_status = MODEM_CFG_UPDATE_NOT_REQUIRED;
           break;
         }
       }
    ...
    if (!bootup)
    {
      //Abort any ongoing refresh on all subs before start processing the MultiMBN.
      mcfg_refresh_abort_all();

      // Delete existing mbns
      for (type = MCFG_TYPE_FIRST; type < MCFG_NUM_OF_TYPES; type++)
      {
        if (MCFG_ERR_NONE != mcfg_utils_delete_all(type, MCFG_STORAGE_MASK_ALL, TRUE))
        {
...
#else
    //Trigger MBN Activation or Reselect command, if multiMBN got processed successfully
    if(processing_status == MODEM_CFG_PROC_SUCCESS)
    {
      MCFG_MSG_HIGH("Mulit mbn processing - DONE");
    
      if(mcfg_utils_selected_config_available(MCFG_TYPE_HW, MCFG_SUB_ID_FIRST))
      {
