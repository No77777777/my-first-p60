...
/*===========================================================================
  ...
  $Header: //components/rel/mcfg_fwk.mpss/7.8.1.1/mcfg_segment/src/mcfg_mmu.c#2 $
  $DateTime: 2021/04/26 21:55:36 $


  when        who     what, where, why
  ---------   ---     ---------------------------------------------------------------------------
  09/007/2017 skotalwa CR 2044088: Framework to reclaim FS3
  06/07/17    skotalwa CR 2055351: Framework to reclaim SBL memory into heap
  01/11/17    skotalwa CR 1110341 : Crash scope not able to load MBA memory region
...
/*------------------------------------------------------------------*
 * data type for mba/SBL reclaiming                                     *
 * 
 *------------------------------------------------------------------*/
#ifdef FEATURE_SBL_MBA_RECLAIM

#define  PHY_POOL	"PHY_POOL"
uint32   mcfg_mmu_MBA_VA = 0;
#ifndef  SBL_MBA_RECLAIM_SIZE
#define  SBL_MBA_RECLAIM_SIZE  0x180000
#endif
extern mem_heap_type modem_mem_heap;
void     mcfg_reclaim_SBL_MBA();
uint32   SBL_START_ADDRESS = SBL_PHYS_ADDR;
uint32   SBL_MBA_RECLAIM_MEM_SIZE = SBL_MBA_RECLAIM_SIZE;
mcfg_reclaim_status_t mcfg_reclaim_stat = MCFG_RECLAIM_INIT;

typedef void (*perf_err_cb_ptr)(void);
extern void perf_err_cb_register_in_pre_STM(perf_err_cb_ptr cb);
extern void* crashdump_overlay_mem_alloc(int size, int align);
void mcfg_overlay_SBL_region_CLADE();
uint32 SBL_size =0x180000;
uint32 align_size = 0x1000;
uint32 *v_address;
void mcfg_mmu_add_SBL_entery(void);
qurt_addr_t sbl_overlay_v_add =0;
qurt_addr_t sbl_overlay_p_add =0;
qurt_addr_t sbl_v_add =0;
qurt_addr_t sbl_p_add =0;


#endif /*FEATURE_SBL_MBA_RECLAIM*/

/*------------------------------------------------------------------*
 * data type for FS3 reclaiming                                     *
 * 
 *------------------------------------------------------------------*/
#ifdef FEATURE_FS3_RECLAIM
#define  FS3_PHY_POOL "FS3_PHY_POOL"
#ifndef  FS3_RECLAIM_SIZE
#define  FS3_RECLAIM_SIZE  0x100000
#endif 
uint32   mcfg_fs3_mmu_va=0;
uint32   FS3_START_ADDRESS = FS3_PHYS_ADDR;
uint32   FS3_RECLAIM_MEM_SIZE =  FS3_RECLAIM_SIZE;
extern mem_heap_type modem_mem_heap;
void      fs3_reclaim();
mcfg_FS3_reclaim_status_t mcfg_FS3_reclaim_stat = MCFG_FS3_RECLAIM_INIT;
#endif
/*------------------------------------------------------------------*
 * data type for feature reclaiming shared location between boot    *
 * and modem
 *------------------------------------------------------------------*/
#ifdef FEATURE_SHARED_BOOT_MODEM_MEMORY_RECLAIM

#define  SHARED_PHY_POOL	"SHARED_PHY_POOL"
uint32   shared_mem_pool_VA = 0;
#ifndef  SHARED_MEM_POOL_RECLAIM_SIZE
#define  SHARED_MEM_POOL_RECLAIM_SIZE  0x300000
#endif
extern mem_heap_type modem_mem_heap;

uint32   MEM_POOL_START_ADDRESS = SHARED_BOOT_MODEM_MEMORY_RECLAIM_PHYS_ADDR;
int mcfg_mmu_mem_pool_VA = 0;
#endif /*FEATURE_SHARED_BOOT_MODEM_MEMORY_RECLAIM*/
/*------------------------------------------------------------------*
...
mcfg_mmu_update_status_t mcfg_mmu_update(void)
{
...
#ifndef FEATURE_CLADE2_MODEM_HEAP  
...
#else
  section_info = (mem_heap_section*)modem_mem_alloc(MEM_HEAP_SECTIONS_MAX*sizeof(mem_heap_section),MODEM_MEM_CLIENT_UNCOMPRESSED_HEAP); 
  if(!section_info)
  {
    mcfg_mmu_update_last_error = MCFG_MMU_UPDATE_MALLOC_FAIL;
    return mcfg_mmu_update_last_error;
 ...
 section_info = &modem_internal_mem_heap.sections[0];
 #endif
...
void mcfg_mmu_update_init(void)
{
  struct fs_stat sbuf;
  #ifdef FEATURE_SBL_MBA_RECLAIM
    /*FATAL_ERR callback- overlay SBL contents on CLADE*/
    perf_err_cb_register_in_pre_STM(mcfg_overlay_SBL_region_CLADE); 
  #endif
  if(mcfg_mmu_update() != MCFG_MMU_UPDATE_SUCCESS)
  {
   ...
   if (efs_stat("/", &sbuf)==0)
   {
    mcfg_FS3_reclaim_stat = MCFG_FS3_EFS_SYNC_SUCCESS;
    fs3_reclaim();
   }
   #endif 
#ifdef FEATURE_SBL_MBA_RECLAIM
  mcfg_mmu_add_SBL_entery();
...
FUNCTION mcfg_overlay_SBL_MBA_region_CLADE

DESCRIPTION
  During crash SBL contents will get overwritten. This function will overlay contents of SBL on clade region. 
...
===========================================================================*/
void mcfg_overlay_SBL_region_CLADE(void)
{
  v_address = (uint32*) crashdump_overlay_mem_alloc(SBL_size,align_size);
  memscpy((void*)v_address, SBL_size, (void *)mcfg_mmu_MBA_VA, SBL_size);
  sbl_overlay_v_add = v_address;
  sbl_overlay_p_add = qurt_lookup_physaddr(sbl_overlay_v_add);
 //*v_address = 0xAAAA;
}
...
FUNCTION mcfg_mmu_add_SBL_entery

DESCRIPTION
  This function adds MMU mapping for SBL-MBA memory region to MMU table. This mapping acts as place holder. AFter NPA call back 
  SBL physical memory will be mapped to this virtual address.  
...
void mcfg_mmu_add_SBL_entery()
{
  ...
  int i;
/* 
  if(E_SUCCESS != mba_xpu_lock_region(SBL_START_ADDRESS, (SBL_PHYS_ADDR+SBL_MBA_RECLAIM_MEM_SIZE), MBA_XPU_PARTITION_RECLAIM_MBA))
  {
     mcfg_mmu_MBA_VA = 1;
     mcfg_reclaim_stat = MCFG_MBA_XPU_LOCK_FAILED;
     MCFG_MSG_HIGH("mba_xpu_lock_region Failed.");
     return;
  }
*/
  if(QURT_EOK != qurt_mem_pool_create(PHY_POOL, (SBL_PHYS_ADDR >> 12),(SBL_MBA_RECLAIM_MEM_SIZE>>12), &mem_pool))
  {
     mcfg_mmu_MBA_VA = 2;
     mcfg_reclaim_stat = MCFG_QURT_POOL_CREATION_FAILED;
  ...
  mem_attribs.ppn = (unsigned int) (SBL_PHYS_ADDR >> 12);

  if(QURT_EOK == qurt_mem_region_create(&mem_region,
                                SBL_MBA_RECLAIM_MEM_SIZE, mem_pool, 
                                &mem_attribs))
  {
     ...
     mcfg_mmu_add_entry((qurt_addr_t)mcfg_mmu_MBA_VA,SBL_MBA_RECLAIM_MEM_SIZE);
     sbl_v_add = (qurt_addr_t)mcfg_mmu_MBA_VA;
     sbl_p_add = mcfg_VA_to_PA(sbl_v_add);


  }
  else
  {
     mcfg_reclaim_stat = MCFG_QURT_MEM_REGION_CREATION_FAILED;
     mcfg_mmu_MBA_VA = 3;
  }
}
/*===========================================================================

FUNCTION mcfg_reclaim_SBL_MBA

DESCRIPTION
  This is a call back functions which will reclaim SBL/MBA memory region into MPSS heap.
...
===========================================================================*/
void mcfg_reclaim_SBL_MBA(void)
{
  uint32 *ptr;
  int i;
  ...
  mem_heap_add_section(&modem_mem_heap,(void *)mcfg_mmu_MBA_VA,(unsigned long)SBL_MBA_RECLAIM_MEM_SIZE);
  mcfg_reclaim_stat = MCFG_RECLAIM_SUCCESS;
 /*
   ptr = (uint32 *)mcfg_mmu_MBA_VA;
...
void fs3_reclaim(void)
{
  ...
  if(E_SUCCESS != mba_xpu_lock_region(FS3_START_ADDRESS,(FS3_START_ADDRESS + FS3_RECLAIM_MEM_SIZE), MBA_XPU_PARTITION_MODEM_EFS))
  {
     mcfg_FS3_reclaim_stat = MCFG_FS3_XPU_LOCK_FAILED;
     MCFG_MSG_HIGH("mba_xpu_lock_region Failed.");
     return;
  }

    if(QURT_EOK != qurt_mem_pool_create(FS3_PHY_POOL, (FS3_START_ADDRESS >> 12),(FS3_RECLAIM_MEM_SIZE>>12), &mem_pool))
  {
     mcfg_FS3_reclaim_stat = MCFG_FS3_QURT_POOL_CREATION_FAILED;
     MCFG_MSG_HIGH("qurt_mem_pool_create Failed.");
     return;
  }
  ...
  qurt_mem_region_attr_set_type (&mem_attribs, QURT_MEM_REGION_SHARED);
  qurt_mem_region_attr_set_bus_attr(&mem_attribs, 1);
  mem_attribs.ppn = (unsigned int) (FS3_PHYS_ADDR >> 12);

  if(QURT_EOK == qurt_mem_region_create(&mem_region,FS3_RECLAIM_SIZE, mem_pool, &mem_attribs))
  {
     qurt_mem_region_attr_get(mem_region,
                            &mem_attribs );

     qurt_mem_region_attr_get_virtaddr(&mem_attribs,
                                     (unsigned int *)&mcfg_fs3_mmu_va);
     
     mem_heap_add_section(&modem_mem_heap,(void *)mcfg_fs3_mmu_va,(unsigned long)FS3_RECLAIM_SIZE);
     mcfg_mmu_add_entry((qurt_addr_t)mcfg_fs3_mmu_va,FS3_RECLAIM_SIZE);
     mcfg_FS3_reclaim_stat = MCFG_FS3_RECLAIM_SUCCESS ;
    /* ptr = (uint32 *)mcfg_fs3_mmu_va;
     for (i =0; i < 100; i++)
     {
        ptr = ptr + 10 ;
        *ptr = 0xABABAB;
        counter_fs3 = counter_fs3+1;
     }*/
     
  }
  else
  {
     mcfg_FS3_reclaim_stat = MCFG_FS3_QURT_MEM_REGION_CREATION_FAILED;
  }
}
#endif/*FEATURE_FS3_RECLAIM*/
#ifdef FEATURE_SHARED_BOOT_MODEM_MEMORY_RECLAIM
/*===========================================================================
...
{
  qurt_mem_pool_t              shared_mem_pool; 
  qurt_mem_region_attr_t       mem_attribs;
  qurt_mem_region_t            mem_region;

  if(QURT_EOK != qurt_mem_pool_create(SHARED_PHY_POOL, (MEM_POOL_START_ADDRESS >> 12),(SHARED_MEM_POOL_RECLAIM_SIZE>>12), &shared_mem_pool))
  {
     mcfg_mmu_mem_pool_VA = 2;
     MCFG_MSG_HIGH("qurt_mem_pool_create Failed.");
     return;
  }
  ...
  qurt_mem_region_attr_set_type (&mem_attribs, QURT_MEM_REGION_SHARED);
  qurt_mem_region_attr_set_bus_attr(&mem_attribs, 1);
  mem_attribs.ppn = (unsigned int) (MEM_POOL_START_ADDRESS >> 12);

  if(QURT_EOK == qurt_mem_region_create(&mem_region,
                                SHARED_MEM_POOL_RECLAIM_SIZE, shared_mem_pool, 
                                &mem_attribs))
  {
     qurt_mem_region_attr_get(mem_region,
                            &mem_attribs );
     ...
     qurt_mem_region_attr_get_virtaddr(&mem_attribs,
                                     (unsigned int *)&shared_mem_pool_VA);
     
     mem_heap_add_section(&modem_mem_heap,(void *)shared_mem_pool_VA,(unsigned long)SHARED_MEM_POOL_RECLAIM_SIZE);
     
    if(mcfg_mmu_update() != MCFG_MMU_UPDATE_SUCCESS)
      {
        MCFG_MSG_HIGH("Modem Config MMU Table Update Failed.");
        mcfg_mmu_mem_pool_VA = 5;
      }
  ...
  {
     MCFG_MSG_HIGH("Create mem region failed.");
  }
