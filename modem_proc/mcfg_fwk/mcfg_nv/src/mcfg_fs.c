...
$Header: //components/rel/mcfg_fwk.mpss/7.8.1.1/mcfg_nv/src/mcfg_fs.c#1 $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  else
  {
    ...
    if (status == MCFG_FS_STATUS_OK)
    {
      ...
      {
        MCFG_MSG_SPRINTF_2("mcfg_fs_write -  sub_id: 0x%0x, filepath %s", sub_id, f_name);
        MCFG_MSG_MED_2("efs_ret %d - efs_errno %d", efs_ret, efs_errno);
        status = MCFG_FS_STATUS_EFS_ERR;
      }
    } /* status == MCFG_FS_STATUS_OK */
  }

  MCFG_MSG_LOW_1("status %d", status);
  return status;
} /* mcfg_fs_write() */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (status != MCFG_FS_STATUS_OK && status != MCFG_FS_STATUS_NOT_ACTIVE)
  {
    MCFG_MSG_MED_2("efs_ret %d - efs_errno %d", efs_ret, efs_errno);
    MCFG_MSG_SPRINTF_2("mcfg_fs_read -  sub_id: 0x%0x, filepath %s", sub_id, f_name);
  }
