...
Version Control
  ...
  $Header: //components/rel/gnss8.mpss/11.1/gnss/os_api/inc/aries_os_api.h#3 $
  $DateTime: 2020/03/06 00:09:22 $
  $Author: pwbldsvc $
...
extern "C" {
  ...
  /* NOTE: that should be used only in pairs, no function return in the middle is allowed */
...
#define C_OS_FLAG_GPM_CM_CD  0x40000000
#define C_OS_FLAG_GPM_CM_NF  0x20000000
#define C_OS_GPM_CM_WAIT_TIME 40
...
*/
  ...
  */
    ...
    #define GNSS_DETERMINISTIC_TIMING_EXIT            \
      ...
      GNSS_GPS_RTC_MUTEX_GIVE;                               \
      if( q_retry_count >= CSC_RETRY_COUNT_MAX )      \
        MSG_3(MSG_SSID_GPSOS, MSG_LEGACY_FATAL, "DT - QURT CSC max retries",0,0,0);    \
    }
 ...
 *   alignment. A buffer allocated using os_MemAlign must be freed using the
 *   API os_MemAlignFree.
 *
 * Parameters:
 *   block_size - Alignment needed (in bytes).
...
/*
 ******************************************************************************
 * os_SleepMs_NonDeferrableSig()
 *
 * This function sleeps for a caller specified number of milliseconds
 * (using non-deferrable timer for sleep, will wake up even in power collapse)
 *
 *Task is resumed when Concurrency manager provides Q6 resources for GNSS.
 *
 * return TRUE if sleep duration is for MAX(40 ms), otherwise FALSE
 * (for example, if system doesn't support non-deferrable sleep)
 ******************************************************************************
 */
boolean os_SleepMs_NonDeferrableSig();

/*
 ******************************************************************************
 *Set the signal to revoke task sleep if they are "waiting" due to Concurrency Manager 
 *API response
 
******************************************************************************
*/
void os_TaskSetSig(uint32);
void os_taskConcMgrSetSig();
