...
/*============================================================================
  ...
  $Header: //components/rel/gnss8.mpss/11.1/gnss/loc_mw/aon/src/aon_api.cpp#1 $ 
...
/* -----------------------------------------------------------------------*//**
@brief
  This function is used by clients to create a AON request for an Outdoor
  Trip Batching Session

@retval  aonOperationStatusType: SUCCESS or FAIL
@param[in] q_ReqId A unique request Id for this location request
@param[in] q_MaxLatency maximum latency threshold within which location updates are needed
@param[in] q_FixTimeout Configures the fix session timeout duration
*//* ------------------------------------------------------------------------*/
aonOperationStatusType aonCreateOtbRequest
(
   uint32   q_ReqId,
   uint32   q_MaxLatency,
   uint32   q_FixTimeout
...
{
   int32 err = 0;
   AonOtbTransac*  pz_OtbTransac = NULL;

   AON_MSG_MED("aonCreateOtbRequest invoked reqId %u, maxLatency %u, fixTimeout %u",
      q_ReqId, q_MaxLatency, q_FixTimeout );
   GNSS_NEW_ARGS(CGPS_LOCMW_MODULE_AON, AonOtbTransac,pz_OtbTransac,
                  q_ReqId, q_MaxLatency, q_FixTimeout,
                  g_aonDatabase, g_aonGmProxy,
                  g_locCb, g_statusCb);
   if (NULL == pz_OtbTransac) 
   {
      err = -1;
   }
   else if (FALSE == g_aonDatabase->addAonTransac(pz_OtbTransac))
   {
      err = -2; 
      GNSS_DELETE(CGPS_LOCMW_MODULE_AON, AonOtbTransac, pz_OtbTransac);
   }
   else if (AON_SUCCEEDED != pz_OtbTransac->start())
   {
      err = -3;
      (void)g_aonDatabase->delAonTransac(pz_OtbTransac); //pz_OtbTransac will get deleted inside.
   }

   if (0 != err)
   {
      LOC_MSG_ERROR("aonCreateDbtRequest failed err = %d, reqID = %d", 
      err, q_ReqId, 0);
      return AON_FAILED;
   }
...
)
{
   ...
   if (NULL != aonTransac)
   {
      ...
      {
         AON_MSG_MED("aonDeleteRequest: delAonTransac aonReqId %d succeed", reqId,0,0);
