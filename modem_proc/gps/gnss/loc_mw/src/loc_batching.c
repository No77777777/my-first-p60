...
/*============================================================================
 ...
 $Header: //components/rel/gnss8.mpss/11.1/gnss/loc_mw/src/loc_batching.c#1 $
 $DateTime: 2019/10/24 01:30:36 $
 $Author: pwbldsvc $

 when       who     what, where, why
 --------   ---     ----------------------------------------------------------
 06/22/18   ak      Adding defensive F3 to debug non duplicable crashes
...
)
{
  ...
  if(curr_write_index < client_ptr->batching_info.max_array_size )
 {
  client_ptr->batching_info.loc_batching_array[curr_write_index] = (*batching_element);
     LOC_MSG_HIGH("loc_batching safely written at Write idx = %d,  max array size = %d ", curr_write_index, client_ptr->batching_info.max_array_size, 0);
  }
  else
  {
     ERR_FATAL(" loc_batching Write idx = %d, exceeds max array size = %d ", 
                                   curr_write_index,
                                   client_ptr->batching_info.max_array_size, 0);
  }
...
@param[in]   pz_LocClient            Loc Client Info
@param[in]   pz_BatchedReport        Batched position report sent to control point


@retval    TRUE           event reporting success
@retval    FALSE          event reporting fail
*//* ------------------------------------------------------------------------*/
boolean loc_batching_report_live_position_event
(
   loc_client_info_s_type                 *pz_LocClient,
   const qmiLocBatchedReportStructT_v02   *pz_BatchedReport
)
{
   qmiLocEventLiveBatchedPositionReportIndMsgT_v02 z_LiveBatchedPosReportInd = { 0 };
   locQmiShimIndInfoStructT z_QmishimIndInfo = { 0 };

   if (NULL == pz_LocClient ||
      NULL == pz_BatchedReport)
   {
      LOC_MSG_ERROR("loc_batching_report_live_position_event: NUll pointer", 0, 0, 0);
      return FALSE;
   }

   /* Send report only if the client registered */
   if (LOC_EVENT_MASK_LIVE_BATCHED_POSITION_REPORT !=
       (pz_LocClient->t_EventRegMask & LOC_EVENT_MASK_LIVE_BATCHED_POSITION_REPORT))
   {
      LOC_MSG_MED("loc_batching_report_live_position_event \
                  Client did not register for QMI_LOC_EVENT_MASK_LIVE_BATCHED_POSITION_REPORT_V02, suppressing",
                  0, 0, 0);
      return FALSE;
   }

   /* fill the position report into the indication */
   z_LiveBatchedPosReportInd.liveBatchedReport = (*pz_BatchedReport);

   z_QmishimIndInfo.p_Msg = &z_LiveBatchedPosReportInd;
   z_QmishimIndInfo.q_MsgLen = sizeof(z_LiveBatchedPosReportInd);
   z_QmishimIndInfo.q_Id = QMI_LOC_EVENT_LIVE_BATCHED_POSITION_REPORT_IND_V02;
...
{
  loc_client_info_s_type *client_info_ptr = locClnt_FindHandle(loc_client_handle);
...
)
{
  ...
  for (i = 0; i < number_of_reads; i++)  
  {
    ...
    LOC_MSG_MED("PULL: updated read (%d) write (%d) indexes, total_occupied = %d", pz_LocClient->batching_info.read_index,
  ...
  LOC_MSG_MED("loc_batching_pull_from_lifo: number of entries returning %d total occupied %d", i, pz_LocClient->batching_info.total_occupied, 0);
...
)
{
   ...
   {
      LOC_MSG_MED("loc_handle_batching_read: NULL pointer", 0, 0, 0);
      return FALSE;
   }
   LOC_MSG_MED("loc_handle_batching_read: number of entries to read %d", pz_ReadFromBatchReq->numberOfEntries, 0, 0);
