...
This section contains comments describing changes made to the module.
 ...
 $Header: //components/rel/gnss8.mpss/11.1/gnss/loc_mw/src/loc_aon.c#1 $
...
static void locAonOtbHandleStatus
(
   loc_client_handle_type  l_ClientHandle,
   aonSessionStatusType    e_AonSessionStatus,
   int32                   l_Idx
);
...
{
  loc_client_info_s_type *pClientInfo = locClnt_FindHandle(locClientHandle);
...
/** locAonSetOtbUnableToTrack
* Setter for unable to track state flag associted with a 
* particaul LOC client handle
* @return : TRUE: Successfully set
*           FALSE: Could not find this client
*/
static boolean locAonSetOtbUnableToTrack
(
   loc_client_handle_type l_ClientHandle,
   boolean v_OtbUnableToTrack
)
{
   ...
   {
      LOC_MSG_ERROR(" locAonSetOtbUnableToTrack: Invalid client Handle %d",l_ClientHandle,0,0);
      return FALSE;
   }

   pz_LocClient->z_OtbInfo.v_UnableToTrackReported = v_OtbUnableToTrack;
   LOC_MSG_MED("locAonSetOtbUnableToTrack : client handle = %d, v_OtbUnableToTrack = %d", l_ClientHandle, v_OtbUnableToTrack, 0);
   return TRUE;
}

/** locAonGetOtbUnableToTrack
* Getter for unable to track state flag associted with a 
* outdoor trip batching LOC client handle
* @return : TRUE: Successfully got the value
*           FALSE: Could not find this client
*/
static boolean locAonGetOtbUnableToTrack
(
   loc_client_handle_type l_ClientHandle,
   boolean *pv_Val
)
{
   ...
   if (NULL == pz_LocClient)
   {
      LOC_MSG_ERROR(" locAonGetOtbUnableToTrack: Invalid client Handle %d", l_ClientHandle, 0, 0);
      return FALSE;
   }
   if (NULL == pv_Val)
   {
      LOC_MSG_ERROR(" locAonGetOtbUnableToTrack: NULL pv_Val", 0, 0, 0);
      return FALSE;
   }
   *pv_Val = pz_LocClient->z_OtbInfo.v_UnableToTrackReported;

   LOC_MSG_MED("locAonGetOtbUnableToTrack : client handle = %d, value = %d", l_ClientHandle, *pv_Val, 0);
   return TRUE;
}
...
)
{
   ...
   else if (eLOC_AON_REQ_OTB == e_ReqType)
   {
      qmiLocBatchedReportStructT_v02 z_BatchPos = {0};
      ...
      {
         LOC_MSG_ERROR("locAonProcessLocationCallback: Could not convert OTB position", 0, 0, 0);
      }
      else if (FALSE == locOtb_BatchPosition(l_ClientHandle,&z_BatchPos))
      {
         LOC_MSG_ERROR("locAonProcessLocationCallback: Could not batch OTB position", 0, 0, 0);
      }
   }
   else
   {
...
)
{
  ...
  qmiLocStartOutdoorTripBatchingIndMsgT_v02   z_StartOtbInd = {0};
  ...
  switch (e_QmiLocBatchType)
  {
    ...
    case eQMI_LOC_OUTDOOR_TRIP_BATCHING_V02:
    {
      z_StartOtbInd.status = e_QmiLocStatus;
      z_ShimIndInfo.p_Msg = &z_StartOtbInd;
      z_ShimIndInfo.q_MsgLen = sizeof(z_StartOtbInd);
      z_ShimIndInfo.q_Id = QMI_LOC_START_OUTDOOR_TRIP_BATCHING_IND_V02;
    }
    break;

    /* Only 2 enums are defined in e_QmiLocBatchType,
    use QMILOCBATCHINGTYPEENUMT_MAX_ENUM_VAL_V02 in loc_qmi_shim.c to send indication
    message for DBT */
...
)
{
   ...
   if (0 == (pz_LocClient->t_EventRegMask & LOC_EVENT_MASK_BATCHING_STATUS))
   {
...
static void locAonSendStartOtbInd
(
   loc_client_handle_type        l_ClientHandle,
   qmiLocStatusEnumT_v02         e_QmiLocStatus
)
{
   ...
   LOC_MSG_MED("locAonSendStartOtbInd: l_ClientHandle %d e_QmiLocStatus %d",
                l_ClientHandle, e_QmiLocStatus,0);
...
)
{
   ...
   else if (eLOC_AON_REQ_OTB == reqType)
   {
      locAonOtbHandleStatus(
                  locClientHandle, 
                  p_statusCBData->sessionStatus, idx);
   }
   else
   {
...
)
{
   boolean  v_RetVal =  FALSE;
   uint32   q_aonReqId = 0;
   int32    l_Err = 0, l_Idx = 0;
   loc_client_info_s_type *pz_LocClient = locClnt_FindHandle(l_ClientHandle);

   if (NULL == pz_LocClient)
   {
      LOC_MSG_ERROR(" locAonOtbStartReq: Invalid client Handle %d",l_ClientHandle,0,0);
      return v_RetVal;
   }

   LOC_MSG_HIGH("Entered locAonOtbStartReq l_ClientHandle %d ", l_ClientHandle,0,0);

   if (NULL == pz_Resp)
   {
      LOC_MSG_ERROR("locAonOtbStartReq: NULL pz_Resp", 0,0,0);
      return v_RetVal;
   }

   pz_Resp->resp.error  = QMI_ERR_NONE_V01;
   pz_Resp->resp.result = QMI_RESULT_SUCCESS_V01;
   if (NULL == pz_StartOtbgReq)
   {
      pz_Resp->resp.error  = QMI_ERR_INVALID_ARG_V01;
      pz_Resp->resp.result = QMI_RESULT_FAILURE_V01;
      LOC_MSG_ERROR("locAonOtbStartReq: NULL pz_StartOtbgReq",0,0,0);
      return v_RetVal;
   }
   else
   {
      pz_LocClient->z_OtbInfo.q_TotalBatchedPosition = 0;
      pz_LocClient->z_OtbInfo.v_BatchDistanceReported = FALSE;
      (void)locOtb_RecalAccumulatedDistance(pz_LocClient);
      if (TRUE == pz_LocClient->z_OtbInfo.v_Active)
      {
         /* Otb session is active, the client only wants to update batch distance */
         if (pz_LocClient->z_OtbInfo.q_MinTimeInterval != pz_StartOtbgReq->minTimeInterval)
         {
            LOC_MSG_ERROR("locAonOtbStartReq: q_MinTimeInterval mismtach %u %u",
                           pz_LocClient->z_OtbInfo.q_MinTimeInterval,
                           pz_StartOtbgReq->minTimeInterval, 0);
            pz_Resp->resp.error  = QMI_ERR_INVALID_ARG_V01;
            pz_Resp->resp.result = QMI_RESULT_FAILURE_V01;
            return v_RetVal;
         }

         pz_LocClient->z_OtbInfo.q_BatchDistance = pz_StartOtbgReq->batchDistance;
         return TRUE;
      }
      else
      {
         pz_LocClient->z_OtbInfo.q_MinTimeInterval = pz_StartOtbgReq->minTimeInterval;
         if (TRUE == pz_StartOtbgReq->fixSessionTimeout_valid)
         {
            pz_LocClient->z_OtbInfo.q_FixSessionTimeout = pz_StartOtbgReq->fixSessionTimeout;
         }
         else
         {
            pz_LocClient->z_OtbInfo.q_FixSessionTimeout = LOC_BATCHING_GNSS_QOS_SESSION_TIMEOUT_MS;
         }

         pz_LocClient->z_OtbInfo.v_BatchAllPos = FALSE;
         if (TRUE == pz_StartOtbgReq->batchAllPos_valid)
         {
            pz_LocClient->z_OtbInfo.v_BatchAllPos = pz_StartOtbgReq->batchAllPos;
         }
      }
   }

   /* pz_LocClient->z_OtbInfo.v_Active is FLASE, so we need to create a new request */
   if (TRUE == locAonCheckIfLocReqIdPresent(l_ClientHandle, LOC_AON_OTB_DEFAULT_REQ_ID,
                                          eLOC_AON_REQ_OTB))
   {
      l_Err = -1;
      pz_Resp->resp.error  = QMI_ERR_INVALID_ID_V01;//requestId already present
   }
   else if (FALSE == locAonAddRequest(l_ClientHandle, LOC_AON_OTB_DEFAULT_REQ_ID, 
            eLOC_AON_REQ_OTB, &q_aonReqId, NULL /* dont care */, &l_Idx))
   {
      l_Err = -2;
      /*Max number of requests reached*/
      pz_Resp->resp.error  = QMI_ERR_INSUFFICIENT_RESOURCES_V01;
   }
   else if (AON_SUCCEEDED != aonCreateOtbRequest(q_aonReqId,
                                                pz_LocClient->z_OtbInfo.q_MinTimeInterval,
                                                pz_LocClient->z_OtbInfo.q_FixSessionTimeout))
   {
      l_Err = -3;
   ...
   if (0 == l_Err)
   {
      ...
      pz_Resp->resp.result = QMI_RESULT_SUCCESS_V01;
      v_RetVal = TRUE;
      pz_LocClient->z_OtbInfo.v_Active = TRUE;
      pz_LocClient->z_OtbInfo.q_BatchDistance = pz_StartOtbgReq->batchDistance;
   }
   else
   {
      pz_Resp->resp.result = QMI_RESULT_FAILURE_V01;
      LOC_MSG_ERROR("locAonOtbStartReq: Internal Error %d ",l_Err,0,0);
      pz_LocClient->z_OtbInfo.q_MinTimeInterval = 0;
      pz_LocClient->z_OtbInfo.q_FixSessionTimeout = 0;
      pz_LocClient->z_OtbInfo.v_BatchAllPos = FALSE;
      v_RetVal = FALSE;
   }
...
)
{
   if (NULL == pz_Resp)
   {
      LOC_MSG_ERROR("locAonOtbStopReq: NULL pz_Resp",0,0,0);
      return;
   }

   if (NULL == pz_StopBatchingReq)
   {
      pz_Resp->resp.error  = QMI_ERR_INVALID_ARG_V01;
      pz_Resp->resp.result = QMI_RESULT_FAILURE_V01;
      LOC_MSG_ERROR("locAonOtbStopReq: NULL pz_StopBatchingReq",0,0,0);
      return;
   }

   if (eLOC_AON_SUCCESS == locAonOtbStopInternal(l_ClientHandle, LOC_AON_OTB_DEFAULT_REQ_ID, FALSE))
   {
      pz_Resp->resp.result = QMI_RESULT_SUCCESS_V01;
      pz_Resp->resp.error = QMI_ERR_NONE_V01;
   }
   ...
   {
      pz_Resp->resp.error  = QMI_ERR_INVALID_OPERATION_V01;
      pz_Resp->resp.result = QMI_RESULT_FAILURE_V01;
      LOC_MSG_ERROR("locAonOtbStopReq: Err Req not found or start pending ",0,0,0);
   }
   return;
}
...
/* -----------------------------------------------------------------------*//**
@brief
  Handle status callback from AON for the outdoor trip batching client
  
@param[in]   l_ClientHandle       Loc Client Handle
@param[in]   e_AonSessionStatus   Aon Session Status
@param[in]   l_Idx                index of the client in g_AonClientReqMap
...
{
   LOC_MSG_MED("locAonOtbHandleStatus: l_ClientHandle = %d AonSessionStatus = %d l_Idx = %d ",
                l_ClientHandle, e_AonSessionStatus, l_Idx);

   if ((LOC_AON_INVALID_INDEX == l_Idx) ||
      (LOC_CLIENT_HANDLE_INVALID == l_ClientHandle))
   {
      LOC_MSG_ERROR("locAonOtbHandleStatus: invalid input",0,0,0);
      return;
   }

   switch (e_AonSessionStatus)
   {
      case AON_START_SUCCEEDED:
      {
         (void)locAonSendStartOtbInd(l_ClientHandle, eQMI_LOC_SUCCESS_V02);
         g_AonClientReqMap[l_Idx].startPending = FALSE;
         break;
      }
      ...
      {
         //force stop the session
         (void)locAonOtbStopInternal(l_ClientHandle, LOC_AON_OTB_DEFAULT_REQ_ID, FALSE);
      
         (void)locAonSendStartOtbInd(l_ClientHandle, eQMI_LOC_GENERAL_FAILURE_V02);
         g_AonClientReqMap[l_Idx].startPending = FALSE;
         break;
      }

      case AON_UNABLE_TO_TRACK:
      case AON_GNL_FAILURE:
      case AON_GNSS_UNAVAIL:
      {
         boolean v_UnableToTrackState = FALSE;

         if (TRUE == locAonGetOtbUnableToTrack(l_ClientHandle, &v_UnableToTrackState))
         {
            // if Unable to track was previously not reported then report and batch an invalid position
            if (FALSE == v_UnableToTrackState) 
            {
               qmiLocBatchedReportStructT_v02 batchPos = {0};
               (void)locOtb_BatchPosition(l_ClientHandle, &batchPos);
               (void)locAonSendBatchingStatusInd(l_ClientHandle, 
                     eQMI_LOC_BATCH_POS_UNAVAILABLE_V02, eQMI_LOC_OUTDOOR_TRIP_BATCHING_V02);
               //set the unable to track for this client
               (void)locAonSetOtbUnableToTrack(l_ClientHandle, TRUE);
            }
            else
            {
               LOC_MSG_MED("locAonOtbHandleStatus: Already reported unavailable, suppressing unable to track",0,0,0);
            }
         }
         else
         {
            LOC_MSG_ERROR("locAonOtbHandleStatus: Could not get the unable to track state",0,0,0);
         }
         break;
      }
      ...
      case AON_GNSS_AVAIL:
      {
         ...
         {
            // if Unable to track was previously reported then report able to track
            if (TRUE == v_UnableToTrackState) 
            {
               (void)locAonSendBatchingStatusInd(l_ClientHandle,
                     eQMI_LOC_BATCH_POS_AVAILABLE_V02, eQMI_LOC_OUTDOOR_TRIP_BATCHING_V02);

               //clear the unable to track for this client
               (void)locAonSetOtbUnableToTrack(l_ClientHandle, FALSE);
            }
            else
            {
               LOC_MSG_ERROR("locAonOtbHandleStatus: got available when no unavailable was sent",0,0,0);
            }
         }
         else
         {
      ...
      default:
      {
         LOC_MSG_HIGH("locAonOtbHandleStatus: Unknown Aon Session status",0,0,0);
         break;
      }
   }
}
