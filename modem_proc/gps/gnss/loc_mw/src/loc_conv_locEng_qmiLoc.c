...
$Id: //components/rel/gnss8.mpss/11.1/gnss/loc_mw/src/loc_conv_locEng_qmiLoc.c#1 $
...
)
{
  ...
  pBapPosition->spoofReportMask_valid = TRUE;
  pBapPosition->spoofReportMask = pLocEngPosition->bap_spoof_report;
...
{
   if((NULL == pConvertedType) || (0 == apDataMask))
   {
      LOC_MSG_ERROR("convertQmiLocToLocEngWiFiApDataMask: apDataMask 0X%x, pConvertedType %p",
                     apDataMask, pConvertedType, 0);
      return FALSE;
   }
   *pConvertedType = 0;

   if(0 != (apDataMask & QMI_LOC_WIFI_APDATA_MASK_AP_TRANSMIT_POWER_V02))
   {
      *pConvertedType |= LOC_ENG_WIFI_APINFO_MASK_AP_TRANSMIT_POWER_V01;
   }

   if(0 != (apDataMask & QMI_LOC_WIFI_APDATA_MASK_AP_ANTENNA_GAIN_V02))
   {
      *pConvertedType |= LOC_ENG_WIFI_APINFO_MASK_AP_ANTENNA_GAIN_V01;
   }

   if(0 != (apDataMask & QMI_LOC_WIFI_APDATA_MASK_AP_SNR_V02))
   {
      *pConvertedType |= LOC_ENG_WIFI_APINFO_MASK_AP_SNR_V01;
   }

   if(0 != (apDataMask & QMI_LOC_WIFI_APDATA_MASK_AP_DEVICE_TYPE_V02))
   {
      *pConvertedType |= LOC_ENG_WIFI_APINFO_MASK_AP_DEVICE_TYPE_V01;
   }

   if(0 != (apDataMask & QMI_LOC_WIFI_APDATA_MASK_AP_RSSI_V02))
   {
      *pConvertedType |= LOC_ENG_WIFI_APINFO_MASK_AP_RSSI_V01;
   }

   if(0 != (apDataMask & QMI_LOC_WIFI_APDATA_MASK_AP_CHANNEL_V02))
   {
      *pConvertedType |= LOC_ENG_WIFI_APINFO_MASK_AP_CHANNEL_V01;
   }

   if(0 != (apDataMask & QMI_LOC_WIFI_APDATA_MASK_AP_ROUNDTRIP_DELAY_V02))
   {
      *pConvertedType |= LOC_ENG_WIFI_APINFO_MASK_AP_ROUNDTRIP_DELAY_V01;
   }

   if(0 != (apDataMask & QMI_LOC_WIFI_APDATA_MASK_AP_ROUNDTRIP_DELAY_ACCURACY_V02))
   {
      *pConvertedType |= LOC_ENG_WIFI_APINFO_MASK_AP_ROUNDTRIP_DELAY_ACCURACY_V01;
   }

   if(0 != (apDataMask & QMI_LOC_WIFI_APDATA_MASK_MOBILE_SNR_V02))
   {
      *pConvertedType |= LOC_ENG_WIFI_APINFO_MASK_MOBILE_SNR_V01;
   }

   if(0 != (apDataMask & QMI_LOC_WIFI_APDATA_MASK_MOBILE_RSSI_V02))
   {
      *pConvertedType |= LOC_ENG_WIFI_APINFO_MASK_MOBILE_RSSI_V01;
   }

   if(0 != (apDataMask & QMI_LOC_WIFI_APDATA_MASK_RSSI_TIMESTAMP_V02))
   {
      *pConvertedType |= LOC_ENG_WIFI_APINFO_MASK_RSSI_TIMESTAMP_V01;
   }

   if(0 != (apDataMask & QMI_LOC_WIFI_APDATA_MASK_MEASUREMENT_AGE_V02))
   {
      *pConvertedType |= LOC_ENG_WIFI_APINFO_MASK_MEASUREMENT_AGE_V01;
   }

   if(0 != (apDataMask & QMI_LOC_WIFI_APDATA_MASK_SERVING_AP_V02))
   {
      *pConvertedType |= LOC_ENG_WIFI_APINFO_MASK_SERVING_AP_V01;
   }

   if(0 != (apDataMask & QMI_LOC_WIFI_APDATA_MASK_FREQUENCY_V02))
   {
      *pConvertedType |= LOC_ENG_WIFI_APINFO_MASK_FREQUENCY_V01;
   }

   if(0 != (apDataMask & QMI_LOC_WIFI_APDATA_MASK_SSID_V02))
   {
      *pConvertedType |= LOC_ENG_WIFI_APINFO_MASK_SSID_V01;
   }
...
@param [in]   pQmiLocWifiApDataReq :  Pointer to QMI LOC inject wifi AP data listpayload.
              numberOfAps : Number of qmiLocWifiApDataStructT_v02 in pQmiLocWifiApDataReq array.
@param [out]  pWifiApDataReq :  Pointer to Loc Eng inject wifi AP data listpayload.
...
/** convertQmiLocInjectWifiApAdditionalDataReq
@brief : convert QMI wifi AP additional scan data request to Loc Eng.
@param [in]   pQmiLocWifiApAddDataReq :  Pointer to QMI LOC inject wifi AP additional data listpayload.
              q_numberOfAps : Number of qmiLocWifiApAdditionalDataStructT_v02 in pz_QmiLocWifiApAddDataReq array.
@param [out]  pWifiApAddDataReq :  Pointer to Loc Eng inject wifi
              AP additional data listpayload.

@retval TRUE  Successful Translation
@retval FALSE Failed Translation
---------------------------------------------------------------------*/
boolean convertQmiLocInjectWifiApAdditionalDataReq
(
   qmiLocWifiApAdditionalDataStructT_v02*       pz_QmiLocWifiApAddDataReq,
   locEngWifiApAdditionalDataStructT_v01*       p_WifiApAddDataReq,
   uint32                                       q_numberOfAps
)
{
   uint32                                 q_Count = 0;
   qmiLocWifiApAdditionalDataStructT_v02* pz_Temp = NULL;

   /* validate input pointers */
   if ( NULL == pz_QmiLocWifiApAddDataReq ||
        NULL == p_WifiApAddDataReq || 
        0 == q_numberOfAps )
   {
      return FALSE;
   }
   pz_Temp = pz_QmiLocWifiApAddDataReq;

   /* p_WifiApAddDataReq is an array */
   memset(p_WifiApAddDataReq, 0, (sizeof(*p_WifiApAddDataReq) * q_numberOfAps));

   for (q_Count = 0; q_Count < q_numberOfAps; q_Count++)
   {
      /* rssiTimestamp */
      p_WifiApAddDataReq[q_Count].rssiTimestamp = pz_Temp->rssiTimestamp;

      /* measAge */
      p_WifiApAddDataReq[q_Count].measAge = pz_Temp->measAge;

      /* servingAccessPoint */
      p_WifiApAddDataReq[q_Count].servingAccessPoint = pz_Temp->servingAccessPoint;

      /* frequency */
      p_WifiApAddDataReq[q_Count].frequency = pz_Temp->channelFrequency;

      /* copy ssid */
      (void)memscpy(&p_WifiApAddDataReq[q_Count].ssid, sizeof(p_WifiApAddDataReq[q_Count].ssid),
                    &(pz_Temp->ssid), sizeof(pz_Temp->ssid));

      /*apHighResolutionRssi*/
      p_WifiApAddDataReq[q_Count].apHighResolutionRssi = pz_Temp->apHighResolutionRssi;
...
/** convertQmiLocToLocEngSrnDataTechTypeEnum
@brief : convert QMI Srn AP device tech type enum to Loc Eng. 
@param [in]   deviceType :  QMI LOC type enum.
@param [out]  pConvertedType :  Pointer to Loc Eng type.
...
---------------------------------------------------------------------*/
// Routine to translate QMI-LOC WiFi device type enum to LocEng type.
boolean convertQmiLocToLocEngSrnDataTechTypeEnum(qmiLocSrnApDataTechTypeEnumT_v02      deviceType,
                                                 locEngSrnApDataTechTypeEnumT_v01      *pConvertedType)
{
  boolean retVal = TRUE;

  switch (deviceType)
  {
     case eQMI_LOC_SRN_AP_DATA_TECH_TYPE_BT_V02:
       *pConvertedType = eLOC_ENG_SRN_AP_DATA_TECH_TYPE_BT_V01;
       break;
     case eQMI_LOC_SRN_AP_DATA_TECH_TYPE_BTLE_V02:
       *pConvertedType = eLOC_ENG_SRN_AP_DATA_TECH_TYPE_BTLE_V01;
       break;
     case eQMI_LOC_SRN_AP_DATA_TECH_TYPE_NFC_V02:
       *pConvertedType = eLOC_ENG_SRN_AP_DATA_TECH_TYPE_NFC_V01;
       break;
     case eQMI_LOC_SRN_AP_DATA_TECH_TYPE_MOBILE_CODE_V02:
       *pConvertedType = eLOC_ENG_SRN_AP_DATA_TECH_TYPE_MOBILE_CODE_V01;
       break;
     case eQMI_LOC_SRN_AP_DATA_TECH_TYPE_OTHER_V02:
       *pConvertedType = eLOC_ENG_SRN_AP_DATA_TECH_TYPE_OTHER_V01;
       break;
     default:
       retVal = FALSE;
       break;
...
/** convertQmiLocToLocEngSrnApDataMask
@brief : convert QMI Srn AP device type enum to Loc Eng. 
@param [in]   apDataMask :  QMI LOC type mask.
@param [out]  pConvertedType :  Pointer to Loc Eng type.
...
@retval FALSE Failed Translation
---------------------------------------------------------------------*/
// Routine to translate QMI-LOC Srn AP data mask to LocEng type.
boolean convertQmiLocToLocEngSrnApDataMask(qmiLocSrnApDataMaskT_v02   apDataMask,
                                           locEngSrnApDataMaskT_v01   *pConvertedType)
{
   if((NULL == pConvertedType) || (0 == apDataMask))
   {
      LOC_MSG_ERROR("convertQmiLocToLocEngSrnApDataMask: apDataMask 0X%x, pConvertedType %p",
                     apDataMask, pConvertedType, 0);
      return FALSE;
   }
   ...
   *pConvertedType = 0;
   ...
   if(0 != (apDataMask & QMI_LOC_SRN_APDATA_MASK_AP_MAC_V02))
   {
      *pConvertedType |= LOC_ENG_SRN_APDATA_MASK_AP_MAC_V01;
   }

   if(0 != (apDataMask & QMI_LOC_SRN_APDATA_MASK_AP_RSSI_V02))
   {
      *pConvertedType |= LOC_ENG_SRN_APDATA_MASK_AP_RSSI_V01;
   }

   if(0 != (apDataMask & QMI_LOC_SRN_APDATA_MASK_RSSI_TIMESTAMP_V02))
   {
      *pConvertedType |= LOC_ENG_SRN_APDATA_MASK_RSSI_TIMESTAMP_V01;
   }

   return TRUE;
}

/** convertQmiLocInjectSrnApDataReq
@brief : convert QMI Srn AP scan data request to Loc Eng. 
@param [in]   pQmiLocSrnApDataReq :  Pointer to QMI LOC inject 
              Srn AP data listpayload. numberOfAps : Number of
              qmiLocSrnApDataStructT_v02 in pQmiLocSrnApDataReq
              array.
@param [out]  pSrnApDataReq :  Pointer to Loc Eng inject Srn AP
              data listpayload.

@retval TRUE  Successful Translation
@retval FALSE Failed Translation
---------------------------------------------------------------------*/
boolean convertQmiLocInjectSrnApDataReq(const qmiLocSrnBtleApDeviceDataStructT_v02* const pQmiLocSrnApDataReq,
                                        locEngSrnBtleApDeviceDataStructT_v01*       const pSrnApDataReq,
                                        uint32                                      numberOfAps)
{
  ...
  /* validate input pointers */
  if (NULL == pQmiLocSrnApDataReq ||
      NULL == pSrnApDataReq || 0 == numberOfAps)
  {
    return FALSE;
  }

  memset(pSrnApDataReq, 0, (sizeof(locEngSrnBtleApDeviceDataStructT_v01) * numberOfAps));

  for (i = 0; i < numberOfAps; i++)
  {
    /* use conversion function for bit mask */
    convertQmiLocToLocEngSrnApDataMask(pQmiLocSrnApDataReq[i].srnApDataMask,
                                        &pSrnApDataReq[i].srnApDataMask);

    /* copy mac address */
    (void)memscpy(&pSrnApDataReq[i].macAddress, sizeof(pSrnApDataReq[i].macAddress), &pQmiLocSrnApDataReq[i].macAddress, sizeof(pQmiLocSrnApDataReq[i].macAddress));    
    pSrnApDataReq[i].apSrnRssi = pQmiLocSrnApDataReq[i].apSrnRssi;
    pSrnApDataReq[i].apSrnTimestamp = pQmiLocSrnApDataReq[i].apSrnTimestamp;
  }
