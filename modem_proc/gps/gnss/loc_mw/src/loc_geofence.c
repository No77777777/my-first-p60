/*============================================================================
 ...
 Copyright (c) 2015-2019 Qualcomm Techonologies, Inc.  
...
/*============================================================================
 ...
 $Header: //components/rel/gnss8.mpss/11.1/gnss/loc_mw/src/loc_geofence.c#1 $
 $DateTime: 2019/10/24 01:30:36 $
 $Author: pwbldsvc $

 when       who     what, where, why
 --------   ---     ----------------------------------------------------------
05/03/19    yh    Bug fix of checking LOCK status for geofence
   ...
   gfc_qmi_add_geofence_req_s_type *const pGfcAddGeofenceReq)
{
  ...
  {
    if (pAddGeofenceReq->dwellTime > GEOFENCE_DWELL_TIME_MAX)
    {
      /* Return with an error */
      LOC_MSG_ERROR("convertAddCircularGeofenceReqToGfc: Dwell time > %d(s)\n",
                    GEOFENCE_DWELL_TIME_MAX, 0, 0);
      return FALSE;
    }
   ...
   gfc_qmi_set_geofence_engine_config_req_s_type *const pGfcSetGeofenceEngineConfigReq)
{
       ...
       q_index++)
  {
    ...
    else
    {
      ...
      LOC_MSG_ERROR("Unsupported Motion State value %d", p_motionStateInfoQmi->motionState, 0, 0);
    }
  }
   ...
   qmiLocEventGeofenceBatchedBreachIndMsgT_v02 *const pBreachInd)
{
  ...
  if (pGfcBreachInd->pos_rep_mask & GFC_QMI_POS_REPORT_ON_BREACH_TRUE)
  {
    ...
    //Dilution of precision
    memscpy(&pBreachInd->extDOP, sizeof(pBreachInd->extDOP), &pGfcBreachInd->pos_fix.extDOP, sizeof(pGfcBreachInd->pos_fix.extDOP));
    pBreachInd->extDOP_valid = 1;
   ...
   qmiLocEventGeofenceBatchedDwellIndMsgT_v02 *const pDwellInd)
{
  ...
  if (pGfcDwellInd->pos_rep_mask & GFC_QMI_POS_REPORT_ON_BREACH_TRUE)
  {
    ...
    memscpy(&pDwellInd->extDOP, sizeof(pDwellInd->extDOP), &pGfcDwellInd->pos_fix.extDOP, sizeof(pGfcDwellInd->pos_fix.extDOP));
    pDwellInd->extDOP_valid = 1;
   ...
   const gfc_qmi_add_geofence_rsp_s_type *const pAddGeofenceInd)
{
  ...
  qmiLocEventNiGeofenceNotificationIndMsgT_v02 geofenceNiInd = { 0 };
   ...
   const gfc_qmi_query_geofence_rsp_s_type *const pGfcQueryGeofenceInd)
{
  ...
  size_t payloadSize = 0;
  loc_middleware_msg_id_e_type msgId = 0;
  qmiLocQueryGeofenceIndMsgT_v02 geofenceQueryInd = { 0 };
   ...
   const gfc_qmi_proximity_geofence_rsp_s_type *const pGeofenceProximityInd)
{
  ...
  {
    qmiLocEventGeofenceProximityIndMsgT_v02 proxInd = { 0 };
...
static void locGeofenceGetTimeZoneCb(void)
{
  ...
  LOC_MSG_MED("Req QMI Client for TimeZone info", 0, 0, 0);
  if (FALSE == locGeofenceGenerateIPCtolocMW(
   ...
   const gfc_qmi_batch_breach_geofence_rsp_s_type *const pGeofenceBatchBreachInd)
{
  ...
  {
    qmiLocEventGeofenceBatchedBreachIndMsgT_v02 breachInd = { 0 };
...
void locGeofenceHandleEditInd(const os_IpcMsgType *const pIpcMsg)
{
  ...
  {
    qmiLocEditGeofenceIndMsgT_v02 geofenceEditInd = { 0 };
    locQmiShimIndInfoStructT shimIndInfo = { 0 };
   ...
   qmiLocGenRespMsgT_v02 *const pResp)
{
  ...
  loc_client_info_s_type *pz_LocClient =
     locClnt_FindHandle(locClientHandle);
  boolean v_ReqAllowed = FALSE;
  ...
  if (NULL == pz_LocClient)
  {
    ...
    LOC_MSG_ERROR("locGeofenceAdd: invalid Loc client handle %d",
                  locClientHandle, 0, 0);
    return FALSE;
  }
  else if (FALSE == loc_middleware_data.v_AfwAllowAfwPosReq)
  {
    ...
    LOC_MSG_ERROR("locGeofenceAdd: AFW locked", 0, 0, 0);
  }
  else 
  {
    e_qmiLocStatus = eQMI_LOC_SUCCESS_V02;
    v_ReqAllowed = TRUE;
  }
  ...
  if (FALSE == v_ReqAllowed)
  {
    ...
    {
      LOC_MSG_ERROR("locGeofenceAdd: could not send IPC to loc MW\n",
                    0, 0, 0);
    }
...
void locGeofenceSetOperationMode(const qmiLocOperationModeEnumT_v02    op_mode)
{
  ...
  pd_operation_mode = locMW_ConvertFixModeToPdapi(op_mode);
...
void locGeofenceSendTzIndResp(os_IpcMsgType* p_msg)
{
  ...
  qmiLocInjectTimeZoneInfoIndMsgT_v02 injectTzInfoInd n= {0};
