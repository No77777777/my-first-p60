...
$Header: //components/rel/gnss8.mpss/11.1/gnss/loc_mw/src/loc_xtra.c#4 $
...
--------   ---      -------------------------------------------------------
01/28/20  anksha    Support for Navic assist download added
...
// Assembly xtra file
#define MAX_XTRA_FILE_SIZE   (130*1024)
...
(
   const pdsm_xtra_download_info_s_type* pz_XtraDloadInfo
);
...
(
   const pdsm_xtra_time_info_req_s_type*  pz_XtraTimeInfo
);

static void locXtra_ProcessStatusEvent
(
   const pdsm_xtra_info_s_type* pz_XtraInfo
);
...
(
  const pdsm_xtra_config_info_s_type*  pz_XtraConfigInfo
);

static void locXtra_ProcessConfigStatusEvent
(
  const pdsm_xtra_config_status_s_type*  pz_XtraConfigStatus
);
...
boolean locXtra_Init (void)
{
   ...
   do
   {
      ...
      {
         LOC_MSG_ERROR("locXtra_Init: pdsm_client_init for XTRA failed", 0, 0, 0);
         break;
      }
      ...
      {
         LOC_MSG_ERROR("locXtra_Init: pdsm_client_xtra_reg failed, error code = %d", status, 0, 0);
         break;
      }
      ...
      if (status != PDSM_CLIENT_OK)
      {
         LOC_MSG_ERROR ("locXtra_Init: pdsm_client_act for xtra failed, error code =%d", status, 0, 0);
         break;
      }
...
)
{
   ...
   if ((!pz_LocXtraData->v_XtraCmdInProgress) ||
      ((pz_LocXtraData->e_XtraCmd == PDSM_XTRA_CMD_SET_DATA) &&
         (QMI_LOC_INJECT_XTRA_DATA_REQ_V02 == q_QmiLocMsgId)))
   {
      ...
      switch (q_QmiLocMsgId)
      {
         ...
         {
            pz_LocXtraData->e_XtraCmd = PDSM_XTRA_CMD_QUERY_DATA_VALIDITY;
         ...
         {
            pz_LocXtraData->e_XtraCmd = PDSM_XTRA_CMD_INITIATE_DOWNLOAD;
         ...
         case QMI_LOC_INJECT_UTC_TIME_REQ_V02: /* Inject UTC time */
         {
            if(NULL != p_QmiLocMsgData)
            {
               pz_LocXtraData->e_XtraCmd = PDSM_XTRA_CMD_TIME_INFO_INJ;
         ...
         case QMI_LOC_QUERY_XTRA_INFO_REQ_V02:
         {
            if(NULL != p_QmiLocMsgData)
            {
               pz_LocXtraData->e_XtraCmd = PDSM_XTRA_CMD_QUERY_XTRA_CFG_INFO;
...
int locXtra_ProcessCmdRequest (const LocMW_LocCmdStructT * pz_Cmd)
{
   ...
   if (NULL != pz_XtraModuleData)
   {
      ...
      if ((pz_XtraModuleData->v_XtraCmdInProgress) &&
            (pz_XtraModuleData->e_XtraCmd == PDSM_XTRA_CMD_SET_DATA))
      {
         ...
         LOC_MSG_MED ("locXtra_ProcessCmdRequest, data inject: "
                        ...
                        q_XtraDataLen);
         v_Status = pdsm_xtra_set_data (locXtra_CmdCb,
...
)
{
  ...
  /* Function arguments sanity check */
   ...
   LOC_MW_ENTER_CRIT_SECTION(&loc_middleware_data.xtra_crit_sec);
   if (QMI_LOC_MAX_PREDICTED_ORBITS_PART_LEN_V02 < pz_XtraData->partData_len)
   {
      LOC_MSG_ERROR("locXtra_InjectData: Injection can be accepted only if PartDataLen=%d is less than what was defined=%d",
                     pz_XtraData->partData_len, QMI_LOC_MAX_PREDICTED_ORBITS_PART_LEN_V02, 0);
      LOC_MW_LEAVE_CRIT_SECTION(&loc_middleware_data.xtra_crit_sec);
      return LOC_API_INVALID_PARAMETER;
   }
      ...
      (pz_LocXtraData->e_XtraCmd == PDSM_XTRA_CMD_SET_DATA))
   {
      if (NULL == pz_LocXtraData->pb_XtraData)
      {
		 ...
		 #else
		 pz_LocXtraData->pb_XtraData = (char*)os_CladeMemAlloc(MAX_XTRA_FILE_SIZE);
		 #endif
...
)
{
       ...
       (pz_LocXtraData->e_XtraCmd == PDSM_XTRA_CMD_SET_DATA))
   {
      ...
      else
      {
		 ...
		 #else
         os_CladeMemFree((void**)&pz_LocXtraData->pb_XtraData);
		 #endif
...
{
   locXtra_ModuleDataType*  pz_LocXtraData = &(loc_middleware_data.z_locXtra_Data);
   pdsm_xtra_time_info_type z_XtraTimeInfo = {0};
  ...
  /* UTC time conversion */
   ...
   if (pdsm_xtra_inject_time_info(locXtra_CmdCb,
                                  loc_middleware_data.xtra_client_id,
...
static int locXtra_GetDataValidity (void)
{
  ...
  if (pdsm_xtra_query_data_validity(locXtra_CmdCb,
                                    loc_middleware_data.xtra_client_id,
                                    (void*)pz_LocXtraData->l_ClientHandle))
  {
    return LOC_API_SUCCESS;
...
{
  locXtra_ModuleDataType* pz_LocXtraData = &(loc_middleware_data.z_locXtra_Data);
    pdsm_xtra_config_req_info_type z_CfgReqInfo = {0};
  LOC_MSG_MED("locXtra_GetDataValidity called", 0, 0, 0);
  ...
  if (pdsm_xtra_query_config_info(locXtra_CmdCb,
                                    loc_middleware_data.xtra_client_id,
                                    ((void*)pz_LocXtraData->l_ClientHandle),
                                    &z_CfgReqInfo))
  {
...
static int locXtra_GetDataSrc (void)
{
  ...
  if (pdsm_xtra_client_initiate_download_request(locXtra_CmdCb,
                                                 loc_middleware_data.xtra_client_id,
                                                 (void*)pz_LocXtraData->l_ClientHandle))
  {
    return LOC_API_SUCCESS;
...
static uint32 locXtra_ConvertXtraStatusToLocStatus(pdsm_xtra_status_e_type e_XtraCmdStatus)
{
   uint32                  q_RetVal;
...
static void locXtra_ProcessStatusEvent (  const pdsm_xtra_info_s_type* pz_XtraInfo)
{
   locXtra_ModuleDataType* pz_LocXtraData = &(loc_middleware_data.z_locXtra_Data);
   uint32                  q_RetVal;
   pdsm_xtra_status_e_type e_XtraCmdStatus;
   ...
   else
   {
      ...
      if ((pz_LocXtraData->e_XtraCmd == PDSM_XTRA_CMD_SET_DATA) &&
...
(
  const pdsm_xtra_download_info_s_type* p_XtraDloadInfo
)
{
   locXtra_ModuleDataType*                        pz_LocXtraData = &(loc_middleware_data.z_locXtra_Data);
   int                                            i;
...
)
{
   ...
   {
      LOC_MW_LEAVE_CRIT_SECTION(&loc_middleware_data.xtra_crit_sec);
      locXtra_ProcessQmiDownloadEvent(p_XtraDloadInfo); 
      return;
   }
   ...
   if (NULL != pz_InjectPredictedOrbitsReqIndMsg)
   {
      ...
      if(TRUE != locMW_SendLocCmdProxy(LOC_HANDLE_REGISTERED_CLIENTS,
                                       ...
                                       LOC_EVENT_INJECT_XTRA_DATA_REQUEST,
...
static void locXtra_ProcessTimeDownloadEvent (const pdsm_xtra_time_info_req_s_type*  pz_XtraTimeInfo)
{
   ...
   {
      LOC_MSG_ERROR ("locXtra_ProcessTimeDownloadEvent, invalid pdsm_xtra_time_info \n",
                               ...
                               const pdsm_xtra_info_s_type *xtra_info_ptr)
{
   ...
   switch (xtra_event)
   {
      ...
      case PDSM_XTRA_EVENT_CFG_REQ:
      {
         locXtra_ProcessConfigEvent(&(xtra_info_ptr->xtra_union.xtra_cfg_info));
         break;
      }

      case PDSM_XTRA_EVENT_CFG_STATUS:
      {
         locXtra_ProcessConfigStatusEvent(&(xtra_info_ptr->xtra_union.xtra_cfg_status));
         break;
      }
                           ...
                           pdsm_xtra_cmd_err_e_type  xtra_cmd_err)
{
   ...
   pdsm_xtra_cmd_e_type    xtra_cmd_expected = xtra_module_data_ptr->e_XtraCmd;
   uint32                  q_RetVal = LOC_API_GENERAL_FAILURE;
...
static void locXtra_ResetModule (void)
{
   ...
   if (pz_LocXtraData->v_XtraCmdInProgress)
   {
      ...
      pz_LocXtraData->e_XtraCmd           = PDSM_XTRA_CMD_NONE;
...
{
   locXtra_ModuleDataType* pz_LocXtraData = &(loc_middleware_data.z_locXtra_Data);
   pdsm_xtra_config_req_info_type   z_XtraConfigReqInfo = {0};
...
)
{
   ...
   if(NULL != pz_QueryXtraInfoInd)
   {
      ...
      if( pz_XtraConfigInfo->report_mask & XTRA_CFG_INFO_MASK_NAVIC_EPH_ASSIST )
      {
         pz_QueryXtraInfoInd->xtraInfo.reportMask |= QMI_LOC_XTRA_INFO_MASK_NAVIC_EPH_ASSIST_V02;
      }

      if ( pz_XtraConfigInfo->report_mask & XTRA_CFG_INFO_MASK_PRECISE_XTRA_ENABLEMENT_STATUS )
      {
        pz_QueryXtraInfoInd->xtraInfo.reportMask |= QMI_LOC_XTRA_INFO_MASK_PRECISE_XTRA_ENABLED_V02;
      }
...
/* Process config status event */
static void locXtra_ProcessConfigStatusEvent
(
  const pdsm_xtra_config_status_s_type*  pz_XtraConfigStatus
)
{
   qmiLocEventQueryXtraInfoReqIndMsgT_v02*  pz_QueryXtraStatusInd = NULL;
   boolean                          v_Ret = FALSE;

   if(NULL == pz_XtraConfigStatus)
   {
      LOC_MSG_ERROR ("locXtra_ProcessConfigStatusEvent, NULL pz_XtraConfigStatus \n",0, 0, 0);
      return;
   }

   LOC_MSG_HIGH("locXtra_ProcessConfigStatusEvent: usable constellations = %u,"
                " forced enabled = %u reqInfoMask=0x%x",
                pz_XtraConfigStatus->gnss_usable_constellations,
                pz_XtraConfigStatus->gnss_forced_enable_constellations,
                pz_XtraConfigStatus->report_mask);

   pz_QueryXtraStatusInd = (qmiLocEventQueryXtraInfoReqIndMsgT_v02 *)loc_calloc(sizeof(*pz_QueryXtraStatusInd));

   if(NULL != pz_QueryXtraStatusInd)
   {
      if (pz_XtraConfigStatus->report_mask & XTRA_CFG_INFO_MASK_FILE_VERSION )
      {
        pz_QueryXtraStatusInd->xtraFileVersion_valid = TRUE;
        pz_QueryXtraStatusInd->xtraFileVersion.majorVersion = pz_XtraConfigStatus->xtra_file_version.xMajorVersion;
        pz_QueryXtraStatusInd->xtraFileVersion.minorVersion = pz_XtraConfigStatus->xtra_file_version.xMinorVersion;
      }

      if (pz_XtraConfigStatus->report_mask & XTRA_CFG_INFO_MASK_USABLE_CONSTELLATIONS_CONFIG )
      {
        pz_QueryXtraStatusInd->usableConstellationConfig_valid = TRUE;
        SET_XTRA_GNSS_CONSTELLATIONS(pz_XtraConfigStatus->gnss_usable_constellations,
                                     pz_QueryXtraStatusInd->usableConstellationConfig);
      }

      if (pz_XtraConfigStatus->report_mask & XTRA_CFG_INFO_MASK_FORCED_ENABLED_CONSTELLATIONS_CONFIG )
      {
        pz_QueryXtraStatusInd->forcedEnableConfig_valid = TRUE;
        SET_XTRA_GNSS_CONSTELLATIONS(pz_XtraConfigStatus->gnss_forced_enable_constellations,
                                     pz_QueryXtraStatusInd->forcedEnableConfig);
      }

      if (pz_XtraConfigStatus->report_mask & XTRA_CFG_INFO_MASK_MULTIBAND_ENABLED_CONFIG )
      {
        pz_QueryXtraStatusInd->multibandEnabled_valid = TRUE;
        pz_QueryXtraStatusInd->multibandEnabled = pz_XtraConfigStatus->gnss_multiband_enabled;
      }

      if (pz_XtraConfigStatus->report_mask & XTRA_CFG_INFO_MASK_MCC_PRIMARY_SIM_SLOT )
      {
        pz_QueryXtraStatusInd->mccPrimarySimSlot_valid = TRUE;
        pz_QueryXtraStatusInd->mccPrimarySimSlot = pz_XtraConfigStatus->mcc_primary_sim_slot;
      }

      if (pz_XtraConfigStatus->report_mask & XTRA_CFG_INFO_MASK_MCC_SECONDARY_SIM_SLOT )
      {
        pz_QueryXtraStatusInd->mccSecondarySimSlot_valid = TRUE;
        pz_QueryXtraStatusInd->mccSecondarySimSlot = pz_XtraConfigStatus->mcc_secondary_sim_slot;
      }

      if (pz_XtraConfigStatus->report_mask & XTRA_CFG_INFO_MASK_MCC_TERTIARY_SIM_SLOT )
      {
        pz_QueryXtraStatusInd->mccTertiarySimSlot_valid = TRUE;
        pz_QueryXtraStatusInd->mccTertiarySimSlot = pz_XtraConfigStatus->mcc_tertiary_sim_slot;
      }

      if (pz_XtraConfigStatus->report_mask & XTRA_CFG_INFO_MASK_XTRA_REMAINING_VALID_DURATION_MIN )
      {
        pz_QueryXtraStatusInd->xtraRemValidDuration_valid = TRUE;
        pz_QueryXtraStatusInd->xtraRemValidDuration = pz_XtraConfigStatus->xtra_valid_duration_mins;
      }

      if (pz_XtraConfigStatus->report_mask & XTRA_CFG_INFO_MASK_PRECISE_XTRA_ENABLEMENT_STATUS )
      {
        pz_QueryXtraStatusInd->preciseXtraStatus_valid = TRUE;
        pz_QueryXtraStatusInd->preciseXtraStatus = pz_XtraConfigStatus->preciseXtraEnablementStatus;
      }     
      if(TRUE != locMW_SendLocCmdProxy(LOC_HANDLE_REGISTERED_CLIENTS,
                                       LOC_CMD_TYPE_NOTIFY_CLIENT,
                                       LOC_EVENT_QUERY_XTRA_INFO_STATUS,
                                       (void *)pz_QueryXtraStatusInd))
      {
        v_Ret = FALSE;
      }
      else
      ...
      LOC_MSG_HIGH("locXtra_ProcessConfigStatusEvent: queue LOC_EVENT_QUERY_XTRA_INFO_STATUS ", 0, 0, 0);
   }
   else
   {
      // memory allocation error
      LOC_MSG_ERROR ("locXtra_ProcessConfigStatusEvent, malloc failed", 0, 0, 0);
   }
   locXtra_ResetModule();
}
