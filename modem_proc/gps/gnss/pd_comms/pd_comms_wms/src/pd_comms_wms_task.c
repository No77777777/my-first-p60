...
/*=====================================================================
 ...
 Notice that changes are listed in reverse chronological order.
  ...
  $Header: //components/rel/gnss8.mpss/11.1/gnss/pd_comms/pd_comms_wms/src/pd_comms_wms_task.c#1 $
  ...
  boolean u_wait_for_all )
{
  ...
  PDCOMM_APP_MSG_MED("Sending %d bytes for handle_id [%d] to client subid=%d",
                     *p_num_bytes_to_read,q_handle_id,z_handle_table[q_handle_id].e_as_id);
  ...
  *p_num_bytes_in_buffer -= *p_num_bytes_to_read;
  ...
  else
  {
    ...
    /*Clients always waits for a data availability notification
     ...
     if(e_result != PDCOMM_RESULT_OK)
     {
       MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"PDCOMM APP: IPC error %d for handle_id [%d] while sending read_data notification",
...
//   else
//   {
//     MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"SMS message callback: handle_id %d out of range", i, 0, 0);
//
//   }
//
  ...
  os_IpcMsgType* p_msg )
{
    ...
    else
    {
      ...
      *pBufLen += msg_len; // won't be 0
      ...
      if (*pBufLen > 0)
      {
        ...
        if(e_result != PDCOMM_RESULT_OK)
        {
          MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"PDCOMM APP: IPC error %d for handle_id [%d]",  (uint32)e_result,i,0);
  ...
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"Inject message callback: handle_id %d out of range", i, 0, 0);
    ...
    /* We should ACK even though parsedMesgLen is 0 because this msg belongs to us. 
     WMS will call this sms msg callback only if we set shared == FALSE at parse msg callback */
         ...
         (p_info->mt_message_info.message.u.cdma_message.is_tl_ack_requested) )
      {
        ...
        PDCOMM_APP_MSG_MED("WMS SMS call back received on as_id : %d",
         p_info->mt_message_info.as_id, 0, 0);
  ...
  boolean u_notifyClient)
{
  ...
  switch(z_handle_table[z_handle.q_handle_id].e_link_status)
  {
    ...
    case PDCOMM_STATE_OPENING:
    {
     ...
     MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"PDCOMM APP:Illegal state PDCOMM_STATE_OPENING for a SMS connection. Handle_id [%d]",
               z_handle.q_handle_id,0,0);
     if(!b_abort)
     {
  ...
  os_IpcMsgType* p_msg )
{
  ...
  if( z_handle.q_handle_id < PDCOMM_MAX_HANDLES )
  {
       ...
       z_handle_table[z_handle.q_handle_id].w_num_bytes_to_read)
    {
      ...
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"Read called while previous read still pending for handle_id [%d]",
                z_handle.q_handle_id,0,0);
...
/*The entry point for the task and the infinite loop
  ...
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"PDCOMM APP: Could not create a hash function instance",0,0,0);//auto-gen, to change remove 'QSR_' and first param
  }
