...
)
{
  ...
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"PD COMMS IP transport layer proxy open/close registration", 0, 0, 0);
...
)
{
  ...
  if(q_transport_layer_type & PDSM_ATL_TYPE_IP)
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"PDCOMMS IP transport layer proxy open/close de-reg", 0, 0, 0);
...
)
{
  ...
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"PDSM IP transport layer DNS proxy registration", 0, 0, 0);
...
)
{
  ...
  if(transport_layer_type & PDSM_ATL_TYPE_IP)
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"PDSM IP transport layer DNS proxy deregistration", 0, 0, 0);
...
)
{
  ...
  if(p_ipc_ptr == NULL)
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH,"IPC Create failed during pdsm_atl_post_event. Handle Id: %d",
  ...
  else
  {
    ...
    {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH,"IPC Send failed during pdsm_atl_post_event. Handle Id: %d",
                           (uint32)session_handle,0,0);
                              ...
                              const pdsm_atl_open_params_type       *open_params )
{
  ...
  switch( event_id )
  {
    case PDSM_ATL_EVENT_OPEN_SUCCESS:
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"ATL Open Successful",0,0,0);
    ...
    case PDSM_ATL_EVENT_OPEN_FAIL:
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH,"ATL Open Failed",0,0,0);
      q_ds_event_id = DS_ENETNONET;
      break;

    case PDSM_ATL_EVENT_CLOSE_SUCCESS:
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"ATL Close Successful",0,0,0);
      q_ds_event_id = DS_ENETNONET;
      break;

    case PDSM_ATL_EVENT_CLOSE_FAIL:
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH,"ATL Close Failed",0,0,0);
      q_ds_event_id = DS_ENETNONET;
      break;
    ...
    case PDSM_ATL_EVENT_L2_PROXY_OPEN_ACK:
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"ATL Open Ack Received",0,0,0);
      // Log the connection status 
      pd_comms_log_connection_status_event(PD_EVENT_NW_CONNECTING);
      return TRUE;

    case PDSM_ATL_EVENT_L2_PROXY_CLOSE_ACK:
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"ATL Close Ack Received",0,0,0);
      return TRUE;

    case PDSM_ATL_EVENT_DNS_LOOKUP_REQUEST_ACK:
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"ATL DNS lookup request Ack Received",0,0,0);
      return TRUE;

    default:
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"Invalid ATL Event Returned %d",event_id,0,0);
      return FALSE;
  }
...
)
{
  if(q_transport_layer_type & PDSM_ATL_TYPE_IP)
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"PD COMMS IP transport layer proxy default open/close registration", 0, 0, 0);
...
)
{
...
#endif
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"Invalid link type received in call to ATL open",0,0,0);
    return PDCOMM_RESULT_ERROR_UNKNOWN;
  }

  if(z_session_handle.q_handle_id >= PDCOMM_MAX_HANDLES)
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"Invalid session handle received in call to ATL open",0,0,0);
    return PDCOMM_RESULT_ERROR_UNKNOWN;
  }
  ...
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"No ATL open service registered",0,0,0);
    return PDCOMM_RESULT_ERROR_UNKNOWN;
  }
}
...
)
{
...
#endif
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"Invalid link type received in call to ATL close",0,0,0);
    return PDCOMM_RESULT_ERROR_UNKNOWN;
  }

  if(z_session_handle.q_handle_id >= PDCOMM_MAX_HANDLES)
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"Invalid session handle received in call to ATL close",0,0,0);
    return PDCOMM_RESULT_ERROR_UNKNOWN;
  }
  ...
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"No ATL close service registered",0,0,0);
    return PDCOMM_RESULT_ERROR_UNKNOWN;
  }
}
...
)
{
  ...
  */
  ...
  else
  {
    ...
    {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"No ATL dns service registered",0,0,0);
      return FAILED;
    }
  }
}
