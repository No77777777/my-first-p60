...
/*=====================================================================
 ...
 Notice that changes are listed in reverse chronological order.
  ...
  $Header: //components/rel/gnss8.mpss/11.1/gnss/pd_comms/pd_comms_tcp/src/pd_comms_tcp_task.c#9 $
  $DateTime: 2023/04/04 17:19:59 $
  $Author: tfong $

 when      who   what, where, why
 ----      ----  --- ---------------------------------------------------
 02/04/22   jv   Set correct TLS version
 11/05/19   skm  Move to dss_recvfrom for udp listening
 09/30/19   skm  Support for 911 nDDS on DSDS targets 
 07/22/19   nath Added default case explicitly.
 11/25/18   sm   Set TCP SACK for AT&T CDS Requirement CDR-CDS-075
 05/30/18   sum  DogHb sequence upfate
 05/18/18   skm  PS Data off feature support for 3GPP rel-14
 05/18/18   skm  SHA (above SHA256 )and TLS (above 1.1) Support
 04/10/18   py   On DD sub change and refresh, reread APN profiles from h/w
 03/20/18   skm  Supl E911 over Wifi
 02/17/18   skm  Supl connection improvements FR.
 02/15/18   skm  Support Dynamic Certificate sync for SSL connections
 02/05/18   sum  Close the PPP connection if no active handle exist.
 11/16/17   skm  Use HLOS provided IP type for regular connection, if IP type is V4/V6
 07/05/17   kpn  Notify status report to client only if handle is valid
 05/05/17   sum  Correction in Log Message.
 02/16/17   kpn  Set linger time-out value to 0 on TCP abort to avoid crash on PDT test
...
#define PDCOMM_TCP_DATA_DECLARE
#include "pd_comms_tcp_data.h"
#undef PDCOMM_TCP_DATA_DECLARE
...
#ifndef FEATURE_NO_PS_DATA_OFF_SUPPORT
/*Data PS Status*/
static ds_sys_ps_data_avail_status_enum_type ps_status = DS_SYS_PS_DATA_AVAIL_STATUS_UNKNOWN;
#endif /* ! FEATURE_NO_PS_DATA_OFF_SUPPORT */

/****************** Function Definitions**************************************/

/* Initialize the GPS Diag Module */
extern void gpsdiag_init(void);
static void pd_comms_tcp_task_setsockopt(uint32 q_handle_id, boolean u_abort);
static void pd_comms_tcp_task_read_nvs(boolean read_from_hw, sys_modem_as_id_e_type sub);
#ifdef FEATURE_MODEM_CONFIG_REFRESH
...
static void pd_comms_tcp_task_startup(void)
{
  ...
  if (ret == FALSE)
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: IPC Start Failed", 0, 0, 0);
  }

  /* Initialize ULog Handle for PD_COMMS_TCP, last two arguments are default */
  q_ULogInitStatus = os_ULogInit(&pdcomm_tcp_ULogHandle, "PDCOMM_TCP_ULOG", PDCOMM_TCP_ULOG_SIZE, 0, 0);
  if (q_ULogInitStatus != 0)
  {
    /* Check ULOG_ERRORS Enum for Error Code in ULOG.H */
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM_TCP ULOG Handle Init Failed with Error: %d",
          q_ULogInitStatus, 0, 0);
    ERR_FATAL("PDCOMM_TCP ULOG Handle Init Failed", 0, 0, 0);
  }
  else
  {
    OS_ULOG_PRINT_0(pdcomm_tcp_ULogHandle, "PDCOMM_TCP ULOG Handle created successfully");
  }
  ...
  if (ret == FALSE)
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: getHandle Mutex Init Failed", 0, 0, 0);
  }
  ...
  /*For all the subs, Read the NV items frmo Hardware and cache them*/
  for (sub = PD_COMMS_CONFIG_MIN_SUB_ID; sub < PD_COMMS_CONFIG_MAX_SUBS; ++sub)
  {
    pd_comms_tcp_task_read_nvs(TRUE, sub);
  }

  /* For current DD sub, read values from cache*/
  pd_comms_tcp_task_read_nvs(FALSE, pd_comms_nv_efs_get_current_dd_sub());
...
#ifndef FEATURE_NO_PS_DATA_OFF_SUPPORT
/*===========================================================================
FUNCTION pd_comms_tcp_task_ps_status_event_cb
...
SIDE EFFECTS
  None
===========================================================================*/
void pd_comms_tcp_task_ds_event_cb(ds_sys_tech_enum_type               tech_type,
                                   ds_sys_event_enum_type              event_name,
                                   ds_sys_subscription_enum_type       subscription_id,
                                   void *event_info_ptr,
                                   void *user_data_ptr)
{
  ds_sys_ps_data_avail_type *ps_status = NULL;
  pd_comms_tcp_task_ds_event_cb_ipc_hdr_type *p_hdr_ptr;
  ...
  {
    MSG(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: DS Event cb NULL payload");
    return;
  }

  if (DS_SYS_EVENT_3GPP_PS_DATA_AVAIL_CHANGE != event_name)
  {
    return;
  }

  MSG_1(MSG_SSID_GPS, MSG_LEGACY_MED, "DSS event = %d", event_name);


  p_ipc_ptr = os_IpcCreate(sizeof(pd_comms_tcp_task_ds_event_cb_ipc_hdr_type),
                           IPC_ALLOCATION_DYNAMIC, (uint32)THREAD_ID_SM_PDCOMM_TCP);

  if (p_ipc_ptr == NULL)
  {
    MSG(MSG_SSID_GPS, MSG_LEGACY_ERROR, "IPC create failed DS Event CB");
  }
  else
  {
    /*lint -e826 */
    p_hdr_ptr = (pd_comms_tcp_task_ds_event_cb_ipc_hdr_type *)p_ipc_ptr->p_Data;
    /*lint +e826 */
    p_ipc_ptr->q_MsgId = (uint32)PDCOMM_APP_TASK_DSS_EVENT_CB;
    p_ipc_ptr->q_DestThreadId = (uint32)THREAD_ID_SM_PDCOMM_TCP;

    /* currently we only process PS status events.
       Future DS Event handling should be added below as IF blocks
       and processed in IPC handler CB.*/
    if (DS_SYS_EVENT_3GPP_PS_DATA_AVAIL_CHANGE == event_name)
    {
      p_hdr_ptr->ds_event_name = event_name;
      memscpy(&(p_hdr_ptr->ds_event.ps_data_avail_type), sizeof(ds_sys_ps_data_avail_type),
              (ds_sys_ps_data_avail_type *)event_info_ptr, sizeof(ds_sys_ps_data_avail_type));
    }

    if (!os_IpcSend(p_ipc_ptr, (uint32)THREAD_ID_SM_PDCOMM_TCP))
    {
      MSG(MSG_SSID_GPS, MSG_LEGACY_ERROR, "IPC Send failed DS Event CB");
...
#ifndef FEATURE_NO_PS_DATA_OFF_SUPPORT
/*===========================================================================
FUNCTION pd_comms_tcp_task_register_ds_callbacks

DESCRIPTION
  Registers Data Services callabcks needed to listen to DS events.
...
SIDE EFFECTS
  None
===========================================================================*/
static void pd_comms_tcp_task_register_ds_callbacks(void)
{
  int16  dss_errno      = 0;
  int16  dss_ret_val    = 0;
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

  /* Register for PS_SYS_EVENT_3GPP_PS_DATA_AVAIL_CHANGE event */
  dss_ret_val =  ds_sys_event_reg_ex(DS_SYS_TECH_3GPP,
                                     DS_SYS_EVENT_3GPP_PS_DATA_AVAIL_CHANGE,
                                     DS_SYS_DEFAULT_SUBS,
                                     pd_comms_tcp_task_ds_event_cb,
                                     NULL,
                                     &dss_errno);
  if (DSS_ERROR == dss_ret_val)
  {
    MSG_2(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: DS Event Reg failed ret_val=0x%x, err 0x%x", dss_ret_val, dss_errno);
  }
}
#endif /* ! FEATURE_NO_PS_DATA_OFF_SUPPORT */
...
static void pd_comms_tcp_task_init_handle_table(void)
{
  ...
  for (l_i = 0; l_i < PDCOMM_MAX_HANDLES; l_i++)
  {
    ...
    z_handle_table[l_i].is_ppp_down                 = TRUE;
   ...
   pd_comms_disconnect_result_type *p_disc_error)
{
  ...
  if(z_handle_table[q_handle_id].is_ppp_down  == TRUE)
  {
    PDCOMM_APP_MSG_MED("PPP connection is already closed for handle_id [%d]", q_handle_id, 0, 0);
    return SUCCEEDED;
  }
  ...
  /*Set the ppp down flag to TRUE, It means that ATL connection is closed, 
  It helps in identifying at any point of time if the connection is open or not and subsequently prevents redundant closure.
  If close fails there is no way to recover from the scenario, There will not be any reattempt so ATL is deemed to be closed*/
  z_handle_table[q_handle_id].is_ppp_down = TRUE;
...
#ifndef FEATURE_NO_PS_DATA_OFF_SUPPORT
/*===========================================================================
FUNCTION pd_comms_tcp_task_check_ps_status

DESCRIPTION
  Function checks if AGPS connection are allowed when PS status is ON/OFF. 
  Iterate through the list of services and their permission when PS status is OFF
  If OTHER (GPS belongs to this categopry) is NON-EXEMPT return TRUE.
 
DEPENDENCIES
  None

RETURN VALUE
  BOOLEAN:  TRUE , GPS connection should be terminated
            FALSE, GPS connection are allowed.

SIDE EFFECTS
  None
===========================================================================*/
static boolean pd_comms_tcp_task_check_ps_status(ds_sys_ps_data_avail_type *pz_ps_status)
{
  int l_i = 0;
  boolean v_retval = FALSE;

  if (NULL == pz_ps_status)
  {
    MSG(MSG_SSID_GPS, MSG_LEGACY_ERROR, "NULL pointer in PS status check");
    return v_retval;
  }

  if (DS_SYS_PS_DATA_AVAIL_LIST_TYPE_HOME_ROAM != pz_ps_status->list_type)
  {
    MSG(MSG_SSID_GPS, MSG_LEGACY_MED, "PS List type not supported:%d");
    return v_retval;
  }

  for (l_i = 0; l_i < DS_SYS_APPL_SERVICE_NAME_MAX; l_i++)
  {
    if ((DS_SYS_APPL_SERVICE_NAME_OTHER == pz_ps_status->service_info[l_i].service) &&
        (DS_SYS_PS_DATA_AVAIL_TYPE_NON_EXEMPT == pz_ps_status->service_info[l_i].type))
    {
      v_retval = TRUE;
    }
  }

  MSG_1(MSG_SSID_GPS, MSG_LEGACY_MED, "PS Data Status check:%d", v_retval);

  return v_retval;
} /*pd_comms_tcp_task_check_ps_status*/
#endif /* ! FEATURE_NO_PS_DATA_OFF_SUPPORT */
   ...
   uint32 q_result)
{
  ...
  switch (z_new_state)
  {
    case PDCOMM_STATE_CONNECTED:
      {
        if (notify_client && z_old_state != PDCOMM_STATE_CONNECTED)
        {
          ...
          {
            MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: IPC error %d for handle_id [%d]",
    ...
    case PDCOMM_STATE_DISCONNECTED:
      {
        if (!z_handle_table[z_handle.q_handle_id].v_release_handle)
        {
          if (z_old_state != PDCOMM_STATE_DISCONNECTED)
          {
            if (notify_client)
            {
              ...
              else
              {
                ...
                e_result = pd_comms_notify_client(z_handle,
                                                  ...
                                                  z_handle_table[z_handle.q_handle_id].q_disconnect_transaction_id,
                                                  ERROR_RPT, &z_params);
                /*Send State Change also. Send only if handle is still in use and not released.*/
                if (TRUE == z_handle_table[z_handle.q_handle_id].in_use)
                {
                  e_result = pd_comms_notify_client(z_handle,
                                                    z_handle_table[z_handle.q_handle_id].q_source_thread_id,
                                                    z_handle_table[z_handle.q_handle_id].q_base_msg_id,
            ...
            /*This condition checks if the function pointers are properly initialised or not*/
            else if (pd_comms_atl_i_ppp_proxy_exists())
            {
              int l_i;

              for (l_i = 0; l_i < PDCOMM_MAX_HANDLES; l_i++)
              {
                if (z_handle_table[l_i].x_app_id != DSS_ERROR)
                {
                  break;
                }
              }
              /*Debug f3*/
              PDCOMM_APP_MSG_MED("PPP state  %d ", (z_handle_table[z_handle.q_handle_id].is_ppp_down), 0, 0);

              /*Here the check is to see if the ppp is down or not, Close the ATL only if the ppp is not down,
              Redudndantly closer causes problem in case of back to back session*/
              if (l_i >= PDCOMM_MAX_HANDLES && (FALSE == z_handle_table[z_handle.q_handle_id].is_ppp_down))
              {
                /* If PPP connection is open and none of the handles are Active then close
                   the PPP connection. This situation can arise when the ATL was not opened in the last ongoing session */
                /*e_disc_error is not being used, Need to  be cleaned up*/
                PDCOMM_APP_MSG_MED("Closing PPP connection, No active handle exist", 0, 0, 0);
                pd_comms_tcp_task_close_ppp(z_handle.q_handle_id, &e_disc_error);
              }
            }
    ...
    default:
      {
        MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "Invalid State %d recevied for update", z_new_state, 0, 0);
      }
   ...
   sint15 x_err_no)
{
  switch (x_err_no)
  {
    case DS_EMAPP:
      /* Too many apps */
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: Too many apps.", 0, 0, 0);
      break;
    case DS_EBADAPP:
      /* bad application ID */
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: Bad app id.", 0, 0, 0);
      break;
    case DS_ENETCLOSEINPROGRESS:
    ...
    case DS_SOCKEXIST:
      /* socket exists */
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: Socket exists.", 0, 0, 0);
      break;
    case DS_ENETEXIST:
      /* Network exists */
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: Net exists.", 0, 0, 0);
      break;
    case DS_EAFNOSUPPORT:
      /* AF_INET parameter is bad */
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: Bad Addr Family.", 0, 0, 0);
      break;
    case DS_EPROTOTYPE:
      /* invalid protocol type */
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: Bad Protocol type.", 0, 0, 0);
      break;
    case DS_ESOCKNOSUPPORT:
      /* invalid or unsupported socket parameter specified */
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "Bad socket param.", 0, 0, 0);
      break;
    case DS_EPROTONOSUPPORT:
      /*  specified protocol not supported */
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: Bad protocol not supported.", 0, 0, 0);
      break;
    case DS_EMFILE:
      /* too many descriptors open */
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: Too many descriptors open.", 0, 0, 0);
      break;
    case DS_EBADF:
      /* invalid socket descriptor is specfied */
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: Inv Socket Descriptor.", 0, 0, 0);
      break;
    case DS_EFAULT:
      /* addrlen parameter is invalid */
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: DS_EFAULT Inv Addr Len.", 0, 0, 0);
      break;
    case DS_EOPNOTSUPP:
      /* invalid server address specified */
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: Inv Serv Addr.", 0, 0, 0);
      break;
    case DS_EADDRREQ:
      /* destination address is required */
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: Dest Addr Req'd.", 0, 0, 0);
      break;
    case DS_ENETDOWN:
      /* network subsystem unavailable */
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: NW Subsys unavail.", 0, 0, 0);
      break;
    case DS_EISCONN:
      /* a socket descriptor is specified that is already connected */
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: Socket desc already conn.", 0, 0, 0);
      break;
    case DS_ECONNREFUSED:
      /* connection attempt refused */
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: Conn attmpt refused.", 0, 0, 0);
      break;
    case DS_ETIMEDOUT:
      /* connection attempt timed out */
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: Conn attmpt timeout.", 0, 0, 0);
      break;
    case DS_EIPADDRCHANGED:
      /* network level IP changed due to PPP resync */
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: IP Addr changed.", 0, 0, 0);
      break;
    case DS_EPIPE:
      /* broken pipe */
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: Broken Pipe.", 0, 0, 0);
      break;
    case DS_ENOTCONN:
      /* socket not connected */
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: Socket not connected.", 0, 0, 0);
      break;
    case DS_ECONNRESET:
      /* TCP connection reset by server */
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: TCP Conn. reset by server.", 0, 0, 0);
      break;
    case DS_ECONNABORTED:
      /* TCP connection aborted due to timeout or other failure */
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: TCP Connection aborted.", 0, 0, 0);
      break;
    case DS_EWOULDBLOCK:
                                                                ...
                                                                int  *profile_num)
{
  ...
  sys_modem_as_id_e_type as_id = pd_comms_nv_efs_get_current_dd_sub();
  ds_profile_subs_etype sub_id;

  /* Map sys_modem_as_id_e_type to DS specific sub ID*/
  sub_id = as_id + 1;
  ...
  MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH, "Find Emergency Profile Tech using subid %d ds_sub %d tech_type %d", as_id, sub_id, tech_type);

  if (ds_profile_get_list_itr_per_sub(tech_type, &search_list_info, &itr, sub_id) == DS_PROFILE_REG_RESULT_SUCCESS)
  {
    ...
    if (ds_profile_get_info_by_itr(itr, &list_info) == DS_PROFILE_REG_RESULT_SUCCESS)
    {
      ...
      *profile_num = list_info.num;
      ...
      status = ds_profile_begin_transaction_per_sub(trn_type,
                                                    profile_type,
                                                    prf_num,
                                                    sub_id,
                                                    &profile_hndl);
...
/** SYS AS ID to DS SUBS Enum Mapping
  SYS_MODEM_AS_ID_NONE = -1   |    DS_SYS_DEFAULT_SUBS      = 0x0000, < Default >
  SYS_MODEM_AS_ID_1 = 0       |    DS_SYS_PRIMARY_SUBS      = 0x0001, < Primary >
  SYS_MODEM_AS_ID_2 = 1       |    DS_SYS_SECONDARY_SUBS    = 0x0002, < Secondary >
  SYS_MODEM_AS_ID_3 = 2       |    DS_SYS_TERTIARY_SUBS     = 0x0003, < Tertiary > 
  **/
sys_modem_as_id_e_type pd_comms_tcp_task_map_ds_asid_to_sys_asid(ds_sys_subscription_enum_type ds_subId)
{
  switch (ds_subId)
  {
...
ds_sys_subscription_enum_type pd_comms_tcp_task_map_sys_asid_to_ds_asid(sys_modem_as_id_e_type cm_subID)
{
  switch (cm_subID)
  {
    case SYS_MODEM_AS_ID_1:
      return DS_SYS_PRIMARY_SUBS;
    case SYS_MODEM_AS_ID_2:
      return DS_SYS_SECONDARY_SUBS;
    case SYS_MODEM_AS_ID_3:
      return DS_SYS_TERTIARY_SUBS;
    default:
      return DS_SYS_DEFAULT_SUBS;
  }
}

void pd_comms_tcp_task_set_ds_apn_type(const pdsm_atl_apn_type_mask pdsm_apn_type,
                                       dss_net_policy_info_type *p_policy_info)
{

  dss_apn_enum_type apn_type = DSS_APN_TYPE_UNSPECIFIED;

  if (NULL == p_policy_info)
  {
    MSG(MSG_SSID_GPS, MSG_LEGACY_ERROR, "set_ds_apn_type: policy_info ptr is NULL !!");
    return;
  }

  MSG_1(MSG_SSID_GPS, MSG_LEGACY_MED, "set_ds_apn_type: APN type provided: %llx", pdsm_apn_type);

  switch (pdsm_apn_type)
  {
    case PDSM_ATL_APN_TYPE_MASK_DEFAULT:
    case PDSM_ATL_APN_TYPE_MASK_UNSPECIFIED:
      apn_type = DSS__APN_TYPE_INTERNET;
      break;
    case PDSM_ATL_APN_TYPE_MASK_IMS:
      apn_type = DSS_APN_TYPE_IMS;
      break;
    case PDSM_ATL_APN_TYPE_MASK_MMS:
      apn_type = DSS_APN_TYPE_MMS;
      break;
    case PDSM_ATL_APN_TYPE_MASK_DUN:
      apn_type = DSS_APN_TYPE_DUN;
      break;
    case PDSM_ATL_APN_TYPE_MASK_SUPL:
      apn_type = DSS_APN_TYPE_SUPL;
      break;
    case PDSM_ATL_APN_TYPE_MASK_HIPRI:
      apn_type = DSS_APN_TYPE_HIPRI;
      break;
    case PDSM_ATL_APN_TYPE_MASK_FOTA:
      apn_type = DSS_APN_TYPE_FOTA;
      break;
    case PDSM_ATL_APN_TYPE_MASK_CBS:
      apn_type = DSS_APN_TYPE_CBS;
      break;
    case PDSM_ATL_APN_TYPE_MASK_IA:
      apn_type = DSS_APN_TYPE_IA;
      break;
    case PDSM_ATL_APN_TYPE_MASK_EMERGENCY:
      apn_type = DSS_APN_TYPE_EMERGENCY;
      break;
    default:
      apn_type = DSS_APN_TYPE_UNSPECIFIED;
      break;
  }
  /*Set the DS policy's APN type*/
  p_policy_info->apn_type = apn_type;

  return;
}

pdsm_atl_apn_type_mask pd_comms_tcp_task_map_ds_apn_type(ds_sys_apn_enum_type apn_type_mask)
{
  pdsm_atl_apn_type_mask apn_type = PDSM_ATL_APN_TYPE_MASK_UNSPECIFIED;
  switch (apn_type_mask)
  {
    case DS_SYS_APN_TYPE_INTERNET:
      apn_type = PDSM_ATL_APN_TYPE_MASK_DEFAULT;
      break;
    case DS_SYS_APN_TYPE_IMS:
      apn_type = PDSM_ATL_APN_TYPE_MASK_IMS;
      break;
    case DS_SYS_APN_TYPE_MMS:
      apn_type = PDSM_ATL_APN_TYPE_MASK_MMS;
      break;
    case DS_SYS_APN_TYPE_DUN:
      apn_type = PDSM_ATL_APN_TYPE_MASK_DUN;
      break;
    case DS_SYS_APN_TYPE_SUPL:
      apn_type = PDSM_ATL_APN_TYPE_MASK_SUPL;
      break;
    case DS_SYS_APN_TYPE_HIPRI:
      apn_type = PDSM_ATL_APN_TYPE_MASK_HIPRI;
      break;
    case DS_SYS_APN_TYPE_FOTA:
      apn_type = PDSM_ATL_APN_TYPE_MASK_FOTA;
      break;
    case DS_SYS_APN_TYPE_CBS:
      apn_type = PDSM_ATL_APN_TYPE_MASK_CBS;
      break;
    case DS_SYS_APN_TYPE_IA:
      apn_type = PDSM_ATL_APN_TYPE_MASK_IA;
      break;
    case DS_SYS_APN_TYPE_EMERGENCY:
      apn_type = PDSM_ATL_APN_TYPE_MASK_EMERGENCY;
      break;
    default:
      MSG(MSG_SSID_GPS, MSG_LEGACY_MED, "map_ds_apn_type: APN type get failed use UnSpecified");
      break;
  }
  MSG_1(MSG_SSID_GPS, MSG_LEGACY_MED, "map_ds_apn_type: APN type mask: %llx", apn_type);
  return (apn_type);
}
                                                                      ...
                                                                      int  *profile_num)
{
  ...
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "Emergency Profile Tech PDP: Invalid params, using default profile", 0, 0, 0);
    return result;
  }
  ...
  if (pd_comms_tcp_task_find_emergency_profile_by_tech(tech_type_i, pdp_type, profile_num) == SUCCEEDED)
  {
    ...
    *pdp_type = pd_comms_tcp_task_xlate_3gpp_pdp_type_to_atl_pdp_type((ds_profile_3gpp_pdp_type_enum_type) * pdp_type);
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_LOW, "Emergency profile: PDP:%d tech_type: %d profile: %d", (uint32)(*pdp_type), (int)(*tech_type), *profile_num);
    result = SUCCEEDED;
    return result;
  }
                                      ...
                                      ds_profile_tech_etype *tech_type)
{
  ...
  switch (sys_mode)
  {
    ...
    case SYS_SYS_MODE_WCDMA:
      ...
      *tech_type = DS_PROFILE_TECH_3GPP;
      break;
      /*WLAN and LTE use the same APN's Emergency and Internet connections 
        use the APN conf from LTE IDX*/
  ...
  MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH, "Profile type %d Serving System %d", profile_type, srv_sys_type, 0);

  if (srv_sys_type == PDCOMM_APN_PROFILE_SRV_SYS_MAX_INDEX)
  {
    MSG(MSG_SSID_GPS, MSG_LEGACY_HIGH, "Returning Null Apn name");
    return NULL;
  }
  else
  {
   ...
   )
{
  ...
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "DSS_GET_POLICY: policy_info ptr is NULL !!", 0, 0, 0);
    return;
  }
  ...
  else
  {
    ...
    /*For MDM targets Apn Type should not be used. 
      Only APN name should be populated in the Policy*/
    p_policy_info->bring_up_using_apn_type = FALSE;
  ...
  **/
  ...
  p_policy_info->subs_id = (pd_comms_tcp_task_map_sys_asid_to_ds_asid(pd_comms_nv_efs_get_current_dd_sub()));

  PDCOMM_APP_MSG_HIGH("Policy sub_id set to: %d", p_policy_info->subs_id, 0, 0);

  /*If not APN name is configured, the use the default profile*/
  if (apn_name != NULL && apn_name[0] != '\0')
  {

    MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH, "APN name is passed, printing first three charecters %c %c %c",
          apn_name[0], apn_name[1], apn_name[2]);
    /*Set APN Name to be used for the policy 
      DS will use the APN name for the lookup and connection mapping.*/
    p_policy_info->bring_up_using_apn_name = TRUE;
    p_policy_info->umts.apn.length = strlen(apn_name);
    p_policy_info->umts.apn.name = (byte *)apn_name;

    /*For MSM, APN type is always set and should be used for connection bring up. 
      In MDM case, we will used only APN Name for profile lookup*/
    if ((TRUE == pd_comms_atl_i_ppp_proxy_exists()) &&
        (PDSM_ATL_APN_TYPE_MASK_UNSPECIFIED != z_atl_open_params.apn_type))
    {
      p_policy_info->bring_up_using_apn_type = TRUE;
      pd_comms_tcp_task_set_ds_apn_type(z_atl_open_params.apn_type, p_policy_info);
    }

    /*Determine the PDP type to be used.*/
    ...
    else /*APN Type is V4V6 check for preference. 
           Regular SUPL pref is V4.*/
    {
      if ((!pd_comms_atl_i_ppp_proxy_exists() && (PDCOMM_APN_PROFILE_TYPE_LBS_EMERGENCY == z_handle_table[q_handle_id].z_connect_params.z_link_params.z_tcp_params.profile_type)) ||
          (pd_comms_atl_i_ppp_proxy_exists() && (PDSM_ATL_APN_TYPE_MASK_EMERGENCY == z_atl_open_params.apn_type)))
      {
  ...
  {
    /*Apn Name is not available. Set DS policy not to use APN Name / APN TYPE for lookup 
      If APN Type is provided then it should be used*/
    p_policy_info->bring_up_using_apn_name = FALSE;
    p_policy_info->bring_up_using_apn_type = FALSE;

    if ((!pd_comms_atl_i_ppp_proxy_exists() && (PDCOMM_APN_PROFILE_TYPE_LBS_EMERGENCY == z_handle_table[q_handle_id].z_connect_params.z_link_params.z_tcp_params.profile_type)) ||
        (pd_comms_atl_i_ppp_proxy_exists() && (PDSM_ATL_APN_TYPE_MASK_EMERGENCY == z_atl_open_params.apn_type)))
    {
      MSG(MSG_SSID_GPS, MSG_LEGACY_HIGH, "Session is emergency");
      /*Use legacy procedure to determine the PDP type for the emergency profile*/
      if (pd_comms_tcp_task_find_emergency_profile_tech_pdp_type(&tech_type, &pdp_type, &profile_num) == SUCCEEDED)
      {
        /* MSM target return NULL APN name for Emergency connection.
        In this case we use APN Type to construct policy information.*/
        if ((TRUE == pd_comms_atl_i_ppp_proxy_exists()) &&
            (PDSM_ATL_APN_TYPE_MASK_UNSPECIFIED != z_atl_open_params.apn_type))
        {
          p_policy_info->bring_up_using_apn_type = TRUE;
      ...
      {
        MSG(MSG_SSID_GPS, MSG_LEGACY_HIGH, "NULL Apn, Emergency Session - Lookup fail");
        p_policy_info->bring_up_using_apn_type = TRUE;
        pd_comms_tcp_task_set_ds_apn_type(PDSM_ATL_APN_TYPE_MASK_DEFAULT, p_policy_info);
      }
    ...
    {
      MSG(MSG_SSID_GPS, MSG_LEGACY_HIGH, "NULL Apn Non-Emergency Session");
      p_policy_info->bring_up_using_apn_type = TRUE;
      pd_comms_tcp_task_set_ds_apn_type(PDSM_ATL_APN_TYPE_MASK_DEFAULT, p_policy_info);
      /*No APN information configured. Set the policy to use Default profile 
        use the default profile PDP type*/
      pd_comms_tcp_get_iface_ipfamily(p_policy_info, FALSE);
    }
  }

  MSG_4(MSG_SSID_GPS, MSG_LEGACY_HIGH, "DS Policy Family set to: %d, Apn Type:%u, UseApnType: %u, UseApnName:%u",
        p_policy_info->family,
        p_policy_info->apn_type,
        p_policy_info->bring_up_using_apn_type,
        p_policy_info->bring_up_using_apn_name);
...
static dss_iface_id_type pd_comms_tcp_task_dss_get_iface_id(uint32 q_handle_id)
{
  ...
  if ((pd_comms_atl_i_ppp_proxy_exists()) || (z_handle_table[q_handle_id].x_app_id == DSS_ERROR))
  {
    ...
    {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED, "pd_comms_tcp_task_dss_get_iface_id called with NULL network interface id", 0, 0, 0);
    }
   ...
   )
{
  ...
  {
    /*Set SO_LINGER time to 50 msec*/
    pd_comms_tcp_task_setsockopt(q_handle_id, FALSE);
    ...
    else
    {
      switch (x_err_no)
      {
        case DS_EBADF:
          {
            MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: Socket Close - Bad Socket Descriptor for handle_id [%d]",
                  q_handle_id, 0, 0);
            e_status = FAILED;
            *p_disc_error = PDCOMM_DISCONNECT_ERROR_UNKNOWN;
            break;
        ...
        default:
          {
            MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP unknown socket close error: %d for handle_id [%d]",
   ...
   )
{
  ...
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "pd_comms_tcp_task_dss_dns_callback: user_data_ptr is NULL!! Rejecting results", 0, 0, 0);
    return;
  }
  ...
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "Socket callback: handle_id %d out of range", q_handle_id, 0, 0);
  }
   ...
   pd_comms_connect_result_type *p_conn_error)
{
  ...
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "Session could not be created error %d", dss_errno, 0, 0);
    return FAILED;
  }
  ...
  if (DSS_ERROR == x_retval)
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "Set config param returned error %d", dss_errno, 0, 0);

    /*Delete DNS session*/
    x_retval = dss_dns_delete_session(dns_session_handle, &dss_errno);
    if (DSS_ERROR == x_retval)
    {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "DNS session %d could not be deleted", dns_session_handle, 0, 0);
    }
  ...
  */
  ...
  if (DSS_ERROR == x_retval)
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "Set config param DNS Linear Backoff returned error %d", dss_errno, 0, 0);
  }

  /*Get PDP Type to set the correct IP family for DNS query*/
  pd_comms_tcp_get_pdp_type(q_handle_id, &pdp_type);

  MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED, "PDP_TYPE DNS:%d ,Z_Policy IP Family:%d", pdp_type, z_policy_info.family, 0);
  ...
  MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH, "DNS lookup request for Ip family %d pdp_type %d", z_dss_dns_addrinfo.ai_family, pdp_type, 0);
  ...
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "Get_addrinfo returned error %d", dss_errno, 0, 0);
  ...
  */
  ...
  if (DSS_ERROR == z_handle_table[q_handle_id].x_sockfd)
  {
    ...
    if (z_handle_table[q_handle_id].z_connect_params.e_link_type == LINK_TCP)
    {
      ...
      if (x_retval != DSS_ERROR)
      {
        MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH, "TCP NO DELAY set", 0, 0, 0);
      }
      else
      {
        MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH, "TCP NO DELAY set failed", 0, 0, 0);
      }
      ...
      if (x_retval != DSS_ERROR)
      {
        MSG(MSG_SSID_GPS, MSG_LEGACY_HIGH, "TCP SACK set");
      }
      else
      {
        MSG_1(MSG_SSID_GPS, MSG_LEGACY_HIGH, "TCP SACK set failed %d", x_retval);
      }
   ...
   pd_comms_connect_result_type *p_conn_error)
{
  ...
  *p_conn_error = PDCOMM_CONNECT_ERROR_UNKNOWN; /*default error state*/
  ...
  if (q_handle_id < PDCOMM_MAX_HANDLES)
  {
    ...
    {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "Invalid TCP ADDR TYPE specified in connect request for handle_id [%d]",
            q_handle_id, 0, 0);
  ...
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "CONNECT: handle_id %d out of range", q_handle_id, 0, 0);
    return FAILED;
  }
...
void pd_comms_tcp_task_release_app_id(sint15           x_app_id)
{
  ...
  if (x_retval == DSS_ERROR)
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: Close Netlib: Error %d for app_id [%d]",
          x_err_no, x_app_id, 0);
  }
  else
  {
   ...
   )
{
  ...
  if (DSS_ERROR == z_handle_table[z_handle.q_handle_id].x_app_id)
  {
    ...
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED, "PDP_TYPE SLP connection:%d ,Z_Policy IP Family:%d", pdp_type, z_policy_info.family, 0);
    ...
    if (DSS_ERROR == x_retval)
    {
      if (DS_EWOULDBLOCK != x_err_no)
      {
        ...
        print_dss_err(x_err_no);
        MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED, "PPP Failed. Closing netlib", 0, 0, 0);
   ...
   )
{
  ...
  if (x_retval == DSS_SUCCESS)
  {
    ...
    if (x_retval == DSS_ERROR)
    {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: Close Netlib: Error %d for handle_id [%d]",
            x_err_no, q_handle_id, 0);
      return PDCOMM_RESULT_ERROR_UNKNOWN;
    }
    else
  ...
  else
  {
    switch (x_err_no)
    {
      ...
      case DS_EBADAPP:
        {
          MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: PPP Close - Invalid App ID: %d for handle_id [%d]",
      ...
      default:
        {
          MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: PPP Close Error: %d for handle_id [%d]",
                x_err_no, q_handle_id, 0);
          return PDCOMM_RESULT_ERROR_UNKNOWN;
        }
    } /*switch (err_no) ends*/
   ...
   pd_comms_handle_type z_handle)
{
  ...
  sint15 x_err_no;            /* error value as returned from dss functions */
  sint31 x_retval;
  ds_sys_conf_emer_call_apn_type apn_type_mask = { 0 };
  int16 x_ds_retval = 0;
  ...
  else
  {
    ...
    if (SYS_SYS_MODE_WLAN == pd_comms_tcp_get_sys_mode())
    {
      z_open_params.z_open_params.bearer_type = PDSM_ATL_BEARER_TYPE_WLAN;
    }
    else
    {
      z_open_params.z_open_params.bearer_type = PDSM_ATL_BEARER_TYPE_WWAN;
    }
    ...
    if (z_handle_table[z_handle.q_handle_id].z_connect_params.z_link_params.z_tcp_params.profile_type == PDCOMM_APN_PROFILE_TYPE_LBS)
    {
      ...
      z_open_params.z_open_params.apn_type = PDSM_ATL_APN_TYPE_MASK_SUPL;
    }
    else if (z_handle_table[z_handle.q_handle_id].z_connect_params.z_link_params.z_tcp_params.profile_type == PDCOMM_APN_PROFILE_TYPE_LBS_EMERGENCY)
    {
      /* 1)Set the connection type
         2)Determine the APN Type which is used for this Emergency connection*/
      z_open_params.z_open_params.connection_type = PDSM_ATL_CONNECTION_LBS_EMERGENCY;
      x_ds_retval = ds_sys_conf_get_ex(DS_SYS_TECH_ALL,
                                       DS_SYS_CONF_EMERGENCY_CALL_APN_TYPE,
                                       (pd_comms_tcp_task_map_sys_asid_to_ds_asid(pd_comms_nv_efs_get_current_dd_sub())),
                                       (void *)&apn_type_mask,
                                       &x_err_no);
      //Todo; make note in SDS:      PS data off , APN type configuration, for these two we use get_ex() and call with correct sub id
      if ((DSS_ERROR == x_ds_retval) ||
          (DS_SYS_APN_TYPE_MAX <= apn_type_mask.apn_type))
      {
        PDCOMM_APP_MSG_MED("Apn Type UNSPEC", 0, 0, 0);
        z_open_params.z_open_params.apn_type = PDSM_ATL_APN_TYPE_MASK_EMERGENCY;
      }
      else
      {
        z_open_params.z_open_params.apn_type = pd_comms_tcp_task_map_ds_apn_type(apn_type_mask.apn_type);
      }
    }
    else
    {
      z_open_params.z_open_params.connection_type = PDSM_ATL_CONNECTION_WWAN_INTERNET;
      z_open_params.z_open_params.apn_type = PDSM_ATL_APN_TYPE_MASK_DEFAULT;
    }

    z_open_params.z_open_params.as_id = pd_comms_nv_efs_get_current_dd_sub();

    PDCOMM_APP_MSG_MED("Apn Type: 0x%1x, Bearer Type 0x%1x, Connection Type: %d",
                       z_open_params.z_open_params.apn_type,
                       z_open_params.z_open_params.bearer_type,
                       z_open_params.z_open_params.connection_type);

    MSG_1(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "SUB ID in connection request to HLOS %d", z_open_params.z_open_params.as_id);

    /*Call the ATL function provided by PPP proxy to open PPP link*/
    e_return = pd_comms_atl_i_open(z_handle, &z_open_params);

    /*In succes or wait case Set the ppp down flag to false, It means that ATL connection is open, 
    It helps in identifying at any point of time if the connection is open or not and subsequently prevents redundant closure, 
    If open fails then it will be handled in the DS_ENETNONET and the flag will be reset*/

    if (e_return == PDCOMM_RESULT_WAIT)
    {
      PDCOMM_APP_MSG_MED("ATL open returned WAIT for handle_id[%d]", z_handle.q_handle_id, 0, 0);
      z_handle_table[z_handle.q_handle_id].is_ppp_down = FALSE;
      return PENDING;
    }
   ...
   secerrno_enum_type e_error)
{
  ...
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "Got unknown SSL Close callback (handle_id = %d)", q_handle_id, 0, 0);
  }
} /*lint !e818 */
   ...
   )
{
  ...
  if (alert_mask & SECSSL_ALERT_BAD_CERTIFICATE)
  {
    ...
    /*Not closing the connection for BAD CERT alert here . Instead connection will be closed upon receiving
     SSL auth failure event and socket connection re-attempt will be made with auto hslp.*/
    u_closeConnection = FALSE;
  }
  ...
  if (u_closeConnection)
  {
    ...
    if (e_error != E_SUCCESS)
    {
      ...
      {
        MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: Could not close SSL session. Error: %d for handle_id [%d]",
   ...
   )
{
  ...
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "Got unknown SSL event callback (handle_id = %d)", q_handle_id, 0, 0);
  }
  ...
  do
  {
    ...
    if (e_error != E_SUCCESS)
    {
      switch (e_error)
      {
        case E_WOULD_BLOCK:
          {
            ...
            if ((*p_num_bytes_already_read == *p_num_bytes_to_read) || (u_wait_for_all == FALSE))
            {
              ...
              (void)pd_comms_senddata_to_client(z_handle_table[q_handle_id].z_handle,
                                                ...
                                                z_handle_table[q_handle_id].e_as_id, //default sub=-1
                                                &z_params
                                                );
        ...
        default:
          {
            MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: Secure socket read - Unknown Error: %d for handle_id [%d]",
    ...
    else
    {
        ...
        *p_num_bytes_already_read += w__bytes_read;
        ...
        if ((*p_num_bytes_already_read == *p_num_bytes_to_read) || (u_wait_for_all == FALSE))
        {
          ...
          (void)pd_comms_senddata_to_client(z_handle_table[q_handle_id].z_handle,
                                            ...
                                            z_handle_table[q_handle_id].q_base_msg_id,
                                            z_handle_table[q_handle_id].q_read_transaction_id,
                                            z_handle_table[q_handle_id].e_as_id,
                                            &z_params);
      ...
      {
        MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP:Recevied %d bytes for handle_id [%d]", w__bytes_read, q_handle_id, 0);
      }
  ...
  if (q_handle_id < PDCOMM_MAX_HANDLES)
  {
    ...
    switch (e_event)
    {
      case SECSSL_SES_SOCK_READ_EVENT:
        {
          ...
          {
            MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: SSL READ event while link is DISCONNECTED for handle_id [%d]",
                  q_handle_id, 0, 0);
          }
          break;
        } /*case SECSSL_SES_SOCK_READ_EVENT*/
      ...
      case SECSSL_SES_AUTH_FAILURE_EVENT:
        {
          /* Set the socket linger time to be 50 msec so that the TCP block does not remain in
   ...
   pd_comms_handle_type z_handle)
{
  ...
  if (e_error != E_SUCCESS)
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: Error: %d setting SSL parameter", e_error, 0, 0);
    return FALSE;
  }
  ...
  if (e_error != E_SUCCESS)
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: Error: %d setting SSL parameter to ignore certificate expiration check", e_error, 0, 0);
    return FALSE;
  }
  ...
  else if (z_handle_table[z_handle.q_handle_id].z_connect_params.e_ssl_ver_tls == SSL_VER_TLS_1_2)
  {
    z_ssl_params.ssl_version_pdata = SECSSL_SSL_VER_TLS12;
  }

  MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH, "TLS version being used is %d", z_ssl_params.ssl_version_pdata, 0, 0);
  ...
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED, "PDCOMM APP: No Root Cert Dir", 0, 0, 0);
    ...
    {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: Error Retrieving Root Certificate", 0, 0, 0);
      return FALSE;
    }
  ...
  else
  {
    ...
    if ((e_error = secssl_set_sesparam(*p_ssl_handle, SECSSL_SES_CA_CERT_FOLDER, &z_ssl_params))
        != E_SUCCESS)
    {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: Error: %d setting SSL cert folder", e_error, 0, 0);
      return FALSE;
    }
    else
    {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH, "PDCOMM APP: Root Cert Dir set", 0, 0, 0);
    }
  }
  ...
  /* Enable Dynamic Certificate Sync capability for SecSSL sessions.
     Enable SecSSL module to sync Certificate from AP store if not available in /SUPL folder in modem EFS*/
  z_ssl_params.enable_dynamic_certs = TRUE;
  if ((e_error = secssl_set_sesparam(*p_ssl_handle, SECSSL_SES_ENABLE_DYNAMIC_CERTS, &z_ssl_params))
      != E_SUCCESS)
  {
    MSG_1(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: Error: %d setting SSL parameter Dynamic Cert Sync", e_error);
    return FALSE;
  }
   ...
   pd_comms_handle_type z_handle)
{
  ...
  switch (e_error)
  {
    ...
    default:
      {
        /* An error occurred */
        MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: Error: %d opening SSL session for handle_id [%d]",
   ...
   pd_comms_handle_type *p_handle)
{
  ...
  /*Get the mutex lock since get_handle could be called simultaneously by
    two PDCOMM clients*/
  MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED, "PDCOMM APP:Trying to acquire GetHandle Mutex lock", 0, 0, 0);
  os_MutexLock(&z_gethandle_mutex);
  MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED, "PDCOMM APP:GetHandle Mutex lock acquired", 0, 0, 0);
  ...
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "************ No more handles available *************", 0, 0, 0);
    os_MutexUnlock(&z_gethandle_mutex);
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED, "PDCOMM APP:GetHandle Mutex unlocked", 0, 0, 0);
   ...
   boolean u_handle_pending_connect)
{
  ...
  int16 dss_errno = 0;
  int16 dss_ret_val = 0;
  ds_sys_ps_data_avail_type z_ps_data_status;
#endif /* ! FEATURE_NO_PS_DATA_OFF_SUPPORT */
  ...
  if (l_i >= PDCOMM_MAX_HANDLES || z_handle_table[l_i].in_use == FALSE)
  {
    ...
    (void)pd_comms_notify_client(z_handle,
                                 ...
                                 ERROR_RPT, &z_params);
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM_TASK: ************INVALID handle received *************", 0, 0, 0);
...
#ifndef FEATURE_NO_PS_DATA_OFF_SUPPORT
  if (PDCOMM_APN_PROFILE_TYPE_LBS_EMERGENCY != z_handle_table[l_i].z_connect_params.z_link_params.z_tcp_params.profile_type)
  {
    MSG_2(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "sys_conf_get_ex called with as_id: %d, DD Sub:%d ", pd_comms_nv_efs_get_current_dd_sub(),
          pd_comms_tcp_task_map_sys_asid_to_ds_asid(pd_comms_nv_efs_get_current_dd_sub()));
    dss_ret_val = ds_sys_conf_get_ex(DS_SYS_TECH_3GPP,
                                     DS_SYS_CONF_3GPP_PS_DATA_AVAIL_APPL_SERVICE_INFO,
                                     (pd_comms_tcp_task_map_sys_asid_to_ds_asid(pd_comms_nv_efs_get_current_dd_sub())),
                                     (void *)&z_ps_data_status,
                                     &dss_errno);
    if (DSS_ERROR != dss_ret_val)
    {
      MSG_1(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PS Data Status [%d]", z_ps_data_status.data_avail_status);
      ps_status = z_ps_data_status.data_avail_status;
    }

    if ((DS_SYS_PS_DATA_AVAIL_STATUS_OFF == ps_status) &&
        (TRUE == pd_comms_tcp_task_check_ps_status(&z_ps_data_status)))
    {
      pd_comms_notification_params_type z_params;

      z_params.e_new_state = PDCOMM_STATE_DISCONNECTED;
      z_params.q_result = (uint32)PDCOMM_CONNECT_ERROR_CONNECTION_REFUSED;

      z_handle.q_handle_id = l_i;
      z_handle.e_link_type = p_hdr_ptr->z_params.e_link_type;
...
DESCRIPTION
  This function sets the DSS_SO_LINGER option to set linger timeout to configured
  constant value [typically 50msec]. This is mainly used to close the socket
  connection immediately and not wait for any callbacks. The socket will be kept in
  waiting state for only the configured time after RST is sent to network. The
  linger time will make sure that the RST is succesfully sent to network from the
  lower layers. If linger timeout value is set to 0, RST is immediately sent out
  without any wait and soket close is attempted.
...
static void pd_comms_tcp_task_setsockopt(
   /*handle_id corresponding to the connection for which socket 
                                                                                 option is to be set*/
   uint32 q_handle_id,
   /*Boolean to check if it is a TCP abort*/
   boolean u_abort)
{
  ...
  if (z_handle_table[q_handle_id].x_sockfd != DSS_ERROR)
  {
    ...
    if (FALSE == u_abort)
    {
      /* Set timeout value of 50msec to close the connection */
      z_dss_so_linger.l_linger = PDCOMM_DEFAULT_SOCKET_LINGER_TIME_MSEC;
    }
    else
    {
      /*If it is for a TCP abort, set timeout value to 0 so that dss_close() will mostly
        return SUCCESS and no need to wait for socket callback to call dss_close() again*/
      z_dss_so_linger.l_linger = 0;
    }
  ...
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: Socket already closed before setsockopt for handle [%d]",
          q_handle_id, 0, 0);
  }
...

{
  ...
  if (z_handle_table[q_handle_id].x_sockfd != DSS_ERROR)
  {
    pd_comms_tcp_task_setsockopt(q_handle_id, TRUE);
    ...
    if (DSS_SUCCESS == x_dss_retval)
    {
      /*Reset the sockfd only if dss_close() returns SUCCESS. Otherwise sockfd is
        needed to call dss_close() again on socket event callback*/
      z_handle_table[q_handle_id].x_sockfd = DSS_ERROR;
    }
    else
    {
      ...
      switch (x_err_no)
      {
        case DS_EBADF:
          {
            PDCOMM_APP_MSG_HIGH("Socket Close - Bad Socket Descriptor for handle_id [%d]",
                                q_handle_id, 0, 0);
            break;
          }
        case DS_EWOULDBLOCK:
        ...
        default:
          {
            PDCOMM_APP_MSG_MED("Unknown socket close error: %d for handle_id [%d]",
                               x_err_no, q_handle_id, 0);
            break;
          }
      } /*switch(err_no) ends*/
      ...
      (z_handle_table[q_handle_id].z_dns_cb_data.dns_query_handle != DSS_DNS_QUERY_INVALID_HANDLE))
  {
    ...
    if (DSS_ERROR == x_retval)
    {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "DNS session %d could not be deleted",
    ...
    case PDCOMM_STATE_CONNECTING:
      {
        ...
        {
          if (PDCOMM_STATE_CONNECTING == z_handle_table[z_handle.q_handle_id].e_link_status)
          {
            e_error = secssl_abort_open_session(z_handle_table[z_handle.q_handle_id].z_ssl_handle);
          ...
          if (e_error != E_SUCCESS)
          {
            MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: Could not close SSL session. Error: %d", e_error, 0, 0);
...
#ifndef FEATURE_NO_PS_DATA_OFF_SUPPORT
/*===========================================================================
FUNCTION pd_comms_tcp_task_handle_ds_event

DESCRIPTION
  Function to handle DS Events. Currently handled events are listed below.
  1)DS_SYS_EVENT_3GPP_PS_DATA_AVAIL_CHANGE. 
...
SIDE EFFECTS
  None
===========================================================================*/
static void pd_comms_tcp_task_handle_ds_event(ds_sys_event_enum_type q_event)
{
  int l_i = 0;

  if ((DS_SYS_EVENT_3GPP_PS_DATA_AVAIL_CHANGE == q_event) &&
      (DS_SYS_PS_DATA_AVAIL_STATUS_OFF == ps_status))
  {
    /*If PS data is turned OFF ensure all TCP connections are torn down. 
      PS data does not affect Emergency connections. */
    for (l_i = 0; l_i < PDCOMM_MAX_HANDLES; l_i++)
    {
      if ((TRUE == z_handle_table[l_i].in_use) &&
          (LINK_TCP == z_handle_table[l_i].z_connect_params.e_link_type) &&
          (PDCOMM_APN_PROFILE_TYPE_LBS_EMERGENCY != z_handle_table[l_i].z_connect_params.z_link_params.z_tcp_params.profile_type))
      {
  ...
  return;
} /*pd_comms_tcp_task_handle_ds_event*/

/*===========================================================================
FUNCTION pd_comms_tcp_task_handle_ds_event_cb

DESCRIPTION
  Whenever there is a DS event callback, an IPC message is sent to 
  PDCOMM APP. This IPC message is handled in this function. Based upon the 
  recevied event type it takes the appropriate action. 
...
SIDE EFFECTS
  None
===========================================================================*/
static void pd_comms_tcp_task_handle_ds_event_cb(
   /*Pointer to IPC message containing the DS event callback information */
   const os_IpcMsgType *p_msg)
{
  ds_sys_event_enum_type q_ds_event = DS_SYS_EVENT_MIN;
  pd_comms_tcp_task_ds_event_data_type *pz_ds_event = NULL;
  ds_sys_ps_data_avail_type *pz_ps_data_avail = NULL;

  if (p_msg == NULL)
  {
    MSG(MSG_SSID_GPS, MSG_LEGACY_ERROR, "NULL IPC received in security_close callback");
    return;
  }

  q_ds_event = ((pd_comms_tcp_task_ds_event_cb_ipc_hdr_type *)p_msg->p_Data)->ds_event_name;
  pz_ds_event = &(((pd_comms_tcp_task_ds_event_cb_ipc_hdr_type *)p_msg->p_Data)->ds_event);
  pz_ps_data_avail = &(pz_ds_event->ps_data_avail_type);
  ...
  MSG_1(MSG_SSID_GPS, MSG_LEGACY_MED, "Received DS event callback, event: %d", q_ds_event);

  if (DS_SYS_EVENT_3GPP_PS_DATA_AVAIL_CHANGE == q_ds_event)
  {
    /*update the PS DATA Status global flag and take appropriate action*/
    ps_status = pz_ds_event->ps_data_avail_type.data_avail_status;
    MSG_1(MSG_SSID_GPS, MSG_LEGACY_MED, "Received PS status: %d",
          pz_ds_event->ps_data_avail_type.data_avail_status);

    if ((DS_SYS_PS_DATA_AVAIL_STATUS_OFF == ps_status) &&
        (TRUE == pd_comms_tcp_task_check_ps_status(pz_ps_data_avail)))
    {
      pd_comms_tcp_task_handle_ds_event(q_ds_event);
    }
  }
  return;
} /*pd_comms_tcp_task_handle_ds_event_cb*/
#endif /* ! FEATURE_NO_PS_DATA_OFF_SUPPORT */
   ...
   boolean u_wait_for_all)
{
  ...
  uint16 sock_addr_len = 0;
  boolean u_close_ppp = TRUE;
  sys_modem_as_id_e_type         as_id = PD_COMMS_CONFIG_MIN_SUB_ID;

  /*----UDP dss_recvmsg() related definitions----*/
  struct dss_msghdr pd_comm_udp_msghdr;
  struct ps_iovec msg_iov;
  struct dss_cmsghdr *cntl_msg_header;
  struct dss_msghdr *udp_msg = &pd_comm_udp_msghdr;
  void   *cntl_data = NULL;
  uint8  cntl_len = 0;
  dss_in_recv_subs_id_type *revc_subIdInfo;
  /*--------------------------------------------*/
  ...
  {
    pd_comm_udp_msghdr.msg_name = sock_addr;
    pd_comm_udp_msghdr.msg_namelen = sock_addr_len;                           /* size of protocol address */
    msg_iov.ps_iov_base = (byte *)&p_read_data[*p_num_bytes_already_read];    /* Read buffer address*/
    msg_iov.ps_iov_len = (*p_num_bytes_to_read - *p_num_bytes_already_read);  /* number of bytes to read*/
    pd_comm_udp_msghdr.msg_iov = &msg_iov;                                    /* Msg detaisl for dss_recvmsg*/
    pd_comm_udp_msghdr.msg_iovlen = 1;                                        /*# elements in msg_iov*/
    pd_comm_udp_msghdr.msg_flags = 0;

    /*Message control information which contains the SUB_ID info*/
    cntl_len = (sizeof(struct dss_cmsghdr) + sizeof(dss_in_recv_subs_id_type));
    cntl_data = (void *)os_MemAlloc(cntl_len, OS_MEM_SCOPE_TASK);
    /*If CNTL data get Fails we x_bytes_read = 0 will ensure we send an error back to client*/
    x_bytes_read = 0;

    /* Ensure we free the memory allocated for cntl_data in case we need to return in error case */
    if (NULL != cntl_data)
    {
      memset(cntl_data, 0, sizeof(cntl_len));

      pd_comm_udp_msghdr.msg_control = (void *)cntl_data;                      /* CNTL data header + CNTL data payload*/
      pd_comm_udp_msghdr.msg_controllen = cntl_len;                           /* CNTL data length (cntl_hdr + cntl_data)*/

      x_bytes_read = dss_recvmsg(z_handle_table[q_handle_id].x_sockfd,
                                 &pd_comm_udp_msghdr,
                                 0,
                                 &x_err_no);

      cntl_msg_header = DSS_CMSG_FIRSTHDR(udp_msg);                            /* CNTL data get()*/
      if (NULL == cntl_msg_header)
      {
        MSG_2(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: Socket Read - UDP read failed for SocektFD: %d for handle_id [%d]",
              z_handle_table[q_handle_id].x_sockfd, q_handle_id);
        e_read_error = PDCOMM_READ_ERROR_INTERNAL;
        result = FALSE;
      }
      else if (DSS_IP_RECVSUBID == cntl_msg_header->cmsg_type)                /*CNTL data type SUB ID*/
      {
        revc_subIdInfo = (dss_in_recv_subs_id_type *)DSS_CMSG_DATA(cntl_data);   /* CNTL data with DS SUB ID info*/
        z_handle_table[q_handle_id].e_as_id = pd_comms_tcp_task_map_ds_asid_to_sys_asid(revc_subIdInfo->subs_id);

        MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH, "PDCOMM APP: Socket Read - UDP msg read SocketFD: %d DS SubID [%d], CM SubID [%d]",
              z_handle_table[q_handle_id].x_sockfd,
              revc_subIdInfo->subs_id,
              z_handle_table[q_handle_id].e_as_id);
      }
      else
      {
        MSG_2(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: Socket Read - unhandled CNTL MSG type SocketFD: %d for handle_id [%d]",
              z_handle_table[q_handle_id].x_sockfd, q_handle_id);
        e_read_error = PDCOMM_READ_ERROR_INTERNAL;
        result = FALSE;
      }
      /* Ensure we free the memory allocated earlier */
      os_MemFree(&cntl_data);
    }
  }
    ...
    else
    {
      *p_num_bytes_already_read += (uint16)x_bytes_read;
      ...
      if ((*p_num_bytes_already_read == *p_num_bytes_to_read) || (FALSE == u_wait_for_all))
      {
        ...
        /*if the link type is UDP, then only fill the as id field in z_params.*/
        if (z_handle_table[q_handle_id].z_connect_params.e_link_type == LINK_UDP)
        {
          as_id  = z_handle_table[q_handle_id].e_as_id;
        }
  ...
  else if (x_bytes_read == DSS_ERROR)
  {
    switch (x_err_no)
    {
      case DS_EBADF:
        {
          MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: Socket Read - Invalid Socket Descriptor: %d for handle_id [%d]",
      ...
      case DS_ENOTCONN:
        {
          MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: Socket Read - No TCP Connection for handle_id [%d]",
                q_handle_id, 0, 0);
      ...
      case DS_ECONNRESET:
        {
          MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: Socket Read - TCP Connection Reset By Server for handle_id [%d]",
                q_handle_id, 0, 0);
          e_read_error = PDCOMM_READ_ERROR_TCP_CONNECTION_LOST;
          result = FALSE;
          break;
      ...
      case DS_ECONNABORTED:
        {
          MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: Socket Read - TCP Connection Aborted for handle_id [%d]",
                q_handle_id, 0, 0);
          e_read_error = PDCOMM_READ_ERROR_TCP_CONNECTION_LOST;
          result = FALSE;
          break;
      ...
      case DS_EIPADDRCHANGED:
        {
          MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: Socket Read - PPP Resync caused IP Address Change for handle_id [%d]",
                q_handle_id, 0, 0);
          e_read_error = PDCOMM_READ_ERROR_TCP_CONNECTION_LOST;
          result = FALSE;
          break;
      ...
      case DS_EPIPE:
        {
          MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: Socket Read - Peer closed TCP connection for handle_id [%d]",
                q_handle_id, 0, 0);
          e_read_error = PDCOMM_READ_ERROR_TCP_CONNECTION_LOST;
          result = FALSE;
          break;
      ...
      case DS_ENETDOWN:
        {
          MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: Socket Read - Network Subsystem Unavailable for handle_id [%d]",
                q_handle_id, 0, 0);
      ...
      case DS_EFAULT:
        {
          MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: Socket Read - App Buff Not Valid Part Of Addr Space for handle_id [%d]",
                q_handle_id, 0, 0);
          e_read_error = PDCOMM_READ_ERROR_INTERNAL;
          result = FALSE;
          break;
      ...
      case DS_EEOF:
        {
          MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: Socket Read - EOF recevied for handle_id [%d]",
                q_handle_id, 0, 0);
          e_read_error = PDCOMM_READ_ERROR_INTERNAL;
          result = FALSE;
          break;
      ...
      default:
        {
          MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: Socket Read - Unknown Error: %d for handle_id [%d]",
                x_err_no, q_handle_id, 0);
                                                      ...
                                                      dss_iface_id_type iface_id)
{
  if(p_policy_info == NULL)
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"Null policy info pointer",0,0,0);
    return;
  }
      ...
      z_handle_table[q_handle_id].z_connect_params.e_link_type == LINK_UDP)
  {
    ...
    if (DSS_ERROR == z_handle_table[q_handle_id].x_sockfd)
    {
      ...
      /*Set socket option*/
      {
        ...
        if (x_retval == DSS_ERROR)
        {
          PDCOMM_APP_MSG_ERROR("Set Socket option DSS_IP_RECVSUBID failed", z_handle_table[q_handle_id].x_sockfd, x_err_no, 0);
          (void)dss_close(z_handle_table[q_handle_id].x_sockfd, &x_err_no);
          z_handle_table[q_handle_id].x_sockfd = DSS_ERROR;
          return FAILED;
        }
      ...
      /*Install filters V4 and V6 only once for the first SUB*/
   ...
   const pd_comms_handle_type *p_handle)
{
  ...
  if (p_handle == NULL)
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP:NULL handle received in relase handle request", 0, 0, 0);
    return PDCOMM_RESULT_ERROR_UNKNOWN;
  }
  ...
  if (q_handle_id >= PDCOMM_MAX_HANDLES)
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP:release_handle request received with invalid handle[%d]", q_handle_id, 0, 0);
    return PDCOMM_RESULT_ERROR_UNKNOWN;
  }
  ...
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP:release_handle request received for free handle[%d]", q_handle_id, 0, 0);
    os_MutexUnlock(&z_gethandle_mutex);
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED, "PDCOMM APP:GetHandle Mutex unlocked", 0, 0, 0);
    return PDCOMM_RESULT_ERROR_UNKNOWN;
  }
   ...
   pd_comms_handle_type z_handle)
{
  ...
  else
  {
    ...
    switch (x_err_no)
    {
      case DS_EBADF:
        {
          MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: Socket Write - Invalid Socket Descriptor: %d",
      ...
      case DS_ENOTCONN:
        {
          MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: Socket Write - No TCP Connection", 0, 0, 0);
      ...
      case DS_ECONNRESET:
        {
          MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: Socket Write - TCP Connection Reset By Server", 0, 0, 0);
          e_write_error = PDCOMM_WRITE_ERROR_WRITE_TCP_CONNECTION_LOST;
          result = FALSE;
          *w_num_bytes_written = 0;
          z_handle_table[z_handle.q_handle_id].w_num_bytes_to_write = 0;
      ...
      case DS_ECONNABORTED:
        {
          MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: Socket Write - TCP Connection Aborted", 0, 0, 0);
      ...
      case DS_EIPADDRCHANGED:
        {
          MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: Socket Write - PPP Resync caused IP Address Change", 0, 0, 0);
      ...
      case DS_EPIPE:
        {
          MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: Socket Write - Peer closed TCP connection", 0, 0, 0);
      ...
      case DS_ENETDOWN:
        {
          MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: Socket Write - Network Subsystem Unavailable", 0, 0, 0);
          e_write_error = PDCOMM_WRITE_ERROR_WRITE_PPP_CONNECTION_LOST;
          result = FALSE;
          *w_num_bytes_written = 0;
          z_handle_table[z_handle.q_handle_id].w_num_bytes_to_write = 0;
      ...
      case DS_EFAULT:
        {
          MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: Socket Write - App Buff Not Valid Part Of Addr Space", 0, 0, 0);
          e_write_error = PDCOMM_WRITE_ERROR_WRITE_INTERNAL_ERROR;
          result = FALSE;
          *w_num_bytes_written = 0;
          z_handle_table[z_handle.q_handle_id].w_num_bytes_to_write = 0;
      ...
      case DS_EWOULDBLOCK:
        {
          ...
          MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED, "PDCOMM APP: Socket Write - No Free Buffers", 0, 0, 0);
      ...
      default:
        {
          MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: Socket Write - unknown error %d for handle_id [%d]",
                z_handle.q_handle_id, 0, 0);
          e_write_error = PDCOMM_WRITE_ERROR_WRITE_INTERNAL_ERROR;
          result = FALSE;
          *w_num_bytes_written = 0;
   ...
   os_IpcMsgType *p_msg)
{
  ...
  if (z_handle.q_handle_id < PDCOMM_MAX_HANDLES)
  {
           ...
           z_handle_table[z_handle.q_handle_id].w_num_bytes_to_write)
    {
      ...
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "Write called while previous write still pending for handle_id [%d]",
            z_handle.q_handle_id, 0, 0);
    ...
    switch (z_handle_table[z_handle.q_handle_id].z_connect_params.e_link_type)
    {
      ...
      default:
        {
          MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: Invalid link type [%d] recevied in write request for handleid [%d]",
   ...
   os_IpcMsgType *p_msg)
{
  ...
  if (z_handle.q_handle_id < PDCOMM_MAX_HANDLES)
  {
           ...
           z_handle_table[z_handle.q_handle_id].w_num_bytes_to_read)
    {
      ...
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "Read called while previous read still pending for handle_id [%d] read %d toread %d",
            z_handle.q_handle_id,
    ...
    switch (z_handle_table[z_handle.q_handle_id].z_connect_params.e_link_type)
    {
      ...
      default:
        {
          MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: Invalid link type [%d] received in read request for handle id [%d]",
                z_handle_table[z_handle.q_handle_id].z_connect_params.e_link_type, z_handle.q_handle_id, 0);
        }
    } /*switch(z_handle_table[z_handle.q_handle_id].z_connect_params.e_link_type)*/
  } /*if( z_handle.q_handle_id < PDCOMM_MAX_HANDLES )*/
   ...
   os_IpcMsgType *p_msg)
{
  ...
  if ((p_hdr_ptr->e_hash_type != SHA1) || (p_hdr_ptr->q_msg_len > 65535))
  {
    ...
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: Invalid parameters in hash request for transaction_id [%d]",
    ...
    if (e_result != PDCOMM_RESULT_OK)
    {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: IPC error %d for transaction_id [%d]",
  ...
  if (e_error != E_SUCCESS)
  {
    ...
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: Error %d when generating HMAC for transaction_id [%d]",
   ...
   uint32 q_handle_id)
{
  ...
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "pd_comms_tcp_task_dss_unforce_dormancy: handle_id %d out of range", q_handle_id, 0, 0);
    return FALSE;
  }
  ...
  if (q_handle_id < PDCOMM_MAX_HANDLES)
  {
    ...
    */
    if (DSS_ERROR == z_handle_table[q_handle_id].x_app_id)
    {
      ...
      {
        MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "Could not get App id", 0, 0, 0);
        return FALSE;
      }
   ...
   os_IpcMsgType *p_msg)
{
  ...
  if (p_msg == NULL)
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: NULL mesg received in ioctl call", 0, 0, 0);
    return;
  }
  /*lint -e826 */
   ...
   const os_IpcMsgType *p_msg)
{
  ...
  boolean u_close_ppp = TRUE;
  secerrno_enum_type e_error;
  pd_comms_state_type e_new_pdcomm_state = PDCOMM_STATE_DISCONNECTED;
  ...
  if (q_handle_id < PDCOMM_MAX_HANDLES)
  {
    ...
    switch (x_err_no)
    {
      case DS_EBADAPP:
        {
          ...
          MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: Invalid App Id: %d", z_handle_table[q_handle_id].x_app_id, 0, 0);
          ...
          {
            print_dss_err(x_err_no);
            MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"PDCOMM APP: Close Netlib: Error %d for handle_id [%d]", x_err_no, q_handle_id, 0);
          }
          else
          {
            PDCOMM_APP_MSG_MED("Netlib Closed for handle_id [%d]",q_handle_id,0,0);
      ...
      case DS_ENETNONET:
        {
          /*Set the ppp down flag to TRUE, It means that ATL connection is closed, It could be due to expilicit call of ATL close or due to any Network error.
          It helps in identifying at any point of time if the connection is open or not and subsequently prevents redundant closure.*/
          z_handle_table[q_handle_id].is_ppp_down = TRUE;
          ...
          if (z_handle_table[q_handle_id].x_sockfd != DSS_ERROR)
          {
            ...
            {
              print_dss_err(x_err_no);
              /* dss_close can return e_wouldBlock , so we have to call dss_close again on socket callback
                 hence move to disconnecting state so that on socket cb, we can again call dss_close
              */
              e_new_pdcomm_state = PDCOMM_STATE_DISCONNECTING;
            }
            else
            {
              z_handle_table[q_handle_id].x_sockfd = DSS_ERROR;
          ...
          if (z_handle_table[q_handle_id].e_link_status == PDCOMM_STATE_DISCONNECTING)
          {
            ...
            pd_comms_tcp_task_update_state(z_handle_table[q_handle_id].z_handle, e_new_pdcomm_state,
                                           TRUE, (uint32)PDCOMM_RESULT_OK);
          }
          else
          {
      ...
      case DS_ENETCLOSEINPROGRESS: /* DS_ENETCLOSEINPROGRESS should not happen for 6800 and 7500 onwards */
        {
          MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: DS_ENETCLOSEINPROGRESS ", 0, 0, 0);
          break;
        }
      ...
      default:
        {
          MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP:Unknown Network Error [%d] for handle_id [%d]",
                x_err_no, q_handle_id, 0);
          break;
        }
    } /* switch(err_no) ends */
  ...
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "CHECK NETWORK: handle_id %d out of range", q_handle_id, 0, 0);
  }
   ...
   const os_IpcMsgType *p_msg)
{
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "=PDCOMM APPLICATION= Received SOCKET callback for handle_id [%d] with event: %d for app_id[%d]",
        q_handle_id,
        ((pd_comms_tcp_task_tcp_cb_ipc_hdr_type *)p_msg->p_Data)->event_mask,
        x_appid);
  ...
  if (q_handle_id < PDCOMM_MAX_HANDLES)
  {
    ...
    if (l_result & DS_CLOSE_EVENT)
    {
          ...
          )
      {
        ...
        if (x_temp_err_no == DS_EIPADDRCHANGED)
        {
          ...
          MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH, "Ip Address changed, retrying socket connection on new socket", 0, 0, 0);
          MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH, "Close previous socket", 0, 0, 0);
          ...
          {
            pd_comms_connect_result_type e_conn_error;
            MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH, "Connect on new socket", 0, 0, 0);
            z_handle_table[q_handle_id].x_sockfd = DSS_ERROR;
            e_status = pd_comms_tcp_task_connect_socket(q_handle_id, &e_conn_error);

            if (e_status == SUCCEEDED)
            {
              MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED, "Socket conneciton succesfull for new socket", 0, 0, 0);
              /* Enable read data and close event */
            ...
            {
              MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED, "Socket conneciton pending for new socket", 0, 0, 0);
              return;
            }
          ...
          {
            MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "Error in closing previous socket for Ip addr change", 0, 0, 0);
          }
        }
      }
    ...
    if (l_result & DS_WRITE_EVENT)
    {
      ...
      if (z_handle_table[q_handle_id].e_link_status == PDCOMM_STATE_CONNECTING)
      {
        ...
        if (FALSE == result)
        {
          ...
          MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: Socket Connection Failed for handle_id",
                q_handle_id, 0, 0);
    ...
    if (l_result & DS_READ_EVENT)
    {
      ...
      else
      {
        ...
        {
          MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: READ event while link is DISCONNECTED for handle_id [%d]",
                q_handle_id, 0, 0);
        }
      }
    } /*if( l_result & DS_READ_EVENT )*/
    ...
    case PENDING:
      {
        ...
        MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: DNS Lookup state still pending for handle id [%d]. DISCONNECTING",
              q_handle_id, 0, 0);
  ...
  for (l_i = 0; l_i < PDCOMM_MAX_HANDLES; l_i++)
  {
        ...
        z_handle_table[l_i].z_connect_params.e_link_type != LINK_SMS)
    {
      ...
      if (z_handle_table[l_i].l_inactivity_count >= PDCOMM_INACTIVITY_TIMEOUT)
      {
        ...
        z_handle_table[l_i].l_inactivity_count = 0;
        MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: Handle [%d] inactive for more than %d millisec. Aborting connection..",
              l_i, PDCOMM_INACTIVITY_TIMER_PERIOD, 0);

        if (z_handle_table[l_i].e_link_status == PDCOMM_STATE_DISCONNECTED)
        {
          MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: Client already notified about aborted connection. Release Handle [%d]",
                l_i, 0, 0);
        }
        else
        {
...
{
  pd_comms_sys_info_change_params_type *p_sys_info_change_params;
  sys_modem_as_id_e_type dd_sub;
  ...
  p_sys_info_change_params = (pd_comms_sys_info_change_params_type *)(p_msg->p_Data);

  srv_sys_mode = (sys_sys_mode_e_type)(p_sys_info_change_params->srv_system);
  dd_sub = (sys_modem_as_id_e_type)(p_sys_info_change_params->dd_sub);
  as_id  = (sys_modem_as_id_e_type)(p_sys_info_change_params->as_id);
  MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH,
        "Pdcomm: Serving System changed srv_sys= %d, dd_sub=%d, as_id=%d", srv_sys_mode, dd_sub, as_id);

  /*In case of dd_sub change, set correct DD and read NV values from cache for new DD*/
  if (dd_sub != pd_comms_nv_efs_get_current_dd_sub())
  {
    pd_comms_nv_efs_set_current_dd_sub(dd_sub);

    //read NVs from cache for correcy DD sub
    pd_comms_tcp_task_read_nvs(FALSE, pd_comms_nv_efs_get_current_dd_sub());
  }
...
#ifndef FEATURE_NO_PS_DATA_OFF_SUPPORT
      case PDCOMM_APP_TASK_DSS_EVENT_CB:
        {
          pd_comms_tcp_task_handle_ds_event_cb(p_msg);
          break;
        }
#endif /* ! FEATURE_NO_PS_DATA_OFF_SUPPORT */
      ...
      case C_OS_MSG_ID_TIMER_EXPIRY:
        {
          ...
          {
            MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: Invalid timer id %d in timeout message",
   ...
   dword q_dummy)
{
...
#ifndef FEATURE_NO_PS_DATA_OFF_SUPPORT
  pd_comms_tcp_task_register_ds_callbacks();
#endif /* ! FEATURE_NO_PS_DATA_OFF_SUPPORT */

  /* Create a new Hash function crypto instance */
  /*if( secapi_new( &z_hash_handle, SECAPI_SHA ) != E_SUCCESS )
  {  
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"PDCOMM APP: Could not create a hash function instance",0,0,0);
  } */
  ...
  for (;;)
  {
    ...
    if (p_msg == NULL)
    {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM APP: NULL message received", 0, 0, 0);
    }
    else
    {
                                                 ...
                                                 sys_modem_as_id_e_type sub)
{
  ...
  if (pd_comms_nv_efs_reg_item_read(item,
                                    ...
                                    &nv_apn_profiles, force_read_from_hw, sub) == TRUE)
  {
...
{
  sys_modem_as_id_e_type refreshed_sub = PD_COMMS_CONFIG_MIN_SUB_ID;
  ...
  refreshed_sub = (uint8)p_info->sub_index; /* value 0,1 or 2 */

  if ((refreshed_sub < PD_COMMS_CONFIG_MIN_SUB_ID) || (refreshed_sub > PD_COMMS_CONFIG_MAX_SUB_ID))
  {
    MSG_1(MSG_SSID_GPS, MSG_LEGACY_ERROR, "NV Refresh Invalid sub %u. Abort Refresh on this SUB",
          refreshed_sub);
    return FALSE;
  }

  if (refreshed_sub == pd_comms_nv_efs_get_current_dd_sub())
  {
    /*Read LBS Apn profile from hardware for Hot SIM swap */
    pd_comms_tcp_task_read_apn_profiles_from_nv(CGPS_NV_EFS_PDCOMM_LBS_APN_PROFILES,
                                                PDCOMM_APN_PROFILE_TYPE_LBS, TRUE, refreshed_sub);

    /* Read Emergency LBS Apn profile from hardware for Hot SIM swap */
    pd_comms_tcp_task_read_apn_profiles_from_nv(CGPS_NV_EFS_PDCOMM_EMERGENCY_LBS_APN_PROFILES,
                                                PDCOMM_APN_PROFILE_TYPE_LBS_EMERGENCY, TRUE, refreshed_sub);
  }
  else
  {
    MSG_1(MSG_SSID_GPS, MSG_LEGACY_MED, "PDCOMM MCFG_REFRESH refresh ignored on non-DD sub, RefID=%d", p_info->reference_id);
    return FALSE;
  }
...
void pd_comms_tcp_task_handle_set_data_enable(boolean data_enable)
{
  ...
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH, "Data is Enabled", 0, 0, 0);
  }
  else
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH, "Data is Disabled", 0, 0, 0);
  }
}
...
void pd_comms_log_connection_status_event(pd_event_e_type event_type)
{
  ...
  uint8 i;
  MSG_3(MSG_SSID_GPS, MSG_LEGACY_LOW, "Inside pd_comms_log_connection_status_event for %d", event_type, 0, 0);
  ...
  if (p_LogPkt == NULL)
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR, "PDCOMM : log_alloc error", 0, 0, 0);
    return;
  }
  ...
  switch (event_type)
  {
    ...
    case PD_EVENT_SERVER_CONNECTED:
      ...
      break;
    default:
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH, "Unrecognised event to log", event_type, 0, 0);
