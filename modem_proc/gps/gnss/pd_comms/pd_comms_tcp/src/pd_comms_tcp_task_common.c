/*======================================================================
 ...
 INITIALIZATION AND SEQUENCING REQUIREMENTS
  ...
  Copyright (c) 2013 - 2018 Qualcomm Atheros, Inc.
...
/*=====================================================================
 ...
 Notice that changes are listed in reverse chronological order.
  ...
  $Header: //components/rel/gnss8.mpss/11.1/gnss/pd_comms/pd_comms_tcp/src/pd_comms_tcp_task_common.c#1 $
     ...
     (z_handle_table[l_i].e_link_status == PDCOMM_STATE_DISCONNECTING))
  {
    ...
    switch(p_hdr_ptr->z_params.e_link_type)
    {
      case LINK_UDP:
      {
          ...
          result = pd_comms_tcp_task_udp_bind(z_handle,sub); 
        break;
      }
      case LINK_SMS:
      {
       ...
       z_handle_table[l_i].z_connect_params.e_link_type != LINK_SMS)
    {
      ...
      if(z_handle_table[l_i].l_inactivity_count >= PDCOMM_INACTIVITY_TIMEOUT)
      {
        ...
        if(z_handle_table[l_i].e_link_status == PDCOMM_STATE_DISCONNECTED)
        {
          ...
          /* MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"PDCOMM APP: Client already notified about aborted connection. Release Handle [%d]", l_i,0,0); */
        }
        else
        {
