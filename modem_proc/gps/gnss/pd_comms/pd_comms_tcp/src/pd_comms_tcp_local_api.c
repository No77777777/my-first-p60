...
/*=====================================================================
 ...
 Notice that changes are listed in reverse chronological order.
  ...
  $Header: //components/rel/gnss8.mpss/11.1/gnss/pd_comms/pd_comms_tcp/src/pd_comms_tcp_local_api.c#1 $$DateTime: 2019/10/24 01:30:36 $$Author: pwbldsvc $ 

 when      who   what, where, why
 ----      ----  --- ---------------------------------------------------
 09/30/19   skm  Support for 911 nDDS on DSDS targets 
 04/10/18   py   Modified pd_comms_app_srv_sys_change_notification parameter
...
SIDE EFFECTS
  None
===========================================================================*/
void pd_comms_client_srv_system_change_notification_handle(
  /*contains srv_sys_mode , dd sub and as_id*/
...
)
{
  ...
  os_IpcMsgType* p_ipc_ptr = NULL;
  uint32* data_p = NULL;
  pd_comms_sys_info_change_params_type * p_sys_info_change;
  ...
  if(p_ipc_ptr == NULL)
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"Failure during Ipc creation",0,0,0);
    return;
  }
  ...
  p_sys_info_change = (pd_comms_sys_info_change_params_type*)p_ipc_ptr->p_Data;
  p_sys_info_change->srv_system = z_sys_info_change_params.srv_system;
  p_sys_info_change->dd_sub     = z_sys_info_change_params.dd_sub;
  p_sys_info_change->as_id      = z_sys_info_change_params.as_id;
  
  if(!os_IpcSend(p_ipc_ptr,(uint32)THREAD_ID_SM_PDCOMM_TCP))
  {    
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"Failure during Ipc Send",0,0,0);
   (void)os_IpcDelete(p_ipc_ptr);
  }
...
void pd_comms_client_apn_profile_change_notification_handle(uint32 apn_profile_type)
{
  ...
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"Apn profile type %d not supported",apn_profile_type,0,0);
  }

  (void)pd_comms_tcp_task_read_apn_profiles_from_nv(item,profile_type, FALSE, pd_comms_nv_efs_get_current_dd_sub());
#else
  MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"APN profiles NV are not supported when BIT API is enabled",0,0,0);    
...
#else
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"Data Enable can not be set when BIT API is enabled",0,0,0);  
#endif /* FEATURE_GNSS_BIT_API */
}
