...
static void pdcomm_test_driver_start_abort_timer()
{
  ...
  /*if(abort_timer == NULL) 
  {
	MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH,"PDCOMM TD: Abort timer started...",0,0,0);
...
static void pdcomm_test_driver_stop_abort_timer()
{
  ...
  /*if(abort_timer != NULL)
  {
	 MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH,"PDCOMM TD: Abort timer stopped...",0,0,0);
...
static void pdcomm_test_driver_connect()
{
  ...
  {
	MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"PDCOMM TD: pdcomm_client_connect returned error: %d",result,0,0);
  }
  else
  {
...
static void pdcomm_test_driver_write_string()
{
  ...
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"PDCOMM TD: pdcomm_client_write returned error: %d",result,0,0);
  }
...
static void pdcomm_test_driver_disconnect(boolean abort)
{
  ...
  if(result != PDCOMM_RESULT_WAIT && result != PDCOMM_RESULT_OK)
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"PDCOMM TD: pdcomm_client_disconnect returned error: %d",result,0,0);
  }
}
...
static void pdcomm_test_driver_release_handle(void)
{
  ...
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"PDCOMM TD: pdcomm_client_release_handle returned error: %d",result,0,0);
  }
...
static void pdcomm_test_driver_handle_notify(os_IpcMsgType *msg)
{
  ...
  if(hdr_ptr->e_type == READ_DATA)
  {
            ...
            (hdr_ptr->z_handle.e_link_type == handle.e_link_type))
        {
            ...
            MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH,"PDCOMM TD: READ_EVENT from PDCOMM for TCP link. Callng Read()",0,0,0);
            ...
            (hdr_ptr->z_handle.e_link_type == z_sms_handle.e_link_type))
        {
            ...
            MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH,"PDCOMM TD: READ_EVENT from PDCOMM for SMS link. Callng Read() with transaction-Id: %d",
                     transaction_id,0,0);
  ...
  else
  {
    ...
    if(hdr_ptr->e_type == ERROR_RPT)
    {
      ...
      {
         MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH,"PDCOMM TD: Force dormancy operation failed",0,0,0); 
      ...
      {
         MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH,"PDCOMM TD: UnForce dormancy operation failed",0,0,0); 
         /*pdcomm_test_driver_write_string();*/
          pdcomm_test_driver_write_supl_start(); 
          return;
      }
...
/*    if(cur_state != params->e_new_state)*/
    {
      ...
      case PDCOMM_STATE_CONNECTED:
        {
          cur_state = params->e_new_state;
    	    ...
    	    MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH,"PDCOMM TD: state CONNECTED while in state %d",old_state,0,0);
      ...
      case PDCOMM_STATE_DISCONNECTED:
        {
            cur_state = params->e_new_state;
    		pdcomm_test_driver_stop_abort_timer();
    	  MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH,"PDCOMM TD: state DISCONNECTED while in state %d",old_state,0,0);
      ...
      case PDCOMM_STATE_CONNECTING:
    	{
          cur_state = params->e_new_state;
    	  pdcomm_test_driver_start_abort_timer();
    	  MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH,"PDCOMM TD: state CONNECTING while in state %d",old_state,0,0);
    	  break;
    	}
      case PDCOMM_STATE_DISCONNECTING:
    	{
          cur_state = params->e_new_state;
    	  pdcomm_test_driver_start_abort_timer();
    	  MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH,"PDCOMM TD: state DISCONNECTING while in state %d",old_state,0,0);
          if(pd_comm_ctrl.test_earlyConnect)    
      ...
      default:
    	{
    	  MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"PDCOMM TD: Unknown state recvd: %d while in state %d",params->e_new_state,old_state,0);
    	}
...
static int pdcomm_test_driver_handle_read_data(os_IpcMsgType *msg)
{
  ...
  MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH,"PDCOMM TD: %d bytes received:",hdr_ptr->w_data_len,0,0);
  ...
  {
	  MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"PDCOMM TD: recevied invalid string length",0,0,0);
  }
  else
  {
	for(i=0;i<hdr_ptr->w_data_len ;i++)
	{
	  MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH,"%x",p_read_data[i],0,0);
	}
...
static void pdcomm_test_driver_send_sms(void)
{
  ...
  MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"PDCOMM TD: Sending SMS of size %d",sizeof(SUPL_START),0,0);
...
static int pdcomm_test_driver_handle_sms_read_data(os_IpcMsgType *msg)
{
  ...
  /*lint -e415 -e416 -e420*/   
  MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH,"PDCOMM TD: %d bytes received on SMS link",hdr_ptr->w_data_len,0,0);
  p_read_data = (char*)hdr_ptr->p_data;
  ...
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"PDCOMM TD: recevied invalid string length for SMS link",0,0,0);
  }
  else
  {
    for(i=0;i<hdr_ptr->w_data_len ;i++)
...
static void pdcomm_test_driver_handle_write_ack(os_IpcMsgType *msg)
{
  ...
  MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH,"PDCOMM TD: write ack received for %d bytes",hdr_ptr->q_bytes_written,0,0);
...
static void pdcomm_test_driver_handle_sms_ack_report(os_IpcMsgType *msg)
{
  ...
  MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH,"PDCOMM TD: SMS ACK report received: sucess=%d transaction-id=%d",
...
static void pdcomm_test_driver_proc(os_IpcMsgType *msg)
{
  ...
  case PDCOMM_TD_CLIENT_MSG_HANDLE:
	{
	  ...
	  MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH,"PDCOMM TD: handle recvd. Handle_id %d",handle.q_handle_id,0,0);
  ...
  case PDCOMM_TD_CLIENT_MSG_IOCTL_ACK:
    {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"PDCOMM TD: Received IOCTL ACK",0,0,0);
      
      if(pdcomm_td_forceDormancy)
      {
        if(pdcomm_test_driver_mesgIsForceDormAck(msg))
        {
           MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH,"PDCOMM TD: FORCE DORMANCY successfull. Unforcing Dormancy",0,0,0); 
           pdcomm_test_driver_unforce_dormancy();
        }
        else
        if(pdcomm_test_driver_mesgIsUnforceDormAck(msg))
        {
           MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH,"PDCOMM TD: UNFORCE DORMANCY successfull.",0,0,0);  
           /*pdcomm_test_driver_write_string();*/
           pdcomm_test_driver_write_supl_start();    
        }
        else
        ...
        {
          MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH,"PDCOMM TD: SET SSL Certificate successful.",0,0,0);  
          return;    
        }
      }
      else
  ...
  case C_OS_MSG_ID_TIMER_EXPIRY:
	{
	  ...
	  {
	    cur_state = PDCOMM_STATE_DISCONNECTING;
		MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"PDCOMM TD: ABORT timer expired...",0,0,0);
	    /*os_TimerDestroy(abort_timer);*/
	    pdcomm_test_driver_disconnect(TRUE);/*abort connection*/
	  }
	  else
	  {
		MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"PDCOMM TD: Invalid timer id %d in timeout message",*((uint32*)msg->p_Data),0,0);
	  }
	  break;
	}
  default:
	{
	  MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"PDCOMM TD: Unknown Message recvd: %d",msg->q_MsgId,0,0);
	}
...
boolean pdcomm_test_driver_get_handle()
{
  ...
  {
     MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"PDCOMM TD: handle allocation failed",0,0,0);
     return FALSE;
  }
...
{
   MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH,"PDCOMM TD: SMS PARSE CALLBACK CALLED",0,0,0); 
   return TRUE;
}
...
boolean pdcomm_test_driver_verify_sms_push_cb(
   ...
   boolean u_isWapPush)
{
  MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH,"PDCOMM TD: SMS  read message CALLBACK CALLED",0,0,0);
...
void pdcomm_test_driver_test_hash()
{
  ...
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"PDCOMM TD: pd_comms_app_generate_hash returned error %d",retval,0,0);
    return;
  }
  ...
  if(msg == NULL)
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"PDCOMM TD: NULL message received while waiting for generate_has response",0,0,0);
    return;
  }
  else
  {
    if(msg->q_MsgId == PDCOMM_TD_CLIENT_MSG_HASH_RESP)
    {
      ...
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"PDCOMM TD: Hash response received with hash_ouput_len = %d",p_hash_resp->q_output_len,0,0);  
    }
    else
    {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"PDCOMM TD: Unexpected message %d received while waiting for hash reponse",msg->q_MsgId,0,0);  
    }
...
void pdcomm_test_driver_test_sms_link()
{
   ...
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"PDCOMM TD: SMS handle allocation failed",0,0,0);
   ...
   if(msg == NULL)
   {
     MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"PDCOMM TD: NULL message received",0,0,0);
     return;
   }
   else
   {
     ...
     if(msg->q_MsgId == PDCOMM_TD_CLIENT_MSG_NOTIFY)
     {
       ...
       {
          MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"PDCOMM TD: FAILED SMS Link connection",0,0,0);
          os_IpcDelete(msg);
          return;
       }
       else
       {
          MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH,"PDCOMM TD: SMS Link Connected",0,0,0); 
       }
...
void pdcomm_test_driver_task(dword dummy)
{
  ...
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"PDCOMM TD: Test driver failed getting handle from PDCOMM APP",0,0,0);
  }
  else
  {
    pdcomm_test_driver_start_connect_timer();  
