...
/*=====================================================================
 ...
 Notice that changes are listed in reverse chronological order.
  ...
  $Header: //components/rel/gnss8.mpss/11.1/gnss/pd_comms/src/pd_comms_api.c#1 $$DateTime: 2019/10/24 01:30:36 $$Author: pwbldsvc $ 

 when      who   what, where, why
 ----      ----  --- ---------------------------------------------------
 09/30/19   skm  Support for 911 nDDS on DSDS targets 
 04/10/18   py   Updated pd_comms_app_srv_sys_change_notification parameter
...
SIDE EFFECTS
  None
===========================================================================*/
void pd_comms_app_srv_sys_change_notification(
  pd_comms_sys_info_change_params_type z_sys_info_change_params)
{
  ...
  for(; link_type < LINK_TYPE_MAX; link_type++)
  {
    ...
    {
      (void)p_registered_services[link_type].srv_system_change_notification_handle_func(z_sys_info_change_params);
    }
  }
#endif
}
...
void pd_comms_set_apn_profiles(pdsm_client_id_type client_id, uint32 apn_profile_type, pdsm_pa_apn_profiles_type *apn_profile_p)
{
  ...
  if((CGPS_NV_EFS_MAX_NUM_ITEMS != item) &&
     (FALSE != pd_comms_nv_efs_reg_item_write(
                 ...
                 apn_profile_p)))
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH,"Wrote the APN profiles to NV",0,0,0);
  }
...
void pd_comms_get_apn_profiles(uint32 apn_profile_type, pdsm_pa_apn_profiles_type *apn_profile_p)
{
  ...
  if(CGPS_NV_EFS_MAX_NUM_ITEMS != item)
  {
    (void)pd_comms_nv_efs_reg_item_read(
            ...
            apn_profile_p, FALSE,pd_comms_nv_efs_get_current_dd_sub()); 
  }
#else
  MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"APN profiles NV are not supported when BIT API is enabled",0,0,0);  
#endif /* FEATURE_GNSS_BIT_API */
