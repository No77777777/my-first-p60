...
/*==============================================================================
  ...
  $Header: //components/rel/gnss8.mpss/11.1/gnss/common/src/gps_fs_task.c#1 $ 
  $DateTime: 2019/10/24 01:30:36 $ 
  $Author: pwbldsvc $ 
  
when         who     what, where, why
--------     ---     ----------------------------------------------------------
08/02/19     nath    Function to handle async write calls per sub
...
#if 0 /* EFS_SYNC should never be called by GPS */
...
#endif /* 0 - EFS_SYNC should never be called by GPS */
    }
  }   
  ...
  else if ( p_gpsfsPtr->u_FileOP == GPS_FS_PURGE_DIR )
  {
    ...
    while ((dirent = efs_readdir (dirp)) != 0)
    {
      ...
      if (S_ISREG(fmode))
      {
        ...
        {
            MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"Could not delete file errno = %d", efs_errno, 0, 0);
            l_efsPutStatus[u_msgTask] = efs_errno;
            break;
        }
        else
 ...
 * Function description:
 *
 *  gps_fs_ProcSMCmd function processes only write operation for async write
    calls*
 * Parameters: 
 *
 *  p_gpsfsPtr - input GPS FS command information
 ...
 ******************************************************************************
*/
void gps_fs_ProcSMCmd( gps_fs_u_cmd_type *p_gpsfsPtr, uint8 u_msgTask, uint32 q_SrcThreadId)
{
  ...
  if ((GPS_FS_TASK_MAX_MESSAGE  <= u_msgTask) || (p_gpsfsPtr == NULL))
  {
    MSG(MSG_SSID_MGPPE, MSG_LEGACY_HIGH, "Invalid input into gps_fs_ProcSMCmd");
    return;
  }
  
  if ( p_gpsfsPtr->u_FileOP == GPS_FS_CREATE_WRITE_FILE )
  {
    status = mcfg_fs_write((char*)p_gpsfsPtr->p_FileName,
                          p_gpsfsPtr->p_DataPtr, 
                          p_gpsfsPtr->q_DataSize, 
                          MCFG_FS_O_CREAT|MCFG_FS_O_AUTODIR|MCFG_FS_O_TRUNC,
                          MCFG_FS_ALLPERMS,
                          (mcfg_fs_e_type)CGPS_NV_FS_TYPE_EFS,
                          (mcfg_fs_sub_id_e_type)p_gpsfsPtr->u_Reserved[0]);
    
    if (status != MCFG_FS_STATUS_OK)
    {
      GPSFS_MSG_SPRINTF_1(MSG_LEGACY_HIGH, "gps_fs_ProcSMCmd: Unable to write File; error: %d\n", 
                           status);
    }
  }
  else
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"Unhandled MCFG API cmd", 0, 0, 0);
  }
}
...
void gps_fs_Process( os_IpcMsgType *p_Msg )
{
  ...
  {
    gps_fs_ProcSMCmd( (gps_fs_u_cmd_type *)(p_Msg->p_Data), GPS_FS_TASK_SM_MESSAGE, p_Msg->q_SrcThreadId);
    break;
  }
