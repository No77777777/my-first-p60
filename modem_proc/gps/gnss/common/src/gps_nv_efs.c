...
/*==============================================================================
  ...
  $Header: //components/rel/gnss8.mpss/11.1/gnss/common/src/gps_nv_efs.c#1 $
...
----------   ---     -----------------------------------------------------------
17/06/19      py      Fixing Memory leak while writing to EFS
...
/*==============================================================================
  FUNCTION
    cgps_nv_efs_check_write_params

  DESCRIPTION
    Check the parameters of a write issued to GPS NV EFS API.
    
 
  RETURN VALUE
    TRUE: Parameters validated.
    FALSE: Invalid Parameters
==============================================================================*/
static boolean cgps_nv_efs_check_write_params
...
/*==============================================================================
  ...
  RETURN VALUE
    ...
    CGPS_NV_EFS_REG_RW_STAT_INVALID - Invalid paramters
==============================================================================*/
cgps_nv_efs_reg_rw_stat_enum_type cgps_nv_efs_reg_item_write_subsid
( cgps_nv_efs_reg_item_type const item,
  uint32 data_size,
  const void *data_ptr,
  cgps_nv_efs_reg_item_type table_start,
  ...
  cgps_nv_efs_sub_id_type subid )
{
  ...
  if(cgps_nv_efs_check_write_params(
         item,
         data_size,   data_ptr,
         table_start, table_end,
         cgps_nv_efs_reg_items,
...
FUNCTION cgps_nv_efs_file_resp_handler

DESCRIPTION
  This function is called by the GPS_FS task to send back the result of
  file operation (write/append/delete) performed.
...
SIDE EFFECTS
  None.
===========================================================================*/
static void cgps_nv_efs_file_resp_handler
(
  uint32 q_FileOperationResult,
  const uint8 u_PathFilename[],
  uint8 u_FileOp
)
{
  MSG_SPRINTF_1(MSG_SSID_GPSSM, MSG_LEGACY_MED, "CGPS_NV_EFS: file name %s", u_PathFilename);
  if(ENOERR != q_FileOperationResult)
  {
    MSG_2( MSG_SSID_GPSSM, MSG_LEGACY_ERROR,
           "CGPS EFS FILE RESP HANDLER Result = %d, FileOp = %d",
...
)
{
  ...
  MSG_1(MSG_SSID_GPS, MSG_LEGACY_MED,"CGPS_NV_EFS_ASYNC_WRITE: Registry Item Write Started for item:%d", item);
  ...
  if ((entry_ptr = gps_nv_efs_find_subsys_entry(item)) == NULL)
  {
    MSG_1(MSG_SSID_GPS, MSG_LEGACY_ERROR,"CGPS_NV_EFS_ASYNC_WRITE: subsystem not initialized for item:%d", item);
    return CGPS_NV_EFS_REG_RW_STAT_INVALID;
  }
  ...
  if (data_size != item_reg_ptr->size)
  {
    MSG(MSG_SSID_GPS, MSG_LEGACY_ERROR,"CGPS_NV_EFS_ASYNC_WRITE: Destination buffer size incorrect");
    return CGPS_NV_EFS_REG_RW_STAT_INVALID;
  }

  /* Parameters are OK, Allocate all the dynamic memory we need */
  p_Msg = os_IpcCreate( sizeof(gps_fs_u_cmd_type) + data_size, IPC_ALLOCATION_DYNAMIC, THREAD_ID_GPS_FS );
  if(p_Msg == NULL)
  {
    MSG(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"could not allocate IPC");
    return CGPS_NV_EFS_REG_RW_STAT_WR_ERR;
  }

  p_GpsFsCmd = (gps_fs_u_cmd_type *) p_Msg->p_Data;
  p_GpsFsCmd->p_DataPtr = (void*)((char*)p_GpsFsCmd + sizeof(gps_fs_u_cmd_type));

  /* Dynanmic memory is allocated, now issue the IPC to GPS FS TASK.
   * Generate the absolute path for the file */
  p_GpsFsCmd->u_FileNameLen = cgps_nv_efs_reg_get_item_fname(
                                     (char*) p_GpsFsCmd->p_FileName,
                                     sizeof(p_GpsFsCmd->p_FileName),
                                     FALSE,
                                     item_reg_ptr,
                                     entry_ptr->sz_pathname_prefix);

  if(p_GpsFsCmd->u_FileNameLen == 0)
  {
    MSG(MSG_SSID_GPS, MSG_LEGACY_ERROR,
        "CGPS_NV_EFS_ASYNC_WRITE: could not generate filename");

    os_IpcDelete(p_Msg);
    return CGPS_NV_EFS_REG_RW_STAT_INVALID;
  }

  p_Msg->q_MsgId = C_GPS_FS_MSG_SM_CMD;

  /* initialize the IPC */
  p_GpsFsCmd->u_FileOP = PDSM_FS_CREATE_WRITE_FILE;
  /* send the IPC to write/delete the file */
  p_GpsFsCmd->q_DataSize = data_size;

  p_GpsFsCmd->u_Reserved[0] = subid;
  p_GpsFsCmd->u_Reserved[1] = 0;
  p_GpsFsCmd->u_Reserved[2] = 0;
  /* do a force sync of the file so the file is written immediately to EFS not
   * at shutdown */
  p_GpsFsCmd->u_Reserved[3] = 1;

  /* Copy the data to write to EFS */
  memscpy(p_GpsFsCmd->p_DataPtr, p_GpsFsCmd->q_DataSize,data_ptr, data_size);


  p_GpsFsCmd->gps_fs_efs_status_get = cgps_nv_efs_file_resp_handler;
