...
/*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*
 ...
 Confidential and Proprietary - Qualcomm Technologies, Inc.
  ...
  $Header: //components/rel/gnss8.mpss/11.1/gnss/inc/locEng_sm_common_msg_v01.h#1 $
 *====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*/
/*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*
...
#define LOC_ENG_MAX_NUMBER_PLOS_DATA_PER_SYSTEM_V01 255

/**  Maximum number of constellations for the LPPM purpose.  */
#define LOC_ENG_MAX_NUM_CONSTELLATIONS_V01 4
...
#ifdef FEATURE_GNSS_NON_STANDARD_SUPPORT
  eLOC_ENG_MSG_SOURCE_MGP_V01 = 5, /**<  Message was sent by MGP task */
#endif /* FEATURE_GNSS_NON_STANDARD_SUPPORT */
...
#ifdef FEATURE_GNSS_NON_STANDARD_SUPPORT
  eLOC_ENG_TIME_SRC_SLEEP_CLOCK_V01 = 18, /**<  Source of the time is sleep clock  */
  eLOC_ENG_TIME_SRC_SLEEP_CLOCK_TIME_TRANSFER_V01 = 19, /**<  Source of the time is sleep clock time transfer  */
#endif /* FEATURE_GNSS_NON_STANDARD_SUPPORT */
...
#ifdef FEATURE_GNSS_NON_STANDARD_SUPPORT
  eLOC_ENG_MOTION_MODE_SWIMMING_UNKNOWN_V01 = 400, /**<  Device movement is in Swimming mode; nothing else is known about the movement.  */
#endif /* FEATURE_GNSS_NON_STANDARD_SUPPORT */
...
#ifdef FEATURE_GNSS_NON_STANDARD_SUPPORT
  /* Optional */
  /*  Minimum detectable separation for position cross check */
  uint8_t minimum_detectable_separation_crosscheck_valid;  /**< Must be set to true if minimum_detectable_separation_crosscheck is being passed */
  float minimum_detectable_separation_crosscheck;
  /**<   Minimum detectable surface distance separation between two positions
       during consistency check in an emergency session. \n
       - Type: IEEE-754 32-bit float    \n
       - Units: Meters                  \n
       - Valid values: Positive values  \n
  */
...
typedef struct {
  ...
  uint64_t timeTickCount;
  /**<   The modem time tick count (in milliseconds) */

  uint32_t timeTickUnc;
  /**<   One-sided maximum time tick uncertainty (in milliseconds) */
}locEngTimeTickStructT_v01;  /* Type */

/** Indication Message; Gets the GPS time information.  */
typedef struct {
 ...
 Valid Values :\n
      ...
      - eLOC_ENG_MSG_SOURCE_NF (3) --  Message was sent by NF task
      - eLOC_ENG_MSG_SOURCE_SM (4) --  Message was sent by SM task
      - eLOC_ENG_MSG_SOURCE_MGP (5) --  Message was sent by MGP task */
  ...
  /* Mandatory */
  /*  Clock Time Bias */
  float clockTimeBiasMs;
  /**<     Clock bias in msecs. Constrained to > -0.5 and < 0.5
       Type: 32 bit floating point */

  /* Mandatory */
  /*  Clock Time Uncertainty */
  float clockTimeUncMs;
  /**<   One sided max time bias uncertainty in msecs. \n
       - Type: Floating point \n
       - Units: Milliseconds  */
  ...
  /**<   Time source. 
 Valid values: \n
      - eLOC_ENG_TIME_SRC_INVALID (0) --  Invalid time. 
      - eLOC_ENG_TIME_SRC_NETWORK_TIME_TRANSFER (1) --  Time is set by the 1x system. 
      - eLOC_ENG_TIME_SRC_NETWORK_TIME_TAGGING (2) --  Time is set by WCDMA/GSM time tagging (i.e.,
       associating network time with GPS time). 
      - eLOC_ENG_TIME_SRC_EXTERNAL_INPUT (3) --  Time is set by an external injection. 
      - eLOC_ENG_TIME_SRC_TOW_DECODE (4) --  Time is set after decoding over-the-air GPS navigation data
       from one GPS satellite. 
      - eLOC_ENG_TIME_SRC_TOW_CONFIRMED (5) --  Time is set after decoding over-the-air GPS navigation data 
       from multiple satellites. 
      - eLOC_ENG_TIME_SRC_TOW_AND_WEEK_CONFIRMED (6) --  Both time of the week and the GPS week number are known. 
      - eLOC_ENG_TIME_SRC_NAV_SOLUTION (7) --  Time is set by the position engine after the fix is obtained. 
      - eLOC_ENG_TIME_SRC_SOLVE_FOR_TIME (8) --  Time is set by the position engine after performing SFT.
       This is done when the clock time uncertainty is large. 
      - eLOC_ENG_TIME_SRC_GLO_TOW_DECODE (9) --  Time is set after decoding GLO satellites 
      - eLOC_ENG_TIME_SRC_TIME_TRANSFORM (10) --  Time is set after transforming the GPS to GLO time 
      - eLOC_ENG_TIME_SRC_WCDMA_SLEEP_TIME_TAGGING (11) --  Time is set by the sleep time tag provided by the WCDMA network 
      - eLOC_ENG_TIME_SRC_GSM_SLEEP_TIME_TAGGING (12) --  Time is set by the sleep time tag provided by the GSM network 
      - eLOC_ENG_TIME_SRC_UNKNOWN (13) --  Source of the time is unknown 
      - eLOC_ENG_TIME_SRC_SYSTEM_TIMETICK (14) --   Time is derived from system clock (better known as slow clock). 
       GNSS time is maintained irrespective of the GNSS receiver state  
      - eLOC_ENG_TIME_SRC_QZSS_TOW_DECODE (15) --  Time is set after decoding QZSS satellites 
      - eLOC_ENG_TIME_SRC_BDS_TOW_DECODE (16) --  Time is set after decoding BDS satellites 
      - eLOC_ENG_TIME_SRC_GAL_TOW_DECODE (17) --  Time is set after decoding GAL satellites 
      - eLOC_ENG_TIME_SRC_SLEEP_CLOCK (18) --  Source of the time is sleep clock 
      - eLOC_ENG_TIME_SRC_SLEEP_CLOCK_TIME_TRANSFER (19) --  Source of the time is sleep clock time transfer  */
  ...
  /* Optional */
  /*  Time tick */
  uint8_t timeTick_valid;  /**< Must be set to true if timeTick is being passed */
  locEngTimeTickStructT_v01 timeTick;
  /**<   The modem time tick information*/
}locEngTimeTransferGpsTimeIndMsgT_v01;  /* Message */
