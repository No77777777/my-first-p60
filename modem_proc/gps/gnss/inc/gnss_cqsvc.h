...
/*============================================================================
  ...
  $Header: //components/rel/gnss8.mpss/11.1/gnss/inc/gnss_cqsvc.h#1 $ 
...
/** gnss_CQmiSvcSendTimeTransferGpsTimeStatusInd_proxy
 *  
 * This function is to be called by the location engine, to send 
 * an time transfer status and info to the registered clients 
 *  
 * @param [in] statusReady  Status to indicate if the GNSS 
 *        engine is ready for time transfer
 * @param [in] pz_Ind  the time transfer info in LocEng format 
 * 
 * @return 0 if operation is successful
 */
extern uint32 gnss_CQmiSvcSendTimeTransferGpsTimeStatusInd_proxy(boolean statusReady,
                                                                 const locEngTimeTransferGpsTimeIndMsgT_v01 *pz_Ind);


/** gnss_CQmiSvcSendTimeTransferGpsTimeEventInd_proxy
 *  
 * This function is to be called by the location engine, to send 
 * an time transfer info to the registered clients 
 *  
 * @param [in] gpsTimeValid  Status to indicate if the GNSS time 
 *        is valid
 * @param [in] pz_Ind  the time transfer info in LocEng 
 *        format
 * 
 * @return 0 if operation is successful
 */
extern uint32 gnss_CQmiSvcSendTimeTransferGpsTimeEventInd_proxy(boolean gpsTimeValid,
                                                                const locEngTimeTransferGpsTimeIndMsgT_v01 *pz_Ind);

/* -----------------------------------------------------------------------*//**
@brief 
   This function is to be called by MGP task, to send GNSS RF config info to the client
  
@param [in] p_RfConfig: A pointer to hold RF config value to be returned
 
@Retrun return 0 if operation is successful
*//* ------------------------------------------------------------------------*/
extern uint32 gnss_CQmiGetGnssRfConfigInd_proxy(const gnss_RfConfigSplAntennaGainParamsType* const p_RfConfig);
