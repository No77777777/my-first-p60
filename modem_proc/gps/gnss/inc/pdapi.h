...
/*===========================================================================
  ...
  $Header: //components/rel/gnss8.mpss/11.1/gnss/inc/pdapi.h#11 $
...
11/09/23   dy      Adding macros for meastatus and measstatus mask
07/26/23   jv      Update session params when SM is in session
07/08/20   skm     ShortCodes SpecialRequest support for START_REQ and GET_BEST_POS .
06/18/19   sai     Added NAVIC clock deletion related changes
06/06/19   sj      Disable AGNSS for 5G NR.
05/15/19   yh      Add pdsm_get_gps1_lock for Location Privacy Feature
04/25/19   yh      Add PDSM_EXT_STATUS_EVENT_E911_NOTIFICATION_REPORT to PDSM_EXT_STATUS_EVENT_MASK
02/19/19   skm     5G NR-NSA, LPP Rel-15 and SUPL 2.0.4 support 
06/17/18   sj      Added custom SLP for MO AGNSS requests.
02/28/18   nl      Added power mode input for pdsm_get_position()
02/02/18   sum     Added new field for raw vertical uncertainty
02/22/19   mj      Location Privacy Feature
01/24/19   nath    Support for ME Metrics
01/15/19   nl      Support for sending intra-GNSS timebias in measurement rpt
12/21/18   skm     Provide UE WIFI Mac Address in LPPe
12/26/18   py      Added support to delete TGD data. pdsm_delete_parms_flags_ext as extended flag
09/25/18   nath    Added PQDTM change
09/07/18   py      Added f_loc_uncertainty_angle in ext_pos_data for better precision
06/22/18   AP      Added Defensive F3 to help debug Non duplicable crashes
06/17/18   sj      Added custom SLP for MO AGNSS requests.
...
03/22/17   sum     Added Macro PDAPI_NMEA_GNGGA,PDAPI_NMEA_GNRMC,PDAPI_NMEA_GNVTG
02/28/18   nl      Added power mode input for pdsm_get_position()
02/02/18   sum     Added new field for raw vertical uncertainty
...
01/12/15   gk      Added new session related changes.
...
#define PDSM_MAX_URL_ADDR_LEN 255 /*Max length of a URL address*/
...
typedef enum {
  PDSM_WIFI_SCAN_TYPE_UNSOLICITED_DISCOVERY,         /* Unsolicited Discovery Scan */
  PDSM_WIFI_SCAN_TYPE_ON_DEMAND_DISCOVERY,           /* On Demand Discovery Scan*/
  PDSM_WIFI_SCAN_TYPE_RANGING,                       /* Ranging wifi scan */
  PDSM_WIFI_SCAN_TYPE_MAX = 0x10000000
} pdsm_wifi_scan_e_type;
...
typedef struct {
  uint16 url_length; /* URL Length */
  char   url[PDSM_MAX_URL_ADDR_LEN];  /* URL data */
...
/*Generic Constellation type ENUM to be used across SM, LocMW and LBS. 
  When adding new constellations update PDSM_CONSTELLATION_TYPE_ALL Mask*/
typedef enum
{
  PDSM_CONSTELLATION_TYPE_NONE    = 0x00,
  PDSM_CONSTELLATION_TYPE_GPS     = 0x01,
  PDSM_CONSTELLATION_TYPE_GLONASS = 0x02,
  PDSM_CONSTELLATION_TYPE_BDS     = 0x04,
  PDSM_CONSTELLATION_TYPE_GALILEO = 0x08,
  PDSM_CONSTELLATION_TYPE_SBAS    = 0x10,
  PDSM_CONSTELLATION_TYPE_COMPASS = 0x20,
  PDSM_CONSTELLATION_TYPE_QZSS    = 0x40,
  PDSM_CONSTELLATION_TYPE_NAVIC   = 0x80,
  PDSM_CONSTELLATION_TYPE_MAX     = 4294967295u /*Force 32 bit Unsigned int*/
} pdsm_constellation_e_type;

#define PDSM_CONSTELLATION_TYPE_ALL ( \
        PDSM_CONSTELLATION_TYPE_GPS | \
        PDSM_CONSTELLATION_TYPE_GLONASS | \
        PDSM_CONSTELLATION_TYPE_BDS | \
        PDSM_CONSTELLATION_TYPE_GALILEO | \
        PDSM_CONSTELLATION_TYPE_SBAS | \
        PDSM_CONSTELLATION_TYPE_COMPASS | \
        PDSM_CONSTELLATION_TYPE_QZSS    | \
        PDSM_CONSTELLATION_TYPE_NAVIC)
...
typedef enum {
  ...
  PDSM_SESSION_OPERATION_LPPM,
  PDSM_SESSION_OPERATION_MAX = 0x10000000
} pdsm_pd_session_operation_e_type;

typedef enum {
  PDSM_SESSION_DONOTUSE_POWER_MODE_MIN = -2147483647,
  PDSM_SESSION_POWER_MODE_INVALID = 0,
  PDSM_SESSION_POWER_MODE_IMPROVED_ACCURACY,
  PDSM_SESSION_POWER_MODE_NORMAL,
  PDSM_SESSION_POWER_MODE_BACKGROUND_DEFINED_POWER,
  PDSM_SESSION_POWER_MODE_BACKGROUND_DEFINED_TIME,
  PDSM_SESSION_POWER_MODE_BACKGROUND_KEEP_WARM,
  PDSM_SESSION_POWER_MODE_LAST = PDSM_SESSION_POWER_MODE_BACKGROUND_KEEP_WARM,
  PDSM_SESSION_DONOTUSE_POWER_MODE_MAX = 2147483647 /*Force 32 bit integer*/
} pdsm_pd_session_power_e_type;

typedef struct {
  pdsm_pd_session_power_e_type powerMode;
  uint32 tbm_msecs; /* Time between measurements in milliseconds */
} pdsm_pd_session_power_s_type;

typedef enum {
  PDSM_SESSION_DONOTUSE_SPECIAL_REQ_MIN = -2147483647,
  PDSM_SESSION_SPECIAL_REQ_SHORT_CODES = 1,
  PDSM_SESSION_DONOTUSE_SPECIAL_REQ_MAX = 2147483647 /*Force 32 bit integer*/
}pdsm_pd_position_special_req_e_type;

typedef struct {
 boolean v_specialReqValid;
 pdsm_pd_position_special_req_e_type e_SpecialReq;
}pdsm_pd_position_special_req_s_type;
...
typedef struct {
  ...
  pdsm_pd_server_info_s_type     lsinfo;          /* 1x PDE Location server info */
  pdsm_pd_server_info_s_type     umts_slp_lsinfo; /* UMTS SLP Location server info */
  ...
  pdsm_pd_session_power_s_type   power_mode;  /* Session power mode and TBM */
  pdsm_pd_position_special_req_s_type specialReqType; /* Client request needing special handling*/
...
typedef enum pdsm_pd_cmd_e
{
  ...
  PDSM_PD_CMD_INJECT_SRN_INFO,        /* Inject SRN AP scan result (BT,BLE,NFC..)*/
  PDSM_PD_CMD_LPPM_CONTROL,           /* LPPM Control (enable/disable)*/
...
typedef enum pdsm_pa_e {
  ...
  PDSM_PA_POSITION_MODE = 5, /* Position modes that are supported on the mobile */
  ...
  PDSM_PA_1X_MPC_NETWORK_ADRS = 10,       /* Address of the network entity, can be IP or URL */
  ...
  PDSM_PA_NMEA_CONFIG_INFO = 15,   /* To configure NMEA port and reporting interval */
  ...
  PDSM_PA_ON_DEMAND_LOW_POWER_MODE = 20, /* Enable or disable ON_DEMAND in Low Power Mode */
  ...
  PDSM_PA_XTRAT_CLIENT_TOKEN = 25, /* Set XTRA-T Client token */
  ...
  PDSM_PA_SUPL_VERSION = 30,       /* SUPL Version */
  ...
  PDSM_PA_ASST_GLONASS_POS_PROTOCOL = 35,  /* Selection of positioning protocols (RRLP, RRC, LPP) for A-Glonass U&C plane */
  ...
  PDSM_PA_GNSS_EMERGENCY_PROTOCOL_CONFIG, /* Whether Emergency positioning uses a 
                                             User plane or Control Plane protocol.
                                          ...
                                          */
  PDSM_PA_EMERGENCY_LBS_APN_PROFILE = 40, /* APN profiles for Emergency Location Based Services */
  ...
  PDSM_PA_XTRA_VERSION_CHECK = 45,        /* XTRA Version Check */
  ...
  PDSM_PA_PCID,                      /* PCID for XTRA */
  PDSM_PA_EMERGENCY_SUPPORT_CONFIG, /* Config for special handling of emergency sessions */
  PDSM_PA_LPPE_UP_CONFIG = 50,
  PDSM_PA_LPPE_CP_CONFIG,
  PDSM_PA_UAX3_CONFIG,               /* UAX3 CONFIG */
  DEPRECATED_PDSM_PA_SPOOFING_REPORT_CONFIG,    /* Deprecated Spoofing detection config*/
  PDSM_PA_PERSISTENT_SV_MASK,        /* PrisonerSV Extension: GNSS Persistent SV Mask */
  PDSM_PA_GNSS_MEAS_REPORT_CONFIG,   /* Constellation configuration for measurement reporting */
  PDSM_PA_GNSS_OTDOA_CONTROL,        /* Dynamically enable/disbale OTDOA */
  PDSM_PA_ENGINE_MONITOR_REPORT_CONFIG, /* Config for reporting GNSS engine status */
  PDSM_PA_GNSS_REPORTING_CONFIG,     /* Config for enabling/disabling GNSS Report Indications */
  PDSM_PA_OVERRIDE_E911_NOTIFICATION_CONFIG, /* Config to ignore E911 notifications from CM when the override is enabled*/
  PDSM_PA_CONSTELLATION_CONTROL_CONFIG, /* Constellation Control config*/
  PDSM_PA_REPORT_ME_METRICS,
  PDSM_PA_CONTROL_DATUM_NMEA_SENTENCE,
  PDSM_PA_EMERGENCY_EXTENSION_WINDOW, /* Config the time in seconds to extend emergency state once emergency state has ended */
  PDSM_PA_ACTIVATE_MULTIBAND,
...
#define PDSM_GPS_MEAS_REPORT_ENABLED     0x01
#define PDSM_GLO_MEAS_REPORT_ENABLED     0x02
#define PDSM_BDS_MEAS_REPORT_ENABLED     0x04
#define PDSM_GAL_MEAS_REPORT_ENABLED     0x08
#define PDSM_QZSS_MEAS_REPORT_ENABLED    0x10
#define PDSM_NAVIC_MEAS_REPORT_ENABLED   0x20
typedef struct {
 ...
 Valid values: \n
         ...
         #define PDSM_QZSS_MEAS_REPORT_ENABLED    0x10
         #define PDSM_NAVIC_MEAS_REPORT_ENABLED   0x20 */
  boolean reqEphDbFromMgp;
  boolean reqSvPolyDbFromMgp;
...
#define PDSM_SET_U64BITMASK_MAX(n) ((((uint64)0x1) << (n)) - 1)

#define PDSM_PA_DELETE_BITMASK(n)  PDSM_SET_U64BITMASK_BIT(n)

#define PDSM_PA_EXT_DELETE_BITMASK(n)  PDSM_SET_U64BITMASK_BIT(n)
...
#define PDSM_PA_DELETE_IONO_BDS               PDSM_PA_DELETE_BITMASK(55)
#define PDSM_PA_DELETE_IONO_GAL               PDSM_PA_DELETE_BITMASK(56)

/* Deletes Glonass SV health info for all GLONASS satellites*/
#define PDSM_PA_DELETE_HEALTH_GLO    PDSM_PA_DELETE_BITMASK(57)
/* Deletes BDS SV health info for all BDS satellites*/
#define PDSM_PA_DELETE_HEALTH_BDS    PDSM_PA_DELETE_BITMASK(58)
/* Deletes GAL SV health info for all GAL satellites*/
#define PDSM_PA_DELETE_HEALTH_GAL    PDSM_PA_DELETE_BITMASK(59)
/* Deletes Multiband data for all satellites*/
#define PDSM_PA_DELETE_MB_DATA       PDSM_PA_DELETE_BITMASK(60)

/* the following flags, extends the bitmask range used for deleting satellite data
*/
/* Deletes Group Delay data for all GPS satellites*/
#define PDSM_PA_EXT_DELETE_TGD_DATA_GPS         PDSM_PA_EXT_DELETE_BITMASK(0) /*0x0000*/
/* Deletes Group Delay data for all QZSS satellites*/
#define PDSM_PA_EXT_DELETE_TGD_DATA_QZSS        PDSM_PA_EXT_DELETE_BITMASK(1) /*0x0001*/
/* Deletes Group Delay data for all GLO satellites*/
#define PDSM_PA_EXT_DELETE_TGD_DATA_GLO         PDSM_PA_EXT_DELETE_BITMASK(2) /*0x0002*/
/* Deletes Group Delay data for all BDS satellites*/
#define PDSM_PA_EXT_DELETE_TGD_DATA_BDS         PDSM_PA_EXT_DELETE_BITMASK(3) /*0x0004*/
/* Deletes Group Delay data for all GAL satellites*/
#define PDSM_PA_EXT_DELETE_TGD_DATA_GAL         PDSM_PA_EXT_DELETE_BITMASK(4) /*0x0008*/
#define PDSM_PA_EXT_DELETE_HEALTH_NAVIC         PDSM_PA_EXT_DELETE_BITMASK(5)
#define PDSM_PA_EXT_DELETE_EPH_NAVIC            PDSM_PA_EXT_DELETE_BITMASK(6)
#define PDSM_PA_EXT_DELETE_ALM_NAVIC            PDSM_PA_EXT_DELETE_BITMASK(7)
#define PDSM_PA_EXT_DELETE_SVDIR_NAVIC          PDSM_PA_EXT_DELETE_BITMASK(8)
#define PDSM_PA_EXT_DELETE_SVSTEER_NAVIC        PDSM_PA_EXT_DELETE_BITMASK(9)
#define PDSM_PA_EXT_DELETE_ALM_CORR_NAVIC       PDSM_PA_EXT_DELETE_BITMASK(10)
#define PDSM_PA_EXT_DELETE_GNSS_SV_BLACKLIST_NAVIC    PDSM_PA_EXT_DELETE_BITMASK(11)
#define PDSM_PA_EXT_DELETE_TIME_NAVIC           PDSM_PA_EXT_DELETE_BITMASK(12)
#define PDSM_PA_EXT_DELETE_IONO_NAVIC           PDSM_PA_EXT_DELETE_BITMASK(13)
#define PDSM_PA_EXT_DELETE_TGD_DATA_NAVIC       PDSM_PA_EXT_DELETE_BITMASK(14)



/* Delete all flag is used to be used only with QMI_LOC_DELETE_ASSIST_DATA_REQ, Not needed for QMI_LOC_DELETE_GNSS_SERVICE_DATA_REQ */
...
#define PDSM_PA_DELETE_ALL_EXT (\
   PDSM_PA_EXT_DELETE_TGD_DATA_GPS|\
   ...
   PDSM_PA_EXT_DELETE_TGD_DATA_NAVIC\
)
...
#define PDSM_PA_DELETE_CLOCK_INFO_GALTIME_EST              PDSM_SET_U32BITMASK_BIT(18)
#define PDSM_PA_DELETE_CLOCK_INFO_GPS_GAL_TB               PDSM_SET_U32BITMASK_BIT(19)
#define PDSM_PA_DELETE_CLOCK_INFO_GLO_GAL_TB               PDSM_SET_U32BITMASK_BIT(20)
#define PDSM_PA_DELETE_CLOCK_INFO_GAL_BDS_TB               PDSM_SET_U32BITMASK_BIT(21)
#define PDSM_PA_DELETE_CLOCK_INFO_GALWEEK_NUMBER           PDSM_SET_U32BITMASK_BIT(22)
#define PDSM_PA_DELETE_CLOCK_INFO_GAL_RF_GRP_DELAY         PDSM_SET_U32BITMASK_BIT(23)

#define PDSM_PA_DELETE_CLOCK_INFO_NAVICTIME_EST            PDSM_SET_U32BITMASK_BIT(24)
#define PDSM_PA_DELETE_CLOCK_INFO_GPS_NAVIC_TB             PDSM_SET_U32BITMASK_BIT(25)
#define PDSM_PA_DELETE_CLOCK_INFO_GLO_NAVIC_TB             PDSM_SET_U32BITMASK_BIT(26)
#define PDSM_PA_DELETE_CLOCK_INFO_BDS_NAVIC_TB             PDSM_SET_U32BITMASK_BIT(27)
#define PDSM_PA_DELETE_CLOCK_INFO_GAL_NAVIC_TB             PDSM_SET_U32BITMASK_BIT(28)
#define PDSM_PA_DELETE_CLOCK_INFO_NAVICWEEK_NUMBER         PDSM_SET_U32BITMASK_BIT(29)
#define PDSM_PA_DELETE_CLOCK_INFO_NAVIC_RF_GRP_DELAY       PDSM_SET_U32BITMASK_BIT(30)
...
#define PDSM_PA_DELETE_ALL_CLOCK_INFO (\
     ...
     PDSM_PA_DELETE_CLOCK_INFO_GALTIME_EST | \
     PDSM_PA_DELETE_CLOCK_INFO_GPS_GAL_TB | \
     PDSM_PA_DELETE_CLOCK_INFO_GLO_GAL_TB | \
     PDSM_PA_DELETE_CLOCK_INFO_GAL_BDS_TB | \
     PDSM_PA_DELETE_CLOCK_INFO_GALWEEK_NUMBER | \
     PDSM_PA_DELETE_CLOCK_INFO_GAL_RF_GRP_DELAY | \
     PDSM_PA_DELETE_CLOCK_INFO_NAVICTIME_EST | \
     PDSM_PA_DELETE_CLOCK_INFO_GPS_NAVIC_TB  | \
     PDSM_PA_DELETE_CLOCK_INFO_GLO_NAVIC_TB  | \
     PDSM_PA_DELETE_CLOCK_INFO_BDS_NAVIC_TB  | \
     PDSM_PA_DELETE_CLOCK_INFO_GAL_NAVIC_TB  | \
     PDSM_PA_DELETE_CLOCK_INFO_NAVICWEEK_NUMBER  | \
     PDSM_PA_DELETE_CLOCK_INFO_NAVIC_RF_GRP_DELAY  \
     )

/* For Xtra Integrity deletion command. */
#define PDSM_PA_DELETE_XTRA_INTEGRITY_GPS        PDSM_SET_U32BITMASK_BIT(0) /*0x0001*/
#define PDSM_PA_DELETE_XTRA_INTEGRITY_GLO        PDSM_SET_U32BITMASK_BIT(1) /*0x0002*/
#define PDSM_PA_DELETE_XTRA_INTEGRITY_BDS        PDSM_SET_U32BITMASK_BIT(2) /*0x0004*/
#define PDSM_PA_DELETE_XTRA_INTEGRITY_GAL        PDSM_SET_U32BITMASK_BIT(3) /*0x0008*/
#define PDSM_PA_DELETE_XTRA_INTEGRITY_QZSS       PDSM_SET_U32BITMASK_BIT(4) /*0x0010*/
#define PDSM_PA_DELETE_XTRA_INTEGRITY_NAVIC      PDSM_SET_U32BITMASK_BIT(5) /*0x0020*/


#define PDSM_PA_DELETE_XTRA_INTEGRITY_NONE       (0x00)
#define PDSM_PA_DELETE_XTRA_INTEGRITY_ALL (\
        PDSM_PA_DELETE_XTRA_INTEGRITY_GPS  |\
        PDSM_PA_DELETE_XTRA_INTEGRITY_GLO  |\
        PDSM_PA_DELETE_XTRA_INTEGRITY_BDS  |\
        PDSM_PA_DELETE_XTRA_INTEGRITY_GAL  |\
        PDSM_PA_DELETE_XTRA_INTEGRITY_QZSS |\
        PDSM_PA_DELETE_XTRA_INTEGRITY_NAVIC \
        )

typedef struct {
  uint64 pdsm_delete_parms_flags;  /* 64 bits now with BDS support */
  uint64 pdsm_delete_parms_flags_ext; /* It extends pdsm_delete_parms_flag to include additional flags. All bits in latter has been used till 60*/
  uint32 q_delete_xtra_int_mask;   /* XTRA integrity bitmask */
  ...
  uint16 navic_eph_sv_mask; /* delete targeted NavIC EPH */
  uint16 navic_alm_sv_mask; /* delete targeted navIC ALM */
  uint16 navic_gnss_sv_blacklist_mask;  /* delete targeted NavIC SV in GNSS SV blacklist */
...
/* Per Constellation AGNSS Mode Bits Constellation x MSBased/MSAssisted x UP/CP*/
/* AGPS Mode bits */
...
#define PDSM_AGLO_POS_MODE_3G_UP_MS_BASED       0x00020000        /* Enable/Disable A-GLONASS UP MS-BASED for 3G */
#define PDSM_AGLO_POS_MODE_3G_UP_MS_ASSISTED    0x00040000        /* Enable/Disable A-GLONASS UP MS-ASSISTED for 3G */
...
/* ABDS Protocol enable bits */
#define PDSM_ENABLE_ABDS_RRLP_SUPL_MS_BASED     0x00000001        /* Enable/Disable A-ABDS UP MS-BASED using protocol RRLP over SUPL */
...
typedef uint64 pdsm_pa_nmea_type;
// Bits 0 - 5 for Legacy NMEA Sentences (Total 6 Bits)
#define PDAPI_NMEA_GGA PDSM_SET_U64BITMASK_BIT(0)
#define PDAPI_NMEA_RMC PDSM_SET_U64BITMASK_BIT(1)
#define PDAPI_NMEA_GSV PDSM_SET_U64BITMASK_BIT(2)
#define PDAPI_NMEA_GSA PDSM_SET_U64BITMASK_BIT(3)
#define PDAPI_NMEA_VTG PDSM_SET_U64BITMASK_BIT(4)
#define PDAPI_NMEA_PQXFI  PDSM_SET_U64BITMASK_BIT(5)
...
// Bits 6 - 21 for Extended NMEA Sentences (Total 16 Bits)
#define PDAPI_NMEA_GLGSV PDSM_SET_U64BITMASK_BIT(6)  /* Glonass SV in view info */
#define PDAPI_NMEA_GNGSA PDSM_SET_U64BITMASK_BIT(7)  /* Dop and Active SV info iff Glonass SVs are used */
#define PDAPI_NMEA_GNGNS PDSM_SET_U64BITMASK_BIT(8)  /* new GGA message for GNSS */
#define PDAPI_NMEA_GARMC PDSM_SET_U64BITMASK_BIT(9)   /* GAL recommended minimum data */
#define PDAPI_NMEA_GAGSV PDSM_SET_U64BITMASK_BIT(10)   /* GAL SVs in view */
#define PDAPI_NMEA_GAGSA PDSM_SET_U64BITMASK_BIT(11)   /* GAL SV dop and active SV info */
#define PDAPI_NMEA_GAVTG PDSM_SET_U64BITMASK_BIT(12)   /* GAL Speed and heading info */
#define PDAPI_NMEA_PSTIS PDSM_SET_U64BITMASK_BIT(13)  /* proprietary sentence at beginning of each sess */
#define PDAPI_NMEA_GSV_EXTENDED PDSM_SET_U64BITMASK_BIT(14) /*Enable/Disable Extended GGSV*/
#define PDAPI_NMEA_GAGGA PDSM_SET_U64BITMASK_BIT(15)   /* GAL Fix data */
#define PDAPI_NMEA_PQGSA PDSM_SET_U64BITMASK_BIT(16)   /* QZSS Enable PQGSA */
#define PDAPI_NMEA_PQGSV PDSM_SET_U64BITMASK_BIT(17)   /* QZSS Enable PQGSV */
#define PDAPI_NMEA_DEBUG PDSM_SET_U64BITMASK_BIT(18)   /* NMEA debugging enable*/
#define PDAPI_NMEA_VAR_LEN_DEBUG PDSM_SET_U64BITMASK_BIT(19)   /* NMEA variable length debug string enable*/
#define PDAPI_NMEA_GAGNS PDSM_SET_U64BITMASK_BIT(20)  /* GNS message for GAL */
#define PDAPI_NMEA_GPDTM PDSM_SET_U64BITMASK_BIT(21)  /* new DTM message for GAL */

// Bits 22 - 32 for Extended NMEA Sentences v2 (Total 11 Bits)
#define PDAPI_NMEA_GBGGA PDSM_SET_U64BITMASK_BIT(22)   /* BDS: Fix data */
#define PDAPI_NMEA_GBGSA PDSM_SET_U64BITMASK_BIT(23)   /* BDS SV dop and active SV info */
#define PDAPI_NMEA_GBGSV PDSM_SET_U64BITMASK_BIT(24)   /* BDS SV in view info */
#define PDAPI_NMEA_GBRMC PDSM_SET_U64BITMASK_BIT(25)   /* BDS: recommended minimum data */
#define PDAPI_NMEA_GBVTG PDSM_SET_U64BITMASK_BIT(26)   /* BDS: Speed and heading info */
#define PDAPI_NMEA_GQGSV PDSM_SET_U64BITMASK_BIT(27)   /* QZSS SV in view info */
#define PDAPI_NMEA_GIGSV PDSM_SET_U64BITMASK_BIT(28)   /* NAVIC SV in view info */
#define PDAPI_NMEA_GNGGA PDSM_SET_U64BITMASK_BIT(29)   /* Fix data */
#define PDAPI_NMEA_GNRMC PDSM_SET_U64BITMASK_BIT(30)   /* recommended minimum data */
#define PDAPI_NMEA_GNDTM PDSM_SET_U64BITMASK_BIT(31)   /* GNSS Datum Message */
#define PDAPI_NMEA_GNVTG PDSM_SET_U64BITMASK_BIT(32)   /* GNSS: Speed and heading info */

/* update the PDAPI_NMEA_MAX_VALUE when new fields are added */
#define PDAPI_NMEA_NUM_LEGACY (6)
#define PDAPI_NMEA_LEGACY PDSM_SET_U64BITMASK_MAX(PDAPI_NMEA_NUM_LEGACY)

#define PDAPI_NMEA_NUM_EXTENDED (16)
#define PDAPI_NMEA_NUM_EXTENDED_V2 (11)

#define PDAPI_NMEA_MAX_VALUE PDSM_SET_U64BITMASK_MAX(PDAPI_NMEA_NUM_LEGACY + PDAPI_NMEA_NUM_EXTENDED + PDAPI_NMEA_NUM_EXTENDED_V2)
...
/* OEM control for GNSS lock settings for Non-Framework Clients. NV 74235.
   Only NV control. No QMI SET/GET allowed to override OEM settings.

   CP_NILR_OEM_CTRL         = 0 = ACCEPT_ALWAYS (Default), 1 = NFW_LOCK_CONTROLED 
   SUPL_NI_OEM_CTRL         = 0 = ACCEPT_ALWAYS (Default), 1 = REJECT_ALWAYS 
   NFW_OEM_CTRL             = 0 = ACCEPT_ALWAYS (Default), 1 = NFW_LOCK_CONTROLED 
   CP_NILR_NOTIFY_OEM_CTRL  = 0 = DON’T_ALLOW (Default),   1 = ALLOW 
   NFW_NOTIFY_OEM_CTRL      = 0 = DON’T_ALLOW (Default),   1 = ALLOW */
#define PDSM_PA_CP_NILR_OEM_CTRL        PDSM_SET_U32BITMASK_BIT(0)
#define PDSM_PA_SUPL_NI_OEM_CTRL        PDSM_SET_U32BITMASK_BIT(1)
#define PDSM_PA_NFW_OEM_CTRL            PDSM_SET_U32BITMASK_BIT(2)
#define PDSM_PA_CP_NILR_NOTIFY_OEM_CTRL PDSM_SET_U32BITMASK_BIT(3)
#define PDSM_PA_NFW_Notify_OEM_CTRL     PDSM_SET_U32BITMASK_BIT(4)
...
/** UAX3 Operations */
#define PDSM_PA_UAX3_MASK_ENABLE  0x01
#define PDSM_PA_UAX3_MASK_ALL     PDSM_PA_UAX3_MASK_ENABLE
...
#define PDSM_PA_LPP_IN_NR5G_SA_UP_ENABLE      0x04
#define PDSM_PA_LPP_IN_NR5G_SA_CP_ENABLE      0x08


/* LPPE config
   from LSB - MSB 
   Bit 0 Enable/Disable LPPe in UP
   Bit 1 Enable/Disable DBH
   Bit 2 Enable/Disable WLAN measurements */
typedef uint32 pdsm_pa_lppe_config_type;
#define PDSM_PA_LPPE_ENABLE 0x01 /* LPPE UP Enable/disable*/
#define PDSM_PA_LPPE_DBH_ENABLE 0x02 /* LPPE UP DBH Enable/disable*/
#define PDSM_PA_LPPE_WLAN_ENABLE 0x04 /* LPPE UP WLAN Enable/disable*/
#define PDSM_PA_LPPE_SRN_BTLE_ENABLE 0x08 /* LPPE UP SRN BTLE enable/disable*/
#define PDSM_PA_LPPE_UBP_ENABLE 0x10 /* LPPE UP UBP enable/disable */
...
typedef enum {
  ...
  PDSM_PA_SUPL_TLS_VERSION_1_2,  /* SUPL TLS Version 1.2 */
...
/* Supl version value */
#define PDSM_SUPL_VERSION_1_0            (0x00010000)
#define PDSM_SUPL_VERSION_2_0            (0x00020000)
#define PDSM_SUPL_VERSION_2_0_2          (0x00020002)
#define PDSM_SUPL_VERSION_2_0_4          (0x00020004)
...
typedef struct pdsm_emergency_support_config_s
{
  uint32 emergency_support_config_valid; /* Defines the bit mask for validity for each bit. 1 ==valid, 0 ==not valid */
  boolean emergency_cpi_injections_enabled;
  boolean emergency_early_exit_enabled;
}pdsm_emergency_support_config_s_type;

/*  CPI Injections for E911 validity flag */
#define PDSM_PA_E911_CPI_INJECTIONS_VALID  (0x00000001)
/*  Early exit for E911 validity flag */
#define PDSM_PA_E911_EARLY_EXIT_VALID      (0x00000002)

/*  Concurrent sessions validity flag */
#define PDSM_PA_E911_CONCURRENCY_VALID  (0x00000001)

/* configuration for Premium Services Type */
typedef enum
{
  PDSM_PA_PREMIUM_SVC_GTP_CELL,  /* Premium Service: (GTP WWAN)
  ...
  PDSM_PA_PREMIUM_SVC_GTP_WIFI,  /* Premium service: GTP WIFI */
...
typedef enum
{
  ...
  PDSM_PA_PREMIUM_SVC_CFG_ENABLED_THIRDPARTY, /* Premium Service Enabled for Third Party*/
...
/* PrisonerSV extension: GNSS Persistent Blacklist SV Mask type */
typedef struct
{
  ...
  uint16 w_NavicPrnMask;  /* Bits[0-13]: NAVIC PRNs */
  uint64 t_SbasPrnMask;   /* Bits[0-50]: SBAS PRNs */
} pdsm_pa_persist_sv_mask_type;

/* PrisonerSV extension: Set/Clear Persistent Blacklist SV Masks */
typedef struct
{
  pdsm_pa_persist_sv_mask_type z_SetMask;
  pdsm_pa_persist_sv_mask_type z_ClrMask;
} pdsm_pa_persist_sv_mask_s_type;

typedef pdsm_constellation_e_type pdsm_pa_constell_rpt_cfg_mask_e_type;

/********************* Dynamic Constellation Control API *********************/
/* SET API: Bitmask config for GNSS constellations */
typedef uint64_t pdsm_constellation_mask;
#define PDSM_LOC_CONSTELLATION_GLO  ((pdsm_constellation_mask)0x00000001ULL) /**<  Enable GLONASS.  */
#define PDSM_LOC_CONSTELLATION_BDS  ((pdsm_constellation_mask)0x00000002ULL) /**<  Enable BDS.  */
#define PDSM_LOC_CONSTELLATION_QZSS ((pdsm_constellation_mask)0x00000004ULL) /**<  Enable QZSS.  */
#define PDSM_LOC_CONSTELLATION_GAL  ((pdsm_constellation_mask)0x00000008ULL) /**<  Enable GALILEO.  */
#define PDSM_LOC_CONSTELLATION_NAVIC ((pdsm_constellation_mask)0x00000010ULL) /**<  Enable NAVIC.  */

typedef struct
{
  boolean v_resetToNvDefault;
  pdsm_constellation_mask t_enableMask;
} pdsm_constellation_cfg_s_type;

/* GET API: GNSS Constellation Status */
typedef enum
{
  PDSM_CONSTELLATION_STATUS_INVALID = 0,
  /* Enabled Status Reasons */
  PDSM_CONSTELLATION_ENABLED_MANDATORY, //  Mandatory Constellation. Always Enabled
  PDSM_CONSTELLATION_ENABLED_INTERNALLY, //  Enabled Internally by System Software
  PDSM_CONSTELLATION_ENABLED_BY_CLIENT, //  Enabled by External Client

  /* Disabled Status Reasons */
  PDSM_CONSTELLATION_DISABLED_NOT_SUPPORTED = 128, //  Constellation not supported
  PDSM_CONSTELLATION_DISABLED_INTERNALLY, //  Disabled Internally by System Software
  PDSM_CONSTELLATION_DISABLED_BY_CLIENT, //  Disabled by External Client
  PDSM_CONSTELLATION_DISABLED_NO_MEMORY, //  Could not be enabled due to memory allocation failure
  PDSM_CONSTELLATION_STATUS_UNKNOWN = 0xFF
} pdsm_constellation_status_e_type_val;
typedef uint8 pdsm_constellation_status_e_type;

typedef struct
{
  pdsm_constellation_status_e_type gps_status;
  pdsm_constellation_status_e_type glo_status;
  pdsm_constellation_status_e_type bds_status;
  pdsm_constellation_status_e_type qzss_status;
  pdsm_constellation_status_e_type gal_status;
  pdsm_constellation_status_e_type navic_status;
} pdsm_constellation_status_s_type;

/* Common Data Structure to be used by SET/GET */
typedef union
{
  pdsm_constellation_cfg_s_type    z_Set;
  pdsm_constellation_status_s_type z_Get;
} pdsm_constellation_control_u_type;
/*****************************************************************************/

typedef enum pdsm_pa_otdoa_cfg
{
  PDSM_PA_OTDOA_DISABLE,
  PDSM_PA_OTDOA_ENABLE,
  PDSM_PA_OTDOA_MAX,
  PDSM_PA_OTDOA_MAX_RESERVED = 4294967295u /*Force 32 bit unsigned int*/
}pdsm_pa_otdoa_cfg_e_type;

typedef enum
{
  PDSM_GNSS_MEASUREMENT_REPORT = 0x01,
  PDSM_GNSS_SV_INFORMATION_REPORT = 0x02,
  PDSM_GNSS_MEASUREMENT_USAGE_REPORT = 0x04,
  PDSM_GNSS_TIME_CONV_PARAM_REPORT = 0x08,
  PDSM_GNSS_CLOCK_MODEL_PARAM_REPORT = 0x10,
  PDSM_GNSS_KLOBUCHAR_PARAM_REPORT = 0x20,
}pdsm_GnssReportingConfigVal;
typedef uint64 pdsm_GnssReportingConfig;
...
typedef union pdsm_pa_info
{
  ...
  pdsm_emergency_support_config_s_type emergency_support_config;    /* Config for special handling for emergency sessions */
  pdsm_pa_lppe_config_type       lppe_up_config_info;            /* configuration for LPPe UP */
  pdsm_pa_lppe_config_type       lppe_cp_config_info;            /* configuration for LPPe CP */
  uint32                            uax3_config_info;               /* UAX3 Config Info. 0=UAX3 is enabled, 1=UAX3 disabled*/
  pdsm_pa_persist_sv_mask_s_type    persistent_sv_mask;             /* GNSS Persistent Blacklist SV Mask */
  boolean                           on_demand_lpm;                  /* On Demand Low power mode */
  boolean                           on_demand_ready;                /* On Demand READDY mode */
  pdsm_pa_constell_rpt_cfg_mask_e_type meas_report_cfg;             /* GNSS measurement report constellation configuration */
  pdsm_pa_otdoa_cfg_e_type          otdoa_cfg;                      /*Dynamically enable disable OTDOA*/
  boolean                           v_engine_monitor_report_config; /* Config reporting of gnss engine status */
  uint64                            t_gnss_reporting_config;
  boolean                           v_override_e911_notification_config; /* Config to override E911 notification from CM. 
                                                                            TRUE == ignore E911 notifications, FALSE == honor E911 notifications*/
  pdsm_constellation_control_u_type z_constell_cfg;                 /* Constellation Control */
  uint8  u_cont_nmea_sentence;
  uint32                            q_emergency_extension_window_config; /* Config that specify time in seconds to extend emergency state */
  uint64                            v_ActivateMultiband : 1;
  uint64                                                : 63;
} pdsm_pa_info_type;
...
/*~ IF (_DISC_ == PDSM_PA_PCID) pdsm_pa_info.pcid_info */
...
/*Bits to determine datum*/
#define PDSM_PA_WGS84_DATUM_NMEA_SENTENCE  (0x01)
#define PDSM_PA_PZ90_DATUM_NMEA_SENTENCE   (0x02)
...
#define PDSM_PD_EVENT_WIFI_INFO_ON_DEMAND_INJ_RELATED ( (PDSM_PD_EVENT_REQUEST_WIFI_INFO_ON_DEMAND_INJECTION) \
                                                        | (PDSM_PD_EVENT_ACK_WIFI_INFO_ON_DEMAND_INJECTION) \
                                                       )
/* request SRN Scan*/
#define PDSM_PD_EVENT_REQUEST_SRN_INFO_INJECTION 0x00040000
#define PDSM_PD_EVENT_ACK_SRN_INFO_INJECTION 0x00080000
...
#define PDSM_PD_EVENT_MASK  ( (PDSM_PD_GENERAL_EVENT_MASK) \
                             ...
                             | (PDSM_PD_EVENT_ACK_WIFI_INFO_ON_DEMAND_INJECTION) \
                             | (PDSM_PD_EVENT_REQUEST_SRN_INFO_INJECTION) \
                             | (PDSM_PD_EVENT_ACK_SRN_INFO_INJECTION) \
                            )
...
#define PDSM_PD_POSITION_SOURCE_CPI         0x4000  /* Specifically indicate that the EPI is a CPI */
#define PDSM_PD_POSITION_SOURCE_NAVIC       0x8000  /* Source is navIC  */
...
#define PDSM_PD_LAT_LONG_VALID 0x01 /* indicates if the lat/lon in the position report and ext pos is valid */
...
#define PDSM_PD_HEADING_VALID  0x10 /*indicates if heading in position report and ext pos is valid*/
#define PDSM_PD_EXT_VELOCITY_VALID  0x20 /*indicates if ITAR Velocity velHorU is valid*/
#define PDSM_PD_EXT_UNCERTAINTY_VALID 0x40 /*indicates if extended loc uncertainty angle is valid*/
...
#define  PDSM_PD_SV_ID_QZSSL1S_MIN  183
#define  PDSM_PD_SV_ID_QZSSL1S_MAX  191
...
#define  PDSM_PD_SV_ID_NAVIC_MIN     401
#define  PDSM_PD_SV_ID_NAVIC_MAX     414
...
#define PDSM_GNSS_TIME_NAVIC  (0x01<<4)
...
typedef enum {
  ...
  PDSM_GNSS_TIME_SRC_SLEEP_CLOCK              = 0x11, /* Time is set from sleep clock */
  PDSM_GNSS_TIME_SRC_SLEEP_CLOCK_TIME_TRANSFER = 0x12, /* Time is set from sleep/slow clock time */
  PDSM_GNSS_TIME_SRC_NAVIC_TOW_DECODE          = 0x13, /* Time is set after decoding NAVIC satellites */
...
typedef uint64_t pdsm_spoof_type_mask;
#define PDSM_SPOOF_EVENT_POSITION_DOMAIN ((pdsm_spoof_type_mask)0x00000001ull) /**<  Set bit indicates suspected spoofing in position  */
#define PDSM_SPOOF_EVENT_TIME_DOMAIN ((pdsm_spoof_type_mask)0x00000002ull) /**<  Set bit indicates suspected spoofing in time  */
#define PDSM_SPOOF_EVENT_NAVIGATION_DATA_DOMAIN ((pdsm_spoof_type_mask)0x00000004ull) /**<  Set bit indicates suspected spoofing in navigation data  */
#define PDSM_SPOOF_EVENT_ALL_DOMAINS ( PDSM_SPOOF_EVENT_POSITION_DOMAIN |\
                                       PDSM_SPOOF_EVENT_TIME_DOMAIN |\
                                       PDSM_SPOOF_EVENT_NAVIGATION_DATA_DOMAIN )
typedef struct pdsm_spoof_report
{
  pdsm_spoof_type_mask spoof_type_mask;
  uint64 last_spoof_event_timetick;
} pdsm_spoof_report;
...
} pdsm_gnss_jammerIndicatorStructType;

typedef enum
{
  PD_LOC_GNSS_SIGNAL_TYPE_INVALID = 0,
  /**<  Invalid Signal Type  */
  PD_LOC_GNSS_SIGNAL_TYPE_GPS_L1CA = 1,
  /**<  GPS L1CA Signal  */
  PD_LOC_GNSS_SIGNAL_TYPE_GPS_L1C = 2,
  /**<  GPS L1C Signal  */
  PD_LOC_GNSS_SIGNAL_TYPE_GPS_L2C_L = 3,
  /**<  GPS L2C_L RF Band  */
  PD_LOC_GNSS_SIGNAL_TYPE_GPS_L5_Q = 4,
  /**<  GPS L5_Q RF Band  */
  PD_LOC_GNSS_SIGNAL_TYPE_GLONASS_G1 = 5,
  /**<  GLONASS G1 (L1OF) RF Band  */
  PD_LOC_GNSS_SIGNAL_TYPE_GLONASS_G2 = 6,
  /**<  GLONASS G2 (L2OF) RF Band  */
  PD_LOC_GNSS_SIGNAL_TYPE_GALILEO_E1_C = 7,
  /**<  GALILEO E1_C RF Band  */
  PD_LOC_GNSS_SIGNAL_TYPE_GALILEO_E5A_Q = 8,
  /**<  GALILEO E5A_Q RF Band  */
  PD_LOC_GNSS_SIGNAL_TYPE_GALILEO_E5B_Q = 9,
  /**<  GALILEO E5B_Q RF Band  */
  PD_LOC_GNSS_SIGNAL_TYPE_BEIDOU_B1_I = 10,
  /**<  BEIDOU B1_I RF Band  */
  PD_LOC_GNSS_SIGNAL_TYPE_BEIDOU_B1C = 11,
  /**<  BEIDOU B1C RF Band  */
  PD_LOC_GNSS_SIGNAL_TYPE_BEIDOU_B2_I = 12,
  /**<  BEIDOU B2_I RF Band  */
  PD_LOC_GNSS_SIGNAL_TYPE_BEIDOU_B2A_I = 13,
  /**<  BEIDOU B2A_I RF Band  */
  PD_LOC_GNSS_SIGNAL_TYPE_QZSS_L1CA = 14,
  /**<  QZSS L1CA RF Band  */
  PD_LOC_GNSS_SIGNAL_TYPE_QZSS_L1S = 15,
  /**<  QZSS L1S RF Band  */
  PD_LOC_GNSS_SIGNAL_TYPE_QZSS_L2C_L = 16,
  /**<  QZSS L2C_L RF Band  */
  PD_LOC_GNSS_SIGNAL_TYPE_QZSS_L5_Q = 17,
  /**<  QZSS L5_Q RF Band  */
  PD_LOC_GNSS_SIGNAL_TYPE_SBAS_L1_CA = 18,
  /**<  SBAS L1_CA RF Band  */
  PD_LOC_GNSS_SIGNAL_TYPE_NAVIC_L5 = 19,
  /**<  NAVIV_L5 RF Band  */
  PD_LOC_GNSS_SIGNAL_TYPE_BEIDOU_B2A_Q = 20,
  /**<  BEIDOU B2AQ RF Band  */
  PD_LOC_GNSS_SIGNAL_TYPE_MAX = 21,
  /**<  Max  */
} pdsm_LocGnssSignalEnumType;
...
typedef struct pdsm_pd_ext_pos_data_s
{
  ...
  /* Valid if ext_opt_field_mask PDSM_PD_LAT_LONG_VALID is set*/
  float  f_alongAxisUnc;       /* Along axis Unc in meters with higher resolution. Max value is 4294967295. If this value is
                                 greater than 4294967295, then the opt_field_mask for
  ...
  float  f_perpAxisUnc;        /* Perp axis Unc in meters with higher resolution. Max value is 4294967295. If this value is
                                 greater than 4294967295, then the opt_field_mask for
                                 PDSM_PD_UNCERTATINITY_VALID is not set. loc_unc_horizontal value
                                 is used to get the circular UNC   */
  /* Valid if field_mask PDSM_PD_UNCERTAINTY_VALID is set */
  uint16 f_velHorU;             /*this field should be used for publishing hori velocity more than 0x7FFF*/
  /* Valid if opt_field_mask PDSM_PD_EXT_VELOCITY_VALID is set */
  float  f_velHor;            /* Horizontal Velocity: cm/sec., magnitude only, with better precision*/
  /* Valid if opt_field_mask PDSM_PD_VELOCITY_VALID is set */
  float  f_velVert;           /* Vertical Velocity: cm/sec., positive for upward, with better precision */
  /* Valid if opt_field_mask PDSM_PD_VELOCITY_VALID is set */
  ...
  /* Valid if ext_opt_field_mask PDSM_PD_LAT_LONG_VALID is set*/
  uint32        t_dop;               /* [x10]   Range: 1.0 (highest accuracy) 50.0 (lowest accuracy) */
  /* Valid if ext_opt_field_mask PDSM_PD_LAT_LONG_VALID is set*/
  uint32        g_dop;               /* [x10]   Range: 1.0 (highest accuracy) 50.0 (lowest accuracy) */
  /* Valid if ext_opt_field_mask PDSM_PD_LAT_LONG_VALID is set*/
  ...
  float  f_loc_unc_horizontal;     /*  The HEPE value in meters with higher resolution. In case PDSM_PD_UNCERTAINITY_VALID
  ...
  float f_loc_uncertainty_ang;  /* Valid if opt_field_mask PDSM_PD_EXT_UNCERTAINTY_VALID is set */
  /* Elliptical horizontal azimuth in QMI_LOC_EVENT_POSITION_REPORT depends on this */

  float ext_loc_uncertainty_v;  /* Raw altitude uncertainty in meters. Valid only if  opt_field_mask PDSM_PD_ALTITUDE_VALID is set*/
  pdsm_spoof_report spoof_report;
  boolean                              v_JammerIndicatorListValid;
  pdsm_gnss_jammerIndicatorStructType  z_JammerIndicatorList[PD_LOC_GNSS_SIGNAL_TYPE_MAX];
...
#define PDSM_N_ACTIVE_BDS_CHAN     24  /* Total number of active BDS channels */
...
#define PDSM_N_ACTIVE_NAVIC_CHAN   16  /* Total number of active NAVIC channels */


/* Total GPS+GLO+BDS+GAL+QZSS
 Ensure to update Max when new constellation is added
*/
#define PDSM_N_ACTIVE_MAX          (PDSM_N_ACTIVE_GPS_CHAN + \
                                    PDSM_N_ACTIVE_GLO_CHAN + \
                                    PDSM_N_ACTIVE_BDS_CHAN + \
                                    PDSM_N_ACTIVE_GAL_CHAN + \
                                    PDSM_N_ACTIVE_QZSS_CHAN + \
                                    PDSM_N_ACTIVE_NAVIC_CHAN)
...
#define PDSM_GNSS_SV_INFO_USED_IN_POS_VEL_FIX (PDSM_GNSS_SV_INFO_USED_IN_POS_FIX | PDSM_GNSS_SV_INFO_USED_IN_VEL_FIX)

/* If set, GNSS SV is corrected with SBAS IONO correction */
#define PDSM_GNSS_SV_INFO_SBAS_IONO_CORR  0x00000010
/* If set, GNSS SV is corrected with SBAS FAST correction */
#define PDSM_GNSS_SV_INFO_SBAS_FAST_CORR  0x00000020
/* If set, GNSS SV is corrected with SBAS Long-term correction */
#define PDSM_GNSS_SV_INFO_SBAS_LONG_CORR  0x00000040
/* If set, GNSS SV is selected with SBAS RTI */
#define PDSM_GNSS_SV_INFO_SBAS_RTI_CORR  0x00000080
...
typedef struct
{
  ...
  uint16 w_SvPrtyErr;   /* {GPS|BDS} the 10 LSB to indicate parity errors in the 10 words of current subframe
                         ...
                         * = 0:no error
                         */
  uint8 u_UsedPrMeasType; /*PR measurement type used in fix*/
  uint8 u_UsedPrrMeasType; /*PRR measurement type used in fix*/
  uint8 u_AvailMeasMask; /*available measurement types*/
} pdsm_GnssSvInfoType;
...
typedef struct
{
  ...
  uint8               u_NavicNumSvMeas;     /* Number of NavIC SVs with valid measurement (detected by searcher) */
  uint8               u_NavicNumSvPosFix;   /* Number of NavIC SVs used in Position Fix */
  uint8               u_NavicNumSvVelFix;   /* Number of NavIC SVs used in Velocity Fix */
  uint16              w_NavicSvMaskUsed;    /* Bitmask for NavIC SVs used for calculating position fix */
  uint16              w_NavicSvMaskUsable;  /* Bitmask for NavIC SVs usable for calculating position fix */
  pdsm_GnssSvInfoType z_NavicSvInfo[PDSM_N_ACTIVE_NAVIC_CHAN]; /* SV-specific data for NavIC supplemental SV information */
} pdsm_GnssNavSvInfo;
...
typedef enum
{
  PDSM_PD_EXTERN_CPI_SRC_EXTERNAL, /* Position is sourced from an external module (i.e. GNP or third party) */
  PDSM_PD_EXTERN_CPI_SRC_INTERNAL, /* Position is sourced from an internal QCOM module */
  PDSM_PD_EXTERN_CPI_SRC_MAX = 0x10000000  /* force enum to 32 bit */
}pdsm_pd_extern_cpi_src_provider_e_type;
...
typedef struct pdsm_pd_external_position_info
{
  ...
  pdsm_pd_extern_coarse_pos_source_e_type   source;
  pdsm_pd_extern_cpi_src_provider_e_type    src_provider; /* valid when posSrcProviderValid == 1 */
...
#define PDSM_PA_EVENT_ON_DEMAND_LPM               0x2000000  /* ON_DEMAND Low Power mode  - deprecated */
#define PDSM_PA_EVENT_ON_DEMAND_READY_MODE        0x4000000  /* ON_DEMAND READY mode - deprecated */
...
#define PDAPI_SUPLINIT_GNSS_POS_TECH_BITMAP_BDS        0x0040
...
typedef enum
{
  PDSM_LCS_SUPL_CERT_CHOICE_NONE = 0,
  PDSM_LCS_SUPL_CERT_CHOICE_FNAME,
  PDSM_LCS_SUPL_CERT_CHOICE_ID,
  PDSM_LCS_SUPL_CERT_CHOICE_MAX
}pdsm_lcs_supl_cert_choice_e_type;

/* SUPL Certificate for NI AGPS*/
typedef struct pdsm_lcs_supl_cert_info_s {
  pdsm_lcs_supl_cert_choice_e_type supl_cert_choice;
  uint8                            supl_cert_id;
  char                             supl_cert_hash_fname[33]; /*NULL terminate, max 32 bytes*/
}pdsm_lcs_supl_cert_info_s_type;
...
typedef struct pdsm_lcs_supl_notify_verify_req_s {
  ...
  pdsm_lcs_supl_cert_info_s_type cert_info;
  /*AP can choose to specify a particular Certificate file the AGPS session. 
    This field carries
     1)Cert name (32 bytes from the SHA256 key generated for the entore certificate)
     2)Cert ID   [0-9] (value provided when injecting the Certificate)
  */
...
typedef enum pdsm_ext_status_e
{
  ...
  DEPRECATED_PDSM_EXT_STATUS_SPOOFING_REPORT, /* Deprecated Gnss Spoofing detection report */
  PDSM_EXT_STATUS_ENGINE_MONITOR_REPORT,      /* GNSS Engine Monitor report */
  PDSM_EXT_STATUS_OTDOA_SESSION_INFO_TYPE,    /* OTDOA Session info */
  PDSM_EXT_STATUS_OEM_LPPM_STATUS_TYPE,       /* OEM specific LPPM status */
  PDSM_EXT_STATUS_SESS_REQ_TYPE,              /* Session type requested by network */
  PDSM_EXT_STATUS_SV_POSITION_INFO,           /* SV Information */
  PDSM_EXT_STATUS_TIME_CONV_PARAM_REPORT,     /* Time Conversion Parameters */
  PDSM_EXT_STATUS_CLK_MODEL_PARAM_REPORT,     /* Clock Process Model Parameters */
  PDSM_EXT_STATUS_REPORT_ME_METRICS,
  PDSM_EXT_STATUS_UNPROPAGATED_POS_REPORT,    /*Unpropagated fix report*/
  PDSM_EXT_STATUS_LOC_NOTIFICATION_REPORT,    /* Notifies LocMW of the location info shipped outside modem */
  PDSM_EXT_STATUS_E911_STATE_NOTIFICATION_REPORT, /* Indicates E911 state to LocMW */
...
#define PDSM_EXT_STATUS_EVENT_OEM_LPPM_STATUS_TYPE          0x80000    /* OEM specific LPPM Status indication */
#define PDSM_EXT_STATUS_EVENT_OTDOA_SESSION_INFO_TYPE       0x100000   /* OTDOA session Status indication */
#define PDSM_EXT_STATUS_EVENT_SESSION_REQ_TYPE              0x200000   /* Session Req Type ndication */
#define DEPRECATED_PDSM_EXT_STATUS_EVENT_SPOOFING_REPORT    0x400000   /* Deprecated Spoofing detection support */
#define PDSM_EXT_STATUS_EVENT_ENGINE_MONITOR_REPORT         0x800000   /* GNSS Engine Monitor support */
#define PDSM_EXT_STATUS_EVENT_SV_INFORMATION                0x1000000   /* SV Information */
#define PDSM_EXT_STATUS_EVENT_TIME_CONV_PARAM               0x2000000   /* Time Conversion Parameters */
#define PDSM_EXT_STATUS_EVENT_CLK_MODEL_PARAM               0x4000000   /* Clock Model Parameters*/
#define PDSM_EXT_STATUS_EVENT_UNPROPAGATED_POS_REPORT       0x8000000
#define PDSM_EXT_STATUS_EVENT_REPORT_ME_METRICS             0x10000000
#define PDSM_EXT_STATUS_EVENT_LOC_NOTIFICATION_REPORT       0x20000000  /* Locaton Notification */
#define PDSM_EXT_STATUS_EVENT_E911_NOTIFICATION_REPORT      0x40000000  /* E911 Notification */
...
#define PDSM_EXT_STATUS_EVENT_MASK ( (PDSM_EXT_STATUS_EVENT_NMEA_POS_REPORT)             | \
                                     ...
                                     (PDSM_EXT_STATUS_EVENT_OEM_LPPM_STATUS_TYPE)        | \
                                     (PDSM_EXT_STATUS_EVENT_OTDOA_SESSION_INFO_TYPE)     | \
                                     (PDSM_EXT_STATUS_EVENT_SESSION_REQ_TYPE)            | \
                                     (PDSM_EXT_STATUS_EVENT_ENGINE_MONITOR_REPORT)       | \
                                     (PDSM_EXT_STATUS_EVENT_SV_INFORMATION)              | \
                                     (PDSM_EXT_STATUS_EVENT_TIME_CONV_PARAM)             | \
                                     (PDSM_EXT_STATUS_EVENT_CLK_MODEL_PARAM)             | \
                                     (PDSM_EXT_STATUS_EVENT_REPORT_ME_METRICS)           | \
                                     (PDSM_EXT_STATUS_EVENT_UNPROPAGATED_POS_REPORT)     | \
                                     (PDSM_EXT_STATUS_EVENT_LOC_NOTIFICATION_REPORT)     | \
                                     (PDSM_EXT_STATUS_EVENT_E911_NOTIFICATION_REPORT))
...
/* Max Active Channels: 176 (defined in QMI) */
#define PDSM_EXT_MEAS_NUM_SVS 176
...
typedef enum
{
  ...
  PDSM_SV_STATE_TRACK,
  PDSM_SV_STATE_VERIFY_TRACK,
  PDSM_SV_STATE_RESTART,
  PDSM_SV_STATE_DPO_TRACK,
  PDSM_SV_STATE_GLO_10MSEC_BIT_EDGE,
  PDSM_SV_STATE_LPPM_SEARCH,
  PDSM_SV_STATE_MAX,
...
typedef struct
{
  ...
  uint16  c_no;               /* CNo. Units of 0.1 dB-Hz */
  uint16  rf_loss;            /* RF loss from antenna to baseband. Units of 0.1 dB-Hz */
  ...
  pdsm_LocSvSearchStatusEnumT  sv_state;    /* SV Observation state */
  ...
  pdsm_LocGnssSignalEnumType gnss_signal_type; /* GNSS Signal Type */
...
typedef struct
{
  ...
  uint64      navic_eph_svmask;     /* available ephemeris mask (NavIC only) */
  uint64      navic_alm_svmask;     /* available almanac mask (NavIC only) */
  uint64      navic_health_svmask;  /* NavIC SV mask for the health. if the bit is set, health for that SV is good */
  uint32      navic_iono;           /* NavIC IONO is Valid */
  ...
  uint64  bds_health_svmask; /* BDS SV mask for the health. if the bit is set, health for that SV is good */
  uint64  gal_health_svmask; /* GAL SV mask for the health. if the bit is set, health for that SV is good */
  uint32  gal_iono; /* GAL IONO is Valid */
  uint32  glo_health_svmask; /* glo  SV mask for the health. if the bit is set, health for that SV is good */
...
typedef enum pdsm_xtra_cmd_err_e
{
  ...
  PDSM_XTRA_CMD_ERR_NOERR = 0,
  PDSM_XTRA_CMD_ERR_CLIENT_ID_P = 1,
  PDSM_XTRA_CMD_ERR_NO_BUF_L = 2,
  PDSM_XTRA_CMD_ERR_BAD_CRC = 3,
  PDSM_XTRA_CMD_ERR_TIME_OF_VALIDITY_OUT_OF_RANGE = 4,
  PDSM_XTRA_CMD_ERR_DOWNLOAD_INTRVL_OUT_OF_RANGE = 5,
  PDSM_XTRA_CMD_ERR_INTERNAL_RESOURCE_ERROR = 6,
  PDSM_XTRA_CMD_ERR_GPS_SUBSYSTEM_BUSY = 7,
  PDSM_XTRA_CMD_ERR_TIME_INFO_ERROR = 8,
  PDSM_XTRA_CMD_ERR_OTHER = 9,
  PDSM_XTRA_CMD_ERR_GPS_LOCK_S = 10,           /* GPS is locked */
  PDSM_XTRA_CMD_ERR_STATE_S = 11,              /* Command is invalid in this state. (Ex When is phone is in E911CB) */
  PDSM_XTRA_CMD_ERR_UNKNOWN_CMD = 12,          /* Unknown command */
  PDSM_XTRA_CMD_ERR_FEATURE_NOT_ENABLED = 13,  /* Feature not enabled */
  PDSM_XTRA_CMD_ERR_INVALID_PARAM = 14,        /* Invalid parameter */
  PDSM_XTRA_CMD_ERR_RESET_LOCATION_SERVICE_IN_PROGRESS_S = 15, /*Location Service Reset in progress*/
  PDSM_XTRA_CMD_ERR_MAX = 16,
...
typedef enum pdsm_pd_end_e {
  ...
  PDSM_PD_END_LPPM_ALREADY_ENABLED,  /* LPPM is already enabled when enable command is received */
  PDSM_PD_END_LPPM_ALREADY_DISABLED, /* LPPM is already disabled when disable command is received */
  PDSM_PD_END_LPPM_CONTROL_ERROR,    /* LPPM  control change had an error */
  ...
  PDSM_PD_END_SUPL_INVALID_CERTIFICATE_INDEX, /* SUPL error, invalid certificate index/NAME */
...
#define   PDSM_PD_FIX_STATUS_DEBUG_FAIL            (0x0000)       /* Unsuccessful fix*/
#define   PDSM_PD_FIX_STATUS_DEBUG_SUCCESS         (0x0001)       /* Successful Fix.  */
#define   PDSM_PD_FIX_STATUS_DEBUG_LOW_SV          (0x0002)       /* LOW SV's.*/
#define   PDSM_PD_FIX_STATUS_DEBUG_HEPE_FAIL       (0x0004)       /* HEPE Check Failed.   */
#define   PDSM_PD_FIX_STATUS_DEBUG_LOW_RELIABILITY (0x0008)       /* LOW reliability Fix */
...
#define PDSM_OTDOA_SESS_TYPE_FLAG_VALID (0x0001)
typedef enum
{
  PDSM_OTDOA_SESS_BEGIN,
  PDSM_OTDOA_SESS_DONE
}pdsm_otdoa_sess_ind_e_type;

typedef struct
{
  uint32 otdoa_validity_flags;
  pdsm_otdoa_sess_ind_e_type e_sess_type;
} pdsm_pd_ext_status_otdoa_sess_info_type;

typedef struct
{
  uint32 sess_type;
  pdsm_pd_comm_protocol_e_type protocol_type;
} pdsm_pd_ext_status_network_sess_req_type;

#define PDSM_PD_SESS_REQ_TYPE_MSB_GPS 0x0001 /* AGPS MSBased Session Request */
#define PDSM_PD_SESS_REQ_TYPE_MSA_GPS 0x0002 /* AGPS MSAssisted Session Request */
#define PDSM_PD_SESS_REQ_TYPE_OTDOA 0x0004   /* OTDOA Measurement Request */
#define PDSM_PD_SESS_REQ_TYPE_ECID 0x0008    /* ECID Measurement Request */
#define PDSM_PD_SESS_REQ_TYPE_AFLT 0x0010    /* AFLT Measurement Request */
#define PDSM_PD_SESS_REQ_TYPE_MSB_GLO 0x0020 /* AGLO MSBased Session Request */
#define PDSM_PD_SESS_REQ_TYPE_MSA_GLO 0x0040 /* AGLO MSAssisted Session Request */
#define PDSM_PD_SESS_REQ_TYPE_STANDALONE 0x0080 /* Standalone Session Request */

/* Validity bits for RF Group delay structure */
#define  PDSM_MEAS_RF_GROUP_DELAY_VALID      0x01 /* RF Group delay valid */
#define  PDSM_MEAS_RF_GROUP_DELAY_UNC_VALID  0x02 /* RF Group delay Uncertainty valid*/

typedef struct
{
  uint32  q_validMask;         /* Validity mask RF group delay fields*/
  double  d_RfGroupDelay;      /* RF Group delay (meters)*/
  double  d_RfGroupDelayUnc;   /* RF Group delay Uncertainty (meters)*/
} pdsm_meas_rf_group_delay_s_type;

#define PDSM_EXT_GNSS_MEAS_NUM_SVS   24 /**< Maximum number of satellites in measurement block for given system. */
...
typedef enum
{
  ...
  PD_LOC_MEAS_STATUS_LP_VALID                = 0x00000040,
  /**< Lock Point is valid/invalid.*/

  PD_LOC_MEAS_STATUS_LP_POS_VALID                = 0x00000080,
  /**< Lock Point positive/negative */
  ...
  PD_LOC_MEAS_STATUS_TDM_MEAS                     = 0x00004000,

  PD_LOC_MEAS_STATUS_DONT_USE_P                   = 0x00008000,

  PD_LOC_MEAS_STATUS_DONT_USE_I_J                 = 0x02000000,
  PD_LOC_MEAS_STATUS_DONT_USE_L_J                 = 0x04000000,
  PD_LOC_MEAS_STATUS_DONT_USE_S_M                 = 0x10000000,

  PD_LOC_MEAS_STATUS_GNSS_FRESH_MEAS_VALID        = 0x08000000,
...
#define PD_LOC_MEAS_SV_HEALTH_STATUS_HEALTHY    0x01
#define PD_LOC_MEAS_SV_HEALTH_STATUS_UNHEALTHY  0x00
...
typedef enum
{
  ...
  PD_LOC_MEAS_STATUS2_DONT_USE_I_J                 = 0x02000000,
  PD_LOC_MEAS_STATUS2_DONT_USE_L_J                 = 0x04000000,
  PD_LOC_MEAS_STATUS2_DONT_USE_S_M                 = 0x10000000,

}pdsm_LocSvMeasStatus2DontUseMaskType;

/* meas stauts 2 dont use mask */

#define PD_LOC_MEAS_STATUS2_DONT_USE_BIT_MASK  ( PD_LOC_MEAS_STATUS2_DONT_USE_X \
	                                            | PD_LOC_MEAS_STATUS2_DONT_USE_M \
	                                            | PD_LOC_MEAS_STATUS2_DONT_USE_D \
	                                            | PD_LOC_MEAS_STATUS2_DONT_USE_P \
	                                            | PD_LOC_MEAS_STATUS2_DONT_USE_I_J \
	                                            |PD_LOC_MEAS_STATUS2_DONT_USE_L_J \
	                                            | PD_LOC_MEAS_STATUS2_DONT_USE_S_M \
	                                            )


/* meas staus mask */
#define PD_LOC_MEAS_STATUS_BIT_MASK  ( PD_LOC_MEAS_STATUS_SM_VALID \
	                                  | PD_LOC_MEAS_STATUS_SB_VALID \
	                                  | PD_LOC_MEAS_STATUS_MS_VALID \
	                                  | PD_LOC_MEAS_STATUS_BE_CONFIRM \
	                                  | PD_LOC_MEAS_STATUS_VELOCITY_VALID \
	                                  | PD_LOC_MEAS_STATUS_VELOCITY_FINE \
	                                  | PD_LOC_MEAS_STATUS_LP_VALID \
	                                  | PD_LOC_MEAS_STATUS_LP_POS_VALID \
	                                  | PD_LOC_MEAS_STATUS_FROM_RNG_DIFF \
	                                  | PD_LOC_MEAS_STATUS_FROM_VE_DIFF \
...
typedef struct
{
  uint64                             v_healthStatus_valid : 1;
  uint64                             v_lossOfLockValid : 1;
  uint64                             v_dopplerAccelValid : 1;
  uint64                             v_multipathEstValid : 1;
  uint64                             v_fineSpeedValid : 1;
  uint64                             v_fineSpeedUncValid : 1;
  uint64                             v_carrierPhaseValid : 1;
  uint64                             v_carrierPhaseUncValid : 1;
  uint64                             v_cycleSlipCountValid : 1;
  uint64                             v_svDirectionValid : 1;
  uint64                             v_SnrValid : 1;
  uint64                             v_AbsSigPowerValid : 1;
  uint64                             : 52;
  uint16                             w_gnssSvId; /* GNSS SV ID */
  uint8                              u_gloFrequency; /* GLONASS frequency number + 7 */
  pdsm_LocSvSearchStatusEnumT        e_svStatus; /* Satellite Search State */
  uint8                              u_healthStatus; /* Health Status */
  pdsm_LocSvInfoMaskT                e_svInfoMask; /* SV Info Mask */
  uint64                             t_measurementStatus; /* Bitmask indicating SV measurement status */
  uint64                             t_measurementStatusValid; /* Bitmask of valid SV measurement status bits */
  uint16                             w_CNo; /* Carrier to Noise ratio */
  uint16                             w_gloRfLoss; /* GLONASS Rf loss reference to Antenna */
  boolean                            b_lossOfLock; /* Loss of signal lock indicator */
  int16                              x_measLatency; /* Age of the measurement */
  pdsm_LocSVTimeSpeedStructType      z_svTimeSpeed; /* Unfiltered SV Time and Speed information */
  float                              f_dopplerAccel; /* Satellite Doppler Acceleration */
  float                              f_multipathEstimate; /* Estimate of multipath in measurement */
  float                              f_fineSpeed; /* Carrier phase derived speed */
  float                              f_fineSpeedUnc; /* Carrier phase derived speed */
  double                             d_carrierPhase; /* Carrier phase measurement [L1 cycles] */
  uint8                              u_cycleSlipCount; /* Increments when a CSlip is detected */
  float                              f_svAzimuth; /* Satellite Azimuth */
  float                              f_svElevation; /* Satellite Elevation */
  ...
  uint16                             w_ParityErrorCount; /* The 10 LSB to indicate parity errors */
  float   f_CarrierPhaseUnc;     /* Carrier phase uncertainty in L1 cycles */
  float   f_Snr;                /* Signal to noise ratio */
  float   f_AbsSigPower;        /* Absolute Signal Power in dBm */
} pdsm_LocSVMeasurementStructType;

typedef struct
{
  pdsm_constellation_e_type e_system;
...
typedef enum
{
  ...
  */
  /* B2A Q */
  PDSM_MEASUREMENT_CODE_TYPE_Q = 7,

  /** GPS L1C (D), GPS L2C (M), QZSS L1C (D), QZSS L2C (M), LEX(6) S. */
  PDSM_MEASUREMENT_CODE_TYPE_S = 8,

  /** GPS L1 Z-tracking, GPS L2 Z-tracking. */
  PDSM_MEASUREMENT_CODE_TYPE_W = 9,
  ...
  * B3 (I+Q), IRNSS L5 (B+C).
  */
  PDSM_MEASUREMENT_CODE_TYPE_X = 10,

  /** GPS L1Y, GPS L2Y. */
  PDSM_MEASUREMENT_CODE_TYPE_Y = 11,

  /** GALILEO E1 (A+B+C), GALILEO E6 (A+B+C), QZSS L1-SAIF. */
  PDSM_MEASUREMENT_CODE_TYPE_Z = 12,

  /** GPS L1 codeless, GPS L2 codeless. */
  PDSM_MEASUREMENT_CODE_TYPE_CODELESS = 13
}pdsm_gnss_measurement_code_EnumType;
...
typedef struct
{
  ...
  pdsm_InterSystemBiasStructType    z_gpsNavicInterSystemBias; /* Inter GNSS time bias structure */
  pdsm_InterSystemBiasStructType    z_galNavicInterSystemBias; /* Inter GNSS time bias structure */
  pdsm_InterSystemBiasStructType    z_gloNavicInterSystemBias; /* Inter GNSS time bias structure */
  pdsm_InterSystemBiasStructType    z_bdsNavicInterSystemBias; /* Inter GNSS time bias structure */

  pdsm_LocRcvrClockFrequencyInfoStructType z_clockFreq;   /* Freq */

  pdsm_gnss_clockMeasurementStructType           z_GnssMeas;

  pdsm_LocGnssSignalEnumType         e_GnssSignalType;

  pdsm_meas_rf_group_delay_s_type      z_GloRFGroupDelay;       /* Glo RF Group delay, Always valid.
                                                                   This is reported even when we have no SV for GLO constellation
                                                                   Should be outside z_GnssMeas for this reason*/
  double                               d_CumulativeXoPhaseCorr; /* Cumulative XO Phase Correction in nsec since last report.*/
  double                               d_XoFreqCorr;            /* XO Frequency Correction at current report*/
  pdsm_gnss_jammerIndicatorStructType  z_JammerIndicator;

  /*Intra-system time bias can use the same struct as Inter-System*/
  pdsm_InterSystemBiasStructType             z_GpsL1L2cTimeBias;
  pdsm_InterSystemBiasStructType             z_GpsL1L5TimeBias;
  pdsm_InterSystemBiasStructType             z_GloG1G2TimeBias;
  pdsm_InterSystemBiasStructType             z_BdsB1iB2aTimeBias;
  pdsm_InterSystemBiasStructType             z_BdsB1iB1cTimeBias;
  pdsm_InterSystemBiasStructType             z_GalE1E5aTimeBias;
  pdsm_InterSystemBiasStructType             z_GalE1E5bTimeBias;
  pdsm_gnss_measurement_code_EnumType        e_GnssMeasCodeType;
  uint8                                      u_NhzMeasurement; /*TRUE for NHz Measurement*/
} pdsm_pd_ext_gnss_meas_s_type;

/* C_SV_POLY defined in mgp_pe_common.h */
#define PDSM_SV_POLY_FLAGS_INIT         (0)    /* = C_SV_POLY_INIT */
#define PDSM_SV_POLY_FLAGS_COEFF        (1<<0) /* = C_SV_POLY_COEFF */
#define PDSM_SV_POLY_FLAGS_IONO         (1<<1) /* = C_SV_POLY_IONO */
#define PDSM_SV_POLY_FLAGS_TROPO        (1<<2) /* = C_SV_POLY_TROPO */
#define PDSM_SV_POLY_FLAGS_ELEV         (1<<3) /* = C_SV_POLY_ELEV */
#define PDSM_SV_POLY_FLAGS_SRC_ALM_CORR (1<<4) /* = C_SV_POLY_SRC_ALM_CORR */
#define PDSM_SV_POLY_FLAGS_SBAS_IONO    (1<<5) /* = C_SV_POLY_SBAS_IONO */
#define PDSM_SV_POLY_FLAGS_GLO_STR4     (1<<6) /* = C_SV_POLY_GLO_STR4 */
#define PDSM_SV_POLY_FLAGS_FNAV         (1<<9) /* = C_SV_POLY_FROM_2ND_EPHE  */

/* Validity bitmask for TGD and ISC */
#define PDSM_GPS_ISC_L1CA_VALID    (0x0001)
#define PDSM_GPS_ISC_L2C_VALID     (0x0002)
#define PDSM_GPS_ISC_L5I5_VALID    (0x0004)
#define PDSM_GPS_ISC_L5Q5_VALID    (0x0008)
#define PDSM_GPS_TGD_VALID         (0x0010)
#define PDSM_GLO_TGD_G1G2_VALID    (0x0020)
#define PDSM_BDS_TGD_B1_VALID      (0x0040)
#define PDSM_BDS_TGD_B2_VALID      (0x0080)
#define PDSM_BDS_TGD_B2A_VALID     (0x0100)
#define PDSM_BDS_ISC_B2A_VALID     (0x0200)
#define PDSM_GAL_BGD_E1E5A_VALID   (0x0400)
#define PDSM_GAL_BGD_E1E5B_VALID   (0x0800)
#define PDSM_NAVIC_TGD_L5_VALID    (0x1000)
...
typedef struct
{
  ...
  uint64   t_SvPolyValidityMask;
  float f_gpsIscL1ca;        /* Inter Signal Correction - GPS L1C/A */
  float f_gpsIscL2c;         /* Inter Signal Correction - GPS L2C */
  float f_gpsIscL5I5;        /* Inter Signal Correction - GPS L5I5 */
  float f_gpsIscL5Q5;        /* Inter Signal Correction - GPS L5Q5 */
  float f_gpsTgd;            /* Time of Group Delay - GPS */
  float f_gloTgdG1G2;        /* Time of Group Delay - GLONASS G1-G2 */
  float f_bdsTgdB1;          /* Time of Group Delay - BDS B1 */
  float f_bdsTgdB2;          /* Time of Group Delay - BDS B2 */
  float f_bdsTgdB2a;         /* Time of Group Delay - BDS B2A */
  float f_bdsIscB2a;         /* Inter Signal Correction - BDS B2A */
  float f_galBgdE1E5a;       /* Broadcast Group Delay - GALILEO E1-E5a */
  float f_galBgdE1E5b;       /* Broadcast Group Delay - GALILEO E1-E5b */
  float f_navicTgdL5;        /* Time of Group Delay - NAVIC L5 */
  uint32 q_enhancedIOD;      /* Enhanced Reference time / crc/  file id*/
  uint16 w_MbHealth;         /* Multiband health information */
} pdsm_pd_ext_SvPoly_s_type;

/* Wifi on demand request */
typedef struct {
  boolean v_emergencyMode; /* Is emergency mode (priority processing required) */
} pdsm_pd_wifi_info_on_demand_injection_req_s_type;
...
typedef struct {
  ...
  pdsm_pd_wifi_info_on_demand_injection_req_s_type z_wifiOnDemandReq; /* wifi on demand request */
  locEngEventInjectSrnApDataReqIndMsgT_v01 z_srnScanReq; /* SRN (BTLE) scan request */
  locEngInjectSrnApDataIndMsgT_v01 srn_inj_ack; /* Ack for SRN RSSI scan injection */
  pdsm_pd_session_operation_e_type position_mode_type;  /* used to populate the position mode in BEGIN and DONE events */
...
typedef enum
{
  ...
  PDSM_PD_EXT_STATUS_EMERG_SESS_1X_CP,
  PDSM_PD_EXT_STATUS_EMERG_SESS_1X_UP,
  PDSM_PD_EXT_STATUS_EMERG_SESS_NR5G_CP,
  PDSM_PD_EXT_STATUS_EMERG_SESS_NR5G_UP,
  PDSM_PD_EXT_STATUS_EMERG_SESS_DONE
...
/*  Maximum number of constellations for the LPPM purpose.  */

#define PDSM_LPPM_MAX_NUM_CONSTELLATIONS 4

typedef enum
{
  PDSM_LPPM_STATEENGAGEDNUMT_MIN_ENUM_VAL = -2147483647, /* To force a 32 bit signed enum.  Do not change or use*/
  PDSM_LPPM_STATE_ENGAGED_LOW_UNC = 0,                   /* LPPM engaged with low uncertainty. */
  PDSM_LPPM_STATE_ENGAGED_HIGH_UNC = 1,                  /*  LPPM engaged with high uncertainty.*/
  PDSM_LPPM_STATE_DISENGAGED = 2,                        /* LPPM Disengaged */
  PDSM_LPPM_STATE_INITIALIZE = 3,                        /* Enabled, and engaged in initialization mode due to no coarse time */
  PDSM_LPPM_STATE_WAITING_CLOCK = 4,                     /* Enabled, but waiting for clock */
  PDSM_LPPM_STATE_SUSPEND = 5,                           /* Enabled, but suspended due to NI call */
  PDSM_LPPM_STATE_DISABLED = 6,                          /* LPPM Disabled */
  PDSM_LPPM_STATE_UNKNOWN = 7,                           /* Unknown */
  PDSM_LPPM_STATEENGAGEDNUMT_MAX_ENUM_VAL = 2147483647,  /* To force a 32 bit signed enum.  Do not change or use*/
}pdsm_lppm_state_e_type;


typedef enum
{
  PDSM_LPPM_SERVICENUMT_MIN_ENUM_VAL = -2147483647, /* To force a 32 bit signed enum.  Do not change or use*/
  PDSM_LPPM_NOT_IN_SERVICE = 0,                    /* Not in service.*/
  PDSM_LPPM_IN_SERVICE = 1,                        /* In service.*/
  PDSM_LPPM_SERVICENUMT_MAX_ENUM_VAL = 2147483647,  /* To force a 32 bit signed enum.  Do not change or use*/
} pdsm_lppm_service_ind_e_type;

typedef enum
{
  PDSM_LPPM_MOTIONSTATENUMT_MIN_ENUM_VAL = -2147483647, /* To force a 32 bit signed enum.  Do not change or use*/
  PDSM_LPPM_MOTION_STATE_UNKNOWN = 0,                   /* Device state is not known.  */
  PDSM_LPPM_MOTION_STATE_STATIONARY = 1,                /* Device state is Stationary. */
  PDSM_LPPM_MOTION_STATE_IN_MOTION = 2,                 /* Device state is In Motion.  */
  PDSM_LPPM_MOTIONSTATENUMT_MAX_ENUM_VAL = 2147483647,  /* To force a 32 bit signed enum.  Do not change or use*/
}pdsm_lppm_motion_state_e_type;


typedef enum
{
  PDSM_LPPM_MOTIONMODENUMT_MIN_ENUM_VAL = -2147483647, /* To force a 32 bit signed enum.  Do not change or use*/
  PDSM_LPPM_MOTION_MODE_MIN = 0,                        /* Device movement is not known. */
  PDSM_LPPM_MOTION_MODE_UNKNOWN = 0,                    /* Device movement is not known. */
  PDSM_LPPM_MOTION_MODE_PEDESTRIAN,                    /* Device movement is in Pedestrian mode; nothing else is known about the movement. */
  PDSM_LPPM_MOTION_MODE_PEDESTRIAN_WALKING,            /* Device movement is in pedestrian Walking mode. */
  PDSM_LPPM_MOTION_MODE_PEDESTRIAN_RUNNING,            /* Device movement is in pedestrian Running mode. */
  PDSM_LPPM_MOTION_MODE_VEHICLE,                       /* Device movement is in Vehicular mode; nothing else is known about the movement.  */
  PDSM_LPPM_MOTIONMODENUMT_MAX_ENUM_VAL = 2147483647,  /* To force a 32 bit signed enum.  Do not change or use*/
} pdsm_lppm_motion_mode_e_type;


typedef struct
{

  pdsm_constellation_e_type constellationType; /* Constellation type for the LPPM purpose. */

  uint32 requiredSvOrbitalModelBitMask;
  ...
  */
  ...
  uint32 availableSvOrbitalModelBitMask;
  ...
  */
}pdsm_lppm_constellation_data_s_type;

/* Indication Message; LPPM status structure */
typedef struct {

  /*  LPPM State */
  pdsm_lppm_state_e_type lppmState;

  /* Optional */
  /* In Service indicator */
  uint8 inServiceIndicator_valid;  /* set to true if inServiceIndicator is being passed */
  pdsm_lppm_service_ind_e_type inServiceIndicator;

  /* Optional */
  /*   Circular Horizontal Position Uncertainty */
  uint8 horUncCircular_valid;  /* set to true if horUncCircular is being passed */
  float horUncCircular; /*   Horizontal position uncertainty (circular).
                            - Units: Meters */

  /* Optional */
  /*  Time Uncertainty */
  uint8 timeUnc_valid;  /* set to true if timeUnc is being passed */
  float timeUnc; /*   Time uncertainty. 
                      - Units: Milliseconds */

  /* Optional */
  /*  Motion State */
  uint8 motionState_valid;  /* set to true if motionState is being passed */
  pdsm_lppm_motion_state_e_type motionState;  /* Motion state. */

  /* Optional */
  /*  Motion Mode */
  uint8 motionMode_valid;  /* set to true if motionMode is being passed */
  pdsm_lppm_motion_mode_e_type motionMode; /*   Motion mode. */

  /* Optional */
  /*  Constellation Data For LPPM */
  uint8 constellationData_valid;  /* set to true if constellationData is being passed */
  uint8 constellationData_len;   /* set to # of elements in constellationData */
  pdsm_lppm_constellation_data_s_type constellationData[PDSM_LPPM_MAX_NUM_CONSTELLATIONS];
} pdsm_pd_ext_status_oem_lppm_status_type;
...
#define PDSM_PD_MEAS_REPORT_UNAVAILABLE  ( 0x00000001 )
#define PDSM_PD_POS_REPORT_UNAVAILABLE  ( 0x00000002 )

typedef struct
{
  /* GNSS Engine Monitor Status */
  uint64 t_engine_monitor_status;
}pdsm_pd_ext_engine_monitor_report_s_type;

/* Structure definitions for reporting SV INfO */

typedef enum
{
  PDSM_SV_POS_SRC_UNKNOWN = 0,
  PDSM_SV_POS_SRC_EPH_MSB,
  PDSM_SV_POS_SRC_EPH_OTA,
  PDSM_SV_POS_SRC_EPH_XTRA,
  PDSM_SV_POS_SRC_PRED_XTRA,
  PDSM_SV_POS_SRC_PRED_SELF,
}pdsm_SvPosSrcEnumTypeVal;
typedef uint8 pdsm_SvPosSrcEnumType;

typedef enum
{
  PDSM_SV_HEALTH_SRC_UNKNOWN = 0,
  PDSM_SV_HEALTH_SRC_OTA,
  PDSM_SV_HEALTH_SRC_OTA_OTHER_SV,
  PDSM_SV_HEALTH_SRC_MSB,
  PDSM_SV_HEALTH_SRC_XTRA,
}pdsm_SvHealthSrcEnumTypeVal;
typedef uint8  pdsm_SvHealthSrcEnumType;

/*Anchor point, containing the SV position in meters in WGS84 and the SV clock bias in nanoseconds 
  at the offset (-15, -5, 5 and 15 s) from the time of applicability. A positive value of SV clock 
  bias indicates that the clock is ahead of its GNSS sytem time.*/
typedef struct
{
  double d_X;          /*Unit: meter*/
  double d_Y;          /*Unit: meter*/
  double d_Z;          /*Unit: meter*/
  double d_ClkBias;    /*Unit: nanosec*/
}pdsm_SvAnchorPointType;

typedef enum
{
  PDSM_SV_HEALTH_UNKNOWN = 0, /*SV health is unknown*/
  PDSM_SV_HEALTH_GOOD,        /*SV health is good */
  PDSM_SV_HEALTH_DONT_USE,    /*Do not use the SV: bad*/
}pdsm_SvHealthEnumTypeVal;
typedef uint8 pdsm_SvHealthEnumType; /*Force enum to uint8*/

typedef struct
{
  uint16 w_SvId;
  double d_Toa;          /*Time of applicability in full GNSS seconds*/
  uint8  u_TimeInterval; /*Time interval in seconds for SV positions to be calculated 
                           between 2 neighboring anchor points for SV positions.
                           Default value is 10 seconds*/
  pdsm_SvPosSrcEnumType  e_PosSrc;
  pdsm_SvAnchorPointType z_AnchorPoint[4];
  float                  f_UraOrbit;       /*User range accuracy due to satellite position error
                          Unit: meters*/
  float                  f_UraSvClock;     /*User range accuracy due to satellite clock error
                          Unit: nanosec*/
  pdsm_SvHealthEnumType e_EphHealthOta;       /*Eph health from OTA decoding*/
  pdsm_SvHealthEnumType e_EphHealthMsb;       /*Eph health from network*/
  pdsm_SvHealthEnumType e_EphHealthXtra;      /*Eph health from assisted file*/
  pdsm_SvHealthEnumType e_AlmHealthOta;       /*Alm health from OTA decoding in the SV itself*/
  pdsm_SvHealthEnumType e_AlmHealthOtaOtherSv; /*Alm health from OTA decoding from other SV*/
  pdsm_SvHealthEnumType e_AlmHealthXtra;      /*Alm health from assisted file*/
  pdsm_SvHealthEnumType e_Integrity;          /*SV integrity sourced from an external reference network*/

  double d_IonoCorr; /*How the SV time is affected by ionospheric delays. Unit: nanosec*/
  double d_TropCorr; /*How the SV time is affected by tropospheric delays. Unit: nanosec*/
  double d_Tgd;      /*The group delay induced by the transmitting satellite. Unit: nanosec*/
}pdsm_SvInfoStructType;

/* Unified SvInfo structure. ALE SHALL always report SvInfo for all constellations to SM component
via a common API with this payload */
typedef struct
{
  uint8                               u_SeqNum;
  uint8                               u_MaxSeqNum;
  uint8                               u_NumOfSvs; /* Num Sv's used to index into pz_SvInfo */
  pdsm_constellation_e_type           e_GnssType;
  pdsm_SvInfoStructType               *pz_SvInfo; /*MAX_N_ACTIVE_CHAN_FOR_ONE_GNSS*/
  uint32                              q_size;     /*side of p_Data in bytes*/
  uint8                               p_Data[1];  /*Pointer to block of useful data*/
}pdsm_GnssSvInfoStructType;
/* type def SV Info structure to pdsm_pd_ext* type name*/
typedef pdsm_GnssSvInfoStructType pdsm_pd_ext_gnss_sv_info_s_type;

/* GNSS Time Conversion Parameters */
/*  Mask of valid time conversion parameters */
typedef uint8 pdsm_TimeConvValidityMaskType;
#define PDSM_TIME_CONV_PARAM_LS_CHANGE_KNOWLEDGE ((pdsm_TimeConvValidityMaskType)0x01)
/* Leap Second Change Knowledge is valid */

#define PDSM_TIME_CONV_PARAM_GPS_TO_UTC ((pdsm_TimeConvValidityMaskType)0x02)
/* GPS to UTC time conversion parameters are valid */

#define PDSM_TIME_CONV_PARAM_GLO_TO_UTC_GPS ((pdsm_TimeConvValidityMaskType)0x04)
/* GLONASS to UTC and GPS time conversion parameters are valid */

#define PDSM_TIME_CONV_PARAM_GAL_TO_UTC ((pdsm_TimeConvValidityMaskType)0x08)
/* Galileo to UTC time conversion parameters are valid */

#define PDSM_TIME_CONV_PARAM_GAL_TO_GPS ((pdsm_TimeConvValidityMaskType)0x10)
/* Galileo to GPS time conversion parameters are valid */

#define PDSM_TIME_CONV_PARAM_BDS_TO_UTC ((pdsm_TimeConvValidityMaskType)0x20)
/* BDS to UTC time conversion parameters are valid */

#define PDSM_TIME_CONV_PARAM_BDS_TO_GNSS ((pdsm_TimeConvValidityMaskType)0x40)
/* BDS to GNSS time conversion parameters are valid */

/* Structure definitions for reporting TIME parameters */

/* Leap second change knowledge */
typedef enum
{
  PDSM_LS_CHANGE_KNOWLEDGE_UNKNOWN               = 0, /* Leap second change unknown */
  PDSM_LS_CHANGE_KNOWLEDGE_CHANGING              = 1, /* Leap second changing in future */
  PDSM_LS_CHANGE_KNOWLEDGE_NOT_CHANGING          = 2  /* Leap second not changing in future */
} pdsm_GnssLeapSecChangeKnowledgeTypeVal;
typedef uint8 pdsm_GnssLeapSecChangeKnowledgeType;
...
typedef struct
{
  ...
  uint8    u_WNt;      /* UTC data reference week */
  ...
  int8     b_DeltaTlsf; /* Delta time due to leap seconds after the new LS
                           effective */
} pdsm_GalUnscaledUtcOffsetStructType;
...
typedef struct
{
  ...
  uint8   u_WNlsf;        /* Week number of the new leap second */
  uint8   u_DN;           /* Day number of the week of the new leap second */
} pdsm_BdsUnscaledUtcOffsetStructType;
...
} pdsm_BdsUnscaledTimeOffsetStructType;

/* GPS to UTC time conversion parameters */
typedef struct
{
  pdsm_GpsUtcInfoRptStructType        z_UtcInfo;
  uint64                              x_TimeofLeapSecChange;
} pdsm_GpsToUtcTimeConversionParamType;

/* GAL */
typedef struct
{
  pdsm_GalUnscaledUtcOffsetStructType z_GalUtcInfo;
} pdsm_GaltoUtcTimeConversionParamType;

typedef struct
{
  pdsm_BdsUnscaledUtcOffsetStructType z_BdsUtcInfo;
} pdsm_BdstoUtcTimeConversionParamType;

typedef struct
{
  uint8                                 u_ValidMask;                /* Validity Mask of the following fields */
  pdsm_GnssLeapSecChangeKnowledgeType   e_LeapSecChangeKnowledge;   /* Leap Second Change Knowledge */
  pdsm_GpsToUtcTimeConversionParamType  z_GpsToUtcTimeConvParam;    /* GPS to UTC */
  pdsm_GloUnscaledUtcStructType         z_GloToUtcGpsTimeConvParam; /* GLO to UTC and GPS */
  pdsm_GaltoUtcTimeConversionParamType  z_GalToUtcTimeConvParam;    /* GAL to UTC */
  pdsm_GalGstGpsStructType              z_GalToGpsTimeConvParam;    /* GAL to GPS */
  pdsm_BdstoUtcTimeConversionParamType  z_BdsToUtcTimeConvParam;    /* BDS to UTC */
  pdsm_BdsUnscaledTimeOffsetStructType  z_BdsToGnssTimeConvParam;   /* BDS to GPS/GLO/GAL */
} pdsm_GnssTimeConversionParamType;

typedef pdsm_GnssTimeConversionParamType pdsm_pd_ext_time_conv_info_s_type;

/* GNSS Clock Model Parameters */
typedef struct
{
  uint32 q_RefFcount;                /* Receiver's ms counter value. */
  float  f_ClkPhaseProcessNoise;     /* Clock Phase Noise */
  float  f_ClkFreqProcessNoise;      /* Clock Frequency Noise */
  float  f_ClkPhaseProcessNoiseVar;  /* Clock Phase Noise Variance */
  float  f_ClkFreqProcessNoiseVar;   /* Clock Frequency Noise Variance */
}pdsm_GnssClkModelParamType;

typedef pdsm_GnssClkModelParamType pdsm_ext_clk_model_info_s_type;

/*ME Metrics constellations*/

#define PDSM_GNSS_SIG_TYPE_GPS_L1CA  C_MGP_GNSS_SIG_TYPE_GPS_L1CA //( 0x0000 )
#define PDSM_GNSS_SIG_TYPE_GPS_L2C   C_MGP_GNSS_SIG_TYPE_GPS_L2C  //( 0x0001 )
#define PDSM_GNSS_SIG_TYPE_GPS_L5    C_MGP_GNSS_SIG_TYPE_GPS_L5   //( 0x0002 )
#define PDSM_GNSS_SIG_TYPE_GLO_G1    C_MGP_GNSS_SIG_TYPE_GLO_G1   //( 0x0003 )
#define PDSM_GNSS_SIG_TYPE_GLO_G2    C_MGP_GNSS_SIG_TYPE_GLO_G2   //( 0x0004 )
#define PDSM_GNSS_SIG_TYPE_BDS_B1    C_MGP_GNSS_SIG_TYPE_BDS_B1I  //( 0x0005 )
#define PDSM_GNSS_SIG_TYPE_BDS_B2    C_MGP_GNSS_SIG_TYPE_BDS_B2A  //( 0x0006 )
#define PDSM_GNSS_SIG_TYPE_GAL_E1    C_MGP_GNSS_SIG_TYPE_GAL_E1   //( 0x0007 )
#define PDSM_GNSS_SIG_TYPE_GAL_E5A   C_MGP_GNSS_SIG_TYPE_GAL_E5A  //( 0x0008 )
#define PDSM_GNSS_SIG_TYPE_LAST_VAL  ( PDSM_GNSS_SIG_TYPE_GAL_E5A )
...
typedef struct
{
  ...
  pdsm_PerGnssRfStructType  z_PerGnssRfStats[PDSM_GNSS_SIG_TYPE_LAST_VAL+1];
} pdsm_MERfStructType;
...
typedef struct
{
  ...
  pdsm_MERfStructType  z_MERfStats;
} pdsm_ext_report_ME_metrics_type;

/**  Maximum string length for the requestor string ID  */
#define PDSM_PD_MAX_REQUESTOR_ID_LEN 20

/**  Maximum string length for the client string ID  */
#define PDSM_PD_MAX_CLIENT_ID_LEN 4

typedef enum
{
 PDSM_PD_LOC_PRTL_CTRL_PLANE,
 PDSM_PD_LOC_PRTL_SUPL,
 PDSM_PD_LOC_PRTL_IMS,
 PDSM_PD_LOC_PRTL_SIM,
 PDSM_PD_LOC_PRTL_MDT,
 PDSM_PD_LOC_PRTL_TLOC,
 PDSM_PD_LOC_PRTL_OTHER 
}pdsm_pd_loc_req_protocol_stack_type;

typedef enum
{
  PDSM_PD_LOC_REQUESTOR_CARRIER,
  PDSM_PD_LOC_REQUESTOR_OEM,
  PDSM_PD_LOC_REQUESTOR_MODEM_CHIPSET_VENDOR,
  PDSM_PD_LOC_REQUESTOR_GNSS_CHIPSET_VENDOR,
  PDSM_PD_LOC_REQUESTOR_OTHER_CHIPSET_VENDOR,
  PDSM_PD_LOC_REQUESTOR_AUTO_CLIENT,
  PDSM_PD_LOC_REQUESTOR_OTHER
}pdsm_pd_loc_requestor_type;

typedef enum
{
  PDSM_PD_LOC_RESPONSE_REJECTED,
  PDSM_PD_LOC_RESPONSE_ACCEPTED_NO_LOC_PROVIDED,
  PDSM_PD_LOC_RESPONSE_ACCEPTED_LOC_PROVIDED
}pdsm_pd_loc_response_type;

typedef enum
{
  PDSM_PD_LOC_INFO_GNSS_POSITION,
  PDSM_PD_LOC_INFO_GNSS_MEAS,
  PDSM_PD_LOC_INFO_OTDOA_MEAS,
  PDSM_PD_LOC_INFO_DBH_DATA,
  PDSM_PD_LOC_INFO_SRN,
  PDSM_PD_LOC_INFO_ECID,
  PDSM_PD_LOC_INFO_WLAN,
  PDSM_PD_LOC_INFO_BARO,
 PDSM_PD_LOC_INFO_PPM
}pdsm_pd_loc_info_type;

#define PDSM_PD_LOC_NOTIFY_TECH_GNSS_POSITION        PDSM_SET_U32BITMASK_BIT(0)
#define PDSM_PD_LOC_NOTIFY_TECH_GNSS_MEAS            PDSM_SET_U32BITMASK_BIT(1)
#define PDSM_PD_LOC_NOTIFY_TECH_OTDOA_MEAS           PDSM_SET_U32BITMASK_BIT(2)
#define PDSM_PD_LOC_NOTIFY_TECH_DBH_DATA             PDSM_SET_U32BITMASK_BIT(3)
#define PDSM_PD_LOC_NOTIFY_TECH_SRN                  PDSM_SET_U32BITMASK_BIT(4)
#define PDSM_PD_LOC_NOTIFY_TECH_ECID                 PDSM_SET_U32BITMASK_BIT(5)
#define PDSM_PD_LOC_NOTIFY_TECH_WLAN                 PDSM_SET_U32BITMASK_BIT(6)
#define PDSM_PD_LOC_NOTIFY_TECH_BARO                 PDSM_SET_U32BITMASK_BIT(7)
#define PDSM_PD_LOC_NOTIFY_TECH_PPM                  PDSM_SET_U32BITMASK_BIT(8)


/* Location Notification */
typedef struct {
  pdsm_pd_loc_req_protocol_stack_type e_protocol_stack;
  pdsm_pd_loc_requestor_type e_location_requestor;
  char loc_requestor_id[PDSM_PD_MAX_REQUESTOR_ID_LEN + 1];
  pdsm_pd_loc_response_type e_location_response;
  pdsm_pd_loc_info_type e_location_info;
  boolean v_e911_state;
  boolean v_cached_location;
  boolean v_client_id_valid;
  char client_id[PDSM_PD_MAX_CLIENT_ID_LEN + 1];
} pdsm_pd_location_notification_info_s_type;

/* E911 state notification */
typedef struct {
  boolean v_e911_state;  /* TRUE == E911 state; FALSE == non-E911 state */
} pdsm_pd_e911_state_notification_s_type;

typedef union
{
  pdsm_pd_ext_pos_s_type           ext_pos_report_type;
  pdsm_pd_ext_nmea_s_type          ext_nmea_pos_report_type;
  pdsm_pd_ext_var_len_debug_nmea_s_type     ext_nmea_var_len_pos_report_type;
  ...
  pdsm_pd_ext_gnss_meas_s_type       ext_gnss_meas_report;
  pdsm_pd_ext_engine_monitor_report_s_type ext_engine_monitor_report;
  pdsm_pd_ext_status_otdoa_sess_info_type ext_otdoa_session_info_type;
  pdsm_pd_ext_status_oem_lppm_status_type ext_oem_lppm_status_type;
  pdsm_pd_ext_status_network_sess_req_type ext_session_req_type;
  pdsm_pd_ext_gnss_sv_info_s_type    ext_sv_info_report;
  pdsm_pd_ext_time_conv_info_s_type  ext_time_conv_param;
  pdsm_ext_clk_model_info_s_type     ext_clk_model_param;
  pdsm_ext_report_ME_metrics_type    ext_report_ME_metrics;
  pdsm_pd_location_notification_info_s_type z_ext_location_notification_info; /* Contains details on what location info was shipped out of modem */
  pdsm_pd_e911_state_notification_s_type z_ext_e911_state_notification; /* Indicates E911 state to LocMW */
...
typedef enum pdsm_xtra_cmd_e {
  ...
  PDSM_XTRA_CMD_QUERY_XTRA_CFG_INFO,      /* gpsOneXTRA client is querying xtra config information */
...
#define PDSM_XTRA_EVENT_CFG_REQ            0x08
#define PDSM_XTRA_EVENT_CFG_STATUS         0x10

#define PDSM_XTRA_EVENT_MASK   (   (PDSM_XTRA_EVENT_DOWNLOAD_REQ) \
                                 | (PDSM_XTRA_EVENT_STATUS) \
                                 | (PDSM_XTRA_EVENT_TIME_REQ) \
                                 | (PDSM_XTRA_EVENT_CFG_REQ) \
                                 | (PDSM_XTRA_EVENT_CFG_STATUS) \
                                )
...
typedef struct pdsm_xtra_version_info_s
{
  uint8_t xMajorVersion;      // XTRA client major version
  uint8_t xMinorVersion;      // XTRA client minor version
}pdsm_xtra_version_info_s_type;
...
#define XTRA_CFG_INFO_MASK_NAVIC_EPH_ASSIST  (0x00000040)   // support for Navic assist at modem
#define XTRA_CFG_INFO_MASK_MODEM_VERSION  (0x00000080)  //modem xtra version info
#define XTRA_CFG_INFO_MASK_FILE_VERSION      (0x00000100)  //xtra file version
#define XTRA_CFG_INFO_MASK_USABLE_CONSTELLATIONS_CONFIG  (0x00000200)  //Gnss Band1 Config
#define XTRA_CFG_INFO_MASK_FORCED_ENABLED_CONSTELLATIONS_CONFIG  (0x00000400) //Gnss forced enabled constellations
#define XTRA_CFG_INFO_MASK_MULTIBAND_ENABLED_CONFIG (0x00000800)  //Gnss multiband enabled
#define XTRA_CFG_INFO_MASK_MCC_PRIMARY_SIM_SLOT     (0x00001000)  //SIM Slot1 MCC
#define XTRA_CFG_INFO_MASK_MCC_SECONDARY_SIM_SLOT   (0x00002000) //SIM Slot 2 MCC
#define XTRA_CFG_INFO_MASK_MCC_TERTIARY_SIM_SLOT    (0x00004000)  //SIM Slot3 MCC
#define XTRA_CFG_INFO_MASK_XTRA_DISABLED            (0x00008000)   // XTRA disablement
#define XTRA_CFG_INFO_MASK_XTRA_REMAINING_VALID_DURATION_MIN (0x00010000) //Remaining Valid Xtra age in Minutes
#define XTRA_CFG_INFO_MASK_PRECISE_XTRA_ENABLEMENT_STATUS (0x00020000) //Precise Xtra enablement status info

#define XTRA_GNSS_CONSTELLATION_NAVIC    (1 << 0) /*XC4.0 Bit 0 NAVIC Used for setting Usable/forced Enable bit Mask*/
#define XTRA_GNSS_CONSTELLATION_QZSS     (1 << 1) /*XC4.0 Bit 1 QZSS  Used for setting Usable/forced Enable bit Mask*/
#define XTRA_GNSS_CONSTELLATION_GAL      (1 << 2) /*XC4.0 Bit 2 GAL   Used for setting Usable/forced Enable bit Mask*/
#define XTRA_GNSS_CONSTELLATION_BDS3     (1 << 3) /*XC4.0 Bit 3 BDS3  Used for setting Usable/forced Enable bit Mask*/
#define XTRA_GNSS_CONSTELLATION_GLO      (1 << 4) /*XC4.0 Bit 4 GLO   Used for setting Usable/forced Enable bit Mask*/
#define XTRA_GNSS_CONSTELLATION_GPS      (1 << 5) /*XC4.0 Bit 5 GPS   Used for setting Usable/forced Enable bit Mask*/

#define SET_XTRA_GNSS_CONSTELLATIONS(IN, OUT)          \
        if ((IN) & XTRA_GNSS_CONSTELLATION_NAVIC) {    \
            (OUT) |= eQMI_SYSTEM_NAVIC_V02;            \
        }                                              \
                                                       \
        if ((IN) & XTRA_GNSS_CONSTELLATION_QZSS) {     \
            (OUT) |=  eQMI_SYSTEM_QZSS_V02;            \
        }                                              \
                                                       \
        if ((IN) & XTRA_GNSS_CONSTELLATION_GAL) {      \
            (OUT) |=  eQMI_SYSTEM_GAL_V02;             \
        }                                              \
                                                       \
        if ((IN) & XTRA_GNSS_CONSTELLATION_BDS3) {     \
            (OUT) |=  eQMI_SYSTEM_BDS_V02;             \
        }                                              \
                                                       \
        if ((IN) & XTRA_GNSS_CONSTELLATION_GLO) {      \
            (OUT) |= eQMI_SYSTEM_GLO_V02;              \
        }                                              \
                                                       \
        if ((IN) & XTRA_GNSS_CONSTELLATION_GPS) {      \
            (OUT) |= eQMI_SYSTEM_GPS_V02;              \
        }                                              \

typedef struct pdsm_xtra_config_ind_info_s
{
  pdsm_xtra_status_e_type  status;
  uint32   report_mask;

  uint32   prefered_valid_age_hrs;
  uint32   abs_age_hrs;
  uint64   rel_age_utc;
...
}pdsm_xtra_config_info_s_type;

typedef struct pdsm_xtra_config_status_ind_s
{
  pdsm_xtra_status_e_type  status;
  uint32   report_mask;

  uint32   gnss_usable_constellations; //gnss receiver band1 config
  uint32   gnss_forced_enable_constellations; //gnss receiver forced enable constellations
  uint32   xtra_valid_duration_mins; //xtra remaining valid duration in mins
  uint16   mcc_primary_sim_slot;  //MCC for SLOT 1
  uint16   mcc_secondary_sim_slot; //MCC for SLOT 2
  uint16   mcc_tertiary_sim_slot;  //MCC for SLOT 3
  uint8    gnss_multiband_enabled; //gnss multiband config enabled
  uint8    preciseXtraEnablementStatus; //precise Xtra feature support 
  pdsm_xtra_version_info_s_type xtra_file_version; //Xtra file version
} pdsm_xtra_config_status_s_type;
...
typedef union pdsm_xtra_req_u
{
  ...
  pdsm_xtra_config_info_s_type    xtra_cfg_info;
  pdsm_xtra_config_status_s_type  xtra_cfg_status;
...
typedef struct pdsm_xtra_config_req_info
{
  uint32     config_mask;
} pdsm_xtra_config_req_info_type;
...
#define PDSM_MAX_APS_IN_WIFI_SCAN 150 // Do not change this number since it affects RPC and log packet sizes
...
/* The type of network on which the ap is seen */
typedef enum
{
  PDSM_AP_NET_TYPE_ADHOC,
  PDSM_AP_NET_TYPE_INFRA,
  PDSM_AP_NET_TYPE_MAX = 0xFFFF
} pdsm_ap_network_e_type;


/* The category of the AP which is seen */
typedef enum
{
  PDSM_AP_CAT_STATIC,
  PDSM_AP_CAT_MOBILE,
  PDSM_AP_CAT_HOTSPOT,
  PDSM_AP_CAT_UNKNOWN,
  PDSM_AP_CAT_MAX = 0xFFFF
} pdsm_ap_cat_e_type;


#define PDSM_AP_INFO_SSID_AVAIL_MASK                   0x1
#define PDSM_AP_INFO_COUNTRY_CODE_AVAIL_MASK           0x2
#define PDSM_AP_INFO_PHY_MODE_AVAIL_MASK               0x4
#define PDSM_AP_INFO_NET_TYPE_AVAIL_MASK               0x8
#define PDSM_AP_INFO_CATEGORY_AVAIL_MASK               0x10


typedef struct
{
  /* Two character country code as described in document ISO/IEC 3166-1 */
  uint8_t u_country[2];
  /* OPerating environment */
  uint8_t u_oper_env;
}pdsm_country_code_type;


typedef enum
{
  PDSM_AP_PHY_MODE_11A,              /* 11a Mode */
  PDSM_AP_PHY_MODE_11G,              /* 11b/g Mode */
  PDSM_AP_PHY_MODE_11B,              /* 11b Mode */
  PDSM_AP_PHY_MODE_11GONLY,          /* 11g only Mode */
  PDSM_AP_PHY_MODE_11NA_HT20,        /* 11na HT20 mode */
  PDSM_AP_PHY_MODE_11NG_HT20,        /* 11ng HT20 mode */
  PDSM_AP_PHY_MODE_11NA_HT40,        /* 11na HT40 mode */
  PDSM_AP_PHY_MODE_11NG_HT40,        /* 11ng HT40 mode */
  PDSM_AP_PHY_MODE_11AC_VHT20,       /* 5G 11ac VHT20 mode */
  PDSM_AP_PHY_MODE_11AC_VHT40,       /* 5G 11ac VHT40 mode */
  PDSM_AP_PHY_MODE_11AC_VHT80,      /* 5G 11ac VHT80 mode */
  PDSM_AP_PHY_MODE_11AC_VHT20_2G,   /* 2G 11ac VHT20 mode */
  PDSM_AP_PHY_MODE_11AC_VHT40_2G,   /* 2G 11ac VHT40 mode */
  PDSM_AP_PHY_MODE_11AC_VHT80_2G,   /* 2G 11ac VHT80 mode */
  PDSM_AP_PHY_MODE_11AC_VHT80_80,
  PDSM_AP_PHY_MODE_11AC_VHT160,
  PDSM_AP_PHY_MODE_11AX_HE20,
  PDSM_AP_PHY_MODE_11AX_HE40,
  PDSM_AP_PHY_MODE_11AX_HE80,
  PDSM_AP_PHY_MODE_11AX_HE80_80,
  PDSM_AP_PHY_MODE_11AX_HE160,
  PDSM_AP_PHY_MODE_11AX_HE20_2G,
  PDSM_AP_PHY_MODE_11AX_HE40_2G,
  PDSM_AP_PHY_MODE_11AX_HE80_2G,
  PDSM_AP_PHY_MODE_MAX = 0xFFFF,
}pdsm_ap_phy_mode_e_type;
...
typedef struct {
   ...
   */
  int32           meas_age;
  /* Set to TRUE only if this AP is/was a serving AP when the measurement was collected */
  boolean u_isServing;

  uint32 q_ApInfoValidMask;
  /* Valid only if AP_INFO_COUNTRY_CODE_AVAIL_MASK is set */
  /* Country code for the AP. Refer fusion_csm_country_code_type */
  pdsm_country_code_type z_CountryCode;

  /* Valid only if AP_INFO_PHY_MODE_AVAIL_MASK is set */
  /* Country code for the AP. Refer pdsm_ap_phy_mode_e_type */
  pdsm_ap_phy_mode_e_type e_PhyMode;

  /* Valid only if AP_INFO_NET_TYPE_AVAIL_MASK is set */
  /* Network type for the AP. Refer pdsm_ap_network_e_type  */
  pdsm_ap_network_e_type e_NetType;

  /* Valid only if AP_INFO_CATEGORY_AVAIL_MASK is set */
  /* AP category . Refer pdsm_ap_cat_e_type */
  pdsm_ap_cat_e_type e_ApType;
...
typedef struct {
  ...
  pdsm_wifi_scan_e_type e_wifi_scan_type;
  t_wlan_channel_info_type    z_wlan_channel_info;
  /* When the APs in the wifi scan are sent in a batch this variable indicates the batch number */
  uint8 u_batch_num;
  /* Boolean to indicate whether this is the last batch in the batch processing of APs in the
     wifi scan */
  boolean u_is_last_batch;
} t_wiper_ap_set_struct_type;


/* Extended wifi scan list information */
/* Warning . Do not use this structure for IPC/RPC as the size of the struct is huge
   Do not allocate this structure on a stack .
   Use this structure for internal purposes only inside a task without IPCs */
typedef struct {
  uint8   num_of_aps;
  t_wiper_ap_info_struct_type ap_info[PDSM_MAX_APS_IN_WIFI_SCAN];
  pdsm_wifi_scan_src_e_type e_wifi_scan_src;
  pdsm_wifi_scan_e_type e_wifi_scan_type;
  t_wlan_channel_info_type    z_wlan_channel_info;
} pdsm_wifi_scan_info_ext_s_type;

/* Maximum number of measurmeents per AP in the ranging scan. DO not change this number
   since it would exceed the max IPC memory that can be allocated and the IPC carrying this
   data is not sent.  */
#define PDSM_RANGING_SCAN_PER_AP_MEAS_MAX 5
...
typedef enum
{
  ...
  RANGING_BW_160MHZ,
  RANGING_BW_MAX = 0xFF
}pdsm_RangingScanBwType;



/* Per AP measurements from the ranging scan */
typedef struct
{
  /**
   * Measurement age msec
  ...
  /* Ranging scan bandwidth */
  pdsm_RangingScanBwType e_Bandwidth;

}pdsm_RangingScanApMeasType;

/* Defines available RTT types */
typedef enum
{
  PDSM_RTT1_RANGING = 0,    /* Ranging with no Multipath correction */
  PDSM_RTT2_RANGING,        /* Ranging with Multipath correction */
  PDSM_RTT3_RANGING,        /* Two-sided Ranging as defined in 802.11mc spec */
  PDSM_RTT_BEST_EFFORT_RANGING  /* One of the above, which ever the peer supports */
}pdsm_RttType;
/* Defines available Wifi nodes */
typedef enum
{
  PDSM_WIFI_NODE_TYPE_UNKNOWN = 0,
  PDSM_WIFI_NODE_TYPE_ACCESS_POINT,
  PDSM_WIFI_NODE_TYPE_PEER_DEVICE
}pdsm_WifiNodeType;

/**
 * Information for AP when requesting for ranging scan
 */
typedef struct
{
  /** MacId of the Node*/
  uint8  u_MacAddr[WIPER_MAC_ADDR_LENGTH];
  /** Frequency in Mhz, which the node is transmitting*/
  uint32 q_Freq;
  /** Either PEER_DEVICE or ACCESS_POINT*/
  pdsm_WifiNodeType e_NodeType;
  /* Spoof MacId needed for peer device ranging.
   * Only valid if nodeType is PEER_DEVICE*/
  uint8 u_SpoofMacId[WIPER_MAC_ADDR_LENGTH];

  /** The Type Of RTT to be performed */
  pdsm_RttType e_RttType;

  /* RSSI of the AP */
  int32           rssi;

}pdsm_RangingScanReqApInfoType;


/* For AP Ranging scan list request information */
typedef struct {
  /* Request ID from the client if any*/
  uint32 q_RequestId;
  /* The number of Aps for which the ranging scan is requested */
  uint8   q_NumAps;
  pdsm_RangingScanReqApInfoType ap_info[WIPER_MAX_REPORTED_APS_PER_LOG_MSG];

} pdsm_RangingScanReqType;



/* The AP information for the ranging scan */
typedef struct
{
  /** MacId of the Node*/
  uint8  u_MacAddr[WIPER_MAC_ADDR_LENGTH];

  /* The RTT measurements for the AP */
  pdsm_RangingScanApMeasType z_MeasArr[PDSM_RANGING_SCAN_PER_AP_MEAS_MAX];

  /** The Type Of RTT which was performed on this AP */
  pdsm_RttType e_RttType;

  /* The raw RTT measurement count */
  uint8 u_RawRttMeasCount;

  /* Mean RTT value in ns which is calculated by the client if RTT ranging scan was requested for this AP */
  uint32 q_MeanRtt;
  /* RTT variance value in ns which is calculated by the client if RTT ranging scan was requested for this AP */
  uint32 q_RttVariance;
  /* Number of RTT measurements if RTT ranging scan was requested for this AP */
  uint8 u_RttMeasurementCount;
}pdsm_RangingScanRespApInfoType;


/* The response information for the ranging scan */
typedef struct {
  /* Request ID from the client if any*/
  uint32 q_RequestId;
  /* The number of Aps for which the ranging scan data is available */
  uint8   q_NumAps;
  pdsm_RangingScanRespApInfoType ap_info[WIPER_MAX_REPORTED_APS_PER_LOG_MSG];
}pdsm_RangingScanRespType;
...
typedef enum pdsm_lppm_power_level_enum_type
{
  PDSM_LPPM_POWER_LEVEL_LOW,     /* LPPM power level low  */
  PDSM_LPPM_POWER_LEVEL_MEDIUM,  /* LPPM power level medium  */
  PDSM_LPPM_POWER_LEVEL_HIGH     /* LPPM power level high */
} pdsm_lppm_power_level_enum_type;

typedef enum pdsm_pd_lppm_e_type
{
  PDSM_PD_LPPM_NONE = -1,
  PDSM_PD_LPPM_DISABLE,
  PDSM_PD_LPPM_ENABLE,
  PDSM_PD_LPPM_E_SIZE = 0x10000000
} pdsm_pd_lppm_e_type;

typedef struct pdsm_pd_lppm_control_s_type
{
  pdsm_pd_lppm_e_type e_lppm_control;   /* LPPM Control command to enable or disable LPPM */
  uint32              min_gnss_period;  /* MIN GNSS period if LPPM is enabled. This field is obsolete and not used */
  pdsm_lppm_power_level_enum_type e_lppm_power_level; /* LPPM power level requirements */
}pdsm_pd_lppm_control_s_type;
...
typedef struct {
  ...
  boolean v_reason_service_interaction; /* Ending this session due to service interaction */
...
FUNCTION pdsm_lppm_control

DESCRIPTION
  This API initiates a command to Enable or Disable LPPM mode. 
...
===========================================================================*/
boolean pdsm_lppm_control(
   pdsm_pd_cmd_cb_f_type   cmd_cb_func, /* command result callback function */
   /*~ PARAM pa_motion_data_cb_func POINTER */
   pdsm_client_id_type   client_id,
   void   *client_data_ptr, /* client provided reference data pointer */
   pdsm_pd_lppm_control_s_type *lppm_control_param /* parameters for lppm_control */
   );
...
FUNCTION pdsm_xtra_query_config_info
...
typedef struct
{
  ...
  uint8_t ue_mac_address[6]; /* UE Wifi MacAddress */
  /*Below two pointers are pointers to array containing the info of access points in the scan.
      The order of wifi access points and the number of wifi access points in the two arrays must
      be same*/
  locEngWifiApInfoStructT_v01 *first_wifi_ap_ptr; /*Wif info array pointer*/
  locEngWifiApAdditionalDataStructT_v01 *first_wifi_ap_add_info_ptr; /*Additional Wif info array pointer*/
} pdsm_wifi_ap_scan_info_s_type;

typedef struct
{
  locEngWlanApErrEnumT_v01 e_errorCause; /*Error Cause if any*/
  boolean v_on_demand_scan; /*Whether it is an on demand scan. Default: 0*/
  uint64 t_request_timestamp_utc; /*UTC when scan was requested. 
                                                         If timestamp is not known, set it to 0.
                                                         Default: 0 (unknown)*/
  uint64 t_recieved_timestamp_utc; /*UTC when scan was requested. 
                                                         If timestamp is not known, set it to 0.
                                                         Default: -(unknown)*/
  pdsm_wifi_ap_scan_info_s_type z_wifi_ap_scan_info; /*Wifi scan info*/
...
/* PDSM Gera client IDs for different clients */
typedef enum
{
  PDSM_GERA_CLIENT_ID_FIRST = 0,
  /* Client Id for E911 */
  PDSM_GERA_CLIENT_ID_E911 =  PDSM_GERA_CLIENT_ID_FIRST,
  /* Client Id for AP MP time transfer */
  PDSM_GERA_CLIENT_ID_AP_MP_TTR = 1,
  /* Client Id for DPO */
  PDSM_GERA_CLIENT_ID_DPO = 2,
  /* Client Id for Standalone */
  PDSM_GERA_CLIENT_ID_STANDALONE = 3,
  /* Client Id for MSA */
  PDSM_GERA_CLIENT_ID_MSA = 4,
  /* Client Id for MSB */
  PDSM_GERA_CLIENT_ID_MSB = 5,
  PDSM_GERA_CLIENT_ID_LAST = PDSM_GERA_CLIENT_ID_MSB,
  PDSM_GERA_CLIENT_ID_MAX = 0xFFFFFFF
} pdsm_gera_client_id_e_type;
...
FUNCTION pdsm_gera_cmd_client_register

DESCRIPTION
  This function is to register a client to GERA through a test interface.
...
===========================================================================*/
boolean pdsm_gera_cmd_client_register(pdsm_gera_client_id_e_type e_client_id);

/*===========================================================================

FUNCTION pdsm_gera_cmd_client_deregister

DESCRIPTION
  This function is to de-register a client to GERA through a test interface.
...
===========================================================================*/
boolean pdsm_gera_cmd_client_deregister(pdsm_gera_client_id_e_type e_client_id);


/*===========================================================================

FUNCTION pdsm_gera_client_enable_request

DESCRIPTION
  This function is to request for enable of a GERA client through a test interface. 
...
===========================================================================*/
boolean pdsm_gera_cmd_client_enable_request(pdsm_gera_client_id_e_type e_ind_client_id, uint32 q_dep_client_id_mask);


/*===========================================================================

FUNCTION pdsm_gera_client_disable_request

DESCRIPTION
  This function is to request for disabling a GERA client through a test interface. 
...
===========================================================================*/
boolean pdsm_gera_cmd_client_disable_request(pdsm_gera_client_id_e_type e_client_id);
...
DESCRIPTION
  This function to acknowledge to GERA that a client has disabled.
...
===========================================================================*/
boolean pdsm_gera_cmd_client_disabled(pdsm_gera_client_id_e_type e_client_id);


/*===========================================================================

FUNCTION pdsm_gera_cmd_reset

DESCRIPTION
  This function is to reset the GERA module through a test interface.
...
===========================================================================*/
boolean pdsm_gera_cmd_reset(void);

/*===========================================================================

FUNCTION pdsm_gera_cmd_client_query_state

DESCRIPTION
  This function is to query the state of each of the GERA clients through a test interface.
...
===========================================================================*/
boolean pdsm_gera_cmd_client_query_state(void);


typedef struct
{
  uint32 q_num_srn_aps; /* number of wifi access points */
  /*Below pointer holds the array containing the info of access points in the SRN scan.*/
  locEngSrnBtleApDeviceDataStructT_v01 *srn_ap_ptr; /*Wif info array pointer*/
} pdsm_srn_ap_scan_info_s_type;

typedef struct
{
  locEngSrnApDataTechTypeEnumT_v01 e_srnApTechType;
  ...
  uint64 t_recieved_timestamp_utc; /*UTC when scan was requested.
                                                         If timestamp is not known, set it to 0.
                                                         Default: -(unknown)*/
  pdsm_srn_ap_scan_info_s_type z_srn_ap_scan_info; /*Srn scan info*/
} pdsm_srn_scan_info_list_s_type;


/*===========================================================================

FUNCTION pdsm_pd_inject_srn_scan_info()

DESCRIPTION
  PDSM client calls this function to inject Srn scan results to SM
...
===========================================================================*/
extern boolean pdsm_pd_inject_srn_scan_info(
   pdsm_pd_cmd_cb_f_type    pd_cb_func,        /* Command callback */
   /*~ PARAM pd_cb_func POINTER */
   void                     *client_data_ptr,   /* user data pointer */
   pdsm_client_id_type      client_id,          /* Client id of user */
   pdsm_srn_scan_info_list_s_type  *srn_scan_info_data_list_ptr /* Pointer to srn scan result */
   /*~ PARAM ni_info POINTER */
   );
/*~ FUNCTION pdsm_pd_inject_srn_scan_info */
...
FUNCTION pdsm_get_gps1_lock

DESCRIPTION This is a helper function to read NV_GPS1_LOCK_I(452) value

RETURN VALUE
  NV_GPS1_LOCK_I read value

===========================================================================*/
pdsm_gps_lock_e_type pdsm_get_gps1_lock(void);
/*~ FUNCTION pdsm_get_gps1_lock */
