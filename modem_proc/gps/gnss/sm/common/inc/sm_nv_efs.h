...
Version Control
  ...
  $Header: //components/rel/gnss8.mpss/11.1/gnss/sm/common/inc/sm_nv_efs.h#4 $
...
enum
{
  GNSS_NV_SUPL_DENY_UDP_BIND = 0x0,
  GNSS_NV_SUPL_ALLOW_UDP_BIND = 0x1,
  GNSS_NV_SUPL_ALLOW_UDP_BIND_E911 = 0x2
};
...
#else
  #if( defined(FEATURE_GNSS_PREMIUM_SAP_DEFAULT) || defined(FEATURE_INTERNAL_USE_FFABUILDS_ONLY) )
    #define GNSS_NV_EFS_SM_CONFIG_SAP_PREMIUM_DEFAULT (2)   /* SAP Premium services by default */
  #else
    #define GNSS_NV_EFS_SM_CONFIG_SAP_PREMIUM_DEFAULT (1)   /* SAP Basic services by default */
  #endif
...
#endif
/* SUPL UDP: 0: Disabled, 1: Always Enabled, 2: Enabled only during 911*/
...
#define GNSS_NV_EFS_SM_TM_SUPL_UDP_ENABLE_DEFAULT (GNSS_NV_SUPL_DENY_UDP_BIND)
#else
#define GNSS_NV_EFS_SM_TM_SUPL_UDP_ENABLE_DEFAULT (GNSS_NV_SUPL_ALLOW_UDP_BIND)
...
#define GNSS_NV_EFS_SM_XTRA_TRACK_XC_CONFIG_INFO_HANDSHAKE_DEFAULT  (0)  /*Default config handshake info tracker */
...
#define GNSS_NV_EFS_SM_ALLOW_MO_SUPL_DURING_CONCURRENCY_DEFAULT (0x0)  /* Do not allow MO-SUPL during concurrency while in early RCVR mode */
...
#ifdef FEATURE_GNSS_NON_STANDARD_SUPPORT
#define GNSS_NV_EFS_SM_LBS_CONFIG_DEFAULT       (0)  /* No LBS debug feature by default*/
#endif /* FEATURE_GNSS_NON_STANDARD_SUPPORT */
#define GNSS_NV_EFS_SM_TM_MEAS_QMI_REPORT_DEFAULT 0x03          /*  00  --   All Disabled 
                                                                    01...--..Poly Disabled   MEAS Enabled 
                                                                    10...--..Poly Enabled    Meas Disabled
                                                                    11...--.. both enabled  */

#ifdef FEATURE_GNSS_NON_STANDARD_SUPPORT /* currently this NV is only used for LPP Control Plane */
...
#define GNSS_NV_EFS_SM_CONCURRENT_SESSION_SUPPORT_CONFIG_DEFAULT         (0x01) /*Default Concurrency feature is enabled */
...
#define GNSS_NV_EFS_SM_EMERGENCY_EXTENSION_WINDOW_DEFAULT (0) /* Default to 0seconds. i.e No extending the E911 state */
  #define GNSS_NV_EFS_SM_GNSS_LOCK_CTRL_FOR_OEM_DEFAULT (0) /* Default OEM settings:
                                                               bit0 = CP_NILR_OEM_CTRL         = 0 = ACCEPT_ALWAYS
                                                               bit1 = SUPL_NI_OEM_CTRL         = 0 = ACCEPT_ALWAYS
                                                               bit2 = NFW_OEM_CTRL             = 0 = ACCEPT_ALWAYS
                                                             bit3 = CP_NILR_NOTIFY_OEM_CTRL  = 0 = DON’T_ALLOW
                                                             bit4 = NFW_NOTIFY_OEM_CTRL      = 0 = DON’T_ALLOW */


#ifdef FEATURE_GNSS_NON_STANDARD_SUPPORT
#define GNSS_NV_EFS_SM_TM_MEAS_QMI_REPORT_DISABLE (0)
#define GNSS_NV_EFS_SM_TM_MEAS_QMI_REPORT_ENABLE  (1)
#endif /* FEATURE_GNSS_NON_STANDARD_SUPPORT */

/*Dynamic OTDOA enable/disable command*/
#ifdef FEATURE_GNSS_NON_STANDARD_SUPPORT
#define GNSS_NV_EFS_SM_TM_OTDOA_CONTROL_DEFAULT (PDSM_PA_OTDOA_DISABLE)
#else
#define GNSS_NV_EFS_SM_TM_OTDOA_CONTROL_DEFAULT (PDSM_PA_OTDOA_ENABLE)
#endif /* FEATURE_GNSS_NON_STANDARD_SUPPORT */
#define GNSS_NV_EFS_SM_TECH_SEL_LPPM_TBM_PROFILE_DEFAULT  (0) /* Default Tech Sel LPPM tbm profile*/

#define GNSS_NV_EFS_SM_TM_SHA_TLS_SUPPORT_DEFAULT (0)     /*Default is disable for
                                                            1)SHA version above 256, configured in upper 16 bits (0xFFFF0000)
                                                            2)TLS version above 1.1  configured in lower 16 bits (0x0000FFFF)*/

#define GNSS_NV_EFS_SM_TM_AGPS_FEATURE_SUPPORT_DEFAULT (0) /*Default is disable for all NEW AGPS features.
                                                             1) 0x00000001 : Enables SUPL over VoWIFI for 911
                                                             2. 0x00000002 : Enable SUPL over nDDS 911
                                                             3) 0xFFFFFFFC : Reserved for future AGPS features.*/

#define GNSS_NV_EFS_SM_TM_WGS84_DATUM_NMEA_SENTENCE (1)    /*Default is WGS84 NMEA sentence and the value is 
                                                             1) 0x01 : WGS84
                                                             2) 0x02 : PZ90*/
#define GNSS_NV_EFS_DYN_HEPE_DISABLED_FOR_BETTER_TTFF_DEFAULT         (0)  /*NV will 0 by default, i.e Dyn HEPE will be used by default */
#define GNSS_NV_EFS_SM_TM_EXTENDED_NMEA_SENTENCE_MASK_V02_DEFAULT     (0)  /*NV will 0 by default, i.e Dyn HEPE will be used by default */
#define GNSS_NV_EFS_SM_SECOND_BOUNDARY_ALIGNED_NMEA_DEFAULT           (0)  /* NV will 0 by default*/
...
#define GNSS_NV_TECHSEL_WIFI_POS_WAIT_MSECS_DEFAULT      (30 * 1000) /*30 seconds*/
#define GNSS_NV_TECHSEL_WIFI_WEAK_HYST_MSECS_DEFAULT     (0) /*default weak WIFI hysteresis timer value zero implies timer would be not run*/
#define GNSS_NV_TECHSEL_WIFI_UNKNOWN_HYST_MSECS_DEFAULT  (0) /*default unknown WIFI hysteresis timer value zero implies timer would be not run*/
#define GNSS_NV_TECHSEL_ODCPI_BACKOFF_MSECS_DEFAULT      (900 * 1000) /*15 min ODCPI back off interval*/
...
/*Structure used to configure ODCPI timers
*/
typedef PACKED struct PACKED_POST
{
  uint32  q_posWaittimer;
  uint32  q_weakHysttimer;
  uint32  q_unknowHysttimer;
  uint32  q_odcpiBackOfftimer;
} tech_sel_odcpi_timers_s_type;
