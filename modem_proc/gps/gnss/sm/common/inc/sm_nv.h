...
Version Control
  ...
  $Header: //components/rel/gnss8.mpss/11.1/gnss/sm/common/inc/sm_nv.h#1 $
...
07/08/19   sj      change NMEA default configuration at bootup to include GNGNS 
...
/* Enable only "legacy" + GNGNS  nmea sentences. GNGNS is needed in the default
 * config as certain clients (UIM) request a position fix before the HLOS nmea
 * configuration enabling all the sentences is applied. These clients will only 
 * get the nmea sentences enabled by default in SM NV. Because GNGNS is needed in this
 * default nmea set, we must enable it in NV.
 */
#define NV_GNSS_NMEA_DEFAULT_GNSS_SENTENCE_TYPE          ( PDAPI_NMEA_LEGACY )

#define NV_GNSS_NMEA_DEFAULT_GNSS_EXTENDED_SENTENCE_TYPE ( PDAPI_NMEA_GNGNS  )
...
#define NV_GNSS_SUPL_VERSION_V                        0x00020004 /* from MSB to right: rsvd, maj, min, servInd */
#else
...
#define NV_GNSS_OEM_FEATURE_MASK_15_GF_CELL_CPI_ENABLE_CFG       0x00004000 /* Configure Geofence engine CPI hepe acceptable to 10000 meters
                                                                                                                              to let Geofenceing use Cell CPI fixes*/
...
===========================================================================*/
boolean sm_nv_sync_read_from_hw_sub
(
