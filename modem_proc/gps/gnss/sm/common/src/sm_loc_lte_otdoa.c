...
/*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*
                  ...
                  Session Manager to LTE_LOC_OTODA Interface Module

GENERAL DESCRIPTION
  This module acts as an interface between the session manager and the 
  OTDOA module 
...
/*===========================================================================
  ...
  $Header: //components/rel/gnss8.mpss/11.1/gnss/sm/common/src/sm_loc_lte_otdoa.c#1 $
...
11/8/14    gk    new pdapi events for start/done
                                   ...
                                   sys_modem_as_id_e_type e_as_id)
{
  ...
  z_StartCmd.b_SubId = (int8)e_as_id;
  sess_type = u_sess_type;
  loc_lte_otdoa_StartSessCmd (&z_StartCmd);

  sm_loc_lte_send_pdapi_event(SM_LOC_LTE_OTDOA_PDAPI_EVENT_OTDOA_START);
...
/*===========================================================================
  ...
  Return value: 
    boolean: SUCCESS or FAIL
=============================================================================*/
boolean sm_loc_lte_otdoa_get_measurements(const gnss_ClockStructType *const p_gnss_clock, uint32 u_sess_type, boolean earlyMeasurement)
{
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"SM_PL_LTE_OTDOA_API: GET measurements CMD...",0,0,0);
  ...
  */
  ...
  if(ipc_msg_ptr != NULL)
  {
     MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"SM_PL_LTE_OTDOA_API: IPC msg send to OTDOA, id = %d",ipc_msg_ptr->q_MsgId,0,0);
...
boolean sm_loc_lte_otdoa_stop_cmd(uint8 u_reason, uint32 u_sess_type, sys_modem_as_id_e_type e_as_id)
{
  ...
  z_StopCmd.b_SubId = (int8)e_as_id;
  sess_type = u_sess_type;
  loc_lte_otdoa_StopSessCmd(&z_StopCmd);
  
  sm_loc_lte_send_pdapi_event(SM_LOC_LTE_OTDOA_PDAPI_EVENT_OTDOA_STOP);
  return TRUE;
  #if 0
  os_IpcMsgType   *ipc_msg_ptr  = NULL;
  uint32          msg_id        = 0;
...
boolean sm_loc_lte_ota_reset(sys_modem_as_id_e_type e_as_id)
{
  ...
  z_OtaResetInfo.b_SubId = (int8)e_as_id;

  /* OTA RESET - Request PGI to send a command to RRC to refresh TTR data from 
     SIB8/16  */
  mgp_pgi_SendSibTimeRefreshMsg((int8)e_as_id);
