...
/*===========================================================================
  ...
  $Header: //components/rel/gnss8.mpss/11.1/gnss/sm/common/src/gnss_msgr_task.c#1 $
...
boolean gnss_msgr_receive_nas_msg(
   ...
   uint32 *nas_id,
   uint8 *inst_id,
   sys_radio_access_tech_e_type *p_rat_type  
   )
{
#ifdef FEATURE_LTE
  ...
  if (pkt_buf == NULL || buf_size == 0 || pkt_size == NULL || nas_id == NULL || inst_id == NULL || p_rat_type == NULL)
  {
    ...
    do
    {
      ...
      else
      {
        *nas_id = GNSS_MSGR_NTOHL(*nas_id); // NAS ID is in network/big endian order
        /* we can get NAS messages over LTE or NR 5G NAS */
      #ifdef FEATURE_NR5G_SA
        *p_rat_type = msg_ptr->msg.nas_dl_lpp.rat;
      #else
        *p_rat_type = SYS_RAT_LTE_RADIO_ACCESS; 
      #endif /* FEATURE_NR5G_SA */
      }
...
/* Below are off-target testing code for SMART */
extern boolean smart_lpp_cp_receive_nas_msg(uint8 *pkt_buf, uint32 buf_size, uint32 *pkt_size, uint32 *nas_id, uint8 *inst_id, sys_radio_access_tech_e_type *p_rat_type);
...
{
  return smart_lpp_cp_receive_nas_msg(pkt_buf, buf_size, pkt_size, nas_id, inst_id, p_rat_type);
}
