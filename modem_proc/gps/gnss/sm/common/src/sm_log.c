...
/*===========================================================================
  ...
  $Header: //components/rel/gnss8.mpss/11.1/gnss/sm/common/src/sm_log.c#1 $ $DateTime: 2019/10/24 01:30:36 $ $Author: pwbldsvc $
...
LOG_RECORD_DEFINE(LOG_CGPS_PDSM_EXT_STATUS_POS_REPORT_C)
  ...
  int16 x_Altitude;                          /*!< Altitiude valid if u_OptFieldMask has PDSM_PD_ALTITUDE_VALID bit set */
  uint16 w_Heading;                          /*!< Heading valid if u_OptFieldMask has PDSM_PD_HEADING_VALID bit set */
  uint16 w_VelocityHor;                      /*!< Horizontal velocity valid if u_OptFieldMask has PDSM_PD_VELOCITY_VALID bit set */
  uint8 u_FixType;                           /*!< Fix type, TRUE if 3D */
  int8 s_VelocityVer;                        /*!< Vertical velocity valid if u_OptFieldMask has PDSM_PD_VELOCITY_VALID bit set */
  uint8 u_LocUncertaintyV;                   /*!< loc uncertainity vertical valid if u_OptFieldMask has PDSM_PD_ALTITUDE_VALID bit set */
  ...
  uint8 u_UncEllConfidence;                  /*!< confidence of the loc unc. valid if u_OptFieldMask has PDSM_PD_UNCERTAINTY_VALID bit set */
  int8 s_utcOffset;                          /*!< Time utc offset valid if u_OptFieldMask has PDSM_PD_UTCOFFSET_VALID bit set */
  int32 q_ExtLoclat;                         /*!< Latitude: LSB = Pi/2^32, Range -Pi/2 to Pi/2. Valid PDSM_PD_LAT_LONG_VALID bit set */
  int32 q_ExtLoclon;                         /*!<  Longitude: LSB = Pi/2^31, Range -Pi to Pi. Valid PDSM_PD_LAT_LONG_VALID bit set */
  uint16 w_ExtLocUncA;                       /*!< Along axis Unc in meters. Max value is 65535. If this value is
                                                  ...
                                                  PDSM_PD_UNCERTATINITY_VALID is not set. f_loc_unc_horizontal value
...
typedef PACKED struct PACKED_POST
{
  ...
  gps_TimeStructType_DmLog  z_GnssTime; /* GPS|BDS|GAL |QZSS Time at w_RefFCount */
...
typedef PACKED struct PACKED_POST
{
  ...
  uint32 f_TDOP;                /* Time DOP (unitless) */
  uint32 f_GDOP;                /* Geometrical DOP (unitless) */
...
} LOG_GNSS_OEMDRE_POSITION_REPORT_C_type;
...
#define LOG_GNSS_OEMDR_MAX_ELEV_UNC (-90.0f * ((FLT)C_PI/180.0f))  /* Max elev mask, 90 deg in radians */
...
void sm_log_set_parameters_request(pdsm_cmd_s_type *p_CmdType)
{
  ...
  switch (p_CmdType->cmd.pa.info.set_type)
  {
    ...
    case PDSM_PA_LPPE_UP_CONFIG:
      p_LogPkt->u_Length = 4;
      p_LogPkt->u_ParameterValue[0] = (uint32)p_CmdType->cmd.pa.info.pa_set.lppe_up_config_info;
      break;
 ...
 ******************************************************************************
*/
void sm_log_report_position
(
   const pdsm_pd_info_s_type *p_PdInfoType, 
   boolean sft, 
   boolean kf,
   gnss_diag_buffer_hdl      dbuff_hdl
)
{
  ...
  if(FALSE == gnss_dm_log_status(
                  LOG_GNSS_PDSM_POSITION_REPORT_CALLBACK_C, 
                  dbuff_hdl))
  {
    MSG(MSG_SSID_GPSSM, MSG_LEGACY_MED, "LOG_GNSS_PDSM_POSITION_REPORT_CALLBACK_C turned off!");
    return;
  }

  /* Try to allocate memory for the log packet */  
  p_LogPkt = (LOG_GNSS_PDSM_POSITION_REPORT_CALLBACK_C_type *)gnss_diag_buffer_log_alloc(
                    dbuff_hdl,
                    LOG_GNSS_PDSM_POSITION_REPORT_CALLBACK_C,
  ...
  if (p_LogPkt)
  {
    ...
    p_LogPkt->w_ExtLocUncA = (uint16)p_ExtPosData.f_alongAxisUnc;
    p_LogPkt->w_ExtLocUncP = (uint16)p_ExtPosData.f_perpAxisUnc;
    p_LogPkt->q_VelHoriz = (int32)p_ExtPosData.f_velHor;
    p_LogPkt->q_VelVert  = (int32)p_ExtPosData.f_velVert;
    ...
    p_LogPkt->q_Tdop = (uint32)p_ExtPosData.t_dop;
    p_LogPkt->q_Gdop = (uint32)p_ExtPosData.g_dop;
    ...
    p_LogPkt->q_LocUncHorizontal = (uint32)p_ExtPosData.f_loc_unc_horizontal;
    ...
    if (p_PdInfoType->pd_info.pd_data.ext_pos_data.time_src & PDSM_GNSS_TIME_NAVIC)
    {
      p_LogPkt->u_TimeMask |= SM_LOG_TIME_SOURCE_MASK_NAVIC;
    }
  ...
  if (p_LogPkt->u_EventType == (uint8)PDSM_PA_NONE)
  {
    ...
    switch (p_PaInfoType->pa_event)
    {
      ...
      default: /* Unsupported PDSM PA events */
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "No logging support for PDSM PA event (%d)!", p_PaInfoType->pa_event, 0, 0);
        break;
    }
  }
...
)
{
  ...
  if(FALSE == gnss_dm_log_status(
                  LOG_CGPS_PDSM_EXT_STATUS_POS_REPORT_C, 
                  dbuff_hdl))
  {
    MSG(MSG_SSID_GPSSM, MSG_LEGACY_MED, "LOG_CGPS_PDSM_EXT_STATUS_POS_REPORT_C turned off!");
    return;
  }

  /* Try to allocate memory for the log packet */  
  p_LogPkt = (LOG_CGPS_PDSM_EXT_STATUS_POS_REPORT_C_type *)gnss_diag_buffer_log_alloc(
                    dbuff_hdl,
                    LOG_CGPS_PDSM_EXT_STATUS_POS_REPORT_C,
...
/*
 ******************************************************************************
 * sm_log_ext_nmea_var_len_position_report
 *
 * Function description:
 *
 * This function generates a DM log message when variable length nmea report is sent to the
 * PDAPI.
 *
 * Parameters:
 * Return value:
 ...
 ******************************************************************************
*/
void sm_log_ext_nmea_var_len_position_report(uint32 l_ClientId,
                                             pdsm_pd_ext_var_len_debug_nmea_s_type const *const p_ExtNMEAType)
{
  LOG_CGPS_PDSM_EXT_STATUS_NMEA_REPORT_C_type *p_LogPkt;
  uint8 i = 0;

  if ((NULL == p_ExtNMEAType) || (NULL == p_ExtNMEAType->pu_NmeaBuff) || (0 == p_ExtNMEAType->q_Length))
  {
    return;
  }
  /* Try to allocate memory for the log packet */
  ...
  if (p_LogPkt != NULL)
  {
    ...
    p_LogPkt->q_ClientId = l_ClientId;
    p_LogPkt->q_NmeaSentenceType = p_ExtNMEAType->nmea_type;
    p_LogPkt->u_NmeaSentenceLength = (uint8)p_ExtNMEAType->q_Length;
    memscpy(p_LogPkt->u_NmeaData, LOG_PDSM_EXT_NMEA_RPT_DATA_LEN,
            p_ExtNMEAType->pu_NmeaBuff, p_ExtNMEAType->q_Length);
    log_commit(p_LogPkt);
  }
  return;
}
...
)
{
  ...
  if(FALSE == gnss_dm_log_status(
                  LOG_GNSS_PDSM_EXT_STATUS_MEAS_REPORT_C, 
                  dbuff_hdl))
  {
    MSG(MSG_SSID_GPSSM, MSG_LEGACY_MED, "LOG_GNSS_PDSM_EXT_STATUS_MEAS_REPORT_C turned off!");
    return;
  }

  /* Try to allocate memory for the log packet */
  p_LogPkt = (LOG_GNSS_PDSM_EXT_STATUS_MEAS_REPORT_C_type *)gnss_diag_buffer_log_alloc(
                    dbuff_hdl,
                    LOG_GNSS_PDSM_EXT_STATUS_MEAS_REPORT_C,
...
void sm_log_oemdre_measurement_report(const sm_GpsMeasRptStructType *p_measReport)
{
  ...
  uint8 i = 0;
  volatile FLT f_data;
  FLT f_Temp = LOG_GNSS_OEMDR_MAX_ELEV_UNC;
  ...
  if (p_LogPkt)
  {
    ...
    p_LogPkt->e_MeasRptReason                  = p_measReport->z_MeasBlk.z_GnssHeader.e_GnssMeasRptreason;
    p_LogPkt->z_MeasBlk.u_NumSvs               = p_measReport->z_MeasBlk.z_GnssHeader.u_NumSvs;
    p_LogPkt->z_MeasBlk.u_SeqNum               = p_measReport->z_MeasBlk.z_GnssHeader.u_SeqNum;
    p_LogPkt->z_MeasBlk.u_MaxMessageNum        = p_measReport->z_MeasBlk.z_GnssHeader.u_MaxMessageNum;
    ...
    pz_SrcClk = &p_measReport->z_MeasBlk.z_GnssHeader.z_MeasClock;
    ...
    pz_SrcCDMAClk = &p_measReport->z_MeasBlk.z_GnssHeader.z_CDMAClockInfo;
    ...
    p_LogPkt->z_MeasBlk.e_MeasBlkSrc           = p_measReport->z_MeasBlk.z_GnssHeader.e_MeasBlkSrc;
    ...
    if (GNSS_MEAS_BLK_SRC_GPS == p_LogPkt->z_MeasBlk.e_MeasBlkSrc)
    {
      ...
      for (i = 0; i < MIN(ARR_SIZE(p_LogPkt->z_MeasBlk.n_Meas.z_Gps),
                          p_measReport->z_MeasBlk.z_GnssHeader.u_NumSvs); i++)
      {
    ...
    if (GNSS_MEAS_BLK_SRC_QZSS_SBAS == p_LogPkt->z_MeasBlk.e_MeasBlkSrc)
    {
      gnss_MeasStructType_ForDmLog *pz_Dst;
      const gnss_MeasStructType *pz_Src;
 ...
 ******************************************************************************
*/
void sm_log_oemdre_position_fix_report(const sm_FixInfoStructType *p_fixReport)
{
  ...
  const sm_NavPositionStructType *pz_SrcNavSol;
  ...
  if (p_LogPkt)
  {
    ...
    SM_MEMCPY(pz_DstNavSol->f_TDOP, pz_SrcNavSol->f_TDOP);
    SM_MEMCPY(pz_DstNavSol->f_GDOP, pz_SrcNavSol->f_GDOP);
    ...
    {
      if ((p_fixReport->z_NavSvInfo.z_GpsSvInfo[i].q_SvInfoMask & (PDSM_GNSS_SV_INFO_USED_IN_POS_FIX | PDSM_GNSS_SV_INFO_USED_IN_VEL_FIX)))
      {
    ...
    {
      if ((p_fixReport->z_NavSvInfo.z_GloSvInfo[i].q_SvInfoMask & (PDSM_GNSS_SV_INFO_USED_IN_POS_FIX | PDSM_GNSS_SV_INFO_USED_IN_VEL_FIX)))
      {
    ...
    {
      if ((p_fixReport->z_NavSvInfo.z_BdsSvInfo[i].q_SvInfoMask & (PDSM_GNSS_SV_INFO_USED_IN_POS_FIX | PDSM_GNSS_SV_INFO_USED_IN_VEL_FIX)))
      {
    ...
    {
      if ((p_fixReport->z_NavSvInfo.z_GalSvInfo[i].q_SvInfoMask & (PDSM_GNSS_SV_INFO_USED_IN_POS_FIX | PDSM_GNSS_SV_INFO_USED_IN_VEL_FIX)))
      {
...
void sm_log_oemdre_sv_poly_report(const gnss_SvPoly *p_svPolyReport)
{
  ...
  if (p_LogPkt)
  {
    ...
    p_LogPkt->w_Sv             = p_svPolyReport->w_Sv;
    p_LogPkt->b_FreqNum        = p_svPolyReport->b_FreqNum;
    p_LogPkt->u_SvPolyFlags    = (uint8)p_svPolyReport->w_SvPolyFlags;
    ...
    memset(p_LogPkt->d_VelCoef, 0, sizeof(p_LogPkt->d_VelCoef));
    if (SLOTID_OK(p_svPolyReport->w_Sv))
    {
      uint64 *pd_VelCoefOut = p_LogPkt->d_VelCoef;
      const DBL *pd_XyzN = p_svPolyReport->pd_xyzN;
      for (i = 0; i < 3; i++)
      {
        *pd_VelCoefOut++ = (uint64)(*pd_XyzN++);
        *pd_VelCoefOut++ = (uint64)((*pd_XyzN++) * 2.0);
        *pd_VelCoefOut++ = (uint64)((*pd_XyzN++) * 3.0);
        pd_VelCoefOut++;
      }
    }
