...
/*===========================================================================
  ...
  $Header: //components/rel/gnss8.mpss/11.1/gnss/sm/common/src/pdapi.c#4 $
...
)
{
  ...
  */
  ...
  cmd_info_ptr->session_end_info.v_reason_service_interaction = end_session_opt->v_reason_service_interaction;
...
)
{
  ...
  if (client_ptr == NULL) 
  {
    PDSM_MSG_ERROR("PDSM: Null Client pointer", 0,0,0);
    return ret_value;
  }
...
)
{
  ...
  PDSM_MSG_HIGH("--------- GPS Command --------------------",0,0,0);
  PDSM_MSG_HIGH("pdsm_xtra_query_config_info()",0,0,0);
  PDSM_MSG_HIGH("------------------------------------------",0,0,0);
  ...
  */
  ...
  xtra_cmd_ptr->client_id = client_id;
  xtra_cmd_ptr->data_block_ptr = client_data_ptr;
  xtra_cmd_ptr->cmd = PDSM_XTRA_CMD_QUERY_XTRA_CFG_INFO;

  /* Initialize the Information fields. cmd_err field is
  ** initialized so that the interface of passing command's to Session
  ** manager in spite of errors is available.
  */
  cmd_info_ptr->cmd_err = PDSM_XTRA_CMD_ERR_NOERR;
  ...
  */
  ...
  if ( NULL != pXtraCfgReqInfo)
  {
    ...
    xtra_cmd_ptr->info.navic_assist_supported = (pXtraCfgReqInfo->config_mask & 0x02)? TRUE :FALSE;
    xtra_cmd_ptr->info.precise_xtra_supported = (pXtraCfgReqInfo->config_mask & 0x04)? TRUE: FALSE;
  }
...
)
{
  ...
  PDSM_MSG_HIGH("--------- GPS Command --------------------",0,0,0);
  PDSM_MSG_HIGH("pdsm_send_wiper_position_report_ex Deprecated.()",0,0,0);
  PDSM_MSG_HIGH("------------------------------------------",0,0,0);
...
)
{
  ...
  if (client_ptr == NULL) 
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Invalid Client Id",0,0,0);
    return ret_value;
  }
  ...
  */
  ...
  if (ret_value == FALSE) 
  {    
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Failed to queue pdsm command",0,0,0);
...
boolean pdsm_read_qmi_services_config( pdsm_QMIServiceConfig *qmi_Config )
{
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"PDSM not initialized yet", 0, 0, 0);
    return FALSE;
  }
...
)
{
  ...
  else
  {
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Failed to allocate %d bytes buffer for NI msg",ni_msg->len+1,0,0);
      b_retval = FALSE;
    }
    else
    {
      ...
      {    
        ...
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Failed to queue pdsm command INJECT_NI_MSG",0,0,0);
        if ( pd_cb_func != NULL )
        {
...
DESCRIPTION
  This API initiates a command to start or stop LPPM mode. 
...
boolean pdsm_lppm_control(
   ...
   pdsm_client_id_type   client_id,
   void   *client_data_ptr, /* client provided reference data pointer */
   pdsm_pd_lppm_control_s_type *lppm_control_info_ptr /* parameters for lppm_control */
)
{
  MSG(MSG_SSID_GPSSM, MSG_LEGACY_ERROR, "Control LPPM is not supported yet");
  return FALSE;
}
...
)
{
  ...
  if ( (client_ptr != NULL) && (wifi_info_on_demand_data_list_ptr != NULL) &&
       ( wifi_info_on_demand_data_list_ptr->z_wifi_ap_scan_info.q_num_wifi_aps <= QMI_LOC_WIFI_MAX_REPORTED_APS_PER_MSG_V01 ) )
  {
    PDSM_MSG_MED( "--------- GPS Command --------------------", 0, 0, 0 );
    PDSM_MSG_MED( "pdsm_pd_inject_wifi_info_on_demand(): %u, %d",
                  wifi_info_on_demand_data_list_ptr->z_wifi_ap_scan_info.q_num_wifi_aps,
                  wifi_info_on_demand_data_list_ptr->e_errorCause, 0 );
    PDSM_MSG_MED( "------------------------------------------", 0, 0, 0 );
    ...
    z_cmd_data.cmd.wifi_on_demand_info_list_cmd.wifi_on_demand_info_list.e_errorCause =
      wifi_info_on_demand_data_list_ptr->e_errorCause;

    z_cmd_data.cmd.wifi_on_demand_info_list_cmd.wifi_on_demand_info_list.v_on_demand_scan =
      wifi_info_on_demand_data_list_ptr->v_on_demand_scan;

    z_cmd_data.cmd.wifi_on_demand_info_list_cmd.wifi_on_demand_info_list.t_request_timestamp_utc =
      wifi_info_on_demand_data_list_ptr->t_request_timestamp_utc;

    z_cmd_data.cmd.wifi_on_demand_info_list_cmd.wifi_on_demand_info_list.t_recieved_timestamp_utc =
      wifi_info_on_demand_data_list_ptr->t_recieved_timestamp_utc;

    z_cmd_data.cmd.wifi_on_demand_info_list_cmd.wifi_on_demand_info_list.z_wifi_ap_scan_info.q_num_wifi_aps =
      wifi_info_on_demand_data_list_ptr->z_wifi_ap_scan_info.q_num_wifi_aps;

    /* Copy UE Mac address*/
    memscpy ( &(z_cmd_data.cmd.wifi_on_demand_info_list_cmd.wifi_on_demand_info_list.z_wifi_ap_scan_info.ue_mac_address),
              sizeof(z_cmd_data.cmd.wifi_on_demand_info_list_cmd.wifi_on_demand_info_list.z_wifi_ap_scan_info.ue_mac_address),
              &(wifi_info_on_demand_data_list_ptr->z_wifi_ap_scan_info.ue_mac_address),
              sizeof(wifi_info_on_demand_data_list_ptr->z_wifi_ap_scan_info.ue_mac_address));
      
    /* Number of APs can be zero for some error cause like when WIFI is OFF, no measuerments can be
       generated etc. However still TM need to receive this wlan-ap measuerments. */
    if ( ( wifi_info_on_demand_data_list_ptr->z_wifi_ap_scan_info.q_num_wifi_aps != 0 ) &&
         (wifi_info_on_demand_data_list_ptr->z_wifi_ap_scan_info.first_wifi_ap_ptr != NULL) &&
         (wifi_info_on_demand_data_list_ptr->z_wifi_ap_scan_info.first_wifi_ap_add_info_ptr != NULL) )
    {
      z_cmd_data.cmd.wifi_on_demand_info_list_cmd.wifi_on_demand_info_list.z_wifi_ap_scan_info.first_wifi_ap_ptr =
        (locEngWifiApInfoStructT_v01 *)os_MemAlloc( (wifi_info_on_demand_data_list_ptr->z_wifi_ap_scan_info.q_num_wifi_aps * sizeof(locEngWifiApInfoStructT_v01)), OS_MEM_SCOPE_TASK );

      z_cmd_data.cmd.wifi_on_demand_info_list_cmd.wifi_on_demand_info_list.z_wifi_ap_scan_info.first_wifi_ap_add_info_ptr =
        (locEngWifiApAdditionalDataStructT_v01 *)os_MemAlloc( (wifi_info_on_demand_data_list_ptr->z_wifi_ap_scan_info.q_num_wifi_aps * sizeof(locEngWifiApAdditionalDataStructT_v01)), OS_MEM_SCOPE_TASK );

      if ( ( z_cmd_data.cmd.wifi_on_demand_info_list_cmd.wifi_on_demand_info_list.z_wifi_ap_scan_info.first_wifi_ap_ptr == NULL) ||
           ( z_cmd_data.cmd.wifi_on_demand_info_list_cmd.wifi_on_demand_info_list.z_wifi_ap_scan_info.first_wifi_ap_add_info_ptr == NULL)
          )
      {
        MSG_3( MSG_SSID_GPSSM, MSG_LEGACY_ERROR, "Failed to allocate buffer for wifi-on-demand-scan", 0, 0, 0 );

        if ( z_cmd_data.cmd.wifi_on_demand_info_list_cmd.wifi_on_demand_info_list.z_wifi_ap_scan_info.first_wifi_ap_ptr != NULL )
        {
          os_MemFree( (void **)&(z_cmd_data.cmd.wifi_on_demand_info_list_cmd.wifi_on_demand_info_list.z_wifi_ap_scan_info.first_wifi_ap_ptr) ); // free the buffer
        }

        if ( z_cmd_data.cmd.wifi_on_demand_info_list_cmd.wifi_on_demand_info_list.z_wifi_ap_scan_info.first_wifi_ap_add_info_ptr != NULL )
        {
          os_MemFree( (void **)&(z_cmd_data.cmd.wifi_on_demand_info_list_cmd.wifi_on_demand_info_list.z_wifi_ap_scan_info.first_wifi_ap_add_info_ptr) ); // free the buffer
        }
        v_retval = FALSE;
      }
      else
      {
        MSG_3( MSG_SSID_GPSSM, MSG_LEGACY_MED, "Allocated mem. at %lx and %lx",
               z_cmd_data.cmd.wifi_on_demand_info_list_cmd.wifi_on_demand_info_list.z_wifi_ap_scan_info.first_wifi_ap_ptr,
               z_cmd_data.cmd.wifi_on_demand_info_list_cmd.wifi_on_demand_info_list.z_wifi_ap_scan_info.first_wifi_ap_add_info_ptr,
               0 );
        memscpy( (void *)z_cmd_data.cmd.wifi_on_demand_info_list_cmd.wifi_on_demand_info_list.z_wifi_ap_scan_info.first_wifi_ap_ptr,
                 ( wifi_info_on_demand_data_list_ptr->z_wifi_ap_scan_info.q_num_wifi_aps * sizeof(locEngWifiApInfoStructT_v01)),
                 (void *)wifi_info_on_demand_data_list_ptr->z_wifi_ap_scan_info.first_wifi_ap_ptr,
                 ( wifi_info_on_demand_data_list_ptr->z_wifi_ap_scan_info.q_num_wifi_aps * sizeof(locEngWifiApInfoStructT_v01)) );

        memscpy( (void *)z_cmd_data.cmd.wifi_on_demand_info_list_cmd.wifi_on_demand_info_list.z_wifi_ap_scan_info.first_wifi_ap_add_info_ptr,
                 ( wifi_info_on_demand_data_list_ptr->z_wifi_ap_scan_info.q_num_wifi_aps * sizeof(locEngWifiApAdditionalDataStructT_v01)),
                 (void *)wifi_info_on_demand_data_list_ptr->z_wifi_ap_scan_info.first_wifi_ap_add_info_ptr,
                 ( wifi_info_on_demand_data_list_ptr->z_wifi_ap_scan_info.q_num_wifi_aps * sizeof(locEngWifiApAdditionalDataStructT_v01)) );
        v_retval = TRUE; /* put the wlan-ap meas in the queue */
      }
    }
    else
    {
    ...
    if ( TRUE == v_retval )
    {
      v_retval = pdsm_cmd_queue( &z_cmd_data );

      if ( v_retval == FALSE )
      {
        os_MemFree( (void **)&(z_cmd_data.cmd.wifi_on_demand_info_list_cmd.wifi_on_demand_info_list.z_wifi_ap_scan_info.first_wifi_ap_ptr) ); // free the buffer
        os_MemFree( (void **)&(z_cmd_data.cmd.wifi_on_demand_info_list_cmd.wifi_on_demand_info_list.z_wifi_ap_scan_info.first_wifi_ap_add_info_ptr) ); // free the buffer
        MSG_3( MSG_SSID_GPSSM, MSG_LEGACY_ERROR, "Failed to queue pdsm command PDSM_CMD_TYPE_WIFI_ON_DEMAND", 0, 0, 0 );
      }
    }
  }
  ...
  {    
    if ( pd_cb_func != NULL )
    {
      ...
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Failed to process wifi injection", 0, 0, 0);
    } /* End if no callback is provided. */
  }
  return ( v_retval );
}
...
DESCRIPTION
  To create RPC call from ARM 11 to ARM 9 informing whether driver loading
  succeeded or not.
...
===========================================================================*/
void pdsm_notify_wiper_status(boolean wps_available)
{
}
...
boolean pdsm_gm_in_mem_logging_start(uint32 q_max_log_size, uint32 q_period_s)
{
  ...
  if(q_max_log_size > PDSM_GM_IN_MEM_MAX_LOG_SIZE_B)
  {
    ...
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Log size is greater than the max supported. Setting to %d bytes", 
            q_max_log_size, 0, 0);
  }
  if(q_period_s < PDSM_GM_IN_MEM_MIN_PERIOD_S)
  {
    q_period_s = PDSM_GM_IN_MEM_MIN_PERIOD_S;
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Log period is lesser than the min supported. Setting to %d s ", 
            q_period_s, 0, 0);
  }
...
FUNCTION pdsm_gera_proc_cmd

DESCRIPTION
  This function is for processing test command and sending it to GERA.
...
===========================================================================*/
boolean pdsm_gera_proc_cmd(tm_gera_test_cmd_e_type e_gera_cmd_type, 
                           pdsm_gera_client_id_e_type e_client_id, 
                           uint32 q_dep_client_mask)
{

            
  os_IpcMsgType* p_ipc_ptr = NULL;
  tm_gera_test_cmd_s_type* p_hdr_ptr = NULL;
    
     
  if((e_gera_cmd_type > TM_GERA_TEST_CMD_LAST) || (e_client_id > PDSM_GERA_CLIENT_ID_LAST))
  {
    MSG_2(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Invalid GERA test command %u or ClientId %u passed in ",e_gera_cmd_type, e_client_id);
    return FALSE;
  }
  /*Send an IPC to GM task */  
  p_ipc_ptr =  os_IpcCreate(sizeof(tm_gera_test_cmd_s_type),
                            IPC_ALLOCATION_DYNAMIC,(uint32)THREAD_ID_SM_TM);
          
  if(NULL == p_ipc_ptr)
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Ipc creation failed ",0,0,0);
    return FALSE;  
  }
  p_ipc_ptr->q_MsgId          = (uint32)TM_CORE_MSG_ID_GERA_TEST_CMD;
  p_ipc_ptr->q_DestThreadId   = (uint32)THREAD_ID_SM_TM;
  p_ipc_ptr->q_Size           = sizeof(tm_gera_test_cmd_s_type);
  p_hdr_ptr = (tm_gera_test_cmd_s_type*) p_ipc_ptr->p_Data; /*lint !e826*/
    
  p_hdr_ptr->q_client_id = (uint32)e_client_id;
  p_hdr_ptr->e_gera_cmd_type = e_gera_cmd_type;
  p_hdr_ptr->q_dep_client_mask = q_dep_client_mask;

   
  if(!os_IpcSend(p_ipc_ptr,(uint32)THREAD_ID_SM_TM))
  {
    (void)os_IpcDelete(p_ipc_ptr);
    MSG_1(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Failed to send IPC for GERA test CMD %d into SM_TM",e_gera_cmd_type);
    return FALSE;
  }  
  return TRUE;
...
{
  boolean b_ret = FALSE;
  MSG_1(MSG_SSID_GPSSM, MSG_LEGACY_MED," GERA test CMD for register client %u", e_client_id);
  b_ret = pdsm_gera_proc_cmd(TM_GERA_TEST_CMD_CLIENT_REG, e_client_id, 0x0);
...
{
  boolean b_ret = FALSE;
  MSG_1(MSG_SSID_GPSSM, MSG_LEGACY_MED," GERA test CMD for deregister client %u", e_client_id);
  b_ret = pdsm_gera_proc_cmd(TM_GERA_TEST_CMD_CLIENT_DEREG, e_client_id, 0x0);
...
FUNCTION pdsm_gera_cmd_client_enable_request

DESCRIPTION
  This function is to request for client enable request to GERA through a test interface.
...
===========================================================================*/
boolean pdsm_gera_cmd_client_enable_request(pdsm_gera_client_id_e_type e_client_id, uint32 q_dep_client_id_mask)
{
  boolean b_ret = FALSE;
  MSG_1(MSG_SSID_GPSSM, MSG_LEGACY_MED," GERA test CMD for enable client %u ", e_client_id);
  b_ret = pdsm_gera_proc_cmd(TM_GERA_TEST_CMD_CLIENT_ENABLE_REQ, e_client_id, q_dep_client_id_mask);
...
FUNCTION pdsm_gera_cmd_client_disable_request
...
{
  boolean b_ret = FALSE;
  MSG_1(MSG_SSID_GPSSM, MSG_LEGACY_MED," GERA test CMD for disable client %u", e_client_id);
  b_ret = pdsm_gera_proc_cmd(TM_GERA_TEST_CMD_CLIENT_DISABLE_REQ, e_client_id, 0x0);
...
FUNCTION pdsm_gera_cmd_client_disabled
...
{
  boolean b_ret = FALSE;
  MSG_1(MSG_SSID_GPSSM, MSG_LEGACY_MED," GERA test CMD for disabled info from client %u", e_client_id);
  b_ret = pdsm_gera_proc_cmd(TM_GERA_TEST_CMD_CLIENT_DISABLED, e_client_id, 0x0);
...
{
  boolean b_ret = FALSE;
  MSG(MSG_SSID_GPSSM, MSG_LEGACY_MED," GERA test CMD for GERA reset");
  b_ret = pdsm_gera_proc_cmd(TM_GERA_TEST_CMD_RESET, 0, 0x0);
...
{
  boolean b_ret = FALSE;
  MSG(MSG_SSID_GPSSM, MSG_LEGACY_MED," GERA test CMD for GERA query client state");
  b_ret = pdsm_gera_proc_cmd(TM_GERA_TEST_CMD_QUERY_CLIENT_STATE, 0, 0x0);
  return b_ret;
}
...
)
{
  ...
  if( (NULL == client_ptr) || (NULL == srn_scan_info_data_list_ptr) || 
      ( srn_scan_info_data_list_ptr->z_srn_ap_scan_info.q_num_srn_aps > LOC_ENG_SRN_MAX_REPORTED_APS_PER_MSG_V01 ) ||
      ( srn_scan_info_data_list_ptr->z_srn_ap_scan_info.srn_ap_ptr == NULL ) 
    )
  {
    MSG(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Invalid SRN injection");
    b_retval = FALSE;
  }
  else
  {
    PDSM_MSG_MED("--------- GPS Command --------------------",0,0,0);
    PDSM_MSG_MED("pdsm_pd_inject_srn_scan_info(): %u",srn_scan_info_data_list_ptr->z_srn_ap_scan_info.q_num_srn_aps,0,0);
    PDSM_MSG_MED("------------------------------------------",0,0,0);

    /* pack data for pdsm_cmd_queue */
    z_cmd_data.cmd_type = PDSM_CMD_TYPE_SRN_INFO;
    z_cmd_data.cmd.srn_scan_info_list_cmd.client_id = client_id;
    z_cmd_data.cmd.srn_scan_info_list_cmd.client_data_ptr = client_data_ptr;
    z_cmd_data.cmd.srn_scan_info_list_cmd.cmd_cb_func = pd_cb_func;

    z_cmd_data.cmd.srn_scan_info_list_cmd.srn_scan_info_list.e_errorCause = 
      srn_scan_info_data_list_ptr->e_errorCause;

    z_cmd_data.cmd.srn_scan_info_list_cmd.srn_scan_info_list.t_request_timestamp_utc = 
      srn_scan_info_data_list_ptr->t_request_timestamp_utc;

    z_cmd_data.cmd.srn_scan_info_list_cmd.srn_scan_info_list.t_recieved_timestamp_utc = 
      srn_scan_info_data_list_ptr->t_recieved_timestamp_utc;
  
    z_cmd_data.cmd.srn_scan_info_list_cmd.srn_scan_info_list.z_srn_ap_scan_info.q_num_srn_aps = 
      srn_scan_info_data_list_ptr->z_srn_ap_scan_info.q_num_srn_aps;

    z_cmd_data.cmd.srn_scan_info_list_cmd.srn_scan_info_list.z_srn_ap_scan_info.srn_ap_ptr = 
      (locEngSrnBtleApDeviceDataStructT_v01*)os_MemAlloc( (srn_scan_info_data_list_ptr->z_srn_ap_scan_info.q_num_srn_aps * sizeof(locEngSrnBtleApDeviceDataStructT_v01) )
                                                          , OS_MEM_SCOPE_TASK );

    if ( NULL == z_cmd_data.cmd.srn_scan_info_list_cmd.srn_scan_info_list.z_srn_ap_scan_info.srn_ap_ptr )
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Failed to allocate buffer for srn scan", 0,0,0);
      b_retval = FALSE;
    }
    else
    {
      MSG_1(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Allocated mem. at %lx ", 
        z_cmd_data.cmd.srn_scan_info_list_cmd.srn_scan_info_list.z_srn_ap_scan_info.srn_ap_ptr);
       
      memscpy( (void *)z_cmd_data.cmd.srn_scan_info_list_cmd.srn_scan_info_list.z_srn_ap_scan_info.srn_ap_ptr, 
               ( srn_scan_info_data_list_ptr->z_srn_ap_scan_info.q_num_srn_aps * sizeof(locEngSrnBtleApDeviceDataStructT_v01) ),
               (void *)srn_scan_info_data_list_ptr->z_srn_ap_scan_info.srn_ap_ptr,
               ( srn_scan_info_data_list_ptr->z_srn_ap_scan_info.q_num_srn_aps * sizeof(locEngSrnBtleApDeviceDataStructT_v01) ) );
      ...
      if (b_retval == FALSE) 
      {    
        os_MemFree((void **)&(z_cmd_data.cmd.srn_scan_info_list_cmd.srn_scan_info_list.z_srn_ap_scan_info.srn_ap_ptr)); // free the buffer
        MSG(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Failed to queue pdsm command PDSM_CMD_TYPE_SRN_INFO");
      }
    }
  ...
  {    
    if ( pd_cb_func != NULL )
    {
      pd_cb_func( client_data_ptr, PDSM_PD_CMD_INJECT_SRN_INFO, PDSM_PD_CMD_ERR_NO_BUF_L );
    } /* End if no callback is provided. */
    MSG(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Failed to process srn injection");
  }
  return b_retval;
}
...
pdsm_gps_lock_e_type pdsm_get_gps1_lock(void)
{
  ...
  nv_item_type  gps1_lock_nv_read_buffer;

  (void)sm_nv_read_ex(NV_GPS1_LOCK_I, (void *)&gps1_lock_nv_read_buffer);
  gps_lock = (pdsm_gps_lock_e_type)gps1_lock_nv_read_buffer.gps1_lock;
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "pdsm_get_gps1_lock %d", gps_lock, 0, 0);
  return gps_lock;
}