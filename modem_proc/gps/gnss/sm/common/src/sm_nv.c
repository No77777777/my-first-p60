...
Version Control
  ...
  $Header: //components/rel/gnss8.mpss/11.1/gnss/sm/common/src/sm_nv.c#1 $
...
12/04/19   py      Wrapper function support to read from H/w and cached for any sub
08/31/18   sum     replace pdsm_getbuf() with os_MemAlloc() while allocating memory for NV reading.
04/09/19   mj      Deprecate NV1930
04/02/18   py      Cached UMTS_PDE_SERVER_ADDR_IPV6_I & VX_APP_TRUSTED_SETTINGS_I
...
/* Below are carrier-specific NVs (one copy per sub) */
  ...
  uint32   cgps_umts_pde_server_addr_ipv4[SM_CONFIG_MAX_SUBS];  
  /*UMTS PDE Server URL */
  uint8  cgps_umts_pde_server_addr_url[SM_CONFIG_MAX_SUBS][128];
  /*UMTS PDE Server Ipv6 Address */
  uint16   cgps_umts_pde_server_addr_ipv6[SM_CONFIG_MAX_SUBS][8];  
  /*SUPL Version*/
  ...
  uint32 cgps_1x_mpc_server_addr_ipv4[SM_CONFIG_MAX_SUBS];
  /* 1x MPC Server Port */
  uint32 cgps_1x_mpc_server_port[SM_CONFIG_MAX_SUBS];
  /* Item to disable or enable Vx LCS Agent functionality. */
  uint8  gps1_vx_lcs_agent[SM_CONFIG_MAX_SUBS];
  /*NV_GPS1_VX_APP_TRUSTED_SETTINGS_I*/
  uint32 gps1_vx_app_trusted_settings[SM_CONFIG_MAX_SUBS];
  /* Read Vx teleservice ID */
  word   gps1_vx_ni_teleservice_id[SM_CONFIG_MAX_SUBS];
  /* Read Max MO duration */
  dword  gps1_vx_mo_max_duration[SM_CONFIG_MAX_SUBS];
...
static sm_nv_cached_struct_type sm_nv_cached_values;
...
void sm_nv_load_default_values_to_cache(void)
{
  ...
  /*1X PDE Server Ipv4 Address */
  sm_nv_cached_values.cgps_1x_pde_server_addr_ipv4 = 0; 
  /*1X PDE Server Port */
  sm_nv_cached_values.cgps_1x_pde_server_port = 0;
  /*1X PDE Server URL */
  sm_nv_cached_values.cgps_1x_pde_server_addr_url[0] = '\0';
  /*NMEA Config Info*/
  sm_nv_cached_values.cgps_nmea_config_info.nmea_port_type = NV_CGPS_NMEA_DEFAULT_PORT_TYPE ;
  sm_nv_cached_values.cgps_nmea_config_info.nmea_reporting_type = NV_CGPS_NMEA_DEFAULT_REPORTING_TYPE;
  
  /*UMTS PDE Server Port */
  sm_nv_cached_values.cgps_umts_pde_server_port = 0;
  /*XTRA download interval*/
  sm_nv_cached_values.gps1_xtra_download_interval = NV_AAGPS_DEFAULT_XTRA_DOWNLOAD_INTERVAL;
  /*XTRA Auto Download Enabled*/
  sm_nv_cached_values.gps1_xtra_auto_download_enabled = GNSS_NV_DEFAULT_XTRA_AUTO_DL_DISABLED;
  /*SBAS User Preference*/
  sm_nv_cached_values.cgps_sbas_user_preference = 1;  
  /* 1x MPC Server Address URL */
  sm_nv_cached_values.cgps_1x_mpc_server_addr_url[0] = '\0';    
   /* MT LR Support */
  sm_nv_cached_values.aagps_mt_lrsupport = 1;
   /* MO Method */
  sm_nv_cached_values.cgps_mo_method = MO_UP;
   /* Simple gpsOne PD API password */
  memset((void*)&sm_nv_cached_values.gpsone_password, 0 , sizeof(sm_nv_cached_values.gpsone_password));
   /* Rate of security updates sent to client */
  sm_nv_cached_values.gps1_sec_update_rate = 0;
   /* Configures the On-Demand Positioning support */
  sm_nv_cached_values.cgps_on_demand_enabled = FALSE; 
   /* 2G MO-LR Support */
  sm_nv_cached_values.aagps_2g_mo_lrsupport = NV_AAGPS_2G_MO_LRSUPPORT_V;       
   /* 3F MO LR Support */
   sm_nv_cached_values.aagps_3g_mo_lrsupport = NV_AAGPS_3G_MO_LRSUPPORT_V; 

   /* Below are for carrier-specific NVs (one copy per sub) */
   for (i = 0; i < SM_CONFIG_MAX_SUBS; i++)
   {
    /* GPS Lock */
#if( defined(FEATURE_GNSS_LOCK_AT_BOOTUP) && !defined(FEATURE_INTERNAL_USE_FFABUILDS_ONLY) )
    /* Default value: GPS Lock = LOCK_ALL if the feature is defined */
    sm_nv_cached_values.gps1_lock[i] = (dword)PDSM_GPS_LOCK_ALL;
#else
  /* Default value: GPS Lock = LOCK_NONE if the feature is not defined */
    sm_nv_cached_values.gps1_lock[i] = (dword)PDSM_GPS_LOCK_NONE;
#endif  
    /*NMEA Sentence Type*/
    sm_nv_cached_values.gnss_nmea_sentence_type[i] = NV_GNSS_NMEA_DEFAULT_GNSS_SENTENCE_TYPE;
    /*NMEA Extended Sentence Type*/
    sm_nv_cached_values.gnss_nmea_extended_sentence_type[i] = NV_GNSS_NMEA_DEFAULT_GNSS_EXTENDED_SENTENCE_TYPE;
    /*UMTS PDE Server URL */
    sm_nv_cached_values.cgps_umts_pde_server_addr_url[i][0] = NV_CGPS_UMTS_PDE_SERVER_ADDR_URL_V;
    /*SUPL Version*/  
#ifdef FEATURE_GNSS_NON_STANDARD_SUPPORT      
    sm_nv_cached_values.gnss_supl_version[i] = 0; /*SUPL disabled by default*/  
#else
    sm_nv_cached_values.gnss_supl_version[i] = NV_GNSS_SUPL_VERSION_V;  
#endif
    /* Enable User Plane Secure Transport */
    sm_nv_cached_values.aagps_use_transport_security[i] = NV_AAGPS_DEFAULT_USE_TRANSPORT_SECURITY_V;
    /* Positioning Modes Supported */
    sm_nv_cached_values.aagps_positioning_modes_supported[i] = NV_AAGPS_POSITIONING_MODES_SUPPORTED_V;  
    /* 1x MPC Server Address IPV4 */
    sm_nv_cached_values.cgps_1x_mpc_server_addr_ipv4[i] = 0;
    /* 1x MPC Server Port */
    sm_nv_cached_values.cgps_1x_mpc_server_port[i] = 0;  
    /* Item to disable or enable Vx LCS Agent functionality. */
    sm_nv_cached_values.gps1_vx_lcs_agent[i] = PDSM_LCS_AGENT_DISABLED;    
    /* vx App trusted settings*/
    sm_nv_cached_values.gps1_vx_app_trusted_settings[i] = LCS_AGENT_DEFAULT_ALL_DISABLED;
    /* Read Vx teleservice ID */
    sm_nv_cached_values.gps1_vx_ni_teleservice_id[i] = LCS_AGENT_DEFAULT_TELESERVICE_ID;
    /* Max MO duration */
    sm_nv_cached_values.gps1_vx_mo_max_duration[i] = LCS_AGENT_DEFAULT_MAX_DURATION;
    /*UMTS PDE Server Ipv4 Address */
    sm_nv_cached_values.cgps_umts_pde_server_addr_ipv4[i] = 0; 
    /*UMTS PDE Server Ipv6 Address */
    sm_nv_cached_values.cgps_umts_pde_server_addr_ipv6[i][0] = '\0';
...
{
  nv_item_type* nv_item_ptr = (nv_item_type*)item_ptr;
  ...
  if ((sub < SM_CONFIG_MIN_SUB_ID) || (sub > sm_nv_max_sub))
  {
   ...
   /*
     * Specific NV items written by NON_PDS_GOBI_CONFIG are not flushed to the hw, as doing so
     * means excessive wait times for the user. In any case, these items are 
     * configured at init time, at bootup, therefore we can afford to keep only
     * a RAM copy.
     */
  /*All NV's modified by pdsm_set_param is supported*/
  switch(nv_item)
  {
    case NV_CGPS_1X_PDE_SERVER_ADDR_IPV4_I:
              memscpy((void*)&(sm_nv_cached_values.cgps_1x_pde_server_addr_ipv4),
                   sizeof(sm_nv_cached_values.cgps_1x_pde_server_addr_ipv4),
                   (void*) &(nv_item_ptr->cgps_1x_pde_server_addr_ipv4), 
                    sizeof(nv_item_ptr->cgps_1x_pde_server_addr_ipv4));                       
              break;
    case NV_CGPS_1X_PDE_SERVER_PORT_I:
              memscpy((void*)&(sm_nv_cached_values.cgps_1x_pde_server_port),
                  sizeof(sm_nv_cached_values.cgps_1x_pde_server_port),
                  (void*) &(nv_item_ptr->cgps_1x_pde_server_port), 
                  sizeof(nv_item_ptr->cgps_1x_pde_server_port));          
              break;
    case NV_CGPS_1X_PDE_SERVER_ADDR_URL_I:
              memscpy((void*)&(sm_nv_cached_values.cgps_1x_pde_server_addr_url),
                  sizeof(sm_nv_cached_values.cgps_1x_pde_server_addr_url),
                   (void*) &(nv_item_ptr->cgps_1x_pde_server_addr_url), 
                  sizeof(nv_item_ptr->cgps_1x_pde_server_addr_url));                
              break;
    case NV_CGPS_NMEA_CONFIG_INFO_I:
              memscpy((void*)&(sm_nv_cached_values.cgps_nmea_config_info),
                  sizeof(sm_nv_cached_values.cgps_nmea_config_info),
                  (void*) &(nv_item_ptr->cgps_nmea_config_info), 
                  sizeof(nv_item_ptr->cgps_nmea_config_info));                      
              break;
    case NV_CGPS_UMTS_PDE_SERVER_PORT_I:
              memscpy((void*)&(sm_nv_cached_values.cgps_umts_pde_server_port),
                   sizeof(sm_nv_cached_values.cgps_umts_pde_server_port),
                   (void*) &(nv_item_ptr->cgps_umts_pde_server_port), 
                  sizeof(nv_item_ptr->cgps_umts_pde_server_port)); 
              break;
    case NV_GPS1_XTRA_DOWNLOAD_INTERVAL_I:
              memscpy((void*)&(sm_nv_cached_values.gps1_xtra_download_interval),
                  sizeof(sm_nv_cached_values.gps1_xtra_download_interval),
                   (void*) &(nv_item_ptr->gps1_xtra_download_interval), 
                  sizeof(nv_item_ptr->gps1_xtra_download_interval));
              break;
    case NV_GPS1_XTRA_AUTO_DOWNLOAD_ENABLED_I:
              memscpy((void*)&(sm_nv_cached_values.gps1_xtra_auto_download_enabled),
                  sizeof(sm_nv_cached_values.gps1_xtra_auto_download_enabled),
                   (void*) &(nv_item_ptr->gps1_xtra_auto_download_enabled), 
                  sizeof(nv_item_ptr->gps1_xtra_auto_download_enabled));
              break;
    case NV_CGPS_SBAS_USER_PREFERENCE_I:
              memscpy((void*)&(sm_nv_cached_values.cgps_sbas_user_preference),
                  sizeof(sm_nv_cached_values.cgps_sbas_user_preference),
                   (void*) &(nv_item_ptr->cgps_sbas_user_preference), 
                  sizeof(nv_item_ptr->cgps_sbas_user_preference));
              break;
    case NV_CGPS_1X_MPC_SERVER_ADDR_URL_I:
              memscpy((void*)&(sm_nv_cached_values.cgps_1x_mpc_server_addr_url),
                  sizeof(sm_nv_cached_values.cgps_1x_mpc_server_addr_url),
                   (void*) &(nv_item_ptr->cgps_1x_mpc_server_addr_url), 
                  sizeof(nv_item_ptr->cgps_1x_mpc_server_addr_url));
              break;  
    case NV_AAGPS_MT_LRSUPPORT_I:
              memscpy((void*)&(sm_nv_cached_values.aagps_mt_lrsupport),
                  sizeof(sm_nv_cached_values.aagps_mt_lrsupport),
                   (void*) &(nv_item_ptr->aagps_mt_lrsupport), 
                  sizeof(nv_item_ptr->aagps_mt_lrsupport));
              break;   
    case NV_CGPS_MO_METHOD_I:
              memscpy((void*)&(sm_nv_cached_values.cgps_mo_method),
                  sizeof(sm_nv_cached_values.cgps_mo_method),
                   (void*) &(nv_item_ptr->cgps_mo_method), 
                  sizeof(nv_item_ptr->cgps_mo_method));
              break;   
    case NV_GPSONE_PASSWORD_I:
              memscpy((void*)&(sm_nv_cached_values.gpsone_password),
                  sizeof(sm_nv_cached_values.gpsone_password),
                   (void*) &(nv_item_ptr->gpsone_password), 
                  sizeof(nv_item_ptr->gpsone_password));
              break; 
    case NV_GPS1_SEC_UPDATE_RATE_I:
              memscpy((void*)&(sm_nv_cached_values.gps1_sec_update_rate),
                  sizeof(sm_nv_cached_values.gps1_sec_update_rate),
                   (void*) &(nv_item_ptr->gps1_sec_update_rate), 
                  sizeof(nv_item_ptr->gps1_sec_update_rate));
              break; 
    case NV_CGPS_ON_DEMAND_ENABLED_I:
              memscpy((void*)&(sm_nv_cached_values.cgps_on_demand_enabled),
                  sizeof(sm_nv_cached_values.cgps_on_demand_enabled),
                   (void*) &(nv_item_ptr->cgps_on_demand_enabled), 
                  sizeof(nv_item_ptr->cgps_on_demand_enabled));
              break; 
    case NV_AAGPS_2G_MO_LRSUPPORT_I:
              memscpy((void*)&(sm_nv_cached_values.aagps_2g_mo_lrsupport),
                  sizeof(sm_nv_cached_values.aagps_2g_mo_lrsupport),
                   (void*) &(nv_item_ptr->aagps_2g_mo_lrsupport), 
                  sizeof(nv_item_ptr->aagps_2g_mo_lrsupport));
              break;  
   case NV_AAGPS_3G_MO_LRSUPPORT_I:
              memscpy((void*)&(sm_nv_cached_values.aagps_3g_mo_lrsupport),
                  sizeof(sm_nv_cached_values.aagps_3g_mo_lrsupport),
                   (void*) &(nv_item_ptr->aagps_3g_mo_lrsupport), 
                  sizeof(nv_item_ptr->aagps_3g_mo_lrsupport));
              break;                 
  /* Below are for carrier-specific NVs (one copy per sub) */
    case NV_GPS1_LOCK_I:
              memscpy((void*)&(sm_nv_cached_values.gps1_lock[sub]),
                sizeof(sm_nv_cached_values.gps1_lock[sub]),
                (void*) &(nv_item_ptr->gps1_lock), 
                sizeof(nv_item_ptr->gps1_lock));
              break;
    case NV_GNSS_NMEA_SENTENCE_TYPE_I:
              memscpy((void*)&(sm_nv_cached_values.gnss_nmea_sentence_type[sub]),
                  sizeof(sm_nv_cached_values.gnss_nmea_sentence_type[sub]),
                   (void*) &(nv_item_ptr->gnss_nmea_sentence_type), 
                  sizeof(nv_item_ptr->gnss_nmea_sentence_type));
              break;
    case NV_GNSS_NMEA_EXTENDED_SENTENCE_TYPE_I:
              memscpy((void*)&(sm_nv_cached_values.gnss_nmea_extended_sentence_type[sub]),
                  sizeof(sm_nv_cached_values.gnss_nmea_extended_sentence_type[sub]),
                   (void*) &(nv_item_ptr->gnss_nmea_extended_sentence_type), 
                  sizeof(nv_item_ptr->gnss_nmea_extended_sentence_type));     
              break;
    case NV_CGPS_UMTS_PDE_SERVER_ADDR_IPV4_I:
              memscpy((void*)&(sm_nv_cached_values.cgps_umts_pde_server_addr_ipv4[sub]),
                  sizeof(sm_nv_cached_values.cgps_umts_pde_server_addr_ipv4[sub]),
                   (void*) &(nv_item_ptr->cgps_umts_pde_server_addr_ipv4), 
                  sizeof(nv_item_ptr->cgps_umts_pde_server_addr_ipv4)); 
              break;
    case NV_CGPS_UMTS_PDE_SERVER_ADDR_IPV6_I:
      memscpy((void *)&(sm_nv_cached_values.cgps_umts_pde_server_addr_ipv6[sub]),
              sizeof(sm_nv_cached_values.cgps_umts_pde_server_addr_ipv6[sub]),
              (void *)&(nv_item_ptr->cgps_umts_pde_server_addr_ipv6),
              sizeof(nv_item_ptr->cgps_umts_pde_server_addr_ipv6));
      break;              
    case NV_CGPS_UMTS_PDE_SERVER_ADDR_URL_I:
              memscpy((void*)&(sm_nv_cached_values.cgps_umts_pde_server_addr_url[sub]),
                  sizeof(sm_nv_cached_values.cgps_umts_pde_server_addr_url[sub]),
                   (void*) &(nv_item_ptr->cgps_umts_pde_server_addr_url), 
                  sizeof(nv_item_ptr->cgps_umts_pde_server_addr_url));
              break;
    case NV_GNSS_SUPL_VERSION_I:
              memscpy((void*)&(sm_nv_cached_values.gnss_supl_version[sub]),
                  sizeof(sm_nv_cached_values.gnss_supl_version[sub]),
                   (void*) &(nv_item_ptr->gnss_supl_version), 
                  sizeof(nv_item_ptr->gnss_supl_version));
              break;        
    case NV_AAGPS_USE_TRANSPORT_SECURITY_I:
              memscpy((void*)&(sm_nv_cached_values.aagps_use_transport_security[sub]),
                  sizeof(sm_nv_cached_values.aagps_use_transport_security[sub]),
                   (void*) &(nv_item_ptr->aagps_use_transport_security), 
                  sizeof(nv_item_ptr->aagps_use_transport_security));
              break;  
    case NV_AAGPS_POSITIONING_MODES_SUPPORTED_I:
              memscpy((void*)&(sm_nv_cached_values.aagps_positioning_modes_supported[sub]),
                  sizeof(sm_nv_cached_values.aagps_positioning_modes_supported[sub]),
                   (void*) &(nv_item_ptr->aagps_positioning_modes_supported), 
                  sizeof(nv_item_ptr->aagps_positioning_modes_supported));
              break;     
    case NV_CGPS_1X_MPC_SERVER_ADDR_IPV4_I:
              memscpy((void*)&(sm_nv_cached_values.cgps_1x_mpc_server_addr_ipv4[sub]),
                  sizeof(sm_nv_cached_values.cgps_1x_mpc_server_addr_ipv4[sub]),
                   (void*) &(nv_item_ptr->cgps_1x_mpc_server_addr_ipv4), 
                  sizeof(nv_item_ptr->cgps_1x_mpc_server_addr_ipv4));
              break;     
    case NV_CGPS_1X_MPC_SERVER_PORT_I:
              memscpy((void*)&(sm_nv_cached_values.cgps_1x_mpc_server_port[sub]),
                  sizeof(sm_nv_cached_values.cgps_1x_mpc_server_port[sub]),
                   (void*) &(nv_item_ptr->cgps_1x_mpc_server_port), 
                  sizeof(nv_item_ptr->cgps_1x_mpc_server_port));
              break;  
    case NV_GPS1_VX_LCS_AGENT_I:
              memscpy((void*)&(sm_nv_cached_values.gps1_vx_lcs_agent[sub]),
                  sizeof(sm_nv_cached_values.gps1_vx_lcs_agent[sub]),
                   (void*) &(nv_item_ptr->gps1_vx_lcs_agent), 
                  sizeof(nv_item_ptr->gps1_vx_lcs_agent));
              break;   
    case NV_GPS1_VX_APP_TRUSTED_SETTINGS_I:
      memscpy((void *)&(sm_nv_cached_values.gps1_vx_app_trusted_settings[sub]),
              sizeof(sm_nv_cached_values.gps1_vx_app_trusted_settings[sub]),
              (void *)&(nv_item_ptr->gps1_vx_app_trusted_settings),
              sizeof(nv_item_ptr->gps1_vx_app_trusted_settings));
      break;
    case NV_GPS1_VX_NI_TELESERVICE_ID_I:
              memscpy((void*)&(sm_nv_cached_values.gps1_vx_ni_teleservice_id[sub]),
                  sizeof(sm_nv_cached_values.gps1_vx_ni_teleservice_id[sub]),
                   (void*) &(nv_item_ptr->gps1_vx_ni_teleservice_id), 
                  sizeof(nv_item_ptr->gps1_vx_ni_teleservice_id));
              break;   
    case NV_GPS1_VX_MO_MAX_DURATION_I:
              memscpy((void*)&(sm_nv_cached_values.gps1_vx_mo_max_duration[sub]),
                  sizeof(sm_nv_cached_values.gps1_vx_mo_max_duration[sub]),
                   (void*) &(nv_item_ptr->gps1_vx_mo_max_duration), 
                  sizeof(nv_item_ptr->gps1_vx_mo_max_duration));
              break;   

  /* Above are for carrier-specific NVs (one copy per sub) */
...
FUNCTION sm_nv_handle_async_rdwr_cmd
...
static void sm_nv_handle_async_rdwr_cmd(void)
{
  ...
  if (sm_nv_initialized)
  {
    ...
    if ( NULL == nv_ptr )
    {
      MSG(MSG_SSID_GPSSM, MSG_LEGACY_ERROR, "sm_nv_handle_async_rdwr_cmd NULL Ptr");
      return;
    }

    /* ASYNC NV Read API is not used */
    ...
    {      
      ...
      if(sm_nv_write_to_cache_sub(nv_ptr->item, 
                                  ...
                                  nv_ptr->sub))
      {
        MSG_2(MSG_SSID_GPSSM, MSG_LEGACY_MED, "SM NV Cache Write: %d (sub=%d)",
              nv_ptr->item, nv_ptr->sub);
      }
      else
      {
        MSG_2(MSG_SSID_GPSSM, MSG_LEGACY_MED, "SM NV Cache Write FAILED: %d (sub=%d)",
              nv_ptr->item, nv_ptr->sub);
...
void sm_nv_read_default (nv_items_enum_type nv_item, void *item_ptr)
{
  ...
  switch (nv_item)
  {
    ...
    case NV_GNSS_OEM_FEATURE_MASK_I:
      ...
      ((nv_item_type*)item_ptr)->gnss_oem_feature_mask |= NV_GNSS_OEM_FEATURE_MASK_1_OEMDRE; //uint32
...
static boolean sm_nv_read_from_cache_sub(nv_items_enum_type nv_item, void* item_ptr, sys_modem_as_id_e_type sub)
{
  ...
  if ((sub < SM_CONFIG_MIN_SUB_ID) || (sub > sm_nv_max_sub))
  {
    ...
    {
      case NV_CGPS_1X_PDE_SERVER_ADDR_IPV4_I:
                memscpy((void*) &(nv_item_ptr->cgps_1x_pde_server_addr_ipv4),
                    sizeof(nv_item_ptr->cgps_1x_pde_server_addr_ipv4),
                     (void*)&(sm_nv_cached_values.cgps_1x_pde_server_addr_ipv4),                    
                    sizeof(sm_nv_cached_values.cgps_1x_pde_server_addr_ipv4));     
                break;
      case NV_CGPS_1X_PDE_SERVER_PORT_I:
                memscpy((void*) &(nv_item_ptr->cgps_1x_pde_server_port), 
                    sizeof(nv_item_ptr->cgps_1x_pde_server_port),
                     (void*)&(sm_nv_cached_values.cgps_1x_pde_server_port),                  
                    sizeof(sm_nv_cached_values.cgps_1x_pde_server_port));          
                break;
      case NV_CGPS_1X_PDE_SERVER_ADDR_URL_I:
                memscpy((void*) &(nv_item_ptr->cgps_1x_pde_server_addr_url),
                    sizeof(nv_item_ptr->cgps_1x_pde_server_addr_url),
                     (void*)&(sm_nv_cached_values.cgps_1x_pde_server_addr_url),                   
                    sizeof(sm_nv_cached_values.cgps_1x_pde_server_addr_url));                
                break;
      case NV_CGPS_NMEA_CONFIG_INFO_I:
                memscpy((void*) &(nv_item_ptr->cgps_nmea_config_info),
                    sizeof(nv_item_ptr->cgps_nmea_config_info),
                       (void*)&(sm_nv_cached_values.cgps_nmea_config_info),                   
                    sizeof(sm_nv_cached_values.cgps_nmea_config_info));                        
                break;
      case NV_CGPS_UMTS_PDE_SERVER_PORT_I:
                memscpy((void*) &(nv_item_ptr->cgps_umts_pde_server_port), 
                    sizeof(nv_item_ptr->cgps_umts_pde_server_port),
                       (void*)&(sm_nv_cached_values.cgps_umts_pde_server_port),                  
                    sizeof(sm_nv_cached_values.cgps_umts_pde_server_port)); 
                break;
      case NV_GPS1_XTRA_DOWNLOAD_INTERVAL_I:
                memscpy((void*) &(nv_item_ptr->gps1_xtra_download_interval), 
                    sizeof(nv_item_ptr->gps1_xtra_download_interval),
                       (void*)&(sm_nv_cached_values.gps1_xtra_download_interval),                  
                    sizeof(sm_nv_cached_values.gps1_xtra_download_interval));
                break;
      case NV_GPS1_XTRA_AUTO_DOWNLOAD_ENABLED_I:
                memscpy((void*) &(nv_item_ptr->gps1_xtra_auto_download_enabled), 
                    sizeof(nv_item_ptr->gps1_xtra_auto_download_enabled),
                       (void*)&(sm_nv_cached_values.gps1_xtra_auto_download_enabled),                  
                    sizeof(sm_nv_cached_values.gps1_xtra_auto_download_enabled));
                break;
      case NV_CGPS_SBAS_USER_PREFERENCE_I:
                memscpy((void*) &(nv_item_ptr->cgps_sbas_user_preference), 
                    sizeof(nv_item_ptr->cgps_sbas_user_preference),
                       (void*)&(sm_nv_cached_values.cgps_sbas_user_preference),                  
                    sizeof(sm_nv_cached_values.cgps_sbas_user_preference));
                break;
      case NV_CGPS_1X_MPC_SERVER_ADDR_URL_I:
                memscpy((void*) &(nv_item_ptr->cgps_1x_mpc_server_addr_url), 
                    sizeof(nv_item_ptr->cgps_1x_mpc_server_addr_url),
                       (void*)&(sm_nv_cached_values.cgps_1x_mpc_server_addr_url),                  
                    sizeof(sm_nv_cached_values.cgps_1x_mpc_server_addr_url));
                break;
      case NV_AAGPS_MT_LRSUPPORT_I:
                memscpy((void*) &(nv_item_ptr->aagps_mt_lrsupport), 
                    sizeof(nv_item_ptr->aagps_mt_lrsupport),
                       (void*)&(sm_nv_cached_values.aagps_mt_lrsupport),                  
                    sizeof(sm_nv_cached_values.aagps_mt_lrsupport));
                break;     
      case NV_CGPS_MO_METHOD_I:
                memscpy((void*) &(nv_item_ptr->cgps_mo_method), 
                    sizeof(nv_item_ptr->cgps_mo_method),
                       (void*)&(sm_nv_cached_values.cgps_mo_method),                  
                    sizeof(sm_nv_cached_values.cgps_mo_method));
                break;  
      case NV_GPSONE_PASSWORD_I:
                memscpy((void*) &(nv_item_ptr->gpsone_password), 
                    sizeof(nv_item_ptr->gpsone_password),
                       (void*)&(sm_nv_cached_values.gpsone_password),                  
                    sizeof(sm_nv_cached_values.gpsone_password));
                break;     
      case NV_GPS1_SEC_UPDATE_RATE_I:
                memscpy((void*) &(nv_item_ptr->gps1_sec_update_rate), 
                    sizeof(nv_item_ptr->gps1_sec_update_rate),
                       (void*)&(sm_nv_cached_values.gps1_sec_update_rate),                  
                    sizeof(sm_nv_cached_values.gps1_sec_update_rate));
                break; 
      case NV_CGPS_ON_DEMAND_ENABLED_I:
                memscpy((void*) &(nv_item_ptr->cgps_on_demand_enabled), 
                    sizeof(nv_item_ptr->cgps_on_demand_enabled),
                       (void*)&(sm_nv_cached_values.cgps_on_demand_enabled),                  
                    sizeof(sm_nv_cached_values.cgps_on_demand_enabled));
                break;      
      case NV_AAGPS_2G_MO_LRSUPPORT_I:
                memscpy((void*) &(nv_item_ptr->aagps_2g_mo_lrsupport), 
                    sizeof(nv_item_ptr->aagps_2g_mo_lrsupport),
                       (void*)&(sm_nv_cached_values.aagps_2g_mo_lrsupport),                  
                    sizeof(sm_nv_cached_values.aagps_2g_mo_lrsupport));
                break;  
      case NV_AAGPS_3G_MO_LRSUPPORT_I:
                memscpy((void*) &(nv_item_ptr->aagps_3g_mo_lrsupport),
                    sizeof(nv_item_ptr->aagps_3g_mo_lrsupport),
                       (void*)&(sm_nv_cached_values.aagps_3g_mo_lrsupport),                  
                    sizeof(sm_nv_cached_values.aagps_3g_mo_lrsupport));
                break;              

      /* Below are for carrier-specific NVs (one copy per sub) */
      case NV_GPS1_LOCK_I:
                memscpy((void*) &(nv_item_ptr->gps1_lock),
                    sizeof(nv_item_ptr->gps1_lock),
                      (void*)&(sm_nv_cached_values.gps1_lock[sub]),
                    sizeof(sm_nv_cached_values.gps1_lock[sub]));
                break;
      case NV_GNSS_NMEA_SENTENCE_TYPE_I:
                memscpy((void*) &(nv_item_ptr->gnss_nmea_sentence_type), 
                     sizeof(nv_item_ptr->gnss_nmea_sentence_type),
                       (void*)&(sm_nv_cached_values.gnss_nmea_sentence_type[sub]),                  
                    sizeof(sm_nv_cached_values.gnss_nmea_sentence_type[sub]));
                break;
      case NV_GNSS_NMEA_EXTENDED_SENTENCE_TYPE_I:
                memscpy((void*) &(nv_item_ptr->gnss_nmea_extended_sentence_type), 
                    sizeof(nv_item_ptr->gnss_nmea_extended_sentence_type),
                       (void*)&(sm_nv_cached_values.gnss_nmea_extended_sentence_type[sub]),                   
                    sizeof(sm_nv_cached_values.gnss_nmea_extended_sentence_type[sub]));     
                break;
        case NV_CGPS_UMTS_PDE_SERVER_ADDR_IPV4_I:
                  memscpy((void*) &(nv_item_ptr->cgps_umts_pde_server_addr_ipv4),
                      sizeof(nv_item_ptr->cgps_umts_pde_server_addr_ipv4),
                         (void*)&(sm_nv_cached_values.cgps_umts_pde_server_addr_ipv4[sub]),                   
                      sizeof(sm_nv_cached_values.cgps_umts_pde_server_addr_ipv4[sub])); 
                  break;
      case NV_CGPS_UMTS_PDE_SERVER_ADDR_IPV6_I:
        memscpy((void *)&(nv_item_ptr->cgps_umts_pde_server_addr_ipv6),
                sizeof(nv_item_ptr->cgps_umts_pde_server_addr_ipv6),
                (void *)&(sm_nv_cached_values.cgps_umts_pde_server_addr_ipv6[sub]),
                sizeof(sm_nv_cached_values.cgps_umts_pde_server_addr_ipv6[sub]));
        break;

      case NV_CGPS_UMTS_PDE_SERVER_ADDR_URL_I:
                memscpy((void*) &(nv_item_ptr->cgps_umts_pde_server_addr_url),
                    sizeof(nv_item_ptr->cgps_umts_pde_server_addr_url),
                       (void*)&(sm_nv_cached_values.cgps_umts_pde_server_addr_url[sub]),                   
                    sizeof(sm_nv_cached_values.cgps_umts_pde_server_addr_url[sub]));
                break;
      case NV_GNSS_SUPL_VERSION_I:
                memscpy((void*) &(nv_item_ptr->gnss_supl_version), 
                       sizeof(nv_item_ptr->gnss_supl_version),
                       (void*)&(sm_nv_cached_values.gnss_supl_version[sub]),
                       sizeof(sm_nv_cached_values.gnss_supl_version[sub]));
                break;                
      case NV_AAGPS_USE_TRANSPORT_SECURITY_I:
                memscpy((void*) &(nv_item_ptr->aagps_use_transport_security), 
                    sizeof(nv_item_ptr->aagps_use_transport_security),
                       (void*)&(sm_nv_cached_values.aagps_use_transport_security[sub]),                  
                    sizeof(sm_nv_cached_values.aagps_use_transport_security[sub]));
                break;
      case NV_AAGPS_POSITIONING_MODES_SUPPORTED_I:
                memscpy((void*) &(nv_item_ptr->aagps_positioning_modes_supported), 
                    sizeof(nv_item_ptr->aagps_positioning_modes_supported),
                       (void*)&(sm_nv_cached_values.aagps_positioning_modes_supported[sub]),                  
                    sizeof(sm_nv_cached_values.aagps_positioning_modes_supported[sub]));
                break;              
      case NV_CGPS_1X_MPC_SERVER_ADDR_IPV4_I:
                memscpy((void*) &(nv_item_ptr->cgps_1x_mpc_server_addr_ipv4), 
                    sizeof(nv_item_ptr->cgps_1x_mpc_server_addr_ipv4),
                       (void*)&(sm_nv_cached_values.cgps_1x_mpc_server_addr_ipv4[sub]),                  
                    sizeof(sm_nv_cached_values.cgps_1x_mpc_server_addr_ipv4[sub]));
                break;              
      case NV_CGPS_1X_MPC_SERVER_PORT_I:
                memscpy((void*) &(nv_item_ptr->cgps_1x_mpc_server_port), 
                    sizeof(nv_item_ptr->cgps_1x_mpc_server_port),
                       (void*)&(sm_nv_cached_values.cgps_1x_mpc_server_port[sub]),                  
                    sizeof(sm_nv_cached_values.cgps_1x_mpc_server_port[sub]));
                break;  
      case NV_GPS1_VX_APP_TRUSTED_SETTINGS_I:
                memscpy((void *)&(nv_item_ptr->gps1_vx_app_trusted_settings),
                        sizeof(nv_item_ptr->gps1_vx_app_trusted_settings),
                        (void *)&(sm_nv_cached_values.gps1_vx_app_trusted_settings[sub]),
                        sizeof(sm_nv_cached_values.gps1_vx_app_trusted_settings[sub]));
                break;
      case NV_GPS1_VX_LCS_AGENT_I:
                memscpy((void*) &(nv_item_ptr->gps1_vx_lcs_agent), 
                    sizeof(nv_item_ptr->gps1_vx_lcs_agent),
                       (void*)&(sm_nv_cached_values.gps1_vx_lcs_agent[sub]),                  
                    sizeof(sm_nv_cached_values.gps1_vx_lcs_agent[sub]));
                break;
      case NV_GPS1_VX_NI_TELESERVICE_ID_I:
                memscpy((void*) &(nv_item_ptr->gps1_vx_ni_teleservice_id), 
                    sizeof(nv_item_ptr->gps1_vx_ni_teleservice_id),
                       (void*)&(sm_nv_cached_values.gps1_vx_ni_teleservice_id[sub]),                  
                    sizeof(sm_nv_cached_values.gps1_vx_ni_teleservice_id[sub]));
                break;
      case NV_GPS1_VX_MO_MAX_DURATION_I:
                memscpy((void*) &(nv_item_ptr->gps1_vx_mo_max_duration), 
                    sizeof(nv_item_ptr->gps1_vx_mo_max_duration),
                       (void*)&(sm_nv_cached_values.gps1_vx_mo_max_duration[sub]),                  
                    sizeof(sm_nv_cached_values.gps1_vx_mo_max_duration[sub]));
                break;
...
FUNCTION sm_nv_sync_read_from_hw
...
===========================================================================*/
static boolean sm_nv_sync_read_from_hw(nv_items_enum_type nv_item, void *item_ptr)
{
   ...
   */
  cmd_ptr = (nv_cmd_type*)os_MemAlloc(sizeof(nv_cmd_type), OS_MEM_SCOPE_TASK);
...
FUNCTION sm_nv_sync_read_from_hw_sub
...
===========================================================================*/
boolean sm_nv_sync_read_from_hw_sub(nv_items_enum_type nv_item, void *item_ptr, sys_modem_as_id_e_type sub)
{
...
FUNCTION sm_nv_read_from_hw_sub_and_cache

DESCRIPTION
  Read from HW for a particular sub and cache return value 
...
===========================================================================*/
boolean sm_nv_read_from_hw_sub_and_cache (nv_items_enum_type nv_item, void* item_ptr,sys_modem_as_id_e_type sub)
{
  /* Since NV read and writes should normally be done in GPS idle state
   * (no sessions), no separate allocation of space is memory is done.
   * The buffers will be used from a dynamic buffer pool.
  ...
  if (item_ptr == NULL)
  {
    ...
    /* sub-dedepdent NV */
  ret_val = sm_nv_sync_read_from_hw_sub(nv_item, item_ptr, sub);

  MSG_3( MSG_SSID_GPSSM, MSG_LEGACY_MED, "SM NV Hw SUB Read: %d (sub=%d) ret=%d", nv_item, sub, ret_val);
...
boolean sm_nv_read (nv_items_enum_type nv_item, void* item_ptr)
{
  ...
  if (sub == SM_CONFIG_MIN_SUB_ID)
  {
    ...
    ret_val = sm_nv_sync_read_from_hw(nv_item, item_ptr);
  }
  else
  {
    /* sub-dedepdent NV */
...
FUNCTION   sm_nv_async_rdrw_done
...
boolean sm_nv_async_rdrw_done(void)
{
  ...
  else
  {
    ...
    if (q_cnt(&sm_nv_cmd_q) != 0)
    {
      ...
      MSG(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "SM_NV_RDRW: dequeue/process next NV");
      sm_nv_handle_async_rdwr_cmd();
    }
  }
...
boolean sm_nv_async_read (nv_items_enum_type nv_item, const nv_item_type* item_ptr, uint32 handle)
{
  ...
  if (sm_nv_initialized)
  {
     ...
     * The buffers will be used from a dynamic buffer pool.
     */
    nv_ptr = (sm_nv_item_s_type *)os_MemAlloc(sizeof(sm_nv_item_s_type), OS_MEM_SCOPE_TASK);
