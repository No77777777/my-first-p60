...
/*=============================================================================
  ...
  $Header: //components/rel/gnss8.mpss/11.1/gnss/sm/lm/inc/lm_api.h#9 $ 

when       who     what, where, why
--------   ---     ----------------------------------------------------------
08/26/20  jeev   Fix stack exhaustion while inject aiding data
06/02/20   mj    Handle E911 CPI fix qualification when E911 switches to main
01/28/20   anksha  Added support to send Navic assist data to TM->LM
01/14/19   mj      SV filter support for E911 MSA main client during concurrency
11/07/16   rn      Added Xtra Integrity support.
02/23/16   mj      Support for GNSS Engine Monitoring
...
typedef enum {
  LM_LPPM_MODE_NONE = -1,
  LM_LPPM_INVALID_MODE,
  LM_NON_LPPM_IMPROVED_ACCURACY_MODE, /* M1: Invalid LPPM Mode*/
  LM_LPPM_FOREGROUND_FIXED_TIME,      /* M2: Fixed Time LPPM Mode*/
  LM_LPPM_BACKGROUND_FIXED_POWER,     /* M3: Background Fixed Power LPPM Mode */
  LM_LPPM_BACKGROUND_FIXED_TIME,      /* M4: Background Fixed Time LPPM Mode */
  LM_LPPM_BACKGROUND_KEEP_WARM,       /* M5: Background Keep Warm LPPM Mode */
  LM_LPPM_MAX_MODE = LM_LPPM_BACKGROUND_KEEP_WARM
} lm_lppm_mode_e_type;

typedef struct
{
  lm_lppm_mode_e_type e_lppm_mode_type; /*LPPM Mode*/
  uint32               q_TbmMsecs;  /*Time Between Measurements in msec */
} lm_session_request_lppm_s_type;
...
typedef struct {
  ...
  boolean           v_no_force_idle;
  /* Indicates if FORCE IDLE should be performed 
        before the LM session is started. Used to skip FORCE IDLE
        when the E911 sessions moves from E911 concurrent to
        non-concurrent state */
  lm_session_request_lppm_s_type z_lppm_config;
  boolean           v_aa_not_needed;
  /* AA not needed for MSA session after switching to main client in LM */
...
typedef struct {
  ...
  /*LM_SESSION_REQUEST_SESSION and
    (LM_SESSION_REQUEST_START or LM_SESSION_REQUEST_CONTINUE)*/
  lm_session_request_lppm_s_type    z_lppm_request;
...
typedef enum {
  ...
  LM_SESSION_REQUEST_LPPM                 =   0x40 /*stop/start/continue LPPM*/
...
typedef enum
{
  ...
  LM_SET_PARAM_GNSS_CONSTELLATION_CONFIG
...
typedef union
{
  ...
  pdsm_constellation_control_u_type z_ConstellConfig;
...
typedef enum
{
  LM_MGP_GERA_RCVR_OFF_REASON_SHUTOFF_TIMER, /* Turn OFF RCVR due to shutoff timer pop for autotrack*/
  LM_MGP_GERA_RCVR_OFF_REASON_FIX_END,       /* Turn OFF RCVR due to fix end */
  LM_MGP_RCVR_IDLE_REASON_RC_ON_TIMEOUT,     /* Turn IDLE RCVR due to RC_ON timer timeout */
  LM_MGP_RCVR_IDLE_REASON_SESS_END,          /* Turn IDLE RCVR due to session end */
  LM_MGP_RCVR_OFF_REASON_SESS_END,           /* Turn OFF RCVR due to session end */
  LM_MGP_RCVR_OFF_TM_SESS_END        /* Turn OFF RCVR due to TM SESS DONE during concurrency */
}lm_gera_rcvr_mode_change_reason_e_type;

typedef struct
{
  boolean v_force_mgp_off;
  lm_gera_rcvr_mode_change_reason_e_type e_rcvr_off_reason;
} lm_gera_rcvr_off_s_type;

typedef struct
{
  boolean                  v_bestFixValid;
  sm_FixInfoStructType     z_sessBestFixRpt;
  boolean                  v_gnssFixReceivedThisSession;
  boolean                  v_reflocReceived;
}lm_request_update_e911_fix_qual_type;

typedef enum
{
  LM_LDM_E911_CLIENT,
  LM_LDM_APPTRACK_CLIENT,
  LM_LDM_SINGLE_SHOT_CLIENT
}lm_ldm_client_disable_type;

typedef struct
{
  lm_ldm_client_disable_type e_lm_ldm_client_id;
  lm_gera_rcvr_mode_change_reason_e_type e_rcvr_off_reason;
  boolean u_forceMgpOff;
} tm_ldm_client_disable_s_type;

typedef enum
{
  LM_GNSS_FIX_MONITOR,
  LM_GNSS_MEAS_MONITOR,
} lm_gnss_engine_monitor_e_type;
...
typedef enum {
  ...
  LM_REQUEST_LPPM_STATUS,
  LM_REQUEST_GERA_RECVR_OFF,
  LM_REQUEST_LDM_RECVR_CHANGE,
  LM_REQUEST_SET_ENGINE_MONITOR_CONFIG,
  LM_REQUEST_GET_ENGINE_MONITOR_CONFIG,
  LM_REQUEST_UPDATE_E911_FIX_QUALIFICATION,
} lm_request_e_type;
...
typedef union {
  ...
  /*LM_REQUEST_LPPM_STATUS*/
  lm_session_request_lppm_s_type     z_lppm_status_request;
  /* LM_REQUEST_GERA_RCVR_OFF_REASON */
  lm_gera_rcvr_off_s_type           e_gera_rcvr_off;
  boolean                           v_engine_monitor_enabled;
  /* LM_REQUEST_UPDATE_E911_FIX_QUALIFICATION */
  lm_request_update_e911_fix_qual_type *pz_e911_fix_qual;
}lm_request_u_type;
...
typedef union {
  ...
  /*LM_SESSION_INFO_CONCURR_UPDATE */
  boolean                         v_concurrent_e911_ongoing;
}lm_info_u_type;
...
/* Prescribed dwell command version 54.
 * This version is used with GNSS Engine rev 56. */
typedef PACKED struct PACKED_POST
{
  uint8   u_Version;                 /* Set to 54 for this type */
  uint8   u_CmdControlFlag;
  uint32  q_SeqNum;                  /* Seq number from cmd */
...
#define C_GNSS_LM_MULTICHAN_TRACK_VER57 57
/* Supports L5,E5A,B2A, NAVIC in addition to Band1 */
typedef PACKED struct PACKED_POST
{
  uint8 u_Version;
  ...
  uint8 u_BDS_B2A_SV_ID;            /* BDS B2A satellite PRN [0..37] */
  uint8 u_BDS_B2A_BitEdge_Setting;  /* Disables bit-edge detection for BDS B2A */
  uint8 u_NAVIC_SV_ID;              /* NAVIC satellite PRN [0..14] */
  uint8 u_NAVIC_BitEdge_Setting;    /* Disables bit-edge detection for NAVIC */
} lm_multichan_track_type;

#define C_GNSS_LM_MULTICHAN_TRACK_VER56 56
/* Version 56 supports L5/E5 in additiont to GPS, BDS, GLO, GAL Band1*/
typedef PACKED struct PACKED_POST
{
  uint8 u_Version;
  uint8 u_Test_Mode;            /* See gnss_RfDevTrackEnumType */
...
#define C_GNSS_LM_MULTICHAN_TRACK_CUR_VER C_GNSS_LM_MULTICHAN_TRACK_VER57
...
typedef PACKED struct PACKED_POST
{
  ...
  uint8   u_GpsL5JobsEnabled;
  uint8   u_NavicJobsEnabled;
  uint8   u_GalE5bJobsEnabled;
} lm_SpectrumAnalyzerCmdType;

typedef PACKED struct PACKED_POST
{
  boolean v_PfaTestRequested;
} lm_PfaTestType;
 ...
 ******************************************************************************
*/
void lm_mode_switch(cgps_sarf_mode_switch_enum_type e_sarf_state);

/*
 ******************************************************************************
 * lm_send_xtra_navic_assist_data
 *
 * Function description:
 *   Send XTRA Navic ephemeris assist IPC to LM
 *
 * Parameters: 
 * gnss_AssistDataInfoStructType *p_xtraNavicAssistData     - ptr to Xtra Navic Assist
 ...
 ******************************************************************************
*/
void lm_send_xtra_navic_assist_data( const gnss_AssistDataInfoStructType *p_xtraNavicAssistData );

/*
 **********************************************************************************************
 * lm_send_xtra_Control_record_data
 *
 * Function description:
 *   Send XTRA Control Record (navic assist Enable/Disable ) IPC to LM
 *
 * Parameters: 
 * uint16  w_xtraControlRecordVal     - ptr to Xtra Navic Assist
 *                                      structure holding ephemeris
 *                                      records.
 * Return value: 
 * None
 *
 ***********************************************************************************************
*/
void lm_send_xtra_Control_record_data( const uint16 w_xtraControlRecordVal );
