...
/*===========================================================================
                           ...
                           Edit History
  $Header: //components/rel/gnss8.mpss/11.1/gnss/sm/tm/is801/src/tm_is801_util.c#3 $  
...
)
{
  ...
  MSG_4(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"GpsPRM Time %d %d %d %d", 
        input_prm_buf_ptr->z_MeasBlk.z_GnssHeader.z_MeasClock.z_Time.z_GpsTime.w_GpsWeek, 
	input_prm_buf_ptr->z_MeasBlk.z_GnssHeader.z_MeasClock.z_Time.z_GpsTime.q_GpsMsec, 
	(uint32 )(input_prm_buf_ptr->z_MeasBlk.z_GnssHeader.z_MeasClock.z_Time.z_GpsTime.f_ClkTimeBias * 1000), 
	(uint32 )(input_prm_buf_ptr->z_MeasBlk.z_GnssHeader.z_MeasClock.z_Time.z_GpsTime.f_ClkTimeUncMs * 1000));
  
  
  gps_to_cdma_time(&input_prm_buf_ptr->z_MeasBlk.z_GnssHeader.z_MeasClock.z_Time.z_GpsTime, 
                   cdma_timestamp);
  ...
  if (input_ppm_buf_ptr && input_ppm_buf_ptr->num_pilots)
  {
    ...
    q_CDMARtcAtTTCCx8 = input_prm_buf_ptr->z_MeasBlk.z_GnssHeader.z_CDMAClockInfo.q_CdmaRtcSync80CCx8 % FULL_CDMA_PN_CIRCLE_CCX8;
    ...
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"CDMA RtcDiff %d (CCx16), RTC at TT 0x%x (CCx8), RTC from PPM 0x%x (CCx8)",
             l_CDMARtcDiffCx16,
             input_prm_buf_ptr->z_MeasBlk.z_GnssHeader.z_CDMAClockInfo.q_CdmaRtcSync80CCx8,
    ...
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Total adjustment %d (GCx1024), GNSSRTCatSync80 %d (GCx1280)",
            l_AdjustmentGCx1024,
            input_prm_buf_ptr->z_MeasBlk.z_GnssHeader.z_CDMAClockInfo.q_GpsRtcSubMsSync80GCx32, 0);
    /* Convert into MS */
    f_AdjustmentMs = (FLT)l_AdjustmentGCx1024 * (1.0f/(1024.0f*1023.0f));

    /* Callisto engine has a gnss rtc that is chipx1280 */
    f_AdjustmentMs -=
      (FLT) input_prm_buf_ptr->z_MeasBlk.z_GnssHeader.z_CDMAClockInfo.q_GpsRtcSubMsSync80GCx32 *
    ...
    l_TempCdmaRtcLatchedCCx32 = (int32) input_prm_buf_ptr->z_MeasBlk.z_GnssHeader.z_CDMAClockInfo.q_CdmaRtcLatchedCCx32;

    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"AdjustmentMs %d (GCx1024), VSRCDiff %d",
            (int32)(f_AdjustmentMs * 1023.0f * 1024.0f),
            input_prm_buf_ptr->z_MeasBlk.z_GnssHeader.z_CDMAClockInfo.q_CdmaRtcLatchedCCx32, 0);
    ...
    for ( i = 0; i < input_prm_buf_ptr->z_MeasBlk.z_GnssHeader.u_NumSvs; i++ )
    {
  ...
  */
  ...
  for ( i=0; i < input_prm_buf_ptr->z_MeasBlk.z_GnssHeader.u_NumSvs && i < N_ACTIVE_GPS_CHAN; i++ )
  {
       ...
       )
    {
      is801_util_gps_meas_info_to_is801_prm( &output_prm_buf_ptr->var[output_prm_buf_ptr->fix.num_ps_ranges_p++], 
                                             ...
                                             input_prm_buf_ptr->z_MeasBlk.z_GnssHeader.z_MeasClock.z_Freq.f_ClkFreqBias);
    }
  ...
  if( TRUE == *z_FtcalData.p_FTCalEnabled )
  {
    ...
    if( input_prm_buf_ptr->z_MeasBlk.z_GnssHeader.u_NumSvs != 1 )
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"F-TCal bad num of SVs reported %d",
                input_prm_buf_ptr->z_MeasBlk.z_GnssHeader.u_NumSvs, 0, 0);
    ...
    if( z_FTCal.u_ErrorCode == 0 )
    {
      ...
      z_FTCalGps.f_GpsSpeedMperSec      = input_prm_buf_ptr->z_MeasBlk.z_GnssHeader.z_MeasClock.z_Freq.f_ClkFreqBias - p_GpsMeas->z_FiltMeas.f_SvSpeed;
      ...
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"F-TCal GPS C/No %d Latency %dms Speed %dm/s",
               ...
               (int32)(input_prm_buf_ptr->z_MeasBlk.z_GnssHeader.z_MeasClock.z_Freq.f_ClkFreqBias -
...
)
{
  ...
  */
  ...
  else
  {
    ...
    */
    f_scale_up_to_99pct_conf = tm_util_get_cep_scale_ratio_from_pct( (FLT)LOC_UNC_CONFIDENCE/100.0,0.99f);
  ...
  if (pddm_rec_hdr.num_requests > 0)
  {
    for (i = 0; i < pddm_rec_hdr.num_requests; i++)
    {
      ...
      if((buffer_pos < length) && 
         paix_xlate_ext_req_header(&(start[buffer_pos]), &pddm_req_hdr, (length - buffer_pos)))
      {
