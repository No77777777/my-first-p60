...
/*===========================================================================
  ...
  $Header: //components/rel/gnss8.mpss/11.1/gnss/sm/tm/umts_up/inc/tm_umts_up_supl.h#3 $
  $DateTime: 2023/01/19 23:42:25 $
  $Author: pwbldsvc $
  ...
  06/21/16    gk      Reduce the max num of GF to 1 from 16 to reduce memory  
...
/* Maximum (or last) SUPL timer id */
#define TM_SUPL_TIMER_ID_MAX     TM_UMTS_UP_TIMER_ID_RANGE + 0x000000FF

/* 
*  UMTS UP timer id ranges are sub-divided into two categories:
*  1. SUPL timer ids
*  2. LPP UP timer ids. 
*  SUPL Timer id range from TM_UMTS_UP_TIMER_ID_RANGE to TM_SUPL_TIMER_ID_MAX.
*  SUPL can have maximum of 255 timer ids. Any intend to increase SUPL timer id beyond
*  255 affect LPP UP timer id. Hence the LPP_UP_TIMER_ID_START_RANGE & LPP UP
*  dispatch mask (i.e. TM_LPP_UP_TIMER_ID_DISPATCH_MASK) should be adjusted accordingly.
*/
...
#define SUPL_GLO_ALM_REQ_HD_TIMER_ID          (SUPL_CF_TIMER_ID + 11) 
#define SUPL_GLO_EPH_REQ_HD_TIMER_ID          (SUPL_CF_TIMER_ID + 12)
#define SUPL_WIFI_WAIT_TIMER_ID               (SUPL_CF_TIMER_ID + 13)
#define SUPL_GM_REQ_HD_TIMER_ID               (SUPL_CF_TIMER_ID + 14)
#define SUPL_LTE_CELLINFO_RETRY_TIMER_ID      (SUPL_CF_TIMER_ID + 15)
#define SUPL_BDS_ALM_REQ_HD_TIMER_ID          (SUPL_CF_TIMER_ID + 16)
#define SUPL_BDS_EPH_REQ_HD_TIMER_ID          (SUPL_CF_TIMER_ID + 17)
#define SUPL_BDS_REFTIME_REQ_HD_TIMER_ID      (SUPL_CF_TIMER_ID + 18)
#define SUPL_BDS_REFLOC_REQ_HD_TIMER_ID       (SUPL_CF_TIMER_ID + 19)
#define SUPL_BDS_RTI_REQ_HD_TIMER_ID          (SUPL_CF_TIMER_ID + 20)
#define SUPL_BDS_TIMEMODEL_REQ_HD_TIMER_ID    (SUPL_CF_TIMER_ID + 21)  /* Caution: No more than 255 */
	
/* Define the starting range of LPP UP timer id. */
#define LPP_UP_TIMER_ID_START_RANGE    (TM_SUPL_TIMER_ID_MAX + 1)
...
#define SUPL_RRLP_CURRENT_VERSION_MAJOR 13  /* current ASN1's major */
#define SUPL_RRLP_CURRENT_VERSION_TECH   0  /* current ASN1's technical */
#define SUPL_RRLP_CURRENT_VERSION_EDIT   0  /* current ASN1's editorial */
...
#define C_SUPL2_GANSS_ID_GLONASS 4
#define C_SUPL2_GANSS_ID_BDS     5
#define OTHER_SETID_SIZE (sizeof(supl_SETId) - 4)

#define C_SUPL2_GANSS_SIGNAL_ID_MASK_BDS_BL1  (0x80)
typedef struct 
{
  uint8 valid;
...
typedef struct
{
  uint32 q_gps_session_timeout_value_msec; /* LM & MGP session timeout */
...
typedef enum  
{
  ...
  C_SUPL_CF_STATE_SUPLPOS_XCHG = 0x0080,    /* SUPL_POS messages are exchanged in this state. The SUPL_POS
                                                  may contains measurements(LPP/RRLP GNSS+OTDOA) positions, response
                                                  (LPP ProvCap) or acknowledgement data(RRLP ACK). */
  C_SUPL_CF_STATE_CELL_INFO_REQ = 0x0100,           /* wait for RRC to provide cell info. and move to
start SLP contact for non-areaEvent upon receiving cell info. or timeout */
  ...
  C_SUPL_CF_STATE_WIFI_WAIT_SUPLPOSINIT =    0x1000000,  /* continue waiting for WiFi state without timer; send SUPL_POSINIT once Wifi available  */
  ...
  C_SUPL_CF_STATE_LTE_CELLID_RETRY_TIMER_SUPLSTART  =  0x4000000,  /* wait to retry  LTE camped cell info  get and move to SUPL START TX upon success
                                                                  or timeout. This sub-state is entered from one of these main states C_SUPL_CF_STATE_UT0_SUPLSTART, 
                                                                  C_SUPL_CF_STATE_WIFI_WAIT_SUPLSTART, C_SUPL_CF_STATE_WIFI_WAIT_TIMER_SUPLSTART
                                                                  */
  C_SUPL_CF_STATE_LTE_CELLID_RETRY_TIMER_SUPLPOSINIT  =  0x8000000, /* wait to retry  LTE camped cell info  get and move to SUPL_POS_INIT TX upon success
                                                                  or timeout. This sub-state is entered from one of these main states C_SUPL_CF_STATE_UT0_SUPLPOSINIT
                                                                  C_SUPL_CF_STATE_WIFI_WAIT_SUPLPOSINIT,C_SUPL_CF_STATE_WIFI_WAIT_TIMER_SUPLPOSINIT or during supl_reponse_proc*/
...
typedef struct
{
  ...
  /* TRUE if lpp has reported an AGNSS error to the HSLP */
  boolean   u_agnss_error_reported;   
    
  boolean   u_supl_posMethod_ecid_valid;
    
  /* used when a position is reported to pdapi prior to completion of the 
     network call flow */
  tm_umts_common_pos_for_net_s_type      z_pending_new_pos_to_network;
  ...
  * True - last supl msg sent-out sucessfully by SSL & ack is received. False - Ack isn't received. */
  ...
  /* Pointer to WLAN AP Measurements. If this pointer is not NULL, 
   * then WiFi data is VALID. */
  pdsm_wifi_on_demand_info_list_s_type  *p_wifi_ap_meas;
  
  /*Is Session TCP connection Pending*/
  boolean                             u_tcp_connection_attempt_pending;
#ifdef FEATURE_GNSS_NON_STANDARD_SUPPORT
  tm_sess_stop_reason_e_type          u_supl_end_reason;
  ...
  boolean                             u_slp_support_gps;
  boolean                             u_slp_support_glonass;
  boolean                             u_slp_support_bds;

  q_type                              supl_msg_q;
  boolean                             supl_msg_q_status; /*Active or inactive*/
  boolean                             v_privacy_override_set; /* TRUE if privacy override is set for this session */  
  boolean                             v_send_no_more_location_notifications; /* Dont send any more notifications for failure */
  sys_modem_as_id_e_type              session_sub_id; 
...
#define C_MAX_GEOFENCE_NUM 2  /* num. the phone will operate with */
...
extern tm_umts_common_pos_for_net_s_type    z_supl_cached_pos_for_net;
extern os_TimerMsgType *supl_cf_timer;
extern os_TimerMsgType *wcdma_l1_mrl_req_timer;
...
extern os_TimerMsgType *supl_lte_get_cell_info_retry_timer;
...
extern os_TimerMsgType*          supl_bds_alm_req_holdoff_timer;
extern os_TimerMsgType*          supl_bds_eph_req_holdoff_timer;
extern os_TimerMsgType*          supl_bds_reftime_req_holdoff_timer;
extern os_TimerMsgType*          supl_bds_refloc_req_holdoff_timer;
extern os_TimerMsgType*          supl_bds_rti_req_holdoff_timer;
extern os_TimerMsgType*          supl_bds_timemodel_req_holdoff_timer;
...
boolean tm_umts_up_supl_suplpos_rrlp_mpr_err_tx (supl_status_s_type *p_supl_session_status, rrlp_enc_LocErrorReason e_error_reason);
...
boolean supl_lte_cell_information_construct(void);
...
void supl_set_cf_state( supl_cf_state_enum_type e_cf_state, uint32 const cq_TimeoutMsec );
