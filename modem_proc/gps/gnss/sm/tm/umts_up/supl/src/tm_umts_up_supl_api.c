...
/*===========================================================================
  ...
  $Header: //components/rel/gnss8.mpss/11.1/gnss/sm/tm/umts_up/supl/src/tm_umts_up_supl_api.c#10 $
  $DateTime: 2020/08/28 04:03:18 $
  $Author: pwbldsvc $

  when       who     what, where, why
  --------   ---     ----------------------------------------------------------
  07/21/20   skm     Ensure Measurements are sent out at RLI timeout and not at PRM/Fix timeout.
  ...
  09/30/19   mj      Allow SUPL session during E911, even with ES bit is set to FALSE
  ...
  12/20/18   py      Avoid sending two SUPL ENDs in a session.
  11/30/18   skm     911 VoWIFI IoT fix for NO_SRV on WIFI
  11/08/18   py      Report cached RRLP SUPL measurement on timer expiry
  03/20/18   skm     Supl E911 over Wifi
  08/31/17   ak       Add changes to retry LTE cell ID retrival if it fails for first time.
  03/21/17   gk      UE should not send pos/meas after 911 call end.
  ...
  07/05/16   rk      Pass all measurements to protocols. It will decide which measurements should be
                     reported to server.
  05/26/16   rk      Ignore the LM PRM & LR timeout when SUPL state is in OTDOA request measurement.
...
static boolean supl_is_waiting_for_tcp_to_send_breach(void)
{
  ...
  for (u_i = 0; u_i < C_MAX_GEOFENCE_NUM; u_i++)
  {
    if (z_supl_area_event_session_status[u_i].u_session_active)
    {
      ...
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "Breach Report Pending", 0, 0, 0);
        return TRUE;
      }
    }
  }
   ...
   )
{
  ...
  if (NULL == start_sess_req_param ||
      NULL == actions)
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR, "TM_UMTS_SUPL: INVALID PARAMETER", 0, 0, 0);
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "TM_UMTS_SUPL: SUPL NOT ENABLED", 0, 0, 0);
    return PRTL_START_SESS_REQ_NOT_ALLOWED;
  }

  p_supl_session_status->tm_active_session_handle = sess_handle;

  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "SUPL: start_sess_req_handler. PosMethodType %d", p_supl_session_status->u_agps_mode, 0, 0);

  /* Modify this if condition above to use emergency serving system also*/
  /* MO SUPL is not allowed when UE is on WLAN*/
  ...
  {
    MSG(MSG_SSID_GPSSM, MSG_LEGACY_ERROR, "TM_UMTS_SUPL: SUPL MO not allowed on WLAN");
    return PRTL_START_SESS_REQ_NOT_ALLOWED;
  }
  ...
  { /* MO session*/
    ...
    if((phone_state_info.e911_state != TM_CM_E911_STATE_NONE) && (phone_state_info.emerg_asubs_id != tm_cm_dsds_get_dd_subs()))
    {
      /* Emergency MO session on non-DD sub */
      if(0 == TM_CORE_FEATURE_BIT_IS_SET(TM_SUPL_ON_NDDS_ENABLE))
      {
        MSG_1(MSG_SSID_GPSSM, MSG_LEGACY_ERROR, "MO SUPL on nDDS not allowed by NV. NV value= %d",
                    tm_core_info.config_info.subs_cfgs[phone_state_info.emerg_asubs_id].q_agps_feature_support);
        return PRTL_START_SESS_REQ_NOT_ALLOWED;
      }
    ...
    p_supl_session_status->session_sub_id = tm_core_get_current_dd_subs_id();
    }
  }
  ...
  if (p_supl_session_status->u_callflow_kind == C_SUPL_SESSION_NI)
  {
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "TM_UMTS_SUPL: Emergency SUPL INIT: Allow the session", 0, 0, 0);
    }
    else if (((z_umts_nv_item_ram_copy.q_gnss_lock_ctrl_for_oem)& PDSM_PA_SUPL_NI_OEM_CTRL)!= 0)
    {
  ...
  if (tm_umts_up_supl_pdapi_mapping(start_sess_req_param))
  {
    ...
    if (p_supl_session_status->u_callflow_kind == C_SUPL_SESSION_NONE)
    {
      ...
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR, "SUPL: serving system undef", 0, 0, 0);
        return PRTL_START_SESS_REQ_NOT_ALLOWED;
      }
          ...
          )
      {
        ...
        {
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "IS801 did not allow the UI session", 0, 0, 0);
          return PRTL_START_SESS_REQ_NOT_ALLOWED;
        }
      }
    }
             ...
             )
    {
      ...
      /* 1) WCDMA cell info check is done here, if no cell info available
            we request info explicitly with tm_umts_up_supl_req_cell_info_from_rrc().
         2) If we are not camped on WCDMA, then we should be on
            a)LTE cell info is processed at a later stage, in the call to
              tm_umts_up_supl_start_slp_contact().
            b)TDSCDMA RAT and cell info is checked/processed in
              tm_umts_up_supl_ck_req_tdscmda_cell_info().
            c)GSM RAT is not checked here and the GSM cell info is directly
              procured and encoded in supl_cell_info_construct.
      */
    ...
    else if (p_supl_session_status->u_agps_mode == C_SUPL_AGPS_MODE_AUTO)
    {
      ...
      {
        actions->lr_qos = (p_supl_session_status->z_session_qos.q_gps_session_timeout_value_msec / 1000);
      }
    ...
    {
      actions->op_req = TM_OP_REQ_NONE;
      actions->prm_qos = (p_supl_session_status->z_session_qos.q_gps_session_timeout_value_msec / 1000); /* pdapi's max: 255  */
      actions->accuracy_threshold = p_supl_session_status->z_session_qos.q_hor_acc_meters;
  ...
  if (ret_val == PRTL_START_SESS_REQ_ALLOWED)
  {
    ...
    if (phone_state_info.srv_system == CGPS_SRV_SYS_LTE)
    {
      ...
      */
      lte_ml1_subscribe_gps_mrl(p_supl_session_status->session_sub_id, TRUE);
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "SUPL: TmCore startSession denied", 0, 0, 0);
  }
   ...
   )
{
  boolean u_ret_val      = FALSE,
     v_rpt_rrlp_meas = FALSE; /* Reported RRLP measurements to server */
  ...
  if (sess_handle == p_supl_session_status->tm_active_session_handle)
  {
    ...
    else if (stop_reason == TM_STOP_REASON_ON_RESET_LOCATION_SERVICE)
    {
      ...
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "EndSession: Clear assist req. holdoff times", 0, 0, 0);
    ...
    if (u_ret_val == TRUE)
    {
      ...
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "Supl %d state 0x%X at timeout in %d mode",
              p_supl_session_status->u_position_protocol, p_supl_session_status->e_cf_state,
              p_supl_session_status->u_agps_mode);

        if (p_supl_session_status->e_cf_state & (C_SUPL_CF_STATE_SUPLPOS_XCHG | C_LPP_UP_STATE_OTDOA_GRT_24_MEAS))
        {
          ...
          if (p_supl_session_status->u_position_protocol ==  TM_POS_PRTL_RRLP)
          {
            /* If measurement can't be send, then mark the flag to send RRLP error msg */
            if (C_SUPL_AGPS_MODE_UE_ASSISTED == p_supl_session_status->u_agps_mode)
            {
              v_rpt_rrlp_meas = tm_umts_up_rrlp_handle_prm_data(p_supl_session_status);
            }

            /* if still in the state waiting to respond to the network, early termination is needed */
            if ((FALSE == v_rpt_rrlp_meas) && (!tm_umts_up_supl_suplpos_rrlp_mpr_err_tx(p_supl_session_status, rrlp_notEnoughSats)))
            {
          ...
          {
            tm_lpp_up_stop_sess_req_handler(stop_reason, p_supl_session_status);
            tm_lpp_up_set_tm_session_done(TRUE);
          }
#endif /* FEATURE_CGPS_LTE_CELLDB && !FEATURE_GNSS_LOW_MEMORY */
        } /* has been in the state of waiting to respond to SLP with RRLP */
        else if (p_supl_session_status->e_cf_state & C_SUPL_CF_STATE_UT3)
        {
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "Supl CF state when LM Timeout %d. Wait for UT3 to expire before session closes",
   ...
   )
{
  ...
  switch (sess_info_type)
  {
    ...
    case PRTL_SESS_INFO_PRM:
      {
        MSG_1(MSG_SSID_GPSSM, MSG_LEGACY_MED, "SUPL: got PRM of type %u (GPS/GLO/BDS/GAL/QZSS)",
          p_sess_info_param->prm_data.z_MeasBlk.z_GnssHeader.e_MeasBlkSrc);
    ...
    case PRTL_SESS_INFO_LR:
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "SUPL: got LR", 0, 0, 0);
    ...
    case PRTL_SESS_INFO_STATUS:
      {
        ...
        q_gps_wl_mask = p_sess_info_param->aiding_data_status.z_assist_data_status.q_WishListMask;
        ...
        if ((tm_umts_up_supl_rrlp_assisted_glonass_enabled() ||
             tm_core_get_asst_glo_lpp_up_enable()) &&
            ...
            )
        {
          ...
          q_glo_wl_mask = p_sess_info_param->aiding_data_status.z_assist_data_status.z_GloAssistData.q_WishListMask;
        }
        else
        {
        ...
        {
          /* --------------------------------------------------------------------*/
          /* Simply copy over the Wishlist for BDS, protocol will deal with it!  */
          /* Because this session can turn into a BDS only session also, we dont */
          /* modify the BDS WL at this time                                      */
          /* --------------------------------------------------------------------*/

          p_supl_session_status->z_BdsAssistData =
             p_sess_info_param->aiding_data_status.z_assist_data_status.z_BdsAssistData;
          q_bds_wl_mask = p_sess_info_param->aiding_data_status.z_assist_data_status.z_BdsAssistData.q_WishListMask;
        }
        else
        {
          MSG(MSG_SSID_GPSSM, MSG_LEGACY_MED, "RRLP SUPL ABDS disabled. Set BDS WL to 0");
          memset(&p_supl_session_status->z_BdsAssistData, 0, sizeof(p_supl_session_status->z_BdsAssistData));
          q_bds_wl_mask = p_sess_info_param->aiding_data_status.z_assist_data_status.z_BdsAssistData.q_WishListMask = 0;
        }

        /* during emergency call, always request a refLoc from the server */
        if ((phone_state_info.e911_state != TM_CM_E911_STATE_NONE) &&
            (p_supl_session_status->u_agps_mode == C_SUPL_AGPS_MODE_UE_BASED))
        {
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "Phone in emergency state, SUPL MSB should always request RefL", 0, 0, 0);
          p_sess_info_param->aiding_data_status.z_assist_data_status.q_WishListMask |=  C_GPS_ASSIS_REFLOC;
          p_sess_info_param->aiding_data_status.z_assist_data_status.z_BdsAssistData.q_WishListMask |=  C_GNSS_ASSIST_REFLOC;
          q_gps_wl_mask = p_sess_info_param->aiding_data_status.z_assist_data_status.q_WishListMask;
        }
        // Copy the constructed GPS + Glonass + BDS AD wishlist
        ...
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "PreHoldoff Wishlist: GPS=0x%lX, GLONASS=0x%lX, BDS 0x%lX ",
              q_gps_wl_mask, q_glo_wl_mask, q_bds_wl_mask);
            ...
            (q_gps_wl_mask || q_glo_wl_mask || q_bds_wl_mask))
        {
          ...
          if (l_long_word > 0)
          {
            MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "GF holdoff remaining = %ld msec", l_long_word, 0, 0);
            ...
            p_sess_info_param->aiding_data_status.z_assist_data_status.z_BdsAssistData.q_WishListMask = 0;
            q_bds_wl_mask = 0;
          }
        }
        ...
        if (q_gps_wl_mask)
        {
          tm_umts_up_supl_assist_req_holdoff_timers_check_start(&q_gps_wl_mask);
        }
        if (q_glo_wl_mask)
        {
          tm_umts_up_supl_glo_assist_req_holdoff_timers_check_start(&q_glo_wl_mask);
        }
        if (q_bds_wl_mask)
        {
          tm_umts_up_supl_bds_assist_req_holdoff_timers_check_start(&q_bds_wl_mask);
        }
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "PostHoldoff Wishlist: GPS=0x%lX, GLO=0x%lX BDS=0x%lX",
              q_gps_wl_mask, q_glo_wl_mask, q_bds_wl_mask);
        ...
        if (q_gps_wl_mask == C_GNSS_ASSIST_REFTIME)
        {
          q_gps_wl_mask = 0;
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "GPS RefT only req. suppressed", 0, 0, 0);
        }
        if (q_glo_wl_mask == C_GNSS_ASSIST_TIME_MODELS)
        {
          q_glo_wl_mask = 0;
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "GLO time model only req. suppressed", 0, 0, 0);
        }
        if (q_bds_wl_mask == C_GNSS_ASSIST_TIME_MODELS)
        {
          q_bds_wl_mask = 0;
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "BDS time model only req. suppressed", 0, 0, 0);
        }
        ...
        /*
         * We dont need to convert BDS WL (q_bds_wl_mask) to any intermediate 
         * formats. We will convert the native MGP WL to the protocol specific 
         * request in the protocol module itself.
        */

        if ((q_gps_wl_mask || q_glo_wl_mask || q_bds_wl_mask) ||
        ...
        } /* WL Mask || (u_callflow_kind == C_SUPL_SESSION_NI) ... */
        else
        {
          ...
          tm_umts_up_supl_inform_core_nt_assist_end(&z_supl_session_status);
    ...
    case PRTL_SESS_INFO_OTDOA:
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "Received OTDOA measurement information", 0, 0, 0);
      tm_lpp_up_handle_otdoa_data(&(p_sess_info_param->otdoa_data), &z_supl_session_status);
      break;
#endif /* FEATURE_CGPS_LTE_CELLDB && !FEATURE_GNSS_LOW_MEMORY */

    default:
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "PRTL_SESS_INFO_ not processed: %u",
            sess_info_type, 0, 0);
  ...
  if (timer_param != NULL)
  {
    switch (timer_param->q_TimerId)
    {
      case SUPL_CF_TIMER_ID: /* one timer, many possible states */
        {
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_LOW, "SUPL Callflow timer expired. CF State 0x%lX", p_supl_session_status->e_cf_state, 0, 0);
          ...
          if (p_supl_session_status->e_cf_state & C_SUPL_CF_STATE_CELL_INFO_REQ)
          {
            ...
            /* effort to update cell info. fails, go ahead anyway hoping new
               update may come before it's ready to send the message */
            MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "Celll_id req. to RRC timed out, start SLP contact", 0, 0, 0);
            /* Initializing the flags so that we can attempt a fresh
               connection*/
            z_pdcomm_tcp_info.tls_session_authentication_failure = FALSE;
            z_pdcomm_tcp_info.used_auto_configured_hslp_address = FALSE;
          ...
          if (p_supl_session_status->e_cf_state & C_SUPL_CF_STATE_UT0_SUPLEND)
          {
            ...
            MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "SUPL: UT0 - SuplEnd expired", 0, 0, 0);
          }
          if (p_supl_session_status->e_cf_state & C_SUPL_CF_STATE_UT0_SUPLREPORT_QUERY)
          {
            ...
            MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "SUPL: UT0 - SuplReport_query expired", 0, 0, 0);
          }
          if (p_supl_session_status->e_cf_state & C_SUPL_CF_STATE_UT10_INFO_QUERY_END)
          {
            ...
            MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "SUPL: UT10 - waiting SUPL_END after query expired", 0, 0, 0);
          }
          ...
          for (u_i = 0; u_i < C_MAX_GEOFENCE_NUM; u_i++)
          {
            if (z_supl_area_event_session_status[u_i].u_session_active)
            {
                  ...
                  )
              {
                ...
                MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "Clearing areaEventTrig session [%u]: UT exp. - %x",
          ...
          {
            MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "SUPL: UTx expired: %u", p_supl_session_status->e_cf_state, 0, 0);
          ...
          else if (p_supl_session_status->e_cf_state & C_SUPL_CF_STATE_SUPLPOS_XCHG)
          {

            if (p_supl_session_status->u_position_protocol ==  TM_POS_PRTL_RRLP)
            {
              MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "SUPL: RRLP msrPosReqRespTime expired", 0, 0, 0);
      
              /* 
               * If no GNSS Measurement reports have been received OR
               * reports were received, but we could not qualify them/send them in tm_umts_up_rrlp_handle_prm_data
               * send an RRLP Meas Pos Resp Error
               */
              if ((0 == p_supl_session_status->z_gnss_meas_report.u_meas_reports_bmap) ||
                  (FALSE == tm_umts_up_rrlp_handle_prm_data(p_supl_session_status)))
              {
                tm_umts_up_supl_terminate_timer_cancel_cf_state(C_SUPL_CF_STATE_SUPLPOS_XCHG);

                if (!tm_umts_up_supl_suplpos_rrlp_mpr_err_tx(p_supl_session_status, rrlp_notEnoughSats))
                {
            ...
            {
              tm_lpp_up_handle_supl_cf_timer_expiry(p_supl_session_status);
            } /* if LPP*/
#endif /* FEATURE_CGPS_LTE_CELLDB && !FEATURE_GNSS_LOW_MEMORY */
          }
      ...
      case SUPL_GM_REQ_HD_TIMER_ID:
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "SUPL timer %u expires", timer_param->q_TimerId, 0, 0);
        break;

      case SUPL_UDP_BIND_RETRY_TIMER_ID:
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "SUPL UDP BIND retry timer expired", 0, 0, 0);
        (void)tm_umts_up_supl_comm_udp_bind(FALSE);
        break;

      case SUPL_MULTIPART_WAP_PUSH_WAIT_TIMER_ID:
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "SUPL MULTIPART WAP PUSH wait timer expired", 0, 0, 0);
        tm_umts_supl_comm_sms_read_state_init();
        break;

      case SUPL_TCP_STAY_ON_ONLY_TIMER_ID:
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "SUPL_TCP_STAY_ON_ONLY_TIMER_ID expired", 0, 0, 0);
        tm_umts_up_supl_comm_close_connection();
        break;

      case SUPL_WIFI_WAIT_TIMER_ID:
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "SUPL_WIFI_WAIT timer expired", 0, 0, 0);
        if (p_supl_session_status->e_cf_state & C_SUPL_CF_STATE_WIFI_WAIT_TIMER_NO_TCP)
        {
          tm_umts_up_supl_terminate_timer_cancel_cf_state(C_SUPL_CF_STATE_WIFI_WAIT_TIMER_NO_TCP);
        }
            ...
            )
        {
          ...
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "Sending SUPL_START at WIFI_WAIT timeout with cf_state of %x", p_supl_session_status->e_cf_state, 0, 0);
            ...
            )
        {
          ...
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "Sending SUPL_POSINIT at WIFI_WAIT timeout with cf_state of %x", p_supl_session_status->e_cf_state, 0, 0);
      ...
      case SUPL_LTE_CELLINFO_RETRY_TIMER_ID:
        MSG(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "SUPL_LTE_CELLINFO_RETRY_TIMER_ID timer expired");

        /*Get the LTE cell information.*/
        supl_lte_cell_information_construct();

        if (0 != ((p_supl_session_status->e_cf_state) & (C_SUPL_CF_STATE_LTE_CELLID_RETRY_TIMER_SUPLSTART)))
        {
          tm_umts_up_supl_terminate_timer_cancel_cf_state(C_SUPL_CF_STATE_LTE_CELLID_RETRY_TIMER_SUPLSTART);
          if (!supl_SuplStart_tx())
          {
            tm_umts_up_supl_callflow_disrupt_handler();
          }
          ...
          if ((p_supl_session_status->e_cf_state & C_SUPL_CF_STATE_WIFI_WAIT_SUPLSTART) ||
              (p_supl_session_status->e_cf_state & C_SUPL_CF_STATE_WIFI_WAIT_TIMER_SUPLSTART) ||
              (!(p_supl_session_status->e_cf_state & C_SUPL_CF_STATE_WIFI_WAIT_TIMER_NO_TCP)))
          {
        ...
        else if (0 != (p_supl_session_status->e_cf_state & C_SUPL_CF_STATE_LTE_CELLID_RETRY_TIMER_SUPLPOSINIT))
        {
          tm_umts_up_supl_terminate_timer_cancel_cf_state(C_SUPL_CF_STATE_LTE_CELLID_RETRY_TIMER_SUPLPOSINIT);
          if (!supl_SuplPosInit_tx())
          {
            tm_umts_up_supl_callflow_disrupt_handler();
          }
      ...
      default:
        if (0 != (timer_param->q_TimerId & TM_LPP_UP_TIMER_ID_DISPATCH_MASK))
        {
          tm_lpp_up_handle_timer_expiry(timer_param);
        }
        else
        {
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR, "SUPL got undefined SUPL timer ID", 0, 0, 0);
        }
    }
  }
  else
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "Cannot process null timer msg", 0, 0, 0);
  }
...
)
{
  ...
  boolean ret_val = TRUE;
  sys_modem_as_id_e_type emerg_call_asid;
  ...
  if( (NULL == p_event_payload) &&
      ...
      (event_type != PRTL_EVENT_TYPE_UMTS_OTA_GPS_RESET) )
  {
  ...
  switch (event_type)
  {
    ...
    case PRTL_EVENT_TYPE_LCS:
      {
        ...
        {
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "SUPL: TmCore cb LCS type with event CLINET_INIT", 0, 0, 0); /* QQQ: ? */
        }
        else if (p_event_payload->lcs_event_payload.lcs_event == PRTL_LCS_EVENT_TYPE_CLINET_RELEASE)
        {
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "SUPL: TmCore cb LCS type with event CLINET_RELEASE", 0, 0, 0); /* QQQ: ? */
        }
    ...
    case PRTL_EVENT_TYPE_PHONE:
      {
        ...
        {
          emerg_call_asid = p_event_payload->phone_event_payload.payload.emerg_call_info.emerg_call_asid;
          
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "SUPL: got Ell call orig", 0, 0, 0);
          tm_umts_up_supl_E911_handler();

          if(emerg_call_asid != dd_sub_id)
          {
            /* Set up NVs for emergency sub*/
            MSG(MSG_SSID_GPSSM, MSG_LEGACY_MED, "E911 orig on non-DD. Reconfig UMTS NVs");
            tm_umts_common_utils_update_config_for_sub(emerg_call_asid);
          }
        ...
        else if (p_event_payload->phone_event_payload.event == TM_PHONE_EVENT_E911_CALL_END)
        {
		  emerg_call_asid = p_event_payload->phone_event_payload.payload.emerg_call_info.emerg_call_asid;
          if(emerg_call_asid != dd_sub_id)
          {          
            /* Set up NVs for DD sub*/
            MSG(MSG_SSID_GPSSM, MSG_LEGACY_MED, "E911 end on non-DD. Reconfig UMTS NVs");
            tm_umts_common_utils_update_config_for_sub(dd_sub_id);
          }
          }
                 ...
                 )
        {
          ...
          {
            MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "Camped to UMTS: G or W or L or WLAN", 0, 0, 0);
          }


          else if (p_event_payload->phone_event_payload.event == TM_PHONE_EVENT_IN_TDS_COVERAGE)
          {
            MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "Camped to TDSCDMA", 0, 0, 0);
            tm_umts_up_supl_ck_req_tdscmda_cell_info();
          }
        ...
        {
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "Out Of Service or LPM mode", 0, 0, 0);
        ...
        {
          MSG_1(MSG_SSID_GPSSM, MSG_LEGACY_MED, "SUPL: got TmCore cb event PHONE %d - not E911",
    ...
    case PRTL_EVENT_TYPE_CELL_DB:    /* cell DB info. update from MGP OR external cell DB info from PDAPI */
      ...
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "SUPL: got unknown cell_db_event type: %u", p_event_payload->cell_db_event_payload.cell_db_event, 0, 0);
        ret_val = FALSE;
      }
      break;
    ...
    case PRTL_EVENT_TYPE_UMTS_OTA_GPS_RESET:
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "LTE/UMTS OTA RESET message received, Clearing Hold off timers", 0, 0, 0);
        tm_umts_up_supl_ota_delete_proc();
    ...
    case PRTL_EVENT_TYPE_LTE_OTA_OTDOA_RESET:
      {
#ifdef FEATURE_LTE
        MSG(MSG_SSID_GPSSM, MSG_LEGACY_MED, "LTE OTA OTDOA RESET message received");
        tm_lpp_up_set_ota_reset_status(TRUE);
#else
        MSG(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "LTE_OTA_OTDOA_RESET not enabled");
#endif /* FEATURE_LTE */
      }
      break;
    ...
    case PRTL_EVENT_TYPE_WIFI_SCAN_RESULT:
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "SUPL got Wifi scan result: %u", p_event_payload->wifi_scan_event_payload.z_wifi_scan_info.z_wifi_ap_scan_info.q_num_wifi_aps, 0, 0);
    ...
    case PRTL_EVENT_TYPE_CARRIER_NV_REFRESH:
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "CARRIER NV REFRESH received for SUPL", 0, 0, 0);
        tm_umts_common_utils_refresh_carrier_specific_nv();
#if defined (FEATURE_CGPS_LTE_CELLDB) && !defined (FEATURE_GNSS_LOW_MEMORY)
        tm_lpp_up_refresh_efs_nv(p_event_payload->nv_refresh_payload.read_from_hw);
#endif /* FEATURE_CGPS_LTE_CELLDB && ! FEATURE_GNSS_LOW_MEMORY */
        break;
      }
    case PTRL_EVENT_TYPE_DBH_POS_REPORT:
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "Best position report received for SUPL", 0, 0, 0);
        tm_lpp_up_handle_best_pos(&(p_event_payload->z_dbh_pos_report));
        break;
      }

    case PRTL_EVENT_TYPE_READ_NEW_IMSI:
      {
        MSG(MSG_SSID_GPSSM, MSG_LEGACY_MED, "READ_NEW_IMSI received for SUPL");
        /* Retrieve imsi for supl */
        (void) tm_umts_up_supl_comm_retrieve_imsi();
        break;
      }
    default:
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "SUPL: got unknown TmCore cb event: %u", event_type, 0, 0);
      ret_val = FALSE;
...
void tm_umts_up_supl_init(void)
{
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "tm_supl_init", 0, 0, 0);
  ...
  wcdma_l1_mrl_req_timer            = os_TimerCreateInCgpsTimerGroup(SUPL_L1_MRL_REQ_TIMER_ID, THREAD_ID_SM_TM);
  ...
  supl_lte_get_cell_info_retry_timer = os_TimerCreateInCgpsTimerGroup(SUPL_LTE_CELLINFO_RETRY_TIMER_ID, THREAD_ID_SM_TM);
#ifdef FEATURE_ABDS_RRLP_SUPL
  supl_bds_alm_req_holdoff_timer       = os_TimerCreateInCgpsTimerGroup(SUPL_BDS_ALM_REQ_HD_TIMER_ID, THREAD_ID_SM_TM);
  supl_bds_eph_req_holdoff_timer       = os_TimerCreateInCgpsTimerGroup(SUPL_BDS_EPH_REQ_HD_TIMER_ID, THREAD_ID_SM_TM);
  supl_bds_reftime_req_holdoff_timer   = os_TimerCreateInCgpsTimerGroup(SUPL_BDS_REFTIME_REQ_HD_TIMER_ID, THREAD_ID_SM_TM);
  supl_bds_refloc_req_holdoff_timer    = os_TimerCreateInCgpsTimerGroup(SUPL_BDS_REFLOC_REQ_HD_TIMER_ID, THREAD_ID_SM_TM);
  supl_bds_rti_req_holdoff_timer       = os_TimerCreateInCgpsTimerGroup(SUPL_BDS_RTI_REQ_HD_TIMER_ID, THREAD_ID_SM_TM);
  supl_bds_timemodel_req_holdoff_timer = os_TimerCreateInCgpsTimerGroup(SUPL_BDS_TIMEMODEL_REQ_HD_TIMER_ID, THREAD_ID_SM_TM);
#endif  
...
#if defined (FEATURE_CGPS_LTE_CELLDB) && !defined (FEATURE_GNSS_LOW_MEMORY)
  /* TODO : Consider moving. LPP Tx manager init is done in the LPP UP module.
   * Now that we have decoupled LPP UP and SUPL somewhat, we dont need to call this here */
...
boolean tm_umts_up_supl_api_close_session(tm_sess_stop_reason_e_type  stop_reason)
{
  ...
  if ( z_pdcomm_tcp_info.curr_state == PDCOMM_STATE_CONNECTED && z_supl_session_status.u_send_supl_end_at_fail)
  {
 ...
 )
{
  ...
  if (status == RRC_NEW_CELL_ID_AVAIL)
  {
    ...
    else
    {
      ...
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "DCH-WCDMA cell info updating cached Non-W: New MCC=%lu,MNC=%lu,CI=%lu",
              q_mcc_new,
              q_mnc_new,
              cell_id_info.cell_id);
      }
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "CELLIDDCH: Cell info updated by RRC, start SLP contact", 0, 0, 0);
    }
    else if (z_supl_session_status.e_cf_state & C_SUPL_CF_STATE_CELL_INFO_REQ_AREA_EVENT)
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "CELLIDDCH: Cell info updated by RRC, start SLP contact areaEvent", 0, 0, 0);
    }
    else
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "CELLIDDCH: Cell info updated by RRC while not expecting", 0, 0, 0);
    }
  ...
  else
  {
    if (z_supl_session_status.e_cf_state & C_SUPL_CF_STATE_CELL_INFO_REQ)
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "CELLIDDCH: Recvd no-new cell info from RRC, start SLP contact", 0, 0, 0);
    }
    else if (z_supl_session_status.e_cf_state & C_SUPL_CF_STATE_CELL_INFO_REQ_AREA_EVENT)
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "CELLIDDCH: Recvd no-new cell info from RRC, start SLP contact areaEvent", 0, 0, 0);
    }
    else
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "CELLIDDCH: Recvd no-new cell info from RRC while not expecting", 0, 0, 0);
    }
...
static void tm_umts_up_supl_gm_geofence_add_notify_handler(sm_gm_client_notification_type *gm_notification)
{
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "no matching transaction id %lu, ignored",
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "Geofence Add Succesfull Geofence Id %d", gm_notification->notification.geofence_id, 0, 0);
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "Geofence Add Error %d", gm_notification->notification.return_code, 0, 0);
   ...
   )
{
  ...
  switch (msg_type)
  {
    ...
    case GEOFENCE_DELETE_NOTIFY:
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "Geofence %d deleted by GM", ((sm_gm_client_notification_type *)msg_data_ptr)->notification.geofence_id, 0, 0);
    ...
    case GEOFENCE_BREACH_NOTIFY:
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "Geofence id %d breached", ((sm_gm_client_breach_notify_type *)msg_data_ptr)->geofence_id, 0, 0);
    ...
    case GEOFENCE_POS_REQUEST_NOTIFY:
      if (z_supl_session_status.u_callflow_kind == C_SUPL_SESSION_NONE)
      {
        ...
        else
        {
          ...
          {
            MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "SUPL not active protocol", 0, 0, 0);
          }
        }
    ...
    case GEOFENCE_CLIENT_GEOFENCE_HANDELS_NOTIFY:
      {
        ...
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "Num. of existing geofences: %u",
        ...
        for (u_i = 0; u_i < p_notification->num_of_client_geofence_handles; u_i++)
        {
          ...
          {
            MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR, "Geofence blob request failed: %u", error_code, 0, 0);
          }
          else
          {
            MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "Geofence blob request successful for id %u", gm_blob_request.geofence_id, 0, 0);
          }
        }
    ...
    case GEOFENCE_CLIENT_BLOB_NOTIFY:
      {
        ...
        for (u_i = 0; u_i < C_MAX_GEOFENCE_NUM; u_i++)
        {
          ...
          if (z_supl_area_event_session_status[u_i].u_session_active == FALSE)
          {
            ...
            MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "Geofence blob for id %u taking slot %u",
                  p_gm_blob_notification->geofences_client_blob_info.geofence_id,
                  u_i, 0);
