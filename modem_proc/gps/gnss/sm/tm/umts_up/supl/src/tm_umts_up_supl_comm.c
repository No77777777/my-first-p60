...
/*===========================================================================
  ...
  $Header: //components/rel/gnss8.mpss/11.1/gnss/sm/tm/umts_up/supl/src/tm_umts_up_supl_comm.c#4 $
  $DateTime: 2023/02/21 18:08:15 $
  $Author: pwbldsvc $

  when       who     what, where, why
  --------   ---     ----------------------------------------------------------   
  02/11/23   bp      Removed incrementing of disconnect transaction id while disconnecting pdcomms
  07/13/20   skm     Correct sub ID check to ensure we block UDP bind 
  07/06/20   skm     Ensure we read HSLP from correct sub for nDDS SUPL sessions
  09/30/19   skm     Support for 911 nDDS on DSDS targets
  08/01/18   sj      Fix custom MO SLP bug in passing HSLP to PDCOMMs
  06/17/18   sj      Added custom SLP for MO AGNSS requests.
  10/05/16   kpn     UDP port should always bind to port 7275
  08/12/16   ak      Add lenght check for SUPL_INIT over SMS/UDP
...
/* OMA ULP standard port (7275)*/
#define OMA_ULP_UDP_PORT (7275)

os_TimerMsgType*          supl_udp_bind_retry_timer;
os_TimerMsgType*          supl_multipart_wap_push_wait_timer;

/* for pdcomm rx buffer */
uint8 *p_pdcomm_rx_buff;
uint16 w_pdcomm_rx_buff_size;
uint8 u_buffer_counter; //temp 
...
pd_comms_return_type tm_umts_up_supl_comm_tcp_connect( uint8 u_supl_version_main )
{
  ...
  else
  {
    ...
    { 
      /* NV 74121 configures, TLS 1.2 and above, also SHA verion above SHA256
         If : the NV is congigured, pick the TLS Version to be used from New NV.
         ELse: use the old NV's TLS configuration*/
      if ( ( z_umts_nv_item_ram_copy.aagps_sha_tls_support & 0x1 ) != 0 )
      {
         z_comms_connect_params_tcp.e_ssl_ver_tls = SSL_VER_TLS_1_2;
      }
      else if ( ( z_umts_nv_item_ram_copy.aagps_use_transport_security & 0x2 ) == 0 )
      {
         z_comms_connect_params_tcp.e_ssl_ver_tls = SSL_VER_TLS_1_1;
      }
    }

    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"TLS version being passed is %d", z_comms_connect_params_tcp.e_ssl_ver_tls ,0,0);
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"TCP connect tr_id = %lx. CONNECTING", z_comms_connect_params_tcp.q_transaction_id, 0, 0 ); 
...
void tm_umts_up_supl_comm_udp_bind( boolean srv_system_changed )
{
  ...
  if (TM_CORE_CONFIG_DD(gnss_sm_udp_allow) == (uint8)GNSS_NV_SUPL_DENY_UDP_BIND)
  {
     MSG(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "UDP bind not allowed");
     return;
  }
  ...
  /*UDP port should be bound to port:7275. This is the standard OMA ULP Port 
    LCS agents always send the SUPL_INIT UDP packet to this port as dictated by
    by OMA-Spec. TCP port can be configured in NV to be other than 7275.*/
  bind_params.z_link_params.z_tcp_params.w_ip_port = (uint16) OMA_ULP_UDP_PORT;
  ...
  {    
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"UDP bind failed",0,0,0);
    if(supl_pdcomm_udp_bind_retry_count < SUPL_UDP_BIND_MAX_RETRY_COUNT)
    {
      /*Start the UDP retry timer for retrying*/
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Starting the udp bind retry timer",0,0,0);
      supl_pdcomm_udp_bind_retry_count++;
      (void) os_TimerStart(supl_udp_bind_retry_timer, SUPL_UDP_BIND_RETRY_VALUE_MSEC, NULL );
    }
    else
    {      
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Max number of UDP bind retries for this RAT over",0,0,0);
    }
  }
...
boolean tm_umts_up_supl_comm_init ( void )
{
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"pdcomm tcp handle not acquired at TM start", 0, 0, 0); /* depend on when pdcomm_tcp starts */
  }
  else
  {
    pdcomm_tcp_handle_get_done = TRUE;

    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"pdcomm tcp handle acquired at TM start", 0, 0, 0); /* depend on when pdcomm_tcp starts */
  ...
  if ( pd_comms_app_get_handle (&z_pdcomm_get_handle_params, &z_pdcomm_handle_sms ) != PDCOMM_RESULT_OK )
  {
    ...
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"fails to get pdcomm handle for SMS at TM start", 0, 0, 0);
    error_flag = TRUE;
  }
  ...
  else 
  {
    ...
    {
      /* shouldn't happen, fatal */ 
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"fails to connect pdcomm for sms", 0, 0, 0);
      error_flag = TRUE;
    }
  ...
  {
    /* shouldn't happen, fatal */
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"fails to get pdcomm handle for UDP at TM start", 0, 0, 0);
    error_flag = TRUE;
  }
...
pd_comms_return_type tm_umts_up_supl_comm_write(uint8 *p_data, uint16 w_size)
{
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"PDCOMM TD: pdcomm_client_write returned error: %d",ret_val,0,0);
  }
...
void tm_umts_up_supl_comm_close_connection( void )
{
  ...
  */
  ...
  else
  {
    ...
    */
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_LOW, "Failed to Disconnect with SLP ",0,0,0 );
    z_pdcomm_tcp_info.curr_state = PDCOMM_STATE_DISCONNECTED;
  }
...
boolean tm_umts_up_supl_comm_handle_read_data(void *msg_data_ptr)
{
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Wrong link type: %d", read_hdr_ptr->z_handle.e_link_type, 0, 0 );
      return FALSE;
    }
    ...
    if ( p_pdcomm_info->curr_state == PDCOMM_STATE_CONNECTED )
    {
      if( read_hdr_ptr->w_data_len != 0 )
      {
        ...
        if( p_pdcomm_info->recv_state == TM_SUPL_COMM_RECV_HEADER )
        {
          ...
          if ( read_hdr_ptr->w_data_len == 2 )
          {
            ...
            *p_pdcomm_rx_buff_local++ = u_num_bytes_to_read_lsb; /* LSB of the length field in SUPL message buffer */ 
            ...
            MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_LOW,"Read data of length %d",p_pdcomm_info->num_bytes_to_read, 0, 0);

            p_pdcomm_info->recv_state = TM_SUPL_COMM_RECV_PAYLOAD;
          }
          else
          {
            MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"PDCOMM data header size wrong: %u", read_hdr_ptr->w_data_len, 0, 0);
            ret_val = FALSE;
          }
        ...
        else /*if( p_pdcomm_info->recv_state == TM_SUPL_COMM_RECV_PAYLOAD )*/
        {
          ...
          //MSG_1(MSG_SSID_GPSSM, MSG_LEGACY_LOW,"%d bytes of SUPL message recv.", read_hdr_ptr->w_data_len);
          ...
          {
            MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"data_len %u: greater than buffer", read_hdr_ptr->w_data_len, 0, 0);            
            tm_umts_up_supl_comm_free_pdcomm_rx_buff();
            return FALSE;
          }
          ...
          if(p_pdcomm_info->num_bytes_to_read > read_hdr_ptr->w_data_len) /*Check so that num_bytes_to_read does not overflow*/
          {
            ...
            MSG_1(MSG_SSID_GPSSM, MSG_LEGACY_MED,
                  "More data expected from PDCOMM(%u)",p_pdcomm_info->num_bytes_to_read);
            /*SUPL_INIT comes as a single packet in that case if more payload is expected for SUPL Init,
          ...
          else 
          {
            ...
            if ( read_hdr_ptr->z_handle.e_link_type == LINK_SMS ||
                 read_hdr_ptr->z_handle.e_link_type == LINK_UDP )
            {
              /*SUPL init handling block*/
              z_supl_session_status.session_sub_id = read_hdr_ptr->as_id;

              /*Ensure we read the NV's from the correct SUB to generate HASH 
                suplInitProc() will ensure PDCOMM's is notified of SUB to use for the
                session. Here read only HSLP related NV's from the correct sub.*/
              if (z_supl_session_status.session_sub_id != tm_cm_dsds_get_dd_subs()) 
              {
                 tm_umts_common_utils_update_config_for_sub(z_supl_session_status.session_sub_id);
              }

              MSG_1(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,
                  "SUPL session sub ID for NI session as returned from PDCOMMS : %d",read_hdr_ptr->as_id);
              ...
              if ( ! supl_comm_generate_hash ( p_pdcomm_rx_buff, pdcomm_rx_buff_len) )
              {
                 ...
                 {
                   MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"SUPL INIT Hash Generation Failed", 0, 0, 0 );
      ...
      else
      {
        ...
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Received data of zero length from TM-Core",0,0,0);
        ret_val = FALSE;
      }
    ...
    else
    {
      ...
      ** connected.
      */
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Received data from TM-Core in error state %d",p_pdcomm_info->curr_state,0,0);
      ret_val = FALSE;
    }
...
void tm_umts_up_supl_comm_handle_notification ( void *msg_data_ptr )
{
  ...
  if(notify_hdr_ptr->e_type == STATE_CHANGE) /* notified of state change */
  {
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"SUPL: got TCP state change: %d", params->e_new_state, 0, 0);
    }
       ...
       )
    {
      ...
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"TCP connection with SLP up",0,0,0);
              ...
              )
      {
        ...
        {
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Previous session Disconnect received",0,0,0);
          return;
        }        
        
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"TCP disconnecting/disconnected",0,0,0);
            ...
            (z_pdcomm_tcp_info.tls_session_authentication_failure == TRUE) )
        {
          ...
          {
            MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Waiting for complete disconnect before proceding",0,0,0);
            return;
          }

          if(p_supl_session_status->u_tcp_connection_attempt_pending)
          {          
            MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"START Next TCP Connection",0,0,0);
        ...
        if( z_pdcomm_tcp_info.tls_session_authentication_failure )
        {
          ...
          {  
             case C_SUPL_SESSION_NI:
               if ( supl_comm_generate_hash ( p_pdcomm_rx_buff, pdcomm_rx_buff_len ) )
               {
                  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Re-attempting socket connection with auto-config H-SLP addr",0,0,0);
                  return;
               }
               else
               { 
                  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"SUPL INIT Hash Generation Failed during socket re-attempt", 0, 0, 0 );
                  return;
               }
               break;
            ...
            )
    {
      ...
      if(params->e_new_state == PDCOMM_STATE_CONNECTED)
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"SMS connection with SLP up",0,0,0); /* this should be a one-time occurrance since reset */
      ...
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"recv SMS connection state other than CONNECTED, %d", params->e_new_state, 0, 0); 
            ...
            )
    {
      ...
      if(params->e_new_state == PDCOMM_STATE_CONNECTED)
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"UDP socket created",0,0,0); /* this should be a one-time occurrance since reset */
      ...
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"recv UDP connection state other than CONNECTED, %d", params->e_new_state, 0, 0); 
  ...
  else if ( notify_hdr_ptr->e_type == READ_DATA )  /* notified of data to read */
  {
    ...
    if ( notify_hdr_ptr->z_handle.e_link_type == LINK_TCP )
    {
      ...
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Read data in error state %d", z_pdcomm_tcp_info.curr_state,0,0);
      }
    ...
    else if ( notify_hdr_ptr->z_handle.e_link_type == LINK_SMS )
    {
      ...
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_LOW,"SMS Read notification from TM-Core",0,0,0);

      if ( z_pdcomm_sms_info.curr_state == PDCOMM_STATE_CONNECTED )
      {
        if( z_pdcomm_sms_info.recv_state == TM_SUPL_COMM_RECV_HEADER)
        {
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_LOW,"Read SUPL header from SMS",0,0,0);
          (void) supl_comm_read( SUPL_LENGTH_HDR_SIZE, LINK_SMS, TRUE ); /* tell PDCOMM to read SUPL_LENGTH_HDR_SIZE bytes */
        }
        else
        {
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_LOW,"Read SUPL payload of length %d from SMS", z_pdcomm_sms_info.num_bytes_to_read, 0, 0);
          (void) supl_comm_read( z_pdcomm_sms_info.num_bytes_to_read, LINK_SMS, TRUE );
        }
      }
      else
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Read data in error state %d", z_pdcomm_sms_info.curr_state,0,0);
      }
    ...
    else if ( notify_hdr_ptr->z_handle.e_link_type == LINK_UDP )
    {
      ...
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Read data in error state %d", z_pdcomm_udp_info.curr_state,0,0);
      }
    ...
    else
    {
      ...
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Read notif. of a wrong link type: %d", notify_hdr_ptr->z_handle.e_link_type, 0, 0 );
    }
  ...
  else if ( notify_hdr_ptr->e_type == ERROR_RPT )  /* notified of a communication error */
  {
    ...
    if ( notify_hdr_ptr->z_handle.e_link_type == LINK_TCP ) 
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"SUPL: got TCP Error Report: %d", params->q_result, 0, 0);
    ...
    {          
           ...
           (notify_hdr_ptr->z_params.q_result == PDCOMM_READ_ERROR_INTERNAL)) 
       {
          ...
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"UDP read error, Socket still OPEN",0,0,0);
          z_pdcomm_udp_info.recv_state = TM_SUPL_COMM_RECV_HEADER;
          z_pdcomm_udp_info.num_bytes_to_read = 0;
       }
       else
...
void tm_umts_up_supl_comm_handle_hash_resp (void *msg_data_ptr)
{
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"SUPL - HASH too short: %u", hash_resp_ptr->q_output_len, 0, 0);   
      return;
    }
...
void tm_umts_up_supl_comm_root_certs_updated(void)
{
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Could not update SSL with cert dir", 0, 0, 0);
  }
}
...
boolean tm_umts_up_supl_comm_pass_root_cert_dir(void)
{
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"SUPL: root cert. passed to pdcomm ok", 0, 0, 0);
    ret_val = TRUE;
  }
...
DESCRIPTION
  Obtains the appropriate slp address and port number to use from NV or PDAPI
  params or the auto generated hslp from mcc/mnc combination.
...
static boolean supl_comm_get_slp_address_portnum( pd_comms_tcp_connect_params_type  *p_connect_params )
{
  static uint8 server_addr_url[C_MAX_URL_SIZE_BYTE];
  static char port_string[6];
  uint16 i = 0;
  uint16 j = 0;
  ...
  boolean b_ret = FALSE;
  supl_status_s_type          *p_supl_session_status = &z_supl_session_status;
  /* UMTS SLP Location server info */
  pdsm_pd_server_info_s_type  *p_umts_slp_lsinfo     = &p_supl_session_status->tm_sess_req_param.pd_option.umts_slp_lsinfo;
  ...
  if (p_supl_session_status->z_emergency_data.b_emergency_ni == TRUE)
  {
    ...
    if (p_supl_session_status->z_emergency_data.b_eslp_present == TRUE)
    {
      if (z_pdcomm_tcp_info.tls_session_authentication_failure == FALSE) 
      {
        ...
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"SUPL eslp address length %d ", p_supl_session_status->z_emergency_data.z_eslp_address.supl_slp_address.server_adrs_url.url_length,0,0 );
      ...
      else if (z_cell_info_cached.u_CachedCellIDValid == TRUE)
      {
        ...
        {
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"SUPL: E-SLP address configuration from IMSI failed", 0, 0, 0);
          return FALSE;
        }
      }
    }
...
#endif /* FEATURE_RD_SUPL_HSLP_N_CERTS_FRM_USIM */
    {
        /* If the callflow is MO and the server option to use is specified by PDAPI
         * use that server option instead of provisioned in NV */
        if((p_supl_session_status->u_callflow_kind != C_SUPL_SESSION_NI) &&
           (p_umts_slp_lsinfo->server_option == PDSM_SERVER_OPTION_USE_LOCAL))
        {
            z_pdcomm_tcp_info.used_auto_configured_hslp_address = FALSE;
            if(p_umts_slp_lsinfo->server_adrs.server_addr_type == PDSM_SERVER_TYPE_IPV4_ADDRESS)
            {
               p_connect_params->e_type = PD_COMMS_IP_ADDR;
               p_connect_params->ip_addr_type = PD_COMMS_IP_ADDR_IPV4;  
               p_connect_params->w_ip_port = (uint16) p_umts_slp_lsinfo->server_adrs.server_adrs.server_adrs_v4.port_id;
               p_connect_params->q_ip_addr.q_ip_addr_v4 = p_umts_slp_lsinfo->server_adrs.server_adrs.server_adrs_v4.server_adrs;
               MSG_1(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Using custom MO HSLP : 0x%x", p_connect_params->q_ip_addr.q_ip_addr_v4);
            }
            else if(p_umts_slp_lsinfo->server_adrs.server_addr_type == PDSM_SERVER_TYPE_IPV6_ADDRESS)
            {
               p_connect_params->e_type = PD_COMMS_IP_ADDR;
               p_connect_params->ip_addr_type = PD_COMMS_IP_ADDR_IPV6;
               p_connect_params->w_ip_port = (uint16) p_umts_slp_lsinfo->server_adrs.server_adrs.server_adrs_v6.port_id;
               memscpy(&p_connect_params->q_ip_addr.q_ip_addr_v6.ps_s6_addr16[0],
                       ...
                       &p_umts_slp_lsinfo->server_adrs.server_adrs.server_adrs_v6.server_adrs[0],
                       sizeof(p_umts_slp_lsinfo->server_adrs.server_adrs.server_adrs_v6.server_adrs));
               MSG(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Using IPV6 from get_pos MO MSB");
            }
            else if(p_umts_slp_lsinfo->server_adrs.server_addr_type == PDSM_SERVER_TYPE_URL_ADDRESS)
            {
               p_connect_params->e_type = PD_COMMS_HOST_NAME;
               p_connect_params->w_ip_port = SUPL_PORT_NUM_OFFICIAL_V1_0;
               memscpy(&server_addr_url[0],
                       sizeof(server_addr_url),
                       p_umts_slp_lsinfo->server_adrs.server_adrs.server_adrs_url.url,
                       p_umts_slp_lsinfo->server_adrs.server_adrs.server_adrs_url.url_length);

               /* organize UMTS PDE server URL address */
               if (((server_addr_url[0] == 'h') || (server_addr_url[0] == 'H')) &&
                   ((server_addr_url[1] == 't') || (server_addr_url[1] == 'T')) &&
                   ((server_addr_url[2] == 't') || (server_addr_url[2] == 'T')) &&
                   ((server_addr_url[3] == 'p') || (server_addr_url[3] == 'P')) &&
                    (server_addr_url[4] == ':') && (server_addr_url[5] == '/') && (server_addr_url[6] == '/'))
               {
               ...
               /* Copy the URL until we hit ":" or end of string*/
               while ((server_addr_url[i] != ':') && (server_addr_url[i] != '\0') && (i < (C_MAX_URL_SIZE_BYTE - 1)))
               {
                 p_connect_params->p_host_name[i - j] = server_addr_url[i];
                 i++;
               }
               ...
               if (i >= j)
               {
                 p_connect_params->p_host_name[i - j] = '\0';  /* null terminated FQDN */
               }
               
               MSG_SPRINTF_1( MSG_SSID_GPSSM, MSG_LEGACY_MED, "Custom H-Slp address: %s", p_connect_params->p_host_name);
               i++;
               ...
               else
               {
                        ...
                        )
                 {
                   if (((i - j) < 6) && ((i - j) >= 0))
                   {
                     port_string[i - j] = server_addr_url[i];
                   }
                   i++;
                 }
                 ...
                 p_connect_params->w_ip_port = (uint16)atoi(port_string); /* no range check if port_string > "65535" */
                 if (p_connect_params->w_ip_port == 0)
                 {
            ...
            if ( 0 == p_connect_params->w_ip_port ) 
            {
              p_connect_params->w_ip_port = SUPL_PORT_NUM_OFFICIAL_V1_0;
            }
             MSG_1(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Using Port from get_pos MO MSB: %d",p_connect_params->w_ip_port);
        }
        else /* Use default MO SLP */
        {
           MSG(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Using NV for SLP address");    
           p_connect_params->w_ip_port = z_umts_nv_item_ram_copy.slp_port_number;
               ...
               (z_pdcomm_tcp_info.tls_session_authentication_failure == TRUE) )
           {
              ...
              if(retVal != TRUE)
              {
                 MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"SUPL: H-SLP address configuration from IMSI failed", 0, 0, 0);
                 return FALSE;
              }
              else
              {
...
#else
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Not constructing HSLP address from MCC MNC",0,0,0);
    return FALSE;
#endif
}
...
static boolean supl_comm_eslp_address_build_from_mcc_mnc (char * constructedHslpAddr, cgps_CellInfoCachedStruct *p_cached_cell_info)
{
    ...
    */
    ...
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"SUPL: generating SLP from IMSI",0,0,0);
    ...
    return TRUE;
#else
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Not constructing ESLP address from MCC MNC",0,0,0);
    return FALSE;
#endif
}
...
pd_comms_return_type supl_comm_disconnect( boolean b_abort )
{
  ...
  disconnect_transaction_id = z_comms_connect_params_tcp.q_transaction_id;
  ...
  if(ret_val == PDCOMM_RESULT_OK)
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_LOW,"TM_UMTS_UP SUPL disconnected", 0, 0, 0);
  }
  else if(ret_val == PDCOMM_RESULT_WAIT)
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_LOW,"TM_UMTS_UP SUPL wait_disconnect", 0, 0, 0);
  }
  else
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"pd_comms_app_disconnect returned error: %d",ret_val,0,0);
  }
  return ret_val;
...
static pd_comms_return_type supl_comm_read(uint16 length, pd_comms_link_type link_type, boolean wait_all )
{
  ...
  if ( link_type == LINK_TCP )
  {
    ...
    if ( length > 2 )
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Ask PDCOMM to read TCP SUPL payload %u bytes", read_params.w_data_len, 0, 0);
    }

    if ( ret_val != PDCOMM_RESULT_WAIT )
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Error reading TCP SUPL payload",0,0,0);
    }
  ...
  else if ( link_type == LINK_SMS )
  {
    ...
    if ( length > 2 )
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Ask PDCOMM to read SMS SUPL payload %u bytes", read_params.w_data_len, 0, 0);
    }

    if ( ret_val != PDCOMM_RESULT_WAIT )
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Error reading SMS SUPL payload",0,0,0);
    }
  ...
  else if ( link_type == LINK_UDP )
  {
    ...
    if ( length > 2 )
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Ask PDCOMM to read UDP SUPL payload %u bytes", read_params.w_data_len, 0, 0);
    }

    if ( ret_val != PDCOMM_RESULT_WAIT )
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Error reading UDP SUPL payload",0,0,0);
    }
  ...
  else
  {
    ...
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Wrong link type when comm read %d", link_type, 0, 0);
  ...
  if (client_ts_data->u.gw_pp.tpdu_type == WMS_TPDU_DELIVER)
  {
    if (client_ts_data->u.gw_pp.u.deliver.user_data_header_present)
    {
      ...
      for(i=0; i<client_ts_data->u.gw_pp.u.deliver.user_data.num_headers; i++)
      {
        ...
        {
          /* Is a SUPL MT-SMS ? */
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"MT-SMS dest_port=%u",
                   client_ts_data->u.gw_pp.u.deliver.user_data.headers[i].u.wap_16.dest_port,
                   0,
                   0);
          ret_value = TRUE;
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"No user_data header", 0, 0, 0);
    }
  }
  else
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"SMS not WMS_TPDU_DELIVER", 0, 0, 0);
  }
  return ret_value;
}
...
)
{
  ...
  if (msg_ptr == NULL)
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Null Parameter Passed in supl_mt_sms_ck", 0, 0, 0);
    return FALSE;
  }
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Incorrect Message Mode", 0, 0, 0);
    return FALSE;
  }
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Memory Allocation Failed!", 0, 0, 0);
  }
  else
  {
    if(msg_ptr->msg_hdr.message_mode == WMS_MESSAGE_MODE_CDMA)
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Teleservice id recieved %d", msg_ptr->u.cdma_message.teleservice,0,0);
    ...
    else if (msg_ptr->msg_hdr.message_mode == WMS_MESSAGE_MODE_GW)
    {
      ...
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"SMS Message Decode Failed!", 0, 0, 0);
      }
    }
    else
    {
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"supl_mt_sms_ck = %d", ret_value, 0, 0);
  return ret_value;
...
)
{
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"supl_comm_sms_verify_read_cb CALLED, %d",u_isWapPush,0,0);

  if ((!p_data) || (!p_ts_data))
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Null pointers for p_data/p_ts_data", 0, 0, 0);
    return FALSE;
  }	
  ...
  {
     MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Message mode is CDMA, but not a teleservice-4115 message", 0, 0, 0);
	 return FALSE; 
  }
  ...
  if ( !u_isWapPush )
  {
    ...
    if ( !u_is_mt_sms )
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"supl_comm_sms_verify_read_cb: SMS not WAP-PUSH or MT-SUPL", 0, 0, 0);
      *p_MesgLen = 0;
      return FALSE;
    }
  }
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Wrong SMS format",0,0,0);
    return FALSE;
  }
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"supl_comm_sms_verify_read_cb: # of hdrs = %d",num_headers,0,0);

  for ( cnt = 0; cnt < num_headers; cnt++ )
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"supl_comm_sms_verify_read_cb: hdr_id = %x",headers[cnt].header_id,0,0);
    ...
    if ( headers[cnt].header_id == WMS_UDH_CONCAT_8 )
    {
      ...
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"supl_comm_sms_verify_read_cb: msg_ref = %d, total_sm = %d, seq_num = %d", msg_ref, total_sm, seq_num);
      ...
      if (wap_push_multipart_supl_init_found && wap_push_multipart_in_progress && ( msg_ref != wap_push_multipart_msg_ref ))
      {
                                          ...
                                          &push_content_len_local ) )
        {
          ...
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Wap Push does not contain SUPL_INIT, ignoring it",0,0,0);
          *p_MesgLen = 0;
        ...
        else
        {
          ...
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Inconsistent Wap Push, but contains SUPL_INIT, treating as fresh NI attempt",0,0,0);
      ...
      else
      {
        ...
        {
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"supl_comm_sms_verify_read_cb: Inconsistent WAP Push segment received - Discarding",0,0,0);
          tm_umts_supl_comm_sms_read_state_init();
          *p_MesgLen = 0;
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"No space remaining in Wap Push Buffer",0,0,0);
  }

  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"supl_comm_sms_verify_read_cb: wap_push_multipart_in_progress = %d, seq_num = %d, wap_push_multipart_total_sm = %d",wap_push_multipart_in_progress,seq_num,wap_push_multipart_total_sm);

  if ( wap_push_multipart_in_progress && 
       seq_num != wap_push_multipart_total_sm &&
       !u_is_mt_sms)
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"supl_comm_sms_verify_read_cb: Partial message check for SUPL Init",0,0,0);
                                      ...
                                      &push_content_len ) )
    {
      ...
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"supl_comm_sms_verify_read_cb: WAP_PUSH truncated", 0, 0, 0);
    ...
    else
    {
      ...
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"SUPL MULTIPART WAP PUSH WAIT timer started",0,0,0);
       ...
       ( wap_push_multipart_in_progress && seq_num == wap_push_multipart_total_sm ) )
  {
    ...
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"supl_comm_sms_verify_read_cb: Complete message check for SUPL Init",0,0,0);
  ...
  else
  {
    ...
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"supl_comm_sms_verify_read_cb: WAP PUSH Validity not checked as not the last segment",0,0,0);
    *p_MesgLen = 0;
    return TRUE;  
  }
...
static boolean supl_comm_dsm_verify_partial_wap_push( uint8 *data, uint16 data_len, uint8 **ppPushContent, uint16 *pPushLen )
{
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"supl_comm_dsm_verify_partial_wap_push: data_len = %d", data_len,0,0);
  if ( data_len < 4 )
  {
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"supl_comm_dsm_verify_partial_wap_push: PDUType = %d", PDUType,0,0);
  if ( PDUType != 0x6 ) /* PDU Type 0x6 == Push */
    return( FALSE );

  PushHeaderLen = *pUserData++; /* Push Header Length */

  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"supl_comm_dsm_partial_verify_wap_push: PushHeaderLen = %d", PushHeaderLen,0,0);
  if ( PushHeaderLen == 0 )
    return( FALSE );
  ...
  else if ( data_len >= 7)
  {
    ...
    else
    {
      ...
      else
      {
        ...
        {
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"supl_comm_dsm_verify_wap_push: return false",0,0,0);
...
static boolean supl_comm_dsm_verify_wap_push( uint8 *data, uint16 data_len, uint8 **ppPushContent, uint16 *pPushLen )
{
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"supl_comm_dsm_verify_wap_push: data_len = %d", data_len,0,0);
  if ( data_len < 3 )
  {
  ...
  pUserData++;                     /* TID */
  PDUType = *pUserData++;          /* PDU Type */
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"supl_comm_dsm_verify_wap_push: PDUType = %d", PDUType,0,0);
  if ( PDUType != 0x6 ) /* PDU Type 0x6 == Push */
  {
    return( FALSE );
  }
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"supl_comm_dsm_verify_wap_push: PushHeaderLen = %d", PushHeaderLen,0,0);
  if ( PushHeaderLen < 4 )
  {
    return( FALSE );
  }
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"supl_comm_dsm_verify_wap_push: PushHeaderLen > data_len - 3 = %d", data_len - 3,0,0);
  ...
  if ( (PushHeaderLen >= 33) && !strcmp("application/vnd.omaloc-supl-init", (char *)pUserData ) )
  {
    pUserData += 33; /* content type string plus null terminator */
    PushHeaderLen -= 33;
  }
  /*check for assigned number for content type ID code of "application/vnd.omaloc-supl-init" */
  else
  {
    q_word = (uint32) (*pUserData) << 24;
    q_word |= (uint32) ( *(pUserData + 1) ) << 16;
    ...
    else
    {
      ...
      else
      {
        ...
        {
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"WAPPUSH not for SUPL: ContentType check failed",0,0,0);
          return FALSE; /* WAP PUSH not for SUPL */
        }
      }
    }
  ...
  while (PushHeaderLen > 1)
  {
    if (*pUserData == OMNA_APP_ID)
    {
      ...
      else
      {
        ...
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"0xAF not following by x-application-Id-filed, keep searching",0,0,0);
        continue; 
      }

      /* Skip remaining header bytes, to get to the data payload. */
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"x-application-id and its field valid",0,0,0);
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"WAPPUSH not for SUPL: no valid x-wap-application-id or its filed",0,0,0);
  return FALSE;
...
boolean supl_comm_generate_hash ( uint8 *p_raw_pdu, uint16 w_raw_pdu_len )
{
  ...
  if (NULL == p_raw_pdu)
  {
     MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"SUPL: NULL parameter passed for PDU data", 0,0,0);
     return FALSE;
  }
 
  if (TRUE == b_pending_hash) 
  {
     MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Drop NI request, since Hash Resp pending",0,0,0);
     return FALSE;
  }
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"SUPL: ASN1 decoding failed during Hash Gen", 0,0,0);
    ...
    if (p_suplInit->m.notificationPresent == 1  )
    {
      if ( p_suplInit->notification.m.ver2_Notification_extensionPresent == 1 )
      {
        ...
        {                                                                                            
          ...
          {
            MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"SUPL_COMM: Using ESLP to generate hash", 0,0,0);
  ...
  else
  {
    if (b_eslp_present == FALSE)
    {
      if (z_umts_nv_item_ram_copy.slp_address[0] != '\0')
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Using SUPL address from NV for hash emergeny as ESLP is not provided",0,0,0);
        /* if no ESLP provided used the NV configured SLP address */
      ...
      {
        /* construct the slp address from MCC/MNC of the current servign network */
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Using SUPL address from IMSI for hash emergeny as ESLP is not provided",0,0,0);
        if (z_cell_info_cached.u_CachedCellIDValid == TRUE)
        {
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"cached cell id valid ",0,0,0);
  ...
  if (asn1_decode_status== 0 || b_ret == FALSE)
  {
    ...
    if (b_ret == FALSE)
    {
       MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"SUPL: Hash Generation failed", 0,0,0);
       return FALSE;
    }
  }
...
void tm_umts_up_supl_comm_ni_sms(uint32 length, uint8 *data)
{
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Invalid SUPL NI SMS (len=%d data=%x)",length,data,0);
    return;
  }
  ...
  {    
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"SUPL NI SMS injection failed",0,0,0);
  }
