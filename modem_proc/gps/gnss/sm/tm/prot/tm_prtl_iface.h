...
/*===========================================================================
  ...
  $Header: //components/rel/gnss8.mpss/11.1/gnss/sm/tm/prot/tm_prtl_iface.h#2 $  
...
12/04/18   py      Payload prtl_carrier_nv_refressh_s_type updated to include sub
04/25/18   mj      Define new stop reason TM_STOP_REASON_SUPL_SESS_PREEMT
...
/* Session Power Modes from LocMW to TM-Core */
typedef enum
{
  TM_SESS_POWER_MODE_NONE = -1,
  TM_SESS_POWER_MODE_INVALID,
  TM_SESS_POWER_MODE_IMPROVED_ACCURACY,
  TM_SESS_POWER_MODE_NORMAL,
  TM_SESS_POWER_MODE_BACKGROUND_DEFINED_POWER,
  TM_SESS_POWER_MODE_BACKGROUND_DEFINED_TIME,
  TM_SESS_POWER_MODE_BACKGROUND_KEEP_WARM
} tm_sess_power_mode_e_type;

typedef struct
{
  tm_sess_power_mode_e_type e_PowerMode;
  uint32 q_TbmMsecs;
} tm_sess_power_mode_s_type;
...
typedef enum
{
  ...
  TM_STOP_REASON_BACKGROUND_LPPM_CANNOT_ENGAGE, /* Background LPPM cannot engage */
  TM_STOP_REASON_SUPL_SESS_PREEMT,      /* Preempt ongoing SUPL session due to new session start */
...
typedef struct
{
  ...
  tm_sess_power_mode_s_type      lppm_mode;
  ...
  pdsm_pd_position_special_req_s_type  specialReqType;   /* Special Client Request types*/
...
typedef enum
{
  ...
  PRTL_EVENT_TYPE_UMTS_OTA_GPS_RESET,
  PTRL_EVENT_TYPE_DBH_POS_REPORT, /* DBH position report */
  PRTL_EVENT_TYPE_READ_NEW_IMSI, /* PRTL event to read new IMSI during sub ready event from RUIM */
} prtl_event_e_type;
...
typedef struct
{
  pdsm_wifi_on_demand_info_list_s_type z_wifi_scan_info; /*Wifi scan info*/
}prtl_wifi_scan_event_s_type; 

/* RUIM Nv Read structure*/
typedef enum
{
  PRTL_EVENT_RUIM_CURR_NAM_NV_READ,	
  PRTL_EVENT_RUIM_MSID_NV_READ,
  PRTL_EVENT_RUIM_1X_NV_READ,
  PRTL_EVENT_RUIM_VX_NV_READ,
  PRTL_EVENT_RUIM_CURR_NAM_READ_OP,
...
typedef struct
{
  ...
  sys_modem_as_id_e_type sub;  /* denotes which sub refresh event came on*/
}prtl_carrier_nv_refressh_s_type;

typedef struct
{
  boolean v_read_new_imsi; 
}prtl_read_new_imsi_s_type;
...
typedef union
{
  ...
  prtl_read_new_imsi_s_type  read_new_imsi_payload;  /*PRTL event to ready new IMSI during sub ready event from RUIM */
} prtl_event_u_type;
...
typedef enum
{
  ...
  TM_POST_DATA_TYPE_BDS_GLOBAL_IONO         /// BDS Global Iono
} tm_post_data_e_type;
...
typedef union
{
  ...
  bds_IonoStructType               global_iono_bds_data;
