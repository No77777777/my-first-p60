...
INITIALIZATION AND SEQUENCING REQUIREMENTS
  ...
  Qualcomm Atheros Confidential and Proprietary. All Rights Reserved. 
  Copyright (c) 2015, 2016, 2017, 2019 Qualcomm Technologies, Inc. 
  All Rights Reserved.
...
/*===========================================================================
  ...
  $Header: //components/rel/gnss8.mpss/11.1/gnss/sm/tm/umts_cp/src/tm_umts_cp.c#1 $
  ...
  06/28/17   mj      Fix protocol index selection based on E911 state
...
void tm_umts_cp_prtl_iface_init(void)
{
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"tm_umts_cp_init", 0, 0, 0);
  ...
  tm_umts_common_utils_read_nv_at_bootup();
  #endif
...
)
{
  ...
  if((NULL == p_event_payload) &&
     (event_type != PRTL_EVENT_TYPE_UMTS_OTA_GPS_RESET)) /* For OTA reset, the event payload can be NULL */
  {
     ...
     )
  {
    ...
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"event_type %u not supported", event_type, 0, 0 );
    return FALSE;
  }
  ...
  {
    if (CGPS_SRV_SYS_GSM == phone_state_info.srv_system)
    {
  ...
  {
    if (CGPS_SRV_SYS_GSM == phone_state_info.emerg_srv_system)
    {
      #if (defined(FEATURE_CGPS_UMTS_CP_GSM) && defined (FEATURE_CGPS_UMTS_CP_WCDMA))
        tbl_ndx = 1;
      #endif
  ...
  if ( event_type == PRTL_EVENT_TYPE_PHONE )
  {
                                          ...
                                          TM_PHONE_EVENT_E911_CALL_END) )
    {
        ...
        tm_cm_iface_get_phone_state(&phone_state_info);
        if (CGPS_SRV_SYS_GSM == p_event_payload->phone_event_payload.payload.emerg_call_info.emerg_call_sys_mode)
        {
  ...
  else if (PRTL_EVENT_TYPE_UMTS_OTA_GPS_RESET == event_type)
  {
    #ifdef FEATURE_CGPS_UMTS_CP_WCDMA
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"fcn_table NULL", 0, 0, 0);
    return FALSE;
  }
