...
/*===========================================================================
  ...
  $Header: //components/rel/gnss8.mpss/11.1/gnss/sm/tm/umts_cp/wcdma/src/tm_umts_cp_wcdma_api.c#1 $
...
09/25/17   rk      Clean-up WCDMA non-network connected session when E911 call end received.
06/19/17   skm     E911 call end should turn off MGP receiver turned ON during early start
04/25/17   gk      stop session should handle tm core clean up for E911 if another session starts
...
static void tm_umts_cp_wcdma_e911_sess_end();
 ...
 )
{
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"UMTS-CP: start_sess_req_handler", 0, 0, 0);
  ...
  if ( tm_umts_cp_wcdma_pdapi_mapping(start_sess_req_param) )
  {
    ...
    if ( p_umts_cp_wcdma_session_status->e_gnssPosMode == eWCDMA_POSITION_MODE_AGPS_UE_BASED )
    {
      ...
      if ( p_umts_cp_wcdma_session_status->u_callflow_kind == C_UMTS_CP_WCDMA_SESSION_UI )
      {
        ...
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"MO-MB starts", 0, 0, 0);
    ...
    else if ( p_umts_cp_wcdma_session_status->e_gnssPosMode == eWCDMA_POSITION_MODE_AGPS_UE_ASSISTED )
    {
      ...
      if ( p_umts_cp_wcdma_session_status->u_callflow_kind == C_UMTS_CP_WCDMA_SESSION_UI )
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"MO-MA starts", 0, 0, 0);
    ...
    if ( p_umts_cp_wcdma_session_status->u_callflow_kind == C_UMTS_CP_WCDMA_SESSION_UI )
    {
      if ( p_umts_cp_wcdma_session_status->e_gnssPosMode == eWCDMA_POSITION_MODE_AGPS_UE_ASSISTED )
      {
        ...
        if ( !tm_umts_cp_request_molr(start_sess_req_param, LOCATION_ESTIMATE, NULL) )
        {
          ...
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Attempt sending MoLrInvoke failed, reject TmCore sess. req", 0, 0, 0);
  ...
  else
  {
    ...
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"GPS mode from TmCore invalid, reject TmCore sess. req", 0, 0, 0);
    ret_val = PRTL_START_SESS_REQ_NOT_ALLOWED;
  }
 ...
 )
{
  ...
  {
    MSG_2(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"CP-WCDMA: Another emergency session started. clean up WCDMA_SESS ", sess_handle, stop_reason);
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"CP-WCDMA: MT-LR sess in progress. Stop attempt rejected ", 0, 0, 0);
    u_ret_val = FALSE;
    return ( u_ret_val );
  }
  #endif
 ...
 )
{
  ...
  switch ( sess_info_type )
  {
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"CP-WCDMA: got FINAL LR", 0, 0, 0);
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"CP-WCDMA: got UseRefPos", 0, 0, 0);
    ...
    case PRTL_SESS_INFO_STATUS:
    {
      ...
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"response to additional assist. request arrives", 0, 0, 0);
      }
      ...
      if ( p_umts_cp_wcdma_session_status->u_force_request_ref_location == TRUE )
      {
        ...
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"SessInfo wishlist with forcing GpsRefLoc = %lx",
      ...
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"After adj. SessInfo wishlist = 0x%lx (GPS) 0x%lx (GLO), additionaReq = %u ",
              p_sess_info_param->aiding_data_status.z_assist_data_status.q_WishListMask,
              p_sess_info_param->aiding_data_status.z_assist_data_status.z_GloAssistData.q_WishListMask,
    ...
    default:
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Case (%d) not supported in tm_umts_cp_wcdma_sess_info_handler", sess_info_type, 0, 0);
      break;
...
void tm_umts_cp_wcdma_timer_cb_handler( void *timer_data )
{
  ...
  if ( timer_param != NULL )
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"timerId 0x%lX expired", timer_param->q_TimerId, 0, 0);
    ...
    if ( (timer_param->q_TimerId & TM_UMTS_CP_WCDMA_TIMER_DISPATCH_MASK) )
    {
      switch ( timer_param->q_TimerId )
      {
        case CP_WCDMA_CF_TIMER_ID:
        {
          ...
          {
            MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"UMTS-CP-W: Timeout waiting network message", 0, 0, 0);
          ...
          {
            MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"UMTS-CP-W: Generate measurement timeout", 0, 0, 0);
            tm_umts_cp_wcdma_terminate_timer_cancel_cf_state(C_UMTS_CP_WCDMA_CF_STATE_TIMER_RRC_REPORT);
          ...
          {
            MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"UMTS-CP-W: Wait-MoLrReturnResult timeout", 0, 0, 0);
          ...
          else if ( p_umts_cp_wcdma_session_status->e_cf_state == C_UMTS_CP_WCDMA_CF_STATE_MT_GUARD_TIME )
          {
            ...
            MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"UMTS-CP-W: MT guard timer timeout", 0, 0, 0);
            tm_umts_cp_wcdma_terminate_timer_cancel_cf_state(C_UMTS_CP_WCDMA_CF_STATE_MT_GUARD_TIME);
        ...
        {
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"CP-WCDMA: MaxMgpOn timer expired", 0, 0, 0);
        ...
        default:
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Timer ID %u egnored", timer_param->q_TimerId, 0, 0);
 ...
 )
{
  ...
  switch ( event_type )
  {
    ...
    case PRTL_EVENT_TYPE_LCS:
    {
      ...
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"UMTS-CP-W: TmCore cb LCS type with event CLINET_INIT", 0, 0, 0); /* QQQ: ? */
      }
      else if ( p_event_payload->lcs_event_payload.lcs_event == PRTL_LCS_EVENT_TYPE_CLINET_RELEASE )
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"UMTS-CP-W: TmCore cb LCS type with event CLINET_RELEASE", 0, 0, 0); /* QQQ: ? */
      }
      break;
    }
    ...
    case PRTL_EVENT_TYPE_PHONE:
    {
      ...
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"UMTS-CP-W: CM delivers LCS_MOLR_CONF", 0, 0, 0);
      ...
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"UMTS-CP-W: CM delivers LCS_MOLR_COMP", 0, 0, 0);
      ...
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"UMTS-CP-W: CM delivers MTLR_NTFY_IND", 0, 0, 0);
      ...
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"UMTS-CP-W: CM confirms LCS-LocNotifResp queued to NAS ", 0, 0, 0);
      }
               ...
               (p_event_payload->phone_event_payload.event == TM_PHONE_EVENT_E911_CALL_CONNECT))
      {
        /*
      ...
      else if ( TM_PHONE_EVENT_E911_CALL_END == p_event_payload->phone_event_payload.event )
      {
        MSG(MSG_SSID_GPSSM,MSG_LEGACY_MED,"TM_WCDMA_CP: got Emergency end event");
        tm_umts_cp_wcdma_e911_sess_end();
      }
      break;
    ...
    default:
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"UMTS-CP-W: wrong prtl_event_type %u", event_type, 0, 0);
...
void tm_umts_cp_wcdma_init( void )
{
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"tm_cp_wcdma init", 0, 0, 0);
...
tm_umts_cp_wcdma_ue_pos_capability_struct_type tm_umts_cp_wcdma_ue_pos_capability_get( void )
{
  ...
  if ( tm_umts_common_glonass_enabled() && tm_umts_cp_wcdma_a_glo_rrc_cp_enabled() )
  {
    ...
    if ( tm_core_pos_mode_allows_aglo_msb_cp( TM_POS_PRTL_RRC ) &&
         tm_core_pos_mode_allows_aglo_msa_cp( TM_POS_PRTL_RRC ) )
    {
      z_pos_capabilities.ganss_supported_list[0].ganss_mode = GANSS_MODE_BOTH;
    }
    else if ( tm_core_pos_mode_allows_aglo_msa_cp( TM_POS_PRTL_RRC ) )
    {
      z_pos_capabilities.ganss_supported_list[0].ganss_mode = GANSS_MODE_NW_BASED;
    }
    else if ( tm_core_pos_mode_allows_aglo_msb_cp( TM_POS_PRTL_RRC ) )
    {
...
void tm_umts_cp_wcdma_e911_sess_start()
{
  ...
  if ((phone_state_info.emerg_srv_system == CGPS_SRV_SYS_WCDMA) &&
  ...
  {    
      if ( (z_umts_cp_wcdma_session_status.e_cf_state == C_UMTS_CP_WCDMA_CF_STATE_NONE) &&
           ...
           (z_umts_cp_wcdma_session_status.tm_core_session_started == FALSE)
         )
      {                 
...
FUNCTION  tm_umts_cp_wcdma_e911_sess_end

DESCRIPTION
  This function ends an ongoing positioning session when E911 call ends.
...
===========================================================================*/
void tm_umts_cp_wcdma_e911_sess_end()
{
  umts_cp_wcdma_session_status_s_type *p_umts_cp_wcdma_session_status = &z_umts_cp_wcdma_session_status;

#ifdef FEATURE_GNSS_NON_STANDARD_SUPPORT
  if ( ( C_UMTS_CP_WCDMA_CF_STATE_TIMER_RRC_REPORT == p_umts_cp_wcdma_session_status->e_cf_state ) &&
       ( TRUE == p_umts_cp_wcdma_session_status->u_network_session_on ) )
  {
    tm_umts_cp_wcdma_encode_rrc_msrReport_err( RRC_UNDEFINED_ERROR, NULL );
  }
  p_umts_cp_wcdma_session_status->u_network_session_on = FALSE;
  tm_umts_cp_wcdma_inform_core_stop_session( TM_STOP_REASON_USER_TERMINATED );
#else
  /* End the early wcdma session if NILR or MO is not started. */
  if ( FALSE == p_umts_cp_wcdma_session_status->u_network_session_on )
  {
    tm_umts_cp_wcdma_inform_core_stop_session( TM_STOP_REASON_USER_TERMINATED );
  }
