...
/*===========================================================================
  ...
  $Header: //components/rel/gnss8.mpss/11.1/gnss/sm/tm/umts_cp/wcdma/src/tm_umts_cp_wcdma.c#1 $
  ...
  06/21/19   mj      Location Privacy: Set correct response for MSA and MSB
  02/27/19   PY     Send ERROR if cached measurements are not good before session termination
  02/27/19   PY     While cache encode is success, dont wait for max sequence number msr blk.  
  ...
  04/28/17   RK     Cache the full position information from GNSS-engine to report GLO position
                     data correctly.
  11/15/16   kpn     Send TUNC based on explicit knowledge of msec-known measurement state
  11/02/16   rk      Fix the MO LocationEstimate issue where the accuracy is overwritten with zero.
  07/05/16   rk      Pass all measurements to protocols. It will decide which measurements should be
                     reported to server.
  02/17/16   rk      Clear the measurement report structure once for each measurement sets.
  02/17/16   rk      Compute Glonass Time when GPS UTC & GPS Reference Time is injected in any order.
 ...
 *  is_cached_meas : Is incoming measurement taken from cache or current epoch
 *
 * Return:
 ...
 **********************************************************************************************************************
 */
static boolean cp_wcdma_send_rrc_msrReport_meas( sm_GpsMeasRptStructType *p_prm_data, boolean status_check_needed, boolean is_cached_meas );
                                                       ...
                                                       rrc_meas_rpt_struct_type *const p_measrpt )
{
  ...
  if( (NULL == p_prm_data) ||
      (p_prm_data->z_MeasBlk.z_GnssHeader.e_MeasBlkSrc != GNSS_MEAS_BLK_SRC_GLO) )
  {
    return ( FALSE );
  }
  ...
  if ( p_GloMeasRpt != NULL )
  {
    ...
    if (p_GloMeasRpt->z_GnssHeader.u_NumSvs < N_ACTIVE_GLO_CHAN)
    {
      u_TotalGloSvs = p_GloMeasRpt->z_GnssHeader.u_NumSvs;
    }
    else
    {
    ...
    for ( u_i = 0; (u_i < u_TotalGloSvs); ++u_i, ++p_GloMeasPerSv )
    {
      if ((p_GloMeasPerSv->w_Cno > 0) &&
          (FALSE == gnss_SvIsBlacklisted(&tm_core_info.config_info.z_GnssPersistSV, p_GloMeasPerSv->w_Sv)))
      {
        /* If status_check_needed is FALSE, explicitly verify the msec-known-bits. */
        if ((FALSE == v_tunc_to_include) && (FALSE == status_check_needed))
        {
          /* If atleast one of the SVs doesn't have the msec-known-bits set, then send TUNC. */
          if (FALSE == gnss_meas_status_bits_check(p_GloMeasPerSv->q_MeasStatus, TRUE))
          {
            v_tunc_to_include = TRUE;
          }
        }

        if (gnss_meas_status_bits_check(p_GloMeasPerSv->q_MeasStatus, status_check_needed))
        {
          ...
          f_Num = (p_GloMeasPerSv->z_FiltMeas.f_SvSpeed - p_GloMeasRpt->z_GnssHeader.z_MeasClock.z_Freq.f_ClkFreqBias);
          ...
          d_AdjSubMsec = (double)p_GloMeasPerSv->z_FiltMeas.z_SvTime.f_SubMs
             + (double)p_GloMeasRpt->z_GnssHeader.z_MeasClock.z_Time.z_GpsTime.f_ClkTimeBias;
        ...
        {
          /* If status_check_needed is FALSE, nothing to do, it was already evaluated.
             Otherwise it means that atleast one of SVs doesn't have msec-known-bits set, so send TUNC. */
          if ((FALSE == v_tunc_to_include) && (TRUE == status_check_needed))
          {
            v_tunc_to_include = TRUE;
          }
        }
    ...
    */
    ...
    /* Sanity check */
    if ((p_GloMeasRpt->z_GnssHeader.z_MeasClock.z_Time.z_GloTime.u_FourYear == 255) ||
        (p_GloMeasRpt->z_GnssHeader.z_MeasClock.z_Time.z_GloTime.w_Days == 65535) ||
        (p_GloMeasRpt->z_GnssHeader.z_MeasClock.z_Time.z_GloTime.q_GloMsec > DAY_MSECS)
        )
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,
            "GlonassTOD invalid: Num4Year=%u, NumDaysIn4Year=%u, ToD_ms=%lu",
            p_GloMeasRpt->z_GnssHeader.z_MeasClock.z_Time.z_GloTime.u_FourYear,
            p_GloMeasRpt->z_GnssHeader.z_MeasClock.z_Time.z_GloTime.w_Days,
            p_GloMeasRpt->z_GnssHeader.z_MeasClock.z_Time.z_GloTime.q_GloMsec);
      return ( FALSE );
    }
    else
    {
      ...
      /* Protocol : (0..3599999) */
      p_measrpt->ganss_measured_results.u.ganss_ReferenceTimeOnly.gANSS_tod = p_GloMeasRpt->z_GnssHeader.z_MeasClock.z_Time.z_GloTime.q_GloMsec % HOUR_MSECS;
      ...
      if (v_tunc_to_include)
      {
        ...
        /* Protocol (0..127) */
        p_measrpt->ganss_measured_results.u.ganss_ReferenceTimeOnly.gANSS_tod_uncertainty = tm_rrc_utils_microsec_to_gadk(p_GloMeasRpt->z_GnssHeader.z_MeasClock.z_Time.z_GloTime.f_ClkTimeUncMs * 1000);
      }
    }
...
void tm_umts_cp_wcdma_session_cleanup( void )
{
  ...
  if ( p_umts_cp_wcdma_session_status->pz_CacheLrNavPos != NULL )
  {
    os_MemFree( (void **)&(p_umts_cp_wcdma_session_status->pz_CacheLrNavPos) );
  }
#endif
...
void tm_umts_cp_wcdma_inform_core_nt_assist_end( boolean u_permit_qostime_reset )
{
  ...
  if ( p_umts_cp_wcdma_session_status->u_callflow_kind == C_UMTS_CP_WCDMA_SESSION_UI )
  {
         ...
         (p_umts_cp_wcdma_session_status->e_gnssPosMode == eWCDMA_POSITION_MODE_A_GPS_GLO_UE_ASSISTED) )
    {
      ...
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"UMTS-CP-W: ntwk over, cont, prm_qos=%u, acc=%lu",
              p_req_param->continue_param.prm_qos,
              p_req_param->continue_param.accuracy_threshold,
              0);
    }
    else
    {
      ...
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"UMTS-CP-W: ntwk over, cont, lr_qos=%u, acc=%lu",
              p_req_param->continue_param.lr_qos,
              p_req_param->continue_param.accuracy_threshold,
              0);
    }
  ...
  else if ( p_umts_cp_wcdma_session_status->u_callflow_kind == C_UMTS_CP_WCDMA_SESSION_NI )
  {
    ...
    if ( tm_session_req == TM_SESS_REQ_START )
    {
           ...
           (p_umts_cp_wcdma_session_status->e_gnssPosMode == eWCDMA_POSITION_MODE_A_GPS_GLO_UE_ASSISTED) )
      {
        ...
        if ( tm_umts_common_utils_emergency_effect() == C_EMERGENCY_HANDLING )
        {
        ...
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"UMTS-CP-W: ntwk over, start, prm_qos=%u, acc=%lu, req=%u",
      ...
      else  /* UeBased */
      {
        ...
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"UMTS-CP-W: ntwk over, start, lr_qos=%u, acc=%lu",
                p_req_param->start_param.lr_qos,
                p_req_param->start_param.accuracy_threshold,
                0);
    ...
    else  /************************* Continue ********************************/
    {
           ...
           (p_umts_cp_wcdma_session_status->e_gnssPosMode == eWCDMA_POSITION_MODE_A_GPS_GLO_UE_ASSISTED) )
      {
        ...
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"UMTS-CP-W: ntwk over, cont, prm_qos=%u, acc=%lu, req=%u",
                p_req_param->continue_param.prm_qos,
                p_req_param->continue_param.accuracy_threshold,
                tm_session_req);
      }
...
void tm_umts_cp_wcdma_set_cf_state( umts_cp_wcdma_cf_state_enum_type e_new_cf_state, boolean u_cf_molr_invoke_timer )
{
      ...
      )
  {
    ...
    if ( e_new_cf_state == C_UMTS_CP_WCDMA_CF_STATE_WAIT_RRC )
    {
      ...
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"UMTS-CP-W: start WaitRRC timer: %u sec.", z_umts_nv_item_ram_copy.default_smlc_comm_timeout_sec, 0, 0);
    }
    else if ( e_new_cf_state == C_UMTS_CP_WCDMA_CF_STATE_WAIT_RETURN_RESULT )
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"UMTS-CP-W: start WaitMoLrRetRes timer: %u sec.", z_umts_nv_item_ram_copy.default_smlc_comm_timeout_sec, 0, 0);
    }
  ...
  else if ( e_new_cf_state & C_UMTS_CP_WCDMA_CF_STATE_TIMER_RRC_REPORT )
  {
    ...
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"UMTS-CP-W: start RRC report timer: %u sec.", q_rrc_timer, 0, 0);
  ...
  else if ( e_new_cf_state & C_UMTS_CP_WCDMA_CF_STATE_MT_GUARD_TIME )
  {
    ...
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Start allowing RRC for the next %u seconds",
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"UMTS-CP-W: set timer; cf state = %u", (uint8)e_new_cf_state, 0, 0);
...
void tm_umts_cp_wcdma_lcs_molr_conf_proc( tm_cm_sups_data_s_type *p_sups_data )
{
  ...
  if ( p_umts_cp_wcdma_session_status->e_cf_state == C_UMTS_CP_WCDMA_CF_STATE_WAIT_RETURN_RESULT )
  {
    ...
    if ( p_molr_res->data_type == LCS_MOLR_INFO )
    {
      ...
      if ( p_data != NULL )
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED," UMTS-CP-W: MOLR-RES LocEst received", 0, 0, 0);
...
void tm_umts_cp_wcdma_rrc_proc( prtl_rrc_event_s_type *rrc_event_payload_ptr )
{
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"CP-WCDMA: NULL ptr passed to tm_umts_cp_wcdma_rrc_proc",0, 0, 0);
    return;
  }
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Undefined RRC event type: %u",
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"CP-WCDMA: AS_ID sent from RRC failed validation", 0, 0, 0);
    return;
  }	
  #endif
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"CP-WCDMA got RRC_MEAS_CTRL", 0, 0, 0);
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"CP-WCDMA got RRC_ASSIST_DATA", 0, 0, 0);
  ...
  else if ( p_rrc_meas_param->params_type == RRC_STATE_CHANGE_PARMS )
  {
    ...
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"CP-WCDMA got RRC_STATE_CHANGE", 0, 0, 0);
    ...
    switch ( p_rrc_meas_param->u.trans_params.trans_cmd_type )
    {
       ...
       {
         MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"RRC_STATE_CHANGE MeasDelete/MeasDeleteAll", 0, 0, 0);
       ...
       case RRC_MEAS_SAVE:
       {
         MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"RRC_STATE_CHANGE MeasSave", 0, 0, 0);
       ...
       case RRC_MEAS_RESUME:
       {
         ...
         MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"RRC_STATE_CHANGE MeasResume", 0, 0, 0);
...
#error code not present
  ...
  {
    /* shouldn't get here */
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"RRC meas. param. not to be processed: %u", p_rrc_meas_param->params_type, 0, 0);
  }
...
void tm_umts_cp_wcdma_mm_proc( prtl_mm_event_s_type *p_mm_event )
{
  ...
  if ( p_mm_event->mm_event_data_ptr->e_event_type == MM_CGPS_MSG_EVENT_ABORT )
  {
    ...
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"got LCS abort from MM -- no action needed", 0, 0, 0);
  }
  ...
  else if ( p_mm_event->mm_event_data_ptr->e_event_type == MM_CGPS_MSG_EVENT_RESET_UE_POS_INFO )
  {
    ...
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"got reset-UE-pos-info command from MM - already processed", 0, 0, 0);
  }
...
===========================================================================*/
void tm_umts_cp_wcdma_handle_lr_data( sm_FixInfoStructType *lr_data_ptr, boolean u_is_final )
{
  umts_cp_wcdma_session_status_s_type *p_umts_cp_wcdma_session_status = &z_umts_cp_wcdma_session_status;
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"CP-WCDMA: Handle LR data: Final=%u", u_is_final, 0, 0);
      ...
      )
  {
    /* if position has been passed on to pdapi via TmCore already in this session,
       don't repeat that */
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"LR ignored: pdapi report already done", 0, 0, 0);
    return;
  }
      ...
      if ( p_umts_cp_wcdma_session_status->z_session_qos.u_gps_session_timeout_value_secs != 0 )
      {
        if ( p_umts_cp_wcdma_session_status->w_cp_periodic_reports_remaining > 0 )
        {
          ...
          if ( (p_umts_cp_wcdma_session_status->w_lm_report_count >= p_umts_cp_wcdma_session_status->q_cp_periodic_reporting_interval_sec) )
          {
            ...
            /* report to RRC */
            (void)tm_umts_cp_wcdma_encode_rrc_msrReport_pos( &z_ellipse, lr_data_ptr->z_NavPos.q_GpsTimeMs,
          ...
          else
          {
            ...
            MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"skipping subsqnt interm report: total=%u, remain=%u, lm_rpt_cnt=%u",
                    p_umts_cp_wcdma_session_status->w_cp_periodic_reports_total_num,
                    p_umts_cp_wcdma_session_status->w_cp_periodic_reports_remaining,
                    p_umts_cp_wcdma_session_status->w_lm_report_count);
          }
  ...
  if ( p_umts_cp_wcdma_session_status->e_cf_state & C_UMTS_CP_WCDMA_CF_STATE_TIMER_RRC_REPORT )
  {
    ...
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"CP-WCDMA: send final position to RRC", 0, 0, 0);
...
#ifdef FEATURE_GANSS_SUPPORT
    /* Copy the full position information. Used currently for reporting GLO position information.
       So even the memalloc fail its fine to report GPS position info */
    if ( NULL == p_umts_cp_wcdma_session_status->pz_CacheLrNavPos )
    {
      p_umts_cp_wcdma_session_status->pz_CacheLrNavPos = os_MemAlloc(
        sizeof(*p_umts_cp_wcdma_session_status->pz_CacheLrNavPos), OS_MEM_SCOPE_TASK );
    }
    if ( p_umts_cp_wcdma_session_status->pz_CacheLrNavPos != NULL )
    {
      *(p_umts_cp_wcdma_session_status->pz_CacheLrNavPos) = lr_data_ptr->z_NavPos;
    }
#endif
  }
...
void tm_umts_cp_wcdma_handle_prm_data( sm_GpsMeasRptStructType *p_prm_data )
{
  ...
  if (NULL == p_prm_data)
  {
    TM_ERROR_0("tm_umts_cp_wcdma_handle_prm_data() NULL prm data");
    return;
  }
  ...
  if (p_prm_data->z_MeasBlk.z_GnssHeader.e_GnssMeasRptreason != GNSS_MEAS_RPT_MEAS_DONE_SM)
  {
    ...
    if ( q_session_remaining_time_msec > 2500 )
    {
      /* Unless approaching end of session, not interested in anything other
         than DONE, ignore it */
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"disgarding non-DONE MeasRpt", 0, 0, 0);
      return;
    }
    else
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"CP-W: proc non-DONE MeasRpt or send cached meas: %u", z_umts_cp_wcdma_session_status.z_cached_gnss_meas.u_meas_reports_bmap, 0, 0);
      ...
      {
        /* Either send valid Cached measurement or send ERROR to network and close the session*/
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"CP-W: processing DONE MeasRpt", 0, 0, 0);
  }
  ...
  if ( ((tm_umts_common_utils_emergency_effect() == C_EMERGENCY_HANDLING) && (q_session_remaining_time_msec > 2500)) ||
       !(z_umts_cp_wcdma_session_status.e_cf_state & C_UMTS_CP_WCDMA_CF_STATE_TIMER_RRC_REPORT) )
  {
    ...
    if (GNSS_MEAS_BLK_SRC_GPS == p_prm_data->z_MeasBlk.z_GnssHeader.e_MeasBlkSrc)
    {
      ...
      sm_GnssMeas_memscpy(&z_umts_cp_wcdma_session_status.z_cached_gnss_meas.z_meas_reports[SM_GNSS_MEAS_SYSTEM_GPS],
    ...
    else if (GNSS_MEAS_BLK_SRC_GLO == p_prm_data->z_MeasBlk.z_GnssHeader.e_MeasBlkSrc)
    {
      ...
      sm_GnssMeas_memscpy(&z_umts_cp_wcdma_session_status.z_cached_gnss_meas.z_meas_reports[SM_GNSS_MEAS_SYSTEM_GLONASS],
        sizeof(z_umts_cp_wcdma_session_status.z_cached_gnss_meas.z_meas_reports[SM_GNSS_MEAS_SYSTEM_GLONASS]),
        p_prm_data);
    }
    else if (GNSS_MEAS_BLK_SRC_BDS == p_prm_data->z_MeasBlk.z_GnssHeader.e_MeasBlkSrc)
    {
      ...
      */
      ...
      sm_GnssMeas_memscpy(&z_umts_cp_wcdma_session_status.z_cached_gnss_meas.z_meas_reports[SM_GNSS_MEAS_SYSTEM_BDS],
        sizeof(z_umts_cp_wcdma_session_status.z_cached_gnss_meas.z_meas_reports[SM_GNSS_MEAS_SYSTEM_BDS]),
        p_prm_data);
    }
    else if (GNSS_MEAS_BLK_SRC_GAL == p_prm_data->z_MeasBlk.z_GnssHeader.e_MeasBlkSrc)
    {
      /* The max message & sequence field in BDS is required to send meas report correctly. Its also required to count each epoch.
      ...
      */
      ...
      sm_GnssMeas_memscpy(&z_umts_cp_wcdma_session_status.z_cached_gnss_meas.z_meas_reports[SM_GNSS_MEAS_SYSTEM_GAL],
        sizeof(z_umts_cp_wcdma_session_status.z_cached_gnss_meas.z_meas_reports[SM_GNSS_MEAS_SYSTEM_GAL]),
        p_prm_data);
    }
    else if (GNSS_MEAS_BLK_SRC_QZSS_SBAS == p_prm_data->z_MeasBlk.z_GnssHeader.e_MeasBlkSrc)
    {
      /* The max message & sequence field in BDS is required to send meas report correctly. Its also required to count each epoch.
      ...
      */
      ...
      sm_GnssMeas_memscpy(&z_umts_cp_wcdma_session_status.z_cached_gnss_meas.z_meas_reports[SM_GNSS_MEAS_SYSTEM_QZSS],
        sizeof(z_umts_cp_wcdma_session_status.z_cached_gnss_meas.z_meas_reports[SM_GNSS_MEAS_SYSTEM_QZSS]),
        p_prm_data);
    }

    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Cached measurements %3u", z_umts_cp_wcdma_session_status.z_cached_gnss_meas.u_meas_reports_bmap, 0, 0);
    return;
  }


  if ( cp_wcdma_send_rrc_msrReport_meas(p_prm_data, status_check_needed, FALSE) )
  {
    ...
    if ( p_umts_cp_wcdma_session_status->u_callflow_kind == C_UMTS_CP_WCDMA_SESSION_UI )
    {
      ...
      if ( p_umts_cp_wcdma_session_status->z_set_assist_pos_time_stamp.m.refTimeWeekNumValid )
      {
        p_umts_cp_wcdma_session_status->z_set_assist_pos_time_stamp.t_pos_time_stamp_ms =
           ...
           p_prm_data->z_MeasBlk.z_GnssHeader.z_MeasClock.z_Time.z_GpsTime.q_GpsMsec);
...
void tm_umts_cp_wcdma_handle_nofix( void )
{
  ...
  if ( z_umts_cp_wcdma_session_status.z_session_qos.u_gps_session_timeout_value_secs != 0 )
  {
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"skipping subsqnt no-fix report: total=%u, remain=%u, lm_rpt_cnt=%u",
...
void tm_umts_cp_wcdma_notif_verif_resp_proc( pdsm_lcs_notification_resp_s_type *p_user_resp )
{
  ...
  {
    u_accepted = TRUE;
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Notif/Verif ACCEPT", 0, 0, 0);
  }
  ...
  else if ( p_user_resp->resp == PDSM_LCS_NOTIFY_VERIFY_DENY )
  {
        ...
        )
    {
      ...
      u_accepted = TRUE;
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Notif/Verif REJECT overiden as ACCEPT due to priv_type", 0, 0, 0);
    }
    else
    {
      u_accepted = FALSE;
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Notif/Verif REJECT", 0, 0, 0);
    }
  ...
  else /* no application response, or invalid responses */
  {
        ...
        )
    {
      ...
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Notif/Verif NORESP taken as ACCEPT due to priv_type", 0, 0, 0);
      }
      else
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Notif/Verif invalid resp. taken as ACCEPT due to priv_type", 0, 0, 0);
      }
    }
    else if ( p_req_data->notification_priv_type == PDSM_LCS_USER_NOTIFY_VERIFY_NOT_ALLOW_NO_RESP )
    {
      ...
      if ( p_user_resp->resp == PDSM_LCS_NOTIFY_VERIFY_NORESP )
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Notif/Verif NORESP taken as REJECT due to priv_type", 0, 0, 0);
      }
      else
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Notif/Verif invalid resp. taken as REJECT due to priv_type", 0, 0, 0);
      }
    }
    else /* should never get here, only for completeness */
    {
      u_accepted = FALSE;
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Notif/Verif no-resp with unexpected priv_type taken as REJECT", 0, 0, 0);
    }
  ...
  if ( *p_GloWishlist & C_GNSS_ASSIST_NAVMODL )
  {
    ...
    if ( l_long_word > 0 )
    {
      *p_GloWishlist &= ~C_GNSS_ASSIST_NAVMODL;
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "GANSS(GLO) eph holdoff remaining = %ld msec", l_long_word, 0, 0 );
    }
  }
  if ( *p_GloWishlist & C_GNSS_ASSIST_ALM )
  {
    ...
    if ( l_long_word > 0 )
    {
      *p_GloWishlist &= ~C_GNSS_ASSIST_ALM;
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "GANSS(GLO) alm. holdoff remaining = %ld msec", l_long_word, 0, 0 );
    }
  }
  if ( *p_GloWishlist & C_GNSS_ASSIST_AUXI_INFO )
  {
    ...
    if ( l_long_word > 0 )
    {
      *p_GloWishlist &= ~C_GNSS_ASSIST_AUXI_INFO;
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "GANSS(GLO) auxi info holdoff remaining = %ld msec", l_long_word, 0, 0 );
    }
  }
  if ( *p_GloWishlist & C_GNSS_ASSIST_TIME_MODELS )
  {
    ...
    if ( l_long_word > 0 )
    {
      *p_GloWishlist &= ~C_GNSS_ASSIST_TIME_MODELS;
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "GANSS(GLO) time model holdoff remaining = %ld msec", l_long_word, 0, 0 );
    }
  }
#endif
...
(
  ...
  uint32 q_toa_pos_ms,
  sm_NavPositionStructType const * const pz_LrNavPos
 )
{
       ...
       (eWCDMA_POSITION_MODE_A_GPS_GLO_UE_BASED == p_umts_cp_wcdma_session_status->e_gnssPosMode) )
  {
    ...
    if ( pz_LrNavPos->z_PosFlags.b_IsGpsUsed && pz_LrNavPos->z_PosFlags.b_IsGloUsed )
    {
      /* GPS + GLONASS */
    ...
    else if ( pz_LrNavPos->z_PosFlags.b_IsGpsUsed )
    {
      /* GPS only */
      p_measrpt->positioning_estimate.positionData[0] = RRC_GANSS_POSITION_DATA_GPS_BMASK;
      /* GPS Time & GPS Position Estimate is filled above in GPS section */
    }
    else if ( pz_LrNavPos->z_PosFlags.b_IsGloUsed )
    {
      /* GLONASS only */
      ...
      p_measrpt->positioning_estimate.u.ganss_ReferenceTimeOnly.gANSS_tod = tm_umts_common_glonass_tod_mod_hour(pz_LrNavPos->u_GloNumFourYear,
                                                                                                                pz_LrNavPos->w_GloNumDaysInFourYear,
                                                                                                                pz_LrNavPos->q_GloTimeMs);
    ...
    if ( pz_LrNavPos->z_PosFlags.b_IsVelValid )
    {
      ...
      tm_rrc_convert_velocity_estimate(pz_LrNavPos->f_VelEnu, pz_LrNavPos->f_Vunc3dMps,
                                       pz_LrNavPos->f_VuncVertMps,
                                       &p_measrpt->positioning_estimate.velocityEstimate.velocity_type);
    }
  }
  MSG_4(MSG_SSID_GPSSM, MSG_LEGACY_MED, "Glo PE : %d, AGNSS Mode %d, PosData %3u, PE %3u",
        NULL==pz_LrNavPos, p_umts_cp_wcdma_session_status->e_gnssPosMode, p_measrpt->positioning_estimate.positionData[0],
  ...
  if ( p_current_rrc_session_param->z_saved_meas.u_Saved == FALSE )
  {
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"WCDMA-CP: Failure sending MeasReport to RRC", 0, 0, 0);

      if (C_UMTS_CP_WCDMA_SESSION_NI == p_umts_cp_wcdma_session_status->u_callflow_kind)
      { 
        /* Construct and send location notification for FAILURE */
        location_notification.q_prtl_type = TM_PRTL_TYPE_UMTS_CP;
        location_notification.e_pdapi_client_id = p_umts_cp_wcdma_session_status->q_pdapi_client_id;
        location_notification.e_pdapi_client_type = PDSM_CLIENT_TYPE_NI;
        location_notification.e_oper_mode = tm_umts_cp_wcdma_gps_mode_to_tm_oper_mode(p_umts_cp_wcdma_session_status->e_gnssPosMode);
        #ifdef FEATURE_CGPS_DUAL_WCDMA
		  location_notification.e_sub_id = p_umts_cp_wcdma_session_status->z_rrc_as_id_info.e_cached_rrc_as_id;
		#else
		  location_notification.e_sub_id = SYS_MODEM_AS_ID_1;
		#endif
        ...
        p_umts_cp_wcdma_session_status->v_send_no_more_location_notifications = TRUE;
      }											
    }
    else
    {
  ...
  else
  {
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Malloc failed for storing measurement report", 0, 0, 0);
    }
  }
                                                   ...
                                                   sm_InternalAssistDataStatusStructType const *const p_assist_data_status )
{
  ...
  if ( p_measrpt->pos_error.error_cause == RRC_ASSISTANCE_DATA_MISSING )
  {
               ...
               p_assist_data_status->u_NumSvs )
          {
                  ...
                  u_i++ )
            {
              ...
              if (u_i == (p_assist_data_status->u_NumSvs - 1) )
              {
                ...
                MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"UMTS-CP-W: Corrupting Iode for Sv %u", p_assist_data_status->z_SvEphIode[u_i].w_Sv - 1);
              }
#endif
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"AssistDataMissing but no data status given", 0, 0, 0);
    }
...
static boolean cp_wcdma_encode_rrc_msrReport_gps_meas( sm_GpsMeasRptStructType *p_prm_data, boolean status_check_needed )
{
  ...
  float                                   f_Num;
  double                                  d_Num = 0;
  double                                  d_AdjSubMsec = 0;
  boolean                                 v_tunc_to_include = FALSE;

  /* if this number is ever greater than 16, limit to the first 16 */
  if (p_prm_data->z_MeasBlk.z_GnssHeader.u_NumSvs < 16)
  {
    u_num_svs = p_prm_data->z_MeasBlk.z_GnssHeader.u_NumSvs;
  }
  else
  {
  ...
  MSG(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "cp_wcdma_encode_rrc_msrReport_gps_meas: Calling BlacklistFilter");
  for ( u_i = 0; u_i < u_num_svs; u_i++ )
  {
    ...
    /* exclude blacklisted and zero CN0 SVs in the report */
    if ((p_prm_gps_meas_per_sv->w_Cno > 0) &&
        (FALSE == gnss_SvIsBlacklisted(&tm_core_info.config_info.z_GnssPersistSV, p_prm_gps_meas_per_sv->w_Sv)))
    {
      /* If status_check_needed is FALSE, explicitly verify the msec-known-bits. */
      if ((FALSE == v_tunc_to_include) && (FALSE == status_check_needed))
      {
        /* If atleast one SV doesn't have the msec-known bits set, include TUNC. */
        if (FALSE == gps_meas_status_bits_check(p_prm_gps_meas_per_sv->q_MeasStatus, TRUE))
        {
          v_tunc_to_include = TRUE;
        }
      }
      ...
      if (gps_meas_status_bits_check(p_prm_gps_meas_per_sv->q_MeasStatus, status_check_needed))
      {
        ...
        f_Num = (p_prm_gps_meas_per_sv->z_FiltMeas.f_SvSpeed - p_prm_data->z_MeasBlk.z_GnssHeader.z_MeasClock.z_Freq.f_ClkFreqBias)
        ...
        d_AdjSubMsec = (double)p_prm_gps_meas_per_sv->z_FiltMeas.z_SvTime.f_SubMs
           + (double)p_prm_data->z_MeasBlk.z_GnssHeader.z_MeasClock.z_Time.z_GpsTime.f_ClkTimeBias;
        ...
        {
          /* this should not happen */
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"CP-WCDMA: WholeChip out of range", 0, 0, 0);
          return ( FALSE );
        }
  ...
  p_measrpt->gps_measured_results.u.gps_reference_time = p_prm_data->z_MeasBlk.z_GnssHeader.z_MeasClock.z_Time.z_GpsTime.q_GpsMsec;
  ...
  if (v_tunc_to_include)
  {
    ...
    #endif
    {
      p_measrpt->gps_measured_results.gps_reference_time_uncertainity_incl = TRUE;
        p_measrpt->gps_measured_results.gps_reference_time_uncertainity = tm_umts_common_utils_microsec_to_GADK(p_prm_data->z_MeasBlk.z_GnssHeader.z_MeasClock.z_Time.z_GpsTime.f_ClkTimeUncMs * 1000);
    }
    else
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"ERROR: Status check was needed if RRC REL7 is not supported", 0, 0, 0);
    }
  }
#else
...
static boolean cp_wcdma_send_rrc_msrReport_meas( sm_GpsMeasRptStructType *p_prm_data, boolean status_check_needed , boolean is_cached_meas)
{
  ...
  MSG_4(MSG_SSID_GPSSM, MSG_LEGACY_MED, "MeasSrc: %d, Seq#: %3u, MaxMsg#: %3u, CF: %d", p_prm_data->z_MeasBlk.z_GnssHeader.e_MeasBlkSrc,
        p_prm_data->z_MeasBlk.z_GnssHeader.u_SeqNum, p_prm_data->z_MeasBlk.z_GnssHeader.u_MaxMessageNum, p_umts_cp_wcdma_session_status->e_gnssPosMode);

  /* Initialize the LSM measurement report struct & set generic fields _once_ for
  *  each epoch or measurement sets.
  */
  if (1 == p_prm_data->z_MeasBlk.z_GnssHeader.u_SeqNum)
  {
    cp_wcdma_encode_rrc_msrReport_common_meas_params(); 
  }
  ...
  {
    switch (p_prm_data->z_MeasBlk.z_GnssHeader.e_MeasBlkSrc)
    {
       case GNSS_MEAS_BLK_SRC_GPS:
       {
         ...
         /*
          * Propagate each SV's measurement to the common reference point: p_prm_data->z_MeasBlk.z_GnssHeader.z_MeasClock.q_RefFCount.
       ...
       case GNSS_MEAS_BLK_SRC_GLO:
       {
         ...
         if ( p_umts_cp_wcdma_session_status->e_gnssPosMode & WCDMA_POSITION_METHOD_A_GLO )
         {
           ...
           */
            if ((p_prm_data->z_MeasBlk.z_GnssHeader.u_NumSvs > 0) && (p_prm_data->z_MeasBlk.z_GnssHeader.u_NumSvs <= N_ACTIVE_GLO_CHAN))
           {
             /*
              * Propagate each SV's measurement to the common reference point: p_prm_data->z_MeasBlk.z_GnssHeader.z_MeasClock.q_RefFCount.
             */
             ...
             if ( TRUE == cp_wcdma_encode_rrc_msrReport_glo_meas(p_prm_data, status_check_needed, p_measrpt) )
             {
               ...
               /* Copy the GPS meas clock info */
                p_umts_cp_wcdma_session_status->z_GpsTime4GloMeas = p_prm_data->z_MeasBlk.z_GnssHeader.z_MeasClock.z_Time.z_GpsTime;
             }
           }
           else
           {
              MSG_1(MSG_SSID_GPSSM, MSG_LEGACY_MED, "Num of measured Glonass SVs %d", p_prm_data->z_MeasBlk.z_GnssHeader.u_NumSvs);
           }
         }
         break;
       } /* Glonass meas */
       ...
       default:
        MSG_1(MSG_SSID_GPSSM, MSG_LEGACY_MED, "Can't process measurement type %d", p_prm_data->z_MeasBlk.z_GnssHeader.e_MeasBlkSrc);
         break;
    }
  }
  else
  ...
  */
  if(is_cached_meas)
  {
    /*If cached meas are being sent, wait till all the cached meas are processed*/
    if(z_umts_cp_wcdma_session_status.z_cached_gnss_meas.u_meas_reports_bmap != 0) 
    {
      return (FALSE); 
    }
  }
  ...
  {
    /*If DONE/NON-DONE from the current epoch are being sent , check if all meas in this epoch is processed or not*/
  ...
  /* If we reach here , send out valid Enc Meas. All measurement types in cached or this epoch is received */
  ...
  */
  if ( p_umts_cp_wcdma_session_status->u_GnssMeasEncResultBmap )
  {
    ...
    if ( p_current_rrc_session_param->z_saved_meas.u_Saved == FALSE )
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"RRC-msrReport: Num of GPS SVs:%u, EncMeasBMap %d", p_measrpt->gps_measured_results.no_of_satellites,
  ...
  {
    /* Either cached meas or current epoch meas didn't qualified*/
    b_RetVal = FALSE;

    /* If this function is not called for sending cached measurement , rather , live measurements :
         We have just processed maximum sequence number meas block , and found that current epochs measurements were not 
         good enought to qualify for final report, so as We have still time left on CF timer, 
         we should wait for next epoch measurements . Wait for new epoch    
    */

    if(is_cached_meas)
    {
      /* If we were trying to send cached measurements, but no cached meas qualified to be send to server(encoded bitmap is 0),
         then we should send out Error to network and tear down the session.
      */
      if ( eWCDMA_POSITION_MODE_A_GPS_GLO_UE_ASSISTED == z_umts_cp_wcdma_session_status.e_gnssPosMode )
      {
        /* MSA with GPS and GLO*/
        (void)tm_umts_cp_wcdma_encode_rrc_msrReport_err(RRC_NOT_ENOUGH_GANSS_SATELLITES, NULL);
      }
      else
      {
...
static boolean cp_wcdma_rrc_send_measrpt( rrc_meas_rpt_struct_type *p_measrpt )
{
  ...
  if ( rrc_cmd_ptr == NULL )
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"UMTS-CP-W: Failed to get RRC command buffer!", 0, 0, 0);
    return ( FALSE );
  }
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Failed to allocate memory of size 0x%lX", sizeof(rrc_meas_rpt_struct_type), 0, 0);
    return ( FALSE );
  }
  ...
  *meas_rpt_ptr = *p_measrpt;
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"UMTS-CP-W: Sent response to RRC", 0, 0, 0);
                                                                   ...
                                                                   uint8 u_ReportMode )
{
  ...
  if ( p_measrpt == NULL )
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Failed to allocate Meas Rpt Buffer!!", 0, 0, 0);
  }
  else
  {
...
void tm_umts_cp_wcdma_inform_core_stop_session( tm_sess_stop_reason_e_type stop_reason )
{
  ...
  if(p_umts_cp_wcdma_session_status->tm_core_session_started != TRUE)
  {
    MSG(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, 
        "UMTS-CP-W: No ongoing session! Dont call TM_SESS_REQ_STOP");
    return;
  }

  if ((stop_reason != TM_STOP_REASON_COMPLETED) &&
      (FALSE == p_umts_cp_wcdma_session_status->v_send_no_more_location_notifications) &&
      (C_UMTS_CP_WCDMA_SESSION_NI == p_umts_cp_wcdma_session_status->u_callflow_kind))
  { 
    /* Construct and send location notification for FAILURE */
    location_notification.q_prtl_type = TM_PRTL_TYPE_UMTS_CP;
    location_notification.e_pdapi_client_id = p_umts_cp_wcdma_session_status->q_pdapi_client_id;
  ...
  MSG(MSG_SSID_GPSSM, MSG_LEGACY_MED, "UMTS-CP-W: Stop TmCore session");
...
FUNCTION cp_wcdma_convert_and_send_pos_mode_to_pdapi

DESCRIPTION
  This function reports the an event of the requested session type by the network to
  PDAPI.
...
===========================================================================*/
void  cp_wcdma_convert_and_send_pos_mode_to_pdapi( wcdma_position_mode_e_type e_gnssPosMode)
{
  uint32 u_SessionType = 0;
  
  switch ( e_gnssPosMode)
  {
     case eWCDMA_POSITION_MODE_AGPS_UE_BASED :
       u_SessionType |= PDSM_PD_SESS_REQ_TYPE_MSB_GPS;
       break;
     case eWCDMA_POSITION_MODE_AGPS_UE_ASSISTED:
       u_SessionType |= PDSM_PD_SESS_REQ_TYPE_MSA_GPS;
       break;
     case eWCDMA_POSITION_MODE_A_GPS_GLO_UE_BASED:
       u_SessionType |= PDSM_PD_SESS_REQ_TYPE_MSB_GPS;
       u_SessionType |= PDSM_PD_SESS_REQ_TYPE_MSB_GLO;
       break;
     case eWCDMA_POSITION_MODE_A_GPS_GLO_UE_ASSISTED:
       u_SessionType |= PDSM_PD_SESS_REQ_TYPE_MSA_GPS;
       u_SessionType |= PDSM_PD_SESS_REQ_TYPE_MSA_GLO;
       break;
     case eWCDMA_POSITION_MODE_AGPS_CONV:
       u_SessionType |= PDSM_PD_SESS_REQ_TYPE_STANDALONE;
       break;
     default:
       MSG_1(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "Session type not reported to PDAPI %d",e_gnssPosMode);
       break;
  }

  MSG_1(MSG_SSID_GPSSM, MSG_LEGACY_MED, "Requested sess type %d,", u_SessionType);
  
  if ( 0!= u_SessionType )
  {
    tm_send_sess_type_pdapi_event(u_SessionType, PDSM_PD_COMM_PROTOCOL_UMTS_CP_WCDMA);
  }
}
...
static void cp_wcdma_meas_control_proc( rrc_meas_ctrl_struct_type *p_rrc_meas_ctrl )
{
  ...
  if ( p_rrc_meas_ctrl->meas_start_incl != TRUE )
  {
...

    {
      ...
      {
        MSG_3( MSG_SSID_GPSSM, MSG_LEGACY_ERROR, "Meas Rpt pointer is NULL", 0, 0, 0 );
      }
  ...
  {
    if ( (((z_umts_nv_item_ram_copy.q_gnss_lock_ctrl_for_oem)&PDSM_PA_CP_NILR_OEM_CTRL)!= 0) &&
  ...
  if ( p_rrc_params->u_rrc_meas_cmd_type == RRC_MEAS_SETUP )
  {
    ...
    {
      MSG_3( MSG_SSID_GPSSM, MSG_LEGACY_MED, "Stopping MTLR guard timer", 0, 0, 0 );
      tm_umts_cp_wcdma_terminate_timer_cancel_cf_state( C_UMTS_CP_WCDMA_CF_STATE_MT_GUARD_TIME );
    }
  }
  ...
  */
  ...
  if ( p_umts_cp_wcdma_session_status->u_callflow_kind == C_UMTS_CP_WCDMA_SESSION_NONE )
  {
    ...
    MSG_3( MSG_SSID_GPSSM, MSG_LEGACY_MED, "UMTS-CP-W: first NILR/MTLR RRC meas.Control arrived", 0, 0, 0 );
  ...
  */
  ...
  /* For NI, convert the network provided accuracy from GAD to meter. If network didn't provide
     the accuracy use the default. */
  ...
  {
        p_umts_cp_wcdma_session_status->z_session_qos.q_hor_acc_meters =
        (uint32)tm_umts_common_utils_GADK_to_meter( TRUE, p_rrc_meas_ctrl->rpt_quan.horizontal_accuracy );
  }
    else
  {
      ...
      p_umts_cp_wcdma_session_status->z_session_qos.q_hor_acc_meters = z_umts_nv_item_ram_copy.aagps_default_qos_unc;
    }
  }
  /* In E911 by default the session should run to timeout for better accuracy. When early exit is
  *  enabled the horizontal accuracy should be set based on the threshold.
    */
  if ( e_e911_effect != C_EMERGENCY_HANDLING_NULL )
    {
    if ( ( p_umts_cp_wcdma_session_status->u_callflow_kind == C_UMTS_CP_WCDMA_SESSION_NI ) &&
         ((p_umts_cp_wcdma_session_status->e_gnssPosMode & WCDMA_POSITION_METHOD_TYPE_UE_BASED ) != 0) &&
         ((TM_E911_EARLY_EXIT_ENABLE & tm_core_get_emergency_support_config()) != 0) )
       {
       p_umts_cp_wcdma_session_status->z_session_qos.q_hor_acc_meters = 
        tm_util_get_e911_early_exit_qos_config( p_umts_cp_wcdma_session_status->z_session_qos.q_hor_acc_meters );
    }
    else
    {
  ...
  MSG_1( MSG_SSID_GPSSM, MSG_LEGACY_MED, "QoS accuracy %u settled",
         p_umts_cp_wcdma_session_status->z_session_qos.q_hor_acc_meters );
  ...
  else if ( p_rrc_params->u_report_mode == RRC_PERIODIC_RPT )
  {
        ...
        )
    {
           ...
           (p_umts_cp_wcdma_session_status->e_gnssPosMode == eWCDMA_POSITION_MODE_A_GPS_GLO_UE_ASSISTED) )
      {
        ...
        {
          /* Send ERROR, if no cache present. */
            if (eWCDMA_POSITION_MODE_A_GPS_GLO_UE_ASSISTED == z_umts_cp_wcdma_session_status.e_gnssPosMode)
            {
              (void)tm_umts_cp_wcdma_encode_rrc_msrReport_err(RRC_NOT_ENOUGH_GANSS_SATELLITES, NULL);
            }
    ...
    /* send an event to PDAPI on the session mode requested by the network */
    cp_wcdma_convert_and_send_pos_mode_to_pdapi(p_umts_cp_wcdma_session_status->e_gnssPosMode );
    ...
    */
    ...
    else  /* else: if multiple periodic */
    {
      ...
      MSG_3( MSG_SSID_GPSSM, MSG_LEGACY_MED, "Proc. multiple rpt req. Int. long %u, cp %lu",
             p_umts_cp_wcdma_session_status->w_rrc_reporting_interval_long,
         ...
         (eWCDMA_POSITION_MODE_A_GPS_GLO_UE_BASED == p_umts_cp_wcdma_session_status->e_gnssPosMode) )
    {
          ...
          )
      {
        ...
        /* For periodic keep the LM running */
        if ( p_rrc_params->rrc_periodic_multi_report )
        {
        p_req_param->continue_param.accuracy_threshold = 0;
        }
        else
        ...
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"UMTS-CP-W: req. TmCore for wishlist", 0, 0, 0);
    ...
    else
    {
      (void)tm_umts_cp_wcdma_encode_rrc_msrReport_pos( &p_umts_cp_wcdma_session_status->z_pending_pos_to_network,
                                                       ...
                                                       p_umts_cp_wcdma_session_status->pz_CacheLrNavPos );
                                                 ...
                                                 rrc_gps_reference_loc_struct_type const *const p_GpsRefLoc )
{
  ...
  if ( p_GpsRefLoc != NULL )
  {
                                                   ...
                                                   w_GpsRefTimeWeekNum, q_GpsRefTimeTowMs) )
    {
      ...
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"CP-WCDMA: refLoc to Tmcore data post failed", 0, 0, 0);
      }
      else
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"CP-WCDMA: refLoc posted to TmCore", 0, 0, 0);
      }
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"CP-WCDMA:  success in posting refL from MC as extended status position event to OEM", 0, 0, 0);
    }
    else
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"CP-WCDMA: did not post refL from MC as extended status position event to OEM", 0, 0, 0);
    }
...
static boolean cp_wcdma_rrc_gps_assist_proc( rrc_assistance_data_struct_type *p_assist )
{
  ...
  */
  ...
  if ( p_assist->gps_ref_time_incl )
  {
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"CP-WCDMA: GPS refTime to Tmcore data post failed", 0, 0, 0);
    }
    else
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"CP-WCDMA: GPS refTime posted to TmCore", 0, 0, 0);
    }
  ...
  if ( p_assist->gps_navigation_model_incl )
  {
    ...
    while ( u_i < p_assist->gps_navigation_model.no_of_satellites )
    {
      ...
      if ( p_satellite_info->ephermeral_clock_params_incl )
      {
        ...
        {
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"GPS SV %u eph. post to TmCore failed", z_tm_post_data_payload.eph_data.u_Sv, 0, 0);
        }
        else
        {
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"GPS SV %u eph. posted to TmCore", z_tm_post_data_payload.eph_data.u_Sv, 0, 0);
        }
  ...
  if ( p_assist->gps_iono_incl )
  {
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"CP-WCDMA GPS IONO to Tmcore data post failed", 0, 0, 0);
    }
    else
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"CP-WCDMA: GPS IONO posted to TmCore", 0, 0, 0);
    }
  ...
  if ( p_assist->gps_utc_model_incl )
  {
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"CP-WCDMA: GPS UTC to Tmcore data post failed", 0, 0, 0);
    }
    else
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"CP-WCDMA: GPS UTC posted to TmCore", 0, 0, 0);
    }
  ...
  if ( p_assist->gps_almanac_incl )
  {
    ...
    while ( u_i < p_assist->gps_almanc.no_of_almanac_sat )
    {
      ...
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"CP-WCDMA: GPS SV %u alm post to TmCore failed", z_tm_post_data_payload.alm_data.u_Sv, 0, 0);
      }
      else
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"CP-WCDMA: GPS SV %u alm. posted to TmCore", z_tm_post_data_payload.alm_data.u_Sv, 0, 0);
      }
  ...
  if ( p_assist->gps_acq_assistance_incl )
  {
         ...
         (p_umts_cp_wcdma_session_status->e_gnssPosMode == eWCDMA_POSITION_MODE_A_GPS_GLO_UE_ASSISTED) )
    {
      ...
      if ( z_tm_post_data_payload.gnss_sv_dir.u_NumSvs ) /* Optional AdditionalAngle */
      {
        ...
        {
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"CP-WCDMA: GPS SvDir to TmCore data post failed", 0, 0, 0);
        }
        else
        {
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"CP-WCDMA: GPS SvDir posted to TmCore", 0, 0, 0);
        }
      }
      ...
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"CP-WCDMA: GPS Steering to Tmcore data post failed", 0, 0, 0);
      }
      else
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"CP-WCDMA: GPS Steering posted to TmCore", 0, 0, 0);
      }
...
static void cp_wcdma_rrc_ganss_assist_proc( rrc_meas_ctrl_struct_type const *const p_RrcMeasCtrl )
{
  ...
  */
  ...
  if ( TRUE == p_assist->ganss_ref_time_incl )
  {
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"CP-WCDMA: GloRefTime failed post to Tmcore", 0, 0, 0);
    }
    else
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"CP-WCDMA: GloRefTime posted to TmCore", 0, 0, 0);
    }
  ...
  */
  ...
  if ( TRUE == p_assist->ganss_addlono_model_incl )
  {
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"CP-WCDMA: GLO IONO to Tmcore data post failed", 0, 0, 0);
    }
    else
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"CP-WCDMA: GLO IONO posted to TmCore", 0, 0, 0);
    }
        ...
        if ( (TRUE == p_GanssGenericAD->time_mode_list_incl) && (p_GanssGenericAD->list_cnt > 0) )
        {
          ...
          for ( u_i = 0; u_i < p_GanssGenericAD->list_cnt; ++u_i )
          {
            ...
            if ( GANSS_ID_GPS == p_GanssGenericAD->time_mode_list[u_i].ganss_to_id )
            {
              ...
              {
                MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"RRC GLO TimeModel failed posting to TmCore", 0, 0, 0);
              }
              else
              {
                MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"RRC GLO TimeModel posted to TmCore", 0, 0, 0);
              }
            }
          }
        }
...
static void cp_wcdma_action_upon_ni_notif_verif_accepted( void )
{
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"CP-WCDMA: MT-LR sess started", 0, 0, 0);
#endif
...
void tm_umts_cp_wcdma_start_max_mgp_on_timer( uint32 q_time_ms )
{
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"CP-WCDMA: Start max. MGP_ON timer; t=%d", q_time_ms, 0, 0);
}
...
void tm_umts_cp_wcdma_cancel_max_mgp_on_timer( void )
{
  if ( os_TimerCheck(umts_cp_wcdma_max_mgp_on_timer) )
  {
    ...
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"CP-WCDMA: Cancel max. MGP_ON timer", 0, 0, 0);
  }
}
         ...
         (p_umts_cp_wcdma_session_status->w_cp_periodic_reports_remaining <= p_umts_cp_wcdma_session_status->w_cp_periodic_reports_total_num) )
    {
      ...
      */
           ...
           (TRUE == b_first_done_meas) )
      {
        ...
        if ( cp_wcdma_send_rrc_msrReport_meas(p_prm_data, status_check_needed, FALSE) ) //Meas from current Epoch
        {
          /* clear timer and call flow state. Note if the above function call returns FALSE,
             don't stop this state, just keep processing incoming measurements until timeout
             at which time UEP-Error will be sent */
...
*
* Function description:
*   Send RRC measurementReport with cached GPS and/or GLONASS measurement if available and qualified.
*   Otherwise send RRC ERROR message . Anycase, end the session afterwards.
*
* Parameters:
*  None
*
...
boolean tm_umts_cp_wcdma_rrc_send_cached_gnss_meas_and_end_session( void )
{
  ...
  if ( z_umts_cp_wcdma_session_status.z_cached_gnss_meas.u_meas_reports_bmap & SM_GNSS_MEAS_REPORTS_BMAP_GPS )
  {
    ...
    * not sent due to impending GLO/BDS measurement (or) failure in processing/sending GPS meas. */
    b_GpsMeasStatus = cp_wcdma_send_rrc_msrReport_meas( p_prm_data, TRUE, TRUE );
  }
  ...
  if ( z_umts_cp_wcdma_session_status.z_cached_gnss_meas.u_meas_reports_bmap & SM_GNSS_MEAS_REPORTS_BMAP_GLONASS )
  {
    ...
    * not sent due to impending GPS/BDS measurement (or) failure in processing/sending GLO meas. */
    b_GloMeasStatus = cp_wcdma_send_rrc_msrReport_meas( p_prm_data, TRUE, TRUE );
  }
  ...
  if ( z_umts_cp_wcdma_session_status.z_cached_gnss_meas.u_meas_reports_bmap & SM_GNSS_MEAS_REPORTS_BMAP_BDS )
  {
     ...
     *  Note the BDS measurement itself isn't processed & supported by this module
     */
    b_BdsMeasStatus = cp_wcdma_send_rrc_msrReport_meas( p_prm_data, TRUE, TRUE );
  }
  ...
  if ( z_umts_cp_wcdma_session_status.z_cached_gnss_meas.u_meas_reports_bmap & SM_GNSS_MEAS_REPORTS_BMAP_GAL )
  {
     ...
     *  Note the GAL measurement itself isn't processed & supported by this module
     */
    b_GalMeasStatus = cp_wcdma_send_rrc_msrReport_meas( p_prm_data, TRUE, TRUE );
  }
  ...
  if ( z_umts_cp_wcdma_session_status.z_cached_gnss_meas.u_meas_reports_bmap & SM_GNSS_MEAS_REPORTS_BMAP_QZSS )
  {
     ...
     *  Note the QZSS/SBAS measurement itself isn't processed & supported by this module
     */
    b_QzssMeasStatus = cp_wcdma_send_rrc_msrReport_meas( p_prm_data, TRUE, TRUE );
  }
...
/*===========================================================================
FUNCTION tm_umts_cp_wcdma_gps_mode_to_tm_oper_mode

DESCRIPTION
  This function maps from SUPL agps mode type to TM operation mode

PARAMETERS: wcdma_position_mode_e_type e_agps_mode
...
tm_sess_operation_mode_e_type tm_umts_cp_wcdma_gps_mode_to_tm_oper_mode(wcdma_position_mode_e_type e_agps_mode)
{
