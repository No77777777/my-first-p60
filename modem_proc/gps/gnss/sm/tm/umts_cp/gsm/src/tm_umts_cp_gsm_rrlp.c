...
/*===========================================================================
  ...
  $Header: //components/rel/gnss8.mpss/11.1/gnss/sm/tm/umts_cp/gsm/src/tm_umts_cp_gsm_rrlp.c#1 $ 
  ...
  02/17/16   mj      Add NULL check to function tm_umts_cp_gsm_proc_msr_pos_req_qos_mtmb
...
(
   const rrlp_dec_MsrPosition_Req  *p_mpr
);
  ...
  const tm_post_data_payload_type   *p_payload )
{
  ...
  if( post_ok )
  {
    ...
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"CP-GSM: %d posted to TmCore; rcvd=0x%08X, ~rcvd=0x%08X", 
            (int) data_type,
            z_umts_cp_gsm_session_status.q_agps_data_rcvd, 
            ~z_umts_cp_gsm_session_status.q_agps_data_rcvd );
  }
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"CP-GSM: post of %d to Tmcore failed", 
              (int)data_type, 0, 0 );
  }
}
...
)
{
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"CP-GSM: process_ref_time; e_agps_mode=%d", e_agps_mode, 0, 0);
...
)
{
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"CP-GSM: process_ref_pos", 0, 0, 0);
...
(
  const rrlp_dec_ControlHeader  *p_assist,
  uint16                         w_gps_week 
)
{
  tm_post_data_payload_type           z_tm_post_data_payload;
  const rrlp_dec_SeqOfNavModelElement *navModelList;
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"CP-GSM: process_nav_model", 0, 0, 0);
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"GSM-CP: EphSvList=0x%lx", 
...
)
{
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"CP-GSM: process iono model", 0, 0, 0);
...
)
{
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"CP-GSM: process_utc", 0, 0, 0);
...
{
  tm_post_data_payload_type           z_tm_post_data_payload;
  const rrlp_dec_SeqOfAlmanacElement  *almanacList;
  rrlp_dec_AlmanacElement             *p_Almanac;
  uint16                              w_AlmanacWeek;
  uint8                               u_i;
  
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"CP-GSM: process_almanac", 0, 0, 0);
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_LOW,"CP-GSM: can't get 10-bit alm. week since there is no ref time", 0, 0, 0);
  }
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"GSM-CP: AlmSvList=0x%lx", 
...
)
{
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"CP-GSM: process_acq_assist", 0, 0, 0);
  ...
  u_num_existing_svs = p_global_cached->gnss_aa_data.u_NumSvs;
    ...
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Caching ACQ: Sv_dir %u  Steer %u", 
...
static void tm_umts_cp_gsm_process_rti_assist( rrlp_dec_ControlHeader *p_assist)
{
  rrlp_dec_SeqOf_BadSatelliteSet const *const p_RTI = &p_assist->realTimeIntegrity; 
  ...
  rrlp_dec_ControlHeader  *p_assist )
{
  ...
  */
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"CP-GSM: Ignoring DGPS data", 0, 0, 0);
  ...
  if ( p_assist->m.navigationModelPresent )
  {
    if ( !p_assist->m.referenceTimePresent )
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_LOW,"CP-GSM: can't get eph. week since there is no ref time", 0, 0, 0);
    }
...
===========================================================================*/
void  cp_gsm_and_send_pos_mode_to_pdapi( )
{
  uint32 u_SessionType = 0;
  
  switch ( z_umts_cp_gsm_session_status.e_agps_mode)
  {
     ...
     case C_UMTS_CP_GSM_AGPS_MODE_CONV:
       ...
       break;
     default:
       MSG_1(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "Session type not reported to PDAPI %d",z_umts_cp_gsm_session_status.e_agps_mode);
       break;
  }
  ...
  if ( 0!= u_SessionType )
  {
    tm_send_sess_type_pdapi_event(u_SessionType, PDSM_PD_COMM_PROTOCOL_UMTS_CP_GSM);
  }
}
...
)
{
  ...
  rrlp_dec_PositionInstruct             *p_rrlp_pos_instruct;
  ...
  if (p_mpr == NULL)
  {
    MSG(MSG_SSID_GPSSM, MSG_LEGACY_MED, "CP-GSM: Invalid MPR from Network");
    return FALSE;
  }
  ...
  if ( (z_umts_cp_gsm_session_status.u_mpr_rcvd == FALSE) && 
        (tm_umts_common_utils_emergency_effect() == C_EMERGENCY_HANDLING)
     ) 
  {
   ...
   */
  if( p_rrlp_pos_instruct->positionMethod == rrlp_dec_PositionMethod_eotd )
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"CP-GSM: rrlp_eotd not supported", 0, 0, 0);
  ...
  if( e_callflow_kind == C_UMTS_CP_GSM_SESSION_UI)
  {
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"CP-GSM: Invalid switch to MS-Assisted", 0, 0, 0);
      u_invalid_switch = TRUE;
    }
    else if( (p_rrlp_pos_instruct->methodType.t == rrlp_msBased_chosen) &&
             (e_agps_mode != C_UMTS_CP_GSM_AGPS_MODE_UE_BASED) )
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"CP-GSM: Invalid switch to MS-Based", 0, 0, 0);
      u_invalid_switch = TRUE;
    }
  ...
  cp_gsm_and_send_pos_mode_to_pdapi ();
  /*
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"CP-GSM: Sending Cached Position to Network", 0, 0, 0);
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_LOW,"CP-GSM: Setting u_final_mpr_rcvd", 0, 0, 0);
  ...
  */
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Start max_mgp_on timer", 0, 0, 0);
          ...
          ) /* max_mgp_on timer is currently running */ 
  {
    ...
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"mprRespTime > remaining max_revr_on: %u sec, %u msec", 
...
(
  const rrlp_dec_PositionInstruct     *p_rrlp_pos_instruct,
  rrlp_params_s_type                  *p_rrlp_params,
  uint32                              *p_hor_acc_meters,
  uint8                               u_meas_resp_time
)
{
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_LOW,"CP-GSM: proc_msr_pos_req_qos_msa", 0, 0, 0);
...
)
{
  ...
  if (p_rrlp_params == NULL || p_rrlp_pos_instruct == NULL ||  p_hor_acc_meters == NULL)
  {
     MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"CP-GSM: NULL RRLP Params/POS Instruct/Horizontal accuracy pointer", 0, 0, 0);
     /*Return the Response time unchanged 
       Horizontal Accuracy: In this case will be the Default NV set QOS*/
     return u_meas_resp_time;
  }

  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_LOW,"CP-GSM: proc_msr_pos_req_qos_mtmb", 0, 0, 0);
                                                ...
                                                C_UMTS_CP_GSM_SESSION_NI )
  {
         ...
         !z_umts_cp_gsm_data.u_e911_subsequent_rrlp_sess )
    {
      if ( !z_umts_cp_gsm_session_status.u_preemption_flag ) 
      {
        ...
        /* Set this flag so that Accuracy/Qos during 911 can be provided*/
    ...
    if ( u_use_rrlp_accuracy )
    {
      ...
      */
      if (z_umts_cp_gsm_data.u_e911_in_progress) 
      {
        *p_hor_acc_meters = tm_util_get_e911_early_exit_qos_config(*p_hor_acc_meters);
      }                      
...
)
{
  ...
  rrlp_params_s_type                *p_rrlp_params;
  uint8                             u_timeout_sec;
  const rrlp_dec_PositionInstruct   *p_rrlp_pos_instruct = &p_mpr->positionInstruct;

  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_LOW,"CP-GSM: proc_msr_pos_req_qos", 0, 0, 0);
...
)
{
  ...
  *p_ack_required = TRUE;
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"CP-GSM: RRLP Decode Failure: %d",asn1_decode_status,0,0);
  ...
  *p_asn1_decode_status = RRLP_ASN1_DECODE_ERR_PDU_DECODED;
  ...
  {      
    ...
    if(e_e911_effect == C_EMERGENCY_HANDLING_NULL)
    {
      ...
      if ((((z_umts_nv_item_ram_copy.q_gnss_lock_ctrl_for_oem)& PDSM_PA_CP_NILR_OEM_CTRL)!= 0) &&
	  	  (( z_umts_nv_item_ram_copy.gps_lock == PDSM_GPS_LOCK_ALL ) || ( z_umts_nv_item_ram_copy.gps_lock == PDSM_GPS_LOCK_MT ))
         )
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"MT GPS Locked",0,0,0 );

         
        /* Construct and send location notification for REJECT */
        location_notification.q_prtl_type = TM_PRTL_TYPE_UMTS_CP;
        location_notification.e_pdapi_client_id = z_umts_cp_gsm_session_status.q_pdapi_client_id;
        location_notification.e_pdapi_client_type = PDSM_CLIENT_TYPE_NI;
        location_notification.e_oper_mode = tm_umts_cp_gsm_gps_mode_to_tm_oper_mode(z_umts_cp_gsm_session_status.e_agps_mode);
        location_notification.e_sub_id = z_umts_cp_gsm_session_status.z_rr_as_id_info.e_cached_rr_as_id;
   ...
   */
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"CP-GSM: RefNums 1=%d, 2=%d",
  ...
  if( !z_umts_cp_gsm_session_status.u_tm_core_session_started )
  {
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Unable to Start %d session, service interaction failed", e_agps_mode, 0, 0);
      tm_osys_asn1_free_pdu (p_rrlp , PROTO_RRLP);
      return TRUE; 
    }
    if( !tm_umts_cp_gsm_start_sess_req( e_agps_mode ) )
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Unable to Start %d session", e_agps_mode, 0, 0);
      tm_osys_asn1_free_pdu (p_rrlp , PROTO_RRLP);
      return TRUE; 
    }
  }
  ...
  switch ( p_rrlp->component.t )
  {
    case rrlp_msrPositionReq_chosen:
      ...
      *p_ack_required = FALSE;
      ...
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"CP-GSM: err RRLP-msrPosReq", 0, 0, 0);
        success = FALSE;
      }
      break;

    case rrlp_msrPositionRsp_chosen:
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"CP-GSM: Unexpected RRLP-msrPosRsp received",0,0,0);
      *p_ack_required = FALSE;
      success = FALSE;
      break;
    ...
    case rrlp_assistanceData_chosen:
    {
      ...
      if( p_Assist->m.gps_AssistDataPresent )
      {
        ...
        if( !parse_ok )
        {
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"CP-GSM: Unable to parse AssistanceData",0,0,0);
          success = FALSE;
        }
      }
    ...
    {
      /* Unknown type */
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"CP-GSM: Unknown RRLP-choice received",0,0,0);
      *p_ack_required = FALSE;
      success = FALSE;
      break;
    }
...
)
{
  ...
  if( fix_type != rrlp_threeDFix )
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"CP-GSM: Invalid Fix Type in send_pos_rrlp", 0, 0, 0 );
   ...
   */
  if (TRUE == tm_umts_cp_gsm_log_and_tx_rrlp(  pdu_buff.value, 
  ...
  else
  {
    ...
    { 
      ...
      z_umts_cp_gsm_session_status.v_send_no_more_location_notifications = TRUE;                                       
    }    
  }
...
)
{
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_LOW,"CP-GSM: tx_rrlp_mpr_err called", 0, 0, 0);
  
 
  memset(&z_rrlp, 0, sizeof(rrlp_enc_PDU) );
  memset(&z_rrlp_MsrPosition_Rsp, 0, sizeof(z_rrlp_MsrPosition_Rsp));
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_LOW,"CP-GSM: requesting assistance data", 0, 0, 0);
  ...
  if( p_rrlp_params->z_rrlp_rel5_ext.rel5_ext_present )
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_LOW,"CP-GSM: adding REL-5 Extension", 0, 0, 0);
...
)
{
  ...
  if ( p_prm_data->z_MeasBlk.z_GnssHeader.e_GnssMeasRptreason != GNSS_MEAS_RPT_MEAS_DONE_SM )
  {
    if ( q_session_remaining_time_msec > 2500 )
    {
      ...
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"CP-GSM: discarding non-DONE MeasRpt", 0, 0, 0);
      return;
    }
    else
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"CP-GSM: proc non-DONE MeasRpt or send cached: %u", 
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"CP-GSM: processing DONE MeasRpt", 0, 0, 0);
  }
     ...
     )
  {
    if ( !z_umts_cp_gsm_session_status.u_preemption_flag ) 
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"In E911, not rept. meas, wait to TO", 0, 0, 0);
      
      /* cache it for later (at responseTime ) report used by E911 */
      z_umts_cp_gsm_session_status.z_cached_gps_meas.u_valid = TRUE;
      sm_GnssMeas_memscpy((void*)&z_umts_cp_gsm_session_status.z_cached_gps_meas.z_cached_gps_meas_rpt,
        sizeof(z_umts_cp_gsm_session_status.z_cached_gps_meas.z_cached_gps_meas_rpt), p_prm_data);
      return;
    }
  }
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"CP-GSM: MeasRpt from TM contains 0 SV", 0, 0, 0);
    return;
  }
   ...
   * status check.
   */
  u_list_len = p_prm_data->z_MeasBlk.z_GnssHeader.u_NumSvs;
   ...
   */
  MSG(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "tm_umts_cp_gsm_handle_prm_data: Calling BlacklistFilter");
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"CP-GSM: MeasRpt from TM contains 0 qualified SV", 0, 0, 0);
    return;
  }
  ...
  if ( z_umts_cp_gsm_session_status.e_cf_state !=
                  C_UMTS_CP_GSM_CF_STATE_TIMER_RRLP_MEAS_RESP_TIME )
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"CP-GSM: GPS meas. ignored - not expecting", 0, 0, 0);
    return;
  }
      ...
      if( p_msa_pos_time_stamp->u_ref_time_week_num_valid )
      {
        p_msa_pos_time_stamp->q_pos_time_stamp = (uint32)
                ( tm_umts_common_utils_full_gps_ms(
                        p_msa_pos_time_stamp->w_ref_time_week_num,
                        p_prm_data->z_MeasBlk.z_GnssHeader.z_MeasClock.z_Time.z_GpsTime.q_GpsMsec ) / SEC_MSECS );
    ...
    else
    {
      ...
      if( p_msa_pos_time_stamp->u_ref_time_week_num_valid )
      {
        p_msa_pos_time_stamp->q_pos_time_stamp = (uint32)
         ( tm_umts_common_utils_full_gps_ms(
           p_msa_pos_time_stamp->w_ref_time_week_num,
           cached_gps_meas_prt->z_cached_gps_meas_rpt.z_MeasBlk.z_GnssHeader.z_MeasClock.z_Time.z_GpsTime.q_GpsMsec ) /
           SEC_MSECS );
        p_msa_pos_time_stamp->u_pos_time_stamp_valid = TRUE;
      }
...
)
{
  ...
  uint8                         u_i = 0;
  uint8                         u_list_len = 0;
  uint8                         u_num_non_zero_cno_sv = 0;
  const gnss_MeasStructType     *p_gps_meas_per_sv;

  rrlp_enc_PDU                  z_rrlp;
  rrlp_enc_MsrPosition_Rsp      z_rrlp_MsrPosition_Rsp;
  rrlp_enc_GPS_MsrSetElement    z_rrlp_GPS_MsrSetElement;
  rrlp_enc_GPS_MsrElement       z_MsrElement[16]; /* max. RRLP gps-measurement can have 16 SVs */
   ...
   *  Only SVs that are not blacklisted are considered
   */
  MSG(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "tm_umts_cp_gsm_encode_msrReport_meas: Calling BlacklistFilter");
  p_gps_meas_per_sv = &p_prm_data->z_MeasBlk.z_Gnss[0];
  u_num_non_zero_cno_sv = 0;
  for (u_i=0; u_i < u_list_len; u_i++, p_gps_meas_per_sv++)
  {
        ...
        (FALSE == gnss_SvIsBlacklisted(&tm_core_info.config_info.z_GnssPersistSV, p_gps_meas_per_sv->w_Sv)))
    {
      ...
      {
        if (FALSE == gps_meas_status_bits_check(p_gps_meas_per_sv->q_MeasStatus, TRUE))
        {
  ...
  if(v_tunc_to_include)
  {
    ...
    p_mpr->rel_98_MsrPosition_Rsp_Extension.timeAssistanceMeasurements.gpsReferenceTimeUncertainty = 
      tm_umts_common_utils_microsec_to_GADK(p_prm_data->z_MeasBlk.z_GnssHeader.z_MeasClock.z_Time.z_GpsTime.f_ClkTimeUncMs * 1000);
  }
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"CP-GSM: transmitting PRM data to SMLC", 0, 0, 0);
  
  /*
   *  Transmit the data to the SMLC via RR
   */
  v_ret_val = tm_umts_cp_gsm_log_and_tx_rrlp(  pdu_buff.value, 
                                        (uint16) pdu_buff.length );
  if(FALSE == v_ret_val)
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"CP-GSM: Unable to transmit PRM data to SMLC", 0, 0, 0);
...
)
{
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"CP-GSM: Unable to write data to RR", 0, 0, 0);
    return FALSE;
  }
...
)
{
  ...
  rrlp_enc_ErrorCodes                       rrlp_protocol_reason;
  tm_network_assist_s_type                  *p_net_assist;

  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"CP-GSM: Parsing RRLP", 0, 0, 0);
  ...
  if( w_length == 0 )
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"CP-GSM: Zero Length RRLP message rcvd", 0, 0, 0);
  ...
  if( decode_ok )
  {
    if( u_ack_required )
    {
      ...
      if( p_net_assist->u_MoreToCome )
      {
        ...
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Expect more AD from SMLC", p_net_assist->u_MoreToCome, 0, 0); 
  ...
  else if( asn1_decode_status == RRLP_ASN1_DECODE_ERR_PDU_DECODED )
  {
     ...
     */
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"CP-GSM: protocol error or ignored message. Reset GSM cached as_id", 0, 0, 0);
  ...
  else
  {
    ...
    {
      rrlp_protocol_reason = rrlp_enc_ErrorCodes_messageTooShort;
    }
    else if ( asn1_decode_status == RRLP_ASN1_DECODE_ERR_PDU_INCORRECT_DATA )
    {
      rrlp_protocol_reason = rrlp_enc_ErrorCodes_incorrectData;
    }
    else 
    {
      rrlp_protocol_reason = rrlp_enc_ErrorCodes_unDefined; 
    }

    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Sending RRLP-protocolError: %d", rrlp_protocol_reason, 0, 0);
...
)
{
   ...
   */
  memset( &z_rrlp, 0, sizeof( z_rrlp ));
  memset( &z_protocol_error, 0, sizeof(rrlp_enc_ProtocolError));
...
static void tm_umts_cp_gsm_tx_rrlp_ack( void )
{
  ...
  {	
    tm_umts_cp_gsm_inform_core_stop_session( TM_STOP_REASON_GENERAL_ERROR);
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"CP-GSM: TX_RRLP failed", 0, 0, 0);
  }
...
void tm_umts_cp_gsm_post_cached_acq_assist( )
{
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Posting cached SvDir %u Steer %u", 
             z_umts_cp_gsm_session_status.z_cached_acq_assist.z_cached_svdir.gnss_sv_dir.u_NumSvs,
             z_umts_cp_gsm_session_status.z_cached_acq_assist.z_cached_steer.gnss_aa_data.u_NumSvs,
             0 );
}
