...
/*===========================================================================
  ...
  $Header: //components/rel/gnss8.mpss/11.1/gnss/sm/tm/umts_cp/ss/src/tm_umts_cp_ss.c#1 $
  ...
  03/01/19   gk      Add changes for new privacy support
...
void tm_umts_cp_ss_init (void)
{
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"tm_cp_ss init", 0, 0, 0);
  tm_umts_cp_ss_session_init();
...
)
{
  ...
  else
  {
    ...
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"UMTS-CP-SS: invalid mo_lr_type: %u", mo_lr_type, 0, 0 );
    return FALSE;
  }
  ...
  if ( ret_val )
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "Sending MOLR-Inovke w/ ref_num = %u", 
...
boolean tm_umts_ss_send_lcs_loc_notif_return_result(const pdsm_lcs_notification_resp_s_type *p_user_resp )
{
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"TM-CM iface fails to send LN-notif-verif response", 0, 0, 0);
  }
...
void tm_umts_cp_ss_lcs_molr_proc(void)
{
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"UMTS-CP-SS: CM confirms molr-invoke queued to NAS", 0, 0, 0);
...
void tm_umts_cp_ss_lcs_loc_notif_ind_proc (tm_cm_sups_data_s_type* p_sups_data)
{
  ...
  if ( p_sups_data->lcs_mess.location_notification_ind.notification_type == NOTIFY_LOCATION_ABORTED )
  {
    ...
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Confirmation of releaseComplete to NotifInd received", 0, 0, 0); 
  }
  else
  {
    if   ( e_e911_effect == C_EMERGENCY_HANDLING_NULL) 
    {
      /* If not in emergency call and if NI is locked send a deny to the 
          network, otherwise pass the notification to AP */
            ...
            
      {
        ...
        else if (phone_state_info.srv_system == CGPS_SRV_SYS_WCDMA)
        {
          /* Send Reject notification */
          location_notification.q_prtl_type = TM_PRTL_TYPE_UMTS_CP;
          location_notification.e_pdapi_client_id = z_umts_cp_wcdma_session_status.q_pdapi_client_id;
          location_notification.e_pdapi_client_type = PDSM_CLIENT_TYPE_NI;
          location_notification.e_oper_mode = tm_umts_cp_wcdma_gps_mode_to_tm_oper_mode(z_umts_cp_wcdma_session_status.e_gnssPosMode);
          #ifdef FEATURE_CGPS_DUAL_WCDMA
				location_notification.e_sub_id = z_umts_cp_wcdma_session_status.z_rrc_as_id_info.e_cached_rrc_as_id;
		  #else
        ...
        {
          MSG_1(MSG_SSID_GPSSM, MSG_LEGACY_ERROR, "Privacy: Attempting to send notification for non-UMTS RAT %u", (uint8)phone_state_info.srv_system);
        }
      ...
      {
        /* if auto accept is enabled, do not send to AP */
        if ( TRUE == tm_core_mtlr_auto_accept() )
        {
          /* Auto accept is enabled, no need to send notification */
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"MTLR auto accepted e911_setting = %d, auto_accept set to %d",
        ...
        {
          /* auto accept not enabled, send to AP */
    ...
    else
    {
      /* E911 case: Process regardless of LBS lock. Respond
         immediately. NO AP notification if auto accept is enabled */
      if ( TRUE == tm_core_mtlr_auto_accept() )
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"MTLR auto accepted e911_setting = %d, auto_accept set to %d",
                e_e911_effect, tm_core_mtlr_auto_accept(), 0);
      ...
      {
        /* 911 case: auto accept is not set. send to user notification */
        cp_ss_pass_notification_verifcation_to_pdapi(p_sups_data);
      }      
    }
  }
...
void tm_umts_cp_ss_lcs_molr_conf_proc (tm_cm_sups_data_s_type *p_sups_data)
{
  ...
  if ( z_umts_cp_ss_session_status.third_party == FALSE ) 
  {
    ...
    z_cm_stop_info.stop_params.lcs_molr_completed_cmd_params.ss_ref = 
     ...
     {
       MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"SS realseComp req.", 0, 0, 0);
     }
     else
     {
       MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Failed releaseComp", 0, 0, 0);
     }
  }
  ...
  {
    /* MOLR-ReturnResult */
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "recvd CONF-RetRes refNum=%u", p_sups_data->u_ss_ref_num, 0, 0);
    cp_ss_log_lcs_molr_return_result ( &p_molr_res->data.molr_res_data );
  }

  else if ( p_molr_res->data_type == SS_ERROR_INFO )
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "recvd CONF_SS_Error refNum=%u", p_sups_data->u_ss_ref_num, 0, 0);
    cp_ss_log_lcs_molr_return_error ( &p_molr_res->data.cm_ss_error );
  }

  else if ( p_molr_res->data_type == CC_CAUSE_INFO )
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "recv CC_CAUSE refNum=%u", p_sups_data->u_ss_ref_num, 0, 0);
  }
 
  else
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"UMTS-CP-SS: ignore molr_conf of %u", p_molr_res->data_type, 0, 0 );
  }  
...
{ 
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"UMTS-CP-SS: pass notif_verif to pdapi", 0, 0, 0);
  ...
  {
    /* QQQ: TBD */
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"UMTS-CP-SS: unsupported loc type: %u", p_umts_cp_ss_pdapi_notif_verif->location_type, 0, 0);
    return;
  }
