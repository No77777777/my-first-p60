...
/*===========================================================================
  ...
  $Header: //components/rel/gnss8.mpss/11.1/gnss/sm/tm/1x_up/vx/src/tm_vx.c#1 $  
  
when       who     what, where, why
--------   ---     ----------------------------------------------------------
12/04/19   py      On bootup, read NV for all subs and cache and update VX config
...
/*===========================================================================
FUNCTION: tm_vx_read_nv_from_cache

DESCRIPTION: VX config keep only one copy of NV and so that will be updated 
             after reading Cached NV stored for DD sub in sm_nv_cached_values

PARAMETERS: None

===========================================================================*/
void tm_vx_read_nv_from_cache(void)
{

  MSG( MSG_SSID_GPSSM, MSG_LEGACY_HIGH ,"VX NVs read from cache");
  
  /* Read Vx version first */
  (void) sm_nv_read_ex(NV_GPS1_VX_LCS_AGENT_I, (void *)&tm_vx_nv_read_buffer);
  ...
  (void) sm_nv_read_ex(NV_GPS1_VX_APP_TRUSTED_SETTINGS_I, (void *)&tm_vx_nv_read_buffer);
  ...
  (void) sm_nv_read_ex(NV_GPS1_VX_NI_TELESERVICE_ID_I, (void *)&tm_vx_nv_read_buffer);
  tm_vx_gen_info.vx_config.vx_ni_teleservice_id = (uint32)tm_vx_nv_read_buffer.gps1_vx_ni_teleservice_id;


  /* Read Max MO duration */
  (void) sm_nv_read_ex(NV_GPS1_VX_MO_MAX_DURATION_I, (void *)&tm_vx_nv_read_buffer);
  tm_vx_gen_info.vx_config.vx_mo_max_duration = (uint32)tm_vx_nv_read_buffer.gps1_vx_mo_max_duration;  

#if 0
  /*Following 2 NVs are not used anywhere anymore*/
  /* Read Prev6 only setting */
  (void) sm_nv_read_ex(NV_GPS1_VX_LCS_AGENT_PREV6_ONLY_I, (void *)&tm_vx_nv_read_buffer);
  ...
  (void) sm_nv_read_ex(NV_GPS1_VX_GPS_DURING_VOICE_CALL_I, (void *)&tm_vx_nv_read_buffer);
  ...
  (void) sm_nv_read_ex( NV_CGPS_1X_MPC_SERVER_ADDR_IPV4_I, (void *)&tm_vx_nv_read_buffer );
   tm_vx_gen_info.vx_config.vx_mpc_adrs = tm_vx_nv_read_buffer.cgps_1x_mpc_server_addr_ipv4;

  (void)sm_nv_read_ex( NV_CGPS_1X_MPC_SERVER_PORT_I, (void *)&tm_vx_nv_read_buffer );
  tm_vx_gen_info.vx_config.vx_mpc_port_id = tm_vx_nv_read_buffer.cgps_1x_mpc_server_port;
}

/*===========================================================================
FUNCTION: tm_vx_read_nv_from_hw_sub

DESCRIPTION: For a particular sub, read NV value from HW and cached the return values in sm_nv_cached_values. 

PARAMETERS: Sub on which HW read needs to be issued

===========================================================================*/
void tm_vx_read_nv_from_hw_sub(sys_modem_as_id_e_type sub)
{
  MSG( MSG_SSID_GPSSM, MSG_LEGACY_HIGH ,"TM VX Read NVs from H/W and cache");
  /* Read Vx version first */
  (void) sm_nv_read_from_hw_sub_and_cache(NV_GPS1_VX_LCS_AGENT_I, (void *)&tm_vx_nv_read_buffer,sub);

  /* Read Client Trusted Settings */
  (void) sm_nv_read_from_hw_sub_and_cache(NV_GPS1_VX_APP_TRUSTED_SETTINGS_I, (void *)&tm_vx_nv_read_buffer,sub);

  /* Read Vx teleservice ID */
  (void) sm_nv_read_from_hw_sub_and_cache(NV_GPS1_VX_NI_TELESERVICE_ID_I, (void *)&tm_vx_nv_read_buffer,sub);

  /* Read Max MO duration */
  (void) sm_nv_read_from_hw_sub_and_cache(NV_GPS1_VX_MO_MAX_DURATION_I, (void *)&tm_vx_nv_read_buffer,sub);

#if 0
  /*Following 2 NVs are not used anywhere anymore*/
  /* Read Prev6 only setting */
  (void) sm_nv_read_from_hw_sub_and_cache(NV_GPS1_VX_LCS_AGENT_PREV6_ONLY_I, (void *)&tm_vx_nv_read_buffer,sub);

  /* Read GPS during Voice setting */
  (void) sm_nv_read_from_hw_sub_and_cache(NV_GPS1_VX_GPS_DURING_VOICE_CALL_I, (void *)&tm_vx_nv_read_buffer,sub);
#endif

  /* Read MPC IP*/
  (void) sm_nv_read_from_hw_sub_and_cache( NV_CGPS_1X_MPC_SERVER_ADDR_IPV4_I, (void *)&tm_vx_nv_read_buffer,sub );

  /*Read MPC Port*/
  (void)sm_nv_read_from_hw_sub_and_cache( NV_CGPS_1X_MPC_SERVER_PORT_I, (void *)&tm_vx_nv_read_buffer,sub );
...
void tm_vx_init(void)
{
  ...
  /* Read NV items */
  /* For each sub, read NV from HW and cache returned values in sm_nv_cached_values*/
  for (sub = SM_CONFIG_MIN_SUB_ID; sub <= SM_CONFIG_MAX_SUB_ID; sub++)
  {
    tm_vx_read_nv_from_hw_sub(sub);
  }
  /*calls a function to update VX config with cached values for DD sub in sm_nv_cached_values*/
  tm_vx_read_nv_from_cache();
...
)
{
  ...
  /*Following change is done to inform protocol to refresh their NV values acc to current sub. Order is maintained in same way 
    protocol initialization is done , 1x read followed by vx read
  */
  if(event_type == PRTL_EVENT_TYPE_CARRIER_NV_REFRESH)
  {
    tm_1x_up_is801_drv_event_handler(event_type, event_payload);
#ifdef FEATURE_GPSONE_VX_LCS_AGENT/* FEATURE_GPSONE_VX_LCS_AGENT */   
    if(event_payload->nv_refresh_payload.read_from_hw)
    {
        /*As on date ,only if refresh event came on DD sub , protocols are notified*/
        tm_vx_read_nv_from_hw_sub((sys_modem_as_id_e_type)event_payload->nv_refresh_payload.sub);

        tm_vx_read_nv_from_cache();
    }
    else
    {  /*Event is DD sub change, so internally read from cache for DD sub and update VX config*/
        tm_vx_read_nv_from_cache();
    }
#endif /* FEATURE_GPSONE_VX_LCS_AGENT */   
    ret_val = TRUE;
