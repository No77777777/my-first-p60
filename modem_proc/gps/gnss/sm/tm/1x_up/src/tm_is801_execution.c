...
static boolean save_prm_to_buf(sm_GpsMeasRptStructType *prm_meas)
{
  ...
  if( (prm_meas->z_MeasBlk.z_GnssHeader.e_MeasBlkSrc != GNSS_MEAS_BLK_SRC_GPS) &&
      (prm_meas->z_MeasBlk.z_GnssHeader.e_MeasBlkSrc != GNSS_MEAS_BLK_SRC_GLO) )
  {
    ERR_FATAL("Unknown constellation passed in prm %d", 
                 (uint8) prm_meas->z_MeasBlk.z_GnssHeader.e_MeasBlkSrc, 0, 0);
  }
  ...
  if( (prm_meas->z_MeasBlk.z_GnssHeader.e_MeasBlkSrc == GNSS_MEAS_BLK_SRC_GLO) &&
  ...
  if(up_prm_buff_ptr)
  {
     if(prm_meas->z_MeasBlk.z_GnssHeader.u_NumSvs == 0)
     {
       /* create bogus PRMs */
       memset( up_prm_buff_ptr, 0, sizeof(*up_prm_buff_ptr)); /* sizeof sm_GpsMeasRptStructType */
       up_prm_buff_ptr->z_MeasBlk.z_GnssHeader.u_NumSvs = 0;
       up_prm_buff_ptr->z_MeasBlk.z_GnssHeader.z_MeasClock.z_Time.z_GpsTime.w_GpsWeek 
                                           = C_GPS_WEEK_UNKNOWN;
     }
     else
     {
       sm_GnssMeas_memscpy((void *)up_prm_buff_ptr, sizeof(*up_prm_buff_ptr), prm_meas);
     }
...
===========================================================================*/
boolean tm_1x_up_execute_fns(const is801_operation ops[], const void *params[])
{
  int         idx = 0;
  boolean     ret_val = TRUE;
  post_data   u;
