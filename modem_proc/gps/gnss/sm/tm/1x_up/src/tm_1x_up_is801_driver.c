...
/*===========================================================================
  ...
  $Header: //components/rel/gnss8.mpss/11.1/gnss/sm/tm/1x_up/src/tm_1x_up_is801_driver.c#1 $

when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
03/12/19   mj      Location Privacy Feature
04/17/18   py      event PRTL_EVENT_TYPE_CARRIER_NV_REFRESH handled
...
void tm_1x_up_is801_drv_end_session(tm_sess_stop_reason_e_type stop_reason)
{
  ...
  if (TM_STOP_REASON_COMPLETED != stop_reason)
  {
    if (!TM_CORE_CONFIG_DD(gnss_1x_up_supl_enable))
    {
      location_notification.q_prtl_type = TM_PRTL_TYPE_1X_UP;
      location_notification.e_pdapi_client_id = tm_1x_up_session_info.api_param.pd_api_info.client_id;
  ...
  */
      ...
      ((tm_1x_up_is801_drv_session_active()) && (tm_1x_up_session_info.mode_info.mode == TM_1X_UP_MODE_DLOAD)) )
  {
    ...
    if (tm_1x_up_config.msb_throttling_info.msb_throttling_enabled)
    {
      ...
      (void) time_get_uptime(current_sys_time);
...
// TBD check if ok to delete this            TM_1X_UP_SESS_START_MASK  | 
  ...
  /* Send location notification when Ref_Loc is sent to PDE */
  if (!TM_CORE_CONFIG_DD(gnss_1x_up_supl_enable))
  {
    location_notification.q_prtl_type = TM_PRTL_TYPE_1X_UP;
    location_notification.e_pdapi_client_id = tm_1x_up_session_info.api_param.pd_api_info.client_id;
...
boolean tm_1x_up_is801_drv_handle_prm( sm_GpsMeasRptStructType  *info_ptr)
{
  ...
  if (info_ptr->z_MeasBlk.z_GnssHeader.u_SeqNum == info_ptr->z_MeasBlk.z_GnssHeader.u_MaxMessageNum)
  {
    tm_1x_up_session_info.mgp_info.prm_on = FALSE;
  ...
  /* Send location notification when Meas are sent to PDE */
  if (!TM_CORE_CONFIG_DD(gnss_1x_up_supl_enable))
  {
    location_notification.q_prtl_type = TM_PRTL_TYPE_1X_UP;
    location_notification.e_pdapi_client_id = tm_1x_up_session_info.api_param.pd_api_info.client_id;
...
)
{
  ...
  systemtime = time_get_uptime_secs();
