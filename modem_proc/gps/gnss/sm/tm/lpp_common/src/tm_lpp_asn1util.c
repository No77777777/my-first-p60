...
INITIALIZATION AND SEQUENCING REQUIREMENTS
  ...
  $Header: //components/rel/gnss8.mpss/11.1/gnss/sm/tm/lpp_common/src/tm_lpp_asn1util.c#2 $
  $DateTime: 2020/09/04 13:58:08 $
  $Author: pwbldsvc $

  when       who     what, where, why
  --------   ---     ------------------------------------------------------
  03/06/14   jv      Check to ensure that asn1 ctxt mem reset happnes only if no tx are pending
                              ...
                              tm_pdu_buf_type *pdu_buf_ptr)
{
  ...
  if ( (encode_status = pu_setBuffer (&(tm_lpp_osys_priv_ptr->dd_ptr->tm_asn1_ctxt),
                                  NULL, 
                                  0, 
                                  FALSE)
       ) != 0 
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"TM_LPP_ASN_UTIL: ASN1 LPP encoding error: stat = %u", encode_status, 0, 0);
  ...
  if(encode_status != 0)
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"TM_LPP_ASN_UTIL: Encoding failed. Status = %d", encode_status, 0, 0);
...
void tm_lpp_init_encoding_context()
{
  /* release all dynamically allocated buffers, so there is no carried over
     memory , if no tx pending in the table */
  if(tm_lpp_tx_mgr_is_osys_ctxt_mem_reset_allowed())
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"TM_LPP_ASNUTIL:**********Init LPP context**********", 0,0,0);
                              ...
                              int *decode_status_ptr)
{
  ...
  *decode_status_ptr = pu_setBuffer(&(tm_lpp_osys_priv_ptr->dd_ptr->tm_asn1_ctxt),
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"TM_LPP_ASN1_UTIL:Decode Error. Could not initialize buffer", 0, 0, 0);
  ...
  *decode_status_ptr =
  ...
  if (*decode_status_ptr != 0)
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"TM_LPP_ASN_UTIL: Decoding failure: %d", *decode_status_ptr, 0, 0);
    rtxErrPrint(&(tm_lpp_osys_priv_ptr->dd_ptr->tm_asn1_ctxt));
    if (0 != rtxErrReset(&(tm_lpp_osys_priv_ptr->dd_ptr->tm_asn1_ctxt)))
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"TM_LPP_ASN_UTIL: rtxErrReset failure", 0, 0, 0);
    }
  ...
  else
  {
    ...
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"TM_LPP_ASN_UTIL: Decoding status %d, length: %d", 
...
)
{
  ...
  if ( ptr != NULL) 
  {
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Null Osys ptr 0x%x", ptr, 0, 0);
    }
    *ptr = NULL;
  }
  else
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Null Osys ptr ptr", 0, 0, 0);
  }
  return;
...
void tm_asn1_free_lpp_message_IEs(LPP_Message *p_lpp_pdu, int pdu_num)
{
  ...
  if ( p_lpp_pdu == NULL )
  {                 
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"TM_LPP_ASN_UTIL: Nothing to free. PDU is null %d", pdu_num, 0,0);
    return;
  }
  
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"TM_LPP_ASN_UTIL: Free LPP PDU  %d", pdu_num, 0,0);
...
)
{
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"TM_LPP_ASN1UTIL: Null data ptr",0,0,0);
    return result;
  }
