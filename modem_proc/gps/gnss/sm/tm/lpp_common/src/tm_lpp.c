...
/*===========================================================================
  ...
  $Header: //components/rel/gnss8.mpss/11.1/gnss/sm/tm/lpp_common/src/tm_lpp.c#3 $ $DateTime: 2020/08/18 18:25:02 $ $Author: pwbldsvc $
  ...
  08/20/18   sum     Added support for extended ARFCN.
  05/29/18   sum     Always enabled GNSS_LPP_DOPPLER_CONF_EARFCN feature flag for OTDOA
  03/20/18   skm     Supl E911 over Wifi
  12/08/17   py      Corrected filling LPP prov capab with correct camped cell band 
  10/26/17   rk      Report the GLO only position report as GLO position with GPS ref time.
  10/18/17   rk      LPP Abort checks for 'do not request ACK' bit to decide if the ACK need to
                      present in the outgoing message.
  06/20/17   gk      disable AA support if MSA is disabled.
  05/08/17   rn      Added LPP CP 2-Timer support.
  04/12/17   kpn     Include OTDOA Error in PLI if indicated by OTDOA engine along with valid measurements
  03/10/17   gk      Disable Support for IONO in AGPS
  03/10/17   gk      Disable Support for UTC model in AGPS
  01/23/16   kpn     Rework on passing as_id to modified APIs that need as_id implementation for L+L.
  09/19/16   kpn     Pass as_id to modified APIs to fix compilation errors for L+L implementation.
  07/28/16   skm     2Timer re-arc for LPPe includes CR's(985835,1034212,1034213,1034216,1034219) 
  07/27/16   rn      Fixed tm_lpp_encode_request_assistance to properly handle Glonass TimeModel. 
  06/30/16   skm     set correct EarlyFixReport flags for greater than 24AD cases
  06/15/16   gk      req cap aux and time models should be only sent when GLO is enabled.
  03/11/16   ak      Update API to read supported LTE bands.
  ...
  03/02/15   rk      Fixed decode error from filling ext element.
...
static boolean tm_lpp_copy_lppe_epdu
(
  tm_lpp_transaction_info_s_type *p_lpp_tx_type,
  EPDU_Sequence                  *p_epdu_seq
)
{
  boolean b_retval = FALSE;
  if((p_lpp_tx_type == NULL) || (p_epdu_seq == NULL)) 
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Null ptr!!", 0, 0, 0);
    return b_retval;
  }

  p_epdu_seq->n = 1;
  p_epdu_seq->elem = 
    tm_lpp_asn1_osys_IE_malloc(sizeof(*(p_epdu_seq->elem)));

  if(p_epdu_seq->elem == NULL)
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Could not allocate EPDU buffer!", 0, 0, 0);
    return b_retval;    
  }

  p_epdu_seq->elem->ePDU_Body.data = 
          tm_lpp_asn1_osys_IE_malloc(p_lpp_tx_type->z_lpp_data_type.z_epdu_buf.length);
  if(p_epdu_seq->elem->ePDU_Body.data == NULL)
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Could not allocate EPDU buffer!", 0, 0, 0);
    tm_lpp_asn1_osys_IE_free((void **)&p_epdu_seq->elem);
    return b_retval;    
  }
  
  p_epdu_seq->elem->ePDU_Identifier.m.ePDU_NamePresent = 0;
  p_epdu_seq->elem->ePDU_Identifier.ePDU_ID = 1;        
  p_epdu_seq->elem->ePDU_Body.numocts = 
          p_lpp_tx_type->z_lpp_data_type.z_epdu_buf.length;
  memscpy( 
          (void *) (p_epdu_seq->elem->ePDU_Body.data), 
          p_epdu_seq->elem->ePDU_Body.numocts, 
          (void *) p_lpp_tx_type->z_lpp_data_type.z_epdu_buf.value, 
          p_lpp_tx_type->z_lpp_data_type.z_epdu_buf.length);
                                         ...
                                         uint16 const u_total_supp_bands )
{
  ...
  while ( u_ue_supp_extn_bands != 0 )
  {
    ...
    {
      tm_lpp_fill_support_extended_bands_impl( p_otdoa_prov_cap, lte_band);
    }
                                    ...
                                    sys_lte_band_mask_e_type const ue_supported_lte_bands )
{
  ...
  **/
  if ( ( camped_lte_cell_band < ASN1V_maxBands) || (u_ue_supp_normal_bands != 0) )
  {
  ...
  */
  if ( u_total_supp_normal_bands < ASN1V_maxBands )
  {
    ...
    *  camped_lte_cell_band will be (1..63) or (1..256) when LTE U feature enabled.
    **/
    if ( (camped_lte_cell_band > ASN1V_maxBands) || (ue_supported_lte_bands.bits_65_128 != 0) ||
  ...
  */
  ...
  if ( TRUE == b_ue_has_extn_bands )
  {
    ...
    /* Check the camped cell LTE band is in extended range (65..256) & populate the IE accordingly. */
    if ( camped_lte_cell_band > ASN1V_maxBands )
    {
      /* Extended bands :
      *  Fill camped extended LTE cell band IE.  Spec : bandEUTRA0v9a0 (65..256). */
  ...
  {
    /* Normal bands:
    *  Fill camped normal LTE cell band IE.  Spec : bandEUTRA (1..64). MPSS : camped_lte_cell_band (1..63)
    */
    p_otdoa_prov_cap->supportedBandListEUTRA.elem[p_otdoa_prov_cap->supportedBandListEUTRA.n].bandEUTRA = (uint8)camped_lte_cell_band;
    p_otdoa_prov_cap->supportedBandListEUTRA.n++;
  }
  ...
  {
    lte_band = 1; /* Start from LTE band 1..64 */
    ...
    while ( u_ue_supp_normal_bands != 0 )
  {
    ...
    {
      /* bandEUTRA (1..64) */
      p_otdoa_prov_cap->supportedBandListEUTRA.elem[p_otdoa_prov_cap->supportedBandListEUTRA.n].bandEUTRA = lte_band;
      p_otdoa_prov_cap->supportedBandListEUTRA.n++;
    }
  ...
  if ( TRUE == b_ue_has_extn_bands )
  {
    ...
    tm_lpp_fill_support_extended_bands( ue_supported_lte_bands.bits_65_128, camped_lte_cell_band, 65, p_otdoa_prov_cap, u_total_supp_bands );
    /* Extended LTE band starting from 129..192 */
    tm_lpp_fill_support_extended_bands( ue_supported_lte_bands.bits_129_192, camped_lte_cell_band, 129, p_otdoa_prov_cap, u_total_supp_bands );
    /* Extended LTE band starting from 193..256 */
    tm_lpp_fill_support_extended_bands( ue_supported_lte_bands.bits_193_256, camped_lte_cell_band, 193, p_otdoa_prov_cap, u_total_supp_bands );
  }
#endif
                                                      ...
                                                      GNSS_ID_gnss_id u_gnss_id )
{
  ...
  p_lpp_generic_ad_element->m.gnss_AcquisitionAssistanceSupportPresent = 0;
#ifdef FEATURE_GNSS_LPP_DOPPLER_CONF_EARFCN
  ...
  if (LPP_GNSS_ID_GLONASS == u_gnss_id)
  {
                                  ...
                                  )
{
  ...
  if ((NULL == p_lpp_pdu) ||
      (NULL == p_lpp_tx_type))
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"TM_LPP: Couldnt allocate memory for LPP Prov Cap Msg", 0, 0, 0);
    return ( FALSE );
  }

  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"TM_LPP: Encode Provide Cap. Flags 0x%x", 
           p_lpp_tx_type->z_lpp_data_type.p_lpp_prov_cap_type->q_flags, 0, 0);
  ...
  if ( p_lpp_tx_type->u_lpp_session_type == TM_LPP_SESSION_TYPE_CP )
  {
    ...
    (void)tm_lpp_cp_get_as_id(&e_as_id);
  }
  /* LPP CP Specific Changes : END */
  else
  {
    /* u_lpp_session_type is TM_LPP_SESSION_TYPE_UP.
       the SUB ID to be used is set by.
       1)In coming SUPL_INIT in case of NI
       2)In case of MO, its the Default DD sub*/
    e_as_id = tm_core_get_current_dd_subs_id(); 
  ...
  p_lpp_prov_cap->criticalExtensions.u.c1->u.provideCapabilities_r9 =
    (ProvideCapabilities_r9_IEs *) tm_lpp_asn1_osys_IE_malloc(sizeof(ProvideCapabilities_r9_IEs));

  if (p_lpp_prov_cap->criticalExtensions.u.c1->u.provideCapabilities_r9 == NULL)
  {
    tm_asn1_free_lpp_message_IEs(p_lpp_pdu, LPP_MESSAGE_PROV_CAP);
    return ( FALSE );
  }
...
#ifdef FEATURE_GNSS_LPPE
  if(tm_lpp_e_encode_provide_cap(p_lpp_tx_type))
  {
    if(tm_lpp_copy_lppe_epdu(
              p_lpp_tx_type, 
              &p_lpp_prov_cap_r9->epdu_ProvideCapabilities) == TRUE)
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"EPDU copied successfully!!", 0, 0, 0);
      p_lpp_prov_cap_r9->m.epdu_ProvideCapabilitiesPresent = 1;
    }
    else
    {
      p_lpp_prov_cap_r9->m.epdu_ProvideCapabilitiesPresent = 0;
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Could not copy EPDU!!", 0, 0, 0);      
    }

    /* Either we have copied over the EPDU into the LPP buffer, or not. 
     * Either way, we dont need the encoded EPDU buffer any more. Free it! */
    tm_lpp_e_asn1_free_osys_buf(p_lpp_tx_type->z_lpp_data_type.z_epdu_buf.value);
  }
#endif
  if ( p_lpp_tx_type->z_lpp_data_type.p_lpp_prov_cap_type->q_flags & LPP_PROV_AGNSS_CAP )
  {
    if ( (p_lpp_tx_type->z_lpp_data_type.p_lpp_prov_cap_type->z_agnss_capabilities.u_pos_mode_supported) != 0 )
    {
      ...
      /* advertise AA AD support only if MSA is enabled*/
      if (p_lpp_tx_type->z_lpp_data_type.p_lpp_prov_cap_type->z_agnss_capabilities.u_pos_mode_supported &
                LPP_POS_MODE_UE_ASSISTED)
      {
      ...
      if ( p_lpp_tx_type->z_lpp_data_type.p_lpp_prov_cap_type->u_glonass_supported == TRUE )
      {
        ...
        /* advertise AA AD support only if MSA is enabled*/
        if (p_lpp_tx_type->z_lpp_data_type.p_lpp_prov_cap_type->z_agnss_capabilities.u_pos_mode_supported & 
                  LPP_AGNSS_POS_MODE_UE_ASSISTED)
        {
          p_lpp_generic_ad_element->m.gnss_AcquisitionAssistanceSupportPresent = 1;
        }
        else
  ...
  if ( p_lpp_tx_type->z_lpp_data_type.p_lpp_prov_cap_type->q_flags & LPP_PROV_OTDOA_CAP )
  {
    if ( p_lpp_tx_type->z_lpp_data_type.p_lpp_prov_cap_type->u_otdoa_supported )
    {
      ...
      OSSETBIT(p_lpp_prov_cap_r9->otdoa_ProvideCapabilities.otdoa_Mode, LPP_PROV_OTDOA_PROV_CAP);
      ...
      if ( p_lpp_tx_type->z_lpp_data_type.q_flags & LPP_CELL_INFO_VALID )
      {
        ...
        camped_cell_band = tm_umts_get_lte_band_info(e_as_id,
                                                     p_lpp_tx_type->z_lpp_data_type.z_cached_cell_info.z_CellInfoGlobalCached.z_CellInfoLTEGlobal.freq,
                                                     &camped_cell_band_valid);
        
        /* lte_rrc_get_band_from_dl_earfcn LTE api expects the caller to increment the band value before usage whether it is normal/extended*/
		camped_cell_band++;

        /* Secondly get list of other UE supported bands. Atpresent UE doesn't support band more than 63.*/
        ue_support_lte_bands = tm_umts_get_lte_band_pref_info(e_as_id);
      ...
      if ( TRUE == tm_umts_lte_is_inter_freq_supported(e_as_id) )
      {
  ...
  if ( p_lpp_tx_type->z_lpp_data_type.p_lpp_prov_cap_type->q_flags & LPP_PROV_ECID_CAP )
  {
    ...
    if ( p_lpp_tx_type->z_lpp_data_type.p_lpp_prov_cap_type->u_ecid_supported )
    {
      ...
      /* Since Nta_offset is always added in the measurements when UE is in TDD mode. always set this flag to TRUE*/
      p_lpp_prov_cap_r9->ecid_ProvideCapabilities.m._v2ExtPresent = 1;
      p_lpp_prov_cap_r9->ecid_ProvideCapabilities.m.ueRxTxSupTDD_r13Present = 1;
      p_lpp_prov_cap_r9->ecid_ProvideCapabilities.ueRxTxSupTDD_r13 = 
                                      ECID_ProvideCapabilities_ueRxTxSupTDD_r13_true;
      #endif /* FEATURE_NR5G_SA */
    }
  }

#ifdef FEATURE_NR5G_SA
  /* WLAN,BTLE,BARO and DBH in Rel13 IE's are not supported,
     set the _r13Present to "0", ti indicate empty message since
     provide capabilities does not have an error code which can be sent */
  if ( p_lpp_tx_type->z_lpp_data_type.p_lpp_prov_cap_type->q_flags & LPP_PROV_REL15_IE_CAP )
  {
    /*Mark the _v2ExtPresent flag for the new capabilities to be decoded*/
    p_lpp_prov_cap_r9->m._v2ExtPresent = 1;
    p_lpp_prov_cap_r9->m.bt_ProvideCapabilities_r13Present = 0;
    p_lpp_prov_cap_r9->m.sensor_ProvideCapabilities_r13Present = 0;
    p_lpp_prov_cap_r9->m.wlan_ProvideCapabilities_r13Present = 0;
    p_lpp_prov_cap_r9->m.tbs_ProvideCapabilities_r13Present = 0;
  }
#endif /* FEATURE_NR5G_SA */
  ...
  if ( (encode_status != 0) || (p_lpp_tx_type->z_lpp_data_type.z_pdu_buf.value == NULL) || (p_lpp_tx_type->z_lpp_data_type.z_pdu_buf.length == 0) )
  {
    ...
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"TM_LPP: Error encoding LPP Provide Cap", 0, 0, 0);
...
boolean tm_lpp_encode_request_assistance( tm_lpp_transaction_info_s_type *p_lpp_tx_type )
{
  ...
  MSG(MSG_SSID_GPSSM, MSG_LEGACY_MED,"TM_LPP: Encode Request AD");

  if ( p_lpp_tx_type == NULL )
  {
    MSG(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"TM_LPP: Null transcation pointer");
    return ( FALSE );
  }
  ...
  if ( p_lpp_pdu == NULL )
  {
    MSG(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"TM_LPP: Buffer allocation failed");
    return ( FALSE );
  }
  p_lpp_tx_type->z_lpp_data_type.p_pdu_ptr = p_lpp_pdu;
  p_lpp_pdu->m.transactionIDPresent = 1;
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"TM_LPP: Buffer allocation failed", 0, 0, 0);
  ...
  /*encode the LTEcellid only if the ACTIVE RAT is LTE.
    In the case for SUPL over WLAN, CM can show srv_sys as LTE but call
    could be on WLAN RAT. in which case we should WLAN MAC address for Primary cell/Location ID.*/
  if ( ( p_lpp_tx_type->z_lpp_data_type.z_cached_cell_info.u_CachedCellIDValid == TRUE ) &&
       ( p_lpp_tx_type->z_lpp_data_type.z_cached_cell_info.e_ActiveRAT == CELLDB_ACTIVERAT_LTE ) )
  {
    ...
    if ( b_ret == FALSE )
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"TM_LPP: failed to convered cellId", 0, 0, 0);
    }
#else
       ...
       (p_GloAssistData->q_WishListMask) ) // Glonass
  {
    ...
    if ( p_GloAssistData->q_WishListMask & C_GNSS_ASSIST_NAVMODL ||
         ...
         p_GloAssistData->q_WishListMask & C_GNSS_ASSIST_TIME_MODELS ||
      ...
      if ( p_GnssAssistData->q_WishListMask & C_GPS_ASSIS_NAVMODL )
      {
        ...
        if ( (p_GnssAssistData->u_EphInfoIncl == TRUE) && (p_GnssAssistData->w_GpsWeek != C_GPS_WEEK_UNKNOWN) )
        {
          ...
          if ( p_generic_ad_req->gnss_NavigationModelReq.u.storedNavList->satListRelatedDataList.n > 0 )
          {
            ...
            if ( p_generic_ad_req->gnss_NavigationModelReq.u.storedNavList->satListRelatedDataList.elem != NULL )
            {
              for ( u_i = 0; u_i < p_generic_ad_req->gnss_NavigationModelReq.u.storedNavList->satListRelatedDataList.n; ++u_i )
              {
                if ( p_GnssAssistData->z_SvEphIode[u_i].w_Sv > 0 )
                {
                  ...
                  if ( b_ret == FALSE )
                  {
                    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"TM_LPP: could not convert the int to bitstring", 0, 0, 0);
                  }
                }
              }
            }
            ...
            {
              MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"TM_LPP: LPP ReqAD buffer allocation failed", 0, 0, 0);
              tm_asn1_free_lpp_message_IEs(p_lpp_pdu, LPP_MESSAGE_REQ_AD);
              return ( FALSE );
            }
          }
  ...
  if ( p_GloAssistData->q_WishListMask )
  {
    ...
    if ( u_GnssGenericADReqElemIndex < p_req_ad_r9->a_gnss_RequestAssistanceData.gnss_GenericAssistDataReq.n )
    {
      ...
      if ( p_GloAssistData->q_WishListMask & C_GNSS_ASSIST_NAVMODL )
      {
        ...
        if ( p_generic_ad_req->gnss_NavigationModelReq.u.storedNavList == NULL )
        {
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"TM_LPP: Glonass nav buffer allocation failed", 0, 0, 0);
          tm_asn1_free_lpp_message_IEs(p_lpp_pdu, LPP_MESSAGE_REQ_AD);
          return ( FALSE );
        }
        ...
        {
          MSG_1(MSG_SSID_GPSSM, MSG_LEGACY_LOW,"TM_LPP: Load the available Glonass eph. Num SVs %u", p_GnssAssistData->z_GloAssistData.u_NumSvs);
            ...
            if ( p_generic_ad_req->gnss_NavigationModelReq.u.storedNavList->satListRelatedDataList.elem != NULL )
            {
              for ( u_i = 0; u_i < p_generic_ad_req->gnss_NavigationModelReq.u.storedNavList->satListRelatedDataList.n; u_i++ )
              {
                if ( p_GnssAssistData->z_GloAssistData.z_SvEphIode[u_i].w_Sv > GLO_SLOTID_OFFSET )
                {
                  ...
                  if ( b_ret == FALSE )
                  {
                    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"TM_LPP: Glonass : could not convert the int to bitstring", 0, 0, 0);
                  }
                }
              }
            }
  ...
  {
    /* ASN.1 encoding failed */
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"TM_LPP: Error encoding LPP Request AD", 0, 0, 0);
...
boolean tm_lpp_gps_sv_least_available( const gnss_MeasBlkStructType *const pGnssMeasBlk )
{
  ...
  if ( pGnssMeasBlk->z_GnssHeader.u_NumSvs > 0 )
  {
    if ( GNSS_MEAS_BLK_SRC_GPS == pGnssMeasBlk->z_GnssHeader.e_MeasBlkSrc )
    {
      /* Check GPS Meas clock is valid */
      for ( uI = 0; uI < pGnssMeasBlk->z_GnssHeader.u_NumSvs; ++uI )
      {
...
boolean tm_lpp_glo_sv_least_available( const gnss_MeasBlkStructType *const pGnssMeasBlk )
{
  ...
  if ( pGnssMeasBlk->z_GnssHeader.u_NumSvs > 0 )
  {
    if ( GNSS_MEAS_BLK_SRC_GLO == pGnssMeasBlk->z_GnssHeader.e_MeasBlkSrc )
    {
 ...
 *    b_ls_gnss_method_bmap : LS(LocationServer) interest in GNSS measurement system (GPS and/or
 *    GLONASS)
 *
 * Return value:
                                         ...
                                         uint8 b_ls_gnss_method_bmap )
{
  ...
  /* Stringent measurement evaluation is required or not */
  boolean b_status_check_needed = FALSE;
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"TM_LPP: Build LS msg from %u GNSS system meas report. LS choice 0x%02X.", p_meas_reports->u_meas_reports_bmap,
  ...
  if ( (p_meas_reports->u_meas_reports_bmap & SM_GNSS_MEAS_REPORTS_BMAP_GPS) &&
       (GNSS_MEAS_BLK_SRC_GPS == p_meas_reports->z_meas_reports[SM_GNSS_MEAS_SYSTEM_GPS].z_MeasBlk.z_GnssHeader.e_MeasBlkSrc) )
  {
    ...
    {
      pGnssMeasClock = &p_meas_reports->z_meas_reports[SM_GNSS_MEAS_SYSTEM_GPS].z_MeasBlk.z_GnssHeader.z_MeasClock;
  ...
  if ( b_ls_gnss_method_bmap & C_POS_SOURCE_GLO )
  {
    if ( (p_meas_reports->u_meas_reports_bmap & SM_GNSS_MEAS_REPORTS_BMAP_GLONASS) &&
         (GNSS_MEAS_BLK_SRC_GLO == p_meas_reports->z_meas_reports[SM_GNSS_MEAS_SYSTEM_GLONASS].z_MeasBlk.z_GnssHeader.e_MeasBlkSrc) )
    {
      ...
      {
        pGnssMeasClock = &p_meas_reports->z_meas_reports[SM_GNSS_MEAS_SYSTEM_GLONASS].z_MeasBlk.z_GnssHeader.z_MeasClock;
      }
    }
    else
    {
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"TM_LPP: No qualified measurement to build.", 0, 0, 0);
    return ( FALSE ); // failure, fill error code in the calling func
  }

  b_status_check_needed = p_meas_reports->m.s_StatusCheckNeeded;

  /* GPS */
  if ( pGpsMeasRpt != NULL )
  {
    /* if this number is ever greater than 16, limit to the first 16 */
    if ( pGpsMeasRpt->z_GnssHeader.u_NumSvs < N_ACTIVE_GPS_CHAN )
    {
      uTotalGpsSvs = pGpsMeasRpt->z_GnssHeader.u_NumSvs;
    }
    else
    {
    ...
    MSG(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "tm_lpp_prov_loc_gnss_meas_build: Calling BlacklistFilter");
    for ( u_i = 0; u_i < uTotalGpsSvs; u_i++ )
    {
      if ( (FALSE == gnss_SvIsBlacklisted(&tm_core_info.config_info.z_GnssPersistSV, p_gps_meas_per_sv->w_Sv)) &&
           (p_gps_meas_per_sv->w_Cno > 0)   &&
  ...
  if ( pGloMeasRpt != NULL )
  {
    ...
    if ( pGloMeasRpt->z_GnssHeader.u_NumSvs < N_ACTIVE_GLO_CHAN )
    {
      uTotalGloSvs = pGloMeasRpt->z_GnssHeader.u_NumSvs;
    }
    else
    {
    ...
    for ( u_i = 0; (u_i < uTotalGloSvs); ++u_i, ++p_glo_meas_per_sv )
    {
      if ( (p_glo_meas_per_sv->w_Cno > 0) &&
           (FALSE == gnss_SvIsBlacklisted(&tm_core_info.config_info.z_GnssPersistSV, p_glo_meas_per_sv->w_Sv)) && 
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"TM_LPP: Can't provide all LS requested measurements. Fill GNSS-Error.", 0, 0, 0);
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"TM_LPP: ProvLocInfo GNSS-Error build buffer alloc failure", 0, 0, 0);
      return ( FALSE ); /* failure, fill error code in the calling func */
    }
  ...
  if ( pGnssMeasClock != NULL )
  {
    ...
    if ( p_element == NULL )
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"TM_LPP: ProvLocInfo GNSS Meas build buffer alloc failure", 0, 0, 0);
      return ( FALSE ); // failure, fill error code in the calling func
    }
    /* Initialize */
  ...
  if ( (pGpsMeasRpt != NULL) && (p_element != NULL) )
  {
    ...
    if ( p_gnss_elem == NULL )
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"TM_LPP: ProvLocInfo GPS Meas build buffer alloc failure", 0, 0, 0);
    ...
    if ( NULL ==  p_sat_elem )
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"TM_LPP: Prov Loc GPS Meas build buffer alloc failure", 0, 0, 0);
    ...
    for ( u_i = 0; u_i < uTotalGpsSvs; ++u_i, ++p_gps_meas_per_sv )
    {
      if ((FALSE == gnss_SvIsBlacklisted(&tm_core_info.config_info.z_GnssPersistSV, p_gps_meas_per_sv->w_Sv)) &&
          ...
          (gps_meas_status_bits_check((uint16)p_gps_meas_per_sv->q_MeasStatus, b_status_check_needed)) )
      {
        if((p_gps_meas_per_sv->w_Sv - 1) > C_LPP_PRM_GNSS_SV_ID_MAX)
        {
          MSG_1(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"TM_LPP: Invalid GpsSV ID %d", p_gps_meas_per_sv->w_Sv);
          continue;		
        }
        ...
        /* doppler: provided in m/s
            ...
            */
        f_Num = (p_gps_meas_per_sv->z_FiltMeas.f_SvSpeed  - pGpsMeasRpt->z_GnssHeader.z_MeasClock.z_Freq.f_ClkFreqBias);
        ...
        d_AdjSubMsec = (double)p_gps_meas_per_sv->z_FiltMeas.z_SvTime.f_SubMs
                       + (double)pGpsMeasRpt->z_GnssHeader.z_MeasClock.z_Time.z_GpsTime.f_ClkTimeBias;
    ...
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"GPS Measurements built for %d SVs", uTotalGoodGpsSVs, 0, 0);
  ...
  if ( (pGloMeasRpt != NULL) && (uTotalGoodGloSVs > 0) && (p_element != NULL) )
  {
    ...
    for ( u_i = 0; u_i < uTotalGloSvs; ++u_i, ++p_glo_meas_per_sv )
    {
           ...
           (gnss_meas_status_bits_check(p_glo_meas_per_sv->q_MeasStatus, b_status_check_needed)) )
      {
        ...
        if((p_glo_meas_per_sv->w_Sv - (GLO_SLOTID_OFFSET + 1)) > C_LPP_PRM_GNSS_SV_ID_MAX)
        {
          MSG_1(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"TM_LPP: Invalid GloSV ID %d", p_glo_meas_per_sv->w_Sv);
          continue;
        }
        ...
        f_Num = (p_glo_meas_per_sv->z_FiltMeas.f_SvSpeed  - pGloMeasRpt->z_GnssHeader.z_MeasClock.z_Freq.f_ClkFreqBias);
        x_Word = (int16)(f_Num * 25.0f); /* resolution .04 m/s */
        p_sat_elem->m.dopplerPresent = 1;
        p_sat_elem->doppler = x_Word * (-1); /* need to change polarity since protocol definition of doppler is reverse of what ME is using */
        ...
        d_AdjSubMsec = (double)p_glo_meas_per_sv->z_FiltMeas.z_SvTime.f_SubMs
                       + (double)pGloMeasRpt->z_GnssHeader.z_MeasClock.z_Time.z_GpsTime.f_ClkTimeBias;
    ...
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Glonass measurements built for %d SVs", uTotalGoodGloSVs, 0, 0);
                                        ...
                                        OTDOA_ProvideLocationInformation *p_otdoa_ie)
{
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"TM_LPP: Build Prov Loc OTDOA Meas. GlobalRefPresent %d ",
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"TM_LPP: Could not convert int to bitstring", 0, 0, 0);
  }
  ...
  if ( p_sm_otdoa_data->m.earfcnRefPresent == 1 )
  {
    ...
    else if ( p_sm_otdoa_data->earfcnRef <= ASN1V_maxEARFCN2 )
    {
      ...
      p_otdoa_ie->otdoaSignalMeasurementInformation.m._v2ExtPresent = 1;
      MSG(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "EARFCN populated");
    }
  }
  ...
  if ( p_otdoa_ie->otdoaSignalMeasurementInformation.neighbourMeasurementList.n > 0 )
  {
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"TM_LPP: Prov Loc OTDOA Meas build buffer alloc failure", 0, 0, 0);
      tm_lpp_asn1_osys_IE_free((void **)&p_otdoa_ie->otdoaSignalMeasurementInformation.cellGlobalIdRef.mnc.elem);
      ...
      if ( p_sm_otdoa_data->neighbourMeasurementList.elem[u_i].m.earfcnNeighbourPresent == 1 )
      {
        ...
        else if ( p_sm_otdoa_data->neighbourMeasurementList.elem[u_i].earfcnNeighbour <= ASN1V_maxEARFCN2 )
        {
          ...
          p_otdoa_ie->otdoaSignalMeasurementInformation.neighbourMeasurementList.elem[u_i].earfcnNeighbour_v9a0 =
            p_sm_otdoa_data->neighbourMeasurementList.elem[u_i].earfcnNeighbour;
          p_otdoa_ie->otdoaSignalMeasurementInformation.neighbourMeasurementList.elem[u_i].m._v2ExtPresent = 1;
          MSG(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "EARFCN populated");
        }
      }
...
boolean tm_lpp_ecid_meas_element_build( MeasuredResultsElement *p_meas_element, gnss_lpp_ECID_MeasuredResultsElement *p_sm_data )
{
  ...
  if(p_sm_data->arfcnEUTRA <= ((ASN1V_maxEARFCN)))
  {
    p_meas_element->arfcnEUTRA = p_sm_data->arfcnEUTRA;
  }
  else
  {
    p_meas_element->arfcnEUTRA_v9a0 = p_sm_data->arfcnEUTRA;
    p_meas_element->m.arfcnEUTRA_v9a0Present = TRUE;
    p_meas_element->m._v2ExtPresent = TRUE;
    p_meas_element->arfcnEUTRA = ASN1V_maxEARFCN;
  }
  ...
  {
    #ifdef FEATURE_NR5G_SA
    /*If rsrp_Result_v1470 is present we need to ensure we set the legacy
         rsrp_Result to boundry value according to range for
         legacy rsrp_Result [0..97]
      if rsrq_Result_v1470 is in the range [-17..-1]
         set rsrq_result to boundry valye "0"*/
     if ( p_sm_data->rsrp_Result < RSRP_RESULT_OTA_MIN ) 
     {
       p_meas_element->rsrp_Result = RSRP_RESULT_OTA_MIN;
       p_meas_element->m.rsrp_Result_v1470Present =1;
       p_meas_element->rsrp_Result_v1470 = p_sm_data->rsrp_Result;
     }
     else
     #endif /* FEATURE_NR5G_SA */
     {
       ...
       #ifdef FEATURE_NR5G_SA
       p_meas_element->m.rsrp_Result_v1470Present = 0;
       #endif /* FEATURE_NR5G_SA */
     }
  }
  if ( p_meas_element->m.rsrq_ResultPresent == 1 )
  {
    ...
    #ifdef FEATURE_NR5G_SA
    /*If rsrq_Result_v1470 is present we need to ensure we set the legacy
      rsrq_Result to boundry values according to range for
      legacy rsrp_Result [0..34]
      if rsrq_Result_v1470 is in the range [-30..-1]
         set rsrq_Result to boundry Value "0"
      if rsrp_Result_v1470 is in the range [35..46]
         set rsrq_Result to boundry Value "34" */
    if ( p_sm_data->rsrq_Result < RSRQ_RESULT_OTA_MIN )
    {
      p_meas_element->rsrq_Result = RSRQ_RESULT_OTA_MIN;
      p_meas_element->m.rsrq_Result_v1470Present = 1;
      p_meas_element->rsrp_Result_v1470 = p_sm_data->rsrq_Result;
    }
    else if ( p_sm_data->rsrq_Result > RSRQ_RESULT_OTA_MAX ) 
    {
      p_meas_element->rsrq_Result = RSRQ_RESULT_OTA_MAX;
      p_meas_element->m.rsrq_Result_v1470Present = 1;
      p_meas_element->rsrp_Result_v1470 = p_sm_data->rsrq_Result;
    }
    else
  ...
  #ifdef FEATURE_NR5G_SA
 /* Since these fields rsrp_Result_va1470 and rsrq_Result_v1470 were added as
    part of LPP rel-15, they are encoded only when "_v4ExtPresent" bit is set. */
  if( ( p_meas_element->m.rsrp_Result_v1470Present == 1 ) ||
      ( p_meas_element->m.rsrq_Result_v1470Present == 1 ) )
  {
    p_meas_element->m._v4ExtPresent = 1;
  }
  #endif /* FEATURE_NR5G_SA */
                                         ...
                                         ECID_ProvideLocationInformation *p_ecid_ie )
{
  uint8                                      u_index         = 0;
  boolean                                    b_ret           = TRUE;
  gnss_lpp_ECID_SignalMeasurementInformation *p_sm_ecid_data = NULL;
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"TM_LPP: Prov Loc ECID Meas build", 0, 0, 0);
...

{
  ...
  if ( p_velocity == NULL )
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"TM_LPP: Mem not allocated for vel.", 0, 0, 0);
    return;
  }
  ...
  p_velocity->bearing = (uint16)(abs(s_bearing));
...
FUNCTION lpp_agnss_providelocationinfo_gps_reftime

DESCRIPTION
  This function populates the LPP ProvLocationInfo's location estimates IE with GPS measurement
  reference time information.

PARAMETERS
  p_GnssProvLocInfo : ASN1 LPP ProvLocationInfo structure pointer
  cp_PosReport : Position report from the GNSS engine.
...
===========================================================================*/
static void lpp_agnss_providelocationinfo_gps_reftime( A_GNSS_ProvideLocationInformation *const p_GnssProvLocInfo,
                                                       tm_lpp_pending_pos_to_network_s_type const *const cp_PosReport )
{
  uint32 q_GnssTodMsecModHour = 0;
  float  f_GnssTodFracPart = 0.0F;
  uint16 w_GnssTodFrac250ns = 0;

  if ( ( NULL == p_GnssProvLocInfo ) || (NULL == cp_PosReport) )
  {
    return;
  }

  p_GnssProvLocInfo->m.gnss_LocationInformationPresent = 1;
  p_GnssProvLocInfo->gnss_LocationInformation.measurementReferenceTime.gnss_TimeID.gnss_id = LPP_GNSS_ID_GPS;

  /* convert GPS time to the right format for LPP */
  q_GnssTodMsecModHour = cp_PosReport->q_fix_time_gps_msec;
  f_GnssTodFracPart = -(cp_PosReport->f_ClockBias);
  if ( f_GnssTodFracPart < 0.0f )
  {
    w_GnssTodFrac250ns = (uint16)(1.0f + f_GnssTodFracPart) * 4000;
    q_GnssTodMsecModHour -= 1;
  ...
  p_GnssProvLocInfo->gnss_LocationInformation.measurementReferenceTime.m.gnss_TOD_fracPresent = 1;
  p_GnssProvLocInfo->gnss_LocationInformation.measurementReferenceTime.gnss_TOD_frac = w_GnssTodFrac250ns;

  p_GnssProvLocInfo->gnss_LocationInformation.measurementReferenceTime.gnss_TOD_msec = q_GnssTodMsecModHour;
  p_GnssProvLocInfo->gnss_LocationInformation.measurementReferenceTime.m.gnss_TOD_uncPresent = 1;
  p_GnssProvLocInfo->gnss_LocationInformation.measurementReferenceTime.gnss_TOD_unc =
    tm_lpp_utils_microsec_to_GADK( cp_PosReport->f_ClockBiasUncMs * 1000 );
}
...
void tm_lpp_set_end_trans_flag( tm_lpp_transaction_info_s_type *p_lpp_tx_type, LPP_Message *p_lpp_pdu )
{
  ...
  if ( (p_lpp_tx_type->z_lpp_data_type.q_flags & LPP_ENC_PROV_LOC_PERIODIC_RPT) == LPP_ENC_PROV_LOC_PERIODIC_RPT )
  {
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"FINAL multi-rpt", 0, 0, 0);
      p_lpp_tx_type->b_end_sent = TRUE;
      p_lpp_pdu->endTransaction = TRUE;
    }
    else
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"NOT FINAL multi-rpt", 0, 0, 0);
  ...
  else
  {
    ...
    if ( p_lpp_tx_type->z_lpp_data_type.z_rli.p_otoda_data != NULL )
    {
      sm_loc_lte_otdoa_meas_info_type *const pOtodaData = p_lpp_tx_type->z_lpp_data_type.z_rli.p_otoda_data;
    ...
    if ((p_lpp_tx_type->z_lpp_data_type.z_rli.s_lpp_ext & C_LPP_EXT_EARLY_RESP)==0) 
    {
...
)
{
  ...
  if ( p_lpp_tx_type == NULL )
  {
    MSG(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"TM_LPP: Null transaction pointer");
    return ( FALSE );
  }

  MSG_1(MSG_SSID_GPSSM, MSG_LEGACY_MED,"TM_LPP:Enc Prov Loc. Flag 0x%lX", p_lpp_tx_type->z_lpp_data_type.q_flags);

  /* This malloc function initializes p_lpp_pdu structure to zeros */
  p_lpp_pdu =
  ...
  if ( p_lpp_pdu == NULL )
  {
    ...
    MSG(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"TM_LPP:Enc Prov Loc buffer alloc failure");
    return ( FALSE );
  }
  p_lpp_tx_type->z_lpp_data_type.p_pdu_ptr = p_lpp_pdu;
  z_trans_id = p_lpp_tx_type->z_current_trans_id;
  ...
  if ( p_lpp_tx_type->u_lpp_session_type == TM_LPP_SESSION_TYPE_CP )
  {
    ...
    tm_lpp_cp_get_nxt_out_seq_no(p_lpp_tx_type);
    p_lpp_pdu->sequenceNumber = p_lpp_tx_type->u_out_sequence_num;
    /* For Early Response - Ack is not requested to avoid unnecessary race conditions. */
    if (0 == (p_lpp_tx_type->z_lpp_data_type.z_rli.s_lpp_ext & C_LPP_EXT_EARLY_RESP))
    {
      /* For Non-Early Response Messages always request Ack. */
      p_lpp_pdu->acknowledgement.ackRequested = 1;  /* Request Server ACK for LPP Prov Loc Info */
      p_lpp_pdu->m.acknowledgementPresent = 1;
    }
  }
  /* LPP CP Specific Changes : END */
  ...
  if ( (p_lpp_pdu->lpp_MessageBody.u.c1 = tm_lpp_asn1_osys_IE_malloc(sizeof(LPP_MessageBody_c1))) == NULL )
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"TM_LPP:Enc Prov Loc buffer alloc failure", 0, 0, 0);
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"TM_LPP flags %d", p_lpp_tx_type->z_lpp_data_type.q_flags, 0, 0);
  ...
  if ((p_lpp_tx_type->z_lpp_data_type.z_rli.s_lpp_ext & C_LPP_EXT_EARLY_RESP)!=0) 
  {
     ...
     if (((p_lpp_tx_type->z_lpp_data_type.q_flags & LPP_ENC_OTDOA_DATA)!=0)&&
         (NULL!=p_lpp_tx_type->z_lpp_data_type.z_rli.p_otoda_data))
     {
        if (p_lpp_tx_type->z_lpp_data_type.z_rli.p_otoda_data->u_SeqNum !=
            p_lpp_tx_type->z_lpp_data_type.z_rli.p_otoda_data->u_MaxMessageNum) 
        {
...
#ifdef FEATURE_GNSS_LPPE
  if(tm_lpp_e_encode_pli(p_lpp_tx_type))
  {
    if(tm_lpp_copy_lppe_epdu(
              p_lpp_tx_type, 
              &p_prov_loc_r9->epdu_ProvideLocationInformation) == TRUE)
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"EPDU copied successfully!!", 0, 0, 0);
      p_prov_loc_r9->m.epdu_ProvideLocationInformationPresent = 1;
    }
    else
    {
      p_prov_loc_r9->m.epdu_ProvideLocationInformationPresent = 0;
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Could not copy EPDU!!", 0, 0, 0);      
    }
  ...
  if ( (p_lpp_tx_type->z_lpp_data_type.q_flags & LPP_ERROR_DATA_VALID) &&
       (p_lpp_tx_type->z_lpp_data_type.z_lpp_err.w_ErrType & LPP_ERROR_TYPE_PROV_LOC_COMMON_ERROR) )
  {
    // send gnss-error here
    p_prov_loc_r9->m.commonIEsProvideLocationInformationPresent = 1;
    p_prov_loc_r9->commonIEsProvideLocationInformation.m.locationErrorPresent = 1;
    p_prov_loc_r9->commonIEsProvideLocationInformation.locationError.locationfailurecause =
      p_lpp_tx_type->z_lpp_data_type.z_lpp_err.u_CommonErrCode;
  }
  else
  {
    if ( p_lpp_tx_type->z_lpp_data_type.q_flags & LPP_ENC_POS_DATA )
    {
      ...
      if ( p_lpp_tx_type->z_lpp_data_type.q_flags & LPP_POS_DATA_VALID )
      {
        ...
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"ALT %d %d", p_lpp_tx_type->z_lpp_data_type.z_pos_info.w_Alt,
                p_prov_loc_r9->commonIEsProvideLocationInformation.locationEstimate.u.ellipsoidPointWithAltitudeAndUncertaintyEllipsoid->altitudeDirection, 0);
             ...
             !(p_lpp_tx_type->z_lpp_data_type.z_pos_info.q_pos_sources & ~C_POS_SOURCE_GLO) )  // No other pos sources should be used
        {
          ...
          if ( p_lpp_tx_type->z_lpp_data_type.z_rli.s_gnss_method & C_POS_SOURCE_GLO )
          {
          ...
          /* Though server requested for GPS position, our engine was able to generate GLO only
             position. So report as GLO position with GPS reference time */
          else if ( p_lpp_tx_type->z_lpp_data_type.z_rli.s_gnss_method & C_POS_SOURCE_GPS )
          {
            lpp_agnss_providelocationinfo_gps_reftime( &(p_prov_loc_r9->a_gnss_ProvideLocationInformation),
                                                       &(p_lpp_tx_type->z_lpp_data_type.z_pos_info) );
          }
          else
          {
            MSG_2( MSG_SSID_GPSSM, MSG_LEGACY_HIGH,
                   "Glonass meas ref time (%lu) & pos available but server not interested (0x%lX)",
                   p_lpp_tx_type->z_lpp_data_type.z_pos_info.q_GloTodMs,
                   p_lpp_tx_type->z_lpp_data_type.z_rli.s_gnss_method );
          }
        } /* Glonass only */
        else if ( p_lpp_tx_type->z_lpp_data_type.z_pos_info.q_pos_sources & C_POS_SOURCE_GPS )
        {
          ...
          if ( p_lpp_tx_type->z_lpp_data_type.z_rli.s_gnss_method & C_POS_SOURCE_GPS )
          {
            lpp_agnss_providelocationinfo_gps_reftime( &(p_prov_loc_r9->a_gnss_ProvideLocationInformation),
                                                &(p_lpp_tx_type->z_lpp_data_type.z_pos_info) );
          }
          ...
          {
            MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"GPS meas ref time (%lu) available but server not interested (0x%lX)",
                    p_lpp_tx_type->z_lpp_data_type.z_pos_info.q_fix_time_gps_msec,
                    p_lpp_tx_type->z_lpp_data_type.z_rli.s_gnss_method, 0);
          }
        } /* GPS */
        else
        {
      ...
      else
      {
                                                                                                      ...
                                                                                                      tm_lpp_asn1_osys_IE_malloc(sizeof(GNSS_TargetDeviceErrorCauses))) == NULL )
        {
          ...
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"TM_LPP:Enc Prov Loc buffer alloc failure for error causes", 0, 0, 0);
          tm_asn1_free_lpp_message_IEs(p_lpp_pdu, LPP_MESSAGE_PROV_LOC_INFO);
          return ( FALSE );
        }
        // send gnss-error here
        ...
        if ( p_lpp_tx_type->z_lpp_data_type.z_lpp_err.w_ErrType & LPP_ERROR_TYPE_PROV_LOC_AGNSS_ERROR )
        {
          p_prov_loc_r9->a_gnss_ProvideLocationInformation.gnss_Error.u.targetDeviceErrorCauses->cause =
            p_lpp_tx_type->z_lpp_data_type.z_lpp_err.u_GnssErrCode;
        }
        else
        {
    ...
    if ( p_lpp_tx_type->z_lpp_data_type.q_flags & LPP_ENC_OTDOA_DATA )
    {
      ...
      sm_loc_lte_otdoa_meas_info_type const *const p_otoda_data_tx      = p_lpp_tx_type->z_lpp_data_type.z_rli.p_otoda_data;
      ...
      if ( p_otoda_data_tx != NULL )
      {
        ...
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"TM_LPP:OTDOA Measurements will be sent u_cell_cnt = %d. LppErrType %u", u_cell_cnt,
                p_lpp_tx_type->z_lpp_data_type.z_lpp_err.w_ErrType, 0);

        /* force RSTD meas. cnt to 1 or less for a hybrid fix (only for debugging) */
        if ( (u_cell_cnt > 1) && (TRUE == b_force_lte_hybrid) )
        {
          u_cell_cnt = 1;
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"TM_LPP: Force OTDOA meas. to 1", 0, 0, 0);
        }

        /* Send error in these cases:-
        *  1. If RSTD measurment present bit set to 1, but no cell measurements are found
        *  2. OTDOA engine provided with cause for OTDOA error (direct cause)
        */
        /* Validity/non-NULL condition of p_otoda_data_tx->p_meas_info is implcit when u_cell_cnt is non-zero so
           there is a strict dependency on the former condtion being checked first before checking for the latter
        */
        if ((0 == u_cell_cnt) || (p_otoda_data_tx->p_meas_info->m.otdoa_ErrorPresent))
        {
      ...
      if ( TRUE == b_OtdoaErrDirOrInDir )
      {
        ...
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"TM_LPP:Fill OTDOA Prov loc err", 0, 0, 0);
        p_prov_loc_r9->m.otdoa_ProvideLocationInformationPresent = 1;
        ...
        */
        ...
        if ( (p_otoda_data_tx != NULL) && (p_otoda_data_tx->p_meas_info != NULL) &&
        ...
        else
        {
          ...
          if ( p_lpp_tx_type->z_lpp_data_type.z_lpp_err.w_ErrType & LPP_ERROR_TYPE_PROV_LOC_OTDOA_ERROR )
          {
            p_prov_loc_r9->otdoa_ProvideLocationInformation.otdoa_Error.u.targetDeviceErrorCauses->cause =
              p_lpp_tx_type->z_lpp_data_type.z_lpp_err.u_OtdoaErrCode;
        ...
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"TM_LPP:Sending OTDOA PRov loc err cause %d",
    ...
    if ( p_lpp_tx_type->z_lpp_data_type.q_flags & LPP_ENC_ECID_DATA )
    {
      if ( p_lpp_tx_type->z_lpp_data_type.q_flags & LPP_ECID_DATA_VALID )
      {
        ...
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"TM_LPP:ECID MEasurements build ret %d", b_ret, 0, 0);
        if ( b_ret == FALSE ||
             p_lpp_tx_type->z_lpp_data_type.z_lpp_err.w_ErrType & LPP_ERROR_TYPE_PROV_LOC_ECID_ERROR )
        {
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"TM_LPP:Sending ECID PRov loc err", 0, 0, 0);
          ...
          if ( p_lpp_tx_type->z_lpp_data_type.z_lpp_err.w_ErrType & LPP_ERROR_TYPE_PROV_LOC_ECID_ERROR )
          {
            p_prov_loc_r9->ecid_ProvideLocationInformation.ecid_Error.u.targetDeviceErrorCauses->cause =
              p_lpp_tx_type->z_lpp_data_type.z_lpp_err.u_EcidErrCode;
          ...
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"TM_LPP:Sending ECID PRov loc err cause %d",
  ...
  {
    /* ASN.1 encoding failed */
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"TM_LPP: Error encoding LPP ProvLocInfo", 0, 0, 0);
                            ...
                            )
{
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"TM_LPP:Enc ERROR", 0, 0, 0);
  if ( p_lpp_tx_type == NULL )
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"TM_LPP: Null transcation pointer", 0, 0, 0);
    return ( FALSE );
  ...
  if ( p_lpp_pdu == NULL )
  {
    ...
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"TM_LPP:Enc error buffer alloc failure", 0, 0, 0);
    return ( FALSE );
  }
  p_lpp_tx_type->z_lpp_data_type.p_pdu_ptr = p_lpp_pdu;
  ...
  if ( p_lpp_tx_type->z_lpp_data_type.q_flags & LPP_ERROR_DATA_VALID &&
       p_lpp_tx_type->z_lpp_data_type.z_lpp_err.w_ErrType & LPP_ERROR_TYPE_LPP_ERROR )
  {
    p_lpp_pdu->lpp_MessageBody.u.c1->u.error->u.error_r9->commonIEsError.errorCause =
      p_lpp_tx_type->z_lpp_data_type.z_lpp_err.u_ErrorErrCode;
  }
  else
  {
  ...
  {
    /* ASN.1 encoding failed */
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"TM_LPP: Error encoding LPP Error", 0, 0, 0);
                                ...
                                )
{
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"TM_LPP:Enc ABORT", 0, 0, 0);
  if ( p_lpp_tx_type == NULL )
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"TM_LPP: Null transcation pointer", 0, 0, 0);
    return ( FALSE );
  ...
  if ( p_lpp_pdu == NULL )
  {
    ...
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"TM_LPP:Enc Abort buffer alloc failure", 0, 0, 0);
    return ( FALSE );
  }
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"TM_LPP:Enc Abort alloc failure", 0, 0, 0);
    tm_asn1_free_lpp_message_IEs(p_lpp_pdu, LPP_MESSAGE_ABORT);
    return ( FALSE );
  }
  ...
  if ( p_lpp_tx_type->z_lpp_data_type.q_flags & LPP_ERROR_DATA_VALID &&
       p_lpp_tx_type->z_lpp_data_type.z_lpp_err.w_ErrType & LPP_ERROR_TYPE_LPP_ABORT )
  {

    p_lpp_pdu->lpp_MessageBody.u.c1->u.abort_->criticalExtensions.u.c1->u.abort_r9->commonIEsAbort.abortCause =
      p_lpp_tx_type->z_lpp_data_type.z_lpp_err.u_AbortErrCode;
  }
  else
  {
  ...
  {
    /* ASN.1 encoding failed */
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"TM_LPP: Error encoding LPP Abort", 0, 0, 0);
...
boolean tm_lpp_encode_ack( uint8 u_seq_number, tm_pdu_buf_type  *p_pdu_buf, boolean end_flag )
{
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"<<<< [UE]: UE ACK [SEQ=%d]", u_seq_number, 0, 0);
  ...
  */
  ...
  if ( p_pdu_buf == NULL )
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"TM_LPP: null encode buffer found", 0, 0, 0);
    return ( FALSE );
  }
  else
  {
  ...
  {
    /* ASN.1 encoding failed */
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"TM_LPP: Error encoding LPP ACK", 0, 0, 0);
                             ...
                             )
{
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"TM_LPP:Decode Message ", 0, 0, 0);
  p_lpp_pdu =
              tm_lpp_asn1_osys_IE_malloc(sizeof(LPP_Message));

  if ( p_lpp_pdu == NULL )
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"TM_LPP: Cannot allocate OSYS memory for the LPP data", 0, 0, 0);
    return ( NULL );
                               ...
                               )
{
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"TM_LPP:Encode Prov Cap Message ", 0, 0, 0);
                              ...
                              tm_lpp_transaction_info_s_type *p_lpp_tx_type )
{
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"TM_LPP:Encode Req AD Message ", 0, 0, 0);
  if ( p_client->client_cb_func )
  {
                                     ...
                                     tm_lpp_transaction_info_s_type *p_lpp_tx_type )
{
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"TM_LPP:Encode Prov LocInfo Message ", 0, 0, 0);
  if ( p_lpp_tx_type->b_new_tx == TRUE || ((p_lpp_tx_type->z_lpp_data_type.z_rli.s_lpp_ext & C_LPP_EXT_EARLY_RESP)!=0))
  {
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"<<<< [UE]: Prov Loc [UE SEQ=%d] [WaitAck=%d] [Qflag=0x%x]",
                           ...
                           tm_lpp_transaction_info_s_type *p_lpp_tx_type )
{
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"TM_LPP:Encode LPP ERROR ", 0, 0, 0);
  if ( p_client->client_cb_func )
  {
                             ...
                             tm_lpp_transaction_info_s_type *p_lpp_tx_type )
{
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"TM_LPP:Encode LPP ABORT ", 0, 0, 0);
  if ( p_client->client_cb_func )
  {
                               ...
                               tm_lpp_transaction_info_s_type *p_lpp_tx_type )
{
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,">>>> TM_LPP:Handle Prov Ad Message ", 0, 0, 0);
                                    ...
                                    tm_lpp_transaction_info_s_type *p_lpp_tx_type )
{
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,">>>> TM_LPP:Handle Req LocInfo Message ", 0, 0, 0);
  if ( p_lpp_tx_type->b_new_tx == TRUE )
  {
                               ...
                               tm_lpp_transaction_info_s_type *p_lpp_tx_type )
{
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,">>>> TM_LPP:Handle Req Cap Message ", 0, 0, 0);
  if ( p_lpp_tx_type->b_new_tx == TRUE )
  {
...

{
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,">>>> TM_LPP:Handle Error Message ", 0, 0, 0);
  if ( p_lpp_tx_type->b_new_tx == TRUE )
  {
...
boolean tm_lpp_handle_abort( const tm_lpp_tx_mgr_client_type *p_client,
                             tm_lpp_transaction_info_s_type *p_lpp_tx_type )
{
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,">>>> TM_LPP:Handle Abort Message ", 0, 0, 0);
  if ( p_lpp_tx_type->b_new_tx == TRUE )
  {
 ...
 *  
 * Parameters:
 *   p_NavModelEph : pointer to the Glonass nav. model.
 *   p_TmPostDataGloEph : pointer to post data glo ephe
 *
 * Return value: 
 *   None.
 *
...
{
  if ((NULL == p_NavModelEph) || (NULL == p_TmPostDataGloEph))
  {
    MSG_2(MSG_SSID_GPSSM, MSG_LEGACY_ERROR, "Null Pointer access in tm_lpp_glo_eph_to_tmcore_post_data_payload(), "
      "p_NavModelEph ? Null: %d, p_TmPostDataGloEph?Null: %d", (NULL == p_NavModelEph), (NULL == p_TmPostDataGloEph));
    return;
  }
  ...
  {
    p_TmPostDataGloEph->z_Mask.z_FilledMask.u_GloTypeFlag = 0;
    ...
    if ( T_GNSS_OrbitModel_glonass_ECEF == p_NavModelEph->gnss_OrbitModel.t )
    {
      ...
      /* M: Integer (0..3) */
      p_TmPostDataGloEph->u_GloType = p_NavModelEph->gnss_OrbitModel.u.glonass_ECEF->gloM;
      p_TmPostDataGloEph->z_Mask.z_FilledMask.u_GloTypeFlag = GNSS_NEW_GLO_TYPE_AVAILABLE;  
                                                  ...
                                                  )
{
  ...
  if  ( p_ref_loc == NULL )
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"TM_LPP: Err: ref pos. source info. invalid", 0, 0, 0);
    return ( FALSE );
  }

  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"TM_LPP:RefPos to tmcore post", 0, 0, 0);
                                              ...
                                              tm_post_data_payload_type   *p_tm_post_data_payload )
{
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"TM_LPP:Iono to tmcore post", 0, 0, 0);
                                            ...
                                            )
{
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"TM_LPP:Eph to tmcore post", 0, 0, 0);
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"TM_LPP: No orbit model\n", 0, 0, 0);
  }
  else
  {
                                            ...
                                            )
{
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"TM_LPP:Alm to tmcore post", 0, 0, 0);
                                                  ...
                                                  tm_post_data_payload_type   *p_tm_post_data_payload )
{
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"TM_LPP:UTCModel to tmcore post", 0, 0, 0);
...
#define LPP_AZ_RAD (DEG_TO_RADIANS * (0.703125f)) // 0.0122718463030851562500000000000
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"TM_LPP: GPS AcqAssist (%lu) to tmcore post", p_gnss_acq_assist->n, 0, 0);
                                                          ...
                                                          )
{
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"TM_LPP:AcqAssistSteer sanity failed (%p, %p)", p_gnss_acq_assist, p_tm_post_data_payload, 0);
    return;
  }
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"TM_LPP:AcqAssistSteer (%lu) to tmcore post", p_acq_assist->n, 0, 0);
...
#define LPP_AZ_RAD (DEG_TO_RADIANS * (0.703125f))
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"TM_LPP_UP: Invalid glonass steering in/out struct.", 0, 0, 0);
    return;
  }
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"TM_LPP_UP: Build %lu Glonass AA SVDirection & Steering info", p_gnss_acq_assist_list->n, 0, 0);
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"TM_LPP_UP: Can't handle AA data of %u elements. Max %d can be consumed.", p_gnss_acq_assist_list->n, N_GLO_FREQ, 0);
  ...
  for ( u_i = 0; u_i < u_num_acq_assist; ++u_i, ++p_gnss_acq_assist_elem )
  {
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"TM_LPP: Glonass AA SvId %u invalid", p_gnss_acq_assist_elem->svID.satellite_id, 0, 0);
    }
  } /* SV for-loop */
                                                    ...
                                                    )
{
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"TM_LPP:OTDOA Assist to tmcore post", 0, 0, 0);
      ...
      if (  p_otdoa_assist->otdoa_ReferenceCellInfo.prsInfo.m.prs_MutingInfo_r9Present == 1 )
      {
        ...
        switch ( p_otdoa_assist->otdoa_ReferenceCellInfo.prsInfo.prs_MutingInfo_r9.t  )
        {
           case LPP_PRS_MUTINGINFO_POS2:
             ...
             MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"TM_LPP:PRS_VALUE %d, muting pattern %d", prs_value, p_otdoa_assist->otdoa_ReferenceCellInfo.prsInfo.prs_MutingInfo_r9.t, 0);
             break;
              ...
              if ( p_otdoa_assist->otdoa_NeighbourCellInfo.elem[i].elem[j].prsInfo.m.prs_MutingInfo_r9Present == 1 )
              {
                ...
                switch ( p_otdoa_assist->otdoa_NeighbourCellInfo.elem[i].elem[j].prsInfo.prs_MutingInfo_r9.t )
                {
                   case LPP_PRS_MUTINGINFO_POS2:
                     ...
                     MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"TM_LPP:PRS_VALUE %d, muting pattern %d", prs_value, p_otdoa_assist->otdoa_NeighbourCellInfo.elem[i].elem[j].prsInfo.prs_MutingInfo_r9.t, 0);
                     break;
    ...
    for ( j = 0; j < num_elem; ++j ) // iterate each nav satellite
    {
      ...
      if ( p_NavModelEph->svID.satellite_id < N_GLO_SV ) // 0..23
      {
        ...
        if ( z_prot_glo_eph_buf[p_NavModelEph->svID.satellite_id].u_freq_num_valid )
        {
          ...
          {
            MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"TM_LPP(GLO) to Tmcore data post failed", 0, 0, 0);
          }
          else
          {
            q_eph_svs_injected |= 1L <<  p_tm_post_data_payload->eph_glo_data.u_SvId;
            MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"TM_LPP(GLO): SV %u eph. posted to TmCore", p_tm_post_data_payload->eph_glo_data.u_SvId, 0, 0);
          }
  ...
  if ( p_elem->gnss_Almanac.gnss_AlmanacList.n > 0 )
  {
    for ( i = 0; i < p_elem->gnss_Almanac.gnss_AlmanacList.n; i++ )
    {
      ...
      if ( T_GNSS_AlmanacElement_keplerianGLONASS == p_Almanac->t )
      {
        ...
        {
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"TM_LPP(GLO) Alm. to Tmcore data post failed", 0, 0, 0);
        }
        else
        {
          q_alm_svs_injected |= 1L << (p_Almanac->u.keplerianGLONASS->gloAlmnA - 1); // gloAlmnA is (1..24) - SV slot id
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"TM_LPP_UP: SUPL(GLO): Slot Id %u Alm. posted to TmCore", p_tm_post_data_payload->alm_glo_data.u_SvId, 0, 0);
                          ...
                          tm_post_data_payload_type *p_tm_post_data_payload )
{
  ...
  if ( p_elem->gnss_RealTimeIntegrity.gnss_BadSignalList.n > 0 )
  {
    ...
    if ( q_RealTimeIntegrity0 )
    {
      ...
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"TM_LPP(Glo): RTI to Tmcore data post failed", 0, 0, 0);
      }
      else
      {
                          ...
                          tm_post_data_payload_type *p_tm_post_data_payload )
{
  if ( p_elem->gnss_UTC_Model.t == T_GNSS_UTC_Model_utcModel3 ) /* Glonass UTC model */
  {
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"TM_LPP(Glo) UTC to Tmcore data post failed", 0, 0, 0);
    }
    else
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"TM_LPP(Glo): UTC posted to TmCore", 0, 0, 0);
    }
  } /* if utc model 3*/
  else
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Glo Time Model = %d not utcModel3", p_elem->gnss_UTC_Model.t, 0, 0);
  }
}
                                    ...
                                    tm_post_data_payload_type *p_tm_post_data_payload )
{
  ...
  for ( u_i = 0; u_i < p_elem->gnss_TimeModels.n; ++u_i )
  {
    if ( LPP_GNSS_TO_ID_GPS == p_elem->gnss_TimeModels.elem[u_i].gnss_TO_ID )
    {
      ...
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"TM_LPP_UP: SUPL(Glo) TimeModel to Tmcore data post failed", 0, 0, 0);
      }
      else
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"TM_LPP_UP: SUPL(Glo): TimeModel posted to TmCore", 0, 0, 0);
      }
    }
  }
                         ...
                         tm_post_data_payload_type *p_tm_post_data_payload )
{
  ...
  for ( i = 0; i < N_GLO_SV; ++i )
  {
        ...
        )
    {
      ...
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"TM_LPP_CP: CP(Glo): Eph. SV %u inj. to TmCore failed",
 ...
 *  e_as_id    : Sub ID on which the stop request is intended
 * Return value:
 *  None.
 *
 ******************************************************************************
*/
void tm_lpp_proc_otdoa_stop_req( tm_sess_handle_type sess_handle,
                                            tm_sess_stop_reason_e_type const reason,
                                            ...
                                            sys_modem_as_id_e_type e_as_id)
{
  ...
  z_tm_post_data_payload.otdoa_stop.e_as_id= e_as_id;
