...
/*===========================================================================
  ...
  $Header: //components/rel/gnss8.mpss/11.1/gnss/sm/tm/pdapi/src/pdapiclient.c#1 $ $DateTime: 2019/10/24 01:30:36 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
04/02/19   mj      Location Privacy Feature
...
void pdsm_set_ready_flag(void)
{
  ...
  if (errCode > 0)
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"pdsm_set_ready_flag failed with error code = %d", errCode, 0, 0);
  }
  ...
  tm_cm_phone_state_info_s_type phone_state_info = {0};
   ...
   if ( lcs_info_ptr->lcs_cb_cmd_type == PDSM_LCS_CMD_TYPE_SUPL_REQ )
   {
     ...
     if (((TM_CM_E911_STATE_NONE == phone_state_info.e911_state)&&
          (PDSM_LCS_PRIVACY_OVERRIDE == lcs_info_ptr->lcs_info.supl_req.notification_priv_type)) ||
         (q_pdapi_gnss_oem_feature_mask & PDSM_SM_NV_GNSS_OEM_FEATURE_MASK_4_SUPL_NI_NOTIF_LOOPBK)
        )
#else
