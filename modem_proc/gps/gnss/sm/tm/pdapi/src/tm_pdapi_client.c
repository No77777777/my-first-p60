...
/*===========================================================================
  ...
  $Header: //components/rel/gnss8.mpss/11.1/gnss/sm/tm/pdapi/src/tm_pdapi_client.c#4 $
  $DateTime: 2020/02/05 11:49:36 $ $Author: pwbldsvc $
...
02/23/16   mj      Support for GNSS Monitoring
...
void tm_pdapi_cmd_proc( void* p_data )
{
  ...
  switch( p_cmd_type->cmd_type )
  {
    ...
    case PDSM_CMD_TYPE_PD:
      ...
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"TM_PDAPI: Reset Location Service in progress",0,0,0);
        tm_pd_client_cmd_status( p_cmd_type, PDSM_PD_CMD_ERR_RESET_LOCATION_SERVICE_IN_PROGRESS_S );
        return;
      }

      if ( PDSM_PD_CMD_GET_POS == p_cmd_type->cmd.pd.cmd ) 
      {
      ...
      break;  /* switch case PDSM_CMD_TYPE_PD */
  ...
  case PDSM_CMD_TYPE_DPO_STATUS_RPT:
    {
      ...
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"PDSM_CMD_TYPE_DPO_STATUS_RPT: Enable: %d, ret_val: %d",
    ...
    case PDSM_CMD_TYPE_SRN_INFO:
#ifdef FEATURE_GNSS_LPPE
      /* Handle SRN/BTLE scan results */
      tm_srn_scan_cmd_proc(p_cmd_type);
#else
      MSG(MSG_SSID_GPSSM, MSG_LEGACY_ERROR," TM_PDAPI: LPPE disabled.");
  ...
  switch( p_pd_cmd_type->cmd )
  {
    case PDSM_PD_CMD_GET_POS:
    {
      ...
      */
      ...
      if ((p_pd_cmd_type->info.option_type.umts_slp_lsinfo.server_option < PDSM_SERVER_OPTION_USE_DEFAULT) ||
        (p_pd_cmd_type->info.option_type.umts_slp_lsinfo.server_option > PDSM_SERVER_OPTION_USE_LOCAL))
      {
        e_cmd_err = PDSM_PD_CMD_ERR_LSINFO_P;
      }
      if (p_pd_cmd_type->info.option_type.umts_slp_lsinfo.server_option  == PDSM_SERVER_OPTION_USE_LOCAL)
      {
        /* Check if IP Addr or URL are valid */
        if((p_pd_cmd_type->info.option_type.operation_mode == PDSM_SESSION_OPERATION_MSASSISTED) &&
           (((p_pd_cmd_type->info.option_type.umts_slp_lsinfo.server_adrs.server_addr_type == PDSM_SERVER_TYPE_IPV4_ADDRESS) &&
           ((p_pd_cmd_type->info.option_type.umts_slp_lsinfo.server_adrs.server_adrs.server_adrs_v4.server_adrs == 0) ||
           (p_pd_cmd_type->info.option_type.umts_slp_lsinfo.server_adrs.server_adrs.server_adrs_v4.port_id == 0))) ||
           ((p_pd_cmd_type->info.option_type.umts_slp_lsinfo.server_adrs.server_addr_type == PDSM_SERVER_TYPE_URL_ADDRESS) &&
           (p_pd_cmd_type->info.option_type.umts_slp_lsinfo.server_adrs.server_adrs.server_adrs_url.url[0] == '\0'))))
        {
          e_cmd_err = PDSM_PD_CMD_ERR_LSINFO_P;
        }
      }      
  ...
  )
{
  ...
  */
  ...
  if ( ( pdsmclient_valid_client_id( p_cmd_type->cmd.pd.client_id )) == FALSE )
  {
    ...
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"TM_PDAPI: PDSM_PD_CMD_ERR_CLIENT_ID_ID_P",0,0,0);
    tm_pd_client_cmd_status( p_cmd_type, e_cmd_err );
    return;
  }
  ...
  */
  ...
  case PDSM_PD_CMD_END_SESSION_EX:  
    ...
    z_end_sess_type.receiver_off =  p_cmd_type->cmd.pd.info.session_end_info.receiver_off;
	  z_end_sess_type.v_reason_service_interaction =  p_cmd_type->cmd.pd.info.session_end_info.v_reason_service_interaction;
    ...
    case PDSM_PD_CMD_GET_POS:
      ...
      */
      if (e_cmd_err == PDSM_PD_CMD_ERR_NOERR)
      {
        ...
        #ifdef FEATURE_GNSS_DTF
        if (TRUE == tm_dtf_events_db.pos_ongoing.status)
        {
          tm_qsh_report_event(SYS_MODEM_AS_ID_1,(tm_qsh_event_e) TM_DTF_EVENT_GET_POS_ONGOING);
        }
        #endif
    ...
    case PDSM_PD_CMD_RESET_LOCATION_SERVICE:
      ...
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Reset Location Service: Client type not OEM",0,0,0);
        e_cmd_err = PDSM_PD_CMD_ERR_CLIENT_ID_P;
      }
      else if(phone_state_info.e911_state != TM_CM_E911_STATE_NONE)
      {        
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Reset Location Service: Emergency Call is progress",0,0,0);
  ...
  case PDSM_PD_CMD_GET_AVAIL_POS_FOR_SRC:
    {
      ...
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"PDAPI cmd to get avail position estimate", 0, 0, 0);
      ...
      if (PDSM_PD_CMD_ERR_NOERR == e_cmd_err)
      {
        ...
        {
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"PDAPIcmd DataSource %d not supported. Defaulting to GNSS", q_get_available_pos_src, 0, 0);
          z_get_available_pos_param.source = SM_GPS_STATE_INFO_REQ_TYPE_GET_GNSS_AVAIL_POS_QMI;
        }
...
static pdsm_pa_cmd_err_e_type tm_pa_cmd_get_param_para_check( pdsm_pa_cmd_s_type   *p_PaCmdType )
{
  ...
  */
  ...
  if ( ( pdsmclient_valid_client_id( p_PaCmdType->client_id )) == FALSE )
  {
    ...
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"TM_PDAPI: PDSM_PA_CMD_ERR_CLIENT_ID_ID_P",0,0,0);
  }
  ...
  */
  ...
  switch( p_pa_cmd_type->info.set_type)
  {
    ...
    case PDSM_PA_EFS_DATA:
      ...
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"TM_PDAPI: PDSM_PA_EFS_DATA: Null file name", 0,0,0);
        e_cmd_err = PDSM_PA_CMD_ERR_PARAM_TYPE_P;
      }
      ...
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"TM_PDAPI: PDSM_PA_EFS_DATA: NULL data ptr for write cmd", 0,0,0);
        e_cmd_err = PDSM_PA_CMD_ERR_PARAM_TYPE_P;
      }
      ...
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"TM_PDAPI: PDSM_PA_EFS_DATA: Invalid directory file name,  file_operation %d, file len %d",
          p_pa_cmd_type->info.pa_set.efs_data.efs_data_file_operation,
    ...
    case PDSM_PA_ON_DEMAND_READY_MODE:
      e_cmd_err = PDSM_PA_CMD_ERR_OTHER; /* OnDemand feature deprecated */
      break;
    ...
    case PDSM_PA_EXTERNAL_SPI:
     ...
     case PDSM_PA_XTRAT_CLIENT_TOKEN:
      ...
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"TM_PDAPI: PDSM_PA_XTRAT_CLIENT_TOKEN: parameter out of range (0-0xFFFFFFFF)", 0,0,0);
        e_cmd_err = PDSM_PA_CMD_ERR_PARAM_TYPE_P;
      }
      break;
  ...
  case PDSM_PA_EMERGENCY_LBS_APN_PROFILE: 
        {
          ...
          for(;profile_index < PDSM_PA_MAX_APN_PROFILES; profile_index++)
          {
            if(p_pa_cmd_type->info.pa_set.apn_profiles[profile_index].pdp_type > PDSM_PA_APN_PROFILE_PDN_TYPE_MAX)
            {
              ...
              MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Invalid PDN type",0,0,0);
              break;
            }
            if(p_pa_cmd_type->info.pa_set.apn_profiles[profile_index].srv_system_type > PDSM_PA_APN_PROFILE_SRV_SYS_MAX)
            {
              ...
              MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Invalid SRV SYS type",0,0,0);
              break;
            }
          }
        }
    ...
    case PDSM_PA_PREMIUM_SERVICES_CONFIG:
      ...
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Invalid Premium Service type=%d", p_pa_cmd_type->info.pa_set.premium_svc_cfg.service, 0, 0);
        e_cmd_err = PDSM_PA_CMD_ERR_PARAM_TYPE_P;
      }
      break;
    ...
    case PDSM_PA_UAX3_CONFIG:
      if (p_pa_cmd_type->info.pa_set.uax3_config_info > PDSM_PA_UAX3_MASK_ALL)
      {
        MSG_1(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Invalid PDSM_PA_UAX3_CONFIG value=%d", p_pa_cmd_type->info.pa_set.uax3_config_info);
        e_cmd_err = PDSM_PA_CMD_ERR_PARAM_TYPE_P;
      }
      break;
...
)
{
  ...
  /* Call the user supplied command callback.
  */
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_LOW, "TM_PDAPI: tm_lcs_cmd_status () getting called",0,0,0);
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR, " LCS cmd err, cmd = %d, err = %d, client = %d",
...
)
{
  ...
  */
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR, " EXT POS cmd err, cmd = %d, err = %d, client = %d",
...
static pdsm_pa_event_type tm_get_event_mask_for_pa_type(pdsm_pa_e_type e_pa_set_type) {
  pdsm_pa_event_type      e_pa_event = PDSM_PA_EVENT_NONE;
  switch (e_pa_set_type)
  {
    case PDSM_PA_DELETE_PARAMS_EX:
    case  PDSM_PA_DELETE_PARAMS:
    case  PDSM_PA_DELETE_PARAMS_EX1:
    {
      e_pa_event = PDSM_PA_EVENT_DELETE_PARAMS;
      break;
    }
    ...
    case PDSM_PA_SUPL_UDP_ENABLE_CONFIG:
    {
      e_pa_event = (pdsm_pa_event_type)PDSM_PA_SUPL_UDP_ENABLE_CONFIG;
      break;
    }
    ...
    case PDSM_PA_EMERGENCY_SUPPORT_CONFIG:
    {
      e_pa_event = (pdsm_pa_event_type)PDSM_PA_EMERGENCY_SUPPORT_CONFIG;
      break;
    }
    case PDSM_PA_LPPE_UP_CONFIG:
    {
      e_pa_event = (pdsm_pa_event_type)PDSM_PA_LPPE_UP_CONFIG;
      break;
    }
    case PDSM_PA_LPPE_CP_CONFIG:
    {
      e_pa_event = (pdsm_pa_event_type)PDSM_PA_LPPE_CP_CONFIG;
      break;
    }
    case PDSM_PA_EMERGENCY_EXTENSION_WINDOW:
    {
      e_pa_event = (pdsm_pa_event_type)PDSM_PA_EMERGENCY_EXTENSION_WINDOW;
      break;
    }
    default:
    {
      TM_ERROR_1("Unknown PA Enum Type %d", e_pa_set_type);
      break;
    }
  }
  return e_pa_event;
...
)
{
  ...
  boolean                 u_ret = FALSE;
  tm_pdapi_param_s_type   *p_pa_param_type = NULL;
  ...
  */
  ...
  else if(tm_core_is_reset_location_service_in_progress())
  {
    ...
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"TM_PDAPI: PDSM_PA_CMD_ERR_RESET_LOCATION_SERVICE_IN_PROGRESS_S",0,0,0);
    tm_pa_client_cmd_status( p_cmd_type, e_cmd_err );
    return;
  }
  ...
  if( (p_pa_cmd_type->cmd == PDSM_PA_CMD_SET_PARAM) || 
      (p_pa_cmd_type->cmd == PDSM_PA_CMD_GET_PARAM) )
  {
    p_pa_param_type = (tm_pdapi_param_s_type *) os_MemAlloc(
                          sizeof(*p_pa_param_type), 
                          OS_MEM_SCOPE_TASK);
      
    if(NULL == p_pa_param_type)
    {
      MSG(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"TM_PDAPI: Out of Memory!!");
      tm_pa_client_cmd_status( p_cmd_type, PDSM_PA_CMD_ERR_NO_BUF_L );
      return;
    }
  }
  ...
  if(p_pa_cmd_type->cmd == PDSM_PA_CMD_SET_PARAM)
  {
    /* pa_info_type_ptr needs to be allocated only for SET_PARAM, not for GET */
    p_pa_param_type->pa_info_type_ptr = 
                     (pdsm_pa_info_type *) os_MemAlloc(
                         sizeof(*(p_pa_param_type->pa_info_type_ptr)), 
                         OS_MEM_SCOPE_TASK);
    if(NULL == p_pa_param_type->pa_info_type_ptr)
    {
      MSG(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"TM_PDAPI: Out of Memory!!");
      os_MemFree((void**) &p_pa_param_type);
      tm_pa_client_cmd_status( p_cmd_type, PDSM_PA_CMD_ERR_NO_BUF_L );
      return;
    }
  }
  ...
  switch ( p_pa_cmd_type->cmd )
  {
    case PDSM_PA_CMD_SET_PARAM:
      ...
      */
      ...
      u_ret = tm_core_param_check(p_pa_param_type, PDSM_PA_CMD_SET_PARAM);
      if( u_ret )
      {
      ...
      */
      ...
      if (e_cmd_err == PDSM_PA_CMD_ERR_NOERR)
      {
        *p_pa_param_type->pa_info_type_ptr = p_cmd_type->cmd.pa.info.pa_set;
        ...
        if(tm_core_param_always_allowed(p_pa_param_type->e_pa_type, PDSM_PA_CMD_SET_PARAM, p_pa_param_type))
      {
          /* Do not queue up these set parameter commands, always apply immediately */
          if(tm_core_handle_set_param(p_pa_param_type) == FALSE)
        {
            MSG_1(MSG_SSID_GPSSM, MSG_LEGACY_ERROR, 
              "Error handling set_param cmd %d", p_pa_param_type->e_pa_type);
        }
          os_MemFree((void**) &p_pa_param_type->pa_info_type_ptr);
          os_MemFree((void**) &p_pa_param_type);
	  
      }
        else
        {
          /* call the tmcore function to queue the set parameters request */
          e_cmd_err = tm_core_queue_set_param(p_pa_param_type );
        }

        /* Send the PA event call back to the client indicating error or success */
        p_pa_info_type->client_id = p_cmd_type->cmd.pa.client_id;
        p_pa_info_type->pa_event  = p_cmd_type->cmd.pa.info.set_type;
        p_pa_info_type->pa_info   = p_cmd_type->cmd.pa.info.pa_set;
        if(p_cmd_type->cmd.pa.info.set_type == PDSM_PA_SSD_DATA)
      {
          /* Indicate to client only the size of the SSD that is written to NV and
          ** not the actual SSD data (ssd_data should be NULL) */
          memset((void *)p_pa_info_type->pa_info.ssd_info.ssd_data,
                 0,
                 sizeof(p_pa_info_type->pa_info.ssd_info.ssd_data));
        }
        tm_pdapi_pa_event_callback_ex( p_pa_info_type, e_cmd_err );
        }

      if (e_cmd_err != PDSM_PA_CMD_ERR_NOERR)
        {
        /* If there is any cmd error, free the memory we allocated. If no error
         * we will free when we are finished processing the command */
        os_MemFree((void**) &p_pa_param_type->pa_info_type_ptr);
        os_MemFree((void**) &p_pa_param_type);
        }
    ...
    case PDSM_PA_CMD_GET_PARAM:
      ...
      u_ret = tm_core_param_check(p_pa_param_type, PDSM_PA_CMD_GET_PARAM);
      if (u_ret)
      {
        e_cmd_err = tm_pa_client_cmd_check( p_cmd_type );
      }
      ...
      */
      ...
      /* call the tmcore function to get the parameters */
      u_ret = tm_core_handle_get_param(p_pa_param_type );
      memscpy(&p_pa_info_type->pa_info,
             sizeof(p_pa_info_type->pa_info),
             p_pa_param_type->pa_info_type_ptr,
             sizeof(p_pa_info_type->pa_info));  

      p_pa_info_type->pa_event =  p_cmd_info_type->set_type;

      /* Check if get parameters response is available */
      if (u_ret)
      {
        e_pa_event = tm_get_event_mask_for_pa_type(p_cmd_info_type->set_type);
  ...
  )
{
  ...
  else if(tm_core_is_reset_location_service_in_progress())
  {
    ...
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"TM_PDAPI: PDSM_LCS_CMD_ERR_RESET_LOCATION_SERVICE_IN_PROGRESS_S",0,0,0);
    tm_lcs_client_cmd_status( p_cmd_type, e_cmd_err );
    return;
  }
  

  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_LOW, "TM_PDAPI: tm_lcs_cmd_proc () getting called",0,0,0);
  /* Here commands are handled which are passed from a FIFO queue.
  ** Don't worry about priority and QOS for Alpha release.
  */
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"TM_PDAPI: tm_lcs_cmd_proc () cmd type is %d, return value is %", p_cmd_type->cmd.lcs.cmd, u_ret,0);
...
)
{
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_LOW, "TM_PDAPI: tm_motion_client_cmd_proc() getting called",0,0,0);
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"TM_PDAPI: Invalid Motion Data Params", 0,0,0);
...
)
{
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_LOW, "TM_PDAPI: tm_ni_msg_client_cmd_proc() getting called",0,0,0);
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"TM_PDAPI: Invalid NI Message Params (type=%d len=%d)", 
...
)
{
  ...
  {
    if (tm_core_handle_inject_wifi_scans(&p_cmd_type->cmd.wifi_on_demand_info_list_cmd.wifi_on_demand_info_list) == FALSE) // fail to inject
    {
      cmd_status = PDSM_PD_CMD_ERR_INVALID_PARAM;  
    }
  }
  ...
  if(p_cmd_type != NULL)
  {    
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Freeing mem. at %lx",p_cmd_type->cmd.wifi_on_demand_info_list_cmd.wifi_on_demand_info_list.z_wifi_ap_scan_info.first_wifi_ap_ptr, 0, 0);
    os_MemFree((void **)&(p_cmd_type->cmd.wifi_on_demand_info_list_cmd.wifi_on_demand_info_list.z_wifi_ap_scan_info.first_wifi_ap_ptr)); 
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Freeing mem. at %lx",p_cmd_type->cmd.wifi_on_demand_info_list_cmd.wifi_on_demand_info_list.z_wifi_ap_scan_info.first_wifi_ap_add_info_ptr, 0, 0);
    os_MemFree((void **)&(p_cmd_type->cmd.wifi_on_demand_info_list_cmd.wifi_on_demand_info_list.z_wifi_ap_scan_info.first_wifi_ap_add_info_ptr));   
  }
  
  /* issue callback with appropriate status. Note that for success case ack is already sent, so dont repeat */
  if(cmd_status != PDSM_PD_CMD_ERR_NOERR)
  {
    /* Client ID will be filled later at LocMiddleware with loc_middleware_data.z_locPd_Data.wifi_ap_scan_client_handle */
    ...
    if (TRUE == p_cmd_type->cmd.wifi_on_demand_info_list_cmd.wifi_on_demand_info_list.v_on_demand_scan)
    {
    ...
    tm_ack_inject_wifi_scans(eLOC_ENG_GENERAL_FAILURE_V01, l_clientId);
  }
  return;
...
FUNCTION tm_srn_scan_cmd_proc

DESCRIPTION
  This function is used handle Srn scan result client commands.
...
)
{
  ...
  else if( p_cmd_type->cmd.srn_scan_info_list_cmd.srn_scan_info_list.z_srn_ap_scan_info.q_num_srn_aps == 0 ||
           p_cmd_type->cmd.srn_scan_info_list_cmd.srn_scan_info_list.z_srn_ap_scan_info.q_num_srn_aps > LOC_ENG_SRN_MAX_REPORTED_APS_PER_MSG_V01 ||
           p_cmd_type->cmd.srn_scan_info_list_cmd.srn_scan_info_list.z_srn_ap_scan_info.srn_ap_ptr == NULL)
  {
    MSG_2(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"TM_PDAPI: Invalid SRN scan info injection (numSrnAps=%d srn_ap_ptr=%lx )", 
              p_cmd_type->cmd.srn_scan_info_list_cmd.srn_scan_info_list.z_srn_ap_scan_info.q_num_srn_aps,
              p_cmd_type->cmd.srn_scan_info_list_cmd.srn_scan_info_list.z_srn_ap_scan_info.srn_ap_ptr);
    cmd_status = PDSM_PD_CMD_ERR_INVALID_PARAM;
  }
  else
  {
    if (tm_core_handle_inject_srn_scan_msg(&p_cmd_type->cmd.srn_scan_info_list_cmd.srn_scan_info_list) == FALSE) // fail to inject
    {
      cmd_status = PDSM_PD_CMD_ERR_INVALID_PARAM;  
    }
  }
  ...
  if(p_cmd_type != NULL)
  {    
    MSG_1(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Freeing mem. at %lx",
        p_cmd_type->cmd.srn_scan_info_list_cmd.srn_scan_info_list.z_srn_ap_scan_info.srn_ap_ptr);
    os_MemFree((void **)&(p_cmd_type->cmd.srn_scan_info_list_cmd.srn_scan_info_list.z_srn_ap_scan_info.srn_ap_ptr));  
  }

  /* issue callback with appropriate status */
  if(p_cmd_type!= NULL && p_cmd_type->cmd.srn_scan_info_list_cmd.cmd_cb_func)
  {
    p_cmd_type->cmd.srn_scan_info_list_cmd.cmd_cb_func(p_cmd_type->cmd.srn_scan_info_list_cmd.client_data_ptr, 
                                                             PDSM_PD_CMD_INJECT_SRN_INFO, cmd_status);
  }

  return;
} /* tm_srn_scan_cmd_proc */
#endif /*FEATURE_GNSS_LPPE*/
                                ...
                                )
{
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"TM_PDAPI: Gps session is internal ", 0,0,0);
    return;
  }
  ...
  if (e_pd_event & PDSM_PD_EVENT_POSITION)
  {
    ...
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"tm_pdapi_pos_event_callback: PDSM_PD_EVENT_POSITION, Successful Fixes %u",
  ...
  if( tm_core_is_internal_session_on() != TRUE )
  {
    ...
    /* Spoof Report should not be cleared for NI clients */
    if (p_pd_info_type->client_id != pdsmclient_get_client_id_map(PDSM_CLIENT_TYPE_NI))
    {
      sdm_ClearSpoofReport();
  }
    else
    {
      MSG_2(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "NI Client: Retain Spoof Report 0x%X at TTick:%lu", 
            sdm_GetSpoofReport().spoof_type_mask, sdm_GetSpoofReport().last_spoof_event_timetick);
}
  }
}
                                ...
                                )
{
  ...
  {
    sm_log_report_position(p_pd_info_type, is_sft, is_kf, tm_core_info.diag_buffer_hdl);
                                  ...
                                  tm_pdapi_sess_type  sess_type)
{
  ...
  if (p_pd_info_type == NULL)
  {
    ...
    return;
           ...
           (!(p_pd_info_type->e_pd_event & PDSM_PD_EVENT_WIFI_INFO_ON_DEMAND_INJ_RELATED))
         )
  {
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Internal Session: PDAPI event %d not sent to PDAPI clients", p_pd_info_type->e_pd_event, 0,0);
    send_event = FALSE;
  }

  if (send_event == TRUE)
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"PDAPI event 0x %lx sent to PDAPI clients", p_pd_info_type->e_pd_event,0, 0);
                                 ...
                                 pdsm_lcs_info_s_type *p_lcs_info_type)
{
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_LOW, "TM_PDAPI: tm_pdapi_lcs_event_callback () getting called",0,0,0);
                                                 ...
                                                 pdsm_lcs_info_s_type  *p_lcs_info_type) /* data to be sent to UI client */
{
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_LOW, "TM_PDAPI: tm_lcs_send_notification_verfication () getting called",0,0,0);
                                          ...
                                          pdsm_ext_status_info_s_type *p_ext_status_info_type)
{
  ...
  switch(e_ext_status_event)
  {
    case PDSM_EXT_STATUS_MEASUREMENT:
      sm_log_ext_meas_report((uint32)p_ext_status_info_type->client_id, &p_ext_status_info_type->ext_status_info.ext_meas_report_type, tm_core_info.diag_buffer_hdl);
      ext_status_event_type = PDSM_EXT_STATUS_EVENT_MEASUREMENT;
      break;
    case PDSM_EXT_STATUS_NMEA_POS_REPORT:
     {
       if ( PDAPI_NMEA_VAR_LEN_DEBUG ==
              p_ext_status_info_type->ext_status_info.ext_nmea_var_len_pos_report_type.nmea_type )
       {
         sm_log_ext_nmea_var_len_position_report((uint32)p_ext_status_info_type->client_id,
&p_ext_status_info_type->ext_status_info.ext_nmea_var_len_pos_report_type);
       }
       else
       {
    ...
    case PDSM_EXT_STATUS_POS_REPORT:
      sm_log_diag_position((uint32)p_ext_status_info_type->client_id,
                          &p_ext_status_info_type->ext_status_info.ext_pos_report_type, POS_FROM_MGP,
                           (boolean)FALSE, (boolean)TRUE, tm_core_info.diag_buffer_hdl);
    ...
    case PDSM_EXT_STATUS_PERIODIC_PPM_REPORT:
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"firing PDSM_EXT_STATUS_PERIODIC_PPM_REPORT ", 0,0,0);
      #ifdef FEATURE_GNSS_PERIODIC_PPM
      #error code not present
#endif
      break;
    case PDSM_EXT_STATUS_POS_INJECT_REQUEST:
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"TM_PDAPI: request position injection ", 0,0,0);
    ...
    case PDSM_EXT_STATUS_RESET_LOCATION_SERVICE:
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"TM_PDAPI: request reset location service ", 0,0,0);    
    ...
    case PDSM_EXT_STATUS_GNSS_MEASUREMENT:
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"TM_PDAPI: PDSM_EXT_STATUS_GNSS_MEASUREMENT ", 0,0,0);
      ext_status_event_type = PDSM_EXT_STATUS_EVENT_GNSS_MEASUREMENT;
      break;
	case PDSM_EXT_STATUS_SV_POLY_REPORT:
		MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"TM_PDAPI: PDSM_EXT_STATUS_SV_POLY_REPORT ", 0,0,0);
		 p_ext_status_info_type->client_id = cached_client_id;
		ext_status_event_type = PDSM_EXT_STATUS_EVENT_SV_POLY;
		break;
	
    case PDSM_EXT_STATUS_AVAIL_WWAN_POS_REPORT:
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"TM_PDAPI: Report avail WWAN position", 0, 0, 0);
    ...
    case PDSM_EXT_STATUS_AVAIL_POS_REPORT:
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"TM_PDAPI: Report avail position", 0, 0, 0);
   ...
   case PDSM_EXT_STATUS_REPORT_ME_METRICS:
     MSG(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"TM_PDAPI:  ME Metrics");
     ext_status_event_type = PDSM_EXT_STATUS_EVENT_REPORT_ME_METRICS;
     p_ext_status_info_type->client_id = cached_client_id;
   break;

   case PDSM_EXT_STATUS_ENGINE_MONITOR_REPORT:
     MSG(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"TM_PDAPI: GNSS Engine Monitor Status Report Type");
     ext_status_event_type = PDSM_EXT_STATUS_EVENT_ENGINE_MONITOR_REPORT;
     break;

    case PDSM_EXT_STATUS_UNPROPAGATED_POS_REPORT:
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "TM_PDAPI: Report unpropagated position", 0, 0, 0);
      ext_status_event_type = PDSM_EXT_STATUS_EVENT_UNPROPAGATED_POS_REPORT;
      break;

   case PDSM_EXT_STATUS_LOC_NOTIFICATION_REPORT:
     MSG(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "TM_PDAPI: Privacy: Report location privacy notification");
     ext_status_event_type = PDSM_EXT_STATUS_EVENT_LOC_NOTIFICATION_REPORT;
     break;

   case PDSM_EXT_STATUS_E911_STATE_NOTIFICATION_REPORT:
     MSG(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "TM_PDAPI: Privacy: Report E911 state notification");
     ext_status_event_type = PDSM_EXT_STATUS_EVENT_E911_NOTIFICATION_REPORT;
     break;
     
   case PDSM_EXT_STATUS_TIME_CONV_PARAM_REPORT:
     TM_HIGH_0("TM_PDAPI: GNSS Time Conv Param Report Type");
     ext_status_event_type = PDSM_EXT_STATUS_EVENT_TIME_CONV_PARAM;
     break;
