...
/*===========================================================================
  ...
  $Header: //components/rel/gnss8.mpss/11.1/gnss/sm/tm/src/tm_mm_iface.c#1 $
...
boolean tm_mm_iface_send_event( mm_cgps_event_data_s_type*  p_mm_event_data )
{
  ...
  else 
  {
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Error in sending MM msg to TM thread",0,0,0);
      (void)os_IpcDelete(ipc_msg_ptr);
      ipc_msg_ptr = NULL;
    } /* End status */
    else
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"sending MM msg to TM thread", 0,0,0);
    } /* end else status */
...
void tm_mm_iface_proc_event( void *p_msg )
{
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "TM_RRC_IFACE: tm_rr_iface_proc_event called", 0, 0, 0 );
