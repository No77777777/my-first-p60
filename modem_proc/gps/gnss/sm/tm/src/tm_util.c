...
/*===========================================================================
  ...
  $Header: //components/rel/gnss8.mpss/11.1/gnss/sm/tm/src/tm_util.c#2 $ 
  $DateTime: 2020/01/15 14:48:26 $ 
  $Author: pwbldsvc $ 
...
{
  /* Valid file names: "/GNSS/PE/AlmFile00" to "/GNSS/PE/AlmFile31"                       */
  /* prefix_len,                           prefix,                 suffix :length:min:max */
  {  sizeof("/GNSS/PE/AlmFile") - 1,        "/GNSS/PE/AlmFile",             2,     0,  31 },

  /* Valid file names: "/GNSS/PE/GloAlmFile00" to "/GNSS/PE/GloAlmFile23"                 */
  /*  prefix_len,                          prefix,                 suffix :length:min:max */
  {  sizeof("/GNSS/PE/GloAlmFile") - 1,     "/GNSS/PE/GloAlmFile",          2,     0,  23 },

  /* Valid file name: "/GNSS/PE/SbasCannedFile"                                           */
  /*  prefix_len,                          prefix,                 suffix :length:min:max */
  {  sizeof("/GNSS/PE/SbasCannedFile") - 1, "/GNSS/PE/SbasCannedFile",      0,     0,  0  },

  /* Valid file name: "/GNSS/ME/mcc_bsa.txt"                                              */
  /* prefix_len,                           prefix                  suffix :length:min:max */
  { sizeof("/GNSS/ME/mcc_bsa.txt") - 1,     "/GNSS/ME/mcc_bsa.txt",         0,     0,  0  },
...
FUNCTION tm_util_get_pos_uncert

DESCRIPTION
    Converts an uncertainty specified according to IS801 spec into meters
...
===========================================================================*/
float tm_util_get_pos_uncert(int16 un)
{

  if (un < ARR_SIZE(f_PosUncArr))
  {
    return (f_PosUncArr[un]);
  }
  else
  {
    return f_PosUncArr[MAX_POS_UNC_IND - 1];
  }
}
   ...
   )
{
  ...
  if ((nav_ptr != NULL) && (pos_info != NULL))
  {
    ...
    pos_info->positioning_source |= (nav_ptr->z_PosFlags.b_IsGalUsed) ?
       PDSM_PD_POSITION_SOURCE_GAL : 0;

    pos_info->positioning_source |= (nav_ptr->z_PosFlags.b_IsQzssUsed) ?
       PDSM_PD_POSITION_SOURCE_QZSS : 0;
    
    pos_info->positioning_source |= (nav_ptr->z_PosFlags.v_NavicUsed) ?
   ...
   const sm_NavPositionStructType         *nav_ptr)
{
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR, "null ptr passed %x %x ", ext_pos_data_ptr, nav_ptr, 0);
    return;
  }
   ...
   )
{
  ...
  */
  ...
  if (cl_info == NULL || pd_pos_info_ptr == NULL || ext_pos_data_ptr == NULL ||
      lr_ptr == NULL)
  {
    MSG_4(MSG_SSID_GPSSM, MSG_LEGACY_ERROR, "Null pointer %d %d %d %d",
          (NULL == cl_info), (NULL == pd_pos_info_ptr), (NULL == ext_pos_data_ptr), (NULL == lr_ptr));
    return PDSM_PD_EVENT_END;
  }
  /* Use the time in packet, no need to do time_get()
  */

  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "Formatting Position Data for TM-Core", 0, 0, 0);
  ...
  */
  ...
  cl_info->pd_info.pd_data.ext_pos_data.f_loc_unc_horizontal = sqrt((ext_pos_data_ptr->ext_loc_uncrtnty_a *
                                                                     ext_pos_data_ptr->ext_loc_uncrtnty_a) + (ext_pos_data_ptr->ext_loc_uncrtnty_p *
                                                                                                              ext_pos_data_ptr->ext_loc_uncrtnty_p));
  cl_info->pd_info.pd_data.ext_pos_data.f_alongAxisUnc = ext_pos_data_ptr->ext_loc_uncrtnty_a;
  cl_info->pd_info.pd_data.ext_pos_data.f_perpAxisUnc = ext_pos_data_ptr->ext_loc_uncrtnty_p;
  ...
  */
  ...
  /* Ext Pos Data */
  cl_info->pd_info.pd_data.ext_pos_data.f_velHorU = (uint16)(ext_pos_data_ptr->ext_vel_horiz * 100);
  cl_info->pd_info.pd_data.ext_pos_data.f_velHor = (int16)(ext_pos_data_ptr->ext_vel_horiz * 100);

  if (ext_pos_data_ptr->ext_vel_horiz  < (float)(TM_CORE_MIN_VELOCTY))
  {
    cl_info->pd_info.pd_data.ext_pos_data.f_velHor = 0.0;
    cl_info->pd_info.pd_data.ext_pos_data.f_velHorU = 0;
  }
  else if (ext_pos_data_ptr->ext_vel_horiz > INT16_MAX)
  {
    cl_info->pd_info.pd_data.ext_pos_data.f_velHor = INT16_MAX;
  }
  ...
  cl_info->pd_info.pd_data.ext_pos_data.f_velVert =
     (float)(ext_pos_data_ptr->ext_vel_vert * 100);
  ...
  if (lr_ptr->velocity_incl)
  {
    ...
    cl_info->pd_info.pd_data.ext_opt_field_mask     |= PDSM_PD_EXT_VELOCITY_VALID;
  ...
  */
  if (lr_ptr->height_incl)
  {
    ...
    cl_info->pd_info.pd_data.ext_pos_data.ext_loc_uncertainty_v = lr_ptr->ext_loc_uncrtnty_v;
  ...
  if (tm_core_get_utc_offset((uint8 *)&(cl_info->pd_info.pd_data.gpsUtcOffset)))
  {
  ...
  cl_info->pd_info.pd_data.ext_pos_data.t_dop = (uint32)(ext_pos_data_ptr->t_dop * 10.0 + 0.5);
  cl_info->pd_info.pd_data.ext_pos_data.g_dop = (uint32)(ext_pos_data_ptr->g_dop * 10.0 + 0.5);
  ...
  */
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "tm_util_pd_info_data_init: nv_use_rpt_valid=%d", cl_info->pd_info.pd_data.z_NavSvInfo.u_SvInfoValid, 0, 0);
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "GpsSvMask used=0x%x usable=0x%x",
        cl_info->pd_info.pd_data.z_NavSvInfo.q_GpsSvMaskUsed,
        cl_info->pd_info.pd_data.z_NavSvInfo.q_GpsSvMaskUsable, 0);
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "GloSvMask used=0x%x usable=0x%x",
        cl_info->pd_info.pd_data.z_NavSvInfo.q_GloSvMaskUsed,
        cl_info->pd_info.pd_data.z_NavSvInfo.q_GloSvMaskUsable, 0);
  
  MSG_2(MSG_SSID_GPSSM, MSG_LEGACY_MED, "NavICSVMask used=0x%x usable=0x%x",
        cl_info->pd_info.pd_data.z_NavSvInfo.w_NavicSvMaskUsed,
        cl_info->pd_info.pd_data.z_NavSvInfo.w_NavicSvMaskUsable);

  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "BdsSvMask used=0x%08x%08x",
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "BdsSvMask usable=0x%08x%08x",
        (uint32)((uint32)(cl_info->pd_info.pd_data.z_NavSvInfo.t_BdsSvMaskUsed >> 32) & 0x00000000FFFFFFFF),
        (uint32)(cl_info->pd_info.pd_data.z_NavSvInfo.t_BdsSvMaskUsable & 0x00000000FFFFFFFF), 0);
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "GalSvMask used=0x%08x%08x",
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "GalSvMask usable=0x%08x%08x",
        (uint32)((uint32)(cl_info->pd_info.pd_data.z_NavSvInfo.t_GalSvMaskUsed >> 32) & 0x00000000FFFFFFFF),
        (uint32)(cl_info->pd_info.pd_data.z_NavSvInfo.t_GalSvMaskUsable & 0x00000000FFFFFFFF), 0);
 ...
 )
{
  ...
  {
    d_Azimuth = fix_ptr->z_NavPos.f_HeadingRad * C_RAD_TO_DEG;
  }
  else
  {
    d_Azimuth = 0;
   ...
   )
{
  ...
  else if (input_lr_buf_ptr->z_NavPos.z_PosFlags.v_NavicUsed)
  {
    /* Only Navic constellation was used in fix */
    output_lr_buf_ptr->time_src  = PDSM_GNSS_TIME_NAVIC;
  }
  ...
  */
  ...
  output_lr_buf_ptr->latitude = tm_util_nearest_long(input_lr_buf_ptr->z_NavPos.d_PosLla[0] * C_RAD_TO_DEG *
                                                     LAT_LONG_SCALE);

  /* Convert longitude from radians to degrees and scale. */
  output_lr_buf_ptr->longitude = tm_util_nearest_long(input_lr_buf_ptr->z_NavPos.d_PosLla[1] * C_RAD_TO_DEG *
                                                      LAT_LONG_SCALE);
  ...
  */
  ...
  output_lr_buf_ptr->ext_loc_uncrtnty_v = input_lr_buf_ptr->z_NavPos.f_PuncVertMeters;
  ...
  MSG_2(MSG_SSID_GPSSM, MSG_LEGACY_MED,
        "NAVIC sv mask used=0x%02x usable=0x%02x",
        output_lr_buf_ptr->z_NavSvInfo.w_NavicSvMaskUsed,
        output_lr_buf_ptr->z_NavSvInfo.w_NavicSvMaskUsable);

  MSG_6(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,
        "Final fix Used SVs (GPS=%u, GLO=%u, BDS=%u, GAL=%u, QZSS=%u,  NAVIC=%u)",
        ...
        output_lr_buf_ptr->z_NavSvInfo.u_NavicNumSvPosFix);  
   ...
   )
{
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "TM_UTIL: Invalid file operation", file_operation, 0, 0);
    return FALSE;
  }
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "TM_UTIL file length %d > %d", file_size, PDSM_PD_EFS_MAX_FILE_LEN_BYTES, 0);
    return FALSE;
  }
  ...
  while (cgps_valid_efs_paths[u_index].prefix_length != 0)
  {
    ...
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_LOW, "path name valid", 0, 0, 0);
    return TRUE;
  }
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR, "path name invalid", 0, 0, 0);
  return FALSE;
}
 ...
 )
{
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "TM Core post RTI/Health list 0x%x", q_eph_unhealthy_list, 0, 0);
...
/*===========================================================================
FUNCTION tm_util_convert_Wgs84ToPZ90_datum

DESCRIPTION
Converts WGS84 to PZ90 datum
...
SIDE EFFECTS
===========================================================================*/
void tm_util_convert_Wgs84ToPZ90_datum(const sm_FixInfoStructType *p_fix,
                                       double *d_PosEcefPZ90, double *d_PosLlaPZ90)
{
  gnss_Wgs84ToDtmEcef(p_fix->z_NavPos.d_PosEcef, d_PosEcefPZ90, GNSS_DTM_PZ9011);
  gnss_Ecef2LlaGet(d_PosEcefPZ90, d_PosLlaPZ90, GNSS_DTM_PZ9011);
  return;
}
 ...
 ******************************************************************************
*/
int tm_util_calculate_position_print(const sm_FixInfoStructType *p_fix, 
                                     char *p_Str, int q_buf_len)
{
  ...
  if (TM_CORE_CONFIG_DD(u_control_nmea_sentence) & PDSM_PA_PZ90_DATUM_NMEA_SENTENCE)
  {
    ...
    d_Lat = d_PosLlaPZ90[0] * (180.0 / C_PI);
    d_Long = d_PosLlaPZ90[1] * (180.0 / C_PI);
  }
  else
  {
   ...
   )
{
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_FATAL, "Could not generate random num. Err %d", errorVal, 0, 0);
  }
#endif
...
******************************************************************************
 *
 * tm_util_get_e911_early_exit_qos_config
 *
 * Function description:
 *   Checks if E911 early exit feature is enabled and sets the QOS accordingly.
 *   if E911 enabled
 *      If QOS accuracy (QOS > 50 || QOS == 0 ) meters, set it to 50 
 * Parameters:
 *   None
 *
 * Return value:
 *  uint32 QOS accuracy in meters.
 *
 ******************************************************************************/

uint32 tm_util_get_e911_early_exit_qos_config(uint32 accuracy_qos)
{
  uint32 adjusted_qos = 0;

  if (TM_E911_EARLY_EXIT_ENABLE & tm_core_get_emergency_support_config())
  {
    if (accuracy_qos > MAX_E911_ACCURACY_THRESHOLD || accuracy_qos == 0)
    {
      adjusted_qos = MAX_E911_ACCURACY_THRESHOLD;
    }
    else
    {
      adjusted_qos = accuracy_qos;
    }
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "Accuracy QOS adjusted, E911 Early exit enabled, QOS:%d", adjusted_qos, 0, 0);
  }
  else
  {
    adjusted_qos = accuracy_qos;
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "Accuracy QOS unchanged, E911 Early exit NOT enabled, QOS:%d", adjusted_qos, 0, 0);
  }
...
void tm_util_clean_qzss_sbas_meas_rpt(sm_GpsMeasRptStructType *p_measReport, boolean v_keep_sbas)
{
  ...
  if ((p_measReport == NULL) ||
      (p_measReport->z_MeasBlk.z_GnssHeader.u_NumSvs == 0) ||
      (sm_util_get_master_constellation(p_measReport->z_MeasBlk.z_GnssHeader.e_MeasBlkSrc) != GNSS_MEAS_BLK_SRC_QZSS_SBAS))
  {
    TM_ERROR_0("Non QZSS/SBAS OR Empty Measurement Report");
    return;
  }

  memset((void *)&temp_z_Qzss, 0, sizeof(temp_z_Qzss));

  TM_LOW_1("Original u_NumSvs = %u", p_measReport->z_MeasBlk.z_GnssHeader.u_NumSvs);

  u_itr_max = MIN(N_ACTIVE_CHAN, p_measReport->z_MeasBlk.z_GnssHeader.u_NumSvs);

  if (TRUE == v_keep_sbas)
  {
    w_sv_min = C_FIRST_SBAS_SV_PRN - C_SBAS_SV_PRN_OFFSET;
    w_sv_max = C_LAST_SBAS_SV_PRN - C_SBAS_SV_PRN_OFFSET;
  }
  else
  {
    w_sv_min = C_FIRST_QZSS_SV_PRN;
    w_sv_max = C_LAST_QZSS_SV_PRN;
  }
  ...
  for (u_itr = 0; u_itr < u_itr_max; u_itr++)
  {
    ...
    if ((w_Sv >= w_sv_min) && (w_Sv <= w_sv_max))
    {
      ...
      TM_LOW_1("Adding SV %u", temp_z_Qzss[temp_u_NumSvs].w_Sv);
      temp_u_NumSvs++;
    }
  }
  ...
  if (p_measReport->z_MeasBlk.z_GnssHeader.u_NumSvs != temp_u_NumSvs)
  {
    p_measReport->z_MeasBlk.z_GnssHeader.u_NumSvs = temp_u_NumSvs;
    TM_LOW_1("New u_NumSvs = %u", p_measReport->z_MeasBlk.z_GnssHeader.u_NumSvs);
  }
}
...
******************************************************************************
*/
boolean tm_ConvertToLmLppmMode(const tm_sess_power_mode_s_type *pz_TmConfig, lm_session_request_lppm_s_type *pz_LmConfig)
{
  if ((NULL == pz_TmConfig) || (NULL == pz_LmConfig))
  {
    return FALSE;
  }

  pz_LmConfig->q_TbmMsecs = pz_TmConfig->q_TbmMsecs;
  switch (pz_TmConfig->e_PowerMode)
  {
    case TM_SESS_POWER_MODE_NORMAL:
      pz_LmConfig->e_lppm_mode_type = LM_LPPM_FOREGROUND_FIXED_TIME;
      break;

    case TM_SESS_POWER_MODE_BACKGROUND_DEFINED_POWER:
      pz_LmConfig->e_lppm_mode_type = LM_LPPM_BACKGROUND_FIXED_POWER;
      break;

    case TM_SESS_POWER_MODE_BACKGROUND_DEFINED_TIME:
      pz_LmConfig->e_lppm_mode_type = LM_LPPM_BACKGROUND_FIXED_TIME;
      break;

    case TM_SESS_POWER_MODE_BACKGROUND_KEEP_WARM:
      pz_LmConfig->e_lppm_mode_type = LM_LPPM_BACKGROUND_KEEP_WARM;
      break;

    case TM_SESS_POWER_MODE_IMPROVED_ACCURACY:
      pz_LmConfig->e_lppm_mode_type = LM_NON_LPPM_IMPROVED_ACCURACY_MODE;
      break;

    default:
      MSG_1(MSG_SSID_GPSSM, MSG_LEGACY_ERROR, "Invalid LPPM Mode %d. Default to Normal", pz_TmConfig->e_PowerMode);
      pz_LmConfig->e_lppm_mode_type = LM_LPPM_FOREGROUND_FIXED_TIME;
      return FALSE;
  }
  return TRUE;
...
boolean tm_GetMgpReadiness(mgp_BootupCalStateEnumType e_BootUpCalStatus)
{
  ...
  if ((e_BootUpCalStatus != C_MGP_BOOTUP_CAL_NOT_STARTED) &&
      (e_BootUpCalStatus != C_MGP_BOOTUP_CAL_IN_PROG))
  {
    v_retVal = TRUE;
  }
  TM_HIGH_2("MGPReadinessStatus %d. BootupCalStatus", v_retVal, e_BootUpCalStatus);
  return v_retVal;
}