...
/*============================================================================
  ...
  $Header: //components/rel/gnss8.mpss/11.1/gnss/sm/tm/src/tm_time_util.c#1 $
...
/*===========================================================================
FUNCTION tm_time_util_determine_ls_correction

DESCRIPTION
Checks to see what leap second correction need to be applied. It could be +ve, -ve or none.

DEPENDENCIES

RETURN VALUE
tm_leap_second_correction_e_type:
Indicates the type of leap second correction that need to be applied

SIDE EFFECTS
===========================================================================*/
tm_leap_second_correction_e_type tm_time_util_determine_ls_correction(const sm_FixInfoStructType *p_fix)
{
  static uint32 q_GpsTimeCorrAppliedSec = 0;
  DBL d_FixTimeSec = 0;
  tm_leap_second_correction_e_type e_leap_sec_correction = TM_ZERO_LS_CORRECTION;
  uint32 q_GpsTimeSec = (p_fix->z_NavPos.q_GpsTimeMs) / 1000L;

  /* Get complete GPS time and then convert it into complete UTC time for leap second time change comparison*/
  d_FixTimeSec = ((p_fix->z_NavPos.w_GpsWeek * DAYS_IN_ONE_WEEK * DAY_SECS) + q_GpsTimeSec)
    - tm_core_info.config_info.z_LeapSecondInfo.u_DeltaLeapSec;

  MSG_4(MSG_SSID_GPSSM, MSG_LEGACY_MED,
        "GPS week no = %u, Fix GPS time(week secs) %lu, Fix UTC time(secs) %lu, apply correction at (secs) = %lu",
        p_fix->z_NavPos.w_GpsWeek, q_GpsTimeSec, d_FixTimeSec,
        tm_core_info.config_info.z_LeapSecondInfo.d_UtcLsChangeTimeSec);

  if (d_FixTimeSec == tm_core_info.config_info.z_LeapSecondInfo.d_UtcLsChangeTimeSec)
  {
    if ((TM_NEGATIVE_LS_CORRECTION == tm_core_info.config_info.z_LeapSecondInfo.b_Correction) &&
      (q_GpsTimeCorrAppliedSec != (q_GpsTimeSec - 1)))
    {
      e_leap_sec_correction = TM_NEGATIVE_LS_CORRECTION;
      q_GpsTimeCorrAppliedSec = q_GpsTimeSec;
      MSG_1(MSG_SSID_GPSSM, MSG_LEGACY_MED,
            "Negative Leap Second Correction applied at GPS week time (Sec) = %u", q_GpsTimeCorrAppliedSec);
    }
    else if ((TM_POSITIVE_LS_CORRECTION == tm_core_info.config_info.z_LeapSecondInfo.b_Correction) &&
      (q_GpsTimeCorrAppliedSec != (q_GpsTimeSec - 1)))
    {
      e_leap_sec_correction = TM_POSITIVE_LS_CORRECTION;
      q_GpsTimeCorrAppliedSec = q_GpsTimeSec;
      MSG_1(MSG_SSID_GPSSM, MSG_LEGACY_MED,
            "Positive Leap Second Correction applied at GPS week time (Sec) = %u", q_GpsTimeCorrAppliedSec);
    }
    else
    {
      MSG(MSG_SSID_GPSSM, MSG_LEGACY_MED, "No Leap Second Correction applied");
    }
  }
  return e_leap_sec_correction;
}
...
)
{
  ...
  boolean v_RetVal = FALSE;
  tm_leap_second_correction_e_type e_ls_correction = TM_ZERO_LS_CORRECTION;
  uint32 q_StartingYear = 1980;

  if (NULL == pz_Fix || NULL == pz_TimeOut || NULL == q_TotalDays)
  {
    MSG_2(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"null ptr passed in=%x out=%x", pz_Fix, pz_TimeOut);
    return FALSE;
  }
  ...
  {
    v_RetVal = tm_util_construct_gps_time(pz_NavPos->w_GpsWeek,
  ...
  {
    q_StartingYear = 1996;
    v_RetVal = tm_util_construct_glo_time(pz_NavPos->u_GloNumFourYear,
  ...
  if (FALSE == v_RetVal)
  {
    TM_ERROR_1("tm_util_get_fix_time() failed. GPS %d", (C_GPS_WEEK_UNKNOWN != pz_NavPos->w_GpsWeek));
    return FALSE;
  }

  e_ls_correction = tm_time_util_determine_ls_correction(pz_Fix);
  switch (e_ls_correction)
  {
    case TM_POSITIVE_LS_CORRECTION:
      pz_TimeOut->q_Hour = TM_POSITIVE_LS_CORRECTION_HOURS;
      pz_TimeOut->q_Minute = TM_POSITIVE_LS_CORRECTION_MINUTES;
      pz_TimeOut->q_Second = TM_POSITIVE_LS_CORRECTION_SECONDS;
      break;

    case TM_NEGATIVE_LS_CORRECTION:
      pz_TimeOut->q_Hour = TM_NEGATIVE_LS_CORRECTION_HOURS_MINS_SECS;
      pz_TimeOut->q_Minute = TM_NEGATIVE_LS_CORRECTION_HOURS_MINS_SECS;
      pz_TimeOut->q_Second = TM_NEGATIVE_LS_CORRECTION_HOURS_MINS_SECS;
      break;

    case TM_ZERO_LS_CORRECTION:
    default:
      break;
  }
  *q_TotalDays -= e_ls_correction;

  fill_calendar_date(q_StartingYear, *q_TotalDays, pz_TimeOut);
...
/**
* @fn tm_util_FindOldGpsWeekFromRomGpsAlm
...
*/
uint16 tm_util_FindOldGpsWeekFromRomGpsAlm(void)
{
  uint8   u_I = 0;
  uint16  w_GpsWeek = C_GPS_WEEK_UNKNOWN;           /* Oldest GPS week */
  gps_AlmStructType pez_RomAlm[N_GPS_SV],
    *pz_RomAlm[N_GPS_SV];

  for (u_I = 0; u_I < N_GPS_SV; u_I++)
  {
    memset(&pez_RomAlm[u_I], 0, sizeof(gps_AlmStructType));
    pez_RomAlm[u_I].u_Sv = C_GPS_SV_ALM_INVALID;
    pz_RomAlm[u_I] = &pez_RomAlm[u_I];
  }

  gps_GetGpsRomAlmanac(pz_RomAlm);

  /* Find the oldest GPS week number of all the SVs */
  for (u_I = 0; u_I < N_GPS_SV; u_I++)
  {
    if ((pz_RomAlm[u_I]->u_Sv != C_GPS_SV_ALM_INVALID) && (pz_RomAlm[u_I]->w_GpsWeek < w_GpsWeek))
    {
      w_GpsWeek = pz_RomAlm[u_I]->w_GpsWeek;
    }
  }
...
/**
* @fn tm_util_ConvertBuildTime2GpsWeek
...
*         C_GPS_WEEK_UNKNOWN)
*/
uint16 tm_util_ConvertBuildTime2GpsWeek()
{
  char                 b_Month[4] = {0};
  uint16               w_GpsWeek = C_GPS_WEEK_UNKNOWN;
  uint32               q_GpsTowMs = 0;
  tm_CalendarTimeT     z_CalTime = {0};
  char const           b_MonthNames[] = "JanFebMarAprMayJunJulAugSepOctNovDec";
  uint8                u_LeapSeconds = 0;
  pdsm_utc_time_s_type z_PdsmUtcTime = {0};

  /* Parse the date in MMM DD YYYY format to corresponding fields in calendar struct */
  if (3 == sscanf(ver_date, "%3s %2ld %4ld", b_Month, &z_CalTime.q_Day, &z_CalTime.q_Year))
  {
    z_CalTime.q_Month = ((strstr(b_MonthNames, b_Month) - b_MonthNames) / 3) + 1;

    /* Parse the time in HH::MM::SS format to corresponding fields in calendar struct */
    if (3 == sscanf(ver_time, "%2ld:%2ld:%2ld", &z_CalTime.q_Hour, &z_CalTime.q_Minute, &z_CalTime.q_Second))
    {
      /* Convert calender date & time to pdsm utc format, then get the leap seconds & finally
      convert the pdsm utc time to GPS week & ToW */
      if (tm_util_time_to_pdsm_time(&z_CalTime, &z_PdsmUtcTime))
      {
        /* Get the leap second & don't care about the uncertainity return by the function */
        tm_core_get_utc_offset(&u_LeapSeconds);
        gnss_ConvertUtcTimeStamptoGpsTime(&z_PdsmUtcTime,
                                          u_LeapSeconds,
                                          &w_GpsWeek,
                                          &q_GpsTowMs);
      }
  ...
  MSG_6(MSG_SSID_GPSSM, MSG_LEGACY_MED, "Compile time (MM-DD-YYYY HH:mm:ss) %02ld-%02ld-%04ld %02ld:%02ld:%02ld", z_CalTime.q_Month, z_CalTime.q_Day,
        z_CalTime.q_Year, z_CalTime.q_Hour, z_CalTime.q_Minute, z_CalTime.q_Second);
