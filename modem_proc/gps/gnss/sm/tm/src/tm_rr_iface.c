...
$Header: //components/rel/gnss8.mpss/11.1/gnss/sm/tm/src/tm_rr_iface.c#1 $

when        who     what, where, why
--------    ---     ----------------------------------------------------------
03/19/18    py      Sending AS_ID in APDU abort and Session Abort event
        ...
        const rr_cgps_event_data_type * event_data )
{
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "TM_RR_IFACE: tm_rr_iface_send_event called", 0, 0, 0);

  if( event_data == NULL)
  {
    MSG( MSG_SSID_GPSSM, MSG_LEGACY_ERROR, "CP-GSM:NULL Pointer passed to function tm_rr_iface_send_event");
    return;
  }

  if((event_data->as_id < SYS_MODEM_AS_ID_1) || (event_data->as_id > SYS_MODEM_AS_ID_3))
  {
  	MSG_1(MSG_SSID_GPS, MSG_LEGACY_ERROR, "CGPS_NV_EFS: Invalid as_id",(uint32)event_data->as_id);
    return;
  }
  ...
  if( ipc_msg_ptr == NULL )
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR, "TM_RR_IFACE: Can't allocate Event IPC Message", 0, 0, 0);
    return;
  }
  ...
  switch( event_data->event_id )
  {
    ...
    default:
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR, "TM_RR_IFACE: Invalid RR->TM event_id=%d", event_data->event_id, 0, 0 );
      return;
  }
  ...
  if( !ipc_send_ok )
  {
    ...
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR, "TM_RR_IFACE: Error sending RR message to TM thread", 0, 0, 0 );
  }
  else
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "TM_RR_IFACE: Sending RR Event %d to TM thread with as_id %d", rr_msg_ptr->event_id, (uint8)rr_msg_ptr->as_id, 0 );
  }
}
...
{
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "TM_RR_IFACE: tm_rr_iface_proc_event called", 0, 0, 0 );

  if( p_msg == NULL )
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR, "TM_RR_IFACE: Invalid NULL pointer in proc_event", 0, 0, 0 );
    return;
  }
...
{
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "TM_RR_IFACE: tm_rr_iface_init called", 0, 0, 0 );

  /*
   *  Register an event handler with RR
   */
 /* To-do: this flag FEATURE_CGPS_NO_UMTS_TDS will create problem when gsm only build is created as it will undef all GSM and UMTS feature Flags */
