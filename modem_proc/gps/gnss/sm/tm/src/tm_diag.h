...
/*===========================================================================
  ...
  $Header: //components/rel/gnss8.mpss/11.1/gnss/sm/tm/src/tm_diag.h#3 $  
...
enum
{
  ...
  TM_DIAG_GPS_CMD_CODE_LTE_CATM_TX_START,                             /* 45 */
  TM_DIAG_GPS_CMD_CODE_LTE_CATM_TX_END,                               /* 46 */
#endif
...
typedef PACKED struct PACKED_POST
{
  ...
  uint32 q_GalE5bCnoP1DBHz; /* C/N0 reported in units of 0.1 dB-Hz */
  int32  l_GalE5bFreqHz;    /* freq reported in Hz */
  uint32 q_NavicCnoP1DBHz; /* C/N0 reported in units of 0.1 dB-Hz */
  int32  l_NavicFreqHz;    /* freq reported in Hz */
  uint32 q_GpsL2CnoP1DBHz; /* C/N0 reported in units of 0.1 dB-Hz */
  int32  l_GpsL2FreqHz;    /* freq reported in Hz */
...
#define C_GPSDIAG_EXT_WBFFT_RESULTS_RSP_VER 4 /* v4 adds support for GAL E5b,
                                                 NavIC, and GPS L2*/
typedef PACKED struct PACKED_POST
{
  gpsdiag_CmdHdrType_v2 z_DiagCmdHdrV2;     /* diagpkt_subsys_header_type */
...
/* Definition for PFA Test request and responses (Immediate and Delayed) */
typedef PACKED struct PACKED_POST
{
  gpsdiag_CmdHdrType_v2 z_DiagCmdHdrV2; /* diagpkt_subsys_header_type */
  uint8  u_Version; /* Min value of 1 */
  uint16 w_GpsPrescDwellCount; /* Number of desired prescribed dwell attempts */
  uint8  u_GpsPfa;       /* PFA is internally represented as a power of 10. 
                            This field defines the exponent without the negative sign. 
                            For example, if PFA 10^(-4) is desired, then this 
                            field equals 4.*/
  uint16 w_GpsNumPostD;  /* Number of post-detections */
  uint16 w_GloPrescDwellCount; /* Number of desired prescribed dwell attempts */
  uint8  u_GloPfa;       /* PFA is internally represented as a power of 10. 
                            This field defines the exponent without the negative sign. 
                            For example, if PFA 10^(-4) is desired, then this 
                            field equals 4.*/
  uint16 w_GloNumPostD;  /* Number of post-detections */
  uint16 w_BdsPrescDwellCount; /* Number of desired prescribed dwell attempts */
  uint8  u_BdsPfa;       /* PFA is internally represented as a power of 10. 
                            This field defines the exponent without the negative sign. 
                            For example, if PFA 10^(-4) is desired, then this 
                            field equals 4.*/
  uint16 w_BdsNumPostD;  /* Number of post-detections */
  uint16 w_GalPrescDwellCount; /* Number of desired prescribed dwell attempts */
  uint8  u_GalPfa;       /* PFA is internally represented as a power of 10. 
                            This field defines the exponent without the negative sign. 
                            For example, if PFA 10^(-4) is desired, then this 
                            field equals 4.*/
  uint16 w_GalNumPostD;  /* Number of post-detections */

} gpsdiag_PfaTestReqType;

/* Immediate Response */
typedef PACKED struct PACKED_POST
{
  gpsdiag_CmdHdrType_v2 z_DiagCmdHdrV2;        /* diagpkt_subsys_header_type */
} gpsdiag_PfaTestImmedRspType;

/* Delayed Response */
/* PFA test states */
typedef enum
{
  C_TM_DIAG_PFA_TEST_INIT = 0,
  C_TM_DIAG_PFA_TEST_RUNNING,
  C_TM_DIAG_PFA_TEST_SUCCESS,
  C_TM_DIAG_PFA_TEST_FAILURE
} gpsdiag_PfaTestStateEnum;

/* Structure for each SV */
typedef PACKED struct PACKED_POST
{
  uint32 q_SvValidCnt;
  uint32 q_SvDwellCnt;
  uint32 q_MinCN0p1dB; /* units of 0.1 dB */
  uint32 q_MaxCN0p1dB; /* units of 0.1 dB */
} gpsdiag_PfaTestSvType;

typedef PACKED struct PACKED_POST
{
  gpsdiag_CmdHdrType_v2 z_DiagCmdHdrV2; /* diagpkt_subsys_header_type */
  uint8 u_Version; /* Min value of 1 */
  uint8 u_TestNum; /* Test number */
  uint8 e_State; /* gpsdiag_PfaTestStateEnum, for the state */
  uint32 q_GpsAggregatedPfa; /* Average PFA across all SVs and dwells in units of 0.0001 */
  uint32 q_GloAggregatedPfa; /* Average PFA across all SVs and dwells in units of 0.0001 */
  uint32 q_BdsAggregatedPfa; /* Average PFA across all SVs and dwells in units of 0.0001 */
  uint32 q_GalAggregatedPfa; /* Average PFA across all SVs and dwells in units of 0.0001 */
  gpsdiag_PfaTestSvType z_Gps[N_GPS_SV];
  gpsdiag_PfaTestSvType z_Glo[C_MAX_GLO_CHANNELS_12];
  gpsdiag_PfaTestSvType z_Bds[N_BDS_SV];
  gpsdiag_PfaTestSvType z_Gal[N_GAL_SV];
} gpsdiag_PfaDelayedRspType;

/* Definition of Get PFA Results command  */
typedef PACKED struct PACKED_POST
{
  gpsdiag_CmdHdrType_v2 z_DiagCmdHdrV2;     /* diagpkt_subsys_header_type */
} gpsdiag_GetPfaResultsReqType;
...
/*===========================================================================
FUNCTION tm_diag_PfaTestRequest

DESCRIPTION
  Request that PFA Test be run.
...
SIDE EFFECTS
  PFA test will be started, which can take quite a long time to complete.
  As series of prescribed dwells are requested while the test runs.
===========================================================================*/
void tm_diag_PfaTestRequest(gpsdiag_PfaTestReqType *p_Req,
                            gpsdiag_PfaTestImmedRspType *p_ImmedRsp);

/*===========================================================================
FUNCTION tm_diag_PfaTestPrescDwellResultsHandler

DESCRIPTION
  Process prescribed dwell results for a particular SV.
...
SIDE EFFECTS
  If a dwell is completed, then another Prescribed Dwell will be requested.
  If the PFA test is completed, then a delayed DIAG response will be
  sent with the final test results.
===========================================================================*/
void tm_diag_PfaTestPrescDwellResultsHandler(sm_PfaTestPresDwellResultsType
                                             *p_Results);

/*===========================================================================
FUNCTION tm_diag_PfaTestSanityTimeout

DESCRIPTION
  Sanity timer expired while waiting for a prescribed dwell to complete.
...
SIDE EFFECTS
  PFA test will be marked as failed.  If the prescribed dwell failed
  because the GNSS receiver is in a bad state, then subsequent PFA
  test attempts may also fail, or could put MC Jobmanager into a
  bad state.
===========================================================================*/
extern void tm_diag_PfaTestSanityTimeout(void);

/*===========================================================================
FUNCTION tm_diag_PfaGetResults

DESCRIPTION
  Get the results for the PFA test
...
SIDE EFFECTS
  None.
===========================================================================*/
extern void tm_diag_PfaGetResults(gpsdiag_GetPfaResultsRspType *p_Rsp);
...
extern void tm_api_nav_config_change_resp_handler(uint32 q_CurrGnssConfig);
