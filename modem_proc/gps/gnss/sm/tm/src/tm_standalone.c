...
/*===========================================================================
                           ...
                           Edit History
  $Header: //components/rel/gnss8.mpss/11.1/gnss/sm/tm/src/tm_standalone.c#2 $ 
  $DateTime: 2020/03/19 12:31:17 $ 
  $Author: pwbldsvc $ 

when       who     what, where, why
--------   ---     ----------------------------------------------------------
06/06/19   sj      Disable AGNSS for 5G NR.
04/10/19   mj      Deprecate NV1930
05/25/18   sum     Added support for AGLO over LTE + RRLP
...
tm_sess_operation_mode_e_type tm_standalone_convert_op_mode(pdsm_pd_session_operation_e_type op_mode)
{
  switch (op_mode)
  {
    ...
    default:
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "set to default operation mode", 0, 0, 0);
      return TM_SESS_OPERATION_MODE_STANDALONE;
  }
}
...
FUNCTION tm_standalone_convert_power_mode

DESCRIPTION
  Converts PDSM Power Mode to TM Type
...
===========================================================================*/
static void tm_standalone_convert_power_mode(
  const pdsm_pd_session_power_s_type* pz_PdsmPowerMode, 
  tm_sess_power_mode_s_type* pz_TmPowerMode)
{
  if ((NULL == pz_PdsmPowerMode) || (NULL == pz_TmPowerMode))
  {
    MSG(MSG_SSID_GPSSM, MSG_LEGACY_ERROR, "NULL Pointer. Cannot Convert");
    return;
  }

  pz_TmPowerMode->q_TbmMsecs = pz_PdsmPowerMode->tbm_msecs;
  switch (pz_PdsmPowerMode->powerMode)
  {
    /* Power Mode M1 */
    case PDSM_SESSION_POWER_MODE_IMPROVED_ACCURACY:
      pz_TmPowerMode->e_PowerMode = TM_SESS_POWER_MODE_IMPROVED_ACCURACY;
      break;

      /* Power Mode M2 */
    case PDSM_SESSION_POWER_MODE_NORMAL:
      pz_TmPowerMode->e_PowerMode = TM_SESS_POWER_MODE_NORMAL;
      break;

      /* Power Mode M3 */
    case PDSM_SESSION_POWER_MODE_BACKGROUND_DEFINED_POWER:
      pz_TmPowerMode->e_PowerMode = TM_SESS_POWER_MODE_BACKGROUND_DEFINED_POWER;
      break;

      /* Power Mode M4 */
    case PDSM_SESSION_POWER_MODE_BACKGROUND_DEFINED_TIME:
      pz_TmPowerMode->e_PowerMode = TM_SESS_POWER_MODE_BACKGROUND_DEFINED_TIME;
      break;

      /* Power Mode M5 */
    case PDSM_SESSION_POWER_MODE_BACKGROUND_KEEP_WARM:
      pz_TmPowerMode->e_PowerMode = TM_SESS_POWER_MODE_BACKGROUND_KEEP_WARM;
      break;

      /* Any other mode */
    default:
      MSG_1(MSG_SSID_GPSSM, MSG_LEGACY_MED, "Invalid Mode %d. Default to Normal", pz_PdsmPowerMode->powerMode);
      pz_TmPowerMode->e_PowerMode = TM_SESS_POWER_MODE_NORMAL;
      break;
  }
}
   ...
   )
{
  ...
  if (PDSM_SESSION_OPERATION_STANDALONE_ONLY == start_sess_req_param->pd_option.operation_mode)
  {
    if (!tm_xtra_data_validity_check(GPS_TIME_CHECK_SOURCE_PER_FIX))
    {
      ...
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR, "Failed to invoke XTRA-T", 0, 0, 0);
      }
  ...
  /* Power Mode*/
  tm_standalone_convert_power_mode(
    &start_sess_req_param->pd_option.power_mode, 
    &tm_standalone_sess_info.power_mode);
  ...
  action_ptr->lppm_mode = tm_standalone_sess_info.power_mode;
...
void tm_standalone_resume_session(void)
{
  ...
  tm_sess_handle_type            e911_session_handle = TM_SESS_HANDLE_NONE;
  ...
  else if (phone_state.srv_system == CGPS_SRV_SYS_NR5G)
  {
    MSG(MSG_SSID_GPSSM, MSG_LEGACY_MED, "Standalone only supported for NR5G!!");
    need_dload = FALSE;
  }
  else
  {
  ...
  */
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "Standalone session initiated by 1x UP NI. Resume 1x to handle LR Reporting", 0, 0, 0);
    need_dload = TRUE;
  }
  ...
  if (prtl_tbl_idx == -1)
  {
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "NO AGPS protocol supported in this configuration. Fall back to Standalone", 0, 0, 0);
      need_dload = FALSE;
    }
  }
#endif
  
  if (TM_CM_E911_STATE_CONCURRENT == phone_state.e911_state)
  {
    if ((tm_core_info.config_info.q_allow_mo_supl_during_concurrency & TM_CORE_ALLOW_MO_SUPL_DURING_CONCURRENCY) != 0)
    {
      /* NV72533 allows standalone session to go to server during concurrency */
    
      /* Get main and secondary client  from LDM */
      tm_lpc_get_main_secondary_clients(&e_main_client_id, &e_secondary_client_id);
      if (LDM_CLIENT_ID_E911 == e_main_client_id)
      {
        ...
        {
          e911_session_handle = p_z_lpc_client->sessionHandle; 
        }
      }
      else if (LDM_CLIENT_ID_E911 == e_secondary_client_id)
      {
        p_z_lpc_client = lpc_get_loc_processing_client(e_secondary_client_id);

        if (NULL == p_z_lpc_client)
        {
          MSG_1(MSG_SSID_GPSSM, MSG_LEGACY_ERROR, "LPC: Client %d not found in database", e_secondary_client_id);
        }
        else
        {
          e911_session_handle = p_z_lpc_client->sessionHandle; 
      ...
      {
        MSG(MSG_SSID_GPSSM, MSG_LEGACY_MED, "LPC: E911 client not found");
      }        
        
      if ((TM_SESS_HANDLE_E911 == e911_session_handle)||
          (TM_SESS_HANDLE_NONE == e911_session_handle))
      {
        MSG_1(MSG_SSID_GPSSM, MSG_LEGACY_MED, "LPC: Allow standalone session to go to server during concurrency if needed. need_dload = %d", need_dload);
      }
      else
      {
        /* do not allow assistance download and continue in standalone mode*/
        need_dload = FALSE;
        MSG_1(MSG_SSID_GPSSM, MSG_LEGACY_MED, "LPC:Do not allow standalone session to go to server during concurrency. need_dload = %d", need_dload);
      }
    }
    else
    {
      /* NV72533 does not allow standalone session to go to server during concurrency*/
      need_dload = FALSE;
      MSG_1(MSG_SSID_GPSSM, MSG_LEGACY_MED, "LPC: Do not allow standalone session to go to server during concurrency due to NV72533. need_dload = %d", need_dload);
    }
  }
  ...
  else
  {
    ...
    sessReqParam.continue_param.lppm_mode          = tm_standalone_sess_info.power_mode;
    sessReqParam.continue_param.dynamic_qos_enabled = 
      (tm_standalone_sess_info.power_mode.e_PowerMode <= TM_SESS_POWER_MODE_NORMAL);
...
void tm_standalone_handle_wish_list_update(const tm_aiding_data_wishlist_s_type *aiding_data_status_ptr)
{
  ...
  /* If Glonass is enabled, AND */
     (0 != (C_RCVR_GNSS_CONFIG_GLO_ENABLED & sm_get_supported_constellation_mask()))
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "Eph invalid - GPS (GLO not chkd)", 0, 0, 0);
  ...
  else
  {
    ...
    if (aglo_ok_over_up)
    {
      ...
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "Eph invalid - GLONASS", 0, 0, 0);
        tm_standalone_sess_info.aiding_data_status.eph_validity = FALSE;
      }
      else
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "Eph valid - GPS+GLONASS", 0, 0, 0);
        tm_standalone_sess_info.aiding_data_status.eph_validity = TRUE;  /*lint !e506 */
      }
    }
    else
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "Eph valid - GPS (GLO not supported)", 0, 0, 0);
      tm_standalone_sess_info.aiding_data_status.eph_validity = TRUE;  /*lint !e506 */
    }
  }
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "Alm invalid - GPS (GLO not chkd)", 0, 0, 0);
    tm_standalone_sess_info.aiding_data_status.alm_validity = FALSE;
  }
  else
  {
    ...
    if (aglo_ok_over_up)
    {
      ...
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "Alm invalid - GLONASS", 0, 0, 0);
        tm_standalone_sess_info.aiding_data_status.alm_validity = FALSE;
      }
      else
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "Alm valid - GPS+GLONASS", 0, 0, 0);
        tm_standalone_sess_info.aiding_data_status.alm_validity = TRUE;  /*lint !e506 */
      }
    }
    else
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "Alm valid - GPS (GLO not supported)", 0, 0, 0);
      tm_standalone_sess_info.aiding_data_status.alm_validity = TRUE;  /*lint !e506 */
    }
  }
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "Ref Loc invalid", 0, 0, 0);
    tm_standalone_sess_info.aiding_data_status.pos_unc = TM_LR_THRESHOLD_NO_POS;
  }
  else
  {
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "Ref Loc valid", 0, 0, 0);
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "Pos Uncertainty %lu", (uint32)tm_standalone_sess_info.aiding_data_status.pos_unc, 0, 0);
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "Time Uncertainty %lu", (uint32)tm_standalone_sess_info.aiding_data_status.time_unc, 0, 0);
  ...
  if (lr_resp_ptr != NULL)
  {
    ...
    if (tm_util_convert_lr(lr_data, lr_resp_ptr))
    {
      ...
      */
      ...
      tm_standalone_ext_pos_data.t_dop = lr_data->z_NavPos.f_TDOP;
      tm_standalone_ext_pos_data.g_dop = lr_data->z_NavPos.f_GDOP;
      ...
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "Time Source from PE: %d", lr_resp_ptr->time_src, 0, 0);
      ...
      if (tm_core_get_utc_offset(&u_leap_sec))
      {
        tm_standalone_info_buffer.pd_info.pd_data.ext_opt_field_mask |= PDSM_PD_UTCOFFSET_VALID;
      }
      else
      {
        tm_standalone_info_buffer.pd_info.pd_data.ext_opt_field_mask &= (~PDSM_PD_UTCOFFSET_VALID);
      ...
      tm_standalone_info_buffer.pd_info.pd_data.gpsUtcOffset = (int8)u_leap_sec;
   ...
   )
{
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR, "Can not match handle tm-core = %d, standalone = %d",
          sess_handle,
          tm_standalone_sess_info.sess_handle,
          0);
    ret_val = FALSE;
