...
/*===========================================================================
  ...
  $Header: //components/rel/gnss8.mpss/11.1/gnss/sm/tm/src/tm_prtl_iface.c#1 $  
  ...
  switch(req_type)
  {
    case TM_SESS_REQ_START:
      ...
      { 
		...
		case TM_CM_E911_STATE_CONCURRENT:	
        {
...
#if !(defined(FEATURE_GNSS_LEAN_BUILD) || defined(FEATURE_GNSS_SA))
          if ((((z_umts_nv_item_ram_copy.q_gnss_lock_ctrl_for_oem)& PDSM_PA_CP_NILR_OEM_CTRL)!= 0) &&
              (( TM_CORE_CONFIG_DD(gps_lock) == PDSM_GPS_LOCK_MT ) || ( TM_CORE_CONFIG_DD(gps_lock) == PDSM_GPS_LOCK_ALL ))
             )
          {          
            MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"MT GPS is locked", 0, 0, 0);
            mt_allowed = FALSE;

            /* Send Reject Notification */
            location_notification.q_prtl_type = TM_PRTL_TYPE_LPP_CP;
...
#endif /* !(defined(FEATURE_GNSS_LEAN_BUILD) || defined(FEATURE_GNSS_SA)) */
         if(mt_allowed && tm_core_session_active())  // if MT GPS is locked, don't bother to continue checking
          {
            ...
            {
              MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_LOW,"Got MT Request while there is an internal session", 0, 0, 0); // allow it
            }
            else // There is an active non-internal session
            {
              ...
              */
              ...
              {
                 MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Accept 1X CP MT and terminate current session (handle=%d)", 
              ...
              else
              {
                      ...
                      tm_core_info.session_info.active_sess_handle != TM_SESS_HANDLE_E911)
                {
                  ...
                  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Accept LPP CP session and terminate current TM session (handle=%d)", 
                           tm_core_info.session_info.active_sess_handle, 0, 0);
                   ...
                   tm_core_info.session_info.active_sess_handle == TM_SESS_HANDLE_E911)
                {
                  ...
                  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Accept CP/UP session from (prtl= %d) even if early rcvr ongoing (handle=%d)", 
                           prtl_type, tm_core_info.session_info.active_sess_handle, 0);
                }
                else
                {
                  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Rejecting MT Request(handle=%d mtflag=%d) during active session(handle=%d)", 
      ...
      if(mt_allowed)
      {
          ...
          else if (TM_CM_E911_STATE_CONCURRENT == phone_state_info.e911_state)
          {
			...
			{
			  tm_core_info.pz_concurrent_client_info.client_id = pdsm_ni_client_id_get();
			}
			else if (LDM_CLIENT_ID_MAX == e_secondary_client)
			{
			  MSG(MSG_SSID_GPSSM, MSG_LEGACY_MED, "No secondary client added yet into LDM. Store NI client id in concurrent mode");
			  tm_core_info.pz_concurrent_client_info.client_id = pdsm_ni_client_id_get();
			}
			else
			{
          ...
          if(TRUE == retVal)
          {            
            tm_core_report_sess_start_to_techsel(&(req_param->start_param), sess_handle);
          }
      }
      else
      {
...
)
{
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Null pointer passed by protocol %d sess_handle %d", 
...
/*===========================================================================
FUNCTION  tm_core_pos_mode_allows_aglo_msb_up 

DESCRIPTION
  Checks if the correct bits are set in NV 1920 to allow AGLO MSB position
  method type for U-Plane position protocols.

DEPENDENCIES

RETURN VALUE
  TRUE: If the AGLO MSB bit are set correctly in the NV 1920.
  FALSE: Else, otherwise.

SIDE EFFECTS

===========================================================================*/
boolean tm_core_pos_mode_allows_aglo_msb_up( tm_positioning_protocols const e_pos_protocol)
{
  if (FALSE == tm_umts_common_glonass_enabled())
  {
    return FALSE;
  }
  ...
  switch ( e_pos_protocol )
  {
     case TM_POS_PRTL_RRLP:
       /* Both A-GPS & A-GLO MSB bit is set to allow AGLO MSB for RRLP */
       if ((( TM_CONFIG_POS_MODE_BIT_IS_SET( PDSM_POS_MODE_UP_MS_BASED ) != 0) &&
            ( TM_CONFIG_POS_MODE_BIT_IS_SET( PDSM_AGLO_POS_MODE_3G_UP_MS_BASED ) != 0 )))
       {
         return ( TRUE );
       }
       break; 
     ...
     case TM_POS_PRTL_LPP:
       /* Both A-GPS & A-GLO MSB bit is set to allow AGLO MSB for LPP */
...
/*===========================================================================
FUNCTION  tm_core_pos_mode_allows_aglo_msa_up

DESCRIPTION
  Checks if the correct bits are set in NV 1920 to allow AGLO MSA position
  method type for U-Plane position protocols.

DEPENDENCIES

RETURN VALUE
  TRUE: If the AGLO MSA bit are set correctly in the NV 1920.
  FALSE: Else, otherwise.

SIDE EFFECTS

===========================================================================*/
boolean tm_core_pos_mode_allows_aglo_msa_up( tm_positioning_protocols const e_pos_protocol)
{
  if (FALSE == tm_umts_common_glonass_enabled())
  {
    return FALSE;
  ...
  switch ( e_pos_protocol )
  {
     case TM_POS_PRTL_RRLP:
       ...
       if ( ( TM_CONFIG_POS_MODE_BIT_IS_SET( PDSM_POS_MODE_UP_MS_ASSISTED ) != 0) &&
            ( TM_CONFIG_POS_MODE_BIT_IS_SET( PDSM_AGLO_POS_MODE_3G_UP_MS_ASSISTED ) != 0 ) )
       {
         return ( TRUE );
       }
       break; 
...
/*===========================================================================
FUNCTION  tm_core_pos_mode_allows_aglo_msb_cp
...
===========================================================================*/
boolean tm_core_pos_mode_allows_aglo_msb_cp( tm_positioning_protocols const e_pos_protocol)
{
  if (FALSE == tm_umts_common_glonass_enabled())
  {
    return FALSE;
...
/*===========================================================================
FUNCTION  tm_core_pos_mode_allows_aglo_msa_cp
...
===========================================================================*/
boolean tm_core_pos_mode_allows_aglo_msa_cp( tm_positioning_protocols const e_pos_protocol)
{
  if (FALSE == tm_umts_common_glonass_enabled())
  {
    return FALSE;
