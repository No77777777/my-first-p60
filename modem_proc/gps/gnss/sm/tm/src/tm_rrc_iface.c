...
/*===========================================================================
  ...
  $Header: //components/rel/gnss8.mpss/11.1/gnss/sm/tm/src/tm_rrc_iface.c#1 $
...
boolean tm_rrc_iface_send_event( rrc_cgps_event_data_s_type*  rrc_event_data_ptr )
{
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR, "Null ptr sent to tm_rrc_iface_send_event", 0, 0, 0);
    return FALSE;
  }
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "rrc_meas_param_ptr is NULL", 0, 0, 0);
    return FALSE;
  }
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "meas_param_type not supported %d", 
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Error in sending RRC msg to TM thread",0,0,0);
...
void tm_rrc_iface_proc_event( void *p_msg )
{
  ...
  if( p_msg == NULL )
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR, "TM_RRC_IFACE: Invalid NULL pointer in proc_event", 0, 0, 0 );
    return;
  }
  ...
  #else
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "TM_RRC_IFACE: tm_rrc_iface_proc_event called", 0, 0, 0 );
  #endif
...
void tm_rrc_iface_init( void )
{
	...
	#endif
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"tm_rrc_iface: event_cb reg. failed", 0, 0, 0);
    }
    ...
    else /* register another call back if the previous one succeeds. */
    {
	  ...
	  #endif
	  {
	    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"tm_rrc_iface: ue_cap. reg. failed", 0, 0, 0);
	  }
	  #endif /*FEATURE_DUAL_WCDMA && FEATURE_CGPS_DUAL_WCDMA*/    
      #endif
    }
