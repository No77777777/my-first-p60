...
$Header: //components/rel/gnss8.mpss/11.1/gnss/sm/tm/src/tm_xspi.c#1 $
$Author: pwbldsvc $
$Created:$
$Modified:$
===========================================================================*/
...
void tm_xspi_handle_status_injection(pdsm_xspi_cmd_s_type *p_cmd)
{
   ...
   if(p_cmd)
   {
      ...
      {        
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"TM_XSPI: Reset Location Service in progress",0,0,0);
      ...
      else if(p_module_data && p_module_data->b_Enabled)
      {
         ...
         default:
            MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"unknown data motion state [%d]", p_cmd->xspi_data.q_state, 0, 0);
            p_sensor_data->q_Value = (uint32) C_MGP_EXTERNAL_DATA_MOTION_UNKNOWN;
            break;
      ...
      {
         MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"module data is NULL!", 0, 0, 0);
         cmd_status = PDSM_PD_CMD_ERR_INVALID_PARAM; // should not happen
      }
      else //!b_enabled
      {
         MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"module is not enabled", 0, 0, 0);
...
void tm_xspi_enable(void)
{
   ...
   MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"XSPI state = %d", (p_module_data ? p_module_data->b_Enabled : 0), 0, 0);
}
...
void tm_xspi_handle_injection_request(tm_xspi_request_type* p_reqData)
{
   ...
   if(p_reqData && p_module_data && p_module_data->b_Enabled)
   {
      ...
      case TM_XSPI_STREAM_STATUS_ENABLE:
         MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"TM_XSPI_STREAM_STATUS_ENABLE", 0, 0, 0);
      ...
      default:
         MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"TM_XSPI_STREAM_STATUS_DISABLE", 0, 0, 0);
         event_info.pd_event_data.pd_info_type.pd_info.xspi_data.q_stream_status =
      ...
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"TM_XSPI: raise injection request event", 0, 0, 0);
...
void tm_xspi_handle_timer_expiry(const os_TimerExpiryType *p_timerData)
{
  ...
  if(p_timerData && p_module_data)
  {
    switch(p_timerData->q_TimerId)
    {
      ...
      default:
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"TM_XSPI: Uknown timer expired", 0, 0, 0);
      break;
    }
  }
  else
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Can not process null timer msg", 0, 0, 0);
  }
...
void tm_xspi_handle_injection_timer_expiry(tm_xspi_module_data_type *p_module_data)
{
   if(p_module_data)
   {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"TM_XSPI: tm_xspi_handle_injection_timer_expiry()", 0, 0, 0);
      ...
      {
         /* inject cached data */
         MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"TM_XSPI: injecting unknown status to MGP", 0, 0, 0);
...
{
   MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"TM_XSPI: tm_xspi_handle_stream_timer_expiry()", 0, 0, 0);
   if(p_module_data && p_module_data->b_Enabled && p_module_data->b_TimerInjectionEnabled)
   {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"TM_XSPI: injecting cached data with confidence = %d, mode = %d",
...
void tm_xspi_init_and_read_nv(void)
{
   ...
   if(!p_module_data)
   {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"no memory to initialize module ", 0, 0, 0);
   } /* end else(p_module_data)*/
   else
   {
      if(!tm_xspi_init_module_data(p_module_data))
      {
         MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"unable to initialize module data", 0, 0, 0);
      }
...
void tm_xspi_stop_stream_timer(tm_xspi_module_data_type *p_module_data)
{
   if(p_module_data && p_module_data->p_timerStream)
   {
      ...
      {
         MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"unable to stop stream timer", 0, 0, 0);
      }
   }
...
void tm_xspi_start_stream_timer(tm_xspi_module_data_type *p_module_data)
{
   ...
   if(p_module_data && p_module_data->p_timerStream && p_module_data->q_StreamPeriod)
   {
      ...
      {
         MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"unable to start stream timer", 0, 0, 0);
      }
   }
...
void tm_xspi_start_injection_timer(tm_xspi_module_data_type *p_module_data)
{
   ...
   if(p_module_data && p_module_data->p_timerInjection && p_module_data->q_InjectionWaitPeriod)
   {
      ...
      {
         MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"unable to start injection timer", 0, 0, 0);
      }
   }
...
void tm_xspi_stop_injection_timer(tm_xspi_module_data_type *p_module_data)
{
   if(p_module_data && p_module_data->p_timerInjection)
   {
      ...
      {
         MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"unable to stop injection timer", 0, 0, 0);
      }
   }
