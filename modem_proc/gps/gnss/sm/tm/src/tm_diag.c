...
/*===========================================================================
  ...
  $Header: //components/rel/gnss8.mpss/11.1/gnss/sm/tm/src/tm_diag.c#3 $  
  $DateTime: 2020/09/08 23:56:58 $
  $Author: pwbldsvc $
...
/* Default supported IQ test mask (see e_CollectModeTypeVal for details)
   = ( (1 << COLLECT_MODE_WBIQ_GPS_L1) | \
       (1 << COLLECT_MODE_WBIQ_GLO_G1) | \
       (1 << COLLECT_MODE_NBIQ_GPS_L1_AUTO) | \
       (1 << COLLECT_MODE_NBIQ_GPS_L1_MANUAL) | \
       (1 << COLLECT_MODE_FBIQ_GLO_G1) | \
       (1 << COLLECT_MODE_L1_ADC_IQ) | \
       (1 << COLLECT_MODE_WBIQ_BDS_B1) | \
       (1 << COLLECT_MODE_WBIQ_GLO_G1) | \
       (1 << COLLECT_MODE_WBIQ_GAL_E1) )        \
*/
...
void tm_diag_process_rf_dev_cmd(void)
{
  ...
  switch (rf_dev_req_ptr->gps_control)
  {
    ...
    case 1: /* Provodie AA */
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "TM-Diag received AA from script", 0, 0, 0);
      ...
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR, "Attempting to over flow buffer", 0, 0, 0);
        return;
      }
    ...
    case 2: /* Provodie SA */
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "TM-Diag received SA from script", 0, 0, 0);
    ...
    case 5: /* Provodie LR */
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "TM-Diag received LR from script", 0, 0, 0);
    ...
    case 8: /* Provide ALM */
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "TM-Diag received ALM from script", 0, 0, 0);
    ...
    case 9: /* Provide EPH */
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "TM-Diag received EPH from script", 0, 0, 0);
    ...
    case 7: /* start/stop PPM/PRM */
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "TM-Diag received PPM/PRM control command = %x", rf_dev_req_ptr->supporting_data[0], 0, 0);
    ...
    case TM_DIAG_SESS_CGPS_SUB_COMMAND:
      ...
      switch (rf_dev_req_ptr->supporting_data[0])
      {
        case TM_DIAG_SESS_CGPS_START_SBAS_TEST_SUB_COMMAND:
          {
            ...
            MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "Turning ON SBAS test: SvMask 0x%x", q_SvMask, 0, 0);

            mgp_SetSbasCannedSvList(q_SvMask);

            /* Now turn on MGP */
            p_OnParams->e_OperationMode = MGP_OPERATION_SBAS_TESTMODE;
            p_OnParams->u_MeasRptPeriod   = 0;
        ...
        case TM_DIAG_SESS_CGPS_STOP_SBAS_TEST_SUB_COMMAND:
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "Turning OFF SBAS test", 0, 0, 0);
    ...
    case TM_DIAG_SESS_LTE_OTDOA_COMMAND:
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "LTE OTDOA DIAG command", 0, 0, 0);
    ...
    case TM_DIAG_SESS_LTE_FORCE_HYBRID_COMMAND:
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "LTE OTDOA force hybrid fix command %d %d",
    ...
    case TM_DIAG_LOWI_ENABLE_CMD:
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "LOWI diag command %d %d",
        ...
        switch (rf_dev_req_ptr->supporting_data[0])
        {
          ...
          case TM_DIAG_LOWI_DISCOVERY_SCAN_SUB_COMMAND:
            ...
            {
              lowi_queue_passive_scan_req(rf_dev_req_ptr->supporting_data[2],  // Number of requests
                                          rf_dev_req_ptr->supporting_data[3]); // Delay between requests
            }
            else
            {
              lowi_queue_discovery_scan_req(rf_dev_req_ptr->supporting_data[2],  // Mode (Forced fresh..)
                                            rf_dev_req_ptr->supporting_data[3],               // Band (2.4G..)
                                            rf_dev_req_ptr->supporting_data[4],               // Type (Active / Passive)
                                            rf_dev_req_ptr->supporting_data[5],               // Meas Filter
                                            rf_dev_req_ptr->supporting_data[6],               // Fallback tolerance
                                            rf_dev_req_ptr->supporting_data[7],               // Number of requests
                                            rf_dev_req_ptr->supporting_data[8]);                // Delay between requests
            }
            break;
          ...
          case TM_DIAG_LOWI_EVENTS_SUBSCRIPTION_CANCEL_SUB_COMMAND:
            ...
            break;
          case TM_DIAG_LOWI_RANGING_SUB_COMMAND:
            lowi_queue_ranging_req(rf_dev_req_ptr->supporting_data[1],   // Single sided 0, two sided 1
                                   rf_dev_req_ptr->supporting_data[2],   // Basic (0), Detailed results (1)
                                   rf_dev_req_ptr->supporting_data[3],   // Max Num of Nodes to range with
                                   rf_dev_req_ptr->supporting_data[4],   // Min RSSI
                                   rf_dev_req_ptr->supporting_data[5],   // Num of requests
                                   rf_dev_req_ptr->supporting_data[6]);  // Delay
            break;
          case TM_DIAG_LOWI_RANGING_SINGLE_SUB_COMMAND:
            lowi_queue_single_ranging_req(rf_dev_req_ptr->supporting_data[1],  // Single sided 0, two sided 1
                                          rf_dev_req_ptr->supporting_data[2],  // Basic (0), Detailed results (1)
                                          rf_dev_req_ptr->supporting_data[3],  // mac[0]
                                          rf_dev_req_ptr->supporting_data[4],  // mac[1]
                                          rf_dev_req_ptr->supporting_data[5],  // mac[2]
                                          rf_dev_req_ptr->supporting_data[6],  // mac[3]
                                          rf_dev_req_ptr->supporting_data[7],  // mac[4]
                                          rf_dev_req_ptr->supporting_data[8],  // mac[5]
                                          rf_dev_req_ptr->supporting_data[9],  // Num of request
                                          rf_dev_req_ptr->supporting_data[10]);  // Delay
            break;
          default:
            break;
        }
  ...
  if (data != NULL)
  {
    ...
    switch (diag_cmd)
    {
      ...
      case TM_DIAG_SESS_POST_DATA_COMMAND:
        ...
        {
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR, "Post Data Succeeded", 0, 0, 0);
        }
        else
        {
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR, "Posd Data Failed", 0, 0, 0);
        }
        break;
      ...
      case TM_DIAG_SESS_POLLING_PPM_COMMAND:
        ...
        if (tm_sess_req(TM_PRTL_TYPE_DIAG,
                        ...
                        &tm_diag_sess_req_param))
        {
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR, "Polling PPM Succeeded", 0, 0, 0);
        }
        else
        {
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR, "Polling PPM Failed", 0, 0, 0);
        }
      ...
      case TM_DIAG_PORT_CONTROL_COMMAND:
        ...
        {
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "Trying to open NMEA port %d", tm_diag_nmea_cmd_info.port_to_open, 0, 0);
          tm_nmea_diag_open_port(tm_diag_nmea_cmd_info.port_to_open);
        }
        else if (tm_diag_nmea_cmd_info.operation == 0)
        {
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "Closing NMEA port", 0, 0, 0);
   ...
   )
{
  ...
  if (NULL == req_pkt_ptr)
  {
    ...
    return ((void*)diagpkt_err_rsp(DIAG_BAD_PARM_F, req_pkt_ptr, pkt_len));
  }
...
PACKED void* tm_diag_sarf_cmd_handler(PACKED void *p_PktReq, uint16 w_PktLength)
{
  ...
  {
    /* Return a diag error packet indicating bad parameters were passed */
    return ((void*)diagpkt_err_rsp(DIAG_BAD_PARM_F, p_PktReq, w_PktLength));
  }

  if (p_Req->z_DiagCmdCommon.u_GpsCmdCode >= TM_DIAG_GPS_CMD_CODE_LAST)
  {
    return ((void*)diagpkt_err_rsp(DIAG_BAD_CMD_F, p_PktReq, w_PktLength));
  }
  ...
  else if (lm_dstn_ipc_msg_id == ARIES_MSG_ID_TM_LM_IQ_TEST)
  {
      ...
      if (p_CommonRsp != NULL)
      {
        ...
        p_CommonRsp->q_ConstellationMask = tm_diag_rfdev_track_constellation_mask;
         MSG_1(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,
               "SARF RfDevTrackCapabilities: RF Dev Constellation Mask 0x%08X",
                tm_diag_rfdev_track_constellation_mask);
      }
      return (p_CommonRsp);
    }
    else
    { /* IQ test is not supported */
      ...
      return ((void*)diagpkt_err_rsp(DIAG_BAD_MODE_F, p_PktReq, w_PktLength));
    }
  }
  else if (lm_dstn_ipc_msg_id == ARIES_MSG_ID_TM_LM_GET_LATEST_WB_FFT_RESULTS)
  {
    ...
    if (p_RFDevWbiqRsp != NULL)
    {
      ...
      if (tm_diag_wbiq_info.q_GnssConfig & C_RFDEV_CONFIG_GPS_L1_ENABLED)
      {
      ...
      else if (tm_diag_wbiq_info.q_GnssConfig & C_RFDEV_CONFIG_GLO_G1_ENABLED)
      {
      ...
      else if (tm_diag_wbiq_info.q_GnssConfig & C_RFDEV_CONFIG_BDS_B1_ENABLED)
      {
      ...
      else if (tm_diag_wbiq_info.q_GnssConfig & C_RFDEV_CONFIG_GAL_E1_ENABLED)
      {
        ...
        p_RFDevWbiqRsp->l_Freq = (int32)tm_diag_wbiq_info.f_GalE1FreqHz;
                  ...
                  (C_RFDEV_CONFIG_GPS_L5_ENABLED | 
                   C_RFDEV_CONFIG_GAL_E5A_ENABLED |
                   C_RFDEV_CONFIG_BDS_B2A_ENABLED))
      {
      ...
      else if (tm_diag_wbiq_info.q_MultiBandConfig & C_RFDEV_CONFIG_GAL_E5B_ENABLED)
      {
        p_RFDevWbiqRsp->q_Cno = (uint32)(10 * tm_diag_wbiq_info.f_GalE5bCNoDBHz);
        p_RFDevWbiqRsp->l_Freq = (int32)tm_diag_wbiq_info.f_GalE5bFreqHz;
      }
      else if (tm_diag_wbiq_info.q_MultiBandConfig & C_RFDEV_CONFIG_NAVIC_ENABLED)
      {
        p_RFDevWbiqRsp->q_Cno = (uint32)(10 * tm_diag_wbiq_info.f_NavicCNoDBHz);
        p_RFDevWbiqRsp->l_Freq = (int32)tm_diag_wbiq_info.f_NavicFreqHz;
      }
      else if (tm_diag_wbiq_info.q_MultiBandConfig & C_RFDEV_CONFIG_GPS_L2_ENABLED)
      {
        p_RFDevWbiqRsp->q_Cno = (uint32)(10 * tm_diag_wbiq_info.f_GpsL2CNoDBHz);
        p_RFDevWbiqRsp->l_Freq = (int32)tm_diag_wbiq_info.f_GpsL2FreqHz;
      }
      else
   ...
   )
{
  ...
  if (cmd_ptr != NULL)
  {
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "Received NMEA Port Control Command", 0, 0, 0);
...
void gpsdiag_EfsFileRespHandler(uint32 q_FileOperationResult,
                                ...
                                uint8 u_FileOp)
{
  gpsdiag_EfsFileSysRspType *delayed_rsp_ptr = NULL;
  ...
  if ((u_FileOp > GPS_FS_DELETE_FILE_LIST) ||
      ((w_pathfilelen == 0) || (w_pathfilelen >= C_GPSDIAG_MAX_FULL_FILENAME_LENGTH)))
  {
    MSG_2(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,
          "gpsdiag_EfsFileRespHandler Invalid Params: FileOp:%d, PathFileLen:%d",
          u_FileOp, w_pathfilelen);
    return;
  }

  rsp_len = sizeof(gpsdiag_EfsFileSysRspType);
  ...
  if (delayed_rsp_ptr != NULL)
  {
    ...
    /* Fill the response file operation */
    delayed_rsp_ptr->u_FileOperation = u_FileOp;

    /* Initialize and fill response pathfilename */
    memset(&delayed_rsp_ptr->u_DirectoryAndFilename, 0, C_GPSDIAG_MAX_FULL_FILENAME_LENGTH);
    memscpy((void *)&delayed_rsp_ptr->u_DirectoryAndFilename,
            sizeof(delayed_rsp_ptr->u_DirectoryAndFilename),
            (void *)&u_PathFilename[0], w_pathfilelen);

    /* Commit response buffer */
    diagpkt_delay_commit(delayed_rsp_ptr);
  }
  else
  {
    MSG(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,
        "gpsdiag_EfsFileRespHandler failed allocating Resp buffer");
  }
}
   ...
   )
{
  ...
  {
    return ((void*)diagpkt_err_rsp(DIAG_BAD_PARM_F, p_PktReq, pkt_len));
  }
                                           ...
                                           word w_PktLength)
{
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR, "gpsdiag_DataDumpCmdHandler: Rsp packet is NULL!!", 0, 0, 0);
  }
...
void tm_api_nav_config_change_resp_handler(uint32 q_CurrGnssConfig)
{
  gpsdiag_NavConfigRspType *p_DelayedRsp = NULL;
  /* Map Gnsscfg to 8-bit values for Rfdev tools compatibility */
  uint8 u_CurrGnssCfgRfdev = gnss_Convert32BitGnssCfgTo8BitRfdevCfg(q_CurrGnssConfig);
  ...
  if (p_DelayedRsp != NULL)
  {
    ...
    if (z_NavConfigCommand.u_GnssDesiredConfig == u_CurrGnssCfgRfdev)
    {
    ...
    {
      MSG_2(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,
            "Requested Rf Config 0x%x, Actual Config 0x%x",
            z_NavConfigCommand.u_GnssDesiredConfig, u_CurrGnssCfgRfdev);
    ...
    p_DelayedRsp->u_GnssSupportedConfig = z_NavConfigCommand.u_HwGnssSupportedConfig &
          (C_RCVR_GNSS_CONFIG_GPS_ENABLED |
           ...
           C_RCVR_GNSS_CONFIG_GAL_ENABLED_RFDEV );
    p_DelayedRsp->u_GnssActualConfig =  gnss_Convert32BitGnssCfgTo8BitRfdevCfg(q_CurrGnssConfig &
          (C_RCVR_GNSS_CONFIG_GPS_ENABLED |
           C_RCVR_GNSS_CONFIG_GLO_ENABLED |
           C_RCVR_GNSS_CONFIG_BDS_ENABLED_WORLDWIDE |
    ...
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,
        "Sending delayed response for Nav config change cmd 0x%x 0x%x", 
          p_DelayedRsp->u_GnssSupportedConfig,p_DelayedRsp->u_GnssActualConfig, 0 );
                                     ...
                                     gpsdiag_NavConfigRspType *p_ImmedRsp)
{
  ...
  uint32 q_GnssActualConfig;
  uint8  u_GnssActualConfigRfdev;
  uint32 q_HwGnssSupportedConfig;
  uint8  u_HwGnssSupportedConfigRfdev;
  uint32 q_GnssDesiredConfig;
  uint8  u_GnssDesiredConfigRfdev;
  ...
  if ((p_Req != NULL) && (p_ImmedRsp != NULL))
  {
    ...
    mgp_GetActiveGnssRcvrConfiguration(&q_GnssActualConfig);
    u_GnssActualConfigRfdev = 
      gnss_Convert32BitGnssCfgTo8BitRfdevCfg(q_GnssActualConfig);

    /* Obtain the HWsupported receiver configuration */
    mgp_GetHwSupportedRcvrConfiguration(&q_HwGnssSupportedConfig);
    u_HwGnssSupportedConfigRfdev = 
      gnss_Convert32BitGnssCfgTo8BitRfdevCfg(q_HwGnssSupportedConfig);

    u_GnssDesiredConfigRfdev = p_Req->u_GnssDesiredConfig;
    q_GnssDesiredConfig = 
      gnss_Convert8BitRfdevCfgTo32BitGnssCfg(u_GnssDesiredConfigRfdev);

    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "GNSS Nav config 13 (Rfdev_fmt) 0x%2x, 0x%2x, 0x%2x ", u_GnssActualConfigRfdev,
          p_Req->u_GnssDesiredConfig, u_HwGnssSupportedConfigRfdev);
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "GNSS Nav config 13 (MGP_fmt), 0x%x, 0x%x, 0x%x ", q_GnssActualConfig,
          q_GnssDesiredConfig, q_HwGnssSupportedConfig);

    /* Fill out the response */
    p_ImmedRsp->u_GnssSupportedConfig = 
      gnss_Convert32BitGnssCfgTo8BitRfdevCfg( q_HwGnssSupportedConfig &
          (C_RCVR_GNSS_CONFIG_GPS_ENABLED |
           C_RCVR_GNSS_CONFIG_GLO_ENABLED |
           C_RCVR_GNSS_CONFIG_BDS_ENABLED_WORLDWIDE |
           C_RCVR_GNSS_CONFIG_GAL_ENABLED ));
    p_ImmedRsp->u_GnssActualConfig =  
      gnss_Convert32BitGnssCfgTo8BitRfdevCfg(q_GnssActualConfig &
          (C_RCVR_GNSS_CONFIG_GPS_ENABLED |
           C_RCVR_GNSS_CONFIG_GLO_ENABLED |
           C_RCVR_GNSS_CONFIG_BDS_ENABLED_WORLDWIDE |
    ...
    MSG_2(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "GNSS Nav config 13 rsp (Rfdev_fmt) 0x%x, 0x%x", 
          p_ImmedRsp->u_GnssSupportedConfig,
          p_ImmedRsp->u_GnssActualConfig);
     ...
     */
    if ((q_GnssDesiredConfig == 0) ||
        (q_GnssDesiredConfig == q_GnssActualConfig))
    {
      /* Nothing more to follow */
    ...
    else
    {
      ...
      z_NavConfigCommand.u_HwGnssSupportedConfig = u_HwGnssSupportedConfigRfdev;

      /* Send the config to MGP through LM */
      z_NavConfig.q_GnssDesiredConfig = q_GnssDesiredConfig;
                                                   ...
                                                   uint16 w_PktLength)
{
  ...
  else
  {
    ...
    if (lm_send_sarf_cmd((uint8 *)&(p_Req->u_Cmd), ARIES_MSG_ID_TM_LM_NAV_SET_TEST_MODE_CMD) == FALSE)
    {
      /* Set the status in the response, 0 => Success */
      diagpkt_subsys_set_status(&(p_Rsp->z_DiagCmdHdrV2), 0);
    }
...
PACKED void* tm_diag_factory_iq_test_cmd_handler(PACKED void *p_PktReq,
                                                 uint16 w_PktLength)
{
  gpsdiag_FactoryIqTestReqType *p_Req = (gpsdiag_FactoryIqTestReqType *)p_PktReq;
  gpsdiag_FactoryIqTestImmedRspType *p_ImmedRsp = NULL;
  ...
  {
    /* Allocate space through diag for response packet */
    p_ImmedRsp = (gpsdiag_FactoryIqTestImmedRspType *)
       diagpkt_subsys_alloc_v2((diagpkt_subsys_id_type)DIAG_SUBSYS_GPS,
                               TM_DIAG_FACTORY_IQ_TEST_CMD,
                               sizeof(gpsdiag_FactoryIqTestImmedRspType));
...
/*===========================================================================
FUNCTION tm_diag_pfa_test_cmd_handler

DESCRIPTION
  This function handles the Probability of False Alarm (PFA) Test Command
...
SIDE EFFECTS
  None.
===========================================================================*/
PACKED void* tm_diag_pfa_test_cmd_handler(PACKED void *p_PktReq,
                                          uint16 w_PktLength)
{
  gpsdiag_PfaTestReqType *p_Req = (gpsdiag_PfaTestReqType *)p_PktReq;
  gpsdiag_PfaTestImmedRspType *p_ImmedRsp = NULL;
  ...
  /* Allocate space through diag for response packet */
  p_ImmedRsp = (gpsdiag_PfaTestImmedRspType *)
     diagpkt_subsys_alloc_v2((diagpkt_subsys_id_type)DIAG_SUBSYS_GPS,
                             TM_DIAG_PFA_TEST_CMD,
                             sizeof(gpsdiag_PfaTestImmedRspType));

  /* PFA test request will fill in the immediate response */
  tm_diag_PfaTestRequest(p_Req, p_ImmedRsp);
...
/*===========================================================================
FUNCTION tm_diag_get_pfa_results_cmd_handler

DESCRIPTION
  This function handles the Probability of False Alarm (PFA) Results
  request command
...
SIDE EFFECTS
  None.
===========================================================================*/
PACKED void* tm_diag_get_pfa_results_cmd_handler(PACKED void *p_PktReq,
                                                 uint16 w_PktLength)
{
  gpsdiag_GetPfaResultsRspType *p_Rsp = NULL;
  ...
  p_Rsp = (gpsdiag_GetPfaResultsRspType *)
     diagpkt_subsys_alloc_v2((diagpkt_subsys_id_type)DIAG_SUBSYS_GPS,
                             TM_DIAG_GET_PFA_RESULTS_CMD,
                             sizeof(gpsdiag_GetPfaResultsRspType));

  /* PFA test submodule will fill in the response */
  tm_diag_PfaGetResults(p_Rsp);
...
/*===========================================================================
FUNCTION tm_diag_offline_cxm_test_cmd_handler

DESCRIPTION
  This function is used to send a request to start the Offline CXM test.
...
SIDE EFFECTS
  None.
===========================================================================*/
PACKED void* tm_diag_offline_cxm_test_cmd_handler(PACKED void *p_PktReq,
                                                  uint16 w_PktLength)
{
  gpsdiag_OfflineCxmTestReqType *p_Req =
     (gpsdiag_OfflineCxmTestReqType *)p_PktReq;
  gpsdiag_OfflineCxmTestRspType *p_Rsp = NULL;
  ...
  if (p_Req == NULL)
  {
    ...
    MSG(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,
        "tm_diag_offline_cxm_test_cmd_handler: p_Req is NULL!!");
    /* Return a diag error packet indicating bad parameters were passed */
    return ((void*)diagpkt_err_rsp(DIAG_BAD_PARM_F, p_PktReq, w_PktLength));
  }

  /* Allocate space through diag for response packet */
  p_Rsp = (gpsdiag_OfflineCxmTestRspType *)
     diagpkt_subsys_alloc_v2((diagpkt_subsys_id_type)DIAG_SUBSYS_GPS,
                             TM_DIAG_OFFLINE_CXM_TEST_CMD,
                             sizeof(gpsdiag_OfflineCxmTestRspType));

  /* Fill in response packet */
  if (p_Rsp == NULL)
  {
    MSG(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,
        "tm_diag_offline_cxm_test_cmd_handler: Rsp packet is NULL!!");
  }
  else
  {
    /* Set the first (0) response count */
                                                    ...
                                                    uint16 w_PktLength)
{
  ...
  else
  {
    ...
    p_Rsp->u_Version = C_GPSDIAG_EXT_WBFFT_RESULTS_RSP_VER;
    ...
    p_Rsp->u_GnssConfig = 
      gnss_Convert32BitGnssCfgTo8BitRfdevCfg(tm_diag_wbiq_info.q_GnssConfig);
    ...
    p_Rsp->q_GalE5bCnoP1DBHz =
       (uint32)(10 * tm_diag_wbiq_info.f_GalE5bCNoDBHz);
    p_Rsp->l_GalE5bFreqHz = (int32)tm_diag_wbiq_info.f_GalE5bFreqHz;
    p_Rsp->q_NavicCnoP1DBHz =
       (uint32)(10 * tm_diag_wbiq_info.f_NavicCNoDBHz);
    p_Rsp->l_NavicFreqHz = (int32)tm_diag_wbiq_info.f_NavicFreqHz;
    p_Rsp->q_GpsL2CnoP1DBHz =
       (uint32)(10 * tm_diag_wbiq_info.f_GpsL2CNoDBHz);
    p_Rsp->l_GpsL2FreqHz = (int32)tm_diag_wbiq_info.f_GpsL2FreqHz;
  }
...
/*===========================================================================
FUNCTION tm_diag_jammermeas_cmd_handler

DESCRIPTION
  This function is used to send the diag command to control the BP based
  Jammer module to NAVRX.
...
SIDE EFFECTS
  None.
===========================================================================*/
PACKED void* tm_diag_jammermeas_cmd_handler(PACKED void *p_PktReq,
                                            uint16 w_PktLength)
{
  gpsdiag_NavBPJammerCtlReqType *p_Req = (gpsdiag_NavBPJammerCtlReqType *)p_PktReq;
  gpsdiag_NavBPJammerCtlRspType *p_Rsp = NULL;
  lm_NavBpJammerCtlType z_NavBPJammerCtl = { 0 };
  ...
  /* Allocate space through diag for response packet */
  p_Rsp = (gpsdiag_NavBPJammerCtlRspType *)
     diagpkt_subsys_alloc_v2((diagpkt_subsys_id_type)DIAG_SUBSYS_GPS,
                             TM_DIAG_JAMMER_MEAS_TEST_CMD,
                             sizeof(gpsdiag_NavBPJammerCtlRspType));

  /* Fill in response packet */
  if (p_Rsp == NULL)
  {
    MSG(MSG_SSID_GPSSM, MSG_LEGACY_ERROR, "tm_diag_jammermeas_cmd_handler: Rsp packet is NULL!!");
  }
  else
  {
    /* Set the first (0) response count */
    ...
    /*Set the request params*/
    z_NavBPJammerCtl.u_RequestCode = p_Req->u_RequestCode;

    /*Send the config to LM */
    if (lm_send_sarf_cmd((uint8 *)&z_NavBPJammerCtl, ARIES_MSG_ID_TM_LM_BP_JAMMER_CTRL_CMD) == TRUE)
    {
      diagpkt_subsys_set_status(&(p_Rsp->z_DiagCmdHdrV2), 1);
    }
    else
...
static const diagpkt_user_table_entry_type gpsdiag_SubsysCmdTable_v2[] =
{
  ...
  { TM_DIAG_PFA_TEST_CMD, TM_DIAG_PFA_TEST_CMD, tm_diag_pfa_test_cmd_handler },
  { TM_DIAG_GET_PFA_RESULTS_CMD, TM_DIAG_GET_PFA_RESULTS_CMD, tm_diag_get_pfa_results_cmd_handler },
  { TM_DIAG_EXT_WB_FFT_RESULTS_CMD, TM_DIAG_EXT_WB_FFT_RESULTS_CMD, tm_diag_ext_wb_fft_results_cmd_handler },
  { TM_DIAG_OFFLINE_CXM_TEST_CMD, TM_DIAG_OFFLINE_CXM_TEST_CMD, tm_diag_offline_cxm_test_cmd_handler },
  { TM_DIAG_JAMMER_MEAS_TEST_CMD, TM_DIAG_JAMMER_MEAS_TEST_CMD, tm_diag_jammermeas_cmd_handler },
  ...
  if (session_handle == TM_SESS_HANDLE_FTEST)
  {
    switch (sess_info_type)
    {
      ...
      case PRTL_SESS_INFO_PRM:
        if ( sess_info_param->prm_data.z_MeasBlk.z_GnssHeader.e_GnssMeasRptreason == GNSS_MEAS_RPT_MEAS_DONE_SM )
        {
          if (tm_diag_in_ftest_mode == TRUE) /*lint !e506  !e731 */
          {
            ...
            else
            {
              ...
              MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "Caching PRM", 0, 0, 0);
      ...
      case PRTL_SESS_INFO_STATUS:
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "Got Aiding Data Wishlist", 0, 0, 0);
        break;
   ...
   )
{
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR, "Fail to get memory for aiding_data_ptr", 0, 0, 0);
    return FALSE;
  }
  ...
  for (i = 0; i < pddm_info.num_pddm; i++)
  {
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "Ignore IS801 requests for TM Diag", 0, 0, 0);
    }
...
{
  uint32 q_PrimaryGnssCfgNv = 0;
  uint32 q_MultibandCfgNv = mgp_NvEfsReadGnssMultibandConfig();

  (void)mgp_GnssGetConstellationConfig(&q_PrimaryGnssCfgNv,NULL);

  /* Build the Rfdev track capabilites config in the format needed for Rfdev
     tools. Start with GPS enabled */
  tm_diag_rfdev_track_constellation_mask = (1 << MultiChan_Track_L1_ENABLED);
  if (q_PrimaryGnssCfgNv & C_RCVR_GNSS_CONFIG_GLO_ENABLED)
  {
    tm_diag_rfdev_track_constellation_mask |= (1 << MultiChan_Track_G1_ENABLED);
  }
  if (q_PrimaryGnssCfgNv & C_RCVR_GNSS_CONFIG_BDS_ENABLED)
  {
    tm_diag_rfdev_track_constellation_mask |= (1 << MultiChan_Track_B1_ENABLED);
  }
  if (q_PrimaryGnssCfgNv & C_RCVR_GNSS_CONFIG_GAL_ENABLED)
  {
    tm_diag_rfdev_track_constellation_mask |= (1 << MultiChan_Track_E1_ENABLED);
  }
  if (q_MultibandCfgNv & C_RCVR_GNSS_MULTIBAND_CONFIG_GPS_L5_ENABLED)
  {
    tm_diag_rfdev_track_constellation_mask |= (((uint32)1) << MultiChan_Track_L5_ENABLED);
  }

  if (q_MultibandCfgNv & C_RCVR_GNSS_MULTIBAND_CONFIG_GAL_E5A_ENABLED)
  {
    tm_diag_rfdev_track_constellation_mask |= (((uint32)1) << MultiChan_Track_E5a_ENABLED);
  }

  if (q_MultibandCfgNv & C_RCVR_GNSS_MULTIBAND_CONFIG_BDS_B2A_ENABLED)
  {
    tm_diag_rfdev_track_constellation_mask |= (((uint32)1) << MultiChan_Track_B2a_ENABLED);
  }
  ...
  if (p_ReportWBIQInfo != NULL)
  {
    ...
    if (z_tm_diag_factory_iq_test.v_FactoryIqTestRequested == TRUE)
    {
      ...
      if (p_DelayedRsp != NULL)
      {
        ...
        p_DelayedRsp->u_GnssConfig = gnss_Convert32BitGnssCfgTo8BitRfdevCfg(p_ReportWBIQInfo->q_GnssConfig);
        ...
        p_DelayedRsp->q_MultiBandConfig = p_ReportWBIQInfo->q_MultiBandConfig;
        ...
        p_DelayedRsp->q_GalE5bCnoP1DBHz =
           ((uint32)(10.0 * p_ReportWBIQInfo->f_GalE5bCNoDBHz));
        p_DelayedRsp->l_GalE5bFreqHz = 
          ((int32)p_ReportWBIQInfo->f_GalE5bFreqHz);
        p_DelayedRsp->q_NavicCnoP1DBHz =
           ((uint32)(10.0 * p_ReportWBIQInfo->f_NavicCNoDBHz));
        p_DelayedRsp->l_NavicFreqHz =
           ((int32)p_ReportWBIQInfo->f_NavicFreqHz);
        p_DelayedRsp->q_GpsL2CnoP1DBHz =
           ((uint32)(10.0 * p_ReportWBIQInfo->f_GpsL2CNoDBHz));
        p_DelayedRsp->l_GpsL2FreqHz =
           ((int32)p_ReportWBIQInfo->f_GpsL2FreqHz);
