...
/*=============================================================================
  ...
  $Header: //components/rel/gnss8.mpss/11.1/gnss/sm/tm/umts_common/src/tm_umts_common_utils.c#3 $
  $DateTime: 2020/01/29 21:37:54 $
  $Author: pwbldsvc $
  ...
  11/02/17   sj      Remove ECBM state, rely on CM MSG Router Emergency Mode status messages
  10/30/17   nath    Updated alongAxisUnc and perpAxisUnc to float
...
===========================================================================*/
boolean tm_umts_common_glonass_enabled(void)
{
#ifdef FEATURE_CGPS_UMTS_UP_SUPL
  
  if (C_RCVR_GNSS_CONFIG_GLO_ENABLED & sm_get_supported_constellation_mask())
  {
    return TRUE;
  }
  else
...
/*=========================================================================== 
FUNCTION tm_umts_common_bds_enabled
...
RETURN VALUE
  ...
  True if BDS is enabled.
    
SIDE EFFECTS

===========================================================================*/
boolean tm_umts_common_bds_enabled(void)
{
  #ifdef FEATURE_CGPS_UMTS_UP_SUPL
  #ifdef FEATURE_ABDS_RRLP_SUPL
  if (C_RCVR_GNSS_CONFIG_BDS_ENABLED & sm_get_supported_constellation_mask())
  {
    return TRUE;
  }
  else
 ...
 ******************************************************************************
 *
 * tm_umts_common_utils_read_from_hw_sub
 *
 * Function description:
 *  This function reads UMTS NVs from HW or EFS and cache them. If NV is carrier-specific
 *  it will be read from sub param , else from default sub 0.
 *
 * Parameters:
 *   sub id : from which NV has to be read.
 *
 * Return value: 
 *   None.
 *
...
void tm_umts_common_utils_read_from_hw_sub(sys_modem_as_id_e_type sub)
{
  uint32 q_aags_sha_tls;
  uint32 q_nv_item_value = 0;

  /* NV #04703 UMTS PDE Server Address URL  */
  (void)sm_nv_read_from_hw_sub_and_cache(NV_CGPS_UMTS_PDE_SERVER_ADDR_URL_I, (void *)&z_tm_umts_nv_read_buffer,sub);
  /* NV #04701 Fetch the HSLP IPv4 Address */
  (void)sm_nv_read_from_hw_sub_and_cache( NV_CGPS_UMTS_PDE_SERVER_ADDR_IPV4_I, (void *)&z_tm_umts_nv_read_buffer, sub);
  /* NV #4702 Fetch the HSLP IPv6 Address */
  (void)sm_nv_read_from_hw_sub_and_cache( NV_CGPS_UMTS_PDE_SERVER_ADDR_IPV6_I, (void *)&z_tm_umts_nv_read_buffer, sub);
  /* NV #6792  SUPL Version: Bytes from MSB to LSB: rsvd, major, minor, serviceIndicator  */
  (void)sm_nv_read_from_hw_sub_and_cache( NV_GNSS_SUPL_VERSION_I, (void *)&z_tm_umts_nv_read_buffer, sub);

  (void)sm_nv_efs_reg_item_read_from_sub(GNSS_NV_EFS_SM_TM_SHA_TLS_SUPPORT,sizeof(uint32),&q_aags_sha_tls,
                                                      sub);

  /* NV #3758 AGPS Transport Security ID  */
  (void)sm_nv_read_from_hw_sub_and_cache( NV_AAGPS_USE_TRANSPORT_SECURITY_I, (void *)&z_tm_umts_nv_read_buffer, sub );
  
  (void)sm_nv_efs_reg_item_read_from_sub(GNSS_NV_EFS_SM_EMERGENCY_EXTENSION_WINDOW, sizeof(uint32),&q_nv_item_value, sub);

  (void)sm_nv_efs_reg_item_read_from_sub(GNSS_NV_EFS_SM_GNSS_LOCK_CTRL_FOR_OEM, sizeof(uint32),&q_nv_item_value, sub);
}
/*
 ******************************************************************************
 *
 * tm_umts_common_utils_read_nv_from_cache
 *
 * Function description:
 *  This function reads UMTS NVs from SM NV cache. If NV is carrier-specific
 *  it will be read from sm_nv_current_sub , else from default sub 0.
 *
 * Parameters:
 *   None.
 *
...
void tm_umts_common_utils_read_nv_from_cache(void)
{
#ifdef FEATURE_CGPS_UMTS_UP_SUPL
    ...
    /* NV #04703 UMTS PDE Server Address URL  */
    (void)sm_nv_read_ex(NV_CGPS_UMTS_PDE_SERVER_ADDR_URL_I, (void *)&z_tm_umts_nv_read_buffer);
    {
    ...
    } /*End of read of NV #04703 */
  
    /* NV #04701 Fetch the HSLP IPv4 Address */
    (void)sm_nv_read_ex( NV_CGPS_UMTS_PDE_SERVER_ADDR_IPV4_I, (void *)&z_tm_umts_nv_read_buffer);
    z_umts_nv_item_ram_copy.slp_ip_addr = z_tm_umts_nv_read_buffer.cgps_umts_pde_server_addr_ipv4;
  
    /* NV #4702 Fetch the HSLP IPv6 Address */
    (void)sm_nv_read_ex( NV_CGPS_UMTS_PDE_SERVER_ADDR_IPV6_I, (void *)&z_tm_umts_nv_read_buffer);
    ...
    /* NV #6792  SUPL Version: Bytes from MSB to LSB: rsvd, major, minor, serviceIndicator  */
    (void)sm_nv_read_ex( NV_GNSS_SUPL_VERSION_I, (void *)&z_tm_umts_nv_read_buffer);
    z_umts_nv_item_ram_copy.gnss_supl_version = z_tm_umts_nv_read_buffer.gnss_supl_version;
  
    /* NV #3758 AGPS Transport Security ID  */
    (void)sm_nv_read_ex( NV_AAGPS_USE_TRANSPORT_SECURITY_I, (void *)&z_tm_umts_nv_read_buffer );
    ...
    if( FALSE == ( sm_nv_efs_reg_item_read_ex(GNSS_NV_EFS_SM_TM_SHA_TLS_SUPPORT, sizeof(uint32),&q_aags_sha_tls)))
    {
      z_umts_nv_item_ram_copy.aagps_sha_tls_support = GNSS_NV_EFS_SM_TM_SHA_TLS_SUPPORT_DEFAULT;
    }
    else
    {
      z_umts_nv_item_ram_copy.aagps_sha_tls_support = q_aags_sha_tls;
    }
  
    if( FALSE == ( sm_nv_efs_reg_item_read_ex(GNSS_NV_EFS_SM_EMERGENCY_EXTENSION_WINDOW, sizeof(uint32),
          &q_nv_item_value)))
    {
      z_umts_nv_item_ram_copy.q_emergency_extension_window = GNSS_NV_EFS_SM_EMERGENCY_EXTENSION_WINDOW_DEFAULT;
    }
    else
    {
      z_umts_nv_item_ram_copy.q_emergency_extension_window = q_nv_item_value;
    }

    if( FALSE == ( sm_nv_efs_reg_item_read_ex(GNSS_NV_EFS_SM_GNSS_LOCK_CTRL_FOR_OEM, sizeof(uint32),
          &q_nv_item_value)))
    {
      z_umts_nv_item_ram_copy.q_gnss_lock_ctrl_for_oem = GNSS_NV_EFS_SM_GNSS_LOCK_CTRL_FOR_OEM_DEFAULT;
    }
    else
    {
      z_umts_nv_item_ram_copy.q_gnss_lock_ctrl_for_oem = q_nv_item_value;
    }
 ...
 ******************************************************************************
*/
void tm_umts_common_utils_read_carrier_specific_nv (boolean read_from_hw)
{
  sys_modem_as_id_e_type e_sub; 

  if(read_from_hw == TRUE)
  {
    for(e_sub = SM_CONFIG_MIN_SUB_ID; e_sub <= sm_nv_max_sub; ++e_sub)
    {
      tm_umts_common_utils_read_from_hw_sub(e_sub);
    }
  }
...
void tm_umts_common_utils_read_nv_at_bootup (void)
{
  ...
  tm_umts_common_utils_read_carrier_specific_nv(TRUE);
 ...
 ******************************************************************************
 *
 * tm_umts_common_utils_update_config_for_sub
 *
 * Function description:
 *  This function reads UMTS NVs from cache with given sub id.
 *
 * Parameters:
 *   None.
 *
...
void tm_umts_common_utils_update_config_for_sub(sys_modem_as_id_e_type sub)
{
  sys_modem_as_id_e_type original_current_sub = sm_nv_current_sub;
  /*Temporary move current_sub to incoming sub, so that while reading from cache,
    this new sub id is used.*/
  sm_nv_current_sub = sub;
  
  tm_umts_common_utils_read_nv_from_cache();

  sm_nv_current_sub = original_current_sub;
}
...
static boolean tm_umts_common_utils_convert_to_default_conv(
   const sm_FixInfoStructType *pz_lr_data,
   uint8 *u_AlongUncK,
   uint8 *u_PerpUncK)
{
...
(
  ...
  const sm_FixInfoStructType*   const lr_data_ptr,
 ...
 ******************************************************************************
 *
 * tm_umts_common_utils_emergency_on
 *
 * Function description: This function would checks if phone indcates emergency mode or emergency
 * simulation NV is set to enabled.
 *
 * Parameters:
 *   None
 *
 * Return value:
 * True if UE is in emergency mode. Otherwise false.
 *
 ******************************************************************************
*/
boolean tm_umts_common_utils_emergency_on( void )
{
  return ( tm_umts_common_utils_cm_emergency_call_status() || tm_agps_emergency_call_simulate() );
}
...
emergency_handling_e_type tm_umts_common_utils_emergency_effect ( )
{
  ...
  if ( tm_umts_common_utils_emergency_on() )
  {
    e_e911_effect = C_EMERGENCY_HANDLING;     
  }
...
)
{
  ...
  if (p_assist_data_wishlist->q_WishListMask & C_GPS_ASSIS_NAVMODL )
  {
    ...
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "Target eph. inc. %u, nSvs: %u", 
...
)
{
  ...
  */
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "Ephemeris Wishlist GLONASS:%u",
  ...
  if (b_wishlist_eph)
  {
    ...
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "GLONASS Target eph. inc.:%u, nSvs.:%u",  
...
)
{
  ...
  *pd_event_ptr = PDSM_PD_EVENT_POSITION;
  ...
  else /* if-polygon */
  {
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"MOLR-RR-Polygon 0 pnt", 0, 0, 0);
      return FALSE;
    }
    else if ( ( u_octet1_low_nibble == 1 ) || ( u_octet1_low_nibble == 2 ) )
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"MOLR-RR-Polygon pnt %u", u_octet1_low_nibble, 0, 0);
    }
    else if ( u_octet1_low_nibble > 3 )
    {
      /* QQQ: polygon with N>3, use MOLR-ReturnResult with AdditionalPosition */
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"MOLR-RRpolygon>3 - forcing 3", 0, 0, 0);
  ...
  else if ( q_shape_code  == C_POS_SHAPE_ELLIP_UNC_CIR )
  {
    ...
    pos_data_ptr->ext_pos_data.f_alongAxisUnc = 
      f_unc_meters;
    pos_data_ptr->ext_pos_data.f_perpAxisUnc = 
      f_unc_meters;
  ...
  else if ( q_shape_code == C_POS_SHAPE_ELLIP_ARC )
  {
      ...
      /* ext_pos_data.alongAxisUnc = (uint16)ext_pos_data_ptr->ext_loc_uncrtnty_a */
      pos_data_ptr->ext_pos_data.f_alongAxisUnc = f_unc_meters;
      pos_data_ptr->ext_pos_data.f_perpAxisUnc =  f_unc_meters;
  ...
  {
    pos_data_ptr->ext_pos_data.f_loc_unc_horizontal = sqrt((pos_data_ptr->ext_pos_data.f_alongAxisUnc *
                                                                  pos_data_ptr->ext_pos_data.f_alongAxisUnc) +
                                                                 (pos_data_ptr->ext_pos_data.f_perpAxisUnc *
                                                                  pos_data_ptr->ext_pos_data.f_perpAxisUnc) );
  }
...
void tm_umts_common_utils_mm_session_status_notify( uint8 started, sys_modem_as_id_e_type e_as_id )
{
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Notify MM of LCS start while in CS", 0, 0, 0);
    // return;
  }
...
)
{
  ...
  {      
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"NI client triggers get_pos successfully", 0, 0, 0);
    ret_val = TRUE;
  }
  else
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"NI client fails to trigger get_pos", 0, 0, 0);
    ret_val = FALSE;
  }
...
( sm_InternalAssistDataStatusStructType *p_assist_data_status )
{
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Time unknown - GPS wishlist=%lu, GpsWeek=%u, TimeUncValid=%u", 
  ...
  else /* when time is known to PE, check ref.Loc and refTime requests */
  {
    ...
    else /* PE reports no position at all */
    {
      ...
      {
        /* no need to req. ref. loc., due to identical cell */
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"ref loc req. suppressed due to identical cell", 0, 0, 0);
    ...
    {
     MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"refTime demanded by PE", 0, 0, 0);
    }
           ...
           )
    {
     ...
     MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"refTime needed", 0, 0, 0);
    }
...
( sm_InternalAssistDataStatusStructType *p_assist_data_status )
{
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Time unknown - GLO wishlist=%lu, GpsWeek=%u, TimeUncValid=%u", 
            p_assist_data_status->q_WishListMask, 
            p_assist_data_status->w_GpsWeek, 
            p_assist_data_status->z_TimeUnc.u_Valid);
  }
  else
  {
    ...
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"PE provided GLO wishlist: 0x%lX -- will remove T/L/U if present", 
                                     ...
                                     const float  *f_vel_vertical)
{
  ...
  if ( (f_heading_ang == NULL) ||
      ...
      (f_vel_vertical == NULL) )
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"TMCOMMON: Invalid Parameters", 0, 0, 0);
    return ;
  }
  ...
  *w_hor_velocity = (uint16)f_temp;
  ...
  {
    *f_heading_ang = (float)(atan2(*f_vel_east, *f_vel_north) * C_RAD_TO_DEG);
  }
 ...
 * This is obtained from the GAD spec V3.2.0
 *
 * The uncertainty r, expressed in meters, is computed from SemiMajor and
 * SemiMinor values with the following formula:
 * IMP: SemiMajor and SemiMinor are converted from GADK units to Meters
 *      before plugging them in the below formula.
 *     r = sqrt[ (SemiMajor * SemiMajor) + (SemiMinor * SemiMinor) ] 
 *
 * This function simply returns the r value which is Uncertainity in meters.
 * 
 * 
 * Parameters:
 *
 *   u_UncMajorK - Coded SemiMajor value in GADK units
 *   u_UncMinorK - Coded SemiMinor value in GADK units
 *   u_Horiz - If True, Return Horizontal Unc. Otherwise return Vertical.
 *   u_Confidence - Confidence parameter to be used for scaling.
 *
 * Return value: 
 *
 *   Unc in meters
 ...
 ******************************************************************************
*/
float tm_umts_common_utils_GADK_unc_to_meter( uint8 u_Horiz, uint8 u_UncMajorK,
                                              uint8 u_UncMinorK, uint8 u_Confidence )
{
  float f_UncMajor = 0.0;
  float f_UncMinor = 0.0;
  float f_UncMeters = 0.0;
  float f_scale_up_to_63_conf = 0.0;

  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "u_UncMajorK: %u, u_UncMinorK: %u, u_Confidence: %u",
          u_UncMajorK, u_UncMinorK, u_Confidence);

  f_UncMajor  = cgps_utils_GADK_to_meter( u_Horiz, u_UncMajorK );
  f_UncMinor  = cgps_utils_GADK_to_meter( u_Horiz, u_UncMinorK );

  /*Scale the UncMajor and UncMinor to 68% before calculating the UncMeters Circular*/
  f_scale_up_to_63_conf =  tm_util_get_cep_scale_ratio_from_pct( (float)u_Confidence * 0.01f, 0.63f);
 
  f_UncMajor *= f_scale_up_to_63_conf;
  f_UncMinor *= f_scale_up_to_63_conf;

  f_UncMeters = sqrt( ( f_UncMajor * f_UncMajor ) + ( f_UncMinor * f_UncMinor ) );

  MSG_1(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "u_UncMajorK: %f",f_UncMeters);
