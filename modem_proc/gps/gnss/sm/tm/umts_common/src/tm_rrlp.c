...
/*===========================================================================
  ...
  $Header: //components/rel/gnss8.mpss/11.1/gnss/sm/tm/umts_common/src/tm_rrlp.c#1 $ $DateTime: 2019/10/24 01:30:36 $ $Author: pwbldsvc $

  when       who     what, where, why
  --------   ---     ----------------------------------------------------------
  01/09/17   gk      UT tlsf is not copied correctly when sending to engine.
...
{
  if((p_ganss_ref_time == NULL) || (p_tm_post_data_payload == NULL))
  {
     MSG_2(MSG_SSID_GPSSM, MSG_LEGACY_LOW,"NULL pointers %x %x",
           p_ganss_ref_time, p_tm_post_data_payload);
     return FALSE;
  }

  if(p_ganss_ref_time->ganssRefTimeInfo.m.ganssTimeIDPresent != 1)
  {
     MSG(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Gal RefTime not handled");
     return FALSE;
  }

	memset((void*) &p_tm_post_data_payload->gnss_reftime_data, 
         0, sizeof(p_tm_post_data_payload->gnss_reftime_data));

  if(p_ganss_ref_time->ganssRefTimeInfo.ganssTimeID == T_rrlp_GANSSRefTime_ID_BDS)
  {
    p_tm_post_data_payload->gnss_reftime_data.e_gnss_id = MGP_GNSS_TYPE_BDS;
  }
  else
  {
     MSG_1(MSG_SSID_GPSSM, MSG_LEGACY_LOW,"GNSS RefTime type (%d) not handled", 
           p_ganss_ref_time->ganssRefTimeInfo.ganssTimeID);
     return FALSE;
  }	  
  
  p_tm_post_data_payload->gnss_reftime_data.q_gnss_tod_sec = 
           p_ganss_ref_time->ganssRefTimeInfo.ganssTOD;

  if(p_ganss_ref_time->ganssRefTimeInfo.m.ganssDayPresent == 1)
  {
    p_tm_post_data_payload->gnss_reftime_data.w_gnss_day = 
           p_ganss_ref_time->ganssRefTimeInfo.ganssDay;
  }

  if(p_ganss_ref_time->ganssRefTimeInfo.m.ganssTODUncertaintyPresent == 1)
  {
    /* The uncertainty r, in microseconds, is mapped to a number K, with the 
     * following formula:	r = C*(((1+x)K)-1)
     * with C = 0.0022 and x = 0.18. */
    p_tm_post_data_payload->gnss_reftime_data.q_gnss_tod_unc_ms = 
           (0.0022 / 1000)* (pow(1.18, p_ganss_ref_time->ganssRefTimeInfo.ganssTODUncertainty) - 1) ;
  }
  else
  {
    /* Set to default value from NV since server did not provide us with an unc */
    if ( (z_umts_nv_item_ram_copy.aagps_default_ref_time_unc_50ms * 50) >= 1000 )
    {
      p_tm_post_data_payload->gnss_reftime_data.q_gnss_tod_unc_ms = 
        z_umts_nv_item_ram_copy.aagps_default_ref_time_unc_50ms * 50;
    }
    else
    {
      p_tm_post_data_payload->gnss_reftime_data.q_gnss_tod_unc_ms = 1000;
    }
  }
  ...
  if ( !tm_post_data ( 
            prtl_type,
            sess_handle,
            TM_POST_DATA_TYPE_REFTIME_BDS,
            p_tm_post_data_payload)
     )
  {
     MSG(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "BDS RefTime to Tmcore data post failed");
     return FALSE;
  }
  else
  {
     MSG(MSG_SSID_GPSSM, MSG_LEGACY_MED,"BDS refTime posted to TmCore");
  }
...
)
{
  ...
  uint16 w_GpsWeek; 
  uint8 u_i;
  struct rrlp_dec_GPSTOWAssist *p_rrlp_tow = NULL;
  struct rrlp_dec_GPSTOWAssistElement *p_rrlp_tow_elem;
  ...
  if( p_assist->referenceTime.m.gpsTowAssistPresent )
  {
    ...
    for ( u_i=0; 
          ...
          u_i++ )
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_LOW,"Original satelliteID: %d tlmWord: %d antiSpoof: %d ", 
              p_rrlp_tow_elem->satelliteID+1,
              p_rrlp_tow_elem->tlmWord, 
              p_rrlp_tow_elem->antiSpoof);
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_LOW,"Original satelliteID: %d Alert: %d RsvdBits: %d ", 
              p_rrlp_tow_elem->satelliteID+1,
...
)
{
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Err: ref pos. source info. invalid", 0, 0, 0);
    return FALSE;
  }
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_LOW,"RRLP RefLoc shape not allowed: %u", u_RefLocShape, 0, 0);
    return FALSE;
  }
...
)
{
  ...
  rrlp_dec_IonosphericModel *p_Iono = &p_assist->ionosphericModel;
...
{
  rrlp_dec_UTCModel *p_Utc = &p_assist->utcModel;
  gps_UtcInfoRptStructType *p_tm_post_data_payload_utc_model = &p_tm_post_data_payload->utc_model_data;

  if ( (NULL == p_assist) || (NULL == p_tm_post_data_payload) )
  {
    MSG(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,  "Error: Null pointer");
    return;
  }
  p_Utc = &p_assist->utcModel;
  p_tm_post_data_payload_utc_model = &p_tm_post_data_payload->utc_model_data;
  ...
  p_tm_post_data_payload_utc_model->u_DeltaTlsf = p_Utc->utcDeltaTlsf;
...
)
{
  ...
  rrlp_dec_SeqOfAcquisElement *AcqElement = &p_assist->acquisAssist.acquisList;
  rrlp_dec_AcquisElement      *p_Acq;
...
{
  rrlp_enc_GPS_MeasureInfo      *p_measinfo;
  rrlp_enc_GPS_MsrSetElement    *p_msrset;
  rrlp_enc_GPS_MsrElement       *p_element;
  ...
  p_msrset->gpsTOW = p_prm_data->z_MeasBlk.z_GnssHeader.z_MeasClock.z_Time.z_GpsTime.q_GpsMsec % C_FOUR_HRS_MSEC;

  p_gps_meas_per_sv = &p_prm_data->z_MeasBlk.z_Gnss[0];
  u_sv_cnt = 0;

  MSG(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "tm_rrlp_mprsp_gps_meas_build: Calling BlacklistFilter");
  ...
  { 
       ...
       )  
    {
      ...
      f_Num = ( p_gps_meas_per_sv->z_FiltMeas.f_SvSpeed  - p_prm_data->z_MeasBlk.z_GnssHeader.z_MeasClock.z_Freq.f_ClkFreqBias )
      ...
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,  "Error: WholeChip out of range", 0, 0, 0);
      }
...
)
{
  ...
  rrlp_enc_GANSS_SgnTypeElement   *p_rrlp_GANSS_SgnTypeElement = NULL;
  rrlp_enc_GANSS_MsrElement       *p_rrlp_GANSS_MsrElement = NULL;
  rrlp_enc_GANSS_SgnElement       *p_rrlp_GANSS_SgnElement = NULL;
  rrlp_enc_GANSS_SgnElement       *p_rrlp_GANSS_SgnElement_First = NULL; /* keeps a copy of the pointer to the first element*/
  ...
  if (p_GloMeasRpt != NULL)
  {
    ...
    p_rrlp_GANSS_SgnTypeElement = os_MemAlloc ( sizeof(*p_rrlp_GANSS_SgnTypeElement), OS_MEM_SCOPE_TASK );
    if ( p_rrlp_GANSS_SgnTypeElement != NULL ) 
    {
      memset(p_rrlp_GANSS_SgnTypeElement, 0, sizeof(*p_rrlp_GANSS_SgnTypeElement));
    }
    else
    {
      b_ret = FALSE;
    ...
    p_rrlp_GANSS_MsrElement = os_MemAlloc ( sizeof(*p_rrlp_GANSS_MsrElement), OS_MEM_SCOPE_TASK );
    if ( p_rrlp_GANSS_MsrElement != NULL) 
    {
      memset(p_rrlp_GANSS_MsrElement, 0, sizeof(*p_rrlp_GANSS_MsrElement));
    }
    else
    {
      b_ret = FALSE;
    ...
    p_rrlp_GANSS_SgnElement = os_MemAlloc ( sizeof(*p_rrlp_GANSS_SgnElement) * u_TotalGoodGloSVs, OS_MEM_SCOPE_TASK );
    if ( p_rrlp_GANSS_SgnElement != NULL) 
    {
      memset(p_rrlp_GANSS_SgnElement, 0, sizeof(*p_rrlp_GANSS_SgnElement) * u_TotalGoodGloSVs );
    ...
    if ( !b_gps_meas_included ) /* if GPS meas. is not included, populate GNSS meas. time stamp */
    {
      ...
      if ( ( p_prm_data->z_MeasBlk.z_GnssHeader.z_MeasClock.z_Time.z_GloTime.u_FourYear == 255 ) ||
           ( p_prm_data->z_MeasBlk.z_GnssHeader.z_MeasClock.z_Time.z_GloTime.w_Days == 65535 ) ||
           ( p_prm_data->z_MeasBlk.z_GnssHeader.z_MeasClock.z_Time.z_GloTime.q_GloMsec  > DAY_MSECS )
         )
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, 
              "GlonassTOD invalid: Num4Year=%u, NumDaysIn4Year=%u, ToD_ms=%lu", 
              p_prm_data->z_MeasBlk.z_GnssHeader.z_MeasClock.z_Time.z_GloTime.u_FourYear,
              p_prm_data->z_MeasBlk.z_GnssHeader.z_MeasClock.z_Time.z_GloTime.w_Days,
              p_prm_data->z_MeasBlk.z_GnssHeader.z_MeasClock.z_Time.z_GloTime.q_GloMsec);
        return FALSE;
      }
      else
      {
        p_rrlp_GANSS_MsrSetElement->ganssTODm = 
          p_prm_data->z_MeasBlk.z_GnssHeader.z_MeasClock.z_Time.z_GloTime.q_GloMsec % HOUR_MSECS;
      }
    ...
    for (u_i = 0; (u_i < u_TotalGloSvs); ++u_i, ++p_GloMeasPerSv)
    {
          ...
          (gnss_meas_status_bits_check(p_GloMeasPerSv->q_MeasStatus, status_check_needed)))
      {
        ...
        f_Num = (p_GloMeasPerSv->z_FiltMeas.f_SvSpeed  - p_prm_data->z_MeasBlk.z_GnssHeader.z_MeasClock.z_Freq.f_ClkFreqBias);
        ...
        {
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"GPS meas. absent, GNSS code phase assigned from FC to GLONASS TOD", 0, 0, 0);
          ...
          d_AdjSubMsec = (double)p_GloMeasPerSv->z_FiltMeas.z_SvTime.f_SubMs
             + (double)p_prm_data->z_MeasBlk.z_GnssHeader.z_MeasClock.z_Time.z_GloTime.f_ClkTimeBias;
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"No qualified SV in GLO meas. block, No GANSS meas. sent", 0, 0, 0);
    return FALSE;   
  }
}
...
)
{
  ...
  rrlp_dec_GANSSRefTimeInfo *p_glo_ref_time = 
     ...
     )
  {
    ...
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Unsupported GANSS_TIME info: bitmask=%x, ID=%u",
              temp_m,
  ...
  */
  p_tm_post_data_payload->alm_glo_data.q_DracCorrect = 
    tm_umts_common_utils_2s_comp_to_sign_mag(
      p_glo_alm_set->gloAlmDeltaTA, 0x00200000 );
...
)
{
  ...
  */
  ...
  p_ganss_generic_assist_data_elem->ganssAddUTCModel.u.utcModel3->tauC
  #endif
  ...
  p_tm_post_data_payload->utc_model_glo_data.w_Nt = 
   p_ganss_generic_assist_data_elem->ganssAddUTCModel.u.utcModel3->nA; 

  /* B1 and B2 are OPTIONAL. Do not use */
  #if 0 /* TM API has no B1 or B2 */
  if ( p_ganss_generic_assist_data_elem->ganssAddUTCModel.u.utcModel3->m.b1Present == 1 )
  {
    ?? = p_ganss_generic_assist_data_elem->ganssAddUTCModel.u.utcModel3->b1
  }
  if ( p_ganss_generic_assist_data_elem->ganssAddUTCModel.u.utcModel3->m.b2Present == 1 )
  {      
    ?? = p_ganss_generic_assist_data_elem->ganssAddUTCModel.u.utcModel3->b2
  }
  #endif

  if ( p_ganss_generic_assist_data_elem->ganssAddUTCModel.u.utcModel3->m.kpPresent == 1 )
  {
    ...
    p_tm_post_data_payload->utc_model_glo_data.u_Kp = 
      *(p_ganss_generic_assist_data_elem->ganssAddUTCModel.u.utcModel3->kp.data);
...
)
{
  ...
  rrlp_dec_GANSSRefMeasurementElement  *p_gnss_acq_assist_elem = NULL;
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Invalid pointer: %lx, %lx", prot_glo_acqasst_buffer_ptr, ganssRefMeasAssistList_ptr, 0);
    return;
  }

  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Preparing AGLO-AcqAssistData for MGP injection", 0, 0, 0); 
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Build %lu Glonass AA SVDirection & Steering info", ganssRefMeasAssistList_ptr->n, 0, 0);

  /* ME can't eat more than it can digest */
  if ( ganssRefMeasAssistList_ptr->n > N_GLO_FREQ )
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"TM_LPP_UP: Can't handle AA data of %u elements. Max %d can be consumed.", 
  ...
  for ( u_i=0; u_i<u_num_acq_assist; ++u_i, ++p_gnss_acq_assist_elem )
  {
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Glonass AA SvId %u invalid", p_gnss_acq_assist_elem->svID, 0, 0);
    }
