...
/*===========================================================================
  ...
  $Header: //components/rel/gnss8.mpss/11.1/gnss/sm/tm/umts_common/src/tm_rrc.c#1 $ 
                                                ...
                                                )
{
  ...
  {   
    ...
    {    
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_LOW,"Original satelliteID: %d tlmWord: %d tlmRsvd: %d ", 
              p_rrcTow->satellite_identity+1,
              p_rrcTow->tlm_msg,
              p_rrcTow->tlm_rsvd );

      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_LOW,"Original satelliteID: %d Alert: %d Spoof: %d ", 
              p_rrcTow->satellite_identity+1,
              p_rrcTow->alert, 
              p_rrcTow->anti_spoof);                                                                                                                                                                                                                                                    
                                                          ...
                                                          )
{
  ...
  while ( u_i < p_acq_assist->no_of_gps_sat )
  {
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"TM-RRC: Invalid AA SV: %d", p_SvSteerPerSv->w_Sv, 0, 0);
...
{
  if ((NULL == p_RrcNavSatelliteInfo) || (NULL == pz_ProtGloEphToPost))
  {
    MSG_2(MSG_SSID_GPSSM, MSG_LEGACY_ERROR, "Null Pointer access in tm_rrc_glo_eph_to_common_prot_struct(), "
      "p_RrcNavSatelliteInfo ? Null: %d, pz_ProtGloEphToPost?Null: %d", (NULL == p_RrcNavSatelliteInfo),
      (NULL == pz_ProtGloEphToPost));
  ...
  {
    pz_ProtGloEphToPost->z_Mask.z_FilledMask.u_GloTypeFlag = 0;
    ...
    *  */
    ...
    if ( TRUE == p_RrcNavSatelliteInfo->ganss_orbit_model.glonass_ECEF_incl )
    {
      ...
      {
        pz_ProtGloEphToPost->u_GloType = p_RrcNavSatelliteInfo->ganss_orbit_model.glonass_ECEF.gloM;
        pz_ProtGloEphToPost->z_Mask.z_FilledMask.u_GloTypeFlag = GNSS_NEW_GLO_TYPE_AVAILABLE;
      }
