...
/*===========================================================================
  ...
  $Header: //components/rel/gnss8.mpss/11.1/gnss/sm/tm/1x_cp/src/tm_1x_cp.c#1 $

when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
06/05/19   mj      Fix issue where location privacy notification was not sent 
11/02/17   sj      Remove ECBM state, rely on CM MSG Router Emergency Mode status messages
03/11/19   mj      Location Privacy Feature
...
12/08/14   gk      new session events to pdapi
...
)
{
  ...
  if (reason != TM_STOP_REASON_COMPLETED)
  {
    ...
    location_notification.q_prtl_type = TM_PRTL_TYPE_1X_CP;
    ...
    location_notification.e_oper_mode = TM_SESS_OPERATION_MODE_MSA;
    location_notification.e_sub_id = tm_1x_cp_get_cached_as_id();
...
)
{
  ...
  if( (prm_ptr->z_MeasBlk.z_GnssHeader.e_MeasBlkSrc != GNSS_MEAS_BLK_SRC_GPS) &&
      (prm_ptr->z_MeasBlk.z_GnssHeader.e_MeasBlkSrc != GNSS_MEAS_BLK_SRC_GLO) )
  {
    MSG_1(MSG_SSID_GPSSM, MSG_LEGACY_ERROR, 
          "Unknown constellation passed in prm %d", 
          (uint8) prm_ptr->z_MeasBlk.z_GnssHeader.e_MeasBlkSrc);
    return TRUE;
  }
  ...
  if(prm_ptr->z_MeasBlk.z_GnssHeader.e_MeasBlkSrc == GNSS_MEAS_BLK_SRC_GLO)
  {
  ...
  if(prm_buff_ptr)
  {
     if(prm_ptr->z_MeasBlk.z_GnssHeader.u_NumSvs == 0)
     {
       /* create bogus PRMs */
       memset( prm_buff_ptr, 0, sizeof(*prm_buff_ptr)); /* sm_GpsMeasRptStructType */
       prm_buff_ptr->z_MeasBlk.z_GnssHeader.u_NumSvs = 0;
       prm_buff_ptr->z_MeasBlk.z_GnssHeader.z_MeasClock.z_Time.z_GpsTime.w_GpsWeek 
                                           = C_GPS_WEEK_UNKNOWN;
     }
     else
     {
       sm_GnssMeas_memscpy((void *)prm_buff_ptr, sizeof(*prm_buff_ptr), prm_ptr);
     }
	...
	* SV measurements */
       prm_buff_ptr->z_MeasBlk.z_GnssHeader.z_MeasClock = prm_ptr->z_MeasBlk.z_GnssHeader.z_MeasClock;	     
       cp_prm_buff_ptr_gps = prm_buff_ptr;
     }
     MSG_4(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"GpsPRM Time %d %d %d %d", 
          prm_buff_ptr->z_MeasBlk.z_GnssHeader.z_MeasClock.z_Time.z_GpsTime.w_GpsWeek, 
          prm_buff_ptr->z_MeasBlk.z_GnssHeader.z_MeasClock.z_Time.z_GpsTime.q_GpsMsec, 
          (uint32 )(prm_buff_ptr->z_MeasBlk.z_GnssHeader.z_MeasClock.z_Time.z_GpsTime.f_ClkTimeBias * 1000), 
          (uint32 )(prm_buff_ptr->z_MeasBlk.z_GnssHeader.z_MeasClock.z_Time.z_GpsTime.f_ClkTimeUncMs * 1000));
...
)
{
  ...
  /* Handle PRM parts */
  if(prm_ptr->z_MeasBlk.z_GnssHeader.u_SeqNum < prm_ptr->z_MeasBlk.z_GnssHeader.u_MaxMessageNum)
  {
    /* If more prm parts are to follow, simply save this part and exit */
    return tm_1x_cp_save_prm_part(prm_ptr);
  }
  else if(prm_ptr->z_MeasBlk.z_GnssHeader.u_SeqNum == prm_ptr->z_MeasBlk.z_GnssHeader.u_MaxMessageNum)
  {
  ...
  else
  {
    ...
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"PRM part num %d > max parts %d", 
                 prm_ptr->z_MeasBlk.z_GnssHeader.u_SeqNum, 
                 prm_ptr->z_MeasBlk.z_GnssHeader.u_MaxMessageNum, 0);
    tm_1x_cp_terminate_session( TM_1X_CP_TERM_SRC_NONE, 
...
FUNCTION: i_tm_1x_cp_convert_and_send_pos_mode_to_pdapi

DESCRIPTION: This function sends a PDAPI event indicating the session type

PARAMETERS: request_type from the network
  
===========================================================================*/
static void  i_tm_1x_cp_convert_and_send_pos_mode_to_pdapi(is801_fwd_link_req_mask_type request_type)
{
  uint32 sess_type = 0;
  
  /* only GPS modes supported for now */
  switch (request_type)
  {
     ...
     case IS801_FWD_LINK_REQ_MASK_PPM:
       sess_type |= PDSM_PD_SESS_REQ_TYPE_AFLT; 
       break;
     default:
       MSG_1( MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "Unhandled request from network %d", request_type);
       break;
  }
  
  if ( 0 != sess_type )
  {
    tm_send_sess_type_pdapi_event(sess_type, PDSM_PD_COMM_PROTOCOL_1X_CP);
  }
}
...
)
{
  ...
  for(j = 0; j < tm_1x_cp_session_info.fwd_link_pddm.hdr.num_requests; j++)
  {
    ...
    /* send an event to PDAPI on the session request type */
    i_tm_1x_cp_convert_and_send_pos_mode_to_pdapi(req_rd->request_type); 
  }
...
)
{
  ...
  for(i = 0; i < num_pddm; i++)
  {
    ...
    if(tm_1x_cp_session_info.fwd_link_pddm.hdr.sess_start == 1)
    {
      ...
      {
        tm_core_send_pdapi_emerg_sess_type(PDSM_PD_EXT_STATUS_EMERG_SESS_1X_CP);
      }
