...
$Header: //components/rel/gnss8.mpss/11.1/gnss/wwanme/gts/inc/gts.h#11 $
$DateTime: 2024/04/12 02:01:22 $
$Author: pwbldsvc $
...
/* Slow clock ticks per msec. 19.2MHz / (586*1000) = 32.764505 Hz */
#define GTS_SLOW_CLK_TICKS_PER_MS   (DBL)32.764505
...
/* NR5G STMR rollover value - every 10.24secs (10240 * 30720) */
#define GTS_NR5G_STMR_ROLLOVER  314572800

/* Max Time between two NR5G time - max paging cycle length is 2.56secs.
 * Keep the max diff slightly larger than 2.56secs (3secs) */
#define GTS_MAX_NR5G_DIFF_MS    3000

/* NR5G Clock Speed */
#define GTS_NR5G_STMR_PER_MS 30720.0
...
/* Relax the Network clock drift to be 20ppm */
  #define GTS_NETWORK_CLOCK_DRIFT (DBL)20e-6
...
#define GTS_TIME_UNC_MODE_SWITCH_MS    ((FLT)0.001)
...
#define GTS_XO_FEST_SCALING_FACTOR     ((FLT)1024.0)

/* Max error in Freq Est of XO Clock in PPB (35 PPM) */
#define GTS_FREQ_EST_ERR_BOUND_1024_PPB  35*1024*1024
...
/* NR5G Time Unc based on MAR of 10 Km (33us) + Base Station Clock error (5us) */
#define GTS_NR5G_MAR_TUNC_MS          0.038
...
/* Based on the current FT state, XO frequency error Unc (USTMR/QTime) will vary */
/* Following are the Max UNC constants for few of the FT States, For all 
   ...
   Automotive PLs have a larger operating temperature range (-30 to +105 degC)
   than mobile phones (-30 to +85 degC) */
#ifdef FEATURE_GNSS_GEN9_AUTOMOTIVE_ENABLED

#define GTS_FT_STATE_COARSE_CAL   5
#define GTS_COARSE_CAL_UNC        35e-6

#define GTS_FT_STATE_IFC          6
#define GTS_IFC_UNC               17e-6

#define GTS_FT_STATE_FINE_CAL     7
#define GTS_FINE_CAL_UNC          15e-6

#define GTS_FT_STATE_FT1          8
#define GTS_FT1_UNC               12e-6

#define GTS_FT_STATE_FT2          11
#define GTS_FT2_UNC               10e-6

#else /* #ifdef FEATURE_GNSS_GEN9_AUTOMOTIVE_ENABLED */
...
#endif /* #ifdef FEATURE_GNSS_GEN9_AUTOMOTIVE_ENABLED */

/* Define the Max Ustmr clock drift to be the coarse Cal UNC */
...
#define GTS_DETECT_FCOUNT_ROLLOVER_MS 16384
  ...
  #define GTS_MIN_WWAN_TUNC_DEFAULT_MS    (DBL)1.0
#else
  #define GTS_MIN_WWAN_TUNC_DEFAULT_MS    (DBL)0.1
#endif

#define GTS_MIN_WWAN_TUNC_LPPM_MS       (DBL)0.05

/* Conversion from MS to Ns, us */
#define GTS_NS_PER_MS                1000000
#define GTS_US_PER_MS                1000
...
/* Max Propagation for Latch Ms. Little more than 1Hz fix rate (in millisec) */
#define GTS_MAX_PROP_LATCH_MS 1100
...
/* One latch pair of GPS RTC/USTMR can have upto +/- 30ns error which is ~3 GPS RTC ticks (each GPS tick
 * is 12.2ns - @81.84 MHz). Since we are comparing 2 back-to-back 1ms latches, the max. error can be
 * twice that which is 6 GPS ticks */
#define GTS_GPS_RTC_LATCH_JITTER  6

/* We can tolerate upto 1 USTMR tick jitter for USTMR, which is 52.08ns or ~5 GPS RTC ticks*/
#define GTS_USTMR_LATCH_JITTER    1
#define GTS_GPS_RTC_CNT_ONE_USTMR_TICK    5

/* If USTMR has 1 tick error, then max. GPS RTC error should be +/- (5 + 6) */
#define GTS_MAX_DIFF_GPS_RTC_USTMR_TICK   (GTS_GPS_RTC_CNT_ONE_USTMR_TICK + GTS_GPS_RTC_LATCH_JITTER)

#define GTS_RTC_USTMR_LATCH_UNC_MS  ((FLT)0.00003) //30nsec
...
/* 100ms periodic time update value for V2X */
#define GTS_V2X_PERIODIC_UPDATE_TIMER_VALUE   100
 ...
 * For CV2X PLs, worst case diff between FEST updates will be every 5.2 sec accounting for the max. WWAN paging duration (1x). 
 * And we give a grace of 800ms, leading to 6 secs or 6 * 19.2MHz = 115200000. 
...
#ifdef FEATURE_V2X
  #define GTS_MAX_DIFF_FOR_XO_FEST_PROP   115200000
#else
  #define GTS_MAX_DIFF_FOR_XO_FEST_PROP   28800000
#endif

#define GTS_DEFAULT_LEAP_SEC             18
#define GTS_DEFAULT_LEAP_SEC_UNC_MS      2500

/* Minimum error for TUNC bound when performing consistency check using
 * Qtime - 2 USTMR ticks or 104.2nsec */
#define GTS_USTMR_CONSISTENCY_BOUND_MS (0.000104)

/* Maximum age for providing PE velocity info to CV2X - 1 sec */
#define GTS_PE_VELOCITY_AGE_THRESHOLD ((DBL)1000.0)

/* Macros for processing PMIC RTC */
#define GTS_MAX_PMICRTC_TUNC   	  ((DBL)(200e-6))
#define GTS_PMICRTC_RESOLUTION_ERROR_MS ((DBL)1000.0) //1sec error

/* Delay timer to log F3's after bootup/reboot - 30 secs  */
#define GTS_EFS_PMIC_LOG_TIMER_VALUE_MS   30000
#define GTS_EFS_DELAY_LOG_MAX_CTR    4

/* Minimum TUNC for PACE src to be injected into ME/SM - 90usec */
#define GTS_MIN_PACE_TUNC_MS ((FLT)0.09)
...
typedef struct
{
  U8 e_ClientInfo; //of enum type 'gts_ClientType'
...
typedef struct
{
  gts_Nr5gTimeInfoType z_Nr5gInfo;
...
typedef struct
{
  ...
  S32 l_FreqEst; /* Raw freq error in 2^10PPB */
  U8  e_FestSrc; /* FEST source of enum type 'gts_FestSrcType' */
} gts_FreqEstDBType;
...
typedef union
{
  ...
  gts_PosEstMsgType z_PePosInfo;
  gts_CdmaInfoDBType z_CdmaInfo;
  gts_Nr5gInfoDBType z_Nr5gInfo;
} gts_InputInfoType;
...
typedef struct
{
  ...
  gts_TimeSrcType e_LastUpdateModule;
  /* Elements with Time information */
  gts_TimeDbElementType z_TimeDbElement[GTS_TIME_SRC_MAX_SRC];
} gts_TimeDbType;
...
typedef struct
{
  ...
  gts_SrvSysInfoType z_SrvSys[TM_CM_MAX_NO_SUBS][TM_CM_MAX_NO_STACKS];
  ...
  boolean b_DelayStatus;
  mgp_GtsGpsRfPathDelayType q_GroupDelayNs;

  /* V2X timer for periodic updates */
  os_TimerMsgType *p_V2xUpdateTimer;
  boolean b_V2xTimerEnabled;
  ...
  /* Min. TUNC for WWAN periodic updates */
  DBL d_WwanMinTunc;

  /* Timer to delay logging of EFS contents */
  os_TimerMsgType *p_EfsLogDelayTimer;

  /* GNSS primary constellation info */
  gts_TimeFormatType e_PrimaryConst;
  uint8 u_EfsLogCtr;

  /* Parameters for DIAG cmd to override XO FUNC and/or skip a few updates */
  boolean b_IsOverrideXoFunc;
  FLT f_DefaultXoFuncMs;
  boolean b_EnableSkipFestUpdates;
  uint32 q_MaxSkipFestUpdates;
  uint32 q_FestCtr;
...
typedef struct
{
  ...
  gts_QtimeType z_QtimeCalStateUpdate;
...
/*
 * This is the actual structure of the PMIC EFS data as it resides in EFS.
 * It consists of some header information (used for error checking)
 * and the data itself.
 */
typedef struct
{
  uint32 q_Xsum32; /* Sum of the data section only */
  uint16 w_Len; /* Length of data section only (in bytes) */
} gts_GpsEfsHdrStruct;

typedef struct
{
  pm_hal_rtc_time_type z_PmicRtc; /*Pmic Rtc time in seconds */
...
typedef enum
{
  ...
  GTS_EFS_PRIMARY_CONST_MISMATCH
...
typedef struct
{
  ...
  boolean b_RtcCont; /* RTC continuity flag checking flag  */
  int16 x_FdRead; /* File open descriptor checking  flag */
  int32 l_BytesRead; /* Number of bytes read */
  uint32 q_CsumOfEfsRead; /* Checksum of EFS data */
  pm_err_flag_type e_PmErr; /* Pmic Flag success checking */
  pm_hal_rtc_time_type z_CurrPmicRtc; /* Current PmicRtc time */
  pm_hal_rtc_time_type z_PrevPmicRtc; /* Previous PmicRtc time */
  int64 r_DiffPmicRtcMs;
  ...
  gts_GpsEfsHdrStruct z_EfsHdr; /* Header info stored in EFS */
 ...
 ******************************************************************************
*/
boolean gts_InputSrcDisabled(const gts_TimeSrcType e_ModSrc);
 ...
 *  GPS ME Fix time which was based on Freq Est to current Qtime
 *
 * Parameters:
 *
 ...
 ******************************************************************************
*/
void gts_ProcFreqEstMsg( const gts_FreqEstInfoType *pz_FreqEst );

/*
 ******************************************************************************
 * Function gts_ProcFreqEstMsgForPace
 *
 * Description:
 *
 *  This function will use the Freq Estimate Error to propagate PACE time which
 *  was based on Freq Est to current Qtime
 *
 * Parameters:
 *
 *  pz_FreqEst - Freq Estimate Information
 ...
 ******************************************************************************
*/
void gts_ProcFreqEstMsgForPace( const gts_FreqEstInfoType *pz_FreqEst );
 ...
 ******************************************************************************
*/
void gts_ProcPosEstMsg(const gts_PosEstMsgType *pz_PosEst);

/*
 ******************************************************************************
 * Function gts_ProcPeToMeClockPutMsg
 *
 * Description:
 *
 *  This function handles the Time estimate from PE clock_put sent to Measurement Engine
 *
 * Parameters:
 *
 ...
 ******************************************************************************
*/
void gts_ProcTimeEstRptMsg( const gts_TimeType *pz_TimeEstMsg );
 ...
 * Parameters: 
 *
 *  p_DelTime - Reason for the deletion
 *
 * Dependencies:
 * 
 *  None
 ...
 ******************************************************************************
*/
void gts_ProcDelTime( gts_DelTimeType *p_DelTime );
...
/*
 ******************************************************************************
 * Function gts_ProcNr5gTimeMsg
 *
 * Description:
 *
 *  Processes NR5G Time Info Msg. This Function will propagate the last NR5G
 *  based GPS Time to current NR5G Time.
 *
 * Parameters:
 *
 ...
 ******************************************************************************
 */
void gts_ProcNr5gTimeMsg(const gts_Nr5gTimeInfoType *pz_Nr5gTime);

/*
 ******************************************************************************
 * Function gts_ProcWcdmaTimeMsg
 *
 * Description:
 *
 *  Processes WCDMA Time Info Msg. This Function will propagate the last WCDMA
 *  based GPS Time to current WCDMA Time.
 *
 * Parameters:
 *
 *  pz_WcdmaTime - WCDMA Time Information
 *
 * Dependencies:
 *
 *  None
...
/*
 ******************************************************************************
 * Function gts_ProcTdsTimeMsg
 *
 * Description:
 *
 *  Processes TDS Time Info Msg. This Function will propagate the last TDS
 *  based GPS Time to current TDS Time.
 *
 * Parameters:
 *
 *  pz_LteTime - TDS Time Information
 *
 * Dependencies:
 *
 *  None
 ...
 *
 * Description:
 *  This function copies the contents of time DB element
 *
 * Parameters:
 *  e_Mod - Source Module for the DB
 *  p_RetDB - Return Pointer to the DB element of type gts_TimeDbElementType*
 *
 * Dependencies:
 *  None
 *
 ...
 ******************************************************************************
 */
void gts_GetDbElem( const gts_TimeSrcType e_Mod, gts_TimeDbElementType *p_RetDB );
 ...
 *
 * Description:
 *  This function updates the time DB element
 *
 * Parameters:
 *  e_Mod - Source Module for the DB
 *  p_GtsTime - GTS time Pointer that needs to be updated
 *
 * Dependencies:
 *  None
 *
 ...
 ******************************************************************************
 */
void gts_SetDbElem( const gts_TimeSrcType e_Mod, const gts_TimeDbElementType *p_TimeDb );
 ...
 * Description:
 *
 *  Get the current Leap Sec info
 *
 * Parameters:
 *
 *  p_LeapSec - Pointer to send leap Second value
 *  p_LeapSecUncMs- Pointer to send leap secoond uncertainity value (in millisecs)
 *
 * Dependencies:
 *
 *  None
 ...
 ******************************************************************************
*/
void gts_GetLeapSecAndUnc(U8 *p_LeapSec, U16 *p_LeapSecUncMs);
 ...
 ******************************************************************************
 */
void gts_TimeRefreshUpdate(const boolean b_Successful,
                           const gts_TimeSrcType e_SrcModule);
...
/*
 ******************************************************************************
 * Function: gts_CopyGtsTimeToGnssTime
 *
 * Description:
 *  This function Copies GTS Time into GNSS Time structure
 *
 * Parameters:
 *  pz_GtsTime - GTS Clock Structure
 *  pz_GnssTime - GTS Clock Structure
 *
 * Dependencies:
 *  None
 *
 ...
 ******************************************************************************
 */
void gts_CopyGtsTimeToGnssTime( const gts_TimeType* pz_GtsTime , gnss_TimeFormatType* pz_GnssTime);

/*
 ******************************************************************************
 * Function: gts_CopyGnssTimeToGtsTime
 *
 * Description:
 *  This function Copies GNSS Time into GTS Time structure
 *
 * Parameters:
 *  pz_GnssTime - GTS Clock Structure
 *  pz_GtsTime - GTS Clock Structure
 ...
 ******************************************************************************
 */
void gts_CopyGnssTimeToGtsTime( const gnss_TimeFormatType* pz_GnssTime, gts_TimeType* pz_GtsTime);
 ...
 ******************************************************************************
 */
void gts_UpdComDbInfo(const gts_TimeSrcType e_Mod, const gts_QtimeType *pz_Qtime);
...
uint32 gts_CalcFCount32( const uint32 q_RefFCount, const uint32 q_RtcFCount );
...
/*
 ******************************************************************************
 * Function gts_UpdateVelocityInfo
 *
 * Description:
 *
 *  This function reads the current Velocity 
 *
 * Parameters:
 *
 ...
 ******************************************************************************
*/
boolean gts_UpdateVelocityInfo( FLT* f_VelocityMs );
...
/*
 ******************************************************************************
 * Function gts_IsLteActive
 *
 * Description:
 *  Checks if LTE is active on a particular sub/any stack
 *
 * Parameters:
 *  u_SubId - SUB ID to check if LTE is active on that or not
 *
 * Dependencies:
 *  None
 *
 * Return value:
 *  True - LTE is active on the provided SUB ID
 *  False - LTE is not active on the provided SUB ID
 *
 ******************************************************************************
*/
boolean gts_IsLteActive( const uint8 u_SubId );

/*
 ******************************************************************************
 * Function gts_ProcModifyWwanTunc
 *
 * Description:
 *
 *  Function to adjust the min. TUNC for WWAN periodic time updates
 *  
 * Parameters: 
 *
 ...
 ******************************************************************************
*/
void gts_ProcModifyWwanTunc( const gts_ModifyWwanTuncMsgType *p_ModifyTunc );
 ...
 *  q_IgnoreSrc - Bit mask to ignore any sources
 *
 * Dependencies:
 *
 *  None
 ...
 ******************************************************************************
*/
gts_TimeSrcType gts_FindBestGpsTime( gts_TimeType *pz_TimeInfo, const gts_QtimeType* pz_Qtime, const uint32 q_IgnoreSrc );
 ...
 *
 * Function
 *   gts_QtimeDiffXoUnits
 *
 * Description:
 *  Calculate difference between two QTIME values ( Qtime2-Qtime1 )
 *
 * Parameters:
 *  pz_Qt2 - Qtime at Instance 2
 *  pz_Qt1 - Qtime at Instance 1
 *  pd_Diff - Difference value XO units is updated to this pointer
 *
 * Return value:
 *  TRUE if the calculation was successful, FALSE otherwise
 *
...
boolean gts_QtimeDiffXoUnits( const gts_QtimeType* pz_Qt2, const gts_QtimeType* pz_Qt1, 
                              S64* pt_Diff );

/*
 ******************************************************************************
 * Function gts_MsTimeToGtsApi
 *
 * Description:
 *
 *  This function converts GPS Time in double floating point format
 *  into a data structure suitable for output to clients.  The structure
 *  output is gts_TimeType.
 *
 * Parameters:
 *
 *  t_GpsMs       - GPS Time (in milliseconds)
 *  q_GpsUncMs    - Time Uncertainty (in milliseconds)
 *  p_GtsTime     - pointer to location in which to store the converted
 *                  GTS time format
 *
 * Dependencies:
 *
 ...
 ******************************************************************************
 */
void gts_MsTimeToGtsApi(DBL d_GpsMs, DBL d_GpsUncMs, gts_TimeType *p_GtsTime);
 ...
 * Description:
 *
 *  This function will return the minimum GPS week number.
 *
 * Parameters:
 *
 *  None
 ...
 *
  ******************************************************************************
*/
uint16 gts_GetMinWeekNum(void);

/*
 ******************************************************************************
 * Function gts_InvalidatePmicRtc
 *
 * Description:
 *
 ...
 ******************************************************************************
*/
U16 gts_Checksum16(const uint8 *p_Data, uint16 n_Bytes);
 ...
 *********************************************************************************
*/
int32 gts_EFSWrite(const char *p_Path, void *p_Data, const uint32 q_Size);
/*
 ******************************************************************************
 * Function gts_ValidateClockBias
 *
 * Description:
 *
 *  Validating Clock bias. Return True if Clock bias is in the range [-0.5, 0.5] and a valid number
 *  Return false if it is out if bound or NaN
 *
 * Parameters:
 *
 *   f_ClockBias - Clock bias injected from the absolute source
 *   e_Src - Enum to denote Source
 *
 * Dependencies:
 *
 *  None
...
boolean gts_ValidateClockBias(FLT f_ClkTimeBias, uint8 e_Src);

/*
 ******************************************************************************
 * Function gts_ValidateClockTunc
 *
 * Description:
 *
 *  Validating Clock Tunc. Return True if Tunc is positive and a valid number
 *  Return false if it is negative and NaN
 *
 * Parameters:
 *
 *   f_ClkTuncMs - Tunc injected from the external source
 *   e_Src - Enum to denote source type
 *
 * Dependencies:
 *
 *  None
...
boolean gts_ValidateClockTunc(FLT f_ClkTuncMs , uint8 e_Src);
/*
 ******************************************************************************
 * Function gts_ValidateClockFunc
 *
 * Description:
 *
 *  Validating Clock Func. Return True if Func is positive and a valid number
 *  Return false if it is negative and NaN
 *
 * Parameters:
 *
 *   f_ClkFuncMs - Tunc injected from the external source
 *   e_Src - Enum to denote source type
 *
 * Dependencies:
 *
...
boolean gts_ValidateClockFunc(FLT f_ClkFuncMs , uint8 e_Src);
/*
******************************************************************************
* Function:
*  gts_GetPrimaryConstInfo
*
* Description:
*  This function will return the GTS primary constellation type
*
* Parameters:
*  None
*
* Return value:
*  gts_TimeFormatType - GTS constellation type
*
******************************************************************************
*/
gts_TimeFormatType gts_GetPrimaryConstInfo(void);
