...
$Header: //components/rel/gnss8.mpss/11.1/gnss/wwanme/lte_otdoa/5_0/src/loc_lte_otdoa_control.h#1 $
...
#define LLOC_MAX_LTE_FREQ_BANDS   47
...
/* Min and Max symbol offsets for PRS hypothesis for TDD and FDD scenarios */
#define LLOC_MIN_SYM_OFFSET_TDD    (int8)-1
#define LLOC_MAX_SYM_OFFSET_TDD    (int8)0
#define LLOC_MIN_SYM_OFFSET_FDD    (int8)-1
#define LLOC_MAX_SYM_OFFSET_FDD    (int8)7

/* Min and Max EARFCN for TDD cells. Band class 33 to 48 */
#define LLOC_MIN_LTE_EARFCN_TDD   36000
#define LLOC_MAX_LTE_EARFCN_TDD   56739

/* Testmode where we control LTE OTDOA operation */
typedef enum
{
  LOC_LTE_OTDOA_DIAG_INJECT_ASSIST_DATA_SUBCMD = LOC_LTE_OTDOA_DIAG_SUBCMD_START, /* 1 */
  LOC_LTE_OTDOA_DIAG_START_SESSION_SUBCMD = 2,

  /* DIAG commands to mimic cmds sent by SM */
  LOC_LTE_OTDOA_DIAG_SM_TEST_SUBCMD = 6,

  /* Sub commands for enabling/disabling PRS logger mode (cont. search
   * without NC */
  LOC_LTE_OTDOA_DIAG_ENABLE_PRS_LOGGER_SUBCMD = 7,
  LOC_LTE_OTDOA_DIAG_DISABLE_PRS_LOGGER_SUBCMD = 8,
  ...
  LOC_LTE_OTDOA_DIAG_ENABLE_PRS_SCAN_SUBCMD = 9,
  LOC_LTE_OTDOA_DIAG_DISABLE_PRS_SCAN_SUBCMD = 10,

  /* Sub command to override PRS paramters */
  LOC_LTE_OTDOA_DIAG_OVERRIDE_PRS_PARAMS_SUBCMD = 11,

  /* Sub command to enable NC Integration */
  LOC_LTE_OTDOA_DIAG_ENABLE_NC_INT_SUBCMD = 12,
  LOC_LTE_OTDOA_DIAG_DISABLE_NC_INT_SUBCMD = 13,

  /* Sub commands to force aiding ref cell to be ref cell in RSTD report.
   * Similar to OTA reset command */
  LOC_LTE_OTDOA_DIAG_FORCE_AIDING_REF_CELL = 14,
  LOC_LTE_OTDOA_DIAG_DISABLE_AIDING_REF_CELL = 15,

  /* Sub command to modify QOS timers value */
  LOC_LTE_OTDOA_DIAG_MODIFY_QOS_TIMER = 16,

  /* Reserve 17, 18 for OPCRS */

  /* Sub command to enable/disable computing offending cells after finishing disc phase */
  LOC_LTE_OTDOA_DIAG_ENABLE_OFFENDING_CELLS_COMPUTE  = 19,
  LOC_LTE_OTDOA_DIAG_DISABLE_OFFENDING_CELLS_COMPUTE = 20,

  /* Sub command to enable/disable computing PRS Gain Bias for cells after finishing disc phase */
  LOC_LTE_OTDOA_DIAG_ENABLE_PRS_GAIN_BIAS_COMPUTE = 21,
  LOC_LTE_OTDOA_DIAG_DISABLE_PRS_GAIN_BIAS_COMPUTE = 22,

  /* Sub command to adjust the FA threshold */
  LOC_LTE_OTDOA_DIAG_ADJUST_FA_THRESHOLD = 23,

  /* Sub command to adjust the num of hyps per batch, num of hyps per occ */
  LOC_LTE_OTDOA_DIAG_HYPS_PER_BATCH = 24,
  LOC_LTE_OTDOA_DIAG_HYPS_PER_OCC = 25,

  /* Sub command to enable/disable computing offending cells after finishing disc phase */
  LOC_LTE_OTDOA_DIAG_ENABLE_PRSIC_OFFENDING_COMPUTE = 26,
  LOC_LTE_OTDOA_DIAG_DISABLE_PRSIC_OFFENDING_COMPUTE = 27,

  /* Sub command to enable/disable computing of RSTD for IF cells based on best ref cell TOA */
  LOC_LTE_OTDOA_DIAG_ENABLE_VAR_REF_TOA_FOR_IF_NGBR = 28,
  LOC_LTE_OTDOA_DIAG_DISABLE_VAR_REF_TOA_FOR_IF_NGBR = 29,

  LOC_LTE_OTDOA_DIAG_MAX = LOC_LTE_OTDOA_DIAG_SUBCMD_END,
}
lte_loc_otdoa_control_DiagSubCmdEnumType;
...
typedef enum
{
  ...
  LLOC_MAX_WIN_SIZE_4HYP_FOR_ALL_CELLS,
  LLOC_MAX_WIN_SIZE_1HYP_FOR_ALL_CELLS,
...
typedef struct
{
  ...
  /* FW signal-to-signal+noise ratio for meas. cell */
  uint32 q_AvgSSNRx0;
  uint32 q_AvgSSNRx1;
...
typedef struct
{
  ...
  /* PRS bandwidth in assistance from Loc server and the actual value used by LocSW/ML1. These will be different
   * if the PRS BW in AD is > LTE system BW */
...
typedef enum
{
  /* In this mode, all cells will be searched continuously  */
  LLOC_CONT_SRCH_MODE,
  /* In this mode, all 504 physical cell IDs will be searched starting from #0. Assitance data is generated
   * internally */
  LLOC_PRS_SCAN_MODE,
  /* Similar to continuous search, but without continuous AD update */
...
typedef struct
{
  ...
  /* serving cell info */
  loc_lte_otdoa_ServingCellInfoType z_ScellInfo;
  ...
  /* Peak position offset in case if AD is re-adjusted when serving cell is not part of AD */
  int16 x_PeakPosOffset;
  boolean b_ApplyPeakOffset;

  /* Store the config to inflate search size when serving cell is not in AD */
  uint32 q_InflateSrchWinSizeConfig;

  /* Store the SUB ID for OTDOA session */
...
extern lloc_ParamType lloc_Params;
...
/*
 ******************************************************************************
 * Function: lloc_ResetDiagAssistData
 *
 * Description:
 *  Reset all diag related variables
 *
 * Parameters:
 *  None
 *
...
extern void lloc_ProcDiagPkt
(
  const loc_lte_otdoa_control_DiagPktMsgType * p_DiagPktMsg
);
