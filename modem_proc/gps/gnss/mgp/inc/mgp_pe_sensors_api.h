...
$Header: //components/rel/gnss8.mpss/11.1/gnss/mgp/inc/mgp_pe_sensors_api.h#1 $  
$Author: pwbldsvc $
...
typedef struct
{
 uint64 t_TimeTickMsUpdate;        /* Timestamp (Msec) when the data arrives to 
                                   NF task */      
 uint64 t_TimeTickMsStart;         /* TimeStamp (Msec) at the start of the 
                                   integration interval */
 mgp_PeSensorsTimeStamp z_IntStartTime; /* Integration start time */

 uint32 q_IntegrationIntervalMsec; /* Sensor measurement integration interval */
 ...
 uint8  u_BiasId;                  /* ID of the most recent bias correction packet
                                   that has been applied to this sensor meas */
 uint8  u_Mode;                    /* Static or Dynamic: indicates the property of 
 ...
 DBL d_ImuAccelFact[3];            /* Unbiased, Unrotated Average Accelerations in IMU frame. 
																		   Units: m/s^2 */
 DBL d_AverageGyroFact[3];         /* Unbiased Average Gyro measurement over the integration 
                                       interval. Units: radian/sec */
 ...
 DBL d_IntegratedQuaternionFact[4];/* Quaternion corresponding to unbiased gyro 
                                       measurements. Units: None */
 DBL d_Temperature;                /* Temperature of the sensor board. Units: Degree C*/

 boolean v_ZmdDecision;            /* ZMD decision: TRUE if zero-motion, FALSE if non-
                                   stationary. */
 boolean v_GyroOutage;             /* Flag to detect gyro outage during gyro meas integration*/
 boolean v_AccelOutage;            /* Flag to detect accel outage during accel meas integration*/
...
/*
******************************************************************************
* mgp_PeSensorStreamingEventPut
*
* Function description:
*   This function is used to pass the Sensors Streaming flag to PE/NF.
*
* Parameters:
*   v_SensorStreamingFlag: TRUE indicates Sensor Streaming started
*                        FALSE otherwise
*
* Return value: None
*
******************************************************************************
*/
void mgp_PeSensorStreamingEventPut(boolean v_SensorStreamingFlag);
