...
Version Control
  ...
  $Header: //components/rel/gnss8.mpss/11.1/api/cgps_api.h#1 $
...
typedef struct {
  ...
  /* Measurement data */
  uint32 arfcnEUTRA;          /* EUTRA arfcn */
  uint32  systemFrameNumber;  /* frame number */
  int8  rsrp_Result;          /* rsrp result */
  int8  rsrq_Result;          /* rsrq result */
 ...
 *  This function is used by external modules to set GPS in SARF mode or exit SARF
 * mode. Call-back is called with SUCCESS or FAILURE depending on whether the 
 * switch actually happpened. When there's a failure, CB is called after LM_GOING_ON_TIMEOUT
 * ms. This is currently set to 10 sec.
 *
 * Parameters: 
 *
 *  e_new_state - Desired MGP state.
