######################################################################
# $Header: //components/rel/gnss8.mpss/11.1/gnss_tools/pdapi_scripts/QXDM4/pd_DelAll.pm#1 $
...
{
  my ($arg) = @_;
  my ($byte1, $byte2, $byte3, $byte4,$byte5, $byte6, $byte7, $byte8);

  # not sure of perl's bitwise operators - change code later
  $byte1 = $arg%256; $arg = $arg/256;
  $byte2 = $arg%256; $arg = $arg/256;
  $byte3 = $arg%256; $arg = $arg/256;
  $byte4 = $arg%256; $arg = $arg/256;
  $byte5 = $arg%256; $arg = $arg/256;
  $byte6 = $arg%256; $arg = $arg/256;
  $byte7 = $arg%256; $arg = $arg/256;
  $byte8 = $arg%256;
...
sub SetCaptureCmd
{
  ...
  my @arg_bytes = uint32to8Bytes($self->{"delete_bitmap"});
  my $i = 13;
  foreach (@arg_bytes)
  {
    $req_pkt->Put($i, $_);
  ...
  my @arg_bytes = uint32to8Bytes($self->{"gnss_delete_bitmap"});
  my $i = 21;
  foreach (@arg_bytes)
  {
    $req_pkt->Put($i, $_);
