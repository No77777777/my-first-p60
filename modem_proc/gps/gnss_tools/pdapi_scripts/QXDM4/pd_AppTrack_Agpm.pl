##########################################################################
# $Header: //components/rel/gnss8.mpss/11.1/gnss_tools/pdapi_scripts/QXDM4/pd_AppTrack_Agpm.pl#1 $
...
{
  print ("\nUsage: perl pd_AppTrack.pl <COM Port> <Client Type> <Number of Sessions> <TBF> <PowerMode>\n\n");
...
if ((not defined ($power_mode)) || ($power_mode < 1) || ($power_mode > 5))
{
  if ($count == 0)
  {
    die "\nUsage: perl pd_AppTrack.pl <COM Port> <Client Type> <Number of Sessions> <TBF> <PowerMode>\n\n";
  ...
  print ("\n");
  # Prompt user to enter GNSS Power Mode
  print ("Please enter desired GNSS power mode. Valid Values:\n
    1 - Improved Accuracy Mode   (M1)
    2 - Normal Mode   (Default)  (M2)
    3 - Background Defined Power (M3)
    4 - Background Defined Time  (M4)
    5 - Background Keep Warm     (M5)
    \n");
  chomp ($power_mode = <STDIN>);
  print("\n");
  if (($power_mode < 1) || ($power_mode > 5))
  {
    die "Invalid power mode $power_mode. Valid values are 1-5 \n";
  }
}
...
######################################################################
  ...
  $req_getpos->{"power_mode"}        = $power_mode;   # GNSS power mode
######################################################################
...
Example:  perl pd_AppTrack.pl <COM Port> <Client Type> <Number of Sessions> <TBF> <PowerMode>
