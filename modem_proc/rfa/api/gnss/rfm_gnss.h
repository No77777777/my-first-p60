...
Copyright (c) 1999 - 2002, 2006, 2007-2016 by Qualcomm Technologies, Incorporated.  All Rights Reserved.
Copyright (c) 2008-2024 by Qualcomm Technologies, Incorporated.  All Rights Reserved.
...
$Header: //components/rel/rfa.mpss/4.11/api/gnss/rfm_gnss.h#2 $
$DateTime: 2024/03/11 22:23:22 $
$Author: mplcsds1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
02/27/24   ssk     Added new API to get both WGR and WTR IDs 
11/20/19   up      Added support for GNSS L5 mode
...
typedef enum
{
  ...
  RFGNSS_DEVICE_CMD_L5_ML5_ELNA_INIT,
  /* rfm_gnss_device_seq_execute(RFGNSS_DEVICE_CMD_L5_ML5_ELNA_INIT, NULL) */
...
/*!
   @brief
   API for NavRF to get Rf device ID
*/
uint32 rfm_gnss_get_device_id(void);//return uint32 to avoid scon changes in multiple modules

/*----------------------------------------------------------------------------*/
/*!
   @brief
   API for NavRF to get Rf device IDs of both SDR and WGR
*/
boolean rfm_gnss_get_device_id_v2(rfdevice_id_enum_type *device_id_1, rfdevice_id_enum_type *device_id_2);
