...
$Header: //components/rel/rfa.mpss/4.11/api/gsm/rfgsm_core_types.h#1 $
$DateTime: 2021/03/09 05:11:02 $
$Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
10/23/18   par     Adding LDO mode to APT_MODE type 
...
01/23/14   ec      Add new flags for debug NV mechanism 
...
#define FEATURE_RF_DEVICE_SPLIT //to be removed once rf dev branch is merged back to mainline
...
typedef enum
{
  ...
  TX_GRFC_SCRIPT,
  TX_ATUNER_SCRIPT,
  TX_COUPLER_FWD_SCRIPT,
  TX_COUPLER_REV_SCRIPT,
  TX_COUPLER_OPEN_SCRIPT,
  TX_MAX_SCRIPT
...
/* Delta between GSM RX Stop and CCS Release */
#define RFGSM_LAST_RX_STOP_ENQUEUE_OFFSET_QS  0    /* When RX Stop is enqueued with respect to stop trigger time */
#define RFGSM_WORST_CASE_DELAY_QS             120  /* Legacy hard coded value from GFW */
#define RFGSM_PWR_MEAS_GAMMA_QS               ( RFGSM_LAST_RX_STOP_ENQUEUE_OFFSET_QS + RFGSM_WORST_CASE_DELAY_QS )
...
/* This macro is to be used where the calling function returns void */
#define RF_NULL_CHECK_SET_STATUS(p, txt, status) \
{                                            \
  if (p == NULL)                             \
  {                                          \
    RF_MSG(MSG_LEGACY_ERROR, txt);           \
...
typedef struct
{
  ...
  int16	vbatt_comp_parange_dBx100[RFGSM_MAX_TX_GAIN_RANGE];
...
typedef struct
{
  ...
  int32  mon_idle_rf_setup_time_qs; /* time required to setup RF for monitor burst. This will only be used in DR-DSDS when GSM is in Recieve Idle mode only */ 
  ...
  int32  tx_rf_setup_time_vswr_qs;  /* time required to setup RF for Tx burst with VSWR measurement */ 
