...
--------   ---     ------------------------------------------------------------------
01/20/20   ssl     support for dynmic rf scripts execution
...
/*! Specifies rflte script execution start time in us units  */
int16 rflte_offset_time:16;
/*Indicate if rflte script need to be executed  */
boolean rflte_script_execute;
/*! Specifies rflte script execution time in us units  */
uint16 rflte_exec_time:16;
...
typedef struct
{
  ...
  /* rfm path of each Rx path */
  rfm_path_type rfm_path[RFLTE_PATH_TYPE_MAX_RX];

  /* Un-used RX Path SNR should be filled with floored value */
  int32 snr_db10[RFLTE_PATH_TYPE_MAX_RX];
  
  /* Carrier ID */
  uint32 carrier_id; 

} rfa_rf_lte_per_cc_maat_rx_params_type_s;
...
typedef struct
{
  ...
  rfa_rf_lte_per_cc_maat_rx_params_type_s carrier[RFA_RF_LTE_MAX_CELL_SUPPORTED];

  /* VOLTE active */
  boolean  volte_active;
...
} rfa_rf_lte_per_cc_maat_tx_params_type_s;
...
typedef struct
{
  ...
  /* MAAT RX params for the carriers */
  rfa_rf_lte_per_cc_maat_tx_params_type_s carrier[RFA_RF_LTE_MAX_TX_CELLS_SUPPORTED];
