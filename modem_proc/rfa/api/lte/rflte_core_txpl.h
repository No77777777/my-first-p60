...
Copyright (c) 2010 - 2020 by QUALCOMM TECHNOLOGY, Inc.  All Rights Reserved.
...
$Header: //components/rel/rfa.mpss/4.11/api/lte/rflte_core_txpl.h#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
01/27/20   ssl     [CR 2610648] AMPR back off based on UE Power Class 
01/20/20   ssl     FR 55657: Replace RF Enter/Exit with RF Sleep/Wakeup for L+L DSDS awake
01/16/20   ssl     FR56108: Use p_emax_db10 from ML1 during MTPL instead of p_emax
12/23/19   ssl     Adding UE power class as input argument to rflte_core_txpl_set_sar_limit_index() API
08/20/18   pvsr    Assign Pemax to Max power from NV when NW doesn't signal the Pemax for MTPL computation and Support for pcmax_adjust under all scenarios
01/30/18   ks      KW Fix
02/10/17   sk      Fix initialisation of sar,coex and other backoffs to make sure max power works for power class 2.
02/10/17   sk      ue_power_class addition in mtpl_per_sf intf.
...
typedef enum
{
   RFLTE_CORE_POWER_CLASS1 = 0,
   RFLTE_CORE_POWER_CLASS2 = 1,
   RFLTE_CORE_POWER_CLASS3 = 2,
   RFLTE_CORE_POWER_CLASS_DEFAULT = RFLTE_CORE_POWER_CLASS3,
   RFLTE_CORE_POWER_CLASS4 = 3,
   RFLTE_CORE_POWER_CLASS_MAX = 4,
   RFLTE_CORE_POWER_CLASS_LEGACY_DEFAULT = 5,
...
typedef struct
{
  ...
  ** provided to FED (value ranges between 0-9) */
  ...
  /* P_emax_db10 for the subframe */
  int16 p_emax_db10[RFA_RF_LTE_MAX_TX_CELLS_SUPPORTED];
  /* Pe_Max for the SRS Sub frame */
  /* Co-Ex Max for the subframe */
  
    /* P_emax signaled by NW for the subframe */
  boolean p_emax_signaled_by_nw[RFLM_LTE_MAX_TX_NUM_CARRIER];
  
  int16 cxm_pmax_db10[RFA_RF_LTE_MAX_TX_CELLS_SUPPORTED];
  ...
  ** FALSE : If CA NS Back-off has not been requested by Network */
  ...
  /* power class type to control Pemax value */
  rflte_core_txpl_power_class_type power_class_type[RFA_RF_LTE_MAX_TX_CELLS_SUPPORTED];
...
/*--------------------------------------------------------------------------*/
int16 rflte_core_txpl_get_actual_vs_digital_tx_pwr_delta (uint8 sub_id);
/*--------------------------------------------------------------------------*/
int16 rflte_core_txpl_get_sar(uint8 sub_id);
/*--------------------------------------------------------------------------*/
int16 rflte_core_txpl_get_sar_pcmax_offset(uint8 sub_id);
/*--------------------------------------------------------------------------*/
void rflte_core_compute_p_cmax_update(uint8 sub_id,
                                      uint32 carrier_idx,
                                      uint8 subframe_num,
...
/*--------------------------------------------------------------------------*/
void rflte_core_compute_mtpl_per_sf(uint8   sub_id,
...
void rflte_core_txpl_enable_v2(uint8   sub_id,
                               uint32 carrier_idx,boolean enable, 
                               rfcom_lte_earfcn_type tx_channel, 
                               lm_handle_type txlm_buffer_idx);

/*--------------------------------------------------------------------------*/
void rflte_core_txpl_set_hdet_low_power_mode(uint8  sub_id,
                                             uint32 carrier_idx, 
                                              boolean enable, 
                                              lm_handle_type txlm_buffer_idx);
/*--------------------------------------------------------------------------*/
void rflte_core_txpl_set_mtpl_for_sg(uint8  sub_id,
                                     uint32 carrier_idx,
...
/*--------------------------------------------------------------------------*/
void rflte_core_txpl_do_mtpl_fw_reads_hdet_phy_rb(uint8 sub_id,
                                                 uint32 carrier_idx,
...
void rflte_core_txpl_do_fbrx_based_mtpl_update(uint8 sub_id,
                                               uint8 carrier_idx, 
                                               int16 txpl_txagc_val, 
                                               int16 fbrx_gain_err,
                                               uint8 phy_rb_block,
                                               ...
                                               uint8 sys_bw );  
/*--------------------------------------------------------------------------*/
int32 rflte_core_txpl_get_mtpl_for_logging(uint8 sub_id,uint32 carrier_idx);
/*--------------------------------------------------------------------------*/
int16 rflte_core_txpl_get_sar_limit_for_logging(uint8 sub_id,uint32 carrier_idx);
/*--------------------------------------------------------------------------*/
uint16 rflte_core_txpl_get_hdet_adc_for_logging(uint8 sub_id,uint32 carrier_idx);
/*--------------------------------------------------------------------------*/
int16 rflte_core_txpl_get_hdet_for_logging(uint8 sub_id,uint32 carrier_idx);
/*--------------------------------------------------------------------------*/
uint8 rflte_core_txpl_get_mpr_for_logging(uint8 sub_id,uint32 carrier_idx);
/*--------------------------------------------------------------------------*/
uint8 rflte_core_txpl_get_ampr_for_logging(uint8 sub_id,uint32 carrier_idx);
/*--------------------------------------------------------------------------*/
uint8 rflte_core_txpl_get_ns_flag_for_logging(uint8 sub_id,uint32 carrier_idx);
/*--------------------------------------------------------------------------*/
uint8 rflte_core_txpl_get_hdet_mode_for_logging(uint8 sub_id,uint32 carrier_idx);
/*--------------------------------------------------------------------------*/
boolean rflte_core_txpl_set_dac_setpoint(uint8  sub_id,
                                         uint32 carrier_idx, 
                                         uint16 dac_setpoint_override_enable, 
                                         uint16 dac_setpoint_value);
/*--------------------------------------------------------------------------*/
void rflte_core_txpl_set_sar_limit_index(uint8 sub_id,
                                         lm_handle_type txlm_buffer_idx,
                                         uint32 carrier_idx,
                                         rfcom_lte_earfcn_type tx_channel,
                                         rflte_core_txpl_power_class_type ue_power_class);
/*--------------------------------------------------------------------------*/
uint16 rflte_core_txpl_get_pdet_mode_swpt(uint8 sub_id,uint32 carrier_idx);
/*--------------------------------------------------------------------------*/
boolean rflte_core_txpl_get_low_power_mode_enabled(uint8 sub_id,uint32 carrier_idx);
/*--------------------------------------------------------------------------*/
int16 rflte_core_txpl_get_pcmax(uint8 sub_id);
/*--------------------------------------------------------------------------*/
uint16 rflte_core_txpl_get_hdet_threshold(uint8 sub_id,uint32 carrier_idx);
/*--------------------------------------------------------------------------*/
#ifdef FEATURE_RF_HAS_WTR3925
...
void rflte_core_txpl_update_param(uint32 carrier_idx, 
                                  ...
                                  lm_handle_type txlm_buffer_idx);
/*--------------------------------------------------------------------------*/
int16 rflte_core_txpl_get_min_tx_power_nv(uint8 sub_id,
                                          uint32 carrier_idx, 
                                          rfcom_lte_band_type rfcom_lte_band);
/*--------------------------------------------------------------------------*/
int16 rflte_core_txpl_get_nv_txpl(uint8 sub_id,uint32 carrier_idx);
/*--------------------------------------------------------------------------*/
boolean rflte_core_txpl_set_mtpl_for_coex_wlan(uint8 sub_id,int16 coex_limit_db10);
/*--------------------------------------------------------------------------*/
int32 rflte_core_xpt_path_delay_comp(uint8 sub_id,
                                     uint32 carrier_idx,
                                     rfcom_lte_earfcn_type tx_channel,
...
/*--------------------------------------------------------------------------*/
void rflte_core_txpl_set_tdd_config_backoff(uint8 sub_id,
...
void rflte_core_txpl_set_sar_tdd_cfg_backoff( uint16 tdd_config_limit_dB10, 
                                              lm_handle_type txlm_buffer_idx);
/*--------------------------------------------------------------------------*/
uint16 rflte_core_txpl_get_sar_tdd_cfg_backoff(uint8 sub_id,uint32 carrier_idx);
/*--------------------------------------------------------------------------*/
...
uint32 rflte_core_get_pwr_trking_loop_timer(uint32 carrier_idx, 
                                            lm_handle_type txlm_buffer_idx);
/*--------------------------------------------------------------------------*/
uint16 rflte_core_txpl_get_fbrx_gain_err_for_logging(uint8 sub_id,uint32 carrier_idx);
...
/*--------------------------------------------------------------------------*/
void rflte_core_txpl_update_nlic_per_sf(uint8    sub_id,
