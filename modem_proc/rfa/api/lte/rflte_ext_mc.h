...
$Header: //components/rel/rfa.mpss/4.11/api/lte/rflte_ext_mc.h#1 $
$DateTime: 2021/03/09 05:11:02 $

when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
01/30/20   ssl     [CR 2610317]Removal of un used variable from rflte_mc_bw_class_info_per_band_combo_s
01/20/20   ssl     FR 55657: Replace RF Enter/Exit with RF Sleep/Wakeup for L+L DSDS awake
12/30/19   ssl     FR 56309: RF WU in parellel to MCPM config for HA modem
09/26/18   ssl     FR 30850: AMPR Setting based on MCC and LTE band
...
/*--structure to hold ns value and ns ca value mask*/
typedef struct
{
...
boolean rflte_mc_tx_coex_wlan_set_txplim(uint8 sub_id,int16 coex_limit_db10) ;
...
void rflte_mc_light_wakeup(uint8 sub_id);

void rflte_mc_lte_clear_ccs_signal(uint8 sub_id);
...
void rflte_mc_fed_get_linear_tx_power(uint8 sub_id,
	                              rflte_mc_linear_tx_power_input_s* tx_power_input,
                                      rflte_mc_linear_tx_power_output_s* tx_power_output);

boolean rflte_mc_get_tuner_scenario_list(uint8 sub_id,
...
boolean rflte_mc_tuner_stop_mcl(uint8 sub_id,
	                        rfm_device_enum_type device,
                                rflte_mc_ext_carrier_type carrier_type,
                                uint32 channel,
                                rfm_tuner_handle_type tuner_handle);
