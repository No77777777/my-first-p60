...
$Header: //components/rel/rfa.mpss/4.11/api/common/rfm_umid_list.h#1 $

when       who    what, where, why
--------   ---    ---------------------------------------------------------- 
01/24/20   moh     RF MDUMP Changes
02/17/15   zhh   Added the new structures for AMAM based autoPin calculation
01/20/15   px     Added new structures for rffe scan device dump feature
...
#define RFA_RF_COMMON_MSG_ID_CALC_AUTO_PIN_POUT           0x03

/* Below Constant is used while updating the MAAT info in MAAT MGR*/ 
#ifdef FEATURE_RFA_ACL
#define RFA_RF_COMMON_MSG_ID_MAAT_UPDATE_IND              0x04
#endif

#ifdef FEATURE_QSH_MDUMP
#define RFA_RF_COMMON_MSG_ID_QSH_CALLBACK_IND              0x04
#endif
/*---------------------------------------------------------------------------*/
/*
  @brief
...
/*
  @brief
  This is the structure of the AMAM based autoPin calculation that RF dispatches to apps task
...
typedef struct
{
  msgr_hdr_struct_type          hdr;
  rffe_scan_efs_dump_payload_type payload;
} rfm_rf_cmn_calc_auto_pin_pout_req_s;
...
/*
  @brief
  This is the structure which contains QSH callback params for RF client which RFM dispatches to apps task
*/
typedef struct
{
   /* MSGR header */
...
enum
{
  ...
  MSGR_DEFINE_UMID(RFA, RF_COMMON, REQ, CALC_AUTO_PIN_POUT, RFA_RF_COMMON_MSG_ID_CALC_AUTO_PIN_POUT, rfm_rf_cmn_calc_auto_pin_pout_req_s),
};
...
#ifdef FEATURE_RFA_ACL
/* MSGR UMID for MAAT UPDATE IND */
enum
{
  MSGR_DEFINE_UMID(RFA, RF_COMMON, IND, MAAT_UPDATE_IND, RFA_RF_COMMON_MSG_ID_MAAT_UPDATE_IND, rfm_rf_cmn_maat_update_ind_s),   
};
#endif
