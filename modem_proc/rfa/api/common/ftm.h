...
$Header: //components/rel/rfa.mpss/4.11/api/common/ftm.h#1 $
...
09/28/15   sku     Renamed variables to 'Rx path select' instead of 'Antenna Select' 
09/22/15  jps/al   Add support for FTM_TDSCDMA_FBRX_SET_RX_TX_DELAY
09/03/15   sku     Added support for simultaneous PRx/DRx IQ capture.
09/03/15   ska     Adding support for ULCA Non-Signaling FTM Support
08/21/15   sc      Deprecate subscription 3 reducing memory footprint
08/11/15   zhw     Addes support for HDR in FTM RF TEST
07/27/15   hj      Modify ftm_get_tune_code_status_type array size in ftm_get_tune_code_response_type. 
                   Include rfc_class.h for RFC_MAX_DEVICE_INSTANCES_PER_BAND defination
07/09/15   yb      Fix KW error
...
06/21/15   dbc     Added support for pa_tunable_cap_offset
06/16/15   yb      Added support for get tune code API
06/08/15   stw     Changes to support 32 bit channel in SET_CHAN
05/28/15   ra      Add support for APQ 8996
05/20/15   sc      Add FTM GSM enh debug dispatcher command
05/19/15   sdu     Added FTM interface for FTM_SET_PATH_SEL_VERIFICATION_INDEX
...
04/02/15   zbz     Increase FTM_RF_MEAS_TX_PARAMS_MAX_PAYLOAD_SIZE to 61
03/27/15    br     Added support for NLIC params override
03/25/15   jmf     [selftest] Update Self-Test Payload Return max size
03/03/15    br     Added embedded support for Sensitivity/CNR calculation
02/20/15    br     Added FTM interface for FTM_RF_IQ_CAPTURE_ANALYZE_SAMPLES() Command.
12/15/14   ska     Fixed OFT Compilation Errors
12/12/14   stw     Add Multiple Scell Support in NS mode
12/5/14    ska     Added FTM API for NLIC characterization
...
{
  boolean mode;
  uint32 fbrx_rx_tx_delay;
}ftm_tdscdma_fbrx_set_rx_tx_delay_data_type;
...
/* Tuner get tune code data structure as per the ICD document */
typedef PACK(struct)
{
  ftm_mode_id_type        mode;
  uint8                   device_type;
}ftm_tuner_get_tune_code_data_type;
...
typedef PACK(struct){
  ...
  boolean is_tuner_enabled;  /* Is NVG active */
...
typedef PACK(struct){
  ...
  uint2 ftm_cmd;
  uint8 num_tuners;
  ftm_get_tune_code_status_type tune_code_status[RFC_MAX_DEVICE_INSTANCES_PER_BAND];
} ftm_get_tune_code_response_type;

/*FTM_GET_TUNER_STATUS */
typedef PACK(struct){
  uint8 tuner_id;            /* Index of tuner from which tune code is read */
  uint8 tune_code_size;      /* actual size of tune code*/
  uint8 tune_code[8];        /* 8 is the max size of tune code*/
  boolean nvg_status;        /* Is NVG active */
  boolean detune_status;     /* Not used*/
} ftm_tuner_status_data_type;
...
typedef PACK(struct){
  ...
  uint2 ftm_cmd;
  uint8 num_tuners;
  ftm_tuner_status_data_type tuner_status[RFC_MAX_DEVICE_INSTANCES_PER_BAND];
} ftm_tuner_status_resp_pkt_type;
...
typedef PACK(struct){
  ...
  uint8 tune_code_size;  /* actual size of tune code*/
  uint8 tune_code[8];    /* 8 is the max size of tune code*/
  boolean nvg_enable;    /* 1 -Enable the NVG ,0 -Disable the NVG */
} ftm_tuner_program_data_type;

typedef PACK(struct){
  uint8 num_tuners;
  ftm_tuner_program_data_type tuner_program_data[RFC_MAX_DEVICE_INSTANCES_PER_BAND];
} ftm_tuner_program_req_pkt_type;

typedef PACK(struct){
  uint1 cmd;
  uint1 sub_sys;
  uint2 ftm_cmd;
  boolean status[RFC_MAX_DEVICE_INSTANCES_PER_BAND];
} ftm_tuner_program_resp_pkt_type;
...
typedef PACK(struct)
{
  ...
  uint32 sample_size; // Number of samples to be acquired
  byte carrier_id;  // Carrier whose samples are to acquired , Note: Antenna chain will be determined by sub sys cmd code in the COMMAND.
  uint16 select_rx_chain;     // bit mask to select Rx chain.
}ftm_iq_capture_acquire_prxdrx_samples_type;
...
typedef PACK(struct)
{
  ...
  uint32 sample_offset;  //  Offset (in unit of IQ pairs) at which samples are collected
  uint32 sample_size;    // Number of samples (IQ-pairs) that are to be collected
  byte antenna_sel;     // flag to choose PRx/DRx IQ samples to fetch
} ftm_iq_capture_get_prxdrx_samples_type;
...
typedef PACK(union)
{
  ...
  ftm_iq_capture_acquire_prxdrx_samples_type ftm_iq_capture_acquire_prxdrx_samples;

  ftm_iq_capture_get_prxdrx_samples_type ftm_iq_capture_get_prxdrx_samples;  // GET SAMPLESSS for PRX/DRX
...
}ftm_iq_capture_get_prxdrx_samples_rsp_pkt_type;
...
#define FTM_RF_MEAS_TX_PARAMS_MAX_PAYLOAD_SIZE 64
...
//  Version control enum for NLIC override params
typedef enum
{
  RFLTE_NLIC_OVERRIDE_PARAMS_INFO_VER_A   = 0x0A,
  RFLTE_NLIC_OVERRIDE_PARAMS_INFO_INVALID = 0xFF
}rflte_nlic_override_params_info_version_type;

//Action type enum for each of the NLIC override params
typedef enum
{
  RFLTE_NLIC_OVERRIDE_ACTION_NONE         = 0x0,
  RFLTE_NLIC_OVERRIDE_ACTION_OVERRIDE_VAL = 0x1
}rflte_nlic_override_action_type;

// NLIC override params version-A

typedef PACK(struct)
{
  /*Action for nlic_on_off */
  rflte_nlic_override_action_type nlic_on_off_action;
  /*NLIC On-Off value */  
  uint8 nlic_on_off; 
  /* Action for nlic_delay_val */
  rflte_nlic_override_action_type delay_val_action;
  /*NLIC delay value */  
  int16 delay_val;   
  
  /* Action for kernel_sel */
  rflte_nlic_override_action_type kernel_sel_action;
  /*NLIC kernel to be selected  */  
  uint16 kernel_sel;   
  
   /*Action for Rx path selection*/
  rflte_nlic_override_action_type rx_path_sel_action;
  /*Rx path selection value */  
  uint8 rx_path_sel; 

  /* Action for pcc_band */
  rflte_nlic_override_action_type pcc_band_action;
  /*PCC Band Enum value*/
  uint8 pcc_band;

  /* Action for scc_band */
  rflte_nlic_override_action_type scc_band_action;
  /*SCC Band Enum value */
  uint8 scc_band;

  /* Action for victim carrier index */
  rflte_nlic_override_action_type victim_carrier_index_action;
  /*Victim Carrier Index*/
  uint8 victim_carrier_index;

  /* Action for Tx power threshold */
  rflte_nlic_override_action_type tx_pwr_threshold_action;
  /* Tx Power threshold */
  int8 tx_pwr_threshold;      

  /* Action for linear_m */
  rflte_nlic_override_action_type linear_m_action;
  /* Linear m value*/
  int8 linear_m;

  /* Action for linear_q */
  rflte_nlic_override_action_type linear_q_action;    
  /* Linear q value */
  int16 linear_q;  
  
}rflte_nlic_override_params_info_data_type_v_a;

// NLIC override params provided by diag/tools 

typedef PACK(struct)
{
  /* NLIC override Structure determined by version in use*/
  uint8 version; 

  union   {
    rflte_nlic_override_params_info_data_type_v_a nlic_params_ver_a;
  };
  
}rflte_nlic_override_params_type;


#ifdef FEATURE_LTE
typedef PACK(struct)
{
   /* Wrap an unpacked structure */
   rflte_mc_device_info_type lte_device_per_carr[RFLTE_MC_MAX_CELL_IDX_SUPPORTED];
}ftm_lte_device_info_arr_type;
...
typedef PACK(struct)
{
    ...
    PACK(union)
    {
	    ...
	    ftm_tuner_get_tune_code_data_type ftm_tuner_get_tune_code_data;
	    ftm_tdscdma_fbrx_set_rx_tx_delay_data_type ftm_tdscdma_fbrx_set_rx_tx_delay;
...
extern boolean ftm_gsm_ber_cleanup( rfm_device_enum_type remapped_rx_device );
...
/*---------------------------------------------------------------------------*/
/*! @struct ftm_gsm_cmd_api_cmn_driver_type                                  */
/*! @brief structure for the common RF GSM API driver input arguments        */
/*! @param primary_device - Primary Rx/Tx device                             */
/*! @param drx_device - Diversity Rx device                                  */
/*! @param prx_drx_mode - PRx-only, DRx-only or PRx + DRx                    */
/*! @param rf_sequence_number - overrun detection                            */
/*! @param triple_buffer_index - GL1 managed buffer for memory management    */
typedef PACK(struct)
{
  ...
  uint8 triple_buffer_index;
}ftm_gsm_cmd_api_cmn_driver_type;



/*---------------------------------------------------------------------------*/
/*! @struct ftm_gsm_cmd_api_asd_driver_type                                  */
/*! @brief structure for the asd RF GSM API driver input arguments           */
/*! @param ant_pos - position value to switch the antenna to                 */
typedef PACK(struct)
{
  uint8 ant_pos;
}ftm_gsm_cmd_api_asd_driver_type;
...
/*! @struct ftm_gsm_cmd_api_asd_driver_type                                  */
/*! @brief structure for the asd RF GSM API driver input arguments           */
/*! @param slot_active - Rx burst is active or not determined by the tool    */
/*! @param rx_band - GSM band burst is tuning for                            */
/*! @param rx_arfcn - GSM channel the burst is tuning for                    */
/*! @param rx_lvl_dbm - expected Rx level in dBm (tool passes postive value) */
/*! @param burst_type - Rx, Mon, Tx                                          */
/*! @param rx_burst_type - Sync, SCH, ACQ, MON, PCH                          */
/*! @param probe_burst - if monitor is a probe burst                         */
/*! @param enh_rx_enable_flag - if LIF is enabled                            */
/*! @param enh_rx_offset_flag - frequency to offset PLL for LIF              */
/*! @param blanked - if slot is blanked                                      */
/*! @param rf_sequence_number - overrun detection                            */
typedef PACK(struct)
{
  uint32  slot_active;
  ...
  uint8   rx_burst_type;
  uint8   probe_burst;
  uint32  enh_rx_enable_flag;
  uint32  enh_rx_offset_flag;
  uint32  blanked;
  uint32 rf_sequence_number;
}ftm_gsm_cmd_api_rx_slot_params_type;



/*---------------------------------------------------------------------------*/
/*! @struct ftm_gsm_cmd_api_rx_driver_type                                   */
/*! @brief structure for the Rx burst RF GSM API driver input arguments      */
/*! @param enable_drx - flag used to enable/disable RxD                      */
/*! @param enable_gl1_lower - whether to lower CCS task priority             */
/*! @param rx_freq_error - frequency error reported by GL1 to offset PLL     */
/*! @param slot_params - params for all slots                                */
/* @TODO max Rx bursts needs to use a #define                                */
typedef PACK(struct)
{
  uint32 enable_drx;
  uint32 enable_gl1_lower;
  uint16 rx_freq_error;
  ftm_gsm_cmd_api_rx_slot_params_type slot_params[8];
}ftm_gsm_cmd_api_rx_driver_type;



/*---------------------------------------------------------------------------*/
/*! @struct ftm_gsm_cmd_api_tx_slot_params_type                              */
/*! @brief Per Tx slot variables                                             */
/*! @param slot_active - Tx slot is active or not determined by the tool     */
/*! @param slot_pcl - Slot power control level targeted                      */
/*! @param slot_vbatt_backoff - vbatt backoff to apply to Tx agc             */
/*! @param slot_desense_backoff - desense backoff to apply to Tx agc         */
/*! @param slot_modulation - modulation type of the slot                     */
typedef PACK(struct)
{
  boolean slot_active;
  uint16 slot_pcl; 
  uint16 slot_vbatt_backoff; 
  uint16 slot_desense_backoff; 
  uint8  slot_modulation; // GMSK, 8-PSK  or Unknown either 0, 1 or 2 respectively
}ftm_gsm_cmd_api_tx_slot_params_type;



/*---------------------------------------------------------------------------*/
/*! @struct ftm_gsm_cmd_api_tx_driver_type                                   */
/*! @brief structure for the Tx burst RF GSM API driver input arguments      */
/*! @param tx_band - GSM band burst is tuning for                            */
/*! @param tx_arfcn - GSM channel the slot is tuning for                     */
/*! @param tx_freq_error - frequency error reported by GL1 to offset PLL     */
/*! @param slot_params - params for all slots                                */
/* @TODO max Tx slots needs to use a #define                                 */
typedef PACK(struct)
{
  uint8 tx_band;
  uint16 tx_arfcn;
  uint16 tx_freq_error;
  ftm_gsm_cmd_api_tx_slot_params_type slot_params[5];
}ftm_gsm_cmd_api_tx_driver_type;



/*---------------------------------------------------------------------------*/
/*! @struct ftm_gsm_cmd_api_cmn_driver_type                                  */
/*! @brief structure for the Antenna Position RF GSM API driver input args   */
typedef PACK(struct)
{
   uint8 antenna_position;
   uint8 device;
}ftm_gsm_cmd_api_antenna_driver_type;
...
{
  uint32 intf_idx;
}ftm_gsm_cmd_api_rflm_driver_type;



/*---------------------------------------------------------------------------*/
/*! @struct ftm_gsm_enh_debug_pkt_type                                       */
/*! @brief packet definition for the FTM GSM enh debug dispatcher            */
/*! @details this defines the input data that can come in the form of        */
/*!          send sync command                                               */
typedef PACK(struct)
{
  ftm_diagpkt_subsys_header_type diag_hdr;
  ...
  union
  {
    uint32 q6_clock_speed_khz;
    uint32 bimc_rate_kbps;
    ftm_gsm_cmd_api_cmn_driver_type ftm_gsm_cmn_details;
    uint32 state_machine_input;
  }ftm_gsm_cmn_params;

  union
  {
    ftm_gsm_cmd_api_rx_driver_type  ftm_gsm_rx_details;
    ftm_gsm_cmd_api_tx_driver_type  ftm_gsm_tx_details;
    ftm_gsm_cmd_api_antenna_driver_type ftm_gsm_antenna_details;
    ftm_gsm_cmd_api_rflm_driver_type ftm_gsm_rflm_details;
  }ftm_gsm_params;
}ftm_gsm_enh_debug_pkt_type;
...
typedef PACK(struct)
{
    ...
    union {
        ...
        uint8 paRange;
        /* This is the payload information for FTM_SET_PA_TUNABLE_CAP */
        uint8 pa_tunable_cap_offset;
        ...
        /* This is the payload information for FTM_LTE_NS_ENABLE_MULTI_SCELL_RX_TX*/
        ftm_lte_ns_enable_multi_scell_rx_tx_req_data_type enable_multi_scell_rx_tx_req_data;  
        /* This is the payload information for FTM_LTE_NS_DISABLE_RX_TX_MULTI_SCELL*/
        ftm_lte_ns_disable_multi_scell_req_data_type disable_multi_scell_rx_tx_req_data;      
        /* This is the payload information for FTM_LTE_NS_ACTIVE_DL_CARRIER_MASK*/
        ftm_lte_ns_active_dl_carrier_mask_req_data_type active_dl_carrier_mask_req_data; 		
        ...
        ftm_config_deconfig_rx_chain_type ftm_config_deconfig_rx_chain;
        /* Payload for FTM_SET_ALT_PATH_OVERRIDE_INDEX*/
        rfa_rf_lte_ftm_alt_path_oride_params_s rfa_rf_lte_alt_path_override_params_data;

        /*NLIC override data structres*/
        rflte_nlic_override_params_type nlic_override_data;
#ifdef FEATURE_LTE
        /*FTM carrier to device map*/
        ftm_lte_device_info_arr_type ftm_lte_device_map;
         /* Payload for FTM_SET_PATH_SEL_VERIFICATION_INDEX*/
        rfa_rf_lte_ftm_alt_path_verif_params_s rfa_rf_lte_alt_path_verif_params_data;
#endif        
    };
...
FUNCTION FTM_QSH_MDUMP
	
DESCRIPTION
  This API is used to dump the ftm variable using qsh framework.
