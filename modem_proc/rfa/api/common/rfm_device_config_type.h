...
$Header: //components/rel/rfa.mpss/4.11/api/common/rfm_device_config_type.h#2 $

when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
09/02/22   tej     FR69305 To add support for new error codes for QMI_DMS_SET_OPERATING_MODE
10/30/17   pv      Make RFM band bit mask size flexible to incorporate addition of new bands.
...
#define RFM_BANDS_MASK_ARR_SZ ( ( (uint8)( (SYS_BAND_CLASS_MAX - 1) / 64 ) ) + 1 )

/*! @brief Enumeration of failure reason for RF initialization 
*/
typedef enum
{
  RFM_INIT_FAILURE_INVALID,
  /**< Invalid value - Set 0 as the invalid value of the error code */

  RFM_INIT_FAILURE_NONE,
  /**< No RF failure - set when RFM initialization status is successful */
...
typedef struct 
{
  uint64 mask[ RFM_BANDS_MASK_ARR_SZ ];
}rfm_bands_bitmask;
