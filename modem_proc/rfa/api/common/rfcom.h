...
/*===========================================================================
  ...
  $Header: //components/rel/rfa.mpss/4.11/api/common/rfcom.h#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
02/07/20   hari    [CR 2617607] Added Gnss band enum.
10/11/17   vp      Changed the enum value of B71 to unblock CRM releae
10/26/17   sk      Enable LTE B71 support
12/23/15   jhap      LTE Band 66 support
11/03/15   sml     New function for device to path mapping for LTE
10/14/15   mah     Added enum to support LTE ca modes
10/07/15   rs      Updated the CDMA Band 20 in the CDMA Band list 
09/28/15   yb      Update NV definition
09/13/15   jhp     Added rfcom_lte_ca_mode_type
09/10/15   yb      Fix Pack structure for NV
09/03/15   yb      Correct param definition
09/02/15   yb      Moved vector 16 definition
07/27/15   hj      Added RFC_MAX_DEVICE_INSTANCES_PER_BAND definition
05/14/15   stw     Added support for LTE bands 252 to 255
05/13/15   dps     Added support for power logging using rfdevice_wtr_config_bitmask
04/21/15   ac      rxagc changes for w+w
...
/* Number of RF device instances per band from rfc file */
#define RFC_MAX_DEVICE_INSTANCES_PER_BAND 7
...
#define RFCOM_MULTI_CARRIER_ID_ALL ((1<<RFCOM_MULTI_CARRIER_ID__0)|( 1<<RFCOM_MULTI_CARRIER_ID__1)|( 1<<RFCOM_MULTI_CARRIER_ID__2)|(1<<RFCOM_MULTI_CARRIER_ID__3))
...
/* Enums for supported WLAN bands */
typedef enum 
{
  RFCOM_BAND_WLAN_2P4GHZ,
  RFCOM_BAND_WLAN_5GHZ,
  RFCOM_NUM_WLAN_BANDS,
  RFCOM_BAND_WLAN_INVALID
}rfcom_wlan_band_type;
...
typedef enum 
{
  ...
  RFCOM_BAND_LTE_B71 = 58, /*To keep In line with ATLAS definition*/
  RFCOM_NUM_LTE_BANDS,
...
/*! 
    @brief
    Enum contains the GNSS blanking types with respect to bands
*/

typedef enum 
{
  RFLTE_GNSS_BLANK_INVALID,
  RFLTE_GNSS_L1_BLANK_TDD= 1,
  RFLTE_GNSS_L1_BLANK_MTJ,
  RFLTE_GNSS_L1_BLANK_IDC,
  RFLTE_GNSS_L1_BLANK_MTJ_IDC,
  RFLTE_GNSS_L5_BLANK_TDD,
  RFLTE_GNSS_L5_BLANK_MTJ,
  RFLTE_GNSS_L5_BLANK_IDC,
  RFLTE_GNSS_L5_BLANK_MTJ_IDC,
  RFLTE_GNSS_L1_L5_BLANK_TDD,
  RFLTE_GNSS_L1_L5_BLANK_MTJ,
  RFLTE_GNSS_L1_L5_BLANK_IDC,
  RFLTE_GNSS_L1_L5_BLANK_MTJ_IDC, 
  RFLTE_GNSS_BLANK_MAX
} rflte_mc_ext_gnss_blanking_type;
...
/* Enums for supported CDMA bandwidths */
typedef enum 
{
  RFCOM_BW_CDMA_1P4MHz,
  RFCOM_BW_CDMA_3MHz,
  RFCOM_BW_CDMA_5MHz,
  RFCOM_BW_CDMA_10MHz,
  RFCOM_NUM_CDMA_BWS,
  RFCOM_BW_CDMA_INVALID
} rfcom_cdma_bw_type;

/* Enums for supported WCDMA bandwidths */
typedef enum 
{
  RFCOM_BW_WCDMA_3MHz,
  RFCOM_BW_WCDMA_5MHz,
  RFCOM_BW_WCDMA_10MHz,
  RFCOM_BW_WCDMA_15MHz,
  RFCOM_NUM_WCDMA_BWS,
  RFCOM_BW_WCDMA_INVALID
} rfcom_wcdma_bw_type;

/* Enums for supported TDSCDMA bandwidths */
typedef enum 
{
  RFCOM_BW_TDSCDMA_1P4MHz,
  RFCOM_NUM_TDSCDMA_BWS,
  RFCOM_BW_TDSCDMA_INVALID
} rfcom_tdscdma_bw_type;
...
typedef union
{
  ...
  rfcom_cdma_bw_type cdma_bw;
  rfcom_wcdma_bw_type wcdma_bw;
  rfcom_tdscdma_bw_type tdscdma_bw;
...
/*! 
  @brief
  Structure for doing the vector operations on 16bit signed interger
*/
typedef struct
{
  int8 real;
...
/*! 
  @brief
  Structure for Tuner static data
*/
typedef PACK (struct)
{
  /* Enum indicating closed loop type */
  uint8 algo_type;

  /* Band Mask indicating CL should be enabled or not in CA mode
       The tech band NV represents the PCELL and following mask represents the SCELL band */
  uint64 multi_carrier_cl_mask;

  /* Threshold for ACL to enter lock state */
  int16 acl_lock_threshold;

  /* Threshold for ACL to exit lock state */
  int16 acl_unlock_threshold;

  /* Scenario to be applied when AOL to ACL transitions */
  uint8 acl_ol_start_scenario;

  /* Scenario index from which ACL calculations should start */
  uint8 acl_params_start_idx;

  /* Scenario index at which ACL calculations should stop */
  uint8 acl_params_stop_idx;

  /* Offset to be added */
  uint16 acl_params_offset_correction[3];

  /* Free space reference */
  uint16 acl_params_fs_reference[3];

  /* Free space offset tolerance */
  uint8 acl_fs_offset_tolerance;

  /* Aperture mask */
  uint8 aperture_mask[2];
}rfcommon_atuner_static_params_data_type;
