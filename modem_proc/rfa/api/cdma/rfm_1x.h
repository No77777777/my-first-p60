...
/*==============================================================================
  ...
  $Header: //components/rel/rfa.mpss/4.11/api/cdma/rfm_1x.h#1 $
...
03/30/15   zhw     API Name touch up
03/23/15   wwl     Change wakeup TX v2 function to include PRX device
03/06/15   wwl     Add new API function prototypes to split RX TX wakeup, retune
                   and action time control functionalities
...
/*----------------------------------------------------------------------------*/
 /*! Macro defining invalid EcIo value that will be provided to MAAT when
     FLDRx is enabled*/
  
#define RFM_1x_MAAT_INVALID_C2K_EcIo 0x7FFF
...
(
  ...
  const rfm_device_enum_type prx_device,
  const lm_handle_type txlm_handle,
  const rfm_cb_handler_type callback,
  void* const userdata
);
