...
$Header: //components/rel/rfa.mpss/4.11/api/wcdma/rfwcdma_mc.h#1 $
...
06/16/15   vbh     Update rfwcdma_mc_validate_state to accept an enum specifying the API
...
05/08/16   ak      [Rx/Tx Split]Support to Cal 6 Rx Devices
04/21/09   ac      w plus w rxagc bringup chnage
04/09/15   vbh     Update the power ON/OFF API interface
04/09/15   vbh     [Rx/Tx split] Update the enable tx interface
04/02/15   vbh     Added a mc wakeup Tx routine
04/23/15   rmb     Add a new api for handle based cleanup of WCDMA mdsp data.
02/27/15   sd      OFT error on TH1.0.1 mc state missing param init_once_flag
02/11/15   dw      Added common_enter/exit and power_on_off, removed a few obsolete functions
02/09/14   dw      Added common enter/exit routine
...
#define RFWCDMA_MAX_RX_DEVICES RFM_DEVICE_6
...
/*! @brief WCDMA State indicating completion of building of scripts for Rx Tune */
  ...
  rfwcdma_mc_disable_wcdma_tx() and is exited by either rfwcdma_mc_sleep() or
...
/*! @brief WCDMA State indicating completion of building of scripts for Rx Tune. 
  ...
  /*! @brief WCDMA Sleep state.  This state is entered by rfwcdma_mc_sleep()
...
/*
  Enum to indicate the antenna which is "forced" to be enabled from SAR NV
*/
typedef enum
{
  RFWCDMA_MC_SAR_ANTENNA_NONE,                /* no SAR forced selection */
  RFWCDMA_MC_SAR_ANTENNA_0,        /* pass through position */
  RFWCDMA_MC_SAR_ANTENNA_1,             /* swapped position */
  RFWCDMA_MC_SAR_ANTENNA_MAX,                 /* enum limit for ant conditions */
} rfwcdma_mc_sar_forced_antenna_enum_type;
...
typedef struct
{
  ...
  uint32 rx_multi_carrier_idx[RFM_MAX_WAN_DEVICES];
  
  /* Varibale to hold the last multi_carrier index on a device.
   Used to stop AGC on all applicable carriers during tune */

  uint32 rx_multi_carrier_idx_shadow[RFM_MAX_WAN_DEVICES];

  uint32 tx_multi_carrier_idx;
  
  rfcom_multi_carrier_hspa_id_type rx_carriers[RFM_MAX_WAN_DEVICES];
  ...
  uint16 asm_status_bit_map[RFM_MAX_WAN_DEVICES]; 
  
  /*! Tuner status bit map for a RFM device */
  uint16 tuner_status_bit_map[RFM_MAX_WAN_DEVICES]; 
...
/*! Remember primary and diversity Rx devices for DRDs scenario*/
  ...
  /* Tracks the device that has entered */
  uint32 device_enter_mode_mask;

  /* Tracks device power on mask, for debugging purpose*/
  uint32 device_power_on_mask;

  /* Variable to track the device type */
  rfm_wcdma_device_type device_type[RFM_MAX_WAN_DEVICES];
...
typedef enum
{
  ...
  RFWCDMA_MC_SET_MCS_TX_PWR_LIMIT,
  ...
  RFWCDMA_MC_GET_MCS_TX_PWR_LIMIT,
...
typedef rfm_wcdma_rxagc_input_params rfwcdma_mc_rxagc_input_params;

/*----------------------------------------------------------------------------*/

typedef rfm_wcdma_rxagc_output_params rfwcdma_mc_rxagc_output_params;
...
/*----------------------------------------------------------------------------*/
extern boolean rfwcdma_mc_init_tx(rfcom_device_enum_type device, 
                                  uint32 txlm_buf_idx, 
                                  uint32 multi_carrier_idx,
                                  uint16 tx_channel[RFM_WCDMA_MAX_CARRIERS]);
/*----------------------------------------------------------------------------*/
extern boolean rfwcdma_mc_tx_wakeup( rfm_device_enum_type device, 
                           uint32 txlm_buf_idx, 
                           uint32 multi_carrier_idx,
                           uint16 tx_channel);
...
(
 uint32 device_mask,
 boolean skip_agc_disable[RFM_MAX_WAN_DEVICES], 
 uint32 rxlm_buf_idx[RFM_MAX_WAN_DEVICES]
);
/*----------------------------------------------------------------------------*/
rfm_api_status_t
...
/*----------------------------------------------------------------------------*/
int32 rfwcdma_mc_tx_rx_freq_ratio(uint16 channel);
/*----------------------------------------------------------------------------*/
boolean
...
void 
rfwcdma_mc_asdiv_elna_callback_handler
(
  void *data, 
  uint32 *ant_position 
);
...
void
rfwcdma_mc_asdiv_qat_callback_handler
( 
  uint32 *ant_position
);
...
/*!
  ...
  Bandmask
*/
uint64 rfwcdma_mc_get_band_mask ( rfcom_device_enum_type device );
