...
$Header: //components/rel/rfa.mpss/4.11/api/wcdma/rfm_wcdma.h#1 $
$DateTime: 2021/03/09 05:11:02 $
$Author: pwbldsvc $

when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
01/13/20   nik     FEATURE_WCDMA_DB_DC_HSDPA is disabled in HA - Pass device to rfm_write_tech_dependent_nv_items_to_mdsp
...
/*--------------------------------------------------------------------------------------------------------------------*/
/*! Structure to store Tx AGC parameters per carrier for Modem Assisted Antenna Tuning (MAAT) */
typedef struct
{
  /*Tx channel of the carrier*/  
  uint16 tx_channel;

  /* rfm path of each Tx path */
  ...
  /* Carrier ID */
  uint8 carrier_id; 
} rfa_wcdma_per_cc_maat_tx_params_type_s;
...
typedef struct
{
  ...
  /* MAAT TX params for the carriers */
  rfa_wcdma_per_cc_maat_tx_params_type_s carrier[RFM_WCDMA_CARRIER_MAX];
} rfa_wcdma_maat_tx_info_type_s;
