...
$Header: //components/rel/rfa.mpss/4.11/device/rfdev_intf/inc/rfdevice_lte_interface.h#1 $

when       who     what, where, why
--------   ---     ---------------------------------------------------------
05/25/20   up      Store RX and tx path info sub_id specifics
12/08/16   ry      Added API to build hkadc clk buff override script
05/13/16   ry      Changes for LTE 1Rx FR
01/19/16   ry      Updated the ICI support API
10/12/15   mah     40 Mhz Intra-ULCA HDET compensation support
10/11/15   mah     Interface to check if band support on a device is over ICI
10/09/15   vkm     WTR ICI support: pass the correct CA mode
09/28/15   mg      TDD HORXD LIF changes
09/25/15   sb      FR30081:Apply gain offsets for ASDIV switch position change
09/17/15   sma     Added support for DMCS (Deep mode control sleep)
09/16/15   sd      Added support for Rx AGC reload
09/16/15   bm      Add back interface for get_hdet40_scaling_lut() API
08/20/15   stw     FR 26008 : SCC1 SCC2 Non-Contiguous CA
08/03/15   bm      Added rfdevice_lte_tx_get_dac_backoff() API
                   Renamed rfdevice_lte_tx_get_single_rb_data API
07/20/15   aks/mg  Interface changes to get device list per carrier and current
                   rx operation mode from tech layer
07/15/15   bm      Added get_hdet40_scaling_lut() API interface
...
/*----------------------------------------------------------------------------*/
boolean rfdevice_lte_rx_get_agc_reload_script(
  rfcom_lte_band_type rf_band,
  uint32 freq_khz,
  rfcom_lte_bw_type rfcom_bw,
  rfm_device_enum_type rfm_device,
...
/*----------------------------------------------------------------------------*/
boolean rfdevice_lte_get_hkadc_override_script(
  rfm_device_enum_type device, 
  rfcom_lte_band_type rf_band, 
  rfdevice_lte_script_data_type* script_data_ptr,
  rfdevice_lte_tx_mcs_transition_enum_type mcs_trans   
);
...
/*----------------------------------------------------------------------------*/
boolean rfdevice_lte_rx_get_rx_inter_ca_support(
 rfm_device_enum_type device,
...
/*----------------------------------------------------------------------------*/
boolean rfdevice_lte_rx_get_ici_path_info(
  rfm_device_enum_type device,
  rfcom_lte_band_type  band, 
...
/*----------------------------------------------------------------------------*/
boolean rfdevice_lte_dmcs_enable
(
  rfm_device_enum_type device, 
  rfcom_lte_band_type rf_band,
  boolean enable 
...
boolean rfdevice_lte_tx_pwr_wakeup_v2
(
  rfm_device_enum_type device, 
  rfcom_lte_band_type rf_band,
  rfm_device_enum_type prev_device, 
  rfcom_lte_band_type prev_rf_band

);

boolean rfdevice_lte_rx_pwr_wakeup_v2
(
  rfm_device_enum_type device, 
  rfcom_lte_band_type rf_band,
  rfm_device_enum_type prev_dev,
...
/*----------------------------------------------------------------------------*/
uint16 *
rfdevice_lte_tx_get_hdet40_scaling_lut
(
  rfm_device_enum_type rfm_device,
  rfcom_lte_band_type rf_band,
...
/*----------------------------------------------------------------------------*/
boolean
rfdevice_lte_rx_get_lna_bbf_on_off_script
(
  rfm_device_enum_type device, 
  rfcom_lte_band_type rf_band,
  boolean *on_off,
