...
$Header: //components/rel/rfa.mpss/4.11/device/rfdev_intf/inc/rfdevice_asm_intf.h#1 $

when       who   what, where, why
--------   ---   ---------------------------------------------------------------
09/14/15   hm    fixed issues while testing the GRFC Tuner 
08/04/15   sw    Dont program XSW if already done by GRF 
...
extern "C" {
  ...
  boolean rfdevice_asm_update_scenario(uint8 scenario_num);

  /*----------------------------------------------------------------------------*/

  boolean rfdevice_asm_update_state(rfdevice_asm_cfg_type *asm_cfg_p,
                                    void *device_ptr,
                                    void *buff_obj_ptr,
                                    rf_device_execution_type dev_action,
                                    int16 script_timing);
