...
/*===========================================================================
  ...
  $Header: //components/rel/rfa.mpss/4.11/rf/task/rex_msgr/src/rf_rex_msgr_interface.c#1 $
...
/* for rf_task_2 which is for subscriber 2 a different set of UMIDs need to be registered for GL1<->RF communication*/
    ...
    ret_status &= rfgsm_ftm_msgr_register_sub2(&(rf_task_rex_msgr[task_num].client), rf_task_rex_msgr[task_num].mb);
#endif /*FEATURE_GSM*/
  }
  else if ( task_num == RF_TASK_3 )
  {
    /* Task 3 is not currently supported */
    /* Returning FALSE here causes the any initialisation in rf task 3 to be */
    /* de-initialised and any memory created for the task to be freed.       */
    MSG_1( MSG_SSID_RF, MSG_LEGACY_ERROR,
           "rf_rex_task_msgr_register() rf task %d is not supported", 
           RF_TASK_3 );
    ret_status = FALSE;
  }
