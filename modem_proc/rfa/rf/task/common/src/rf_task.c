...
/*===========================================================================
  ...
  $Header: //components/rel/rfa.mpss/4.11/rf/task/common/src/rf_task.c#2 $
...
08/13/15   Saul    Improve debug messaging for rf_init_complete_send_rsp API
...
rf_task_rfm_init_state_type rf_task_rfm_init_state = RF_TASK_RFM_INIT_STATE_NOT_RAN;
...
extern rfm_init_status_type* rfm_init ( rex_tcb_type *,
                          rex_sigs_type,
                          void (*)( rex_sigs_type ));
...
rfm_init_status_type* p_rfm_init_status = NULL;
...
void rf_init_complete_send_rsp( void )
{
  ...
  {
    RF_MSG( RF_HIGH, "rf_init_complete_send_rsp rfm_init PENDING" );
  ...
  if(NULL == p_rfm_init_status)
  {
    RF_MSG( RF_HIGH, "rf_init_complete_send_rsp No response from rfm_init()" );
    rf_init_comp_msg.rf_init_success = FALSE;
    rf_init_comp_msg.failure_type = RF_INIT_FAILURE_SW;
  }
  else
  {
    rf_init_comp_msg.rf_init_success = p_rfm_init_status->rfm_init_status;
    rf_init_comp_msg.failure_type = (rf_init_failure_type_e_type)(p_rfm_init_status->rfm_init_failure);
  }

  RF_MSG_1( RF_HIGH, "rf_init_complete_send_rsp rfm_init COMPLETE. MSGR RSP Issued. Provided rfm_init status %d",
            rf_init_comp_msg.rf_init_success );
  retval = msgr_send(&rf_init_comp_msg.hdr, sizeof(rf_init_comp_msg));
  
  if (retval == E_SUCCESS)
  {
    MSG_1( MSG_SSID_RF, MSG_LEGACY_HIGH, "rf_init_complete_send_rsp MSGR Send Success. Msg ID %0x", 
           RFA_RF_CONTROL_INIT_COMPLETE_RSP );
  }
  else
  {
    MSG_1( MSG_SSID_RF, MSG_LEGACY_ERROR, "rf_init_complete_send_rsp MSGR Send Failed. Msg ID %0x", 
           RFA_RF_CONTROL_INIT_COMPLETE_RSP );
  }
...
//  rex_def_timer(&ftm_wcdma_enter_exit_wait_timer_rf_tcb, &rf_tcb, FTM_WCDMA_ENTER_EXIT_COMPLETE_SIG);
  ...
  */
  ...
  rf_task_rfm_init_state = RF_TASK_RFM_INIT_STATE_STARTED;

  #ifdef FEATURE_MODEM_RCINIT
  p_rfm_init_status = rfm_init ( rex_self(),
  ...
  rf_task_rfm_init_state = RF_TASK_RFM_INIT_STATE_COMPLETE;
...
)
{
  ...
  {
  ret_status = msgr_register_block_variant(MSGR_RFA_RF_CONTROL,
                                   msgr_client,
                                   mailbox,
  ...
  {
    MSG_ERROR("MSGR_RFA_RF_CONTROL reg error %d on LTE sub id %d", ret_status, sub_id, 0);
    init_status = FALSE;
  }
  }
