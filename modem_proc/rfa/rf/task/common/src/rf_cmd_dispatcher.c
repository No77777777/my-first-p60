...
$Header: //components/rel/rfa.mpss/4.11/rf/task/common/src/rf_cmd_dispatcher.c#1 $

when       who     what, where, why
--------   ---     -------------------------------------------------------------
12/10/15   jr      Phase 1 device status read
11/20/15   vr      Autopin check-in
11/05/15   ag      Added Autopin cmd handle for WCDMA for msg support
11/05/15   whz     Added cmd handle for RFLM_CMN_AUTOPIN_RSP
08/25/15   gh/zhh  Added cmd handle for RFLM_WCDMA_DO_THERM_READ_CMD
08/21/15   cdb     Autopin message support for C2K
06/12/15   aa      Autopin Support for WCDMA
...
)
{
  ...
  if ( (cmd->msgr_hdr.id == RFLM_WCDMA_DO_MTPL_CMD) || 
       (cmd->msgr_hdr.id == RFLM_WCDMA_NPA_ISSUE_CMD) || 
       (cmd->msgr_hdr.id == RFLM_WCDMA_NPA_CANCEL_CMD) || 
       (cmd->msgr_hdr.id == RFLM_WCDMA_PROCESS_AUTO_PIN_CMD) || 
  ...
  else if ( cmd->msgr_hdr.id == RFLM_TDSCDMA_TRIGGER_DEVICE_READ_CMD)
  {
    rf_dispatch_command( rf_task_get_dispatch_handle(),cmd );
    rf_dispatch_done = TRUE;
  }
  ...
  #ifdef FEATURE_CDMA
  if ( cmd->msgr_hdr.id == RFLM_C2K_TRIGGER_AUTOPIN_CMD )
  {
    rf_dispatch_command( rf_task_get_dispatch_handle(),cmd );
    rf_dispatch_done = TRUE;
  }
