...
/*==============================================================================
  ...
  $Header: //components/rel/rfa.mpss/4.11/rf/task/common/src/rf_dispatch.c#1 $
    ...
    if ( duplicate_cid == FALSE )
    {
      if ( tech_node->tech_cids.cid_list != NULL )
      {
        ...
        if ( (reg_action & (1 << RF_DISPATACH_MSGR_REG))  == TRUE )
        {
          ...
          {
          msgr_res = msgr_register_variant( module_self, 
          ...
          if ( msgr_res != E_SUCCESS )
          {
            if ( dispatch_handle->debug_mode == TRUE )
            {
              RF_STR_MSG_5( RF_ERROR, "rf_dispatch_register_command: [ %s ] "
                            "MOD.0x%x CID.0x%x FP.0x%x - Failed MSGR "
                            "registration on LTE sub id %d", dispatch_handle->task_name, 
                            module_self, cid, cid_handler, sub_id );
            } /* if ( dispatch_handle->debug_mode == TRUE ) */
            else
            {
              RF_MSG_5( RF_ERROR, "rf_dispatch_register_command: [ 0x%x ] "
                        "MOD.0x%x CID.0x%x FP.0x%x - Failed MSGR registration on LTE sub id %d",
                        dispatch_handle->task_uid, module_self, cid, 
                        cid_handler, sub_id );    
            } /* if ! ( dispatch_handle->debug_mode == TRUE ) */
            ret_val = FALSE;
  ...
  if ( tech_node != NULL )
  {
          ...
          cid_idx++ )
    {
      ...
      if ( cid_ptr->cid == cid )
      {
        ...
        {
          /* De-register with MSGR on both LTE subs*/
          for (sub_id = 0; sub_id < RF_MAX_NUMBER_SUB; sub_id++) 
          {
          msgr_res = msgr_deregister_variant( module_self, 
                                     &(dispatch_handle->msgr_data.client), 
                                     cid,
                                     sub_id);
          ...
          if ( msgr_res != E_SUCCESS )
          {
            if ( dispatch_handle->debug_mode == TRUE )
            {
              RF_STR_MSG_4( RF_ERROR, "rf_dispatch_deregister_command: "
                            "[ %s ] MOD.0x%x CID.0x%x - MSGR Failure on LTE sub id %d", 
                            dispatch_handle->task_name, module_self, cid, sub_id );    
            } /* if ( dispatch_handle->debug_mode == TRUE ) */
            else
            {
              RF_MSG_4( RF_ERROR, "rf_dispatch_deregister_command: "
                        "[ 0x%x ] MOD.0x%x CID.0x%x - MSGR Failure on LTE sub id %d", 
                        dispatch_handle->task_uid, module_self, cid, sub_id );    
            } /* if ! ( dispatch_handle->debug_mode == TRUE ) */
            ret_val = FALSE;
          } /* if ( msgr_res != E_SUCCESS ) */
          else
