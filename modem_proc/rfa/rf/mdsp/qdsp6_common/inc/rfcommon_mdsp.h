...
/*==============================================================================
  ...
  $Header: //components/rel/rfa.mpss/4.11/rf/mdsp/qdsp6_common/inc/rfcommon_mdsp.h#1 $

when       who     what, where, why
--------   ---     -------------------------------------------------------------
10/28/15   as      Implement the asd abort script API
06/18/15   pl      Modify CCS event build flow to enable CCS task merge
08/07/15   zbz     selftest VSWR split capture margin change
...
03/30/15   zhw     Add LM exported RFA layer modem chain number
02/11/15   jmf     [selftest] New APIs for spectrum flatness support
...
/*! 
    @brief
    NUMBER OF POSSIBLE TX CHAINS.
*/
#define RFCOMMON_MDSP_NUM_OF_TX_CHAINS                            TXLM_CHAIN_NUM
...
#define RFCOMMON_MDSP_VSWR_SPLIT_SAMP 1024
#define RFCOMMON_MDSP_VSWR_SPLIT_MARGIN 7168
...
typedef struct
{
  /*general selftest meas results */
  uint16 evm;
  uint32 rx_rms_mag;
  uint32 gsm_power;


}rfcommon_selftest_meas_result;
...
/*----------------------------------------------------------------------------*/
boolean
rfcommon_mdsp_event_start_push_and_commit_opt
(
  uint32 event_id,
  rfcommon_mdsp_event_handle** event_handle,
  const rfcommon_mdsp_event_settings_type* event_settings,
...
/*----------------------------------------------------------------------------*/
boolean
rfcommon_mdsp_abort_asdiv_script
(
  rfm_device_enum_type device,
  rfm_mode_enum_type rfm_mode,
  uint32 rxlm_handle
...
(
  int32 txlm_handle_id,
  rfcommon_core_txlin_script_type_t sequence_id
);
...
boolean
rfcommon_mdsp_setup_self_test_upsampling_params_specflat
( 
  uint8 tx_upsamp
);
uint8
rfcommon_mdsp_get_first_samp_idx
( 
   uint8 buffer_index
);
