...
/*==============================================================================
  ...
  $Header: //components/rel/rfa.mpss/4.11/rf/mdsp/qdsp6_meas/inc/rfmeas_mdsp.h#1 $ 

when       who     what, where, why
--------   ---     -------------------------------------------------------------
06/18/18   sk      IRAT workaround for meas script build delay 
05/11/16   npi     Update IQMC APIs to use 32-bit ARFCNs
06/10/15   stw     LTE_U : Fix for IRAT_EVENT ID construction overflow for 32 bit channels
...
/*----------------------------------------------------------------------------*/
boolean
rfmeas_mdsp_meas_delete_event
(
  uint32 handle_id,
  uint32 event_1_script_id,
  uint32 event_2_script_id
);
