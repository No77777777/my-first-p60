...
$Header: //components/rel/rfa.mpss/4.11/rf/qmi_svc/rfrpe/src/rf_qmi_rfrpe_svc.c#1 $

when         who   what, where, why
--------   ---   ------------------------------------------------------------------- 
07/23/15   adk   Fixed a crash in rf_qmi_rfrpe_set_scenarios_resp()
04/01/15   adk   Register RFA RFRPE QMI dynamic task with RCINIT at build-time
...
uint8 rf_qmi_rfrpe_svc_init()
{
...
#ifndef FEATURE_CENTRALIZED_THREAD_PRIORITY
  ...
  /* Look up task info */
  info_handle = rcinit_lookup("RFRPE");
	   
  if (!info_handle) 
  {
    ERR_FATAL("RFRPE task info not found",0 ,0 ,0);
  }
  else
  {
    priority = (uint32_t)rcinit_lookup_prio_info(info_handle);
    if (priority > 255)
    {
      ERR_FATAL("Invalid Priority:%d or Stack Size: %d", priority, 0, 0);
    }
  }
#endif
