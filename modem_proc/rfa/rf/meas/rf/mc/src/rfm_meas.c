...
/*===========================================================================
Copyright (c) 2008 - 2025 by Qualcomm Technologies, Incorporated.  All Rights
...
$Header: //components/rel/rfa.mpss/4.11/rf/meas/rf/mc/src/rfm_meas.c#2 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
02/25/25   ag      [CR4066616] - Debug print for Active carrier index from L1
04/24/15   rmb     Porting from mainline to dev branch.
03/11/15   rmb     Add common init params API instead of using Init Enter, Setup and Exit
03/10/15   zhw     Add support to initialize RF header
03/05/15   zhw     Fix incorrect indexing
...
/*!
  @brief
    This function is called when the measurement is requested. Each technology
  ...
  @pre
  rfm_init() must have already been called.
  rfm_meas_common_init_params() must also been called to initialise the params to default
  We should be in a certain technology.
...
{
  rfm_meas_result_type result = RFM_MEAS_COMMON_SUCCESS;
  rfm_meas_common_param_type *meas_scripts_param = NULL;

  if(rfm_meas_setup_param->setup_params != NULL)
  {
    meas_scripts_param = rfm_meas_setup_param->setup_params;
    MSG_1(MSG_SSID_RF, MSG_LEGACY_HIGH, "rfm_meas_common_script_build_scripts: Source active_carrier_idx_mask %d", 
         meas_scripts_param->source_param.lte_params.active_carrier_idx_mask);
  }
...
/*!
  @brief
    ...
    in the right state and do any clean-ups required. This should be called after 
    physical end of the gap or after tune back scripts are executed
  ...
  @pre
  rfm_meas_common_script_enter->rfm_meas_common_script_build_scripts functions must have been called.
...
/*!
  @brief
  This function is called to initialise a rfm_meas_header_type structure.
  ...
  none.
  */
void rfm_meas_common_init_rf_header(rfm_meas_header_type* header)
{
  uint8 i,j;

  /* default all the tx and rx devices to invalid */
  for(i=0; i< MAX_NUMBER_CA_PATH; i++)
  {
    for (j=0; j<MAX_RX_DEV_HANDLE_ITEM; j++ )
    {
      header->source_param_v2[i].rx_dev_handle[j].device = RFM_INVALID_DEVICE;
    }
    for (j=0; j<MAX_TX_DEV_HANDLE_ITEM; j++ )
    {
      header->source_param_v2[i].tx_dev_handle[j].device = RFM_INVALID_DEVICE;
    }
  }

  /* default all the target tx and rx devices to invalid */
  for(i = 0; i< MAX_NUMBER_CA_PATH; i++)
  {
    for (j=0; j<MAX_RX_DEV_HANDLE_ITEM; j++ )
    {
      header->target_param_v2[i].rx_dev_handle[j].device = RFM_INVALID_DEVICE;
    }
    for (j=0; j<MAX_TX_DEV_HANDLE_ITEM; j++ )
    {
      header->target_param_v2[i].tx_dev_handle[j].device = RFM_INVALID_DEVICE;
    }
  }
}/* rfm_meas_common_init_rf_header */
/*----------------------------------------------------------------------------*/
/*!
  @brief
    This function is called to initialise a rfm_meas_common_param_type structure.

  @param rfm_meas_param : inits the structure to zero and fills the
    device lists with RFM_INVALID_DEVICE.

  @pre
  This is ideally called by L1 before any rfm_meas_common_script_enter() to initialise
  the generic paramters to default. L1 maintains this globally and uses the same information
  in Enter, Build Script and Exit. If Enter/ Build script/ Exit had additional parameters to receive,
  then L1 has to populate it appropriately before sending it to RF

  @retval
  none.
  */
void rfm_meas_common_init_params(rfm_meas_common_param_type *rfm_meas_param)
{

  if (rfm_meas_param == NULL) 
  {
    RF_MSG( RF_ERROR, "rfm_meas_common_init_params: NULL params " );
    return;
  }

  /* fill the struct with zero */
  memset(rfm_meas_param, 0, sizeof(rfm_meas_common_param_type));

  /*Initialize RF header */
  rfm_meas_common_init_rf_header(&(rfm_meas_param->header));
} /* rfm_meas_common_init_params*/
