...
$Header: //components/rel/rfa.mpss/4.11/rf/common/rf/core/inc/rfcommon_core_maat_manager.h#1 $

when       who     what, where, why
-------------------------------------------------------------------------------
09/08/16   yb       Updated static data type to nv data type
09/02/16   ndb      Added MAAT logging support
08/11/16   ndb      Initial version.
...
#define RFCMN_CORE_MAAT_MAX_NUM_ANTENNAS 4
...
#define RFCMN_CORE_MAAT_MAX_NUM_CARRIERS 5
...
#define RFCMN_CORE_MAAT_MAX_SUMMED_METRIC_VALUE 32
...
typedef union
{
  ...
  #ifdef FEATURE_WCDMA
  rfa_wcdma_maat_tx_info_type_s wcdma_maat_data;
  #endif

  #ifdef FEATURE_CDMA1X
  rfm_cdma_maat_tx_info_type_s cdma_maat_data;
  #endif
...
typedef union
{
  ...
  #ifdef FEATURE_WCDMA
  rfm_wcdma_maat_rx_info_type_s wcdma_maat_data;
  #endif
  
  #ifdef FEATURE_CDMA1X  
  rfm_cdma_maat_rx_info_type_s cdma_maat_data;
  #endif
...
/*!
  @brief
  This is the interface API used to send the MSGR message to 
  RF Common Core MAAT manager
  ...
  @param payload_ptr
  Pointer to payload containing the metrics data
  
  @return
*/
void rfcmn_core_maat_update_send_msgr
(
  rfcom_mode_enum_type rfm_mode,
  rfcmn_core_maat_update_id_type update_id,
  void* payload_ptr
);
...
/*!
  @brief
  This is the call back interface registered with RF APPS task for getting 
  the MAAT info from various modules.
  
  @param cmd_ptr
  Pointer to data structure containing reponse command data
 
  @param cid_info
  Pointer to data structure containing command ID info
 
  @param cb_data
  Pointer to data  structure containing callback data
  
  @return

*/
void rfcmn_core_maat_update_handler
(
  void* cmd_ptr, 
