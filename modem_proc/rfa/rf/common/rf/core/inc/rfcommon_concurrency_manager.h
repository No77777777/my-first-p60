...
typedef struct
{
  ...
  rfm_device_operation_type device_op;   /*!< PRx/DRx/Tx mode of op  */
  rfm_device_sub_id_type    sub_id;      /*!< sim1/sim2 distinction  */
...
/*! 
  ...
  @details
  The structure is used primarily for rftech interface. The structure is populated
  by rftechs and passed to concurrency manager to get the alt paths required for
  the given scenario.
...
typedef struct
{
  ...
  }device_allocation[RFM_MAX_WAN_DEVICES];
}rfm_alt_path_scenario_check_req_type;


/*----------------------------------------------------------------------------*/
/*! 
  @brief
  Structure to hold the alt path for all logical devices returned to rftech
 
*/
typedef struct
{
  uint8 alt_path[RFM_MAX_WAN_DEVICES]; 
}rfm_alt_path_scenario_check_rsp_type; 
