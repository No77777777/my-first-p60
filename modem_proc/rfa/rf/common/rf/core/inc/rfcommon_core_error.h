...
$Header: //components/rel/rfa.mpss/4.11/rf/common/rf/core/inc/rfcommon_core_error.h#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
01/21/15   ra      Update copyright. Also, use __FILENAME__ to avoid file paths in trace msg
...
#define RF_FATAL_ERROR( xx_msg ) \
  rfcommon_core_error( __LINE__, __FILENAME__, NULL )
...
#define RF_ASSERT( xx_exp ) \
  if ( !(xx_exp) )          \
  {                         \
    rfcommon_core_error( __LINE__, __FILENAME__, "RF Assertion failed" ); \
  }                         \
  else {}
