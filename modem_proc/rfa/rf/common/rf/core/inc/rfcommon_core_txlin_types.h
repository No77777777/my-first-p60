...
/*==============================================================================
  ...
  $Header: //components/rel/rfa.mpss/4.11/rf/common/rf/core/inc/rfcommon_core_txlin_types.h#1 $
...
02/16/15    dw     Added delta gain Pin Pout LUT in Tx band Cal data
...
06/09/15   jfc     Fix missing trailing slash in comment 
...
#define RFCOM_TXLIN_NUM_PIN_LUT           4
...
/*! 
    @brief
    Numer of transactions supported for ASM data and trigger task combined
*/
#define RFCMN_TXLIN_AGC_ASM_TASK_NUM_TRANS                                     8
/*----------------------------------------------------------------------------*
...
typedef enum
{
  ...
  RFCMN_TXLIN_AGC_ASM_TRIG_TASK = 9,

  RFCMN_TXLIN_AGC_MAX_TASK = 10,/*!< Number of TxAGC tasks */
...
typedef enum
{
  ...
  TX_LINEARIZER_FIRST_SWEEP_40MHZ,
  TX_LINEARIZER_DATA_TYPE_40MHZ,  
...
typedef struct 
{
  ...
  /*!< For each PA state, store NV data from new pin_cal_vs_freq 
    NV into this data structure */

  boolean skip_pout_normalization[RFCOM_TXLIN_NUM_LINEARIZERS];
  /*!< For each PA state, store from NV whether Pout offset from static 
    char must be normalized to ref temp or not */
...
typedef struct
{
  ...
  int8 pin_misalign[RFCOM_TXLIN_NUM_PIN];
  /*! @brief gain compression/expansion  */
  int8 gain_comp[RFCOM_TXLIN_NUM_PIN];
  /*! @brief Delta Pout  */
  int8 pout_delta[RFCOM_TXLIN_NUM_PIN];      
} tx_delta_gain_pin_pout_table_type;
...
typedef struct 
{
  ...
  /*! @brief AutoPin LUT for Pin Pout Gain delta */
  tx_delta_gain_pin_pout_table_type *pin_lut[RFCOM_TXLIN_NUM_PIN_LUT];
} tx_band_cal_type;
...
/*!
  @brief enum of type of sequence that txagc can automatically handle
*/
typedef enum
{
  /*! @brief TX_2_TX sequence type*/
  RFCOM_TXLIN_TXAGC_TX_TO_TX,

  /*! @brief  OFF_2_TX sequence type */
  RFCOM_TXLIN_TXAGC_OFF_TO_TX,

  /*! @brief  not using this sequnec now */
  RFCOM_TXLIN_TXAGC_BUFFER_2,
  
  /*! @brief  max type */
  RFCOM_TXLIN_TXAGC_SEQ_MAX_TYPE
  
}rfcommon_core_txlin_script_type_t;
