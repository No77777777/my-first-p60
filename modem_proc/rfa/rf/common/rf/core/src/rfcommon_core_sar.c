...
$Header: //components/rel/rfa.mpss/4.11/rf/common/rf/core/src/rfcommon_core_sar.c#1 $

when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
01/20/20   ssl     FR 55657: Replace RF Enter/Exit with RF Sleep/Wakeup for L+L DSDS awake
03/15/16   dsm     Setting rfm_mode for TX devices
01/20/16   bir     SAR update for all active techs
01/14/16   mah     Fixed SAR update for ASDIV
...
void rfcommon_core_sar_trigger_update_device(rfm_device_enum_type device)
{
  ...
  switch (curr_mode)
  {
    ...
    case RFCOM_GSM_MODE:
      rfgsm_mc_update_sar_backoff(tx_device);
      break;
  #endif

  #ifdef FEATURE_LTE
    case RFCOM_LTE_MODE:
      rflte_mc_tx_update_sar_backoff(sub_id,tx_device);
      break;
  #endif
...
void rfcommon_core_sar_trigger_update(void)
{
  ...
  rfcom_mode_enum_type rfm_current_mode[2];
  uint8 counter = 0;
  for ( device = 0 ; device < RFM_MAX_DEVICES ; device++)
  {
    if ((rfm_get_current_mode(device) != RFM_PARKED_MODE) &&
       (rfm_get_current_mode(device) != RFM_INVALID_MODE))
    {
      if(counter == 0)
      {
        rfm_current_mode[counter] = rfm_get_current_mode(device);
        rfcommon_core_sar_trigger_update_device(device);
        RF_MSG_1( RF_HIGH,"rfcommon_core_sar_trigger_update: device %d ", device);
        counter++;
      }
      else
      {
        /* call second time sar update only if the tech is not same as first sar             update */
        if(rfm_get_current_mode(device) != rfm_current_mode[0])
        {
          rfcommon_core_sar_trigger_update_device(device);
          RF_MSG_1( RF_HIGH,"rfcommon_core_sar_trigger_update: device %d ", device);
           break;
