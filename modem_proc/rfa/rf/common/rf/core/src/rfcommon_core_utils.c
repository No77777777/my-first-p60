...
$Header: //components/rel/rfa.mpss/4.11/rf/common/rf/core/src/rfcommon_core_utils.c#2 $

when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
10/12/22   ag      [CR3309436] - Inclusion of LTE-B31 and LTE-B32 for Sys Band to RFCOM Band conversion API
06/17/20   nik     Modifying proper Feature flag for W. Crash was seen while disabling TDSCDMA
01/20/20   ssl     FR 55657: Replace RF Enter/Exit with RF Sleep/Wakeup for L+L DSDS awake
03/28/18   ks      Add fix for SYS band to RFCOM band in B71
10/26/17   sk      Enable LTE B71 support
08/11/16   ndb    Added the API rfcommon_core_get_tx_handle_from_device()
05/11/16   rs      Removed excessive F3 msgs
12/09/15   pk      Added support for CDMA rx\tx split
11/15/15   mah     Added support device to band mapping for Tx DPD loading.
11/02/15   vc      Rx/Tx split changes
10/10/15   zhh     add WTR ID check API
08/28/15   zhh     Add the API to check wtr instance from device
06/17/15   aro     Added Tx device mapping
06/09/15   jak     Add ftm tech param for rfcommon_core_get_tx_default_device()
06/08/15   Saul    RFCOM_BAND_LTE_B32 to sys band conversion
06/05/15   pkg     Support XPT Capture in FTM RF mode.
06/03/15   pkg     Backout changes to get a remapped device in
                   rfcommon_core_get_tx_chain_from_device() API, since the device passed is valid.
06/01/15   jak     Handle TDS condition for rfcommon_core_get_tx_default_device()
05/29/15   pkg     Enable XPT CHAR on TH.2.x PL's.
05/28/15   aro     Support to test ATLAS WCDMA driver in THOR baseline
05/14/15   stw     Added support for LTE-U bands
04/22/15   ak      [Rx/Tx Split]Changes to support Cal with backward compatibility
04/13/15   ak      Rx/Tx Split Changes to support CRAT design
04/08/15   tks     Added support for LTE-U bands
03/30/15   zhw     Add rfcommon_core_get_tx_default_device() to map Tx device
                   based on Rx device
...
extern boolean rfc_wcdma_data_get_device_info(rfc_cfg_params_type *cfg, rfc_device_info_type **device_info_pptr);
#endif /* FEATURE_WCDMA */
...
uint8 rfcommon_core_get_tx_chain_from_device( rfm_device_enum_type device )
{
  ...
  switch(current_mode)
  {
     case RFCOM_WCDMA_MODE:
       #ifdef FEATURE_WCDMA
       #ifdef ATLAS_RF_WCDMA_PRESI
       modem_chain = rf_wcdma_mc_tx_get_modem_chain( device );
       #else
       cfg.band = rfwcdma_data_get_curr_band(device);
     ...
     case RFCOM_LTE_FTM_MODE:
       #ifdef FEATURE_LTE
       {
         /* To handle Inter-band ULCA. */
         cfg.band = rflte_state_get_tx_band_from_tx_device(device);
       cfg.req = RFC_REQ_DEFAULT_GET_DATA;
       cfg.logical_device = device;
       cfg.rx_tx = RFC_CONFIG_TX;
     ...
     default:
       RF_MSG_1( RF_ERROR, "rfcommon_core_get_tx_chain_from_device:"
                   " invalid mode from device %d", device ); 
       break;
  }
  return modem_chain;
}
...
/*!
  @brief
  Converts device to tx handle.
 
  @param device
  The device to convert to tx handle

  @return
  Corresponding tx handle for given device
*/
uint32 
rfcommon_core_get_tx_handle_from_device
( 
  rfm_device_enum_type device
)
{
  rfm_mode_enum_type current_mode;
  uint32 txlm_handle = 0xFFFFFFFF;
  ...
  switch(current_mode)
  {
     case RFCOM_WCDMA_MODE:
       #ifdef FEATURE_WCDMA
       txlm_handle = (uint32)rfwcdma_data_get_rflm_buf_idx(device);
       #endif
       break;
     ...
     case RFCOM_1XEVDO_MODE:
       #ifdef FEATURE_CDMA1X
       txlm_handle = (uint32)rf_cdma_data_get_txlm_handle(device);
       #endif
       break;
     ...
     case RFCOM_LTE_FTM_MODE:
       #ifdef FEATURE_LTE
       carrier_idx = rflte_state_get_carrier_idx_from_band(sub_id,
                              rflte_state_get_tx_band_from_tx_device(device));
       rflte_state_get_txlm_buffer_idx(sub_id,carrier_idx,(lm_handle_type*)&txlm_handle);
       #endif
       break;

     case RFCOM_TDSCDMA_MODE:
       #ifdef FEATURE_TDSCDMA
       txlm_handle = (uint32)rf_tdscdma_mc_lm_bufs.tx_buf_wrk.buf_idx;
       #endif
       break;

     default:
       RF_MSG_1( RF_ERROR, "rfcommon_core_get_tx_handle_from_device:"
                   " invalid mode from device %d", device ); 
       break;
  }
  return txlm_handle;
...
/*!
  @brief
  Core utility API that converts a given Rx device to tx device.
  The input of the API must be a Rx device.
  Tx device input behavior is undefined yet.
 
  Currently the implementation is dependent on tech.
  Although based on current design it should be from a RFC API.
  For a given tech if support is not added, it returns the input device
...
rfcommon_core_get_tx_default_device( rfm_device_enum_type device , rfm_mode_enum_type rf_tech )
{
  rfm_device_enum_type tx_device = device;
  ...
  RF_MSG_2( RF_HIGH, "rfcommon_core_get_tx_default_device:"
                           "device %d , rf_tech = %d", device ,rf_tech );   
  ...
  switch(rf_tech)
  {
     ...
     case RFCOM_WCDMA_MODE:
       #ifdef ATLAS_RF_WCDMA_PRESI
       return rfc_common_get_preferred_associated_tx_device(device);
       #else
       mapped_rx_device = ftm_wcdma_get_remapped_rx_device(device);
       tx_device = ftm_wcdma_get_remapped_tx_device(mapped_rx_device);
       return tx_device; /* Hack for validation*/
       #endif
       break;
     #endif
     ...
     case RFCOM_1XEVDO_MODE:
     {
       ...
       rfm_device_enum_type rx_device = device;
       ...
       if( rfc_common_get_preferred_tx_device ( sys_band ) != RFM_INVALID_DEVICE )
       {
         tx_device = rfc_common_get_preferred_tx_device ( sys_band ); 
       }
       else
       {
         RF_MSG_2( RF_ERROR, "rfcommon_core_get_tx_default_device:"
                     " invalid device Rx %d Tx %d", device, tx_device ); 
       }
     }
    break;
       #endif
     ...
     case RFCOM_LTE_FTM_MODE:
       {
         ...
         source_device_info.tech = current_mode;
         source_device_info.band = rflte_mc_sm_get_band(RFLTE_MC_PCELL_IDX);
         ...
         //tx_device = rfc_common_get_preferred_associated_tx_device( mapped_rx_device );
         ...
         RF_MSG_3( RF_HIGH, "rfcommon_core_get_tx_default_device: passed dev: %d, rfc rx dev: %d, LTE Tx dev:%d",
                            device, mapped_rx_device, tx_device );
       }
       break;
     ...
     case RFCOM_TDSCDMA_MODE:
       ...
       if ( tx_device == RFM_INVALID_DEVICE )
       {
         ...
         RF_MSG_1( RF_HIGH, "rfcommon_core_get_tx_default_device:"
                           "rf_tdscdma_mc_get_curr_tx_device get called in invalid state , return default tx device %d", tx_device ); 
       }
       break;
	    #endif
     default:
       RF_MSG_1( RF_ERROR, "rfcommon_core_get_tx_default_device:"
...
int rfcommon_core_convert_sys_band_to_rfcom_band(sys_band_class_e_type band)
{
  ...
  switch(band)
  {
    ...
    case SYS_BAND_LTE_EUTRAN_BAND32:
      rf_card_band = RFCOM_BAND_LTE_B32;
      break;
...
sys_band_class_e_type rfcommon_core_convert_rfcom_band_to_sys_band(rfm_mode_enum_type tech, rfcom_band_type_u band)
{
  ...
  case RFM_LTE_FTM_MODE:
    ...
    case RFCOM_BAND_LTE_B31:
      ...
      break;
    case RFCOM_BAND_LTE_B32:
      sys_band = SYS_BAND_LTE_EUTRAN_BAND32;
      break;
...
)
{
  ...
  case RFCOM_1XEVDO_MODE:
    ...
    {
      RF_MSG_1( RF_ERROR, "rfcommon_core_get_tx_device_ptr : "
			"Null logical device for device id %d", device );
    }
    #endif
    break;
...
/*!
  ...
  @return
  Corresponding device ID from current device
*/
rfdevice_id_enum_type
rfcommon_core_get_wtr_id_from_device
(
  rfm_device_enum_type device,
  rfcom_wcdma_band_type band,
  rfm_mode_enum_type current_mode
)
{
  ...
  switch(current_mode)
  {
     case RFCOM_WCDMA_MODE:
       ...
       if(rfc_wcdma_data_get_device_info(&cfg, &device_info_ptr) == TRUE)
       {
         wtr_id = device_info_ptr->rf_asic_info[RFDEVICE_TRANSCEIVER].device_id;
         RF_MSG_2( RF_LOW, "rfcommon_core_get_wtr_id_from_device:"
                   " wtr_id %d on dev %d", wtr_id, device ); 
       }
       else
       {
         RF_MSG_1( RF_ERROR, "rfcommon_core_get_wtr_id_from_device:"
                   " could not get chain from dev %d", device ); 
       }
       #endif
       break;
...
/*!
  @brief
  This function is a helper function to convert band number to rfcom_lte_band_type.
  ...
  @return
  rfcom_lte_band_type: rfcom lte band type
*/

rfcom_lte_band_type rfcommon_core_convert_lte_band_num_to_rfcom_band
(
  uint16 band_num
)
{
  /*initialize the rfcom variable.*/
  rfcom_lte_band_type rfcom_band = RFCOM_BAND_LTE_INVALID;

  switch(band_num)
  {
  case 1:
    rfcom_band = RFCOM_BAND_LTE_B1;
    break;

  case 2:
    rfcom_band = RFCOM_BAND_LTE_B2;
    break;

  case 3:
    rfcom_band = RFCOM_BAND_LTE_B3;
    break;

  case 4:
    rfcom_band = RFCOM_BAND_LTE_B4;
    break;

  case 5:
    rfcom_band = RFCOM_BAND_LTE_B5;
    break;

  case 6:
    rfcom_band = RFCOM_BAND_LTE_B6;
    break;

  case 7:
    rfcom_band = RFCOM_BAND_LTE_B7;
    break;

  case 8:
    rfcom_band = RFCOM_BAND_LTE_B8;
    break;

  case 9:
    rfcom_band = RFCOM_BAND_LTE_B9;
    break;

  case 10:
    rfcom_band = RFCOM_BAND_LTE_B10;
    break;

  case 11:
    rfcom_band = RFCOM_BAND_LTE_B11;
    break;

  case 12:
    rfcom_band = RFCOM_BAND_LTE_B12;
    break;

  case 13:
    rfcom_band = RFCOM_BAND_LTE_B13;
    break;

  case 14:
    rfcom_band = RFCOM_BAND_LTE_B14;
    break;

  case 17:
    rfcom_band = RFCOM_BAND_LTE_B17;
    break;

  case 18:
    rfcom_band = RFCOM_BAND_LTE_B18;
    break;

  case 19:
    rfcom_band = RFCOM_BAND_LTE_B19;
    break;

  case 20:
    rfcom_band = RFCOM_BAND_LTE_B20;
    break;

  case 21:
    rfcom_band = RFCOM_BAND_LTE_B21;
    break;

  case 23:
    rfcom_band = RFCOM_BAND_LTE_B23;
    break;

  case 25:
    rfcom_band = RFCOM_BAND_LTE_B25;
    break;

  case 26:
    rfcom_band = RFCOM_BAND_LTE_B26;
    break;

  case 27:
    rfcom_band = RFCOM_BAND_LTE_B27;
    break;

  case 28:
    rfcom_band = RFCOM_BAND_LTE_B28;
    break;

  case 29:
    rfcom_band = RFCOM_BAND_LTE_B29;
    break;

  case 30:
    rfcom_band = RFCOM_BAND_LTE_B30;
    break;

  case 32:
    rfcom_band = RFCOM_BAND_LTE_B32;
    break;

  case 33:
    rfcom_band = RFCOM_BAND_LTE_B33;
    break;

  case 34:
    rfcom_band = RFCOM_BAND_LTE_B34;
    break;

  case 35:
    rfcom_band = RFCOM_BAND_LTE_B35;
    break;

  case 36:
    rfcom_band = RFCOM_BAND_LTE_B36;
    break;

  case 37:
    rfcom_band = RFCOM_BAND_LTE_B37;
    break;

  case 38:
    rfcom_band = RFCOM_BAND_LTE_B38;
    break;

  case 39:
    rfcom_band = RFCOM_BAND_LTE_B39;
    break;

  case 40:
    rfcom_band = RFCOM_BAND_LTE_B40;
    break;

  case 41:
    rfcom_band = RFCOM_BAND_LTE_B41;
    break;

  case 42:
    rfcom_band = RFCOM_BAND_LTE_B42;
    break;

   case 43:
    rfcom_band = RFCOM_BAND_LTE_B43;
    break;

  case 252:
    rfcom_band = RFCOM_BAND_LTE_B252;
    break;

  case 253:
    rfcom_band = RFCOM_BAND_LTE_B253;
    break;

  case 254:
    rfcom_band = RFCOM_BAND_LTE_B254;
    break;

  case 255:
    rfcom_band = RFCOM_BAND_LTE_B255;
    break;

  case 66:
    rfcom_band = RFCOM_BAND_LTE_B66;
    break;

  case 71:
    rfcom_band = RFCOM_BAND_LTE_B71;
    break;

   /*case 46:
    rfcom_band = RFCOM_BAND_LTE_B46;
    break;*/

  default:
    MSG_ERROR("Invalid band_num=%d in rfcommon_core_convert_lte_band_num_to_rfcom_band().", band_num, 0, 0);
    break;
  }
  return rfcom_band;
}
...
/*!
  @brief
  This function is a helper function to convert band number to rfm_cdma_band_class_type.
  ...
  @return
  rfm_cdma_band_class_type: rfcom cdma band type
*/

rfm_cdma_band_class_type rfcommon_core_convert_cdma_band_num_to_rfcom_band
(
  uint16 band_num
)
{
  /*initialize the rfcom variable.*/
  rfm_cdma_band_class_type rfcom_band = RFM_CDMA_MAX_BAND;

  switch(band_num)
  {
    case 0:
      rfcom_band = RFM_CDMA_BC0;
      break;

    case 1:
      rfcom_band = RFM_CDMA_BC1;
      break;

    case 2:
      rfcom_band = RFM_CDMA_BC2;
    break;

    case 3:
      rfcom_band = RFM_CDMA_BC3;
    break;

    case 4:
      rfcom_band = RFM_CDMA_BC4;
    break;

    case 5:
      rfcom_band = RFM_CDMA_BC5;
    break;

    case 6:
      rfcom_band = RFM_CDMA_BC6;
    break;

    case 7:
      rfcom_band = RFM_CDMA_BC7;
    break;

    case 8:
      rfcom_band = RFM_CDMA_BC8;
    break;

    case 9:
      rfcom_band = RFM_CDMA_BC9;
    break;

    case 10:
      rfcom_band = RFM_CDMA_BC10;
    break;

    case 11:
      rfcom_band = RFM_CDMA_BC11;
    break;

    case 12:
      rfcom_band = RFM_CDMA_BC12;
    break;

    case 13:
      rfcom_band = RFM_CDMA_BC13;
    break;

    case 14:
      rfcom_band = RFM_CDMA_BC14;
    break;

    case 15:
      rfcom_band = RFM_CDMA_BC15;
    break;

    case 16:
      rfcom_band = RFM_CDMA_BC16;
    break;

    case 17:
      rfcom_band = RFM_CDMA_BC17;
    break;

    case 18:
      rfcom_band = RFM_CDMA_BC18;
    break;

    case 19:
      rfcom_band = RFM_CDMA_BC19;
    break;

    case 20:
      rfcom_band = RFM_CDMA_BC20;
    break;

    default:
      MSG_ERROR("Invalid band_num=%d in rfcommon_core_convert_cdma_band_num_to_rfcom_band().", band_num, 0, 0);
    break;
  }

  return rfcom_band;
}/*rfcommon_core_convert_cdma_band_num_to_rfcom_band*/


/* ----------------------------------------------------------------------- */
/*!
  @brief
  This function is a helper function to convert band number to rfm_wcdma_band_type.
  ...
  @return
  rfm_wcdma_band_type: rfcom wcdma band type
*/
rfcom_wcdma_band_type rfcommon_core_convert_wcdma_band_num_to_rfcom_band
(
  uint16 band_num
)
{
  rfcom_wcdma_band_type rfcom_band = RFCOM_BAND_INVALID;

  /* perform conversion */
  switch ( band_num )
  {
    case 1:
      rfcom_band = RFCOM_BAND_IMT;
    break;
    case 2:
      rfcom_band = RFCOM_BAND_1900;
    break;
    case 3:
      rfcom_band = RFCOM_BAND_BC3;
    break;
    case 4:
      rfcom_band = RFCOM_BAND_BC4;
    break;
    case 5:
      rfcom_band = RFCOM_BAND_800;
    break;
    case 8:
      rfcom_band = RFCOM_BAND_BC8;
    break;
    case 9:
      rfcom_band = RFCOM_BAND_BC9;
    break;
    case 11:
      rfcom_band = RFCOM_BAND_BC11;
    break;
    case 19:
      rfcom_band = RFCOM_BAND_BC19;
    break;
    default:
      MSG_ERROR("Invalid band_num=%d in rfcommon_core_convert_wcdma_band_num_to_rfcom_band().", band_num, 0, 0);
    break;
  } /* switch ( band_num ) */

  return rfcom_band;

} /* rfcommon_core_convert_wcdma_band_num_to_rfcom_band */


/* ----------------------------------------------------------------------- */
/*!
  @brief
  This function is a helper function to convert band number to rfcom_tdscdma_band_type.
  ...
  @return
  rfcom_tdscdma_band_type: rfcom tdscdma band type
*/

rfcom_tdscdma_band_type rfcommon_core_convert_tdscdma_band_num_to_rfcom_band
(
  uint16 band_num
)
{
  ...
  switch(band_num)
  {
    ...
    default:
      MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR,
       "rfcommon_core_convert_tdscdma_band_num_to_rfcom_band: Invalid TDSCDMA band %d!", band_num);
    break;
  }

  return rfcom_band;
}/*rfcommon_core_convert_tdscdma_band_num_to_rfcom_band*/


/* ----------------------------------------------------------------------- */
/*!
  @brief
  This function is a helper function to convert band number to rfcom_gsm_band_type.
  ...
  @return
  rfcom_gsm_band_type: rfcom gsm band type
*/
rfcom_gsm_band_type rfcommon_core_convert_gsm_band_num_to_rfcom_band
(
  uint16 band_num
)
{
  ...
  switch(band_num)
  {
    ...
    default:
      MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR,
        "rfgsm_core_get_get_band_num: Invalid GSM Rx band %d!", band_num);
      break;
  }

  return rfcom_band;
} /*rfcommon_core_convert_gsm_band_num_to_rfcom_band*/
...
)
{
  ...
  switch(mode)
  {
    case RFM_LTE_MODE:
      ...
      break;
    case RFM_IMT_MODE:
      band->wcdma_band= rfcommon_core_convert_wcdma_band_num_to_rfcom_band(band_num);
      ...
      break;
    case RFM_EGSM_MODE:
      band->gsm_band = rfcommon_core_convert_gsm_band_num_to_rfcom_band(band_num);
      ...
      break;
    case RFM_1X_MODE:
      band->cdma_band = rfcommon_core_convert_cdma_band_num_to_rfcom_band(band_num);
      if(band->cdma_band >= RFM_CDMA_MAX_BAND)
      {
        status =  FALSE;
      }
      break;
    case RFM_TDSCDMA_MODE:
      band->tdscdma_band = rfcommon_core_convert_tdscdma_band_num_to_rfcom_band(band_num);
...
/*!
  @brief
  This function is a helper function for getting the max rfcom bands for the given 
  mode.
  ...
  @return
  uint16 : Max supported rfcombands
*/

uint16 rfcommon_core_get_max_rfcom_band
(
  rfm_mode_enum_type mode
)
{  
  uint8 band_counter_max = 0;
  
  if ( mode >= RFM_NUM_MODES )
  {
    RF_MSG_1( RF_ERROR,"Invalid mode %d in rfcommon_core_get_max_rfcom_band()",
                       mode);
    return band_counter_max;
  }
  ...
  switch (mode)
  {
    #ifdef FEATURE_WCDMA
    case RFCOM_WCDMA_MODE:
        band_counter_max = RFCOM_NUM_WCDMA_BANDS;
        break;
    #endif
    ...
    case RFCOM_1X_MODE:
    case RFCOM_1XEVDO_MODE:
        band_counter_max = RFM_CDMA_MAX_BAND;
        break;
    #endif

    #ifdef FEATURE_GSM
    case RFCOM_GSM_MODE:
        band_counter_max = RFCOM_NUM_GSM_BANDS;
        break;
    #endif
        
    #ifdef FEATURE_LTE
    case RFCOM_LTE_MODE:
        band_counter_max = RFCOM_NUM_LTE_BANDS;
        break;
    #endif

    #ifdef FEATURE_TDSCDMA
    case RFCOM_TDSCDMA_MODE:
        band_counter_max = RFCOM_NUM_TDSCDMA_BANDS;
        break;
    #endif

    default:
        RF_MSG_1( RF_ERROR,"Invalid mode %d in rfcommon_core_get_max_rfcom_band()",mode);
        break;
  }
  return band_counter_max; 
}/*rfcommon_core_get_max_rfcom_band*/
