...
/*==============================================================================
  ...
  Copyright (c) 2014 - 2023 Qualcomm Technologies Incorporated. All Rights Reserved
...
--------------------------------------------------------------------------------
10/13/23   ag      [CR3618199] Alt path handling in Rx-only configuration
04/01/20   akm     add ptx support in rfcmn_concurrency_mgr_get_alt_path_for_scenario 
12/21/15   gvn     For EXPECTED reservations reset the RFM_DEVICE to default state
11/02/15   kg      Temporarily ignore matching Tx devices till RFC tbl populated
11/06/15   hbl     [Ported]Change IRAT alt path lookup implementation 
11/02/15   kg      Temporarily ignore matching Tx devices till RFC tbl populated
10/14/15   kg      Bug fix for tbl entry which is a partial match to the DB
10/14/15   kg      F3 reduction
10/01/15   kg      Generalize the rx_op enum to account for tx rfm_devices
08/26/15   kg      Add get alt path for specific scenario api for LTE feature
08/16/15   kg      Fix bug; non-lte tech match was getting skipped in some cases
08/11/15   kr      Add F3 while concurrency mgr update.
08/04/15   kg      Ensure that valid rfm_dev is passed to update_rx_state
07/31/15   sml     The concurrency DB should be updated only for valid RFC entries 
                   or else we will end up not updating concurrency DB correctly
07/27/15   kg      Do PRx matching based on ccry_db's device_op, not from RFC
03/27/15   kg      Do PRx/DRx matching only for LTE tech 
02/23/15   kg      Consider DRx for alt path lookup to support horxd
...
)
{
  ...
  /* Update concurrency manager table only if valid rfm_dev*/
  if ( rfm_dev < RFM_MAX_WAN_DEVICES )
  {
    ...
    rfcmn_concurrency_db.curr_state[rfm_dev].device_op   = RFM_INVALID_OPERATION; 
    rfcmn_concurrency_db.curr_state[rfm_dev].sub_id      = RFM_INVALID_SUB_ID; 
  }
...
/*----------------------------------------------------------------------------*/
boolean rfcmn_concurrency_mgr_get_alt_path_for_scenario
(
  rfm_alt_path_scenario_check_req_type *dev_list,
  rfm_alt_path_scenario_check_rsp_type *alt_path_list
)
{
  uint8 arr_idx = 0; 
  uint32 idx                 = 0;
  ...
  boolean match_found        = FALSE;
  boolean entry_found        = FALSE; 
  boolean mode_band_match    = FALSE;
  boolean compare_tbl_entry  = FALSE;  
  int test_rf_band           = -1; 
  rfcom_mode_enum_type test_rf_mode = RFCOM_NUM_MODES; 
  rfm_device_enum_type temp_rfm_dev = RFM_INVALID_DEVICE; 
  rfm_device_operation_type test_dev_op = RFM_INVALID_OPERATION; 
  ...
  RFM_CM_DEBUG_EN_Q(&rfcmn_concurrency_mgr_get_alt_path_for_scenario, &dev_list, 0, 0, 0, 0);

  if (dev_list == NULL)
  {
    RF_MSG(RF_ERROR, "RF tech called concurrency manager with null dev_list"); 
    return FALSE; 
  }

  if (rf_ccry_mgr_time_prof)
  {
    RF_MSG_8(RF_HIGH, "rfcmn_concurrency_mgr_get_alt_path_for_scenario: Band input - %d, %d, %d, %d, %d, %d, %d, %d", 
             ...
             rfcommon_core_convert_sys_band_to_rfcom_band(dev_list->device_allocation[6].band),
             rfcommon_core_convert_sys_band_to_rfcom_band(dev_list->device_allocation[7].band)); 
    RF_MSG_8(RF_HIGH, "rfcmn_concurrency_mgr_get_alt_path_for_scenario: OP input from Tech - %d, %d, %d, %d, %d, %d, %d, %d",  
             dev_list->device_allocation[0].operation, 
             dev_list->device_allocation[1].operation, 
             dev_list->device_allocation[2].operation, 
             dev_list->device_allocation[3].operation, 
             dev_list->device_allocation[4].operation, 
             dev_list->device_allocation[5].operation,
             dev_list->device_allocation[6].operation,
             dev_list->device_allocation[7].operation); 
  }
  ...
  if ((curr_tbl != NULL) && (tbl_size != 0))
  {
    ...
    for (idx = 0; idx < tbl_size; idx++)
    {
      ...
      {
        test_rf_mode   = dev_list->device_allocation[temp_rfm_dev].tech;
        test_rf_band   = rfcommon_core_convert_sys_band_to_rfcom_band(dev_list->device_allocation[temp_rfm_dev].band);
        test_dev_op    = dev_list->device_allocation[temp_rfm_dev].operation;
        compare_tbl_entry = FALSE;

        /* Evaluate if we want to do device matching. Basic algo is:
           If ccry db has Tx device and there is no corresponding Tx device in RFC AG table, then skip match.
           If Tx device entry is found in the RFC AG, then do proceed to matching part. */
        if (((test_dev_op == RFM_PTX_OPERATION) && ((curr_tbl + idx * lst_size + temp_rfm_dev)->rx_path == RFC_PTX_PATH)) || 
             (test_dev_op != RFM_PTX_OPERATION))
        { 
          compare_tbl_entry = TRUE;
        }

        else if((test_dev_op == RFM_PTX_OPERATION) && ((curr_tbl + idx * lst_size + temp_rfm_dev)->rx_path != RFC_PTX_PATH))
        {
          entry_found = FALSE;
        }

        /* Check if a table match is found 
           We cannot successfully do PRx/DRx matching to get a hit in the RFC alt path table;
           This is because in cases of PRx,DRx = G/1x,Inv we want the Inv match to hit G/1x div
           but we dont want the match to hit cases where Inv, Inv + G will end up matching *,* + G.
           Workaround is to do the PRx/DRx matching only for LTE for now since only LTE really needs this. */
          /*We need to check the RFC table only until valid entries or else the ALT PATH will be wrong
            when we scan for all the devices */
       if(((((curr_tbl + idx * lst_size + temp_rfm_dev)->tech) != RFM_INVALID_MODE) || test_rf_mode != RFM_INVALID_MODE) && compare_tbl_entry == TRUE)
       {
         mode_band_match = ((((curr_tbl + idx * lst_size + temp_rfm_dev)->band) & ((uint64)1 << test_rf_band)) || test_rf_mode == RFM_INVALID_MODE ) && 
              ((curr_tbl + idx * lst_size + temp_rfm_dev)->tech == test_rf_mode);

         if (test_rf_mode == RFM_LTE_MODE)
         {
            if( mode_band_match == TRUE && (((test_dev_op == RFM_PRX_OPERATION) && ((curr_tbl + idx * lst_size + temp_rfm_dev)->rx_path == RFC_PRX_PATH)) || ((test_dev_op == RFM_DRX_OPERATION) && ((curr_tbl + idx * lst_size + temp_rfm_dev)->rx_path == RFC_DRX_PATH)) || ((test_dev_op == RFM_PTX_OPERATION) && ((curr_tbl + idx * lst_size + temp_rfm_dev)->rx_path == RFC_PTX_PATH))) )
            {
              match_found &= TRUE; 
              entry_found  = TRUE; 
            }
          ...
          /* Matching the PRx entries for the ccry db for non-LTE techs.
             We have to check the RFC table for PRx dev techs in the DB*/
          else
          {
            if( mode_band_match == TRUE && test_dev_op == RFM_PRX_OPERATION)
            {
              match_found &= TRUE; 
              entry_found  = TRUE; 
            }
  ...
  if (rf_ccry_mgr_time_prof)
  {
    RF_MSG_8(RF_HIGH, "rfcmn_concurrency_mgr_get_alt_path_for_scenario: ALT_PATH_DBG: alt_path_list->alt_path[0 - 8] - %d, %d, %d, %d, %d, %d, %d, %d", 
            alt_path_list->alt_path[0],
            alt_path_list->alt_path[1],
            alt_path_list->alt_path[2],
            alt_path_list->alt_path[3],
            alt_path_list->alt_path[4],
            alt_path_list->alt_path[5],
            alt_path_list->alt_path[6],
            alt_path_list->alt_path[7]);
  }			
  return TRUE;

} /* end of rfcmn_concurrency_mgr_get_alt_path_for_scenario*/
...
)
{
  ...
  if (rf_ccry_mgr_time_prof)
  {
    RF_MSG_8(RF_HIGH, "rf_ccry_update_dev_alloc: Tech input from MCS - %d, %d, %d, %d, %d, %d, %d, %d", 
             ...
             dev_list->device_allocation[6].tech,
             dev_list->device_allocation[7].tech); 
    RF_MSG_8(RF_HIGH, "rf_ccry_update_dev_alloc: Band input from MCS - %d, %d, %d, %d, %d, %d, %d, %d", 
             rfcommon_core_convert_sys_band_to_rfcom_band(dev_list->device_allocation[0].band),
             rfcommon_core_convert_sys_band_to_rfcom_band(dev_list->device_allocation[1].band),
             rfcommon_core_convert_sys_band_to_rfcom_band(dev_list->device_allocation[2].band),
             rfcommon_core_convert_sys_band_to_rfcom_band(dev_list->device_allocation[3].band),
    ...
    RF_MSG_8(RF_HIGH, "rf_ccry_update_dev_alloc: OP input from MCS - %d, %d, %d, %d, %d, %d, %d, %d",  
             dev_list->device_allocation[0].operation, 
             dev_list->device_allocation[1].operation, 
             dev_list->device_allocation[2].operation, 
             dev_list->device_allocation[3].operation, 
        ...
        arr_idx++ )
  {
    ...
    if ( (dev_list->device_allocation[arr_idx].allocation_type == RFM_DEVICE_ASSIGNED) || 
         (dev_list->device_allocation[arr_idx].allocation_type == RFM_DEVICE_ALLOCATION_INVALID) )
    {
      ...
      /* Update rx operation typ */
      rfcmn_concurrency_db.curr_state[arr_idx].device_op 
        = dev_list->device_allocation[arr_idx].operation; 
      
      /* Update sub id */
      rfcmn_concurrency_db.curr_state[arr_idx].sub_id
        = dev_list->device_allocation[arr_idx].sub_id; 
    }
    else
    {
      ...
      rfcmn_concurrency_db.curr_state[arr_idx].logical_dev
        = (rfm_device_enum_type)arr_idx;
      rfcmn_concurrency_db.curr_state[arr_idx].alt_path    = 0;
      rfcmn_concurrency_db.curr_state[arr_idx].device_op   = RFM_INVALID_OPERATION;
      rfcmn_concurrency_db.curr_state[arr_idx].sub_id      = RFM_INVALID_SUB_ID;
    }
  }
  ...
  if ((curr_tbl != NULL) && (tbl_size != 0))
  {
    ...
    for (idx = 0; idx < tbl_size; idx++)
    {
            ...
            temp_rfm_dev < (rfm_device_enum_type)lst_size; temp_rfm_dev++ )
      {
        ...
        test_dev_op  = rfcmn_concurrency_db.curr_state[temp_rfm_dev].device_op;  
        compare_tbl_entry = FALSE;
      ...
      MSG_4(MSG_SSID_RF_DEBUG, MSG_LEGACY_HIGH, "rf_ccry_update_dev_alloc: MF - %d, EF = %d, Index = %d, Tbl_index = %d",  
  ...
  if (rf_ccry_mgr_time_prof)
  {
    RF_MSG_8(RF_HIGH, "rf_ccry_update_dev_alloc: updated DB tech - %d, %d, %d, %d, %d, %d, %d, %d",  
             ...
             rfcmn_concurrency_db.curr_state[6].curr_mode,
             rfcmn_concurrency_db.curr_state[7].curr_mode); 
    RF_MSG_8(RF_HIGH, "rf_ccry_update_dev_alloc: updated DB band - %d, %d, %d, %d, %d, %d, %d, %d",  
             ...
             rfcmn_concurrency_db.curr_state[6].curr_band,
             rfcmn_concurrency_db.curr_state[7].curr_band); 
    RF_MSG_8(RF_HIGH, "rf_ccry_update_dev_alloc: updated DB alt_path - %d, %d, %d, %d, %d, %d, %d, %d",  
             ...
             rfcmn_concurrency_db.curr_state[6].alt_path,
             rfcmn_concurrency_db.curr_state[7].alt_path); 
    RF_MSG_8(RF_HIGH, "rf_ccry_update_dev_alloc: updated DB device_op - %d, %d, %d, %d, %d, %d, %d, %d",  
             rfcmn_concurrency_db.curr_state[0].device_op, 
             rfcmn_concurrency_db.curr_state[1].device_op,
             rfcmn_concurrency_db.curr_state[2].device_op,
             rfcmn_concurrency_db.curr_state[3].device_op,
             rfcmn_concurrency_db.curr_state[4].device_op,
             rfcmn_concurrency_db.curr_state[5].device_op,
             rfcmn_concurrency_db.curr_state[6].device_op,
             rfcmn_concurrency_db.curr_state[7].device_op); 
  }
...
)
{
       ...
       rfcmn_concurrency_db.curr_state[dev_client->rfm_dev].curr_mode != RFM_PARKED_MODE )
  {
    ...
    RFM_CM_DEBUG_EN_Q(&rfcmn_concurrency_mgr_rx_validate_scenario, dev_client->rfm_dev, dev_client->tech, dev_client->band, dev_client->operation, &rx_reconfig);
  ...
  for (idx = 0; idx < tbl_size; idx++)
  {
          ...
          temp_rfm_dev < (rfm_device_enum_type)lst_size; temp_rfm_dev++ )
    {
      ...
      if(dev_client->rfm_dev == temp_rfm_dev)
      {
        ...
        test_dev_op  = dev_client->operation; 
      }
      else
      {
  ...
  else
  {
    ...
    MSG_1(MSG_SSID_RF_DEBUG, MSG_LEGACY_HIGH,"rf_ccry_validate_scenario: Match not found - %d", tbl_index);
  }
