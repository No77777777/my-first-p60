...
/*==============================================================================
  ...
  $Header: //components/rel/rfa.mpss/4.11/rf/common/rf/core/src/rfcommon_core_xpt.c#1 $

when       who     what, where, why
--------   ---     -------------------------------------------------------------
01/20/20   ssl     Removal of Unsupported ET mode rf_common_xpt_load_dpd_tbls in TA/HA
01/18/17   sk      Send Tx handle to XPT api's for LTE
08/11/16   ndb     Update the TXLM handle in rflm_txagc_load_dpd_cfg_t
06/11/15   spa     KW warning fix
06/10/15   spa     Reduce stack usage in rfcommon_core_populate_tx_dpd API
04/09/15   zhw     Remove deprecated function
  ...
  if ( dpd_table_sz >  interp_dpd_num )
  {
          ...
          idx++ )
    {
      dpd_table[idx] = last_amam;
          ...
          idx < dpd_table_sz;
          idx++ ) */
    MSG_3( MSG_SSID_RF_DEBUG, MSG_LEGACY_HIGH, "rfcommon_xpt_dpd_padding: DPD %d Padded - Table "
...
/*!
  @brief
  Helper API to free the memory allocated for AMAM/AMPM/EPDT tables
...
*/
static void 
rfcommon_core_xpt_free_dpd_tbl_mem
(
  uint16 *amam_tbl,
  uint16 *ampm_tbl,
  uint16 *epdt_tbl
...
{
  /* Deallocate the memory pointers which are not null */
  if (amam_tbl!=NULL)
  {
    modem_mem_free( amam_tbl, MODEM_MEM_CLIENT_RFA );
  }

  if (ampm_tbl!=NULL)
  {
    modem_mem_free( ampm_tbl, MODEM_MEM_CLIENT_RFA );
  }

  if (epdt_tbl!=NULL)
  {
    modem_mem_free( epdt_tbl, MODEM_MEM_CLIENT_RFA );
  }
}/* rfcommon_core_xpt_free_dpd_tbl_mem */
/*----------------------------------------------------------------------------*/
/*!
...
)
{
  ...
  uint8  bank_index;
  uint16 bank_offset;
  uint16 *amam_tbl = NULL;
  uint16 *ampm_tbl = NULL;
  uint16 *epdt_tbl = NULL;
  uint16 lin_row_idx;
  uint16 dpd_idx;
  ...
  /* Allocate memory for tables on heap */
  amam_tbl = modem_mem_alloc((64)*sizeof(uint16), 
                                   MODEM_MEM_CLIENT_RFA);

  ampm_tbl = modem_mem_alloc((64)*sizeof(uint16), 
                                  MODEM_MEM_CLIENT_RFA);

  epdt_tbl = modem_mem_alloc((128)*sizeof(uint16), 
                                   MODEM_MEM_CLIENT_RFA);

  /* Make sure the pointers are valid after allocation */
  if (( amam_tbl == NULL )|| ( ampm_tbl == NULL )|| ( epdt_tbl == NULL ))
  {
    RF_MSG( RF_ERROR, "rfcommon_core_populate_tx_dpd - Memory allocation"
                      " failure for AMAM/AMPM/EPDT Tables " );
    /* Free the memory before return*/
    rfcommon_core_xpt_free_dpd_tbl_mem( amam_tbl, ampm_tbl, epdt_tbl);
  ...
  {
    expansion_ratio.amam = DPD_TYPE_AMAM_EXPANDED_TBL_SIZE/XPT_DPD_NUM_COEFF;
    expansion_ratio.ampm = DPD_TYPE_AMPM_EXPANDED_TBL_SIZE/XPT_DPD_NUM_COEFF;
    expansion_ratio.epdt = DPD_TYPE_EPDT_EXPANDED_TBL_SIZE/XPT_DPD_NUM_COEFF;
  }
  else
  {
          ...
          lin_row_idx++)
    {
      ...
      if (lin_dpd_lut[dpd_idx] == 0xFF)
      {
        ...
        if (api_status == TRUE)
        {
          ...
          if ( DPD_TYPE_AMAM_EXPANDED_TBL_SIZE > (expansion_ratio.amam*XPT_DPD_NUM_COEFF) )
          {
            rfcommon_xpt_dpd_padding( DPD_TYPE_AMAM,
                                      DPD_TYPE_AMAM_EXPANDED_TBL_SIZE,
                                      expansion_ratio.amam*XPT_DPD_NUM_COEFF,
                                      amam_tbl );
          } /* if ( DPD_TYPE_AMAM_EXPANDED_TBL_SIZE > 
            (expansion_ratio.amam*XPT_DPD_NUM_COEFF) ) */
        ...
        if (api_status == TRUE)
        {
          api_status &=
                  rf_common_xpt_load_dpd_tbls(device, 
                                              tx_handle,
                                              ampm_tbl,
                                              DPD_TYPE_AMPM_EXPANDED_TBL_SIZE,
                                              DPD_TYPE_AMPM,
                                              &bank_index,
                                              &bank_offset, 
                                              XPT_DPD_ACTION_SAVE,
          ...
          if (api_status == TRUE)
          {
            api_status &=
                    rf_common_xpt_load_dpd_tbls(device, 
                                                tx_handle,
                                                epdt_tbl,
                                                DPD_TYPE_EPDT_EXPANDED_TBL_SIZE,
                                                DPD_TYPE_EPDT,
                                                &bank_index,
                                                &bank_offset, 
                                                XPT_DPD_ACTION_SAVE,
...
)
{
 ...
 api_status &= 
    rf_common_xpt_load_dpd_tbls(device, 
                                ...
                                &amam_bank_offset, 
                                XPT_DPD_ACTION_INIT,
                                RFCOMMON_MDSP_XPT_MODE_ET);  --> ET mode not supported in TA/HA
...
)
{
  ...
  if ((txlm_handle == NULL) || (current_mode != RFCOM_LTE_MODE))
  {
    local_txlm_handle = rfcommon_core_get_tx_handle_from_device( (rfm_device_enum_type)device );
  }
  else
  {
    local_txlm_handle = *txlm_handle;
  }

  RF_MSG_2( RF_MED,"rf_common_xpt_load_dpd_tbls()->modem_chain:%d, txlm_handle:%d",
                   modem_chain,local_txlm_handle);

  load_dpd_cfg.device = device;
  load_dpd_cfg.modem_chain = modem_chain; 
  load_dpd_cfg.txlm_handle = local_txlm_handle;
...
)
{
  ...
  RF_MSG_4( RF_HIGH, "rfcommon_xpt_load_linearizer_for_override: "
                     ...
                     "RFM Mode %d, Dev %d, [Status %d], time elapsed: %d us",
