...
$Header: //components/rel/rfa.mpss/4.11/rf/common/rf/core/src/rfcommon_core_txlin.c#1 $ 


when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
02/07/20   ssl     [CR2616180]Heap opt: making ssbi_script_size as zero 
03/28/18   rp      KW Fixes.
01/15/18   am      F3 reduction
03/30/16    ag      Pick correct therm bin for pin calculation from NV during autopin
03/29/16    kr     FR 32798: LTE 40M separated calibration for QPA4340
03/01/16   mah     Converted common linearizer channel to 32 bits
02/23/16   zhh    Added delta gain Pin Pout suppor
12/28/15    sk     Bug fix for APT power computation for Intra-band ULCA LUT construction.
11/20/15    sk     Apply 20 vs 40 Mhz RGI Char Delta only once
11/20/15    sk     Pass Tx APT CHAR data by reference to avoid stack overflow
11/03/15    am     Added support for constructing APT char based Intra-band ULCA Linearizer.
09/23/15    pv     Added APT char for Intra-band ULCA LUT construction.
09/17/15   kma     Added auto pin support for Pin cal
08/06/15    pv     Intra-band ULCA LUT hook up for APT Char.
08/04/15   cd      Support skipping of Pout normalization for temp comp
07/24/15   vs      Add support for RGI based phase compensation 
06/23/15    pv     Intra-band ULCA LUT hook up.
06/22/15   cd      KW fix - check for num_queries to be within valid range
04/01/15   jr      Add ASM device to TxAGC configuration 
04/02/15   vr      Check for NULL pointer in sorting functionality
03/30/15   zhw     Remove hard-coded device mapping in common TxAGC
03/23/15   rcui    added rgi reporting support 
...
/*----------------------------------------------------------------------------*/
boolean
rfcommon_core_txlin_get_delta_gain_pin_pout
(
  const void * const nv_data_ptr,
  const uint32 num_bytes_available,
  uint32 * const extracted_element_size,
...
rfcommon_core_txlin_get_tx_agc_buffer ( rfm_device_enum_type device )
{
  ...
  uint8 chain_index = 0xFF ;

  chain_index = rfcommon_core_get_tx_chain_from_device(device);

  if ( chain_index >= RFCOMMON_MDSP_NUM_OF_TX_CHAINS )
  {
    RF_MSG_2 ( RF_ERROR , "rfcommon_core_txlin_get_tx_agc_buffer(): "
                          "Invalid chain idx Device passed: %d, "
                          "Tx_chain returned %d" , device, chain_index ) ;
  }
  else 
  {
    tx_agc_buff = &(tx_lin_info[chain_index].tx_agc_buffer[0]); 
  }
...
rfcommon_core_txlin_get_fbrx_lut_buffer ( rfm_device_enum_type device )
{
  ...
  if ( chain_index >= RFCOMMON_MDSP_NUM_OF_TX_CHAINS )
  {
    RF_MSG_2 ( RF_ERROR , "rfcommon_core_txlin_get_fbrx_lut_buffer(): "
                          "Invalid chain idx Device passed: %d, "
                          "Tx_chain returned %d" , device, chain_index ) ;
  }
  else 
  {
    fbrx_lut_buffer = &(tx_lin_info[chain_index].fbrx_lut_buffer[0]) ;
  }
...
rfcommon_core_txlin_get_dummy_trig_buffer ( rfm_device_enum_type device )
{
  ...
  if ( chain_index >= RFCOMMON_MDSP_NUM_OF_TX_CHAINS )
  {
    RF_MSG_2 ( RF_ERROR , "rfcommon_core_txlin_get_dummy_trig_buffer(): "
                          "Invalid chain idx Device passed: %d, "
                          "Tx_chain returned %d" , device, chain_index ) ;
  }
  else 
  {
    dummy_trig_buff = tx_lin_info[chain_index].dummy_trig_buffer ; 
  }
...
rfcommon_core_txlin_get_pa_trig_buffer ( rfm_device_enum_type device )
{
  ...
  if ( chain_index >= RFCOMMON_MDSP_NUM_OF_TX_CHAINS )
  {
    RF_MSG_2 ( RF_ERROR , "rfcommon_core_txlin_get_pa_trig_buffer(): "
                          "Invalid chain idx Device passed: %d, "
                          "Tx_chain returned %d" , device, chain_index ) ;
  }
  else 
  {
    pa_trig_buff = tx_lin_info[chain_index].pa_trig_buffer ; 
  }
...
)
{
  ...
  if ( chain_index >= RFCOMMON_MDSP_NUM_OF_TX_CHAINS )
  {
    RF_MSG_2 ( RF_ERROR , "rfcommon_core_txlin_get_lin_log_buffer(): "
                          "Invalid chain idx Device passed: %d, "
                          "Tx_chain returned %d" , device, chain_index ) ;
  }
  else 
  {
    log_data = tx_lin_info[chain_index].log_data; 
  }
  return log_data;
}
...
)
{
  ...
  {
    MSG_1( MSG_SSID_RF_DEBUG, MSG_LEGACY_MED, "Failed to find linearizer table for Tx frequency:%d!", tx_freq);
  ...
  for (rgi_index=0;rgi_index<RFCOM_TXLIN_NUM_SEGMENTS;rgi_index++)
  {
    ...
    if ( tx_final_linearizer->table[rgi_index].rgi != 0xFFFF )
    {
      ...
      else
      {
        ...
        if((&tx_band_cal_data->tx_pa_state_cal_data[pa_range])->linearizer_index[TX_LINEARIZER_DATA_TYPE_40MHZ][0].node_data.lin_data == NULL)
        {
...
)
{
              ...
              RFCOM_TXLIN_NORMALIZE_TO_CURR_FREQ_REF_TEMP )
  {
    ...
    normalize_freq_index = freq_index;
              ...
              RFCOM_TXLIN_NORMALIZE_SKIP )
  {
    /* When normalization is not needed, set this to invalid */
    normalize_freq_interp = -1;
    normalize_freq_index = 255;
  }
  else
  {
    RF_MSG_1( RF_ERROR, 
  ...
  if ( normalization != RFCOM_TXLIN_NORMALIZE_SKIP )
  {
  ...
  {
    /* Normalization not required. Set value to 0 */
    center_point_pcomp_val = 0;
  }
  ...
  switch ( temp_comp_algo )
  {
     ...
     case RFCOM_TXLIN_TEMP_COMP_ALGO_MULTILIN_V3:
       ...
       *pin_offset += temp_vs_freq_params->ref_xpt_pin_offset;
       ...
       if ( temp_comp_nv_data->skip_pout_normalization[pa_state] == TRUE )
       {
         *pout_offset = 
           rfcommon_core_txlin_get_lin_vs_temp_vs_freq_compensation_w_adc(
                                   temp_vs_freq_params,
    ...
    switch ( temp_comp_algo )
    {
       case RFCOM_TXLIN_TEMP_COMP_ALGO_MULTILIN_V1:
         ...
         status = rfcommon_core_txlin_temp_comp_update_apt( input_tx_lut,
                                                            ...
                                                            pout_comp_val,
                                                            valid_lut_entries );
         RF_MSG_2( RF_MED, "rfcommon_core_txlin_temp_vs_freq_comp_update: "
       ...
       case RFCOM_TXLIN_TEMP_COMP_ALGO_MULTILIN_V3:
       {
         ...
         else
         {
           ...
           MSG_3( MSG_SSID_RF_DEBUG, MSG_LEGACY_MED, "rfcommon_core_txlin_temp_vs_freq_comp_update: "
       ...
       default:
         RF_MSG_1( RF_ERROR, "rfcommon_core_txlin_temp_vs_freq_comp_update: "
...
)
{
  ...
  else if ( num_queries < 1 || num_queries > RFCOM_TXLIN_NUM_SEGMENTS )
  {
    RF_MSG_1( RF_ERROR, "rfcommon_core_txlin_get_tx_device_lut_data: "
                        "num_queries value greater than %d",
  ...
  else
  {
    ...
    /*
    RF_MSG_2( RF_HIGH, "rfcommon_core_txlin_get_tx_device_lut_data: "
                       "input linearizer for Cal BW: %d, Test BW:%d",
              pwr_offset_bw.requested_offset_cal_bw.lte_bw, bw.lte_bw ); */
    ...
    for ( lin_index = 0; lin_index < num_queries; lin_index++ )
    {
      ...
      {
        /* Log the RGI used */
        log_data->pa_state_data[pa_state].rgi_data[lin_index].rgi = full_bias_rgi;
             ...
             txagc_buf[RFCMN_TXLIN_AGC_WXE_FBRX_TASK] != NULL )
        {
          ...
          /* Query LUT data for 20_plus_20Mhz */
          if (bw.lte_bw == RFCOM_BW_LTE_20MHz_plus_20MHz)
          {
            /* Only use the cal_bw in case of 20+20 Mhz case */
            pwr_offset_bw[lin_index].requested_offset_cal_bw.lte_bw = cal_bw.lte_bw;

            ret_val &= rfdevice_cmn_get_tx_lut_scripts_with_bw( wtr_device, 
                                                                rfm_mode, 
                                                                rfm_band,
                                                                bw,
                                                                pa_state,
                                                                ...
                                                                RFDEVICE_CREATE_SCRIPT,
                                                                &lut_data ,
                                                                &pwr_offset_bw[lin_index]);
          }
          else
          {
            ret_val &= rfdevice_cmn_get_tx_lut_scripts(wtr_device, 
                                                       rfm_mode, 
                                rfm_band,
                                pa_state,
                                full_bias_rgi,
  ...
  else
  {
    ...
    for ( lin_index = 0; lin_index < num_queries; lin_index++ )
    {
      ...
      if ( txagc_buf[RFCMN_TXLIN_AGC_PA_TASK] != NULL )
      {
        ...
        pa_params_s.bw = rfdevice_pa_convert_to_pa_bw(rfm_mode,bw);
...
/*!
  ...
  @param asm_device
  Pointer to the ASM device for band/tech/device
...
(
  ...
  rfcom_band_type_u rfm_band,
  int32 tx_handle_id,
  rfcommon_core_txlin_script_type_t script_buffer_id,
  void* wtr_device,
  void* pa_device,
  void* papm_device,
  rf_buffer_intf* tech_asm_tx_agc,
...
)
{
  ...
  boolean force_new_task[RFCMN_TXLIN_AGC_MAX_TASK] =
  {
    ...
    FALSE, /* RFCMN_TXLIN_AGC_ASM_TRIG_TASK */
  };
  ...
  else
  {
    ...
    for ( task_type = RFCMN_TXLIN_AGC_PAPM_DATA_TASK;
          ...
          task_type++ )
    {
      if( task_type == RFCMN_TXLIN_AGC_ASM_TRIG_TASK)
      {
        txagc_buf[RFCMN_TXLIN_AGC_ASM_TRIG_TASK] = tech_asm_tx_agc;
      }
      else
      {
      if ( txagc_buf[task_type] != NULL )
    ...
    if ( wtr_device != NULL )
    {
      ...
      /* Query FB Rx enable data */
      ret_val &= rfdevice_cmn_get_fbrx_enable_script( wtr_device,
                            0,
                            RFDEVICE_CREATE_SCRIPT,
                            txagc_buf[RFCMN_TXLIN_AGC_WXE_FBRX_TASK],
                            FALSE,
    ...
    if ( ret_val == TRUE)
    {
      ...
      ret_val &= rfcommon_mdsp_txagc_config_finalize_seq( tx_handle_id, script_buffer_id );
...
)
{
        ...
        lin_index++ )
  {
    ...
    if ( lin_index < RFCOMMON_MDSP_TXLIN_EPT_TBL_SIZE &&
         xpt_mode_type != RFCOMMON_MDSP_XPT_MODE_APT )
    {
       if(last_valid_index < RFCOMMON_MDSP_TXLIN_EPT_TBL_SIZE)
       {        
          tx_lin_data->add_env_scale_cap[lin_index] = 
       ...
       {
         RF_MSG_1( RF_ERROR, "rfcommon_core_txlin_get_non_device_linearizer_data: "
                             "wrong linearizer index %d for EPT PA state",
                              last_valid_index);
       }
    }
...
)
{
  ...
  rfcommon_core_txlin_row_info_type *row_info;
  rfcom_bw_type_u bw,cal_bw;
  rfdevice_tx_power_offset_bw_type pwr_offset_bw[RFCOM_TXLIN_NUM_SEGMENTS];
  ...
  /* For LTE ULCA need to know what is the current BW */
  bw.lte_bw = curr_tx_ovr->bw.lte_bw; 
  cal_bw.lte_bw = curr_tx_ovr->cal_bw.lte_bw;
  ...
  memset( row_info, 0, sizeof(rfcommon_core_txlin_row_info_type)*
                                                       num_tx_overrides );
  memset( &pwr_offset_bw, 0, sizeof(rfdevice_tx_power_offset_bw_type)*
                                                       RFCOM_TXLIN_NUM_SEGMENTS );
  ...
  /* Query WTR data */
  ret_val &= rfcommon_core_txlin_get_tx_device_lut_data( rfm_mode, 
                                                         device,
                                                         rfm_band, 
                                                         tx_handle_id,
...
*/
uint16
rfcommon_core_lininterp_s16_u16
(
  const int16 x[],
  const uint16  y[],
  const int    N,
...
*/
int16
rfcommon_core_lininterp_u16_s16
(
  const uint16 x[],
  const int16  y[],
  const int    N,
    ...
    if ( x[0] < x[N-1] )
    {
      ...
      else if ( i == N )
      {
        ...
        res = rf_divide_with_rounding( y[N-1]  *(unknown-x[N-2])
                                       + y[N-2]*(x[N-1]-unknown),
                                       (x[N-1]-x[N-2]) );
        return (int16)rf_limit_u32( res, 0, 65535 );
      }
      else
      {
        int32 result;
        ...
        result = rf_divide_with_rounding(   y[i]  *(unknown-x[i-1])
                                          + y[i-1]*(x[i]-unknown),
                                          (x[i]-x[i-1]) );
        return (int16)rf_limit_u32( result, 0, 65535 );
      }
    } /* if ( x[0] < x[N-1] ) */
    else
    {
...
void rfcommon_core_txlin_construct_linearizer
(
  rfcom_bw_type_u bw,
  rfcom_bw_type_u cal_bw,
  rfcommon_core_txlin_row_info_type *row_info,
  rfnv_tx_static_data_type *curr_pa_state_apt_char_tbl,
  rfdevice_tx_power_offset_bw_type *pwr_offset_bw,
  boolean apt_char_based_linearizer_construction
)
{
  uint8 lin_index = 0, vcc_index = 0, max_vcc_index_entry = 0;

  /* Characterization data: Power measurement */
  int16   local_apt_pwr_db10 [RFNV_TX_STATIC_APT_CHAR_ENTRIES]; 

  /* Characterization data: Applied VCC measurement */
  uint16  local_vcc_mv[RFNV_TX_STATIC_APT_CHAR_ENTRIES]; 
  int16   local_gain_delta_vs_rgi[RFCOM_TXLIN_NUM_SEGMENTS];
  uint16  local_vcc_vs_rgi[RFCOM_TXLIN_NUM_SEGMENTS];
  int16   apt_pwr_gain_delta = 0;
  uint8   apt_gain_lin_index = 0;

  RF_MSG_6(RF_MED, 
           "rfcommon_core_txlin_config_init_linearizer: "
           "Linearizer contruction allowed:%d"
           "APT Char BW:%d, APT Tx earfcn:%d, APT PA State:%d, APT NV container:%d, xpt_mode_type:%d", 
           apt_char_based_linearizer_construction,
           curr_pa_state_apt_char_tbl->tx_bandwidth,
           curr_pa_state_apt_char_tbl->tx_earfcn,
           curr_pa_state_apt_char_tbl->pa_state_idx,
           curr_pa_state_apt_char_tbl->nv_container_idx,
           row_info[0].xpt_mode); 

  /* Only for Intra-band uLCA case */
  if ((bw.lte_bw == RFCOM_BW_LTE_20MHz_plus_20MHz) && 
      (apt_char_based_linearizer_construction == TRUE)&&
      (curr_pa_state_apt_char_tbl->tx_earfcn != 0)&&
      (curr_pa_state_apt_char_tbl->tx_bandwidth == bw.lte_bw)&&
      (curr_pa_state_apt_char_tbl->vcc_mv[0] != 
       curr_pa_state_apt_char_tbl->vcc_mv[RFNV_TX_STATIC_APT_CHAR_ENTRIES-1])&&
      (curr_pa_state_apt_char_tbl->pwr_db10[0] != 
       curr_pa_state_apt_char_tbl->pwr_db10[RFNV_TX_STATIC_APT_CHAR_ENTRIES-1]))
  {
    max_vcc_index_entry = RFNV_TX_STATIC_APT_CHAR_ENTRIES;

    /* Prepare the complete table of 32 entries by expanding the last entry all the way */
    for (vcc_index = 0; vcc_index < RFNV_TX_STATIC_APT_CHAR_ENTRIES; vcc_index++)
    { 
      if (vcc_index < max_vcc_index_entry)
      { 
        /* Detect the max row for APT Char table supplied*/
        if (curr_pa_state_apt_char_tbl->vcc_mv[vcc_index] == 0) 
        {
          max_vcc_index_entry = vcc_index;
        }
        else
        {
          /* Copy the valid entries into a local buffer for inter-polation */
          local_apt_pwr_db10[vcc_index] = curr_pa_state_apt_char_tbl->pwr_db10[vcc_index];
          local_vcc_mv[vcc_index] = curr_pa_state_apt_char_tbl->vcc_mv[vcc_index];
        }
      }
    } 

    /* Prepare the Gain Delta */
    for ( lin_index = 0; lin_index < (RFCOM_TXLIN_NUM_SEGMENTS); lin_index++ )
    {
      local_gain_delta_vs_rgi[lin_index] = row_info[lin_index].fullbias_pwr - row_info[lin_index].smps_pwr;
      local_vcc_vs_rgi[lin_index] = row_info[lin_index].papm_bias;
      if (lin_index > 0)
      {
       /*
        RF_MSG_8(RF_MED, 
                   "rfcommon_core_txlin_config_init_linearizer: "
                   "VCC vs RGI [%d]:%d, VCC vs RGI [%d]:%d"
                   "Gain Delta vs RGI[%d]:%d,Gain Delta vs RGI[%d]:%d ", 
                   lin_index, local_vcc_vs_rgi[lin_index],
                   lin_index-1, local_vcc_vs_rgi[lin_index-1],
                   lin_index, local_gain_delta_vs_rgi[lin_index],
                   lin_index-1, local_gain_delta_vs_rgi[lin_index-1]); */
      }
    }

    /* Sweep for Computing:
       Step1: FullBias table with 20+20 LUT and power delta
       Step2: APT Bias for 20+20 Linearizer based on 20+20 APT char and 10Mhz RGI from Cal */
    for ( lin_index = 0; lin_index < RFCOM_TXLIN_NUM_SEGMENTS; lin_index++ )
    {
      apt_pwr_gain_delta = 0;
      if ((row_info[lin_index].xpt_mode == RFDEVICE_APT_MODE) && 
          (row_info[lin_index].fullbias_rgi_index != RFCOM_TXLIN_INVALID_RGI ) &&
          (pwr_offset_bw[lin_index].offset_valid == TRUE))
      {
        /*
        RF_MSG_7( RF_MED, 
                  "rfcommon_core_txlin_config_init_linearizer: "
                  "Calibrated smps_rgi_index:%d, "
                  "smps_pwr:%d,papm_bias:%d,Full Bias RGI Index:%d, fullbias_pwr:%d, PA ICQ:%d, RGI PWR delta:%d ", 
                  row_info[lin_index].smps_rgi_index, 
                  row_info[lin_index].smps_pwr,
                  row_info[lin_index].papm_bias,
                  row_info[lin_index].fullbias_rgi_index,
                  row_info[lin_index].fullbias_pwr,
                  row_info[lin_index].pa_curr,
                  pwr_offset_bw[lin_index].offset_db10); */

        if (lin_index >= RFNV_TX_STATIC_APT_CHAR_ENTRIES)
        {
          vcc_index = RFNV_TX_STATIC_APT_CHAR_ENTRIES - 1;
        }
        else
        {
          vcc_index = lin_index;
        ...
        /* Step B: Construct the 20+20 Mhz Full Bias Table */
         /* The delta is 20Mhz - 40Mhz, hence the new Linearizer = 20Mhz (Cal power) - RGI delta */
        row_info[lin_index].fullbias_pwr -= pwr_offset_bw[lin_index].offset_db10;
        
        /* Find the APT Bias based on the APT pwr extracted */
        row_info[lin_index].papm_bias = rfcommon_core_lininterp_s16_u16(local_apt_pwr_db10,
                                                                        local_vcc_mv,
                                                                        max_vcc_index_entry,
                                                                        row_info[lin_index].fullbias_pwr);
        /* Determine APT gain Delta */
        for (apt_gain_lin_index = 0; 
             apt_gain_lin_index < RFCOM_TXLIN_NUM_SEGMENTS-1;
             apt_gain_lin_index++)
        {
          apt_pwr_gain_delta = 0;
          
          if ((row_info[lin_index].papm_bias > local_vcc_vs_rgi[apt_gain_lin_index]) &&
              (row_info[lin_index].papm_bias <= local_vcc_vs_rgi[apt_gain_lin_index+1]))
          {
            apt_pwr_gain_delta = rfcommon_core_lininterp_u16_s16(&(local_vcc_vs_rgi[apt_gain_lin_index]), 
                                                                 &(local_gain_delta_vs_rgi[apt_gain_lin_index]),
                                                                 2,
                                                                 row_info[lin_index].papm_bias);
            /*
            RF_MSG_9(RF_MED, 
                 "rfcommon_core_txlin_config_init_linearizer Interpolation for APT Gain Delta: "
                 "VCC vs RGI [%d]:%d, VCC vs RGI [%d]:%d"
                 "Gain Delta vs RGI[%d]:%d,Gain Delta vs RGI[%d]:%d ", 
                 apt_gain_lin_index, local_vcc_vs_rgi[apt_gain_lin_index],
                 apt_gain_lin_index+1, local_vcc_vs_rgi[apt_gain_lin_index+1],
                 apt_gain_lin_index, local_gain_delta_vs_rgi[apt_gain_lin_index],
                     apt_gain_lin_index+1, local_gain_delta_vs_rgi[apt_gain_lin_index+1], apt_pwr_gain_delta); */
            break;
          }
        }

        row_info[lin_index].smps_pwr = row_info[lin_index].fullbias_pwr 
                                       - apt_pwr_gain_delta;

        row_info[lin_index].pa_curr = curr_pa_state_apt_char_tbl->icq[0];
        
        /*
         RF_MSG_7(RF_MED, 
                  "rfcommon_core_txlin_config_init_linearizer: "
                  "After Computed smps_rgi_index:%d,smps_pwr:%d,papm_bias:%d,"
                  "Full Bias RGI Index:%d, fullbias_pwr:%d, RGI PWR delta:%d PA ICQ:%d", 
                  row_info[lin_index].smps_rgi_index, 
                  row_info[lin_index].smps_pwr,
                  row_info[lin_index].papm_bias,
                  row_info[lin_index].fullbias_rgi_index,
          ...
          RF_MSG_3(RF_MED, 
                  "rfcommon_core_txlin_config_init_linearizer: "
                  "Characterized smps_pwr:%d, papm_bias:%d, icq:%d"
                  "Computed APT smps_pwr:%d, APT smps_bias:%d", 
                  curr_pa_state_apt_char_tbl->pwr_db10[vcc_index],
                  curr_pa_state_apt_char_tbl->vcc_mv[vcc_index],
                  curr_pa_state_apt_char_tbl->icq[vcc_index]);*/
      }
    }
  }
  else
...
)
{
  ...
  /* Linearizer Construction Algo */
  rfcommon_core_txlin_construct_linearizer(bw,
                                           cal_bw,
                                           &row_info[0],
                                           curr_pa_state_apt_char_tbl,
                                           &pwr_offset_bw[0],
                                           apt_char_based_linearizer_construction);

  /* Query PA data */
  ret_val &= rfcommon_core_txlin_get_pa_device_lut_data(rfm_mode, device, rfm_band, 
                                              device_info->pa_device,
                                              tx_lin_data->calibrated_lin_size,
                                              row_info,
                                              log_data,
  ...
  /* Query PAPM data */
  ret_val &= rfcommon_core_txlin_get_papm_device_lut_data(rfm_mode, device , rfm_band, 
                                              device_info->papm_device,
                                              tx_lin_data->calibrated_lin_size,
                                              row_info,
                                              log_data,
  ...
  /* Query non-device related linearizer data  */
  ret_val &= rfcommon_core_txlin_get_non_device_linearizer_data( rfm_mode, device, 
                                              rfm_band,
                                              tx_lin_data->full_bias_lin_size,
                                              tx_lin_data->calibrated_lin_size,
                                              xpt_mode_type,
  ...
  ret_val &= rfcommon_mdsp_txagc_config_static_linearizer(tx_handle_id,
                                            pa_state,
                                            RFCOM_TXLIN_NUM_SEGMENTS,
                                            tx_lin_data );

  ret_val &= rfcommon_mdsp_txagc_config_dynamic_linearizer( tx_handle_id,
                                            pa_state,
                                            RFCOM_TXLIN_NUM_SEGMENTS,
                                            tx_lin_data , 
                                            FALSE /* Dont Skip Pout Comp*/ ) ;
  ...
  if ( ret_val == TRUE )
  {
    RF_MSG_2( RF_MED, "rfcommon_core_txlin_config_init_linearizer: "
                      "Configured for PA State %d, time elapsed: %d us", 
...
)
{
  ...
  if ( ret_val == TRUE )
  {
    MSG_2( MSG_SSID_RF_DEBUG, MSG_LEGACY_MED, "rfcommon_core_txlin_update_linearizer: Configured for "
                                              "PA State %d,time elapsed: %d us", 
              pa_state, prof_elapsed_t );
  }
  else
  {
    ...
    for( type_id_idx = 0; (type_id_idx < variant_marker.num_elements) && (api_status == TRUE); type_id_idx++ )
    {
      ...
      switch( type_id )
      {
        ...
        api_status &= rfcommon_core_tx_multilin_extract_lin_v4( 
                                            ...
                                            &et_dpd.num_dpd_avail );
        break ;
        case RFNV_DATA_DELTA_GAIN_PIN_POUT_TYPE:
        api_status &= rfcommon_core_txlin_get_delta_gain_pin_pout(
                                            temp_data_ptr,
                                            temp_num_bytes_left,
                                            &nv_element_size,
                                            tx_cal_data);
          ...
          if (api_status == TRUE)
          {
                  ...
                  row_idx++ )
            {
              ...
              if (dpd_idx_allocated == FALSE)
              {
                ...
                /* AM */
				if(row_idx_w_dpd < RFCOM_TXLIN_NUM_SEGMENTS)
				{
...
/*!
  @brief  
  HELPER API for extracting delta pin pout table
...
)
{
  ...
  boolean api_status = TRUE;
  /* The item to extract from NV */
  tx_delta_gain_pin_pout_nv_type tx_delta_gain_pin_pout;
  /* Contains the number of bytes for the element in NV */
  uint32 num_bytes_needed;
  /* For indexing */
  uint16 row_idx;
  tx_delta_gain_pin_pout_table_type* delta_gain_pin_pout_ptr = NULL;

  num_bytes_needed = sizeof( tx_delta_gain_pin_pout_nv_type);
  ...
  *extracted_element_size = 0;
  ...
  if( num_bytes_available < num_bytes_needed )
  {
    RF_MSG( RF_ERROR, "MultiLin V3 - Not enough elements left to extract item: Delta Gain Pin Pout Table!" );
    api_status = FALSE;
  }
  ...
  {
    /* Extract the element */
    memscpy( (void*) &tx_delta_gain_pin_pout, num_bytes_needed, nv_data_ptr, num_bytes_needed );
    *extracted_element_size = num_bytes_needed;

    /* Validate parameters */
    if(tx_delta_gain_pin_pout.dpd_idx >= RFCOM_TXLIN_NUM_PIN_LUT)
    {
      RF_MSG_2( RF_ERROR, "MultiLin V3 - Param validation failed. dpd_idx %d >= %d.",
                           tx_delta_gain_pin_pout.dpd_idx,
                           RFCOM_TXLIN_NUM_PIN_LUT );
      api_status = FALSE;
    }

    if(tx_delta_gain_pin_pout.num_pin > RFCOM_TXLIN_NUM_PIN)
    {
      RF_MSG_2( RF_ERROR, "MultiLin V3 - Param validation failed. num_pin %d > %d.",
                           tx_delta_gain_pin_pout.num_pin, 
                           RFCOM_TXLIN_NUM_PIN );
      api_status = FALSE;
    }
    ...
    if(TRUE == api_status)
    {
      delta_gain_pin_pout_ptr = (tx_delta_gain_pin_pout_table_type*)modem_mem_alloc( sizeof(tx_delta_gain_pin_pout_table_type),
                                                                        MODEM_MEM_CLIENT_RFA );

      if(NULL == delta_gain_pin_pout_ptr)
      {
        RF_MSG( RF_ERROR, "MultiLin V3 - No Available.memory for Pin LUT");
        api_status = FALSE;
      }
    }
    ...
    if ( TRUE == api_status )
    {
      delta_gain_pin_pout_ptr->dpd_idx = tx_delta_gain_pin_pout.dpd_idx;
      delta_gain_pin_pout_ptr->num_pin = tx_delta_gain_pin_pout.num_pin;
      for(row_idx = 0; row_idx < tx_delta_gain_pin_pout.num_pin; row_idx++)
      {
        delta_gain_pin_pout_ptr->pin_misalign[row_idx] = tx_delta_gain_pin_pout.pin_misalign[row_idx];
        delta_gain_pin_pout_ptr->gain_comp[row_idx] = tx_delta_gain_pin_pout.gain_comp[row_idx];
        delta_gain_pin_pout_ptr->pout_delta[row_idx] = tx_delta_gain_pin_pout.pout_delta[row_idx];
      }

      tx_cal_data->pin_lut[tx_delta_gain_pin_pout.dpd_idx] = delta_gain_pin_pout_ptr;
    }
  } /* if( api_status == TRUE ) */
