...
$Header: //components/rel/rfa.mpss/4.11/rf/common/rf/core/src/rfcommon_core_device_manager.c#1 $

when       who     what, where, why
-------------------------------------------------------------------------------
04/05/18   am      Moved high hitting F3 msgs to Tranciever log packet
08/06/15   hkm     F3 optimization.
...
)
{
  ...
  RF_MSG_3(RF_HIGH,"rfcommon_core_dev_power_vote: called on device:%d,dev_status:%d,dev_voting_type:%d",rfm_dev_val,dev_status,dev_voting_type);

  RFDEVICE_LOG_MSG_3(LOG_RFA_DEVICE_TRASCEIVER_C, MSG_LEGACY_LOW,
		  ...
		  if (rfm_dev_val == rfcmn_core_mgr.rfm_dev_for_gps)
		  {
            for (cntr = 0; cntr < rfcmn_core_mgr.phy_dev_count; cntr++ )
            {
              if (rfcmn_core_mgr.gps_dev_mgr[cntr] != NULL)
              {
                ...
                {
                  RFDEVICE_LOG_MSG_6(LOG_RFA_DEVICE_TRASCEIVER_C, MSG_LEGACY_MED,
                  ...
                  else /* enable GPS when WWAN is already ON */
                  {
                    ...
                    RF_MSG(RF_HIGH,"rfdevice_cmn_enable_gps: called with RFDEV_VOTE_ON with NORMAL voting ");
                  }
                }
                else
                {
                 RFDEVICE_LOG_MSG_3(LOG_RFA_DEVICE_TRASCEIVER_C, MSG_LEGACY_MED, 
              ...
              {
                RFDEVICE_LOG_MSG_1(LOG_RFA_DEVICE_TRASCEIVER_C, MSG_LEGACY_MED, 
                         "Entering rfcmn_core_dev_power_vote()GPS based Device#: %d, "
                         "doesn't exist ",cntr); 
                 ...
                 dev_mgr_index++ )
            {
              ...
              if (rfcmn_core_mgr.is_wwan_vote_on == FALSE)
              {
                if (rfcmn_core_mgr.is_gps_vote_on == FALSE) /* enable WWAN from chip OFF state */
                {
                 ...
                 RF_MSG(RF_HIGH,"rfdevice_cmn_power_up: called with RFDEV_VOTE_ON with NORMAL voting ");
                ...
                else /* enable WWAN when GPS is already ON */
                {
                  ...
                  RF_MSG(RF_HIGH,"rfdevice_cmn_enable_wwan: called with RFDEV_VOTE_ON with NORMAL voting "); 
                }
              }
              else
              {
                RFDEVICE_LOG_MSG_2(LOG_RFA_DEVICE_TRASCEIVER_C, MSG_LEGACY_MED, 
                        "WWAN is already ON; "
  ...
  RFDEVICE_LOG_MSG_1(LOG_RFA_DEVICE_TRASCEIVER_C, MSG_LEGACY_LOW,
...
rfcommon_core_dev_light_sleep()
{
  ...
  {
    RFDEVICE_LOG_MSG(LOG_RFA_DEVICE_TRASCEIVER_C, MSG_LEGACY_MED, 
...
boolean rfcommon_core_dev_mgr_init()
{
  ...
  RFDEVICE_LOG_MSG(LOG_RFA_DEVICE_TRASCEIVER_C, MSG_LEGACY_MED," Entering rfcmn_core_dev_mgr_init()");
...

{
  ...
  */
  if (rfm_dev_val == rfcmn_core_mgr.rfm_dev_for_gps)
  {
	...
	RF_MSG(RF_HIGH,"rfdevice_cmn_enable_gps: Called for VOTE_OFF with RFDEV_CMN_NO_VOTING ");
	
    status_of_opr &= 
        rfdevice_cmn_power_reset(rfcmn_core_mgr.gps_dev_mgr[cntr]->dev,
                                 RFDEV_CMN_GPS_CHIP_OFF);

	RF_MSG(RF_HIGH,"rfdevice_cmn_power_reset: Called for RFDEV_CMN_GPS_CHIP_OFF");
      }
      else
      {
        RFDEVICE_LOG_MSG_1(LOG_RFA_DEVICE_TRASCEIVER_C, MSG_LEGACY_MED,"Entering rfcmn_core_dev_power_vote()GPS based Device#: %d, "
                        "doesn't exist ", cntr);
      }
    }
            ...
            dev_mgr_index++ )
  {
      if (rfcmn_core_mgr.dev_mgr[dev_mgr_index].dev_status == RFCMN_CORE_DEV_ON)
    {
      ...
      rfdevice_cmn_enable_wwan(rfcmn_core_mgr.dev_mgr[dev_mgr_index].dev,
        ...
        RF_MSG(RF_HIGH,"rfdevice_cmn_enable_wwan: Called for VOTE_OFF with RFDEV_CMN_NO_VOTING ");

        status_of_opr &= rfdevice_cmn_power_reset(rfcmn_core_mgr.dev_mgr[dev_mgr_index].dev,
                                                  RFDEV_CMN_WWAN_RESET);

       RF_MSG(RF_HIGH,"rfdevice_cmn_power_reset: Called with RFDEV_CMN_WWAN_RESET");
...
boolean rfcommon_core_all_phy_dev_power_down()
{
  ...
  if (rfcmn_core_mgr.debug_flag == TRUE)
  {
    RFDEVICE_LOG_MSG(LOG_RFA_DEVICE_TRASCEIVER_C, MSG_LEGACY_MED," Entering rfcmn_core_dev_check_all_phy_dev_power_down()!");
  }
          ...
          ++dev_index)
    {
      ...
      if (transceiver_turn_off == TRUE)
      {
        ...
        (rfdevice_cmn_power_up(rfcmn_core_mgr.dev_mgr[dev_index].dev,
         ...
         RF_MSG(RF_HIGH,"rfdevice_cmn_power_up: called for VOTE_OFF with RFDEV_CMN_NO_VOTING"); 
...
void rfcommon_core_mgr_debug_msgs()
{
  ...
  RFDEVICE_LOG_MSG(LOG_RFA_DEVICE_TRASCEIVER_C, MSG_LEGACY_MED," RF Global Manager Structure!!");
...
//  rfdevice_rxtx_common_class* temp_gps_dev_param = NULL;
  ...
  {
    RFDEVICE_LOG_MSG(LOG_RFA_DEVICE_TRASCEIVER_C, MSG_LEGACY_MED,"detect_gnns_devices(): GNSS devices detected");
  }
}
