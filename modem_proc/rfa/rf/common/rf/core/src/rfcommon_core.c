...
$Header: //components/rel/rfa.mpss/4.11/rf/common/rf/core/src/rfcommon_core.c#2 $
...
05/28/15   sty     deleted redundant F3
...
rfm_init_status_type_e_type rfc_init_status = RFM_INIT_FAILURE_NONE;
...
/*!
  ...
  @retval api_status: 0 if rfc_set_card reports failure
  (due to illegal RF card request), 1 if successful
*/
rfm_init_status_type_e_type rfcommon_core_init(rf_path_enum_type path)
{
...
/* Initialize MSM Block */
 ...
 /* initialize the mdsp interface layer for IRAT. */
  ...
  rfc_init_status = rfc_set_card(rf_hw);
  if(rfc_init_status != RFM_INIT_FAILURE_NONE)
  {
    RF_MSG_1(RF_ERROR,"rfcommon_core_init(): rfc_set_card failed.returning %d.",rfc_init_status);
    return rfc_init_status;
  }
  else
  {
  	api_status = TRUE;
 ...
 /* Initialize the Device Manager structures */
  api_status &= rfcommon_core_dev_mgr_init();

  #ifdef FEATURE_RFA_ACL

  /* Initialize the MAAT manager structures */
  api_status &= rfcmn_core_maat_mgr_init();

  #endif

  api_status &= rfc_load_internal_device_calibration();

  /* Initializes the callback services */
  rfcommon_core_cb_init();

  /* Initialize tx linearizer config data */
  api_status &= rfcommon_core_txlin_init_data();
  
  if (api_status)
    return RFM_INIT_FAILURE_NONE;
  else
    return RFM_INIT_FAILURE_SW;
...
rf_path_enum_type rfcommon_core_device_to_path( rfcom_device_enum_type device )
{
  ...
  switch (device)
  {
    ...
    default :
      ...
      MSG_1(MSG_SSID_RF_DEBUG, MSG_LEGACY_ERROR, "Invalid device: no path found for Device: %d",device);
      temp_path = RF_PATH_0;
      #endif
	  #endif
      break;
  ...
  if(rfwcdma_data_get_device_type(device) == RFM_WCDMA_TX_DEVICE)
  {
    temp_path  =    RF_PATH_0 ;/*Temp hack. Need to fix. WCDMA to deprecate the usage of this API*/
   }
  #endif
  #endif
  return temp_path;
...
*/
rf_path_enum_type rfcommon_core_lte_device_to_path( rfcom_device_enum_type device )
{
  rf_path_enum_type temp_path;
  /* Hard coded for bring-up. Needs to be fixed.*/
...
/*!
  @brief
  Get GNSS blanking enable status.
 
  @details
  GNSS blanking enable is determinated based on RFNV_GNSS_BLANKING_CONTROL_I.
  When in blanking state is disable, GNSS_BLANK signal is to be skipped from programming
 
  @param tech
  The RF technology calling this API
 
  @param band
  The RF band for the calling technology
 
  @return
  TRUE  - Blanking enabled, GNSS_BLANK signal is to be programmed
  FALSE - Blanking disabled, GNSS_BLANK signal is NOT to be programmed
*/
boolean
rfcommon_get_gnss_blanking_is_enabled
(
...
)
{
  ...
  rfnv_rf_tech_band_type* rf_tech_band;
  uint32 i = 0;

  if (rfcommon_nv_tbl == NULL)
  {
    RF_MSG( RF_ERROR, "RF GNSS blanking - NULL NV Ptr. GNSS blanking enabled." );
    return TRUE;
  }

  /* If NV is not enabled, blanking enabled by default */
  if( rfcommon_nv_tbl->gnss_blanking_control.nv_enable != 1 )
  {
    return TRUE;
  }

  /* Determine if this tech band enables gnss blanking */
  for (i = 0; i < RFNV_GNSS_BLANKING_CONTROL_NUM_RF_TECH_BANDS; i++)
  {
    rf_tech_band = &rfcommon_nv_tbl->gnss_blanking_control.rf_tech_bands[i];

    if( (rf_tech_band->rf_tech == (uint8)-1) && (rf_tech_band->rf_band == (uint16)-1) )
    {
      /* This means tech/band were not found: blanking enabled */
      return TRUE;
    }

    if( (rf_tech_band->rf_tech == tech) && (rf_tech_band->rf_band == band) )
    {
      RF_MSG_2( RF_HIGH, "RF GNSS blanking disabled for RF tech %d band %d", tech, band );
      return FALSE;
    }
  }

  /* If it reached here, NV is enabled and the band was not found in NV: blanking enabled */
  return TRUE;
} /* rfcommon_get_gnss_blanking_is_enabled */
...
/*!
  @brief
  Create a conf file in EFS to help save the adc.dat file as part of the saved qcn.
 
  @details
  Checks if the conf file is already present or not, if not then it willcraete the file
  else return. 
  ...
  @return
   void
*/
void rfcommon_create_adc_vcm_conf_file ()
{


#define EFS_ADC_VCM_CONF_PATH         "nv/item_files/conf/adc_vcm.conf" 
 ...
 //location of the adc.dat and log files which needs to be backedup 
 char arr[]  = "/selfcal/modem/adc.dat\n";
 char arr1[] = "/selfcal/modem/adclog.dat\n";

 /* check to see whether the file is available or not */
 if( efs_stat(EFS_ADC_VCM_CONF_PATH,&temp_buf) == 0 )
 {
   RF_MSG( RF_HIGH, "ADC VCM Conf file already exists");
   return ;
 }
 ...
 fd = efs_open (EFS_ADC_VCM_CONF_PATH, oflag, mode);
 if (fd > 0)
 {
   ret_val = efs_write (fd, arr, strlen(arr));
   if (ret_val == -1) 
   {
   ...
   ret_val = efs_write (fd, arr1, strlen(arr1));
   if (ret_val == -1) 
   {
      MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, "Error in appending data = %d", ret_val);
      efs_close (fd);
