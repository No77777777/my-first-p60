...
$Header: //components/rel/rfa.mpss/4.11/rf/common/rf/rfc/inc/rfc_class.h#1 $

when       who     what, where, why
--------   ---     -------------------------------------------------------------
08/16/17   Om      Add RFFE V2 support
11/06/15   hbl     [Ported]Change IRAT alt path lookup implementation
10/11/15   mah     iRAT restriction band mask support
07/27/15   Hj      Remove RFC_MAX_DEVICE_INSTANCES_PER_BAND definition 
                   Definition move to rfcom.h
...
05/26/15   fhuo    Added "void create_wlan_rfc(void)" for WLAN support
05/26/15   Saul    RFC Dynamic Properties API
04/30/15   Saul    CRAT Updates
03/17/15   al      Increase RFC_MAX_DEVICE_INSTANCES_PER_BAND
03/09/15   al      Back out previous change
03/03/15   al      Increase RFC_MAX_DEVICE_INSTANCES_PER_BAND
...
/* Structure used to pass params for dynamic properties generation */
typedef struct
{
  rfm_device_enum_type device;
  uint8 dev_instance;
  rfm_device_capability_type trx_capability;
  sys_band_class_e_type sys_band;
  uint8 antenna;
  boolean supports_multi_rat;
  uint32 max_bw_khz;
  boolean restrict_irat_on_non_preferred_dev;
} rfc_dynamic_properties_params_type;
...
public:
  ...
  boolean set_fbrx_phy_path_adc_mapping(       
  ...
  rfm_device_capability_type
    rfc_get_device_capability
  (
       rfm_device_enum_type device
  );

  boolean
    generate_rfcard_dynamic_properties
    (
      rfc_dynamic_properties_params_type *rfc_dynamic_params
    );

  boolean
    get_rfcard_dynamic_properties
    (
...
protected:
  ...
  rfm_rfcard_dynamic_properties_type rfcard_dynamic_properties;
...
private:
  ...
  // Contains status for dynamic properties
  boolean rfc_dynamic_properties_valid;
