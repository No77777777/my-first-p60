...
/*===========================================================================
Copyright (c) 1999 - 2022 by Qualcomm Technologies, Incorporated.  All Rights Reserved.
...
$Header: //components/rel/rfa.mpss/4.11/rf/common/rf/mc/src/rfm.c#5 $

when       who     what, where, why
-------    ---    -------------------------------------------------------
11/09/22   hd      [CR3328769] Skip rfm_init based on SOC ID for Gaming
09/02/22   tej     FR69305 To add support for new error codes for QMI_DMS_SET_OPERATING_MODE
03/14/22   cv      [CR3150347] Skip rfm_init based on SOC ID for APQ
09/16/21   ag      [CR3038008] - KW Error Fixes
03/12/20   ssl     F3 reduction: Moving F3s under TXPL Debug flag
01/27/20   Hari   [CR 2610528] LTE RF changes for Minidump 
01/24/20   moh     RF MDUMP Changes
01/20/20   ssl     FR 55657: Replace RF Enter/Exit with RF Sleep/Wakeup for L+L DSDS awake
12/17/19   ssl     Adding of MCPM_Config_Modem RF_START in CAL
12/16/19   ssl     Adding of MCPM_Config_Modem RF_START and Stop Req in rfm_init
06/07/18   rp      HPUE non_signal support for LTE B41
10/30/17   pv      Unified band mask support to Policyman
10/26/17   sk      Enable LTE B71 support
01/06/16   sbo     Create conf file to help save the adc.dat file as part f backed up qcn.
05/11/16   rs      Removed excessive F3 msgs
05/03/16   hm      Added a new API 
                   rfm_get_sys_lte_band_mask_from_rflte_band_mask()
04/05/15   ndb     Enable the tech level API for the FR 32791
04/05/16   ak      Fix the logic in returning the forced antenna position to WL1
03/08/16   kg      Adding support for enabling/disabling band capability from bc config
01/20/16   dyc     Added new data types for SAR forced antenna support
03/01/16   mah      Changes to populate band 66 bit mask.
02/29/16   hm      Add CDMA ASDIV abort API
12/07/15   sg      Remove code featurized under RFLTE_BOLT_UL_CA_SUPPORTED
10/20/15  vkm      Remove the support of getting PA state for higher layers for LTE
10/20/14   hk      Intimating Gl1 the number of WTRs present on RF card so that 
                   sleep/wakeup time can be adjusted for SS/DSDS and DSDA cases
08/21/14   sml      Add GSM case in rfm_set_antenna_imm()
01/28/15   vc      Propogating immediate flag for FTM AsDiv support
01/28/15   vc      Logging the rfm_set_antenna call also to the rfm cmd buffer 
10/20/14   hk      Intimating Gl1 the number of WTRs present on RF card so that 
                   sleep/wakeup time can be adjusted for SS/DSDS and DSDA cases
08/21/14   sml      Add GSM case in rfm_set_antenna_imm()
09/21/15   sk      Make AFC TxLM Handle based
08/13/15   Saul    Provide zeroes and FAIL status for RFC properties to TRM 
                   when rfm_init fails
07/27/15   piy     Updating the rfm_get_aggregated_band_mask() API to get the 
                   RFM_DEVICE based BC_CONFIG NV values
...
06/23/15   rp      Adding API that can be called from FW to dump WTR GRFC
                   RXLM and TXLM regs
11/06/15   piy     Updating GSM Band support being passed to TRM to be RFM_DEVICE based 
...
05/08/15   ak      [Rx/TX Split]Remove Device hardcoding
04/16/15   pkg     Changes to support Tx Rx Split Band design part 2.
04/20/15    sk     Inter-band UL CA support 
04/16/15   pkg     Changes to support Tx Rx Split Band design part 2.
04/08/15   bsh     FED MC layer cleanup
04/08/15   tks     Added support for LTE-U bands
03/13/15   ska     Add support for DGLNA feature
01/26/15   ra      Keep Modem enabled for TA RUMI
...
rfm_bands_bitmask policyman_rf_band_mask;  /*Debug purpose*/

extern boolean rfcmn_core_maat_mgr_debug_flag;

rf_bandset_info_type rf_bandset_info = 
{
  RF_BANDSET_INIT_PENDING,
  NULL
};
...
extern uint8 rflte_dm_get_current_pa_state(uint8 sub_id);
...
/* Customer specific change - HPUE non_signal support  */
boolean hpue_non_signal_support = FALSE;
...
/*!
  @brief
  Global variable to store rfm init status.

  @details
  Add the rfm_init_status_type_e_type to the rfm_init to indicate the SW failure or HW 
  failure if there is any failure been detected during rf initialization.

*/
static rfm_init_status_type rfm_init_result;
...
/*!
  @brief
  Maintains the state on the forced antenna condition from SAR.
*/
static rfm_sar_forced_antenna_enum_type rfm_forced_sar_antenna_state = RFM_SAR_ANTENNA_NONE;
...
extern rfm_bands_bitmask rfc_bc_cfg_bands[RFM_MAX_WAN_DEVICES];
...
)
{
  ...
  rfm_init_status_type_e_type rfcommon_mc_init_status = RFM_INIT_FAILURE_NONE; 
  mcpm_request_parms_type rf_dummy = {0};

  RF_MSG( RF_HIGH, "rfm_init: START." );

  if((ChipInfo_GetChipId() == CHIPINFO_ID_DIVAR_GAMING)||(ChipInfo_GetChipId() == CHIPINFO_ID_STP_DIVAR_L)||(ChipInfo_GetChipId() == CHIPINFO_ID_STP_DIVAR))
  {
    return FALSE;
  }
      /* Init the rfm_init_result */
  rfm_init_result.rfm_init_status = FALSE;
  rfm_init_result.rfm_init_failure = RFM_INIT_FAILURE_NONE;
  ...
  RF_MSG( RF_HIGH, "rfm_init:MCPM_RF_START_REQ");
  ...
  #ifdef RF_POWER_REPORTING_FEATURE
  #if 1
  if (MCPM_is_qdss_en()) 
  {
    rflm_diag_power_enable_qdss_log();
  }
  #endif
  #endif
  ...
  if (rfm_initialized==RFM_INIT_NOT_RUN)
  {
    ...
    rfcommon_mc_init_status =rfcommon_mc_init(RFCOM_TRANSCEIVER_0,
                                           caller_tcb_ptr, task_nv_wait_sig,
                                           task_wait_func_ptr);

    /* Fail common init if there is any failre from API rfcommon_mc_init() */
    if(rfcommon_mc_init_status != RFM_INIT_FAILURE_NONE)
    {
      common_init_success &= FALSE;
    }
  ...
  if(rfm_is_tech_supported(RFCOM_TRANSCEIVER_0,RFCOM_WCDMA_MODE))
  {
    ...
    if (rf_bandset_init() != RF_BANDSET_INIT_SUCCESS)
    {
      common_init_success = FALSE;
    }
  ...
  if ( (common_init_success == TRUE && technology_init_success == TRUE) == FALSE )
  {
    MSG_3( MSG_SSID_RF, MSG_LEGACY_ERROR,
           "rfm_init: Failed RF initialization.  Common: %d, Tech: %d, RFM_INIT_STATE: %d",
...
#ifdef FEATURE_QSH_MDUMP
  // Register RFA common QSH client
  ...
  #if ((defined T_RUMI_EMULATION) && (!defined FEATURE_TABASCO_MODEM))
  MCPM_Config_Modem(MCPM_RF_STOP_REQ, &rf_dummy);
  #endif

  RF_MSG( RF_HIGH, "rfm_init:MCPM_RF_STOP_REQ:");

  MCPM_Config_Modem(MCPM_RF_STOP_REQ, &rf_dummy);

  //Create a conf fie if not already exists
  //This will help backup the adc.dat file as prt of backup qcn.
  rfcommon_create_adc_vcm_conf_file();

  RF_MSG_1( RF_HIGH, "rfm_init: END. status %d", rfm_initialized );

   if(RFM_INIT_SUCCEEDED == rfm_initialized)
  {
    rfm_init_result.rfm_init_status = TRUE;
    rfm_init_result.rfm_init_failure = RFM_INIT_FAILURE_NONE;
  }
  else
  {
    rfm_init_result.rfm_init_status = FALSE;
    // TO ADD: rfm_init_failure type
    rfm_init_result.rfm_init_failure = rfcommon_mc_init_status;
  }

  return &rfm_init_result;
}
                        ...
                        )
{
  ...
  RF_MSG_2( RF_HIGH, "rfm_enter_mode: START. device %d mode %d", device, rf_mode );

  #ifndef FEATURE_WCDMA_RUMI_SUPPORT
  /* Bypass init success check for RUMI */
  if ( rfm_is_rfm_init_success() == FALSE )
  {
  ...
  {
    RF_MSG_3( RF_ERROR, 
              "rfm_enter_mode: Error invalid device %d. Cannot enter mode %d. Failure code %d returned.", 
              device, rf_mode, wait_delay_ms);
    return wait_delay_ms;
  }
  ...
  if (rfm_mode[device] != rf_mode)
  {
    ...
    */
    ...
    {
      RF_MSG_3( RF_ERROR, 
                "rfm_enter_mode: device=%d NOT in parked mode, but in mode=%d. Cannot enter CDMA mode %d", 
                device,  rfm_mode[device], rf_mode);
  ...
  default:
    MSG_FATAL("rfm_enter_mode: Invalid Mode for Device: (device= %d, mode = %d)", device, rf_mode, NULL);
    break;
  }
  ...
  if(result == TRUE)
  {
    ...
    RF_MSG_2( RF_HIGH, "rfm_enter_mode: Updated device %d mode %d", device, rfm_mode[device] );
  }
  else
  {
    RF_MSG_1( RF_ERROR, "rfm_enter_mode: Failed for mode %d", 
              rf_mode);
  }

  RF_MSG_3( RF_HIGH, "rfm_enter_mode: END. device %d mode %d err code %d", device, rf_mode, wait_delay_ms );
                     ...
                     rfcom_mode_enum_type to_rfmode)
{
  ...
  {
    RF_MSG_1(RF_ERROR, "rfm_exit_mode: Error invalid device %d. Skipping exit mode.", device);
    return 0;
  }

  RF_MSG_3( RF_HIGH, "rfm_exit_mode: START. device %d curr mode %d new mode %d", device, rfm_mode[device], to_rfmode );
  ...
  default:
    MSG_FATAL( "rfm_exit_mode: Invalid Mode for Device: (device= %d, mode = %d)", device, 
  ...
  RF_MSG_3( RF_HIGH, "rfm_exit_mode: END. device %d mode %d err code %d", device, to_rfmode, ret_val );
                       ...
                       )
{
  ...
  case RFCOM_WCDMA_MODE:
    ...
    #ifdef FEATURE_WCDMA
    MSG(MSG_SSID_RF, MSG_LEGACY_ERROR, "Failed rfwcdma_mc_enable_tx - Call rfm_wcdma_tx_enable API");
    #endif
    break;
  case RFCOM_GSM_MODE:
    #ifdef FEATURE_GSM
                 ...
                 )
{
  ...
  case RFCOM_WCDMA_MODE:
    #ifdef FEATURE_WCDMA
     MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, "Failed rfwcdma_mc_sleep - should not be called via rfm %d", rfm_mode[device]);
    #endif
    break;
                  ...
                  )
{
  ...
  case RFCOM_WCDMA_MODE:
    #ifdef FEATURE_WCDMA
    MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, "Failed rfwcdma_mc_wakeup - should not be called via rfm %d", rfm_mode[device]);
    #endif
    break;
...
/*
  @brief
  Returns the aggregated bitmask of the WWAN bands from all paths BC Config NV.
  For a given band, if any RF path supports it, the return value turn it on.

  @details
  This interface is only valid for 1x, HDR, WCDMA, and GSM WWAN technologies.
  This interface is currently only called by 1x

  @return
  A 64 bitmask with the bands read from BC config NV for all paths
...
{
  uint64 aggregated_bc_config = 0;

  rfm_device_enum_type rfm_dev;

  RFM_DEBUG_EN_Q(&rfm_get_aggregated_band_mask, 0, 0, 0, 0);

  for ( rfm_dev = RFM_DEVICE_0; rfm_dev < RFM_DEVICE_5; rfm_dev ++)
  {
    if(rfc_common_get_device_capability(rfm_dev) == RFM_DEVICE_RX_SUPPORTED)
    {         
      aggregated_bc_config |= rfcommon_nv_get_rx_device_bc_config(rfm_dev);
    }
  }
  return aggregated_bc_config;
...
{
  int dword_offset = band / 64;
  int mask_offset = band - (dword_offset * 64);
  ASSERT( ARR_SIZE(mask->mask) > dword_offset );
  ASSERT( 8*sizeof(mask->mask[0]) > mask_offset );

  if (mask->mask[dword_offset] & (1ULL << mask_offset))
  {
    return TRUE;
  }
  else
...
)
{
  ...
  mask->mask[dword_offset] |= (1ULL << mask_offset);
} /* rfm_bands_bitmask_set_bit() */
...
/*!
  @brief
  Api to init rf bandset with all supported bands by RF.
 
  @return rf_bandset_init_status
*/
rf_bandset_init_status
rf_bandset_init
(
...
)
{
  ...
  RFM_DEBUG_EN_Q(&rf_bandset_init, rf_bandset_info.init_status, 0, 0, 0);
  RF_MSG_1( RF_HIGH, "rf_bandset_init - Start. Status %d.", rf_bandset_info.init_status );

  if (rf_bandset_info.rf_bandset == NULL)
  {
    rf_bandset_info.rf_bandset = policyman_rf_bandset_new();
  }

  if (rf_bandset_info.rf_bandset == NULL)
  {
    RF_MSG( RF_ERROR, "rf_bandset_init - Cannot create new rf bandset. No bands provided.");
    rf_bandset_info.init_status = RF_BANDSET_INIT_FAILED;
  }

  if (rf_bandset_info.init_status != RF_BANDSET_INIT_FAILED)
  {
    /* Get RF's internal band mask */
    mask = rfommon_mc_get_aggregated_rfm_bands_bitmask();
    policyman_rf_band_mask=mask;

    /* Populate rf_bandset */
    for (band_idx = 0; band_idx < SYS_BAND_CLASS_MAX; band_idx++)
    {
      if( rfm_bands_bitmask_bit_is_set(&mask,band_idx) )
      {
        if( !policyman_rf_bandset_add_band_to_set( rf_bandset_info.rf_bandset, band_idx ) )
        {
          rf_bandset_info.init_status = RF_BANDSET_INIT_FAILED;
          RF_MSG_1( RF_ERROR, "rf_bandset_init - FAILURE adding SYS BAND %d to rf bandset.", band_idx );
        }
      }
    } /* for (band_idx) */
  } /* if (rf_bandset_info.init_status != RF_BANDSET_INIT_FAILED) */
  ...
  if (rf_bandset_info.init_status != RF_BANDSET_INIT_FAILED)
  {
    rf_bandset_info.init_status = RF_BANDSET_INIT_SUCCESS;
  }

  RFM_DEBUG_EN_Q(&rf_bandset_init, rf_bandset_info.init_status, 0, 0, 0);
  RF_MSG_1( RF_HIGH, "rf_bandset_init - End. Status %d.", rf_bandset_info.init_status );

  return rf_bandset_info.init_status;
} /* rf_bandset_init */

/*----------------------------------------------------------------------------*/
/*!
  @brief
  Api to communiate all supported bands for all RF WWAN techs to other modules.

  @detail
  API can return null if rfm_init failed or bandset is invalid.
 
  @return bandset_ptr
  This item contains all supported bands for all RF WWAN techs.
*/
...
)
{
  ...
  RFM_DEBUG_EN_Q(&rfm_get_supported_rf_bandset, rf_bandset_info.init_status, 0, 0, 0);
  RF_MSG_1( RF_HIGH, "rfm_get_supported_rf_bandset - Start. Status %d.", rf_bandset_info.init_status );

  if(rf_task_rfm_init_state != RF_TASK_RFM_INIT_STATE_COMPLETE)
  {
    /* WARNING: DO NOT REMOVE THIS. Otherwise, callers may get invalid/incomplete data.
       Instead, if a caller needs data this early in their init, 
       they must use: mcs_rcevt_wait_name("RFM_INIT_COMPLETE"); before calling this API */
    ERR_FATAL( "rfm_get_supported_rf_bandset: Called before rf task completed.", 0, 0, 0 );
  }

  /* Intentionally do not provide any info to upper layers to force fixing RF issues */

  /* If bandset init was not ok */
  if (rf_bandset_info.init_status == FALSE)
  {
    RF_MSG( RF_ERROR, "rfm_get_supported_rf_bandset - rf bandset init is not success. No bands provided.");
    return NULL;
  }

  /* Only if rfm_init was OK */
  if ( rfm_init_get_state() != RFM_INIT_SUCCEEDED )
  {
    RF_MSG( RF_ERROR, "rfm_get_supported_rf_bandset - RFM INIT status is not success. No bands provided.");
    return NULL;
  } /* if ( rfm_init_get_state() != RFM_INIT_SUCCEEDED ) */

  RFM_DEBUG_EN_Q(&rfm_get_supported_rf_bandset, rf_bandset_info.init_status, 0, 0, 0);
  RF_MSG_1( RF_HIGH, "rfm_get_supported_rf_bandset - End. Status %d.", rf_bandset_info.init_status );

  return rf_bandset_info.rf_bandset;
} /* rfm_get_supported_rf_bandset */
...
)
{
  ...
  RF_MSG( RF_HIGH, "MCPM_RF_START_REQ:" );
...
)
{
  ...
  rfm_device_enum_type rfm_device = RFM_DEVICE_0;
  uint8 mask_cntr = 0;
  ...
  /* Check if rfm init has passed */
  if ( rfm_is_rfm_init_success() == FALSE )
  {
    RF_MSG( RF_ERROR, "rfm_get_devices_configuration: rfm_init FAILED - Cannot provide configuration info.");
    (void) memset( dev_cfg, 0x0, sizeof( rfm_devices_configuration_type ) );
    return FALSE;
  }

  for(rfm_device = RFM_DEVICE_0; rfm_device < RFM_MAX_WAN_DEVICES; rfm_device++)
          {
    if (dev_cfg->device_support[rfm_device].capability == RFM_DEVICE_RX_SUPPORTED)
          {
	  for(mask_cntr = 0; mask_cntr < 3; mask_cntr++)
        {
        /* AND the band support from the RFC with the ones from the BC config NV */
        dev_cfg->device_support[rfm_device].bands_supported.mask[mask_cntr] &= rfc_bc_cfg_bands[rfm_device].mask[mask_cntr]; 
        dev_cfg->device_support[rfm_device].preferred_bands_supported.mask[mask_cntr] &= dev_cfg->device_support[rfm_device].bands_supported.mask[mask_cntr];
        }
      }
    }
...
/*!
  @brief
  Returns rfm_init state of type rfm_init_state_enum_type
  ...
  @return
  rfm_initialized value
*/
/*----------------------------------------------------------------------------*/
#ifdef FEATURE_QSH_MDUMP
QSH_MDUMP_FN_ATTR
rfm_init_state_enum_type rfm_init_get_state_mdump
(
  void
)
{
...
rfm_init_state_enum_type rfm_init_get_state
(
  void
)
{
  switch (rfm_initialized)
  {
  case RFM_INIT_SUCCEEDED:
    break;
  case RFM_INIT_NOT_RUN:
    RF_MSG( RF_ERROR, "rfm_init_get_state: rfm_init status: NOT RUN. NO RF FUNCTIONALITY AVAILABLE.");
    break;
  case RFM_INIT_FAILED:
    RF_MSG( RF_ERROR, "rfm_init_get_state: rfm_init status: FAILED. NO RF FUNCTIONALITY AVAILABLE.");
    break;
  default:
    // DO NOT REMOVE this ERR_FATAL, if a new enum was added, add a case statement above to handle it.
    ERR_FATAL( "rfm_init_get_state: rfm_init state %d not supported. NO RF FUNCTIONALITY AVAILABLE.", rfm_initialized, 0, 0 );
    break;
  }
  return rfm_initialized;
} /* rfm_init_get_status */
...
)
{
  ...
  case RFM_INIT_SUCCEEDED:
    break;
  case RFM_INIT_NOT_RUN:
    RF_MSG( RF_ERROR, "rfm_is_rfm_init_success: rfm_init status: NOT RUN. NO RF FUNCTIONALITY AVAILABLE.");
    break;
  case RFM_INIT_FAILED:
    RF_MSG( RF_ERROR, "rfm_is_rfm_init_success: rfm_init status: FAILED. NO RF FUNCTIONALITY AVAILABLE.");
    break;
  default:
    // DO NOT REMOVE this ERR_FATAL, if a new enum was added, add a case statement above to handle it.
    ERR_FATAL( "rfm_is_rfm_init_success: rfm_init status %d not supported. NO RF FUNCTIONALITY AVAILABLE.", rfm_initialized, 0, 0 );
    break;
  }
...
/*!
  ...
  @details
  This is an accessor function which will Set the current active Mode on the
  given device.
...
)
{
  ...
  /* Perform Device validity Check. Return Invalid Mode, if the device is
  ** not supported */
  if ( (device < RFM_MAX_DEVICES) && (tech <= RFM_INVALID_MODE))
  {
    rfm_mode[device] = tech;
    ret_val = TRUE;
  }
  else
  {
    RF_MSG(RF_ERROR, "rfm_set_current_mode: Invalid device or tech");
    ret_val = FALSE;
  }
  return ret_val;
...
/*!
  @brief
  RF driver query function to determine if a technology can tune a channel,
...
)
{
  ...
  case RFM_1XEVDO_MODE:
    ...
    if( FALSE == is_band_chan_supported )
    {
      RF_MSG_3( RF_ERROR, "rfm_is_band_chan_supported_v2: band %d, chan %d not "
	      "supported for mode %d", band, (rfm_cdma_chan_type)chan, rf_mode );
    }
    break;
...
)
{
  ...
  switch(mode)
  {
    /* Currently only implemented for LTE */
      ...
      rflte_mc_get_rxpll_script(sub_id,carrier_idx, rxpll_script_buf_ptr);       
    break;
    #endif /*FEATURE_LTE*/
...
)
{
  ...
  boolean result = FALSE;
  uint32 carrier_idx = 0; /*Hardcode to PCELL for legacy api*/
  ...
  switch(mode)
  {
    /* Currently only implemented for LTE */  
      ...
      result = rflte_mc_tx_update_pll_scripts ( sub_id,
	  	                                carrier_idx,
                                                rx_freq_error_in_hz,
                                                txlm_buffer_idx,
...
/*!
  ...
  @param carrier_idx
  specifes the carrier or cell for which Tx PLL script
...
)
{
  ...
  switch(mode)
  {
    ...
    MSG_ERROR("Invalid RF mode = %d for rfm_get_cell_txpll_script", mode, 0, 0);     
  ...
  if (result == FALSE)
  {
    RF_MSG_1( RF_LOW, "rfm_get_cell_txpll_script status = %d", result);
  }
  #endif /* FEATURE_LTE */
...
/*!
  ...
  @param  txlm_handle 
  TxLM Handle for which AFC correction should happen
...
)
{
  ...
  if((common_nv_tbl_ptr != NULL) && (common_nv_tbl_ptr->rf_lte_enable_txpl_debug_msg))
  {
   RF_MSG_1( RF_HIGH, "rfm_get_cell_txpll_script_v2: Received on sub_id: %d",sub_id);
  }
  ...
  switch(mode)
  {
   ...
   if((common_nv_tbl_ptr != NULL) && (common_nv_tbl_ptr->rf_lte_enable_txpl_debug_msg))
    {
      RF_MSG_3( RF_HIGH,"rfm_get_cell_txpll_script_v2 called for carrier : %d, TXLM Handle : %d, Rx_freq_error(Hz) : %d", 
                carrier_idx, txlm_handle, rx_freq_error_in_hz);
    }
    ...
    MSG_ERROR("Invalid RF mode = %d for rfm_get_cell_txpll_script_v2", mode, 0, 0);     
  ...
  if (result == FALSE)
  {
    RF_MSG_1( RF_LOW, "rfm_get_cell_txpll_script_v2 status = %d", result);
  }
  #endif /* FEATURE_LTE */
...
)
{
  ...
  uint8 sub_id =0; // No caller to this API in Rflte call flow ,just added for compilation
  ...
  case RFM_SVDO_MODE:
    #ifdef FEATURE_CDMA1X
    ret_val &= rf_cdma_mc_abort_set_antenna_to_position( device, 
                                                         ant_sw_pos,
                                                         cb_func, 
                                                         cb_data );
    #endif
  ...
  case RFM_LTE_MODE:
    #ifdef FEATURE_LTE
    rflte_mc_send_switch_ant_abort_msgr (sub_id,device, ant_sw_pos, cb_func, cb_data);
    #endif
    break;
  case RFM_TDSCDMA_MODE:
...
boolean rfm_is_tech_supported(rfcom_device_enum_type device, rfm_mode_enum_type tech)
{
  ...
  case RFM_IMT_MODE:
    #ifdef FEATURE_WCDMA
     #ifdef ATLAS_RF_WCDMA_PRESI
     is_supported = rf_wcdma_is_supported();  
     #else
...
/*!
  @brief
  API to read ASM registers
*/ 

void rfm_read_asm_regs(void)
{
  uint8 instance = 0;
  rfdevice_class *asm_dev_ptr = NULL;

  RF_MSG(RF_HIGH, "rfm_read_asm_regs: Dumping all instances of ASM supported on the RF Card");

  for (instance = 0; instance < RFC_MAX_DEVICE_INSTANCES; instance++)
  {
    asm_dev_ptr = rfc_common_get_cmn_rf_device_object(RFDEVICE_ASM, instance);

    rfdevice_asm_dump_reg(asm_dev_ptr);
  }
}

/*!-------------------------------------------------*/
/*!
  @brief Sets/Resets DGLNA parameters to beused by RF.
 
  @param dglna_info
  Structure containing DGLNA information.
 
  @return
  FALSE: Parameter checking failed. Internal RF DGLNA
  state-machines and state not updated.
...
RF_MSG_2(RF_HIGH, "rfm_dglna_set_attributes:feature_dglna_disable:%d,enable_dglna:%d ", 
        dglna_info.feature_dglna_disable,
        dglna_info.enable_dglna);

RF_MSG_4(RF_HIGH, "rfm_dglna_set_attributes:tech 1 band:%d,tech 1 primary device :%d,tech 1 secondary device :%d,tech 1 technology :%d ",
        dglna_info.tech1_info.band,
        dglna_info.tech1_info.primary_device,
        dglna_info.tech1_info.secondary_device,
        dglna_info.tech1_info.tech);

RF_MSG_4(RF_HIGH, "rfm_dglna_set_attributes:tech 2 band:%d,tech 2 primary device :%d,tech 2 secondary device :%d,tech 2 technology :%d ", 
        dglna_info.tech2_info.band,
        dglna_info.tech2_info.primary_device,
        dglna_info.tech2_info.secondary_device,
        dglna_info.tech2_info.tech);
...
)
{
  ...
  if ( rfc_cfg == NULL )
  {
    RF_MSG( RF_ERROR, "rfm_get_rfcard_dynamic_properties: NULL param provided. Cannot provide RFC dynamic properties.");
    return FALSE;
  }

  if ( rfm_is_rfm_init_success() == FALSE )
  {
    RF_MSG( RF_ERROR, "rfm_get_rfcard_dynamic_properties: rfm_init FAILED - Cannot provide dynamic properties.");
    (void) memset( rfc_cfg, 0x0, sizeof( rfm_rfcard_dynamic_properties_type ) );
    return FALSE;
  }

  /* Obtain the device configuration information from RFC */
  api_status = rfc_get_rfcard_dynamic_properties( rfc_cfg );
...
/*
  @brief
  This API will return the antenna position that is requested from SAR based config.

  @details
  This API will return the antenna position that is requested from SAR based
  configuration setting which explicitly identifies which antenna to be on.

  If a RF_SAR_FORCE_OTHER_ANTENNA entry is encountered in a SAR_table[ant][DSI] for a given
  DSI, this API will generate the appropriate enum return for L1 to take requested antenna
  selection.

  This will be independent of MTPL based calculations.

  @param
  ptr to rfm input_parm structure
...
{
  rfcom_mode_enum_type mode = input_parm->rfmode;
  rfcom_device_enum_type device = input_parm->device;
  rfm_sar_forced_antenna_enum_type forced_sar_return = RFM_SAR_ANTENNA_NONE;

  RFM_DEBUG_EN_Q(&rfm_get_sar_forced_antenna, mode, device, 0, 0);
  ...
  #ifdef FEATURE_LTE
  case RFM_LTE_MODE:
    switch (rflte_mc_get_sar_forced_ant())
    {
       case RFLTE_MC_SAR_ANTENNA_0:
          forced_sar_return = RFM_SAR_ANTENNA_CONFIG_0;
          break;
       case RFLTE_MC_SAR_ANTENNA_1:
          forced_sar_return = RFM_SAR_ANTENNA_CONFIG_1;
          break;
       default:
          break;
    }
  ...
  #ifdef FEATURE_WCDMA
  case RFM_IMT_MODE:
    switch (rfwcdma_mc_get_sar_forced_ant())
    {
       case RFWCDMA_MC_SAR_ANTENNA_0:
          forced_sar_return = RFM_SAR_ANTENNA_CONFIG_0;
          break;
       case RFWCDMA_MC_SAR_ANTENNA_1:
          forced_sar_return = RFM_SAR_ANTENNA_CONFIG_1;
          break;
       default:
	   forced_sar_return = RFM_SAR_ANTENNA_NONE;	
          break;
    }
    break;
  #endif /* FEATURE_WCDMA */
  ...
  case RFM_1X_MODE:
  case RFM_1XEVDO_MODE:
    switch (rf_cdma_mc_get_sar_forced_ant(device, mode))
    {
       case RF_CDMA_MC_SAR_ANTENNA_CONFIG_0:
          forced_sar_return = RFM_SAR_ANTENNA_CONFIG_0;
          break;
       case RF_CDMA_MC_SAR_ANTENNA_CONFIG_1:
          forced_sar_return = RFM_SAR_ANTENNA_CONFIG_1;
          break;
       default:
          break;
  ...
  case RFM_EGSM_MODE_2:
  case RFM_EGSM_MODE_3:
    switch(rfgsm_mc_get_sar_forced_ant())
    {
       case RFGSM_MC_SAR_ANTENNA_0:
          forced_sar_return = RFM_SAR_ANTENNA_CONFIG_0;
     break;
       case RFGSM_MC_SAR_ANTENNA_1:
          forced_sar_return = RFM_SAR_ANTENNA_CONFIG_1;
          break;
       default:
          break;
  ...
  default:
    RF_MSG_1( RF_MED, "rfm_get_sar_forced_antenna: Mode not supported %d", mode);
    break;
  }
#endif /* FEATURE_RF_ASDIV */

  /* provide message only if forced antenna condition has changed state */
  if ( forced_sar_return != rfm_forced_sar_antenna_state )
  {
    RF_MSG_3( RF_MED, "SAR forced antenna condition state=%d, mode=%d, dev=%d", \
              forced_sar_return, mode, device);

    /* update state */
    rfm_forced_sar_antenna_state = forced_sar_return;
  }

  return (forced_sar_return);
}
...
{
  uint32 grfc_reg_val = 0;
  RF_MSG_1(RF_HIGH,"rfm_read_grfc_regs: CCS GRFC SET REGS 0x%x", HWIO_IN(CCS_RF_GRFC_SET));
}


void rfm_dump_regs(uint8 dump_mode_mask)
{
  switch(dump_mode_mask)
  {
    case RFLM_LOG_NO_REGS:
        break;
        
    case RFLM_LOG_CRIT_WTR_ONLY:
        rfm_read_wtr_regs(0);
        break;
        
    case RFLM_LOG_ALL_WTR_ONLY:
        rfm_read_wtr_regs(1);
        break;
        
    case RFLM_LOG_ALL_GRFC_ONLY:
        rfm_read_grfc_regs();
        break;
        
    case RFLM_LOG_CRIT_WTR_ALL_GRFC:
        rfm_read_grfc_regs();
        rfm_read_wtr_regs(0);
        break;
        
    case RFLM_LOG_ALL_WTR_ALL_GRFC:
        rfm_read_grfc_regs();
        rfm_read_wtr_regs(1);
        break;  
    default:
...
)
{
#ifdef FEATURE_RFA_ACL
  boolean status = TRUE;
  rfm_rf_cmn_maat_update_ind_s maat_update_msg;
  ...
  {
    /*Send the MSGR*/
    rfcmn_core_maat_update_send_msgr( maat_params->rfmode,
                                      RFCMN_CORE_MAAT_UPDATE_RX,
                                      maat_params->tech_payload);
    if(rfcmn_core_maat_mgr_debug_flag)
    {
      RF_MSG_2( RF_HIGH,"MAAT_DEBUG: rfm_update_maat_info() is called for rfm_mode:%d, status:%d",
                                     maat_params->rfmode,status);
    }
  }
  return status;
  #else
...
extern void* get_rflm_diag_trace_buffer(uint32*);
extern void* get_rflm_dm(uint32*);

extern void* get_rfcommon_fbrx_nv_tech_cal_data(uint32*);
#if defined(FEATURE_RF_ASDIV) || defined(FEATURE_RF_HAS_TPTUNER_SWITCH)
extern void* get_asdiv_manager_state(uint32*);
#endif
extern void* get_rfcommon_nv_tbl(uint32*);
extern void* get_rfcommon_rfnv_info_table(uint32*);
extern void* get_rfmdsp_meas_data(uint32*);
extern void* get_tqp_usage(uint32*);
extern void* get_rflm_cmn_rxagc_master_data(uint32*);
extern void* get_rfcommon_data(uint32*);

/*----------------------------------------------------------------------------*/
/*
  @brief
  Gets the flag that indicates if the modem has entered LPM mode.

  @details
  Tuner manager will call this function to retrieve the status of the flag.
...
{
  uint32 size = 0;
  qsh_mdump_collect_high((void *) &common_init_success, sizeof(common_init_success));
  qsh_mdump_collect_high((void *) &technology_init_success, sizeof(technology_init_success) );
 
  qsh_mdump_collect_high((void *) &rfm_mode, sizeof(rfm_mode) );
  qsh_mdump_collect_high((void *) &rfm_calibration_mode_en, sizeof(rfm_calibration_mode_en) );
  qsh_mdump_collect_high((void *) &rfm_initialized, sizeof(rfm_initialized) );
  
  void* rflm_diag_trace_buf_p = get_rflm_diag_trace_buffer(&size);
  qsh_mdump_collect_high((void *) rflm_diag_trace_buf_p, size);
  
  void* rflm_dm_p = get_rflm_dm(&size);
  qsh_mdump_collect_high((void *) rflm_dm_p, size);
  
  void* rfcommon_fbrx_nv_tech_cal_data_p = get_rfcommon_fbrx_nv_tech_cal_data(&size);
  qsh_mdump_collect_high((void *) rfcommon_fbrx_nv_tech_cal_data_p, size);

#if defined(FEATURE_RF_ASDIV) || defined(FEATURE_RF_HAS_TPTUNER_SWITCH)  
  void* asdiv_manager_state_p = get_asdiv_manager_state(&size);
  qsh_mdump_collect_high((void *) asdiv_manager_state_p, size);
#endif 

  void* rfcommon_nv_tbl_p = get_rfcommon_nv_tbl(&size);
  qsh_mdump_collect_high((void *) rfcommon_nv_tbl_p, size);
  
  void* rfcommon_rfnv_info_table_p = get_rfcommon_rfnv_info_table(&size);
  qsh_mdump_collect_high((void *) rfcommon_rfnv_info_table_p, size);
  
  //void* rfmdsp_meas_data_p = get_rfmdsp_meas_data(&size);
  //qsh_mdump_collect_high((void *) rfmdsp_meas_data_p, size);
  
  void* tqp_usage_p = get_tqp_usage(&size);
  qsh_mdump_collect_high((void *) tqp_usage_p, size);
  
  void* rflm_cmn_rxagc_master_data_p = get_rflm_cmn_rxagc_master_data(&size);
  qsh_mdump_collect_high((void *) rflm_cmn_rxagc_master_data_p, size);
  
  void* rfcommon_data_p = get_rfcommon_data(&size);
  qsh_mdump_collect_high((void *) rfcommon_data_p, size);
