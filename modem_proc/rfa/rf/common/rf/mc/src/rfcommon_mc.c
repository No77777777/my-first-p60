...
$Header: //components/rel/rfa.mpss/4.11/rf/common/rf/mc/src/rfcommon_mc.c#2 $

when       who     what, where, why
--------   ---     -------------------------------------------------------------
09/02/22   tej     FR69305 To add support for new error codes for QMI_DMS_SET_OPERATING_MODE
01/06/20   rp      snapshot for WTR
03/14/18   rp      KW Fix.
...
03/08/16   ag      Adding support for enabling/disabling band capability from bc config
03/10/16   ag      Adding support for enabling/disabling band capability from bc config
08/10/15   yzw     Added lock for accessing the shared xpt cal data structure between tasks
08/04/15   ck      Support GSM fbrx power meas
05/29/15   ndb     Update to new interface
04/20/15   aro     OFT fix
02/20/15   px      Moved apps task start signal to be independent of NV success
...
extern rfm_init_status_type_e_type rfcommon_core_init(rf_path_enum_type path);
extern uint64 rfcommon_nv_get_rf_bc_config(rf_path_enum_type path);
extern uint64 rfcommon_nv_get_rf_lte_bc_config(void);


rfm_bands_bitmask rfc_bc_cfg_bands[RFM_MAX_WAN_DEVICES] = {0};
                 ...
                 void (*task_wait_func_ptr)( rex_sigs_type ))
{
  ...
  rfm_init_status_type_e_type rfc_common_mc_init_status = RFM_INIT_FAILURE_NONE;

  RF_MSG_1( RF_HIGH, "rfcommon_mc_init: Start. Dev %d", device );
  ...
  if ( nv_init_success != TRUE )
  {
    ...
    /*read the asdiv offset char data from EFS file*/
...
#endif
    {
      ...
      rfc_common_mc_init_status = rfcommon_core_init(path);

      /* Signal RF_FWRSP_TASK and RF_APPS_TASK to start */
      if ( rfc_common_mc_init_status == RFM_INIT_FAILURE_NONE )
      {
        rfcommon_core_init_success = TRUE; 
 ...
 if((!rfcommon_core_init_success) &&  (rfc_common_mc_init_status != RFM_INIT_FAILURE_HW))
  {
    rfc_common_mc_init_status = RFM_INIT_FAILURE_SW;
  }
  ...
  RF_MSG_2( RF_HIGH, "rfcommon_mc_init: End. Dev %d success %d", device, rfc_common_mc_init_status );

  /* return final result of API */
  return rfc_common_mc_init_status;
...
{
  return rfcommon_mdsp_fetch_selftest_meas(buffer_index, meas_result );
}
...
)
{
  ...
  for( tuner_chain_idx = 0; tuner_chain_idx < RF_TUNER_CHAIN_NUM; tuner_chain_idx++  )
  {
    ...
    {
      api_status &= rfcommon_atuner_acknowledge_script( tuner_cb_data->tuner_data[tuner_chain_idx].ant_tuner,
...
/*!
  @brief
 Function to be called from rfm_init()
...
void rfcommon_mc_config_trm_band_support()
{
  ...
  memset(&rfc_bc_cfg_bands, 0, RFM_MAX_WAN_DEVICES*sizeof(rfm_bands_bitmask));

  for(rfm_device = RFM_DEVICE_0; rfm_device < RFM_MAX_WAN_DEVICES; rfm_device++) 
  { 
    if (rfc_common_get_device_capability(rfm_device) == RFM_DEVICE_RX_SUPPORTED)
    {
      /* Loop through all the RF devs
         The !=0 check also takes of devices which might not have any bc config associated with them*/
      tmp_band_supp = rfcommon_nv_get_rx_device_bc_config(rfm_device);
      if(tmp_band_supp  != 0)  
      {   
        for(loop_cntr = 0; loop_cntr < SYS_SBAND_MAX; loop_cntr++)
        {
          ...
          {       
            rfc_bc_cfg_bands[rfm_device].mask[sys_band_bc_lookup[loop_cntr].index] = 
              rfc_bc_cfg_bands[rfm_device].mask[sys_band_bc_lookup[loop_cntr].index] | 
  ...
  /* Removed the code from here as LTE Bands will now be populated in get aggregated band mask API*/	
  #if 0 
      /*LTE uses a different BC config NV. Don't touch LTE band support. Make it all 1s. Same goes for TDS*/
      rfc_bc_cfg_bands[rfm_device].mask[0] |= rfc_all_lte_mask.mask[0];
      rfc_bc_cfg_bands[rfm_device].mask[1] |= rfc_all_lte_mask.mask[1];  
      rfc_bc_cfg_bands[rfm_device].mask[2] |= rfc_all_lte_mask.mask[2]; 
  #endif	  
...
/*
  @brief
  Returns the aggregated bands bitmask for all the RF WAN techs.
  No path distinction is made. A band's corresponding bit is set when any RF path supports it.

  @return
  An array of 64-bit masks representing an enum in sys_band_class_e_type
*/
rfm_bands_bitmask rfommon_mc_get_aggregated_rfm_bands_bitmask( void )
{
  rfm_bands_bitmask rfm_band_mask;
  uint64 rf_tds_mask=0;
  rfcom_lte_band_type band_idx = RFCOM_NUM_LTE_BANDS;
  sys_band_class_e_type sys_band=SYS_BAND_CLASS_NONE;
  rfcom_band_type_u tds_band;
  rfcom_lte_band_type full_band = RFCOM_NUM_LTE_BANDS;
  rfm_device_enum_type rfm_device;
  uint8 mask_cntr = 0;

  memset(&rfm_band_mask,0,sizeof(rfm_bands_bitmask));
#ifdef FEATURE_LTE

  for (band_idx = RFCOM_BAND_LTE_B1; band_idx <  RFCOM_NUM_LTE_BANDS; band_idx++)
  {
    full_band = rflte_util_get_full_band(band_idx);

    if (rflte_util_get_sys_band_from_rfcom_band(full_band, &sys_band))
    {
      if ((rflte_mc_check_if_band_is_calibrated(full_band)))
      {
        rfm_bands_bitmask_set_bit(&rfm_band_mask, sys_band);
      }
    }
  }
#endif
...
#ifdef FEATURE_TDSCDMA
  rf_tds_mask= (rfcommon_nv_tbl.rf_tds_bc_config[0])|(rfcommon_nv_tbl.rf_tds_bc_config[1]);
 
  if(rf_tds_mask&0x0001)
  {
    tds_band.tdscdma_band=RFCOM_BAND_TDSCDMA_B34;
    sys_band = rfcommon_core_convert_rfcom_band_to_sys_band(RFM_TDSCDMA_MODE,tds_band);
    rfm_bands_bitmask_set_bit(&rfm_band_mask, sys_band);
  }
  if(rf_tds_mask&0x0020)
  {
    tds_band.tdscdma_band=RFCOM_BAND_TDSCDMA_B39;
    sys_band = rfcommon_core_convert_rfcom_band_to_sys_band(RFM_TDSCDMA_MODE,tds_band);
    rfm_bands_bitmask_set_bit(&rfm_band_mask, sys_band);
  }
  if(rf_tds_mask&0x0010)
  {
    tds_band.tdscdma_band=RFCOM_BAND_TDSCDMA_B40;
    sys_band = rfcommon_core_convert_rfcom_band_to_sys_band(RFM_TDSCDMA_MODE,tds_band);
    rfm_bands_bitmask_set_bit(&rfm_band_mask, sys_band);
  }
#endif 
  ...
  for(rfm_device = RFM_DEVICE_0; rfm_device < RFM_MAX_WAN_DEVICES; rfm_device++)
  {
        for(mask_cntr = 0; mask_cntr < RFM_BANDS_MASK_ARR_SZ; mask_cntr++)
        {
          /* AND the band support from the RFC with the ones from the BC config NV */
          rfm_band_mask.mask[mask_cntr] |=rfc_bc_cfg_bands[rfm_device].mask[mask_cntr];		  
          
          /*LTE uses a different BC config NV. Don't touch LTE band support. Make it all 1s. Same goes for TDS*/  
          rfc_bc_cfg_bands[rfm_device].mask[mask_cntr] |= rfc_all_lte_mask.mask[mask_cntr];
        }
  }
  
  return rfm_band_mask;
}
...
typedef struct debug_wtr_dump_t
{
  uint32 start;
  uint32 loop;
  uint32 obj;
...
debug_wtr_dump_type debug_wtr_dump = {0};
...
)
{
  ...
  rfdevice_rxtx_common_class* wtr_object = NULL;
  
  uint8 instance = 0;

  //time_tick = rf_time_get_tick();

  RF_MSG(RF_HIGH, "rfcommon_mc_snapshot_cmn_reg: start collection");
  debug_wtr_dump.start ++;
  
  for (instance = RFCMN_CORE_PHY_DEVICE_0; instance < RFCMN_CORE_MAX_PHY_DEVICES; instance++)
  {
    wtr_object = (rfdevice_rxtx_common_class*) rfc_common_get_cmn_rf_device_object(RFDEVICE_TRANSCEIVER, instance);
    debug_wtr_dump.loop ++;
    ...
    if ( wtr_object != NULL )
    {
      ...
      if( TRUE == rfdevice_cmn_reg_dump(wtr_object, RFDEVICE_CRITICAL_REG_DUMP))
      {	  
        //time_prof = rf_time_get_elapsed( time_tick, RF_MSEC );

        RF_MSG_1(RF_HIGH, "WTR %d done", instance);
...
boolean rflm_ccs_wakeup_flag = FALSE;
...
)
{
  ...
  if(rflm_ccs_wakeup_flag)
  {
    if ( rfcommon_mc_snapshot_rffe_devices() == TRUE )
    {
      //time_prof = rf_time_get_elapsed( time_tick, RF_MSEC );
      RF_MSG(RF_HIGH, "RFFE dump done ");
    }
    ...
    RF_MSG(RF_HIGH, "Finished saving RF registers");
    return;
	
  }
  RF_MSG(RF_HIGH, "Skipped Saving RF registers");
...
/*
  @brief
  Returns the supported bands on rf card 

  @return
  An array of 64-bit masks representing an enum in sys_band_class_e_type
*/
rfm_bands_bitmask rfommon_mc_get_supported_rfm_bands_bitmask( void )
{
  rfm_bands_bitmask temp_bitmask;
  /*initialize bit mask before returning*/
  memset(&temp_bitmask, 0, sizeof(rfm_bands_bitmask));
  
  return temp_bitmask;//rfc_common_get_supported_rfm_bands_bitmask();
}
#endif
