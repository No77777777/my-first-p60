...
/*==============================================================================
  ...
  $Header: //components/rel/rfa.mpss/4.11/rf/common/rf/mc/src/rfcommon_asdiv_manager.c#2 $

when       who     what, where, why
--------   ---     -------------------------------------------------------------
09/16/21   ag      [CR3038008] - KW Error fixes
08/28/20   ssl     [CR2756645]Added NULL check before using asdiv_manager_state data
01/24/20   moh     RF MDUMP Changes
03/23/16   hm      Added changes to avoid NULL memory reference.
03/21/16   hm      Execute asdiv RF call back outside of critical section
11/26/15   ndb     Skip the SNUM creation for Immediate ASDIV
11/02/15   as      Avoid double cleanup of ASDiv data
10/27/15   as      Implement the ASD ABORT API 
10/06/15   adk     Support for ASDiv FTM log
06/18/15   vr      Disabled tune code imm update timer in rfcommon_asdiv_manager_complete_switch()
06/02/15   dyc     Added support for general call back ASDiv switch completion
05/11/15   as/dyc  Putting mutex lock in APIs which access asdiv state variables
01/14/14   sg/dyc  GL1->RF handshake mechanism to handle ASD response error cases
                   Support Tuner abort in ASDiv Manager
...
typedef PACK(struct)
{
  ...
  /* return valute to be logged */
  uint8 ret_val;
} rfcommon_asdiv_sub_pkt_type;
...
typedef PACK(struct)
{
  ...
  /* ASDiv Tuner data */
  rfcommon_asdiv_sub_pkt_type data;

} rfcommon_asdiv_pkt_type;

/* maximum nunber of call back clients supported */
#define ASDIV_MAX_CB_CLIENTS 10

/*----------------------------------------------------------------------------*/
/*! ASDiv general purpose call back type */
typedef struct
{
   /* handle identifier assigned to call back */
   rfcommon_asdiv_cb_handler_type handle;

   /* call back function pointer */
   asdiv_cb_func_type func_ptr;

   /* call back parameter1 - sw pos variable ptr */
   rfcommon_asdiv_position_type *sw_pos_data_ptr;

   /* call back parameter2 - optional user data pointer */
   void *data_ptr;

   /* technology associated with the call back (single or all) */
   rfm_mode_enum_type rfm_mode;

   /* call back mode of operation,  single or repeated */
   rfcommon_asdiv_cb_type cb_mode;

} rfcommon_asdiv_cb_data_type;
...
typedef struct
{
  ...
  rfcommon_asdiv_cb_data_type asdiv_registered_cb[ASDIV_MAX_CB_CLIENTS];
  /*!< general purpose ASDiv call back registry for ASDiv completion */

  uint8 asdiv_cb_client_count;
  /*!< asdiv call back count */

  uint32 rxlm_handle;
  /*!< Store rxlm handle in state machine to use it for abort script*/
...
#ifdef FEATURE_QSH_MDUMP
QSH_MDUMP_FN_ATTR
void* get_asdiv_manager_state(uint32* size)
{
  *size = sizeof(rfcommon_asdiv_manager_state_type);
  return (void*)&asdiv_manager_state;
}
#endif
...
/*!
  @brief
  API to log ASDiv info

  @details
  Update the ASDiv log packet with status information.

  @param
  1. rfm device
  2. technology mode
  3. antenna position
  4. return value of API

  @return
  boolean indicating pass (TRUE) or fail (FALSE)
*/
boolean rfcommon_asdiv_log_update
(
...
)
{
  ...
  rfcommon_asdiv_pkt_type* log  = NULL;

  if( log_status( LOG_RFA_ASDIV_C))
  {
    log = (rfcommon_asdiv_pkt_type *)log_alloc( LOG_RFA_ASDIV_C,
                               sizeof( rfcommon_asdiv_pkt_type));
    /*Populate fields and Commit Log*/
    if( log != NULL )
    {
        log->header.ftm_log_id = (word) LOG_RFA_ASDIV_C;
        log->data.current_switch_position = (uint8)antenna_position;
        log->data.device = (uint8)device;
        log->data.mode = (uint8)rfm_mode;
        log->data.ret_val = (uint8)ret_val;
     ...
     {
       RF_MSG( RF_MED, "ASDIV: rfcommon_asdiv_log_update(): NULL log packet");
     }
  }
  else
  {
    RF_MSG( RF_MED, "ASDIV: rfcommon_asdiv_log_update(): LOG_RFA_ASDIV_C not enabled");
  }
...
extern void
rfcommon_asdiv_dispatch_cb
(
  rfm_mode_enum_type rfm_mode,
  rfcommon_asdiv_position_type sw_pos
);

/*----------------------------------------------------------------------------*/
extern void rfcommon_asdiv_init_cb
(
  void
);
 ...
 if(!rfcommon_mdsp_asdiv_check_aborted_event(asdiv_cb_data->update_buffer_ptr))
 {
  ...
  if ( asdiv_manager_state.l1_switch_complete_cb_func != NULL )
  {
   ...
   MSG_4(MSG_SSID_RF, MSG_LEGACY_MED,"[ASDiv Manager] Complete Switch [Done : Dev %d "
    ...
    /* dispatch any RF call backs that has registered with ASDiv */
    rfcommon_asdiv_dispatch_cb( mode, asdiv_manager_state.current_sw_pos );
...
)
{
  ...
  if (asdiv_manager_state.init_done == FALSE)
  {
    ...
    asdiv_manager_state.l1_switch_complete_cb_data = NULL;
    asdiv_manager_state.l1_switch_complete_cb_func = NULL;
    asdiv_manager_state.rxlm_handle = 0xFFFFFFFF;
    /* initialize general callback states */
    rfcommon_asdiv_init_cb();
...
)
{
  ...
  /*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
    ...
    if(immediate == TRUE)
    {
       /*For Immediate switch dispatch all callbacks immediately */
       rfcommon_asdiv_dispatch_cb(rfm_mode, asdiv_manager_state.current_sw_pos );	
    }
    ...
    if ( send_script_cfg.cb_func != NULL ) 
    {
      ...
      /* Store rxlm handle */
      asdiv_manager_state.rxlm_handle = rxlm_handle;
        ...
        if ( switch_script_ptr != NULL )
      {
		...
		/* Send the script if it is not immediate */
        if (FALSE == immediate)
        {
  ...
  /* Update log packet with antenna position */
  rfcommon_asdiv_log_update (device, rfm_mode, antenna_position, ret_val);
  /* Return */
  return ret_val;
...
)
{
  ...
  rfcommon_asdiv_position_type temp_pos;
 ...
 if ( (rfm_mode >= RFM_NUM_MODES) || (rfm_mode <= RFM_PARKED_MODE) )
  ...
  {
    /*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
    /* [1] go back to previous position */
    /*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
    temp_pos = asdiv_manager_state.current_sw_pos;
    asdiv_manager_state.current_sw_pos = asdiv_manager_state.prev_sw_pos;
    asdiv_manager_state.prev_sw_pos = temp_pos;
    ...
    /*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
    /* [3] Update AsDIV tuners to go back to previous position */
    /*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
   ...
   /*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
    /* [4] Clean-up the CCS event and the SNUM call back data */
    /*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/

    ret_val &= rfcommon_mdsp_abort_asdiv_script(device,rfm_mode,asdiv_manager_state.rxlm_handle);
  }
  else
  {
    RF_MSG_2( RF_ERROR, "[ASDiv Manager] Abort Switch [ERROR : Dev %d "
...
{
  boolean switch_in_proc;
  /* Lock the ASDIV manager interface */
  rf_common_enter_critical_section(&(asdiv_manager_state.api_lock));

  switch_in_proc = asdiv_manager_state.switch_in_process;

  /* Release ASDIV manager interface*/
  rf_common_leave_critical_section(&(asdiv_manager_state.api_lock));

  return switch_in_proc;
}
...
/*!
  @brief
  API to initialize the general purpose ASDiv call back service.

  @details
  API to initial the ASDiv call back service.  Reset the variables and clears the
  call back registry.

  This API can be used in the context of ASDiv internal.

  @retval None.

*/
void rfcommon_asdiv_init_cb( void )
{
   uint8 i;

   /* reset number of registered cb */
   asdiv_manager_state.asdiv_cb_client_count = 0;

   /* clear the registery */
   for (i=0; i < ASDIV_MAX_CB_CLIENTS; i++ )
   {
      asdiv_manager_state.asdiv_registered_cb[i].handle = 0;
      asdiv_manager_state.asdiv_registered_cb[i].func_ptr = NULL;
      asdiv_manager_state.asdiv_registered_cb[i].data_ptr = NULL;
      asdiv_manager_state.asdiv_registered_cb[i].sw_pos_data_ptr = NULL;
      asdiv_manager_state.asdiv_registered_cb[i].rfm_mode = RFM_PARKED_MODE;
   }
}
...
/*!
  @brief
  API to register an ASDiv call back which will be called when ASDiv switch
  completes.

  @details
  API to register an ASDiv call back which will be called when ASDiv switch
  completes.   The call back function will be called along with parameters provided
  with the call back registration.

  This API can be used in the context of ASDiv external.

  Parameters:
  1. pointer call back function
  2. pointer to call back parameter data (if any)
  3. pointer to variable the switch position after call back
  4. technology type,  Default 0 = call back against all techs
  5. call back type (once or repeat), Default 0 = Once.

  Once   - will de-register itself automatically after call back is called.
  Repeat - will have call back remain in effect and must be de-register via de-registration API.

  @retval rfcommon_asdiv_cb_handler_type
  Registration is successful, return value will be an ASDiv call back handle.  This handle is used
  for call back de-registration.

  @retval NULL
  Registration fails due to ASDIV_MAX_CB_CLIENTS reached.

*/
rfcommon_asdiv_cb_handler_type rfcommon_asdiv_register_cb
(
   void *asdiv_cb_func_ptr,
   void *asdiv_cb_data_ptr,
   rfcommon_asdiv_position_type *asdiv_cb_sw_pos_data_ptr,
   rfm_mode_enum_type rfm_mode,
   rfcommon_asdiv_cb_type cb_mode
)
{
   uint8 reg_index;
   rfcommon_asdiv_cb_data_type *asdiv_reg_cb_ptr;

   /* check for valid registration criteri */
   if (( NULL == asdiv_cb_func_ptr) ||
       (asdiv_manager_state.asdiv_cb_client_count > ASDIV_MAX_CB_CLIENTS) ||
       (cb_mode >= ASDIV_CB_MAX))
   {
     RF_MSG_2(RF_ERROR, "ASDiv: Callback registration failed, fp=0x%x, cb_count=%d", \
                        asdiv_cb_func_ptr, asdiv_manager_state.asdiv_cb_client_count);
     return (NULL);
   }

   /* find next empty slot in registry */
   for (reg_index=0; reg_index < ASDIV_MAX_CB_CLIENTS; reg_index++ )
   {
     asdiv_reg_cb_ptr = &asdiv_manager_state.asdiv_registered_cb[reg_index];

     if ( NULL == asdiv_reg_cb_ptr->handle )
     {
        /* Found empty slot, register the call backs */
        asdiv_reg_cb_ptr->func_ptr = asdiv_cb_func_ptr;
        asdiv_reg_cb_ptr->data_ptr = asdiv_cb_data_ptr;
        asdiv_reg_cb_ptr->sw_pos_data_ptr = asdiv_cb_sw_pos_data_ptr;

        /* assign the cb handle, add the regristry index to make it unique  */
        asdiv_reg_cb_ptr->handle = (rfcommon_asdiv_cb_handler_type)asdiv_cb_func_ptr;
        asdiv_reg_cb_ptr->handle += reg_index;

        /* assign the cb types */
        asdiv_reg_cb_ptr->cb_mode = cb_mode;

        /* set of ALL modes if default 0 is passed in */
        if ( rfm_mode == RFM_PARKED_MODE )
        {
           asdiv_reg_cb_ptr->rfm_mode = RFM_ALL_MODES;
        }
        else
        {
           asdiv_reg_cb_ptr->rfm_mode = rfm_mode;
        }

        /* increase the registered count */
        asdiv_manager_state.asdiv_cb_client_count++;

       /* return the cb handle, needed if required to de-register later */
        return ( asdiv_reg_cb_ptr->handle );
     }
   }

   /* unable to register, registry full */
   RF_MSG_1(RF_ERROR, "ASDiv cb unable to register. Registry full. count=%d", \
                      asdiv_manager_state.asdiv_cb_client_count);
...
/*!
  @brief
  API to de-register an previously registered ASDiv call back.

  @details
  API to de-register an previously registered ASDiv call back.  The asdiv handle
  that was provided when the call back was registered is needed to de-register the
  call back.

  This API can be used in the context of ASDiv external.

  Parameter:
  asdiv_cb_handle – the ASDiv call back handle that was returned when call back was successfully registered

  @retval TRUE
  De-registration was successful

  @retval FALSE
  No de-registration action taken, handled not found and already de-registered.

*/
boolean rfcommon_asdiv_deregister_cb
(
...
)
{
   ...
   /* find associated handler slot in registry */
   asdiv_reg_cb_ptr = &asdiv_manager_state.asdiv_registered_cb[0];

   for (reg_index=0; reg_index < ASDIV_MAX_CB_CLIENTS; reg_index++ )
   {
     if (( NULL != asdiv_reg_cb_ptr->handle ) &&
         ( asdiv_cb_handle == asdiv_reg_cb_ptr->handle ))
     {
        /* Found it, clear the call back entries */
        asdiv_reg_cb_ptr->func_ptr = NULL;
        asdiv_reg_cb_ptr->data_ptr = NULL;
        asdiv_reg_cb_ptr->sw_pos_data_ptr = NULL;
        asdiv_reg_cb_ptr->handle = NULL;
        asdiv_reg_cb_ptr->rfm_mode = RFM_PARKED_MODE;
        asdiv_reg_cb_ptr->cb_mode = ASDIV_CB_ONCE;
   ...
   /* unable to de-register, handle not found - already de-registered */
   RF_MSG_2(RF_HIGH, "ASDiv cb unable to de-register. Cannot find handle:0x%x count=%d", \
                      asdiv_cb_handle,
                      asdiv_manager_state.asdiv_cb_client_count);
...
/*!
  @brief
  API to dispatch the ASDiv client registered call backs.

  @details
  API to dispatch the ASDiv client registered call backs.   The callbacks functions
  will be called with the parameters pointers that was provided during the
  call back registration.

  Call back can be matched with the rfm_mode type that it was registered or by default call back will
  be dispatch on all ASDiv switches.
...
*/
void rfcommon_asdiv_dispatch_cb
(
   rfm_mode_enum_type asdiv_rfm_mode,
   rfcommon_asdiv_position_type curr_sw_pos
)
{
   uint8 reg_index;
   rfm_mode_enum_type asdiv_check_mode;
   rfcommon_asdiv_cb_data_type *asdiv_reg_cb_ptr;

   /* walk through registry and initiate call back one after another */
   for (reg_index=0; reg_index < ASDIV_MAX_CB_CLIENTS; reg_index++ )
   {
     asdiv_reg_cb_ptr = &asdiv_manager_state.asdiv_registered_cb[reg_index];

     if ((NULL != asdiv_reg_cb_ptr) && ( NULL != asdiv_reg_cb_ptr->func_ptr ) &&
         (( asdiv_reg_cb_ptr->rfm_mode == asdiv_rfm_mode ) ||
          ( asdiv_reg_cb_ptr->rfm_mode == RFM_ALL_MODES)))
     {

        RF_MSG_3(RF_MED, "ASDiv: call back[%d] executing. mode=0x%x, asdiv_rfm_mode=0x%x", \
                         reg_index, asdiv_reg_cb_ptr->rfm_mode, asdiv_rfm_mode);
        /* Set switch position variable in call back */
        if(asdiv_reg_cb_ptr->sw_pos_data_ptr != NULL)
        {
          *asdiv_reg_cb_ptr->sw_pos_data_ptr = curr_sw_pos;
        }

        if((asdiv_reg_cb_ptr->func_ptr != NULL) && (asdiv_reg_cb_ptr->data_ptr != NULL) && (asdiv_reg_cb_ptr->sw_pos_data_ptr != NULL))
        {
        /* Execute the call back based on maching tech and others */
        (asdiv_reg_cb_ptr->func_ptr)((asdiv_reg_cb_ptr->data_ptr),(asdiv_reg_cb_ptr->sw_pos_data_ptr));
        }

        /* deregister call back if request is to run once */
        if (( NULL != asdiv_reg_cb_ptr->func_ptr ) &&
            ( ASDIV_CB_ONCE == asdiv_reg_cb_ptr->cb_mode ))
        {
          asdiv_reg_cb_ptr->func_ptr = NULL;
          asdiv_reg_cb_ptr->data_ptr = NULL;
          asdiv_reg_cb_ptr->handle = NULL;
