...
/*==============================================================================
  ...
  $Header: //components/rel/rfa.mpss/4.11/rf/common/rf/atuner/inc/rfcommon_asdiv_tuner_manager.h#1 $

when       who     what, where, why
--------   ---     -------------------------------------------------------------
09/28/15   adk     Enabled ASDIV plus tuners
07/15/15   dyc     Un-paired (dummy) tuner support.
06/22/15   yb      Remove CA port
...
public:
  ...
  boolean update_rfc_asdiv_config_info( rfc_cfg_params_type *cfg,
                                        rfc_device_info_type *device_info_ptr);

  /* update the asdiv config table with dummy tuner info */
  boolean update_rfc_asdiv_config_info_dtuner( uint32 ant_path,
...
private:
  ...
  boolean populate_asdiv_tuner_objects(uint8 antenna, rfc_asdiv_ant_dev_info_type *asdiv_ant_dev_info_ptr);

  rfcommon_atuner_intf *asdiv_tuner_objects[RFC_MAX_ANTENNA][RFC_ASDIV_MAX_DEVICES_PER_ANT_PATH];

  /* used for dummy device determination, to check for matched device pairing for given antenna */
  boolean device_pair[RFC_MAX_ANTENNA][RFDEVICE_TYPE_MAX_NUM];

  /* used to determine which antenna is pair with which when swapping */
