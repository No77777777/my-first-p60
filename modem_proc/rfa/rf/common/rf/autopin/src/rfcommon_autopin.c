...
/*==============================================================================
  ...
  $Header: //components/rel/rfa.mpss/4.11/rf/common/rf/autopin/src/rfcommon_autopin.c#1 $

when       who     what, where, why
--------   ---     -------------------------------------------------------------
10/27/16   am      Do not rest autopin state variables when autopin is in progress
05/28/16   wxl     Add API to get Autopin disable bitmask from NV 
05/22/16   am      Separate out fw_autopin_fn and reflog done msg reg
05/22/16   am      Initialize fw_autopin_fn ptr if it is NULL
04/07/16   vr      Make sure to Apply correction factor only once.
04/12/16   vr      Legacy Autopin Support for WTR3925
03/31/15   ag      Make sure to call autopin xpt sanity during autopin enable
03/17/16   qzh     Add protection for invalid input when init autopin settings
03/08/16   qzh     Add WTR3925 AMAM autopin Txagc related changes
03/03/16   zhh     Move the delta_pin_pout copy to WTR3925 macro
03/01/16   zhh     Populate delta PinPout cal result to rflm_dm
...
12/22/15   am      Putting F3 to print pin_accum
12/15/15   whz     Set imme_trig to FALSE by default
                   Sanity check on REFLOG and WTR timings
12/10/12   ag      Addition of F3s
...
11/12/15   kma     Updated capture param interface
...
#ifdef FEATURE_RF_HAS_WTR2965
extern void
rf_cdma_autopin_update_par_pin
(
	  uint32 handle,
	  uint8 pa_state,
	  int16 pin_accum
...
void rfcommon_autopin_get_nv_info(rfm_mode_enum_type rfm_tech)
{
  ...
  else
  {
    MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR,
          "rfcommon_autopin_get_nv_info(), tech is invalid, rfm_tech is %d",
          rfm_tech);
  }
}
...
{
  rflm_tech_id_t tech = rfcommon_autopin_mc_convert_rfm_mode_to_rflm_tech(rfm_tech);
  return rfcommon_autopin_is_enabled_efs(tech);
}

/* Check if AutoPin enable flag is TRUE for rfm_tech. Should be called only when tx handle is valid
 */
boolean rfcommon_autopin_is_enabled_flag_check(rflm_handle_tx_t tx_handle)
{
  ...
  {
    return  autopin_template->autopin_ctl.autopin_enable_flag;
  }
  else
  {
    MSG(MSG_SSID_RF, MSG_LEGACY_HIGH,"tx_handle is invalid to check autopin enable flag");
    return TRUE;
  }
}
...
#ifdef FEATURE_RF_HAS_WTR2965
  if (rflm_autopin_fw_autopin_fn == NULL)
  {
                                                          ...
                                                          tx_band_cal_type *tx_cal_data)
{
  ...
  if (!rfcommon_autopin_is_enabled_efs_nv(tech))
  {
    MSG(MSG_SSID_RF, MSG_LEGACY_HIGH,"AutoPin is not enabled");
    autopin_template->autopin_ctl.autopin_enable_flag = FALSE;
    return RFCOMMON_AUTOPIN_ERROR;
  }

  autopin_template->autopin_ctl.autopin_enable_flag = TRUE;
 ...
 rflm_autopin_xpt_sanity_check(tx_handle, RFLM_TXAGC_MAX_XPT_MODE);
  ...
  autopin_template->autopin_ctl.rxiqbuf.num_samp = out.static_settings.num_ref_samples + (out.static_settings.num_rx_taps-1);

  memset(autopin_template->autopin_ctl.pin_accum, 0, sizeof(rflm_db10_t)*RFLM_TXAGC_PA_STATE_MAX);

  memset(autopin_template->autopin_ctl.run_state, RFLM_AUTOPIN_DEFAULT, sizeof(rflm_autopin_state)*RFLM_TXAGC_PA_STATE_MAX);

  autopin_template->autopin_ctl.pout_delta = 0;

  if(NULL != tx_cal_data)
  {
    for(lp_var = 0; lp_var < RFCOM_TXLIN_NUM_PIN_LUT; lp_var++)
    {
      if(NULL != tx_cal_data->pin_lut[lp_var])
      {
        autopin_template->pin_pout_lut.valid[lp_var] = TRUE;
        autopin_template->pin_pout_lut.lut[lp_var].dpd_idx = tx_cal_data->pin_lut[lp_var]->dpd_idx;
        autopin_template->pin_pout_lut.lut[lp_var].num_pin = tx_cal_data->pin_lut[lp_var]->num_pin;
        for(inner_lp_var = 0;
            inner_lp_var < autopin_template->pin_pout_lut.lut[lp_var].num_pin;
            inner_lp_var++)
        {
          autopin_template->pin_pout_lut.lut[lp_var].pin_misalign[inner_lp_var] =
                              tx_cal_data->pin_lut[lp_var]->pin_misalign[inner_lp_var];
          autopin_template->pin_pout_lut.lut[lp_var].gain_comp[inner_lp_var] =
                              tx_cal_data->pin_lut[lp_var]->gain_comp[inner_lp_var];
          autopin_template->pin_pout_lut.lut[lp_var].pout_delta[inner_lp_var] =
                              tx_cal_data->pin_lut[lp_var]->pout_delta[inner_lp_var];
        }
      }
      else
      {
        autopin_template->pin_pout_lut.valid[lp_var] = FALSE;
      }
    }
  }
  else
                                      ...
                                      rfcommon_autopin_capture_control_type capture_input)
{
  ...
  int16 wtr_correction_factor = 0;/* in 100dbm units cal result showed a correction of  0.1dBm  */
...
*/
 ...
 else
 {
  ...
  RF_MSG_2(RF_HIGH,"rfcommon_autopin_update_pin: autopin_ctl.pa_state %d, autopin_ctl.pin_accum %d",
  ...
  case RFLM_TECH_HDR:
    ...
    rf_cdma_autopin_update_par_pin(handle,
                         autopin_data_ptr->autopin_ctl.pa_state,
                         autopin_data_ptr->autopin_ctl.pin_accum[autopin_data_ptr->autopin_ctl.pa_state]);
#endif
    break;
