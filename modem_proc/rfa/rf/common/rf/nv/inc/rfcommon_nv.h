...
Copyright (c) 1997 - 2018 by QUALCOMM Technologies, Inc.  All Rights Reserved.
===========================================================================*/
...
$Header: //components/rel/rfa.mpss/4.11/rf/common/rf/nv/inc/rfcommon_nv.h#1 $
...
09/28/15   yb      Update Band specific NV definition
09/1/14    pv      Added support for Intra-band ULCA PA swp.
09/02/15   yb      Added support for new version of Tuner CL NVs
06/29/14   sd      FR 27572 Add support for AsDiv SAR backoff NV
04/13/15   adk     Remove db10 units from return_loss and hypothesis thresholds
...
#define RFCOMMON_ASDIV_MAX_POSITION 2
#define RFCOMMON_SAR_BACK_OFF_NV_SIZE 8
/*----------------------------------------------------------------------------*/
/*! 
  @brief
...
extern boolean band_has_rx_switchpt_datatype;
...
typedef enum
{
  ...
  RFCOMMON_NV_ATUNER_CL_CTRL_INFO_V_B_ID   = 0x0B,
...
typedef PACK(struct)
{
  ...
  /*Defines the threshold for AOL scenario or when to use to sensor 
...
typedef PACK(struct)
{
  ...
  /* Tuner instance data type: Max 3 Tuners */
  rfcommon_atuner_static_params_data_type tuner_instance_static_params[3];

}rfcommon_nv_ant_tuner_cl_ctrl_data_type_v_b;
...
#define RFCOMMON_NV_RESERVED_ROW_V2  2
#define RFCOMMON_NV_MAX_CDMA_RATE    6

/*--------------------------------------------------------------------------*/
/*!>
  @brief
  This Macro defines the index for Header for PA STATIC NV in the new NV desgin
  
*/
#define RF_CMN_PA_STATIC_HEADER_INDEX         0
...
/* LTE PA switchpoint structure - Used by LTE tech only */
typedef PACK(struct)
{
    /* PA Fall switchpoint: 0-> Timer Hystersis value | 1-> R1 fall | 2 -> R2 fall etc */
  int16 pa_fall_sw[RFCOMMON_NV_MAX_PA_STATE];
...
} rfcommon_nv_lte_pa_static_type;
...
typedef PACK(struct)
{
  ...
  /* Pout Control for Tx linearizer offset */
  int16 pout_control[RFCOMMON_NV_MAX_PA_STATE];

  /* Reserved Rows can be used for future extentions */
  int16 reserved[RFCOMMON_NV_RESERVED_ROW_V2][RFCOMMON_NV_MAX_PA_STATE];
  
  /* LTE Intra_band_ULCA PA switchpoints */
  rfcommon_nv_lte_pa_static_type lte_intra_band_ulca_pa_swpts;

} rfcommon_nv_cmn_pa_static_v2_type;
...
/*RF WCDMA SAR NV structure*/
typedef PACK(struct)
{
  /*Flag to indicate if NV was enabled for the band*/
  boolean band_enabled;
  uint16 sar_back_off_table_asdiv_pos_1[RFCOMMON_SAR_BACK_OFF_NV_SIZE]; 
} rfwcdma_nv_sar_asdiv_type;
/*===========================================================================
...
/*! NV Structure that will store timing information of type int16 */
typedef PACK(struct)
{
  /*! Start timing unit */
  int16 start;
  ...
  /*! Stop timing unit */
  int16 stop;
} rfnv_timing_pair_i16_type;

/*! NV Structure that will store RF tech-band information */
typedef PACK(struct)
{
  /*! The rf tech enum as defined in rfm_mode_enum_type */
  uint8 rf_tech;
  /*! The corresponding rf band as defined in rfcom_abc_band_type, for example: rfcom_gsm_band_type */
  uint16 rf_band;
} rfnv_rf_tech_band_type;

/*! Defines the NV array size for rf tech-band pairs */
#define RFNV_GNSS_BLANKING_CONTROL_NUM_RF_TECH_BANDS 256

/*! NV Structure that will store RFNV_GNSS_BLANKING_CONTROL_I */
typedef PACK(struct)
{
  /*! Determines if this NV is active. 
      0 means this NV is not used (default setting). Blanking controlled by RFC.
      1 means this NV will disable blanking only for the tech-band listed in the array */
  uint8 nv_enable;

  /*! GSM start/stop timing offsets */
  rfnv_timing_pair_i16_type gsm_timing_offset;

  /*! LTE start/stop timing offsets */
  rfnv_timing_pair_i16_type lte_timing_offset;

  /*! TDS start/stop timing offsets */
  rfnv_timing_pair_i16_type tds_timing_offset;

  /*! Table for rf tech and rf band pairs for which to disable GNSS blanking */
  rfnv_rf_tech_band_type rf_tech_bands[ RFNV_GNSS_BLANKING_CONTROL_NUM_RF_TECH_BANDS ];
} rfnv_gnss_blanking_control_type;
...
typedef struct
{
  /*! This represents the rf mode enum "FTM_RF_Mode_Enum" as used by DLL to populate the char data */
  uint8 rf_mode;
  /*! AsDIv offset for given rf_mode, in db10 */
  int8 asdiv_offset_db10;  
  
}rfcommon_asdiv_offset_char_data;

#define ASDIV_EFS_MAX_PATH_SIZE 50
