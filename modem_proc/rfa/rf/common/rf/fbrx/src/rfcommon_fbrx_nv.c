...
/*==============================================================================
  ...
  $Header: //components/rel/rfa.mpss/4.11/rf/common/rf/fbrx/src/rfcommon_fbrx_nv.c#1 $

when       who     what, where, why
--------   ---     -------------------------------------------------------------
06/03/20   cv      Heap Clade2 changes
03/03/20   Hari    Heap Clade2 changes CR 2677758
03/31/20   cv      Correct feature flag
03/03/20   Hari    Heap Clade2 changes CR 2629903
01/24/20   moh     RF MDUMP Changes
01/20/20   ssl     FR 55657: Replace RF Enter/Exit with RF Sleep/Wakeup for L+L DSDS awake
10/26/17   sk      Support FBRX LTE channel list as offset fro B71/B66
10/26/17   sk      Enable LTE B71 support
10/26/17   gk      Compilation Fixes for RFA for migration of 6.x LLVM to 8.x 
01/16/17   sk      Fix droop cal nv data retrieve after NV-reload
08/26/16   aa      Fix to load LTE droop NV in RF Cal
08/11/16   sk      New Static NV for ULCA FBRx switch points
07/05/16   aa      Changes for fbrx cal for mode2 different channel list support 
...
08/03/15    aa     Fix KW error
08/03/15    aa     Fix F3 to avoid blacklist patch from being flagged 
07/21/15    aa     FBRx C1 NV container fix to avoid nv_read_size buffer overflow
...
06/03/15   zhw     Fix BC Config mapping during FBRX NV load
05/28/15   aro     Support to test ATLAS WCDMA driver in THOR baseline
05/28/15   zhw     Fixed BC Config mapping
05/27/15   zhw     Map BC COnfig to logical device for CDMA                   
05/19/15   zhw     Fixed FBRX NV loading using RFM_DEVICE_6
04/23/15    ka     Remove hardcoded device 0 for fbrx
04/02/15    aa     Fix compiler due related to missing featurization
03/25/15    jmf    [selftest] Add function to read Undroop filter coeffs from NV for spec. flatness meas.
...
02302/15    kr     Code changes to avoid compilation errors in WCDMA Compile out flavor.
02/28/15   ska     Fix KW error
02/26/15    jr     Code changes to avoid compilation errors in LTE Compile out flavor.
02/26/15    rp     Code changes to avoid compilation errors in LTE Compile out flavor.
02/25/15    ka     Default to longer sample sizes
02/09/15    aa     Fxi bug to resolve crash during droop cal
...
12/30/14    aa     Fix KW errors
12/12/14    aa     Changes to support less than 16 channels FBRx temp/freq comp char
12/03/14    aa     Fix bug during FBRx temp comp normalization
...
rfcommon_fbrx_nv_mem_alloc_init_type rfcommon_fbrx_nv_tech_init_mem_data = { {FALSE}, {FALSE}, {FALSE}, {FALSE} };
...
static uint32 rfcommon_fbrx_increase_sample_size = 0; /* FBRx control NV data */
...
static uint8 rfcommmon_fbrx_mode_cal_chan_idx[RFCOMMON_FBRX_NUM_MODES][RFCOMMON_FBRX_NUM_CAL_CHANNELS] = {0};


#ifdef FEATURE_QSH_MDUMP
QSH_MDUMP_FN_ATTR
void* get_rfcommon_fbrx_nv_tech_cal_data(uint32* size)
{
  *size = sizeof(rfcommon_fbrx_nv_cal_data_type);
  return (void*) &rfcommon_fbrx_nv_tech_cal_data;
}
#endif
/*----------------------------------------------------------------------------*/
/*!
                                                          ...
                                                          rfcommon_fbrx_nv_container_type c_val )
{
  ...
  if (c_val == RFCOMMON_FBRX_NV_CONTAINER_C0) 
  {
    switch (band)
    {
     ...
     case   RFCOM_BAND_LTE_B30:
      ...
      default:
        RF_MSG_2(RF_ERROR, "rfcommon_fbrx_get_lte_cal_nv_id: FBRX NVs not supported for LTE Band = %d, NV container = %d", band, c_val);
        *nv_item_gain = RFNV_LTE_C0_B1_FBRX_GAIN_FREQ_I;
        *nv_item_sp = RFNV_LTE_C0_B1_FBRX_GAIN_SWITCH_POINT_I;
        *nv_item_dc = RFNV_LTE_C0_B1_FBRX_DC_CAL_VAL_I;
        *nv_item_temp = RFNV_LTE_C0_B1_FBRX_MODE_VS_GAIN_VS_TEMP_VS_FREQ_I;
    ...
    } /* end switch case (band) */
  } /* end if ( c_val == RFCOMMON_FBRX_NV_CONTAINER_C0 ) */
  else if ( c_val == RFCOMMON_FBRX_NV_CONTAINER_C1 ) 
  {
    switch (band)
    {
      case   RFCOM_BAND_LTE_B1:
        *nv_item_gain = RFNV_LTE_C1_B1_FBRX_GAIN_FREQ_I;
        *nv_item_sp = RFNV_LTE_C1_B1_FBRX_GAIN_SWITCH_POINT_I;
        *nv_item_dc = RFNV_LTE_C1_B1_FBRX_DC_CAL_VAL_I;
        *nv_item_temp = RFNV_LTE_C1_B1_FBRX_MODE_VS_GAIN_VS_TEMP_VS_FREQ_I;
        break;

      case   RFCOM_BAND_LTE_B2:
        *nv_item_gain = RFNV_LTE_C1_B2_FBRX_GAIN_FREQ_I;
        *nv_item_sp = RFNV_LTE_C1_B2_FBRX_GAIN_SWITCH_POINT_I;
        *nv_item_dc = RFNV_LTE_C1_B2_FBRX_DC_CAL_VAL_I;
        *nv_item_temp = RFNV_LTE_C1_B2_FBRX_MODE_VS_GAIN_VS_TEMP_VS_FREQ_I;
        break;

      case   RFCOM_BAND_LTE_B3:
        *nv_item_gain = RFNV_LTE_C1_B3_FBRX_GAIN_FREQ_I;
        *nv_item_sp = RFNV_LTE_C1_B3_FBRX_GAIN_SWITCH_POINT_I;
        *nv_item_dc = RFNV_LTE_C1_B3_FBRX_DC_CAL_VAL_I;
        *nv_item_temp = RFNV_LTE_C1_B3_FBRX_MODE_VS_GAIN_VS_TEMP_VS_FREQ_I;
        break;

      case   RFCOM_BAND_LTE_B4:
        *nv_item_gain = RFNV_LTE_C1_B4_FBRX_GAIN_FREQ_I;
        *nv_item_sp = RFNV_LTE_C1_B4_FBRX_GAIN_SWITCH_POINT_I;
        *nv_item_dc = RFNV_LTE_C1_B4_FBRX_DC_CAL_VAL_I;
        *nv_item_temp = RFNV_LTE_C1_B4_FBRX_MODE_VS_GAIN_VS_TEMP_VS_FREQ_I;
        break;

      case   RFCOM_BAND_LTE_B21:
        *nv_item_gain = RFNV_LTE_C1_B21_FBRX_GAIN_FREQ_I;
        *nv_item_sp = RFNV_LTE_C1_B21_FBRX_GAIN_SWITCH_POINT_I;
        *nv_item_dc = RFNV_LTE_C1_B21_FBRX_DC_CAL_VAL_I;
        *nv_item_temp = RFNV_LTE_C1_B21_FBRX_MODE_VS_GAIN_VS_TEMP_VS_FREQ_I;
        break;

      case   RFCOM_BAND_LTE_B25:
        *nv_item_gain = RFNV_LTE_C1_B25_FBRX_GAIN_FREQ_I;
        *nv_item_sp = RFNV_LTE_C1_B25_FBRX_GAIN_SWITCH_POINT_I;
        *nv_item_dc = RFNV_LTE_C1_B25_FBRX_DC_CAL_VAL_I;
        *nv_item_temp = RFNV_LTE_C1_B25_FBRX_MODE_VS_GAIN_VS_TEMP_VS_FREQ_I;
        break;

      case   RFCOM_BAND_LTE_B39:
        *nv_item_gain = RFNV_LTE_C1_B39_FBRX_GAIN_FREQ_I;
        *nv_item_sp = RFNV_LTE_C1_B39_FBRX_GAIN_SWITCH_POINT_I;
        *nv_item_dc = RFNV_LTE_C1_B39_FBRX_DC_CAL_VAL_I;
        *nv_item_temp = RFNV_LTE_C1_B39_FBRX_MODE_VS_GAIN_VS_TEMP_VS_FREQ_I;
        break;

      case   RFCOM_BAND_LTE_B66:
        *nv_item_gain = RFNV_LTE_C1_B66_FBRX_GAIN_FREQ_I;
        *nv_item_sp   = RFNV_LTE_C1_B66_FBRX_GAIN_SWITCH_POINT_I;
        *nv_item_dc   = RFNV_LTE_C1_B66_FBRX_DC_CAL_VAL_I;
        // This band only supports the compressed gain_vs_temp_vs_freq item
        *nv_item_temp = RFNV_ITEMS_ENUM_MAX;
        break;

      default:
        RF_MSG_2(RF_HIGH, "rfcommon_fbrx_get_lte_cal_nv_id: FBRX NVs not supported for LTE Band = %d, NV container = %d", band, c_val);
        *nv_item_gain = RFNV_LTE_C1_B1_FBRX_GAIN_FREQ_I;
        *nv_item_sp = RFNV_LTE_C1_B1_FBRX_GAIN_SWITCH_POINT_I;
        *nv_item_dc = RFNV_LTE_C1_B1_FBRX_DC_CAL_VAL_I;
        *nv_item_temp = RFNV_LTE_C1_B1_FBRX_MODE_VS_GAIN_VS_TEMP_VS_FREQ_I;
  ...
  } /* end if ( c_val == RFCOMMON_FBRX_NV_CONTAINER_C1 ) */
  else
  {
    RF_MSG_2(RF_ERROR, "rfcommon_fbrx_get_lte_cal_nv_id: FBRX NVs not supported for LTE Band = %d, NV container = %d", band, c_val);
    *nv_item_gain = RFNV_LTE_C0_B1_FBRX_GAIN_FREQ_I;
...
/*!
  @brief
  rfcommon_fbrx_get_lte_c1_cal_nv_id
...
rfcommon_fbrx_error_type rfcommon_fbrx_get_lte_c1_cal_nv_id( rfcom_lte_band_type band, 
                                                             rfnv_item_id_enum_type *nv_item_gain, 
                                                             rfnv_item_id_enum_type *nv_item_sp,
                                                             rfnv_item_id_enum_type *nv_item_dc,
                                                             rfnv_item_id_enum_type *nv_item_temp )
{
  ...
  switch (band)
  {
    ...
    default:
      RF_MSG_1(RF_ERROR, "rfcommon_fbrx_get_lte_c1_cal_nv_id: Invalid Band = %d", band);
      *nv_item_gain = RFNV_LTE_C1_B1_FBRX_GAIN_FREQ_I;
      *nv_item_sp = RFNV_LTE_C1_B1_FBRX_GAIN_SWITCH_POINT_I;
      *nv_item_dc = RFNV_LTE_C1_B1_FBRX_DC_CAL_VAL_I;
      *nv_item_temp = RFNV_LTE_C1_B1_FBRX_MODE_VS_GAIN_VS_TEMP_VS_FREQ_I;
                                                                     ...
                                                                     rfcommon_fbrx_nv_container_type c_val )
{
  ...
  if ( c_val == RFCOMMON_FBRX_NV_CONTAINER_C0 ) 
  {
    switch (band)
    {
      ...
      default:
        RF_MSG_2(RF_ERROR, "rfcommon_fbrx_get_lte_cal_compressed_nv_id: FBRX NV not supported for Band = %d, nv container = %d", band, c_val);
        *nv_item_temp_z = RFNV_LTE_C0_B1_FBRX_MODE_VS_GAIN_VS_TEMP_VS_FREQ_Z_I;
        ret_value = RFCOMMON_FBRX_ERROR;
  ...
  else if ( c_val == RFCOMMON_FBRX_NV_CONTAINER_C1 ) 
  {
    ...
    {
      case   RFCOM_BAND_LTE_B1:
        *nv_item_temp_z = RFNV_LTE_C1_B1_FBRX_MODE_VS_GAIN_VS_TEMP_VS_FREQ_Z_I;
        break;

      case   RFCOM_BAND_LTE_B2:
        *nv_item_temp_z = RFNV_LTE_C1_B2_FBRX_MODE_VS_GAIN_VS_TEMP_VS_FREQ_Z_I;
        break;

      case   RFCOM_BAND_LTE_B3:
        *nv_item_temp_z = RFNV_LTE_C1_B3_FBRX_MODE_VS_GAIN_VS_TEMP_VS_FREQ_Z_I;
        break;

      case   RFCOM_BAND_LTE_B4:
        *nv_item_temp_z = RFNV_LTE_C1_B4_FBRX_MODE_VS_GAIN_VS_TEMP_VS_FREQ_Z_I;
        break;

      case   RFCOM_BAND_LTE_B21:
        *nv_item_temp_z = RFNV_LTE_C1_B21_FBRX_MODE_VS_GAIN_VS_TEMP_VS_FREQ_Z_I;
        break;

      case   RFCOM_BAND_LTE_B25:
        *nv_item_temp_z = RFNV_LTE_C1_B25_FBRX_MODE_VS_GAIN_VS_TEMP_VS_FREQ_Z_I;
        break;

      case   RFCOM_BAND_LTE_B39:
        *nv_item_temp_z = RFNV_LTE_C1_B39_FBRX_MODE_VS_GAIN_VS_TEMP_VS_FREQ_Z_I;
        break;

      case   RFCOM_BAND_LTE_B66:
        *nv_item_temp_z = RFNV_LTE_C1_B66_FBRX_MODE_VS_GAIN_VS_TEMP_VS_FREQ_Z_I;
        break;
                                                                               ...
                                                                               rfcommon_fbrx_nv_container_type c_val )
{
  ...
  if ( c_val == RFCOMMON_FBRX_NV_CONTAINER_C0 ) 
  {
    switch (band)
    {
     ...
     case   RFCOM_BAND_LTE_B71:
      ...
      default:
        RF_MSG_2(RF_ERROR, "rfcommon_fbrx_get_lte_band_bw_droop_compressed_nv_id: FBRx NV not supported for Band = %d, nv container = %d", band, c_val);
        *nv_item_temp_z = RFNV_LTE_C0_B1_FBRX_DROOP_FIR_FILTER_COEFF_Z_I;
        ret_value = RFCOMMON_FBRX_ERROR;
  ...
  else if ( c_val == RFCOMMON_FBRX_NV_CONTAINER_C1 ) 
  {
    ...
    {
      case   RFCOM_BAND_LTE_B1:
        *nv_item_temp_z = RFNV_LTE_C1_B1_FBRX_DROOP_FIR_FILTER_COEFF_Z_I;
        break;

      case   RFCOM_BAND_LTE_B2:
        *nv_item_temp_z = RFNV_LTE_C1_B2_FBRX_DROOP_FIR_FILTER_COEFF_Z_I;
        break;

      case   RFCOM_BAND_LTE_B3:
        *nv_item_temp_z = RFNV_LTE_C1_B3_FBRX_DROOP_FIR_FILTER_COEFF_Z_I;
        break;

      case   RFCOM_BAND_LTE_B4:
        *nv_item_temp_z = RFNV_LTE_C1_B4_FBRX_DROOP_FIR_FILTER_COEFF_Z_I;
        break;

      case   RFCOM_BAND_LTE_B21:
        *nv_item_temp_z = RFNV_LTE_C1_B21_FBRX_DROOP_FIR_FILTER_COEFF_Z_I;
        break;

      case   RFCOM_BAND_LTE_B25:
        *nv_item_temp_z = RFNV_LTE_C1_B25_FBRX_DROOP_FIR_FILTER_COEFF_Z_I;
        break;

      case   RFCOM_BAND_LTE_B39:
        *nv_item_temp_z = RFNV_LTE_C1_B39_FBRX_DROOP_FIR_FILTER_COEFF_Z_I;
        break;

      case   RFCOM_BAND_LTE_B66:
        *nv_item_temp_z = RFNV_LTE_C1_B66_FBRX_DROOP_FIR_FILTER_COEFF_Z_I;
        break;
...
)
{
  ...
  if (tx_cfg < RFCOMMON_FBRX_NUM_TX_CONFIG_MAX)
  {
    ...
    {
      lte_droop_data = rfcommon_fbrx_nv_tech_cal_data.lte_cal_data[c_val].droop_data[band.lte_band];
...
rfcommon_fbrx_error_type rfcommon_fbrx_get_current_droop_filter
( 
  ftm_rf_technology_type ftm_tech,
  rfcom_band_type_u band,
  rfcommon_fbrx_tx_cfg_type tx_cfg,
...
{
  rfcommon_fbrx_error_type ret_value = RFCOMMON_FBRX_ERROR;
  rfcommon_fbrx_nv_lte_band_droop_params_type *lte_droop_data = NULL;
  uint32 handle=0;
  ...
  if( ftm_tech == FTM_RF_TECH_LTE )
  {
    ...
    if( lte_droop_data == NULL ) 
    {
      ...
      if( lte_droop_data != NULL ) 
      {
        ...
        rfcommon_fbrx_nv_tech_cal_data.lte_cal_data[c_val].droop_data[band.lte_band] = lte_droop_data;
      }
    }
  ...
  handle = ftm_common_fbrx_get_valid_tx_handle( ftm_common_ftm_tech_to_rfm_mode(ftm_tech), 
                                                rfm_tx_device, 
  ...
  if (ret_value != RFCOMMON_FBRX_ERROR && fbrx_data_ptr != NULL )
  {  
...
/*!
  @brief  
  HELPER API for extracting FBRX Therm ADC for a given band
...
rfcommon_fbrx_error_type
rfcommon_fbrx_nv_extract_therm_val
(
  const void * const nv_data_ptr,
  const uint32 num_bytes_available,
  uint32 * const extracted_element_size,
...
)
{
  ...
  rfcommon_fbrx_error_type api_status = RFCOMMON_FBRX_SUCCESS;
  /* The item to extract from NV */
  rfcommon_fbrx_therm_adc_val_type fbrx_therm_data;
  ...
  *extracted_element_size = 0;
  ...
  /* The number of bytes needed to extract element */
  num_bytes_needed = sizeof(fbrx_therm_data);

  /* Ensure there is enough memory left before actually extracting the element */
  if( num_bytes_available < num_bytes_needed )
  {
    RF_MSG( RF_ERROR, "rfcommon_fbrx_nv_extract_therm_val: Not enough elements left to extract");
    api_status = RFCOMMON_FBRX_ERROR;
  }
  ...
  {
    /* Extract the element */
    memscpy( (void*) &fbrx_therm_data, sizeof(fbrx_therm_data), nv_data_ptr, num_bytes_needed );

    /* update this variable so that caller API can know how many bytes are copied in this call */
    *extracted_element_size = num_bytes_needed;

    fbrx_gain_nv_data->fbrx_therm_adc = fbrx_therm_data;
    ...
    for( type_id_idx = 0; (type_id_idx < num_type_id) ; type_id_idx++ )
    {
     ...
     /* EXTRACT ELEMENT corresponding to the this type ID */
      ...
      switch( type_id )
      {
        ...
        case RFNV_DATA_TYPE_FBRX_THERM_ADC_V1:
          if (nv_version == 1)
          {
            api_status = rfcommon_fbrx_nv_extract_therm_val(temp_data_ptr,
                                                            temp_num_bytes_left,
                                                            &nv_element_size,
                                                            (rfcommon_fbrx_nv_gain_vs_freq_type_v2*)fbrx_gain_nv_data );  
          }
                                ...
                                rfcommon_fbrx_nv_container_type c_val  )
{
  ...
  switch (tech)
  {
    case RFM_IMT_MODE:
      if (band.wcdma_band < RFCOM_NUM_WCDMA_BANDS)
      {
	  ...
	  #else
          clade_heap_calloc(1,sizeof(rfcommon_fbrx_nv_gain_vs_freq_type_v2));
          #endif		
        }
	...
	#else
        clade_heap_calloc(1,sizeof(rfcommon_fbrx_nv_attn_switch_pt_type));
	#endif 
        }
	  ...
	  #else
          clade_heap_calloc(1,sizeof(rfcommon_fbrx_nv_temp_type));
	  #endif
             ...
             (rfcommon_fbrx_nv_tech_cal_data.wcdma_cal_data.temp_comp_data[band.wcdma_band] == NULL) )
        {
          ...
          rfcommon_fbrx_nv_tech_init_mem_data.wcdma_init[band.wcdma_band].cal_data = FALSE;
          ret_val = FALSE;
        }
        else
        {
          rfcommon_fbrx_nv_tech_init_mem_data.wcdma_init[band.wcdma_band].cal_data = TRUE;
        }
      }
      else
      {
    ...
    case RFM_SVDO_MODE:
      if (band.cdma_band < RFM_CDMA_MAX_BAND)
      {
             ...
             (rfcommon_fbrx_nv_tech_cal_data.cdma_cal_data.temp_comp_data[band.cdma_band] == NULL) )
        {
          ...
          rfcommon_fbrx_nv_tech_init_mem_data.cdma_init[band.cdma_band].cal_data = FALSE;
          ret_val = FALSE;
        }
        else
        {
          rfcommon_fbrx_nv_tech_init_mem_data.cdma_init[band.cdma_band].cal_data = TRUE;
        }
      }
      else
      {
    ...
    case RFM_LTE_FTM_MODE:
      ...
      {
        /* Allocate memory to load fbrx gain cal data */
        if (rfcommon_fbrx_nv_tech_cal_data.lte_cal_data[c_val].gain_data[band.lte_band] == NULL)
        {
          rfcommon_fbrx_nv_tech_cal_data.lte_cal_data[c_val].gain_data[band.lte_band] = 
	  #ifndef FEATURE_CLADE_HEAP		  	
          modem_mem_calloc(1,sizeof(rfcommon_fbrx_nv_gain_vs_freq_type_v2), MODEM_MEM_CLIENT_RFA);
	  #else
          clade_heap_calloc(1,sizeof(rfcommon_fbrx_nv_gain_vs_freq_type_v2));
        ...
        /* Allocate memory to load fbrx sp cal data */
        if (rfcommon_fbrx_nv_tech_cal_data.lte_cal_data[c_val].sp_data[band.lte_band] == NULL)
        {
          rfcommon_fbrx_nv_tech_cal_data.lte_cal_data[c_val].sp_data[band.lte_band] = 
          #ifndef FEATURE_CLADE_HEAP		  			  	
          modem_mem_calloc(1,sizeof(rfcommon_fbrx_nv_attn_switch_pt_type), MODEM_MEM_CLIENT_RFA);
	  #else
          clade_heap_calloc(1,sizeof(rfcommon_fbrx_nv_attn_switch_pt_type));			
        ...
        /* Allocate memory to load fbrx temp comp cal data */
        if (rfcommon_fbrx_nv_tech_cal_data.lte_cal_data[c_val].temp_comp_data[band.lte_band] == NULL)
        {
          rfcommon_fbrx_nv_tech_cal_data.lte_cal_data[c_val].temp_comp_data[band.lte_band] = 
	  #ifndef FEATURE_CLADE_HEAP			  	
          modem_mem_calloc(1,sizeof(rfcommon_fbrx_nv_temp_type), MODEM_MEM_CLIENT_RFA);
	  #else
          clade_heap_calloc(1,sizeof(rfcommon_fbrx_nv_temp_type));
        ...
        if (rfcommon_fbrx_nv_tech_cal_data.lte_cal_data[c_val].droop_data[band.lte_band] == NULL)
        {
          rfcommon_fbrx_nv_tech_cal_data.lte_cal_data[c_val].droop_data[band.lte_band] = 
	  #ifndef FEATURE_CLADE_HEAP		  	
          modem_mem_calloc(1,sizeof(rfcommon_fbrx_nv_lte_band_droop_params_type), MODEM_MEM_CLIENT_RFA);
	  #else
          clade_heap_calloc(1,sizeof(rfcommon_fbrx_nv_lte_band_droop_params_type));
	  #endif
        }

        /* Check if memory was successfully allocated or not to store FBRx NV data */
        if ( (rfcommon_fbrx_nv_tech_cal_data.lte_cal_data[c_val].gain_data[band.lte_band] == NULL) || 
             (rfcommon_fbrx_nv_tech_cal_data.lte_cal_data[c_val].sp_data[band.lte_band] == NULL) || 
             (rfcommon_fbrx_nv_tech_cal_data.lte_cal_data[c_val].temp_comp_data[band.lte_band] == NULL) ||
             (rfcommon_fbrx_nv_tech_cal_data.lte_cal_data[c_val].droop_data[band.lte_band] == NULL) )
        {
          RF_MSG_2( RF_FATAL, "rfcommon_fbrx_nv_allocate_mem: Memory Allocation failed" 
                    "for C%d NV FBRX data for LTE band:%d", c_val, band.lte_band );          
          rfcommon_fbrx_nv_tech_init_mem_data.lte_init[c_val][band.lte_band].cal_data = FALSE;
          ret_val = FALSE;
        }
        else
        {
          rfcommon_fbrx_nv_tech_init_mem_data.lte_init[c_val][band.lte_band].cal_data = TRUE;
        }
      }
      else
      {
    ...
    case RFM_TDSCDMA_MODE_FTM:
      if (band.tdscdma_band < RFCOM_NUM_TDSCDMA_BANDS)
      {
             ...
             (rfcommon_fbrx_nv_tech_cal_data.tdscdma_cal_data.temp_comp_data[band.tdscdma_band] == NULL) )
        {
          ...
          rfcommon_fbrx_nv_tech_init_mem_data.tds_init[band.tdscdma_band].cal_data = FALSE;
          ret_val = FALSE;
        }
        else
        {
          rfcommon_fbrx_nv_tech_init_mem_data.tds_init[band.tdscdma_band].cal_data = TRUE;
        }
      }
      else
      {
...
/*!
  @brief
  Generic function to allocate memory for global fbrx
  struct to strore FBRX DC Cal NV data 
...
boolean 
rfcommon_fbrx_nv_allocate_dc_mem ( rfm_mode_enum_type tech,
                                   rfcom_band_type_u band,
                                   rfcommon_fbrx_nv_container_type c_val  )
{
  ...
  switch (tech)
  {
    case RFM_IMT_MODE:
      if (band.wcdma_band < RFCOM_NUM_WCDMA_BANDS)
      {
        ...
        if ( rfcommon_fbrx_nv_tech_cal_data.wcdma_cal_data.dc_data[band.wcdma_band] == NULL )
        {
          RF_MSG_1( RF_FATAL, "rfcommon_fbrx_nv_allocate_dc_mem: Memory Allocation failed" 
                    "for FBRX DC data for WCDMA band:%d", band.wcdma_band);
          ret_val = FALSE;
          rfcommon_fbrx_nv_tech_init_mem_data.wcdma_init[band.wcdma_band].dc_data = FALSE;
        }
        else
        {
          rfcommon_fbrx_nv_tech_init_mem_data.wcdma_init[band.wcdma_band].dc_data = TRUE;
        }
      }
      else
      {
        RF_MSG_1(RF_ERROR, "rfcommon_fbrx_nv_allocate_dc_mem: Invalid WCDMA Band = %d ", band.wcdma_band);
        ret_val = FALSE;
      }
      break;
    ...
    case RFM_SVDO_MODE:
      if (band.cdma_band < RFM_CDMA_MAX_BAND)
      {
        ...
        {
          RF_MSG_1( RF_FATAL, "rfcommon_fbrx_nv_allocate_dc_mem: Memory Allocation failed" 
                    "for FBRX DC data for CDMA band:%d", band.cdma_band );
          ret_val = FALSE;
          rfcommon_fbrx_nv_tech_init_mem_data.cdma_init[band.cdma_band].dc_data = FALSE;
        }
        else
        {
          rfcommon_fbrx_nv_tech_init_mem_data.cdma_init[band.cdma_band].dc_data = TRUE;
        }
      }
      else
      {
        RF_MSG_1(RF_ERROR, "rfcommon_fbrx_nv_allocate_dc_mem: Invalid CDMA Band = %d ", band.cdma_band);
        ret_val = FALSE;
      }
      break;
    ...
    case RFM_LTE_FTM_MODE:
      ...
      {
        /* Allocate memory to load fbrx dc cal data */
        if (rfcommon_fbrx_nv_tech_cal_data.lte_cal_data[c_val].dc_data[band.lte_band] == NULL)
        {
          rfcommon_fbrx_nv_tech_cal_data.lte_cal_data[c_val].dc_data[band.lte_band] = 
            modem_mem_calloc(1,sizeof(rfcommon_fbrx_nv_dc_type), MODEM_MEM_CLIENT_RFA);
        }
        ...
        if ( rfcommon_fbrx_nv_tech_cal_data.lte_cal_data[c_val].dc_data[band.lte_band] == NULL)              
        {
          RF_MSG_2( RF_FATAL, "rfcommon_fbrx_nv_allocate_dc_mem: Memory Allocation failed" 
                    "for C%d FBRX DC data for LTE band:%d", c_val, band.lte_band );
          rfcommon_fbrx_nv_tech_init_mem_data.lte_init[c_val][band.lte_band].dc_data = FALSE;
          ret_val = FALSE;
        }
        else
        {
          rfcommon_fbrx_nv_tech_init_mem_data.lte_init[c_val][band.lte_band].dc_data = TRUE;
        }
      }
      else
      {
        RF_MSG_1(RF_ERROR, "rfcommon_fbrx_nv_allocate_dc_mem: Invalid LTE Band = %d ", band.lte_band);
        ret_val = FALSE;
      }
      break;
    ...
    case RFM_TDSCDMA_MODE_FTM:
      if (band.tdscdma_band < RFCOM_NUM_TDSCDMA_BANDS)
      {
        ...
        {
          RF_MSG_1( RF_FATAL, "rfcommon_fbrx_nv_allocate_dc_mem: Memory Allocation failed" 
                    "for FBRX DC data for TDS band:%d", band.tdscdma_band );
          ret_val = FALSE;
          rfcommon_fbrx_nv_tech_init_mem_data.tds_init[band.tdscdma_band].dc_data = FALSE;
        }
        else
        {
          rfcommon_fbrx_nv_tech_init_mem_data.tds_init[band.tdscdma_band].dc_data = TRUE;
        }
      }
      else
      {
        RF_MSG_1(RF_ERROR, "rfcommon_fbrx_nv_allocate_dc_mem: Invalid TDSCDMA Band = %d ", band.tdscdma_band);
        ret_val = FALSE;
      }
      break;

    default:
      RF_MSG_1(RF_ERROR, "rfcommon_fbrx_nv_allocate_dc_mem: Invalid Tech = %d ", tech);
      ret_val = FALSE;
      break;
  } /* switch case*/
...
} /* rfcommon_fbrx_nv_allocate_dc_mem() */ 
                                  ...
                                  rfcommon_fbrx_nv_container_type c_val  )
{
  ...
  switch (tech)
  {
	...
	#else
	clade_heap_free((void*)rfcommon_fbrx_nv_tech_cal_data.wcdma_cal_data.gain_data[band.wcdma_band]);
	#endif
	...
	#else
	clade_heap_free((void*)rfcommon_fbrx_nv_tech_cal_data.wcdma_cal_data.sp_data[band.wcdma_band]);
	#endif
        rfcommon_fbrx_nv_tech_cal_data.wcdma_cal_data.sp_data[band.wcdma_band] = NULL;
	#ifndef FEATURE_CLADE_HEAP
        modem_mem_free((void*)rfcommon_fbrx_nv_tech_cal_data.wcdma_cal_data.temp_comp_data[band.wcdma_band], MODEM_MEM_CLIENT_RFA);
	#else
	clade_heap_free((void*)rfcommon_fbrx_nv_tech_cal_data.wcdma_cal_data.temp_comp_data[band.wcdma_band]);
	#endif
        rfcommon_fbrx_nv_tech_cal_data.wcdma_cal_data.temp_comp_data[band.wcdma_band] = NULL;
	...
	#else
	clade_heap_free((void*)rfcommon_fbrx_nv_tech_cal_data.cdma_cal_data.gain_data[band.cdma_band]);
	#endif
	...
	#else
	clade_heap_free((void*)rfcommon_fbrx_nv_tech_cal_data.cdma_cal_data.sp_data[band.cdma_band]);
	#endif
        rfcommon_fbrx_nv_tech_cal_data.cdma_cal_data.sp_data[band.cdma_band] = NULL;
	...
	#else
	clade_heap_free((void*)rfcommon_fbrx_nv_tech_cal_data.cdma_cal_data.temp_comp_data[band.cdma_band]);
	#endif
        rfcommon_fbrx_nv_tech_cal_data.cdma_cal_data.temp_comp_data[band.cdma_band] = NULL;
    ...
    case RFM_LTE_FTM_MODE:
      if (band.lte_band < RFCOM_NUM_LTE_BANDS)
      {
        /* Do clean up */
	#ifndef FEATURE_CLADE_HEAP
        modem_mem_free((void*)rfcommon_fbrx_nv_tech_cal_data.lte_cal_data[c_val].gain_data[band.lte_band], MODEM_MEM_CLIENT_RFA);
	#else
	clade_heap_free((void*)rfcommon_fbrx_nv_tech_cal_data.lte_cal_data[c_val].gain_data[band.lte_band]);
	#endif
        rfcommon_fbrx_nv_tech_cal_data.lte_cal_data[c_val].gain_data[band.lte_band] = NULL;
		
	#ifndef FEATURE_CLADE_HEAP
        modem_mem_free((void*)rfcommon_fbrx_nv_tech_cal_data.lte_cal_data[c_val].sp_data[band.lte_band], MODEM_MEM_CLIENT_RFA);
	#else
	clade_heap_free((void*)rfcommon_fbrx_nv_tech_cal_data.lte_cal_data[c_val].sp_data[band.lte_band]);
	#endif
        rfcommon_fbrx_nv_tech_cal_data.lte_cal_data[c_val].sp_data[band.lte_band] = NULL;
		
        #ifndef FEATURE_CLADE_HEAP
        modem_mem_free((void*)rfcommon_fbrx_nv_tech_cal_data.lte_cal_data[c_val].temp_comp_data[band.lte_band], MODEM_MEM_CLIENT_RFA);
	#else
	clade_heap_free((void*)rfcommon_fbrx_nv_tech_cal_data.lte_cal_data[c_val].temp_comp_data[band.lte_band]);
	#endif
        rfcommon_fbrx_nv_tech_cal_data.lte_cal_data[c_val].temp_comp_data[band.lte_band] = NULL;
        ...
        {
          #ifndef FEATURE_CLADE_HEAP
          modem_mem_free((void *)rfcommon_fbrx_nv_tech_cal_data.lte_cal_data[c_val].droop_data[band.lte_band], MODEM_MEM_CLIENT_RFA); 
          #else
          clade_heap_free((void *)rfcommon_fbrx_nv_tech_cal_data.lte_cal_data[c_val].droop_data[band.lte_band]); 
	  #endif
          rfcommon_fbrx_nv_tech_cal_data.lte_cal_data[c_val].droop_data[band.lte_band] = NULL;
        }
      }
	...
	#else
	clade_heap_free((void*)rfcommon_fbrx_nv_tech_cal_data.tdscdma_cal_data.gain_data[band.tdscdma_band]);
	#endif 		
	...
	#else
	clade_heap_free((void*)rfcommon_fbrx_nv_tech_cal_data.tdscdma_cal_data.sp_data[band.tdscdma_band]);
	#endif		
        rfcommon_fbrx_nv_tech_cal_data.tdscdma_cal_data.sp_data[band.tdscdma_band] = NULL;
	...
	#else
	clade_heap_free((void*)rfcommon_fbrx_nv_tech_cal_data.tdscdma_cal_data.temp_comp_data[band.tdscdma_band]);
	#endif
...
/*!
  @brief
  Generic function to de-allocate memory for global fbrx
  struct which strores FBRX DC Cal NV data 
...
boolean 
rfcommon_fbrx_nv_deallocate_dc_mem ( rfm_mode_enum_type tech,
                                     rfcom_band_type_u band,
                                     rfcommon_fbrx_nv_container_type c_val  )
{
  ...
  switch (tech)
  {
    case RFM_IMT_MODE:
      ...
      {
        RF_MSG_1(RF_ERROR, "rfcommon_fbrx_nv_deallocate_dc_mem: Invalid WCDMA Band = %d ", band.wcdma_band);
        ret_val = FALSE;
      }
      break;
    ...
    case RFM_LTE_FTM_MODE:
      ...
      {
        /* Do clean up */
        modem_mem_free((void*)rfcommon_fbrx_nv_tech_cal_data.lte_cal_data[c_val].dc_data[band.lte_band], MODEM_MEM_CLIENT_RFA);
        rfcommon_fbrx_nv_tech_cal_data.lte_cal_data[c_val].dc_data[band.lte_band] = NULL;
        rfcommon_fbrx_nv_tech_init_mem_data.lte_init[c_val][band.lte_band].dc_data = FALSE;
      }
      else
      {
        RF_MSG_1(RF_ERROR, "rfcommon_fbrx_nv_deallocate_dc_mem: Invalid LTE Band = %d ", band.lte_band);
        ret_val = FALSE;
      }
      break;
    ...
    case RFM_TDSCDMA_MODE_FTM:
      ...
      {
        RF_MSG_1(RF_ERROR, "rfcommon_fbrx_nv_deallocate_dc_mem: Invalid TDSCDMA Band = %d ", band.tdscdma_band);
        ret_val = FALSE;
      }
      break;
...
} /* rfcommon_fbrx_nv_deallocate_dc_mem() */ 
                                      ...
                                      void (* task_wait_func_ptr)( rex_sigs_type ) )
{
  ...
  if (nv_data_ptr == NULL)
  {
    RF_MSG_1( RF_ERROR, "rfcommon_fbrx_nv_retrieve_wcdma_data: Failed to allocate buffer memory on heap for %d bytes", nv_read_size);
    ret_val = FALSE;
    return ret_val;
  }
  ...
  for (band = RFCOM_BAND_IMT; band<RFCOM_NUM_WCDMA_BANDS; band++)
  {
    ...
    if (rfwcdma_core_is_band_supported(band))
    {
      ...
      if ( rfcommon_fbrx_get_wcdma_cal_nv_id( band, &fbrx_gain_nv_id, &fbrx_sp_nv_id, &fbrx_dc_nv_id, &fbrx_temp_nv_id ) )
      {
        ...
        /* if FBRx dc cal nv memory was already previously allocated, deallocate now first */
        if (rfcommon_fbrx_nv_tech_init_mem_data.wcdma_init[band].dc_data == TRUE)
        {
          /* Do clean up 1st */
          ret_val = rfcommon_fbrx_nv_deallocate_dc_mem(RFCOM_WCDMA_MODE, rfcom_band, 0);
        }

        if( !( rfcommon_fbrx_nv_allocate_dc_mem(RFM_IMT_MODE, rfcom_band, 0)) )
        {
          RF_MSG_1(RF_FATAL, "rfcommon_fbrx_nv_retrieve_wcdma_data: rfcommon_fbrx_nv_allocate_dc_mem failed for WCDMA band:%d", band);
          ret_val = FALSE;
          return ret_val;
        }

        /* if FBRx cal nv memory was already previously allocated, deallocate now first */
        if (rfcommon_fbrx_nv_tech_init_mem_data.wcdma_init[band].cal_data == TRUE)
        {
          /* Do clean up 1st */
          ret_val = rfcommon_fbrx_nv_deallocate_mem(RFCOM_WCDMA_MODE, rfcom_band, 0);
        }

        nv_read_size = sizeof(rfnv_item_type);
        fbrx_gain_nv_read_status = rfcommon_rfnv_get_item( fbrx_gain_nv_id,
                                                            nv_data_ptr,
                                                            &nv_read_size,
                                                            caller_tcb_ptr,
        ...
        /* If FBRx Gain cal NV data has been successfully retrieved proceed to populating the data structure */
        if ( fbrx_gain_nv_read_status == NV_DONE_S )
        {
          /* If memory was successfully allocated to store FBRx NV data */
          if ( rfcommon_fbrx_nv_allocate_mem(RFM_IMT_MODE, rfcom_band, 0) )
          {
            ...
            if ( rfcommon_fbrx_get_wcdma_cal_compressed_nv_id( band, &fbrx_temp_nv_id_z ) ) 
            {
          ...
          } /* end if rfcommon_fbrx_nv_allocate_mem(RFM_IMT_MODE, rfcom_band, 0) */
          else
          {
            RF_MSG_1(RF_FATAL, "rfcommon_fbrx_nv_retrieve_wcdma_data: rfcommon_fbrx_nv_allocate_mem failed for WCDMA band:%d", band);
            ret_val = FALSE;
            return ret_val;
          }
        }/* end if fbrx_gain_nv_read_status == NV_DONE_S */
        ...
        {
          RF_MSG_3(RF_HIGH, "rfcommon_fbrx_nv_retrieve_wcdma_data: rfcommon_rfnv_get_item failed with error status = %d, nv_id = %d for WCDMA band:%d",  
                              fbrx_gain_nv_read_status, fbrx_gain_nv_id, band); 
        }

      } /* end if  rfcommon_fbrx_get_wcdma_cal_nv_id( band, &fbrx_gain_nv_id, &fbrx_sp_nv_id, &fbrx_dc_nv_id, &fbrx_temp_nv_id ) */ 
      else
      {
        RF_MSG_1(RF_HIGH, "rfcommon_fbrx_nv_retrieve_wcdma_data: FBRX Cal NV not supported for WCDMA band:%d", band); 
      }   
    }/* if (rfwcdma_core_is_band_supported(band,RF_PATH_0)) */
    else
    {
      RF_MSG_1(RF_HIGH, "rfcommon_fbrx_nv_retrieve_wcdma_data: WCDMA band :%d not supported for current RF card", band); 
    }               
                                   ...
                                   void (* task_wait_func_ptr)( rex_sigs_type ))
{
  ...
  if (nv_data_ptr == NULL)
  {
    RF_MSG_1( RF_ERROR, "rfcommon_fbrx_nv_retrieve_1x_data: Failed to allocate buffer memory on heap for %d bytes", nv_read_size);
    ret_val = FALSE;
    return ret_val;
  }
  ...
  {
    if ( rf_cdma_is_band_supported_on_any_device (band) )
    {
      ...
      if ( rfcommon_fbrx_get_1x_cal_nv_id( band, &fbrx_gain_nv_id, &fbrx_sp_nv_id, &fbrx_dc_nv_id, &fbrx_temp_nv_id ) )
      {
        ...
        /* if FBRx dc cal nv memory was already previously allocated, deallocate now first */
        if (rfcommon_fbrx_nv_tech_init_mem_data.cdma_init[band].dc_data == TRUE)
        {
          /* Do clean up 1st */
          ret_val = rfcommon_fbrx_nv_deallocate_dc_mem(RFCOM_1X_MODE, rfcom_band, 0);
        }

        if( !(rfcommon_fbrx_nv_allocate_dc_mem(RFCOM_1X_MODE, rfcom_band, 0)) )
        {
          RF_MSG_1(RF_FATAL, "rfcommon_fbrx_nv_retrieve_1x_data: rfcommon_fbrx_nv_allocate_dc_mem failed for CDMA band:%d", band);
          ret_val = FALSE;
          return ret_val;
        }

        /* if FBRx cal nv memory was already previously allocated, deallocate now first */
        if (rfcommon_fbrx_nv_tech_init_mem_data.cdma_init[rfcom_band.cdma_band].cal_data == TRUE)
        {
          /* Do clean up 1st */
          ret_val = rfcommon_fbrx_nv_deallocate_mem(RFCOM_1X_MODE, rfcom_band, 0);
        }
        ...
        {
          /* If memory was successfully allocated to store FBRx NV data */
          if ( rfcommon_fbrx_nv_allocate_mem(RFCOM_1X_MODE, rfcom_band, 0) )
          {
            ...
            if (api_status != RFCOMMON_FBRX_SUCCESS )
            {
              RF_MSG_1(RF_FATAL, "rfcommon_fbrx_nv_retrieve_1x_data: Load fbrx dc data from NV failed for CDMA band:%d", band);
              ret_val = FALSE;
              return ret_val;
            }

            if ( rfcommon_fbrx_get_1x_cal_compressed_nv_id( band, &fbrx_temp_nv_id_z ) ) 
            {
          ...
          {
            RF_MSG_1(RF_FATAL, "rfcommon_fbrx_nv_retrieve_1x_data: rfcommon_fbrx_nv_allocate_mem failed for CDMA band:%d", band);
            ret_val = FALSE;
            return ret_val;
          }
        }/* end if fbrx_gain_nv_read_status == NV_DONE_S */
        ...
        {
          RF_MSG_3(RF_HIGH, "rfcommon_fbrx_nv_retrieve_1x_data: rfcommon_rfnv_get_item failed with error status = %d, nv_id = %d for CDMA band:%d",  
                              fbrx_gain_nv_read_status, fbrx_gain_nv_id, band); 
        }

      } /* end if rfcommon_fbrx_get_1x_cal_nv_id( band, &fbrx_gain_nv_id, &fbrx_sp_nv_id, &fbrx_dc_nv_id, &fbrx_temp_nv_id ) */ 
      else
      {
        RF_MSG_1(RF_HIGH, "rfcommon_fbrx_nv_retrieve_1x_data: FBRX Cal NV not supported for CDMA band:%d", band ); 
      }   
    }/* end if rf_cdma_is_band_supported (device, band) */
    else
    {
      RF_MSG_1(RF_HIGH, "rfcommon_fbrx_nv_retrieve_1x_data: CDMA band :%d not supported for current RF card", band ); 
    }  
  } /* end for (band = RFM_CDMA_BC0; band < RFM_CDMA_MAX_BAND; band++) */
...
/* Helper API to retrieve the LTE (band, BW) specific Droop NV data */
void rfcommon_fbrx_nv_retrieve_lte_band_droop_data
(  
  rex_tcb_type* caller_tcb_ptr,
  rex_sigs_type task_nv_wait_sig,
  void (* task_wait_func_ptr)( rex_sigs_type ),
...
)
{
  ...
  /* Get FBRX band specific droop data from NV */
  if ( (band<RFCOM_NUM_LTE_BANDS) && (c_val < RFCOMMON_FBRX_NV_CONTAINER_MAX) && (nv_data_ptr != NULL) )
  {
    if ( rfcommon_fbrx_get_lte_band_bw_droop_compressed_nv_id( band, &fbrx_droop_nv_id_z, c_val ) )
    {
      /* Allocate memory to load LTE <band,BW> specific droop data */
      if (rfcommon_fbrx_nv_tech_cal_data.lte_cal_data[c_val].droop_data[band] == NULL)
      {
        rfcommon_fbrx_nv_tech_cal_data.lte_cal_data[c_val].droop_data[band] = 
        #ifndef FEATURE_CLADE_HEAP
          modem_mem_calloc(1,sizeof(rfcommon_fbrx_nv_lte_band_droop_params_type), MODEM_MEM_CLIENT_RFA);
        #else
          clade_heap_calloc(1,sizeof(rfcommon_fbrx_nv_lte_band_droop_params_type));
      ...
      if ( rfcommon_fbrx_nv_tech_cal_data.lte_cal_data[c_val].droop_data[band] == NULL )
      {
        RF_MSG_2( RF_FATAL, "rfcommon_fbrx_nv_retrieve_lte_band_droop_data: Memory Allocation failed" 
                  "for C%d NV FBRX data for LTE band:%d", c_val, band );          
        rfcommon_fbrx_nv_tech_init_mem_data.lte_init[c_val][band].droop_data = FALSE;
      }
      else
      {
        rfcommon_fbrx_nv_tech_init_mem_data.lte_init[c_val][band].droop_data = TRUE;

        /* Load the fbrx lte band specific droop data from compressed NV to struct for current LTE Band */
        api_status = rfcommon_fbrx_nv_load_lte_band_droop_params( rfcommon_fbrx_nv_tech_cal_data.lte_cal_data[c_val].droop_data[band], 
                                                                  fbrx_droop_nv_id_z, 
                                                                  caller_tcb_ptr, 
                                                                  task_nv_wait_sig, 
                                                                  task_wait_func_ptr );
        ...
        if (api_status != RFCOMMON_FBRX_SUCCESS )
        {
          RF_MSG_2(RF_HIGH, "rfcommon_fbrx_nv_retrieve_lte_data: Load fbrx LTE band specific droop params from"
                             " NV failed for LTE band: %d, NV container: C%d", band, c_val);
          ...
          #ifndef FEATURE_CLADE_HEAP   
          modem_mem_free( (void*)rfcommon_fbrx_nv_tech_cal_data.lte_cal_data[c_val].droop_data[band], MODEM_MEM_CLIENT_RFA);
          #else
          clade_heap_free( (void*)rfcommon_fbrx_nv_tech_cal_data.lte_cal_data[c_val].droop_data[band]);
          #endif      
          rfcommon_fbrx_nv_tech_cal_data.lte_cal_data[c_val].droop_data[band] = NULL;
          rfcommon_fbrx_nv_tech_init_mem_data.lte_init[c_val][band].droop_data = FALSE;
        }
      }
    }
    ...
    {
      RF_MSG_2(RF_ERROR, "rfcommon_fbrx_get_lte_band_bw_droop_compressed_nv_id:"
                         "failed for LTE band: %d, NV container: C%d", band, c_val);
    }
  }
} /* rfcommon_fbrx_nv_retrieve_lte_band_droop_data */
                                    ...
                                    void (* task_wait_func_ptr)( rex_sigs_type ))
{
  ...
  boolean ret_val = TRUE;
  rfnv_item_type *nv_data_ptr = NULL;
  rfcommon_fbrx_nv_container_type c_val = 0; 
  ...
  if (nv_data_ptr == NULL)
  {
    RF_MSG_1( RF_ERROR, "rfcommon_fbrx_nv_retrieve_lte_data: Failed to allocate buffer memory on heap for %d bytes", nv_read_size);
    ret_val = FALSE;
    return ret_val;
  }
  ...
  for (band = RFCOM_BAND_LTE_B1; band<RFCOM_NUM_LTE_BANDS; band++)
  {
    ...
    for (c_val = RFCOMMON_FBRX_NV_CONTAINER_C0; c_val < RFCOMMON_FBRX_NV_CONTAINER_MAX; c_val++  ) 
    {
      if (rflte_nv_get_tx_band_support(band) == TRUE) 
      {
        if ( rfcommon_fbrx_get_lte_cal_nv_id( band, &fbrx_gain_nv_id, &fbrx_sp_nv_id, &fbrx_dc_nv_id, &fbrx_temp_nv_id, c_val ) )
        {

          /* if FBRx dc cal nv memory was already previously allocated, deallocate now first */
          if (rfcommon_fbrx_nv_tech_init_mem_data.lte_init[c_val][band].dc_data == TRUE)
          {
            /* Do clean up 1st */
            ret_val = rfcommon_fbrx_nv_deallocate_dc_mem(RFCOM_LTE_MODE, rfcom_band, c_val);
          }

          if( !(rfcommon_fbrx_nv_allocate_dc_mem(RFCOM_LTE_MODE, rfcom_band, c_val)) )
          {
            RF_MSG_2(RF_FATAL, "rfcommon_fbrx_nv_retrieve_lte_data: rfcommon_fbrx_nv_allocate_dc_mem failed for LTE band: %d, NV container: C%d", band, c_val);
            ret_val = FALSE;
            return ret_val;
          }

          /* if FBRx cal nv memory was already previously allocated, deallocate now first */
          if (rfcommon_fbrx_nv_tech_init_mem_data.lte_init[c_val][rfcom_band.lte_band].cal_data == TRUE)
          {
            ret_val = rfcommon_fbrx_nv_deallocate_mem(RFCOM_LTE_MODE, rfcom_band, c_val);
          }
          ...
          if ( fbrx_gain_nv_read_status == NV_DONE_S )
          {
            ...
            /* If memory was successfully allocated to store FBRx NV data */
            if ( rfcommon_fbrx_nv_allocate_mem(RFCOM_LTE_MODE, rfcom_band, c_val) )
            {
              /**********************************************************************************************************
                Step 1: Copy fbrx gain, switch point, DC, Temp Comp NV items for current supported LTE Bands 
              ***********************************************************************************************************/

              /* Load the fbrx gain data from NV to struct for current LTE Band */
              api_status = rfcommon_fbrx_nv_load_gain( rfcommon_fbrx_nv_tech_cal_data.lte_cal_data[c_val].gain_data[band], 
                                                       fbrx_gain_nv_id, 
                                                       caller_tcb_ptr, 
                                                       task_nv_wait_sig, 
                                                       task_wait_func_ptr );
              ...
              if (api_status != RFCOMMON_FBRX_SUCCESS )
              {
                RF_MSG_2(RF_FATAL, "rfcommon_fbrx_nv_retrieve_lte_data: Load fbrx gain data from NV failed for LTE band: %d, NV container: C%d", band, c_val);
                ret_val = FALSE;
                return ret_val;
              }

              /* Load the fbrx switch point data from NV to struct for current LTE Band */
              api_status = rfcommon_fbrx_nv_load_attn_switch_pts( rfcommon_fbrx_nv_tech_cal_data.lte_cal_data[c_val].sp_data[band], 
                                                                  fbrx_sp_nv_id, 
                                                                  caller_tcb_ptr, 
                                                                  task_nv_wait_sig, 
                                                                  task_wait_func_ptr );
              if (api_status != RFCOMMON_FBRX_SUCCESS )
              {
                RF_MSG_2(RF_FATAL, "rfcommon_fbrx_nv_retrieve_lte_data: Load fbrx switchpoint data from NV failed for LTE band: %d, NV container: C%d", band, c_val);
                ret_val = FALSE;
                return ret_val;
              }

              /* Load the fbrx dc data from NV to struct for current LTE Band */
              api_status = rfcommon_fbrx_nv_load_dc( rfcommon_fbrx_nv_tech_cal_data.lte_cal_data[c_val].dc_data[band], 
                                                     fbrx_dc_nv_id, 
                                                     caller_tcb_ptr, 
                                                     task_nv_wait_sig, 
                                                     task_wait_func_ptr );
              ...
              if (api_status != RFCOMMON_FBRX_SUCCESS )
              {
                RF_MSG_2(RF_FATAL, "rfcommon_fbrx_nv_retrieve_lte_data: Load fbrx dc data from NV failed for LTE band: %d, NV container: C%d", band, c_val);
                ret_val = FALSE;
                return ret_val;
              }

              if ( rfcommon_fbrx_get_lte_cal_compressed_nv_id( band, &fbrx_temp_nv_id_z, c_val ) ) 
              {
                /* Load the fbrx temp comp data from compressed NV to struct for current LTE Band */
                api_status = rfcommon_fbrx_nv_load_temp( rfcommon_fbrx_nv_tech_cal_data.lte_cal_data[c_val].temp_comp_data[band], 
                                                         fbrx_temp_nv_id_z, 
                                                         caller_tcb_ptr, 
                                                         task_nv_wait_sig, 
                                                         task_wait_func_ptr );
              ...
              {
                /* It should never come here */
                RF_MSG_2(RF_FATAL, "rfcommon_fbrx_nv_retrieve_lte_data: Load fbrx compressed NV "
                                   "failed for LTE band: %d, NV container: C%d", band, c_val);
                ret_val = FALSE;
                return ret_val;
              }
              ...
              if (api_status != RFCOMMON_FBRX_SUCCESS )
              {
                RF_MSG_2(RF_FATAL, "rfcommon_fbrx_nv_retrieve_lte_data: Load fbrx temp comp data from NV failed for LTE band: %d, NV container: C%d", band, c_val);
                ret_val = FALSE;
                return ret_val;
              }
          
              /* Allocate memory for LTE band specific droop data and update NV data */
              rfcommon_fbrx_nv_retrieve_lte_band_droop_data(caller_tcb_ptr,
                                                            task_nv_wait_sig,
                                                            task_wait_func_ptr,
                                                            band,
                                                            nv_data_ptr,
            ...
            } /* end if rfcommon_fbrx_nv_allocate_mem(RFCOM_LTE_MODE, rfcom_band, c_val) */
            else
            {
              RF_MSG_2(RF_FATAL, "rfcommon_fbrx_nv_retrieve_lte_data: rfcommon_fbrx_nv_allocate_mem failed for LTE band: %d, NV container: C%d", band, c_val);
              ret_val = FALSE;
              return ret_val;
            }
          ...
          {
             RF_MSG_4(RF_HIGH, "rfcommon_fbrx_nv_retrieve_lte_data: rfcommon_rfnv_get_item failed with error status = %d, nv_id = %d for LTE band: %d, NV Container: C%d",  
                                 fbrx_gain_nv_read_status, fbrx_gain_nv_id, band, c_val); 
          }
       } /* end if rfcommon_fbrx_get_lte_cal_nv_id( band, &fbrx_gain_nv_id, &fbrx_sp_nv_id, &fbrx_dc_nv_id, &fbrx_temp_nv_id, c_val ) */
       else
       {
         RF_MSG_2(RF_HIGH, "rfcommon_fbrx_nv_retrieve_lte_data: FBRX Cal NV not supported for LTE band: %d, NV Container: C%d", band, c_val);
       }
     }/* if band supported */ 
     else
     {
       RF_MSG_2(RF_HIGH, "rfcommon_fbrx_nv_retrieve_lte_data: LTE band :%d, NV Container: C%d not supported for current RF card", band, c_val); 
     }     
   } /* end for c_val loop */       
 } /* for (band = RFCOM_BAND_LTE_B1; band<RFCOM_NUM_LTE_BANDS; band++) */
  ...
  for (band = RFCOM_BAND_TDSCDMA_B34; band<RFCOM_NUM_TDSCDMA_BANDS; band++)
  {
   ...
   if (1) /* @todo: check for band support */
   {
     ...
     if ( rfcommon_fbrx_get_tdscdma_cal_nv_id( band, &fbrx_gain_nv_id, &fbrx_sp_nv_id, &fbrx_dc_nv_id, &fbrx_temp_nv_id) )
     {
       ...
       /* if FBRx dc cal nv memory was already previously allocated, deallocate now first */
       if (rfcommon_fbrx_nv_tech_init_mem_data.tds_init[band].dc_data == TRUE)
       {
         /* Do clean up 1st */
         ret_val = rfcommon_fbrx_nv_deallocate_dc_mem(RFCOM_TDSCDMA_MODE, rfcom_band, 0);
       }

       if( !(rfcommon_fbrx_nv_allocate_dc_mem(RFCOM_TDSCDMA_MODE, rfcom_band, 0)) )
       {
         RF_MSG_1(RF_FATAL, "rfcommon_fbrx_nv_retrieve_tdscdma_data: rfcommon_fbrx_nv_allocate_dc_mem failed for TDS band:%d", band);
         ret_val = FALSE;
         return ret_val;
       }

       /* if FBRx cal nv memory was already previously allocated, deallocate now first */
       if (rfcommon_fbrx_nv_tech_init_mem_data.tds_init[rfcom_band.tdscdma_band].cal_data == TRUE)
       {
         ret_val = rfcommon_fbrx_nv_deallocate_mem(RFCOM_TDSCDMA_MODE, rfcom_band, 0);
       }
       ...
       {
         /* If memory was successfully allocated to store FBRx NV data */
         if ( rfcommon_fbrx_nv_allocate_mem(RFCOM_TDSCDMA_MODE, rfcom_band, 0) )
         {
           ...
           /* Load the fbrx dc data from NV to struct for current TDS Band */
           ...
           if ( rfcommon_fbrx_get_tdscdma_cal_compressed_nv_id( band, &fbrx_temp_nv_id_z ) ) 
           {
         ...
         } /* rfcommon_fbrx_nv_allocate_mem(RFCOM_TDSCDMA_MODE, rfcom_band, 0) */
         else
         {
           RF_MSG_1(RF_FATAL, "rfcommon_fbrx_nv_retrieve_tdscdma_data: rfcommon_fbrx_nv_allocate_mem failed for TDS band:%d", band);
           ret_val = FALSE;
           return ret_val;
         }
       }/* end if fbrx_gain_nv_read_status == NV_DONE_S */
       ...
       {
         RF_MSG_3(RF_HIGH, "rfcommon_fbrx_nv_retrieve_tdscdma_data: rfcommon_rfnv_get_item failed with error status = %d, nv_id = %d for TDS band:%d",  
                             fbrx_gain_nv_read_status, fbrx_gain_nv_id, band); 
       }
     } /* end if  rfcommon_fbrx_get_tds_cal_nv_id( band, &fbrx_gain_nv_id, &fbrx_sp_nv_id, &fbrx_dc_nv_id, &fbrx_temp_nv_id ) */ 
     else
     {
       RF_MSG_1(RF_HIGH, "rfcommon_fbrx_nv_retrieve_tdscdma_data: FBRX Cal NV not supported for TDS band:%d", band); 
     }   

   }/* end if band supported */
   else
   {
     RF_MSG_1(RF_HIGH, "rfcommon_fbrx_nv_retrieve_tdscdma_data: TDS band :%d not supported for current RF card", band); 
   }               
  } /* end for (band = RFCOM_BAND_TDSCDMA_B34; band<RFCOM_NUM_TDSCDMA_BANDS; band++) */
                                         ...
                                         void (* task_wait_func_ptr)( rex_sigs_type ))
{
  ...
  rfnv_item_id_enum_type fbrx_increase_sample_size_nv_id = RFNV_COMMON_FBRX_INCREASE_SAMPLE_SIZE_CONTROL_I;
  ...
  /* Allocate memory for raw NV data */
  nv_data_ptr = (rfnv_item_type *)modem_mem_calloc(1, 
  ...
  {
    RF_MSG( RF_ERROR, 
            "rfcommon_fbrx_nv_retrieve_control_data: Failed to allocate buffer memory on heap");
    ret_val = FALSE;
    return ret_val;
  }
  ...
  {
    memscpy( &rfcommon_fbrx_sys_control_nv_data, 
             sizeof(int32), (void*)nv_data_ptr, sizeof(int32) );
  } 
  else if ( fbrx_nv_read_status == NV_NOTACTIVE_S )
  {
    ...
    RF_MSG_1( RF_ERROR, 
              "rfcommon_fbrx_nv_retrieve_control_data: rfcommon_rfnv_get_item NOT ACTIVE "
                        "for NV = %d", fbrx_control_nv_id);	
  }
  else
  {
    RF_MSG_1( RF_ERROR, 
              "rfcommon_fbrx_nv_retrieve_control_data: rfcommon_rfnv_get_item failed for NV=%d", 
              fbrx_control_nv_id);	
    ret_val = FALSE;
  }

  /* Reset the NV data local buffer */
  memset ((void*)nv_data_ptr, 0, sizeof(int32));

  /* Retreive the Sample Size Increased or Not NV */
  fbrx_nv_read_status = rfcommon_rfnv_get_item( fbrx_increase_sample_size_nv_id,
                                                nv_data_ptr,
                                                &nv_item_size,
                                                caller_tcb_ptr,
                                                task_nv_wait_sig,
  ...
  if ( fbrx_nv_read_status == NV_DONE_S )
  {
    memscpy( &rfcommon_fbrx_increase_sample_size, sizeof(int32), (void*)nv_data_ptr, sizeof(int32) );
    RF_MSG_2( RF_HIGH, "rfcommon_fbrx_nv_retrieve_control_data: rfcommon_rfnv_get_item ACTIVE "
                        "for NV = %d and value:%d", fbrx_increase_sample_size_nv_id,
                        rfcommon_fbrx_increase_sample_size);

  } 
  else if ( fbrx_nv_read_status == NV_NOTACTIVE_S )
  {
    /* Default NV data to 0 */
    rfcommon_fbrx_increase_sample_size = 0;
    RF_MSG_1( RF_ERROR, "rfcommon_fbrx_nv_retrieve_control_data: rfcommon_rfnv_get_item NOT ACTIVE "
                        "for NV = %d", fbrx_increase_sample_size_nv_id);	
  }
  else
  {
    RF_MSG_1( RF_ERROR, 
              "rfcommon_fbrx_nv_retrieve_control_data: rfcommon_rfnv_get_item failed for NV=%d", 
              fbrx_increase_sample_size_nv_id);	
    ret_val = FALSE;
  }
...
)
{
  ...
  uint8 i = 0, fbrx_gain_chan_size = 0, chan_idx = 0;
  ...
  uint8 min_cal_chan_idx=0, max_cal_chan_idx =0;
  uint16 lte_integer_band = 0;
  rfcom_lte_earfcn_type tx_earfcn = RFLTE_INVALID_EARFCN;
  ...
  switch (tech)
  {
    ...
    case RFM_IMT_MODE:
      ...
      {
        if(gain_ptr->fbrx_gain[i][mode_idx][gs_idx].data_valid == TRUE)
        {
          tx_cal_freq_list[chan_idx] = rfwcdma_core_get_freq_from_ul_arfcn(gain_ptr->fbrx_gain_chan[i]);
          rfcommmon_fbrx_mode_cal_chan_idx[mode_idx][chan_idx] = i;
          chan_idx++;
      }
      }
      fbrx_gain_chan_size = chan_idx;
      break;
    #endif /* FEATURE_WCDMA */
    ...
    case RFM_SVDO_MODE:
      ...
      for (i=0;i<gain_ptr->fbrx_gain_chan_size;i++)
      {
        if(gain_ptr->fbrx_gain[i][mode_idx][gs_idx].data_valid == TRUE)
        {
          tx_cal_freq_list[chan_idx] = rf_cdma_get_tx_carrier_freq(band.cdma_band, gain_ptr->fbrx_gain_chan[i]);
          rfcommmon_fbrx_mode_cal_chan_idx[mode_idx][chan_idx] = i;
          chan_idx++;
      }
      }
    ...
    case RFM_LTE_FTM_MODE:
      ...
      for (i=0;i<gain_ptr->fbrx_gain_chan_size;i++)
      {
        if(gain_ptr->fbrx_gain[i][mode_idx][gs_idx].data_valid == TRUE)
        {
          if ((band.lte_band == RFCOM_BAND_LTE_B66) || (band.lte_band == RFCOM_BAND_LTE_B71))
          {
            lte_integer_band = rflte_util_get_integer_band_from_rfcom_band(band.lte_band);
            tx_earfcn = rflte_util_get_absolute_tx_earfcn_from_offset(gain_ptr->fbrx_gain_chan[i],
                                                                      lte_integer_band);
          }
          else
          {
            tx_earfcn = (rfcom_lte_earfcn_type)(gain_ptr->fbrx_gain_chan[i]);
          }
          tx_cal_freq_list[chan_idx] = rflte_util_get_tx_freq_from_uarfcn(tx_earfcn, band.lte_band);
          rfcommmon_fbrx_mode_cal_chan_idx[mode_idx][chan_idx] = i;
          chan_idx++;
        }
      }
    ...
    case RFM_TDSCDMA_MODE_FTM:
      ...
      for (i=0;i<gain_ptr->fbrx_gain_chan_size;i++)
      {
         if(gain_ptr->fbrx_gain[i][mode_idx][gs_idx].data_valid == TRUE)
         {
           tx_cal_freq_list[chan_idx] = rf_tdscdma_core_util_get_freq_from_chan(gain_ptr->fbrx_gain_chan[i]);
          rfcommmon_fbrx_mode_cal_chan_idx[mode_idx][chan_idx] = i;
          chan_idx++;
      }
      }
  ...
  for (i=0;i<fbrx_gain_chan_size;i++)
  {
  ...
  if (fbrx_dm_ptr != NULL)
  {
     ...
     if( fbrx_gain_chan_size > RFLM_FBRX_NUM_CAL_CHANNELS )
     {
       RF_MSG_1(RF_ERROR, "rfcommon_fbrx_nv_get_freq_comp_gain: Invalid channel size: %d ", fbrx_gain_chan_size);
     }

     fbrx_dm_ptr->fbrx_ctl.fbrx_gain_chan_size = fbrx_gain_chan_size;
  ...
  if (tx_freq < min_freq)
  {
    ...
    min_cal_chan_idx = rfcommmon_fbrx_mode_cal_chan_idx[mode_idx][min_freq_idx];
    result.gain = gain_ptr->fbrx_gain[min_cal_chan_idx][mode_idx][gs_idx].gain;
    result.predet_index = gain_ptr->fbrx_gain[min_cal_chan_idx][mode_idx][gs_idx].predet_index;
    result.tx_pwr = gain_ptr->fbrx_gain[min_cal_chan_idx][mode_idx][gs_idx].tx_power;
    result.ubias = gain_ptr->fbrx_gain[min_cal_chan_idx][mode_idx][gs_idx].ubias;
    result.rx_scale = gain_ptr->fbrx_gain[min_cal_chan_idx][mode_idx][gs_idx].rx_scale;
  }
  else if (tx_freq > max_freq)
  {
    /* Use the highest cal point */
    max_cal_chan_idx = rfcommmon_fbrx_mode_cal_chan_idx[mode_idx][max_freq_idx];
    result.gain = gain_ptr->fbrx_gain[max_cal_chan_idx][mode_idx][gs_idx].gain;
    result.predet_index = gain_ptr->fbrx_gain[max_cal_chan_idx][mode_idx][gs_idx].predet_index;
    result.tx_pwr = gain_ptr->fbrx_gain[max_cal_chan_idx][mode_idx][gs_idx].tx_power;
    result.ubias = gain_ptr->fbrx_gain[max_cal_chan_idx][mode_idx][gs_idx].ubias;
    result.rx_scale = gain_ptr->fbrx_gain[max_cal_chan_idx][mode_idx][gs_idx].rx_scale;
  }
  else
  {
    min_cal_chan_idx = rfcommmon_fbrx_mode_cal_chan_idx[mode_idx][upper_bin];
    max_cal_chan_idx = rfcommmon_fbrx_mode_cal_chan_idx[mode_idx][lower_bin];

    /* Interpolate between the two cal points  */
    if (tx_cal_freq_list[upper_bin] - tx_cal_freq_list[lower_bin] != 0)
    {
      result.gain = ((gain_ptr->fbrx_gain[min_cal_chan_idx][mode_idx][gs_idx].gain - gain_ptr->fbrx_gain[max_cal_chan_idx][mode_idx][gs_idx].gain) *
                     (tx_freq - tx_cal_freq_list[lower_bin])) /
                       (tx_cal_freq_list[upper_bin] - tx_cal_freq_list[lower_bin]) +
                      gain_ptr->fbrx_gain[max_cal_chan_idx][mode_idx][gs_idx].gain;
      result.tx_pwr = ( (gain_ptr->fbrx_gain[min_cal_chan_idx][mode_idx][gs_idx].tx_power - gain_ptr->fbrx_gain[max_cal_chan_idx][mode_idx][gs_idx].tx_power) *
                       (tx_freq - tx_cal_freq_list[lower_bin])) /
                         (tx_cal_freq_list[upper_bin] - tx_cal_freq_list[lower_bin]) +
                        gain_ptr->fbrx_gain[max_cal_chan_idx][mode_idx][gs_idx].tx_power;
    }
    else
    {
      result.gain = gain_ptr->fbrx_gain[max_cal_chan_idx][mode_idx][gs_idx].gain;
                            ...
                            rfcommon_fbrx_nv_container_type c_val)
{
  ...
  rflm_fbrx_sp_nv_item_type sp_idx = 0;
  uint8 gain_idx = 0;
  uint8 mode_idx = 0;
  ...
  boolean fbrx_cal_data_valid = FALSE;

  if (fbrx_dm_ptr!= NULL && num_channels != 0)
  {
    /* Get the current FBRx mode */
    cur_fbrx_mode_idx = fbrx_dm_ptr->fbrx_ctl.fbrx_mode;
    if (cur_fbrx_mode_idx >= RFLM_FBRX_MODE_MAX)
    {
      cur_fbrx_mode_idx = RFLM_FBRX_MODE_MAX-1;
      RF_MSG_2(RF_ERROR, "rfcommon_fbrx_nv_update_dm: Invalid fbrx mode:%d, defaulting to %d",
                          fbrx_dm_ptr->fbrx_ctl.fbrx_mode, cur_fbrx_mode_idx);  
    }

    /* Initialize the fbrx dm cal data to 0 */
    memset(&fbrx_dm_ptr->fbrx_swp[0], 0, sizeof(int16)*RFLM_FBRX_NUM_GAIN_STATES );
    memset(&fbrx_dm_ptr->exp_gain_data.cal_params[0].fbrx_mode[cur_fbrx_mode_idx], 0, sizeof(rflm_fbrx_gain_idx_data_t));
    memset(&fbrx_dm_ptr->temp_comp_data, 0, sizeof(rfcommon_fbrx_temp_num_gain_type));
    ...
    switch (tech)
    {
      ...
      case RFM_IMT_MODE:
        if ( (band.wcdma_band < RFCOM_NUM_WCDMA_BANDS) && 
             (cur_fbrx_mode_idx < RFLM_FBRX_MODE_MAX) )
        {
          if (rfcommon_fbrx_nv_tech_init_mem_data.wcdma_init[band.wcdma_band].cal_data == TRUE) 
          {
          ...
          {
            RF_MSG_2(RF_ERROR, "rfcommon_fbrx_nv_update_dm: Invalid WCDMA tech init mem cal data = %d for band =%d", 
                     rfcommon_fbrx_nv_tech_init_mem_data.wcdma_init[band.wcdma_band].cal_data, band.wcdma_band );
            api_status = RFCOMMON_FBRX_ERROR;
          }
        }
        else
        {
          RF_MSG_2(RF_ERROR, "rfcommon_fbrx_nv_update_dm: Invalid WCDMA Band = %d or fbrx mode = %d", band.wcdma_band, cur_fbrx_mode_idx);
          api_status = RFCOMMON_FBRX_ERROR;
        }
        break;
        #endif /* FEATURE_WCDMA */
      ...
      case RFM_SVDO_MODE:
        if ( (band.cdma_band < RFM_CDMA_MAX_BAND) && 
             (cur_fbrx_mode_idx < RFLM_FBRX_MODE_MAX) )
        {
          if (rfcommon_fbrx_nv_tech_init_mem_data.cdma_init[band.cdma_band].cal_data == TRUE) 
          {
            gain_data_ptr = (rfcommon_fbrx_nv_gain_vs_freq_type_v2*) 
                             rfcommon_fbrx_nv_tech_cal_data.cdma_cal_data.gain_data[band.cdma_band];
            sp_data_ptr = (rfcommon_fbrx_nv_attn_switch_pt_type*) 
          ...
          {
            RF_MSG_2(RF_ERROR, "rfcommon_fbrx_nv_update_dm: Invalid CDMA tech init mem cal data = %d for band=%d", 
                     rfcommon_fbrx_nv_tech_init_mem_data.cdma_init[band.cdma_band].cal_data, band.cdma_band);
            api_status = RFCOMMON_FBRX_ERROR;
          }
        }
        else
        {
          RF_MSG_2(RF_ERROR, "rfcommon_fbrx_nv_update_dm: Invalid CDMA Band = %d or fbrx mode = %d", band.cdma_band, cur_fbrx_mode_idx);
          api_status = RFCOMMON_FBRX_ERROR;
        }
        break;
      #endif
      ...
      case RFM_LTE_FTM_MODE:
        if ( (band.lte_band < RFCOM_NUM_LTE_BANDS) && 
             (cur_fbrx_mode_idx < RFLM_FBRX_MODE_MAX) )
        {
          if (rfcommon_fbrx_nv_tech_init_mem_data.lte_init[c_val][band.lte_band].cal_data == TRUE) 
          {
            gain_data_ptr = (rfcommon_fbrx_nv_gain_vs_freq_type_v2*) 
                             rfcommon_fbrx_nv_tech_cal_data.lte_cal_data[c_val].gain_data[band.lte_band];
            sp_data_ptr = (rfcommon_fbrx_nv_attn_switch_pt_type*) 
                            rfcommon_fbrx_nv_tech_cal_data.lte_cal_data[c_val].sp_data[band.lte_band];
            temp_comp_data_ptr = (rfcommon_fbrx_temp_num_gain_type*)
                 &(rfcommon_fbrx_nv_tech_cal_data.lte_cal_data[c_val].temp_comp_data[band.lte_band]->fbrx_mode[cur_fbrx_mode_idx]);
          }
          else
          {
             RF_MSG_3(RF_ERROR, "rfcommon_fbrx_nv_update_dm: Invalid LTE tech init mem cal data = %d, for band = %d, c_val = %d", 
                      rfcommon_fbrx_nv_tech_init_mem_data.lte_init[c_val][band.lte_band].cal_data, band.lte_band, c_val);
             api_status = RFCOMMON_FBRX_ERROR;
          }
        }
        else
        {
          RF_MSG_2(RF_ERROR, "rfcommon_fbrx_nv_update_dm: Invalid LTE Band = %d or fbrx mode = %d", band.lte_band, cur_fbrx_mode_idx);
          api_status = RFCOMMON_FBRX_ERROR;
        }
        break;
      #endif /* FEATURE_LTE */
      ...
      case RFM_TDSCDMA_MODE_FTM:
        if ( (band.tdscdma_band < RFCOM_NUM_TDSCDMA_BANDS) && 
             (cur_fbrx_mode_idx < RFLM_FBRX_MODE_MAX) )
        {
          if (rfcommon_fbrx_nv_tech_init_mem_data.tds_init[band.tdscdma_band].cal_data == TRUE) 
          {
            gain_data_ptr = (rfcommon_fbrx_nv_gain_vs_freq_type_v2*) 
                             rfcommon_fbrx_nv_tech_cal_data.tdscdma_cal_data.gain_data[band.tdscdma_band];
            sp_data_ptr = (rfcommon_fbrx_nv_attn_switch_pt_type*) 
          ...
          {
            RF_MSG_2(RF_ERROR, "rfcommon_fbrx_nv_update_dm: Invalid TDSCDMA tech init mem cal data = %d for band =%d", 
                     rfcommon_fbrx_nv_tech_init_mem_data.tds_init[band.tdscdma_band].cal_data, band.tdscdma_band );
            api_status = RFCOMMON_FBRX_ERROR;
          }
        }
        else
        {
          RF_MSG_2(RF_ERROR, "rfcommon_fbrx_nv_update_dm: Invalid TDSCDMA Band = %d or fbrx mode =%d ", band.tdscdma_band, cur_fbrx_mode_idx);
          api_status = RFCOMMON_FBRX_ERROR;
        }
        break;
        #endif /* FEATURE_TDSCDMA */
    ...
    /* Get the mid-point tx frequency */
    tx_freq = fbrx_dm_ptr->fbrx_ctl.cur_tx_freq;
    ...
    if (api_status == RFCOMMON_FBRX_SUCCESS)
    {
      ...
      if (sp_data_ptr != NULL )
      {
        for(sp_idx = 0; sp_idx < MIN(sp_data_ptr->num_switch_pts, RFLM_FBRX_NUM_GAIN_STATES) ; sp_idx++)
        {
         ...
         for(sp_idx = RFLM_FBRX_SP_NV_item_5; sp_idx < (sp_data_ptr->num_switch_pts + RFLM_FBRX_SP_NV_item_5) ; sp_idx++)
        {
          fbrx_dm_ptr->fbrx_ulca_swp_data.fbrx_swp_ulca[ulca_sp_idx] = sp_data_ptr->switch_pwr_dbm[sp_idx];
          if (fbrx_dm_ptr->fbrx_ulca_swp_data.fbrx_swp_ulca[ulca_sp_idx]!=0) 
          {
             fbrx_dm_ptr->fbrx_ulca_swp_data.valid_swp = TRUE;
          }
          ulca_sp_idx++;
        }
      }
      ...
      if (gain_data_ptr != NULL)
      {
        for (mode_idx = 0; mode_idx < RFCOMMON_FBRX_NUM_MODES ; mode_idx++)
        {
          ...
          {
            for (chan_loop = 0; chan_loop < gain_data_ptr->fbrx_gain_chan_size; chan_loop++)
            {
              if(gain_data_ptr->fbrx_gain[chan_loop][mode_idx][gain_idx].data_valid == TRUE)
              {
                 fbrx_cal_data_valid = TRUE;
                 break;
              }
            }

            if( fbrx_cal_data_valid == TRUE )
              {
...
/*!
  @brief
  Generic function to copy the FBRX DC Cal and Droop NV data from the global fbrx
  var to the FBRX DM struct. 
...
rfcommon_fbrx_error_type
rfcommon_fbrx_nv_update_dc_droop_to_dm( rflm_fbrx_dm_template_t* fbrx_dm_ptr, 
                                        rfm_mode_enum_type tech,
                                        rfcom_band_type_u band,
                                        rfdevice_rxtx_common_class *device_ptr,
                                        rfcommon_fbrx_nv_container_type c_val)
{
  ...
  uint8 gain_idx = 0, mode_idx = 0, cur_fbrx_mode_idx = 0 ;
  int64 temp_fbrx_dc_val = 0;
  rfcommon_fbrx_dc_num_gain_type dc_cal_mode_3 = {0};
  rfcommon_fbrx_dc_num_gain_type dc_cal_mode_cur = {0};
  boolean fbrx_dc_val_equal_mode = TRUE;
  boolean fbrx_dc_val_zero_mode = TRUE;
  ...
  if (fbrx_dm_ptr!= NULL )
  {
    ...
    if (tx_cfg < RFCOMMON_FBRX_NUM_TX_CONFIG_MAX)
    {
      ...
      if( ( tech == RFM_LTE_MODE ) || ( tech == RFM_LTE_FTM_MODE ) )
      {
        ...
        if( lte_droop_data != NULL )
        {
          if( lte_droop_data->num_bw_payloads_populated <= RFCOMMON_FBRX_MAX_LTE_BW_COMBOS ) 
          {
            for ( bw_idx = 0; bw_idx < (lte_droop_data->num_bw_payloads_populated); bw_idx++  ) 
            {
              if ( lte_droop_data->bw_params[bw_idx].bw_cfg == tx_cfg ) 
              {
                /* Config found for LTE so no need to use
                   the common droop structures*/
                use_common_droop_db = FALSE;
                ...
                memscpy( (void *)( &(fbrx_dm_ptr->droop_cal_data) ), 
                         sizeof(rfcommon_fbrx_nv_droop_type), 
                         (void *)( &(lte_droop_data->bw_params[bw_idx].droop_params) ), 
                         sizeof(rfcommon_fbrx_nv_droop_type) );
              }
            }
          }
        ...
        {
          RF_MSG_2(RF_ERROR, "rfcommon_fbrx_nv_update_dm: lte_droop_data = NULL for LTE "
                             "band:%d tx_cfg = %d", band.lte_band, tx_cfg );
        }
      }
    ...
    switch (tech)
    {
      ...
      case RFM_IMT_MODE:
        if ( (band.wcdma_band < RFCOM_NUM_WCDMA_BANDS) && 
             (cur_fbrx_mode_idx < RFLM_FBRX_MODE_MAX) )
        {
          if (rfcommon_fbrx_nv_tech_init_mem_data.wcdma_init[band.wcdma_band].dc_data == TRUE) 
          {
          ...
          {
            RF_MSG_2(RF_ERROR, "rfcommon_fbrx_nv_update_dc_to_dm: Invalid WCDMA init mem dc cal data = %d for band=%d", 
                     rfcommon_fbrx_nv_tech_init_mem_data.wcdma_init[band.wcdma_band].dc_data, band.wcdma_band);
            api_status = RFCOMMON_FBRX_ERROR;
          }
        }
        else
        {
          RF_MSG_2(RF_ERROR, "rfcommon_fbrx_nv_update_dc_to_dm: Invalid WCDMA Band = %d or fbrx mode = %d", band.wcdma_band, cur_fbrx_mode_idx);
          api_status = RFCOMMON_FBRX_ERROR;
        }
        break;
        #endif /* FEATURE_WCDMA */
      ...
      case RFM_SVDO_MODE:
        if ( (band.cdma_band < RFM_CDMA_MAX_BAND) && 
             (cur_fbrx_mode_idx < RFLM_FBRX_MODE_MAX) )
        {
          if (rfcommon_fbrx_nv_tech_init_mem_data.cdma_init[band.cdma_band].dc_data == TRUE) 
          {
            dc_data_ptr = (rfcommon_fbrx_dc_num_gain_type*)
          ...
          {
            RF_MSG_2(RF_ERROR, "rfcommon_fbrx_nv_update_dc_to_dm: Invalid CDMA init mem dc cal data = %d for band=%d", 
                     rfcommon_fbrx_nv_tech_init_mem_data.cdma_init[band.cdma_band].dc_data, band.cdma_band);
            api_status = RFCOMMON_FBRX_ERROR;
          }
        }
        else
        {
          RF_MSG_2(RF_ERROR, "rfcommon_fbrx_nv_update_dc_to_dm: Invalid CDMA Band = %d or fbrx mode = %d", band.cdma_band, cur_fbrx_mode_idx);
          api_status = RFCOMMON_FBRX_ERROR;
        }
        break;
      #endif
      ...
      case RFM_LTE_FTM_MODE:
        if ( (band.lte_band < RFCOM_NUM_LTE_BANDS) && 
             (cur_fbrx_mode_idx < RFLM_FBRX_MODE_MAX) )
        {
           if (rfcommon_fbrx_nv_tech_init_mem_data.lte_init[c_val][band.lte_band].dc_data == TRUE) 
           {
             dc_data_ptr = (rfcommon_fbrx_dc_num_gain_type*)
                  &(rfcommon_fbrx_nv_tech_cal_data.lte_cal_data[c_val].dc_data[band.lte_band]->fbrx_mode[cur_fbrx_mode_idx]);

             if (cur_fbrx_mode_idx == RFLM_FBRX_MODE_1 || cur_fbrx_mode_idx == RFLM_FBRX_MODE_2)  
             {
               if ( rfcommon_fbrx_nv_tech_cal_data.lte_cal_data[c_val].dc_data[band.lte_band] != NULL ) 
               {

                 memscpy( (void*)(&dc_cal_mode_3), sizeof(rfcommon_fbrx_dc_num_gain_type), 
                          (void*)(&(rfcommon_fbrx_nv_tech_cal_data.lte_cal_data[c_val].dc_data[band.lte_band]->fbrx_mode[RFLM_FBRX_MODE_3])), 
                          sizeof(rfcommon_fbrx_dc_num_gain_type) );

                 memscpy( (void*)(&dc_cal_mode_cur), sizeof(rfcommon_fbrx_dc_num_gain_type), 
                          (void*)(&(rfcommon_fbrx_nv_tech_cal_data.lte_cal_data[c_val].dc_data[band.lte_band]->fbrx_mode[cur_fbrx_mode_idx])), 
                          sizeof(rfcommon_fbrx_dc_num_gain_type) );

                 for (gain_idx = 0; gain_idx < RFCOMMON_FBRX_NUM_GAIN_STATES; gain_idx++) 
                 {
                   if ( (dc_cal_mode_cur.gain_state[gain_idx].dc_i != dc_cal_mode_3.gain_state[gain_idx].dc_i) ||
                        (dc_cal_mode_cur.gain_state[gain_idx].dc_q != dc_cal_mode_3.gain_state[gain_idx].dc_q) ) 
                   {
                      //FBRx LTE20 MHz Cal != LTE10MHz Cal
                      fbrx_dc_val_equal_mode = FALSE;
                      break;
                   }
                 }
                 for (gain_idx = 0; gain_idx < RFCOMMON_FBRX_NUM_GAIN_STATES; gain_idx++)
                 {
                   if ( (dc_cal_mode_cur.gain_state[gain_idx].dc_i != 0) ||
                        (dc_cal_mode_cur.gain_state[gain_idx].dc_q != 0) ) 
                   {
                     //FBRx LTE20 MHz Cal != 0
                     fbrx_dc_val_zero_mode = FALSE;
                     break;
                   }
                 }

                 if ( fbrx_dc_val_zero_mode == TRUE || fbrx_dc_val_equal_mode == TRUE ) 
                 {
                    
                    if (rfdevice_id_get((void *)device_ptr) == WTR4905)
                    {
                      for (gain_idx = 0; gain_idx < RFCOMMON_FBRX_NUM_GAIN_STATES; gain_idx++)
                      {

                        temp_fbrx_dc_val = 60115*dc_cal_mode_3.gain_state[gain_idx].dc_i;                 
                        dc_cal_mode_cur.gain_state[gain_idx].dc_i = (int32)((temp_fbrx_dc_val >> 16) + 5060);

                        temp_fbrx_dc_val = 60115*dc_cal_mode_3.gain_state[gain_idx].dc_q;                 
                        dc_cal_mode_cur.gain_state[gain_idx].dc_q = (int32)((temp_fbrx_dc_val >> 16) + 5060);
                    ...
                    else
                    {
                      ...
                      {
                         temp_fbrx_dc_val = 60115*dc_cal_mode_3.gain_state[gain_idx].dc_i;                 
                         dc_cal_mode_cur.gain_state[gain_idx].dc_i = (int32)(temp_fbrx_dc_val >> 16) ;

                         temp_fbrx_dc_val = 60115*dc_cal_mode_3.gain_state[gain_idx].dc_q;                 
                         dc_cal_mode_cur.gain_state[gain_idx].dc_q = (int32)(temp_fbrx_dc_val >> 16) ;
                     }
                    }

                    memscpy((void *)(dc_data_ptr), sizeof(rfcommon_fbrx_dc_num_gain_type), 
                            (void*)&dc_cal_mode_cur , sizeof(rfcommon_fbrx_dc_num_gain_type) );

                 }//end if dc val mode

               } // end if rfcommon_fbrx_nv_tech_cal_data <dc data> != NULL
               else
               {
                 RF_MSG_1(RF_ERROR, "rfcommon_fbrx_nv_update_dc_to_dm: Invalid DC data for LTE Band = %d", band.lte_band);
                 api_status = RFCOMMON_FBRX_ERROR;
               }
             }//end if (cur_fbrx_mode_idx == RFLM_FBRX_MODE_1 || cur_fbrx_mode_idx == RFLM_FBRX_MODE_2)

           }
           else
           {
              RF_MSG_3(RF_ERROR, "rfcommon_fbrx_nv_update_dc_to_dm: Invalid LTE init mem dc cal data = %d for band=%d, c_cal =%d", 
                     rfcommon_fbrx_nv_tech_init_mem_data.lte_init[c_val][band.lte_band].dc_data, band.lte_band, c_val);
              api_status = RFCOMMON_FBRX_ERROR;
           }
        }
        else
        {
          RF_MSG_2(RF_ERROR, "rfcommon_fbrx_nv_update_dc_to_dm: Invalid LTE Band = %d or fbrx mode = %d", band.lte_band, cur_fbrx_mode_idx);
          api_status = RFCOMMON_FBRX_ERROR;
        }
        break;
      #endif /* FEATURE_LTE */
      ...
      case RFM_TDSCDMA_MODE_FTM:
        if ( (band.tdscdma_band < RFCOM_NUM_TDSCDMA_BANDS) && 
             (cur_fbrx_mode_idx < RFLM_FBRX_MODE_MAX) )
        {
          if (rfcommon_fbrx_nv_tech_init_mem_data.tds_init[band.tdscdma_band].dc_data == TRUE) 
          {
             dc_data_ptr = (rfcommon_fbrx_dc_num_gain_type*)
          ...
          {
            RF_MSG_2(RF_ERROR, "rfcommon_fbrx_nv_update_dc_to_dm: Invalid TDSCDMA init mem dc cal data = %d for band=%d", 
                     rfcommon_fbrx_nv_tech_init_mem_data.tds_init[band.tdscdma_band].dc_data, band.tdscdma_band);
            api_status = RFCOMMON_FBRX_ERROR;
          }
        }
        else
        {
          RF_MSG_2(RF_ERROR, "rfcommon_fbrx_nv_update_dc_to_dm: Invalid TDSCDMA Band = %d or fbrx mode =%d ", band.tdscdma_band, cur_fbrx_mode_idx);
          api_status = RFCOMMON_FBRX_ERROR;
        }
        break;
        #endif /* FEATURE_TDSCDMA */
      ...
      default:
        RF_MSG_1(RF_ERROR, "rfcommon_fbrx_nv_update_dc_to_dm: Invalid Tech = %d ", tech);
        api_status = RFCOMMON_FBRX_ERROR;
        break;
    } /* switch case*/
    ...
    if (api_status == RFCOMMON_FBRX_SUCCESS)
    {
      ...
      {
        RF_MSG(RF_ERROR, "rfcommon_fbrx_nv_update_dc_to_dm: FBRX DC Cal Data is NULL" );
        api_status = RFCOMMON_FBRX_ERROR;
      }
    } /* end if (api_status == TRUE) */
  ...
  {
    RF_MSG(RF_ERROR, "rfcommon_fbrx_nv_update_dc_to_dm: fbrx_dm_ptr = NULL");
    api_status = RFCOMMON_FBRX_ERROR;
  }

  return api_status;
} /* rfcommon_fbrx_nv_update_dc_to_dm() */


/* NV control of the sample size is deprecated on MDM9x45. This function will always return TRUE */
boolean
rfcommon_fbrx_nv_get_increase_sample_size_nv_status( rflm_tech_id_t tech )
{
  uint32 fbrx_ctrl_val = 0;

   switch (tech)
  {
    case RFLM_TECH_WCDMA:
      fbrx_ctrl_val = RFCOMMON_FBRX_CONTROL_BITMASK & rfcommon_fbrx_increase_sample_size;        
      break;

    case RFLM_TECH_LTE:
      fbrx_ctrl_val = RFCOMMON_FBRX_CONTROL_BITMASK & (rfcommon_fbrx_increase_sample_size >> 4);
      break;

    case RFLM_TECH_TDSCDMA:
      fbrx_ctrl_val = RFCOMMON_FBRX_CONTROL_BITMASK & (rfcommon_fbrx_increase_sample_size >> 8);
      break;

    case RFLM_TECH_1X:
      fbrx_ctrl_val = RFCOMMON_FBRX_CONTROL_BITMASK & (rfcommon_fbrx_increase_sample_size >> 12);
      break;
      
    case RFLM_TECH_HDR:
      fbrx_ctrl_val = RFCOMMON_FBRX_CONTROL_BITMASK & (rfcommon_fbrx_increase_sample_size >> 16);        
      break;
   ...
   /* This NV control is deprecated on MDM9x45. Always return TRUE */
   /*if (fbrx_ctrl_val > 0)
   {
     return TRUE;
   }
                                  ...
                                  rfcommon_fbrx_nv_container_type c_val)
{
  ...
  case RFM_SVDO_MODE:
    ...
    break;
  case RFM_LTE_MODE:
    if (rfcommon_fbrx_nv_tech_cal_data.lte_cal_data[c_val].dc_data[band] != NULL)
    {
      *dc_i = rfcommon_fbrx_nv_tech_cal_data.lte_cal_data[c_val].dc_data[band]->fbrx_mode[fbrx_mode_index].gain_state[gain_state_index].dc_i;
      *dc_q = rfcommon_fbrx_nv_tech_cal_data.lte_cal_data[c_val].dc_data[band]->fbrx_mode[fbrx_mode_index].gain_state[gain_state_index].dc_q;
      ret_value = RFCOMMON_FBRX_SUCCESS;
    }
    else
    {
...
rfcommon_fbrx_error_type
rfcommon_fbrx_nv_update_dc_globals( rfm_device_enum_type rfm_tx_device,
                                    rfm_mode_enum_type tech,
                                    uint16 band,
                                    uint8 fbrx_mode_index,
                                    uint8 gain_state_index,
                                    ...
                                    rfcommon_fbrx_nv_container_type c_val)
{
  ...
  case RFM_IMT_MODE:
    if (rfcommon_fbrx_nv_tech_cal_data.wcdma_cal_data.dc_data[band] != NULL)
    {
      ...
      rfcommon_fbrx_nv_tech_cal_data.wcdma_cal_data.dc_data[band]->fbrx_mode[fbrx_mode_index].gain_state[gain_state_index].dc_q = dc_q;
         #ifdef ATLAS_RF_WCDMA_PRESI
         handle = rf_wcdma_mc_data_tx_get_lm_handle(rfm_tx_device);
         #else
      handle = rfwcdma_data_get_rflm_buf_idx(rfm_tx_device);
         #endif
      ret_value = RFCOMMON_FBRX_SUCCESS;
    }
    else
    ...
    if (rfcommon_fbrx_nv_tech_cal_data.cdma_cal_data.dc_data[band] != NULL)
    {
      ...
      handle = rf_cdma_data_get_txlm_handle(rfm_tx_device);
      ret_value = RFCOMMON_FBRX_SUCCESS;
    }
    else
    {
  ...
  case RFM_LTE_MODE:
    if (rfcommon_fbrx_nv_tech_cal_data.lte_cal_data[c_val].dc_data[band] != NULL)
    {
      rfcommon_fbrx_nv_tech_cal_data.lte_cal_data[c_val].dc_data[band]->fbrx_mode[fbrx_mode_index].gain_state[gain_state_index].dc_i = dc_i;
      rfcommon_fbrx_nv_tech_cal_data.lte_cal_data[c_val].dc_data[band]->fbrx_mode[fbrx_mode_index].gain_state[gain_state_index].dc_q = dc_q;
      (void) rflte_state_get_txlm_buffer_idx(sub_id,rflte_ftm_mc_sm_get_carrier_index(), &handle);
      ret_value = RFCOMMON_FBRX_SUCCESS;
    }
    else
    {
  ...
  case RFM_TDSCDMA_MODE:
    if (rfcommon_fbrx_nv_tech_cal_data.tdscdma_cal_data.dc_data[band] != NULL)
    {
      ...
      handle = rf_tdscdma_mc_lm_bufs.tx_buf_wrk.buf_idx;
      ret_value = RFCOMMON_FBRX_SUCCESS;
    }
    else
    {
  ...
  default:
   ...
   fbrx_data_ptr = (rflm_fbrx_dm_template_t*)rflm_dm_get_fbrx_buf_ptr(handle);
   if (ret_value != RFCOMMON_FBRX_ERROR && fbrx_data_ptr != NULL && gain_state_index < RFLM_FBRX_NUM_GAIN_STATES)
   {  
   ...
   {
      RF_MSG_1(RF_ERROR, "rfcommon_fbrx_nv_update_dc_globals: Failed to update DC values to FBRx DM for tx handle = %d", handle);
   }
...
rfcommon_fbrx_error_type
rfcommon_fbrx_nv_get_lim_temp_freq_cal_data( rflm_fbrx_dm_template_t* fbrx_data_ptr,
                                             uint8 freq_idx,
                                             uint8 gain_idx,
                                             ...
                                             rfcommon_fbrx_nv_container_type c_val )
{
  uint8 bin_idx = 0;
  rfcommon_fbrx_error_type api_status = RFCOMMON_FBRX_SUCCESS;
  rfcommon_fbrx_nv_temp_type *temp_comp_data_ptr = NULL;
  rfm_mode_enum_type tech = 0;
  uint8 fbrx_band = 0;

  if (fbrx_data_ptr!= NULL)
  {
    tech = fbrx_data_ptr->fbrx_ctl.rfm_mode; 
    fbrx_band = fbrx_data_ptr->fbrx_ctl.fbrx_band;
  }
  else
  {
    RF_MSG(RF_ERROR, "rfcommon_fbrx_nv_get_lim_temp_freq_cal_data: fbrx_data_ptr = NULL");
    api_status = RFCOMMON_FBRX_ERROR;
  }

  /* Get the fbrx temp comp data pointer for current tech, band, and fbrx mode */
  switch (tech)
  {
    #ifdef FEATURE_WCDMA
    case RFM_IMT_MODE:
      if ( fbrx_band < RFCOM_NUM_WCDMA_BANDS)
      {
        temp_comp_data_ptr = (rfcommon_fbrx_nv_temp_type*)
             (rfcommon_fbrx_nv_tech_cal_data.wcdma_cal_data.temp_comp_data[fbrx_band]);
      }
      else
      {
        RF_MSG_1(RF_ERROR, "rfcommon_fbrx_nv_get_lim_temp_freq_cal_data: Invalid WCDMA Band = %d ", fbrx_band);
        api_status = RFCOMMON_FBRX_ERROR;
      }
      break;
    #endif /* FEATURE_WCDMA */
    ...
    case RFM_1XEVDO_MODE:
    case RFM_SVDO_MODE:
      if (fbrx_band < RFM_CDMA_MAX_BAND)
      {
        temp_comp_data_ptr = (rfcommon_fbrx_nv_temp_type*)
             (rfcommon_fbrx_nv_tech_cal_data.cdma_cal_data.temp_comp_data[fbrx_band]);
      }
      else
      {
        RF_MSG_1(RF_ERROR, "rfcommon_fbrx_nv_get_lim_temp_freq_cal_data: Invalid CDMA Band = %d ", fbrx_band);
        api_status = RFCOMMON_FBRX_ERROR;
      }
      break;
    #endif
    ...
    case RFM_LTE_MODE:
    case RFM_LTE_FTM_MODE:
      if (fbrx_band < RFCOM_NUM_LTE_BANDS)
      {
        temp_comp_data_ptr = (rfcommon_fbrx_nv_temp_type*)
             (rfcommon_fbrx_nv_tech_cal_data.lte_cal_data[c_val].temp_comp_data[fbrx_band]);
      }
      else
      {
        RF_MSG_1(RF_ERROR, "rfcommon_fbrx_nv_get_lim_temp_freq_cal_data: Invalid LTE Band = %d ", fbrx_band);
        api_status = RFCOMMON_FBRX_ERROR;
      }
      break;
    #endif /* FEATURE_LTE */
    ...
    case RFM_TDSCDMA_MODE:
    case RFM_TDSCDMA_MODE_FTM:
      if (fbrx_band < RFCOM_NUM_TDSCDMA_BANDS)
      {
        temp_comp_data_ptr = (rfcommon_fbrx_nv_temp_type*)
             (rfcommon_fbrx_nv_tech_cal_data.tdscdma_cal_data.temp_comp_data[fbrx_band]);
      }
      else
      {
        RF_MSG_1(RF_ERROR, "rfcommon_fbrx_nv_get_lim_temp_freq_cal_data: Invalid TDSCDMA Band = %d ", fbrx_band);
        api_status = RFCOMMON_FBRX_ERROR;
      }
      break;
      #endif /* FEATURE_TDSCDMA */
    ...
    default:
      RF_MSG_1(RF_ERROR, "rfcommon_fbrx_nv_get_lim_temp_freq_cal_data: Invalid Tech = %d ", tech);
      api_status = RFCOMMON_FBRX_ERROR;
      break;
  }

  if ( (temp_comp_data_ptr != NULL) && 
       (lim_temp_freq_cal_data != NULL) && 
       (gain_idx < RFCOMMON_FBRX_NUM_GAIN_STATES) &&
       (freq_idx < RFCOMMON_FBRX_NUM_TEMP_FREQ ) )        
  {
    /* Copy the temp data for FBRx Mode 0 */
    for (bin_idx =0; bin_idx< RFCOMMON_FBRX_TEMP_TABLE_SIZ; bin_idx++)
    {
      /* FBRX mode 0 temp comp data is to be used to emulate lim_vs_temp_vs_freq nv */
      lim_temp_freq_cal_data[bin_idx] = temp_comp_data_ptr->fbrx_mode[0].gain_state[gain_idx].temp[bin_idx][freq_idx];
    }
  }
  else
  {
    RF_MSG(RF_ERROR, "rfcommon_fbrx_nv_get_lim_temp_freq_cal_data: lim_temp_freq_cal_data update failed ");
    api_status = RFCOMMON_FBRX_ERROR;
  }
...
/*!
  @brief
  rfcommon_fbrx_nv_get_ref_therm_val
 
  @details
  Helper API: This function updates FBRx therm adc raw and scaled value for given tech/band
  ...
  @param
  output params -
  *therm_raw : pointer to the raw therm based on tech/band
  *therm_scaled : pointer to the scaled therm based on tech/band
 
*/
rfcommon_fbrx_error_type 
rfcommon_fbrx_nv_get_ref_therm_val( rfm_mode_enum_type tech, 
                                    uint8 band, 
                                    uint16 *therm_raw, 
                                    uint16 *therm_scaled,
                                    rfcommon_fbrx_nv_container_type c_val)
{
  rfcommon_fbrx_error_type return_val = RFCOMMON_FBRX_SUCCESS;

  switch (tech)
  {
     #ifdef FEATURE_WCDMA  
     case RFM_IMT_MODE:
       if (rfcommon_fbrx_nv_tech_cal_data.wcdma_cal_data.gain_data[band] != NULL)
       {
         *therm_raw = rfcommon_fbrx_nv_tech_cal_data.wcdma_cal_data.gain_data[band]->fbrx_therm_adc;
         *therm_scaled = rfwcdma_core_temp_comp_get_scaled_therm_read( *therm_raw );
       }
       else
       {
         RF_MSG_2(RF_ERROR, "rfcommon_fbrx_nv_get_ref_therm_val: Gain cal pointer returned = NULL for rfm tech = %d, band = %d ", tech, band);
         return_val = RFCOMMON_FBRX_ERROR;
       }
       break;
     #endif
     case RFM_1X_MODE:
     case RFM_1XEVDO_MODE:
     case RFM_SVDO_MODE:
       if ( rfcommon_fbrx_nv_tech_cal_data.cdma_cal_data.gain_data[band] != NULL )
       {
         *therm_raw = rfcommon_fbrx_nv_tech_cal_data.cdma_cal_data.gain_data[band]->fbrx_therm_adc;
         *therm_scaled = *therm_raw; /* Scaling not required for C2k */
       }
       else
       {
         RF_MSG_2(RF_ERROR, "rfcommon_fbrx_nv_get_ref_therm_val: Gain cal pointer returned = NULL for rfm tech = %d, band = %d ", tech, band);
     ...
     case RFM_LTE_MODE:
     case RFM_LTE_FTM_MODE:
       if (rfcommon_fbrx_nv_tech_cal_data.lte_cal_data[c_val].gain_data[band] != NULL)
       {
         *therm_raw = rfcommon_fbrx_nv_tech_cal_data.lte_cal_data[c_val].gain_data[band]->fbrx_therm_adc;
         *therm_scaled = rflte_core_temp_comp_raw_to_scaled( RFLTE_MC_PCELL_IDX, *therm_raw );
       }
       else
       {
         RF_MSG_2(RF_ERROR, "rfcommon_fbrx_nv_get_ref_therm_val: Gain cal pointer returned = NULL for rfm tech = %d, band = %d ", tech, band);
     ...
     case RFM_TDSCDMA_MODE:
     case RFM_TDSCDMA_MODE_FTM:
       if (rfcommon_fbrx_nv_tech_cal_data.tdscdma_cal_data.gain_data[band] != NULL)
       {
         *therm_raw = rfcommon_fbrx_nv_tech_cal_data.tdscdma_cal_data.gain_data[band]->fbrx_therm_adc;
         *therm_scaled = rf_tdscdma_core_temp_comp_scale_therm( *therm_raw, FALSE );
       }
       else
       {
         RF_MSG_2(RF_ERROR, "rfcommon_fbrx_nv_get_ref_therm_val: Gain cal pointer returned = NULL for rfm tech = %d, band = %d ", tech, band);
     ...
     default:
       RF_MSG_2(RF_ERROR, "rfcommon_fbrx_nv_get_ref_therm_val: Invalid rfm tech = %d, band = %d ", tech, band);
       return_val = RFCOMMON_FBRX_ERROR;
       break;
  }
...
uint32 rfcommon_fbrx_get_tx_freq_from_ul_arfcn( rfm_mode_enum_type tech,
                                                rfcom_band_type_u band,
                                                uint32 chan_array[],                                       
                                                uint8 num_channels ) 
{
  ...
  switch (tech) 
  {
    ...
    case RFM_IMT_MODE:
      ...
      {
        RF_MSG_1(RF_ERROR, "rfcommon_fbrx_get_tx_freq_from_ul_arfcn: Invalid WCDMA band =%d", band.wcdma_band );
      }
      break;
      #endif /* FEATURE_WCDMA */
    ...
    case RFM_SVDO_MODE:
      ...
      {
        RF_MSG_1(RF_ERROR, "rfcommon_fbrx_get_tx_freq_from_ul_arfcn: Invalid CDMA Band = %d", band.cdma_band );
      }
      break;
    #endif
    ...
    case RFM_LTE_FTM_MODE:
      ...
      {
        RF_MSG_1(RF_ERROR, "rfcommon_fbrx_get_tx_freq_from_ul_arfcn: Invalid LTE band = %d ", band.lte_band);
      }
      break;
    #endif /* FEATURE_LTE */
    ...
    case RFM_TDSCDMA_MODE_FTM:
      ...
      {
        RF_MSG_1(RF_ERROR, "rfcommon_fbrx_get_tx_freq_from_ul_arfcn: Invalid TDSCDMA band =%d", band.tdscdma_band );
      }
      break;
      #endif /* FEATURE_TDSCDMA */

    default:
      RF_MSG_1(RF_ERROR, "rfcommon_fbrx_get_tx_freq_from_ul_arfcn: Invalid Tech = %d ", tech);
      break;
  }
