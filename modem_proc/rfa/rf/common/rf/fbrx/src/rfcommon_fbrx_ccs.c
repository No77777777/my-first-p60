  ...
  */
  if ( NULL == rflm_dm_get_script_buf_ptr ( fbrx_handle, event_id ) )
  {
    ...
    switch ( event_id ) 
    {
      ...
      /* Allocate buf ptr for FBRX gain update script */
       case RFLM_FBRX_CCS_GAIN_STATE_0_EVENT:
       case RFLM_FBRX_CCS_GAIN_STATE_1_EVENT:
       case RFLM_FBRX_CCS_GAIN_STATE_2_EVENT:
       case RFLM_FBRX_CCS_GAIN_STATE_3_EVENT:
      {
        buff_meta.type = SCRIPT_TYPE_COMMON;
        buff_meta.size_in_byte = RFCOMMON_FBRX_DEFAULT_SCRIPT_SIZE;
        buff_meta.fmt = SCRIPT_FMT_DEVICE_FMT;
...
) 
{
  ...
  switch ( event_id )
  {
    case RFLM_FBRX_CCS_SETUP_EVENT: 
    {
      ...
      ccs_event_transaction_params->priority = 14;
  ...
  if (fbrx_data_ptr != NULL)
  {
    ...
    if ( fbrx_data_ptr->fbrx_script_ptr[event_id] != NULL  )
    {
      ...
      if( is_event_issue_seq == FALSE )
      {
        ...
        if ( api_status == TRUE )
        {
          MSG_4( MSG_SSID_RF_DEBUG, MSG_LEGACY_MED, "rfcommon_fbrx_ccs_configure_event: Begin CCS Flow: "
...
/*!
  @brief
  rfcommon_fbrx_ccs_configure_fbrx_gain_update_scripts
 
  @details
  Creates the FBRX gain update event script. The scripts can be either 
  SSBI, RFFE or GRFC based, depends on what is supported
 
  @param device
  Device which needs to be configured for FBRX gain change
...
*/
boolean 
rfcommon_fbrx_ccs_configure_fbrx_gain_state_scripts
( 
  rfm_device_enum_type    device,
  lm_handle_type          fbrx_handle,
  rflm_handle_tx_t        tx_handle 
)
{
  ...
  uint8 event_id, fbrx_gain_state;

  /* Buffer object to hold FBRX gain state script */
  ...
  if (device_ptr == NULL)
  {
    RF_MSG( RF_ERROR, "rfcommon_fbrx_ccs_configure_fbrx_gain_state_scripts: Null WTR device ptr" );
    api_status = FALSE;
    return api_status;
  }

  for ( event_id = RFLM_FBRX_CCS_GAIN_STATE_0_EVENT , fbrx_gain_state = 0  ; event_id <= RFLM_FBRX_CCS_GAIN_STATE_3_EVENT ; event_id++ , fbrx_gain_state++ )
  {

    fbrx_event_handle = &(rfcommon_fbrx_ccs_event_handle_ptr[event_id]); 
    /* Create generic buffer interface object to holdnm script settings */
    ...
    if( rf_fbrx_script != NULL )
    {
     ...
     api_status = rfdevice_cmn_get_fbrx_enable_script(device_ptr, fbrx_gain_state , RFDEVICE_CREATE_SCRIPT, rf_fbrx_script , FALSE, 0, (rfm_mode_enum_type)fbrx_data_ptr->fbrx_ctl.rfm_mode );
      ...
      {
        RF_MSG_2 ( RF_ERROR, "rfcommon_fbrx_ccs_configure_fbrx_gain_state_scripts: rfdevice_cmn_get_fbrx_enable_script for gain state %d failed"
                          "for dev_ptr:%d", fbrx_gain_state ,  device_ptr );
      }
    ...
    {
      RF_MSG( RF_ERROR, "rfcommon_fbrx_ccs_configure_fbrx_gain_state_scripts: "
