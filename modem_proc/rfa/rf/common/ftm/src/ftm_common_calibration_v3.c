...
/*==============================================================================
  ...
  $Header: //components/rel/rfa.mpss/4.11/rf/common/ftm/src/ftm_common_calibration_v3.c#1 $

when       who     what, where, why
--------   ---     -------------------------------------------------------------
12/06/15   aen     Fix compressed ESC Cal Command 
06/26/15   br      Fix KW warnings from previous checkin
06/19/15   br      Add support for FTM  Response Packet Compression
05/08/15   zhw     Dev 5 support during Cal V3 Rx cal
...
(
  void *ftm_req_data,
  boolean enableResCompression
)
{
  int16 diag_limit = 0; /* Diag Limitataion in bytes */
  int16 result_len = 0; /* Length of calibration results to be sent */
  uint32 unread_buff_size = 0; /* Unread Result Buffer Sizer */
  ftm_calibration_data_type *cal_data; /* Calv3 Data */
  uint8 result_query_count = 0; /* Local copy of result query count */

  /*Packet Compression status*/
  int8 compress_status = Z_OK;
  /*Size after compression*/
  uint32 compressed_result_len = 0;
  /* compressed Response Packet and pointer definitions*/
  ftm_common_pkt_type *compressed_pkt = NULL;
  uint8 *compressed_result_ptr;
  ...
  if ( cal_data->result.buff_read_en == TRUE )
  {
    ...
    /* Calculate unread buffer size */
        unread_buff_size = (uint32)(cal_data->result.buff_written - 
                                       cal_data->result.buff_read);
    FTM_MSG_1(FTM_ERROR, "Unread buffer size = %d", unread_buff_size);
    if (enableResCompression == TRUE) 
    {
      if ( cal_data->result.buff_read == 0 ) 
      {
        FTM_MSG(FTM_ERROR, "First query-Compress result");
        compressed_pkt = (ftm_common_pkt_type *)modem_mem_alloc(
           unread_buff_size, MODEM_MEM_CLIENT_RFA);

        if (compressed_pkt == NULL) 
        {
          FTM_MSG(FTM_ERROR, "Compressed Packet is NULL");
          ftm_rsp_data = ftm_common_malloc_rsp_pkt(ftm_req_data,
                                                   FPOS(ftm_cal_sweep_get_results_rsp_pkt_type, cal_result) + unread_buff_size);

          /* Extract Packet Payload from New Response Packet, ftm_rsp_data */
          rsp_pkt = (ftm_cal_sweep_get_results_rsp_pkt_type *)ftm_rsp_data.pkt_payload;

          /* Send back a BAD response packet for NULL compression packet*/
          if (rsp_pkt == NULL) 
          {
            ftm_rsp_data.cmd = FTM_RSP_BAD_PARM;
          ...
          ftm_rsp_data.cmd = FTM_RSP_BAD_LEN;
          FTM_MSG(FTM_ERROR, " : Failed "
                  "During Payload Compression Malloc");
          return ftm_rsp_data;
        } /* if (compressed_pkt == NULL) */

        compressed_result_ptr = (uint8 *)compressed_pkt;
        compressed_result_len = unread_buff_size;
        /*Compress result payload*/
        compress_status = zlib_compress(compressed_result_ptr,
                                        &compressed_result_len,
                                        (const uint8 *)(cal_data->result.read_ptr),
                                        unread_buff_size);
        FTM_MSG_2(FTM_ERROR,"compress status = %d,compressed_result_len = %d", compress_status,compressed_result_len);
        if (compressed_result_len == unread_buff_size)
        {
          FTM_MSG_2(FTM_ERROR, "No compression done:Compress status = %d, Original Result len = %d", compress_status, unread_buff_size);
        } 
        else
        {
          unread_buff_size = compressed_result_len;
          cal_data->result.buff_written = unread_buff_size;
          cal_data->result.read_ptr = compressed_result_ptr;
          FTM_MSG_3(FTM_ERROR, "Compress status = %d, Compressed result len = %d,Buffer written = %d", compress_status, unread_buff_size, cal_data->result.buff_written);
        }

      } 
      else 
      {
        FTM_MSG_2(FTM_ERROR, "Compression already done:Buffer Read = %d,Buffer Written = %d", cal_data->result.buff_read, cal_data->result.buff_written);
        unread_buff_size = cal_data->result.buff_written - cal_data->result.buff_read;
        FTM_MSG_1(FTM_ERROR, "No compression case:Current length = %d", unread_buff_size);
      }
    }
    ...
    FTM_MSG_1(FTM_ERROR, "Length of result returned = %d", result_len);
    ...
    if (enableResCompression == TRUE && cal_data->status.seg_error_log.first_seg_error == 0) 
    {
    
      if (compress_status == Z_BUF_ERROR)
      {
          FTM_MSG(FTM_ERROR,"Buffer error");
        rsp_pkt->seg_error_code = FTM_CALIBRATION_COMPRESSION_BUFFER_FAILURE;
      }
      
      else if (compress_status == Z_DATA_ERROR)
      {
        FTM_MSG(FTM_ERROR,"Data error");
        rsp_pkt->seg_error_code = FTM_CALIBRATION_COMPRESSION_DATA_FAILURE;
      }

      else if (compress_status == Z_OK)
      {
        FTM_MSG_1(FTM_ERROR,"Compress successul:Error code = %d",cal_data->status.seg_error_log.first_seg_error);
    ...
    if ( unread_buff_size > 0 )
    {
      ...
      else
      {
        ...
        if (enableResCompression == TRUE)
        {
          /* Compressed packet has been used, so free it */
          modem_mem_free(compressed_pkt, MODEM_MEM_CLIENT_RFA);
        }
