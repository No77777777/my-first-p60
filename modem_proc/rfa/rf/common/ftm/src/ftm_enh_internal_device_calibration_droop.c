...
/*==============================================================================
  ...
  $Header: //components/rel/rfa.mpss/4.11/rf/common/ftm/src/ftm_enh_internal_device_calibration_droop.c#1 $
...
05/21/15    br     [Rx/Tx Split]Remove device hardcoding and use current tx device
                                                          ...
                                                          )
{
  ...
  iq_cap_instr.primary_device = tx_device; 
  iq_cap_instr.feedback_device = tx_device; 
...
)
{
  ...
  /****************************************************************/
  FTM_MSG_1(FTM_ERROR,"Current technology = %d",curr_tx_ftm_rf_tech);
   ...
   *res_len = sizeof( ftm_enh_internal_device_cal_fb_droop_result_type);
   ...
   ret_val = ftm_enh_internal_device_cal_fbrx_iq_cap_config(fbrx_tech, fbrx_bw,band_union,tx_device);
   if (ret_val != FTM_ENH_INTERNAL_DEVICE_CAL_RESULTS_SUCCESS)
   {
   ...
   if (do_processing == 1)
   {
     ...
     for ( iter2 = 0; iter2 < num_droop_filters; iter2++ )
     {
       ...
       nv_read_write_error = rfcommon_fbrx_override_droop_globals( tx_device,
                                                                   ftm_get_rf_technology(), 
                                                                   band_union,
                                                                   (rfcommon_fbrx_tx_cfg_type)droop_result->fbrx_droop_res[iter2].fbrx_tech_bw_type, 
                                                                   droop_data_nv,
                                                                   RFCOMMON_FBRX_NV_CONTAINER_C0 );
  ...
  for (filt_iter = 0; filt_iter< numNomEq; filt_iter++)
  {
    ...
    for (row_iter = 0; row_iter < CPLX_NCALEQ; row_iter++)
    {
      ...
      for (valid_freq_iter = 0; valid_freq_iter < num_caps_techbw; valid_freq_iter++ )
      {
        ...
        case RFCOMMON_FBRX_LTE_3:
        case RFCOMMON_FBRX_LTE_5:
		  if(valid_freq_iter < filter_grp_delay_LTE15)
		  {
          ...
          {
		    FTM_MSG_1(FTM_ERROR, "Invalid filter for LTE band : %d " ,valid_freq_iter);
		  }
		  break;
        case RFCOMMON_FBRX_LTE_10:
