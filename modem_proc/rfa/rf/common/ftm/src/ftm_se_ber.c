...
INITIALIZATION AND SEQUENCING REQUIREMENTS
  ...
  Copyright (c) 2007 - 2021 by Qualcomm Technologies INCORPORATED. All Rights Reserved.
...
/*===========================================================================
  ...
  $Header: //components/rel/rfa.mpss/4.11/rf/common/ftm/src/ftm_se_ber.c#2 $
  $DateTime: 2021/09/22 08:25:43 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
09/22/21   ag      [CR3042151] - KW Error Fix
04/10/14   zbz     Fix KW
...
boolean acquire_pn_synch( se_ber_pn_sequence_type* sequence, uint32* data, uint8 data_len_dword, uint16* threshold, uint32* current_bit_pos )
{
  ...
  uint8 rx_bit_data = 0x0, rx_bit_local_pn_gen = 0x0, data_offset = 0x0, shift = 0x0, block_end = 0x0;

  if(sequence->gen_polynomial_degree <= FTM_SE_PN_TYPE_PN15 )
  {
    uint8 R = gen_poly_degree_lut[sequence->gen_polynomial_degree];
  ...
  {
    FTM_MSG_HIGH("In acquire_pn_synch : gen_polynomial_degree array element has reached MAX value, ret_val = %d",ret_val,0,0);
  }
  return ret_val;
}
