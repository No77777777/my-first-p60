...
/*==============================================================================
  ...
  $Header: //components/rel/rfa.mpss/4.11/rf/common/ftm/src/ftm_common_radio.c#1 $

when       who     what, where, why
--------   ---     -------------------------------------------------------------
12/04/15   dyc     Support ASDiv callbacks on set antenna IMM
07/15/15   cdb     Add support to override Autopin operation
04/13/15   ak      Rx/Tx Split Changes to support CRAT
...
)
{
...
#if defined (FEATURE_RF_ASDIV)
    FTM_MSG_2( FTM_LOW,"ftm_common_radio_set_antenna: callback mode=%d, sw_pos=%d", mode, antenna_req_ptr->position);

    /* dispatch ASDiv RF internal callbacks registered with ASDiv */
    rfcommon_asdiv_dispatch_cb( mode, antenna_req_ptr->position );
#endif
  ...
  FTM_MSG_5( FTM_LOW, "ftm_common_radio_set_antenna: [Status %d] "
             "Device %d, Position %d, Action IMM %d mode=%d : [Done]",
             rfm_ret,
...
)
{
  ...
  boolean ret_val;
  #if defined FEATURE_WCDMA
  rfm_wcdma_device_type device_type = 0;
  #endif
  ...
  FTM_MSG_1( FTM_ERROR, "RF Device type: %d", enter_mode_req_ptr->device_type);

  rf_mode = (rfm_mode_enum_type)(enter_mode_req_ptr->rf_mode);
  device =  (rfm_device_enum_type)(enter_mode_req_ptr->device);
  
  /*Added as the enter mode for W needs device type to be passed. Other Tech might need it too*/
  ...
  switch ( rf_mode )
  {
    ...
    #if defined FEATURE_WCDMA	
	device_type = (rfm_wcdma_device_type)(enter_mode_req_ptr->device_type);
	FTM_MSG_1( FTM_ERROR, "RF Device type: %d", enter_mode_req_ptr->device_type);
    case RFM_IMT_MODE:
	 FTM_MSG_1( FTM_ERROR, "Debug Msg: Unsupported RF Device type: %d", device_type);	  
         ret_val = ftm_wcdma_radio_enter_mode(device, rf_mode, enter_mode_req_ptr->device_type);
    break;
    #endif
...
/*!
  @brief
  Overrides for Autopin debugging

  @details
  Controls operation of the Autopin execution loop
...
*/
ftm_rsp_pkt_type
ftm_common_autopin_override
(
  void  *ftm_req_data
)
{
  ftm_rsp_pkt_type ftm_rsp_data;          /* Packet returned to Diag */
  PACK(struct) ftm_common_autopin_override_req_s /* Input parameter structure */
  {
    uint8 cmd;
    uint8 sub_sys;
    uint16 mode;
    ...
    uint8  ovr_val1;
    uint8  ovr_val2;
  } *autopin_override_ptr;

  autopin_override_ptr = 
       (struct ftm_common_autopin_override_req_s *)ftm_req_data;
       
  rfcommon_autopin_override_ctrl(autopin_override_ptr->tx_handle, 
                                 autopin_override_ptr->ovr_cmd_id, 
                                 autopin_override_ptr->ovr_enable, 
                                 autopin_override_ptr->ovr_val1, 
                                 autopin_override_ptr->ovr_val2);
  
  FTM_MSG_2( FTM_HIGH, "ftm_common_autopin_override: [Done]", 0, 0);
