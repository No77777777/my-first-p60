...
/*===========================================================================
  ...
  $Header: //components/rel/rfa.mpss/4.11/rf/common/ftm/src/ftm_log.c#1 $

when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
10/06/15   adk     Added support for ASDiv related logging
06/29/15   jfc     Zero out log_config.cb_struct; NULL out pointer after free
06/18/15   vr      [ported] fixed timer_undef issue  
...
#endif
{
  ...
  { FTM_LOG_ASDIV_DATA,          { NULL,    NULL,              TRUE,       0 },   NULL,                   NULL                   },
  { FTM_LOG_ASDIV_TUNER_DATA,    { NULL,    NULL,              TRUE,       0 },   NULL,                   NULL                   }
};
...
void ftm_log_enable(ftm_log_id_type log_id, int4 interval, int num_buffer)
{
  ...
  if( log_config_ptr->log_config.active == FALSE)
  {
    ...
    if( log_config_ptr->log_config.cb_func != NULL)
    {
         ...
         memset(log_config_ptr->log_config.cb_struct, 0, sizeof(timer_type));
...
void ftm_log_disable(ftm_log_id_type log_id)
{
  ...
  if(log_config_ptr->log_config.active)
  {
    ...
    {
        (void) timer_clr(log_config_ptr->log_config.cb_struct, T_NONE);
        timer_undef(log_config_ptr->log_config.cb_struct);
        ftm_free(log_config_ptr->log_config.cb_struct);
        log_config_ptr->log_config.cb_struct = NULL;
    }
...
ftm_rsp_pkt_type ftm_log_dispatch(ftm_log_req_pkt_type  *cmd_ptr)
{
  ...
  {
  case FTM_LOG_ENABLE:
    FTM_MSG_MED("FTM Log dispatching packet cmd: %d, id: 0x%x, num_bufs: %d",
               cmd_ptr->ftm_hdr.cmd_id, cmd_ptr->params.log_id, cmd_ptr->params.num_buffer);
  ...
  case FTM_LOG_DISABLE:
    FTM_MSG_MED("FTM Log dispatching packet cmd: %d, id: 0x%x",
               cmd_ptr->ftm_hdr.cmd_id, cmd_ptr->params.log_id, 0);
