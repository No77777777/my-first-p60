...
$Header: //components/rel/rfa.mpss/4.11/rf/common/ftm/src/ftmdiag.c#2 $
$DateTime: 2021/03/24 03:28:39 $

when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
02/04/21   garyz   Add ftmdiag_is_blocked_command() to block unwanted command from Diag port
09/24/15   zhw     Featurize internal FTM command per diag security requirement 
07/28/15   zhw     Enabled C2K FTM Chain C4/C5 in command register table
...
05/29/15   sc      Set fp required for setting pkt size for FTM GSM enh debug
05/20/15   sc      Add FTM GSM enh debug dispatcher command
05/12/15   ak      [Rx/Tx Split]Changes to support 6 Rx Chain RF test
05/04/15   aro     Enabled FTM RF TEST subsys ID
04/21/15   zbz     Add diag support for GSM C4/C5 
...
#define FTM_RSP_MAX_PKT_SIZE 2048 
/* ----------------------------------------------------------------------- */
...
)
{
  ...
  if ((pkt->subsys_id == FTM && pkt->subsys_cmd_code == FTM_1X_C0_C) ||
      ...
      (pkt->subsys_id == FTM && pkt->subsys_cmd_code == FTM_RF_TEST_C) ||
      ...
      (pkt->subsys_id == FTM && pkt->subsys_cmd_code == FTM_WCDMA_RX_5_C) ||
      (pkt->subsys_id == FTM && pkt->subsys_cmd_code == FTM_WCDMA_RX_6_C) ||      
      (pkt->subsys_id == FTM && pkt->subsys_cmd_code == FTM_WCDMA_RX_7_C) ||  
      (pkt->subsys_id == FTM && pkt->subsys_cmd_code == FTM_WCDMA_RX_8_C) ||  
      ...
      (pkt->subsys_id == FTM && pkt->subsys_cmd_code == FTM_GSM_C4_C) ||
      (pkt->subsys_id == FTM && pkt->subsys_cmd_code == FTM_GSM_C5_C) ||
      ...
      (pkt->subsys_id == FTM && pkt->subsys_cmd_code == FTM_1X_DYNAMIC_CHAIN_C) ||
      (pkt->subsys_id == FTM && pkt->subsys_cmd_code == FTM_WCDMA_DYNAMIC_CHAIN_C) || 
      (pkt->subsys_id == FTM && pkt->subsys_cmd_code == FTM_GSM_DYNAMIC_CHAIN_C) ||
      (pkt->subsys_id == FTM && pkt->subsys_cmd_code == FTM_TDS_DYNAMIC_CHAIN_C) ||
      (pkt->subsys_id == FTM && pkt->subsys_cmd_code == FTM_GSM_EXTENDED_DYNAMIC_CHAIN_C)||
      (pkt->subsys_id == FTM && pkt->subsys_cmd_code == FTM_1X_DYNAMIC_CHAIN_DRX_C) ||
      (pkt->subsys_id == FTM && pkt->subsys_cmd_code == FTM_WCDMA_DYNAMIC_CHAIN_DRX_C) || 
      (pkt->subsys_id == FTM && pkt->subsys_cmd_code == FTM_GSM_DYNAMIC_CHAIN_DRX_C) ||
      (pkt->subsys_id == FTM && pkt->subsys_cmd_code == FTM_TDS_DYNAMIC_CHAIN_DRX_C) ||
      (pkt->subsys_id == FTM && pkt->subsys_cmd_code == FTM_GSM_EXTENDED_DYNAMIC_CHAIN_DRX_C) ||
      (pkt->subsys_id == FTM && pkt->subsys_cmd_code == FTM_1X_C4_C) ||
      (pkt->subsys_id == FTM && pkt->subsys_cmd_code == FTM_1X_C5_C) 
...
#else
    uint16        rsp_pkt_len = 0;

    if (pkt->subsys_id == FTM && pkt->subsys_cmd_code == FTM_RFNV_C) // only FTM_RFNV case and allocate up to max of efs size.
    {
      ftm_composite_cmd_header_type *pkthdr = (ftm_composite_cmd_header_type*)req_pkt;
      if ( pkthdr->ftm_hdr.cmd_rsp_pkt_size < FTM_RSP_MAX_PKT_SIZE )
          rsp_pkt_len = FTM_RSP_MAX_PKT_SIZE;
      else
          rsp_pkt_len = pkthdr->ftm_hdr.cmd_rsp_pkt_size;         
    }
    else
    {
      rsp_pkt_len = FTM_RSP_MAX_PKT_SIZE;
    }
    rsp_ptr = (void*) diagpkt_subsys_alloc((diagpkt_subsys_id_type)DIAG_SUBSYS_FTM, 1, rsp_pkt_len );      
    if ( NULL == rsp_ptr )
    {
      rsp_ptr = 0;
    }
    ftm_msg->rsp_pkt_len = rsp_pkt_len;
...
boolean ftmdiag_process_async_rsp_pkt (ftm_rsp_pkt_type rsp_pkt)
{
  ...
  case FTM_RSP_BAD_MODE:
    {
      ...
      */
      ...
      else if ((DIAGPKT_MAX_ERR + 1) < ftm_pkt_data.rsp_pkt_len)
      {
        diagpkt_shorten( ftm_pkt_data.data_rsp_ptr, DIAGPKT_MAX_ERR + 1 );
        ftm_pkt_data.rsp_pkt_len = DIAGPKT_MAX_ERR + 1; 
      }
  ...
  case FTM_RSP_DO_LEGACY:
    ...
    {
      if (ftm_pkt_data.data_rsp_ptr == NULL && ftm_pkt_data.rsp_pkt_len > 0)
      {
      ...
      if (ftm_pkt_data.data_rsp_ptr != NULL)
      {
        ...
        memscpy (ftm_pkt_data.data_rsp_ptr, ftm_pkt_data.rsp_pkt_len, 
                 ftm_pkt_data.ftm_data_ptr, rsp_pkt.pkt_len );
      }
      else
      {
        good_status = FALSE;
...
static const diagpkt_user_table_entry_type ftmdiag_tbl[] = 
{
      ...
      {FTM_RF_TEST_C,FTM_RF_TEST_C,ftm_diag},
#endif
      ...
      {FTM_GSM_C4_C,FTM_GSM_C4_C,ftm_diag},

      {FTM_GSM_C5_C,FTM_GSM_C5_C,ftm_diag},
      ...
      {FTM_GSM_EXTENDED_C4_C,FTM_GSM_EXTENDED_C4_C,ftm_diag},

      {FTM_GSM_EXTENDED_C5_C,FTM_GSM_EXTENDED_C5_C,ftm_diag},
      ...
      {FTM_WCDMA_RX_5_C,FTM_WCDMA_RX_5_C,ftm_diag},

      {FTM_WCDMA_RX_6_C,FTM_WCDMA_RX_6_C,ftm_diag},	  

      {FTM_WCDMA_RX_7_C,FTM_WCDMA_RX_7_C,ftm_diag},	 

      {FTM_WCDMA_RX_8_C,FTM_WCDMA_RX_8_C,ftm_diag},	 
      ...
      {FTM_GSM_BER_C4_C,FTM_GSM_BER_C4_C,ftm_diag},

      {FTM_GSM_BER_C5_C,FTM_GSM_BER_C5_C,ftm_diag},
      ...
      //{FTM_LTE_DEV,FTM_LTE_DEV,ftm_diag},
...
#ifdef FEATURE_RFA_INTERNAL_FTM_CMD
      {FTM_GSM_ENH_0_C, FTM_GSM_ENH_0_C, ftm_diag},  //Internal use only
      {FTM_GSM_ENH_1_C, FTM_GSM_ENH_1_C, ftm_diag},  //Internal use only

      {FTM_1X_DYNAMIC_CHAIN_C,FTM_1X_DYNAMIC_CHAIN_C,ftm_diag},

      {FTM_WCDMA_DYNAMIC_CHAIN_C,FTM_WCDMA_DYNAMIC_CHAIN_C,ftm_diag},

      {FTM_GSM_DYNAMIC_CHAIN_C,FTM_GSM_DYNAMIC_CHAIN_C,ftm_diag},

      {FTM_TDS_DYNAMIC_CHAIN_C,FTM_TDS_DYNAMIC_CHAIN_C,ftm_diag},

      {FTM_GSM_EXTENDED_DYNAMIC_CHAIN_C,FTM_GSM_EXTENDED_DYNAMIC_CHAIN_C,ftm_diag},

      {FTM_1X_DYNAMIC_CHAIN_DRX_C,FTM_1X_DYNAMIC_CHAIN_DRX_C,ftm_diag},

      {FTM_WCDMA_DYNAMIC_CHAIN_DRX_C,FTM_WCDMA_DYNAMIC_CHAIN_DRX_C,ftm_diag},

      {FTM_GSM_DYNAMIC_CHAIN_DRX_C,FTM_GSM_DYNAMIC_CHAIN_DRX_C,ftm_diag},

      {FTM_TDS_DYNAMIC_CHAIN_DRX_C,FTM_TDS_DYNAMIC_CHAIN_DRX_C,ftm_diag},

      {FTM_GSM_EXTENDED_DYNAMIC_CHAIN_DRX_C,FTM_GSM_EXTENDED_DYNAMIC_CHAIN_DRX_C,ftm_diag},
#endif /* FEATURE_RFA_INTERNAL_FTM_CMD */
      {FTM_1X_C4_C,FTM_1X_C4_C,ftm_diag},

      {FTM_1X_C5_C,FTM_1X_C5_C,ftm_diag},
