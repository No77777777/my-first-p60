...
/*==============================================================================
  ...
  Copyright (c) 2002 - 2017 Qualcomm Technologies Incorporated. All Rights Reserved
...
/*==============================================================================
  ...
  $Header: //components/rel/rfa.mpss/4.11/rf/common/ftm/src/ftm.c#1 $
   
when       who     what, where, why
--------   ---     ------------------------------------------------------------- 
01/24/20   moh     RF MDUMP Changes
05/02/18   rs      Memory cleaup during 1X deinit
10/26/17   sk      Enable LTE B71 support
05/02/16   vc      Heap mem deallocation in FTM deinit
12/08/15   ak      Changes to do MCPM stop after rfmode exit
05/28/15   aro     Support to test ATLAS WCDMA driver in THOR baseline
05/12/15   ak      Implemenatation of the common FTM RF test framework
03/10/15   spa     Call MCPM enable only in one place 
...
ftm_rf_technology_type ftm_get_rf_technology ( void )
{
  ...
  switch( ftm_current_rf_mode )
  {
     ...
     case FTM_DB_RF_LTE_BC32:
     case FTM_DB_RF_LTE_BC125:
     case FTM_DB_RF_LTE_BC126:
     case FTM_DB_RF_LTE_BC127:
                                ...
                                prot_trans_type trans_id )
{
  ...
  ftm_gsm_rftest_init();
   ...
   /* sends the rf_start command to MCPM*/
  ftm_common_resource_config_request(TRUE);
                                  ...
                                  prot_trans_type trans_id )
{
  ...
  ftm_mode = DMSS_MODE;
   ...
   /* sends the rf_stop command to MCPM. Exit mode needs MCPM to be up. Hence moving this after Exit mode*/
  ftm_common_resource_config_request(FALSE);
...
)
{
  ...
  switch( phone_mode )
  {
    ...
    case FTM_PHONE_MODE_LTE_B125:
    case FTM_PHONE_MODE_LTE_B126:
    case FTM_PHONE_MODE_LTE_B127:
...
/*!
  @brief
  API to dump ftm variables using QSH MDUMP.
...
{
    qsh_mdump_collect_high((void *)&ftm_mode, sizeof(ftm_mode_type));    
    qsh_mdump_collect_high((void *)&ftm_task_cmd_q, sizeof(ftm_task_cmd_q));    
    qsh_mdump_collect_high((void *)&ftm_task_event_q, sizeof(ftm_task_event_q));    
#if 0
    qsh_mdump_collect_high((void *)&ftm_info_pkt, sizeof(ftm_info_pkt));    
    qsh_mdump_collect_high((void *)&ftm_fac_cal_params, sizeof(ftm_fac_cal_params));    
    qsh_mdump_collect_high((void *)ftm_common_data, sizeof(ftm_common_data_type));    
#endif
    ftm_rf_test_data_type *framework_data = get_ftm_rf_test_data();
	if(framework_data != NULL)
    {
        qsh_mdump_collect_high((void *)framework_data, sizeof(ftm_rf_test_data_type));
    }    
