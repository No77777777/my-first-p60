...
$Header: //components/rel/rfa.mpss/4.11/rf/common/ftm/src/ftm_common_dispatch.c#2 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
02/07/20   ssl     [CR2616180]Heap opt: making ssbi_script_size as zero 
...
08/25/16   sk      Set FBRX DM with correct FTM cal mode
08/18/16   nsh     Disard positive and invalid RxAGC value when doing FTM_RX_WAIT_DBM_V2
04/12/16   vr      Legacy Autopin Support for WTR3925
03/23/16   sk      Use Tx band for feeback query and pass FB device from RFC
02/18/16  zhh     send the calc command to APPs task to start the autoPin calc
01/20/16  hm       Fix for KW errors
01/19/16 ndb/hm    Added FTM_GET_TUNER_STATUS & FTM_PROGRAM_TUNER
...
08/25/15   zhw     Added LTE carrier mapping support in rx_wait_dbm_v2
08/20/15   zhw     kw fix
08/20/15   zhw     Added FTM device manager support for rx_wait_dbm / v2
07/20/15   fhuo    Add support for multi-bytes(up to 16) read and write
07/15/15   zhw     Enabled FTM device manager Bypass mode support
07/15/15   cdb     Add support to override Autopin operation
07/08/15   zhw     Added support for DR re-config command
06/25/15   lm      Added TxDAC msb cal codes dump and reprogram API intf for QRCT
06/19/15   br      Add support  for FTM  Response Packet Compression
06/19/15   yb      ACL clean-up
06/11/15   aro     Fixes to coex TH/AT for AT PreSi
06/10/15   yzw     Usd remapped device number in ftm_common_fetch_et_delay()
06/09/15   jak     Add ftm tech param for rfcommon_core_get_tx_default_device()
06/03/15   dw      FTM char related fixes
05/29/15   pkg     Enable XPT CHAR on TH.2.x PL's.
05/29/05   tsr     Add feature GSM to FTM_SET_CALIBRATION_STATE case
...
05/21/15   jfc     [Rx/Tx Split] Remap devices for ftm_common_get_rffe_device_ids() 
05/18/15   id      Add support for adjustable (variable) MLine Compression Point
05/08/15   adk     Added FTM API to query CL algo state
05/08/15   ak      [Rx/Tx Split]Remove Rx device hardcoding
04/30/15   ak      [Rx/Tx Split]Changes to support Cal
04/30/15   jak     Query RFC for remapped device in ftm_common_get_rffe_device_info()
04/24/15   tsr     FTM GSM CRAT Dev:Add Support for C4/C5
                   Query and pass the remmaped rx device in ftm_gsm_get_rssi_dbx16
...
03/30/15   zhw     Convert Tx device from Rx device input during XPT Cal
03/23/15    JJ      Latch DTR chain0 after chain1 to bypass DTR1 latch HW limitation
03/13/15   ska     Add FTM RF support for DGLNA feature
03/30/15   zhw     Convert Tx device from Rx device input during XPT Cal
03/19/15   al      KW error fix - Add array length as an input parameter
03/17/15   al      Modified get_rffe_device_IDs command
03/10/15   al      KW error fix
03/03/15   al      Add command to get rffe device IDs
01/25/15   kg      Added command to return FBRX sample captures
01/14/15   sd      Replace memcpy with safe memscpy
...
#ifdef FEATURE_LTE
extern rflte_nv_tbl_type* rflte_mc_get_nv_tbl_ptr(uint8 sub_id,uint32 carrier_idx,lm_handle_type lm_buffer_index);
#endif
...
boolean ftm_drds_reconfig_enable = FALSE;
...
ftm_rsp_pkt_type ftm_common_rf_mode_exit(ftm_common_pkt_type *cmd_ptr)
{
  ...
  for (device_loop = 0;device_loop < RFM_MAX_DEVICES; device_loop++ )
  {
    ...
    if((ftm_get_current_state(device_loop) == FTM_STATE_WCDMA) && (rfm_get_calibration_state() == TRUE))
    {
      FTM_MSG(FTM_HIGH, "ftm_rfmode_enter: Exiting WCDMA Devices" );
      ftm_rfmode_exit(device_loop, FTM_STATE_PARK);
    }	
...
ftm_rsp_pkt_type ftm_common_ant_div_switch(ftm_common_pkt_type *cmd_ptr)
{
  ftm_ant_div_switch_response_type* ant_div_switch_rsp = NULL;
  boolean success = TRUE;
  int device_loop = 0;
  
  ftm_rsp_pkt_type rsp = ftmdiag_create_new_pkt(sizeof(ftm_ant_div_switch_response_type));

  if(NULL == rsp.pkt_payload)
  {
    FTM_MSG_ERROR("Cannot allocate memory for response packet payload in as div switch, cmd id = %u", cmd_ptr->ftm_hdr.cmd_id, 0, 0);
    return rsp;
  }

  ant_div_switch_rsp = (ftm_ant_div_switch_response_type*) rsp.pkt_payload;

  // Copy data from request to response
  memscpy(ant_div_switch_rsp, sizeof(ftm_composite_cmd_header_type), 
          cmd_ptr, sizeof(ftm_composite_cmd_header_type));

  if (success == TRUE)
  {
    ant_div_switch_rsp->status = FALSE;
  }
  else
  {
    ant_div_switch_rsp->status = TRUE;
  }
...
DESCRIPTION
   ...
   !!! Input Device is always treated as from legacy RFC mapping. They are
   converted to new RFM Devices based on set mode prior to this API call.
...
ftm_rsp_pkt_type ftm_common_rx_wait_dbm_v2(ftm_common_pkt_type *cmd_ptr)
{
  ...
  rfm_device_enum_type remapped_rx_device = RFM_INVALID_DEVICE;
  ...
  do
  {
    /* LTE Code does not go through FTM Device Manager. For a given legacy device,
    LTE maps it to a carrier, and updates the FTM STM correspondingly */
    if ( rx_wait_req_data->rf_tech == FTM_RF_TECH_LTE )
    {
      #ifdef FEATURE_LTE
      rflte_mc_cell_idx_type mapped_carrier_index;
      rf_lte_dev_type device_type;

      /* Mapp Input device to LTE Carrier. RFM_DEVICE_0 in legacy target is
      always the PCELL PRX device. This can change to any device in new targets.
      PCELL PRX device may be RFM_DEVICE_4 for one band but RFM_DEVICE_2 for another
      band. This mapping is needed for users that still has FTM test tools that
      passes in the legacy device. */
      success = rflte_ftm_mc_legacy_device_to_carrier(rx_wait_req_data->device_id,
                                                      &mapped_carrier_index,
                                                      &device_type);

      if ( success == TRUE )
      {
        rfm_device_enum_type device;
        uint32 temp_carrier_idx = rflte_ftm_mc_sm_get_carrier_index();

        rflte_ftm_mc_sm_set_carrier_idx(mapped_carrier_index);
        
        if ( device_type == RF_LTE_PRX)
        {
          device = rflte_ftm_mc_sm_get_prx_device();        
        }
        else if ( device_type == RF_LTE_DRX)
        {
          device = rflte_ftm_mc_sm_get_drx_device();        
        }
        else
        {
          FTM_MSG_1(FTM_ERROR,"ftm_common_rx_wait_dbm_v2: Unsupported LTE device Type", device_type);
          success = FALSE;
        }

        if ( success == TRUE )
        {
          rx_wait_resp->rx_power = rflte_ftm_mc_get_rxagc_val(device); 
        }
    ...
    {
    /* This command is used as part of FTM RF test, where the device_id passed in is expected to
       be in legacy mapping. Call ftm device manager to get assigned device based on new mapping
       and dispatch it to techs */
    remapped_rx_device = ftm_device_manager_get_assigned_device(rx_wait_req_data->device_id);

    if ( remapped_rx_device == RFM_INVALID_DEVICE )
    {
      FTM_MSG_1(FTM_ERROR, "ftm_common_rx_wait_dbm_v2: Legacy device %d mapped to invalid device.",
                rx_wait_req_data->device_id );
      success = FALSE;
    }
    else
    {
      ...
      case FTM_RF_TECH_GSM:
        rx_wait_resp->rx_power = ftm_gsm_get_rssi_dbx16( remapped_rx_device );
        break;
      #endif /* FEATURE_GSM */
      ...
      case FTM_RF_TECH_CDMA:
        {
          ...
          success = rfm_1x_ftm_get_rx_agc_data(remapped_rx_device, &rx_agc_data_dbm10, RFM_CDMA_AGC_FORMAT__DBM10);
      ...
      case FTM_RF_TECH_WCDMA:
        {
          int16 rxLevelAgc;
          success = ftm_wcdma_get_rx_level_dbm(remapped_rx_device, &rxLevelAgc, &rx_wait_resp->rx_power);
        }
        break;
        #endif
      ...
      case FTM_RF_TECH_TDSCDMA:
        {
          ...
          success = ftm_tdscdma_get_rx_level_dbm(remapped_rx_device, &rx_level_agc, &rx_level_dbm10);
          rx_wait_resp->rx_power = rx_level_dbm10;

	  if (!success)
	  {
	     FTM_MSG_HIGH("Fetch RxAGC Failed, continue the loop",0,0,0);
	     continue;
	  }

	  if (rx_level_dbm10>=0)
	  {
	     FTM_MSG_HIGH("RxAGC Value invalid, continue the loop",0,0,0);
	     continue;
          }
...
ftm_rsp_pkt_type ftm_common_rx_wait_dbm(ftm_common_pkt_type *cmd_ptr)
{
  ...
  do
  {
    ...
    rfm_device_enum_type legacy_device = 
          ftm_common_convert_subsys_id_to_legacy_device( rx_wait_req_data->rf_tech );

    if ( legacy_device >= RFM_MAX_WAN_DEVICES )
    {
      FTM_MSG_1(FTM_ERROR, "ftm_common_rx_wait_dbm_v2: unsupported Legacy chain %d.",
                rx_wait_req_data->rf_tech );
      success = FALSE;

    }
    else
    {
      remapped_rx_device = ftm_device_manager_get_assigned_device(legacy_device); 
...
)
{
  ...
  case FTM_RF_TECH_WCDMA:
      #ifdef FEATURE_WCDMA
      #ifdef ATLAS_RF_WCDMA_PRESI
      ovr_params->device = rfm_wcdma_ftm_tx_get_rfm_device();
      #else
      ovr_params->device = ftm_wcdma_get_cur_tx_device();
      #endif
...
)
{
  ...
  rfm_device_enum_type device, txdev;
  ...
  txdev = rfcommon_core_get_tx_default_device(device,ftm_common_ftm_tech_to_rfm_mode(ftm_rf_tech));
  ...
  api_status &= ftm_common_xpt_get_curr_tx_override( txdev, 
                                                     &curr_tx_ovr );

  /* Pass in Rx device to ftm_common_xpt_txagc_verride. Techs will do conversion
  to Tx device inside in each tech's subroutines */
  ...
  if(curr_tx_ovr.pa_state < RFM_PA_GAIN_STATE_NUM)
  {
  ...
  if(api_status == TRUE)
  {
        ...
        (action_type == XPT_DPD_ACTION_INIT))
    {
      ...
      api_status &= 
        rf_common_xpt_load_dpd_tbls(txdev, 
                                    NULL, 
                                    NULL, 
                                    0, 
                                    0, 
  ...
  if(api_status == TRUE)
  {
        ...
        (action_type == XPT_DPD_ACTION_COMMIT))
    {
      ...
      api_status &= ftm_common_xpt_set_curr_tx_override( txdev, 
                                                         &curr_tx_ovr );
    }
  }
...
)
{
  ...
  uint32 tx_nv_container = RFLTE_NV_TX_CONTAINER_DEFAULT;
  #endif
  #ifdef FEATURE_TDSCDMA
  ...
  if (ftm_rf_tech == FTM_RF_TECH_CDMA)
  {
    ...
    if (xpt_type == XPT_TYPE_EPT)
    {
      ...
      here we are picking the first available tx device as there are no device
      information passed in this API */
      mline_params.env_scale = (int32)(rf_cdma_xpt_get_cfg_param( rf_cdma_data_get_curr_tx_device(), EPT_CFG_CMN_EVN_SCALE_UNITY ));
      FTM_MSG_1(FTM_LOW, "ftm_common_xpt_mline_compute: "
    ...
    else if (xpt_type == XPT_TYPE_ET)
    {
      ...
      here we are picking the first available tx device as there are no device
      information passed in this API */
      mline_params.env_scale = (int32)(rf_cdma_xpt_get_cfg_param( rf_cdma_data_get_curr_tx_device(), ET_CFG_CMN_ENV_SCALE_UNITY ));
      FTM_MSG_1(FTM_LOW, "ftm_common_xpt_mline_compute: "
                                         "CDMA ET Env Scale = %d",
                                         mline_params.env_scale);
    }
  ...
  if (ftm_rf_tech == FTM_RF_TECH_LTE)
  {
    ...
    rflte_state_get_txlm_buffer_idx(sub_id,current_carrier_idx,&txlm_buffer_idx);
    rflte_nv_tbl_ptr = rflte_mc_get_nv_tbl_ptr(sub_id,current_carrier_idx,txlm_buffer_idx);
    if (rflte_nv_tbl_ptr != NULL)
    {
      tx_nv_container = rflte_mc_get_tx_nv_container(current_carrier_idx,txlm_buffer_idx);
      if (tx_nv_container >= LTE_TX_MAX_NUM_CONTAINERS)
      {
        MSG_2 ( MSG_SSID_RF, MSG_LEGACY_ERROR,
                            "ftm_common_xpt_mline_compute : ERROR : "
                             "Invalid NV Container for Tx : %d "
                             "Initializing to default value : %d",
                             tx_nv_container,
                             RFLTE_NV_TX_CONTAINER_DEFAULT);
          
        tx_nv_container    = RFLTE_NV_TX_CONTAINER_DEFAULT;
      }
      if (xpt_type == XPT_TYPE_EPT)
      {
        mline_params.env_scale =\
			(int32)((rflte_nv_tbl_ptr->tx_path_nv_data[tx_nv_container]).\
                                        tx_xpt_dpd_config_params[EPT_CFG_CMN_EVN_SCALE_UNITY]);
        FTM_MSG_1(FTM_LOW, "ftm_common_xpt_mline_compute: "
                                           "LTE EPT Env Scale = %d",
                                           mline_params.env_scale);
      }
      else if (xpt_type == XPT_TYPE_ET)
      {
        mline_params.env_scale =\
			(int32)((rflte_nv_tbl_ptr->tx_path_nv_data[tx_nv_container]).\
                                            tx_xpt_dpd_config_params[ET_CFG_CMN_ENV_SCALE_UNITY]);
        FTM_MSG_1(FTM_LOW, "ftm_common_xpt_mline_compute: "
                                           "LTE ET Env Scale = %d",
                                           mline_params.env_scale);
      }
  ...
  /* With FTM API, only MLine Bias Option of 0 will be supported */
  mline_params.mline_bias_opt = DATA_MLINE_CALC_INTERP_BIAS;
...
)
{
  ...
  case FTM_RF_TECH_WCDMA:
    #ifdef FEATURE_WCDMA
    #ifdef ATLAS_RF_WCDMA_PRESI
    tx_device = rfc_common_get_preferred_associated_tx_device(iq_capt_params->device);
    fb_device = iq_capt_params->device_fb;
    #else
    remapped_rx_device = ftm_wcdma_get_remapped_rx_device(iq_capt_params->device);
    tx_device = ftm_wcdma_get_remapped_tx_device(remapped_rx_device);
    fb_device = ftm_wcdma_get_remapped_rx_device(iq_capt_params->device_fb);
    #endif
      {
      /* Perform capture */
      if(api_status == TRUE)
      {
        api_status &=
          ftm_wcdma_xpt_iq_capture(tx_device, 
                                   fb_device,
                                   (uint8)XPT_CAPTURE_DELAY,
                                   iq_capt_params->num_samples,
  ...
  case FTM_RF_TECH_LTE:
    #ifdef FEATURE_LTE
    {
      if(api_status == TRUE)
      {
         ...
         rfm_device_enum_type rfc_rx_device = RFM_INVALID_DEVICE;
         rfm_device_enum_type rfc_fb_device = RFM_INVALID_DEVICE;
         ...
         source_device_info.device = iq_capt_params->device;
         source_device_info.tech = RFM_LTE_FTM_MODE;
         source_device_info.band = rflte_ftm_mc_sm_get_band();
         ...
         rfc_rx_device = remapped_device_info.device;
         //rfc_fb_device = rfc_common_get_preferred_associated_rx_device( rfc_rx_device );
         //lte_tx_device = rfc_common_get_preferred_associated_tx_device( rfc_rx_device );

	 rfcom_band_type_u band_u;
	 uint64 sys_band;
	 band_u.lte_band = source_device_info.band;
         sys_band = rfcommon_core_convert_rfcom_band_to_sys_band(RFM_LTE_MODE , band_u);
         lte_tx_device = rfc_common_get_preferred_tx_device( (sys_band_class_e_type) sys_band);

         rfc_fb_device = rfc_lte_get_fb_device(lte_tx_device, band_u.lte_band,rflte_ftm_mc_sm_carrier_get_drx_band(RFLTE_MC_PCELL_IDX));

				 
         FTM_MSG_4( FTM_ERROR, "XPT_CHAR:ftm_common_fetch_et_delay:dev passed: %d, rfc rx dev :%d, rfx fb dev:%d, tx dev:%d ",
		   iq_capt_params->device, rfc_rx_device, rfc_fb_device, lte_tx_device);

        api_status &= ftm_lte_xpt_iq_capture(lte_tx_device, 
                                             rfc_fb_device,
                                             iq_capt_params->num_samples,
                                             iq_capt_params->first_trig,
                                             iq_capt_params->last_trig,
  ...
  if (ftm_rf_tech == FTM_RF_TECH_LTE)
  {
     ...
     current_band = rflte_core_get_band_from_tx_uarfcn(rflte_state_get_curr_chan_tx(carrier_idx,txlm_buffer_idx));
     rfdevice_lte_tx_get_et_delay_compensation( RFCOM_TRANSCEIVER_0,
                                             current_band,
                                             rflte_state_get_bw_tx(carrier_idx,txlm_buffer_idx),
                                             &delay_comp_x10ns );
  }
#endif 
  ...
  if (ftm_rf_tech == FTM_RF_TECH_WCDMA)
  {
    ...
    rfm_wcdma_ftm_tx_get_xpt_delay_comp( tx_device, radio_state.rf_band, &delay_comp_x10ns );
    #else
    rfdevice_wcdma_tx_get_et_delay_compensation(tx_device,
                                                ftm_curr_wcdma_mode,
                                                &delay_comp_x10ns);
    #endif
  ...
  {
#if 0 
    rfdevice_tdscdma_tx_get_et_delay_compensation(RFCOM_TRANSCEIVER_0,
                                                  ftm_curr_tdscdma_mode,
                                                  &delay_comp_x10ns);
#endif
  }
#endif
...
)
{
  ...
  case FTM_RF_TECH_WCDMA:
      ...
      {
        rfm_device_enum_type remapped_rx_device, tx_device, fb_device;
        remapped_rx_device = ftm_wcdma_get_remapped_rx_device(iq_capt_params->device);
        tx_device = ftm_wcdma_get_remapped_tx_device(remapped_rx_device);
        fb_device = ftm_wcdma_get_remapped_rx_device(iq_capt_params->device_fb);
      /* Perform capture */
  ...
  case FTM_RF_TECH_LTE:
    #ifdef FEATURE_LTE
    {
      ...
      fb_device = rfc_lte_get_fb_device(iq_capt_params->device, rflte_ftm_mc_sm_get_tx_band(),rflte_ftm_mc_sm_carrier_get_drx_band(RFLTE_MC_PCELL_IDX));
      if(api_status == TRUE)
      {
  ...
  case FTM_RF_TECH_LTE:
    #ifdef FEATURE_LTE
    {
	 ...
	 band_u.lte_band = source_device_info.band;
        ...
        FTM_MSG_4( FTM_ERROR, "XPT_CHAR:ftm_common_trigger_ept_iq_cap:dev passed: %d, rfc rx dev :%d, rfx fb dev:%d, tx dev:%d ",
               iq_capt_params->device, rfc_rx_device, rfc_fb_device, lte_tx_device);
        api_status &= ftm_lte_xpt_iq_capture(lte_tx_device, 
                                             rfc_fb_device,
                                             iq_capt_params->num_samples,
...
)
{
  ...
  lut_info.tx_gain_script = rf_buffer_create(0, 
                                             RFCMN_TXLIN_AGC_WTR_DATA_TASK_NUM_TRANS, 
                                             0);

  lut_info.bbf_script =  rf_buffer_create( 0, 
                                           RFCMN_TXLIN_AGC_WXE_BBF_TASK_NUM_TRANS, 
                                           0 );

  lut_info.trigger_script = rf_buffer_create( 0, 
  ...
  memscpy( &(rsp_pkt->lut_ids_placeholder),sizeof(uint16)*rsp_pkt->num_valid_paStates, &lut_id_results[0], sizeof(lut_id_results));
...
FUNCTION ftm_common_get_tuner_status

DESCRIPTION

This function returns the tune status of all active and inactive tuners
...
SIDE EFFECTS
   None.
===========================================================================*/
ftm_rsp_pkt_type ftm_common_get_tuner_status
(
  ftm_common_pkt_type *cmd_ptr
)
{
  ftm_tuner_status_resp_pkt_type *resp_pkt = NULL;
  ...
  uint8 device_instance = 0; 
  uint8 device_id = 0xFF;
  boolean ret_val = FALSE;


  uint8 tune_code_size = 0;
  uint8 tune_code_idx = 0;
  uint8 tune_code[8];
  
  memset(tune_code,0xFF,sizeof(tune_code));

  ftm_rsp_data = ftmdiag_create_new_pkt(sizeof(ftm_tuner_status_resp_pkt_type));

  if(cmd_ptr == NULL)
  {
    FTM_MSG_ERROR( "ftm_common_pkt_type:req pkt is NULL"
                   "in ftm_common_get_tuner_status()",0,0,0);
    return ftm_rsp_data;
  }
  
  if( ftm_rsp_data.pkt_payload == NULL)
  {
    FTM_MSG_ERROR( "Mem alloc FAILED for response packet payload. ftm_common_get_tuner_status",
                   0, 0, 0);
    return ftm_rsp_data;
  }

  resp_pkt = (ftm_tuner_status_resp_pkt_type *)ftm_rsp_data.pkt_payload;

  if(resp_pkt == NULL)
  {
    FTM_MSG_ERROR( "ftm_tuner_status_resp_pkt_type:resp pkt is NULL"
                   "in ftm_common_get_tuner_status()",0,0,0);
    return ftm_rsp_data;
  }
  /* Init response packet */
  memset( resp_pkt, 0, sizeof( ftm_tuner_status_resp_pkt_type));

  resp_pkt->cmd     = cmd_ptr->diag_hdr.cmd_code;
  resp_pkt->sub_sys = cmd_ptr->diag_hdr.subsys_id;
  resp_pkt->ftm_cmd = cmd_ptr->diag_hdr.subsys_cmd_code;

  while ( device_instance < RFC_MAX_DEVICE_INSTANCES )
  {
    tuner_device_obj_list = rfc_common_get_cmn_rf_device_object(RFDEVICE_TUNER,
                                                               device_instance); 
    if((tuner_device_obj_list != NULL ) && (tuner_count < RFC_MAX_DEVICE_INSTANCES_PER_BAND) )
    {
      rfdevice_antenna_tuner_get_tune_code(tuner_device_obj_list, &tune_code_size, tune_code,&device_id);

      resp_pkt->tuner_status[tuner_count].tune_code_size = tune_code_size;
      
      resp_pkt->tuner_status[tuner_count].tuner_id = device_id;

      for(tune_code_idx=0; (tune_code_idx<tune_code_size) && (tune_code_idx<8) ;tune_code_idx++)
      {
        resp_pkt->tuner_status[tuner_count].tune_code[tune_code_idx] = tune_code[tune_code_idx];
      }
      resp_pkt->tuner_status[tuner_count].nvg_status = 
      rfdevice_antenna_tuner_get_nvg_status(tuner_device_obj_list);

      tuner_count++;

      /*Reset all the local variables*/
      tuner_device_obj_list = NULL;
      device_id = 0xFF;
      tune_code_size = 0;
      memset(tune_code,0xFF,sizeof(tune_code));
  ...
  resp_pkt->num_tuners = tuner_count;

  if(tuner_count == 0)
  {
    RF_MSG(RF_ERROR,"NO Tuner Device found in ftm_common_tuner_get_tune_status()");
  }

  return ftm_rsp_data;
} /* ftm_common_get_tuner_status() */

/*===========================================================================

FUNCTION ftm_common_program_tuner

DESCRIPTION

This function programs the tuner with the given tune code
...
SIDE EFFECTS
   None.
===========================================================================*/
ftm_rsp_pkt_type ftm_common_program_tuner
(
  ftm_common_pkt_type *cmd_ptr
)
{
  ftm_tuner_program_req_pkt_type  *req_pkt = NULL;
  ftm_tuner_program_resp_pkt_type *resp_pkt = NULL;
  ...
  ftm_rsp_data = ftmdiag_create_new_pkt(sizeof(ftm_tuner_program_resp_pkt_type));

  if(cmd_ptr == NULL)
  {
    FTM_MSG_ERROR( "ftm_common_pkt_type:req pkt is NULL"
                   "in ftm_common_program_tuner()",0,0,0);
    return ftm_rsp_data;
  }

  req_pkt = (ftm_tuner_program_req_pkt_type *)(&(cmd_ptr->ftm_extended_params));

  if(req_pkt == NULL)
  {
    FTM_MSG_ERROR( "ftm_tuner_program_req_pkt_type:req pkt is NULL"
                   "in ftm_common_program_tuner()",0,0,0);
    return ftm_rsp_data;
  }
  ...
  resp_pkt = (ftm_tuner_program_resp_pkt_type *)ftm_rsp_data.pkt_payload;

  if(resp_pkt == NULL)
  {
    FTM_MSG_ERROR( "ftm_tuner_program_resp_pkt_type:resp pkt is NULL"
                   "in ftm_common_program_tuner()",0,0,0);
    return ftm_rsp_data;
  }

  /* Init response packet */
  memset( resp_pkt, 0, sizeof( ftm_tuner_program_resp_pkt_type));

  resp_pkt->cmd     = cmd_ptr->diag_hdr.cmd_code;
  resp_pkt->sub_sys = cmd_ptr->diag_hdr.subsys_id;
  resp_pkt->ftm_cmd = cmd_ptr->diag_hdr.subsys_cmd_code;

  FTM_MSG_HIGH ("FTM_PROGRAM_TUNER: NumTuners:%d",req_pkt->num_tuners,0,0);

  for ( num_tuners = 0; num_tuners < req_pkt->num_tuners && num_tuners < RFC_MAX_DEVICE_INSTANCES_PER_BAND;
                        num_tuners++)
  {
    /*Get the tuner object */
    tuner_device_obj_list = rfc_common_get_cmn_rf_device_object(RFDEVICE_TUNER,
                                                              req_pkt->tuner_program_data[num_tuners].tuner_id); 
    
    FTM_MSG_HIGH ("FTM_PROGRAM_TUNER: TunerNo:%d,TunerID:%d,NVG:%d",num_tuners,req_pkt->tuner_program_data[num_tuners].tuner_id,
    req_pkt->tuner_program_data[num_tuners].nvg_enable);
    
    if(tuner_device_obj_list != NULL )
    {
      status = rfdevice_antenna_tuner_program_tuner( tuner_device_obj_list,
                                                     req_pkt->tuner_program_data[num_tuners].nvg_enable,
                                                     req_pkt->tuner_program_data[num_tuners].tune_code_size,
                                                     req_pkt->tuner_program_data[num_tuners].tune_code);
    }
    else
    {
      FTM_MSG_ERROR( "tuner_device_obj_list is NULL for num_tuners:%d,TunerID:%d",num_tuners,
                     req_pkt->tuner_program_data[num_tuners].tuner_id,0);
     
    }
    resp_pkt->status[num_tuners] = status;
    status = FALSE;
  }
  return ftm_rsp_data;
} /* ftm_common_tuner_get_tune_status() */
...
{  
  ...
  rsp_pkt.cmd = FTM_RSP_MAX;
  rsp_pkt.pkt_len = 0;
  rsp_pkt.delete_payload = FALSE;
  rsp_pkt.pkt_payload = NULL;
  

  FTM_MSG_HIGH("Deprecated Command",0,0,0);
  return rsp_pkt;
}
...
FUNCTION ftm_common_process_get_tuner_cl_algo_state

DESCRIPTION
   This function handles requests to get tuner CL algorithm state
...
===========================================================================*/
ftm_rsp_pkt_type
ftm_common_process_get_tuner_cl_algo_state
(
  ftm_common_pkt_type *cmd_ptr
)
{
  ftm_get_tuner_info_request_type  *request = NULL;
  ftm_get_tuner_info_response_type *tuner_info_response = NULL;
  /* Packet returned to Diag */
  ftm_rsp_pkt_type ftm_rsp_data = {FTM_RSP_DO_LEGACY, 0, NULL, FALSE};

  uint8 rfm_dev_index = 0;
  ftm_rfstate_enum_type tech = FTM_STATE_UNKNOWN;
  void *tuner_mgr_obj = NULL;
  boolean ret_val = FALSE;

  request = (ftm_get_tuner_info_request_type *)(&(cmd_ptr->ftm_extended_params));

  if( request == NULL)
  {
    FTM_MSG_ERROR( "ftm_get_tuner_info_request_type *request is NULL",
                   0,0,0);
    return ftm_rsp_data;
  }
  if( RFM_MAX_DEVICES < RFM_MAX_WAN_DEVICES)
  {
    FTM_MSG_ERROR( "ERROR: RFM_MAX_DEVICES = %d < %d = RFM_MAX_WAN_DEVICES",
                   RFM_MAX_DEVICES, RFM_MAX_WAN_DEVICES, 0);
    return ftm_rsp_data;
  }

  ftm_rsp_data = ftmdiag_create_new_pkt( sizeof( ftm_get_tuner_info_response_type));

  if( ftm_rsp_data.pkt_payload == NULL)
  {
    FTM_MSG_ERROR( "Mem alloc FAILED for response packet payload. cmd id = %u",
                   request->ftm_cmd, 0, 0);
    return ftm_rsp_data;
  }

  tuner_info_response = (ftm_get_tuner_info_response_type *)ftm_rsp_data.pkt_payload;

  /* Init response packet */
  memset( tuner_info_response, 0, sizeof( ftm_get_tuner_info_response_type));

  /* Copy data from request to response */
  tuner_info_response->cmd     = request->cmd;
  tuner_info_response->sub_sys = request->sub_sys;
  tuner_info_response->ftm_cmd = request->ftm_cmd;

  tech = (ftm_rfstate_enum_type)(request->tech);

  for( rfm_dev_index = 0; rfm_dev_index < (uint8)RFM_MAX_WAN_DEVICES; rfm_dev_index++)
  {
    tuner_mgr_obj = ftm_common_get_tuner_mgr_object((rfm_device_enum_type)rfm_dev_index, tech);

    ret_val = FALSE;
    if( tuner_mgr_obj != NULL)
    {
      ret_val = rfcommon_atuner_get_cl_algo_state(
                     (rfcommon_atuner_intf *)tuner_mgr_obj);
    }
    tuner_info_response->cl_algo_state[ rfm_dev_index] = (uint8)ret_val;

    FTM_MSG_3( FTM_MED, "ftm_common_process_get_tuner_cl_algo_state: tech %d, "
                        "rfm_device %d, cl_algo_state %d (1/0 Enable/Disable)",
                        tech, rfm_dev_index,
                        tuner_info_response->cl_algo_state[rfm_dev_index]);
  }
  tuner_info_response->num_rfm_devices = rfm_dev_index;
...
)
{
    ...
    rfm_device_enum_type remapped_rx_device; /* remapped rx device */
    rfm_device_enum_type remapped_tx_device; /* remapped tx device */	
	...
	source_device_info.device = rfm_device;
	source_device_info.tech = ftm_common_ftm_tech_to_rfm_mode(rf_tech);
	source_device_info.rxtx = RFC_CONFIG_RX; /* use Rx config to get remapped Rx device */
	source_device_info.band = band;
	/* use 0 for alt_path by default , we should use rfcmn_concurrency_mgr_update_rx_path() to get 
	alt path but we are not clear what device ID we should pass in for rfcmn_concurrency_mgr_update_rx_path()
	, should we use remapped device or unmapped device? */  
	source_device_info.alt_path = 0; 
	
	/* query RFC for remapped device */
	if ( rfc_common_get_remapped_device_info( &source_device_info , &remapped_device_info ) == FALSE)
	{
	  FTM_MSG( FTM_ERROR,"rfc get remapped device info failed ");
      rffe_device_info_resp->status = 0x1;
      return ftm_rsp_data;  	  
	}
	remapped_rx_device = remapped_device_info.device;	
	remapped_tx_device = rfc_common_get_preferred_associated_tx_device( remapped_rx_device );
        /*In Cal Overwrite the tranceiver based Tx device to the preferred device to support Split Rx -Tx Cal on 2 Diff tranceiver*/
	remapped_tx_device = ftm_common_get_preferred_tx_device(band, rf_tech);
	
	FTM_MSG_5( FTM_HIGH, "rf mode: %d, rfm_device: %d, rffe_device_type: %d , remapped_rx_device = %d , remapped_tx_device = %d ", rf_mode, rfm_device, rffe_device_type , remapped_rx_device , remapped_tx_device );
        ...
        case FTM_RF_TECH_CDMA:
            {
                ...
                remapped_tx_device = rfc_common_get_preferred_tx_device ( band );
				
                if (rffe_device_type == RFFE_DEVICE_PA)
                {
                    if( FALSE == rfc_cdma_get_rffe_device_info(remapped_tx_device, band, RFDEVICE_PA, 
                ...
                {
                    if( FALSE == rfc_cdma_get_rffe_device_info(remapped_tx_device, band, RFDEVICE_ASM, 
                                                        rffe_device_info_resp->mfg_id, rffe_device_info_resp->prd_id, 
                                                        rffe_device_info_resp->rev_id))
                    {
                        FTM_MSG( FTM_ERROR,"rfc_cdma_get_pa_device_info failed");
                ...
                {
                    if( FALSE == rfc_cdma_get_rffe_device_info(remapped_tx_device, band, RFDEVICE_PAPM, 
                                                        rffe_device_info_resp->mfg_id, rffe_device_info_resp->prd_id, 
                                                        rffe_device_info_resp->rev_id))
                    {
        ...
        case FTM_RF_TECH_GSM:
            {
                    ...
                    if (rffe_device_type == RFFE_DEVICE_PA)
                    {
                        if( FALSE == rfc_gsm_get_rffe_device_info(remapped_tx_device, band, RFDEVICE_PA,
                                                            rffe_device_info_resp->mfg_id, rffe_device_info_resp->prd_id, 
                                                            rffe_device_info_resp->rev_id))
                        {
                    ...
                    {
                        if( FALSE == rfc_gsm_get_rffe_device_info(remapped_rx_device, band, RFDEVICE_ASM,
                                                            rffe_device_info_resp->mfg_id, rffe_device_info_resp->prd_id, 
                                                            rffe_device_info_resp->rev_id))
                        {
                    ...
                    {
                        if( FALSE == rfc_gsm_get_rffe_device_info(remapped_tx_device, band, RFDEVICE_PAPM,
                                                            rffe_device_info_resp->mfg_id, rffe_device_info_resp->prd_id, 
                                                            rffe_device_info_resp->rev_id))
                        {
                            FTM_MSG( FTM_ERROR,"rfc_gsm_get_rffe_device_info failed");
        ...
        case FTM_RF_TECH_WCDMA:
            {
                ...
                if (rffe_device_type == RFFE_DEVICE_PA)
                {
                    if( FALSE == rfdevice_wcdma_pa_get_device_info(remapped_tx_device, band, 
                                                    rffe_device_info_resp->mfg_id, rffe_device_info_resp->prd_id, 
                ...
                {
                    if( FALSE == rfdevice_wcdma_asm_get_device_info(remapped_rx_device, band, 
                                                    rffe_device_info_resp->mfg_id, rffe_device_info_resp->prd_id, 
                                                    rffe_device_info_resp->rev_id, RFC_COMMON_MAX_DEVICE_INSTANCES_PER_BAND))
                    {
                ...
                {
                    if( FALSE == rfdevice_wcdma_papm_get_device_info(remapped_tx_device, band, 
                                                    rffe_device_info_resp->mfg_id, rffe_device_info_resp->prd_id, 
                                                    rffe_device_info_resp->rev_id, RFC_COMMON_MAX_DEVICE_INSTANCES_PER_BAND))
                    {
        ...
        case FTM_RF_TECH_LTE:
            {
              ...
              band_u.lte_band = source_device_info.band;
              sys_band = rfcommon_core_convert_rfcom_band_to_sys_band(RFM_LTE_MODE , band_u);
              remapped_tx_device = rfc_common_get_preferred_tx_device( (sys_band_class_e_type) sys_band);

                /*right now we only support query PA device. The support for other devices can be added later on*/
                if (rffe_device_type == RFFE_DEVICE_PA)
                {
                    if( FALSE == rfdevice_lte_pa_get_device_info(remapped_tx_device, band, 
                                                    rffe_device_info_resp->mfg_id, rffe_device_info_resp->prd_id, 
                                                    rffe_device_info_resp->rev_id, RFC_COMMON_MAX_DEVICE_INSTANCES_PER_BAND))
                    {
                ...
                {
                    if( FALSE == rfdevice_lte_asm_get_device_info(remapped_rx_device, band, 
                                                    rffe_device_info_resp->mfg_id, rffe_device_info_resp->prd_id, 
                                                    rffe_device_info_resp->rev_id, RFC_COMMON_MAX_DEVICE_INSTANCES_PER_BAND))
                    {
                ...
                {
                    if( FALSE == rfdevice_lte_papm_get_device_info(remapped_tx_device, band, 
                                                    rffe_device_info_resp->mfg_id, rffe_device_info_resp->prd_id, 
                                                    rffe_device_info_resp->rev_id, RFC_COMMON_MAX_DEVICE_INSTANCES_PER_BAND))
                    {
        ...
        case FTM_RF_TECH_TDSCDMA:
            {
                ...
                if (rffe_device_type == RFFE_DEVICE_PA)
                {
                    if( FALSE == rfdevice_tdscdma_pa_get_device_info(remapped_tx_device, band, 
                                                    rffe_device_info_resp->mfg_id, rffe_device_info_resp->prd_id, 
                                                    rffe_device_info_resp->rev_id, RFC_COMMON_MAX_DEVICE_INSTANCES_PER_BAND))
                    {
                ...
                {
                    if( FALSE == rfdevice_tdscdma_asm_get_device_info(remapped_rx_device, band, 
                                                    rffe_device_info_resp->mfg_id, rffe_device_info_resp->prd_id, 
                                                    rffe_device_info_resp->rev_id, RFC_COMMON_MAX_DEVICE_INSTANCES_PER_BAND))
                    {
                ...
                {
                    if( FALSE == rfdevice_tdscdma_papm_get_device_info(remapped_tx_device, band, 
                                                    rffe_device_info_resp->mfg_id, rffe_device_info_resp->prd_id, 
                                                    rffe_device_info_resp->rev_id, RFC_COMMON_MAX_DEVICE_INSTANCES_PER_BAND))
                    {
...
/*===========================================================================
FUNCTION FTM_COMMON_GET_NUM_DEVICES

DESCRIPTION
   This routine gets the actual number of devices and populates response packet
   with the number of devices and each device's IDs (mfr_id, prd_id, rev_id)
...
RETURN VALUE
   None
===========================================================================*/
void ftm_common_get_num_devices
(
    uint16 *mfg_id,
    uint16 *prd_id,
    uint16 *rev_id,
	ftm_get_rffe_device_ids_response *rffe_device_ids_resp
)
{
    uint16 dev_idx = 0;

	if (rffe_device_ids_resp == NULL)
	{
	  FTM_MSG_ERROR("NULL rffe device IDs response", 0, 0, 0);
	  return;
	}

	if ((mfg_id == NULL) || (prd_id == NULL) || (rev_id == NULL))
	{
	  FTM_MSG_ERROR("NULL device ID array: mfg_id = %x prd_id = %x rev_id = %x", mfg_id, prd_id, rev_id);
      rffe_device_ids_resp->max_num_devices = dev_idx;
	  return;
	}

	while ((dev_idx < RFC_MAX_DEVICE_INSTANCES_PER_BAND) && (mfg_id[dev_idx] != 0)) 
	{
	  rffe_device_ids_resp->mfg_prd_rev_id[dev_idx][0] = mfg_id[dev_idx];
	  rffe_device_ids_resp->mfg_prd_rev_id[dev_idx][1] = prd_id[dev_idx];
	  rffe_device_ids_resp->mfg_prd_rev_id[dev_idx][2] = rev_id[dev_idx];
	  dev_idx++;
	}
...
/*!
  @brief
  Get RFFE dvice IDs {mfg_id, prd_id, rev_id} for FTM cmd FTM_GET_RFFE_DEVICE_IDS

  @details
  This function will return the actual number of devices per band, and RFFE device (PA, ASM, PAPM) information
  {mfg_id, prd_id, rev_id} based on FTM rf mode and rfm device number 

  @param *request
  Request Data Packet

  @return
  Response packet to the diag for FTM cmd FTM_GET_RFFE_DEVICE_IDS
*/
/*----------------------------------------------------------------------------*/
ftm_rsp_pkt_type
ftm_common_get_rffe_device_ids
(
  ftm_get_rffe_device_info_request *request
)
{
    /* Response Packets */
    ftm_rsp_pkt_type ftm_rsp_data;          /* Packet returned to Diag */
    ftm_get_rffe_device_ids_response * rffe_device_ids_resp = NULL;
    
    ftm_mode_id_type rf_mode;
    rfm_device_enum_type rfm_device;
    rffe_device_enum_type rffe_device_type;
    uint8 band = 0;
	uint8 rffe_device_ids_resp_hdr_len = 10; /*number of bytes in rffe_device_ids_resp header: count from cmd to max_num_devices */
    ftm_rf_technology_type rf_tech;

	uint16 mfg_id[RFC_MAX_DEVICE_INSTANCES_PER_BAND] = {0,0,0,0,0,0,0};
	uint16 prd_id[RFC_MAX_DEVICE_INSTANCES_PER_BAND] = {0,0,0,0,0,0,0};
	uint16 rev_id[RFC_MAX_DEVICE_INSTANCES_PER_BAND] = {0,0,0,0,0,0,0};

    ftm_rsp_data = ftmdiag_create_new_pkt(sizeof(ftm_get_rffe_device_ids_response));
  ...
  boolean rfc_remapped_status;
    ...
    rffe_device_ids_resp = (ftm_get_rffe_device_ids_response*) ftm_rsp_data.pkt_payload;
    /*initialize the response packet*/
    memset(rffe_device_ids_resp, 0, sizeof(ftm_get_rffe_device_ids_response));
    
    /*Copy data from request to response*/
    rffe_device_ids_resp->cmd = request->cmd;
    rffe_device_ids_resp->sub_sys = request->sub_sys;
    rffe_device_ids_resp->mode = request->mode;
    rffe_device_ids_resp->ftm_cmd = request->ftm_cmd;
    ...
    {
      FTM_MSG_1( FTM_ERROR,"rf mode not valid, mode %d ", rf_mode);
      rffe_device_ids_resp->status = 0x1;
      return ftm_rsp_data;
    }
    ...
    /*so far, driver  only support querying PA, PAPM and Rx ASM. 
	  The support for other RFFE devices will be added when it's needed*/
    if( (rffe_device_type < RFFE_DEVICE_PA) && (rffe_device_type > RFFE_DEVICE_PAPM)) 
    {
      FTM_MSG_1( FTM_ERROR,"rffe device type %d is not valid", rffe_device_type);     
      rffe_device_ids_resp->status = 0x1;
  ...
  remapped_rx_device = remapped_device_info.device; 
  /*Override the transceiver based Tx device with the actual tx device*/
  remapped_tx_device = ftm_common_get_preferred_tx_device(band, rf_tech);
  ...
  FTM_MSG_5( FTM_HIGH, "rf mode: %d, rfm_device: %d, rffe_device_type: %d , remapped_rx_device = %d , remapped_tx_device = %d ", rf_mode, rfm_device, rffe_device_type , remapped_rx_device , remapped_tx_device );
    ...
    switch(rf_tech)
    {
        ...
        if ( FALSE == rfc_cdma_get_rffe_device_info(remapped_tx_device, band, RFDEVICE_PA, 
					...
					{
					  ftm_common_get_num_devices(&mfg_id[0], &prd_id[0],&rev_id[0], rffe_device_ids_resp);
					}
                }
                if (rffe_device_type == RFFE_DEVICE_ASM)
                {
        ...
        case FTM_RF_TECH_WCDMA:
            {
                ...
                if (rffe_device_type == RFFE_DEVICE_PA)
                {
                  if ( 
                      #ifdef ATLAS_RF_WCDMA_PRESI
                      FALSE == rfm_wcdma_ftm_get_rffe_device_info(remapped_tx_device, (rfm_wcdma_band_type)band, 
                                                                  RFM_WCDMA_RFFE_PA, FALSE,
                                                                  &mfg_id[0], &prd_id[0],&rev_id[0],
                                                                  RFC_MAX_DEVICE_INSTANCES_PER_BAND)
                      #else
                      FALSE == rfdevice_wcdma_pa_get_device_info(remapped_tx_device, band, 
                                                                  &mfg_id[0], &prd_id[0],&rev_id[0],
                                                                  RFC_MAX_DEVICE_INSTANCES_PER_BAND)
                      #endif
                      )
                ...
                else if(rffe_device_type == RFFE_DEVICE_ASM)
                {
                  if (
                      #ifdef ATLAS_RF_WCDMA_PRESI
                      FALSE == rfm_wcdma_ftm_get_rffe_device_info(remapped_rx_device, (rfm_wcdma_band_type)band, 
                                                                  RFM_WCDMA_RFFE_ASM, TRUE,
                                                                  &mfg_id[0], &prd_id[0],&rev_id[0],
                                                                  RFC_MAX_DEVICE_INSTANCES_PER_BAND)
                      #else 
                      FALSE == rfdevice_wcdma_asm_get_device_info(remapped_rx_device, band, 
                                                                   &mfg_id[0], &prd_id[0],&rev_id[0],
                                                                   RFC_MAX_DEVICE_INSTANCES_PER_BAND)
                      #endif
                      )
                ...
                else if(rffe_device_type == RFFE_DEVICE_PAPM)
                {
                  if (
                      #ifdef ATLAS_RF_WCDMA_PRESI
                      FALSE == rfm_wcdma_ftm_get_rffe_device_info(remapped_tx_device, (rfm_wcdma_band_type)band, 
                                                                  RFM_WCDMA_RFFE_PAPM, FALSE,
                                                                  &mfg_id[0], &prd_id[0],&rev_id[0],
                                                                  RFC_MAX_DEVICE_INSTANCES_PER_BAND)
                      #else 
                      FALSE == rfdevice_wcdma_papm_get_device_info(remapped_tx_device, band, 
                                                                    &mfg_id[0], &prd_id[0],&rev_id[0],
                                                                    RFC_MAX_DEVICE_INSTANCES_PER_BAND)
                      #endif
                      )
    ...
    /* Return Response Packet without zero padding*/
	ftm_rsp_data.pkt_len = rffe_device_ids_resp_hdr_len + sizeof(rffe_device_ids_resp->mfg_prd_rev_id[0]) * rffe_device_ids_resp->max_num_devices;
    return ftm_rsp_data;
}

/*---------------------------------------------------------------------------------------*/
/*!
  @brief Copies all the DGLNA parameters to a structure to beused in RFM layer.
 
  @param cmd_ptr
  Structure containing DGLNA information in raw bytes
 
  @return
  Response packet contains the status
*/ 

ftm_rsp_pkt_type ftm_common_dglna_enable(ftm_common_pkt_type *cmd_ptr)
{
  ftm_dglna_descriptor_type *dglna = NULL;
  boolean status;
  rfm_dglna_descriptor_type dglna_info;
  ftm_dglna_descriptor_response_type *dglna_response = NULL;
  ftm_rsp_pkt_type ftm_rsp_data;

  /* initialize dglana info*/
  memset(&dglna_info, 0, sizeof(rfm_dglna_descriptor_type));

  ftm_rsp_data = ftmdiag_create_new_pkt(sizeof(ftm_dglna_descriptor_response_type));

    /*check if packet creation is successful*/  
	if (NULL == ftm_rsp_data.pkt_payload)
	{
	   FTM_MSG_ERROR("Cannot allocate memory for response packet payload", 0, 0, 0);
	   return ftm_rsp_data;
	}

    dglna_response = (ftm_dglna_descriptor_response_type*) ftm_rsp_data.pkt_payload;

     /*Copy data from request to response*/
	 memscpy(dglna_response, sizeof(ftm_composite_cmd_header_type), 
			 cmd_ptr, sizeof(ftm_composite_cmd_header_type));

   dglna = (ftm_dglna_descriptor_type*)&(cmd_ptr->ftm_extended_params);

  FTM_MSG( FTM_HIGH, "ftm_common_dglna_enable: before dglna != NULL");

  if(dglna != NULL)
  {
   dglna_info.feature_dglna_disable = dglna->feature_dglna_disable;
   dglna_info.enable_dglna = dglna->enable_dglna;

   dglna_info.tech1_info.tech = dglna->tech1_2_info[0].tech;
   
   switch(dglna_info.tech1_info.tech)
   {
   	case RFM_1X_MODE:
    dglna_info.tech1_info.band = (rfcom_1x_band_type)(dglna->tech1_2_info[0].band);
    break;

	case RFM_EGSM_MODE:
	dglna_info.tech1_info.band = (rfcom_gsm_band_type)(dglna->tech1_2_info[0].band);
	break;

	case RFM_IMT_MODE:
    dglna_info.tech1_info.band = (rfcom_wcdma_band_type)(dglna->tech1_2_info[0].band);
    break;

	case RFM_LTE_MODE:
    dglna_info.tech1_info.band = (rfcom_lte_band_type)(dglna->tech1_2_info[0].band);
    break;

	default:
          FTM_MSG( FTM_ERROR,"Not a Valid DGLNA technology");
   }
   
   dglna_info.tech1_info.primary_device = dglna->tech1_2_info[0].primary_device;
   dglna_info.tech1_info.secondary_device = dglna->tech1_2_info[0].secondary_device;
   
   dglna_info.tech2_info.tech = dglna->tech1_2_info[1].tech;

   switch(dglna_info.tech2_info.tech)
   {
   	case RFM_1X_MODE:
    dglna_info.tech2_info.band = (rfcom_1x_band_type)(dglna->tech1_2_info[0].band);
    break;

	case RFM_EGSM_MODE:
	dglna_info.tech2_info.band = (rfcom_gsm_band_type)(dglna->tech1_2_info[0].band);
	break;

	case RFM_IMT_MODE:
    dglna_info.tech2_info.band = (rfcom_wcdma_band_type)(dglna->tech1_2_info[0].band);
    break;

	case RFM_LTE_MODE:
    dglna_info.tech2_info.band = (rfcom_lte_band_type)(dglna->tech1_2_info[0].band);
    break;
   ...
   //dglna_info.tech2_info.band = dglna->tech1_2_info[1].band;
   dglna_info.tech2_info.primary_device = dglna->tech1_2_info[1].primary_device;
   dglna_info.tech2_info.secondary_device = dglna->tech1_2_info[1].secondary_device;

    status = rfm_dglna_set_attributes(dglna_info);
   }
   else
   {
     FTM_MSG( FTM_HIGH, "ftm_common_dglna_enable:dglna is NULL pointer");
     status = FALSE;
   }

   dglna_response->ftm_cmd = status;
   return ftm_rsp_data;	
}
/*===========================================================================
...
ftm_rsp_pkt_type ftm_common_dispatch(ftm_common_pkt_type *cmd_ptr)
{
  ...
  rfm_device_enum_type           tx_g_device = RFM_INVALID_DEVICE;
  ...
  if (cmd_ptr != NULL)
  {
	...
	case FTM_TXLM_MSB_CAL_CODE_DUMP:
      rflm_dtr_tx_msb_cal_code_dump();
      break;

	case FTM_TXLM_MSB_CAL_CODE_RELOAD:
      rflm_dtr_tx_and_et_dac_reprogram_msbcal_code(TXLM_CHAIN_0,FALSE);
	  rflm_dtr_tx_and_et_dac_reprogram_msbcal_code(TXLM_CHAIN_1,FALSE);
      break;
...
//    break;
  ...
  #if defined(FTM_HAS_UMTS)||defined(FEATURE_GSM)|| defined( FEATURE_CDMA1X )
    case FTM_GET_THERM: 
      {
        /*
    ...
    case FTM_GET_ENH_THERM: 
...
#ifndef ATLAS_RF_WCDMA_PRESI 
        #ifdef ATLAS_RF_WCDMA_PRESI
        tx_dev = rfm_wcdma_ftm_tx_get_rfm_device();
        #else
        tx_dev = ftm_wcdma_get_cur_tx_device();
        #endif
        rfdevice_wcdma_tx_cmd_dispatch(tx_dev,ftm_curr_wcdma_mode,RFDEVICE_GET_THERMISTER_VALUE, &enh_therm_val);
#endif
#else
        therm_read_error = TRUE;
#endif
     ...
     tx_g_device = rfgsm_get_current_tx_device();
     (uint32)rfdevice_gsm_cmd_dispatch(tx_g_device,RF_PATH_0,RFDEVICE_GET_THERMISTER_VALUE, &enh_therm_val);
#else
...
#if defined (FEATURE_CDMA1X) || defined (FEATURE_TDSCDMA) || defined (FEATURE_GSM)
    case FTM_SET_CALIBRATION_STATE:
     ...
     in this regard. */
      ftm_cal_state = (uint8)(cmd_ptr->ftm_extended_params) ;
      {
      ...
      (void)ftm_common_fbrx_set_cal_state((boolean)ftm_cal_state);
...
//          ftm_pwr_pdm_freq[pa_index].pwr_list[ch_index][0] = req_load_tx_fcomp->tx_fcomp_pwr_list[pa_index][ch_index];
    ...
    case FTM_CALIBRATION_SWEEP_GET_RESULTS:
      rsp_pkt = ftm_calibration_fetch_sweep_results( cmd_ptr , FALSE );
      rsp_pkt_created = TRUE;
      break;

    case FTM_CALIBRATION_SWEEP_GET_RESULTS_COMPRESSED:
      rsp_pkt = ftm_calibration_fetch_sweep_results( cmd_ptr , TRUE );
      rsp_pkt_created = TRUE;
      break;
    ...
    case FTM_INT_CALV3_DELTA_PIN_POUT:
      ftm_common_send_calc_delta_pin_pout_cmd(cmd_ptr);
      rsp_pkt = ftm_common_create_rsp_pkt(FTM_COMMON_NO_RSP);
      rsp_pkt_created = TRUE;
      break;
    ...
    case FTM_ACTIVATE_FTM_MODE:
      MSG_1(MSG_SSID_FTM, MSG_LEGACY_ERROR,"Recevied Cmd From MMOC task for activating FTM ", 0);
    ...
    case FTM_DEACTIVATE_FTM_MODE:
      MSG_1(MSG_SSID_FTM, MSG_LEGACY_ERROR,"Recevied Cmd From MMOC task for de-activating FTM ", 0);
  ...
  case FTM_PROGRAM_QFE_SCENARIO:
  {
    ...
    for(rf_device_idx =RFM_DEVICE_0; rf_device_idx<RFM_MAX_WAN_DEVICES; rf_device_idx++)
    {
      ...
      else
      {
        ...
        switch( ftm_rf_tech)
        {
          ...
          case FTM_RF_TECH_WCDMA:
            ...
            tx_freq = rfwcdma_core_get_freq_from_ul_arfcn(rfwcdma_data_get_curr_chan(tx_dev,RFCOM_SINGLE_CARRIER));
            /* Initializes hdet circuitry in WCDMA mode */
            ret_value = rfdevice_hdet_wcdma_init( tx_dev, NULL,
                                      RFDEVICE_EXECUTE_IMMEDIATE,
                                      rfwcdma_data_get_curr_band(tx_dev),
                                      tx_freq);
            if( ret_value == TRUE)
            {
              result.error_code = FTM_SUCCESS;
        ...
        else
        {
          ...
          result.error_code = FTM_SUCCESS;
            ...
            switch (ftm_rf_tech)
            {
              ...
              tx_freq = rfwcdma_core_get_freq_from_ul_arfcn(rfwcdma_data_get_curr_chan(tx_dev,RFCOM_SINGLE_CARRIER));
                /* Initializes hdet circuitry in WCDMA mode */
                rfdevice_hdet_wcdma_init (tx_dev, 
  ...
  case FTM_GET_TUNER_CL_ALGO_STATE:
    rsp_pkt = ftm_common_process_get_tuner_cl_algo_state( cmd_ptr); 
    rsp_pkt_created = TRUE;
    break;

  case FTM_GET_TUNER_STATUS:
    rsp_pkt = ftm_common_get_tuner_status( cmd_ptr); 
    rsp_pkt_created = TRUE;
    break;
    
  case FTM_PROGRAM_TUNER:
    rsp_pkt = ftm_common_program_tuner( cmd_ptr); 
    rsp_pkt_created = TRUE;
    break;
   ...
   case FTM_DO_FBRX_RSB_CAL:
	...
	case FTM_GET_RFFE_DEVICE_IDS:
	  {
	    get_rffe_req = (ftm_get_rffe_device_info_request *)cmd_ptr;
        rsp_pkt = ftm_common_get_rffe_device_ids(get_rffe_req);
        rsp_pkt_created = TRUE;	
	  }
	  break;
    ...
    case FTM_RF_ANT_DIV_SWITCH:
      rsp_pkt = ftm_common_ant_div_switch(cmd_ptr);
      rsp_pkt_created = TRUE;
    break;
    ...
    case FTM_FBRX_SINGLE_TRIGGER: 
      if(cmd_ptr->ftm_hdr.cmd_rsp_pkt_size == 94 || cmd_ptr->ftm_hdr.cmd_rsp_pkt_size == 0x5E)  //new resp required per customer doc for PA aging tool (ubias/rxscale override)
         rsp_pkt = ftm_common_fbrx_single_trigger_v2(cmd_ptr);
      else
    ...
    case FTM_FBRX_SET_DEBUG_CONTROL: 
      rsp_pkt = ftm_common_fbrx_set_debug_control_condition(cmd_ptr); 
      rsp_pkt_created = TRUE; 
      break; 
    ...
    case FTM_FBRX_DEBUG_GET_SAMPLE_CAPTURES:
      rsp_pkt = ftm_common_fbrx_debug_get_sample_captures(cmd_ptr); 
      rsp_pkt_created = TRUE;
      break;
    ...
    case FTM_SET_DGLNA_ATTRIBUTES:
      rsp_pkt = ftm_common_dglna_enable(cmd_ptr);
	  rsp_pkt_created = TRUE;
	break;
    ...
    case FTM_DRDS_RECONFIG_ENABLE:
       ftm_drds_reconfig_enable = TRUE;
       ftm_device_manager_set_concurrent_mode(TRUE);
    	 rsp_pkt = ftm_common_drds_reconfig_enable(cmd_ptr);
    	 rsp_pkt_created = TRUE;
    	 break;
       
    case FTM_DEVICE_MANAGER_MODE:
      rsp_pkt = ftm_device_manager_set_bypass_mode(cmd_ptr);
      rsp_pkt_created = TRUE;
      break;
...
)
{
  ...
  case FTM_COMMON_AUTOPIN_OVERRIDE:
    ftm_rsp_data = ftm_common_autopin_override(ftm_req_data);
    break;
