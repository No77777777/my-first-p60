...
/*==============================================================================
  ...
  $Header: //components/rel/rfa.mpss/4.11/rf/common/ftm/src/ftm_common_autopin.c#1 $

when       who     what, where, why
--------   ---     -------------------------------------------------------------
10/16/20   aa      1x compilation error fix for LWG build variant
04/12/16   vr      Legacy Autopin Support for WTR3925
02/18/16   sbo     CCS MMU changes 
12/24/15   vr      Autopin check-in_2
12/22/15   am       Use correct format for Reflog PAR result
12/07/15   ag       Fix KW warnings
12/03/15   kma     Updated CCS delay by gain state
12/02/15   vr      Timing changes for Autopin
11/30/15   ag      Change Timing related parameters for W
11/24/15   kma     Added time alignment
11/20/15   vr      Autopin check-in 
11/18/15   kma     Logged capture time
11/12/15   kg      Autopin PAR support
11/12/15   kma     Updated capture param interface
11/09/15   kma     Added WTR PAR CCS programming delay 
11/06/15   kma     Integrated new FW interface for REFLOG capture
11/05/15   ag      Autopin Changes for WCDMA
...
ftm_rsp_pkt_type ftm_common_autopin_capture(ftm_common_pkt_type *cmd_ptr)
{
  ...
  rflm_ccs_enq_event_config_t event_config;
  ...
  //rflm_lte_dm_for_all_cell_s* all_cell_ptr	   = NULL;
  ...
  switch (curr_ftm_rf_tech)
  {
     ...
     case FTM_RF_TECH_WCDMA:
         ...
         if (tx_device < RFM_MAX_WAN_DEVICES)
         {
           ...
           device_ptr = rfcommon_core_get_tx_device_ptr(tx_device, RFCOM_WCDMA_MODE, band_union);
	       tx_handle_id = (rflm_handle_tx_t)rfwcdma_data_get_rflm_buf_idx(tx_device); 
          }
	  ...
	  if (rflm_cdma_data_ptr != NULL)
	  {
	   ...
	   /*Adjust WTR capture time = ref time + capture delay -  CCS programming time 181us*/
	    refTime +=  autopin_temp_dm_ptr->tx_capture_delay;
           ...
           event_config.task_p       = &(autopin_event->header_task);
           event_config.start_offset = (refTime&0x3FFFFF);
           event_config.callback     = ftm_common_autopin_wtr_callback;
           event_config.callback_arg = NULL;          
           event_config.tq           = rflm_c2k_ccs_get_tx_tq(rflm_tech_mode)->tq_handle;
           event_config.evt_chk_ptr  = NULL;
           event_config.no_cb_order  = FALSE;
           event_config.tech_id      = RFLM_TECH_AUTOPIN;
           event_info = rflm_ccs_rf_enqueue_event(&event_config);
  ...
  break;
          ...
          case FTM_RF_TECH_WCDMA:
            ...
            FTM_MSG_3(FTM_HIGH, "ftm_common_autopin_capture: Enqueue at USTMR time %d, chain %d, tx handle %d ", refTime, tx_chain, tx_handle_id);
	      ...
	      return rsp;
            ...
            tq_ptr = rflm_wcdma_ccs_get_tx_tq(tx_handle_id);
            ASSERT(tq_ptr != NULL);
	    ...
	    refTime = refTime + extra_capture_time;
            ...
            event_config.callback     = ftm_common_autopin_wtr_callback;
            event_config.callback_arg = NULL;          
            event_config.tq           = tq_ptr->tq_handle;
            event_config.evt_chk_ptr  = NULL;
            event_config.no_cb_order  = FALSE;
            event_config.tech_id      = RFLM_TECH_AUTOPIN;
            event_info = rflm_ccs_rf_enqueue_event(&event_config);
            ...
            /* Debug Message */
            FTM_MSG_1(RF_MED,
                      "[WCDMA][ENABLE_AUTOPIN][Schedule] @ Autopin_script_ptr 0x%x ",
                      autopin_event
                      );
          ...
          case FTM_RF_TECH_LTE:
            ...
            #ifdef FEATURE_TABASCO_MODEM 
            tx_handle_id = rflte_ftm_mc_sm_get_carrier_txlm_buffer(RFLTE_MC_PCELL_IDX);           
            lte_data = rflm_lte_dm_get_data_ptr(tx_handle_id);
            //all_cell_ptr = rflm_lte_dm_get_ptr();
            ...
            rfcommon_msm_get_par_capture_params(RFLM_TECH_LTE,
                                                &(reflog_capture_input.capture_length),
                                                &(reflog_capture_input.txc_down_sampling_rate), 
                                                &(reflog_capture_input.tx_capture_delay),
                                                &wtr_capture_size);
          ...
          case FTM_RF_TECH_TDSCDMA:
            ...
            (void)rflm_tds_ccs_event_commit(rflmTdsRfModeCtlState.script.autopinWriteScriptPtrArray[param_ptr->fbrx_gain_state-1], 
                                                   NULL,
...
if (pout != NULL)
{  
  reflog_par_result =  (uint16)((pout->result.par)>>16);
}
