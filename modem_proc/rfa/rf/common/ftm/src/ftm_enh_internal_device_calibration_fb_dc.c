...
/*==============================================================================
  ...
  $Header: //components/rel/rfa.mpss/4.11/rf/common/ftm/src/ftm_enh_internal_device_calibration_fb_dc.c#1 $
...
07/30/15    aa     Get NV container val from FBRx DM and remove C0 hard-coding during fbrx dc cal
...
12/23/14    ka     Add support to remove nominal DC for WTR4905
...
#define RFCOMMON_FBRX_NOMINAL_DC_I_4905 0
#define RFCOMMON_FBRX_NOMINAL_DC_Q_4905 0
...
) 
{
   ...
   uint32 tx_freq = 0;
   uint64 tx_freq_hz=0;
   int32 nominal_dc_i = 0, nominal_dc_q = 0;
  ...
  case FTM_RF_TECH_WCDMA:
    {
      ...
      band_union.wcdma_band = rfwcdma_data_get_curr_band(tx_device);
      #endif
      FTM_MSG_1(FTM_HIGH, "ftm_enh_internal_device_cal_fb_dc_capture_sequence: tx_freq: %d", tx_freq);
  ...
  tx_freq_hz = tx_freq * 1000; 
   ...
   for (iter = 0; iter < num_captures; iter++) 
   {
      ...
      {
        nominal_dc_i = RFCOMMON_FBRX_NOMINAL_DC_I_4905;
        nominal_dc_q = RFCOMMON_FBRX_NOMINAL_DC_Q_4905;
      }

      api_status = ftm_enh_internal_device_cal_fb_dc_process(samp_buf_id, num_iq_samples, 0, &ftm_enh_int_cal_fbrx_dc_val_i, &ftm_enh_int_cal_fbrx_dc_val_q, nominal_dc_i, nominal_dc_q);
      if ( api_status == FALSE )
      {
...
) 
{
   ...
   *res_len = sizeof(ftm_enh_internal_device_cal_fb_dc_result_type);
   ...
   else
   {
      ...
      nv_band = rflte_util_get_band_from_tx_uarfcn_bw_path_override(sub_id,
                                                                    nv_band,tx_chan, 
                                                                    rfcom_bw.lte_bw, 
                                                                    path_override_index);
   }
   ...
   /****************************************************************/
  /* Get current Tx Device information     */
  /****************************************************************/

  switch ( curr_tx_ftm_rf_tech )
  {
  #if defined(FEATURE_CDMA1X) && defined(FEATURE_CDMA) 
  case FTM_RF_TECH_CDMA:
    tx_device = rf_cdma_data_get_curr_tx_device();
    FTM_MSG_1(FTM_HIGH,"Current TxDevice = %d",tx_device);
    break;
  #endif /* FEATURE_CDMA */
  ...
  default:
   ...
   ret_val = ftm_enh_internal_device_cal_fbrx_iq_cap_config(fbrx_tech, fbrx_bw,band,tx_device );
   ...
   if (ret_val != FTM_ENH_INTERNAL_DEVICE_CAL_RESULTS_SUCCESS)
   {
     ...
     ftm_enh_internal_device_cal_fbrx_iq_cap_deconfig(fbrx_tech,tx_device);
   ...
   ret_val = ftm_enh_internal_device_cal_fbrx_iq_cap_deconfig(fbrx_tech,tx_device);
   if (ret_val != FTM_ENH_INTERNAL_DEVICE_CAL_RESULTS_SUCCESS)
   {
   ...
   case FTM_RF_TECH_LTE:
        if(nv_band < RFCOM_NUM_LTE_BANDS)
        {
          ...
          if (fbrx_mode_index == RFLM_FBRX_MODE_3) 
          {             
             //update the FBRx DC values for LTE 20 and LTE 15 MHz
           ftm_enh_internal_device_cal_fb_dc_global_nv(
                                   tx_device,
                                   debug_mode_en,
                                   fbrx_rfm_mode,
              ...
              //update the FBRx DC values for LTE 20+20
              ftm_enh_internal_device_cal_fb_dc_global_nv( tx_device,
                                                           debug_mode_en,
                                                           fbrx_rfm_mode,
                                                           fbrx_tech,
                                                           fbrx_band,
                                                           ...
                                                           ftm_enh_int_cal_fbrx_dc_val_q,
                                                           internal_device_cal_data );
          }// end if (fbrx_mode_index == RFLM_FBRX_MODE_3) 
       }
        break;
   default:
...
)
{
  ...
  ret_val &= rfcommon_mdsp_calc_xpt_cap_rx_dc(cap_buf_id, num_iq_samples, discard_samps, dc_val_i, dc_val_q, nominal_dc_i, nominal_dc_q);
  return ret_val;
}
 ...
 )
{
  ...
  tx_handle = ftm_common_fbrx_get_valid_tx_handle( fbrx_rfm_mode, tx_device, rflte_ftm_mc_sm_get_carrier_index() );
  #else
  tx_handle = ftm_common_fbrx_get_valid_tx_handle(fbrx_rfm_mode, tx_device, 0 );
  #endif
  fbrx_data_ptr = (rflm_fbrx_dm_template_t*)rflm_dm_get_fbrx_buf_ptr(tx_handle);

   if (fbrx_data_ptr != NULL) 
   {
      c_val = fbrx_data_ptr->fbrx_ctl.container_val;
   }
   ...
   if (debug_mode_en != 0)
   {
     ...
     FTM_MSG_4(FTM_MED, "ftm_enh_internal_device_cal_fb_dc_global_nv: Current DC NV data for Tech %d Band %d FBRx Mode %d Gain State %d", fbrx_tech, fbrx_band, fbrx_mode_index, fbrx_gain_state);
     FTM_MSG_2(FTM_MED, "ftm_enh_internal_device_cal_fb_dc_global_nv: Current DC value I: %d, Q: %d", dc_i_nv, dc_q_nv);

   }
    FTM_MSG(FTM_HIGH,"Update dc globals");
 
   nv_read_write_error = rfcommon_fbrx_nv_update_dc_globals( tx_device,
                                       fbrx_rfm_mode,
                                       nv_band,
                                       fbrx_mode_index,
                                       fbrx_gain_state,
   ...
   if (debug_mode_en != 0)
   {
     ...
     FTM_MSG_4(FTM_MED, "ftm_enh_internal_device_cal_fb_dc_global_nv: Updated DC NV data for Tech %d Band %d FBRx Mode %d Gain State %d", fbrx_tech, fbrx_band, fbrx_mode_index, fbrx_gain_state);
     FTM_MSG_2(FTM_MED, "ftm_enh_internal_device_cal_fb_dc_global_nv: Updated DC value I: %d, Q: %d", dc_i_nv, dc_q_nv);
   }
   return ret_val;
}
