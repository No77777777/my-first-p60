...
/*===========================================================================
  ...
  $Header: //components/rel/rfa.mpss/4.11/rf/common/ftm/src/rf_test_task.c#1 $

when       who     what, where, why
--------   ---     -------------------------------------------------------------
03/17/20   mpa     to remove DOG_FTM_RPT_TIME AND added local static variable
03/28/17   rp      KW Fixes.
08/02/17   gk      Added Msgr variant id changes
09/24/15   zhw     Fix compiler error
09/24/15   zhw     Featurize internal FTM command per diag security requirement
...
07/22/15   zhw/tsr Added GSM support to FTM device manager
07/21/15   sb      Fix to avoid race condition between ftm log, ftm command processing 
07/20/15   zhw     Handle G extended/ber dispatch correctly
07/13/15   zhw     Hook up GSM/EXTENDED/BER dispatch APIs with assigned device
07/13/15   tsr     Move GSM Non-Signalling state variables under common chain variables 
07/08/15   zhw     Added support for FTM Device Manager SSIM & MSIM mode mapping
...
06/11/15   stw     Changes to support 32 bit channel in SET_CHAN
06/09/15   zhw     Added support for LTE to update agressor dev durign NPT test
06/05/15   zhw     Remap solution for FTM RF test
06/03/15   zhw     Added support for DRX NPT test
06/01/15   zhw     Added TRX Split NPT test support
05/13/15   zhw     Remove TH_2_0_MEAS featurization
05/20/15   sc      Add FTM GSM enh debug dispatcher command
05/13/14   zhw     Remove TH_2_0_MEAS featurization
05/04/15   ak      [TH.2.0 CRAT]Change to support 6 Rx path RF test
04/16/15   aro     Added support to enable FTM RF test
04/14/15   zbz     Added C4 and C5 and change mode id to rfm_dev mapping
04/13/15   cj      Commented call to ftm_lte_meas_register under CRAT feature flag
...
12/30/14   JJ      move set_carrier_index from ftm_lte_rex_dispatch to ftm_dispatch
...
ftm_gsm_chains_enum_type ftm_gsm_ns_active_chain = FTM_GSM_INVALID_CHAIN;
...
static const rex_timer_cnt_type rfa_dog_ftm_rpt_time = 0x0799;

#ifdef FEATURE_LTE

extern void ftm_log_lte_ca_enabled_rx_agc(int4);
...
static msgr_umid_type rfgsm_ftm_umid_list[] = 
{
  /*! The response command list used to register with  */
  /*! FTM Task with MSGR for subscription 1(RF Task1)  */
  RFA_RF_GSM_FTM_SET_PATH_DELAY_SUB1_RSP,
  RFA_RF_GSM_FTM_SET_SLOT_OVERRIDE_FLAG_SUB1_RSP,
  RFA_RF_GSM_FTM_SET_LINEAR_PA_RANGE_SUB1_RSP,
  RFA_RF_GSM_FTM_SET_LINEAR_RGI_SUB1_RSP,
  RFA_RF_GSM_FTM_SET_QPOET_MODE_BIAS_SUB1_RSP,
  RFA_RF_GSM_FTM_CAPTURE_IQ_SUB1_RSP,
  RFA_RF_GSM_FTM_SET_TX_POW_DBM_SUB1_RSP,
  RFA_RF_GSM_FTM_TUNE_TX_SUB1_RSP,
  RFA_RF_GSM_FTM_TUNER_TUNE_CODE_OVERRIDE_SUB1_RSP,
  RFA_RF_GSM_FTM_SET_IP2_CAL_PARAMS_SUB1_RSP,
  RFA_RF_GSM_FTM_GET_IP2_CAL_PARAMS_SUB1_RSP,
  RFA_RF_GSM_FTM_GET_IP2_CAL_RESULTS_SUB1_RSP,
  RFA_RF_GSM_FTM_SET_SAWLESS_LIN_MODE_SUB1_RSP,
  RFA_RF_GSM_FTM_GET_SAWLESS_LIN_MODE_SUB1_RSP,

  /*! The response command list used to register with  */
  /*! FTM Task with MSGR for subscription 2(RF Task2)  */
  RFA_RF_GSM_FTM_SET_PATH_DELAY_SUB2_RSP,
  RFA_RF_GSM_FTM_SET_SLOT_OVERRIDE_FLAG_SUB2_RSP,
  RFA_RF_GSM_FTM_SET_LINEAR_PA_RANGE_SUB2_RSP,
  RFA_RF_GSM_FTM_SET_LINEAR_RGI_SUB2_RSP,
  RFA_RF_GSM_FTM_SET_QPOET_MODE_BIAS_SUB2_RSP,
  RFA_RF_GSM_FTM_CAPTURE_IQ_SUB2_RSP,
  RFA_RF_GSM_FTM_SET_TX_POW_DBM_SUB2_RSP,
  RFA_RF_GSM_FTM_TUNE_TX_SUB2_RSP,
  RFA_RF_GSM_FTM_TUNER_TUNE_CODE_OVERRIDE_SUB2_RSP,
  RFA_RF_GSM_FTM_SET_IP2_CAL_PARAMS_SUB2_RSP,
  RFA_RF_GSM_FTM_GET_IP2_CAL_PARAMS_SUB2_RSP,
  RFA_RF_GSM_FTM_GET_IP2_CAL_RESULTS_SUB2_RSP,
  RFA_RF_GSM_FTM_SET_SAWLESS_LIN_MODE_SUB2_RSP,
  RFA_RF_GSM_FTM_GET_SAWLESS_LIN_MODE_SUB2_RSP,

};

/*! The response command list used to register with FTM Task */
/*!  with MSGR to recieve messages asynchronously             */
static msgr_umid_type rfgsm_rex_ftm_umid_list[] = 
{
  /*! set pa range is a placeholder. Blocking receive is being
...
/*! @brief Definitions of the FTM message count for FTM Task for the mailboxes */
#define RF_GSM_FTM_UMID_CNT(x) \
...
===========================================================================*/
  ...
  extern boolean ftm_gsm_ber_is_start_mode_req(ftm_gsm_ber_pkt_type *cmd_ptr);
#endif
#endif
...
void ftm_kick_watchdog(void)
{
  ...
  (void) rex_set_timer(&ftm_rpt_timer, rfa_dog_ftm_rpt_time);
...
boolean rf_lte_ftm_msgr_register( msgr_client_t *client_id, msgr_id_t id )
{
  ...
  /* Register for the RF LTE FTM message FTM task is  interested in receiving on both LTE subs*/
  for (sub_id = 0; sub_id < RF_MAX_NUMBER_SUB; sub_id++) 
  {
  ret_status = msgr_register_block_variant(MSGR_FTM_CORE,
                                   client_id,
                                   id,
  ...
  {
      FTM_MSG_ERROR("rf_lte_ftm_msgr_register: Unable to register FTM message router register block.on LTE sub id %d",sub_id,0,0);
      rc = FALSE;
  }
  }
  return (rc);
...
static void ftm_rfgsm_init_rex_msgr_client( void )
{
  ...
  /* UMID registration with FTM Task to receive asynchronous messages*/
  msgr_error = msgr_register_block( MSGR_FTM_GSM,
                                    &ftm_rfgsm_rex_msgr_client,
                                    ftm_rfgsm_rex_msgr_id,
                                    rfgsm_rex_ftm_umid_list,
                                    RF_GSM_FTM_UMID_CNT(rfgsm_rex_ftm_umid_list) );
...
static void ftm_rfgsm_init_ftm_msgr_client( void )
{
  ...
  if( msgr_error != E_SUCCESS)
  {
     FTM_MSG_ERROR( "ftm_rfgsm_init_ftm_msgr_client() Unable to create FTM MSGR client for "
                    "receiving blocking RFGSM messages from RF task.",
                    0,0,0);  
  };
  ...
  if(msgr_error != E_SUCCESS)
  {
     FTM_MSG_ERROR( "ftm_rfgsm_init_ftm_msgr_client() Unable to add mailbox for "
                    "blocking RFGSM messages from RF task to the FTM MSGR client",
                    0,0,0); 
  }

  /*Response UMID registration with FTM Task for blocking messages*/
  msgr_error = msgr_register_block( MSGR_RFA_RF_GSM_FTM,
                                    &ftm_rfgsm_msgr_client,
                                    ftm_rfgsm_msgr_mb_id,
                                    rfgsm_ftm_umid_list,
                                    RF_GSM_FTM_UMID_CNT(rfgsm_ftm_umid_list) );

  if(msgr_error != E_SUCCESS)
  {
    FTM_MSG_ERROR( "ftm_rfgsm_init_ftm_msgr_client() Unable to register for "
                   "blocking RF GSM FTM messages with FTM MSGR client",
                   0,0,0);
...
static boolean ftm_npt_mode_set[FTM_STATE_MAX] = {0};
                                    ...
                                    )  
{
    ...
    #ifdef FEATURE_FTM_GSM_BER
    boolean gsm_ns_device = RFM_INVALID_DEVICE;
    #endif
    /* route command depending on header information */
    ...
    if ( ftm_mode == FTM_MODE || bypass_ftm_mode_check)
    {
      ...
      assigned_device = ftm_common_convert_subsys_id_to_legacy_device( header->subsys_cmd_code );

      switch (header->subsys_id)
      {
      case FTM:

        /*! FTM DEVICE MANAGER - Assign Device Based on Scenarios, including 
          legacy Device Mapping */
        if (ftm_device_manager_is_subsys_id_supported(header->subsys_cmd_code) == TRUE) 
        {
          assigned_device = ftm_device_manager_allocate_device(header->subsys_cmd_code,
                                                               (ftm_pkt_type*)pRequestBytes);
        }
        else if ( ftm_device_manager_is_subsys_id_extended_format(header->subsys_cmd_code) == TRUE )
        {
          #ifdef FEATURE_FTM_GSM_BER
          /* Non_Signalling rx device obtained from RFC at the start FTM GSM BER mode
             This device is updated to ftm device manager structure to be passed in
             as assigned device in the following commands. The device is not updated
             for Bypass mode. Legacy device is passed in bypass mode */
          if ( ftm_device_manager_get_operating_mode() != FTM_DEVICE_MANAGER_BYPASS_MODE ) 
          {
             /*Check if ftm cmd id is FTM GSM BER Start mode req*/
             if ( ftm_gsm_ber_is_start_mode_req((ftm_gsm_ber_pkt_type *)pRequestBytes) == TRUE ) 
             {
                /*obtain the NS device and update FDM manager device structure*/
                gsm_ns_device = ftm_gsm_allocate_ns_device(); 
                ftm_device_manager_set_explicit_device_mapping(assigned_device, gsm_ns_device); 
                assigned_device = gsm_ns_device;
             } 
             else /* retreive the device stored in FDM device structure during Start mode req */
             {
                assigned_device = ftm_device_manager_get_assigned_device(assigned_device);
             }
          }
          #endif
        }
        ...
        case FTM_1X_C0_C:
          ...
          rsp_pkt = ftm_cdma_dispatch(assigned_device , (ftm_pkt_type*)pRequestBytes);
          break;
        ...
        case FTM_HDR_C:
          ...
          break;
        case FTM_HDR_C0_C:
          rsp_pkt = ftm_hdr_dispatch(assigned_device, (ftm_pkt_type*)pRequestBytes);
          break;
        case FTM_HDR_C1_C:
          rsp_pkt = ftm_hdr_dispatch(assigned_device, (ftm_pkt_type*)pRequestBytes);
          break;
        ...
        #ifdef FEATURE_GSM
        case FTM_GSM_C0_C:
          rsp_pkt = ftm_gsm_dispatch( assigned_device, (ftm_pkt_type*) pRequestBytes);
          break;
        ...
        case FTM_GSM_EXTENDED_C0_C:
          rsp_pkt = ftm_gsm_extended_dispatch( assigned_device, (ftm_gsm_extended_pkt_type*) pRequestBytes);
          break;
...
#ifdef FEATURE_RFA_INTERNAL_FTM_CMD
        case FTM_GSM_ENH_0_C: //Internal use only
          rsp_pkt = ftm_gsm_enh_debug_dispatch( 0, (ftm_gsm_enh_debug_pkt_type*) pRequestBytes );
          break;

        case FTM_GSM_ENH_1_C: //Internal use only
          rsp_pkt = ftm_gsm_enh_debug_dispatch( 1, (ftm_gsm_enh_debug_pkt_type*) pRequestBytes );
          break;
#endif /* FEATURE_RFA_INTERNAL_FTM_CMD */
        #endif /* FEATURE_GSM */

        #ifdef FEATURE_WCDMA
        case FTM_WCDMA_C:
          rsp_pkt = ftm_wcdma_dispatch(assigned_device, (ftm_pkt_type*) pRequestBytes);
          break;
        ...
        #ifdef FTM_HAS_TDSCDMA
        case FTM_TDSCDMA_C:
          rsp_pkt = ftm_tdscdma_dispatch(assigned_device, (ftm_pkt_type*) pRequestBytes);
          break;
        ...
        case FTM_GSM_BER_C0_C:
          rsp_pkt = ftm_gsm_ber_dispatch( assigned_device, (ftm_gsm_ber_pkt_type*) pRequestBytes);
          break;
        ...
        case FTM_LTE_CAR2_RX_2_C:
          ...
          if( pRequestBytes != NULL)
          {
            ...
            if( ftm_lte_build_msgr_msg( payload_ptr, msgr_msg_ptr, iRequestPacketSize) == TRUE )
            {
              is_cmd_valid = TRUE;
            ...
            else
            {
              CmdInProgressBlockSequencer = TRUE;
              /* Set Carrier Index */
              if (ftm_lte_set_carrier_index((void*)lte_msg_ptr) < RFA_RF_LTE_MAX_CELL_SUPPORTED)
              {
...
#ifdef FEATURE_RFA_INTERNAL_FTM_CMD
       case FTM_RF_TEST_C:
         rsp_pkt = ftm_rf_test_dispatch( (ftm_rfnv_common_header_type*) pRequestBytes, iRequestPacketSize ) ;
         break;
	   #if defined ( FEATURE_CDMA1X )
        case FTM_1X_DYNAMIC_CHAIN_C:
        {
           rfm_device_enum_type device = RFM_INVALID_DEVICE;
           ftm_pkt_type* ftm_cmd_ptr = (ftm_pkt_type *)pRequestBytes;

           if ( ftm_cmd_ptr->rf_params.ftm_rf_cmd == FTM_SET_MODE && ftm_npt_mode_set[FTM_STATE_1X] == FALSE)
           {
             device = ftm_device_manager_allocate_msim_device(ftm_cmd_ptr); 
             ftm_npt_mode_set[FTM_STATE_1X] = TRUE;
           }
           else
           {
             device = ftm_device_manager_get_tech_prx_device( FTM_STATE_1X );                         
           }

                   
           if ( device != RFM_INVALID_DEVICE)
           {
             rsp_pkt = ftm_cdma_dispatch( device, ftm_cmd_ptr);   
           }
           else
           {
             FTM_MSG_2(FTM_ERROR,"Dynamic dispatch: Invalid device assigned. "
                                 "Skip dispatching command %d for tech %d",
                       ftm_cmd_ptr->rf_params.ftm_rf_cmd, FTM_STATE_1X);
           }
           break;           
        }

        case FTM_1X_DYNAMIC_CHAIN_DRX_C:
        {
           rfm_device_enum_type device = RFM_INVALID_DEVICE;
           ftm_pkt_type* ftm_cmd_ptr = (ftm_pkt_type *)pRequestBytes;

           device = ftm_device_manager_get_tech_drx_device( FTM_STATE_1X );                         
           ...
           {
             FTM_MSG_2(FTM_ERROR,"Dynamic dispatch: Invalid DRx device assigned. "
                                 "Skip dispatching command %d for tech %d",
                       ftm_cmd_ptr->rf_params.ftm_rf_cmd, FTM_STATE_1X);
           }
		...
		#ifdef FEATURE_WCDMA
        case FTM_WCDMA_DYNAMIC_CHAIN_C:
        {
           rfm_device_enum_type device = RFM_INVALID_DEVICE;

           ftm_pkt_type* ftm_cmd_ptr = (ftm_pkt_type *)pRequestBytes;

           if ( ftm_cmd_ptr->rf_params.ftm_rf_cmd == FTM_SET_MODE && ftm_npt_mode_set[FTM_STATE_WCDMA] == FALSE)
           {
             device = ftm_device_manager_allocate_msim_device(ftm_cmd_ptr); 
             ftm_npt_mode_set[FTM_STATE_WCDMA] = TRUE;
           }
           else
           {
             device = ftm_device_manager_get_tech_prx_device( FTM_STATE_WCDMA );                         
           }
           
           if ( device != RFM_INVALID_DEVICE)
           {
             rsp_pkt = ftm_wcdma_dispatch( device, ftm_cmd_ptr); 
           }
           else
           {
             FTM_MSG_2(FTM_ERROR,"Dynamic dispatch: Invalid device assigned. "
                                 "Skip dispatching command %d for tech %d",
                       ftm_cmd_ptr->rf_params.ftm_rf_cmd, FTM_STATE_WCDMA);
           }

           break;           
        }

        case FTM_WCDMA_DYNAMIC_CHAIN_DRX_C:
        {
           rfm_device_enum_type device = RFM_INVALID_DEVICE;

           ftm_pkt_type* ftm_cmd_ptr = (ftm_pkt_type *)pRequestBytes;

           device = ftm_device_manager_get_tech_drx_device(FTM_STATE_WCDMA);                         
		...
		#ifdef FEATURE_GSM
        case FTM_GSM_DYNAMIC_CHAIN_C:
        {
           rfm_device_enum_type device = RFM_INVALID_DEVICE;

           ftm_pkt_type* ftm_cmd_ptr = (ftm_pkt_type *)pRequestBytes;

           if ( ftm_cmd_ptr->rf_params.ftm_rf_cmd == FTM_SET_MODE && ftm_npt_mode_set[FTM_STATE_GSM] == FALSE)
           {
             device = ftm_device_manager_allocate_msim_device(ftm_cmd_ptr); 
             ftm_npt_mode_set[FTM_STATE_GSM] = TRUE;
           }
           else
           {
             device = ftm_device_manager_get_tech_prx_device( FTM_STATE_GSM );                         
           }

           if ( device != RFM_INVALID_DEVICE)
           {
             rsp_pkt = ftm_gsm_dispatch(device, ftm_cmd_ptr); 
           }
           else
           {
             FTM_MSG_2(FTM_ERROR,"Dynamic dispatch: Invalid device assigned. "
                                 "Skip dispatching command %d for tech %d",
                       ftm_cmd_ptr->rf_params.ftm_rf_cmd, FTM_STATE_GSM);
           }
           break;           
        }
        case FTM_GSM_EXTENDED_DYNAMIC_CHAIN_C:
        {
          rfm_device_enum_type device = RFM_INVALID_DEVICE;
          ...
          if ( device != RFM_INVALID_DEVICE)
          {
            rsp_pkt = ftm_gsm_extended_dispatch( device, (ftm_gsm_extended_pkt_type*) pRequestBytes);            
          }
          else
          {
            FTM_MSG(FTM_ERROR,"Dynamic dispatch: Invalid device assigned. "
                                "Skip dispatching extended GSM command");
          }
        ...
        case FTM_TDS_DYNAMIC_CHAIN_C:
        {
           ...
           {
             device = ftm_device_manager_get_tech_prx_device( FTM_STATE_TDSCDMA );                         
           }
           
           rsp_pkt = ftm_tdscdma_dispatch( device, ftm_cmd_ptr); 
           break;           
        }


        case FTM_TDS_DYNAMIC_CHAIN_DRX_C:
        {
           rfm_device_enum_type device = RFM_INVALID_DEVICE;

           ftm_pkt_type* ftm_cmd_ptr = (ftm_pkt_type *)pRequestBytes;

           device = ftm_device_manager_get_tech_drx_device( FTM_STATE_TDSCDMA );                         
           ...
           else
           {
             FTM_MSG_2(FTM_ERROR,"Dynamic dispatch: Invalid DRx device assigned. "
                                 "Skip dispatching command %d for tech %d",
                       ftm_cmd_ptr->rf_params.ftm_rf_cmd, FTM_STATE_TDSCDMA);
           }
  ...
  */
  ...
  for (;;)
  {
   ...
   /* This signal is coming from the RFA, L1, L2 tasks, 
    ...
    if ((sigs & FTM_LTE_LOGGING_CMD_Q_SIG) == FTM_LTE_LOGGING_CMD_Q_SIG)
    {
      (void) rex_clr_sigs( TASK_HANDLE(ftm), FTM_LTE_LOGGING_CMD_Q_SIG);
      ftm_log_lte_ca_enabled_rx_agc(0);
    }
...
static void ftm_deinit(void)
{
  ...
  {
    FTM_MSG_ERROR( "ftm_deinit(): Failed to delete the client ",0,0,0); 
    ASSERT(0);
  }
  
  //ASSERT(msgr_client_delete(&(ftm_task_info.ftm_msgr_client)) == E_SUCCESS);
...
static void ftm_gsm_deinit_rex_msgr_client( void )
{
  ...
  /* Deregister all the messages associated with REX GSM FTM_MSGR client */
  ...
  if( msgr_error != E_SUCCESS)
  {
     FTM_MSG_ERROR( "ftm_gsm_deinit_rex_msgr_client(): deregistration for "
                    "Rex GSM FTM MSGR register block failed.",
                    0,0,0);  
  };

  /* Release mailbox associated with REX GSM FTM_MSGR client */
  msgr_error = msgr_client_delete( &ftm_rfgsm_rex_msgr_client );
  
  if(msgr_error != E_SUCCESS)
  {
    FTM_MSG_ERROR( "ftm_gsm_deinit_rex_msgr_client(): Failed to delete the client",0,0,0); 
    ASSERT(0);
  }
  //ASSERT(msgr_client_delete( &ftm_rfgsm_rex_msgr_client ) == E_SUCCESS);
}
...
static void ftm_gsm_deinit_msgr_client( void )
{
  ...
  /*deregister response UMIDs associated with GSM FTM_MSGR client*/
  msgr_error = msgr_deregister_block( MSGR_FTM_GSM,
                                      &ftm_rfgsm_msgr_client,
                                      rfgsm_ftm_umid_list,
                                      RF_GSM_FTM_UMID_CNT(rfgsm_ftm_umid_list));
  ...
  if( msgr_error != E_SUCCESS)
  {
     FTM_MSG_ERROR( "ftm_gsm_deinit_msgr_client(): deregistration for "
                    "GSM FTM MSGR register block failed",
                    0,0,0);  
  };

  /* Release mailboxes associated with FTM_GSM client */
  msgr_error = msgr_client_delete( &ftm_rfgsm_msgr_client );
  
  if(msgr_error != E_SUCCESS)
  {
    FTM_MSG_ERROR( "ftm_gsm_deinit_msgr_client(): Failed to delete the client ",0,0,0); 
    ASSERT(0);
  }
  //ASSERT(msgr_client_delete( &ftm_rfgsm_msgr_client ) == E_SUCCESS);
}
#endif /* FEATURE_GSM */
