...
/*==============================================================================
  ...
  $Header: //components/rel/rfa.mpss/4.11/rf/common/ftm/src/ftm_common_enh_internal_device_calibration.c#1 $
...
03/23/16   sk      Use Tx band for feeback query and pass FB device from RFC
                   Update but not write FBRX ADC settings for LTE-tune away support
...
(
   ...
   uint8 bw,rfcom_band_type_u  fbrx_band,
...
)
{
  ...
  rfm_device_enum_type fbrx_device;
  ...
  fbrx_rfm_mode = ftm_common_ftm_tech_to_rfm_mode(tech);
  device_ptr = rfcommon_core_get_tx_device_ptr(tx_device, 
                                                fbrx_rfm_mode, 
                                                fbrx_band);
  ...
  #ifdef FEATURE_WCDMA
  case FTM_RF_TECH_WCDMA:
    api_status = ftm_wcdma_xpt_cal_config(tx_device, tx_device, (uint8)FTM_XPT_CONFIG_FBRX, TRUE);
    FTM_MSG_4(FTM_HIGH, "ftm_wcdma_xpt_cal_config(dev %d, dev_fb %d, xpt_mode %d) returned %d", tx_device, tx_device, FTM_XPT_CONFIG_FBRX , api_status );
    if (ftm_wcdma_xpt_fb_path_buffer.is_buf_allocated)
    {
      ...
      /* Update device-specfic FBRx LM settings */
  ...
  case FTM_RF_TECH_LTE:
    ...
    if(lte_ftm_mc_sm_ptr == NULL)
    {
      ...
      return FTM_ENH_INTERNAL_DEVICE_CAL_IQ_CAP_CONFIG_FAILURE;
        ...
        fbrx_device = rfc_lte_get_fb_device(tx_device, fbrx_band.lte_band,rflte_ftm_mc_sm_carrier_get_drx_band(RFLTE_MC_PCELL_IDX));
	api_status = ftm_lte_xpt_cal_configure(tx_device,fbrx_device,FTM_XPT_CONFIG_FBRX);
	FTM_MSG_4(FTM_HIGH, "ftm_lte_xpt_cal_configure(dev %d, dev_fb %d, xpt_mode %d) returned %d", tx_device, fbrx_device, 3, api_status );
	fbrx_carrier_idx = rflte_ftm_mc_sm_get_carrier_index();
	FTM_MSG_2(FTM_HIGH, "rflte_ftm_mc_get_carrier_index_from_device obtained fbrx_carrier_idx %d for device %d", fbrx_carrier_idx, tx_device);
    ...
    if (rflte_state_get_fbrxlm_buffer_idx(sub_id,fbrx_carrier_idx) != LM_BUFFER_NOT_ALLOCATED)
    {
      ...
      api_status &= (rfcommon_fbrx_msm_update_static_settings( rflte_state_get_fbrxlm_buffer_idx(sub_id,fbrx_carrier_idx) , rfcommon_fbrx_tech_bw ) == RFCOMMON_FBRX_SUCCESS);
      FTM_MSG_3(FTM_HIGH, "rfcommon_fbrx_lm_update_static_settings (buf_id %d, tech_bw %d) success status %d ", 
      ...
      /* Update device-specific FBRx LM settings */
      fbrx_modem_chain = (uint8)(rfc_lte_get_fb_chain(tx_device,
                                                       rflte_ftm_mc_sm_get_tx_band()));
      rfcommon_fbrx_msm_config_dynamic_adc_settings(rflte_state_get_fbrxlm_buffer_idx(sub_id,fbrx_carrier_idx),
                                                    (uint32)rfdevice_id_get((void *)device_ptr),
                                                    fbrx_modem_chain,
                                                    RFLM_TECH_LTE);
  ...
  case FTM_RF_TECH_CDMA:
    ...
    fbrx_modem_chain = 3; /* WB3 is the default FBRX chain for CDMA */	
    ...
    /* Update device-specifc FBRx LM settings */
    fbrx_modem_chain = rfc_cdma_get_fbrx_modem_chain(tx_device, fbrx_band.cdma_band);	
    rfcommon_fbrx_msm_config_dynamic_adc_settings(fbrx_lm_handle, (uint32)rfdevice_id_get((void *)device_ptr),fbrx_modem_chain,RFLM_TECH_1X);
   ...
   #ifdef FEATURE_TDSCDMA
  case FTM_RF_TECH_TDSCDMA:
    api_status = ftm_tdscdma_xpt_cal_config(tx_device, tx_device, (uint8)FTM_XPT_CONFIG_FBRX, TRUE);
    FTM_MSG_4(FTM_HIGH, "ftm_tdscdma_xpt_cal_config(dev %d, dev_fb %d, xpt_mode %d) returned %d", tx_device, tx_device, FTM_XPT_CONFIG_FBRX , api_status );
...
)
{
  ...
  #ifdef FEATURE_WCDMA
  case FTM_RF_TECH_WCDMA:
    api_status = ftm_wcdma_xpt_cal_config(tx_device, tx_device, (uint8)FTM_XPT_CONFIG_FBRX, FALSE);
    break;
  #endif
  #ifdef FEATURE_LTE
  case FTM_RF_TECH_LTE:
    fbrx_device = rfc_lte_get_fb_device(tx_device, rflte_ftm_mc_sm_get_tx_band(),rflte_ftm_mc_sm_carrier_get_drx_band(RFLTE_MC_PCELL_IDX));
    api_status = ftm_lte_xpt_cal_deconfigure(tx_device, fbrx_device, FTM_XPT_CONFIG_FBRX );
    break;
  #endif
  #ifdef FEATURE_CDMA
  case FTM_RF_TECH_CDMA:
  ...
  #ifdef FEATURE_TDSCDMA
  case FTM_RF_TECH_TDSCDMA:
    api_status = ftm_tdscdma_xpt_cal_config(tx_device, tx_device, (uint8)FTM_XPT_CONFIG_FBRX, FALSE);
    break;
  #endif
  default:
...
)
{
  ...
  if ( rfcommon_xpt_is_capture_done() == TRUE )
  {
    ...
    #ifdef FEATURE_LTE
    case FTM_RF_TECH_LTE:
      fb_device = rfc_lte_get_fb_device(iq_cap_instr_ptr->primary_device, rflte_ftm_mc_sm_get_tx_band(),rflte_ftm_mc_sm_carrier_get_drx_band(RFLTE_MC_PCELL_IDX));
        ...
        // Need to reset above count to 0, since right now we have single cap sequence functional, remove this when multi-cap functional
         ...
         capture_param.capture_type =
           ftm_common_xpt_map_capture_type(
             (ftm_calibration_xpt_iq_capture_enum)iq_cap_instr_ptr->capture_type );
         capture_param.num_samples = iq_cap_instr_ptr->num_samples;
         capture_param.first_trig = iq_cap_instr_ptr->first_trigger;
         capture_param.last_trig = iq_cap_instr_ptr->last_trigger;
         capture_param.do_processing = iq_cap_instr_ptr->proc_flag;
         capture_param.buffer_id = buffer_ids;
         capture_param.split_samp = 0;

         (void)ftm_cdma_data_get_device_status( iq_cap_instr_ptr->primary_device, &dev_state );

         /* Store the IQ Caprure Data */
         iq_cap_data.device = iq_cap_instr_ptr->primary_device;
         iq_cap_data.device_fb = iq_cap_instr_ptr->feedback_device;
         iq_cap_data.channel = dev_state.curr_chan;
         iq_cap_data.is_last_trig = iq_cap_instr_ptr->last_trigger;
         ...
         /* Perform EPT Capture */
         api_status = rfm_1x_ftm_xpt_iq_capture( iq_cap_instr_ptr->primary_device,
                                                    iq_cap_instr_ptr->feedback_device,
                                                    capture_param,
                                                    NULL );

         FTM_MSG_1(FTM_HIGH, "rfm_1x_ftm_xpt_iq_capture returns %d", api_status);
        break;
      }
   ...
   uint16 fbrx_band)
{
  ...
  case FTM_RF_TECH_LTE:
    ...
    case 59:
       nv_band = (uint16)RFCOM_BAND_LTE_B71;  /*To keep inline with ATLAS*/     
       break;
    default:
