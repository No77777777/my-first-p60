...
/*==============================================================================
  ...
  $Header: //components/rel/rfa.mpss/4.11/rf/common/ftm/src/ftm_calibration_v3_data.c#1 $
...
09/12/16   vkm      Store PIN cal offset correctly in lte nv table 
03/28/16   zhh     Check the data_module_ptr before use it
02/18/16   zhh     Add new APIs to support AMAM based autoPin delta pout calc
12/09/15   pk      Added support for CDMA rx\tx split
09/01/15   abh     Initialize Number of selection sweep powers for Meas Pwr and 
                   Calc Pwr List in ET/EPT to 0
08/26/15   sk      [Phase 1] WTR hopping - Move TX MC SM variables to hadnle based DM
8/06/15	   cac     Support for FBRx Gain Cal Agc
7/21/15    shk     Added support for populating AMAM Rx RMS value as part of 
                   XPT capture Results opcode
...
05/13/15   kr      Fix compiler error
04/22/15   ak      [Rx/Tx Split] Changes to support Cal with backward compatibility.
04/23/15   sb      Back out TX device remap in cal data processing
04/22/15   sb      Remap device for XPT APIs
...
3/23/15    JJ      Latch DTR chain0 after chain1 to bypass DTR1 latch HW limitation
03/30/15   zhw     Use rfcommon_core_get_tx_default_device() to map Tx device
                   from Rx device during DPD Cal
...
)
{
  ...
  tx_device = ftm_common_xpt_get_tx_default_device( device_ind, cal_data );
  ...
  */
  ...
  {
    FTM_MSG_1( FTM_ERROR, "Invalid pa_state : %d" , curr_tx_ovr.pa_state);
    return FALSE;
  }
  ...
  case FTM_RF_TECH_WCDMA:
      #ifdef FEATURE_WCDMA
	  /*For WCDMA overide the device to the remapped device*/
	  ovr_params.device = tx_device;
  ...
  if(return_val == TRUE)
  {
    ...
    return_val &= 
      rf_common_xpt_load_dpd_tbls(tx_device, 
                                  NULL, 
                                  NULL, 
                                  0, 
                                  0, 
   ...
   boolean debug_on)
{
  ...
  case DATA_MLINE_CLOSEST_PWR_RET_MAX_ENTRY:
      ...
      break;
  case DATA_MLINE_CALC_WITH_p1dB_COMPPT_BACKOFF:
  case DATA_MLINE_CALC_WITH_p3dB_COMPPT_BACKOFF:
  case DATA_MLINE_CALC_WITH_p5dB_COMPPT_BACKOFF:
     break;
    default:
...
)
{
  ...
  if( return_val == TRUE )
  {
    ...
    mline_params->mline_bias_opt = data_param->data_param.mline_params.mline_bias_option;
  ...
  if( return_val == TRUE )
  {
	...
	sys_band_class_e_type sys_band = SYS_BAND_CLASS_MAX;		
        ...
        /*!!! To support splitted Tx and Rx device without changing Cal tool intf. 
          ...
          converted to associated Tx device based on RFC. This is valid until tool
          interface is updated to pass in both Rx & Tx devices*/
        rx_device = ftm_cdma_get_remapped_rx_device( data_param->device );
       ...
       (void)ftm_cdma_data_get_device_status( rx_device, &rx_dev_state );
        band_u.cdma_band = (rfm_cdma_band_class_type)rx_dev_state.curr_band;
	...
	tx_device = rfc_common_get_preferred_tx_device ( sys_band );
        ...
        mline_params->env_scale = 
          (int32)(rf_cdma_xpt_get_cfg_param(  tx_device,  EPT_CFG_CMN_EVN_SCALE_UNITY ));
      }
      #endif
      ...
      if (ftm_rf_tech == FTM_RF_TECH_LTE)
      {
        ...
        {
         mline_params->env_scale = (int32)((rflte_nv_tbl_ptr->tx_path_nv_data[tx_nv_container]).\
      ...
      num_bytes = sizeof(ftm_common_xpt_mline_outputs) * num_rgi_delta;
       if(num_bytes)
       {
      mline_outputs_ptr_ept = (ftm_common_xpt_mline_outputs*)modem_mem_alloc(num_bytes, MODEM_MEM_CLIENT_RFA);
       }
	...
	sys_band = rfcommon_core_convert_rfcom_band_to_sys_band( RFM_1X_MODE, band_u );	
        ...
        /* This needs a device input!! */
        mline_params->env_scale = 
          (int32)(rf_cdma_xpt_get_cfg_param(tx_device, 
      ...
      if (ftm_rf_tech == FTM_RF_TECH_LTE)
      {
        ...
        if(rflte_nv_tbl_ptr != NULL)
        {
         mline_params->env_scale = (int32)rflte_nv_tbl_ptr->tx_path_nv_data[tx_nv_container].tx_xpt_dpd_config_params[ET_CFG_CMN_ENV_SCALE_UNITY];
        }
      }
      #endif
      ...
      num_bytes = sizeof(ftm_common_xpt_mline_outputs) * num_rgi_delta ;
       if(num_bytes)
       {
        mline_outputs_ptr_et = (ftm_common_xpt_mline_outputs*)modem_mem_alloc(num_bytes, MODEM_MEM_CLIENT_RFA);
       }
  ...
  if( sel_swp2_calc_pwr_data != NULL )
  {
    ...
    if( data_param->data_param.mline_params.comp_dB_et != COMP_POINT_INV && mline_outputs_ptr_et!= NULL)
    {
...
)
{
  ...
  if( return_val == TRUE )
  {
    ...
    if (ftm_rf_tech == FTM_RF_TECH_WCDMA)
    {
      rfm_device_enum_type remapped_rx_device, tx_device;
      #ifdef ATLAS_RF_WCDMA_PRESI
      tx_device = rfc_common_get_preferred_associated_tx_device( data_param->device );
      #else
      remapped_rx_device = ftm_wcdma_get_remapped_rx_device(data_param->device);
      ...
      delay_comp_done = rfm_wcdma_ftm_tx_get_xpt_delay_comp( tx_device, radio_state.rf_band, &delay_comp_x10ns );
      #else
      delay_comp_done = rfdevice_wcdma_tx_get_et_delay_compensation(tx_device,
                                                  ftm_curr_wcdma_mode,
                                                  &delay_comp_x10ns);
      #endif
      if (delay_comp_done) 
    ...
    if (ftm_rf_tech == FTM_RF_TECH_CDMA)
    {
      ...
      band_u.cdma_band = (rfm_cdma_band_class_type)rx_dev_state.curr_band;
      sys_band = rfcommon_core_convert_rfcom_band_to_sys_band( RFM_1X_MODE, band_u );
      dev = rfc_common_get_preferred_tx_device  (sys_band);
      band = rf_cdma_data_get_curr_band ( dev ) ;
	...
	tx_device = rfc_common_get_preferred_tx_device ( sys_band );
        ...
        rf_cdma_xpt_override_et_delay_value( tx_device,
                                          cal_result->et_path_delay_results.delay_p1ns,
    ...
    if (ftm_rf_tech == FTM_RF_TECH_LTE)
    {
      ...
      { 
        ...
        {
           FTM_MSG_1(FTM_ERROR,"ftm_calibration_data_process_delay: invalid tx device:%d", tx_device);
           return FALSE;
        }

        rfnv_band = rflte_core_convert_rfcom_band_to_rfnv_band(ftm_mc_sm_ptr->carrier[ftm_mc_sm_ptr->current_carrier_idx].tx_band);
        rflte_state_get_txlm_buffer_idx(sub_id,ftm_mc_sm_ptr->current_carrier_idx,&txlm_buffer_idx);
        rflte_nv_tbl_ptr = rflte_nv_get_tbl_ptr(rfnv_band);
        if (rflte_nv_tbl_ptr != NULL)
        {
          tx_nv_container = rflte_mc_get_tx_nv_container(ftm_mc_sm_ptr->current_carrier_idx,txlm_buffer_idx);
          if (tx_nv_container >= LTE_TX_MAX_NUM_CONTAINERS)
          {
            MSG_2 ( MSG_SSID_RF, MSG_LEGACY_ERROR,
                                "ftm_calibration_data_process_delay : ERROR : "
          ...
          delay_comp_done = FALSE;
          delay_comp_done = rfdevice_lte_tx_get_et_delay_compensation( tx_device,
           ...
           if (cal_data->cal_config.debug_mode_en)
	   {
          ...
          (rflte_nv_tbl_ptr->tx_path_nv_data[tx_nv_container]).\
           xpt_path_delay.num_of_active_entries = 1; 
          for (i=0; i<RFLTE_XPT_PATH_DELAY_ARRAY_SIZ; i++)
          {
            (rflte_nv_tbl_ptr->tx_path_nv_data[tx_nv_container]).\
              xpt_path_delay.upper_bound_tx_earfcn[i] = 0;
            switch ((rfcom_lte_bw_type)cal_result->et_path_delay_results.bandwidth)
            {
            case RFCOM_BW_LTE_5MHz:
              (rflte_nv_tbl_ptr->tx_path_nv_data[tx_nv_container]).\
               xpt_path_delay.upper_bound_tx_earfcn[i] = (uint16) (data_param->data_param.et_delay_params.upper_bound_tx_earfcn);
              (rflte_nv_tbl_ptr->tx_path_nv_data[tx_nv_container]).\
               xpt_path_delay.delay_value_bw5[i] = cal_result->et_path_delay_results.delay_p1ns;
              break;
            case RFCOM_BW_LTE_10MHz:
              (rflte_nv_tbl_ptr->tx_path_nv_data[tx_nv_container]).\
               xpt_path_delay.upper_bound_tx_earfcn[i] = (uint16) (data_param->data_param.et_delay_params.upper_bound_tx_earfcn);
              (rflte_nv_tbl_ptr->tx_path_nv_data[tx_nv_container]).\
               xpt_path_delay.delay_value_bw10[i] = cal_result->et_path_delay_results.delay_p1ns;
              break;
            case RFCOM_BW_LTE_20MHz:
              (rflte_nv_tbl_ptr->tx_path_nv_data[tx_nv_container]).\
               xpt_path_delay.upper_bound_tx_earfcn[i] = (uint16) (data_param->data_param.et_delay_params.upper_bound_tx_earfcn);
              (rflte_nv_tbl_ptr->tx_path_nv_data[tx_nv_container]).\
               xpt_path_delay.delay_value_bw20[i] = cal_result->et_path_delay_results.delay_p1ns;
              break;
            default:
              (rflte_nv_tbl_ptr->tx_path_nv_data[tx_nv_container]).\
               xpt_path_delay.upper_bound_tx_earfcn[i] = (uint16) (data_param->data_param.et_delay_params.upper_bound_tx_earfcn);
    ...
    if (ftm_rf_tech == FTM_RF_TECH_TDSCDMA)
    {
      ...
      delay_comp_done = FALSE;
#if 0   //todo
      delay_comp_done = rfdevice_tdscdma_tx_get_et_delay_compensation(RFCOM_TRANSCEIVER_0,
      ...
      {
        FTM_MSG( FTM_HIGH, "Could not get ET delay compensation for RC process variation. "
           "API rfdevice_tdscdma_tx_get_et_delay_compensation not implemented" );
      }
      tds_nv_tbl_ptr = rf_tdscdma_core_util_init_tx_nv_table(ftm_tdscdma_current_mode);
      if (tds_nv_tbl_ptr == NULL)
      {
        FTM_MSG(FTM_ERROR,"ftm_calibration_data_process_delay: Could not get TDS NV table pointer");
        return_val &= FALSE;
      }
      else /* overwrite NV table value with computed valued */
      {
        //tds_nv_tbl_ptr->et_delay_value = cal_result->et_path_delay_results.delay_p1ns;
        tds_nv_tbl_ptr->xpt_path_delay.num_of_active_entries = 1;
        tds_nv_tbl_ptr->xpt_path_delay.delay_value[0] = cal_result->et_path_delay_results.delay_p1ns;
        tds_nv_tbl_ptr->xpt_path_delay.upper_bound_tx_chan[0] = (uint16) (data_param->data_param.et_delay_params.upper_bound_tx_earfcn);
      }
    }
    #endif
  }
...
)
{
  ...
  ftm_calibration_pin_results_type *cal_result = NULL; // must do null checks
  ...
  if( return_val == TRUE )
  {
    ...
    cal_result->et_pin_results.pin_compensation = ftm_cal_data_get_pin_from_table(chan,
	  ...
	  if (nv_tbl == NULL)
      {
       ...
       /* overwrite NV table value with computed valued */
		   return_val = ftm_cal_data_store_pin_cal_offset(
					...
					pa_state,
					chan,
					cal_result->et_pin_results.pin_compensation);
    ...
    if (ftm_rf_tech == FTM_RF_TECH_CDMA)
    {
      ...
      band_u.cdma_band = (rfm_cdma_band_class_type)rx_dev_state.curr_band;
      sys_band = rfcommon_core_convert_rfcom_band_to_sys_band( RFM_1X_MODE, band_u );
      rfm_device = rfc_common_get_preferred_tx_device ( sys_band );
    ...
    if (ftm_rf_tech == FTM_RF_TECH_LTE)
    {
      ...
      { 
        ...
        rflte_state_get_txlm_buffer_idx(sub_id,ftm_mc_sm_ptr->current_carrier_idx,&txlm_buffer_idx);
        rflte_nv_tbl_ptr = rflte_nv_get_tbl_ptr(rfnv_band);
        /*Get Tx NV container for the current Tx Device and Carrier*/
        tx_nv_container = rflte_mc_get_tx_nv_container(ftm_mc_sm_ptr->current_carrier_idx,txlm_buffer_idx);
        if (rflte_nv_tbl_ptr != NULL)
        {
          if (tx_nv_container < LTE_TX_MAX_NUM_CONTAINERS)
          {
            pa_state_nv_ptr = &(rflte_nv_tbl_ptr->tx_path_nv_data[tx_nv_container].pa_state_nv);
          }
          else
          {
            FTM_MSG_1(FTM_ERROR,"ftm_calibration_data_process_pin: "
                     "Invalid NV Container for Tx : %d",tx_nv_container );
             return_val &= FALSE;
          }
           ...
           return_val &= ftm_cal_data_store_pin_cal_offset(&(pa_state_nv_ptr->lin_temp_comp_data),
                                                           ...
                                                           pa_state,
                                                           chan,
                                                          (-1)* cal_result->et_pin_results.pin_compensation);
   ...
   if( return_val == TRUE )
  {
    ...
    {
      memscpy( res_write_ptr, sizeof(uint32),
               &(cal_result->et_pin_results.channel), sizeof(uint32));
      UPDATE_SIZE_N_PTR( written_size, res_write_ptr, 
                         sizeof(uint32) );
      free_size -= sizeof(uint32);
    } /* if ( free_size >= sizeof(uint32) ) */
  ...
  if( return_val == TRUE )
  {
	...
	if (cal_data->cal_config.debug_mode_en)
        ...
        if( return_val == TRUE )
        {
          ...
          dpd_map_row->curr_tx_override.txfe0_handle_id = 0xFFFFFFFF;
          if (cal_data->cal_config.debug_mode_en)
	  {
...
/*!
  @brief
  Process the FBRx AGc Table Data Sub Opcode Instruction
 
  @details
  Process the FBRX Agc for the Data Opcode.
  The payload includes the FBRx Gain Cal data for all channels which
  needs to be saved in the data module for future processing.
...
*/
boolean 
ftm_calibration_data_process_fbrx_agc_tbl_instr
( 
  struct ftm_calibration_data_type_t *cal_data,
  ftm_cal_data_parameter_type const *data_param,
  ftm_cal_opcode_dispatch_info_type *oc_disp
...
{ 
  uint8 num_fbrx_agc_records = 0;
  uint8 lp_var = 0;
  boolean return_val = FALSE;
  ftm_cal_data_fbrx_agc_map_type *fbrx_agc_tbl = NULL;
  uint8 *res_write_ptr; /* results write pointer */
  uint16 written_size = 0; /* Total written size */
  uint16 free_size = 0; /* Total free size */
  uint8 res_type = FTM_CAL_OPCODE_DATA;
  uint8 res_action_type = DATA_RES_ACTION_FBRX_AGC_TBL;
  uint8 version = 0;

  num_fbrx_agc_records = 
    data_param->data_param.fbrx_agc_tbl.num_fbrx_entries;
  if (cal_data->cal_config.debug_mode_en)
  {
   FTM_MSG_1( FTM_MED, "Num FBRx Agc table entries = %d", num_fbrx_agc_records);
  }

  if( num_fbrx_agc_records > MAX_NUM_FBRX_AGC_RECORDS )
  {
    FTM_MSG_1( FTM_ERROR, "Number of records in the FBRx Agc TBL "
                          "cannot be greater than %d!", MAX_NUM_FBRX_AGC_RECORDS);

    return FALSE;
  }

  fbrx_agc_tbl = (ftm_cal_data_fbrx_agc_map_type *)modem_mem_alloc(
             sizeof(ftm_cal_data_fbrx_agc_map_type), MODEM_MEM_CLIENT_RFA);

  if( fbrx_agc_tbl == NULL )
  {
    FTM_MSG( FTM_ERROR, "Failed to allocate memory for FBRx Agc Table!");
    return FALSE;
  }
  ...
  if( cal_data->cal_config.debug_mode_en )
  {
    FTM_MSG( FTM_MED, "=== FBRx Agc Table ===");
  }
  
  fbrx_agc_tbl->num_fbrx_entries = (uint8)data_param->data_param.fbrx_agc_tbl.num_fbrx_entries;

  for( lp_var = 0; lp_var < num_fbrx_agc_records; ++lp_var )
  {
    fbrx_agc_tbl->table[lp_var].chan = 
      (uint32)data_param->data_param.fbrx_agc_tbl.table[lp_var].chan;

    fbrx_agc_tbl->table[lp_var].fbrx_mode = 
      (uint8)data_param->data_param.fbrx_agc_tbl.table[lp_var].fbrx_mode;

	fbrx_agc_tbl->table[lp_var].fbrx_gain_state = 
      (uint8)data_param->data_param.fbrx_agc_tbl.table[lp_var].fbrx_gain_state;

	fbrx_agc_tbl->table[lp_var].fbrx_gain = 
      (uint16)data_param->data_param.fbrx_agc_tbl.table[lp_var].fbrx_gain;

	fbrx_agc_tbl->table[lp_var].power = 
      (int16)data_param->data_param.fbrx_agc_tbl.table[lp_var].power_db10;

	fbrx_agc_tbl->table[lp_var].fbrx_ubias = 
      (uint8)data_param->data_param.fbrx_agc_tbl.table[lp_var].fbrx_ubias;

	fbrx_agc_tbl->table[lp_var].fbrx_rxscale = 
      (uint16)data_param->data_param.fbrx_agc_tbl.table[lp_var].fbrx_rxscale;

	if( cal_data->cal_config.debug_mode_en )
	{
      FTM_MSG_7( FTM_MED, "Channel, FBRxMode, FBRxGS, FBRxGain, Power, Ubias, Rxscale = %d, %d, %d, %d, %d, %d, %d", 
                 fbrx_agc_tbl->table[lp_var].chan,
                 fbrx_agc_tbl->table[lp_var].fbrx_mode,
				 fbrx_agc_tbl->table[lp_var].fbrx_gain_state,
				 fbrx_agc_tbl->table[lp_var].fbrx_gain,
  ...
  /* Fill invalid values for the rest of the table */
  for( ; lp_var < MAX_NUM_FBRX_AGC_RECORDS; ++lp_var )
  {
    fbrx_agc_tbl->table[lp_var].chan = 0;
    fbrx_agc_tbl->table[lp_var].fbrx_mode = 0;
	fbrx_agc_tbl->table[lp_var].fbrx_gain_state = 0;
	fbrx_agc_tbl->table[lp_var].fbrx_gain = 0;
	fbrx_agc_tbl->table[lp_var].power = 0;
	fbrx_agc_tbl->table[lp_var].fbrx_ubias = 0;
	fbrx_agc_tbl->table[lp_var].fbrx_rxscale = 0;
  ...
  return_val = ftm_cal_data_push_fbrx_agc_map(fbrx_agc_tbl);
  ...
  if( return_val == TRUE )
  {
    ...
    {
      FTM_MSG_4( FTM_ERROR, "[calv3][Segment %3d][DATA FBRX AGC TBL.{0x%x}] "
                 "ResultPush : ResType - ReqSize %d FreeSize %d",
                 cal_data->seg_data.cal_instance,
                 FTM_CAL_OPCODE_DATA,
                 sizeof(uint8),
  ...
  if( fbrx_agc_tbl != NULL )
  {
    modem_mem_free((void*)fbrx_agc_tbl, MODEM_MEM_CLIENT_RFA);
  }
   ...
   )
{
   ...
   for(i = 0 ; i <dpd_map_size; i++ )
   {
     ...
     if ( ret_option & RESULT_RX_RMS ) 
     {
       /*==========================================================================*/
       /* Populate Pre Equalized AMAM Rx RMS Value */
       /*==========================================================================*/
        dpd_result = (fw_dpd_cal_result_t*)module_ptr->data_module[module_ptr->dpd_map_cur_chan_idx].dpd_map->table[i].dpd;
        ...
        {
          memscpy( res_write_ptr, sizeof(uint32),
                   &dpd_result->rx_rms_mag, sizeof(uint32));
          UPDATE_SIZE_N_PTR( written_size, res_write_ptr, sizeof(uint32));

          free_size -= sizeof(uint32);
          FTM_MSG_1(FTM_HIGH, "Rx RMS Value is %d", dpd_result->rx_rms_mag);
        }
        else
        {
          FTM_MSG_4( FTM_ERROR, "[calv3][Segment %3d][DATA PROCESS XPT CAPTURE RESULTS{0x%x}] "
...
)
{
  ...
  if( load_action == LOAD_CALC_DPD_ACTION_LOAD_UNITY )
  {
    ...
    if( return_val == TRUE )
    {
      ...
      /* Get the current Tx override */
      memset( &curr_tx_ovr, 0, sizeof(rfcommon_core_xpt_tx_override_type) );
      return_val &= ftm_common_xpt_get_curr_tx_override( tx_device, 
                                                         &curr_tx_ovr );
      ovr_params.device = device_ind;
      ovr_params.xpt_mode = RFCOMMON_MDSP_XPT_MODE_APT;
      ovr_params.iq_gain_action_type = FTM_COMMON_XPT_TX_OVERRIDE_SKIP;
...
/*!
  @brief
  Process the AMAM based autoPin calculation for Data Opcode
 
  @details
  Process the AMAM based autoPin calculation, prepare the parameters, and then send compute 
  command to RF Test task, RF Test task will transfer this command to RF APPS task, we will 
  do the final calculation in the RF APPS task
...
*/
boolean 
ftm_calibration_data_process_delta_pinpout
( 
  struct ftm_calibration_data_type_t *cal_data,
  ftm_cal_data_parameter_type const *data_param,
  ftm_cal_opcode_dispatch_info_type *oc_disp
...
{ 
  /* Config radio request */
  ftm_cal_data_calc_delta_pin_pout_req_type data_pinpout_req;
  uint8 num_lut = 0;
  uint8 lp_var = 0;
  boolean return_val = FALSE;
  uint8 *res_write_ptr; /* results write pointer */
  ...
  uint16 free_size = 0; /* Total free size */
  uint8 res_type = FTM_CAL_OPCODE_DATA;
  uint8 res_action_type = DATA_RES_ACTION_CALC_DELTA_PIN_POUT;
  uint8 version = 0;
  ftm_cal_data_module_type  *data_module_ptr;
  
  data_module_ptr = ftm_common_data_get_data_module_ptr();

  num_lut = data_param->data_param.delta_pinpout_params.num_lut;
  data_module_ptr->delta_pinpout_module.num_lut = num_lut;

  if(NULL == data_module_ptr->delta_pinpout_module.table)
  {
    data_module_ptr->delta_pinpout_module.table = 
        modem_mem_alloc( num_lut * sizeof(ftm_cal_data_instr_payload_calc_delta_pinpout_type),
                         MODEM_MEM_CLIENT_RFA );
  }

  if(NULL == data_module_ptr->delta_pinpout_module.delta_pinpout_results_tbl)
  {
    data_module_ptr->delta_pinpout_module.delta_pinpout_results_tbl = 
        modem_mem_alloc( sizeof(delta_pinpout_results),
                         MODEM_MEM_CLIENT_RFA );
  }

  if((NULL != data_module_ptr->delta_pinpout_module.table) && 
     (NULL != data_module_ptr->delta_pinpout_module.delta_pinpout_results_tbl))
  {
    memscpy(data_module_ptr->delta_pinpout_module.table, 
            (sizeof(ftm_cal_data_instr_payload_calc_delta_pinpout_type)*num_lut),
            &data_param->data_param.delta_pinpout_params.table[0],
            (sizeof(ftm_cal_data_instr_payload_calc_delta_pinpout_type)*num_lut));
  }
  else
  {
    FTM_MSG( FTM_ERROR, "No Valid Memory for AutoPin Module!!");
    return FALSE;
  }

  /* the Calc Status is FALSE means that the Calculation result is not ready */
  data_module_ptr->delta_pinpout_module.delta_pinpout_results_tbl->calc_status = FALSE;

  /* Populate the Request Packet to do mline calculations */
  data_pinpout_req.cmd = DIAG_SUBSYS_CMD_F;
  data_pinpout_req.sub_sys = FTM;
  data_pinpout_req.mode = FTM_COMMON_C;
  data_pinpout_req.cdma_cmd = FTM_INT_CALV3_DELTA_PIN_POUT;
  data_pinpout_req.data_param = (uint32)data_param;

  ftm_common_send_radio_command( &data_pinpout_req,
                                 sizeof(ftm_cal_data_calc_delta_pin_pout_req_type) );
  ...
  {
    FTM_MSG_4( FTM_ERROR, "[calv3][Segment %3d][DATA PROCESS Delta PinPut.{0x%x}] "
               "ResultPush : ResType - ReqSize %d FreeSize %d",
               cal_data->seg_data.cal_instance,
               FTM_CAL_OPCODE_DATA,
               sizeof(uint8),
  ...
  {
    FTM_MSG_4( FTM_ERROR, "[calv3][Segment %3d][DATA Calc Delta PinPout.{0x%x}] "
               "ResultPush : dMask - ReqSize %d FreeSize %d",
               cal_data->seg_data.cal_instance,
               FTM_CAL_OPCODE_DATA,
               sizeof(ftm_cal_data_device_type),
  ...
  {
    FTM_MSG_4( FTM_ERROR, "[calv3][Segment %3d][DATA PROCESS Delta PinPout.{0x%x}] "
               "ResultPush : ResType - ReqSize %d FreeSize %d",
               cal_data->seg_data.cal_instance,
               DATA_RES_ACTION_CALC_DELTA_PIN_POUT,
               sizeof(uint8),
               free_size );
  } /* if ( free_size >= sizeof(uint8) ) */
...
/*!
  @brief
  Prepare AMAM data for delta pin/pout calculation
 
  @details
  Get the alignment AMAM data and DPD AMAM data from calV3 data module, then call
  Calc API to generate the LUTs
...
*/
void 
ftm_calibration_data_calc_delta_pin_pout
(
  void* cmd_ptr, 
...
)
{
  ...
  uint8 lp_var;
  uint8 num_lut;
  ftm_cal_data_module_table_type* data_module_tbl_ptr = NULL;
  
  uint16* ref_amam_addr_array[FTM_COMMON_XPT_NUM_DPD_MAP_ROWS];
  uint16* dpd_amam_addr_array[FTM_COMMON_XPT_NUM_DPD_MAP_ROWS];
  uint8 ref_amam_idx = 0;
  uint8 dpd_amam_idx = 0;
  ...
  num_lut = data_module_ptr->delta_pinpout_module.num_lut;

  data_module_tbl_ptr = &data_module_ptr->data_module[data_module_ptr->dpd_map_cur_chan_idx];

  data_module_ptr->delta_pinpout_module.delta_pinpout_results_tbl->num_lut = num_lut;

  if(NULL != data_module_ptr->delta_pinpout_module.table)
  {
    //Prepare the amam tables for the calculation: 
    //save the dpd amam and ref amam to the corresponding array
    for(lp_var = 0; lp_var < data_module_tbl_ptr->dpd_map->num_map_entries; lp_var++)
    {
      if(RFCOMMON_MDSP_XPT_MODE_EPT == 
           data_module_tbl_ptr->dpd_map->table[lp_var].curr_tx_override.xpt_mode)
      {
        if(FTM_COMMON_XPT_CUR_ORIDE_LOADED == 
           data_module_tbl_ptr->dpd_map->table[lp_var].status)
        {
          dpd_amam_addr_array[dpd_amam_idx++] = &data_module_tbl_ptr->dpd_table->table[lp_var].am_dpd[0];
        }
        else if(FTM_COMMON_XPT_IQ_PROCESSING_COMPLETE == 
                data_module_tbl_ptr->dpd_map->table[lp_var].status)
        {
          ref_amam_addr_array[ref_amam_idx++] = &data_module_tbl_ptr->dpd_table->table[lp_var].am_dist[0];
        }
        else
        {
          break;
    ...
    //Delta_pinpout_module has all the tools' params
    ftm_common_xpt_delta_gain_pin_pout_calc(&data_module_ptr->delta_pinpout_module,
                                            &ref_amam_addr_array[0],
                                            &dpd_amam_addr_array[0],
                                            cal_data->cal_config.debug_mode_en);

    /* the Calc Status is TRUE means that Calculation result is ready */
    data_module_ptr->delta_pinpout_module.delta_pinpout_results_tbl->calc_status = TRUE;
  }
  else
  {
    FTM_MSG(FTM_ERROR,"ftm_calibration_data_calc_delta_pin_pout: CMD Table Ptr is NULL");
  }

  prof_cal_func = timetick_get_elapsed(prof_cal_func, T_USEC);
  RF_MSG_1(RF_HIGH, "ftm_calibration_data_process_calc_delta_pinpout_instr:Profiling - %d", prof_cal_func);
} /* ftm_calibration_data_process_calc_delta_pinpout_instr */
/*! @} */

/*----------------------------------------------------------------------------*/
/*!
  @brief
  Prepare the delta pin/pout result and return to tools
 
  @details
  This function get the delta pin/pout results from calV3 data module, and return it to 
  tools.
 
  @param NULL
 
  @return
  Flag indicating the status of this API
*/
boolean
ftm_calibration_data_process_delta_pinpout_results
(
  struct ftm_calibration_data_type_t *cal_data,
  ftm_cal_data_parameter_type const *data_param,
  ftm_cal_opcode_dispatch_info_type *oc_disp
...
{ 
  ...
  uint8 row_num_lut = 0;
  uint8 lp_var = 0;
  uint8 inner_lp_var = 0;
  boolean return_val = FALSE;
  uint8 *res_write_ptr; /* results write pointer */
  uint16 written_size = 0; /* Total written size */
  uint16 free_size = 0; /* Total free size */
  uint8 res_type = FTM_CAL_OPCODE_DATA;
  uint8 res_action_type = DATA_RES_ACTION_GET_DELTA_PIN_POUT;
  uint8 version = 0;
  ftm_cal_data_module_type* data_module_ptr;
  
  data_module_ptr = ftm_common_data_get_data_module_ptr();
  
  if(NULL == data_module_ptr)
  {
    FTM_MSG(FTM_ERROR,"data_module_ptr is NULL");
    return FALSE;
  }
  else
  {
    if(NULL == data_module_ptr->delta_pinpout_module.delta_pinpout_results_tbl)
	{
	  FTM_MSG(FTM_ERROR,"delta_pinpout_results_tbl is NULL");
	  return FALSE;
	}
  }

  num_lut = data_module_ptr->delta_pinpout_module.delta_pinpout_results_tbl->num_lut;
  
  /* Populate the Request Packet to do mline calculations */
  if(FALSE == data_module_ptr->delta_pinpout_module.delta_pinpout_results_tbl->calc_status)
  {
    FTM_MSG(FTM_ERROR,"The calculation still running!!");
    return FALSE;
  }
  else
  {
  ...
  {
    FTM_MSG_4( FTM_ERROR, "[calv3][Segment %3d][Return Delta PinPout Results.{0x%x}] "
                          "ResultPush : ResType - ReqSize %d FreeSize %d",
                          cal_data->seg_data.cal_instance,
                          FTM_CAL_OPCODE_DATA,
                          sizeof(uint8),
  ...
  else
  {
    FTM_MSG_4( FTM_ERROR, "[calv3][Segment %3d][Return Delta PinPout Results.{0x%x}] "
                          "ResultPush : ResType - ReqSize %d FreeSize %d",
                          cal_data->seg_data.cal_instance,
                          DATA_RES_ACTION_GET_DELTA_PIN_POUT,
                          sizeof(uint8),
                          free_size );
  } /* if ( free_size >= sizeof(uint8) ) */

  /*==========================================================================*/
  /* Populate Number of Look Up Table */
  /*==========================================================================*/
  if ( free_size >= sizeof(uint8) )
  {
    memscpy( res_write_ptr, sizeof(uint8),
             &(data_module_ptr->delta_pinpout_module.delta_pinpout_results_tbl->num_lut), 
             sizeof(uint8));
    UPDATE_SIZE_N_PTR( written_size, res_write_ptr, 
                       sizeof(uint8) );
    free_size -= sizeof(uint8);
  ...
  /*==========================================================================*/
  /* Populate the Gain Pin Pout Table of Each DPD Index */
  /*==========================================================================*/
  for(lp_var = 0;lp_var < num_lut; lp_var++)
  {
    row_num_lut = data_module_ptr->delta_pinpout_module.delta_pinpout_results_tbl->lut_results[lp_var].num_lut_row;

    if(free_size>= sizeof(uint8))
    {
      memscpy( res_write_ptr, sizeof(uint8),
               &(data_module_ptr->delta_pinpout_module.delta_pinpout_results_tbl->lut_results[lp_var].num_lut_row), 
               sizeof(uint8));
      UPDATE_SIZE_N_PTR( written_size, res_write_ptr, 
                         sizeof(uint8) );
      free_size -= sizeof(uint8);
    ...
    {
      memscpy( res_write_ptr, sizeof(uint8),
               &(data_module_ptr->delta_pinpout_module.delta_pinpout_results_tbl->lut_results[lp_var].dpd_idx), 
               sizeof(uint8));
      UPDATE_SIZE_N_PTR( written_size, res_write_ptr, 
                         sizeof(uint8) );
      free_size -= sizeof(uint8);
    ...
    for(inner_lp_var = 0; inner_lp_var < row_num_lut; inner_lp_var++)
    {
      if(free_size>= sizeof(uint8))
      {
        memscpy( res_write_ptr, sizeof(delta_pinp_out_lut_rows_results),
                 &(data_module_ptr->delta_pinpout_module.delta_pinpout_results_tbl->lut_results[lp_var].lut_rows[inner_lp_var]), 
                 sizeof(delta_pinp_out_lut_rows_results));
        UPDATE_SIZE_N_PTR( written_size, res_write_ptr, 
                           sizeof(delta_pinp_out_lut_rows_results) );
        free_size -= sizeof(delta_pinp_out_lut_rows_results);
      }
      else
      {
        FTM_MSG_4( FTM_ERROR, "[calv3][Segment %3d][Return Delta PinPout Results.{0x%x}] "
  ...
  if(NULL!=data_module_ptr->delta_pinpout_module.table)
  {
    modem_mem_free(data_module_ptr->delta_pinpout_module.table,MODEM_MEM_CLIENT_RFA);
    data_module_ptr->delta_pinpout_module.table = NULL;
    data_module_ptr->delta_pinpout_module.num_lut = 0;
  }

  if(NULL!=data_module_ptr->delta_pinpout_module.delta_pinpout_results_tbl)
  {
    modem_mem_free(data_module_ptr->delta_pinpout_module.delta_pinpout_results_tbl,MODEM_MEM_CLIENT_RFA);
    data_module_ptr->delta_pinpout_module.delta_pinpout_results_tbl = NULL;
  }
...
)
{
  ...
  /* AutoPin Action params */
  uint8 num_delta_pinpout_lut;
  ...
  /*--------------------------------------------------------------------------*/
  /* 0x80 : Pin Val processing */
  /*--------------------------------------------------------------------------*/
  if( curr_action & DATA_OPCODE_MASK_PROCESS_PIN_VAL )
  {
  ...
  /*--------------------------------------------------------------------------*/
  /* 0x800 : FBRx Agc Table */
  /*--------------------------------------------------------------------------*/
  if( curr_action & DATA_OPCODE_MASK_PUSH_FBRX_AGC_TBL )
  {
	memscpy( &(data_unpack->data_param.fbrx_agc_tbl.data_type),sizeof(uint8),
            payload_ptr, sizeof(uint8) );
  
    /* Update the payload size and Payload pointer */
    UPDATE_SIZE_N_PTR( payload_size, payload_ptr, sizeof(uint8) );

    memscpy( &(data_unpack->data_param.fbrx_agc_tbl.num_fbrx_entries),sizeof(uint8),
            payload_ptr, sizeof(uint8) );
  
    /* Update the payload size and Payload pointer */
    UPDATE_SIZE_N_PTR( payload_size, payload_ptr, sizeof(uint8) );

    uint8 num_fbrx_agc_records = data_unpack->data_param.fbrx_agc_tbl.num_fbrx_entries;
  
    for( lp_var = 0; lp_var < num_fbrx_agc_records; ++lp_var )
    {
      memscpy( &(data_unpack->data_param.fbrx_agc_tbl.table[lp_var]),sizeof(ftm_cal_data_fbrx_agc_map_row_type),
              payload_ptr, sizeof(ftm_cal_data_fbrx_agc_map_row_type) );
  
      /* Update the payload size and Payload pointer */
      UPDATE_SIZE_N_PTR( payload_size, payload_ptr, sizeof(ftm_cal_data_fbrx_agc_map_row_type) );
    }
  }

  /*--------------------------------------------------------------------------*/
  /* 0x4000 : Calculate Delta Gain Pin Pout Table */
  /*--------------------------------------------------------------------------*/
  if( curr_action & DATA_OPCODE_MASK_CALC_DELTA_PIN_POUT )
  {
    memscpy( &(data_unpack->data_param.delta_pinpout_params.num_lut),sizeof(uint8),
             payload_ptr, sizeof(uint8) );
    /* Update the payload size and Payload pointer */
    UPDATE_SIZE_N_PTR( payload_size, payload_ptr, sizeof(uint8) );
    num_delta_pinpout_lut= data_unpack->data_param.delta_pinpout_params.num_lut;
  
    for(lp_var = 0; lp_var < num_delta_pinpout_lut; lp_var++)
    {
      memscpy( &(data_unpack->data_param.delta_pinpout_params.table[lp_var]),
               sizeof(ftm_cal_data_instr_payload_calc_delta_pinpout_type),
               payload_ptr, 
               sizeof(ftm_cal_data_instr_payload_calc_delta_pinpout_type) );
      /* Update the payload size and Payload pointer */
      UPDATE_SIZE_N_PTR( payload_size, 
                         payload_ptr, 
                         sizeof(ftm_cal_data_instr_payload_calc_delta_pinpout_type) );
  
      /* this msg is only for FR bring up, will delete after FR complete*/
      if( cal_data->cal_config.debug_mode_en )
      {
        FTM_MSG_4( FTM_MED, "dpd_tbl_idx, align_swp_idx pin_mis_align_min"
                            " pin_mis_align_max = %d, %d, %d, %d", 
                              data_unpack->data_param.delta_pinpout_params.table[lp_var].dpd_tbl_idx,
                              data_unpack->data_param.delta_pinpout_params.table[lp_var].align_swp_idx,
                              data_unpack->data_param.delta_pinpout_params.table[lp_var].pin_mis_align_min,
                              data_unpack->data_param.delta_pinpout_params.table[lp_var].pin_mis_align_max);
      }
    }
  }
  
  /*--------------------------------------------------------------------------*/
  /* 0x8000 : Get Result of Delta Gain Pin Pout Table */
  /*--------------------------------------------------------------------------*/
  if( curr_action & DATA_OPCODE_MASK_GET_DELTA_PIN_POUT )
  {
    /* No paremeters of OpCode 15*/
    if( cal_data->cal_config.debug_mode_en )
    {
      FTM_MSG_1( FTM_MED, "No Parameter of OpCode = %d", DATA_OPCODE_MASK_GET_DELTA_PIN_POUT);
    }
  }
...
)
{
  ...
  /* 0x80 : Pin Processing */
  /*--------------------------------------------------------------------------*/
  else if( curr_action & DATA_OPCODE_MASK_PROCESS_PIN_VAL )
  {    
    if (cal_data->cal_config.debug_mode_en)
    {
     FTM_MSG_2( FTM_HIGH, "[calv3][Segment %3d][DATA.{0x%x}] "
  ...
  /* 0x800 : FBRx AGC Table*/
  /*--------------------------------------------------------------------------*/
  else if( curr_action & DATA_OPCODE_MASK_PUSH_FBRX_AGC_TBL )
  {
    FTM_MSG_2( FTM_HIGH, "[calv3][Segment %3d][DATA.{0x%x}] "
               "Process : FBRx Agc Data Push Table Sub Opcode Processing",
               cal_data->seg_data.cal_instance,
               FTM_CAL_OPCODE_DATA );  

    ftm_calibration_data_process_fbrx_agc_tbl_instr( cal_data, 
													&(cfg_data->up_param), 
                                                    oc_disp );
  ...
  else if( curr_action & DATA_OPCODE_MASK_CALC_DELTA_PIN_POUT )
  {
    FTM_MSG_2( FTM_HIGH, "[calv3][Segment %3d][DATA.{0x%x}] "
               "Process : Calc Delta PinPout Sub Opcode Processing",
               cal_data->seg_data.cal_instance,
               DATA_OPCODE_MASK_CALC_DELTA_PIN_POUT );  

    ftm_calibration_data_process_delta_pinpout( cal_data, 
                                                &(cfg_data->up_param), 
                                                oc_disp );
  ...
  else if( curr_action & DATA_OPCODE_MASK_GET_DELTA_PIN_POUT )
  {
    FTM_MSG_2( FTM_HIGH, "[calv3][Segment %3d][DATA.{0x%x}] "
               "Process : Return Delta PinPout Sub Opcode Processing",
               cal_data->seg_data.cal_instance,
               DATA_OPCODE_MASK_GET_DELTA_PIN_POUT );  

    return_val = ftm_calibration_data_process_delta_pinpout_results( cal_data, 
                                                          &(cfg_data->up_param), 
                                                          oc_disp );
  } /* if( curr_action & DATA_OPCODE_MASK_GET_DELTA_PIN_POUT ) */
