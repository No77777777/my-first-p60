...
/*==============================================================================
  ...
  $Header: //components/rel/rfa.mpss/4.11/rf/common/ftm/src/ftm_calibration_v3_tx.c#1 $
  ...
  while ( device_mask_1bit != 0 )
  {
    ...
    if ( device_ind < FTM_CAL_V3_TX_MAX_DEV )
    {
      ...
      while ( action_mask_1bit != 0 )
      {
		...
		case TX_MEAS_FB_GAIN_AGC:
          /*------------------------------------------------------------------*/
          /* Populate FB Gain Agc Meas Results */
          /*------------------------------------------------------------------*/
          if ( free_size >= sizeof(ftm_cal_tx_meas_fb_gain_agc_res_type) )
          {
            memscpy( 
               res_write_ptr, sizeof(ftm_cal_tx_meas_fb_gain_agc_res_type),
               &(cal_result->tx_meas_param[device_ind].fb_gain_agc_meas.fb_agc_meas_res),
               sizeof(ftm_cal_tx_meas_fb_gain_agc_res_type));
            UPDATE_SIZE_N_PTR( written_size, res_write_ptr, 
                               sizeof(ftm_cal_tx_meas_fb_gain_agc_res_type) );
            free_size -= sizeof(ftm_cal_tx_meas_fb_gain_agc_res_type);
          } /*if ( free_size >= sizeof(ftm_cal_tx_meas_fb_gain_res_type) )*/
          else
          {
            FTM_MSG_4( FTM_ERROR, "[calv3][Segment %3d][TX_MEASURE.{0x%x}] "
                       "ResultPush : FB Gain Agc Meas - ReqSize %d FreeSize %d",
                       cal_data->seg_data.cal_instance,
                       FTM_CAL_OPCODE_TX_MEASURE,
                       sizeof(ftm_cal_tx_meas_fb_gain_agc_res_type),
                       free_size );
  ...
  while ( device_mask_1bit != 0 )
  {
    ...
    if ( device_ind < FTM_CAL_V3_TX_MAX_DEV )
    {
      ...
      while ( tx_meas_1bit != 0 )
      {
		...
		case TX_MEAS_FB_GAIN_AGC:
          memscpy( &(tm_unpack->tx_meas_param[device_ind].fb_gain_agc.fb_gain_agc_params.fb_gain_state),
                   sizeof(uint8), payload_ptr, sizeof(uint8) );
          UPDATE_SIZE_N_PTR( payload_size, payload_ptr, sizeof(uint8) );

          memscpy( &(tm_unpack->tx_meas_param[device_ind].fb_gain_agc.fb_gain_agc_params.exp_power),
                   sizeof(int16), payload_ptr, sizeof(int16) );
          UPDATE_SIZE_N_PTR( payload_size, payload_ptr, sizeof(int16) );

		  memscpy( &(tm_unpack->tx_meas_param[device_ind].fb_gain_agc.fb_gain_agc_params.fbrx_action),
                   sizeof(uint8), payload_ptr, sizeof(uint8) );
          UPDATE_SIZE_N_PTR( payload_size, payload_ptr, sizeof(uint8) );

          memscpy( &(tm_unpack->tx_meas_param[device_ind].fb_gain_agc.fb_gain_agc_params.debug_mode_en),
                   sizeof(uint8), payload_ptr, sizeof(uint8) );
          UPDATE_SIZE_N_PTR( payload_size, payload_ptr, sizeof(uint8) );
