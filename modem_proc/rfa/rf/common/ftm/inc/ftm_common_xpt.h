...
/*==============================================================================
  ...
  $Header: //components/rel/rfa.mpss/4.11/rf/common/ftm/inc/ftm_common_xpt.h#1 $

when       who     what, where, why
--------   ---     -------------------------------------------------------------
04/13/16   zhh    Support Pin/Pout LUT Vin Scale calculation
02/18/16   zhh    Add new structures and API for autoPin calculation
...
/*----------------------------------------------------------------------------*/
/*! Number about autoPin Calculation*/
#define FTM_XPT_COMMON_NUM_DPD_AMAM_POINTS                64
...
typedef struct
{
  ...
  uint8 mline_bias_opt;
  /*!< MLine Bias Option value for M Line calculation */
...
/*!
  @brief
  Structure template for autoPin calculation

  @details
  Contains the misalign_pin, gain, misalign_pout row result
*/
typedef struct
{
  int8 misalign_pin;
  int8 gain;
  int8 misalign_pout;
}delta_pinp_out_lut_rows_results;

/*----------------------------------------------------------------------------*/
/* Structure for the autoPin calculation results of each LUT*/
typedef struct
{
  uint8 num_lut_row;
  uint8 dpd_idx;
  delta_pinp_out_lut_rows_results lut_rows[RFCOM_TXLIN_NUM_PIN];
}delta_gain_pin_pout_lut_results;

/*----------------------------------------------------------------------------*/
/* Structure for the autoPin calculation results*/
typedef struct
{
  boolean calc_status;
  uint8 num_lut;
  delta_gain_pin_pout_lut_results lut_results[RFCOM_TXLIN_NUM_PIN_LUT];
}delta_pinpout_results;

/*----------------------------------------------------------------------------*/
/* Structure for the autoPin calculation commands from tools*/
typedef PACK(struct)
{
  uint8 dpd_tbl_idx;
  uint8 align_swp_idx;
  uint8 lut_step_size;
  int8 pin_mis_align_min;
  int8 pin_mis_align_max;
}ftm_cal_data_instr_payload_calc_delta_pinpout_type;

/*----------------------------------------------------------------------------*/
/* Structure for the autoPin calculation commds and results*/
typedef struct
{
  uint8 num_lut;
  ftm_cal_data_instr_payload_calc_delta_pinpout_type* table;
  delta_pinpout_results* delta_pinpout_results_tbl;
}ftm_cal_data_delta_pinpout_table_type;
...
ftm_common_xpt_get_tx_default_device
( 
...
/*----------------------------------------------------------------------------*/
boolean
ftm_common_xpt_delta_gain_pin_pout_calc
( 
  ftm_cal_data_delta_pinpout_table_type* delta_pinpout_tbl, 
  uint16* ref_amam_addr_array_ptr[],
  uint16* dpd_amam_addr_array_ptr[],
  boolean debug_msg
);

/*----------------------------------------------------------------------------*/
uint16
ftm_common_xpt_vin_ndb_ref_idx_calc
( 
  uint16* ref_amam_ptr,
  uint16* dpd_amam_ptr,
  float smallsig_thresh,
  boolean debug_msg
);
