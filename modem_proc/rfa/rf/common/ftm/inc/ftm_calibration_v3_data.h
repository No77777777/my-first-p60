...
/*==============================================================================
  ...
  $Header: //components/rel/rfa.mpss/4.11/rf/common/ftm/inc/ftm_calibration_v3_data.h#1 $

when       who     what, where, why
--------   ---     -------------------------------------------------------------
01/27/15   zhh    Add the new API for AMAM based autoPin calculation
...
/*----------------------------------------------------------------------------*/
/*! Enumeration to indicate Maximum number of FBRx Agc records in the 
    FBRx Agc Table */
#define MAX_NUM_FBRX_AGC_RECORDS 96
...
typedef enum
{
  ...
  DATA_OPCODE_MASK_PUSH_FBRX_AGC_TBL    = 2048,
  /*!< Push FBRx Agc Table Data  */

  DATA_OPCODE_MASK_CALC_DELTA_PIN_POUT  = 16384,
  /*!< Process Data - AMAM based autoPin Calculation */
  
  DATA_OPCODE_MASK_GET_DELTA_PIN_POUT   = 32768
  /*!< Return Results - AMAM based autoPin Results */
...
typedef enum
{
  ...
  DATA_RES_ACTION_FBRX_AGC_TBL          = 11,
  /*!< Result type for FBRx Agc TBL Action */

  DATA_RES_ACTION_CALC_DELTA_PIN_POUT   = 14,
  /*!< Result type for AMAM based autoPin Calc Action */
  
  DATA_RES_ACTION_GET_DELTA_PIN_POUT   = 15,
  /*!< Result type to return AMAM based autoPin results */
...
typedef enum
{
   ...
   RESULT_RX_RMS                = 8
   /*!< XPT Result Action - Return Pre Equalized AMAM Rx RMS Value*/
...
typedef enum
{
  ...
  DATA_MLINE_CALC_WITH_p1dB_COMPPT_BACKOFF = 4,
  ...
  DATA_MLINE_CALC_WITH_p3dB_COMPPT_BACKOFF = 5,
  ...
  DATA_MLINE_CALC_WITH_p5dB_COMPPT_BACKOFF = 6
...
/*!
  @brief
  FTM Common Dispatch Request Packet to be used for AMMA based autoPin Calculation 
*/
typedef PACK(struct)
{
  uint8 cmd;
...
} ftm_cal_data_calc_delta_pin_pout_req_type;
...
/*!
  @brief
  Structure template for Data Push FBRx Agc Payload

  @details
  Contains the Payload for Data Push FBRx Agc Gain Cal Table
*/
typedef PACK(struct)
{
  uint32 chan; 
  /*!< channel in the FBRx Agc Table */

  uint8 fbrx_mode;
  /*!< FBRx mode entry for the FBRx Agc Table */

  uint8 fbrx_gain_state;
  /*!< FBRx gain state entry for the FBRx Agc Table */

  int16 fbrx_gain;
  /*!< FBRx Gain value in the  FBRx Agc Table */

  int16 power_db10;
  /*!< Power entry in the FBRx Agc Table */

  uint8 fbrx_ubias;
  /*!< FBRx Ubias entry in the  FBRx Agc Table */

  uint16 fbrx_rxscale;
  /*!< FBRx rxscale entry in the FBRx Agc Table */

} ftm_cal_data_instr_payload_fbrx_agc_map_row_type;
...
typedef struct
{
  ...
  struct
  {
    ...
    {
	  uint8 data_type;
	  uint8 num_fbrx_entries;
      ftm_cal_data_instr_payload_fbrx_agc_map_row_type table[MAX_NUM_FBRX_AGC_RECORDS];
    }fbrx_agc_tbl;
    ...
    {
      uint8 num_lut;
      ftm_cal_data_instr_payload_calc_delta_pinpout_type table[MAX_NUM_MEAS_PWRS];
    }delta_pinpout_params;
    /*!< Delta PinPout for AutoPin Params */
