...
$Header: //components/rel/rfa.mpss/4.11/rf/common/ftm/inc/ftm_common_iq_processing.h#1 $ 
$DateTime: 2021/03/09 05:11:02 $ 
$Author: pwbldsvc $

when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
02/11/15   jmf     [selftest] Add parameters for LTE Spec. Flatness Meas. Support
...
typedef struct {
  ...
  uint32 samp_freq;
  uint32 center_freq;
  uint32 band_edge_low;
  uint32 band_edge_high;
} ftm_common_spec_flat_input_type;
...
typedef struct {
  ...
  uint16 indexes[8];
  int32 flatness[8];
} ftm_common_spec_flat_output_type;

typedef enum {
  FORMAT_8BITIQ = 0 
} ftm_common_tx_sample_format_enum;

typedef enum {
  FORMAT_PCFL20 = 0
} ftm_common_rx_sample_format_enum;

typedef struct {
  void * tx_sample_ptr;
  void * rx_sample_ptr;
  uint16 tx_sample_len;
  uint16 rx_sample_len;
  ftm_common_tx_sample_format_enum tx_sample_format;
  ftm_common_rx_sample_format_enum rx_sample_format;
  uint16 search_center;
  uint16 search_width;
  uint16 process_len;
  uint8  tx_first_samp_idx;
 } ftm_common_txrx_offset_calc_in_struct;
...
typedef struct {
  ...
  int32 rx_dcI;
  int32 rx_dcQ;
  int32 quality_indicator;
 } ftm_common_txrx_offset_calc_out_struct;
...
uint8 group_delay;
uint8 num_taps; 
int32 coeffs[2*RFCOMMON_FBRX_NUM_FILTER_TAPS];
boolean is_complex;
} ftm_common_undroop_filter_struct_type;
...
boolean ftm_common_reinit_fft_data_for_spec_flatness(
ftm_rfstate_enum_type mode_current,
rfcom_lte_bw_type bw_current,
rfcommon_fbrx_nv_droop_type *droop_data
);

boolean ftm_common_selftest_parse_undroop_filter(rfcommon_fbrx_nv_droop_type *droop_data);
...
void  ftm_common_find_txrx_offset(
   ftm_common_txrx_offset_calc_in_struct * input, 
   ftm_common_txrx_offset_calc_out_struct * output );

void ftm_common_parse_tx_IQ8BIT_to_IQ32BIT( 
   ftm_rfstate_enum_type mode,
   int32* samples_out,
   uint16 sample_count,
   int32* samples_in,
...
void ftm_common_get_samples_fft_avg_tx(uint16 fft_size, int32* samples_iq);

void ftm_common_get_undroop_filter_psd(uint16 fft_size, int32 * sample_buffer);

void ftm_common_get_flatness_measurement(
   ftm_rfstate_enum_type mode,
