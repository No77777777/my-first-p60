...
/*==============================================================================
  ...
  $Header: //components/rel/rfa.mpss/4.11/rf/common/ftm/inc/ftm_calibration_v3_tx.h#1 $
...
*/
#define FTM_CAL_TX_MEAS_BUFF_SIZE_MAX 140

/*----------------------------------------------------------------------------*/
/*! Enumeration to indicate the maximum number of devices */
#define FTM_CAL_V3_TX_MAX_DEV RFM_MAX_WAN_DEVICES
...
typedef PACK(struct) 
{
  ...
  uint16 fb_meas_stat;
  /*!< FB Gain Status Error*/

  int16 fb_gain_val;
  /*!< FB Gain Gain Result Value */

  uint8 fb_ubias;
  /*!< FB Gain Ubias Result */

  uint16 fb_rxscale;
  /*!< FB Gain Meas RxScale Result */

  uint32 fb_residual_dc;
  /*!< FB Residual DC Result */

  uint32 fb_rsb;
  /*!< FB Complex RSB Result */

  int16 fb_meas_gain_imb;
  /*!< FB Gain Imbalance Result */

  int16 fb_meas_phase_imb;
  /*!< FB Gain hase Imbalance Result */

  uint32 fb_meas_tx_tx_conj_corr;
  /*!< FB Gain Measurement tx_tx_conj_corr Result */

  uint32 fb_meas_tx_tx_corr;
  /*!< FB Gain Measurement tx_tx_corr Result */

  uint32 fb_meas_tx_sum;
  /*!< FB Gain Measurement tx_sum Result */

  uint32 fb_meas_rx_tx_conj_corr;
  /*!< FB Gain Measurement rx_tx_conj_corr Result */

  uint32 fb_meas_rx_tx_corr;
  /*!< FB Gain Measurement rx_tx_corr Result */

  uint32 fb_meas_rx_sum;
  /*!< FB Gain Measurement rx_sum Result */

  uint32 fb_meas_rx_rx_conj_corr;
  /*!< FB Gain Measurement rx_rx_conj_corr Result */

  uint32 fb_meas_rx_rx_corr;
  /*!< FB Gain Measurement rx_rx_corr Result */

  int32 fb_rms_signal_level;
  /*!< FB Gain Measurement RMS Signal Level */

  uint32 fb_meas_ls_err;
  /*!< FB Gain LS Error in the Result */

  uint16 fb_max_corr_peak;
  /*!< FB Gain Max Correlation Peak Result */

  uint8 fb_max_corr_peak_idx;
  /*!< FB Gain Max Correlation Peak Index Result */

  uint16 fb_meas_xcorr_log[FTM_CALIBRATION_FBRX_GAIN_MEAS_XCORR_LOG_SIZE];
  /*!< FB Gain Measurement Correlation Log of the Result */

} ftm_cal_tx_meas_fb_gain_agc_res_type;
...
typedef enum
{
  ...
  TX_MEAS_FB_GAIN_AGC = 5, 
  /*!< FB Gain AGC measurement */
...
/*!
  @brief
  Structure used to hold the unpacked FB Gain Agc meas parameter 
*/
typedef struct
{
  uint8 fb_gain_state;
  ...
  int16 exp_power;
  /*!< Expected power for FBRx Cal */

  uint8 fbrx_action;
  /*!< Compute Ubias and RxScale based on the FBRx Action mask*/

  boolean debug_mode_en;
  /*!< Debug Mode Enable for the FBRx Cal */

} ftm_cal_tx_meas_fb_gain_agc_payload_type;
...
typedef struct
{
  ...
  struct
  {
	...
	struct
    {
      ftm_cal_tx_meas_fb_gain_agc_payload_type fb_gain_agc_params;
      /*!< FB Power Measurement Params */

    } fb_gain_agc;
    /*!< FB Gain measurements based on agc*/
...
typedef struct
{
  ...
  struct
  {
    ...
    struct
    {
      ftm_cal_tx_meas_fb_gain_agc_res_type fb_agc_meas_res;
      /*!< FB Gain Measurement Results */

    } fb_gain_agc_meas;
    /*!< FB Gain Agc Measurement Results */
