...
/*==============================================================================
  ...
  $Header: //components/rel/rfa.mpss/4.11/rf/common/ftm/inc/ftm_calibration_data_module.h#1 $

when       who     what, where, why
--------   ---     -------------------------------------------------------------
02/17/15   zhh   Add the data module for AMAM based autoPin calculation
08/06/15   cac     Support for FBRx Gain Cal Agc
...
#define FTM_COMMON_FBRX_AGC_MAP_ROWS 96
...
/*---------- FBRx Agc Map Table Row ----------*/
typedef struct
{
  uint32 chan; /* channel for which FBRx record is stored */
  uint8 fbrx_mode;
  ...
  uint8 fbrx_ubias;
  uint16 fbrx_rxscale;
}ftm_cal_data_fbrx_agc_map_row_type;


/*---------- FBRx Agc Table Map ----------*/
typedef struct
{
  /*! @brief table containing rows of FBRx Agc map data */
  uint8 num_fbrx_entries;
  ftm_cal_data_fbrx_agc_map_row_type table[FTM_COMMON_FBRX_AGC_MAP_ROWS];
} ftm_cal_data_fbrx_agc_map_type;
...
typedef struct
{
  ...
  /* data module for AMAM based autoPin calculation*/
  ftm_cal_data_delta_pinpout_table_type delta_pinpout_module;
...
boolean ftm_cal_data_push_multiple_rgid_data(uint32 chan,
                                    ...
                                    ftm_cal_data_rgid_type *rgid_table);
/* -------------------------------------------------------------------------- */
boolean ftm_cal_data_push_fbrx_agc_map(ftm_cal_data_fbrx_agc_map_type *input_fbrx_agc_map);
/* -------------------------------------------------------------------------- */
