...
Copyright (c) 2005 - 2015 by QUALCOMM Technologies, Inc.  All Rights Reserved.
...
$Header: //components/rel/rfa.mpss/4.11/rf/common/ftm/inc/ftm_common_dispatch.h#1 $ $DateTime:

when       who     what, where, why
--------   ---     --------------------------------------------------------- 
05/08/15   adk     Added FTM API to query CL algo state
03/13/15   ska     Add FTM RF support for DGLNA feature
03/17/15   al      Modified type def of ftm_get_rffe_device_ids_response
03/03/15   al      New type def of ftm_get_rffe_device_ids_response
...
#define MAX_TECH 2 /*Max tech allowed sub1 and sub2*/
...
typedef PACK(struct){
	...
	uint2 ftm_cmd;
    int16 tech;
} ftm_get_tuner_info_request_type;
...
typedef PACK(struct) {
  ...
  uint2 rsp_len;
  uint2 status;
} ftm_ant_div_switch_response_type;
...
typedef PACK(struct){
	...
	uint2 ftm_cmd;
	uint16 status;
    uint16 mfg_id[RFC_COMMON_MAX_DEVICE_INSTANCES_PER_BAND];
	uint16 prd_id[RFC_COMMON_MAX_DEVICE_INSTANCES_PER_BAND];
	uint16 rev_id[RFC_COMMON_MAX_DEVICE_INSTANCES_PER_BAND];
...
typedef PACK(struct){
	...
	uint1 sub_sys;
	uint2 ftm_cmd;
	uint8 num_rfm_devices;
    uint8 cl_algo_state[RFM_MAX_DEVICES];
} ftm_get_tuner_info_response_type;
...
typedef PACK(struct){
	...
	uint2 ftm_cmd;
	uint16 status;
	uint16 max_num_devices;
	uint16 mfg_prd_rev_id[RFC_MAX_DEVICE_INSTANCES_PER_BAND][3];/*Column 0:mfg_id; column 1:prd_id; column 2:rev_id*/
...
typedef PACK (struct){
  ...
  uint8 secondary_device;
}ftm_tech_descriptor_type;

typedef PACK (struct){
  /*Flag to enable or disable DGLNA feature. If this flag is 
    FALSE/0, all the remainder of the fields are dont care. This
    flag is useful in disabling DGLNA feature*/
  uint8 feature_dglna_disable;
  ...
  uint8 enable_dglna;
  ftm_tech_descriptor_type tech1_2_info[MAX_TECH];
}ftm_dglna_descriptor_type;
...
typedef PACK(struct){
    ...
    uint16 rsp_len;
	uint8 status;
}ftm_dglna_descriptor_response_type;
