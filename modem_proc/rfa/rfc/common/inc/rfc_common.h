...
$Header: //components/rel/rfa.mpss/4.11/rfc/common/inc/rfc_common.h#1 $

when       who     what, where, why
--------   ---     -------------------------------------------------------------
11/10/15   fhuo    FR31436: Add rfc_lte_properties_type and rfc_lte_channel_support_type for LTE partial band
10/21/15   mah     Added RFC_MAX_SUPPORTED_NUM_ALTERNATE_PATH
10/13/15   Saul    GNSS Blanking
09/28/15   stw     FR28012 : RFC_PTX_PATH type and alternate path add for Split Config Table
09/20/15   fhuo    Add support for grfc switch for antenna sharing
08/04/15   fhuo    Added RFC_BW_INVALID in rfc_lte_bw_type and redefined rfc_lte_bw_type to rfc_bw_type;
07/14/15   Saul    Debug messages.
06/16/15   sb      Prerequisite changes for TX c1 calibration
06/12/15   zhw     Added remapper present flag.
...
03/16/15   stw     40MHz added as bandwidth type
...
/* Defining the max supported alternate paths in an RFC */
#define RFC_MAX_SUPPORTED_NUM_ALTERNATE_PATH 5

#define RFC_DEFAULT_ALTERNATE_PATH 0

/* Header for eeprom content to check integrity of data read. ASCII for RFC* */
#define RFC_EEPROM_MAGIC_NUM 0x5246432A

#define RFC_PSEUDO_ALT_PATH_START_IDX 100

/*!NOTE:This enum is for RFC internal use only. Should not be used by rfc_tech or any other software modules outside RFC.
  Choose LTE max bands as the max bands limit per tech*/
#define RFC_MAX_BANDS_PER_TECH RFCOM_NUM_LTE_BANDS
...
typedef rfc_lte_bw_type rfc_bw_type;
...
typedef struct
{
  /*!  Device to be re-mapped/translated */
  rfm_device_enum_type device;

  /*! The tech making the request. 
      Only following enums are recognized for tech queries:
      RFCOM_1X_MODE
      RFCOM_1XEVDO_MODE
      RFCOM_GSM_MODE
      RFCOM_LTE_MODE
  ...
  */
  ...
  /*! Indicates if the remapping is for an rx or tx rfc request */
  ...
  /*! Flag that indicates if remapper table is implemented for a given card. 
    Must be populated as TRUE for FTM to use remapper. Otherwise FTM controls
    will automatically find the best preferred device */
  boolean remapper_present;
} rfc_cal_device_remap_info_type;
...
boolean rfc_common_config_signal(rfc_sig_cfg_type *rf_signals, boolean gnss_blanking_enabled);
...
/* put all the WLAN GRFC/GPIO pins to their initial state, it's used for GRFC switch driven antenna sharing*/
