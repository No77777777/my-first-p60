...
$Header: //source/qcom/qct/modem/fw/components/fw_wcdma.mpss/rel/17.0/api/wfw_mod_intf.h#1 $ $DateTime: 2019/06/16 13:38:34 $ $Author: deepemm $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
04/05/16   EM      TPR Metric logging check in (CR809279)
04/24/15   FT      Added combo ETFCI workaround support
06/08/15   KT      Integrate CL 8131243 from BO to added dcTxPowerLowerBoundThres for CR 830420
...
/* !@brief number of concurrent dpch cells */
#define WFW_TX_MAX_NUM_SHO_DPCH_CELLS 6
...
typedef struct
{
  ...
  /*! @brief Scrambling code init seed for data 0 for DCH (25-bit) */
  //UWord32 dchScrCodeInitData0;

  /*! @brief Scrambling code type, 0=SHORT, 1=LONG */
  //UWord8  scrCodeType;
...
typedef struct
{
  ...
  /*! @brief Spread factor for UL DPDCH. Please see WfwTxSpreadFactorEnum. */
  //UWord8 dpdchSpreadFactor;
  /*! @brief OVSF code number for UL DPDCH */
  //UWord8 dpdchOvsfNum;

  /*! @brief PDU done cnt for R99 DPDCH */
  //UWord8 pduDoneCnt __attribute__((aligned(WFW_MOD_INTF_CACHE_LINE_SZ)));

  /*! @brief RF retune completion counter for 5MHz <-> 10MHz for 2nd carrier activation/deact.
      SW to increment for every RF retune completion and reset at DC configured/deconfigured */
  //UWord32 rfRetuneDoneCnt __attribute__((aligned(WFW_MOD_INTF_CACHE_LINE_SZ)));
...
typedef struct
{
  ...
  union {
    ...
    /*! @brief use 0x4778 for FW custom logging Requirement */
    Word16 debugLog[24];
  };
...
typedef struct
{
  /* Log filtered DP energy per frame scaled by Number of slot active */
  Word32 dpEnergy[WFW_TX_MAX_NUM_SHO_DPCH_CELLS];
  /* Log filtered NTIO per frame scaled by Number of slots active */
  Word32 ntio[WFW_TX_MAX_NUM_SHO_DPCH_CELLS];
  /* Contains the cell Id of the active DPCH CELL */
  Word8 cellIdx[WFW_TX_MAX_NUM_SHO_DPCH_CELLS];
  /* Each bit represent one active cell for dp/ntio, max 6 bits, bits 0-5 */
  Word8 cellValidBitMask;
  /* Last UL cfn logged to SW */
  UWord8  cfn;
} WfwTxTprMetricLogStruct; 
...
typedef struct
{
  ...
  Word16 reserved1;
  /*! @brief Per cell traffic power ratio metric to L1 */
  WfwTxTprMetricLogStruct tprMetric;
...
typedef struct
{
  ...
  /*! @brief Spread factor for UL HSDPCCH. Please see WfwTxSpreadFactorEnum. */
  //UWord8  hsdpcchSpreadFactor;

  /*! @brief OVSF code number for UL HSDPCCH */
  //UWord8  hsdpcchchOvsfNum;
  /*!@}*/
...
typedef struct
{
  ...
  UWord8 paRangeWr;
  /*! @brief SW programs threshold which tpc down command will be ignored, default value
      is -65dBm (-462dB10), initialized by FW, SW can disable this feature by setting value to -32768 */
  Word16 dcTxPowerLowerBoundThres;
  /*!@}*/
  ...
  /*! @brief ETFCI workaround to delay PA range switching when transitioning from high ETFCI to low ETFCI 
      paRangeExtendCurEtfci default value 120
      paRangeExtendNextEtfci default value 50
      Workaround kicks in when curEtfci >= paRangeExtendCurEtfci and nextEtfci <= paRangeExtendNextEtfci
      To disable workaround, set paRangeExtendCurEtfci to 0xff
  */
  UWord8 paRangeExtendCurEtfci;
  UWord8 paRangeExtendNextEtfci;

  /*! @brief ETFCI workaround to advance PA range switching when transitioning from low ETFCI to high ETFCI 
      paRangePreBoostCurEtfci default value 50
      paRangePreBoostNextEtfci default value 115
      Workaround kicks in when curEtfci <= paRangePreBoostCurEtfci and nextEtfci >= paRangePreBoostNextEtfci
      To disable workaround, set paRangePreBoostNextEtfci to 0xff
  */
  UWord8 paRangePreBoostCurEtfci;
  UWord8 paRangePreBoostNextEtfci;

  /*! @brief ETFCI workaround to extend PA range hysteresis period, default value 115, extend when etfci >= threshold 
      To disable workaround, set paRangeExtendHysEtfci to 0xff
  */
  UWord8 paRangeExtendHysEtfci;
  ...
  /*! @brief differenciate WTR 3k and 2k */
  boolean wtr3kFlag;
  /*! END Special NOTE */
  /* ----------------- */
...
/*! @brief Structure for logging per-carrier beta info for rate adjust calculations. */
typedef struct
{
  /* HW betaC value this slot */
  UWord16 betaCgain;
  /* HW betaC value during alt-gain portion of this slot */
  UWord16 betaCaltGain;
  /* Sum of HW beta squares this slot */
  UWord32 sumBetaSqGain;
  /* Sum of HW beta squares for alt-gain portion of this slot */
  UWord32 sumBetaSqAltGain;
} WfwTxAgcRateAdjStruct;
...
typedef struct
{
  ...
  /*! Rate Adjust per carrier logging values, per carrier, per slot in sfn. */
  WfwTxAgcRateAdjStruct rateAdjComponents[2][3];
...
typedef struct
{
  ...
  Word16 betaGainDpcchRd[WFW_NUM_MAX_UPLINK_CARRIERS][5];
  /*! @brief HW BetaD programmed */
  Word16 betaGainDpdchRd[5];
  /*! @brief HW BetaHs programmed */
  Word16 betaGainHsDpcchRd[5];
  /*! @brief HW BetaEc programmed */
  Word16 betaGainEdpcchRd[WFW_NUM_MAX_UPLINK_CARRIERS][5];
  /*! @brief HW BetaEd programmed */
  Word16 betaGainEdpdchRd[WFW_NUM_MAX_UPLINK_CARRIERS][WFW_MOD_NUM_SUPPORTED_EDPDCH][5];
