...
$Header: //source/qcom/qct/modem/fw/components/fw_wcdma.mpss/rel/17.0/api/wfw_sw_cmd_proc_intf.h#1 $
$DateTime: 2019/06/16 13:38:34 $
$Author: deepemm $

when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
02/27/15   SK      X + W Coexmgr implementation.
...
#define WFW_COEX_MGR_MAX_MULTICARRIER_SUPPORTED_RX  2
#define WFW_COEX_MGR_MAX_MULTICARRIER_SUPPORTED_TX  2

#define WFW_COEX_MGR_MAX_NUM_PRIO_TIERS 7

/*IRAT gap end time programmed by SW*/
#define WFW_X2W_GAP_END_TIME_INVALID 0xFFFFFFFF
...
/*!@}*/
  ...
  WFW_SW_INTF_BUF_SIGNAL_MPLT_TE_PROC_CMD = 8,
  WFW_SW_INTF_BUF_SIGNAL_MPLT_TE_PROC_RSP = 9,
...
/*! @brief RF command interface */
  WFW_RXAGC_CMD_2_ID =                   52,   /* !< @brief RXAGC_CMD_2 command Id */
...
#define WFW_CMD_ID_LAST                      WFW_SRCH_CYCLE_CMD_ID
#endif
...
#define WFW_COEX_UNKNOWN_FREQUENCY_ID_16BIT (0xFFF0)
...
typedef enum
{
  ...
  /*! Low priority searcher activity */
  WFW_COEX_ACTIVITY_SEARCHER_HIGH_PRIO,
  /*! WCDMA FW is active */
  WFW_COEX_ACTIVITY_WFW_APP_ACTIVE,
  /*! High priority searcher activity */
  WFW_COEX_ACTIVITY_SEARCHER_LOW_PRIO,
...
typedef enum
{
  ...
  /*! Index for interfreq CM/FMO/IDLE (W2W) for RX */
  WFW_COEX_RX_INTERFREQ_FREQ_ID_PING,
  /*! Index for interfreq CM/FMO/IDLE (W2W) for RX */
  WFW_COEX_RX_INTERFREQ_FREQ_ID_PONG,
...
#define WFW_RXAGC_CMDWORD_NUM_BITS_PER_CAR 4
...
typedef struct
{
  ...
  /*! @ brief Flag to notify FW increase the DC loop gain for a short period of duration
           0: Normal mode
           1: Increase DC loop gain for short period of durtaion */
  UWord8 dcLoopGainBurst;

  /*! @ brief Duration in BPG of the burst period of DC loop gain after which FW can
      revert back to normal gain */
  UWord8 dcloopGainBurstDur;

  /*! @ brief DC loop gain used during dcloopGainBurstDur period
      and revert back to normal gain after the period ends */
  UWord32 dcDynamicGain;
...
typedef struct
{
  ...
  /*! LTE capable indicator
      1: LTE capable SUB (LWG sub)
      0: non-LTE caapble SUB (WG sub)
      For W+W, one SUB has to be LTE capable and one SUB has to be non-LTE capable */
  UWord8 lteCapable;
  /*! @brief Sample Server format */
  //WfwSSFormatTypeEnum ssFormat;
  /*! @brief Qlink Packet format */
  //WfwPacketFormatTypeEnum packetFormat;
  /*! @Early Wake up time for starting coex registrations in us */
  /*! @WL1 needs to set it to valid value only for X->W nQTA, zero for all other scenarios */
  //UWord32 earlyWakeUpTime;
...
typedef struct
{
  ...
  /*! @brief Spreading factor for RACH MSG data channel.  Please see WfwTxSpreadFactorEnum. */
  //UWord32 rachMsgDataSf : 16;

  /* Word 6 */

  /*! @brief Scrambling code init seed for data 0 for RACH (25-bit) */
  //UWord32 rachScrCodeInitData0 : 25;
  //UWord32 reserved : 7;

  /* @brief Boolean value - set to nonzero to enable signature_array for HS-RACH non-EAI. */
  //UWord8 enSigArray;

   
  /*! @brief WL1-determined signature list for use if enSigArray nonzero, HS-RACH is active, 
             and EAI is NOT cfg'd. */
  //UWord8 signature_array[64];
...
#else
#define WFW_SRCH_CLK_STATE_LEVEL_5 364
#endif
...
typedef struct
{
  /*! @brief NQTA flag indicator */
  //UWord16 isNqtaGap: 1;
  /*!brief QDTA gap indicator*/
  UWord16 isQdtaGap: 1;                  
  ...
  /*! @brief W2L QTA gap flag indicator */
  //UWord16 isW2LGap:1;
...
typedef struct
{
  ...
  /*! Table of available priority tiers for WCDMA in decreasing order of priority */
  UWord32 priorityTierTbl[WFW_COEX_MGR_MAX_NUM_PRIO_TIERS];
...
typedef struct 
{
  ...
  UWord8  channelID; 
  UWord32 freqID;
} WfwCoexFreqChannelIDStruct;

typedef struct
{
  /* Number of Rx carriers currently configured */
  UWord8 numRxCarr;
  /* Number of Tx carriers currently configured */
  UWord8 numTxCarr;
  /*! Freq and Channel Id information for Rx (Home and Away)*/
  WfwCoexFreqChannelIDStruct  Rx[WFW_COEX_MGR_MAX_MULTICARRIER_SUPPORTED_RX][WFW_COEX_RX_FREQ_ID_MAX];
  /*! Freq and Channel Id information for Tx (Home)*/
  WfwCoexFreqChannelIDStruct  Tx[WFW_COEX_MGR_MAX_MULTICARRIER_SUPPORTED_TX][WFW_COEX_TX_FREQ_ID_MAX];      
} WfwCoexFreqChanIDTableStruct;
