...
$Header: //source/qcom/qct/modem/fw/components/fw_wcdma.mpss/rel/17.0/api/wfw_sw_intf.h#1 $
...
typedef enum
{
  ...
  WFW_SW_Q0_SEARCH_DONE_FLAG,
  WFW_SW_Q1_SEARCH_DONE_FLAG,
  ...
  /* Flag to indicate IRPM MCVS request */
  WFW_SW_IRPM_MCVS_REQ_FLAG,
  /* Flag to indicate RF Tune was skipped */
  WFW_SW_X2W_RF_TUNE_SKIP_FLAG,
  /*L2W connected mode gap end Flag*/
  WFW_SW_X2W_GAP_END_FLAG,
  /*skip flag for W2W*/
  WFW_SW_W2W_RF_TUNE_SKIP_FLAG,
  WFW_SW_NUM_FLAGS
...
typedef struct
{
   Bitmask of enabled sample servers. Bit 0: C0_ANT0, Bit 1: C1_ANT0, Bit 2: C0_ANT1, Bit3: C1_ANT1
  UWord32 ssMask;

   Pointers to IQ samples in external memory
  UWord32 ssBufPtr[5];

   Length of each sample server in W32 units
  UWord32 ssBufLenW32PerSs;

   Enable indicator for MCDMA based sample injection
  UWord32 enableSampleInjection;
} WfwPbsInjectionStruct;
*/
...
typedef struct
{
  ...
  WfwSWCmdRspStruct syncBuf[WFW_NUM_SYNC_INTERFACES];
  UWord32 swToFwCmdSentCounter[WFW_NUM_SYNC_INTERFACES];
  ...
  WfwSrchQ0CmdStruct            srchQ0CmdBuf[WFW_NUM_SYNC_INTERFACES];
  WfwSrchQ1CmdStruct            srchQ1CmdBuf[WFW_NUM_SYNC_INTERFACES];
  ...
  /* IRPM MCVS control Structure */
  WfwDemodIrpmMCVSAsyncWriteStruct demodIrpmMCVSCtrl;

  /* VLC timeline control structure */
  WfwDemodVlcAsyncWriteStruct  demodVlcCtrl;
  /* PCOV control structure */
  WfwDemodPcovAsyncWriteStruct demodPcovCtrl;
  ...
  /* Enable MCDMA based sample injection on Rumi (to bypass PBS) */
  //WfwPbsInjectionStruct pbsInj;
  /* Padding to move past current cache line */
  UWord8 padding2[32];
  ...
  WfwSrchQ0RsltStruct           srchQ0RsltBuf[WFW_NUM_SYNC_INTERFACES];
  WfwSrchQ1RsltStruct           srchQ1RsltBuf[WFW_NUM_SYNC_INTERFACES];
  WfwCmInterratRsltUnion        cmInterratRsltUnion;
  WfwSrchAsyncReadStruct        srchAsyncReadBuf[WFW_NUM_SYNC_INTERFACES];
  ...
  //WfwDembackDecobBaseAddrStruct        dbackDecobBaseAddrBuf;
  ...
  /* IRPM MCVS control Structure */
  WfwDemodIrpmMCVSAsyncReadStruct  demodIrpmMCVSData;
  ...
  UWord32 swCmdDoneFlagCounter[WFW_NUM_SYNC_INTERFACES][WFW_SW_NUM_FLAGS];
  ...
  WfwCoexMgrLogAsyncReadStruct cxmLogAsyncReadBuf[WFW_NUM_SYNC_INTERFACES];

  WfwAgcPowerLogAsyncReadStruct agcPwrLogAsyncReadBuf;

  WfwCoexMgrParamsAsyncWriteStruct cxmParamsAsyncWriteBuf[WFW_NUM_SYNC_INTERFACES];

  UWord16 swErrorRecoveryReason;

  /*! @brief Structure for miniDump - READ ONLY BY L1 */

  wfw_qsh_dump_s             qshDump;

/*
  #if defined(SIM_PLATFORM)
  UWord16 teBuild;  
  #endif 
*/
