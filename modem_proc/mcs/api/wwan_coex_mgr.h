...
$Header: //components/rel/mcs.mpss/5.2/api/wwan_coex_mgr.h#1 $

when       who     what, where, why
--------   ---     ------------------------------------------------------------
06/17/15   jm      Include IRAT link type; add link type mask to broadcast indication
05/20/15   sg      WCDMA2 priority update and RXTX split channel ID support
04/23/15   rj      Handle DS indication to provide Data+Voice info to GPS
...
typedef struct {
  ...
  /*! Macro priority SUB state from subs_prio.h e.g Voice High for active voice call,
      ...
      In case of a tech in both Voice + Data, macro state would be MAX state, i.e Voice */
  sp_subs_states_enum_t      macro_state; 

  /* Mask for Tech State (it can be in Voice + Data) */
  uint32                    macro_state_mask;
  ...
  /*! Frame type associated with UL Band of each carrier */
  cxm_tech_frame_type        ul_band_frame_type[CXM_MAX_CARRIERS];

  /*! Frame type associated with DL Band of each carrier */
  cxm_tech_frame_type        dl_band_frame_type[CXM_MAX_CARRIERS];
  
  /*! Mask of all link types registered (uplink) */
  uint32                     ul_link_type_mask;

  /*! Mask of all link types registered (downlink) */
  uint32                     dl_link_type_mask;
