...
$Header: //components/rel/mcs.mpss/5.2/api/wcn_coex_mgr.h#2 $

when       who     what, where, why
--------   ---     ------------------------------------------------------------
05/01/15   btl     Add interface support for LTE UL CA
...
#define CXM_MAX_SSID_LEN              32
#define CXM_MAX_WIFI_CHNLS_PER_CONN   8

#define CXM_MAX_BT_CHAN_MAP_SIZE      10
...
typedef enum 
{
  ...
  CXM_HIGH_PRIO_ACQ_SRCH,
  CXM_HIGH_PRIO_STOP_SVC, /*!< Sent to indicate leaving service: all other high 
                               prio events are ignored until svc starts, any 
                               ongoing high prio events are considered stopped. */
  CXM_HIGH_PRIO_START_SVC /*!< Sent to indicate service is starting: any high 
                               prio events are valid now */
...
typedef enum
{
  CXM_WLAN_DISABLED,   /*!< WLAN connection has been disabled */
  CXM_WLAN_SETUP,      /*!< WLAN is setting up the connection */
  CXM_WLAN_STEADY      /*!< WLAN connection has been established and is in a steady state */
} cxm_wlan_conn_state_e;

typedef enum
{
  CXM_WLAN_CONN_NONE,        /*!< WLAN is not in Connected mode */
  CXM_WLAN_CONN_STATION,     /*!< WLAN is in Station Connected mode */
  CXM_WLAN_CONN_SOFTAP,      /*!< WLAN is in Soft Access Point Connected mode */
  CXM_WLAN_CONN_P2P_GO,      /*!< WLAN is in Peer-to-peer Group Owner Connected mode */
  CXM_WLAN_CONN_P2P_CLIENT,  /*!< WLAN is in Peer-to-peer Client Connected mode */
  CXM_WLAN_CONN_AMP          /*!< WLAN is in Amplify Connected mode */
} cxm_wlan_conn_mode_e;

typedef enum
{
  CXM_LTE_L3_STATE_INVALID,
  CXM_LTE_L3_STATE_INACTIVE,
  CXM_LTE_L3_STATE_ACTIVE
} cxm_lte_l3_state_e;

typedef enum
{
  CXM_LTEU_L3_STATE_INVALID  = 0x00,
  CXM_LTEU_L3_STATE_ACTIVE   = 0x01,
  CXM_LTEU_L3_STATE_INACTIVE = 0x02,
  CXM_LTEU_L3_STATE_MEAS     = 0x04,
  CXM_LTEU_L3_STATE_MEAS_OFF = 0x08
} cxm_lteu_l3_state_e;

typedef enum
{
  CXM_GNSS_STATE_OFF = 0,
  CXM_GNSS_STATE_ON  = 1
}cxm_gnss_state_e;

typedef enum
{
  CXM_LTE_PAGE_STATUS_INVALID = -1,  /*!< Default/Invalid status */
  CXM_LTE_PAGE_STATUS_NONE,          /*!< No status */
  CXM_LTE_PAGE_STATUS_OK,            /*!< Page was not missed */
  CXM_LTE_PAGE_STATUS_MISSED,        /*!< Page was missed */
  CXM_LTE_PAGE_STATUS_MAX            /*!< Max number of status */
}cxm_lte_page_status_e;

/* Enumeration to indicate GNSS receiver status */
typedef enum
{
  CXM_GNSS_RX_STATE_INVALID      = -1,
  CXM_GNSS_RX_STATE_OFF          = 0,
  CXM_GNSS_RX_STATE_ON           = 1,
  CXM_GNSS_RX_STATE_MAX
}cxm_gnss_rx_state_e;
...
typedef struct
{
  ...
  cxm_lte_page_status_e  page_status;  /*!< LTE paging status*/
  int32                  prev_rssi;    /*!< RSSI calculated in the previous paging cycle */
...
typedef struct
{
  ...
  cxm_carrier_e     cc_id;           /*!< Component carrier this info applies to */
...
typedef union
{
  ...
  cxm_lte_tx_adv_ntc_info_s lte2; /*!< LTE2 specific parameters */
...
/*! @brief WLAN band info */
typedef struct
{
  uint32 freq;  /*!< Frequency in MHz */
  uint32 bw;    /*!< Bandwidth in MHz */
} cxm_wlan_band_info_s;

typedef struct
{
  uint32           rb_count;    /*!< count of RB values filtered */
  uint32           rb_filtered; /*!< filtered RB values in Q8 Format */  
  boolean          rrc_state;   /*!< TRUE if RRC procedure or RACH ongoing. For SCC, only indicates RACH */
  int16            dl_pathloss; /*!< DL path loss in dB */
  cxm_carrier_e    cc_id;       /*!< Component carrier this info applies to */
} cxm_tx_pwr_lmt_cndtns_s;
...
typedef struct
{
  msgr_hdr_struct_type                  msg_hdr;                     /*!< msgr header containing msg_id */
  cxm_tech_type                         tech_id;                     /*!< WWAN Technology */
  cxm_tx_adv_ntc_tech_specific_params_u params;                      /*!< DEPRECATED: DO NOT USE */
  uint8                                 num_cc_params;               /*!< Number of valid cc_params */
  cxm_tx_adv_ntc_tech_specific_params_u cc_params[CXM_CARRIER_MAX];  /*!< Tech specific params for tx adv notice */
}cxm_coex_tech_tx_adv_ntc_ind_s;

/*! @brief Message Sent by CXM to LTE to query Tx power limiting conditions */
typedef struct
{
  msgr_hdr_struct_type msg_hdr;                 /*!< msgr header containing msg_id */
  uint32               num_ccs;                 /*!< number of items in cc_ids array which are valid */
  cxm_carrier_e        cc_ids[CXM_CARRIER_MAX]; /*!< Component carriers this msg applies to */
}cxm_coex_tx_pwr_lmt_lte_cndtns_req_s;

/*! @brief Message used by WWAN-LTE tech as a response to CXM's Tx pwr lmt conditions check query */
typedef struct
{
  msgr_hdr_struct_type    msg_hdr;     /*!< msgr header containing msg_id */
  uint32                  rb_count;    /*!< DEPRECATED -- do not use */
  uint32                  rb_filtered; /*!< DEPRECATED -- do not use */
  boolean                 rrc_state;   /*!< DEPRECATED -- do not use */
  int16                   dl_pathloss; /*!< DEPRECATED -- do not use */
  uint32                  num_ccs;     /*!< number of items in cc_ids array which are valid */
  cxm_tx_pwr_lmt_cndtns_s cndtns[CXM_CARRIER_MAX]; /*!< array of valid CC response payloads */
...
typedef struct
{
  msgr_hdr_struct_type msg_hdr;     /*!< msgr header containing msg_id */
  boolean              rrc_state;   /*!< TRUE if RRC procedure or RACH ongoing. For SCC, only indicates RACH */
  cxm_carrier_e        cc_id;       /*!< Component carrier this msg applies to */
}cxm_coex_tx_pwr_lmt_lte_cndtns_ind_s;

/*! @brief Message used by CXM to get LTE service and LTEU configuration updates from RRC/L3
    @note */
typedef struct
{
  msgr_hdr_struct_type       msg_hdr; /*!< msgr header containing msg_id */
  cxm_lte_l3_state_e         lte_state; /*!< indication of LTE in service/out of service */
  cxm_lteu_l3_state_e        lteu_state; /*!< indication of LTEU configured or unconfigured */
}cxm_coex_lteu_rrc_info_s;
...
/*! @brief Message used by data services to report a WLAN connection for third party */
typedef struct
{
  msgr_hdr_struct_type   msg_hdr;                           /*!< msgr header containing msg_id */
  uint8                  ssid_len;                          /*!< SSID char length */
  char                   ssid[CXM_MAX_SSID_LEN];            /*!< SSID */
  cxm_wlan_conn_state_e  state;                             /*!< Connection state */
  cxm_wlan_conn_mode_e   mode;                              /*!< Connection mode */
  uint8                  band_len;                          /*!< Number of connected bands */
  cxm_wlan_band_info_s   band[CXM_MAX_WIFI_CHNLS_PER_CONN]; /*!< Connected bands */
}cxm_coex_wlan_conn_ind_s;

/*! @brief Message used by GNSS to report its state to WLAN */
typedef struct
{
  msgr_hdr_struct_type   msg_hdr;                           /*!< msgr header containing msg_id */
  cxm_gnss_state_e       state;                             /*!< GNSS demod state */
}cxm_coex_gnss_state_ind_s;

/*! @brief Message sent by CXM controller to report BT Channel map info to GNSS */
typedef struct
{
  msgr_hdr_struct_type   msg_hdr;                               /*!< msgr header containing msg_id */
  uint8                  channel_map_size;                      /*!< Number of bytes in channel map */
  uint8                  bt_chan_map[CXM_MAX_BT_CHAN_MAP_SIZE]; /*!< BT Channel map info */
}cxm_coex_bt_chan_map_ind_s;

/*! @brief Message used by GNSS to report its receiver state to MCS */
typedef struct
{
  msgr_hdr_struct_type   msg_hdr;                           /*!< msgr header containing msg_id */
  cxm_gnss_rx_state_e    state;                             /*!< GNSS RX state */
}cxm_coex_gnss_rx_state_ind_s;
/*===========================================================================
...
/*=========================================================================
  FUNCTION:  cxm_report_gnss_state

===========================================================================*/
/*!
  @brief
    Used to report the GNSS demod state
 
  @return
    boolean
*/
/*=========================================================================*/
boolean cxm_report_gnss_state(
  cxm_gnss_state_e gnss_state
);
...
enum
{
  ...
  MSGR_DEFINE_UMID( MCS, CXM, REQ, COEX_TX_PWR_LMT_LTE_CNDTNS,
                    COEX_TX_PWR_LMT_LTE_CNDTNS_ID, cxm_coex_tx_pwr_lmt_lte_cndtns_req_s),
  ...
  /* Sent to CXM to inform of WLAN's connection state */
  MSGR_DEFINE_UMID( MCS, CXM, IND, COEX_WLAN_CONN_STATE,
                    COEX_WLAN_CONN_STATE_ID, cxm_coex_wlan_conn_ind_s ),

  /* Sent to CXM to inform of LTE and LTEU states */
  MSGR_DEFINE_UMID( MCS, CXM, IND, COEX_LTEU_RRC_INFO,
                    COEX_LTEU_RRC_INFO_ID, cxm_coex_lteu_rrc_info_s ),

  /* Sent to CXM to inform of GNSS demod state */
  MSGR_DEFINE_UMID( MCS, CXM, IND, COEX_GNSS_CONN_STATE,
                    COEX_GNSS_STATE_ID, cxm_coex_gnss_state_ind_s ),

  /* Sent by CXM controller to GNSS to inform of bluetooth channel mapping */
  MSGR_DEFINE_UMID( MCS, CXM, IND, COEX_BT_CHAN_MAP,
                    COEX_BT_CHAN_MAP_ID, cxm_coex_bt_chan_map_ind_s ),

  /* Sent by GNSS to CXM with its receiver status */
  MSGR_DEFINE_UMID( MCS, CXM, IND, COEX_GNSS_RX_STATE,
                    COEX_GNSS_RX_STATE_ID, cxm_coex_gnss_rx_state_ind_s ),
