...
$Header: //components/rel/mcs.mpss/5.2/api/fws_types.h#1 $
...
typedef struct ALIGN(32)
{
  ...
  /*===== QSH dump pointer, to be cast to (cmnfw_qsh_dump_tag_0_s *) ======*/
  uint32          qsh_dump_ptr;
} fws_smem_intf_t;
...
typedef struct ALIGN(8)
{
  ...
  uint32 has_instance:1;    /*!< Is there an instance # associated with this log */
  uint32 instance:3;        /*!< Instance for this log, if has_instance == 1 */
  uint32 reserved:28;       /*!< Pad to 8-bytes. Implied by alignment, but make sure. */
