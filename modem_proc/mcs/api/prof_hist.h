...
/*! @brief Histogram invalid Event ID. */
#define PROF_HIST_INVALID_EVENT_ID        (0xFFFF)

/*! @brief Max number of histogram thresholds. */
#define PROF_HIST_TABLE_NUM_ENTRIES_MAX   (128)

/*! @brief Histogram EFS threshold cfg file name. */
#ifndef TEST_FRAMEWORK
#define PROF_HIST_THR_CFG_PATH  "/nv/item_files/mcs/prof/prof_hist_thr.cfg"
#else
...
/*! @brief Histogram value threshold configs. */
typedef struct
{
  boolean initd;  /*!< Threshold initialized. */
  int32   lower;  /*!< Threshold lower bound. */
  int32   upper;  /*!< Threshold upper bound. */
  uint16  decay;  /*!< Threshold violation decay, Q16 format. */
  uint16  limit;  /*!< Threshold violation limit. */
  uint32  count;  /*!< Threshold violation accumulation, Q16 format. */
} prof_hist_thr_t;
...
typedef struct
{
  ...
  uint8   bin_type;     /*!< Type/size of each bin (see PROF_HIST_BIN_TYPE_x) */
  boolean               regd;   /*!< Registered flag. */
  prof_hist_thr_t       thr;    /*!< Threshold values. */
...
{
  uint8               num_entries;
  prof_hist_param_t * entry[PROF_HIST_TABLE_NUM_ENTRIES_MAX];
} prof_hist_table_t;
...
#define __PROF_HIST_PARAM_DECLARE(...) MACRO_EXPAND(___PROF_HIST_PARAM_DECLARE(__VA_ARGS__))
