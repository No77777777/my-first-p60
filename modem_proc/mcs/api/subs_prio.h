...
$Header: //components/rel/mcs.mpss/5.2/api/subs_prio.h#1 $
...
09/04/2015       sg     New interface to update active tech combination
08/27/2015       jm     Deprecate multimode sub API
04/23/2015       rj     Handle DS indication to provide Data+Voice info to GPS
...
#define SP_IS_SUB_IN_DATA_CALL(state) (((state)==SP_DATA_ONLY_STATE) || ((state)==SP_DATA_HIGH_ONLY_STATE))

#define SP_MAX_STACK_PER_SUB 3

#define SP_MSK_BIT_ON(mask, bit)          ( (mask) & ( 1 << bit ) )
#define SP_SET_BIT_ON(mask, bit)          ( (mask) |= ( 1 << bit ) )
#define SP_SET_BIT_OFF(mask, bit)         ( (mask) &= ~( 1 << bit ) )
...
typedef struct 
{
  ...
  /* This is tech active per subscription indicating the RATs 
     that are going to be activated by system determination on each SUB.
     This is different from MAX capability.
     This information is not always accurate but the best guess on which
     combination of technologies are going to be active 
     MCS will use this information to program FW */
  uint32  tech_active_bitmask[MAX_NUM_OF_SUBSCRIPTIONS];
  ...
  /* Indicates the number of simul mode stacks active per sub */
  uint8                          num_active_simul_mode_stacks[MAX_NUM_OF_SUBSCRIPTIONS];  
...
typedef struct
{
  /* MAX sub States of different subscriptions */
  sp_subs_states_enum_t   state;

  /* Tech id of stack entering/exiting a call */ 
  sys_sys_mode_e_type     mode;

  /* Mask for Tech State (it can be in Voice + Data) */
  uint32                  tech_state_mask;
...
SIDE EFFECTS
  None
========================================================================*/
void sp_update_multimode_sub /* DEPRECATED */
(
...
/*-----------------------------------------------------------------------
  Input and output structures for special priority mapping mode
-------------------------------------------------------------------------*/

typedef struct 
{

    /* Start/End indication for special priority mapping mode */
    boolean  start_operation;

   /* AS ID for RATs that require special priority mapping */
    sys_modem_as_id_e_type asid;

}sp_special_mode_input_type;


typedef struct
{
    /*Output boolean flag to indicate success/failure of operation*/
    boolean special_mode_output; 

}sp_special_mode_output_type; 


/*======================================================================= 
FUNCTION SP_UPDATE_SPECIAL_PRIORITY_MAPPING 
 
DESCRIPTION 
This function enables/disables the special priority mapping mode for TRM 
clients dependning on the request received from MMCP 
...
void sp_update_special_priority_mapping
(
    /* Input request format consisting of enable/disable special access
       mode flag and as_id */
    sp_special_mode_input_type *input, 

    /* Out request format returning a success or a failure boolean value*/
    sp_special_mode_output_type *output
);   
