...
$Header: //components/rel/mcs.mpss/5.2/api/vstmr_geran.h#6 $
...
/*! @brief Nominal GERAN FCW value in Q-VSTMR_FCW_FRAC_BITS.
           (13 MHz) / 19.2e6 = (0.67708333...) * 2^VSTMR_FCW_FRAC_BITS
           = 48788995963180373.333..
           = 0xAD555555555555 (floored)
*/
#define VSTMR_GERAN_FCW_EXT            (0xAD555555555555uLL)

/*! @brief Nominal GERAN FCW inverse value in Q-VSTMR_FCW_INV_FRAC_BITS.
           1 / ((13 MHz) / 19.2e6)
           = 1/(0.67708333...) * 2^VSTMR_FCW_INV_FRAC_BITS
           ~= 106423523502170490.09230769230769
           ~= 17A17A17A17A17A (floored)
*/
#define VSTMR_GERAN_FCW_INV_EXT        (0x17A17A17A17A17AuLL)

/*! @brief Define FCW error range from the nominal in Q30 and Q-VSTMR_FCW_FRAC_BITS.
           NOTE: The new epoch framework doesn't impose restriction on the amount
                 of FCW error from the nominal. But using a synthetic error range
                 as below can allow us to check if FCW is potentially invalid. */
#define VSTMR_GERAN_FCW_ERR_RANGE      (0xFFFFF)
#define VSTMR_GERAN_FCW_EXT_ERR_RANGE  (((uint64)VSTMR_GERAN_FCW_ERR_RANGE)<<VSTMR_FCW_FRAC_BITS_EXT)

/*! @brief Min/Max FCW in Q30.
*/
#define VSTMR_GERAN_FCW_MIN            (VSTMR_GERAN_FCW - VSTMR_GERAN_FCW_ERR_RANGE)
#define VSTMR_GERAN_FCW_MAX            (VSTMR_GERAN_FCW + VSTMR_GERAN_FCW_ERR_RANGE)

/*! @brief Min/Max FCW in Q-VSTMR_FCW_FRAC_BITS.
*/
#define VSTMR_GERAN_FCW_EXT_MIN        (VSTMR_GERAN_FCW_EXT - VSTMR_GERAN_FCW_EXT_ERR_RANGE)
#define VSTMR_GERAN_FCW_EXT_MAX        (VSTMR_GERAN_FCW_EXT + VSTMR_GERAN_FCW_EXT_ERR_RANGE)
...
/*! @brief Defines RTC modulo inverse in Q64.
           Formula: floor((2^64)/VSTMR_GERAN_RTC_MODULO))
*/
#define VSTMR_GERAN_RTC_MODULO_INV     (0x1179EC9CBDuLL)
...
void vstmr_geran_update_fcw_ext
(
  vstmr_geran_view_s  *g_view,
  uint64               fcw_ext
);
