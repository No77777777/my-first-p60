...
$Header: //source/qcom/qct/modem/fw/components/fw_geran.mpss/rel/17.0/api/gfw_sw_intf.h#5 $
...
#define GFW_INTERFACE_VERSION "$Revision: #5 $\0"
...
#define MAX_NUM_PRX_DRX_IQ_SAMPLES (4 * (324))  // 2 x 162 (enough for ACQ = 156 and SYNC-RX = 162 )  PRX/DRX RF logging
...
#define  SCH_SCHEDULED_BIT   (1 << 16)
      // SCH is scheduled
#define  ENTER_MODE_LATE_CANCEL_BURST (1<<24)
      // Burst cancelled due to enter mode late      
...
typedef struct
{
  ...
  boolean bDoParallelSch;
  uint32  rfSeqNr;
  boolean bDisablePowerOpt15;
...
typedef struct
{
  ...
  union
  {
    ...
    struct
    {
      uint16 modDetPRx    :1; //!< Detected modulation scheme on PRx
      uint16 modDetDRx    :1; //!< Detected modulation scheme on DRx
      uint16 aciIdxPRx    :2; //!< ACI index for PRx
      uint16 aciIdxDRx    :2; //!< ACI index for DRx
      uint16 rxdOn        :1; //!< Flag to indicate RxD was enabled
      uint16 rxdSelStream :3; //!< Selected stream for RxD - 0:comb 1:pri 2:sec
      uint16 rxdCtrl      :3; //!< RxD Control
      uint16 strmSelEnab  :1; //!< Stream Selection enabled
      uint16 strmSelAlg   :1; //!< Stream Sel Alg - 0:RSSI 1:SNR                              
      uint16 aeq4Enabled  :1; //!< AEQ4 Enabled flag                                                                                                
    };     
  }rxdMisc;
  ...
  union
  {
    ...
    struct
    {
      uint32 timeslot :  8; //!< timeslot number for current burst                                                                
      uint32 meq44    :  1; //!< flag to indicate when MEQ44 configuration used
      uint32          : 23; //!< unused bits
    };     
  }rxdMisc1;
...
typedef struct
{
   int32  coarseFreqEst;   // Coarse Freq Offset
   int32  fineFreqEst;     //  we can have it if its not difficult 0
   int16  schStartPosQS;   // SCH start in Parallel SCh
   int32  rotatorVal;      // Rotator Value
   uint64 schTscCorrEnergy; // Chan energy
   uint16 schCirRef;        // Cirref
   uint16 schTscSnr;        //  we can have it if its not difficult 0
   uint32 schSnr;           //Par SCH Snr 
   uint32 rssi;
...
typedef struct
{
  ...
  int16 samples[MAX_NUM_PRX_DRX_IQ_SAMPLES]; //!< Buffer to store the IQ samples
...
typedef struct
{
  uint16                    len;          //!< len of packet in words
  uint16                    seqNum;       //!< seq num issued in command
  uint32                    reason[10];   //!< bit 15= True for successful schedule
                                          //!<       = FALSE otherwise,
                                          //!< failure code in bits 0-7
...
typedef struct
{
  ...
  int16                     rxLevelDrxdB;             // Expected signal level for DRx in dB
...
typedef struct
{
   ...
   uint32              rfSeqNr;
   boolean             selfTest;
   uint32              fbRxlmBufIndex; 
   fw_rf_capture_param_t  *selfTestCmdParam;
   fw_rf_capture_config_t *selfTestconfig;
   boolean             extendedTxAlpha;
} GfwGsmTxCmd;
...
typedef  struct
{
  ...
  boolean       scheduleTimeError[MDSP_MAX_STANDALONE_RSSI_MEASUREMENTS];
...
{
  GfwMdspCmdType      cmd;                   /* command code   */
  uint16              preLoadTimeoffset;      /* time offset for X2G Preload trigger (in QS) */
  uint32              rxlmBufIndex;          /*index for rxlm*/
  boolean             allowExpiredOffset;
  boolean             doRegisterStartup; /*Flag to indicate to register Startup activity. To be set only if Preload and Startup are in same frame*/
  GfwTuneAwayType     isTuneAway;            /* X2G tune-away type */
  uint32              desenseId;             /* Desense ID for X2G CXM reg    */
  uint32              actPriority;           /* CxM priority for any activity */
  int8                channel_id0;
  uint16              startupTimeoffset;    /*X2G Startup offset to register early in QTA*/
}GfwX2GPreLoadCmd;
...
//W2G CxM tune-away
  ...
  int8                channel_id0;
  boolean             readRfAlpha;			 /* flag indicating whether GFW should read alpha programming time from RFLM */
  boolean             firstBurstFlag;        /* Do not cancel X2G startup command if set to TRUE */
}GfwX2GStartupCmd;
...
typedef struct
{
  ...
  int             rfBufStartIndex;       /* start index in the IRAT scripts array */
  int             numRfEvents;           /* number of events in the array above */
  boolean         sendReply;             /* fw to send async message to GL1 after clean up if flag is true */
}GfwX2GCleanupCmd;
...
typedef struct
{
  ...
  int             rfBufStartIndex;       /* start index in the IRAT scripts array */
  int             numRfEvents;           /* number of events in the array above */
  boolean         doImmediateStartup;    /* flag to indicate to execute G2X startup trigger immediately*/
}GfwG2XStartupCmd;
...
typedef struct
{
  ...
  int             rfBufStartIndex;       /* start index in the IRAT scripts array */
  int             numRfEvents;           /* number of events in the array above */
  boolean         doImmediateCleanup;    /* flag to indicate to execute G2X cleanup trigger immediately*/
  boolean         sendReply;             /* fw to send async message to GL1 after clean up if flag is true */
}GfwG2XCleanupCmd;
...
#define  GFW_ENTER_MODE_PRX_BIT     (1 << 0)
#define  GFW_ENTER_MODE_DRX_BIT     (1 << 1)
#define  GFW_ENTER_MODE_TX_BIT      (1 << 2)

typedef struct
{
  GfwMdspCmdType cmd;
  GfwAppModeType mode;   
  uint8 enterMode;                /* ACTIVE/MEASUREMENT */
  uint32 rxlmBufIndex;  //
  uint32 rxlmDrxBufIndex;
  uint32 txlmBufIndex;
  uint8	 tx_channel_id0;					 /* CxM channel	Id*/			
  uint8  deviceID;
} GfwEnterModeCmd;
...
typedef struct
{
  ...
  uint16  parSchScheduled;   // Field to indicate if this was parallel SCh which was scheduled.
  uint16 schCombSNR;         //!< Result of SNR estimation for SCH combined acq.  
  uint16 jdetReading;
  uint32 coarse_freq_sch;   //!<  Upper 16 bits of Estimated Coarse Freq
  boolean scheduleTimeError; //!< If set to one, this means there was an error meeting the time deadlline to schedule this burst.
  uint32 rssi;
} GfwSchResultStruct;
...
*/
typedef uint8 GfwHostCmdBuff[1216] ALIGN(32); //38*CACHE_LINE_SZ [32]
...
typedef struct
{
  ...
  GfwSleepStateStruct gfwSleepState ALIGN(MAX_CACHE_LINE_SZ);
...
typedef struct
{
  ...
  GfwPllelSchLogPckt          gfwParallelSchLogBuffer;    
...
typedef struct
{
  ...
  GfwAsyncCmd                 gfwAsyncCmd ALIGN(MAX_CACHE_LINE_SZ);

  GfwDumpCmd                  gfwDumpCmd;
  ...
  boolean                     gfwHaltFirmware ALIGN(MAX_CACHE_LINE_SZ);
  uint8                       dummy[CACHE_LINE_SZ] ALIGN(MAX_CACHE_LINE_SZ);  //here to insure gfwHaltFirmware is one complete line
  //as it is cache invalidated in FW, and we do whole lines at a time.

  GfwSharedVariableType       gfwSharedVariables;
  char                        gfwInterfaceVersion[sizeof(GFW_INTERFACE_VERSION)] ALIGN(MAX_CACHE_LINE_SZ); //used to check that FW/SW interface is build with compatible versions
