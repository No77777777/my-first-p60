...
static void cmrpm_tds_reset_sim (void)
{
  ...
  CM_MSG_HIGH_1("TDS-RPM : RPM UE is in LPM. LPM done with no error, reset USIM now slot %d", slot);
...
boolean cmrpm_tds_init( void )
{
  ...
  if(cmrpm_tds_read_efs_info(&rpm_efs_info))
  {
    if(rpm_efs_info.is_rpm_enabled == TRUE && !mcfg_gcf_nv_get_status())
    {
  ...
  if(rpm_ptr->is_rpm_enabled)
  {
    switch(evt)
    {
      case CMRPM_SIM_ILLEGAL_SUBSCRIPTION:
        ...
        if(cmss_get_info_sys_mode(cmmsc_get_1x_sub(), MM_STACK_0) != SYS_SYS_MODE_TDS &&
            cmss_get_info_sys_mode(cmmsc_get_1x_sub(), MM_STACK_0) != SYS_SYS_MODE_GSM)
        {
          CM_MSG_HIGH_1("SIM event is not for TDSCDMA or GSM: current RAT %d",
                        cmss_get_info_sys_mode(cmmsc_get_1x_sub(), MM_STACK_0));
          return;
        }
