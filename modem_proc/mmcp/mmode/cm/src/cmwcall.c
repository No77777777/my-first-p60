...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/mmode/cm/src/cmwcall.c#3 $
...
static cm_srvcc_nas_call_mapping_table srvcc_ims_to_nas_call_mapping[] =
{
  ...
  /*INCOM CALL - setup processed  */
  {CM_CALL_CONV_SUBST_NONE,CM_CALL_INCOM_SUBST_SETUP_PROCESSED,CM_CALL_ORIG_SUBST_NONE,FALSE,CC_STATE_U6, HOLD_IDLE, MPTY_IDLE },
...
)
{
  ...
  */
  ...
  if ((ss_stack_ptr = cmss_get_info_stk_ptr(asubs_id, MM_STACK_0)) == NULL)
  {
    return;
  }

  if(cmss_get_true_sys_mode(call_ptr->mm_id) != SYS_SYS_MODE_NO_SRV)
  {
    call_ptr->sys_mode = cmss_get_true_sys_mode(call_ptr->mm_id);
  }
  ...
  */
  if (!cmcall_set_favored_mode_pref_for_origination ( call_ptr->mm_id,
      call_ptr->call_type,
      call_ptr->srv_type,
      TRUE, /* drs_bit always
            ** set to TRUE.
...
)
{
  ...
  const gsm_wcdma_cmcall_ps_type *ps_orig_call_info_ptr;
  /* Pointer to the PS data call command information */
  cm_mm_ss_info_sub_prop_s_type *cmss_sub_ptr
    = cmss_get_info_sub_prop(ps_call_ptr->mm_id.asubs_id);
  cm_mm_ss_info_gen_stack_info_s_type *cmss_common_stk0_info = NULL;
  ps_orig_call_info_ptr = &ps_call_ptr->cmcall_mode_info.info.gsm_wcdma_ps_info;

  if (cmss_sub_ptr == NULL)
  {
    return CM_CALL_CMD_ERR_INVALID_SIM_STATE_S;
  }

  cmss_common_stk0_info = cmss_sub_ptr->stack_info[MM_STACK_0].gen_info;

  if (cmss_common_stk0_info == NULL)
  {
    return CM_CALL_CMD_ERR_INVALID_SIM_STATE_S;
  }

  /* Check if the SIM allows packet data calls */
  if (cmss_common_stk0_info->mode_info.gw_info.sim_state == SYS_SIM_STATE_PS_INVALID ||
      cmss_common_stk0_info->mode_info.gw_info.sim_state == SYS_SIM_STATE_CS_PS_INVALID )
  {
    return CM_CALL_CMD_ERR_INVALID_SIM_STATE_S;
  }
  ...
  if (is_orig_call_primary_pdp) /* Call being originated is primary */
  {
    while (call_ptr != NULL)
    {
      ...
      if( ( call_ptr->call_state == CM_CALL_STATE_IDLE )                ||
          ...
          (!call_ptr->cmcall_mode_info.info.gsm_wcdma_ps_info.primary_pdp) ||
          (call_ptr->mm_id.asubs_id != ps_call_ptr->mm_id.asubs_id)
        )
      {
        call_ptr = cmcall_obj_get_next(&call_itr);
        continue;
    ...
    while (call_ptr != NULL)
    {
      ...
      */
      ...
      if (cmwcall_ps_sec_group_match (call_ptr, ps_call_ptr))
      {
        ...
        {
          /* Send reject to NAS */
          cmwcall_send_ps_rej_rsp(ps_call_ptr->call_id, SM_INSUFFICIENT_RESOURCES);
          ps_call_ptr->end_status = CM_CALL_END_MAX_PS_CALLS;
          cmwcall_end_req(ps_call_ptr);
...
)
{
  ...
  stack_id = cmph_determine_ss_per_domain(call_cmd_ptr->cmd_subs, SYS_SRV_DOMAIN_CS_ONLY);
  ...
  srv_status = cmss_get_info_srv_status(asubs_id, stack_id);
  ...
  */
  if((srv_status != SYS_SRV_STATUS_SRV) && (srv_status != SYS_SRV_STATUS_LIMITED))
  {
  ...
  */
  if((ph_ptr->device_prop.als_pref.line_switching == CM_ALS_LINE_SWITCHING_NOT_ALLOWED) &&
      (call_ptr->call_type    == CM_CALL_TYPE_VOICE) &&
      (call_ptr->line != ph_ptr->device_prop.als_pref.line))
  {
...
)
{
  ...
  switch( recall_rsp )
  {
    case CM_RECALL_RSP_CCBS_ACCEPT:
      return CCBS_ACCEPT;

    case CM_RECALL_RSP_CCBS_REJECT:
      return CCBS_REJECT;

    case CM_RECALL_RSP_CCBS_HOLD_ACCEPT:
      return CCBS_HOLD_ACCEPT;

    case CM_RECALL_RSP_CCBS_REL_ACCEPT:
      return CCBS_REL_ACCEPT;
...
)
{
  ...
  {
    case CCBS_RET_RES:
      return CM_CCBS_CNF_RET_RES;

    case CCBS_RET_ERR:
      return CM_CCBS_CNF_RET_ERR;

    case CCBS_REJ:
      return CM_CCBS_CNF_REJ;
...
static boolean cmwcall_check_if_mo_call_possible( const cmcall_s_type *call_ptr )
{
  ...
  */
  ...
  while(temp_call_ptr != NULL)
  {
       ...
       )
    {
      if (cmph_is_msim())
      {
        if (temp_call_ptr->mm_id.asubs_id == call_ptr->mm_id.asubs_id ||
            cmph_is_dsds()
  ...
  */
  ...
  if(cmcall_msim_is_cross_sub_concurrency_forbidden(call_ptr, FALSE))
  #else
  if(!cmcall_is_cross_sub_concurrency_allowed(call_ptr->call_type, call_ptr->mm_id.asubs_id, FALSE))
  #endif
  {
    orig_call = FALSE;
  }
...
static boolean cmwcall_check_if_mt_call_possible( const cmcall_s_type *call_ptr )
{
  ...
  cm_iterator_type        call_itr;
  multimode_stack_e_type  stack_id = call_ptr->mm_id.stk_id;

  /* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */

  if(!cm_get_ui_ready_status())
  {
    CM_MSG_HIGH_0("Rejecting MT page since UI not ready");
    return FALSE;
  }
  ...
  */
  ...
  {
    if (cmss_get_info_sys_mode(call_ptr->mm_id.asubs_id, stack_id) == SYS_SYS_MODE_GSM &&
        (cmcall_is_there_a_call_type_no_call_id_per_sub(call_ptr->mm_id.asubs_id,CM_CALL_TYPE_VOICE,
  ...
  */
  if(cmph_get_volte_sub_info_ptr(call_ptr->mm_id.asubs_id)->ue_based_cw.use_ue_based_cw &&
      ...
      ((cmcall_is_there_a_call_type_no_call_id_per_sub(call_ptr->mm_id.asubs_id, CM_CALL_TYPE_VOICE,call_ptr->call_id) != CM_CALL_ID_INVALID)||
       (cmcall_is_there_a_call_type_no_call_id_per_sub(call_ptr->mm_id.asubs_id, CM_CALL_TYPE_VT, CM_CALL_ID_INVALID) != CM_CALL_ID_INVALID))
    )
  {
    accept_call = FALSE;
  ...
  while(temp_call_ptr != NULL)
  {
    ...
    */
    ...
    {
      /* Reject incoming call based on call activity on the same sub.
      ...
      */
      if(cmcall_check_voice_emerg_in_orig(temp_call_ptr) 
         #ifdef FEATURE_HA_MAINLINE_ONLY_ENABLE
         && (temp_call_ptr->mm_id.asubs_id == call_ptr->mm_id.asubs_id)
         #endif
         )
      {
      ...
      #ifdef FEATURE_HA_MAINLINE_ONLY_ENABLE
      else if(temp_call_ptr->is_vowlan == TRUE &&
              temp_call_ptr->mm_id.asubs_id == call_ptr->mm_id.asubs_id)
      #else
  ...
  /* Reject incoming call based on call activity on the other sub
  */
  #ifdef FEATURE_HA_MAINLINE_ONLY_ENABLE
  if(cmcall_msim_is_cross_sub_concurrency_forbidden(call_ptr, FALSE))
  #else
...
FUNCTION cmwcall_check_mt_anonymous_due_to_ue_based_barr

DESCRIPTION
  Check if a MT anonymous voice call is allowed if ue_based call barring is enabled
...
===========================================================================*/
static boolean cmwcall_check_mt_anonymous_due_to_ue_based_barr( cmcall_s_type *call_ptr,const cm_hdr_type   *rpt_ptr )
{
   cm_rpt_type              *cm_rpt_ptr = (cm_rpt_type *) rpt_ptr;

   if(cmph_get_volte_sub_info_ptr(call_ptr->mm_id.asubs_id)->ue_based_sups[bain_idx].use_ue_based_sups == TRUE && //bain
          cmph_get_volte_sub_info_ptr(call_ptr->mm_id.asubs_id)->ue_based_sups[bain_idx].is_enabled == TRUE &&
          call_ptr->num.pi)/*if pi == 0 , number is not anonymous, any non- zero value of pi indicates number to be anonymous*/
  {
    ...
    */
    ...
    cause.cause_value            = INCOMING_CALL_BARRED_WITHIN_CUG;
    ...
    **/
    ...
    CM_MSG_HIGH_0("UE based Call barring enabled for anonymous call, reject MT voice call");
...
FUNCTION cmwcall_check_mt_call_due_to_ue_based_barr

DESCRIPTION
  Check if a MT voice call is allowed if ue_based call barring is enabled
...
===========================================================================*/
static boolean cmwcall_check_mt_call_due_to_ue_based_barr( cmcall_s_type *call_ptr,const cm_hdr_type   *rpt_ptr )
{
cm_rpt_type              *cm_rpt_ptr = (cm_rpt_type *) rpt_ptr;
  sys_mcc_type  current_mcc = SYS_WILDCARD_MCC;
  sys_mcc_type  home_mcc = SYS_WILDCARD_MCC;
  cmph_s_type              *ph_ptr              = cmph_ptr();
  cm_mm_ss_info_gen_stack_info_s_type *ss_stack_ptr = cmss_get_info_stk_ptr(call_ptr->mm_id.asubs_id, MM_STACK_0);
  cm_mm_ss_info_sub_prop_s_type *ss_sub_ptr= cmss_get_info_sub_prop(SYS_MODEM_AS_ID_1); 
  if(ss_sub_ptr)
  {
    ss_stack_ptr = ss_sub_ptr->stack_info[MM_STACK_0].gen_info;
  }
  sys_plmn_get_mcc(ph_ptr->sub_info[call_ptr->mm_id.asubs_id]->gwl_imsi_plmn, &home_mcc);
  if(ss_stack_ptr)
  {
    sys_plmn_get_mcc(ss_stack_ptr->sys_id.id.plmn, &current_mcc);
  }
  /*** if the CB is set to be UE based and it s enabled */
  if((cmph_get_volte_sub_info_ptr(call_ptr->mm_id.asubs_id)->ue_based_sups[baic_idx].use_ue_based_sups == TRUE &&   //barringOfIncomingCalls
      cmph_get_volte_sub_info_ptr(call_ptr->mm_id.asubs_id)->ue_based_sups[baic_idx].is_enabled == TRUE)||
     (cmph_get_volte_sub_info_ptr(call_ptr->mm_id.asubs_id)->ue_based_sups[bicRoam_idx].use_ue_based_sups == TRUE &&   //bicRoam
      cmph_get_volte_sub_info_ptr(call_ptr->mm_id.asubs_id)->ue_based_sups[bicRoam_idx].is_enabled == TRUE &&
    ...
    **/
    call_ptr->send_call_event_end = FALSE;
    CM_MSG_HIGH_0("UE based Call barring enabled, reject MT voice call");
    cm_call_id_deallocate ( cmcall_call_id_get( call_ptr ) );
    return FALSE;
...
FUNCTION cmwcall_set_sys_mode_for_HO_complete

DESCRIPTION
  Set sys_mode of target RAT to the call_ptr during HO complete event.
DEPENDENCIES
  Call object must have already been initialized with
  cmcall_init().
...
SIDE EFFECTS
  none
===========================================================================*/
void cmwcall_set_sys_mode_for_HO_complete(

  sys_ho_type_e_type sys_ho_type,
...
{ 
 ...
 switch(sys_ho_type)
  {
  	case SYS_HO_TYPE_GSM_UMTS:
		*sys_mode = SYS_SYS_MODE_WCDMA;
		break;
	case SYS_HO_TYPE_GSM_GSM:
		*sys_mode = SYS_SYS_MODE_GSM;
		break;
	case SYS_HO_TYPE_UMTS_UMTS:
		*sys_mode = SYS_SYS_MODE_WCDMA;
		break;
    case SYS_HO_TYPE_UMTS_GSM:
		*sys_mode = SYS_SYS_MODE_GSM;
		break;
	case SYS_HO_TYPE_LTE_GSM_SRVCC:
		*sys_mode = SYS_SYS_MODE_GSM;
		break;
	case SYS_HO_TYPE_LTE_UMTS_SRVCC:
		*sys_mode = SYS_SYS_MODE_WCDMA;
		break;
    default:
		break;
  }
    CM_MSG_HIGH_1("sys_mode = %d",*sys_mode);
} /* cmwcall_set_sys_mode_for_HO_complete () */
...
/*lint -esym(715,call_ptr) */
{
  ...
  if( call_ptr != NULL )
  {
    sm_ptr->cmd.pdp_activate_rej_resp.as_id = call_ptr->mm_id.asubs_id;
...
===========================================================================*/
static boolean cmwcall_is_ps_data_allowed (
  sys_modem_as_id_e_type  asubs_id
)
{
  ...
  */
  if ( (ss_info_sub_prop_ptr != NULL &&
        ...
        (ss_info_sub_prop_ptr->stack_info[MM_STACK_0].gen_info->sys_mode == SYS_SYS_MODE_GSM) &&
#if defined(FEATURE_GSM_DTM)
        (ss_info_sub_prop_ptr->stack_info[MM_STACK_0].gpp_info->cell_srv_ind.dtm_supp != SYS_DTM_SUPPORT_AVAIL) &&
#endif
        cmcall_check_if_gw_cs_call_active (CM_CALL_STATE_NONE, cmph_map_sub_stk_to_id(asubs_id, MM_STACK_0))) ||
       (cmemg_get_reason_per_sub(asubs_id) == CM_AC_REASON_THIRD_PARTY_IMS_E911)
     )
  {
    return FALSE;
  }
...
)
{
  ...
  CM_MSG_HIGH_6("qos valid = %d apn valid %d apn len %d llc sapi valid = %d, sapi = %d, req_type=%d",
                pdp_info_ptr->qos.valid, pdp_info_ptr->apn_name.valid,
                pdp_info_ptr->apn_name.apn_addr_len,
                pdp_info_ptr->llc_sapi.valid, pdp_info_ptr->llc_sapi.sapi, pdp_info_ptr->request_type.req_type);

  CM_MSG_HIGH_4(" trans id = %d pdp addr present = %d, len = %d pdp as_id %d",
...
)
{
  ...
  CM_MSG_HIGH_6("qos valid = %d apn name valid %d apn name len %d llc sapi valid = %d, sapi = %d, req_type=%d",
                pdp_info_ptr->qos.valid, pdp_info_ptr->apn_name.valid, pdp_info_ptr->apn_name.apn_addr_len,
                pdp_info_ptr->llc_sapi.valid, pdp_info_ptr->llc_sapi.sapi, pdp_info_ptr->request_type.req_type);

  CM_MSG_HIGH_3 (" trans id = %d  pdp addr present = %d, len = %d ", pdp_info_ptr->trans_id, pdp_info_ptr->pdp_addr.valid,
                 pdp_info_ptr->pdp_addr.pdp_addr_len);
...
)
{
  ...
  */
  ...
  if (ps_wcall_info_p->primary_pdp)
  {
    ...
    sm_orig_ptr->as_id          = call_ptr->mm_id.asubs_id;
  ...
  else
  {
    ...
    sm_sec_orig_ptr->as_id          = call_ptr->mm_id.asubs_id;
...
)
{
  ...
  cm_call_type_e_type local_call_type = call_ptr->call_type;
  boolean ignore_srv_status_for_nal_call = FALSE;
  cmregprx_stack_info_s_type *cmregprx_stack_info_ptr = cmregprx_get_stk_ptr(call_ptr->mm_id);
  
  #ifdef FEATURE_UUS
  cm_user_data_T     *mn_user_data_p = NULL;
#endif
  ...
  if(ss_true_stk_info_ptr == NULL || cmregprx_stack_info_ptr == NULL)
  {
 ...
 cmcall_update_speech_codec_info(call_ptr->mm_id.asubs_id);
 
   /* Special Emergency Number Processing */
  if(call_ptr->special_emergency)
  {
     CM_MSG_HIGH_4("spl emergency:Current Service status : %d , Current Roam Status : %d, stable svc : %d, net_sel_mode %d",
                       ss_true_stk_info_ptr->srv_status, 
                       ss_true_stk_info_ptr->roam,
                       ss_true_stk_info_ptr->is_stable_in_svc,
                       cmregprx_stack_info_ptr->last_srv_req.network_selection_mode);
					   
	 /*For NAL Calls, when we will give OK_TO_ORIG_G at CAMPED_IND
	  service status shall be limited and NAL Call shall be placed as
	  Emergency Call, so we need to ignore service status check to allow
	  NAL Call getting placed as Normal Call*/	
	  
	 ignore_srv_status_for_nal_call = (ss_true_stk_info_ptr->is_stable_in_svc == FALSE &&
	                                   cmregprx_stack_info_ptr->last_srv_req.network_selection_mode == SYS_NETWORK_SELECTION_MODE_AUTOMATIC );
									   
     if((ss_true_stk_info_ptr->srv_status==SYS_SRV_STATUS_SRV &&
       ss_true_stk_info_ptr->roam==SYS_ROAM_STATUS_OFF)||
       ignore_srv_status_for_nal_call)
	 {
		local_call_type = CM_CALL_TYPE_VOICE;
     }
  }
  ...
  {
    /*
      ** CM will to check the emerg_srv_category if it has been changed since the call_control
      */
    cmcc_check_and_update_emerg_srv_category(call_ptr, NULL);

  }


  switch( local_call_type )
  {
    ...
    */
    case CM_CALL_TYPE_EMERGENCY:
      ...
      mn_e911_orig_p->as_id = call_ptr->mm_id.asubs_id;
      ...
      ** and FALSE otherwise.
      */
      if(policyman_get_subs_boolean_with_default("cm:e911_urn_subcategory",
          mn_e911_orig_p->as_id, FALSE))
      {
        CM_MSG_HIGH_2("ORIG: CM->NAS: sub %d, setting emerg_srv_categ to none as per uscc requirement: %d->0",
                      mn_e911_orig_p->as_id,
                      call_ptr->num.emerg_srv_categ);
        mn_e911_orig_p->service_category.present = FALSE;
        mn_e911_orig_p->service_category.emergency_category = CM_EMERG_NUM_TYPE_NONE;
      }
      else
      {
#endif
...
CM_MSG_HIGH_1("ORIG: CM->NAS: E911: Emerg srv category being sent = %d",
                     mn_e911_orig_p->service_category.emergency_category);


      cmcall_set_lte_plmn_before_csfb_no_vops(call_ptr);

      /*Set this flag to true to indicate CS domain has been attempted for the current 
        emergency call.
      */
      call_ptr->is_emerg_cs_attempted = TRUE;
    ...
    default:
      ...
      {
        call_ptr->call_type = CM_CALL_TYPE_VOICE;
        CM_MSG_HIGH_2 ("ORIG: CM->NAS: Converting VT call to voice, call_id %d, sub %d", 
                          call_ptr->call_id,
                          call_ptr->mm_id.asubs_id);
      }
      ...
      */
      if (wcall_info_p->is_ccbs_recall_resp)
      {
        ...
        */
        ...
        mn_recall_p->as_id  = call_ptr->mm_id.asubs_id;
      ...
      /* Set the subscription-id of the call */
      mn_orig_p->as_id  = call_ptr->mm_id.asubs_id;
      ...
      */
      /* Depricating the CM_MO_USER_DATA_REQ API as per NAS
                Need to embed this info in CM_MO_NORMAL_CALL_REQ.
           */
      if (wcall_info_p->mo_mt_user_data.present)
      {
        ...
        mn_user_data_p = &(mn_orig_p->mo_mt_user_data);

        /* indicates user data is present */
        mn_user_data_p->present = wcall_info_p->mo_mt_user_data.present;
        ...
        mn_user_data_p->user_user_data_length = memscpy ( mn_user_data_p->user_user_data,
                                                sizeof(mn_user_data_p->user_user_data),
      ...
      */
      ...
      {
        (void)cmals_ptr_mmgsdi()->resolve_mo_call_req_func( cmph_ptr()->device_prop.als_pref.line,
            mn_ptr );
  ...
  call_ptr->sr.orig_sent_on_mode = cmcall_get_sd_ok_to_orig_mode();
  call_ptr->is_gw_cs_orig_ever_sent = TRUE;

 CM_MSG_HIGH_4("ORIG: CM->NAS: cmwcall_send_orig: orig_sent_on_mode %d for call_id %d, call_type %d, sub %d",
...
)
{
  ...
  CM_MSG_HIGH_0_EXT( "cmwcall_end_req()", (call_ptr->mm_id.asubs_id+1));
  CM_ASSERT( call_ptr != NULL );
...
FUNCTION cmwcall_populate_mt_setup_response

DESCRIPTION
  This function is central API to populate MT SETUP RES command.
...
===========================================================================*/
void cmwcall_populate_mt_setup_response
(
...
)
{
  ...
  cm_mt_setup_res->cm_accept = is_accept;

  cm_mt_setup_res->as_id     = mm_id.asubs_id;
  ...
  */
  if ( is_accept )
  {
    ...
    if( cmcall_check_if_gw_cs_call_active(CM_CALL_STATE_CONV, mm_id) )
      /* user busy = TRUE irrespective if CS voice calls active */
    {
      cm_mt_setup_res->cm_cc_cause.present = TRUE;
      cm_mt_setup_res->cm_cc_cause.coding_standard = STD_CODESTANDARD;
      cm_mt_setup_res->cm_cc_cause.location = STD_LOCATION;
      cm_mt_setup_res->cm_cc_cause.recommendation_present = FALSE;
      cm_mt_setup_res->cm_cc_cause.diagnostic_length = 0;
      cm_mt_setup_res->cm_cc_cause.cause_value = USER_BUSY;
    }
    else
    {
      cm_mt_setup_res->cm_cc_cause.present = FALSE;
    }
  }
  else
  {
    /* otherwise there are other cause reasons */
    cm_mt_setup_res->cm_cc_cause = *cc_cause_ptr;
  }

  if(bc_repeat_indicator_ptr != NULL)
    cm_mt_setup_res->bc_repeat_indicator = *bc_repeat_indicator_ptr;

  if(bearer_capability_1_ptr != NULL)
    cm_mt_setup_res->bearer_capability_1 = *bearer_capability_1_ptr;

  if(bearer_capability_2_ptr != NULL)
    cm_mt_setup_res->bearer_capability_2 = *bearer_capability_2_ptr;
...
)
{
  ...
  /* populate mt setup res with received bearer capability parameters and 
  ** codec list 
  */
  cmwcall_populate_mt_setup_response(call_ptr->mm_id,
  	                                 is_accept,
  	                                 cc_cause_ptr,
  	                                 bc_repeat_indicator_ptr,
  	                                 bearer_capability_1_ptr,
  	                                 bearer_capability_2_ptr,
  	                                 &mn_ptr->cmd.cm_mt_setup_res);
  ...
  switch( call_cmd_ptr->cmd )
  {
    ...
    case CM_CALL_CMD_ANSWER:
      ...
      else
      {
        ...
        mn_ptr->cmd.cm_mt_call_res.as_id         = call_ptr->mm_id.asubs_id;
        ...
        switch ( call_ptr->call_type )
        {
          case CM_CALL_TYPE_PS_DATA:
            ...
            */
            ...
            {
              other_sub = (call_ptr->mm_id.asubs_id == SYS_MODEM_AS_ID_1)?SYS_MODEM_AS_ID_2:
			                                                    SYS_MODEM_AS_ID_1;
              ...
              if(cmph_is_msim() && cmsds_is_there_ext_volte_or_cellular_call_per_sub(other_sub))
              {
                sm_ptr->cmd.pdp_deactivate_req.cause         = SM_LOCAL_PDP_DEACTIVATION;
              }
              else
              {
                CM_MSG_HIGH_0("Voice call active on other sub, changing deact cause to local");
                /* if there is voice call on other sub, always make this local deact*/
              ...
              sm_ptr->cmd.pdp_deactivate_req.as_id  = call_ptr->mm_id.asubs_id;
          ...
          case CM_CALL_TYPE_EMERGENCY:
            ...
            mn_ptr->cmd.cm_n_x_send_req.connection_id   = connection_id;
            mn_ptr->cmd.cm_n_x_send_req.cause_value     = NORMAL_CALL_CLEARING;
            mn_ptr->cmd.cm_n_x_send_req.as_id           = call_ptr->mm_id.asubs_id;
        ...
        for ( i = 0; i < call_cmd_ptr->info.num_end_call_ids; i++ )
        {
          ...
          */
          ...
          switch ( call_ptr->call_type )
          {
            ...
            case CM_CALL_TYPE_CS_DATA:
            case CM_CALL_TYPE_EMERGENCY:
              cmwcall_send_cs_end_req (connection_id, call_ptr->mm_id.asubs_id);
              break;
    ...
    case CM_CALL_CMD_PDP_MODIFY:
      ...
      */
      ...
      sm_ptr->cmd.pdp_modify_req.as_id      = call_ptr->mm_id.asubs_id;
    ...
    case CM_CALL_CMD_PDP_NW_MODIFY_RSP:
      ...
      if( ps_cmd_info_ptr->accept )
      {
        ...
        sm_ptr->cmd.pdp_nw_modify_cnf.as_id  =  call_ptr->mm_id.asubs_id;
      }
      else
      {
        sm_ptr = cm_sm_get_buf_else_err_fatal(SMREG_PDP_NW_MODIFY_REJ);
        sm_ptr->cmd.pdp_nw_modify_rej.cause  =  ps_cmd_info_ptr->cause;
        sm_ptr->cmd.pdp_nw_modify_rej.as_id  =  call_ptr->mm_id.asubs_id;
      }
    ...
    case CM_CALL_CMD_RAB_REESTAB:
      ...
      */
      ...
      if (cmph_is_msim())
      {
        ...
        {
          cm_mode_pref_e_type ph_ss_mode_pref = cmph_get_ss_mode_pref(call_ptr->mm_id.asubs_id);
          ...
          */
          ...
          fav_mode_pref = cmutil_misc_intersect_mode_pref( srv_type,
                          ph_ss_mode_pref );
          ...
          */
          ...
          {
            (void)cmtask_orig_para_change_mode_pref(call_ptr->mm_id,
        ...
        */
        is_top_changed = cmtask_orig_para_change_act_priority(call_ptr->mm_id,
                         CM_ACT_TYPE_DATA_CALL,
        ...
        /* The pointer to the top element of the priority queue */
        ph_orig_para_top_ptr = cmtask_orig_para_get_top( call_ptr->mm_id );
        ...
        if ( ph_orig_para_top_ptr != NULL && is_top_changed )
        {
          ...
          cmmsc_pack_cmd_for_pref_sys_chgd_per_top_queue(call_ptr->mm_id,
              ...
              cmph_map_cm_call_type_to_sd_orig_mode( CM_CALL_TYPE_PS_DATA,
                  call_ptr->mm_id.asubs_id),
              FALSE,
    ...
    case CM_CALL_CMD_RECALL_RSP:
      ...
      */
      ...
      mn_ptr->cmd.cm_recall_rsp.as_id = call_ptr->mm_id.asubs_id;
...
#error code not present
    ...
    case CM_CALL_CMD_DATA_ACTIVITY:
      sm_ptr = cm_sm_get_buf_else_err_fatal(SMREG_DATA_CALL_ACTIVITY_IND);

      sm_ptr->cmd.data_call_activity.activity = cmd_info_ptr->data_activity_info.data_activity;
      sm_ptr->cmd.data_call_activity.as_id= cmd_info_ptr->data_activity_info.asubs_id;
...
)
{
  ...
  */
  ...
  CM_MSG_HIGH_2( "CALL_END: CM->NAS: cmwcall_end(), call_id %d sub %d",
                 call_ptr->call_id, mm_id.asubs_id);
  ...
  */
  ...
  if (cmwcall_emergency_call_no_sim)
  {
    ...
    cmph_notify_mmoc_no_sim(mm_id.asubs_id);
  }
  ...
  */
  ...
  /* Update the priority queue's for this activity end
  */
  {
  ...
  if( cmcall_is_no_call_in_gw_cs_per_subs(mm_id.asubs_id) &&
      cmcall_is_no_call_in_gw_ps_per_subs(mm_id.asubs_id)
#ifdef FEATURE_CM_LTE
  ...
  */
  if (call_mode_being_ended == CM_CALL_MODE_INFO_GW_CS &&
      cmcall_is_no_call_in_gw_cs_per_subs(mm_id.asubs_id)
     )
  {
    cmss_rel_cipher_ind (SYS_SRV_DOMAIN_CS_ONLY, mm_id.asubs_id);
  }
  else if (call_mode_being_ended == CM_CALL_MODE_INFO_GW_PS)
  {
    if (cmcall_is_no_call_in_gw_ps_per_subs(mm_id.asubs_id)
#ifdef FEATURE_CM_LTE
        && cmcall_is_no_call_in_lte_ps()
#endif
       )
    {
      cmss_rel_cipher_ind (SYS_SRV_DOMAIN_PS_ONLY, mm_id.asubs_id);
    }
  }
}
/*===========================================================================
...
#error code not present
    ...
    case CM_CALL_CMD_ANSWER:
      ...
      */
      if( (ph_ptr->device_prop.ph_oprt_mode.oprt_mode != SYS_OPRT_MODE_ONLINE) &&
          (ph_ptr->device_prop.ph_oprt_mode.oprt_mode != SYS_OPRT_MODE_NET_TEST_GW))
      {
        cmd_err = CM_CALL_CMD_ERR_OFFLINE_S;
        break;
      }
    ...
    case CM_CALL_CMD_PS_DATA_AVAILABLE:
      ...
      else if(cmss_get_info_srv_domain(call_cmd_ptr->cmd_subs, MM_STACK_0)
              != SYS_SRV_DOMAIN_PS_ONLY &&
              cmss_get_info_srv_domain(call_cmd_ptr->cmd_subs, MM_STACK_0)
      ...
      else if (cmss_get_info_sys_mode(call_cmd_ptr->cmd_subs, MM_STACK_0) == SYS_SYS_MODE_CDMA &&
               call_cmd_ptr->cmd_subs == cmmsc_get_1x_sub() &&
               cmcall_is_there_a_call_type_no_call_id_per_sub(call_cmd_ptr->cmd_subs,
    ...
    case CM_CALL_CMD_PS_SIG_REL:
    {
      ...
      else if(cmss_get_info_srv_domain(call_cmd_ptr->cmd_subs, MM_STACK_0) != SYS_SRV_DOMAIN_PS_ONLY &&
              cmss_get_info_srv_domain(call_cmd_ptr->cmd_subs, MM_STACK_0) != SYS_SRV_DOMAIN_CS_PS
             )
      {
...
#error code not present
#else
      cmd_err = CM_CALL_CMD_ERR_FUNCTION_UNAVAILABLE_DUE_TO_UNDEFINED_FEATURE;
#endif
...
FUNCTION  cmwcall_is_valid_asubs_id_from_ll
...
===========================================================================*/
static boolean cmwcall_is_valid_asubs_id_from_ll (
...
/*lint -esym(818, cm_rpt_ptr) */
{
       ...
       ( cm_notify_ss->cm_ss_code.ss_code == (byte)ccbs )  )
  {
    ...
    if (ccbs_indicator->ccbsIndex.present)
    {
      ...
      */
      call_ptr->mm_id.stk_id = cmph_determine_ss_per_domain(cm_rpt_ptr->cmd.call_ss_ind.as_id,
                               SYS_SRV_DOMAIN_CS_ONLY);
      if (!cmwcall_is_valid_asubs_id_from_ll(cm_rpt_ptr->cmd.call_ss_ind.as_id))
      {
        CM_ERR_1(" Invalid subs id %d",cm_rpt_ptr->cmd.call_ss_ind.as_id);
        return TRUE;
      }
      call_ptr->mm_id.asubs_id = cmph_get_as_id_from_ll_ind(cm_rpt_ptr->cmd.call_ss_ind.as_id);
      ...
      */
      if (!cmph_remove_ccbs_entry (call_ptr->mm_id.asubs_id,
                                   ccbs_indicator->ccbsIndex.ccbs_Index))
      {
  ...
  */
  ...
  switch( call_ptr->cmcall_mode_info.info_type )
  {
    case CM_CALL_MODE_INFO_GW_PS:
      ...
      ps_info_mode_ptr->backoff_timer.valid = ps_wcall_mode_ptr->backoff_timer.valid;

      ps_info_mode_ptr->reattempt_indicator.valid = ps_wcall_mode_ptr->reattempt_indicator.valid;
      ps_info_mode_ptr->reattempt_indicator.ratc_val
        = ps_wcall_mode_ptr->reattempt_indicator.ratc_val;
...
)
{
  ...
  sys_srv_status_e_type srv_status_stk0;
  ...
  stk = (call_ptr->mm_id.stk_id == MM_STACK_ALL?MM_STACK_0:call_ptr->mm_id.stk_id);
  ph_sub_ptr = cmph_get_sub_ptr(call_ptr->mm_id.asubs_id);

  if(!(call_ptr->mm_id.asubs_id > SYS_MODEM_AS_ID_NONE &&
       call_ptr->mm_id.asubs_id < MAX_SIMS) ||
      !(stk >= MM_STACK_0 && stk < MM_STACK_ALL)
      || ss_info_sub_prop_ptr == NULL || ph_sub_ptr == NULL)
  {
    return CM_CALL_CMD_ERR_OP_NOT_SUPPORTED_ON_SUBS;
  }
  ...
  */
  ...
  if ( call_ptr->call_type == CM_CALL_TYPE_PS_DATA )
  {
    ...
    if (! cmwcall_is_ps_data_allowed(call_ptr->mm_id.asubs_id))
    {
  ...
  if ( ( ( call_ptr->call_type == CM_CALL_TYPE_VOICE )  ||
       ...
       ( ( ss_info_sub_prop_ptr->stack_info[MM_STACK_0].gen_info->mode_info.gw_info.sim_state
           == SYS_SIM_STATE_CS_INVALID &&
           !(call_ptr->is_volte && call_ptr->call_type == CM_CALL_TYPE_VOICE))
         ||
         ( ss_info_sub_prop_ptr->stack_info[MM_STACK_0].gen_info->mode_info.gw_info.sim_state
  ...
  subscription_source = cmph_get_gw_subscription_source (call_ptr->mm_id.asubs_id);
  if (call_ptr->call_type != CM_CALL_TYPE_EMERGENCY)
  {
    if (!ph_sub_ptr->valid_card || subscription_source == CM_CC_SUBSCRIPTION_SOURCE_NONE)
    {
      CM_MSG_HIGH_2("Orig: No subscription information! source %d valid_card %d",
                    subscription_source, ph_sub_ptr->valid_card);
  ...
  */
  ...
  {
    CM_MSG_HIGH_1( "sub %d:Orig: Subscription info not available!", call_ptr->mm_id.asubs_id);
    return CM_CALL_CMD_ERR_UIM_NOT_PRESENT_S;
  }
  ...
  if ( call_ptr->call_type != CM_CALL_TYPE_EMERGENCY &&
       cmph_aoc_no_funds_available(call_ptr->mm_id.asubs_id)
     )
  {
  ...
  */
      ...
      && call_ptr->mm_id.stk_id == MM_STACK_0)
  {
    ...
    voice_call_id = cmcall_is_there_a_call_type_no_call_id_per_sub(
                      call_ptr->mm_id.asubs_id, CM_CALL_TYPE_VOICE, call_ptr->call_id);
    ...
    if (voice_call_id != CM_CALL_ID_INVALID)
    {
      ...
      if(voice_call_ptr != NULL)
      {
        ...
        */
        if ( (voice_call_ptr->mm_id.stk_id == MM_STACK_0)             &&
  ...
  overall_call_state = cmcall_overall_call_state_per_subs(call_ptr->mm_id.asubs_id);
  if (overall_call_state  != CM_OVERALL_CALL_STATE_CONV  &&
      call_ptr->ph_ptr->sub_info[call_ptr->mm_id.asubs_id] != NULL)
  {
    call_ptr->ph_ptr->sub_info[call_ptr->mm_id.asubs_id]->phone_charge.CCM = 0;
    call_ptr->ph_ptr->sub_info[call_ptr->mm_id.asubs_id]->phone_charge.last_CCM_to_update_ACM = 0;
  }
  ...
  */
  ...
  {
    CM_ERR_2("ph_orig_top_ptr NULL for sub %d stk %d",
             call_ptr->mm_id.asubs_id, call_ptr->mm_id.stk_id);
  ...
  ** call obj
  */
  srv_status_stk0 = cmss_get_info_srv_status(call_ptr->mm_id.asubs_id, MM_STACK_0);

  if ( ph_orig_top_ptr->act_type == CM_ACT_TYPE_POLICYMAN_RESTRICT ||
       ( call_ptr->call_type == CM_CALL_TYPE_VOICE &&
         call_ptr->mm_id.stk_id == MM_STACK_0 &&
         (srv_status_stk0 == SYS_SRV_STATUS_NO_SRV ||
          srv_status_stk0 == SYS_SRV_STATUS_NO_SRV_INTERNAL ) &&
         ph_orig_top_ptr->act_type == cmtask_map_call_type_to_act_type( call_ptr->call_type ) &&
         ph_orig_top_ptr->orig->act_priority == CM_ACT_PRIORITY_30 )
     )
  {
    CM_MSG_HIGH_6( "cmwcall_orig(): ph_orig_top->act_type %d, call_type %d, sub %d, stk %d, main_srv_stat %d, force_top_pref %d",
                   ph_orig_top_ptr->act_type, call_ptr->call_type, call_ptr->mm_id.asubs_id,
                   call_ptr->mm_id.stk_id, srv_status_stk0, call_ptr->force_top_pref);
...
)
{
  ...
  stack_id = call_ptr->mm_id.stk_id;
  if(stack_id == MM_STACK_ALL || stack_id == MM_STACK_NONE)
  {
    stack_id = MM_STACK_0;
  }
  ss_misc_stack_ptr = ss_misc_sub_ptr->stack_info[stack_id].gen_info;
  if(ss_misc_stack_ptr == NULL)
  {
    return;
  }
  if (ss_misc_stack_ptr->last_system_reported != SYS_SYS_MODE_NO_SRV)
  {
    call_ptr->sys_mode = ss_misc_stack_ptr->last_system_reported;
  }

  CM_MSG_HIGH_4_EXT("sys_mode_chgd: %d, call_id %d, last_system_reported %d, sub %d", 
                        call_ptr->sys_mode, 
                        call_ptr->call_id,
                        ss_misc_stack_ptr->last_system_reported,
                        call_ptr->mm_id.asubs_id,
...
/*===========================================================================
FUNCTION cmwcall_map_sys_ho_type_to_voice_ho_type

DESCRIPTION
  Map sys_ho_type_e_type to sys_voice_handover_e_type based on mapping table
...
===========================================================================*/
sys_voice_handover_e_type cmwcall_map_sys_ho_type_to_voice_ho_type(
  sys_ho_type_e_type sys_ho_type
)
{
  ...
  for(ho_mapping_index=0; ho_mapping_index<ARR_SIZE(cmwcall_ho_mapping_table);
      ho_mapping_index++)
  {
    if(cmwcall_ho_mapping_table[ho_mapping_index].rrc_ho_type == sys_ho_type)
    {
      voice_ho_type = cmwcall_ho_mapping_table[ho_mapping_index].client_ho_type;
      CM_MSG_HIGH_2("Mapped ho type %d , RRC ho_type %d",
                    voice_ho_type,sys_ho_type);
      break;
    }
  }
  if(ho_mapping_index == ARR_SIZE(cmwcall_ho_mapping_table))
  {
    CM_MSG_HIGH_1("invalid rrc ho_type %d", sys_ho_type);
    voice_ho_type = SYS_VOICE_HO_NONE;
  }
...
)
{
  ...
  cmph_sub_info = cmph_get_sub_ptr(ll_as_id);

  CM_MSG_HIGH_5("RX: CODEC_INFO_IND, network %d, as_id %d, cmd %d, ho_type %d, oprt_mode %d",
                network_mode,
                ll_as_id,
  ...
  if(call_id == CM_CALL_ID_INVALID )
  {
    ...
    call_info_ptr->voice_ho_type = cmwcall_map_sys_ho_type_to_voice_ho_type(codec_info_ptr->ho_type);

    /* Notify clients list of call event. */

    cmcall_fill_connected_call_status_per_sub(call_info_ptr, CM_CALL_ID_INVALID);
  ...
  else
  {
    ...
    call_ptr->voice_ho_type = cmwcall_map_sys_ho_type_to_voice_ho_type(codec_info_ptr->ho_type);

    /* Notify clients list of call event. */
    switch(codec_info_ptr->cmd)
    {
      case SYS_SPEECH_CODEC_INFO:
		cmcall_update_speech_codec_info_for_gw_cs_calls(network_mode,codec_info_ptr);
        break;
      case SYS_HO_START:
      ...
      case SYS_HO_FAIL:
        ...
        break;
      case SYS_HO_COMPLETE:
		cmwcall_set_sys_mode_for_HO_complete(codec_info_ptr->ho_type,&call_ptr->sys_mode);
        if(codec_info_ptr->ho_type == SYS_HO_TYPE_LTE_GSM_SRVCC || codec_info_ptr->ho_type == SYS_HO_TYPE_LTE_UMTS_SRVCC )
        {
          CM_MSG_HIGH_0("Drop dup HO complete for L->G/ L->W srvcc");
          break;
...
void cmwcall_set_midcall_srvcc_info(cmcall_s_type *call_ptr, sys_radio_access_tech_e_type rat)
{
  ...
  if(call_ptr->call_type == CM_CALL_TYPE_VT)
  {
    CM_MSG_HIGH_2("call_type_chgd: VT change to voice, call_id %d, sub %d", 
                      call_ptr->call_id,
                      call_ptr->mm_id.asubs_id);
    call_ptr->call_type = CM_CALL_TYPE_VOICE;
    ...
    if(call_info_ptr == NULL)
    {
      CM_MSG_HIGH_0("CM_MM_SRVCC, can not alloc memory for call info object");
      return;
    }

    /* use call_info_ptr to update the log packet */
    (void) cmcall_info_get( call_ptr, call_info_ptr );
    cmlog_call_event_report_payload( CM_CALL_EVENT_HO_START, call_info_ptr);
...
void cmwcall_handle_get_srvcc_context_req(cm_rpt_type *cm_rpt_ptr)
{
  ...
  CM_MSG_HIGH_2_EXT("RXD: CM_GET_SRVCC_CONTEXT_REQ FROM MN, rat=%d, as_id=%d",
  ...
  {
    CM_MSG_HIGH_0_EXT("Call_context cache is NULL, Ignore CM_GET_SRVCC_CONTEXT_REQ",(as_id+1));
    return;
  }
  ...
  cmph_sub_info = cmph_get_sub_ptr(as_id);
  ...
  if (cmph_sub_info != NULL && !cmph_sub_info->mid_call_support)
  {
    if(!cmwcall_is_int_ims())
    {
      CM_MSG_HIGH_0_EXT("CM_GET_SRVCC_CONTEXT_REQ:legacy ext IMS",(as_id+1));
      call_id = cm_call_id_allocate_new(FALSE);
      if(call_id == CM_CALL_ID_INVALID)
      {
        CM_MSG_HIGH_0_EXT("CM_GET_SRVCC_CONTEXT_REQ, call not found",(as_id+1));
      ...
      call_ptr->mm_id.asubs_id = as_id;
      call_ptr->mm_id.stk_id = cmph_determine_ss_per_domain(call_ptr->mm_id.asubs_id,SYS_SRV_DOMAIN_CS_ONLY);
    ...
    {
      call_id = cmcall_get_active_volte_call((CM_CALL_TYPE_VOICE_MASK | CM_CALL_TYPE_EMERG_MASK | CM_CALL_TYPE_VT_MASK), as_id);
      CM_MSG_HIGH_0_EXT("CM_GET_SRVCC_CONTEXT_REQ:legacy IMS",(as_id+1));
  ...
  {
    CM_MSG_HIGH_0_EXT("CM_GET_SRVCC_CONTEXT_REQ: midcall supported",(as_id+1));
    /* timer expired ,send HO FAIL to IMS*/
    if(cm_get_cache_ind().state ==  CMWCALL_CACHE_EXPIRED)
    {
      CM_MSG_HIGH_0_EXT("CM_GET_SRVCC_CONTEXT_REQ:Int/Ext, send HO FAIL context,timer expired",(as_id+1));
    ...
    else if( cm_get_cache_ind().state == CMWCALL_CACHE_EXPECTED)
    {
      ...
      CM_MSG_HIGH_0_EXT("CM_GET_SRVCC_CONTEXT_REQ:ext IMS, NAS Request queued till cache expiry", (as_id+1));
      cm_mem_free(cm_srvcc_call_context_p);
      return;
    }
    ...
    else if( cm_get_cache_ind().state== CMWCALL_CACHE_RECIEVED)
    {
      ...
      CM_MSG_HIGH_0_EXT("CM_GET_SRVCC_CONTEXT_REQ: send NAS response with valid cache",(as_id+1));
        ...
        if( call_ptr->is_srvcc_call)
        {
          ...
          switch(cm_rpt_ptr->cmd.nas_srvcc_get_req_ind.active_rat)
          {
            case SYS_RAT_GSM_RADIO_ACCESS:
              ...
              if(call_ptr->call_state == CM_CALL_STATE_ORIG)
              {
                CM_MSG_HIGH_1("cmwcall_handle_get_srvcc_context_req: setting sr orig_sent_on_mode to GSM for call %d",
                              call_ptr->call_id);
            ...
            case SYS_RAT_UMTS_RADIO_ACCESS:
              ...
              if(call_ptr->call_state == CM_CALL_STATE_ORIG )
              {
                CM_MSG_HIGH_1("cmwcall_handle_get_srvcc_context_req: setting sr orig_sent_on_mode to WCDMA for call %d",
                              call_ptr->call_id);
...

{
  ...
  cm_call_state_e_type callptr_call_state;
  /* call state in call_ptr*/

  cm_call_substate_type  callptr_call_subst;
  /*call sub state in call_ptr*/
  ...
  if(cahce_rpt_ptr->as_id != cm_get_cache_ind().asubs_id)
  {
	CM_MSG_HIGH_2( "Ignore the IMS cache received on SUB%d as SRVCC handover is initiated on SUB%d",
	                cahce_rpt_ptr->as_id, cm_get_cache_ind().asubs_id);
    return;
  }
    ...
    for(i=0; i<cahce_rpt_ptr->num_of_calls; i++)
    {
        ...
        for(j = 0; j<tot_participants; j++)
        {
              ...
              == CM_CALL_STATE_CONV)
          {
            ...
            */
            ...
            call_ptr->mm_id.asubs_id = cahce_rpt_ptr->as_id;
            call_ptr->mm_id.stk_id = MM_STACK_0;
            ...
            if (cmwcall_is_int_ims())
            {
              CM_MSG_HIGH_2("handle_srvcc_ims_cache: internal IMS num.len = %d, num.pi = %d",
                            cahce_rpt_ptr->call_context[i].call_info.ip_conf_call.participant_list[j].participant_num.len,
                            cahce_rpt_ptr->call_context[i].call_info.ip_conf_call.participant_list[j].participant_num.pi);

              /* For internal IMS, only copy the new number buffer and PI from IMS */
              if ( cahce_rpt_ptr->call_context[i].call_info.ip_conf_call.participant_list[j].participant_num.len != 0 )
              {
                call_ptr->num.len = memscpy( call_ptr->num.buf,
                                             sizeof(call_ptr->num.buf),
                                             cahce_rpt_ptr->call_context[i].call_info.ip_conf_call.participant_list[j].participant_num.buf,
                                             cahce_rpt_ptr->call_context[i].call_info.ip_conf_call.participant_list[j].participant_num.len ) ;
              }
              call_ptr->calling_num.pi = cahce_rpt_ptr->call_context[i].call_info.ip_conf_call.participant_list[j].participant_num.pi;
            }
            else
            {
              CM_MSG_HIGH_2("handle_srvcc_ims_cache: external IMS num.len = %d, num.pi = %d",
                            cahce_rpt_ptr->call_context[i].call_info.ip_conf_call.participant_list[j].participant_num.len,
                            cahce_rpt_ptr->call_context[i].call_info.ip_conf_call.participant_list[j].participant_num.pi);
              /* For external IMS, copy the entire num */
              call_ptr->num =
      ...
      else
      {
            ...
            cahce_rpt_ptr->call_context[i].call_info.ip_call.call_state == CM_CALL_STATE_ORIG)
        {
          ...
          /*Store current call state and substate as per call_ptr */
          callptr_call_state = call_ptr->call_state;
          callptr_call_subst = call_ptr->call_subst;
          ...
          if (!cmwcall_is_int_ims())
          {
            if (cahce_rpt_ptr->call_context[i].call_info.ip_call.call_state == CM_CALL_STATE_ORIG)
            {
              ...
              cmcall_set_favored_mode_pref_for_origination (
                call_ptr->mm_id, // this param is not used for non DSDS
                call_ptr->call_type,
                call_ptr->srv_type,
              ...
              if(call_ptr->call_type == CM_CALL_TYPE_EMERGENCY )
              {
                call_ptr->favored_roam_pref = CM_ROAM_PREF_ANY;
              }
              else
              {
                call_ptr->favored_roam_pref = CMPH_SS_ROAM_PREF(cahce_rpt_ptr->as_id, cmph_ptr());
              }
              call_ptr->favored_mode_pref = CM_MODE_PREF_GWL;
              call_ptr->favored_prl_pref = cmcall_get_favored_prl_pref(call_ptr->call_type);

              call_ptr->orig_mode = cmph_map_cm_call_type_to_sd_orig_mode(call_ptr->call_type,
                                                                          call_ptr->mm_id.asubs_id );
              call_ptr->sr.is_ok_to_orig =  TRUE;

            /* Assign SR MAX Uptime
                    ** - If Third Party IMS case.
                    ** - Call State is Orig
                    ** - Call has been SRVCCed to NAS
                    ** - As call is SRVCCed from third party IMS, we do not have prior context.
                    ** - We would update the sr_max_uptime to have a max cap on time for which silent redial tried on SRVCCed call
                    */
               ...
               */
               ...
               sr_ptr->last_call_delay_evt_sent_timestamp = uptime;
          ...
          CM_MSG_HIGH_4_EXT ("SRVCC call id: %d , substate: conv_subst %d, incom_subst %d , orig_subst %d",
          ...
          CM_MSG_HIGH_2("handle_srvcc_ims_cache: internal IMS num.len = %d, num.pi = %d",
                        cahce_rpt_ptr->call_context[i].call_info.ip_call.call_num.len,
                        cahce_rpt_ptr->call_context[i].call_info.ip_call.call_num.pi);

          /* For internal IMS, only copy the new number buffer and PI from IMS */
          if ( cahce_rpt_ptr->call_context[i].call_info.ip_call.call_num.len != 0 )
          {
            call_ptr->num.len = memscpy( call_ptr->num.buf,
                                         sizeof(call_ptr->num.buf),
                                         cahce_rpt_ptr->call_context[i].call_info.ip_call.call_num.buf,
                                         cahce_rpt_ptr->call_context[i].call_info.ip_call.call_num.len ) ;
          }
          call_ptr->calling_num.pi = cahce_rpt_ptr->call_context[i].call_info.ip_call.call_num.pi;
          ...
          /* If call has already been answered by user at instant when cache is received
          ** call_subst as per call_ptr will be INCOM_SUBST_ANSWERED but substate in
          ** IMS cache is INCOM_SUBST_ALERTING(cc_state is U7): update CC_state to U8
          ** To ensure cc state informed to NAS is correct as per current call state.
          */
          if((callptr_call_state == CM_CALL_STATE_INCOM &&
              callptr_call_subst.incom == CM_CALL_INCOM_SUBST_ANSWERED) &&
              (call_context_t->individual_call_context[call_context_ind].srvcc_call_cc_state == CC_STATE_U7 ))
          {
            CM_MSG_HIGH_0("SRVCC:Update NAS CC state for answered call ");
            call_context_t->individual_call_context[call_context_ind].srvcc_call_cc_state = CC_STATE_U8;
          }
  ...
  CM_MSG_HIGH_1_EXT("Total calls in SRVCC HO:,num_calls:%d",
                    call_context_t->num_calls,
                    (cahce_rpt_ptr->as_id+1));
...
)
{
  ...
  if(ims_call_id == CM_CALL_ID_INVALID)
  {
    ...
    if(cmd == CM_SRVCC_HO_IND_FAILURE )
    {
      ...
      else
      {
        ...
        if ( call_info_ptr == NULL )
        {
          CM_MSG_HIGH_0_EXT("Ext IMS:CM_MM_SRVCC_HANDOVER_FAILURE_IND, can not alloc memory for call info object",
                            (asubs_id+1));
          return;
        }
        call_info_ptr->voice_ho_type = cmwcall_map_sys_ho_type_to_voice_ho_type(cm_get_cache_ind().ho_type);

        call_info_ptr->asubs_id = asubs_id;
        cmcall_fill_connected_call_status_per_sub(call_info_ptr, CM_CALL_ID_INVALID);

        cmclient_list_call_event_ntfy ( CM_CALL_EVENT_HO_FAIL, call_info_ptr );

        CM_MSG_HIGH_0_EXT("Ext IMS:CM_MM_SRVCC_HANDOVER_FAILURE_IND, Notify clients list of call event",
                          (asubs_id+1));
    ...
    else if (cmd == CM_SRVCC_HO_IND_COMPLETE)
    {
      ...
      CM_MSG_HIGH_0_EXT("Ext IMS:CM_NAS_SRVCC_HANDOVER_COMPLETE_IND, Notify clients list of call event",
                        (asubs_id+1));
    }
  ...
  else
  {
    ...
    if(cmd == CM_SRVCC_HO_IND_FAILURE || cmd == CM_SRVCC_HO_IND_CANCEL )
    {
      ...
      {
        cmcall_event( call_ptr, CM_CALL_EVENT_HO_FAIL );
        CM_MSG_HIGH_0_EXT("CM_MM_SRVCC_HANDOVER_FAILURE_IND, Notify clients list of call event",(asubs_id+1));
      }
    ...
    else if (cmd == CM_SRVCC_HO_IND_COMPLETE)
    {
      ...
      CM_MSG_HIGH_0_EXT("CM_NAS_SRVCC_HANDOVER_COMPLETE_IND, Notify clients list of call event",
                        (asubs_id+1));
...
)
{
  ...
  cm_act_id_type   srvcced_act_id = CM_ACT_ID_NONE;

#endif
  sys_modem_as_id_e_type ll_as_id = SYS_MODEM_AS_ID_1;
  mm_sub_stk_id_s_type rpt_mm_id;
  ...
  CM_ASSERT( cm_rpt_ptr != NULL );
  rpt_mm_id.asubs_id = cm_rpt_ptr->asubs_id;
...
#error code not present
    ...
    case CM_CALL_CONNECTED_IND:
      ...
      */
      ...
      {
        cmpd_session_grant_proc( pd_ptr->session_type, call_ptr->mm_id.asubs_id);
      }
      ...
      {
        cmpd_session_grant_proc( pd_dmod_ptr->session_type, call_ptr->mm_id.asubs_id);
      }
#endif

      cmcall_update_gsdi_and_log_call_state(call_ptr->mm_id);
      break;
    ...
    case CM_MT_SETUP_IND:
      ...
      /* Assign the SS of the call based on as_id
         */
      call_ptr->mm_id.asubs_id = cmph_get_as_id_from_ll_ind(cm_rpt_ptr->cmd.setup_ind.as_id);
      call_ptr->mm_id.stk_id = cmph_determine_ss_per_domain(call_ptr->mm_id.asubs_id,SYS_SRV_DOMAIN_CS_ONLY);
      CM_ASSERT_ENUM_IS_INRANGE(call_ptr->mm_id.asubs_id, SYS_MODEM_AS_ID_MAX);
...
#error code not present
      ...
      if (((wcall_info_ptr->bearer_capability_1.present) &&
           ...
           &&
           (wcall_info_ptr->bearer_capability_1.information_transfer_capability
            != cmph_ptr()->device_prop.itc_aux_val_to_use
           )
#endif
          )
          ||
      ...
      */
      if( (( CMPH_SS_DISABLE_CALL_TYPE_MASK(call_ptr->mm_id.asubs_id, ph_ptr) & CM_CALL_TYPE_VOICE_MASK ) &&
           ( call_ptr->call_type == CM_CALL_TYPE_VOICE ||
             call_ptr->call_type == CM_CALL_TYPE_EMERGENCY ))
          ||
          ( ( CMPH_SS_DISABLE_CALL_TYPE_MASK(call_ptr->mm_id.asubs_id, ph_ptr) & CM_CALL_TYPE_CS_DATA_MASK )&&
            ( call_ptr->call_type == CM_CALL_TYPE_CS_DATA ))
        )
      {
        ie_cm_cc_cause_T             cause;
      ...
      */
      ...
      if(!cmwcall_check_mt_call_due_to_ue_based_barr(call_ptr,rpt_ptr))
      {
        break;
      }
      ...
      call_ptr->call_state = CM_CALL_STATE_INCOM;
      call_ptr->call_subst.incom = CM_CALL_INCOM_SUBST_SETUP;
      CM_MSG_HIGH_4("NAS->CM: CALL: CM_MT_SETUP_IND, incom subst: %d, call_id %d, sub %d, call_type %d", 
      ...
      if( wcall_info_ptr->calling_party_bcd_number.present )
      {
        ...
        */
        ...
        if(!cmwcall_check_mt_anonymous_due_to_ue_based_barr(call_ptr,rpt_ptr ))
          {
            break;
           }
...

      {
        ...
        if(cmss_get_true_sys_mode(call_ptr->mm_id))
        {
          call_ptr->sys_mode = cmss_get_true_sys_mode(call_ptr->mm_id);
        }
      ...
      if (cmcall_overall_call_state_per_subs(call_ptr->mm_id.asubs_id) == CM_OVERALL_CALL_STATE_INCOM &&
          call_ptr->mm_id.asubs_id > SYS_MODEM_AS_ID_NONE &&
          call_ptr->mm_id.asubs_id < MAX_SIMS &&
          call_ptr->ph_ptr->sub_info[call_ptr->mm_id.asubs_id] != NULL)
      {
        call_ptr->ph_ptr->sub_info[call_ptr->mm_id.asubs_id]->phone_charge.CCM = 0;
      ...
      /* Terminate ongoing periodic search
      */
      if(cmutil_is_periodic_manual_search())
      {
        CM_MSG_HIGH_0("GET_NET: PERIODIC: TERMINATE: MT setup indication");
    ...
    case CM_MT_CALL_IND:
      ...
      CM_MSG_HIGH_4("NAS->CM: CALL: CM_MT_CALL_IND, connection_id %d, call_id %d, sub %d, call_state %d", 
                        cm_rpt_ptr->cmd.call_ind.connection_id,
                        call_id,
                        cm_rpt_ptr->cmd.call_ind.as_id,
                        call_ptr->call_state);
    ...
    case CM_MT_DISC_IND:
      ...
      CM_MSG_HIGH_5("NAS->CM: CALL: sub %d, RXD: CM_MT_DISC_IND inband_tone_present = %d call_state %d direction %d, call_id %d",
                    cm_rpt_ptr->cmd.disc_ind.as_id,
                    cm_rpt_ptr->cmd.disc_ind.inband_tone_present,
                    call_ptr->call_state,
                    call_ptr->direction,
                    call_ptr->call_id);
    ...
    case CM_MT_END_CALL_IND:
      ...
      */
      ...
      CM_MSG_HIGH_5_EXT("NAS->CM: CALL: CM_MT_END_CALL_IND, connection_id %d, call_id %d, sub %d, call_state %d, sys_mode %d", 
                        cm_rpt_ptr->cmd.end_call_ind.connection_id,
                        call_id,
                        cm_rpt_ptr->cmd.end_call_ind.as_id,
                        call_ptr->call_state,
                        call_ptr->sys_mode,
                        (cm_rpt_ptr->cmd.end_call_ind.as_id+1));

      /* This indication should be ignored if the call has already been re-originated over IMS
      */
      if(call_ptr->is_vowlan
#if defined(FEATURE_IP_CALL) && defined(FEATURE_CM_LTE)
          || (call_ptr->is_volte && !call_ptr->is_srvcc_call)
#endif
        )
      {
        CM_MSG_HIGH_1("ignore CM_MT_END_CALL_IND for call %d since the call has been re-originated over IMS",
                      call_ptr->call_id);
        break;
      }
    ...
    case CM_MT_CALL_PAGE_FAIL_IND:
      ...
      CM_MSG_HIGH_4( "NAS->CM: CALL: sub %d, RXD: CM_MT_CALL_PAGE_FAIL_IND rat %d, fail cause %d, rrc page cause %d",
                     cm_rpt_ptr->cmd.call_fail_after_page_info.as_id,
                     cm_rpt_ptr->cmd.call_fail_after_page_info.rat,
    ...
    case CM_5_SEND_CONF:
      ...
      */
      if (wcall_info_ptr->ccbs_ret_res.ccbs_cnf == CM_CCBS_CNF_RET_RES)
      {
        ...
        */
        if (!cmph_add_ccbs_entry (call_ptr->mm_id.asubs_id, ccbs_conf_ptr->rsp.ret_res.ccbsIndex.ccbs_Index,
...
#error code not present
    ...
    case CM_PDP_ACTIVATE_CNF:
      ...
      */
      cmss_update_ps_dormant_entry_in_queue(TRUE, cm_rpt_ptr->cmd.pdp_act_conf.as_id);
    ...
    case CM_RAB_RELEASE_IND:
      ...
      CM_MSG_HIGH_5("RXD: CM_RAB_RELEASE_IND, connection_id %d, sub %d, info_type %d, call_id %d sys_mode %d",
                    cm_rpt_ptr->cmd.rab_release_ind.connection_id,
                    cm_rpt_ptr->cmd.rab_release_ind.as_id,
                    call_ptr->cmcall_mode_info.info_type,
                    call_id,
    ...
    case CM_RAB_REESTABLISH_IND:
      ...
      CM_MSG_HIGH_6("RXD: CM_RAB_REESTABLISH_IND, connection_id %d, sub %d, info_type %d, sys_rat %d, call_id %d sys_mode %d",
                    cm_rpt_ptr->cmd.rab_reestab_ind.connection_id,
                    cm_rpt_ptr->cmd.rab_reestab_ind.as_id,
                    call_ptr->cmcall_mode_info.info_type,
                    cm_rpt_ptr->cmd.rab_reestab_ind.sys_rat,
                    call_id,
                    call_ptr->sys_mode);
    ...
    case CM_PDP_ACTIVATE_REJ:
      ...
      if (cmph_is_msim())
      {
        rpt_mm_id.asubs_id = cm_rpt_ptr->cmd.pdp_act_rej.as_id;
      }
      if (!cmwcall_is_valid_asubs_id_from_ll(rpt_mm_id.asubs_id))
      {
        CM_ERR_1(" Invalid as_id %d", rpt_mm_id.asubs_id);
        break;
      }

      rpt_mm_id.stk_id = cmph_determine_ss_per_domain(rpt_mm_id.asubs_id,
                         SYS_SRV_DOMAIN_PS_ONLY);
      ...
      CM_ASSERT( call_ptr != NULL );
      CM_ASSERT(rpt_mm_id.stk_id == call_ptr->mm_id.stk_id);
      CM_ASSERT(rpt_mm_id.asubs_id == call_ptr->mm_id.asubs_id);
      ...
      ps_wcall_info_ptr->backoff_timer.valid
        = cm_rpt_ptr->cmd.pdp_act_rej.backoff_timer.valid;

      ps_wcall_info_ptr->reattempt_indicator.valid
        = cm_rpt_ptr->cmd.pdp_act_rej.reattempt_indicator.valid;
      ps_wcall_info_ptr->reattempt_indicator.ratc_val
        = cm_rpt_ptr->cmd.pdp_act_rej.reattempt_indicator.ratc_val;
      ...
      **
      */
      if(! cmcall_check_if_ps_call_active(rpt_mm_id))
      {
        cmph_set_ps_sig_down_wait_time(CMPH_GW_PKT_SIGNALING_DOWN_WAIT_TIME, rpt_mm_id.asubs_id);
      }
      break;
    ...
    case CM_PDP_DEACTIVATE_CNF:
      ...
      if (cmph_is_msim())
      {
        rpt_mm_id.asubs_id = cm_rpt_ptr->cmd.pdp_deact_conf.as_id;
      }
      if (!cmwcall_is_valid_asubs_id_from_ll(rpt_mm_id.asubs_id))
      {
        CM_ERR_1("Invalid as_id %d",rpt_mm_id.asubs_id);
      ...
      */
      ...
      /* At this point, call id has been deallocated, and call_ptr->mm_id is updated as well */
    ...
    case CM_PDP_DEACTIVATE_IND:
    {
      ...
      */
      ...
      if (cmph_is_msim())
      {
        rpt_mm_id.asubs_id = cm_rpt_ptr->cmd.pdp_deact_ind.as_id;
      }
      if (!cmwcall_is_valid_asubs_id_from_ll(rpt_mm_id.asubs_id))
      {
        CM_ERR_1("Invalid as_id %d",rpt_mm_id.asubs_id);
      ...
      ps_wcall_info_ptr->backoff_timer.valid = cm_rpt_ptr->cmd.pdp_deact_ind.backoff_timer.valid;
    ...
    case CM_PDP_DEACTIVATE_REJ:
      ...
      */
      if (cmph_is_msim())
      {
        rpt_mm_id.asubs_id = cm_rpt_ptr->cmd.pdp_deact_rej.as_id;
      }
      if (!cmwcall_is_valid_asubs_id_from_ll(rpt_mm_id.asubs_id))
      {
        CM_ERR_1("Invalid as_id %d",rpt_mm_id.asubs_id);
    ...
    case CM_PDP_ACTIVATE_IND:
      ...
      */
      ...
      call_ptr->mm_id.asubs_id = cmph_get_as_id_from_ll_ind(cm_rpt_ptr->cmd.pdp_act_ind.as_id);
      call_ptr->mm_id.stk_id = cmph_determine_ss_per_domain(cm_rpt_ptr->cmd.pdp_act_ind.as_id,
                               SYS_SRV_DOMAIN_PS_ONLY);
      /* If an invalid subscription-id is sent, reject the incoming call
      */
      if (!cmwcall_is_valid_asubs_id_from_ll(cm_rpt_ptr->cmd.pdp_act_ind.as_id))
      {
      ...
      */
      ...
      else
      {
        ...
        */
        if ( cmcall_count_primary_pdp_calls(call_ptr->mm_id) >
    ...
    case CM_PDP_MODIFY_IND:
      if( !cm_find_call_id_per_subs( CONN_ID_SM,
                            ...
                            &call_id,
							cm_rpt_ptr->cmd.pdp_modify_ind.as_id) )
      {
        CM_ERR_1(" ERROR: Unknown Connection ID: %d",
                 cm_rpt_ptr->cmd.pdp_modify_ind.connection_id);
        break;
    ...
    case CM_PDP_MODIFY_CNF:
      if( !cm_find_call_id_per_subs( CONN_ID_SM,
                            ...
                            &call_id,
                            cm_rpt_ptr->cmd.pdp_modify_conf.as_id) )
      {
        CM_ERR_1("ERROR: Unknown Connection ID: %d",
                 cm_rpt_ptr->cmd.pdp_modify_conf.connection_id);
        break;
    ...
    case CM_PDP_DEACTIVATE_PROMOTE_IND:
      ...
      if( !cm_find_call_id_per_subs( CONN_ID_SM,
                            ...
                            &call_id,
                            cm_rpt_ptr->cmd.pdp_deact_prom_ind.as_id) )
      {
        CM_ERR_1("ERROR: Unknown Connection ID:%d ",
                 cm_rpt_ptr->cmd.pdp_deact_prom_ind.connection_id);
        break;
    ...
    case CM_PDP_DEACTIVATE_PROMOTE_CNF:
      if( !cm_find_call_id_per_subs( CONN_ID_SM,
                            ...
                            &call_id,
                            cm_rpt_ptr->cmd.pdp_deact_prom_cnf.as_id) )
      {
        CM_ERR_1("ERROR: Unknown Connection ID: %d",
                 cm_rpt_ptr->cmd.pdp_deact_prom_cnf.connection_id);
        break;
    ...
    case CM_PDP_ACTIVATE_SEC_CNF:
      if( !cm_find_call_id_per_subs(CONN_ID_SM,
                           ...
                           &call_id,
                           cm_rpt_ptr->cmd.pdp_act_sec_conf.as_id) )
      {
        CM_ERR_1("ERROR: Unknown Connection ID: %d",
                 cm_rpt_ptr->cmd.pdp_act_sec_conf.connection_id);
        break;
      ...
      ** in the priority queue.
      */
      cmss_update_ps_dormant_entry_in_queue(TRUE, cm_rpt_ptr->cmd.pdp_act_sec_conf.as_id);
    ...
    case CM_PDP_ACTIVATE_SEC_REJ:
      ...
      ps_wcall_info_ptr->backoff_timer.valid
        = cm_rpt_ptr->cmd.pdp_act_sec_rej.backoff_timer.valid;

      ps_wcall_info_ptr->reattempt_indicator.valid
        = cm_rpt_ptr->cmd.pdp_act_sec_rej.reattempt_indicator.valid;
      ps_wcall_info_ptr->reattempt_indicator.ratc_val
        = cm_rpt_ptr->cmd.pdp_act_sec_rej.reattempt_indicator.ratc_val;
    ...
    case CM_PDP_MODIFY_REJ:
      ...
      ps_wcall_info_ptr->backoff_timer.valid
        = cm_rpt_ptr->cmd.pdp_modify_rej.backoff_timer.valid;

      ps_wcall_info_ptr->reattempt_indicator.valid
        = cm_rpt_ptr->cmd.pdp_modify_rej.reattempt_indicator.valid;
      ps_wcall_info_ptr->reattempt_indicator.ratc_val
        = cm_rpt_ptr->cmd.pdp_modify_rej.reattempt_indicator.ratc_val;
    ...
    case CM_MMGSDI_REFRESH_FCN_IND:
    {
      ...
      if(ph_sub_ptr != NULL)
      {
        CM_MSG_HIGH_2("RXD: SIM_REF: refresh_vote_ok = %d, is_only_data_call = %d",
                      ph_sub_ptr->refresh_vote_ok, is_only_data_call);
      }
      ...
      {
        if(is_emergency_call_active || cmph_is_in_emergency_cb())
        {
      ...
      if(ph_sub_ptr == NULL)
      {
        CM_MSG_HIGH_1("Voting TRUE as no subscription on slot %d", slot_id);
          ...
          refresh_data->stage == MMGSDI_REFRESH_STAGE_WAIT_FOR_OK_TO_INIT)
      {
        if (!is_refresh_sent)
        {
          cmmmgsdi_ok_to_refresh(slot_id,
                                 cm_rpt_ptr->cmd.card_status_mmgsdi.session_id,
                                 ph_sub_ptr->refresh_vote_ok?TRUE:is_only_data_call);
        }
        break;
      }
      ...
      /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
              ...
              refresh_data->stage == MMGSDI_REFRESH_STAGE_WAIT_FOR_OK_TO_FCN)
      {
        if(ph_sub_ptr->refresh_vote_ok == TRUE || is_only_data_call == TRUE)
        {
          cmmmgsdi_ok_to_refresh( slot_id,
                                  cm_rpt_ptr->cmd.card_status_mmgsdi.session_id,
                                  TRUE );
        ...
        */
        if(refresh_data->refresh_files.file_list_ptr != NULL )
		{
          if(!cmmmgsdi_validate_received_file_list(
           ...
           {
             cmmmgsdi_refresh_init( &refresh_data->refresh_files, asubs_id);
             if (refresh_data->refresh_files.file_list_ptr != NULL)
             {
               cm_mem_free(refresh_data->refresh_files.file_list_ptr);
               refresh_data->refresh_files.file_list_ptr = NULL;
      ...
      /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
                ...
                &&
                (cmph_get_received_card_init_completed_evt(asubs_id) == TRUE)
              )
      {
        cmph_set_received_card_init_completed_evt(FALSE, asubs_id);

        /* If CDMA subscription from card is already sent, make CDMA
           ** subscription not available
           */
        if (cmph_get_cdma_subs_from_card_sent(asubs_id) == TRUE
        ...
        /* If GW subscription from card is already sent, make GW
           ** subscription not available
           */
        if (cmph_get_gw_subs_from_card_sent(asubs_id) == TRUE
        ...
        */
        ...
        {
          cm_subscription_not_avail_cause_e_type cause = CM_SUBSCRIPTION_NOT_AVAIL_CAUSE_OTHERS;

          /* If standby change is not needed, map cause to NO_STDBY */
          if(!cmph_is_standby_chg_on_refresh(refresh_data->mode, refresh_data->source))
          {
            cause = CM_SUBSCRIPTION_NOT_AVAIL_CAUSE_REFRESH_NO_STDBY;
          }
          if(!cm_ph_cmd_subscription_not_available4(
                NULL,
                NULL,
    ...
    {
      if (!cmwcall_is_valid_asubs_id_from_ll(cm_rpt_ptr->cmd.ps_sig_rel_ind.as_id))
      {
        CM_ERR_1("Invalid as_id %d",cm_rpt_ptr->cmd.ps_sig_rel_ind.as_id);
        break;
      }

      CM_MSG_HIGH_1("RXD: CM_PS_SIG_CON_REL_IND on Sub %d", cm_rpt_ptr->cmd.ps_sig_rel_ind.as_id);

      cmph_set_ps_sig_down_wait_time(CMPH_GW_PKT_WAIT_TIME_NONE,
                                     cm_rpt_ptr->cmd.ps_sig_rel_ind.as_id);
    }
    break;
      ...
      while(call_ptr != NULL)
      {
            ...
            call_ptr->call_state == CM_CALL_STATE_CC_IN_PROGRESS)
        {
          if((call_ptr->call_type == CM_CALL_TYPE_VOICE ||
              ...
              (call_ptr->mm_id.asubs_id != call_info_ptr->asubs_id))
          {
            call_ptr->sr.next_orig_uptime = uptime ;
            if (call_ptr->call_state == CM_CALL_STATE_ORIG)
            {
    ...
    case CM_PDP_ACTIVATE_SEC_IND:
      ...
      */
      ...
      call_ptr->mm_id.asubs_id = cmph_get_as_id_from_ll_ind(cm_rpt_ptr->cmd.pdp_act_sec_ind.as_id);
      call_ptr->mm_id.stk_id = cmph_determine_ss_per_domain(cm_rpt_ptr->cmd.pdp_act_sec_ind.as_id,
                               SYS_SRV_DOMAIN_PS_ONLY);
      /* If an invalid subscription-id is sent, reject the incoming call
      */
      if (!cmwcall_is_valid_asubs_id_from_ll(cm_rpt_ptr->cmd.pdp_act_sec_ind.as_id))
      {
    ...
    case CM_PDP_NW_MODIFY_REQ:
      if( !cm_find_call_id_per_subs( CONN_ID_SM,
                            ...
                            &call_id,
							cm_rpt_ptr->cmd.pdp_nw_modify_req.as_id) )
      {
        CM_ERR_0("ERROR: Unknown Connection ID");
        break;
      }
      ...
      ps_wcall_info_ptr->pdp_addr          =
        cm_rpt_ptr->cmd.pdp_nw_modify_req.pdp_addr;
    ...
    case CM_NAS_SRVCC_HANDOVER_COMPLETE_IND:
      ...
      CM_MSG_HIGH_2_EXT("RXD: CM_NAS_SRVCC_HANDOVER_COMPLETE_IND, as_id=%d,success=%d",
                        ll_as_id,
      ...
      if(cm_rpt_ptr->cmd.srvcc_handover_complete_ind.success)
      {
        ...
        while(call_ptr != NULL)
        {
          if (call_ptr->is_srvcc_call)
          {
            ...
            if(call_ptr->is_int_ims)
            {
              if (!cm_unreserve_connection_id(call_ptr->ims_connection_id, call_ptr->mm_id.asubs_id ))
              {
                CM_ERR_2( "Could not unreserve MN connection id %d for %d.",
                          call_ptr->ims_connection_id, call_ptr->call_id);
              }

              ph_p_orig = cmtask_orig_para_search_act_type(call_ptr->mm_id,CM_ACT_TYPE_PH_OBJ ) ;
              p_orig = cmtask_orig_para_search_act_id(call_ptr->mm_id,(cm_act_id_type)call_ptr->call_id);
              ...
              if(p_orig != NULL)
              {
                ...
                ** made with LTE_ONLY) */
                ...
                /*Store call's act_id to be used later */
                srvcced_act_id = p_orig->orig->act_id;
              }
            }
            else
            {
              ...
              */
              ...
              {
                p_orig_emg = cmtask_orig_para_search_act_id( call_ptr->mm_id, cmemg_get_act_id(call_ptr->mm_id.asubs_id) );
                if(p_orig_emg == NULL)
                {
                  (void)cmph_update_orig_param_incoming_call(call_ptr);
                }
                ...
                /*Store call's act_id to be used later */
                srvcced_act_id = (cm_act_id_type) call_ptr->call_id;
              }
              /* Exit LTE e911 searching state */
              cmemg_reset_emerg_state(ll_as_id);
            }
...

            {
              ...
              cm_mode_pref_e_type resultant_mode_pref;

              /* Look for the OBJ corresponding to SRVCCed call in priority queue and
              ** update mode_pref to remove LTE (Since call that has been successfully
              ** transferred to CS domain through SRVCC will not go back to IMS domain) */

              /*Reset p_orig to NULL first */
              p_orig = NULL;
              /*Search for obj in pri queue using act_id */
              p_orig =  cmtask_orig_para_search_act_id( call_ptr->mm_id, srvcced_act_id ) ;

              if(p_orig != NULL)
              {
                resultant_mode_pref = cm_remove_mode_pref_components(
                                        p_orig->orig->orig_mode_pref,
                                        1,
                                        SD_SS_MODE_PREF_LTE
                                      );
                if(resultant_mode_pref != CM_MODE_PREF_NONE)
                {
                  p_orig->orig->orig_mode_pref = resultant_mode_pref;
                  CM_MSG_MED_1("update orig mode pref to %d after SRVCC",p_orig->orig->orig_mode_pref);

                }
              }

              /* If favored mode pref is not NONE and resultant one after removing
              ** LTE is not NONE, remove LTE from call_ptr->favored_mode_pref
              */
              if(call_ptr != NULL && call_ptr->favored_mode_pref != CM_MODE_PREF_NONE )
              {
                resultant_mode_pref = CM_MODE_PREF_NONE;
                resultant_mode_pref = cm_remove_mode_pref_components(
                                        call_ptr->favored_mode_pref,
                                        1,
                                        SD_SS_MODE_PREF_LTE
                ...
                if(resultant_mode_pref != CM_MODE_PREF_NONE)
                {
                  call_ptr->favored_mode_pref = resultant_mode_pref;
                  CM_MSG_MED_1("update fav mode pref to %d after SRVCC",call_ptr->favored_mode_pref);
                }
              }
            }
            call_ptr->cmcall_mode_info.info_type = CM_CALL_MODE_INFO_GW_CS;
           ...
           /* If RTT is enabled for the call, disable RTT mode since call is moving to CS 
           */
           if(call_ptr->cmcall_mode_info.info.ip_info.ipcall_attrib_info.rtt_mode == CM_CALL_RTT_MODE_FULL_MODE )
           {
             call_ptr->cmcall_mode_info.info.ip_info.ipcall_attrib_info.rtt_mode = CM_CALL_RTT_MODE_DISABLED;
           }
            ...
            if(call_ptr->call_type == CM_CALL_TYPE_EMERGENCY )
            {
              ...
              if(ph_sub_config_ptr->emerg_call_prop.is_ecbm_required)
              {
            ...
            if ( call_info_ptr == NULL )
            {
              CM_MSG_HIGH_0_EXT("CM_NAS_SRVCC_HANDOVER_COMPLETE_IND, can not alloc memory for call info object",
                                (ll_as_id+1));
              break;
            }
            ...
            {
              CM_MSG_HIGH_0_EXT("CM_NAS_SRVCC_HANDOVER_COMPLETE_IND, can not alloc memory for srvcc call info object",
                                (ll_as_id+1));
              break;
            }
            ...
            * (&(ho_comp_list->info[num_srvcc_calls])) = (*srvcc_call_info_ptr);
            ...
            CM_MSG_HIGH_3_EXT("CM_NAS_SRVCC_HANDOVER_COMPLETE_IND, call id:%d,call_staet:%d,call_tid:%d",
        ...
        if(cmwcall_is_int_ims())
        {
          ...
          this indication */
          cmsds_reset_ims_reg_and_voice_avail_status(ll_as_id);
        ...
        {
          if(CM_CALL_ID_INVALID == cmcall_is_there_volte_call_sub_ignore(ll_as_id, ims_call_id))
          {
            cmph_send_trm_priority_req_proc_type(SYS_PROC_TYPE_NONE,
                                                  ll_as_id,
                                                  MM_STACK_0,
        ...
        {
          cmmsc_proc_cmd_pref_sys_chgd_s_type  cmd;
          mm_sub_stk_id_s_type id= cmph_map_sub_stk_to_id(ll_as_id, MM_STACK_0);
          ph_orig_top_ptr = cmtask_orig_para_get_top(id);
          if (ph_orig_top_ptr != NULL)
          {
            CM_MSG_HIGH_4("CM->MSGR: sub %d, srvcc_complete:  act_id %d orig_mode %d orig mode pref %d",
                          ll_as_id, ph_orig_top_ptr->orig->act_id,
        ...
        cm_ims_cache_reset();
        CM_MSG_HIGH_0_EXT("CM_NAS_SRVCC_HANDOVER_COMPLETE_IND, msgr sent HO calls",(ll_as_id+1));
      ...
      else
      {
        ...
        cm_rpt_ptr->hdr.cmd          = CM_MM_SRVCC_HANDOVER_FAILURE_IND_INTERNAL;
        cm_rpt_ptr->cmd.srvcc_handover_failure_ind.as_id = ll_as_id;
        cm_set_ims_cache_error();
        CM_MSG_HIGH_0_EXT("CM_NAS_SRVCC_HANDOVER_COMPLETE_IND received with NAS FAIL",(ll_as_id+1));
    ...
    case CM_MM_SRVCC_HANDOVER_FAILURE_IND_INTERNAL:
#if defined(FEATURE_IP_CALL) && defined(FEATURE_CM_LTE)

      ll_as_id = cmph_get_as_id_from_ll_ind(cm_rpt_ptr->cmd.srvcc_handover_failure_ind.as_id);

      CM_MSG_HIGH_1_EXT("RXD: CM_MM_SRVCC_HANDOVER_FAILURE_IND, as_id=%d", ll_as_id, (ll_as_id+1));
          ...
          && cm_get_srvcc_ho_error_state() != CM_SRVCC_HO_UNMANAGED_CALL_COMBINATION  )
      {
        while(call_ptr != NULL)
        {
          ...
          {
            cmcall_end(call_ptr);
            CM_MSG_HIGH_0_EXT("CM_MM_SRVCC_HANDOVER_FAILURE_IND, cleared ext IMS single calls",(ll_as_id+1));
          }
          ...
          else if ( call_ptr->is_srvcc_call && cmwcall_is_int_ims())
          {
            ...
            {
              cmcall_end(call_ptr);
              CM_MSG_HIGH_0_EXT("CM_MM_SRVCC_HANDOVER_FAILURE_IND, cleared IMS CS participant calls",(ll_as_id+1));
            }
            /* single ims calls , Retain IMS connection id  */
            else
            {
              CM_MSG_HIGH_0_EXT("CM_MM_SRVCC_HANDOVER_FAILURE_IND, retain IMS conn id and remove GW conn id",
                                (ll_as_id+1));
              if (!cm_unreserve_connection_id(call_ptr->gw_connection_id, call_ptr->mm_id.asubs_id ))
              {
              ...
              if(call_ptr->call_state == CM_CALL_STATE_ORIG)
              {
                CM_MSG_HIGH_1("cmwcall_rpt_proc: setting sr orig_sent_on_mode to LTE for call %d",
                              call_ptr->call_id);
              ...
              if(call_ptr->ims_call_type == CM_CALL_TYPE_VT)
              {
                ...
                if(call_info_ptr == NULL)
                {
                  CM_MSG_HIGH_0_EXT("CM_MM_SRVCC_HANDOVER_FAILURE_IND, can not alloc memory for call info object",
                                    (ll_as_id+1));
                  break;
                }
                ...
                /* use call_info_ptr to update the log packet */
                (void) cmcall_info_get( call_ptr, call_info_ptr );
                cmlog_call_event_report_payload( CM_CALL_EVENT_HO_FAIL, call_info_ptr);
                cm_mem_free(call_info_ptr);
              }
            }
          }
        ...
        /* suppress HO_FAIL call event if rpt from NAS,
        ** HO_FAIL call event will be sent when receives CM_RRC_CODEC_INFO_IND
        */
        if(!cmwcall_is_int_ims() &&
            cm_rpt_ptr->hdr.cmd == CM_MM_SRVCC_HANDOVER_FAILURE_IND_INTERNAL)
        {
        ...
        else
        {
          ...
          ** HO_FAIL call event will be sent when receives CM_RRC_CODEC_INFO_IND
          */
          if(cm_rpt_ptr->hdr.cmd == CM_MM_SRVCC_HANDOVER_FAILURE_IND_INTERNAL)
          {
            cmwcall_send_srvcc_events_to_clients(CM_SRVCC_HO_IND_FAILURE,CM_CALL_ID_INVALID,ims_call_id, ll_as_id);
          }
    ...
    {
      call_info_ptr = cmcall_info_alloc();
      CM_MSG_HIGH_1("RXD: CM_CONNECTION_STATE_IND, channel_connected %d",
                    cm_rpt_ptr->cmd.cm_connection_state_info.channel_connected);

      CM_ASSERT( call_info_ptr != NULL );

      call_info_ptr->asubs_id = cm_rpt_ptr->cmd.cm_connection_state_info.as_id;
      if (cm_rpt_ptr->cmd.cm_connection_state_info.channel_connected == TRUE)
      {
        call_info_ptr->voice_call_reestab_status.status = CM_VOICE_CALL_REESTAB_SUCCESS;
      }
      else
      {
        call_info_ptr->voice_call_reestab_status.status = CM_VOICE_CALL_REESTAB_STARTED;
      }

      cmclient_list_call_event_ntfy( CM_CALL_EVENT_VOICE_CALL_REESTABLISHMENT, call_info_ptr );
    ...
    {
      ph_ptr->device_prop.is_lte_suspend_cnf_received = TRUE;
      cmcall_obj_iterator_init(&call_itr);
      call_ptr = cmcall_obj_get_next(&call_itr);

      while(call_ptr != NULL)
      {
        /* Send call orig after receiving LTE suspend cnf */
        if(call_ptr->mm_id.asubs_id != cm_rpt_ptr->cmd.lte_suspend_cnf.as_id &&
            call_ptr->direction == CM_CALL_DIRECTION_MO &&
            call_ptr->is_waiting_on_els_rsp == TRUE)
        {
          call_ptr->is_waiting_on_els_rsp = FALSE;
    ...
    case CM_END_PRE_ALERT_MT_VOLTE_CALL:
    {
      /*If the MT INVITE couldnt get connected/alerted within 7 (default) seconds,
             then end VoLTE call and allow MT CS Page */
#if defined (FEATURE_IP_CALL) && defined (FEATURE_CM_LTE)
      {
        cmph_sub_info_s_type *ph_sub_config_ptr = cmph_get_sub_ptr(cm_rpt_ptr->asubs_id);
        if(ph_sub_config_ptr != NULL &&
            ph_sub_config_ptr->mt_cs_page_reject_param.allow_cs_page_during_mt_volte_call &&
            (cmcall_is_present_ip_call () != CM_CALL_ID_INVALID) &&
            (ph_sub_config_ptr->mt_cs_page_reject_uptime <= time_get_uptime_secs()))
        {
          rpt_mm_id.asubs_id = cm_rpt_ptr->asubs_id;
          rpt_mm_id.stk_id   = MM_STACK_0;
          cmcall_end_pre_alert_mt_volte_call(rpt_mm_id);
          CM_MSG_HIGH_1("mt_cs_page_reject timer expire for sub[%d],end MT VoLTE call",
                        cm_rpt_ptr->asubs_id);
        }
      }
#endif
...
)
{
  ...
  */
  if(cmcall_is_no_call_in_gw_ps_per_subs(asubs_id) == FALSE )
  {
    ...
    */
    ...
    cmcall_invalidate_all_nsapi(asubs_id);
#endif

    /* Set the Subscriptio based on the SS on which we are sending pdp abort
    */
    sm_ptr->cmd.pdp_abort_req.as_id = asubs_id;
