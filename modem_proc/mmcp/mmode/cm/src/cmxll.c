...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/mmode/cm/src/cmxll.c#1 $
...
)
{
  ...
  if(!cmtask_is_valid_report((int)cmd_ptr->hdr.cmd))
  {
    sys_err_fatal_invalid_value_exception((int)cmd_ptr->hdr.cmd);
  }
...
{
  cm_mt_dbm_msg_extn(dbm_burst_type, format, data, data_len, SYS_MODEM_AS_ID_1);
} /* cm_mt_dbm_msg() */

/*===========================================================================

FUNCTION cm_mt_dbm_msg_extn
...
#ifdef FEATURE_CDSMS
      cm_mt_sms_msg_extn(format, data_len, data, asubs_id);
#else  /* !FEATURE_CDSMS */
...
{
  cm_mt_sms_msg_extn( format, len, data, SYS_MODEM_AS_ID_1);
} /* cm_mt_sms_msg() */

/*===========================================================================

FUNCTION cm_mt_sms_msg_extn
...
{
  cmxsms_mt_msg(format, len, data, asubs_id);
}
...
SIDE EFFECTS
  None
==========================================================================*/
void cm_send_hdr_unlock_rf_cmd(sys_modem_as_id_e_type          asubs_id)
{
  ...
  CM_MSG_HIGH_1("CM->MSGR: sub %d, send HDR_CP_UNLOCK_HDR_RF_REQ", asubs_id);
  (void) cm_msgr_send( HDR_CP_UNLOCK_HDR_RF_REQ, MSGR_MM_CM,
...
void cm_send_hdr_abort_resel_cmd(sys_modem_as_id_e_type         asubs_id)
{
  ...
  CM_MSG_HIGH_1("CM->MSGR: sub %d, send HDR_CP_CM_ABORT_LTE_RESEL_REQ", asubs_id);
  (void) cm_msgr_send( HDR_CP_CM_ABORT_LTE_RESEL_REQ, MSGR_MM_CM,
                       &(hdrcp_req->hdr), sizeof(hdrcp_msg_req_u),
                       asubs_id);
...
{
  cm_mc_unlock_resources_extn(SYS_MODEM_AS_ID_1);
}

/*========================================================================

FUNCTION cm_mc_unlock_resources_extn
...
SIDE EFFECTS
  None
==========================================================================*/
void cm_mc_unlock_resources_extn(
  sys_modem_as_id_e_type  asubs_id
)
{
#ifdef FEATURE_HDR_HYBRID
  ...
  */
  if( cmph_ptr()->device_prop.trm_cap.is_svdo_supported ||
      !cm_get_cur_hybr_status1(asubs_id)
    )
  {
    return;
  }
  ...
  */
  if(cmxsms_is_mo_sms_in_progress(asubs_id) ||
...
/*=========================================================================
FUNCTION
  cm_mc_get_sub_diff_action

DESCRIPTION
  This function should be called by 1x upon receiveing a cmd from CM with a sub_id that does
  not match the current 1x sub (sub which MMOC has told 1x is the 1x sub). This function will
  return the action that 1x should take with the cmd. The current return options are to handle
  the cmd, ignore the cmd, or unexpected which means the device will end up crashing in 1x.
...
=========================================================================*/
cm_mc_sub_diff_action_e_type cm_mc_get_sub_diff_action(
  cm_mc_sub_diff_action_s_type  sub_diff_info
)
{
  cm_mc_sub_diff_action_e_type action = MC_UNEXPECTED_CMD;

  switch(sub_diff_info.cmd)
  {
    case MC_DDTM_CNTL_F:
    case MC_BCSMS_ENABLE_F:
    case MC_DEREGISTER_F:
    case MC_END_F:
      action = MC_HANDLE_CMD;
      break;
