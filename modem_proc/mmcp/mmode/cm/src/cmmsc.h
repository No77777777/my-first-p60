...
$Header: //components/rel/mmcp.mpss/7.9.0/mmode/cm/src/cmmsc.h#1 $
...
#define MSC_STK_ID_TO_MM_STK_ID(id)  \
     ((id == 2)?MM_STACK_2:((id == 1)?MM_STACK_1:MM_STACK_0))

#define MM_STK_ID_TO_MSC_STK_ID(id)  \
     ((id == MM_STACK_2)?2:((id == MM_STACK_1)?1:0))
...
typedef struct
{
  ...
  */
  ...
  /**< Indicate the current mode preference term -one call, power cycle, etc. */
...
typedef enum
{
  ...
  /* Used when ss = max but feature mode is not svlte */
...
typedef struct cmmsc_3gpp2_stack_common_info_s
{
...
typedef struct cmmsc_stack_common_info_s
{
  ...
  sys_sys_mode_mask_e_type        ue_mode_capability_from_pm;
  sys_srv_domain_e_type           ue_domain_capability;
  ...
  cmmsc_3gpp2_stack_info_s_type   gpp2_info;

  /* This variable indicates if the CMMSC is currently functioning in emergency call */
  ...
  /* TRUE when UI volte setting or IMS permenent permenent failure for LTE only capable 
  ** non DDS sub.
  */

  boolean                         non_dds_lte_removal;
  /* TRUE when IMS reg fail or vops off on non DDS sub and EFS is configured */
...
typedef struct cmmsc_stack_s
{
  ...
  /* Having service / no service. In a sense, state is like a sub-status */
...
/* State machine: contains multiple stacks */
typedef struct cmmsc_state_machine_s
{
  ...
  /* Previous state-machine operation mode */
...
(
  mm_sub_stk_id_s_type           mm_id,
  /**< multimode id struct containing sub id and stack id.
  */
...
===========================================================================*/
sys_modem_as_id_e_type cmmsc_get_1x_sub(void);
...
sys_modem_as_id_e_type cmmsc_get_1x_capable_sub(void);
...
DESCRIPTION
 indicates whether the SUB has given RAT enabled
...
cmmsc_routing_table_s_type *cmmsc_route_table_ptr(void);
