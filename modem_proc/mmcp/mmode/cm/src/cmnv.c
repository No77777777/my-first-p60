...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/mmode/cm/src/cmnv.c#1 $
...
)
{
  ...
  } cmnv_efs_id_to_path_mapping_tbl[] =
  {
    ...
    { CMNV_EFS_ID_LTE_RPM_INFO,     "/nv/item_files/modem/mmode/lte_rpm_info"}
  };
  ...
  if( cmnv_cmd_sync.status != NV_DONE_S )
  {
    ...
    switch( cmnv_cmd_sync.item )
    {
      ...
      case NV_RTRE_CONFIG_I:
        data_ptr->rtre_config = NV_RTRE_CONFIG_RUIM_ONLY;
        break;
      ...
      case NV_DUAL_STANDBY_CFG_ITEMS_I:
        ...
        data_ptr->dual_standby_cfg_items.dual_standby_pref = SYS_MODEM_DS_PREF_SINGLE_STANDBY;
        break;
...
===========================================================================*/
boolean cmnv_get_uim_spc_status(sys_modem_as_id_e_type as_id)
{
  ...
  if (cmph_is_msim())
  {
    session_type = AS_ID_TO_1X_SESSION(as_id);
  }
...
)
{
  ...
  */
  ...
  if( cmnv_cmd_sync.status != NV_DONE_S )
  {
    CM_ERR_2( "Failed sending the CONFIG Command to NV %d sub%d",
              cmnv_cmd_sync.status, context);
  }
