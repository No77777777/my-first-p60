...
boolean
===========================================================================*/
static boolean cmipsups_is_ussd_sups_rpt(
  cm_name_type   rpt
)
{
...
)
{
  ...
  hdr_mm_id.asubs_id = sub_id;
  hdr_mm_id.stk_id = MM_STACK_1;
  cmss_get_service_status_per_stack(hdr_mm_id, &stack1_srv_info);

  CM_MSG_MED_2("cmipsups_route: cmd=%d, sups domain pref=%d",sups_cmd,cmsups_sub_info_ptr->sups_svc_domain_pref);
  ...
  if(cmipsups_is_ussd_sups_cmd(sups_cmd))
  {
    ...
    else if(cmipapp_find_ussd_support_app(sub_id) == CMIPAPP_ID_INVALID)
    {
      /* This API should cover all VoLTE (both Normal/Emergency) Calls but not VoWLAN Emergency Call
      ** During VoWLAN (both Normal/Emergency) Call, USSD will always be attempted either
      ** on IMS or CS i.e. not rejected here
      */
  ...
  {
    boolean is_ics_support = (cmipapp_find_ics_support_app(sub_id) != CMIPAPP_ID_INVALID);
    ...
    if(sups_cmd == CM_SUPS_CMD_GET_PASSWORD_RES )
       
    {
      is_sups_cmd_supported_by_ims = FALSE;
    }

    switch(cmsups_sub_info_ptr->sups_svc_domain_pref)
    {
      ...
      case SYS_SUPP_SRV_DOMAIN_PREF_AUTO:
        ...
        if(
          (
          #ifdef FEATURE_LTE
            (sds_sub_prop_ptr->gen_sub_info.curr_voice_dom_selected == SYS_CM_DOMAIN_SEL_DOMAIN_IMS ) ||
            (cmipapp_find_active_app(SYS_SYS_MODE_WLAN, CM_CALL_TYPE_VOICE,sub_id) != CMIPAPP_ID_INVALID) ||
          #endif
             ((cmipapp_find_active_app(SYS_SYS_MODE_HDR, CM_CALL_TYPE_SUPS,sub_id) != CMIPAPP_ID_INVALID) &&
              (stack1_srv_info.srv_status == SYS_SRV_STATUS_SRV) &&
              (sub_id == ph_ptr->device_prop.msim_prop.default_data_subs)
             )
           ) &&
...
static void cmipsups_common_cmd_proc(
...
)
{
  ...
  cmipsups_password_change_ss_req_s_type        *ip_password_change_cmd_ptr;
  ...
  switch(in_sups_cmd_ptr->cmd)
  {
    ...
    case CM_SUPS_CMD_REGISTER:
    {
      ...
      if(in_sups_cmd_info_ptr->fwd_type == CM_FORWARD_TO_NUMBER)
      
      {
        ip_register_cmd_ptr->forwarded_to_number = in_sups_cmd_info_ptr->forwarded_to_number;
      }
      if(in_sups_cmd_info_ptr->fwd_type == CM_FORWARD_TO_URI){
        ip_register_cmd_ptr->fwd_type = CM_FORWARD_TO_URI;
        ip_register_cmd_ptr->forwarded_to_uri = in_sups_cmd_info_ptr->forwarded_to_uri;
      }
	...
	case CM_SUPS_CMD_REG_PASSWORD:
      ip_password_change_cmd_ptr = (cmipsups_password_change_ss_req_s_type *)cm_mem_malloc(sizeof(cmipsups_password_change_ss_req_s_type));
      ip_password_change_cmd_ptr->ss_new_password = in_sups_cmd_info_ptr->ss_new_password;
      ip_password_change_cmd_ptr->ss_new_password_again = in_sups_cmd_info_ptr->ss_new_password_again;
      ip_cmd_ptr = (cmipsups_cmd_s_type *)ip_password_change_cmd_ptr;
	break;
  ...
  ip_cmd_ptr->ss_password = in_sups_cmd_info_ptr->ss_password;
  
  ip_cmd_ptr->is_all_call_fwd_sups_ext_req = in_sups_cmd_info_ptr->is_all_call_fwd_sups_ext_req;
  ...
  cmsups_cmd_copy_fields(in_sups_cmd_ptr);
  return;
}
...
)
{
  ...
  switch (ss_info->info_type)
  {
    case FWD_INFO:
      ...
      if(sups_conf_ptr->forwarding_info.forwarding_feature_list.forwarding_feature_list_length != 0)
      {
        for(i = 0;
            i < sups_conf_ptr->forwarding_info.forwarding_feature_list.forwarding_feature_list_length;
            i++)
        {
          ...
          if(ss_info->info.forwarding_info.forwarding_feature_list.forwarding_feature[i].forwarded_to_uri != NULL)
          {
            sups_conf_ptr->fwd_feature_list.forwarding_feature[i].forwarded_to_uri =ss_info->info.forwarding_info.forwarding_feature_list.forwarding_feature[i].forwarded_to_uri;
            CM_MSG_MED_2("forward to URI: length: %d, URI: %x", sups_conf_ptr->fwd_feature_list.forwarding_feature[i].forwarded_to_uri->fwd_to_uri_length, sups_conf_ptr->fwd_feature_list.forwarding_feature[i].forwarded_to_uri);
          }
    ...
    case CALL_BARRING_INFO:
      ...
      break;
    case NEW_PWD_INFO:
		sups_conf_ptr->ss_new_password = ss_info->info.ss_new_password;
		break;
...
static void cmipsups_copy_ext_resp_info(const cmipsups_common_ss_conf_type   *common_cnf, cm_sups_info_s_type *sups_info_ptr )
{
  /* Null check sups_conf_ptr */
  if(sups_info_ptr == NULL)
  {
    CM_ERR_0("cmipsups_copy_ext_resp_info NULL");
    return;
  }

  sups_info_ptr->is_ext_resp = common_cnf->is_ext_resp;
  sups_info_ptr->ext_resp    = common_cnf->ext_resp;

  CM_MSG_HIGH_1("cmipsups_copy_ext_resp_info:resp_type %d",
              common_cnf->ext_resp.resp_type);

  #if defined (FEATURE_IP_CALL) && defined (FEATURE_CM_LTE)
  if(common_cnf->ext_resp.resp_type == ALL_CALL_FWD_INFO)
  {
    CM_MSG_HIGH_4("cmipsups_copy_ext_resp_info:cfu_success %d cfb_sccess %d cfnry_success %d cfnrc_success %d",
                  common_cnf->ext_resp.info.all_call_fwd_ext_resp.cfu_success,
                  common_cnf->ext_resp.info.all_call_fwd_ext_resp.cfb_success,
                  common_cnf->ext_resp.info.all_call_fwd_ext_resp.cfnry_success,
                  common_cnf->ext_resp.info.all_call_fwd_ext_resp.cfnrc_success);

    if(common_cnf->ext_resp.info.all_call_fwd_ext_resp.cfu_success == FALSE)
    {
      CM_MSG_HIGH_4("cmipsups_copy_ext_resp_info: cfu_retry_duration %d error_code %d error_code_tag %d present %d",
                    common_cnf->ext_resp.info.all_call_fwd_ext_resp.cfu_retry_duration,
                    common_cnf->ext_resp.info.all_call_fwd_ext_resp.cfu_error.error_code,
                    common_cnf->ext_resp.info.all_call_fwd_ext_resp.cfu_error.error_code_tag,
                    common_cnf->ext_resp.info.all_call_fwd_ext_resp.cfu_error.present);
      
      if (common_cnf->ext_resp.info.all_call_fwd_ext_resp.cfu_sups_ss_error_text != NULL)
      {
        CM_MSG_HIGH_1("cmipsups_copy_ext_resp_info:cfu_sups_ss_error_text Length %d", 
                       common_cnf->ext_resp.info.all_call_fwd_ext_resp.cfu_sups_ss_error_text->err_text_length);
      }
    }


    if(common_cnf->ext_resp.info.all_call_fwd_ext_resp.cfb_success == FALSE)
    {
      CM_MSG_HIGH_4("cmipsups_copy_ext_resp_info: cfb_retry_duration %d error_code %d error_code_tag %d present %d",
                    common_cnf->ext_resp.info.all_call_fwd_ext_resp.cfb_retry_duration,
                    common_cnf->ext_resp.info.all_call_fwd_ext_resp.cfb_error.error_code,
                    common_cnf->ext_resp.info.all_call_fwd_ext_resp.cfb_error.error_code_tag,
                    common_cnf->ext_resp.info.all_call_fwd_ext_resp.cfb_error.present);
      
      if (common_cnf->ext_resp.info.all_call_fwd_ext_resp.cfb_sups_ss_error_text != NULL)
      {
          CM_MSG_HIGH_1("cmipsups_copy_ext_resp_info:cfb_sups_ss_error_text Length %d", 
                         common_cnf->ext_resp.info.all_call_fwd_ext_resp.cfb_sups_ss_error_text->err_text_length);
      }
    }


    if(common_cnf->ext_resp.info.all_call_fwd_ext_resp.cfnry_success == FALSE)
    {
      CM_MSG_HIGH_4("cmipsups_copy_ext_resp_info: cfnry_retry_duration %d error_code %d error_code_tag %d present %d",
                    common_cnf->ext_resp.info.all_call_fwd_ext_resp.cfu_retry_duration,
                    common_cnf->ext_resp.info.all_call_fwd_ext_resp.cfnry_error.error_code,
                    common_cnf->ext_resp.info.all_call_fwd_ext_resp.cfnry_error.error_code_tag,
                    common_cnf->ext_resp.info.all_call_fwd_ext_resp.cfnry_error.present);
      
      if (common_cnf->ext_resp.info.all_call_fwd_ext_resp.cfnry_sups_ss_error_text != NULL)
      {
          CM_MSG_HIGH_1("cmipsups_copy_ext_resp_info:cfnry_sups_ss_error_text Length %d", 
                         common_cnf->ext_resp.info.all_call_fwd_ext_resp.cfnry_sups_ss_error_text->err_text_length);
      }
    }

    if(common_cnf->ext_resp.info.all_call_fwd_ext_resp.cfnrc_success == FALSE)
    {
      CM_MSG_HIGH_4("cmipsups_copy_ext_resp_info: cfnrc_retry_duration %d error_code %d error_code_tag %d present %d",
                    common_cnf->ext_resp.info.all_call_fwd_ext_resp.cfnrc_retry_duration,
                    common_cnf->ext_resp.info.all_call_fwd_ext_resp.cfnrc_error.error_code,
                    common_cnf->ext_resp.info.all_call_fwd_ext_resp.cfnrc_error.error_code_tag,
                    common_cnf->ext_resp.info.all_call_fwd_ext_resp.cfnrc_error.present);
      
      if (common_cnf->ext_resp.info.all_call_fwd_ext_resp.cfnrc_sups_ss_error_text != NULL)
      {
          CM_MSG_HIGH_1("cmipsups_copy_ext_resp_info:cfnrc_sups_ss_error_text Length %d", 
                         common_cnf->ext_resp.info.all_call_fwd_ext_resp.cfnrc_sups_ss_error_text->err_text_length);
      }
    }
...
)
{
  ...
  {
    CM_ERR_1("cmipsups_common_conf_proc: sups_ptr->sub_info[i] NULL ptr for asubs_id %d",
             asubs_id);
    return;
  }
  ...
  cmsups_update_asubs_id(asubs_id, &sups_info_ptr);
#endif
  ...
  CM_MSG_HIGH_6("cmipsups_common_conf_proc: retry duration: %d , error code: %d, error tag : %d asub_id %d is_ext_resp = %d err_present %d",
                sups_info_ptr->retry_duration,
                common_cnf->sups_ss_info.info.cm_ss_error.error_code,
                common_cnf->sups_ss_info.info.cm_ss_error.error_code_tag,
                sups_info_ptr->asubs_id,
                common_cnf->is_ext_resp,
                common_cnf->sups_ss_info.info.cm_ss_error.present);

  if(common_cnf->is_ext_resp)
  {
    cmipsups_copy_ext_resp_info(common_cnf, sups_info_ptr);
  }
...
)
{
  ...
  if(cmsups_sub_info_ptr == NULL)
  {
    CM_ERR_1("cmipsups_interrogate_conf_proc: sups_ptr->sub_info[i] NULL ptr for asubs_id %d",
             asubs_id);
    return;
  }
  ...
  CM_MSG_HIGH_3("cmipsups_interrogate_conf_proc: retry duration: %d , error code: %d, error tag : %d",
                interrogate_cnf->retry_duration,
                interrogate_cnf->interrogate_ss_res.cm_ss_error.error_code,
                interrogate_cnf->interrogate_ss_res.cm_ss_error.error_code_tag);
  ...
  if(interrogate_cnf->success)
  {
    ...
    switch( sups_info_ptr->conf_type )
    {
      ...
      case FWD_FEATURE_LIST_INFO:
        sups_info_ptr->fwd_feature_list = interr_res_ptr->forwarding_feature_list;
        if(sups_info_ptr->fwd_feature_list.forwarding_feature_list_length != 0)
        {
          for(i = 0; i < sups_info_ptr->fwd_feature_list.forwarding_feature_list_length; i++)
          {
            ...
            if(interr_res_ptr->forwarding_feature_list.forwarding_feature[i].forwarded_to_uri != NULL)
            {
              sups_info_ptr->fwd_feature_list.forwarding_feature[i].forwarded_to_uri = interr_res_ptr->forwarding_feature_list.forwarding_feature[i].forwarded_to_uri;
              CM_MSG_MED_2("forward to URI: length: %d, URI: %x", sups_info_ptr->fwd_feature_list.forwarding_feature[i].forwarded_to_uri->fwd_to_uri_length, sups_info_ptr->fwd_feature_list.forwarding_feature[i].forwarded_to_uri);
            }
...
)
{
  ...
  if(cmsups_sub_info_ptr == NULL)
  {
    CM_ERR_1("cmipsups_unstructured_ss_ind_proc: sups_ptr->sub_info[i] NULL ptr for asubs_id %d",
             asubs_id);
    return;
  }
...
)
{
  ...
  if(cmsups_sub_info_ptr == NULL)
  {
    CM_ERR_1("cmipsups_process_unstructured_ss_conf: sups_ptr->sub_info[i] NULL ptr for asubs_id %d",
             asubs_id);
    return;
  }
  ...
  if(sups_info_ptr == NULL)
  {
    CM_ERR_1("cmipsups_process_unstructured_ss_conf: sups_info_ptr NULL ptr for asubs_id %d",
             asubs_id);
    return;
  }
  ...
  if(process_unstructured_ss_conf->success)
  {
    ...
    switch( sups_info_ptr->conf_type )
    {
      ...
      /* USS data */
      case USS_DATA_INFO:
        sups_info_ptr->uss_data = ss_conf_res_ptr->data.uss_data;
        break;

      /* SS error */
      case SS_ERROR_INFO:
        sups_info_ptr->ss_error = ss_conf_res_ptr->data.cm_ss_error;
        break;

      /* Cause */
      case CC_CAUSE_INFO:
        sups_info_ptr->cc_cause = ss_conf_res_ptr->data.cm_cc_cause;
        break;
...
)
{
  ...
  if(cmsups_sub_info_ptr == NULL)
  {
    CM_ERR_1("cmipsups_rel_unstructured_ss_ind: sups_ptr->sub_info[i] NULL ptr for asubs_id %d",
             asubs_id);
    return;
  }
...
===========================================================================*/
boolean  cmipsups_client_cmd_forward_ip(
...
)
{
  ...
  {
    app_id = cmipapp_find_ussd_support_app(in_sups_cmd_info_ptr->cmd_subs_id);
  ...
  {
    app_id = cmipapp_find_ics_support_app(in_sups_cmd_info_ptr->cmd_subs_id);
    if(app_id == CMIPAPP_ID_INVALID)
    {
  ...
  switch( in_sups_cmd_ptr->cmd )
  {
    ...
    case CM_SUPS_CMD_REG_PASSWORD:
      ...
      #if (defined FEATURE_SUPS_RETRY && defined CM_GW_SUPPORTED)
        sups_info_list = cmwsups_info_list_find(in_sups_cmd_info_ptr->cmd_subs_id, in_sups_cmd_info_ptr->ss_ref);
        if(sups_info_list != NULL)
          sups_info_list->sups_cmd_route = CMSUPS_CMD_ALLOW_IMS;
      #endif
      break;
  ...
  {
    cmsups_event(in_sups_cmd_info_ptr->cmd_subs_id, sups_event_type);
  }
      ...
      in_sups_cmd_ptr->cmd != CM_SUPS_CMD_INTERROGATE)
  {
    ...
    CM_MSG_HIGH_1("Released ref for in_sups_cmd_info_ptr->call_barring_num_list cmd: %d",
                  in_sups_cmd_ptr->cmd);
  }
  if(in_sups_cmd_info_ptr->fwd_type == CM_FORWARD_TO_URI && in_sups_cmd_info_ptr->forwarded_to_uri != NULL)
  {
    ref_cnt_obj_release(in_sups_cmd_info_ptr->forwarded_to_uri);
    CM_MSG_HIGH_1("Released ref for in_sups_cmd_info_ptr->fwd_to_uri cmd: %d",
                  in_sups_cmd_ptr->cmd);
  }
...
)
{
  ...
  switch(cmd)
  {
    ...
    case CM_IP_SUPS_PASSWORD_CHG_CONF:
      ss_req_cmd = CM_SUPS_CMD_REG_PASSWORD;
      break;
...
===========================================================================*/
void cmipsups_reset_sups_info_error(
  cm_sups_info_s_type   *sups_info_ptr
)
{
  if(sups_info_ptr == NULL)
...
FUNCTION cmipsups_reset_ext_ip_sups_resp
...
===========================================================================*/
void cmipsups_reset_ext_ip_sups_resp(
  cm_sups_info_s_type   *sups_info_ptr
)
{
  if(sups_info_ptr == NULL)
  ...
  CM_MSG_MED_0("resetting ext ip sups response ");
  sups_info_ptr->is_ext_resp = FALSE;
  memset(&sups_info_ptr->ext_resp, 0, sizeof(cmipsups_ext_resp_s_type));
...
static void cmipsups_convert_to_ue_based_cw(const cmipapp_rpt_s_type *cmipapp_sups_rpt_ptr)
{
  ...
  memset(&sups_cmd_info, 0, sizeof(cm_sups_cmd_info_s_type));
  ...
  switch(cmipapp_sups_rpt_ptr->hdr.cmd)
  {
    ...
    case CM_IP_SUPS_INTERROGATE_SS_CONF:
    {
      ...
      /* Both BSG list and cm_ss_status not present*/
      else
      {
        sups_cmd_info.ss_ref = cmipapp_sups_rpt_ptr->ip_ind.ind.interrogate_ss_conf.cm_ss_ref;
        sups_cmd_info.basic_service.present  = cmipapp_sups_rpt_ptr->ip_ind.ind.interrogate_ss_conf.interrogate_ss_res.basic_service_group_list.present;
        sups_cmd_info.ss_code = cw;
        cmsups_create_interrogate_cnf_internal(&sups_cmd_info,cw);
      }
...
)
{
  ...
  app_id = cmipapp_find_ussd_support_app(asubs_id);
  if(app_id != CMIPAPP_ID_INVALID)
  {
...
)
{
  ...
  uss_info_ptr        = cmsups_get_uss_info_ptr(asubs_id);
#endif


#if (defined(FEATURE_GSM) || defined(FEATURE_WCDMA))
  if (uss_info_ptr != NULL && uss_info_ptr->is_fallback_to_cs == TRUE &&
           cmipapp_sups_rpt_ptr->hdr.cmd   == CM_IP_SUPS_RELEASE_UNSTRUCTURED_SS_IND)
  {
    if(cmsups_get_mt_uss_routing_context_info( asubs_id, &sups_route) == TRUE && sups_route == CMSUPS_CMD_ALLOW_IMS )
    {
      cmsups_reset_mt_uss_context_info( asubs_id);
    }
  ...
  {
    case CM_IP_SUPS_ACTIVATE_SS_CONF:
      CM_MSG_MED_5("act_cnf, type %d, err_present %d, tag %d, code %d sub_id %d",
    ...
    case CM_IP_SUPS_DEACTIVATE_SS_CONF:
      CM_MSG_MED_5("deact_cnf, type %d, err_present %d, tag %d, code %d sub_id %d",
    ...
    case CM_IP_SUPS_REGISTER_SS_CONF:
     ...
     case CM_IP_SUPS_PASSWORD_CHG_CONF:
       cmipsups_common_conf_proc(&cmipapp_sups_rpt_ptr->ip_ind.ind.password_change_ss_conf_type,
                                asubs_id);
      break;
     
		
    case CM_IP_SUPS_INTERROGATE_SS_CONF:
      CM_MSG_MED_5("intg_cnf, type %d, err_present %d, tag %d, code %d sub_id %d",
...
#if (defined(FEATURE_GSM) || defined(FEATURE_WCDMA))
  if(cmsups_sub_info_ptr != NULL && sups_info_ptr != NULL)
  {
    if(  ((sups_info_ptr->sups_err_code == CM_IPSUPS_FAILURE_CAUSE_PDP_FAILURE
          ...
          && ((cmsups_sub_info_ptr->sups_svc_domain_pref == SYS_SUPP_SRV_DOMAIN_PREF_AUTO) ||
              (cmsups_sub_info_ptr->sups_svc_domain_pref == SYS_SUPP_SRV_DOMAIN_PREF_PS_PREF)) )
         ||
         (sups_info_ptr->sups_err_code == CM_IPSUPS_FAILURE_CAUSE_USS_FALLBACK_TO_CS
          ...
          && cmipsups_is_ussd_sups_rpt(cmipapp_sups_rpt_ptr->hdr.cmd)))
         &&
         (cmcall_is_there_volte_call_sub(asubs_id) == CM_CALL_ID_INVALID)
      )
    {
	 ...
	 || (cmregprx_get_state(cmph_map_sub_stk_to_id(asubs_id, MM_STACK_0))
          ...
          (cmregprx_get_substate(cmph_map_sub_stk_to_id(asubs_id,MM_STACK_0))
           == CMREGPRX_SUBSTATE_WAIT_STOP_CNF) ||
          ...
          cmcall_is_lte_only_mode(cmph_map_sd_mode_pref_to_cm_mode_pref(comm_mode),\
            ...
            !cmsds_is_cs_domain_sups_allowed(asubs_id)
      ...
      {
        CM_MSG_HIGH_3("cmipsups: sups shouldnt be routed comm_mode=%d,comm_band=0x%08x %08x comm_lte_band=",\
                      comm_mode,QWORD_HIGH(comm_band), QWORD_LOW(comm_band));
        cm_print_lte_band_mask(comm_lte_band);
      }
      else
      {
        CM_MSG_MED_2("cmipsups: fallback reason: %d , sups_cnf: %d",
                     sups_info_ptr->sups_err_code,
                     cmipapp_sups_rpt_ptr->hdr.cmd);
        cmwsups_client_cmd_forward_gw(asubs_id,
                                      cmipsups_map_conf_with_request(cmipapp_sups_rpt_ptr->hdr.cmd),
                                      TRUE);
        if (uss_info_ptr != NULL) uss_info_ptr->is_fallback_to_cs = TRUE;
        /* Increase the pending sups count */
        cmsups_sub_info_ptr->num_pending_sups_cmds++;
        cmipsups_reset_sups_info_error(sups_info_ptr);
        cmipsups_reset_ext_ip_sups_resp(sups_info_ptr);
        return;
      }
    }
  }
  ...
  if (sups_info_ptr != NULL)
  {
    ...
    if ((sups_info_ptr->ss_code                    == (byte)allForwardingSS) &&
        (sups_info_ptr->is_ext_resp == TRUE) &&
        (sups_info_ptr->ext_resp.resp_type == ALL_CALL_FWD_INFO)
       )
    {
      if (sups_info_ptr->ext_resp.info.all_call_fwd_ext_resp.cfu_sups_ss_error_text != NULL)
      {
        sups_info_ptr->ext_resp.info.all_call_fwd_ext_resp.cfu_sups_ss_error_text = NULL;
      }

      if (sups_info_ptr->ext_resp.info.all_call_fwd_ext_resp.cfb_sups_ss_error_text != NULL)
      {
        sups_info_ptr->ext_resp.info.all_call_fwd_ext_resp.cfb_sups_ss_error_text = NULL;
      }

      if (sups_info_ptr->ext_resp.info.all_call_fwd_ext_resp.cfnry_sups_ss_error_text != NULL)
      {
        sups_info_ptr->ext_resp.info.all_call_fwd_ext_resp.cfnry_sups_ss_error_text = NULL;
      }

      if (sups_info_ptr->ext_resp.info.all_call_fwd_ext_resp.cfnrc_sups_ss_error_text != NULL)
      {
        sups_info_ptr->ext_resp.info.all_call_fwd_ext_resp.cfnrc_sups_ss_error_text = NULL;
      }

    }

    #ifdef FEATURE_SUPS_RETRY
    sups_list_info_ptr = cmwsups_info_list_find(asubs_id, sups_info_ptr->ss_ref);
    if( sups_list_info_ptr != NULL && cmsups_sub_info_ptr != NULL)
    {
      if(sups_list_info_ptr == cmsups_sub_info_ptr->sups_info_list)
      {
