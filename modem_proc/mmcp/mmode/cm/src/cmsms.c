...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/mmode/cm/src/cmsms.c#1 $
...
FUNCTION cmsms_sub_info_ptr

DESCRIPTION
  This function returns the pointer to cmsms sub info.
...
===========================================================================*/
cmsms_sub_info_s_type *cmsms_sub_info_ptr(
...
)
{
  ...
  if(!(asubs_id >= SYS_MODEM_AS_ID_1 &&
       asubs_id < ARR_SIZE(sms_ptr->sub_info) &&
       asubs_id < SYS_MODEM_AS_ID_NO_CHANGE))
  {
    CM_MSG_HIGH_1("cmsms get sub prop wrong sub %d", asubs_id);
    asubs_id = SYS_MODEM_AS_ID_1;
  }
  return sms_ptr->sub_info[asubs_id];
}
...
void cmsms_init( void )
{
  ...
  cmsms_sub_info_s_type              *sms_sub_info;
  ...
  */
  ...
  sms_ptr->count = num_sub;
  for(i=0; i<num_sub; i++)
  {
    if(sms_ptr->sub_info[i] == NULL)
    {
      sms_ptr->sub_info[i] = (cmsms_sub_info_s_type *)cm_mem_malloc
                             (sizeof(cmsms_sub_info_s_type));
    }
    sms_sub_info = sms_ptr->sub_info[i];
#ifdef FEATURE_CDSMS
    sms_sub_info->sms_state                  = CM_SMS_STATE_IDLE;
    sms_sub_info->sms_client_ptr             = CM_CLIENT_PTR_NONE;
    sms_sub_info->l2_ack_requested           = FALSE;
    sms_sub_info->awi_ack.seq_no             = 0;
    sms_sub_info->awi_ack.bf                 = 0;
    sms_sub_info->awi_ack.error_class        = 0;
    sms_sub_info->awi_ack.status             = 0;
#endif
    sms_sub_info->data                       = NULL;
    sms_sub_info->mo_status                  = CM_SMS_MO_NONE;
    sms_sub_info->mt_sms_ptr                 = NULL;
  }
...
/*lint -esym(818,cmd_ptr) */
{
  ...
  {
    asubs_id = sms_cmd_ptr->cmd_subs = SYS_MODEM_AS_ID_1;
  }

  ph_sub_ptr = cmph_get_sub_ptr(asubs_id);
  if(ph_sub_ptr == NULL)
  {
    CM_MSG_HIGH_1("sub %d ph_sub_ptr is NULL", asubs_id);
    return CM_SMS_CMD_ERR_OTHER;
  }

  sms_sub_info_ptr = cmsms_sub_info_ptr(asubs_id);
  if(sms_sub_info_ptr == NULL)
  {
    CM_MSG_HIGH_1( "sub %d sms_sub_info_ptr is NULL", asubs_id);
    return CM_SMS_CMD_ERR_OTHER;
  }
    ...
    case CM_SMS_CMD_MO_MSG:
      ...
      */
      ...
      CM_MSG_MED_2("Allow SMS in ECBM %d for sub %d",
                   ph_sub_ptr->emerg_call_prop.is_allow_sms_in_ecbm_enabled, asubs_id);
      /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
      ...
      ** acquisition which is ongoing on SD side as a part of BSR */
      ...
      if((cmd_info_ptr->sys_mode == SYS_SYS_MODE_CDMA ) &&
          (cmregprx_is_gwl_protocol_active(cmph_map_sub_stk_to_id(asubs_id, MM_STACK_0))== TRUE))
      {
        cmd_err = CM_SMS_CMD_ERR_MODE_PREF;
      ...
      if ((cmd_info_ptr->sys_mode != SYS_SYS_MODE_LTE) &&
          cmss_get_info_sys_mode(sms_cmd_ptr->cmd_subs, MM_STACK_0) == SYS_SYS_MODE_LTE)
      {
        cmd_err = CM_SMS_CMD_ERR_MODE_PREF;
      ...
      cmd_err = cmipsms_client_cmd_check( cmd_info_ptr, asubs_id );
      ...
      {
        cmd_err = CM_SMS_CMD_ERR_OTHER;
        return cmd_err;
      }
      else if( sms_cmd_ptr->client_ptr == CM_CLIENT_PTR_NONE )
      {
        /* Invalid client ID */
      ...
      /* If NV allow_sms_in_ecbm is enabled, SMS is allowed
          ...
          */
      else if((ph_sub_ptr->emerg_call_prop.is_allow_sms_in_ecbm_enabled == FALSE) &&
      ...
      else if( ph_sub_ptr->sub_3gpp2_pref.cdma_lock_mode == CM_CDMA_LOCK_MODE_ON )
      {
        /* CDMA lock is On */
...
#ifdef FEATURE_CDSMS
      else if (sms_sub_info_ptr->sms_state != CM_SMS_STATE_IDLE)
      {
      ...
      */
      ...
      else if( cm_is_jcdma_enable() && ( cmcall_count_on_ss(cmph_map_sub_stk_to_id(asubs_id, MM_STACK_1)) != 0 ) &&
               ( ( ph_sub_ptr->sub_3gpp2_pref.sub_ddtm_pref.cur_ddtm_act_mask & SYS_DDTM_ACT_MASK_SUPPRESS_MO_DBM ) ||
                 ( ( ph_sub_ptr->sub_3gpp2_pref.sub_ddtm_pref.ddtm_pref == CM_DDTM_PREF_ON )&&
                   ( ph_sub_ptr->sub_3gpp2_pref.sub_ddtm_pref.ddtm_act_mask & SYS_DDTM_ACT_MASK_SUPPRESS_MO_DBM )))
             )
      {
      ...
      else
      {
        ...
        sys_sys_mode_e_type sys_mode_stk0 = cmss_get_info_sys_mode(asubs_id, MM_STACK_0);
        sys_srv_status_e_type srv_status_stk1 = cmss_get_info_srv_status(asubs_id, MM_STACK_1);

        if( ( !cmss_is_stack1_operational(asubs_id) || (srv_status_stk1 != SYS_SRV_STATUS_SRV))
            &&
            (sys_mode_stk0 == SYS_SYS_MODE_NO_SRV))
        {
          cmd_err = CM_SMS_CMD_ERR_NO_SRV_S;
        }
#if defined (FEATURE_LTE_TO_1X) && defined (FEATURE_DOMAIN_SELECTION)
        else if ((sys_mode_stk0 == SYS_SYS_MODE_LTE) &&
                 ((cmsds_get_csfb_type_allowed(asubs_id) != CM_CALL_CSFB_TYPE_1XCSFB) ||
                  (sds_sub_prop_ptr != NULL &&
                   sds_sub_prop_ptr->gen_sub_config.is_sms_over_s102_supported == FALSE))
                )
        {
          cmd_err = CM_SMS_CMD_ERR_NO_SRV_S;
...
)
{
  ...
  return (cmxsms_client_cmd_forward_mc( sms_cmd_ptr ));
...
static boolean cmsms_info_get(
  ...
  const cmsms_sub_info_s_type     *sms_sub_info_ptr
)
{
  ...
  */
  CM_ASSERT( CM_INIT_CHECK(cmsms_ptr()->init_mask) );
  ...
  */
#ifdef FEATURE_CDSMS
  sms_info_ptr->sms_state             = sms_sub_info_ptr->sms_state;
  sms_info_ptr->sms_client_id         = cmclient_get_client_id (sms_sub_info_ptr->sms_client_ptr );
  sms_info_ptr->awi_ack.bf            = sms_sub_info_ptr->awi_ack.bf;
  sms_info_ptr->awi_ack.error_class   = sms_sub_info_ptr->awi_ack.error_class;
  sms_info_ptr->awi_ack.seq_no        = sms_sub_info_ptr->awi_ack.seq_no;
  sms_info_ptr->awi_ack.status        = sms_sub_info_ptr->awi_ack.status;
  sms_info_ptr->l2_ack_requested      = sms_sub_info_ptr->l2_ack_requested;
#endif
  sms_info_ptr->mo_status             = sms_sub_info_ptr->mo_status;

  if( sms_sub_info_ptr->mt_sms_ptr != NULL )
  {
    sms_info_ptr->mt_sms_format     = (uint8)sms_sub_info_ptr->mt_sms_ptr->sms.format;
    sms_info_ptr->length            = sms_sub_info_ptr->mt_sms_ptr->sms.data_len;
    sms_info_ptr->data              = sms_sub_info_ptr->mt_sms_ptr->sms.data;
    sms_info_ptr->sys_mode          = sms_sub_info_ptr->mt_sms_ptr->sys_mode;
  }
  ...
  */
  return CM_BOOLEAN( CM_INIT_CHECK(cmsms_ptr()->init_mask) );
}
...
/*lint -esym(715,sms_ptr)*/
{
  ...
  {
    cmsms_sms_data_buffer_free(sms_sub_info_ptr->data);
    sms_sub_info_ptr->data = NULL;
  }
  ...
  */
  (void) cmsms_info_get( sms_info_ptr, sms_sub_info_ptr );
  sms_info_ptr->asubs_id = asubs_id;
  ...
  */
  ...
  switch( sms_cmd_ptr->cmd )
  {
      ...
      switch( cmd_info_ptr->sys_mode)
      {
        ...
        case SYS_SYS_MODE_CDMA:
          ...
          */
          ...
          sms_sub_info_ptr->sms_client_ptr     = sms_cmd_ptr->client_ptr;
          sms_sub_info_ptr->sms_state          = CM_SMS_STATE_HOLD_ORIG_RETRY;
          sms_sub_info_ptr->data               = cmd_info_ptr->data;
          sms_sub_info_ptr->l2_ack_requested   = cmd_info_ptr->l2_ack_requested;
          sms_sub_info_ptr->is_ok_to_send      = TRUE;


          cmxsms_mo_msg_start( sms_sub_info_ptr, sms_cmd_ptr->cmd_subs);
        ...
        case SYS_SYS_MODE_HDR:
          ...
          */
          cmsms_event( sms_sub_info_ptr, CM_SMS_EVENT_MO_MSG, sms_cmd_ptr->cmd_subs);
    ...
    case CM_SMS_CMD_AWI_ACK:
      ...
      /* update SMS state and copy relevant info to SMS object */
      sms_sub_info_ptr->sms_client_ptr         = sms_cmd_ptr->client_ptr;
      sms_sub_info_ptr->awi_ack.bf             = cmd_info_ptr->awi_ack.bf;
      sms_sub_info_ptr->awi_ack.error_class    = cmd_info_ptr->awi_ack.error_class;
      sms_sub_info_ptr->awi_ack.status         = cmd_info_ptr->awi_ack.status;
      sms_sub_info_ptr->awi_ack.seq_no         = cmd_info_ptr->awi_ack.seq_no;

      cmsms_event( sms_sub_info_ptr, CM_SMS_EVENT_AWI_ACK, sms_cmd_ptr->cmd_subs);
    ...
    case CM_SMS_CMD_BC_ENABLE:
      ...
      /* update SMS state and copy relevant info to SMS object */
      sms_sub_info_ptr->sms_client_ptr         = sms_cmd_ptr->client_ptr;
      sms_sub_info_ptr->bc_enable              = cmd_info_ptr->bc_enable;

      cmsms_event( sms_sub_info_ptr, CM_SMS_EVENT_BC_ENABLED, sms_cmd_ptr->cmd_subs);
...
)
{
  ...
  sms_sub_info_ptr = cmsms_sub_info_ptr(sms_cmd_ptr->cmd_subs);
  if(sms_sub_info_ptr == NULL)
  {
    return;
  }
  ...
  */
  sms_cmd_err = cmsms_client_cmd_check( sms_cmd_ptr );
  ...
  */
  wait_for_reply = cmsms_client_cmd_forward_ll( sms_cmd_ptr );
  ...
  */
  if( ! wait_for_reply )
  {
    cmsms_client_cmd_ll_reply( sms_sub_info_ptr,
                               sms_cmd_ptr,
