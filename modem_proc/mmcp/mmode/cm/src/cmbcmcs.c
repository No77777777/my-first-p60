...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/mmode/cm/src/cmbcmcs.c#1 $
...
static cmbcmcs_s_type *bcmcs_obj_ptr[MAX_SIMS] = {NULL};
...
===========================================================================*/
static void                           cmbcmcs_reset_buffers(

  cmbcmcs_s_type                *bcmcs_ptr
)
{
#if (defined FEATURE_BCMCS) && (defined FEATURE_HDR)
...
===========================================================================*/
static void cmbcmcs_init( cmbcmcs_s_type  *bcmcs_ptr )
{
  ...
  */
  bcmcs_ptr->waiting_pwrsave_exit_for_ss = MM_STACK_NONE;
  ...
  */
  cmbcmcs_reset_buffers(bcmcs_ptr);
...
===========================================================================*/
cmbcmcs_s_type  *cmbcmcs_ptr(

  sys_modem_as_id_e_type   as_id
)
{
  /* If this is not a valid AS_ID - ERR_FATAL. This will ensure we dont have
  ** validate the as_id/bcmcs_ptr elsewhere in the file.
  */
  if (!cmutil_is_as_id_valid(as_id) )
  {
    CM_ERR_FATAL_1("BCMCS pointer attempted for invalid sub %d",as_id);
  }
  if ( bcmcs_obj_ptr[as_id] == NULL )
  {
    bcmcs_obj_ptr[as_id] = (cmbcmcs_s_type *) cm_mem_malloc(sizeof(cmbcmcs_s_type));
    cmbcmcs_init(bcmcs_obj_ptr[as_id]);
    /* Initialize transaction id*/
    bcmcs_obj_ptr[as_id]->transaction_id = *cmbcmcs_transaction_id_ptr(as_id);
    CM_MSG_MED_2("Allocating BCMCS object for sub %d at location 0x%x",as_id,bcmcs_obj_ptr[as_id]);
  }
  return bcmcs_obj_ptr[as_id];
...
)
{
  ...
  cm_mm_ss_info_3gpp2_stack_info_s_type *ss_info_gpp2_stack_0_ptr,*ss_info_gpp2_stack_1_ptr;
  ...
  */
  ...
  if(ss_info_sub_prop_ptr == NULL ||
      ss_info_sub_prop_ptr->stack_info[MM_STACK_0].gpp2_info == NULL)
  {
    *bcmcs_srv_supported = FALSE;
    *bcmcs_srv_status = SYS_BCMCS_SRV_STATUS_NO_SRV;
    return;
  }
  ss_info_gpp2_stack_0_ptr = ss_info_sub_prop_ptr->stack_info[MM_STACK_0].gpp2_info;
  ss_info_gpp2_stack_1_ptr = ss_info_sub_prop_ptr->stack_info[MM_STACK_1].gpp2_info;

  if(ss_info_gpp2_stack_1_ptr != NULL && cmss_is_stack1_operational(asubs_id))
  {
    *bcmcs_srv_supported = ss_info_gpp2_stack_1_ptr->ss_bcmcs_srv_supported;
    *bcmcs_srv_status    = ss_info_gpp2_stack_1_ptr->ss_bcmcs_srv_status;
  }
  else
  {
    *bcmcs_srv_supported = ss_info_gpp2_stack_0_ptr->ss_bcmcs_srv_supported;
    *bcmcs_srv_status    = ss_info_gpp2_stack_0_ptr->ss_bcmcs_srv_status;
  }
...
===========================================================================*/
static void                           cmbcmcs_set_state(

  sys_modem_as_id_e_type         asubs_id,

  cmbcmcs_s_type                *bcmcs_ptr,

  cm_bcmcs_state_e_type          new_state
)
{
  CM_ASSERT( bcmcs_ptr != NULL );
...
===========================================================================*/
static void                           cmbcmcs_mc_rpt_store(
...
** warning
*/
static boolean                        cmbcmcs_mc_rpt_check(

  const cmbcmcs_s_type                 *bcmcs_ptr,
...
/*lint -esym(715, cm_mc_rpt_ptr) not referenced */
{
  ...
  switch(cm_mc_rpt_ptr->hdr.cmd)
  {
    ...
    case CM_HDR_BCMCS_BROADCAST_DISCONTINUED_F:
      ...
      */
      if(bcmcs_ptr->transaction_id != cm_mc_rpt_ptr->bcmcs_rpt.transaction_id)
      {
        CM_ERR_0("Transaction ids do not match, discard report ...");
        CM_ERR_2("BCMCS id = %d, LL RPT id = %d", bcmcs_ptr->transaction_id,
...
/*lint -esym(529, bcmcs_ptr) not referenced */
{
  ...
  */
  ...
  cmbcmcs_s_type            *bcmcs_ptr      = cmbcmcs_ptr(as_id);
  ...
  */
  ...
  CM_ASSERT( bcmcs_ptr != NULL );
  CM_ASSERT( CM_INIT_CHECK( bcmcs_ptr->init_mask) );
  CM_MSG_LOW_2("START cmbcmcs_event(), BCMCS event=%d as_id=%d",bcmcs_event,as_id );
...
===========================================================================*/
static byte cmbcmcs_next_transaction_id( cmbcmcs_s_type *bcmcs_ptr )
{
  CM_ASSERT( bcmcs_ptr != NULL );
  CM_ASSERT( CM_INIT_CHECK( bcmcs_ptr->init_mask) );
  ...
  if(bcmcs_ptr->transaction_id == CM_BCMCS_TRANSACTION_ID_NONE )
  {
    bcmcs_ptr->transaction_id++;
  }
...
===========================================================================*/
static void                           cmbcmcs_cmd_xlate_ll_send(
  ...
  */
  ...
  if(num_of_flows  == 0)
  {
    hdrcp_req->bcmcs_discontinue_bc.transaction_id =
      cmbcmcs_next_transaction_id(bcmcs_ptr);
    ...
    */
    case CM_BCMCS_STATE_REQUESTING:
      ...
      */
      ...
      {
        multimode_stack_e_type  pref_ss    = MM_STACK_NONE;
        ...
        mm_sub_stk_id_s_type  mm_id = cmph_map_sub_stk_to_id(as_id, MM_STACK_1);

        if(ss_info_sub_prop_ptr == NULL || !cmutil_is_as_id_valid(as_id) ||
        ...
        */
        if(cmss_is_stack1_operational(as_id))
        {
          pref_ss = MM_STACK_1;
          if(ss_info_sub_prop_ptr->stack_info[pref_ss].gen_info != NULL &&
              ss_info_sub_prop_ptr->stack_info[pref_ss].gen_info->srv_status == SYS_SRV_STATUS_PWR_SAVE ||
              (ss_misc_stk1_ptr != NULL && ss_misc_stk1_ptr->no_srv_uptime != CMSS_NO_SRV_UPTIME_NONE &&
               cmss_true_stack_info_ptr(mm_id) != NULL &&
               SYS_SRV_STATUS_PWR_SAVE_INTERNAL == \
               cmss_true_stack_info_ptr(mm_id)->srv_status))
          {
            in_pwrsave = TRUE;
          }
        }
        else
        {
          ...
          */
          ...
          if(ss_info_sub_prop_ptr->stack_info[pref_ss].gen_info != NULL &&
              ss_info_sub_prop_ptr->stack_info[pref_ss].gen_info->srv_status == SYS_SRV_STATUS_PWR_SAVE ||
              (ss_misc_stk0_ptr != NULL && ss_misc_stk0_ptr->no_srv_uptime != CMSS_NO_SRV_UPTIME_NONE &&
               cmss_true_stack_info_ptr(mm_id) != NULL &&
               SYS_SRV_STATUS_PWR_SAVE_INTERNAL == \
               cmss_true_stack_info_ptr(mm_id)->srv_status))
          {
        ...
        */
        if(!sending_discontinue_broadcast
            &&
            (in_pwrsave ||
             bcmcs_ptr->waiting_pwrsave_exit_for_ss != MM_STACK_NONE))
        {
          if( bcmcs_ptr->waiting_pwrsave_exit_for_ss != pref_ss)
          {
            ...
            pref_ptr->mode_pref               = CM_MODE_PREF_NO_CHANGE;
            ...
            CM_MSG_HIGH_2(">>PH FORCE_PREF: Kicking sub %d stk %d to wake it up", as_id, pref_ss);
            bcmcs_ptr->waiting_pwrsave_exit_for_ss = pref_ss;

            (void)cmph_force_pref_on_the_fly_stack(
              cmph_map_sub_stk_to_id(as_id, pref_ss),
        ...
        else
        {
          ...
          cmbcmcs_cmd_xlate_ll_send(bcmcs_ptr);
          cmbcmcs_set_state(as_id,bcmcs_ptr,CM_BCMCS_STATE_WAITING_FOR_ACK);
        }
      ...
      */
      cmbcmcs_event( as_id, CM_BCMCS_EVENT_FLOW_REQUEST_FAILURE );

      /* Move BCMCS into idle state
      */
      cmbcmcs_set_state(as_id, bcmcs_ptr,CM_BCMCS_STATE_IDLE);
...
{
  cmbcmcs_s_type      *bcmcs_ptr  =  cmbcmcs_ptr(bcmcs_cmd_ptr->asubs_id);
  cm_bcmcs_cmd_s_type *bcmcs_temp_cmd_ptr = &(bcmcs_ptr->bcmcs_cmd);
  ...
  */
  ...
  switch( bcmcs_cmd_ptr->cmd )
  {
    ...
    case CM_BCMCS_CMD_FLOW_DEACT_REQUEST_F:
      ...
      */
      cmbcmcs_orig_proc(bcmcs_cmd_ptr->asubs_id);
...
)
{
  ...
  */
  ...
  if (cmph_is_ssim())
  {
    bcmcs_cmd_ptr->asubs_id = SYS_MODEM_AS_ID_1;
  }

  /* Get BCMCS service indicators
  */
  cmbcmcs_get_bcmcs_srv_indicators(&bcmcs_srv_supported,
                                   &bcmcs_srv_status, bcmcs_cmd_ptr->asubs_id);
...
)
{
  ...
  */
  ...
  cmbcmcs_s_type    *bcmcs_ptr = NULL;
  /* Point at BCMCS object
  */
  sys_modem_as_id_e_type asubs_id = cm_mc_rpt_ptr->hdr.asubs_id;
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    bcmcs_ptr = cmbcmcs_ptr(asubs_id);
  }
  /* Check the report for errors
  */
  if(!cmbcmcs_mc_rpt_check(bcmcs_ptr,cm_mc_rpt_ptr))
  {
    return;
  }
  ...
  /* Store the report
  */
  cmbcmcs_mc_rpt_store(bcmcs_ptr,cm_mc_rpt_ptr);
  ...
  switch( cm_mc_rpt_ptr->hdr.cmd )
  {
    ...
    */
    case CM_HDR_BCMCS_UPDATE_FLOW_REQ_SUCCESS_F:
      ...
      */
      ...
      /* Send event to registered clients
      */
      cmbcmcs_event( asubs_id, CM_BCMCS_EVENT_FLOW_REQUEST_SUCCESS );

      /* Move BCMCS into idle state
      */
      cmbcmcs_set_state(asubs_id,bcmcs_ptr, CM_BCMCS_STATE_IDLE);

      cmbcmcs_free(asubs_id);
    ...
    */
    case CM_HDR_BCMCS_UPDATE_FLOW_REQ_FAILURE_F:
      ...
      */
      cmbcmcs_set_state(asubs_id,bcmcs_ptr,CM_BCMCS_STATE_REQUESTING);

      /* Resend the last command
      */
      cmbcmcs_orig_proc(asubs_id);
    ...
    case CM_HDR_BCMCS_BROADCAST_RESET_F:
      ...
      /* Send event to registered clients
      */
      cmbcmcs_event( asubs_id, CM_BCMCS_EVENT_BROADCAST_RESET );
    ...
    */
    case CM_HDR_BCMCS_FLOW_STATUS_F:
      ...
      /* Send event to registered clients
      */
      cmbcmcs_event( asubs_id, CM_BCMCS_EVENT_FLOW_STATUS_CHANGE );
    ...
    */
    ...
    case CM_HDR_BCMCS_REG_STATUS_F:
      ...
      /* Send event to registered clients
      */
      cmbcmcs_event( asubs_id, CM_BCMCS_EVENT_REGISTRATION_STATUS );
...
)
{
  ...
  switch( sd_rpt_ptr->hdr.cmd )
  {
    ...
    {
      sys_modem_as_id_e_type as_id = sd_rpt_ptr->ok_to_orig.hdr.asubs_id;
      cmbcmcs_s_type         *bcmcs_ptr  = cmbcmcs_ptr(as_id);
      if(sd_rpt_ptr->ok_to_orig.stack_id == bcmcs_ptr->waiting_pwrsave_exit_for_ss)
      {
        CM_MSG_HIGH_2("bcmcs: OK_TO_ORIG, sub %d stack %d",as_id, sd_rpt_ptr->ok_to_orig.stack_id);
        bcmcs_ptr->waiting_pwrsave_exit_for_ss = MM_STACK_NONE;
        cmbcmcs_orig_proc(as_id);
      }
    }
    break;
    ...
    case CM_ACQ_FAIL_F:
    {
      sys_modem_as_id_e_type as_id = sd_rpt_ptr->acq_fail.hdr.asubs_id ;
      cmbcmcs_s_type         *bcmcs_ptr  = cmbcmcs_ptr(as_id);

      if(sd_rpt_ptr->ok_to_orig.stack_id == bcmcs_ptr->waiting_pwrsave_exit_for_ss)
      {
        CM_MSG_HIGH_2("bcmcs: ACQ_FAIL, sub %d, stack %d", as_id, sd_rpt_ptr->acq_fail.stack_id);
        bcmcs_ptr->waiting_pwrsave_exit_for_ss = MM_STACK_NONE;
        cmbcmcs_orig_proc(as_id);
      }
    }
...
===========================================================================*/
void cmbcmcs_free( sys_modem_as_id_e_type   as_id )
{
  if ( bcmcs_obj_ptr[as_id] != NULL)
  {
    CM_MSG_MED_1("De allocating BCMCS object at location 0x%x",bcmcs_obj_ptr);
    *cmbcmcs_transaction_id_ptr(as_id) = bcmcs_obj_ptr[as_id]->transaction_id;
    if(bcmcs_obj_ptr[as_id]->mbms_flow.flow_addr != NULL)
    {
      cm_mem_free(bcmcs_obj_ptr[as_id]->mbms_flow.flow_addr);
      bcmcs_obj_ptr[as_id]->mbms_flow.flow_addr = NULL;
    }
    cm_mem_free(bcmcs_obj_ptr[as_id]);
    bcmcs_obj_ptr[as_id] = NULL;
  }
  return;
}
...
===========================================================================*/
byte *cmbcmcs_transaction_id_ptr( sys_modem_as_id_e_type   as_id )
{
  static byte bcmcs_transaction_id[MAX_SIMS] = {CM_BCMCS_TRANSACTION_ID_NONE};

  return &bcmcs_transaction_id[as_id];
}

/*===========================================================================

FUNCTION cmbcmcs_is_flow_monitoring

DESCRIPTION

  Check if the flow_monitoring state is active on the given subs_id
...
===========================================================================*/
EXTERN boolean                  cmbcmcs_is_flow_monitoring(
...
)
{
  if ( cmutil_is_as_id_valid(as_id) &&
       bcmcs_obj_ptr[as_id] != NULL)
  {
    return ( bcmcs_obj_ptr[as_id]->flow_monitoring_state == CM_BCMCS_FLOW_MONITORING_STATE_ACTIVE );
  }
  return FALSE;
}
