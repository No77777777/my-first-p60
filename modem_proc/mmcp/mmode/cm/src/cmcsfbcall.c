...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/mmode/cm/src/cmcsfbcall.c#1 $
...
)
{
  ...
  */
  ...
  if ((cm_is_esr_allowed_in_srlte_per_subs(call_ptr->mm_id.asubs_id) == FALSE
       || cmph_is_oprting_in_1xsrlte_mode(call_ptr->mm_id.asubs_id) == FALSE
  ...
  */
     ...
     )
  {
    ...
    if(cm_is_esr_allowed_in_srlte_per_subs(call_ptr->mm_id.asubs_id))
    {
      ...
      client_request->esm_1xCSFB_call_req.partial_esr = TRUE;
            ...
            && cmcall_is_esr_allowed_in_hvolte(call_ptr->mm_id.asubs_id))
    {
  ...
  CM_MSG_HIGH_5( "ESR: CM->NAS: sub %d, NAS_ESM_1XCSFB_CALL_REQ, Call ID %d, csfb_service_type %d, partial_esr %d, is_stack2_operational %d",
...
)
{
  ...
  */
  ...
  /* Set partial ESR bit to suspend LTE for all non 1xCSFB cases */
  client_request->esm_1xCSFB_call_req.partial_esr = TRUE;
  ...
  CM_MSG_HIGH_6( "ESR: CM->NAS: sub %d, SEND: NAS_ESM_1XCSFB_CALL_REQ. CSFB Call ID %d, call_id %d, partial_esr %d, csfb_service_type %d, is_stack2_operational %d",
                 asubs_id, client_request->esm_1xCSFB_call_req.csfb_call_id,
                 call_id,
                 client_request->esm_1xCSFB_call_req.partial_esr,
                 client_request->esm_1xCSFB_call_req.csfb_service_type,
...
)
{
  ...
  */
  ...
  CM_MSG_HIGH_2("CM->NAS: sub %d, SEND: NAS_ESM_1XCSFB_ABORT_REQ for call_id = %d ",
                asubs_id, call_id);
...
static byte cmcall_map_onexrej_cc_rej (byte onex_rej_value)
{
  switch ((lte_irat_1xcsfb_ho_failure_cause_e)onex_rej_value)
  {
    ...
    case LTE_IRAT_1XCSFB_HO_FAILURE_SRCH_TT_FAIL:
      return ONEX_REJ_1XCSFB_HO_FAILURE_SRCH_TT_FAIL;

    case LTE_IRAT_1XCSFB_HO_FAILURE_TCH_INIT_FAIL:
      return ONEX_REJ_1XCSFB_HO_FAILURE_TCH_INIT_FAIL;

    case LTE_IRAT_1XCSFB_HO_FAILURE_USER_CALL_END:
      return ONEX_REJ_1XCSFB_HO_FAILURE_USER_CALL_END;

    case LTE_IRAT_1XCSFB_HO_FAILURE_RETRY_EXHAUST:
      return ONEX_REJ_1XCSFB_HO_FAILURE_RETRY_EXHAUST;

    case LTE_IRAT_1XCSFB_HO_FAILURE_CALL_REL_REG_REJ:
      return ONEX_REJ_1XCSFB_HO_FAILURE_CALL_REL_REG_REJ;

    case LTE_IRAT_1XCSFB_HO_FAILURE_CALL_REL_NW_REL_ODR:
      return ONEX_REJ_1XCSFB_HO_FAILURE_CALL_REL_NW_REL_ODR;
    ...
    if(call_ptr->is_rlf_in_1xcsfb_call == TRUE)
    {
      ...
      if (srv_type == SYS_SRV_STATUS_NO_SRV)
      {
        ...
        */
        ...
        cmsds_check_and_deact_csfb_mode (call_ptr->mm_id.asubs_id);
  ...
  */
  ...
  switch( cm_rpt_ptr->msg_hdr.id )
  {
    ...
    case MM_CM_1XCSFB_CALL_RSP:
    {
      ...
      CM_MSG_HIGH_5("NAS->CM: sub %d, 1XCSFB_CALL_RSP, call_id = %d rej present = %d,rej_type = %d,rej_value = %d",\
                    asubs_id,
      ...
      ** we needed this since we didnt have call obj when CMEMG E911 call is originated */
      ...
      if ( call_id == CM_MT_UNALLOCATED_CALL_ID &&
           cm_is_esr_allowed_in_srlte_per_subs(asubs_id) &&
           cmph_is_oprting_in_1xsrlte_mode(asubs_id) &&
           IS_VALID_PH_SUB_PTR(asubs_id) &&
           cmph_ptr()->sub_info[asubs_id]->sub_3gpp2_pref.is_1x_mt_call_pending)
      {
      ...
      /*ESR is allowed in SRLTE+G and VOLTE+G when ue mode is CSFB_PREFERRED*/
      /* Call orig should still be sent even if user already performed PS detach and thus HYBR2 is not operational */
      if( (cmmsimc_state_machine_ptr(asubs_id)->stack_common_info.ue_mode == SYS_UE_MODE_1XSRLTE_CSFB_PREFERRED ||
             cmmsimc_state_machine_ptr(asubs_id)->stack_common_info.ue_mode == SYS_UE_MODE_1XSRLTE_ONLY) &&
          (asubs_id == cmph_ptr()->device_prop.msim_prop.default_data_subs) &&
            cmph_is_subs_feature_mode_1x_sxlte(asubs_id) &&
          !cmcall_sent_on_1x_while_lte_active_in_single_mode(call_ptr) &&
          ...
          call_ptr->is_waiting_on_esr_rsp)
      {
        ...
        */
        else if( call_ptr->mm_id.asubs_id == asubs_id
              && call_ptr->call_type == CM_CALL_TYPE_EMERGENCY
        ...
        /* process ESR response for case when
        ** 1. camped on 1x+LTE dual stack in SRLTE or VOLTE+G in UE MODE : CSFB_PREFERRED
      ...
      */
      ...
      {
        call_ptr->is_waiting_on_esr_rsp = FALSE;
        if(call_ptr->switch_to_srlte == FALSE)
        {
        ...
        {
          /* If CM already update voip state to FALLBACK_TO_CS
          ** remove LTE from call prefs and force 
          */
          if(call_ptr->end_status != CM_CALL_END_NONE && 
             cmcall_act_get(call_ptr) == CMCALL_ACT_FORCE_NON_IP)
          {
            cmcall_remove_mode_and_force_and_sched_sr(call_ptr);
          }
          else
      ...
      {
        CM_MSG_HIGH_0("1XCSFB:call type is not 1XCSFB. So ignore the csfb report processing");
        return;
      }

      CM_MSG_HIGH_2("call_ptr->as_id=%d, set to rpt sub=%d",
                    call_ptr->mm_id.asubs_id, asubs_id);
      ...
      if(call_ptr->direction == CM_CALL_DIRECTION_MT)
      {
        ...
        CM_MSG_HIGH_3("MT_REDIAL:rej_cause:%d rej_value:%d is_mtcsfb_redial:%d",
                      ...
                      call_mode_info_ptr->cc_reject.rej_value,
                      sds_sub_prop_ptr->gpp2_sub_config.is_mtcsfb_call_redial_supported);
        ...
        */
        if( sds_sub_prop_ptr->gpp2_sub_config.is_mtcsfb_call_redial_supported == TRUE &&
            ...
            call_mode_info_ptr->cc_reject.rej_value == AS_REJ_LRRC_LOWER_LAYER_FAILURE)
        {
          ...
          ph_orig_ptr = cmtask_orig_para_search_act_id(call_ptr->mm_id,
                        (cm_act_id_type) call_ptr->call_id );

          cmcsfbcall_send_redial_req_for_mt_call(call_ptr->mm_id.asubs_id,call_ptr->call_id);
          ...
          if(ph_orig_ptr != NULL && IS_VALID_PH_SUB_PTR(asubs_id))
          {
            ph_orig_ptr->orig->orig_mode_pref = cm_remove_mode_pref_components(
                                                  ph_ptr->sub_info[asubs_id]->pref_info.mode_pref, 1,
        ...
        /* For MT page failures, indicate to clients that rej value is present
        */
        CM_MSG_HIGH_0("Marking cc_reject as TRUE for MT call failures");
        call_mode_info_ptr->cc_reject.present = TRUE;

        if (call_mode_info_ptr->cc_reject.rej_value != ONEX_REJ_1XCSFB_HO_FAIL_CALL_REL_SO_REJ)
        {
          cmcsfbcall_send_csfb_abort_req(call_ptr->call_id, asubs_id);
        }
        ...
        switch ( call_mode_info_ptr->cc_reject.rej_type)
        {
          ...
          case ONEX_CSFB_REJECT_CAUSE:
          {
            switch (call_mode_info_ptr->cc_reject.rej_value)
            {
              ...
              case ONEX_REJ_1XCSFB_HO_FAILURE_SRCH_TT_FAIL:
              case ONEX_REJ_1XCSFB_HO_FAILURE_TCH_INIT_FAIL:
              case ONEX_REJ_1XCSFB_HO_FAILURE_USER_CALL_END:
              case ONEX_REJ_1XCSFB_HO_FAIL_MSG_INVAILD:
              case ONEX_REJ_1XCSFB_HO_FAILURE_RETRY_EXHAUST:
              case ONEX_REJ_1XCSFB_HO_FAILURE_CALL_REL_REG_REJ:
              case ONEX_REJ_1XCSFB_HO_FAILURE_CALL_REL_NW_REL_ODR:
      ...
      if ( call_ptr->call_state == CM_CALL_STATE_ORIG )
      {
        cmsds_sub_prop_s_type *sds_sub_prop_ptr = cmsds_get_sub_prop_ptr(call_ptr->mm_id.asubs_id);
        if(sds_sub_prop_ptr == NULL)
        {
          return;
        }
        ...
        */
        ...
        if( (call_ptr->sr.esr_retry_counter <
             sds_sub_prop_ptr->gpp2_sub_config.n_min_MO_call_soft_retry)  &&
        ...
        /* Before calling cmcall_orig_proc(), check if silent redials have exhausted and
        ** call needs to be ended,
        ** inidicate that cc_reject cause is present
        */
        if(call_ptr->sr.sr_left <= 1 || uptime > call_ptr->sr.sr_max_uptime)
        {
          CM_MSG_HIGH_0("Marking cc_reject as TRUE for MO call silent redial");
          call_mode_info_ptr->cc_reject.present = TRUE;
        }
    ...
    case MM_CM_1XCSFB_ABORT_RSP:
      ...
      CM_MSG_HIGH_2("NAS->CM: sub %d, Received MM_CM_1XCSFB_ABORT_RSP for call_id=%d",
                    asubs_id,
                    cm_rpt_ptr->cm_1xcsfb_abort_rsp.csfb_call_id);

#if defined(FEATURE_LTE_TO_1X) && defined(FEATURE_MMODE_SC_SVLTE)
      if ( cm_is_esr_allowed_in_srlte_per_subs(asubs_id) &&
           cmph_is_oprting_in_1xsrlte_mode(asubs_id))
      {
        CM_MSG_HIGH_1("sub %d, clear 1XSRLTE call", asubs_id);
...
===========================================================================*/
boolean cmcsfbcall_send_mm_dual_rx_1xcsfb_tau_req(
  sys_modem_as_id_e_type  asubs_id
)
{
  ...
  mm_ptr->cmd.cm_mm_dual_rx_1xcsfb_tau_req.as_id = asubs_id;
  mm_put_cmd( mm_ptr );
  CM_MSG_HIGH_1("CM->NAS: sub %d, Send MMCM_DUAL_RX_1XCSFB_TAU_REQ to MM", asubs_id);
...
===========================================================================*/
void cmcsfbcall_unforce_csfb_call_end_preferences(
...
)
{
  ...
  while(q_ptr != NULL)
  {
    ...
    CM_MSG_HIGH_3_EXT(" Orig params act_update_reas=%d, orig_mode=%d mode_pref=%d",\
                      q_ptr->orig->act_update_reas, q_ptr->orig->orig_mode, call_mode_pref, (mm_id.asubs_id+1));
    ...
    if ((q_ptr->orig->act_update_reas == CM_ACT_UPDATE_REAS_ACT_END)&&
        ...
        ((call_mode_pref == CM_MODE_PREF_EMERGENCY && !cmsds_is_LTE_emerg_call_possible(mm_id.asubs_id)) ||
         (call_mode_pref != CM_MODE_PREF_EMERGENCY && cmcall_misc_is_mode_pref(call_mode_pref,CM_MODE_PREF_LTE_ONLY) == FALSE))
       )
    {
  ...
  {
    cmph_check_and_unforce_orig_mode(mm_id, FALSE);
  }
}
...
===========================================================================*/
void cmcsfbcall_unforce_hard_failure_preferences(
  sys_modem_as_id_e_type  asubs_id
)
{
...
)
{
  ...
  req->esm_1xCSFB_call_req.csfb_call_id = call_ptr->call_id;
  req->esm_1xCSFB_call_req.csfb_service_type = LTE_MO_CSFB_1xCSFB;
  req->esm_1xCSFB_call_req.partial_esr = partial_esr;

  CM_MSG_HIGH_4("ESR: CM->NAS: sub %d, 1XCSFB_CALL_REQ, call_id = %d on sub %d, partial_esr=%d, LTE_MO_CSFB_1xCSFB",
                esr_sub, call_ptr->call_id, call_ptr->mm_id.asubs_id, partial_esr);
  (void) cm_msgr_send( NAS_ESM_1XCSFB_CALL_REQ, MSGR_MM_CM,
