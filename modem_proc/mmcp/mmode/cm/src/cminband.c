...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/mmode/cm/src/cminband.c#2 $
...
)
{
  ...
  if(call_ptr != NULL)
  {
    ...
    else
    {
      ...
      sys_mode = (sys_sys_mode_e_type)(ss_true_stk_info_ptr->mode);
    }
    CM_MSG_HIGH_2("Inband cmd on stack=%d,sys mode=%d",call_ptr->mm_id.stk_id,sys_mode);
  }
  ...
  */
  switch( inband_cmd_ptr->cmd )
  {
    ...
    case CM_CMD_SIP_DTMF:

      /* check command parameter errors */
      #ifdef FEATURE_IP_CALL
        cmd_err = cmipcall2_cmd_sip_dtmf_para_check(inband_cmd_ptr);
      #else
        CM_MSG_HIGH_0("SIP_DTMF: FEATURE_IP_CALL not defined");
        cmd_err = CM_INBAND_CMD_ERR_FUNCTION_UNAVAILABLE_DUE_TO_UNDEFINED_FEATURE;
      #endif
      break;
...
)
{
  ...
  cmd_info_ptr = CMD_INFO_PTR( inband_cmd_ptr );
  if ((call_ptr = cmcall_ptr(cmd_info_ptr->call_id)) == NULL)
  {
    CM_ERR_1("ERROR: Unknown call ID in inband cmd %d", inband_cmd_ptr->cmd);
    return;
  }

  inband_info_ptr->asubs_id = call_ptr->mm_id.asubs_id;
  ...
  */
  ...
  switch (inband_cmd_ptr->cmd)
  {
    ...
    case CM_CMD_SIP_DTMF:
      inband_info_ptr->call_id  = cmd_info_ptr->call_id;
      inband_info_ptr->sip_dtmf_info = cmd_info_ptr->sip_dtmf_info;
      inband_event = CM_EVENT_IP_SIP_DTMF;
      break;
...
)
{
  ...
  inband_info_ptr->asubs_id = cmd_ptr->hdr.asubs_id;
  ...
  */
  ...
  switch (cmd_ptr->hdr.cmd)
  {
    ...
    default:
      CM_ERR_2( "sub %d, Bad MC report %d", cmd_ptr->hdr.asubs_id, cmd_ptr->hdr.cmd);
      break;
...
)
{
  ...
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  inband_info_ptr->asubs_id = cm_rpt_ptr->hdr.asubs_id;
...
)
{
  ...
  */
  if( (call_ptr->mm_id.stk_id == MM_STACK_0) &&
      !cm_is_sys_mode_digital( ss_info_all_stack_ptr->sys_mode ) &&
  ...
  switch( inband_cmd_ptr->cmd )
  {
    ...
    case CM_CMD_SIP_DTMF:
      cmipcall2_mo_sip_dtmf_proc(call_ptr, cmd_info_ptr->sip_dtmf_info);
      break;
...
===========================================================================*/
void cminband_ip_rpt_proc(
  cm_name_type             rpt_type,
  boolean                  status,
...
)
{
  ...
  inband_info_ptr->asubs_id = asubs_id;
...
===========================================================================*/
void cminband_ip_ind_proc(
  cm_name_type                 rpt_type,
  cmipapp_ind_dtmf_ind_s_type  dtmf_ind
)
{
  cm_inband_info_s_type     *inband_info_ptr = cminband_info_alloc();
  cmcall_s_type             *call_ptr = NULL;
...
FUNCTION cminband_mt_sip_dtmf_ind_proc

DESCRIPTION
  Process MT SIP DTMF received from IMS

RETURN VALUE
  None

===========================================================================*/
void cminband_mt_sip_dtmf_ind_proc(
  cmipapp_ind_sip_dtmf_ind_s_type  dtmf_ind
)
{
  cm_inband_info_s_type  *inband_info_ptr = cminband_info_alloc();
  cmcall_s_type          *call_ptr = NULL;
  ...
  {
    CM_ERR_0("SIP_DTMF: ERROR: null inband_info_ptr");
    return;
  }

  if (call_ptr == NULL)
  {
    CM_ERR_1("SIP_DTMF: ERROR: Unknown call ID %d in MT SIP DTMF ind",
             dtmf_ind.call_id);
    return;
  }

  if (dtmf_ind.dtmf_info == NULL ||
      dtmf_ind.dtmf_info->dtmf_string == NULL)
  {
    CM_ERR_0("SIP_DTMF: ERROR: mt dtmf string null ptr");
    return;
  }
  ...
  inband_info_ptr->call_id = dtmf_ind.call_id;
  inband_info_ptr->asubs_id = call_ptr->mm_id.asubs_id;
  inband_info_ptr->sip_dtmf_info = dtmf_ind.dtmf_info;

  CM_MSG_HIGH_5_EXT("SIP_DTMF: RXD: MT DTMF ind for call_id %d, dtmf_str %d %d, dtmf str len %d, sub %d",
                    dtmf_ind.call_id,
                    dtmf_ind.dtmf_info,
                    dtmf_ind.dtmf_info->dtmf_string,
                    strlen(dtmf_ind.dtmf_info->dtmf_string),
                    call_ptr->mm_id.asubs_id,
                    call_ptr->mm_id.asubs_id);

  /* Add ref before sending sip dtmf ind event */
  ref_cnt_obj_add_ref(inband_info_ptr->sip_dtmf_info);

  cmclient_list_inband_event_ntfy(CM_EVENT_IP_SIP_DTMF_IND, inband_info_ptr);

  /* Release ref after sip dtmf ind event */
  REF_CNT_OBJ_RELEASE_IF(inband_info_ptr->sip_dtmf_info);
  inband_info_ptr->sip_dtmf_info = NULL;
