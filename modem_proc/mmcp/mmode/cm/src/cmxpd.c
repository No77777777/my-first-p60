...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/mmode/cm/src/cmxpd.c#1 $
...
===========================================================================*/
cmpd_session_s_type *cmpd_ptr(
  cmpd_session_obj_e_type pd_session_obj
)
{
...
static cm_pd_info_s_type *cmpd_info_alloc(void)
{
  ...
  */
  pd_info_ptr = (cm_pd_info_s_type *)cm_mem_malloc(sizeof(cm_pd_info_s_type));
...
)
{
  ...
  if (cmph_is_msim())
  {
    pd_info_ptr->asubs_id = asubs_id;
  }
  else
  {
    pd_info_ptr->asubs_id = SYS_MODEM_AS_ID_1;
  }
...
)
{
  ...
  cm_mc_send_cmd(reva_cmd_ptr->asubs_id, mc_ptr);
}
...
)
{
...
#if (defined(FEATURE_MMODE_CDMA_800) || defined(FEATURE_MMODE_CDMA_1900))
    cmph_mo_sms_end(asubs_id, duration, (cm_act_id_type)pd_ptr);
#endif
      ...
      pd_ptr->end_status != CM_PD_SESSION_STATUS_END)
  {
    ...
    */
    ...
    cmph_mo_sms_end(asubs_id, (dword)duration, (cm_act_id_type)pd_ptr );
#endif
  ...
  */
  cmph_check_and_unforce_orig_mode(cmph_map_sub_stk_to_id(asubs_id, MM_STACK_0), FALSE);
  cmph_check_and_unforce_orig_mode(cmph_map_sub_stk_to_id(asubs_id, MM_STACK_1), FALSE);

  /* Notify client a PD event */
  CM_MSG_HIGH_1("cmpd_session_abort() Notify PD. Status = %d", pd_ptr->end_status);

  cmpd_event(pd_ptr, CM_PD_EVENT_SESSION_END, asubs_id);
...
)
{
  ...
  cm_act_orig_s_type             *last_pref_ptr = cmph_get_last_pref_sent(cmph_map_sub_stk_to_id(asubs_id,MM_STACK_0));
  ...
  */
      ...
      cm_call_state == CM_OVERALL_CALL_STATE_ORIG_INCOM)
  {
    ...
    ph_orig_ptr = cmtask_orig_para_get_top(cmph_map_sub_stk_to_id(asubs_id,
                                           MM_STACK_0));
  ...
  if(call_ptr != NULL && cmcall_misc_is_mode_pref(call_ptr->favored_mode_pref,
      CM_MODE_PREF_GSM_WCDMA_ONLY))
  {
    return CM_PD_SESSION_STATUS_DENY_GSM_ORIG;
  }
  ...
  */
  if(!cmcall_is_no_call_in_gw_cs_per_subs(asubs_id) ||
      !cmcall_is_no_call_in_gw_ps_per_subs(asubs_id))
  {
    return CM_PD_SESSION_STATUS_DENY_GSM_CALL;
  }
  ...
  if( session_type == CM_PD_SESSION_TYPE_WITH_GPS_DMOD &&
      ( cmcall_is_hdr_call() != CM_CALL_ID_INVALID ||
        ( call_ptr != NULL && ( call_ptr->mm_id.stk_id == MM_STACK_1||
                                ( call_ptr->mm_id.stk_id == MM_STACK_0 &&
                                  sys_mode_stk0 == SYS_SYS_MODE_HDR ) )
        )
      )
    )
  {
...
)
{
  ...
  */
  if(ph_sub_ptr->pref_info.mode_pref == CM_MODE_PREF_HDR_ONLY)
  {
    if(grant_session && pd_ptr->session_state != CMPD_SESSION_STATE_GRANTED)
    {
      ...
      cmpd_event(pd_ptr, CM_PD_EVENT_SESSION_GRANTED, as_id);
    }
    return;
  }
  ...
  */
  ...
  cm_curr_mode_pref = cmcall_add_mode_pref_components(cm_curr_mode_pref,
                      1,
  ...
  */
  if(cm_curr_mode_pref != CM_MODE_PREF_MAX && !pd_ptr->is_mode_forced )
  {
    ...
    pref_ptr->pref_term              = CM_PREF_TERM_CM_1_CALL;
    ...
    pref_ptr->mode_pref              = cm_curr_mode_pref;
  ...
  /* Delay the session grant procedure only if GSM is active */
#ifdef CM_GW_SUPPORTED
  if(cmregprx_get_state(mm_id) != CMREGPRX_STATE_ACTIVE &&
      cmregprx_get_substate(mm_id) == CMREGPRX_SUBSTATE_NULL)
#endif
  {
...
)
{
  ...
  cmpd_session_obj_e_type      session_obj_type =
    ( session_type == CM_PD_SESSION_TYPE_WITH_GPS_DMOD ?
      CMPD_SESSION_OBJ_DMOD  :
      CMPD_SESSION_OBJ_MAIN );
  cmpd_session_s_type         *pd_ptr = cmpd_ptr(session_obj_type);
  ...
  cmpd_session_grant_status = cmpd_session_is_ok_to_grant(session_type, asubs_id);
  ...
  if(cmpd_session_grant_status == CM_PD_SESSION_STATUS_HOLD_FORCE_MODE)
  {
    ...
    */
    ...
    cmpd_session_force_mode(session_type, FALSE, asubs_id);
    return;
  }
  ...
  */
  ...
  /* If the session cannot be granted, then report session deny event to clients */
  if(cmpd_session_grant_status != CM_PD_SESSION_STATUS_GRANTED)
  {
    pd_ptr->session_state = CMPD_SESSION_STATE_NONE;
    pd_ptr->session_type = session_type;
    cmpd_event(pd_ptr, CM_PD_EVENT_SESSION_DENIED, asubs_id);
    cmpd_reset(session_obj_type);
    return;
  }
  ...
  */
  ...
  if(call_id != CM_CALL_ID_INVALID)
  {
    ...
    pd_ptr->session_state = CMPD_SESSION_STATE_GRANTED;
    pd_ptr->session_type = session_type;
    cmpd_event(pd_ptr, CM_PD_EVENT_SESSION_GRANTED, asubs_id);
    return;
  }

  cmpd_session_force_mode(session_type, TRUE, asubs_id);
}
...
)
{
  ...
  */
  ...
  pd_ptr->dormant_status = dormant_status;
  cmpd_event(pd_ptr, CM_PD_EVENT_SESSION_DORMANT_CHGD, asubs_id);
...
SIDE EFFECTS
  none
===========================================================================*/
boolean cmpd_is_session_active(
  const cmpd_session_s_type  *pd_ptr
)
{
  CM_ASSERT(pd_ptr != NULL);
...
)
{
  ...
  */
...
#ifdef CM_GW_SUPPORTED
      && (cmregprx_get_state(mm_id) != CMREGPRX_STATE_ACTIVE)
      && (cmregprx_get_substate(mm_id) == CMREGPRX_SUBSTATE_NULL )
#endif
    )
  {
    cmpd_session_grant_proc(pd_ptr->session_type, as_id);
  }
  ...
  {
    cmpd_session_grant_proc(pd_dmod_ptr->session_type, as_id);
  }
...
)
{
  ...
  if (cmph_is_msim())
  {
    asubs_id = pd_cmd_ptr->asubs_id;
  }
  else
  {
    pd_cmd_ptr->asubs_id = asubs_id = SYS_MODEM_AS_ID_1;
  }
  ph_sub_ptr = cmph_get_sub_ptr(asubs_id);

  if (!cmutil_is_as_id_valid(asubs_id) || ph_sub_ptr == NULL)
  {
    return CM_PD_CMD_ERR_SESSION_TYPE_INVALID;
  }
  ...
  switch(pd_cmd_ptr->cmd)
  {
    case CM_PD_CMD_SESSION_START:
      if(!CMPD_SESSION_START_IS_OPRT_MODE_VALID(ph_ptr->device_prop.ph_oprt_mode.oprt_mode))
      {
        cmd_err = CM_PD_CMD_ERR_OFFLINE_S;
        CM_ERR_0("CM_PD_CMD_ERR_OFFLINE_S");
      }
      else if(cmph_get_is_in_use(asubs_id))
      {
      ...
      else if(ph_sub_ptr->pref_info.mode_pref == CM_MODE_PREF_EMERGENCY)
      {
...
)
{
  ...
  as_id = pd_cmd_ptr->asubs_id;

#if (defined (FEATURE_GPSONE_MSBASED) || defined (FEATURE_GPSONE))
  mm_id = cmph_map_sub_stk_to_id(as_id, MM_STACK_0);
#endif
  ...
  */
  ...
  if(cmd_err != CM_PD_CMD_ERR_NOERR || !cmutil_is_as_id_valid(as_id))
  {
    return;
  }
  ...
  switch(pd_cmd_ptr->cmd)
  {
    case CM_PD_CMD_SESSION_START:
      ...
      */
      if(cmpd_is_session_granted(cmd_info_ptr->session_type))
      {
        ...
        {
          cmpd_event( pd_dmod_ptr, CM_PD_EVENT_SESSION_START, as_id);
          cmph_pd_session_start();
          cmpd_event( pd_dmod_ptr, CM_PD_EVENT_SESSION_GRANTED, as_id);
        }
        else
        {
          cmpd_event( pd_ptr, CM_PD_EVENT_SESSION_START, as_id);
          cmph_pd_session_start();
          cmpd_event( pd_ptr, CM_PD_EVENT_SESSION_GRANTED, as_id);
        }
        break;
      ...
      switch(cmd_info_ptr->session_type)
      {
        case CM_PD_SESSION_TYPE_WITH_SYS_TIME:
        {
          ...
          CM_MSG_HIGH_1(">>PH FORCE_PREF: PD session with SYS Time sub %d!", as_id);
        ...
        case CM_PD_SESSION_TYPE_WITH_GPS_MODE:
        {
          ...
          pref_ptr->roam_pref              = CM_ROAM_PREF_NO_CHANGE;
          pref_ptr->hybr_pref              = CM_HYBR_PREF_NO_CHANGE;
          pref_ptr->mode_pref              = CM_MODE_PREF_GPS_ONLY;
          pref_ptr->srv_domain_pref        = CM_SRV_DOMAIN_PREF_NO_CHANGE;
          pref_ptr->csg_id                 = SYS_CSG_ID_INVALID;
          pref_ptr->csg_rat                = SYS_RAT_NONE;

          CM_MSG_HIGH_1(">>PH FORCE_PREF: PD session with GPS mode!sub %d", as_id);
        ...
        case CM_PD_SESSION_TYPE_WITH_GPS_NORMAL:
          ...
          pd_ptr->is_mode_forced = FALSE;
          cmpd_event(pd_ptr, CM_PD_EVENT_SESSION_START, as_id);
          cmpd_session_grant_proc(cmd_info_ptr->session_type, as_id);
          break;
    ...
    case CM_PD_CMD_FIX_START:
      ...
      cmpd_event(pd_ptr, CM_PD_EVENT_FIX_START, as_id);
      cmlog_block_hdr_orig_during_gps(as_id);
      cmph_update_ddtm_status(as_id);
      break;
    ...
    case CM_PD_CMD_FIX_END:
      ...
      /* Notify client a PD event */
      pd_ptr->gps_fix_status = cmpd_fix.status;
      cmpd_event(pd_ptr, CM_PD_EVENT_FIX_END, as_id);
      cmlog_allow_hdr_orig_during_gps(as_id);
      cmph_update_ddtm_status(as_id);
      break;
#endif /* FEATURE_HDR_HYBRID */
  ...
  switch(cm_mc_rpt_ptr->hdr.cmd)
  {
    case CM_GPS_MODE_GRANTED_F:
         ...
         )
      {
        ...
        cmpd_event(pd_ptr, CM_PD_EVENT_SESSION_GRANTED, cm_mc_rpt_ptr->hdr.asubs_id);
      }
      break;
    ...
    case CM_GPS_MODE_EXIT_F:
      ...
      if (pd_ptr->session_type == CM_PD_SESSION_TYPE_WITH_GPS_MODE)
      {
        ...
        cmpd_session_abort(CM_PD_SESSION_STATUS_ABORT_GPS_MODE_EXIT,
                           ...
                           (dword)0, cm_mc_rpt_ptr->hdr.asubs_id);
      }
      break;
    ...
    case CM_BS_STATUS_RSP_F:
      ...
      */
      ...
      cmpd_event(pd_ptr,CM_PD_EVENT_REVA_MSG, cm_mc_rpt_ptr->hdr.asubs_id);
      break;
#endif /*FEATURE_IS2000_REL_A*/
  ...
  switch(sd_rpt_ptr->hdr.cmd)
  {
    case CM_OK_TO_ORIG_F:
         ...
         )
      {
        ...
        CM_MSG_MED_1("sub %d, Session with Sys Time is granted", sd_rpt_ptr->hdr.asubs_id);
        pd_ptr->session_state = CMPD_SESSION_STATE_GRANTED;

        /* Send a PD event to the client */
        cmpd_event(pd_ptr, CM_PD_EVENT_SESSION_GRANTED, sd_rpt_ptr->hdr.asubs_id);
      }
      break;
    ...
    case CM_ACQ_FAIL_F:
      ...
      if (pd_ptr->session_type == CM_PD_SESSION_TYPE_WITH_SYS_TIME)
      {
        ...
        cmpd_session_abort(CM_PD_SESSION_STATUS_ABORT_ACQ_FAIL,
                           ...
                           (dword)0, sd_rpt_ptr->hdr.asubs_id);
      }
      break;
      ...
      if(cmcall_misc_is_mode_pref(mode_pref, CM_MODE_PREF_GSM_WCDMA_ONLY))
      {
            ...
            pd_ptr->session_state != CMPD_SESSION_STATE_ABORTED )
        {
          cmpd_session_abort(CM_PD_SESSION_STATUS_ABORT_PHONE_BUSY,
                             pd_ptr->session_type,(dword)0, asubs_id);
          session_ended = TRUE;
        }
        ...
        {
          cmpd_session_abort(CM_PD_SESSION_STATUS_ABORT_PHONE_BUSY,
                             pd_dmod_ptr->session_type,(dword)0, asubs_id);
          session_ended = TRUE;
        }
        ...
        if(session_ended)
        {
          ...
          */
          cmph_get_last_pref_sent(cmph_map_sub_stk_to_id(asubs_id,MM_STACK_0))->orig_mode_pref = mode_pref;
          return TRUE;
        }
...
void cmpd_call_start(
  ...
  */
  mm_sub_stk_id_s_type      mm_id,
  /* Stack on which the call is attempted
  */
  ...
  */
  switch(sys_mode)
  {
    ...
    case SYS_SYS_MODE_CDMA:
      ...
      if(pd_dmod_ptr->session_state == CMPD_SESSION_STATE_GRANTED &&
          cmcall_check_if_no_srv(asubs_id, TRUE))
      {
        cmpd_session_abort(CM_PD_SESSION_STATUS_ABORT_PHONE_BUSY,
                           pd_dmod_ptr->session_type,
                           (dword)0, asubs_id);
      ...
      switch(mode_info)
      {
        ...
        case CM_CALL_MODE_INFO_CDMA:
          ...
          if(call_type == CM_CALL_TYPE_PS_DATA)
          {
            ...
            cm_mm_ss_info_gen_stack_info_s_type *ss_stk0_ptr, *ss_stk1_ptr;
            ...
            ss_stk0_ptr = ss_sub_ptr->stack_info[MM_STACK_0].gen_info;
            ss_stk1_ptr = ss_sub_ptr->stack_info[MM_STACK_1].gen_info;
            ...
            */
            if(mm_id.stk_id == MM_STACK_1 ||
                ( !cmss_is_stack1_operational(asubs_id) &&
                  ss_stk0_ptr->sys_mode == SYS_SYS_MODE_HDR ) )
            {
              /* Depending on the session type abort the session */
              if( pd_dmod_ptr->session_state == CMPD_SESSION_STATE_GRANTED )
              {
