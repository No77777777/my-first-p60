...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/mmode/cm/src/cmxsms.c#1 $
...
static void cmsms_send_mo_msg(
  ...
  cmsms_sub_info_s_type              *sms_sub_info_ptr
...
)
{
  ...
  CM_ASSERT( sms_sub_info_ptr != NULL );
  ...
  mc_ptr->mo_dbm.ack_req      = sms_sub_info_ptr->l2_ack_requested;
  mc_ptr->mo_dbm.dbm_buf_ptr  = (void *)sms_sub_info_ptr->data;
...
===========================================================================*/
static void cmsms_check_mo_status_timer(
  sys_modem_as_id_e_type  asubs_id
)
{

  cmsms_sub_info_s_type *sms_sub_info_ptr = cmsms_sub_info_ptr(asubs_id);
  dword                   uptime;
  /* get the current uptime */

  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

  if(sms_sub_info_ptr == NULL)
  {
    return;
  }
  ...
  */
  if (sms_sub_info_ptr->sms_state != CM_SMS_STATE_WAIT_FOR_STATUS)
  {
    return;
  }
  ...
  */
  ...
  if (uptime > sms_sub_info_ptr->max_wait_for_status_time)
  {
    /* Do necessary phone's MO SMS end activities.
    */
    cmph_mo_sms_end( asubs_id,(dword) 0, (cm_act_id_type)cmsms_ptr());

    sms_sub_info_ptr->mo_status     = CM_SMS_MO_L2_ACK_FAILURE;
    sms_sub_info_ptr->sms_state     = CM_SMS_STATE_IDLE;
    cmph_update_ddtm_status( asubs_id );        /* update the DDTM satus */
    cmsms_event( sms_sub_info_ptr, CM_SMS_EVENT_MO_STATUS, asubs_id);
...
===========================================================================*/
boolean cmxsms_is_mo_sms_in_progress(sys_modem_as_id_e_type asubs_id)
{
  cmsms_sub_info_s_type *sms_sub_info_ptr = cmsms_sub_info_ptr(asubs_id);
  ...
  */
  if (sms_sub_info_ptr->sms_state != CM_SMS_STATE_IDLE)
  {
    status = TRUE;
  }
#endif /* FEATURE_CDSMS */
  ...
  switch( sms_cmd_ptr->cmd )
  {
    ...
    case CM_SMS_CMD_AWI_ACK:
      ...
      */
      ...
      /* Send command to MC.
      */
      cm_mc_send_cmd( sms_cmd_ptr->cmd_subs, mc_ptr );
...
)
{
  ...
  dword                     uptime;
  /* get the current uptime */
  sys_sys_mode_e_type       sys_mode = cmss_get_info_sys_mode(asubs_id, MM_STACK_0);

  if(sms_sub_info_ptr == NULL || !cmutil_is_as_id_valid(asubs_id))
  {
    return;
  }
  ...
  */
  if (sms_sub_info_ptr->sms_state != CM_SMS_STATE_HOLD_ORIG_RETRY)
  {
    return;
  }
  ...
  */
  ...
  {
    sms_sub_info_ptr->mo_status =  CM_SMS_MO_NO_SRV;
  }
  ...
  */
  if( sms_sub_info_ptr->mo_status != CM_SMS_MO_NONE )
  {
    /* Do necessary phone's MO SMS end activities.
    */
    cmph_mo_sms_end( asubs_id,(dword) 0, (cm_act_id_type)cmsms_ptr() );
  ...
  if( cmss_get_misc_sub_prop_ptr(asubs_id) != NULL &&
      cmss_get_misc_sub_prop_ptr(asubs_id)->sub_info.gpp2_sub_info.is_hdr_session_open )
  {
  ...
  */
  ...
  if( sms_sub_info_ptr->is_ok_to_send )
  {
    cmsms_send_mo_msg(asubs_id, sms_sub_info_ptr);
    sms_sub_info_ptr->sms_state      = CM_SMS_STATE_WAIT_FOR_STATUS;
    sms_sub_info_ptr->is_ok_to_send  = FALSE;
    sms_sub_info_ptr->max_wait_for_status_time =
  ...
  */
  ...
  if (uptime > sms_sub_info_ptr->max_hold_orig_time)
  {
    CM_MSG_MED_2("SMS not sent hence end it uptime:%d sms_retry_period:%d",uptime,sms_sub_info_ptr->max_hold_orig_time);
    /* Do necessary phone's MO SMS end activities.
    */
    cmph_mo_sms_end( asubs_id,(dword) 0, (cm_act_id_type)cmsms_ptr());

    sms_sub_info_ptr->mo_status     = CM_SMS_MO_HOLD_ORIG_RETRY_TIMEOUT;
    sms_sub_info_ptr->sms_state     = CM_SMS_STATE_IDLE;
    cmph_update_ddtm_status( asubs_id );        /* update the DDTM satus */
    cmsms_event( sms_sub_info_ptr, CM_SMS_EVENT_MO_STATUS, asubs_id);
    return;
...
)
{
  ...
  if(ph_sub_ptr == NULL || sms_sub_info_ptr == NULL)
  {
    return;
  }
#ifdef FEATURE_DEDICATED_C2K_MODE
  ...
  sms_sub_info_ptr->max_hold_orig_time    = uptime + MAX_SMS_HOLD_ORIG_TIME;

  /* set mo status to none */
  sms_sub_info_ptr->mo_status                           = CM_SMS_MO_NONE;

  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

#ifdef FEATURE_DEDICATED_C2K_MODE
  if(ss_misc_sub_ptr->sub_info.gpp2_sub_info.is_hdr_session_open )
  {
  ...
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  overall_call_state = cmcall_overall_call_state_per_ss(cmph_map_sub_stk_to_id(asubs_id, MM_STACK_0));
  CM_MSG_MED_2("Overall_call_state %d s_hybr_hdr_call %d",
               overall_call_state, cmcall_is_hybr_hdr_call(NULL, asubs_id));
  ...
  */
...
#ifdef FEATURE_HDR_HYBRID
       || (cmcall_is_hybr_hdr_orig(asubs_id) !=  NULL)
       || cmcall_is_hybr_hdr_call(NULL, asubs_id)
#endif
      ...
      && IS_VALID_PH_SUB_PTR(asubs_id))
  {
    ...
    pref_ptr->pref_term              = CM_PREF_TERM_CM_MO_SMS;
    pref_ptr->network_sel_mode_pref  = CM_NETWORK_SEL_MODE_PREF_NONE;
    pref_ptr->acq_order_pref         = CM_GW_ACQ_ORDER_PREF_NO_CHANGE;
...
#ifdef FEATURE_HDR_HYBRID
    pref_ptr->mode_pref              = cmph_select_dbm_mode_pref();
#else
    ...
    CM_MSG_HIGH_1(">>PH FORCE_PREF: SMS sub %d", asubs_id);
    ...
    sms_sub_info_ptr->sms_state = CM_SMS_STATE_HOLD_ORIG_RETRY;
  ...
  */
  cmsms_event( sms_sub_info_ptr, CM_SMS_EVENT_MO_MSG, asubs_id);
...
/*lint -esym(715,timer_event) */
{
  ...
  for(sub=0; sub<cmsms_ptr()->count; sub++)
  {
    sms_sub_info_ptr = cmsms_sub_info_ptr((sys_modem_as_id_e_type)sub);
    sys_mode = cmss_get_info_sys_mode(sub, MM_STACK_0);
    sds_sub_prop_ptr = cmsds_get_sub_prop_ptr((sys_modem_as_id_e_type)sub);

    if(sms_sub_info_ptr == NULL  || sds_sub_prop_ptr == NULL)
    {
      continue;
    }

    if((sds_sub_prop_ptr->gen_sub_config.is_sms_over_s102_supported == TRUE) &&
       (sms_sub_info_ptr->hold_orig_time_s102 != CMCALL_UPTIME_NONE) &&
       (sms_sub_info_ptr->hold_orig_time_s102 <= uptime) &&
        (sms_sub_info_ptr->hold_orig_time_s102 < sms_sub_info_ptr->max_hold_orig_time) &&
        (sys_mode == SYS_SYS_MODE_LTE) &&
        (sms_sub_info_ptr->sms_state == CM_SMS_STATE_HOLD_ORIG_RETRY))
    {
      sms_sub_info_ptr->is_ok_to_send = TRUE;
      sms_sub_info_ptr->hold_orig_time_s102 = CMCALL_UPTIME_NONE;
    }

    /* check whether need to process MO SMS or not */
    cmxsms_mo_msg_retry((sys_modem_as_id_e_type)sub);
    cmsms_check_mo_status_timer((sys_modem_as_id_e_type)sub);
...
)
{
  ...
  cmsms_sub_info_s_type *sms_sub_info_ptr = cmsms_sub_info_ptr(cm_mc_rpt_ptr->hdr.asubs_id);
  /* Point at sms object */
  cmcall_s_type       *call_ptr;
  /* Point at call object */
  cm_iterator_type    call_itr;
#endif
  sys_sys_mode_e_type sys_mode = cmss_get_info_sys_mode(cm_mc_rpt_ptr->hdr.asubs_id, MM_STACK_0);
  ...
  switch( cm_mc_rpt_ptr->hdr.cmd )
  {
    ...
    */
...
#ifndef FEATURE_MMODE_REMOVE_1X
      CM_MSG_HIGH_3("SMS_STATUS: msg_type:%d, state_:%d, status;%d",
                    cm_mc_rpt_ptr->mo_dbm_status.msg_type,
                    sms_sub_info_ptr->sms_state,
      ...
      if(cm_mc_rpt_ptr->mo_dbm_status.msg_type == CAI_SHORT_MSG_SERVICES)
      {
        if (sms_sub_info_ptr->sms_state == CM_SMS_STATE_WAIT_FOR_STATUS)
        {
          if(cm_mc_rpt_ptr->mo_dbm_status.status == CM_DBM_MO_HOLD_ORIG)
          {
            ...
            {
              sms_sub_info_ptr->hold_orig_time_s102 = uptime + CMCALL_SR_PERIOD_TIME;
            }
            /* try to re-send MO SMS */
            cmxsms_mo_msg_retry(cm_mc_rpt_ptr->hdr.asubs_id);
          }
          else
          {
            uint8 hold_duration = CMSMS_HOLD_MODE_PREF_TIME;
            /* For below error codes, as SMS is never sent to NW,
            ** there is no need to wait for TL Ack
            */
            switch(cm_mc_rpt_ptr->mo_dbm_status.status)
            {
              ...
              case CM_DBM_MO_PREF_CHN_NOT_AVAIL:
                hold_duration = 0;
                break;
              default:
                break;
            ...
            /* Do necessary phone's MO SMS end activities */
            cmph_mo_sms_end( cm_mc_rpt_ptr->hdr.asubs_id, hold_duration, (cm_act_id_type)cmsms_ptr() );
            ...
            sms_sub_info_ptr->mo_status =
              (cm_sms_mo_status_e_type) (cm_mc_rpt_ptr->mo_dbm_status.status);
            cmph_update_ddtm_status( cm_mc_rpt_ptr->hdr.asubs_id );/* update the DDTM satus */
            cmsms_event( sms_sub_info_ptr, CM_SMS_EVENT_MO_STATUS, cm_mc_rpt_ptr->hdr.asubs_id);
        ...
        */
        ...
        {
          CM_ERR_0( "CM_MO_DBM_STATUS_F" " else make no sense" );
        }
      }
    ...
    case CM_MSG_WAITING_F:
      sms_sub_info_ptr->msgs = cm_mc_rpt_ptr->msgs.msgs;
      cmsms_event( sms_ptr, CM_SMS_EVENT_MSG_WAITING, cm_mc_rpt_ptr->hdr.asubs_id);
      break;
...
)
{
  ...
  /* Pointer to SD report */
  cmsms_sub_info_s_type *sms_sub_info_ptr = cmsms_sub_info_ptr(sd_rpt_ptr->hdr.asubs_id);
  ...
  */
  ...
  switch( sd_rpt_ptr->hdr.cmd )
  {
    ...
    case CM_OK_TO_ORIG_F:
      ...
      */
      if( sd_rpt_ptr->ok_to_orig.stack_id == MM_STACK_0&&
          sd_rpt_ptr->ok_to_orig.si_ok_to_orig.mode == SD_MODE_CDMA )
      {
        /* Send MO SMS as necessary.
        */
        sms_sub_info_ptr->is_ok_to_send = TRUE;
        cmxsms_mo_msg_retry(sd_rpt_ptr->hdr.asubs_id);
      }
      break;
#endif /* FEATURE_CDSMS */
...
)
{
  ...
  sms_sub_info_ptr->mt_sms_ptr = &mt_sms;

  cmsms_event(sms_sub_info_ptr,CM_SMS_EVENT_MT_MSG, asubs_id);
}/* cmxsms_mt_msg */
...
)
{
  ...
  sms_sub_info_ptr->mo_status =  CM_SMS_MO_ABORT;
#endif /* FEATURE_CDSMS */
