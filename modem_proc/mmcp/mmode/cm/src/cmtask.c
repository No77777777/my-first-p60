...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/mmode/cm/src/cmtask.c#7 $
...
uint32 cmtask_stop_sig_rcvd_time = 0;
...
#if defined(FEATURE_MMODE_ML_LOGGING)
#define CM_PH_CMD_OFFSET 0x8000
#define CM_CALL_CMD_OFFSET 0x4000
#define CM_SMS_CMD_OFFSET 0x2000
#define CM_SS_CMD_OFFSET 0x1000
#define CM_SUPS_CMD_OFFSET 0x0800
#define CM_AC_CMD_OFFSET 0x0400
#define CM_RPT_OFFSET 0x0200
#endif
...
/* CM_CALL_ID_MAX - Max number of calls in the sub.
** 3  - 3 phone objects for 3 possible stacks.(M+H+H2)
** 3  - 3 access control objects for 3 possible stacks.(M+H+H2)
** 3  - 3 policy manager objects for 3 possible stacks.(M+H+H2)
*/
#define CM_PH_ORIG_PARA_POOL_SIZE  (CM_CALL_ID_MAX + 9)

typedef struct
{

  boolean           is_activated;
  /* Array of prio queues */
  cm_cmd_q_type     prio_q;

} cm_stack_prio_q_s_type;

typedef struct
{

  /* Buffers for queues. */
  cm_orig_q_s_type  cm_ph_orig_arr[CM_PH_ORIG_PARA_POOL_SIZE];

  /* Number of stacks currently instantiated on this sub. */
  uint8             num_stacks;

  /* Targeted number of stacks after subscription change complete. */
  uint8             targeted_num_stacks;

  /* A bitmask of stacks awaiting deactivation. */
  uint8             stk_bm_pending_deact;

  /* A single free queue per sub.*/
  cm_cmd_q_type     cm_ph_orig_free_q;

  /* Array of prio queues */
  cm_stack_prio_q_s_type     cm_stk_q[MM_STACK_ALL];

} cm_priority_q_s_type;

static cm_priority_q_s_type *cm_prio_q[MAX_SIMS];

#define PRIO_Q(mm_id) cm_prio_q[(mm_id).asubs_id]->cm_stk_q[(mm_id).stk_id].prio_q
#define IS_Q_ACTIVE(mm_id) cm_prio_q[(mm_id).asubs_id]->cm_stk_q[(mm_id).stk_id].is_activated
/*-------------------------------------------------------------------------*/
...
void cm_cmd_dealloc( void  *ptr )
{
  ...
  */
  ...
  cm_inband_cmd_s_type *tmp_inband = NULL;
    ...
    switch(tmp->cmd_type)
    {
      case CM_CMD_TYPE_CALL:
        ...
        REF_CNT_OBJ_RELEASE_IF(
            tmp_call->info.ip_call.ipcall_attrib_info.ip_gen_param);        
        REF_CNT_OBJ_RELEASE_IF(
            tmp_call->info.ip_call.ipcall_attrib_info.call_composer_info.picture_url);
        REF_CNT_OBJ_RELEASE_IF(
            tmp_call->info.ip_call.ipcall_attrib_info.call_composer_info.subject);
		REF_CNT_OBJ_RELEASE_IF(
			tmp_call->info.ip_call.ipcall_attrib_info.call_composer_info.org_header);

		break;

      case CM_CMD_TYPE_INBAND:
        tmp_inband = (cm_inband_cmd_s_type *)ptr;
        REF_CNT_OBJ_RELEASE_IF(
            tmp_inband->info.sip_dtmf_info);   
        break;
...
FUNCTION cm_fetch_source_task

DESCRIPTION
  This function fetches the source task which is responsible for sending the command to CM.
...
#error code not present
#endif
static log_inter_layer_module_type                cm_fetch_source_task
(

  cm_cmd_type *cmd_ptr
  /* Pointer to CM command message buffer.
  */

)
{

  cm_client_type_e_type client_id_mapped = cmclient_get_client_type(cmd_ptr->client_id);

  switch(client_id_mapped)
  {
    case CM_CLIENT_TYPE_UI:
	  return LOG_ML_UI;

	case CM_CLIENT_TYPE_SMS:
	  return LOG_ML_SMS;

	case CM_CLIENT_TYPE_DS:
    case CM_CLIENT_TYPE_ATCOP:
	  return LOG_ML_DS;
  
    case CM_CLIENT_TYPE_CAT_APP:
    case CM_CLIENT_TYPE_GSTK:
	case CM_CLIENT_TYPE_GSDI:
	case CM_CLIENT_TYPE_PBM:
	  return LOG_ML_MMGSDI;

	case CM_CLIENT_TYPE_RM_NETWORK_INTERFACE:
	case CM_CLIENT_TYPE_BREW:
	case CM_CLIENT_TYPE_BREW_APPS:
	  return LOG_ML_QMI;

	case CM_CLIENT_TYPE_MCFG:
	  return LOG_ML_MCFG;

	case CM_CLIENT_TYPE_DIAG:
	  return LOG_ML_DIAG;

	case CM_CLIENT_TYPE_DTF:
	  return LOG_ML_DTF;
...
FUNCTION cm_get_ph_cmd_asub_id

DESCRIPTION
  This function fetches the as_id on which ph cmd is being sent.
...
#error code not present
#endif
static sys_modem_as_id_e_type                cm_get_ph_cmd_asub_id
(
...
{
  switch((PH_CMD_PTR(cmd_ptr))->cmd)
  {
    case CM_PH_CMD_ACTIVATE_SUBS:
    case CM_PH_CMD_DEACTIVATE_SUBS:
      if((PH_CMD_PTR(cmd_ptr))->info.cmd_subs_mask == SYS_MODEM_AS_ID_1_MASK)
	  {
        return SYS_MODEM_AS_ID_1;
	  }
	  else if((PH_CMD_PTR(cmd_ptr))->info.cmd_subs_mask == SYS_MODEM_AS_ID_2_MASK)
	  {
    ...
    case CM_PH_CMD_DDS_SWITCH_ALLOWED:
      return (PH_CMD_PTR(cmd_ptr))->info.dds_switch_allowed_info.asubs_id;

    default:
      return (PH_CMD_PTR(cmd_ptr))->info.cmd_subs;
  }
}
/*===========================================================================

FUNCTION cm_get_call_cmd_asub_id

DESCRIPTION
  This function fetches the as_id on which call cmd is being sent.
...
#error code not present
#endif
static sys_modem_as_id_e_type                cm_get_call_cmd_asub_id
(
...
)
{
  ...
  switch((CALL_CMD_PTR(cmd_ptr))->cmd)
  {
	...
	case CM_CALL_CMD_SECURE_CALL:
	case CM_CALL_CMD_SUPS:
	case CM_CALL_CMD_INCOMING_PAGE_RSP:
	case CM_CALL_CMD_PS_DATA_AVAILABLE:
	case CM_CALL_CMD_PS_SIG_REL:
	  return  (CALL_CMD_PTR(cmd_ptr))->cmd_subs;

	case CM_CALL_CMD_END:
		call_ptr_for_check=cmcall_ptr((CALL_CMD_PTR(cmd_ptr))->info.end_params[0].call_id);
		if(call_ptr_for_check == NULL)
	    {
	      return SYS_MODEM_AS_ID_NONE;
	    }
	    return call_ptr_for_check->mm_id.asubs_id;

	default:
      call_ptr_for_check=cmcall_ptr((CALL_CMD_PTR(cmd_ptr))->info.call_id);
	  if(call_ptr_for_check == NULL)
	  {
        CM_MSG_HIGH_0("The Returned AS_ID is None, since there is no call_id filled");
		return SYS_MODEM_AS_ID_NONE;
	  }
	  return call_ptr_for_check->mm_id.asubs_id;
  }	
...
#error code not present
#endif
static sys_modem_as_id_e_type                cm_fetch_as_id
(
...
)
{
  ...
  {
    case CM_CMD_TYPE_PH:
	  return cm_get_ph_cmd_asub_id(cmd_ptr);

	case CM_CMD_TYPE_CALL:
		return cm_get_call_cmd_asub_id(cmd_ptr);

	case CM_CMD_TYPE_SMS:
	  return (SMS_CMD_PTR(cmd_ptr))->cmd_subs;

	case CM_CMD_TYPE_SS:
	  return (SS_CMD_PTR(cmd_ptr))->cmd_subs;

    #ifdef CM_GW_SUPPORTED
	case CM_CMD_TYPE_SUPS:
	  return (SUPS_CMD_PTR(cmd_ptr))->cmd_info.cmd_subs_id;
    #endif
	default:
	  CM_MSG_HIGH_0("As of now, the default Case Handling is for the AC CMD");
	  return (AC_CMD_PTR(cmd_ptr))->info.asubs_id;

  }
}
/*===========================================================================

FUNCTION cm_fetch_msg_id

DESCRIPTION
  This function fetches the msg_id for which cmd is being sent.
...
)
{
  ...
  {
    case CM_CMD_TYPE_PH:
	  return ((PH_CMD_PTR(cmd_ptr))->cmd + CM_PH_CMD_OFFSET);

	case CM_CMD_TYPE_CALL:
	  return ((CALL_CMD_PTR(cmd_ptr))->cmd + CM_CALL_CMD_OFFSET);

	case CM_CMD_TYPE_SMS:
	  return ((SMS_CMD_PTR(cmd_ptr))->cmd + CM_SMS_CMD_OFFSET);

	case CM_CMD_TYPE_SS:
	  return ((SS_CMD_PTR(cmd_ptr))->cmd + CM_SS_CMD_OFFSET);

    #ifdef CM_GW_SUPPORTED
	case CM_CMD_TYPE_SUPS:
	  return ((SUPS_CMD_PTR(cmd_ptr))->cmd + CM_SUPS_CMD_OFFSET );
    #endif
	default:
	  return ((AC_CMD_PTR(cmd_ptr))->cmd + CM_AC_CMD_OFFSET);
...
void cm_cmd_queue( cm_cmd_type *cmd_ptr )
{
  ...
  /* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  #if defined(FEATURE_MMODE_ML_LOGGING)
  log_msg=(log_inter_layer_msg_type*)cm_mem_malloc(sizeof(log_inter_layer_msg_type));
  
  if( CMD_TYPE(cmd_ptr) == CM_CMD_TYPE_PH ||
	  CMD_TYPE(cmd_ptr) == CM_CMD_TYPE_CALL ||
	  CMD_TYPE(cmd_ptr) == CM_CMD_TYPE_SMS ||
	  CMD_TYPE(cmd_ptr) == CM_CMD_TYPE_SS ||
	  CMD_TYPE(cmd_ptr) == CM_CMD_TYPE_SUPS ||
	  CMD_TYPE(cmd_ptr) == CM_CMD_TYPE_AC)
  {
    log_msg->msg_id = cm_fetch_msg_id(cmd_ptr);
    log_msg->src_sub_id = cm_fetch_as_id(cmd_ptr);
    log_msg->dest_sub_id = cm_fetch_as_id(cmd_ptr);
    log_msg->source_module = cm_fetch_source_task(cmd_ptr);

    log_msg->dest_module = LOG_ML_CM;
...
DESCRIPTION
 Count the number of commands in hold command queue of specific ph cmd type.
 eg: sys_sel_pref_cmd
...
uint8 cm_hold_cmd_q_cnt_of_type(

  const void *cmd
  /* Type of phone command
  */
)
{
  int count = 0;
  cm_cmd_type   *hold_cmd_ptr = (cm_cmd_type *)q_check(&cm_hold_cmd_q);
  ...
  while( hold_cmd_ptr != NULL && count < q_cnt(&cm_hold_cmd_q))
  {
    if( CMD_TYPE(hold_cmd_ptr) == CM_CMD_TYPE_PH &&
        PH_CMD_PTR(hold_cmd_ptr)->cmd ==  *(cm_ph_cmd_e_type *)cmd
      )
    {
      count++;
    }
    hold_cmd_ptr = (cm_cmd_type *)q_next(&cm_hold_cmd_q, &hold_cmd_ptr->link);
  }
...
===========================================================================*/
rex_timer_cnt_type cmtask_attach_complete_timer_start(
  unsigned long time,
...
)
{
  ...
  CM_MSG_HIGH_5("ATTACH: Starting ATTACH COMPLETE_IND timer %d ms, sv_oprt_mode %d %d, enabled %d, sub %d", 
                   time, 
...
void cmtask_attach_complete_timer_clear(
  sys_modem_as_id_e_type asubs_id
)
{
  cmsoa_state_sub_info_s_type     *cmsoa_state = cmsoa_state_sub_info_ptr(asubs_id);
  ...
  CM_MSG_HIGH_0("ATTACH: Clearing ATTACH COMPLETE IND timer");
...
FUNCTION cmtask_add_prio_q

DESCRIPTION
  When new priority queues need to be added to the subscription.
...
===========================================================================*/
void                       cmtask_add_prio_q(

  sys_modem_as_id_e_type     asubs_id,

  uint8                      new_stack_bitmask
)
{
  uint8 stk;

  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if( asubs_id >= MAX_SIMS || asubs_id < SYS_MODEM_AS_ID_1 || cm_prio_q[asubs_id] == NULL)
  {
    CM_ERR_2("PRIO_Q: Invalid param. Unable to update the stack mask for sub %d to 0x%0x",
             asubs_id, new_stack_bitmask);
    return;
  }

  CM_MSG_HIGH_6_ASID("PRIO_Q: new_stack_bitmask 0x%x, curr stk_bm_pending_deact 0x%x, is_activated %d %d %d",
                     asubs_id,
                     new_stack_bitmask,
                     cm_prio_q[asubs_id]->stk_bm_pending_deact,
                     cm_prio_q[asubs_id]->cm_stk_q[MM_STACK_0].is_activated,
                     cm_prio_q[asubs_id]->cm_stk_q[MM_STACK_1].is_activated,
                     cm_prio_q[asubs_id]->cm_stk_q[MM_STACK_2].is_activated);

  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  for( stk=MM_STACK_0; stk < MM_STACK_ALL; stk++ )
  {
    /* If the stack was inactive activate it. */
    if( (new_stack_bitmask & BM(stk)) &&
        cm_prio_q[asubs_id]->cm_stk_q[stk].is_activated != TRUE )
    {
      cm_prio_q[asubs_id]->cm_stk_q[stk].is_activated = TRUE;
      (void)q_init( &PRIO_Q(cmph_map_sub_stk_to_id(asubs_id,stk)));

      /* Insert the phone object onto it but dont force any prefs below. */
      cmph_orig_mode_insert_new( cmph_map_sub_stk_to_id(asubs_id,stk),
                                 CM_DEFAULT_VALUE );
    }

    /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
    /* If stack was marked for deactivation, it is no longer to be deactivated. */
    if( (new_stack_bitmask & BM(stk)) &&
        (cm_prio_q[asubs_id]->stk_bm_pending_deact & BM(stk))
      )
    {
      CM_MSG_HIGH_2_ASID("PRIO_Q: num_stack_chgd: stk %d no longer to be deactivated",
                         asubs_id, stk);
      cm_prio_q[asubs_id]->stk_bm_pending_deact &= ~(BM(stk));
    }

    /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
    /* If stack is currently active, and new bitmask does not contain it,
...
===========================================================================*/
void                       cmtask_remove_prio_q(

  sys_modem_as_id_e_type     asubs_id,

  uint8                      stk_active_in_ll_bitmask
)
{
  uint8 stk;
  uint8 deact_cnt = 0;
  ...
  if(asubs_id >= MAX_SIMS || asubs_id < SYS_MODEM_AS_ID_1 || cm_prio_q[asubs_id] == NULL)
  {
    CM_ERR_2("PRIO_Q: Inavlid param. Unable to update the stack mask for sub %d to 0x%0x",
             asubs_id,stk_active_in_ll_bitmask);
    return;
  }
  ...
  for( stk=MM_STACK_0; stk < MM_STACK_ALL; stk++ )
  {
    /* If the stack is pending deactivation and is not active in the bitmask
    ** received from lower layers, deactivate the stack.
    */
    if( !(stk_active_in_ll_bitmask & BM(stk)) &&
        (cm_prio_q[asubs_id]->stk_bm_pending_deact & BM(stk))
      )
    {
      mm_sub_stk_id_s_type local_mm_id = cmph_map_sub_stk_to_id(asubs_id,stk);

      /* Clear out all the objects on the queue. */
      cmtask_orig_para_reset(local_mm_id, FALSE);

      /* De-activate the stack. */
      cm_prio_q[asubs_id]->cm_stk_q[stk].is_activated = FALSE;
      cm_prio_q[asubs_id]->stk_bm_pending_deact &= ~(BM(stk));
      deact_cnt++;
    }
  }
  cm_prio_q[asubs_id]->num_stacks -= deact_cnt;

}

/*===========================================================================

FUNCTION cmtask_init_priority_q

DESCRIPTION
  At task initialisation, intialise priority queues for number of Sims there
  will be as per Policy Manager.
...
===========================================================================*/
static void cmtask_init_priority_q(size_t nSims)
{
  uint8 sub;

  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if(nSims == 0 || nSims > MAX_SIMS)
  {
    CM_ERR_FATAL_1("Attempted to create priority queues with invalid Sims %u",nSims);
  }

  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  /* For each sub do the following. */
  for( sub=0; sub < nSims; sub++ )
  {
    int idx1;

    cm_prio_q[sub] = (cm_priority_q_s_type *)cm_mem_malloc(sizeof(cm_priority_q_s_type));
    /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
    /* Initialise the free queue. */
    (void)q_init(&(cm_prio_q[sub]->cm_ph_orig_free_q ));

    /* Link and place each of the orig para buffers into the free queue. */
    for( idx1=0; idx1 < ARR_SIZE(cm_prio_q[sub]->cm_ph_orig_arr); idx1++ )
    {
      (void)q_link(&(cm_prio_q[sub]->cm_ph_orig_arr[idx1] ), &(cm_prio_q[sub]->cm_ph_orig_arr[idx1].link));
      q_put( &(cm_prio_q[sub]->cm_ph_orig_free_q ), &(cm_prio_q[sub]->cm_ph_orig_arr[idx1].link));
    }

    /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
    /* Determine the number of stacks. At task initialization this should be 1.*/

    cm_prio_q[sub]->num_stacks = 0;
    cm_prio_q[sub]->targeted_num_stacks = 0;
    cm_prio_q[sub]->stk_bm_pending_deact = 0;
...
FUNCTION cmtask_map_num_stacks_to_bitmask

DESCRIPTION
  Map number of stacks to bitmask of active stacks.
...
===========================================================================*/
uint8 cmtask_map_num_stacks_to_bitmask( uint8 num_stacks )
{
  switch( num_stacks )
  {
    case 3:
      return (uint8)(MM_STACK_0_MASK|MM_STACK_1_MASK|MM_STACK_2_MASK);
    case 2:
      return (uint8)(MM_STACK_0_MASK|MM_STACK_1_MASK);
    case 1:
    default:
      return (uint8)(MM_STACK_0_MASK);
  }
}

/*===========================================================================

FUNCTION cmtask_update_new_prio_q

DESCRIPTION
  Read the new number of stacks as derived from data in CMPMPRX and activate
  the still inactive stacks.
...
===========================================================================*/
void  cmtask_update_new_prio_q(
  sys_modem_as_id_e_type          asubs_id,
  boolean                         is_update_cmss,
  uint8                           active_subs
)
{
  uint8 new_num_stacks = 0;
  uint8 stk_bm = 0;
  uint8 curr_num_stacks = 0;

  if(cm_prio_q[asubs_id] != NULL)
  {
    curr_num_stacks = cm_prio_q[asubs_id]->num_stacks;
    new_num_stacks = cmpmprx_determine_num_stacks(asubs_id, active_subs);

    /* no num stack change and no pending deactivation */
    if(curr_num_stacks == new_num_stacks && cm_prio_q[asubs_id]->stk_bm_pending_deact == 0)
    {
      return;
    }

    cm_prio_q[asubs_id]->targeted_num_stacks = new_num_stacks;

    /* If more stacks are needed, allocate and change num_stacks now.
    ** If less stacks are needed, do not change num_stacks until later.
    */
    if(curr_num_stacks < new_num_stacks)
    {
      cm_prio_q[asubs_id]->num_stacks = new_num_stacks;
    }
    stk_bm = cmtask_map_num_stacks_to_bitmask(new_num_stacks);
    cmtask_add_prio_q(asubs_id,stk_bm);

    if (curr_num_stacks != new_num_stacks)
    {
      CM_MSG_HIGH_5_ASID("num_stack_chgd: old_num_stacks %d, new_num_stacks %d, updated num_stacks %d, stk_bm_pending_deact 0x%x",
                         asubs_id, curr_num_stacks, new_num_stacks,
                         cm_prio_q[asubs_id]->num_stacks,
                         cm_prio_q[asubs_id]->stk_bm_pending_deact);
    }
    if(is_update_cmss)
    {
      cmss_update_stack_upon_subsc_chg(asubs_id, new_num_stacks);
    }
  }
}
...
boolean cmtask_is_valid_mm_id_for_prio_q(mm_sub_stk_id_s_type mm_id)
{
  return ( mm_id.asubs_id > SYS_MODEM_AS_ID_NONE &&
           mm_id.asubs_id < MAX_SIMS &&
           (uint8)mm_id.asubs_id < cmpmprx_get_num_of_sims() &&
           cm_prio_q[mm_id.asubs_id] != NULL &&
           (cm_prio_q[mm_id.asubs_id]->num_stacks > mm_id.stk_id ||
...
FUNCTION cmtask_get_num_stacks

DESCRIPTION
  Get the numer of priority queues per sub..

DEPENDENCIES
  cm_prio_q should be initialized.
...
===========================================================================*/
uint8 cmtask_get_num_stacks(sys_modem_as_id_e_type asubs_id)
{
  if(cmutil_is_as_id_valid(asubs_id) && cm_prio_q[asubs_id] != NULL)
  {
    return(cm_prio_q[asubs_id]->num_stacks);
  }
  return 0;
}
...
FUNCTION cmtask_get_targeted_num_stacks

DESCRIPTION
  Get the targeted num of stacks per sub.
...
===========================================================================*/
uint8 cmtask_get_targeted_num_stacks(sys_modem_as_id_e_type asubs_id)
{
  if(cmutil_is_as_id_valid(asubs_id) && cm_prio_q[asubs_id] != NULL)
  {
    return(cm_prio_q[asubs_id]->targeted_num_stacks);
  }
  return 0;
}
...
FUNCTION cmtask_is_stack_pending_for_deact

DESCRIPTION
  check if the stack is pending for deactivation
  (waiting for CM_SUBS_CHGD_DONE from MMOC)
...
===========================================================================*/
boolean cmtask_is_stack_pending_for_deact(mm_sub_stk_id_s_type mm_id)
{
  if(mm_id.asubs_id > SYS_MODEM_AS_ID_NONE && mm_id.asubs_id < MAX_SIMS
      && mm_id.stk_id >= MM_STACK_0 && mm_id.stk_id < MM_STACK_ALL
      && cm_prio_q[mm_id.asubs_id] != NULL)
  {
    return (cm_prio_q[mm_id.asubs_id]->targeted_num_stacks <= mm_id.stk_id);
  }
  return FALSE;
}
...
===========================================================================*/
cm_orig_q_s_type *cmtask_orig_para_alloc( mm_sub_stk_id_s_type mm_id )
{
  ...
  if( cmtask_is_valid_mm_id_for_prio_q(mm_id) &&
      mm_id.stk_id != MM_STACK_ALL )
  {
    orig_q = cmtask_orig_para_get( &(cm_prio_q[mm_id.asubs_id]->cm_ph_orig_free_q));
  }
...
)
{
  ...
  return( q_orig_ptr->orig->act_priority > ph_orig_ptr->orig->act_priority );
...
)
{
  ...
  cm_cmd_q_type        *temp_q_ptr  = NULL;
  /* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  if( cmtask_is_valid_mm_id_for_prio_q(mm_id) &&
      mm_id.stk_id != MM_STACK_ALL &&
      IS_Q_ACTIVE(mm_id))
  {
    temp_q_ptr = &(PRIO_Q(mm_id));
  }

  if(temp_q_ptr != NULL)
  {
    item_ptr = (q_generic_item_type *)q_check( temp_q_ptr );
    ...
    while( item_ptr != NULL )
    {
      ...
      item_ptr = (q_generic_item_type *)q_next( (q_type *)temp_q_ptr, &item_ptr->link );
    }
  }
...
)
{
  ...
  */
  while(q_ptr != NULL)
  {
    ...
    q_ptr = cmtask_orig_para_get_next(q_ptr, mm_id);
  }
...
FUNCTION cmtask_is_there_a_high_priority_activity_with_exclusions

DESCRIPTION
  Checks if there is a high priority activity than the cmp_act_type in UE except the "excl_act_type"
...
==============================================================================*/
boolean cmtask_is_there_a_high_priority_activity_with_exclusions
(
  mm_sub_stk_id_s_type mm_id,
  /* SS to select the queue
  */
  ...
  */
  ...
  cm_act_type_e_type  excl_act_type
  /* Activity type to be excluded in the comparision
  */
...
)
{
  ...
  */
  while(q_ptr != NULL)
  {
    ...
    if( (q_ptr->act_type != excl_act_type) &&
        (q_ptr->act_type < cmp_act_type )
      )
    {
      CM_MSG_HIGH_2("Act_type %d of priority greater than act_type %d in the queue",
...
)
{
  ...
  if( cmtask_is_valid_mm_id_for_prio_q(mm_id) )
  {
    if(IS_Q_ACTIVE(mm_id))
    {
      return cmtask_orig_para_search_act_id_2(&PRIO_Q(mm_id),act_id);
    }
  } /* Valid mm_id. */
  ...
  cm_act_update_reas_e_type act_update_reas)
{
  ...
  ph_orig_ptr = cmtask_orig_para_search_act_id(mm_id, (cm_act_id_type)cmph_ptr() );

  if( ph_sub_ptr != NULL && ph_orig_ptr != NULL && ph_orig_ptr->orig != NULL &&
      (reason == SD_SS_PREF_REAS_3GPP_PS_CALL_CONNECT || ph_sub_ptr->force_data_activity_pref_ongoing == TRUE) )
  {
...
)
{
  ...
  CM_MSG_LOW_4( "cmtask_orig_para_search_act_id act_id %d act_priority %d subs %d stk %d",
                act_id, act_priority, mm_id.asubs_id, mm_id.stk_id );
  ...
  if( temp_q_ptr != NULL )
  {
    return cmtask_orig_para_search_act_id_act_priority_2( temp_q_ptr,
           act_id,
           act_priority );
  }
  return NULL;
...
FUNCTION cmtask_remove_orig_csg_params
...
===========================================================================*/
void cmtask_remove_orig_csg_params(
...
{
  cm_orig_q_s_type      *q_ptr, *q_next_ptr;

  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

  CM_MSG_HIGH_3("cmtask_remove_orig_csg_params for subs %d stk %d csg_rat %d",
                mm_id.asubs_id, mm_id.stk_id, csg_rat);
...
)
{
  ...
  while( q_ptr != NULL &&
         cmutil_cmp_act_priorities(q_ptr->orig->act_priority, act_priority ) > 0 )
  {
    act_count++;
    q_ptr = cmtask_orig_para_get_next(q_ptr, mm_id);
  }
...
)
{
  ...
  /* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  for(; asubs_id > SYS_MODEM_AS_ID_NONE &&
      asubs_id < MAX_SIMS &&
      (uint8)asubs_id < cmpmprx_get_num_of_sims(); asubs_id++)
  {
    multimode_stack_e_type stk_id = MM_STACK_0;
    while( stk_id < cm_prio_q[asubs_id]->num_stacks )
    {
      mm_sub_stk_id_s_type local_mm_id;
      local_mm_id.asubs_id = asubs_id;
      local_mm_id.stk_id   = stk_id;

      if( cmtask_orig_para_search_act_type( local_mm_id,obj_act_type) != NULL )
      {
        return TRUE;
      }
      stk_id++;
...
)
{
  if( cmtask_is_valid_mm_id_for_prio_q(mm_id) &&
      ...
      IS_Q_ACTIVE(mm_id))
  {
    return PRIO_Q(mm_id).cnt;
  }

  CM_MSG_HIGH_2("cmtask_orig_para_count: Invalid or NULL subs %d or stack %d", mm_id.asubs_id,mm_id.stk_id);
  return 0;
...
)
{
  ...
  /* If the mm_id is invalid, or all stacks are selected or the selected stack
  ** is not active return error.
  */
  if( !cmtask_is_valid_mm_id_for_prio_q(mm_id)||
      !(IS_Q_ACTIVE(mm_id))
    )
  ...
  q_ptr           = cmtask_orig_para_get_first( mm_id );
  total_act_count = cmtask_orig_para_count( mm_id );
...
)
{
  ...
  if( temp_q_ptr != NULL )
  {
    return cmtask_orig_para_search_act_type_2( temp_q_ptr, act_type );
  }
...
)
{
  ...
  {
    CM_MSG_MED_3( "cmtask_orig_para_search_act_type %d ss %d sub %d",
                  act_type, mm_id.stk_id,mm_id.asubs_id);
  }
...
)
{
  cm_cmd_q_type        *temp_q_ptr = NULL;
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  CM_MSG_LOW_3("START cmtask_orig_para_get_first() sub %d ss %d orig %d",mm_id.asubs_id,mm_id.stk_id, orig_mode );
  ...
  if(temp_q_ptr != NULL)
  {
    return cmtask_orig_para_search_orig_mode_2(temp_q_ptr,orig_mode);
  }
...
===========================================================================*/
static cm_srv_domain_pref_e_type cmtask_add_srv_domain_pref(
...
)
{
  ...
  cm_orig_q_s_type *q_ptr = cmtask_orig_para_get_first(mm_id);
  ...
  while(q_ptr != NULL)
  {
    ...
    */
    ...
    if( (cmutil_cmp_act_priorities(q_ptr->orig->act_priority, CM_ACT_PRIORITY_PH) >= 0 )
#ifdef CM_GW_SUPPORTED
        || ( cmutil_cmp_act_priorities(q_ptr->orig->act_priority, CM_ACT_PRIORITY_BELOW_PH) == 0 &&
             q_ptr->act_type == CM_ACT_TYPE_DATA_CALL
           )
#endif
      )
    {
      orig_srv_domain_pref = cmtask_add_srv_domain_pref(
...
)
{
  ...
  {
    CM_MSG_HIGH_5_ASID( "cmtask_orig_para_delete act_type %d act_id %d stk %d num_stacks %d",
                        mm_id.asubs_id, ph_orig_ptr->act_type, ph_orig_ptr->orig->act_id, mm_id.stk_id,
      ...
      IS_Q_ACTIVE(mm_id) )
  {
    top_ptr_changed = cmtask_orig_para_delete_2( ph_orig_ptr,
                      &(PRIO_Q(mm_id)),
                      &(cm_prio_q[mm_id.asubs_id]->cm_ph_orig_free_q),
                      free_buf );
  }
...
)
{
  ...
  if(ph_orig_ptr->act_type != CM_ACT_TYPE_PH_OBJ)
  {
    CM_MSG_HIGH_5_ASID( "cmtask_orig_para_insert act_type %d act_id %d stk %d num_stacks %d",
                        mm_id.asubs_id, ph_orig_ptr->act_type,
                        ph_orig_ptr->orig->act_id, mm_id.stk_id,
                        cmtask_get_num_stacks(mm_id.asubs_id));
  }
  ...
  if( cmtask_is_valid_mm_id_for_prio_q(mm_id) &&
      ...
      IS_Q_ACTIVE(mm_id) )
  {
    return cmtask_orig_para_insert_2( ph_orig_ptr, &(PRIO_Q(mm_id)) );
  }
...
)
{
  ...
  if(temp_q_ptr != NULL)
  {
    return( (cm_orig_q_s_type *) q_check( temp_q_ptr ) );
  }
  return NULL;
...
)
{
  ...
  if(temp_q_ptr != NULL)
  {
    return ( (cm_orig_q_s_type *) q_next( temp_q_ptr, &q_ptr->link ) );
  }
  return NULL;
...
)
{
  ...
  cm_orig_q_s_type  *orig_q_obj_ptr = NULL;
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  while( (orig_q_obj_ptr =
            cmtask_orig_para_search_act_type_act_priority( mm_id,
                act_type,
                curr_priority )) != NULL )
  {
    ...
    */
    found_one = TRUE;
    (void)cmtask_orig_para_delete(mm_id, orig_q_obj_ptr, FALSE);

    orig_q_obj_ptr->orig->act_priority = new_priority;

    (void)cmtask_orig_para_insert( mm_id, orig_q_obj_ptr );
...
)
{
  ...
  CM_MSG_LOW_3( "cmtask_orig_para_search_act_type_mode_pref %d subs %d stk %d", act_type, mm_id.asubs_id,mm_id.stk_id);
  ...
  if(temp_q_ptr != NULL)
  {
    return cmtask_orig_para_search_act_type_mode_pref_2( mm_id.asubs_id,
           temp_q_ptr,
           act_type,
           mode_pref);
  }
...
)
{
  ...
  CM_MSG_LOW_3( "cmtask_orig_para_search_act_type_orig_mode %d subs %d ss %d", act_type, mm_id.asubs_id,mm_id.stk_id);
  ...
  if(temp_q_ptr != NULL)
  {
    return cmtask_orig_para_search_act_type_orig_mode_2( temp_q_ptr,
           act_type,
           orig_mode );
  }
  return NULL;
...
)
{
  ...
  while( (orig_q_obj_ptr =
            cmtask_orig_para_search_act_type_orig_mode( mm_id,
                act_type,
                orig_mode )) != NULL )
  {
    found_one = TRUE;
    ...
    */
    orig_q_obj_ptr->orig->orig_mode = orig_mode;
...
)
{
  ...
  cm_orig_q_s_type  *orig_q_ptr = NULL;
  cmph_s_type        *ph_ptr    = cmph_ptr();
  boolean            found_one  = FALSE;
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  while( (orig_q_ptr =
            cmtask_orig_para_search_act_type_mode_pref( mm_id,
                act_type,
                mode_pref )) != NULL )
  {
    ...
    */
    found_one = TRUE;
    (void)cmtask_orig_para_delete(mm_id, orig_q_ptr, FALSE);

    orig_q_ptr->orig->orig_mode_pref = mode_pref;
    orig_q_ptr->orig->orig_band_pref = CMPH_SS_BAND_PREF(mm_id.asubs_id, ph_ptr);
    orig_q_ptr->orig->user_mode_pref = CMPH_SS_MODE_PREF(mm_id.asubs_id, ph_ptr);;

    orig_q_ptr->orig->orig_lte_band_pref = CMPH_SS_LTE_BAND_PREF(mm_id.asubs_id, ph_ptr);
    orig_q_ptr->orig->orig_tds_band_pref = CMPH_SS_TDS_BAND_PREF(mm_id.asubs_id, ph_ptr);

    (void)cmtask_orig_para_insert( mm_id, orig_q_ptr );
...
)
{
  ...
  if( !cmtask_is_valid_mm_id_for_prio_q(mm_id) )
  {
    CM_MSG_HIGH_2("cmtask_change_act_priority: invalid sub %d or stack %d",mm_id.asubs_id,mm_id.stk_id);
    return FALSE;
  }
  ...
  if( mm_id.stk_id == MM_STACK_ALL )
  {
    ...
    for(stk_idx = 0; stk_idx < cm_prio_q[mm_id.asubs_id]->num_stacks; stk_idx++)
    {
      mm_sub_stk_id_s_type local_mm_id;

      local_mm_id.asubs_id = mm_id.asubs_id;
      local_mm_id.stk_id = stk_idx;

      is_priority_changed |= cmtask_orig_para_change_act_priority_2( local_mm_id,
                             act_type,
                             curr_priority,
                             new_priority,
                             global_change );
  ...
  {
    is_priority_changed = ( cmtask_orig_para_change_act_priority_2( mm_id,
                            act_type,
                            curr_priority,
                            new_priority,
                            global_change ));
  ...
  {
    CM_MSG_MED_4( "cmtask_change_act_priority act_type %d sub %d ss %d new_priority %d",
                  act_type, mm_id.asubs_id,mm_id.stk_id, new_priority );
...
===========================================================================*/
boolean cmtask_orig_para_change_act_priority_4(
...
)
{
  ...
  {
    orig_q_ptr = cmtask_orig_para_search_act_id_act_priority( mm_id,
                 act_id,
                 curr_priority );
  }
  else
  {
    orig_q_ptr = cmtask_orig_para_search_act_id( mm_id, act_id );
  }
...
void cmtask_orig_para_reset(
  ...
  mm_sub_stk_id_s_type      mm_id,
  /* The sub/stack id for which the Q is to be reset
  */
...
)
{
  ...
  /* Just check that the prio_q is allocated. We might be here when the sub/q
  ** might no longer be valid and we are trying to clear the data and memory.
  */
  if( !(cm_prio_q[mm_id.asubs_id]!= NULL &&
        mm_id.stk_id != MM_STACK_ALL &&
        IS_Q_ACTIVE(mm_id) ))
  {
    return;
  ...
  /* Go through the orig queues and if a entry is found delete it
  */
  q_ptr = cmtask_orig_para_get_first( mm_id );

  while(q_ptr != NULL)
  {
    cm_orig_q_s_type *q_next_ptr = cmtask_orig_para_get_next( q_ptr, mm_id );
...
*/
{
  ...
  */
  ...
  cm_rpt_ptr->srv_ind_info.stack_id = mm_id.stk_id;
  cm_rpt_ptr->srv_ind_info.si_info  = *si_info_ptr;
  cm_rpt_ptr->hdr.asubs_id          = mm_id.asubs_id;
...
)
{
  ...
  */
  ...
  cm_rpt_ptr->acq_fail.hdr.asubs_id    = mm_id.asubs_id;
  cm_rpt_ptr->acq_fail.stack_id        = mm_id.stk_id;
...
)
{
  ...
  */
  ...
  cm_rpt_ptr->ok_to_orig.hdr.asubs_id   = mm_id.asubs_id;
  cm_rpt_ptr->ok_to_orig.stack_id       = mm_id.stk_id;
...
)
{
  ...
  */
  ...
  cm_rpt_ptr->hdr.asubs_id = asubs_id;
...
)
{
  ...
  */
  ...
  cm_rpt_ptr->hdr.asubs_id  = mm_id.asubs_id;
  cm_rpt_ptr->cs_emerg_scan_fail.stack_id = mm_id.stk_id;
...
FUNCTION cm_emerg_srch_fail_cb

DESCRIPTION
  EMERG Scan fail report from SD

  CM registers this function with System Determination (SD) during power-up
  initialization.

  SD calls on this function whenever when one round scan finishes and no service
  found during E911 call
...
===========================================================================*/
void cm_emerg_srch_fail_cb(
    mm_sub_stk_id_s_type  mm_id,

    sd_ss_mode_pref_e_type mode_pref
    /*ACQ failed on above mode(s)*/
)
{
  ...
  /* Queue EMERG SRCH FAIL command to CM.
  */
  cm_rpt_ptr->emerg_srch_fail.hdr.cmd  = CM_EMERG_SRCH_FAIL_F;
  cm_rpt_ptr->hdr.asubs_id = mm_id.asubs_id;
  cm_rpt_ptr->emerg_srch_fail.mode_pref = mode_pref;
...
*/
{
  ...
  */
  cm_rpt_ptr->hdr.asubs_id  = emcb_info.mm_id.asubs_id;
  cm_rpt_ptr->e911_callback.hdr.cmd = CM_E911_CALLBACK_F;
  cm_rpt_ptr->e911_callback.emcb_info = emcb_info;
  cm_sd_rpt( cm_rpt_ptr );
...
void cm_process_answer_cmd_on_ims_cache(void)
{
  ...
  if((cmims_cache_ind.state == CMWCALL_CACHE_EXPECTED ||
      cmims_cache_ind.state == CMWCALL_CACHE_RECIEVED) &&
...
)
{
  ...
  if(ph_sub_ptr != NULL)
  {
    cmims_cache_ind.cache_expiry_timer = (time_get_uptime_secs()*1000) +
                                         ph_sub_ptr->ims_cache_expiry_duration;
...
*/
{
  ...
  is_1xcsfb_mode = cmsds_is_in_1xcsfb_mode(mm_id.asubs_id);
#endif

  /* Should be modified to inform MMOC instead.
  */
  if ( mm_id.asubs_id == cmmsc_get_1x_sub() &&
       mm_id.stk_id == MM_STACK_0
       &&
       (mode == SD_MODE_GSM ||
        ((mode == SD_MODE_WCDMA ||
...
/*lint -esym(715,ss) */
{
  ...
  */
  ...
  cm_rpt_ptr->hdr.asubs_id = mm_id.asubs_id;
  cm_rpt_ptr->asubs_id = mm_id.asubs_id;
  cm_rpt_ptr->cmd.data_suspend_chgd.data_suspend       = data_suspend;
  cm_rpt_ptr->cmd.data_suspend_chgd.stack_id           = mm_id.stk_id;


  CM_MSG_HIGH_3_ASID("SD->CM: RXD: CM_DATA_SUSPEND_CHGD_F rpt on stack: %d with data_suspend %d",
                     mm_id.asubs_id, mm_id.stk_id, data_suspend);

  cm_put_cmd( cm_rpt_ptr );
#endif
} /* cm_si_data_suspend_cb_ss() */
/*lint +esym(715,data_suspend) */
/*lint +esym(715,ss) */
...
)
{
  ...
  */
  ...
  cm_rpt_ptr->hdr.asubs_id = mm_id.asubs_id;
  cm_rpt_ptr->ss_stack_status.stack_id= mm_id.stk_id;
...
)
{
  ...
  switch (sd_generic_data->type)
  {
    ...
    {
      cmss_send_mcc_msg(&sd_generic_data->data.mcc_rpt, mm_id);
      break;
    }
    ...
    {
      cm_generic_rpt_s_type *cm_rpt_ptr = cm_generic_rpt_get_buf_else_err_fatal();
      cm_rpt_ptr->hdr.cmd = CM_ATTACH_COMPLETE_RPT;
      cm_rpt_ptr->hdr.asubs_id = mm_id.asubs_id;
      cm_generic_rpt( cm_rpt_ptr );
      break;
    }

    case SD_GENERIC_CB_E911_LTE_FULL_SRV_ADDED:
    {
      cm_sd_rpt_u_type *cm_rpt_ptr = cm_sd_rpt_get_buf_else_err_fatal();
      cm_rpt_ptr->e911_lte_full_srv_added.hdr.cmd = CM_E911_LTE_FULL_SRV_ADDED_F;
      cm_rpt_ptr->hdr.asubs_id = mm_id.asubs_id;
	  if(sd_generic_data->data.lte_full_srv_added_rpt.mode == SD_MODE_LTE)
	  	{
		 cm_rpt_ptr->e911_lte_full_srv_added.mode = SD_MODE_LTE;
		 cm_rpt_ptr->e911_lte_full_srv_added.lte_band = sd_generic_data->data.lte_full_srv_added_rpt.lte_band;
	  	}
...
{
  cell_info_type cell_info;
  memset((void *)&cell_info,0x00,sizeof(cell_info_type));
...
)
{
  ...
  */
  ...
  cm_send_rr_cell_info_rpt(SYS_MODEM_AS_ID_1,
#ifdef FEATURE_CM_NEW_RR_CELL_INFO_INTERFACE
                           scell_info
#else
                           cell_info
...
DESCRIPTION
  ...
  RR calls on this function whenever ARFCN list changes which includes
    pcell arfcn, nbr arfcn list and hopping arfcn list.

  CM sends msgr indication to QMI NAS
...
===========================================================================*/
void cm_arfcn_list_rr_cb(

  eng_mode_all_frequency_list_T          arfcn_list,
...
)
{
  ...
  mmode_qmi_nas_arfcn_list_ind_msg_type *qmi_nas_arfcn_list;

  qmi_nas_arfcn_list = (mmode_qmi_nas_arfcn_list_ind_msg_type *)cm_mem_malloc(
                          sizeof(mmode_qmi_nas_arfcn_list_ind_msg_type));

  if(qmi_nas_arfcn_list == NULL)
  {
    CM_ERR_0("Heap exhausted, cm_mem_malloc() returned NULL");
    return;
  }

  qmi_nas_arfcn_list->arfcn_list = arfcn_list;

  #ifdef TEST_FRAMEWORK
  #error code not present
#else
  (void) cm_msgr_send( QMI_NAS_ARFCN_LIST_IND, MSGR_MM_CM,
                       (msgr_hdr_s *)qmi_nas_arfcn_list, sizeof(mmode_qmi_nas_arfcn_list_ind_msg_type),
                       asubs_id);
  #endif
  
  CM_MSG_HIGH_1("CM->MSGR: QMI_NAS_ARFCN_LIST_IND sub %d",
                asubs_id);
  
  cm_mem_free(qmi_nas_arfcn_list);

}/* cm_arfcn_list_rr_cb */

/*===========================================================================
FUNCTION  cm_arfcn_list_rr_cb_sub1
...
void cm_arfcn_list_rr_cb_sub1(

  eng_mode_all_frequency_list_T          arfcn_list

)
{
  cm_arfcn_list_rr_cb(arfcn_list, SYS_MODEM_AS_ID_1);
}
/*===========================================================================
FUNCTION  cm_arfcn_list_rr_cb_sub2
...
===========================================================================*/
void cm_arfcn_list_rr_cb_sub2(

  eng_mode_all_frequency_list_T          arfcn_list

)
{
  cm_arfcn_list_rr_cb(arfcn_list, SYS_MODEM_AS_ID_2);
}
...
)
{
  ...
  */
  ...
  cm_send_rr_cell_info_rpt(SYS_MODEM_AS_ID_2,
#ifdef FEATURE_CM_NEW_RR_CELL_INFO_INTERFACE
                           scell_info
#else
                           cell_info
...
)
{
  ...
  */
  ...
  cm_send_rr_cell_info_rpt(SYS_MODEM_AS_ID_3,
#ifdef FEATURE_CM_NEW_RR_CELL_INFO_INTERFACE
                           scell_info
#else
                           cell_info
...
)
{
  ...
  */
  ...
  cm_rpt_ptr->hdr.asubs_id       = mm_id.asubs_id;
  cm_rpt_ptr->srv_lost.stack_id  = mm_id.stk_id;
...
)
{
  ...
  //CM_MSG_HIGH_2("full_srv_lost cb called, sub %d stk %d", mm_id.asubs_id, mm_id.stk_id);

  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

  cm_rpt_ptr->full_srv_lost.hdr.cmd        = CM_FULL_SRV_LOST_F;
  cm_rpt_ptr->full_srv_lost.hdr.asubs_id   = mm_id.asubs_id;
  cm_rpt_ptr->full_srv_lost.stack_id       = mm_id.stk_id;
...
#endif
{
  ...
  if(qrarb_cb_info.client_id != QRARB_CLNT_CM ||
      qrarb_cb_info.qlink_error != QRARB_QLINK_CRITICAL_ERROR ||
      qrarb_cb_info.qrarb_state != QRARB_STATE_QLINK_RESTART_START )
  {
    CM_MSG_HIGH_3("QRARB unregistered client_id %d error %d state %d",
                  qrarb_cb_info.client_id,qrarb_cb_info.qlink_error,qrarb_cb_info.qrarb_state );
    return(FALSE);
  }

  if ((cm_rpt_ptr = cm_generic_rpt_get_buf_else_err_fatal())!=NULL)
  {
    cm_rpt_ptr->hdr.cmd = CM_QRARB_RESTART_RPT;
    cm_rpt_ptr->generic_rpt.qrarb_info.qrarb_restart_id = qrarb_cb_info.qrarb_restart_id;
    cm_generic_rpt(cm_rpt_ptr);
  }
#endif
  return TRUE;
...
FUNCTION cm_qrarb_register

DESCRIPTION
  Function used to register with QRARB module for notification of
  QLINK critical errors
...
===========================================================================*/
void cm_qrarb_register(void)
{

#ifdef FEATURE_DISABLED_HABANERO

  qrarb_reg_cb_info_t reg_parm;

  memset((void *)&reg_parm,0x00,sizeof(qrarb_reg_cb_info_t));

  reg_parm.client_id = QRARB_CLNT_CM;
  reg_parm.qlink_error_mask = QRARB_CONV_ENUM_TO_BIT_MASK(QRARB_QLINK_CRITICAL_ERROR);
  reg_parm.qrarb_state_mask = QRARB_CONV_ENUM_TO_BIT_MASK(QRARB_STATE_QLINK_RESTART_START);
  reg_parm.client_cb = cm_qrarb_client_cb;

  if(qrarb_register_cb(reg_parm) != TRUE)
  {
    CM_ERR_0("QRARB registration failed");
  }
#endif
}
/*===========================================================================
...
static void cmtask_msgr_init(void)
{
  ...
  for(sub = SYS_MODEM_AS_ID_1; sub < MAX_SIMS; sub++)
  {
    ...
    msgr_error = msgr_register_variant(MSGR_MM_CM,&cm_msgr_client,
                                       MSGR_ID_REX,MM_CM_ACT_DEFAULT_BEARER_CONTEXT_REQUEST_IND,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);

    msgr_error = msgr_register_variant(MSGR_MM_CM,&cm_msgr_client,
                                       MSGR_ID_REX,MM_CM_DEACT_BEARER_CONTEXT_REQUEST_IND,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);

    msgr_error = msgr_register_variant(MSGR_MM_CM,&cm_msgr_client,
                                       MSGR_ID_REX,MM_CM_ACT_DRB_RELEASED_IND,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);

    msgr_error = msgr_register_variant(MSGR_MM_CM,&cm_msgr_client,
                                       MSGR_ID_REX,MM_CM_DRB_SETUP_IND,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);

    msgr_error = msgr_register_variant(MSGR_MM_CM,&cm_msgr_client,
                                       MSGR_ID_REX,MM_CM_DRB_REESTABLISH_FAILED_IND,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);

    msgr_error = msgr_register_variant(MSGR_MM_CM,&cm_msgr_client,
                                       MSGR_ID_REX,MM_CM_ACT_DEDICATED_BEARER_CONTEXT_REQUEST_IND,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);

    msgr_error = msgr_register_variant(MSGR_MM_CM,&cm_msgr_client,
                                       MSGR_ID_REX,MM_CM_DRB_REESTABLISH_REJECT_IND,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);

    msgr_error = msgr_register_variant(MSGR_MM_CM,&cm_msgr_client,
                                       MSGR_ID_REX,MM_CM_GET_PDN_CONNECTIVITY_REQUEST_IND,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);

    msgr_error = msgr_register_variant(MSGR_MM_CM,&cm_msgr_client,
                                       MSGR_ID_REX,MM_CM_PDN_CONNECTIVITY_REJECT_IND,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);

    msgr_error = msgr_register_variant(MSGR_MM_CM,&cm_msgr_client,
                                       MSGR_ID_REX,MM_CM_PDN_CONNECTIVITY_FAILURE_IND,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);

    msgr_error = msgr_register_variant(MSGR_MM_CM,&cm_msgr_client,
                                       MSGR_ID_REX,MM_CM_PDN_DISCONNECT_REJECT_IND,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);

    msgr_error = msgr_register_variant(MSGR_MM_CM,&cm_msgr_client,
                                       MSGR_ID_REX,MM_CM_BEARER_RESOURCE_ALLOC_REJECT_IND,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);

    msgr_error = msgr_register_variant(MSGR_MM_CM,&cm_msgr_client,
                                       MSGR_ID_REX,MM_CM_BEARER_RESOURCE_ALLOC_FAILURE_IND,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);

    msgr_error = msgr_register_variant(MSGR_MM_CM,&cm_msgr_client,
                                       MSGR_ID_REX,MM_CM_BEARER_CONTEXT_MODIFY_REJECT_IND,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);

    msgr_error = msgr_register_variant(MSGR_MM_CM,&cm_msgr_client,
                                       MSGR_ID_REX,MM_CM_MODIFY_BEARER_CONTEXT_REQUEST_IND,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);

    msgr_error = msgr_register_variant(MSGR_MM_CM,&cm_msgr_client,
                                       MSGR_ID_REX,MM_CM_PDN_CONNECTIVITY_PACKED_REQUEST_IND,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);

    msgr_error = msgr_register_variant(MSGR_MM_CM,&cm_msgr_client,
                                       MSGR_ID_REX,LTE_CPHY_RSSI_IND,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);

    msgr_error = msgr_register_variant(MSGR_MM_CM,&cm_msgr_client,
                                       MSGR_ID_REX,LTE_RRC_SERVICE_IND,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);

    msgr_error = msgr_register_variant(MSGR_MM_CM,&cm_msgr_client,
                                       MSGR_ID_REX,LTE_RRC_TO_CM_ACTIVATION_IND,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);
#ifdef FEATURE_DISABLED_HABANERO
    msgr_error = msgr_register_variant(MSGR_MM_CM,&cm_msgr_client,
                                       MSGR_ID_REX,LTE_RRC_DATA_INACTIVITY_INFO_IND,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);
#endif
    msgr_error = msgr_register_variant(MSGR_MM_CM,&cm_msgr_client,
                                       MSGR_ID_REX,LTE_RRC_EMBMS_COVERAGE_STATE_IND,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);

    msgr_error = msgr_register_variant(MSGR_MM_CM,&cm_msgr_client,
                                       MSGR_ID_REX,LTE_RRC_CONN_REL_IND,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);

    msgr_error = msgr_register_variant(MSGR_MM_CM,&cm_msgr_client,
                                       MSGR_ID_REX,LTE_RRC_MANUAL_BST_UPDATE_IND,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);

#ifdef FEATURE_LTE_TO_1X
    msgr_error = msgr_register_variant(MSGR_MM_CM,&cm_msgr_client,
                                       MSGR_ID_REX,MM_CM_1XCSFB_ABORT_RSP,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);


    msgr_error = msgr_register_variant(MSGR_MM_CM,&cm_msgr_client,
                                       MSGR_ID_REX,MM_CM_1XCSFB_CALL_RSP,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);
#endif /* FEATURE_LTE_TO_1X */
    /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

    msgr_error = msgr_register_variant(MSGR_MM_CM,&cm_msgr_client,
                                       MSGR_ID_REX,NAS_EMM_ATTACH_COMPLETE_IND,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);

    msgr_error = msgr_register_variant(MSGR_MM_CM,&cm_msgr_client,
                                       MSGR_ID_REX,NAS_EMM_TAU_COMPLETE_IND,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);

    msgr_error = msgr_register_variant(MSGR_MM_CM,&cm_msgr_client,
                                       MSGR_ID_REX,NAS_EMM_RA_CAP_UPDATE_FAIL_IND,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);

    /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

#ifdef FEATURE_DOMAIN_SELECTION
    msgr_error = msgr_register_variant(MSGR_MM_DOM_SEL,&cm_msgr_client,
                                       MSGR_ID_REX,MM_DOM_SEL_DOMAIN_SELECTED_GET_REQ,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);

    msgr_error = msgr_register_variant(MSGR_MM_DOM_SEL,&cm_msgr_client,
                                       MSGR_ID_REX,MM_DOM_SEL_DOMAIN_SELECTED_LTE_IMS_PREF_GET_REQ,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);

    msgr_error = msgr_register_variant(MSGR_MM_DOM_SEL,&cm_msgr_client,
                                       MSGR_ID_REX,MM_DOM_SEL_UPDATE_IMS_REG_STATUS_IND,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);

    msgr_error = msgr_register_variant(MSGR_MM_DOM_SEL,&cm_msgr_client,
                                       MSGR_ID_REX,MM_DOM_SEL_CONFIG_UPDATE_LTE_SMS_DOMAIN_PREF_REQ,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);

    msgr_error = msgr_register_variant(MSGR_MM_DOM_SEL,&cm_msgr_client,
                                       MSGR_ID_REX,MM_DOM_SEL_CONFIG_LTE_SMS_DOMAIN_PREF_GET_REQ,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);

    msgr_error = msgr_register_variant(MSGR_MM_DOM_SEL,&cm_msgr_client,
                                       MSGR_ID_REX,MM_DOM_SEL_DOMAIN_SELECTED_IMS_PREF_GET_EXT_REQ,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);

    msgr_error = msgr_register_variant(MSGR_MM_CM,&cm_msgr_client,
                                       MSGR_ID_REX,NAS_EMM_T3402_CHANGED_IND,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);
#endif

    msgr_error = msgr_register_variant(MSGR_MM_CM,&cm_msgr_client,
                                       MSGR_ID_REX,NAS_EMM_TAI_LIST_IND,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);

#ifdef FEATURE_IP_CALL
    msgr_error = msgr_register_variant(MSGR_CM_IMS_SRVCC_CNTXT,&cm_msgr_client,
                                       MSGR_ID_REX, MM_CM_SRVCC_CONTEXT_RSP,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);
    msgr_error = msgr_register_variant(MSGR_MM_CM,&cm_msgr_client,
                                       MSGR_ID_REX,MM_CM_AUDIO_RELEASE_IND,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);
#endif

    msgr_error = msgr_register_variant(MSGR_MODULE_EXT_VOLTE_STATE,&cm_msgr_client,
                                       MSGR_ID_REX, MM_CM_EXT_VOLTE_CALL_STATE_IND,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);

    msgr_error = msgr_register_variant(MSGR_MM_CM,&cm_msgr_client,
                                       MSGR_ID_REX,LTE_RRC_BARRING_UPDATE_IND,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);

    msgr_error = msgr_register_variant(MSGR_MM_CM,&cm_msgr_client,
                                       MSGR_ID_REX,MM_CM_VOLTE_STATE_NOTIFICATION_CMD,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);

    msgr_error = msgr_register_variant(MSGR_MM_CM,&cm_msgr_client,
                                       MSGR_ID_REX,MM_CM_SET_SSAC_HYST_TIMER_REQ,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);


    msgr_error = msgr_register_variant(MSGR_MM_CM,&cm_msgr_client,
                                       MSGR_ID_REX,MM_CM_GET_SSAC_HYST_TIMER_REQ,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);

#endif /* FEATURE_CM_LTE */

    msgr_error = msgr_register_variant(MSGR_MM_CM,&cm_msgr_client,
                                       MSGR_ID_REX,MM_CM_SET_GPP_WIFI_CELLULAR_SCAN_TIMER_IND,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);


    msgr_error = msgr_register_variant(MSGR_MM_CM,&cm_msgr_client,
                                       MSGR_ID_REX,MM_MMOC_OFFLINE_IND,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);

    msgr_error = msgr_register_variant(MSGR_MM_CM,&cm_msgr_client,
                                       MSGR_ID_REX,RFA_RF_CONTROL_INIT_COMPLETE_RSP,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);

#ifdef FEATURE_CGPS
    msgr_error = msgr_register_variant(MSGR_MM_CM,&cm_msgr_client,
                                       MSGR_ID_REX,GPS_PGI_INIT_COMPLETE_RSP,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);
#endif

    msgr_error = msgr_register_variant(MSGR_MM_CM,&cm_msgr_client,
                                       MSGR_ID_REX,MM_CM_ESM_NOTIFICATION_IND,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);

    msgr_error = msgr_register_variant(MSGR_MM_CM,&cm_msgr_client,
                                       MSGR_ID_REX,POLICYMAN_CFG_UPDATE_MSIM_IND,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);

    msgr_error = msgr_register_variant(MSGR_MM_CM,&cm_msgr_client,
                                       MSGR_ID_REX,DS_3GPP_CM_RS_RA_DONE_IND,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);

#ifdef FEATURE_TDSCDMA
    msgr_error = msgr_register_variant( MSGR_MM_CM, &cm_msgr_client, MSGR_ID_REX,
                                        TDSCDMA_RRC_SCELL_SIGNAL_STATUS_IND,sub);
    CM_ASSERT(msgr_error == E_SUCCESS)
#endif

    msgr_error = msgr_register_variant(MSGR_MM_CM,&cm_msgr_client,
                                       MSGR_ID_REX,MM_CM_AC_EMERGENCY_ENTER_REQ,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);

    msgr_error = msgr_register_variant(MSGR_MM_CM,&cm_msgr_client,
                                       MSGR_ID_REX,MM_CM_AC_EMERGENCY_EXIT_REQ,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);

#ifdef FEATURE_CM_CFM_AVAILABLE
    msgr_error = msgr_register_variant(MSGR_MM_CM,&cm_msgr_client,
                                       MSGR_ID_REX,MM_CM_CFCM_MONITOR_THERMAL_PA_EM_IND,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);
#endif /* FEATURE_CM_CFM_AVAILABLE */

    msgr_error = msgr_register_variant(MSGR_MM_CM,&cm_msgr_client,
                                       MSGR_ID_REX,MM_CM_SUBSC_CHGND_RSP,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);

    msgr_error = msgr_register_variant(MSGR_MM_CM,&cm_msgr_client,
                                       MSGR_ID_REX,MM_TUI_CSG_SELECTION_CMD,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);

    msgr_error = msgr_register_variant(MSGR_MM_CM,&cm_msgr_client,
                                       MSGR_ID_REX,MM_CM_SET_RPM_PARAMETERS_REQ,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);

    msgr_error = msgr_register_variant(MSGR_MM_CM,&cm_msgr_client,
                                       MSGR_ID_REX,MM_CM_GET_RPM_PARAMETERS_REQ,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);

    msgr_error = msgr_register_variant(MSGR_MM_CM,&cm_msgr_client,
                                       MSGR_ID_REX,MM_CM_SET_RPM_CONFIG_REQ,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);

    msgr_error = msgr_register_variant(MSGR_MM_CM,&cm_msgr_client,
                                       MSGR_ID_REX,MM_CM_SET_T_WWAN_911_REQ,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);

    msgr_error = msgr_register_variant(MSGR_MM_CM,&cm_msgr_client,
                                       MSGR_ID_REX,MM_CM_GET_T_WWAN_911_REQ,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);
    #ifdef FEATURE_DISABLED_HABANERO
    msgr_error = msgr_register_variant(MSGR_MM_CM,&cm_msgr_client,
                                       MSGR_ID_REX,MM_CM_E911_AVOID_PLMN_IND,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);
    #endif
    
    msgr_error = msgr_register_variant(MSGR_MM_CM,&cm_msgr_client,
                                       MSGR_ID_REX,QMI_NAS_PRIORITIZE_LTE_IND,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);

    msgr_error = msgr_register_variant(MSGR_MM_CM,&cm_msgr_client,
                                       MSGR_ID_REX,MM_CM_MEDIA_RESTRICTION_IND,sub);
    CM_ASSERT(msgr_error == E_SUCCESS);
  }
...
static void cmtask_register_msgs(void)
{
  ...
  (void) cm_msgr_send(RFA_RF_CONTROL_INIT_COMPLETE_CMD, MSGR_MM_CM,
  ...
  */
  msgr_init_hdr_variant(&gnss_pgi_complete_cmd.hdr, MSGR_GPS_PGI, GPS_PGI_INIT_COMPLETE_CMD, MSGR_NO_VARIANT);
  ...
  client_req.monitor_mask = (CFCM_MONITOR_MASK_THERMAL_PA|
                             CFCM_MONITOR_MASK_THERMAL_CX|
                             CFCM_MONITOR_MASK_MDM_TEMP|
                             CFCM_MONITOR_MASK_VDD_PEAK_CURR_EST
                             #ifdef FEATURE_DISABLED_HABANERO
                             | CFCM_MONITOR_MASK_SKIN_TEMP
                             #endif
                             );
  ...
  /* Register with QRARB module for qrarb state notification to handle
  ** QLINK critical errors
  */
  cm_qrarb_register();
...
void cm_init_after_task_start( void )
{
  ...
  */
  cmpmprx_init(FALSE, SYS_MODEM_AS_ID_ALL_ACTIVE_MASK);

  /* Initialize the origination parameters queue
  */
  cmtask_init_priority_q(cmpmprx_get_num_of_sims());
  ...
  */
  cmsoa_state_info_ptr()->count = 0;
  cmsoa_init(TRUE, SYS_MODEM_AS_ID_ALL_ACTIVE_MASK);
#endif
  ...
  */
  ...
  cmregprx_init(NULL, 0, SYS_MODEM_AS_ID_ALL_ACTIVE_MASK, TRUE);
  ...
  */
  ...
  /* Initialize the cmcc2 object.
  */
  cmcc_init(SYS_MODEM_AS_ID_ALL_ACTIVE_MASK, TRUE);
...
#error code not present
  ...
  */
  cmsups_init(SYS_MODEM_AS_ID_ALL_ACTIVE_MASK, TRUE);
...
static void cm_nv_refresh(const cm_generic_rpt_s_type *gen_rpt_ptr  )
{
  ...
  */
  ...
  /* If MCFG refresh doesnt have valid device mode return without continuing with NV refresh
  ...
  */
  ...
  /* Initialize the CM SUPS object.
  */
  cmsups_init(BM(asubs_id), FALSE);

#endif /* FEATURE_WCDMA || FEATURE_GSM */
  cmregprx_init(NULL, 0, BM(asubs_id), FALSE);

  cmcc_init(BM(asubs_id), FALSE);
  //cmrpm
  //cmrpm_tds
} /* cm_nv_refresh() */
...
#else
      if(!INRANGE(gen_rpt_ptr->generic_rpt.refresh_info.as_id, SYS_MODEM_AS_ID_1, SYS_MODEM_AS_ID_2))
#endif
      {
...
#ifdef FEATURE_DOMAIN_SELECTION
      /* MCFG refresh will trigger stop mode request.
      ** NAS will reset the blocked plmns when stop mode
      ** request is received. But in case the plmns are blocked
      ** after stop mode request is sent, reset them here.
      */
      cmph_reg_send_block_plmn_reset_req(
                                     gen_rpt_ptr->generic_rpt.refresh_info.as_id,
                                     SYS_BLOCK_PLMN_CAUSE_VOICE_NOT_AVAILABLE
                                     );
#endif
      CM_MSG_HIGH_2("MCFG_Refresh:sub %d refresh complete %d",
                    gen_rpt_ptr->generic_rpt.refresh_info.as_id,
                    gen_rpt_ptr->generic_rpt.refresh_info.reference_id);
      break;
#endif
    default:
      break;
  ...
  */
  if ( rpt_buf != NULL )
  {
    ...
    if(cmipapp_rpt_ptr->hdr.cmd == CM_IP_CALL_IND_MT_INVITE)
    {
      ...
      if(cmipapp_rpt_ptr->ip_ind.ind.mt_invite.call_attrib_info.call_composer_info.picture_url != NULL)
      {
        REF_CNT_OBJ_RELEASE_IF(
          cmipapp_rpt_ptr->ip_ind.ind.mt_invite.call_attrib_info.call_composer_info.picture_url);
      }
      if(cmipapp_rpt_ptr->ip_ind.ind.mt_invite.call_attrib_info.call_composer_info.subject != NULL)
      {
        REF_CNT_OBJ_RELEASE_IF(
          cmipapp_rpt_ptr->ip_ind.ind.mt_invite.call_attrib_info.call_composer_info.subject);
      }
      if(cmipapp_rpt_ptr->ip_ind.ind.mt_invite.call_attrib_info.call_composer_info.org_header != NULL)
      {
        REF_CNT_OBJ_RELEASE_IF(
          cmipapp_rpt_ptr->ip_ind.ind.mt_invite.call_attrib_info.call_composer_info.org_header);
      }
    }
    ...
    #endif
    if (cmipapp_rpt_ptr->hdr.cmd == CM_IP_CALL_SIP_DTMF_IND &&
        cmipapp_rpt_ptr->ip_ind.ind.sip_dtmf_ind.dtmf_info != NULL)
    {
      REF_CNT_OBJ_RELEASE_IF(cmipapp_rpt_ptr->ip_ind.ind.sip_dtmf_ind.dtmf_info);
    }
#endif
    cm_mem_free(rpt_buf);
  }
...
#endif
#ifdef CM_GW_SUPPORTED
  cmwsups_sd_rpt_proc(cm_rpt_ptr);
#endif /* (defined(FEATURE_WCDMA) || defined(FEATURE_GSM)) */
...
#endif
  cmcall_generic_rpt_proc(generic_rpt_ptr);
...
void cm_msgr_handle( void )
{
  ...
  {
    asubs_id = cmutil_get_variant(&rpt_ptr->cmd.msg_hdr);

    if(CM_MSGR_IS_MMOC_OFFLINE_IND_EVT( rpt_ptr->cmd.offline_ind.hdr.id))
    {
      CM_MSG_HIGH_0("MSGR RXD: MM_MMOC_OFFLINE_IND");
      cm_offline();
    }
#ifdef FEATURE_CM_CFM_AVAILABLE
    else if(CM_MSGR_CFCM_THERMAL_EMERG_EVT( rpt_ptr->cmd.cfcm_ind.hdr.id) )
    {
      CM_MSG_HIGH_0("MSGR RXD: MM_CM_CFCM_MONITOR_THERMAL_PA_EM_IND");
      cm_cfcm_thermal_event_proc(&rpt_ptr->cmd.cfcm_ind);
    }
#endif /* FEATURE_CM_CFM_AVAILABLE */
    ...
    {
      CM_MSG_HIGH_2("MSGR RXD: LTE_CPHY_RSSI_IND, sub %d valid %d", asubs_id,rpt_ptr->cmd.lte_sys.lte_rssi_ind.valid);
      if (rpt_ptr->cmd.lte_sys.lte_rssi_ind.valid)
      {
        cmss_msgr_rpt_proc((void *)(&(rpt_ptr->cmd.lte_sys)), asubs_id);
      }
    }
    else if (CM_MSGR_MSG_ESM(rpt_ptr->cmd.esm.msg_hdr.id))
    {
      CM_MSG_HIGH_1("MSGR RXD: NAS->CM: ESM LTE report, sub %d", asubs_id);
      cmltecall_rpt_proc(&(rpt_ptr->cmd.esm), asubs_id);
#if defined (FEATURE_CM_LTE) && defined(FEATURE_IP_CALL)
      cmvoltecall_rpt_proc(&(rpt_ptr->cmd.esm), asubs_id);
#endif
    }

    else if (CM_MSGR_MSG_AC(rpt_ptr->cmd.emg_cmd.msg_hdr.id))
    {
      CM_MSG_HIGH_1("MSGR RXD: AC, sub %d", asubs_id);
      cmemg_msgr_cmd_proc (&(rpt_ptr->cmd.emg_cmd), asubs_id);
    }

#ifdef FEATURE_LTE_TO_1X
    else if(CM_MSGR_MSG_CSFB( rpt_ptr->cmd.esm.msg_hdr.id))
    {
      CM_MSG_HIGH_1("MSGR RXD: CSFB, sub %d", asubs_id);
      cmcsfbcall_rpt_proc(&(rpt_ptr->cmd.esm), asubs_id);
    }
#endif

    else if (CM_MSGR_MSG_LTE_RRC_SERVICE(rpt_ptr->cmd.lte_sys.lte_cell_info.msg_hdr.id))
    {
      CM_MSG_HIGH_1("MSGR RXD: LTE_RRC_SERVICE_IND, sub %d", asubs_id);
    ...
    else if (CM_MSGR_MSG_LTE_RRC_BST_UPDATE(rpt_ptr->cmd.lte_sys.lte_cell_info.msg_hdr.id))
    {
      CM_MSG_HIGH_0("MSGR RXD: LTE_RRC_MANUAL_BST_UPDATE_IND");
      cmregprx_update_manual_search_bst(&(rpt_ptr->cmd.lte_sys.lte_manual_bst_update_ind));
    }
#ifdef FEATURE_DISABLED_HABANERO
    else if (CM_MSGR_MSG_LTE_RRC_DATA_INACTIVITY_INFO(rpt_ptr->cmd.lte_rrc_data_inact_timer_info_ind.msg_hdr.id))
    {
      CM_MSG_HIGH_1("MSGR RXD: LTE_RRC_DATA_INACTIVITY_INFO_IND,sub %d",asubs_id);
      cmss_trigger_inact_timer_info_evt(&(rpt_ptr->cmd.lte_rrc_data_inact_timer_info_ind),asubs_id);
    }
#endif
    else if(CM_MSGR_MSG_LTE_RRC_BARRING_UPDATE(rpt_ptr->cmd.barring_update_ind.msg_hdr.id))
    {
      CM_MSG_HIGH_1("MSGR RXD: LTE_RRC_BARRING_UPDATE_IND, sub %d", asubs_id);
      cmcall_read_barring_params(&(rpt_ptr->cmd.barring_update_ind), asubs_id);
      cmss_read_emerg_barring_params(&(rpt_ptr->cmd.barring_update_ind), asubs_id);
    }

#ifdef FEATURE_MMODE_LTE_RESEL
    else if (CM_MSGR_LTE_IRAT_RESEL(rpt_ptr->cmd.lte_sys.lte_actd_ind.msg_hdr.id))
    {
      CM_MSG_HIGH_1("MSGR RXD: LTE_RRC_TO_CM_ACTIVATION_IND, sub %d", asubs_id);
      cmregprx_msgr_rpt_proc(rpt_ptr, asubs_id);
    }
    ...
    else if(CM_MSGR_EMM_ATTACH_COMPLETE(rpt_ptr->cmd.emm_attach_complete_ind.msg_hdr.id))
    {
      ...
      {
        cmrpm_lte_handle_attach_complete_ind(&rpt_ptr->cmd.emm_attach_complete_ind, asubs_id);
      }
      
      CM_MSG_HIGH_3("MSGR RXD: NAS->CM: NAS_EMM_ATTACH_COMPLETE_IND sub %d stk %d is_rpm_enabled %d",
                    mm_id.asubs_id, mm_id.stk_id, cmrpm_lte_is_rpm_enabled());

      /* Activate HDR tunnel mode when LTE attached */
#ifdef FEATURE_LTE_TO_HDR_OH
      cmsds_activate_hdr_tunnel_req(mm_id.asubs_id);
#endif
      sd_misc_attach_complete_ind(mm_id);
    }
    else if(CM_MSGR_EMM_TAU_COMPLETE(rpt_ptr->cmd.emm_tau_complete_ind.msg_hdr.id))
    {
      ...
      if(TRUE == cmrpm_lte_is_rpm_enabled())
      {
        cmrpm_lte_handle_tau_complete_ind(&rpt_ptr->cmd.emm_tau_complete_ind, asubs_id);
      }
      CM_MSG_HIGH_3("MSGR RXD: NAS->CM: NAS_EMM_TAU_COMPLETE_IND sub %d stk %d is_rpm_enabled %d",
                    mm_id.asubs_id, mm_id.stk_id, cmrpm_lte_is_rpm_enabled());
      sd_misc_attach_complete_ind(mm_id);

      cmsds_process_tau_complete_ind(&rpt_ptr->cmd.emm_tau_complete_ind, asubs_id);
    }
    else if(CM_MSGR_EMM_RA_CAP_UPDATE_FAIL(rpt_ptr->cmd.emm_ra_cap_update_fail_ind.msg_hdr.id))
    {

      CM_MSG_HIGH_1("MSGR RXD: NAS->CM: NAS_EMM_RA_CAP_UPDATE_FAIL_IND sub %d", asubs_id);

      cmph_handle_ra_cap_update_fail_ind(asubs_id);
    }
    else if (CM_MSGR_MSG_LTE_RRC_EMBMS(rpt_ptr->cmd.lte_sys.lte_embms_coverage_state_ind.msg_hdr.id))
    {
      CM_MSG_HIGH_1("MSGR RXD: LTE_RRC_EMBMS_COVERAGE_STATE_IND, sub %d", asubs_id);
      cmss_msgr_rpt_proc((void *)(&(rpt_ptr->cmd.lte_sys)), asubs_id);
    }
    ...
    {
      CM_MSG_HIGH_1("MSGR RXD: LTE_RRC_CONN_REL_IND, sub %d", asubs_id);
      cmsoa_msgr_rpt_proc(asubs_id);
    }
#endif
    ...
    {
      CM_MSG_HIGH_1("MSGR RXD: NAS_EMM_T3402_CHANGED_IND, sub %d", asubs_id);
      /* Update T3402 value based on T3402_CHANGED_IND */
      cmsds_t3402_changed_ind_proc(&(rpt_ptr->cmd.emm_t3402_changed_ind), asubs_id);
    }

    else if (CM_MSGR_MSG_CMSDS(rpt_ptr->cmd.client_cmd.msg_hdr.id))
    {
      CM_MSG_HIGH_1("MSGR RXD: CMSDS, sub %d", asubs_id);
      cmsds_client_cmd_proc(rpt_ptr, asubs_id);
    }
#endif
    ...
    {
#ifdef FEATURE_MMODE_DUAL_SIM
      CM_MSG_HIGH_1("MSGR RXD: TDSCDMA_RRC_SCELL_SIGNAL_STATUS_IND: as_id:%d",
    ...
    {
      CM_MSG_HIGH_0("MSGR RXD: RFA_RF_CONTROL_INIT_COMPLETE_RSP");
...
#ifdef FEATURE_DOMAIN_SELECTION
    else if(CM_MSGR_IS_DS_3GPP_CM_RS_RA_DONE_IND(rpt_ptr->cmd.ds_rsra_done.msg_hdr.id))
    {
      cmsds_ds_rsra_done_ind_proc(asubs_id);
    }
#endif
    ...
    {
      CM_MSG_HIGH_1("MSGR RXD: MM_CM_SRVCC_CONTEXT_RSP, sub %d", rpt_ptr->cmd.srvcc_rsp.as_id);
...
#endif
    else if (CM_MSGR_IS_MM_CM_SUBSC_CHGND_RSP(rpt_ptr->cmd.subsc_chgnd_rsp.msg_hdr.id))
    {
      CM_MSG_HIGH_1("CLNUP: MSGR RXD: MM_CM_SUBSC_CHGND_RSP, sub %d", rpt_ptr->cmd.subsc_chgnd_rsp.as_id);
#ifdef FEATURE_IP_CALL
    ...
    {
      CM_MSG_HIGH_1("MSGR RXD: EXT_IMS_CALL: MM_CM_EXT_VOLTE_CALL_STATE_IND, sub %d", rpt_ptr->cmd.ext_volte_state.asubs_id);
    ...
    {
      CM_MSG_HIGH_1("MSGR RXD: MM_TUI_CSG_SELECTION_CMD, sub %d", rpt_ptr->cmd.csg_select_cfg.csg_sel.asubs_id);
    ...
    else if (CM_MSGR_IS_MM_SET_SSAC_HYST_TIMER_REQ(rpt_ptr->cmd.set_ssac_hyst_timer_req.msg_hdr.id))
    {
      CM_MSG_HIGH_1("MSGR RXD: MM_CM_SET_SSAC_HYST_TIMER_REQ, sub %d", asubs_id);
      /* set t_ssac i.e. hVOLTE hysteresis timer and update NV as well */
      cmph_set_ssac_hyst_timer_req(rpt_ptr->cmd.set_ssac_hyst_timer_req, asubs_id);
    }
    else if (CM_MSGR_IS_MM_GET_SSAC_HYST_TIMER_REQ(rpt_ptr->cmd.get_ssac_hyst_timer_req.msg_hdr.id))
    {
      CM_MSG_HIGH_1("MSGR RXD: MM_CM_GET_SSAC_HYST_TIMER_REQ, sub %d", asubs_id);
      /* send the value of t_ssac hysteresis timer value */
      cmph_send_get_ssac_hyst_timer_rsp(asubs_id);
    }
    ...
    else if (CM_MSGR_IS_MM_CM_SET_T_WWAN_911_REQ(rpt_ptr->cmd.set_t_wwan_911_req.msg_hdr.id))
    {
      ...
      cmph_set_t_wwan_911_req(rpt_ptr->cmd.set_t_wwan_911_req.t_wwan_911_val, asubs_id);
    }
    else if (CM_MSGR_IS_MM_CM_GET_T_WWAN_911_REQ(rpt_ptr->cmd.get_t_wwan_911_req.msg_hdr.id))
    {
      ...
      cmph_send_get_t_wwan_911_rsp(asubs_id);
    }
    else if (CM_MSGR_SET_GPP_WIFI_CELLULAR_SCAN_TIMER_IND(rpt_ptr->cmd.set_gpp_wifi_cellular_scan_timer_ind.msg_hdr.id))
    {
      CM_MSG_HIGH_1("MSGR RXD: SET_GPP_WIFI_CELLULAR_SCAN_TIMER_IND, sub %d", asubs_id);
      /* update NV */
      cmph_update_gpp_wifi_cellular_scan_timer(rpt_ptr->cmd.set_gpp_wifi_cellular_scan_timer_ind.gpp_wifi_cellular_scan_timer_val,asubs_id);
    }
    #ifdef FEATURE_DISABLED_HABANERO
    #if defined(FEATURE_IP_CALL) && defined(FEATURE_CM_LTE)
	else if(CM_MSGR_IS_MM_CM_911_AVOID_PLMN_LIST_IND(rpt_ptr->cmd.avoid_plmn_list_req.msg_hdr.id))
	{
          errno_enum_type msgr_error = E_SUCCESS;
		
          sys_plmn_id_s_type avoid_plmn_from_ap;
          cm_plmn_list_to_avoid_s_type  avoid_plmn_list_req_buf;
          sys_modem_as_id_e_type subs_id = asubs_id;
          cmph_sub_info_s_type   *ph_sub_config_ptr = cmph_get_sub_ptr(subs_id);
          uint8 idx = 0;
          uint8 list_len = rpt_ptr->cmd.avoid_plmn_list_req.plmn_list_to_avoid_len;

          if(ph_sub_config_ptr == NULL)
          {
            CM_MSG_HIGH_1("cmph sub config pointer[%d] is NULL",subs_id);
            return;
          }
          cm_avoid_list_init(subs_id);
          for(;idx<list_len;idx++)
          {
            avoid_plmn_from_ap = rpt_ptr->cmd.avoid_plmn_list_req.plmn_list_to_avoid[idx];
            if(cmph_add_avoid_plmn_to_list(subs_id,avoid_plmn_from_ap))
            {
              send_ind = TRUE;
            }
          }
          ...
          if(send_ind == TRUE)
          {
            memscpy(avoid_plmn_list_req_buf.list_to_avoid,sizeof(sys_plmn_id_s_type)*SYS_AVOID_PLMN_LIST_MAX_LENGTH,
            ph_sub_config_ptr->avoid_plmn_list_info.plmn,sizeof(sys_plmn_id_s_type)*SYS_AVOID_PLMN_LIST_MAX_LENGTH);

            /*Update the number of plmns from cmph_ptr()*/
            avoid_plmn_list_req_buf.num_of_plmns_to_avoid = ph_sub_config_ptr->avoid_plmn_list_info.num_of_plmns_to_avoid;
            avoid_plmn_list_req_buf.as_id = subs_id;
		
            /*use message router to send the avoid plmn list to LRRC.*/
            msgr_error = cm_msgr_send( MM_CM_PLMN_AVOID_LIST_IND, MSGR_MM_CM,
			                       (msgr_hdr_s*)&(avoid_plmn_list_req_buf), 
			                       sizeof(cm_plmn_list_to_avoid_s_type),
			                       subs_id);

		CM_MSG_HIGH_1("CM->MSGR:, cm_msgr_handle(), e911 plmn avoid list to LRRC msgr_error=%d",msgr_error );
          }
    }
    #endif
    #endif
    else if(CM_MSGR_IS_QMI_NAS_PRIORITIZE_LTE_IND(rpt_ptr->cmd.prioritize_lte_scan_req.msg_hdr.id))
    {
      cmph_prioritize_lte_scan_req(asubs_id);
    }
    else
    {
      CM_MSG_HIGH_0("MSGR RXD: Unexpected msg id comes to cm_lterpt_handle");
    }
    ...
    while(call_ptr != NULL)
    {
      ...
      {
        CM_MSG_HIGH_4("After cm_msgr_handle: call_id %d, sub %d, call_type %d, orig_sent_on_mode %d",
                      call_ptr->call_id,
...
FUNCTION cm_ok_to_log_ml_report
...
===========================================================================*/
boolean cm_ok_to_log_ml_report(
  cm_hdr_type       *rpt_ptr
)
{
  switch(rpt_ptr->cmd)
...
{ 
  ...
  log_msg->msg_id = rpt_ptr->cmd + CM_RPT_OFFSET;
  log_msg->src_sub_id = rpt_ptr->asubs_id;
  log_msg->dest_sub_id = rpt_ptr->asubs_id;
  log_msg->dest_module = LOG_ML_CM;
  ...
  switch(rpt_ptr->cmd)
  {
	 ...
	 case CM_MM_PS_DATA_SUSPEND_IND:
	 case CM_PS_DETACH_CNF:
	   log_msg->source_module = LOG_ML_NAS ;
	   break;

	 case CM_LPM_OK_F:
		 log_msg->source_module = LOG_ML_MMOC ;
	   break;

	 case CM_IP_CALL_IND_MT_INVITE:
	 case CM_IP_CALL_IND_CONNECTED:
	 case CM_IP_CALL_IND_CALL_END:
	   log_msg->source_module = LOG_ML_IMS ;
	   break;

	 default:
	   log_msg->source_module = LOG_ML_SD ;
  }
...
void cm_rpt_handle( void )
{
  ...
  */
  ...
  {
    #if defined(FEATURE_MMODE_ML_LOGGING)
	if(cm_ok_to_log_ml_report(rpt_ptr))
	{
	  cm_process_ml_report(rpt_ptr); 
	}
    #endif
    ...
    /*TAKE CARE : Whenever adding new report ranges here, please add to cmtask_is_valid_report as well */
...
*/
{
  ...
  */
  ...
  if ( cmd_ptr != NULL )
  {
    ...
    {
      CM_MSG_HIGH_4("CLIENT_CMD: START: Command type %d command %d waiting for reply, is_reused %d, client_id %d",
                    cmd_ptr->cmd_type, PH_CMD_PTR(cmd_ptr)->cmd,
                    cmd_ptr->is_reused, cmd_ptr->client_id);
      return;
    }
  } /* if ( cmd_ptr != NULL ) */
...
#error code not present
    ...
    */
    ...
    if ( cm_reply_should_cmd_wait( cmd_ptr ) )
    {
      CM_MSG_HIGH_2("CLIENT_CMD: Command type %d command %d waiting for reply",
                    cmd_ptr->cmd_type, PH_CMD_PTR(cmd_ptr)->cmd);
    ...
    */
    ...
    {
      CM_MSG_HIGH_2 ("CLIENT_CMD: END: is_reused = 1, cmd_ptr = 0x%x, cmd_type=%d",
                     cmd_ptr,
                     CMD_TYPE(cmd_ptr));
    }
...
)
{
  ...
  boolean                  other_sys_sel_pref_still_buffered = FALSE;
  cm_cmd_type           *cmd_ptr;
  cm_cmd_q_type      cm_client_cmd_local_q;
  cmph_s_type   *ph_ptr = cmph_ptr();

 (void) q_init( &cm_client_cmd_local_q );
    ...
    switch ( CMD_TYPE(hold_cmd_ptr) )
    {
      ...
      case CM_CMD_TYPE_PH:
        ...
        */
        ...
        /* If the sys sel pref change still has to be buffered, return CONTINUE
        ** other_sys_sel_pref_still_buffered flag is used to ensure that we don't unbuffer any
        ** cmds out of order. For example if there are 2 cmds buffered and the first one still
        ** needs to be buffered, we should not unbuffer the 2nd cmd
        */
        if((PH_CMD_PTR(hold_cmd_ptr)->info.hold_cmd_reason == CM_HOLD_CMD_REAS_PREF_CHG &&
            (cmph_is_buffer_sys_sel_pref_activity(&PH_CMD_PTR(hold_cmd_ptr)->info) ||
             cmph_is_cap_update_during_ext_audio_call(&PH_CMD_PTR(hold_cmd_ptr)->info))) ||
            (PH_CMD_PTR(hold_cmd_ptr)->info.hold_cmd_reason == CM_HOLD_CMD_REAS_PREF_CHG &&
             other_sys_sel_pref_still_buffered)
          )
        {
          CM_MSG_HIGH_0("Clnup or MM is busy, don't process sys_sel_pref cmd");
          other_sys_sel_pref_still_buffered = TRUE;
          hold_cmd_ptr = next_cmd_ptr;
          continue;
        }
        ...
        */
        if(PH_CMD_PTR(hold_cmd_ptr)->info.hold_cmd_reason == CM_HOLD_CMD_REAS_EMG &&
            ...
            (IS_VALID_PH_SUB_PTR(PH_CMD_PTR(hold_cmd_ptr)->info.cmd_subs) &&
             ph_ptr->sub_info[PH_CMD_PTR(hold_cmd_ptr)->info.cmd_subs]->emerg_call_prop.ecbm_rat == SYS_SYS_MODE_WLAN &&
             ph_ptr->sub_info[PH_CMD_PTR(hold_cmd_ptr)->info.cmd_subs]->emerg_call_prop.ph_state == CM_PH_STATE_EMERG_CB)||
             cmac_is_in_thermal_emergency(PH_CMD_PTR(hold_cmd_ptr)->info.cmd_subs))
          )
        {
        ...
        */
        {
          ...
          {
            ss_info_sub_prop_ptr = cmss_get_info_sub_prop((sys_modem_as_id_e_type)i);
            ...
            {
              ss_info_all_stack_ptr = ss_info_sub_prop_ptr->stack_info[j].gen_info;
              if(j == MM_STACK_1 || ss_info_all_stack_ptr == NULL)
              {
                continue;
              }
              if( ss_info_all_stack_ptr->is_stack_operational &&
                  (ss_info_all_stack_ptr->sys_mode == SYS_SYS_MODE_LTE))
              {
                /* LTE on hybrid-2 stack */
                lte_srv_status = ss_info_all_stack_ptr->srv_status;
                break;
              }
            }
          }
        ...
        */
        ...
        if( (( PH_CMD_PTR(hold_cmd_ptr)->cmd == CM_PH_CMD_SUBSCRIPTION_AVAILABLE ) ||
             ( PH_CMD_PTR(hold_cmd_ptr)->cmd == CM_PH_CMD_SUBSCRIPTION_NOT_AVAILABLE ) ||
             ( PH_CMD_PTR(hold_cmd_ptr)->cmd == CM_PH_CMD_SUBSCRIPTION_CHANGED ) ||
             ( PH_CMD_PTR(hold_cmd_ptr)->cmd == CM_PH_CMD_SYS_SEL_PREF ) ))
        {

          {
            ...
            if(ph_buff_cmd_ptr!=NULL)
            {
              *ph_buff_cmd_ptr = *(PH_CMD_PTR( hold_cmd_ptr ));
			   ...
			   if( PH_CMD_PTR(hold_cmd_ptr)->cmd != CM_PH_CMD_SYS_SEL_PREF )
			   {
               ph_buff_cmd_ptr->info.hold_cmd_reason = CM_HOLD_CMD_REAS_NONE;
			   }  
              CM_MSG_HIGH_1("cmd %d processed moved hold cmd q to client q",PH_CMD_PTR(hold_cmd_ptr)->cmd);

              cm_cmd_q_put( &cm_client_cmd_local_q, (cm_cmd_type *)ph_buff_cmd_ptr  );
            }
          }
        }
        else
  ...
  if(q_cnt(&cm_client_cmd_local_q))
  {
    while( (cmd_ptr = cm_client_cmd_q_get()) != NULL )
    {
      cm_cmd_q_put(&cm_client_cmd_local_q, cmd_ptr);
    }
    while( (cmd_ptr = cm_cmd_q_get(&cm_client_cmd_local_q)) != NULL )
    {
      cm_client_cmd_q_put(cmd_ptr);
    }
  }
...
FUNCTION cmtask_get_act_priority
...
cm_act_priority_e_type cmtask_get_act_priority(
...
FUNCTION cmtask_map_call_type_to_act_type
...
cm_act_type_e_type cmtask_map_call_type_to_act_type(
...
)
{
  ...
  switch(call_type)
  {
    ...
    case CM_CALL_TYPE_CS_DATA:
      ...
      {
        if (cmcall_is_there_csfb_call_per_sub((sys_modem_as_id_e_type)i,
...
} /* cmtask_map_call_type_to_act_type */
...
*/
{
  ...
  */
  ...
  for(i=0; i<cmss_ptr()->count; i++)
  {
    cmss_timer_proc(timer_event, (sys_modem_as_id_e_type)i);
    cmss_stack_2_timer_proc(timer_event, (sys_modem_as_id_e_type)i);
  }
  ...
  for(;;)                         /* Wait on REX signals, repeat forever */
  {
    ...
    */
    if (sigs & TASK_STOP_SIG)
    {
      ...
      cmtask_stop_sig_rcvd_time = time_get_uptime_secs();
    ...
    */
    ...
    if (sigs & CM_ATTACH_COMPLETE_TIMER_SIG)
    {
      ...
      for (sub = SYS_MODEM_AS_ID_1; sub < cmpmprx_get_num_of_sims(); sub ++)
      {
        (void)cmsoa_attach_complete_rpt_proc(TRUE, sub);
      }
    }
#endif
...
/*lint -esym(715,dummy) */
{
  ...
  */
  ...
  mcs_rcevt_signal_name(CM_RCEVT_READY);
...
/*===========================================================================
FUNCTION  cm_end_pre_alert_mt_volte_call

DESCRIPTION

Send trigger to CM to push VOLTE call end request to IMS for pending MT VOLTE calls
...
void cm_end_pre_alert_mt_volte_call(
...
*/
{
  ...
  if ( (cmd_ptr = cm_get_cmd_buf()) != NULL )
  {
    CM_MSG_HIGH_1("cm_end_pre_alert_mt_volte_call(), ll as_id=%d", as_id);

    /* Fill in command type */
    cmd_ptr->hdr.cmd  = CM_END_PRE_ALERT_MT_VOLTE_CALL;

    cmd_ptr->asubs_id  = as_id;
...
FUNCTION cmtask_is_pending_1xsub_switch

DESCRIPTION
  check if 1x sub is in progress of switching from one sub to the other due to maxcap change.
  (waiting for CM_SUBS_CHGD_DONE from MMOC)
...
===========================================================================*/
boolean cmtask_is_pending_1xsub_switch(
  sys_modem_as_id_e_type as_id
)
{
  cmph_s_type  *ph_ptr = cmph_ptr();
  sys_modem_as_id_e_type other_sub = cmph_get_other_sub(as_id);
  uint8 stacks = cmtask_get_num_stacks(as_id);
  uint8 stacks_other_sub = cmtask_get_num_stacks(other_sub);
  ...
  {
    CM_MSG_HIGH_4("MAXCAP: 1x sub switch pending. 1XSUB: %d, num_stacks %d, other_sub: %d, num_stacks %d", 
                   as_id, stacks, other_sub, stacks_other_sub);
    return TRUE;
  }
...
FUNCTION cmtask_is_valid_report

DESCRIPTION
check if report getting queued is a valid report in expected range.
...
===========================================================================*/
boolean cmtask_is_valid_report(
  cm_name_type   rpt_name
)
{

  /* Check if report is in known ranges */
  if(CMLL_RPT_IS_SD(rpt_name) || CMLL_RPT_IS_1X(rpt_name) || 
       CMLL_RPT_IS_GW(rpt_name) || CMLL_RPT_IS_IP(rpt_name) ||
       CMLL_RPT_IS_IPSMS(rpt_name) || CMLL_RPT_IS_GENERIC(rpt_name))
  {
    return TRUE;
  }

  /* IF report is not in any valid range, reutrn FALSE */
  return FALSE;
}

#ifdef FEATURE_QSH_MDUMP
/*===========================================================================
FUNCTION cmtask_qsh_mdump_collect
  
DESCRIPTION
  This function collects the mdump for cmtask global variables
...
QSH_MDUMP_FN_ATTR
#endif
void cmtask_qsh_mdump_collect()
{
  qsh_mdump_collect_high(&cm_task_info,sizeof(cmtask_info_s_type));
  qsh_mdump_collect_high(&cm_client_cmd_q,sizeof(cm_cmd_q_type));
  qsh_mdump_collect_high(&cm_tcb_ptr,sizeof(rex_tcb_type));
  qsh_mdump_collect_high(&cm_hold_cmd_q,sizeof(cm_cmd_q_type));
  qsh_mdump_collect_high(&cm_rpt_q,sizeof(cm_cmd_q_type));
  qsh_mdump_collect_high(&cm_rssi_timer,sizeof(rex_timer_type));

#if defined(FEATURE_IP_CALL) && defined(FEATURE_CM_LTE)
  qsh_mdump_collect_high(&cm_srvcc_timer,sizeof(rex_timer_type));
#endif

#ifdef FEATURE_MMODE_DYNAMIC_SV_OPERATION_SWITCH
  qsh_mdump_collect_high(&cm_attach_complete_timer,sizeof(rex_timer_type));
#endif

  qsh_mdump_collect_high(&cm_prio_q,sizeof(cm_priority_q_s_type));
  qsh_mdump_collect_high(&cm_msgr_q,sizeof(cm_cmd_q_type));

#ifndef CM_DEBUG
  qsh_mdump_collect_high(&cm_dog_rpt_var,sizeof(dog_report_type));
#endif

  qsh_mdump_collect_high(&cm_timer,sizeof(rex_timer_type));
  qsh_mdump_collect_high(&cm_msgr_client,sizeof(msgr_client_t));
  qsh_mdump_collect_high(&cmtask_stop_sig_rcvd,sizeof(boolean));
}
#endif
