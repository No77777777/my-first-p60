...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/mmode/cm/src/cmautoreg.c#1 $
...
static uint32 cm_auto_reg_sms_retry_timer_value = CM_AUTO_REG_CT_DEFAULT_RETRY_TIME;
...
static sys_modem_as_id_e_type          ct_autoreg_asid = SYS_MODEM_AS_ID_NONE;

/**< Differentiate between multiple incoming messages. */
wms_transaction_id_type      cm_auto_reg_trans_id = WMS_DUMMY_TRANSACTION_ID;
...
{
  CM_MSG_HIGH_3("cm_auto_reg_dereg current_ss_mode %d ct_autoreg_asid %d cm_auto_reg_cmds %d",current_ss_mode,ct_autoreg_asid,cm_auto_reg_cmds);
  ...
  /* This time is used only for CT */
    ...
    ct_autoreg_asid = SYS_MODEM_AS_ID_NONE;
...
boolean cm_auto_reg_efs_model_data()
{
  ...
  if(   ( cm_get_operator_info(SYS_MODEM_AS_ID_1) == OPERATOR_CT) ||
        ...
        ( cm_get_operator_info(SYS_MODEM_AS_ID_2) == OPERATOR_CT) ||
#endif
...
===========================================================================*/
void cm_auto_reg_ss_event_mdm_cb(
...
)
{
  ...
  CM_MSG_HIGH_5("sub %d, SS EM CB sys_mode : %x srv_status: %x srv_domain: %x. Operator is %d ",
               asid,
               sys_mode,
               srv_status,
               cmss_get_info_srv_domain(asid, MM_STACK_0),
               cm_get_operator_info(asid));
      ...
      (srv_status == SYS_SRV_STATUS_SRV ) )
  {
    ...
    {
      CM_MSG_HIGH_3("sub %d, cm_auto_reg_cmds:%d, cm_auto_reg_imsi_mcc:%d",
                    asid, cm_auto_reg_cmds, cmmmgsdi_get_imsi_mcc_per_sub(asid));
      if ( cm_auto_reg_cmds == CM_AUTO_REG_INFO_CLEAR_CMD)
      {
        if(!cm_is_operator_ct(asid) ||
            (cmmsc_get_1x_sub() != asid))
        {
          CM_MSG_HIGH_2("asid %d, 1x_asid %d, ignore invalid/stale rpt",
                        asid, cmmsc_get_1x_sub());
          return;
        }
      }

      if (ct_autoreg_asid != SYS_MODEM_AS_ID_NONE && asid != ct_autoreg_asid)
      {
        CM_MSG_HIGH_2("asid != ct_autoreg_asid, asid %d, ct_autoreg_asid %d",
                      asid, ct_autoreg_asid);
        cm_auto_reg_dereg();
        return;
      }
    }
    ...
    {
      CM_ERR_2("Operator EFS : %d. Auto Register not initiated, sub %d",cm_get_operator_info(asid), asid);
      return;
    }

    CM_MSG_HIGH_2("sub %d, cm_auto_reg_cmds:%d", asid, cm_auto_reg_cmds);
    ...
    * only once per power up. */
    ...
    if (cm_auto_reg_cmds & CM_AUTO_REG_INFO_WAIT_CMD)
    {
      ...
      if (cm_auto_reg_cmds & CM_AUTO_REG_INFO_CTS_CMD)
      {
        ...
        CM_MSG_HIGH_2("sub %d, cm_auto_reg_cmds:%d, CM_AUTO_REG_INFO_SEND_CMD",
                      asid, cm_auto_reg_cmds);
        handleAutoRegister(0, asid);    // Sending invalid value 0 as this is not a Call Back
      }
    }
    else
    {
      ...
      * SMS message needs to be sent. */
      ...
      {
        /* Only set it once - not support auto reg on both subs */
        if (ct_autoreg_asid == SYS_MODEM_AS_ID_NONE)
        {
          ct_autoreg_asid = asid;
        }

        cm_auto_reg_cmds |= CM_AUTO_REG_INFO_CHECK_CMD;
        CM_MSG_HIGH_3("sub %d, cm_auto_reg_cmds:%d,CM_AUTO_REG_INFO_CHECK_CMD ct_autoreg_asid %d",asid, cm_auto_reg_cmds,ct_autoreg_asid);
        handleAutoRegister(0, asid);    // Sending invalid value 0 as this is not a Call Back
      }
    }
  }
  ...
  /* return if not cdma or not sub1 */
  if((current_ss_mode = sys_mode) == SYS_SYS_MODE_CDMA || asid != SYS_MODEM_AS_ID_1)
  {
    return;
  }
...
DESCRIPTION
  ...
  SMS message, if necessary.  Only applicable to CT.
...
SIDE EFFECTS
  None
===========================================================================*/
void cm_auto_reg_info_check(sys_modem_as_id_e_type       asid)
{
  ...
  #ifdef FEATURE_MMGSDI_SESSION_LIB
  if(cm_mmgsdi_ssn_tbl_get_type_for_asubs_id_1x(cmmsc_get_1x_sub(),&cm_mmgsdi_session_type) != CM_MMGSDI_SUCCESS)
  {
    CM_ERR_0("Invalid MMGSDI SESSION TYPE");
  }
  #endif
  ...
  if (!(cm_auto_reg_cmds & CM_AUTO_REG_INFO_WAIT_CMD))
  {
    ...
    CM_MSG_HIGH_2("sub %d, cm_auto_reg_cmds:%d- not CM_AUTO_REG_INFO_WAIT_CMD",
                  asid, cm_auto_reg_cmds);

    /* Set the WAIT command so that the CHECK command is not called again. */
    cm_auto_reg_cmds |= CM_AUTO_REG_INFO_WAIT_CMD;

    /* Get the model info read status */
    if(!cm_auto_reg_efs_model_data())
    {
      CM_ERR_1("sub %d, Read EFS MODEL failed, deregister it", asid);
      cm_auto_reg_dereg();

      /*Store ESN if model info can't be retrived. This is to ensure that, any other module(HDR) who is relying on the 
             ESN store are not impacted due to missing autoreg config.*/
      #ifdef FEATURE_MMGSDI_SESSION_LIB
      ...
      {
        CM_MSG_HIGH_1("mmgsdi_session_store_esn(), successful...for ses_type:%d",cm_mmgsdi_session_type);
      }
      else
      {
        CM_ERR_2("mmgsdi_session_store_esn(),failed :status:%d,ses_type:%d",cm_mmgsdi_status,cm_mmgsdi_session_type);
      }
      #endif
    ...
    if (cm_auto_reg_read_ruim_items(NV_RUIM_NAM, asid) != NV_DONE_S)
    {
      /* If the RUIM items could not be read, the SMS message should not be
       * sent. */
      CM_ERR_2("sub %d, cm_auto_reg_cmds:%d- Read RUIM NAM failed",
               asid, cm_auto_reg_cmds);
      return;
    }

    /* Read the IMSI/MEID/ESN items stored in the local NVRAM. */
    if (cm_auto_reg_read_efs_imsi(SYS_MODEM_AS_ID_1) != TRUE)
    {
      /* If the EFS imsi could not be read, the SMS message should not be
       * sent. */
      CM_ERR_2("sub %d, cm_auto_reg_cmds:%d- Read EFS IMSI failed. IMSI not configured in EFS. Proceed with RUIM value",
               asid, cm_auto_reg_cmds);

    }

    /* Read the timer value stored in the local NVRAM. */
    if (cm_auto_reg_read_efs_timer_value(asid) != TRUE)
    {
      /* If the EFS imsi could not be read, the default timer should be used
          to retry . */
      CM_ERR_2("cm_auto_reg_cmds:%d- Read EFS Timer value failed.Not configured in EFS.Proceed with default value=%d",cm_auto_reg_cmds,cm_auto_reg_sms_retry_timer_value);
    }
    ...
    if (cm_auto_reg_get_local_reg_ind(asid))
    {
      /* If the items in the RUIM and EFS are the same, exit. */
      if (cm_auto_reg_compare_efs_ruim_items(asid) && cm_auto_reg_compare_sw_ver(asid))
      {
        CM_MSG_HIGH_1("sub %d, RUIM card and SW Version matches. No Registration Required.", asid);
        cm_auto_reg_dereg();
        return;
      }
      CM_MSG_HIGH_1("sub %d, RUIM card/SW Version differs. Registration Required.", asid);
      cm_auto_reg_set_local_reg_ind(FALSE, asid);
    }
...
)
{
  ...
  cmnv_read_wait_extn (NV_IMSI_MCC_I, &nvi, asid);
  ...
  CM_MSG_HIGH_3("cm_auto_reg_cmds:%d- Read RUIM imsi_mcc=%d- Read RUIM nam=%d",
                cm_auto_reg_cmds, cm_auto_reg_ruim_imsi_data.imsi_mcc.imsi_mcc,
                nvi.imsi_11_12.nam);

  cmnv_read_wait_extn (NV_IMSI_11_12_I, &nvi, asid);
  ...
  cmnv_read_wait_extn (NV_MIN2_I, &nvi, asid);
  ...
  cmnv_read_wait_extn (NV_MIN1_I, &nvi, asid);

  cm_auto_reg_ruim_imsi_data.min1 = nvi.min1;

  CM_MSG_HIGH_6("auto_reg_cmds %d: Read RUIM imsi_mcc_11_12 %d, min2[0] %d min2[1] %d - min1[0] %d min1[1] %d",
...
SIDE EFFECTS
  None
===========================================================================*/
boolean cm_auto_reg_read_efs_imsi(sys_modem_as_id_e_type       asid)
{
  ...
  if(!cmefs_get(CMEFS_IMSI_MCC, (byte *)&cm_auto_reg_efs_imsi_data.imsi_mcc.imsi_mcc, size, asid))
  {
    CM_ERR_2("sub %d, cm_auto_reg_cmds:%d- Read CMEFS_IMSI_MCC failed", asid, cm_auto_reg_cmds);
    return FALSE;
  }
  
  CM_MSG_HIGH_3("SUB %d cm_auto_reg_cmds:%d- Read efs imsi_mcc=%d-",asid,cm_auto_reg_cmds, cm_auto_reg_efs_imsi_data.imsi_mcc.imsi_mcc);

  size = sizeof(cm_auto_reg_efs_imsi_data.min1.min1[CDMAMIN]);
  if(!cmefs_get(CMEFS_IMSI_MIN1, (byte *)&cm_auto_reg_efs_imsi_data.min1.min1[CDMAMIN], size, asid))
  {
    CM_ERR_2("sub %d, cm_auto_reg_cmds:%d- Read CMEFS_IMSI_MIN1 failed", asid, cm_auto_reg_cmds);
    return FALSE;
  }

  size = sizeof(cm_auto_reg_efs_imsi_data.min2.min2[CDMAMIN]);
  if(!cmefs_get(CMEFS_IMSI_MIN2, (byte *)&cm_auto_reg_efs_imsi_data.min2.min2[CDMAMIN], size, asid))
  {
    CM_ERR_2("sub %d, cm_auto_reg_cmds:%d- Read CMEFS_IMSI_MIN2 failed", asid, cm_auto_reg_cmds);
    return FALSE;
  }

  size = sizeof(cm_auto_reg_efs_imsi_data.imsi_11_12.imsi_11_12);
  if(!cmefs_get(CMEFS_IMSI_11_12, (byte *)&cm_auto_reg_efs_imsi_data.imsi_11_12.imsi_11_12, size, asid))
  {
    CM_ERR_2("sub %d, cm_auto_reg_cmds:%d- Read CMEFS_IMSI_11_12 failed", asid, cm_auto_reg_cmds);
    return FALSE;
  }

  CM_MSG_HIGH_1("sub %d, cm_auto_reg_read_efs_imsi(), read IMSI from efs complete", asid);

  return TRUE;
}

/*===========================================================================
FUNCTION cm_auto_reg_read_efs_timer_value

DESCRIPTION
  Reads needed parameters from NVRAM to use later to see if registration
  is necessary.
...
SIDE EFFECTS
  None
===========================================================================*/
boolean cm_auto_reg_read_efs_timer_value(sys_modem_as_id_e_type asid)
{

  int32  size = 0;

  /* Read the Auto Reg SMS retry timer value information from the on-board EFS. */

  size = sizeof(cm_auto_reg_sms_retry_timer_value);
  if(!cmefs_get(CMEFS_REG_SMS_RETRY_TIMER_VALUE, (byte *)&cm_auto_reg_sms_retry_timer_value, size,asid))
  {
    cm_auto_reg_sms_retry_timer_value = CM_AUTO_REG_CT_DEFAULT_RETRY_TIME;
    CM_ERR_1("cm_auto_reg_cmds:%d- Read CMEFS_CT_SMS_TIMER_VALUE failed",cm_auto_reg_cmds);
    return FALSE;
  }

  CM_MSG_HIGH_1("cm_auto_reg_read_efs_timer_value(), read timer value=%d",cm_auto_reg_sms_retry_timer_value);
...
DESCRIPTION
  Reads local NV item to determine registration status.  Only applicable to CT.
...
SIDE EFFECTS
  None
===========================================================================*/
boolean cm_auto_reg_get_local_reg_ind(sys_modem_as_id_e_type        asid)
{
  boolean efs_i = FALSE;
  int32 size = sizeof(boolean);

  if (!cmutil_is_as_id_valid(asid))
  {
    CM_MSG_HIGH_1("sub %d, cm_auto_reg_get_local_reg_ind() FAILED due to invalid sub", asid);
    return FALSE;
  }

  /* Read the local NV item for the local registration indicator. */
  if(cmefs_get(CMEFS_LOCAL_REG_IND, (byte *)&efs_i, size, SYS_MODEM_AS_ID_1))
  {
    CM_MSG_HIGH_2("sub %d, cm_auto_reg_get_local_reg_ind()efs value:%d", asid, efs_i);
  ...
  else
  {
    ...
    CM_MSG_HIGH_1("sub %d, cm_auto_reg_get_local_reg_ind()default:FALSE", asid);
    cm_auto_reg_set_local_reg_ind(FALSE, asid);
    efs_i = FALSE;

    if(!cmefs_fs_write( CMEFS_LOCAL_REG_IND,
                           (byte *)&efs_i,
                           size,
                          SYS_MODEM_AS_ID_1))
    {
      CM_MSG_HIGH_3("sub %d, LOCAL_REG_IND write FAIL size %d value %d", asid,size,efs_i);
    }
...
)
{
  ...
  CM_MSG_HIGH_2("sub %d, cm_auto_reg_set_local_reg_ind():%d", asid, local_reg_ind);
  if (!cmutil_is_as_id_valid(asid))
  {
    CM_MSG_HIGH_1("sub %d, invalid", asid);
    return;
  }
  ...
  if(cmefs_get(CMEFS_LOCAL_REG_IND, (byte *)&efs_i, size, SYS_MODEM_AS_ID_1))
  {
    ...
    if(!cmefs_fs_write( CMEFS_LOCAL_REG_IND,
                          ...
                          SYS_MODEM_AS_ID_1))
    {
      CM_MSG_HIGH_3("LOCAL_REG_IND write FAILURE sub %d, value %d,size %d ", asid,efs_i,size);
    }
  }
  else
  {
...
DESCRIPTION
  Called when WMS polls CM to check if it is receipient of the Msg. Only applicable to CT.
...
{
  /*Parse the message*/
  if((msg_ptr != NULL) && (msg_ptr->msg_hdr.message_mode == WMS_MESSAGE_MODE_CDMA) &&
...
)
{
  ...
  CM_MSG_HIGH_4("sub %d, wms_msg_ms_ack_ext:Status=%d,TL_Status=%d,error=%d",
                asid, status,ack_info.u.cdma.tl_status,ack_info.u.cdma.error_class);
}
...
DESCRIPTION
  Called when the client reg with WMS layer.  Only applicable to CT.
...
SIDE EFFECTS
  None
===========================================================================*/
void cm_auto_reg_info_send(sys_modem_as_id_e_type       asid)
{
  ...
  if (asid != ct_autoreg_asid)
  {
    CM_MSG_HIGH_2("ERROR: ct_autoreg_asid != asid, ct_autoreg_asid %d, asid %d", ct_autoreg_asid, asid);
  }
...
{
  char *p, *start, *end, model_info[CM_AUTO_REG_MANU_CODE_MAX_LEN+CM_AUTO_REG_DEV_MODEL_CT_LEN+1+18];
  char short_model[CM_AUTO_REG_DEV_MODEL_CT_LEN + 1]= {0};
  char short_manu_code[CM_AUTO_REG_MANU_CODE_MAX_LEN]= {0};
  uint32 crc32 = 0;
  uint8 chars_to_write = 0;
  uint8 no_chars_to_write = 0;
  ...
  no_chars_to_write = (uint8) strlen(cm_auto_reg_efs_data->manufacturer_code);
  CM_MSG_HIGH_2("reg_msg_data-manufacturer code:%s,len=%d", cm_auto_reg_efs_data->manufacturer_code, no_chars_to_write);
  ...
  if(no_chars_to_write > CM_AUTO_REG_MANU_CODE_LEN)
  {
    no_chars_to_write = CM_AUTO_REG_MANU_CODE_LEN;
  }

  /* Clear Buffer*/
  memset(short_model, 0, sizeof(short_model));
  memset(model_info, 0, sizeof(model_info));
  memset(short_manu_code, 0, sizeof(short_manu_code));
  ...
  /* Here no_chars_to_write=CM_AUTO_REG_MANU_CODE_MAX_LEN=CM_AUTO_REG_MANU_CODE_LEN + 1 */

  (void) strlcpy(short_manu_code, cm_auto_reg_efs_data->manufacturer_code,(no_chars_to_write+1));
  short_manu_code[no_chars_to_write] = '\0';


  if(chars_to_write || no_chars_to_write)
  {
    tmp_len = no_chars_to_write + chars_to_write + 18 ; /*Length of <a1><b1>-</b1><b2>*/

    CM_MSG_HIGH_3("reg_msg_data- Manu Code=%s,len:%d,tmp_len:%d", short_manu_code,
                  no_chars_to_write, tmp_len);

    (void) snprintf(model_info,sizeof(model_info),"<a1><b1>%s-%s</b1><b2>",short_manu_code,short_model);
...
DESCRIPTION
  Called when a UI received MT SMS or Report. Only applicable to CT.
...
)
{
  ...
  switch(event)
  {
    case  WMS_MSG_EVENT_SUBMIT_REPORT:
      CM_ASSERT(cm_auto_reg_wms_msg != NULL);

      if(cm_auto_reg_wms_msg == NULL)
      {
        CM_ERR_0("SUBMIT REPORT received for NULL msg");
        return;
      }
      ...
      if(cm_auto_reg_wms_msg->u.cdma_message.is_tl_ack_requested  == TRUE)
      {
        ...
        {
          cm_auto_reg_set_local_reg_ind(TRUE, ct_autoreg_asid);
        }
      }
    ...
    case WMS_MSG_EVENT_RECEIVED_MESSAGE:
      ...
      CM_MSG_HIGH_6("sub %d, MT SMS message recv mode:%d TsID:%d;acq_req:%d, ct_autoreg_asid %d, ,trans_id:%d",
                    info_ptr->mt_message_info.as_id,
      ...
      if (cmph_is_msim())
      {
        asid = info_ptr->mt_message_info.as_id;
      }
          ...
          (info_ptr->mt_message_info.message.u.cdma_message.teleservice == CM_AUTO_REG_CT_TELESERVICE_ID))
      {
        if(wms_ts_decode( &info_ptr->mt_message_info.message.u.cdma_message.raw_ts, client_ts_data ) != WMS_OK_S)
        {
          ...
          {
            /*Need not save transaction id. Verify the TL ACK REPORT only for valid MT SMS*/
            cm_auto_reg_msg_send_ack(info_ptr->mt_message_info.transaction_id,WMS_ERROR_TEMP, WMS_TL_UNRECOGNIZED_PARM_VALUE_S, asid);
          }
        ...
        else
        {
              ...
              client_ts_data->u.cdma.user_data.data[1] == 0x04)
          {
            ...
            if( info_ptr->mt_message_info.message.u.cdma_message.is_tl_ack_requested == TRUE )
            {
              /*Save transaction id to verify the TL ACK REPORT*/
              cm_auto_reg_trans_id = info_ptr->mt_message_info.transaction_id;
              cm_auto_reg_msg_send_ack(info_ptr->mt_message_info.transaction_id,WMS_ERROR_NONE, WMS_TL_MAX32, asid);
            }

            else
            {
              /* Deregister  WMS since no TL ACK needs to be sent */
              cm_auto_reg_dereg();
            }

            cm_auto_reg_set_local_reg_ind(TRUE, asid);
          if( cm_auto_reg_info_complete(asid))
            {
              ...
              cm_mmgsdi_status = cm_mmgsdi_ssn_tbl_get_id_for_type(cm_mmgsdi_session_type,
                                 &cm_mmgsdi_session_id);
            CM_MSG_HIGH_3("mmgsdi status %d session type %d session id %d",cm_mmgsdi_status,cm_mmgsdi_session_type,cm_mmgsdi_session_id);
              if( (cm_mmgsdi_status == MMGSDI_SUCCESS) &&
                  (MMGSDI_SUCCESS == mmgsdi_session_store_esn(cm_mmgsdi_session_id,cmmmgsdi_session_store_esn_cb,(mmgsdi_client_data_type) cm_mmgsdi_session_type)))
              {
                CM_MSG_HIGH_1("mmgsdi_session_store_esn(), successful...for ses_type:%d",cm_mmgsdi_session_type);
              ...
              {
              CM_MSG_HIGH_2("mmgsdi_session_store_esn(),failed :status:%d,ses_type:%d",cm_mmgsdi_status,cm_mmgsdi_session_type);
              }
#endif
            }
            else
            {
            CM_MSG_HIGH_0("SMS Reg FAILED...");
            }
          }
          else
          {
            ...
            {
              /*Need not save transaction id. Verify the TL ACK REPORT only for valid MT SMS*/
              cm_auto_reg_msg_send_ack(info_ptr->mt_message_info.transaction_id,WMS_ERROR_TEMP, WMS_TL_UNEXPECTED_PARM_VALUE_S, asid);
            }
          }
        }
      }
    ...
    case  WMS_MSG_EVENT_ACK_REPORT:
      if (cmph_is_msim())
      {
        asid = info_ptr->ack_report_info.as_id;
      }

      CM_MSG_HIGH_5("SUB %d WMS_MSG_EVENT_ACK_REPORT: TL ACK status:%d error:%d,trans_id:%d,cl_id:%d",
                    info_ptr->ack_report_info.as_id,info_ptr->ack_report_info.is_success,
                    info_ptr->ack_report_info.error_status,info_ptr->ack_report_info.transaction_id,
                    info_ptr->ack_report_info.client_id);

      if( (info_ptr->ack_report_info.transaction_id == cm_auto_reg_trans_id) &&
          (info_ptr->ack_report_info.client_id == cm_auto_reg_wms_cid))
      {
        /* Deregister  WMS after receiving ack/nack for "ack of confirm sms" */
        cm_auto_reg_dereg();

        /*Reset Trans id*/
        cm_auto_reg_trans_id = WMS_DUMMY_TRANSACTION_ID;
        /*Setting of reg_status and EFS is completed upon valid MT CONFIRMATION SMS*/
      }
...
DESCRIPTION
  Called when the WMS message should be sent. Only applicable to CT.
...
)
{
  ...
  cm_mm_ss_info_sub_prop_s_type *ss_sub_ptr = cmss_get_info_sub_prop(ct_autoreg_asid);

  if (ct_autoreg_asid == SYS_MODEM_AS_ID_NONE)
  {
    return;
  }
  else if (ct_autoreg_asid != cmmsc_get_1x_sub())
  {
    cm_auto_reg_dereg();
    return;
  }
  ...
  mm_id.asubs_id = ct_autoreg_asid ;
  mm_id.stk_id = MM_STACK_0;
  cmss_get_service_status_per_stack(mm_id, &srv_info);

  if (srv_info.srv_status != SYS_SRV_STATUS_SRV ||
      srv_info.sys_mode != SYS_SYS_MODE_CDMA ||
      srv_info.roam_status != SYS_ROAM_STATUS_OFF)
  {
    CM_MSG_HIGH_4("cm_auto_reg_send_sms:reg sms not sent as srv: %d,mode:%d,roam:%d. Retry timer %d sec started", srv_info.srv_status,srv_info.sys_mode,srv_info.roam_status, cm_auto_reg_sms_retry_timer_value);
    cm_auto_reg_set_timer(cm_auto_reg_sms_retry_timer_value,cm_auto_reg_send_sms);
    return;
  }
  ...
  wms_status  = wms_msg_ms_send(cm_auto_reg_wms_cid, ct_autoreg_asid, cm_auto_reg_send_reg_msg_cb,
                                0, WMS_SEND_MODE_CLIENT_MESSAGE, cm_auto_reg_wms_msg);

  CM_MSG_HIGH_2("SUB %d cm_auto_reg_info_send: wms_status %d",ct_autoreg_asid, wms_status);
...
DESCRIPTION
  Called when the ACK message from the server is received. Only applicable to CT.
...
SIDE EFFECTS
  None
===========================================================================*/
boolean cm_auto_reg_info_complete(sys_modem_as_id_e_type        asid)
{
  nv_stat_enum_type efs_status = NV_FAIL_S;

  if (cm_auto_reg_get_local_reg_ind(asid))
  {
    efs_status=cm_auto_reg_write_efs_imsi(SYS_MODEM_AS_ID_1);
    CM_MSG_HIGH_2("cm_auto_reg_info_complete() IMSI update sub %d, status %d",asid,efs_status);
    efs_status=cm_auto_reg_write_efs_sw_version(SYS_MODEM_AS_ID_1);
    CM_MSG_HIGH_2("cm_auto_reg_info_complete() SW update sub %d, status %d",asid,efs_status);
    return TRUE;
  }
  return FALSE;
}
...
DESCRIPTION
  Stores the values read from RUIM into the EFS. Only applicable to CT.
...
SIDE EFFECTS
  None
===========================================================================*/
nv_stat_enum_type cm_auto_reg_write_efs_imsi(sys_modem_as_id_e_type        asid)
{
  ...
  if ( !cmefs_fs_write( CMEFS_IMSI_MIN1,(byte *)&cm_auto_reg_ruim_imsi_data.min1.min1[CDMAMIN],size, asid))
  {
    CM_MSG_HIGH_2("sub %d, cm_auto_reg_write_efs_imsi: write nv s1 error 0x%X",
             asid, efs_status);
    return efs_status;
  }
  ...
  if ( !cmefs_fs_write( CMEFS_IMSI_MIN2,(byte *)&cm_auto_reg_ruim_imsi_data.min2.min2[CDMAMIN],size, asid))
  {
    CM_MSG_HIGH_2("sub %d, cm_auto_reg_write_efs_imsi: write nv s2 error 0x%X",
             asid, efs_status);
    return efs_status;
  }
  ...
  if ( !cmefs_fs_write( CMEFS_IMSI_MCC,(byte *)&cm_auto_reg_ruim_imsi_data.imsi_mcc.imsi_mcc,size, asid))
  {
    CM_MSG_HIGH_2("sub %d, cm_auto_reg_write_efs_imsi: write nv mcc error 0x%X",
             asid, efs_status);
    return efs_status;
  }
  CM_MSG_HIGH_3("SUB %d cm_auto_reg_cmds:%d- write efs imsi_mcc=%d-",asid,cm_auto_reg_cmds, cm_auto_reg_ruim_imsi_data.imsi_mcc.imsi_mcc);
  ...
  if ( !cmefs_fs_write( CMEFS_IMSI_11_12,(byte *)&cm_auto_reg_ruim_imsi_data.imsi_11_12.imsi_11_12,size, asid))
  {
    CM_MSG_HIGH_2("sub %d, cm_auto_reg_write_efs_imsi: write nv mcc error 0x%X",
             asid, efs_status);
    return efs_status;
  ...
  CM_MSG_HIGH_1("sub %d, cm_auto_reg_write_efs_imsi() Complete", asid);

  return NV_DONE_S;
}

/*===========================================================================
FUNCTION cm_auto_reg_write_efs_sw_version

DESCRIPTION
  Stores the sw version informed in AUTO REG SMS into the EFS.
...
SIDE EFFECTS
  None
===========================================================================*/
nv_stat_enum_type cm_auto_reg_write_efs_sw_version(sys_modem_as_id_e_type asid)
{
  
  int32 size = 0;
  cm_device_model_info_efs *model_ptr = cm_get_device_info();

  /*sw version */
  size = sizeof (model_ptr->sw_version);

  if(cmefs_fs_write(CMEFS_REG_SMS_SW_VER, (byte*)model_ptr->sw_version, size,SYS_MODEM_AS_ID_1))
  {
    CM_MSG_HIGH_3("Write Efs SW Version : Value of sw_ver, 0x%x,0x%x,0x%x", model_ptr->sw_version[0],
										   model_ptr->sw_version[1],
										   model_ptr->sw_version[2]);
  }
  else
  {
    CM_MSG_HIGH_0("Failed to write CMEFS_REG_SMS_SW_VER...");
	return NV_FAIL_S;
  }
...
)
{
  ...
  if (cmd_err == WMS_CMD_ERR_NONE)
  {
    ...
    if((cm_auto_reg_wms_msg != NULL) && (cm_auto_reg_wms_msg->u.cdma_message.is_tl_ack_requested  == TRUE))
    {
...
DESCRIPTION
  Compares UI parameters to see if registration is necessary. Only applicable to CT.

RETURN
  False - A registration message need to be sent.
  True - A registration message does not need to be sent..
...
SIDE EFFECTS
  None
===========================================================================*/
boolean cm_auto_reg_compare_efs_ruim_items(sys_modem_as_id_e_type            asid)
{
  mmgsdi_esn_me_change_enum_type esn_change_flag = mmgsdi_get_esn_me_change_flag(cm_mmgsdi_ssn_tbl_get_slot_for_type(AS_ID_TO_1X_SESSION(asid)));
  ...
  if (esn_change_flag != MMGSDI_ESN_ME_CHANGE && esn_change_flag != MMGSDI_ESN_ME_UPDATE_PENDING)
  {
        ...
        cm_auto_reg_efs_imsi_data.imsi_11_12.imsi_11_12 == cm_auto_reg_ruim_imsi_data.imsi_11_12.imsi_11_12)
    {
      ...
      CM_MSG_MED_1("sub %d, cm_auto_reg_compare_efs_ruim_items: IMSI match", asid);
      /* ...this is the same RUIM as before. */
      return (TRUE);
    }
    else
      /* The RUIM has been reconfigured since last boot. */
      CM_MSG_MED_1("sub %d, cm_auto_reg_compare_efs_ruim_items: IMSI does not match", asid);
  }
...
/*===========================================================================
FUNCTION cm_auto_reg_compare_sw_ver

DESCRIPTION
  Compares current sw version and sw version already informed in AUTO REG SMS.
...
SIDE EFFECTS
  None
===========================================================================*/
boolean cm_auto_reg_compare_sw_ver(sys_modem_as_id_e_type  asid)
{

cm_device_model_info_efs *model_ptr = NULL;
int32 size = 0;
char reg_sms_sw_version[CM_AUTO_REG_SW_VER_MAX_LEN];
...
memset(reg_sms_sw_version,0x0,sizeof(reg_sms_sw_version)); /* memset the entire structure to NULL as this is the entry point */
size = sizeof(reg_sms_sw_version);
if(!cmefs_get(CMEFS_REG_SMS_SW_VER, (byte*)&reg_sms_sw_version, size, SYS_MODEM_AS_ID_1))
{
  CM_ERR_1("cm_auto_reg_cmds:%d- Read CMEFS_REG_SMS_SW_VER failed",cm_auto_reg_cmds);				
  return FALSE;
}

/*compare*/
if(memcmp(reg_sms_sw_version, model_ptr->sw_version, sizeof(reg_sms_sw_version)) != 0)
{
  CM_MSG_HIGH_1("cm_auto_reg_cmds:%d- SW_VER changed",cm_auto_reg_cmds);				
  return FALSE;
}
...
void cm_auto_reg_min1_to_str(dword dwPhoneNo, byte *pszText)
{
  ...
  if( dwPhoneNo > 0 )
  {
    ...
    pszText[nIndex++] = DigitToAscTable[ (wTemp/100)%10 ];
    wTemp %= 100;
...
)
{
  ...
  else if (cmmsc_get_1x_sub() != asid || cm_get_operator_info(asid) != OPERATOR_CT)
  {
    cm_auto_reg_dereg();
  }
  else if ( cm_get_operator_info(asid) == OPERATOR_CT)
  {
    ...
    if (cm_auto_reg_cmds)
    {
      ...
      if (cm_auto_reg_cmds & CM_AUTO_REG_INFO_QUEUE_CMD)
      {
        ...
        cm_auto_reg_ss_event_mdm_cb(cmss_get_info_sys_mode(asid, MM_STACK_0),cmss_get_info_srv_status(asid, MM_STACK_0), asid);
      }
...
DESCRIPTION
  Timer callback function used to transmit registration message. Only applicable to CT.
...
)
{
  ...
  CM_MSG_HIGH_1("cm_auto_reg_cts_msg_timer_cb cm_auto_reg_cmds  %d",cm_auto_reg_cmds);
  if(cm_auto_reg_cmds != CM_AUTO_REG_INFO_COMPLETE_CMD)
  {
  ...
  if ((cm_generic_rpt_ptr = cm_generic_rpt_get_buf_else_err_fatal())!=NULL)
  {
    ...
    cm_generic_rpt_ptr->hdr.asubs_id = ct_autoreg_asid;
...
/*===========================================================================
FUNCTION   cm_auto_reg_init

DESCRIPTION
  This function resets the cmautoreg state machine.
...
void cm_auto_reg_init
(
  void
)
{
  CM_MSG_HIGH_1("cm_auto_reg_init() cm_auto_reg_cmds:%d ->0",cm_auto_reg_cmds);
  ...
  /*Reset auto reg variables*/

        
  cm_auto_reg_sms_sent = FALSE;
  current_ss_mode = SYS_SYS_MODE_NONE;
  cm_auto_reg_sms_retry_timer_value = CM_AUTO_REG_CT_DEFAULT_RETRY_TIME;

  /* Variables used by the China Telecom code
  to send a registration message.
  */
  cm_auto_reg_wms_cid = (wms_client_id_type) WMS_CLIENT_TYPE_MAX;
  cm_auto_reg_cmds = CM_AUTO_REG_INFO_CLEAR_CMD;
  cm_auto_reg_send_sms_retry = 0;
  ct_autoreg_asid = SYS_MODEM_AS_ID_NONE;
