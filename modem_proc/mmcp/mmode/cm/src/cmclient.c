...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/mmode/cm/src/cmclient.c#1 $
...
)
{
  ...
  */
  ...
  if(temp==NULL)
  {
    (void)strlcpy(client_ptr->task_name, "UNKNOWN\0", ARR_SIZE(client_ptr->task_name) );
  ...
  else
  {
    ...
    (void)rex_get_task_name(temp, client_ptr->task_name,sizeof(client_ptr->task_name), NULL);

#else
    (void)strlcpy(client_ptr->task_name, temp->task_name, ARR_SIZE(client_ptr->task_name) );
    client_ptr->task_name[ARR_SIZE(client_ptr->task_name)-1] = '\0';
    /*lint -restore */
#endif
  }
...
)
{
  ...
  {
    CM_MSG_LOW_1("cm_client_release():Client id %d out of range",
                 client_id);
    return CM_CLIENT_OK;
  }
...
FUNCTION cmclient_get_task_name
...
#error code not present
#endif
const char *cmclient_get_task_name(
  cm_client_id_type client_id
  /* Client Id */
)
{
  cm_client_s_type *client_ptr = cmclient_get_client_ptr(client_id);

  if ( client_ptr != NULL )
  {
    return client_ptr->task_name;
  }
  else
  {
    return "UNKNOWN\0";
...
)
{
  ...
  if(!BETWEEN(client_id, CM_CLIENT_ID_ANONYMOUS, CM_CLIENT_IDS_MAX ))
  {
    CM_MSG_LOW_1("cm_client_act: Invalid client id %d", client_id);
    return CM_CLIENT_ERR_CLIENT_ID;
  }
...
)
{
  if(!BETWEEN(client_id, CM_CLIENT_ID_ANONYMOUS, CM_CLIENT_IDS_MAX ))
  {
    CM_MSG_LOW_1("cm_client_deact: Invalid client id %d", client_id);
    return CM_CLIENT_ERR_CLIENT_ID;
  }
...
/*lint -esym(818, client_ptr) */
{
  ...
  {
    CM_MSG_LOW_0("bcmcs_info_ptr NULL");
    return;
  }
...
)
{
  ...
  */
  ...
  if (ss_event == CM_SS_EVENT_RSSI || ss_event == CM_SS_EVENT_HDR_RSSI || ss_event == CM_SS_EVENT_GW_RSSI)
  {
    cmdbg_qsh_update_rssi_fluctuations(ss_info_ptr);
  }
#endif
#endif
  /* Traverse client list and notify the
...
/*lint -esym(818, mbms_info_ptr) */
{
  ...
  #error code not present
#else
  CM_MSG_LOW_0("FEATURE_MBMS not defined");
#endif /* #if defined (FEATURE_MBMS) */
  ...
  */
       ...
       client_ptr = cmclient_list_next(client_ptr) )
  {
    ...
    */
    ...
    {
      if(ph_info_ptr == NULL &&
          (client_ptr->asubs_mask & SYS_MODEM_AS_ID_1_MASK))
      {
        ...
        */
        (void) cmph_info_get( ph_info_ptr, SYS_MODEM_AS_ID_1, CM_PH_EVENT_INFO);
      }
      ...
      */
      ...
      if (cmph_is_msim())
      {
        ...
        ** allocated phone info buffer.
        */
        if( client_ptr->asubs_mask & SYS_MODEM_AS_ID_2_MASK )
        {
          (void) cmph_info_get( ph_info_ptr, SYS_MODEM_AS_ID_2, CM_PH_EVENT_INFO);
        ...
        ** allocated phone info buffer.
        */
        if( client_ptr->asubs_mask & SYS_MODEM_AS_ID_3_MASK )
        {
          (void) cmph_info_get( ph_info_ptr, SYS_MODEM_AS_ID_3, CM_PH_EVENT_INFO);
...
/*lint -esym(818, stats_info_ptr)*/
{
  ...
  CM_MSG_LOW_2("stats evt %d, stats_info_ptr asubs_id %d",
...
)
{
  ...
  switch(client_cmd->cmd)
  {
    ...
    case CM_CLIENT_CMD_ACT:
    {
      ...
      (void) cmph_info_get( ph_info_ptr, SYS_MODEM_AS_ID_1, CM_PH_EVENT_INFO_AVAIL);
    ...
    default:
      CM_MSG_LOW_1("Client cmd %d is unknown", client_cmd->cmd);
  }
...
===========================================================================*/
static void cm_cmd_client_init(
  cm_client_cmd_s_type *client_cmd_ptr
)
{
...
cm_client_cmd_s_type *cm_cmd_alloc_client_init( void )
{
  ...
  /* If command allocation is successful, initialize the client command fields */
