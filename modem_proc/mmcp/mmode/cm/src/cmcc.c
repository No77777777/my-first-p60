...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/mmode/cm/src/cmcc.c#3 $
...
/*Since VIDEO CC is disabled treating same as voice*/
#ifdef FEATURE_GSTK_CC_VIDEO_DISABLED
#define GSTK_VOICE_VIDEO GSTK_VOICE
#endif

typedef struct{

  boolean                         addtnl_char_for_voice_cc;
  /* Indicates whether additional characters need to be
  ** added for international roaming numbers.
  */
}cmcc_sub_info_s_type;

typedef struct{

  cmcc_sub_info_s_type   *sub_info[MAX_SIMS];
  /* cmcc sub info.
  */
...
FUNCTION cmcc_get_sub_ptr

DESCRIPTION
  Gets cmcc sub info pointer

RETURN VALUE
  cmcc_sub_info_s_type*

===========================================================================*/
cmcc_sub_info_s_type* cmcc_get_sub_ptr(
...
)
{
  ...
  {
    return cmcc_ptr()->sub_info[asubs_id];
  }
  else
  {
    return NULL;
...
DESCRIPTION
  Initialize cmcc related information.
...
{
  uint8                    sub_idx;
  cmcc_s_type            *cc_ptr = cmcc_ptr();
  cmcc_sub_info_s_type   *cmcc_sub_ptr;

  if(is_pwr_on)
  {
    for(sub_idx = SYS_MODEM_AS_ID_1; cmutil_is_as_id_valid(sub_idx); sub_idx++)
    {
      cmcc_sub_info_mem_free((sys_modem_as_id_e_type)sub_idx);
      if(cc_ptr->sub_info[sub_idx] == NULL)
      {
        cc_ptr->sub_info[sub_idx] = (cmcc_sub_info_s_type*)
                                  cm_mem_malloc(sizeof(cmcc_sub_info_s_type));
      }

      if(cc_ptr->sub_info[sub_idx] == NULL) return;

      memset(cc_ptr->sub_info[sub_idx], 0, sizeof(cmcc_sub_info_s_type));
    }
  }

  for(sub_idx = SYS_MODEM_AS_ID_1; cmutil_is_as_id_valid(sub_idx); sub_idx++)
  {
    cmcc_sub_ptr = cmcc_get_sub_ptr(sub_idx);
    if(cmcc_sub_ptr && (refresh_subs_mask & BM(sub_idx)))
    {
      if(cmefs_get(CMEFS_ADDTNL_CHAR_FOR_VOICE_CC,
                   (byte *)&(cmcc_sub_ptr->addtnl_char_for_voice_cc),
                   sizeof(cmcc_sub_ptr->addtnl_char_for_voice_cc),
                   (sys_modem_as_id_e_type)sub_idx))
      {
        CM_MSG_HIGH_2("EFS read for CMEFS_ADDTNL_CHAR_FOR_VOICE_CC success sub - %d, "
                      "addtnl_char_for_voice_cc %d",
                       sub_idx,
                       cmcc_sub_ptr->addtnl_char_for_voice_cc);
      }
      else
      {
        cmcc_sub_ptr->addtnl_char_for_voice_cc = FALSE;
...
FUNCTION cmcc_sub_info_mem_free

DESCRIPTION
  This functions frees cmcc sub info.

RETURN VALUE
  None.

===========================================================================*/
void cmcc_sub_info_mem_free(
  sys_modem_as_id_e_type asubs_id
)
{
  cmcc_s_type *cc_ptr = cmcc_ptr();
  ...
  if(!(asubs_id >= SYS_MODEM_AS_ID_1 &&
       asubs_id < ARR_SIZE(cc_ptr->sub_info) &&
     asubs_id < SYS_MODEM_AS_ID_NO_CHANGE))
  {
    CM_MSG_HIGH_1("Free cmcc memory, incorrect sub %d", asubs_id);
    asubs_id = SYS_MODEM_AS_ID_1;
  }

  if(cc_ptr->sub_info[asubs_id] != NULL)
  {
    CM_MSG_HIGH_1("Free cmcc memory for sub %d", asubs_id);
    cm_mem_free(cc_ptr->sub_info[asubs_id]);
    cc_ptr->sub_info[asubs_id] = NULL;
  }
}
...
FUNCTION cmcc_gererate_cc_object_tid

DESCRIPTION
  Allocate a 16 bit TID for call/ sups Call control object

DEPENDENCIES
  None

RETURN VALUE
  uint 16 bit TID value in series

SIDE EFFECTS
  none

===========================================================================*/
uint16 cmcc_gererate_cc_object_tid()
{
  static uint16 tid = 0;
  tid++;
  /* TID 0 is meant to be used for SUPS command only
   * so that it may not interfere with simultaneous call*/
  if (tid == 0) tid ++;
  return tid;
} /* cmcc_gererate_cc_object_tid */
...
)
{
  ...
  */
  ...
  if (call_ptr != NULL && cmcc_object_ptr != NULL)
  {
    /*generate and assign Call control object id to call ptr and call control obj*/
    call_ptr->cc_object_tid = cmcc_gererate_cc_object_tid();
    cmcc_object_ptr->tid    = call_ptr->cc_object_tid;
    CM_MSG_HIGH_1_EXT( "CM CC object allocated for call id %d", call_id, (call_ptr->mm_id.asubs_id+1));
  }
...
/*lint -esym(715,session_type)*/
{
  ...
  client_categ = CALL_CMD_PTR(cc_object_ptr->user_data)->client_categ;
 {
    ...
    if(pbm_session_type < PBM_SESSION_DEFAULT || pbm_session_type >= PBM_SESSION_MAX)
    {
      return;
    }
    ...
    {
      if(cmcc_check_and_update_emerg_srv_category(call_ptr, cc_object_ptr))
      {
        // Emerg category and special category is resolved, no further processing needed. 
        return;
      }
...
)
{
      ...
      file_length != 0)
  {
    ...
    /* handle network provided APNs for both LTE and GW */
#if defined(FEATURE_CM_LTE)
    if(cc_object_ptr->orig_request.ps_cc_sys_mode == SYS_SYS_MODE_LTE)
    {
      if(!cc_object_ptr->orig_request.gw_ps_cc_params.apn_name.valid && acl_file_content[sim_data_index] != 0xdd)
      {
        pass = TRUE;
      }
    }
...
void cmcc_send_envelope_cmd(cmcall_s_type *call_ptr)
{
  ...
  CM_MSG_HIGH_2("Updating cc_object_ptr->stack from %d to %d", cc_object_ptr->stack_id,call_ptr->mm_id.stk_id);
  cc_object_ptr->stack_id = call_ptr->mm_id.stk_id;
  cc_object_ptr->asubs_id = call_ptr->mm_id.asubs_id;
...
)
{
  ...
  CM_MSG_HIGH_6_EXT ("CC_after_sim_cap: call_id %d, cc call_type %d, is_fdn_to_check %d, cdma subs src %d, gw subs src %d, ps_cc_sys_mode=%d",                     
  ...
  if (is_call_allowed_cdma || is_call_allowed_gw)
  {
    ...
    if (cc_object_ptr->orig_request.ps_cc_sys_mode == SYS_SYS_MODE_LTE)
    {
      is_fdn_enabled = FALSE;
      CM_MSG_HIGH_0("LTE FDN disabled");
    }
    else
    #endif
    {
    ...
    ** if FDN is enabled and if this is not a SETUP_CALL */
    ...
    CM_MSG_HIGH_6_EXT("Is FDN enabled %d on session id %d, client_categ %d, ph info slot id: %d prov type: %d, ssn type %d",
                      is_fdn_enabled,
                      cc_object_ptr->sim_capabilities.response_header.session_id,
                      client_categ,
                      pb_info.slot_id,
                      pb_info.prov_type,
                      sess_info.session_type,
  ...
  CM_MSG_HIGH_1_EXT("CC: app_type=%d", app_type, (cc_object_ptr->asubs_id+1));
  ...
  if (cc_object_ptr->call_type == CM_CALL_TYPE_PS_DATA)
  {
    if(is_call_allowed_gw == TRUE)
    {
      ...
      #ifdef FEATURE_CM_LTE
      CM_MSG_HIGH_4_EXT("is_acl_enabled %d, ps_cc_sys_mode %d, acl_avail %d, acl_status %d",
                        is_acl_enabled,
                        cc_object_ptr->orig_request.ps_cc_sys_mode,
...
/*lint -esym(818, gsdi_cnf)*/
{
  ...
  // cmmmgsdi_get_call_id_from_cc_object_tid
  cc_obj_tid = (uint16)gsdi_cnf->response_header.client_data;
  cc_object_ptr = (cmcc_object_s_type *) cmmmgsdi_get_cc_object_from_tid(cc_obj_tid);

  if (cc_object_ptr != NULL)
  {
    call_id = cc_object_ptr->call_id;
    call_ptr = cmcall_ptr (call_id);

    if (cc_object_ptr->call_type == CM_CALL_TYPE_VOICE &&
        (call_ptr == NULL || call_ptr->cc_object_tid != cc_obj_tid))
    {
      CM_MSG_HIGH_0("call_ptr is NULL or cc_object_ptr tid is different from what is stored in call_ptr");
      return;
    }

    cmmmgsdi_deallocate_sim_read_req(call_id, FALSE);
    ...
    CM_MSG_HIGH_5("CC Sim cap callback, status %d, cnf %d, sim_capabilities_valid=%d, call_id=%d, call_type=%d",
      status, cnf,
                  cc_object_ptr->sim_capabilities_valid,
                  cc_object_ptr->call_id,
                  cc_object_ptr->call_type);
...
)
{
  ...
  gstk_call_enum_type gstk_call_type = GSTK_VOICE_VIDEO;
  #else
  ...
  if (cc_object_ptr->sim_cc_required == CMCC_CARD_STATUS_CC_REQUIRED)
  {
    ...
    switch ( gstk_call_type )
    {
      ...
      #ifdef FEATURE_DISABLED_HABANERO
      case GSTK_VOICE_VIDEO:
      {
        if(cc_object_ptr->call_type == CM_CALL_TYPE_VOICE)
        {
          command_data.gstk_address_string.voice_video.media_type = GSTK_MEDIA_TYPE_VOICE;
        }
        else if(cc_object_ptr->call_type == CM_CALL_TYPE_VT)
        {
          command_data.gstk_address_string.voice_video.media_type = GSTK_MEDIA_TYPE_VIDEO;
        }

        command_data.gstk_address_string.voice_video.address.TON =
          GSTK_UNKNOWN_TON;
        command_data.gstk_address_string.voice_video.address.NPI =
          GSTK_ISDN_TELEPHONY_NPI;
        if(call_ptr != NULL)
        {
          CM_MSG_HIGH_1("Fill in number_type %d from call_ptr to cc command", call_ptr->num.number_type);
          switch(call_ptr->num.number_type)
          {
            case CM_NUM_TYPE_INTERNATIONAL:
              command_data.gstk_address_string.voice_video.address.TON = GSTK_INTERNATIONAL_NUM;
              break;

            case CM_NUM_TYPE_NATIONAL:
              command_data.gstk_address_string.voice_video.address.TON = GSTK_NATIONAL_NUM;
              break;

            case CM_NUM_TYPE_NETWORK_SPECIFIC:
              command_data.gstk_address_string.voice_video.address.TON =  GSTK_NETWORK_SPECIFIC_NUM;
              break;

            default:
              command_data.gstk_address_string.voice_video.address.TON =  GSTK_UNKNOWN_TON;
              break;
          }
        }

        /* Special handling if the number buffer has "+" sign in the beginning */
        if( buf[0] == '+')
        {
          command_data.gstk_address_string.voice_video.address.TON =
        ...
        gstk_ascii_to_gstk_address_type(buf,
                                        length,
                                        &command_data.gstk_address_string.voice_video.address);
        /*lint -restore */
        ...
        if(CALL_CMD_PTR(cmd_ptr)->info.cm_gw_mode_info_u.gsm_wcdma_info.bearer_capability_1.present ==
            TRUE)
        {
          int tmp_len = command_data.gstk_address_string.voice_video.ccp1.length;
          if(!(pack_bearer_capability(
                 command_data.gstk_address_string.voice_video.ccp1.ccp,
                 &CALL_CMD_PTR(cmd_ptr)->info.cm_gw_mode_info_u.gsm_wcdma_info.bearer_capability_1,
                 &tmp_len )))
          {
            CM_MSG_HIGH_0("BC1 pack failed");
          }
          command_data.gstk_address_string.voice_video.ccp1.length = (uint8)tmp_len;
        }
        else
#endif /* (defined(FEATURE_WCDMA) || defined(FEATURE_GSM)) */
        {
          command_data.gstk_address_string.voice_video.ccp1.length = 0;
        }
#ifdef CM_GW_SUPPORTED
        if(CALL_CMD_PTR(cmd_ptr)->info.cm_gw_mode_info_u.gsm_wcdma_info.bearer_capability_2.present ==
            TRUE)
        {
          int tmp_len = command_data.gstk_address_string.voice_video.ccp2.length;
          if(!(pack_bearer_capability(
                 command_data.gstk_address_string.voice_video.ccp2.ccp,
                 &CALL_CMD_PTR(cmd_ptr)->info.cm_gw_mode_info_u.gsm_wcdma_info.bearer_capability_2,
                 &tmp_len )))
          {
            CM_MSG_HIGH_0("BC2 pack failed");
          }
          command_data.gstk_address_string.voice_video.ccp2.length = (uint8)tmp_len;
        }
        else
#endif /* (defined(FEATURE_WCDMA) || defined(FEATURE_GSM)) */
        {
          command_data.gstk_address_string.voice_video.ccp2.length = 0;
        }
        if(CALL_CMD_PTR(cmd_ptr)->info.num.subaddr.num_char > 0)
        {
         
            command_data.gstk_address_string.voice_video.subaddress.length =
                memscpy(&command_data.gstk_address_string.voice_video.subaddress.subaddress[0],
                sizeof(command_data.gstk_address_string.voice_video.subaddress.subaddress),
                CALL_CMD_PTR(cmd_ptr)->info.num.subaddr.chari,
                CALL_CMD_PTR(cmd_ptr)->info.num.subaddr.num_char);
        }
        else
        {
          command_data.gstk_address_string.voice_video.subaddress.length = 0;
        }
      ...
      case GSTK_VOICE:
      {
        ...
        if(call_ptr != NULL)
        {
          ...
          {
            case CM_NUM_TYPE_INTERNATIONAL:
              command_data.gstk_address_string.voice.address.TON = GSTK_INTERNATIONAL_NUM;
              break;

            case CM_NUM_TYPE_NATIONAL:
              command_data.gstk_address_string.voice.address.TON = GSTK_NATIONAL_NUM;
              break;

            case CM_NUM_TYPE_NETWORK_SPECIFIC:
              command_data.gstk_address_string.voice.address.TON =  GSTK_NETWORK_SPECIFIC_NUM;
              break;

            default:
              command_data.gstk_address_string.voice.address.TON =  GSTK_UNKNOWN_TON;
              break;
          }
        }
    ...
    ss_info_sub_prop_ptr = cmss_get_info_sub_prop(cc_object_ptr->asubs_id);
    if(ss_info_sub_prop_ptr == NULL || !cmutil_is_as_id_valid(cc_object_ptr->asubs_id) ||
        cc_object_ptr->stack_id < MM_STACK_0 || cc_object_ptr->stack_id > MM_STACK_ALL)
    {
      return;
    }
    stack_id = cc_object_ptr->stack_id;
    if(cc_object_ptr->stack_id == MM_STACK_ALL)
    {
      stack_id = MM_STACK_0;
    }
    ss_info_all_stack_ptr = ss_info_sub_prop_ptr->stack_info[stack_id].gen_info;
    ...
    if(ss_info_all_stack_ptr->sys_mode == SYS_SYS_MODE_LTE || cc_object_ptr->orig_request.ps_cc_sys_mode == SYS_SYS_MODE_LTE)
    {
      ...
      command_data.loc_info.gwl_info.mcc_and_mnc[0] = ss_info_gpp_stack_ptr->cell_info.plmn_id.identity[0];
      command_data.loc_info.gwl_info.mcc_and_mnc[1] = ss_info_gpp_stack_ptr->cell_info.plmn_id.identity[1];
      command_data.loc_info.gwl_info.mcc_and_mnc[2] = ss_info_gpp_stack_ptr->cell_info.plmn_id.identity[2];
      command_data.loc_info.gwl_info.loc_area_code[1] = (byte)(ss_info_gpp_stack_ptr->cell_info.tac & 0x00FF);
      command_data.loc_info.gwl_info.loc_area_code[0] = (byte)((ss_info_gpp_stack_ptr->cell_info.tac & 0xFF00) >> 8);
      if(!ss_info_gpp_stack_ptr->cell_info.lac_id != ss_info_all_stack_ptr->sys_id.id.plmn_lac.lac &&
          !((ss_info_gpp_stack_ptr->cell_info.plmn_id.identity[0] != ss_info_all_stack_ptr->sys_id.id.plmn_lac.plmn.identity[0]) ||
            (ss_info_gpp_stack_ptr->cell_info.plmn_id.identity[1] != ss_info_all_stack_ptr->sys_id.id.plmn_lac.plmn.identity[1]) ||
            (ss_info_gpp_stack_ptr->cell_info.plmn_id.identity[2] != ss_info_all_stack_ptr->sys_id.id.plmn_lac.plmn.identity[2]))
        )
      {
		  //Formatting the cell id per UIM spec ETSI 131 111, Section 8.19 before sending to GSTK 
          eci = (ss_info_gpp_stack_ptr->cell_info.cell_id << 4) | 0x0000000F;
          command_data.loc_info.gwl_info.cell_id[0] = (uint8)((eci >> 24) & 0x000000FF); 
          command_data.loc_info.gwl_info.cell_id[1] = (uint8)((eci >> 16) & 0x000000FF);
          command_data.loc_info.gwl_info.cell_id[2] = (uint8)((eci >> 8) & 0x000000FF);
          command_data.loc_info.gwl_info.cell_id[3] = (uint8)(eci & 0x000000FF);
         
        if(ss_info_all_stack_ptr->sys_mode != SYS_SYS_MODE_GSM)
        {
      ...
      CM_MSG_HIGH_1_EXT("cell_id_len =%d,sys_mode=SYS_SYS_MODE_LTE", 
                        command_data.loc_info.gwl_info.cell_id_len, (cc_object_ptr->asubs_id+1));
    }
    ...
    {
      command_data.rat = cmcc_map_sys_mode_to_rat(ss_info_all_stack_ptr->sys_mode);

      if(ss_info_all_stack_ptr->sys_mode == SYS_SYS_MODE_CDMA)
      {
        command_data.loc_info.cdma_info.base_id = ss_info_all_stack_ptr->mode_info.cdma_info.base_id;
        command_data.loc_info.cdma_info.base_lat = ss_info_all_stack_ptr->mode_info.cdma_info.base_lat;
        command_data.loc_info.cdma_info.base_long = ss_info_all_stack_ptr->mode_info.cdma_info.base_long;
        command_data.loc_info.cdma_info.imsi_11_12 = ss_info_all_stack_ptr->sys_id.id.is95.imsi_11_12;
        command_data.loc_info.cdma_info.mcc = ss_info_all_stack_ptr->sys_id.id.is95.mcc;
        command_data.loc_info.cdma_info.nid = ss_info_all_stack_ptr->sys_id.id.is95.nid;
        command_data.loc_info.cdma_info.sid = ss_info_all_stack_ptr->sys_id.id.is95.sid;
      }
      else
      {
        command_data.loc_info.gwl_info.mcc_and_mnc[0] = ss_info_all_stack_ptr->sys_id.id.plmn.identity[0];
        command_data.loc_info.gwl_info.mcc_and_mnc[1] = ss_info_all_stack_ptr->sys_id.id.plmn.identity[1];
        command_data.loc_info.gwl_info.mcc_and_mnc[2] = ss_info_all_stack_ptr->sys_id.id.plmn.identity[2];
        command_data.loc_info.gwl_info.loc_area_code[1] = (byte)(ss_info_all_stack_ptr->sys_id.id.plmn_lac.lac & 0x00FF);
        command_data.loc_info.gwl_info.loc_area_code[0] = (byte)((ss_info_all_stack_ptr->sys_id.id.plmn_lac.lac & 0xFF00) >> 8);
        if(ss_info_gpp_stack_ptr->cell_info.lac_id == ss_info_all_stack_ptr->sys_id.id.plmn_lac.lac &&
            (ss_info_gpp_stack_ptr->cell_info.plmn_id.identity[0] == ss_info_all_stack_ptr->sys_id.id.plmn_lac.plmn.identity[0]) &&
            (ss_info_gpp_stack_ptr->cell_info.plmn_id.identity[1] == ss_info_all_stack_ptr->sys_id.id.plmn_lac.plmn.identity[1]) &&
            (ss_info_gpp_stack_ptr->cell_info.plmn_id.identity[2] == ss_info_all_stack_ptr->sys_id.id.plmn_lac.plmn.identity[2])
        ...
        {
          command_data.loc_info.gwl_info.cell_id[1] = (uint8)(ss_info_gpp_stack_ptr->cell_info.cell_id & 0x000000FF);
          command_data.loc_info.gwl_info.cell_id[0] = (uint8)((ss_info_gpp_stack_ptr->cell_info.cell_id & 0x0000FF00) >> 8);
          command_data.loc_info.gwl_info.cell_id[3] = (uint8)((ss_info_gpp_stack_ptr->cell_info.cell_id & 0x00FF0000) >> 16);
          command_data.loc_info.gwl_info.cell_id[2] = (uint8)((ss_info_gpp_stack_ptr->cell_info.cell_id & 0xFF000000) >> 24);
          if(ss_info_all_stack_ptr->sys_mode != SYS_SYS_MODE_GSM)
          {
            command_data.loc_info.gwl_info.cell_id_len = 4;
          }
        ...
        CM_MSG_HIGH_2("cell_id_len =%d,sys_mode=%d", command_data.loc_info.gwl_info.cell_id_len, ss_info_all_stack_ptr->sys_mode);
      }
    }
    ...
    if( status != GSTK_SUCCESS )
    {
      ...
      cc_object_ptr->orig_request.call_control_result = CM_CC_RESULT_NOT_ALLOWED;
      cc_object_ptr->orig_request.error_cause = CM_CALL_CMD_ERR_CAUSE_OTHER;
...
)
{
  ...
  if(call_ptr != NULL)
  {
    ...
    if(call_ptr->mm_id.stk_id == MM_STACK_0)
    {
      /*3gpp only*/
          ...
          cmcall_misc_is_mode_pref(call_ptr->favored_mode_pref,CM_MODE_PREF_CDMA_ONLY))
      {
        ...
        if(ss_info_all_stack_ptr->srv_status == SYS_SRV_STATUS_SRV)
        {
          switch(ss_info_all_stack_ptr->sys_mode)
          {
            case SYS_SYS_MODE_CDMA:
              return FALSE;
            case SYS_SYS_MODE_GSM:
    ...
    else if(call_ptr->mm_id.stk_id == MM_STACK_1)
    {
      return FALSE;
    }
  }
...
)
{
  ...
  call_ptr = cmcall_ptr (cc_object_ptr->call_id);
  sups_cmd_ptr = SUPS_CMD_PTR(cmd_ptr);

  if (call_ptr != NULL)
  {
    /* Reassign tid to call control object which is stored in call ptr */
    cc_object_ptr->tid = call_ptr->cc_object_tid;
  }
  ...
  else
  {
    ...
    cc_object_ptr->stack_id = MM_STACK_0;
    cc_object_ptr->asubs_id = sups_cmd_ptr->cmd_info.cmd_subs_id;
  }
  ...
  /* If call type is SUPS :
  ** 1.set voip_cc_sys_mode to WLAN only when 
  ** route calculation indicates sups will go over WLAN .
  ** 2. Exempt this for ue_based_Cw . set voip_cc_sys_mode=WLAN even if
  ** basic WLAN sups app is registered since it will be handeled within UE
  **
  ** If call type is not SUPS, set voip_cc_sys_mode to WLAN based on
  ** registered app on WLAN only 
  */
  sups_sub_info_ptr = cmsups_get_sub_info_ptr(cc_object_ptr->asubs_id);  

  if((cc_object_ptr->call_type == CM_CALL_TYPE_SUPS &&
      cmsups_is_wlan_sups_cmd(sups_cmd_ptr->cmd, cc_object_ptr->asubs_id) &&
      ((sups_sub_info_ptr->sups_cmd_route == CMSUPS_CMD_ALLOW_IMS) ||
        (cmph_get_volte_sub_info_ptr(cc_object_ptr->asubs_id)->ue_based_cw.use_ue_based_cw == TRUE &&
          sups_cmd_ptr->cmd_info.ss_code == cw ))
      ) ||
     (cc_object_ptr->call_type != CM_CALL_TYPE_SUPS &&
      cmcall_is_active_ip_app(SYS_SYS_MODE_WLAN, call_type, cc_object_ptr->asubs_id)) 
    )
  {
    cc_object_ptr->orig_request.voip_cc_sys_mode = SYS_SYS_MODE_WLAN;
  ...
  cc_object_ptr->gw_result_ptr   = &cc_object_ptr->orig_request;
  cc_object_ptr->orig_request.call_control_result = CM_CC_RESULT_ALLOWED_NO_MODIFICATIONS;
  ...
  *(&(cc_object_ptr->cc_result_buf)) = *(&(cc_object_ptr->orig_request));
  ...
  cc_object_ptr->cc_result_buf.call_control_result = CM_CC_RESULT_ALLOWED_NO_MODIFICATIONS;
...
)
{
  ...
  */
  ...
  {
    asubs_id = cc_object_ptr->asubs_id;
  }
  ...
  */
  ...
  CM_MSG_HIGH_4("CALL_CC: asubs_id=%d, session_id %d, session_type=%d, resolved call_type %d",
  ...
  if(call_ptr != NULL)
  {
    ...
    if((call_ptr->call_type == CM_CALL_TYPE_VT) && (cc_object_ptr->call_type == CM_CALL_TYPE_EMERGENCY))
    {
      ...
      CM_MSG_HIGH_1("CALL_CC: Set is_em_vt %d",call_ptr->is_em_vt);
    }
    else
    {
    ...
    if(cc_object_ptr->call_type == CM_CALL_TYPE_EMERGENCY)
    {
      ...
      CM_MSG_HIGH_1_EXT("CALL_CC: E911: Is number emerg on other sub: is_emerg_on_other_sub: %d",
                      call_ptr->is_emerg_on_other_sub, (asubs_id+1));
    }
  ...
  CM_MSG_HIGH_4("CALL_CC: call_control L: call_id %d, call_type %d cc_object_ptr %x, cc_object_ptr->user_data %x",
                cc_object_ptr->call_id, cc_object_ptr->call_type,
  ...
  if(MMGSDI_SUCCESS != mmgsdi_session_get_app_capabilities(
        ...
        (uint32)cc_object_ptr->tid))
  {
    CM_MSG_HIGH_0("CALL_CC: mmgsdi_session_get_app_capabilities failed");
    cmmmgsdi_deallocate_sim_read_req(cc_object_ptr->call_id, FALSE);
    cmcc_object_deallocate( cc_object_ptr );
    return FALSE;
  }
  /*lint -restore */
...
boolean cmcc_call_control_processing_lte (
  ...
  multimode_stack_e_type                  stack_id,
  /* On which stack the call is made */
...
)
{
  ...
  */
  ...
  cmcc_call_control_processing_common(cc_object_ptr,
                                      ...
                                      NULL,
                                      lte_ps_cc_params_ptr);
  cc_object_ptr->stack_id = stack_id;
  cc_object_ptr->asubs_id = asubs_id;

  CM_MSG_HIGH_3_EXT("lte_cc: ps_cc_sys_mode=%d, stack=%d, lte_ps_cc_param as_id=%d",
                    cc_object_ptr->orig_request.ps_cc_sys_mode,
                    cc_object_ptr->stack_id,
                    cc_object_ptr->orig_request.lte_ps_cc_params.as_id,
                    (asubs_id+1));

  /* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

  cm_mmgsdi_status = cm_mmgsdi_ssn_tbl_get_id_for_type(AS_ID_TO_GW_SESSION(asubs_id),
  ...
  else
  {
    ...
    */
    ...
    CM_MSG_HIGH_1_EXT("lte_cc: ps_cc_sys_mode=%d",
                      cc_object_ptr->orig_request.ps_cc_sys_mode, (asubs_id+1));
...
)
{
  ...
  *(&(cc_object_ptr->cc_result_buf)) = *(&(cc_object_ptr->orig_request));
  ...
  else
  {
    CM_ASSERT( CMD_TYPE(cmd_ptr) == CM_CMD_TYPE_SUPS );
    cc_object_ptr->stack_id = MM_STACK_0;
    cc_object_ptr->asubs_id = SUPS_CMD_PTR(cmd_ptr)->cmd_info.cmd_subs_id;
  }
#endif

#if defined(FEATURE_MMODE_TRIPLE_SIM)
  if (cc_object_ptr->asubs_id == SYS_MODEM_AS_ID_3)
  {
...
#endif
    if (cmmsc_get_1x_sub() == cc_object_ptr->asubs_id &&
        cmph_is_subs_feature_mode_1x_sxlte(cc_object_ptr->asubs_id) &&
        cc_object_ptr->stack_id == MM_STACK_2)
    {
  ...
  cc_object_ptr->sim_capabilities_valid = FALSE;
  if(cc_object_ptr->asubs_id == SYS_MODEM_AS_ID_1)
  {
    if(mmgsdi_result != CM_MMGSDI_SUCCESS)
    {
      sess_info.session_type = MMGSDI_GW_PROV_PRI_SESSION;
...
/*lint -esym(818, req_rsp)*/
{
  ...
  CM_MSG_HIGH_0_EXT("GSTK SIM CC callback", (cc_object_ptr->asubs_id+1));
  if (cc_object_ptr->sim_cc_required == CMCC_CARD_STATUS_CC_REQUIRED)
  {
      ...
      */
      switch(req_rsp->cmd.cc_rsp.call_ctrl_result)
      {
        case GSTK_ALLOWED_BUT_MOD:
        {
          ...
          boolean is_voice_video_chg = FALSE;
          req_action = req_rsp->cmd.cc_rsp.cc_req_action;
          #ifdef FEATURE_DISABLED_HABANERO
          CM_MSG_HIGH_3("Call type %d modified to call type: %d, media_type %d",
                        cc_object_ptr->call_type, req_action.call_type, req_action.gstk_address_string.voice_video_info.media_type);

          #ifdef FEATURE_GSTK_CC_VIDEO_DISABLED
          req_action.gstk_address_string.voice_video_info.media_type = GSTK_MEDIA_TYPE_VOICE;
          #endif
          #endif
          switch ( req_action.call_type )
          {
            ...
            case GSTK_VOICE_VIDEO:
              ...
              if(req_action.gstk_address_string.voice_video_info.media_type == GSTK_MEDIA_TYPE_VOICE)
              {
                sim_cc_result_ptr->call_control_result =
                  CM_CC_RESULT_ALLOWED_BUT_MODIFIED_TO_VOICE;

#ifndef FEATURE_GSTK_CC_VIDEO_DISABLED
                if(cc_object_ptr->call_type == CM_CALL_TYPE_VT)
                {
                  cc_object_ptr->call_type = CM_CALL_TYPE_VOICE;
                  is_voice_video_chg = TRUE;
                }
#endif
              }
              else if(req_action.gstk_address_string.voice_video_info.media_type == GSTK_MEDIA_TYPE_VIDEO)
              {
                sim_cc_result_ptr->call_control_result =
              ...
              else if(req_action.gstk_address_string.voice_video_info.media_type == GSTK_MEDIA_TYPE_NONE)
              {
                /*voice/video call Call Control, MEDIA_TYPE_NONE in response means no change in call type
                but if original CC was for SS/USSD and card changed it to VOICE_VIDEO, then MEDIA_TYPE_NONE would default to voice call*/
                switch(cc_object_ptr->call_type)
                {
              ...
              sim_cc_result_ptr->num.len =
                req_action.gstk_address_string.voice_video_info.address.length;
              (void) gstk_address_to_ascii
              (&req_action.gstk_address_string.voice_video_info.address,
               sim_cc_result_ptr->num.buf, CM_MAX_NUMBER_CHARS);
              sim_cc_result_ptr->num.digit_mode = CM_DIGIT_MODE_8BIT_ASCII;

              switch (req_action.gstk_address_string.voice_video_info.address.TON)
              {
              ...
              switch (req_action.gstk_address_string.voice_video_info.address.NPI)
              {
              ...
              if (req_action.gstk_address_string.voice_video_info.ccp1.length
                  > CM_MAX_CCP_LEN)
              {
                sim_cc_result_ptr->ccp1.length = memscpy( sim_cc_result_ptr->ccp1.ccp,
                                                 sizeof(sim_cc_result_ptr->ccp1.ccp),
                                                 req_action.gstk_address_string.voice_video_info.ccp1.ccp,
                                                 sizeof(req_action.gstk_address_string.voice_video_info.ccp1.ccp) );
              }
              else
              {
                sim_cc_result_ptr->ccp1.length = memscpy( sim_cc_result_ptr->ccp1.ccp,
                                                 sizeof(sim_cc_result_ptr->ccp1.ccp),
                                                 req_action.gstk_address_string.voice_video_info.ccp1.ccp,
                                                 req_action.gstk_address_string.voice_video_info.ccp1.length );
              }

              if (req_action.gstk_address_string.voice_video_info.ccp2.length
                  > CM_MAX_CCP_LEN)
              {
                sim_cc_result_ptr->ccp2.length = memscpy( sim_cc_result_ptr->ccp2.ccp,
                                                 sizeof(sim_cc_result_ptr->ccp2.ccp),
                                                 req_action.gstk_address_string.voice_video_info.ccp2.ccp,
                                                 sizeof(req_action.gstk_address_string.voice_video_info.ccp2.ccp) );
              }
              else
              {
                sim_cc_result_ptr->ccp2.length = memscpy( sim_cc_result_ptr->ccp2.ccp,
                                                 sizeof(sim_cc_result_ptr->ccp2.ccp),
                                                 req_action.gstk_address_string.voice_video_info.ccp2.ccp,
                                                 req_action.gstk_address_string.voice_video_info.ccp2.length );
              }


              if(req_action.gstk_address_string.voice_video_info.subaddress.length > 0)
              {
                  ...
                  sim_cc_result_ptr->num.subaddr.num_char =
                       memscpy(&(sim_cc_result_ptr->num.subaddr.chari),
                         MIN(sizeof(sim_cc_result_ptr->num.subaddr.chari), 
                           req_action.gstk_address_string.voice_video_info.subaddress.length),
                           &req_action.gstk_address_string.voice_video_info.subaddress.subaddress,
                             req_action.gstk_address_string.voice_video_info.subaddress.length);
              }
              else
              {
...
#endif
            if(cc_object_ptr->call_type == CM_CALL_TYPE_VOICE || cc_object_ptr->call_type == CM_CALL_TYPE_VT)
            {
              #ifdef FEATURE_DISABLED_HABANERO
              old_call_type = GSTK_VOICE_VIDEO;
              #else
          ...
          CM_MSG_HIGH_4("SIM initiated, client_categ=%d,old_call_type=%d, new_call_type=%d, call_type=%d",
                        client_categ, old_call_type, req_action.call_type, cc_object_ptr->call_type);
          /* if the call is initiated from SIM and
                 ...
                 */
          if(client_categ == CM_ORIG_CLIENT_CATEG_CARD && \
              (old_call_type != req_action.call_type || is_voice_video_chg)
            )
          {
  ...
  }
  {
    ...
    if (cc_object_ptr->call_type == CM_CALL_TYPE_VOICE &&
        (call_ptr == NULL || call_ptr->cc_object_tid != cc_object_ptr->tid))
    {
      CM_MSG_HIGH_2("No call object found for call id %d, %x in cc callback", cc_object_ptr->call_id, call_ptr);
      /* Deallocate cmd_ptr. Otherwise leads to memory leak */
      cm_cmd_dealloc( cc_object_ptr->user_data );
    }
    else
    {
      /* CC complete, invoke CM callback */
...
)
{
  ...
  supp_sd_mode_pref = (cmph_map_cm_mode_pref_to_sd_mode_pref(
                         cmutil_get_supp_mode_pref(CMPH_SS_MODE_PREF(call_ptr->mm_id.asubs_id, cmph_ptr()),
                             CMPH_SS_BAND_PREF(call_ptr->mm_id.asubs_id, cmph_ptr()),
                             CMPH_SS_LTE_BAND_PREF(call_ptr->mm_id.asubs_id, cmph_ptr()),
                             CMPH_SS_TDS_BAND_PREF(call_ptr->mm_id.asubs_id, cmph_ptr()))))
                      &
                      (cmph_map_cm_mode_pref_to_sd_mode_pref(call_ptr->favored_mode_pref));
  CM_MSG_HIGH_1_EXT("supp_sd_mode_pref %d",supp_sd_mode_pref,(call_ptr->mm_id.asubs_id+1));

  if (cmpmprx_read_cm_policy_consumption_eligibility(call_ptr->mm_id.asubs_id))
  {
    cmmsc_state_machine_s_type     *msc_ptr = cmmsimc_state_machine_ptr(call_ptr->mm_id.asubs_id);
    // if SUB is enabled check cap from Policyman pref
    supp_sd_mode_pref &= (cmph_map_cm_mode_pref_to_sd_mode_pref(
                         cmutil_get_supp_mode_pref(
             /*arg 1(mode_pref) */     cmph_map_sd_mode_pref_to_cm_mode_pref(cmutil_map_sys_mode_mask_to_sd_mode_pref(
                                        msc_ptr->stack_common_info.ue_mode_capability)),
                                      cmutil_map_sd_band_pref_to_cm_band_pref((sd_ss_band_pref_e_type)msc_ptr->stack_common_info.ue_band_capability),
                                      msc_ptr->stack_common_info.ue_lte_band_capability,
                                      cmutil_map_sd_tds_band_pref_to_cm_tds_band_pref((sd_ss_band_pref_e_type)msc_ptr->stack_common_info.ue_tds_band_capability))));
     CM_MSG_HIGH_1_EXT("after ANDing with MSC : supp_sd_mode_pref %d",supp_sd_mode_pref,(call_ptr->mm_id.asubs_id+1));
  }
...
)
{
  ...
  /*
  If HDR is in service and app is registered for call type SUPS, route of the SUPS command is over IMS
  and there is no GW stack active, then skip the call control for SUPS command.
  */
  else if ((cmipapp_find_active_app(SYS_SYS_MODE_HDR, CM_CALL_TYPE_SUPS,asubs_id) != CMIPAPP_ID_INVALID) &&
           (stack1_srv_info.srv_status == SYS_SRV_STATUS_SRV) &&
           (sups_sub_info_ptr->sups_cmd_route == CMSUPS_CMD_ALLOW_IMS) &&
           (cc_object_ptr->call_type == CM_CALL_TYPE_SUPS)
           #ifdef CM_GW_SUPPORTED
           && (cmregprx_is_3gpp_inactive(asubs_id))
           #endif
          )
  {
    CM_MSG_HIGH_3("skip further CC for SUPS command over HDR; call_type: %d, cdma_subs_src %d, gw_subs_src %d",
                cc_object_ptr->call_type,
                cdma_subscription_source,
                gw_subscription_source);
    return FALSE;
            ...
            cmclnup_is_ongoing_subsc(BM(call_ptr->mm_id.asubs_id))
          )
  {
...
FUNCTION cmcc_check_and_update_emerg_srv_category

DESCRIPTION
  Before sending the call origination to lower layers, CM will check with PBM if the 
  emerg_srv_Category has been changed since last queried during the call control 

DEPENDENCIES
  Call object must have already been initialized with cmcall_init().

RETURN VALUE
  Return TRUE: if the resolve_call_type needs to be exited right away. 
  Return FALSE: if the further code needs to be executed.
...
boolean cmcc_check_and_update_emerg_srv_category(
cmcall_s_type     *call_ptr, 
cmcc_object_s_type *cc_object_ptr)
{
  ...
  cm_cc_subscription_source_e_type      gw_subscription_source;
  cm_cc_subscription_source_e_type      cdma_subscription_source;
  cm_mmgsdi_session_info_s_type session_info =   {MMGSDI_MAX_SESSION_TYPE_ENUM, 0 };
  cm_mmgsdi_error_e_type        mmgsdi_result = CM_MMGSDI_NOTFOUND;
  
  pbm_ecc_call_resolution_type* pbm_emerg_cat_srv_resolution;
  ...
  pbm_emerg_cat_srv_resolution = (pbm_ecc_call_resolution_type*)cm_mem_malloc(sizeof(pbm_ecc_call_resolution_type));

  if(pbm_emerg_cat_srv_resolution == NULL)
  {
    sys_err_fatal_null_ptr_exception();
    return TRUE;
  }

  /* Get the GW and 1x subscription source */
  
  gw_subscription_source = cmph_get_gw_subscription_source(call_ptr->mm_id.asubs_id);

  cdma_subscription_source = cmph_get_cdma_subscription_source(call_ptr->mm_id.asubs_id);


 	
  /* Decide whether it is a 1x or GW session */
  if(gw_subscription_source != CM_CC_SUBSCRIPTION_SOURCE_NONE &&
      cmcc_call_control_use_gw_session(call_ptr->call_id))
  {
    mmgsdi_result = cm_mmgsdi_ssn_tbl_get_ssn_info_for_gw_slot(
                      ((gw_subscription_source == CM_CC_SUBSCRIPTION_SOURCE_SIM_3)?MMGSDI_SLOT_3:\
                       ((gw_subscription_source == CM_CC_SUBSCRIPTION_SOURCE_SIM_2)?MMGSDI_SLOT_2\
                        :MMGSDI_SLOT_1)), &session_info);
  }
  ...
  else if(cdma_subscription_source != CM_CC_SUBSCRIPTION_SOURCE_NONE)
  {
    ...
    else
    {
      ...
      /* call control on GW SIM card when cdma subscription is from NV */
      mmgsdi_result = cm_mmgsdi_ssn_tbl_get_ssn_info_for_slot(MMGSDI_SLOT_1,
                                                              &session_info);
    }
  }
  ...
  if(mmgsdi_result != CM_MMGSDI_SUCCESS)
  {
    session_info.session_type = MMGSDI_GW_PROV_PRI_SESSION;
    (void)cm_mmgsdi_ssn_tbl_get_id_for_type(session_info.session_type, &session_info.session_id);
    CM_MSG_HIGH_0("mmgsdi_sess_info_ptr NULL. session type set to GW PRI");
  }

  /* Update the PBM session category 
  */
  pbm_emerg_cat_srv_resolution->p_session_type = pbmif_mmgsdi_session_to_pbm_session(session_info.session_type);

  if(pbm_emerg_cat_srv_resolution->p_session_type < PBM_SESSION_DEFAULT
     || pbm_emerg_cat_srv_resolution->p_session_type >= PBM_SESSION_MAX)
  {
    cm_mem_free(pbm_emerg_cat_srv_resolution);
    return TRUE; 
  }
  ...
  if(cc_object_ptr != NULL)
  {
    ...
    pbm_emerg_cat_srv_resolution->num = orig_num_ptr->buf;
    pbm_emerg_cat_srv_resolution->len  = orig_num_ptr->len;
  }
  else
  {
    pbm_emerg_cat_srv_resolution->num = call_ptr->num.buf;
    pbm_emerg_cat_srv_resolution->len = call_ptr->num.len;
  }
	


  // Initializing the emerg category to default 
  pbm_emerg_cat_srv_resolution->ecc_category_ptr_gw = CM_EMERG_NUM_TYPE_NONE;
  pbm_emerg_cat_srv_resolution->ecc_category_ptr_1x = CM_EMERG_NUM_TYPE_NONE;

  

#ifndef FEATURE_NO_SPECIAL_EMERGENCY_SUPPORT
		if(cmph_is_msim() && (cmph_ptr()->device_prop.msim_prop.internal_standby_pref == SYS_MODEM_DS_PREF_SINGLE_STANDBY))
		{
		  pbm_ret = pbm_session_emergency_number_cat_ext2(pbm_emerg_cat_srv_resolution);
		
		  if(!(pbm_emerg_cat_srv_resolution->found_in_gw || pbm_emerg_cat_srv_resolution->found_in_1x))
		  {
		    pbm_ret = pbm_session_check_emergency_number_on_all_slots_ext(pbm_emerg_cat_srv_resolution);
		  }
		}
		else
		{
  ...
  if(cmph_is_msim() && (cmph_ptr()->device_prop.msim_prop.internal_standby_pref == SYS_MODEM_DS_PREF_SINGLE_STANDBY))
  {
	pbm_ret = pbm_session_emergency_number_cat_ext( pbm_emerg_cat_srv_resolution->p_session_type,
											 pbm_emerg_cat_srv_resolution->num,
											 pbm_emerg_cat_srv_resolution->len,
											 &pbm_emerg_cat_srv_resolution->found_in_gw,
											 &pbm_emerg_cat_srv_resolution->found_in_1x,
											 &pbm_emerg_cat_srv_resolution->ecc_category_ptr_gw,
											 &pbm_emerg_cat_srv_resolution->ecc_category_ptr_1x);
	  
    if(!(pbm_emerg_cat_srv_resolution->found_in_gw || pbm_emerg_cat_srv_resolution->found_in_1x))
    {
      pbm_ret = pbm_session_check_emergency_number_on_all_slots(
  											 pbm_emerg_cat_srv_resolution->num,
  											 pbm_emerg_cat_srv_resolution->len,
  											 &pbm_emerg_cat_srv_resolution->found_in_gw,
  											 &pbm_emerg_cat_srv_resolution->found_in_1x,
  ...
  if( cc_object_ptr != NULL)
  {
    ...
    {
      if(call_ptr != NULL && cmcall_get_asubs_id(call_ptr) != cmmsc_get_1x_sub())
      {
        pbm_emerg_cat_srv_resolution->found_in_1x = FALSE;
      }

	  if((pbm_emerg_cat_srv_resolution->special_emergency_number) && (BM(cc_object_ptr->asubs_id) & (pbm_emerg_cat_srv_resolution->special_emergency_number_subs_mask)))
	  {
	    cc_object_ptr->call_type= CM_CALL_TYPE_EMERGENCY;
        call_ptr->special_emergency=TRUE;
        CM_MSG_HIGH_1 ("Nal e call, special_emergency_number_subs_mask %d",pbm_emerg_cat_srv_resolution->special_emergency_number_subs_mask);
	  }
      ...
      if(
         ...
         (pbm_emerg_cat_srv_resolution->found_in_gw)||(pbm_emerg_cat_srv_resolution->found_in_1x)
       ...
       {
         cc_object_ptr->call_type = CM_CALL_TYPE_EMERGENCY;
         if(!(pbm_emerg_cat_srv_resolution->found_in_gw))
         {
           cc_object_ptr->gw_result_ptr = &(cc_object_ptr->cc_result_buf);
         ...
         {
           cc_object_ptr->gw_result_ptr->num.emerg_srv_categ = (cm_emerg_srv_categ_type)pbm_emerg_cat_srv_resolution->ecc_category_ptr_gw;
           CM_MSG_HIGH_1 ("e call, gw categ %d",pbm_emerg_cat_srv_resolution->ecc_category_ptr_gw);
         }
         if(!(pbm_emerg_cat_srv_resolution->found_in_1x))
         {
           cc_object_ptr->cdma_result_ptr  = &(cc_object_ptr->cc_result_buf);
         ...
         else
         {
           ...
           CM_MSG_HIGH_1 ("e call, cdma categ %d",pbm_emerg_cat_srv_resolution->ecc_category_ptr_1x);
         }
         cm_mem_free(pbm_emerg_cat_srv_resolution);

         if (!cmph_is_subscription_available(SYS_MODEM_AS_ID_1_MASK)
             && (!cmph_is_msim() || !cmph_is_subscription_available(SYS_MODEM_AS_ID_2_MASK))
             )
         {
           /**  CHINA EMERGENCY NUMBER TO BE SCANNED ON 1X FIRST **/    
           char* chn_emg_num[]={"110","119","120","122"};
           int i;
  
           for (i=0;i<ARR_SIZE(chn_emg_num);i++)
           {
             if(cm_num_cmp_exact( orig_num_ptr,
               (uint8*)chn_emg_num[i],
                strlen(chn_emg_num[i]) ))
             {
               CM_MSG_HIGH_0("Setting special_emergency to TRUE for");
               for (i=0;i<call_ptr->num.len;i++)
               {
                 CM_MSG_HIGH_1(" %u",call_ptr->num.buf[i]);
               }
               call_ptr->special_emergency=TRUE;
               break;
             }
  ...
  {
    /* When 380 is receied with standard emergency categ for a detected emergency call,
    ** CM should not query for srv_category from PBM.
    */
    if((pbm_ret == PBM_SUCCESS && pbm_emerg_cat_srv_resolution->ecc_category_ptr_gw != CM_EMERG_NUM_TYPE_NONE)
    ...
    )
    {
      ...
      if(call_ptr->num.emerg_srv_categ != (cm_emerg_srv_categ_type) pbm_emerg_cat_srv_resolution->ecc_category_ptr_gw)
  	  {
  	    CM_MSG_HIGH_2 ("emerg_srv_category updated from  %d  to %d", call_ptr->num.emerg_srv_categ, 
                                                                pbm_emerg_cat_srv_resolution->ecc_category_ptr_gw);
  
        call_ptr->num.emerg_srv_categ = (cm_emerg_srv_categ_type) pbm_emerg_cat_srv_resolution->ecc_category_ptr_gw;
      }
...
FUNCTION cmcc_evaluate_addtnl_char_for_voice_cc

DESCRIPTION
  Check whether an additional character '+', needs to be
  added for international roaming numbers. This change was added as a part
  of FR 91121.

  At the moment, only the numbers 525525813300 and 525522729100 (voicemail)
  are being checked.
...
===========================================================================*/
void cmcc_evaluate_addtnl_char_for_voice_cc(
  cmcall_s_type   *call_ptr
)
{
  cmcc_sub_info_s_type    *cmcc_sub_ptr;
  ...
  boolean                  is_roam_mcc;
  uint8                          num_1[CM_MAX_NUMBER_CHARS]
                             = {'5','2','5','5','2','5','8','1','3','3','0','0'};
  uint8                          num_2[CM_MAX_NUMBER_CHARS]
                                   = {'5','2','5','5','2','2','7','2','9','1','0','0'};
  uint8                          num_to_check[2][CM_MAX_NUMBER_CHARS];
  uint8                    num_to_check_len = 12;
  int                      loop = 0;
  cm_mm_ss_info_sub_prop_s_type *ss_sub_ptr = cmss_get_info_sub_prop(call_ptr->mm_id.asubs_id);
  cm_orig_call_control_s_type   *gw_cc_info_ptr = NULL;
  cm_mm_ss_info_gen_stack_info_s_type *ss_stack_ptr;

  memscpy(num_to_check[0], num_to_check_len,
          num_1, num_to_check_len);
  memscpy(num_to_check[1], num_to_check_len,
          num_2, num_to_check_len);
  ...
  CM_MSG_HIGH_1("cmcc_evaluate_addtnl_char_for_voice_cc :  mcc %d",current_mcc);

  is_roam_mcc = cmutil_is_roam_mcc(current_mcc, call_ptr->mm_id.asubs_id);
  cmcc_sub_ptr = cmcc_get_sub_ptr(call_ptr->mm_id.asubs_id);
  
  #ifdef CM_GW_SUPPORTED
  gw_cc_info_ptr = &call_ptr->gw_cc_modified_info;
  #endif
  if(cmcc_sub_ptr == NULL) return;

  if(gw_cc_info_ptr)
  {
    CM_MSG_HIGH_4("CM CC: addtnl_char_for_voice_cc %d, is_roam_mcc %d, call_ptr->num.len %d, gw_cc_info_ptr->num.len %d",
                   cmcc_sub_ptr->addtnl_char_for_voice_cc,
                   is_roam_mcc,
                   call_ptr->num.len,
                   gw_cc_info_ptr->num.len);
  }


  if(cmcc_sub_ptr->addtnl_char_for_voice_cc &&
     is_roam_mcc &&
      ...
      call_ptr->call_type == CM_CALL_TYPE_VT))
  {
    ...
    /* Check both the numbers 525525813300 and 525522729100. */
    for(i = 0; i < 2; i++)
    {  
      /* Adding '+' if user dialed the num_to_check number.
    */
    if(cm_num_cmp_exact(&call_ptr->num,
                      num_to_check[i],
                        MAX(call_ptr->num.len, num_to_check_len)))
    {
    call_ptr->num.buf[0] = '+';
    call_ptr->num.len = call_ptr->num.len + 1;
    memscpy(&call_ptr->num.buf[1], call_ptr->num.len - 1,
            num_to_check[i], num_to_check_len);

    CM_MSG_HIGH_0("Modified number:");
        for(loop = 0; loop < MIN(call_ptr->num.len, num_to_check_len + 1); ++loop)
    {
      CM_MSG_HIGH_1("%c", call_ptr->num.buf[loop]);
    }
    }
   /* Adding '+' when number is modified to num_to_check after call control phase 2.
   ** The num in gw_cc_info_ptr is modified and sent for call origination after call control 
   ** phase 2 while the num in call_ptr remains unchanged.
   */
   if(gw_cc_info_ptr &&
     cm_num_cmp_exact(&gw_cc_info_ptr->num,
                          num_to_check[i],
                      MAX(gw_cc_info_ptr->num.len, num_to_check_len)))
   {
    gw_cc_info_ptr->num.buf[0] = '+';
    gw_cc_info_ptr->num.len = gw_cc_info_ptr->num.len + 1;
    memscpy(&gw_cc_info_ptr->num.buf[1], gw_cc_info_ptr->num.len - 1,
                num_to_check[i], num_to_check_len);
  
    /* Note: Print to be removed after testing.
    */
    CM_MSG_HIGH_0("Modified number:");
    for(loop = 0; loop < MIN(gw_cc_info_ptr->num.len, num_to_check_len + 1); ++loop)
    {
      CM_MSG_HIGH_1("%c", gw_cc_info_ptr->num.buf[loop]);
    }
   }
   }
  }
...
)
{
  ...
  if(!cmph_is_msim()
  || cmph_ptr()->device_prop.msim_prop.internal_standby_pref == SYS_MODEM_DS_PREF_SINGLE_STANDBY)
  {
    return FALSE;
  }
  if(call_ptr == NULL)
  ...
  pbm_ecc_call_resolution=(pbm_ecc_call_resolution_type*)cm_mem_malloc(sizeof(pbm_ecc_call_resolution_type));
  if(pbm_ecc_call_resolution == NULL)
  {
    sys_err_fatal_null_ptr_exception();
    return FALSE;
  }
  pbm_ecc_call_resolution->num = orig_num_ptr->buf;
  pbm_ecc_call_resolution->len = orig_num_ptr->len;
  session_type = AS_ID_TO_GW_SESSION(cmph_get_other_sub(call_ptr->mm_id.asubs_id));

  pbm_ecc_call_resolution->p_session_type = pbmif_mmgsdi_session_to_pbm_session(session_type);

    if(pbm_ecc_call_resolution->p_session_type < PBM_SESSION_DEFAULT
       || pbm_ecc_call_resolution->p_session_type >= PBM_SESSION_MAX)
    {
      return FALSE;
    }

#ifndef FEATURE_NO_SPECIAL_EMERGENCY_SUPPORT
  pbm_ret = pbm_session_emergency_number_cat_ext2(pbm_ecc_call_resolution);
#else
  pbm_ret = pbm_session_emergency_number_cat_ext( pbm_ecc_call_resolution->p_session_type,
                                                orig_num_ptr->buf,
                                                orig_num_ptr->len,
                                                &pbm_ecc_call_resolution->found_in_gw,
                                                &pbm_ecc_call_resolution->found_in_1x,
                                                &pbm_ecc_call_resolution->ecc_category_ptr_gw,
                                                &pbm_ecc_call_resolution->ecc_category_ptr_1x);
#endif

  /* If number is emergency only in CDMA & call is originated on sub2,
     ** then number callnot be emergency on other sub*/
  if(pbm_ret == PBM_SUCCESS &&
     (pbm_ecc_call_resolution->found_in_gw || (pbm_ecc_call_resolution->found_in_1x && call_ptr->mm_id.asubs_id != cmmsc_get_1x_sub())))
  {
    cm_mem_free(pbm_ecc_call_resolution);
    return TRUE;
  }
  cm_mem_free(pbm_ecc_call_resolution);
  return FALSE;
