...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/mmode/cm/src/cmreply.c#1 $
...
)
{
  ...
  if ( IS_VALID_PH_SUB_PTR(SYS_MODEM_AS_ID_1)&&
       ph_ptr->sub_info[SYS_MODEM_AS_ID_1]->sub_3gpp2_pref.is_hold_user_act)
  {
    CM_MSG_HIGH_1(" HOLD USER ACTIVITY status %d", ph_ptr->sub_info[SYS_MODEM_AS_ID_1]->sub_3gpp2_pref.is_hold_user_act);
    return TRUE;
  }
  ...
  if (CMD_TYPE(cmd_ptr)== CM_CMD_TYPE_PH )
  {
    ...
    */
    if( PH_CMD_PTR(cmd_ptr)->cmd == CM_PH_CMD_PACKET_STATE &&
        cm_reply_check(&ph_ptr->device_prop.reply, CM_DATA_PACKET_STATE_RPT))
    {
      cmd_wait = FALSE;
    }

    /* unblock ph command CM_PH_CMD_SET_PROC_TYPE when waiting
    ** for IMS SRV_DOM confirmation
    */
    if( PH_CMD_PTR(cmd_ptr)->cmd == CM_PH_CMD_SET_PROC_TYPE &&
       cm_reply_check(&ph_ptr->device_prop.reply, CM_IP_APP_ALLAPPS_SRV_DOM_CNF))
    {
      CM_MSG_HIGH_0("DUAL_LTE: LTE_PROC_TYPE: Allowed while waiting for SRV_DOM_CNF");
      cmd_wait = FALSE;
    }
           ...
           CALL_CMD_PTR(cmd_ptr)->info.call_id != CM_CALL_ID_INVALID)
  {
    ...
    {
      cm_ph_cmd_s_type  *reply_cmd_ptr = (cm_ph_cmd_s_type *)ph_ptr->device_prop.reply.cmd_ptr;
      ...
      else if( CALL_CMD_PTR(cmd_ptr)->cmd == CM_CALL_CMD_ORIG &&
               (CALL_CMD_PTR(cmd_ptr)->info.call_type != CM_CALL_TYPE_PS_DATA) &&
               (reply_cmd_ptr != NULL) &&
               (reply_cmd_ptr->cmd == CM_PH_CMD_DUAL_STANDBY_PREF) &&
               (reply_cmd_ptr->info.dds_switch_type == DDS_CAUSE_PERMANENT))
      {
        cmd_wait = TRUE;
      }
