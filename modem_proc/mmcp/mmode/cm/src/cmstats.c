...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/mmode/cm/src/cmstats.c#1 $
...
/*lint -esym(528, cmstats_process_cm_stats_mask) not referenced */
{
  ...
  if (ss_info_sub_prop_ptr->stack_info[MM_STACK_0].gen_info == NULL)
  {
    return;
  }
  ...
  */
  ...
  if ( bit_mask & SYS_CM_PH_PREF_BIT_MASK )
  {
    ...
    stats_info_ptr->prm.cm_info.ph_info.pref_sys =
      ...
      (CMPH_PRST_SS_PRL_PREF(asubs_id,cmph_ptr() ) );

    /* Phone mode */
    /*lint -e666 */
    stats_info_ptr->prm.cm_info.ph_info.mode_pref =
      (sys_stats_mode_pref_e_type) (CMPH_PRST_SS_MODE_PREF(asubs_id, cmph_ptr() ));

    /* Band capability */
    stats_info_ptr->prm.cm_info.ph_info.band_cap = cmph_ptr()->device_prop.hw_capability.band_capability;
  ...
  if ( bit_mask & SYS_CM_SYS_SEL_INFO_BIT_MASK )
  {
    ...
    /* Selected system */
    stats_info_ptr->prm.cm_info.sys_sel_info.sel_sys = ss_info_sub_prop_ptr->stack_info[MM_STACK_0].gen_info->sys_id;

    /* Raoming indication */
    stats_info_ptr->prm.cm_info.sys_sel_info.roam_ind =
      (sys_stat_roam_e_type) ss_info_sub_prop_ptr->stack_info[MM_STACK_0].gen_info->roam_status;
  ...
  {
    /* PRL version */
    stats_info_ptr->prm.cm_info.prl_ver = sd_misc_prl_ver_get(asubs_id);
  ...
  */
  switch (cmd_info_ptr->module)
  {
    ...
    case SYS_MODEM_MODULE_WCDMA:
      ...
      */
#ifdef FEATURE_SEGMENT_LOADING
      IWCDMA_wcdma_set_modem_stats_mask_sub (pi_mmode_wcdma,stats_cmd_ptr->asubs_id,
                                             (uint16)wcdma_bit_mask );
#else
      wcdma_set_modem_stats_mask_sub (stats_cmd_ptr->asubs_id,(uint16)wcdma_bit_mask );
#endif
    ...
    case SYS_MODEM_MODULE_CM:
      ...
      /* Process CM request */
      cmstats_process_cm_stats_mask( global_cm_bit_mask, stats_cmd_ptr->asubs_id);
      break;
    ...
    case SYS_MODEM_MODULE_HDR:
    {
      ...
      CM_MSG_HIGH_2("CM->MSGR: sub %d, stats: HDR bit mask set - %d",
                    stats_cmd_ptr->asubs_id, cmd_info_ptr->bit_mask);
      (void) cm_msgr_send( HDR_CP_SET_FTD_STATS_MASK_REQ, MSGR_MM_CM,
                           &(hdrcp_req->hdr), sizeof(hdrcp_msg_req_u),
                           stats_cmd_ptr->asubs_id);
...
#ifdef FEATURE_SEGMENT_LOADING
  IWCDMA_wcdma_get_modem_stats_info_sub( pi_mmode_wcdma, asubs_id,wcdma_info_ptr );
#else
  wcdma_get_modem_stats_info_sub(asubs_id,wcdma_info_ptr);
#endif
...
/*lint -esym(714,cmstats_process_lte_stats_info) */
{
  ...
  */
  ...
  for (cnt = 0; cnt < temp_sys_info_ptr->lte_inter.num_freqs; cnt++)
  {
    ...
    CM_ASSERT ( temp_sys_info_ptr->lte_inter.freqs[cnt].num_lte_cells <= SYS_LTE_NGBR_IND_NUM_CELLS);
  ...
  CM_ASSERT (temp_sys_info_ptr->gsm.num_freq_groups <= SYS_LTE_NGBR_IND_NUM_FREQS);

  CM_CHECK_MAX_AND_DISCARD_ITEMS( temp_sys_info_ptr->gsm.num_freq_groups,
                                  SYS_LTE_NGBR_IND_NUM_FREQS,
                                  "GSM Freq groups")

  for (cnt = 0; cnt < temp_sys_info_ptr->gsm.num_freq_groups; cnt++)
  {
    ...
    for (cnt2 = 0; cnt2 < temp_sys_info_ptr->gsm.freq_group[cnt].num_gsm_arfcn; cnt2++)
    {
  ...
  CM_ASSERT (temp_sys_info_ptr->wcdma.num_wcdma_freqs <= SYS_LTE_NGBR_IND_NUM_FREQS)

  CM_CHECK_MAX_AND_DISCARD_ITEMS( temp_sys_info_ptr->wcdma.num_wcdma_freqs,
                                  SYS_LTE_NGBR_IND_NUM_FREQS,
                                  "W freqs")

  for (cnt=0; cnt < temp_sys_info_ptr->wcdma.num_wcdma_freqs; cnt++)
  {
    ...
    CM_ASSERT (temp_lte_info_ptr->wcdma.freq[cnt].num_wcdma_cells <= SYS_LTE_NGBR_IND_NUM_CELLS);
    ...
    for (cnt2 = 0; cnt2 < temp_sys_info_ptr->wcdma.freq[cnt].num_wcdma_cells ; cnt2++ )
    {
...
/*lint -esym(765, cmstats_process_stats_signal) */
{
  ...
  switch(module)
  {
    ...
    case SYS_MODEM_MODULE_CDMA:
      ...
      cmstats_process_cdma_stats_info(asubs_id);
      break;
#endif
    ...
    case SYS_MODEM_MODULE_HDR:
      ...
      cmstats_process_hdr_stats_info(asubs_id);
      break;
