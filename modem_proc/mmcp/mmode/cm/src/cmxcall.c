...
static cmxcall_out_of_order_msg_s_type *ooo_msg_ptr[MAX_SIMS] = {NULL};
...
)
{
  ...
  if( call_ptr->sr.orig_sent_on_mode == SYS_SYS_MODE_CDMA )
  {
    ...
    */
    /* If call is placed on 1x and need to switch to SRLTE at call end,
    ** wait till 1x call end cnf then switch to SRLTE so that
    ** 1x can end the call with the nw gracefully.
    */
    if( ((ph_sub_ptr->ss_susp_info[MM_STACK_2].is_suspend == TRUE ||
         ph_sub_ptr->ss_susp_info[MM_STACK_1].is_suspend == TRUE )
        &&
        cmcall_is_1x_data_srv_opt(call_ptr->cmcall_mode_info.info.cdma_info.srv_opt)) 
      ||
        call_ptr->switch_to_srlte
      )
    {
  ...
  */
  switch( cm_sys_mode )
  {
    case SYS_SYS_MODE_CDMA:
      return MC_DIGITAL_MODE_CDMA;
    case SYS_SYS_MODE_HDR:
      return MC_DIGITAL_MODE_HDR;
    default:
...
)
{
  ...
  {
    case SYS_SYS_MODE_CDMA:
      return HDRCP_DIGITAL_MODE_CDMA;
    case SYS_SYS_MODE_HDR:
      return HDRCP_DIGITAL_MODE_HDR;
    default:
...
/*lint -esym(715,call_ptr, end_reason)*/
{
  ...
  CM_MSG_HIGH_2("CM->MSGR: sub %d, send HDR_CP_CALL_END_REQ, reason=%d",
                call_ptr->mm_id.asubs_id, end_reason);
...
)
{
  ...
  CM_MSG_HIGH_2 ("Send Call End: id:%d, reason:%d", call_id, end_reason );
...
FUNCTION cmcall_send_mc_csfb_end_cmd

DESCRIPTION
  Send CSFB end command to MC task
...
===========================================================================*/
void                             cmcall_send_mc_csfb_end_cmd(
...
{
#if !defined(FEATURE_MMODE_REMOVE_1X) && defined(FEATURE_LTE_TO_1X)
  mc_msg_type *mc_ptr;

  /* The hybrid operation is not enabled so
    send the end to MC to end the call*/

  CM_MSG_HIGH_2 ("Send CSFB Call End: as_id %d id:%d", asubs_id, call_id );
  ...
  mc_ptr->csfb_call_end.call_id = call_id;

  cm_mc_send_cmd( asubs_id, mc_ptr );
#else
  SYS_ARG_NOT_USED(call_id);
  SYS_ARG_NOT_USED(asubs_id);
  CM_ERR_0("FSN: 1X is not supported on this target");
#endif
}
...
)
{
  ...
  {
#if defined (FEATURE_LTE_TO_1X)
    if (call_ptr->csfb_type == CM_CALL_CSFB_TYPE_1XCSFB && sds_sub_prop_ptr->gpp2_sub_info.is_1xcsfb_activated)
    {
      ...
      */
      cmcsfbcall_send_csfb_abort_req(call_ptr->call_id, call_ptr->mm_id.asubs_id);
    ...
    cmcall_send_mc_end_cmd(call_ptr->mm_id.asubs_id,call_ptr->call_id,end_reason);
...
)
{
  ...
  */
  ...
  ss_info_sub_prop_ptr = cmss_get_info_sub_prop(cm_mc_rpt_ptr->hdr.asubs_id);
  if(ss_info_sub_prop_ptr == NULL || !cmutil_is_as_id_valid(cm_mc_rpt_ptr->hdr.asubs_id))
  {
    return;
  }
  ...
  */
  ...
  call_info_ptr->asubs_id   = cm_mc_rpt_ptr->hdr.asubs_id;

  call_info_ptr->mode_info.info_type = CM_CALL_MODE_INFO_CDMA;
  call_info_ptr->msg_type            = cm_mc_rpt_ptr->info_hdr.msg_type;
  ...
  switch(call_event)
  {
    ...
    case CM_CALL_EVENT_CALLER_ID:
      ...
      */
      ...
      num_details.current_mobile_country_code = ss_info_sub_prop_ptr->sub_info.gpp2_sub_info.hpcd_info.current_mcc;
      num_details.home_mobile_country_code = ss_info_sub_prop_ptr->sub_info.gpp2_sub_info.hpcd_info.home_mcc;
      num_details.num                  = call_info_ptr->num;

      num_details.hs_based_plus_dial_setting = ss_info_sub_prop_ptr->sub_info.gpp2_sub_info.hpcd_info.plus_dial_setting;
...
)
{
  ...
  */
  {
    mm_sub_stk_id_s_type local_mm_id = call_ptr->mm_id;
  ...
  */
  ...
  if (call_mode_info_ptr->otasp_info.is_commit_successful)
  {
    ...
    */
    cmph_send_nam_change_to_mc( call_ptr->mm_id.asubs_id, ph_ptr->device_prop.curr_nam );

    /* Flag set to show that subscription change is pending.
    */
    if(ph_sub_ptr)
    {
      ph_sub_ptr->sub_3gpp2_pref.is_otasp_commit_pending = TRUE;
    }
  }
#endif /* FEATURE_OTASP */
...
===========================================================================*/
static void cmxcall_suspend_hybrid_operation(sys_modem_as_id_e_type as_id)

{
  ...
  /* If stack 2 is not pending for deactivation
  ** and operational there is some request pending with MMOC
  ** or currently stack 2 is not suspended, send suspend to HYBR2 and update
  ** Cmph to indicate that CM is waiting for MMOC response
  */
  if( !cmtask_is_stack_pending_for_deact(cmph_map_sub_stk_to_id(as_id, MM_STACK_2)) &&
      ( ph_sub_ptr->ss_susp_info[MM_STACK_2].is_suspend == FALSE ||
        ph_sub_ptr->ss_susp_info[MM_STACK_2].num_requests != 0
      )
    )
  {
    mm_id.stk_id = MM_STACK_2;
    mmoc_cmd_suspend_stack(mm_id,TRUE,FALSE);
    cmph_update_suspend_status(mm_id, TRUE, TRUE);
  }
  /* If stack 1 is not pending for deactivation
  ** and is supported and there is some request pending with MMOC,
     or currently stack 1 is not suspended ,send suspend to stack 1 and update
     Cmph to indicate that CM is waiting for MMOC response
  */
  if(!cmtask_is_stack_pending_for_deact(cmph_map_sub_stk_to_id(as_id, MM_STACK_1)) &&
      ( ph_sub_ptr->ss_susp_info[MM_STACK_1].is_suspend == FALSE ||
...
===========================================================================*/
void cmxcall_unsuspend_hybrid_operation( sys_modem_as_id_e_type as_id )

{
  ...
  */
      ...
      ph_sub_ptr->sub_3gpp2_pref.mt_1x_data_await_stage == CM_AWAITING_NONE)
  {
    ...
    ** is_suspend state is false and stack is not pending for deactivation,
    ** but there is a pending request sent to MMOC
    ...
    */
    ...
    {
      mm_id.stk_id = MM_STACK_1;
      if( cmss_is_stack1_operational(mm_id.asubs_id) )
      {
        mmoc_cmd_suspend_stack(mm_id, FALSE,FALSE);
      }
      else
...
===========================================================================*/
static void cmxcall_page_proceed(sys_modem_as_id_e_type asubs_id)

{
  ...
  */
  cmxcall_suspend_hybrid_operation(asubs_id);

  /* If no suspend cmd has been sent, inform 1xcp immediately to proceed
  */
  if(ph_sub_ptr->sub_3gpp2_pref.mt_1x_data_await_stage != CM_AWAITING_NONE)
  {
    if( ph_sub_ptr->ss_susp_info[MM_STACK_1].num_requests == 0 &&
        ph_sub_ptr->ss_susp_info[MM_STACK_2].num_requests == 0 )
    {
      ph_sub_ptr->sub_3gpp2_pref.mt_1x_data_await_stage = CM_AWAITING_INCOMING_F;
      cmxcall_mc_incoming_page_proceed(asubs_id, TRUE);
    }
    else
    {
      ph_sub_ptr->sub_3gpp2_pref.mt_1x_data_await_stage = CM_AWAITING_SUSPEND_RSP;
    }

    CM_MSG_HIGH_1("mt_1x_data_wait_stage moved to %d",ph_sub_ptr->sub_3gpp2_pref.mt_1x_data_await_stage);
  }
...
)
{
  ...
  {
    CM_MSG_HIGH_1("Invalid sub %d", asubs_id);
    return;
  }
  ...
  */
  ...
  {
    ph_sub_ptr->sub_3gpp2_pref.mt_1x_data_await_stage = CM_AWAITING_NONE;
    cmxcall_mc_incoming_page_proceed(asubs_id, FALSE);
  }
  else
  {
    /* If CM decides to proceed with 1x page, it needs to suspend HYBR2 and
    ** HYBR1 stacks.*/
    cmxcall_page_proceed(asubs_id);
  }
...
)
{
...
#endif
  ss_info_sub_prop_ptr = cmss_get_info_sub_prop(call_ptr->mm_id.asubs_id);
  if(ss_info_sub_prop_ptr == NULL || ph_sub_ptr == NULL)
  {
    return;
  }
  ...
  */
  ...
  mc_ptr->origination.is_dormant_packet_data =
    (ph_sub_ptr->ps_pref.packet_state == CM_PACKET_STATE_DORMANT ) ? TRUE : FALSE;
  ...
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  CM_MSG_HIGH_6("MC_ORIG: is_sr=%d, sr_cnt=%d, is_emrg=%d is_pkt=%d, is_dormant_pkt=%d, special_srv =%d",
  ...
  */
  ...
  /* Log MC origination */
  cmlog_call_orig_send_mc_orig(call_ptr->mm_id.asubs_id);
#else
#error code not present
#endif
}
...
)
{
  ...
  CM_MSG_HIGH_1("CM->MSGR: sub %d, send HDR_CP_CALL_ORIGINATION_REQ",
                call_ptr->mm_id.asubs_id);
  ...
  cmlog_call_orig_send_hdr_orig(call_ptr->mm_id.asubs_id);
...

{
  ...
  ** all Voice calls in the Priority Queue */
  ...
  /* Currently CDMA service can only be acquired on stack 0
  ** so we only need to search the pri queue for stack 0.
  ** If this behavior changes in future, the ss value must be
  ** accordingly initialized */

  ph_orig_q_ptr = cmtask_orig_para_get_top( call_ptr->mm_id );
  ...
  if( ph_orig_q_ptr != NULL )
  {
    while( ph_orig_q_ptr->act_type != CM_ACT_TYPE_PH_OBJ )
    {
      ...
      ph_orig_q_ptr = cmtask_orig_para_get_next( ph_orig_q_ptr,call_ptr->mm_id);
      if ( ph_orig_q_ptr == NULL )
      {
        break;
      }
...
)
{
...
#ifdef FEATURE_HDR
  ss_info_sub_prop_ptr = cmss_get_info_sub_prop(call_ptr->mm_id.asubs_id);
  if(ss_info_sub_prop_ptr == NULL || !cmutil_is_mm_id_valid(call_ptr->mm_id) ||
  ...
  {
#ifdef FEATURE_HDR
    if((call_ptr->mm_id.stk_id == MM_STACK_1||
        ( call_ptr->mm_id.stk_id == MM_STACK_0 &&
          ss_info_all_stack_ptr->sys_mode == SYS_SYS_MODE_HDR
        )) && cmss_get_misc_sub_prop_ptr(call_ptr->mm_id.asubs_id) != NULL
      )
    {
#ifdef FEATURE_DEDICATED_C2K_MODE
      CM_MSG_HIGH_0("DCM: Opening HDR session");
      cmss_get_misc_sub_prop_ptr(call_ptr->mm_id.asubs_id)->sub_info.gpp2_sub_info.is_hdr_session_open = TRUE;
      cmph_update_ddtm_status( call_ptr->mm_id.asubs_id );
#endif
      CM_MSG_HIGH_3("ORIG: CM->HDR: Send HDRMC orig for call_id %d, call_type %d, sub %d",
                       call_ptr->call_id,
                       call_ptr->call_type,
                       call_ptr->mm_id.asubs_id);
...
#endif /* FEATURE_HDR */
    {
      ...
      */
      ...
      CM_MSG_HIGH_4("ORIG: CM->1X: Send MC orig, is_pri_voice_call %d, call_id %d, call_type %d, sub %d",
                       is_pri_voice_call,
                       call_ptr->call_id,
                       call_ptr->call_type,
                       call_ptr->mm_id.asubs_id);
...
)
{
                                     ...
                                     CMXCALL_MOBILE_ORIGINATED) )
  {
    ...
    CM_ERR_0( "CM_CALL_CMD_ERR_SRV_OPT_P");
           ...
           (cmph_get_user_mode_pref(call_cmd_ptr->cmd_subs) != CM_MODE_PREF_HDR_ONLY) &&
  ...
  /* Currently, we are not supporting Band Class 2 & 13 */
            ...
            !( sd_misc_is_target_supp_mode_band_class(SD_SS_MODE_PREF_CDMA,
                cmutil_map_otasp_act_code_to_sd_band(call_cmd_ptr->info.cdma_info.otasp_act_code))) )
  {
  ...
  */
  switch( call_cmd_ptr->cmd )
  {
    ...
    case CM_CALL_CMD_ANSWER:
      ...
      ss_info_sub_prop_ptr = cmss_get_info_sub_prop(call_ptr->mm_id.asubs_id);
      if(ss_info_sub_prop_ptr == NULL || !cmutil_is_as_id_valid(call_ptr->mm_id.asubs_id) ||
          ss_info_sub_prop_ptr->stack_info[MM_STACK_0].gen_info == NULL)
      {
      ...
      */
      ...
      if( ss_info_all_stack_ptr->sys_mode == SYS_SYS_MODE_NO_SRV )
      {
    ...
    case CM_CALL_CMD_SUPS:
      ...
      switch( call_ptr->call_state )
      {
        ...
        case  CM_CALL_STATE_INCOM:
          ...
          */
          if( !cm_is_sys_mode_digital( ss_info_all_stack_ptr->sys_mode ))
          {
  ...
  */
  ...
  switch( call_cmd_ptr->cmd )
  {
    ...
    case CM_CALL_CMD_ANSWER:
      ...
      if(call_ptr == NULL)
      {
        CM_MSG_HIGH_1("call_ptr is NULL, cmd_info_ptr->call_id = %d", cmd_info_ptr->call_id);
        break;
      }
    ...
    case CM_CALL_CMD_END:
...
#endif /* FEATURE_OTASP_HOLD_CMD_IN_COMMIT */
        {
          ...
          */
              ...
              (call_ptr->sr.orig_sent_on_mode == SYS_SYS_MODE_NONE))
          {
            ...
            if (cm_is_esr_allowed_in_srlte_per_subs(call_ptr->mm_id.asubs_id)
                && cmph_is_oprting_in_1xsrlte_mode(call_ptr->mm_id.asubs_id)
                && ((call_ptr->call_type == CM_CALL_TYPE_VOICE) ||
                   (call_ptr->call_type == CM_CALL_TYPE_EMERGENCY))
                )
            {
          ...
          (call_ptr!= NULL) && (call_ptr->call_type == CM_CALL_TYPE_SMS) )
      {
        ...
        sys_modem_as_id_e_type other_sub1, other_sub2;
        if(call_ptr->mm_id.asubs_id == SYS_MODEM_AS_ID_1)
        {
          other_sub1 = SYS_MODEM_AS_ID_2;
          other_sub2 = SYS_MODEM_AS_ID_3;
        }
        else if(call_ptr->mm_id.asubs_id == SYS_MODEM_AS_ID_2)
        {
          other_sub1 = SYS_MODEM_AS_ID_1;
          other_sub2 = SYS_MODEM_AS_ID_3;
        }
        // if there is a voice call pending on SUB-2  or SUB-3 then unforce the preferences for SMS call now
        if(((cmcall_is_there_a_call_type_no_call_id_per_sub(other_sub1,
             CM_CALL_TYPE_VOICE, CM_CALL_ID_INVALID)!= CM_CALL_ID_INVALID)) ||\
            cmcall_is_there_a_call_type_no_call_id_per_sub(other_sub2,
                CM_CALL_TYPE_VOICE, CM_CALL_ID_INVALID)!= CM_CALL_ID_INVALID )
#endif
        {
          q_ptr = cmtask_orig_para_search_act_id(call_ptr->mm_id, (uint32)call_ptr->call_id);

          if (q_ptr != NULL)
          {
            /* Remove the objects with priority end and above PH obj */
            if (cmtask_orig_para_delete(call_ptr->mm_id, q_ptr, TRUE) )
            {
              cmph_force_orig_mode_on_the_stack(call_ptr->mm_id);
      ...
      */
      {
        ...
        if(ph_sub_ptr)
        {
          ph_sub_ptr->sub_3gpp2_pref.privacy_pref = cmd_mode_info_ptr->privacy_pref;
        }
      }
...
)
{
  ...
  sys_sys_mode_e_type sys_mode = cmss_get_info_sys_mode(call_ptr->mm_id.asubs_id, MM_STACK_0);
  ...
  */
  ...
  {
    db_item.conv = TRUE;
    mcc_db_put( DB_CONV, &db_item, call_ptr->mm_id.asubs_id );
  }
#if (defined(FEATURE_CDMA_800) || defined(FEATURE_CDMA_1900))
  if(!cmutil_is_as_id_valid(call_ptr->mm_id.asubs_id))
  {
    return;
  }
  ...
  */
  ...
  cmcall_update_gsdi_and_log_call_state(call_ptr->mm_id.asubs_id);
...
FUNCTION cmxcall_get_out_of_order_msg_ptr

DESCRIPTION
  Get the pointer to buffer of messages received out of order from MC.
  Allocate the memory in case this is the first message received from MC.
...
===========================================================================*/
EXTERN cmxcall_out_of_order_msg_s_type *cmxcall_get_out_of_order_msg_ptr(

  sys_modem_as_id_e_type           asubs_id
)
{
  /* If this is not a valid AS_ID - ERR_FATAL. This will ensure we dont have
  ** validate the as_id/ooo_msg_ptr elsewhere in the file.
  */
  if (!cmutil_is_as_id_valid(asubs_id))
  {
    CM_ERR_FATAL_1("OOO message pointer requested for invalid sub %d",asubs_id);
  }

  if ( ooo_msg_ptr[asubs_id] == NULL )
  {
    ooo_msg_ptr[asubs_id] = (cmxcall_out_of_order_msg_s_type *) cm_mem_malloc(sizeof(cmxcall_out_of_order_msg_s_type));
    CM_MSG_MED_2("Allocating OOO msg object for sub %d at location 0x%x",asubs_id,ooo_msg_ptr[asubs_id]);
  }
  return ooo_msg_ptr[asubs_id];
}

/*===========================================================================

FUNCTION cmxcall_free_out_of_order_msg_ptr

DESCRIPTION
  Free the buffer of messages received out of order from MC.
...
===========================================================================*/
EXTERN void                             cmxcall_free_out_of_order_msg_ptr(
...
)
{
  ...
  */
  if ( !cmutil_is_as_id_valid(asubs_id) )
  {
    CM_MSG_HIGH_1("OOO message pointer free for invalid sub %d",asubs_id);
  }

  if ( ooo_msg_ptr[asubs_id] != NULL )
  {
    cm_mem_free(ooo_msg_ptr[asubs_id]);
    ooo_msg_ptr[asubs_id] = NULL;
  }
...
)
{
  ...
  cmxcall_out_of_order_msg_s_type *saved_data = NULL;
  /* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */

  CM_ASSERT(cmutil_is_as_id_valid(asubs_id));

  /* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */

  saved_data = cmxcall_get_out_of_order_msg_ptr(asubs_id);
  ...
  cmxcall_free_out_of_order_msg_ptr(asubs_id);
}
...
)
{
  ...
  */
  ...
  if(ph_sub_ptr != NULL)
  {
    call_info_ptr->mode_info.info.cdma_call.is_privacy = ph_sub_ptr->sub_3gpp2_pref.is_privacy;
  }
  else
  {
    call_info_ptr->mode_info.info.cdma_call.is_privacy = FALSE;
  }
...
#if defined (FEATURE_LTE_TO_1X)
  call_info_ptr->mode_info.info.cdma_call.cc_reject = call_ptr->cmcall_mode_info.info.cdma_info.cc_reject;
  CM_MSG_HIGH_3("cmxcall_info_get: cc_reject present=%d rej_type=%d rej_val=%d",
                call_info_ptr->mode_info.info.cdma_call.cc_reject.present,
                call_info_ptr->mode_info.info.cdma_call.cc_reject.rej_type,
                call_info_ptr->mode_info.info.cdma_call.cc_reject.rej_value);
#endif
...
)
{
  ...
  if ( cmph_ptr()->device_prop.is_tau_needed == TRUE
       && cmph_is_oprting_in_1xsrlte_mode(asubs_id))
  {
    cm_call_id_type cdma_call_id = cmcall_is_there_a_cdma_call_except(call_id);

    CM_MSG_HIGH_2("CM->NAS: sub %d, Send TAU: cdma_call_id %d", asubs_id, cdma_call_id);
    if (cdma_call_id == CM_CALL_ID_INVALID)
    {
      /* send TAU here , since we will not wait for any response from 1X
         */
      cmcsfbcall_send_mm_dual_rx_1xcsfb_tau_req(asubs_id);
      cmph_ptr()->device_prop.is_tau_needed = FALSE;
    }
  }
#endif /* FEATURE_MMODE_SC_SVLTE  */
}
...
)
{
  ...
  CM_MSG_HIGH_6 ("CALL_END: CM->1X: state=%d status=%d is_tau_needed %d CallType=%d SrvType=%d, call_id %d",
                 call_ptr->call_state,
                 call_ptr->end_status,
                 cmph_ptr()->device_prop.is_tau_needed,
                 call_ptr->call_type,
                 call_ptr->srv_type,
                 call_ptr->call_id);
  ...
  */
    ...
    */
        ...
        ps_call_rel_on_1x_in_conv_state )
    {
      ...
      */
      ...
      cmmsimc_proc_disable_hybr_hdr(mm_id.asubs_id);
    }
  }
#else
  CM_MSG_HIGH_0("Reselect - restore from cmxcall_end()");
  cmmsimc_proc_enable_hybr_hdr(mm_id.asubs_id, TRUE);
  ...
  if(call_ptr->call_state == CM_CALL_STATE_ORIG)
  {
    cmlog_call_orig_end(call_ptr->end_status, call_ptr->mm_id.asubs_id);
    call_ptr->call_state = CM_CALL_STATE_IDLE;
  ...
  */
  ...
  if(call_ptr->call_type == CM_CALL_TYPE_VOICE
      || call_ptr->call_type == CM_CALL_TYPE_EMERGENCY)
  {
    cmxcall_srlte_send_tau(call_ptr->call_id, mm_id.asubs_id);
  }
  ...
  */
  ...
  {
    cmxcall_unsuspend_hybrid_operation(mm_id.asubs_id);
  }
#endif //FEATURE_MMODE_SC_SVLTE
  ...
  {
    cmph_update_ddtm_status(mm_id.asubs_id);                   /* update DDTM status */
  }

  /* Inform call has ended, should be called after call object is
  ** deallocated.
  */
  cmmsimc_proc_call_end(mm_id.asubs_id);
...
)
{
  ...
  sys_modem_as_id_e_type   subId = call_ptr->mm_id.asubs_id ;

  uint8                    stk_id = MM_STACK_0;
  cmph_sub_info_s_type *ph_sub_ptr = cmph_get_sub_ptr(subId);
  ...
  if (!cmutil_is_mm_id_valid(call_ptr->mm_id) || ph_sub_ptr == NULL)
  {
  ...
  */
      ...
      (call_ptr->call_type == CM_CALL_TYPE_CS_DATA))
  {
    ...
    voice_call_id     = cmcall_is_there_a_call_type_no_call_id_per_device(CM_CALL_TYPE_VOICE,
                        call_ptr->call_id);

    emergency_call_id = cmcall_is_there_a_call_type_no_call_id_per_device(CM_CALL_TYPE_EMERGENCY,
                        call_ptr->call_id);

#ifdef FEATURE_JCDMA
    pd_call_id = cmcall_is_there_a_call_type_no_call_id_per_device(CM_CALL_TYPE_PD,
                 call_ptr->call_id);
#endif

    otapa_call_id = cmcall_is_there_a_call_type_no_call_id_per_device(CM_CALL_TYPE_OTAPA,
                    call_ptr->call_id);

    otasp_call_id = cmcall_is_there_a_call_type_no_call_id_per_device(CM_CALL_TYPE_STD_OTASP,
                    call_ptr->call_id);

    svd_supported     = cmss_is_cdma_svd_supported(call_ptr->mm_id.asubs_id);
    ...
    */
    CM_MSG_HIGH_2_EXT("otapa_call_id=%d, otasp_call_id=%d",
                      otapa_call_id, otasp_call_id, (call_ptr->mm_id.asubs_id+1));
    ...
    */
    if (ph_sub_ptr->sub_3gpp2_pref.mt_1x_data_await_stage != CM_AWAITING_NONE ||
        ph_sub_ptr->ss_susp_info[MM_STACK_2].is_suspend == TRUE ||
        ph_sub_ptr->ss_susp_info[MM_STACK_1].is_suspend == TRUE)

    {
      CM_MSG_HIGH_3("SO:Cannot orig DATA call while in susp state %d %d %d",
                    ph_sub_ptr->sub_3gpp2_pref.mt_1x_data_await_stage,
                    ph_sub_ptr->ss_susp_info[MM_STACK_2].is_suspend,
                    ph_sub_ptr->ss_susp_info[MM_STACK_1].is_suspend);
      return CM_CALL_CMD_ERR_IN_USE_S;
    }
#endif //FEATURE_MMODE_SC_SVLTE
        ...
        */
        if(call_ptr->mm_id.stk_id == MM_STACK_1)
        {
          if( voice_call_ptr->mm_id.stk_id == MM_STACK_0 &&
              voice_call_ptr->mm_id.asubs_id == call_ptr->mm_id.asubs_id &&
              !voice_call_ptr->is_volte
        ...
        else if(call_ptr->mm_id.stk_id == MM_STACK_0)
        {
          ...
          */
          ...
          if( voice_call_ptr->mm_id.asubs_id != call_ptr->mm_id.asubs_id)
          {
          ...
          else if ( voice_call_ptr->mm_id.stk_id  == MM_STACK_0 &&
                    voice_call_ptr->mm_id.asubs_id  == call_ptr->mm_id.asubs_id &&
                    voice_call_ptr->direction == CM_CALL_DIRECTION_MO  &&
                    cmcall_is_ipcall_over_mode_poss (SYS_SYS_MODE_HDR, voice_call_ptr)  &&
                    call_ptr->call_type       == CM_CALL_TYPE_PS_DATA
                  )
          {
            is_call_allowed = TRUE;
          ...
          else if(voice_call_ptr->mm_id.stk_id == MM_STACK_1)
          {
    ...
    if(emergency_call_id != CM_CALL_ID_INVALID)
    {
      ...
      /*
      ** If ss = MM_STACK_1, the emergency call is being attempted over hybrid HDR,
      ...
      */
      if( (e_call_ptr->mm_id.stk_id == MM_STACK_0 || e_call_ptr->mm_id.stk_id == MM_STACK_ALL)
  ...
  */
  ...
  if(cmcall_msim_is_cross_sub_concurrency_forbidden(call_ptr,
     (call_ptr->call_type == CM_CALL_TYPE_PS_DATA)))
  #else
  if(!cmcall_is_cross_sub_concurrency_allowed(call_ptr->call_type, call_ptr->mm_id.asubs_id,
     (call_ptr->call_type == CM_CALL_TYPE_PS_DATA)))
  #endif
  {
    return CM_CALL_CMD_ERR_IN_USE_S;
...

  {
    ...
    */
    if( call_ptr->call_type == CM_CALL_TYPE_STD_OTASP &&
        prl_hdr_nv_get_ptr(call_ptr->mm_id.asubs_id)->is_valid &&
        !prl_hdr_nv_get_ptr(call_ptr->mm_id.asubs_id)->is_disabled)
    {
#if defined( FEATURE_UIM_RUN_TIME_ENABLE )
      boolean use_ruim_nam = FALSE;
#endif
      ...
      nam_ext.context = (uint16)call_ptr->mm_id.asubs_id;
      if (nv_rtre_use_ruim_for_nam_ext(nam_ext))
      {
        use_ruim_nam = TRUE;
      }
#endif
      if (cmnv_get_uim_spc_status(call_ptr->mm_id.asubs_id)
#if defined( FEATURE_UIM_RUN_TIME_ENABLE )
          && use_ruim_nam
#endif
         )
      ...
      if (!ph_sub_ptr->sub_3gpp2_pref.is_spc_zero
#if defined( FEATURE_UIM_RUN_TIME_ENABLE )
          && !use_ruim_nam
#endif
         )
    ...
    */
    if (CM_CALL_TYPE_IS_OTASP(call_ptr->call_type))
    {
      if ( cmcall_is_there_a_call_type_no_call_id_per_device(CM_CALL_TYPE_OTAPA, call_ptr->call_id) != CM_CALL_ID_INVALID )
      {
    ...
    */
        ...
        call_ptr->favored_hybr_pref == CM_HYBR_PREF_CDMA__HDR)
    {
      ...
      pref mode_pref set to CM_MODE_PREF_HDR_ONLY and
      pref hybr_pref set to CM_HYBR_PREF_ON (user is allowed to
      ...
      - pref hybr_pref will be CM_HYBR_PREF_ON
      ...
      Therefore we need to call cmss_is_stack1_operational()
      ...
      call is not allowed, then we should flag an error.
        */
      if(cmss_is_stack1_operational(call_ptr->mm_id.asubs_id) &&
          !cmmsc_auto_is_hybr_hdr_call_allowed(call_ptr->mm_id.asubs_id))
      {
    ...
    */
    ...
    // Reject the PS call on 1x if UE is in SRLTE mode and IRAT from DO2LTE is in progress
    if (call_ptr->call_type == CM_CALL_TYPE_PS_DATA           && 
        call_ptr->cmcall_mode_info.info_type == CM_CALL_MODE_INFO_CDMA &&
        cmcall_is_1x_data_srv_opt(call_ptr->cmcall_mode_info.info.cdma_info.srv_opt) && 
        call_ptr->mm_id.stk_id == MM_STACK_0                  &&
        (cmregprx_get_substate(cmph_map_sub_stk_to_id(call_ptr->mm_id.asubs_id, 
          cmph_is_oprting_in_1xsrlte_mode(call_ptr->mm_id.asubs_id)?MM_STACK_2:MM_STACK_0)) == CMREGPRX_SUBSTATE_WAIT_SRV_IND))
    {
  ...
  */
  ...
  if ( !cmss_is_cdma_svd_supported(call_ptr->mm_id.asubs_id))
  {
    if (cmxcall_is_pc_otapa() &&
        !cmxcall_is_conv_pc_otapa_commit()
        &&(!(cmph_ptr()->device_prop.trm_cap.is_svdo_supported
             && (call_ptr->call_type == CM_CALL_TYPE_PS_DATA)))
       )
    {
      end_call_id = cmcall_is_there_a_call_type_no_call_id_per_device (CM_CALL_TYPE_OTAPA,
                    CM_CALL_ID_INVALID);
    ...
    if (voice_call_id != CM_CALL_ID_INVALID)
    {
      ...
      if(voice_call_ptr != NULL)
      {
        ...
        ** CALL_END_EVENT */
        ...
        {
          CM_MSG_HIGH_4( "cmxcall_client_orig_proc(): stack %d, direction %d, call_mode_info %d orig_sent_on_mode %d",
                         voice_call_ptr->mm_id.stk_id, voice_call_ptr->direction,
                         voice_call_ptr->cmcall_mode_info.info_type,
  ...
  */
  if ( ( call_ptr->call_type == CM_CALL_TYPE_VOICE ||
         call_ptr->call_type == CM_CALL_TYPE_SMS )
       && !cmss_is_cdma_svd_supported(call_ptr->mm_id.asubs_id) )
  {
    ...
    */
    while(end_call_ptr != NULL)
    {
      if ( ( end_call_ptr->call_type == CM_CALL_TYPE_CS_DATA   ||
           ...
           end_call_ptr->cmcall_mode_info.info_type == CM_CALL_MODE_INFO_CDMA
           && (end_call_ptr->mm_id.stk_id == MM_STACK_0||
               (end_call_ptr->mm_id.stk_id == MM_STACK_1 && !cmxcall_is_svdo_capable(call_ptr)) )
         )
      {
        if(((call_ptr->call_type == CM_CALL_TYPE_VOICE &&
             !cmcall_is_active_ip_app(SYS_SYS_MODE_HDR, CM_CALL_TYPE_VOICE, call_ptr->mm_id.asubs_id))
            ||
            (call_ptr->call_type == CM_CALL_TYPE_SMS &&
             !cmcall_is_active_ip_app(SYS_SYS_MODE_HDR, CM_CALL_TYPE_SMS, call_ptr->mm_id.asubs_id))
           )
        ...
        if( end_call_ptr->call_state  == CM_CALL_STATE_ORIG
            && cmss_is_stack1_operational(end_call_ptr->mm_id.asubs_id)
            && end_call_ptr->mm_id.stk_id == MM_STACK_0)
        {
          end_call = TRUE;
        }
  ...
  */
  if (call_ptr->call_type == CM_CALL_TYPE_EMERGENCY)
  {
    ...
    if ( !cmss_is_cdma_svd_supported(call_ptr->mm_id.asubs_id) &&
         cmcall_is_no_call_in_gw_cs_per_subs(call_ptr->mm_id.asubs_id) &&
         cmcall_is_no_call_in_gw_ps_per_subs(call_ptr->mm_id.asubs_id)
#ifdef FEATURE_CM_LTE
         && cmcall_is_no_call_in_lte_ps ()
#endif
       )
    {
      end_call_id = cmcall_is_there_a_call_type_no_call_id_per_device (CM_CALL_TYPE_CS_DATA,
                    CM_CALL_ID_INVALID);
      ...
      end_call_id = cmcall_is_there_a_call_type_no_call_id_per_device (CM_CALL_TYPE_PS_DATA,
                    CM_CALL_ID_INVALID);
  ...
  */
  ...
  if ( !cmss_is_cdma_svd_supported(call_ptr->mm_id.asubs_id))
  {
    end_call_id = cmcall_is_there_a_call_type_no_call_id_per_device(CM_CALL_TYPE_PD,
                  CM_CALL_ID_INVALID);
  ...
  */
  if ( !cmss_is_cdma_svd_supported(call_ptr->mm_id.asubs_id))
  {
    if( call_ptr->call_type != CM_CALL_TYPE_PS_DATA )
    {
      end_call_id = cmcall_is_there_a_call_type_no_call_id_per_device(CM_CALL_TYPE_SMS,
                    CM_CALL_ID_INVALID);
      ...
      (call_ptr->call_type != CM_CALL_TYPE_CS_DATA))
  {
    ...
    cmxsms_abort_mo_msg(call_ptr->mm_id.asubs_id);
#endif  /* FEATURE_CDSMS */
  }
  ...
  */
  if( (!( call_ptr->call_type == CM_CALL_TYPE_PS_DATA
          && call_ptr->mm_id.stk_id == MM_STACK_1))
      &&
      (!( call_ptr->call_type == CM_CALL_TYPE_PS_DATA
          && (!cmss_is_stack1_operational(call_ptr->mm_id.asubs_id)) ))
      &&
      (!((call_ptr->call_type == CM_CALL_TYPE_VOICE)
         && cmcall_is_ipcall_over_mode_poss (SYS_SYS_MODE_HDR, call_ptr)))
      &&
      ((call_ptr->mm_id.stk_id == MM_STACK_0 || call_ptr->mm_id.stk_id == MM_STACK_ALL)
       &&(!cmxcall_is_svdo_capable(call_ptr)))
    )
  {
    cmss_misc_sub_prop_s_type *ss_misc_sub_ptr = cmss_get_misc_sub_prop_ptr(call_ptr->mm_id.asubs_id);
    end_call_id = cmcall_is_hdr_call();

#ifdef FEATURE_DEDICATED_C2K_MODE
    if( ss_misc_sub_ptr != NULL &&
        ss_misc_sub_ptr->sub_info.gpp2_sub_info.is_hdr_session_open )
    {
    ...
    else if((end_call_ptr = cmcall_is_hybr_hdr_orig(call_ptr->mm_id.asubs_id)) != NULL )
    {
...
)
{
  ...
  */
  ...
  while(ipcall_ptr != NULL  )
  {
    ...
    if(ipcall_ptr->mm_id.asubs_id== mm_id.asubs_id)
    {
...
)
{
  ...
  ph_cmd_ptr = PH_CMD_PTR( ph_ptr->device_prop.reply.cmd_ptr );
  ...
  /* Override response if we are waiting for LPM command to complete
     ...
     4. Deadlocks: CM waits for MMOC, MMOC waits for 1X, 1X waits for CM.
  */
  if( ph_ptr->device_prop.reply.cmd_ptr != NULL )
  {
    if( ph_ptr->device_prop.reply.cmd_ptr->cmd_type == CM_CMD_TYPE_PH )
    {
...
)
{
  ...
  mm_sub_stk_id_s_type mm_id = cmph_map_sub_stk_to_id(asubs_id, MM_STACK_2);
  cmss_misc_sub_prop_s_type *ss_misc_sub_ptr = cmss_get_misc_sub_prop_ptr(asubs_id);
  cmss_misc_gen_stack_info_s_type *ss_misc_stk2_ptr;
  sd_si_info_s_type *ss_true_stk_info_ptr = cmss_true_stack_info_ptr(mm_id);

  if(!cmutil_is_as_id_valid(asubs_id) || ss_misc_sub_ptr == NULL)
  {
    return;
  }
  ss_misc_stk2_ptr = ss_misc_sub_ptr->stack_info[mm_id.stk_id].gen_info;
  if(ss_misc_stk2_ptr == NULL)
  {
    return;
  }
  ...
  CM_MSG_HIGH_5("Hybr_2_No_Srv_Uptime: call_type %d hybr_gw_opr %d clk time %d hybr_gw_no_srv_uptime %d HYBR_2 srv %d",
                ...
                clk_time,
                ss_misc_stk2_ptr->no_srv_uptime,
                cmss_is_3gpp_in_srv(SYS_SYS_MODE_LTE, FALSE));

  if( (ss_true_stk_info_ptr->srv_status == SYS_SRV_STATUS_NO_SRV_INTERNAL &&
       cmss_get_info_sys_mode(asubs_id, mm_id.stk_id) == SYS_SYS_MODE_LTE ) ||
      (ss_true_stk_info_ptr->srv_status == SYS_SRV_STATUS_NO_SRV &&
       !cmss_is_3gpp_in_srv( SYS_SYS_MODE_LTE, FALSE ) )
    )
  {
    is_no_srv = TRUE;
  ...
  */
  if( cmxcall_is_call_type_voice( call_ptr->call_type ) &&
      ss_misc_stk2_ptr->no_srv_uptime != CMSS_NO_SRV_UPTIME_NONE &&
      ss_misc_stk2_ptr->no_srv_uptime > clk_time &&
      cmss_is_stack2_operational(asubs_id) &&
      is_no_srv
    )
  {
...
===========================================================================*/
static cm_call_id_type cmxcall_is_there_a_ongoing_1x_voice_call (
  sys_modem_as_id_e_type   asubs_id
)
{
  cmcall_s_type    *call_ptr;                /* Call object pointer */
  ...
  uint8                    stk_id = MM_STACK_0;
  sys_sys_mode_e_type      sys_mode = cmss_get_info_sys_mode(asubs_id, stk_id);
  ...
  {
#ifdef FEATURE_HDR_HYBRID
    CM_MSG_HIGH_6_EXT( "Check_1x_Voice_Call: sub %d, sys_mode %d, call_id %d, call_type %d Call State %d, Wait for HDR Call End %d",
                       asubs_id, sys_mode, call_ptr->call_id,
                       call_ptr->call_type, call_ptr->call_state, call_ptr->waiting_for_hdr_call_end, (asubs_id+1));
#endif
...
FUNCTION cmxcall_1x_orig_fail_ind
...
)
{
  if (call_ptr->call_type == CM_CALL_TYPE_SMS &&
      ...
      call_ptr->end_status != CM_CALL_END_NONE)
  {
    cmcall_event(call_ptr,CM_CALL_EVENT_1x_ORIG_FAIL);
...
)
{
  ...
  cm_mc_send_cmd(cmmsc_get_1x_sub(), mc_ptr );
...
)
{
     ...
     )
  {
    ...
    {
      CM_MSG_MED_2 ("sub %d, %d: Invalid digit count!", asubs_id, stn_sr_num.reg_cnt  );
    }
    else if(stn_sr_num.e911_cnt > CM_NV_MAX_STN_SR_DIGITS && orig_param->is_emergency )
    {
      CM_MSG_MED_2 ("sub %d, %d: Invalid digit count!", asubs_id, stn_sr_num.e911_cnt );
    }
    ...
    if(!orig_param->is_emergency )
    {
      ...
      for (dig_len = 0; dig_len < stn_sr_num.reg_cnt; dig_len++)
      {
             ...
             (stn_sr_num.regular_dialed_digits[dig_len] != '+'))
        {
          ...
          CM_MSG_MED_1 ("sub %d, Invalid digit set for normal STN_SR", asubs_id);
        }
      }
    ...
    else
    {
      ...
      for (dig_len = 0; dig_len < stn_sr_num.e911_cnt; dig_len++)
      {
             ...
             (stn_sr_num.e911_dialed_digits[dig_len] != '+'))
        {
          ...
          CM_MSG_MED_1 ("sub %d, %d: Invalid digit set for e911 STN_SR", asubs_id);
        }
      }
  ...
  {
    CM_MSG_HIGH_1 ("sub %d, STN_SR number not set for e911/normal call", asubs_id);
  }

  CM_MSG_HIGH_4 ("sub %d, STN_SR provisioned %s, reg_len= %d, e911_len = %d",
                 asubs_id, orig_param->called_address,
...
)
{
  ...
  */
  if(!is_drvcc_call)
  {
    cmxcall_set_srvcc_called_party( orig_param, call_ptr->mm_id.asubs_id);
  }
...
FUNCTION cmxcall_update_call_debug_info_in_stop_alert

DESCRIPTION
  Process MC report STOP_ALERT_F and updates call debug info regarding
  whether it is an abnormal or normal call end

  1. map CALL_RELEASE_F with reason NORMAL to normal call end
  2. leave CALL_RELEASE_F with other cause codes (SPC_ERR, SO_REJ) as is
      ( it would be considered an abnormal call failure)
  3. map CALL_FADE_F to abnormal call end (RLF or weak signal)
...
===========================================================================*/
static void cmxcall_update_call_debug_info_in_stop_alert(

  cm_mc_rpt_type                      *cm_mc_rpt_ptr,
  /* rpt from 1x - stop_alert_f */

  cmcall_s_type                       *call_ptr
  /* call ptr to MT call on which rpt is being sent */
...
)
{
  ...
  if (call_ptr      == NULL ||
      cm_mc_rpt_ptr == NULL ||
      cm_mc_rpt_ptr->hdr.cmd != CM_STOP_ALERT_F)
  {
    return;
  }
  ...
  switch (cm_mc_rpt_ptr->stop.rpt_type)
  {
    case CM_CALL_RELEASE_F:
      if (cm_mc_rpt_ptr->stop.error_code.rel_error_code == CM_CALL_REL_NORMAL)
      {
        call_ptr->call_debug_info |= BM(CMCALL_DEBUG_INFO_NORMAL_CALL_END);
      }
      CM_MSG_HIGH_2("release code %d,call_ptr->call_debug_info = %d",
                    cm_mc_rpt_ptr->stop.error_code.rel_error_code, call_ptr->call_debug_info);
      break;
    case CM_CALL_FADE_F:
      call_ptr->call_debug_info |= BM(CMCALL_DEBUG_INFO_ABNORMAL_FAILURE);
      CM_MSG_HIGH_2("fade code %d, call_ptr->call_debug_info = %d",
                    cm_mc_rpt_ptr->stop.error_code.fade_error_code, call_ptr->call_debug_info);
      break;
    default:
      break;
  }
...
)
{
  ...
  */
...
#endif
  sys_modem_as_id_e_type ll_as_id = cm_mc_rpt_ptr->hdr.asubs_id;
  cm_mm_ss_info_sub_prop_s_type *ss_info_sub_prop_ptr;
  cm_mm_ss_info_gen_stack_info_s_type *ss_info_all_stack_0_ptr, *ss_info_all_stack_1_ptr, *ss_info_all_stack_2_ptr;
  cmph_sub_info_s_type *ph_sub_ptr = cmph_get_sub_ptr(ll_as_id);
  ...
  CM_ASSERT( pd_dmod_ptr != NULL );
#endif
  ss_info_sub_prop_ptr = cmss_get_info_sub_prop(ll_as_id);
  if(ss_info_sub_prop_ptr == NULL || ph_sub_ptr == NULL ||
      ss_info_sub_prop_ptr->stack_info[MM_STACK_0].gen_info == NULL)
  {
    return;
  ...
  if(ll_as_id != cmmsc_get_1x_sub() &&  cm_mc_rpt_ptr->hdr.cmd != CM_CALL_RELEASE_F && cm_mc_rpt_ptr->hdr.cmd != CM_HDR_CALL_RELEASE_F &&
    cm_mc_rpt_ptr->hdr.cmd != CM_CALL_END_CNF_F && cm_mc_rpt_ptr->hdr.cmd != CM_HDR_CALL_END_CNF_F)
  {
    CM_MSG_HIGH_2("1x->CM: Drop rpt since CDMA report received on wrong sub:%d, sub_capability:%d",
                  ll_as_id, cmmsc_get_curr_msc_max_cap(ll_as_id));
    return;
  }
  ...
  */
  switch( cm_mc_rpt_ptr->hdr.cmd )
  {
    ...
    case CM_SIGNAL_F:
      ...
      */
      ...
      if (call_ptr->call_state == CM_CALL_STATE_IDLE)
      {
        ...
        cmxcall_out_of_order_msg_s_type *out_of_order_msgs =
          cmxcall_get_out_of_order_msg_ptr(ll_as_id);
        /* Set the flags */
      ...
      else
      {
        ...
        call_ptr->msg_type        = cm_mc_rpt_ptr->info_hdr.msg_type;
    ...
    case CM_HDR_INCOMING_CALL_F:
      ...
      ph_sub_ptr->sub_3gpp2_pref.is_1x_mt_call_pending = FALSE;
      #ifdef FEATURE_MMODE_SC_SVLTE
      if(cmcall_is_1x_data_srv_opt(cm_mc_rpt_ptr->incoming_call.so))
      {
        CM_MSG_HIGH_1("mt_1x_data_wait_stage moved to %d as incoming call received",ph_sub_ptr->sub_3gpp2_pref.mt_1x_data_await_stage);
        ph_sub_ptr->sub_3gpp2_pref.mt_1x_data_await_stage = CM_AWAITING_NONE;
      }
      #endif
      ...
      */
      call_ptr->mm_id.stk_id = MM_STACK_0;
      call_ptr->mm_id.asubs_id = cm_mc_rpt_ptr->hdr.asubs_id;
...
#ifdef FEATURE_MMODE_DUAL_SIM
      if (cmph_is_msim() &&
          cmutil_is_as_id_valid(cm_mc_rpt_ptr->hdr.asubs_id) &&
          (cm_mc_rpt_ptr->hdr.cmd == CM_INCOMING_CALL_F) &&
          cmxcall_is_voice_srv_opt(cm_mc_rpt_ptr->incoming_call.so) &&
          cm_mc_rpt_ptr->incoming_call.vfr != cm_vfr_map[cm_mc_rpt_ptr->hdr.asubs_id])
      {
        CM_MSG_HIGH_2("VFR mismatch in MMODE %d and 1x %d",
                      cm_vfr_map[cm_mc_rpt_ptr->hdr.asubs_id],
                      cm_mc_rpt_ptr->incoming_call.vfr);

        cmcall_send_mc_end_cmd(cm_mc_rpt_ptr->hdr.asubs_id, cm_mc_rpt_ptr->incoming_call.call_id, CAI_REL_NORMAL);
      ...
      */
         ...
         )
      {
        ...
        cmph_sub_info_s_type *ph_sub_ptr = cmph_get_sub_ptr(call_ptr->mm_id.asubs_id);
#endif
        data_call_id = cmcall_is_there_a_call_type_no_call_id_per_sub(call_ptr->mm_id.asubs_id,
        ...
        */
             ...
             ph_sub_ptr != NULL)
        {
          ...
          */
          ...
          if (((ph_sub_ptr->ss_susp_info[MM_STACK_1].is_suspend == FALSE) &&
               (!(cmtask_is_stack_pending_for_deact(cmph_map_sub_stk_to_id(cm_mc_rpt_ptr->hdr.asubs_id, MM_STACK_1))))) ||
              ((ph_sub_ptr->ss_susp_info[MM_STACK_2].is_suspend == FALSE) &&
               (!(cmtask_is_stack_pending_for_deact(cmph_map_sub_stk_to_id(cm_mc_rpt_ptr->hdr.asubs_id, MM_STACK_2)))))
             )
          {
            CM_MSG_HIGH_0("SO: Hybr stack not susp. End incom call");
            /* Send an end command to MC to end the call */
            cmcall_send_mc_end_cmd(call_ptr->mm_id.asubs_id, call_ptr->call_id, CAI_REL_NORMAL);
            cm_call_id_deallocate (cm_mc_rpt_ptr->incoming_call.call_id);
            cmxcall_unsuspend_hybrid_operation(ll_as_id);
            call_ptr = NULL;
            break;
          }
          ...
          else /* no need to check svd_supported, as svd_supported will always be false */
          {
            ...
            cmph_sub_info_s_type *ph_sub_ptr = cmph_get_sub_ptr(end_call_ptr->mm_id.asubs_id);
            ...
            cmcall_send_mc_end_cmd(call_ptr->mm_id.asubs_id, end_call_ptr->call_id,CAI_REL_NORMAL);
            /*cmxcall_set_call_wait_for_1X_end_cnf(call_ptr);*/
          ...
          if (cmcall_is_1x_data_srv_opt(cm_mc_rpt_ptr->incoming_call.so))
          {
               ...
               )
            {
              ...
              */
              ...
              cmcall_send_mc_end_cmd(call_ptr->mm_id.asubs_id, cm_mc_rpt_ptr->incoming_call.call_id,CAI_REL_NORMAL);
      ...
      */
           ...
           cmxcall_is_voice_srv_opt( cm_mc_rpt_ptr->incoming_call.so ) )
      {
        ...
        cm_call_id_type   ongoing_voice_call_id  = cmxcall_is_there_a_ongoing_1x_voice_call(ll_as_id);

        /* Read the NV item to check if the Call End Event is to be sent */

        if ( ongoing_voice_call_id != CM_CALL_ID_INVALID &&
             ph_sub_ptr->sub_3gpp2_pref.is_awim_mo_call_end_event )
        {
      ...
      /* If voice call converted to SMS call, then send end event first*/
      if ( cm_mc_rpt_ptr->hdr.cmd == CM_INCOMING_CALL_F &&
           is_sms_srv_opt( cm_mc_rpt_ptr->incoming_call.so ) )
      {
        cmcall_s_type    *ongoing_voice_call_ptr = NULL;
        cm_call_id_type   ongoing_voice_call_id  = cmxcall_is_there_a_ongoing_1x_voice_call(ll_as_id);
        ...
        if ( ongoing_voice_call_id != CM_CALL_ID_INVALID )
        {
          ...
          if ( ongoing_voice_call_ptr != NULL &&
              ongoing_voice_call_id == cm_mc_rpt_ptr->incoming_call.call_id)
          {
            /*Set the call end status to INCOM_CALL so AP can recognize
                    ** the reason for MO Call end
                    */
            ongoing_voice_call_ptr->end_status = CM_CALL_END_INCOM_CALL;
            CM_MSG_HIGH_1("MO voice & MT collision, sending end event for MO call_id:%d",ongoing_voice_call_id);
            cmcall_event( ongoing_voice_call_ptr, CM_CALL_EVENT_END );
      ...
      if(  cmph_is_msim() &&
           ...
           && (ph_ptr->device_prop.msim_prop.default_data_subs != cm_mc_rpt_ptr->hdr.asubs_id)
           && (ph_ptr->device_prop.msim_prop.active_subs & BM(ph_ptr->device_prop.msim_prop.default_data_subs))
      ...
      */
      ...
      #endif
      {
        CM_MSG_HIGH_0("End MT call due to call activity on the other sub");
        if(cm_mc_rpt_ptr->hdr.cmd == CM_INCOMING_CALL_F)
        {
        cmcall_send_mc_end_cmd(call_ptr->mm_id.asubs_id, cm_mc_rpt_ptr->incoming_call.call_id, CAI_REL_NORMAL);
        }        
#ifdef FEATURE_HDR_HYBRID
        else if (cm_mc_rpt_ptr->hdr.cmd == CM_HDR_INCOMING_CALL_F)
        {
            cmcall_send_hdrmc_end_cmd(call_ptr,CAI_REL_NORMAL);
        }
#endif
        cm_call_id_deallocate(cm_mc_rpt_ptr->incoming_call.call_id);
        break;
      ...
      */
      if( (( CMPH_SS_DISABLE_CALL_TYPE_MASK(call_ptr->mm_id.asubs_id, ph_ptr)& CM_CALL_TYPE_VOICE_MASK ) &&
          ...
          (( CMPH_SS_DISABLE_CALL_TYPE_MASK(call_ptr->mm_id.asubs_id, ph_ptr) & CM_CALL_TYPE_CS_DATA_MASK ) &&
           ( call_ptr->call_type == CM_CALL_TYPE_CS_DATA ))
          ||
          ( (cmss_is_stack2_operational(call_ptr->mm_id.asubs_id)
#if defined(FEATURE_LTE_TO_1X) && defined(FEATURE_MMODE_SC_SVLTE)
             || (cmsoa_state_sub_info_ptr(call_ptr->mm_id.asubs_id) != NULL &&
                 cmsoa_state_sub_info_ptr(call_ptr->mm_id.asubs_id)->info.curr_sv_oprt_mode == CMSOA_SV_OPRT_MODE_SVLTE)
#endif
            )&&
            (call_ptr->call_type == CM_CALL_TYPE_VOICE || call_ptr->call_type == CM_CALL_TYPE_EMERGENCY) &&
            (cmcall_is_there_volte_call_obj_per_sub(
               CM_CALL_TYPE_VOICE_MASK |
               CM_CALL_TYPE_EMERG_MASK |
               CM_CALL_TYPE_VT_MASK,
               call_ptr->mm_id.asubs_id) != CM_CALL_ID_INVALID))
        )
      {
#if !defined(FEATURE_MMODE_REMOVE_1X)
      ...
      */
...
#endif
      {
        ...
        */
        ...
        {
          cmph_reset_lte_do_redir(ll_as_id);
          if(cmss_get_misc_sub_prop_ptr(ll_as_id) != NULL &&
              cmss_get_misc_sub_prop_ptr(ll_as_id)->sub_info.gen_sub_info.main_buffer_srv_uptime != CMSS_INFORM_SRV_CHANGE_INTER_NONE)
          {
            cmss_update_cdma_srv_lte_do_redir(ll_as_id);
          }
        }
      ...
      */
      ...
      {
        if (ss_info_all_stack_0_ptr->sys_mode != SYS_SYS_MODE_NO_SRV)
        {
          call_ptr->sys_mode = ss_info_all_stack_0_ptr->sys_mode;
        }
      }
      ...
      */
      ...
      cmph_update_ddtm_status(ll_as_id);               /* update DDTM status */
      ...
      */
          ...
          cm_mc_rpt_ptr->hdr.cmd == CM_INCOMING_CALL_F )
      {
        ...
        thr_call_status.mode_info.cdma_info
          = ss_info_all_stack_0_ptr->mode_info.cdma_info;

        if( call_ptr->sys_mode == SYS_SYS_MODE_HDR &&
            ss_info_all_stack_1_ptr != NULL &&
            cmss_is_stack1_operational(ll_as_id)

          )
        {
          thr_call_status.sys_id = ss_info_all_stack_1_ptr->sys_id;
          thr_call_status.mode_info.cdma_info.band_class = ss_info_all_stack_1_ptr->active_band;
        }
        else
        {
          thr_call_status.sys_id = ss_info_all_stack_0_ptr->sys_id;
          thr_call_status.mode_info.cdma_info.band_class = ss_info_all_stack_0_ptr->active_band;
        }
        ...
        */
        ...
        (void) cmss_orig_thr_update_tbl_entry_with_call_status(
          cmss_ptr()->cmss_orig_thr_tbl_ptr,
          &thr_call_status,
          call_ptr->mm_id.asubs_id);
      ...
      */
      ...
      /* Terminate ongoing periodic search if this is a voice/SMS call
      */
      if(cmutil_is_periodic_manual_search() 
        ...
        (cmxcall_is_voice_srv_opt(cm_mc_rpt_ptr->incoming_call.so) ||
         is_sms_srv_opt(cm_mc_rpt_ptr->incoming_call.so)))
      {
        CM_MSG_HIGH_0("GET_NET: PERIODIC: TERMINATE: 1x Incoming call");
    ...
    {
      cm_call_type_e_type temp_call_type = CM_CALL_TYPE_NONE;

      #ifdef FEATURE_HA_MAINLINE_ONLY_ENABLE
      cmcall_s_type   *temp_call =(cmcall_s_type *) cm_mem_malloc(sizeof(cmcall_s_type));
      CM_ASSERT( temp_call != NULL );

      cmcall_reset(temp_call);
      temp_call->mm_id.asubs_id = ll_as_id;
      temp_call->direction = CM_CALL_DIRECTION_MT;
      temp_call->cmcall_mode_info.info_type = CM_CALL_MODE_INFO_CDMA;
      #endif

      if(cmxcall_is_voice_srv_opt(cm_mc_rpt_ptr->incoming_page.so))
      {
        temp_call_type = CM_CALL_TYPE_VOICE;
      }
      else if(is_cs_data_srv_opt(cm_mc_rpt_ptr->incoming_page.so))
      {
        temp_call_type = CM_CALL_TYPE_CS_DATA;
      }
      else if(is_pkt_data_srv_opt(cm_mc_rpt_ptr->incoming_page.so,
                                  CMXCALL_MOBILE_TERMINATED))
      {
        temp_call_type = CM_CALL_TYPE_PS_DATA;
      }

      #ifdef FEATURE_HA_MAINLINE_ONLY_ENABLE
        temp_call->call_type = temp_call_type;
      #endif

#ifdef FEATURE_MMODE_DUAL_SIM
      if (cmph_is_msim() &&
          cmutil_is_as_id_valid(cm_mc_rpt_ptr->hdr.asubs_id) &&
          cmxcall_is_voice_srv_opt(cm_mc_rpt_ptr->incoming_page.so) &&
          cm_mc_rpt_ptr->incoming_page.vfr != cm_vfr_map[cm_mc_rpt_ptr->hdr.asubs_id])
      {
        CM_MSG_HIGH_2("VFR mismatch in MMODE %d and 1x %d",
                      cm_vfr_map[cm_mc_rpt_ptr->hdr.asubs_id],
                      cm_mc_rpt_ptr->incoming_page.vfr);

        cmxcall_mc_incoming_page_proceed(cm_mc_rpt_ptr->hdr.asubs_id, FALSE);
      }
      else
#endif
      #ifdef FEATURE_HA_MAINLINE_ONLY_ENABLE
      if(cmcall_is_there_vowlan_call_sub(ll_as_id) != CM_CALL_ID_INVALID)
      #else   
      if(cmcall_is_there_vowlan_call() != CM_CALL_ID_INVALID)   
      #endif
        {
          ...
          if (cmxcall_is_voice_srv_opt(cm_mc_rpt_ptr->incoming_page.so))
          {
            ...
            cmxcall_mc_incoming_page_proceed(ll_as_id, FALSE);
          }
        }
#ifdef FEATURE_MMODE_SC_SVLTE
      ...
      /* If there is VoLTE call existing, 1X call should be rejected
        */
        else if (cmcall_is_there_volte_call_obj_per_sub(CM_CALL_TYPE_VOICE_MASK |
                 CM_CALL_TYPE_EMERG_MASK |
                 CM_CALL_TYPE_VT_MASK,
                 ll_as_id) != CM_CALL_ID_INVALID)
        {
      ...
      /* Reject call depending on call activity on the other sub
        */
      #ifdef FEATURE_HA_MAINLINE_ONLY_ENABLE
        else if(cmcall_msim_is_cross_sub_concurrency_forbidden(temp_call,
                (temp_call->call_type == CM_CALL_TYPE_PS_DATA)))
      #else
        else if(!cmcall_is_cross_sub_concurrency_allowed(temp_call_type, ll_as_id,
                (temp_call_type == CM_CALL_TYPE_PS_DATA)))
      #endif
        {
          CM_MSG_HIGH_0("Reject 1X MT page due to call activity on the other sub");
          cmxcall_mc_incoming_page_proceed(ll_as_id, FALSE);
        }

        else if( cmss_is_stack1_operational(ll_as_id) == FALSE &&
                 cmss_is_stack2_operational(ll_as_id) == FALSE)
        {
          /* If  SVDO & SVLTE is not supported, Immediately respond to 1xcp to
          ** proceed with call
          */
          CM_MSG_HIGH_1_ASID("SO:Page proceed, stk 1 and 2 not operational.",
                             ll_as_id);
          cmxcall_mc_incoming_page_proceed(ll_as_id, TRUE);
        }
        else if( cmcall_is_1x_data_srv_opt(cm_mc_rpt_ptr->incoming_page.so) )
        {
          ...
          */
          ...
          {
            ph_sub_ptr->sub_3gpp2_pref.mt_1x_data_await_stage= CM_AWAITING_DS_RSP;
            cmcall_event_2( NULL,
          ...
          */
          else if( cm_mc_rpt_ptr->incoming_page.so == CAI_SO_FULL_TDSO)
          {
            ...
            */
            ...
            ph_sub_ptr->sub_3gpp2_pref.mt_1x_data_await_stage = CM_AWAITING_SUSPEND_RSP;
            cmxcall_page_proceed(ll_as_id);
          }

          CM_MSG_HIGH_2("mt_1x_data_wait_stage %d, incoming page so:%d",
                            ph_sub_ptr->sub_3gpp2_pref.mt_1x_data_await_stage,cm_mc_rpt_ptr->incoming_page.so);
      ...
      */
        else
        {
          CM_MSG_HIGH_2("SO:Page proceed -so %d is_tau_needed %d",
                        cm_mc_rpt_ptr->incoming_page.so,cmph_ptr()->device_prop.is_tau_needed);
#if defined(FEATURE_LTE_TO_1X) && defined(FEATURE_MMODE_SC_SVLTE)
          /* Set the flag so that ue mode switch is not triggerred.
          */
          ph_sub_ptr->sub_3gpp2_pref.is_1x_mt_call_pending = TRUE;
          ...
          */
          if(cmph_is_oprting_in_1xsrlte_mode(ll_as_id)
              && cm_is_esr_allowed_in_srlte_per_subs(ll_as_id)
              && ss_info_all_stack_2_ptr != NULL
              && ss_info_all_stack_2_ptr->srv_status == SYS_SRV_STATUS_SRV
              && cmregprx_get_state(cmph_map_sub_stk_to_id(ll_as_id, MM_STACK_2))
              ...
              && cmph_ptr()->device_prop.is_tau_needed == FALSE)
          {
            ...
            */
            cmcsfbcall_send_orig_req_call_obj_not_allocated(CM_MT_UNALLOCATED_CALL_ID, ll_as_id);
            cmph_ptr()->device_prop.is_tau_needed = TRUE;
          }
          else
#endif
          {
      ...
      /* Terminate ongoing periodic search if this is a voice/SMS page
      */
      if(cmutil_is_periodic_manual_search() 
        &&
        (cmxcall_is_voice_srv_opt(cm_mc_rpt_ptr->incoming_page.so) ||
         is_sms_srv_opt(cm_mc_rpt_ptr->incoming_page.so)))
      {
        CM_MSG_HIGH_0("GET_NET: PERIODIC: TERMINATE: 1x Incoming page");
        cmph_terminate_periodic_get_networks_req_only(ll_as_id);
      }
      #ifdef FEATURE_HA_MAINLINE_ONLY_ENABLE
      cm_mem_free(temp_call);
      #endif
...
#if defined (FEATURE_LTE_TO_1X)
      cmcsfbcall_unforce_csfb_call_end_preferences(cmph_map_sub_stk_to_id(ll_as_id,MM_STACK_0));
#endif
...
#if defined(FEATURE_LTE_TO_1X)
      if (cm_is_esr_allowed_in_srlte_per_subs(ll_as_id)
          && cmph_is_oprting_in_1xsrlte_mode(ll_as_id))
      {
        ...
        */
        ...
        CM_MSG_HIGH_3("sub %d, INCOMING_PAGE_FAIL:%d is_tau_needed %d",
                      ll_as_id, cdma_call_id,cmph_ptr()->device_prop.is_tau_needed);
        if (cdma_call_id == CM_CALL_ID_INVALID
            && cmph_ptr()->device_prop.is_tau_needed == TRUE)
        {
          cmph_ptr()->device_prop.is_tau_needed = FALSE;
          /* send TAU here , since we will not wait for any response from 1X
          */
          cmcsfbcall_send_mm_dual_rx_1xcsfb_tau_req(ll_as_id);
        }
      }
#endif
...
#endif /* FEATURE_MMODE_SC_SVLTE */
      /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
      /* update whether call drop is normal or abnormal
      */
      cmxcall_update_call_debug_info_in_stop_alert(cm_mc_rpt_ptr, call_ptr);

      /* Reset the flag since the call is tracked by call id and call obj now */
      ph_sub_ptr->sub_3gpp2_pref.is_1x_mt_call_pending = FALSE;
#ifdef FEATURE_MMODE_SC_SVLTE
      if(cmcall_is_1x_data_srv_opt(cm_mc_rpt_ptr->stop.srv_opt))
      {
        CM_MSG_HIGH_1("mt_1x_data_wait_stage moved to %d as incoming call received",ph_sub_ptr->sub_3gpp2_pref.mt_1x_data_await_stage);
        ph_sub_ptr->sub_3gpp2_pref.mt_1x_data_await_stage = CM_AWAITING_NONE;
      }
      ...
      */
      ...
      else
      {
        ...
        */
        ...
        {
          CM_ERR_0( "CM_STOP_ALERT_F" " else make no sense" );
        }
      }
        ...
        switch( call_ptr->call_state )
        {
          ...
          case CM_CALL_STATE_ORIG:
            ...
            */
            ...
            cmcall_set_rel_code(call_ptr, cm_mc_rpt_ptr->release.rel_codes[i]);
            cmxcall_e911_orig_fail_ind(call_ptr);
            call_ptr->is_cs_attempted_before_fallback_expiry = TRUE;
            cmcall_orig_proc( call_ptr );
            break;
    ...
    */
    ...
    case CM_HDR_CALL_ORIG_FAIL_F:
      ...
      */
      if( call_ptr->end_status != CM_CALL_END_NONE )
      {
        ...
        cmxcall_e911_orig_fail_ind(call_ptr);
        /* Inform Clients about origination failure at 1x.*/
		cmxcall_1x_orig_fail_ind(call_ptr);
        cmcall_orig_proc( call_ptr );
      }
      ...
      for ( i = 0; i < num_call_id; i++ )
      {
        ...
        if( call_ptr->call_state == CM_CALL_STATE_CONV )
        {
          if ( cm_mc_rpt_ptr->otasp_commit_status.commit_in_progress )
          {
            ...
            {
              CM_ERR_0( "CM_OTASP_COMMIT_STATUS_F" " else make no sense" );
            }
        ...
        switch( call_ptr->call_state)
        {
          ...
          case CM_CALL_STATE_INCOM:
            ...
            */
            ...
            if (call_ptr->csfb_type != CM_CALL_CSFB_TYPE_1XCSFB)
            {
              cmph_update_ddtm_status(call_ptr->mm_id.asubs_id);         /* update DDTM status */
            }
#else
            cmph_update_ddtm_status(call_ptr->mm_id.asubs_id);         /* update DDTM status */
#endif
    ...
    case CM_NDSS_REDIRECTION_F:
      ...
      */
      ...
      {
        CM_ERR_0( "CM_NDSS_REDIRECTION_F" " else make no sense" );
      }
      break;
    ...
    case CM_CALL_FAIL_RETRY_ORDER_F:
      ...
      {
        CM_ERR_0( "CM_CALL_FAIL_RETRY_ORDER_F" " else make no sense" );
      }
      break;
#endif /* FEATURE_IS2000 */
    ...
    case CM_HDR_CONNECTION_DENY_F:
      for ( i = 0; i < cm_mc_rpt_ptr->conn_deny.num_call_ids; i++ )
      {
        ...
        {
          CM_MSG_HIGH_1( "CONNECTION_DENY: Call Id %d not allocated",
    ...
    case CM_CALL_EMERGENCY_FLASHED_F:
    {
      ...
      emergency_flash_call_id_call_ptr->msg_type = cm_mc_rpt_ptr->info_hdr.msg_type;
    ...
    case CM_PRIVACY_SETTING_F:
    {
      ...
      */
      ...
      if( ph_sub_ptr->sub_3gpp2_pref.is_privacy != is_privacy )
      {
        ph_sub_ptr->sub_3gpp2_pref.is_privacy = is_privacy;
        cmcall_obj_iterator_init(&call_itr);
        call_ptr = cmcall_obj_get_next(&call_itr);
        while( call_ptr != NULL )
        {
    ...
    {
      sys_modem_as_id_e_type  asubs_id
        = cm_mmgsdi_ssn_tbl_get_subs_id_for_id(cm_mc_rpt_ptr->voting_mmgsdi.session_id );
          ...
          cm_mc_rpt_ptr->voting_mmgsdi.refresh.stage == MMGSDI_REFRESH_STAGE_WAIT_FOR_OK_TO_INIT)
      {
        ...
        if(ph_sub_ptr->refresh_vote_ok == FALSE)
        {
          if(cm_state == CM_OVERALL_CALL_STATE_IDLE)
          {
            cmmmgsdi_ok_to_refresh(
              cm_mc_rpt_ptr->voting_mmgsdi.refresh.slot,
      ...
      {
        if(cmph_get_received_card_init_completed_evt(asubs_id) == TRUE)
        {
          ...
          */
              ...
              cdma_subs_status == CM_SUBSCRIPTION_STATUS_CHANGE)
          {
            ...
            if(!cmph_is_standby_chg_on_refresh(cm_mc_rpt_ptr->voting_mmgsdi.refresh.mode,
                                               cm_mc_rpt_ptr->voting_mmgsdi.refresh.source ))
            {
            ...
            {
              CM_ERR_3(" subs %d na CDMA %d, GW %d", asubs_id,
                       cdma_subs_status, gw_subs_status);
            }
            else
            {
              CM_MSG_HIGH_3(" subs %d na CDMA %d, GW %d",asubs_id,
                            cdma_subs_status, gw_subs_status);
            }
          } /* if (gw_subs_status ==... */
    ...
    case CM_1XCSFB_ORIG_RES_F:
      ...
      if(cm_mc_rpt_ptr->csfb_org_res.is_allowed)
      {
        ...
        if((call_ptr->sr.is_ok_to_orig == TRUE) &&
            (ss_info_all_stack_0_ptr->sys_mode == SYS_SYS_MODE_LTE))
        {
          cmcsfbcall_send_orig_req (call_ptr);
        }
        else
        {
          if(cmsds_is_csfb_deact_allowed(call_ptr->mm_id.asubs_id))
          {
      ...
      else
      {
        ...
        if((call_ptr->end_status == CM_CALL_END_1XCSFB_HARD_FAILURE ||
            call_ptr->end_status == CM_CALL_END_1XCSFB_SOFT_FAILURE ||
            call_ptr->end_status == CM_CALL_END_REDIR_OR_HANDOFF) &&
            cmsds_is_csfb_deact_allowed(call_ptr->mm_id.asubs_id))
        {
          cmcsfbcall_send_1xcsfb_call_clear( call_ptr );
        }
        /* Process silent redial */
    ...
    case CM_1XCSFB_INCOMING_CALL_F:
      ...
      if (cmph_is_msim() &&
          cmutil_is_as_id_valid(cm_mc_rpt_ptr->hdr.asubs_id) &&
          cm_mc_rpt_ptr->csfb_mt_call.call_type == CM_CALL_TYPE_VOICE &&
          cm_mc_rpt_ptr->csfb_mt_call.vfr != cm_vfr_map[cm_mc_rpt_ptr->hdr.asubs_id])
      {
        CM_MSG_HIGH_2("VFR mismatch in MMODE %d and 1x %d",
                      cm_vfr_map[cm_mc_rpt_ptr->hdr.asubs_id],
                      cm_mc_rpt_ptr->csfb_mt_call.vfr);

        cmcall_send_mc_csfb_end_cmd(cm_mc_rpt_ptr->hdr.asubs_id, cm_mc_rpt_ptr->csfb_mt_call.call_id);
    ...
    case CM_1XSRVCC_HO_COMPLETE_IND_F:
      ...
      if(call_ptr->is_int_ims)
      {
        ...
        p_orig =  cmtask_orig_para_search_orig_mode( call_ptr->mm_id, call_ptr->orig_mode ) ;
        if(p_orig != NULL)
        {
      ...
      if(cmsds_get_sub_prop_ptr(ll_as_id) != NULL)
      {
        cmsds_get_sub_prop_ptr(ll_as_id)->gpp2_sub_info.is_cdma_srvcc_activated = FALSE;
      }
      ...
      if(cm_get_srvcc_ho_error_state() != CM_SRVCC_HO_ERR_UNMANAGED_CALLS)
      {
        while(call_ptr != NULL)
        {
          ...
          else if ( call_ptr->is_srvcc_call && call_ptr->is_int_ims)
          {
            ...
            /* Send MC_SRVCC_CALL_CLEAR_F indication to 1x
            */
            {
              mc_msg_type *mc_ptr = cm_mc_get_buf_else_err_fatal();
...
#if defined(FEATURE_IP_CALL) && defined(FEATURE_CM_LTE)
    {
      ...
      cmcall_event( call_ptr, CM_CALL_EVENT_HO_COMPLETE );
      cmipapp_send_drvcc_notification(CMIPAPP_DRVCC_HO_COMPLETE_NOTIFY,call_ptr->orm_sent_status, call_ptr->mm_id.asubs_id);
      CM_MSG_HIGH_1("CM->MSGR: sub %d, CM_1XDRVCC_HO_COMPLETE_IND_F", call_ptr->mm_id.asubs_id);
      /* set total calls that got transferred to GW */
      ho_comp_list->number_calls= num_srvcc_calls;
...
)
{
  ...
  if(cmd_ptr->cmd == CM_CALL_CMD_ORIG)
  {
    call_ptr->is_hybr_allowed = cmmsc_auto_is_hybr_allowed(
                                  ...
                                  cmutil_map_cm_hybr_pref_to_sd_hybr_pref(
...
)
{
  ...
  */
  ...
  if(call_ptr->call_type == CM_CALL_TYPE_PS_DATA &&
      call_ptr->cmcall_mode_info.info_type == CM_CALL_MODE_INFO_CDMA &&
      end_status == CM_CALL_END_STANDBY_PREF_CHG)
  {
    call_ptr->end_cause_to_client= end_status;
  }
...
/*lint -esym(818, voice_call_ptr) */
{
  ...
  if(!cmph_ptr()->device_prop.trm_cap.is_svdo_supported)
  {
    return FALSE;
  }
  ...
  {
    CM_MSG_HIGH_2("SVDO : Call on sub %d, stack %d, non SVDO scenario",
                  voice_call_ptr->mm_id.asubs_id,
                  voice_call_ptr->mm_id.stk_id);
    return FALSE;
  }
...
boolean cmxcall_is_cdma_ps_call_optimization_enabled(cmcall_s_type  *call_ptr)
{
  ...
  cm_mm_ss_info_sub_prop_s_type *ss_info_sub_prop_ptr = cmss_get_info_sub_prop(call_ptr->mm_id.asubs_id);
  cm_mm_ss_info_gen_stack_info_s_type *ss_info_all_stack_0_ptr;
  cm_mm_ss_info_3gpp2_stack_info_s_type *ss_info_3gpp2_stack_0_ptr;
  cmph_sub_info_s_type *ph_sub_ptr = cmph_get_sub_ptr(call_ptr->mm_id.asubs_id);

  if (!cmutil_is_mm_id_valid(call_ptr->mm_id) || ss_info_sub_prop_ptr == NULL ||
      ss_misc_sub_ptr == NULL || ph_sub_ptr == NULL)
  {
    return FALSE;
  }

  ss_info_all_stack_0_ptr = ss_info_sub_prop_ptr->stack_info[MM_STACK_0].gen_info;
  ss_info_3gpp2_stack_0_ptr = ss_info_sub_prop_ptr->stack_info[MM_STACK_0].gpp2_info;

  if(ss_info_all_stack_0_ptr == NULL || ss_info_3gpp2_stack_0_ptr == NULL)
  {
    CM_MSG_HIGH_1("sub %d stack 1 stack info is NULL", call_ptr->mm_id.asubs_id);
    return FALSE;
  }

  CM_MSG_HIGH_3("ps_call_bsr_uptime %d, curr uptime %d, is_custom_home %d",
                ss_misc_sub_ptr->sub_info.gpp2_sub_info.cdma_ps_call_bsr_uptime,
                curr_uptime, ss_info_3gpp2_stack_0_ptr->is_3gpp2_custom_home);

  if( ( ( ph_sub_ptr->sub_3gpp2_pref.ps_call_optimized.allowed == CMXCALL_PS_OPT_ENABLED_ROAM_ONLY &&
          ss_info_all_stack_0_ptr->roam_status != SYS_ROAM_STATUS_OFF &&
          !ss_info_3gpp2_stack_0_ptr->is_3gpp2_custom_home)
        ||
        ph_sub_ptr->sub_3gpp2_pref.ps_call_optimized.allowed == CMXCALL_PS_OPT_ENABLED_HOME_ROAM
      )
      &&
      ss_misc_sub_ptr->sub_info.gpp2_sub_info.cdma_ps_call_bsr_uptime > curr_uptime
      &&
      call_ptr->call_type == CM_CALL_TYPE_PS_DATA
      &&
