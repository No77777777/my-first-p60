...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/mmode/cm/src/cmi.h#4 $
...
//#define IS_VALID_PH_SUB_PTR(asubs_id) (cmutil_is_as_id_valid(asubs_id) && cmph_ptr()->sub_info[(asubs_id)] != NULL )
...
typedef enum cm_hold_cmd_reas_e
{
  ...
  CM_HOLD_CMD_REAS_PREF_CHG,
  /* Hold command until sys_sel_pref cmd can be processed -if
  ** there is no ppp cleanup and MM is not busy
  */
...
typedef struct cm_call_cmd_info_s
{
  ...
  */
  ...
  /* Info to notify CM of DDS switch or MMS */
...
/* Struct to hold online emergency pending information 
*/
typedef struct cm_emerg_pending_info_s{
  boolean                       enabled;
  
  cm_e911_pending_state_e_type  state;
   /* Pending E911 call state*/

  sys_modem_as_id_e_type        asubs_id;
    /* Sub on which E911 call has to be placed */
}cm_emerg_pending_info_s_type;
...
typedef struct cm_ph_cmd_info_s
{
  ...
  */
  ...
  cm_band_pref_e_type         network_tds_band_pref;
  /* TDS band preference */

  boolean                     is_requeue;
  /* Indicates a re-queued GET_NET cmd */
  ...
  */
  ...
  boolean  is_allow_buffering_cmd;
  /**< Indicates if sys_sel_pref has to be buffered while
   **     UE is busy in certain operations - cleanup, MM busy*/

  boolean close_loopback;
  /*Close Loop Back Mode Ready Ind*/

  cm_ph_secure_call_capability_e_type  secure_call_capability;
  /* Secure call capability/ TF card status */

  cm_ph_ecall_timer_and_residual_time_s_type ecall_timer_residual_value;
  /**<  This command is for APP to inform Ecall/NAS Module about the
        Ecall timer and its residual value (Via CM)*/
  ...
  /* Info used to determine if DDS switch is allowed */
  ...
  uint8                         is_ch_removed_from_mode_pref;
  /* Stores whether CH is removed from mode_pref due to net sel changing to manual */
  
  uint16                          max_search_time;

  sys_incremental_results_s_type  incremental_results;  

  boolean is_data_call_pending_post_dds;
...
typedef struct cm_sups_cmd_info_s
{
  ...
  */
  ...
  boolean          is_all_call_fwd_sups_ext_req;
...
typedef union cm_msgr_cmd_u
{
  ...
  lte_rrc_data_inactivity_info_ind_s    lte_rrc_data_inact_timer_info_ind;
  /* Notify sent from LTE RRC when the Inactivity Timer Expires or RRC Connection Rel Occurs */
#endif
  emm_ra_cap_update_fail_ind_type emm_ra_cap_update_fail_ind;
  ...
  cm_ds_rsra_done_s   ds_rsra_done;
  ...
  cm_mm_get_ssac_hyst_timer_req_s_type get_ssac_hyst_timer_req;
  ...
  LTE_emergency_avoid_plmn_list_type avoid_plmn_list_req;
  mmode_qmi_nas_prioritize_lte_ind_msg_type prioritize_lte_scan_req;
  cm_set_gpp_wifi_cellular_scan_timer_s_type  set_gpp_wifi_cellular_scan_timer_ind;
} cm_msgr_cmd_u_type;
...
#define CM_MAX_SYS_SEL_PREF_HOLD 5
/* Maximum number of sys_sel_pref phcmds to hold in the queue
*/
...
#define CM_MSGR_MSG_CMSDS( msg_id ) \
    (( msg_id >= MM_DOM_SEL_DOMAIN_SELECTED_GET_REQ   && \
      ...
      msg_id == MM_CM_MEDIA_RESTRICTION_IND)
...
*/
#define CM_MSGR_MSG_LTE_RRC_SERVICE( msg_id ) ( msg_id == LTE_RRC_SERVICE_IND )

#define CM_MSGR_MSG_LTE_RRC_BARRING_UPDATE( msg_id ) ( msg_id == LTE_RRC_BARRING_UPDATE_IND )


#define CM_MSGR_MSG_LTE_RRC_BST_UPDATE( msg_id ) ( msg_id == LTE_RRC_MANUAL_BST_UPDATE_IND )
...
#define CM_MSGR_MSG_LTE_RRC_DATA_INACTIVITY_INFO( msg_id ) ( msg_id == LTE_RRC_DATA_INACTIVITY_INFO_IND )
...
/* Macro to check MSG ID for EMM RADIO CAPABILITY UPDATE FAIL
*/
#define CM_MSGR_EMM_RA_CAP_UPDATE_FAIL( msg_id ) ( msg_id == NAS_EMM_RA_CAP_UPDATE_FAIL_IND )
...
#define CM_MSGR_IS_DS_3GPP_CM_RS_RA_DONE_IND( msg_id ) \
                            (msg_id == DS_3GPP_CM_RS_RA_DONE_IND)
...
#define CM_MSGR_IS_MM_SET_SSAC_HYST_TIMER_REQ( msg_id ) \
                            (msg_id == MM_CM_SET_SSAC_HYST_TIMER_REQ)

#define CM_MSGR_IS_MM_GET_SSAC_HYST_TIMER_REQ( msg_id ) \
                            (msg_id == MM_CM_GET_SSAC_HYST_TIMER_REQ)
...
#define CM_MSGR_IS_MM_CM_SUBSC_CHGND_RSP( msg_id ) \
                            (msg_id == MM_CM_SUBSC_CHGND_RSP)

#define CM_MSGR_IS_MM_CM_911_AVOID_PLMN_LIST_IND( msg_id ) \
                            (msg_id == MM_CM_E911_AVOID_PLMN_IND)

#define CM_MSGR_IS_QMI_NAS_PRIORITIZE_LTE_IND( msg_id ) \
                            (msg_id == QMI_NAS_PRIORITIZE_LTE_IND)

                            
#define CM_MSGR_SET_GPP_WIFI_CELLULAR_SCAN_TIMER_IND( msg_id ) \
								(msg_id == MM_CM_SET_GPP_WIFI_CELLULAR_SCAN_TIMER_IND)
...
/*ss codes index to use in ue_based_sups*/
typedef enum {
  colr_idx = 0,
  allCallRestrictionSS_idx = 1,
  baoc_idx = 2,
  boic_idx = 3,
  boicExHC_idx = 4,
  baic_idx = 5,
  bicRoam_idx = 6,
  bain_idx = 7,
  cw_idx = CMEFS_UE_BASED_SUPS_ARRAY_USED
} ss_code_idx;
...
extern uint8 cm_hold_cmd_q_cnt_of_type(
...
===========================================================================*/
extern boolean cmtask_orig_para_change_act_priority_4(
...
extern boolean cmtask_is_valid_mm_id_for_prio_q(
  mm_sub_stk_id_s_type mm_id
);
...
===========================================================================*/
extern cm_orig_q_s_type *cmtask_orig_para_alloc( mm_sub_stk_id_s_type mm_id );
...
extern void cm_end_pre_alert_mt_volte_call(
...
FUNCTION cmutilt_map_cm_ss_to_mmid

DESCRIPTION
  Map CM SS to unique stack id (subid+stack id)
  should be temporary as CM SS will be deprecated soon.
...
===========================================================================*/
extern boolean is_contain_3GPP_mode_pref
(
  cm_mode_pref_e_type mode_pref
);
...
===========================================================================*/
extern boolean                         is_contain_3GPP2_mode_pref(
...
===========================================================================*/
extern int cmutil_cmp_act_priorities(
...
===========================================================================*/
extern cm_act_priority_e_type cmtask_get_act_priority(
...
===========================================================================*/
extern boolean cmutil_is_as_id_valid(
...
===========================================================================*/
extern boolean cmutil_is_valid_lte_mode_band_pref(
...
===========================================================================*/
extern boolean cmutil_is_valid_mode_band_pref(
...
===========================================================================*/
extern boolean cmutil_is_valid_tds_mode_band_pref(
...
===========================================================================*/
extern cm_act_type_e_type cmtask_map_call_type_to_act_type(
...
===========================================================================*/
extern sd_ss_band_pref_e_type   cmutil_map_cm_band_pref_to_sd_band_pref(
...
===========================================================================*/
extern sd_ss_pref_camp_mode_e_type cmutil_map_cm_net_sel_mode_pref_to_camp_pref
(
...
SIDE EFFECTS
  none
===========================================================================*/
extern sd_ss_orig_mode_e_type  cmutil_map_cm_network_sel_mode_pref_to_orig_mode
(
  cm_network_sel_mode_pref_e_type    network_sel_mode_pref
);
...
===========================================================================*/
extern sd_ss_prl_pref_e_type cmutil_map_cm_prl_pref_to_sd_prl_pref(
...
===========================================================================*/
extern sd_ss_roam_pref_e_type cmutil_map_cm_roam_pref_to_sd_roam_pref(
...
===========================================================================*/
extern sd_ss_srv_domain_pref_e_type cmutil_map_cm_srv_domain_pref_to_sd(

  cm_srv_domain_pref_e_type  srv_domain_pref
  /* CM Service domain preference that gets mapped to sd srv mode pref*/
);
...
===========================================================================*/
extern sd_ss_band_pref_e_type   cmutil_map_cm_tds_band_pref_to_sd_tds_band_pref(
...
===========================================================================*/
extern sd_band_e_type   cmutil_map_otasp_act_code_to_sd_band(
...
===========================================================================*/
extern sd_blksys_e_type   cmutil_map_otasp_act_code_to_sd_blksys(
...
===========================================================================*/
extern cm_band_pref_e_type  cmutil_map_sd_band_pref_to_cm_band_pref(
...
===========================================================================*/
extern sd_ss_mode_pref_e_type   cmutil_map_cm_emerg_mode_pref_to_sd_mode_pref( void );
...
===========================================================================*/
extern sys_sys_mode_mask_e_type cmutil_map_sd_mode_pref_to_sys_mode_mask(
...
===========================================================================*/
extern cm_band_pref_e_type  cmutil_map_sd_tds_band_pref_to_cm_tds_band_pref(
...
===========================================================================*/
extern sd_ss_mode_pref_e_type   cmutil_map_sys_mode_mask_to_sd_mode_pref(
...
===========================================================================*/
extern cm_srv_type_e_type cmutil_map_sys_mode_to_srv_type(
  sys_sys_mode_e_type sys_mode
);
...
===========================================================================*/
extern cm_mode_pref_e_type cmutil_misc_intersect_mode_pref(
...
===========================================================================*/
extern cm_hybr_pref_e_type cmutil_map_sd_hybr_pref_to_cm_hybr_pref
(
...
===========================================================================*/
extern sd_ss_hybr_pref_e_type cmutil_map_cm_hybr_pref_to_sd_hybr_pref
(
  cm_hybr_pref_e_type       hybr_pref
  /* hybr pref to map */
);
...
===========================================================================*/
extern sd_mode_e_type cmutil_map_sys_sys_mode_to_sd_mode(sys_sys_mode_e_type mode);
...
SIDE EFFECTS
  None.
===========================================================================*/
extern boolean    cmmsc_auto_is_hybr_hdr_call_allowed(
  sys_modem_as_id_e_type  asubs_id
);
...
===========================================================================*/
extern cm_mode_pref_e_type cmutil_get_supp_mode_pref(
...
===========================================================================*/
extern void cmtask_remove_orig_csg_params(
...
===========================================================================*/
extern uint8 cmtask_get_num_stacks(sys_modem_as_id_e_type asubs_id);
...
===========================================================================*/
extern uint8 cmtask_get_targeted_num_stacks(sys_modem_as_id_e_type asubs_id);
...
===========================================================================*/
extern boolean cmtask_is_stack_pending_for_deact(mm_sub_stk_id_s_type mm_id);

/*===========================================================================

FUNCTION cmtask_add_new_prio_q
...
===========================================================================*/
extern sys_modem_as_id_e_type cmutil_get_variant(
...
===========================================================================*/
boolean cmutil_is_periodic_manual_search(void);
...
/*Returns TRUE if dialed number is international*/
boolean cmutil_is_dialed_number_international(
   sys_mcc_type          current_mcc, 
   cm_num_s_type   num);

/*Returns TRUE if dialed number is international but not to home country*/
boolean cmutil_is_dialed_number_international_except_home( 
