...
$Header: //components/rel/mmcp.mpss/7.9.0/mmode/cm/src/cmipapp.c#3 $
...
static boolean last_ims_reg_avail[MAX_SIMS] = {FALSE};
static boolean last_ims_emerg_cap_avail[MAX_SIMS] = {FALSE};
...
)
{
  ...
  uint8  cap_index = 0;
  boolean ims_reg_avail = FALSE;
  boolean ims_emerg_cap_avail = FALSE;
  boolean is_changed = FALSE;

  if (!(asubs_id >= SYS_MODEM_AS_ID_1 && asubs_id < MAX_SIMS))
  {
    CM_MSG_HIGH_1("cmipapp_send_reg_status: Invalid asubs_id %d, default to as_id_1", asubs_id);
  ...
  */
  ...
  {
    if (cap_index == CM_CALL_TYPE_VOICE
        &&
        (cmipapp_arr[app_id].app_capability.call_type[cap_index] & CM_CALL_TYPE_VOICE_MASK)
        &&
        (cmipapp_arr[app_id].app_capability.sys_mode[cap_index] & SYS_SYS_MODE_MASK_LTE)
        &&
        (ipapp_reg_status == CM_IPAPP_REG_STATUS_REGISTERED)
       )
    {
      ims_reg_avail = TRUE;
    }

    else if (cap_index == CM_CALL_TYPE_EMERGENCY
             &&
             (cmipapp_arr[app_id].app_capability.call_type[cap_index] & CM_CALL_TYPE_EMERG_MASK)
             &&
             (cmipapp_arr[app_id].app_capability.sys_mode[cap_index] & SYS_SYS_MODE_MASK_LTE)
             &&
             (ipapp_reg_status == CM_IPAPP_REG_STATUS_REGISTERED)
    ...
    {
      ims_emerg_cap_avail = TRUE;
    }
  }

  /* Queue sd rpt only if there is a change in ims reg/e911 cap/ipapp reg status
  */
  if (ims_reg_avail != last_ims_reg_avail[asubs_id] ||
      ims_emerg_cap_avail != last_ims_emerg_cap_avail[asubs_id] ||
      app_status_chgd)
  {
    is_changed = TRUE;
    ...
    rpt_cmd_ptr->hdr.asubs_id = cmipapp_arr[app_id].asubs_id;
    ...
    rpt_cmd_ptr->ip_app_reg_status.ipapp_cause = cmipapp_arr[app_id].reason.ipapp_cause;

    /* Warning info tied to registration */
    rpt_cmd_ptr->ip_app_reg_status.warn_info = cmipapp_arr[app_id].reason.warn_info;

    last_ims_reg_avail[asubs_id] = ims_reg_avail;
    last_ims_emerg_cap_avail[asubs_id] = ims_emerg_cap_avail;
  }
...
)
{
  ...
  switch (cmcall_end_cause)
  {
    ...
    case CM_CALL_END_PRECONDITION_FAILURE:
      cmipcall_end_cause = CMIPAPP_END_CAUSE_PRECONDITION_FAILURE;
      break;

    case CM_CALL_END_ACCEPT_MT_CS_PAGE:
      cmipcall_end_cause = CMIPAPP_END_CAUSE_ACCEPT_MT_CS_PAGE;
      break;
...
)
{
  ...
  */
  ...
  cmipapp_arr[app_id].asubs_id = SYS_MODEM_AS_ID_NONE;
  rex_leave_crit_sect(cmipapp_crit_sect_ptr());
...
/*lint -esym(818,app_cb_ptr) */
{
  ...
  app_cb_ptr->cmipapp_inform_oprt_mode_sub                =   NULL;
  ...
  /* For IP ECT Functionality */
  app_cb_ptr->cmipapp_cmd_ip_ect_sub_func_ptr                 =   NULL;

  app_cb_ptr->cmipapp_drvcc_status_sub_func_ptr               =   NULL;

  /* For secure call Functionality */
  app_cb_ptr->cmipapp_cmd_inform_secure_call_capability_func_ptr      =   NULL;

  /* For secure call Functionality */
  app_cb_ptr->cmipapp_cmd_inform_secure_context_func_ptr              =   NULL;

  /* To send SIP DTMF string */
  app_cb_ptr->cmipapp_cmd_sip_dtmf_sub_func_ptr = NULL;
...
/*lint -esym(715,app_id) */
{
  ...
  */
  ...
  asubs_id = cmipapp_arr[app_id].asubs_id;
  ...
  */
  ...
  if (asubs_id >= SYS_MODEM_AS_ID_1 && asubs_id < MAX_SIMS)
  {
    last_ims_reg_avail[asubs_id] = FALSE;
    last_ims_emerg_cap_avail[asubs_id] = FALSE;
  }
  ...
  */
  ...
  rpt_cmd_ptr->hdr.asubs_id = asubs_id;

  CM_MSG_HIGH_2 ("IP_APP: app_id %d deallocated sub %d", app_id, asubs_id);
...
)
{
  switch (sys_mode)
  {
    case CMIPAPP_SYS_MODE_NONE:
      return SYS_SYS_MODE_NONE;

    case CMIPAPP_SYS_MODE_CDMA:
      return SYS_SYS_MODE_CDMA;

    case CMIPAPP_SYS_MODE_HDR:
      return SYS_SYS_MODE_HDR;

    case CMIPAPP_SYS_MODE_GSM:
      return SYS_SYS_MODE_GSM;

    case CMIPAPP_SYS_MODE_WCDMA:
      return SYS_SYS_MODE_WCDMA;

    case CMIPAPP_SYS_MODE_LTE:
      return SYS_SYS_MODE_LTE;

    case CMIPAPP_SYS_MODE_WLAN:
...
)
{
    ...
    */
    for(cap_index=0; cap_index<CMIPAPP_CAP_ARR_MAX; cap_index++)
    {
      if((cmipapp_arr[id_count].app_id != CMIPAPP_ID_INVALID)
          ...
          &&
          (cmipapp_arr[id_count].asubs_id == asubs_id)
        )
      {
  ...
  */
  ...
  {
    CM_MSG_HIGH_4("IP_CALL: Found active app %d for call type %d, sys mode %d, asubs_id %d",
                  app_id, call_type, sys_mode, asubs_id);
  }
  else if (sys_mode == SYS_SYS_MODE_LTE && call_type == CM_CALL_TYPE_EMERGENCY)
  {
    CM_MSG_HIGH_3("IP_CALL: Not Found active app for call type %d, sys mode %d, asubs_id %d",
                  call_type, sys_mode, asubs_id);
  }
...
DESCRIPTION
Finds an active app sys mode for a particular call type
...
)
{
  ...
  */
  ...
  if (id_count < ARR_SIZE(cmipapp_arr))
  {
    CM_MSG_HIGH_4("IP_CALL: Found active app sys mode %d for call type %d, app_id %d, asubs_id %d",
                  sys_mode, call_type, cmipapp_arr[id_count].app_id,
                  asubs_id);
  }
...
/*lint -esym(715,sys_mode) */
{
  ...
  */
  if(app_id == CMIPAPP_ID_INVALID ||
      app_id >= ARR_SIZE(cmipapp_arr))
  {
    CM_MSG_HIGH_1("CAP_REG: app_id %d is not valid", app_id);
  ...
  */
  ...
  CM_MSG_HIGH_4 ("CAP_REG: Resetting capabilties before taking new ones, app %d, call_type 0x%x, sys_mode 0x%x, sub %d",
                 app_id, call_type, sys_mode,
                 cmipapp_arr[app_id].asubs_id);

  for(cap_index=0; cap_index<CMIPAPP_CAP_ARR_MAX; cap_index++)
  {
    /* Do not reset emergency capability */
    if(cap_index == CM_CALL_TYPE_EMERGENCY)
    {
      continue;
    }
    /* Call types app can service */
  ...
  {
    CM_MSG_HIGH_3 ("CAP_REG: Call Type 0x%x and Sys Mode 0x%x of APP %d update failed",
...
/*lint -esym(715,app_run_status) */
{
  ...
  */
  if(app_id == CMIPAPP_ID_INVALID ||
      app_id >= ARR_SIZE(cmipapp_arr))
  {
    CM_MSG_HIGH_1("IP_APP: INFORM_STATUS: app_id %d is not valid", app_id);
  ...
  */
  ...
  if (cmipapp_arr[app_id].app_run_status != app_run_status)
  {
    CM_MSG_HIGH_4 ("IP_APP: INFORM_STATUS: %d->%d, App %d, sub %d",
                   cmipapp_arr[app_id].app_run_status,
                   app_run_status, app_id, cmipapp_arr[app_id].asubs_id);
  }
  ...
  */
  ...
  {
    cmipapp_send_reg_status (app_id, CM_IPAPP_REG_STATUS_FAILURE, TRUE, cmipapp_arr[app_id].asubs_id);
  ...
  */
    ...
    )
  {
    ...
    */
    ...
    cmipapp_send_reg_status (app_id, CM_IPAPP_REG_STATUS_DEREGISTERED, TRUE, cmipapp_arr[app_id].asubs_id);
    ...
    )
  {
    ...
    */
    if ( ((cmipapp_arr[app_id].app_cb.cmipapp_cmd_orig_func_ptr == NULL) &&
         ...
         ||
         (cmipapp_arr[app_id].app_cb.cmipapp_cmd_sip_dtmf_sub_func_ptr == NULL)
       )
    {
      CM_ERR_1("IP_APP: INFORM_STATUS: App %d is active but no cb functions", app_id);
      return CMIPAPP_STATUS_INFORM_FAILED;
    }
    ...
    */
    ...
    cmipapp_send_reg_status (app_id, CM_IPAPP_REG_STATUS_REGISTERED, TRUE, cmipapp_arr[app_id].asubs_id);

    return CMIPAPP_STATUS_OK;
  }
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

  CM_MSG_HIGH_3("IP_APP: INFORM_STATUS: App %d run_status %d, sub %d", 
                   app_id, cmipapp_arr[app_id].app_run_status,
                   cmipapp_arr[app_id].asubs_id);
...
/*lint -esym(715,voice_call_id) */
{
  ...
  */
  ...
  CM_MSG_HIGH_3("CM->CM: CM_IP_CALL_IND_VS_STATUS, voice_call_id %d, app_run_status %d, sub %d",
                voice_call_id,
                app_run_status,
                cmipapp_arr[app_id].asubs_id);
...
FUNCTION cmipapp_id_allocate_sub
...
===========================================================================*/
extern cmipapp_status_e_type cmipapp_id_allocate_sub (
  cmipapp_id_allocate_s_type  info
)
{
#if defined(FEATURE_IP_CALL)
  cmipapp_id_type             app_id = CMIPAPP_ID_INVALID;
  ...
  */
  uint8                  cap_index   = 0;
  cmipapp_call_type_e_type     call_type = info.call_type;
  cmipapp_sys_mode_e_type      sys_mode = info.sys_mode;
  cmipapp_id_type             *app_id_ptr = info.app_id_ptr;
  ...
  */
  CM_ASSERT (BETWEEN (call_type, CMIPAPP_CALL_TYPE_NONE, CMIPAPP_CALL_TYPE_MAX));
  CM_ASSERT (BETWEEN (sys_mode, CMIPAPP_SYS_MODE_NONE, CMIPAPP_SYS_MODE_MAX));
  ...
  for (id_count = 0; id_count < ARR_SIZE(cmipapp_arr); id_count++)
  {
    for(cap_index=0; cap_index<CMIPAPP_CAP_ARR_MAX; cap_index++)
    {
      ...
      if ((cmipapp_arr[id_count].app_id    != CMIPAPP_ID_INVALID)
          ...
          &&
          (cmipapp_arr[id_count].asubs_id == info.asubs_id)
         )
      {
  ...
  */
  if (!is_duplicate_app_present)
  {
    for (id_count = 0; id_count < ARR_SIZE(cmipapp_arr); id_count++)
    {
      if (cmipapp_arr[id_count].app_id == CMIPAPP_ID_INVALID)
      {
        ...
        cmipapp_arr[app_id].asubs_id = info.asubs_id;

        /* Access the table to set sys_mode and call type */
        if(cmipapp_cap_update(call_type, sys_mode, app_id) == CMIPAPP_STATUS_OK)
        {
          CM_MSG_HIGH_4 ("IP_APP: Call Type: 0x%x and Sys Mode: 0x%x of APP %d updated sucessfully, sub %d",
                         call_type, sys_mode, app_id,
                         cmipapp_arr[app_id].asubs_id);
        }
        break;
  ...
  CM_MSG_HIGH_2( "IP_APP: App_id %d allocated, sub %d", app_id, info.asubs_id);
...
/*===========================================================================
FUNCTION cmipapp_map_mask_cap_sys_mode_to_sys_mode

DESCRIPTION
  Map capability sys mode mask to ipapp sys mode mask

RETURN VALUE
  ipapp sys mode mask

===========================================================================*/
cmipapp_sys_mode_e_type cmipapp_map_mask_cap_sys_mode_to_sys_mode(
  cmipapp_cap_sys_mode_mask_e_type cap_sys_mode_mask
)
{
  cmipapp_sys_mode_e_type app_sys_mode_mask = CMIPAPP_SYS_MODE_NONE;

  if((cap_sys_mode_mask & CMIPAPP_CAP_SYS_MODE_MASK_CDMA) != 0)
  {
    app_sys_mode_mask |= CMIPAPP_SYS_MODE_CDMA;
  }
  if((cap_sys_mode_mask & CMIPAPP_CAP_SYS_MODE_MASK_HDR) != 0)
  {
    app_sys_mode_mask |= CMIPAPP_SYS_MODE_HDR;
  }
  if((cap_sys_mode_mask & CMIPAPP_CAP_SYS_MODE_MASK_GSM) != 0)
  {
    app_sys_mode_mask |= CMIPAPP_SYS_MODE_GSM;
  }
  if((cap_sys_mode_mask & CMIPAPP_CAP_SYS_MODE_MASK_WCDMA) != 0)
  {
    app_sys_mode_mask |= CMIPAPP_SYS_MODE_WCDMA;
  }
  if((cap_sys_mode_mask & CMIPAPP_CAP_SYS_MODE_MASK_LTE) != 0)
  {
    app_sys_mode_mask |= CMIPAPP_SYS_MODE_LTE;
  }
#ifdef FEATURE_NR5G_SA
  if((cap_sys_mode_mask & CMIPAPP_CAP_SYS_MODE_MASK_NR5G) != 0)
  {
    app_sys_mode_mask |= CMIPAPP_SYS_MODE_NR5G;
  }
#endif
  if((cap_sys_mode_mask & CMIPAPP_CAP_SYS_MODE_MASK_WLAN) != 0)
  {
    app_sys_mode_mask |= CMIPAPP_SYS_MODE_WLAN;
  }

  return app_sys_mode_mask;
}

/*===========================================================================

FUNCTION cmipapp_cap_set
...
with this new one.
If sys mode is set to no change for a call type, the capability
of that call type will remain unchanged.

RETURN VALUE
CMIPAPP_STATUS_OK            - Capabilities successfully updated

===========================================================================*/
cmipapp_status_e_type cmipapp_cap_set(

  cmipapp_id_type             app_id,
  /* App ID pointer
  */

  cmipapp_all_cap_s_type  *all_cap_sys_mode
  /* System modes app can support for calls
  */
)
{
  ...
  */
  ...
  CM_MSG_HIGH_3( "CAP_UPDATE: App %d, app_run_status %d, sub %d",
                 app_id,
                 cmipapp_arr[app_id].app_run_status,
                 cmipapp_arr[app_id].asubs_id);
  CM_MSG_HIGH_6( "CAP_UPDATE: sys_mode: voice 0x%x, vt 0x%x, sups 0x%x, e911 0x%x, sms 0x%x, vs 0x%x",
                 all_cap_sys_mode->voice_sys_mode,
                 all_cap_sys_mode->vt_sys_mode,
                 all_cap_sys_mode->sups_sys_mode,
                 all_cap_sys_mode->e911_sys_mode,
                 all_cap_sys_mode->sms_sys_mode,
                 all_cap_sys_mode->vs_sys_mode);

  if(all_cap_sys_mode->voice_sys_mode != CMIPAPP_CAP_SYS_MODE_MASK_NO_CHANGE)
  {
    cmipapp_arr[app_id].app_capability.sys_mode[CM_CALL_TYPE_VOICE] = 
        cmipapp_map_mask_cap_sys_mode_to_sys_mode(all_cap_sys_mode->voice_sys_mode);
    cmipapp_arr[app_id].app_capability.call_type[CM_CALL_TYPE_VOICE] = SYS_BM_64BIT(CM_CALL_TYPE_VOICE);

  }
  if(all_cap_sys_mode->e911_sys_mode != CMIPAPP_CAP_SYS_MODE_MASK_NO_CHANGE)
  {
    cmipapp_arr[app_id].app_capability.sys_mode[CM_CALL_TYPE_EMERGENCY] = 
        cmipapp_map_mask_cap_sys_mode_to_sys_mode(all_cap_sys_mode->e911_sys_mode);
    cmipapp_arr[app_id].app_capability.call_type[CM_CALL_TYPE_EMERGENCY] = SYS_BM_64BIT(CM_CALL_TYPE_EMERGENCY);
  }
  if(all_cap_sys_mode->vt_sys_mode != CMIPAPP_CAP_SYS_MODE_MASK_NO_CHANGE)
  {
    cmipapp_arr[app_id].app_capability.sys_mode[CM_CALL_TYPE_VT] = 
      cmipapp_map_mask_cap_sys_mode_to_sys_mode(all_cap_sys_mode->vt_sys_mode);
    cmipapp_arr[app_id].app_capability.call_type[CM_CALL_TYPE_VT] = SYS_BM_64BIT(CM_CALL_TYPE_VT);
  }
  if(all_cap_sys_mode->vs_sys_mode != CMIPAPP_CAP_SYS_MODE_MASK_NO_CHANGE)
  {
    cmipapp_arr[app_id].app_capability.sys_mode[CM_CALL_TYPE_VS] = 
      cmipapp_map_mask_cap_sys_mode_to_sys_mode(all_cap_sys_mode->vs_sys_mode);
    cmipapp_arr[app_id].app_capability.call_type[CM_CALL_TYPE_VS] = SYS_BM_64BIT(CM_CALL_TYPE_VS);
  }
  if(all_cap_sys_mode->sms_sys_mode != CMIPAPP_CAP_SYS_MODE_MASK_NO_CHANGE)
  {
    cmipapp_arr[app_id].app_capability.sys_mode[CM_CALL_TYPE_SMS] = 
      cmipapp_map_mask_cap_sys_mode_to_sys_mode(all_cap_sys_mode->sms_sys_mode);
    cmipapp_arr[app_id].app_capability.call_type[CM_CALL_TYPE_SMS] = SYS_BM_64BIT(CM_CALL_TYPE_SMS);
  }
  if(all_cap_sys_mode->sups_sys_mode != CMIPAPP_CAP_SYS_MODE_MASK_NO_CHANGE)
  {
    cmipapp_arr[app_id].app_capability.sys_mode[CM_CALL_TYPE_SUPS] = 
      cmipapp_map_mask_cap_sys_mode_to_sys_mode(all_cap_sys_mode->sups_sys_mode);
  ...
  for(cap_index=0; cap_index<CMIPAPP_CAP_ARR_MAX; cap_index++)
  {
    if(cmipapp_arr[app_id].app_capability.sys_mode[cap_index] || cmipapp_arr[app_id].app_capability.call_type[cap_index])
      CM_MSG_HIGH_3( "CAP_UPDATE: index %d, CallTypeMask: 0x%x , SysmodeMask: 0x%x",
                     cap_index, cmipapp_arr[app_id].app_capability.call_type[cap_index],
                     cmipapp_arr[app_id].app_capability.sys_mode[cap_index]);
  }

  if ( cmipapp_arr[app_id].app_run_status == CMIPAPP_RUN_STATUS_ACTIVE)
  {
    cmipapp_send_reg_status (app_id, CM_IPAPP_REG_STATUS_REGISTERED, FALSE, cmipapp_arr[app_id].asubs_id);
  }
  else if (cmipapp_arr[app_id].app_run_status == CMIPAPP_RUN_STATUS_INACTIVE)
  {
    cmipapp_send_reg_status (app_id, CM_IPAPP_REG_STATUS_DEREGISTERED, FALSE, cmipapp_arr[app_id].asubs_id);
  }
...
/*lint -esym(818,app_id_ptr) */
{
  ...
  cmipapp_id_allocate_s_type   app_info;

  app_info.call_type = call_type;
  app_info.sys_mode = sys_mode;
  app_info.app_id_ptr = app_id_ptr;
  app_info.asubs_id = SYS_MODEM_AS_ID_1;

  return cmipapp_id_allocate_sub(app_info);
...
/*lint -esym(818,app_id_ptr) */
{
  ...
  */
  ...
  CM_MSG_HIGH_5( "CAP_UPDATE: App %d, CallTypeMask: 0x%x and SysmodeMask 0x%x, app_run_status %d, sub %d",
                 app_id, call_type, sys_mode,
                 cmipapp_arr[app_id].app_run_status,
                 cmipapp_arr[app_id].asubs_id);
...
/*lint -esym(715,cmipapp_inform_srv_domain_func_ptr) */
{
  ...
  CM_MSG_HIGH_2("Registering srv_domain_func, app_id %d sub %d", app_id, cmipapp_arr[app_id].asubs_id);
...
/*lint -esym(715,cmipapp_inform_oprt_mode_func_ptr) */
{
  ...
  CM_MSG_HIGH_2("Registering oprt_mode_func, app_id %d sub %d", app_id, cmipapp_arr[app_id].asubs_id);
...

{
  ...
  CM_MSG_HIGH_2("Registering subs_cmd_cb, app_id %d sub %d", app_id, cmipapp_arr[app_id].asubs_id);
...
FUNCTION cmipapp_add_reference_allfwd_sups_error_text

DESCRIPTION
add reference to the reference counted object during sups conf
...
===========================================================================*/
void cmipapp_add_reference_allfwd_sups_error_text
(
  cmipsups_ext_resp_s_type  *ext_resp
)
{
  if (ext_resp != NULL)
  {
    if(ext_resp->resp_type == ALL_CALL_FWD_INFO)
    {
      if(ext_resp->info.all_call_fwd_ext_resp.cfu_sups_ss_error_text != NULL)
      { 
        CM_MSG_HIGH_0("added ref for cfu_sups_ss_error_text");
              
        ref_cnt_obj_add_ref(ext_resp->info.all_call_fwd_ext_resp.cfu_sups_ss_error_text);
      }
    
      if(ext_resp->info.all_call_fwd_ext_resp.cfb_sups_ss_error_text != NULL)
      { 
        CM_MSG_HIGH_0("added ref for cfb_sups_ss_error_text ");
              
        ref_cnt_obj_add_ref(ext_resp->info.all_call_fwd_ext_resp.cfb_sups_ss_error_text);
      }
      
      if(ext_resp->info.all_call_fwd_ext_resp.cfnry_sups_ss_error_text != NULL)
      { 
        CM_MSG_HIGH_0("added ref for cfnry_sups_ss_error_text ");
              
        ref_cnt_obj_add_ref(ext_resp->info.all_call_fwd_ext_resp.cfnry_sups_ss_error_text);
      }
    
      if(ext_resp->info.all_call_fwd_ext_resp.cfnrc_sups_ss_error_text != NULL)
      { 
        CM_MSG_HIGH_0("added ref for cfnrc_sups_ss_error_text");
              
        ref_cnt_obj_add_ref(ext_resp->info.all_call_fwd_ext_resp.cfnrc_sups_ss_error_text);
      }
    }
  }
...
(
  ...
  cmipapp_rpt_s_type  *cmipapp_rpt_ptr
...
)
{
  ...
  switch( cmipapp_rpt_ptr->hdr.cmd )
  {
    ...
    case CM_IP_CALL_IND_MT_INVITE:
      ...
      {
        CM_MSG_HIGH_0("cmippap_add_ref :Added ref for alert_info");
        ref_cnt_obj_add_ref(cmipapp_rpt_ptr->ip_ind.ind.mt_invite.call_attrib_info.alert_info);
      }
      ...
      {
        if (cmipapp_rpt_ptr->ip_ind.ind.mt_invite.call_attrib_info.call_composer_info.subject &&
           cmipapp_rpt_ptr->ip_ind.ind.mt_invite.call_attrib_info.call_composer_info.subject->subject_length != 0 &&
           cmipapp_rpt_ptr->ip_ind.ind.mt_invite.call_attrib_info.call_composer_info.subject->subject_text
        )
        {
          CM_MSG_HIGH_1("call_composer: cmippap_add_ref :Added ref for subject, call_id %d",
                            cmipapp_rpt_ptr->ip_ind.ind.mt_invite.id);
          ref_cnt_obj_add_ref(
              cmipapp_rpt_ptr->ip_ind.ind.mt_invite.call_attrib_info.call_composer_info.subject);
        }
  
        if (cmipapp_rpt_ptr->ip_ind.ind.mt_invite.call_attrib_info.call_composer_info.picture_url &&
           cmipapp_rpt_ptr->ip_ind.ind.mt_invite.call_attrib_info.call_composer_info.picture_url->pic_url
        )
        {
          CM_MSG_HIGH_1("call_composer: cmippap_add_ref :Added ref for picture_url, call_id %d",
                           cmipapp_rpt_ptr->ip_ind.ind.mt_invite.id);
          ref_cnt_obj_add_ref(
              cmipapp_rpt_ptr->ip_ind.ind.mt_invite.call_attrib_info.call_composer_info.picture_url);
        }
        if (cmipapp_rpt_ptr->ip_ind.ind.mt_invite.call_attrib_info.call_composer_info.org_header &&
            cmipapp_rpt_ptr->ip_ind.ind.mt_invite.call_attrib_info.call_composer_info.org_header->org_header_length != 0 &&
            cmipapp_rpt_ptr->ip_ind.ind.mt_invite.call_attrib_info.call_composer_info.org_header->org_header_text
        )
        {
          CM_MSG_HIGH_1("call_composer: cmippap_add_ref :Added ref for org_header, call_id %d",
                            cmipapp_rpt_ptr->ip_ind.ind.mt_invite.id);
          ref_cnt_obj_add_ref(
      ...
      #else
      cmipapp_rpt_ptr->ip_ind.ind.mt_invite.call_attrib_info.call_composer_info.info_included = FALSE;
      #endif
    ...
    case CM_IP_SUPS_ERASE_SS_CONF:
      ...
      if((cmipapp_rpt_ptr->ip_ind.ind.erase_ss_conf.is_ext_resp == TRUE) &&
         (cmipapp_rpt_ptr->ip_ind.ind.erase_ss_conf.ext_resp.resp_type == ALL_CALL_FWD_INFO))
      {
        cmipapp_add_reference_allfwd_sups_error_text(&(cmipapp_rpt_ptr->ip_ind.ind.erase_ss_conf.ext_resp));
      }
    ...
    case  CM_IP_SUPS_ACTIVATE_SS_CONF:
      ...
      if((cmipapp_rpt_ptr->ip_ind.ind.activate_ss_conf.is_ext_resp == TRUE) &&
         (cmipapp_rpt_ptr->ip_ind.ind.activate_ss_conf.ext_resp.resp_type == ALL_CALL_FWD_INFO))
      {
        cmipapp_add_reference_allfwd_sups_error_text(&(cmipapp_rpt_ptr->ip_ind.ind.activate_ss_conf.ext_resp));
      }
    ...
    case CM_IP_SUPS_DEACTIVATE_SS_CONF:
      ...
      if((cmipapp_rpt_ptr->ip_ind.ind.deactivate_ss_conf.is_ext_resp == TRUE) &&
         (cmipapp_rpt_ptr->ip_ind.ind.deactivate_ss_conf.ext_resp.resp_type == ALL_CALL_FWD_INFO))
      {
        cmipapp_add_reference_allfwd_sups_error_text(&cmipapp_rpt_ptr->ip_ind.ind.deactivate_ss_conf.ext_resp);
      }
    ...
    case CM_IP_SUPS_REGISTER_SS_CONF:
      ...
      if(cmipapp_rpt_ptr->ip_ind.ind.register_ss_conf.sups_ss_info.info.forwarding_info.present)
      {
        if(cmipapp_rpt_ptr->ip_ind.ind.register_ss_conf.sups_ss_info.info.forwarding_info.forwarding_feature_list.present)
        {
          for(i=0;i<cmipapp_rpt_ptr->ip_ind.ind.register_ss_conf.sups_ss_info.info.forwarding_info.forwarding_feature_list.forwarding_feature_list_length;i++)
          {
            if(cmipapp_rpt_ptr->ip_ind.ind.register_ss_conf.sups_ss_info.info.forwarding_info.forwarding_feature_list.forwarding_feature[i].forwarded_to_uri != NULL)
            {
              ref_cnt_obj_add_ref(cmipapp_rpt_ptr->ip_ind.ind.register_ss_conf.sups_ss_info.info.forwarding_info.forwarding_feature_list.forwarding_feature[i].forwarded_to_uri);
            }
          }
        }
      }
      ...
      if((cmipapp_rpt_ptr->ip_ind.ind.register_ss_conf.is_ext_resp == TRUE) &&
         (cmipapp_rpt_ptr->ip_ind.ind.register_ss_conf.ext_resp.resp_type == ALL_CALL_FWD_INFO))
      {
        cmipapp_add_reference_allfwd_sups_error_text(&cmipapp_rpt_ptr->ip_ind.ind.register_ss_conf.ext_resp);
      }
    ...
    case CM_IP_SUPS_INTERROGATE_SS_CONF:
      ...
      if(cmipapp_rpt_ptr->ip_ind.ind.interrogate_ss_conf.interrogate_ss_res.forwarding_feature_list.present)
      {
        for(i=0;i<cmipapp_rpt_ptr->ip_ind.ind.interrogate_ss_conf.interrogate_ss_res.forwarding_feature_list.forwarding_feature_list_length;i++)
        {
          if(cmipapp_rpt_ptr->ip_ind.ind.interrogate_ss_conf.interrogate_ss_res.forwarding_feature_list.forwarding_feature[i].forwarded_to_uri != NULL)
          {
            ref_cnt_obj_add_ref(cmipapp_rpt_ptr->ip_ind.ind.interrogate_ss_conf.interrogate_ss_res.forwarding_feature_list.forwarding_feature[i].forwarded_to_uri);
          }
        }
      }
    ...
    case CM_IP_CALL_IND_CALL_END:
      ...
      /* CM should add reference for subject and picture URL received during CALL END IND as 
      ** IMS clears the reference after sending indication to CM. 
      */
      if (cmipapp_rpt_ptr->ip_ind.ind.call_end.call_composer_info.info_included)
      {
        if (cmipapp_rpt_ptr->ip_ind.ind.call_end.call_composer_info.subject &&
           cmipapp_rpt_ptr->ip_ind.ind.call_end.call_composer_info.subject->subject_length != 0 &&
           cmipapp_rpt_ptr->ip_ind.ind.call_end.call_composer_info.subject->subject_text
        )
        {
          CM_MSG_HIGH_1("call_composer: cmippap_add_ref :Added ref for subject, call_id %d",
                            cmipapp_rpt_ptr->ip_ind.ind.call_end.id);
          ref_cnt_obj_add_ref(
              cmipapp_rpt_ptr->ip_ind.ind.call_end.call_composer_info.subject);
        }

        if (cmipapp_rpt_ptr->ip_ind.ind.call_end.call_composer_info.picture_url &&
           cmipapp_rpt_ptr->ip_ind.ind.call_end.call_composer_info.picture_url->pic_url
        )
        {
          CM_MSG_HIGH_1("call_composer: cmippap_add_ref :Added ref for picture_url, call_id %d",
                           cmipapp_rpt_ptr->ip_ind.ind.call_end.id);
          ref_cnt_obj_add_ref(
              cmipapp_rpt_ptr->ip_ind.ind.call_end.call_composer_info.picture_url);
        }
        if (cmipapp_rpt_ptr->ip_ind.ind.call_end.call_composer_info.org_header &&
           cmipapp_rpt_ptr->ip_ind.ind.call_end.call_composer_info.org_header->org_header_length != 0 &&
           cmipapp_rpt_ptr->ip_ind.ind.call_end.call_composer_info.org_header->org_header_text
        )
        {
          CM_MSG_HIGH_1("call_composer: cmippap_add_ref :Added ref for org_header, call_id %d",
                            cmipapp_rpt_ptr->ip_ind.ind.call_end.id);
          ref_cnt_obj_add_ref(
              cmipapp_rpt_ptr->ip_ind.ind.call_end.call_composer_info.org_header);
        }
      }	  
    ...
    case CM_IP_SUPS_PASSWORD_CHG_CONF:
      if(cmipapp_rpt_ptr->ip_ind.ind.password_change_ss_conf_type.sups_ss_error_text != NULL)
      {
        ref_cnt_obj_add_ref(cmipapp_rpt_ptr->ip_ind.ind.password_change_ss_conf_type.sups_ss_error_text);
      }
      break;
    ...
    case CM_IP_CALL_SIP_DTMF_IND:
      if (cmipapp_rpt_ptr->ip_ind.ind.sip_dtmf_ind.dtmf_info &&
          cmipapp_rpt_ptr->ip_ind.ind.sip_dtmf_ind.dtmf_info->dtmf_string)
      {
        CM_MSG_HIGH_1("SIP_DTMF: cmippap_add_ref: Added ref for sip_dtmf_info, call_id %d",
                      cmipapp_rpt_ptr->ip_ind.ind.sip_dtmf_ind.call_id);
        ref_cnt_obj_add_ref(cmipapp_rpt_ptr->ip_ind.ind.sip_dtmf_ind.dtmf_info);
      }
...
)
{
  ...
  if(call_ptr!= NULL)
  {
    ...
    {
      CM_MSG_HIGH_2("Released ref for end reason, call_id %d, sub %d",
                       call_ptr->call_id,
...
)
{
  ...
  if(call_ptr!= NULL)
  {
    ...
    {
      CM_MSG_HIGH_2("Released ref for alternate_uri, call_id %d, sub %d",
                       call_ptr->call_id,
                       call_ptr->mm_id.asubs_id);
...
FUNCTION cmipapp_release_reference_allfwd_sups_error_text
...
===========================================================================*/
void cmipapp_release_reference_allfwd_sups_error_text
(
  const cmipsups_ext_resp_s_type  *ext_resp
)
{
  if (ext_resp != NULL)
  {
    if(ext_resp->resp_type == ALL_CALL_FWD_INFO)
    {
      if(ext_resp->info.all_call_fwd_ext_resp.cfu_sups_ss_error_text != NULL)
      { 
        CM_MSG_HIGH_0("Released ref for cfu_sups_ss_error_text");
              
        ref_cnt_obj_release(ext_resp->info.all_call_fwd_ext_resp.cfu_sups_ss_error_text);
      }
    
      if(ext_resp->info.all_call_fwd_ext_resp.cfb_sups_ss_error_text != NULL)
      { 
        CM_MSG_HIGH_0("Released ref for cfb_sups_ss_error_text");
              
        ref_cnt_obj_release(ext_resp->info.all_call_fwd_ext_resp.cfb_sups_ss_error_text);
      }
      
      if(ext_resp->info.all_call_fwd_ext_resp.cfnry_sups_ss_error_text != NULL)
      { 
        CM_MSG_HIGH_0("Released ref for cfnry_sups_ss_error_text");
              
        ref_cnt_obj_release(ext_resp->info.all_call_fwd_ext_resp.cfnry_sups_ss_error_text);
      }
    
      if(ext_resp->info.all_call_fwd_ext_resp.cfnrc_sups_ss_error_text != NULL)
      { 
        CM_MSG_HIGH_0("Released ref for cfnrc_sups_ss_error_text");
        ref_cnt_obj_release(ext_resp->info.all_call_fwd_ext_resp.cfnrc_sups_ss_error_text);
      }
    }
  }
  ...
  switch( cmipapp_rpt_ptr->hdr.cmd )
  {
    case CM_IP_SUPS_ERASE_SS_CONF:
      ...
      if((cmipapp_rpt_ptr->ip_ind.ind.erase_ss_conf.is_ext_resp == TRUE) &&
         (cmipapp_rpt_ptr->ip_ind.ind.erase_ss_conf.ext_resp.resp_type == ALL_CALL_FWD_INFO))
      {
        cmipapp_release_reference_allfwd_sups_error_text(&(cmipapp_rpt_ptr->ip_ind.ind.erase_ss_conf.ext_resp));
      }
    ...
    case  CM_IP_SUPS_ACTIVATE_SS_CONF:
      ...
      if((cmipapp_rpt_ptr->ip_ind.ind.activate_ss_conf.is_ext_resp == TRUE) &&
         (cmipapp_rpt_ptr->ip_ind.ind.activate_ss_conf.ext_resp.resp_type == ALL_CALL_FWD_INFO))
      {
        cmipapp_release_reference_allfwd_sups_error_text(&(cmipapp_rpt_ptr->ip_ind.ind.activate_ss_conf.ext_resp));
      }
    ...
    case CM_IP_SUPS_DEACTIVATE_SS_CONF:
      ...
      if((cmipapp_rpt_ptr->ip_ind.ind.deactivate_ss_conf.is_ext_resp == TRUE) &&
         (cmipapp_rpt_ptr->ip_ind.ind.deactivate_ss_conf.ext_resp.resp_type == ALL_CALL_FWD_INFO))
      {
        cmipapp_release_reference_allfwd_sups_error_text(&(cmipapp_rpt_ptr->ip_ind.ind.deactivate_ss_conf.ext_resp));
      }
    ...
    case CM_IP_SUPS_REGISTER_SS_CONF:
      ...
      if(cmipapp_rpt_ptr->ip_ind.ind.register_ss_conf.sups_ss_info.info.forwarding_info.present)
      {
        if(cmipapp_rpt_ptr->ip_ind.ind.register_ss_conf.sups_ss_info.info.forwarding_info.forwarding_feature_list.present)
        {
          for(i=0;i<cmipapp_rpt_ptr->ip_ind.ind.register_ss_conf.sups_ss_info.info.forwarding_info.forwarding_feature_list.forwarding_feature_list_length;i++)
          {
            if(cmipapp_rpt_ptr->ip_ind.ind.register_ss_conf.sups_ss_info.info.forwarding_info.forwarding_feature_list.forwarding_feature[i].forwarded_to_uri != NULL)
            {
              ref_cnt_obj_release(cmipapp_rpt_ptr->ip_ind.ind.register_ss_conf.sups_ss_info.info.forwarding_info.forwarding_feature_list.forwarding_feature[i].forwarded_to_uri);
            }
          }
        }
      }
      ...
      if((cmipapp_rpt_ptr->ip_ind.ind.register_ss_conf.is_ext_resp == TRUE) &&
         (cmipapp_rpt_ptr->ip_ind.ind.register_ss_conf.ext_resp.resp_type == ALL_CALL_FWD_INFO))
      {
        cmipapp_release_reference_allfwd_sups_error_text(&(cmipapp_rpt_ptr->ip_ind.ind.register_ss_conf.ext_resp));
      }
    ...
    case CM_IP_SUPS_INTERROGATE_SS_CONF:
      ...
      if(cmipapp_rpt_ptr->ip_ind.ind.interrogate_ss_conf.interrogate_ss_res.forwarding_feature_list.present)
      {
        for(i=0;i<cmipapp_rpt_ptr->ip_ind.ind.interrogate_ss_conf.interrogate_ss_res.forwarding_feature_list.forwarding_feature_list_length;i++)
        {
          if(cmipapp_rpt_ptr->ip_ind.ind.interrogate_ss_conf.interrogate_ss_res.forwarding_feature_list.forwarding_feature[i].forwarded_to_uri != NULL)
          {
            ref_cnt_obj_release(cmipapp_rpt_ptr->ip_ind.ind.interrogate_ss_conf.interrogate_ss_res.forwarding_feature_list.forwarding_feature[i].forwarded_to_uri);
          }
        }
      }
    ...
    case CM_IP_SUPS_PASSWORD_CHG_CONF:
      if(cmipapp_rpt_ptr->ip_ind.ind.password_change_ss_conf_type.sups_ss_error_text != NULL)
      {
        ref_cnt_obj_release(cmipapp_rpt_ptr->ip_ind.ind.password_change_ss_conf_type.sups_ss_error_text);
      }
      break;
...
/*lint -esym(715,data_block) */
{
  ...
  CM_MSG_HIGH_1("IMS_RPT: RPT name %d ", ip_rpt_name);
  ...
  */
  ...
  /* if report getting queued is not valid , error fatal */
  if(!cmtask_is_valid_report(ip_rpt_name))
  {
    sys_err_fatal_invalid_value_exception(ip_rpt_name);
  }
...
{
  #if defined(FEATURE_IP_CALL)
  return cm_call_id_allocate_new(FALSE);
  #else
...
)
{
  ...
  */
  ...
  if(!cmph_is_ims_capable_on_sub(ss_info_ptr->asubs_id))
  {
    return;
  }
  ...
  */
  if (cmss_is_stack1_operational(ss_info_ptr->asubs_id))
  {
  ...
  for (list_loop = 0; list_loop < CMIPAPP_POOL_SIZE; ++list_loop)
  {
    ...
    if ( app_id != CMIPAPP_ID_INVALID && sys_info_funcptr != NULL)
    {
      ...
      */
      ...
      {
        CM_MSG_HIGH_3("CM->IMS: sys_info, lte_srv_status %d, app_id %d, as_id %d",
...
*/
{
  ...
  cmipapp_inform_oprt_mode_sub_f_type  *oprt_mode_sub_funcptr = NULL;
  ...
  for (list_loop = 0; list_loop < CMIPAPP_POOL_SIZE; ++list_loop)
  {
    ...
    oprt_mode_sub_funcptr = cmipapp_arr[list_loop].app_cb.cmipapp_inform_oprt_mode_sub;
    app_id = cmipapp_arr[list_loop].app_id;
    oprt_mode_sent = cmipapp_arr[list_loop].oprt_mode_sent;
    user_data_block = cmipapp_arr[list_loop].user_data_block;
    cmd_common.asubs_id = cmipapp_arr[list_loop].asubs_id;

    if(app_id != CMIPAPP_ID_INVALID &&
        (oprt_mode_funcptr != NULL ||
         oprt_mode_sub_funcptr != NULL) &&
        (oprt_mode_sent != oprt_mode ||
         oprt_mode == SYS_OPRT_MODE_LPM) )
    {
      /* In dual-LTE, send oprt mode only to the apps with asubs_id matching
      ** the passed in asubs_id since deregistration needs to happen
      ** one sub at a time.
      */
      if(!cmph_is_dual_lte() ||
         cmd_common.asubs_id == asubs_id)
      {
        ...
        */
        ...
        /* Store the active trans id only if we are going to wait for cleanup. 
        ** Otherwise, if we are simply informing IMS of the oprt mode, do not 
        ** store the active trans id since we are not expecting any response.
        */
        if(wait_for_cleanup)
        {
    ...
    if(do_callback)
    {
      /* Call the app supplied function for service indication
      */
      if(oprt_mode_sub_funcptr != NULL)
      {
        is_cb_success = oprt_mode_sub_funcptr(oprt_mode,
                                              trans_id,
                                              wait_for_cleanup,
                                              user_data_block,
                                              cmd_common);
      ...
      CM_MSG_HIGH_6("CLNUP: CM->IMS: send oprt_mode %d is_cb_success %d, app_id %d sub %d trans_id %d wait_for_cleanup %d", \
                    oprt_mode, is_cb_success, app_id, cmd_common.asubs_id, trans_id, wait_for_cleanup );
    }
  }
...
*/
{
  ...
  */
  ...
  sys_modem_as_id_e_type      current_asubs_id;
  ...
  for (list_loop = 0; list_loop < CMIPAPP_POOL_SIZE; ++list_loop)
  {
    ...
    current_asubs_id = cmipapp_arr[list_loop].asubs_id;

    if(app_id != CMIPAPP_ID_INVALID &&
        srv_domain_funcptr &&
        current_srv_domain != srv_domain &&
        current_asubs_id == asubs_id)
    {
      /* Trans id for sending oprt mode
      */
      trans_id = cmipapp_next_trans_id();
    ...
    if(do_callback)
    {
      ...
      */
      if (!srv_domain_funcptr (srv_domain,
                               ...
                               cmd_common))
      {
        CM_ERR_3 ("CM->IMS: domain_chgd: Informing app id %d for srv_domain changed, srv dom %d sub %d",
                  app_id, srv_domain, asubs_id);

      }
      else
      {
        CM_MSG_HIGH_3_EXT ("CM->IMS: domain_chgd: callback successfull for app %d srv dom %d sub %d",
                           app_id,srv_domain, asubs_id, (asubs_id+1));
...
*/
{
  ...
  */
  if ( srv_domain != CM_SRV_DOMAIN_PREF_PS_DETACH  &&
       srv_domain != CM_PS_DETACH_BY_MMODE_TRIGGER &&
       srv_domain != CM_PS_DETACH_BY_MMODE_DDS_SWITCH &&
       srv_domain != CM_NO_PS_DETACH_BY_MMODE_DDS_SWITCH)
  {
    return FALSE;
  }
  ...
  */
  ...
  for (list_loop = 0; list_loop < CMIPAPP_POOL_SIZE; ++list_loop)
  {
    ...
    if (  ( app_id != CMIPAPP_ID_INVALID)
          ...
          &&
          (cmipapp_arr[list_loop].asubs_id == asubs_id)
       )
    {
      is_wait = TRUE;
      break;
  ...
  if(is_wait)
  {
    CM_MSG_HIGH_3 ("domain_chgd: App %d has cb registered for srv_domain_pref %d asubs_id %d",
                   app_id, srv_domain, asubs_id);
  }
...
===========================================================================*/
boolean cmipapp_is_srv_dom_ps_detach(sys_modem_as_id_e_type          asubs_id)
{
  ...
  for (list_loop = 0; list_loop < CMIPAPP_POOL_SIZE; ++list_loop)
  {
    ...
    if (   app_id != CMIPAPP_ID_INVALID
           ...
           (cmipapp_arr[list_loop].srv_domain_sent == CM_SRV_DOMAIN_PREF_PS_DETACH ||
            ...
            cmipapp_arr[list_loop].srv_domain_sent == CM_PS_DETACH_BY_MMODE_DDS_SWITCH)
       )
    {
      ret = TRUE;
      CM_MSG_HIGH_3 ("App %d srv_domain_pref sent %d, sub %d",
                        app_id,cmipapp_arr[list_loop].srv_domain_sent,
                        cmipapp_arr[list_loop].asubs_id);
      break;
    }
  }
  rex_leave_crit_sect(cmipapp_crit_sect_ptr());
...
*/
{
  ...
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  /* IMS performs deregistration only for LPM and PWR_OFF
  */
  ...
  for (list_loop = 0; list_loop < CMIPAPP_POOL_SIZE; ++list_loop)
  {
    ...
    if (  (app_id != CMIPAPP_ID_INVALID)
          ...
          (cmipapp_arr[list_loop].app_cb.cmipapp_inform_oprt_mode ||
           cmipapp_arr[list_loop].app_cb.cmipapp_inform_oprt_mode_sub)
          &&
          (cmipapp_arr[list_loop].oprt_mode_sent == oprt_mode)
          &&
          (cmipapp_arr[list_loop].active_trans_id != CMIPAPP_TRANS_ID_NONE)
  ...
  CM_MSG_HIGH_3("CLNUP: oprt_mode, is_wait %d, app id %d, oprt_mode %d",
                is_wait, app_id, oprt_mode);
...
/*lint -esym(715,trans_id) */
{
  ...
  if ( !(app_id < ARR_SIZE(cmipapp_arr)))
  {
    CM_ERR_1("CLNUP: oprt_mode_cnf, incorrect app id %d", app_id);
    return;
  }

  /*----------------------------------------------------------------------*/

  CM_MSG_HIGH_3 ("CLNUP: IMS->CM: App %d confirming oprt mode, trans_id %d, sub %d", 
                    app_id, trans_id,
                    cmipapp_arr[app_id].asubs_id);

  if (cmipapp_arr[app_id].active_trans_id != trans_id)
  {
    CM_ERR_3 ("CLNUP: IMS->CM: Incorrect trans_id %d from app %d, expected %d",
  ...
  /* Store the operating mode being confirmed
  */
  ...
  */
  for (list_loop = 0; list_loop < CMIPAPP_POOL_SIZE; ++list_loop)
  {
    ...
    {
      /* This app has not confirmed yet, CM needs to wait */
      slow_app_id = cmipapp_arr[list_loop].app_id;
    }
  }

  /* When cnf from all the apps have been received,
  ** loop to reset oprt_mode_sent for all the apps
  ** so that later when oprt mode is sent again, it should be sent
  ** to all IMS APPs
  */
  if(slow_app_id == CMIPAPP_ID_INVALID)
  {
    for (list_loop = 0; list_loop < CMIPAPP_POOL_SIZE; ++list_loop)
    {
      if(cmipapp_arr[list_loop].oprt_mode_sent == oprt_mode &&
         (!cmph_is_dual_lte() || 
          cmipapp_arr[list_loop].asubs_id == asubs_id))
      {
        cmipapp_arr[list_loop].oprt_mode_sent = SYS_OPRT_MODE_NONE;
      }
    }
  }
  rex_leave_crit_sect(cmipapp_crit_sect_ptr());
  ...
  if(slow_app_id != CMIPAPP_ID_INVALID)
  {
    CM_MSG_MED_1("CLNUP: IMS->CM: Wait for oprt_mode_cnf from App id %d", slow_app_id);
    return;
  }

  /*----------------------------------------------------------------------*/

  /* CM has received the confirmation from all the apps, so
  ** CM_IP_APP_ALLAPPS_OPRT_CNF is queued to CM
  */
  rpt_cmd_ptr = cm_sd_rpt_get_buf_else_err_fatal();
  rpt_cmd_ptr->hdr.cmd                       = CM_IP_APP_ALLAPPS_OPRT_CNF;

  CM_MSG_HIGH_0("CLNUP: IMS->CM: Sending CM_IP_APP_ALLAPPS_OPRT_CNF");
  cm_sd_rpt (rpt_cmd_ptr);
...
/*lint -esym(715,trans_id) */
{
  ...
  */
  ...
  CM_MSG_HIGH_2_EXT ("IMS->CM: App %d confirming srv domain, sub %d", 
                        app_id, 
                        ind_common.asubs_id,
                        (ind_common.asubs_id+1));
  ...
  */
  ...
  /*
  ** If there is another app registered with srv_domain func cb on the same sub,
  ** return if it has not been informed of the
  ...
  */
  for (list_loop = 0; list_loop < CMIPAPP_POOL_SIZE; ++list_loop)
  {
    ...
    if (  (cmipapp_arr[list_loop].app_id != CMIPAPP_ID_INVALID)
          ...
          &&
          (cmipapp_arr[list_loop].asubs_id == ind_common.asubs_id)
       )
    {
      slow_app_id = cmipapp_arr[list_loop].app_id;
    }
  ...
  /* When cnf from all the apps on the sub have been received,
  ** loop to reset srv_domain_sent for all the apps
  ** so that later when srv domain is sent again, it should be sent
  ** to all IMS APPs
  */
  if(slow_app_id == CMIPAPP_ID_INVALID)
  {
    for (list_loop = 0; list_loop < CMIPAPP_POOL_SIZE; ++list_loop)
    {
      if(cmipapp_arr[list_loop].srv_domain_sent == srv_domain &&
          cmipapp_arr[list_loop].asubs_id == ind_common.asubs_id)
      {
  ...
  */
  ...
  CM_MSG_HIGH_0 ("IMS->CM: Sending CM_IP_APP_ALLAPPS_SRV_DOM_CNF");
...
*/
{
  ...
  */
  ...
  for (list_loop = 0; list_loop < CMIPAPP_POOL_SIZE; ++list_loop)
  {
    rex_enter_crit_sect(cmipapp_crit_sect_ptr());
    app_id = cmipapp_arr[list_loop].app_id;
    sub_id = cmipapp_arr[list_loop].asubs_id;
    srvcc_funcptr = cmipapp_arr[list_loop].app_cb.cmipapp_srvcc_status_sub_func_ptr;
    user_data_block = cmipapp_arr[list_loop].user_data_block;
    rex_leave_crit_sect(cmipapp_crit_sect_ptr());

    if ( app_id != CMIPAPP_ID_INVALID && (as_id == sub_id) && srvcc_funcptr != NULL)
    {
      /* Call the app supplied function for service indication
      */
  ...
  for (list_loop = 0; list_loop < CMIPAPP_POOL_SIZE; ++list_loop)
  {
    ...
    if ( app_id != CMIPAPP_ID_INVALID && drvcc_funcptr != NULL)
    {
      ...
      */
      ...
      {
        CM_MSG_HIGH_3("drvcc callback successfull for app %d, notification %d, sub %d", 
                         app_id, drvcc_status,
                         as_id);
      }
    }
...
)
{
  ...
  CM_MSG_HIGH_6_EXT ("CM->IMS: CMIPAPP: Sending IP orig, sys_mode %d, app_id %d pi %d orig call_id %d, call_type %d, as_id %d",\
                     sys_mode, app_id, pi, call_id, call_type, cmd_common.asubs_id, (cmd_common.asubs_id+1));
...
)
{
  ...
  CM_MSG_MED_5("CM->IMS: CMIPAPP: Sending Invite response to app %d, for call_id %d, as_id %d, accept %d, answer_cause %d",
               app_id, call_id, cmd_common.asubs_id, accept, answer_cause);
...
)
{
  ...
  CM_MSG_MED_2 ("CMIPAPP: Send stop inband DTMF for call %d, sub %d", call_id, asubs_id);
...
FUNCTION cmipapp_send_sip_dtmf_cmd

DESCRIPTION
  Send SIP DTMF string to IMS.
...
===========================================================================*/
boolean cmipapp_send_sip_dtmf_cmd(
...
{
  cmipapp_cmd_sip_dtmf_sub_f_type *sub_fn_ptr = NULL;
  cmipapp_cmd_common_s_type cmd_common;
  cmipapp_s_type *cmipapp_ptr = cmipapp_get_app_ptr(app_id);
  ...
  cmd_common.asubs_id = asubs_id;
  sub_fn_ptr = cmipapp_ptr->app_cb.cmipapp_cmd_sip_dtmf_sub_func_ptr;

  if(sub_fn_ptr == NULL)
  {
    CM_MSG_HIGH_1("CMIPAPP: SIP_DTMF: CB is invalid for app %d", app_id);
    return FALSE;
  }

  CM_MSG_HIGH_2_EXT ("CM->IMS: CMIPAPP: SIP_DTMF: Send sip dtmf for call %d, as_id %d",
                call_id, cmd_common.asubs_id, cmd_common.asubs_id);

  return sub_fn_ptr(call_id, sip_dtmf_info, cmipapp_arr[app_id].user_data_block, cmd_common);
...
)
{
  ...
  CM_MSG_MED_4 ("CM->IMS: CMIPAPP: Send modify resp call %d, as_id %d rsp %d, call em_type %d",
...
/*===========================================================================
FUNCTION cmipapp_send_cancel_ims_call_cmd

DESCRIPTION
Sends cmd IMS to cancel upgrade/downgrade/cancelHold/cancelResume

RETURN VALUE
TRUE - Sent change call type cmd to IP App.
FALSE - Failed to send to IP App.
===========================================================================*/
boolean cmipapp_send_cancel_ims_call_req_cmd(

                                     cmipapp_id_type                app_id,

                                     const cm_call_id_type          call_id,

                                     cm_ip_call_cancel_ims_call_req_e_type  cancel_ims_req,
                                     /* cancel upgrade/downgrade/cancel hold/cancel resume */
...
{
  cmipapp_cmd_cancel_ims_call_req_sub_f_type *sub_fn_ptr = NULL;
  cmipapp_cmd_common_s_type cmd_common;
  cmipapp_s_type *cmipapp_ptr = cmipapp_get_app_ptr(app_id);
  ...
  cmd_common.asubs_id = asubs_id;
  sub_fn_ptr = cmipapp_ptr->app_cb.cmipapp_cmd_cancel_ims_call_req_sub_func_ptr;

  if(sub_fn_ptr == NULL)
  {
    CM_MSG_HIGH_1("CMIPAPP: Fail to send \
				  Cancel IMS call request(upgrade/downgrade/cancelHold/cancelResume) app %d", app_id);
    return FALSE;
  }

  CM_MSG_MED_3 ("CM->IMS:CMIPAPP:Send cmd to cancel upgrade/downgrade/Hold/Resume \
call id %d, cancel_type %d, as_id %d",
    call_id, cancel_ims_req, asubs_id);

  return sub_fn_ptr(call_id,cancel_ims_req,cmipapp_arr[app_id].user_data_block, cmd_common);
...
)
{
  ...
  CM_MSG_MED_4_EXT ("CM->IMS: CMIPAPP: Send PS sups cmd, type=%d, app_id %d, is_all_call_fwd_sups_ext_req %d, as_id %d ",
                sups_cmd->cmd_type, app_id, sups_cmd->is_all_call_fwd_sups_ext_req, asubs_id,asubs_id);
...
cmipapp_id_type
===========================================================================*/
cmipapp_id_type cmipapp_find_ics_support_app(
  sys_modem_as_id_e_type  asubs_id
)
{
  cmipapp_id_type   app_id = CMIPAPP_ID_INVALID;
  ...
  */
  ...
  if(app_id != CMIPAPP_ID_INVALID)
  {
    CM_MSG_HIGH_2("Found ICS app %d for asubs_id %d", app_id, asubs_id);
  }
  return app_id;
...
cmipapp_id_type
===========================================================================*/
cmipapp_id_type cmipapp_find_ussd_support_app(
  sys_modem_as_id_e_type  asubs_id
)
{
        ...
        && (cmipapp_arr[id_count].asubs_id == asubs_id))
    {
      ...
      for(cap_index=0; cap_index<CMIPAPP_CAP_ARR_MAX; cap_index++)
      {
            ...
            (cmipapp_is_ussd_supported_call_type(cmipapp_arr[id_count].app_capability.call_type[cap_index])))
        {
          ...
          CM_MSG_HIGH_3("Found USSD app %d over sys_mode %d, asubs_id %d",
                        app_id,
                        cmipapp_arr[id_count].app_capability.sys_mode[cap_index],
                        asubs_id);
          break;
        }
      }
...
)
{
  ...
  /* Check that the cmd requires IMS deregistration
  */
  ...
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  /* CM would have already got SUBSC_NOT_AVAIL/SUBSC_AVAILE before MCFG refresh and would
  ** have informed third party(THPY) IMS stack about the subsc change. No need to
  ** wait again for a CNF from THPY IMS during MCFG refresh as IMS would have
  ** cleaned up already during subsc_not_avail process. CM need not
  ** wait for CNF even in case of DS pref change as AP would know.
  */
  if(ph_sub_ptr->active_ims_stack == SYS_IMS_STACK_STATUS_ENABLED_THPY)
  {
    if(subs_cmd == CM_PH_CMD_SUBSCRIPTION_NOT_AVAILABLE)
    {
      CM_MSG_HIGH_0 ("CLNUP: EXT IMS enabled, wait for IMS subsc cnf");
      is_wait = TRUE;
    }
    return is_wait ;
  }
  ...
  else
  {
    ...
    CM_MSG_HIGH_3("CLNUP: subsc, is_wait %d, app id %d, subs_cmd %d",
                  is_wait, app_id, subs_cmd);
  }
...
)
{
  ...
  uint8                total_clnup_subs_mask = cmclnup_get_subs_id_bitmask();
  /* Mask of all subs currently being cleaned up */
  ...
  CM_MSG_HIGH_4 ("CLNUP: IMS->CM: App %d confirming subs cmd, trans_id %d, subs_id_bitmask 0x%x, sub %d",
  ...
  /* Store subs_cmd being confirmed
  */
  subs_cmd   = cmipapp_arr[app_id].subs_cmd;
  cmipapp_arr[app_id].active_trans_id = CMIPAPP_TRANS_ID_NONE;

  /* Check if CM is still waiting for any other apps that are registered for
  ** subscription cb to reply.
  */
  for (list_loop = 0; list_loop < CMIPAPP_POOL_SIZE; ++list_loop)
  {
    if (  (cmipapp_arr[list_loop].app_id != CMIPAPP_ID_INVALID)
          ...
          ((cmipapp_arr[list_loop].subs_cmd != subs_cmd) ||
           (cmipapp_arr[list_loop].active_trans_id != CMIPAPP_TRANS_ID_NONE))
          &&
          (BM(cmipapp_arr[list_loop].asubs_id) & total_clnup_subs_mask)
       )
    {
      /* This app has not confirmed yet, CM needs to wait */
      slow_app_id = cmipapp_arr[list_loop].app_id;
  ...
  if(slow_app_id != CMIPAPP_ID_INVALID)
  {
    CM_MSG_MED_1("CLNUP: IMS->CM: Wait for subs_cmd_cnf from App id %d", slow_app_id);
    return;
  }
  ...
  /* CM has received the confirmation from all the apps, so
  ** CM_IP_APP_ALLAPSS_SUBSC_CNF is qeued to CM.
  */
  memset(&ipapp_ind, 0, sizeof(cmipapp_ind_s_type));
  ipapp_ind.ind.subs_chng_cnf.subs_id_bitmask = total_clnup_subs_mask;

  CM_MSG_HIGH_0("CLNUP: IMS->CM: Sending CM_IP_APP_ALLAPSS_SUBSC_CNF");
...
)
{
  ...
  boolean is_cb_success = FALSE;
  cmipapp_id_type        app_id = CMIPAPP_ID_INVALID;
  uint8                   app_sub_bitmask = SYS_MODEM_AS_ID_NO_ACTIVE_MASK;
  cmipapp_trans_id_type  trans_id =0 ;
  uint32                 user_data_block;
  boolean                do_callback = FALSE;
  sys_modem_as_id_e_type asubs_id = ACTIVE_MASK_TO_AS_ID(subs_id_bitmask);
  cmph_sub_info_s_type *ph_sub_ptr = cmph_get_sub_ptr(asubs_id);
  ...
  if(ph_sub_ptr->active_ims_stack == SYS_IMS_STACK_STATUS_ENABLED_THPY &&
      (subs_id_bitmask & SYS_MODEM_AS_ID_ALL_ACTIVE_MASK))
  {
    cm_subsc_chgnd_ind_msg_type subsc_chgnd_ind;
    sys_modem_as_id_e_type       sub_idx = SYS_MODEM_AS_ID_NONE;

    for(sub_idx = SYS_MODEM_AS_ID_1; sub_idx < MAX_AS_IDS; sub_idx++)
    {
      if(BM(sub_idx) & subs_id_bitmask)
      {
        subsc_chgnd_ind.subs_cmd = subs_cmd;
        subsc_chgnd_ind.as_id = sub_idx;
        ...
        CM_MSG_HIGH_3("CLNUP: CM->MSGR: MM_CM_SUBSC_CHGND_IND, subs_cmd %d, asubs_id %d, subs_id_bitmask 0x%x",
                      subs_cmd, sub_idx, subs_id_bitmask);
      }
    }
  }
  ...
  else
  {
    for (list_loop = 0; list_loop < CMIPAPP_POOL_SIZE; ++list_loop)
    {
      ...
      app_id = cmipapp_arr[list_loop].app_id;
      user_data_block = cmipapp_arr[list_loop].user_data_block;
      app_sub_bitmask = BM(cmipapp_arr[list_loop].asubs_id);

      if(app_id != CMIPAPP_ID_INVALID &&
          subs_cmd_funcptr != NULL &&
          (app_sub_bitmask & subs_id_bitmask) &&
          cmipapp_arr[list_loop].active_trans_id == CMIPAPP_TRANS_ID_NONE)
      {
      ...
      if(do_callback)
      {
        /* Call the app supplied function
        */
        ...
        CM_MSG_HIGH_4("CLNUP: CM->IMS: send subs_cmd, is_cb_success %d, app_id %d, trans_id %d, sub bitmask %d",
                      is_cb_success, app_id, trans_id, app_sub_bitmask);
      } /* do_callback */
    }   /* for (...)   */
  }     /* ! THPY_IMS  */
...
boolean cmipapp_send_switch_speech_codec_cmd(
  ...
  uint32       switch_to_codec,
  /* destination number on which call is to be deflected */

  sys_speech_codec_e_type        priority_codec,
    /* Codec that is highest priority */
...
)
{
  ...
  CM_MSG_HIGH_4("CM->IMS: CMIPAPP: Send switch speech codec for call id %d, switch_to_codec %d, as_id %d pri_codec %d",
                call_id, switch_to_codec,asubs_id, priority_codec);

  switch_codec_params.call_id = call_id;
  switch_codec_params.switch_to_codec = switch_to_codec;
  switch_codec_params.priority_codec = priority_codec;

  return fn_ptr(switch_codec_params, cmipapp_arr[app_id].user_data_block, cmd_common);
}
/*===========================================================================

FUNCTION cmipapp_check_ims_reg_ect_cb

DESCRIPTION
Check if IMS has registered its callback for ECT or not.
...
boolean cmipapp_check_ims_reg_ect_cb (
	cmipapp_id_type 						app_id
)
...
)
{
  ...
  CM_MSG_HIGH_4 ("CM->IMS: CMIPAPP: Send IP ECT to app %d, as_id %d, ect_type %d, target_call_id %d",
                 app_id, asubs_id, ect_type, transfer_target_call_id);
...
FUNCTION cmipapp_notify_TIR_mode

DESCRIPTION
Api called by app to to set the ue_based_sups_mode.
...
void cmipapp_notify_tir_mode (
  sys_modem_as_id_e_type               asubs_id,
  uint8                              mode
)

{
  cm_ue_based_sups_mode_s_type ue_based_sups_mode;

  volte_sub_info_s_type *volte_sub_info_ptr = cmph_get_volte_sub_info_ptr(asubs_id);

  /*If TIR mode given by IMS is different from the current TIR mode stored, it will be updated here */

  if(mode != volte_sub_info_ptr->sups_mode[cmsups_map_code_to_index(colr)])
  {
      memset(&ue_based_sups_mode, 0, sizeof(cm_ue_based_sups_mode_s_type));
      volte_sub_info_ptr->sups_mode[cmsups_map_code_to_index(colr)] = mode;
    
    memscpy(ue_based_sups_mode.ue_based_sups_mode, sizeof(ue_based_sups_mode.ue_based_sups_mode), 
            volte_sub_info_ptr->sups_mode, sizeof(volte_sub_info_ptr->sups_mode));

    cmefs_fs_write(CMEFS_UE_BASED_SUPS_MODE,
                   (byte *)&(ue_based_sups_mode.ue_based_sups_mode),
                   sizeof(ue_based_sups_mode.ue_based_sups_mode),
                   asubs_id);
    CM_MSG_HIGH_1("IMS -> CM : cmipapp_notify_tir_mode() TIR mode = %d", volte_sub_info_ptr->sups_mode[cmsups_map_code_to_index(colr)]);
  }
...
FUNCTION cmipapp_get_TIR_mode

DESCRIPTION
Api called by app to to get the ue_based_sups_mode.
...
uint8 cmipapp_get_TIR_mode (
  sys_modem_as_id_e_type               asubs_id
)

{
  volte_sub_info_s_type *volte_sub_info_ptr = cmph_get_volte_sub_info_ptr(asubs_id);
  CM_MSG_HIGH_1("cmipapp_get_TIR_mode() TIR mode = %d", volte_sub_info_ptr->sups_mode[cmsups_map_code_to_index(colr)]);
  return volte_sub_info_ptr->sups_mode[cmsups_map_code_to_index(colr)];

}

/*===========================================================================

FUNCTION cmipapp_is_there_an_app_with_oprt_mode_cb_on_sub

DESCRIPTION
  Returns whether or not there is an app registered on the given sub with an
  oprt mode cb.

DEPENDENCIES
  This API should be used in dual-LTE configuration only.

===========================================================================*/
boolean cmipapp_is_there_an_app_with_oprt_mode_cb_on_sub (
...
)
{
  ...
  /* Search for an app registered on the given sub with oprt mode cb
  ** registered
  */
  rex_enter_crit_sect(cmipapp_crit_sect_ptr());
  ...
  for (list_loop = 0; list_loop < CMIPAPP_POOL_SIZE; ++list_loop)
  {
    if(cmipapp_arr[list_loop].asubs_id == asubs_id &&
       cmipapp_arr[list_loop].app_id != CMIPAPP_ID_INVALID &&
      (cmipapp_arr[list_loop].app_cb.cmipapp_inform_oprt_mode != NULL ||
       cmipapp_arr[list_loop].app_cb.cmipapp_inform_oprt_mode_sub != NULL))
    {
      ret = TRUE;
      break;
    }
