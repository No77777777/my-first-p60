...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/mmode/cm/src/cmxdbm.c#1 $
...
)
{
  ...
  if (cmph_is_msim())
  {
    asubs_id = dbm_cmd_ptr->asubs_id;
  }
  else
  {
    dbm_cmd_ptr->asubs_id = asubs_id = SYS_MODEM_AS_ID_1;
  }
  ph_sub_ptr = cmph_get_sub_ptr(asubs_id);

  if (ph_sub_ptr == NULL)
  {
    CM_ERR_1( "Invalid sub %d for DBM command",asubs_id);
  ...
  ss_info_sub_prop_ptr = cmss_get_info_sub_prop(asubs_id);
    ...
    case CM_DBM_CMD_SEND_MSG:
      ...
      */
      ...
      /* 655: bit-wise operation uses (compatible) enum's */
               ...
               cmcall_count_on_ss(cmph_map_sub_stk_to_id(asubs_id, MM_STACK_1)) != 0  &&
      ...
      else
      {
        if( ss_info_all_stack_0_ptr->sys_mode == SYS_SYS_MODE_NO_SRV &&
            ( ss_info_all_stack_1_ptr == NULL ||
              ss_info_all_stack_1_ptr->srv_status == SYS_SRV_STATUS_NO_SRV ))
        {
  ...
  */
  ...
  else
  {
    ...
    */
    ...
    info.asubs_id = dbm_cmd_ptr->asubs_id;

    cmph_update_ddtm_status( dbm_cmd_ptr->asubs_id );        /* update the DDTM satus */
...
static void cmdbm_forward_msg_to_mc(
  sys_modem_as_id_e_type asubs_id,
  cm_dbm_cmd_info_s_type  *msg,
  uint8 burst_type)
{
  mc_msg_type *mc_ptr;
...
static void cmdbm_forward_msg_to_hdrmc(
...
#define             CM_HDR_DOS_HEADER_SIZE 2
  ...
  */
  ...
  CM_MSG_HIGH_1("CM->MSGR: sub %d, send HDR_CP_SEND_DATA_OVER_SIGNALING_REQ", asubs_id);
...
)
{
  ...
  ss_info_sub_prop_ptr = cmss_get_info_sub_prop(asubs_id);
  if(ss_info_sub_prop_ptr == NULL || !IS_VALID_PH_SUB_PTR(asubs_id) ||
      ss_info_sub_prop_ptr->stack_info[MM_STACK_0].gen_info == NULL)
  {
    return;
  }
    ...
    switch (curr_dbm_obj.cm_dbm_cmd.cmd)
    {
      case CM_DBM_CMD_SEND_MSG:
        ...
        if( curr_dbm_obj.cm_dbm_cmd.burst_type == CAI_SHORT_DATA_BURST &&
            cmssidm_get_base_station_idm(asubs_id) == SYS_SYS_MODE_HDR )
        {
          CM_MSG_HIGH_0("HDRMC ready for DBM, send ...");
          cmdbm_forward_msg_to_hdrmc(asubs_id,&curr_dbm_obj.cm_dbm_cmd.info);
        }
        else
#endif /* FEATURE_HDR_DOS */
#endif /* CAI_SHORT_DATA_BURST */
          if(ss_info_all_stack_0_ptr->sys_mode != SYS_SYS_MODE_CDMA)
          {
                ...
                cmss_get_misc_sub_prop_ptr(asubs_id)->sub_info.gpp2_sub_info.is_hdr_session_open )
            {
...
#endif
              if (cmcall_overall_call_state_per_subs(asubs_id) == CM_OVERALL_CALL_STATE_IDLE)
              {
                ...
                CM_MSG_HIGH_1(">>PH FORCE_PREF: MC not ready for DBM, force ... sub %d", asubs_id);
              ...
              else
              {
                ...
                cmdbm_forward_msg_to_mc(asubs_id, &curr_dbm_obj.cm_dbm_cmd.info,
                                        curr_dbm_obj.cm_dbm_cmd.burst_type);
              }

        cmph_update_ddtm_status( asubs_id );       /* update DDTM status */
        break;
...
)
{
  ...
  */
  ...
  cmdbm_client_cmd_proc_next(dbm_cmd_ptr->asubs_id);
}
          ...
          CAI_SHORT_DATA_BURST == cm_mc_rpt_ptr->mo_dbm_status.msg_type )
      {
        ...
        else
        {
              ...
              (curr_dbm_obj.is_cmd_sent_to_MC == CMDBM_MSG_SENT))
          {
            ...
            info.asubs_id = cm_mc_rpt_ptr->hdr.asubs_id;
...
#if (defined(FEATURE_MMODE_CDMA_800) || defined(FEATURE_MMODE_CDMA_1900))
              cmph_mo_sms_end(cm_mc_rpt_ptr->hdr.asubs_id, (dword)0, (cm_act_id_type)&curr_dbm_obj );
#endif
            }
            else
            {
              ...
              ** without timing out.
              */
              cmph_update_orig_mode_uptime(cmph_map_sub_stk_to_id(cm_mc_rpt_ptr->hdr.asubs_id,MM_STACK_0),
                                           (cm_act_id_type) &curr_dbm_obj );
          ...
          */
          cmdbm_client_cmd_proc_next(cm_mc_rpt_ptr->hdr.asubs_id);
    ...
    case CM_OK_TO_ORIG_F:
          ...
          && curr_dbm_obj.is_cmd_sent_to_MC != CMDBM_MSG_SENT)
      {
        ...
        if( sd_rpt_ptr->ok_to_orig.si_ok_to_orig.mode == SD_MODE_HDR )
        {
          ...
          cmdbm_forward_msg_to_hdrmc( sd_rpt_ptr->hdr.asubs_id,
                                      &curr_dbm_obj.cm_dbm_cmd.info);

#endif /* FEATURE_HDR_DOS */
        }
        else
#ifdef FEATURE_DEDICATED_C2K_MODE
          if( cmss_get_misc_sub_prop_ptr(sd_rpt_ptr->hdr.asubs_id) != NULL &&
              !cmss_get_misc_sub_prop_ptr(sd_rpt_ptr->hdr.asubs_id)->sub_info.gpp2_sub_info.is_hdr_session_open )
#endif
          {
            cmdbm_forward_msg_to_mc( sd_rpt_ptr->hdr.asubs_id, &curr_dbm_obj.cm_dbm_cmd.info,
                                     curr_dbm_obj.cm_dbm_cmd.burst_type );
          }
  ...
  */
      ...
      curr_scan_dbm->cm_dbm_cmd.l2_ack_wait_time <= time_get_uptime_secs())
  {
    ...
    */
    ...
    info.asubs_id = curr_scan_dbm->cm_dbm_cmd.asubs_id;

    cmph_update_ddtm_status( curr_scan_dbm->cm_dbm_cmd.asubs_id );       /* update the DDTM satus */
    cmdbm_event(&info, CM_DBM_EVENT_MO_STATUS);
...
/*lint -esym(715,timer_event) */
{
  ...
  */
      ...
      curr_dbm_obj.cm_dbm_cmd.l2_ack_wait_time <= time_get_uptime_secs())
  {
    ...
    info.asubs_id = curr_dbm_obj.cm_dbm_cmd.asubs_id;
    ...
    */
    ...
    cmph_update_ddtm_status( curr_dbm_obj.cm_dbm_cmd.asubs_id );        /* update the DDTM satus */
    cmdbm_event(&info, CM_DBM_EVENT_MO_STATUS);
    ...
    if (is_pref_forced)
    {
      if (cmdbm_get_queue_count(cmdbm_client_dbm_q) <= 0)
      {
        ...
        *------------------------------------------*/
        CM_MSG_LOW_0("Unforce mode after DBM msg sent ...");
#if (defined(FEATURE_MMODE_CDMA_800) || defined(FEATURE_MMODE_CDMA_1900))
        cmph_mo_sms_end( curr_dbm_obj.cm_dbm_cmd.asubs_id, (dword)0, (cm_act_id_type)&curr_dbm_obj );
#endif
      }
    }
    cmdbm_client_cmd_proc_next(curr_dbm_obj.cm_dbm_cmd.asubs_id);
  }
}
/*lint +esym(715,timer_event) */
...
===========================================================================*/
boolean cmxdbm_is_mo_dbm_in_progress(sys_modem_as_id_e_type asubs_id)
{
