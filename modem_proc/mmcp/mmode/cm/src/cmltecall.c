...
/*===========================================================================
 ...
 $Header: //components/rel/mmcp.mpss/7.9.0/mmode/cm/src/cmltecall.c#1 $
...
)
{
  ...
  call_info_ptr->asubs_id = call_ptr->mm_id.asubs_id;
  ...
  switch( call_ptr->cmcall_mode_info.info_type )
  {
    case CM_CALL_MODE_INFO_LTE:
      ...
      *(&(lte_info_mode_ptr->sdf_qos)) = *(&(lte_mode_ptr->sdf_qos));
      ...
      lte_info_mode_ptr->esm_local_cause = lte_mode_ptr->esm_local_cause;
      ...
      */
      ...
      lte_info_mode_ptr->backoff_timer.valid= \
        lte_mode_ptr->backoff_timer.valid;
...
)
{
  ...
  memset(&lte_mode_ptr->esm_cause, 0, sizeof(lte_mode_ptr->esm_cause));
  memset(&lte_mode_ptr->esm_local_cause, 0, sizeof(lte_mode_ptr->esm_local_cause));
  memset(&lte_mode_ptr->emm_failure_cause, 0, sizeof(lte_mode_ptr->emm_failure_cause));
  ...
  switch( cm_rpt_ptr->msg_hdr.id )
  {
    ...
    case MM_CM_DEACT_BEARER_CONTEXT_REQUEST_IND:
      ...
      CM_MSG_MED_4("MM_CM_DEACT_BEARER_CONTEXT_REQUEST sub %d rpt eps_bearer_id %d, connection_id %d, pco valid %d",
                   asubs_id,
      ...
      lte_mode_ptr->backoff_timer.active        = cm_rpt_ptr->cm_deact_bearer_context_request_ind.backoff_timer.active;
      lte_mode_ptr->backoff_timer.timer_count   = cm_rpt_ptr->cm_deact_bearer_context_request_ind.backoff_timer.timer_count;
      lte_mode_ptr->backoff_timer.valid         = cm_rpt_ptr->cm_deact_bearer_context_request_ind.backoff_timer.valid;
      lte_mode_ptr->eps_bearer_id               = cm_rpt_ptr->cm_deact_bearer_context_request_ind.eps_bearer_id;
      lte_mode_ptr->connection_id               = cm_rpt_ptr->cm_deact_bearer_context_request_ind.esm_hdr.connection_id;
      lte_mode_ptr->sdf_id                      = cm_rpt_ptr->cm_deact_bearer_context_request_ind.esm_hdr.sdf_id;
      lte_mode_ptr->esm_cause.valid             = cm_rpt_ptr->cm_deact_bearer_context_request_ind.esm_cause.valid;
      ...
      lte_mode_ptr->esm_local_cause = cm_rpt_ptr->cm_deact_bearer_context_request_ind.esm_local_cause;
    ...
    case MM_CM_ACT_DRB_RELEASED_IND:
      ...
      CM_MSG_HIGH_1("MM_CM_ACT_DRB_RELEASED_IND sub=%d", asubs_id);

      lte_mode_ptr->esm_cause        =
        cm_rpt_ptr->cm_act_drb_released_ind.esm_cause;

      lte_mode_ptr->esm_local_cause        =
        cm_rpt_ptr->cm_act_drb_released_ind.esm_local_cause;
    ...
    case MM_CM_DRB_SETUP_IND:
      ...
      CM_MSG_MED_4("M_CM_DRB_SETUP_IND sub=%d rpt eps_bearer_id %d, connection_id %d, rb_id %d",
                   asubs_id,
    ...
    case MM_CM_DRB_REESTABLISH_FAILED_IND:
      ...
      CM_MSG_MED_6("DRB_REESTAB_FAIL: sub %d eps_bearer_id %d conn_id %d call_id %d eps_bearer_id %d bearer type %d",
                   asubs_id,
      ...
      lte_mode_ptr->esm_local_cause = cm_rpt_ptr->cm_drb_reestablish_failed_ind.esm_local_cause;

      /* copy emm cause code*/
      lte_mode_ptr->emm_failure_cause.cause_type =
        cm_rpt_ptr->cm_drb_reestablish_failed_ind.emm_failure_cause.cause_type;
      memscpy(&(lte_mode_ptr->emm_failure_cause.detailed_cause),sizeof(uint16),
              &(cm_rpt_ptr->cm_drb_reestablish_failed_ind.emm_failure_cause.cause),
              sizeof(cm_rpt_ptr->cm_drb_reestablish_failed_ind.emm_failure_cause.cause));
      lte_mode_ptr->emm_failure_cause.access_barring_rem_time =
        cm_rpt_ptr->cm_drb_reestablish_failed_ind.emm_failure_cause.access_barring_rem_time;
      lte_mode_ptr->emm_failure_cause.deprior_started =
        cm_rpt_ptr->cm_drb_reestablish_failed_ind.emm_failure_cause.deprior_started;
      CM_MSG_MED_4("emm cause %d, detailed cause %d, acc_barr_time %d, depri %d",
                   lte_mode_ptr->emm_failure_cause.cause_type,
    ...
    case MM_CM_DRB_REESTABLISH_REJECT_IND:
      ...
      CM_MSG_MED_6("DRB_REESTAB_REJ: sub %d eps_bearer_id %d conn_id %d call_id %d eps_bearer_id %d bearer_type %d",
                   asubs_id,
      ...
      lte_mode_ptr->esm_local_cause = cm_rpt_ptr->cm_drb_reestablish_reject_ind.esm_local_cause;
      ...
      lte_mode_ptr->emm_failure_cause.deprior_started =
        cm_rpt_ptr->cm_drb_reestablish_reject_ind.emm_failure_cause.deprior_started;
      CM_MSG_MED_4("emm cause %d, detailed cause %d, acc_barr_time %d, depri %d",
                   lte_mode_ptr->emm_failure_cause.cause_type,
                   lte_mode_ptr->emm_failure_cause.detailed_cause,
                   lte_mode_ptr->emm_failure_cause.access_barring_rem_time,
    ...
    case MM_CM_MODIFY_BEARER_CONTEXT_REQUEST_IND:
      ...
      CM_MSG_MED_6("MODIFY_BEARER_REQ: sub %d eps_bearer_id %d conn_id %d call_id %d eps_bearer_id %d bearer_type %d",
                   asubs_id,
  ...
  switch (call_event)
  {
    ...
    case CM_CALL_EVENT_PDN_CONN_REJ_IND:
      ...
      call_info_ptr->mode_info.info.lte_call.esm_local_cause = \
          cm_rpt_ptr->cm_pdn_connectivity_reject_ind.esm_local_cause;
      ...
      call_info_ptr->mode_info.info.lte_call.emm_failure_cause.deprior_started=
        cm_rpt_ptr->cm_pdn_connectivity_reject_ind.emm_failure_cause.deprior_started;
      CM_MSG_MED_4("emm cause %d, detailed cause %d, acc_barr_time %d, depri %d",
                   call_info_ptr->mode_info.info.lte_call.emm_failure_cause.cause_type,
      ...
      call_info_ptr->mode_info.info.lte_call.backoff_timer.valid = \
          cm_rpt_ptr->cm_pdn_connectivity_reject_ind.backoff_timer.valid;

      call_info_ptr->mode_info.info.lte_call.reattempt_indicator.valid  = \
          cm_rpt_ptr->cm_pdn_connectivity_reject_ind.reattempt_indicator.valid;
      call_info_ptr->mode_info.info.lte_call.reattempt_indicator.ratc_val  = \
          cm_rpt_ptr->cm_pdn_connectivity_reject_ind.reattempt_indicator.ratc_val;
    ...
    case CM_CALL_EVENT_PDN_DISCONNECT_REJECT_IND:
      ...
      call_info_ptr->mode_info.info.lte_call.esm_local_cause = \
          cm_rpt_ptr->cm_pdn_disconnect_reject_ind.esm_local_cause;
    ...
    case CM_CALL_EVENT_RES_ALLOC_REJ_IND:
      ...
      call_info_ptr->mode_info.info.lte_call.esm_local_cause = \
          cm_rpt_ptr->cm_bearer_resource_alloc_reject_ind.esm_local_cause;
      ...
      call_info_ptr->mode_info.info.lte_call.backoff_timer.valid = \
          cm_rpt_ptr->cm_bearer_resource_alloc_reject_ind.backoff_timer.valid;

      call_info_ptr->mode_info.info.lte_call.reattempt_indicator.valid  = \
          cm_rpt_ptr->cm_bearer_resource_alloc_reject_ind.reattempt_indicator.valid;
      call_info_ptr->mode_info.info.lte_call.reattempt_indicator.ratc_val  = \
          cm_rpt_ptr->cm_bearer_resource_alloc_reject_ind.reattempt_indicator.ratc_val;
    ...
    case CM_CALL_EVENT_BEARER_MOD_REJ_IND:
      ...
      call_info_ptr->mode_info.info.lte_call.esm_local_cause = \
          cm_rpt_ptr->cm_bearer_context_modify_reject_ind.esm_local_cause;
      
      call_info_ptr->mode_info.info.lte_call.reattempt_indicator.valid  = \
          cm_rpt_ptr->cm_bearer_context_modify_reject_ind.reattempt_indicator.valid;
      call_info_ptr->mode_info.info.lte_call.reattempt_indicator.ratc_val  = \
          cm_rpt_ptr->cm_bearer_context_modify_reject_ind.reattempt_indicator.ratc_val;
  ...
  */
    ...
    /* Send NAS_ESM_ACT_DEFAULT_BEARER_CONTEXT_REJ_IND to ESM */
  {
        ...
        CM_LTE_BEARER_TYPE_DEFAULT)
    {
      ...
      act_bearer_context_rsp->esm_act_default_bearer_context_rej.\
      esm_local_cause = lte_info->esm_local_cause;
...
)
{
  ...
  */
  ...
  (void) cm_msgr_send( NAS_ESM_DRB_REESTABLISH_REQ, MSGR_MM_CM,
                       ...
                       call_ptr->cmcall_mode_info.info.lte_info.as_id);
...
)
{
  ...
  */
  ...
  CM_MSG_MED_2("CM->NAS: sub %d, Send LTE callcmd %d",
               call_cmd_ptr->info.lte_info.as_id, call_cmd_ptr->cmd);
  switch( call_cmd_ptr->cmd )
  {
    ...
    case CM_CALL_CMD_PDN_CONNECTIVITY_ABORT_REQ:
      ...
      client_request->esm_pdn_connectivity_abort_req.esm_local_cause =
        call_cmd_ptr->info.lte_info.esm_local_cause;
    ...
    case CM_CALL_CMD_PDN_DISCONNECT_REQ:
      ...
      client_request->esm_pdn_disconnect_req.esm_local_cause =
        call_cmd_ptr->info.lte_info.esm_local_cause;
    ...
    case CM_CALL_CMD_RES_ALLOC_ABORT_REQ:
      ...
      client_request->esm_bearer_resource_alloc_abort_req.esm_local_cause =
        call_cmd_ptr->info.lte_info.esm_local_cause;
    ...
    case CM_CALL_CMD_BEARER_RES_MOD_REQ:
      ...
      *(&(client_request->esm_bearer_resource_modification_req.sdf_qos)) = \
      ...
      client_request->esm_bearer_resource_modification_req.esm_local_cause = \
          call_cmd_ptr->info.lte_info.esm_local_cause;
    ...
    case CM_CALL_CMD_MOD_BEARER_RSP:
      ...
      else
      {
        ...
        CM_MSG_MED_1("CM->MSGR: SEND: NAS_ESM_MODIFY_BEARER_CONTEXT_REJ_IND as_id %d",
                     call_cmd_ptr->info.lte_info.as_id);
        (void) cm_msgr_send( NAS_ESM_MODIFY_BEARER_CONTEXT_REJ_IND, MSGR_MM_CM,
                             (msgr_hdr_s *)client_request, sizeof(esm_msg),
                             call_cmd_ptr->info.lte_info.as_id);
    ...
    default:
      CM_ERR_1( "CM->NAS: LTE callcmd %d", (int)(call_cmd_ptr->cmd));
  } /* end of switch */
...
)
{
  ...
  */
  ...
  if (cmph_ptr()->device_prop.msim_prop.dds_status == CMPH_DDS_STATUS_REMOVE_DATA_OBJ)
  {
    ...
    cmph_ptr()->device_prop.msim_prop.dds_status = CMPH_DDS_STATUS_NONE;
  ...
  if( cmcall_is_no_call_in_gw_ps_per_subs(mm_id.asubs_id) && cmcall_is_no_call_in_lte_ps() )
  {
  ...
  */
  ...
  if(lte_mode_ptr->gsm_umts_connection_id.valid)
  {
    if (!cmcall_unreserve_sm_connection_id(
          lte_mode_ptr->gsm_umts_connection_id.connection_id, mm_id.asubs_id ))
    {
...
)
{
  ...
  */
  ...
  CM_MSG_HIGH_3_EXT("lt cc cb: as_id=%d, ps_cc_sys_mode=%d, call_control_result=%d",
                    ...
                    (gw_call_control_info_ptr->lte_ps_cc_params.as_id+1));
...
#endif
  multimode_stack_e_type  lte_stack_id;
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  */
  ...
  switch( cm_rpt_ptr->msg_hdr.id )
  {
    ...
    case MM_CM_ACT_DEDICATED_BEARER_CONTEXT_REQUEST_IND:
      ...
      /* Do not process the report if LPM is ongoing
      */
      if(cm_reply_check (&(cmph_ptr()->device_prop.reply), CM_LPM_OK_F) && cmph_ptr()->device_prop.ph_oprt_mode.is_oprt_mode_change_ongoing)
      {
        CM_MSG_HIGH_1("Ignoring rpt %d as LPM is ongoing",cm_rpt_ptr->msg_hdr.id);
        return;
      }
          ...
          MM_CM_ACT_DEFAULT_BEARER_CONTEXT_REQUEST_IND)
      {
        ...
        CM_MSG_HIGH_4("NAS->CM: ACT DEFAULT BEARER req call_id=%d, esm conn_id=%d, sm_conn_id=%d, sub=%d",
          call_id,
          cm_rpt_ptr->cm_act_default_bearer_context_request_ind.esm_hdr.connection_id,
          cm_rpt_ptr->cm_act_default_bearer_context_request_ind.gsm_umts_connection_id.connection_id,
          asubs_id);
      ...
      else
      {
        ...
        CM_MSG_HIGH_4("NAS->CM: ACT DEDICATED BEARER req call_id=%d, esm conn_id=%d, sm_conn_id=%d, sub=%d",
          call_id,
          cm_rpt_ptr->cm_act_dedicated_bearer_context_request_ind.esm_hdr.connection_id,
          cm_rpt_ptr->cm_act_dedicated_bearer_context_request_ind.gsm_umts_connection_id.connection_id,
          asubs_id);
      ...
      if (is_found)
      {
        ...
        if( call_ptr == NULL )
        {
          CM_ERR_1( "NAS->CM: Call id %d exists but No call object", call_id);
          break;
        }

        /*Update call_ptr */
        cmltecall_update_call_ptr( call_ptr, cm_rpt_ptr, asubs_id);
      ...
      else
      {
        ...
        if( !cm_new_incom_call_id ( CM_CALL_TYPE_PS_DATA, &call_id,
                                    ...
                                    esm_hdr.connection_id) )
        {
          CM_ERR_0( "NAS->CM: BEARER_CONTEXT_REQUEST_IND: ERROR: No call id for BEARER_CONTEXT_REQUEST_IND");
          break;
        }
        ...
        if( call_ptr == NULL )
        {
          CM_ERR_1( "NAS->CM: BEARER_CONTEXT_REQUEST_IND, No call object for call_id=%d exists", call_id);
          break;
        }
    ...
    case MM_CM_DEACT_BEARER_CONTEXT_REQUEST_IND:
      ...
      CM_MSG_MED_4("NAS->CM: RXD: DEACT_BEARER_CONTEXT_REQUEST_IND: connection_id=%d, call_id=%d, sub=%d, is_found=%d",
        cm_rpt_ptr->cm_deact_bearer_context_request_ind.esm_hdr.connection_id,
        call_id,
        asubs_id,
        is_found);
    ...
    case MM_CM_ACT_DRB_RELEASED_IND:
      ...
      CM_MSG_MED_4("NAS->CM: RXD: CM_ACT_DRB_RELEASED_IND: connection_id=%d, call_id=%d, sub=%d, is_found=%d",
        cm_rpt_ptr->cm_act_drb_released_ind.esm_hdr.connection_id,
        call_id,
        asubs_id,
        is_found);
    ...
    case MM_CM_DRB_SETUP_IND:
      ...
      CM_MSG_MED_4("NAS->CM: RXD: CM_DRB_SETUP_IND: connection_id=%d, call_id=%d, sub=%d, is_found=%d",
                      cm_rpt_ptr->cm_drb_setup_ind.esm_hdr.connection_id,
                      call_id,
                      asubs_id,
                      is_found);
    ...
    case MM_CM_DRB_REESTABLISH_FAILED_IND:
      ...
      CM_MSG_MED_4("NAS->CM: RXD: CM_DRB_REESTABLISH_FAILED_IND: connection_id=%d, call_id=%d, sub=%d, is_found=%d",
                      cm_rpt_ptr->cm_drb_reestablish_failed_ind.esm_hdr.connection_id,
                      call_id,
                      asubs_id,
                      is_found);
    ...
    case MM_CM_DRB_REESTABLISH_REJECT_IND:
      ...
      CM_MSG_MED_4("NAS->CM: RXD: CM_DRB_REESTABLISH_REJECT_IND: connection_id=%d, call_id=%d, sub=%d, is_found=%d",
                      cm_rpt_ptr->cm_drb_reestablish_reject_ind.esm_hdr.connection_id,
                      call_id,
                      asubs_id,
                      is_found);
    ...
    case MM_CM_MODIFY_BEARER_CONTEXT_REQUEST_IND:
      ...
      */
      ...
      {
        CM_ERR_0( "NAS->CM: No call id for MM_CM_MODIFY_BEARER_CONTEXT_REQUEST_IND");
      }
    ...
    case MM_CM_PDN_CONNECTIVITY_PACKED_REQUEST_IND:
    {
      ...
      */
      ...
      /* as_id */
      lte_ps_cc_params_ptr->as_id = asubs_id;

      CM_MSG_MED_2("NAS->CM: RXD: PDN_CONNECITIVITY_PACKED_REQUEST_IND, asubs_id=%d, pti=%d",
      ...
      *(&(lte_ps_cc_params_ptr->apn_name)) = *(&(cm_rpt_ptr->cm_pdn_connectivity_packed_request_ind.apn_name));
      ...
      lte_stack_id = cmph_determine_stk_per_rat(lte_ps_cc_params_ptr->as_id,
                     SYS_SYS_MODE_LTE);
      cdma_subscription_source = cmph_get_cdma_subscription_source(lte_ps_cc_params_ptr->as_id);
      gwl_subscription_source = cmph_get_gw_subscription_source(lte_ps_cc_params_ptr->as_id);

      CM_MSG_MED_3("map to stack=%d, cdma_subscription_source=%d,gwl_subscription_source=%d",
                   lte_stack_id, cdma_subscription_source, gwl_subscription_source);
    ...
    */
    ...
    {
      CM_MSG_HIGH_2("NAS->CM: RXD: MM_CM_ESM_NOTIFICATION_IND %d, sub %d",
          ...
          && cm_get_srvcc_ho_error_state() != CM_SRVCC_HO_UNMANAGED_CALL_COMBINATION  )
      {
        while(call_ptr != NULL)
        {
          ...
          else if ( call_ptr->is_srvcc_call && cmwcall_is_int_ims())
          {
            ...
            else
            {
              ...
              {
                CM_MSG_HIGH_1("cmltecall_rpt_proc: setting sr orig_sent_on_mode to LTE for call %d",
                              call_ptr->call_id);
        ...
        {
          cmwcall_send_srvcc_events_to_clients(CM_SRVCC_HO_IND_CANCEL,CM_CALL_ID_INVALID,ims_call_id, asubs_id);
        ...
        else
        {
          ...
          cmipapp_send_srvcc_notification(CMIPAPP_SRVCC_HO_CANCEL_NOTIFY, asubs_id);
    ...
    default:
      CM_ERR_1("RXD: Wrong msg_id %d", cm_rpt_ptr->msg_hdr.id);
      break;
...
)
{
  ...
  if (cmph_is_ssim())
  {
    call_cmd_ptr->cmd_subs = call_cmd_ptr->info.lte_info.as_id = SYS_MODEM_AS_ID_1;
  }
  else if (!cmutil_is_as_id_valid(call_cmd_ptr->cmd_subs))
  {
    CM_MSG_HIGH_2( "LTE_CALL: cmd %d, cmd_subs %d invalid, default to sub 0",
                           call_cmd_ptr->cmd,
  ...
  ph_sub_ptr = cmph_get_sub_ptr(call_cmd_ptr->cmd_subs);
  ...
  if( call_cmd_ptr->cmd == CM_CALL_CMD_PDN_CONNECTIVITY_REQ ||
  ...
  )
  {
    ...
    else if(!cmph_is_lte_capable_on_sub(call_cmd_ptr->cmd_subs))
    {
      cmd_err = CM_CALL_CMD_ERR_OP_NOT_SUPPORTED_ON_SUBS;
    }
    else if(call_cmd_ptr->cmd == CM_CALL_CMD_PDN_CONNECTIVITY_REQ && 
             (cmregprx_is_stop_mode_dual_switch(call_cmd_ptr->cmd_subs, SYS_SYS_MODE_LTE)
             || ((ph_ptr->sub_info[call_cmd_ptr->cmd_subs] != NULL) 
                 && ph_ptr->sub_info[call_cmd_ptr->cmd_subs]->ps_detach_in_progress)
             )
            )
    {
      cmd_err = CM_CALL_CMD_ERR_DUAL_SWITCH;
    }
    else if(call_cmd_ptr->cmd == CM_CALL_CMD_PDN_CONNECTIVITY_REQ && 
            !cmregprx_is_rat_in_rat_priority_on_subs(call_cmd_ptr->cmd_subs, SYS_SYS_MODE_LTE))
    {
      cmd_err = CM_CALL_CMD_ERR_OP_NOT_SUPPORTED_ON_SUBS;
    }
    else if (call_cmd_ptr->cmd == CM_CALL_CMD_PDN_CONNECTIVITY_REQ &&
       ph_sub_ptr != NULL && ph_sub_ptr->reject_ps_activity)
    {
      cmd_err = CM_CALL_CMD_ERR_MPLMN_IN_PROGRESS;      
    }
  }

  if (cmd_err != CM_CALL_CMD_ERR_NOERR)
  {
    CM_MSG_HIGH_6_ASID( "LTE_CALL: rejected cmd %d, max_cap 0x%x, lte_in_rat_priority %d, oprt_mode %d, cmd_err %d",
                         call_cmd_ptr->cmd_subs, 
                         call_cmd_ptr->cmd,
                         cmmsc_get_curr_msc_max_cap(call_cmd_ptr->cmd_subs),
                         cmregprx_is_rat_in_rat_priority_on_subs(call_cmd_ptr->cmd_subs, SYS_SYS_MODE_LTE),
...
/*lint -esym(818, cmd_ptr) */
{
  ...
  switch( call_cmd_ptr->cmd )
  {
    case CM_CALL_CMD_ACT_BEARER_RSP:
      ...
      CM_MSG_MED_5("DS->CM: ACT_BEARER_RSP: eps_bearer_id in DS cmd is %d eps_bearer_id in call_id %d is %d, lte_info.as_id %d, call asubs_id %d",
                   call_cmd_ptr->info.lte_info.eps_bearer_id,
                   call_ptr->call_id,
                   call_ptr->cmcall_mode_info.info.lte_info.eps_bearer_id,
                   call_cmd_ptr->info.lte_info.as_id,
      ...
      cmcall_end_stale_gw_ps_calls(call_ptr->mm_id);
    ...
    case CM_CALL_CMD_RAB_REESTAB:
      ...
      CM_MSG_MED_4("DS->CM: RAB_REESTAB: call_id=%d, connection_id=%d, bearer_id=%d, asubs_id %d",
      ...
      call_ptr->cmcall_mode_info.info.lte_info.as_id = call_ptr->mm_id.asubs_id;
    ...
    case CM_CALL_CMD_MOD_BEARER_RSP:
      ...
      if (call_ptr->mm_id.asubs_id != call_cmd_ptr->info.lte_info.as_id)
      {
        CM_ERR_2 ("DS->CM: MOD_BEARER_RSP: err in MOD_BEARER_RSP CMD, call_id %d, asubs_id %d",
...
)
{
  ...
  else if (target_rat == SYS_RAT_LTE_RADIO_ACCESS)
  {
    while( ((call_ptr = cmcall_obj_get_next(&call_itr)) != NULL))
    {
      if((call_ptr->mm_id.asubs_id == asubs_id) && (call_ptr->cmcall_mode_info.info_type == CM_CALL_MODE_INFO_GW_PS))
      {
        if(call_ptr->call_state == CM_CALL_STATE_ORIG)
        {
          if(call_ptr->sr.orig_sent_on_mode == SYS_SYS_MODE_NONE)
  ...
  while(call_ptr != NULL)
  {
    ...
    {
      CM_MSG_HIGH_2("After cmltecall_irat_call_object_transfer_per_subs: call_id %d, sr orig_sent_on_mode %d",
                    call_ptr->call_id,
