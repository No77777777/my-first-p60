...
)
{
  ...
  status = cmmmgsdi_write(SYS_MODEM_AS_ID_1,
                          MMGSDI_USIM_7F66_PROP1_RPM_CNTRS,
...
static boolean cmrpm_is_rpm_sim(void)
{
  ...
  if(!IS_VALID_PH_SUB_PTR(SYS_MODEM_AS_ID_1) ||
      sys_plmn_id_is_valid(ph_ptr->sub_info[SYS_MODEM_AS_ID_1]->gwl_imsi_plmn) == FALSE)
  {
    ...
    */
    ...
    else
    {
      ...
      for(i=0; i < CMRPM_MAX_SIM_SUPPORTED; i++)
      {
        ...
        if(sys_plmn_match(rpm_plmns, ph_ptr->sub_info[SYS_MODEM_AS_ID_1]->gwl_imsi_plmn))
        {
...
)
{
  ...
  */
        ...
        (cmd_info_ptr->oprt_mode == SYS_OPRT_MODE_FTM)))
  {
    ...
    {
      if( cmrpm_oprt_mode_tbl[i].from_oprt_mode == cmph_ptr()->device_prop.ph_oprt_mode.oprt_mode &&
...
FUNCTION cmrpm_app_reset_reject_proc_wrapper

DESCRIPTION
  Processes PH cmd from CM when app reset was rejected

DEPENDENCIES
  RPM object must have already been initialized with cmrpm_subs_avail_init().
...
SIDE EFFECTS
  None
===========================================================================*/
void cmrpm_app_reset_reject_proc_wrapper(void)
{
...
} /* cmrpm_app_reset_reject_proc */

/*===========================================================================

FUNCTION cmrpm_oprt_mode_read_efs_sys_time
...
===========================================================================*/
void cmrpm_oprt_mode_read_efs_sys_time(void)
{
...
FUNCTION cmrpm_oprt_mode_write_efs_sys_time
...
===========================================================================*/
void  cmrpm_oprt_mode_write_efs_sys_time(
...
} /* cmrpm_oprt_mode_write_efs_sys_time */

/*===========================================================================
FUNCTION cmrpm_oprt_mode_process_app_reset

DESCRIPTION
  Process oprt mode change. Not supported in MSIM config.
...
SIDE EFFECTS
  None
===========================================================================*/
void cmrpm_oprt_mode_process_app_reset(
...
)
{
  ...
  ret_val_lte_rpm = cmrpm_lte_process_app_reset( ph_cmd_ptr );

#endif

  CM_MSG_HIGH_2("CM RPM process app reset %d, LTE RPM process app reset %d", ret_val, ret_val_lte_rpm);

} /* cmrpm_oprt_mode_process_app_reset */
