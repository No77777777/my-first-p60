...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/mmode/cm/src/cmdrvcc.c#1 $
...
)
{
  ...
  */
  ...
  CM_MSG_HIGH_2("DRVCC: switch in call sub %d stk %d",
                switch_in_call_ptr->mm_id.asubs_id, switch_in_call_ptr->mm_id.stk_id);

  /* This function will write CM_MODE_PREF_NONE into favored_mode_pref
     if it was not able to compute it */
  if (!cmcall_drvcc_set_favored_mode_pref_for_setup ( switch_in_call_ptr->mm_id.asubs_id,
  ...
  cmdbg_print_fav_mode_pref_for_origination( switch_in_call_ptr->mm_id.asubs_id,
      switch_in_call_ptr->call_type,
      CM_SRV_TYPE_AUTOMATIC,
      CMPH_SS_MODE_PREF(switch_in_call_ptr->mm_id.asubs_id, ph_ptr),
      cmd_info_ptr->favored_mode_pref,
      cmd_info_ptr->favored_hybr_pref );

  if( !cmph_is_valid_call_mode_pref_on_ss(switch_in_call_ptr->mm_id.asubs_id,
                                          cmd_info_ptr->favored_mode_pref, (cm_cmd_type *)call_cmd_ptr) )
  {
    ...
    cm_call_cmd_s_type *call_cmd_ptr)
{
  ...
  switch_in_call_ptr->line = cmph_ptr()->device_prop.als_pref.line;
  ...
  if (cmph_is_msim())
  {
    ...
    ** originated.
    */
    asubs_id = switch_in_call_ptr->mm_id.asubs_id = call_cmd_ptr->cmd_subs;
    CM_ASSERT_ENUM_IS_INRANGE(asubs_id, SYS_MODEM_AS_ID_MAX);
    switch_in_call_ptr->mm_id.stk_id = MM_STACK_0;
  ...
  {
    switch_in_call_ptr->mm_id.asubs_id = SYS_MODEM_AS_ID_1;
    switch_in_call_ptr->mm_id.stk_id = MM_STACK_0;
  }
}
...
===========================================================================*/
boolean cmcall_drvcc_set_favored_mode_pref_for_setup (
  sys_modem_as_id_e_type asubs_id,
...
boolean cmcall_drvcc_send_xdrvcc_orig(cmcall_s_type *call_ptr)
{
  ...
  if (call_ptr->csfb_type == CM_CALL_CSFB_TYPE_1XCSFB &&
      ...
      cmsds_get_sub_prop_ptr(call_ptr->mm_id.asubs_id) &&
      cmsds_get_sub_prop_ptr(call_ptr->mm_id.asubs_id)->gpp2_sub_info.is_1xcsfb_activated)
  {
...
void cmcall_drvcc_check_and_abort_drvcc_setup(cmcall_s_type *call_ptr)
{
  ...
  if(abort_drvcc_setup == TRUE)
  {
    ...
    cmcall_drvcc_trigger_ho_fail_ipapp(switch_in_call_ptr->orm_sent_status,
                                       switch_in_call_ptr->mm_id.asubs_id);
...
{
  if(cmcall_drvcc_cache.state != CM_DRVCC_STATE_NONE)
  {
