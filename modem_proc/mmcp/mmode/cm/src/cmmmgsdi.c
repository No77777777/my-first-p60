...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/mmode/cm/src/cmmmgsdi.c#1 $
...
static word                          cm_auto_reg_imsi_mcc[MAX_SIMS] = {0};
static byte                          cm_auto_reg_imsi_mnc[MAX_SIMS]= {0};
static boolean                       cm_received_card_pwr_down_cnf[MAX_SIMS];
static boolean                       cm_received_card_error_evt[MAX_SIMS];
...
)
{
  ...
  mmgsdi_session_type_enum_type cm_mmgsdi_session_type = MMGSDI_1X_PROV_PRI_SESSION;
  ...
  cm_mmgsdi_session_type = AS_ID_TO_1X_SESSION(asubs_id);
  ...
  */
  ...
  {
    status = cmmmgsdi_read( cm_mmgsdi_session_type,
                            MMGSDI_CDMA_IMSI_M,
                            0,
                            file_size,
                            (uint8 *)cdma_imsi,
  ...
  if( file_size == CM_AUTO_REG_CT_CDMA_IMSI_LEN )
  {
    ...
    */
    cm_auto_reg_imsi_mcc[asubs_id] =  (cdma_imsi[CM_AUTO_REG_CT_CDMA_IMSI_LEN - 1] & 0x3);
    cm_auto_reg_imsi_mcc[asubs_id] = (cm_auto_reg_imsi_mcc[asubs_id]<< 8);
    cm_auto_reg_imsi_mcc[asubs_id] |=  cdma_imsi[CM_AUTO_REG_CT_CDMA_IMSI_LEN - 2];
    cm_auto_reg_imsi_mnc[asubs_id] =  cdma_imsi[CM_AUTO_REG_CT_CDMA_IMSI_LEN - 4] ;

    CM_MSG_HIGH_2("MMGSDI_CDMA_IMSI_M read - MCC %d MNC:%d ",
                  cm_auto_reg_imsi_mcc[asubs_id], cm_auto_reg_imsi_mnc[asubs_id]);
...
{
  return cm_auto_reg_imsi_mcc[0];
}

/*===========================================================================

FUNCTION NAME cmmmgsdi_get_imsi_mcc_per_sub
...
DEPENDENCIES
  None
===========================================================================*/
word cmmmgsdi_get_imsi_mcc_per_sub(
  sys_modem_as_id_e_type asubs_id
)
{
  if (cmph_is_ssim())
  ...
  {
    return cm_auto_reg_imsi_mcc[asubs_id];
  }
  else
  {
    return cm_auto_reg_imsi_mcc[0];
...
FUNCTION NAME cmmmgsdi_set_imsi_mcc
...
DEPENDENCIES
  None
===========================================================================*/
void cmmmgsdi_set_imsi_mcc
(
  word mcc
)
{
  cm_auto_reg_imsi_mcc[0] = mcc;
  return;
}
...
FUNCTION cmmmgsdi_ssn_tbl_update_slot_for_session_id

DESCRIPTION

  This function sets the slot mentioned by session_id
...
cm_mmgsdi_error_e_type cmmmgsdi_ssn_tbl_update_slot_for_session_id
(
  mmgsdi_session_id_type session_id,
...
)
{
  ...
  {
    result = CM_MMGSDI_SUCCESS;
    CM_MSG_HIGH_4("Set slot value from %d to %d on as_id %d ssn_type %d",ssn_entry->app_info.slot,slot_id,
      ssn_entry->asubs_id, ssn_entry->session_type);

    ssn_entry->app_info.slot = slot_id;
  }
...
FUNCTION cm_mmgsdi_ssn_tbl_update_slot_for_asubs_id_1x

DESCRIPTION

  This function sets the slot mentioned by asubs_id
...
cm_mmgsdi_error_e_type cm_mmgsdi_ssn_tbl_update_slot_for_asubs_id_1x
(
  sys_modem_as_id_e_type asubs_id,
...
FUNCTION cm_mmgsdi_ssn_tbl_get_entry_by_current_state

DESCRIPTION

  Get session table entry matching given asubs_id for session with given state.
  Assumption: At most one entry associated for given criteria, if not first
  entry is returned.

DEPENDENCIES
  For now usage with CMMMGSDI_STATE_SUBSCRIPTION_PENDED, please recheck when using with other states
...
=====================================================================*/
cm_mmgsdi_session_id_table_entry_s_type *cm_mmgsdi_ssn_tbl_get_entry_by_current_state
(
  sys_modem_as_id_e_type asubs_id,
...
)
{
  ...
  for(index = 0; index < ARR_SIZE(cm_mmgsdi_session_id_table); index++)
  {
    if((cm_mmgsdi_session_id_table[index].current_state == state)&&
        (cm_mmgsdi_session_id_table[index].asubs_id == asubs_id)
      )
    {
      ssn_entry = &cm_mmgsdi_session_id_table[index];
      break;
...
)
{
  ...
  if(ph_sub_ptr->self_activation_support == TRUE)
  {
...
FUNCTION cm_mmgsdi_get_asubs_id_for_slot_id

DESCRIPTION
  Wrapper function to get asubs_id from the slot_id

DEPENDENCIES

RETURN VALUE
  asubs_id for the respective slot_id
...
sys_modem_as_id_e_type cm_mmgsdi_get_asubs_id_for_slot_id(mmgsdi_slot_id_enum_type slot_id)
{
  cm_mmgsdi_session_info_s_type session_info;
  if(cm_mmgsdi_ssn_tbl_get_ssn_info_for_slot(slot_id, &session_info) == CM_MMGSDI_SUCCESS)
  {
    return SESSION_TO_AS_ID(session_info.session_type);
  }
  ...
  if (status == MMGSDI_SUCCESS)
  {
    switch(cnf)
    {
      ...
      case MMGSDI_CARD_PDOWN_CNF:
        ...
        asubs_id = cmph_map_slot_id_to_asubs_id(cnf_ptr->card_pdown_cnf.response_header.slot_id);
        ...
        if ( ph_sub_ptr != NULL
             && ph_sub_ptr->sub_3gpp2_pref.is_otasp_commit_pending
             && cm_received_card_error_evt[asubs_id] == FALSE)
        {
          cm_received_card_pwr_down_cnf[asubs_id] = TRUE;
        }

        /* If CARD ERROR EVT is received, send power up */
        if(ph_sub_ptr != NULL && ph_sub_ptr->sub_3gpp2_pref.is_otasp_commit_pending
           && (cm_received_card_error_evt[asubs_id] == TRUE))
        {
			mmgsdi_slot_id_enum_type slot = MMGSDI_SLOT_1;
#ifdef FEATURE_MMGSDI_SESSION_LIB
            slot = cnf_ptr->card_pdown_cnf.response_header.slot_id;
            slot = ((slot == MMGSDI_MAX_SLOT_ID_ENUM)?MMGSDI_SLOT_1:slot);
#endif

          CM_MSG_HIGH_2("Card pwrup sent for OTASP comm, asubs_id %d slot %d",
                        asubs_id, slot);
			...
			*/
			if(mmgsdi_card_pup(cm_mmgsdi_get_client_id(),
							   ...
							   0 ) != MMGSDI_SUCCESS)
			{
			  CM_ERR_0("UIM->CM: CARD_ERROR_EVT: Unable to power-up card");
			}
            cm_received_card_error_evt[asubs_id] = FALSE;
			ph_sub_ptr->sub_3gpp2_pref.is_otasp_commit_pending = FALSE;
        }
  ...
  else /* status != MMGSDI_SUCCESS */
  {
    switch(cnf)
    {
      ...
      case MMGSDI_CARD_PDOWN_CNF:
        ...
        if ( ph_sub_ptr != NULL
             && ph_sub_ptr->sub_3gpp2_pref.is_otasp_commit_pending
             && cmph_get_cdma_subs_from_card_sent(asubs_id))
        {
          cm_received_card_pwr_down_cnf[asubs_id] = FALSE;
        }
        break;
      case MMGSDI_REFRESH_CNF:
        switch (cnf_ptr->refresh_cnf.orig_refresh_req)
...
)
{
  ...
  if(index < ARR_SIZE(cm_mmgsdi_session_id_table))
  {
    sub = SESSION_TO_AS_ID(cm_mmgsdi_session_id_table[index].session_type);
    if (!cmutil_is_as_id_valid(sub)) return;
    ...
    if (IS_GW_SESSION(cm_mmgsdi_session_id_table[index].session_type))
    {
      gw_subs_status = CM_SUBSCRIPTION_STATUS_CHANGE;
      cmph_set_gw_subs_from_card_sent(sub, FALSE );
      cm_mmgsdi_set_waiting_to_proceed_with_refresh(TRUE, index);
    }
    else if (IS_1X_SESSION(cm_mmgsdi_session_id_table[index].session_type))
    {
  ...
  if(!cmph_is_standby_chg_on_refresh(mode, source))
  {
    cause = CM_SUBSCRIPTION_NOT_AVAIL_CAUSE_REFRESH_NO_STDBY;
  }
else if(ds3g_is_otasn_feature_supported(sub))
  {
     cause =   CM_SUBSCRIPTION_NOT_AVAIL_CAUSE_REFRESH;
  }

  if (cdma_subs_status == CM_SUBSCRIPTION_STATUS_CHANGE ||
      gw_subs_status == CM_SUBSCRIPTION_STATUS_CHANGE)
  {
    CM_MSG_HIGH_2("OK to REFRESH DEACTIVATE, send subscription not available. sess_type: %d sub %d",
                  cm_mmgsdi_session_id_table[index].session_type, sub);
...
)
{
  ...
  if( cmclnup_is_ongoing_subsc(BM(as_id)))
  {
    CM_MSG_HIGH_1("CLNUP: Proceed with refresh after cleanup on sub %d",as_id);
    return;
  }
  ...
  for(index = 0; index < tbl_size; index++)
  {
    ...
    /* Needs modification for 1x_SEC once ss is done away with */
    if ((AS_ID_TO_1X_SESSION(as_id) == session_type ||
         AS_ID_TO_GW_SESSION(as_id) == session_type)
        &&
...
)
{
  ...
  else
  {
    ...
    CM_MSG_LOW_4 ("Values read from MMGSDI-> cnf- %d, data_len: %x, data_ptr: %x,file_enum: %x",
                  cnf,
...
)
{
  ...
  CM_MSG_HIGH_2 ("cmmmgsdi_wait_for_read_acl_complete_cb: cnf_ptr - \
                 %x status - %d ",cnf_ptr, status );
...
boolean cmmmgsdi_is_jcdma_card_white_equiv(sys_modem_as_id_e_type asubs_id)
{
  mmgsdi_jcdma_card_info_enum_type jcdma_card_info= MMGSDI_JCDMA_UNKNOWN_CARD;

  mmgsdi_session_type_enum_type session_type = AS_ID_TO_1X_SESSION(asubs_id);

  if(cmmmgsdi_get_jcdma_card_info(session_type, &jcdma_card_info))
  {
...
cm_mmgsdi_read_uptime_s_type *cmmmgsdi_allocate_sim_read_req(cm_call_id_type call_id)
{
  ...
  CM_MSG_HIGH_2("Allocating sim read req for call id:%d, ptr %x", call_id, mmgsdi_acl_read_req);
...
DEPENDENCIES
  None
===========================================================================*/
void cmmmgsdi_deallocate_sim_read_req(cm_call_id_type call_id, boolean free_cc_obj)
{
  cmmmgsdi_s_type           *mmgsdi_ptr = cmmmgsdi_ptr();
  ...
  if(mmgsdi_ptr->mmgsdi_read_req_uptime[call_id] != NULL)
  {
    if (free_cc_obj)
    {
      cmcc_object_s_type *cc_object_ptr =
        (cmcc_object_s_type *)mmgsdi_ptr->mmgsdi_read_req_uptime[call_id]->cc_object_ptr;
      if (cc_object_ptr != NULL)
      {
        if (cc_object_ptr->user_data != NULL)
        {
...
FUNCTION NAME cmmmgsdi_get_cc_object_from_tid

DESCRIPTION
  Returns valid call id from mmgsdi_read_req cache for tid passed
...
DEPENDENCIES
  None
===========================================================================*/
void *cmmmgsdi_get_cc_object_from_tid(uint16 cc_object_tid)
{
  cmmmgsdi_s_type           *mmgsdi_ptr = cmmmgsdi_ptr();
  void *ret_val = NULL;
  cm_call_id_type call_id;
  ...
  {
    if(mmgsdi_ptr->mmgsdi_read_req_uptime[call_id] != NULL &&
        mmgsdi_ptr->mmgsdi_read_req_uptime[call_id]->cc_object_ptr != NULL)
    {
      cmcc_object_s_type *cc_object_ptr =
        (cmcc_object_s_type *)mmgsdi_ptr->mmgsdi_read_req_uptime[call_id]->cc_object_ptr;

      if (cc_object_ptr->tid == cc_object_tid)
      {

        ret_val = cc_object_ptr;
        cm_mem_free(mmgsdi_ptr->mmgsdi_read_req_uptime[call_id]);
        mmgsdi_ptr->mmgsdi_read_req_uptime[call_id] = NULL;
        break;
      }
  ...
  CM_MSG_HIGH_2("cmmmgsdi_get_call_id_from_cc_object_tid returns call_id %d for tid %d", ret_val, cc_object_tid);
  return ret_val;
}
...
boolean cmmmgsdi_validate_sim_read_req(cm_call_id_type call_id)
{
  ...
  CM_MSG_HIGH_2("cmmmgsdi_validate_sim_read_req at index %d returns %d",call_id, ret_val);
  return ret_val;
}
...
)
{
  ...
  cm_mmgsdi_status = cm_mmgsdi_ssn_tbl_get_id_for_type(AS_ID_TO_GW_SESSION(cc_object_ptr->asubs_id),
                     &cm_mmgsdi_session_id);

  if (CM_MMGSDI_SUCCESS != cm_mmgsdi_status)
  {
    CM_ERR_1("Unable to get session id; err: %d",cm_mmgsdi_status);
  }

  CM_MSG_HIGH_2("Updated Session type: %d for Session id: %d", AS_ID_TO_GW_SESSION(cc_object_ptr->asubs_id), cm_mmgsdi_session_id);
  ...
  if (status != MMGSDI_SUCCESS)
  {
    ...
    CM_MSG_HIGH_0("mmgsdi_read_transparent acl status is not MMGSDI_SUCCESS");
...
void cmmmgsdi_refresh_files_read_proc(
  sys_modem_as_id_e_type                asubs_id,
  /* subscription whose files to read */
...
/*lint -esym(715,cm_client_id) */
{
  ...
  session_type = AS_ID_TO_GW_SESSION(asubs_id);
  ...
  if(cmph_is_rat_bal_allowed(asubs_id) && app_type != MMGSDI_APP_USIM)
  {
       ...
       file_num_count++)
  {
    switch (refresh_file_list->file_list_ptr[file_num_count])
    {
      ...
      case MMGSDI_USIM_7F66_PROP1_RAT:
        ...
        if (cmph_is_rat_bal_allowed(asubs_id) && rat_mode_result == TRUE)
        {
          if (!cmmmgsdi_read(
                session_type,
                MMGSDI_USIM_7F66_PROP1_RAT,
          ...
          else
          {
            ...
            ** is present.
            */
            cmph_update_uim_usim_rat_present(asubs_id, TRUE);
            ...
            */
            ...
            if ( !SYS_LTE_BAND_MASK_CHK_IF_EMPTY(&ph_ptr->device_prop.hw_capability.lte_band_capability))
            {
            ...
            {
              cmmmgsdi_update_rat_mode(cm_client_id, rat_bal_new_mode_pref, asubs_id);
            }
          }
...
)
{
  ...
  if(cmmmgsdi_ptr()->sub_info[asubs_id].sim_busy_status.is_sim_busy == FALSE)
  {
...
)
{
  ...
  */
  if ( app_type == MMGSDI_APP_SIM )
  {
    sim_filename = MMGSDI_GSM_PLMNWACT;
  }
  else
  {
    sim_filename = MMGSDI_USIM_PLMNWACT;
  }

  if ( sim_filename == MMGSDI_GSM_PLMNWACT )
  {
...
)
{
  ...
  */
  if( cmmmgsdi_read_pplmn_w_act_list( asubs_id, app_type, user_pref_plmn_list_ptr,
                                      is_packed))
  {
    return TRUE;
  }
  else
  {
    ...
    */
    status = cmmmgsdi_read_pplmn_list(asubs_id, user_pref_plmn_list_ptr,is_packed );
...
void cmmmgsdi_refresh_init(
  ...
  sys_modem_as_id_e_type                    asubs_id
  /* sub on which the refresh is initiated */
)
{
  mmgsdi_session_id_type        cm_mmgsdi_session_id = 0;
  mmgsdi_session_type_enum_type cm_mmgsdi_session_type = MMGSDI_GW_PROV_PRI_SESSION;
  ...
  cm_mmgsdi_session_type = AS_ID_TO_GW_SESSION(asubs_id);

  CM_MSG_HIGH_2("SIM_REF: asubs_id %d, cm_mmgsdi_session_type %d",
                asubs_id, cm_mmgsdi_session_type);
  ...
  cmmmgsdi_refresh_files_read_proc(asubs_id,
                                   refresh_file_list,
...
)
{
  ...
  new_mode_pref = ph_ptr->sub_info[asubs_id]->pref_info.mode_pref;
  ...
  */
  ...
  if(cmph_is_rat_bal_allowed(asubs_id))
  {
    CM_MSG_HIGH_3("cmmmgsdi_update_rat_mode after mode_pref=%d, user_mode_pref=%d,rat_mode=%d",
                  ph_ptr->sub_info[asubs_id]->pref_info.mode_pref,
                  ph_ptr->sub_info[asubs_id]->pref_info.user_mode_pref,
                  rat_mode );

    if(!cm_ph_cmd_sys_sel_pref_4(
          NULL,
          NULL,
          client_id,
          asubs_id,
      ...
      )
    {
      CM_ERR_2("Can't change phone's mode preference from ef_rat:%x to user_mode_pre:%x for "
               "RAT balancing",
               new_mode_pref, ph_ptr->sub_info[asubs_id]->pref_info.user_mode_pref);
    }
    else
    {
      ...
      CM_MSG_HIGH_2 (" Queued req to CM to change phone's mode_pref as"
                     ...
                     "%d to %d",
                     ph_ptr->sub_info[asubs_id]->pref_info.user_mode_pref,new_mode_pref);
      /* Set the flag */
...
)
{
  ...
  file_access.access_method = MMGSDI_EF_ENUM_ACCESS;
  file_access.file.file_enum = file_name;
  if(cmmmgsdi_ptr()->sub_info[asubs_id].sim_busy_status.is_sim_busy == TRUE)
  {
  ...
  if( universal_pin.pin_replacement == MMGSDI_PIN_REPLACED_BY_UNIVERSAL )
  {
    ...
    */
    ...
    else if( universal_pin.status == MMGSDI_PIN_UNBLOCKED )
    {
      ...
      {
        if( !cm_ph_cmd_subscription_available4(
              NULL,
              NULL,
              CM_CLIENT_ID_ANONYMOUS,
              asubs_id,
              ...
              gw_subs_status))
        {
          CM_ERR_3("pin unblock: subs status CDMA %d, GW %d, asubs_id %d",
                   cdma_subs_status,
                   gw_subs_status,
                   asubs_id);
        }
        ...
        {
          CM_MSG_HIGH_3("pin unblock: subs status CDMA %d, GW %d, asubs_id %d",
                        cdma_subs_status,
                        gw_subs_status,
                        asubs_id);
        }
...
DESCRIPTION
  ...
  If there is a VOWLAN call presentthen it send gsdi mode as GW,
  NON PS CALL ONGOING as call mask and  GW session ID
...
/* if there is a VOWLAN call present or ext VOWLAN call present, send gsdi mode as GW, NON PS CALL ONGOING as call mask and  GW session ID*/
  if( (cmcall_is_there_vowlan_call_sub(asubs_id) != CM_CALL_ID_INVALID) || mode == SYS_SYS_MODE_WLAN)
  {
  ...
  {
    index = (int)AS_ID_TO_GW_SESSION(asubs_id);
  }
  else
  {
    index = (int)AS_ID_TO_1X_SESSION(asubs_id);
  }

  active_call_mask = cm_is_there_call_type_per_sub(
                       asubs_id,
  ...
  CM_MSG_HIGH_4 ("Informing traffic state to MMGSDI, traffic:%d, gsdi_mode:%d, active_call_mask:%d, mmgsdi_call_mask:%d",
                 in_traffic,gsdi_mode,active_call_mask,mmgsdi_call_mask);
...
)
{
  ...
  CM_MSG_HIGH_2("<<CM mmgsdievt %d, session_id %d", event->evt, event->session_id);
  ...
  switch ( event->evt )
  {
    ...
    case MMGSDI_SESSION_ILLEGAL_SUBSCRIPTION_EVT:
      ...
      */
      ...
      switch ( event->evt )
      {
        case MMGSDI_CARD_INSERTED_EVT:
          *(&(mmgsdi_rpt_ptr->data.card_inserted)) = *(&(event->data.card_inserted));
          if ( cm_get_operator_info(SYS_MODEM_AS_ID_1) == OPERATOR_CT)
          {
            ...
            {
              CM_MSG_HIGH_0( "UIM->CM: cmmmgsdi_card_insert_evt - ICCID Changed");
              //Clear Avoidance List
              if(! cm_ph_cmd_avoid_sys1( NULL,
                                         NULL,
                                         CM_CLIENT_TYPE_NONE,
            ...
            {
              CM_MSG_HIGH_1( "UIM->CM: cmmmgsdi_card_insert_evt-ICCID Not Changed in,idx:%d", idx);
            }
          }
          ...
          /*Call CMRMP_TDS*/
#ifdef FEATURE_MMODE_RPM_TDSCDMA
          CM_MSG_HIGH_1("UIM->CM: MMGSDI_CARD_INSERTED_EVT send to CMRPM_TDS, idx: %d", idx);
        ...
        {
          cmph_sub_info_s_type *ph_sub_ptr;
          sys_modem_as_id_e_type        card_error_sub = cmph_map_slot_id_to_asubs_id(event->data.card_error.slot);
          *(&(mmgsdi_rpt_ptr->data.card_error)) = *(&(event->data.card_error));
          ...
          ph_sub_ptr = cmph_get_sub_ptr(card_error_sub);

          CM_MSG_HIGH_3_EXT("UIM->CM: MMGSDI_CARD_ERROR_EVT, slot %d error_info %d, sub %d",
          ...
          if ( ph_sub_ptr != NULL && ph_sub_ptr->sub_3gpp2_pref.is_otasp_commit_pending 
              && cm_received_card_pwr_down_cnf[card_error_sub] == FALSE)
          {
            cm_received_card_error_evt[card_error_sub] = TRUE;
          }

          /* If OTASP commit is pending */
          /* If subscription is from card, reset the card */
          if ( ph_sub_ptr != NULL && ph_sub_ptr->sub_3gpp2_pref.is_otasp_commit_pending
               &&(cm_received_card_pwr_down_cnf[card_error_sub] == TRUE) )
          {
            mmgsdi_slot_id_enum_type slot = MMGSDI_SLOT_1;
#ifdef FEATURE_MMGSDI_SESSION_LIB
        ...
        case MMGSDI_SESSION_ILLEGAL_SUBSCRIPTION_EVT:
        {
          CM_MSG_HIGH_1("UIM->CM: MMGSDI_SESSION_ILLEGAL_SUBSCRIPTION_EVT, session-id %d",
          ...
          *(&(mmgsdi_rpt_ptr->data.illegal_session)) = *(&(event->data.illegal_session));
          ...
          /*Call CMRMP_TDS*/
#ifdef FEATURE_MMODE_RPM_TDSCDMA
          CM_MSG_HIGH_0("UIM->CM: MMGSDI_SESSION_ILLEGAL_SUBSCRIPTION_EVT send to CMRPM_TDS");
        ...
        case MMGSDI_SESSION_CHANGED_EVT:
          CM_MSG_HIGH_3_EXT("UIM->CM: MMGSDI_SESSION_CHANGED_EVT, slot %d app type %d activated %d",
                            ...
                            (cm_mmgsdi_ssn_tbl_get_subs_id_for_id(event->session_id)+1));
    ...
    case MMGSDI_PERSO_EVT:
      switch(event->data.perso.status)
      {
...

        {
          ...
          asubs_id = SESSION_TO_AS_ID(sess_info.session_type);

          /* Perso feature is not needed for 1X subscription*/
          if (IS_GW_SESSION(sess_info.session_type))
          {
            gw_subs_status = CM_SUBSCRIPTION_STATUS_CHANGE;
          }
          ...
          */
          if( gw_subs_status == CM_SUBSCRIPTION_STATUS_CHANGE)
          {
            ...
            {
              CM_ERR_3("UIM->CM: perso subs a CDMA %d, GW %d, asubs_id %d",
                       CM_SUBSCRIPTION_STATUS_NO_CHANGE, gw_subs_status, asubs_id);
            }
            else
            {
              CM_MSG_HIGH_3("UIM->CM: perso subs a CDMA %d, GW %d, asubs_id %d",
                            CM_SUBSCRIPTION_STATUS_NO_CHANGE, gw_subs_status, asubs_id);
            }
        ...
        default:
          CM_MSG_HIGH_1("UIM->CM: perso status %d", event->data.perso.status);
          break;
      }
    ...
    case MMGSDI_SUBSCRIPTION_READY_EVT:
    {
      ...
      CM_MSG_HIGH_4_EXT("UIM->CM: MMGSDI_SUBSCRIPTION_READY_EVT, slot %d app type %d, act_status %d, session_id %d",
      ...
      cmph_set_received_card_init_completed_evt( TRUE, cm_mmgsdi_get_asubs_id_for_slot_id(event->data.subscription_ready.app_info.slot));
...
#ifdef FEATURE_MMODE_DUAL_SIM
      {
        ...
        */
        ...
        {
          cm_generic_rpt(cm_generic_rpt_ptr);
          CM_MSG_HIGH_3("UIM->CM: SUB_READY: cm_generic_rpt sent!!!!, slot %d, app_type %d, session_id %d",
                        event->data.subscription_ready.app_info.slot,
                        event->data.subscription_ready.app_info.app_data.app_type,
                        event->session_id);
        }
        ...
        {
          CM_ERR_3("UIM->CM: SUB_READY: MMGSDI app_info read fail session id %d cm_generic_rpt not sent,  slot %d, app_type %d",
                   event->session_id,
                   event->data.subscription_ready.app_info.slot,
                   event->data.subscription_ready.app_info.app_data.app_type);/*lint !e641 */
        }
...
#ifdef FEATURE_MMODE_RPM_TDSCDMA
      CM_MSG_HIGH_0_EXT("MMGSDI_SUBSCRIPTION_READY_EVT send to CMRPM_TDS",
                        (cm_mmgsdi_ssn_tbl_get_subs_id_for_id(event->session_id)+1));
    ...
    case MMGSDI_SESSION_LEGAL_SUBSCRIPTION_EVT:
    {
      ...
      CM_MSG_HIGH_1("UIM->CM: MMGSDI_SESSION_LEGAL_SUBSCRIPTION_EVT, session id: %d",
                    event->data.illegal_session.session_id);
    ...
    case MMGSDI_SIM_BUSY_EVT:
    {
      ...
      CM_MSG_HIGH_2("UIM->CM: SIMBUSY: MMGSDI_SIM_BUSY_EVT, slot: %d, sim_busy:%d",
    ...
    {
      CM_MSG_HIGH_3("UIM->CM: SIM_REF: MMGSDI_REFRESH_EVT, mode %d stage %d, slot %d",
      ...
      else if(event->data.refresh.stage == MMGSDI_REFRESH_STAGE_WAIT_FOR_PROCEED_WITH_REFRESH)
      {
        cmmmgsdi_proceed_with_refresh_stage_deactivate(event->session_id,
            ...
            event->data.refresh.source);
      }
#endif
      else
      {
        CM_MSG_HIGH_0("UIM->CM: SIM_REF: Unhandled mode stage combination for refresh");
      }
    }
    break;
#endif  /* FEATURE_GSTK */
...
void cmmmgsdi_timer_proc(void)
{
  ...
  {
    call_ptr = cmcall_ptr((cm_call_id_type)index);
    if(mmgsdi_ptr->mmgsdi_read_req_uptime[index] == NULL ||
        call_ptr == NULL || !cmutil_is_as_id_valid(call_ptr->mm_id.asubs_id))
    {
      continue;
    }
    ...
    if(mmgsdi_ptr->mmgsdi_read_req_uptime[index]->uptime <= uptime)
    {
      ...
      **fake the response as an error , so that CM would continue */
      if(mmgsdi_ptr->sub_info[call_ptr->mm_id.asubs_id].sim_busy_status.is_sim_busy == TRUE)
      {
      ...
      if (mmgsdi_ptr->mmgsdi_read_req_uptime[index]->response_cb_ptr == (mmgsdi_callback_type)cmcc_sim_cap_cb)
      {
        cmcc_object_s_type *cc_object_ptr =
          (cmcc_object_s_type *)mmgsdi_ptr->mmgsdi_read_req_uptime[index]->cc_object_ptr;
        if (cc_object_ptr != NULL)
        {
          gsdi_cnf->response_header.client_data = (uint32) cc_object_ptr->tid;
        }
        else
        {
          CM_ERR_0( "cc_object ptr is NULL");
          cm_mem_free(gsdi_cnf);
          return;
        }
      }

      mmgsdi_ptr->mmgsdi_read_req_uptime[index]->response_cb_ptr(
        status,
        MMGSDI_READ_CNF,
        gsdi_cnf);
    }
      ...
      && mmgsdi_ptr->read_als_uptime.uptime <= uptime)
  {
    /*since MMGSDI  didnt respond in stipulated time ,
    **fake the response as an error , so that CM would continue */
    if(mmgsdi_ptr->sub_info[SYS_MODEM_AS_ID_1].sim_busy_status.is_sim_busy == TRUE)
    {
      status = MMGSDI_SIM_BUSY;
    }
    else
...
void cmmmgsdi_init(void)
{
  ...
  */
  ...
  {
    cm_received_card_error_evt[i] = FALSE;
    cm_received_card_pwr_down_cnf[i] = FALSE;
  }
...
FUNCTION cmmmgsdi_count_given_state_per_sub
...
===========================================================================*/
uint8 cmmmgsdi_count_given_state_per_subs(cmmmgsdi_session_state_type  state, sys_modem_as_id_e_type asubs_id)
{
  ...
  if(stateCount !=0)
  {
    CM_MSG_HIGH_2("oprt_mode_online: cmmmgsdi_count_given_state sub:%d, count %d",asubs_id,stateCount);
  }
  return stateCount;
}
...
/*===========================================================================
FUNCTION cmmmgsdi_qsh_mdump_collect
  
DESCRIPTION
  This function collects the mdump for cmmmgsdi global variables
...
QSH_MDUMP_FN_ATTR
#endif
void cmmmgsdi_qsh_mdump_collect()
{
  qsh_mdump_collect_high(&(cm_mmgsdi_session_id_table[0]),ARR_SIZE(cm_mmgsdi_session_id_table)*sizeof(cm_mmgsdi_session_id_table_entry_s_type));
}
#endif
