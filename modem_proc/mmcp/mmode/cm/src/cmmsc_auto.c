...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/mmode/cm/src/cmmsc_auto.c#1 $
...
static cmmsc_routing_table_auto_entry_s_type cmmsc_auto_routes[] =
{
  ...
  */
  ...
  /* SYS_SEL_PREF_MAX event: Force to all stacks when not online
  */
  {
  ...
  */
...

  {
    ...
    {CMMSC_STACK_STATE_SUSPENDED,        CMMSC_STACK_STATE_SUSPENDED,              CMMSC_STACK_STATE_SUSPENDED},
  {{0,CMMSC_ACTION_RESUME},            {2,CMMSC_ACTION_FORCE_USER_WITH_RESUME},  {1,CMMSC_ACTION_FORCE_USER_WITH_RESUME}},
    { CMMSC_STACK_STATE_ACTIVE,          CMMSC_STACK_STATE_ACTIVE,                 CMMSC_STACK_STATE_ACTIVE}
  },
...
SIDE EFFECTS
  None.
===========================================================================*/
void cmmsc_auto_routing_table_init(
  cmmsc_routing_table_s_type  *auto_route_table
)
{
...
)
{
  ...
  /* If LTE is on the main stack, remove GWL so that we don't trigger gwl acquisition */
  if (!cmss_is_stack2_operational(state_machine->asubs_id))
  {
    (void)cmph_remove_orig_mode_with_priority_end(cmph_map_sub_stk_to_id(state_machine->asubs_id, MM_STACK_0));
    ...
    (void)cmmsc_auto_update_gwl_operation (FALSE,
                                           ...
                                           CM_MODE_PREF_GWL,
                                           FALSE,
                                           cmph_map_sub_stk_to_id(state_machine->asubs_id, MM_STACK_0)
                                          );
#endif
  }
  /* Send STACK_SYNC_UP_BSR indication to SD if there is service on MAIN or on HYBR HDR */
  else
  {
...
)
{
  ...
  if(ph_ptr->sub_info[state_machine->asubs_id]->is_shorten_bsr)
  {
    ph_ptr->sub_info[state_machine->asubs_id]->is_shorten_bsr = FALSE;
  }
#endif
}
...
)
{
  ...
  mm_id = cmph_map_sub_stk_to_id(state_machine->asubs_id, MM_STACK_1);
  ph_orig_top_ptr = cmtask_orig_para_get_top(mm_id);

  if (ph_orig_top_ptr == NULL)
  {
    CM_ERR_2("ph_orig_top_ptr null ptr sub %d stk %d",
             state_machine->asubs_id, MM_STACK_1);
    return;
  }

  /* Check if we are in hybrid operation, else ignore it */
  if ( !cmss_is_stack1_operational(state_machine->asubs_id) )
  {
    return;
  }
  ...
  {
    CM_MSG_HIGH_0("do not force preference due to call in progress");
...
)
{
  ...
  mm_id = cmph_map_sub_stk_to_id(state_machine->asubs_id, MM_STACK_1);
  ph_orig_hybr1_top_ptr = cmtask_orig_para_get_top(mm_id);

  /* Check if HDR is already enabled */
  if (ph_orig_hybr1_top_ptr == NULL || !cmmsc_auto_get_is_hybr1_disabled(state_machine))
  {
    return;
  }
  ...
  if ( !cmcall_is_mobile_in_hdr_call() )
  {
    ...
    event_pkg.state_machine = state_machine;
    event_pkg.orig_mode = SD_SS_ORIG_MODE_COLLOC;
    event_pkg.hybr_pref = cmutil_map_sd_hybr_pref_to_cm_hybr_pref(
...
)
{
  ...
  {
    CM_MSG_HIGH_2("MSC: call end on asubs_id %d, kick hdr, hybr_pref %d",
...
)
{
  ...
  */
  if (hybr_bsr_to_hdr_status == SD_HYBR_BSR_TO_HDR_START)
  {
    ...
    event_pkg.orig_mode = (cmss_is_stack1_operational(state_machine->asubs_id)?
                           SD_SS_ORIG_MODE_COLLOC :
                           SD_SS_ORIG_MODE_NONE );
    event_pkg.hybr_pref =
      cmutil_map_sd_hybr_pref_to_cm_hybr_pref(   ph_ptr->sub_info[state_machine->asubs_id]->sub_3gpp2_pref.int_hybr_pref);
...
{
  if (!state_machine || !cmutil_is_as_id_valid(state_machine->asubs_id))
  {
    return;
  }
  ...
  if (mm_id.stk_id == MM_STACK_0 || mm_id.stk_id == MM_STACK_ALL)
  {
    ...
    cmph_sub_info_s_type       *ph_sub_ptr = cmph_get_sub_ptr(state_machine->asubs_id);
    if (ph_sub_ptr == NULL)
    {
      return;
    }
    ...
    if(ph_orig_top_ptr == NULL)
    {
      CM_ERR_2("Top ptr is NULL for sub %d, stack %d", state_machine->asubs_id, mm_id.stk_id);
      return;
    }
    ...
    is_ph_obj_3gpp_mode =
      cmcall_misc_is_mode_pref(ph_orig_top_ptr->orig->orig_mode_pref, CM_MODE_PREF_GWL);

    /* Check if phone ptr has 3GPP */
    is_ph_ptr_3gpp_mode =
      cmcall_misc_is_mode_pref(ph_sub_ptr->pref_info.mode_pref,
                               CM_MODE_PREF_GWL);
    ...
    mode_cap = cmutil_map_sys_mode_mask_to_sd_mode_pref(
...
)
{
  ...
  cm_orig_q_s_type  *pm_orig_ptr =
    cmtask_orig_para_search_act_type( mm_id,
  ...
  if(pm_orig_ptr == NULL)
  {
    pm_orig_ptr = cmtask_orig_para_search_act_type( mm_id,
                  CM_ACT_TYPE_PH_OBJ );
    pm_orig_obj = CM_ACT_TYPE_PH_OBJ;
  }

  if(pm_orig_ptr == NULL || ph_ptr == NULL )
  {
    CM_ERR_2("pm_orig_ptr null ptr sub %d stk %d",
  ...
  /* If an external VOLTE call is present, defer this activity
  */
  if ( cmsds_is_ext_volte_call_active_per_sub(asubs_id) &&
       !allowed &&
       SD_MODE_CONTAIN(SD_SS_MODE_PREF_LTE,cmph_map_cm_mode_pref_to_sd_mode_pref(cm_mode_pref)))
  {
    CM_MSG_HIGH_1("EXT_IMS_CALL: Defer removing 3GPP due to ext VOLTE call on sub %d", asubs_id);
    return FALSE;
  }
  ...
  */
  mode_cap = cmmsc_get_stk_mode_cap(mm_id);
  ...
  main_stack_pref = &(ph_sub_ptr->pref_info);
  ...
  */
  ...
  CM_MSG_HIGH_6_EXT("MSC: HICPS: mode_pref: cm %d ph obj %d ph_ptr %d, is_cdma_mode %d is_ph_obj_cdma_mode %d 1x_subs %d",
                    cm_mode_pref,
                    pm_orig_ptr->orig->orig_mode_pref,
                    main_stack_pref->mode_pref,
                    is_cdma_mode,
                    ...
                    (asubs_id+1));
  CM_MSG_HIGH_6("MSC: HICPS: is_gw_mode: %d, is_ph_ptr_gw_mode: %d, is_ph_obj_gw_mode: %d, allowed %d, is_hdr_acq_fail %d, pm_orig_obj %d",
                is_gw_mode,
                is_ph_ptr_gw_mode,
                is_ph_obj_gw_mode,
                allowed,
  ...
  */
    ...
    )
  {
    ...
    */
    ...
    cm_srv_domain_pref_e_type       local_srv_domain_pref;
    ...
    memscpy(pref_ptr, sizeof(cm_pref_s_type), main_stack_pref, sizeof(cm_pref_s_type));

    local_net_sel_pref = (pm_orig_obj == CM_ACT_TYPE_PH_OBJ)?
                         main_stack_pref->network_sel_mode_pref:
                         cmpmprx_compute_restricted_mode(asubs_id,
                             main_stack_pref->network_sel_mode_pref);

    local_srv_domain_pref = (pm_orig_obj == CM_ACT_TYPE_PH_OBJ)?
                            main_stack_pref->srv_domain_pref:
                            CM_SRV_DOMAIN_PREF_CS_ONLY;

    pref_ptr->network_sel_mode_pref  = local_net_sel_pref;
    pref_ptr->mode_pref              = mode_pref;
    pref_ptr->srv_domain_pref        = local_srv_domain_pref;

    CM_MSG_HIGH_4_EXT(">>PH FORCE_PREF: HICPS: Removing 3GPP, sub %d mode_pref %d srv_domain_pref %d network_sel_mode_pref %d",
                      asubs_id,
                      pref_ptr->mode_pref,
    ...
    if ( pm_orig_obj == CM_ACT_TYPE_POLICYMAN_RESTRICT && ph_orig_ptr != NULL)
    {
      /*Force the current main stack srv domain pref and net sel mode pref back to the phone object*/

      pref_ptr->network_sel_mode_pref  = main_stack_pref->network_sel_mode_pref;
      pref_ptr->srv_domain_pref        = main_stack_pref->srv_domain_pref;

      CM_MSG_HIGH_2(">>PH FORCE_PREF: HICPS Remove 3GPP : Update HICPS in Phone Object in FULL RAT MODE, srv domain pref %d, net sel mode pref %d", pref_ptr->srv_domain_pref, pref_ptr->network_sel_mode_pref );
    ...
    cmph_activate_hicps_gwl_avoid_timer(asubs_id);
          ...
          )
  {
    ...
    if (ph_sub_ptr->is_mode_pref_none == TRUE)
    {
    ...
    CM_MSG_HIGH_4_EXT(">>PH FORCE_PREF: HICPS: Restoring 3GPP, sub %d mode_pref %d srv_domain_pref %d network_sel_mode_pref %d",
                      asubs_id,
                      pref_ptr->mode_pref,
                      pref_ptr->srv_domain_pref,
                      pref_ptr->network_sel_mode_pref,
    ...
    if ( pm_orig_obj == CM_ACT_TYPE_POLICYMAN_RESTRICT && ph_orig_ptr != NULL)
    {
      ...
      CM_MSG_HIGH_2(">>PH FORCE_PREF: HICPS Restore 3GPP : Exit HICPS in Phone Object in FULL RAT MODE, srv domain pref %d, net sel mode pref %d", pref_ptr->srv_domain_pref, pref_ptr->network_sel_mode_pref );
  ...
  */
           ...
           &&
           (!cmcall_check_if_no_srv( asubs_id, FALSE ) &&
            cmss_get_info_sys_mode(asubs_id, MM_STACK_0) == SYS_SYS_MODE_CDMA)
         )
  {
    ...
    CM_MSG_HIGH_4(">>PH FORCE_PREF: HICPS: Restoring 3GPP, LTE BSR 1 sec sub %d mode_pref %d srv_domain_pref %d network_sel_mode_pref %d",
                  asubs_id,
                  pref_ptr->mode_pref,
                  pref_ptr->srv_domain_pref,
                  pref_ptr->network_sel_mode_pref);
    ...
    *cmph_get_is_kicked_hybr_ptr() = FALSE;
    ...
    if(ph_sub_ptr->shorten_bsr_timer < curr_uptime)
    {
      ph_sub_ptr->shorten_bsr_timer = curr_uptime+3;
      CM_MSG_HIGH_1("start shorten bsr timer=%d",ph_sub_ptr->shorten_bsr_timer );
    }
...
)
{
  ...
  as_id = state_machine->asubs_id;

  if ((ph_sub_ptr = cmph_get_sub_ptr(as_id)) == NULL)
  {
    return;
  }

  mm_id = cmph_map_sub_stk_to_id(as_id, MSC_STK_ID_TO_MM_STK_ID(stack_idx));
  ph_obj = cmtask_orig_para_search_act_id(mm_id, (cm_act_id_type)cmph_ptr());

  if (ph_obj == NULL)
  {
    CM_MSG_HIGH_2("Error: ph_obj null ptr sub %d stk %d", as_id, stack_idx);
    return;
  }
  ...
  pref_info.mode_pref              = ph_sub_ptr->pref_info.mode_pref ;
  pref_info.band_pref              = ph_sub_ptr->pref_info.band_pref ;
  pref_info.lte_band_pref          = ph_sub_ptr->pref_info.lte_band_pref ;
  pref_info.tds_band_pref          = ph_sub_ptr->pref_info.tds_band_pref;
  pref_info.prl_pref               = ph_sub_ptr->pref_info.prl_pref;
  pref_info.roam_pref              = ph_sub_ptr->pref_info.roam_pref;
  pref_info.srv_domain_pref        = ph_sub_ptr->pref_info.srv_domain_pref;
  pref_info.acq_order_pref         = ph_sub_ptr->pref_info.acq_order_pref;
  pref_info.rat_acq_order_ptr      = cmph_get_acq_pri_order_per_sub(as_id);
  ...
  pref_info.hybr1_pref.hybr_mode_pref = ph_sub_ptr->pref_info.mode_pref;
...
)
{
  ...
  mode = cmutil_map_sys_mode_mask_to_sd_mode_pref(pm_mode);
      ...
      for (stack_idx = 0; stack_idx < CMMSC_AUTO_STACK_NUM; stack_idx ++)
      {
        ...
        */
        state_machine->stack[stack_idx].capablity.mode =
          SD_GET_COMMON_MODE(mode, (op_mode_max_cap_table[table_idx]).max_cap[stack_idx]);
        ...
        /* Remove LTE if power_save_mode is true */
        if (state_machine->stack_common_info.power_save_mode ||
          state_machine->stack_common_info.non_dds_lte_removal)
        {
          CM_MSG_HIGH_3("sub %d Removing LTE from capability: power_save %d, LTE_removal %d",
                           state_machine->asubs_id,
                           state_machine->stack_common_info.power_save_mode,
                           state_machine->stack_common_info.non_dds_lte_removal);
          state_machine->stack[stack_idx].capablity.mode =
            sd_misc_remove_mode_pref(
                state_machine->stack[stack_idx].capablity.mode,
        ...
        state_machine->stack[stack_idx].capablity.domain   = domain;
...
{
  size_t                    index = 0;
  boolean                   skip_3gpp2_cap_emerg = FALSE;
  ...
  if (state_machine->stack_common_info.ue_domain_capability == SYS_SRV_DOMAIN_CS_VOLTE)
  {
    CM_MSG_HIGH_0("DUAL_LTE: domain_cap CS_VOLTE, e911 call on non dds  sub");
    state_machine->stack[0].capablity.domain = SYS_SRV_DOMAIN_CS_VOLTE;
  }
  else
  {
    state_machine->stack[0].capablity.domain    = SYS_SRV_DOMAIN_CS_PS;
  }

  skip_3gpp2_cap_emerg 
    = (policyman_get_subs_boolean_value(
            "cm:skip_3gpp2_cap_emerg", 
            state_machine->asubs_id, 
            &skip_3gpp2_cap_emerg) 
      && skip_3gpp2_cap_emerg);

  CM_MSG_HIGH_2("E911:skip_3gpp2_cap_emerg %d, sub %d", 
                  skip_3gpp2_cap_emerg, 
                  state_machine->asubs_id);
  
  if(skip_3gpp2_cap_emerg)
  {
    state_machine->stack[0].capablity.mode
                       &= SD_SS_MODE_PREF_ANY_BUT_CDMA_HDR;
  }
...
)
{
  ...
  switch(ue_mode)
  {
    ...
    case SYS_UE_MODE_1XSRLTE_CSFB_PREFERRED:
      ...
      if(cmsoa_get_current_sv_oprt_state(state_machine->asubs_id) == CMSOA_SV_OPRT_MODE_SVLTE)
      {
        return FALSE;
      }
      else
...
)
{
  ...
  cm_sub_level_policy_s_type *pm_sub_ptr = cmpm_sub_ptr(state_machine->asubs_id);
  ...
  if(!cmpmprx_is_device_mask_set(PM_CHG_MAX_MODE_CAP,NULL) &&
      cmpmprx_get_ue_mode(state_machine->asubs_id,&curr_ue_mode)== TRUE)
  {
    if( curr_ue_mode != state_machine->stack_common_info.ue_mode )
    {
      ...
      {
        cmsoa_srlte_ue_mode_update(curr_ue_mode, state_machine->asubs_id);
      }
#endif
      state_machine->stack_common_info.ue_mode = curr_ue_mode;
      state_machine->stack_common_info.ue_mode_forced = FALSE;
      /* Do not reset acq fail if it is Full RAT/Limited */
      if(pm_sub_ptr->policy_items.service_scan_mode != POLICYMAN_SVC_MODE_FULL_RAT &&
         pm_sub_ptr->policy_items.service_scan_mode != POLICYMAN_SVC_MODE_LIMITED)
      {
      policyman_reset_acq_fail(state_machine->asubs_id, stackMask);
        CM_MSG_HIGH_2("Reset acq_fail on stack %d, sub %d", stackMask, state_machine->asubs_id);
      }
    }
  }
}
...
FUNCTION cmmsc_auto_evaluate_cap_update_required_sxlte

DESCRIPTION
  This function determines if a capability update should be notified to NAS through a PS
  detach or TAU due to a capability change on the LTE stack
...
===========================================================================*/
void cmmsc_auto_evaluate_cap_update_required_sxlte(
  cmmsc_state_machine_s_type  *state_machine
)
{
  cmmsc_stack_s_type         *old_stack_pointer;
  cmmsc_stack_s_type         *new_stack_pointer;
  ...
  /* Determine which stacks are the old/new LTE stacks based on if hybr2 stack is up */
  if(cmmsc_auto_is_hybr2_on(state_machine->prev_op_mode))
  {
    old_stack_pointer = &(state_machine->stack[2]);
  }
  else
  {
    old_stack_pointer = &(state_machine->stack[0]);
  }

  if(cmmsc_auto_is_hybr2_on(state_machine->op_mode))
  {
    new_stack_pointer = &(state_machine->stack[2]);
  }
  else
  {
    new_stack_pointer = &(state_machine->stack[0]);
  }

  ph_mode      = cmph_map_cm_mode_pref_to_sd_mode_pref(CMPH_SS_MODE_PREF(asubs_id, ph_ptr));
  ph_band      = cmutil_map_cm_band_pref_to_sd_band_pref(CMPH_SS_BAND_PREF(asubs_id, ph_ptr));
  ph_lte_band  = CMPH_SS_LTE_BAND_PREF(asubs_id, ph_ptr);
  ph_tds_band  = cmutil_map_cm_tds_band_pref_to_sd_tds_band_pref(CMPH_SS_TDS_BAND_PREF(asubs_id, ph_ptr));

  old_mode     = old_stack_pointer->prev_registration_capability.mode;
  old_band     = old_stack_pointer->prev_registration_capability.band;
  old_lte_band = old_stack_pointer->prev_registration_capability.lte_band;
  old_tds_band = old_stack_pointer->prev_registration_capability.tds_band;

  /* When the hybr2 stack is up and the sub feature mode is SVLTE/SRLTE,
  ** must consider HDR as part of the old LTE capability
  */
  if(cmmsc_auto_is_hybr2_on(state_machine->prev_op_mode) &&
      SD_GET_COMMON_MODE(state_machine->stack[0].prev_registration_capability.mode,
                         SD_SS_MODE_PREF_HDR) == SD_SS_MODE_PREF_HDR
    )
  {
    old_mode = SD_MODE_ADD(old_mode, SD_SS_MODE_PREF_HDR);
  }

  /* If SD added LTE  (mode or band) for full service during an E911 call even though lte was not part of the
  ** idle capability, add lte to the old capability
  */
  if(ph_sub_ptr->ps_pref.e911_lte_full_srv_added.full_srv_added)
  {
    if(ph_sub_ptr->ps_pref.e911_lte_full_srv_added.mode == SYS_SYS_MODE_LTE )
      {
    old_mode = SD_MODE_ADD(old_mode, SD_SS_MODE_PREF_LTE);
    SYS_LTE_BAND_MASK_ADD_MASK(&old_lte_band, &(ph_sub_ptr->ps_pref.e911_lte_full_srv_added.lte_band));
    ph_sub_ptr->ps_pref.e911_lte_full_srv_added.full_srv_added = FALSE;
      }
  }

  new_mode     = SD_GET_COMMON_MODE(ph_mode, new_stack_pointer->capablity.mode);
  new_band     = SD_GET_COMMON_BAND(ph_band, new_stack_pointer->capablity.band);
  new_lte_band = SYS_LTE_BAND_MASK_COMMON(&ph_lte_band, &(new_stack_pointer->capablity.lte_band));
  new_tds_band = SD_GET_COMMON_BAND(ph_tds_band, new_stack_pointer->capablity.tds_band);

  /* When the hybr2 stack is up and the sub feature mode is SVLTE/SRLTE,
  ** must consider HDR as part of the new LTE capability
  */
  if(cmmsc_auto_is_hybr2_on(state_machine->op_mode)&&
      SD_GET_COMMON_MODE(ph_mode, SD_SS_MODE_PREF_HDR) == SD_SS_MODE_PREF_HDR
    )
  {
    new_mode = SD_MODE_ADD(new_mode, SD_SS_MODE_PREF_HDR);
  }

  /* When the hybr2 stack is up and the sub feature mode is SRLTE,
  ** must consider CDMA as part of the new LTE capability
  */
  if(cmph_is_subs_feature_mode_srlte(state_machine->asubs_id) &&
      cmmsc_auto_is_hybr2_on(state_machine->op_mode))
  {
    new_mode = SD_MODE_ADD(new_mode, SD_SS_MODE_PREF_CDMA);
  }

  if(old_mode != new_mode)
  {
     CM_MSG_MED_4("PS_DETACH_EVAL: sub %d, ph_mode 0x%x old mode 0x%x, new mode 0x%x",
                  asubs_id, ph_mode, old_mode, new_mode);
  }
  if(old_band != new_band 
      || old_tds_band != new_tds_band)
  {
    CM_MSG_MED_6("PS_DETACH_EVAL: old bands: cgw 0x%08x %08x. tds 0x%08x %08x, is_band_diff %d, is_tds_band_diff %d",
                    QWORD_HIGH(old_band),
                    QWORD_LOW(old_band),
    ...
    CM_MSG_MED_4("PS_DETACH_EVAL: new bands: cgw 0x%08x %08x. tds 0x%08x %08x",
                 QWORD_HIGH(new_band),
                 QWORD_LOW(new_band),
...
)
{
  ...
  if(cmph_is_subs_feature_mode_srlte(state_machine->asubs_id) &&
    ...
    ** (A) to SWITCH_UE_MODE for:
    **      CSFB -> CSFB_PREF
    **      CSFB_PREF (single stack) -> CSFB
    **      This helps SD to isolate this case to return continue
    ** (B) to PM_UE_MODE_CHG for:
    **       for cases where ue mode is changed by PM but UE is not performing any switches
    **       but number of stacks have changed. Effectively, it should be set in 
    **       CSFB_PREF(dual stack) -> CSFB transition.
    **     
    */
    ...
    {
      pref_info->pref_update_reason = SD_SS_PREF_UPDATE_REASON_SWITCH_UE_MODE_NO_STK_CHG;
      CM_MSG_HIGH_0("srlte: ue_mode_chg hybr2 not toggled");
    }
        else if(!(pref_info->pref_update_reason == SD_SS_PREF_UPDATE_REASON_SWITCH_SVLTE ||
        ...
        {
          pref_info->pref_update_reason = SD_SS_PREF_UPDATE_REASON_PM_UE_MODE_CHG;
          CM_MSG_HIGH_0("srlte: ue_mode_chg from csfb_pref dual stack to csfb single stack");
        }
  }
}
...
FUNCTION cmmsc_auto_check_and_unblock_1x_sxlte_plmn
...
===========================================================================*/
void cmmsc_auto_check_and_unblock_1x_sxlte_plmn(
  sys_modem_as_id_e_type              asubs_id,
  boolean                             unblock,
  boolean                             *is_fplmn_sxlte
)
{
  ...
  CM_MSG_HIGH_2("local_fplmn_list.length %d asubs_id:%d",local_fplmn_list->length, asubs_id);

  if ((local_fplmn_list != NULL) && (local_fplmn_list->length > 0))
  {
    boolean is_1xsrlte_mcc = FALSE;

    for(loop=0 ; loop<(local_fplmn_list->length) ; loop++)
    {
      if(policyman_svd_possible_on_plmn_per_subs(&(local_fplmn_list->plmn[loop]),
          ...
          POLICYMAN_STATUS_SUCCESS)
      {
        CM_MSG_HIGH_4("SRLTE: UNBLOCK_PLMNs: PLMN:0x%x 0x%x 0x%x.  is_1xsrlte_mcc:%d",
                      ...
                      local_fplmn_list->plmn[loop].identity[2],
                      is_1xsrlte_mcc);

        if(is_1xsrlte_mcc == TRUE && unblock == TRUE)
        {
          cmph_reg_send_unblock_plmn_req(local_fplmn_list->plmn[loop],
                                         asubs_id,
                                         SYS_BLOCK_PLMN_CAUSE_VOICE_NOT_AVAILABLE);
        }
        if(is_fplmn_sxlte != NULL)
        {
          *is_fplmn_sxlte=is_1xsrlte_mcc;
        }
      }
    }
  }
  else if((unblock == TRUE) && cmph_is_ssim() && cmph_is_subs_feature_mode_svlte(asubs_id) &&
          (cmsoa_state != NULL ) && (cmsoa_state->info.is_activated))
  {
    node_ptr = (cmsoa_svlte_plmn_id_node_s_type *)q_check(&(cmsoa_state->config.svlte_plmn_list));
    ...
    if( q_counter > 0)
    {
      while(node_ptr != NULL)
      {
        ...
        cmph_reg_send_unblock_plmn_req(node_ptr->plmn,
                                       asubs_id,
                                       SYS_BLOCK_PLMN_CAUSE_VOICE_NOT_AVAILABLE);
        node_ptr = q_next(&(cmsoa_state->config.svlte_plmn_list), &(node_ptr->link));
      }
    }
  }

  if ((unblock == TRUE) && cmreg_sub_ptr)
  {
...
===========================================================================*/
boolean   cmmsc_auto_update_op_mode (
  cmmsc_state_machine_s_type  *state_machine,
...
{
  cmmsc_state_machine_op_mode_e_type  new_op_mode = CMMSC_OP_MODE_AUTO_MAIN;
  sys_ue_mode_e_type old_ue_mode = state_machine->stack_common_info.ue_mode;
  boolean is_op_mode_chgd = FALSE;
  #ifdef FEATURE_CM_LTE
  cmsds_sub_prop_s_type  *sds_sub_prop_ptr = cmsds_get_sub_prop_ptr(state_machine->asubs_id);
  #endif
...
#if (defined(FEATURE_LTE_TO_1X) && defined(FEATURE_MMODE_SC_SVLTE)) || defined(FEATURE_MMODE_DYNAMIC_SV_OPERATION_SWITCH)
  cmsoa_state_sub_info_s_type *cmsoa_state = cmsoa_state_sub_info_ptr(asubs_id);
  boolean is_stack_2_switched_on = FALSE;
  boolean is_stack_2_switched_off = FALSE;
  ...
  {
    CM_MSG_HIGH_5_ASID("OP_MODE: is_new_policy_tobe_read %d, is_cdma_sub_avail %d, is_stk1_allowed %d, is_stk2_allowed %d",
                       state_machine->asubs_id,
                       is_new_policy_tobe_read,
                       is_cdma_sub_avail, is_hybr1_allowed, is_hybr2_allowed);
  }
...
#if (defined(FEATURE_LTE_TO_1X) && defined(FEATURE_MMODE_SC_SVLTE)) || defined(FEATURE_MMODE_DYNAMIC_SV_OPERATION_SWITCH)
  is_stack_2_switched_on = cmmsc_auto_is_hybr2_switched_on(state_machine->op_mode, new_op_mode);
  is_stack_2_switched_off = cmmsc_auto_is_hybr2_switched_off(state_machine->op_mode, new_op_mode);
#endif
  ...
  */
...
#if defined(FEATURE_LTE_TO_1X) && defined(FEATURE_MMODE_SC_SVLTE)
  {
    ...
    {
      if(is_stack_2_switched_on &&
          (!cmmsc_auto_is_ue_mode_csfb(state_machine)))
      {
        cmmsc_auto_check_and_unblock_1x_sxlte_plmn(state_machine->asubs_id,TRUE,NULL);
  ...
  */
  ...
  if(pref_info != NULL &&
      ...
      ((is_new_policy_tobe_read &&
        (is_stack_2_switched_on || is_stack_2_switched_off))
       ||
    ...
    )
  {
    ...
    CM_MSG_HIGH_3_ASID("CM->EFS: write current sub mode: %d, pref_update_reas: %d",
                       state_machine->asubs_id, cmsoa_state->config.curr_sv_oprt_mode,
                       pref_info->pref_update_reason);

    cmsoa_write_soa_config_efs(&temp_config, asubs_id);
  }

  if(is_stack_2_switched_off)
  {
    /* clear switch delay timer if stack 2 is switched off */
    if(cmsoa_state->info.switch_delay_reason == CMSOA_SV_SWITCH_DELAY_REAS_PINGPONG ||
  ...
  if(is_stack_2_switched_on)
  {
    /* arm switch delay timer if stack 2 is switched on*/
    if(cmsoa_state->info.switch_delay_timer== 0 &&
        cmsoa_state->config.sxlte_timers.switch_pingpong_guard_timer > 0)
    {
      cmsoa_arm_timer(CMSOA_SV_OPRT_MODE_NONE,
  ...
  /* Update op_mode */
  is_op_mode_chgd = cmmsc_state_machine_op_mode_update(state_machine, new_op_mode);

  if(state_machine->asubs_id <= SYS_MODEM_AS_ID_NONE ||
      state_machine->asubs_id >= MAX_SIMS)
  {
...
)
{
  ...
  if(cmph_is_subs_feature_mode_srlte(state_machine->asubs_id))
  {
        ...
        cmmsc_auto_is_hybr2_switched_off(old_op_mode, new_op_mode))
    {
      ...
      CM_MSG_HIGH_2("srlte: is_force_reg_pending %d ue_mode_forced %d",
...
)
{
  ...
  local_mm_id.asubs_id = state_machine->asubs_id;
  local_mm_id.stk_id = MSC_STK_ID_TO_MM_STK_ID(stack_idx);
  ...
  if(!cmtask_is_valid_mm_id_for_prio_q(local_mm_id))
  {
    return FALSE;
  }
  return TRUE;
...
boolean cmmsc_auto_is_hybr_gwl_allowed(
  ...
  */
  ...
  boolean                          is_check_ue_mode
  /* Do we need to check for ue_mode and sv_state
  */
)
{
  /* If ue mode is csfb, not allow hybr2 */
  boolean                is_ue_mode_csfb = FALSE;
  ...
  boolean                is_cdma_valid_mode_pref = cmph_is_valid_mode_pref(
        CM_MODE_PREF_CDMA_ONLY, asubs_id);

  boolean                is_lte_valid_mode_pref = cmph_is_valid_mode_pref(
        CM_MODE_PREF_LTE_ONLY, asubs_id);
  ...
  if(!cmutil_is_as_id_valid(asubs_id)) return FALSE;

  /* Hybr2 is not allowed for non DDS sub */
  if(asubs_id != cmph_ptr()->device_prop.msim_prop.new_default_data_subs) 
  {
    return FALSE;
  }

  if(is_check_ue_mode == TRUE)
  {
    is_ue_mode_csfb = cmmsc_auto_is_ue_mode_csfb(state_machine);
  }
  ...
  cmph_get_comm_mode_band_capab_with_pm(&comm_mode_cap, &comm_band_cap, &comm_lte_band_cap, &comm_tds_band_cap, asubs_id);
  ...
  CM_MSG_HIGH_5("IS_HYBR2_ALLOWED: sfmode1=%d, ue_mode=%d hybr_pref=%d, is_cdma_subsc_avail=%d, oprt_mode=%d",
  ...
  switch(hybr_pref)
  {
    case SD_SS_HYBR_PREF_CDMA__LTE__HDR:
      if(SD_MODE_CONTAIN(comm_mode_cap, SD_SS_MODE_PREF_CDMA) &&
    ...
    case SD_SS_HYBR_PREF_CDMA_HDR:
      if(SD_MODE_CONTAIN(comm_mode_cap, SD_SS_MODE_PREF_CDMA) &&
         SD_MODE_CONTAIN(comm_mode_cap, SD_SS_MODE_PREF_LTE) &&
          ...
          is_cdma_subsc_avail)
      {
        /* For fmode=SRLTE & ue-mode=SRLTE_ONLY, ignore hybr_pref and enable HYBR2 stack */
...
)
{
  ...
  cm_mm_ss_info_gen_stack_info_s_type *ss_all_stack_1_info_ptr;
  mm_sub_stk_id_s_type stk1_mm_id = cmph_map_sub_stk_to_id(asubs_id, MM_STACK_1);
  sys_srv_status_e_type srv_status_stk0 = cmss_get_info_srv_status(asubs_id, MM_STACK_0);

  if(ss_misc_sub_ptr == NULL || ss_info_sub_prop_ptr == NULL ||
      ss_misc_sub_ptr->stack_info[MM_STACK_0].gen_info == NULL)
  {
    return FALSE;
  }
  ss_all_stack_1_info_ptr = ss_info_sub_prop_ptr->stack_info[MM_STACK_1].gen_info;
  ...
  if ( !sd_misc_is_mode_pref(cmmsc_get_stk_mode_cap(stk1_mm_id),
                             SD_SS_MODE_PREF_HDR))
  {
  ...
  */
  if ( ss_all_stack_1_info_ptr != NULL && cmss_is_stack1_operational(asubs_id) &&
       sys_srv_status_is_srv( ss_all_stack_1_info_ptr->srv_status ) )
  {
    return TRUE;
  }
  ...
  if ( cmss_is_stack1_operational(asubs_id) &&
       sd_misc_is_opt_ho_lte_do(asubs_id))
  {
    CM_MSG_HIGH_0("OPT HO: allow hybr hdr data call");
    return TRUE;
  }

  /* Get hdr stack (stack 1) state */
  hybr1_state = cmmsimc_get_msc_stack_state(stk1_mm_id);
  CM_MSG_HIGH_3_ASID("STK1_ALLOWED: state %d, is_colloc %d",
                     asubs_id, hybr1_state,
  ...
  else if(hybr1_state == CMMSC_STACK_STATE_HDR_EXT_NO_SRV || !cmss_is_stack1_operational(asubs_id))
  {
    return FALSE;
  }
  else if(sys_srv_status_is_srv(srv_status_stk0) &&
          !ss_misc_sub_ptr->sub_info.gpp2_sub_info.is_colloc )
  {
    return FALSE;
  }
...
)
{
  ...
  else if (cmtask_get_targeted_num_stacks(state_machine->asubs_id) < 2)
  {
    *is_hybr1_allowed = FALSE;
    *is_hybr2_allowed = FALSE;
  }
  else
  {
    ...
    if (cmtask_get_targeted_num_stacks(state_machine->asubs_id) < 3)
    {
      *is_hybr2_allowed = FALSE;
    }
    else
    {
      *is_hybr2_allowed = cmmsc_auto_is_hybr_gwl_allowed(state_machine,
                          hybr_pref,
                          is_cdma_sub_avail,
                          TRUE);
    }

    *is_hybr1_allowed = cmmsc_auto_is_hybr_allowed(common_mode, hybr_pref);

    CM_MSG_HIGH_5_EXT("OP_MODE: mode_pref 0x%x & PM mode_cap 0x%x = 0x%x, is_hybr1_allowed %d, is_hybr2_allowed %d",
                      mode_pref, mode, common_mode, *is_hybr1_allowed, *is_hybr2_allowed, (state_machine->asubs_id+1));
...
{
  state_machine->stack_common_info.gpp2_info.is_hybr1_disabled = is_hybr1_disabled;
}
...
{
  return state_machine->stack_common_info.gpp2_info.is_hybr1_disabled;
}
...
)
{
  ...
  state_machine->stack_common_info.gpp2_info.stack_kick.is_hybr_2_kick_pending = is_pending;
}
...
{
  if(state_machine == NULL) return;
  state_machine->stack_common_info.gpp2_info.stack_kick.is_kick_hdr_pending = is_kick_hdr_pending;
}
...
{
  if(state_machine == NULL) return;
  state_machine->stack_common_info.gpp2_info.stack_kick.is_stack_sync_bsr_pending =
...
{
  if(state_machine == NULL) return;
  state_machine->stack_common_info.gpp2_info.stack_kick.is_kick_gw = is_kick_gw;
}
...
)
{
  ...
  return state_machine->stack_common_info.gpp2_info.stack_kick.is_kick_hdr_pending;
}
...
{
  if(state_machine == NULL) return FALSE;
  return state_machine->stack_common_info.gpp2_info.stack_kick.is_hybr_2_kick_pending;
}
...
{
  if(state_machine == NULL) return FALSE;
  return state_machine->stack_common_info.gpp2_info.stack_kick.is_stack_sync_bsr_pending;
}
...
{
  if(state_machine == NULL) return FALSE;
  return state_machine->stack_common_info.gpp2_info.stack_kick.is_kick_gw;
}
...
)
{
  ...
  return state_machine->stack_common_info.gpp2_info.timer.acq_hybr_1_uptime;
}
...
{
  if(state_machine == NULL) return 0;
  return state_machine->stack_common_info.gpp2_info.timer.lost_main_uptime;
}
...
{
  if(state_machine == NULL) return 0;
  return state_machine->stack_common_info.gpp2_info.timer.avoid_hybr_hdr_uptime;
}
...
{
  if(state_machine == NULL) return;
  if(state_machine->stack_common_info.gpp2_info.timer.acq_hybr_1_uptime != uptime)
  {
    state_machine->stack_common_info.gpp2_info.timer.acq_hybr_1_uptime = uptime;
...
{
  if(state_machine == NULL) return;
  if(uptime != state_machine->stack_common_info.gpp2_info.timer.lost_main_uptime)
  {

    state_machine->stack_common_info.gpp2_info.timer.lost_main_uptime = uptime;
    if(CMMSC_UPTIME_NONE != uptime)
    {
...
{
  if(state_machine == NULL) return;
  state_machine->stack_common_info.gpp2_info.timer.avoid_hybr_hdr_uptime = uptime;
...
{
  mm_sub_stk_id_s_type mm_id = cmph_map_sub_stk_to_id(msc->asubs_id, MM_STACK_1);
  cm_orig_q_s_type *hybr1_top = cmtask_orig_para_get_top(mm_id);
  ...
  if((hybr1_top->act_type != CM_ACT_TYPE_PH_OBJ &&
      ...
      ||(cmregprx_get_state(cmph_map_sub_stk_to_id(msc->asubs_id, MM_STACK_0))
         == CMREGPRX_STATE_ACTIVE) ||
      (cmregprx_get_state(cmph_map_sub_stk_to_id(msc->asubs_id, MM_STACK_2))
       == CMREGPRX_STATE_ACTIVE)
#endif
    )
  {
...
{
  mm_sub_stk_id_s_type  mm_id = cmph_map_sub_stk_to_id(msc->asubs_id, MM_STACK_0);
  cm_orig_q_s_type     *main_top = cmtask_orig_para_search_act_type(mm_id, CM_ACT_TYPE_PH_OBJ);

  if(main_top == NULL)
  {
    CM_ERR_0("Top_ptr is NULL for stack_0");
    return FALSE;
  }

  return cmcall_misc_is_mode_pref(main_top->orig->orig_mode_pref, CM_MODE_PREF_GWL);
}
...
)
{
  ...
  return cmcall_misc_is_mode_pref(ph_sub_ptr->pref_info.mode_pref,
                                  CM_MODE_PREF_GWL );
}
...
)
{
  ...
  mm_id = cmph_map_sub_stk_to_id(msc->asubs_id, MM_STACK_1);
  hybr1_top = cmtask_orig_para_get_top(mm_id);
  if(hybr1_top == NULL)
  {
    CM_ERR_0("Top ptr is NULL for HYBR1 stack");
    return;
  ...
  is_hdr_present = cmcall_misc_is_mode_pref(hybr1_top->orig->orig_mode_pref,
  ...
  if(is_hdr_present && is_3gpp_ph_obj)
  {
    cmmsc_auto_update_gwl_operation(FALSE,
                                    ...
                                    CM_MODE_PREF_GWL,
                                    FALSE,
                                    cmph_map_sub_stk_to_id(msc->asubs_id, 0));
  }
}
...
)
{
  ...
  cmss_misc_gen_stack_info_s_type *ss_misc_stk1_ptr;
  sd_si_info_s_type *ss_true_stk1_info_ptr;

  /* Exit if 3GPP2 is not allowed on the sub */
  if (msc == NULL || cmmsc_get_1x_sub() != msc->asubs_id)
  {
    return;
  }
  mm_id = cmph_map_sub_stk_to_id(msc->asubs_id, MM_STACK_1);
  ss_misc_sub_ptr = cmss_get_misc_sub_prop_ptr(msc->asubs_id);
  hybr1_top = cmtask_orig_para_get_top(mm_id);

  if(!cmutil_is_mm_id_valid(mm_id) || ss_misc_sub_ptr == NULL)
  {
    return;
  }
  ss_misc_stk1_ptr = ss_misc_sub_ptr->stack_info[MM_STACK_1].gen_info;
  ss_true_stk1_info_ptr = cmss_true_stack_info_ptr(cmph_map_sub_stk_to_id(msc->asubs_id, MM_STACK_1));

  if(hybr1_top == NULL || ss_misc_stk1_ptr == NULL)
  {
    CM_ERR_0("Top ptr is NULL for HYBR1 stack");
    return;
  }
  ...
  if (!cmph_is_subs_feature_mode_1x_sxlte(msc->asubs_id)   ||
      (cmph_is_subs_feature_mode_1x_sxlte(msc->asubs_id) &&
       (cmmsc_auto_is_kick_hdr_pending(msc) ||
        ...
        ((ss_misc_stk1_ptr->no_srv_uptime != CMSS_NO_SRV_UPTIME_NONE )&&
         (ss_true_stk1_info_ptr != NULL)&&
         (sys_srv_status_is_srv(ss_true_stk1_info_ptr->srv_status) == FALSE ))||
#endif
        (sys_srv_status_is_srv(cmss_get_info_srv_status(msc->asubs_id, MM_STACK_1)) == FALSE )))
     )
  {
...
)
{
  ...
  top = cmtask_orig_para_get_top(mm_id);

  switch(stack_idx)
  {
    case 1:
    {
      CM_MSG_MED_1("execute action is kick pending %d", *cmph_hdr_kick_pending());

      if(*cmph_hdr_kick_pending())
      {
        if(cmmsc_auto_is_kick_hdr_pending(msc) && cmmsc_is_in_srv_state(msc, 0))
        {
        ...
        pref_package.pref_reas = cmcall_is_hybr_hdr_orig(msc->asubs_id)?
                                 SD_SS_PREF_REAS_ORIG_START_PS:
                                 pref_package.pref_reas;

        pref_package.orig_mode = SD_SS_ORIG_MODE_COLLOC;
        pref_package.hybr_pref = cmutil_map_sd_hybr_pref_to_cm_hybr_pref(
    ...
    case 2:
    {
      boolean is_volte_stk2 = cmcall_is_there_volte_call_stack2(msc->asubs_id);
      /* do not kick stack 2 if emerg call active or in ECBM or stack 2 has volte call */
      if(ph_sub_ptr->emerg_call_prop.ph_state == CM_PH_STATE_EMERG_CB ||
          cmcall_emergency_call_active() ||
          is_volte_stk2)
      {
        CM_MSG_MED_3("asubs_id=%d, stack 2 kick, ph_state %d, is_volte_stack2 %d",
                     msc->asubs_id, ph_sub_ptr->emerg_call_prop.ph_state, is_volte_stk2);
        break;
      }
...
)
{
  ...
  mm_id = cmph_map_sub_stk_to_id(msc->asubs_id, MSC_STK_ID_TO_MM_STK_ID(stack_idx));

  /* Exit if 3GPP2 is not allowed on the sub */
  if (cmmsc_get_1x_sub() != msc->asubs_id)
  {
    return;
  }
  ...
  switch(stack_idx)
  {
    case 0:
    {
      cmmsc_auto_update_gwl_operation(TRUE,
                                      SD_SS_PREF_REAS_STACK_SYNC_UP,
                                      CM_MODE_PREF_GWL,
                                      TRUE,
                                      mm_id);
    ...
    case 2:
    {
      CM_MSG_MED_2_ASID("execute action is is_shorten_bsr %d",
                        msc->asubs_id, ph_sub_ptr->is_shorten_bsr);

      if(ph_sub_ptr->is_shorten_bsr)
      {
...
)
{
  ...
  if(cmph_is_get_networks_ongoing(mm_id.asubs_id) == TRUE &&
      ph_ptr->device_prop.mm_id_for_get_networks.asubs_id == msc->asubs_id)
  {
    CM_MSG_HIGH_1("get_networks ongoing on asubs_id=%d. skip restore 3gpp", mm_id.asubs_id);
    return;
  }
...
)
{
  ...
  mm_id = cmph_map_sub_stk_to_id(msc->asubs_id, MSC_STK_ID_TO_MM_STK_ID(stack_idx));
  top_ph = cmtask_orig_para_search_act_type(mm_id, CM_ACT_TYPE_PH_OBJ);
...
)
{
  ...
  if(ph_orig_top_ptr == NULL)
  {
    CM_ERR_2("Top_ptr NULL for sub %d stk %d",msc->asubs_id, stack_idx);
    return;
  }
...
{
  cmss_misc_sub_prop_s_type *ss_misc_sub_ptr;
  cmss_misc_gen_stack_info_s_type *ss_misc_stk1_ptr, *ss_misc_stk2_ptr;
  sd_si_info_s_type *ss_true_stk1_info_ptr, *ss_true_stk2_info_ptr;
  cmph_sub_info_s_type *ph_sub_ptr = NULL;

  if (!msc) return;
  if ((ph_sub_ptr = cmph_get_sub_ptr(msc->asubs_id)) == NULL) return;
  if ((ss_misc_sub_ptr = cmss_get_misc_sub_prop_ptr(msc->asubs_id)) == NULL) return;
  ...
  ss_misc_stk1_ptr = ss_misc_sub_ptr->stack_info[MM_STACK_1].gen_info;
  ss_misc_stk2_ptr = ss_misc_sub_ptr->stack_info[MM_STACK_2].gen_info;
  ss_true_stk1_info_ptr = cmss_true_stack_info_ptr(cmph_map_sub_stk_to_id(msc->asubs_id, MM_STACK_1));
  ss_true_stk2_info_ptr = cmss_true_stack_info_ptr(cmph_map_sub_stk_to_id(msc->asubs_id, MM_STACK_2));
  switch(stack_idx)
  {
    case 1:
    {
    ...
    case 2:
    {
      ...
      else if(!cmcall_misc_is_mode_pref(ph_sub_ptr->pref_info.mode_pref,
                                        CM_MODE_PREF_GSM_WCDMA_ONLY ))
      {
        if ( ((cmss_get_info_srv_status(msc->asubs_id, MM_STACK_2) == SYS_SRV_STATUS_PWR_SAVE) ||
              (ss_misc_stk2_ptr->no_srv_uptime != CMSS_NO_SRV_UPTIME_NONE &&
               ss_true_stk2_info_ptr != NULL &&
               ss_true_stk2_info_ptr->srv_status ==
    ...
    case 0:
    {
          ...
          msc->op_mode == CMMSC_OP_MODE_AUTO_MAIN_HYBR1_HYBR2)
      {
        (void)cmmsc_auto_update_gwl_operation(TRUE,
                                              ...
                                              CM_MODE_PREF_GWL,
                                              FALSE,
                                              cmph_map_sub_stk_to_id(msc->asubs_id, MM_STACK_0));
      }
...
)
{
  ...
  multimode_stack_e_type srv_ind_stk = srv->rpt_ptr->srv_ind_info.stack_id;
  ...
  switch(stack_idx)
  {
    case 1:
    {
      ...
      */
      ...
      if(kick_hdr)
      {
            ...
            !cmcall_is_mobile_in_hdr_call())
        {
          ...
          pref_package.hybr_pref
            = cmutil_map_sd_hybr_pref_to_cm_hybr_pref(ph_sub_ptr->sub_3gpp2_pref.int_hybr_pref);
          cmmsc_action_force_ph_ext(msc, &pref_package, stack_idx);
        }
        else
        {
      ...
      ss_true_stk1_info_ptr = cmss_true_stack_info_ptr(mm_id);
      if(ss_true_stk1_info_ptr != NULL &&
          !sys_srv_status_is_srv(ss_true_stk1_info_ptr->srv_status) &&
    ...
    case 2:
    {
      if(srv_ind_stk == MM_STACK_2)
      {
        cmss_misc_sub_prop_s_type *ss_misc_sub_ptr = cmss_get_misc_sub_prop_ptr(msc->asubs_id);
        cmss_misc_gen_stack_info_s_type *ss_misc_stk0_ptr, *ss_misc_stk1_ptr;
        sd_si_info_s_type *ss_true_stk0_info_ptr, *ss_true_stk1_info_ptr;
        if(!cmutil_is_as_id_valid(msc->asubs_id) || ss_misc_sub_ptr == NULL)
        {
          return;
        }
...
#if (defined (FEATURE_HDR_HYBRID))
        if(ss_misc_stk0_ptr != NULL && ss_misc_stk1_ptr != NULL)
        {
          CM_MSG_HIGH_2("hybr2 stack sync-up %d %d",
                        ss_misc_stk0_ptr->is_pref_srv_acq,
                        ss_misc_stk1_ptr->is_pref_srv_acq);
        }
#endif
        mm_id.stk_id = MM_STACK_0;
        ss_true_stk0_info_ptr = cmss_true_stack_info_ptr(mm_id);
        ss_true_stk1_info_ptr = cmss_true_stack_info_ptr(cmph_map_sub_stk_to_id(msc->asubs_id, MM_STACK_1));
        if ((ss_true_stk0_info_ptr != NULL &&
             sys_srv_status_is_full_srv( ss_true_stk0_info_ptr->srv_status )
             && ss_misc_stk0_ptr != NULL
             && ss_misc_stk0_ptr->is_pref_srv_acq == FALSE
            ) ||
            (ss_true_stk1_info_ptr != NULL &&
             sys_srv_status_is_full_srv( ss_true_stk1_info_ptr->srv_status )
#if (defined (FEATURE_HDR_HYBRID))
             && ss_misc_stk1_ptr != NULL
             && ss_misc_stk1_ptr->is_pref_srv_acq == FALSE
#endif
            ))
        {
      ...
      else
      {
        ...
        else if(ss_true_stk0_info_ptr != NULL &&
                sys_srv_status_is_srv(ss_true_stk0_info_ptr->srv_status) &&
                (is_realign_hdr || is_realign_lte))
        {
          cmmsc_action_force_top(msc, stack_idx, SD_SS_PREF_REAS_STACK_SYNC_UP_BSR);
        }
        else if(srv_ind_stk == MM_STACK_1)
        {
          cmmsc_action_force_top(msc, stack_idx, SD_SS_PREF_REAS_STACK_SYNC_UP_BSR);
        }
...
{
  cmmsc_event_acq_fail_s_type  acq_fail;
  int                          stack_idx = MM_STK_ID_TO_MSC_STK_ID(stk_id);
  mm_sub_stk_id_s_type         mm_id;
  cm_orig_q_s_type  *ph_orig_ptr;
  cmmsc_event_e_type           event = CMMSC_EVENT_NONE;
  cmph_sub_info_s_type *ph_sub_ptr = NULL;
  ...
  mm_id = cmph_map_sub_stk_to_id(msc->asubs_id, stk_id);
  ...
  if( ph_orig_ptr == NULL )
  {
    CM_ERR_2("ph_orig_ptr null ptr sub %d stk %d",
             msc->asubs_id, stk_id);
    return;
  }

  CM_MSG_HIGH_2("RPT->MSC: ACQ_FAIL sub %d stack = %d", msc->asubs_id, stk_id);
...
)
{
  ...
  cmmsc_event_e_type          event = CMMSC_EVENT_NONE;
  int                         stack_idx = MM_STK_ID_TO_MSC_STK_ID(rpt_ptr->srv_ind_info.stack_id);
  ...
  cmmsc_state_machine_s_type  *state_machine = cmmsimc_state_machine_ptr(msc->asubs_id);
  ...
  */
  if(mode == SYS_SYS_MODE_LTE &&
      ...
      ((cmmsc_auto_is_hybr2_on(state_machine->op_mode))?(stack_idx == 0):(stack_idx == 2)))
  {
  ...
  srv_evt.prev_srv_status_ss = prev_srv_status;
...
)
{
  ...
  while(call_ptr != NULL)
  {
    ...
    {
      CM_MSG_MED_3("lte data call call:%d, sub %d, stack %d",
                   call_ptr->call_id, call_ptr->mm_id.asubs_id, call_ptr->mm_id.stk_id);
