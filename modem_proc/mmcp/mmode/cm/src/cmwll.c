...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/mmode/cm/src/cmwll.c#1 $
...
)
{
  ...
  CM_MSG_HIGH_1( "Send cmd 0x%x to SM",sm_cmd_ptr->header.cmd_id);
...
)
{
  ...
  CM_MSG_HIGH_1( "Send cmd 0x%x to RABM",rabm_cmd_ptr->header.cmd_id);
...
)
{
  ...
  CM_MSG_HIGH_1( "Send cmd 0x%x to TC",tc_cmd_ptr->header.cmd_id);
...
===========================================================================*/
void cmwll_send_rrc_get_dch_cell_info_req(sys_modem_as_id_e_type asubs_id)
{
  ...
  CM_MSG_HIGH_1("DCH cell info sent on sub=%d",asubs_id);
  #ifdef FEATURE_SEGMENT_LOADING
  if(pi_mmode_wcdma != NULL)
  {
    IWCDMA_rrc_put_cm_cmd_sub(pi_mmode_wcdma, asubs_id,cmd_ptr );
  }
#else
  rrc_put_cm_cmd_sub (asubs_id, cmd_ptr);
#endif
#endif
}
