...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/mmode/cm/src/cmipsms.c#1 $
...
)
{
  ...
  CM_MSG_LOW_2("START cmipcall_rpt_proc, sub %d, cmd=%d",
               cmipapp_rpt_ptr->hdr.asubs_id, cmipapp_rpt_ptr->hdr.cmd);
  sms_sub_info_ptr = cmsms_sub_info_ptr(cmipapp_rpt_ptr->hdr.asubs_id);
  ...
  */
  switch( cmipapp_rpt_ptr->hdr.cmd )
  {
    ...
    case CM_IP_SMS_IND_MO_SMS_STATUS:
      ...
      */
      sms_sub_info_ptr->mo_status = cmipapp_rpt_ptr->ip_ind.ind.mo_sms_status.status;

      cmsms_event( sms_sub_info_ptr, CM_SMS_EVENT_MO_STATUS, cmipapp_rpt_ptr->hdr.asubs_id);

      break;

    case CM_IP_SMS_IND_MT_SMS:

      sms_sub_info_ptr->mt_sms_ptr = &cmipapp_rpt_ptr->ip_ind.ind.mt_sms;

      /* Send MT SMS to registered clients.
      */
      cmsms_event( sms_sub_info_ptr, CM_SMS_EVENT_MT_MSG, cmipapp_rpt_ptr->hdr.asubs_id);
...
)
{
  ...
  */
  ...
  cmd_ptr->app_id = cmipapp_find_active_app( cmd_ptr->sys_mode, CM_CALL_TYPE_SMS, asubs_id );
