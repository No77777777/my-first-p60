...
static boolean cmrpm_lte_get_pm_rpm_enabled_flag(boolean  *enabled_flag_ptr)
{
  ...
  */
  status = policyman_get_subs_boolean_value("cm:rpm_enabled",
           SYS_MODEM_AS_ID_1, enabled_flag_ptr);
...
boolean cmrpm_lte_init( void )
{
      ...
      (TRUE == efs_time_read_status) )
  {
    CM_MSG_HIGH_3("cmrpm_lte_init: is_rpm_enabled %d, is_pm_rpm_enabled %d, is_gcf_enabled %d",
                  read_rpm_efs_info.is_rpm_enabled, is_pm_rpm_enabled, mcfg_gcf_nv_get_status());
...
)
{
  ...
  CM_MSG_HIGH_3("process_app_reset: client_id %d, from mode %d -> mode %d",
                ph_cmd_ptr->client_id, cmph_ptr()->device_prop.ph_oprt_mode.oprt_mode, cmd_info_ptr->oprt_mode);
...
/*===========================================================================
  ...
  FUNCTION cmrpm_lte_handle_attach_complete_ind

  DESCRIPTION
    Process reports from NAS.
...
void cmrpm_lte_handle_attach_complete_ind(
...
)
{
  ...
  if(lte_rrc_get_3gpp_release_version(asubs_id) <= LTE_3GPP_REL10_MAX) 
    {
      /* If attach complete or tau complete indication is recieved from NAS for REL 10 or below:
...
/*===========================================================================
  ...
  FUNCTION cmrpm_lte_handle_tau_complete_ind
...
void cmrpm_lte_handle_tau_complete_ind(
...
)
{
  ...
  */
  ...
  CM_MSG_HIGH_1("DBG: 3gpp_rel_version %d", lte_rrc_get_3gpp_release_version(sd_rpt_ptr->hdr.asubs_id));
    ...
    */
    case CM_SRV_IND_INFO_F:
      if( (sd_rpt_ptr->srv_ind_info.si_info.srv_status == SYS_SRV_STATUS_SRV) &&
          ...
          (cmph_is_lte_capable_on_sub(sd_rpt_ptr->hdr.asubs_id)) &&
		  (lte_rrc_get_3gpp_release_version(sd_rpt_ptr->hdr.asubs_id) >= LTE_3GPP_REL11))
      {
        /* Reset all LTE RPM timer and counters */
        cmrpm_lte_reset_efs_info();
...
void cmrpm_lte_send_update_ftai_list(sys_rpm_ftai_list_action_e_type action)
{
  ...
  mm_ptr->cmd.cm_mm_update_ftai_list.as_id            = cmph_get_first_sub_with_lte_cap();
