...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/mmode/cm/src/cm.c#4 $
...
/* The ASID to VFR mapping
*/
sys_vfr_e_type cm_vfr_map[MAX_AS_IDS];
...
===========================================================================*/
static cm_bcmcs_cmd_s_type *cm_cmd_alloc_bcmcs_init(
...
)
{
  ...
  */
  ...
  bcmcs_cmd_ptr->asubs_id        = asubs_id;
...
#error code not present
#endif
cm_call_cmd_s_type *cm_cmd_alloc_call_init( void )
{
...
void cm_cmd_ph_init( cm_ph_cmd_s_type *ph_cmd_ptr)
{
  ...
  */
  ...
  cmd_info_ptr->network_band_pref     = CM_BAND_PREF_NONE;
  SYS_LTE_BAND_MASK_CLR_ALL_BANDS(&cmd_info_ptr->network_lte_band_pref);
  cmd_info_ptr->network_tds_band_pref = CM_BAND_PREF_NONE;
  cmd_info_ptr->network_list_type     = CM_NETWORK_LIST_NONE;
  cmd_info_ptr->ignore_manual_bst     = FALSE;
  ...
  */
  ...
  cmd_info_ptr->ui_ready_status         = FALSE;
...
#endif
  cmd_info_ptr->is_ch_removed_from_mode_pref = 0;
  cmd_info_ptr->is_data_call_pending_post_dds = FALSE;
}
#ifdef FEATURE_NON_DEMAND_PAGED_FUNCTION
#error code not present
#endif
...
#error code not present
#endif
cm_inband_cmd_s_type *cm_cmd_alloc_inband_init( void )
{
...
static cm_sups_cmd_s_type *cm_cmd_alloc_sups_init( void )
{
  ...
  */
  ...
  cmd_info_ptr->cmd_subs_id      = SYS_MODEM_AS_ID_NONE;
   cmd_info_ptr->fwd_type         = CM_FORWARD_TO_NUMBER;
  sups_cmd_ptr->cmd_info.is_all_call_fwd_sups_ext_req = 0;
  return sups_cmd_ptr;
...
boolean cm_dbm_cmd_mo_msg1 (
  ...
  */
  ...
  sys_modem_as_id_e_type             asubs_id
  /* Subscription on which the DB msg is to be sent. */
)
...
/*lint -esym(715, asubs_id) */
{
  ...
  */
  ...
  {
    if ((*return_call_id_ptr = cm_call_id_allocate_new(FALSE)) == CM_CALL_ID_INVALID)
    {
  ...
  */
  ...
  if ( cmd_err == CM_CALL_CMD_ERR_NOERR )
  {
    ...
    */
    ...
    CM_MSG_HIGH_1_EXT ("Emerg serv categ given = %d",
                       called_number->emerg_srv_categ, (asubs_id+1));

    if( cc_generic_params_ptr != NULL )
    {
      CM_MSG_HIGH_1_EXT ("FDN check to do = %d",
                         cc_generic_params_ptr->is_fdn_to_check, (asubs_id+1));
    ...
    if ( cdma_orig_params_ptr != NULL )
    {
      ...
      */
      if( srv_type == CM_SRV_TYPE_CDMA_SPECIFIC ||
          srv_type == CM_SRV_TYPE_CDMA_HDR || srv_type == CM_SRV_TYPE_CS_ONLY )
      {
      ...
      cmd_info_ptr->favored_hybr_pref = CM_HYBR_PREF_NO_CHANGE;
      ...
      */
      ...
      else if (cmutil_is_as_id_valid(asubs_id))
      {
        ...
        */
        if (ph_sub_ptr)
        {
          cmd_info_cdma_ptr->drs_bit = ph_sub_ptr->sub_3gpp2_pref.voice_drs_bit;
        }
      }
#endif /* FEATURE_IS2000 */
  ...
  if(ip_orig_params_ptr != NULL)
  {
    ...
    CM_MSG_HIGH_5("DIAL_CALL: em_type %d, Audio %x, Video %d RTT %d, call_composer incl %d",
                     ip_orig_params_ptr->call_attrib.em_type,
                     ip_orig_params_ptr->call_attrib.audio_attrib,
                     ip_orig_params_ptr->call_attrib.video_attrib,
                     ip_orig_params_ptr->call_attrib.rtt_mode,
                     ip_orig_params_ptr->call_attrib.call_composer_info.info_included);

    CM_MSG_HIGH_2("redial_reason %d  redial_mode %d ", ip_orig_params_ptr->call_attrib.redial_param.reason,
		           ip_orig_params_ptr->call_attrib.redial_param.mode);
    
    if (ip_orig_params_ptr->call_attrib.call_composer_info.info_included)
    {
      CM_MSG_HIGH_6("DIAL_CALL: CALL_COMPOSER: imp %d, subject %d, picture_url %d, loc incl %d, shape %d, sub %d",
                       ip_orig_params_ptr->call_attrib.call_composer_info.importance,
                       ip_orig_params_ptr->call_attrib.call_composer_info.subject,
                       ip_orig_params_ptr->call_attrib.call_composer_info.picture_url,
                       ip_orig_params_ptr->call_attrib.call_composer_info.geo_location.location_included,
                       ip_orig_params_ptr->call_attrib.call_composer_info.geo_location.geo_shape,
                       asubs_id);

      if(cmd_info_ptr->ip_call.ipcall_attrib_info.call_composer_info.picture_url != NULL)
      {
        ref_cnt_obj_add_ref(
            cmd_info_ptr->ip_call.ipcall_attrib_info.call_composer_info.picture_url);
      }
      if(cmd_info_ptr->ip_call.ipcall_attrib_info.call_composer_info.subject != NULL)
      {
        ref_cnt_obj_add_ref(
            cmd_info_ptr->ip_call.ipcall_attrib_info.call_composer_info.subject);
      }
    }
...
)
{
  ...
  switch( ans_params_ptr->info_type )
  {
    ...
    case CM_CALL_MODE_INFO_GW_CS:
    {
      ...
      if (cmph_ptr()->device_prop.qmss_enabled == TRUE)
      {
    ...
    case CM_CALL_MODE_INFO_IP:
      ...
      CM_MSG_HIGH_2 ("CALL_CMD_ANSWER pi value %d RTT %d", ans_params_ptr->ans_params.ip_ans.pi,
                     ans_params_ptr->ans_params.ip_ans.call_attrib.rtt_mode);
      break;
...
)
{
  ...
  */
  ...
  call_cmd_ptr->cmd_subs = cmd_info_ptr->lte_info.as_id;
  ...
  cmd_info_ptr->lte_info.esm_local_cause =
    act_bearer_rsp_ptr->esm_local_cause;
...
)
{
  ...
  cmd_info_ptr->lte_info.esm_local_cause =
    pdn_connectivity_abort_ptr->esm_local_cause;
...
)
{
  ...
  cmd_info_ptr->lte_info.esm_local_cause =
    pdn_disconnect_ptr->esm_local_cause;
...
)
{
  ...
  cmd_info_ptr->lte_info.esm_local_cause =
    res_alloc_abort_ptr->esm_local_cause;
...
)
{
  ...
  */
  ...
  cmd_info_ptr->lte_info.esm_local_cause =
    mod_bearer_rsp_ptr->esm_local_cause;
...
/*lint -esym(715,reestab_params_ptr) */
{
  ...
  */
  ...
  else
  {
    ...
    CM_MSG_HIGH_3( "DS->CM: RAB_REESTAB: sys_lte_reestab_calltype = %d, call_id=%d, eps_bearer_id %d", 
                      reestab_params_ptr->sys_lte_reestab_calltype,
                      call_id,
                      cmd_info_ptr->lte_info.eps_bearer_id);
#endif
...
/*lint -esym(715,call_id) */
{
  ...
  */
  ...
  cmd_mode_info_ptr->config_options = modify_params_ptr->protocol_config_options;
  #endif
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
...
FUNCTION cm_mm_call_cmd_ps_sig_release_per_subs

DESCRIPTION
  Request CM to release PS signaling per subs. 
...
===========================================================================*/
boolean cm_mm_call_cmd_ps_sig_release_per_subs(
...
)
{
  ...
  */
  ...
  if(sups_type == CM_CALL_SUPS_TYPE_SWITCH_SPEECH_CODEC)
  {
    ...
    cmd_info_ptr->ip_call.ip_sups_param.priority_codec = sups_params_ptr->ip_params.priority_codec;
  }
...
/*lint -esym(715, call_type) */
{
  ...
  */
  ...
  else if ((*return_call_id_ptr = cm_call_id_allocate_new(FALSE)) == CM_CALL_ID_INVALID )
  {
    cmd_err = CM_CALL_CMD_ERR_NO_CALL_ID_S;
  }
...
)
{
  ...
  CM_MSG_HIGH_5_EXT( "PH CMD - cm_ph_cmd_oprt_mode(), mode=%d, uptime=%d, client_id=%d, cmd_cb_func=%d, data_block_ptr=%d",
                     oprt_mode, time_get_uptime_secs(), client_id,
                     cmd_cb_func, data_block_ptr, 0); /*Device level information, sub value passed is 0 */
...
sys_oprt_mode_e_type cm_ph_get_oprt_mode( void )
{
  ...
  */
  return cmph_ptr()->device_prop.ph_oprt_mode.oprt_mode;
...
{
  return cm_ph_get_voice_domain_pref_per_sub(SYS_MODEM_AS_ID_1);
}
...
)
{
  ...
  */
  if (ph_sub_ptr)
  {
    return ph_sub_ptr->pref_info.voice_domain_pref;
  }
  else
  {
    return SYS_VOICE_DOMAIN_PREF_NONE;
  }
}
...
boolean cm_ph_cmd_meas_mode1(
  ...
  sys_modem_as_id_e_type asubs_id
  /* Subscription on which the command is to be processed. */
)
{
...
)
{
  ...
  sys_sel_pref_params.is_allow_buffering_cmd = FALSE;
...
)
{
  ...
  */
  ...
  cmd_info_ptr->is_allow_buffering_cmd = sys_sel_pref_params_ptr->is_allow_buffering_cmd;
  ...
  */
  CM_MSG_HIGH_6_EXT("sys_sel_pref(), sub %d mode_pref %d pref_term %d acq_order %d domain_pref %d net_sel_mode %d",
                    asubs_id,
                    cmd_info_ptr->mode_pref, cmd_info_ptr->pref_term, cmd_info_ptr->acq_order_pref,
                    cmd_info_ptr->srv_domain_pref, cmd_info_ptr->network_sel_mode_pref, (asubs_id+1));

  CM_MSG_HIGH_6_EXT("sys_sel_pref(), roam_pref %d, hybr_pref %d, prl_pref %d, req_id %u, CSG ID %d, CSG RAT %d",
                    cmd_info_ptr->roam_pref, cmd_info_ptr->hybr_pref, cmd_info_ptr->prl_pref,
                    req_id, cmd_info_ptr->csg_id, cmd_info_ptr->csg_rat, (asubs_id+1));

  CM_MSG_HIGH_3_EXT("sys_sel_pref(), ue_usage_setting %d, voice_domain_pref %d, lte_disbale_cause %d",
                    cmd_info_ptr->ue_usage_setting,cmd_info_ptr->voice_domain_pref,cmd_info_ptr->lte_disable_cause, (asubs_id+1));

  CM_MSG_HIGH_4_EXT("sys_sel_pref(),bands: cgw 0x%08x %08x , bands: tds 0x%08x %08x",
                    QWORD_HIGH(cmd_info_ptr->band_pref),
                    QWORD_LOW(cmd_info_ptr->band_pref),
                    QWORD_HIGH(cmd_info_ptr->tds_band_pref),
                    QWORD_LOW(cmd_info_ptr->tds_band_pref), (asubs_id+1) );

#if (LTE_BAND_NUM == 256)
  CM_MSG_MED_4_EXT("LTE bands 193_256: 0x%08x %08x, 129_192: 0x%08x %08x",
                   QWORD_HIGH(cmd_info_ptr->lte_band_pref.bits_193_256),
                   QWORD_LOW(cmd_info_ptr->lte_band_pref.bits_193_256),
                   QWORD_HIGH(cmd_info_ptr->lte_band_pref.bits_129_192),
                   QWORD_LOW(cmd_info_ptr->lte_band_pref.bits_129_192),
                   (asubs_id+1));
  CM_MSG_MED_4_EXT("LTE bands  65_128: 0x%08x %08x,  1_64:   0x%08x %08x",
                   QWORD_HIGH(cmd_info_ptr->lte_band_pref.bits_65_128),
                   QWORD_LOW(cmd_info_ptr->lte_band_pref.bits_65_128),
                   QWORD_HIGH(cmd_info_ptr->lte_band_pref.bits_1_64),
                   QWORD_LOW(cmd_info_ptr->lte_band_pref.bits_1_64),
                   (asubs_id+1));
#else
  CM_MSG_MED_2_EXT("LTE bands    1_64: 0x%08x %08x", QWORD_HIGH(cmd_info_ptr->lte_band_pref),
                   QWORD_LOW(cmd_info_ptr->lte_band_pref),
                   (asubs_id+1));
#endif
  ...
  CM_MSG_HIGH_4_EXT("PLMN - undefined %d pcs_digit %d. PLMN(%d-%d)",
                    plmn_id_is_undefined, mnc_includes_pcs_digit,
                    mcc, mnc, (asubs_id+1));
...
{
  return cm_ph_cmd_ddtm_pref1(cmd_cb_func,data_block_ptr,client_id,
                              ddtm_pref,ddtm_act,so_list_act,num_srv_opt,
                              srv_opt_list,SYS_MODEM_AS_ID_1);
}
...
===========================================================================*/
boolean cm_ph_cmd_block_srv_opt1(
  cm_ph_cmd_cb_f_type         cmd_cb_func,
  /*Client callback function. */
...
)
{
  ...
  */
  ...
  CM_MSG_HIGH_2 ("Block SO : sub%d, ddtm_num_srv_opt=%d",asubs_id, num_srv_opt);
...
===========================================================================*/
boolean cm_ph_cmd_answer_voice1(
...
===========================================================================*/
boolean cm_ph_cmd_packet_state1(
...
{
  return cm_ph_cmd_voc_activity1(cmd_cb_func,data_block_ptr,client,
                                 enable,voc_1,voc_2,voc_8,SYS_MODEM_AS_ID_1);
}
...
===========================================================================*/
boolean cm_ph_cmd_voc_activity1(
...
boolean cm_ph_cmd_perso_info_available(
  ...
  sys_modem_as_id_e_type          asubs_id,
  /*Subscription on which perso locked*/
...
)
{
  ...
  CM_MSG_HIGH_3( "CM_PH_CMD_PERSO_INFO_AVAILABLE, asubs_id=%d, cdma=%d, gwl=%d",
                 asubs_id, cdma_status,gwl_status);
...
FUNCTION cm_ph_cmd_subscription_available4
...
DESCRIPTION
  ...
  THIS WILL EVENTUALLY REPLACE THE "SUBS_AVAIL3" FUNC ABOVE ONCE THE
  FUNCTIONALITY AND UNDERLYING DATA STRUCTURE MODIFICATIONS ARE IMPLEMENTED.
...
===========================================================================*/
boolean cm_ph_cmd_subscription_available4(
...
)
{
  ...
  CM_MSG_HIGH_3( "cm_ph_cmd_subscription_available4, cdma=%d, gwl=%d asubs_id = %d",
                 cdma_status,gwl_status, asubs_id);
...
)
{
  ...
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  CM_MSG_HIGH_4_EXT( "CM_PH_CMD_SUBSCRIPTION_NOT_AVAILABLE, cdma=%d, gwl=%d, asubs_id=%d cause=%d",
                     cdma_status,gwl_status,asubs_id,cause,(asubs_id+1));
...
FUNCTION cm_ph_cmd_subscription_changed4
...
===========================================================================*/
static boolean cm_ph_cmd_subscription_changed4(
...
)
{
  ...
  CM_MSG_HIGH_3( "CM_PH_CMD_SUBSCRIPTION_CHANGED, cdma=%d, gwl=%d asubs_id=%d",
                 cdma_status,gwl_status, asubs_id);
...
)
{
  ...
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if( cdma_status == CM_SUBSCRIPTION_STATUS_CHANGE ||
      gwl_status  == CM_SUBSCRIPTION_STATUS_CHANGE )
  {
    ret_val= cm_ph_cmd_subscription_changed4( cmd_cb_func,
             data_block_ptr,
             client_id,
             SYS_MODEM_AS_ID_1,
             cdma_status,
    ...
    if(ret_val == FALSE)
    {
      CM_MSG_HIGH_0("Unable to queue Subs_chgd cmd for Sub0");
      return ret_val;
    }
  }
  ...
  if( gw2_status == CM_SUBSCRIPTION_STATUS_CHANGE )
  {
    ...
    if(ret_val == FALSE)
    {
      CM_MSG_HIGH_0("Unable to queue Subs_chgd cmd for Sub1");
      return ret_val;
    }
  }
  ...
  if( gw3_status == CM_SUBSCRIPTION_STATUS_CHANGE )
  {
    ...
    if(ret_val == FALSE)
    {
      CM_MSG_HIGH_1("Unable to queue Subs_chgd cmd for Sub%d", as_id);
    }
  }
...
===========================================================================*/
boolean cm_ph_cmd_subscription_changed2(
...
/*lint -esym(715,client_id) */
{
  ...
  return cm_ph_cmd_get_networks_per_subs_bst_extn ( cmd_cb_func,
         data_block_ptr,
         client_id,
         list_type,
         network_type,
...
FUNCTION cm_ph_cmd_get_networks_per_subs_bst_extn_freq
...
SIDE EFFECTS
  ...
  NETWORKS_CONF event.
===========================================================================*/
boolean cm_ph_cmd_get_networks_per_subs_bst_extn_freq (
...
)
{
  ...
  */
  ...
  cmd_info_ptr->cmd_subs          = asubs_id;
  cmd_info_ptr->band_pref_lmt     = cmd_info_ptr->network_band_pref     = band_pref_lmt;
  cmd_info_ptr->lte_band_pref_lmt = cmd_info_ptr->network_lte_band_pref = sys_map_band_mask_to_lte_band_mask(lte_band_pref_lmt);
  cmd_info_ptr->tds_band_pref_lmt = cmd_info_ptr->network_tds_band_pref = tds_band_pref_lmt;
  cmd_info_ptr->ignore_manual_bst = ignore_manual_bst;
  cmd_info_ptr->is_requeue        = is_requeue;
  cmd_info_ptr->incremental_results             = incremental_results;
  cmd_info_ptr->max_search_time                 = max_search_time;
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

  /* print the cmd details
  */
  CM_MSG_HIGH_6("GET_NET: network_type %d, list_type %d, subs_id %d, GW BAND PREF 0x%08x %08x, max_search_time %d",
                network_type, list_type, asubs_id,QWORD_HIGH(band_pref_lmt), QWORD_LOW(band_pref_lmt), max_search_time);

  CM_MSG_HIGH_6("GET_NET: User LTE BAND PREF 0x%08x %08x, User TDS BAND PREF 0x%08x %08x, ignore_mbst %d, is_requeue %d",
                QWORD_HIGH(lte_band_pref_lmt), QWORD_LOW(lte_band_pref_lmt),
                QWORD_HIGH(tds_band_pref_lmt), QWORD_LOW(tds_band_pref_lmt),
                ignore_manual_bst, is_requeue);

  CM_MSG_HIGH_2("incremental results enabled %d periodicity %d",incremental_results.enabled,incremental_results.periodicity);
  
  if(pf_scan_info != NULL)
  {
    // If device prop pointer is NULL, this is the first attempt (start of the periodic search)
    if(ph_ptr->device_prop.pf_scan_info == NULL)
    {
      uint8 i;
      
      CM_MSG_HIGH_4("GET_NET: ARFCN list count WCDMA %d LTE %d, periodicity %d, max time %d", 
                    pf_scan_info->arfcn_list.wcdma_count,
                    pf_scan_info->arfcn_list.lte_count,
                    pf_scan_info->search_periodicity, 
                    max_search_time);

      if((cmd_info_ptr->pf_scan_info = (sys_periodic_frequency_scan_info_s_type *)
                                       cm_mem_malloc(sizeof(sys_periodic_frequency_scan_info_s_type))) == NULL)
      {
        sys_err_fatal_null_ptr_exception();
        return FALSE;
      }

      memscpy(cmd_info_ptr->pf_scan_info, sizeof(*cmd_info_ptr->pf_scan_info),
              pf_scan_info, sizeof(*pf_scan_info));

      cmd_info_ptr->pf_scan_info->asubs_id = asubs_id;
      cmd_info_ptr->pf_scan_info->network_type = (uint8)network_type;
      cmd_info_ptr->pf_scan_info->network_band_pref = (uint64)band_pref_lmt;
      cmd_info_ptr->pf_scan_info->network_lte_band_pref = sys_map_band_mask_to_lte_band_mask(lte_band_pref_lmt);
      cmd_info_ptr->pf_scan_info->network_list_type = (uint8)list_type;
      cmd_info_ptr->pf_scan_info->ignore_manual_bst = ignore_manual_bst;

      /* Set priority to low for the 1st periodic srch attempt.
      ** Set priority to high for one-time srch.
      */
      if (pf_scan_info->search_periodicity)
      {
        cmd_info_ptr->pf_scan_info->priority = SYS_SRCH_PRIORITY_LOW;
      }
      else
      {
        cmd_info_ptr->pf_scan_info->priority = SYS_SRCH_PRIORITY_HIGH;
      }

      for(i = 0; (i < pf_scan_info->arfcn_list.wcdma_count && i < MAX_WCDMA_ARFCN_LIST_COUNT); i++)
      {
        CM_MSG_HIGH_2("GET_NET: WCDMA ARFCN[%d]: %d", i, pf_scan_info->arfcn_list.wcdma_arfcn[i]);
      }
      
      for(i = 0; (i < pf_scan_info->arfcn_list.lte_count && i < MAX_LTE_ARFCN_LIST_COUNT); i++)
      {
        CM_MSG_HIGH_2("GET_NET: LTE ARFCN[%d]: %d", i, pf_scan_info->arfcn_list.lte_arfcn[i]);
      }

      if(pf_scan_info->preferred_plmn_list.count != 0)
      {
        CM_MSG_HIGH_1("GET_NET: Preferred PLMN list count %d", pf_scan_info->preferred_plmn_list.count);

        for(i = 0; (i < pf_scan_info->preferred_plmn_list.count && i < MAX_PREFERRED_PLMN_LIST_SIZE); i++)
        {
          CM_MSG_HIGH_4("GET_NET: PLMN[%d]: 0x%x 0x%x 0x%x", 
                        i, 
                        pf_scan_info->preferred_plmn_list.plmn_list[i].identity[0],
                        pf_scan_info->preferred_plmn_list.plmn_list[i].identity[1],
                        pf_scan_info->preferred_plmn_list.plmn_list[i].identity[2]);
        }
      }
    }

    /* If the periodic scan info is already present in the phone object, the scan
    ** is already in progress and this is another scan attempt within the overall
    ** search. So we will use the same info instead of allocating memory for new
    ** info (in this case, the info should be the same).
    */
    else
    {
      cmd_info_ptr->pf_scan_info = ph_ptr->device_prop.pf_scan_info;
    }
  }
...
FUNCTION cm_ph_cmd_get_networks_per_subs_bst_extn
...
SIDE EFFECTS
  ...
  NETWORKS_CONF event.
===========================================================================*/
boolean cm_ph_cmd_get_networks_per_subs_bst_extn (
...
)
{
  ...
  incremental_results.enabled = FALSE;
  incremental_results.periodicity = 0;
  
  return cm_ph_cmd_get_networks_per_subs_bst_extn_freq(
           cmd_cb_func,
           data_block_ptr,
           client_id,
           list_type,
...
)
{
  ...
  CM_MSG_HIGH_1("cm_ph_cmd_wakeup_from_standby_per_subs as_id %d",
                asubs_id);
...
/*lint -esym(818,mru_table) */
{
  ...
  CM_MSG_HIGH_5( "CM_MRU_UPDATE: update_type %d, client_id %d, mode %d, index %d, sub %d",
                 mru_update_e_type,
                 client_id, mode,
                 index,
                 asubs_id);
...
===========================================================================*/
boolean cm_ph_cmd_change_rtre_config1(
...
/*lint -esym(715, rtre_config) */
{
  ...
  CM_MSG_HIGH_3_ASID("CM_PH_CMD_CHANGE_RTRE_CONFIG client_id %d rtre_config %d",
                     asubs_id, client_id, rtre_config);
...
boolean cm_ph_cmd_dual_standby_pref(
  ...
  */
  ...
  boolean                              data_call_pending
  /* Indicates whether DS will send a data call post DDS switch on new DDS */
)
{
  cm_ph_cmd_s_type        *ph_cmd_ptr;
  cm_ph_cmd_info_s_type   *cmd_info_ptr;
  ...
  */
       ...
       BETWEEN(default_data_subs, SYS_MODEM_AS_ID_NONE,SYS_MODEM_AS_ID_MAX)))
  {
    ...
    CM_MSG_HIGH_6("DS prefs: Reject dual stdby, stdby pref %d active subs %d pri subs %d dflt voice %d dflt data %d dds type %d",
  ...
  */
  ...
  cmd_info_ptr->is_data_call_pending_post_dds = data_call_pending;

  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

  CM_MSG_HIGH_5("Current DS prefs: standby pref %d active subs mask %d priority subs %d dflt voice %d dflt data %d",
                cmph_ptr()->device_prop.msim_prop.user_standby_pref,
                cmph_ptr()->device_prop.msim_prop.active_subs,
                cmph_ptr()->device_prop.msim_prop.priority_subs,
                cmph_ptr()->device_prop.msim_prop.default_voice_subs,
  ...
  CM_MSG_HIGH_1("New DS prefs: data_call_pending %d",data_call_pending);
...
)
{
  ...
  */
  if(!(BETWEEN(standby_pref,      invalid_range_check_cnt,  SYS_MODEM_DS_PREF_MAX) &&
       ...
       BETWEEN(default_data_subs, SYS_MODEM_AS_ID_NONE,SYS_MODEM_AS_ID_MAX)))
  {
    CM_MSG_HIGH_6("DS prefs: Reject dual stdby, stdby pref %d active subs mask %d pri subs %d dflt voice %d dflt data %d dds type %d",
                  standby_pref, active_subs, priority_subs,
                  default_voice_subs, default_data_subs, dds_switch_type);
  ...
  */
  ...
  CM_MSG_HIGH_6("New DS prefs: stdby pref %d active subs mask %d pri subs %d dflt voice %d dflt data %d dds_cause %d",
                standby_pref, active_subs, priority_subs,
                default_voice_subs, default_data_subs,dds_switch_type);
...
)
{
  ...
  */
  ...
  CM_MSG_HIGH_2("Current preferences: active subs %d, New preferences: active subs %d",
                cmph_ptr()->device_prop.msim_prop.active_subs, subs_mask);
...
{
#ifdef CM_GW_SUPPORTED
  if (cmph_is_ssim()) asubs_id = SYS_MODEM_AS_ID_1;
  if(!cmutil_is_as_id_valid(asubs_id))
  {
    return 0;
  }

  return cmwaoc_get_ccm(asubs_id);
#else
  SYS_ARG_NOT_USED(asubs_id);
  return 0;
#endif
...
===========================================================================*/
boolean cm_ph_cmd_update_cdma_capability1(
...
FUNCTION cm_ss_cmd_get_dch_cell_info_per_sub
...
===========================================================================*/
boolean cm_ss_cmd_get_dch_cell_info_per_sub
(
  cm_ss_cmd_cb_f_type     cmd_cb_func,
  /* client callback function */
...
/*lint -esym(715,client_id)*/
{
  return cm_ss_cmd_get_dch_cell_info_per_sub(cmd_cb_func,
                                      data_block_ptr,
                                      client_id,
                                      SYS_MODEM_AS_ID_1); 
...
===========================================================================*/
boolean cm_ss_cmd_get_measurement1(
...
boolean cm_ss_cmd_orig_thr_update_tbl1
(
...
FUNCTION cm_sms_cmd_bc_enable1
...
{
  cmph_sub_info_s_type *ph_sub_ptr = cmph_get_sub_ptr(SYS_MODEM_AS_ID_1);

  if(mode_pref == CM_MODE_PREF_NO_CHANGE && ph_sub_ptr)
  {
    mode_pref = ph_sub_ptr->pref_info.mode_pref;
  }
  if(band_pref == CM_BAND_PREF_NO_CHANGE && ph_sub_ptr)
  {
    band_pref = ph_sub_ptr->pref_info.band_pref;
  }
  return cmutil_is_valid_mode_band_pref( mode_pref, band_pref );
}
...
{
  return (cmutil_is_valid_mode_band_pref( mode_pref, band_pref ) ||
          cmutil_is_valid_lte_mode_band_pref(mode_pref, lte_band_pref ));
}
...
/*~ FUNCTION cm_is_valid_mode_band_pref2 */
{
  ...
  return (cmutil_is_valid_mode_band_pref( mode_pref, band_pref ) ||
          cmutil_is_valid_lte_mode_band_pref(mode_pref, lte_band_pref ) ||
          cmutil_is_valid_tds_mode_band_pref(mode_pref, tds_band_pref ));
...
===========================================================================*/
boolean cm_get_cur_hybr_status1(
...
{
#ifdef FEATURE_HDR_HYBRID
  return cmss_is_stack1_operational(asubs_id);
#else
  return FALSE;
#endif
}
...
)
{
  ...
  sups_cmd_info_ptr->is_all_call_fwd_sups_ext_req = reg_cmd_params->is_all_call_fwd_sups_ext_req;
  ...
  CM_MSG_HIGH_2 ("Register: Forward Type : %d is_all_call_fwd_sups_ext_req %d",
                    reg_cmd_params->fwd_type, reg_cmd_params->is_all_call_fwd_sups_ext_req);
  

  if (reg_cmd_params->fwd_type == CM_FORWARD_TO_URI && reg_cmd_params->fwd_to_uri != NULL){
    ref_cnt_obj_add_ref(reg_cmd_params->fwd_to_uri);
    CM_MSG_HIGH_0 ("Register: Added ref for forward_to_URI");
    CM_MSG_HIGH_2("Forward URI length : %d, Forward URI %x",reg_cmd_params->fwd_to_uri->fwd_to_uri_length,reg_cmd_params->fwd_to_uri);
    sups_cmd_info_ptr->fwd_type = reg_cmd_params->fwd_type;
    sups_cmd_info_ptr->forwarded_to_uri = reg_cmd_params->fwd_to_uri;
...
)
{
  ...
  sups_cmd_info_ptr->cmd_subs_id    = asubs_id;
  sups_cmd_info_ptr->is_all_call_fwd_sups_ext_req = act_cmd_params->is_all_call_fwd_sups_ext_req;

  CM_MSG_HIGH_1( "is_all_call_fwd_sups_ext_req = %d", sups_cmd_info_ptr->is_all_call_fwd_sups_ext_req);
...
)
{
  ...
  */
  ...
  sups_cmd_info_ptr->is_all_call_fwd_sups_ext_req = deact_cmd_params->is_all_call_fwd_sups_ext_req;
...
)
{
  ...
  sups_cmd_info_ptr->is_all_call_fwd_sups_ext_req = erase_cmd_params->is_all_call_fwd_sups_ext_req;
...
@FUNCTION cm_mm_call_cmd_cancel_ims_call_req

@DESCRIPTION
  This API is used to send cancel IMS call request
...
boolean cm_mm_call_cmd_cancel_ims_call_req(
  ...
  cm_ip_call_cancel_ims_call_req_e_type  cancel_type,
    /* Type of cancel request */
...

{
  ...
  */
  ...
  call_cmd_ptr->data_block_ptr  = data_block_ptr;
  call_cmd_ptr->client_id       = client_id;
  call_cmd_ptr->cmd             = CM_CALL_CMD_CANCEL_IMS_CALL_REQ;

  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if (!BETWEEN(cancel_type, CM_CANCEL_CALL_MODIFY_NONE, CM_CANCEL_CALL_MODIFY_MAX))
  {
    CM_ERR_1 ("cancel_type is not valid %d", cancel_type);
  }

/* Only forward the command if its IMS (IP) call */
  if(call_mode == CM_CALL_MODE_INFO_IP)
  {
    CM_MSG_HIGH_3("CM_CALL_CMD_CANCEL_IMS_CALL_REQ: client = %d, cancel_type = %d, call_id = %d",
                      client_id, cancel_type,cmd_info_ptr->call_id);

      ip_cmd_mode_info_ptr = &(call_cmd_ptr->info.ip_call);
      ip_cmd_mode_info_ptr->ipcall_attrib_info.cancel_ims_call_req = cancel_type;
  }
  else
  {
    CM_MSG_HIGH_3("CM_CALL_CMD_CANCEL_IMS_CALL_REQ:call mode is not IP,return err call_id %d, cancel_type %d, call_mode %d",
                cmd_info_ptr->call_id, cancel_type,call_mode);
    cmd_info_ptr->cmd_err = CM_CALL_CMD_ERR_INVALID_MODE_S;
    return FALSE;
  }
  ...
  */
  ...
  #else /* defined(FEATURE_IP_CALL) && defined(FEATURE_CM_LTE) */

  SYS_ARG_NOT_USED(client_id);
  SYS_ARG_NOT_USED(call_id);
  SYS_ARG_NOT_USED(cancel_type);

  if( cmd_cb_func != NULL )
  {
    cmd_cb_func( data_block_ptr, CM_CALL_CMD_CANCEL_IMS_CALL_REQ,
      CM_CALL_CMD_ERR_FUNCTION_UNAVAILABLE_DUE_TO_UNDEFINED_FEATURE);
  }
  return FALSE;
...
===========================================================================*/
boolean cm_bcmcs_cmd_flow_request3(
  ...
  sys_modem_as_id_e_type          asubs_id
  /* Subscription on which this command will be executed. */
)
...
/*lint -esym(715, client_id)*/
{
  ...
  */
  ...
  bcmcs_cmd_ptr = cm_cmd_alloc_bcmcs_init( cmd_cb_func,
                  data_block_ptr,
                  client_id,
                  asubs_id );
...
boolean cm_bcmcs_cmd_registration_request1(
...
boolean cm_bcmcs_cmd_registration_handoff1(
...
{
  return cm_bcmcs_cmd_bom_cache_setup1(cmd_cb_func,data_block_ptr,client_id,
                                       stale_BOM_factor, enable_strict_caching, SYS_MODEM_AS_ID_1);
}
/*===========================================================================
FUNCTION cm_bcmcs_cmd_bom_cache_setup
...
===========================================================================*/
boolean cm_bcmcs_cmd_bom_cache_setup1(
...
===========================================================================*/
boolean cm_mbms_cmd_flow_deact_request1(
...
/*lint -esym(715,plmn_blocking_info)*/
{
  ...
  */
  if(client_id == CM_CLIENT_ID_ANONYMOUS && plmn_blocking_info.cause == SYS_BLOCK_PLMN_CAUSE_NONE)
  {
...
)
{
  ...
  cm_set_drx_req_init(&set_drx_info);
...
FUNCTION cm_set_drx_req_init

DESCRIPTION
  This function initializes payload used to make set drx request.
...
boolean cm_set_drx_req_init(cm_set_drx_info_s_type        *set_drx_info_ptr)
{
  if(set_drx_info_ptr == NULL)
  {
    return FALSE;
  }
  set_drx_info_ptr->asubs_id = SYS_MODEM_AS_ID_1;
  set_drx_info_ptr->drx_coefficient = SYS_DRX_CN_COEFFICIENT_S1_NONE;
  set_drx_info_ptr->sys_mode = SYS_SYS_MODE_MAX;
...
/*lint -esym(715,drx_coefficient)*/
{
  ...
  */
  ...
  cmd_info_ptr->sys_mode               = set_drx_info_ptr->sys_mode;
...
FUNCTION cm_ph_cmd_set_ecall_inactivity_trigger

DESCRIPTION
  Command to stop the ECALL timer upon user intervention
...
===========================================================================*/
extern boolean cm_ph_cmd_set_ecall_inactivity_trigger(
...
)
{
  ...
  */
  ...
  ph_cmd_ptr->data_block_ptr          = data_block_ptr;
  ph_cmd_ptr->client_id               = client_id;
  ph_cmd_ptr->cmd                     = CM_PH_CMD_TRIGGER_ECALL_INACTIVITY;
  ...
  */
  ...
  /* Print debug messages for params.
  */
  CM_MSG_HIGH_1("cm_ph_cmd_set_ecall_inactivity_trigger() recvd on sub_id=%d", asubs_id);
...
FUNCTION cm_ph_cmd_set_ecall_timer_and_residual_value

DESCRIPTION
  Command to inform CM about ECall Timer and its Residual Time that was
  pending before it expired.
...
===========================================================================*/
extern boolean cm_ph_cmd_set_ecall_timer_and_residual_value(
  ...
  cm_ph_ecall_timer_and_residual_time_s_type  ecall_residual_time,
...
)
{
  ...
  */
  ...
  ph_cmd_ptr->data_block_ptr          = data_block_ptr;
  ph_cmd_ptr->client_id               = client_id;
  ph_cmd_ptr->cmd                     = CM_PH_CMD_ECALL_TIMER_AND_RESIDUAL_VALUE;
  ...
  */
  ...
  /* ECall Timer ID and Timer Residual Value*/
  cmd_info_ptr->ecall_timer_residual_value = ecall_residual_time;

  /* Print debug messages for params.
  */
  CM_MSG_HIGH_3 ( "cm_ph_cmd_set_ecall_timer_and_residual_value() recvd on sub_id, timer, value %d, %d, %d",
                  asubs_id,
                  cmd_info_ptr->ecall_timer_residual_value.ecall_timer_id,
                  cmd_info_ptr->ecall_timer_residual_value.ecall_timer_value);
...
)
{
  ...
  */
  ...
  if(call_mode == CM_CALL_MODE_INFO_IP)
  {
    ...
    {
      CM_MSG_HIGH_3("CM_CALL_CMD_MO_MODIFY_REQ client = %d, new_call_type_req = %d RTT_MODE %d",
                    client_id, new_call_type_req, new_call_attrib->rtt_mode);
    ...
    {
      CM_MSG_HIGH_0("CM_CALL_CMD_MO_MODIFY_REQ invalid new call attributes param");
...
*/
{
  ...
  if( is_modem_cc_api == TRUE )
  {
    ...
    */
    ...
    CM_MSG_HIGH_3_EXT("New Modem API: cmd_ptr->is_reused = %d, call_id = %d, cmd_ptr = 0x%x",
                      cmd_ptr->is_reused, call_id, cmd_ptr, 
                      (SUPS_CMD_PTR(cmd_ptr)->cmd_info.cmd_subs_id+1));
...
===========================================================================*/
sys_sys_mode_e_type cm_get_current_acq_sys_mode1(

  sys_modem_as_id_e_type             asubs_id
  /* Subscription on which the command is to be processed. */
)
{
  return sd_misc_get_curr_acq_sys_mode(asubs_id);
}
...
{
  return (int)MM_STACK_ALL*(int)MAX_SIMS;
}
...
DESCRIPTION
  ...
  Index 0 = service on stack 0 - CDMA/GW/GWL
  Index 1 = service on stack 1 - HDR
  Index 2 = service on stack 2 - GWL
...
)
{
  ...
  if(srv_count < cm_get_services_count() || srv_list == NULL )
  {
  ...
  for(i=0; i<MAX_SIMS; i++)
  {
    ...
    {
      mm_id.stk_id = (multimode_stack_e_type)j;
      cmss_get_service_status_per_stack(mm_id, &srv_list[(int)(i*(uint8)MM_STACK_ALL+j)]);
    }
  }
...
)
{
  ...
  else if ( num_ptr->len > ARR_SIZE(num_ptr->buf) )
  {
  ...
  else if ( num_ptr->number_type > (uint8)CM_NUM_TYPE_MAX )
  {
    return_code = CM_NUM_ERR_INVALID_NUM_TYPE;
  }
  else if ( num_ptr->number_plan > (uint8)CM_NUM_PLAN_MAX )
  {
    return_code = CM_NUM_ERR_INVALID_NUM_PLAN;
  }
  else if ( num_ptr->pi > CAI_NOT_AVAIL_PI )
  {
    return_code = CM_NUM_ERR_INVALID_PI;
  }
  else if ( num_ptr->si > CAI_NET_SI )
  {
...
)
{
  ...
  return(sd_misc_is_supp_pref(cm_util_map_sys_sys_mode_to_sd_ss_mode_pref(sys_mode),
                              cmutil_map_cm_band_pref_to_sd_band_pref(band_pref)));
...
)
{
  ...
  */
  if(! ph_ptr->device_prop.is_event_process )
  {
    return;
  }
  ...
  */
  ...
  if(! ph_sub_ptr->is_net_lists_present)
  {
    return;
  }
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

  for (sub = 0; sub < cmpmprx_get_num_of_sims() && sub < MAX_SIMS; sub ++)
  {
    cmph_sub_info_s_type *local_ph_sub_ptr = cmph_get_sub_ptr(sub);
    if ((sys_modem_as_id_e_type)sub == asubs_id && local_ph_sub_ptr != NULL)
    {
      /* Copy detailed information now.
      */
      if(local_ph_sub_ptr->plmn_list.plmn_list.length > 0 )
      {
        info_ptr->gw_available_networks = local_ph_sub_ptr->plmn_list.plmn_list;
      }
      info_ptr->pci_plmn_list = local_ph_sub_ptr->plmn_list.pci_plmn_list;

      if (local_ph_sub_ptr->user_pref_networks.length > 0 )
      {
        info_ptr->gw_user_pref_networks = local_ph_sub_ptr->user_pref_networks;
      }
      break;
    }
  }
...
{
  return cmph_get_gw_subscription_source(SYS_MODEM_AS_ID_1);
}
...
{
  return cmph_get_cdma_subscription_source(SYS_MODEM_AS_ID_1);
}
...
boolean cm_mbms_cmd_generic1 (
  ...
  sys_modem_as_id_e_type              asubs_id
  /* Subscription on which this command is to be processed.*/
)
/*lint -esym(715,cmd_cb_func)*/
/*lint -esym(715,data_block_ptr)*/
/*lint -esym(715,client_id)*/
...
{
  cm_unlock_hdr_resources1(SYS_MODEM_AS_ID_1);
}
...
)
{
  ...
  cm_send_hdr_unlock_rf_cmd(asubs_id);
...
@SIDE EFFECTS
  None
===========================================================================*/
boolean cm_get_cdma_position_info1(
...
)
{
  ...
  CM_MSG_HIGH_1("CM returning CDMA postion information to client, sub%d", asubs_id);
...
)
{
  ...
  {
    return (cmmsc_get_curr_msc_max_cap(asubs_id) == SYS_SYS_MODE_MASK_GSM);
  }
}
...
)
{
  ...
  CM_MSG_HIGH_6("is_ct_sim(): asubs_id %d, 1x_avail %d, 1x_mcc %d, gw_avail %d, gw_mcc %d, gw_mnc %d",
                asubs_id,
                ph_sub_ptr->is_3gpp2_subs_avail,
                cmmmgsdi_get_imsi_mcc_per_sub(asubs_id),
                ph_sub_ptr->is_3gpp_subs_avail,
                ph_sub_ptr->gwl_mcc,
                ph_sub_ptr->gwl_mnc);

  if((ph_sub_ptr->is_3gpp2_subs_avail &&
      cmmmgsdi_get_imsi_mcc_per_sub(asubs_id) == IMSI_MCC_CHINATEL_DEFAULT)
      ||
      (!ph_sub_ptr->is_3gpp2_subs_avail &&
       ph_sub_ptr->is_3gpp_subs_avail &&
       (ph_sub_ptr->gwl_mcc == CM_MCC_CHINA &&
        ph_sub_ptr->gwl_mnc == CM_MNC_CHINATEL_3GPP_DEFAULT)))
  {
    return TRUE;
  }
  return FALSE;
...
@FUNCTION cm_is_dds_sub

@DESCRIPTION
   Checks if asubs_id is default data sub
...
===========================================================================*/
boolean cm_is_dds_sub
(
  sys_modem_as_id_e_type asubs_id
)
{
  cmph_s_type                  *ph_ptr  = cmph_ptr ();
  return (asubs_id == ph_ptr->device_prop.msim_prop.default_data_subs);
}
/*===========================================================================

FUNCTION cm_get_hdr_intl_srv_status1

DESCRIPTION
  Returns the internal servcie status of HDR mode per sub
  Used by DS team
...
===========================================================================*/
sys_srv_status_e_type cm_get_hdr_intl_srv_status1(
  sys_modem_as_id_e_type  asubs_id
)
{
  cmph_s_type           *ph_ptr        = cmph_ptr();
  ...
  cmph_sub_info_s_type *ph_sub_ptr = cmph_get_sub_ptr(asubs_id);
  sd_si_info_s_type *ss_true_stk_ptr = NULL;

  if(!IS_VALID_PH_SUB_PTR(asubs_id) || ph_sub_ptr == NULL)
  {
    return SYS_SRV_STATUS_NO_SRV;
  }
  if ((cmph_map_cm_mode_pref_to_sd_mode_pref(ph_sub_ptr->pref_info.mode_pref) & SD_SS_MODE_PREF_HDR)
      == SD_SS_MODE_PREF_NONE)
  {
    return SYS_SRV_STATUS_NO_SRV;
  }
  ...
  ss_true_stk_ptr = cmss_true_stack_info_ptr(mm_id);
  if(ss_true_stk_ptr == NULL)
  {
    return SYS_SRV_STATUS_NO_SRV;
  }

  if(ss_true_stk_ptr->mode == SYS_SYS_MODE_HDR ||
      ss_true_stk_ptr->mode == SYS_SYS_MODE_NO_SRV)
  {
    hdr_srv_status = ss_true_stk_ptr->srv_status;
  }
...
boolean cm_ac_cmd_PPP_nw_cleanup1(
...
)
{
  ...
  CM_MSG_HIGH_3("ACCMD: PPP nw cleanup, cmd %d reason %d as_id %d",
                ac_cmd_ptr->cmd, cmd_info_ptr->reason, asubs_id);
...
)
{
  ...
  */
  ...
  cmd_info_ptr->is_allow_buffering_cmd = user_pref->is_allow_buffering_cmd;
  ...
  /* Print debug messages for params.
  */
  CM_MSG_HIGH_6_EXT("sys_sel_pref(), mode_pref %d, pref_term %d, acq_order %d, sub %d, roam_pref %d, hybr_pref %d",
                    cmd_info_ptr->mode_pref, cmd_info_ptr->pref_term, cmd_info_ptr->acq_order_pref,
                    cmd_info_ptr->cmd_subs, cmd_info_ptr->roam_pref, cmd_info_ptr->hybr_pref, (cmd_info_ptr->cmd_subs+1));


  CM_MSG_HIGH_6_EXT("sys_sel_pref(), DS SUB %d domain_pref %d net_sel_mode %d prl_pref %d ue_usage %d allow_buffering %d",
                    cmd_info_ptr->cmd_subs + 1, cmd_info_ptr->srv_domain_pref, cmd_info_ptr->network_sel_mode_pref, cmd_info_ptr->prl_pref,
                    cmd_info_ptr->ue_usage_setting, cmd_info_ptr->is_allow_buffering_cmd, (cmd_info_ptr->cmd_subs+1));

  CM_MSG_HIGH_4_EXT("sys_sel_pref(), req_id %u, CSG ID %d, CSG RAT %d ,voice_domain_pref %d",
                    cmd_info_ptr->sys_sel_pref_req_id, cmd_info_ptr->csg_id, cmd_info_ptr->csg_rat, cmd_info_ptr->voice_domain_pref, (cmd_info_ptr->cmd_subs+1));
...
)
{
  ...
  sys_modem_as_id_e_type       other_asubs_id = SYS_MODEM_AS_ID_1;
  ...
  while(call_ptr != NULL)
  {
    if ( (call_ptr->mm_id.asubs_id != asubs_id
         ) &&
  ...
  if(cmsds_is_ext_volte_call_type_active_per_sub(other_asubs_id,call_type_mask))
  {
    return TRUE;
  }
...
)
{
  ...
  CM_MSG_HIGH_3("ACCMD: control_nw_access cmd %d reason %d as_id %d",
...
FUNCTION cm_ph_set_proc_type

DESCRIPTION
  Inform the current procedure type to RRC, so that LTE can set the proper TRM
  priority
...
boolean cm_ph_set_proc_type(
  ...
  /* procedure type used by RRC to set TRM priority */
)
{
  cm_ph_cmd_s_type              *ph_cmd_ptr;
  cm_ph_cmd_info_s_type         *cmd_info_ptr;
  ...
  */
  cmd_info_ptr                         = CMD_INFO_PTR( ph_cmd_ptr );
  cmd_info_ptr->cmd_subs               = asubs_id;
  cmd_info_ptr->proc_type              = proc_type;
  cmd_info_ptr->cmd_err                = CM_PH_CMD_ERR_NOERR;

  CM_MSG_HIGH_3("LTE_PROC_TYPE: SET: proc_type %d, subs %d, client_id %d",
                   proc_type, cmd_info_ptr->cmd_subs, client_id);
...
/*===========================================================================
FUNCTION cm_ph_cmd_ui_ready_status

DESCRIPTION

  Queue CM_PH_CMD_UI_READY_STATUS command to CM.
...
===========================================================================*/
boolean cm_ph_cmd_ui_ready_status(
...
/*lint -esym(715,client_id)*/
{
  ...
  /*
  ** Now that all function parameters are correct,
  ** get command buffer to queue CM_PH_CMD_UI_READY_STATUS
  */
  ph_cmd_ptr = cm_cmd_alloc_ph_init();
  ...
  */
  ...
  ph_cmd_ptr->data_block_ptr          = data_block_ptr;
  ph_cmd_ptr->client_id               = client_id;
  ph_cmd_ptr->cmd                     = CM_PH_CMD_UI_READY_STATUS;
  ...
  */
  cmd_info_ptr                         = CMD_INFO_PTR( ph_cmd_ptr );
  cmd_info_ptr->cmd_err                = CM_PH_CMD_ERR_NOERR;
  cmd_info_ptr->ui_ready_status        = ui_ready_status;

  CM_MSG_HIGH_2("current ui_ready_status = %d, new ui_ready_status = %d",
                cmph_ptr()->device_prop.ui_ready_status, ui_ready_status);
...
@FUNCTION cm_ph_cmd_close_lb_ready_per_subs

@DESCRIPTION
  Command CM to send CLOSE LOOPBACK READY IND to NAS.
...
@x
===========================================================================*/
boolean cm_ph_cmd_close_lb_ready_per_subs(
...
)
{
  ...
  */
  ...
  ph_cmd_ptr->data_block_ptr          = data_block_ptr;
  ph_cmd_ptr->client_id               = client_id;
  ph_cmd_ptr->cmd                     = CM_PH_CMD_CLOSE_LB_READY;
  ...
  */
  ...
  cmd_info_ptr->cmd_subs             = asubs_id;
  cmd_info_ptr->close_loopback       = close_loopback;

  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

  /* Print debug messages for params.
  */
  CM_MSG_HIGH_2 ("enqueuing CLOSE_LB_READY - ready_ind=%d,asubs_id=%d",
                 close_loopback,asubs_id);
...
)
{
  ...
  */
  ...
  /* Print debug messages for params.
  */
  CM_MSG_HIGH_4 ("PH_CMD_SET_ROAMING_PREF - roaming_type: %d,asubs_id: %d,int roam %d, dom roam %d",
    roaming_pref.cmd_type, asubs_id, roaming_pref.international_voice_roaming_allowed,
    roaming_pref.domestic_voice_roaming_allowed);
...
@DESCRIPTION
  DEPRECATED, command CM to change the data priority of sub
...
{
  CM_MSG_HIGH_0("cm_msim_set_data_priority() is deprecated, use cm_set_data_proc_type()");
...
@FUNCTION cm_set_data_proc_type

@DESCRIPTION
  Command CM to change the data priority of sub using proc_type
...
@x
===========================================================================*/
boolean cm_set_data_proc_type(
  sys_modem_as_id_e_type asubs_id,
  /* SUB id for which priority has to be updated */

  sys_proc_type_e_type proc_type
  /* specifies the proc_type to be set for corresponding asubs_id*/
)
{
  cmph_s_type            *ph_ptr = cmph_ptr();
  cmph_sub_info_s_type   *ph_sub_ptr = cmph_get_sub_ptr(asubs_id);
  ...
  if(cmpmprx_get_num_of_active_data() != 2 || ph_sub_ptr == NULL)
  {
    CM_MSG_HIGH_0("set_data_proc_type: CM_PS_PS dual data feature not supported");
    return FALSE;
  }
  if(!cmph_is_msim())
  {
    CM_MSG_HIGH_0("set_data_proc_type: msim not supported");
    return FALSE;
  }

  if(ph_sub_ptr->is_data_activity_ongoing == TRUE
    && proc_type == SYS_PROC_TYPE_MMS)
  {
    cmph_update_sp_special_priority_mode(TRUE, TRUE,
...
boolean cm_ph_cmd_reset_clnup_st_mc( )
{
  ...
  CM_MSG_HIGH_0("CLNUP: Done.  cm_ph_cmd_reset_clnup_st_mc is called");
...
FUNCTION cm_mm_call_cmd_secure_call

DESCRIPTION
  Command CM to setup secure call flag
...
RETURN VALUE
  TRUE - request sent, FALSE - No buffer to send request
===========================================================================*/
boolean cm_mm_call_cmd_secure_call(
  ...
  uint8                  is_secure_call,
  /* indicates secure call enabled or disabled */

  uint8                  is_MO,
  /* indicates MO/MT secure call */
...
)
{
  ...
  CM_MSG_HIGH_3("CM_CALL_CMD_SECURE_CALL on subs %d: is_secure %d, is_MO %d",
                asubs_id, is_secure_call, is_MO);
  ...
  */
  ...
  call_cmd_ptr->data_block_ptr   = data_block_ptr;
  call_cmd_ptr->client_id        = client_id;
  call_cmd_ptr->cmd              = CM_CALL_CMD_SECURE_CALL;
  call_cmd_ptr->cmd_subs         = asubs_id;
  if ( is_MO )
  {
    call_cmd_ptr->direction = CM_CALL_DIRECTION_MO;
  ...
  {
    call_cmd_ptr->direction = CM_CALL_DIRECTION_MT;
  }
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

  /* Copy command information fields.
  */
  cmd_info_ptr->is_secure_call = is_secure_call;
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  return TRUE;
#else
  CM_MSG_HIGH_0("FEATURE_CMCC_SECURE_CALL is undefined");

  if( cmd_cb_func != NULL )
  {
    cmd_cb_func( data_block_ptr, CM_CALL_CMD_SECURE_CALL,
                 CM_CALL_CMD_ERR_FUNCTION_UNAVAILABLE_DUE_TO_UNDEFINED_FEATURE);
  }
  return FALSE;
#endif//FEATURE_CMCC_SECURE_CALL
} /* cm_mm_call_cmd_secure_call */

/*===========================================================================

FUNCTION cm_mm_call_cmd_data_call_activity

DESCRIPTION
  Notify CM of DDS switch start/end or MMS start/end, called by DS
...
RETURN VALUE
  TRUE: cmd is queued
  FALSE: cmd is not queued

===========================================================================*/
boolean cm_mm_call_cmd_data_call_activity(
  cm_call_cmd_cb_f_type              cmd_cb_func,
  /* client callback function */
  ...
  cm_data_activity_cmd_info_s_type   data_activity_info
  /* info for data activity */
)
{
  cm_call_cmd_s_type          *call_cmd_ptr;
  cm_call_cmd_info_s_type     *cmd_info_ptr;
  ...
  CM_MSG_HIGH_3("DUAL_LTE: CM_CALL_CMD_DATA_ACTIVITY on subs %d: data_activity %d, client_id %d",
                data_activity_info.asubs_id, data_activity_info.data_activity,
                client_id);
  ...
  */
  ...
  call_cmd_ptr->cmd_subs             = data_activity_info.asubs_id;

  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

  /* Copy command information fields.
  */
  cmd_info_ptr->data_activity_info   = data_activity_info;
...
FUNCTION cm_ph_cmd_dds_switch_allowed

DESCRIPTION
  Check if DDS switch is allowed, called by DS
...
===========================================================================*/
boolean cm_ph_cmd_dds_switch_allowed(
  cm_ph_cmd_cb_f_type             cmd_cb_func,
  /* client callback function */
  ...
  cm_dds_switch_allowed_s_type    dds_switch_allowed_info
  /* info for dds switch allowed */
)
{
  cm_ph_cmd_s_type        *ph_cmd_ptr;
  cm_ph_cmd_info_s_type   *cmd_info_ptr;
  ...
  CM_MSG_HIGH_1("CM_PH_CMD_DDS_SWITCH_ALLOWED on subs %d",
                dds_switch_allowed_info.asubs_id);
  ...
  */
  ...
  ph_cmd_ptr->data_block_ptr          = data_block_ptr;
  ph_cmd_ptr->client_id               = client_id;
  ph_cmd_ptr->cmd                     = CM_PH_CMD_DDS_SWITCH_ALLOWED;
  ...
  */
  //cmd_info_ptr->cmd_subs                  = asubs_id;
  cmd_info_ptr->dds_switch_allowed_info   = dds_switch_allowed_info;
...
===========================================================================*/
sys_vsid_type cm_get_default_vsid( void )
{

  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

  sys_vsid_type default_vsid = 0;

  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

  default_vsid = cm_ph_get_default_vsid();

  CM_MSG_HIGH_1("cm_get_default_vsid returned %d",default_vsid);
...
FUNCTION: CM_GET_UI_READY_STATUS

   Gets the current UI ready status

@return
  TRUE -- UI is ready. \n
  FALSE -- UI is not ready.

@dependencies
  The phone object must have already been initialized with cmph_init().

===========================================================================*/
boolean cm_get_ui_ready_status(void)
{
  return cmph_ptr()->device_prop.ui_ready_status;
}
...
)
{
  ...
  cm_mm_ss_info_3gpp2_stack_info_s_type *ss_stk0_3gpp2_ptr;

  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

  CM_MSG_HIGH_4("DRVCC: setup command, drvcc_type %d, as_id %d, num_of_calls %d, roaming_allowed %d",
                drvcc_call_info->drvcc_type, drvcc_call_info->as_id, drvcc_call_info->num_of_calls,
                drvcc_call_info->drvcc_allowed_in_roaming);
  ...
  ss_sub_ptr = cmss_get_info_sub_prop(drvcc_call_info->as_id);
  if(ss_sub_ptr == NULL)
  {
    return FALSE;
  }
  ss_stk0_3gpp2_ptr = ss_sub_ptr->stack_info[MM_STACK_0].gpp2_info;

  if (ss_sub_ptr->stack_info[MM_STACK_0].gen_info)
  {
    srv_status = ss_sub_ptr->stack_info[MM_STACK_0].gen_info->srv_status;
    roam_status = ss_sub_ptr->stack_info[MM_STACK_0].gen_info->roam_status;
    if(ss_sub_ptr->stack_info[MM_STACK_0].gen_info->sys_mode == SYS_SYS_MODE_CDMA &&
        ss_stk0_3gpp2_ptr)
    {
      is_cdma_custom_home = ss_stk0_3gpp2_ptr->is_3gpp2_custom_home;
    }
  }
  else
  {
    CM_MSG_HIGH_1("DRVCC: Invalid stack returned for DRVCC setup sub %d",drvcc_call_info->as_id);
    return FALSE;
  }
  ...
  */
  ...
  cmd_info_ptr->call_id                   = cm_call_id_allocate_new(FALSE);
...
FUNCTION: cm_effective_policy_changed

DESCRIPTION
   This CM API will return if there is any effective change in policy which may result in service request from CM.
...
RETURN VALUE
    ...
    specified subscription has camped on the specified PLMN, and returns enum is there is an effective change in policy.

  @param[in]  subsId    Subscription Id for which MCC is required
  @param[in]  pPlmn     Pointer to the PLMN in question.
  @return
  CM_EFFECTIVE_POLICY_NO_CHANGE  if no effective change occurred or else CM_EFFECTIVE_POLICY_CHANGE.
...
cm_effective_policy_change_reason_e_type  cm_effective_policy_changed(  sys_modem_as_id_e_type  subsId,
                                                                               sys_radio_access_tech_e_type active_rat, 
                                                                                    sys_plmn_id_s_type     *pPlmn,
                                                                                    sys_non_acq_param_s_type     *non_acq_param
                                                                                 )
{
  cm_effective_policy_change_reason_e_type result=CM_EFFECTIVE_POLICY_NO_CHANGE;
  ...
  pCmServingInfo.asubs_id = subsId;
  pCmServingInfo.id_type = SYS_SYS_ID_TYPE_UMTS;
  pCmServingInfo.plmn = *pPlmn;
  pCmServingInfo.srv_status = SYS_SRV_STATUS_LIMITED;
  pCmServingInfo.sys_mode = cmregprx_map_sys_rat_to_sd_mode(active_rat);
  pCmServingInfo.srv_domain = SYS_SRV_DOMAIN_CAMPED;
  pCmServingInfo.stack = cmregprx_get_active_stack_on_sub(subsId);
  if( cmpmprx_is_policy_manager_suspended() == TRUE )
  {
    result = CM_EFFECTIVE_POLICY_CHANGE_SUSPENDED;
  }
  else
  {
    pSandbox = policyman_create_sandbox();
    
    pm_cfg_ptr = policyman_report_camping_candidate(&pCmServingInfo, pSandbox);

  /* PM will return NULL if no change in policy will occur*/
    if(pm_cfg_ptr != NULL)
    {
      result=cmpmprx_policy_chg_proc_no_enforce(subsId,(cm_policyman_cfg_s *)pm_cfg_ptr, non_acq_param);
...
FUNCTION: cm_get_mmode_state

DESCRIPTION
   This API returns the latest mmode state info to the caller
...
RETURN VALUE
  @return
  cm_mmode_state_s_type structure containing various mmode state info

SIDE EFFECTS
  None

========================================================================== */
cm_mmode_state_s_type  cm_get_mmode_state(void)
{
  cmph_s_type            *ph_ptr = cmph_ptr();
  cm_mmode_state_s_type   state_info;

  state_info.standby_pref = ph_ptr->device_prop.msim_prop.new_standby_pref;
...
FUNCTION cm_get_vfr_map

DESCRIPTION
  Get the ASID to VFR mapping
...
===========================================================================*/
void cm_get_vfr_map( sys_vfr_e_type *map )
{
  memscpy(map, (sizeof(sys_vfr_e_type)*MAX_AS_IDS),
          &cm_vfr_map[0], (sizeof(sys_vfr_e_type)*MAX_AS_IDS));
}

/*===========================================================================

FUNCTION: cm_get_is_ready_to_scan_more_pref_data

DESCRIPTION
   Informs if Acquisition is triggered on the given RAT.
   RATs spported: HDR and LTE
...
RETURN VALUE
   TRUE for LTE
   if SUb is in Srlte sub mode of operation and LTE acquisition is given on STACK_2

   TRUE for HDR
   if HDR stack is operational and HDR acq is triggered
...
========================================================================== */
extern boolean  cm_get_is_ready_to_scan_more_pref_data(
...
)
{
  ...
  switch(sys_mode)
  {
    case SYS_SYS_MODE_LTE:
      ...
      srv_status     = cmss_get_info_srv_status(asubs_id, mm_id.stk_id);

      if( cmph_is_oprting_in_csfb_pref_srlte(asubs_id) &&
          sys_srv_status_is_srv(srv_status) == FALSE &&
          sd_ss_act_get_per_mm_id(NULL, mm_id) == SD_SS_ACT_ACQ_GWL )
      {
        return TRUE;
      }
      break;
    ...
    case SYS_SYS_MODE_HDR:
      ...
      if( cmss_is_stack1_operational(asubs_id) &&
          sys_srv_status_is_srv(srv_status) == FALSE &&
          sd_ss_act_get_per_mm_id(NULL, mm_id) == SD_SS_ACT_ACQ_HDR )
      {
        return TRUE;
      }
      break;
...
@FUNCTION cm_is_task_stop_sig_rcvd

@DESCRIPTION
   Returns if the task stop signal received by CM task.
...
@RETURN VALUE
   TRUE for LTE
   TRUE if TASK_STOP_SIG has been received by cm task.

@SIDE EFFECTS
  None

===========================================================================*/
boolean  cm_is_task_stop_sig_rcvd (void)
{
  return cmtask_is_stop_sig_rcvd();
}

/*===========================================================================

@FUNCTION cm_get_ims_voice_reg_status
...
===========================================================================*/
sys_ims_reg_state_e_type cm_get_ims_voice_reg_status
(
  sys_modem_as_id_e_type    as_id,
  sys_sys_mode_e_type       sys_mode
)
{
  return cmsds_get_ims_voice_reg_status(as_id, sys_mode);
}
/*===========================================================================

@FUNCTION cm_get_ims_sms_reg_status
...
===========================================================================*/
sys_ims_reg_state_e_type cm_get_ims_sms_reg_status
(
  sys_modem_as_id_e_type    as_id,
  sys_sys_mode_e_type       sys_mode
)
{
  return cmsds_get_ims_sms_reg_status(as_id, sys_mode);
}
/*===========================================================================

FUNCTION: cm_per_stacks_is_mt_page_allowed

DESCRIPTION
   Informs if MT CS Page should be allowed
...
cm_mt_cs_page_status_e_type cm_per_stack_is_mt_cs_page_allowed( sys_modem_as_id_e_type as_id,
    sys_modem_stack_id_e_type stack_id)
{
  cm_mt_cs_page_status_e_type retval = CM_MT_CS_PAGE_STATUS_ALLOWED;
  ...
  /*Invalid SUB ID  (or)
    MT CS Page reject timer in progress - REJECT MT CS PAGE*/

  if(ph_sub_config_ptr == NULL)
  {
    retval = CM_MT_CS_PAGE_STATUS_REJECTED;
  }

  else if (!ph_sub_config_ptr->mt_cs_page_reject_param.allow_cs_page_during_mt_volte_call)
  {
    /*Feature is not enabled. Follow legacy behavior.*/
    retval = CM_MT_CS_PAGE_STATUS_ALLOWED;
    CM_MSG_HIGH_2("disabled : mt_cs_page_allowed[%d] is %d",as_id,retval);
  }

  else if (ph_sub_config_ptr->mt_cs_page_reject_uptime ==0)
  {
    /*Idle mode. ACCEPT MT CS PAGE*/
    //if(cmcall_is_present_ip_call()== CM_CALL_ID_INVALID)
    if(cmcall_is_ip_call_originated () == CM_CALL_ID_INVALID)
    {
      retval = CM_MT_CS_PAGE_STATUS_ALLOWED;
      CM_MSG_HIGH_2("no ip call : mt_cs_page_allowed[%d] is %d",as_id,retval);
    }
    /*Ongoing call. REJECT MT CS PAGE*/
    else
    {
      retval = CM_MT_CS_PAGE_STATUS_REJECTED;
      CM_MSG_HIGH_2("ip call : mt_cs_page_allowed[%d] is %d",as_id,retval);
    }
  }

  /*MT CS Page Reject timer still running. REJECT MT CS Page*/
  else if(ph_sub_config_ptr->mt_cs_page_reject_uptime > time_get_uptime_secs())
  {
    CM_MSG_HIGH_1("MT CS Page Reject timer[%d] still running. REJECT MT CS Page",as_id);
    retval = CM_MT_CS_PAGE_STATUS_REJECTED;
  }

  /*MT CS Page Reject timer expired. ACCEPT MT CS Page*/
  else if(ph_sub_config_ptr->mt_cs_page_reject_uptime <= time_get_uptime_secs())
  {
    retval = CM_MT_CS_PAGE_STATUS_WAIT_AND_PROCEED;
    CM_MSG_HIGH_2("Timer expired : mt_cs_page_allowed[%d] is %d",as_id,retval);
    cm_end_pre_alert_mt_volte_call(as_id);
  }
  rex_leave_crit_sect(cmtask_crit_sect_ptr());
#endif

  CM_MSG_HIGH_2("cm_per_stack_is_mt_cs_page_allowed[%d] is %d",as_id,retval);
...
@FUNCTION cm_is_cs_srv_available

@DESCRIPTION
get if CS service is currenly available
...
===========================================================================*/
boolean cm_is_cs_srv_available
...
{
  return cmss_is_cs_srv_available(as_id, srv_info);
}

/*===========================================================================

FUNCTION: cm_get_tau_cap_update_enabled

DESCRIPTION
  This CM API will return the value of TAU_CAP_UPDATE_ENABLED NV
...
@param[in]  asubs_id      Sub ID on which TAU_CAP_UPDATE_ENABLED to be returned
...
===========================================================================*/
boolean cm_get_tau_cap_update_enabled(

  sys_modem_as_id_e_type asubs_id
  /* Sub ID on which tau_cap_update_enabled to be returned */
)
{
  boolean ret_val = FALSE;
  cmph_sub_info_s_type          *ph_sub_ptr = cmph_get_sub_ptr(asubs_id);

  if(ph_sub_ptr != NULL && ph_sub_ptr->ps_pref.is_tau_cap_update_enabled)
  {
    ret_val = TRUE;
  }
...
FUNCTION cm_ph_cmd_oprt_mode3

DESCRIPTION
  Command CM to put phone in a specified operating mode and 
  also specify the pending emergency state while moving 
  to ONLINE mode.
...
boolean cm_ph_cmd_oprt_mode3(
  ...
  const cm_oprt_mode_params_s_type *oprt_mode_params_ptr
...
)
{
  ...
  if (oprt_mode_params_ptr == NULL)
  {

    return FALSE;

  }
    CM_MSG_HIGH_5_EXT("OPRT_MODE: mode %d, uptime %d, client_id %d, e911_pending info %d %d",
                           oprt_mode_params_ptr->oprt_mode, 
                           time_get_uptime_secs(), 
                           client_id,
                           oprt_mode_params_ptr->e911_pending_state, 
                           oprt_mode_params_ptr->e911_pending_sub, 
                           SYS_MODEM_AS_ID_NONE);
  ...
  */
  ...
  /* Copy command information fields.
  */
  cmd_info_ptr->oprt_mode = oprt_mode_params_ptr->oprt_mode;
...
FUNCTION cm_ph_cmd_get_networks_per_subs_lte_band_ext
...
SIDE EFFECTS
  ...
  NETWORKS_CONF event.
===========================================================================*/
boolean cm_ph_cmd_get_networks_per_subs_lte_band_ext (
...
)
{
  ...
  */
  ...
  cmd_info_ptr->cmd_subs          = asubs_id;
  cmd_info_ptr->band_pref_lmt     = cmd_info_ptr->network_band_pref     = band_pref_lmt;
  cmd_info_ptr->lte_band_pref_lmt = cmd_info_ptr->network_lte_band_pref = lte_band_pref_lmt;
  cmd_info_ptr->tds_band_pref_lmt = cmd_info_ptr->network_tds_band_pref = tds_band_pref_lmt;
  cmd_info_ptr->ignore_manual_bst = ignore_manual_bst;
  cmd_info_ptr->is_requeue        = is_requeue;
  cmd_info_ptr->incremental_results             = incremental_results;
  ...
  */
  ...
  CM_MSG_HIGH_6("GET_NET: User LTE BAND PREF 0x%08x %08x, User TDS BAND PREF 0x%08x %08x, ignore_mbst %d, is_requeue %d",
                   QWORD_HIGH(lte_band_pref_lmt.bits_1_64), 
                   QWORD_LOW(lte_band_pref_lmt.bits_1_64),
                   QWORD_HIGH(tds_band_pref_lmt), 
                   QWORD_LOW(tds_band_pref_lmt),
                   ignore_manual_bst, 
                   is_requeue);
  ...
  if(pf_scan_info != NULL)
  {
    ...
    if(ph_ptr->device_prop.pf_scan_info == NULL)
    {
      ...
      cmd_info_ptr->pf_scan_info->network_type = (uint8)network_type;
      cmd_info_ptr->pf_scan_info->network_band_pref = (uint64)band_pref_lmt;
      cmd_info_ptr->pf_scan_info->network_lte_band_pref = lte_band_pref_lmt;
      cmd_info_ptr->pf_scan_info->network_list_type = (uint8)list_type;
      cmd_info_ptr->pf_scan_info->ignore_manual_bst = ignore_manual_bst;
...
FUNCTION cm_msim_get_mt_page_concurrency_config

DESCRIPTION
  Provides MT page concurrency configuration info

DEPENDENCIES
  Clients should call this API after CM task init & NV reading is done

RETURN VALUE
  cm_mt_page_concurrency_config_s_type structure containing the requested info

SIDE EFFECTS
  none

===========================================================================*/
cm_mt_page_concurrency_config_s_type cm_msim_get_mt_page_concurrency_config(void)
{
  cmph_s_type                           *ph_ptr = cmph_ptr();
  cm_mt_page_concurrency_config_s_type   mt_page_concurrency_config;

  memset(&mt_page_concurrency_config, 0, sizeof(cm_mt_page_concurrency_config_s_type));

  mt_page_concurrency_config.is_cs_mt_page_concurrency_allowed =
    ph_ptr->device_prop.msim_prop.mt_page_concurrency_config.is_cs_mt_page_concurrency_allowed;

  mt_page_concurrency_config.is_ims_mt_page_concurrency_allowed =
    ph_ptr->device_prop.msim_prop.mt_page_concurrency_config.is_ims_mt_page_concurrency_allowed;

  return mt_page_concurrency_config;
}
#endif
/*===========================================================================

  FUNCTION cm_mm_inband_cmd_sip_dtmf()

  DESCRIPTION
    Command to send SIP DTMF string from user

  RETURN VALUE
    boolean

===========================================================================*/
boolean cm_mm_inband_cmd_sip_dtmf(
  cm_inband_cmd_cb_f_type     cmd_cb_func,
  void                        *data_block_ptr,
  cm_client_id_type           client_id,
  cm_call_id_type             call_id,
...
)
{
  ...
  if (sip_dtmf_info == NULL ||
      sip_dtmf_info->dtmf_string == NULL)
  {
    if (cmd_cb_func != NULL )
    {
      CM_ERR_0("ERROR: NULL sip_dtmf_info ptr");
      cmd_cb_func(data_block_ptr, CM_CMD_SIP_DTMF, CM_INBAND_CMD_ERR_NULL_BUF_P);
    }
    return FALSE;
  }
  ...
  */
  ...
  inband_cmd_ptr->data_block_ptr  = data_block_ptr;
  inband_cmd_ptr->client_id       = client_id;
  inband_cmd_ptr->cmd             = CM_CMD_SIP_DTMF;
  ...
  */
  cmd_info_ptr->call_id           = call_id;
  cmd_info_ptr->sip_dtmf_info     = sip_dtmf_info;
  if (cmd_info_ptr->sip_dtmf_info != NULL)
  {
    ref_cnt_obj_add_ref(cmd_info_ptr->sip_dtmf_info);
  }

  CM_MSG_HIGH_4("SIP_DTMF: cm_mm_inband_cmd_sip_dtmf call_id %d, dtmf_str %d %d, dtmf str len %d",
                call_id,
                sip_dtmf_info,
                sip_dtmf_info->dtmf_string,
                strlen(sip_dtmf_info->dtmf_string));
...
FUNCTION cm_is_there_call_type_per_sub

DESCRIPTION
  Checks if there is a requested call type on the specified sub
  Bitmasks for call types:
    Voice:     CM_CALL_TYPE_VOICE_MASK (0x0001)
    Emergency: CM_CALL_TYPE_EMERG_MASK (0x0200)
    VT:        CM_CALL_TYPE_VT_MASK    (0x0800)
    SMS:       CM_CALL_TYPE_SMS_MASK   (0x0008)
...
===========================================================================*/
uint64 cm_is_there_call_type_per_sub(
  sys_modem_as_id_e_type       asubs_id,
...
)
{
  ...
  if (cmsds_is_ext_voice(asubs_id) &&
     (call_type_mask & CM_CALL_TYPE_VOICE_MASK)
  )
  {
    active_call_type_mask |= SYS_BM_64BIT(CM_CALL_TYPE_VOICE);
  }

  if (cmsds_is_ext_vt(asubs_id) &&
     (call_type_mask & CM_CALL_TYPE_VT_MASK)
  )
  {
    active_call_type_mask |= SYS_BM_64BIT(CM_CALL_TYPE_VT);
  }

  if (sds_sub_prop_ptr != NULL &&
     (act_id_ptr = cmtask_orig_para_search_act_id(mm_id,
                      (cm_act_id_type)sds_sub_prop_ptr)) != NULL
  )
  {
    if (act_id_ptr->act_type == CM_ACT_TYPE_VOICE_CALL)
    {
      active_call_type_mask |= SYS_BM_64BIT(CM_CALL_TYPE_VOICE);
    }
    else if (act_id_ptr->act_type == CM_ACT_TYPE_VT_CALL)
    {
      active_call_type_mask |= SYS_BM_64BIT(CM_CALL_TYPE_VT);
    }
  }

  if (cmemg_is_sub_ext_e911_mode(asubs_id) &&
     (call_type_mask & CM_CALL_TYPE_EMERG_MASK)
  )
  {
    active_call_type_mask |= CM_CALL_TYPE_EMERG_MASK;
  }
