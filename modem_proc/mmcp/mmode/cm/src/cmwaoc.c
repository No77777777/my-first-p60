...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/mmode/cm/src/cmwaoc.c#1 $
...
/* Pointer to head of the call list. One for each sub */
static cmcall_s_type *call_list_head[MAX_SIMS];
...
)
{
  ...
  return call_list_head[asubs_id];
}
...
static void cmwaoc_set_list_head(
  ...
  /* call used to set list_head for this sub */
)
{
  CM_ASSERT_ENUM_IS_INRANGE(asubs_id, SYS_MODEM_AS_ID_MAX);

  call_list_head[asubs_id] = list_head;
}
...
)
{
  ...
  if(cnf_ptr->write_cnf.response_header.mmgsdi_status != MMGSDI_SUCCESS)
  {
    ...
    gen_rpt_ptr->hdr.asubs_id = asubs_id;
...
)
{
  ...
  cmmmgsdi_refresh_files_read_proc(asubs_id, &file_info, CM_CLIENT_ID_ANONYMOUS);
}
...
)
{
  ...
  if(CMPH_SS_PHONE_CHARGE(asubs_id, ph_ptr).aoc_ready)
  {
                        ...
                        ) != TRUE)
    {
      ...
      {
        ph_ptr->sub_info[asubs_id]->phone_charge.aoc_ready = FALSE;
      }
    }
    else
    {
...
)
{
  ...
  CM_MSG_HIGH_6("AOC Charges e1-e6 %d %d %d %d %d %d",
                charge_info->e1, charge_info->e2, charge_info->e3,
                charge_info->e4, charge_info->e5, charge_info->e6);
  ...
  if(!call_charge_ptr->aoc_initialized || !call_charge_ptr->use_ccm_timeout)
  {
    ...
    {
      CM_MSG_HIGH_2("AOC: ropping call due to max charge being reached. ACM %d ACMMAX %d",
                    cmwaoc_get_acm(asubs_id), cmwaoc_get_acmmax(asubs_id));
    ...
    call_charge_ptr->carry_forward_CCM = 0;
    {
      qword time_now = { 0 };
      uint64 curr_uptime_ms;
      time_get_uptime_ms( time_now );
      curr_uptime_ms                    = QW_CVT_Q2N(time_now);
      call_charge_ptr->last_CCM_started = curr_uptime_ms;
    }
    ...
    /* Set ccm_timeout with e7 (use_e7 = TRUE). If e7 is 0 use e2 to set the timer */
...
)
{
  ...
  if(!cmutil_is_as_id_valid(asubs_id) || ph_ptr->sub_info[asubs_id] == NULL ||
  ...
  CM_MSG_HIGH_6("AOC %d, last_AOC %d, CCM %d, ACM %d, last_CCM_started %d, carry_fwd %d",\
                call_charge_ptr->AOC, call_charge_ptr->last_AOC_to_update_CCM, ph_charge_ptr->CCM, ph_charge_ptr->ACM,\
                call_charge_ptr->last_CCM_started,call_charge_ptr->carry_forward_CCM);
...
)
{
  ...
  else
  {
    ...
    if(call_charge_ptr->e.e5 && (additional_segments >= call_charge_ptr->e.e5))
    {
      ...
      call_charge_ptr->segment_count_in_AOC += call_charge_ptr->e.e5 * additional_e5s;
    }
    ...
    if( call_charge_ptr->use_ccm_timeout && call_charge_ptr->ccm_timeout == 0 )
    {
      ...
      /* Set ccm_timeout with e2. (use_e7 = FALSE). if e2 is 0 dont start the timer */
...
)
{
  ...
  uint64 curr_uptime_ms = 0;

  rex_timer_cnt_type elapsed_cnt;     /* value to set the timer   */
  rex_timer_type *ccm_timer_ptr;   /* pointer to timer to set */
  ...
  if(call_ptr == NULL)
  {
    CM_MSG_HIGH_0("cmwaoc_end_call(), call_ptr is NULL");
    return;
  }
  ...
  {
    CM_MSG_HIGH_1("cmwaoc_end_call(), asubs_id %d is invalid", asubs_id);
    return;
  }
  ...
  if(ph_sub_ptr == NULL)
  {
    CM_MSG_HIGH_1("cmwaoc_end_call(), ph_sub_ptr is NULL for asubs_id %d", asubs_id);
    return;
  }

  phone_charge = &ph_sub_ptr->phone_charge;

  if(call_list_head[asubs_id] != NULL)
  {
  ...
  ccm_timer_ptr = &ph_sub_ptr->phone_charge.ccm_timer;

  elapsed_cnt =  call_charge_ptr->ccm_timeout - rex_get_timer(ccm_timer_ptr);

  {
    qword time_now = { 0 };
    uint64 curr_ccm_period_ticks      = 0;

    time_get_uptime_ms( time_now );
    curr_uptime_ms                    = QW_CVT_Q2N(time_now);

    curr_ccm_period_ticks = (curr_uptime_ms - call_charge_ptr->last_CCM_started);

    if (phone_charge != NULL)
    {
      CM_MSG_HIGH_6("curr_ccm_tick %d, last_AOC %d, CCM %d, ACM %d, last_CCM_started %d, carry_fwd %d",\
                    curr_ccm_period_ticks, call_charge_ptr->last_AOC_to_update_CCM, phone_charge->CCM, phone_charge->ACM,\
                    call_charge_ptr->last_CCM_started,call_charge_ptr->carry_forward_CCM);
    }


    if ((call_charge_ptr->carry_forward_CCM + curr_ccm_period_ticks) >= call_charge_ptr->ccm_timeout &&
        ((phone_charge != NULL) && phone_charge->ccm_timer_running))
    {
      CM_MSG_HIGH_3("call_end current AOC %d, increasing AOC by %d, rex_timer %d ",call_charge_ptr->AOC, \
                    call_charge_ptr->e.e1 * call_charge_ptr->e.e3, rex_get_timer(ccm_timer_ptr));
...
)
{
  ...
  while(call != NULL)
  {
    ...
    call_charge_ptr = &call->cmcall_mode_info.info.gsm_wcdma_info.wcall_charge_data;
    ...
    if(call_charge_ptr->e.e1 > 0 && call_charge_ptr->e.e2 > 0)
    {
      ...
      if(time_left > 0)
      {
        ...
        additional_aoc += increments * call_charge_ptr->e.e1 * call_charge_ptr->e.e3;
      }
    }

    /* If there are data charges associated with the call, account for those now */
...
)
{
  ...
  /* ccm_timer in phone is initialized with ccm_timeout of first call in call_list_head.
  ** Phone's ccm_timer counts down and expires when it reaches 0.
  */
  timer_to_set_ptr = &ph_ptr->sub_info[asubs_id]->phone_charge.ccm_timer;
  ...
  while(call_ptr != NULL)
  {
    if(call_charge_ptr->use_ccm_timeout)
    {
      ...
      {
        if (call_charge_ptr->last_CCM_started)
        {
          qword time_now = { 0 };
          uint64 curr_uptime_ms;
          time_get_uptime_ms( time_now );
          ...
          call_charge_ptr->carry_forward_CCM += (curr_uptime_ms - call_charge_ptr->last_CCM_started) % ((uint64)call_charge_ptr->ccm_timeout);

          CM_MSG_HIGH_5("call id %d, ccm_timeout %d, curr_uptime_ms %d, last_ccm_start %d, carry_fwd %d",
                        call_charge_ptr->ccm_timeout, curr_uptime_ms, call_charge_ptr->last_CCM_started, call_charge_ptr->carry_forward_CCM, call_ptr->call_id
                       );
        }

        while(call_charge_ptr->carry_forward_CCM >= call_charge_ptr->ccm_timeout)
        {
          CM_MSG_HIGH_2("increment AOC(%d) by %d",call_charge_ptr->AOC,call_charge_ptr->e.e1 * call_charge_ptr->e.e3);
          call_charge_ptr->AOC += call_charge_ptr->e.e1 * call_charge_ptr->e.e3;
          call_charge_ptr->carry_forward_CCM %= call_charge_ptr->ccm_timeout;
        }
...
)
{
  ...
  else
  {
    while(list_call_ptr->next_charged_call != NULL)
    {
      ...
      /* If the next call is after the one being inserted, break out of the while loop */
      if(!list_call_charge_ptr->use_ccm_timeout ||
          list_call_charge_ptr->ccm_timeout >= call_charge_ptr->ccm_timeout)
      {
        break;
...
)
{
  ...
  ccm_timer = &ph_sub_ptr->phone_charge.ccm_timer;
...
)
{
  ...
  if(!cmwaoc_get_list_head(asubs_id) || !IS_VALID_PH_SUB_PTR(asubs_id))
  {
    return;
  }
  ...
  {
    {
      ...
      CM_MSG_HIGH_6("=dbg: updating last_ccm_started from %d to %d, carry %d, ccm_timeout %d, AOC %d, ccm_timeout %d", \
                    list_call_charge_ptr->last_CCM_started, curr_uptime_ms, list_call_charge_ptr->carry_forward_CCM, list_call_charge_ptr->ccm_timeout,\
                    list_call_charge_ptr->AOC, list_call_charge_ptr->ccm_timeout);

      list_call_charge_ptr->last_CCM_started = curr_uptime_ms;

    }
    (void)rex_set_timer(&(phone_charge->ccm_timer), list_call_charge_ptr->ccm_timeout);
...
)
{
  ...
  /* If either aoc is not ready or charge meter timeout for call is 0 return */
  if ( !(CMPH_SS_PHONE_CHARGE(asubs_id,ph_ptr).aoc_ready) ||
       (call_charge_ptr->ccm_timeout == 0)
     )
  {
  ...
  /* Serialize data access call_list_head */
...
)
{
  ...
  ph_ptr->sub_info[asubs_id]->phone_charge.ccm_timer_running = FALSE;
...
)
{
  ...
  */
  if(IS_VALID_PH_SUB_PTR(asubs_id))
  {
    ph_ptr->sub_info[asubs_id]->phone_charge.drop_calls        = FALSE;
    ph_ptr->sub_info[asubs_id]->phone_charge.sent_warning      = FALSE;
    ph_ptr->sub_info[asubs_id]->phone_charge.ccm_timer_running = FALSE;
    ph_ptr->sub_info[asubs_id]->phone_charge.acm_timer_running = FALSE;
    ph_ptr->sub_info[asubs_id]->phone_charge.last_CCM_to_update_ACM = CM_DEFAULT_VALUE;
    ph_ptr->sub_info[asubs_id]->phone_charge.CCM               = CM_DEFAULT_VALUE;
    ph_ptr->sub_info[asubs_id]->phone_charge.aoc_ready         = FALSE;
    ph_ptr->sub_info[asubs_id]->phone_charge.ACM               = CM_DEFAULT_VALUE;
    ph_ptr->sub_info[asubs_id]->phone_charge.ACMmax            = CM_DEFAULT_VALUE;
  }
  call_list_head[asubs_id] = NULL;
}
...
)
{
  ...
  {
    CM_ERR_2("Unable to get session id; session type: %d, err: %d",
             cm_mmgsdi_session_type, cm_mmgsdi_status );
  }
  else
  {
    file_access.access_method = MMGSDI_EF_ENUM_ACCESS;
