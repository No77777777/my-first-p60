...
$Header: //components/rel/mmcp.mpss/7.9.0/mmode/cm/inc/cmregprx.h#1 $
...
#define CMREGPRX_MMOC_MODE_PREF(asubs_id, mmoc_mode)\
  if (asubs_id == SYS_MODEM_AS_ID_2)\
  {\
    mmoc_mode = MMOC_MODE_GWL_2;\
  }\
  else if((asubs_id) == SYS_MODEM_AS_ID_3)\
  {\
    mmoc_mode = MMOC_MODE_GWL_3;\
  }\
  else\
  {\
    mmoc_mode = MMOC_MODE_GWL_1;\
  }
...
SIDE EFFECTS
  None.
===========================================================================*/
extern boolean cmregprx_is_3gpp_inactive
(
  sys_modem_as_id_e_type        asubs_id
);
