...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/mmode/sd/src/sdmmss.c#2 $
...
/********************************************************************************/
/*******************Psuedo MLPL/MSPL file content from  CT SIM *****************/

byte  ct_mlpl_content[] = {  
               0, 57, 1, 0, 8, 1, 5, 7, 
               70, 15, 255, 1, 1, 32, 100, 240,  
               255, 0, 1, 7, 69, 95, 255, 1,  
               1, 32, 84, 245, 255, 0, 1, 0,  
               2, 7, 69, 15, 255, 1, 1, 32,  
               84, 240, 255, 0, 3, 7, 68, 15,
               255, 1, 1, 32, 68, 240, 255, 0,
...
byte ct_mspl_content [] = { 
               0, 36, 1, 0, 8, 3, 1, 4, 
               7, 31, 0, 14, 0, 125, 0, 30,  
               128, 32, 48, 97, 240, 40, 248, 7,  
               220, 6, 8, 12, 62, 5, 31, 0,  
               10, 0, 125, 192, 255, 255, 255, 255, 
               255, 255, 255, 255, 255, 255, 255, 255, 
               255, 255, 255, 255, 255, 255, 255, 255, 
               255, 255, 255, 255, 255, 255, 255, 255,  
               255, 255, 255, 255, 255, 255, 255, 255,  
...
)
{
  ...
  gwl_rec_ptr->rec[idx].band         = (uint8)PRL_LTE_EUTRAN_46;
  gwl_rec_ptr->rec[idx].uarfcn_incl  = PRL_UARFCN_NOT_INCL;

  idx++;

  gwl_rec_ptr->rec[idx].band         = (uint8)PRL_LTE_EUTRAN_47;
  gwl_rec_ptr->rec[idx].uarfcn_incl  = PRL_UARFCN_NOT_INCL;

  idx++;

  gwl_rec_ptr->rec[idx].band         = (uint8)PRL_LTE_EUTRAN_48;
  gwl_rec_ptr->rec[idx].uarfcn_incl  = PRL_UARFCN_NOT_INCL;
  ...
  gwl_rec_ptr->rec[idx].band         = (uint8)PRL_LTE_EUTRAN_71;
  gwl_rec_ptr->rec[idx].uarfcn_incl  = PRL_UARFCN_NOT_INCL;

  idx++;
  gwl_rec_ptr->rec[idx].band         = (uint8)PRL_LTE_EUTRAN_250;
  gwl_rec_ptr->rec[idx].uarfcn_incl  = PRL_UARFCN_NOT_INCL;

  //To add another band, increment idx and then add band to gwl_rec_ptr
  //Should be in contiguous order
  //Ex:
  //idx++;
  //gwl_rec_ptr->rec[idx].band         = (uint8)PRL_LTE_EUTRAN_###;
  //gwl_rec_ptr->rec[idx].uarfcn_incl  = PRL_UARFCN_NOT_INCL;
    ...
    if (bst_rec_type == (byte)SDMMSS_BST_REC_TYPE_1)
    {
      ...
      */
      ...
      for (j = 0, num_rat = 0; j < bst_ptr->bst_tbl[bst_rec].num_mode; j ++)
      {
        ...
        */
        band_chan_rec_type = SD_B_UNPACKB(buf_ptr,offset,SDMMSS_DECODE_SIZE_8_BITS);
        offset += SDMMSS_DECODE_SIZE_8_BITS;
...
)
{
  ...
  byte                         band_chan_rec_type = (byte)SDMMSS_BAND_CHAN_REC_TYPE_1;
  byte                         band           = 0;
  byte                         num_chan       = 0;
...
)
{
  ...
  */
  ...
  if (is_valid)
  {
    ...
    */
    is_valid = sdefs_fs_write_per_subs(efs_id,temp_bst_ptr,SDMMSS_BST_TBL_MAX_SIZE, global_sd_asubs_id);
  ...
  */
  if(temp_bst_ptr != NULL)
  {
    SD_MEM_FREE( temp_bst_ptr, MODEM_MEM_CLIENT_MMODE);
    temp_bst_ptr = NULL;
  }
...
)
{
  ...
  */
  if (is_reset)
  {
    if(plmn_sel_ptr)
    {
      SD_MEM_FREE( plmn_sel_ptr, MODEM_MEM_CLIENT_MMODE);
      plmn_sel_ptr = NULL;
    }
    return NULL;
  }
  ...
  */
  if (is_reset)
  {
    ...
    if(mspl_ptr)
    {
      ...
      {
        SD_MEM_FREE(mspl_ptr->mspl_tbl,MODEM_MEM_CLIENT_MMODE);
        mspl_ptr->mspl_tbl = NULL;
      }
      SD_MEM_FREE( mspl_ptr, MODEM_MEM_CLIENT_MMODE);
      mspl_ptr = NULL;
    }
    return NULL;
  }
    ...
    if(mlpl_ptr)
    {
      if (mlpl_ptr->mlpl_recs)
      {
        ...
        for (k=0; k<mlpl_ptr->num_mlpl_recs; k++)
        {
          ...
          {
            SD_MEM_FREE(mlpl_ptr->mlpl_recs[k].loc_param_val.loc_tag,MODEM_MEM_CLIENT_MMODE);
            mlpl_ptr->mlpl_recs[k].loc_param_val.loc_tag= NULL;
          }
        }

        SD_MEM_FREE(mlpl_ptr->mlpl_recs,MODEM_MEM_CLIENT_MMODE);
        mlpl_ptr->mlpl_recs = NULL;
      }
      SD_MEM_FREE( mlpl_ptr, MODEM_MEM_CLIENT_MMODE);
      mlpl_ptr = NULL;
    }
    return NULL;
  }
...
)
{
  ...
  */
  if (is_reset)
  {
    if(bst_ptr)
    {
      SD_MEM_FREE( bst_ptr, MODEM_MEM_CLIENT_MMODE);
      bst_ptr = NULL;
    }
    return NULL;
  }
...
void         sdmmss_update_tot_list ( void )
{
  ...
  */
  ...
  for(i= 0; i< bst_ptr->length; i++)
  {
    ...
    {
      prl_acq_rec.acq_type = PRL_ACQ_REC_3GPP_GENERIC;
...
EXTERN void sdmmss_extract_bst( void )
{
  ...
  */
  is_valid = sdefs_fs_read_per_subs(SDEFS_BST_TBL, efs_bst_ptr, &efs_bst_size,global_sd_asubs_id);
  ...
  */
  if(efs_bst_ptr != NULL)
  {
    SD_MEM_FREE( efs_bst_ptr, MODEM_MEM_CLIENT_MMODE);
    efs_bst_ptr = NULL;
  }
...
)
{
  ...
  */
  if (is_reset)
  {
    ...
    if(filedb_ptr->mspl)
    {
      SD_MEM_FREE( filedb_ptr->mspl, MODEM_MEM_CLIENT_MMODE);
      SD_MSG_MED_0("EqPRL:Free mem for mspl");
    }

    if(filedb_ptr->mlpl)
    {
      SD_MEM_FREE(filedb_ptr->mlpl, MODEM_MEM_CLIENT_MMODE);
      SD_MSG_MED_0("EqPRL:Free mem for mlpl");
    }

    if(filedb_ptr->plmndb.imsi)
    {
      SD_MEM_FREE(filedb_ptr->plmndb.imsi, MODEM_MEM_CLIENT_MMODE);
      SD_MSG_MED_0("EqPRL:Free mem for imsi");
    }
    if(filedb_ptr->plmndb.ad)
    {
      SD_MEM_FREE(filedb_ptr->plmndb.ad, MODEM_MEM_CLIENT_MMODE);
      SD_MSG_MED_0("EqPRL:Free mem for admin data");
    }
    if(filedb_ptr->plmndb.oplmnwact)
    {
      SD_MEM_FREE(filedb_ptr->plmndb.oplmnwact, MODEM_MEM_CLIENT_MMODE);
      SD_MSG_MED_0("EqPRL:Free mem for oplmnwact");
    }
    if(filedb_ptr->plmndb.plmnwact)
    {
      SD_MEM_FREE(filedb_ptr->plmndb.plmnwact, MODEM_MEM_CLIENT_MMODE);
      SD_MSG_MED_0("EqPRL:Free mem for plmnwact");
    }

    if(filedb_ptr->plmndb.gsm_plmn)
    {
      SD_MEM_FREE(filedb_ptr->plmndb.gsm_plmn, MODEM_MEM_CLIENT_MMODE);
      SD_MSG_MED_0("EqPRL:Free mem for gsm_plmn");
    }

    if(filedb_ptr->plmndb.usim_ehplmn)
    {
      SD_MEM_FREE(filedb_ptr->plmndb.usim_ehplmn, MODEM_MEM_CLIENT_MMODE);
      SD_MSG_MED_0("EqPRL:Free mem for usim_ehplmn");
    }
    SD_MEM_FREE( filedb_ptr, MODEM_MEM_CLIENT_MMODE);
    filedb_ptr = NULL;
    return NULL;
  }
...
static  boolean         sdmmss_is_ignore_uplmn( void )
{
  ...
  is_valid = sdefs_fs_read_per_subs(SDEFS_MMSS_IGNORE_UPLMN, &ign_uplmn, &size,global_sd_asubs_id);
  if(is_valid)
  {
  ...
  switch(file_name)
  {
    ...
    */
    ...
    default:
      ...
      SD_MEM_FREE( new_buf, MODEM_MEM_CLIENT_MMODE);
      new_buf = NULL;
      break;
        ...
        */
        if(sdmmss_is_mcc_mnc_based_sys_rec(&sys_rec,plmn))
        {
          ...
          */
          if(plmn_list)
          {
            SD_MEM_FREE( plmn_list, MODEM_MEM_CLIENT_MMODE);
            plmn_list = NULL;
          }
          return TRUE;
        }
...
)
{
  ...
  */
  ...
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  is_valid          = FALSE;
  acq_rec->acq_type = PRL_ACQ_REC_3GPP_GENERIC;
  acq_rec->type.gwl_generic.version = 1;
...
)
{
  ...
  */
  ...
  for( j = 0; j < intersect_lst->length; j++ )
  {
    ...
    */
    ...
    if(!is_band || !sys_plmn_id_is_valid(intersect_lst->info[j].plmn))
    {

      SD_MSG_HIGH_3("Not added plmn to sys_rec and acq_rec list %x %x %x",intersect_lst->info[j].plmn.identity[0],
                 intersect_lst->info[j].plmn.identity[1],
                 intersect_lst->info[j].plmn.identity[2]);
    ...
    */
    if(prl_hdr_ptr != NULL)
    {
      sys_rec.acq_index = acq_idx + (byte)prl_hdr_ptr->num_acq_recs;
    }
  ...
  */
  ...
  {
    SD_MEM_FREE( intersect_lst, MODEM_MEM_CLIENT_MMODE);
    intersect_lst = NULL;
  }
...
static  void                    sdmmss_grouping_algorithm( void )
{
  ...
  */
  if(mlpl_plmn_list)
  {
    SD_MEM_FREE(mlpl_plmn_list, MODEM_MEM_CLIENT_MMODE);
    mlpl_plmn_list = NULL;
  }
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
...
===========================================================================*/
EXTERN  boolean                   sdmmss_create_equivalent_prl(sys_modem_as_id_e_type as_id)
{
  sdmmss_file_s_type   *filedb_ptr   = NULL;
  sdss_subsc_e_type    subsc         = sdss_get_subsc_type(as_id);
  ...
  */
  ...
  if(filedb_ptr->mspl == NULL || num_mspl_recs == 0 ||
     filedb_ptr->mlpl == NULL || num_mlpl_recs == 0 )
  {
    sdmmss_mspl_s_type           *mspl_ptr = sdmmss_mspl_ptr(TRUE);
    sdmmss_mlpl_s_type           *mlpl_ptr = sdmmss_mlpl_ptr(TRUE);

    SD_MSG_HIGH_0_EXT("EqPRL: No mspl/mlpl", (as_id+1));

	if(sdss_if_psuedo_prl_config_enable(as_id) == TRUE &&
      	 sdss_get_operator_name(as_id) == SD_OPERATOR_CT )
	{
	   SD_MSG_HIGH_0("CT card has NO MMSS file, creating psuedo file ");
	   sd_misc_update_filedb(MMGSDI_TELECOM_MLPL,ct_mlpl_content,sizeof(ct_mlpl_content)/sizeof(ct_mlpl_content[0]));
	   sd_misc_update_filedb(MMGSDI_TELECOM_MSPL,ct_mspl_content,sizeof(ct_mspl_content)/sizeof(ct_mspl_content[0]));
  ...
  */
  ret_status = sdprl_write_to_prl_buffer(as_id);
...
void         sdmmss_update_mmss_gwl_list ( void )
{
  ...
  */
  for(i= 0; i< bst_ptr->length; i++)
  {
    ...
    prl_acq_rec.type.gwl_generic.version = 1;
