...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/mmode/sd/src/sdssscr.h#1 $
...
SD_BEGIN_ENUM(sdssscr_con_e) {
  ...
  */
  ...
  SD_CONSTR_ELEMENT(RESTART_DUP_BAND),
                            /* RESTART_DUP_BAND - restart the
                            ** DUP_BAND  */

  SD_CONSTR_ELEMENT(SET_NEW_ACQ_CNT_FOR_LIST),
                            /* SET_NEW_ACQ_CNT_FOR_LIST - sets
                            ** new_acq_cnt for all the elements of the list to 
                            ** ss ptr new_acq_cnt */
                            
  SD_CONSTR_ELEMENT(AVOID_MAP),
                            /* AVOID_MAP( LIST) - Mark the
                            ** system records that are referenced by the
                            ** system record list/item LIST to be avoided
                            ** from any acquisition attempts for configured seconds
                            */
  ...
  */
  ...
  SD_CONSTR_ELEMENT(SET_LTE_SKIP_PREF_SCAN_FLAG),  /* set lte skip more pref scan flag */
  ...
  SD_CONSTR_ELEMENT(CHK_SET_EXTEND_BSR),   /* check and set flag to extend 1x/LTE BSR scan in C2K alt algorithm */
  ...
  SD_CONSTR_ELEMENT(CHK_SET_TELE_BSR_AFTER_CALL_END),
  SD_CONSTR_ELEMENT(BPLMN_ACQ_DB_TIMER), /* set bplmn acq db timer based on current stage */
  ...
  SD_CONSTR_ELEMENT(LOOP_TIME_OOS),
                            /* LOOP_TIME_OOS - Repeat executing the script
  ...
  SD_CONSTR_ELEMENT(REMOVE_SCANNED_PREF),/*  Set SDSS Pref to the newly added preferences */
  SD_CONSTR_ELEMENT(RESTORE_SCANNED_PREF),/* Set SDSS Pref to the originally given full rat preferences.*/
  SD_CONSTR_ELEMENT(EMERG_SRCH_FAIL), /* used to inform AP the failure after one full scan for e911 call */
  SD_CONSTR_ELEMENT(EMERG_FULL_SRV_FAIL), /* Resetting the is_full_srv_req_during_emerg flag */
  ...
  */
  SD_CONSTR_ELEMENT(COND)         SD_ENUM_ELEMENT_INDEX(= 156),
                                         /* FOR RANGE CHECKING */
  ...
  SD_CONSTR_ELEMENT(IS_FULLRAT_STATUS),
                           /*IS_FULLRAT_STATUS (VAL)-    Evaluates to TRUE 
                           if VAL equals current fullrat status */
  
  SD_CONSTR_ELEMENT(IS_PREV_FULLRAT_STATUS),
                           /*IS_PREV_FULLRAT_STATUS (VAL)-   Evaluates to TRUE 
                           if VAL equals previous fullrat status */
  SD_CONSTR_ELEMENT(IS_SCAN_SCOPE),
                            /* IS_SCAN_SCOPE(  VAL ) - Evaluates to TRUE
                            ** if scan scope mentioned in VAL are enabled. */

                                 /* 219 */
  SD_CONSTR_ELEMENT(IS_PREF_UPDATE_REAS_MASK),
                            /* IS_PREF_UPDATE_REAS_MASK(  VAL ) - Evaluates to TRUE
                            ** iff VAL includes the pref update reason 
                            ** of the last preferred system changed. */

  SD_CONSTR_ELEMENT(IS_TRUE_EVENT),
                            /*IS_TRUE_EVENT(VAL) - Evaluates to TRUE 
                            if VAL equals current true event.*/

  SD_CONSTR_ELEMENT(IS_EMERG_CALL_END_IN_FULL_SRV),
                            /*IS_TRUE_EVENT(VAL) - Evaluates to TRUE 
                            if VAL equals current true event.*/

  SD_CONSTR_ELEMENT(IS_CNT_SUB),
...
typedef enum {
  ...
  SDSSSCR_SRV_LOST_GW_ORIG_NO_PREF_CHGD,
  ...
  SDSSSCR_CALLED_RESERVED_35,

                                  /* 92 */
  SDSSSCR_CALLED_RESERVED_36,           

  SDSSSCR_CALLED_RESERVED_37,      
  ...
  SDSSSCR_3GPP_SRV_LOST_C2K_SCAN_OPT,  /* optimize 3gpp2 scans during WRLF or IDLE FULL RAT mode*/ 
 
                                  /* 108 */
  SDSSSCR_START_BSR_TIMER,      /* Script to BSR timers */
  
  SDSSSCR_GWL_OPR_SRV_INFO_SYS_LOST, /* Do GWL opr srv info and sys lost processing */
  ...
  SDSSSCR_CAMP_FULL_MODE_SCAN,   /* If device in CAMP_ONLY mode perform REG_BEST_PREF acq */

  SDSSSCR_CAMP_MINI_MODE_SCAN,

  SDSSSCR_CREATE_EMERG_LIST,  /*  populate emergency list */

  SDSSSCR_ACQ_DB_EMERG_CALL_ORIG, /* Acq DB for Em call orig cases */

  SDSSSCR_ORIG_END_ACQ_DB, /* Acq DB for orig end case */

  SDSSSCR_SRV_LOST_CHINA_EMERG_MODE, /*service lost script for first round druring emerg call */
...
#define SDSSSCR_MICRO2_SYS ((uint16) (-11))  /* Start acq from the system that is
                                         ** one after the system that equal
                                         ** to SDSR_ACQ_MICRO_SYS */

#define SDSSSCR_FULL_OSR_SYS  ((uint16) (-12))
                                         /* Start acq from the system that is
                                         ** one after the system that equal
                                         ** to SDSR_ACQ_FULL_OSR_SYS */

#define SDSSSCR_FULL_AOOS_SYS  ((uint16) (-13))
                                         /* Start acq from the system that is
                                         ** one after the system that equal
                                         ** to SDSR_ACQ_FULL_AOOS_SYS */

#define SDSSSCR_MICRO_ABSR_SYS ((uint16) (-14))
                                         /* Start acq from the system that is
                                         ** one after the system that equal
...
#define SDSSSCR_PREV_TIMELINE                          ((word) (-3))
...
/* Enumeration of pref_update_reas combinations.
*/
typedef enum {

  SDSSSCR_SD_SS_PREF_UPDATE_REASON_MASK_NONE        = BM( SD_SS_PREF_UPDATE_REASON_NONE ),

  SDSSSCR_SD_SS_PREF_UPDATE_REASON_MASK_UE_MODE_CHGD = BM( SD_SS_PREF_UPDATE_REASON_SWITCH_UE_MODE_NO_STK_CHG ) |
                                                       BM( SD_SS_PREF_UPDATE_REASON_PM_UE_MODE_CHG ),

  SDSSSCR_SD_SS_PREF_UPDATE_REASON_MASK_MAX
} sdssscr_pref_update_reas_mask_e_type;
...
typedef enum {
  ...
  SDSSSCR_COND_3GPP_SRV_LOST_C2K_SCAN_OPTI_ENABLED,   /* Optimization to scan 3gpp2 
                                  during WRLF or IDLE FULL_RAT mode, is enabled */
  ...
  SDSSSCR_COND_EMERG_ATTACHED, /* Check for emerg attached */

  SDSSSCR_COND_FULLRAT_OPT,/* CHECK WHETEHR FULLRAT OPTIMISATION IS ENABLED */

  SDSSSCR_COND_CONN_MODE_SET, /* CHeck for conn mode */

  SDSSSCR_COND_SRV_DOMAIN_PREF_CHG_VOLTE_PS, /* Check if the service domain pref is
                               changed from CS_VOLTE <-> CS_PS or VOLTE <-> PS_ONLY */

  SDSSSCR_COND_3GPP_BPLMN_MODE_PREF,
...
typedef enum {
  ...
  SDSSSCR_EFS_ITEM_CALL_END_TELEBSR_DELTA_TIMER = 11,
  /* Threshold timer value to perform telebsr after call end */

  SDSSSCR_EFS_ITEM_HDR_ALT_ALGR = 12,
  /**< EFS id to enable alternate HDR scan algorithm */

  SDSSSCR_EFS_ITEM_LTE_TELE_BSR_MAX_TIME = 13,
  /**< EFS id to set the maximum time for the LTE telescopic BSR */

  /* EFS items associated with 3GPP OOS timeline start
  */
  SDSSSCR_EFS_ITEM_3GPP_SLOW_SCAN_ENABLED = 14,
  /* EFS id to enable/disable slow OOS/limited to full scans
     Default value = 0 ( disabled) 
     Can be either 1 or 0
  */

  SDSSSCR_EFS_ITEM_3GPP_OOS_T_NORM_MAX = 15,
  ...
  */
  ...
  SDSSSCR_EFS_ITEM_3GPP_OOS_TIME_DELTA = 19,
  ...
  */
  ...
  SDSSSCR_EFS_ITEM_3GPP_BSR_T_NORM_MAX = 20,
  /* EFS id to determine max time between limited to full scans in normal mode
     ( not stationary and not low battery). 
   Default value = 60. Should be an integer in the range 30 - 180.
   */

  SDSSSCR_EFS_ITEM_3GPP_BSR_REPS_PER_STAGE = 21,
  /**< EFS id to set the number of scans per stage of the BSR timeline 
    Default value = 1.
    It can only be an integer in the range 1 - 15.
  */

  SDSSSCR_EFS_ITEM_3GPP_BSR_MAX_TIME = 22,
  /**< BSR timer will be capped at this value .
    Default value = 300s.
    It can be an integer in the range 1 - 600s
  */

  SDSSSCR_EFS_ITEM_3GPP_BSR_INITIAL_VAL = 23,
  /**< Timer value TO used for inital BSR 
    The timer values must be integers in the range 1 to 600 to be considered valid
  */

  SDSSSCR_EFS_ITEM_3GPP_BSR_TIME_DELTA = 24 ,
  /**< Timer value Tdelta used for BSR
     Timer[stagex] = T0 + Tdelta * x      
  */

  SDSSSCR_EFS_ITEM_HDR_MAPE_AVOIDANCE_TIMER = 32,
  /**< Timer value 1 used for HDR MAPE timeline 
    The timer values must be integers in the range 1 to 120 to be considered valid
  */

  SDSSSCR_EFS_ITEM_HDR_MAPE_AVOIDANCE_TIMER_MAX = 33,
  /**< Timer value 1 used to restrict the range of HDR MAPE timeline 
     The timer values must be integers to be considered valid
  */
...
**
** UPDATE sd_device_config_timers_default.items to include default values for all new
** enums that are added.
**
** DO NOT DELETE any enum before you run out of space in the NV array. If you
** end up deleting unused enums due to lack of space in the NV array, then
** increment the value of "sd_device_config_timers_default.version" by 1.
*/
typedef enum {

  SDSSSCR_SD_TELE_BSR_ALGR_NDDS = 0,
  /* EFS item to enable Tele BSR on MMSS non DDS sub
  */

  SDSSSCR_SD_CDMA_TELE_BSR_STAGE1_TIME_NDDS = 1,
  /* EFS item to store 1x stage 1 time
  */

  SDSSSCR_SD_CDMA_TELE_BSR_STAGE2_TIME_NDDS = 2,
  /* EFS item to store 1x stage 2 time
  */

  SDSSSCR_SD_LTE_TELE_BSR_TIME_NDDS = 3,
  /* EFS item to store the starting timer for LTE BSR
  */

  SDSSSCR_SD_TMR_LTE_LONG_RESEL_NDDS = 4,
  /*EFS item to store the Long BSR timer for LTE
  */

  SDSSSCR_SD_LTE_NOT_AVAIL_ACQ_FAIL_MAX_COUNT_NDDS = 5,
  /* LTE ACQ_FAIL Max count to declare LTE Not Available area
  ** Trigger LBSR if the fail counter is equal or bigger than the max count
  */

  SDSSSCR_SD_LTE_TELE_BSR_STAGE_NDDS = 6,
  /* LTE telescopic BSR stage indicate how many BSR timer incremental stages
  ** will be used
  */

  SDSSSCR_SD_LTE_TELE_BSR_RPT_COUNT_NDDS =7,
  /* LTE telescopic BSR repeat counter indicate how many times one stage timer
  ** will run
  */

  /* After adding a new enum, do the following:
  ** if (SDSSSCR_SD_DEVICE_CONFIG_TIMERS_MAX > SDSSSCR_SD_DEVICE_CONFIG_TIMERS_SLOTS )
  **   Adjust SDSSSCR_SD_DEVICE_CONFIG_TIMERS_SLOTS such that
  **     SDSSSCR_SD_DEVICE_CONFIG_TIMERS_SLOTS >= SDSSSCR_SD_DEVICE_CONFIG_TIMERS_MAX and
  **     Work with Diag team to update QXDM NV Browser software with new
  **       value of SDSSSCR_SD_DEVICE_CONFIG_TIMERS_SLOTS.
  */
  SDSSSCR_SD_DEVICE_CONFIG_TIMERS_MAX,
    /* Internal use. */

  SDSSSCR_SD_DEVICE_CONFIG_TIMERS_SLOTS = 20

} sdssscr_sd_device_config_timers_e_type;


/* Local structure for storing configurable SD device level timers read from EFS.
*/
typedef PACKED struct PACKED_POST {
  ...
  /* Configurable SD values */
  uint32                                        items[SDSSSCR_EFS_ITEM_SLOTS];
} sdssscr_sd_device_config_timers_s_type;
...
===========================================================================*/
EXTERN  void                      sdssscr_efs_items_init( uint8 submask );
...
===========================================================================*/
EXTERN  void                      sdssscr_read_sd_device_config_timers(void);
