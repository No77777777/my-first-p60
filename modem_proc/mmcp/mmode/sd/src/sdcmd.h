...
typedef struct
{
  ...
  sd_si_data_s_type           *si_data[MAX_SIMS][MM_STACK_ALL];
...
#define sd_stack_enabled(stk_id) \
               (curr_stack_mask & BM(stk_id))

#define sd_stack_enabled_subs(asubs_id,stk_id) \
               ((stack_mask[asubs_id].current_stack_active_mask) & BM(stk_id))
...
typedef struct
{
  ...
  sdss_man_srch_bst_node_s_type plmn_rat_band_data;
} sd_update_manual_search_bst_cmd;
...
typedef struct
{
  ...
  /* TO DO: Added only for compilation, need to be replaced*/
...
typedef struct
{
  ...
  */
  ...
  boolean                         status;
} sd_misc_mru_update_cmd;

/* Type for holding the per sub emergency specific information.
*/
typedef struct
{  
  cm_e911_pending_state_e_type        pending_emerg_state;
  /* flag to indicate if there is a pending emergency call */
...
sd_ss_common_s_type   *sd_ss_common_ptr(void);
...
sd_si_s_type *sd_si_common(void);
...
#define sd_invoke_info_callback(stk_id)  sd_invoke_info_callback_subs(global_sd_asubs_id,stk_id)
...
#define sd_si_is_srv_on(stk_id)   sd_si_is_srv_on_subs(global_sd_asubs_id,stk_id)
...
EXTERN  void                          sd_si_reg(
  ...
  */
   ...
   ** this function once the BSR status has changed */
   ...
   sd_si_emerg_srch_fail_f_type      *emerg_srch_fail_cb_func
   /**<SD report for sending scan failure during e911*/
...
boolean
_sd_ss_hybr_meas_list_get2 ( sd_ss_meas_list_cmd *cmd);
...
boolean
_sd_ss_hybr_meas_list_rprt ( sd_ss_meas_list_rprt_cmd  *cmd);
...
boolean                   sdcmd_hybr_meas_list_get2(
  sd_ss_meas_list_cmd *cmd
);
...
===========================================================================*/
EXTERN boolean sd_ss_is_acq_status( mm_sub_stk_id_s_type  mm_id, sys_acq_status_e_type acq_status);

EXTERN sdss_common_s_type             *sdss_common_ptr(void);

EXTERN void                      sd_ss_init(

  sd_ss_s_type              *ss_ptr,
  sys_modem_as_id_e_type     asubs_id
);

EXTERN void                      sd_si_init(
...
===========================================================================*/
EXTERN void sd_ss_set_global_ptr(void);
...
===========================================================================*/
EXTERN void sd_ss_set_curr_stack_mask(void);
...
===========================================================================*/
extern void sdcmd_process_sensor_chg (
  sd_cmd_t *cmd
);
#endif  //FEATURE_MMODE_CAFI

#define EVENT_SD_RPT_EXT_VERSION DIAG_EXT_API_VERSION_1

/*===========================================================================
FUNCTION  sd_sys_as_id_to_uint32
...
} sd_ss_ind_misc_active_subs_update_cmd;
