...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/mmode/sd/src/sd.c#4 $
...
02/02/03   RC      Added a call to sd_si_ok_to_orig( MM_STACK_0 ) in
                   sd_ss_ind_misc_orig_success().
...
extern sd_ss_stack_active_mask_s_type stack_mask[MAX_SIMS];
extern uint8 curr_stack_mask;
...
)
{
  ...
  cmd = sdtask_sd_cmd_new(sizeof(sd_cmd_t), fn, rtrn_act_ptr, NULL,mm_id.asubs_id, mm_id.stk_id);
...
)
{
  ...
  cmd = sdtask_sd_cmd_new(sizeof(sd_cmd_t), fn, rtrn_act_ptr, NULL, asubs_id, stk_id);
  act = sdtask_queue_put_cmd_wait(cmd);
  ref_cnt_obj_release(cmd);
...
)
{
  ...
  /* Legacy API, Default mm_id for Sub1 */
...
FUNCTION sd_ss_meas_list_get_per_ss
...
===========================================================================*/
extern  boolean                   sd_ss_meas_list_get_per_ss(
...
FUNCTION sd_ss_meas_list_get_per_sub
...
===========================================================================*/
EXTERN  boolean                   sd_ss_meas_list_get_per_sub(
...
)
{
  ...
  */
  ...
  cmd->pos = pos;
  cmd->rtrn_mode_ptr = &mode_pref;
  cmd->rtrn_band_ptr = &band_pref;
  cmd->rtrn_chan_ptr = rtrn_chan_ptr;
  cmd->rat_pri_list_ptr = &rat_pri_list;
...
FUNCTION sd_ss_meas_list_rprt_per_sub
...
EXTERN  boolean                   sd_ss_meas_list_rprt_per_sub(
...
)
{
  ...
  return sd_ss_meas_list_rprt_per_ss(pos,meas_val, mm_id);
}
...
FUNCTION sd_ss_meas_list_rprt_per_ss
...
===========================================================================*/
boolean                   sd_ss_meas_list_rprt_per_ss(
...
{
  return sd_ss_generic_cmd(sdcmd_user_offline_cdma, asubs_id, MM_STACK_0, rtrn_act_ptr);
}
...
{
  return sd_ss_generic_cmd(sdcmd_user_pwr_save_exit, asubs_id, MM_STACK_0, rtrn_act_ptr);
}
...
EXTERN  sd_ss_act_e_type                    sd_ss_ind_user_ss_pref8(
  ...
  */
  ...
  const sys_addtl_action_s_type       *addl_action,
...
)
{
  ...
  fn = (execute_fn) sdcmd_user_ss_pref8;
  ...
  cmd->fullrat_status= fullrat_status;
  cmd->csg_id = csg_id;
  cmd->csg_rat=csg_rat;
  cmd->voice_domain_pref = voice_domain_pref;
  cmd->rat_capability = rat_capability;
  cmd->addl_action = addl_action;
  cmd->pending_emerg_state = CM_E911_PENDING_STATE_OFF;
  act = sdtask_queue_put_cmd_wait((sd_cmd_t *) cmd);
  ref_cnt_obj_release(cmd);
...
{
  return sd_ss_generic_cmd(sdcmd_user_nam_sel, mm_id.asubs_id,mm_id.stk_id, rtrn_act_ptr);
}
...
{
  return sd_ss_generic_cmd(sdcmd_user_pwr_down, mm_id.asubs_id,mm_id.stk_id, rtrn_act_ptr);
}

EXTERN  sd_ss_act_e_type          sd_ss_ind_term(
...
{
  return sd_ss_generic_cmd(sdcmd_user_term, mm_id.asubs_id,mm_id.stk_id, rtrn_act_ptr);
}
...
)
{
  ...
  cmd = (sd_ss_ind_user_ss_pref_cmd *) sdtask_sd_cmd_new(
          sizeof(sd_ss_ind_user_ss_pref_cmd),
          (execute_fn) sdcmd_hybr_2_user_ss_pref6,
          rtrn_act_ptr,
          NULL,
...
)
{
  ...
  multimode_stack_e_type stk_id = sd_misc_get_ss_for_mode(asubs_id,mode);

  if(stk_id < MM_STACK_0 || stk_id > MM_STACK_2)
  {
    stk_id = MM_STACK_0;
  }
...
FUNCTION sd_ss_ind_user_get_net_extn

DESCRIPTION
  Inform SD that the user issued a get networks command.
  Extended old interface to include ARFCN parameters for periodic
  frequency search.
...
===========================================================================*/
EXTERN  sd_ss_act_e_type          sd_ss_ind_user_get_net_extn(
  ...
  */
  ...
  /* Ignore Manual Search BST filtering
  */
  ...
  */
  ...
  const sys_periodic_frequency_scan_info_s_type  *pf_scan_info,
...
)
{
  ...
  cmd->ignore_manual_bst = ignore_manual_bst;
  cmd->is_requeue = is_requeue;
  cmd->pf_scan_info      = pf_scan_info;
  cmd->incremental_results = incremental_results;
  cmd->max_search_time     = max_search_time;

  SD_MSG_HIGH_4("GET_NET: sub %d stk %d list_type %d ignore_mbst %d",
                mm_id.asubs_id,
                mm_id.stk_id,
                cmd->list_type,
                 cmd->ignore_manual_bst);
...
{
  return sd_ss_generic_ss_cmd(sdcmd_user_term_get_net_gw_ss, mm_id, rtrn_act_ptr);
}
...
{
  return sd_ss_generic_cmd(sdcmd_cdma_acq_failed_pilot, asubs_id, MM_STACK_0, rtrn_act_ptr);
} /* sd_ss_ind_cdma_acq_failed_pilot */
...
{
  return sd_ss_generic_cmd(sdcmd_cdma_acq_failed_sync2, asubs_id, MM_STACK_0, rtrn_act_ptr);
} /* sd_ss_ind_cdma_acq_failed_pilot */
...
{
  return sd_ss_generic_cmd(sdcmd_cdma_acq_prot_mis, asubs_id, MM_STACK_0,rtrn_act_ptr);
} /* sd_ss_ind_cdma_acq_prot_mis */
...
{
  return sd_ss_generic_cmd(sdcmd_cdma_opr_prot_mis, asubs_id, MM_STACK_0, rtrn_act_ptr);
}
...
===========================================================================*/
sd_ss_act_e_type          sd_ss_ind_cdma_acq_schm(
...
{
  return sd_ss_generic_cmd(sdcmd_cdma_acq_ho_fail, asubs_id, MM_STACK_0, rtrn_act_ptr);
...
{
  return sd_ss_generic_cmd(sdcmd_cdma_acq_redir_abort, asubs_id, MM_STACK_0, rtrn_act_ptr);
}
...
{
  return sd_ss_generic_cmd(sdcmd_cdma_opr_rescan, asubs_id, MM_STACK_0, rtrn_act_ptr);
}
...
{
  return sd_ss_generic_cmd(sdcmd_cdma_opr_reg_rej, asubs_id, MM_STACK_0, rtrn_act_ptr);
}
...
)
{
  ...
  */
  ...
  return( sd_ss_ind_cdma_opr_redir( asubs_id,
                                    is_rif,
                                    redir_type,
                                    &rec_list,
                                    1,
...
{
  return sd_ss_generic_cmd(sdcmd_cdma_opr_acpt_rej, asubs_id, MM_STACK_0, rtrn_act_ptr);
}
...
{
  return sd_ss_generic_cmd(sdcmd_cdma_opr_ndss_off, asubs_id, MM_STACK_0, rtrn_act_ptr);
}
...
{
  return sd_ss_generic_cmd(sdcmd_cdma_opr_lock, asubs_id, MM_STACK_0, rtrn_act_ptr);
}
...
{
  return sd_ss_generic_cmd(sdcmd_cdma_opr_unlock, asubs_id, MM_STACK_0, rtrn_act_ptr);
}
...
{
  return sd_ss_generic_cmd(sdcmd_cdma_opr_cam_err, asubs_id, MM_STACK_0, rtrn_act_ptr);
}
...
{
  return sd_ss_generic_cmd(sdcmd_cdma_opr_release, asubs_id, MM_STACK_0, rtrn_act_ptr);
}
...
)
{
  ...
  cmd->fullrat_status= SD_SS_FULLRAT_NO_CHANGE;
  cmd->csg_rat = SYS_RAT_NONE;
  cmd->voice_domain_pref = SYS_VOICE_DOMAIN_PREF_NO_CHANGE;
  cmd->rat_capability = SD_SS_MODE_PREF_ANY;
  cmd->addl_action = NULL;
...
FUNCTION sd_ss_ind_hdr_user_ss_pref4
...
EXTERN  sd_ss_act_e_type          sd_ss_ind_hdr_user_ss_pref4(
...
)
{
  ...
  /* Call the HDR API
  */
  return ( sd_ss_ind_hdr_user_ss_pref3(asubs_id, pref_reason,
                                       orig_mode,
                                       mode_pref,
                                       band_pref,
                                       prl_pref,
  ...
  return (sd_ss_ind_user_ss_pref8( asubs_id, pref_reason,
                                   orig_mode,
                                   mode_pref,
                                   band_pref,
...
FUNCTION sd_ss_ind_hybr_1_user_ss_pref4
...
===========================================================================*/
EXTERN  sd_ss_act_e_type          sd_ss_ind_hybr_1_user_ss_pref4(
...
)
{
  ...
  /* Call the HDR API
  */
  return ( sd_ss_ind_hdr_user_ss_pref4(asubs_id, pref_reason,
                                       orig_mode,
                                       mode_pref,
                                       band_pref,
                                       prl_pref,
...
} /* sd_ss_ind_hybr_1_user_ss_pref4 */
...
)
{
  ...
  multimode_stack_e_type stk_id = sd_misc_get_ss_for_mode(asubs_id,SYS_SYS_MODE_HDR);

  if(stk_id != MM_STACK_1 && stk_id != MM_STACK_0)
  {
    stk_id = MM_STACK_1;
  }
...
{
  return sd_ss_generic_cmd(sdcmd_hybr_1_user_pwr_save_exit, asubs_id, MM_STACK_1, rtrn_act_ptr);
}
...
{
  return sd_ss_generic_cmd(sdcmd_hybr_2_user_pwr_save_exit, asubs_id, MM_STACK_2, rtrn_act_ptr);
}
...
)
{
  ...
  if(sd_misc_is_hybr_opr(asubs_id))
  {
    stk_id = MM_STACK_1;
  }
...
)
{
  ...
  */
  ...
  return sd_ss_generic_cmd(sdcmd_hdr_acq_failed_pilot, asubs_id, stk_id, rtrn_act_ptr);
}
...
)
{
  ...
  */
  ...
  sdss_set_flag_subs(asubs_id, MM_STACK_2, SDSS_FLAG_HDR_DEACT_BY_HYBR2_BSR, FALSE);
  ...
  return sd_ss_generic_cmd(sdcmd_hdr_acq_failed_sync, asubs_id, stk_id, rtrn_act_ptr);
}
...
)
{
  ...
  return sd_ss_generic_cmd(sdcmd_hdr_acq_prot_mis, asubs_id, stk_id, rtrn_act_ptr);
}
...
)
{
  ...
  return sd_ss_generic_cmd(sdcmd_hdr_opr_prot_mis, asubs_id, stk_id, rtrn_act_ptr);
}
...
)
{
  ...
  return sd_ss_generic_cmd(sdcmd_hdr_acq_redir_abort, asubs_id, stk_id, rtrn_act_ptr);
}
...
===========================================================================*/
extern  sd_ss_act_e_type          sd_ss_ind_hdr_opr_oh_info(
...
)
{
  ...
  return sd_ss_generic_cmd(sdcmd_hdr_opr_resel_ok, asubs_id, stk_id, rtrn_act_ptr);
}
...
)
{
  ...
  return sd_ss_generic_cmd(sdcmd_hdr_opr_ses_abort, asubs_id, stk_id, rtrn_act_ptr);
}
...
)
{
  ...
  return sd_ss_generic_cmd(sdcmd_hdr_opr_ses_nego_timeout, asubs_id, stk_id, rtrn_act_ptr);
}
...
)
{
  ...
  return sd_ss_generic_cmd(sdcmd_hdr_opr_ses_opened, asubs_id, stk_id, rtrn_act_ptr);
}
...
{
  return sd_ss_ind_hdr_opr_start_activity( asubs_id, rtrn_act_ptr,
...
{
  return sd_ss_ind_hdr_opr_stop_activity( asubs_id, rtrn_act_ptr,
                                          SD_SS_HDR_ACT_MASK_TRAFFIC);
} /* sd_ss_ind_hdr_opr_release */
...
{
  return sd_ss_generic_ss_cmd(sdcmd_lte_suitable_search_end, mm_id, rtrn_act_ptr);
}

/*===========================================================================

@FUNCTION sd_ss_irat_fail_ind

@DESCRIPTION
  Reset SD's IRAT flag.
...
===========================================================================*/
extern void sd_ss_hdr_irat_fail_ind( sys_modem_as_id_e_type asubs_id )
{
  (void) sd_ss_generic_cmd((execute_fn) sdcmd_hdr_irat_fail_ind, asubs_id,MM_STACK_0, NULL );
}
...
{
  (void) sd_ss_generic_ss_cmd((execute_fn) sdcmd_reset_rlf_conn_mode_status, mm_id, NULL);
}
...
{
  return sd_ss_generic_ss_cmd(sdcmd_wcdma_suitable_search_end, mm_id, rtrn_act_ptr);
}
...
{
  return sd_ss_generic_ss_cmd(sdcmd_wcdma_conn_mode_end, mm_id, rtrn_act_ptr);
}
...
(          /*lint -esym(715, chan) */ /* param not referenced */
  ...
  */
  ...
  boolean                 only_for_ui,
  /* should be used to indicate CM that the vplmn camped is not
  the manually selected plmn or the NAS requested plmn.*/
...
{
  return sd_ss_generic_ss_cmd(sdcmd_gw_opr_acc_done_ss, mm_id, rtrn_act_ptr);
}
...
{
  return sd_ss_generic_ss_cmd(sdcmd_gw_ps_detach, mm_id, rtrn_act_ptr);
}
...
)
{
  ...
  return sd_ss_ind_misc_meas_rprt_per_ss (meas_id, resel_is_allowed, rtrn_act_ptr, mm_id );
}
...
FUNCTION sd_ss_ind_misc_meas_rprt_per_ss
...
EXTERN  sd_ss_act_e_type          sd_ss_ind_misc_meas_rprt_per_ss(
...
{
  return sd_ss_generic_cmd(sdcmd_misc_orig_success, asubs_id, MM_STACK_0, rtrn_act_ptr);
}
...
{
  return sd_ss_generic_cmd(sdcmd_misc_pwr_save_enter, mm_id.asubs_id, mm_id.stk_id, rtrn_act_ptr);
}
...
{
  return sd_ss_generic_cmd(sdcmd_misc_cnf, asubs_id, MM_STACK_0, rtrn_act_ptr);
}
...
EXTERN  void                           sd_misc_mru_write( sys_modem_as_id_e_type asubs_id )
{
  (void) sd_ss_generic_cmd((execute_fn) sdcmd_misc_mru_write, asubs_id, MM_STACK_0, NULL);
}
...
FUNCTION sd_update_manual_search_bst
...
EXTERN boolean sd_update_manual_search_bst (
  /*PLMN */
  sys_plmn_id_s_type            plmn,
...
)
{
  ...
  {
    local_band_pref.lte_band = sys_map_band_mask_to_lte_band_mask(band);
  }
  else
  {
    local_band_pref.chgwt_band = band;
  }

  return sd_update_manual_search_bst_1(plmn,mode,local_band_pref);
}
...
{
  sd_ss_act_e_type            act = SD_SS_ACT_CONTINUE;
  sd_update_manual_search_bst_cmd  *cmd = NULL;

  /*To do: will move  manual bst data to common from sub level after internal discussion*/
  if(sd_is_manual_bst_valid(global_sd_asubs_id))
  {
    SD_MSG_HIGH_1("MAN_SRCH BST Update Ind from RAT:%d",mode);

    /*Check if Entry already present in BST*/
    cmd = (sd_update_manual_search_bst_cmd *) sdtask_sd_cmd_new(
            sizeof(sd_update_manual_search_bst_cmd),
            (execute_fn) sdcmd_update_manual_search_bst,
            NULL,
            NULL,
            global_sd_asubs_id,
            MM_STACK_0
    ...
    cmd->plmn_rat_band_data.plmn = plmn;
    cmd->plmn_rat_band_data.mode = mode;
    cmd->plmn_rat_band_data.band_pref = band;
    act = sdtask_queue_put_cmd_wait((sd_cmd_t *) cmd);
    ref_cnt_obj_release(cmd);
    return (boolean)act;
  }
...
FUNCTION sd_update_manual_search_bst_per_subs_1
...
EXTERN boolean sd_update_manual_search_bst_per_subs_1 (
  /*PLMN */
  sys_plmn_id_s_type            plmn,
  /*RAT and band on which acq success*/
  sys_sys_mode_e_type           mode,
...
)
{
  ...
  if(sd_is_manual_bst_valid(as_id))
  {
    SD_MSG_HIGH_1("MAN_SRCH BST Update Ind from RAT:%d",mode);
...
FUNCTION sd_update_manual_search_bst_per_subs
...
EXTERN boolean sd_update_manual_search_bst_per_subs (
  /*PLMN */
  sys_plmn_id_s_type            plmn,
  /*RAT and band on which acq success*/
  sys_sys_mode_e_type           mode,
...
)
{
  ...
  if(sd_is_manual_bst_valid(as_id))
  {
    SD_MSG_HIGH_2("MAN_SRCH BST Update Ind from RAT:%d on as_id:%d",mode,as_id);
    ...
    cmd->plmn_rat_band_data.plmn = plmn;
    cmd->plmn_rat_band_data.mode = mode;
    cmd->plmn_rat_band_data.band_pref = local_band_pref;
    cmd->as_id = as_id;
    act = sdtask_queue_put_cmd_wait((sd_cmd_t *) cmd);
    ref_cnt_obj_release(cmd);
    return (boolean)act;
...
)
{
  ...
  (void)sd_ss_act_get_true_curr_act_ss(mm_id,&ss_act);
  ss_act.prm.acq_gwl.net_sel_type = nw_sel_mode;
  sd_ss_act_set_true_curr_act_ss(mm_id,&ss_act);
...
EXTERN  sd_ss_act_e_type    sd_ss_ind_gw_data_suspended_ss (
  mm_sub_stk_id_s_type  mm_id
)
{
  ...
  */
  sd_si_update_data_suspend_state (mm_id, TRUE,  si_info_ptr->srv_status);
#endif
...
===========================================================================*/
EXTERN  word                      sd_misc_prl_ver_get( sys_modem_as_id_e_type asubs_id )
{
  return prl_hdr_nv_get_ptr(asubs_id)->pr_list_id;
}
...
{
  sdprl_update_cdma_imsi(asubs_id,card_imsi_val,data_size);
}
...
FUNCTION sd_misc_update_3gpp_imsi
...
===========================================================================*/
EXTERN  void                   sd_misc_update_3gpp_imsi(
  sys_modem_as_id_e_type        asubs_id,
...
)
{
  ...
  sdprl_update_3gpp_imsi(asubs_id,mcc,mnc);
...
)
{
  ...
  if(mm_id.stk_id == MM_STACK_1 &&
      sd_ss_ptr_subs(mm_id)!= NULL)
  {
    /* This is needed to HDR_USER_HYBR_START for first HYBR_HDR kick */
    SD_CRIT_SECT_ENTER();
    sd_ss_ptr_subs(mm_id)->is_start_instance = TRUE;
    SD_CRIT_SECT_EXIT();
  }

  /* Call on the SS-Core to update its operating mode.
  */
  sdss_opr_mode_update( mm_id,SDSS_OPR_MODE_ONLINE );
}
...
)
{
  /* Call on the SS-Core to update its operating mode.
  */
  sdss_opr_mode_update( mm_id, SDSS_OPR_MODE_PWR_DOWN );
}
...
)
{
  ...
  {
    SD_MSG_HIGH_0("Checking for Target Support");
...
===========================================================================*/
EXTERN  sys_lte_band_mask_e_type  sd_misc_get_supp_band_pref_lte( sys_modem_as_id_e_type asubs_id)
{
  mm_sub_stk_id_s_type   mm_id;
  mm_id.stk_id = sdss_get_ss_for_mode( asubs_id, SD_MODE_LTE );
  mm_id.asubs_id = asubs_id;

  SD_MSG_HIGH_2("sd_misc_get_supp_band_pref_lte(): LTE sub %d stk %d",mm_id.asubs_id, mm_id.stk_id);
  ...
  return sdss_get_curr_lte_band_pref( mm_id );
}
...
FUNCTION sd_set_custom_emerg_info

 Set the custom emerg info at SD. 
 SD gets the values from CM and SD will not read the NV on its own.  
...
EXTERN void sd_set_custom_emerg_info(
boolean is_ral_stored_in_efs, 
boolean is_alternate_redial_algorithm,
sys_modem_as_id_e_type asubs_id )

{
   sdss_set_custom_emerg_info(is_ral_stored_in_efs,is_alternate_redial_algorithm, asubs_id);
}
...
)
{
  ...
  */
  ...
  if(!target_support)
  {
    SD_MSG_HIGH_4("is_supp_pref:Return as mode=0x%x band=0x%08x 0x%08x target_support:%d",
                  mode_pref,
...
===========================================================================*/
EXTERN  boolean                   sd_misc_is_cdma_locked(
  sys_modem_as_id_e_type    asubs_id
  /* Subscription ID
  */
)
{
  ...
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  return sdss_is_cdma_locked(asubs_id);
...
===========================================================================*/
EXTERN  boolean                   sd_misc_is_cdma_pseudo_online(
  sys_modem_as_id_e_type    asubs_id
  /* Subscription ID
  */
)
{
  ...
  return( sdss_get_pref_reason(mm_id) == SD_SS_PREF_REAS_PSEUDO_ONLINE );
}
...
===========================================================================*/
EXTERN  byte                      sd_misc_get_cdma_request_mode(
  sys_modem_as_id_e_type    asubs_id
  /* Subscription ID
  */
)
{
  return sdss_get_cdma_request_mode(asubs_id);
}
...
===========================================================================*/
EXTERN  byte                      sd_misc_get_cdma_return_cause(
  sys_modem_as_id_e_type    asubs_id
  /* Subscription ID
  */
)
{
  ...
  return sd_ss_ptr_subs(mm_id)->return_cause;
}
...
)
{
  ...
  return sd_misc_get_curr_acq_sys_mode_ss(mm_id);
}
...
===========================================================================*/
sys_rat_pri_list_info_s_type   sd_misc_get_rat_priority_list( sys_modem_as_id_e_type asubs_id )
{
  sys_rat_pri_list_info_s_type    ss_rat_pri_list_info;

  if(sdss_is_sfmode_1x_sxlte(asubs_id) &&
      sd_stack_enabled_subs(asubs_id,MM_STACK_2))
  {
    sdss_get_rat_pri_list_info( asubs_id, MM_STACK_2, &ss_rat_pri_list_info);
  ...
  sdss_get_rat_pri_list_info( asubs_id, MM_STACK_0, &ss_rat_pri_list_info);
  return ss_rat_pri_list_info;
}
...
===========================================================================*/
sys_rat_pri_list_info_s_type  sd_misc_get_main_rat_priority_list( sys_modem_as_id_e_type asubs_id )
{
  sys_rat_pri_list_info_s_type    ss_rat_pri_list_info;
...
EXTERN  void   sd_misc_update_acq_status(
  ...
  mm_sub_stk_id_s_type        mm_id,
  /* sub and stack on which acq_status to update
  */
...
)
{
  ...
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  sdss_update_acq_status(mm_id, acq_status);
...
EXTERN boolean   sd_misc_mru_update(
  ...
  sd_mru_update_e_type            update_type
...
)
{
  ...
  cmd = (sd_misc_mru_update_cmd *) sdtask_sd_cmd_new(
          sizeof(sd_misc_mru_update_cmd),
          (execute_fn) sdcmd_misc_mru_update,
          NULL,
          NULL,
          asubs_id,
          MM_STACK_0
  ...
  cmd->mru_table = mru_table;
  cmd->index = index;
  cmd->mode = mode;
  cmd->update_type = update_type;
  cmd->status = FALSE;
...
)
{
  ...
  sdprl_read_mru(asubs_id, mru_table,index);
  return;
...
)
{
  ...
  return !sdss_is_hybr_pref_per_sub( mm_id, SD_SS_HYBR_PREF_NONE );
#else
  return FALSE;
#endif
...
)
{
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  SD_ASSERT_ENUM_IS_INRANGE(mm_id.stk_id, MM_STACK_ALL);
  return ( sdss_misc_is_handoff( mm_id, ho_type));
...
===========================================================================*/
EXTERN  boolean                  sd_misc_is_opt_ho_lte_do(sys_modem_as_id_e_type asubs_id)
{
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  return ( sdss_is_opt_ho_lte_do(asubs_id));

} /* sd_misc_is_opt_ho_lte_do() */
...
)
{
  ...
  return sdss_get_ss_sys_lost( mm_id );
}
...
===========================================================================*/
EXTERN  multimode_stack_e_type             sd_misc_get_ss_for_mode(
  sys_modem_as_id_e_type   asubs_id,

  sys_sys_mode_e_type      sys_mode
  /*lint -esym(715, sys_mode) *//* param not referenced*/)
{
  return sdss_get_ss_for_mode( asubs_id,sys_mode );
}
...
EXTERN  boolean     sd_ss_get_avoid_list_updated_flag( sys_modem_as_id_e_type asubs_id)
{
  return ( sdss_get_avoid_list_updated_flag(asubs_id));
}
...
EXTERN  void     sd_ss_set_avoid_list_updated_flag( sys_modem_as_id_e_type asubs_id, boolean is_avoid_list_updated)
{

  sdss_set_avoid_list_updated_flag(asubs_id, is_avoid_list_updated);
}
...
EXTERN  void     sd_ss_set_avoid_plmn_list_flag( sys_modem_as_id_e_type asubs_id, boolean avoid_plmn_flag)
{
  sdss_set_avoid_plmn_list_flag(asubs_id, avoid_plmn_flag);
}
...
DESCRIPTION
  GETS  the avoidance plmn list flag for IMS end cause 381.
...
EXTERN  boolean  sd_ss_get_avoid_plmn_list_flag( sys_modem_as_id_e_type asubs_id)
{
  return(sdss_get_avoid_plmn_list_flag(asubs_id));
}
#endif
/* <EJECT> */
/*===========================================================================
...
===========================================================================*/
EXTERN  void                      sd_misc_get_mode_band_per_sub(
...
{
  sdss_get_mode_band_pref(mm_id, mode_ptr, band_ptr, band_ptr_lte, band_ptr_tds );
}
...
)
{
  ...
  sdss_reset_attach_pending(mm_id);
  ...
  if(si_common_ptr->sd_generic_cb_func != NULL)
  {
    ...
    si_common_ptr->sd_generic_cb_func(&attach_complete_rpt, mm_id);
  }
...
===========================================================================*/
EXTERN  sys_sim_state_e_type          sd_ss_gw_get_sim_state(sys_modem_as_id_e_type asubs_id)
{

  return sdss_get_sim_state_per_subs(asubs_id);
...
===========================================================================*/
void sd_ss_reset( void )
{
  int i,j;
  mm_sub_stk_id_s_type mm_id;

  for(i=0; i<sdss_num_sim_get() && i<MAX_SIMS; i++)
  {
    for(j=0; j<MM_STACK_ALL; j++)
    {
      if(stack_mask[i].current_stack_active_mask & BM(j))
      {
        mm_id.asubs_id = i;
        mm_id.stk_id = j;
        sd_ss_init(sd_ss_ptr_subs(mm_id),mm_id.asubs_id);
      }
    }
  }
}
...
FUNCTION sd_ss_act_get_per_mm_id
...
===========================================================================*/
EXTERN sd_ss_act_e_type          sd_ss_act_get_per_mm_id(
...
)
{
  ...
  sd_ss_s_type *sd_ss_sub_ptr;

  /* If caller supplied a buffer, copy the most recent SS-Action into
  ** into the supplied buffer.
  */
  SD_CRIT_SECT_ENTER();
  sd_ss_sub_ptr = sd_ss_ptr_subs(mm_id);
  SD_CRIT_SECT_EXIT();

  if(sd_ss_sub_ptr)
  {
    act = sd_ss_sub_ptr->true_curr_act.act;
    if( rtrn_act_ptr != NULL )
    {
      *rtrn_act_ptr = sd_ss_sub_ptr->true_curr_act;
    }
  }
...
FUNCTION sd_ss_act_get_per_sub
...
===========================================================================*/
EXTERN sd_ss_act_e_type          sd_ss_act_get_per_sub(
...
)
{
  ...
  */
  ...
  act = sd_ss_ptr_subs(mm_id)->true_curr_act.act;
  if( rtrn_act_ptr != NULL )
  {
    *rtrn_act_ptr = sd_ss_ptr_subs(mm_id)->true_curr_act;
  }
  SD_CRIT_SECT_EXIT();

  SD_MSG_HIGH_3("sub %d stk %d :true curr act %d",mm_id.asubs_id, mm_id.stk_id, act);
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
...
)
{
  ...
  if (sd_si_is_srv_on_subs(asubs_id,MM_STACK_1) &&
      sdss_is_flag_set_subs(asubs_id, MM_STACK_2, SDSS_FLAG_HYBR2_BSR))
  {
    sdss_set_flag_subs(asubs_id, MM_STACK_2,SDSS_FLAG_HDR_DEACT_BY_HYBR2_BSR, TRUE);
  }
...
)
{
  ...
  */
  ...
  SD_CRIT_SECT_EXIT();
  //SD_MSG_HIGH_1("hybr1:true curr act %d",act);
  return act;
...
)
{
  ...
  return sd_ss_hdr_act_get( SYS_MODEM_AS_ID_1, rtrn_act_ptr );
...
{
  sd_si_s_type          *si_ptr     = sd_si_ptr_subs( mm_id.asubs_id, mm_id.stk_id );
  sd_ss_s_type          *ss_ptr     = sd_ss_ptr_subs( mm_id );
  sd_ss_act_s_type      ss_act;
  ...
  */
  ...
  (void)sd_ss_act_get_true_curr_act_ss( mm_id, &ss_act);
  ss_state = sd_get_ss_state_subs(mm_id.asubs_id,mm_id.stk_id);
  ...
  */
  ...
  {
    switch (sdss_eng_stack_get_state( mm_id.stk_id ))
    {
  ...
  */
  ...
  log_ptr->sd_state_flags.is_redir =
    !sdss_is_redir(mm_id.stk_id,SDSS_REDIR_VAL_NONE);
  log_ptr->sd_state_flags.is_ho = !sdss_is_handoff(mm_id.stk_id,SDSS_HO_VAL_NONE);
  ...
  */
  log_util_log_packet_commit( mm_id.asubs_id, log_ptr );
...
)
{
  ...
  */
  ...
  /* Need to be changed to sub and remove this coment*/
  mm_id.asubs_id = asubs_id;
  mm_id.stk_id = MM_STACK_0;
  act = sd_ss_act_get_per_mm_id( rtrn_act_ptr,mm_id );
...
{
  sdss_set_emerg_attached_state(asubs_id, is_emc_attached);
}
/*===========================================================================
...
{
  sdss_set_emerg_bar_status_subs(asubs_id, emerg_bar_status);
}
/*===========================================================================
...
{
  /*shivank: To be updated to connected_mode_subs */
  sdss_set_lte_connected_mode(mm_id, connected_time);
}
...
{
#if defined ( FEATURE_WRLF_SYSTEM_SEL)
  sdss_set_wcdma_connected_mode_subs(connected_time, mm_id.asubs_id,mm_id.stk_id);
#endif
}
...
{
  sdcmd_update_sys_sel_pref_req_id(mm_id, req_id);
...
(
  ...
  mm_sub_stk_id_s_type  mm_id
  /* sub and stack on which indication comes
  */
)
{
...
#error code not present
#endif
EXTERN  sd_ss_band_pref_e_type    sd_misc_get_supp_band_pref(sys_modem_as_id_e_type asubs_id)
{
  sd_ss_band_pref_e_type     band_pref = SD_SS_BAND_PREF_NONE;
  ...
  */
  ...
  if (sdtask_is_init_complete())
  {
    ...
    */
    ...
    if( !sdss_is_hybr_pref_per_sub( mm_id, SD_SS_HYBR_PREF_NONE ))
    {
      mm_id.stk_id = MM_STACK_1;
      sdss_get_mode_band_pref( mm_id,
                               &ss_mode_pref_hybr,
...
===========================================================================*/
EXTERN  boolean    sd_misc_is_gwl_full_srv_allowed(sys_modem_as_id_e_type asubs_id)
{
  mm_sub_stk_id_s_type   mm_id;
  sdss_subsc_e_type      subsc        = sdss_get_subsc_type(asubs_id);
  boolean                sim_domain_valid;
  ...
  sim_domain_valid = sdss_gw_sim_domain_valid (mm_id);

  ss_orig_mode = sdss_get_orig_mode_subs(asubs_id,MM_STACK_0);

  /* If origination mode is emergency (origination or callback),
  ** return false
  */
...
===========================================================================*/
EXTERN  boolean    sd_misc_is_lte_full_srv_allowed(sys_modem_as_id_e_type asubs_id)
{

#ifdef FEATURE_SD_LTE
  sdss_subsc_e_type      subsc        = sdss_get_subsc_type(asubs_id);
  sd_ss_orig_mode_e_type ss_orig_mode = sdss_get_orig_mode_subs(asubs_id,MM_STACK_0);
  sd_ss_orig_mode_e_type ss2_orig_mode;
  sys_sim_state_e_type   ss_sim_state = sdss_get_sim_state_per_subs(asubs_id);
  ...
  if( sd_stack_enabled_subs(asubs_id,MM_STACK_2))
  {
    ss2_orig_mode = sdss_get_orig_mode_subs(asubs_id,MM_STACK_2);
    if(ss2_orig_mode == SD_SS_ORIG_MODE_EMERG_ORIG        ||
  ...
  */
  if(sdss_get_rat_pri_sys_idx(asubs_id, MM_STACK_0,SD_MODE_LTE) ==
      SDSS_DFLT_NUM_RAT)
  {
    /* Main rat pri list does not have LTE if we got here
     */
    if(sdss_is_sfmode_1x_sxlte(asubs_id) &&
        sd_stack_enabled_subs(asubs_id,MM_STACK_2)&&
        (sdss_get_rat_pri_sys_idx(asubs_id, MM_STACK_2,SD_MODE_LTE) !=
         SDSS_DFLT_NUM_RAT)
      )
    {
    ...
    SD_MSG_HIGH_0("LTE not in rat_pri_list of main or hybr2");
    return FALSE;
  }
...
)
{
  ...
  switch (sys_id.id_type)
  {
    case SYS_SYS_ID_TYPE_UMTS      :
      SD_MSG_MED_4("SYS_ID: Lac: 0x%x.  Plmn:Identity [0] 0x%x [1] 0x%x [2] 0x%x",
                   sys_id.id.plmn_lac.lac,
    ...
    case SYS_SYS_ID_TYPE_IS95      :
      SD_MSG_MED_2("SYS_ID: Sid: %d Nid: %d", sys_id.id.is95.sid,
                   sys_id.id.is95.nid);
...
===========================================================================*/
EXTERN  boolean                      sd_misc_is_del_tmsi(sys_modem_as_id_e_type       asubs_id)
{
...
)
{
  ...
  return( sdss_is_orig_mode_subs( mm_id.asubs_id, mm_id.stk_id, orig_mode_val ) );
...
{
  sdss_misc_update_session_info ( asubs_id, app_type);
}
...
EXTERN  void                      sd_init_before_app_task_start( void )
{
  ...
  sd_si_s_type   *sd_si_common_ptr  = sd_si_common();
  mm_sub_stk_id_s_type  mm_id;
  int i;
  ...
  global_sd_mm_id.asubs_id = SYS_MODEM_AS_ID_1;
  global_sd_mm_id.stk_id = MM_STACK_0;
...
#error code not present
  ...
  /* Initialize the system selection core. SDSS alloc happens here.
   */
  sdss_component_init();
  ...
  */
  ...
  /* This needs to be called after sdss_component_init() where num_sims is
    ** initialized */
   ...
   */
  sd_si_common_ptr->si_info_cb_func              = NULL;
  sd_si_common_ptr->acq_fail_cb_func             = NULL;
  sd_si_common_ptr->ok_to_orig_cb_func           = NULL;
  sd_si_common_ptr->emerg_cb_mode_cb_func        = NULL;
  sd_si_common_ptr->autonam_change_cb_func       = NULL;
  sd_si_common_ptr->gw_net_list_cb_func          = NULL;
  sd_si_common_ptr->sim_state_cb_func            = NULL;
  sd_si_common_ptr->reg_done_cb_func             = NULL;
  sd_si_common_ptr->data_suspend_cb_func         = NULL;
  sd_si_common_ptr->si_prl_load_complete_cb_func = NULL;
  sd_si_common_ptr->ss_stack_status_cb_func      = NULL;
  sd_si_common_ptr->cdma_bsr_stat_chgd_cb_func   = NULL;
  sd_si_common_ptr->cs_emerg_scan_fail_cb_func   = NULL;
  sd_si_common_ptr->emerg_srch_fail_cb_func      = NULL;


  for(i=0; i<sdss_common_ptr()->num_sims && i < MAX_SIMS ; i++)
  {
    /* Allocate stack 0 for sub1 */
    mm_id.asubs_id = i;
    mm_id.stk_id = MM_STACK_0;

    /* Set global ids for use within internal helper function */
    global_sd_mm_id = mm_id;
    global_sd_asubs_id = i;

    stack_mask[i].current_stack_active_mask = 1;
    stack_mask[i].old_stack_active_mask = 0;
    ...
    */
    ...
    sdss_lbsr_mcc_items_init((sys_modem_as_id_e_type)i);


    sdss_emerg_call_full_srv_mcc_items_init( (sys_modem_as_id_e_type)i );

    sd_ss_alloc(mm_id);
    sd_ss_set_global_ptr(); /* Set global ptr to current asubs_id */

    sdss_alloc_sdss_stacks(MM_STACK_0_MASK, i);

    sd_ss_init( sd_ss_ptr_subs(mm_id),mm_id.asubs_id);

    /* Initialize the main service indicators component.
    */
    sd_si_init( mm_id, sd_ss_ptr(MM_STACK_0), SDSR_SI_SYS );

    /* Allocate sdprl buffer */
    sdprl_alloc((sys_modem_as_id_e_type)i);
    prl_data_alloc((sys_modem_as_id_e_type)i);
    sdsr_lists_allocate(MM_STACK_0_MASK, i);
    sdss_1xcsfb_call_end_opt_init((sys_modem_as_id_e_type)i);
    sdss_1xcsfb_ecbm_init((sys_modem_as_id_e_type)i);
#ifdef FEATURE_MMODE_SC_SVLTE
    /* Initialize LTEAvailFile/LTENotAvailFile */
    sdss_lte_avail_file_init((sys_modem_as_id_e_type)i);
#endif
  ...
  sdss_special_scan_mcc_items_init();
...
EXTERN  boolean                   sd_init2(
  ...
  */
  ...
  void                      (*timer_func_ptr) (int4  ms, mm_sub_stk_id_s_type mm_id)
  ...
  ** SS-Timer
  ** Common function for all timers to be started in MMOC */
)
/*lint -esym(715,hybr_2_timer_func_ptr) */
{
  ...
  */
  ...
  for(i=0; i<sdss_num_sim_get() && i < MAX_SIMS; i++)
  {
    /* Set global_asubs_id to given sub id */
    global_sd_asubs_id = i;
    global_sd_mm_id.asubs_id = i;
    global_sd_mm_id.stk_id = MM_STACK_0;
    ...
    */
    ...
    sd_ss_alloc(mm_id);
    sd_ss_set_global_ptr(); /* Set global to current asubs  id */

    sd_ss_init( sd_ss_ptr_subs(mm_id),mm_id.asubs_id);

    /* Initialize the main service indicators component.
       */
    sd_si_init( global_sd_mm_id, sd_ss_ptr(MM_STACK_0), SDSR_SI_SYS );

    /* Allocate sdprl buffer */
    sdprl_alloc((sys_modem_as_id_e_type)i);
    prl_data_alloc((sys_modem_as_id_e_type)i);

    /* Note: Other stacks of sd_ss will be initialized during NAM sel */
    ...
    */
    ...
    sdss_set_sv_dynamic_switch_enabled(FALSE,i);
#endif
  }
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

  sd_ss_common_ptr()->timer_func_ptr = timer_func_ptr;
...
FUNCTION sd_is_manual_bst_valid
...
===========================================================================*/
extern boolean sd_is_manual_bst_valid(sys_modem_as_id_e_type asubs_id)
{
  return  sdss_is_manual_bst_valid(asubs_id);
}

/*===========================================================================

FUNCTION sd_is_data_suspend_ss

DESCRIPTION
  Determine if Data has been suspended on the stack.

DEPENDENCIES
  None.

RETURN VALUE
  Return TRUE if Data has been suspended on the stack.
...
===========================================================================*/
extern boolean sd_is_data_suspend_ss(

  mm_sub_stk_id_s_type               mm_id
  /* the stack to get the data suspend status from
    */)
{
  if(mm_id.asubs_id >= MAX_SIMS || mm_id.asubs_id < SYS_MODEM_AS_ID_1)
  {
    mm_id.asubs_id = SYS_MODEM_AS_ID_1;
  }

  if((stack_mask[mm_id.asubs_id].current_stack_active_mask & BM(mm_id.stk_id)) == 0)
  {
    mm_id.stk_id = MM_STACK_0;
  }
  
  return  sd_si_ptr_subs(mm_id.asubs_id, mm_id.stk_id)->data_suspend;
}
...
boolean  sd_misc_is_true_power_save(
  mm_sub_stk_id_s_type  mm_id
)
{

  boolean ret = FALSE;
  sdss_state_e_type ss_state = sd_get_ss_state_subs(mm_id.asubs_id,mm_id.stk_id);
  sd_ss_act_s_type        ss_act;

  //Get last true action
  (void)sd_ss_act_get_curr_act_ss(mm_id, &ss_act);

  SD_MSG_HIGH_4("ss_state:%d act:%d pwr_save_reas:%d ss:%d", ss_state,
                ss_act.act, ss_act.prm.pwr_save.reas, mm_id.stk_id);
...
@RETURN VALUE
 ...
 MM_STACK_ALL if invalid

@SIDE EFFECTS
  None.

===========================================================================*/
extern  multimode_stack_e_type             sd_misc_get_ss_for_user_mode_pref(
  sys_modem_as_id_e_type   asubs_id,

  sys_sys_mode_e_type      sys_mode
)
{
  return sdss_get_ss_for_user_mode_pref( asubs_id, sys_mode );
}
/*===========================================================================
...
{
  sdss_set_is_special_volte_e911_calls(asubs_id, special_volte_e911_calls);
}
...
)
{
  ...
  sdss_set_call_mode(as_id, call_mode);
}
...
{
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  SD_ASSERT_ENUM_IS_INRANGE(asubs_id, SYS_MODEM_AS_ID_NO_CHANGE);
  return sdss_is_mmss_operation_per_sub(asubs_id);
...
{
  mm_sub_stk_id_s_type   mm_id;
  multimode_stack_e_type ss = sdss_get_ss_for_mode(asubs_id, SD_MODE_HDR);
  ...
  if(sdss_get_opr_mode_per_sub( asubs_id, ss ) != SDSS_OPR_MODE_ONLINE)
  {
    return FALSE;
  }

  /* Check if HDR is on MAIN or HYBR1*/
  if (ss != MM_STACK_0 && ss != SD_SS_HYBR_HDR)
  {
    return FALSE;
  }

  /* Ensure HDR is in opr state . IF not , return FALSE*/
  if (!( sdss_get_ss_state_subs(mm_id.asubs_id,mm_id.stk_id) == SDSS_STATE_OPR &&
         sd_si_ptr_subs(mm_id.asubs_id,mm_id.stk_id)->si_info.srv_status == SYS_SRV_STATUS_SRV &&
         sd_si_ptr_subs(mm_id.asubs_id,mm_id.stk_id)->si_info.mode  == SD_MODE_HDR
       )
     )
  {
    SD_MSG_HIGH_3("Invalid state %d %d %d", ss, sdss_get_ss_state_subs(mm_id.asubs_id,mm_id.stk_id),sd_si_ptr_subs(mm_id.asubs_id,mm_id.stk_id)->si_info.srv_status );
    return FALSE;
  }
  ...
  {
    uint8  i = 0;
    sys_plmn_list_s_type *plmn_lst = sdss_get_more_pref_plmn_list(mm_id);
...
FUNCTION sd_ss_ind_ims_emerg_call_state

DESCRIPTION
Update SD about current volte e911 call state
...
===========================================================================*/
extern void sd_ss_ind_ims_emerg_call_state
(
  mm_sub_stk_id_s_type          mm_id,
...
)
{
  ...
  sdss_set_ims_emerg_call_state(mm_id, call_state);
...
{
  sdss_set_flag_subs(mm_id.asubs_id, mm_id.stk_id, SDSS_FLAG_SPECIAL_VOLTE_REDIAL, is_special_volte_pref);
}
...
DESCRIPTION
Update SD about current volte call status. This API is used for both internal and 3rd party. And it is only for
normal call. Don't use it for emergency call.
...
)
{
  ...
  sdss_set_volte_call_state(mm_id, call_state);

}

/*===========================================================================

FUNCTION sd_ss_set_suppress_no_srv_due_to_suspend

DESCRIPTION
Update SD about whether to suppress sending no srv or power save to CM
This is set when the other sub is suspended due to emerg call and reset when the sub is resumed
...
===========================================================================*/
extern void sd_ss_set_suppress_no_srv_due_to_suspend
(
  sys_modem_as_id_e_type asubs_id,
...
{
  sdss_set_suppress_no_srv_due_to_suspend(asubs_id, is_suppress);
}

/*===========================================================================
=============================================================================
=============================================================================
============================ Full Async Access APIs ========================
=============================================================================
=============================================================================
===========================================================================*/
...
EXTERN  boolean                          sd_nam_sel4(
  ...
  void                             (*callbk_ptr)(void *),
  /* Async call return callback ptr */

  void *context,
  /* Async call context ptr */
  ...
  */
  ...
  /**< Indicates if camp_mode is ON or OFF */
  sd_ss_fullrat_pref_e_type         fullrat_status,
...
)
{
  ...
  */
  ...
  cmd->subs_capability = subs_capability;
  cmd->stack_active = stack_active;
  cmd->voice_domain_pref = voice_domain_pref;
  cmd->camp_mode = camp_mode;
  cmd->fullrat_status = fullrat_status;
  cmd->rat_capability = rat_capability;
  ...
  /* Asynchronous call. On completion, callbk_ptr will send MMOC RPT */
  sdtask_queue_put_cmd_no_wait((sd_cmd_t *) cmd, callbk_ptr, context);
  ref_cnt_obj_release(cmd);
...
FUNCTION sd_process_sensor_state_change

DESCRIPTION
  determines the SS where the change needs to be processed on the given sub.
  OOS or BSR timer could be running on either stack 0 or stack 2 of the given sub.
  SD core will make the decision on whether any timer values need to be updated if
  it is running and adjust the values accordingly
...
EXTERN  void          sd_process_sensor_state_change(
...
)
{
  ...
  sd_cmd_t                       *cmd    = NULL;
  multimode_stack_e_type          stk_id = MM_STACK_0;
  sd_si_info_s_type               *si_info_ptr = &sd_si_ptr_subs(asubs_id, MM_STACK_0)->si_info;

  /* possible scenarios for sub being registered for sensor updates :
     1. OOS on MAIN in 3GPP only mode ( single stk mode : stk_id = 0)
     2. OOS on HYBR2 In SXLTE mode (dual stk : stk_id = 2)
     3. Lmtd to full BSR on MAIN . No timer on HYBR2 ( in SXLTE, or single stk mode : stk_id = 0)
     4. Lmtd to full BSR on HYBR2 . ( in SXLTE, or single stk mode : stk_id = 2)                                                                       )
     */
  /* This boils down to -
     1. Check if there is 3GPP service on MAIN stack. If there is , the stack is 0.(regardless of SRLTE)
     2. If LTE is in the priority list of MAIN stack ( SRLTE is not possible) and the stack is 0.
     3. Otherwise, if SRLTE is enabled, and LTE is part of capability on stack 2,  the stack is 2.
     4. If not , stack is 0. ( initialized to 0)
  */
  if (sys_srv_status_is_srv(si_info_ptr->srv_status) &&
      sd_misc_is_mode_3gpp(si_info_ptr->mode))
  {
    stk_id = MM_STACK_0;
  }
  else if(sdss_get_rat_pri_sys_idx(asubs_id, MM_STACK_0,SD_MODE_LTE) !=
          SDSS_DFLT_NUM_RAT)
  {
    stk_id = MM_STACK_0;
  }
  else if( sdss_is_sfmode_1x_sxlte(asubs_id) &&
           sd_stack_enabled_subs(asubs_id, MM_STACK_2)&&
           (sdss_get_rat_pri_sys_idx(asubs_id, MM_STACK_2,SD_MODE_LTE) !=
            SDSS_DFLT_NUM_RAT) )
  {
  ...
  cmd = (sd_cmd_t *) sdtask_sd_cmd_new(
          sizeof(sd_cmd_t),
          (execute_fn) sdcmd_process_sensor_chg,
          NULL,
          NULL,
          asubs_id,
          stk_id
...
FUNCTION sd_ss_current_mcc_match_mcc_list_emerg_call_full_srv
...
===========================================================================*/
EXTERN boolean sd_ss_current_mcc_match_mcc_list_emerg_call_full_srv (
...
)
{
  ...
  /*  Get current camped mcc fromt the current PLMN id
   */
  sys_plmn_get_mcc(*camped_plmn,&current_mcc);
  
     
  mcc_match_found = sdss_current_mcc_match_with_efs(current_mcc,asubs_id);
...
FUNCTION sd_ss_is_emergency_psap_cb_mandatory

DESCRIPTION
  Checks if PSAP Callback is mandatory while originating EM call.
...
===========================================================================*/
EXTERN boolean sd_ss_is_emergency_psap_cb_mandatory (
  mm_sub_stk_id_s_type               mm_id
)
{
    boolean        mcc_match_found   = FALSE;
    ...
    mcc_match_found = sdss_is_emergency_psap_cb_mandatory(mm_id);
...
)
{
  ...
  sdss_set_sv_dynamic_switch_enabled(is_enabled,asubs_id);
}
...
#ifdef FEATURE_QSH_MDUMP
/*===========================================================================
FUNCTION sd_qsh_mdump_collect
  
DESCRIPTION
  This function collects the mdump for sd global variable
...
QSH_MDUMP_FN_ATTR
#endif
void sd_qsh_mdump_collect()
{
  qsh_mdump_collect_high(sd_crit_sect_ptr(),sizeof(rex_crit_sect_type));
}
#endif

/*===========================================================================
FUNCTION sd_misc_is_sub_multimode_capable
...
===========================================================================*/
boolean sd_misc_is_sub_multimode_capable(sys_modem_as_id_e_type asubs_id)
{
  return sdss_is_sub_multimode_capable(asubs_id);
}

/*===========================================================================

FUNCTION sd_ss_set_lte_deact_dual_switch_flag


DESCRIPTION
  sets the lte deact dual switch flag
...
EXTERN  void     sd_ss_set_lte_deact_dual_switch_flag( mm_sub_stk_id_s_type mm_id, boolean value)
{
  sdss_set_is_lte_deact_dual_switch(mm_id,value);
}
/*===========================================================================

FUNCTION sd_ss_ind_user_ss_pref9
...
===========================================================================*/
EXTERN  sd_ss_act_e_type                    sd_ss_ind_user_ss_pref9(
...
)
{
  ...
  cmd->rat_capability = rat_capability;
  cmd->addl_action = addl_action;
  cmd->pending_emerg_state = pending_emerg_state;
...
FUNCTION sd_ss_ind_misc_active_subs_update

DESCRIPTION
  Informs SD of active_subs change.
...
===========================================================================*/
EXTERN sd_ss_act_e_type          sd_ss_ind_misc_active_subs_update(

  uint8 active_subs,
  /* New value of active_subs
  */
...
)
{
  ...
  cmd = (sd_ss_ind_misc_active_subs_update_cmd *) sdtask_sd_cmd_new(
          sizeof(sd_ss_ind_misc_active_subs_update_cmd),
          (execute_fn) sdcmd_misc_active_subs_update,
          rtrn_act_ptr,
          NULL,
          SYS_MODEM_AS_ID_1, MM_STACK_0
        );

  cmd->active_subs = active_subs;
  act = sdtask_queue_put_cmd_wait((sd_cmd_t *) cmd);
  ref_cnt_obj_release(cmd);
...
FUNCTION sd_ss_is_opr_state

DESCRIPTION
 check and return if SD is in opr state or not
...
===========================================================================*/
extern boolean sd_ss_is_opr_state
(
    mm_sub_stk_id_s_type          mm_id
)
{
  ...
  return (sdss_get_ss_state_subs(mm_id.asubs_id,mm_id.stk_id) == SDSS_STATE_OPR); 
...
FUNCTION sd_misc_is_lte_band_avoided_per_sub

  Inform the caller to whether a specified LTE band is avoided

DEPENDENCIES
  sd_init() must have already been called to initialize SD.

RETURN VALUE
  TRUE if the specified band is avoided. FALSE otherwise.

SIDE EFFECTS
  None.

===========================================================================*/
extern  boolean                   sd_misc_is_lte_band_avoided_per_sub(

  sys_modem_as_id_e_type asubs_id,
  /* System selection sub
   */
  sys_sband_lte_e_type  lte_band
  /* LTE band to be validated.
  */  
)
{
  mm_sub_stk_id_s_type     mm_id;
  sdss_band_s_type emerg_avoid_band;

  if( asubs_id >= MAX_SIMS ||
      asubs_id < SYS_MODEM_AS_ID_1||
  ...
  memset(&emerg_avoid_band, 0, sizeof(sdss_band_s_type));
  ...
  emerg_avoid_band=sdss_get_avoid_emerg_bands_per_sub(mm_id);
  
  /* If the specified band is NOT avoided, return FALSE.
  */
  if( sdss_is_pref_reason_per_sub( mm_id, SDSSSCR_PREF_REAS_VAL_CALL_ORIG) &&
      sdss_is_orig_mode_subs( mm_id.asubs_id, mm_id.stk_id, SD_SS_ORIG_MODE_VAL_EMERG_ORIG ) &&
      SYS_LTE_BAND_MASK_IS_BAND_PRESENT( &emerg_avoid_band.lte_band, lte_band) 
  	)
  {
    return TRUE;
  }
...
} /* sd_is_lte_band_avoided_per_sub */
#endif
