...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/mmode/sd/src/sdnv.c#1 $
...
static nv_cmd_type        sdnv_cmd_sync;
...
static nv_cmd_ext_type    sdnv_cmd_extn_sync;
/* NV command buffer to do NV synchronous NV write and read */
#endif

static sdnv_data_s_type      sdnv_data_buf;
/* Static SD NV data buffer that is big enough to hold data
** that is associated with SD NV items.
*/
...
)
{
  ...
  */
  ...
  subsc = sdss_get_subsc_type( (sys_modem_as_id_e_type)sdnv_cmd_extn_sync.context );
#else
  subsc = sdss_get_subsc_type( (sys_modem_as_id_e_type)SYS_MODEM_AS_ID_1 );;
#endif
...
FUNCTION sdnv_write_extn

DESCRIPTION
  Write a SD related item to NV memory per subscription
...
===========================================================================*/
EXTERN  boolean                      sdnv_write_extn(
...
#ifdef FEATURE_MMODE_DUAL_SIM  /* set the extended nv command with context  */
  sdnv_cmd_extn_sync.context = context;
  sdnv_cmd_extn_sync.nvcmd   = &sdnv_cmd;
#endif
...
{
  return sdnv_write_extn(nv_item, data_ptr, 0 );
}

/* <EJECT> */
/*===========================================================================

FUNCTION sdnv_read_extn
...
)
{
  ...
  */
  memset(&sdnv_cmd_sync, 0, sizeof(nv_cmd_type));
  sdnv_cmd_sync.item       = nv_item;          /* item to read */
  sdnv_cmd_sync.cmd        = NV_READ_F;        /* NV operation */
  sdnv_cmd_sync.data_ptr   = data_ptr;         /* buffer to store read data  */
  sdnv_cmd_sync.tcb_ptr    = rex_self();       /* Notify this task when done */
  sdnv_cmd_sync.sigs       = sdnv_ptr->nv_sig; /* Signal with this sig when done */
  sdnv_cmd_sync.done_q_ptr = NULL;             /* Return cmd to NO Q when done */

#ifdef FEATURE_MMODE_DUAL_SIM  /* set the extended nv command with context  */
  sdnv_cmd_extn_sync.context = context;
  sdnv_cmd_extn_sync.nvcmd   = &sdnv_cmd_sync;
#endif
  ...
  */
  (void) rex_clr_sigs( rex_self(), sdnv_ptr->nv_sig );
  sdnv_nv_cmd(&sdnv_cmd_sync);

  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

  /* If NV the read operation is successful, return TRUE.
  */
  if( sdnv_cmd_sync.status == NV_DONE_S )
  {
    return TRUE;
  }
  ...
  */
  switch( sdnv_cmd_sync.item )
  {
    ...
    default:
      SD_ERR_1( "Unexpected NV item=%d",sdnv_cmd_sync.item);
      return FALSE;
  ...
  */
  if( sdnv_cmd_sync.status == NV_NOTACTIVE_S ||
      sdnv_cmd_sync.status == NV_FAIL_S )
  {
    SD_MSG_HIGH_1( "Write default value to NV, item=%d",sdnv_cmd_sync.item);
    (void)sdnv_write( sdnv_cmd_sync.item, sdnv_cmd_sync.data_ptr );
  }
...
)
{
  ...
  return sdnv_read_extn(nv_item, data_ptr, 0);

}

/*===========================================================================

FUNCTION sdnv_get_item_ptr

DESCRIPTION
  Returns a pointer to the static global variable sdnv_item_ptr

DEPENDENCIES
  None

RETURN VALUE
  static nv_item_type *sdnv_item_ptr

SIDE EFFECTS
  None

===========================================================================*/
EXTERN nv_item_type *sdnv_get_item_ptr()
{
  return (nv_item_type *)&sdnv_data_buf;
}
