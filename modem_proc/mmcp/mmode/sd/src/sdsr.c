...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/mmode/sd/src/sdsr.c#1 $
...
uint8 sdsr_items_arr_size[MAX_SIMS] = {0};
uint8 sdsr_lists_size[MAX_SIMS] = {0};
...
static sdsr_tbl_s_type   *sdsr_tbl[MAX_SIMS];

/* Global variable for all 3GPP2 systems gwl_band_ptr entries
to point to */
static sd_3GPP_band_s_type   gwl_band_for_3gpp2_entries;

/*===========================================================================

FUNCTION sdsr_get_last_acq_sys_ptr

DESCRIPTION
  Provides a pointer to a static sys rec ptr which stores the last acquired system

DEPENDENCIES
  None.

RETURN VALUE
  Pointer to last acq sys

SIDE EFFECTS
  None.

===========================================================================*/
static sdsr_s_type   *sdsr_get_last_acq_sys_ptr (

  multimode_stack_e_type     ss
)
{

  static sdsr_s_type           last_acq_sys;
  static sdsr_s_type           last_acq_sys_;
  static sd_3GPP_band_s_type   last_acq_sys_band;

  switch (ss)
  {
    case MM_STACK_0:
      last_acq_sys.sys.gwl_band_ptr = &last_acq_sys_band;
      return &last_acq_sys;
    case MM_STACK_1:
      return &last_acq_sys_;
    default:
      return NULL;
  }
}
...
FUNCTION sdsr_create_sr_rec_copy

DESCRIPTION
  Creates a system record and initializes it using an input record
...
===========================================================================*/
EXTERN sdsr_s_type      *sdsr_create_sr_rec_copy(
...
{
  sdsr_s_type *copy_ptr = NULL;

  if(sr_ptr == NULL)
    return NULL;

  /*allocate memory for the system record
  */
  copy_ptr = (sdsr_s_type *)modem_mem_alloc(sizeof(sdsr_s_type), MODEM_MEM_CLIENT_MMODE);

  if(copy_ptr == NULL)
  {
    return NULL;
  }
  memset(copy_ptr,0,sizeof(sdsr_s_type));
  ...
  *copy_ptr = *sr_ptr;
  ...
  {
    copy_ptr->sys.gwl_band_ptr = (sd_3GPP_band_s_type *)modem_mem_alloc(\
                                 sizeof(sd_3GPP_band_s_type), MODEM_MEM_CLIENT_MMODE);
    if( copy_ptr->sys.gwl_band_ptr == NULL)
    {
      sys_err_fatal_null_ptr_exception();
    }
    memset(copy_ptr->sys.gwl_band_ptr,0,sizeof(sd_3GPP_band_s_type));
    *(copy_ptr->sys.gwl_band_ptr) = *(sr_ptr->sys.gwl_band_ptr);
  }
  else
  {
#ifndef SD_DEBUG
    copy_ptr->sys.gwl_band_ptr = &gwl_band_for_3gpp2_entries;
#else
    #error code not present
#endif
  }
  /* update debug info indicating create api has been invoked */

  if( global_sd_asubs_id < MAX_SIMS &&
      global_sd_asubs_id >= SYS_MODEM_AS_ID_1 &&
      sdsr_tbl[global_sd_asubs_id])
  {
    sdsr_tbl[global_sd_asubs_id]->num_create++;
  }
...
FUNCTION sdsr_delete_sr_rec_copy

DESCRIPTION
  deletes memory associated with a system record
...
===========================================================================*/
EXTERN void     sdsr_delete_sr_rec_copy(

  sdsr_s_type               *sr_ptr
  /* Pointer to a system record which needs to be deleted
  */
...
)
{
  ...
  if( global_sd_asubs_id >= MAX_SIMS ||
      global_sd_asubs_id < SYS_MODEM_AS_ID_1 ||
      sdsr_tbl[global_sd_asubs_id] == NULL )
  {
    return;
  }

  if (sr_ptr->sys.mode == SD_MODE_GWL && sr_ptr->sys.gwl_band_ptr)
  {
    modem_mem_free((void *)sr_ptr->sys.gwl_band_ptr, MODEM_MEM_CLIENT_MMODE);
  }
  if (sr_ptr)
  {
    modem_mem_free((void *)sr_ptr, MODEM_MEM_CLIENT_MMODE);
  }
  /* update debug info indicating destroy api has been invoked . It should equal the num_create */
  sdsr_tbl[global_sd_asubs_id]->num_destroy++;
}
/*===========================================================================
...
)
{
  ...
  */
  sr_ptr->sys.mode  = sys_ptr->mode;
  sr_ptr->sys.band  = sys_ptr->band;
  sr_ptr->sys.chan  = sys_ptr->chan;

  /* copy  LTE , TDS band info if the record is of type 3GPP
  */
  if (sr_ptr->sys.mode == SD_MODE_GWL)
  {
    *(sr_ptr->sys.gwl_band_ptr) = *(sys_ptr->gwl_band_ptr);
  }
  else
  {
#ifndef SD_DEBUG
    sr_ptr->sys.gwl_band_ptr = &gwl_band_for_3gpp2_entries;
#else
    #error code not present
#endif
  }
  ...
  */
  sr_ptr->last_acq_ss       = MM_STACK_ALL;
...
)
{
  ...
  {
    case SD_MODE_GWL:
      SD_MSG_HIGH_6_EXT("SDSR: %d, Sys mode=GWL, 2G_3G_band=0x%08x%08x, chan=%-4d tds_band=0x%08x%08x",
                        sr_ref,
                        QWORD_HIGH(sr_ptr->sys.band),
                        QWORD_LOW(sr_ptr->sys.band),
                        sr_ptr->sys.chan,
                        QWORD_HIGH(sr_ptr->sys.gwl_band_ptr->tds_band),
                        QWORD_LOW(sr_ptr->sys.gwl_band_ptr->tds_band),
                        (global_sd_asubs_id+1));
      sd_print_lte_band_mask( sr_ptr->sys.gwl_band_ptr->lte_band);
    ...
    case SD_MODE_MAX:
    default:
      SD_MSG_HIGH_5_EXT("SDSR:%d  Sys mode=%d,     band=0x%08x%08x   chan=%d",
  ...
  switch (sr_ptr->sys.mode)
  {
    case SD_MODE_HDR:
      ...
      SD_MSG_HIGH_4( "prl=%d, roam=%d, sched=%d, idx_exp=%d",
                     sr_ptr->prl, sr_ptr->roam, sr_ptr->sched, sr_ptr->idx_expected );
      break;
    ...
    case SD_MODE_MAX:
    default:
      SD_MSG_HIGH_5( "SID=%d, prl=%d, roam=%d, Expected SID=%d, sched=%d",
  ...
  */
  if( sr_a_ptr->sys.mode != sr_b_ptr->sys.mode ||
      ...
      ( sr_a_ptr->sys.mode == SD_MODE_GWL &&
        !sdsr_chk_lte_band_equal(sr_a_ptr->sys.gwl_band_ptr->lte_band, sr_b_ptr->sys.gwl_band_ptr->lte_band) )
    )
  {
...
)
{
  ...
  */
  if( sr_a_ptr->sys.mode != sr_b_ptr->sys.mode ||
      ...
      ( sr_a_ptr->sys.mode == SD_MODE_GWL &&
        (!sdsr_chk_lte_band_equal(sr_a_ptr->sys.gwl_band_ptr->lte_band, sr_b_ptr->sys.gwl_band_ptr->lte_band)  ||
         sr_a_ptr->sys.gwl_band_ptr->tds_band != sr_b_ptr->sys.gwl_band_ptr->tds_band ) )
    )
  {
  ...
  */
  switch( equal_cat )
  {
    ...
    case SDSR_EQUAL_BAND:
      ...
      {
        if (sr1_ptr->sys.mode == SD_MODE_GWL && sr2_ptr->sys.mode == SD_MODE_GWL)
        {
          if (sdsr_chk_lte_band_equal(sr1_ptr->sys.gwl_band_ptr->lte_band,sr2_ptr->sys.gwl_band_ptr->lte_band))
          {
            return TRUE;
          }
        }
...
EXTERN  int                   sdsr_tbl_size( void )
{
  ...
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  return( sdsr_tbl[global_sd_asubs_id]->size );
...
EXTERN  int                   sdsr_tbl_num_rec( void )
{
  ...
  return( sdsr_tbl[global_sd_asubs_id]->num );
...
)
{
  ...
  if( sdsr_tbl[global_sd_asubs_id]->num > sdsr_tbl[global_sd_asubs_id]->size)
  {
    return NULL;
  }

  if( sr_ref < sdsr_tbl[global_sd_asubs_id]->num )
  {
    return  sdsr_tbl[global_sd_asubs_id]->arr[ sr_ref ];
  }
...
static  void                  sdsr_tbl_init( void )
{
  ...
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if(sdsr_tbl[global_sd_asubs_id] == NULL)
  {
    SD_MSG_HIGH_1("Allocate SDSR TBL asubs_id %d",global_sd_asubs_id);
    if((sdsr_tbl[global_sd_asubs_id] = modem_mem_alloc(sizeof(sdsr_tbl_s_type),MODEM_MEM_CLIENT_MMODE)) == NULL )
    {
      SD_ERR_FATAL_0("Mem alloc failed");
    }
    memset((void *)sdsr_tbl[global_sd_asubs_id],0,sizeof(sdsr_tbl_s_type));
  }
  sdsr_tbl[global_sd_asubs_id]->num = 0;

  for(i=0; i<PRL_TBL_MAX ; i++)
  {
    if(sdsr_tbl[global_sd_asubs_id]->arr[i] !=NULL)
    {
      if (sdsr_tbl[global_sd_asubs_id]->arr[i]->sys.mode == SD_MODE_GWL)
      {
        SD_ASSERT(sdsr_tbl[global_sd_asubs_id]->arr[i]->sys.gwl_band_ptr);
        modem_mem_free((void *)sdsr_tbl[global_sd_asubs_id]->arr[i]->sys.gwl_band_ptr, MODEM_MEM_CLIENT_MMODE);
        sdsr_tbl[global_sd_asubs_id]->num_dealloc++; //debug info
      }
      SD_MEM_FREE(sdsr_tbl[global_sd_asubs_id]->arr[i], MODEM_MEM_CLIENT_MMODE);
    }

    sdsr_tbl[global_sd_asubs_id]->arr[i]  = NULL;
  }

  sdsr_tbl[global_sd_asubs_id]->size = PRL_TBL_MAX;
  ...
  #error code not present
#endif //sd_debug
  memset(&gwl_band_for_3gpp2_entries, 0, sizeof(sd_3GPP_band_s_type));
...
static void sdsr_map_entries_reset_acq_info( void )
{
  ...
  for( entry = 0; entry < sdsr_lists_size[global_sd_asubs_id]; entry++ )
  {
  ...
  for( entry = SDSR_1ST_SYS; entry < sdsr_items_arr_size[global_sd_asubs_id] + SDSR_1ST_SYS; entry++ )
  {
    if(!sdsr_is_entry_updated_on_nam_sel(entry))
    {
      sdsr_list_clr((sdsr_e_type) entry );
...
void sdsr_reset_acq_info ( void )
{
  ...
  sdss_set_avoid_list_updated_flag_all_sub(TRUE);

  sdsr_map_entries_reset_acq_info();

  SD_MSG_HIGH_2("SDSR_RESET: sub %d curr_stack_mask %d",
...
void sdsr_check_dealloc_mem_for_sdsr_tbl(void)
{
  ...
  sdsr_tbl_ptr = sdsr_tbl[global_sd_asubs_id];

  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  for (ix=0; ix<PRL_TBL_MAX; ix++)
  {
    if(sdsr_tbl_ptr->arr[ix]!=NULL)
    {
      if (sdsr_tbl_ptr->arr[ix]->sys.mode == SD_MODE_GWL)
      {
        SD_ASSERT(sdsr_tbl_ptr->arr[ix]->sys.gwl_band_ptr);
        modem_mem_free((void *)sdsr_tbl_ptr->arr[ix]->sys.gwl_band_ptr, MODEM_MEM_CLIENT_MMODE);
        sdsr_tbl_ptr->num_dealloc++; //debug info
      }
      SD_MEM_FREE(sdsr_tbl_ptr->arr[ix], MODEM_MEM_CLIENT_MMODE);
      sdsr_tbl_ptr->arr[ix]=NULL;
    }
  }
  sdsr_tbl_ptr->num=0;
...
/*lint -esym(715,num_acq_sys) */
{
  ...
  {
    sdsr_tbl[global_sd_asubs_id]->num  = 0;
    sdsr_tbl[global_sd_asubs_id]->size = num_acq_sys;
  }
...
)
{
  ...
  */
      ...
      sdsr_tbl[global_sd_asubs_id])
  {
    ...
    ** system record table.
    */
    if( sdsr_tbl[global_sd_asubs_id]->num < (int) sdsr_tbl[global_sd_asubs_id]->size  )
    {

      /* allocate memory  for element */
      sdsr_tbl[global_sd_asubs_id]->arr[sdsr_tbl[global_sd_asubs_id]->num]=(sdsr_s_type *)modem_mem_alloc(\
          sizeof(sdsr_s_type), MODEM_MEM_CLIENT_MMODE);

      /* clear rat pri ptr before updating from acq record
      */
      memset(sdsr_tbl[global_sd_asubs_id]->arr[sdsr_tbl[global_sd_asubs_id]->num],0,sizeof(sdsr_s_type));
      tbl_ptr = sdsr_tbl[global_sd_asubs_id]->arr[ sdsr_tbl[global_sd_asubs_id]->num ];

      /*allocate memory for LTE , TDS bands
      */
      if (sr_ptr->sys.mode == SD_MODE_GWL)
      {
        tbl_ptr->sys.gwl_band_ptr = (sd_3GPP_band_s_type *)modem_mem_alloc(\
                                    sizeof(sd_3GPP_band_s_type), MODEM_MEM_CLIENT_MMODE);
        if( tbl_ptr->sys.gwl_band_ptr == NULL )
        {
          sys_err_fatal_null_ptr_exception();
        }
        memset(tbl_ptr->sys.gwl_band_ptr,0,sizeof(sd_3GPP_band_s_type));
        sdsr_tbl[global_sd_asubs_id]->num_alloc++;
      }
      else
      {
#ifndef SD_DEBUG
        tbl_ptr->sys.gwl_band_ptr = &gwl_band_for_3gpp2_entries;
#else
        #error code not present
#endif
      }
      ...
      sdsr_tbl[global_sd_asubs_id]->num++;
      ...
      */
      sdsr_list_insert_ref( SDSR_TBL_LST,
                            SDSR_POS_NULL,
                            (sdsr_ref_type) (sdsr_tbl[global_sd_asubs_id]->num - 1),
                            TRUE );

      return (sdsr_ref_type) (sdsr_tbl[global_sd_asubs_id]->num - 1);
    }
  }
  ...
  */
  ...
  /* if new entry is 3GPP and the entry to be overwritten is not , allocate memory for GWL bands
  */
  if (sr_ptr->sys.mode == SD_MODE_GWL && lru_ptr->sys.mode != SD_MODE_GWL )
  {
    lru_ptr->sys.gwl_band_ptr = (sd_3GPP_band_s_type *)modem_mem_alloc(\
                                sizeof(sd_3GPP_band_s_type), MODEM_MEM_CLIENT_MMODE);
    if( lru_ptr->sys.gwl_band_ptr == NULL )
    {
      sys_err_fatal_null_ptr_exception();
    }

    memset(lru_ptr->sys.gwl_band_ptr,0,sizeof(sd_3GPP_band_s_type));
    if( global_sd_asubs_id < MAX_SIMS &&
        global_sd_asubs_id >= SYS_MODEM_AS_ID_1 &&
        sdsr_tbl[global_sd_asubs_id])
    {
  ...
  else if (sr_ptr->sys.mode != SD_MODE_GWL && lru_ptr->sys.mode == SD_MODE_GWL )
  {
    SD_ASSERT(lru_ptr->sys.gwl_band_ptr);
    modem_mem_free((void *)lru_ptr->sys.gwl_band_ptr, MODEM_MEM_CLIENT_MMODE);
    if( global_sd_asubs_id < MAX_SIMS &&
        global_sd_asubs_id >= SYS_MODEM_AS_ID_1 &&
        sdsr_tbl[global_sd_asubs_id])
    {
...
{
  sdsr_s_type sr;/* Ok to not use sdsr_create_sr_rec_copy*/
  ...
  */
  ...
  /* Initialize the 3GPP band field
  */
  sr.sys.gwl_band_ptr = NULL;
...
===========================================================================*/
EXTERN  sdsr_ref_type             sdsr_tbl_store8(
...
)
{
  ...
  */
  if (mode == SD_MODE_GWL)
  {
    sr.sys.gwl_band_ptr = (sd_3GPP_band_s_type *)modem_mem_alloc(\
                          sizeof(sd_3GPP_band_s_type), MODEM_MEM_CLIENT_MMODE);
    if( sr.sys.gwl_band_ptr == NULL )
    {
      sys_err_fatal_null_ptr_exception();
    }

    sr.sys.gwl_band_ptr->lte_band = lte_band_pref;
    sr.sys.gwl_band_ptr->tds_band = tds_band_pref;

    if( global_sd_asubs_id < MAX_SIMS &&
        global_sd_asubs_id >= SYS_MODEM_AS_ID_1 &&
        sdsr_tbl[global_sd_asubs_id])
    {
      sdsr_tbl[global_sd_asubs_id]->num_alloc++; //debug info
    }
  }
  ...
  */
  ...
  /* deallocate local memory for LTE , TDS bands now that sdsr table has the entry.
  */
  if (mode == SD_MODE_GWL)
  {
    modem_mem_free((void *)sr.sys.gwl_band_ptr, MODEM_MEM_CLIENT_MMODE);
  ...
  */
  ...
  if(sdss_get_enhanced_f3_dbg_rule () & SD_ENHANCED_F3_DBG_LIST_PRINT)
  {
    sdsr_print(sr_ptr,FALSE,sr_ref);
  }

  if (sr_ref != SDSR_REF_NULL &&
      sr_ptr->sys.mode == SD_MODE_GWL && sr_ptr->sys.gwl_band_ptr == NULL)
  {
    SD_ERR_FATAL_1("sdsr_tbl_store11 sr_ref %d", sr_ref);
  }
...
)
{
  sdsr_ref_type   i = (sdsr_ref_type)SDSR_POS_NULL;
  sdsr_s_type     *tbl_ptr = NULL;
  sdsr_s_type     sr_a; /* memory alloc and dealloc for gwl_band_ptr taken care of within function */
  sr_a.sys.gwl_band_ptr = (sd_3GPP_band_s_type *)modem_mem_alloc(\
                          sizeof(sd_3GPP_band_s_type), MODEM_MEM_CLIENT_MMODE);
  if( sr_a.sys.gwl_band_ptr == NULL )
  {
    sys_err_fatal_null_ptr_exception();
  }
  ...
  if( global_sd_asubs_id < MAX_SIMS &&
      ...
      sdsr_tbl[global_sd_asubs_id])
  {
    sdsr_tbl[global_sd_asubs_id]->num_alloc++; /* increment indicator that GWL band mem has been allocated */
  }

  sr_a.sys.band     = gw_band_pref;
  sr_a.sys.mode     = mode;
  sr_a.sys.gwl_band_ptr->lte_band = lte_band_pref;
  sr_a.sys.gwl_band_ptr->tds_band = 0;
  sr_a.sys.chan     = chan;
  ...
  */
  modem_mem_free((void *)sr_a.sys.gwl_band_ptr, MODEM_MEM_CLIENT_MMODE);
  if( global_sd_asubs_id < MAX_SIMS &&
      global_sd_asubs_id >= SYS_MODEM_AS_ID_1 &&
      sdsr_tbl[global_sd_asubs_id])
  {
    sdsr_tbl[global_sd_asubs_id]->num_dealloc++; /* increment indicator that GWL band mem has been deallocated */
  }
...
static atomic_word_t  *sdsr_lists_arr[MAX_SIMS];
...
static sdsr_ref_type         *sdsr_items_arr[MAX_SIMS];
/*lint +e656 */
...
)
{
  ...
  SD_MEM_FREE(list_ptr->ref, MODEM_MEM_CLIENT_MMODE);
  SD_MEM_FREE(list_ptr, MODEM_MEM_CLIENT_MMODE);
}
...
)
{
  ...
  if (sdsr_alist_is_list(alist_ptr))
  {
    ...
    if ( !INRANGE(list, 0,( (sdsr_lists_size[global_sd_asubs_id])-1)))
    {
      return;
    }
    ...
    */
    old_list_ptr = (sdsr_list_s_type *) atomic_exchange(&sdsr_lists_arr[global_sd_asubs_id][list],
...
)
{
  ...
  */
  ...
  /* Free the old array.
  */
  SD_MEM_FREE(old_array_ptr, MODEM_MEM_CLIENT_MMODE);
...
)
{
  ...
  {
    /*SD_ASSERT( INRANGE(list, 0,( (int) (ARR_SIZE(sdsr_lists_arr[global_sd_asubs_id])-1))) );*/

    SD_ASSERT( INRANGE(list, 0,((int) sdsr_lists_size[global_sd_asubs_id] - 1)) );
    ...
    */
    ...
    alist_ptr->ptr.list = (sdsr_list_s_type *) sdsr_lists_arr[global_sd_asubs_id][list].value;
    /*lint -restore*/
  ...
  else
  {
    ...
    */
    alist_ptr->ptr.item = (INRANGE(i, 0, (int) (sdsr_items_arr_size[global_sd_asubs_id]-1)))?
                          &sdsr_items_arr[global_sd_asubs_id][i] : NULL;
  }
  ...
  */
  ...
  for (pos = 0 ; pos < num ; ++pos)
  {
    if(sdsr_alist_get_sr_ptr(alist_ptr, pos, &list_sr_ptr)
        && list_sr_ptr->sys.mode == SD_MODE_GWL)
    {
      sys_lte_band_mask_e_type sr_lte_band,list_sr_lte_band;


      sr_lte_band = sdsr_get_lte_band(sr_ptr);
      list_sr_lte_band = sdsr_get_lte_band(list_sr_ptr);

      if (   SD_BAND_CONTAIN(list_sr_ptr->sys.band,sr_ptr->sys.band)
  ...
  switch( order_cat )
  {
    case SDSR_ORDER_BAND:
      ...
      */
      ...
      for (j = 0 ; j <= count ; ++j)
      {
        sys_lte_band_mask_e_type  sr_lte_band     = SYS_LTE_BAND_MASK_CONST_NONE;
        sys_lte_band_mask_e_type  sr_ref_lte_band = SYS_LTE_BAND_MASK_CONST_NONE;
        boolean                   is_3gpp         = FALSE;
        ...
        /* access L and T bands only if both records are 3gpp
        */
        if (sr_ptr->sys.mode == SD_MODE_GWL && sr_ref_ptr->sys.mode == SD_MODE_GWL)
        {
          sr_lte_band =  (sr_ptr->sys.gwl_band_ptr->lte_band);
          sr_ref_lte_band =  (sr_ref_ptr->sys.gwl_band_ptr->lte_band);
          is_3gpp = TRUE;
        }
        if( SD_BAND_CONTAIN(sr_ptr->sys.band, sr_ref_ptr->sys.band) ||
            ( is_3gpp && (SYS_LTE_BAND_MASK_CHK_IF_OVERLAP(&sr_lte_band,&sr_ref_lte_band) ||
                          SD_BAND_CONTAIN(sr_ptr->sys.gwl_band_ptr->tds_band, sr_ref_ptr->sys.gwl_band_ptr->tds_band)))
          )
        {
...
)
{
  ...
  if(!INRANGE(list, 0,((int) sdsr_lists_size[global_sd_asubs_id] - 1)) && list< SDSR_1ST_SYS)
  {
    SD_MSG_HIGH_1("List cnt for unallocated list %d",list);
    return 0;
  }
...
===========================================================================*/
static  void                      sdsr_log_list_print(
...
)
{
  ...
  sdsr_ref_type sr_ref;
  sdsr_s_type   *sr_ptr;
  log_code_type         log_code    = LOG_SDSR_LIST_PRINT_C;
  /* Code to be logged.
  */

  unsigned int          log_pkt_len = sizeof( LOG_SDSR_LIST_PRINT_C_type );

  LOG_SDSR_LIST_PRINT_C_type  *log_ptr  = NULL;
  /* log pointer to the log record type.
  */



  cnt = sdsr_alist_cnt(alist_ptr);

  packets = (int)(ceil((double)cnt/(sizeof(char))));

  for(j=0; j< packets; ++j)
  {
    log_ptr = (LOG_SDSR_LIST_PRINT_C_type *) log_alloc_ex( log_code,
              log_pkt_len );
    /* If allocation failed, just return.
    */
    if ( log_ptr == NULL )
    {
      //SD_MSG_HIGH_1 ("Log packet alloc failed for List: %d", alist_ptr->list);
      return;
    }
    ...
    log_ptr->size = (log_ptr->is_complete)?(cnt - j*(sizeof(char))):(sizeof(char));


    log_ptr->list = alist_ptr->list;

    for(i=0; i<log_ptr->size; ++i)
    {
      sr_ref = sdsr_alist_get(alist_ptr, i);
      sr_ptr = sdsr_tbl_get_ptr(sr_ref);
      if (sr_ptr == NULL)
      {
        continue;
      }
      log_ptr->system_record[i].tbl_index = sr_ref;

      log_ptr->system_record[i].mode = sr_ptr->sys.mode;

      log_ptr->system_record[i].band_pref = sr_ptr->sys.band;

      if(sr_ptr->sys.mode == SD_MODE_GWL || sr_ptr->sys.mode == SD_MODE_GW ||
          sr_ptr->sys.mode == SD_MODE_LTE || sr_ptr->sys.mode == SD_MODE_TDS)
      {
        if (sr_ptr->sys.mode == SD_MODE_TDS )
        {
          log_ptr->system_record[i].mode_info.gpp_info.tds_band_pref = sr_ptr->sys.gwl_band_ptr->tds_band;
        }
        log_ptr->system_record[i].mode_info.gpp_info.net_select_mode = sr_ptr->sid_expected.net_select_mode;
        log_ptr->system_record[i].mode_info.gpp_info.plmn_id.identity[0] =
          sr_ptr->sid_expected.prm.gwl.plmn.plmn_id.identity[0];
        log_ptr->system_record[i].mode_info.gpp_info.plmn_id.identity[1] =
          sr_ptr->sid_expected.prm.gwl.plmn.plmn_id.identity[1];
        log_ptr->system_record[i].mode_info.gpp_info.plmn_id.identity[2] =
          sr_ptr->sid_expected.prm.gwl.plmn.plmn_id.identity[2];

        if (sr_ptr->sys.mode == SD_MODE_GWL  || sr_ptr->sys.mode == SD_MODE_LTE)
        {
#if defined(LTE_BAND_NUM) && (LTE_BAND_NUM == 256)
          log_ptr->system_record[i].mode_info.gpp_info.lte_bit_mask_256_t.bits_1_64 =\
              sr_ptr->sys.gwl_band_ptr->lte_band.bits_1_64;
          log_ptr->system_record[i].mode_info.gpp_info.lte_bit_mask_256_t.bits_65_128 =\
              sr_ptr->sys.gwl_band_ptr->lte_band.bits_65_128;
#else
          log_ptr->system_record[i].mode_info.gpp_info.lte_bit_mask_256_t.bits_1_64 =\
              sr_ptr->sys.gwl_band_ptr->lte_band;
#endif
        }
      }
      else
      {
        log_ptr->system_record[i].mode_info.gpp2_info.prl = sr_ptr->prl;
        log_ptr->system_record[i].mode_info.gpp2_info.chan = sr_ptr->sys.chan;
        log_ptr->system_record[i].mode_info.gpp2_info.sid = sr_ptr->sid.is95.sid;
        log_ptr->system_record[i].mode_info.gpp2_info.expected_sid = sr_ptr->sid_expected.prm.is95.sid;
        log_ptr->system_record[i].mode_info.gpp2_info.roam = sr_ptr->roam;
        log_ptr->system_record[i].mode_info.gpp2_info.sched = sr_ptr->sched;
      }
    }

    log_util_log_packet_commit(global_sd_asubs_id, log_ptr);
  }
} /* sdsr_list_print */
...
)
{
  ...
  */
  ...
  SD_MSG_HIGH_3("SDSR list: %d print, count=%d, sub %d", list, cnt, global_sd_asubs_id);
  if(cnt == 1)
  {
    sr_ref = sdsr_alist_get(&alist, 0);
    sr_ptr = sdsr_tbl_get_ptr(sr_ref);
    if (NULL != sr_ptr)
    {
  ...
  {
    sdsr_log_list_print(&alist);
  }

  for (i = 0 ; i < cnt && (sdss_get_enhanced_f3_dbg_rule() & SD_ENHANCED_F3_DBG_LIST_PRINT) && (cnt != 1); ++i)
  {
    if (0 != i && 0 == i%10)
    {
...
)
{
  ...
  */
  ...
  if(list_cnt != 0)
  {
    SD_MSG_HIGH_4( "sdsr_list_clr() list:%d, list_cnt %d, sub %d, list ptr %d", 
                      list, list_cnt, global_sd_asubs_id,
                      sdsr_list_get_ptr(list, 0));
  }
...
)
{
  ...
  sdsr_list_store_and_app3(list1,
                           ...
                           mode,
                           band_pref,
                           SYS_LTE_BAND_MASK_CONST_NONE, // LTE band pref
...
===========================================================================*/
void                  sdsr_tbl_reset_per_ss( multimode_stack_e_type ss )
{
  int i = 0;
  sdsr_s_type *sr = NULL;
  ...
  for(i=0; i<PRL_TBL_MAX; i++)
  {
    sr = sdsr_tbl[global_sd_asubs_id]->arr[i];
...
void                  sdsr_lists_init( void )
{
  ...
  sdsr_reset_sdsr_items_arr(global_sd_asubs_id);

  if(stack_mask[global_sd_asubs_id].old_stack_active_mask
      & MM_STACK_2_MASK)
  {

    sdsr_lists_clr_with_range(SDSR_HYBR_1_LST, SDSR_MAX_LST);
    sdsr_lists_clr_with_range(SDSR_HYBR_1_SYS, SDSR_MAX);
      ...
      & MM_STACK_1_MASK)
  {
    ...
    sdsr_lists_clr_with_range(SDSR_AVAIL_LST_, SDSR_HYBR_1_LST);
    sdsr_lists_clr_with_range(SDSR_ACQ_SYS_, SDSR_HYBR_1_SYS);
      ...
      & MM_STACK_0_MASK)
  {
    ...
    sdsr_lists_clr_with_range(0, SDSR_HDR_LST_);
    sdsr_lists_clr_with_range(SDSR_1ST_SYS, SDSR_HDR_SYS_);
...
FUNCTION sdsr_lists_deallocate

DESCRIPTION
   Deallocate stack specific SDSR lists for SUBs
...
EXTERN void sdsr_lists_deallocate(

  sys_modem_as_id_e_type asubs_id
  /*no stack mask is required as all memory needs to be deallocated*/
)
{
  int i = 0;
  ...
  if(sdsr_lists_arr[asubs_id] != NULL)
  {
    /* Deallocate all lists allocated till now */
    for(i=0; i<sdsr_lists_size[asubs_id]; i++)
    {
      sdsr_list_s_type *list = (sdsr_list_s_type *)sdsr_lists_arr[asubs_id][i].value;
      REF_CNT_OBJ_RELEASE_IF(list);
    }
    SD_MEM_FREE( sdsr_lists_arr[asubs_id], MODEM_MEM_CLIENT_MMODE );
    sdsr_lists_arr[asubs_id] = NULL;
  }

  if(sdsr_items_arr[asubs_id] != NULL)
  {
    SD_MEM_FREE( sdsr_items_arr[asubs_id], MODEM_MEM_CLIENT_MMODE );
    sdsr_items_arr[asubs_id] = NULL;
  }
...
void sdsr_lists_allocate(
  uint8                    stack_active,
  sys_modem_as_id_e_type asubs_id
)
{
  /* In case no stack mask is set, no need to allocate any memory */
  int temp =0;
  if ( stack_active == 0
     )
  {
  ...
  sdsr_lists_deallocate(asubs_id);
  ...
  if(stack_active ==
      (MM_STACK_0_MASK | MM_STACK_1_MASK | MM_STACK_2_MASK) )
  {
    sdsr_lists_arr[asubs_id] = (atomic_word_t *)modem_mem_alloc(sizeof(atomic_word_t) * SDSR_MAX_LST, MODEM_MEM_CLIENT_MMODE);
    memset(sdsr_lists_arr[asubs_id],0,sizeof(atomic_word_t) * SDSR_MAX_LST);
    sdsr_items_arr[asubs_id] = (sdsr_ref_type *)modem_mem_alloc(sizeof(sdsr_ref_type) * (SDSR_MAX - SDSR_1ST_SYS), MODEM_MEM_CLIENT_MMODE);

    sdsr_lists_size[asubs_id] = SDSR_MAX_LST;
    sdsr_items_arr_size[asubs_id] = SDSR_MAX - SDSR_1ST_SYS;
  }
  else if(stack_active ==
          (MM_STACK_0_MASK | MM_STACK_1_MASK) )
  {
    sdsr_lists_arr[asubs_id] = (atomic_word_t *)modem_mem_alloc(sizeof(atomic_word_t) * SDSR_HYBR_1_LST, MODEM_MEM_CLIENT_MMODE);
    memset(sdsr_lists_arr[asubs_id],0,sizeof(atomic_word_t) * SDSR_HYBR_1_LST);
    sdsr_items_arr[asubs_id] = (sdsr_ref_type *)modem_mem_alloc(sizeof(sdsr_ref_type) * (SDSR_HYBR_1_SYS - SDSR_1ST_SYS), MODEM_MEM_CLIENT_MMODE);
    sdsr_lists_size[asubs_id] = SDSR_HYBR_1_LST;
    sdsr_items_arr_size[asubs_id] = SDSR_HYBR_1_SYS - SDSR_1ST_SYS;
  }
  else if(stack_active ==
          (MM_STACK_0_MASK) )
  {
    sdsr_lists_arr[asubs_id] = (atomic_word_t *)modem_mem_alloc(sizeof(atomic_word_t) * SDSR_HDR_LST_, MODEM_MEM_CLIENT_MMODE);
    memset(sdsr_lists_arr[asubs_id],0,sizeof(atomic_word_t) * SDSR_HDR_LST_);
    sdsr_items_arr[asubs_id] = (sdsr_ref_type *)modem_mem_alloc(sizeof(sdsr_ref_type) * (SDSR_HDR_SYS_ - SDSR_1ST_SYS), MODEM_MEM_CLIENT_MMODE);
    sdsr_lists_size[asubs_id] = SDSR_HDR_LST_;
    sdsr_items_arr_size[asubs_id] = SDSR_HDR_SYS_ - SDSR_1ST_SYS;
  }

  //temp = sizeof(sdsr_lists_arr);
  sdsr_reset_sdsr_items_arr(asubs_id);


  if(sdsr_lists_arr[asubs_id] == NULL ||
      sdsr_items_arr[asubs_id] == NULL)
  {
    SD_ERR_0("Can't allocate SDSR LISTS memory");
    return;
  }
...
)
{
  ...
  multimode_stack_e_type sd_ss = sdss_get_ss_for_mode(global_sd_asubs_id, SD_MODE_CDMA);
  sdsr_s_type *temp_sys_ptr = NULL;

  if( (sd_ss >= MM_STACK_ALL) || (sd_ss < MM_STACK_0) )
  {
    return;
  }
  ...
  if((ss == MM_STACK_2) && sdss_is_1x_sxlte() )
  {
        ...
        temp_sys_ptr->sys.mode == SD_MODE_CDMA)
    {
      ...
      sdsr_list_intersect(SDSR_MORE_PREF_HDR_LST, SDSR_GEO_ACQ_PREF_LST);
    }
...
EXTERN  void                      sdsr_list_update_gw_sys(
  ...
  */
  boolean                   update_manual_sys
  /* If true, update the manual system with the system reference.
  */
)
{
#if defined(SD_GSM_SUPPORTED) || defined(SD_WCDMA_SUPPORTED)  || defined(FEATURE_SD_LTE)
  ...
  */
  if (!sd_misc_is_mode_3gpp(tbl_ptr->sys.mode))
  {
    return;
  }
  /* create a local copy of the system record
  */
  ...
  */
  if( sr_ptr->sid_expected.net_select_mode != SD_NET_SELECT_MODE_AUTO  &&
  ...
  */
  if( sr_ptr->sid_expected.net_select_mode == SD_NET_SELECT_MODE_LIMITED )
  {
    sdsr_order( manual_lst, sr_ptr, SDSR_ORDER_BAND );
    is_sys_ltd = TRUE;
  }
  ...
  */
  full_sr_ref = sdsr_tbl_store8( sr_ptr );
  ...
  */
  ...
  ltd_sr_ref = sdsr_tbl_store8( sr_ptr);
  ...
  */
  ...
  user_sr_ref = sdsr_tbl_store8( sr_ptr );
...
)
{
  ...
  */
  ...
  if(tbl_ptr->sid_expected.gw_ss == ss)
  {
  ...
  */
  ...
  sr_ptr->sid_expected.gw_ss = ss;

  /* Store new record */
  sr_ref = sdsr_tbl_store8( sr_ptr );
...
)
{
  ...
  */
  for( i=0; (tbl_ptr=sdsr_tbl_get_ptr(i)) != NULL; i++ )
  {
    ...
    */
    if(  sr_mode == SD_MODE_GWL  &&
         (tbl_ptr->sys.mode == sr_ptr->sys.mode &&
          tbl_ptr->sys.band == sr_ptr->sys.band &&
          tbl_ptr->sys.gwl_band_ptr->tds_band == sr_ptr->sys.gwl_band_ptr->tds_band &&
#ifdef FEATURE_SD_LTE
          sdsr_chk_lte_band_equal(tbl_ptr->sys.gwl_band_ptr->lte_band,sr_ptr->sys.gwl_band_ptr->lte_band) &&
#endif
...
)
{
  ...
  */
  ...
  /* If the network selection mode is not AUTOMATIC or MANUAL, return now.
  */
  if( tbl_ptr->sid_expected.net_select_mode != SD_NET_SELECT_MODE_AUTO  &&
      tbl_ptr->sid_expected.net_select_mode != SD_NET_SELECT_MODE_MANUAL_SYS &&
      tbl_ptr->sid_expected.net_select_mode != SD_NET_SELECT_MODE_LIMITED &&
      tbl_ptr->sid_expected.net_select_mode != SD_NET_SELECT_MODE_HPLMN)
  {
    return;
  }
  ...
  */
  ...
  if( sr_ptr->sid_expected.net_select_mode == SD_NET_SELECT_MODE_HPLMN)
  {
    full_sr_ref = sdsr_tbl_get_gwl_sr_ref(sr_ptr, SD_NET_SELECT_MODE_HPLMN);
  }
  else if(sr_ptr->sid_expected.net_select_mode == SD_NET_SELECT_MODE_MANUAL_SYS)
  {
    full_sr_ref = sdsr_tbl_get_gwl_sr_ref(sr_ptr, SD_NET_SELECT_MODE_MANUAL_SYS);
    ...
    */
    ...
    {
      full_sr_ref = sdsr_tbl_get_gwl_sr_ref(sr_ptr, SD_NET_SELECT_MODE_AUTO);
    }
  ...
  */
  ...
  ltd_sr_ref = sdsr_tbl_get_gwl_sr_ref(sr_ptr, SD_NET_SELECT_MODE_LIMITED);
  ...
  */
  ...
  user_sr_ref = sdsr_tbl_get_gwl_sr_ref(sr_ptr, SD_NET_SELECT_MODE_MANUAL_SYS);
...
EXTERN boolean sdsr_backup_last_acq_sys(void)
{
  ...
  sdsr_s_type               *sr_ptr = sdsr_get_last_acq_sys_ptr (MM_STACK_0);
  sd_3GPP_band_s_type       *gwl_band_ptr = sr_ptr->sys.gwl_band_ptr;
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

  if(acq_sys_ptr && sr_ptr)
  {
    *sr_ptr = *acq_sys_ptr;
    if(acq_sys_ptr->sys.mode == SD_MODE_GWL)
    {
      SD_MSG_HIGH_0("backup_last_acq_sys - mode gwl");
      *gwl_band_ptr = *acq_sys_ptr->sys.gwl_band_ptr;
    }
    sr_ptr->sys.gwl_band_ptr = gwl_band_ptr;
    return TRUE;
  }
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  return FALSE;
...
boolean sdsr_backup_last_acq_hybrid_sys(void)
{
  ...
  sdsr_s_type      *sr_ptr       = sdsr_get_last_acq_sys_ptr (MM_STACK_1);
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  */
  if(acq_sys_ptr && sr_ptr)
  {
    if(acq_sys_ptr->sys.mode == SD_MODE_HDR)
    {
      *sr_ptr = *acq_sys_ptr;
      sr_ptr->sys.gwl_band_ptr = NULL;
      return TRUE;
    }
    else
    {
      sdsr_list_clr(SDSR_ACQ_SYS_);
    }
  }
#endif
  return FALSE;
...
void sdsr_add_last_hybrid_acq_sys(void)
{
  ...
  SD_MSG_HIGH_4("sdsr_add_last_hybrid_acq_sys stk 1, mode %d, gwl_band_ptr %d, net_select_mode %d, sdsr_rec_type %d",
                sr_ptr->sys.mode,
                sr_ptr->sys.gwl_band_ptr,
                sr_ptr->sid_expected.net_select_mode,
                sr_ptr->sdsr_rec_type);

  /* store sys in sdsr table and add ref in sdsr_items_arr*/
  if(sr_ptr->sys.mode == SD_MODE_HDR)
  {
    sr_ref = sdsr_tbl_store11(sr_ptr->sys.mode,
                              sr_ptr->sys.band,
                              sdsr_get_lte_band(sr_ptr),
                              sdsr_get_tds_band(sr_ptr),
                              sr_ptr->sys.chan,
...
void sdsr_add_last_acq_sys(void)
{
  ...
  SD_MSG_HIGH_4("sdsr_add_last_acq_sys stk 0, mode %d, gwl_band_ptr %d, net_select_mode %d, sdsr_rec_type %d",
                sr_ptr->sys.mode,
                sr_ptr->sys.gwl_band_ptr,
                sr_ptr->sid_expected.net_select_mode,
                sr_ptr->sdsr_rec_type);
...
===========================================================================*/
void sdsr_reset_sdsr_items_arr(sys_modem_as_id_e_type asubs_id )
{
  int ix;
  ...
  for (ix=0; ix < sdsr_items_arr_size[asubs_id]; ix++)
  {
    sdsr_items_arr[asubs_id][ix]=SDSR_REF_NULL;
  }
}
...
void          sdsr_update_rec_type()
{
  ...
  SD_MSG_HIGH_0("NAM_SEL_MORE: sdsr_update_rec_type");
...
)
{
  ...
  */
  ...
  for (i = 0 ; i < num ; ++i)
  {
    ...
    {
      if( sr_ptr->sys.mode == SD_MODE_GWL &&
          !sdsr_chk_lte_band_equal(sdsr_get_lte_band(sr_ptr), SYS_LTE_BAND_MASK_CONST_NONE)
        )
      {
        result = TRUE;
...
===========================================================================*/
EXTERN  boolean  sdsr_chk_lte_band_equal(

  sys_lte_band_mask_e_type  sys_lte_band_pref1,

  sys_lte_band_mask_e_type  sys_lte_band_pref2
)
{

  return(SYS_LTE_BAND_MASK_CHK_IF_EQUAL(&sys_lte_band_pref1,&sys_lte_band_pref2));
}

/*===========================================================================

FUNCTION sdsr_get_tds_band

DESCRIPTION
  Provides TDS bands associated with a given system record pointer

DEPENDENCIES
  None

RETURN VALUE
  TDS bands associated with the system record pointer

SIDE EFFECTS
  None.

===========================================================================*/
EXTERN  sd_ss_band_pref_e_type  sdsr_get_tds_band(

  sdsr_s_type    *sr_ptr
  /* pointer to record from which to get TDS band pref*/
)
{
  if (sr_ptr == NULL ||
  ...
  return  sr_ptr->sys.gwl_band_ptr->tds_band;

}

/*===========================================================================

FUNCTION sdsr_get_lte_band

DESCRIPTION
  Provides LTE bands associated with a given system record pointer

DEPENDENCIES
  None

RETURN VALUE
  LTE bands associated with the system record pointer

SIDE EFFECTS
  None.

===========================================================================*/
EXTERN  sys_lte_band_mask_e_type  sdsr_get_lte_band(

  sdsr_s_type    *sr_ptr
  /* pointer to record from which to get LTE band pref*/
)
{
  if (sr_ptr == NULL ||
      sr_ptr->sys.mode != SD_MODE_GWL)
  ...
  return   (sr_ptr->sys.gwl_band_ptr->lte_band);

}

#ifdef FEATURE_QSH_MDUMP
/*===========================================================================
FUNCTION sdsr_qsh_mdump_collect
  
DESCRIPTION
  This function collects the mdump for sdsr global variables
...
QSH_MDUMP_FN_ATTR
#endif
void sdsr_qsh_mdump_collect()
{
  int sub;
  qsh_mdump_collect_high(&sdsr_lists_arr[0],MAX_SIMS*sizeof(atomic_word_t *));
  qsh_mdump_collect_high(&sdsr_items_arr[0],MAX_SIMS*sizeof(sdsr_ref_type *));
  
  for (sub = SYS_MODEM_AS_ID_1; sub < MAX_SIMS; sub++)
  {
    qsh_mdump_collect_high(sdsr_lists_arr[sub],sizeof(atomic_word_t));
    qsh_mdump_collect_high(sdsr_items_arr[sub],sizeof(sdsr_ref_type));
  }
}
#endif
