...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/mmode/sd/src/sdss.h#7 $
...
/**< Maximum number of MCCs for emergency special scan. */
#define SDSS_SPECIAL_SCAN_MCC_LIST_MAX 5

typedef struct
{

  uint32 mcc[SDSS_SPECIAL_SCAN_MCC_LIST_MAX];

} sdss_special_scan_mcc_list_s_type;
...
typedef enum
{
  ...
  SDSS_FLAG_SKIP_WRLF_RECOVERY,    /* Flag to indicate whether we need to skip WRLF recovery process and
                                   ** search for all RATs*/
  ...
  SDSS_FLAG_SKIP_MMSS_IN_UE_MODE_CHG, /* Flag to indicate not sending MMSS service type for user pref change due to UE mode change
                                                                      ** from CSFB_PREF(dual stack) -> CSFB(single stack) */

  SDSS_FLAG_CONSIDER_NEW_ACQ,         /* Consider new_acq_cnt while giving acquistion */
  
  /* 28 */
  SDSS_FLAG_AVOID_PLMN_DUE_TO_380,    /* flag to indicate if plmns need to be avoided during 380 response handling.*/

  SDSS_FLAG_CHINA_PRIORITIZE_L_1X,   /* flag to skip g/W - prioritize LTE+1x for one round */

  SDSS_FLAG_CHINA_EMERG_SKIP_LTE,   /* flag to skip LTE and allow GW only after LTE+1x is done for china emerg */

  SDSS_FLAG_LTE_DEACT_DUAL_SWITCH,   /* flag to indicate lte is deactivated due to dual switch */
  
  /* 32 */
  SDSS_FLAG_PREF_CHANGE_PENDING,   /* flag to indicate whether pref change is pending */
...
typedef enum
{
  ...
  SDSS_PREF_UPDATED_HYBR_PREF,       /* Only Hybrid pref changed */
  SDSS_PREF_UPDATED_MMS, /* Only service domain changed between CS_PS and CS_VOLTE */
...
typedef struct
{
  ...
  sd_ss_band_pref_e_type       band_pref;       /* band preference */
  sys_lte_band_mask_e_type       lte_band_pref;   /* lte band preference */
...
typedef enum
{
  ...
  */
  ...
  SDSS_REDIR_VAL_CDMA_LTE_ANY       = BM(SD_SS_REDIR_CDMA_LTE_RESEL)|
                                      BM(SD_SS_REDIR_CDMA_LTE_REDIR ),
  /* Any LTE to CDMA reselection */
...
typedef enum
{
  ...
  SDSS_CNTED_EVT_PWR_REG_FADE_CNT,  /* Powerup regisatrtion Access fail */
...
typedef enum
{
  ...
  SDSS_EVT_CNT_UPDT_REAS_PWR_REG_FADE_CNT,  /* Access termination during the powerup registartion
                                            ** due to fade timeout
                                            */
...
//  SDSS_IF_COND_HDR_HYBR,         /* If HDR is active on Hybrid */
  ...
  SDSS_IF_COND_C2K_SCAN_OPTI_ALLOWED,    /* Check whether 3GPP2 scan optimization should be enabled for
                                       ** FULL RAT MODE cases */

  /* 77 */
  SDSS_IF_COND_SKIP_WRLF_SET,  /* Check whether WRLF recovery process needs to be skipped */

  SDSS_IF_COND_C2K_LTE_SCAN_FLAG,    /* Check if the LTE to be scanned with call end BSR */

  SDSS_IF_COND_HDR_ALT_ALGR,     /* If HDR Alt Algorithm is enabled */

  SDSS_IF_COND_IS_DDS_SWITCHBACK_FROM_TEMP, /* Check whether DDS switch is permanent and preceded by temporary */

  /* 81 */
  SDSS_IF_COND_TRIGGER_BPLMN_ACQ_DB_SCAN,

  SDSS_IF_COND_LTE_EMERG_REDIAL,    /*check if LTE prioritized scan needed for e911 call */

  SDSS_IF_COND_LAST_SRV_REQ_TYPE,   /* check the last_srv_req_type */

  SDSS_IF_COND_EMERG_IN_ROAM,      /*Check if e911 is intiated when in camped on roam LTE system */

  SDSS_IF_COND_CHINA_EMERG_CRITERIA_WITHOUT_SUB, /* Prioritize 1x with no subscrition available in china*/

  SDSS_IF_COND_SI_SRV_STATUS,       /* If SI SRV status is equal to param */

  SDSS_IF_COND_EMERG_LST_IS_EMPTY,/* Check if emerg list is empty */

  /* 88 */
  SDSS_IF_COND_BUF_SRV_LOST_TRM_FAILURE, /* if srv_lost should be buffered due to TRM NO_RESOURCE */

  SDSS_IF_COND_SUB_SCAN_SCOPE_RULE, /* Check sub level scan scope  */

  SDSS_IF_COND_LAST_ACT_GWL_ACQ_DB, /* Checks if last service req is GWL Acq DB */

  SDSS_IF_COND_FULL_RAT_IN_3GPP_ONLY_MODE,     /* Checks the current full RAT status */

  SDSS_IF_COND_AVOID_PLMN_DUE_TO_380, /* Checks if there are plmns to avoid due to ims end cause 380*/

  SDSS_IF_COND_IS_DUAL_LTE_DDS_SWITCH, /* For DDS switch in dual-LTE config with no max cap change, 
                                       ** both subs should start with acq db scan. */

  /* 94 */
  SDSS_IF_COND_CHINA_EMERG_CRITERIA_WITH_SUB, /* Prioritize 1x+LTE with subscrition available in china*/

  SDSS_IF_COND_EMERG_LST_MODE_PREF_3GPP,        /* If the list contains G/W/L/T */

  SDSS_IF_COND_IS_CHN_EMG_NUM, 

  SDSS_IF_COND_IS_SRV_SYS_CDMA, 

  /* 98 */
  SDSS_IF_COND_SLEEP_TIMER_RUNNING,
  
  /* 115 */
  SDSS_IF_COND_STANDBY_PREF_CHANGED,

  SDSS_IF_COND_IDLE_ORIG_MODE_VAL,
...
/*---------------------------------------------------------------------------
                       SCAN OPTIMIZATION ITEM type that needs to be fetched from PM
---------------------------------------------------------------------------*/
typedef enum
{

  SDSS_FETCH_PM_C2K_SCAN_OPT_VAL = 0,
  /** Fetch whether c2k scan need to be optmized for WLRF or ALL cases */
  SDSS_FETCH_PM_SKIP_AUTO_CAMP_OPT_VAL,
  /** Fetch whether we need to skip mapping LIMITED service req to
  ** AUTO_CAMP_ONLY in FULL RAT mode */
  SDSS_SCAN_OPT_VAL_MAX

} sdss_scan_opt_e_type;
...
typedef struct
{
  ...
  */
  ...
  /*TO DO: after update from PM interface*/
  ...
  */
  ...
  /*Indicates whether fullrat optimistaion is enabled or not */
...
/* Current ACQ state to report to CXM -> Wifi
   */
typedef enum
{
  SDSS_CXM_PROC_ACQ_NONE = -1, /* FOR INTERNAL USE ONLY! */

  SDSS_CXM_PROC_ACQ_END = 0,
  
  SDSS_CXM_PROC_ACQ_START,
  
  SDSS_CXM_PROC_ACQ_MAX    /* FOR INTERNAL USE ONLY! */
  
}sdss_cxm_proc_acq_state_e_type;

/* Enumeration of Type for skip of full srv for Emergency
*/
typedef enum
{
  SDSS_SKIP_EMERG_FULL_SRV_NONE = 0,
  /* Emergency on FULL SRV */

  SDSS_SKIP_EMERG_FULL_SRV_NO_SIM=BM(0),
  /* Skip full srv for Emergency if NO SIM */

  SDSS_SKIP_EMERG_FULL_SRV_MAX=BM(1)

} sdss_skip_emerg_full_srv_e_type;
...
===========================================================================*/
extern  boolean                       sdss_is_orig_mode_subs(
...
#define sdss_is_orig_mode(stk_id,orig_mode_val)  sdss_is_orig_mode_subs(global_sd_asubs_id,stk_id,orig_mode_val)
...
FUNCTION sdss_is_hybr_pref_per_sub
...
===========================================================================*/
extern  boolean                       sdss_is_hybr_pref_per_sub(
...
EXTERN  sd_ss_mode_pref_e_type        sdss_get_true_mode_pref_subs(
...
FUNCTION sdss_get_mode_pref

DESCRIPTION
  Retrieve the Mode Preference for the specified SS.
...
===========================================================================*/
EXTERN  sd_ss_mode_pref_e_type  sdss_get_mode_pref(
...
FUNCTION sdss_get_prev_domain_pref

DESCRIPTION
  Retrieve the previous Domain Preference for the specified SS.
...
===========================================================================*/
EXTERN  sd_ss_srv_domain_pref_e_type  sdss_get_prev_domain_pref(
...
FUNCTION sdss_get_prev_mode_pref

DESCRIPTION
  Retrieve the previous Mode Preference for the specified SS.
...
===========================================================================*/
EXTERN  sd_ss_mode_pref_e_type  sdss_get_prev_mode_pref(
...
===========================================================================*/
EXTERN  sys_scan_scope_e_type     sdss_get_scan_scope_type (void);
...
===========================================================================*/
EXTERN  void     sdss_toggle_scan_scope_type (void);
...
FUNCTION sdss_restore_fullrat_pref

DESCRIPTION
  Restore the fullrat preferences to sdss core.
...
===========================================================================*/
EXTERN  void                          sdss_restore_fullrat_pref(
...
FUNCTION sdss_get_mode_band_pref_per_ss
...
===========================================================================*/
EXTERN  void                          sdss_get_mode_band_pref_per_ss(
...
===========================================================================*/
extern  sd_ss_orig_mode_e_type     sdss_get_orig_mode_subs(
...
#define sdss_get_orig_mode(stk_id)  sdss_get_orig_mode_subs(global_sd_asubs_id,stk_id)

/*===========================================================================

FUNCTION sdss_reset_is_dds_switch_type_temp

DESCRIPTION
   Resets the boolean is_dds_switch_type_temp after event
   processing.
...
extern  void     sdss_reset_is_dds_switch_type_temp(
...
FUNCTION sdss_reset_is_dual_lte_dds_switch

DESCRIPTION
   Resets the boolean is_dual_lte_dds_switch after event
   processing.
...
===========================================================================*/
EXTERN  void     sdss_reset_is_dual_lte_dds_switch(
...
FUNCTION sdss_is_prev_fullrat_status

DESCRIPTION
  Check if ue is not already in fullrat according to variable val

DEPENDENCIES
  None.

RETURN VALUE
  TRUE if val is FULLRAT_ON and ue is already in fullrat
  TRUE if val is FULLRAT_OFF and ue is not already in fullrat
...
boolean sdss_is_prev_fullrat_status(

  multimode_stack_e_type              ss,

  sd_ss_fullrat_pref_e_type      val
);


/*===========================================================================

FUNCTION sdss_is_fullrat_status

DESCRIPTION
  Check whether ue is entering fullrat according to variable val

DEPENDENCIES
  None.

RETURN VALUE
  TRUE if val is FULLRAT_ON and ue is entering fullrat
  TRUE if val is FULLRAT_OFF and ue is not entering fullrat
  FALSE all other cases
...
===========================================================================*/
boolean                   sdss_is_fullrat_status(
...
FUNCTION sdss_is_true_event

DESCRIPTION
  Check whether true event equals val

DEPENDENCIES
  None.

RETURN VALUE
  TRUE if true event equals VAL
  FALSE otherwise
...
FUNCTION sdss_check_spl_emerg_hplmn

DESCRIPTION
  Checks if the number if spl_emerg in the HPLMN
...
RETURN VALUE
  TRUE/FALSE if the number is special emergency

SIDE EFFECTS
  None.

===========================================================================*/
boolean            sdss_check_spl_emerg_hplmn(void );
...
===========================================================================*/
EXTERN  sdss_state_e_type             sdss_get_ss_state_subs(
...
#define sdss_get_ss_state(stk_id)  sdss_get_ss_state_subs(global_sd_asubs_id,stk_id)
...
===========================================================================*/
extern  sys_sim_state_e_type    sdss_get_sim_state(void);

/*===========================================================================

FUNCTION sdss_get_sim_state_per_subs

DESCRIPTION
   Fetches the current sim state per subs from SD System selection core.
...
===========================================================================*/
extern  sys_sim_state_e_type    sdss_get_sim_state_per_subs(
  sys_modem_as_id_e_type asubs_id
);
...
FUNCTION sdss_get_new_srv_acq_reqd

DESCRIPTION
   Fetches the current is_new_srv_acq_reqd from SD System selection core.
...
===========================================================================*/
boolean sdss_get_new_srv_acq_reqd(
  sys_modem_as_id_e_type       asubs_id
);

/*===========================================================================

FUNCTION sdss_set_new_srv_acq_reqd

DESCRIPTION
   Set the current is_new_srv_acq_reqd from SD System selection core.
...
===========================================================================*/
void sdss_set_new_srv_acq_reqd(
  sys_modem_as_id_e_type       asubs_id,
  boolean                      is_new_srv_acq_reqd
);
...
===========================================================================*/
extern void                      sdss_check_reset_emerg_rat_pri_list_info_no_emerg_orig(void);
...
===========================================================================*/
extern multimode_stack_e_type          sdss_get_lte_idle_ss(sys_modem_as_id_e_type asubs_id);

/*===========================================================================

FUNCTION sdss_get_lte_emerg_ss_for_empty_idle_cap

DESCRIPTION
  Returns the ss on which LTE is part of mode pref, during emergency call origination.
  This is required only when idle rat pri list is empty and has no items to perform
  full service acquisition during e911 call.

  Eg:
  1. If during 911 orig, UE is in SVLTE mode of operation.
      This api will ss = HYBR2

   2.else, 911 orig shouls be on stack 0 , use ss = MAIN
   
   3. If during 911 orig, LTE was not part of mode pref on either MAIN or HYBR2
      This api will return ss = MAX
...
RETURN VALUE
  ss on which LTE is acquired, during 911 call

SIDE EFFECTS
  None.

===========================================================================*/
extern  multimode_stack_e_type          sdss_get_lte_emerg_ss_for_empty_idle_cap(
        sys_modem_as_id_e_type asubs_id
);
...
===========================================================================*/
extern  void             sdss_set_wcdma_connected_mode_subs(
...
#define sdss_set_wcdma_connected_mode(conn_time, ss) sdss_set_wcdma_connected_mode_subs(conn_time,global_sd_asubs_id,ss)
/* <EJECT> */
/*===========================================================================
...
/*===========================================================================
  ...
  ===========================================================================*/
EXTERN void sdss_copy_emerg_avoid_dup_band_info(multimode_stack_e_type dest_ss, multimode_stack_e_type src_ss);
...
/*===========================================================================
  ...
  ===========================================================================*/
EXTERN void sdss_add_last_acq_sys_band_to_emerg_dup_band_info(multimode_stack_e_type ss);
...
extern boolean   sdss_is_list_scanned(multimode_stack_e_type                   ss,sdsr_e_type                   list);
...
@SIDE EFFECTS
  None.
===========================================================================*/
extern void  sdss_get_nas_addl_info_per_mm_id(

  mm_sub_stk_id_s_type     mm_id,
  /* SUBS ID and System selection MAIN or HDR.*/
...
FUNCTION sdss_get_ignore_manual_bst_flag
...
===========================================================================*/
EXTERN  boolean                          sdss_get_ignore_manual_bst_flag(
...
FUNCTION sdss_get_use_emerg_rat_pri_list

DESCRIPTION
   Get the use_emerg_rat_pri_list flag

RETURN VALUE
  Boolean

===========================================================================*/
boolean  sdss_get_use_emerg_rat_pri_list(void);

/*===========================================================================

FUNCTION sdss_set_use_emerg_rat_pri_list

DESCRIPTION
  To set use_emerg_rat_pri_list flag.

RETURN VALUE
  None

===========================================================================*/
extern void  sdss_set_use_emerg_rat_pri_list(
   boolean  is_use_emerg_rat_pri_list
);
...
extern  sdss_pref_updated_e_type      sdss_pref_update(
  ...
  */
  ...
  /* Indicates whether we are in fullrat or not
  */
...
===========================================================================*/
extern  byte                  sdss_get_cdma_request_mode(
  sys_modem_as_id_e_type    asubs_id
  /* Subscription ID
  */
);
...
FUNCTION sdss_update_is_sub_reacted

DESCRIPTION 
    Update if the standby pref changed from single standby to dual standby or 
    from dual standby to single standby

RETURN VALUES
    none

===========================================================================*/
extern void sdss_update_is_sub_reacted (
  sys_modem_as_id_e_type  asubs_id,
  boolean is_sub_reacted
);

/*===========================================================================

FUNCTION sdss_get_is_sub_reacted
...
RETURN VALUES
  TRUE if Standby_pref has changed
  FALSE otherwise 
...
===========================================================================*/
extern boolean sdss_get_is_sub_reacted (
  sys_modem_as_id_e_type  asubs_id
);
...
===========================================================================*/
extern  sdss_subsc_e_type                       sdss_get_subsc_type(sys_modem_as_id_e_type asubs_id);


/*===========================================================================

FUNCTION sdss_set_active_subs

DESCRIPTION
  Sets the sd_active_subs with the new value.
...
===========================================================================*/
void sdss_set_active_subs(uint8 active_subs);
...
===========================================================================*/
extern  void                          sdss_set_flag_subs(
...
#define sdss_set_flag(ss,flag,val)  sdss_set_flag_subs(global_sd_asubs_id,ss,flag,val)
...
*===========================================================================*/
extern  boolean        sdss_get_cfg_full_srv_req_in_emerg( sys_modem_as_id_e_type asubs_id );

/*===========================================================================

FUNCTION sdss_get_cfg_psap_call_back_mandatory

DESCRIPTION
  Get the psap_call_back_mandatory configurations.

RETURN VALUE
  Boolean

*===========================================================================*/
extern  boolean  sdss_get_cfg_psap_call_back_mandatory(
  sys_modem_as_id_e_type asubs_id 
);
...
DESCRIPTION
  Return the FULL SRV REQ in EMERG ORIG EXT flag
...
*===========================================================================*/
extern  boolean              sdss_get_cfg_full_srv_req_in_emerg_ext( sys_modem_as_id_e_type asubs_id );
...
===========================================================================*/
extern  boolean                       sdss_is_flag_set_subs(
...
#define sdss_is_flag_set(ss,flag) sdss_is_flag_set_subs(global_sd_asubs_id,ss,flag)
...
FUNCTION sdss_set_non_acq_param_change

DESCRIPTION
  set non_acq_param_change flag
...
===========================================================================*/
EXTERN  void                       sdss_set_non_acq_param_change(
...
FUNCTION sdss_get_non_acq_param_change

DESCRIPTION
  get non_acq_param_change flag
...
===========================================================================*/
EXTERN boolean                       sdss_get_non_acq_param_change(
...
FUNCTION sdss_sr_list_map_subs
...
===========================================================================*/
EXTERN  sdsr_e_type             sdss_sr_list_map_subs(
...
===========================================================================*/
EXTERN dword sdss_calc_max_temp_avoid_timer_from_list(sdsr_e_type list, dword timer_val, multimode_stack_e_type ss);
...
FUNCTION sdss_emerg_call_full_srv_mcc_items_init
...
===========================================================================*/
EXTERN void sdss_emerg_call_full_srv_mcc_items_init( sys_modem_as_id_e_type asubs_id );


/*===========================================================================

FUNCTION sdss_current_mcc_match_with_efs
...
===========================================================================*/
EXTERN boolean sdss_current_mcc_match_with_efs (

  sys_mcc_type       current_mcc,
  /* Current camped MCCs*/

  sys_modem_as_id_e_type asubs_id
  /* Subs id on which the EFS needs to be checked */
...
===========================================================================*/
EXTERN void sdss_lbsr_mcc_items_init( sys_modem_as_id_e_type asubs_id );
...
DESCRIPTION
  ...
  Takes enum multimode_stack_e_type instead of sdss_s_type*
...
RETURN VALUE
  ...
  MM_STACK_ALL otherwise

SIDE EFFECTS
  None.

===========================================================================*/
extern  multimode_stack_e_type                  sdss_get_ss_for_mode(
...
extern uint8 sdss_num_sim_get(void);
...
sys_acq_status_e_type sdss_get_acq_status_only( multimode_stack_e_type ss );


sys_acq_status_e_type sdss_get_acq_status_subs( mm_sub_stk_id_s_type  mm_id);
...
===========================================================================*/
extern void sdss_clear_bsr_timer(multimode_stack_e_type ss);
...
===========================================================================*/
extern void sdss_check_bsr_timer(multimode_stack_e_type ss);
...
/*===========================================================================
FUNCTION sdss_gw_sim_domain_valid_per_sub

DESCRIPTION

  Checks whether SIM is valid for all selected domains for current sub.
  for SXLTE, consider srv domain pref for both MAIN and HYBR1/HYBR2
  for non-SXLTE, consider srv domain pref only for passed ss.
...
===========================================================================*/
EXTERN boolean sdss_gw_sim_domain_valid_per_sub(
...
===========================================================================*/
extern  sd_rat_acq_order_s_type               sdss_get_rat_acq_order(void);
...
FUNCTION sdss_is_mmss_operation_per_sub
...
===========================================================================*/
EXTERN  boolean                       sdss_is_mmss_operation_per_sub(
...
===========================================================================*/
extern  boolean            sdss_is_mmss_operation(void);
...
DESCRIPTION
  ...
  return value  SDSS_DFLT_NUM_RAT- rat is not present.
DEPENDENCIES
  None.
...
===========================================================================*/
boolean                    sdss_is_manual_search_supported_in_wrlf(void);
...
EXTERN  void                          sdss_set_emerg_bar_status_subs(
  sys_modem_as_id_e_type               asubs_id,
...
#define sdss_set_emerg_bar_status(emerg_bar_status) sdss_set_emerg_bar_status_subs(global_sd_asubs_id,emerg_bar_status)
...
FUNCTION sdss_is_lte_ims_voice_allowed

DESCRIPTION
  Return TRUE if current LTE service supports IMS voice
...
EXTERN  boolean                   sdss_is_lte_ims_voice_allowed
(
...
void    sdss_get_mode_band_pref(
  ...
  sd_ss_band_pref_e_type        *tds_band_pref
#ifdef FEATURE_LAPP
  , sys_nr5g_band_mask_e_type    *nr5g_band_pref
#endif
#ifdef FEATURE_NR5G_SA
  , sys_nr5g_band_mask_e_type    *nsa_nr5g_band_pref
#endif
...
FUNCTION sdss_get_full_mode_pref

DESCRIPTION
  Read the mode preference of the SS-Core.
...
===========================================================================*/
sd_ss_mode_pref_e_type    sdss_get_full_mode_pref(
  multimode_stack_e_type    stk_id
);
...
#ifdef FEATURE_NR5G_SA
/*===========================================================================
FUNCTION sdss_is_nr5g_ims_emerg_allowed

DESCRIPTION
  Return TRUE if nr5g support emergency
...
EXTERN  boolean                   sdss_is_nr5g_ims_emerg_allowed
(
  void
);
...
EXTERN  void sdss_set_eHRPD_zone(
  ...
  mm_sub_stk_id_s_type      mm_id
  /* mm_id - indicates the sub,stack which service is updated on */
...
===========================================================================*/
EXTERN void sdss_lte_avail_file_init(sys_modem_as_id_e_type asubs_id);
...
===========================================================================*/
EXTERN void sdss_lte_avail_file_save(sys_modem_as_id_e_type asubs_id);
...
FUNCTION sdss_set_fullrat_status

DESCRIPTION
  Update the fullrat status value.
...
===========================================================================*/
EXTERN  void                          sdss_set_fullrat_status(

  sd_ss_fullrat_pref_e_type fullrat_status_val
);

/*===========================================================================

FUNCTION sdss_get_rat_capability

DESCRIPTION
  Retrieve the current rat capability sent from CM.
...
===========================================================================*/
uint32 sdss_get_rat_capability(void);

/*===========================================================================

FUNCTION sdss_get_is_follow_tot

DESCRIPTION
  Retrieve the is_follow_tot value.
...
===========================================================================*/
boolean   sdss_get_is_follow_tot( void );

/*===========================================================================

FUNCTION sdss_follow_rat_acq_order

DESCRIPTION
  If current mode capability is 3GPP only and EFS is set,
  acquisition order will follow rat_acq_order even if mmss operation is true
...
===========================================================================*/
boolean sdss_follow_rat_acq_order(
  multimode_stack_e_type    stk_id
);
...
===========================================================================*/
EXTERN void sdss_print_capability_info(void);
...
===========================================================================*/
EXTERN  boolean                       sdss_is_lte_supp_for_csfb_call(  multimode_stack_e_type      ss );
...
FUNCTION sdss_get_avoid_emerg_bands_per_sub

DESCRIPTION
  To read the emerg dup bands to avoid per sub.
...
===========================================================================*/
EXTERN sdss_band_s_type sdss_get_avoid_emerg_bands_per_sub(
  mm_sub_stk_id_s_type     mm_id
  /* System selection sub
   */
);
/*===========================================================================
...
===========================================================================*/
extern boolean sdss_is_csg_id_defined (multimode_stack_e_type  ss);
...
===========================================================================*/
extern void sdss_set_csg_data(multimode_stack_e_type                   ss,
                              sys_csg_id_type                csg_id,
                              sys_radio_access_tech_e_type   csg_rat);
...
===========================================================================*/
extern void sdss_set_non_rplmn_csg_data(multimode_stack_e_type                   ss,
                                        sys_csg_id_type                csg_id,
                                        sys_radio_access_tech_e_type   csg_rat,
...
===========================================================================*/
extern void sdss_set_rplmn_csg_data(multimode_stack_e_type                   ss,
                                    sys_csg_id_type                csg_id,
                                    sys_radio_access_tech_e_type   csg_rat,
                                    sys_plmn_id_s_type             csg_plmn,
...
===========================================================================*/
extern void sdss_restore_csg_info_to_ss_on_wrlf(multimode_stack_e_type        ss,
...
===========================================================================*/
void sdss_clear_csg_info_on_ss(multimode_stack_e_type        ss,
...
===========================================================================*/
extern sys_plmn_id_s_type sdss_get_sid_plmn(multimode_stack_e_type ss);
...
===========================================================================*/
extern  boolean            sdss_is_opt_ho_lte_do( sys_modem_as_id_e_type asubs_id );
...
===========================================================================*/
void sdss_read_policyman_tot( uint8 sub_mask);

/*===========================================================================

FUNCTION sdss_is_manual_bst_valid

DESCRIPTION
  Determine if MANUAL BST is available and valid.

DEPENDENCIES
  None.

RETURN VALUE
  Return TRUE if MANUAL BST is available and valid.
...
===========================================================================*/
extern boolean sdss_is_manual_bst_valid(sys_modem_as_id_e_type asubs_id);
/*===========================================================================
...
===========================================================================*/
boolean sdss_is_ofmode_1x_sxlte(sys_modem_as_id_e_type asubs_id);
...
===========================================================================*/
boolean sdss_is_sfmode_svlte(sys_modem_as_id_e_type sub_idx);
...
===========================================================================*/
boolean sdss_is_sfmode_1x_sxlte(sys_modem_as_id_e_type sub_idx);
...
===========================================================================*/
boolean sdss_is_sfmode_1x_srlte(sys_modem_as_id_e_type sub_idx);
...
extern  void     sdss_set_avoid_list_updated_flag( sys_modem_as_id_e_type asubs_id, boolean is_avoid_list_updated );

/*===========================================================================

DESCRIPTION
  SETS  the avoidance list updated flag for all configured SUBs
...
extern  void     sdss_set_avoid_list_updated_flag_all_sub( boolean is_avoid_list_updated );
...
extern  boolean     sdss_get_avoid_list_updated_flag( sys_modem_as_id_e_type asubs_id );


/*===========================================================================

DESCRIPTION
  SETS  the avoidance plmn list flag for IMS end cause 380.
...
extern  void     sdss_set_avoid_plmn_list_flag( sys_modem_as_id_e_type asubs_id, boolean avoid_plmn_flag);

/*===========================================================================

DESCRIPTION
  Return the avoid plmn list flag
...
extern  boolean     sdss_get_avoid_plmn_list_flag( sys_modem_as_id_e_type asubs_id );
...
*===========================================================================*/
EXTERN  boolean              sdss_get_sv_dynamic_switch_enabled(
...
EXTERN  void                sdss_set_sv_dynamic_switch_enabled(
  ...
  sys_modem_as_id_e_type asubs_id
  /*sub id for which the SV dynamic switch feature is enabled.*/
);
...
*===========================================================================*/
EXTERN  boolean                sdss_allow_ltd_acq_fail_mapping(multimode_stack_e_type ss);
...
===========================================================================*/
extern  void               sdss_store_sv_opr_switch_lst( multimode_stack_e_type ss );
#endif
...
===========================================================================*/
extern void                 sdss_read_efs_qmss_enabled (sys_modem_as_id_e_type asubs_id);
...
FUNCTION sdss_read_efs_full_srv_req_in_emerg_ext()

DESCRIPTION
  Read the SDEFS_FULL_SRV_REQ_EMERG_EXT EFS.
  This EFS is used only for issuing full service req in emerg orig requirements at MMODE
  and no other optimization.
 < Keeping the value in separate field in case it nees to be used for some other optimization.>
...
===========================================================================*/
extern void sdss_read_efs_full_srv_req_in_emerg_ext(sys_modem_as_id_e_type asubs_id);
...
===========================================================================*/
extern void sdss_read_efs_emerg_call_after_lu_in_lpm ( sys_modem_as_id_e_type asubs_id );

/*===========================================================================

FUNCTION sdss_read_efs_psap_call_back_mandatory

DESCRIPTION
  Read the PSAP_CALL_BACK_MANDATORY EFS.

RETURN VALUE
  None

===========================================================================*/
extern void sdss_read_efs_psap_call_back_mandatory ( sys_modem_as_id_e_type asubs_id );


/*===========================================================================

FUNCTION sdss_read_efs_enable_fullrat_optimisation()

DESCRIPTION
  Read the sdss_read_efs_enable_fullrat_optimisation EFS.
...
===========================================================================*/
EXTERN void sdss_read_efs_enable_fullrat_optimisation ( void );
...
===========================================================================*/
extern  void sdss_read_efs_roam_ind_update_in_l_to_1x_ho ( sys_modem_as_id_e_type asubs_id );
...
FUNCTION sdss_read_create_psuedo_prl()

DESCRIPTION
  Read the psuedo PRL setting from NV,
  in case MMSS file in not present in CARD, we can create
  pusedo PRL file
...
===========================================================================*/
EXTERN void sdss_read_create_psuedo_prl(
  sys_modem_as_id_e_type asubs_id
);

/*===========================================================================

FUNCTION sdss_if_psuedo_prl_config_enable()

DESCRIPTION
  Return the NV read value

RETURN VALUE
  TRYE if psuedo PRL enable .
  FALSE if psuedo PRL disable

===========================================================================*/
EXTERN boolean sdss_if_psuedo_prl_config_enable(
  sys_modem_as_id_e_type asubs_id
);
...
FUNCTION sdss_read_efs_e911_prioritise_cs()

DESCRIPTION
  Read if the EFS item to prioritise CS RATs for E911 call is enabled.
...
===========================================================================*/
EXTERN void sdss_read_efs_e911_prioritise_cs( sys_modem_as_id_e_type asubs_id );

/*===========================================================================

FUNCTION sdss_is_e911_prioritise_cs()
...
===========================================================================*/
EXTERN boolean sdss_is_e911_prioritise_cs( sys_modem_as_id_e_type asubs_id );

/*===========================================================================

FUNCTION sdss_read_is_follow_tot()
...
===========================================================================*/
EXTERN void sdss_read_is_follow_tot (
  sys_modem_as_id_e_type asubs_id
);
...
===========================================================================*/
EXTERN void sdss_read_efs_extend_lte_bsr_during_1x_activity ( sys_modem_as_id_e_type asubs_id );
...
===========================================================================*/
EXTERN void sdss_read_ehrpd_opt_redial_enabled ( sys_modem_as_id_e_type asubs_id );
...
FUNCTION sdss_set_custom_emerg_info()

DESCRIPTION
  Set custom emerg info from CM
...
extern void sdss_set_custom_emerg_info(
...
===========================================================================*/
extern boolean sdss_is_ral_stored_in_efs(sys_modem_as_id_e_type asubs_id);
...
===========================================================================*/
extern boolean sdss_is_e911_alternate_redial_algorithm(sys_modem_as_id_e_type asubs_id);
...
===========================================================================*/
extern boolean sdss_is_wifi_e911_wwan_full_srv(sys_modem_as_id_e_type asubs_id);
...
FUNCTION sdss_read_em_call_acq_db()

DESCRIPTION
  Read em call acq DB feature is enabled or not
...
===========================================================================*/
extern void sdss_read_sub_scan_scope ( sys_modem_as_id_e_type as_id);
...
===========================================================================*/
extern void sdss_read_buffer_int_srv_lost ( sys_modem_as_id_e_type asubs_id );
...
===========================================================================*/
extern boolean sdss_is_skip_mmss_srv_req(multimode_stack_e_type  ss, sd_ss_act_s_type      *ss_act_ptr);
...
===========================================================================*/
extern boolean sdss_is_mru_3gpp(multimode_stack_e_type ss);
...
EXTERN void sdss_1xcsfb_ecbm_init(sys_modem_as_id_e_type asubs_id);
...
===========================================================================*/
extern multimode_stack_e_type sdss_is_other_ss_in_srv(sys_modem_as_id_e_type asubs_id);
...
===========================================================================*/
void  sdss_set_remove_gwl_on_bsr ( multimode_stack_e_type ss, boolean val );
...
extern boolean sdss_get_1xcsfb_call_end_opt_status(sys_modem_as_id_e_type asubs_id);

/*===========================================================================

FUNCTION sdss_get_1xcsfb_ecbm
...
extern boolean sdss_get_1xcsfb_ecbm(sys_modem_as_id_e_type asubs_id);
...
extern void sdss_1xcsfb_call_end_opt_init(sys_modem_as_id_e_type asubs_id);
...
FUNCTION sdss_special_scan_mcc_items_init
...
===========================================================================*/
void sdss_special_scan_mcc_items_init(void);
...
===========================================================================*/
boolean sdss_is_sfmode_sglte(sys_modem_as_id_e_type sub_idx);
...
===========================================================================*/
EXTERN  multimode_stack_e_type                  sdss_get_ss_for_user_mode_pref(
...
===========================================================================*/
extern boolean sdss_get_special_volte_e911_calls(sys_modem_as_id_e_type asubs_id);
...
===========================================================================*/
void sdss_set_is_special_volte_e911_calls(
...
===========================================================================*/
sys_call_mode_t sdss_get_call_mode( void );
...
===========================================================================*/
void sdss_set_call_mode(sys_modem_as_id_e_type asubs_id, sys_call_mode_t call_mode);
...
FUNCTION sdss_is_emerg_call_needs_orig_in_full_srv

DESCRIPTION
  Returns TRUE if Emergency call is originated and if the emerg call needs to be attempted only in Full service.
...
===========================================================================*/
EXTERN  boolean         sdss_is_emerg_call_needs_orig_in_full_srv(
...
FUNCTION sdss_is_emergency_psap_cb_mandatory

DESCRIPTION
  Returns TRUE if PSAP Callback is mandatory while originating EM call.
...
===========================================================================*/
  EXTERN  boolean sdss_is_emergency_psap_cb_mandatory(
  mm_sub_stk_id_s_type  mm_id
);

/*===========================================================================

FUNCTION sdss_update_camp_plmn
...
===========================================================================*/
EXTERN  void                  sdss_update_camp_plmn(

  mm_sub_stk_id_s_type     mm_id,
  /* System selection MM_ID.
  */

  sys_plmn_id_s_type camp_plmn
  /* New camp plmn.
  */
...
FUNCTION sdss_reset_camp_plmn

DESCRIPTION
   Updates the current camp plmn of the SS-Core.
...
===========================================================================*/
EXTERN  void                  sdss_reset_camp_plmn(
...
FUNCTION sdss_get_camp_plmn

DESCRIPTION
  Return the camp_plmn 
...
*===========================================================================*/
EXTERN  sys_plmn_id_s_type            sdss_get_camp_plmn( 
...
FUNCTION sdss_set_c2k_scan_opt_val

DESCRIPTION
  Set c2k_scan_opt_val to value passed.
...
===========================================================================*/
EXTERN void sdss_set_c2k_scan_opt_val(
  uint8  opt_val,
  sys_modem_as_id_e_type asubs_id
);
...
FUNCTION sdss_get_c2k_scan_opt_val

DESCRIPTION
  To get c2k_scan_opt_val
...
===========================================================================*/
EXTERN uint8 sdss_get_c2k_scan_opt_val(void);

/*===========================================================================

FUNCTION sdss_is_3gpp_srv_lost_c2k_scan_opt_set()

DESCRIPTION
  Retrieves the value for whether 1x scans need to be optimized when entering
  FULL RAT MODE, only when we are in WRLF or for ALL cases and indicates
  whether optimization is enabled or not.
  If returend value is not DISABLED, this API returns TRUE.
...
===========================================================================*/
EXTERN boolean sdss_is_3gpp_srv_lost_c2k_scan_opt_set(void);

/*===========================================================================

FUNCTION sdss_is_c2k_scan_opt_allowed

DESCRIPTION
  Returns if 3GPP2 scan optimization needed for FULL RAT cases.
  Pre-conditions are:
  1. PM returns the scan optimization value as POLICYMAN_3GPP2SCAN_OPT_WRLF and
     coonected mode flag is set or PM returns POLICYMAN_3GPP2SCAN_OPT_ALL.
  2. Configuration is SXLTE+G or SRLTE SS .
  3. MRU[0] is a 3GPP system.
...
RETURN VALUE
  TRUE, if all above conditions are met.
  FALSE, otherwise.
...
===========================================================================*/
EXTERN boolean sdss_is_c2k_scan_opt_allowed(multimode_stack_e_type ss);
...
===========================================================================*/
boolean sdss_get_pref_chg_in_ltd_reg(multimode_stack_e_type      ss);

/*===========================================================================

FUNCTION sdss_get_ims_emerg_call_state

DESCRIPTION
   Fetches the call state of volte e911 call
...
===========================================================================*/
EXTERN  sys_volte_call_state_e_type     sdss_get_ims_emerg_call_state(
...
FUNCTION sdss_set_ims_emerg_call_state

DESCRIPTION
   sets the call state of volte e911 call
...
===========================================================================*/
EXTERN  void     sdss_set_ims_emerg_call_state(

  mm_sub_stk_id_s_type   mm_id,
  /* System selection stack.
  */
  sys_volte_call_state_e_type   call_state
);


EXTERN boolean sdss_init_n_sims( void );
...
EXTERN void sdss_task_init(void);
...
boolean sdss_is_curr_sys_china(multimode_stack_e_type ss);

/*===========================================================================

FUNCTION sdss_set_global_ptr
...
===========================================================================*/
EXTERN void sdss_set_global_ptr(void);
...
FUNCTION sdss_is_emergency_call_orig_per_sub
...
===========================================================================*/
EXTERN  boolean                    sdss_is_emergency_call_orig_per_sub(
...
FUNCTION sdss_get_is_full_srv_req_during_emerg_per_sub
...
===========================================================================*/
EXTERN boolean            sdss_get_is_full_srv_req_during_emerg_per_sub(
...
FUNCTION sdss_set_is_1x_sub

DESCRIPTION
  Sets is_1x_sub per subs capability.
...
FUNCTION sdss_is_1x_sub

DESCRIPTION
  Returns if the sub is 1x sub.
...
FUNCTION sdss_set_timer_start_uptime

DESCRIPTION

  Sets the uptime for starting the OOS or BSR scan
...
extern void sdss_set_timer_start_uptime(multimode_stack_e_type ss);

/*===========================================================================

FUNCTION sdss_get_adjusted_oos_timer

DESCRIPTION
  Returns the new OOS timer value. it will be same as previous if there is no
  Cafi change.
...
RETURN VALUE
  Adjusted timer = New OOS timer value.
...
===========================================================================*/
extern   dword               sdss_get_adjusted_oos_timer(
  ...
  */
  ...
  dword                           oos_uptime
  /* Time when current OOS timer is due to expire
  */

);
/*===========================================================================

FUNCTION sdss_process_sensor_chg
...
extern boolean sdss_process_sensor_chg(dword *timer_ptr);

#endif// FEATURE_MMODE_CAFI

/*===========================================================================

FUNCTION sdss_is_non_lte_sr_and_skip_gwt_scan

DESCRIPTION
   Returns TRUE if GWT scan is skipped and system record doesnt contain LTE
...
===========================================================================*/
boolean sdss_is_non_lte_sr_and_skip_gwt_scan(
     mm_sub_stk_id_s_type   mm_id,
            /* System selection stack.
            */
     sdsr_s_type            *sr_ptr
...
FUNCTION sdss_is_3gpp_service_on_ss

DESCRIPTION
   Returns TRUE if 3gpp service is present on a given stack
...
===========================================================================*/
extern boolean sdss_is_3gpp_service_on_ss(mm_sub_stk_id_s_type mm_id);

/*===========================================================================

FUNCTION  sdss_set_suppress_no_srv_due_to_suspend

DESCRIPTION
  This is a utility function that sets is_suppress_no_srv_due_to_suspend flag
...
SIDE EFFECTS
  None.
===========================================================================*/
extern void sdss_set_suppress_no_srv_due_to_suspend(
  sys_modem_as_id_e_type asubs_id,
...
FUNCTION  sdss_get_suppress_no_srv_due_to_suspend

DESCRIPTION
  This is a utility function that gets is_suppress_no_srv_due_to_suspend flag
...
SIDE EFFECTS
  None.
===========================================================================*/
extern boolean sdss_get_suppress_no_srv_due_to_suspend(
	sys_modem_as_id_e_type asubs_id
);

/*===========================================================================

FUNCTION  sdss_set_pwr_up_reas

DESCRIPTION
  This is a utility function that sets the reason for the most recent user_pwr_up cmd
  received by SD.
...
SIDE EFFECTS
  None.
===========================================================================*/
extern void sdss_set_pwr_up_reas(
  sys_modem_as_id_e_type    asubs_id,

  sd_ss_pwr_up_reas_e_type  pwr_up_reas
);

/*===========================================================================

FUNCTION  sdss_get_pwr_up_reas

DESCRIPTION
  This is a utility function that gets the reason for the most recent user_pwr_up cmd
  received by SD.
...
RETURN VALUE
  sd_ss_pwr_up_reas_e_type - reason for the most recent user_pwr_up cmd received by SD.

SIDE EFFECTS
  None.
===========================================================================*/
extern sd_ss_pwr_up_reas_e_type sdss_get_pwr_up_reas(
  sys_modem_as_id_e_type asubs_id
);

/*===========================================================================

FUNCTION sdss_get_cxm_proc_acq_state

DESCRIPTION
   Returns sdss_cxm_proc_acq_state

DEPENDENCIES
  None.

RETURN VALUE
  ACQ_START or ACQ_END

SIDE EFFECTS
  None.

===========================================================================*/
extern sdss_cxm_proc_acq_state_e_type sdss_get_cxm_proc_acq_state(sys_modem_as_id_e_type    as_id);

/*===========================================================================

FUNCTION sdss_set_cxm_proc_acq_state

DESCRIPTION
   Sets sdss_cxm_proc_acq_state value to ACQ_START or ACQ_END
...
===========================================================================*/
void sdss_set_cxm_proc_acq_state(
    
  sys_modem_as_id_e_type    as_id,

  sdss_cxm_proc_acq_state_e_type    acq_state
);

/*===========================================================================

FUNCTION sdss_cxm_inform_acq_state_start

DESCRIPTION
   Informs ACQ_START to MCS if the below conditions are met

   1. ACQ_START is not informed previously
   2. Any stack is in truly ACQ_STATE i.e. ACQ not buffred at MMOC
...
===========================================================================*/
extern void sdss_cxm_inform_acq_state_start( mm_sub_stk_id_s_type mm_id );

/*===========================================================================

FUNCTION sdss_cxm_inform_acq_state_end

DESCRIPTION
   Informs ACQ_END to MCS if the below conditions are met
   
   1. ACQ_END is not informed previosly
   2. oprt_mode is SDSS_OPR_MODE_PWR_DOWN
   3. Or all the active stacks are in OPR/PWR_SAVE and no Manual search ongoing
...
===========================================================================*/
extern void sdss_cxm_inform_acq_state_end( mm_sub_stk_id_s_type mm_id );

/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

/* SD debug buffer related types
*/
#ifdef FEATURE_SD_DEBUG_BUFFER

#define SD_MAX_DEBUG_BUFFER_SIZE 50
#define SD_MAX_DEBUG_BUFFER_SCRIPTS_SIZE 20

typedef struct {

  /* The script, designated by group and index
  */
  sdss_evt_grp_e_type event_group;
  int16 scr_index;

} sd_dbg_buf_script_s_type;

typedef struct {

  /* Debug buffer array
  */
  sd_dbg_buf_script_s_type        *buf[SD_MAX_DEBUG_BUFFER_SCRIPTS_SIZE];

  /* Current index in debug buffer. At the time of crash this
  ** points to the latest entry.
  */
  int8                             idx;

} sd_scripts_buffer_s_type;


typedef struct {

  /* Sub/stack on which the event was triggered
  */
  mm_sub_stk_id_s_type  mm_id;
  ...
  */
  ...
  /* Event that triggered SD event processing
  */
  sdss_evt_e_type         event;
  ...
  /* Any scripts that were run, allocated dynamically
  */
  sd_scripts_buffer_s_type  *scripts;

  /* Return action, valid if SD returned an action in this script
  */
  sd_ss_act_e_type    act;

} sd_dbg_buf_entry_s_type;

typedef struct {

  /* Debug buffer array
  */
  sd_dbg_buf_entry_s_type          buf[SD_MAX_DEBUG_BUFFER_SIZE];
...
} sd_debug_buffer_s_type;

void sd_dbg_buf_init_dbg_buffer(void);

void sd_dbg_buf_add_script(
  sdss_evt_grp_e_type     event_group,
  int                     scr_index
);

void sd_dbg_buf_add_return_action(
  sd_ss_act_e_type        act
);

void sd_dbg_buf_add_entry(
  sys_modem_as_id_e_type  asubs_id,
  multimode_stack_e_type  stk_id,
  sdss_evt_e_type         event,
  sd_ss_orig_mode_e_type  orig_mode,
...
FUNCTION sdss_is_emerg_rat_acq_order_configured

DESCRIPTION
  Returns whether we need the support for emerg rat acq order for the EMERG Call
===========================================================================*/
boolean sdss_is_emerg_rat_acq_order_configured(sys_modem_as_id_e_type sub_idx);
#ifdef FEATURE_QSH_MDUMP
/*===========================================================================
FUNCTION sdss_qsh_mdump_collect
  
DESCRIPTION
  This function collects the mdump for sdss global variables
...
QSH_MDUMP_FN_ATTR
#endif
void sdss_qsh_mdump_collect();
#endif
/*===========================================================================
FUNCTION sdss_is_sub_multimode_capable
  
DESCRIPTION
  Indicates if the given sub is multimode caable or not
...
===========================================================================*/
boolean sdss_is_sub_multimode_capable(sys_modem_as_id_e_type asubs_id);

/*===========================================================================

FUNCTION  sdss_inform_lte_mode_chage

DESCRIPTION
  This is a utility function that informs clients LTE mode change
...
SIDE EFFECTS
  None.
===========================================================================*/
void sdss_inform_lte_mode_chage(mm_sub_stk_id_s_type   mm_id);

/*===========================================================================

FUNCTION sdss_is_mmss_nondds_tele_bsr_enabled

DESCRIPTION
  Returns if the sub is mmss nondds and telebsr is enabled for nondds.
...
===========================================================================*/
boolean    sdss_is_mmss_nondds_tele_bsr_enabled(void);

/*===========================================================================

FUNCTION sdss_is_in_lte_long_bsr

DESCRIPTION
  Returns TRUE if LTE long BSR timer is running on that stack
...
===========================================================================*/
EXTERN boolean sdss_is_in_lte_long_bsr(multimode_stack_e_type ss);

/*===========================================================================

FUNCTION sdss_set_is_lte_deact_dual_switch

DESCRIPTION
  Set the value for is_lte_deact_dual_switch flag on the SS.
...
===========================================================================*/
EXTERN void sdss_set_is_lte_deact_dual_switch(
...
FUNCTION sdss_get_is_lte_deact_dual_switch

DESCRIPTION
  Get the value for the is_lte_deact_dual_switch flag on the SS.
...
===========================================================================*/
EXTERN boolean sdss_get_is_lte_deact_dual_switch(
...
FUNCTION sdss_emg_reorder_rat_pri_list

DESCRIPTION
  Update the order of rat priority list.

RETURN VALUE
  None

===========================================================================*/
EXTERN void  sdss_emg_reorder_rat_pri_list(
  multimode_stack_e_type  ss,
  sd_ss_act_s_type       *ss_act_ptr
);
...
FUNCTION sdss_skip_limited_routine_for_emergency

DESCRIPTION
   check's if pure limited to be skipped for emergency orig
...
===========================================================================*/
EXTERN boolean sdss_skip_limited_routine_for_emergency(
...
FUNCTION sdss_reset_lte_connected_mode_per_sub
...
===========================================================================*/
EXTERN void             sdss_reset_lte_connected_mode_per_sub(
...
FUNCTION sdss_skip_pure_limited_for_emergency
...
===========================================================================*/
EXTERN boolean sdss_skip_pure_limited_for_emergency(
...
FUNCTION sdss_get_buffer_int_srv_lost

DESCRIPTION
  Get the buf_int_srv_lost flag.

===========================================================================*/
boolean sdss_get_buffer_int_srv_lost(void);

/*===========================================================================

FUNCTION sdss_is_pref_reason_per_sub
...
===========================================================================*/
EXTERN  boolean                       sdss_is_pref_reason_per_sub(
...
FUNCTION 
  sdss_is_skip_lte_during_csfb

DESCRIPTION
  Decide whether to set or reset bst_rat_acq_reqd for LTE in service
  request during csfb call setup. 

RETURN VALUE
  Returns FALSE if skip is not needed.

===========================================================================*/
boolean sdss_is_skip_lte_during_csfb(
  multimode_stack_e_type  ss,
  boolean                 is_full_net_sel_type
);
/*===========================================================================

FUNCTION sd_dbg_buf_get_buffer

DESCRIPTION
  Returns a pointer to the debug buffer specified by sub and stack.
...
sd_debug_buffer_s_type *sd_dbg_buf_get_dbg_buffer(void);
#endif

/*===========================================================================

FUNCTION sdss_get_pref_updated_status

DESCRIPTION
   Get the pref updated status of the SS-Core.

DEPENDENCIES
  None.

RETURN VALUE
  Return the pref updated status of the SS-Core.

SIDE EFFECTS
  None.

===========================================================================*/
EXTERN  sdss_pref_updated_e_type  sdss_get_pref_updated_status(
...
FUNCTION sdss_set_pref_updated_status

DESCRIPTION
   set the pref updated status of the SS-Core.
...
===========================================================================*/
EXTERN  void  sdss_set_pref_updated_status(
...
FUNCTION sdss_is_user_pref_mms_chg

DESCRIPTION
  Check if the user preference change is due to MMS procedure.

RETURN VALUE
  Boolean

===========================================================================*/
boolean  sdss_is_user_pref_mms_chg(
  sd_ss_pref_reas_e_type           pref_reason,
  sd_ss_pref_update_reason_e_type  pref_update_reason
);
#endif
