...
$Header: //components/rel/mmcp.mpss/7.9.0/mmode/coreapp/src/ui_base.c#1 $
...
#define UI_BASE_MAX_SIMS 2
#else
#define UI_BASE_MAX_SIMS 1
#endif

static boolean bAutoAnswer[UI_BASE_MAX_SIMS];
static boolean bThinUI_setup_resp_req[UI_BASE_MAX_SIMS];
...
#endif
  for(sim_idx = SYS_MODEM_AS_ID_1 ; sim_idx < ARR_SIZE(bAutoAnswer); sim_idx++)
  {
  //load the auto answer option from nv74
    if(ui_get_nv(NV_AUTO_ANSWER_I, &nvi, sim_idx) == NV_DONE_S)
  {
      bAutoAnswer[sim_idx] = nvi.auto_answer.enable;
      //Default is same as auto answer
      bThinUI_setup_resp_req[sim_idx] = bAutoAnswer[sim_idx];
    }
  }
  /* Read the TTY mode from NV409 */
  if(ui_get_nv(NV_TTY_I, &nvi, SYS_MODEM_AS_ID_1) == NV_DONE_S)
  {
    ui_tty_mode = nvi.tty;
  }
  #ifdef FEATURE_DUAL_SIM
  TUI_MSG_HIGH_3("Auto answer Sub0: %d, Auto answer Sub1: %d, TTY mode: %d", 
                 bAutoAnswer[0], 
                 bAutoAnswer[1], 
                 ui_tty_mode);
  #else
  TUI_MSG_HIGH_2("Auto answer Sub0: %d, TTY mode: %d",
                 bAutoAnswer[0], 
                 ui_tty_mode);
  #endif
  //Read "ThinUI setup_resp required"? from efs /mmode/thinui_setup_resp_req.txt
  {
    ...
    if( fd > 0 )
    {
      ...
      if(nSize  > 0 )
      {
        if(aData[0] == '1')
        {
          for(sim_idx = SYS_MODEM_AS_ID_1; sim_idx < ARR_SIZE(bThinUI_setup_resp_req); sim_idx++)
          {
            bThinUI_setup_resp_req[sim_idx]= TRUE;
          }
        }
      }
      /* Close the file */
...
)
{
  ...
  {
    if(ui_get_nv(NV_FTM_MODE_I, &nvi, SYS_MODEM_AS_ID_1) != NV_DONE_S)
    {
                     ...
                     )
{
  ...
  TUI_MSG_HIGH_2("Received a call event %d for subId %d", 
                 call_event, 
  ...
  switch (call_event)
  {
    case CM_CALL_EVENT_SETUP_IND:
    {
      ...
      if (call_info_ptr->call_type != CM_CALL_TYPE_VOICE)
        return;
      #ifdef FEATURE_DUAL_SIM
      TUI_MSG_HIGH_5("ThinUI setup_resp req sub0 %d sub1 %d, bAutoAnswer sub0 %d sub1 %d, ui_tty_mode = %d", 
                     bThinUI_setup_resp_req[0], 
                     bThinUI_setup_resp_req[1],
                     bAutoAnswer[0], 
                     bAutoAnswer[1], 
                     ui_tty_mode);
      #else
      TUI_MSG_HIGH_3("ThinUI setup_resp req %d, bAutoAnswer %d, ui_tty_mode = %d", 
                     bThinUI_setup_resp_req[0],
                     bAutoAnswer[0], 
                     ui_tty_mode);
      #endif
      ...
      if(call_info_ptr->asubs_id >= SYS_MODEM_AS_ID_1 && 
         call_info_ptr->asubs_id < UI_BASE_MAX_SIMS && 
         bThinUI_setup_resp_req[call_info_ptr->asubs_id] )
      {
    ...
    case CM_CALL_EVENT_INCOM:
    {
      ...
      if (call_info_ptr->asubs_id >= SYS_MODEM_AS_ID_1 && 
         call_info_ptr->asubs_id < UI_BASE_MAX_SIMS && 
         bAutoAnswer[call_info_ptr->asubs_id])
      {
...
)
{
  ...
  memset( &nvi_ext, 0x00, sizeof(nv_cmd_ext_type) );
...
/* Prepare the NV read command */
  ...
  nvi_ext.nvcmd = &nvi;
  nvi_ext.context = context;

  nv_cmd_ext(&nvi_ext);           /* Issue the request    */
