...
void csg_app_cm_trigger_search(boolean is_app_triggered, boolean send_config_update, boolean is_immediate_search)
{
  ...
  uint32 csg_id=0;
  boolean is_csg_search_allowed=FALSE;
  sys_csg_search_type_e_type csg_search_type=SYS_CSG_SEARCH_TYPE_APP_INIT;
  ...
  if(is_app_triggered == FALSE)
  {
    ...
    csg_search_type = SYS_CSG_SEARCH_TYPE_USER_INIT;
  }

  is_csg_search_allowed = mm_is_csg_app_search_allowed(csg_search_type,SYS_CSG_SELECTION_CONFIG_1,asubs_id);
  TUI_MSG_HIGH_1("Is NAS Allowing CSG search after OCSGL EF check=%d",is_csg_search_allowed);
         ...
         ) )
  {
    ...
    if((ui_base_csg_config_global.timer.is_valid && ui_base_csg_config_global.timer.interval > 0) || is_immediate_search == TRUE)
    {
      ...
      if (is_csg_search_allowed == FALSE)
      {
        TUI_MSG_HIGH("Skipping CSG search..as is_csg_search_allowed=FALSE");
      ...
      else if(csg_app_ss_evt_global.ss_info[asubs_id].stack_info[0].srv_status == SYS_SRV_STATUS_NO_SRV ||
              csg_app_ss_evt_global.ss_info[asubs_id].stack_info[0].srv_status == SYS_SRV_STATUS_NONE)
      {
        TUI_MSG_HIGH("Skipping search and running 1-min timer as srv_status=NO_SRV");
        if(ui_base_csg_config_global.timer.is_valid && ui_base_csg_config_global.timer.interval > 0)
        {
          ui_base_timer_reset((uint16)CSG_SEARCH_SHORTER_TIMER);
        }
      }
      else if(ui_base_csg_config_global.csg_search_select_config.search_mode == 1)
      {
