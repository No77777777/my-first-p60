...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/mmode/coreapp/src/ui_base_msgr_if.c#1 $ $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...
void ui_base_msgr_register( msgr_client_t *msgr_client )
{
  ...
  {
    err = msgr_register_variant( MSGR_MM_TUI, msgr_client, MSGR_ID_REX, QMI_NAS_CSG_SEARCH_SELECTION_CONFIG_CMD, i );
    if(err != E_SUCCESS)
    {
      TUI_MSG_HIGH_1("TUI : QMI_NAS_CSG_SEARCH_SELECTION_CONFIG_CMD register ret %d", err);
    }

    err = msgr_register_variant( MSGR_MM_TUI, msgr_client, MSGR_ID_REX, QMI_NAS_CSG_IMMEDIATE_SEARCH_SELECTION_CMD, i );
    if(err != E_SUCCESS)
    {
      TUI_MSG_HIGH_1("TUI : QMI_NAS_CSG_IMMEDIATE_SEARCH_SELECTION_CMD register ret %d", err);
    }

    err = msgr_register_variant( MSGR_MM_TUI, msgr_client, MSGR_ID_REX, MM_CM_CSG_SEARCH_TRIGGER_IND, i);
    if(err != E_SUCCESS)
    {
