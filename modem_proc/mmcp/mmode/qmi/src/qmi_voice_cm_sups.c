...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/mmode/qmi/src/qmi_voice_cm_sups.c#1 $ $DateTime:$ $Author:$
    ...
    qmi_voice_cm_if_set_sups_service_s *in_data_ptr)
{
  ...
  /*-----------------------------------------------------------------------*/
  QM_ASSERT( in_data_ptr != NULL );
  QM_ASSERT( params_ptr != NULL );
  /*-----------------------------------------------------------------------*/
...
/*=========================================================================
  FUNCTION:  qmi_voice_cm_if_set_all_call_fwd_sups_is_valid

===========================================================================*/
/*!
    @brief
    To check whether RIL_REQUEST_SET_ALL_CALL_FORWARD is received with valid
    parameters
...
*/
/*=========================================================================*/
boolean qmi_voice_cm_if_set_all_call_fwd_sups_is_valid(
    qmi_voice_cm_if_set_all_call_fwd_sups_req_s *in_data_ptr)
{
  ...
  if((in_data_ptr->service < (uint8)QMI_VOICE_CM_SUPS_SERVICE_ACTIVATE) ||
      (in_data_ptr->service >= (uint8)QMI_VOICE_CM_SUPS_SERVICE_MAX))
    return(FALSE);
  ...
  if(in_data_ptr->service == (uint8)QMI_VOICE_CM_SUPS_SERVICE_REGISTER)
  {
    if(!(in_data_ptr->fwd_number_valid))
    {
      QM_MSG_HIGH("Call FWD number should be present for CFW REGISTER");
  ...
  if((in_data_ptr->all_call_fwd_reason_valid == TRUE) &&
      ((in_data_ptr->all_call_fwd_reason != (uint8)QMI_VOICE_CM_SUPS_CCFC_REASON_ALLCALL) &&
       (in_data_ptr->all_call_fwd_reason != (uint8)QMI_VOICE_CM_SUPS_CCFC_REASON_ALLCOND)))
  {
    QM_MSG_HIGH_1("Invalid call forward reason %d", in_data_ptr->all_call_fwd_reason);
    return(FALSE);
  }
...
*/
/*=========================================================================*/
uint8  qmi_voice_cm_sups_get_cfw_ss_code(int reason)
{
