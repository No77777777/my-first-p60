...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/mmode/qmi/src/qmi_mmode_svc.c#1 $
...
===========================================================================*/
void qmi_mmode_svci_free_transaction
(
  qmi_transaction_type   **x_p
)
{
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  free_x = TRUE;
  for( i = 0; i < MIN(MAX_MSGS_IN_BUNDLE, n_cmds); i++ )
  {
...
)
{
  ...
  /*-------------------------------------------------------------------------*/
  QM_ASSERT(svc_cfg);
  QM_ASSERT(svc_cfg->svc_sp);
  QM_ASSERT(msg_hdr);
  QM_ASSERT(sdu_in);
  QM_ASSERT(msg_hdr->common_hdr.client_id != QMI_FRAMEWORK_SVC_MAX_CLIENTS);
  QM_ASSERT(cl_sp);
...
)
{
  ...
  if( FALSE == qmi_mmode_svci_queue_response( &cmd_buf_p, &msg_ptr ) )
  {
    ...
    QM_ASSERT (msg_ptr == NULL); // msg_ptr is freed in queue_response
    return FALSE;
  }

  QM_ASSERT (cmd_buf_p == NULL);
  QM_ASSERT (msg_ptr == NULL);
  ...
  for( i = 0; i < x_p->n_cmds; i++ )
  {
    QM_ASSERT( x_p->resp_list[i] != QMI_SVC_RESPONSE_PENDING );
