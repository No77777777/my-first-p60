...
{
  uint16 value = 0;
  uint8  nibble[3] = {0, 0, 0};            // nibble[0] least significant nibble, nibble[2] is most significant nibble of mcc/mnc
  uint8  i;
...
uint32 qm_util_map_cm_sys_mode_to_qm_bitmask( sys_sys_mode_e_type sys_mode )
{
  ...
  switch ( sys_mode )
  {
    ...
    default:
      QM_MSG_MED_1("Unsupported rat %d", sys_mode);
      break;
  }
...
)
{
  ...
  QM_MSG_MED("qm_util_read_efs: Reading from EFS");
...
)
{
  ...
  if ( data_len == 0 )
  {
    QM_MSG_MED( "Attempted to print 0 length buffer" );
    return;
  }

  out_buf = QM_MEM_ALLOC( QM_MAX_F3_BUF );
...
)
{
  void *ptr;
  void *caller_ptr;
  /*Calling with 0 will give the address of the caller of qm_mem_malloc_debug()*/
  caller_ptr = QM_MEM_ALLOC_CALLER_ADDRESS(0);
  ...
  if((ptr = modem_mem_alloc_setcaller(size, MODEM_MEM_CLIENT_QMI_MMODE, caller_ptr)) == NULL)
  {
    QM_ERR_FATAL_0("modem_mem_alloc() failed!!");
  }
  else
  {
    memset(ptr, 0, size);
    qm_modem_heap_blocks++;
  }
...
)
{
  ...
  */
  ...
  if(ptr != NULL)
  {
    ...
    modem_mem_free_ext(ptr, MODEM_MEM_CLIENT_QMI_MMODE, (char *)funcName, lineNumber);
    qm_modem_heap_blocks--;
  }
