...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/mmode/qmi/src/qmi_nas_internal.h#2 $
  $DateTime: 2022/09/01 04:52:39 $
  $Author: pwbldsvc $
...
#define SEARCH_PERIODICITY_MIN_VALUE 5
#define SEARCH_PERIODICITY_MAX_VALUE 60
#define INC_RESULTS_PERIODICITY_MAX_VALUE 145
#define MAX_SEARCH_TIME_MAX_VALUE 300
#define MAX_SEARCH_TIME_ONE_TIME_SEARCH_MAX_VALUE 255
...
typedef struct qmi_cm_ph_info_s
{
  ...
  **- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /** Bit mask of current disabled RATs. Mask of sys_sys_mode_e_type **/
...
//  uint8                      prev_nw_desc[QMI_NAS_SUBS_MAX][QMI_NAS_NET_DESC_LEN_MAX]; // last reported network description
  ...
  //Flag to check if PLMN is selected through qmi_nasi_initiate_network_register
  boolean     user_plmn_sel;
#endif
  ...
  struct ims_pref_s
  {
    boolean                      prev_reported_info_valid;
    nas_radio_if_enum_v01        prev_reported_rat;
    nas_call_type_mask_type_v01  prev_reported_call_mask;
  ...
  } ims_pref_cache[QMI_NAS_SUBS_MAX];
  ...
  qmi_cmd_buf_type *set_cell_config_req_buf_p;

  qmi_cmd_buf_type *gcell_info_cmd_buf_p;

  qmi_cmd_buf_type *get_negotiated_drx_cmd_buf_p;
  sys_modem_as_id_e_type get_negotiated_drx_asubs_id;

  qmi_cmd_buf_type *rpm_parameters_cmd_buf_p;

  qmi_cmd_buf_type *ecall_deregistration_cmd_buf_p;

  qmi_cmd_buf_type *ecall_timer_restart_cmd_buf_p;

  qmi_cmd_buf_type *b2_thresh_adj_config_req_buf_p;

  qmi_cmd_buf_type *get_arfcn_list_req_buf_p;

  qmi_cmd_buf_type *force_lte_scan_buf_p;
  sys_modem_as_id_e_type force_lte_scan_asubs_id;
  ...
  lte_rrc_ac_barring_info_s lte_rrc_ac_barring_info_cache[QMI_NAS_SUBS_MAX];
  ...
  qmi_cmd_buf_type *get_ssac_hys_timer;

  boolean                 lte_cphy_ca_cache_valid;
  lte_cphy_ca_event_ind_s lte_cphy_ca_cache;

  qmi_cmd_buf_type *set_lte_ue_cap_cfg_cmd_buf_p;

  boolean                  voice_int_roam_cache[QMI_NAS_SUBS_MAX];
  boolean                  voice_int_roam_cache_valid[QMI_NAS_SUBS_MAX];

  boolean                  voice_dom_roam_cache[QMI_NAS_SUBS_MAX];
  boolean                  voice_dom_roam_cache_valid[QMI_NAS_SUBS_MAX];

  boolean                  voice_roam_domestic_temp;
  boolean                  voice_roam_international_temp;
...
typedef struct qmi_nasi_client_state_s
{
  ...
  struct report_status_s
  {
    ...
    boolean     report_emergency_mode_status_ind;
    boolean     report_timer_expiry_ind;
    boolean     report_gcell_info_ind;
    boolean     report_lte_rach_fail_ind;
	boolean     report_sub_blocked_status_ind;
    boolean     report_e911_srch_fail_ind;
    boolean     report_cell_barring_info_ind;
    boolean     report_access_barring_status_ind;
  } report_status;
...
typedef struct
{
  ...
  union {
    ...
    eng_mode_all_frequency_list_T             geran_arfcn_list;
    wcdma_rrc_qmi_current_freqs_type wcdma_arfcn_list;
    lte_rrc_current_cells_s                   lte_arfcn_list;
    
  }list;
  
} qmi_nasi_current_rat_arfcn_list;
#endif
...
struct qmi_nas_cmd_hdr_payload
{
  ...
  hdrcp_mc_cb_params_rsp_union_type parms;
#endif
};
...
/*===========================================================================
  FUNCTION QMI_NAS_GENERATE_ME_BLOCKED_STATUS_IND()

  DESCRIPTION
    Called when ME bloced status changes
  ...
  SIDE EFFECTS
    None
===========================================================================*/
void qmi_nas_generate_sub_blocked_status_ind
(
  qmi_cm_ph_info_s_type  *p_info
);
