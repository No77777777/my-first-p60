...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/mmode/qmi/src/qmi_voice_call_list.c#7 $ $DateTime:$ $Author:$
...
)
{
  ...
  QM_MSG_MED_2( "Number of voice calls in call list (state mask %lu): %lu", state_mask, num_of_calls );
...
)
{
  ...
  QM_MSG_MED_1( "Number of voice calls in call list : %lu", num_of_calls );
...
static qmi_voice_call_list_buf_type *qmi_voice_call_list_new_buf( void )
{
  ...
  buf_ptr = QM_MEM_ALLOC(sizeof(qmi_voice_call_list_buf_type));
...
)
{
  uint8 conn_index;
  qmi_voice_call_list_buf_type *buf_ptr = NULL;
  qmi_voice_call_list_public_type *pub_ptr = NULL;
  ...
  if (buf_ptr != NULL)
  {
    ...
    QM_MSG_ERROR_4("CREATE_NEW_CALL: Fail to add call list entry:  call id=%d (duplicate), state %d, info %d, sub %d", 
                       call_id,
                       state,
                       mode_info_type,
                       as_id);
  ...
  /* Increment num_of_calls here after we are sure that the call will be added */
  ...
  pub_ptr->conf_is_enabled_available = FALSE;
  pub_ptr->is_silent_ui_available    =FALSE;
  pub_ptr->is_crs_info_available     = FALSE;
  pub_ptr->srv_opt = 0;
  ...
  pub_ptr->is_called_party_subaddress_available = FALSE;
  memset(&pub_ptr->called_party_subaddress,0,sizeof(pub_ptr->called_party_subaddress));
  ...
  pub_ptr->is_rtt_mode_available = FALSE;
  pub_ptr->rtt_mode = 0;
  pub_ptr->is_rtt_cap_available = FALSE;
  pub_ptr->rtt_cap.local_rtt_cap = CM_CALL_RTT_MODE_DISABLED;
  pub_ptr->rtt_cap.peer_rtt_cap = CM_CALL_RTT_MODE_DISABLED;
  pub_ptr->is_alternate_uri_available = FALSE;
  memset(&pub_ptr->alternate_uri,0,sizeof(pub_ptr->alternate_uri));
  pub_ptr->is_ip_alert_info_available = FALSE;
  memset(&pub_ptr->ip_alert_info, 0, sizeof(pub_ptr->ip_alert_info));
  pub_ptr->is_local_cap_available = FALSE;
  pub_ptr->is_peer_cap_available = FALSE;
  pub_ptr->is_prog_info_notif_available = FALSE;
  ...
  if((call_attrib.audio_attrib >= CM_CALL_ATTRIB_NONE) && 
  ...
  if((call_attrib.video_attrib >= CM_CALL_ATTRIB_NONE) && 
  ...
  if((call_attrib.attrib_status >= CM_CALL_ATTRIB_STATUS_OK) && 
    (call_attrib.attrib_status <= CM_CALL_ATTRIB_STATUS_MEDIA_NOT_READY))
  {
  ...
  if((call_attrib.rtt_mode >= CM_CALL_RTT_MODE_DISABLED) && (call_attrib.rtt_mode <= CM_CALL_RTT_MODE_FULL_MODE) )
  {
    pub_ptr->rtt_mode = (uint32)call_attrib.rtt_mode;
    pub_ptr->is_rtt_mode_available = TRUE;
  }
  ...
  QM_MSG_MED_8( "ALLOC_CALL: Added CL entry : state=%d, call id=%d,call type=%d, call mode=%d, dir=%d,mode_info_type=%d, line=%d, cm_num_len=%d,",
                 pub_ptr->state, pub_ptr->call_id,
                 pub_ptr->call_type, pub_ptr->call_mode, pub_ptr->direction,
                 pub_ptr->mode_info_type, pub_ptr->line, num.len);


  QM_MSG_MED_7( "ALLOC_CALL: num_len=%d, num_pres =%d,priv_mode=%d, name_len=%d, name_pres =%d,is_as_id_valid=%d, as_id=%d,",
                 pub_ptr->num.len, pub_ptr->num.pi, pub_ptr->privacy, pub_ptr->cnap_info.name_len, pub_ptr->cnap_info.name_presentation,
                 pub_ptr->is_as_id_valid, pub_ptr->asubs_id);

  if (mode_info_type == CM_CALL_MODE_INFO_IP)
  {
    QM_MSG_HIGH_4("ALLOC_CALL: call_id %d, info %d, state 0x%x, direction %d",
                      call_id, 
                      mode_info_type,
                      state,
                      direction);

    QM_MSG_HIGH_8("ALLOC_CALL: a_attrib_valid=%d, a_attrib=%d, v_attrib_valid=%d,v_attib=%d, attrib_status_valid=%d, attrib_status=%d rtt_mode_valid=%d rtt_mode=%d",
                   pub_ptr->is_audio_attrib_valid, 
                   pub_ptr->audio_attrib,
                   pub_ptr->is_video_attrib_valid,
                   pub_ptr->video_attrib,
                   pub_ptr->is_attrib_status_valid, 
                   pub_ptr->attrib_status, 
                   pub_ptr->is_rtt_mode_available, 
                   pub_ptr->rtt_mode);
  }
...
{
  QM_ASSERT( info_ptr != NULL );

  /*-----------------------------------------------------------------------*/

  QM_MSG_MED_6("Change in call, type %d, id %d, state %d -> %d, privacy %d, as_id %d",
               info_ptr->call_type, info_ptr->call_id,
               info_ptr->prev_state, info_ptr->state,
               info_ptr->privacy, info_ptr->asubs_id);
...
)
{
  ...
  qmi_voice_call_list_public_type *call_data = NULL;
  uint32 i;
  memset(&call_info_list,0,sizeof(qmi_voice_call_list_call_info_list_type));
  ...
  call_data = QM_MEM_ALLOC(sizeof(*call_data));
  if(NULL == call_data)
  {
    info->num_of_calls = 0;
    return;
  ...
  for (i=0; i<call_info_list.num_of_calls; i++)
  {
    ...
    if( (call_data->num.len > 0) || (call_data->num.pi != 0) )
    {
      ...
      if( (call_data->num.buf[0] != '+') && (call_data->num.number_type == (uint8)CM_NUM_TYPE_INTERNATIONAL) && (call_data->num.len > 0))
      {
        ...
        QM_MSG_MED_1("Call Info: International num, length %d",info->call_info[i].num.len);
      }
      else
      {
    ...
    if(call_data->is_emerg_srv_categ_available)
    {
      info->call_info[i].is_emerg_srv_categ_available =TRUE;
      info->call_info[i].emerg_srv_categ = call_data->emerg_srv_categ;
    }
    ...
    if(info->call_info[i].call_type == QMI_VOICE_CM_IF_CALL_TYPE_VOIP  ||
       info->call_info[i].call_type == QMI_VOICE_CM_IF_CALL_TYPE_VT)
    {
        ...
        QMI_VOICE_CM_IF_VOIP_CALL(info->call_info[i].call_type))
    {
      ...
      info->call_info[i].is_rtt_cap_available = TRUE;
      info->call_info[i].rtt_cap = call_data->rtt_cap;

      info->call_info[i].is_call_comp_cap_available = TRUE;
      info->call_info[i].call_comp_cap = call_data->call_comp_cap;
    ...
    QM_MSG_HIGH_4("fill_all_call_info  prog_info_avail %d ,notif_type %d , notif_reason %d, notif_text %d",
    call_data->is_prog_info_notif_available, call_data->prog_info.notif_type,call_data->prog_info.notif_reason,call_data->prog_info.notif_text );

    if(((call_data->state == QMI_VOICE_CALL_LIST_STATE_DIALING) ||
        (call_data->state ==QMI_VOICE_CALL_LIST_STATE_ALERTING)  )
     && 
       QMI_VOICE_CM_IF_VOIP_CALL(info->call_info[i].call_type) &&
       (call_data->is_prog_info_notif_available == TRUE))

     {
       qmi_voice_call_list_buf_type *buf_ptr; 
       info->call_info[i].is_prog_info_notif_available = TRUE;
       info->call_info[i].prog_info = call_data->prog_info;
       if ((buf_ptr = qmi_voice_call_list_find( call_info_list.call_id[i] )) != NULL)
       {
         buf_ptr->pub.is_prog_info_notif_available = FALSE; 
         QM_MSG_HIGH("prog_info_notif_available flag got reset");
       }
       QM_MSG_HIGH_3("update_fill_all_call_info prog_info_avail %d , notif_type %d , notif_reason %d",
            call_data->is_prog_info_notif_available,call_data->prog_info.notif_type, call_data->prog_info.notif_reason);
     }

    if(((call_data->state == QMI_VOICE_CALL_LIST_STATE_ALERTING) ||
        (call_data->state == QMI_VOICE_CALL_LIST_STATE_INCOMING) ||
       (call_data->state == QMI_VOICE_CALL_LIST_STATE_WAITING) ||
       (call_data->state == QMI_VOICE_CALL_LIST_STATE_PRE_INCOMING))
        &&
        QMI_VOICE_CM_IF_VOIP_CALL(info->call_info[i].call_type))
    {
      info->call_info[i].is_peer_cap_available = TRUE;
    ...
    QM_MSG_MED_4("Call Info: RTT_peer_cap %d, RTT_local_cap %d, peer_call_comp %d, local_call_comp %d",
                 info->call_info[i].rtt_cap.peer_rtt_cap,
                 info->call_info[i].rtt_cap.local_rtt_cap,
                 info->call_info[i].call_comp_cap.peer_call_comp_support,
                 info->call_info[i].call_comp_cap.local_call_comp_support);
    ...
    if((call_data->is_adnl_call_info_available==TRUE)
       && ((call_data->state == QMI_VOICE_CALL_LIST_STATE_INCOMING) ||
            (call_data->state == QMI_VOICE_CALL_LIST_STATE_WAITING)))
    {
    ...
    if (call_data->is_called_party_subaddress_available == TRUE)
    {
      info->call_info[i].is_called_party_subaddress_available = TRUE;
      info->call_info[i].called_party_subaddress = call_data->called_party_subaddress;
      QM_MSG_HIGH_4("Call Info: Called Party Subaddress extension_bit %d, subaddr_type %d, odd_even_ind %d, Called Party Subaddress len %d",
                    info->call_info[i].called_party_subaddress.extension_bit,
                    info->call_info[i].called_party_subaddress.subaddr_type,
                    info->call_info[i].called_party_subaddress.odd_even_ind,
                    info->call_info[i].called_party_subaddress.subaddr_len);
    }
    ...
    if(call_data->conf_is_enabled_available)
    {
      info->call_info[i].conf_is_enabled_available   = TRUE;
      info->call_info[i].conf_is_enabled      = (uint16)call_data->conf_is_enabled;
      QM_MSG_HIGH_1("Call Info: Is Conf Available %d", info->call_info[i].conf_is_enabled);
    }


    if(call_data->is_rtt_mode_available)
    {
      info->call_info[i].is_rtt_mode_available = TRUE;
      info->call_info[i].rtt_mode       = (uint32)call_data->rtt_mode;
      QM_MSG_HIGH_1("Call Info:RTT Mode %d", info->call_info[i].rtt_mode);
    }
    ...
    if(call_data->is_tir_mode_available && (call_data->state == QMI_VOICE_CALL_LIST_STATE_INCOMING || call_data->state == QMI_VOICE_CALL_LIST_STATE_WAITING))
   {
        info->call_info[i].is_tir_mode_available = TRUE;
        info->call_info[i].tir_mode = (uint32)call_data->tir_mode;
        QM_MSG_HIGH_1("Call info : TIR Mode %d", info->call_info[i].tir_mode);
        call_data->is_tir_mode_available = FALSE;
   }

    /* Only send ip_alert_info when call state is ALERTING/INCOMING/ACTIVE, and if it is an IP call */
    QM_MSG_HIGH_3("Call Info:ip_alert_info avail %d state = 0x%x call type %d", 
                      (call_data->is_ip_alert_info_available == TRUE),
                      call_data->state,
                      info->call_info[i].call_type);
    ...
    if (  ( (call_data->state == QMI_VOICE_CALL_LIST_STATE_ALERTING)||
          ...
          QMI_VOICE_CM_IF_VOIP_CALL(info->call_info[i].call_type) )
    {
      if(call_data->is_ip_alert_info_available == TRUE)
      {
        info->call_info[i].is_ip_alert_info_available = TRUE;
        info->call_info[i].ip_alert_info = call_data->ip_alert_info;
        QM_MSG_HIGH_1("Call Info:ip_alert_info Len %d", info->call_info[i].ip_alert_info.ip_alert_info_length);

        /* Reset the is_ip_alert_info_available flag after filling the current info buffer */
        call_data->is_ip_alert_info_available = FALSE;
      }
    }

    if(QMI_VOICE_CM_IF_VOIP_CALL(info->call_info[i].call_type))
    {
      QM_MSG_HIGH_3("is_crs_available %d , original_call_type %d, crs_type %d", call_data->is_crs_info_available,
        call_data->crs_info.original_call_type , call_data->crs_info.crs_type);
      if(call_data->is_crs_info_available == TRUE)
      {
       
        info->call_info[i].is_crs_info_available = TRUE;
        info->call_info[i].crs_info.original_call_type = 
			qmi_voice_cm_util_map_cm_call_type_mask_to_qmi_call_type(call_data->crs_info.original_call_type, call_data->mode_info_type);
        info->call_info[i].crs_info.crs_type = call_data->crs_info.crs_type;
        QM_MSG_HIGH_2("Call Info:crs_info crs_type %d , original_call_type %d", info->call_info[i].crs_info.crs_type, info->call_info[i].crs_info.original_call_type);
	...
	if(QMI_VOICE_CM_IF_VOIP_CALL(info->call_info[i].call_type))
	{
	  if(call_data->is_silent_ui_available == TRUE)
	  {
	    info->call_info[i].is_silent_ui_available = TRUE;
	    info->call_info[i].silent_ui = call_data->silent_ui;
	    QM_MSG_HIGH_1("Call Info:silent_ui%d", info->call_info[i].silent_ui);

	  }
	}

    #if defined(FEATURE_HA_MAINLINE_ONLY_ENABLE)
    if (call_data->state == QMI_VOICE_CALL_LIST_STATE_PRE_INCOMING)
    {
      /* Only send call composer importance when call state is pre incoming state */
      QM_MSG_HIGH_7("CALL_COMPOSER: fill all call info, call_id %d, state %d, imp avail %d subject avail %d, url avail %d, loc incl %d, shape %d",
                       info->call_info[i].call_id,
                       call_data->state, 
                       call_data->is_call_composer_importance_available,
                       call_data->is_call_composer_subject_available,
                       call_data->is_call_composer_picture_url_available,
                       call_data->call_composer_location.location_included,
                       call_data->call_composer_location.geo_shape);

      if (call_data->is_call_composer_importance_available == TRUE)
      {
        info->call_info[i].is_call_composer_importance_available = TRUE;
        info->call_info[i].call_composer_importance = call_data->call_composer_importance;
      }

      if (call_data->is_call_composer_subject_available == TRUE)
      {
        info->call_info[i].is_call_composer_subject_valid = TRUE;
        info->call_info[i].call_composer_subject = call_data->call_composer_subject;
      }

      if (call_data->is_call_composer_picture_url_available == TRUE)
      {
        info->call_info[i].is_call_composer_picture_url_valid = TRUE;
        info->call_info[i].call_composer_picture_url = call_data->call_composer_picture_url;
      }

      if (call_data->call_composer_location.location_included)
      {
        info->call_info[i].call_composer_location = call_data->call_composer_location;
      }

      if (call_data->is_call_composer_org_header_available == TRUE)
      {
        QM_MSG_HIGH_1("is_call_composer_org_header_available %d",call_data->is_call_composer_org_header_available);
        info->call_info[i].is_call_composer_org_header_valid = TRUE;
        info->call_info[i].call_composer_org_header = call_data->call_composer_org_header;
      }
...
{
  qmi_voice_cm_if_all_call_status_ind_s  *call_ind = NULL;

  call_ind = QM_MEM_ALLOC(sizeof(*call_ind));
...
)
{
  ...
  qmi_voice_call_list_public_type *info= NULL;
  ...
  /*-----------------------------------------------------------------------*/
  QM_MSG_MED_3("Call with call_id(%d) and mode_info(%d), state 0x%x will be added/replaced", 
                  call_id, 
                  mode_info_type,
                  call_state);

  info = QM_MEM_ALLOC(sizeof(*info));
  if(NULL == info)
  {
    return E_FAILURE;
  }
  ...
  else if ( call_state == QMI_VOICE_CALL_LIST_STATE_INCOMING )
  {
    ...
    if ( ( ( call_type == CM_CALL_TYPE_VOICE ) ||
        ...
        &&
       ( qmi_voice_call_list_count_num_of_voice_calls_by_state_and_mode( 
           (uint16)(QMI_VOICE_CALL_LIST_STATE_ACTIVE | QMI_VOICE_CALL_LIST_STATE_HOLDING), 
           mode_info_type, asubs_id) > 0 )
       )
    {
      state = QMI_VOICE_CALL_LIST_STATE_WAITING;
      
      QM_MSG_HIGH_3( "call id %d call state %d -> state %d",
                      call_id, call_state,state );
    }
  }
  else if ( call_state == QMI_VOICE_CALL_LIST_STATE_PRE_INCOMING)
  {
    state = call_state;
    QM_MSG_HIGH_2("CALL_COMPOSER: create a new call id %d, call_state 0x%x",
                      call_id, 
                      call_state);
  }
  else
  {
    /* Invalid state */
    QM_MSG_MED_2("Fail to add call list entry : call id %d (invalid call state %d)",
                 call_id, call_state);
  ...
  {
    status = qmi_voice_call_list_new_all( 
               call_id, call_type, call_mode, direction, state, line, num,
...
{
  qmi_voice_call_list_buf_type *buf_ptr= NULL;
  qmi_voice_call_list_buf_type *next_buf_ptr= NULL;

  /*-----------------------------------------------------------------------*/

  QM_MSG_MED("qmi_voice_call_list_free_all()");
...
{
  IxErrnoType status = E_SUCCESS;
  qmi_voice_call_list_buf_type *buf_ptr= NULL, *wait_buf_ptr = NULL,*wait_buf_ptr_no_active = NULL;
  qmi_voice_call_list_buf_type *prev_buf_ptr= NULL, *next_buf_ptr = NULL;
  uint32 current_num_of_active_voice_calls =0;
...
)
{
  ...
  QM_ASSERT( active_calls_list_ptr != NULL );
  ...
  while ( buf_ptr != NULL )
  {
         ...
         ( buf_ptr->pub.call_type == CM_CALL_TYPE_EMERGENCY ) )
    {
      ...
      if ( buf_ptr->pub.state == QMI_VOICE_CALL_LIST_STATE_ACTIVE && ((!buf_ptr->pub.is_as_id_valid) || (as_id == buf_ptr->pub.asubs_id)))
      {
        ...
        QM_MSG_MED_1("call id:  %d state:  Holding", buf_ptr->pub.call_id);
      }
    }
  ...
  for ( i = 0; i < active_calls_list_ptr->size; i++ )
  {
    ...
    {
      buf_ptr->pub.state = QMI_VOICE_CALL_LIST_STATE_ACTIVE;
      QM_MSG_MED_1("call id:  %d state:  Active", buf_ptr->pub.call_id);
    }
  }
...
)
{
  ...
  QM_ASSERT( call_ids_list_ptr != NULL );
  ...
  while ( buf_ptr != NULL )
  {
         ...
         ( buf_ptr->pub.state & state_mask ) )
    {
      ...
      QM_MSG_MED_3("call list entry with call id %d, state %d, answered %d",
                   buf_ptr->pub.call_id, buf_ptr->pub.state, buf_ptr->pub.answered);
    }
...
)
{
  ...
  if ( buf_ptr != NULL )
  {
    ...
    {
      /* Log call list changes */
      QM_MSG_MED_2("Change in call state of call list entry :-- call id %d, state %d",
                   call_id, buf_ptr->pub.state);
      ...
      /* If in MPTY, state should be WAITING */
      if ( (qmi_voice_call_list_count_num_of_voice_calls_by_state_and_mode( 
             (uint16)(QMI_VOICE_CALL_LIST_STATE_ACTIVE | QMI_VOICE_CALL_LIST_STATE_HOLDING),
             buf_ptr->pub.mode_info_type, 
             as_id ) > 0 ) 
      )
      {
        buf_ptr->pub.state = QMI_VOICE_CALL_LIST_STATE_WAITING;
...
)
{
  ...
  if ( buf_ptr != NULL )
  {
    ...
    {
      /* Log call list changes */
      QM_MSG_MED_3("Change in call state of call list entry : call id %d, num changed, old_len %d, new_len %d",
                    call_id, buf_ptr->pub.num.len, number->len);
    }
...
)
{
  ...
  if ( buf_ptr != NULL )
  {
    ...
    {
      /* Invalid state */
      QM_MSG_MED_2("Fail to replace CList entry : call id %d (invalid call state %d)",
                     call_id, call_state);
      return E_NOT_ALLOWED;
    }

    QM_MSG_MED_2("Found CList entry to be replaced: state %d, call id %d",
                   buf_ptr->pub.state, buf_ptr->pub.call_id );
    ...
    */
    ...
    /* Log CList changes */
    qmi_voice_call_list_log( &buf_ptr->pub );
    QM_MSG_MED_3("Replaced CList entry: state %d, call id %d, mode_info_type %d",
                   buf_ptr->pub.state, buf_ptr->pub.call_id, buf_ptr->pub.mode_info_type);
...
)
{
  ...
  if ( (buf_ptr != NULL) && (call_event_user_data != NULL) )
  {
    ...
    /* Log Call List changes */
    QM_MSG_MED_6("Change in call state of CList entry : call id %d, len %d, type %d, dcs %d, more_data_ind %d, reciver_busy %d",
                  call_id, buf_ptr->pub.uus_data.mt_user_data.user_user_data_length,
                  buf_ptr->pub.uus_data.mt_user_data.user_user_type, buf_ptr->pub.uus_data.mt_user_data.user_user_protocol_disc,
...
/*===========================================================================
  ...
  FUNCTION:  qmi_voice_call_list_update_call_type_voice_vt

===========================================================================*/
/*!
    @brief
    Update call type between voice vt
...
)
{
  ...
  {
    if ( buf_ptr->pub.call_type != call_type &&
		 (buf_ptr->pub.call_type == CM_CALL_TYPE_VOICE ||
		  buf_ptr->pub.call_type == CM_CALL_TYPE_VT) &&
		 (call_type == CM_CALL_TYPE_VOICE ||
		  call_type == CM_CALL_TYPE_VT)
		)
...
/*===========================================================================
  ...
  FUNCTION:  qmi_voice_call_list_update_num_si
...
)
{
  ...
  if ( buf_ptr != NULL )
  {
    if ( buf_ptr->pub.num.si != nsi )
    {
      /* Log call list changes */
      QM_MSG_MED_3("Change in num si of call list entry :  call id %d, num.si %d -> %d", call_id, buf_ptr->pub.num.si, nsi);

      buf_ptr->pub.num.si = nsi;
    }
    else
    {
      status = E_FAILURE;
  ...
  return status;
} /* qmi_voice_call_list_update_num_si() */

/*===========================================================================

  FUNCTION:  qmi_voice_call_list_update_num_plan
...
)
{
  ...
  if ( buf_ptr != NULL )
  {
    if ( buf_ptr->pub.num.number_plan != nplan )
    {
      /* Log call list changes */
      QM_MSG_MED_3("Change in num plan of call list entry :  call id %d, num.plan %d -> %d", call_id, buf_ptr->pub.num.number_plan, nplan);

      buf_ptr->pub.num.number_plan = nplan;
    }
    else
    {
      status = E_FAILURE;
  ...
  return status;
} /* qmi_voice_call_list_update_num_plan() */
...
/*===========================================================================
  ...
  FUNCTION:  qmi_voice_call_list_update_emerg_srv_categ

===========================================================================*/
/*!
    @brief
    updated the emergency service category received from CM
...
)
{
  ...
  if ( buf_ptr != NULL )
  {
    buf_ptr->pub.is_emerg_srv_categ_available = TRUE;
    buf_ptr->pub.emerg_srv_categ = emerg_srv_categ;
  }
  else
  {
    /* Call not in the list */
...
)
{
  ...
  if ( buf_ptr != NULL )
  {
    ...
    QM_MEM_FREE(name_info.caller_name);
  }
  else
  {
    /* Call not in the list */
...
)
{
  ...
  if ( buf_ptr != NULL )
  {
    pub_ptr = &buf_ptr->pub;
    *info_ptr = *pub_ptr;
    QM_MSG_MED_2("Found CList entry : call id %d, state %d",
                   buf_ptr->pub.call_id, buf_ptr->pub.state);
  }
  else
  {
    /* Call id not in the list */
...
)
{
  ...
  if ( buf_ptr != NULL )
  {
    *info_ptr = buf_ptr->pub;
    QM_MSG_MED_3("Found Voice Call List entry : conn index %u, call id %d, state %d",
                 conn_index, buf_ptr->pub.call_id, buf_ptr->pub.state);
  }
  else
  {
...
)
{
  ...
  if ( buf_ptr != NULL )
  {
    pub_ptr = &buf_ptr->pub;
    *info_ptr = *pub_ptr;
    QM_MSG_MED_3("Found Call List entry : conn index %u, call id %d, state %d",
                 conn_index, buf_ptr->pub.call_id, buf_ptr->pub.state);
  }
  else
  {
...
)
{
  ...
  if ( buf_ptr != NULL )
  {
    ...
    {
      /* Log CList changes */
      QM_MSG_MED_3("Change in number info of CList entry : call id %d, num len=%d->%d",
                   call_id, buf_ptr->pub.num.len, num_info->len);
...
)
{
  ...
  if ( buf_ptr != NULL )
  {
    ...
    {
      /* Log CList changes */
      QM_MSG_MED_3("Update in connected number info of CList entry : call id %d, num len=%d->%d",
                   call_id, buf_ptr->pub.conn_num_info.len, num_info->len);
...
)
{
  ...
  buf_ptr = qmi_voice_call_list_find( call_id );
    ...
    /* Log CList changes */
    QM_MSG_HIGH_3( "Debug4: MT_unwanted_call info: call id %d, nw_support_unwanted_call -> %d, mt_call_verstat -> %d",
...
/*===========================================================================
  ...
  FUNCTION:  qmi_voice_call_list_update_conf_capability

===========================================================================*/
/*!
    @brief
    updating the conference capability infoof the remote party. This information is recieved
    from CM with the call event "CM_CALL_EVENT_CONNECT"
...
)
{
  ...
  {
    /* Log CList changes */
    QM_MSG_HIGH_2( "is conf enabled  : call id %d, conf_is_enabled -> %d",
                   call_id, conf_is_enabled);

    buf_ptr->pub.conf_is_enabled_available = TRUE;
    buf_ptr->pub.conf_is_enabled = conf_is_enabled;
...
} /* qmi_voice_call_list_update_conf_capability() */
...
/*===========================================================================
  ...
  FUNCTION:  qmi_voice_call_list_update_called_party_subaddress

===========================================================================*/
/*!
    @brief
    Update Called Party Subaddress of the entry in the list with given Call ID - GW
...
)
{
  ...
  if (buf_ptr != NULL)
  {
    if (cm_called_party_subaddress_ptr != NULL)
    {
      QM_MSG_MED_1("update_called_party_subaddress(): input length = %d", cm_called_party_subaddress_ptr->length);
      if(cm_called_party_subaddress_ptr->length > 0)
      {
        buf_ptr->pub.is_called_party_subaddress_available = TRUE;

        /* Octate 3 first bit (from MSB) is copied in extension bit */
        buf_ptr->pub.called_party_subaddress.extension_bit =
          (boolean)((cm_called_party_subaddress_ptr->data[0] & 0x80) >> 7);

        /*
        ** Octate 3 second, third and fourth bit (from MSB) is copied
        ** in subadd_type
        */
        buf_ptr->pub.called_party_subaddress.subaddr_type =
          ((cm_called_party_subaddress_ptr->data[0] & 0x70) >> 4);

        /* Octate 3 fifth bit (from MSB) is copied in odd_even_ind */
        buf_ptr->pub.called_party_subaddress.odd_even_ind =
          ((cm_called_party_subaddress_ptr->data[0] & 0x08) >> 3);

        buf_ptr->pub.called_party_subaddress.subaddr_len = memscpy( buf_ptr->pub.called_party_subaddress.subaddr,
            sizeof(buf_ptr->pub.called_party_subaddress.subaddr),
            &(cm_called_party_subaddress_ptr->data[1]),
            ((uint8)(MIN(
                       (sizeof(cm_called_party_subaddress_ptr->data)-sizeof(cm_called_party_subaddress_ptr->data[0])),
                       (cm_called_party_subaddress_ptr->length) - 1))) );
        QM_MSG_MED_4("update_called_party_subaddress(): extension_bit %d, subaddr_type %d, odd_even_ind %d, subaddr_len = %d",
                     buf_ptr->pub.called_party_subaddress.extension_bit,
                     buf_ptr->pub.called_party_subaddress.subaddr_type,
                     buf_ptr->pub.called_party_subaddress.odd_even_ind, buf_ptr->pub.called_party_subaddress.subaddr_len);


      }
      else
      {
        QM_MSG_MED("update_called_party_subaddress(): cm_called_party_subaddress_ptr->length is 0");
        buf_ptr->pub.called_party_subaddress.subaddr_len = 0;
      }
    }
    else
    {
      QM_MSG_ERROR_1("update_called_party_subaddress(): cm_called_party_subaddress_ptr is NULL for call_id %d", call_id);
      status = E_FAILURE;
    }
  }
  else
  {
    /* Call not in the list */
    QM_MSG_ERROR_1("update_called_party_subaddress(): buf_ptr is NULL for call_id %d", call_id);
    status = E_FAILURE;
  }

  return status;

} /* qmi_voice_call_list_update_called_party_subaddress() */
...
)
{
  ...
  qmi_voice_call_list_buf_type *buf_ptr = NULL;
  cm_num_s_type num_info;
  memset(&num_info,0,sizeof(cm_num_s_type));
  ...
  if ( buf_ptr != NULL )
  {
    if ( bcd_num->present )
    {
      ...
      QM_MSG_MED_3("Change in number info of CList entry for called party number: call id %d, num len=%d->%d",
                   call_id, buf_ptr->pub.called_party_num_info.len, num_info.len);
...
)
{
  ...
  if ( buf_ptr != NULL )
  {
    if ( num->len > 0)
    {
      /* Log CList changes */
      QM_MSG_MED_3("Change in number info of CList entry : call id %d, num len=%d->%d",
                     call_id, buf_ptr->pub.redirecting_party_num_info.len, num->len);
...
)
{
  ...
  if ( buf_ptr != NULL )
  {
    ...
    if ( ((uint8)call_attrib_info.rtt_mode!= buf_ptr->pub.rtt_mode) &&
         (call_attrib_info.rtt_mode >= CM_CALL_RTT_MODE_DISABLED) &&
         (call_attrib_info.rtt_mode <= CM_CALL_RTT_MODE_FULL_MODE) )
    {
      /* Log CList changes */
      QM_MSG_MED_3("Update in RTT MODE of CList entry : call id %d, RTT MODE %d -> %d",
                   call_id, buf_ptr->pub.rtt_mode, call_attrib_info.rtt_mode);

      buf_ptr->pub.rtt_mode = (uint32)call_attrib_info.rtt_mode;
      buf_ptr->pub.is_rtt_mode_available = TRUE;
    }
    else
    {
      QM_MSG_MED_2("No update made, received :  call id %d, RTT MODE %d",
                   call_id, call_attrib_info.rtt_mode);
    }
  }
  else
  {
...
)
{
  ...
  if ( buf_ptr != NULL )
  {
    ...
    QM_MSG_MED_3("Update in RTT peer cap of CList entry : call id %d, RTT PEER CAP %d -> %d",
                 call_id, buf_ptr->pub.rtt_cap.peer_rtt_cap, peer_attr.rtt_mode);

    buf_ptr->pub.is_rtt_cap_available = TRUE;
    buf_ptr->pub.rtt_cap.peer_rtt_cap = (uint32) peer_attr.rtt_mode;

    if (!buf_ptr->pub.is_call_comp_cap_available ||
       buf_ptr->pub.is_call_comp_cap_available != peer_attr.call_composer_supported
    )
    {
      QM_MSG_HIGH_4("CALL_COMPOSER: Update call id %d, avail %d, PEER CAP %d -> %d",
                       call_id, 
                       buf_ptr->pub.is_call_comp_cap_available,
                       buf_ptr->pub.call_comp_cap.peer_call_comp_support,
                       peer_attr.call_composer_supported);
    }
    #if defined(FEATURE_HA_MAINLINE_ONLY_ENABLE)
    buf_ptr->pub.is_call_comp_cap_available = TRUE;
    buf_ptr->pub.call_comp_cap.peer_call_comp_support = peer_attr.call_composer_supported;
    #else
    buf_ptr->pub.is_call_comp_cap_available = FALSE;
    buf_ptr->pub.call_comp_cap.peer_call_comp_support = FALSE;
    #endif
...
/*===========================================================================
  ...
  FUNCTION:  qmi_voice_call_list_update_prog_info_notif

===========================================================================*/
/*!
    @brief
    Update the prog info notif for the call
...
)
{
  ...
  if(buf_ptr != NULL)
  {
    ...
    if((buf_ptr->pub.prog_info.notif_type != (uint64)prog_info.notif_type) ||
        (buf_ptr->pub.prog_info.notif_reason != prog_info.notif_reason) ||
        (memcmp(buf_ptr->pub.prog_info.notif_text, prog_info.notif_text, sizeof(prog_info.notif_text))!=0))
    {
      buf_ptr->pub.prog_info.notif_type         = (uint64) prog_info.notif_type;
      buf_ptr->pub.prog_info.notif_reason       = prog_info.notif_reason;
      memscpy(buf_ptr->pub.prog_info.notif_text, sizeof(buf_ptr->pub.prog_info.notif_text),
             prog_info.notif_text, sizeof(prog_info.notif_text));
      buf_ptr->pub.is_prog_info_notif_available = TRUE;
      status = E_SUCCESS;
      QM_MSG_HIGH_5("CALL_PROG_INFO update success: Update call id %d, avail %d, notif_type %d , notif_reason %d notif_text %d",
                 call_id, 
                 buf_ptr->pub.is_prog_info_notif_available,
                 buf_ptr->pub.prog_info.notif_type,
                 buf_ptr->pub.prog_info.notif_reason,
                 buf_ptr->pub.prog_info.notif_text);
    ...
    {
     QM_MSG_HIGH_5("CALL_PROG_INFO no update: Update call id %d, avail %d, notif_type %d , notif_reason %d notif_text %d",
                call_id, 
                buf_ptr->pub.is_prog_info_notif_available,
                buf_ptr->pub.prog_info.notif_type,
                buf_ptr->pub.prog_info.notif_reason,
    ...
    if(buf_ptr->pub.prog_info.notif_type == (uint64)CM_CALL_PROG_INFO_NOTIF_TYPE_NONE)
    {
      buf_ptr->pub.is_prog_info_notif_available = FALSE;
      status = E_FAILURE;
      QM_MSG_HIGH("reset prog_info_available as notif_type is none");
    }
...
/*===========================================================================
  ...
  FUNCTION:  qmi_voice_call_list_update_peer_call_comp_capabilities

===========================================================================*/
/*!
    @brief
    Update the peer call composer capabilities for the call
...
)
{
  ...
  {
    if (!buf_ptr->pub.is_call_comp_cap_available ||
       buf_ptr->pub.is_call_comp_cap_available != peer_call_comp_cap
    )
    {
      QM_MSG_HIGH_4("CALL_COMPOSER: Update call id %d, avail %d, PEER CAP %d -> %d",
                       call_id, 
    ...
    #if defined(FEATURE_HA_MAINLINE_ONLY_ENABLE)
    buf_ptr->pub.is_call_comp_cap_available = TRUE;
    buf_ptr->pub.call_comp_cap.peer_call_comp_support = peer_call_comp_cap;
    #else
    buf_ptr->pub.is_call_comp_cap_available = FALSE;
    buf_ptr->pub.call_comp_cap.peer_call_comp_support = FALSE;
    #endif
...
)
{
  ...
  if ( buf_ptr != NULL )
  {
    ...
    QM_MSG_MED_5("Update in RTT cap of CList entry : call id %d, RTT local cap %d -> %d, RTT PEER CAP %d -> %d", call_id,
                 buf_ptr->pub.rtt_cap.local_rtt_cap, local_attr.rtt_mode,
                 buf_ptr->pub.rtt_cap.peer_rtt_cap, peer_attr.rtt_mode);

    buf_ptr->pub.is_rtt_cap_available = TRUE;
    buf_ptr->pub.rtt_cap.local_rtt_cap = (uint32) local_attr.rtt_mode;
    buf_ptr->pub.rtt_cap.peer_rtt_cap  = (uint32) peer_attr.rtt_mode;

    QM_MSG_MED_5("CALL_COMPOSER: Update cap for call id %d, local cap %d -> %d, peer cap %d -> %d",
                     call_id,
                     buf_ptr->pub.call_comp_cap.local_call_comp_support,
                     local_attr.call_composer_supported,
                     buf_ptr->pub.call_comp_cap.peer_call_comp_support,
                     peer_attr.call_composer_supported);

    #if defined(FEATURE_HA_MAINLINE_ONLY_ENABLE)
    buf_ptr->pub.is_call_comp_cap_available = TRUE;
    buf_ptr->pub.call_comp_cap.local_call_comp_support = local_attr.call_composer_supported;
    buf_ptr->pub.call_comp_cap.peer_call_comp_support = peer_attr.call_composer_supported;
    #else
    buf_ptr->pub.is_call_comp_cap_available = FALSE;
    buf_ptr->pub.call_comp_cap.local_call_comp_support = FALSE;
    buf_ptr->pub.call_comp_cap.peer_call_comp_support = FALSE;
    #endif
  }
  else
...
/*===========================================================================
  ...
  FUNCTION:  qmi_voice_call_list_update_ip_alert_info

===========================================================================*/
/*!
    @brief
    Update the IP Alert info for the call
...
)
{
  ...
  QM_MSG_HIGH_1("qmi_voice_call_list_update_ip_alert_info of call %d", call_id);

  buf_ptr = qmi_voice_call_list_find( call_id );

  if ( buf_ptr != NULL )
  {
    if( (ip_alert_info_ptr!= NULL) &&
        (ip_alert_info_ptr->alert_info_text != NULL) &&
        (ip_alert_info_ptr->alert_info_len  != 0) )
    {
      buf_ptr->pub.is_ip_alert_info_available= TRUE;
      buf_ptr->pub.ip_alert_info.ip_alert_info_length = ip_alert_info_ptr->alert_info_len;
      memscpy(buf_ptr->pub.ip_alert_info.ip_alert_info_buf, sizeof(buf_ptr->pub.ip_alert_info.ip_alert_info_buf),
              ip_alert_info_ptr->alert_info_text, QMI_VOICE_IP_ALERT_INFO_MAX_LEN_V02);

      QM_MSG_HIGH_2("qmi_voice_call_list_update_ip_alert_info of call %d,len=%d", call_id,ip_alert_info_ptr->alert_info_len);
    }
  }
  else
  {
    /* Call not in the list */
    QM_MSG_HIGH_1("qmi_voice_call_list_update_ip_alert_info of call %d not found", call_id);
    status = E_FAILURE;
  }

  return status;
} /* qmi_voice_call_list_update_ip_alert_info() */


/*===========================================================================

  FUNCTION:  qmi_voice_call_list_update_crs_info

===========================================================================*/
/*!
    @brief
    Update the CRS info for the call
...
)
{
  ...
  QM_MSG_HIGH_1("qmi_voice_call_list_update_crs_info of call %d", call_id);

  buf_ptr = qmi_voice_call_list_find( call_id );

  if ( buf_ptr != NULL )
  {
    if((crs_info_ptr.crs_type != CM_CALL_CRS_TYPE_NONE) &&
        (crs_info_ptr.original_call_type  != CM_CALL_TYPE_NONE_MASK) )
    {
      buf_ptr->pub.is_crs_info_available= TRUE;
      buf_ptr->pub.crs_info.crs_type = (uint32)crs_info_ptr.crs_type;
      buf_ptr->pub.crs_info.original_call_type = crs_info_ptr.original_call_type;

      QM_MSG_HIGH_3("qmi_voice_call_list_update_crs_info of call %d,crs_type=%d, original_call_type %d", call_id,buf_ptr->pub.crs_info.crs_type, buf_ptr->pub.crs_info.original_call_type);
    }
    else
    {
      buf_ptr->pub.is_crs_info_available= FALSE;
      buf_ptr->pub.crs_info.crs_type = (uint32)crs_info_ptr.crs_type;
      buf_ptr->pub.crs_info.original_call_type = crs_info_ptr.original_call_type;
    }
  }
  ...
  {
    /* Call not in the list */
    QM_MSG_HIGH_1("qmi_voice_call_list_update_crs_info of call %d not found", call_id);
    status = E_FAILURE;
  }

  return status;
} /* qmi_voice_call_list_update_crs_info() */


/*===========================================================================

  FUNCTION:  qmi_voice_call_list_update_silent_ui

===========================================================================*/
/*!
    @brief
    Update the silent ui for the call
...
)
{
  ...
  QM_MSG_HIGH_1("qmi_voice_call_list_update_silent_ui of call %d", call_id);

  buf_ptr = qmi_voice_call_list_find( call_id );

  if ( buf_ptr != NULL )
  {
      buf_ptr->pub.is_silent_ui_available = TRUE;
      buf_ptr->pub.silent_ui = silent_ui;

      QM_MSG_HIGH_2("qmi_voice_call_list_update_silent_ui of call %d,silent_ui %d", call_id,silent_ui);
  }
  else
  {
    /* Call not in the list */
    QM_MSG_HIGH_1("qmi_voice_call_list_update_silent_ui of call %d not found", call_id);
    status = E_FAILURE;
  }
...
/*!
    @brief
    Update the TIR Mode for the call
...
)
{
  ...
  QM_MSG_HIGH_1("qmi_voice_call_list_update_tir_mode of call %d", call_id);

  buf_ptr = qmi_voice_call_list_find( call_id );

  if ( buf_ptr != NULL )
  {
    if(tir_mode == CM_SUPS_TEMPORARY_MODE)
    {
      buf_ptr->pub.is_tir_mode_available= TRUE;
      buf_ptr->pub.tir_mode = (uint8)CM_SUPS_TEMPORARY_MODE;

      QM_MSG_HIGH_2("qmi_voice_call_list_update_tir_mode of call %d,tir_mode=%d", call_id,buf_ptr->pub.tir_mode);
    }
  }
  else
  {
...
)
{
  ...
  //In qmi_voice_call_list_reset_srvcc_caller_name_flags()
...
)
{
  ...
  QM_MSG_MED("In qmi_voice_call_list_reset_drvcc_available_flags().");
...
)
{
  ...
  QM_MSG_MED("In qmi_voice_call_list_reset_drvcc_parent_id_valid_flags().");
...
)
{
  ...
  if ( buf_ptr != NULL )
  {
    if(display_text.displayTextLength > 0)
    {
      ...
      memscpy(buf_ptr->pub.display_text.text,
                sizeof(buf_ptr->pub.display_text.text),
                display_text.display_text,
                display_text.displayTextLength*2);
    }
    else
    {
      //Display text len <= 0
...
)
{
  ...
  for(i=0; i < MIN(cm_data->number_calls,MAX_CALL_NUM_SRVCC); i++)
  {
    ...
    {
      continue;
      /*Replacing existing call list entries will be deferred till after adding new entries following CR#608077*/
    }
...
)
{
  ...
  call_info = QM_MEM_ALLOC(sizeof(qmi_voice_call_list_public_type));
  if(NULL == call_info)
  {
    return FALSE;
  }
  ...
  if ( buf_ptr != NULL )
  {
    ...
    switch(end_status)
    {
      ...
      case CM_CALL_END_PRECONDITION_FAILURE:
        ...
        break;
      case CM_CALL_END_SIP_503_SERVER_UNAVAILABLE:
        buf_ptr->pub.orig_fail_reason = (qmi_voice_cm_if_failure_cause_e_type)QMI_VOICE_CM_IF_END_SIP_503_SERVER_UNAVAILABLE;
        break;
      case CM_CALL_END_TOO_MANY_HOPS:
        buf_ptr->pub.orig_fail_reason = QMI_FAILURE_CAUSE_TOO_MANY_HOPS;
        break;
      case CM_CALL_END_GONE:
        buf_ptr->pub.orig_fail_reason = QMI_FAILURE_CAUSE_GONE;
        break;
      case CM_CALL_END_IP_USER_REJECT:
        buf_ptr->pub.orig_fail_reason = QMI_FAILURE_CAUSE_REJECTED_BY_USER;
        break;
      case CM_CALL_END_MOVED_PERMANENTLY:
        buf_ptr->pub.orig_fail_reason = QMI_FAILURE_CAUSE_MOVED_PERMANENTLY;
        break;
      case CM_CALL_END_UNAUTHORIZED:
        buf_ptr->pub.orig_fail_reason = QMI_FAILURE_CAUSE_UNAUTHORIZED;
        break;
      case CM_CALL_END_METHOD_NOT_ALLOWED:
        buf_ptr->pub.orig_fail_reason = QMI_FAILURE_CAUSE_METHOD_NOT_ALLOWED;
        break;
      case CM_CALL_END_NOT_ACCEPTABLE:
        buf_ptr->pub.orig_fail_reason = QMI_FAILURE_CAUSE_NOT_ACCEPTABLE;
        break;
      case CM_CALL_END_UNSUPPORTED_URI_SCHEME:
        buf_ptr->pub.orig_fail_reason = QMI_FAILURE_CAUSE_UNSUPPORTED_URI_SCHEME;
        break;
      case CM_CALL_END_LOOP_DETECTED:
        buf_ptr->pub.orig_fail_reason = QMI_FAILURE_CAUSE_LOOP_DETECTED;
        break;
      case CM_CALL_END_REMOTE_UNSUPP_MEDIA_TYPE:
        buf_ptr->pub.orig_fail_reason = QMI_FAILURE_CAUSE_REMOTE_UNSUPP_MEDIA_TYPE;
        break;
      case CM_CALL_END_NOT_ACCEPTABLE_GLOBAL:
        buf_ptr->pub.orig_fail_reason = QMI_FAILURE_CAUSE_NOT_ACCEPTABLE_GLOBAL;
        break;
      default:
...
/*===========================================================================
  ...
  FUNCTION:  qmi_voice_call_list_update_call_composer

===========================================================================*/
/*!
    @brief
    Update the call composer for the call upon pre_incoming event
...
)
{
  ...
  if ( buf_ptr != NULL )
  {
    QM_MSG_HIGH_7("CALL_COMPOSER: UPDATE: call %d, imp %d, subject %d, url %d, loc incl %d, shape %d org_header 0x%x", 
                      call_id,
                      call_composer_ptr->importance,
                      call_composer_ptr->subject,
                      call_composer_ptr->picture_url,
                      call_composer_ptr->geo_location.location_included,
                      call_composer_ptr->geo_location.geo_shape,
                      call_composer_ptr->org_header);

    if (call_composer_ptr->importance == CM_CALL_COMP_IMP_NORMAL)
    {
      buf_ptr->pub.is_call_composer_importance_available = TRUE;
      buf_ptr->pub.call_composer_importance = (uint8)CALL_COMP_IMP_NORMAL_V02;
    }
    else if(call_composer_ptr->importance == CM_CALL_COMP_IMP_URGENT)
    {
      buf_ptr->pub.is_call_composer_importance_available = TRUE;
      buf_ptr->pub.call_composer_importance = (uint8)CALL_COMP_IMP_URGENT_V02;
    }

    if (call_composer_ptr->subject != NULL &&       
        call_composer_ptr->subject->subject_length != 0 &&
        call_composer_ptr->subject->subject_text != NULL)
    {
      buf_ptr->pub.is_call_composer_subject_available = TRUE;
      buf_ptr->pub.call_composer_subject.call_composer_subject_len 
        = call_composer_ptr->subject->subject_length;
        
      memscpy(buf_ptr->pub.call_composer_subject.call_composer_subject, 
               2*QMI_VOICE_CM_IF_MAX_CALL_COMP_SUBJECT_LEN,
               call_composer_ptr->subject->subject_text, 
               2*call_composer_ptr->subject->subject_length);
    }

    if (call_composer_ptr->picture_url != NULL &&       
        call_composer_ptr->picture_url->pic_url != NULL)
    {
      buf_ptr->pub.is_call_composer_picture_url_available = TRUE;
        
      memscpy(buf_ptr->pub.call_composer_picture_url.call_composer_pic_url, 
               sizeof(buf_ptr->pub.call_composer_picture_url.call_composer_pic_url),
               call_composer_ptr->picture_url->pic_url, 
               QMI_VOICE_CM_IF_MAX_CALL_COMP_PIC_URL_LEN+1);
    }

    if (call_composer_ptr->geo_location.location_included)
    {
      buf_ptr->pub.call_composer_location = call_composer_ptr->geo_location;
    }

    if (call_composer_ptr->org_header != NULL &&       
        call_composer_ptr->org_header->org_header_length != 0 &&
        call_composer_ptr->org_header->org_header_text != NULL)
    {
      QM_MSG_HIGH_1("CALL_COMPOSER: UPDATE org_header_length: %d ", 
                      call_composer_ptr->org_header->org_header_length);
      buf_ptr->pub.is_call_composer_org_header_available = TRUE;
      buf_ptr->pub.call_composer_org_header.call_composer_org_header_len 
        = call_composer_ptr->org_header->org_header_length;
        
      memscpy(buf_ptr->pub.call_composer_org_header.call_composer_org_header, 
               2*QMI_VOICE_CM_IF_MAX_CALL_COMP_ORG_HDR_LEN,
               call_composer_ptr->org_header->org_header_text, 
               2*call_composer_ptr->org_header->org_header_length);
    }
  }
  else
  {
    QM_MSG_HIGH_1("CALL_COMPOSER: UPDATE: %d not found", 
                      call_id);
    status = E_FAILURE;
  }
