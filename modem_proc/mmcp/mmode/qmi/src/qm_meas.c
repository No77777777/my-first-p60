...
static void update_cm_sig_info_measurements_to_global_last_ind_struct(cm_mm_msim_ss_info_s_type *p_ss_info);
static void update_cm_err_rate_measurements_to_global_last_ind_struct(cm_mm_msim_ss_info_s_type *p_ss_info);
...
// UTILITY FUNCTIONS
//===========================================================================
static float convert_cm_ss_evt_rssi(uint16 rssi)
{
  return (float)((int16)rssi * -1.0);  // saved as dBm
}
static float convert_cm_ss_evt_ecio(int16 ecio)
{
  return (float)ecio* -0.5;  // saved as dBm
}
static float convert_cm_ss_evt_hdr_sinr(byte sinr)
{
  return (float)sinr;  // saved as value in range from 0.0 to 8.0
}
static float convert_cm_ss_evt_hdr_io(int io)
{
  return (float)io;  // saved as dBm
}
static float convert_cm_ss_evt_lte_rsrq(int8 rsrq)
{
  return (float)rsrq;  // saved as dB
}
static float convert_cm_ss_evt_lte_rsrp(int16 rsrp)
{
  return (float)rsrp;  // saved as dBm
}
static float convert_cm_ss_evt_lte_snr(byte sinr)
{
  return (float)(( 2 * sinr ) - 200)/10.0;  // saved as dB
}
static float convert_cm_ss_evt_err_rate(uint16 err_rate)
{
  return (uint32)err_rate;
}
static float convert_cm_ss_evt_gw_err_rate(uint8 err_rate)
{
  return (uint32)err_rate;
}
#ifdef FEATURE_TDSCDMA
static float convert_msgr_evt_tds_rscp(int16 rscp)
{
  return (float)(rscp)/10.0;  // saved as dBm
}
static float convert_msgr_evt_tds_rssi(int16 rssi)
{
  return (float)(rssi)/10.0;  // saved as dBm
}
static float convert_msgr_evt_tds_ecio(int16 ecio)
{
  return (float)(ecio)/10.0;  // saved as dB
}
static float convert_msgr_evt_tds_sinr(int16 sinr)
{
  return (float)(sinr)/10.0;  // saved as dB
}
static float convert_msgr_evt_tds_bler(uint8 bler)
{
  return (uint32)bler;
}
#endif
static int8 convert_global_rssi(float rssi)
{
  return (int8)rssi;  // saved as dBm
}
static int16 convert_global_ecio(float ecio)
{
  return (int16)(ecio/-0.5);  // saved as dB
}
static nas_sinr_enum_v01 convert_global_hdr_sinr(float sinr)
{
  return (nas_sinr_enum_v01)sinr;
}
static int32 convert_global_hdr_io(float io)
{
  return (int32)io;  // saved as dBm
}
static int8 convert_global_lte_rsrq(float rsrq)
{
  return (int8)rsrq;  // saved as dB
}
static int16 convert_global_lte_rsrp(float rsrp)
{
  return (int16)rsrp;  // saved as dBm
}
...
static int8 convert_global_tds_rscp(float rscp)
{
  return (int8)rscp;  // saved as dBm
}
...
static void send_sig_info_ind( void )
{
  ...
  while( index < QM_NAS_MAX_NUM_CLIDS && clid != QM_NAS_INVALID_CLIENT )
  {
    ...
    ind_msg = (nas_sig_info_ind_msg_v01 *) QM_MEM_ALLOC( sizeof( nas_sig_info_ind_msg_v01 ) );
...
static void send_err_rate_ind( void )
{
  ...
  while( index < QM_NAS_MAX_NUM_CLIDS && clid != QM_NAS_INVALID_CLIENT )
  {
    // ask Client module which subscription this client is bound to
    sub_id = qm_nas_clnt_get_binding(clid);
    ind_msg = (nas_err_rate_ind_msg_v01 *) QM_MEM_ALLOC( sizeof( nas_err_rate_ind_msg_v01 ) );
...
static void invalidate_inactive_rats( enum qm_subs_e sub_id, rat_bitmask_type inactive_rat_mask )
{
  ...
  if( inactive_rat_mask & QM_RAT_MASK_C )
  {
    ...
    global.last_ind[sub_id].cdma_sig_str.is_valid = FALSE;
    global.last_ind[sub_id].cdma_sig_str.rssi = QM_MEAS_INVALID_RSSI;
    global.last_ind[sub_id].cdma_sig_str.ecio = QM_MEAS_INVALID_ECIO;
    global.last_ind[sub_id].cdma_sig_str.frame_error_rate = QM_MEAS_INVALID_ERR_RATE;
  }
  //HDR
  if( inactive_rat_mask & QM_RAT_MASK_H )
  {
    ...
    global.last_ind[sub_id].hdr_sig_str.is_valid = FALSE;
    global.last_ind[sub_id].hdr_sig_str.rssi = QM_MEAS_INVALID_RSSI;
    global.last_ind[sub_id].hdr_sig_str.ecio = QM_MEAS_INVALID_ECIO;
    global.last_ind[sub_id].hdr_sig_str.sinr = QM_MEAS_INVALID_SINR;
    global.last_ind[sub_id].hdr_sig_str.io = QM_MEAS_INVALID_IO;
    global.last_ind[sub_id].hdr_sig_str.packet_err_rate = QM_MEAS_INVALID_ERR_RATE;
  }
  //GSM
  if( inactive_rat_mask & QM_RAT_MASK_G )
  {
    ...
    global.last_ind[sub_id].gsm_sig_str.is_valid = FALSE;
    global.last_ind[sub_id].gsm_sig_str.rssi = QM_MEAS_INVALID_RSSI;
    global.last_ind[sub_id].gsm_sig_str.bit_error_rate = QM_MEAS_INVALID_ERR_RATE;
  }
  //WCDMA
  if( inactive_rat_mask & QM_RAT_MASK_W )
  {
    ...
    global.last_ind[sub_id].wcdma_sig_str.is_valid = FALSE;
    global.last_ind[sub_id].wcdma_sig_str.rssi = QM_MEAS_INVALID_RSSI;
    global.last_ind[sub_id].wcdma_sig_str.ecio = QM_MEAS_INVALID_ECIO;
    global.last_ind[sub_id].wcdma_sig_str.block_error_rate = QM_MEAS_INVALID_ERR_RATE;
  }
  //LTE
  if( inactive_rat_mask & QM_RAT_MASK_L )
  {
    ...
    global.last_ind[sub_id].lte_sig_str.is_valid = FALSE;
    global.last_ind[sub_id].lte_sig_str.rssi = QM_MEAS_INVALID_RSSI;
    global.last_ind[sub_id].lte_sig_str.rsrq = QM_MEAS_INVALID_RSRQ;
    global.last_ind[sub_id].lte_sig_str.rsrp = QM_MEAS_INVALID_RSRP;
    global.last_ind[sub_id].lte_sig_str.snr = QM_MEAS_INVALID_SNR;
  }
  //TDSCDMA
  if( inactive_rat_mask & QM_RAT_MASK_T )
  {
    ...
    global.last_ind[sub_id].tdscdma_sig_str.is_valid = FALSE;
    global.last_ind[sub_id].tdscdma_sig_str.rssi = QM_MEAS_INVALID_RSSI;
    global.last_ind[sub_id].tdscdma_sig_str.rscp = QM_MEAS_INVALID_RSCP;
    global.last_ind[sub_id].tdscdma_sig_str.ecio = QM_MEAS_INVALID_ECIO;
    global.last_ind[sub_id].tdscdma_sig_str.sinr = QM_MEAS_INVALID_SINR;
    global.last_ind[sub_id].tdscdma_sig_str.bler = QM_MEAS_INVALID_ERR_RATE;
  }
}
      ...
      while ( QM_NAS_CLNT_ID_VALID(clid) )
      {
        for( i = 0; i < p_ss_info->number_of_stacks && i < STACK_INDEX_MAX; i++)
        {
          if( p_ss_info->stack_info[i].sys_mode == SYS_SYS_MODE_CDMA )
          {
            ...
            if( qm_nas_clnt_crossed_threshold(clid, sub_id, QM_RAT_1X, QM_SIG_TYPE_RSSI, rssi, global.last_ind[sub_id].cdma_sig_str.rssi) ||
                qm_nas_clnt_crossed_threshold(clid, sub_id, QM_RAT_1X, QM_SIG_TYPE_ECIO, ecio, global.last_ind[sub_id].cdma_sig_str.ecio)
              )
            {
          ...
          else if( p_ss_info->stack_info[i].sys_mode == SYS_SYS_MODE_HDR )
          {
            ...
            if( qm_nas_clnt_crossed_threshold(clid, sub_id, QM_RAT_HDR, QM_SIG_TYPE_RSSI, rssi, global.last_ind[sub_id].hdr_sig_str.rssi) ||
                qm_nas_clnt_crossed_threshold(clid, sub_id, QM_RAT_HDR, QM_SIG_TYPE_ECIO, ecio, global.last_ind[sub_id].hdr_sig_str.ecio) ||
                qm_nas_clnt_crossed_threshold(clid, sub_id, QM_RAT_HDR, QM_SIG_TYPE_SINR, sinr, global.last_ind[sub_id].hdr_sig_str.sinr) ||
                qm_nas_clnt_crossed_threshold(clid, sub_id, QM_RAT_HDR, QM_SIG_TYPE_IO, io, global.last_ind[sub_id].hdr_sig_str.io)
              )
            {
              global.send_sig_info_ind[sig_info_ind_index++] = clid;
              break;
          ...
          else if( p_ss_info->stack_info[i].sys_mode == SYS_SYS_MODE_GSM )
          {
            ...
            if( qm_nas_clnt_crossed_threshold(clid, sub_id, QM_RAT_GERAN, QM_SIG_TYPE_RSSI, rssi, global.last_ind[sub_id].gsm_sig_str.rssi) )
            {
              global.send_sig_info_ind[sig_info_ind_index++] = clid;
              break;
            }
          ...
          else if( p_ss_info->stack_info[i].sys_mode == SYS_SYS_MODE_WCDMA )
          {
            ...
            if( qm_nas_clnt_crossed_threshold(clid, sub_id, QM_RAT_UMTS, QM_SIG_TYPE_RSSI, rssi, global.last_ind[sub_id].wcdma_sig_str.rssi) ||
                qm_nas_clnt_crossed_threshold(clid, sub_id, QM_RAT_UMTS, QM_SIG_TYPE_ECIO, ecio, global.last_ind[sub_id].wcdma_sig_str.ecio)
              )
            {
              global.send_sig_info_ind[sig_info_ind_index++] = clid;
              break;
          ...
          else if( p_ss_info->stack_info[i].sys_mode == SYS_SYS_MODE_LTE )
          {
            ...
            if( qm_nas_clnt_crossed_threshold(clid, sub_id, QM_RAT_LTE, QM_SIG_TYPE_RSSI, rssi, global.last_ind[sub_id].lte_sig_str.rssi) ||
                qm_nas_clnt_crossed_threshold(clid, sub_id, QM_RAT_LTE, QM_SIG_TYPE_RSRQ, rsrq, global.last_ind[sub_id].lte_sig_str.rsrq) ||
                qm_nas_clnt_crossed_threshold(clid, sub_id, QM_RAT_LTE, QM_SIG_TYPE_RSRP, rsrp, global.last_ind[sub_id].lte_sig_str.rsrp) ||
                qm_nas_clnt_crossed_threshold(clid, sub_id, QM_RAT_LTE, QM_SIG_TYPE_SNR, snr, global.last_ind[sub_id].lte_sig_str.snr)
              )
            {
              global.send_sig_info_ind[sig_info_ind_index++] = clid;
              break;
...
static void check_send_ind_for_tds_measurements( tds_rrc_scell_signal_status_ind_type *tds_meas, enum qm_subs_e sub_id )
{
  ...
  if( tds_meas )
  {
    ...
    while ( QM_NAS_CLNT_ID_VALID(clid) )
    {
      ...
      {
        QM_MSG_HIGH_3("Adding clid %d to send_sig_info_ind list, sub_id %d, sig_info_ind_index: %d, TDS",
                      clid, sub_id, sig_info_ind_index);
        global.send_sig_info_ind[sig_info_ind_index++] = clid;
      }
  ...
  if(p_ss_info)
  {
    ...
    if( QM_NAS_SUB_ID_VALID(sub_id) )
    {
      for( i = 0; i < p_ss_info->number_of_stacks && i < STACK_INDEX_MAX; i++ )
      {
        ...
        QM_MSG_HIGH_1("update_cm_sig_info_measurements_to_global_last_ind_struct sys_mode %d", p_ss_info->stack_info[i].sys_mode);
        ...
        else if( p_ss_info->stack_info[i].sys_mode == SYS_SYS_MODE_HDR && QM_NAS_SUB_ID_VALID(sub_id) )
        {
        ...
        else if( p_ss_info->stack_info[i].sys_mode == SYS_SYS_MODE_GSM && QM_NAS_SUB_ID_VALID(sub_id) )
        {
          global.last_ind[sub_id].gsm_sig_str.rssi = convert_cm_ss_evt_rssi(p_ss_info->stack_info[i].rssi);
          global.last_ind[sub_id].gsm_sig_str.is_valid = TRUE;
          QM_MSG_MED_1("GSM rssi %d", p_ss_info->stack_info[i].rssi);
        }
        else if( p_ss_info->stack_info[i].sys_mode == SYS_SYS_MODE_WCDMA && QM_NAS_SUB_ID_VALID(sub_id) )
        {
        ...
        else if( p_ss_info->stack_info[i].sys_mode == SYS_SYS_MODE_LTE && QM_NAS_SUB_ID_VALID(sub_id) )
        {
          global.last_ind[sub_id].lte_sig_str.rssi = convert_cm_ss_evt_rssi(p_ss_info->stack_info[i].rssi);
          global.last_ind[sub_id].lte_sig_str.rsrq = convert_cm_ss_evt_lte_rsrq(p_ss_info->stack_info[i].rat_mode_info.lte_mode_info.rsrq);
          global.last_ind[sub_id].lte_sig_str.rsrp = convert_cm_ss_evt_lte_rsrp(p_ss_info->stack_info[i].rat_mode_info.lte_mode_info.rsrp);
          global.last_ind[sub_id].lte_sig_str.snr  = convert_cm_ss_evt_lte_snr(p_ss_info->stack_info[i].sinr );
          global.last_ind[sub_id].lte_sig_str.is_valid = TRUE;
          QM_MSG_MED_4("LTE rssi %d, rsrq %d, rsrp %d, sinr %d",
                       p_ss_info->stack_info[i].rssi, p_ss_info->stack_info[i].rat_mode_info.lte_mode_info.rsrq,
                       p_ss_info->stack_info[i].rat_mode_info.lte_mode_info.rsrp,  p_ss_info->stack_info[i].sinr);
        }
        else
        {
...
/*===========================================================================
  FUNCTION qm_meas_copy_global_sig_info_measurements_to_last_ind()
  ...
  SIDE EFFECTS
    None
===========================================================================*/
static void qm_meas_copy_global_sig_info_measurements_to_last_ind(enum qm_subs_e sub_id, rat_bitmask_type active_rat_mask)
{

  if( QM_NAS_SUB_ID_VALID(sub_id) )
  {
  
    QM_MSG_HIGH_1("qm_meas_copy_global_sig_info_measurements_to_last_ind active_rat_mask %d", active_rat_mask);
  
    // CDMA
    if((active_rat_mask & QM_RAT_MASK_C) && global.meas[sub_id].cdma_sig_str.is_valid)
    {
      global.last_ind[sub_id].cdma_sig_str.rssi = global.meas[sub_id].cdma_sig_str.rssi;
      global.last_ind[sub_id].cdma_sig_str.ecio = global.meas[sub_id].cdma_sig_str.ecio;
      global.last_ind[sub_id].cdma_sig_str.is_valid = TRUE;
      QM_MSG_MED_2("CDMA rssi %d, ecio %d", global.last_ind[sub_id].cdma_sig_str.rssi, global.last_ind[sub_id].cdma_sig_str.ecio);
    }
    if((active_rat_mask & QM_RAT_MASK_H) && global.meas[sub_id].hdr_sig_str.is_valid)
    {
      global.last_ind[sub_id].hdr_sig_str.rssi = global.meas[sub_id].hdr_sig_str.rssi;
      global.last_ind[sub_id].hdr_sig_str.ecio = global.meas[sub_id].hdr_sig_str.ecio;
      global.last_ind[sub_id].hdr_sig_str.sinr = global.meas[sub_id].hdr_sig_str.sinr;
      global.last_ind[sub_id].hdr_sig_str.io   = global.meas[sub_id].hdr_sig_str.io;
      global.last_ind[sub_id].hdr_sig_str.is_valid = TRUE;
      QM_MSG_MED_4("HDR rssi %d, ecio %d, sinr %d, io %d",
                   global.last_ind[sub_id].hdr_sig_str.rssi, global.last_ind[sub_id].hdr_sig_str.ecio,
                   global.last_ind[sub_id].hdr_sig_str.sinr, global.last_ind[sub_id].hdr_sig_str.io);
    }
    if((active_rat_mask & QM_RAT_MASK_G) && global.meas[sub_id].gsm_sig_str.is_valid)
    {
      global.last_ind[sub_id].gsm_sig_str.rssi = global.meas[sub_id].gsm_sig_str.rssi;
      global.last_ind[sub_id].gsm_sig_str.is_valid = TRUE;
      QM_MSG_MED_1("GSM rssi %d", global.last_ind[sub_id].gsm_sig_str.rssi);
    }
    if((active_rat_mask & QM_RAT_MASK_W) && global.meas[sub_id].wcdma_sig_str.is_valid)
    {
      global.last_ind[sub_id].wcdma_sig_str.rssi = global.meas[sub_id].wcdma_sig_str.rssi;
      global.last_ind[sub_id].wcdma_sig_str.ecio = global.meas[sub_id].wcdma_sig_str.ecio;
      global.last_ind[sub_id].wcdma_sig_str.is_valid = TRUE;
      QM_MSG_MED_2("WCDMA rssi %d, ecio %d", global.last_ind[sub_id].wcdma_sig_str.rssi, global.last_ind[sub_id].wcdma_sig_str.ecio);
    }
    if((active_rat_mask & QM_RAT_MASK_L) && global.meas[sub_id].lte_sig_str.is_valid)
    {
      global.last_ind[sub_id].lte_sig_str.rssi = global.meas[sub_id].lte_sig_str.rssi;
      global.last_ind[sub_id].lte_sig_str.rsrq = global.meas[sub_id].lte_sig_str.rsrq;
      global.last_ind[sub_id].lte_sig_str.rsrp = global.meas[sub_id].lte_sig_str.rsrp;
      global.last_ind[sub_id].lte_sig_str.snr  = global.meas[sub_id].lte_sig_str.snr;
      global.last_ind[sub_id].lte_sig_str.is_valid = TRUE;
      QM_MSG_MED_4("LTE rssi %d, rsrq %d, rsrp %d, sinr %d",
                   global.last_ind[sub_id].lte_sig_str.rssi, global.last_ind[sub_id].lte_sig_str.rsrq,
                   global.last_ind[sub_id].lte_sig_str.rsrp, global.last_ind[sub_id].lte_sig_str.snr);
  
  #ifdef FEATURE_LAPP
      global.last_ind[sub_id].nr5g_sig_str.rsrp = global.meas[sub_id].nr5g_sig_str.rsrp;
      global.last_ind[sub_id].nr5g_sig_str.snr  = global.meas[sub_id].nr5g_sig_str.snr;
      global.last_ind[sub_id].nr5g_sig_str.is_valid = TRUE;
      QM_MSG_HIGH_2("NR5G rsrp %d, sinr %d", global.last_ind[sub_id].nr5g_sig_str.rsrp, 
                     global.last_ind[sub_id].nr5g_sig_str.snr);
  #endif
    }
  ...
  {
    QM_MSG_ERROR_2( "Invalid sub_id in copy_global_sig_info_measurements_to_last_ind, sub id %d active_rat_mask %d", 
        sub_id, active_rat_mask );
  }
...
static void update_cm_err_rate_measurements_to_global_last_ind_struct(cm_mm_msim_ss_info_s_type *p_ss_info)
{
  ...
  if(p_ss_info)
  {
    ...
    if( QM_NAS_SUB_ID_VALID(sub_id) )
    {
      for( i = 0; i < p_ss_info->number_of_stacks && i < STACK_INDEX_MAX; i++ )
      {
        QM_MSG_HIGH_1("update_cm_err_rate_measurements_to_global_last_ind_struct sys_mode %d", p_ss_info->stack_info[i].sys_mode);
        ...
        if( p_ss_info->stack_info[i].sys_mode == SYS_SYS_MODE_CDMA && QM_NAS_SUB_ID_VALID(sub_id) )
        {
          ...
          global.last_ind[sub_id].cdma_sig_str.is_valid = TRUE;
          QM_MSG_MED_1("CDMA err_rate %d", p_ss_info->stack_info[i].rat_mode_info.cdma_mode_info.frame_err_rate);
        }
        else if( p_ss_info->stack_info[i].sys_mode == SYS_SYS_MODE_HDR && QM_NAS_SUB_ID_VALID(sub_id) )
        {
          ...
          global.last_ind[sub_id].hdr_sig_str.is_valid = TRUE;
          QM_MSG_MED_1("HDR err_rate %d", p_ss_info->stack_info[i].rat_mode_info.hdr_mode_info.hdr_packet_err_rate);
        }
        else if( p_ss_info->stack_info[i].sys_mode == SYS_SYS_MODE_GSM && QM_NAS_SUB_ID_VALID(sub_id) )
        {
          global.last_ind[sub_id].gsm_sig_str.bit_error_rate = convert_cm_ss_evt_gw_err_rate(p_ss_info->stack_info[i].mode_info.gw_info.bit_err_rate);
          global.last_ind[sub_id].gsm_sig_str.is_valid = TRUE;
          QM_MSG_MED_1("GSM err_rate %d", p_ss_info->stack_info[i].mode_info.gw_info.bit_err_rate);
        }
        else if( p_ss_info->stack_info[i].sys_mode == SYS_SYS_MODE_WCDMA && QM_NAS_SUB_ID_VALID(sub_id) )
        {
...
/*===========================================================================
  FUNCTION qm_meas_copy_global_err_rate_measurements_to_last_ind()
  ...
  SIDE EFFECTS
    None
===========================================================================*/
static void qm_meas_copy_global_err_rate_measurements_to_last_ind(enum qm_subs_e sub_id, rat_bitmask_type active_rat_mask)
{    

  if( QM_NAS_SUB_ID_VALID(sub_id) )
  {
    QM_MSG_HIGH_1("qm_meas_copy_global_err_rate_measurements_to_last_ind active_rat_mask %d", active_rat_mask);
  
    // CDMA
    if( (active_rat_mask & QM_RAT_MASK_C) && global.meas[sub_id].cdma_sig_str.is_valid )
    {
      global.last_ind[sub_id].cdma_sig_str.frame_error_rate = global.meas[sub_id].cdma_sig_str.frame_error_rate;
      global.last_ind[sub_id].cdma_sig_str.is_valid = TRUE;
      QM_MSG_MED_1("CDMA err_rate %d", global.last_ind[sub_id].cdma_sig_str.frame_error_rate);
    }
    if((active_rat_mask & QM_RAT_MASK_H) && global.meas[sub_id].hdr_sig_str.is_valid)
    {
      global.last_ind[sub_id].hdr_sig_str.packet_err_rate = global.meas[sub_id].hdr_sig_str.packet_err_rate;
      global.last_ind[sub_id].hdr_sig_str.is_valid = TRUE;
      QM_MSG_MED_1("HDR err_rate %d", global.last_ind[sub_id].hdr_sig_str.packet_err_rate);
    }
    if((active_rat_mask & QM_RAT_MASK_G) && global.meas[sub_id].gsm_sig_str.is_valid)
    {
      global.last_ind[sub_id].gsm_sig_str.bit_error_rate = global.meas[sub_id].gsm_sig_str.bit_error_rate;
      global.last_ind[sub_id].gsm_sig_str.is_valid = TRUE;
      QM_MSG_MED_1("GSM err_rate %d", global.last_ind[sub_id].gsm_sig_str.bit_error_rate);
    }
    if((active_rat_mask & QM_RAT_MASK_W) && global.meas[sub_id].wcdma_sig_str.is_valid)
    {
      global.last_ind[sub_id].wcdma_sig_str.block_error_rate = global.meas[sub_id].wcdma_sig_str.block_error_rate;
      global.last_ind[sub_id].wcdma_sig_str.is_valid = TRUE;
      QM_MSG_MED_1("WCDMA err_rate %d", global.last_ind[sub_id].wcdma_sig_str.block_error_rate);
    }
    
  }
  else
  {
    QM_MSG_ERROR_2( "Invalid sub_id in copy_global_err_rate_measurements_to_last_ind, sub id %d active_rat_mask %d", 
        sub_id, active_rat_mask );
  }
}
...
void qm_meas_report_new_active_rats( enum qm_subs_e sub_id, rat_bitmask_type active_rat_mask)
{
  ...
  while ( QM_NAS_CLNT_ID_VALID(clid) )
  {
    ...
    {
      sig_ind = (nas_sig_info_ind_msg_v01 *) QM_MEM_ALLOC( sizeof( nas_sig_info_ind_msg_v01 ) );
      ...
      QM_MEM_FREE( (void *) sig_ind );
      sig_ind = NULL;
    }

    clid = qm_nas_clnt_get_next_client_for_sig_info_ind(clid, sub_id);
  }
  qm_meas_copy_global_sig_info_measurements_to_last_ind(sub_id, active_rat_mask);
  ...
  while ( QM_NAS_CLNT_ID_VALID(clid) )
  {
    ...
    {
      err_ind = (nas_err_rate_ind_msg_v01 *) QM_MEM_ALLOC( sizeof( nas_err_rate_ind_msg_v01 ) );
      ...
      QM_MEM_FREE( (void *) err_ind );
      err_ind = NULL;
    }

    clid = qm_nas_clnt_get_next_client_for_err_rate_ind(clid, sub_id);
  }
  qm_meas_copy_global_err_rate_measurements_to_last_ind(sub_id, active_rat_mask);
