...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/mmode/qmi/src/qmi_voice_cmd_list.c#1 $ $DateTime:$ $Author:$
...
{
  qmi_voice_cmd_list_buf_type *buf_ptr = NULL;
  qmi_voice_cmd_list_public_type *pub_ptr = NULL;
  qmi_voice_cmd_list_private_type *priv_ptr = NULL;
...
{
  qmi_voice_cmd_list_buf_type *buf_ptr = cmd_list.head_ptr;
  qmi_voice_cmd_list_buf_type *next_buf_ptr = NULL;
...
)
{
  ...
  qmi_voice_cmd_list_buf_type *buf_ptr = NULL;
  qmi_voice_cmd_list_public_type *pub_ptr = NULL;
  qmi_voice_cmd_list_buf_type *prev_buf_ptr = NULL, *next_buf_ptr = NULL;
...
)
{
  ...
  if (buf_ptr != NULL)
  {
    ...
    else
    {
      ...
      QM_MSG_MED_3("Update cmd list entry : %s , handle %d, state %d",
                    qmi_voice_cm_if_lookup_cmd_name( buf_ptr->pub.cmd ),
                     buf_ptr->pub.hdl, state);
    }
  }
  else
  {
...
)
{
  ...
  if (buf_ptr != NULL)
  {
    ...
    {
      buf_ptr->pub.state = state;
      QM_MSG_MED_4("Update cmd list entry: %s, handle %d, state %d, event %s",
                   qmi_voice_cm_if_lookup_cmd_name(buf_ptr->pub.cmd),
                   buf_ptr->pub.hdl, state,
                   qmi_voice_cm_if_lookup_event_name(pending_event_id));
    }
