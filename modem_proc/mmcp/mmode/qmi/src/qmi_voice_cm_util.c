...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/mmode/qmi/src/qmi_voice_cm_util.c#5 $ $DateTime:$ $Author:$
...
qmi_voice_cm_end_cause_to_qmi_end_reason_table_s_type qmi_voice_cm_if_end_reason_conv_table[] =
{
  ...
  {CM_CALL_END_CAUSE_RETRY_ON_IMS_WITHOUT_RTT,		 QMI_VOICE_CM_IF_END_CAUSE_RETRY_ON_IMS_WITHOUT_RTT},
  {CM_CALL_END_INCOM_REJ_CAUSE_CALL_ONGOING_CB_ENABLED, 		QMI_VOICE_CM_IF_END_INCOMING_REJ_CAUSE_CALL_ONGOING_CB_ENABLED},
  {CM_CALL_END_PROTOCOL_Q850_ERROR,          QMI_VOICE_CM_IF_END_PROTOCOL_Q850_ERROR},
  {CM_CALL_END_SIP_200_ANSWERED_ELSEWHERE, QMI_VOICE_CM_IF_END_ANSWERED_ELSEWHERE},
  {CM_CALL_END_IP_USER_REJECT, QMI_VOICE_CM_IF_END_CALL_REJECTED}
};
...
)
{
      ...
      ( qmi_voice_call_list_is_call_present( call_info_ptr->call_id ) ) )
  {
    ...
    if(dcs == CM_USSD_DCS_7_BIT)
    {
      ...
      name_len = (uint8) num_converted;

      QM_MSG_MED_1("CNAP name of length %d", name_len);
      if(name_len == 0)
      {
        return;
      }
      else if(call_info_ptr->enc_alpha.num_characters != name_len)
      {
        ...
        if( last_7bits == 0x0 )
        {
          ...
          name_len = (uint8) num_converted;
          QM_MSG_MED_1("CNAP name After <CR> padding of length %d", name_len);
        }
        else
        {
          if(call_info_ptr->enc_alpha.num_bytes > 0 && call_info_ptr->enc_alpha.num_bytes <= CM_MAX_ALPHA_TAG_BYTES)
...
)
{
  ...
  if(bcd_num->present)
  {
    ...
    QM_MSG_MED_6("converted pres_scren_ind=%d, pi=%d, si=%d, num len=%d, type=%d, num plan=%d",
                 pres_scren_ind, num->pi, num->si, num->len,
                 num->number_type, num->number_plan);
  }
  else
  {
...
)
{
  ...
  QM_MSG_MED_1("Put file ID %d", efs_id);
...
)
{
  ...
  /* Open the specified resource file. If open fails, return error code  */
  QM_MSG_MED_1("Get file ID %d", efs_id);
...
{
  uint8 stack=0;
  QM_ASSERT(ss_info_ptr);

  if ( (ss_info_ptr->asubs_id < SYS_MODEM_AS_ID_1) || (ss_info_ptr->asubs_id >= QMI_NAS_SUBS_MAX) )
  {
...
{
  QM_MSG_HIGH_1("Sups qmi_voice_cm_util_copy_sups_failure_string sip_code=%d", cm_sip_error_code);

  if((cm_sip_error_code>0) &&
     (sip_error_code != NULL) && (sip_code_valid != NULL))
  {
...
DESCRIPTION
 To copy the encoded USSD error string from CM/IMS.
...
SIDE EFFECTS
  None.
===========================================================================*/
void qmi_voice_cm_util_copy_ussd_failure_string
(
  uint8 *ussd_error_description_valid,
  uint16 *ussd_error_description,
  cm_ip_sups_error_s_type *cm_struct
)
{
    cm_ussd_dcs_e_type dcs=CM_USSD_DCS_UNSPECIFIED;
  ...
  QM_MSG_HIGH_1("USSD qmi_voice_cm_util_copy_ussd_failure_string ussd_err_length=%d",cm_struct->uss_error_data.size);

  if(cm_struct->uss_error_data.size > 0)
  {
     memset(ussd_error_description,'\0',QMI_VOICE_CM_IF_MAX_USS_DATA_LEN);

    dcs = qmi_voice_cm_util_get_ussd_dcs( cm_struct->uss_error_data.uss_data_coding_scheme);
    QM_MSG_HIGH_2("Received DCS in ussd ERR description = %d, converted DCS %d ", cm_struct->uss_error_data.uss_data_coding_scheme,dcs );

    if(dcs == CM_USSD_DCS_8_BIT)
    {
       memscpy( ussd_error_description, sizeof(cm_struct->uss_error_data.ussData), cm_struct->uss_error_data.ussData, cm_struct->uss_error_data.size);
      *ussd_error_description_valid= TRUE;
    }
    else if(dcs == CM_USSD_DCS_UCS2)
    {
      /* Special handling for UCS2 preceded by two GSM 7-bit alphabet. Ignore the first two bytes which represent language indicator. */
       if(0x11 == cm_struct->uss_error_data.uss_data_coding_scheme)
       {
          memscpy(ussd_error_description,sizeof(cm_struct->uss_error_data.ussData),&cm_struct->uss_error_data.ussData[2],(cm_struct->uss_error_data.size-2));
          *ussd_error_description_valid= TRUE;
       }
       else
       {
    ...
    else if(dcs == CM_USSD_DCS_7_BIT)
    {
       ...
       uint16 ussd_utf16_buffer[QMI_VOICE_CM_IF_MAX_USS_DATA_LEN+1];
             //For cm_util_gsm7_to_utf16 buffer needs to be one uint16 bigger
              memset( ussd_utf16_buffer, '\0', sizeof( ussd_utf16_buffer) );
              QM_MSG_HIGH_1("Converting the data to UCS2, length %d", cm_struct->uss_error_data.size);
              (void) cm_util_gsm7_to_utf16(
                cm_struct->uss_error_data.ussData, cm_struct->uss_error_data.size, TRUE,
                ussd_utf16_buffer, sizeof(ussd_utf16_buffer)/sizeof(uint16),
                &response_len_asci);
              if(response_len_asci > QMI_VOICE_USS_DATA_MAX_V02)
              {
                response_len_asci = QMI_VOICE_USS_DATA_MAX_V02;
              }
              memscpy(ussd_error_description,
              sizeof(cm_struct->uss_error_data.ussData), ussd_utf16_buffer,2*(response_len_asci));
             *ussd_error_description_valid= TRUE;
     }
    else
    { 
       memscpy( ussd_error_description, sizeof(cm_struct->uss_error_data.ussData), cm_struct->uss_error_data.ussData, cm_struct->uss_error_data.size);
       *ussd_error_description_valid= TRUE;
       QM_MSG_ERROR_1("Received unhandled DCS in ussd resp= %d ", cm_struct->uss_error_data.uss_data_coding_scheme );
       QM_MSG_HIGH_1("USSD  failure_description copied; len=%d", cm_struct->uss_error_data.size);
     }
    }
  else
  {
    QM_MSG_HIGH_1("USSD failure_description not copied; len=%d", cm_struct->uss_error_data.size);
  }
}
...
)
{
  ...
  QM_MSG_MED_3("Inside map_cm_to_qmi_call_state for cm_state=%d, Sub state Incom(%d), Conv(%d)", 
                  cm_state, 
                  cm_substate.incom, 
                  cm_substate.conv);
  ...
  switch(cm_state)
  {
    ...
    case CM_CALL_STATE_INCOM:
      if(cm_substate.incom == CM_CALL_INCOM_SUBST_SETUP ||
         cm_substate.incom == CM_CALL_INCOM_SUBST_SETUP_PROCESSED)
      {
        qmi_state = QMI_VOICE_CALL_LIST_STATE_SETUP;
      }
      else
      {
...
)
{
  ...
  QM_MSG_MED_1("Inside qmi_voice_cm_util_map_call_state(): for qmi state=%d", state);
  ...
  switch(state)
  {
    ...
    case QMI_VOICE_CALL_LIST_STATE_END:
      ...
      break;
    case QMI_VOICE_CALL_LIST_STATE_PRE_INCOMING:
      call_state = QMI_VOICE_CM_IF_CALL_STATE_PRE_INCOMING;
      break;
    default:
      break;
  }

  QM_MSG_MED_2("Map state qmi call list to qm cm if: 0x%x->0x%x", 
                   state,
                   call_state);
  return call_state;
...
/*===========================================================================
  FUNCTION qmi_voice_cm_util_map_cm_call_type_mask_to_qmi_call_type()

  DESCRIPTION
    Map the CM call type mask to QMI call type
  ...
  SIDE EFFECTS
    None
===========================================================================*/
qmi_voice_cm_if_call_type_e_type qmi_voice_cm_util_map_cm_call_type_mask_to_qmi_call_type
(
...
)
{
  ...
  QM_MSG_MED_1("Inside qmi_voice_cm_util_map_cm_to_qmi_call_type for cm_call_type=%d", call_type_mask);

  switch(call_type_mask)
  {
    case CM_CALL_TYPE_VOICE_MASK:
...
)
{
  ...
  QM_MSG_MED_2("Inside qmi_voice_cm_util_map_sys_mode_to_call_mode(), sys_mode %d, call_mode=%d", mode, call_mode);
  return call_mode;
} /* qmi_voice_cm_util_map_sys_mode_to_call_mode() */

/*===========================================================================
  FUNCTION qmi_voice_cm_util_map_call_mode_to_sys_mode()
...
sys_sys_mode_e_type qmi_voice_cm_util_map_call_mode_to_sys_mode
(
  uint8 mode
)
{
  ...
  switch(mode)
  {
    case QMI_VOICE_CM_IF_MODE_CDMA:
      call_mode = SYS_SYS_MODE_CDMA;
      break;
    case QMI_VOICE_CM_IF_MODE_UMTS:
      call_mode = SYS_SYS_MODE_WCDMA;
      break;
    case QMI_VOICE_CM_IF_MODE_LTE:
      call_mode = SYS_SYS_MODE_LTE;
      break;
    case QMI_VOICE_CM_IF_MODE_GSM:
      call_mode = SYS_SYS_MODE_GSM;
      break;
#ifdef FEATURE_TDSCDMA
    case QMI_VOICE_CM_IF_MODE_TDS:
      call_mode = SYS_SYS_MODE_TDS;
      break;
#endif /*FEATURE_TDSCDMA*/
    case QMI_VOICE_CM_IF_MODE_NO_SRV:
      call_mode = SYS_SYS_MODE_NO_SRV;
      break;
    case QMI_VOICE_CM_IF_MODE_WLAN:
      call_mode = SYS_SYS_MODE_WLAN;
      break;

    #ifdef FEATURE_NR5G_SA
    case QMI_VOICE_CM_IF_MODE_NR5G:
      call_mode = SYS_SYS_MODE_NR5G;
      break;
    #endif
  ...
  QM_MSG_MED_2("Inside qmi_voice_cm_util_map_call_mode_to_sys_mode(), sys_mode %d, call_mode=%d", mode, call_mode);
  return call_mode;
} /* qmi_voice_cm_util_map_call_mode_to_sys_mode() */
...
/*===========================================================================
  FUNCTION qmi_voice_cm_util_map_qmi_end_cause_to_cm_ip_end_reason()
  ...
  SIDE EFFECTS
    None
===========================================================================*/
cm_call_end_e_type qmi_voice_cm_util_map_qmi_end_cause_to_cm_ip_end_reason
(
  qmi_voice_cm_if_end_reason_e_type  ip_end_cause
)
{
  cm_call_end_e_type  end_reason = CM_CALL_END_NONE;
  uint8 i=0;
  uint8 total_size_of_end_reason_table=0;
  ...
  while(i < total_size_of_end_reason_table)
  {
    if(ip_end_cause == qmi_voice_cm_if_end_reason_conv_table[i].reason)
    {
      end_reason = qmi_voice_cm_if_end_reason_conv_table[i].cause;
      break;
    }
    i++;
  }
...
)
{
  ...
  *is_found = TRUE;
  ...
  QM_MSG_HIGH_1("Inside qmi_voice_cm_util_map_cm_to_qmi_ip_call_modified_cause()=%d", cm_call_modified_cause);
  ...
  switch(cm_call_modified_cause)
  {
    ...
    case CM_CALL_MODIFY_CAUSE_DOWNGRADE_DUE_TO_LIPSYNC:
      ...
      break;
    case CM_CALL_MODIFY_CAUSE_UPGRADE_DUE_TO_VIDEO_CRBT:
      qmi_cause = QMI_VOICE_CM_IF_CALL_MODIFIED_CAUSE_UPGRADE_DUE_TO_VIDEO_CRBT;
      break;
    case CM_CALL_MODIFY_CAUSE_DOWNGRADE_DUE_TO_VIDEO_CRBT:
      qmi_cause = QMI_VOICE_CM_IF_CALL_MODIFIED_CAUSE_DOWNGRADE_DUE_TO_VIDEO_CRBT;
      break;
    default:
  ...
  QM_MSG_HIGH_3("qmi_voice_cm_util_map_cm_to_qmi_ip_call_modified_cause cm_call_modified_cause %d qmi_cause %d is_found %d",
                 cm_call_modified_cause,qmi_cause,*is_found);
  return qmi_cause;
...
/*===========================================================================
  ...
  FUNCTION qmi_voice_cm_util_map_call_composer_importance_qmi_to_cm()

  DESCRIPTION
    Map the call composer importance from QMI to CM

  PARAMETERS
    Importance reported to QMI

  RETURN VALUE
    Importance to be sent to IMS

===========================================================================*/
cm_call_composer_importance_e_type qmi_voice_cm_util_map_call_composer_importance_qmi_to_cm(
  call_composer_importance_enum_v02 qmi_imp
)
{
  cm_call_composer_importance_e_type cm_imp = CM_CALL_COMP_IMP_NOT_AVAIL;

  switch(qmi_imp)
  {
    case CALL_COMP_IMP_NORMAL_V02:
      cm_imp = CM_CALL_COMP_IMP_NORMAL;
      break;
    case CALL_COMP_IMP_URGENT_V02:
      cm_imp = CM_CALL_COMP_IMP_URGENT;
      break;
    default:
      break;
  }
  QM_MSG_HIGH_2("QMI->CM: Call_Composer: Imp:  %d->%d",
                   qmi_imp, cm_imp);
...
(
  char                *dest_num_buf_ptr,
  uint32               dest_num_buf_len,
  uint32_t                *dest_len_ptr,
...
FUNCTION qmi_voice_cm_util_get_variant
...
===========================================================================*/
sys_modem_as_id_e_type qmi_voice_cm_util_get_variant(
...
===========================================================================*/
void qmi_voice_util_setup_ind_param_per_sub(
...
)
{
  ...
  cm_util_set_default_voice_bearer_cap_per_sub( bearer_capability_1, asubs_id);
  
  /* Based on TTY mode, iindicates whether CTM text teleophony is supported or not */
  if ( qm_nv_get( NV_TTY_I, &nv_buf, (enum qm_subs_e)asubs_id) == NV_DONE_S &&
       nv_buf.tty != QM_NV_TTY_MODE_OFF )
  {
    bearer_capability_1->ctm = CTM_SUPPORTED;
  }
  else
  {
    bearer_capability_1->ctm = CTM_NOT_SUPPORTED;
  }
  QM_MSG_MED_1("Setting TTY mode in setup indication %d", nv_buf.tty);

  QM_MSG_MED_6("setup_res_params.bearer_capability_1.present = %d info_trans_cap = %d ctm = %d, radio_chnl_req = %d coding_std = %d tf_mode = %d", bearer_capability_1->present, bearer_capability_1->information_transfer_capability, bearer_capability_1->ctm,
  			     bearer_capability_1->radio_channel_requirement, bearer_capability_1->coding_standard, bearer_capability_1->transfer_mode);
  QM_MSG_MED_6("gsm_speech_ver_pref_0 = %d, pref_1 = %d, pref_2 = %d, pref_3 = %d, pref_4 = %d, pref_5 = %d", bearer_capability_1->gsm_speech_ver_pref_0, bearer_capability_1->gsm_speech_ver_pref_1, bearer_capability_1->gsm_speech_ver_pref_2,
  			     bearer_capability_1->gsm_speech_ver_pref_3, bearer_capability_1->gsm_speech_ver_pref_4, bearer_capability_1->gsm_speech_ver_pref_5);
  QM_MSG_MED_3("setup_res_params.bearer_capability_1.compression = %d, structure %d, duplex_mode %d", bearer_capability_1->compression, bearer_capability_1->structure, bearer_capability_1->duplex_mode);
