...
{
  clnt_config_sig_info[clid-1] = (struct qm_nas_clnt_global_s *)QM_MEM_ALLOC(sizeof(struct qm_nas_clnt_global_s));
...
void qm_nas_clnt_config_dealloc ( int32 clid )
{
  if(clnt_config_sig_info[clid-1])
  {
    QM_MEM_FREE(clnt_config_sig_info[clid-1]);
  }
  else
  {
...
/*===========================================================================
  ...
  SIDE EFFECTS
    None
===========================================================================*/
void qm_nas_clnt_send_ind_all( uint16 cmd, void *cstruct, enum qm_subs_e subs, boolean is_ready_due_to_csfb)
{
  qmi_nasi_client_state_type       *cl_sp = NULL;
  dsm_item_type                    *ind = NULL;
  int32                             client;
  struct nas_007B_ind_s           ind_msg;
      ...
      if ( cl_sp->report_status.bound_subs == (enum qmi_nas_subs_e)subs ) // TODO: move bound_subs into qm_nas_clnt
      {
        switch ( cmd )
        {
          case NASI_CMD_VAL_E911_STATE_READY_IND:
            ...
            {
              memset (&ind_msg, 0x00, sizeof(struct nas_007B_ind_s));
                /*Report if the emergency ready is on csfb or not */
                ind_msg.t10.is_ready_due_to_csfb= is_ready_due_to_csfb;
                ind_msg.t10_valid = TRUE;
                ind = NULL;
                QM_MSG_HIGH_1( "Sending E911 State Ready IND to CLID: %d", client);
                if ( qmi_nas_007B_ind_write( &ind_msg, &ind ) == QMI_ERR_NONE)
                {
                  qm_nas_clnt_send_ind( cl_sp->common.clid, NASI_CMD_VAL_E911_STATE_READY_IND, ind );
                }
                else
                {
                  QM_MSG_ERROR_1( "Failed to send E911 State Ready IND: %d", client);
                }
                }
            break;
