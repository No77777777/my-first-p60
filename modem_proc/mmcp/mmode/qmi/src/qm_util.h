...
static uint32 qm_modem_heap_blocks = 0;

#define QM_ERR_FATAL_0(str)            ERR_FATAL( "=MMODE_QMI= " str, 0, 0, 0 )

#if !defined(FEATURE_MMODE_QTF)
#define QM_ASSERT( exp )                                  \
  /*lint -save -e506 -e774 */                               \
  if( !(exp) )                                              \
  /*lint -restore */                                        \
  {                                                         \
    QM_ERR_FATAL_0( "ASSERTION FAILED! " #exp );            \
  }
#else
...
#define QM_MEM_ALLOC_CALLER_ADDRESS(caller_level) ((void *)__builtin_return_address(caller_level))
#else
#define QM_MEM_ALLOC_CALLER_ADDRESS(caller_level) ((void *)__return_address())
#endif
#else
#error code not present
#endif

#define QM_MEM_ALLOC(x) qm_mem_malloc_debug(x)

#if(defined _ARM_ASM_)
#define QM_MEM_FREE(x) qm_mem_free_debug(x, __func__, __LINE__)
#elif (defined __GNUC__) || (defined T_WINNT)
#define QM_MEM_FREE(x) qm_mem_free_debug(x, __FUNCTION__, __LINE__)
#endif
/*===========================================================================
