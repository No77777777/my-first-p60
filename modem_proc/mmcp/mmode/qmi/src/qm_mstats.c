...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/mmode/qmi/src/qm_mstats.c#1 $
...
struct qm_mstats_s
{
  ...
  boolean                nas_is_valid;
  struct nas_mstats_s   *nas_mstats;
};
...
void qm_mstats_timer_expiry_info( sys_nas_timer_expiry_info_s_type timer_info);
void qmi_nasi_ecall_deregistration_rsp(cm_ph_cmd_err_e_type ph_cmd_err);
void qmi_nasi_ecall_timer_restart_resp_msg(cm_ph_cmd_err_e_type ph_cmd_err);
    ...
    if ( subs_id > QM_SUBS_MIN && subs_id < QM_SUBS_MAX )
    {
      switch ( p_mstats_info->type )
      {
        case SYS_MODEM_MODULE_NAS:
          if ( global.mstats[subs_id].nas_mstats == NULL )
          {
            global.mstats[subs_id].nas_mstats = (struct nas_mstats_s *) QM_MEM_ALLOC( sizeof( struct nas_mstats_s ) );
            memset( global.mstats[subs_id].nas_mstats, 0x00, sizeof(struct nas_mstats_s ) );
          }

          if( p_mstats_info->prm.nas_info.changed_bitmask & SYS_NAS_TIMER_EXPIRY_INFO_MASK )
          {
            memscpy( &global.mstats[subs_id].nas_mstats->timer_expiry_info, sizeof(sys_nas_timer_expiry_info_s_type), &p_mstats_info->prm.nas_info.timer_expiry_info, sizeof(sys_nas_timer_expiry_info_s_type) );
            QM_MSG_HIGH("Received Stats from NAS indicating that the ECALL timer has expired");
            qm_mstats_timer_expiry_info(global.mstats[subs_id].nas_mstats->timer_expiry_info);
          }

          if(p_mstats_info->prm.nas_info.changed_bitmask & SYS_NAS_REGISTRATION_INFO_MASK)
          {
            QM_MSG_HIGH("Receved Indication from NAS that response for Ecall Deregistration can be sent");
            if(p_mstats_info->prm.nas_info.reg_nw_info.ecall_inactivity_completed == SYS_DEREG_SUCCESS)
            {
              qmi_nasi_ecall_deregistration_rsp(CM_PH_CMD_ERR_NOERR);
            }
            else if(p_mstats_info->prm.nas_info.reg_nw_info.ecall_inactivity_completed == SYS_DEREG_ERROR)
            {
              qmi_nasi_ecall_deregistration_rsp(CM_PH_CMD_ERR_PREF_TERM_P);
            }
          }

          if(p_mstats_info->prm.nas_info.changed_bitmask & SYS_NAS_INACTIVITY_TIMER_RESUME_INFO_MASK)
          {
            QM_MSG_HIGH("Received Indication from NAS that response for ECALL Timer Restart can be sent");
            if(p_mstats_info->prm.nas_info.ecall_inactivity_timer_response == SYS_INACTIVITY_TIMER_SUCCESS)
            {
              qmi_nasi_ecall_timer_restart_resp_msg(CM_PH_CMD_ERR_NOERR);
            }
            else if(p_mstats_info->prm.nas_info.ecall_inactivity_timer_response == SYS_INACTIVITY_TIMER_ERROR)
            {
              qmi_nasi_ecall_timer_restart_resp_msg(CM_PH_CMD_ERR_PREF_TERM_P);
            }
          }
          global.mstats[subs_id].nas_is_valid = TRUE;
          break;
        case SYS_MODEM_MODULE_CDMA:
          ...
          {
            global.mstats[subs_id].cdma_mstats = (struct cdma_mstats_s *) QM_MEM_ALLOC( sizeof( struct cdma_mstats_s ) );
        ...
        case SYS_MODEM_MODULE_HDR:
          ...
          {
            global.mstats[subs_id].hdr_mstats = (struct hdr_mstats_s *) QM_MEM_ALLOC( sizeof( struct hdr_mstats_s ) );
        ...
        case SYS_MODEM_MODULE_GSM:
          ...
          {
            global.mstats[subs_id].gsm_mstats = (struct gsm_mstats_s *) QM_MEM_ALLOC( sizeof( struct gsm_mstats_s ) );
        ...
        case SYS_MODEM_MODULE_WCDMA:
          ...
          {
            global.mstats[subs_id].wcdma_mstats = (struct wcdma_mstats_s *) QM_MEM_ALLOC( sizeof( struct wcdma_mstats_s ) );
...
{
  QM_MSG_HIGH_2("invalidate_inactive_rats rat mask %d, sub %d", inactive_rat_mask, subs_id);

  if( subs_id == QM_SUBS_MIN )
  {
    subs_id = QM_SUBS_PRI;
  }
...
void qm_mstats_populate_cell_loc_info ( nas_get_cell_location_info_resp_msg_v01 *rsp_msg, enum qm_subs_e subs_id )
{
  ...
  if( qm_si_is_rat_active(subs_id, QM_RAT_UMTS) )
  {
    ...
    if( global.mstats[subs_id].wcdma_is_valid )
    {
      ...
      {
        QM_MSG_HIGH("cell id is invalid. call cm_ss_cmd_get_dch_cell_info_per_sub");
        if ( !cm_ss_cmd_get_dch_cell_info_per_sub( NULL, NULL, qmi_nasi_global.cm_clnt_id ,(sys_modem_as_id_e_type)subs_id) )
        {
          QM_MSG_ERROR("cm_ss_cmd_get_dch_cell_info_per_sub failed");
        }
      }
#endif
