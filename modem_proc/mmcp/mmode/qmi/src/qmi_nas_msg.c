...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/mmode/qmi/src/qmi_nas_msg.c#2 $ $Change: 39419910 $ $DateTime: 2022/09/01 04:52:39 $ $Author: pwbldsvc $
...
#include "qm_util.h"
#define QMI_GPP_WIFI_CELLULAR_SCAN_MIN_TIMER 5
#define QMI_GPP_WIFI_CELLULAR_SCAN_MAX_TIMER 20
#define QMI_GPP_WIFI_CELLULAR_SCAN_DISABLE_TIMER 65535
...
)
{
  ...
  if (!(p_msg->reg_sys_sel_pref_valid      || p_msg->reg_ddtm_events_valid        || p_msg->req_serving_system_valid  ||
        ...
        p_msg->reg_t3346_timer_status_change_valid  || p_msg->reg_call_mode_change_valid || p_msg->reg_emergency_mode_status_ind_valid ||
	p_msg->reg_timer_expiry_ind_valid || p_msg->reg_gcell_info_ind_valid || p_msg->reg_data_subs_priority_change_valid ||
	p_msg->reg_ssac_change_info_valid || p_msg->reg_manual_scan_fail_valid || p_msg->reg_lte_rach_fail_ind_valid ||
	p_msg->reg_e911_search_fail_ind_valid || p_msg->reg_e911_search_fail_ind_valid || p_msg->reg_arfcn_list_ind_valid
        || p_msg->reg_access_barring_status_valid || p_msg->reg_cell_barring_info_valid)
      )
  {
...
qmi_error_e_type qmi_nas_0030_req_check (nas_set_device_config_req_msg_v01 *p_msg)
{
  ...
  if( !p_msg->spc_valid && !p_msg->force_hdr_rev0_valid &&
      !p_msg->hdr_scp_config_valid && !p_msg->roam_pref_valid && !p_msg->gpp_wifi_cellular_scan_timer_valid)
  {
    return QMI_ERR_MISSING_ARG;
  }
  ...
  if(p_msg->gpp_wifi_cellular_scan_timer_valid)
  {
    if(p_msg->gpp_wifi_cellular_scan_timer != QMI_GPP_WIFI_CELLULAR_SCAN_DISABLE_TIMER &&
      (p_msg->gpp_wifi_cellular_scan_timer < QMI_GPP_WIFI_CELLULAR_SCAN_MIN_TIMER ||
       p_msg->gpp_wifi_cellular_scan_timer > QMI_GPP_WIFI_CELLULAR_SCAN_MAX_TIMER))
    {
      return QMI_ERR_INVALID_ARG;
    }
   }
...
#ifdef FEATURE_MMODE_LTE_BANDWIDTH
  if (p_msg->t12_valid == TRUE)
  {
    memscpy(&nas_0031_rsp_p.t12.num_instances, sizeof(nas_0031_rsp_p.t12.num_instances),
            &p_msg->t12.num_instances, sizeof(nas_0031_rsp_p.t12.num_instances));

    for(count = 0; count < p_msg->t12.num_instances; count++)
    {
      memscpy((void *) &nas_0031_rsp_p.t12.instances[count].radio_if, sizeof(nas_0031_rsp_p.t12.instances[count].radio_if),
              (void *) &p_msg->t12.instances[count].radio_if, sizeof(nas_0031_rsp_p.t12.instances[count].radio_if));
      memscpy((void *) &nas_0031_rsp_p.t12.instances[count].bandwidth, sizeof(nas_0031_rsp_p.t12.instances[count].bandwidth),
              (void *) &p_msg->t12.instances[count].bandwidth, sizeof(nas_0031_rsp_p.t12.instances[count].bandwidth));
    }

    if (FALSE == qmi_svc_put_param_tlv(response,
                                       NAS_0031_RSP_T12,
                                       sizeof(nas_0031_rsp_p.t12.num_instances)
                                       + nas_0031_rsp_p.t12.num_instances * (sizeof(uint8) + sizeof(nas_rf_bandwidth_e_type)),
                                       &nas_0031_rsp_p.t12))
    {
      QM_MSG_ERROR("Could not write RF bandwidth info list (extended): Out of memory");
...
)
{
  ...
  if ( p_msg->lte_sig_rpt_config_valid )
  {
    if ( p_msg->lte_sig_rpt_config.rpt_rate   > NAS_LTE_SIG_RPT_RATE_5_SEC_V01 ||
         p_msg->lte_sig_rpt_config.avg_period > NAS_LTE_SIG_AVG_PRD_10_SEC_V01  )
    {
      return QMI_ERR_INVALID_ARG_V01;
    }
  }
...
)
{
  ...
  if ( ( p_msg->cdma_rssi_threshold_list_valid    && p_msg->cdma_rssi_threshold_list_len    > MAX_NUM_SIG_INFO2_THRESHOLDS ) ||
...
qmi_error_type_v01 qmi_nas_0072_req_check(nas_update_ims_status_req_msg_v01 *p_msg)
{
  ...
  if (p_msg->sys_mode != NAS_RADIO_IF_CDMA_1XEVDO_V01 &&
      ...
      p_msg->sys_mode != NAS_RADIO_IF_WLAN_V01)
  {
...
/*===========================================================================
  FUNCTION QMI_NAS_007B_IND_WRITE()

  DESCRIPTION
  Writes NASI_CMD_VAL_E911_STATE_READY_IND response in struct nas_007B_ind_s
  ...
  None
  ===========================================================================*/
qmi_error_e_type qmi_nas_007B_ind_write(struct nas_007B_ind_s * p_msg,dsm_item_type * * response)
{
  struct nas_007B_ind_p_s ind_p;
  ...
  memset(&ind_p, 0, sizeof(ind_p));

  if ( p_msg->t10_valid )
  {
    memscpy((void *)&ind_p.t10.is_ready_due_to_csfb, sizeof(p_msg->t10.is_ready_due_to_csfb),
            (void *)&p_msg->t10.is_ready_due_to_csfb, sizeof(p_msg->t10.is_ready_due_to_csfb));

    if ( !qmi_svc_put_param_tlv( response, NAS_007B_IND_T10, sizeof(ind_p.t10), &ind_p.t10) )
    {
      QM_MSG_ERROR("Could not write t10");
      dsm_free_packet(response);
      return QMI_ERR_NO_MEMORY;
    }
  }
...
qmi_error_type_v01 qmi_nas_0085_req_check( nas_perform_incremental_network_scan_req_msg_v01 *p_msg)
{
  ...
  /* Periodic frequency search error checking
  */

  QM_MSG_HIGH_8("srch_periodicity %d %d max_search_time %d %d incr_results_periodicity %d %d incr_results %d %d",
      p_msg->search_periodicity_valid, p_msg->search_periodicity, 
      p_msg->max_search_time_valid, p_msg->max_search_time,
      p_msg->incremental_results_periodicity_valid, p_msg->incremental_results_periodicity,
      p_msg->incremental_results_valid, p_msg->incremental_results);

  if (p_msg->incremental_results_valid &&
      p_msg->incremental_results == FALSE && 
      p_msg->incremental_results_periodicity_valid &&
      p_msg->incremental_results_periodicity != 0
      )
  {
    return QMI_ERR_INVALID_ARG_V01;
  }

  if (!p_msg->search_periodicity_valid && 
       p_msg->max_search_time_valid &&
       p_msg->max_search_time >= MAX_SEARCH_TIME_ONE_TIME_SEARCH_MAX_VALUE)
  {
    return QMI_ERR_INVALID_ARG_V01;
  }

  if(p_msg->max_search_time_valid &&
      (p_msg->max_search_time == 0 ||
       p_msg->max_search_time > MAX_SEARCH_TIME_MAX_VALUE))
    {
      return QMI_ERR_INVALID_ARG_V01;
    }
      
  if(p_msg->wcdma_arfcn_valid ||
     p_msg->lte_arfcn_valid ||
     p_msg->search_periodicity_valid ||
     p_msg->preferred_plmn_list_valid)
  {

      
    if(!p_msg->scan_type_valid ||
       (p_msg->scan_type != NAS_SCAN_TYPE_PLMN_V01 &&
        p_msg->scan_type != NAS_SCAN_TYPE_MODE_PREF_V01))
    {
      return QMI_ERR_INVALID_ARG_V01;
    }
    ...
    if(p_msg->wcdma_arfcn_valid &&
      (p_msg->wcdma_arfcn_len == 0 ||
       p_msg->wcdma_arfcn_len > NAS_WCDMA_ARFCN_LIST_MAX_V01))
    {
      return QMI_ERR_INVALID_ARG_V01;
    }

    if(p_msg->lte_arfcn_valid &&
      (p_msg->lte_arfcn_len == 0 ||
       p_msg->lte_arfcn_len > NAS_LTE_ARFCN_LIST_MAX_V01))
    {
      return QMI_ERR_INVALID_ARG_V01;
    }

    if(p_msg->search_periodicity_valid &&
       p_msg->search_periodicity != 0 &&
      (p_msg->search_periodicity < SEARCH_PERIODICITY_MIN_VALUE ||
       p_msg->search_periodicity > SEARCH_PERIODICITY_MAX_VALUE))
    {
      return QMI_ERR_INVALID_ARG_V01;
    }      
      
    if(p_msg->preferred_plmn_list_valid &&
       p_msg->preferred_plmn_list_len > MAX_PREFERRED_PLMN_LIST_SIZE)
    {
      return QMI_ERR_INVALID_ARG_V01;
    }
  }

  if(p_msg->incremental_results_periodicity_valid &&
   (!p_msg->incremental_results_valid ||
     p_msg->incremental_results == TRUE) &&
    (p_msg->incremental_results_periodicity == 0 ||
     p_msg->incremental_results_periodicity > INC_RESULTS_PERIODICITY_MAX_VALUE))
  {
    return QMI_ERR_INVALID_ARG_V01;
  }
