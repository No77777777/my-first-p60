...
$Header: //components/rel/mmcp.mpss/7.9.0/mmode/qmi/src/qmi_nas_mmgsdi.c#1 $ $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...
)
{
  ...
  switch(qmi_session)
  {
    ...
    case QMI_NAS_MMGSDI_1x_SEC_SESSION:
#ifdef FEATURE_TRIPLE_SIM
...
)
{
  ...
  switch(qmi_session)
  {
    ...
    case QMI_NAS_MMGSDI_GW_SEC_SESSION:
      ...
      break;
    case QMI_NAS_MMGSDI_1x_SEC_SESSION:
      retval = MMGSDI_1X_PROV_SEC_SESSION;
      break;
#ifdef FEATURE_TRIPLE_SIM
    case QMI_NAS_MMGSDI_GW_TER_SESSION:
...
)
{
  ...
  switch(session)
  {
    ...
    case MMGSDI_GW_PROV_SEC_SESSION:
      ...
      break;
    case MMGSDI_1X_PROV_SEC_SESSION:
      qmi_session = QMI_NAS_MMGSDI_1x_SEC_SESSION;
      break;
#ifdef FEATURE_TRIPLE_SIM
    case MMGSDI_GW_PROV_TER_SESSION:
...
{
  struct qmi_nas_mmgsdi_cmd_rsp_cb_s *ptr = NULL;

  if ( p_info )
  {
    ptr = QM_MEM_ALLOC( sizeof(*ptr) );
    ...
    if ( ptr )
    {
      ...
      QM_MSG_MED_3("response from mmgsdi: status %d cmd %d type %d",
                   status, cmd, p_info->response_header.response_type);
        ...
        case MMGSDI_SESSION_GET_OPERATOR_NAME_CNF:
          if ( p_info->session_get_operator_name_cnf.response_header.mmgsdi_status == MMGSDI_SUCCESS )
          {
            ...
            for(cnt = 0; cnt < p_info->session_get_operator_name_cnf.plmn_list.num_of_plmn_ids; cnt++)
            {
              ...
              {
                ptr->info.session_get_operator_name_cnf.plmn_list.plmn_info_ptr[cnt].plmn_long_name.plmn_name.eons_data.data_ptr = NULL;
              }
              ...
              {
                ptr->info.session_get_operator_name_cnf.plmn_list.plmn_info_ptr[cnt].plmn_short_name.plmn_name.eons_data.data_ptr = NULL;
              }
              ...
              {
                ptr->info.session_get_operator_name_cnf.plmn_list.plmn_info_ptr[cnt].plmn_additional_info.info_data.data_ptr = NULL;
              }
              ...
              for (add_cnt = 0; add_cnt < p_info->session_get_operator_name_cnf.plmn_list.plmn_info_ptr[cnt].num_plmn_additional_names; add_cnt++ )
              {
                ...
                {
                  ptr->info.session_get_operator_name_cnf.plmn_list.plmn_info_ptr[cnt].plmn_additional_names[add_cnt].plmn_long_name.plmn_name.eons_data.data_ptr = NULL;
                }
                ...
                {
                  ptr->info.session_get_operator_name_cnf.plmn_list.plmn_info_ptr[cnt].plmn_additional_names[add_cnt].plmn_short_name.plmn_name.eons_data.data_ptr = NULL;
                }
              }
            }
          }
...
{
  mmgsdi_event_data_type *ptr = NULL;
  int i = 0;

  if (p_evt)
  {
    QM_MSG_MED_1("event from mmgsdi %d", p_evt->evt);
  ...
  mmgsdi_file_enum_type id )
{
  switch (id)
  {
    ...
    case MMGSDI_CSIM_SPN:
      return QMI_NAS_MMGSDI_EF_CDMA_SPN;
...
{
  enum qmi_nas_mmgsdi_cache_ef_e ef = QMI_NAS_MMGSDI_EF_NONE;
...
boolean qmi_nas_mmgsdi_register_refresh( qmi_nas_mmgsdi_session_e_type qmi_session )
{
  ...
  memset( &refresh_files, 0x00, sizeof(refresh_files) );
...
{
  struct valid_success_s         rsp_check = {0};
  boolean                        success = FALSE;
  int                            is_diff = 0, idx = 0;
  enum qmi_nas_mmgsdi_cache_ef_e ef = QMI_NAS_MMGSDI_EF_NONE;
  mmgsdi_session_type_enum_type  session;
  qmi_nas_mmgsdi_session_e_type  qmi_session = QMI_NAS_MMGSDI_SESSION_MIN;
...
void qmi_nas_mmgsdi_process_regular_ef_read_cnf( const struct qmi_nas_mmgsdi_cmd_rsp_cb_s *p_rsp )
{
  ...
  switch (p_rsp->info.read_cnf.access.file.file_enum)
  {
    ...
    {
      mmgsdi_cnf_type *cnf_data =  QM_MEM_ALLOC( sizeof(p_rsp->info) );
...
void qmi_nas_mmgsdi_process_cmd_rsp( qmi_mmode_cmd_enum_type cmd_type, void *user_data_ptr )
{
  ...
  if (ptr->cmd != MMGSDI_READ_CNF)
  {
    QM_MSG_MED_6("mmgsdi cmd rsp latest evt %d %d %d, cmd %d type %d status %d",
                 qmi_nasi_global.mmgsdi.latest_evt[0], qmi_nasi_global.mmgsdi.latest_evt[1],
                 qmi_nasi_global.mmgsdi.latest_evt[2], ptr->cmd, ptr->info.response_header.response_type,
                 ptr->status);
  }
  else
  {
    QM_MSG_MED_8("mmgsdi cmd rsp latest evt %d %d %d, cmd %d type %d status %d file_name %d size_read %",
                 qmi_nasi_global.mmgsdi.latest_evt[0], qmi_nasi_global.mmgsdi.latest_evt[1],
                 qmi_nasi_global.mmgsdi.latest_evt[2], ptr->cmd, ptr->info.response_header.response_type,
                 ptr->status, ptr->info.read_cnf.access.file.file_enum,
                 ptr->info.read_cnf.read_data.data_len);
  }
  ...
  if ( rsp_check.valid )
  {
    switch ( ptr->cmd )
    {
      case MMGSDI_CLIENT_ID_AND_EVT_REG_CNF:
        ...
        {
          uint32    session_type_mask = MMGSDI_GW_PROV_PRI_SESSION_MASK | MMGSDI_GW_PROV_SEC_SESSION_MASK | MMGSDI_1X_PROV_PRI_SESSION_MASK | MMGSDI_1X_PROV_SEC_SESSION_MASK;
#ifdef FEATURE_TRIPLE_SIM
      ...
      case MMGSDI_SESSION_OPEN_EXT_CNF:
        if ( rsp_check.success )
        {
          ...
          {
            QM_MEM_FREE ( ptr->info.session_open_ext_cnf.session_info_ptr );
            ptr->info.session_open_ext_cnf.session_info_ptr = NULL;
      ...
      case MMGSDI_READ_CNF:
        ...
        QM_MEM_FREE ( ptr->info.read_cnf.read_data.data_ptr );
      ...
      {
        mmgsdi_cnf_type *cnf_data =  QM_MEM_ALLOC( sizeof(ptr->info) );
      ...
      case MMGSDI_SESSION_GET_OPERATOR_NAME_CNF:
      {
        ...
        if (  ptr->info.session_get_operator_name_cnf.response_header.mmgsdi_status == MMGSDI_SUCCESS )
        {
          ...
          if ( ptr->info.session_get_operator_name_cnf.plmn_list.plmn_info_ptr )
          {
            ...
            {
              QM_MEM_FREE ( ptr->info.session_get_operator_name_cnf.plmn_list.plmn_info_ptr[cnt].plmn_long_name.plmn_name.eons_data.data_ptr );
              QM_MEM_FREE ( ptr->info.session_get_operator_name_cnf.plmn_list.plmn_info_ptr[cnt].plmn_short_name.plmn_name.eons_data.data_ptr );
              QM_MEM_FREE ( ptr->info.session_get_operator_name_cnf.plmn_list.plmn_info_ptr[cnt].plmn_additional_info.info_data.data_ptr );
              ...
              {
                QM_MEM_FREE ( ptr->info.session_get_operator_name_cnf.plmn_list.plmn_info_ptr[cnt].plmn_additional_names[add_cnt].plmn_long_name.plmn_name.eons_data.data_ptr );
                QM_MEM_FREE ( ptr->info.session_get_operator_name_cnf.plmn_list.plmn_info_ptr[cnt].plmn_additional_names[add_cnt].plmn_short_name.plmn_name.eons_data.data_ptr );
              }
            }
            QM_MEM_FREE ( ptr->info.session_get_operator_name_cnf.plmn_list.plmn_info_ptr );
          }
          QM_MEM_FREE ( ptr->info.session_get_operator_name_cnf.spn.eons_data.data_ptr );
        }
      }
      break;
      ...
      case MMGSDI_GET_SE13_PLMN_NAMES_CNF:
      {
        ...
        if (  ptr->info.get_se13_plmn_names_cnf.response_header.mmgsdi_status == MMGSDI_SUCCESS )
        {
          ...
          if ( ptr->info.get_se13_plmn_names_cnf.plmn_list.plmn_info_ptr )
          {
            ...
            {
              QM_MEM_FREE ( ptr->info.get_se13_plmn_names_cnf.plmn_list.plmn_info_ptr[cnt].plmn_long_name.plmn_name.eons_data.data_ptr );
              QM_MEM_FREE ( ptr->info.get_se13_plmn_names_cnf.plmn_list.plmn_info_ptr[cnt].plmn_short_name.plmn_name.eons_data.data_ptr );
              QM_MEM_FREE ( ptr->info.get_se13_plmn_names_cnf.plmn_list.plmn_info_ptr[cnt].plmn_additional_info.info_data.data_ptr );

              for(add_cnt = 0; add_cnt < ptr->info.get_se13_plmn_names_cnf.plmn_list.plmn_info_ptr[cnt].num_plmn_additional_names; add_cnt++)
              {
                QM_MEM_FREE ( ptr->info.get_se13_plmn_names_cnf.plmn_list.plmn_info_ptr[cnt].plmn_additional_names[add_cnt].plmn_long_name.plmn_name.eons_data.data_ptr );
                QM_MEM_FREE ( ptr->info.get_se13_plmn_names_cnf.plmn_list.plmn_info_ptr[cnt].plmn_additional_names[add_cnt].plmn_short_name.plmn_name.eons_data.data_ptr );
              }
            }
            QM_MEM_FREE ( ptr->info.get_se13_plmn_names_cnf.plmn_list.plmn_info_ptr );
          }
        }
      }
      break;
...
void qmi_nas_mmgsdi_process_evt( qmi_mmode_cmd_enum_type cmd_type, void *user_data_ptr )
{
  ...
  QM_MSG_MED_7("mmgsdi client id %d, session id %d %d %d, session type %d %d %d",qmi_nasi_global.mmgsdi.client_id, qmi_nasi_global.mmgsdi.session_id[0], qmi_nasi_global.mmgsdi.session_id[1], qmi_nasi_global.mmgsdi.session_id[2],
               qmi_nasi_global.mmgsdi.session_type[0], qmi_nasi_global.mmgsdi.session_type[1], qmi_nasi_global.mmgsdi.session_type[2]);
  QM_MSG_MED_6("mmgsdi app type %d %d %d, fcn refresh %d %d %d", qmi_nasi_global.mmgsdi.app_type[0], qmi_nasi_global.mmgsdi.app_type[1], qmi_nasi_global.mmgsdi.app_type[2],
               qmi_nasi_global.mmgsdi.fcn_refresh[0], qmi_nasi_global.mmgsdi.fcn_refresh[1], qmi_nasi_global.mmgsdi.fcn_refresh[2]);
  QM_MSG_MED_9("mmgsdi latest evt %d %d %d, operation ready %d %d %d event %d session %d %d", qmi_nasi_global.mmgsdi.latest_evt[0], qmi_nasi_global.mmgsdi.latest_evt[1], qmi_nasi_global.mmgsdi.latest_evt[2],
               ...
               ptr->evt, session, ptr->session_id);
  ...
  if ( qmi_nas_mmgsdi_is_valid_session(session) )
  {
    ...
    switch ( ptr->evt )
    {
      case MMGSDI_SUBSCRIPTION_READY_EVT:
        ...
        if ( session == MMGSDI_1X_PROV_PRI_SESSION || session == MMGSDI_1X_PROV_SEC_SESSION ) // for 1x, no updating cache
        {
          mmgsdi_access_type      file_access;
      ...
      case MMGSDI_REFRESH_EVT:
        ...
        // free deep copied items
        QM_MEM_FREE ( ptr->data.refresh.refresh_files.file_list_ptr );
        QM_MEM_FREE ( ptr->data.refresh.refresh_files.file_path_ptr );
      ...
      case MMGSDI_EONS_EVT:
      {
        ...
        // free deep copied items
        QM_MEM_FREE( ptr->data.eons.spn.eons_data.data_ptr );
        if ( ptr->data.eons.first_stack_info.is_valid )
        {
          QM_MEM_FREE( ptr->data.eons.first_stack_info.operator_info.plmn_long_name.plmn_name.eons_data.data_ptr );
          QM_MEM_FREE( ptr->data.eons.first_stack_info.operator_info.plmn_short_name.plmn_name.eons_data.data_ptr );
          QM_MEM_FREE( ptr->data.eons.first_stack_info.operator_info.plmn_additional_info.info_data.data_ptr );

          for (i = 0; i < ptr->data.eons.first_stack_info.operator_info.num_plmn_additional_names; i++)
          {
            QM_MEM_FREE( ptr->data.eons.first_stack_info.operator_info.plmn_additional_names[i].plmn_long_name.plmn_name.eons_data.data_ptr );
            QM_MEM_FREE( ptr->data.eons.first_stack_info.operator_info.plmn_additional_names[i].plmn_short_name.plmn_name.eons_data.data_ptr );
          }
        }
        if ( ptr->data.eons.sec_stack_info.is_valid )
        {
          QM_MEM_FREE( ptr->data.eons.sec_stack_info.operator_info.plmn_long_name.plmn_name.eons_data.data_ptr );
          QM_MEM_FREE( ptr->data.eons.sec_stack_info.operator_info.plmn_short_name.plmn_name.eons_data.data_ptr );
          QM_MEM_FREE( ptr->data.eons.sec_stack_info.operator_info.plmn_additional_info.info_data.data_ptr );
          for (i = 0; i < ptr->data.eons.sec_stack_info.operator_info.num_plmn_additional_names; i++)
          {
            QM_MEM_FREE( ptr->data.eons.sec_stack_info.operator_info.plmn_additional_names[i].plmn_long_name.plmn_name.eons_data.data_ptr );
            QM_MEM_FREE( ptr->data.eons.sec_stack_info.operator_info.plmn_additional_names[i].plmn_short_name.plmn_name.eons_data.data_ptr );
          }
        }
      }
      break;
