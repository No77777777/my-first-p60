    ...
    else
    {
      ...
      if ( ret == NV_DONE_S )
      {
        ...
        else
        {
          ...
          ret = qmi_mmode_put_nv_item_ext( NV_TTY_I, data, (uint16)subs );
  ...
  if ( (QM_SUBS_MIN < subs) && (subs < QM_SUBS_MAX) && (data->tty < QM_NV_TTY_MODE_MAX))
  {
    ...
    if(write_to_nv)
    {
      ...
      if ( ret == NV_DONE_S )
      {
        ...
        msgr_init_hdr_variant( (msgr_hdr_struct_type *)&msg_tty_pkt, MSGR_QMI_VOICE,
                               QMI_VOICE_VOICE_TTY_MODE_CHANGE_IND_IND, subs );
