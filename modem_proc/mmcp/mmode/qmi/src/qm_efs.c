...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/mmode/qmi/src/qm_efs.c#1 $
  $Change: 31300932 $
...
#define QM_EFS_CONF_F_NAME "/nv/item_files/conf/mmode_qmi.conf"
...
// filesize = 2 bytes, valid values are 0,1,2 binary.
#define EFS_ENHANCED_F3_DBG_RULE_FILE "/nv/item_files/modem/mmode/qmi/mmode_qmi_f3_dbg_rule"
...
void qm_efs_init( void )
{
  ...
  int conf_fd = -1;
  int result = -1;
  char *mmode_qmi_config_items = "/nv/item_files/modem/mmode/qmi/mmode_qmi_f3_dbg_rule\n\
";

  /* Create NV folder if it doesnt exist
  */
  (void)mcfg_mkdir("/nv",0777, MCFG_FS_TYPE_EFS);
  QM_MSG_HIGH_1("create nv directory, efs_errno=%d", mcfg_fs_errno(MCFG_FS_TYPE_EFS));

  /* Create item_files folder if it doesnt exist
  */
  (void)mcfg_mkdir("/nv/item_files",0777, MCFG_FS_TYPE_EFS);
  QM_MSG_HIGH_1("create directory /nv/item_files, efs_errno=%d", mcfg_fs_errno(MCFG_FS_TYPE_EFS));

  /* Create conf folder if it doesn't exist
  */
  (void)mcfg_mkdir("/nv/item_files/conf",0777, MCFG_FS_TYPE_EFS);
  QM_MSG_HIGH_1("create directory /nv/item_files/conf, efs_errno=%d", mcfg_fs_errno(MCFG_FS_TYPE_EFS));

  /* Create modem folder if it doesnt exist
  */
  (void)mcfg_mkdir("/nv/item_files/modem",0777, MCFG_FS_TYPE_EFS);
  QM_MSG_HIGH_1("create directory /nv/item_files/modem, efs_errno=%d", mcfg_fs_errno(MCFG_FS_TYPE_EFS));

  /* Create mmode folder if it doesnt exist
  */
  (void)mcfg_mkdir("/nv/item_files/modem/mmode",0777, MCFG_FS_TYPE_EFS);
  QM_MSG_HIGH_1("create directory /nv/item_files/modem/mmode, efs_errno=%d", mcfg_fs_errno(MCFG_FS_TYPE_EFS));

  /* Create qmi folder if it doesnt exist
  */
  (void)mcfg_mkdir("/nv/item_files/modem/mmode/qmi",0777, MCFG_FS_TYPE_EFS);
  QM_MSG_HIGH_1("create directory /nv/item_files/modem/mmode/qmi, efs_errno=%d", mcfg_fs_errno(MCFG_FS_TYPE_EFS));

  /* Create mmode_qmi.conf file */
  if( (mcfg_fs_stat(QM_EFS_CONF_F_NAME, &file_stat, MCFG_FS_TYPE_EFS, MCFG_FS_SUBID_NONE)
       != MCFG_FS_STATUS_OK)
      && mcfg_fs_errno(MCFG_FS_TYPE_EFS) == ENOENT )
  {
    QM_MSG_HIGH("mmode_qmi.conf not present yet, creating it now");

    /* Open /nv/item_files/mmode_qmi.conf file */
    conf_fd =  mcfg_fopen (QM_EFS_CONF_F_NAME,
    ...
    if ( conf_fd >= 0 )
    {
      (void)mcfg_fwrite(conf_fd,
                        mmode_qmi_config_items,
                        strlen(mmode_qmi_config_items),
                        MCFG_FS_TYPE_EFS);

    }
    else
    {
      QM_MSG_ERROR_2("Failed to open mmode_qmi.conf efs_err=%d errno %d", conf_fd, mcfg_fs_errno(MCFG_FS_TYPE_EFS));
    }
    (void)mcfg_fclose(conf_fd, MCFG_FS_TYPE_EFS);
    QM_MSG_HIGH_1("mmode_qmi.conf file fd %d",conf_fd);
  }
  ...
  qm_efs_read_enhanced_f3_dbg_rule();
} // qm_efs_init()
...
boolean qm_efs_csg_supported ( void )
{
  ...
  if ( !efs_global.csg_supported.is_file_read )
  {
    ...
    QM_MSG_HIGH_3("mcfg_status %d, csg enum %d, csg supported %d",
                  status, efs_global.csg_supported.is_supported, ret_val);
  ...
  if ( ui_tty_setting <= TTY_MODE_OFF_V02 )
  {
    ...
    if(write_to_efs)
    {
      ...
      if ( ret == TRUE ) /* No error in efs_write() */
      {
        ...
        msgr_init_hdr_variant( (msgr_hdr_struct_type *)&msg_ui_tty_pkt, MSGR_QMI_VOICE,
                               QMI_VOICE_UI_TTY_MODE_SETTING_CMD, MSGR_NO_VARIANT );
...
)
{
  ...
  qmi_voice_cm_if_mcfg_refresh_otasp(subs_index);
...
FUNCTION qm_efs_read_enhanced_f3_dbg_rule()
...
===========================================================================*/
void qm_efs_read_enhanced_f3_dbg_rule(void)
{
  mcfg_fs_status_e_type status = 0;

  status = mcfg_fs_read(EFS_ENHANCED_F3_DBG_RULE_FILE, (void *)&efs_global.enhanced_f3_dbg_rule, (fs_size_t) sizeof(efs_global.enhanced_f3_dbg_rule), MCFG_FS_TYPE_EFS, MCFG_FS_SUBID_0);
  ...
  if(status != MCFG_FS_STATUS_OK)
  {
    QM_MSG_HIGH_2("Read enhanced_f3_dbg_rule EFS failed. Set to default value 0, status %d, sizeof %d",
                  status, sizeof(efs_global.enhanced_f3_dbg_rule));
    efs_global.enhanced_f3_dbg_rule = 0;
  }
  else
  {
    QM_MSG_HIGH_2("status=%d, enhanced_f3_dbg_rule =%d", status, efs_global.enhanced_f3_dbg_rule);
  }
  #endif
}
...
FUNCTION qm_efs_get_enhanced_f3_dbg_rule()

DESCRIPTION
  Get the rule on enhanced debug messages.
...
===========================================================================*/
uint16 qm_efs_get_enhanced_f3_dbg_rule(void)
{
  return efs_global.enhanced_f3_dbg_rule;
}
