...
// DESCRIPTION:
//
//  $Header: //components/rel/mmcp.mpss/7.9.0/mmode/qmi/src/qmi_nas_hdr.c#1 $
//
...
{
#ifdef FEATURE_HDR
  hdrerrno_enum_type ret_val = 1;
  int i;

  // Register for call callbacks
  for ( i = HDRMC_CB_EVENT_SESSION_CLOSE; i <= HDRMC_CB_EVENT_UATI_UPDATE; i++)
  {
    ret_val = hdrcp_mc_client_cb_reg(HDRMC_CB_CLIENT_QMI,
                                     (hdrcp_mc_cb_event_enum_type)i,
...
//===========================================================================
//
void qmi_nas_hdr_evt_cb( hdrcp_mc_cb_params_rsp_union_type *parms, void *client_data )
{
  struct qmi_nas_cmd_hdr_payload *payload = NULL;
  ...
  payload = (struct qmi_nas_cmd_hdr_payload *)QM_MEM_ALLOC( sizeof(*payload) );
