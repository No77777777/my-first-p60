/*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/wms/src/WMSE/wmsbc.c#1 $
...
#ifdef FEATURE_SMS_PWS_OVER_LTE
sys_modem_as_id_e_type wms_global_as_id ;
#endif
...
/*===========================================================================
FUNCTION wms_set_as_id												(INTERNAL)

DESCRIPTION
 set the as id of wms
...
{
  if((as_id>SYS_MODEM_AS_ID_NONE) && (as_id<(sys_modem_as_id_e_type)MAX_AS_IDS))
  {
    wms_global_as_id = as_id;
  }
  else
  {
    wms_global_as_id = SYS_MODEM_AS_ID_1;
  }
  // only for debug purpose remove before actual check in
  MSG_HIGH_1( "In wms_set_as_id(): As id set to %d ",wms_global_as_id);

}

/*===========================================================================
FUNCTION wms_get_as_id												(INTERNAL)

DESCRIPTION
  Return thevalue of as_id
...
SIDE EFFECTS
  None.
===========================================================================*/
sys_modem_as_id_e_type	wms_get_as_id(
  void
)
{
  return wms_global_as_id;
...
{
  static wms_bc_s_type  bc[MAX_AS_IDS];

  return &bc[wms_get_as_id()];
} /* wms_bc_s_ptr() */
...
)
{
  ...
  {
    wms_bc_s_type *bc_s_ptr = NULL;
    wms_set_as_id((sys_modem_as_id_e_type) index);
...
)
{
 int index =0;
...
#ifdef FEATURE_SMS_PWS_OVER_LTE
  {
    ...
    //set back to default
	wms_set_as_id(SYS_MODEM_AS_ID_1);
  }
...
{
#ifdef FEATURE_CDSMS_BROADCAST
  wms_prov_app_info_s_type    *appInfo_ptr = wms_get_appinfo_by_asid_and_message_mode(wms_check_1x_active_sub(),WMS_MESSAGE_MODE_CDMA); 
...
)
{
  ...
  MSG_HIGH_3("BC CONF=%d, PREF=%d, num_sel=%d",
             appInfo_ptr->bc.cd.bc_config,
             appInfo_ptr->bc.cd.bc_pref,
             appInfo_ptr->bc.cd.bc_num_selected);
...
)
{
  ...
  if ( bc_enable_cur != bc_enable_new )
  {
    ...
    wms_cm_bc_enable(appInfo_ptr->bc.cd.bc_enable,as_id);
  }
...
)
{
  ...
  if( cmd_ptr != NULL)
  {
    ...
    cmd_ptr->hdr.appInfo_ptr = wms_get_appinfo_by_asid_and_message_mode(wms_check_1x_active_sub(), WMS_MESSAGE_MODE_CDMA); 
...
)
{
  ...
  wms_log_event_notify_packet(client_id,event, client_ptr->bc_mm_notify_type);
    ...
    switch( tl_ptr->cl_bd.scpt_data_ptr->entry[scpt_index].op_code )
    {
      ...
      case WMS_BC_SCPT_OP_ADD:
        if( (appInfo_ptr->bc.cd.bc_num_active >= WMS_BC_TABLE_MAX) ||
            (appInfo_ptr->bc.cd.bc_table_size >= WMS_BC_TABLE_MAX))
        {
          /* table is full
          */
          ...
          else
          {
            ...
            */
            ...
            MSG_HIGH_2( "Update service table entry %d with index = %d",
                        tbl_index,
                        service_ptr->index );
            if( wms_nv_write_wait_per_subs( cmd_ptr->hdr.as_id, NV_SMS_BC_SERVICE_TABLE_I, &nvi )
                != NV_DONE_S )
            {
            ...
            else
            {
              ...
              if( wms_nv_write_wait_per_subs( cmd_ptr->hdr.as_id,NV_SMS_BC_SERVICE_TABLE_OPTIONS_I,
                                     &nvi ) != NV_DONE_S )
              {
                scpt_status = WMS_BC_SCPT_STATUS_UNSPECIFIED_ERROR;
              }
              else
              {
                ...
                if( wms_nv_write_wait_per_subs( cmd_ptr->hdr.as_id, NV_SMS_BC_SERVICE_TABLE_SIZE_I, &nvi )
                    != NV_DONE_S )
                {
                  scpt_status = WMS_BC_SCPT_STATUS_UNSPECIFIED_ERROR;
                }
                else
                {
                  ...
                  if (NULL != bc_mm_event_ptr)
                  {
                    ...
                    if (table_ok)
                    {
                      wms_bc_update_1x_bcsms_setting(appInfo_ptr,as_id);
...
)
{
  ...
  wms_prov_app_info_s_type *appInfo_ptr = wms_get_appinfo_by_asid_and_message_mode(as_id, WMS_MESSAGE_MODE_CDMA); 

  MSG_HIGH_1("wms_bc_init(): as_id = %d",as_id );
  ...
  /* NV_SMS_BC_CONFIG_I:
  */
  nv_status = wms_nv_read_wait_per_subs(as_id, NV_SMS_BC_CONFIG_I, &nvi );
  ...
  else
  {
	  ...
	  local_bc_config = WMS_BC_CONFIG_DISALLOW;  	 
  }
  ...
  if (WMS_BC_CONFIG_DISALLOW == local_bc_config)
  {
    ...
    (void)wms_nv_write_wait_per_subs( as_id, NV_SMS_BC_USER_PREF_I, &nvi );  

    // Write table size to NV
    nvi.sms_bc_service_table_size = 0;
    (void)wms_nv_write_wait_per_subs( as_id, NV_SMS_BC_SERVICE_TABLE_SIZE_I, &nvi );
  ...
  */
  nv_status = wms_nv_read_wait_per_subs(as_id, NV_SMS_BC_USER_PREF_I, &nvi );
  ...
  else
  {
    ...
    local_bc_pref = WMS_BC_PREF_DEACTIVATE;
	
	MSG_HIGH_1("wms_bc_init(): couldn't read NV_SMS_BC_USER_PREF_I, error %d", nv_status);

    { // Initialize/update NV
      nvi.sms_bc_user_pref = (byte)WMS_BC_PREF_DEACTIVATE;

      if (wms_nv_write_wait_per_subs(as_id, NV_SMS_BC_USER_PREF_I, &nvi) != NV_DONE_S)
      {
        MSG_ERROR_1("Error while writing BC_USER_PREF, %d", nv_status);
      }
    }
  }
  ...
  */
  nv_status = wms_nv_read_wait_per_subs(as_id, NV_SMS_BC_SERVICE_TABLE_SIZE_I, &nvi );
  


  if (NV_DONE_S == nv_status)
  {
    local_bc_table_size = MIN(nvi.sms_bc_service_table_size, WMS_BC_TABLE_MAX);
  }
  else
  {
    // If the item can't be read, the table size is 0
    local_bc_table_size = 0;

    MSG_HIGH_1("wms_bc_init(): couldn't read NV_SMS_BC_SERVICE_TABLE_SIZE_I, error %d", nv_status);

    { // Initialize/update NV
      nvi.sms_bc_service_table_size = 0;

      nv_status = wms_nv_write_wait_per_subs(as_id, NV_SMS_BC_SERVICE_TABLE_SIZE_I, &nvi);

      if (NV_DONE_S != nv_status)
      {
        MSG_ERROR_1("Error while writing SMS_BC_SERVICE_TABLE_SIZE, %d", nv_status);
      }
    }
  }

  MSG_HIGH_3( "wms_bc_init() config %d, pref %d, size %d",
              local_bc_config, local_bc_pref, local_bc_table_size);
  ...
  */
  ...
  for ( i=0; i < local_bc_table_size && i < WMS_BC_TABLE_MAX; i++ )
  {
    ...
    (void) wms_nv_read_wait_per_subs(as_id, NV_SMS_BC_SERVICE_TABLE_I, &nvi );
    ...
    {
      appInfo_ptr->bc.cd.bc_num_active ++;
  
      if (TRUE == pLocalTableEntry->selected)
      {
...
)
{
  ...
  wms_cm_bc_enable( appInfo_ptr->bc.cd.bc_enable,appInfo_ptr->asid);
...
)
{
  ...
  */
  ...
  MSG_HIGH_5("--BC_ADDR-- BC srv = 0x%lx, lang = %d, pri = %d, msg_id = %d, zone_id = %d",
             service,
             language,
             priority,
             msg_id,
...
)
{
  ...
  else
  {
    ...
    {
#ifdef FEATURE_GWSMS_BROADCAST
      for( i=0, j=0; (i < appInfo_ptr->bc.gw.gw_cb_table_size) && (j < (uint16)output->size); i++ )
      {
        if (TRUE == appInfo_ptr->bc.gw.gw_cb_table[i].active_service)
        {
    ...
    {
#ifdef FEATURE_CDSMS_BROADCAST
      for( i=0, j=0; (i < appInfo_ptr->bc.cd.bc_table_size) && (j < (uint16)output->size); i++ )
      {
        if (TRUE == appInfo_ptr->bc.cd.bc_table[i].active_service)
        {
...
)
{
  ...
  MSG_HIGH_3("GW CB CONF=%d, PREF = %d, num_sel = %d", 
             cb_config,
             cb_pref,
             num_selected);
...
)
{
  ...
  if ( (0 == appInfo_ptr->bc.gw.gw_cb_table_size) ||
       (appInfo_ptr->bc.gw.gw_cb_table_size > WMS_BC_MM_TABLE_MAX) )
  {
    return FALSE;
  }
...
)
{
  ...
  MSG_HIGH_2("Maximum Number of CBMI Records = %d, CBMIR Records = %d", cbmi_max, cbmir_max);
  ...
  do
  {
    ...
    else // message mode CDMA
    {
      ...
      // The command parameters are OK, now update NV
      {
        ...
        write_status = wms_nv_write_wait_per_subs(as_id, NV_SMS_BC_USER_PREF_I, &nvi );
...
)
{
  ...
  else
  {
    ...
    do
    {
      ...
      for (service_idx = 0; (service_idx < appInfo_ptr->bc.cd.bc_table_size) && (service_idx < WMS_BC_TABLE_MAX); service_idx++)
      {
      ...
      if (must_update_nv)
      {
        ...
        write_status = wms_nv_write_wait_per_subs( as_id, NV_SMS_BC_SERVICE_TABLE_I, &lnvi );
      ...
      else
      {
        ...
        */
        ...
        MSG_HIGH_2( "Update service table entry %d with index = %d",
                    tbl_index,
                    service_ptr->index );
        if( wms_nv_write_wait_per_subs( appInfo_ptr->asid, NV_SMS_BC_SERVICE_TABLE_I, &nvi )
            != NV_DONE_S )
        {
          MSG_ERROR_0("BC Table NV write failed");
          cmd_err = WMS_CMD_ERR_BC_NV_WRITE;
        ...
        {
          nvi.sms_bc_service_table_options = * opt_ptr;
          if(wms_nv_write_wait_per_subs( appInfo_ptr->asid, NV_SMS_BC_SERVICE_TABLE_OPTIONS_I,
                                &nvi ) != NV_DONE_S )
          {
            MSG_ERROR_0("BC Table NV write failed");
            cmd_err = WMS_CMD_ERR_BC_NV_WRITE;
          ...
          else
          {
            ...
            nvi.sms_bc_service_table_size = appInfo_ptr->bc.cd.bc_table_size;
            if( wms_nv_write_wait_per_subs( appInfo_ptr->asid, NV_SMS_BC_SERVICE_TABLE_SIZE_I, &nvi )
              != NV_DONE_S )
            {
              MSG_ERROR_0("BC Table Size NV write failed");
              cmd_err = WMS_CMD_ERR_BC_NV_WRITE;
...
)
{
    ...
    else
    {
	  ...
	  // if we have other same presidential alert entries in the service table.
        ...
        if( ok_to_delete )
        {
          ...
          */
		  if(appInfo_ptr->bc.gw.gw_cb_num_active > 0)
		  {
            appInfo_ptr->bc.gw.gw_cb_num_active --;              // Not used by modem api
		  }
  ...
  else
  {
    ...
    /* search the table for a free entry
    */
    for (tbl_index = 0; (tbl_index < appInfo_ptr->bc.cd.bc_table_size) && (tbl_index < WMS_BC_TABLE_MAX); tbl_index++ )
    {
      service_ptr = &appInfo_ptr->bc.cd.bc_table[tbl_index];
    ...
    else
    {
      if (NULL != service_ptr)
      {
        ...
        */
        ...
        /* we have one less active entry */
        if(appInfo_ptr->bc.cd.bc_num_active > 0)
        {
         appInfo_ptr->bc.cd.bc_num_active --;            // Not used by modem api
        }
  ...
  else
  {
    ...
    do
    {
      ...
      // Write the NV item
      {
        ...
        write_status = wms_nv_write_wait_per_subs(cmd_ptr->hdr.as_id, NV_SMS_BC_SERVICE_TABLE_I, &lnvi );
...
)
{
  ...
  if (WMS_MESSAGE_MODE_GW == cmd_ptr->cmd.bc_mm_delete_all_srvs.message_mode)
  {
    ...
    {
      // Delete all entries from table
      for (tbl_index = 0; (tbl_index < appInfo_ptr->bc.gw.gw_cb_table_size) && (tbl_index < WMS_BC_MM_TABLE_MAX); tbl_index++)
      {
        boolean                            ok_to_delete  = TRUE;
  ...
  else
  {
      ...
      for (tbl_index = 0; (tbl_index < appInfo_ptr->bc.cd.bc_table_size) && (tbl_index < WMS_BC_TABLE_MAX); tbl_index++)
      {
            ...
            (0 == wms_bc_mm_get_presidential_alert_entry_count_in_client_req(WMS_MESSAGE_MODE_CDMA, srv_id)))
        {
...

          {
            ...
            // Write to NV
            write_status = wms_nv_write_wait_per_subs(cmd_ptr->hdr.as_id, NV_SMS_BC_SERVICE_TABLE_I, &nvi );
    ...
    if (WMS_CMD_ERR_NONE == cmd_err)
    {
      ...
      wms_bc_update_1x_bcsms_setting(appInfo_ptr,cmd_ptr->hdr.as_id);
...
)
{
  ...
  MSG_HIGH_3( "wms_bc_gw_cb_init() asid %d, init_sim %d, add_default_service_ids %d",
              appInfo_ptr->asid,
  ...
  if (NV_DONE_S == nv_status)
  {
    local_gw_cb_table_size = MIN(nvi.sms_gw_cb_service_table_size, WMS_BC_MM_TABLE_MAX);
  }
  else
  {
    // If the item can't be read, the table size is 0
  ...
  else
  {
    ...
    { // Initialize/update NV
      nvi.sms_bmc_reading_pref.reading_advised  = FALSE;
  ...
  if (do_sim_initialization)
  {
   ...
   /* add CMAS mandatory servcie ids when UE is caped on US MCC and in limited service*/
  if(default_service_ids) 
  {
    wms_bc_mm_service_info_s_type    srv_info;
	wms_cmd_err_e_type               ret_val;
	
	srv_info.srv_id.gw_cb_range.from      = WMS_GW_CB_SRV_ID_CMAS_MSGID1;
	srv_info.srv_id.gw_cb_range.to        = WMS_GW_CB_SRV_ID_CMAS_MSGID30;
	srv_info.selected			          = TRUE;
	srv_info.priority			          = WMS_PRIORITY_NORMAL;
	srv_info.label[0]			          = 0;
	srv_info.label_encoding 	          = WMS_ENCODING_GSM_7_BIT_DEFAULT;
	...
	if (WMS_CMD_ERR_NONE != ret_val)
    {
       MSG_ERROR_1("Not able to add default service ids, err %d", ret_val);
    }    
  }
...
/*=========================================================================
FUNCTION wms_bc_ignore_lang_filter_for_msg_id

DESCRIPTION
  This function checks whether language filtering needs to be applied for
  the particular brodcast message id

DEPENDENCIES
  None

RETURN VALUE
  FALSE - language filtering needs to be applied
  TRUE  - language filtering should be ignored

SIDE EFFECTS
  None

=========================================================================*/
boolean wms_bc_ignore_lang_filter_for_msg_id(
   wms_gw_cb_srv_id_type   srv_id
)
{
   /*
     Spec 23.041, section 9.4.1.2.3 indicates: where the message relates
     to a public warning system, the Message Identifier range 4370 through
     4382 relates to warning messages delivered in languages which are
     mandatory to receive. The ME shall not use any language filter mechanisms
     or use the language(s) selected through the MMI to determine whether a
     particular CBS message should be displayed for this Message Identifier range. 
   */
   if ( ( (srv_id >= WMS_GW_CB_SRV_ID_CMAS_MSGID1) &&
          (srv_id <= WMS_GW_CB_SRV_ID_CMAS_MSGID13) ) ||
        (srv_id == WMS_GW_CB_SRV_ID_CMAS_MSGID27) ||
        (srv_id == WMS_GW_CB_SRV_ID_CMAS_MSGID29) )
   { 
      return TRUE;
   }
   else
...
)
{
  ...
  */
  ...
  MSG_HIGH_2("===== CB Page Header (as_id=%d) Msg ID = %ld: ====", as_id, page_header.cb_srv_id);
  MSG_HIGH_8("Total pages: %d, Page number: %d, Geo Scope: %d, Msg Code: %d, Update Number: %d, Dcs.class=%d, Dcs.alphabet=%d, Dcs.language=%d", 
             page_header.total_pages,
             page_header.page_number,
             page_header.geo_scope,
             page_header.message_code,
             page_header.update_number, 
             page_header.cb_dcs.msg_class, 
             page_header.cb_dcs.alphabet,
             page_header.cb_dcs.language);
        ...
        )
      {
        ...
        else if (WMS_BC_PREF_ACTIVATE_TABLE == appInfo_ptr->bc.gw.gw_cb_pref)
        {
           /* 
             ...
             Spec 23.041- section 9.4.1.2.3 states that When the SIM indicates one or more language
             preferences, the ME shall, by default, use the language(s) stored in the SIM (in the EFPL file)
             to set any language filter mechanisms provided by the ME.
          */
          ...
          else if (TRUE == wms_bc_ignore_lang_filter_for_msg_id(srv_id))
          {
            /* Ignore language preferences */
            MSG_HIGH_1("WMS_BC_PREF_ACTIVATE_TABLE: ignoring language preferences for msg id: %d", srv_id);
            result = TRUE;
          }
          else
          {
...
/*=========================================================================
FUNCTION wms_is_KDDI                                (INTERNAL)

DESCRIPTION
  This function checks weather the received PLMN belongs to KKDI or NOT
...
RETURN VALUE
  TRUE  - if PLMN belongs to KDDI
  FALSE - if PLMN belongs to other operator
...
boolean wms_is_KDDI_PLMN(lte_rrc_plmn_s   *plmn)
{
  static uint8 kddi_mnc[][LTE_RRC_MAX_MNC_DIGITS] =
    {{0x00, 0x01, 0x0f},
     {0x05, 0x00, 0x0f},
     {0x05, 0x01, 0x0f},
     {0x05, 0x02, 0x0f},
  ...
  uint8 no_of_mncs = sizeof(kddi_mnc)/LTE_RRC_MAX_MNC_DIGITS;
  uint8 index = 0;
  if ( (WMS_BC_MCC_0_JA_MASK == plmn->mcc[0])  &&
     (WMS_BC_MCC_1_JA_MASK == plmn->mcc[1])  &&
     (WMS_BC_MCC_2_JA_MASK1 == plmn->mcc[2]))
  {
    for (index = 0; index < no_of_mncs; index++)
    {
      if ((plmn->mnc[0] == kddi_mnc[index][0]) &&
           (plmn->mnc[1] == kddi_mnc[index][1]) &&
           (plmn->mnc[2] == kddi_mnc[index][2]))
      {
        return TRUE;
      }
    }
  }
  else if ( (WMS_BC_MCC_0_JA_MASK == plmn->mcc[0])  &&
            (WMS_BC_MCC_1_JA_MASK == plmn->mcc[1])  &&
            (WMS_BC_MCC_2_JA_MASK2 == plmn->mcc[2]))
  {
    if((plmn->mnc[0] == 0x01) &&
        (plmn->mnc[1] == 0x00) &&
        (plmn->mnc[2] == 0x0f) )
...
)
{
  ...
  if (NULL != plmn)
  {
         ...
         ( (WMS_BC_MCC_2_JA_MASK1 == plmn->mcc[2])  || (WMS_BC_MCC_2_JA_MASK2 == plmn->mcc[2])) )
    {
      ...
      if (wms_is_KDDI_PLMN(plmn))
      {
        MSG_HIGH_0( "KDDI PLMN setting expiry time to six min");
        expiration_seconds = WMS_BC_SECS_IN_SIX_MIN;
      }
    }
  }
    ...
    { // Optional additional checks using Geographical Scope
      ...
      */
      ...
      case WMS_BC_GEO_SCOPE_LOW_PRIORITY_CELL_WIDE:
        ...
        {
          MSG_HIGH_2("Cell-ids not equal,old cellid:0x%x,new cellid:0x%x",list_entry->cell_id,new_entry->cell_id);
          is_duplicate = FALSE;
        }
        break;
