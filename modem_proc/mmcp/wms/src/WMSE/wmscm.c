/*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/wms/src/WMSE/wmscm.c#1 $
...
)
{
  if (NULL != ph_info_ptr && asid_is_valid(ph_info_ptr->asubs_id))
  {
    ...
    {
      appInfo_ptr = wms_get_appinfo_by_asid_and_message_mode(ph_info_ptr->asubs_id, WMS_MESSAGE_MODE_CDMA); 
    }
    else
    {
...
)
{
  ...
  if(proc_in_wms_task)
  {
     ...
     if (NULL != cmd_ptr)
     {
       ...
       cmd_ptr->hdr.user_data   = NULL;
       cmd_ptr->hdr.appInfo_ptr = NULL;
       cmd_ptr->hdr.as_id       = ph_info_ptr->asubs_id;
...
)
{
  ...
  case CM_PH_EVENT_SUBSCRIPTION_NOT_AVAILABLE:
    ...
    MSG_HIGH_6("ph_info_ptr->is_cdma_subscription_available = %d, ph_info_ptr->is_gw_subscription_available = %d, "
               "ph_info_ptr->rtre_control = %d, ph_info_ptr->rtre_config = %d, ph_info_ptr->mode_capability = %d,ph_info_ptr->as_id = %d",     
               ph_info_ptr->is_cdma_subscription_available,
               ph_info_ptr->is_gw_subscription_available,
               ph_info_ptr->rtre_control,
               ph_info_ptr->rtre_config,
               ph_info_ptr->mode_capability,
               ph_info_ptr->asubs_id);
  ...
  case CM_PH_EVENT_DUAL_STANDBY_PREF:
    ...
    MSG_HIGH_5( "CM_PH_EVENT_DUAL_STANDBY_PREF: standby_pref = %d active_subs = %d default_voice_subs %d , priority_subs: %d, default_data_subs: %d",
                 ph_info_ptr->standby_pref,
                 ph_info_ptr->active_subs,
                 ph_info_ptr->default_voice_subs,
                 ph_info_ptr->priority_subs,
  ...
  case CM_PH_EVENT_SYS_SEL_PREF:
    {
      if (SYS_MODEM_AS_ID_1 == ph_info_ptr->asubs_id)
      {
        ...
        MSG_HIGH_1("CM_PH_EVENT_SYS_SEL_PREF: cfg_s_ptr->hybr_pref = %d", cfg_s_ptr->hybr_pref);
      }	
...
)
{
  ...
  tmp_ss_info.number_of_stacks = 0;
  switch( ss_event )
  {
    ...
    case CM_SS_EVENT_INFO:       
      ...
      {
        MSG_HIGH_8( "[%d] is_oper %d, srv_status %d, sys_mode %d, id_type %d, srv_domain %d, chg_fields 0x%x, chg_fields2 0x%x",
                    stack_idx,
		...
		MSG_HIGH_1( "cell_id = %d, ",
                  cm_ss_info_ptr->stack_info[stack_idx].cell_info.cell_id);
        ...
        if ( cm_ss_info_ptr->stack_info[stack_idx].is_operational )
        {
		  ...
		  tmp_ss_info.stack_info[stack_idx].cell_info.cell_id = (uint32) cm_ss_info_ptr->stack_info[stack_idx].cell_info.cell_id; 	
  ...
  if ( WMS_CMD_DUMMY != cmd_id ) 
  {
    ...
    MSG_HIGH_3( " wms_cm_ss_event_cb(): event = %d asubs_id = %d, number_of_stacks = %d",
	  	          ss_event,
                  cm_ss_info_ptr->asubs_id, 
                  tmp_ss_info.number_of_stacks );
...
)
{
  ...
  {
    MSG_HIGH_2("Got SMS event %d on SUB :%d", call_event,call_info_ptr->asubs_id);
    ...
    switch( call_event )
    {
      ...
      case CM_CALL_EVENT_1x_ORIG_FAIL:
  ...
  if ( WMS_CMD_DUMMY != cmd_id )
  {
    ...
    if ( NULL != cmd_ptr )
    {
      ...
      cmd_ptr->hdr.client_id   = (uint8) WMS_CLIENT_TYPE_INTERNAL;
      cmd_ptr->hdr.cmd_cb      = NULL;
      cmd_ptr->hdr.user_data   = (void *)call_event;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if( call_cmd_err != CM_CALL_CMD_ERR_NOERR )
  {
    ...
    as_id = (sys_modem_as_id_e_type)(int)user_data ;
...
)
{
  ...
  if( sms_cmd_err != CM_SMS_CMD_ERR_NOERR )
  {
    ...
    if( sms_cmd == CM_SMS_CMD_MO_MSG )
    {
      ...
      wms_msg_mc_mo_status( WMS_ERROR_TEMP, st, cfg_s_ptr->last_used_bearer,as_id );

    }
#ifdef FEATURE_CDSMS_BROADCAST
    else if( sms_cmd == CM_SMS_CMD_BC_ENABLE )
    {
      wms_cm_bc_enable_failure_proc( sms_cmd_err,as_id );
    }
#endif /* FEATURE_CDSMS_BROADCAST */
...
)
{
  ...
  MSG_HIGH_2("Got CM SMS event %d on SUB:%d", sms_event,sms_info_ptr->asubs_id);
    ...
    case CM_SMS_EVENT_MO_STATUS:
      {
        ...
        appInfo_ptr = wms_get_appinfo_by_asid_and_message_mode(sms_info_ptr->asubs_id, WMS_MESSAGE_MODE_CDMA); 
  ...
  case CM_SMS_EVENT_MT_MSG:
      {
        sys_modem_as_id_e_type     as_id = sms_info_ptr->asubs_id;  
        wms_msg_cdma_mt_msg( (wms_format_e_type) (sms_info_ptr->mt_sms_format),
                           ...
                           TRUE, /* ack_pending */
                           as_id,
                           0  ); /* default it to normal SMS for now */
      }
...
)
{
  ...
  (void)cm_sms_cmd_mo_msg3( wms_cm_sms_cmd_cb,
                            (void *)as_id,   // user data
                            cm_s_ptr->cm_client,
                            TRUE,   // request L2 ack
                            & cm_sms_msg,
                            cm_mode_pref,
...
)
{
  ...
  (void)cm_sms_cmd_awi_ack1( NULL, // no cmd cb
                      NULL, // no user data
                      cm_s_ptr->cm_client,
                      seq_no,
                      bf_bit,
...
)
{
   ...
   call_id = cmcall_is_there_volte_call();
   ...
   if (CM_CALL_ID_INVALID != call_id) 
   {
      ...
      if (NULL != call_ptr) 
      {
        ...
        {
           MSG_HIGH_3("call id = %d, sys mode = %d, as id = %d", call_id, call_ptr->sys_mode, call_ptr->mm_id.asubs_id );
        }
      }
      else
      {
...
)
{
  ...
  {
    MSG_HIGH_2("1X activated on same sub : as_id = %d,subs_active = %d" ,as_id, wms_check_1x_active_sub());
    (void) cm_sms_cmd_bc_enable1( wms_cm_sms_cmd_cb,
                               NULL,   // user data
                               cm_s_ptr->cm_client,
                               bc_enable,
                               as_id);
  ...
  {
   MSG_ERROR_2("1X activated on different sub : as_id = %d,subs_active = %d" ,as_id, wms_check_1x_active_sub());
  }
  return;
} /* wms_cm_bc_enable() */
...
)
{
  ...
  //
  MSG_HIGH_8( "SUBS_PREF_INFO(asid %d): session (1X:%d, avail:%d) (GW:%d, avail:%d), operational: %d ,subs_capability:%d, is_default_voice %d",
              pref_info_asid,
...

  {
    ...
    sys_sys_mode_mask_e_type        cur_subs_capability;
 
    MSG_HIGH_8( "WMS_PREF_INFO (asid %d): session (1X:%d, avail:%d) (GW:%d, avail:%d), operational: %d, subs_capability: %d, default_voice_subs %d",
                pref_info_asid,
                ...
                cfg_s_ptr->wms_cm_subs_info.sub[pref_info_asid].subs_capability,
                cfg_s_ptr->wms_ph_info.default_voice_subs);
    ...
    // CDMA/1X is only available on AS_ID_1.
    {
      ...
      cur_subs_capability = cfg_s_ptr->wms_cm_subs_info.sub[pref_info_asid].subs_capability;
      ...
      if ((cur_is_available != subs_info_ptr->is_available_1x)||
          (cur_subs_capability != subs_info_ptr->subs_capability))
      {
        appInfo_ptr = wms_get_appinfo_by_session_type(subs_info_ptr->session_type_1x);

        if (NULL == appInfo_ptr && wms_nv_rtre_control_is_nv(pref_info_asid))
        {
          appInfo_ptr = wms_get_1X_appinfo_by_asid(pref_info_asid);
        }
        ...
        if (NULL != appInfo_ptr)
        {
          if (subs_info_ptr->is_available_1x)
          {
            ...
            appInfo_ptr->subs_capability    = subs_info_ptr->subs_capability;  //1x
            ...
            else
            {
              ...
              MSG_HIGH_3("1X:available(card): appInfo_ptr 0x%x ,sim_init_complete %d,subs_capability :%d",appInfo_ptr, appInfo_ptr->sim_init_complete,appInfo_ptr->subs_capability);
            }
          ...
          else
          {
            ...
            else
            {
              ...
              MSG_HIGH_3("1X:unavailable(card):appInfo_ptr 0x%x, sim_init_complete %d, subs_capability : %d", appInfo_ptr ,appInfo_ptr->sim_init_complete,appInfo_ptr->subs_capability);
            }
    ...
    cfg_s_ptr->wms_cm_subs_info.sub[pref_info_asid].subs_capability = subs_info_ptr->subs_capability;
