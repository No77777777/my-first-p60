/*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/wms/src/WMSE/wmsnv.c#1 $
...
/nv/item_files/modem/sms/enable_retry\n";
...
static const wms_nv_efs_tbl_entry_s_type wms_nv_efs_tbl[] =
{
  ...
  { WMS_NV_EFS_SMS_ENABLE_RETRY,             "/nv/item_files/modem/sms/enable_retry"},
  { WMS_NV_EFS_RRC_DISABLE_TA_FOR_EMBMS,     "/nv/item_files/modem/lte/rrc/rrc_disable_ta_for_embms_valid"}
};
...
)
{
  ...
  if( (nv_st=wms_nv_read_wait_per_subs( as_id, NV_SMS_CD_PARMS_I, & nvi )) == NV_DONE_S )
  {
...
)
{
  ...
  if( wms_nv_write_wait_per_subs( as_id,NV_SMS_CD_PARMS_I, & nvi ) != NV_DONE_S )
  {
    res = FALSE;
  }
...
)
{
  ...
  if( wms_nv_read_wait_per_subs( as_id, NV_ENS_ENABLED_I, & nvi ) == NV_DONE_S )
  {
...
)
{
  ...
  if (wms_nv_read_wait_per_subs(as_id, NV_SMS_MO_RETRY_INTERVAL_I, &nvi ) == NV_DONE_S)
  {
    msg_s_ptr->mo_retry_interval[as_id] = nvi.sms_mo_retry_interval;
  }
  else
  {
    msg_s_ptr->mo_retry_interval[as_id] = WMS_TIMER_RETRY_INTERVAL;
    nvi.sms_mo_retry_interval = WMS_TIMER_RETRY_INTERVAL;
    (void) wms_nv_write_wait_per_subs(as_id, NV_SMS_MO_RETRY_INTERVAL_I, &nvi );
  }

  /* Read Retry Period from NV */
  {
    if (wms_nv_read_wait_per_subs(as_id,NV_SMS_MO_RETRY_PERIOD_I, &nvi ) == NV_DONE_S)
    {
      msg_s_ptr->mo_retry_period[as_id] = nvi.sms_mo_retry_period; /* seconds */
    }
    else
    {
      msg_s_ptr->mo_retry_period[as_id] = WMS_MAX_RETRY_PERIOD; /* seconds */
    }
  }

  MSG_HIGH_3( "Retry Period, Interval = %d s, %d s, as_id %d",
              msg_s_ptr->mo_retry_period[as_id],
              msg_s_ptr->mo_retry_interval[as_id],
              as_id );

#ifdef FEATURE_CDSMS

  /* Read Access or Traffic Channel from RUIM card */
  wms_nv_read_cdma_channel_setting(as_id);

  /* Read Service Option Setting from card */
  wms_nv_read_cdma_service_option(as_id);
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  for (i = 0; i < j; i++) 
  {
    ...
    if (NULL != cdma_template_ptr)
    {
      ...
      {
        if ( ( FALSE == wms_nv_read_cdma_params( 0, nv_cdma_data,i) ) ||
             ( FALSE == wms_ts_cdma_decode_smsp( nv_cdma_data, NV_SMS_CD_PARMS_SIZE, cdma_template_ptr ) ) )
        {
          ...
          if ( FALSE == wms_nv_write_cdma_params( 0, nv_cdma_data,i ) )
          {
            MSG_ERROR_1("wms_nv_write_cdma_params failed on as_id = %d",i);
          }
        }
  ...
  for(sub_id=0; sub_id < MAX_AS_IDS ; sub_id ++)
  {
    ...
    {
      /* Read Retry Interval from NV */
      if ( NV_DONE_S == wms_nv_read_wait_per_subs(sub_id, NV_SMS_MO_RETRY_INTERVAL_I, nvi_ptr ) )
      {
        msg_s_ptr->mo_retry_interval[sub_id] = nvi_ptr->sms_mo_retry_interval;
      }
      else
      {
        msg_s_ptr->mo_retry_interval[sub_id] = WMS_TIMER_RETRY_INTERVAL;
        nvi_ptr->sms_mo_retry_interval = WMS_TIMER_RETRY_INTERVAL;
        (void) wms_nv_write_wait_per_subs(sub_id, NV_SMS_MO_RETRY_INTERVAL_I, nvi_ptr );
      }
      ...
      /* Read NV retry period for both CDMA and GW modes.
          ...
          */
      if ( NV_DONE_S == wms_nv_read_wait_per_subs(sub_id, NV_SMS_MO_RETRY_PERIOD_I, nvi_ptr ) )
      {
        msg_s_ptr->mo_retry_period[sub_id] = nvi_ptr->sms_mo_retry_period;
      }
      else
      {
        msg_s_ptr->mo_retry_period[sub_id] = WMS_MAX_RETRY_PERIOD;
        nvi_ptr->sms_mo_retry_period = WMS_MAX_RETRY_PERIOD;
        (void) wms_nv_write_wait_per_subs(sub_id, NV_SMS_MO_RETRY_PERIOD_I, nvi_ptr );
      }
    ...
    MSG_HIGH_4("Retry Interval[%d]: %d, Retry Period [%d]= %d s", sub_id,msg_s_ptr->mo_retry_interval[sub_id], sub_id,msg_s_ptr->mo_retry_period[sub_id]);
  }
...
===========================================================================*/
void wms_nv_read_cdma_service_option(sys_modem_as_id_e_type as_id)
{
   nv_item_type nvi;

  /* Read Service Option from NV */
  if (NV_DONE_S == wms_nv_read_wait_per_subs(as_id,NV_SMS_SERVICE_OPTION_I, &nvi))
  {
  ...
  else
  {
    ...
    (void) wms_nv_write_wait_per_subs(as_id, NV_SMS_SERVICE_OPTION_I, &nvi);
  }
...
===========================================================================*/
void wms_nv_read_cdma_channel_setting(sys_modem_as_id_e_type as_id)
{
   nv_item_type nvi;

   if(NV_DONE_S == wms_nv_read_wait_per_subs(as_id,NV_SMS_MO_ON_ACCESS_CHANNEL_I, &nvi))
   {
   ...
   else
   {
      ...
      (void) wms_nv_write_wait_per_subs(as_id,NV_SMS_MO_ON_ACCESS_CHANNEL_I, &nvi );

      MSG_HIGH_1( "Access Channel = %d ", dc_s_ptr->is_access_channel_enabled);
   }

   if(NV_DONE_S == wms_nv_read_wait_per_subs(as_id,NV_SMS_MO_ON_TRAFFIC_CHANNEL_I, &nvi))
   {
   ...
   else
   {
      ...
      (void) wms_nv_write_wait_per_subs(as_id,NV_SMS_MO_ON_TRAFFIC_CHANNEL_I, &nvi );
...
DEPENDENCIES
  None
===========================================================================*/
void wms_nv_write_sms_status (sys_modem_as_id_e_type as_id)
{
  nv_item_type nvi;

  nvi.ruim_sms_status.msg_id     = ( word )cfg_s_ptr->cdma_last_message_number;
  if ( NV_DONE_S != wms_nv_write_wait_per_subs (as_id, NV_RUIM_SMS_STATUS_I , &nvi ) )
  {
...
/*===========================================================================
FUNCTION wms_nv_efs_read_rrc_disable_ta_for_embms 
/nv/item_files/modem/lte/rrc/rrc_disable_ta_for_embms_valid
...
DEPENDENCIES
  None
===========================================================================*/
void wms_nv_efs_read_rrc_disable_ta_for_embms ( void )
{
  mcfg_fs_status_e_type       res = MCFG_FS_STATUS_EFS_ERR;
  const char *file_path = wms_nv_efs_map_efs_id_to_filename( WMS_NV_EFS_RRC_DISABLE_TA_FOR_EMBMS );
  ...
  /* Open/Read file */
  res = mcfg_fs_read( file_path,
                      (void*)&cfg_s_ptr->rrc_disable_ta_for_embms,
                      (uint32) sizeof(boolean),
                      MCFG_FS_TYPE_EFS,
                      MCFG_FS_SUBID_NONE );
  ...
  if ( MCFG_FS_STATUS_OK != res )
  {
    cfg_s_ptr->rrc_disable_ta_for_embms = FALSE;
  }
  
  MSG_HIGH_1("WMS_NV_EFS_RRC_DISABLE_TA_FOR_EMBMS setting = %d",cfg_s_ptr->rrc_disable_ta_for_embms);
...
static void wms_nv_efs_read_sms_retry_limit(void)
{
  ...
  for(sub_id =0; sub_id < MAX_AS_IDS;sub_id ++)
  {
    /* Open/Read file */
    res = mcfg_fs_read( file_path,
                       (void *)&cfg_s_ptr->sms_retry_limit[sub_id],
                       (uint32) sizeof(uint8),
                        MCFG_FS_TYPE_EFS,
    ...
    {
      /* Open/Write file */
      cfg_s_ptr->sms_retry_limit[sub_id] = 1;
    ...
    MSG_HIGH_3("wms_nv_efs_read_sms_retry_limit(sub_id: %d): res = %d, SMS retry limit = %d", sub_id,res, cfg_s_ptr->sms_retry_limit[sub_id]);
  }
...
void wms_nv_efs_init(void)
{
...
#ifdef FEATURE_SMS_PWS_OVER_LTE
  //WMS_NV_EFS_RRC_DISABLE_TA_FOR_EMBMS
  wms_nv_efs_read_rrc_disable_ta_for_embms();
#endif
  ...
  //Read E911 SMS setting
  wms_nv_get_e911_sms_allowance();
}
...
)
{
  ...
  rtre_control_val = nv_rtre_control_ext((uint16)asid);
...
)
{
  ...
  if ((NULL != cmd_ptr) && (asid_is_valid(cmd_ptr->hdr.as_id)))
  {
   ...
   /* Read Access or Traffic Channel from RUIM card */
   wms_nv_read_cdma_channel_setting(cmd_ptr->hdr.as_id);

   /* Read Service Option Setting from card */
   wms_nv_read_cdma_service_option(cmd_ptr->hdr.as_id);
   ...
   if (wms_nv_read_wait_per_subs(cmd_ptr->hdr.as_id, NV_SMS_MO_RETRY_INTERVAL_I, &nvi ) == NV_DONE_S)
  {
    msg_s_ptr->mo_retry_interval[cmd_ptr->hdr.as_id] = nvi.sms_mo_retry_interval;
  }
  else
  {
    msg_s_ptr->mo_retry_interval[cmd_ptr->hdr.as_id] = WMS_TIMER_RETRY_INTERVAL;
    nvi.sms_mo_retry_interval = WMS_TIMER_RETRY_INTERVAL;
    (void) wms_nv_write_wait_per_subs(cmd_ptr->hdr.as_id, NV_SMS_MO_RETRY_INTERVAL_I, &nvi );
  }

  /* Read Retry Period from NV */
  {
    if (wms_nv_read_wait_per_subs(cmd_ptr->hdr.as_id,NV_SMS_MO_RETRY_PERIOD_I, &nvi ) == NV_DONE_S)
    {
      msg_s_ptr->mo_retry_period[cmd_ptr->hdr.as_id] = nvi.sms_mo_retry_period; /* seconds */
    }
    else
    {
      msg_s_ptr->mo_retry_period[cmd_ptr->hdr.as_id] = WMS_MAX_RETRY_PERIOD; /* seconds */
    }
  }

  MSG_HIGH_3( "Retry Period, Interval on sub: = %d s, %d s,%d",
              msg_s_ptr->mo_retry_period[cmd_ptr->hdr.as_id],
              msg_s_ptr->mo_retry_interval[cmd_ptr->hdr.as_id],
              cmd_ptr->hdr.as_id);
  ...
  { 
    MSG_HIGH_1("Null cmd_ptr in wms_nv_refresh_cb_proc():  0x%08x or Invalid asid",cmd_ptr);
  }
...
/*===========================================================================
FUNCTION WMS_NV_READ_SMS_RETRY_FLAG

DESCRIPTION
  This function return if efs read succes and what value read
...
boolean wms_nv_read_sms_retry_flag(boolean *sms_retry_flag)
{
  boolean res = TRUE;  
  mcfg_fs_status_e_type res_efs;
  
  const char *file_path =
    wms_nv_efs_map_efs_id_to_filename(WMS_NV_EFS_SMS_ENABLE_RETRY);
  ...
  /* Open/Read file */
  res_efs = mcfg_fs_read( file_path,
                      (void*)&res,
                      (uint32) sizeof(boolean),
                      MCFG_FS_TYPE_EFS,
                      MCFG_FS_SUBID_NONE );
  
  MSG_HIGH_2("wms_nv_read_sms_retry_flag(): res = %d, value is  = %d", res_efs, res);

  if ( MCFG_FS_STATUS_OK != res_efs )
  {
    *sms_retry_flag = FALSE;
	return FALSE;	
  }
  *sms_retry_flag = res;
...
/*===========================================================================
FUNCTION WMS_NV_GET_E911_SMS_ALLOWANCE

DESCRIPTION
  This function queries policyman to determine whether the E911 SMS needs to
  allowed in limited service or not. 
...
DEPENDENCIES
  NONE.
===========================================================================*/
void wms_nv_get_e911_sms_allowance(void)
{
  uint32 sub_id;
  ...
  for (sub_id =0; sub_id < MAX_AS_IDS; sub_id ++)
  {
    ...
    result = policyman_get_subs_boolean_with_default("wms:allow_911_SMS_over_IMS", (sys_modem_as_id_e_type)sub_id, FALSE);
#endif

    cfg_s_ptr->allow_e911_sms_over_ims[sub_id] = result;

    MSG_HIGH_2("wms_nv_get_e911_sms_allowance(sub_id: %d) Allow E911 SMS = %d", sub_id, cfg_s_ptr->allow_e911_sms_over_ims[sub_id]);
  }
