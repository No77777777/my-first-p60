/*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/wms/src/WMSE/wmsdc.c#1 $
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  */
  ...
  {
    wms_cm_call_orig( dc_connect_so,cmd_ptr->hdr.as_id);
...
{
  wms_cmd_err_e_type          cmd_err = WMS_CMD_ERR_NONE;
  cm_call_event_e_type        call_event = (cm_call_event_e_type)(int)cmd_ptr->hdr.user_data;
  boolean                     reest_traffic_ch = TRUE; 
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */

  MSG_HIGH_4("wms_dc_call_end_proc(): call_event: %d, dc_s_ptr->dc_call_id =%d, cmd_ptr->cmd.call_info.call_id =%d, cm_s_ptr->call_info.call_id =%d", 
             call_event,
             dc_s_ptr->dc_call_id,
             cmd_ptr->cmd.call_info.call_id,
             cm_s_ptr->call_info.call_id);

  if ( (CM_CALL_EVENT_1x_ORIG_FAIL == call_event) &&
       (CM_CALL_END_MC_ABORT == cmd_ptr->cmd.call_info.end_status) &&
       (dc_s_ptr->dc_call_id == cmd_ptr->cmd.call_info.call_id) )
  {
    reest_traffic_ch = FALSE;
  }
  ...
  if (!is_dc_call_connected) 
  {
    ...
    wms_msg_stop_dc_resend(reest_traffic_ch);
  }
  ...
  */
  if ( (cmd_err == WMS_CMD_ERR_NONE) && 
       (CM_CALL_EVENT_1x_ORIG_FAIL != call_event) )
  {
    wms_dc_cache_call_info( &(cmd_ptr->cmd.call_info) );
  ...
  if (reest_traffic_ch == FALSE)
  {
      MSG_HIGH_1("SMS call %d originated, but received call origination failure from lower layers", dc_s_ptr->dc_call_id);
