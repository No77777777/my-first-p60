/*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/wms/src/WMSE/wmstscdma.c#1 $
  ...
  while( st == WMS_OK_S  )
  {
	...
	if(( pos + 2 > bd_len) || (parm_len < 2) )
	 ...
	 if(( pos + 1 > bd_len) || (parm_len < 1) )
	 {
      ...
      default:
        MSG_HIGH_3( "Decoding BD: unrecognized parm_id: %d,parm_len: %d , new desired_other_len=%d", 
                    parm_id,
                    parm_len,
    ...
    case WMS_BD_TYPE_DELIVER:
      switch( (wms_teleservice_e_type)ts_type )
      {
        ...
        case WMS_TELESERVICE_CT_CDMA_WAP:
          if( wms_cm_CT_features_supported(wms_check_1x_active_sub()) && 
              !( cl_bd_ptr->mask & WMS_MASK_BD_USER_DATA ) )
          {
            st = WMS_MISSING_PARM_S;
          }
