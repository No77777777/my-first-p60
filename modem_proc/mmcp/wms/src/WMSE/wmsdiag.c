/*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/wms/src/WMSE/wmsdiag.c#1 $
                             ...
                             wms_cfg_event_info_s_type *info_ptr)
{
  ...
  wmsdiag_get_message_list_rsp_type *delay_rsp_pkt = NULL;
  ...
  /*  Allocate our packet */
  delay_rsp_pkt = (wmsdiag_get_message_list_rsp_type *) diagpkt_subsys_alloc_v2_delay(
                              (diagpkt_subsys_id_type)DIAG_SUBSYS_WMS,
                              (diagpkt_subsys_cmd_code_type)WMSDIAG_GET_MESSAGE_LIST_CMD,
                              delay_rsp_id,
                              sizeof(wmsdiag_get_message_list_rsp_type) );
  ...
  else
  {
    ...
    /* Populate Fields */
    delay_rsp_pkt->num_entries  = info_ptr->message_list.len;
    delay_rsp_pkt->storage_type = (uint32)info_ptr->message_list.mem_store;
    delay_rsp_pkt->voice_mail_index = info_ptr->message_list.voice_mail_index;
    delay_rsp_pkt->voice_mail_active = info_ptr->message_list.voice_mail_active;

    for (i = 0; i < (int)(delay_rsp_pkt->num_entries); i++)
    {
      delay_rsp_pkt->messages[i].tag_type =
        (uint32)wmsdiag_map_tag_type(info_ptr->message_list.tags[i]);
      delay_rsp_pkt->messages[i].index = info_ptr->message_list.indices[i];
    }
...
SIDE EFFECTS
  Sets wmsdiag_get_message_list_failed flag to indicate that there was an
  error, if applicable.
                          ...
                          wms_cmd_err_e_type cmd_err)
{
  ...
  if (cmd_err != WMS_CMD_ERR_NONE)
  {
    ...
    else
    {
      ...
      delay_rsp_pkt->num_entries = 0;
      delay_rsp_pkt->storage_type = WMS_MEMORY_STORE_MAX32;
      delay_rsp_pkt->voice_mail_index = (wms_message_index_type)WMS_DUMMY_MESSAGE_INDEX;
      delay_rsp_pkt->voice_mail_active = FALSE;
      delay_rsp_pkt -> status = 1;

      /* Send the Response */
      diagpkt_delay_commit(delay_rsp_pkt);
    }
  }
  /*  If no errors, wait for CFG Callback for Delayed Response*/
  else
  {
...
void wmsdiag_set_routes_result_log (void)
{
  ...
  if (log_rsp)
  {
    ...
    /*  Commit our packet */
  	log_util_log_packet_commit(SYS_MODEM_AS_ID_NONE, log_rsp);
  }
}
...
void wms_diag_print_etws_message(wms_etws_msg_s_type* etws_msg_ptr)
{
   ...
   {
       MSG_HIGH_7( "MCC %d %d %d, Number of MNC digits %d,  MNC %d %d %d",
                   etws_msg_ptr->plmn_info.mcc[0],
                   etws_msg_ptr->plmn_info.mcc[1],
                   etws_msg_ptr->plmn_info.mcc[2],
                   etws_msg_ptr->plmn_info.num_mnc_digits,
   ...
   if (WMS_ETWS_NOTIFICATION_PRIMARY == etws_msg_ptr->type)
   {
      ...
      MSG_HIGH_1("Receiving ETWS primary notification over RAT %d", etws_msg_ptr->rat);
      ...
      if (NULL != prim_msg_ptr)
      {
        ...
        {
          MSG_HIGH_6("ETWS primary notification with service id: %d alphabet: %d language: %d geo scope: %d message code: %d page number: %d",
          ...
          MSG_HIGH_6("ETWS primary notification with raw serial number: %d update number: %d total_page: %d emergency user alert: %d popup: %d warning type: %d", 
   ...
   else if (etws_msg_ptr->type == WMS_ETWS_NOTIFICATION_SECONDARY_UMTS_FORMAT)
   {
      ...
      MSG_HIGH_1("Receiving ETWS secondary notification over RAT %d", etws_msg_ptr->rat);
      ...
      if (NULL != sec_msg_ptr)
      {
        ...
        {
          MSG_HIGH_6("ETWS secondary notification with service id: %d alphabet: %d language: %d geo scope: %d message code: %d page number: %d", 
          ...
          MSG_HIGH_6("ETWS secondary notification with raw serial number: %d update number: %d total_page: %d emergency user alert: %d popup: %d cb_data length: %d",
                               ...
                               boolean *shared )
{
  ...
  else if( event == WMS_MSG_EVENT_RECEIVED_MESSAGE )
  {
    ...
    if (NULL != client_ts_data_ptr)
    {
      ...
      {
        MSG_HIGH_2("Receiving CMAS message with category: %d over RAT %d", service, info_ptr->mt_message_info.rat);
      ...
      else if (WMS_MESSAGE_MODE_GW == info_ptr->mt_message_info.message.msg_hdr.message_mode)
      {
                             ...
                             client_ts_data_ptr ) )
         {
           if(client_ts_data_ptr->format== WMS_FORMAT_GW_CB &&
              ...
              client_ts_data_ptr->u.gw_cb.cb_page_hdr.cb_srv_id <= WMS_GW_CB_SRV_ID_CMAS_MSGID30)
           {
              MSG_HIGH_2("Receiving CMAS message with message id: %d over RAT %d", 
                         client_ts_data_ptr->u.gw_cb.cb_page_hdr.cb_srv_id,
                         info_ptr->mt_message_info.rat);
...

{
  ...
  if (delay_rsp_pkt != NULL)
  {
     ...
     st = wms_cfg_ms_get_message_list (wmsdiag_client_id, 
                                       wmsdiag_convert_as_id(current_as_id),
                                       wmsdiag_get_message_list_cmd_cb,
                                       (void *)(uint32)req_ptr -> transaction_ID,
                                       (wms_memory_store_e_type)req_ptr -> storage_type, 
                                       WMSDIAG_MSG_TAG_VALS [0]);
     ...
     {
        diagpkt_subsys_set_status(delay_rsp_pkt, 1);
        MSG_ERROR_0("Get Message List failed");
     }
     else
     {
       diagpkt_subsys_set_status(delay_rsp_pkt, 0);
     ...
     /* Populate Fields */
     delay_rsp_pkt->num_entries = 0;
     delay_rsp_pkt->storage_type = (wms_memory_store_e_type)req_ptr -> storage_type;
     delay_rsp_pkt->voice_mail_index = (wms_message_index_type)WMS_DUMMY_MESSAGE_INDEX;
     delay_rsp_pkt->voice_mail_active = FALSE;
     delay_rsp_pkt -> status = 1;
...
)
{
  ...
  if (rsp_ptr)
  {
    ...
    {
      st = wms_dc_disconnect(wmsdiag_client_id, NULL, NULL,wms_check_1x_active_sub());
    }
