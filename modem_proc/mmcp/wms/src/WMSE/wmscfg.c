/*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/wms/src/WMSE/wmscfg.c#1 $
...
extern boolean wms_is_qsh_event_valid_and_enabled
(
...
/*===========================================================================
FUNCTION is_ue_camped_on_us_mcc

DESCRIPTION
  Decide whether UE is camped on US MCC or not. US MCC will be in the range of 310-316
...
boolean is_ue_camped_on_us_mcc(cm_country_code_type mcc)
{
  if((mcc>=310) && (mcc <=316))
  {
    return TRUE;   
...
)
{
  ...
  wms_log_event_notify_packet(client_id,event, client_ptr->cfg_notify_type );
...
)
{
...

  {
    ...
    case WMS_CFG_EVENT_MS_READY:
      ...
      break;
    case WMS_CFG_EVENT_MS_READY_STATUS:
      as_id = cfg_event_info_ptr->ms_ready_status.as_id;
      break;  
    case WMS_CFG_EVENT_MS_MEMORY_FULL:
...
void wms_cfg_send_ready_event_ex(
  ...
  int                         dest_client,
  wms_cfg_ready_status_e_type ready_status
)
{
  ...
  do
  {
    ...
    case MMGSDI_GW_PROV_TER_SESSION:
      if (WMS_CFG_READY_STATUS_LIMITED == ready_status)
      {
        cfg_event_ptr->ms_ready_status.as_id        = appInfo_ptr->asid;
        cfg_event_ptr->ms_ready_status.sys_type     = WMS_CFG_SYS_GW;
        cfg_event_ptr->ms_ready_status.ready_status = ready_status;
      }
      else
      {
    ...
    case MMGSDI_1X_PROV_SEC_SESSION:
      ...
      {
        cfg_event_ptr->ms_ready_status.as_id        = appInfo_ptr->asid;
        cfg_event_ptr->ms_ready_status.sys_type     = WMS_CFG_SYS_CDMA;
        cfg_event_ptr->ms_ready_status.ready_status = ready_status;
      }
      else
      {
      ...
      /* Read channel preference and servic eoption at CDMA ready instead of reading at power on */
      if((!appInfo_ptr->smscap_enabled) && 
	    ((WMS_CFG_READY_STATUS_LIMITED == ready_status) || (WMS_CFG_READY_STATUS_FULL == ready_status))&&
		(appInfo_ptr->asid == wms_check_1x_active_sub()))  
		
	  {
		/* Read Access or Traffic Channel from RUIM card */
        wms_nv_read_cdma_channel_setting(appInfo_ptr->asid);

        /* Read Service Option Setting from card */
        wms_nv_read_cdma_service_option(appInfo_ptr->asid);		 
	  }
      break;
    ...
    MSG_HIGH_6( "Sending READY status %d: appInfo_ptr 0x%x, as_id %d, sys_type %d, session_type %d, dest_client %d",
                ready_status,
                appInfo_ptr,
                appInfo_ptr->asid,
    ...
    { // Send to all clients
      ...
      if (WMS_CFG_READY_STATUS_FULL == ready_status)
      {
      ...
      else if (WMS_CFG_READY_STATUS_NO_SRV == ready_status)
      {
        // Send DS NOT_READY event (non-DS event does not exist)
        wms_cfg_event_notify( WMS_CFG_EVENT_MS_NOT_READY, cfg_event_ptr );
      }
      else if (WMS_CFG_READY_STATUS_LIMITED == ready_status)
      {
        // Send READY_STATUS event (non-DS event does not exist)
        wms_cfg_event_notify( WMS_CFG_EVENT_MS_READY_STATUS, cfg_event_ptr );
      }
    }
    else
    { // Send to single client
      ...
      {
        // Send READY_STATUS event (non-DS event does not exist)
        wms_cfg_event_notify_single_client( WMS_CFG_EVENT_MS_READY_STATUS, cfg_event_ptr, client_id );
      }
    }
  } while(0);
...
{
  wms_cfg_send_ready_event_ex(appInfo_ptr, dest_client, WMS_CFG_READY_STATUS_FULL);
...
{
  wms_cfg_send_ready_event_ex(appInfo_ptr, dest_client, WMS_CFG_READY_STATUS_NO_SRV);

  return;
} /* wms_cfg_send_not_ready_event() */

/*===========================================================================
FUNCTION wms_cfg_send_limited_ready_event()                            (INTERNAL)

DESCRIPTION
  Sends limited ready event to clients based on the subscription
  information provided.
...
SIDE EFFECTS
  None
===========================================================================*/
void wms_cfg_send_limited_ready_event(
  wms_prov_app_info_s_type   *appInfo_ptr,
  int                         dest_client
)
{
  wms_cfg_send_ready_event_ex(appInfo_ptr, dest_client, WMS_CFG_READY_STATUS_LIMITED);

  return;
} /* wms_cfg_send_limited_ready_event() */
...
)
{
  ...
  else
  {
    ...
    msgr_set_hdr_variant((msgr_hdr_struct_type*)&cm_lte_domain_pref,(uint8)(cmd_ptr->hdr.as_id));
...
)
{
      ...
      (MMGSDI_APP_USIM == appInfo_ptr->mmgsdi_appInfo.app_type))
  {
    ...
    wms_bc_gw_cb_init(appInfo_ptr,TRUE,FALSE);
#endif
  }
...
#ifdef FEATURE_QSH_EVENT_NOTIFY_TO_QSH
     {
       if(WMS_CFG_SYS_CDMA == cfg_event_ptr->end_sim_init.sys_type )
       {
         qsh_evt= WMS_QSH_EVENT_3GPP2_SIM_INIT_START;
       }
       else if(WMS_CFG_SYS_GW == cfg_event_ptr->end_sim_init.sys_type)
       {
         qsh_evt= WMS_QSH_EVENT_3GPP_SIM_INIT_START;
       }
 
       MSG_HIGH_0("WMS_CFG_EVENT_MS_SIM_INIT_START");
       if(wms_is_qsh_event_valid_and_enabled(qsh_evt,appInfo_ptr->asid) == TRUE)
       {
		 MSG_HIGH_0("WMS_CFG_EVENT_MS_SIM_INIT_START");
         wms_qsh_client_event_log(qsh_evt, 0,appInfo_ptr->asid );
       }
     }
...

  {
    ...
    if (NULL != cfg_event_ptr)
    {
     ...
     if(appInfo_ptr->sim_init_complete)
	 {
       if(WMS_CFG_SYS_CDMA == cfg_event_ptr->end_sim_init.sys_type )
       {
         qsh_evt= WMS_QSH_EVENT_3GPP2_SIM_INIT_END;
       }
       else if(WMS_CFG_SYS_GW == cfg_event_ptr->end_sim_init.sys_type)
       {
         qsh_evt= WMS_QSH_EVENT_3GPP_SIM_INIT_END;
       }
	   
	   MSG_HIGH_0("WMS_CFG_EVENT_MS_SIM_INIT_END");
...
)
{
  ...
  wms_cfg_event_info_s_type          *cfg_event_ptr = NULL;
  wms_prov_app_info_s_type           *appInfo_ptr   = cmd_ptr->hdr.appInfo_ptr;
  wms_client_id_type                  client_id     = cmd_ptr->hdr.client_id;
  boolean                             send_to_NAS   = FALSE;
  ...
  do
  {
    ...
    MSG_HIGH_7("Current link control data : mode = %d ,timer_val = %d , status = %d ,Client's(%d) given link control data : mode = %d,timer_val = %d,status = %d",
                appInfo_ptr->current_used_link_control_mode,
                appInfo_ptr->current_used_link_control_timer_val,
                appInfo_ptr->current_used_link_control_status,
                client_id,
                link_control_mode,
                link_control_timer,
                link_control_status);

    appInfo_ptr->client_link_control_data[client_id].link_control_mode   = link_control_mode;
    appInfo_ptr->client_link_control_data[client_id].link_control_status = link_control_status;
    appInfo_ptr->client_link_control_data[client_id].link_control_timer  = link_control_timer;

    if (WMS_LINK_CONTROL_DISABLED == link_control_mode)
    {
      uint8 i;
      ...
      {
        if (WMS_LINK_CONTROL_ENABLED_ONE == appInfo_ptr->client_link_control_data[i].link_control_mode ||
            WMS_LINK_CONTROL_ENABLE_ALL  == appInfo_ptr->client_link_control_data[i].link_control_mode)
        {
          MSG_HIGH_0("Link Control not disabled, other client/clients using link control");
          send_to_NAS = FALSE;
          break;
        }     
      }
      ...
      if (TRUE == send_to_NAS)
      {
        MSG_HIGH_0("Currently used link control data updated");
        appInfo_ptr->current_used_link_control_mode                = link_control_mode;
        appInfo_ptr->current_used_link_control_timer_val           = link_control_timer;
        appInfo_ptr->current_used_link_control_status              = link_control_status;
      }
    }
    else
    { /*  mode == WMS_LINK_CONTROL_ENABLED_ONE or mode == WMS_LINK_CONTROL_ENABLE_ALL */
      if (link_control_mode  > appInfo_ptr->current_used_link_control_mode)
      {
        send_to_NAS = TRUE;
        appInfo_ptr->current_used_link_control_mode = link_control_mode;
      }
      if (link_control_timer > appInfo_ptr->current_used_link_control_timer_val)
      {
        send_to_NAS = TRUE;
        appInfo_ptr->current_used_link_control_timer_val = link_control_timer;      
      }
      if (link_control_status > appInfo_ptr->current_used_link_control_status)
      {
        appInfo_ptr->current_used_link_control_status = link_control_status;
      }
    }
    ...
    if (TRUE == send_to_NAS)
    {
      MSG_HIGH_3("Semding the link control request to NAS: Mode: %d, Timer: %d, Status: %d",
                 appInfo_ptr->current_used_link_control_mode,
                 appInfo_ptr->current_used_link_control_timer_val,
                 appInfo_ptr->current_used_link_control_status);
      cmd_err = wms_cm_gw_rl_link_control(appInfo_ptr->asid, appInfo_ptr->current_used_link_control_mode, appInfo_ptr->current_used_link_control_timer_val);
    }
  ...
  if (WMS_CMD_ERR_NONE == cmd_err)
  {
    ...
    cfg_event_ptr->link_control.link_control_mode   = appInfo_ptr->current_used_link_control_mode;
    cfg_event_ptr->link_control.link_control_status = appInfo_ptr->current_used_link_control_status;
...
)
{
  ...
  appInfo_ptr->current_used_link_control_mode     = (wms_cfg_link_control_mode_e_type) ind_ptr->mode;
  appInfo_ptr->current_used_link_control_status   = (wms_cfg_link_control_status_e_type) ind_ptr->status;

  MSG_HIGH_0("Resetting link control timer value to Zero and resetting link control data of all clients");
  appInfo_ptr->current_used_link_control_timer_val = 0;

  // Link got released by NAS, reset the link control  client data.
   memset(appInfo_ptr->client_link_control_data, 0 , sizeof(appInfo_ptr->client_link_control_data));
...
)
{
  ...
  if ( (0 != tx_id) && (NULL != send_evt_cb) )
  {
    ...
    MSG_HIGH_4( "wms_cfg_send_sim_file_read_complete_evt_proc() session_type: %d, tx_id: %d, send_evt_cb: (0x%x) appInfo_ptr 0x%x",
...
)
{
  ...
  if (NULL != cfg_s_ptr->wms_app4_info)
  {
    for (idx = 0; idx < WMS_MAX_TEMPLATES; idx++)
    {
      WMS_MEM_FREEIF(cfg_s_ptr->wms_app4_info->smsp_raw_rec[idx].data_ptr);
      cfg_s_ptr->wms_app4_info->smsp_raw_rec[idx].data_len = 0;
    }
  }
...
)
{
  ...
  memset(&cfg_s_ptr->wms_mmgsdi_app4_info, 0, sizeof(wms_prov_mmgsdi_app_info_s_type));
  ...
  cfg_s_ptr->wms_mmgsdi_app4_info.session_type       = MMGSDI_1X_PROV_SEC_SESSION;
  ...
  // Initialize the subs_pref session state
  {
    ...
    for (idx = 0; idx < MAX_AS_IDS; idx++)
    {
      ...
      cfg_s_ptr->wms_cm_subs_info.sub[idx].subs_capability = SYS_SYS_MODE_MASK_NONE;
    }
  }
...
)
{
...

 {
    ...
    for (i = 0; i < MAX_AS_IDS; i++)
    {
      ...
      if (!wms_nv_read_ens_flag(&cfg_s_ptr->ens_flag[i],i))
      {
        cfg_s_ptr->ens_flag[i] = FALSE;
      }
    }
 }
  ...
  // Read the ESN from NV
  {
    ...
    if ( NULL != nvi_ptr && NV_DONE_S == wms_nv_read_wait_per_subs(SYS_MODEM_AS_ID_1,NV_ESN_I, nvi_ptr ) )
    {
...

  {
    ...
    for ( i = 0; i < MAX_AS_IDS; i++ )
    {
      cfg_s_ptr->sys_mode[i] = SYS_SYS_MODE_NO_SRV;   
    }
  }
...
)
{
  ...
  */
  if (wms_nv_rtre_control_is_nv(as_id))
  {
...
)
{
  ...
  if (!wms_check_appinfo_is_valid(appInfo_ptr))
  {
    MSG_HIGH_1("In wms_cfg_update_voice_mail_index: Invalid appInfo_ptr 0x%x", appInfo_ptr);
    return;
  }
...
)
{
  ...
  */
      ...
      for (as_id_index = 0; as_id_index < MAX_AS_IDS; as_id_index++)
      {
        ...
        switch (as_id_index)
        {
          ...
          case 1: // SYS_MODEM_AS_ID2
            appInfo_ptr = wms_get_appinfo_by_session_type(MMGSDI_1X_PROV_SEC_SESSION);
            break;

          default:
            break;
        }
        if ( ( NULL != appInfo_ptr ) &&  NULL != client_ptr->cfg_event_cb)
        {
            ...
            else if (appInfo_ptr->sim_init_limited && appInfo_ptr->is_available)
            {
              wms_cfg_send_limited_ready_event( appInfo_ptr, client_id );
            }
  ...
  if ( ( NULL != client_ptr ) &&
       ...
       (( NULL != client_ptr->msg_event_cb ) || (NULL != client_ptr->transport_status_cb)))
  {
    ...
    for (as_id_index = 0; as_id_index < MAX_AS_IDS; as_id_index++)
    {
      ...
      if (NULL != transport_ptr)
      {
        ...
        if (NULL != client_ptr->transport_status_cb)
        {
         ...
         if (WMS_MSG_TRANSPORT_NW_REG_STATUS_NO_SRV_ALLOW_ACK == status.transport_status)
         {
         ...
         if (client_ptr->transport_status_cb )
         {
           MSG_HIGH_1("Notify client %d transport status change", client_id);
           client_ptr->transport_status_cb(&status);
         }
       }
      }
    }
       ...
       (WMS_OK_S == wms_ts_decode(message, client_ts_ptr)) )
  {
    ...
    if( wms_cfg_check_cphs_msg(&client_ts_ptr->u.gw_pp.u.deliver.address) )
    {
      if( mwi_info_arr != NULL && item_count < mwi_info_arr_size )
      {
        ...
        if ((asid_is_valid(as_id))&&(cfg_s_ptr->ens_flag[as_id] && mwi_info_arr[item_count].is_active))
        {
...
)
{
  ...
  if(msg_ptr->msg_hdr.message_mode == WMS_MESSAGE_MODE_CDMA)
  {
    ...
    else if(wms_cm_CT_features_supported(wms_check_1x_active_sub()) && 
...
)
{
  ...
  MSG_HIGH_5("=SMS Active Bearers= WMS_BEARER_CDMA_1X: %d, WMS_BEARER_CDMA_EVDO: %d, WMS_BEARER_CDMA_WLAN: %d, WMS_BEARER_LTE: %d, S102: %d",
             cfg_s_ptr->active_bearers[WMS_BEARER_CDMA_1X],
...
)
{
  ...
  wms_prov_app_info_s_type       *appInfo_ptr = NULL;
  boolean                         camped_on_us_mcc = FALSE;
  ...
  for ( stack_idx = 0; stack_idx < MIN(cmd_ss_ptr->number_of_stacks, CM_NO_STACKS); stack_idx++ )
  {
    ...
    {
      MSG_HIGH_8( "wms_cfg_ss_change_info: as_id = %d, stack[%d]: srv_status = %d, sys_mode = %d, id_type = %d, srv_domain = %d, current_mcc =%d, home_mcc = %d",
                   cmd_ptr->hdr.as_id,
                   stack_idx,
                   ...
                   cmd_ss_ptr->current_mobile_country_code,
                   cmd_ss_ptr->home_mobile_country_code);

      if (( NULL != cfg_s_ptr ) && (((cmd_ptr->hdr.as_id) > SYS_MODEM_AS_ID_NONE) && ((cmd_ptr->hdr.as_id) < (sys_modem_as_id_e_type)MAX_AS_IDS)))
      {
        if ((SYS_SRV_STATUS_SRV == cmd_ss_stack_ptr->srv_status) && (cmd_ss_stack_ptr->sys_mode == SYS_SYS_MODE_LTE))
        {
           cfg_s_ptr->sys_mode[cmd_ptr->hdr.as_id] = cmd_ss_stack_ptr->sys_mode;
        }
        else
        {
           cfg_s_ptr->sys_mode[cmd_ptr->hdr.as_id] = SYS_SYS_MODE_NO_SRV;
        }
      }
      else
      {
      ...
      if ( SYS_SYS_ID_TYPE_IS95 == cmd_ss_stack_ptr->sys_id.id_type )
      {
        ...
        if ( NULL != appInfo_ptr )
        {
          ...
          if (NULL != appInfo_ptr)
          {
            MSG_HIGH_2("Clear up GW appInfo_ptr->stack_info[%d] on SUB(%d)", stack_idx,cmd_ptr->hdr.as_id);
      ...
      else if ( SYS_SYS_ID_TYPE_UMTS == cmd_ss_stack_ptr->sys_id.id_type )
      {
       ...
       else if(cfg_s_ptr->active_bearers[WMS_BEARER_LTE] && (cfg_s_ptr->lte_sub_info == cmd_ptr->hdr.as_id) )
       {
        ...
        if (NULL != appInfo_ptr)
        {
          ...
          if ( cfg_s_ptr->active_bearers[WMS_BEARER_LTE] &&
		      (SYS_SYS_ID_TYPE_IS95 == appInfo_ptr->stack_info[stack_idx].sys_id.id_type) &&
               (appInfo_ptr->stack_info[stack_idx].asubs_id == cmd_ss_ptr->stack_info[stack_idx].asubs_id) )
          {
             //1x bearer is not sub specific. So need to reset the bearer flag only if current sub is 1x
            if (wms_check_1x_active_sub() == cmd_ptr->hdr.as_id)
            {
        ...
        if ( NULL != appInfo_ptr )
        {
               ...
               (SYS_SRV_STATUS_LIMITED_REGIONAL == cmd_ss_stack_ptr->srv_status) )
          {
            if (!appInfo_ptr->sim_init_complete)
            {
              ...
              {
				camped_on_us_mcc = is_ue_camped_on_us_mcc(cmd_ss_ptr->current_mobile_country_code);
                // Send NV-only BC config to NAS
                wms_bc_gw_cb_init(appInfo_ptr, FALSE,camped_on_us_mcc);
          ...
          wms_msg_notify_SMMA( appInfo_ptr, FALSE );
          {
            ...
            if (NULL != appInfo_ptr)
            {
              MSG_HIGH_2("Clear up CDMA appInfo_ptr->stack_info[%d] on sub(%d) ", stack_idx,cmd_ptr->hdr.as_id);
              memset(&appInfo_ptr->stack_info[stack_idx], 0, sizeof(wms_ss_stack_info_s_type));
            }
          }
        }
 ...
 for (asid = SYS_MODEM_AS_ID_1; asid < MAX_AS_IDS; asid++)
  {
    ...
    if ( (NULL != appInfo_ptr) )
    {
      ...
      else
      {
        ...
        if (appInfo_ptr->sim_init_complete)
        {
          MSG_HIGH_4("Send WMS_CFG_EVENT_GW_READY event: appInfo_ptr 0x%x, asid %d ,LPM mode = %d, WMS sim_init_complete = %d",
                      appInfo_ptr,
                      appInfo_ptr->asid,
                      in_lpm,
...
)
{
  ...
  MSG_HIGH_1("In wms_cfg_handle_cdma_lpm (lpm %d)", in_lpm);

  for (asid = SYS_MODEM_AS_ID_1; asid < MAX_AS_IDS; asid++)
  {
    appInfo_ptr = wms_get_appinfo_by_asid_and_message_mode(asid, WMS_MESSAGE_MODE_CDMA);
    ...
    if ( (NULL != appInfo_ptr) )
    {
      ...
      else
      {
        if (appInfo_ptr->sim_init_complete)
        {
          MSG_HIGH_4( "Send  WMS_CFG_EVENT_CDMA_READY event: appInfo_ptr 0x%x, asid %d,LPM mode = %d, WMS sim_init_complete = %d",
                     appInfo_ptr,
                     appInfo_ptr->asid,
                     in_lpm,
                     appInfo_ptr->sim_init_complete                    
...
)
{
  ...
  MSG_HIGH_1("wms_cfg_rrc_cell_change_cb_proc(): as_id %d", as_id);
  ...
  memset(cfg_s_ptr->umts_loc_info[as_id].mcc_and_mnc, 0x00, 3);

  cfg_s_ptr->umts_loc_info[as_id].cell_id[1] = (uint8)(cell_info_ptr->cell_id & 0x000000FF);
  cfg_s_ptr->umts_loc_info[as_id].cell_id[0] = (uint8)((cell_info_ptr->cell_id >> 8) & 0x000000FF);
  cfg_s_ptr->umts_loc_info[as_id].cell_id[3] = (uint8)((cell_info_ptr->cell_id >> 16) & 0x000000FF);
  cfg_s_ptr->umts_loc_info[as_id].cell_id[2] = (uint8)((cell_info_ptr->cell_id >> 24) & 0x000000FF);
  ...
  MSG_HIGH_3("RRC LAC: 0x%x 0x%x, Cell ID: 0x%x",
           cell_info_ptr->lac[0],
           cell_info_ptr->lac[1],
  ...
  ** ---------------------------------------- */
  cfg_s_ptr->umts_loc_info[as_id].mcc_and_mnc[0] =  (cell_info_ptr->plmn_id.mcc[1] << 4) + cell_info_ptr->plmn_id.mcc[0];

  /* ----------------------------------------
  ** PLMN octet 2 = MNC digit 3 | MCC digit 3
  ** ---------------------------------------- */
  cfg_s_ptr->umts_loc_info[as_id].mcc_and_mnc[1] = (cell_info_ptr->plmn_id.mnc[2] << 4) + cell_info_ptr->plmn_id.mcc[2];

  /* ----------------------------------------
  ** PLMN octet 3 = MNC digit 2 | MNC digit 1
  ** ---------------------------------------- */
  cfg_s_ptr->umts_loc_info[as_id].mcc_and_mnc[2] = (cell_info_ptr->plmn_id.mnc[1] << 4) + cell_info_ptr->plmn_id.mnc[0];

  cfg_s_ptr->umts_loc_info[as_id].loc_area_code[0] = cell_info_ptr->lac[0];
  cfg_s_ptr->umts_loc_info[as_id].loc_area_code[1] = cell_info_ptr->lac[1];
}
#endif /* (defined(FEATURE_WCDMA) || defined (FEATURE_TDSCDMA)) */
