/*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/wms/src/WMSE/wmsts.c#1 $
...
)
{
  ...
  if( dcs->alphabet == WMS_GW_ALPHABET_7_BIT_DEFAULT )
  {
    ...
    if(user_data_header_present)
    {
      ...
      //If UDH is present then its lenght cannot be zero
...
{
  const wms_raw_ts_data_s_type      *raw_ts_ptr	   = NULL;
  wms_status_e_type                 st             = WMS_OK_S;

  if (NULL == is_concatenated)
  {
    wms_check_for_null_ptr();
    return WMS_NULL_PTR_S;
  }
  ...
  *is_concatenated = FALSE;
  ...
  do
  {
    ...
    raw_ts_ptr = &message_ptr->u.gw_message.raw_ts_data;

    if (WMS_FORMAT_GW_PP != raw_ts_ptr->format)
    {
      MSG_ERROR_1("Invalid format: %d", raw_ts_ptr->format);
      st = WMS_INVALID_FORMAT_S;
      break;
    }

    if (WMS_TPDU_SUBMIT != raw_ts_ptr->tpdu_type)
    {
      MSG_ERROR_1("Invalid TPDU type %d", raw_ts_ptr->tpdu_type);
      st = WMS_INVALID_TPDU_TYPE_S;
      break;
    }

    {
      ...
      wms_gw_submit_s_type *submit_tpdu = 
       (wms_gw_submit_s_type *) wms_mem_malloc ( sizeof(wms_gw_submit_s_type) );
      ...
      st = wms_ts_decode_submit(raw_ts_ptr, submit_tpdu, NULL);
      ...
      if (submit_tpdu->user_data_header_present)
      {
        uint8 i;
     
        for (i =0; i < submit_tpdu->user_data.num_headers; i++)
        {
          if (WMS_UDH_CONCAT_8  == submit_tpdu->user_data.headers[i].header_id ||
              WMS_UDH_CONCAT_16 == submit_tpdu->user_data.headers[i].header_id)
          {
            MSG_HIGH_1("wms_ts_submit_msg_is_concatenated returns TRUE, found IE header id %d",
                                                        submit_tpdu->user_data.headers[i].header_id);
            *is_concatenated = TRUE;
            break;
          }
        }
      }
      wms_mem_free( submit_tpdu );
    }
  } while(0);
