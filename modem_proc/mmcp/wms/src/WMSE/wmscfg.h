...
/*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/wms/src/WMSE/wmscfg.h#1 $
...
typedef struct wms_cfg_struct
{
  ...
  wms_prov_mmgsdi_app_info_s_type   wms_mmgsdi_app4_info;
  ...
  wms_prov_app_info_s_type         *wms_app4_info;  // 1X_SEC, subs2
...
#ifdef FEATURE_GWSMS
  boolean                           ens_flag[MAX_AS_IDS];       /* read from NV */
  ...
  */
  ...
  sys_sys_mode_e_type               sys_mode[MAX_AS_IDS];                       // System modes
  sys_sys_mode_e_type               gw_sys_mode;                    // HYBR System mode
  boolean                           is_hybr_gw_operational;
#endif

  boolean                           admin_sms_only[WMS_MSG_TRANSPORT_ARR_MAX]; // only admin sms msg is allowed to be sent over IMS
  boolean                           no_srv_ack_only[WMS_MSG_TRANSPORT_ARR_MAX]; //only MT SMS ack is allowed to be sent over IMS

  boolean                           simul_sms_voice_over_1x_allowed; // whether SMS is allowed to be send
                                                                // over 1x while voice call is active or not
  uint8                        sms_retry_limit[MAX_AS_IDS]; // Limit number of retries within the config retry timer interval                                                                 // over 1x while voice call is active or not
  ...
  gstk_gwl_loc_info                 umts_loc_info[SYS_MODEM_AS_ID_MAX];
  ...
  boolean                           rrc_disable_ta_for_embms; 

  cm_hybr_pref_e_type               hybr_pref;
 
#ifdef FEATURE_QSH_EVENT_NOTIFY_TO_QSH
  boolean                              wms_qsh_event_enabled[MAX_AS_IDS][WMS_QSH_EVENT_MAX];
#endif

  boolean                           allow_e911_sms_over_ims[MAX_AS_IDS]; //whether E911 SMS needs to be allowed in limited service or not.
