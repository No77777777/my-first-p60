/*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/wms/src/WMSE/wmsmsg.c#2 $
  $DateTime: 2024/01/09 09:25:59 $
  $Author: pwbldsvc $
*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*/
...
#define WMS_MAX_RETRY_COUNT_EMBMS                5
/*===========================================================================
======================== LOCAL DATA =========================================
===========================================================================*/
...
static wms_cell_info_s_type current_cell_info_sim[MAX_AS_IDS];
#define current_cell_info current_cell_info_sim[wms_global_as_id]
...
uint32 get_mo_retry_period(sys_modem_as_id_e_type as_id)
{
  if (!asid_is_valid(as_id))
  {
    MSG_ERROR_0( "Invalid as_id" );	
	return msg_s_ptr->mo_retry_period[SYS_MODEM_AS_ID_1];
  }

  return msg_s_ptr->mo_retry_period[as_id];
  
}
uint32 get_mo_retry_interval(sys_modem_as_id_e_type as_id)
{
  if (!asid_is_valid(as_id))
  {
    MSG_ERROR_0( "Invalid as_id" );
	return msg_s_ptr->mo_retry_interval[SYS_MODEM_AS_ID_1];
  }
  return msg_s_ptr->mo_retry_interval[as_id];
}
...
void wms_mn_mo_sms_callback_proc(wms_cmd_type      *cmd_ptr);
...
boolean wms_is_event_debug_any_failure_enabled
(
  wms_qsh_event_e qsh_evt,sys_modem_as_id_e_type as_id
)
{
  if(qsh_evt >= WMS_QSH_EVENT_MAX)
    return FALSE;

  QSH_LOG(QSH_CLT_WMS,QSH_CAT_EVENT,QSH_MSG_TYPE_HIGH,"WMS_QSH_EVENT_FIELD_DEBUG_ANY_FAILURE  Event %d",qsh_evt);
  MSG_HIGH_1("qsh any failure event enabled : %d",cfg_s_ptr->wms_qsh_event_enabled[as_id][(byte)qsh_evt] );
  return (cfg_s_ptr->wms_qsh_event_enabled[as_id][(byte)WMS_QSH_EVENT_FIELD_DEBUG_ANY_FAILURE]);

}


boolean wms_is_qsh_event_valid_and_enabled
(
  wms_qsh_event_e qsh_evt,sys_modem_as_id_e_type as_id
)
{
  ...
  MSG_HIGH_2(" qsh event enabled : %d, qsh_event:%d",cfg_s_ptr->wms_qsh_event_enabled[as_id][(byte)qsh_evt],qsh_evt);
  return (cfg_s_ptr->wms_qsh_event_enabled[as_id][(byte)qsh_evt]);

}


wms_qsh_event_e wms_map_event_to_qsh_event(wms_report_status_e_type status)
{
  wms_qsh_event_e qsh_evt = WMS_QSH_EVENT_MAX; 
  switch(status)
  {
    ...
    case WMS_RPT_CP_ERROR:
      qsh_evt=  WMS_QSH_EVENT_SMS_FAIL_AFTER_RETRY;
      break;

    case WMS_RPT_SMR_TR1M_TIMEOUT:
    case WMS_RPT_SMR_TR2M_TIMEOUT:
      qsh_evt=  WMS_QSH_EVENT_TR1M_OR_TR2M_TIMEOUT;
      break;

    case WMS_RPT_RADIO_UNAVAILABLE: 
      qsh_evt=  WMS_QSH_EVENT_RADIO_UNAVAILABLE;
      break;

    case  WMS_RPT_NO_RESPONSE_FROM_NETWORK:
      qsh_evt=  WMS_QSH_EVENT_NO_RESPONSE_FROM_NETWORK;
      break;

    case WMS_RPT_RADIO_LINK_ENDED_FOR_SUBMIT_REPORT:
      qsh_evt=  WMS_QSH_EVENT_RADIO_LINK_ENDED_FOR_SUBMIT_REPORT;
      break;

     case WMS_RPT_RP_ERROR:
      switch(msg_event_info.submit_report_info.cause_info.cause_value)
      {
        case WMS_RP_CAUSE_UNASSIGNED_NUMBER:
          qsh_evt = WMS_QSH_EVENT_SMS_FAIL_RP_UNASSIGNED_NUMBER;
          break;
        case WMS_RP_CAUSE_CALL_BARRED:
          qsh_evt =  WMS_QSH_EVENT_SMS_FAIL_RP_CALL_BARRED;
          break;
        case WMS_RP_CAUSE_SM_REJECTED:
          qsh_evt =  WMS_QSH_EVENT_SMS_FAIL_RP_SM_REJECTED;
          break;
        case WMS_RP_CAUSE_UNIDENTIFIED_SUBSCRIBER:
          qsh_evt =  WMS_QSH_EVENT_SMS_FAIL_RP_UNIDENTIFIED_SUBSCRIBER;
          break;
        case WMS_RP_CAUSE_UNKNOWN_SUBSCRIBER:
          qsh_evt =  WMS_QSH_EVENT_SMS_FAIL_RP_UNKNOWN_SUBSCRIBER;
          break;
        case WMS_RP_CAUSE_INVALID_MANDATORY_INFORMATION:
          qsh_evt =  WMS_QSH_EVENT_SMS_FAIL_RP_INVALID_MANDATORY_INFORMATION;
          break;
        default:
           qsh_evt =  WMS_QSH_EVENT_SMS_FAIL_GENERAL_ERROR;
           break;
      }
      break;

    case WMS_RPT_1X_AND_VOICE_NOT_ALLOWED:
      qsh_evt =  WMS_QSH_EVENT_SMS_FAIL_1X_AND_VOICE_NOT_ALLOWED;
      break;

    case WMS_RPT_TRANSPORT_NOT_READY    :
    case WMS_RPT_TRANSPORT_NOT_ALLOWED        :
      qsh_evt =  WMS_QSH_EVENT_SMS_FAIL_NO_TRANSPORT;
      break;

    case WMS_RPT_PHONE_NOT_READY:
      qsh_evt =  WMS_QSH_EVENT_SMS_FAIL_PHONE_NOT_READY;
      break;

    case WMS_RPT_NETWORK_NOT_READY:
      qsh_evt =  WMS_QSH_EVENT_SMS_FAIL_NETWORK_NOT_READY;
      break;
...
void wms_qsh_client_event_log(wms_qsh_event_e qsh_evt, wms_report_status_e_type status,sys_modem_as_id_e_type as_id )
{

  qsh_client_event_notify_params_s event_notify_params;
  qsh_client_event_notify_init(&event_notify_params);
  event_notify_params.client = QSH_CLT_WMS;
  event_notify_params.id = qsh_evt;
  ...
  switch(qsh_evt)
  {
    case WMS_QSH_EVENT_SMS_FAIL_RP_UNASSIGNED_NUMBER:
      QSH_LOG(QSH_CLT_WMS,QSH_CAT_EVENT,QSH_MSG_TYPE_HIGH,"AS_ID=%d, SMS_FAIL- end_reason= RP_UNASSIGNED_NUMBER and status: %d",as_id, status );
      break;

    case WMS_QSH_EVENT_SMS_FAIL_RP_CALL_BARRED:
      QSH_LOG(QSH_CLT_WMS,QSH_CAT_EVENT,QSH_MSG_TYPE_HIGH,"AS_ID=%d, SMS_FAIL - end_reason= RP_CALL_BARRED and status:%d",as_id, status );
      break;

    case WMS_QSH_EVENT_SMS_FAIL_RP_SM_REJECTED:
      QSH_LOG(QSH_CLT_WMS,QSH_CAT_EVENT,QSH_MSG_TYPE_HIGH,"AS_ID=%d, SMS_FAIL- end_reason=RP_SM_REJECTED and status:%d",as_id, status );
      break;

    case WMS_QSH_EVENT_SMS_FAIL_RP_UNIDENTIFIED_SUBSCRIBER:
      QSH_LOG(QSH_CLT_WMS,QSH_CAT_EVENT,QSH_MSG_TYPE_HIGH,"AS_ID=%d, SMS_FAIL - end_reason=RP_UNIDENTIFIED_SUBSCRIBER and status:%d",as_id, status );
      break;

    case WMS_QSH_EVENT_SMS_FAIL_RP_UNKNOWN_SUBSCRIBER:
      QSH_LOG(QSH_CLT_WMS,QSH_CAT_EVENT,QSH_MSG_TYPE_HIGH,"AS_ID=%d, SMS_FAIL - end_reason=RP_UNKNOWN_SUBSCRIBER and status:%d",as_id, status );
      break;

    case WMS_QSH_EVENT_SMS_FAIL_RP_INVALID_MANDATORY_INFORMATION:
      QSH_LOG(QSH_CLT_WMS,QSH_CAT_EVENT,QSH_MSG_TYPE_HIGH,"AS_ID=%d, SMS_FAIL - end_reason=RP_INVALID_MANDATORY_INFORMATION and status:%d",as_id, status );
      break;

    case WMS_QSH_EVENT_SMS_FAIL_ERR_AS_ID:
      QSH_LOG(QSH_CLT_WMS,QSH_CAT_EVENT,QSH_MSG_TYPE_HIGH,"AS_ID=%d, SMS_FAIL- end_reason= ERR_AS_ID and status:%d",as_id, status );
      break;

    case WMS_QSH_EVENT_SMS_FAIL_GENERAL_ERROR:
      QSH_LOG(QSH_CLT_WMS,QSH_CAT_EVENT,QSH_MSG_TYPE_HIGH,"AS_ID=%d, SMS_FAIL - end_reason=GENERAL_ERROR and status:%d",as_id, status );
      break;

    case WMS_QSH_EVENT_SMS_FAIL_PHONE_NOT_READY:
      QSH_LOG(QSH_CLT_WMS,QSH_CAT_EVENT,QSH_MSG_TYPE_HIGH,"AS_ID=%d, SMS_FAIL - end_reason=PHONE_NOT_READY and status:%d",as_id, status );
      break;


    case WMS_QSH_EVENT_SMS_FAIL_NO_TRANSPORT:
      QSH_LOG(QSH_CLT_WMS,QSH_CAT_EVENT,QSH_MSG_TYPE_HIGH,"AS_ID=%d, SMS_FAIL - end_reason=FAIL_NO_TRANSPORT and status:%d",as_id, status );
      break;

    case WMS_QSH_EVENT_SMS_FAIL_1X_AND_VOICE_NOT_ALLOWED:
      QSH_LOG(QSH_CLT_WMS,QSH_CAT_EVENT,QSH_MSG_TYPE_HIGH,"AS_ID=%d, SMS_FAIL - end_reason=1X_AND_VOICE_NOT_ALLOWED and status:%d",as_id, status );
      break;

    case WMS_QSH_EVENT_SMS_FAIL_NETWORK_NOT_READY:
      QSH_LOG(QSH_CLT_WMS,QSH_CAT_EVENT,QSH_MSG_TYPE_HIGH,"AS_ID=%d, SMS_FAIL - end_reason=NETWORK_NOT_READY and status:%d",as_id, status );
      break;

    case WMS_QSH_EVENT_CMD_QUEUE_FULL:
      QSH_LOG(QSH_CLT_WMS,QSH_CAT_EVENT,QSH_MSG_TYPE_HIGH,"AS_ID=%d, SMS_FAIL - end_reason= CMD_QUEUE_FULL and status:%d",as_id, status );
      break;

    case WMS_QSH_EVENT_SMS_FAIL_AFTER_RETRY:
      QSH_LOG(QSH_CLT_WMS,QSH_CAT_EVENT,QSH_MSG_TYPE_HIGH,"AS_ID=%d, SMS_FAIL - end_reason= AFTER_RETRY and status:%d",as_id, status );
      break;

     case WMS_QSH_EVENT_TR1M_OR_TR2M_TIMEOUT:
      QSH_LOG(QSH_CLT_WMS,QSH_CAT_EVENT,QSH_MSG_TYPE_HIGH,"AS_ID=%d, SMS_FAIL - end_reason= TR1M_OR_TR2M_TIMEOUT and status:%d",as_id, status );
      break;

     case WMS_QSH_EVENT_RADIO_UNAVAILABLE:
      QSH_LOG(QSH_CLT_WMS,QSH_CAT_EVENT,QSH_MSG_TYPE_HIGH,"AS_ID=%d, SMS_FAIL - end_reason= RADIO_UNAVAILABLE and status:%d",as_id, status );
      break;

     case WMS_QSH_EVENT_NO_RESPONSE_FROM_NETWORK:
      QSH_LOG(QSH_CLT_WMS,QSH_CAT_EVENT,QSH_MSG_TYPE_HIGH,"AS_ID=%d, SMS_FAIL - end_reason= NO_RESPONSE_FROM_NETWORK and status:%d",as_id, status );
      break;

     case WMS_QSH_EVENT_RADIO_LINK_ENDED_FOR_SUBMIT_REPORT:
      QSH_LOG(QSH_CLT_WMS,QSH_CAT_EVENT,QSH_MSG_TYPE_HIGH,"AS_ID=%d, SMS_FAIL - end_reason= RADIO_LINK_ENDED_FOR_SUBMIT_REPORT and status:%d",as_id, status );
      break;

    case WMS_QSH_EVENT_FIELD_DEBUG_ANY_FAILURE:
      QSH_LOG(QSH_CLT_WMS,QSH_CAT_EVENT,QSH_MSG_TYPE_HIGH,"AS_ID=%d, SMS_FAIL - end_reason= ANY FAILURE and status:%d",as_id, status );
      break;
	  
    case WMS_QSH_EVENT_3GPP_SIM_INIT_START:
      QSH_LOG(QSH_CLT_WMS,QSH_CAT_EVENT,QSH_MSG_TYPE_HIGH,"AS_ID=%d, 3GPP SIM init started and status:%d",as_id, status );
      break;

    case WMS_QSH_EVENT_3GPP2_SIM_INIT_START:
      QSH_LOG(QSH_CLT_WMS,QSH_CAT_EVENT,QSH_MSG_TYPE_HIGH,"AS_ID=%d, 3GPP2 SIM init started and status:%d",as_id, status );
      break;

    case WMS_QSH_EVENT_3GPP_SIM_INIT_END:
      QSH_LOG(QSH_CLT_WMS,QSH_CAT_EVENT,QSH_MSG_TYPE_HIGH,"AS_ID=%d, 3GPP SIM init ended and status:%d",as_id, status );
      break;

    case WMS_QSH_EVENT_3GPP2_SIM_INIT_END:
      QSH_LOG(QSH_CLT_WMS,QSH_CAT_EVENT,QSH_MSG_TYPE_HIGH,"AS_ID=%d, 3GPP2 SIM init ended and status:%d",as_id, status );
      break;

    case WMS_QSH_EVENT_MAX:
    default:
      break;
      ...
      case GSTK_SMS_PP_DOWNLOAD_IND_RSP:
      {
        ...
        if(msg_mode == WMS_MESSAGE_MODE_GW)
        {
          ...
          else
          {
            ...
            wms_cmd_buf_ptr->hdr.client_id     = (wms_client_id_type)WMS_CLIENT_TYPE_CAT;
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  wms_log_event_notify_packet(client_id,event, client_ptr->msg_notify_type );
  ...
  // Copy any extras (alpha)
  {
    ...
    else if (event == WMS_MSG_EVENT_TRANSPORT_MWI_NOTIFICATION)
    {
      ...
      MSG_HIGH_2("Outbound event: summary = %d, details = %d", 
              ...
              (msg_event_info_ptr->mt_message_info.wac_info.len > 0) &&
              (NULL != msg_event_info_ptr->mt_message_info.wac_info.data) )
    {
      (*ob_evt)->ev_info.mt_message_info.wac_info.len = msg_event_info_ptr->mt_message_info.wac_info.len;
      (*ob_evt)->ev_info.mt_message_info.wac_info.data = (uint8 *)wms_mem_malloc(msg_event_info_ptr->mt_message_info.wac_info.len);

      // Copy WAC information
      if (NULL != (*ob_evt)->ev_info.mt_message_info.wac_info.data)
      {
        (void)memscpy ((*ob_evt)->ev_info.mt_message_info.wac_info.data,
                       (*ob_evt)->ev_info.mt_message_info.wac_info.len,
                       msg_event_info_ptr->mt_message_info.wac_info.data,
                       msg_event_info_ptr->mt_message_info.wac_info.len);
      }
      else
      {
        MSG_ERROR_0("Memory heap is exhausted");
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  */
  for( i=0; i<(uint32)WMS_CLIENT_TYPE_MAX; i++ )
  {
        ...
        client_ptr->msg_event_cb )
    {
      if ( (as_id > SYS_MODEM_AS_ID_NONE) && 
           (as_id <  WMS_MSG_TRANSPORT_ARR_MAX) &&
           ((uint32)WMS_CLIENT_TYPE_QMI == i) && 
           (WMS_MSG_EVENT_TRANSPORT_REG == event) )
      {
         if (cfg_s_ptr->admin_sms_only[as_id])
         {
         ...
         if (cfg_s_ptr->no_srv_ack_only[as_id])
         {
            MSG_HIGH_0("Only MT sms ack can be sent over ims - send deregister event to QMI");
            msg_event_info_ptr->transport_reg_info.is_registered = FALSE;
         }
      }
  ...
  /* Free the memory which was allocated for outbound event */
  if ((event == WMS_MSG_EVENT_RECEIVED_MESSAGE) &&
      (NULL != ob_evt) &&
      (NULL != ob_evt->ev_info.mt_message_info.wac_info.data))
  {
    WMS_MEM_FREEIF(ob_evt->ev_info.mt_message_info.wac_info.data);
  }
...
)
{
  ...
  if (cdma_tl.teleservice == WMS_TELESERVICE_WAP ||
      (wms_cm_CT_features_supported(wms_check_1x_active_sub()) &&  
...
static wms_msg_cdma_message_record_type* wms_msg_cdma_get_free_mt_rec(void)
{
  ...
  for( i=0; i<WMS_MAX_MT_MESSAGES; i++ )
  {
    if( NULL == msg_s_ptr->cdma_mt_rec[i] )
    {
       ...
       msg_s_ptr->cdma_mt_rec[i]->msg_type = WMS_REGULAR_SMS;
...
)
{
  ...
  for( i=0; i<WMS_MAX_MT_MESSAGES; i++ )
  {
    if( rec_ptr == msg_s_ptr->cdma_mt_rec[i] )
    {
      ...
      (void) rex_undef_timer( & rec_ptr->timer );
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  appInfo_ptr = rec_ptr->appInfo_ptr;
  ...
  MSG_HIGH_3( "Sending SMS OTA: len=%d, fmt=%d, as_id = %d",
              OTA_ptr->data_len,
              OTA_ptr->format,
              as_id);
  ...
  else
  {
    ...
    {
      sys_modem_as_id_e_type    ims_as_id     = as_id;  
      ...
      // Does transport allow admin sms only?
               ...
               (as_id > SYS_MODEM_AS_ID_NONE && as_id <  WMS_MSG_TRANSPORT_ARR_MAX && cfg_s_ptr->admin_sms_only[as_id]) ) &&
               !wms_is_cdma_admin_msg(OTA_ptr) &&
               (rec_ptr->msg_type != WMS_E911_SMS) )
      {
        MSG_ERROR_0("Can't send non-admin message while IMS in limited service");
        return WMS_TRANSPORT_NOT_ALLOWED_S;
      }
      else if( !is_ack && 
               ( (WMS_MSG_TRANSPORT_NW_REG_STATUS_NO_SRV_ALLOW_ACK == transport_ptr->nw_reg_status) || 
                 (as_id > SYS_MODEM_AS_ID_NONE && as_id <  WMS_MSG_TRANSPORT_ARR_MAX && cfg_s_ptr->no_srv_ack_only[as_id]) ) &&
               (rec_ptr->msg_type != WMS_E911_SMS) )
      {
        MSG_ERROR_0("Can't send MO SMS while IMS in NO_SRV_ALLOW_ACK service");
        return WMS_TRANSPORT_NOT_ALLOWED_S;
      }     
      else 
      {
         /* E911 SMS can be sent in VzW mode if LTE is in limited or full service */
         if ( (rec_ptr->msg_type == WMS_E911_SMS) &&
              (NULL == wms_get_lte_appinfo_by_as_id(as_id)) )
         {
           MSG_ERROR_0("Can't send E911 SMS while LTE service is not available"); 
           return WMS_TRANSPORT_NOT_ALLOWED_S;
         }
         ...
         (void) transport_ptr->mo_sms_cb( transport_ptr->as_id, 
                                          &ota_msg
                               #ifndef FEATURE_EMERG_SMS_OVER_IMS
                                          ,rec_ptr->msg_type 
                               #endif
                                         );
         return WMS_OK_S;
    ...
    else
    {
      if ( wms_msg_have_active_bearer() )
      {
        ...
        boolean is_retry = (rec_ptr->retry_time_left != (sint31)get_mo_retry_period(as_id));
        ...
        if( WMS_BEARER_CDMA_1X == bearer ) 
        {
          ...
          if(as_id == wms_check_1x_active_sub())
          {
          ...
          {
             MSG_ERROR_0("Can't send SMS over 1X on inactive SUB");
             return WMS_TRANSPORT_NOT_READY_S;
          }
        ...
        if (WMS_OK_S == st)
        {
          ...
          MSG_HIGH_2( "SMS sent over bearer = %d, ch_pref = %d",
                      bearer,
                      rec_ptr->ch_pref );
          wms_cm_sms_send_cdma_msg2( address_ptr, OTA_ptr, bearer, rec_ptr->ch_pref,as_id );
        }
      }
      else
      {
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */  
  ...
  MSG_HIGH_2("wms_msg_cdma_send_MO(),as_id:%d, sms_type %d ",cmd_ptr->hdr.as_id, sms_type);
  ...
  else
  {
    ...
    if ( NULL != rec_ptr->ota )
    {
      ...
      rec_ptr->as_id        = cmd_ptr->hdr.as_id;
      rec_ptr->msg_type     = sms_type;
      ...
      */
      if ( 0 == get_mo_retry_period(cmd_ptr->hdr.as_id) )
      {
        rec_ptr->retry_time_left = WMS_MAX_MO_STATUS_WAIT_TIME;
      }
      else
      {
        rec_ptr->retry_time_left = get_mo_retry_period(cmd_ptr->hdr.as_id);
      }
      ...
      if( st == WMS_OK_S )
      {
        ...
        {        
          ...
          appInfo_ptr = wms_get_appinfo_by_asid_and_message_mode(cmd_ptr->hdr.as_id, WMS_MESSAGE_MODE_CDMA);  
          ...
          if (NULL != appInfo_ptr)
          {
            if (0 == appInfo_ptr->mo_sms_count)
            {
              ...
              mm_status.as_id = cmd_ptr->hdr.as_id;          
              ...
              mm_status.overall_sms_status = TRUE;
              (void)wms_cm_gw_send_sms_status_to_mm(mm_status);
            }
...
===========================================================================*/
boolean wms_msg_send_3gpp2_sms_over_ims(
   const wms_OTA_message_type* OTA_ptr,
   sys_modem_as_id_e_type  as_id, 
   wms_message_e_type msg_type)
{
   ...
   if (transport_ptr != NULL)
   {
      ...
      if((WMS_MSG_TRANSPORT_NW_REG_STATUS_FULL_SRV != transport_ptr->nw_reg_status) &&
         (msg_type != WMS_E911_SMS))
      {
         send_to_ims = FALSE;
      }
      ...
      if ( (WMS_MSG_TRANSPORT_NW_REG_STATUS_NO_SRV_ALLOW_ACK == transport_ptr->nw_reg_status)  &&
           (msg_type != WMS_E911_SMS) &&
            wms_msg_have_active_bearer() )
      {
         send_to_ims = FALSE;
      }
   ...
   MSG_HIGH_2("wms_msg_send_3gpp2_sms_over_ims returns %d msg_type %d", send_to_ims,msg_type);
   return send_to_ims;
}
...
{
   wms_cmd_err_e_type cmd_err = WMS_CMD_ERR_NONE;
   wms_message_e_type  message_type = WMS_REGULAR_SMS;
...
#ifndef FEATURE_EMERG_SMS_OVER_IMS
   /* Send SMS type as emergency or normal only for VzW mode. For other operators,
      all SMS will be treated as normal SMS. */
   if ( (NULL != cfg_s_ptr) && cfg_s_ptr->allow_e911_sms_over_ims[as_id])
   {
      wms_address_s_type  dest_address = tl_ptr->address;
      
      if (TRUE == wms_msg_mo_sms_emergency_num_check_func(cmd_ptr->hdr.appInfo_ptr, &dest_address))
      {
         message_type = WMS_E911_SMS;
      }
   }
#endif
   ...
   {
      if (wms_msg_send_3gpp2_sms_over_ims(ota_ptr,as_id, message_type))
      {
   ...
   MSG_HIGH_4( "Chosen transport = %d, client_id = %d, ch_pref = %d, asid = %d",
                cmd_ptr->cmd.msg_send.transport_type,
   ...
   // Reject 1X MO SMS if sub is not 1X capable
   if ( (WMS_MSG_TRANSPORT_TYPE_CS == cmd_ptr->cmd.msg_send.transport_type) &&
        (as_id != wms_check_1x_active_sub()) )
   {
     MSG_ERROR_0("Can't send SMS over 1X on a sub without 1X capability");
     return WMS_CMD_ERR_AS_ID;
   }
       ...
       if ( WMS_OK_S == st )
       {
         ...
         else
         {
           ...
           if ( NULL != rec_ptr->ota )
           {
             ...
             */
             if ( 0 == get_mo_retry_period(as_id) )
             {
               rec_ptr->retry_time_left = WMS_MAX_MO_STATUS_WAIT_TIME;
             }
             else
             {
               rec_ptr->retry_time_left = get_mo_retry_period(as_id);
             }
             ...
             MSG_HIGH_4("Msg Saved in REC with  SEQ Num %d , WMS initiating DC status %d, retry time left [%d|%d] ", 
                         rec_ptr->seq_num, 
                         st,
                         get_mo_retry_period(as_id),
...
)
{
  ...
  {
#ifdef FEATURE_SMS_PWS_OVER_LTE
    if ( (TRUE == cfg_s_ptr->rrc_disable_ta_for_embms) &&
         (WMS_MSG_TRANSPORT_TYPE_CS == msg_s_ptr->gw_mo_rec[rec_index]->transport_type) )
    {
      msg_s_ptr->gw_mo_rec[rec_index]->state = WMS_MSG_STATE_IDLE; 
      wms_msg_lte_rrc_msg_state_ind( msg_s_ptr->gw_mo_rec[rec_index]->as_id, 
                                 msg_s_ptr->gw_mo_rec[rec_index]->transport_type,
                                 WMS_MSG_SMS_END );
    }
#endif
...
)
{
  ...
  if (!is_retry)
  {
    mo_rec_ptr->msg_type = message_type;
  ...
  if (cmd_err == WMS_CMD_ERR_NONE)
  {
    ...
    else
    {
      ...
      /* Send SMS type as emergency or normal only for VzW mode. For other operators,
         all SMS will be treated as normal SMS. */
      if ( !is_retry && 
           ( (NULL != cfg_s_ptr) && cfg_s_ptr->allow_e911_sms_over_ims[as_id]) &&
           (TRUE == wms_msg_mo_sms_emergency_num_check_func(appInfo_ptr, 
      ...
      MSG_HIGH_2("MO record transport type: %d, sms type %d",mo_rec_ptr->transport_type, mo_rec_ptr->msg_type);
            ...
            else
            {
              ...
              if ( GSTK_ACCESS_TECH_UTRAN == mo_sms_control_buf.rat ) 
              {
                ...
                /* Spec TS 123 003, section 4.1 specifies that Location Area Indentity consists of:
                       ...
                       LA : 2 octets
                */
                mo_sms_control_buf.loc_info.gwl_info.mcc_and_mnc[0] =  cfg_s_ptr->umts_loc_info[as_id].mcc_and_mnc[0];
                mo_sms_control_buf.loc_info.gwl_info.mcc_and_mnc[1] =  cfg_s_ptr->umts_loc_info[as_id].mcc_and_mnc[1];
                mo_sms_control_buf.loc_info.gwl_info.mcc_and_mnc[2] =  cfg_s_ptr->umts_loc_info[as_id].mcc_and_mnc[2];

                mo_sms_control_buf.loc_info.gwl_info.loc_area_code[0] = cfg_s_ptr->umts_loc_info[as_id].loc_area_code[0];
                mo_sms_control_buf.loc_info.gwl_info.loc_area_code[1] = cfg_s_ptr->umts_loc_info[as_id].loc_area_code[1];
              }
              ...
              if ( WMS_CMD_ERR_NONE == cmd_err ) 
              {
                ...
                MSG_HIGH_5( "mcc_and_mnc[] = %02x %02x %02x , loc_area_code[] = %02x %02x",
  ...
  */
  if(cmd_err == WMS_CMD_ERR_NONE)
  {
...
#endif /* FEATURE_GWSMS_MO_CONTROL */
    {
      if (WMS_CMD_ERR_NONE == cmd_err)
      {
        /* In case SMS retry, decrement the SMS count and send SMS status indication to MM  */
        if (is_retry)
        {
           if(appInfo_ptr != NULL)
           {
             ...
             if (0 == appInfo_ptr->mo_sms_count)
             {
               wms_msg_set_sms_status_inactive(msg_event_info.submit_report_info.as_id);
             }
           }
        }
        ...
        */
            ...
            cdma_tl.teleservice           != WMS_TELESERVICE_WAP )
        {
          ...
          else
          {
            /* use the next message number
            */
            cdma_tl.cl_bd.message_id.id_number = wms_cfg_get_ruim_next_message_number(as_id);
          }
        }
          ...
          */
              ...
              cdma_tl.teleservice           != WMS_TELESERVICE_WAP )
          {
            ...
            else
            {
              /* use the next message number
              */
              cdma_tl.cl_bd.message_id.id_number = wms_cfg_get_ruim_next_message_number(cmd_ptr->hdr.as_id);
            }
          }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  */
  ...
  else if( wms_ts_validate_bd_subparms( & cdma_tl.cl_bd, cdma_tl.teleservice ) != WMS_OK_S ) //1x need to check how to handle this
  {
    cmd_err = WMS_CMD_ERR_MSG_DECODE;
  }
  else
  {
    ...
    if (WMS_BD_TYPE_SUBMIT == cdma_tl.cl_bd.message_id.type &&
        TRUE == appInfo_ptr->in_roaming &&
        wms_cm_CT_features_supported( cmd_ptr->hdr.as_id)) 
    {
      MSG_HIGH_0("Removing Priority and Reply Option");
      cdma_tl.cl_bd.mask &= ~ WMS_MASK_BD_PRIORITY;
      cdma_tl.cl_bd.mask &= ~ WMS_MASK_BD_REPLY_OPTION;
    ...
    */
    ...
    {
      if (wms_cm_CT_features_supported(cmd_ptr->hdr.as_id)&&(cmd_ptr->hdr.client_id ==  WMS_CLIENT_TYPE_CAT)) 
      {
...
(
  uint32   *index_ptr, sys_modem_as_id_e_type as_id
)
{
  uint32  i;
  ...
  for( i=0; i<WMS_MAX_MO_MESSAGES; i++ )
  {
    if( NULL == msg_s_ptr->gw_mo_rec[i] )
    {
      ...
      *index_ptr = i;
      ...
      msg_s_ptr->gw_mo_rec[i]->retry_count_embms  = 0;
      msg_s_ptr->gw_mo_rec[i]->state              = WMS_MSG_STATE_IDLE;  
      msg_s_ptr->gw_mo_rec[i]->as_id              = as_id;
      // If retry logic is disabled, set the retry_time_left to the default value 
      // to prevent WMS from waiting forever if report indication is not received.
      if (0 == get_mo_retry_period(msg_s_ptr->gw_mo_rec[i]->as_id))
      { 
        MSG_HIGH_0("Retry logic is not enabled. Set MO status wait time to default value!");
        msg_s_ptr->gw_mo_rec[i]->retry_time_left = WMS_MAX_MO_STATUS_WAIT_TIME;
      }
      else 
      {
        msg_s_ptr->gw_mo_rec[i]->retry_time_left = (sint31)get_mo_retry_period(msg_s_ptr->gw_mo_rec[i]->as_id);
      }

      MSG_HIGH_3("index = %d, retry_time_left = %d,  retry_wait_time = %d", 
                i,
...
)
{
  ...
  MSG_HIGH_4("Retry Interval[0]: %d, Retry Period[0] = %d , Retry Interval[1]: %d, Retry Period[1] = %d", 
                                                msg_s_ptr->mo_retry_interval[SYS_MODEM_AS_ID_1], msg_s_ptr->mo_retry_period[SYS_MODEM_AS_ID_1],
                                                msg_s_ptr->mo_retry_interval[SYS_MODEM_AS_ID_2], msg_s_ptr->mo_retry_period[SYS_MODEM_AS_ID_2]);
  ...
  if (WMS_MESSAGE_MODE_GW == cmd_ptr->cmd.msg_send.message.msg_hdr.message_mode)
  {
    if (WMS_MSG_TRANSPORT_TYPE_INTERNAL == cmd_ptr->cmd.msg_send.transport_type)
    {
      ...
      if (NULL != transport_ptr && transport_ptr->is_mn_registered &&
           (transport_ptr->nw_reg_status == WMS_MSG_TRANSPORT_NW_REG_STATUS_FULL_SRV))
      {
        cmd_ptr->cmd.msg_send.transport_type = WMS_MSG_TRANSPORT_TYPE_SMS_OVER_IMS;
      }
      else
...
#ifdef FEATURE_GWSMS
  if( (WMS_MESSAGE_MODE_GW == cmd_ptr->cmd.msg_send.message.msg_hdr.message_mode) &&
       (FALSE == wms_msg_get_free_mo_record( & i, cmd_ptr->hdr.as_id  ))  )
  {
  ...
  { 
         ...
         (WMS_MESSAGE_MODE_GW == cmd_ptr->cmd.msg_send.message.msg_hdr.message_mode))
     {
        if(msg_s_ptr->gw_mo_rec[i] != NULL)
        {
          ...
          if ((transport_ptr != NULL) && (transport_ptr->transport_id <  WMS_MSG_TRANSPORT_ARR_MAX))
          {
             msg_s_ptr->gw_mo_rec[i]->retry_time_left = ((msg_s_ptr->timerf_value[transport_ptr->transport_id])/1000)+ 1;
          }
          else
          {
            msg_s_ptr->gw_mo_rec[i]->retry_time_left = 1;
          }
          MSG_HIGH_2("Retry logic is not enabled. Set MO status wait time to timerf value= %d as the SMS is over IMS, transport_ptr 0x%x!",msg_s_ptr->gw_mo_rec[i]->retry_time_left, transport_ptr );
        }
     }
  }
#endif /* FEATURE_GWSMS */
...
)
{
  ...
  wms_msg_set_sms_status_inactive(wms_check_1x_active_sub());
      ...
      */
      ...
      if ( WMS_CMD_ERR_NONE == cmd_err )
      {
         ...
         cmd_err = wms_cm_gw_send_rl_report(msg_s_ptr->gw_mt_rec[rec_index]->as_id,
                                            ...
                                            raw_ts.data,
                                            (uint8)raw_ts.len,
                                            msg_s_ptr->gw_mt_rec[rec_index]->msg_type);
  ...
  switch(report_info_ptr->report_status)
  {
        ...
        WMS_RP_CAUSE_TEMP_REJ == report_info_ptr->cause_info.cause_value)
    {
      ...
      if ((NULL == wms_msg_choose_registered_transport( report_info_ptr->as_id )) ||
          ((asid_is_valid (report_info_ptr->as_id)) && (retry_count < cfg_s_ptr->sms_retry_limit[report_info_ptr->as_id])))
      {
        do_retry = TRUE;
      }
    ...
    case WMS_RPT_PREF_CHN_NOT_AVAIL_ERROR:
      ...
      if(transport_type == WMS_MSG_TRANSPORT_TYPE_SMS_OVER_IMS) 
      {
        if ((asid_is_valid (report_info_ptr->as_id)) && (retry_count < cfg_s_ptr->sms_retry_limit[report_info_ptr->as_id]))
        {
          do_retry = TRUE;
          MSG_HIGH_1("Retry allowed with report status over IMS: %d", report_info_ptr->report_status);
        }
      }
      else
      {
...
#ifdef FEATURE_SMS_PWS_OVER_LTE
      if ( (TRUE == cfg_s_ptr->rrc_disable_ta_for_embms) &&
           (WMS_MESSAGE_MODE_GW == report_info_ptr->message_mode) ) 
      {
        if ( retry_count < WMS_MAX_RETRY_COUNT_EMBMS ) 
        {
          do_retry = TRUE;
        }
        else 
        ...
        MSG_HIGH_4("do_retry %d with %d report status for eMBMS. retry count/limit %d/%d", 
                    do_retry, 
                    report_info_ptr->report_status, 
                    retry_count, 
                    WMS_MAX_RETRY_COUNT_EMBMS);
      }
      else
#endif
      {  
                ...
                */
                ...
                else
                {
                  ...
                  else
                  {
                    ...
                    msg_event_info.mt_message_info.wac_info.len = 0;
                  }
                }
              }
              else
...
)
{
  ...
  switch( mem_store )
  {
    ...
    case WMS_MEMORY_STORE_NV_CDMA:
      ...
      if ( wms_nv_read_cdma_params( 0, ruim_data,cmd_ptr->hdr.as_id) == FALSE )
      {
        cmd_err = WMS_CMD_ERR_MSG_NV_READ;
      }
...
)
{
  ...
  case WMS_MEMORY_STORE_NV_CDMA:
    //TBD:  support multiple templates in the future
    {
      ...
      else if (wms_nv_write_cdma_params( msg_idx, ruim_data, cmd_ptr->hdr.as_id ) == FALSE )
      {
        cmd_err = WMS_CMD_ERR_MSG_NV_WRITE;
      }
    }
...
)
{
  ...
  if ((NULL != appInfo_ptr) && 
      ((MMGSDI_APP_SIM  == appInfo_ptr->mmgsdi_appInfo.app_type) ||
  ...
  (void)wms_msg_event_notify( WMS_MSG_EVENT_SMSC_ADDR_CHANGE, msg_event_info_ptr );
...
)
{
  ...
  if (WMS_RPT_OK == wms_report_status)
  {
    ...
    if (GSTK_ALLOWED_BUT_MOD == mo_control_rsp_ptr->mo_sms_ctrl_result)
    {
       (void)memscpy( &(msg_s_ptr->gw_mo_rec[rec_index]->sent_gw_msg->sc_address),
                      sizeof(wms_address_s_type),
                      &(gw_msg_ptr->sc_address),
                      sizeof(wms_address_s_type) );
...
#ifdef FEATURE_QSH_EVENT_NOTIFY_TO_QSH
    qsh_evt = (wms_qsh_event_e)wms_map_event_to_qsh_event(msg_event_info.submit_report_info.report_status);
    if(wms_is_event_debug_any_failure_enabled(qsh_evt,msg_event_info.submit_report_info.as_id) == TRUE)
    {
      wms_qsh_client_event_log(WMS_QSH_EVENT_FIELD_DEBUG_ANY_FAILURE, wms_report_status,msg_event_info.submit_report_info.as_id );
    }
    else if(wms_is_qsh_event_valid_and_enabled(qsh_evt,msg_event_info.submit_report_info.as_id) == TRUE)
    {
      wms_qsh_client_event_log(qsh_evt, wms_report_status,msg_event_info.submit_report_info.as_id );
    }
#endif
...
)
{
  ...
  */
  ...
  {
    MSG_HIGH_1("SMMA not sent: sms_rec_count = %d", appInfo_ptr->sms_rec_count);
    return;
  }
  ...
  if (mem_available)
  {
    if (0 == (mem_full_flag & (uint8)WMS_SIM_MEM_FULL_MASK))
    {
      ...
      if (wms_msg_map_mo_record(appInfo_ptr->asid, WMS_MSG_SMMA_REQUEST_ID, & rec_index) == FALSE )
      {
         ...
         if (wms_msg_get_free_mo_record( & rec_index, appInfo_ptr->asid ) == TRUE )
         {
            msg_s_ptr->gw_mo_rec[rec_index]->smi             = WMS_MSG_SMMA_REQUEST_ID;
            msg_s_ptr->gw_mo_rec[rec_index]->appInfo_ptr     = appInfo_ptr;
            msg_s_ptr->gw_mo_rec[rec_index]->msg_type        = WMS_REGULAR_SMS;
      ...
      if ((TRUE == retry) || (TRUE == new_req))
      {
        ...
        if (transport_ptr != NULL && transport_ptr->is_mn_registered)
        {
         ...
         if (WMS_CMD_ERR_NONE == cmd_err)
         {
            ...
            if (get_mo_retry_period(appInfo_ptr->asid) > 0)
            {
               wms_set_retry_timer();
            }
         }
...
)
{
   ...
   boolean send_retry_st = FALSE;
   boolean retry_possible = FALSE;
...
#ifndef FEATURE_EMERG_SMS_OVER_IMS
   if (WMS_MSG_TRANSPORT_TYPE_SMS_OVER_IMS == msg_s_ptr->gw_mo_rec[rec_idx]->transport_type) 
   {
      transport_ptr = wms_msg_choose_registered_transport( asid );     
          
      if ( (transport_ptr != NULL) && (transport_ptr->sms_retry_cb != NULL) )
      {
         send_retry_st = TRUE;
      }
   }
   ...
   switch (error_status) {
      ...
      ** else if Memory is still not available keep the flag else clear the flag */
         ...
         {
            if ((get_mo_retry_period(asid) > 0) &&
                (msg_s_ptr->gw_mo_rec[rec_idx]->retry_time_left > (sint31)get_mo_retry_interval(asid)))
            {
               /* Set the retry_wait_time */
               msg_s_ptr->gw_mo_rec[rec_idx]->retry_wait_time = (sint31)get_mo_retry_interval(asid);
       ...
       * clear the flag.
       * Reset send_retry_st so that IMS is not called for success cases
      */
      case WMS_RPT_OK:
         send_retry_st = FALSE;
         break;
   ...
   if (send_retry_st)
   {
     transport_ptr->sms_retry_cb(asid, retry_possible, msg_s_ptr->gw_mo_rec[rec_idx]->msg_type);
   }
#endif
        ...
        else
        {
          if( rec_ptr->send_status_to_clients == TRUE )
          {
            ...
            if(msg_event_info.submit_report_info.report_status != WMS_RPT_OK)
            {
              ...
              if(wms_is_event_debug_any_failure_enabled(qsh_evt,msg_event_info.submit_report_info.as_id) == TRUE)
              {
                wms_qsh_client_event_log(WMS_QSH_EVENT_FIELD_DEBUG_ANY_FAILURE, msg_event_info.submit_report_info.report_status,msg_event_info.submit_report_info.as_id );
              }
              else if(wms_is_qsh_event_valid_and_enabled(qsh_evt,msg_event_info.submit_report_info.as_id) == TRUE)
              {
                wms_qsh_client_event_log(qsh_evt, msg_event_info.submit_report_info.report_status,msg_event_info.submit_report_info.as_id );
              }
            }
#endif
           (void)wms_msg_event_notify(WMS_MSG_EVENT_SUBMIT_REPORT,&msg_event_info);
...
)
{
  ...
  while ( NULL != rec_ptr )
  {
    MSG_HIGH_1("In wms_msg_stop_dc_resend(), retry period = %d", get_mo_retry_period(rec_ptr->as_id));
    if ( (WMS_MSG_STATE_QUEUED == rec_ptr->state) && 
         (WMS_CLIENT_CH_PREF_TCH_ONLY == rec_ptr->ch_pref) ) 
    {
       if (reest_traffic_ch)
       {
         ...
         if (get_mo_retry_period(rec_ptr->as_id) > 0 )
         {
           rec_ptr->retry_wait_time = get_mo_retry_interval(rec_ptr->as_id);
         }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if (WMS_MSG_TRANSPORT_TYPE_SMS_OVER_IMS == rec_ptr->transport_type) 
  {
    transport_ptr = wms_msg_choose_registered_transport( cmd_ptr->hdr.as_id );     
  ...
  if( WMS_SIP_PERM_ERROR_S == cmd_ptr->cmd.mc_mo_status.status )
  {
     /* Make sure we won't send SMS over SIP again due to a permanent error.
       Inform IMS client that there wont be any further retries.
     */
     ...
     if (send_retry_st)
     {
       transport_ptr->sms_retry_cb(cmd_ptr->hdr.as_id, FALSE, rec_ptr->msg_type);
     }
#endif
...
/* --------------------------------------------------------- */
    ...
    if( rec_ptr->state == WMS_MSG_STATE_LAYER2_IN_PROGRESS )
    {
      ...
      if( cmd_ptr->cmd.mc_mo_status.status == WMS_OK_S )
      {
...
#ifdef FEATURE_QSH_EVENT_NOTIFY_TO_QSH
           if(msg_event_info.ack_report_info.error_status != WMS_RPT_OK )
           {
             qsh_evt = (wms_qsh_event_e)wms_map_event_to_qsh_event((wms_report_status_e_type)msg_event_info.ack_report_info.error_status);
             if(wms_is_event_debug_any_failure_enabled(qsh_evt,msg_event_info.ack_report_info.as_id) == TRUE)
             {
               wms_qsh_client_event_log(WMS_QSH_EVENT_FIELD_DEBUG_ANY_FAILURE, (wms_report_status_e_type) msg_event_info.ack_report_info.error_status,msg_event_info.ack_report_info.as_id );
             }    
			 else if(wms_is_qsh_event_valid_and_enabled(qsh_evt,msg_event_info.ack_report_info.as_id) == TRUE)
             {
               wms_qsh_client_event_log(qsh_evt,(wms_report_status_e_type) msg_event_info.ack_report_info.error_status,msg_event_info.ack_report_info.as_id );
             }
           }
#endif
        ...
        wms_msg_set_sms_status_inactive(cmd_ptr->hdr.as_id);
        wms_msg_cdma_free_mt_msg_rec( rec_ptr );
      }
      else
      {
        ...
        if (send_retry_st)
        {
          transport_ptr->sms_retry_cb(cmd_ptr->hdr.as_id, TRUE, rec_ptr->msg_type);
        }
#endif
      }
    }
...
#ifndef FEATURE_EMERG_SMS_OVER_IMS
      if ( (cmd_ptr->cmd.mc_mo_status.status != WMS_OK_S ) && send_retry_st )
      {
          transport_ptr->sms_retry_cb(cmd_ptr->hdr.as_id, FALSE, rec_ptr->msg_type);
      }
#endif
      ...
      else
      {
        ...
        */
            ...
            (cmd_ptr->cmd.mc_mo_status.status != WMS_ACCESS_TOO_LARGE_S ) )
        {
          ...
          if((get_mo_retry_period(cmd_ptr->hdr.as_id) > 0)                                                        &&
             (wms_msg_is_mo_retry_allowed(rec_ptr->retry_count, &msg_event_info.submit_report_info,rec_ptr->transport_type)) &&
             (rec_ptr->retry_time_left > rec_ptr->retry_wait_time))
          {
            ...
            {
              rec_ptr->retry_wait_time = (sint31)get_mo_retry_interval(cmd_ptr->hdr.as_id);
              rec_ptr->retry_count++;
            }
...
)
{
  ...
  */
  ...
  msg_event_info.mt_message_info.rat            = WMS_SYS_RAT_NONE;
  msg_event_info.mt_message_info.msg_type       = WMS_REGULAR_SMS;
  msg_event_info.mt_message_info.wac_info.len   = 0;
  msg_event_info.mt_message_info.wac_info.data  = NULL;
...
wms_status_e_type wms_msg_cdma_deliver(
  ...
  wms_msg_cdma_message_record_type   *rec_ptr,
  wms_msg_transport_type_e_type       transport_type,
  wms_message_e_type                  message_type
)
{
  wms_status_e_type           st                     = WMS_OK_S;
  wms_cmd_err_e_type          cmd_err                = WMS_CMD_ERR_NONE;
  ...
  msg_event_info.mt_message_info.rat                 = WMS_SYS_RAT_NONE;
  msg_event_info.mt_message_info.msg_type            = message_type;
  ...
  */
  ...
  // Only E911 SMS in VzW mode and admin MT SMS are allowed when IMS is in limited service.
  // Drop other non-admin MT SMS if IMS is in limited service.
  if(transport_type == WMS_MSG_TRANSPORT_TYPE_SMS_OVER_IMS)
  {
    if ( (transport_ptr != NULL) &&
  ...
  */
...
#if !defined(FEATURE_MODEM_ATC) || defined (FEATURE_MODEM_AT_CP)
    if (cfg_s_ptr->active_bearers[WMS_BEARER_CDMA_1X] ||
        cfg_s_ptr->active_bearers[WMS_BEARER_CDMA_EVDO])
    {
       msg_event_info.mt_message_info.rat = WMS_SYS_RAT_CDMA_RADIO_ACCESS;
    }
    else
    {
      msg_event_info.mt_message_info.rat  = WMS_SYS_RAT_LTE_RADIO_ACCESS;
    }
#endif
  }
#endif /* FEATURE_CDSMS_BROADCAST */
  ...
  if (WMS_TELESERVICE_CATPT == cdma_tl.teleservice)
  {
    ...
    if ( appInfo_ptr->pp_data_download_enabled &&
         (nv_rtre_control_ext((uint16)appInfo_ptr->asid) == NV_RTRE_CONTROL_USE_RUIM) )
    {
      st = wms_msg_utk_pp_download_proc(cl_ptr, &cdma_tl,appInfo_ptr);
...
)
{
  ...
  */
  ...
  else
  {
    ...
    wms_message_e_type message_type = (WMS_E911_SMS == mt_msg_cmd_ptr->msg_type)? WMS_E911_SMS : WMS_REGULAR_SMS;
         ...
         (TRUE == mt_msg_cmd_ptr->ack_pending) )
    {
      ...
      else  /* found a free record */
      {
        ...
        */
        ...
        rec_ptr->transport_type = transport_type;
        rec_ptr->msg_type = message_type;
        rec_ptr->appInfo_ptr = appInfo_ptr;

        if( no_delivery == FALSE )
        {
          st = wms_msg_cdma_deliver(appInfo_ptr, tid, &cdma_cl, routing_ptr, rec_ptr, transport_type, rec_ptr->msg_type);
        }
        ...
        */
        ...
        MSG_HIGH_2( "MT (tid=%d, seq=%d)ack timer set ",
                    tid,
                    seq_num );
...
/* ---------------------------------------------- */
      ...
      */
      ...
      wms_msg_set_sms_status_inactive(appInfo_ptr->asid);
    } /* if */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  */
  (void) wms_msg_cdma_deliver( appInfo_ptr,
                               ...
                               NULL,
                               WMS_MSG_TRANSPORT_TYPE_CS,
                               WMS_REGULAR_SMS ); /* For now, treat it always as regular SMS*/
...
)
{
  ...
  memset(&mm_status, 0x00, sizeof(wms_mm_sms_call_info_T));
  mm_status.as_id = appInfo_ptr->asid;
  mm_status.overall_sms_status = TRUE;
  (void) wms_cm_gw_send_sms_status_to_mm(mm_status);
...
)
{
  ...
  /* use the next message number
  */
  tl_ptr->cl_bd.message_id.id_number = wms_cfg_get_ruim_next_message_number(cmd_ptr->hdr.as_id);
...
)
{
  ...
  st = wms_msg_ms_transport_init( transport_type, transport_cap, SYS_MODEM_AS_ID_1,  transport_id_ptr );
  return st;
}

/*=========================================================================
FUNCTION
  wms_msg_ms_transport_init
...
=========================================================================*/
wms_status_e_type  wms_msg_ms_transport_init
(
  wms_msg_transport_type_e_type     transport_type,
  wms_msg_transport_cap_type        transport_cap,
  sys_modem_as_id_e_type            as_id,
...
)
{
  ...
  if ((transport_type != WMS_MSG_TRANSPORT_TYPE_SMS_OVER_IMS) || 0 == transport_cap)
  {
    return WMS_UNSUPPORTED_S;
  }
  ...
  MSG_HIGH_3( "wms_msg_transport_init transport_type: %d, transport_cap: %d, as_id %d", 
              transport_type, transport_cap, as_id );

  if (as_id == SYS_MODEM_AS_ID_NONE || as_id >=  WMS_MSG_TRANSPORT_ARR_MAX)
  {
     return WMS_UNSUPPORTED_S;
  }

  if (!msg_transports[as_id].is_registered)
  {
    msg_transports[as_id].is_registered  = TRUE;
    msg_transports[as_id].cap            = transport_cap;
    msg_transports[as_id].mo_sms_cb      = NULL;
    msg_transports[as_id].transport_id   = (wms_msg_transport_id_type)as_id;
    msg_transports[as_id].transport_type = transport_type;
#ifndef FEATURE_EMERG_SMS_OVER_IMS
    msg_transports[as_id].sms_retry_cb   = NULL;
#endif
  }

  *transport_id_ptr = (wms_msg_transport_id_type)as_id;
  msg_transports[as_id].as_id = as_id;

  MSG_HIGH_3( "wms_msg_transport_init returns transport id %d for transport_type: %d on as id %d", 
              *transport_id_ptr, transport_type, msg_transports[as_id].as_id);
...
#ifndef FEATURE_EMERG_SMS_OVER_IMS
  if (transport_ptr->sms_retry_cb != NULL)
  {
    st = wms_msg_transport_ms_reg_sms_retry_status_cb(transport_id, NULL, transport_ptr->as_id);
  }
#endif
...
DESCRIPTION
  Callback function called by NAS to send 3GPP MO SMS messages
...
wms_status_e_type wms_mn_mo_sms_callback(void *msg_ptr)  
{
  ...
  cmd_ptr->hdr.cmd_id      =  WMS_CMD_MSG_SMS_CALLBACK;
  cmd_ptr->hdr.client_id   = (uint8)WMS_CLIENT_TYPE_INTERNAL;
  cmd_ptr->hdr.cmd_cb      = NULL;
  cmd_ptr->hdr.user_data   = (void*)NULL;
  cmd_ptr->hdr.appInfo_ptr = NULL;

  memscpy(&cmd_ptr->cmd.wms_callback.mn_msg_ptr,sizeof(SM_RL_RPDU_T),(SM_RL_RPDU_T *)msg_ptr,sizeof(SM_RL_RPDU_T));
...
/*=========================================================================
FUNCTION
  wms_mn_mo_sms_callback_proc

DESCRIPTION
  Process the RP message information from NAS and notify teh same to IMS in 
  mo_sms_cb.
...
void wms_mn_mo_sms_callback_proc(wms_cmd_type      *cmd_ptr)
{
  SM_RL_RPDU_T             *mn_msg_ptr ;
  wms_msg_transport_s_type *transport_ptr           = NULL;
  boolean                   send_to_transport_layer = FALSE;
  sys_modem_as_id_e_type    as_id                   = SYS_MODEM_AS_ID_1;
  wms_message_e_type        msg_type                = WMS_REGULAR_SMS;

  mn_msg_ptr = (SM_RL_RPDU_T *)&cmd_ptr->cmd.wms_callback.mn_msg_ptr;
  ...
  if (transport_ptr == NULL || transport_ptr->mo_sms_cb == NULL || mn_msg_ptr == NULL || 
     (NULL !=mn_msg_ptr && 0 == mn_msg_ptr->rpdu_length))
  {
    MSG_HIGH_2("transport_ptr(0x%x) or mn_msg_ptr(0x%x) is NULL ",transport_ptr, mn_msg_ptr);
    return;
  }

  MSG_HIGH_2("wms_mn_mo_sms_callback is called to send RP message with type %d for sms type %d", 
             (mn_msg_ptr->rpdu[0] & 0x07),
             mn_msg_ptr->sms_msg_type);
  ...
  case RP_DATA_MS:
    ...
    {
      msg_type = (mn_msg_ptr->sms_msg_type == NAS_E911_SMS)? WMS_E911_SMS : WMS_REGULAR_SMS;
...
)
{
  ...
  if (NULL != msg_event_ptr)
  {
    if (NULL != transport_ptr)
    {
      ...
      MSG_HIGH_3( "Current transport cap = %d; transport capability notified to clients = %d, as id = %d",
                  transport_ptr->cap,
...
)
{
  ...
  if (NULL != transport_ptr)
  {
    ...
    MSG_HIGH_1("wms_msg_transport_reg_proc(): as id %d", transport_ptr->as_id);
    ...
    if (transport_ptr->as_id > SYS_MODEM_AS_ID_NONE && transport_ptr->as_id <  WMS_MSG_TRANSPORT_ARR_MAX)
    {
       if (WMS_MSG_TRANSPORT_CAP_3GPP_IMS_LIMITED == transport_ptr->cap)
       {
         MSG_HIGH_0("IMS only allows MO admin SMS in 3GPP.");
         cfg_s_ptr->admin_sms_only[transport_ptr->as_id] = TRUE;
       }
       else if (WMS_MSG_TRANSPORT_CAP_3GPP2_IMS_LIMITED == transport_ptr->cap)
       {
       ...
       {
         cfg_s_ptr->admin_sms_only[transport_ptr->as_id] = FALSE;
       }
    }
    ...
    if (NULL == transport_ptr->mo_sms_cb)
    {
      ...
      if(NULL != rec_ptr)
      {
        if (WMS_MSG_TRANSPORT_TYPE_SMS_OVER_IMS == rec_ptr->transport_type)
        {
          ...
          wms_msg_mc_mo_status(WMS_ERROR_TEMP,WMS_SIP_TEMP_ERROR_S,WMS_BEARER_IMS_3GPP2,transport_ptr->as_id);
        }
      }
...
/*=========================================================================
FUNCTION
  wms_msg_transport_ms_reg_sms_retry_status_cb

DESCRIPTION
  Allows the SMS Transport Layer to register with WMS with a specified function 
  callback to be used to notify SMS transport layer whether SMS will be reattempted
  on the transport. If a NULL pointer is passed, the callback is deregistered.
...
=========================================================================*/
wms_status_e_type  wms_msg_transport_ms_reg_sms_retry_status_cb
(
  wms_msg_transport_id_type                          transport_id,
  wms_msg_transport_update_sms_retry_status_cb_type  retry_st_cb,
  sys_modem_as_id_e_type                             as_id
)
{
  wms_msg_transport_s_type   *transport_ptr = wms_msg_transport_ptr(transport_id);
  ...
  if (retry_st_cb != NULL)
  {
    MSG_HIGH_2("TRANSPORT %d registers with WMS for retry status on as_id %d.", transport_id, as_id);
  }
  else
  {
    MSG_HIGH_2("TRANSPORT %d deregisters with WMS for retry status on as_id %d.", transport_id, as_id);
  }

  if (NULL != transport_ptr)
  {
    transport_ptr->sms_retry_cb = retry_st_cb;
  }
  else
  {
    return WMS_UNSUPPORTED_S;
...
)
{
  ...
  uint8                            i;
  wms_cfg_s_type                  *temp_cfg_s_ptr = wms_cfg_s_ptr();
  ...
  if (( (as_id > SYS_MODEM_AS_ID_NONE) && (as_id <  WMS_MSG_TRANSPORT_ARR_MAX) )  &&
      (temp_cfg_s_ptr != NULL))
  {
     if (WMS_MSG_TRANSPORT_NW_REG_STATUS_NO_SRV_ALLOW_ACK == transport_nw_reg_status)
     {
       MSG_HIGH_0("IMS only allows MT SMS Ack in no service status. Send IMS registration status as NO_SRV to clients");
       status.transport_status = WMS_MSG_TRANSPORT_NW_REG_STATUS_NO_SRV;
       temp_cfg_s_ptr->no_srv_ack_only[as_id] = TRUE;
     }
     else
     {
       temp_cfg_s_ptr->no_srv_ack_only[as_id] = FALSE;
     }
  }
...
#ifndef FEATURE_EMERG_SMS_OVER_IMS
        mn_msg.sms_msg_type = rpt_ind_ptr->ims_msg_type;
#else
        mn_msg.sms_msg_type = 0; //regular SMS
#endif
...
)
{
  ...
  MSG_HIGH_2("Number of instances: summary  = %d, details= %d ", 
             rpt_mwi_ind_ptr->num_instances_summary,
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  switch( cmd_ptr->hdr.cmd_id )
  {
   ...
   case WMS_CMD_MSG_SMS_CALLBACK:
      wms_mn_mo_sms_callback_proc(cmd_ptr);
      break;
...
)
{
  ...
  */
  ...
  if (WMS_OK_S == status) {
     return wms_cm_gw_send_rl_report(as_id, connection_id, TRUE, 0, 0, 0, msg_type);
  }
  ...
  return wms_cm_gw_send_rl_report(as_id, connection_id, FALSE, (uint8)rp_cause, tpdu, 3, msg_type);
...
)
{
  ...
  */
  ...
  else
  {
    ...
    msg_s_ptr->gw_mt_rec[i]->msg_type       = msg_type;
...
)
{
  ...
  if ( ( NULL == appInfo_ptr ) || !appInfo_ptr->pp_data_download_enabled || (SYS_MODEM_AS_ID_NONE >= appInfo_ptr->asid) || (MAX_AS_IDS <= appInfo_ptr->asid) )
  {
     return FALSE;
  }


  /* ATT requires only PID/DCS 0x7FF6 to be sent to the card. ENS NV needs to be set to 1
     to support this. */
  if (!cfg_s_ptr->ens_flag[appInfo_ptr->asid])
  {
     if ( ( ( WMS_PID_SIM_DATA_DOWNLOAD  == deliver_ptr->pid )   ||
            ( WMS_PID_ANSI136_R_DATA     == deliver_ptr->pid ) ) &&
          ( WMS_MESSAGE_CLASS_2          == deliver_ptr->dcs.msg_class ) )
     {
  ...
  {
    MSG_HIGH_1("DCS = %x", deliver_ptr->dcs.raw_dcs_data);

    if ( ( WMS_PID_SIM_DATA_DOWNLOAD == deliver_ptr->pid )           &&
         ( WMS_MESSAGE_CLASS_2       == deliver_ptr->dcs.msg_class ) &&
         ( WMS_GW_ALPHABET_8_BIT     == deliver_ptr->dcs.alphabet)   &&
         ( (0xF6 & deliver_ptr->dcs.raw_dcs_data) == 0xF6) )
    {
       /* This is a PP download msg; processing follows... */
    }
    else
  ...
  */
  ...
  else
  {
    ...
    */
    msg_s_ptr->gw_mt_rec[i]->as_id          = appInfo_ptr->asid;
    msg_s_ptr->gw_mt_rec[i]->appInfo_ptr    = appInfo_ptr;
    msg_s_ptr->gw_mt_rec[i]->client_id      = (wms_client_id_type)WMS_CLIENT_TYPE_CAT;
    msg_s_ptr->gw_mt_rec[i]->tid            = pp_download_tid;
    msg_s_ptr->gw_mt_rec[i]->smi            = uConnectionId;
    msg_s_ptr->gw_mt_rec[i]->ack_client_id  = (wms_client_id_type)WMS_CLIENT_TYPE_CAT;
    msg_s_ptr->gw_mt_rec[i]->user_data      = NULL;
    msg_s_ptr->gw_mt_rec[i]->transport_type = transport_type;
    msg_s_ptr->gw_mt_rec[i]->state          = WMS_MSG_STATE_WAIT_FOR_ACK;
    msg_s_ptr->gw_mt_rec[i]->msg_type       = msg_type;
...
)
{
  ...
  wms_message_e_type            message_type            = WMS_SMS_MAX;
  ...
  message_type = (NAS_E911_SMS == msg_type)? WMS_E911_SMS : WMS_REGULAR_SMS;
  msg_event_info.mt_message_info.msg_type = message_type;
  ...
  */
  ...
  if( ((length > WMS_SMS_UDL_MAX_7_BIT) && (alphabet == WMS_GW_ALPHABET_7_BIT_DEFAULT)) ||
  ...
  else
  {
     ...
     if (cfg_s_ptr->ens_flag[appInfo_ptr->asid]) 
     {
        wms_gw_deliver_s_type *deliver_ptr = &wms_msg_Gclient_ts.u.gw_pp.u.deliver;
        
        if ( WMS_MESSAGE_CLASS_2 == deliver_ptr->dcs.msg_class )
        {
           if ( ( WMS_PID_ANSI136_R_DATA       == deliver_ptr->pid ) ||
                ( WMS_PID_SIM_DATA_DOWNLOAD  == deliver_ptr->pid ) ) 
           {
              MSG_HIGH_2("Discarding this message: PID = %d, alphabet = %d",
                         deliver_ptr->pid, deliver_ptr->dcs.alphabet);
              msg_event_info.mt_message_info.route = WMS_ROUTE_DISCARD;
              msg_event_info.mt_message_info.message.msg_hdr.mem_store = WMS_MEMORY_STORE_NONE;
              routing.route  = WMS_ROUTE_DISCARD;
  ...
  MSG_HIGH_3( "Msg Route = %d Mem Store type=%d TPDU len = %d",
              routing.route,
              routing.mem_store, 
	      (raw_ts.len + pSCA->length));
			  
  if(routing.route == WMS_ROUTE_STORE_AND_NOTIFY)
  {
  ...
  {	  
     if( (raw_ts.len + pSCA->length + 1 > QMI_WMS_SMS_MAX_LEN ) )
     {
       (void) wms_msg_do_internal_ack( appInfo_ptr->asid,
                                       WMS_MT_MSG_FAILED_S,
                                       WMS_TP_CAUSE_TPDU_NOT_SUPPORTED,
    ...
    case WMS_ROUTE_TRANSFER_ONLY:
      ...
      else
      {
        ...
        */
        ...
        msg_s_ptr->gw_mt_rec[i]->state          = WMS_MSG_STATE_WAIT_FOR_ACK;
        msg_s_ptr->gw_mt_rec[i]->transport_type = transport_type;
        msg_s_ptr->gw_mt_rec[i]->msg_type       = message_type;
...
{
  uint32    rec_idx;
  wms_msg_set_sms_status_inactive(as_id);
...
)
{
  ...
  wms_msg_set_sms_status_inactive(asid);
...
#ifndef FEATURE_EMERG_SMS_OVER_IMS
  if (WMS_MSG_TRANSPORT_TYPE_SMS_OVER_IMS == msg_s_ptr->gw_mt_rec[rec_idx]->transport_type) 
  {
    transport_ptr = wms_msg_choose_registered_transport( asid );     
...
#ifndef FEATURE_EMERG_SMS_OVER_IMS
  if (send_retry_st && (WMS_RPT_OK != error_status) )
  {
    transport_ptr->sms_retry_cb(asid, FALSE, msg_s_ptr->gw_mt_rec[rec_idx]->msg_type);
  }
#endif
...
(
   ...
   wms_report_status_e_type error_status,
   uint8                    cause_value,
   lower_layer_cause_info_T ota_cause_info,
   uint8*                   puData,
   uint32                   uLen,
...
)
{
  ...
  */
  ...
  msg_s_ptr->gw_mo_rec[rec_idx]->state = WMS_MSG_STATE_QUEUED; 
...
#ifndef FEATURE_EMERG_SMS_OVER_IMS
      //Inform IMS client that there wont be any further retry
      if (send_retry_st)
      {
         transport_ptr->sms_retry_cb(asid, FALSE, msg_s_ptr->gw_mo_rec[rec_idx]->msg_type);
      }
#endif
      return;
    }
  ...
  */
  ...
  msg_event_info.submit_report_info.err_info.wms_rej_type          = (wms_reject_e_type)ota_cause_info.ota_reject_type;
  msg_event_info.submit_report_info.err_info.wms_rej_value         = (byte)ota_cause_info.ota_reject_cause;
  ...
  if(WMS_MSG_TRANSPORT_TYPE_SMS_OVER_IMS == msg_s_ptr->gw_mo_rec[rec_idx]->transport_type)
  {
    msg_event_info.submit_report_info.ims_cause_info.sms_over_ims = TRUE;
    msg_event_info.submit_report_info.ims_cause_info.ims_cause_value = ims_cause_code;
  }
  else
  {
    msg_event_info.submit_report_info.ims_cause_info.sms_over_ims = FALSE;
    msg_event_info.submit_report_info.ims_cause_info.ims_cause_value = ims_cause_code;
  }

  MSG_HIGH_2("MO SMS OTA_REJECT_TYPE = %d , OTA_REJECT_CAUSE = %d", ota_cause_info.ota_reject_type, ota_cause_info.ota_reject_cause);

  /* Code to Check GCF Flag for Retry on CS Pref Mode */
  gcf_flag = FALSE;
  gcf_flag = mcfg_gcf_nv_get_status();
  (void)wms_nv_read_sms_retry_flag(&retry_flag);



  MSG_HIGH_3( "GCF Flag for Retry = %d,  CS Domain Fallback = %d,custom retry enable = %d",
              gcf_flag,
  ...
  */
  ...
  else if ((TRUE == gcf_flag)&&(retry_flag == FALSE))
  {
                                                  ...
                                                  domain ) )
  {
    ...
    if( WMS_DOMAIN_PS == domain )
    {
      if ( (SYS_SYS_MODE_LTE == cfg_s_ptr->sys_mode[asid]) && (!cfg_s_ptr->cs_domain_fallback_on_lte) )
      {
  ...
  else if ( ok_to_retry )
  {
    ...
    else if (get_mo_retry_period(asid) > 0) 
    {
      ...
      {
         retry_interval = get_mo_retry_interval(asid);
      }
    }
  }
  ...
  */
...
#ifdef FEATURE_SMS_PWS_OVER_LTE
    if ( (TRUE == cfg_s_ptr->rrc_disable_ta_for_embms) &&
         (WMS_MSG_TRANSPORT_TYPE_CS == msg_s_ptr->gw_mo_rec[rec_idx]->transport_type) &&
         (WMS_RPT_RADIO_UNAVAILABLE == error_status) ) 
    {
       retry_count = msg_s_ptr->gw_mo_rec[rec_idx]->retry_count_embms;
       oos_due_to_embms = TRUE;
    }
    else
#endif 
    {
      retry_count = msg_s_ptr->gw_mo_rec[rec_idx]->retry_count;
      oos_due_to_embms = FALSE;
    }
  ...
  if( (WMS_RPT_OK != error_status) &&
      ...
      (wms_msg_is_mo_retry_allowed(retry_count, &msg_event_info.submit_report_info,msg_s_ptr->gw_mo_rec[rec_idx]->transport_type))
      && (gcf_flag == FALSE)
    )
  {

    if ( TRUE == oos_due_to_embms ) 
    {
       /* Set the retry_wait_time to 1*/
       msg_s_ptr->gw_mo_rec[rec_idx]->retry_wait_time = (sint31)1;
       msg_s_ptr->gw_mo_rec[rec_idx]->retry_count_embms++;
    }
    else
    {
      /* Set the retry_wait_time */
    ...
    MSG_HIGH_2("Prepare to retry : retry_time_left=%d, retry_interval=%d", 
                msg_s_ptr->gw_mo_rec[rec_idx]->retry_time_left, 
                retry_interval);
    ...
    if (send_retry_st)
    {
       transport_ptr->sms_retry_cb(asid, TRUE, msg_s_ptr->gw_mo_rec[rec_idx]->msg_type);
    }
#endif
  ...
  else
  {
    ...
    */
    ...
    if(msg_event_info.submit_report_info.report_status != WMS_RPT_OK)
    {
      ...
      if(wms_is_event_debug_any_failure_enabled(qsh_evt,msg_event_info.submit_report_info.as_id) == TRUE)
      {
        wms_qsh_client_event_log(WMS_QSH_EVENT_FIELD_DEBUG_ANY_FAILURE, error_status,msg_event_info.submit_report_info.as_id  );
      }    
	  else if(wms_is_qsh_event_valid_and_enabled(qsh_evt,msg_event_info.submit_report_info.as_id) == TRUE)
      {
         wms_qsh_client_event_log(qsh_evt, error_status,msg_event_info.submit_report_info.as_id );
      }
    }
#endif
    (void)wms_msg_event_notify( WMS_MSG_EVENT_SUBMIT_REPORT, &msg_event_info );
  ...
  */
  for( i = 0; i < WMS_MAX_MT_MESSAGES; i ++ )
  {
        ...
        rex_get_timer( & msg_s_ptr->cdma_mt_rec[i]->timer ) == 0 )
    {
      ...
      wms_msg_set_sms_status_inactive(rec_ptr->as_id);
      wms_msg_cdma_free_mt_msg_rec( rec_ptr );
    } /* if timer expired */
  } /* for */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if( cmd_ptr != NULL )
  {
    ...
    cmd_ptr->hdr.user_data   = NULL;
    cmd_ptr->hdr.as_id       = as_id;  
    cmd_ptr->hdr.appInfo_ptr = wms_get_appinfo_by_asid_and_message_mode(as_id,WMS_MESSAGE_MODE_CDMA); 
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  MSG_HIGH_2("AWI store: num_of_awi_words=%d , awi_info.cnt=%d ", 
              num_of_awi_words , 
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  */
  ...
  {
    MSG_HIGH_2( "AWI assembly error: bf and seq_no error check , awi_info.seq_no=%d, awi_info.cnt=%d",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    wms_msg_cdma_mt_MWI( *sms_ptr,as_id );  // data is only one byte, i.e. msg_count
    return; /* SHORT-RETURN */
  }
    ...
    )
  {
    ...
    else
    {
      ...
      */
      ...
      if((cmd_ptr->hdr.appInfo_ptr == NULL) && (WMS_MSG_TRANSPORT_TYPE_SMS_OVER_IMS == transport_type))
      {
      ...
      cmd_ptr->cmd.mt_msg.msg_type      = msg_type;
...
)
{
  if( WMS_GW_CB_SRV_ID_CMAS_MSGID1 <= cb_srv_id &&
      cb_srv_id <= WMS_GW_CB_SRV_ID_CMAS_MSGID30)
  {
    return TRUE;
  }
  return FALSE;
...
)
{
  ...
  if (NULL != appInfo_ptr && asid_is_valid(appInfo_ptr->asid))
  {
    ...
    if (NULL != stack_info_ptr)
    {
      MSG_HIGH_2( "Current SYS_MODE: %d, as id %d", stack_info_ptr->sys_mode, appInfo_ptr->asid );
      if (SYS_SYS_MODE_GSM == stack_info_ptr->sys_mode)
      {
        ...
        cell_info.phy_cell_id = stack_info_ptr->cell_info.cell_id;
        MSG_HIGH_1("RR Cell ID: 0x%x",cell_info.phy_cell_id );
      }
               ...
               SYS_SYS_MODE_TDS   == stack_info_ptr->sys_mode)
      {
        ...
        /**
               ...
               ** 3G TS 23.122 Annex A.
               **/
        cell_info.selected_plmn.mcc[0] =  cfg_s_ptr->umts_loc_info[appInfo_ptr->asid].mcc_and_mnc[0] & 0x0F;
        cell_info.selected_plmn.mcc[1] = (cfg_s_ptr->umts_loc_info[appInfo_ptr->asid].mcc_and_mnc[0] & 0xF0) >> 4;
        cell_info.selected_plmn.mcc[2] =  cfg_s_ptr->umts_loc_info[appInfo_ptr->asid].mcc_and_mnc[1] & 0x0F;
        cell_info.selected_plmn.mnc[0] =  cfg_s_ptr->umts_loc_info[appInfo_ptr->asid].mcc_and_mnc[2] & 0x0F;
        cell_info.selected_plmn.mnc[1] = (cfg_s_ptr->umts_loc_info[appInfo_ptr->asid].mcc_and_mnc[2] & 0xF0) >> 4;
        cell_info.selected_plmn.mnc[2] = (cfg_s_ptr->umts_loc_info[appInfo_ptr->asid].mcc_and_mnc[1] & 0xF0) >> 4;
        ...
        cell_info.phy_cell_id = (((((uint32)(cfg_s_ptr->umts_loc_info[appInfo_ptr->asid].cell_id[2])) <<24) & 0xFF000000)|
			                     ((((uint32)(cfg_s_ptr->umts_loc_info[appInfo_ptr->asid].cell_id[3])) <<16) & 0x00FF0000)|
			                     ((((uint32)(cfg_s_ptr->umts_loc_info[appInfo_ptr->asid].cell_id[0])) <<8) & 0x00000FF00)|
			                     (((uint32)(cfg_s_ptr->umts_loc_info[appInfo_ptr->asid].cell_id[1])) & 0x0000000FF));
 
        MSG_HIGH_1("RRC Cell ID: 0x%x",cell_info.phy_cell_id );
      }
      else
      {
  ...
  {
    MSG_ERROR_0("appInfo_ptr is NULL or Invalid as id");
  }
...
void wms_msg_gw_cb_data_ind_proc(
  ...
  uint8                        *cb_page,
  int                          page_length,
  wms_radio_access_tech_e_type current_rat,
  uint32                       wac_len,
  uint8                        *wac_info                 
)
{
  ...
  msg_event_info.mt_message_info.as_id           = appInfo_ptr->asid;
  msg_event_info.mt_message_info.transport_type  = WMS_MSG_TRANSPORT_TYPE_CS;
  msg_event_info.mt_message_info.rat             = current_rat;
  msg_event_info.mt_message_info.msg_type        = WMS_REGULAR_SMS;
  msg_event_info.mt_message_info.wac_info.len    = wac_len;
  msg_event_info.mt_message_info.wac_info.data   = NULL;

  if(msg_event_info.mt_message_info.wac_info.len > 0)
  {
    local_wac_data = (uint8 *)wms_mem_malloc(msg_event_info.mt_message_info.wac_info.len);
    if(local_wac_data != NULL)
    {
       memscpy((void *)local_wac_data, msg_event_info.mt_message_info.wac_info.len,
               (void *)wac_info, wac_len);
       msg_event_info.mt_message_info.wac_info.data = local_wac_data;
    }
    else
    {
      MSG_ERROR_0("Memory allocation for WAC data failed");
    }
  }
  ...
  MSG_HIGH_2("Received CB SMS with page length = %d on RAT %d", page_length, current_rat);
  ...
  wms_msg_gw_cb_send_log(&msg_ptr->u.gw_message.raw_ts_data);
                        ...
                        &wms_msg_Gclient_ts ) )
    {
      if ( (WMS_GW_CB_SRV_ID_CMAS_MSGID1 <= wms_msg_Gclient_ts.u.gw_cb.cb_page_hdr.cb_srv_id) &&
           (wms_msg_Gclient_ts.u.gw_cb.cb_page_hdr.cb_srv_id <= WMS_GW_CB_SRV_ID_CMAS_MSGID30) )
      {
...
)
{
  ...
  etws_msg_ptr->as_id = cb_etws_prim_ind_ptr->as_id;
  etws_msg_ptr->rat = WMS_SYS_RAT_UMTS_RADIO_ACCESS;
  wms_set_as_id(cb_etws_prim_ind_ptr->as_id);
...
)
{
  ...
  etws_msg_ptr->plmn_info_present = TRUE;
  etws_msg_ptr->as_id = cb_etws_sec_ind_ptr->as_id;
  etws_msg_ptr->rat = WMS_SYS_RAT_UMTS_RADIO_ACCESS;
  wms_set_as_id(cb_etws_sec_ind_ptr->as_id);
...
=========================================================================*/
wms_status_e_type wms_msg_ms_set_retry_period
(
  wms_client_id_type              client_id,
...
)
{
  ...
  if( (client_ptr == NULL) || (client_ptr->activated == FALSE) || !asid_is_valid(as_id) )
  {
    st = WMS_GENERAL_ERROR_S;
  }
  ...
  else
  {
      ...
      nv_item_type                 nvi;
      msg_s_ptr->mo_retry_period[as_id] = period;
      nvi.sms_mo_retry_period    = (uint8)period;

      if( NV_DONE_S != wms_nv_write_wait_per_subs( as_id, NV_SMS_MO_RETRY_PERIOD_I, &nvi ) )
      {
        st = WMS_GENERAL_ERROR_S;
      }
      else
      {
        MSG_HIGH_3( "Set Retry Period = %d seconds by Client %d, as_id %d",
                    period,
                    client_id,
                    as_id );
      }
...
=========================================================================*/
wms_status_e_type wms_msg_ms_get_retry_period
(
  wms_client_id_type              client_id,
  uint32                          * period,     /* in seconds */
  sys_modem_as_id_e_type          as_id
)
{
  ...
  else if( (client_ptr == NULL) || (client_ptr->activated == FALSE) || !asid_is_valid(as_id) )
  {
    st = WMS_GENERAL_ERROR_S;
  }
  else if (NULL == msg_s_ptr)
  ...
  {
    *period = get_mo_retry_period(as_id);
  }
  ...
  */
  for(i=0; i< WMS_MAX_MT_MESSAGES; i++)
  {
         ...
         WMS_MSG_STATE_IDLE != msg_s_ptr->cdma_mt_rec[i]->state )
    {
      ...
      {
		sys_modem_as_id_e_type     wms_1x_mt_as_id = msg_s_ptr->cdma_mt_rec[i]->as_id;
		
		if((WMS_MSG_TRANSPORT_TYPE_CS == msg_s_ptr->cdma_mt_rec[i]->transport_type) &&
           ( wms_1x_mt_as_id != wms_check_1x_active_sub()))
        {
           MSG_ERROR_0("Can't send ACK over 1X on inactive SUB");
           msg_s_ptr->cdma_mt_rec[i]->retry_time_left = (sint31)1;
           msg_s_ptr->cdma_mt_rec[i]->retry_wait_time = (sint31)0;
        }
             ...
             ( 0 < msg_s_ptr->cdma_mt_rec[i]->retry_wait_time) )
        {
          ...
          if (msg_s_ptr->cdma_mt_rec[i]->retry_wait_time == 0)
          {
            ...
            else /* other error */
            {
              ...
              wms_msg_set_sms_status_inactive(msg_s_ptr->cdma_mt_rec[i]->as_id);
        ...
        if (msg_s_ptr->cdma_mt_rec[i]->retry_time_left == 0)
        {
          ...
          memset(&mm_status, 0x00, sizeof(wms_mm_sms_call_info_T));
          mm_status.as_id = msg_s_ptr->cdma_mt_rec[i]->as_id;
          mm_status.overall_sms_status = TRUE;
          (void) wms_cm_gw_send_sms_status_to_mm(mm_status);
  ...
  */
  {
    ...
    {
      sys_modem_as_id_e_type     wms_1x_as_id = cdma_mo_rec_ptr->appInfo_ptr->asid;
      MSG_HIGH_4( "client_id = %d, ch_pref = %d, transport_type = %d, as_id = %d",
                  cdma_mo_rec_ptr->client_id,
                  cdma_mo_rec_ptr->ch_pref,
                  cdma_mo_rec_ptr->transport_type,
                  wms_1x_as_id);

      MSG_HIGH_5 ( "cdma_mo_rec = 0x%x, state = %d, retry_wait_time = %d, retry_time_left = %d, seq_num = %d",
                  cdma_mo_rec_ptr,
	 ...
	 if((WMS_MSG_TRANSPORT_TYPE_CS == cdma_mo_rec_ptr->transport_type) &&
        ( wms_1x_as_id != wms_check_1x_active_sub()))
     {
        MSG_ERROR_0("Can't send SMS over 1X on inactive SUB");
      ...
      if ( WMS_MSG_STATE_IDLE != cdma_mo_rec_ptr->state )
      {
        ...
        if (cdma_mo_rec_ptr->retry_time_left > 0)
        {
               ...
               wms_cfg_check_hvolte_oper_mode() )
          {
             ...
             /* Disconnect or abort DC channel bringup if previoudly attempted. */
             if ( dc_s_ptr->wms_initiated_call &&
                  ( WMS_DUMMY_CALL_ID != dc_s_ptr->dc_call_id ) )
             {
               MSG_HIGH_1("Initiate traffic call tear down for call id %d", dc_s_ptr->dc_call_id);
               if( WMS_OK_S != wms_dc_disconnect ( (wms_client_id_type) WMS_CLIENT_TYPE_INTERNAL,
                                                   NULL,
                                                   NULL,
                                                   wms_1x_as_id) )
               ...
               (cdma_mo_rec_ptr->retry_wait_time > 0) )
          {
            ...
            if (cdma_mo_rec_ptr->retry_wait_time == 0)
            {
                   ...
                   (WMS_CLIENT_CH_PREF_TCH_ONLY == cdma_mo_rec_ptr->ch_pref) )
              {
                ...
                {
                  MSG_HIGH_0("SMS is not possible over CS on TCH as 1X bearer is not available");
                  st = WMS_TRANSPORT_NOT_READY_S;
                }
              ...
              else if( st == WMS_1X_AND_VOICE_NOT_ALLOWED_S )
              {
                ...
                if( cdma_mo_rec_ptr->send_status_to_clients == TRUE )
                {
                 ...
                 else if(wms_is_qsh_event_valid_and_enabled(WMS_QSH_EVENT_SMS_FAIL_1X_AND_VOICE_NOT_ALLOWED,msg_event_info.submit_report_info.as_id) == TRUE)
                 {
                   wms_qsh_client_event_log(WMS_QSH_EVENT_SMS_FAIL_1X_AND_VOICE_NOT_ALLOWED, msg_event_info.submit_report_info.report_status,msg_event_info.submit_report_info.as_id );
                 }
#endif
                 (void)wms_msg_event_notify(WMS_MSG_EVENT_SUBMIT_REPORT,
                                            &msg_event_info);
                 {
                    ...
                    {		 
                      ...
                      appInfo_ptr = wms_get_appinfo_by_asid_and_message_mode(wms_1x_as_id, WMS_MESSAGE_MODE_CDMA);  
              ...
              else if ( st == WMS_TRANSPORT_NOT_READY_S || st == WMS_TRANSPORT_NOT_ALLOWED_S )
              {
                ...
                if( cdma_mo_rec_ptr->send_status_to_clients == TRUE )
                {
                  ...
                  {
                    wms_qsh_client_event_log(WMS_QSH_EVENT_SMS_FAIL_NO_TRANSPORT, msg_event_info.submit_report_info.report_status, msg_event_info.submit_report_info.as_id );
                  }
#endif
                   ...
                   (cdma_mo_rec_ptr->retry_wait_time == 0))
		  {
              if ( (WMS_MSG_TRANSPORT_TYPE_CS == cdma_mo_rec_ptr->transport_type) &&
                   ...
                   (WMS_CLIENT_CH_PREF_TCH_ONLY == cdma_mo_rec_ptr->ch_pref) &&
                   (!dc_s_ptr->wms_initiated_call) &&
				   ((cdma_mo_rec_ptr->appInfo_ptr != NULL) && !(cdma_mo_rec_ptr->appInfo_ptr->sim_init_complete)))
              {
                msg_event_info.submit_report_info = cdma_mo_rec_ptr->prev_report;
        ...
        if (cdma_mo_rec_ptr->retry_time_left == 0)
        {
          ...
          if ( cdma_mo_rec_ptr->send_status_to_clients )
          {
            ...
            if ( (WMS_MSG_STATE_LAYER2_IN_PROGRESS == cdma_mo_rec_ptr->state) &&
                 ...
                 (cdma_mo_rec_ptr->msg_type != WMS_E911_SMS) )
            {
              msg_event_info.submit_report_info.delivery_failure_permanent = TRUE;
            }
            else
        ...
        if (msg_s_ptr->gw_mo_rec[i]->retry_time_left == 0)
        {
          ...
          else
          {
            ...
            */
            ...
            MSG_HIGH_2("Retry Period Expired, GCF Flag for Retry = %d, custom retry = %d", gcf_flag,retry_flag);

            if ((TRUE == gcf_flag)&&(retry_flag==FALSE))
            {
...
)
{
  ...
  if (NULL != l_cfg_s_ptr)
  {
    ...
    sys_modem_as_id_e_type as_id_arr[MAX_AS_IDS+2];
    int count =0;
...
#endif
   if (NULL != l_cfg_s_ptr->wms_app4_info)
    {
      if (0 != l_cfg_s_ptr->wms_app4_info->mo_sms_count)
      {
        as_id_arr[count++] = l_cfg_s_ptr->wms_app4_info->asid;
      }

      l_cfg_s_ptr->wms_app4_info->mo_sms_count = 0;
    }
    ...
    for (i = 0; i < count; i++)
    {
      ...
      {
        wms_msg_set_sms_status_inactive(as_id_arr[i]);
      }
    }
  }
...

  {
    ...
    while ( NULL != cdma_mo_rec_ptr )
    {
      ...
      if ( cdma_mo_rec_ptr->send_status_to_clients )
      {
        ...
        else if(wms_is_qsh_event_valid_and_enabled(WMS_QSH_EVENT_SMS_FAIL_NETWORK_NOT_READY,msg_event_info.submit_report_info.as_id) == TRUE)
        {
          wms_qsh_client_event_log(WMS_QSH_EVENT_SMS_FAIL_NETWORK_NOT_READY,msg_event_info.submit_report_info.report_status, msg_event_info.submit_report_info.as_id );
        }
#endif
       (void)wms_msg_event_notify(WMS_MSG_EVENT_SUBMIT_REPORT, &msg_event_info);
      }
...
FUNCTION wms_msg_mo_sms_emergency_num_check_func

DESCRIPTION
  This function checks if number is emergency number or not

DEPENDENCIES
  None.

RETURN VALUE
  TRUE if PBM indicates the number is classfied as emergency number
  FALSE otherwise
...
===========================================================================*/
boolean wms_msg_mo_sms_emergency_num_check_func(
  wms_prov_app_info_s_type *appInfo_ptr,
  wms_address_s_type       *address
)
{
  ...
  {
        MSG_HIGH_2("digit[%d] = %d\n", i, *(pAddress+i));
  }

  /* TBD: For now, VzW needs only 911 as emergency SMS. Use PBM API for phase 2 requirements. */
  if ( (nAddressLen     == 3)    &&
       (*(pAddress + 0) == '9')  &&
       (*(pAddress + 1) == '1')  &&
       (*(pAddress + 2) == '1') )
  ...
  wms_mem_free(pAddress);
  MSG_HIGH_1("wms_msg_mo_sms_emergency_num_check_func() returns %d", result);
  return result;
}
/*===========================================================================
...
)
{
  ...
  wms_set_as_id(as_id);
  MSG_HIGH_3( "In wms_msg_lte_etws_prim_proc(): msg_id 0x%04x with serial 0x%04x  sub_id = %d",
              lte_etws_prim_ptr->msg_id,
              lte_etws_prim_ptr->sr_num,
              as_id);
  ...
  if (!msg_is_dup)
  {
    uint8                pos          = 0;
    wms_etws_msg_s_type *etws_msg_ptr = &(msg_event_info.etws_msg_info);
	etws_msg_ptr->as_id = as_id;

    etws_msg_ptr->type = WMS_ETWS_NOTIFICATION_PRIMARY;
    etws_msg_ptr->plmn_info_present = TRUE;
    etws_msg_ptr->as_id = as_id;
    etws_msg_ptr->rat = WMS_SYS_RAT_LTE_RADIO_ACCESS;
...
)
{
  ...
  wms_set_as_id(as_id);
  MSG_HIGH_3( "In wms_msg_lte_etws_sec_proc(): msg_id 0x%04x with serial 0x%04x sub id = %d ",
              lte_etws_sec_ptr->msg_id,
              lte_etws_sec_ptr->sr_num,
              as_id);
...
)
{
  boolean                   msg_is_dup     = FALSE;
  wms_prov_app_info_s_type *appInfo_ptr    = NULL;
  uint8 num_attach = 0, i;

  dsm_item_type  *cmas_dsm_ptr = NULL ; /* First attachment is CMAS pages */
  dsm_item_type  *wac_dsm_ptr = NULL; /* Second attachment (optional) is for WAC */

  msgr_attach_struct_type  *cmas_att_ptr   = NULL; /* Attachment pointer */
  msgr_attach_struct_type  *wac_att_ptr    = NULL;
   ...
   * Leaving the attachment unfetched will eventually exhaust the buffer pool.
   *
   * 1) Get the number of attachments and verify attachment count==1 or 2
   * 2) Get the reference to the attachment
   * 3) Use the attachment reference to pull in the attachment data
   */
  num_attach = msgr_get_num_attach((msgr_hdr_struct_type *)lte_cmas_ptr);
  MSG_HIGH_4( "In wms_msg_lte_cmas_proc(): msg_id 0x%04x with serial 0x%04x sub id = %d, num_attach %d",
              lte_cmas_ptr->msg_id,
              lte_cmas_ptr->sr_num,
              as_id,
              num_attach);
  ...
  if ( (num_attach == 0) ||
       (num_attach > 2) ||
       ((cmas_att_ptr = msgr_get_attach((msgr_hdr_struct_type *)lte_cmas_ptr, 0)) == NULL) ||
       ((num_attach == 2) &&
        ((wac_att_ptr = msgr_get_attach((msgr_hdr_struct_type *)lte_cmas_ptr, 1)) == NULL) ) )
  {
    MSG_ERROR_3("Number of attachments = %d cmas_att_ptr 0x%x wac_att_ptr 0x%x", 
                num_attach, cmas_att_ptr, wac_att_ptr);
    return;
  }

  //Get dsm attachments
  msgr_get_dsm_attach(cmas_att_ptr, &cmas_dsm_ptr);
  if (NULL == cmas_dsm_ptr)
  {
    MSG_ERROR_0("msgr_get_dsm_attach returns NULL pointer!");
    return;
  }
  ...
  if (num_attach == 2)
  {
    msgr_get_dsm_attach(wac_att_ptr, &wac_dsm_ptr);
    if (NULL == wac_dsm_ptr)
    {
      MSG_ERROR_0("msgr_get_dsm_attach returns NULL pointer!");
      return;
  ...
  appInfo_ptr = wms_get_lte_appinfo_by_as_id(as_id);

  if (NULL == appInfo_ptr)
  {
    MSG_HIGH_0("No LTE service found on any subs");
    dsm_free_packet(&cmas_dsm_ptr);
    dsm_free_packet(&wac_dsm_ptr); //can take NULL pointer
    return;
  }
  ...
  if (!msg_is_dup)
  {
    ...
    uint32 wac_len                              = 0;
    uint8  wac_data[WMS_BC_WAC_LEN_MAX];
    int    byte_count;
    int    page_idx;

    /* Pull DSM attachment for WAC data if present */
    if (NULL != wac_dsm_ptr)
    {
      // Get the length of WAC data
      dsm_packet_length = dsm_length_packet(wac_dsm_ptr);
      MSG_HIGH_1( "CMAS WAC data len = %d:", dsm_packet_length);
      if ( (0 == dsm_packet_length) || (dsm_packet_length > WMS_BC_WAC_LEN_MAX) )
      {
        // Free the attachment buffer -- unfetched data is thrown away
        dsm_free_packet(&cmas_dsm_ptr);
        dsm_free_packet(&wac_dsm_ptr);
        return;
      }

      wac_len = dsm_packet_length;
      (void)dsm_pullup(&wac_dsm_ptr, &wac_data[0], wac_len);  
    }
    ...
    // Get the length of CB data for the secondary notification
    dsm_packet_length = dsm_length_packet(cmas_dsm_ptr);
    ...
    */
    ...
    byte_count = dsm_pullup(&cmas_dsm_ptr, &cmas_num_pages, 1);
    ...
    for ( page_idx = 1; page_idx <= cmas_num_pages; page_idx++)
    {
      if (dsm_packet_length >= 83)
      {
        ...
        byte_count = dsm_pullup(&cmas_dsm_ptr, cmas_page_data + pos, 82);
        dsm_packet_length -= (uint32)byte_count;
        ...
        if (82 == byte_count)
        {
          ...
          byte_count = dsm_pullup(&cmas_dsm_ptr, &cmas_page_len, 1);
          dsm_packet_length -= (uint32)byte_count;
          ...
          {
             /* Send WAC data with the last page. */
             if (page_idx == cmas_num_pages)
             {
               wms_msg_gw_cb_data_ind_proc(appInfo_ptr->asid, cmas_page_data, (int)cmas_page_len, WMS_SYS_RAT_LTE_RADIO_ACCESS, wac_len, wac_data);
             }
             else
             {
               wms_msg_gw_cb_data_ind_proc(appInfo_ptr->asid, cmas_page_data, (int)cmas_page_len, WMS_SYS_RAT_LTE_RADIO_ACCESS, 0, NULL);
             }
          }
        }
        else
...
)
{
  ...
  MSG_HIGH_3( "wms_msg_lte_srv_ind_proc svc_status = %d, global_cell_id = %d sub_id = %d",
  ...
  if (LTE_RRC_SVC_STATUS_AVAILABLE == lte_srv_ind_ptr->svc_status)
  {
    ...
    cell_info.phy_cell_id = (uint32)lte_srv_ind_ptr->camped_svc_info.phy_cell_id;
    ...
    if (plmn_changed || tac_changed || cell_changed)
    {
      ...
      wms_cfg_cell_change_ind_proc(as_id, cb_cell_change);
    }
  }
}
...
//==============================================================================
//==============================================================================
void wms_msg_lte_rrc_msg_state_ind( 
   sys_modem_as_id_e_type         as_id,
   wms_msg_transport_type_e_type  transport_type,
   wms_sms_state_e_type           state
)
{
  wms_sms_status_ind_s_type  *msg_state_ptr; 
  errno_enum_type            msgr_error = E_SUCCESS;
  int i;
  
  MSG_HIGH_3("In wms_lte_rrc_msg_state_ind(): as_id = %d, transport_type = %d, msg state = %d",
             as_id, transport_type, state);

  if ( WMS_MSG_SMS_END == state ) 
  {
    for( i=0; i < WMS_MAX_MO_MESSAGES; i++ )
    {
      if ( (NULL != msg_s_ptr->gw_mo_rec[i]) &&
           (as_id == msg_s_ptr->gw_mo_rec[i]->as_id) && 
           (WMS_MSG_STATE_IDLE != msg_s_ptr->gw_mo_rec[i]->state) )
      {
        return;
      }
    }
  ...
  wms_set_as_id(as_id);
  msg_state_ptr = (wms_sms_status_ind_s_type*) wms_mem_malloc(sizeof(wms_sms_status_ind_s_type));

  if ( NULL != msg_state_ptr ) 
  {
    msg_state_ptr->as_id        = as_id;
    msg_state_ptr->transport_id = transport_type;
    msg_state_ptr->state        = state; 

    msgr_init_hdr((msgr_hdr_struct_type *)&(msg_state_ptr->msg_hdr),
                   MSGR_WMS_LTE,
                   WMS_LTE_SMS_STATUS_IND);

    msgr_set_hdr_variant((msgr_hdr_struct_type *)&(msg_state_ptr->msg_hdr),(uint8)as_id);

    msgr_error = msgr_send((msgr_hdr_struct_type*)&(msg_state_ptr->msg_hdr),
                           sizeof(wms_sms_status_ind_s_type));

    MSG_HIGH_1("msgr_send() returns %d", msgr_error);
  }
  else 
  {
    MSG_HIGH_0("wms_mem_malloc() fail");
  }
  
  WMS_MEM_FREEIF(msg_state_ptr);
}
