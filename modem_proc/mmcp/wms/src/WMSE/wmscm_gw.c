/*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/wms/src/WMSE/wmscm_gw.c#1 $
...
wms_report_status_e_type wmscmi_map_report_status(byte error_status)
{
  ...
  //   case NO_ERROR_ABORT_SM_COMPLETE:
   ...
   MSG_ERROR_2("Error during msg submit: error status %d, report status %d", error_status, report_status);
...
static void wmscmi_process_rpt(wms_rpt_type* rpt_ptr)
{
   ...
   case UASMS_SM_DATA_IND:
      {
         ...
         wms_msg_sm_data_ind_proc( asid,
                              ...
                              #endif
                                   ,rpt_ptr->rpt.rl_data_ind.sms_msg_type
                                    );
      }
      break;
   ...
   case CB_DATA_IND:
      {
         sys_modem_as_id_e_type asid = SYS_MODEM_AS_ID_1;
         wms_radio_access_tech_e_type rat = WMS_SYS_RAT_NONE;
         ...
         if (rpt_ptr->rpt.cb_data_ind.current_rat == SYS_RAT_UMTS_RADIO_ACCESS)
         {
            rat = WMS_SYS_RAT_UMTS_RADIO_ACCESS;
         }
         else if (rpt_ptr->rpt.cb_data_ind.current_rat == SYS_RAT_GSM_RADIO_ACCESS)
         {
            rat = WMS_SYS_RAT_GSM_RADIO_ACCESS;
         }
         else
         {
            MSG_ERROR_1("Incorrect RAT %d", rpt_ptr->rpt.cb_data_ind.current_rat);
         }
#endif
...
/*===========================================================================
FUNCTION wms_cm_gw_map_nas_message_type

DESCRIPTION
  Maps WMS GW message type to NAS message type

DEPENDENCIES
  none

RETURN VALUE
  nas_msg_e_type  Mapped NAS Message type enum 

SIDE EFFECTS
  None.
===========================================================================*/
static nas_msg_e_type wms_cm_gw_map_nas_message_type(
   wms_message_e_type message_type
)
{
  nas_msg_e_type nas_msg_type = NAS_REGULAR_SMS;

#ifndef FEATURE_EMERG_SMS_OVER_IMS
  if (WMS_E911_SMS == message_type)
  {
    nas_msg_type = NAS_E911_SMS;
  }
#endif

  return nas_msg_type;
} /* wms_cm_gw_map_nas_message_type */
...
)
{
   ...
   if (WMS_MSG_TRANSPORT_TYPE_SMS_OVER_IMS == mo_rec_ptr->transport_type)
   {
     ...
     if ( (transport_ptr != NULL) && 
          ...
          (mo_rec_ptr->as_id > SYS_MODEM_AS_ID_NONE) && 
          (mo_rec_ptr->as_id <  WMS_MSG_TRANSPORT_ARR_MAX) )
     {
       if ( (WMS_MSG_TRANSPORT_NW_REG_STATUS_LIMITED_SRV == transport_ptr->nw_reg_status ||
            wms_cfg_s_ptr()->admin_sms_only[mo_rec_ptr->as_id]) && !wms_is_gw_admin_msg(raw_ts_ptr) &&
            (mo_rec_ptr->msg_type != WMS_E911_SMS) ) 
       {
       ...
       if ( ( (WMS_MSG_TRANSPORT_NW_REG_STATUS_NO_SRV_ALLOW_ACK == transport_ptr->nw_reg_status) || 
            wms_cfg_s_ptr()->no_srv_ack_only[mo_rec_ptr->as_id] ) &&
            (mo_rec_ptr->msg_type != WMS_E911_SMS) ) 
       {
       ...
       if ( (mo_rec_ptr->msg_type == WMS_E911_SMS) &&
            (NULL == wms_get_lte_appinfo_by_as_id(mo_rec_ptr->as_id) ) )
       {
...
#ifdef FEATURE_SMS_PWS_OVER_LTE
   if ( (TRUE == wms_cfg_s_ptr()->rrc_disable_ta_for_embms) && 
        (WMS_MSG_TRANSPORT_TYPE_CS == mo_rec_ptr->transport_type) )
   {
     mo_rec_ptr->state = WMS_MSG_STATE_LAYER2_IN_PROGRESS;
     wms_msg_lte_rrc_msg_state_ind( mo_rec_ptr->as_id, mo_rec_ptr->transport_type, WMS_MSG_SMS_START );
   }
#endif
   ...
   cmnas_err = CMCNas_SendData(pCMCNas,
                               ...
                               wms_cm_gw_map_nas_message_type(mo_rec_ptr->msg_type) );
   ...
   MSG_HIGH_2("MO SMS Message Sent to MN %d, message type %d", cmd_err, mo_rec_ptr->msg_type);
...
)
{
   ...
   if (WMS_DBG_CONNECTION_ID != connection_id)
   {
      ...
      cmnas_err = CMCNas_SendReport( pCMCNas,
                                     ...
                                     tpdu_ptr,
                                     wms_cm_gw_map_nas_message_type(msg_type) );
   }
   else
   {
...
)
{
...
#ifdef FEATURE_SMS_PWS_OVER_LTE
   {
      ...
      msgr_set_hdr_variant((msgr_hdr_struct_type *)&msg_id_list_req,(uint8)as_id);
...
)
{
...
#ifdef FEATURE_SMS_PWS_OVER_LTE
   {
      ...
      MSG_HIGH_2("Length of search list 1 = %d, search list 2 = %d ", len1, len2);
      j = 0;
      /* Fill up CBMID Entries */
      for (i = 0, j = 0;
