/*====*====*====*====*====*====*====*====*====*====*==*====*====*====*====*
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/wms/src/WMSE/wmsdbg.c#1 $
...
extern uint32 get_mo_retry_period(sys_modem_as_id_e_type as_id);
extern uint32 get_mo_retry_interval(sys_modem_as_id_e_type as_id);
...
)
{
  ...
  if (cmd_err == WMS_CMD_ERR_NONE)
  {
    ...
    dbg_event_info.retry_interval = get_mo_retry_interval(as_id); /* In seconds */
    dbg_event_info.as_id = as_id;
...
)
{
  ...
  else if (retry_interval >  get_mo_retry_period(as_id))
  {
    cmd_err = WMS_CMD_ERR_DBG_BAD_PARAM;
  }
  else if (get_mo_retry_interval(as_id) != retry_interval)
  {
    nv_item_type          nvi;
    nvi.sms_mo_retry_interval = (byte)retry_interval;

    if (NV_DONE_S != wms_nv_write_wait_per_subs(as_id, NV_SMS_MO_RETRY_INTERVAL_I, &nvi))
    {
      cmd_err = WMS_CMD_ERR_CFG_NV_WRITE;
    }
    else
    {
      MSG_HIGH_2("Setting Retry Interval = %d seconds on as_id %d is successful .", retry_interval, as_id);
      msg_s_ptr->mo_retry_interval[as_id] = retry_interval;
    }
  }
  else
  {
  ...
  */
  if (WMS_CMD_ERR_NONE == cmd_err)
  {
    ...
    /* Fill up Event Information */
    dbg_event_info.retry_interval = get_mo_retry_interval(as_id);
    dbg_event_info.as_id = as_id;
...
)
{
  ...
  if (cmd_err == WMS_CMD_ERR_NONE)
  {
    ...
    // Fill up Event Information
    dbg_event_info.retry_period = get_mo_retry_period(as_id); /* In seconds */
    dbg_event_info.as_id = as_id;
...
)
{
  ...
  else if (WMS_MAX_RETRY_PERIOD < retry_period)
  {
    cmd_err = WMS_CMD_ERR_DBG_BAD_PARAM;
  }
  else if (get_mo_retry_period(as_id) != retry_period)
  {
    nv_item_type          nvi;
    ...
    {
      MSG_HIGH_2("Setting Retry Period = %d seconds on as id %d is successful.", retry_period, as_id);
      msg_s_ptr->mo_retry_period[as_id] = retry_period;
    }
  }
  else
  {
  ...
  if (WMS_CMD_ERR_NONE == cmd_err)
  {
    ...
    // Fill up Event Information
    dbg_event_info.retry_period = get_mo_retry_period(as_id);
    dbg_event_info.as_id = as_id;
  ...
  if (WMS_MSG_TRANSPORT_MT_SMS_IND == rpt_ind_ptr->ind_type)
  {
     ...
     case WMS_FORMAT_CDMA:
       {
         ...
         mt_msg.msg_type = 0; /*Treating as normal SMS*/
     ...
     case WMS_FORMAT_GW_CB:
        ...
        if (rpt_ind_ptr->mt_sms.data_len <= WMS_GW_CB_PAGE_SIZE) {
           as_id = wms_get_as_id_from_message_mode(WMS_MESSAGE_MODE_GW); // FIX: MESSAGE MODE MAY NOT BE SUFFICIENT
           wms_msg_gw_cb_data_ind_proc(as_id, rpt_ind_ptr->mt_sms.data, rpt_ind_ptr->mt_sms.data_len, WMS_SYS_RAT_GSM_RADIO_ACCESS,
                                       0, NULL); //FIX: GSM RAT MAY NOT BE SUFFICIENT
           cmd_err = WMS_CMD_ERR_NONE;
        }
        break;
#endif
...
static wms_dbg_cmd_info_s_type wms_dbg_cmd_info[] =
{
  ...
  {WMS_CMD_MSG_SMS_CALLBACK,                "WMS_CMD_MSG_SMS_CALLBACK"},
