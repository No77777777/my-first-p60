/*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/wms/src/WMSE/wms.c#2 $
  $DateTime: 2021/07/02 21:31:01 $
  $Author: pwbldsvc $
*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*/
...
WMS_QSH_EVENT_NOTIFY_SMS_3GPP  = 0,  /*Handle for originating 3GPP SMS */
WMS_QSH_EVENT_NOTIFY_SMS_3GPP2 = 1,  /*Handle for originating 3GPP2  SMS */
WMS_EVENT_NOTIFY_MAX
...
/*===========================================================================
FUNCTION wms_msgr_register()                                       (INTERNAL)
...
)
{
...
#endif
  {
    ...
    {
      MSG_ERROR_2("msgr_register failed for ID %d as id %d", 
                   umid, as_id);
    }
  }
}
...
/*===========================================================================
FUNCTION wms_msgr_deregister()                                     (INTERNAL)
...
)
{
...
#endif
  {
    ...
    {
      MSG_ERROR_2("msgr_deregister failed for ID %d, as id %d", 
                   umid, as_id);
    }
  }
}
...
static void wms_msgr_init(void)
{
  ...
  wms_msgr_register(MSGR_WMS_LTE, &wms_msgr_client, wms_msgr_rex_q_id, LTE_RRC_ETWS_PRIM_IND);

  /* Register with MSG ROUTER for ETWS secondary notification */
  wms_msgr_register(MSGR_WMS_LTE, &wms_msgr_client, wms_msgr_rex_q_id, LTE_RRC_ETWS_SEC_IND);

  /* Register with MSG ROUTER for CMAS notification */
  wms_msgr_register(MSGR_WMS_LTE, &wms_msgr_client, wms_msgr_rex_q_id, LTE_RRC_CMAS_IND);

  /* Register with MSG ROUTER for LTE CELL CHANGE notification */
  wms_msgr_register(MSGR_WMS_LTE, &wms_msgr_client, wms_msgr_rex_q_id, LTE_RRC_SERVICE_IND);

#endif /* FEATURE_SMS_PWS_OVER_LTE */

  /* Register with MSG ROUTER for Domain Selection Change notification */
  wms_msgr_register(MSGR_WMS_LTE, &wms_msgr_client, wms_msgr_rex_q_id, MM_DOM_SEL_DOMAIN_SELECTED_CHANGED_IND);
 
  /* Register with MSG ROUTER for OFFLINE notification */
  wms_msgr_register(MSGR_WMS_LTE, &wms_msgr_client, wms_msgr_rex_q_id, MM_MMOC_OFFLINE_IND);

#ifdef FEATURE_DOMAIN_SELECTION

  /* Register with MSG ROUTER for LTE DOMAIN PREF change notification */
  wms_msgr_register(MSGR_WMS_CM, &wms_msgr_client, wms_msgr_rex_q_id, MM_DOM_SEL_CONFIG_LTE_SMS_DOMAIN_PREF_CHANGED_IND);
  
#endif /* FEATURE_DOMAIN_SELECTION */

  /* Register with MSG ROUTER for Policy Man device mode change notification */
  wms_msgr_register(MSGR_WMS_POLICYMAN, &wms_msgr_client, wms_msgr_rex_q_id, POLICYMAN_CFG_UPDATE_MSIM_IND);
...
)
{
  ...
  {
    as_id = MSGR_GET_VARIANT(&(rpt_ptr->cmd.msg_hdr));
    MSGR_CLEAR_VARIANT(rpt_ptr->cmd.msg_hdr.id);
    ...
    switch (rpt_ptr->cmd.msg_hdr.id)
    {
      ...
      case LTE_RRC_ETWS_PRIM_IND:
        ...
        wms_msg_lte_etws_prim_proc(&(rpt_ptr->cmd.lte_etws_prim), as_id);
        break;

      case LTE_RRC_ETWS_SEC_IND:
        MSG_HIGH_0("Received ETWS secondary notification");
        wms_msg_lte_etws_sec_proc(&(rpt_ptr->cmd.lte_etws_sec), as_id);
        break;

      case LTE_RRC_CMAS_IND:
        MSG_HIGH_0("Received CMAS notification");
        wms_msg_lte_cmas_proc(&(rpt_ptr->cmd.lte_cmas), as_id);
        break;

      case LTE_RRC_SERVICE_IND:
        MSG_HIGH_0("Received LTE service ready notification");
        wms_msg_lte_srv_ind_proc(&(rpt_ptr->cmd.lte_service_ind), as_id);
        break;
#endif 
...
)
{
  ...
  MSG_HIGH_1("In wms_rrc_cell_change_ind_cb(): as id %d ", cell_info.as_id);

  if ( (NULL != cmd_ptr) && asid_is_valid(cell_info.as_id) )
  {
    cmd_ptr->hdr.cmd_id      = WMS_CMD_CFG_RRC_CELL_CHANGE_EVENT_CB;
    cmd_ptr->hdr.client_id   = (uint8) WMS_CLIENT_TYPE_INTERNAL;
    cmd_ptr->hdr.as_id       = cell_info.as_id;
  ...
  {
    MSG_ERROR_0("Out of cmd buffer or Invalid as id");
  }
...
)
{
  ...
  else if ( WMS_DEVICE_MODE_TSTS == cfg_s_ptr->device_mode )
  {
    j = 3;
  }
  ...
  if (NULL != ptr_W)
  {
     for (i = 0; i < j; i++)
     {
       (void)IWCDMA_rrc_register_serving_cell_change_ind_cb_sub( ptr_W, i, wms_rrc_cell_change_ind_cb );
     }
  }
  ...
  for (i = 0; i < j; i++)
  {
     (void)rrc_register_serving_cell_change_ind_cb_sub( i, wms_rrc_cell_change_ind_cb );
  }
...
)
{
  ...
  // Initialize the MSG (message) module:
  //   - init the timerF
  //   - init transport id
  {
     wms_msg_s_type *msg_s_ptr = wms_msg_s_ptr();

     for (i=0; i< WMS_MSG_TRANSPORT_ARR_MAX; i++)
     {
        msg_s_ptr->timerf_value[i] = 0; 
     }
  }
...
void wms_qsh_notify_process_event_notify
(
...
)
{
    ...
    sys_modem_as_id_e_type	 subs_id		 = param->action_params.event_notify.subs_id;
    wms_event_notify_id	 event_notify_id = param->action_params.event_notify.id;

    MSG_HIGH_2( " WMS event_notify_id : %d sub_id : %d ", event_notify_id,subs_id);

    switch(event_notify_id)
    {
      case WMS_QSH_EVENT_NOTIFY_SMS_3GPP:
      {

        wms_client_message_s_type         *wms_send_qsh_ptr = NULL;
        wms_status_e_type ret_val = WMS_OK_S;


        wms_send_qsh_ptr =  (wms_client_message_s_type *)wms_mem_malloc(sizeof(wms_client_message_s_type));

        if (NULL == wms_send_qsh_ptr)
        {
           MSG_ERROR_1( "malloc failed: wms_send_qsh_ptr=0x%08x",wms_send_qsh_ptr );
        }
        else
        {
          /* using some default data */
          /*  raw_data[] =  { TP-MTI, TP-RD, TP-VPF, TP-SRR, TP_UDHI, TP-RP :1, 
                                               TP-MR: 0, 
                                               TP-DA: 10, 129, 8, 128, 153,84, 38, 
                                               TP_PID:0, 
                                               TP_DCS: 0, 
                                               TP_USERDATA :2, 200, 52 */

          uint8 raw_data[] =  {1, 0, 10, 129, 8, 128, 153,84, 38, 0, 0, 2, 200, 52  };
          memset (wms_send_qsh_ptr, 0x00, sizeof(wms_client_message_s_type));

          /* Fill in the MSG header */
          wms_send_qsh_ptr->msg_hdr.message_mode = WMS_MESSAGE_MODE_GW;
          wms_send_qsh_ptr->msg_hdr.tag = WMS_TAG_MO_NOT_SENT;
          wms_send_qsh_ptr->msg_hdr.mem_store = WMS_MEMORY_STORE_NONE;
          wms_send_qsh_ptr->msg_hdr.index = WMS_DUMMY_MESSAGE_INDEX;

          wms_send_qsh_ptr->u.gw_message.raw_ts_data.format = WMS_FORMAT_GW_PP;
          wms_send_qsh_ptr->u.gw_message.raw_ts_data.len = sizeof(raw_data);
          wms_send_qsh_ptr->u.gw_message.raw_ts_data.tpdu_type = WMS_TPDU_SUBMIT;

          memscpy(wms_send_qsh_ptr->u.gw_message.raw_ts_data.data, sizeof(wms_send_qsh_ptr->u.gw_message.raw_ts_data.data),
                                                                      raw_data, wms_send_qsh_ptr->u.gw_message.raw_ts_data.len);

          ret_val = wms_msg_ms_send(WMS_CLIENT_TYPE_INTERNAL,
                             subs_id,
                             NULL,
                             NULL,
          ...
          MSG_HIGH_1( " WMS send msg: on reciving QSH event %d ", ret_val);
       }

       if(wms_send_qsh_ptr != NULL)
       {
         wms_mem_free(wms_send_qsh_ptr);
       }

      }
      break;
      case WMS_QSH_EVENT_NOTIFY_SMS_3GPP2:
      {
        wms_client_message_s_type *cdma_qsh_ptr= NULL;
        cdma_qsh_ptr = (wms_client_message_s_type *)malloc(sizeof(wms_client_message_s_type));
        int i=0;
        wms_status_e_type ret_val = WMS_OK_S;

        if (NULL == cdma_qsh_ptr)
        {
           MSG_ERROR_1( "malloc failed: cdma_qsh_ptr=0x%08x",cdma_qsh_ptr );
        }
        else
        {
          uint8 raw_data[] = {200,52};

          cdma_qsh_ptr->msg_hdr.message_mode = WMS_MESSAGE_MODE_CDMA;
          cdma_qsh_ptr->msg_hdr.tag          = WMS_TAG_MO_NOT_SENT;
          cdma_qsh_ptr->msg_hdr.mem_store    = WMS_MEMORY_STORE_NONE;
          cdma_qsh_ptr->msg_hdr.index        = 0;
       
          cdma_qsh_ptr->u.cdma_message.is_mo = TRUE;
          cdma_qsh_ptr->u.cdma_message.teleservice = WMS_TELESERVICE_CMT_95;
          cdma_qsh_ptr->u.cdma_message.address.digit_mode = WMS_DIGIT_MODE_4_BIT;
          cdma_qsh_ptr->u.cdma_message.address.number_mode = WMS_NUMBER_MODE_NONE_DATA_NETWORK;
          cdma_qsh_ptr->u.cdma_message.address.number_type = WMS_NUMBER_INTERNATIONAL;
          cdma_qsh_ptr->u.cdma_message.address.number_plan = WMS_NUMBER_PLAN_TELEPHONY;

          cdma_qsh_ptr->u.cdma_message.address.number_of_digits = 12;
          cdma_qsh_ptr->u.cdma_message.address.digits[0] = 9;
          cdma_qsh_ptr->u.cdma_message.address.digits[1] = 1;
          cdma_qsh_ptr->u.cdma_message.address.digits[2] = 9;
          cdma_qsh_ptr->u.cdma_message.address.digits[3] = 9;
          cdma_qsh_ptr->u.cdma_message.address.digits[4] = 0;
          cdma_qsh_ptr->u.cdma_message.address.digits[5] = 1;
          cdma_qsh_ptr->u.cdma_message.address.digits[6] = 2;
          cdma_qsh_ptr->u.cdma_message.address.digits[7] = 3;
          cdma_qsh_ptr->u.cdma_message.address.digits[8] = 4;
          cdma_qsh_ptr->u.cdma_message.address.digits[9] = 5;
          cdma_qsh_ptr->u.cdma_message.address.digits[10] = 6;

          
          cdma_qsh_ptr->u.cdma_message.is_tl_ack_requested = FALSE; 
           
          cdma_qsh_ptr->u.cdma_message.subaddress.number_of_digits = 0; //Subaddress not present
            
          cdma_qsh_ptr->u.cdma_message.is_service_present = FALSE;
          cdma_qsh_ptr->u.cdma_message.service = WMS_SRV_UNKOWN;

          cdma_qsh_ptr->u.cdma_message.raw_ts.format = WMS_FORMAT_CDMA;
          cdma_qsh_ptr->u.cdma_message.raw_ts.tpdu_type = WMS_TPDU_SUBMIT;
          cdma_qsh_ptr->u.cdma_message.raw_ts.len  = 2;
          for (i = 0; i < 2; i++)
          {
              cdma_qsh_ptr->u.cdma_message.raw_ts.data[i] = raw_data[i];
          } 
        ...
        if(cdma_qsh_ptr != NULL)
       {
         wms_mem_free(cdma_qsh_ptr);
       }
...
)
{
  ...
  if((evt >= (byte)WMS_QSH_EVENT_MAX) || 
     (sub_id >= MAX_AS_IDS) || 
     (action > 1))
  {
    MSG_ERROR_3("Invalid QSH configuration for Event %d SUB ID %d action %d", evt, sub_id, action);
    return;
  }

  cfg_s_ptr->wms_qsh_event_enabled[sub_id][evt] = (boolean)(action ^ 0x01); /* 0 is enable, 1 is disable */

}


void wms_qsh_debug_callback
(
  qsh_client_cb_params_s *param
)
{
...
)
{
  ...
  client_reg.client = QSH_CLT_WMS;
  client_reg.client_cb_ptr = wms_qsh_debug_callback;
  client_reg.major_ver = WMS_QSH_MAJOR_VER;   //add   /mmcp/api/wms_qsh_ext.h
  client_reg.minor_ver = WMS_QSH_MINOR_VER;
  ...
  client_reg.cb_action_support_mask = 
     (client_reg.cb_action_support_mask | QSH_ACTION_EVENT_CFG | QSH_ACTION_CFG);
	 

    /* Enable all QSH NAS events by default */
  for(sub_id = SYS_MODEM_AS_ID_1; sub_id < MAX_AS_IDS; sub_id++)
    for(idx = 0; idx < WMS_QSH_EVENT_MAX; idx++)
      cfg_s_ptr->wms_qsh_event_enabled[sub_id][idx] = TRUE;
...
)
{
  ...
  else
  {
     ...
     if(wms_cmd_avail_cnt == 0)
     {
        wms_qsh_client_event_log(WMS_QSH_EVENT_CMD_QUEUE_FULL,0, 0);
     }
#endif
  }
...
boolean wms_check_cmd_is_valid_without_appInfo_ptr(
...
)
{
  ...
  return ret_val;
} /* wms_check_cmd_is_valid_without_appInfo_ptr() */


boolean wms_check_cmd_is_valid_during_sim_busy(
  wms_cmd_type *cmd_ptr
)
{
  boolean ret_val = TRUE;
  switch(cmd_ptr->hdr.cmd_id)
  {
    ...
    case WMS_CMD_CFG_SET_MEMORY_FULL:
      ...
      {
        MSG_ERROR_1( "Command not allowed during SIM_BUSY, cmd_id: %d", cmd_ptr->hdr.cmd_id );
        ret_val = FALSE;
      }
      break;
  ...
  return ret_val;
} /* wms_check_cmd_is_valid_while_sim_busy_event() */

#ifdef FEATURE_GSTK
boolean wms_check_cmd_is_valid_during_sim_refresh(
  wms_cmd_type *cmd_ptr
)
{
  boolean ret_val = TRUE;
    ...
    default:
      {
        ...
        MSG_ERROR_1( "Command not allowed during refresh, cmd_id: %d", cmd_ptr->hdr.cmd_id );
        ret_val = FALSE;
      }
      break;
  }
  ...
  return ret_val;
} /* wms_check_cmd_is_valid_during_sim_refresh() */
#endif /*FEATURE_GSTK */


wms_cmd_err_e_type wms_check_cmd_is_valid_without_cdma_sim_init_complete(
  wms_cmd_type *cmd_ptr
)
{
  ...
  switch(cmd_ptr->hdr.cmd_id)
  {
    ...
    case WMS_CMD_CFG_GET_MEMORY_STATUS:
      switch (cmd_ptr->cmd.cfg_get_mem_status.mem_store)
      {
        ...
        case WMS_MEMORY_STORE_RUIM:
          if (WMS_TAG_MO_TEMPLATE == cmd_ptr->cmd.cfg_get_mem_status.tag)
          {
            if (!cmd_ptr->hdr.appInfo_ptr->sim_file_init_is_done[SMSP_FILE_INDEX])
            {
    ...
    case WMS_CMD_CFG_GET_MESSAGE_LIST:
      switch (cmd_ptr->cmd.cfg_get_msg_list.mem_store)
      {
        case WMS_MEMORY_STORE_CSIM:
        case WMS_MEMORY_STORE_RUIM:
          if (WMS_TAG_MO_TEMPLATE == cmd_ptr->cmd.cfg_get_msg_list.tag)
          {
            if (!cmd_ptr->hdr.appInfo_ptr->sim_file_init_is_done[SMSP_FILE_INDEX])
            {
              ret_cmd_err = WMS_CMD_ERR_BUSY;
    ...
    case WMS_CMD_MSG_SEND:
      ...
      {
        if (FALSE == cmd_ptr->hdr.appInfo_ptr->sim_init_limited)
        {
          ret_cmd_err = WMS_CMD_ERR_BUSY;
        }
        /* Special handling for MO SMS over TCH during WMS Limited ready */
        else if (WMS_CLIENT_CH_PREF_TCH_ONLY == cmd_ptr->cmd.msg_send.ch_pref)
        {
           ret_cmd_err = WMS_CMD_ERR_BUSY;
        }
        else if (WMS_SEND_MODE_MEMORY_STORE == cmd_ptr->cmd.msg_send.send_mode)
        {
    ...
    case WMS_CMD_MSG_WRITE:
      ...
      {
        switch (cmd_ptr->cmd.msg_write.message.msg_hdr.mem_store)
        {
          case WMS_MEMORY_STORE_CSIM:
          case WMS_MEMORY_STORE_RUIM:
            if (!wms_sim_mmgsdi_init_done( cmd_ptr->hdr.appInfo_ptr ))
    ...
    case WMS_CMD_MSG_WRITE_TEMPLATE:
      ...
      {
        switch (cmd_ptr->cmd.msg_write_template.message.msg_hdr.mem_store)
        {
          case WMS_MEMORY_STORE_CSIM:
          case WMS_MEMORY_STORE_RUIM:
            if (!cmd_ptr->hdr.appInfo_ptr->sim_file_init_is_done[SMSP_FILE_INDEX])
  ...
  return ret_cmd_err;
} /* wms_check_cmd_is_valid_without_cdma_sim_init_complete() */

wms_cmd_err_e_type wms_check_cmd_is_valid_without_gw_sim_init_complete(
  wms_cmd_type *cmd_ptr
)
{
  wms_cmd_err_e_type ret_cmd_err = WMS_CMD_ERR_NONE;
  ...
  return ret_cmd_err;
} /* wms_check_cmd_is_valid_without_gw_sim_init_complete() */
...
)
{
  ...
  // Commands allowed without appInfo:
  if (FALSE == wms_check_cmd_is_valid_without_appInfo_ptr( cmd_ptr ))
  {
    if (NULL == cmd_ptr->hdr.appInfo_ptr)
    {
      // Any command not in the list is rejected
      MSG_ERROR_1( "Command not allowed without appInfo_ptr, cmd_id: %d", cmd_ptr->hdr.cmd_id );
  ...
  // Commands that are allowed during SIM_BUSY event:
  if (FALSE == wms_check_cmd_is_valid_during_sim_busy( cmd_ptr ))
  {
    ret_cmd_err = WMS_CMD_ERR_BUSY;
    return ret_cmd_err;
  }
  ...
  {
    if (FALSE == wms_check_cmd_is_valid_during_sim_refresh( cmd_ptr ))
    {
      ret_cmd_err = WMS_CMD_ERR_BUSY;
    }
  }
  ...
  if (((MMGSDI_1X_PROV_PRI_SESSION == cmd_ptr->hdr.appInfo_ptr->mmgsdi_appInfo.session_type)||   //1x
       (MMGSDI_1X_PROV_SEC_SESSION == cmd_ptr->hdr.appInfo_ptr->mmgsdi_appInfo.session_type)) &&
       (!cmd_ptr->hdr.appInfo_ptr->sim_init_complete))
  {
    
    ret_cmd_err = wms_check_cmd_is_valid_without_cdma_sim_init_complete( cmd_ptr );
    if (WMS_CMD_ERR_NONE != ret_cmd_err)
    {
      MSG_ERROR_3( "Command not allowed without 1X sim_init_complete, sim_init_limited: %d, cmd_id: %d , as_id: %d", 
                   cmd_ptr->hdr.appInfo_ptr->sim_init_limited,
                   cmd_ptr->hdr.cmd_id ,
                   cmd_ptr->hdr.appInfo_ptr->asid );  //1x
    }
  ...
  if (( (MMGSDI_GW_PROV_PRI_SESSION == cmd_ptr->hdr.appInfo_ptr->mmgsdi_appInfo.session_type) ||
        ...
        (!cmd_ptr->hdr.appInfo_ptr->sim_init_complete))
  {
    ret_cmd_err = wms_check_cmd_is_valid_without_gw_sim_init_complete( cmd_ptr );
    if (WMS_CMD_ERR_NONE != ret_cmd_err)
    {
      MSG_ERROR_3( "Command not allowed without GW sim_init_complete, sim_init_limited: %d, cmd_id: %d, as_id: %d", 
                   cmd_ptr->hdr.appInfo_ptr->sim_init_limited,
                   cmd_ptr->hdr.cmd_id,
...
)
{
  ...
  do
  {
    ...
    {
      MSG_ERROR_2("Unregistered or non-active client (client %d, client_ptr 0x%08x)",
                   cmd_ptr->hdr.client_id,
                   client_ptr );
...
)
{
  ...
  if (0 != (sigs & TASK_STOP_SIG))
  {
...
#ifdef FEATURE_SMS_PWS_OVER_LTE
     (void) wms_msgr_deregister( MSGR_WMS_LTE,
                                 &wms_msgr_client,
     ...
     (void) wms_msgr_deregister( MSGR_WMS_POLICYMAN,
                                 &wms_msgr_client,
...
)
{
  ...
  do
  {
    ...
    MSG_HIGH_4( "In wms_msg_ms_send_common(client %d, as_id %d, send_mode %d, message_mode %d)",
              client_id,
              as_id,
              send_mode,
...
)
{
  ...
  do
  {
    ...
    //Getting app ptr if ack received for 3GPP2 MT SMS received over IMS for CDMA less device
    if (NULL == appInfo_ptr)
    {
      if ((transport_type == WMS_MSG_TRANSPORT_TYPE_SMS_OVER_IMS) && (ack_info_ptr->message_mode == WMS_MESSAGE_MODE_CDMA))
      {
...
)
{
  ...
  do
  {
    ...
    sys_modem_as_id_e_type  as_id = SYS_MODEM_AS_ID_1;
    wms_prov_app_info_s_type *appInfo_ptr = wms_get_lte_appinfo();
...
)
{
  ...
  if (NULL != cmd_ptr)
  {
    ...
    MSG_HIGH_3( "SEND_SIM_EFS_READ_EVT requested for session_type %d, t_id %d, appInfo_ptr 0x%x",
...
)
{
  ...
  do
  {
    ...
    appInfo_ptr = wms_get_appinfo_by_asid_and_message_mode(wms_check_1x_active_sub(), WMS_MESSAGE_MODE_CDMA);  //1x
    ...
    cmd_ptr->hdr.client_id   = client_id;
    cmd_ptr->hdr.cmd_cb      = cmd_cb;
    cmd_ptr->hdr.as_id       = wms_check_1x_active_sub();  //1x
    cmd_ptr->hdr.appInfo_ptr = appInfo_ptr;
    cmd_ptr->hdr.user_data   = (void *)user_data;
...
)
{
  ...
  MSG_HIGH_3( "In wms_dc_connect(client %d, so %d,as_id : %d)",
              client_id,
              so,
              as_id);
  ...
  do
  {
    ...
    appInfo_ptr = wms_get_appinfo_by_asid_and_message_mode(as_id, WMS_MESSAGE_MODE_CDMA); 
...
)
{
  ...
  do
  {
    ...
    cmd_ptr->hdr.client_id   = client_id;
    cmd_ptr->hdr.cmd_cb      = cmd_cb;
    cmd_ptr->hdr.as_id       = wms_check_1x_active_sub();
    cmd_ptr->hdr.appInfo_ptr = appInfo_ptr;
    cmd_ptr->hdr.user_data   = (void *)user_data;
...
/*=========================================================================
FUNCTION wms_dbg_ms_get_retry_interval
...
SIDE EFFECTS
=========================================================================*/
wms_status_e_type wms_dbg_ms_get_retry_interval(
  wms_client_id_type               client_id,
  wms_cmd_cb_type                  cmd_cb,
  const void                      *user_data,
  sys_modem_as_id_e_type           as_id
)
{
  ...
  MSG_HIGH_2("In wms_dbg_ms_get_retry_interval(client %d, as_id %d)", client_id, as_id);
...
/*=========================================================================
FUNCTION wms_dbg_ms_set_retry_interval
...
SIDE EFFECTS
=========================================================================*/
wms_status_e_type wms_dbg_ms_set_retry_interval(
  wms_client_id_type               client_id,
  wms_cmd_cb_type                  cmd_cb,
  const void                      *user_data,
...
)
{
  ...
  MSG_HIGH_3( "In wms_dbg_ms_set_retry_interval(client %d, retry_interval %d, as_id %d)",
              client_id,
              retry_interval,
              as_id );
...
/*=========================================================================
FUNCTION wms_dbg_ms_get_retry_period
...
SIDE EFFECTS
=========================================================================*/
wms_status_e_type wms_dbg_ms_get_retry_period(
  wms_client_id_type               client_id,
  wms_cmd_cb_type                  cmd_cb,
  const void                      *user_data,
  sys_modem_as_id_e_type           as_id
)
{
  ...
  MSG_HIGH_2("In wms_dbg_ms_get_retry_period(client %d, as_id %d)", client_id, as_id);
...
/*=========================================================================
FUNCTION wms_dbg_ms_set_retry_period
...
SIDE EFFECTS
=========================================================================*/
wms_status_e_type wms_dbg_ms_set_retry_period(
  wms_client_id_type               client_id,
  wms_cmd_cb_type                  cmd_cb,
  const void                      *user_data,
...
)
{
  ...
  MSG_HIGH_3( "In wms_dbg_ms_set_retry_period(client %d, retry_period %d, as_id %d)",
              client_id,
              retry_period,
              as_id );
...
)
{
  ...
  else if (NULL != transport_ptr && NULL != item)
  {
    ...
    case WMS_MSG_TRANSPORT_UPDATE_F_TIMER:
      {
        ...
        if (transport_id <  WMS_MSG_TRANSPORT_ARR_MAX)
        {
           msg_s_ptr->timerf_value[transport_id] = transport_ims_f_timer;
        ...
        {
          MSG_ERROR_1("Incorrect transport id %d found", transport_id);
          st = WMS_GENERAL_ERROR_S;
        }
      }
      break;
    ...
    case WMS_MSG_TRANSPORT_UPDATE_SMS_RETRY_STATUS:
      {
        wms_msg_transport_update_sms_retry_status_cb_type sms_retry_st_cb;
        memscpy( &sms_retry_st_cb, sizeof(wms_msg_transport_update_sms_retry_status_cb_type), item, item_len );
        st = wms_msg_transport_ms_reg_sms_retry_status_cb( transport_id, sms_retry_st_cb, as_id );
      }
      break;
#endif

    case WMS_MSG_TRANSPORT_UPDATE_RAT:
      {        
        st = WMS_OK_S;
      }
      break; 
...
/*=========================================================================
FUNCTION wms_get_source_addr_from_rpdu

DESCRIPTION
  This function is used to to extract source number from RPDU.
  It helps IMS to start timers at IMS before sending MT SMS to WMS

DEPENDENCIES
  None

RETURN VALUE
  FALSE: In case of protocol error or wrong pdu type
  TRUE:  If WMS succesfully decoded source number from RPDU

SIDE EFFECTS
  None

=========================================================================*/
boolean wms_get_source_addr_from_rpdu(wms_ota_msg_s_type *mt_sms,wms_address_s_type *source_no)
{
  byte                      *rp_data;
  byte                      rpdu_len;
  byte                      len, index         = 0;
  wms_raw_ts_data_s_type    raw_ts             = {0};
  wms_status_e_type         st                 = WMS_OK_S;
  wms_client_ts_data_s_type wms_msg_Gclient_ts = {0};

  MSG_HIGH_0( "In wms_get_source_number_from_rpdu ");

  if (mt_sms == NULL || source_no == NULL)
  {
    return FALSE;
  }

  rp_data = mt_sms->data;
  rpdu_len = (byte) MIN(mt_sms->data_len, WMS_MAX_LEN);

  if (mt_sms->format != WMS_FORMAT_GW_PP)
  {
    MSG_ERROR_1("Invalid format: %d", mt_sms->format);
    return FALSE;
  }
  MSG_HIGH_2("rp data %d %d", rp_data[0]&0x07, RP_DATA_N);
  if ((rp_data[0] & 0x07) != RP_DATA_N)
  {
    MSG_ERROR_1("Invalid message type: %d", rp_data[0]);
    return FALSE;
  }
  index = index + 2;       //skipping msg type and msg ref

  if (index >= rpdu_len)   //safety check
  {
    MSG_ERROR_1("index greater than length: %d", rpdu_len);
    return FALSE;
  }
  len = rp_data[index];       //lenght of originating address (SMSC addr)
  if (len < 2 || len > 11)
  {
    MSG_ERROR_1("originating address length mismatch: %d", len);
    return FALSE;
  }
  index = index + 1 + len; // skipping addr len and addr digits
  ...
  len = rp_data[index];       //RP dest length

  if (len != 0)            //For RP data DEST length must be set to zero
  {
    MSG_ERROR_1("RP dest len not zero: %d", len);
    return FALSE;
  }
  index = index + 1;      //skipping dest len
  ...
  if ((rp_data[index+1] & 0x03) != 0)    //not deliver
  {
    MSG_ERROR_0("message type is not deliver");
    return FALSE;
  }

  //got tpdu index
  raw_ts.format = WMS_FORMAT_GW_PP;
  raw_ts.len    = rp_data[index];
  raw_ts.tpdu_type = WMS_TPDU_DELIVER;
  memscpy(raw_ts.data, raw_ts.len, &rp_data[index+1], raw_ts.len);

  st = wms_ts_decode ( &raw_ts, &wms_msg_Gclient_ts);

  if (st != WMS_OK_S)
  {
    MSG_ERROR_0("decode failed");
    return FALSE;
  }

  memscpy(source_no, sizeof(wms_address_s_type), &wms_msg_Gclient_ts.u.gw_pp.u.deliver.address, sizeof(wms_address_s_type));

  MSG_HIGH_8("digits : %d addr %d %d %d %d %d %d %d", source_no->number_of_digits, source_no->digits[0],
  	source_no->digits[1],
  	source_no->digits[2],
  	source_no->digits[3],
  	source_no->digits[4],
