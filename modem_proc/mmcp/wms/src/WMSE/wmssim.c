/*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/wms/src/WMSE/wmssim.c#1 $
...
===========================================================================*/
extern void nvruim_init_wms_svc_items(boolean has_bcsms, boolean has_smscap, nvruim_session_enum_type nvruim_session);
...
/*=========================================================================
FUNCTION wms_sim_do_limited_subs_ready()                                 (INTERNAL)

DESCRIPTION
  Sends the CDMA or GW ready events if both 'sim_init_limited' and
  'is_available' are TRUE for a subscription.
...
SIDE EFFECTS
  None
=========================================================================*/
void wms_sim_do_limited_subs_ready(
  wms_prov_app_info_s_type    *appInfo_ptr
)
{
  MSG_HIGH_0("wms_sim_do_limited_subs_ready");
  if (NULL != appInfo_ptr)
  {
    if (appInfo_ptr->sim_init_limited && appInfo_ptr->is_available)
    {
      wms_cfg_send_limited_ready_event( appInfo_ptr, WMS_CLIENT_ALL );
    }
    else
    {
      // Log the attempt and indicate what is going on
      MSG_HIGH_3( "wms_sim_do_limited_subs_ready: appInfo_ptr 0x%x, sim_init_complete %d, is_available %d",
                  appInfo_ptr,
                  appInfo_ptr->sim_init_limited,
                  appInfo_ptr->is_available );
    }
  }

  return;
} /* wms_sim_do_limited_subs_ready() */
...
)
{
  ...
  case MMGSDI_FILE_UPDATE_EVT:
    ...
    else if (event->session_id == lcfg_s_ptr->wms_mmgsdi_app4_info.session_id)
    {
      if (NULL == lcfg_s_ptr->wms_app4_info)
      {
        lcfg_s_ptr->wms_app4_info = wms_sim_alloc_appInfo_ptr(&lcfg_s_ptr->wms_mmgsdi_app4_info);
      }
      appInfo_ptr = lcfg_s_ptr->wms_app4_info;
    }
...
)
{
  ...
  else if (NULL != lcfg_s_ptr->wms_app4_info &&                 
           lcfg_s_ptr->wms_app4_info->lte_is_available )
  {
    return lcfg_s_ptr->wms_app4_info;
  }
...
wms_prov_app_info_s_type *wms_get_lte_appinfo_by_as_id(
...
)
{
  ...
  if (NULL != lcfg_s_ptr->wms_app0_info &&
  	  (lcfg_s_ptr->wms_app0_info->asid == as_id)&&
	   lcfg_s_ptr->wms_app0_info->lte_is_available)
   {
	 return lcfg_s_ptr->wms_app0_info;
   }
   else if (NULL != lcfg_s_ptr->wms_app1_info &&
   	        (lcfg_s_ptr->wms_app1_info->asid == as_id)&&
			lcfg_s_ptr->wms_app1_info->lte_is_available )
   {
	 return lcfg_s_ptr->wms_app1_info;
   }
   else if (NULL != lcfg_s_ptr->wms_app2_info &&
   	        (lcfg_s_ptr->wms_app2_info->asid == as_id)&&
			lcfg_s_ptr->wms_app2_info->lte_is_available )
   {
	 return lcfg_s_ptr->wms_app2_info;
   }
   else if (NULL != lcfg_s_ptr->wms_app3_info &&
   	        (lcfg_s_ptr->wms_app3_info->asid == as_id)&&
			lcfg_s_ptr->wms_app3_info->lte_is_available )
   {
	 return lcfg_s_ptr->wms_app3_info;
   }
   else if (NULL != lcfg_s_ptr->wms_app4_info &&
   	        (lcfg_s_ptr->wms_app4_info->asid == as_id)&&
			lcfg_s_ptr->wms_app4_info->lte_is_available )
   {
	 return lcfg_s_ptr->wms_app4_info;
   }
...
)
{
  ...
  case MMGSDI_1X_PROV_SEC_SESSION:
    info_ptr = &lcfg_s_ptr->wms_mmgsdi_app4_info;
    break;
...
)
{
  ...
  case MMGSDI_1X_PROV_SEC_SESSION:
    info_ptr = lcfg_s_ptr->wms_app4_info;
    break;
...
)
{
  ...
  if (NULL != lcfg_s_ptr->wms_app4_info &&
      session_id == lcfg_s_ptr->wms_app4_info->mmgsdi_appInfo.session_id)  
  {
    info_ptr = lcfg_s_ptr->wms_app4_info;
  }
...
)
{
  ...
  do
  {
    ...
    {
      if (NULL != lcfg_s_ptr->wms_app0_info &&
          asid == lcfg_s_ptr->wms_app0_info->asid)
      {
        info_ptr = lcfg_s_ptr->wms_app0_info;
      }

      if (NULL != lcfg_s_ptr->wms_app4_info &&
          asid == lcfg_s_ptr->wms_app4_info->asid)
      {
        info_ptr = lcfg_s_ptr->wms_app4_info;
      }
  ...
  case WMS_MEMORY_STORE_CSIM:
    {
      ...
      { // Mem stores for CDMA
        ...
        // Check if 1X_PRI is mapped to this asid
        if (NULL != lcfg_s_ptr->wms_app0_info &&
            asid == lcfg_s_ptr->wms_app0_info->asid)
        {
          info_ptr = lcfg_s_ptr->wms_app0_info;
        ...
        // Check if 1X_TER is mapped to this asid
        if (NULL != lcfg_s_ptr->wms_app4_info &&
            asid == lcfg_s_ptr->wms_app4_info->asid)
        {
          info_ptr = lcfg_s_ptr->wms_app4_info;
...
sys_modem_as_id_e_type wms_check_1x_active_sub(   
  void
)
{
   wms_cfg_s_type  *lcfg_s_ptr = wms_cfg_s_ptr();
   ...
   if ((NULL != lcfg_s_ptr->wms_app0_info) &&
      (((lcfg_s_ptr->wms_app0_info->subs_capability) & SYS_SYS_MODE_MASK_CDMA)||
      ((lcfg_s_ptr->wms_app0_info->subs_capability) & SYS_SYS_MODE_MASK_HDR)))
   {
	 MSG_HIGH_0("1X active on SUB0 :SYS_MODEM_AS_ID_1");
     return SYS_MODEM_AS_ID_1;
   }

   if ((NULL != lcfg_s_ptr->wms_app4_info) &&
      (((lcfg_s_ptr->wms_app4_info->subs_capability) & SYS_SYS_MODE_MASK_CDMA)||
      ((lcfg_s_ptr->wms_app4_info->subs_capability) & SYS_SYS_MODE_MASK_HDR)))
   {
	 MSG_HIGH_0("1X active on SUB1:SYS_MODEM_AS_ID_2 ");
     return SYS_MODEM_AS_ID_2;
   }

   MSG_ERROR_0("Could not get 1X appInfo");
...
wms_prov_app_info_s_type * wms_get_1X_appinfo_by_asid(
  sys_modem_as_id_e_type    asid
)
{
  wms_cfg_s_type            *lcfg_s_ptr = wms_cfg_s_ptr();
  ...
  switch (asid)
  {
    case SYS_MODEM_AS_ID_1:
      if (NULL == lcfg_s_ptr->wms_app0_info)
      {
        lcfg_s_ptr->wms_app0_info = 
          wms_sim_alloc_appInfo_ptr(&lcfg_s_ptr->wms_mmgsdi_app0_info);
      }
      appInfo_ptr = lcfg_s_ptr->wms_app0_info;
      break;
    ...
    case SYS_MODEM_AS_ID_2:
      if (NULL == lcfg_s_ptr->wms_app4_info)
      {
        lcfg_s_ptr->wms_app4_info = 
          wms_sim_alloc_appInfo_ptr(&lcfg_s_ptr->wms_mmgsdi_app4_info);
      }
      appInfo_ptr = lcfg_s_ptr->wms_app4_info;
      break;
...
)
{
   if (NULL != appInfo_ptr)
   {
      ...
      if ( ( (MMGSDI_APP_RUIM == appInfo_ptr->mmgsdi_appInfo.app_type) ||
           ...
           wms_nv_rtre_control_is_nv((appInfo_ptr->mmgsdi_appInfo.slot_id == MMGSDI_SLOT_1)? 0 : 1) ) 
      {
...
)
{
  ...
  {    
    for(i = 0; i < MAX_FILE_INDEX; i++)
    {
      ...
      appInfo_ptr->sim_file_init_started[i] = inited;   
    }	
  }
...
None
===========================================================================*/
mmgsdi_rec_num_type wms_sim_get_next_used_record(
  wms_prov_app_info_s_type   *appInfo_ptr,
  mmgsdi_rec_num_type         next_index   
)
{
  mmgsdi_rec_num_type num_records = appInfo_ptr->wms_seek_data.num_of_records;  
...
boolean wms_sim_mmgsdi_file_is_transparent(uint8 file_enum_index)
{
  boolean ret_val = FALSE;

  switch(file_enum_index)
  {
    case SMSS_FILE_INDEX:
    case CBMI_FILE_INDEX:
    case CBMIR_FILE_INDEX:
...
)
{
  ...
  else if (FALSE == appInfo_ptr->sim_init_limited && 
           TRUE  == wms_sim_mmgsdi_limited_init_done( appInfo_ptr ))
  {
    appInfo_ptr->sim_init_limited = TRUE;
    wms_sim_do_limited_subs_ready(appInfo_ptr);
    wms_msg_send_smsc_address_change_ind( appInfo_ptr );
  }
  
  next_file_enum = wms_sim_map_cache_index_to_file_enum(next_file_enum_index, appInfo_ptr);

  if (wms_sim_mmgsdi_file_is_transparent( next_file_enum_index ))
  {
    ...
    ret = wms_sim_mmgsdi_read_transparent( appInfo_ptr,
                                           next_file_enum,
                                           bytes_to_read );

    if (MMGSDI_SUCCESS == ret)
    {
       appInfo_ptr->sim_file_init_started[next_file_enum_index] = TRUE;
       MSG_HIGH_1(" EF sim init started for enum index %d", next_file_enum_index);
    }
  }
  else
  {
    /* read next file */
    ret = wms_sim_mmgsdi_get_file_attr( appInfo_ptr,
                                        next_file_enum );
    if (MMGSDI_SUCCESS == ret)
    {
       appInfo_ptr->sim_file_init_started[next_file_enum_index] = TRUE;
...
)
{
   ...
   if(appInfo_ptr->mmgsdi_appInfo.session_type == MMGSDI_1X_PROV_PRI_SESSION)
   {
     as_id = SYS_MODEM_AS_ID_1;
   }
   else if(appInfo_ptr->mmgsdi_appInfo.session_type == MMGSDI_1X_PROV_SEC_SESSION)
   {
   ...
   {  
...
#ifndef FEATURE_GWSMS_BROADCAST
      ...
      /* done with reading services - now read  sms files */
   ...
   case MMGSDI_CSIM_SRV_3GPD_EXT:
    MSG_HIGH_4( "BCSMS Enabled: %d, SMSCap Enabled: %d, PP Download Enabled: %d, CB Download Enabled: %d", 
     ...
     nvruim_init_wms_svc_items( appInfo_ptr->bcsms_enabled,
                                appInfo_ptr->smscap_enabled,
                                ((appInfo_ptr->mmgsdi_appInfo.session_type == MMGSDI_1X_PROV_PRI_SESSION)? NVRUIM_1X_PROV_PRI_SESSION : NVRUIM_1X_PROV_SEC_SESSION) 
                              );

     if (appInfo_ptr->smscap_enabled)
     {
        MSG_HIGH_0("SMSCAP is enabled");
        wms_nv_init_nvruim(as_id);
...
#ifdef FEATURE_CDSMS
     {
        ...
        if( NV_DONE_S == wms_nv_read_wait_per_subs(as_id,NV_ESN_I, & nvi ) )
        {
   ...
   default:
      MSG_ERROR_1("Unexpected srv type %d", srv_type);
      break;
   }
...
)
{
  ...
  mmgsdi_rec_num_type          next_record_num = 0;  
  int                          i;
  mmgsdi_return_enum_type      ret;
  mmgsdi_cnf_enum_type         cnf;
  ...
  MSG_HIGH_2("In wms_sim_mmgsdi_seek_cb_proc(): handle cnf = %d, file_enum = %d", cnf, cnf_ptr->search_cnf.access.file.file_enum);
  ...
  if (mapped_cache_index == MAX_FILE_INDEX)
  {
    MSG_ERROR_0("Incorrect file enum");
    return;
  }
  
  if (appInfo_ptr->sim_file_init_started[mapped_cache_index] == FALSE)
  {
     MSG_ERROR_0("Dropping stale seek cnf");
     return;
  }
  ...
  else
  {
    ...
    {
      appInfo_ptr->sim_file_init_started[mapped_cache_index] = FALSE;
...
)
{
       ...
       (MMGSDI_CSIM_SMS_STATUS == appInfo_ptr->smss_file_enum) )
  {
    ...
    appInfo_ptr->sim_file_init_started[SMSR_FILE_INDEX]  = FALSE; 
    // There is no EF_mwis in CSIM and RUIM
    appInfo_ptr->sim_file_init_is_done[MWIS_FILE_INDEX]  = TRUE;
    appInfo_ptr->sim_file_init_started[MWIS_FILE_INDEX]  = FALSE;
  }
  ...
  case MMGSDI_REFRESH_STAGE_WAIT_FOR_OK_TO_FCN:
    {
      ...
      // Always vote TRUE for refresh if Refresh mode is RESET mode and source is ECALL.
      if ( ( MMGSDI_REFRESH_RESET        == event->data.refresh.mode ) && 
           ( MMGSDI_REFRESH_SOURCE_ECALL == event->data.refresh.source ) )
      {
        ok_to_refresh = TRUE;
      }
      else
      {
        ...
        boolean                 nv_refresh_vote_ok  = TRUE;

        // Fetch the WMS NV item 'refresh_vote_ok'
        ...
        MSG_HIGH_1("refresh: Pending MO message count: %d", pending_mo_messages);
          ...
          // Check NV item setting. The behavior is as follows:
         //  If NV is inactive or TRUE: default behavior
        //   Otherwise (FALSE): vote based on WMS busy

        if (item_is_active && !nv_refresh_vote_ok)
        {
          if (!wms_is_idle)
          {
            // No refresh if WMS is busy
            ok_to_refresh = FALSE;
          }
        }
      }
  ...
  case MMGSDI_REFRESH_STAGE_START:
    {
      ...
      appInfo_ptr->sim_init_limited = FALSE;
...
)
{
  ...
  if (NULL != appInfo_ptr)
  {
    ...
    appInfo_ptr->sim_file_init_started[CBMI_FILE_INDEX]  = FALSE;

    appInfo_ptr->sim_file_init_is_done[CBMIR_FILE_INDEX] = FALSE;
    appInfo_ptr->sim_file_init_started[CBMIR_FILE_INDEX] = FALSE;

    appInfo_ptr->sim_file_init_is_done[CBMID_FILE_INDEX] = FALSE;
    appInfo_ptr->sim_file_init_started[CBMID_FILE_INDEX] = FALSE;

    appInfo_ptr->sim_file_init_is_done[SMSS_FILE_INDEX] = FALSE;
    appInfo_ptr->sim_file_init_started[SMSS_FILE_INDEX] = FALSE;
  
    // Clear the cache corresponding to EF_SMSP.
    for (index = 0; index < (uint8)appInfo_ptr->smsp_rec_count; index++) 
    {
    ...
    appInfo_ptr->sim_file_init_started[SMSP_FILE_INDEX] = FALSE;

    // Clear the cache corresponding to EF_SMS.
    for (index = 0; index < (uint8)appInfo_ptr->sms_rec_count; index++)  
    {
    ...
    appInfo_ptr->sim_file_init_started[SMS_FILE_INDEX]  = FALSE;    
    appInfo_ptr->voice_mail_active                      = FALSE;
    appInfo_ptr->voice_mail_index                       = (wms_message_index_type)WMS_DUMMY_MESSAGE_INDEX;

#ifdef FEATURE_GWSMS
    // Clear the cache corresponding to EF_SMSR.
    for (index = 0; index < (uint8)appInfo_ptr->smsr_rec_count; index++)  
    {
...
)
{
  ...
  MSG_HIGH_2("In wms_sim_mmgsdi_event_cb_proc(): process event = %d, appInfo_ptr = 0x%x", event->evt, appInfo_ptr);
  ...
  case MMGSDI_SUBSCRIPTION_READY_EVT:
    {
           ...
           (MMGSDI_APP_CSIM == appInfo_ptr->mmgsdi_appInfo.app_type) )
      {
        ...
        if (wms_nv_rtre_control_is_nv((appInfo_ptr->mmgsdi_appInfo.slot_id == MMGSDI_SLOT_1)? 0 : 1))
        {
          // 1X is in NV mode
  ...
  case MMGSDI_SESSION_CHANGED_EVT:
    {
      MSG_HIGH_6( "SESSION_CHANGED_EVT: appInfo_ptr 0x%x, as_id %d, session_id 0x%x: session_activated %d->%d, subs_capability : %d",
                  appInfo_ptr,
                  appInfo_ptr->asid,
                  event->session_id,
                  appInfo_ptr->session_activated,
                  event->data.session_changed.activated,
                  appInfo_ptr->subs_capability);
      ...
      if (!appInfo_ptr->session_activated)
      {
         ...
         appInfo_ptr->sim_init_complete = FALSE;
         appInfo_ptr->sim_init_limited = FALSE;
         appInfo_ptr->subs_capability  = SYS_SYS_MODE_MASK_NONE; 
  ...
  case MMGSDI_SIM_BUSY_EVT:
     {
        ...
        case MMGSDI_SLOT_2:
          ...
          wms_sim_sim_busy_proc(lcfg_s_ptr->wms_app4_info, event->data.sim_busy.sim_busy);
          break;
...
)
{
  if (NULL != appInfo_ptr)
  {
    ...
    else
    {
      ...
      appInfo_ptr->bc.gw.sim_cbmi_rec_len   = MIN((uint32)data_len,len);
      appInfo_ptr->bc.gw.sim_cbmi_list_size = valid_entry_count;

      MSG_HIGH_4( "EFcbmi asid %d: receivedlength %d,appinfo  length:%d, valid entries %d",
                  appInfo_ptr->asid,
                  data_len,
                  appInfo_ptr->bc.gw.sim_cbmi_rec_len,
                  valid_entry_count );
...
)
{
  if (NULL != appInfo_ptr)
  {
    ...
    else
    {
      ...
      appInfo_ptr->bc.gw.sim_cbmir_rec_len   = MIN((uint32)data_len,len);
      appInfo_ptr->bc.gw.sim_cbmir_list_size = valid_entry_count;

      MSG_HIGH_4( "EFcbmir asid %d: received length %d, valid entries %d,appinfo_length: %d ",
                  appInfo_ptr->asid,
                  data_len,
                  valid_entry_count,
                  appInfo_ptr->bc.gw.sim_cbmir_rec_len);
...
)
{
  if (NULL != appInfo_ptr)
  {
    ...
    else
    {
      ...
      appInfo_ptr->bc.gw.sim_cbmid_rec_len   = MIN((uint32)data_len,len);
      appInfo_ptr->bc.gw.sim_cbmid_list_size = valid_entry_count;

      MSG_HIGH_4( "EFcbmid asid %d: received length %d,appinfo length:%d valid entries %d",
                  appInfo_ptr->asid,
                  data_len,
                  appInfo_ptr->bc.gw.sim_cbmid_rec_len,
                  valid_entry_count );
    }
  }
...
)
{
  ...
  case MMGSDI_CDMA_SMS_STATUS:
  case MMGSDI_CSIM_SMS_STATUS:
    if (appInfo_ptr->sim_file_init_started[SMSS_FILE_INDEX] == TRUE)
    {
    ...
    {
       MSG_ERROR_0("Dropping stale acccess error for EF_SMSS");
    }
    break;
  ...
  case MMGSDI_CDMA_SMS_PARAMS:
  case MMGSDI_CSIM_SMS_PARAMS:
    if (appInfo_ptr->sim_file_init_started[SMSP_FILE_INDEX] == TRUE)
    {
    ...
    {
       MSG_ERROR_0("Dropping stale acccess error for EF_SMSP");
    }
    break;

  case MMGSDI_GSM_CBMI:
  case MMGSDI_USIM_CBMI:
    if (appInfo_ptr->sim_file_init_started[CBMI_FILE_INDEX] == TRUE)
    {
#ifdef FEATURE_GWSMS_BROADCAST
    ...
    {
       MSG_ERROR_0("Dropping stale acccess error for EF_CBMI");
    }
    break;

  case MMGSDI_GSM_CBMIR:
  case MMGSDI_USIM_CBMIR:
    if (appInfo_ptr->sim_file_init_started[CBMIR_FILE_INDEX] == TRUE)
    {
#ifdef FEATURE_GWSMS_BROADCAST
    ...
    {
       MSG_ERROR_0("Dropping stale acccess error for EF_CBMIR");
    }
    break;

  case MMGSDI_GSM_CBMID:
  case MMGSDI_USIM_CBMID:
    if (appInfo_ptr->sim_file_init_started[CBMID_FILE_INDEX] == TRUE)
    {
#ifdef FEATURE_GWSMS_BROADCAST
...
//	    wms_bc_gw_cb_init(appInfo_ptr, TRUE);
    ...
    {
       MSG_ERROR_0("Dropping stale acccess error for EF_CBMID");
    }
    break;
  ...
  case MMGSDI_CDMA_SMS:
  case MMGSDI_CSIM_SMS:
    if (appInfo_ptr->sim_file_init_started[SMS_FILE_INDEX] == TRUE)
    {
      if (MMGSDI_GET_FILE_ATTR_CNF == cnf)
      {
        // If error getting file attributes, consider the file empty
    ...
    {
       MSG_ERROR_0("Dropping stale acccess error for EF_SMS");
    }
    break;

  case MMGSDI_TELECOM_SMSR:
  case MMGSDI_USIM_SMSR:
    if (appInfo_ptr->sim_file_init_started[SMSR_FILE_INDEX] == TRUE)
    {
      if (MMGSDI_GET_FILE_ATTR_CNF == cnf)
      {
        // If error getting file attributes, consider the file empty
    ...
    {
       MSG_ERROR_0("Dropping stale acccess error for EF_SMSR");
    }
    break;

  case MMGSDI_GSM_MWIS:
  case MMGSDI_USIM_MWIS:
    if (appInfo_ptr->sim_file_init_started[MWIS_FILE_INDEX] == TRUE)
    {
      if (MMGSDI_GET_FILE_ATTR_CNF == cnf)
      {
        // If error getting file attributes, consider the file empty
    ...
    {
       MSG_ERROR_0("Dropping stale acccess error for EF_MWIS");
    }
    break;
...
boolean wms_sim_mmgsdi_limited_init_done(
  wms_prov_app_info_s_type *appInfo_ptr
)
{
  boolean init_done = FALSE;
  ...
  if (NULL != appInfo_ptr)
  {
    if (TRUE == appInfo_ptr->sim_file_init_is_done[SMSS_FILE_INDEX] &&
        TRUE == appInfo_ptr->sim_file_init_is_done[SMSP_FILE_INDEX])
    {
      init_done = TRUE;
    }
  }
  return init_done;
} /* wms_sim_mmgsdi_limited_init_done() */
...
)
{
  ...
  if (appInfo_ptr->sim_file_init_started[CBMI_FILE_INDEX] ==  FALSE)
  {
    MSG_ERROR_0("Dropping stale attr cnf for EF-CBMI");
    return;
  }
...
)
{
  ...
  if (appInfo_ptr->sim_file_init_started[CBMIR_FILE_INDEX] ==  FALSE)
  {
    MSG_ERROR_0("Dropping stale attr cnf for EF-CBMIR");
    return;
  }
#ifdef FEATURE_GWSMS_BROADCAST
...
)
{
  ...
  if (appInfo_ptr->sim_file_init_started[CBMID_FILE_INDEX] ==  FALSE)
  {
    MSG_ERROR_0("Dropping stale attr cnf for EF-CBMID");
    return;
  }
...
)
{
  ...
  if (appInfo_ptr->sim_file_init_started[SMSP_FILE_INDEX] ==  FALSE)
  {
    MSG_ERROR_0("Dropping stale attr cnf for EF-SMSP");
    return;
  }
  ...
  {    
    ...
    if (appInfo_ptr->sim_file_init_is_done[SMSP_FILE_INDEX] == TRUE)
    {
       appInfo_ptr->sim_file_init_started[SMSP_FILE_INDEX] = FALSE;
    }
    /* read next file | start reading from SMSP Index since it may fallback to TELECOM*/
    wms_sim_mmgsdi_read_next_sim_file( appInfo_ptr, SMSP_FILE_INDEX );
  }
  else
  {
...
)
{
  ...
  if (appInfo_ptr->sim_file_init_started[MWIS_FILE_INDEX] ==  FALSE)
  {
    MSG_ERROR_0("Dropping stale attr cnf for EF-MWIS");
    return;
  }
...
)
{
  ...
  if (appInfo_ptr->sim_file_init_started[SMSS_FILE_INDEX] ==  FALSE)
  {
    MSG_ERROR_0("Dropping stale attr cnf for EF-SMSS");
    return;
  }
...
)
{
  ...
  if (appInfo_ptr->sim_file_init_started[SMS_FILE_INDEX] ==  FALSE)
  {
    MSG_ERROR_0("Dropping stale attr cnf for EF-SMS");
    return;
  }
...
)
{
  ...
  if (appInfo_ptr->sim_file_init_started[SMSR_FILE_INDEX] ==  FALSE)
  {
    MSG_ERROR_0("Dropping stale attr cnf for EF-SMSR");
    return;
  }
...
void wms_read_cdma_template_from_nv(wms_cdma_template_s_type *template_ptr)
{
   ...
   if (TRUE == wms_nv_read_cdma_params(0, nv_data,SYS_MODEM_AS_ID_1))
   {
