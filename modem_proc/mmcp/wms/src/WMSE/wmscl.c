/*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/wms/src/WMSE/wmscl.c#1 $
...
)
{
  ...
  MSG_HIGH_3( "wms_client_reg_detail_cb(): client_id = %d requested for WMS status info = %d, status %d", 
               client_id,
               req_detail, 
               st); 
...
)
{
  ...
  MSG_HIGH_4( "wms_client_set_cdma_channel_pref(): client_id = %d, client ptr = 0x%x, ch_pref = %d, status %d",
              client_id,
              client_ptr,
              ch_pref,
              st );
...
)
{
  ...
  {
     MSG_HIGH_5("wms_client_get_cdma_channel_pref status %d for client id = %d and ch_Prefptr @0x%x and client ptr @0x%x ,current ch_pref = %d", 
                  st , client_id, ch_pref, client_ptr, *ch_pref);
  }
  else
  {
     MSG_ERROR_4("wms_client_get_cdma_channel_pref status %d for client id = %d and ch_Prefptr @0x%x and client ptr @0x%x", 
                  st , client_id, ch_pref, client_ptr);
  }
...
)
{
  ...
  MSG_HIGH_4("wms_client_set_cdma_service_follow_on_dc_pref status= %d for client id= %d with clientptr @0x%x, follow_on_dc %d", st, client_id, client_ptr, follow_on_dc); 	
...
)
{
  ...
  if (WMS_CLIENT_ERR_NONE == st)
  {
     MSG_HIGH_5("wms_client_get_cdma_service_follow_on_dc_pref status= %d with follow_on_dc @0x%x for client id= %d with clientptr @0x%x, follow_on_dc %d", 
                st, follow_on_dc, client_id, client_ptr, *follow_on_dc); 	
  }
  else
  {
    MSG_ERROR_4("wms_client_get_cdma_service_follow_on_dc_pref status= %d with follow_on_dc @0x%x for client id= %d with clientptr @0x%x", 
                st, follow_on_dc, client_id, client_ptr);
  }
