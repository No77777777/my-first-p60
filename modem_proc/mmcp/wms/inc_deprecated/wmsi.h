...
/*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/wms/inc_deprecated/wmsi.h#1 $
...
typedef struct
{
   ...
   mmgsdi_rec_num_type  num_of_records;       /* size of in_use_records[] */
   mmgsdi_len_type      record_length;        /* length of each record */
} wms_seek_data_type;
...
} wms_client_link_control_data_s_type;
...
typedef struct
{
  ...
  sys_sys_mode_mask_e_type             subs_capability;  /**< Indicating maximum subscription capability, mainly gives info on multimode subs*/    
  ...
  wms_client_link_control_data_s_type  client_link_control_data[WMS_CLIENT_TYPE_MAX];
  
  wms_cfg_link_control_mode_e_type     current_used_link_control_mode;
  uint8                                current_used_link_control_timer_val;
  wms_cfg_link_control_status_e_type   current_used_link_control_status;
  ...
  boolean                              sim_file_init_started[MAX_FILE_INDEX]; // SIM files INIT started
...
typedef struct
{
  ...
  union
  {
    ...
    */
	...
	wms_callback_type                      wms_callback;
