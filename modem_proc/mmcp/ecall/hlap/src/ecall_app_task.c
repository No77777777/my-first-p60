...
$Header: //components/rel/mmcp.mpss/7.9.0/ecall/hlap/src/ecall_app_task.c#1 $
...
#define ECALL_REDIAL_MAX_UPTIME (120)/*We need to limit the redial for dropped calls to 2 mins as per CEN 16062 Clause 7.12.4*/
#define ECALL_MAX_UPTIME 0xffffffff
...
typedef struct {
 ...
 /* Timer to CALL_CLEARDOWN_TIMER */
  ...
  /*Timer to monitor the current uptime for redial in case of dropped calls*/
  dword uptime_dropped_call_redial;
...
#define ECALL_APP_VSS_STREAM_CHARACTER_MAX  31
static char_t ecall_app_stream_name[ECALL_APP_VSS_STREAM_CHARACTER_MAX];
static char    ecall_app_voc_dns[] = "qcom.audio.cvs";
#else
static char_t ecall_app_stream_name[] = "default modem voice";
static char ecall_app_voc_dns[] = "qcom.adsp.cvs";
...
{
  ECALL_MSG_0( ECALL_MED, "ENTERED STATE waiting_for_al_ack " );
...
{
  return ecall_app_task_struct->ecall_enabled_nv;
...
{
  ECALL_MSG_1 ( ECALL_MED, "Process eCall Profile %d (0: Regular, 1: Emergency)", ecall_profile );
...
)
{
  ...
  #ifdef FEATURE_ECALL_HAS_SINGLE_VOICE_SESSION
    // Update VSID whenever receiving CM_PH_EVENT_SUBSCRIPTION_PREF_INFO event
    appInfo_ptr = ecall_sim_get_appinfo_by_as_id(pref_info_asid);
    if (NULL != appInfo_ptr)
    {
      ECALL_MSG_2( ECALL_HIGH, "VSID: update CS Voice VSID %X -> %X",
                                appInfo_ptr->vsid_cs_voice,
                                subs_info_ptr->hw_id[SYS_VSID_APP_CS_VOICE] );
      appInfo_ptr->vsid_cs_voice = subs_info_ptr->hw_id[SYS_VSID_APP_CS_VOICE];
      if (appInfo_ptr->vsid_cs_voice != 0)
      {
        ECALL_MSG_2( ECALL_HIGH, "VSID: VSID %X is now available on as_id %d",
                                  appInfo_ptr->vsid_cs_voice,
                                  appInfo_ptr->as_id );
        appInfo_ptr->is_vsid_available = TRUE;
      }
      else
      {
        ECALL_MSG_1( ECALL_HIGH, "VSID: VSID is not yet available on as_id %d",
                                  appInfo_ptr->as_id );
        appInfo_ptr->is_vsid_available = FALSE;
      }
    }
    else
    {
      ECALL_MSG_1( ECALL_ERROR, "VSID: could not get appInfo for pref_info_asid %d", pref_info_asid);
    }
  #endif /* FEATURE_ECALL_HAS_SINGLE_VOICE_SESSION */
...
/*===========================================================================
  ...
  FUNCTION ECALL_APP_TASK_GET_ECALL_AS_ID

  DESCRIPTION
    This function will return the eCall asubs_id.
...
===========================================================================*/
sys_modem_as_id_e_type ecall_app_task_get_ecall_as_id
(
  void
)
{
  sys_modem_as_id_e_type ecall_as_id = SYS_MODEM_AS_ID_NONE;
  mmgsdi_slot_id_enum_type    slot_id = (mmgsdi_slot_id_enum_type)0;
  ecall_prov_app_info_s_type *appInfo_ptr = NULL;
  ...
  if ((mmgsdi_slot_id_enum_type)0 == slot_id )
  {
    ECALL_MSG_0( ECALL_ERROR, "ecall_app_task_get_ecall_as_id(): Invalid slot_id" );
    goto Done;
  }
  ...
  if (NULL == appInfo_ptr)
  {
    ECALL_MSG_1( ECALL_ERROR, "ecall_app_task_get_ecall_as_id(): Unmapped slot_id %d",
                               slot_id );
    goto Done;
  }

  ecall_as_id = appInfo_ptr->as_id;
...
)
{
  ...
  if(ecall_app_task_struct->redial_pending == TRUE)
  {
	...
	Then if the call is failed at Orig and already we have one redial attempt done
	for the Dropped Call, then we shall use the second timer value of 60 sec for next redial
	Else we will use the the next redial counter based on the call orig failure redial index */
    if ( ecall_app_task_struct->is_call_fail_at_orig == TRUE )
    {
      if(ecall_app_task_struct->call_dropped_redial_index==1)
	  {
        /* Call failed at origination */
...
)
{
  ...
  if ( (call_event == CM_CALL_EVENT_CONNECT) || (call_event == CM_CALL_EVENT_END) || (call_event == CM_CALL_EVENT_SETUP_IND)||(call_event == CM_CALL_EVENT_INCOM))
  {
  ...
  {
    ECALL_MSG_0( ECALL_HIGH, "Received CM_CALL_EVENT_SETUP_IND with call_type VOICE or EMERGENCY, eCall will setup the call" );
    cmd_ptr = ecall_app_task_get_cmd_buf();
    if (cmd_ptr == NULL)
    {
       ECALL_MSG_0( ECALL_ERROR, "ecall_app_task_get_cmd_buf ran out of command buffers in ecall_call_event_cb()" );
    ...
    cmd_ptr->data.setup_call.call_id = call_info_ptr->call_id;
    cmd_ptr->data.setup_call.as_id   = call_info_ptr->asubs_id;
  ...
  {    
    ...
    switch (call_event)
    {
      ...
      case CM_CALL_EVENT_END:
        ECALL_MSG_6( ECALL_MED,
               "Received CM_CALL_EVENT_END, direction = %d, end_status = %d, cc_cause.present = %d, cc_cause.cause_value = %d, cc_reject.present = %d, cc_reject.rej_value = %d", 
       ...
       {
         cmd_ptr->data.end_call_config.redial_pending = ecall_app_task_struct->redial_pending;
       }
        ...
        if ( (current_app_state == ECALL_APP_ORIGINATING_CALL) ||
             ...
             (((current_app_state == ECALL_APP_IN_CALL)||(current_app_state == ECALL_APP_WAITING_FOR_AL_ACK)) &&
            ...
            )  /* HLAP EN 16062 7.12.4 */
        {
          ...
          if (call_info_ptr->direction == CM_CALL_DIRECTION_MO)
          {
            ...
            if (current_app_state == ECALL_APP_ORIGINATING_CALL)
            {
			  ...
			  /*We need to set the redial pending to  true
			  if the  redial_index is less than 10 and we are still
			   in the range of the 2 minutes for call dropped case.*/

              if (ecall_app_task_struct->call_orig_fail_redial_index < ECALL_CALL_ORIG_FAIL_REDIAL_INDEX_MAX && ecall_app_task_struct->call_dropped_redial_index < ECALL_CALL_DROPPED_REDIAL_INDEX_MAX && uptime < ecall_app_task_struct->uptime_dropped_call_redial)
              {
                /* REDIAL */ 
                ECALL_MSG_0( ECALL_HIGH, "WILL REDIAL (Call Orig Failed)" );
                cmd_ptr->data.end_call_config.redial_pending = TRUE;
                ...
                if(ecall_app_task_struct->call_orig_fail_redial_index == ECALL_CALL_ORIG_FAIL_REDIAL_INDEX_MAX )
		{
	        ...
	        else if(uptime >= ecall_app_task_struct->uptime_dropped_call_redial)
		{
		  ECALL_MSG_2( ECALL_HIGH, "WILL NOT REDIAL (Call Orig Failed). Call Orig current uptime (%d) exceed maximum allowable time %d",
                                          uptime,
                                          ecall_app_task_struct->uptime_dropped_call_redial);
		}
		else
		{
		  ECALL_MSG_2( ECALL_HIGH, "WILL NOT REDIAL (Call Orig Failed). Call orig failure redial attempts (%d) exceed maximum allowable number %d",
            ...
            {
              if(ecall_app_task_struct->call_dropped_redial_index < ECALL_CALL_DROPPED_REDIAL_INDEX_MAX && uptime < ecall_app_task_struct->uptime_dropped_call_redial)
              {
                if(ecall_app_task_struct->call_dropped_redial_index==0)
              {
					ecall_app_task_struct->uptime_dropped_call_redial=time_get_uptime_secs()+ECALL_REDIAL_MAX_UPTIME;
				}
                /* REDIAL */ 
              ...
              else 
              {
				...
				{
                  ECALL_MSG_2( ECALL_HIGH, "WILL NOT REDIAL (Call Dropped). Call drop current uptime (%d) exceed maximum allowable time %d",
                                          uptime,
                                          ecall_app_task_struct->uptime_dropped_call_redial);
				}
                cmd_ptr->data.end_call_config.redial_pending = FALSE;
...
{
  ECALL_MSG_1( ECALL_HIGH, "Rx OP MODE ECALL_MODE %d (-1: Unknown, 0: Normal, 1: eCall Only)",
                              operating_mode );
...
static void ecall_app_msgr_init(void)
{
  ...
  for(sub = SYS_MODEM_AS_ID_1; sub < SYS_MODEM_AS_ID_MAX; ++sub) {
    msgr_error = msgr_register_variant(MSGR_ECALL_APP, &ecall_app_msgr_client, ecall_app_msgr_rex_q_id, MM_MMOC_OFFLINE_IND, sub);
    ECALL_MSG_2( ECALL_HIGH, "MM_MMOC_OFFLINE_IND register ret = %d, sub = %d",
                              msgr_error, sub );
  }
...
{
  ecall_app_msgr_cmd_s_type *msg = NULL;
  
  /* Get the command from the ecall_app_msgr_q and process it */
  while ((msg = q_get(&ecall_app_msgr_q)) != NULL)
  {
    ...
    q_put (&ecall_app_msgr_free_q, &(msg->link));
  }
} /* ECALL_APP_MSGR_PROC() */

/*========================================================================
  FUNCTION ecall_app_msgr_send_ecall_status_ind

  DESCRIPTION
    to send the eCall Status Ind to QMI Voice
  ...
  RETURN VALUE
    None
===========================================================================*/
void ecall_app_msgr_send_ecall_status_ind
(
  mmode_qmi_voice_ecall_msd_transmission_status_e_type msd_transmission_status
)
{
  cm_call_id_type                            ecall_call_id = ecall_get_call_id();
  errno_enum_type                            msgr_error;
  mmode_qmi_voice_ecall_status_ind_msg_type  msg_ecall_status_ind_pkt;
  sys_modem_as_id_e_type                     ecall_as_id = ecall_app_task_get_ecall_as_id();

  ECALL_MSG_3( ECALL_HIGH, "ecall_app_msgr_send_ecall_status_ind: call_id = %d, msd_transmission_status = %d, as_id = %d",
                            ecall_call_id,
                            msd_transmission_status,
                            ecall_as_id );

  /* send msgr after this EFS is updated */
  memset( &msg_ecall_status_ind_pkt, 0x00, sizeof(msg_ecall_status_ind_pkt) );

  msgr_init_hdr_variant( (msgr_hdr_struct_type *)&msg_ecall_status_ind_pkt, MSGR_ECALL_APP, QMI_VOICE_ECALL_STATUS_IND, ecall_as_id);

  msg_ecall_status_ind_pkt.ecall_status.cm_call_id = ecall_call_id;
  msg_ecall_status_ind_pkt.ecall_status.ecall_msd_transmission_status = msd_transmission_status;

  msgr_error = msgr_send((msgr_hdr_struct_type*)&msg_ecall_status_ind_pkt, sizeof(mmode_qmi_voice_ecall_status_ind_msg_type));
      
  if (msgr_error != E_SUCCESS)
  {
    ECALL_MSG_1( ECALL_ERROR, "Error sending eCall Status Ind to QMI Voice: %d", (uint16)msgr_error);
  }

} /* ecall_app_msgr_send_ecall_status_ind() */
#endif /* FEATURE_ECALL_HAS_V1_HEADER */
...
)
{
...
#ifdef FEATURE_ECALL_HAS_MCFG_EFS
  mcfg_fs_status_e_type efs_status;
#else
  int32 efs_status =0;
#endif /* FEATURE_ECALL_HAS_MCFG_EFS */
  ...
  /* efs_stat returns MCFG_FS_STATUS_OK if the file exists, others with error*/
#ifdef FEATURE_ECALL_HAS_MCFG_EFS
  efs_status = mcfg_fs_stat(ecall_nv_fpath, &efs_buf, MCFG_FS_TYPE_EFS, MCFG_FS_SUBID_0);
  if(efs_status == MCFG_FS_STATUS_OK)
#else
  /* efs_stat returns 0 if the file exists, -1 on error*/
  efs_status =    efs_stat (ecall_nv_fpath, &efs_buf);
  if(efs_status == 0)
#endif /* FEATURE_ECALL_HAS_MCFG_EFS */
  {
    
  #ifdef FEATURE_ECALL_HAS_MCFG_EFS
    efs_status = mcfg_fs_read(ecall_nv_fpath, 
                              &ecall_app_task_struct->ecall_enabled_nv, 
                              sizeof(ecall_app_task_struct->ecall_enabled_nv),
                              MCFG_FS_TYPE_EFS, MCFG_FS_SUBID_0);
  #else
  ...
  /* Initialize uptime_dropped_call_redial to ECALL Max Uptime */
  ecall_app_task_struct->uptime_dropped_call_redial = ECALL_MAX_UPTIME;
 ...
 -------------------------------------------------------------------------*/
  ...
  for ( ; ; )
  {
    ...
    if ((set_sig &  ECALL_APP_CALL_CLEARDOWN_TIMER_SIG)!= 0 ) 
    {
      ...
      current_app_state = ecall_app_task_get_state();
      ECALL_MSG_1( ECALL_HIGH, "ECALL_APP_CALL_CLEARDOWN_TIMER_SIG EXPIRED, current_app_state = %d",
                                current_app_state );
    ...
    #ifndef FEATURE_ECALL_HAS_V1_HEADER
      ecall_app_msgr_send_ecall_status_ind(MMODE_QMI_VOICE_ECALL_MSD_TRANSMISSION_STATUS_FAILURE);
    #endif /* FEATURE_ECALL_HAS_V1_HEADER */
      ecall_app_task_struct->ecall_app_task_discard_msd = FALSE; /* Do not discard this MSD */
      ecall_app_task_struct->is_session_timer_expired = TRUE;
      ecall_app_task_change_state(ECALL_APP_IN_CALL); /* Transmission never started, go to regular voice call */
...
)
{
  ...
  switch (ivs_status.tx_status)
  {
    ...
    #ifndef FEATURE_ECALL_HAS_V1_HEADER
      ecall_app_msgr_send_ecall_status_ind(MMODE_QMI_VOICE_ECALL_MSD_TRANSMISSION_STATUS_SUCCESS);
    #endif /* FEATURE_ECALL_HAS_V1_HEADER */
    ...
    case ECALL_IVS_MSD_FAST_MODE:
      ECALL_MSG_1( ECALL_HIGH, "IVS Rx ECALL_IVS_MSD_FAST_MODE. Set session_timer to %d ms",
    ...
    switch(cmd_ptr->cmd_id)
    {
      case ECALL_APP_CMD_GPS_FIX_RECVD:
        ...
        ecall_save_position (
                   ...
                   cmd_ptr->data.gps_data.magneticDev_valid,
                   ...
                   cmd_ptr->data.gps_data.magneticDev);
        #elif ( (defined FEATURE_CGPS) && !(defined FEATURE_ECALL_HAS_QMI_LOC) ) /* PDAPI */
      ...
      case ECALL_APP_CMD_END_ECALL:
        ...
        ECALL_MSG_3( ECALL_HIGH, "END_ECALL with call_direction %d, redial_pending %d, is_call_fail_at_orig %d",
         ...
         if(!ecall_app_task_struct->redial_pending)
         {
           ecall_app_task_struct->uptime_dropped_call_redial = ECALL_MAX_UPTIME;
         }
        /* Reset call_id */
      ...
      case ECALL_APP_CMD_ECALL_CONNECTED:
        ECALL_MSG_2( ECALL_HIGH, "Rx ECALL_APP_CMD_ECALL_CONNECTED. Call Direction = %d. Set call_cleardown_timer to %d ms",
                                  ecall_app_task_struct->call_direction,
      ...
      case ECALL_APP_CMD_SETUP_CALL:
        ecall_app_process_setup_call(cmd_ptr->data.setup_call.call_id, cmd_ptr->data.setup_call.as_id);
        break;
...
)
{
  ...
  switch (packet->token)
  {
    ...
    case ECALL_APP_APR_CREATE_PASSIVE_CONTROL_SESSION:
      if (status == APR_EOK)
      {
        ECALL_MSG_3( ECALL_HIGH, "Successfully create passive CVS control session, port %x, pending_mute %d, pending_unmute %d",
    ...
    case ECALL_APP_APR_MUTE_RX_OUT:
      ECALL_MSG_1( ECALL_HIGH, "MUTE voc RX status %d",
                                status );
      ecall_app_task_struct->pending_mute = FALSE;
      __aprv2_cmd_free(ecall_app_apr_handle, packet);
      break;
    ...
    case ECALL_APP_APR_UNMUTE_RX_OUT:
      ECALL_MSG_1( ECALL_HIGH, "UNMUTE voc RX status %d",
                                status );
      ecall_app_task_struct->pending_unmute = FALSE;
      __aprv2_cmd_free(ecall_app_apr_handle, packet);
      break;
...
)
{
...
#ifdef FEATURE_ECALL_HAS_SINGLE_VOICE_SESSION
  {
    ...
    (void)ecall_itoa(vsid_cs_voice, ecall_app_stream_name, 16);
  }
#endif /* FEATURE_ECALL_HAS_SINGLE_VOICE_SESSION */
