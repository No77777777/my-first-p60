...
$Header: //components/rel/mmcp.mpss/7.9.0/ecall/hlap/src/ecall_app.c#1 $
...
{                         
  if ((CM_CALL_EVENT_CONNECT == call_event)||(CM_CALL_EVENT_SETUP_IND == call_event)||(CM_CALL_EVENT_INCOM == call_event))
  {
  ...
  ECALL_MSG_5( ECALL_MED, "Received call_id %d, call_event %d, call_type %d.  Update call_status[%d] to status %d",
                           call_id,
                           call_event,
                           call_type,
                           call_id,
...
)
{
  ...
  for ( call_id = 0; call_id < ECALL_APP_CM_CALL_ID_MAX; call_id++ )
  {
    ...
    {
      ECALL_MSG_2( ECALL_HIGH, "call_id %d is a non-emergency or non-PS call. eCall App ends it.  eCall call_id %d",
                                call_id,
                                ecall_get_call_id() );
...
)
{
  ...
  cm_util_set_default_voice_bearer_cap_per_sub(&(setup_params.bearer_capability_1), as_id);
  ...
  if ( TRUE == ecall_app_task_get_is_era_glonass_enabled() ) /* ERA-GLONASS eCall dialing number */
  {
    switch(ecall_app_struct.ecall_config_params.type_of_call)
    {
      case ECALL_EMERGENCY:
        ECALL_MSG_0( ECALL_HIGH, "ORIGINATE EMERGENCY eCall (ERA-GLONASS), Dialing 112 " );
      ...
      case ECALL_RECONFIG:
        ...
        ECALL_MSG_1( ECALL_HIGH, "ECALL_TEST or RECALL_RECONFIG (ERA-GLONASS), Dial EFS number...len %d",
                                  digits_len );
    ...
    if (ecall_app_struct.ecall_config_params.efs_params.ecall_dial_num == ECALL_DIAL_NORMAL)
    {
      ...
      switch(ecall_app_struct.ecall_config_params.type_of_call)
      {
        ...
        case ECALL_TEST:
          ...
          {
            /* FDN Test number */
            ECALL_MSG_1( ECALL_HIGH, "Test eCall. ECALL_MODE_ECALL_ONLY, Dialing FDN number...len %d ",
          ...
          {
            /* SDN Test number */
            ECALL_MSG_1( ECALL_HIGH, "Test eCall. ECALL_MODE_NORMAL, Dialing SDN number...len %d",
        ...
        case ECALL_RECONFIG:
          ...
          {
            /* FDN Test number */
            ECALL_MSG_1( ECALL_HIGH, "Reconfig eCall. ECALL_MODE_ECALL_ONLY, Dialing FDN number...len %d ",
          ...
          {
            /* SDN Test number */
            ECALL_MSG_1( ECALL_HIGH, "Reconfig eCall. ECALL_MODE_NORMAL, Dial SDN number...len %d",
...
Done:
  ...
  if (retVal == TRUE)
  {
    ECALL_MSG_0( ECALL_MED, " ECALL ecall_originate_call reset msg_id to 0" );
    ecall_app_struct.ecall_msd_msg_id = 0;
  }
  return retVal;
}
/*lint +esym(603,alpha_buf)*/
...
)
{
  ...
  ECALL_MSG_1( ECALL_HIGH, "check_pb_valid(): pb_category %d",
                            pb_id.pb_category );
...
)
{
  ...
  ecall_construct_msd_struct(&ecall_msd, ecall_app_struct.ecall_config_params.efs_params.ecall_msdversion); /* Construct Canned MSD from hardcoded values */
  ...
  if (ecall_app_struct.ecall_config_params.efs_params.canned_msd == TRUE)
  {
    ...
    if (efs_msd_read_status == TRUE)
    {
      ecall_msd.ecall_msdversion = ecall_app_struct.ecall_config_params.efs_params.ecall_msdversion;
    }
  } /* Canned MSD */
  ...
  ecall_serialise_msd(&ecall_msd, ecall_app_msd_byte_array, ecall_app_struct.ecall_config_params.efs_params.ecall_msdversion);
...
)
{
  ...
  ecall_app_struct.ecall_config_params.efs_params.ecall_msdversion  = efs_config->ecall_msdversion;
