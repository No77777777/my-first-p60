...
$Header: //components/rel/mmcp.mpss/7.9.0/ecall/emodem/src/ecall_efs.c#3 $
$DateTime: 2021/09/17 06:50:30 $
$Author: pwbldsvc $ 
*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*/
...
)
{
  ...
  while(*file_contents)                 /* Read until reaching null char for each line*/
  {
    ...
    while (*file_contents)               /* Read until reaching null char */
    {
	  ...
	  else if((ecall_efs_tokens >= ECALL_EFS_ARRAY_SIZE) || (index2 >= ECALL_EFS_STRING_SIZE))
	  {
	     break;
	  }
...
)
{
...
#ifdef FEATURE_ECALL_HAS_MCFG_EFS
  file_descriptor = mcfg_fopen( efs_ps_file_name,  MCFG_FS_O_RDONLY, MCFG_FS_ALLPERMS, MCFG_FS_TYPE_EFS, MCFG_FS_SUBID_0 );
#else
  ...
  /* obtain file size */
#ifdef FEATURE_ECALL_HAS_MCFG_EFS
  file_size = mcfg_lseek (file_descriptor, 0, MCFG_SEEK_END, MCFG_FS_TYPE_EFS);
#else
  ...
  /* close the efs file */
#ifdef FEATURE_ECALL_HAS_MCFG_EFS
  result = mcfg_fclose(file_descriptor, MCFG_FS_TYPE_EFS);
#else
 ...
 ****************************************************************************/
  ...
  if ( file_size > ECALL_EFS_MAX_REC_LEN ) 
  {
     ECALL_MSG_0( ECALL_ERROR, "ERROR: ecall_efs File Size Limit: ECALL_EFS_MAX_REC_LEN. Using Defualt values" );
     return FALSE;
  }
  ...
  /*lint -e732*/
#ifdef FEATURE_ECALL_HAS_MCFG_EFS
  bytes_read = mcfg_fread(file_descriptor, ecall_file_contents, file_size, MCFG_FS_TYPE_EFS);
#else
...
)
{
      ...
      (ecall_efs_read_status == TRUE))
  {
    ...
    ECALL_MSG_4( ECALL_HIGH, "canned_psap %d, enable_slip %d, invert_sig %d, force_pull %d", 
                              ecall_psap_efs_config->canned_psap, 
...
)
{
  ...
  ----------------------------------------------------------------------*/
  (void)ecall_itoa(ecall_msd->ecall_msdversion, itoa_buf, 10);
  if (ecall_msd->ecall_msdversion == ECALL_MSD_VERSION_2)
  {
    (void) strlcpy(ecall_file_contents, "\nMSDVersion   ",nDestSize);
  }
  else
  {
...
#ifdef FEATURE_ECALL_HAS_MCFG_EFS
  fs_res = mcfg_fwrite(fd, ecall_file_contents, strlen(ecall_file_contents), MCFG_FS_TYPE_EFS);
#else
  ...
  if (ecall_msd->ecall_msdversion == ECALL_MSD_VERSION_2)
  {
    (void) strlcat(ecall_file_contents, "\notherStorage    ",nDestSize);
    if(ecall_msd->ecall_vehicle_propulsionstorage.otherStorage == TRUE)
    {
      (void) strlcat(ecall_file_contents, "TRUE",nDestSize);
    }
    else
  ...
  /* obtain file size */
#ifdef FEATURE_ECALL_HAS_MCFG_EFS
  file_size = mcfg_lseek(fd, 0, MCFG_SEEK_CUR , MCFG_FS_TYPE_EFS);
#else
...
#ifdef FEATURE_ECALL_HAS_MCFG_EFS
  result= mcfg_ftruncate(fd, file_size, MCFG_FS_TYPE_EFS);
#else
  result = efs_ftruncate(fd, file_size);
#endif /* FEATURE_ECALL_HAS_MCFG_EFS */
...
{
  int fs_handle, result;
 /*-------------------------------------------------------------------------
    Open the file
  -------------------------------------------------------------------------*/
#ifdef FEATURE_ECALL_HAS_MCFG_EFS
  fs_handle = mcfg_fopen( efs_msd_file_name, MCFG_FS_O_WRONLY|MCFG_FS_O_CREAT, MCFG_FS_ALLPERMS, MCFG_FS_TYPE_EFS, MCFG_FS_SUBID_0 );
#else
...
#ifdef FEATURE_ECALL_HAS_MCFG_EFS
  result = mcfg_fclose(fs_handle, MCFG_FS_TYPE_EFS);
#else
  result =   efs_close(fs_handle);
#endif /* FEATURE_ECALL_HAS_MCFG_EFS */
...
)
{
  ...
  ecall_enable_config->ecall_msdversion   = ECALL_MSD_VERSION_2;
  ...
  if(ecall_efs_read_status  == TRUE)
  {
    ...
    ECALL_MSG_3( ECALL_HIGH, "efs set gnss_update_time_ms to %d, test_only_callback_timer_ms to %d, ecall_usim_slot_id to %d",
    ...
    /* ecall_msdversion */
    ecall_enable_config->ecall_msdversion = (uint8)atoi(ecall_efs_data[15]);
    if (ecall_enable_config->ecall_msdversion != ECALL_MSD_VERSION_1 && ecall_enable_config->ecall_msdversion != ECALL_MSD_VERSION_2)
    {
      ECALL_MSG_0( ECALL_HIGH, "efs ecall_msdversion not set, using default MSD version as 2");
      ecall_enable_config->ecall_msdversion = ECALL_MSD_VERSION_2;
    }
    ECALL_MSG_1( ECALL_HIGH, "efs ecall_msd_version %d", ecall_enable_config->ecall_msdversion);
...
)
{
  ...
  ecall_era_glonass_config->ecall_msdversion = ECALL_MSD_VERSION_2;
  ...
  if(ecall_efs_read_status == TRUE)
  {
    ...
    ECALL_MSG_6( ECALL_HIGH, "efs set voc_config to %d, canned_msd to %d, gnss_update_time_ms to %d, test only callback_timer_ms to %d, call_cleardown_timer_ms to %d, session_timer_ms to %d",
    ...
    /* ecall_msdversion */
    ecall_era_glonass_config->ecall_msdversion = (uint8)atoi(ecall_efs_data[17]);
    if (ecall_era_glonass_config->ecall_msdversion != ECALL_MSD_VERSION_1 && ecall_era_glonass_config->ecall_msdversion != ECALL_MSD_VERSION_2)
    {
      ECALL_MSG_0( ECALL_HIGH, "efs era_glonass ecall_msdversion not set, using default MSD version as 2");
      ecall_era_glonass_config->ecall_msdversion = ECALL_MSD_VERSION_2;
    }
    ECALL_MSG_1( ECALL_HIGH, "efs era_glonass ecall_msd_version %d", ecall_era_glonass_config->ecall_msdversion);
...
)
{
  ...
  if ((ecall_efs_read_status == TRUE) && (ecall_efs_tokens >= 21))
  {
    ...
    if(str_comp(ecall_efs_data[3], "ECALL_MANUAL_INITIATED") == 0)
    {
    ...
    if(str_comp(ecall_efs_data[5], "ECALL_EMERGENCY") == 0)
    {
      /* EN15722: callType(0): Emergency Call */
    ...
    ecall_msd->ecall_control_info.ecall_vehicletype = (ecall_vehicle_e_type)atoi(ecall_efs_data[7]);

    ecall_msd->ecall_control_info.positionconfidence = (str_comp(ecall_efs_data[9],"TRUE") == 0 ) ? TRUE : FALSE;

    ecall_memscpy ( (void *)ecall_msd->ecall_vin, sizeof(ecall_msd->ecall_vin),
                    ecall_efs_data[11],           sizeof(ecall_efs_data[11]) );
 
    /* Timestamp is uint32*/
    tempu32 = (ecall_timestamp_type)atoi(ecall_efs_data[13]);
    ...
    tempi32 = atoi(ecall_efs_data[15]);
    ...
    ecall_msd->ecall_vehicledirection = (uint8 )atoi(ecall_efs_data[19]);

    temp8 = (uint8 )atoi(ecall_efs_data[21]);  
    if(temp8 > 0 && temp8 < ECALL_EFS_U8_MAX)
    {
  ...
  else if((ecall_efs_read_status == TRUE) && (ecall_efs_tokens < 21))
  {
