...
$Header: //components/rel/mmcp.mpss/7.9.0/ecall/emodem/src/ecall_modem_ivs.c#1 $
...
static boolean ecall_modem_voice_agent_granted = FALSE;
#else
static char_t ecall_modem_stream_name[] = "default modem voice";
static char ecall_modem_ivs_voc_dns[] = "qcom.adsp.cvs";
#endif /* FEATURE_ECALL_HAS_SINGLE_VOICE_SESSION */
...
typedef enum
{
  ...
  ECALL_APR_DESTROY_SESSION,                /* APRV2_IBASIC_CMD_DESTROY_SESSION */
  ECALL_APR_SET_ENC_DTX_MODE,               /* VSS_ISTREAM_CMD_SET_ENC_DTX_MODE */
...
{   
  ...
  boolean pending_modulator_resume;
...
)
{
  ...
  {
    ECALL_MSG_0( ECALL_ERROR, "voc_config_restore(): dest_port is 0" );
    return;
  }
  ...
  */
  ecall_modem_set_enc_dtx_mode(TRUE);
...
)
{
  ...
  */
  ecall_modem_set_enc_dtx_mode(FALSE);
...
)
{
  ...
  switch (event_id)
  {
    ...
    default:
          ECALL_MSG_0( ECALL_ERROR, "Unknown eCall event " );
          break;
  }
}
...
{
  ECALL_MSG_0( ECALL_HIGH, "[IVS ] Tx reset RX/TX" );
  ecall_ivs_rx_reset();
  ecall_ivs_tx_reset();
}
...
)
{
   ...
   ecall_modem_ivs.pending_modulator_resume = FALSE;
...
)
{
  ...
  if((ecall_modem_ivs.voc_rx_registered == FALSE) && (ecall_modem_ivs.voc_pending_register != TRUE))
  {
    /* Reset IVS settings */
    ecall_ivs_reset();
  }
  ...
  else if (ecall_modem_ivs.tx_mode == PULL_MODE)
  {
    ...
    /* Register vocoder with RX only mode if not yet registered */
    if( ecall_modem_ivs.voc_rx_registered == FALSE )
    {
    #if ( (defined FEATURE_ECALL_HAS_VOICE_A) || (defined FEATURE_ECALL_HAS_VOICE_B) )

      if( ecall_modem_ivs.voc_pending_register == FALSE )
      {
        ecall_voc_register(ECALL_VOC_MODE_RX_ONLY);
      }
      else
      {
	    ecall_modem_ivs.pending_modulator_resume = TRUE;
        ECALL_MSG_0( ECALL_HIGH, "Pending voc_register exists, do not register for RX_ONLY vocoder");
      }
...
{
  ECALL_MSG_2( ECALL_HIGH,"process_abort_tx: ivs.tx.state %d, discard_msd %d",
                          ivs.tx.state,
                          discard_msd );
...
{  
   ECALL_MSG_0( ECALL_HIGH, "ECALL_IVS_MODEM_ABORT_ALL  " );
   ecall_modem_ivs.drop_tx_pcm_count = 0;
   ecall_modem_ivs.is_msd_received_from_app = FALSE;
...
===========================================================================*/
void ecall_modem_destroy_session
(
  void
)
{
  ...
  if (ecall_modem_ivs.voc_rx_registered == TRUE || ecall_modem_ivs.voc_tx_registered == TRUE)
  {
  ...
  ret = __aprv2_cmd_alloc_send(
            ...
            ecall_modem_apr_dest_addr,             /* dst_addr     */
            ecall_modem_apr_dest_port,             /* dst_port     */
            ECALL_APR_DESTROY_SESSION,             /* token        */
...
{
  ECALL_MSG_3( ECALL_MED,"Request to register vocoder for mode: %d (0:RX only, 1:RX-TX), rx_registered %d, tx_registered %d",
                          voc_mode, ecall_modem_ivs.voc_rx_registered, ecall_modem_ivs.voc_tx_registered );
  ...
  if(voc_mode == ECALL_VOC_MODE_RX_ONLY)
  {
    ...
    {
      /* voc_rx_registered == TRUE && voc_tx_registered == FALSE
         We do not need to regsiter for RX_ONLY again */
      ECALL_MSG_1( ECALL_HIGH, "Vocoder already register for mode %d",
  ...
  else if (voc_mode == ECALL_VOC_MODE_RX_TX)
  {
    ...
    {
      /* voc_rx_registered == TRUE && voc_tx_registered == TRUE
         We do not need to regsiter for RX_TX again */
      ECALL_MSG_1( ECALL_HIGH, "Vocoder already register for mode %d",
                           ecall_modem_ivs.voc_config_mode );
      ecall_modem_ivs.voc_pending_register = FALSE;
      return;
  ...
  ECALL_MSG_0( ECALL_ERROR, "UNSUPPORTED SCENARIO");
...
)
{
  ...
  switch (packet->token)
  {
    case ECALL_APR_MAP_MEMORY:
      ECALL_MSG_1( ECALL_HIGH, "Map memory process status, error code %d",
                                status );
      if(status == APR_EOK)
      {
    ...
    case ECALL_APR_UNMAP_MEMORY:
      ECALL_MSG_1( ECALL_HIGH, "Unmap memory process status, error code %d",
                                status );
      if(status == APR_EOK)
      {
        ecall_modem_ivs.smem_mapped = FALSE;
    ...
    case ECALL_APR_IVPCM_START_RX_TX:
      if(status == APR_EOK)
      {
        ...
        if(ecall_modem_ivs.tx_mode == PULL_MODE && ecall_modem_ivs.pending_modulator_resume == TRUE)
	    {
          ecall_modem_ivs_resume_modulator();
          ecall_modem_ivs.pending_modulator_resume = FALSE;
	    }                 
    ...
    case ECALL_APR_DESTROY_SESSION:
      if (status == APR_EOK)
      {
...
#ifdef FEATURE_ECALL_HAS_SINGLE_VOICE_SESSION
  if (ecall_modem_voice_agent_granted == FALSE)
  {
    ...
    ecall_modem_ivs.voc_re_register_mode = voc_mode;
    ecall_sim_request_voice_agent_grant();
    return;
  }
#endif /* FEATURE_ECALL_HAS_SINGLE_VOICE_SESSION */
...
)
{
...
#ifdef FEATURE_ECALL_HAS_SINGLE_VOICE_SESSION
  {
    ...
    ecall_sim_get_ecall_vsid_cs_voice(&vsid_cs_voice);
  ...
  switch (packet->token)
  {
    ...
    case ECALL_APR_DESTROY_SESSION:
      if (status == APR_EOK)
      {
        ...
        ecall_modem_apr_dest_port = 0;
      #ifdef FEATURE_ECALL_HAS_SINGLE_VOICE_SESSION
        ecall_modem_voice_agent_granted = FALSE;
        ecall_sim_request_voice_agent_release();
      #endif /* FEATURE_ECALL_HAS_SINGLE_VOICE_SESSION */
      }
      else
      {
    ...
    case ECALL_APR_SET_ENC_DTX_MODE:
      if (status == APR_EOK)
      {
...
/*===========================================================================
  ...
  FUNCTION ECALL_MODEM_SET_ENC_DTX_MODE
...
===========================================================================*/
void ecall_modem_set_enc_dtx_mode
( 
  boolean enable
)
{
  ...
  ret = __aprv2_cmd_alloc_send(
            ...
            ecall_modem_apr_dest_addr,                                 /* dst_addr     */
            ecall_modem_apr_dest_port,                                 /* dst_port     */
            ECALL_APR_SET_ENC_DTX_MODE,                                /* token        */ 
...
)
{
  ...
  switch (cmd_id)
  {
    case ECALL_VOICE_IRESOURCE_CMD_REGISTER:
      ...
      if (TRUE == ecall_sim_is_voice_agent_registered(register_ptr->vsid) )
      {
      ...
      /* Queue a command to eCall IVS task to register this Voice Agent */
      cmd_ptr = ecall_get_cmd_buf();
      if (NULL == cmd_ptr)
      {
        ECALL_MSG_1( ECALL_ERROR, "ecall_voice_api(): Register: Cannot allocate cmd_buf for cmd_id %d", cmd_id );
        return ECALL_VOICE_ENOMEMORY;
      }
      cmd_ptr->cmd_id = ECALL_CMD_VOICE_AGENT_REGISTER;
      memscpy(&cmd_ptr->data.voice_agent_register_data,
              sizeof(cmd_ptr->data.voice_agent_register_data),
              params_ptr,
              size);
      ecall_put_cmd(cmd_ptr);
    ...
    case ECALL_VOICE_IRESOURCE_CMD_DEREGISTER:
      ...
      if (FALSE == ecall_sim_is_voice_agent_registered((sys_vsid_type)deregister_ptr->handle))
      {
        ECALL_MSG_1( ECALL_ERROR, "ecall_voice_api(): Deregister: Invalid handle %d", deregister_ptr->handle );
        return ECALL_VOICE_EHANDLE;
      }

      /* Queue a command to eCall IVS task to deregister this Voice Agent */
      cmd_ptr = ecall_get_cmd_buf();
      if (NULL == cmd_ptr)
      {
        ECALL_MSG_1( ECALL_ERROR, "ecall_voice_api(): Deregister: Cannot allocate cmd_buf for cmd_id %d", cmd_id );
        return ECALL_VOICE_ENOMEMORY;
      }
      cmd_ptr->cmd_id = ECALL_CMD_VOICE_AGENT_DEREGISTER;
      memscpy(&cmd_ptr->data.voice_agent_deregister_data,
              sizeof(cmd_ptr->data.voice_agent_deregister_data),
              params_ptr,
              size);
      ecall_put_cmd(cmd_ptr);
    ...
    case ECALL_VOICE_IRESOURCE_CMD_GRANT:
      ...
      if (FALSE == ecall_sim_is_voice_agent_registered((sys_vsid_type)grant_ptr->handle))
      {
        ECALL_MSG_1( ECALL_ERROR, "ecall_voice_api(): Grant: Invalid handle %d", grant_ptr->handle );
        return ECALL_VOICE_EHANDLE;
      }

      /* Queue a command to eCall IVS task to process this grant from Voice Agent */
      cmd_ptr = ecall_get_cmd_buf();
      if (NULL == cmd_ptr)
      {
        ECALL_MSG_1( ECALL_ERROR, "ecall_voice_api(): Grant: Cannot allocate cmd_buf for cmd_id %d", cmd_id );
        return ECALL_VOICE_ENOMEMORY;
      }
      cmd_ptr->cmd_id = ECALL_CMD_VOICE_AGENT_GRANT;
      memscpy(&cmd_ptr->data.voice_agent_grant_data,
              sizeof(cmd_ptr->data.voice_agent_grant_data),
              params_ptr,
              size);
      ecall_put_cmd(cmd_ptr);
...
/*==========================================================================
  ...
  FUNCTION ECALL_MODEM_PROCESS_VOICE_AGENT_GRANT
...
===========================================================================*/
void ecall_modem_process_voice_agent_grant
(
  void
)
{
  ...
  ecall_modem_voice_agent_granted = TRUE;
  ecall_voc_register(ecall_modem_ivs.voc_re_register_mode);
}
