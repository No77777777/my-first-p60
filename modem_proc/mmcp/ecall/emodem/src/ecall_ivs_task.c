...
$Header: //components/rel/mmcp.mpss/7.9.0/ecall/emodem/src/ecall_ivs_task.c#1 $
    ...
    switch(cmd_ptr->cmd_id)
      {
        ...
        case ECALL_CMD_VOICE_AGENT_REGISTER:
          ecall_sim_process_voice_agent_register(cmd_ptr->data.voice_agent_register_data);
          break;

        case ECALL_CMD_VOICE_AGENT_DEREGISTER:
          ecall_sim_process_voice_agent_deregister(cmd_ptr->data.voice_agent_deregister_data);
          break;

        case ECALL_CMD_VOICE_AGENT_GRANT:
          ecall_modem_process_voice_agent_grant();
          break;
...
)
{
  ...
  if (mcfg_fs_stat("/nv/item_files/conf", &file_stat, MCFG_FS_TYPE_EFS, MCFG_FS_SUBID_0) != MCFG_FS_STATUS_OK)
  {
    if (mcfg_fs_stat("/nv/item_files", &file_stat, MCFG_FS_TYPE_EFS, MCFG_FS_SUBID_0) != MCFG_FS_STATUS_OK )
    {
      if (mcfg_fs_stat("/nv", &file_stat, MCFG_FS_TYPE_EFS, MCFG_FS_SUBID_0) != MCFG_FS_STATUS_OK )
      {
        (void)mcfg_mkdir("/nv", ECALL_IVS_PERMISSIONS, MCFG_FS_TYPE_EFS);
      }
      (void)mcfg_mkdir("/nv/item_files", ECALL_IVS_PERMISSIONS, MCFG_FS_TYPE_EFS);
    }
    (void)mcfg_mkdir("/nv/item_files/conf", ECALL_IVS_PERMISSIONS, MCFG_FS_TYPE_EFS);
  }

  if (mcfg_fs_stat("/nv/item_files/modem/ecall", &file_stat, MCFG_FS_TYPE_EFS, MCFG_FS_SUBID_0) != MCFG_FS_STATUS_OK)
  {
    if (mcfg_fs_stat("/nv/item_files/modem", &file_stat, MCFG_FS_TYPE_EFS, MCFG_FS_SUBID_0) != MCFG_FS_STATUS_OK )
    {
      (void)mcfg_mkdir("/nv/item_files/modem", ECALL_IVS_PERMISSIONS, MCFG_FS_TYPE_EFS);
    }
    (void)mcfg_mkdir("/nv/item_files/modem/ecall", ECALL_IVS_PERMISSIONS, MCFG_FS_TYPE_EFS); 
  }

  /* Create ecall.conf file for QPST to copy the NV item to QCN */
  efs_status = mcfg_fs_stat(ecall_conf_fpath, &file_stat, MCFG_FS_TYPE_EFS, MCFG_FS_SUBID_0);
  if( efs_status != MCFG_FS_STATUS_OK )
  {
    ...
    conf_fd = mcfg_fopen( ecall_conf_fpath,
                          MCFG_FS_O_WRONLY|MCFG_FS_O_CREAT|MCFG_FS_O_TRUNC,
                          ECALL_IVS_PERMISSIONS,
                          MCFG_FS_TYPE_EFS, MCFG_FS_SUBID_0 );

    if ( conf_fd >= 0 )
    {
      (void)mcfg_fwrite(conf_fd, (void*)ecall_config_items, strlen(ecall_config_items), MCFG_FS_TYPE_EFS);
    }
    else
    {
  ...
  /* Open/Read file */
  efs_status = mcfg_fs_read( ecall_mode_nv,
  ...
  {
    ECALL_MSG_1( ECALL_HIGH, "mcfg_fs_read error %d",
                              efs_status );

    ecall_ivs.ecall_nv_enable = FALSE;

    /* Open/Write file */
    efs_status = mcfg_fs_write( ecall_mode_nv,
                                &ecall_ivs.ecall_nv_enable,
                                sizeof(ecall_ivs.ecall_nv_enable),
    ...
    if (MCFG_FS_STATUS_OK != efs_status)
    {
      ECALL_MSG_2( ECALL_HIGH, "efs_put error = %d, efs_status = %d",
                                mcfg_fs_errno(MCFG_FS_TYPE_EFS),
                                efs_status );
    }
  }
...
)
{
  ...
  ECALL_MSG_1( ECALL_HIGH, "IVS Task: ECALL NV enabled %d",
                            ecall_ivs.ecall_nv_enable );
