...
$Header: //components/rel/mmcp.mpss/7.9.0/ecall/emodem/src/ecall_sim.c#1 $
...
/* Stores the VAGENT register info */
#define ECALL_VOICE_VSID_MAX 2
typedef struct
{
  boolean                               is_vsid_pending_register;
  ecall_voice_iresource_cmd_register_t  register_data;
} ecall_vagent_register_info;

/* ecall_sim_vagent_register_info[0] for VSID 0x11C05000
 * ecall_sim_vagent_register_info[1] for VSID 0x11DC5000
 */ 
static ecall_vagent_register_info ecall_sim_vagent_register_info[ECALL_VOICE_VSID_MAX];
...
)
{
  ...
  cfg_s_ptr->ecall_app1_info.slot_id              = MMGSDI_SLOT_1;

  cfg_s_ptr->ecall_app2_info.as_id                = SYS_MODEM_AS_ID_2;
  ...
  cfg_s_ptr->ecall_app2_info.slot_id              = MMGSDI_SLOT_2;

  cfg_s_ptr->ecall_app3_info.as_id                = SYS_MODEM_AS_ID_3;
  ...
  cfg_s_ptr->ecall_app3_info.slot_id              = MMGSDI_SLOT_3;
...
#ifdef FEATURE_ECALL_HAS_SINGLE_VOICE_SESSION
  memset(&ecall_sim_vagent_register_info, 0, sizeof(ecall_sim_vagent_register_info));
#endif /* FEATURE_ECALL_HAS_SINGLE_VOICE_SESSION*/
...
)
{
  ...
  ECALL_MSG_1( ECALL_HIGH, "num_instances %d is pending sending operating_mode to NAS", num_instances);

#ifdef FEATURE_ECALL_HAS_SINGLE_VOICE_SESSION
  ecall_register_voice_agent_if_pending();
#endif
...
#ifdef FEATURE_ECALL_HAS_SINGLE_VOICE_SESSION
/*===========================================================================
   FUNCTION:      ECALL_SIM_PROCESS_VOICE_AGENT_REGISTER
   ...
   SIDE EFFECTS:
     None
===========================================================================*/
void ecall_sim_process_voice_agent_register
(
  ecall_voice_iresource_cmd_register_t register_data
)
{
  ...
  appInfo_ptr = ecall_sim_get_appinfo_by_vsid(register_data.vsid);
  if (NULL == appInfo_ptr)
  {
    ECALL_MSG_1( ECALL_ERROR, "ecall_sim_process_voice_agent_register(): Cannot find mapped appInfo for vsid %X, will try to register when VSID is available later",
                               register_data.vsid );
    if( 0x11C05000 == register_data.vsid )
    {
      ecall_sim_vagent_register_info[0].is_vsid_pending_register = TRUE;
      ecall_sim_vagent_register_info[0].register_data = register_data;
    }
    else if( 0x11DC5000 == register_data.vsid )
    {
      ecall_sim_vagent_register_info[1].is_vsid_pending_register = TRUE;
      ecall_sim_vagent_register_info[1].register_data = register_data;
    }
    return;
  }
  ...
  *register_data.ret_handle_ptr = register_data.vsid;
  ...
  ECALL_MSG_2(ECALL_HIGH, "ecall_sim_process_voice_agent_register(): Successfully register with vsid %X, as_id %d",
                           register_data.vsid,
                           appInfo_ptr->as_id);
  appInfo_ptr->is_voice_agent_registered       = TRUE;
  appInfo_ptr->voice_agent_event_callback      = register_data.event_cb;
  appInfo_ptr->voice_agent_session_context_ptr = register_data.session_context_ptr;

} /* ECALL_SIM_PROCESS_VOICE_AGENT_REGISTER */

/*===========================================================================
   FUNCTION:      ECALL_SIM_PROCESS_VOICE_AGENT_DEREGISTER
   ...
   SIDE EFFECTS:
     None
===========================================================================*/
void ecall_sim_process_voice_agent_deregister
(
...
)
{
  ...
  ECALL_MSG_2(ECALL_HIGH, "ecall_sim_process_voice_agent_deregister(): handle %d, as_id %d", deregister_data.handle, as_id);

  appInfo_ptr = ecall_sim_get_appinfo_by_as_id(as_id);
  if (NULL == appInfo_ptr)
  {
    ECALL_MSG_1( ECALL_ERROR, "ecall_sim_process_voice_agent_deregister(): Cannot find mapped appInfo for as_id %d",
                               as_id );
    return;
  }

  /* Reset the eCall SIM appInfo accordingly */
  appInfo_ptr->is_voice_agent_registered       = FALSE;
  appInfo_ptr->voice_agent_event_callback      = NULL;
  appInfo_ptr->voice_agent_session_context_ptr = NULL;

} /* ECALL_SIM_PROCESS_VOICE_AGENT_DEREGISTER */
...
===========================================================================*/
void ecall_sim_request_voice_agent_grant
(
  void
)
{
  ...
  {
    ECALL_MSG_1( ECALL_ERROR, "Request Voice Agent Grant: Unmapped ecall_slot_id %d",
                               ecall_slot_id );
    return;
  }
  ...
  if (NULL == appInfo_ptr)
  {
    ECALL_MSG_0( ECALL_ERROR, "Request Voice Agent Grant: NULL appInfo_ptr" );
    return;
  }

  if (NULL == appInfo_ptr->voice_agent_event_callback)
  {
    ECALL_MSG_0( ECALL_ERROR, "Request Voice Agent Grant: NULL voice_agent_event_callback");
    return;
  }

  if (NULL == appInfo_ptr->voice_agent_session_context_ptr)
  {
    ECALL_MSG_0( ECALL_ERROR, "Request Voice Agent Grant: voice_agent_session_context_ptr");
    return;
  }

  ECALL_MSG_1( ECALL_HIGH, "Request Voice Agent Grant: as_id %d", appInfo_ptr->as_id);
  appInfo_ptr->voice_agent_event_callback(appInfo_ptr->voice_agent_session_context_ptr,
...
/*==========================================================================
  ...
  FUNCTION ECALL_MODEM_REQUEST_VOICE_AGENT_RELEASE
...
===========================================================================*/
void ecall_sim_request_voice_agent_release
(
  void
)
{
  ...
  if ((mmgsdi_slot_id_enum_type)0 == ecall_slot_id)
  {
    ECALL_MSG_1( ECALL_ERROR, "Request Voice Agent Release: Unmapped ecall_slot_id %d",
                               ecall_slot_id );
    return;
  }
  ...
  if (NULL == appInfo_ptr)
  {
    ECALL_MSG_0( ECALL_ERROR, "Request Voice Agent Release: NULL appInfo_ptr" );
    return;
  }
  ...
  ECALL_MSG_1( ECALL_HIGH, "Request Voice Agent Release: as_id %d", appInfo_ptr->as_id);
  appInfo_ptr->voice_agent_event_callback(appInfo_ptr->voice_agent_session_context_ptr,
...
/*===========================================================================
   FUNCTION:      ECALL_SIM_GET_ECALL_VSID_CS_VOICE
   ...
   SIDE EFFECTS:
     None
===========================================================================*/
void ecall_sim_get_ecall_vsid_cs_voice
(
...
)
{
  ...
  ECALL_MSG_2( ECALL_MED, "sim_get_ecall_vsid_cs_voice(): as_id %d, vsid_cs_voice %X",
                           appInfo_ptr->as_id,
                           appInfo_ptr->vsid_cs_voice );
  *ret_vsid_ptr = appInfo_ptr->vsid_cs_voice;

} /* ECALL_SIM_GET_ECALL_VSID_CS_VOICE */

/*===========================================================================
   FUNCTION:      ECALL_SIM_IS_VOICE_AGENT_REGISTERED
   ...
   SIDE EFFECTS:
     None
===========================================================================*/
boolean ecall_sim_is_voice_agent_registered
(
  sys_vsid_type vsid
)
{
  ...
  appInfo_ptr = ecall_sim_get_appinfo_by_vsid(vsid);
  if (NULL == appInfo_ptr)
  {
    ECALL_MSG_1( ECALL_ERROR, "ecall_sim_is_voice_agent_registered(): Cannot find mapped appInfo for vsid %X",
                               vsid );
    return FALSE;
  }

  ECALL_MSG_3( ECALL_MED, "ecall_sim_is_voice_agent_registered(): vsid %X, as_id %d, registered %d",
                           vsid,
                           appInfo_ptr->as_id,
                           appInfo_ptr->is_voice_agent_registered );
  return appInfo_ptr->is_voice_agent_registered;
} /* ECALL_SIM_IS_VOICE_AGENT_REGISTERED */

/*===========================================================================
   FUNCTION:      ecall_register_voice_agent_if_pending

   DESCRIPTION:
     This function will register voice agent with eCall if it is pending.
   ...
   SIDE EFFECTS:
     None
===========================================================================*/
void ecall_register_voice_agent_if_pending(void)
{
  sys_modem_as_id_e_type as_id = SYS_MODEM_AS_ID_1;
  ecall_prov_app_info_s_type *appInfo_ptr = NULL;

  
  for(; as_id < SYS_MODEM_AS_ID_NO_CHANGE; as_id++)
  {
    appInfo_ptr = ecall_sim_get_appinfo_by_as_id(as_id);

    if (appInfo_ptr == NULL) { continue; }

    ECALL_MSG_6(ECALL_HIGH," as_id %d, is_vs_id_avail %d, is_reg %d, vs_id %X, pending register %d %d",
      as_id, appInfo_ptr->is_vsid_available, appInfo_ptr->is_voice_agent_registered, appInfo_ptr->vsid_cs_voice,
      ecall_sim_vagent_register_info[0].is_vsid_pending_register,
      ecall_sim_vagent_register_info[1].is_vsid_pending_register);
    if (appInfo_ptr->is_vsid_available && !appInfo_ptr->is_voice_agent_registered)
    {
      switch (appInfo_ptr->vsid_cs_voice)
      {
        case 0x11C05000:
          if (TRUE == ecall_sim_vagent_register_info[0].is_vsid_pending_register)
          {
            ecall_sim_vagent_register_info[0].is_vsid_pending_register = FALSE;
            ecall_sim_process_voice_agent_register(ecall_sim_vagent_register_info[0].register_data);
          }
          break;

        case 0x11DC5000:
          if (TRUE == ecall_sim_vagent_register_info[1].is_vsid_pending_register)
          {
            ecall_sim_vagent_register_info[1].is_vsid_pending_register = FALSE;
            ecall_sim_process_voice_agent_register(ecall_sim_vagent_register_info[1].register_data);
          }
          break;

        default:
          ECALL_MSG_1( ECALL_ERROR, "ecall_register_voice_agent_if_pending(): Unsupported VSID %X",
                                     appInfo_ptr->vsid_cs_voice );
          break;   
      }
...
)
{
  ...
  else
  {
    ...
    ECALL_MSG_4( ECALL_HIGH, "MMGSDI session opened: session_type %d, session_id 0x%x, slot_id %d, app_type %d",
...
/*==============================================================================
    FUNCTION NAME  ECALL_SIM_GET_APPINFO_BY_VSID
    ...
    OUTPUT
        None
==============================================================================*/
ecall_prov_app_info_s_type* ecall_sim_get_appinfo_by_vsid
(
  sys_vsid_type  vsid
)
{
  ...
  if (vsid == lcfg_s_ptr->ecall_app1_info.vsid_cs_voice)
  {
    info_ptr = &lcfg_s_ptr->ecall_app1_info;
  }
  else if (vsid == lcfg_s_ptr->ecall_app2_info.vsid_cs_voice)
  {
    info_ptr = &lcfg_s_ptr->ecall_app2_info;
  }
  else if (vsid == lcfg_s_ptr->ecall_app3_info.vsid_cs_voice)
  {
    info_ptr = &lcfg_s_ptr->ecall_app3_info;
  }
  ...
  if (NULL == info_ptr)
  {
    ECALL_MSG_1( ECALL_ERROR, "ecall_sim_get_appinfo_by_vsid(): Unmapped vsid %X",
                               vsid );
  }

  return info_ptr;
} /* ecall_sim_get_appinfo_by_vsid() */
...
)
{
  ...
  ECALL_MSG_2( ECALL_HIGH, "card_error_event_proc(): Move ecall_operating_mode to UNKNOWN for as_id %d, slot_id %d",
                            appInfo_ptr->as_id,
                            slot_id );
...
)
{
  ...
  if ( appInfo_ptr == NULL )
  {
    /* Not a eCall related refresh event */
    return;
  }

  ECALL_MSG_3( ECALL_HIGH, "eCall related Refresh Event with session_id 0x%x, session_type %d, REFRESH_STAGE %d",
...
)
{
  ...
  ECALL_MSG_1( ECALL_HIGH, "Received MMGSDI EVT %d",
                            event.evt );
  switch (event.evt)
  {
...
)
{
  ...
  if (NULL != appInfo_ptr)
  {
    ...
    if (slot_id == MMGSDI_MAX_SLOT_ID_ENUM)
	{
	   ...
	   {
			appInfo_ptr->slot_id                 = MMGSDI_SLOT_1;
	   }
	   else
	   {
            appInfo_ptr->slot_id                 = MMGSDI_SLOT_2;
	   }
	}
	else
	{
