...
$Header: //components/rel/mmcp.mpss/7.9.0/ecall/emodem/src/ecall_modem_psap.c#1 $
...
#ifdef FEATURE_ECALL_HAS_SINGLE_VOICE_SESSION
#include "ecall_voice_if.h"
#endif /* FEATURE_ECALL_HAS_SINGLE_VOICE_SESSION */
...
struct
{
  ...
  #ifdef FEATURE_ECALL_HAS_SINGLE_VOICE_SESSION
  boolean is_voice_agent_registered;
  boolean voice_agent_granted;
  ecall_icommon_event_callback_fn_t  voice_agent_event_callback;      // Callback function for eCall to request grant
  void*                              voice_agent_session_context_ptr; // Session context provided by Voice Agent
  #endif /* FEATURE_ECALL_HAS_SINGLE_VOICE_SESSION */
...
static uint16 ecall_modem_apr_src_addr = 0;
...
#ifdef FEATURE_ECALL_HAS_SINGLE_VOICE_SESSION
#define ECALL_VSS_STREAM_CHARACTER_MAX  31
static char_t ecall_modem_stream_name[ECALL_VSS_STREAM_CHARACTER_MAX];
static char    ecall_modem_psap_voc_dns[] = "qcom.audio.cvs";
#else
...
typedef enum
{
  ...
  ECALL_PSAP_APR_DESTROY_SESSION,
  ECALL_PSAP_APR_SET_ENC_DTX_MODE
...
)
{
  ...
  {
    ECALL_MSG_0( ECALL_ERROR, "psap_voc_restore(): dest_port is 0" );
    return;
  }
  
  /*-----------------------------------------------------------------------*/
  /*         Release encoder DTX control after MSD transmission            */
  /*-----------------------------------------------------------------------*/

  /* Release encoder DTX control after MSD transmission.
  ** Vocoder will release the exclusive DTX control when eCall calls this API
  ** with TRUE, and resume the cached DTX value from vocoder itself.
  */
  ecall_modem_psap_set_enc_dtx_mode(TRUE);
...
)
{
  ...
  if(ecall_modem_apr_dest_port == 0)
  {
    ECALL_MSG_0( ECALL_ERROR, "voc_configure(): dest_port is 0" );
    return;
  }

  /*-----------------------------------------------------------------------*/
  /*         Request encoder DTX control for MSD transmission              */
  /*-----------------------------------------------------------------------*/

  /* Disable Encoder DTX Mode for MSD Transmission.
  ** Vocoder will give eCall exclusive control for DTX Off when eCall calls
  ** this API with FALSE, and release control with TRUE.
  */
  ecall_modem_psap_set_enc_dtx_mode(FALSE);
...
)
{
  ...
  memset(&ecall_modem_psap, 0, sizeof(ecall_modem_psap));
  ...
  #ifdef FEATURE_ECALL_HAS_SINGLE_VOICE_SESSION
  ecall_modem_psap.is_voice_agent_registered = FALSE;
  ecall_modem_psap.voice_agent_granted = FALSE;
  #endif /* FEATURE_ECALL_HAS_SINGLE_VOICE_SESSION */
}
...
#ifdef FEATURE_ECALL_HAS_SINGLE_VOICE_SESSION
/*===========================================================================
   FUNCTION:      ECALL_MODEM_PSAP_GET_ECALL_VSID_CS_VOICE

   DESCRIPTION:
     This function will get the eCall VSID for CS Voice.
   ...
   SIDE EFFECTS:
     None
===========================================================================*/
void ecall_modem_psap_get_ecall_vsid_cs_voice
(
  sys_vsid_type *ret_vsid_ptr
);
#endif /* FEATURE_ECALL_HAS_SINGLE_VOICE_SESSION */
...
#ifdef FEATURE_ECALL_HAS_SINGLE_VOICE_SESSION
  {
    sys_vsid_type vsid_cs_voice;
    ecall_modem_psap_get_ecall_vsid_cs_voice(&vsid_cs_voice);
    (void)ecall_itoa(vsid_cs_voice, ecall_modem_stream_name, 16);
  }
#endif /* FEATURE_ECALL_HAS_SINGLE_VOICE_SESSION */
...
/*==========================================================================
  FUNCTION ECALL_MODEM_DESTROY_SESSION

  DESCRIPTION
    This function will send the destroy session command thru APR.
...
===========================================================================*/
void ecall_modem_psap_destroy_session
(
  void
)
{
  ...
  ret = __aprv2_cmd_alloc_send(
            ...
            ecall_modem_apr_dest_addr,             /* dst_addr     */
            ecall_modem_apr_dest_port,             /* dst_port     */
            ECALL_PSAP_APR_DESTROY_SESSION,        /* token        */
            APRV2_IBASIC_CMD_DESTROY_SESSION,      /* opcode       */
            NULL,                                  /* payload      */
            0 );                                   /* payload_size */
  ...
  switch (packet->token)
  {
    ...
    case ECALL_APR_UNMAP_MEMORY:
      if(status == APR_EOK)
      {
        ...
        ecall_modem_psap_destroy_session();
      }
      else
      {
    ...
    case ECALL_PSAP_APR_SET_ENC_DTX_MODE:
      if (status == APR_EOK)
      {
        ECALL_MSG_0( ECALL_HIGH, "Successfully set encoder DTX mode" );
      }
      else
      {
        ECALL_MSG_1( ECALL_ERROR, "Failed to set encoder DTX mode, error code %d",
                                   status );
      }
      __aprv2_cmd_free(ecall_modem_apr_handle, packet);
      break;

    case ECALL_PSAP_APR_DESTROY_SESSION:
      if (status == APR_EOK)
      {
        ECALL_MSG_0( ECALL_HIGH, "Successfully destroy the CVS session, reset the port" );
        ecall_modem_apr_dest_port = 0;
      #ifdef FEATURE_ECALL_HAS_SINGLE_VOICE_SESSION
        ecall_modem_psap.voice_agent_granted = FALSE;
        ecall_modem_psap_request_voice_agent_release();
      #endif /* FEATURE_ECALL_HAS_SINGLE_VOICE_SESSION */
      }
      else
      {
      ...
      __aprv2_cmd_free(ecall_modem_apr_handle, packet);
      /* Release the Vocoder from Voice Agent */
      break;
...
)
{
  ...
  #ifdef FEATURE_ECALL_HAS_SINGLE_VOICE_SESSION
  if (ecall_modem_psap.voice_agent_granted == FALSE)
  {
    ECALL_MSG_0( ECALL_MED, "voc_reg: Voice Agent not yet grant. Will request grant then init APR and create passive control session to get port");
    ecall_modem_psap_request_voice_agent_grant();
    return;
  }
  #endif /* FEATURE_ECALL_HAS_SINGLE_VOICE_SESSION */

  if (ecall_modem_apr_dest_addr == 0)
  {
     /* Initialize APR interface for message routing */
    ECALL_MSG_0( ECALL_MED, "voc_reg: ecall_modem_apr_dest_addr == 0. Will init APR then create passive control session to get port");
  ...
  if (ecall_modem_apr_dest_port == 0)
  {
    /* Initialize CVS stream to get the dest port
       will call ecall_modem_map_shared_memory after successfully created cvs session */
    ECALL_MSG_1( ECALL_MED, "voc_reg: dest_port is 0(%d). Will create passive control session to get port",
                             ecall_modem_apr_dest_port );
    ecall_modem_cvs_init();
    return;
  }
...
)
{
  ...
  ECALL_MSG_0( ECALL_MED, "ecall_modem_apr_init: APR init completed" );
...
)
{
  ...
  switch (packet->opcode)
  {
    ...
    case VSS_IMEMORY_RSP_MAP:
      /* Will free the cmd in the function */
      ecall_modem_psap_process_imemory_rsp_map(packet);
      break;
...
/*===========================================================================
  ...
  FUNCTION ECALL_MODEM_PSAP_SET_ENC_DTX_MODE

  DESCRIPTION
    This function will set the encoder DTX mode.
...
===========================================================================*/
void ecall_modem_psap_set_enc_dtx_mode
( 
  boolean enable
)
{
  int32 ret;
  vss_istream_cmd_set_enc_dtx_mode_t set_dtx_mode_cmd;

  ECALL_MSG_1( ECALL_HIGH,"Request to set Encoder DTX Mode to %d", enable );

  memset(&set_dtx_mode_cmd, 0, sizeof(set_dtx_mode_cmd));
  set_dtx_mode_cmd.enable = enable;
  ...
  ret = __aprv2_cmd_alloc_send(
            ...
            ecall_modem_apr_dest_addr,                                 /* dst_addr     */
            ecall_modem_apr_dest_port,                                 /* dst_port     */
            ECALL_PSAP_APR_SET_ENC_DTX_MODE,                           /* token        */ 
            VSS_ISTREAM_CMD_SET_ENC_DTX_MODE,                          /* opcode       */
            &set_dtx_mode_cmd,                                         /* payload      */
            sizeof(set_dtx_mode_cmd));                                 /* payload_size */
  
  if (ret != APR_EOK)
  {
    ECALL_MSG_1( ECALL_ERROR, "__aprv2_cmd_alloc_send(SET_ENC_DTX_MODE) failed, ret=%d",
                               ret );
  }

} /* ECALL_MODEM_PSAP_SET_ENC_DTX_MODE */

/*==========================================================================
  FUNCTION ECALL_MODEM_PSAP_PROCESS_IMEMORY_RSP_MAP
...
===========================================================================*/
void ecall_modem_psap_process_imemory_rsp_map
(
  aprv2_packet_t* packet
)
{
  ...
  ECALL_MSG_1( ECALL_HIGH, "Map memory successfully with handle: %d, register for vocoder now",
                            rsp_map->mem_handle );
  ecall_modem_psap.ecall_memory_handle = rsp_map->mem_handle;
  ecall_modem_psap.smem_mapped = TRUE;
  ecall_voc_register();

  __aprv2_cmd_free(ecall_modem_apr_handle, packet );

} /* ECALL_MODEM_PSAP_PROCESS_IVPCM_EVT_NOTIFY */
...
/*==========================================================================
  ...
  FUNCTION ECALL_MODEM_PSAP_PROCESS_VOICE_AGENT_GRANT

  DESCRIPTION
    Process the Grant event from Voice Agent.
  ...
  RETURN VALUE
    ...
    @return                ECALL_VOICE_IF_EOK (0) when successful

  SIDE EFFECTS
    None.

===========================================================================*/
void ecall_modem_psap_process_voice_agent_grant
(
  void
)
{
  ECALL_MSG_0( ECALL_HIGH, "ECALL_CMD_VOICE_AGENT_GRANT" );
  /* Update the Voice Agent grant status */
  ecall_modem_psap.voice_agent_granted = TRUE;
  ecall_voc_register();
}

/*===========================================================================
   FUNCTION:      ECALL_MODEM_PSAP_PROCESS_VOICE_AGENT_REGISTER

   DESCRIPTION:
     This function will process the Voice Agent register request.
   ...
   SIDE EFFECTS:
     None
===========================================================================*/
void ecall_modem_psap_process_voice_agent_register
(
  ecall_voice_iresource_cmd_register_t register_data
)
{
  /* Assign the vsid to the client's returned handle.         *
   * Voice Agent will use this handle for subsequence commands */
  *register_data.ret_handle_ptr = register_data.vsid;

  /* Update the eCall SIM appInfo accordingly */
  ECALL_MSG_1(ECALL_HIGH, "ecall_modem_psap_process_voice_agent_register(): Successfully register with vsid %X",
                           register_data.vsid);
  ecall_modem_psap.is_voice_agent_registered       = TRUE;
  ecall_modem_psap.voice_agent_event_callback      = register_data.event_cb;
  ecall_modem_psap.voice_agent_session_context_ptr = register_data.session_context_ptr;

} /* ECALL_MODEM_PSAP_PROCESS_VOICE_AGENT_REGISTER */

/*===========================================================================
   FUNCTION:      ECALL_PSAP_PROCESS_VOICE_AGENT_DEREGISTER

   DESCRIPTION:
     This function will process the Voice Agent deregister request.
   ...
   SIDE EFFECTS:
     None
===========================================================================*/
void ecall_modem_psap_process_voice_agent_deregister
(
  ecall_voice_iresource_cmd_deregister_t deregister_data
)
{
  sys_modem_as_id_e_type      as_id       = (sys_modem_as_id_e_type)deregister_data.handle;

  ECALL_MSG_2(ECALL_HIGH, "ecall_psap_process_voice_agent_deregister(): handle %d, as_id %d", deregister_data.handle, as_id);

  ecall_modem_psap.is_voice_agent_registered       = FALSE;

} /* ECALL_MODEM_PSAP_PROCESS_VOICE_AGENT_DEREGISTER */
...
{
  *ret_vsid_ptr = 0x11c05000;

} /* ECALL_MODEM_PSAP_GET_ECALL_VSID_CS_VOICE */

/*===========================================================================
   FUNCTION:      ECALL_MODEM_PSAP_IS_VOICE_AGENT_REGISTERED

   DESCRIPTION:
     This function will return the Voice Agent registration status
     with the given VSID.
   ...
   SIDE EFFECTS:
     None
===========================================================================*/
boolean ecall_modem_psap_is_voice_agent_registered
(
  sys_vsid_type vsid
)
{
  ECALL_MSG_2( ECALL_MED, "ecall_modem_psap_is_voice_agent_registered(): vsid %X, registered %d",
                           vsid,
                           ecall_modem_psap.is_voice_agent_registered );
  return ecall_modem_psap.is_voice_agent_registered;
} /* ECALL_MODEM_PSAP_IS_VOICE_AGENT_REGISTERED */


/*==========================================================================

  FUNCTION ECALL_MODEM_REQUEST_VOICE_AGENT_GRANT

  DESCRIPTION
    This function will use the Voice Agent callback function to request
    for voice session grant.
...
===========================================================================*/
void ecall_modem_psap_request_voice_agent_grant
(
  void
)
{
  if (NULL == ecall_modem_psap.voice_agent_event_callback)
  {
    ECALL_MSG_0( ECALL_ERROR, "Request Voice Agent Grant: NULL voice_agent_event_callback");
    return;
  }

  if (NULL == ecall_modem_psap.voice_agent_session_context_ptr)
  {
    ECALL_MSG_0( ECALL_ERROR, "Request Voice Agent Grant: voice_agent_session_context_ptr");
    return;
  }

  ECALL_MSG_0( ECALL_HIGH, "Request Voice Agent Grant");
  ecall_modem_psap.voice_agent_event_callback(ecall_modem_psap.voice_agent_session_context_ptr,
                                          ECALL_VOICE_IRESOURCE_EVENT_REQUEST,
                                          NULL,
                                          0);

} /* ECALL_MODEM_REQUEST_VOICE_AGENT_GRANT */

/*==========================================================================

  FUNCTION ECALL_MODEM_PSAP_REQUEST_VOICE_AGENT_RELEASE

  DESCRIPTION
    This function will use the Voice Agent callback function to request
    for voice session release.
...
===========================================================================*/
void ecall_modem_psap_request_voice_agent_release
(
  void
)
{
  if (NULL == ecall_modem_psap.voice_agent_event_callback)
  {
    ECALL_MSG_0( ECALL_ERROR, "Request Voice Agent Release: NULL voice_agent_event_callback");
    return;
  }

  if (NULL == ecall_modem_psap.voice_agent_session_context_ptr)
  {
    ECALL_MSG_0( ECALL_ERROR, "Request Voice Agent Release: voice_agent_session_context_ptr");
    return;
  }

  ECALL_MSG_0( ECALL_HIGH, "Request Voice Agent Release");
  ecall_modem_psap.voice_agent_event_callback(ecall_modem_psap.voice_agent_session_context_ptr,
                                          ECALL_VOICE_IRESOURCE_EVENT_RELEASED,
                                          NULL,
                                          0);

} /* ECALL_MODEM_REQUEST_VOICE_AGENT_RELEASE */
...
/*==========================================================================
  ...
  FUNCTION ECALL_MODEM_PSAP_VOICE_API
...
===========================================================================*/
uint32_t ecall_modem_psap_voice_api
(
  uint32_t  cmd_id,
  void*     params_ptr,
  uint32_t  size
...
{
  uint32_t        status   = ECALL_VOICE_EBADPARAM;
  ecall_cmd_type *cmd_ptr  = NULL;

  ecall_voice_iresource_cmd_register_t   *register_ptr = NULL;
  ecall_voice_iresource_cmd_deregister_t *deregister_ptr = NULL;
  ecall_voice_iresource_cmd_grant_t      *grant_ptr = NULL;
  ecall_voice_iresource_cmd_register_t   register_data;
  ecall_voice_iresource_cmd_deregister_t deregister_data;
  ecall_voice_iresource_cmd_grant_t      grant_data;

  if (size > 0)
  {
    if (params_ptr == NULL)
    {
      ECALL_MSG_1( ECALL_ERROR, "Null Params received for cmd_id 0x%x", cmd_id );
      return ECALL_VOICE_EBADPARAM;
    }
  }
  else if (params_ptr != NULL)
  {
    ECALL_MSG_1( ECALL_ERROR, "Invalid Size for non NULL params, cmd_id 0x%x", cmd_id );
    return ECALL_VOICE_EBADPARAM;
  }

  ECALL_MSG_1( ECALL_HIGH, "ecall_voice_api(): cmd_id 0x%x", cmd_id );
  
  switch (cmd_id)
  {
    case ECALL_VOICE_IRESOURCE_CMD_REGISTER:
      /* Validate Register parameters */
      register_ptr = (ecall_voice_iresource_cmd_register_t*)params_ptr;
      if ( ( register_ptr == NULL ) || ( size != sizeof( ecall_voice_iresource_cmd_register_t ) ) ) 
      {
        return ECALL_VOICE_EBADPARAM;
      }
      if ( (0x11C05000 != (int)register_ptr->vsid ) &&
           (0x11DC5000 != (int)register_ptr->vsid ) )
      {
        ECALL_MSG_1( ECALL_ERROR, "Register: Invalid vsid %X", register_ptr->vsid );
        return ECALL_VOICE_EBADPARAM;
      }

      /* Check if Voice Agent already registered with this as_id */
      if (TRUE == ecall_modem_psap_is_voice_agent_registered(register_ptr->vsid) )
      {
        ECALL_MSG_1( ECALL_ERROR, "ecall_voice_api(): Register: Already registered with vsid %X", register_ptr->vsid );
        return ECALL_VOICE_EALREADY;
      }

      ecall_memscpy(&register_data,
              sizeof(register_data),
              params_ptr,
              size);
      ecall_modem_psap_process_voice_agent_register(register_data);
      status = ECALL_VOICE_EOK;
      break;

    case ECALL_VOICE_IRESOURCE_CMD_DEREGISTER:
      deregister_ptr = (ecall_voice_iresource_cmd_deregister_t*)params_ptr;
      if ((deregister_ptr == NULL) || (size != sizeof(ecall_voice_iresource_cmd_deregister_t)))
      {
        return ECALL_VOICE_EBADPARAM;
      }
      if ( (0x11C05000 != (int)deregister_ptr->handle) &&
           (0x11DC5000 != (int)deregister_ptr->handle) )
      {
        ECALL_MSG_1( ECALL_ERROR, "ecall_voice_api(): Deregister: Invalid as_id(handle) %d", deregister_ptr->handle );
        return ECALL_VOICE_EHANDLE;
      }

      if (FALSE == ecall_modem_psap_is_voice_agent_registered((sys_vsid_type)deregister_ptr->handle))
      {
        ECALL_MSG_1( ECALL_ERROR, "ecall_voice_api(): Deregister: Invalid handle %d", deregister_ptr->handle );
        return ECALL_VOICE_EHANDLE;
      }

      ecall_memscpy(&deregister_data,
              sizeof(deregister_data),
              params_ptr,
              size);
      ecall_modem_psap_process_voice_agent_deregister(deregister_data);
      
      status = ECALL_VOICE_EOK;
      break;

    case ECALL_VOICE_IRESOURCE_CMD_GRANT:
      grant_ptr = (ecall_voice_iresource_cmd_grant_t*)params_ptr;
      if ((grant_ptr == NULL) || (size != sizeof(ecall_voice_iresource_cmd_grant_t)))
      {
        return ECALL_VOICE_EBADPARAM;
      }
      if ( (0x11C05000 != (int)grant_ptr->handle) &&
           (0x11DC5000 != (int)grant_ptr->handle) )
      {
        ECALL_MSG_1( ECALL_ERROR, "ecall_voice_api(): Grant: Invalid as_id(handle) %d", grant_ptr->handle );
        return ECALL_VOICE_EHANDLE;
      }
      if (FALSE == ecall_modem_psap_is_voice_agent_registered((sys_vsid_type)grant_ptr->handle))
      {
        ECALL_MSG_1( ECALL_ERROR, "ecall_voice_api(): Grant: Invalid handle %d", grant_ptr->handle );
        return ECALL_VOICE_EHANDLE;
      }

      ecall_memscpy(&grant_data,
              sizeof(grant_data),
              params_ptr,
              size);
      ecall_modem_psap_process_voice_agent_grant();
      
      status = ECALL_VOICE_EOK;
      break;

    default:
      ECALL_MSG_3( ECALL_ERROR, "ecall_voice_api(): Unsupported command (0x%x, 0x%x) size %d", cmd_id, (uint32)(params_ptr), size);
      status = ECALL_VOICE_EUNSUPPORTED;
      break;
  }
...
{
  return ecall_modem_psap_voice_api(cmd_id, params_ptr, size);
}
...
#else /* FEATURE_ECALL_HAS_SINGLE_VOICE_SESSION not defined */
/*==========================================================================
