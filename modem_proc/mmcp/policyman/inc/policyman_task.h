...
/*
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/policyman/inc/policyman_task.h#1 $
...
@param  clientMsg    Client message which has Pointer to a message to be returned 
                             to a blocked caller, if command was issued as a result of a blocking call.
@param  pState       Pointer to policyman state which got updated with the new info in command
*/
...
/*------- policyman_qsh_dump_collect -------*/
/**
@brief  wrapper around qsh api to dump raw data
*/
void policyman_qsh_dump_collect(
  void*    addr, 
  size_t   size
);
