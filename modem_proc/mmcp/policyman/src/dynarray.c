...
/*
    Copyright (c) 2013-2014,2016,2020 QUALCOMM Technologies Incorporated.
    All Rights Reserved.
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/policyman/src/dynarray.c#1 $
...
)
{
  ...
  dyn_byte_array_ctor(pArray, initSize, maxSize);
...
)
{
  ...
  *pClone = *pArray;
  ...
  dyn_byte_array_clone_buffer(pArray, pClone);
...
QSH_MDUMP_FN_ATTR_PM byte * dyn_byte_array_get_array_ptr(
  dyn_byte_array_t  *pArray
)
{
  return pArray->pBuf;
...
QSH_MDUMP_FN_ATTR_PM byte * dyn_byte_array_get_ptr_to_offset(
  dyn_byte_array_t  *pArray,
  size_t             offset
)
{
