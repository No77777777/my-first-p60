...
/*
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/policyman/src/policyman_timer.c#1 $
...
)
{
  ...
  pTimer = (policyman_timer_t *) mre_named_object_find_with_subs( pName,
                                                                 MRE_NAMED_TIMER,
                                                                 SYS_MODEM_AS_ID_1,
...
/*-------- policyman_timer_get_timer_per_subs --------*/
policyman_timer_t * policyman_timer_get_timer_per_subs(
  char const            *pName,
...
)
{
   ...
   */
  tableSize = mre_set_num_elems(pTimer->pBackoffTable);
   ...
   *  to that value.
   */
  pElem = (size_t *) mre_set_elem_ptr(pTimer->pBackoffTable, index);
  if (pElem == NULL)
  {
...
)
{
  ...
  POLICYMAN_MSG_HIGH_3( LOG_SUBS_PREFIX "policyman_timer_cb called: timer %d expired",
                        LOG_SUBS(pTimer->asubs_id),
                        POLICY_SUBS_SAFE(pTimer->pPolicy),
                        pTimer->id );
...
{
  policyman_timer_condition_t  *pCond   = (policyman_timer_condition_t *)pCondition;
  mre_policy_t                        *pPolicy = EXECUTE_CTX_POLICY(pCtx);
  policyman_timer_t                   *pTimer = NULL;
  boolean                              result  = FALSE;

  if (pCond->pTimerName != NULL)
  {
    pTimer = (policyman_timer_t *)mre_named_object_find( pCond->pTimerName,
  ...
  if (pTimer != NULL)
  {
    result = pTimer->state == POLICYMAN_TIMER_STATE_EXPIRED;
    POLICYMAN_UT_MSG_6( LOG_SUBS_PREFIX SRC_LINE "condition <timer_expired> for subs %d timer %d returns %d",
                        LOG_SUBS(pTimer->asubs_id),
                        POLICY_SUBS(pPolicy),
                        LINE_NUM(pCond),
                        pTimer->asubs_id,
  ...
  {
    POLICYMAN_UT_MSG_3( SUBS_PREFIX SRC_LINE "condition <timer_expired> for no timer returns %d",
                        POLICY_SUBS(pPolicy),
                        LINE_NUM(pCond),
                        result );
  }
...
/*-------- policyman_condition_timer_dtor --------*/
static void policyman_condition_timer_dtor(
  void  *pObj
)
{
  policyman_timer_condition_t *pCondition = (policyman_timer_condition_t *) pObj;
...
)
{
  ...
  char const                          *pStr;
  policyman_timer_condition_t *pCondition;
   ...
   */
  pCondition = POLICYMAN_CONDITION_NEW( policyman_timer_condition_t,
                                        pPolicy,
                                        policyman_condition_timer_expired_evaluate,
                                        policyman_condition_timer_dtor,
                                        SUBS_THIS,
...
/*-------- evaluate_timer_is_running --------*/
STATIC boolean evaluate_timer_is_running(
  policyman_condition_t const *pCondition,
  void                        *pCtx,
  sys_modem_as_id_e_type       subsId
)
{
  ...
  if (NON_NULL(pTimer))
  {
    timerIsRunning = pTimer->state == POLICYMAN_TIMER_STATE_RUNNING;
    POLICYMAN_UT_MSG_6( LOG_SUBS_PREFIX SRC_LINE "condition <timer_is_running> for subs %d timer %d returns %d",
                        LOG_SUBS(pTimer->asubs_id),
                        POLICY_SUBS(pPolicy),
                        LINE_NUM(pCond),
                        pTimer->asubs_id,
  ...
  {
    POLICYMAN_UT_MSG_3( SUBS_PREFIX SRC_LINE "condition <timer_is_running> for no timer returns %d",
                        POLICY_SUBS(pPolicy),
                        LINE_NUM(pCond),
                        timerIsRunning );
  }

  POLICYMAN_MSG_HIGH_3( SUBS_PREFIX SRC_LINE "condition <timer_is_running> returns %d",
                        POLICY_SUBS(pPolicy),
                        LINE_NUM(pCond),
                        timerIsRunning );
...
/*-------- policyman_condition_timer_is_running_evaluate --------*/
STATIC boolean policyman_condition_timer_is_running_evaluate(
  mre_condition_t const *pCondition,
  void                  *pCtx
)
{
  ...
  return policyman_evaluate_cond(pCond, pCtx, evaluate_timer_is_running);
}


/*-------- policyman_condition_timer_is_running --------*/
mre_status_t policyman_condition_timer_is_running(
  mre_xml_element_t const        *pElem,
  mre_policy_t                   *pPolicy,
  mre_condition_t               **ppCondition
)
{
  ...
  if (IS_NULL(pStr))
  {
    POLICYMAN_MSG_HIGH_1(SUBS_PREFIX "<time_running> name required for condition", POLICY_SUBS(pPolicy));
    goto Done;
  }
...
Done:
  POLICYMAN_MSG_MED_2( SUBS_PREFIX "policyman_condition_is_timer_running_new returned status %d",
                       POLICY_SUBS(pPolicy),
                       status );
...
)
{
  ...
  MRE_RELEASE_IF(pMe->pBackoffTable);
...
)
{
  ...
  pTimer = MRE_NAMED_OBJECT_NEW( policyman_timer_t,
                                 policyman_timer_dtor,
                                 POLICYMAN_NAMED_TIMER,
  ...
  (void) mre_named_object_add( (mre_named_object_t *) pTimer,
                                pTimerAction->ns,
                                pPolicy );
...
static mre_set_t * get_timer_backoff_table(
  mre_xml_element_t const *pElem,
  size_t                         multiplier
)
{
  char const      *pStr;
  mre_set_t       *pTable     = NULL;
  char             token[20];
  size_t           interval;
  ...
  pTable = mre_set_new(sizeof(uint32), 5, MRE_SET_NO_MAX_SIZE, NULL, NULL);
  ...
  while (policyman_get_token(&pStr, token, sizeof(token)))
  {
    ...
    mre_set_append(pTable, &interval);
  }
...
)
{
  ...
  pTimer = policyman_timer_ensure(pTimerAction, pPolicy);
