...
/*
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/policyman/src/policyman_voice_domain_pref.c#1 $
...
)
{
  ...
  pItem = POLICYMAN_CFGITEM_NEW( POLICYMAN_ITEM_VOICE_DOMAIN_PREF,
                                 policyman_voice_domain_pref_item_t,
                                 policyman_simple_dtor,
                                 subsId );

  pItem->mandatory    = mandatory;
...
policyman_item_t * policyman_voice_domain_pref_clone(
  policyman_item_t const *pItem
)
{
  policyman_voice_domain_pref_item_t const *pVoiceDomain       = (policyman_voice_domain_pref_item_t const *)pItem;
  policyman_voice_domain_pref_item_t       *pVoiceDomainClone;

  pVoiceDomainClone = policyman_voice_domain_pref_item_new( pVoiceDomain->voice_domain,
                                                            pVoiceDomain->mandatory,
                                                            pVoiceDomain->asubs_id);

  return (policyman_item_t *)pVoiceDomainClone;
}
...
)
{
  ...
  if (policyman_retrieve_voice_domain(subsId, &voiceDomain) )
  {
  ...
  else
  {
    ...
    voiceDomain = SYS_VOICE_DOMAIN_PREF_CS_VOICE_ONLY;

#elif (defined(FEATURE_3GPP_CSFB) && defined (FEATURE_IP_CALL) && defined (FEATURE_LTE))

    voiceDomain = SYS_VOICE_DOMAIN_PREF_CS_VOICE_PREFERRED;

#elif (!defined(FEATURE_3GPP_CSFB) && defined (FEATURE_IP_CALL) && defined (FEATURE_LTE))

    voiceDomain = SYS_VOICE_DOMAIN_PREF_IMS_PS_VOICE_ONLY;

#else

    voiceDomain = SYS_VOICE_DOMAIN_PREF_NONE;
  ...
  pItem = policyman_voice_domain_pref_item_new( voiceDomain,
                                                FALSE,
                                                subsId);
...
)
{
  ...
  *pMandatory = pPref->mandatory;
  ...
  POLICYMAN_MSG_HIGH_5( SUBS_PREFIX "get_voice_domain_pref (from item 0x%08x, seqno %d) returned voice_domain_pref %d, mandatory %d",
                        pItem->asubs_id,
                        pItem,
                        pItem->seqno,
                        pPref->voice_domain,
...
/*-------- policyman_get_voice_domain_pref_for_subs --------*/
policyman_status_t policyman_get_voice_domain_pref_for_subs(
  sys_modem_as_id_e_type        subsId,
  sys_voice_domain_pref_e_type *voiceDomainPref,
  boolean                      *mandatory
)
{
  size_t                nItems;
  ...
  {
    policyman_get_voice_domain_pref( (policyman_item_t const *)pItem,
                                      voiceDomainPref,
                                      mandatory );
  }
...
)
{
  ...
  POLICYMAN_MSG_HIGH_5( SUBS_PREFIX "voice_domain_pref %d, mandatory %d, (item 0x%08x, seqno %d)",
                        pPref->asubs_id,
                        pPref->voice_domain,
                        pPref->mandatory,
                        pItem,
...
void policyman_voice_domain_pref_on_update(
  policyman_item_t  *pItem
)
{
  policyman_voice_domain_pref_item_t       *pPref = (policyman_voice_domain_pref_item_t *) pItem;
  policyman_diag_event_voice_domain_pref_t  event_voice_domain;

  event_voice_domain.voice_domain = pPref->voice_domain;
  event_voice_domain.mandatory    = pPref->mandatory;

  event_report_payload( EVENT_PM_VOICE_DOMAIN_PREF,
                        sizeof(event_voice_domain),
                        &event_voice_domain);
...
)
{
  ...
  {
    MRE_STR_VALUE_ENTRY("CS_ONLY",  SYS_VOICE_DOMAIN_PREF_CS_VOICE_ONLY),
    MRE_STR_VALUE_ENTRY("IMS_ONLY", SYS_VOICE_DOMAIN_PREF_IMS_PS_VOICE_ONLY),
    MRE_STR_VALUE_ENTRY("CS_PREF",  SYS_VOICE_DOMAIN_PREF_CS_VOICE_PREFERRED),
    MRE_STR_VALUE_ENTRY("IMS_PREF", SYS_VOICE_DOMAIN_PREF_IMS_PS_VOICE_PREFERRED)
  };
  ...
  *pVoiceDomain= SYS_VOICE_DOMAIN_PREF_CS_VOICE_ONLY;
  ...
  {
    MRE_LOOKUP_STR_VALUE(map, token, sys_voice_domain_pref_e_type, pVoiceDomain);
  }
...
)
{
  ...
  pSm = policyman_voice_domain_pref_item_new( pvdAction->voice_domain_pref,
                                              pvdAction->mandatory,
                                              POLICY_SUBS(pPolicy) );
  
  mre_set_replace(pItemSet, &pSm);
  ref_cnt_obj_release(pSm);

  POLICYMAN_MSG_HIGH_5( SUBS_PREFIX SRC_LINE "action <voice_domain_pref pmsubs=%d> set voice domain pref to %d, mandatory %d",
                        POLICY_SUBS(pPolicy),
                        LINE_NUM(pvdAction),
                        POLICY_SUBS(pPolicy),
                        pSm->voice_domain,
...
)
{
  ...
  mre_policy_t                            *pPolicy                = EXECUTE_CTX_POLICY(pCtx);
  sys_voice_domain_pref_e_type             currentVoiceDomainPref = SYS_VOICE_DOMAIN_PREF_NONE;
  boolean                                  mandatory              = FALSE;
  boolean                                  result = FALSE;
  policyman_status_t                       status;

  status = policyman_get_voice_domain_pref_for_subs( subsId, 
                                                     &currentVoiceDomainPref,
                                            &mandatory );
  
  if (POLICYMAN_SUCCEEDED(status))
  {
    result = (currentVoiceDomainPref == pCond->voice_domain_pref);
  }

Done:
  POLICYMAN_UT_MSG_6( SUBS_PREFIX SRC_LINE "condition <voice_domain_pref_is pmsubs=%d> returns %d (current %d, condition %d)",
                      POLICY_SUBS(pPolicy),
                      LINE_NUM(pCond),
                      pCond->pmSubsId,
                      result,
...
QSH_MDUMP_FN_ATTR_PM void policyman_voice_domain_pref_mdump(
  policyman_item_t const *pItem
)
{
  policyman_qsh_dump_collect((void *)pItem, sizeof(policyman_voice_domain_pref_item_t));
}
