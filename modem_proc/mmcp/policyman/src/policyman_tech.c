...
/*
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/policyman/src/policyman_tech.c#1 $
...
{
  policyman_tech_present_condition_t  *pCond    = (policyman_tech_present_condition_t *) pCondition;
  mre_policy_t                        *pPolicy  = EXECUTE_CTX_POLICY(pCtx);
  boolean                             result = FALSE;

  result = policyman_tech_is_loaded(pCond->clsid);

  POLICYMAN_UT_MSG_3( SUBS_PREFIX SRC_LINE "condition <tech_present> returns %d",
                      POLICY_SUBS(pPolicy),
                      LINE_NUM(pCond),
                      result );
  return result;
...
)
{
  ...
  rat_include  = (SYS_SYS_MODE_MASK_WCDMA | SYS_SYS_MODE_MASK_TDS);
  ...
  policyman_update_persisted_user_plmn(pState, pPolicy->subs);
