...
/*
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/policyman/src/policyman_rat_order.c#1 $
...
{
  return  POLICYMAN_CFGITEM_NEW( POLICYMAN_ITEM_RAT_ACQ_ORDER,
...
policyman_item_t * policyman_rat_order_clone(
  policyman_item_t const *pItem
)
{
  policyman_rat_order_item_t const *pRatOrder       = (policyman_rat_order_item_t const *)pItem;
  policyman_rat_order_item_t       *pRatOrderClone;

  pRatOrderClone = policyman_rat_order_item_new(pRatOrder->asubs_id);

  pRatOrderClone->rat_order = pRatOrder->rat_order;

  return (policyman_item_t *)pRatOrderClone;
}
...
)
{
  ...
  pItem = policyman_rat_order_item_new(subsId);

  if (!policyman_retrieve_rat_order(&pItem->rat_order, subsId))
  {
    pItem->rat_order = default_rat_acq_order;

    POLICYMAN_MSG_ERROR_1(SUBS_PREFIX "Could not read rat_acq_order from EFS, using default", subsId);
  }
...
)
{
  ...
  *pRatOrder = pOrder->rat_order;
  ...
  POLICYMAN_MSG_HIGH_4( SUBS_PREFIX "get_rat_order (from item 0x%08x, seqno %d) returned rat_order num_rats %d",
                        pOrder->asubs_id,
                        pItem,
                        pOrder->seqno,
...
)
{
  ...
  POLICYMAN_MSG_HIGH_3( SUBS_PREFIX "Rat Acq Order: (item 0x%08x, seqno %d)",
                        pItem->asubs_id,
                        pItem,
                        pItem->seqno);

  POLICYMAN_MSG_HIGH_8( " num rats %d, order %d %d %d %d %d %d %d",
...
void policyman_rat_order_on_update(
  policyman_item_t  *pItem
)
{
  policyman_rat_order_item_t        *pRatOrder = (policyman_rat_order_item_t *)pItem;
  policyman_diag_event_rat_order_t   event_rat_order;
  size_t                             idx;

  event_rat_order.rat_order_table_len    = ARR_SIZE(event_rat_order.rat_order_entries);
  event_rat_order.num_rat_order_entries  = (uint8)pRatOrder->rat_order.num_rat;

  for (idx = 0; idx < ARR_SIZE(event_rat_order.rat_order_entries); ++idx)
  {
    boolean   valid_source;

    valid_source = (idx < pRatOrder->rat_order.num_rat) && (idx < ARR_SIZE(pRatOrder->rat_order.acq_order));

    event_rat_order.rat_order_entries[idx] = valid_source ? pRatOrder->rat_order.acq_order[idx] : 0;
  }

  event_report_payload( EVENT_PM_RAT_ORDER,
                        sizeof(event_rat_order),
                        &event_rat_order );
}
...
)
{
  ...
  while (policyman_get_token(&pStr, token, sizeof(token)))
  {
    ...
    rat = policyman_util_str_to_rat(token);

    if (SYS_SYS_MODE_NONE != rat)
    {
      pRat->acq_order[pRat->num_rat++] = rat;
    }
    else
    {
...
)
{
  ...
  policyman_rat_order_item_t *pRatOrderItem;
  sys_modem_as_id_e_type      asid;

  pRatOrderItem = policyman_rat_order_item_new(POLICY_SUBS(pPolicy));
  
  pRatOrderItem->rat_order = proAction->rat_order;

  mre_set_replace(pItemSet, &pRatOrderItem);
  ref_cnt_obj_release(pRatOrderItem);

  POLICYMAN_MSG_HIGH_4( SUBS_PREFIX SRC_LINE "action <rat_order pmsubs=%d> set rat_order with %d rats",
                        POLICY_SUBS(pPolicy),
                        LINE_NUM(proAction),
                        subsId,
                        pRatOrderItem->rat_order.num_rat );
...
QSH_MDUMP_FN_ATTR_PM void policyman_rat_order_mdump(
  policyman_item_t const *pItem
)
{
  policyman_qsh_dump_collect((void *)pItem, sizeof(policyman_rat_order_item_t));
}
