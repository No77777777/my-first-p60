...
/*
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/policyman/src/policyman_svc_mode.c#1 $
...
)
{
  ...
  /* If SIM Lock not verified, restrict the subscription to limited mode */
  if (!policyman_state_get_imsi_verified(subsId))
  {
    POLICYMAN_MSG_HIGH_1(SUBS_PREFIX "IMSI verification failed, requested svc mode %d updated to LIMITED", svcMode);
    svcMode = POLICYMAN_SVC_MODE_LIMITED;
  }

  pItem = POLICYMAN_CFGITEM_NEW( POLICYMAN_ITEM_SVC_MODE,
...
policyman_item_t * policyman_svc_mode_clone(
  policyman_item_t const *pItem
)
{
  policyman_svc_mode_item_t const *pSvcModeItem   = (policyman_svc_mode_item_t const *)pItem;
  policyman_svc_mode_item_t       *pSvcModeClone;

  pSvcModeClone = policyman_svc_mode_item_new( pSvcModeItem->mode,
                                               pSvcModeItem->asubs_id );

  return (policyman_item_t *)pSvcModeClone;
}
...
)
{
  ...
  if (   (status == MRE_EFS_STATUS_SUCCESS)
      ...
      && (*pMode <= POLICYMAN_SVC_MODE_FULL_RAT) )
  {
    *pSvcMode = *pMode;
    retval = TRUE;
  }
...
)
{
  ...
  if (policyman_retrieve_svc_mode(pCfgCtx->subsId, &svcMode))
  {
    pSvcMode = policyman_svc_mode_item_new(svcMode, pCfgCtx->subsId);
...
)
{
  ...
  *pSvcMode = pMode->mode;
  ...
  POLICYMAN_MSG_HIGH_4( SUBS_PREFIX "get_svc_mode (from item 0x%08x, seqno %d) returned svc_mode %d",
                        pItem->asubs_id,
                        pItem,
                        pItem->seqno,
                        pMode->mode );
...
)
{
  ...
  POLICYMAN_MSG_HIGH_4( SUBS_PREFIX "svc_mode: %d (item 0x%08x, seqno %d)",
                        pMode->asubs_id,
                        pMode->mode,
                        pItem,
                        pItem->seqno );
...
void policyman_svc_mode_on_update(
  policyman_item_t  *pItem
)
{
  policyman_svc_mode_item_t  *pMode = (policyman_svc_mode_item_t *) pItem;
  uint8                      event_svc_mode;

  event_svc_mode = pMode->mode;

  event_report_payload( EVENT_PM_SVC_MODE,
                        sizeof(event_svc_mode),
                        &event_svc_mode);
...
)
{
  ...
  {
    MRE_STR_VALUE_ENTRY("FULL",      POLICYMAN_SVC_MODE_FULL),
    MRE_STR_VALUE_ENTRY("CAMP_ONLY", POLICYMAN_SVC_MODE_CAMP_ONLY),
    MRE_STR_VALUE_ENTRY("LIMITED",   POLICYMAN_SVC_MODE_LIMITED),
    MRE_STR_VALUE_ENTRY("FULL_RAT",  POLICYMAN_SVC_MODE_FULL_RAT),
  };
  ...
  *pSvcMode = POLICYMAN_SVC_MODE_FULL;
  ...
  {
    MRE_LOOKUP_STR_VALUE(map, token, policyman_svc_mode_t, pSvcMode);
  }
...
)
{
  ...
  policyman_svc_mode_item_t *pSvcMode; 

  pSvcMode = policyman_svc_mode_item_new(psvcAction->svc_mode, subsId);

  mre_set_replace(pItemSet, &pSvcMode);
  ref_cnt_obj_release(pSvcMode);
...
)
{
  ...
  pItem = (policyman_svc_mode_item_t *)policyman_cfgitem_find_item_in_itemset_or_database(pItemSet, id, subsId);
  ...
  POLICYMAN_UT_MSG_4( SUBS_PREFIX SRC_LINE "condition <svc_mode_is pmsubs=%d> returns %d",
                      POLICY_SUBS(pPolicy),
                      LINE_NUM(pCond),
                      subsId,
                      result );
...
)
{
  ...
  pItem = policyman_svc_mode_item_new(POLICYMAN_SVC_MODE_FULL_RAT, subsId);
...
QSH_MDUMP_FN_ATTR_PM void policyman_svc_mode_mdump(
  policyman_item_t const *pItem
)
{
  policyman_qsh_dump_collect((void *)pItem, sizeof(policyman_svc_mode_item_t));
}
