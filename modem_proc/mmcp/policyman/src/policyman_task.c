...
/*
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/policyman/src/policyman_task.c#1 $
...
void policyman_register_for_pbm_events(void);
 ...
 */
STATIC mre_set_t    *pBlockedSignals;
...
)
{
  ...
  */
  ...
  pCmd->clientMsg.hook    = NULL;
  pCmd->clientMsg.pMsg    = NULL;
  pCmd->pSandbox          = policyman_get_global_sandbox();
...
static policyman_signal_t     policyman_PBM_ready_signal;
...
)
{
  ...
  policyman_msgr_signal      = (policyman_signal_t) 0x00000010;
  policyman_done_init_signal = (policyman_signal_t) 0x00000020;
  policyman_PBM_ready_signal    = (policyman_signal_t) 0x00000040;
...
{
  policyman_enter_crit_sect(&policyman_blocking_crit_sect);
  pBlockedSignals = mre_set_new(sizeof(policyman_signal_t), 5, MRE_SET_NO_MAX_SIZE, NULL, NULL);
  policyman_leave_crit_sect(&policyman_blocking_crit_sect);
}
...
{
  mre_set_t    *pBlockedSignals_temp;
...
@return
  None
*/
void policyman_block_client_if_initializing(
  void
)
{
  policyman_signal_t signal;
  ...
  policyman_enter_crit_sect(&policyman_blocking_crit_sect);
  if (pBlockedSignals != NULL)
  {
    signal = policyman_client_block_sig_get();
    block = mre_set_add(pBlockedSignals, &signal);
  }
  policyman_leave_crit_sect(&policyman_blocking_crit_sect);
...
#ifdef FEATURE_QSH_MDUMP
QSH_MDUMP_FN_ATTR_PM void policyman_qsh_mdump_handler
(
  qsh_client_cb_params_s *   qsh_client_cb_params_ptr
)
{

  /* check what all needs to be collect */
  policyman_policies_mdump();
  policyman_state_mdump();
  policyman_cfgitems_mdump();

  qsh_mdump_prioritize_client(QSH_CLT_PM,QSH_CLT_CM);
}

QSH_MDUMP_FN_ATTR_PM  void policyman_qsh_callback
(
  qsh_client_cb_params_s *   qsh_client_cb_params_ptr
)
{
  if (qsh_client_cb_params_ptr->action == QSH_ACTION_MDUMP)
  {
    policyman_qsh_mdump_handler(qsh_client_cb_params_ptr);
  }
}

void policyman_register_with_qsh(
  void
)
{
  ...
  client_reg.client = QSH_CLT_PM;
  client_reg.client_cb_ptr = policyman_qsh_callback;
  client_reg.major_ver = 1;
  client_reg.minor_ver = 1;
  client_reg.cb_action_support_mask = (client_reg.cb_action_support_mask|QSH_ACTION_MDUMP); 
...
QSH_MDUMP_FN_ATTR_PM void policyman_qsh_dump_collect(
  void*    addr, 
  size_t   size
)
{
...
#ifdef FEATURE_QSH_MDUMP
  qsh_mdump_collect_high(addr, size);
#endif
...
)
{
  ...
  RCEVT_SIGEX_SIGREX     pm_rcevt_siginfo_pbm;
  ...
  policyman_signalset_add(&policyman_waitsigs, policyman_PBM_ready_signal);
  ...
  */
  ...
  /*  Register MMGSDI/SIMLOCK.
   */
  policyman_register_for_uim_events();

  /*  Register for synchronization with PBM.
   */
  taskReady = policyman_register_for_task_sync( "PBM:READY",
                                                policyman_PBM_ready_signal,
                                                &pm_rcevt_siginfo_pbm );
  if (taskReady)
  {
    POLICYMAN_MSG_HIGH_0("PBM task is up...");
  ...
  while (TRUE)
  {
    ...
    if (policyman_signalset_contains(&fired_sigs, policyman_PBM_ready_signal))
    {
      POLICYMAN_MSG_HIGH_0("PBM task is up...");
      policyman_unregister_for_task_sync("PBM:READY", &pm_rcevt_siginfo_pbm);
      policyman_register_for_pbm_events();
    }
