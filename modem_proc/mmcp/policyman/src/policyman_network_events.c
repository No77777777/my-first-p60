...
/*
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/policyman/src/policyman_network_events.c#1 $
...
)
{
  ...
  if (scanPref != NULL)
  {
    ...
    ratMask      = policyman_rat_capability_evaluate(&scanPref->ratMaskPref, pItemSet, subsId);
...
)
{
   ...
   */
  if (policyman_network_scan_request_search(pState, subsId, &pNetworkInfo->scanPref, pItemSet))
  {
...
)
{
   ...
   */
  MRE_RELEASE_IF(pNetworkInfo->pNetScanTimer);
}
...
)
{
  ...
  if (NULL != pSearchCmd)
  {
    ...
    pNetworkInfo = policyman_state_get_network_info(pSandbox->pState, pSearchCmd->subsId);
  ...
  if (NULL != pScanCmd)
  {
    ...
    switch (pScanCmd->cmd)
    {
      case CM_PH_CMD_GET_NETWORKS:
        policyman_state_enter_crit_section();
        policyman_network_scan_handle_error(pScanCmd->pSandbox->pState, pScanCmd->err, subsId);
        policyman_state_leave_crit_section();
        break;
...
)
{
  ...
  {
    pTestPlmnSet = (mre_set_t *) mre_named_object_find( pCond->pTestList,
   ...
   */
  nElems = mre_set_num_elems(pTestPlmnSet);
  ...
  if (pNetworkInfo->available_networks != NULL)
  {
    ...
    for (i = 0; i < nElems; i++)
    {
      /* Get PLMN out of the set and break if present in scanned list
          */
      testPlmn = (sys_plmn_id_s_type *)mre_set_elem_ptr(pTestPlmnSet, i);
...
Done:
  POLICYMAN_UT_MSG_4( SUBS_PREFIX SRC_LINE "condition <scan_result pmsubs=%d> returns %d",
                      POLICY_SUBS(pPolicy),
                      LINE_NUM(pCond),
                      subsId,
                      result );
...
)
{
  ...
  if (   pnsAction->action == POLICYMAN_NET_SCAN_ACTION_START
      ...
      && policyman_network_scan_request_search(pState, subsId, &pNetworkInfo->scanPref, pItemSet)
     )
  {
