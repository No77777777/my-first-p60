...
/*
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/policyman/src/mre_xml.c#1 $
...
)
{
  ...
  MRE_RELEASE_IF(pElem->pAttributes);
  MRE_RELEASE_IF(pElem->pChildren);
...
)
{
  ...
  MALFORMED_IF(NEXT_CHAR != '"');
  ADVANCE;

  MALFORMED_IF(!mre_collect_text(ppDoc, ppStr, '"', pRoot));
...
@return
  Nothing.
*/
static boolean mre_optional_elements(
  char const        **ppDoc,
  mre_xml_element_t  *pElem,
  mre_xml_root_t     *pRoot
)
{
  ...
  mre_xml_element_t *pNewElem = NULL;
...
Done:
  return    (MRE_STATUS_SUCCESS         == status)
         || (MRE_STATUS_ERR_NOT_PRESENT == status);
}
...
)
{
  ...
  /*  If the next character is a '<', look for node elements; otherwise, treat
   *  the content as text.
   */
  if (NEXT_CHAR == '<')
  {
    MALFORMED_IF(!mre_optional_elements(ppDoc, pElem, pRoot));
  }
  else
  {
    MALFORMED_IF(!mre_collect_text(ppDoc, &pElem->pText, '<', pRoot));
   ...
   */
  MALFORMED_IF(!mre_collect_text(ppDoc, &pTag, '>', pRoot));
...
)
{
   ...
   */
  pElem = (mre_xml_element_t *) mre_mem_alloc(sizeof(mre_xml_element_t));
  LINE_NUM(pElem) = LINE_NUM(pRoot);

  /*  Get the tag name of the element.
   */
  MALFORMED_IF(!mre_require_tag(ppDoc, &pElem->pTag, pRoot));

  /*  Get any attributes.
   */
  status = mre_optional_attributes(ppDoc, pElem, pRoot);
  DONE_IF(status != MRE_STATUS_SUCCESS);
  ...
  status = mre_require_element_content(ppDoc, pElem, pRoot);
...
/*-------- mre_xml_require_xml_header --------*/
static boolean mre_xml_require_xml_header(
  char const        **ppDoc,
  mre_xml_element_t **ppElem,
  mre_xml_root_t     *pRoot
)
{
  ...
  status = mre_require_processing(ppDoc, ppElem, pRoot);
  DONE_IF(status != MRE_STATUS_SUCCESS);

  {
    ...
    /*  Version must be "1.0".
     */
    pStr = mre_xml_get_attribute(*ppElem, "version");
     ...
     */
    pStr = mre_xml_get_attribute(*ppElem, "encoding");
...
)
{
  ...
  *ppRoot = pRoot;
   ...
   */
  MALFORMED_IF(!mre_xml_require_xml_header(ppDoc, &pRoot->pHeader, pRoot));
...
mre_xml_element_t const * mre_xml_get_root_element(
...
/*-------- mre_xml_get_child --------*/
mre_xml_element_t const * mre_xml_get_child(
