...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/mm/inc/gmm_int_v.h#1 $ $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...
typedef struct mm_client_req_drx_s
{
  sys_drx_cn_coefficient_s1_e_type    default_drx;

  sys_drx_cn_coefficient_s1_e_type    mcs_requested_drx;

  sys_drx_cn_coefficient_s1_e_type    ds_requested_drx;

  sys_drx_cn_coefficient_s1_e_type    qmi_requested_drx;

  uint32                              prohibit_timer_value;

  byte                                client_id;

}mm_client_req_drx_s_type;
...
extern gmm_ptmsi_type           gmm_old_ptmsi_sim[MAX_NAS_STACKS];
...
/* gmm_rrc_establish_cause - 
 *
 * introduced   in Rel 12- Spec CR 2538
 * stores       the paging cause received by UE.so that if UE sends RAU due to page,
 *              cause sent in RRC connection estab request is the cause which is received in page indication.
 * default@     RRC_EST_TERM_UNKNOWN
 * set @        while processing of RRC Page Ind if it is detected that RAU needs to be triggered 
 * used @       while sending RRC_EST_REQ, this value is used if it is anything other than
 *              RRC_EST_TERM_UNKNOWN
 * reset @      immediately after sending RRC connection establishment request
 * 
 */
extern rrc_establish_cause_e_type    gmm_rrc_establish_cause_sim[MAX_NAS_STACKS];

/* gprs_service_attempt_counter - 
 *
 * introduced        in Rel 12- Spec CRs 2488, 2937
 * stores            the service attempt counter
 * default@          0
 * increment@        T3317 expiry for UE which is not AC 11-15
 * used @            if this counter reaches 5, TIMER_T3325 is started and
 *                   Service Req from SM gets rejected while TIMER_T3325 is running
 * reset @           i)   attach or combined attach procedure is successfully completed
 ...
 *                   iv)  a service reject message is received
 *                   v)   expiry of TIMER_T3325
 */
extern byte                          gprs_service_attempt_counter_sim[MAX_NAS_STACKS];
#endif
...
extern boolean gmm_rau_upon_entry_to_a_new_cell_sim[MAX_NAS_STACKS];
...
extern byte gmm_buffer_rej_cause_sim[MAX_NAS_STACKS][MAX_ATTEMPT_COUNTER_ATTACH];
...
#ifdef FEATURE_NAS_REL12_SPEC_CHANGE
extern boolean gmm_sync_pmm_state_during_w_to_lg_and_back_to_w_irat_sim[MAX_NAS_STACKS];
#endif
...
extern boolean gmm_rau_is_required_due_to_dsr_sim[MAX_NAS_STACKS] ;
...
extern boolean gmm_pending_imsi_attach_sim[MAX_NAS_STACKS];


#define  gmm_substate gmm_substate_sim[mm_as_id]
#define  gmm_stored_ptmsi gmm_stored_ptmsi_sim[mm_as_id]
#define  gmm_old_ptmsi gmm_old_ptmsi_sim[mm_as_id]
...
#ifdef FEATURE_NAS_REL12_SPEC_CHANGE
#define  gmm_rrc_establish_cause      gmm_rrc_establish_cause_sim[mm_as_id]
...
#define  gmm_rau_upon_entry_to_a_new_cell gmm_rau_upon_entry_to_a_new_cell_sim[mm_as_id]
...
#define gmm_buffer_rej_cause gmm_buffer_rej_cause_sim[mm_as_id]
...
#ifdef FEATURE_NAS_REL12_SPEC_CHANGE
#define gmm_sync_pmm_state_during_w_to_lg_and_back_to_w_irat gmm_sync_pmm_state_during_w_to_lg_and_back_to_w_irat_sim[mm_as_id]
#endif
...
#define  gmm_rau_is_required_due_to_dsr gmm_rau_is_required_due_to_dsr_sim[mm_as_id]
#ifdef FEATURE_GSM_GPRS
...
#define  gmm_pending_imsi_attach    gmm_pending_imsi_attach_sim[mm_as_id]
...
extern boolean gmm_need_to_perform_ecall_inactivity_sim[MAX_AS_IDS];
...
extern boolean gmm_abort_ps_detach_sim[MAX_NAS_STACKS];
#define gmm_abort_ps_detach gmm_abort_ps_detach_sim[mm_as_id]
#ifdef FEATURE_DUAL_SIM
extern boolean gmm_ps_call_in_progess_sim[MAX_NAS_STACKS];
#define gmm_ps_call_in_progess gmm_ps_call_in_progess_sim[mm_as_id]
#endif

extern sys_radio_access_tech_e_type gmm_drx_coeff_client_rat_sim[MAX_AS_IDS];
#define gmm_drx_coeff_client_rat gmm_drx_coeff_client_rat_sim[mm_sub_id]

extern mm_client_req_drx_s_type gmm_client_req_drx_coeff_sim[MAX_AS_IDS];
...
extern gmm_ptmsi_type           gmm_old_ptmsi;

extern gmm_ptmsi_signature_type gmm_stored_ptmsi_signature;
extern boolean   gmm_pending_imsi_attach;
 ...
 * 
 */
extern rrc_establish_cause_e_type    gmm_rrc_establish_cause;
 ...
 *                   v)   expiry of TIMER_T3325
 */
extern byte                          gprs_service_attempt_counter;
#endif
...
extern boolean gmm_rau_upon_entry_to_a_new_cell;
...
extern byte gmm_buffer_rej_cause[MAX_ATTEMPT_COUNTER_ATTACH];
...
#ifdef FEATURE_NAS_REL12_SPEC_CHANGE
/* ---------------------------- ---------------------------- ---------------------------- ----------------------------
 * This variable will be used to handle the following scenario:
 * Spec 24.008 4.7.5.1 - CR 2553: If the MS performs an inter-system change from PMM-CONNECTED mode in Iu mode to A/Gb mode or 
 * S1 mode not due to PS handover, and subsequently performs an intersystem change back to Iu mode without sending a ROUTING AREA 
 * UPDATE REQUEST message while in A/Gb mode or a TRACKING AREA UPDATE REQUEST message while in S1 mode, the MS shall initiate
 * a routing area updating procedure. Without the routing area updating procedure, the PMM modes on MS and netowork side remain
 * un-synchronized, and the MS may miss paging messages from the RNC.
 * ---------------------------- ---------------------------- ---------------------------- ----------------------------*/
extern boolean gmm_sync_pmm_state_during_w_to_lg_and_back_to_w_irat;
#endif
...
extern boolean gmm_rau_is_required_due_to_dsr;
#ifdef FEATURE_GSM_GPRS
...
#ifdef FEATURE_NAS_ECALL 
extern boolean gmm_need_to_perform_ecall_inactivity;
#endif
...
extern boolean gmm_abort_ps_detach;
extern sys_radio_access_tech_e_type gmm_drx_coeff_client_rat;

extern mm_client_req_drx_s_type gmm_client_req_drx_coeff;
...
#ifdef FEATURE_NAS_REL12_SPEC_CHANGE
extern void gmm_send_gmmsm_irat_ps_service_ind(void);
#endif

extern void gmm_send_gmmsm_establish_resend_ind(sm_cm_status_T cause);
...
extern void gmm_buffer_rej_cause_reset(void);
...
#ifdef FEATURE_DUAL_SIM
extern boolean gmm_get_cm_ds_tuneaway_status(void);
...
extern boolean gmm_is_dos_mitigation_required_for_ps_domain( byte msg_id,byte reject_cause);
extern void gmm_perform_dos_mitigation_action(byte msg_id,byte reject_cause);
extern void gmm_handle_retx_pdu_tuneaway_fail_ind(void);

extern sys_drx_cn_coefficient_s1_e_type gmm_process_paging_cycle_selector
(
  sys_drx_cn_coefficient_s1_e_type    client_drx,
  cm_client_type_e_type client_id
);

extern void gmm_reset_mcs_client_info(void);


extern boolean gmm_check_lte_only_client_drx_negotiated(void);

boolean gmm_is_emm_params_to_be_deleted_on_ps_rej(void);
/*===========================================================================

FUNCTION store_rand_gmm
...
===========================================================================*/
void store_rand_gmm(auth_rand_type *p_rand);

/*===========================================================================

FUNCTION get_rand_gmm
...
===========================================================================*/
ssg_mm_rand_type get_rand_gmm(void);
