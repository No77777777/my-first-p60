...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/mm/src/mmsend.c#2 $    $DateTime: 2022/09/01 23:13:35 $ $Author: pwbldsvc $
...
static boolean cs_cipher_status_sim[MAX_AS_IDS]={FALSE,FALSE};
#define cs_cipher_status cs_cipher_status_sim[mm_as_id]
static boolean ps_cipher_status_sim[MAX_AS_IDS]={FALSE,FALSE};
#define ps_cipher_status ps_cipher_status_sim[mm_as_id]
#else
...
extern boolean rrc_is_delay_return_to_lte_nv_enabled(sys_modem_as_id_e_type as_id); 
...
#endif
  if(mm_subs_capability_mask & (SYS_SYS_MODE_MASK_WCDMA| SYS_SYS_MODE_MASK_TDS))
  {
    is_wcdma_sub = TRUE;
  }
  return is_wcdma_sub;
...
#endif
#if defined (FEATURE_LTE) && defined (FEATURE_DUAL_SIM) && !defined (FEATURE_MODEM_ATC) 
   if((mm_waiting_for_list_cnf_type == SYS_NETWORK_SELECTION_MODE_MANUAL ) &&
...
#if defined (FEATURE_DUAL_SIM)
       && 
       (mm_tds_rat_status == SYS_RAT_ACTIVATION_STATUS_ACTIVATED)
#endif
      )
   {
...
#ifdef FEATURE_DUAL_SIM
   mm_update_rat_status(&mm_rat_pri_list_info,SYS_RAT_ACTIVATION_STATUS_NONE);
#endif
...
void mm_send_status( byte reject_cause )
{
  ...
  mm_qsh_ota_metric_signaling_log(MM_STATUS,UE_TO_CN_DIRECTION,0xff, (mm_gmm_reject_cause_debug_type)reject_cause);
#endif
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
   ...
   if(cause_value == MM_REJ_ACCESS_CLASS_BLOCKED)
   {
   		mmcnm_est_rej.cause_value = mm_map_cause();
   }
   else
   {
   ...
   {
      QEVENT(QSH_CLT_MM, MM_QSH_EVENT_CS_CALL_STATUS, MM_QSH_EVENT_CS_CALL_STATUS_VERSION, 
             mm_sub_id, "=EMM= Call status %d", CAFI_KPI_EVENT_IND_FAIL);
   ...
   MSG_HIGH_DS_2( MM_SUB, "=MM= MM sent MMCNM_EST_REJ, type %d, value %d", cause_type,mmcnm_est_rej.cause_value);
   ...
   */
   ...
   {
#ifdef FEATURE_DUAL_SIM
     mm_proc_status_set(SYS_PROC_TYPE_SMS, FALSE);
  ...
  ** ---------------------------------------------- */
  ...
  if(mm_sms_is_active == TRUE)
   ...
   else
   {
		...
		mm_proc_status_set(SYS_PROC_TYPE_SMS, FALSE);
		mm_proc_status_set(SYS_PROC_TYPE_SUPPLEMENTARY_SERVICE, FALSE);
...
void mm_send_mmr_stop_mode_cnf( void )
{
   ...
   mmr_stop_mode_cnf.trans_id = mmr_stop_mode_trans_id;
   /*lint -save -e778 */
   ...
   mm_check_and_perform_action_invalid_state_detection((sys_modem_as_id_e_type)mm_as_id, DEBUG_INVALID_STATE_STOP_MODE_CNF);
#ifdef FEATURE_DUAL_SIM
   mm_last_proc_type_sent_set(SYS_PROC_TYPE_NONE);
   if(mm_serving_plmn.info.active_rat != SYS_RAT_TDS_RADIO_ACCESS)
   {
     mm_update_rat_status((sys_rat_pri_list_info_s_type *)NULL,
...
#ifdef FEATURE_DUAL_SIM
  if(plmn_search_cnf->plmn_search_result != REG_MM_PLMN_SEARCH_PARTIAL)
  {
    mm_update_rat_status(&mm_rat_pri_list_info,
                         SYS_RAT_ACTIVATION_STATUS_NONE);
  }
#endif

#ifdef FEATURE_LTE
  if(((plmn_search_cnf->network_selection_mode == SYS_NETWORK_SELECTION_MODE_AUTOMATIC 
	   && (plmn_search_cnf->service_search == SYS_SERVICE_SEARCH_NONE))||
	   (plmn_search_cnf->service_search == SYS_SERVICE_SEARCH_IRAT_PLMN)))
  {

     mm_emm_send_plmn_search_status_to_mcs(FALSE);
...
#if (LTE_BAND_NUM == 256)
    MSG_HIGH_DS_5(MM_SUB, "MM sent MMR_CAMPED_IND RAT : %d, LTE bands 1_64: 0x%08x %08x,LTE bands 65_128: 0x%08x %08x",
                  camped_ind.active_rat,
                  QWORD_HIGH(camped_ind.active_band.lte_active_band.bits_1_64),
                  QWORD_LOW(camped_ind.active_band.lte_active_band.bits_1_64),
                  QWORD_HIGH(camped_ind.active_band.lte_active_band.bits_65_128),
                  QWORD_LOW(camped_ind.active_band.lte_active_band.bits_65_128));
    MSG_HIGH_DS_4(MM_SUB, " LTE bands 129_192: 0x%08x %08x,LTE bands 193_256: 0x%08x %08x",
                  QWORD_HIGH(camped_ind.active_band.lte_active_band.bits_129_192),
                  QWORD_LOW(camped_ind.active_band.lte_active_band.bits_129_192),
                  QWORD_HIGH(camped_ind.active_band.lte_active_band.bits_193_256),
                  QWORD_LOW(camped_ind.active_band.lte_active_band.bits_193_256));
#else
...
void mm_send_mmr_reg_req_ignored_ind(sys_modem_as_id_e_type as_id)
{
  ...
  MSG_HIGH_DS_0((as_id+1),"=MM= MM sent MMR_REG_REQ_IGNORED_IND");
...
void mm_send_mmr_reg_cnf( void )
{
  ...
  mmcoord_mmr_reg_cnf.service_state.lte_cs_capability = SYS_LTE_CS_CAPABILITY_NONE;
   ...
   if ((((mmcoord_pending_reg_message.cmd.mmr_reg_req.network_selection_mode == SYS_NETWORK_SELECTION_MODE_MANUAL) &&
       ...
       ((mmcoord_pending_reg_message.cmd.mmr_reg_req.network_selection_mode == SYS_NETWORK_SELECTION_MODE_MANUAL_CSG) &&
        (sys_plmn_match(mm_serving_plmn.info.plmn, mmcoord_pending_reg_message.cmd.mmr_reg_req.plmn) == FALSE)))&&
   ...
   else
   {
     ...
     mmcoord_mmr_reg_cnf.service_state.only_for_ui = FALSE;
     ...
     else
     {
       ...
       */
       if(((mm_state != MM_CONNECTION_ACTIVE) &&
          ...
          !(mmcoord_mmr_reg_cnf.cause == MMR_CAUSE_FAILURE_FOREGROUND_PLMN_SEARCH && mm_lu_attempt_counter >= MAX_ATTEMPT_COUNTER_LAU) &&
     ...
     mmcoord_mmr_reg_cnf.oos_srch_done = TRUE;
     ...
     MSG_HIGH_DS_8(MM_SUB, "=MM=   active_rat = %d ps_data_suspend = %d plmn_reg_type = %d \n =MM=   equiv PLMN update=%d list len=%d info_blocked_at_reg = %d \n=MM= LTE IRAT BPLMN: camped cell id %d , serach done %d",
               ...
               mmcoord_mmr_reg_cnf.oos_srch_done);
  #ifdef FEATURE_FEMTO_CSG
...
#endif 
     mmcoord_mmr_reg_cnf.trans_id = mm_reg_req_trans_id;
       ...
       )
  {
     ...
     */
        ...
        (mmcoord_pending_reg_message.cmd.hdr.message_id == (int)MMR_REG_REQ))
     {
       ...
       mm_last_requested_network_selection_mode = mmcoord_pending_reg_message.cmd.mmr_reg_req.network_selection_mode;
     }
   }
...
void mm_send_rr_sim_update_req( void )
{
...
#endif
  {
   ...
   memscpy(rr_sim_update_req.old_ptmsi, sizeof(mm_sim_ptmsi_T), gmm_old_ptmsi.digit,PTMSI_SIZE);
...
void mm_send_imsi_detach( void )
{
   ...
   #endif
#ifdef FEATURE_QSH_EVENT_METRIC
   mm_qsh_ota_metric_signaling_log(IMSI_DETACH_INDICATION,UE_TO_CN_DIRECTION,
         (uint8)(imsi_detach_indication.mobile_identity_data[1] & (byte)(0x07)), MM_GMM_NO_FAILURE_DEBUG);
#endif
}
...
void mm_send_mmr_service_ind(mmr_service_ind_s_type *mmr_service_ind)
{
   ...
   */
   ...
   mmr_service_ind->service_state.only_for_ui = mm_manual_mode_irat_ho_in_non_user_sel_cell;
   ...
   else
   {
     ...
     if(((mm_state != MM_CONNECTION_ACTIVE) &&
        ...
        !(mmr_service_ind->cause == MMR_CAUSE_FAILURE_FOREGROUND_PLMN_SEARCH && mm_lu_attempt_counter >= MAX_ATTEMPT_COUNTER_LAU) &&
        (((gmm_gprs_attach_pending || gmm_ra_update_pending) &&
            !(is_rai_present_foreground_search_list) &&
            !((gmm_substate == GMM_LIMITED_SERVICE) ||
              (gmm_substate == GMM_NO_CELL_AVAILABLE))) ||
...
void mm_send_mmr_sim_available_cnf()
{
   ...
   {
     for (idx = MM_AS_ID_1; idx < MAX_AS_IDS; idx++)
     {
       MSG_HIGH_DS_3(MM_SUB, "=MM= ASID=%d, mm_sim_swapped=%d, mm_sim_card_mode=%d", 
               idx, mm_sim_swapped_sim[idx], mm_sim_card_mode_sim[idx]);

       MSG_HIGH_DS_3(MM_SUB, "=MM= mm_debug_sim_swap_status=%d, mm_debug_mmr_sim_available_status=%d, mm_imsi_is_native=%d", 
               mm_debug_sim_swap_status_sim[idx], mm_debug_mmr_sim_available_status_sim[idx], mm_imsi_is_native_sim[idx]);
     }
   }
...
#ifdef FEATURE_DUAL_SIM
void mm_send_mmr_me_blocked_status_ind(boolean is_me_blocked, mm_as_id_e_type mm_local_as_id)
{
  mmr_me_blocked_status_ind_type reg_me_blocked_ind;
  reg_me_blocked_ind.message_header.message_set = MS_MM_REG;
  reg_me_blocked_ind.message_header.message_id  = (int)MMR_ME_BLOCKED_STATUS_IND;
  reg_me_blocked_ind.is_me_blocked = is_me_blocked;
  reg_me_blocked_ind.as_id = (sys_modem_as_id_e_type)mm_local_as_id;
  PUT_IMH_LEN( sizeof( mmr_me_blocked_status_ind_type) - sizeof(IMH_T),
                          &reg_me_blocked_ind.message_header );
  MSG_HIGH_DS_1(MM_SUB, "=MM= MM sent MMR ME BLOCKED IND.status %d", reg_me_blocked_ind.is_me_blocked);
  mm_send_message( (IMH_T *)&reg_me_blocked_ind, GS_QUEUE_REG );
}
#endif
...
)
{
   if((((mmcoord_mmr_reg_cnf.service_state.service_status == SYS_SRV_STATUS_SRV) ||
       ((mmcoord_pending_reg_message.cmd.mmr_reg_req.network_selection_mode == SYS_NETWORK_SELECTION_MODE_MANUAL ) &&
        (mm_last_requested_network_selection_mode !=SYS_NETWORK_SELECTION_MODE_MANUAL)) ||
       ((mmcoord_pending_reg_message.cmd.mmr_reg_req.network_selection_mode == SYS_NETWORK_SELECTION_MODE_AUTOMATIC) &&
        (mm_last_requested_network_selection_mode !=SYS_NETWORK_SELECTION_MODE_AUTOMATIC)))&&
        (mmcoord_pending_reg_message.cmd.hdr.message_id == (int)MMR_REG_REQ)) ||
...
(
  mmr_multimode_subs_chgd_req_s_type*  subs_chgd_req_ptr
)
{
    sys_modem_as_id_e_type as_id;
    ...
    {
      rr_multimode_subs_chgd_req.subs_capability_mask[as_id] = subs_chgd_req_ptr->subs_capability_mask[as_id];
      rr_multimode_subs_chgd_req.subs_capability[as_id] = SUBS_CAPABILITY_NONE;
    }
...
if(mm_wcdma_supported)
{
  ...
  else
  {
     ...
     for(as_id = SYS_MODEM_AS_ID_1; as_id < MAX_AS_IDS;as_id++)
     {
       rrc_cmd->cmd.rrc_multimode_subs_chgd_ind.subs_capability_mask[as_id] = subs_chgd_req_ptr->subs_capability_mask[as_id];
     }
     send_cmd_to_rrc( rrc_cmd, SYS_RAT_UMTS_RADIO_ACCESS );
  }
}
...
if(mm_tdscdma_supported)
{
  ...
  else
  {
    ...
    {
      rrc_cmd_td->cmd.rrc_multimode_subs_chgd_ind.subs_capability_mask[as_id_td] = subs_chgd_req_ptr->subs_capability_mask[as_id_td];
    }
#ifdef FEATURE_TRIPLE_SIM
    rrc_cmd_td->cmd.rrc_multimode_subs_chgd_ind.subs_capability_mask[as_id_td] = SYS_SYS_MODE_MASK_NONE;
#endif
...
void mm_send_rrc_suitable_search_end_ind(void)
  {
    ...
    else if(((gmm_state == GMM_DEREGISTERED) || (gmm_state == GMM_NULL)) &&
...
FUNCTION mm_send_rr_mo_csfb_failure_ind
...
===========================================================================*/
void mm_send_rr_mo_csfb_failure_ind(boolean depriotize_cells)
{
   ...
   /*lint -restore */
    ...
    MSG_HIGH_DS_1(MM_SUB,"=MM= MM sent RR_MT_CSFB_FAILURE_IND cell_deprioritize during MO call= %d",depriotize_cells);
...
void mm_send_mmr_cell_access_ind( void )
{
  ...
  {
    cell_access_ind.cell_access_status.cs_signalling_bar_status = mm_check_if_cs_cell_access_barred(mm_get_cs_cell_access(PROCEDURE_CS_REGISTRATION));
    cell_access_ind.cell_access_status.ps_signalling_bar_status = gmm_check_if_ps_cell_access_barred(gmm_get_ps_cell_access(PROCEDURE_PS_REGISTRATION));
  }
  else
  {
    //setting the default value - Unbarred
    cell_access_ind.cell_access_status.cs_signalling_bar_status = 0;
    cell_access_ind.cell_access_status.ps_signalling_bar_status = 0;
  }

  PUT_IMH_LEN( sizeof( mmr_cell_access_ind_s_type ) - sizeof(IMH_T),
                                                      &cell_access_ind.message_header );
  
  MSG_HIGH_DS_4(MM_SUB,"MM sent MMR_CELL_ACCESS_IND  CS_BAR_STATUS : %d, PS_BAR_STATUS : %d, CS_SIGNALLING_BAR_STATUS : %d, PS_SIGNALLING_BAR_STATUS : %d,",
                            mm_get_cs_cell_access(PROCEDURE_NONE),
                            gmm_get_ps_cell_access(PROCEDURE_NONE),
                            cell_access_ind.cell_access_status.cs_signalling_bar_status,
                            cell_access_ind.cell_access_status.ps_signalling_bar_status);
...
DESCRIPTION
  This function sends a command to GRR to inform if IRATs are allowed
...
void mm_send_rr_irat_allowed_status_ind( void )
{
   ...
   rr_irat_allowed_status_ind_T rr_irat_allowed_status_ind;
   if(mm_serving_plmn.info.active_rat != SYS_RAT_GSM_RADIO_ACCESS)
   {
     return;
   }
   memset(&rr_irat_allowed_status_ind,0x00,sizeof(rr_irat_allowed_status_ind_T));
   rr_irat_allowed_status_ind.message_header.message_set = MS_MM_RR;
   rr_irat_allowed_status_ind.message_header.message_id  = (int)RR_IRAT_ALLOWED_STATUS_IND;

#ifdef FEATURE_LTE
   rr_irat_allowed_status_ind.lte_redir_allowed = emm_redirection_allowed_status_ind(mm_serving_plmn.info.plmn);
   rr_irat_allowed_status_ind.lte_reselection_allowed = rr_irat_allowed_status_ind.lte_redir_allowed && mm_is_lte_reselection_allowed(mm_serving_plmn.info.plmn);
#endif 
   rr_irat_allowed_status_ind.wcdma_resel_redir_allowed = mm_redirection_allowed_status_ind(mm_serving_plmn.info.plmn);

   PUT_IMH_LEN( sizeof( rr_irat_allowed_status_ind_T ) - sizeof(IMH_T),
                &rr_irat_allowed_status_ind.message_header );

   MSG_HIGH_DS_2(MM_SUB,"=MM= MM sent RR_IRAT_ALLOWED_STATUS_IND  : %d %d ", 
                           rr_irat_allowed_status_ind.lte_redir_allowed, rr_irat_allowed_status_ind.wcdma_resel_redir_allowed);
   
   mm_send_message(&rr_irat_allowed_status_ind.message_header, GS_QUEUE_RR );  
...
void mm_send_mmcnm_srvcc_handover_complete(void )
{
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
   ...
   MSG_HIGH_DS_0(MM_SUB,"=MM= MM sent MMCNM_SRVCC_HANDOVER_COMPLETE with trigger");
...
void mm_send_mmr_set_drx_cnf(boolean result)
{
  ...
  gmm_set_drx_req_pending = NAS_DRX_CFG_REQ_NONE;
}
...
#if defined(FEATURE_LTE)
  if (is_lte_mode() &&
      gmm_check_lte_only_client_drx_negotiated())
  {
    return;
  }
#endif

  if(mm_sub_is_rat_enabled(SYS_RAT_UMTS_RADIO_ACCESS))
  {
#ifdef FEATURE_SEGMENT_LOADING
  if (mm_wcdma_supported)
  {
...
void mm_send_rr_dds_sub_ind(sys_modem_as_id_e_type dds_sub)
{
  rr_dds_sub_ind_T rr_dds_sub_ind;
  memset(&rr_dds_sub_ind,0x00,sizeof(rr_dds_sub_ind_T));
  rr_dds_sub_ind.message_header.message_set = MS_MM_RR;
  rr_dds_sub_ind.message_header.message_id  = (int)RR_DDS_SUB_IND;
  /*lint -save -e778 */
  PUT_IMH_LEN( sizeof( rr_dds_sub_ind_T ) - sizeof(IMH_T),
               &rr_dds_sub_ind.message_header );
  rr_dds_sub_ind.as_id = dds_sub;
  /*lint -restore */
  MSG_HIGH_DS_1(dds_sub+1,"=MM= MM sent RR_DDS_SUB_IND, dds_sub %d", dds_sub);
  mm_send_message( (IMH_T *)&rr_dds_sub_ind, GS_QUEUE_RR );
}
...
DESCRIPTION
  This function sends a RRC_DDS_SUB_IND to RRC.
...
void mm_send_rrc_dds_sub_ind(sys_modem_as_id_e_type dds_sub)
{
#ifdef FEATURE_WCDMA
       rrc_cmd_type *rrc_cmd = NULL;
    ...
    /* This feature is not applicable to TDSCDMA */
      ...
      if (mm_wcdma_supported)
      {
        ...
        rrc_cmd->cmd_hdr.cmd_id = RRC_DDS_SUB_IND;
        rrc_cmd->cmd.rrc_dds_sub_ind.dds_sub_id= dds_sub;
...
{
   if(mm_last_proc_type_sent != proc_type)
   {
     rr_procedure_ind_T rr_procedure_ind;
     memset(&rr_procedure_ind,0x00,sizeof(rr_procedure_ind_T));
     rr_procedure_ind.message_header.message_set = MS_MM_RR;
     rr_procedure_ind.message_header.message_id  = (int)RR_PROCEDURE_IND;
     rr_procedure_ind.proc_type = proc_type;
     /*lint -save -e778 */
     PUT_IMH_LEN( sizeof( rr_procedure_ind_T ) - sizeof(IMH_T),
                  &rr_procedure_ind.message_header );
     /*lint -restore */
     MSG_HIGH_DS_1(MM_SUB,"=MM= MM sent RR_PROCEDURE_IND proc_type %d", proc_type);
     mm_send_message( (IMH_T *)&rr_procedure_ind, GS_QUEUE_RR );
	 mm_last_proc_type_sent_set(proc_type);
   }
}
...
#ifdef FEATURE_WCDMA
   if(is_ps_wcdma_mode(as_id) && (rrc_is_delay_return_to_lte_nv_enabled(as_id)))
   {
     rrc_cmd_type *rrc_cmd = mm_get_rrc_cmd_buf( mm_serving_plmn.info.active_rat );
  
     if (rrc_cmd == NULL)
     {
        MSG_FATAL_DS( MM_SUB, "=MM= Out of memory while sending rrc_abnormal_call_end_ind", 0,0,0 );
     }
     else
     {
     rrc_cmd->cmd_hdr.cmd_id  = RRC_ABNORMAL_CALL_END_IND;
     mm_set_as_id_rrc_abnormal_call_end_ind(as_id);
          /*lint -restore */
     send_cmd_to_rrc( rrc_cmd, mm_serving_plmn.info.active_rat );
     }
   }
...
void mm_send_rrc_procedure_ind(sys_proc_type_e_type proc_type)
{
	    ...
	    (proc_type != SYS_PROC_TYPE_NONE))))
   {
     ...
     {
       active_rat = mm_wrlf_serving_plmn_ptr->info.active_rat;

     }
#endif
     rrc_cmd_p = mm_get_rrc_cmd_buf( active_rat );

     if (rrc_cmd_p == NULL)
     {
        MSG_FATAL_DS( MM_SUB, "=MM= ERROR: Unable to allocate memory", 0,0,0 );
     }
     rrc_cmd_p->cmd_hdr.cmd_id = RRC_PROCEDURE_IND;
     rrc_cmd_p->cmd.rrc_procedure_ind.proc_type = proc_type;
     /*lint -restore */
     send_cmd_to_rrc( rrc_cmd_p, active_rat );
     if (proc_type == SYS_PROC_TYPE_NONE_DELAYED)
     {
       proc_type = SYS_PROC_TYPE_NONE;
     }
	 mm_last_proc_type_sent_set(proc_type);
...
void mm_send_procedure_ind(sys_proc_type_e_type proc_type)
{
  ...
  {
    mm_send_rrc_procedure_ind(proc_type);
  }
  else if(is_gsm_mode())
  {
    mm_send_rr_procedure_ind(proc_type);
  }
   ...
   if (list != NULL)
   {
         ...
         mm_sub_is_rat_enabled(SYS_RAT_UMTS_RADIO_ACCESS))
     {
       ...
       else
       {
		  ...
		  rrc_cmd->cmd.rrc_lai_reject_list_ind.nas_lai_reject_list.length = MIN(list ->length , MAX_NO_FORBIDDEN_LAIS);
		  rrc_cmd->cmd.rrc_lai_reject_list_ind.nas_lai_reject_list.write_ptr = list->write_ptr;

		  for(i=0; i<rrc_cmd->cmd.rrc_lai_reject_list_ind.nas_lai_reject_list.length ; i++)
		  {
            memscpy( &rrc_cmd->cmd.rrc_lai_reject_list_ind.nas_lai_reject_list.lai[i],sizeof(inter_task_lai_T),&(list->lai[i]),sizeof(inter_task_lai_T) );
		  }
		  send_cmd_to_rrc( rrc_cmd, SYS_RAT_UMTS_RADIO_ACCESS );
       }
     }
        ...
        )
     {
       ...
       {
          rrc_cmd->cmd_hdr.cmd_id = RRC_LAI_REJECT_LIST_IND;
		  rrc_cmd->cmd.rrc_lai_reject_list_ind.nas_lai_reject_list.length = list->length;
		  rrc_cmd->cmd.rrc_lai_reject_list_ind.nas_lai_reject_list.write_ptr = list->write_ptr;

		  for(i=0; i<list->length && i< MAX_NO_FORBIDDEN_LAIS; i++)
		  {
            memscpy( &rrc_cmd->cmd.rrc_lai_reject_list_ind.nas_lai_reject_list.lai[i],sizeof(inter_task_lai_T),&(list->lai[i]),sizeof(inter_task_lai_T) );
		  }
          send_cmd_to_rrc( rrc_cmd, SYS_RAT_TDS_RADIO_ACCESS );
...
void mm_send_rr_lai_reject_list_ind( nas_lai_reject_list_T* list )
{
...
#endif
   {
     ...
     for(i=0; i<rr_lai_reject_list_ind.nas_lai_reject_list.no_of_entries; i++)
     {
      memscpy(&(rr_lai_reject_list_ind.nas_lai_reject_list.location_area_identification[0]), sizeof(rr_lai_reject_list_ind.nas_lai_reject_list.location_area_identification), &(list->lai[0]), sizeof(inter_task_lai_T));
     }
...
void mm_send_rr_csg_white_list_ind(void)
{
   ...
   /*lint -restore */
   MSG_HIGH_DS_0(MM_SUB, "=MM= MM sent RR_CSG_WHITE_LIST_UPDATE_IND");
...
#ifdef FEATURE_LTE
  if(((mmr_plmn_search_cnf_ptr->network_selection_mode == SYS_NETWORK_SELECTION_MODE_AUTOMATIC 
	   && (mmr_plmn_search_cnf_ptr->service_search == SYS_SERVICE_SEARCH_NONE))||
	   (mmr_plmn_search_cnf_ptr->service_search == SYS_SERVICE_SEARCH_IRAT_PLMN)))
  {
...
FUNCTION MM_SEND_TDSRRC_SUBSCRIPTION_ID_IND

DESCRIPTION
  This function sends TDSRRC_SUBSCRIPTION_ID_IND to TDSRRC
...
void mm_send_tdsrrc_subscription_id_ind(void)
{
   ...
   {
      tdsrrc_cmd->cmd_hdr.cmd_id = TDSRRC_SUBSCRIPTION_ID_IND;
      tdsrrc_cmd->cmd.rrc_subscription_id.as_id = (sys_modem_as_id_e_type)mm_sub_id;
   } 
...
FUNCTION MM_SEND_RRC_SECURE_CALL_IND

DESCRIPTION
  This function sends RRC_SECURE_CALL_IND to RRC
...
===========================================================================*/
void mm_send_rrc_secure_call_ind
(
...
#ifdef FEATURE_DUAL_SIM
  tdsrrc_cmd->cmd.rrc_secure_call_ind.as_id = (sys_modem_as_id_e_type)mm_sub_id;
#endif
MSG_HIGH_DS_0(MM_SUB, "=MM= MM sent TDSRRC_SECURE_CALL_IND");
  tdsrrc_cmd->cmd_hdr.cmd_id = TDSRRC_SECURE_CALL_IND;
  tdsrrc_cmd->cmd.rrc_secure_call_ind.secure_voice_status = secure_voice_status;
...
} /* mm_send_rrc_secure_call_ind () */
#endif
#ifdef FEATURE_TDSCDMA
void mm_send_rrc_barring_info_ind
(
...
#ifdef FEATURE_DUAL_SIM
  tdsrrc_cmd->cmd.tdsrrc_barring_info_ind.as_id = (sys_modem_as_id_e_type)mm_sub_id;
#endif
  MSG_HIGH_DS_0(MM_SUB, "=MM= MM sent TDSRRC_BARRING_INFO_IND");
  tdsrrc_cmd->cmd_hdr.cmd_id = TDSRRC_BARRING_INFO_IND;
  tdsrrc_cmd->cmd.tdsrrc_barring_info_ind.nas_cs_domain_no_rsp_type = call_response_type;
...
void mmsend_mmr_service_stat_ind( mm_as_id_e_type mm_as_id_in)
{
  ...
  MSG_HIGH_DS_2(mm_as_id_in+1,"=MM= Sending MMR_SERVICE_STAT_IND with service domain = %d and service status = %d",
...
FUNCTION mm_send_rrc_g_meas_stop_ind

DESCRIPTION
  This function notifies to RRC whether to begin or end GSM measurements
...
===========================================================================*/
void mm_send_rrc_g_meas_stop_ind(boolean status )
    {
#ifdef FEATURE_WCDMA
           rrc_cmd_type *rrc_cmd = NULL;
        ...
        /* This feature is not applicable to TDSCDMA */
          ...
          if (mm_wcdma_supported)
          {
            ...
            rrc_cmd->cmd_hdr.cmd_id = (int)RRC_G_MEAS_STOP_IND;
            rrc_cmd->cmd.rrc_g_meas_stop_ind.stop_g_meas= status;
            MSG_HIGH_DS_1(MM_SUB,"=MM= sent RRC_G_MEAS_STOP_IND to RRC with stop_g_meas %d",status);
...
FUNCTION MM_SEND_MMR_NON_ACQ_UPDATE_IND
...
===========================================================================*/
void mm_send_mmr_non_acq_update_ind(sys_voice_domain_pref_e_type voice_domain_pref)
{
  mmr_non_acq_update_ind_s_type mmr_non_acq_update_ind;

  mmr_non_acq_update_ind.message_header.message_set = MS_MM_REG;
  mmr_non_acq_update_ind.message_header.message_id  = (int)MMR_NON_ACQ_UPDATE_IND;
  PUT_IMH_LEN( sizeof( mmr_non_acq_update_ind_s_type ) - sizeof(IMH_T),
                            &mmr_non_acq_update_ind.message_header );

  mmr_non_acq_update_ind.voice_domain_pref = voice_domain_pref;
  MSG_HIGH_DS_0(MM_SUB,"=MM= MM sent MMR_NON_ACQ_UPDATE_IND");

  mm_send_message( (IMH_T *)&mmr_non_acq_update_ind, GS_QUEUE_REG );
}
#endif
