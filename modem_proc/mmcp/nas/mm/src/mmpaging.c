...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/mm/src/mmpaging.c#1 $    $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...
static void mmsend_page_response_on_new_rrc_conn(mm_cmd_type * msg_p)
{
   ...
   QEVENT(QSH_CLT_MM, MM_QSH_EVENT_CS_CALL_STATUS, MM_QSH_EVENT_CS_CALL_STATUS_VERSION, mm_sub_id,
             "=MM= Call status %d Call direction %d", CAFI_KPI_EVENT_IND_START, CM_CALL_DIRECTION_MT);
...
#ifdef FEATURE_LTE_TO_GSM
   mm_last_page_type = paging_type;
#endif 
     MSG_HIGH_DS_1(MM_SUB,"=MM= mm_last_page_type set to %d",mm_last_page_type); 
   /*Starting Time for GSM case, for l2g, timer already started in emm*/
  if(
#ifdef FEATURE_DUAL_SIM                
