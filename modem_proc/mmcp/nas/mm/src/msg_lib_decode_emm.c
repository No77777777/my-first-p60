...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/mm/src/msg_lib_decode_emm.c#4 $
$Author: pwbldsvc $
$DateTime: 2024/09/05 02:50:41 $
...
)
{
  ...
  ASSERT(dl_generic_nas_transport != NULL);
  ...
  {
    MSG_ERROR_DS_0(MM_SUB, "=NAS_MSG_LIB= Invalid Generic message container type = %d received. Valid range 1-2");
    *buffer_length = 0;
    return FALSE;
  }
...
)
{
  ...
  ASSERT(pos != NULL);
  ASSERT(buffer_length != NULL);
  ASSERT(cs_serv_notification != NULL);
      ...
      case CALLING_LINE_INFO_IEI:
        if(opt_iei_index == 1)
        {
          ...
          if(iei_len > 0)
          {
            ...
            num_bits_to_read = 4;
            while(iei_len && (index < MAX_CLI_DIGITS))
            {
              ...
              /* Per spec 24.008, section 10.5.118, a valid number digit can take values
                 0-9,*,#, A,B,C. If number has odd number of digits, the last digit is marked as "1111".
              */
              if (LOWER_BCD(cs_serv_notification->calling_line_info.digits[index]) >= BCD_NUMBER_END_MARKER) 
              {
                 MSG_ERROR_DS_0(MM_SUB, "NAS_MSG_LIB: Calling BCD number digit is not BCD coded.");
                 valid_ie = FALSE;
                 cs_serv_notification->calling_line_info_exists = FALSE;

                 /* Ajust buffer length for the current read octet and the remaining octets */
                 ADJUST_OFFSET_AND_BUF_LEN_EMM(8 * (iei_len + 1), pos, buffer_length);
                 break;
              }
              else
              {
                 /* Per spec 24.008 section 10.5.93, if the calling party BCD number contains an odd
                    number of digits, bits 5 to 8 of the last octet shall be filled with an end mark
                    coded as "1111".
                 */

                 if (BCD_NUMBER_END_MARKER == cs_serv_notification->calling_line_info.digits[index+1])
                 {
                   if (iei_len > 1)
                   {
                     MSG_ERROR_DS_0(MM_SUB, "=NAS_MSG_LIB= Invalid BCD number received");
                     valid_ie = FALSE;
                     cs_serv_notification->calling_line_info_exists = FALSE;
                   ...
                   {
                      cs_serv_notification->calling_line_info.number_of_digits += 1;
                      valid_ie = TRUE;
                      /* Ajust buffer length for the last octet read */
            ...
            if (FALSE == valid_ie)
            {
              MSG_ERROR_DS_0(MM_SUB, "=NAS_MSG_LIB= Invalid CLI IE received");
              break;
            }
          }
          else
      ...
      case UNIV_LOCAL_TIME_ZONE_IEI:
        if(opt_iei_index <= 4)
        {
          ...
          else
          {
            MSG_ERROR_DS_1(MM_SUB, "=NAS_MSG_LIB= Insufficient buffer length = %d",(*buffer_length/8));
            buffer_length_check_failed = TRUE;
            emm_info->tp_serv_center_time_stamp_exists = FALSE;
            valid_ie = FALSE;
          }
          opt_iei_index = 5;
        }
    ...
    switch(iei_val)
    {
      ...
      case T3412_TIMER_IEI:
        if(opt_iei_index == 1)
        {
          ...
          valid_ie = TRUE;
          MSG_HIGH_DS_1(MM_SUB, "=NAS_MSG_LIB= T3412 value = %d",tau_accept->t3412_val);
          opt_iei_index = 2;
        }
        else
        {
      ...
      case EMM_CAUSE_IEI:
        if(opt_iei_index <= 7)
        {
          ...
          else
          {
            ...
            valid_ie = TRUE;
            MSG_HIGH_DS_1(MM_SUB, "=NAS_MSG_LIB= EMM cause IE decoded successfully, EMM cause value = %d",tau_accept->emm_cause);
          }
          opt_iei_index = 8;
        }
        else
      ...
      default:
        ...
        if(ADDITIONAL_UPDATE_RESULT_IEI == (byte)(0xF0 & iei_val))
        {
          if(opt_iei_index <= 13)
          {
            ...
            {
              MSG_ERROR_DS_1(MM_SUB, "=NAS_MSG_LIB= Decoding Additional update result IE value = %d is reserved",
                             (0x03 & iei_val));
	    ...
	    else if(NETWORK_POLICY_IEI == (byte)(0xF0 & iei_val))
        {
           tau_accept->emm_network_policy_exists = TRUE;
           if(opt_iei_index <= 15)
           {
              MSG_HIGH_DS_1(MM_SUB, "=NAS_MSG_LIB= Decoding NETWORK_POLICY_IEI %d ", iei_val);
              if((lte_nas_emm_network_policy_type)(0x0F & iei_val) == 0x01)
              {
                  MSG_HIGH_DS_1(MM_SUB, "=NAS_MSG_LIB= NETWORK_POLICY_IEI = %d",(0x0F & iei_val));
                  tau_accept->emm_unsec_redir_allowed = UNSECUR_REDIRECTION_TO_GRR_NOT_ALLOWED;
              }
              else
              {
                 MSG_HIGH_DS_1(MM_SUB, "=NAS_MSG_LIB= NETWORK_POLICY_IEI = %d ",
							 (0x0F & iei_val));
                 tau_accept->emm_unsec_redir_allowed = UNSECUR_REDIRECTION_TO_GRR_ALLOWED;
              }
              valid_ie = TRUE;
              opt_iei_index = 16;
	        }
	        else
            {
               ...
               MSG_ERROR_DS_1(MM_SUB, "=NAS_MSG_LIB= Skip out of sequence or repeated NETWORK_POLICY_IEI, opt_iei_idx = %d",
								 opt_iei_index);
               valid_ie = TRUE;
            }
        }
...
)
{
  ...
  while(*buffer_length)
  {
      ...
      case ESM_IEI:
        if(opt_iei_index == 1)
        {
          if(*buffer_length >= MIN_ESM_CONTAINER_LEN)
          {
            ...
            ADJUST_OFFSET_AND_BUF_LEN_EMM(num_bits_to_read, pos, buffer_length);
            send_nas_ota_msg_log_packet(LOG_LTE_NAS_ESM_OTA_IN_MSG_LOG_C,*buffer_length/8,src + (*pos/8));
            nas_esm_index++;
    ...
    if(valid_ie == FALSE)
    {
      ...
      {
         if(iei_val == ESM_IEI && *buffer_length >= MIN_ESM_CONTAINER_LEN)
        {
          num_bits_to_read = 16;
      ...
      default:
        ...
        if(ADDITIONAL_UPDATE_RESULT_IEI == (byte)(0xF0 & iei_val))
        {
          if(opt_iei_index <= 10)
          {
            ...
            {
              MSG_ERROR_DS_1(MM_SUB, "=NAS_MSG_LIB= Decoding Additional update result IE, result value = %d is reserved",
                             (0x03 & iei_val));
        ...
        {
            attach_accept->emm_network_policy_exists = TRUE;
            if(opt_iei_index <= 12)
            {
              MSG_HIGH_DS_1(MM_SUB, "=NAS_MSG_LIB= Decoding NETWORK_POLICY_IEI %d ", iei_val);
              if((lte_nas_emm_network_policy_type)(0x0F & iei_val) == 0x01)
              {
                  MSG_HIGH_DS_1(MM_SUB, "=NAS_MSG_LIB= NETWORK_POLICY_IEI = %d",(0x0F & iei_val));
                  attach_accept->emm_unsec_redir_allowed = UNSECUR_REDIRECTION_TO_GRR_NOT_ALLOWED;
              }
              else
              {
                 MSG_HIGH_DS_1(MM_SUB, "=NAS_MSG_LIB= NETWORK_POLICY_IEI = %d is reserved",
							 (0x0F & iei_val));
                 attach_accept->emm_unsec_redir_allowed = UNSECUR_REDIRECTION_TO_GRR_ALLOWED;
              }
              valid_ie = TRUE;
              opt_iei_index = 13;
            }
    ...
    switch(iei_val)
    {
      ...
      case NONCE_UE_IEI:
        ...
        else
        {
          ...
          /* Replayed nonceUE IE is of 5 octets and format Type TV. It doesn't have Length
             part so it cannot be skipped correctly after the switch block. Hence set valid_ie
             to TRUE and skip the IE right here. IEI part is already read, hence skip 4*8 bits */
          valid_ie = TRUE;
          num_bits_to_read = 4*8;
          ADJUST_OFFSET_AND_BUF_LEN_EMM(num_bits_to_read, pos, buffer_length);
        }
      break;
      ...
      case NONCE_MME_IEI:
        if(opt_iei_index <= 3)
        {
          ...
          if((*buffer_length/8) >= sizeof(security_mode_cmd->nonce_mme))
          {
...
)
{
  ...
  else
  {
    *buffer_length = 0;
    return_status = FALSE;
    MSG_ERROR_DS_1(MM_SUB, "=NAS_MSG_LIB= EMM cause value = %d ", unpackedbytes );
  }
