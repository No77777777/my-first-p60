...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/mm/src/mmplmnsrch.c#1 $    $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
               ...
               */
                   ...
                   )
               {
                 mmr_plmn_search_cnf->plmn_search_result = REG_MM_PLMN_SEARCH_ABORTED;
                  if( (mm_emm_is_qrd_device == FALSE) && 
                      (mm_partial_search_allowed == FALSE)&&
                      (mm_emm_manual_search_partial_get() == FALSE))
                  {
               ...
               if((mmr_plmn_search_cnf->plmn_search_result != REG_MM_PLMN_SEARCH_PARTIAL) ||
                  ((mm_emm_is_qrd_device == TRUE || 
					...
					mm_emm_manual_search_partial_get() == TRUE) &&
                   (mmr_plmn_search_cnf->found_plmn_list.plmn_list.length != 0 ) &&
                   (mmr_plmn_search_cnf->plmn_search_result == REG_MM_PLMN_SEARCH_PARTIAL)))
               {
               ...
               if(mm_plmn_search_in_progress == FALSE)
               {
            ...
            case RRC_PLMN_LIST_CNF:
               ...
               */
               ...
               if ( (msg_p->cmd.rrc_plmn_list_cnf.status == MM_AS_ABORTED) ||
                    (msg_p->cmd.rrc_plmn_list_cnf.status == MM_AS_ABORTED_DUE_TO_NO_LOCK) )
               {
                 mmr_plmn_search_cnf->plmn_search_result = (msg_p->cmd.rrc_plmn_list_cnf.status == MM_AS_ABORTED_DUE_TO_NO_LOCK)?
                                                            REG_MM_PLMN_SEARCH_ABORTED_DUE_TO_NO_LOCK : REG_MM_PLMN_SEARCH_ABORTED;
                 mmr_plmn_search_cnf->found_plmn_list.plmn_list.length = 0;
                 if((mm_emm_is_qrd_device == FALSE) &&
                    (mm_partial_search_allowed == FALSE) && 
					(mm_emm_manual_search_partial_get() == FALSE))
...
void mm_request_list_from_rr(mmr_plmn_search_req_s_type *plmn_srch_ptr)
{
   ...
   */
#ifdef FEATURE_DUAL_SIM
  mm_update_rat_status(&plmn_srch_ptr->rat_pri_list_info,
       ...
       (plmn_srch_ptr->rat == SYS_RAT_TDS_RADIO_ACCESS))
   {
...

      {
		...
		rrc_cmd_p->cmd.plmn_list_req.search_priority = SYS_SRCH_PRIORITY_NORMAL;
           ...
           (rrc_cmd_p->cmd.plmn_list_req.service_search == SYS_SERVICE_SEARCH_NONE))
        {
            ...
            if(plmn_srch_ptr->incremental_results.enabled == TRUE && 
			   plmn_srch_ptr->srch_priority != SYS_SRCH_PRIORITY_NORMAL)
            {
              mm_emm_manual_search_timer_set(plmn_srch_ptr->incremental_results.periodicity,plmn_srch_ptr->arfcn_list,plmn_srch_ptr->rat_pri_list_info);
               if(plmn_srch_ptr->max_search_time != 0)
              {
                rrc_cmd_p->cmd.plmn_list_req.search_timer_value.MaxManualPLMNTimer = 
                                                     plmn_srch_ptr->max_search_time;

              }
            }
            rrc_cmd_p->cmd.plmn_list_req.search_timer_value = mm_emm_manual_search_timer_value;

            if(plmn_srch_ptr->srch_priority == SYS_SRCH_PRIORITY_NORMAL)
            {
               if(plmn_srch_ptr->incremental_results.periodicity != 0 )
              {
                  rrc_cmd_p->cmd.plmn_list_req.search_timer_value.FirstPLMNTimer = 
                                        plmn_srch_ptr->incremental_results.periodicity;
                  rrc_cmd_p->cmd.plmn_list_req.search_timer_value.MorePLMNTimer = 
                                        plmn_srch_ptr->incremental_results.periodicity;
              }
            ...
            mm_emm_manual_search_param_set(plmn_srch_ptr->srch_priority , plmn_srch_ptr->incremental_results.enabled);
			rrc_cmd_p->cmd.plmn_list_req.arfcn_list = plmn_srch_ptr->arfcn_list;
			rrc_cmd_p->cmd.plmn_list_req.preferred_plmn_list = plmn_srch_ptr->preferred_plmn_list;

	        rrc_cmd_p->cmd.plmn_list_req.search_priority = plmn_srch_ptr->srch_priority;
             MSG_HIGH_DS_3(MM_SUB, "=MM= Manual PLMN Timer value FIRST %u  MORE %u  MAX %u",
                              rrc_cmd_p->cmd.plmn_list_req.search_timer_value.FirstPLMNTimer,
                              rrc_cmd_p->cmd.plmn_list_req.search_timer_value.MorePLMNTimer,
                              rrc_cmd_p->cmd.plmn_list_req.search_timer_value.MaxManualPLMNTimer);

			MSG_HIGH_DS_5(MM_SUB,"=MM= srch_priority %d enabled %d lte count %d Wcdma cound %d ppl count %d",
                         plmn_srch_ptr->srch_priority ,
                         plmn_srch_ptr->incremental_results.enabled,
                         rrc_cmd_p->cmd.plmn_list_req.arfcn_list.lte_count,
                         rrc_cmd_p->cmd.plmn_list_req.arfcn_list.wcdma_count,
                         rrc_cmd_p->cmd.plmn_list_req.preferred_plmn_list.count);
		
			
#if defined (FEATURE_LTE) && defined (FEATURE_DUAL_SIM) && !defined (FEATURE_MODEM_ATC) 
            emm_send_rrc_manual_search_indication(TRUE);
#endif
 ...
 if((rrc_cmd_p->cmd.plmn_list_req.network_select_mode == SYS_NETWORK_SELECTION_MODE_MANUAL) &&
...
#ifndef FEATURE_DISABLED_HABANERO   
	  rr_plmn_list_req->srch_priority = SYS_SRCH_PRIORITY_NORMAL;
         ...
         (rr_plmn_list_req->service_search == SYS_SERVICE_SEARCH_NONE))
      {
			 ...
			 plmn_srch_ptr->srch_priority != SYS_SRCH_PRIORITY_NORMAL)
        {
          mm_emm_manual_search_timer_set(plmn_srch_ptr->incremental_results.periodicity,plmn_srch_ptr->arfcn_list,plmn_srch_ptr->rat_pri_list_info);
            if(plmn_srch_ptr->max_search_time != 0)
              {
                  rr_plmn_list_req->search_timer_value.MaxManualPLMNTimer = 
                                        plmn_srch_ptr->max_search_time;
              
              }
        }
         rr_plmn_list_req->srch_priority = plmn_srch_ptr->srch_priority;
		 rr_plmn_list_req->search_timer_value = mm_emm_manual_search_timer_value;
		 

        if(plmn_srch_ptr->srch_priority == SYS_SRCH_PRIORITY_NORMAL)/*FR52036*/
          {
            if(plmn_srch_ptr->incremental_results.periodicity != 0 )
            {
                rr_plmn_list_req->search_timer_value.FirstPLMNTimer = 
                                      plmn_srch_ptr->incremental_results.periodicity;
                rr_plmn_list_req->search_timer_value.MorePLMNTimer = 
                                      plmn_srch_ptr->incremental_results.periodicity;
            }
              if(plmn_srch_ptr->max_search_time != 0)
              {
		 ...
		 rr_plmn_list_req->arfcn_list = plmn_srch_ptr->arfcn_list;
		 rr_plmn_list_req->preferred_plmn_list= plmn_srch_ptr->preferred_plmn_list;
         MSG_HIGH_DS_8(MM_SUB,"Manual PLMN Timer  value FIRST %d	MORE %d  MAX %d Search Prio %d Partial enabled = %d lte count %d Wcdma cound %d ppl count %d",
                       rr_plmn_list_req->search_timer_value.FirstPLMNTimer,
                       rr_plmn_list_req->search_timer_value.MorePLMNTimer,
                       rr_plmn_list_req->search_timer_value.MaxManualPLMNTimer,
                       rr_plmn_list_req->srch_priority,
                       plmn_srch_ptr->incremental_results.enabled,
                       rr_plmn_list_req->arfcn_list.lte_count,
                       rr_plmn_list_req->arfcn_list.wcdma_count,
                       rr_plmn_list_req->preferred_plmn_list.count);
         MSG_HIGH_DS_1( MM_SUB, "=MM= BST changes length of mcc mode filter =%d",
                                      plmn_srch_ptr->mcc_mode_band_filter.num_items);
   ...
   if(((plmn_srch_ptr->network_selection_mode == SYS_NETWORK_SELECTION_MODE_AUTOMATIC 
  	 && (plmn_srch_ptr->service_search == SYS_SERVICE_SEARCH_NONE))||
  	 (plmn_srch_ptr->service_search == SYS_SERVICE_SEARCH_IRAT_PLMN)))
   	{
