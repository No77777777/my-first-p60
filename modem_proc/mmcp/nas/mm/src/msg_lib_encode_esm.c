...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/mm/src/msg_lib_encode_esm.c#1 $
$Author: pwbldsvc $
...
lte_nas_esm_outgoing_msg_type *get_esm_container(lte_nas_outgoing_msg_type *out_buf)
{
  ...
  else if(out_buf->nas_hdr.pd == EPS_MOBILITY_MANAGEMENT_MESSAGES)
  {
    ...
    case ATTACH_COMPLETE:
      ...
      break;
    default:
      MSG_ERROR_DS_1(MM_SUB, "MSG_LIB: ESM encoding was called from the EMM message type with enum id = %d",
                    (byte)out_buf->nas_hdr.msg_id);
      MSG_ERROR_DS_0(MM_SUB, "This case is impossible, it would be due to some memory corruption");
      out_msg = NULL;
      break;
