...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/mm/src/gmmmsg.c#2 $ $DateTime: 2021/09/09 05:07:01 $ $Author: pwbldsvc $
...
boolean  gmm_force_for_always_on_data_sim[MAX_NAS_STACKS] ;
...
ssg_mm_rand_type rand_val_store_gmm_sim[MAX_NAS_STACKS];
#define rand_val_store_gmm rand_val_store_gmm_sim[mm_as_id]
#else
boolean  data_is_available ;
boolean  gmm_force_for_always_on_data;
ssg_mm_rand_type rand_val_store_gmm;
...
{
#if defined FEATURE_DUAL_SIM || defined FEATURE_SGLTE
  MSG_ERROR_DS_0( 1, "=MM= DUAL SIM: Wrong API called get_revision_level_indicator()" );
...
DESCRIPTION
  This function stores rand value for GMM_AUTH
...
{
	rand_val_store_gmm.value_length=p_rand->value_length;
	memscpy((void *)&rand_val_store_gmm.value_data[0], sizeof(rand_val_store_gmm.value_data), 
        (void *)&p_rand->value_data[0], p_rand->value_length);
...
DESCRIPTION
  This function returns rand value for GMM_AUTH
...
#if defined(FEATURE_NAS_REL12_SPEC_CHANGE) && defined(FEATURE_REL_12_MSRAC_CHANGE)
   ...
   /* If both "E-UTRA FDD support" and "E-UTRA TDD support" bits are set to '0', 
    ...
    TRUNCATE_RAC (max_rac_length, 1, encoder, access_caps_length_bit_pos)
     if (
         ...
         (!MM_IS_IN_SGLTE_STATE_4) &&
#endif
         (mm_checkif_rat_pri_list_has_rat(&mm_rat_pri_list_info_sim[as_id],SYS_RAT_LTE_RADIO_ACCESS) ||
          mm_checkif_rat_pri_list_has_rat(&mm_rat_pri_list_info_sim[as_id],SYS_RAT_UMTS_RADIO_ACCESS) || 
          mm_checkif_rat_pri_list_has_rat(&mm_rat_pri_list_info_sim[as_id],SYS_RAT_TDS_RADIO_ACCESS)) &&
         (mm_checkif_rat_pri_list_has_rat(&mm_rat_pri_list_info_sim[as_id],SYS_RAT_GSM_RADIO_ACCESS)))
	 {
           csn_encode_bit(encoder, (byte) 1 );
	 }
	else
...
)
{
 ...
 #ifdef FEATURE_DUAL_SIM
	status = ghdi_ds_read_hw_classmark( &hw_classmark,(sys_modem_as_id_e_type)mm_sub_id);
 #else
 ...
 */
  ...
  ** --------------------- */
  ...
  gmm_ps_fop_req = FALSE;
   ...
   if (is_umts_mode() &&
	...
	||gmm_force_for_always_on_data==TRUE))
  {
    /* -------------------------------------------------
  ...
  **-------------------------------------------*/
   ...
   ** in the COMBINED ATTACH REQUEST message
   **-----------------------------------------------------------------------------------------*/
  ...
  ** --------------------------------------- */
  ...
  mm_ssgccs_handle_outgoing_msg(GMM_ATTACH_REQUEST_MSG,temp_identity,mm_serving_plmn.info.active_rat );
  ...
  gmm_qsh_ota_metric_signaling_log(GMM_ATTACH_REQUEST,UE_TO_CN_DIRECTION,
                                                     (uint8)attach_type, MM_GMM_NO_FAILURE_DEBUG);
#endif
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  mm_ssgccs_handle_outgoing_msg(GMM_ATTACH_COMPLETE_MSG,NO_PAYLOAD,mm_serving_plmn.info.active_rat );
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  ** query PS again */
  ...
  if ((is_umts_mode()) &&
      ...
      ( 
        ...
        gmm_service_request_initiator == MS_CC_MM || gmm_force_for_always_on_data==TRUE))
    {
    /* -------------------------------------------------
    ** Follow-on request (bit 4) applicable only in UMTS
    ** ------------------------------------------------- */
  ...
  ** --------------------- */
...
#endif
  {
    ...
    ** If the MS has stored a valid TMSI, the MS shall include the TMSI based NRI container IE 
    ** in the COMBINED RAU REQUEST message
    **------------------------------------------------------------------------------------------*/
#ifdef FEATURE_NAS_REL11
      if(mm_nv_nas_rel_compliance >= NV_NAS_COMPLIANCE_REL11)
      {
  ...
  ** ---------------------------------------------------- */
  ...
  mm_ssgccs_handle_outgoing_msg(GMM_ROUTING_AREA_UPDATE_REQUEST_MSG,NO_PAYLOAD,mm_serving_plmn.info.active_rat );
  ...
  if(gmm_rrc_establish_cause != RRC_EST_TERM_UNKNOWN)
  {
    mmrrcconn_send_L3_message_to_rrc(
      &ra_update_request, gmm_rrc_establish_cause, RRC_PS_DOMAIN_CN_ID );
...
#ifdef FEATURE_QSH_EVENT_METRIC
      gmm_qsh_ota_metric_signaling_log(GMM_ROUTING_AREA_UPDATE_REQUEST,UE_TO_CN_DIRECTION,
                                       (uint8)(ra_update_type & 0x07), MM_GMM_NO_FAILURE_DEBUG);
#endif
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  mm_ssgccs_handle_outgoing_msg(GMM_ROUTING_AREA_UPDATE_COMPLETE_MSG,NO_PAYLOAD,mm_serving_plmn.info.active_rat );
      ...
      case GMM_IEI_PTMSI:
            ...
            ((msg_index + 2+ iei_length) <= msg_length))
        {
          ...
          else
          {
            for (i=0; i<PTMSI_SIZE; i++)
            {
              if (message->cmd.mm_data_ind.L3_data[msg_index+3+i] != 0xFF)
              {
                ptmsi_valid = TRUE;
                break;
              }
...
#endif
  {
    /* TS 24.007, subclause 11.4.2  
     * The mobile station will consider the request for a non-supported algorithm as an error.
     * When that happens the mobile station will follow TS 24.008, subclause 8.8
    */
    if (gmm_anc_request_message->ciphering_algorithm == (int) GMMLLC_GEA_1)
    {
      gmm_send_gmm_status_message( SEMANTICALLY_INCORRECT_MESSAGE );
    }
    else
        ...
        */
        if (rand_present == FALSE)
        {
          ...
          if(msg_index+MAX_AUTH_DATA_LENGTH < total_msg_length)
          {
			...
			store_rand_gmm(&gmm_anc_request_message->rand);
			mm_ssgccs_handle_icoming_msg(GMM_AUTHENTICATION_AND_CIPHERING_REQUEST,mm_serving_plmn.info.active_rat,PS_DOMAIN_PROCEDURE,(mm_cmd_type *)&gmm_anc_request_message->rand);
          }
          else
          {
            MSG_ERROR_DS_0(MM_SUB,"=MM= Index out of bounds and buffer overflow");
