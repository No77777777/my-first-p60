...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/mm/src/mmrat.c#1 $ $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
         ...
         if (msg_p->cmd.hdr.message_id == (int) MMR_REG_REQ)
         {
                ...
                (mm_serving_plmn.info.reg_service_domain == SYS_SRV_DOMAIN_CS_PS))
            {
               if ((mm_serving_plmn.info.reg_service_domain == SYS_SRV_DOMAIN_CS_PS) &&
                   ...
                   (mm_serving_plmn.network_op_mode == NETWORK_OP_MODE_1)&& mm_gs_association_established)
               {
               ...
               else if (mm_state == MM_IDLE )
               {
                  ...
                  {  
                     if ((msg_p->cmd.mmr_reg_req.req_service_domain ==
                         ...
                         (mm_serving_plmn.gmm_service_state == SERVICE_NOT_AVAILABLE))|| 
						 ((mm_serving_plmn.info.reg_service_domain == SYS_SRV_DOMAIN_CS_PS) &&
                          (mm_serving_plmn.network_op_mode == NETWORK_OP_MODE_1) && mm_gs_association_established == FALSE)))
                     {
               ...
               if ((mm_state == MM_IDLE) &&
                   ((gmm_state == GMM_NULL) || (gmm_state == GMM_DEREGISTERED) ||
                    (mm_serving_plmn.gmm_service_state == SERVICE_NOT_AVAILABLE) ||
                    (((gmm_state == GMM_REGISTERED) || (gmm_state == GMM_ROUTING_AREA_UPDATING_INITIATED)) && 
                     (msg_p->cmd.mmr_reg_req.volte_only == TRUE &&
  ...
  else
  {
    switch (msg_p->cmd.hdr.message_id)
    {
      case RR_ACTIVATION_IND:
        ...
        {
          /* WtoGCCO will only receive when PMM is in connected or idle state as well */
            ...
            mm_backup_rrc_connection_status  = rrc_connection_status;
            mm_backup_cs_session_status      = cs_session_status;
            mm_backup_ps_session_status      = ps_session_status;
...
#error code not present
          ...
          /* Spec 24.008 4.7.5.1 - CR 2553: W to G reselection triggered in PMM connected mode. If UE reselects back to W without triggering 
           * RAU in A/Gb mode, then need to trigger RAU in Iu mode to sync PMM state */
          ...
          {
            gmm_sync_pmm_state_during_w_to_lg_and_back_to_w_irat = TRUE;
            MSG_HIGH_DS_0(MM_SUB,"=MM= WtoG reselection triggered when PMM connected");
          }
#endif
...
&&get_wcdma_interface()
            ...
            (gmm_state == GMM_DEREGISTERED_INITIATED))
        {
          gmm_handle_connection_management( msg_p );
                ...
                (gmm_state == GMM_REGISTERED) && gmm_gprs_detach_pending)
       {
        ...
        if (msg_p->cmd.rrc_activation_ind.status == MM_AS_WTOG_CC_ORDER_FAILED)
        {
          ...
          WtoGCCO_state = CCO_FAILED;
          rrc_connection_status = mm_backup_rrc_connection_status;
          cs_session_status     = mm_backup_cs_session_status;
          ps_session_status     = mm_backup_ps_session_status;
        }
#endif /* FEATURE_SPEC_COMPLIANT_WTOG_CELL_CHANGE_ORDER */
...
void mm_handle_stop_mode_cnf (mm_cmd_type *msg_p)
{
  ...
  ** -------------------------------- */
...
#ifdef FEATURE_DUAL_SIM
      mm_update_rat_status(&mmcoord_pending_reg_message.cmd.mmr_reg_req.rat_pri_list_info,SYS_RAT_ACTIVATION_STATUS_DEACTIVATED);
#endif
...
#ifdef FEATURE_DUAL_SIM
  else if((mm_timer_status[TIMER_MM_DUAL_SWITCH] == TIMER_ACTIVE) && (!is_lte_mode()))
  {
	  mm_send_activation_rsp_if_needed();
  ...
  {
    MSG_HIGH_DS_2(MM_SUB,"=MM= WRLF rat change pending, mmcoord_reg_req_pending:%d, mm_ps_detach_state:%d", 
                           mmcoord_reg_req_pending, mm_ps_detach_state);
    mm_serving_plmn.mm_service_state  = SERVICE_NOT_AVAILABLE;
    mm_serving_plmn.gmm_service_state = SERVICE_NOT_AVAILABLE;
...
void mm_send_rr_stop_mode_req( rr_mm_gsm_stop_reason_T stop_reason )
{
  ...
  if (!mm_waiting_for_stop_mode_cnf)
  {
    ...
    if((mmcoord_pending_reg_message.cmd.mmr_reg_req.network_selection_mode == SYS_NETWORK_SELECTION_MODE_LIMITED) &&
       ...
       (stop_reason == RRC_USER_RAT_CHANGE || stop_reason == RRC_RAT_CHANGE))
    {
