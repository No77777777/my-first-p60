...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/mm/src/gmmsuspend.c#1 $ $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...
void gmm_handle_gprs_resumption ( mm_cmd_type *message )
{
  ...
  boolean send_sm_resend_ind = FALSE;
...
** ---------------------------- ---------------------------- ---------------------------- ----------------------------*/
        ...
        else
        {
           ...
           {
             MSG_HIGH_DS_0(MM_SUB, "=MM= Perform local detach on resumption failure due to high priority user action");
      ...
      if(send_sm_resend_ind == TRUE && gmmllc_get_glif_llc_conn_state() == LLC_RESUMED)
      {
