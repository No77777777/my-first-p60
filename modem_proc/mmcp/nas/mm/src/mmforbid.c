...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/mm/src/mmforbid.c#1 $ $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...
nas_lai_reject_list_T   lai_reject_list_sim[MAX_NAS_STACKS];
...
#else
static nas_lai_reject_list_T   lai_reject_list;
...
static void mm_remove_forbidden_roaming( uint32 area, inter_task_lai_T *p_lai )
{
   ...
   for (count = 0; count < forbidden_list[area].write_ptr; count++ )
   {
                  ...
                  LAI_SIZE ) == 0 )    /* if match found */
      {
         ...
         {
            (void)memscpy( &forbidden_list[area].lai[count],LAI_SIZE+1,
                           &forbidden_list[area].lai[count+1], LAI_SIZE+1 );
         }
...
void mm_add_forbidden_roaming( byte area, inter_task_lai_T * p_lai,boolean is_lai_added_due_to_t3247 )
{
   byte count = 0;
   boolean is_lai_already_present_in_list = FALSE;
   /*
   ...
   if ( mm_check_forbidden_roaming( area, p_lai ) == FALSE )
   {
      ...
      if ( forbidden_list[area].write_ptr == MAX_NO_FORBIDDEN_LAIS )
      {
         ...
         forbidden_list[area].lai[forbidden_list[area].write_ptr-1].is_lai_added_due_to_t3247 = is_lai_added_due_to_t3247;
      }
      else
      {
         (void)memscpy( &forbidden_list[area].lai[forbidden_list[area].write_ptr], LAI_SIZE,
                        p_lai, LAI_SIZE );
         forbidden_list[area].lai[forbidden_list[area].write_ptr].is_lai_added_due_to_t3247 = is_lai_added_due_to_t3247;
         forbidden_list[area].write_ptr = (byte)(forbidden_list[area].write_ptr + 1) ;
      }
	  if(is_lai_added_due_to_t3247)
	  {
       MSG_HIGH_DS_5(MM_SUB,"=MM= LAI %x %x %x %x %x  added to forbbiden list due to DOS Mitigation",forbidden_list[area].lai[count].lai.PLMN_id.identity[0],
	   	                                                                 forbidden_list[area].lai[count].lai.PLMN_id.identity[1],forbidden_list[area].lai[count].lai.PLMN_id.identity[2],
	   	                                                                 forbidden_list[area].lai[count].lai.location_area_code[0],forbidden_list[area].lai[count].lai.location_area_code[1]);
	  }
   ...
   {
     for (count = 0; count < forbidden_list[area].write_ptr && count < MAX_NO_FORBIDDEN_LAIS ; count++ )
     {
      if (memcmp( &forbidden_list[area].lai[count],
                  p_lai,
                  LAI_SIZE ) == 0 )    
      {
       is_lai_already_present_in_list = TRUE;
        break;
      }
    }
	if(is_lai_already_present_in_list == TRUE && forbidden_list[area].lai[count].is_lai_added_due_to_t3247 == FALSE && is_lai_added_due_to_t3247)
	{
       forbidden_list[area].lai[count].is_lai_added_due_to_t3247 = is_lai_added_due_to_t3247;
	   	  if(is_lai_added_due_to_t3247)
	  {
       MSG_HIGH_DS_5(MM_SUB,"=MM= LAI %x %x %x %x %x  added to forbbiden list due to DOS Mitigation",forbidden_list[area].lai[count].lai.PLMN_id.identity[0],
	   	                                                                 forbidden_list[area].lai[count].lai.PLMN_id.identity[1],forbidden_list[area].lai[count].lai.PLMN_id.identity[2],
...
void mm_add_forbidden_national_roaming( inter_task_lai_T * p_lai , boolean is_lai_added_due_to_t3247)
{
   mm_add_forbidden_roaming( NATIONAL_ROAMING, p_lai,is_lai_added_due_to_t3247 );
}
...
VOID_FUNC mm_add_forbidden_regional_roaming( inter_task_lai_T * p_lai, boolean is_lai_added_due_to_t3247)
{
   mm_add_forbidden_roaming( REGIONAL_ROAMING, p_lai,is_lai_added_due_to_t3247);
}
...
void fill_rrc_forbidden_lai_list(rrc_lai_list_type *list_p)
{
   ...
   for (count = 0; count < forbidden_list[NATIONAL_ROAMING].write_ptr; count++)
   {
      plmn_id.identity[0] = forbidden_list[NATIONAL_ROAMING].lai[count].lai.PLMN_id.identity[0];
      plmn_id.identity[1] = forbidden_list[NATIONAL_ROAMING].lai[count].lai.PLMN_id.identity[1];
      plmn_id.identity[2] = forbidden_list[NATIONAL_ROAMING].lai[count].lai.PLMN_id.identity[2];
      list_p->lai[no_of_entries].plmn =
         mmumts_convert_nas_plmn_id_to_rrc_plmn_id( plmn_id );

      list_p->lai[no_of_entries].lac[0] =
         forbidden_list[NATIONAL_ROAMING].lai[count].lai.location_area_code[0];
      list_p->lai[no_of_entries].lac[1] =
         forbidden_list[NATIONAL_ROAMING].lai[count].lai.location_area_code[1];
...
boolean mm_per_subs_rrc_registration_in_progress(sys_modem_as_id_e_type sub_id)
{
  ...
  MSG_HIGH_DS_1( sub_id+1, "=MM= mm_per_subs_rrc_registration_in_progress(only applicable to WRLF) returns %d",reg_in_progress);
  ...
  if(lai != NULL)
  {
    ...
    for(i=0;i<lai_reject_list.length;i++)
    {
                ...
                LAI_SIZE ) == 0 )    /* if match found */
      {
		...
		if( lai_reject_list.lai[i].is_lai_added_due_to_t3247 == FALSE && is_lai_due_to_t3247 )
		{
          lai_reject_list.lai[i].is_lai_added_due_to_t3247 = TRUE;
          MSG_HIGH_DS_0(MM_SUB,"=MM= LAI is updated as DOS Mitigation LAI");
		}
        break;
      }
    }
    ...
    if(!found)
    {
      ...
      lai_reject_list.lai[lai_reject_list.write_ptr].is_lai_added_due_to_t3247 = is_lai_due_to_t3247;
      ...
      MSG_HIGH_DS_3(MM_SUB, "=MM= LAI added in Reject List. PLMN [%X %X %X]",
                lai->PLMN_id.identity[0], lai->PLMN_id.identity[1], lai->PLMN_id.identity[2]);
      MSG_HIGH_DS_3(MM_SUB, "=MM= LAC [%X %X],is_lai_added_due_to_dos_mitigation -%d", lai->location_area_code[0], lai->location_area_code[1],is_lai_due_to_t3247);
    }
  }
...
)
{
  ...
  for(i=0;i<lai_reject_list.length; i++)
  {
    if(sys_plmn_match(plmn, lai_reject_list.lai[i].lai.PLMN_id))
    {
...
===========================================================================*/
 ...
 void mm_copy_lau_reject_list_to_forbidden_list(void)
 {
   ...
   {
     MSG_HIGH_DS_0(MM_SUB,"Inavalid size of reject list ");
     return ;
   }
...
DESCRIPTION: This function is used to check if national roaming is allowed
on a given PLMN
...
boolean mm_check_plmn_forbidden_national_roaming(sys_plmn_id_s_type plmn)
{
   ...
   for (count = 0; count < forbidden_list[NATIONAL_ROAMING].write_ptr; count++ )
   {
      if (sys_plmn_match(forbidden_list[NATIONAL_ROAMING].lai[count].lai.PLMN_id, plmn))
      {
         return_value = TRUE;
         break;
      }
...
FUNCTION  mm_clear_dos_non_dos_forbidden_LAIs

DESCRIPTION: This function is used to check & clear if PLMN is dos forbidden or not
...
void  mm_clear_dos_non_dos_forbidden_LAIs(boolean is_lai_t3247_specific)
{
   byte count,area;
   boolean rr_update_required = FALSE;
   MSG_HIGH_DS_1(MM_SUB, "=MM= Clearing T3247 specific forbidden lai list -%d",is_lai_t3247_specific);  //cob
   for(area =0; area<2; area++)
    {
   for (count = 0; count < forbidden_list[area].write_ptr && count < MAX_NO_FORBIDDEN_LAIS ; count++ )
   {
      if ((is_lai_t3247_specific && forbidden_list[area].lai[count].is_lai_added_due_to_t3247 == TRUE) || 
         (is_lai_t3247_specific == FALSE && forbidden_list[area].lai[count].is_lai_added_due_to_t3247 == FALSE))
...
boolean mm_is_lai_forbidden_due_to_dos_mitigation(inter_task_lai_T * p_lai)
{
  byte count = 0, area;
  ...
  for(area =0; area<2; area++)
  {
      for (count = 0; count < forbidden_list[area].write_ptr && count < MAX_NO_FORBIDDEN_LAIS ; count++ )
      {
         if (forbidden_list[area].lai[count].is_lai_added_due_to_t3247 == TRUE &&
             memcmp(&forbidden_list[area].lai[count].lai,p_lai,LAI_SIZE) == 0)
         {
            result = TRUE;
			
			MSG_HIGH_DS_5(MM_SUB,"=MM= LAI %x %x %x %x %x  is forbidden due to DOS Mitigation",forbidden_list[area].lai[count].lai.PLMN_id.identity[0],
																			  forbidden_list[area].lai[count].lai.PLMN_id.identity[1],forbidden_list[area].lai[count].lai.PLMN_id.identity[2],
																			  forbidden_list[area].lai[count].lai.location_area_code[0],forbidden_list[area].lai[count].lai.location_area_code[1]);
            break;
         }
...
void mm_clear_t3247_specific_lai_reject_list()
{
  byte count ;
  boolean is_lower_layer_update_req = FALSE;
  MSG_HIGH_DS_0(MM_SUB, "=MM= Clearing T3247 specific lai reject list");
  for (count = 0; count < lai_reject_list.length; count++ )
  {
     if(lai_reject_list.lai[count].is_lai_added_due_to_t3247 == TRUE)
     {
        is_lower_layer_update_req = TRUE;
        for(; (count<lai_reject_list.write_ptr-1)  && (count<MAX_NO_FORBIDDEN_LAIS-1); count++)
        {
           (void) memscpy(&lai_reject_list.lai[count],LAI_SIZE+1,
                    &lai_reject_list.lai[count+1],LAI_SIZE+1);
        }
     lai_reject_list.write_ptr = (byte)(lai_reject_list.write_ptr -1);
     }
  }
  if(is_lower_layer_update_req)
  {
     mm_send_rrc_lai_reject_list_ind(&lai_reject_list);
     mm_send_rr_lai_reject_list_ind(&lai_reject_list);
  }
