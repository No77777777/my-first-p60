...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/mm/src/emm_security.c#6 $
...
extern byte dl_sqn_for_current_msg_sim[MAX_AS_IDS];
...
#if defined(FEATURE_DUAL_SIM) && defined(FEATURE_DUAL_DATA)
byte computed_kasme_sim[MAX_AS_IDS][32];
#define computed_kasme computed_kasme_sim[mm_as_id]

byte computed_nas_int_key_sim[MAX_AS_IDS][16];
#define computed_nas_int_key computed_nas_int_key_sim[mm_as_id]

byte computed_nas_enc_key_sim[MAX_AS_IDS][16];
#define computed_nas_enc_key computed_nas_enc_key_sim[mm_as_id]

static client_id  auth_transaction_id_sim[MAX_AS_IDS] = {(client_id)NULL, (client_id)NULL};
#define auth_transaction_id auth_transaction_id_sim[mm_as_id]

static boolean reset_nas_ul_count_sim[MAX_AS_IDS] = {FALSE, FALSE};
#define reset_nas_ul_count reset_nas_ul_count_sim[mm_as_id]

static boolean reset_nas_dl_count_sim[MAX_AS_IDS] = {FALSE, FALSE};
#define reset_nas_dl_count reset_nas_dl_count_sim[mm_as_id]

static lte_nas_message_id ongoing_sec_procedure_sim[MAX_AS_IDS] = {ESM_LAST_UNDEFINED,ESM_LAST_UNDEFINED};
#define ongoing_sec_procedure ongoing_sec_procedure_sim[mm_as_id]

static lte_nas_emm_native_sec_context_type log_native_sec_ctxt_sim[MAX_AS_IDS];
#define log_native_sec_ctxt log_native_sec_ctxt_sim[mm_as_id]
 

static boolean eps_aka_run_sim[MAX_AS_IDS] = {FALSE, FALSE};
#define eps_aka_run eps_aka_run_sim[mm_as_id]

static boolean dummy_ctxt_sim[MAX_AS_IDS] = {FALSE, FALSE};
#define dummy_ctxt dummy_ctxt_sim[mm_as_id]

static boolean eps_nsc_len_chk_failed_sim[MAX_AS_IDS] = {FALSE, FALSE};
#define eps_nsc_len_chk_failed eps_nsc_len_chk_failed_sim[mm_as_id]

static boolean invalidate_ul_count_sim[MAX_AS_IDS] = {FALSE, FALSE};
#define invalidate_ul_count invalidate_ul_count_sim[mm_as_id]

static byte emm_ck_key_value_sim[MAX_AS_IDS][MAX_AUTH_DATA_LENGTH];
#define emm_ck_key_value emm_ck_key_value_sim[mm_as_id]

static byte emm_ik_key_value_sim[MAX_AS_IDS][MAX_AUTH_DATA_LENGTH];
#define emm_ik_key_value emm_ik_key_value_sim[mm_as_id]

static auth_gprs_cipherkey_type emm_srvcc_ck_key_value_sim[MAX_AS_IDS];
#define emm_srvcc_ck_key_value emm_srvcc_ck_key_value_sim[mm_as_id]

static auth_gprs_integritykey_type emm_srvcc_ik_key_value_sim[MAX_AS_IDS];
#define emm_srvcc_ik_key_value emm_srvcc_ik_key_value_sim[mm_as_id]

static byte   emm_srvcc_eKSI_sim[MAX_AS_IDS];  // Key Set Identifier
#define emm_srvcc_eKSI emm_srvcc_eKSI_sim[mm_as_id]

static byte  prev_dl_cnt_sim[MAX_AS_IDS][4];
#define prev_dl_cnt prev_dl_cnt_sim[mm_as_id]

static auth_rand_type rand_value_cache_sim[MAX_AS_IDS];
#define rand_value_cache rand_value_cache_sim[mm_as_id]

static auth_autn_type autn_cache_sim[MAX_AS_IDS];
#define autn_cache autn_cache_sim[mm_as_id]

static byte  nasksi_cache_sim[MAX_AS_IDS];
#define nasksi_cache nasksi_cache_sim[mm_as_id]

static emm_security_context_info emm_last_known_security_context_sim[MAX_AS_IDS];
#define emm_last_known_security_context emm_last_known_security_context_sim[mm_as_id]


#else
byte computed_kasme[32];

byte computed_nas_int_key[16];

byte computed_nas_enc_key[16];
...
static emm_security_context_info emm_last_known_security_context;
#endif
...
boolean parse_usim_epsnsc( byte *sim_buffer, emm_security_context_info *usim_security_context_ptr)
{
  ...
  secure_memset(&swapped_nas_counts[0],0,sizeof(swapped_nas_counts));
  secure_memset(invalid_kasme,0xFF,sizeof(invalid_kasme));
  secure_memset(invalid_dl_nas_count,0xFF,sizeof(invalid_dl_nas_count));
  secure_memset(invalid_ul_nas_count,0xFF,sizeof(invalid_ul_nas_count));

  ASSERT( sim_buffer != NULL );

  ASSERT ( usim_security_context_ptr != NULL );

  secure_memset(&log_native_sec_ctxt,0,sizeof(lte_nas_emm_native_sec_context_type));
...
log_native_sec_ctxt.eps_nas_sec_context_len = ((log_native_sec_ctxt.eps_nas_sec_context_len>>8)|(log_native_sec_ctxt.eps_nas_sec_context_len<<8));
 ...
 /*Reverse the byte order of NAS DL count, it will be stored in reverse order in USIM*/
  ...
  {
    MSG_HIGH_DS_8(MM_SUB, "=EMM= UL NAS COUNT read from USIM: UL_NAS_COUNT[0] = %d, UL_NAS_COUNT[1] = %d, UL_NAS_COUNT[2] = %d, UL_NAS_COUNT[3] = %d \
                  DL NAS COUNT read from USIM: DL_NAS_COUNT[0] = %d, DL_NAS_COUNT[1] = %d, DL_NAS_COUNT[2] = %d, DL_NAS_COUNT[3] = %d",
                  usim_security_context_ptr->nas_ul_cnt[0], usim_security_context_ptr->nas_ul_cnt[1], 
                  usim_security_context_ptr->nas_ul_cnt[2], usim_security_context_ptr->nas_ul_cnt[3],
                  usim_security_context_ptr->nas_dl_cnt[0], usim_security_context_ptr->nas_dl_cnt[1],
                  usim_security_context_ptr->nas_dl_cnt[2], usim_security_context_ptr->nas_dl_cnt[3]);
  }
...
)
{
  ...
  secure_memset( (void *)&Ck, 0, sizeof(auth_gprs_cipherkey_type));
  secure_memset( (void *)&Ik, 0, sizeof(auth_gprs_integritykey_type));
  secure_memset( (void *)&Ck_srvcc, 0, sizeof(auth_gprs_cipherkey_type));
  secure_memset( (void *)&Ik_srvcc, 0, sizeof(auth_gprs_integritykey_type));
  secure_memset( (void *)&Kc, 0, sizeof(auth_gsm_cipherkey_type));
  secure_memset( (void *)&gsm_kc_128bit, 0, sizeof(auth_gsm_cipherkey_type));

  is_key_valid = emm_estimate_dl_count(rrc_nas_gw_key_ind,emm_ctrl_data_ptr);
  ...
  if(rrc_nas_gw_key_ind->msg_hdr.id == LTE_RRC_NAS_UMTS_KEY_IND)
  {
    ...
    emm_send_nas_umts_key_rsp(Ck, Ik, Ck_srvcc, Ik_srvcc,is_key_valid);
  }
  else if(rrc_nas_gw_key_ind->msg_hdr.id == LTE_RRC_NAS_GSM_KEY_IND)
  {
    ...
    emm_send_nas_gsm_key_rsp(Ck, Ik, Ck_srvcc, Ik_srvcc, Kc,gsm_kc_128bit,emm_srvcc_eKSI,is_key_valid);
  }
...
)
{
  ...
  secure_memset((void *)nas_sec_param_to_eutra, 0, sizeof(nas_sec_param_to_eutra));
  ...
  secure_memset((void *)context_info->nas_ul_cnt,0xFF, MAX_NAS_COUNT_IN_BYTES);
  emm_send_nas_lte_key_rsp(context_info->nas_kasme, context_info->nas_ul_cnt);
  emm_send_ssg_sec_ctxt_set_ind(EMM_SSG_SEC_CONTEXT_TYPE_MAPPED_EPS);
  /* Set the DL and UL NAS counts to 0 */
  secure_memset( (void *)context_info->nas_ul_cnt, 0, sizeof(context_info->nas_ul_cnt));
  secure_memset( (void *)context_info->nas_dl_cnt, 0, sizeof(context_info->nas_dl_cnt));
...
)
{
    ...
    (current_context_ptr->context_type == CONTEXT_TYPE_FULL_NATIVE_EPS))
 {
  ...
  if( emm_compute_nas_enc_key(emm_ctrl_data_ptr, mapped_context_ptr) == FALSE )
  {
   ...
   secure_memset(&emm_current_sec_context_log,0,sizeof(lte_nas_emm_current_sec_context_type));
  ...
  emm_ctrl_data_ptr->last_incoming_sqn = 0;
   ...
   emm_ctrl_data_ptr->est_nas_count = 0;
   emm_ctrl_data_ptr->dl_nas_count_bmp = 0xFF;
...
void emm_abort_ongoing_procedure(boolean cancel_auth)
{
  ...
  secure_memset((void *)&emm_failure_cause,0,sizeof(emm_failure_type));
   if((auth_transaction_id != NULL && auth_is_procedure_to_be_aborted(auth_transaction_id))
       || cancel_auth == TRUE)
   {
      MSG_HIGH_DS_0(MM_SUB,"=EMM= Processing SIM busy");
...
)
{
  ...
  secure_memset((void *)emm_ctrl_data_ptr->out_msg_ptr,
         0x0,
                            ...
                            emm_ctrl_data_type *emm_ctrl_data_ptr)
{
  ...
  emm_ssgccs_handle_outgoing_msg(&emm_ctrl_data_ptr->out_msg_ptr->emm_outgoing_msg);
...
)
{
  ...
  */
  ...
  if ((mm_sim_card_mode == MMGSDI_APP_USIM) && 
         ...
         ((emm_ctrl_data_ptr->autn_amf[0]  & 0x80) == 0x80))
  {
...
)
{
  ...
  */
  ...
  if (FALSE == dos_mitigation_reqd)
  {
     ...
     this reject is due to authentication reject */  
  ...
  {
	//Release cached PDN so that next time attach won't be started with cached
	//PDN because after sending detach IND ESM will cleanup ongoing proc
...
void emm_load_security_context(emm_ctrl_data_type *emm_ctrl_data_ptr)
{
  ...
  secure_memset( (void *)&saved_security_context, 0xFF, sizeof(saved_security_context));

  secure_memset( (void *)&usim_security_context_ptr, 0xFF, sizeof(usim_security_context_ptr));
  ...
  if( emm_ctrl_data_ptr->eps_storage_supported ) 
  {
   ...
   /* Read the USIM EPS NSC file */
    ...
    secure_memset(( void *)&log_native_sec_ctxt, 0, sizeof( log_native_sec_ctxt));
     ...
     */
     ...
     emm_ctrl_data_ptr->est_nas_count = (( dword) context_ptr->nas_dl_cnt[2] << 16  ) | ( ( dword)context_ptr->nas_dl_cnt[1] << 8 ) | ( dword) context_ptr->nas_dl_cnt[0];
    ...
    else
    {
      MSG_HIGH_DS_1(MM_SUB, "=EMM= usim_security_context_ptr.nasKSI.keyset_id = %d",
                    usim_security_context_ptr.nasKSI.keyset_id);
      /* Delete current security context in global memory if any */
      emm_invalidate_current_security_ctxt(emm_ctrl_data_ptr);
    }
  }
...
FUNCTION 
  emm_write_efs_security_context_to_efs

DESCRIPTION
  This function writes the security context to the EFS using the given emm_efs_security_context_info.

RETURN VALUE
  TRUE/FALSE - status of the write
...
boolean emm_write_efs_security_context_to_efs(emm_efs_security_context_info* efs_security_context_ptr)
{
  ...
  */
  ...
  ASSERT( efs_security_context_ptr != NULL );
  ...
  if( status != MCFG_FS_STATUS_OK )
  {
    MSG_ERROR_DS_2(MM_SUB, "=EMM= Write to EFS Native security context file failed, status - %d and mcfg_fs_errno = %d ",status,mcfg_fs_errno(MCFG_FS_TYPE_EFS));
    return FALSE;
  }
  return TRUE;
}
...
boolean emm_write_efs_security_context(emm_security_context_info* security_context_ptr)
{
  ...
  */
  ...
  secure_memset( (void *)&efs_security_context, 0xFF, sizeof(emm_efs_security_context_info));
...
void emm_invalidate_nas_security_params( emm_ctrl_data_type *emm_ctrl_data_ptr)
{
  ...
  secure_memset(context_ptr->nas_ul_cnt,0xFF, sizeof(context_ptr->nas_ul_cnt));

  secure_memset(context_ptr->nas_dl_cnt, 0xFF, sizeof(context_ptr->nas_dl_cnt));
  ...
  emm_ctrl_data_ptr->est_nas_count = 0xFFFFFFFF;
 ...
 emm_ssgccs_send_sec_ctxt_del_ind(EMM_SSG_SEC_CONTEXT_TYPE_FULL_NATIVE_EPS);
...
boolean emm_update_usim_eps_security_context( emm_security_context_info  *context_info_ptr)
{
  ...
  byte local_nas_ul_cnt[4] = {0};  // temporary NAS UL COUNT

  emm_ctrl_data_type  *emm_db_ptr;

  secure_memset(swapped_nas_count,0,sizeof(swapped_nas_count));
  ...
  secure_memset(write_buffer,0xFF, sizeof(write_buffer));
  ...
  */
   ...
   /* Copy the UL count in local variable, do not write incremented count to USIM */
   memscpy((void*)local_nas_ul_cnt, sizeof(local_nas_ul_cnt),
           (void*)context_info_ptr->nas_ul_cnt, sizeof(local_nas_ul_cnt));
   if( context_info_ptr->nas_ul_cnt[0] == 0xFF )
   {
     ...
     {
       overflow_cnt++;
       local_nas_ul_cnt[0] = 0;
       memscpy((void*)&local_nas_ul_cnt[1],
               sizeof(local_nas_ul_cnt) - 1,
               (void*)&overflow_cnt, sizeof(word));
     }   
   }
   else
   {
     local_nas_ul_cnt[0] = local_nas_ul_cnt[0] + 1;
   }
   ...
   /*UL NAS count is stored in EMM, with Index[0] = LSB, this must come at the very end (while writing to USIM),  
    ...
    emm_swap_array_of_bytes(&local_nas_ul_cnt[0],
                            sizeof(local_nas_ul_cnt),
                            &swapped_nas_count[0], sizeof(local_nas_ul_cnt));
  ...
  if(mmgsdi_is_test_mode_logging_enabled_for_subscription((sys_modem_as_id_e_type)mm_sub_id))
  {
    MSG_HIGH_DS_8(MM_SUB, "=EMM= UL COUNT to be written to USIM: UL_NAS_COUNT [0]= %d, UL_NAS_COUNT [1]= %d, UL_NAS_COUNT [2]= %d, UL_NAS_COUNT [3]= %d, \
                  DL COUNT to be written to USIM: DL_NAS_COUNT [0]= %d DL_NAS_COUNT [1]= %d, DL_NAS_COUNT [2]= %d, DL_NAS_COUNT [3]= %d",
                  swapped_nas_count[0], swapped_nas_count[1], swapped_nas_count[2], swapped_nas_count[3],
                  swapped_nas_count[4],swapped_nas_count[5], swapped_nas_count[6],swapped_nas_count[7]);
  }
                                   ...
                                   emm_ctrl_data_type                *emm_ctrl_data_ptr)
{
  ...
  boolean reset_nas_est_dl_count = FALSE;
...
if(memcmp( (void *)&emm_ctrl_data_ptr->ue_security_capability,
  ...
  { 
...
#endif
    {
      ...
      MSG_HIGH_DS_0(MM_SUB, "=EMM= copied the context info in SMC into allocated CONTEXT_TYPE_PARTIAL_NATIVE_EPS");
        ...
        emm_send_ssg_sec_ctxt_set_ind(EMM_SSG_SEC_CONTEXT_TYPE_FULL_NATIVE_EPS);
        MSG_HIGH_DS_0(MM_SUB, "=EMM= copied the context info in SMC into allocated CONTEXT_TYPE_PARTIAL_NATIVE_EPS");
      }
      else
      {
  ...
  */
  ...
  /* reset DL NAS COUNT bitmap */
  emm_ctrl_data_ptr->dl_nas_count_bmp = 0xFF;

  if(reset_nas_est_dl_count == TRUE)
  {
    emm_ctrl_data_ptr->est_nas_count = (( dword) context_info->nas_dl_cnt[2] << 16  ) | ( ( dword) context_info->nas_dl_cnt[1] << 8 ) | ( dword) context_info->nas_dl_cnt[0] ;
    /*Rset est_nas_count*/
    reset_nas_est_dl_count = FALSE;
    MSG_HIGH_DS_1(MM_SUB, " EMM: reset est dl cnt to nas dl cnt due to KASME used was changed reset_nas_est_dl_count = %d",reset_nas_est_dl_count);
                                     ...
                                     emm_ctrl_data_type *emm_ctrl_data_ptr)
{
  ...
  if(imeisv_req)
  {
    ...
    secure_memset(imeisv, 0, sizeof(imeisv));

    emm_populate_imeisv(imeisv);
...
,  emm_security_context_info *context_info
)
 {
   ...
   secure_memset(&curr_security_info, 0, sizeof(lte_nas_dl_security_info_type));
...
void emm_cancel_security_mode(emm_ctrl_data_type *emm_ctrl_data_ptr)
{
  ...
  if( context_ptr != NULL )
  {
    ...
    secure_memset((void *)context_ptr->nas_ul_cnt,0xFF, MAX_NAS_COUNT_IN_BYTES);

    secure_memset((void *)context_ptr->nas_dl_cnt, 0xFF, MAX_NAS_COUNT_IN_BYTES);
...
===========================================================================*/
  ...
  */
  ...
  secure_memset(input_str, 0, sizeof(input_str));

  secure_memset( output_str,0, sizeof(output_str));
 ...
 */
  ...
  secure_memset((void *)&input_str[0], 0, sizeof(input_str));

  secure_memset((void *)&output_str[0], 0, sizeof(output_str));
...
void emm_reset_security_info (emm_security_context_info *context_info)
{
  ...
  secure_memset((void *)&context_info->nas_ul_cnt[0],0xFF, sizeof(context_info->nas_ul_cnt));

  secure_memset((void *)&context_info->nas_dl_cnt[0], 0xFF, sizeof(context_info->nas_dl_cnt));
  ...
  if( context_ptr != NULL )
  {
...
#endif
    {
      ...
      */
      ...
      {
        secure_memset( (void *)nas_security->nas_count, 0, sizeof(nas_security->nas_count));
        reset_nas_ul_count = FALSE;
                                                        ...
                                                        byte *incoming_ota_message)
{
  ...
  boolean estimate_dl_count_from_sqn = FALSE;
  ...
  if( context_ptr != NULL )  
  {
...
#endif
    {
      ...
      */
      ...
      {
        estimate_dl_count_from_sqn = emm_estimate_dl_nas_count_from_sqn(sqn, nas_security, emm_ctrl_data_ptr);
        if(estimate_dl_count_from_sqn == FALSE)
        {
  ...
  if ( emm_state == EMM_DEREGISTERED )
  {
    ...
    for ( index = 0; index < EMM_MAX_CONTEXTS; index++ )
    {
      ...
      if( ( context_ptr->in_use) && ( context_ptr->context_type == CONTEXT_TYPE_MAPPED_EPS) )
      {
        ...
        emm_reset_security_info(context_ptr);
		emm_ssgccs_send_sec_ctxt_del_ind(EMM_SSG_SEC_CONTEXT_TYPE_MAPPED_EPS);
      }
...
)
{
  ...
  secure_memset((void *)&response, 0, sizeof( lte_nas_emm_ms_id_type)); 
...
)
{
  ...
  { 
    ...
    case EMM_REGISTERED_INITIATED:
      {
        MSG_HIGH_DS_0(MM_SUB, "=EMM= GUTI REALLOC CMD ignored - Attach in progress");
#ifdef TEST_FRAMEWORK
        #error code not present
#endif /* TEST_FRAMEWORK */
        return;
...

{
  ...
  if (emm_ctrl_data_ptr->emc_srv_status != LTE_NAS_EMC_SRV_ONLY)
  {
    ...
    secure_memset( (void *)&curr_security_context, 0, sizeof( curr_security_context));
    ...
    if ( temp_context_ptr != NULL )
  {
    ...
    { /* FULL native exists , proceed to writing to USIM */
     ...
     if( memcmp((void *)&emm_last_known_security_context, (void *)&curr_security_context, sizeof(emm_security_context_info)  ) == 0 ) 
     { 
     ...
     {   
       ...
       emm_last_known_security_context = curr_security_context;
                          ...
                          emm_ctrl_data_type   *emm_ctrl_data_ptr)
{
  ...
  secure_memset(&reg_info_ind,0,sizeof(mmr_mm_information_ind_s_type));
  ...
  if( emm_info->nw_full_name_exists )
  {
    ...
    secure_memset( (void *)info->full_name.name, 0, sizeof( info->full_name.name));
  ...
  if( emm_info->nw_partial_name_exists )
  {
    ...
    secure_memset( (void *)info->short_name.name, 0, sizeof( info->short_name.name));
  ...
  MSG_HIGH_DS_0(MM_SUB, "=EMM= Sent MMR_MM_INFORMATION_IND to REG");
...
)
{
  ...
  secure_memset((void*)&invalid_ck_ik[0], 0xFF, sizeof(invalid_ck_ik));
...
boolean emm_compute_ck_ik_from_kasme( emm_ctrl_data_type *emm_ctrl_data_ptr, 
  ...
  */
  ...
  if(use_dl_count == TRUE)
  {
    ...
    input_str[1]  = 0;     // always use 0 here
    input_str[2]  = (byte) ((emm_ctrl_data_ptr->est_nas_count & 0xFF0000)>> 16);  // overflow cnt
    input_str[3]  = (byte) ((emm_ctrl_data_ptr->est_nas_count & 0xFF00)>> 8);  // overflow cnt
    input_str[4]  = (byte) (emm_ctrl_data_ptr->est_nas_count & 0xFF);  // SQN
    // DL NAS COUNT ( 4 byte )
  }
  else
  {
...
void emm_get_eps_params( mm_gw_identity_param_type *mm_identity)
{
  ...
  if( emm_compute_ck_ik_from_kasme( emm_ctrl_data_ptr,&mm_identity->Ik, &mm_identity->Ck, FALSE ) == FALSE)
  {
...
void emm_get_eps_handover_auth_params( mm_gw_identity_param_type *mm_identity)
{
  ...
  /* P-TMSI signature */
  secure_memset((void *)&mm_identity->ptmsi_signature.value[1], 0xFF, sizeof(word));
                ...
                auth_autn_type autn_value)
{
    ...
    secure_memset((void *)&rand_value_cache.value_data[0], 0, MAX_AUTH_DATA_LENGTH);
    ...
    secure_memset((void *)&autn_cache.value_data[0], 0, MAX_AUTH_DATA_LENGTH);
...
FUNCTION    EMM_PROCESS_GUTI_PURGE_REQUEST

DESCRIPTION
   This function purge GUTI Security file , update EFS and 
call callback to indicate status to  UIM
...
void emm_process_guti_purge_ind(mm_cmd_type* msg)
{
  boolean status = FALSE;  
  emm_security_context_info *context_ptr = NULL;
  byte                    epsloci[EPSLOCI_LENGTH]={0};
  mcfg_fs_status_e_type   efs_status = MCFG_FS_STATUS_OK;
  boolean guti_purge_status = TRUE;
  boolean epsnsc_purge_status = TRUE;
  emm_ctrl_data_type* emm_ctrl_data_ptr = emm_db_get_ctrl_data();
  ASSERT(emm_ctrl_data_ptr != NULL); 
 // do global reset and write to EFS
 // 1 purge GUTI first

  secure_memset(&epsloci[0],0xFF,EPSLOCI_LENGTH);
  emm_ctrl_data_ptr->emm_guti_valid = FALSE ;
  secure_memset((void *)&emm_ctrl_data_ptr->emm_guti, 0xFF, sizeof(lte_nas_emm_guti_type)) ;
  if(emm_ctrl_data_ptr->plmn_service_state!=NULL)
  {
  ...
  secure_memset((void *)&emm_ctrl_data_ptr->last_registered_tai.plmn,0xFF,sizeof(sys_plmn_id_s_type)) ;
  emm_ctrl_data_ptr->emm_update_status  = EMM_STATUS_NOT_UPDATED ;

  epsloci[EMM_GUTI_LENGTH+EMM_PLMN_ID_LENGTH] = 0xFE;
  epsloci[EMM_GUTI_LENGTH+EMM_PLMN_ID_LENGTH+1] = 0xFF;
  epsloci[EMM_UPDATE_STATUS] = EMM_STATUS_NOT_UPDATED;
  ...
  if( context_ptr != NULL) 
  {
    ...
    mm_reset_ul_count_at_generation();
    emm_ctrl_data_ptr->dl_nas_count_bmp = 0xFF;
    epsnsc_purge_status = emm_write_efs_security_context(context_ptr);
  }

  MSG_HIGH_DS_2(MM_SUB,"=EMM= Guti efs status = %d Nas security EFS status %d ",guti_purge_status,epsnsc_purge_status);

  if ((guti_purge_status == TRUE) && (epsnsc_purge_status== TRUE) )
  {
    status = TRUE;
  }

// call mmgsdi call back
  msg->cmd.gsdi_guti_purge_ind.mmgsdi_cb(msg->cmd.gsdi_guti_purge_ind.user_data,status);
}
...
SIDE EFFECTS
  None
===========================================================================*/
void emm_invalidate_current_security_ctxt(emm_ctrl_data_type *emm_ctrl_data_ptr)
{
  emm_security_context_info *context_ptr = NULL;
...
DESCRIPTION
  ...
  Bit 1 is mapped to the status of current DL NAS COUNT and bit 8 is mapped to
...
SIDE EFFECTS
  None
===========================================================================*/
void emm_update_dl_nas_count_bitmap
(
...
{
  if(emm_ctrl_data_ptr->estimate_dl_count_from_sqn == TRUE)
  {
    /* new DL NAS COUNT is greater than the old DL NAS COUNT */
    if(dl_nas_count_old < dl_nas_count_new)
    {
      /* left shift the DL NAS COUNT bitmap (new - old) times and set the bit 0 to 1 explicitly */
      emm_ctrl_data_ptr->dl_nas_count_bmp = (emm_ctrl_data_ptr->dl_nas_count_bmp << (dl_nas_count_new - dl_nas_count_old)) | 0x1;
    }
    /* new DL NAS COUNT is smaller than the old DL NAS COUNT and the difference is not more than 7 */
    else if((dl_nas_count_old != dl_nas_count_new) &&
            ((dl_nas_count_old - dl_nas_count_new) <= 7))
    {
      emm_ctrl_data_ptr->dl_nas_count_bmp = emm_ctrl_data_ptr->dl_nas_count_bmp | (byte)(1 << (dl_nas_count_old - dl_nas_count_new));
    }
  }
} /* end emm_update_dl_nas_count_bitmap() */
...
RETURN VALUE
  TRUE if estimated DL NAS COUNT was already used else FALSE

SIDE EFFECTS
  None
===========================================================================*/
boolean emm_check_replayed_dl_nas_count
(
  dword               stored_count,
...
{
  if(estimated_count == stored_count)
  {
    return TRUE;
  }
  else if((estimated_count < stored_count) &&
          ((stored_count - estimated_count) <= 7))
  {
    /* check the bit correponding to the estimated count in emm_ctrl_data_ptr->dl_nas_count_bmp */
    if((emm_ctrl_data_ptr->dl_nas_count_bmp & (1 << (stored_count - estimated_count))) > 0)
    {
      return TRUE;
    }
  }
  return FALSE;
} /* end emm_check_replayed_dl_nas_count() */
...
SIDE EFFECTS
  None
===========================================================================*/
boolean emm_estimate_dl_nas_count_from_sqn
(
  byte                          sqn,
  lte_nas_dl_security_info_type *nas_security,
  emm_ctrl_data_type            *emm_ctrl_data_ptr
)
{
  ...
  MSG_HIGH_DS_1(MM_SUB, "=EMM= estimate_dl_count_from_sqn = %d",
  ...
  if(emm_ctrl_data_ptr->estimate_dl_count_from_sqn == TRUE)
  {
    ...
    if(mmgsdi_is_test_mode_logging_enabled_for_subscription((sys_modem_as_id_e_type)mm_sub_id))
    {
      MSG_HIGH_DS_4(MM_SUB, "=EMM= [Current UE DL NAS Count] nas_dl_cnt[0] = %d, nas_dl_cnt[1] = %d, nas_dl_cnt[2] = %d, nas_dl_cnt[3] = %d",
    ...
    MSG_HIGH_DS_4(MM_SUB, "=EMM= y1 = %d, y2 = %d, y_hash = %d, z = %d ", y1, y2, y_hash, z);
    ...
    if(emm_check_replayed_dl_nas_count(nas_count, out_nas_count, emm_ctrl_data_ptr) == TRUE)
    {
      MSG_HIGH_DS_1(MM_SUB, "=EMM= DL NAS Count %d already used", out_nas_count);
      return FALSE;
    }
    else
    {
      nas_security->nas_count[0] = out_count[0];
      nas_security->nas_count[1] = out_count[1];
      nas_security->nas_count[2] = out_count[2];
      nas_security->nas_count[3] = out_count[3];
    }
    ...
    if(mmgsdi_is_test_mode_logging_enabled_for_subscription((sys_modem_as_id_e_type)mm_sub_id))
    {
      MSG_HIGH_DS_6(MM_SUB, "=EMM= [estimated_dl_count] out_count[0] = %d, out_count[1] = %d, out_count[2], out_count[3] = %d, over_flow_cnt = %d, under_flow_cnt = %d", 
  ...
  return FALSE;
} /* end emm_estimate_dl_nas_count_from_sqn() */

#ifdef FEATURE_QSH_MDUMP

QSH_MDUMP_FN_ATTR void emm_log_mdump_static_vars() 
{
#if defined FEATURE_DUAL_SIM
...
#endif
  {
  qsh_mdump_collect_high(&emm_ck_key_value,sizeof(emm_ck_key_value));  
  qsh_mdump_collect_high(&emm_ik_key_value,sizeof(emm_ik_key_value));  
  qsh_mdump_collect_high(&emm_srvcc_ck_key_value,sizeof(emm_srvcc_ck_key_value)); 
  qsh_mdump_collect_high(&emm_srvcc_eKSI,sizeof(emm_srvcc_eKSI)); 
  qsh_mdump_collect_high(&emm_srvcc_ik_key_value,sizeof(emm_srvcc_ik_key_value)); 
  }
#if defined FEATURE_DUAL_SIM
  mm_as_id = temp_as_id;
#endif
