...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/mm/src/gmmllcm.c#1 $ $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...
static boolean glif_is_start_non_drx_ind_sent_sim[MAX_NAS_STACKS] = { FALSE, FALSE } ;
#define glif_is_start_non_drx_ind_sent glif_is_start_non_drx_ind_sent_sim[mm_as_id]
...
static boolean glif_is_start_non_drx_ind_sent = FALSE ;
...
void glif_enter_mm_non_drx_mode( void )
{
  ...
  {
    gmm_start_mm_non_drx_ind.non_updated_lai_in_nmo1 = TRUE;
  }
  else
  {
    gmm_start_mm_non_drx_ind.non_updated_lai_in_nmo1 = FALSE;
  }

  glif_is_start_non_drx_ind_sent = TRUE;

  MSG_HIGH_DS_1( MM_SUB, "=MM= GLIF: Sent Start MM Non-DRX to GRR Non-Updated LAI = %d",gmm_start_mm_non_drx_ind.non_updated_lai_in_nmo1 );
...
void glif_exit_mm_non_drx_mode( void )
{
  ...
  glif_is_start_non_drx_ind_sent = FALSE;
...
#endif
    {
      if((gmm_detach_after_rau_pending || gmm_state == GMM_DEREGISTERED_INITIATED) 
          && mm_serving_plmn.ms_op_mode == SYS_SRV_DOMAIN_CS_ONLY)
      {
        rr_gmm_tlli_assignment.service_domain = SYS_SRV_DOMAIN_CS_PS;
      }
...
)
{
  ...
  if(mm_checkif_rat_pri_list_has_rat(&mm_rat_pri_list_info, SYS_RAT_LTE_RADIO_ACCESS))
  {
    ...
    else if((emm_ctrl_data_ptr->TIN == NAS_MM_TIN_NONE) && mm_is_ptmsi_valid())
    {
      ptmsi = gmm_stored_ptmsi;
      ptmsi_present = TRUE;   
    }
    else if((emm_ctrl_data_ptr->TIN == NAS_MM_TIN_NONE) && (emm_ctrl_data_ptr->emm_guti_valid))
    {
      emm_map_guti_to_ptmsi(&ptmsi, emm_ctrl_data_ptr);
      ptmsi_present = TRUE; 
    }
...
)
{
  ...
  if (glif_sending_gmm_message == FALSE)
  {
    ...
    else
    {
      ...
      {
        if (mm_is_cs_connected_due_to_srvcc == TRUE && 
            gmm_ra_update_pending == TRUE && 
            mm_lu_pended_nmo_unknown == TRUE)
        {
          MSG_HIGH_DS_0(MM_SUB, "=MM= Don't pend cell update during SRVCC while RAU is pending") ;
        }
        else
        {
  ...
  { 
    if (primitive->cmd.llgmm_status_ind.cause == GLLCMM_L2_TX_FAILURE)
    {
...
#ifdef FEATURE_DUAL_SIM 
          mm_gmm_set_illegal_non_illegal_cause(gmm_reject_cause);
#endif
      ...
      else if(gmm_last_signalling_message_type == GMM_DETACH_REQUEST)
      {
         ...
         else if(mm_timer_status[TIMER_T3321] == TIMER_ACTIVE)
         {
           MSG_HIGH_DS_0(MM_SUB, "=MM= Retrying for Detach within T3321 window in DDS switch case");
           gmm_initiate_gprs_detach( &mmcoord_pending_reg_message );           
         }
      }
      else
...
/* 24.301 5.5.3.3.2 d) */        
      ...
      &&(emm_ctrl_data_ptr->sg_association_to_be_established == NAS_EMM_SG_ASSOC_NOT_REQ))
      {
        ...
        MSG_HIGH_DS_0(MM_SUB,"=MM= Set SGs association flag - Combined RAU, CCO completed");
      }
#endif
  ...
  switch (primitive->cmd.hdr.message_id)
  {
    ...
    */
    ...
    case GLLCMM_LLGMM_NEW_TLLI_IND:
      ...
      **--------------------------------------- --------------------------------------- --------------------------------------- ---------------------------------------*/
      if(primitive->cmd.hdr.message_id == GLLCMM_LLGMM_NEW_TLLI_IND)
      {
        gmm_reset_old_ptmsi();
...
boolean gmmllcm_is_start_non_drx_ind_sent(void)
{
  return glif_is_start_non_drx_ind_sent;
}

#ifdef FEATURE_QSH_MDUMP
QSH_MDUMP_FN_ATTR void gmm_log_mdump_static_vars() 
{
...
#endif
{
  qsh_mdump_collect_high(&glif_pended_rau_req,sizeof(glif_pended_rau_req));
  qsh_mdump_collect_high(&glif_rau_in_progress,sizeof(glif_rau_in_progress));
  qsh_mdump_collect_high(&glif_state,sizeof(glif_state));
}
#if defined FEATURE_DUAL_SIM
mm_as_id = temp_as_id;
#endif
