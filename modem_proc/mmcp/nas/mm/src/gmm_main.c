...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/mm/src/gmm_main.c#1 $ $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...
gmm_ptmsi_type           gmm_old_ptmsi_sim[MAX_NAS_STACKS];
...
rrc_establish_cause_e_type    gmm_rrc_establish_cause_sim[MAX_NAS_STACKS]; 
#endif
...
boolean gmm_rau_upon_entry_to_a_new_cell_sim[MAX_NAS_STACKS];
...
byte    gmm_buffer_rej_cause_sim[MAX_NAS_STACKS][MAX_ATTEMPT_COUNTER_ATTACH];
...
#ifdef FEATURE_NAS_REL12_SPEC_CHANGE
boolean gmm_sync_pmm_state_during_w_to_lg_and_back_to_w_irat_sim[MAX_NAS_STACKS];
#endif
...
boolean gmm_rau_is_required_due_to_dsr_sim[MAX_NAS_STACKS];
...
boolean   gmm_pending_imsi_attach_sim[MAX_NAS_STACKS];
...
boolean gmm_need_to_perform_ecall_inactivity_sim[MAX_AS_IDS];
#define gmm_need_to_perform_ecall_inactivity gmm_need_to_perform_ecall_inactivity_sim[mm_as_id]
#endif
...
#ifdef FEATURE_NAS_REL12_SPEC_CHANGE
byte    gprs_service_attempt_counter_sim[MAX_NAS_STACKS];
#define gprs_service_attempt_counter gprs_service_attempt_counter_sim[mm_as_id]
#endif
...
boolean gmm_abort_ps_detach_sim[MAX_NAS_STACKS];
#define gmm_abort_ps_detach gmm_abort_ps_detach_sim[mm_as_id]

extern boolean gmm_force_local_detach_sim[];
#ifdef FEATURE_DUAL_SIM
boolean gmm_ps_call_in_progess_sim[MAX_NAS_STACKS];
#define gmm_ps_call_in_progess gmm_ps_call_in_progess_sim[mm_as_id]
#endif

sys_radio_access_tech_e_type gmm_drx_coeff_client_rat_sim[MAX_AS_IDS];
#define gmm_drx_coeff_client_rat gmm_drx_coeff_client_rat_sim[mm_sub_id]
...
#ifdef FEATURE_NAS_REL12_SPEC_CHANGE
rrc_establish_cause_e_type    gmm_rrc_establish_cause;
byte                          gprs_service_attempt_counter;
#endif
...
boolean gmm_rau_upon_entry_to_a_new_cell;
...
byte gmm_buffer_rej_cause[MAX_ATTEMPT_COUNTER_ATTACH];
...
#ifdef FEATURE_NAS_REL12_SPEC_CHANGE
boolean gmm_sync_pmm_state_during_w_to_lg_and_back_to_w_irat;
#endif
...
boolean gmm_rau_is_required_due_to_dsr;
#ifdef FEATURE_GSM_GPRS
...
#ifdef FEATURE_NAS_ECALL 
boolean gmm_need_to_perform_ecall_inactivity;
#endif
...
boolean   gmm_pending_imsi_attach;

boolean gmm_abort_ps_detach;
sys_radio_access_tech_e_type gmm_drx_coeff_client_rat;
...
#ifdef FEATURE_LTE
#ifdef FEATURE_DUAL_SIM
nas_drx_set_cfg_req_enum_T gmm_set_drx_req_pending_sim[MAX_AS_IDS];
...
#else
nas_drx_set_cfg_req_enum_T gmm_set_drx_req_pending;
...
mm_client_req_drx_s_type gmm_client_req_drx_coeff_sim[MAX_AS_IDS];
#define gmm_client_req_drx_coeff gmm_client_req_drx_coeff_sim[mm_sub_id]
...
mm_client_req_drx_s_type gmm_client_req_drx_coeff;
...
#ifdef FEATURE_LTE
void gmm_handle_non_acq_param_change_ind(mm_cmd_type *message);
#endif

#ifdef FEATURE_LTE
/*===========================================================================

FUNCTION gmm_is_cap_changed
...
boolean gmm_is_cap_changed( void )
{
  ...
  if((gmm_is_lte_advertised_in_last_reg != is_lte_present_in_rat_list) ||
     (emm_get_radio_cap_update_state() == EMM_RADIO_CAP_UPDATE_PENDING))
  {
    return TRUE;
  }
  return FALSE;
  ...
  ** --------------------------------------- */
  switch (new_state)
  {
    ...
    case GMM_ROUTING_AREA_UPDATING_INITIATED:
      ...
      if(emm_get_radio_cap_update_state() == EMM_RADIO_CAP_UPDATE_PENDING)
      {
        emm_set_radio_cap_update_state(EMM_RADIO_CAP_UPDATE_PENDING_FOR_LTE);
      }
#endif
      break;
    ...
    case GMM_DEREGISTERED:
     gmm_last_signalling_sent_rat = SYS_RAT_NONE;
      ...
      if ( is_umts_mode() || is_gsm_mode() )
      {
        ...
        else if (gmm_set_drx_req_pending == NAS_DRX_CFG_REQ_PROCESSING)
        {
          gmm_set_drx_req_pending = NAS_DRX_CFG_REQ_PENDING;
        }
...
/*Reset the flag when GMM is in De-registered state*/
  ...
  {
#ifdef FEATURE_DUAL_SIM
    mm_proc_status_set(SYS_PROC_TYPE_PS_CALL_SIGNALING, FALSE);
    if((new_state == GMM_SERVICE_REQUEST_INITIATED) || gmm_ps_call_in_progess)
    {
...
** ---------------------------- ---------------------------- ---------------------------- ----------------------------*/
      ...
      case GMM_DEREGISTERED :
        ...
        */
        ...
        switch (previous_state)
        {
          ...
          case GMM_ROUTING_AREA_UPDATING_INITIATED:
          case GMM_DEREGISTERED_INITIATED:
           if (is_gsm_mode() && gmmllcm_is_start_non_drx_ind_sent())
            ...
            {
               gmm_need_to_perform_ecall_inactivity = FALSE;
...
#ifdef FEATURE_DUAL_SIM
  else if((previous_state == GMM_SERVICE_REQUEST_INITIATED) &&
          (new_state != GMM_SERVICE_REQUEST_INITIATED)&&
          (gmm_ps_call_in_progess == FALSE))
  {
    mm_proc_status_set(SYS_PROC_TYPE_PS_CALL_SIGNALING, FALSE);
    mm_send_procedure_ind(mm_evaluate_proc_type());
  }
  ...
  if (previous_state == GMM_DEREGISTERED_INITIATED)
  {
       ...
       (mm_serving_plmn.info.active_rat != SYS_RAT_LTE_RADIO_ACCESS))
   {
     ...
     {
       if (mmcoord_reg_req_pending_sim[mm_other_as_id] == FALSE || mm_serving_plmn_sim[mm_other_as_id].info.active_rat == SYS_RAT_LTE_RADIO_ACCESS)
       {
...
void gmm_set_substate( gmm_substate_type new_substate )
{
  ...
  ** --------------------------------------------- */
  if ((old_substate == GMM_ATTEMPTING_TO_ATTACH) && (old_substate != new_substate))
  {
    gprs_attach_upon_entry_to_a_new_cell = FALSE;
  }
  if ((old_substate == GMM_ATTEMPTING_TO_UPDATE) && (old_substate != new_substate))
  {
...
/*4.7.5.1.5 (j)*/
        ...
        {
          MSG_HIGH_DS_0( MM_SUB, "=MM= Attach required for ENS or Mode change purposes-forced to do Attach here or Dynamic DRX");
          gmm_initiate_gprs_attach();
        }
#endif
      }
...
#ifdef FEATURE_NAS_REL12_SPEC_CHANGE
        MSG_HIGH_DS_1( MM_SUB, "=MM= gmm_sync_pmm_state_during_w_to_lg_and_back_to_w_irat = %d", gmm_sync_pmm_state_during_w_to_lg_and_back_to_w_irat);
#endif
...
#endif
#ifdef FEATURE_LTE
            else if (((gmm_set_drx_req_pending == NAS_DRX_CFG_REQ_PENDING) ||
...
/*4.7.1.7a  Intersystem change from S1 mode to A/Gb mode or S1 mode to Iu mode with ISR activated*/
                   ...
                   (is_umts_mode() && (gmm_sync_pmm_state_during_w_to_lg_and_back_to_w_irat == TRUE)) ||
#endif
                   ((gmm_processing_directed_signalling_reestablishment || gmm_rau_is_required_due_to_dsr) &&
                    (mm_timer_status[TIMER_T3311] == TIMER_STOPPED ||
                     mm_timer_status[TIMER_T3302] == TIMER_STOPPED)))
          {
            if ((mm_serving_plmn.ms_op_mode == SYS_SRV_DOMAIN_CS_PS) &&
            ...
            if(gmm_rau_is_required_due_to_dsr)
            {
...
#ifdef FEATURE_NAS_REL12_SPEC_CHANGE
  gmm_rrc_establish_cause                            = RRC_EST_TERM_UNKNOWN;
#endif  
...
/* TSTS Variables Initialization  - Start */
 ...
 gmm_rau_is_required_due_to_dsr = FALSE;
  ...
  gmm_is_lte_advertised_in_last_reg = FALSE;
  gmm_drx_coeff_client_rat = SYS_RAT_NONE;
  
  gmm_client_req_drx_coeff.default_drx = SYS_DRX_CN_COEFFICIENT_S1_MAX;
  gmm_client_req_drx_coeff.ds_requested_drx = SYS_DRX_CN_COEFFICIENT_S1_MAX;
  gmm_client_req_drx_coeff.qmi_requested_drx = SYS_DRX_CN_COEFFICIENT_S1_MAX;
  gmm_client_req_drx_coeff.mcs_requested_drx = SYS_DRX_CN_COEFFICIENT_S1_MAX;
  gmm_client_req_drx_coeff.client_id = 0x00;
...
void gmm_load_psloci_data( void )
{
  ...
  ** ----------------------------------------------------------- */
  ...
  if(MM_SUB_IS_SGLTE)
  {
    ...
    HYBRID_STACK_DATA_MEMS_COPY(&gmm_old_ptmsi,PTMSI_SIZE,&gmm_old_ptmsi,PTMSI_SIZE)
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  ** -------------------- */
  ...
  if((gmm_ps_fop_resp != PS_FOP_NW_GRANTED) && (mm_timer_status[TIMER_T3340] == TIMER_ACTIVE))
  {
     gmm_ps_fop_resp = PS_FOP_NW_GRANTED;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  ** -------------------- */
  ...
  tc_cmd->cmd.gmm_unitdata_ind.as_id = (sys_modem_as_id_e_type)mm_sub_id;
#endif
    ...
    case MS_MM_RR:
      switch (message->cmd.hdr.message_id)
      {
        ...
        case RR_GMM_RETX_PDU_TUNEAWAY_FAIL_IND:
          if (is_gsm_mode())
          {
            gmm_handle_retx_pdu_tuneaway_fail_ind();
          }
          break;
          ...
          ** -------------------------------------------------------------------- */
          ...
          if (protocol_discriminator ==  PD_GMM)
          {
            ...
            ** ------------------------------------------------------------ */
              ...
              (message->cmd.mm_data_ind.message_header.message_len_lsb >= 2))
            {
              ...
              ** --------------------------------- */
			  if(message->cmd.mm_data_ind.L3_data[1]!=GMM_AUTHENTICATION_AND_CIPHERING_REQUEST)
			  {
               ...
               /*-----------------------------------------------------------------------------------------------
                ...
                case GMM_STATUS:
                  ...
                  ** -------------------------------------------------------------- */
                  ...
                  gmm_qsh_ota_metric_signaling_log(GMM_STATUS,CN_TO_UE_DIRECTION,
                                                   0xff, MM_GMM_NO_FAILURE_DEBUG);
#endif
                  break;
          ...
          else if (protocol_discriminator == PD_TEST)
          {
            if (message->cmd.mm_data_ind.message_header.message_len_lsb >= 2)
            {
              ...
              ** the first loopback signalling message on DL.
              ** --------------------------------------------------------------------- */
              if ( pmm_mode != PMM_CONNECTED && is_tdscdma_mode())
              {
                pmm_mode = PMM_CONNECTED;
                MSG_HIGH_DS_0( MM_SUB, "=MM= pmm_mode = PMM_CONNECTED");
    ...
    case MS_MM_MM:
       switch(message->cmd.hdr.message_id)
       {
         ...
         case MM_NON_ACQ_PARAM_CHANGE_IND:
           gmm_handle_non_acq_param_change_ind(message);
           break;
#endif
         default:
...
FUNCTION gmm_handle_non_acq_param_change_ind

DESCRIPTION
   Function to handle non acq param change ind
...
{
  emm_ctrl_data_type *emm_ctrl_data_ptr = NULL;
  sys_voice_domain_pref_e_type              prev_voice_domain_pref = gmm_reg_pended_lte_cs_param.voice_domain_pref;

  emm_ctrl_data_ptr = emm_db_get_ctrl_data();
  
  ASSERT(emm_ctrl_data_ptr != NULL);

  MSG_HIGH_DS_2(MM_SUB, "=MM= Previous updated voice domain pref %d New voice domain pref %d",
                  gmm_reg_lte_cs_param.voice_domain_pref,
                  message->cmd.non_acq_param_change_ind.voice_domain_pref);
  ...
  gmm_reg_pended_lte_cs_param.voice_domain_pref = message->cmd.non_acq_param_change_ind.voice_domain_pref;
  ...
  {
    mmcoord_pending_reg_message.cmd.mmr_reg_req.lte_cs_domain_param.voice_domain_pref = message->cmd.non_acq_param_change_ind.voice_domain_pref;
  } 
  else if((emm_ctrl_data_ptr->reg_req_pending_mm_cmd_ptr != NULL) && (emm_ctrl_data_ptr->reg_req_pended_indication == TRUE))
  {
	emm_ctrl_data_ptr->reg_req_pending_mm_cmd_ptr->cmd.mmr_reg_req.lte_cs_domain_param.voice_domain_pref = message->cmd.non_acq_param_change_ind.voice_domain_pref;
  } 
  if((gmm_reg_lte_cs_param.voice_domain_pref != message->cmd.non_acq_param_change_ind.voice_domain_pref) &&
    ...
    /* GMM is in registered state */
      if ((gmm_state == GMM_REGISTERED) &&
        ((gmm_substate == GMM_NORMAL_SERVICE) ||
         ...
         (gmm_substate == GMM_ATTEMPTING_TO_UPDATE_MM)) &&
        (mmcoord_reg_req_pending == FALSE) && (mm_checkif_rat_pri_list_has_rat(&mm_rat_pri_list_info,
                                                                   SYS_RAT_LTE_RADIO_ACCESS) == TRUE ))
      {
       ...
       {
        MSG_HIGH_DS_1(MM_SUB,"=MM= RAU Pended - GMM state = %d",gmm_state);
       }
    }
  }
  else if(prev_voice_domain_pref != message->cmd.non_acq_param_change_ind.voice_domain_pref &&
  	     (gmm_state == GMM_REGISTERED_INITIATED || gmm_state == GMM_ROUTING_AREA_UPDATING_INITIATED))
  {
    MSG_HIGH_DS_3(MM_SUB, "=MM= RAU delayed - GMM state %d, voice domain pended %d ongoing voice domain %d",
                    gmm_state,
                    emm_ctrl_data_ptr->emm_voice_domain_change_pended,
                    mmcoord_pending_reg_message.cmd.mmr_reg_req.lte_cs_domain_param.voice_domain_pref);
    if((mmcoord_pending_reg_message.cmd.mmr_reg_req.lte_cs_domain_param.voice_domain_pref == message->cmd.non_acq_param_change_ind.voice_domain_pref) &&
		emm_ctrl_data_ptr->emm_voice_domain_change_pended == NAS_NON_ACQ_REQ_PENDING)
    {
  ...
  /*If no signalling takes place in GW on current sub yet,gmm_reg_lte_cs_param will not be updated */
  else if(MM_IS_NON_DDS_SUB && gmm_reg_lte_cs_param.is_voice_domain_ie_valid == FALSE) 
  {
     emm_ctrl_data_ptr->emm_voice_domain_change_pended = NAS_NON_ACQ_REQ_PENDING;
     MSG_HIGH_DS_0(MM_SUB,"=MM= Voice domain pended on nDDS");
  }
}
