...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/mm/src/mmsim.c#1 $    $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...
mm_debug_sim_swap_status_e_type mm_debug_sim_swap_status_sim[MAX_NAS_STACKS] = {MM_DEBUG_UNDEFINED, MM_DEBUG_UNDEFINED, MM_DEBUG_UNDEFINED};
#define mm_debug_sim_swap_status mm_debug_sim_swap_status_sim[mm_as_id]
ghdi_status_T mm_debug_nv_imsi_read_error_sim[MAX_NAS_STACKS];
#define mm_debug_nv_imsi_read_error mm_debug_nv_imsi_read_error_sim[mm_as_id]
boolean mm_sim_swapped_sim[MAX_NAS_STACKS] = {FALSE, FALSE, FALSE};
#define mm_sim_swapped mm_sim_swapped_sim[mm_as_id]

#elif defined(FEATURE_DUAL_SIM) || defined(FEATURE_SGLTE)
mm_debug_sim_swap_status_e_type mm_debug_sim_swap_status_sim[MAX_NAS_STACKS] = {MM_DEBUG_UNDEFINED, MM_DEBUG_UNDEFINED};
#define mm_debug_sim_swap_status mm_debug_sim_swap_status_sim[mm_as_id]
ghdi_status_T mm_debug_nv_imsi_read_error_sim[MAX_NAS_STACKS];
#define mm_debug_nv_imsi_read_error mm_debug_nv_imsi_read_error_sim[mm_as_id]
boolean mm_sim_swapped_sim[MAX_NAS_STACKS] = {FALSE, FALSE};
#define mm_sim_swapped mm_sim_swapped_sim[mm_as_id]

#else
mm_debug_sim_swap_status_e_type mm_debug_sim_swap_status = MM_DEBUG_UNDEFINED;
ghdi_status_T mm_debug_nv_imsi_read_error;
#endif
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  ** ------------------------------------------------------------- */
  ...
  {
    if(mm_sub_is_rat_enabled(SYS_RAT_TDS_RADIO_ACCESS))
    {
#ifdef FEATURE_WCDMA
     ...
     )
   {
      ...
      if (has_valid_tmsi())
      {
...
#endif
        {
		  ...
		  MSG_HIGH_DS_1(MM_SUB,"=MM= stat signla sent for registartion status, mm ecall substate %d", mm_idle_substate);
        }
      }
#ifdef FEATURE_NAS_ECALL	  
	  else if(update_status == UPDATING_DISABLED && mm_idle_substate == MM_ECALL_INACTIVE)
	  {
#ifdef FEATURE_DUAL_SIM
        if(MM_MODEM_STATISTICS_SUB1)
#endif
...
/*===========================================================================
  ...
  FUNCTION 
	mm_check_sim_swap_event
  
  DESCRIPTION
	Function that checks whether any SIM swap (due to flex mapping change OR hard swap) is triggered
  
  RETURN VALUE
	sim_swapped - boolean to indicate sim is swapped or not.
  
===========================================================================*/
boolean mm_check_sim_swap_event
(
  imsi_data_T imsi_data,
  byte* imsi_from_nv,
  mm_as_id_e_type* mm_as_id_target_p
)
{
  boolean sim_swapped = FALSE;
  mm_as_id_e_type mm_as_id_target = MM_AS_ID_1;
  mm_as_id_e_type mm_sub_id_current = mm_sub_id;
  mm_as_id_e_type mm_as_id_current = mm_as_id;
  byte target_sim_imsi_from_nv[IMSI_LEN_IN_BYTES]={0};
  boolean nv_status;

  MSG_HIGH_DS_2(MM_SUB, "=MM= mm_sim_max_subscription: %d, MAX_AS_IDS: %d", mm_sim_max_subscription, MAX_AS_IDS);  
  for(mm_as_id_target = MM_AS_ID_1; (mm_as_id_target < mm_sim_max_subscription) && (mm_as_id_target < MAX_AS_IDS); mm_as_id_target++)
  {
    /* Change context */
    mm_set_as_id(mm_as_id_target);
    MSG_HIGH_DS_2(MM_SUB, "=MM= check sim swap event with mm_as_id: %d, mm_sub_id: %d", mm_as_id, mm_sub_id);

    if(mm_sub_id != mm_sub_id_current)
    {
      /* If target subscription is already available no need to consider it as a candidate for swap */
      if((mm_sim_card_mode == MMGSDI_APP_SIM) || (mm_sim_card_mode == MMGSDI_APP_USIM))
      {
        MSG_HIGH_DS_0(MM_SUB, "=MM= Subscription already became available. No need to consider it for swap");
        mm_debug_sim_swap_status = MM_DEBUG_SIM_CHANGED_NOT_SWAPPED_SUB_AVAILABLE;
        continue;
      }

      /* Read NV IMSI */
      NAS_ENTER_CRIT_SECT(mm_sim_swap_crit_sec);
      nv_status = mm_nv_read_imsi(target_sim_imsi_from_nv);
      NAS_EXIT_CRIT_SECT(mm_sim_swap_crit_sec);
      
	  MSG_HIGH_DS_8(MM_SUB, "=MM= TARGET NV IMSI[0] = %d  IMSI[1] = %d  IMSI[2] = %d IMSI[3] = %d \
	                          IMSI[4] = %d IMSI[5] = %d IMSI[6] = %d IMSI[7] = %d",target_sim_imsi_from_nv[0],
							  target_sim_imsi_from_nv[1],target_sim_imsi_from_nv[2],target_sim_imsi_from_nv[3],
							  target_sim_imsi_from_nv[4],target_sim_imsi_from_nv[5],target_sim_imsi_from_nv[6],
							  target_sim_imsi_from_nv[7]);  

      if (nv_status)
      {
        /* Compare current sub SIM IMSI with target sub NV IMSI. They will be same In case of SIM soft swap */
        if(memcmp(imsi_data.data, target_sim_imsi_from_nv, imsi_data.length) == 0)
        {
          sim_swapped = TRUE;
          *mm_as_id_target_p = mm_as_id;
          MSG_HIGH_DS_2(MM_SUB, "=MM= IMSI matched for swap sim:%d with %d", mm_as_id_current, mm_as_id);
          break;
        }
        else
        {
          MSG_HIGH_DS_0(MM_SUB, "=MM= IMSI is not matched for swap ");
          mm_debug_sim_swap_status = MM_DEBUG_SIM_CHANGED_NOT_SWAPPED_IMSI_NO_MATCH;
        }		
      }
      else
      {
        MSG_HIGH_DS_0(MM_SUB, "=MM= NV IMSI read failed");
        mm_debug_sim_swap_status = MM_DEBUG_SIM_CHANGED_NOT_SWAPPED_NV_IMSI_READ_FAILED;
      }
    }
  }
  ...
  if(sim_swapped)
  {
    MSG_HIGH_DS_2(MM_SUB, "=MM= SIM swap triggered for AS IDs: %d - %d ", mm_as_id_current, *mm_as_id_target_p);
  }
...
DESCRIPTION
  Writes updated data (due to SIM soft swap) into NV.
  Called when SIM soft swap is triggered to swap NV data of the SUBs.
	
RETURN VALUE
  None

===========================================================================*/
void mm_write_nv_after_sim_swap
(
  void
)
{
  byte security_buffer[9] = {0};
  ghdi_status_T nv_status;
  ...
  if(auth_gsm_gprs_keys_present == FALSE)
  {
    /* Write GSM KCGPRS info to NV */
    if (auth_gsm_gprs_cipherkey.value_length == GSM_SECURITY_KEY_LENGTH)
    {
      (void)memscpy((void *)security_buffer,9,(void *)&auth_gsm_gprs_cipherkey.value_data,GSM_SECURITY_KEY_LENGTH);
      security_buffer[8] = auth_gsm_gprs_cksn;
      if ((nv_status = mm_ghdi_nvmem_write(
    ...
    {
      MSG_ERROR_DS(MM_SUB, "=MM= Invalid GSM ciphering key length %d",auth_gsm_gprs_cipherkey.value_length,0,0);
    }
  }
}
...
DESCRIPTION
  Writes PSLOCI data (due to SIM soft swap) into NV.
  Called when SIM soft swap is triggered to swap NV data of the SUBs.
...
)
{
  ...
  /* Write GSM GPRS LOCI to NV */
  (void)memscpy(psloci, PSLOCI_LEN, gmm_stored_ptmsi.digit, PTMSI_SIZE);
  (void)memscpy(psloci+PTMSI_SIZE, PSLOCI_LEN-PTMSI_SIZE, gmm_stored_ptmsi_signature.value, PTMSI_SIGNATURE_SIZE);
  (void)memscpy(psloci+PTMSI_SIZE+PTMSI_SIGNATURE_SIZE, PSLOCI_LEN-(PTMSI_SIZE+PTMSI_SIGNATURE_SIZE), (byte *)&gmm_stored_rai, RAI_SIZE);
  psloci[RA_UPDATE_OFFSET] = gmm_update_status;
...
===========================================================================*/
void mm_swap_nv_imsi
(
  mm_as_id_e_type mm_as_id_target
)
{
  byte current_sim_imsi_from_nv[IMSI_LEN_IN_BYTES]={0};
  byte target_sim_imsi_from_nv[IMSI_LEN_IN_BYTES]={0};
  mm_as_id_e_type mm_as_id_current = mm_as_id;
  boolean last_nv_status = FALSE;
  ghdi_status_T nv_write_status;

  /* Read IMSI from NV for current SUB */
  last_nv_status = mm_nv_read_imsi(current_sim_imsi_from_nv);
  ...
  if(last_nv_status)
  {
    last_nv_status = mm_nv_read_imsi(target_sim_imsi_from_nv);
  }

  /* Write current SUB IMSI into NV of target SUB */
  if(last_nv_status)
  {
    if ((nv_write_status = mm_ghdi_nvmem_write(NV_GSMUMTS_IMSI_I, (ghdi_nvmem_data_T *)current_sim_imsi_from_nv)) != GHDI_SUCCESS)
    {
      MSG_HIGH_DS_1(MM_SUB,"Unable to access NV to save IMSI", nv_write_status) ;
      last_nv_status = FALSE;
    }
  }
  ...
  if(last_nv_status)
  {
    if ((nv_write_status = mm_ghdi_nvmem_write(NV_GSMUMTS_IMSI_I, (ghdi_nvmem_data_T *)target_sim_imsi_from_nv)) != GHDI_SUCCESS)
    {
      MSG_HIGH_DS_1(MM_SUB,"Unable to access NV to save IMSI", nv_write_status) ;
      last_nv_status = FALSE;
    }
...
boolean mm_load_sim_data( void )
{
...
#if(defined(MM_INT_TESTING) && defined(FEATURE_LTE))
   boolean eps_storage_supported_current = FALSE;
   boolean eps_storage_supported_actual = FALSE;
#endif
   ...
   */
   ...
   MSG_HIGH_DS_0(MM_SUB,"=MM= mm_load_sim_data called ");
   ...
   else
   {
	  ...
	  mm_debug_sim_swap_status = MM_DEBUG_SIM_CARD_MODE_NOT_DEFINED;
   ...
   */
   ...
   if ( !status )
   {
      /*------------------------------------------------------
       ...
       ------------------------------------------------------*/
      MSG_ERROR_DS(MM_SUB,"SIM/USIM read failed for IMSI file, GSDI status %d",  status,0,0);
      mm_debug_sim_swap_status = MM_DEBUG_SIM_IMSI_READ_FAILED;
      return FALSE;
   }
   else
   {
	 ...
	 MSG_HIGH_DS_1(MM_SUB, "=MM= SIM IMSI length: %d ", imsi_data.length);
     ...
     if (imsi_illegal)
     {
		...
		mm_debug_sim_swap_status = MM_DEBUG_SIM_IMSI_ILLEGAL;
        return FALSE;
     }
     else
     {
  ...
  NAS_ENTER_CRIT_SECT(mm_sim_swap_crit_sec);
#endif
  if(!(status = mm_nv_read_imsi(imsi_from_nv)))
  {
    mm_debug_sim_swap_status = MM_DEBUG_NV_IMSI_READ_FAILED;
  }
#if defined(FEATURE_DUAL_SIM)
  NAS_EXIT_CRIT_SECT(mm_sim_swap_crit_sec);
#endif

  MSG_HIGH_DS_8(MM_SUB, "=MM= NV IMSI[0] = %d  IMSI[1] = %d  IMSI[2] = %d  IMSI[3] = %d  \
                             IMSI[4] = %d  IMSI[5] = %d  IMSI[6] = %d  IMSI[7] = %d  ",imsi_from_nv[0],
							 imsi_from_nv[1],imsi_from_nv[2],imsi_from_nv[3],imsi_from_nv[4],imsi_from_nv[5],
							 imsi_from_nv[6],imsi_from_nv[7]);
  ...
  mm_sim_swapped = FALSE;
  if((!mm_imsi_is_native) && status)
  {
    mm_debug_sim_swap_status = MM_DEBUG_SIM_CHANGED;
    /* SIM IMSI is different from NV IMSI. Check if there is any SIM swap due to flex mapping change OR hard swap */
    mm_sim_swapped = mm_check_sim_swap_event(imsi_data, imsi_from_nv, &mm_as_id_target);

    MSG_HIGH_DS_2(MM_SUB, "=MM= sim_changed:1, mm_sim_swapped:%d mm_as_id_target:%d", 
                   mm_sim_swapped, mm_as_id_target);

    if(mm_sim_swapped)
    {
      /* Swap MM and EMM specific NV items and corresponding cache of current sub with target sub */
	  mm_handle_sim_swap(mm_as_id_target);

      /* SIM soft swap triggered. SIM shouldn't be treated as new SIM now */
	  mm_imsi_is_native = TRUE;
    }
  }
#endif
...
/* hack for passing CIT scripts */
#if(defined(MM_INT_TESTING) && defined(FEATURE_LTE))
  if(!mm_imsi_is_native)
  {
    eps_storage_supported_current = emm_ctrl_data_ptr->eps_storage_supported;
  }
#endif 
...
/* hack for passing CIT scripts */
  ...
  if(!mm_imsi_is_native)
  {
    eps_storage_supported_actual = emm_ctrl_data_ptr->eps_storage_supported;
    emm_ctrl_data_ptr->eps_storage_supported = eps_storage_supported_current;
  }
#endif 
...
/* hack for passing CIT scripts  */
  ...
  if(!mm_imsi_is_native)
  {
    emm_ctrl_data_ptr->eps_storage_supported = eps_storage_supported_actual;
  }
#endif 
      ...
      if ((mm_cmd_p = mm_get_cmd_buf()) != NULL)
      {
       ...
       if(status == MMGSDI_SUCCESS)
       {
         mm_cmd_p->cmd.mmgsdi_read_file_cnf.cnf_data.read_data.data_ptr = 
                       (uint8 *)modem_mem_calloc(1, sizeof( uint8 ) * cnf_ptr->read_cnf.read_data.data_len, MODEM_MEM_CLIENT_NAS);
...
#ifdef FEATURE_DUAL_SIM
      mm_emm_clear_dos_counters_per_subs(as_id); 
      mm_is_sim_to_be_cs_invalid_after_power_off_sim[as_id] = FALSE;
      mm_is_sim_to_be_ps_invalid_after_power_off_sim[as_id] = FALSE;
#else
...
#ifdef FEATURE_DUAL_SIM
      if (mm_illegal_me_sub_sim[as_id] == ILLEGAL_ME_ON_CURRENT_SUB)
      {
         mm_emm_send_legal_event_on_other_subs_per_subs(as_id);
         mm_send_mmr_me_blocked_status_ind(FALSE, as_id);
      }
      mm_illegal_me_sub_sim[as_id] = ILLEGAL_ME_SUB_NO_CAUSE;
#endif
    }
  }
...
)
{
  ...
  {
    MSG_ERROR_DS_0(MM_SUB, "=MM= Invalid AS ID ");
    return FALSE;
  }
...
void mmsim_csg_list_read_complete(void)
{
      ...
      mm_sub_is_rat_enabled(SYS_RAT_UMTS_RADIO_ACCESS)))
  {
    ...
    if(mm_sim_operator_csg_list.csg != NULL)
    {
      ...
      if(mm_sim_operator_csg_list.length > 0)
      {
        mm_sim_active_operator_csg_list.csg = 
                            (sys_csg_list_info_s_type*)modem_mem_calloc(1, sizeof(sys_csg_list_info_s_type) * mm_sim_operator_csg_list.length,  MODEM_MEM_CLIENT_NAS);
    ...
    if(mm_sim_csg_indicator_list.list != NULL)
    {
      ...
      if(mm_sim_csg_indicator_list.length > 0)
      {
        mm_sim_active_csg_indicator_list.list = 
          (mm_sim_display_indicator_info_s_type*) modem_mem_calloc(1, sizeof(mm_sim_display_indicator_info_s_type) * mm_sim_csg_indicator_list.length,  MODEM_MEM_CLIENT_NAS);
    ...
    if(mm_sim_allowed_csg_list.csg != NULL)
    {
      ...
      if(mm_sim_allowed_csg_list.length > 0)
      {
        mm_sim_active_allowed_csg_list.csg = 
                            (sys_csg_list_info_s_type*)modem_mem_calloc(1, sizeof(sys_csg_list_info_s_type) * mm_sim_allowed_csg_list.length,  MODEM_MEM_CLIENT_NAS);
    ...
    if(mm_reg_req__pended_due_to_csg_read == TRUE)
    {
      MSG_HIGH_DS_0(MM_SUB,"=MM= REG Req - CSG reading complete, send ignore IND to REG");
...
void mm_sim_read_allowed_csg_from_nv(sys_csg_list_s_type *csg_list)
{
  ...
  csg_data = (byte *) modem_mem_calloc(1, data_size, MODEM_MEM_CLIENT_NAS);
...
void mmsim_start_read_allowed_csg_list(void)
{
  ...
  if(mm_sim_allowed_csg_list.csg == NULL)
  {
    mm_sim_allowed_csg_list.csg = 
           (sys_csg_list_info_s_type*)modem_mem_calloc(1, sizeof(sys_csg_list_info_s_type)*SYS_CSG_LIST_MAX_LENGTH,  MODEM_MEM_CLIENT_NAS);
...
void mmsim_start_read_operator_csg_list(void)
{
  ...
  if(mm_sim_csg_indicator_list.list == NULL)
  {
    mm_sim_csg_indicator_list.list  = 
               (mm_sim_display_indicator_info_s_type*)modem_mem_calloc(1, sizeof(mm_sim_display_indicator_info_s_type) * SYS_CSG_LIST_MAX_LENGTH,  MODEM_MEM_CLIENT_NAS);
...
{
  mmgsdi_rec_num_type next_record = 0; 
...
==============================================================================*/
sys_modem_as_id_e_type mm_sim_get_as_id_from_slot
(
...
)
{
  ...
  if(mm_sim_mmgsdi_info_sim[SYS_MODEM_AS_ID_1].slot_id == slot_id)
  {
    as_id = SYS_MODEM_AS_ID_1;
  }
  else if(mm_sim_mmgsdi_info_sim[SYS_MODEM_AS_ID_2].slot_id == slot_id)
  {
    as_id = SYS_MODEM_AS_ID_2;
  }
#ifdef FEATURE_TRIPLE_SIM
  else if(mm_sim_mmgsdi_info_sim[SYS_MODEM_AS_ID_3].slot_id == slot_id)
  {
    as_id = SYS_MODEM_AS_ID_3;
  }
#endif  
  /* No SIM in SLOT 1 and SUB1 is not mapped to any slot now */
  else if( mm_sim_mmgsdi_info_sim[SYS_MODEM_AS_ID_1].slot_id == MMGSDI_MAX_SLOT_ID_ENUM )
  {
    as_id = SYS_MODEM_AS_ID_1;
  }
  /* No SIM in SLOT 1 and SUB2 is not mapped to any slot now */
  else if( mm_sim_mmgsdi_info_sim[SYS_MODEM_AS_ID_2].slot_id == MMGSDI_MAX_SLOT_ID_ENUM )
  {
    as_id = SYS_MODEM_AS_ID_2;
  }
#ifdef FEATURE_TRIPLE_SIM
  ...
  {
    MSG_HIGH_DS_0(MM_SUB, "=REG= Slot ID not mapped to any AS ID, Read default data from AS_ID1");
  }
  return as_id;
#else
...
FUNCTION NAME
  ...
  mm_sim_get_as_id_from_session_type

DESCRIPTION

  Function maps the as_id from the session type passed as parameter
...
==============================================================================*/
sys_modem_as_id_e_type mm_sim_get_as_id_from_session_type
(
  mmgsdi_session_type_enum_type  session_type
)
{
  ...
  if(mm_sim_mmgsdi_info_sim[SYS_MODEM_AS_ID_1].session_type== session_type)
  {
    as_id = SYS_MODEM_AS_ID_1;
  }
  else if(mm_sim_mmgsdi_info_sim[SYS_MODEM_AS_ID_2].session_type == session_type)
  {
    as_id = SYS_MODEM_AS_ID_2;
  }
#ifdef FEATURE_TRIPLE_SIM
  else if(mm_sim_mmgsdi_info_sim[SYS_MODEM_AS_ID_3].session_type == session_type)
  {
    as_id = SYS_MODEM_AS_ID_3;
  }
#endif  
  ...
  {
    MSG_HIGH_DS_0(MM_SUB, "=REG= Session type not mapped to any AS ID, Read default data from AS_ID1");
  }
  return as_id;
#else
  return SYS_MODEM_AS_ID_1;
...
boolean nas_mmgsdi_purge_temporary_identity_req 
(
         mmgsdi_session_type_enum_type session_type,
         uint32 user_data,
         nas_mmgsdi_purge_temporary_identity_callback_type mmgsdi_call_back
)
{
  mm_cmd_type *mm_cmd_p;
#if defined FEATURE_DUAL_SIM || defined FEATURE_SGLTE
  sys_modem_as_id_e_type as_id=SYS_MODEM_AS_ID_1;
#endif
  if (mmgsdi_call_back == NULL)
  {
     MSG_ERROR_DS_0(MM_SUB, "=MM= MMGSDI pass mmgsdi_call_back as NULL return");
     return FALSE;
  }
  mm_cmd_p = mm_get_cmd_buf();
  if (mm_cmd_p==NULL)
  {
    MSG_ERROR_DS_0(MM_SUB, "=MM= Error: Not able to Allocate MM Buffer return");
    return FALSE;
  }
#if defined FEATURE_DUAL_SIM || defined FEATURE_SGLTE
  as_id = mm_sim_get_as_id_from_session_type(session_type);
  if(IS_NOT_VALID_SUB_ID(as_id))
  {
    MSG_ERROR_DS(MM_LOCAL_SUB, "=MM= Invalid AS ID: %d", as_id+1,0,0);
  }
  mm_cmd_p->cmd.gsdi_guti_purge_ind.as_id = as_id;
#endif
  MSG_HIGH_DS_2(MM_LOCAL_SUB, "=MM= Purge Ind session type 0x%2x user data 0x%2x ", 
                                                            session_type, user_data);
  mm_cmd_p->cmd.gsdi_guti_purge_ind.message_header.message_set = MS_GSDI;
  mm_cmd_p->cmd.gsdi_guti_purge_ind.message_header.message_id = SIM_MM_PURGE_GUTI_IND;
  mm_cmd_p->cmd.gsdi_guti_purge_ind.message_header.message_len_lsb =
                   (sizeof(sim_mm_guti_purge_type_T) - sizeof(IMH_T)) % 256;
  mm_cmd_p->cmd.gsdi_guti_purge_ind.message_header.message_len_msb =
                   (sizeof(sim_mm_guti_purge_type_T) - sizeof(IMH_T)) / 256;
  mm_cmd_p->cmd.gsdi_guti_purge_ind.user_data = user_data;
  mm_cmd_p->cmd.gsdi_guti_purge_ind.mmgsdi_cb = mmgsdi_call_back;
  mm_put_cmd(mm_cmd_p);
  return TRUE;
}
...
#ifdef FEATURE_FEMTO_CSG
/*===========================================================================
FUNCTION      MMSIM_UPDATE_NV_CSG_SUPPORT_DATA
...
void mmsim_update_nv_csg_support_data(void)
{
  byte                    sst_buffer[13] = {0 };
  mmgsdi_return_enum_type mmgsdi_status = MMGSDI_SUCCESS;
  mmgsdi_file_enum_type   sim_file_name_sst = MMGSDI_NO_FILE_ENUM;
  mmgsdi_len_type         file_length = 0;
  ...
  if(mm_nv_csg_support > SYS_CSG_SUPPORT_DISABLED)
  {
    ...
    if(mm_sub_is_rat_enabled(SYS_RAT_LTE_RADIO_ACCESS) || 
      mm_sub_is_rat_enabled(SYS_RAT_UMTS_RADIO_ACCESS))
    {
      if(((mm_sim_allowed_csg_list_read_started == FALSE) || (mm_sim_operator_csg_list_read_started == FALSE)) &&
  	   ((mm_sim_active_operator_csg_list.csg != NULL) || (mm_sim_active_allowed_csg_list.csg != NULL)))
      {
        mm_sim_read_csg_list();
      }
    }
...
/*===========================================================================
FUNCTION      MM_SIM_ALLOWED_SERVICE_DOMAIN
...
sys_srv_domain_e_type mm_sim_allowed_service_domain
(
  sys_srv_domain_e_type mm_service_domain
)
{
  sys_srv_domain_e_type allowed_service_domain = mm_service_domain;

  /*
  ** Determine the allowed service domain based upon the
  ** current state of the SIM/USIM.
  */
  switch (mm_service_domain)
  {
    case SYS_SRV_DOMAIN_CS_ONLY:
    ...
    case SYS_SRV_DOMAIN_CS_PS:
      ...
      else if (mm_serving_plmn.info.sim_state == SYS_SIM_STATE_CS_PS_INVALID)
      {
        allowed_service_domain = SYS_SRV_DOMAIN_NO_SRV;
      }
      break;
