...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/mm/src/mmabort.c#1 $    $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...
void mm_abort_procedure( mm_cmd_type *msg_p )
{
   ...
   mm_qsh_ota_metric_signaling_log(ABORT,CN_TO_UE_DIRECTION,0xff,(mm_gmm_reject_cause_debug_type)reject_cause);
#endif
      ...
      ** ------------------------------------------------------------------------- */
      ...
      if (reject_cause == ILLEGAL_ME)
      {
         ...
         ** ------------------------------------------------------------------------ */
         ...
         if (mm_is_dos_mitigation_required_for_cs_domain(ABORT,reject_cause) )
         {
            mm_perform_dos_mitigation_action(ABORT,reject_cause);
         }
         else
         {
            previous_sim_state = mm_serving_plmn.info.sim_state;
...
void mm_abort_rr( rrc_cn_domain_identity_e_type domain_id, boolean delay_sig_con_release )
{
 ...
 if((!(mm_hard_abort_pending || mm_hard_abort_deep_sleep_pending))
#ifdef FEATURE_DUAL_SIM
...
void mm_connection_release( mm_cmd_type *msg_p )
{
      ...
      )
   {
     ...
     ** -------------------------------------------------------------------- */
      ...
      ** ------------------------------- */
      ...
      {
         MSG_HIGH_DS_2( MM_SUB, "=MM= MM received RR_REL_IND with cause %d mm_waiting_for_mocall_connect = %d", msg_p->cmd.rr_rel_ind.RR_cause,mm_waiting_for_mocall_connect);
         if((mm_per_nas_stacks_is_mt_page_in_progress((sys_modem_as_id_e_type)mm_as_id)) && 
            (mm_state!=MM_CONNECTION_ACTIVE) && 
            (( (mm_timer_status[TIMER_T3240] != TIMER_ACTIVE) || (mm_connection_activated == FALSE) ) ||(mm_waiting_for_csmt_response == TRUE)))
         {
           if(mm_waiting_for_csmt_response == TRUE)
           {
			 ...
			 /* Perform force detach to recover subsequent MT call after RA capability mismatch detection*/
							...
							(memcmp( mm_location_information.lai, mm_serving_lai, LAI_SIZE ) == 0))
           {
              ...
              {
                 MSG_HIGH_DS_0(MM_SUB,"=MM= Set combined updated for TAU needed, Set SG association to perfrom TAU on going back to LTE");
         ...
         /*If connection for L2G MO CSFB call is released before ACCEPT, depriorotize the current cell*/
#ifdef FEATURE_LTE
            if ((mm_waiting_for_mocall_connect == TRUE) &&
                ((mm_location_information.location_update_status == UPDATED) &&
                 (memcmp( mm_location_information.lai, mm_serving_lai, LAI_SIZE ) == 0)) &&
                      (mm_state == MM_WAIT_FOR_OUTGOING_MM_CONNECTION)) // or MM state 14
            {
                mm_send_rr_mo_csfb_failure_ind(TRUE);
            }
#endif   
            ...
            (msg_p->cmd.hdr.message_id  == MMCNM_REL_REQ))
   {
   	   ...
   	   mm_waiting_for_mocall_connect = FALSE;
   	   if (mm_timer_status[TIMER_FRLTE] != TIMER_STOPPED)
       {
      ...
      { 
        MSG_HIGH_DS_0(MM_SUB,"Clearing pending MO call after call is ended during LU/conn establishment");
        mm_send_mmcnm_rel_cnf();
      }
      else if((mm_state == MM_WAIT_FOR_RR_CONNECTION_MM) &&
               (mm_held_cnm_service_req.present == TRUE) &&
                ...
                (mm_held_cnm_service_req.interat_status == MM_AS_INTERRAT_CC_ORDER)))
      {
         ...
         MSG_HIGH_DS_0(MM_SUB,"Clearing pending MO call and aborting connection after call is ended during interrat");
...
#if defined(FEATURE_NAS_REL12_SPEC_CHANGE) && (defined (FEATURE_MMGPS) || defined ( FEATURE_CGPS_UMTS_CP))
                 if (mm_lcs_active)
                 {
   ...
   else
   {
      switch (msg_p->cmd.hdr.message_id)
      {
         case RR_ABORT_IND:
            ...
            mm_check_for_page_replay(msg_p->cmd.rr_abort_ind.RR_abort_cause);
            mm_waiting_for_csmt_response = FALSE;
            mm_waiting_for_mocall_connect = FALSE;
...
FUNCTION MM_RR_PREEMPTION_IND

DESCRIPTION
  This function deal with preemption ind
DEPENDENCIES
  None
...
void mm_rr_preemption_ind(mm_cmd_type * msg_p)
{
  ...
  MSG_HIGH_DS_0(MM_SUB, "=MM= Received RR_PREEMPTION_IND");  
  ...
  mm_stop_timer( TIMER_T3241 );
#endif /* end of (FEATURE_MMGPS || FEATURE_CGPS_UMTS_CP) */   
  mm_state_control(MM_STD_RADIO_PREEMPTION);
...
FUNCTION mm_check_for_page_replay

DESCRIPTION
  We check if we need to replay CSMT page response upon receiving rach failure
DEPENDENCIES
  None
...
{
     MSG_HIGH_DS_3(MM_SUB, "=MM= checking for page replay rr_abort_cause = %d mm_last_page_type = %d mm_state = %d",rr_abort_cause,mm_last_page_type,mm_state);  
  if (mm_state == MM_WAIT_FOR_NETWORK_COMMAND &&
      mm_timer_status[TIMER_MM_PAGE_RETRY] != TIMER_STOPPED &&
      rr_abort_cause == RANDOM_ACCESS_FAILURE 
#ifdef FEATURE_LTE_TO_GSM
      && (mm_last_page_type != PAGE_TYPE_NONE)
#endif
  )
  {
      MSG_HIGH_DS_1(MM_SUB, "=MM= mm_retry_page_rsp made to TRUE from %d",mm_retry_page_rsp); 
      mm_retry_page_rsp = TRUE;
