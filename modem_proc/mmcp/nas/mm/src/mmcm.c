...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/mm/src/mmcm.c#1 $    $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
   ...
   else if (is_OTA_message( msg_p ))
   {
      ...
      if ((GET_PD(msg_p->cmd.rr_data_ind.layer3_message[0]) == PD_CC) && 
          (msg_p->cmd.rr_data_ind.layer3_message[1] == SETUP))
      {
...
void mm_handle_test_control_data(mm_cmd_type *msg_p)
{
       ...
       (msg_p->cmd.hdr.message_id == (int)MM_DATA_IND))
   {
...
#ifdef FEATURE_DUAL_SIM
      mm_proc_status_set(SYS_PROC_TYPE_TEST_LOOPBACK, TRUE);
      ...
      (void)memscpy( (void *)tc_cmd->cmd.mm_unitdata_ind.data,gmm_max_L3_msg_length(MM_CS_DOMAIN),
       ...
       tc_cmd->cmd.mm_unitdata_ind.as_id = (sys_modem_as_id_e_type)mm_sub_id;
#endif
...
#ifdef FEATURE_DUAL_WCDMA
         tc_cmd->cmd.mm_rab_est_ind.as_id = (sys_modem_as_id_e_type)mm_sub_id;
#endif
...
#ifdef FEATURE_DUAL_WCDMA
         tc_cmd->cmd.mm_error_ind.as_id = (sys_modem_as_id_e_type)mm_sub_id;
#endif
