...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/mm/src/lte_nas_msg_parse.c#3 $
$Author: pwbldsvc $
$DateTime: 2022/10/25 10:48:38 $

when         who                          what                              
--------   --------  ------------------------------------------------
06/02/10   RI        Fixed the processing of SMC header.
...
)
{
  ...
  switch(security_hdr_type)
  {
        ...
        if(!dl_security_info->nas_connected_mode)
        {
          switch(message_id)
          {
            ...
            case EMM_IDENTITY_REQUEST:
              ...
              {
                MSG_ERROR_DS_0(MM_SUB, "=NAS_MSG_LIB= Received IDENTITY_REQ without IMSI as identity parameter, discarding this message");
    ...
    default:
      MSG_ERROR_DS_0(MM_SUB, "=NAS_MSG_LIB= NAS message with security header type = %d received, dropping this message");
      total_num_messages = 0;
      return FALSE;
...
)
{
  ...
  nas_event_report_payload(EVENT_LTE_EMM_OTA_INCOMING_MSG,sizeof(event),
  ...
  *pos += num_bits_to_read;
  ...
  switch(message_id)
  {
    case ATTACH_ACCEPT:
      ...
      if(nas_incoming_msg_ptr[nas_emm_index].emm_incoming_msg.attach_accept.emm_cause_exists)
      {
        emm_qsh_ota_metric_signaling_log(message_id,CN_TO_UE_DIRECTION,
          (uint8)(nas_incoming_msg_ptr[nas_emm_index].emm_incoming_msg.attach_accept.eps_attach_result),
          nas_incoming_msg_ptr[nas_emm_index].emm_incoming_msg.attach_accept.emm_cause);
      }
      else
      {
        emm_qsh_ota_metric_signaling_log(message_id,CN_TO_UE_DIRECTION,
...
#ifdef FEATURE_QSH_EVENT_METRIC
      emm_qsh_ota_metric_signaling_log(message_id,CN_TO_UE_DIRECTION,
        0xff,nas_incoming_msg_ptr[nas_emm_index].emm_incoming_msg.attach_rej.emm_cause);
#endif
      break;
...
#ifdef FEATURE_QSH_EVENT_METRIC
      if(nas_incoming_msg_ptr[nas_emm_index].emm_incoming_msg.mt_detach_req.emm_cause_exists)
      {
        emm_qsh_ota_metric_signaling_log(message_id,CN_TO_UE_DIRECTION,
          (uint8)(nas_incoming_msg_ptr[nas_emm_index].emm_incoming_msg.mt_detach_req.detach_req),
          nas_incoming_msg_ptr[nas_emm_index].emm_incoming_msg.mt_detach_req.emm_cause);
      }
      else
      {
        emm_qsh_ota_metric_signaling_log(message_id,CN_TO_UE_DIRECTION,
...
#ifdef FEATURE_QSH_EVENT_METRIC
      emm_qsh_ota_metric_signaling_log(message_id,CN_TO_UE_DIRECTION,
        0xff,nas_incoming_msg_ptr[nas_emm_index].emm_incoming_msg.service_rej.emm_cause);
#endif
      total_num_messages = 1;
      buf_len_bits = 0;
...
#ifdef FEATURE_QSH_EVENT_METRIC
      if(nas_incoming_msg_ptr[nas_emm_index].emm_incoming_msg.tau_accept.emm_cause_exists)
      {
        emm_qsh_ota_metric_signaling_log(message_id,CN_TO_UE_DIRECTION,
          (uint8)(nas_incoming_msg_ptr[nas_emm_index].emm_incoming_msg.tau_accept.eps_update_result),
          nas_incoming_msg_ptr[nas_emm_index].emm_incoming_msg.tau_accept.emm_cause);
      }
      else
      {
        emm_qsh_ota_metric_signaling_log(message_id,CN_TO_UE_DIRECTION,
...
#ifdef FEATURE_QSH_EVENT_METRIC
      emm_qsh_ota_metric_signaling_log(message_id,CN_TO_UE_DIRECTION,
        0xff,nas_incoming_msg_ptr[nas_emm_index].emm_incoming_msg.tau_rej.emm_cause);
#endif
      break;
...
#ifdef FEATURE_QSH_EVENT_METRIC
      emm_qsh_ota_metric_signaling_log(message_id,CN_TO_UE_DIRECTION,
        (uint8)(nas_incoming_msg_ptr[nas_emm_index].emm_incoming_msg.identity_req.mobile_identity),
        LTE_NAS_CAUSE_NONE);
#endif
      total_num_messages = 1;
      buf_len_bits = 0;
...
#ifdef FEATURE_QSH_EVENT_METRIC
      emm_qsh_ota_metric_signaling_log(message_id,CN_TO_UE_DIRECTION, 0xff, LTE_NAS_CAUSE_NONE);
#endif
      total_num_messages = 1;
...
#ifdef FEATURE_QSH_EVENT_METRIC
      emm_qsh_ota_metric_signaling_log(message_id,CN_TO_UE_DIRECTION,
        0xff,nas_incoming_msg_ptr[nas_emm_index].emm_incoming_msg.mt_emm_status.emm_cause);
#endif
      total_num_messages = 1;
      buf_len_bits = 0;
...
#ifdef FEATURE_QSH_EVENT_METRIC
      emm_qsh_ota_metric_signaling_log(message_id,CN_TO_UE_DIRECTION,
        (uint8)(nas_incoming_msg_ptr[nas_emm_index].emm_incoming_msg.cs_serv_notification.paging_id),
        LTE_NAS_CAUSE_NONE);
#endif
      total_num_messages = 1;
      buf_len_bits = 0;
...
)
{
  ...
  switch(out_buf->nas_hdr.pd)
  {
      ...
      if(encoding_present == TRUE)
      {
        if(out_buf->emm_outgoing_msg.service_req.security_hdr != SERV_REQ)
        {
          ...
          send_nas_ota_msg_log_packet(LOG_LTE_NAS_EMM_OTA_OUT_MSG_LOG_C,
                                      (word)(pos/8 -6),
                                      ota_buf + (skip_offset_for_normal_encoding/8));
        }
      }
      else
      {
        ...
        if(out_buf->emm_outgoing_msg.service_req.security_hdr != SERV_REQ)
        {
          ...
          nas_event_report_payload(EVENT_LTE_EMM_OTA_OUTGOING_MSG,
    ...
    case EPS_SESSION_MANAGEMENT_MESSAGES:
      ...
      if(encoding_present == TRUE)
      {
        send_nas_ota_msg_log_packet(LOG_LTE_NAS_ESM_OTA_OUT_MSG_LOG_C,
                                    (word)(pos/8-6),
                                    ota_buf + (skip_offset_for_normal_encoding/8));
      }
      else
      {
        ...
        event.out_msg = out_buf->nas_hdr.msg_id;
        nas_event_report_payload(EVENT_LTE_ESM_OTA_OUTGOING_MSG,
                                     sizeof(event), (void*)&event,mm_message_as_id);
      }
      break;
