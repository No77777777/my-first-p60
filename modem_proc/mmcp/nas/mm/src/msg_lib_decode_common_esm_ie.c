...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/mm/src/msg_lib_decode_common_esm_ie.c#1 $
$Author: pwbldsvc $
...
)
{
  ...
  ADJUST_OFFSET_AND_BUF_LEN_ESM(num_bits_to_read, pos, buffer_length, esm_msg_len);
  ...
  if((tft->num_filters == 0) || (tft->num_filters > MAX_PACKET_FILTERS))
  {
    ...
    ADJUST_OFFSET_AND_BUF_LEN_ESM((tft_len + extra_length)*8, pos, buffer_length, esm_msg_len);
    return TRUE;
  }
...
)
{
  ...
  if(length_to_skip > 0)
  {
    ...
    ADJUST_OFFSET_AND_BUF_LEN_ESM(length_to_skip*8, pos, buffer_length, esm_msg_len);
  }
...
)
{
  ...
  if(pkt_flow_id_len > MAX_PKT_FLOW_ID_LENGTH)
  {
    ...
    ADJUST_OFFSET_AND_BUF_LEN_ESM((pkt_flow_id_len - MAX_PKT_FLOW_ID_LENGTH)*8,
...
)
{
  ...
  if(extra_length > MAX_APN_LENGTH)
  {
    MSG_HIGH_DS_1(MM_SUB, "=NAS_MSG_LIB= Discarding additional %d octets in APN IE",
                  extra_length);
    ADJUST_OFFSET_AND_BUF_LEN_ESM(extra_length*8, pos, buffer_length, esm_msg_len);
  }
...
)
{
  ...
  word extra_length = 0;
  word original_buffer_length = 0;

  ASSERT(src != NULL);
  ASSERT(pos != NULL);
  ASSERT(buffer_length != NULL);
  ASSERT(prot_conf_opt != NULL);
  ASSERT(esm_msg_len != NULL);
  ...
  while((prot_conf_opt_len > 0) && (index < NUM_PROTOCOL_ID) )
  {
                         ...
                         (prot_conf_loop < MAX_PROTOCOL_ID_CONTENT); prot_conf_loop++)
    {
      ...
      {
        MSG_ERROR_DS_3(MM_SUB, "=NAS_MSG_LIB= Insufficient PCO Length = %d to decode Protocol ID Contents for Protocol ID %d, buffer_length = %d",
                       prot_conf_opt_len, index+1, (*buffer_length/8));
        return FALSE;
      }
  ...
  if((prot_conf_opt_len + extra_length) > 0)
  {
    ...
    ADJUST_OFFSET_AND_BUF_LEN_ESM((prot_conf_opt_len + extra_length)*8,
                                   pos, buffer_length, esm_msg_len);
  }
...
)
{
  ...
  if(apn_ambr_len > MAX_APN_AMBR_LENGTH)
  {
    ...
    ADJUST_OFFSET_AND_BUF_LEN_ESM((apn_ambr_len - MAX_APN_AMBR_LENGTH)*8,
                                  pos, buffer_length, esm_msg_len);
  }
...
)
{
  ...
  if(t3396_length > MAX_T3396_VALUE_LENGTH)
  {
    ...
    ADJUST_OFFSET_AND_BUF_LEN_ESM((t3396_length - MAX_T3396_VALUE_LENGTH)*8, pos, buffer_length,
                                   esm_msg_len);
  }
...
)
{
  ...
  /***** Length of IE *****/
  num_bits_to_read = 8;
  ratc_length = b_unpackb(src, *pos, num_bits_to_read);
  ADJUST_OFFSET_AND_BUF_LEN_ESM(num_bits_to_read, pos, buffer_length, esm_msg_len);

  /* Based on the length of IE decode the other fields */
  if(ratc_length > (*buffer_length/8))
  {
    MSG_ERROR_DS_2(MM_SUB, "=NAS_MSG_LIB= Insufficient buffer length = %d, ie_len = %d",
                   (*buffer_length/8), ratc_length);
    buffer_length_check_failed = TRUE;
    return FALSE;
  }

  if(ratc_length < MIN_RATC_VALUE_LENGTH)
  {
    MSG_ERROR_DS_2(MM_SUB, "=NAS_MSG_LIB= Received Reattempt Indicator IE len %d < %d Min allowed",
                   ratc_length, MIN_RATC_VALUE_LENGTH);
    return FALSE;
  }

  /* Read timer octet to decode RATC value */
  num_bits_to_read = 8;
  reattempt_ind->ratc_val = b_unpackb(src, *pos, num_bits_to_read);

  /*Mask the spare bits */
  reattempt_ind->ratc_val = (reattempt_ind->ratc_val & 0x01);

  ADJUST_OFFSET_AND_BUF_LEN_ESM(num_bits_to_read, pos, buffer_length, esm_msg_len);

  /* Discard additional octets in IE */
  if(ratc_length > MAX_RATC_VALUE_LENGTH)
  {
    MSG_HIGH_DS_1(MM_SUB, "=NAS_MSG_LIB= Discarding additional %d octets in Reattempt Indicator IE",
                  (ratc_length - MAX_RATC_VALUE_LENGTH));
    ADJUST_OFFSET_AND_BUF_LEN_ESM((ratc_length - MAX_RATC_VALUE_LENGTH)*8, pos, buffer_length,
                                   esm_msg_len);
  }
