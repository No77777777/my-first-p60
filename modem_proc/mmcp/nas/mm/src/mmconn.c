...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/mm/src/mmconn.c#1 $    $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...
/*===========================================================================
Function: mm_waiting_for_regs_complete_txn_get

Description:
Returns mm_waiting_for_regs_complete_txn
============================================================================*/
int8 mm_waiting_for_regs_complete_txn_get(mm_as_id_e_type as_id) 
{
#if defined FEATURE_DUAL_SIM || defined FEATURE_SGLTE
  return mm_waiting_for_regs_complete_txn_sim[as_id];
#else
  (void )as_id;
  return mm_waiting_for_regs_complete_txn;
#endif
}
...
FUNCTION mm_sms_over_ims_in_progress_in_other_sub

DESCRIPTION
  If any SMS over ims IS IN PROGRESS ON the other sub, return TRUE
...
{
  mm_as_id_e_type mm_other_as_id = MM_OTHER_SUB(mm_as_id);
  emm_ctrl_data_type *emm_ctrl_data_ptr = NULL;
  emm_ctrl_data_ptr = emm_db_get_ctrl_data_per_subs(mm_other_as_id);
  ASSERT(emm_ctrl_data_ptr != NULL);
  return emm_ctrl_data_ptr->wms_sms_status.sms_over_ims_status;
}
#endif
/*===========================================================================
...
void mm_perform_mm_connection_management(mm_cmd_type * msg_p)
{
  ...
  MSG_HIGH_DS_3(MM_SUB, "=MM= wlan_call_in_progress = %d, is_cs_mt_page_concurrency_allowed = %d, is_ims_mt_page_concurrency_allowed = %d",
                 mm_emm_wlan_call_in_progress(emm_db_get_ctrl_data_per_subs(mm_other_as_id)),
...
#ifdef FEATURE_DUAL_SIM
            /* -----------------------------------------------------------------
            ** Ignore MT page indication if MO/MT CS call on going on other sub.
            ** --------------------------------------------------------------- */

            if(MMCNM_IS_MODE_DUAL_STANDBY)
            {
              if (mm_check_other_sub_call_in_progress(mm_other_as_id))
              {
                MSG_HIGH_DS_0(MM_SUB, "=MM= MM ignored RR_PAGING_IND as CS call is active on other sub");
                break;
              }
            }
#ifdef FEATURE_LTE
            if((mm_emm_wlan_call_in_progress( emm_db_get_ctrl_data_per_subs(mm_other_as_id))) && 
               (mt_page_concurrency_config.is_cs_mt_page_concurrency_allowed == FALSE))
            {
              MSG_HIGH_DS_0(MM_SUB, "=MM= MM ignored RR_PAGING_IND as WLAN call active on other sub");
              break;
            }
            else if(mm_sms_over_ims_in_progress_in_other_sub())
            {
              MSG_HIGH_DS_0(MM_SUB, "=MM= MM ignored RR_PAGING_IND as IMS SMS progress on other sub");
              break;
            }
#endif
            ...
            ** MM procedure. */
                   ...
                   (gmm_attach_type == GMM_GPRS_ATTACH_WHILE_IMSI_ATTACHED)) ) ))
            {
              MSG_HIGH_DS_2( MM_SUB, "=MM= MM ignored RR_PAGING_IND, combined NMO1 Reg in progress, gmm state is %d, gmm_ra_update_type - %d", gmm_state,gmm_ra_update_type);
                       ...
                       (mmcoord_pending_reg_message.cmd.mmr_stop_mode_req.stop_mode_reason == SYS_STOP_MODE_REASON_POWER_OFF ||
                        mmcoord_pending_reg_message.cmd.mmr_stop_mode_req.stop_mode_reason == SYS_STOP_MODE_REASON_LPM))
                           || 
                       (mm_sim_card_mode == MMGSDI_APP_UNKNOWN)) && 
                     (mm_nreg_req_received) &&
                     (msg_p->cmd.rr_paging_ind.paging_type == CS_PAGING)
                    )
            {
               MSG_HIGH_DS_0(MM_SUB, "=MM= MM ignored RR_PAGE_IND WITH TMSI as IMSI detach already sent");
            }
#ifdef FEATURE_DUAL_SIM
            ...
            else if (mm_state == MM_IDLE)
            {
                               ...
                               mm_serving_plmn.ms_op_mode== SYS_SRV_DOMAIN_CS_PS);
              {
                if (cs_registered) 
                {
                  ...
                  {
                    /* -----------------------------------------------------------------
                    ** If other sub has any on-going MO/MT SMS, abort SMS(give 
                    ** priority to MT CS call over SMS) and process MT page indication.
                    ** --------------------------------------------------------------- */
#ifdef FEATURE_DUAL_SIM
                    if (MMCNM_IS_MODE_DUAL_STANDBY)
                    {
                       mm_check_and_abort_sms_conn(mm_other_as_id);
                    }
#endif
            ...
            {
              MSG_HIGH_DS_1( MM_SUB, "=MM= MM ignored RR_PAGING_IND mm state is %d or ignore mt page on other sub %d", mm_state);
            }
            break;
        ...
        case RRC_PAGE_IND:
            ...
            ** --------------------------------------------------------------- */
            if(MMCNM_IS_MODE_DUAL_STANDBY)
            {
              if (mm_check_other_sub_call_in_progress(mm_other_as_id))
              {
                MSG_HIGH_DS_0(MM_SUB, "=MM= MM ignored RRC_PAGE_IND as CS call is active on other sub");
                break;
              }
            }
#ifdef FEATURE_LTE
            if((msg_p->cmd.rrc_page_ind.paging_cause != RRC_PAGE_CAUSE_SMS) &&
               (msg_p->cmd.rrc_page_ind.paging_cause != RRC_PAGE_CAUSE_LOW_PRI_SIGNALLING) &&
               ((mm_emm_wlan_call_in_progress( emm_db_get_ctrl_data_per_subs(mm_other_as_id))) && 
                (mt_page_concurrency_config.is_cs_mt_page_concurrency_allowed == FALSE)))
            {
              MSG_HIGH_DS_0(MM_SUB, "=MM= MM ignored RRC_PAGE_IND as WLAN call active on other sub");
              break;
            }
            else if(mm_sms_over_ims_in_progress_in_other_sub())
            {
              MSG_HIGH_DS_0(MM_SUB, "=MM= MM ignored RRC_PAGE_IND as IMS SMS progress on other sub");
              break;
            }
#endif
#endif
            ...
            ** MM procedure. */
                   ...
                   (gmm_attach_type == GMM_GPRS_ATTACH_WHILE_IMSI_ATTACHED)) ) ))
            {
              MSG_HIGH_DS_2( MM_SUB, "=MM= MM ignored RRC_PAGING_IND, combined NMO1 Reg in progress, gmm state is %d, gmm_ra_update_type - %d", gmm_state,gmm_ra_update_type);
                      ...
                      (mm_sim_card_mode == MMGSDI_APP_UNKNOWN)) &&
                     (mm_nreg_req_received) &&
                     (msg_p->cmd.rrc_page_ind.page_ue_id == RRC_PAGE_WITH_TMSI)
                    )
            {
               MSG_HIGH_DS_0(MM_SUB, "=MM= MM ignored RRC_PAGE_IND WITH TMSI as IMSI detach already sent");
            }
            else if ((rrc_connection_status == MMRRC_RELEASE_IN_PROGRESS) &&
                     (mmcoord_reg_req_pending == TRUE) && 
                     (mmcoord_pending_reg_message.cmd.mmr_reg_req.network_selection_mode == SYS_NETWORK_SELECTION_MODE_MANUAL) && 
            ...
            else if (mm_state == MM_IDLE)
            {
              ...
              if (cs_registered)
              {
               ...
               {
                 MSG_HIGH_DS_0(MM_SUB, "=MM= MM ignored RRC_PAGING_IND as power down is initiated or stop mode is pending");
               }
               else if (mm_ps_detach_state != MM_PS_DETACH_STATE_NONE)
               {
            ...
            {
              MSG_HIGH_DS_1( MM_SUB, "=MM= MM ignored RRC_PAGE_IND MM state is %d ", mm_state);
            }
            break;
   ...
   else if ( msg_p->cmd.hdr.message_set == MS_CC_MM )
   {
      if (msg_p->cmd.hdr.message_id == MMCNM_REL_REQ)
      {
         ...
         if (mm_state == MM_WAIT_FOR_RR_CONNECTION_MM || mm_state == MM_WAIT_FOR_ADDITIONAL_OUTGOING_MM_CONNECTION)
         {
           pending_mm_msg_after_release = FALSE;
         }
   ...
   else if ( msg_p->cmd.hdr.message_set == MS_TIMER )
   {
      switch (msg_p->cmd.timer_expiry.timer_id)
      {
         ...
         case TIMER_T3240:
#ifdef FEATURE_LTE
               if((msg_p->cmd.timer_expiry.timer_id == TIMER_T3240) &&
                  emm_ctrl_data_ptr->is_srvcc_in_progress && 
                              ...
                              IMH_T * message)
{
   ...
   */
   if (is_umts_mode())
   {
...
#ifdef FEATURE_DUAL_SIM
      if ((establishment_cause == MM_RR_MO_FULL_SPEECH) ||
          ...
          (establishment_cause == MM_RR_CALL_REEST) ||
          ((establishment_cause == MM_RR_ANSWER_TO_PAGING) &&
           (mm_last_paging_cause != RRC_PAGE_CAUSE_SMS) &&
           (mm_last_paging_cause != RRC_PAGE_CAUSE_LOW_PRI_SIGNALLING)) ||
          ((mm_follow_on_request == TRUE) &&
           (establishment_cause == MM_RR_LOCATION_UPDATE)))
      {
        mm_proc_status_set(SYS_PROC_TYPE_CS_CALL,TRUE);
      }
      else if ((establishment_cause == MM_RR_SHORT_MESSAGE) ||
               ((establishment_cause == MM_RR_ANSWER_TO_PAGING) &&
                ((mm_last_paging_cause == RRC_PAGE_CAUSE_SMS) ||
                 (mm_last_paging_cause == RRC_PAGE_CAUSE_LOW_PRI_SIGNALLING))))
      {
      ...
      else if(establishment_cause == MM_RR_SUPPLEMENTARY_SERVICE)
      {
        mm_set_is_ss_request_rrc_sent(TRUE);
        mm_proc_status_set(SYS_PROC_TYPE_SUPPLEMENTARY_SERVICE,TRUE);
      }
      else if((establishment_cause == MM_RR_LOCATION_UPDATE) ||
              (establishment_cause == MM_RR_IMSI_DETACH))
      {
   ...
   else if(is_gsm_mode())
   {
      ...
      /* Pack class mark info start position. If GRR is in cell selection state, it will rebuild the class mark information 
       * again with latest cell details starting from this position
       */
      rr_est_req.cm1_start_pos = 0;
      rr_est_req.cm2_start_pos = 0;
			
      if(rr_est_req.layer3_message[1] == LOCATION_UPDATING_REQUEST)
      {
        rr_est_req.cm1_start_pos = (3 * sizeof(byte)) + LAI_SIZE; /* skip_pd + msgtype + lu type + LA ID */
      }
      else if(rr_est_req.layer3_message[1] == IMSI_DETACH_INDICATION)
      {
        rr_est_req.cm1_start_pos = (2 * sizeof(byte)); /* skip_pd + msgtype */
      }
      else if((rr_est_req.layer3_message[1] == CM_SERVICE_REQUEST) ||
#ifdef FEATURE_LTE_TO_GSM
              (rr_est_req.layer3_message[1] == PAGING_RESPONSE) ||
#endif
              (rr_est_req.layer3_message[1] == CM_REESTABLISHMENT_REQUEST))
      {
        rr_est_req.cm2_start_pos = (3 * sizeof(byte)); /* skip_pd + msgtype  + cksn */
      }
...
#ifdef FEATURE_DUAL_SIM
      rr_est_req.proc_type = mm_evaluate_proc_type();
      mm_last_proc_type_sent_set(rr_est_req.proc_type);
#endif
      ...
      case SYS_SRV_DOMAIN_CS_PS:
         switch (mm_sms_preferred_domain_id)
         {
            ...
            case CS_PREFERRED:
               ...
               (is_umts_mode() && (ps_session_status == MMRRC_ACTIVE) && !(gmm_ps_fop_resp != PS_FOP_NW_GRANTED) &&
                (cs_session_status != MMRRC_ACTIVE) &&
...
/* 24.301 5.5.3.3.2 */
          ...
          /* rejecting the call if active rat is not TDS and call type is secure call*/
           if(mm_emm_secure_status_type == SYS_SECURE_CALL_START && 
              (cnm_service_request_service_type == MO_CALL_ESTABLISHMENT) &&
              (mm_serving_plmn.info.active_rat != SYS_RAT_TDS_RADIO_ACCESS) )
           {
             MSG_HIGH_DS_0(MM_SUB,"=MM= Rejecting the call as as call type is secure but active rat is not TDS");
#ifdef FEATURE_SMS_ENHANCED_DOMAIN_SELECTION
                                             /*Final argument is added as a default*/
          ...
          */
          ...
          if ((msg_p->cmd.mmcnm_est_req.CNM_service_type == MO_CALL_ESTABLISHMENT  ||
              ...
              (mm_csfb_state != MM_CSFB_STATE_MO_CALL_ORIG))
          {
            QEVENT(QSH_CLT_MM, MM_QSH_EVENT_CS_CALL_STATUS,
                   MM_QSH_EVENT_CS_CALL_STATUS_VERSION, mm_sub_id,
                   "=MM= Call status %d Call direction %d", 
                   CAFI_KPI_EVENT_IND_START, CM_CALL_DIRECTION_MO);
           ...
           ********************************************/
           ...
           // place the emergency call in limited state
             ...
             if ((cnm_service_request_service_type == EMERGENCY_CALL_ESTABLISHMENT) && 
                 (((sd_ss_current_mcc_match_mcc_list_emerg_call_full_srv(&mm_serving_plmn.info.plmn, 
                  ...
                  (mm_check_if_emcall_allowed() == TRUE)|| 
                        ...
                        (emergency_call_after_lu_in_lpm == TRUE))))
           {
              ...
              else
              {
                 ...
                 if(is_umts_mode() && (rrc_connection_status == MMRRC_ACTIVE) &&
                    ((cs_cell_access == MM_AS_ACCESS_NORMAL_ONLY) || (cs_cell_access == MM_AS_ACCESS_NO_CALLS)))
                 {
                 ...
                 if (mm_ecall_mode_type == MMR_ECALL_MODE_ECALL_ONLY)
                 {
                   ...
                   {
                    if(mm_state == MM_IMSI_DETACH_INITIATED || mm_state == MM_WAIT_FOR_RR_CONNECTION_IMSI_DETACH)
                    {
                    ...
                    else if ( mm_location_information.location_update_status == UPDATED )
                    {
                      ...
                      */
                      if (memcmp( mm_location_information.lai, mm_serving_lai, LAI_SIZE ) == 0)
                      {
                        ...
                        if (((sd_ss_current_mcc_match_mcc_list_emerg_call_full_srv(&mm_serving_plmn.info.plmn, 
...
#ifdef FEATURE_NAS_ECALL
                       mm_call_est_cause = MO_CALL_ESTABLISHMENT;
#endif
   ...
   */
            ...
            if(mm_state == MM_WAIT_FOR_ADDITIONAL_OUTGOING_MM_CONNECTION && 
                     mm_get_nv_nas_rel_compliance_per_subs(
									 ...
									 mm_gw_intersystem_change_during_additional_mo_conn==TRUE)
              {     
      ...
      if (is_OTA_message(msg_p))
      {
         switch (get_OTA_msg_type(msg_p))
         {
            ...
            case CM_SERVICE_REJECT:
            {
...
#ifdef FEATURE_QSH_EVENT_METRIC
               mm_qsh_ota_metric_signaling_log(CM_SERVICE_REJECT,CN_TO_UE_DIRECTION,0xff, (mm_gmm_reject_cause_debug_type)reject_cause);
#endif

			  if(((mm_cnm_srv_est_cause == MM_RR_MO_FULL_SPEECH) ||
              ...
              {
                if((reject_cause ==NO_CIRCUIT_CHANNEL_AVAILABLE  )||
                   (reject_cause ==NETWORK_OUT_OF_ORDER)||
                   (reject_cause ==TEMPORARY_FAILURE)||
                   (reject_cause ==SWITCHING_EQUIPMENT_CONGESTION)||
                   (reject_cause ==REQUESTED_CIRCUIT_CHANNEL_NOT_AVAILABLE)||
                   (reject_cause ==RESOURCES_UNAVAILABLE_UNSPECIFIED))
                {
                  mm_send_rrc_abnormal_call_end_ind(mm_as_id);
                }
              }
#ifdef FEATURE_NAS_REL10
               msg_index = 3;
               ...
               {
#ifdef FEATURE_DUAL_SIM
			   if((cm_get_operator_info((sys_modem_as_id_e_type)mm_sub_id) == OPERATOR_CMCC)
#else
			   if((cm_get_operator_info(SYS_MODEM_AS_ID_1) == OPERATOR_CMCC)
#endif
                  &&(reject_cause == NO_SUITABLE_CELLS_IN_LA && is_tdscdma_mode()))
				  {
#ifdef FEATURE_SMS_ENHANCED_DOMAIN_SELECTION
                  mm_send_mmcnm_est_rej(AS_REJECT_CAUSE,AS_REJ_LOW_LEVEL_FAIL, MMCC_SMS_CS);
#else
               ...
               else if ( reject_cause == ILLEGAL_ME )
               {
                 ...
                 if (mm_is_dos_mitigation_required_for_cs_domain(CM_SERVICE_REJECT,reject_cause) )
                 {
                    mm_perform_dos_mitigation_action(CM_SERVICE_REJECT,reject_cause);
                 }
                 else
                 {
                    ...
                    mm_update_mm_serving_plmn_info( SYS_SIM_STATE_CS_INVALID,
                                                    ...
                                                    TRUE );
#ifdef FEATURE_DUAL_SIM
                    mm_gmm_set_illegal_non_illegal_cause(reject_cause); 
#endif
                 ...
                 if (mm_get_t3247_behavior_enabled() && mm_integrity_protected == FALSE)
                 {
              ...
              &&(reject_cause == NO_SUITABLE_CELLS_IN_LA && is_tdscdma_mode()))
              {
                mm_send_rrc_barring_info_ind(CALL_STATE_MO_REJECT_CAUSE15);
              }
#endif
#if defined ( FEATURE_MMGPS ) || defined ( FEATURE_CGPS_UMTS_CP )
               if (mm_lcs_active)
...
void mm_csfb_process_pending_call(void)
{
  ...
  {      
    ...
    {
      mm_waiting_for_mocall_connect = TRUE;
      MSG_HIGH_DS_0(MM_SUB,"=MM= CSFB: Setting mm_waiting_for_mocall_connect to TRUE");
    }
...
QSH_MDUMP_FN_ATTR void mm_log_mdump_static_vars() 
{
#if defined FEATURE_DUAL_SIM
...
#endif
{
  qsh_mdump_collect_high(&mm_waiting_for_regs_complete_txn,sizeof(mm_waiting_for_regs_complete_txn));
}
#if defined FEATURE_DUAL_SIM
mm_as_id = temp_as_id;
#endif
