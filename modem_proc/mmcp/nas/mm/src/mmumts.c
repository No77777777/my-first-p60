...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/mm/src/mmumts.c#1 $ $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...
/* this code is to query cnm whether data is for SMS or not and depending
    on that we set call back function */
      ...
      */
     rr_data_req.cm1_start_pos = 0;
     rr_data_req.cm2_start_pos = 0;

     message_type = rr_data_req.layer3_message[1] & 0x3F;
     if(message_type == LOCATION_UPDATING_REQUEST)
     {
       rr_data_req.cm1_start_pos = (3 * sizeof(byte)) + LAI_SIZE; /* skip_pd + msgtype + lu type + LA ID */
     }
     else if(message_type == IMSI_DETACH_INDICATION)
     {
       rr_data_req.cm1_start_pos = (2 * sizeof(byte)); /* skip_pd + msgtype */ 
     }
     else if((message_type == CM_SERVICE_REQUEST) ||
#ifdef FEATURE_LTE_TO_GSM
             (message_type == PAGING_RESPONSE) ||
#endif
             (message_type == CM_REESTABLISHMENT_REQUEST))
     {
       rr_data_req.cm2_start_pos = (3 * sizeof(byte)); /* skip_pd + msgtype  + cksn */
     }
