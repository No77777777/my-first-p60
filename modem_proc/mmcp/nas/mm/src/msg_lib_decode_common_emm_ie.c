...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/mm/src/msg_lib_decode_common_emm_ie.c#1 $
$Author: pwbldsvc $
...
)
{
  ...
  ASSERT(pos != NULL);
  ASSERT(buffer_length != NULL);
  ASSERT(emergency_num_list != NULL);
...
)
{
  ...
  /* Skip 2 bits and read ESR PS bit directly */
  ...
  /* Read CS-LCS bits directly */
  num_bits_to_read = 2;
  eps_nw_ft_spt->cs_lcs_val = b_unpackb(src,(word)(*pos+3), num_bits_to_read);
  if(eps_nw_ft_spt->cs_lcs_val >= SYS_CS_LCS_MAX)
  {
    MSG_ERROR_DS_1(MM_SUB, "CS-LCS value = %d is reserved", eps_nw_ft_spt->cs_lcs_val);
    return FALSE;
  }
...
)
{
  ...
  ASSERT(pos != NULL);
  ASSERT(buffer_length != NULL);
  ASSERT(daylight_saving_time != NULL);
...
)
{
  ...
  num_bits_to_read = 8;
  unpacked_byte = b_unpackb(src, *pos, num_bits_to_read);
  
  tau_accept->eps_bearer_context_status.eps_bearer_context_7_exists = (boolean)((unpacked_byte & 0x80) > 0);
  tau_accept->eps_bearer_context_status.eps_bearer_context_6_exists = (boolean)((unpacked_byte & 0x40) > 0);
  tau_accept->eps_bearer_context_status.eps_bearer_context_5_exists = (boolean)((unpacked_byte & 0x20) > 0);

  tau_accept->nw_bc_bm |= (uint16)(unpacked_byte >> 5);

  /* Read octet 4: EBI(8) - EBI(15) */

  unpacked_byte = b_unpackb(src, (word)(*pos+8), num_bits_to_read);

  tau_accept->eps_bearer_context_status.eps_bearer_context_15_exists = (boolean)((unpacked_byte & 0x80) > 0);  
  tau_accept->eps_bearer_context_status.eps_bearer_context_14_exists = (boolean)((unpacked_byte & 0x40) > 0);
  tau_accept->eps_bearer_context_status.eps_bearer_context_13_exists = (boolean)((unpacked_byte & 0x20) > 0);
  tau_accept->eps_bearer_context_status.eps_bearer_context_12_exists = (boolean)((unpacked_byte & 0x10) > 0);  
  tau_accept->eps_bearer_context_status.eps_bearer_context_11_exists = (boolean)((unpacked_byte & 0x08) > 0);
  tau_accept->eps_bearer_context_status.eps_bearer_context_10_exists = (boolean)((unpacked_byte & 0x04) > 0);
  tau_accept->eps_bearer_context_status.eps_bearer_context_9_exists = (boolean)((unpacked_byte & 0x02) > 0);  
  tau_accept->eps_bearer_context_status.eps_bearer_context_8_exists = (boolean)((unpacked_byte & 0x01) > 0);

  tau_accept->nw_bc_bm |= ((uint16)unpacked_byte << 3);

  /* Octet 3 & 4 is read. Adjust buffer length */
  ADJUST_OFFSET_AND_BUF_LEN_EMM(16, pos, buffer_length);
...
)
{
  ...
  {
    MSG_HIGH_DS_1(MM_SUB, "=NAS_MSG_LIB= Discarding additional %d bits in TAI list IE",
