...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/mm/src/emm_gps_lib.c#1 $ $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $ 
...
)
{
  ...
  if(emm_msgr_send((msgr_hdr_s *)&failure_ind, sizeof(failure_ind)) == E_NO_MEMORY)
  {
    MSG_ERROR_DS_0(MM_SUB, "=EMM= Failed to send NAS_EMM_GENERIC_NAS_TRANSPORT_FAILURE_IND due to no memory");
  }
  else if(emm_msgr_send((msgr_hdr_s *)&failure_ind, sizeof(failure_ind)) != E_SUCCESS)
  {
