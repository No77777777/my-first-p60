...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/mm/src/gmmreg.c#3 $ $DateTime: 2023/08/14 05:37:59 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------       
07/22/13   sn      Using previous LA information if waiting for EST_CNF from RRC and received NO_SERVICE from RRC
...
extern nas_drx_set_cfg_req_enum_T gmm_set_drx_req_pending_sim[MAX_AS_IDS];
...
#else
extern nas_drx_set_cfg_req_enum_T gmm_set_drx_req_pending;
...
#ifdef FEATURE_LTE
extern void emm_invalidate_mobility_info
(
...
/*4.7.5.1.5 (j)*/
          ...
          )
      {
        ...
        else
        {
             ...
             (mm_serving_plmn.ms_op_mode == SYS_SRV_DOMAIN_CS_PS))
          {
            ...
            if(mm_gs_association_established == TRUE &&
               gmm_rau_attempt_counter >= 6 &&
               (memcmp(mm_previous_serving_lai,mm_serving_lai,LAI_SIZE) == 0) && 
    ...
    /* REL12 Spec CR 2678: Deactivate non transferable PDPs at SM task after X2G transfer when no RAU is triggered */
    if((is_gsm_mode()) &&
    ...
    {
      MSG_HIGH_DS_1(MM_SUB, "=MM= Send No RAU indication to SM. gmm_state = %d", gmm_state);
      gmm_send_gmmsm_irat_ps_service_ind();
    }
#endif
...
#endif
  else if(mm_manual_mode_irat_ho_in_non_user_sel_cell == TRUE)
  {
        gmm_set_state( GMM_DEREGISTERED );
        if (mm_serving_plmn.gmm_service_state == SERVICE_NOT_AVAILABLE)
        {
...
/*Reset PS handover RAU initiation flag*/
...
#ifdef FEATURE_NAS_REL12_SPEC_CHANGE
    gmm_sync_pmm_state_during_w_to_lg_and_back_to_w_irat = FALSE;
#endif
    ...
    */
...
#ifdef FEATURE_LTE
    if (gmm_set_drx_req_pending == NAS_DRX_CFG_REQ_PENDING)
    {
      gmm_set_drx_req_pending = NAS_DRX_CFG_REQ_PROCESSING;
    }
#endif
    ...
    gmm_rau_is_required_due_to_dsr = FALSE;
...
#ifdef FEATURE_LTE
    mmcoord_pending_reg_message.cmd.mmr_reg_req.lte_cs_domain_param.is_voice_domain_ie_valid  = gmm_reg_pended_lte_cs_param.is_voice_domain_ie_valid;
    mmcoord_pending_reg_message.cmd.mmr_reg_req.lte_cs_domain_param.voice_domain_pref  = gmm_reg_pended_lte_cs_param.voice_domain_pref;
#endif
    /* ---------------------------------------------
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  ** ------------------------------------ */
  ...
  if (gprs_attach_attempt_counter > MAX_ATTEMPT_COUNTER_ATTACH)
  {
     MSG_HIGH_DS_2(MM_SUB, "=MM= T3310 timeout counter = %d and GPRS Attach Attempt counter = %d", t3310_timeout_counter, MAX_ATTEMPT_COUNTER_ATTACH);
  }
  else
  {
...
void gmm_handle_gprs_attach( mm_cmd_type *message )
{
  ...
  boolean                 gmm_buffer_rej_cause_network_failure = FALSE;
#ifdef FEATURE_WCDMA
...
#endif
  boolean is_legacy_required = TRUE;
  mm_as_cell_access_status_T ps_cell_access = gmm_get_ps_cell_access(PROCEDURE_PS_REGISTRATION); 
  mm_as_cell_access_status_T cs_cell_access = mm_get_cs_cell_access(PROCEDURE_CS_REGISTRATION); 
  sys_sim_state_e_type previous_sim_state = mm_serving_plmn.info.sim_state;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
      ...
      switch (message->cmd.hdr.message_id)
      {
        case RR_ACTIVATION_IND:
          ...
          ** ------------------- */
		  ...
		  mm_ssgccs_handle_outgoing_msg(MM_ATTACH_REQ_FAIL_IND,0,mm_serving_plmn.info.active_rat );
    ...
    case MS_MM_TDSRRC:
      switch (message->cmd.hdr.message_id)
      {
        ...
        case RRC_ABORT_IND:
          ...
          else
          {
            ...
            ** ------------------- */
            ...
            {
               mm_gmm_reg_failed_cell_id = mm_serving_cell_id;
        ...
        case MM_DATA_IND:
          ...
          {
			 mm_ssgccs_handle_icoming_msg(message->cmd.mm_data_ind.L3_data[1],mm_serving_plmn.info.active_rat,PS_DOMAIN_PROCEDURE,message);
            case GMM_ATTACH_ACCEPT:
              ...
              if (gmm_valid_attach_accept_message( message, attach_accept_msg ))
              {
                ...
                {
                  gmm_qsh_ota_metric_signaling_log(GMM_ATTACH_ACCEPT,CN_TO_UE_DIRECTION,
                            attach_accept_msg->attach_result, (mm_gmm_reject_cause_debug_type)attach_accept_msg->gmm_cause);
                }
                else
                {
                  gmm_qsh_ota_metric_signaling_log(GMM_ATTACH_ACCEPT,CN_TO_UE_DIRECTION,
                                                 attach_accept_msg->attach_result, MM_GMM_NO_FAILURE_DEBUG);
                }
#endif
#ifdef FEATURE_LTE
                /*Reset flag on Attach accept  */
            ...
            */                
                ...
                ** ---------------------------- */
                   ...
                   ** and the indicated value is larger than the last received value, or the indicated value is "deactivated"
                   ** or zero, the MS shall use the last received value.
                   ------------------------------------------------------------------------------------------------------------*/
                  if((is_umts_mode()) &&
                ...
                if (attach_accept_msg->allocated_ptmsi_present)
                {
                  ...
                  ** ------------------------------------------- */
                  for (i=0; i < PTMSI_SIZE; i++)
                  {
                    if(is_gsm_mode())
                    {
                      gmm_old_ptmsi.digit[i] = gmm_stored_ptmsi.digit[i];
                    }
                    else
                    {
      ...
      #ifdef FEATURE_LTE
                if (gmm_set_drx_req_pending == NAS_DRX_CFG_REQ_PROCESSING)
                {
...
#ifdef FEATURE_NAS_REL12_SPEC_CHANGE
                   /* Spec 24.008, Section 4.7.13, Spec CR 2488
                    * The service request attempt counter shall be reset when:
                    *   i.)  an attach or combined attach procedure is successfully completed;
                    *   ii)  a normal or periodic routing area updating or a combined routing area 
                    *        updating procedure is successfully completed; or
                    *   iii) a service request procedure is successfully completed.
                    */
                    gprs_service_attempt_counter = 0; 
                    mm_stop_timer(TIMER_T3325);
                    MSG_HIGH_DS_0(MM_SUB, "=MM= gprs_service_attempt_counter is reset"); /* !tocheck! can all ATTACH Accept blocks be brought out */
#endif
                    ...
                    if ( attach_accept_msg->gmm_cause_present)
                    {
                      switch (attach_accept_msg->gmm_cause)
                      {
                        case IMSI_UNKNOWN_IN_HLR:
                          ...
                          gprs_service_attempt_counter = 0;
                          mm_stop_timer(TIMER_T3325);
                          MSG_HIGH_DS_0(MM_SUB, "=MM= gprs_service_attempt_counter is reset");
...
** ---------------------------- ---------------------------- ----------------------------*/
                        default:
                          ...
                          ** -------------------------------------------------------- */
                          ...
                          gprs_service_attempt_counter = 0;
                          mm_stop_timer(TIMER_T3325);
                          MSG_HIGH_DS_0(MM_SUB, "=MM= Service attempt counter is reset");
#endif
                          ...
                          if(attach_accept_msg->gmm_cause == CONGESTTION)
                          {
                            ...
                            gmm_rau_attempt_counter = 5;
                            /* Spec 24.008, Section 4.2.5.1.7
                             * 4.2.5.1.7 Substate, ATTEMPTING-TO-UPDATE-MM
                             * initiate routing area update indicating "combined RA/LA updating with IMSI attach" 
                             * on the expiry of timers T3311 or T3302;
                             */
...
#endif
                          if (gmm_rau_attempt_counter > MAX_ATTEMPT_COUNTER_RAU)
                          {
                             MSG_HIGH_DS_1(MM_SUB, "=MM= RAU Attempt counter = %d", MAX_ATTEMPT_COUNTER_RAU);
                          }
                          else
                          {
                             MSG_HIGH_DS_1(MM_SUB, "=MM= RAU Attempt counter = %d", gmm_rau_attempt_counter);
                          }
                          if (gmm_rau_attempt_counter <= MAX_ATTEMPT_COUNTER_RAU && gmm_rau_attempt_counter > 0 )
                          {
                            gmm_buffer_rej_cause[gmm_rau_attempt_counter-1] = attach_accept_msg->gmm_cause;
                          }
                          for (i=0; i<MAX_ATTEMPT_COUNTER_RAU; i++)
                          {
                            if (gmm_rau_attempt_counter == MAX_ATTEMPT_COUNTER_RAU && gmm_buffer_rej_cause[i] == NETWORK_FAILURE)
                            {
                              gmm_buffer_rej_cause_network_failure = TRUE; 
                              break;
                            }
                          }
                          if (mm_managed_roaming_enabled && gmm_buffer_rej_cause_network_failure == TRUE)
                          {
...
/*24.008: The behaviour of the MS in those cases is specified in subclause 4.7.3.2.5.*/
                ...
                if (is_umts_mode() && (gmm_ps_fop_req)) 
                {
				  ...
				  if(gmm_force_for_always_on_data==FALSE)
				  	{
                ...
                if(is_gsm_mode() && mm_service_ind_blocked_due_to_cs_reg == FALSE)
                {
...
#ifdef FEATURE_LTE
                if ((gmm_state == GMM_REGISTERED) &&
                    ((emm_ctrl_data_ptr->emm_voice_domain_change_pended == NAS_NON_ACQ_REQ_PENDING)
                     && (mm_checkif_rat_pri_list_has_rat(&mm_rat_pri_list_info,SYS_RAT_LTE_RADIO_ACCESS) == TRUE)) &&
                    (gmm_reg_lte_cs_param.voice_domain_pref != gmm_reg_pended_lte_cs_param.voice_domain_pref))
                {
                  MSG_HIGH_DS_0(MM_SUB, "=MM= Start RAU procedure - change in voice_domain_pref");
                  gmm_update_ra_type();
		...
		if ((((gmm_get_map_rej_cause7_to_rej_cause14()) 
                     && (!gmm_is_camped_on_hplmn())) || (gmm_get_gsm_map_rej_hplmn_cause7_to_rej_cause14())) 
                     && (attach_reject_msg.gmm_cause == GPRS_SERVICES_NOT_ALLOWED))
                { //CMCC GSM Rej7
              ...
              mm_send_gstk_gprs_attach_reject_evt(attach_reject_msg.gmm_cause);
...
#ifdef FEATURE_QSH_EVENT_METRIC
                gmm_qsh_ota_metric_signaling_log(GMM_ATTACH_REJECT,CN_TO_UE_DIRECTION,
                         0xff, (mm_gmm_reject_cause_debug_type)attach_reject_msg.gmm_cause);
#endif
                ...
                if(mm_checkif_rat_pri_list_has_rat(&mm_rat_pri_list_info,SYS_RAT_LTE_RADIO_ACCESS) || gmm_is_emm_params_to_be_deleted_on_ps_rej())
                {
...
#ifdef FEATURE_DUAL_SIM
              if ((mm_illegal_me_sub == ILLEGAL_ME_ON_CURRENT_SUB) && 
                    (attach_reject_msg.gmm_cause != ILLEGAL_ME))
              {
                  mm_emm_unblock_me();
              }
#endif
...
/* Spec 24.008, Section 4.7.1.9, Spec CR 2576
 ...
 */
                    ...
                    if ((mm_nv_nas_rel_compliance >= NV_NAS_COMPLIANCE_REL12) &&
                        (attach_reject_msg.gmm_cause == GPRS_SERVICES_AND_NON_GPRS_SERVICES_NOT_ALLOWED) && 
                          is_umts_mode()) 
                    {
                       mm_start_timer(TIMER_T3340,DEFAULT_TIMEOUT);
                    }
                    ...
                    ** --------------------------------------------------- */
                    ...
                    if (gmm_is_dos_mitigation_required_for_ps_domain(GMM_ATTACH_REJECT, attach_reject_msg.gmm_cause))
                    {
                      gmm_perform_dos_mitigation_action(GMM_ATTACH_REJECT, attach_reject_msg.gmm_cause);
                    }
                    else
                    {
...
#ifdef FEATURE_DUAL_SIM
                      mm_gmm_set_illegal_non_illegal_cause(attach_reject_msg.gmm_cause);
#endif
                      if (mm_reg_waiting_for_reg_cnf)
                      {
                        mmcoord_mmr_reg_cnf.service_state  = mm_serving_plmn.info;
                    ...
                    if (mm_get_t3247_behavior_enabled() && gmm_integrity_protection_activated == FALSE)
                    {
                  ...
                  case GPRS_SERVICES_NOT_ALLOWED:
                     ...
                     *  d) may start the timer T3340 if the MS receives any of the reject cause values #7 or #8.
                     */
                    if ((mm_nv_nas_rel_compliance >= NV_NAS_COMPLIANCE_REL12) && is_umts_mode())
                    {
                      mm_start_timer(TIMER_T3340,DEFAULT_TIMEOUT);
                    }
#endif
                    ...
                    if(mm_serving_plmn.ms_op_mode == SYS_SRV_DOMAIN_CS_PS)
                    {
                      ...
                      if (mm_serving_plmn.network_op_mode == NETWORK_OP_MODE_1)
                      {
                        ...
                        ** ----------------------------------------------------- */
                        ...
                        if (is_gsm_mode() && 
                            (mm_state == MM_IDLE) && 
                            (mmcoord_pending_reg_message.cmd.hdr.message_set == (int) MS_MM_REG) &&
                            (mmcoord_pending_reg_message.cmd.hdr.message_id == (int) MMR_REG_REQ))
                        {
                  ...
                  case PLMN_NOT_ALLOWED:
                    ...
                    ** ---------------------------------------------- */
					...
					if(attach_reject_msg.gmm_cause == PLMN_NOT_ALLOWED &&
				         (gmm_manual_mode_user_selection_procedure)	&&
				         (sys_plmn_match(mm_serving_plmn.info.plmn, mmcoord_pending_reg_message.cmd.mmr_reg_req.plmn))&&
				   	  #ifdef FEATURE_DUAL_SIM
						(reg_sim_per_subs_plmn_forbidden( mm_serving_plmn.info.plmn, (sys_modem_as_id_e_type)mm_sub_id ))
					...
					{
						MSG_HIGH_DS_0(MM_SUB, "Timer T3247 should not be started");
					}
                    else
                    {
                       if (mm_timer_status [TIMER_T3247] != TIMER_ACTIVE && mm_get_t3247_behavior_enabled() && gmm_integrity_protection_activated == FALSE)
                       {
                          mm_start_timer( TIMER_T3247,DEFAULT_TIMEOUT );
                       }
                    }
...
gmm_manual_mode_user_selection_procedure = FALSE;
                    ...
                    ** ------------------------------------------------------------- */
                    ...
                    {
                      mmcoord_mmr_reg_cnf.service_state  = mm_serving_plmn.info;
                      if ((mm_held_cnm_service_req.present == TRUE || mm_state == MM_WAIT_FOR_RR_CONNECTION_MM) &&
                          (mm_held_cnm_service_req.establishment_cause == (int) MM_RR_EMERGENCY_CALL)
                          )
                      {
                         mmcoord_mmr_reg_cnf.cause = MMR_CAUSE_ABORT_PLMN_SELECTION;
                      }
                      mm_send_mmr_reg_cnf();
                    ...
                    else
                    {
                      ...
                      {
                         mmr_service_ind.cause = MMR_CAUSE_ABORT_PLMN_SELECTION;
                      }
                      mm_send_mmr_service_ind( &mmr_service_ind );
                    }
                    break;
                    ...
                    **************************************************************/
                    ...
                    if (attach_reject_msg.gmm_cause == LA_NOT_ALLOWED)
                    {
                      ...
                      ** --------------------------------------------------------- */
                      ...
                      {
                        mm_add_forbidden_national_roaming( &gmm_current_lai,FALSE );
                      }
                      ...
                      {
                         mm_add_forbidden_regional_roaming( &gmm_current_lai,FALSE );
                      }
                      mmcoord_mmr_reg_cnf.cause = MMR_CAUSE_FAILURE_REMAIN_ON_PLMN;
#endif
                      gmm_manual_mode_user_selection_procedure = FALSE;
                    ...
                    else if (attach_reject_msg.gmm_cause == NATIONAL_ROAMING_NOT_ALLOWED)
                    {
                       ...
                       mmcoord_mmr_reg_cnf.cause = MMR_CAUSE_FAILURE_INITIATE_PLMN_SELECTION;
                       gmm_manual_mode_user_selection_procedure = FALSE;
                    ...
                    else if (attach_reject_msg.gmm_cause == NO_SUITABLE_CELLS_IN_LA)
                    {
                       ...
                       {
                          mm_add_forbidden_national_roaming( &gmm_current_lai,FALSE );
                          mm_add_lai_in_reject_list(&gmm_current_lai,FALSE);
                       }
                      mmcoord_mmr_reg_cnf.cause = MMR_CAUSE_FAILURE_LAI_NOT_ALLOWED;
                  ...
                  case GPRS_SERVICES_NOT_ALLOWED_IN_THIS_PLMN:
                    ...
                    ** ---------------------------------------------------------- */
                    ...
                    {
#ifdef FEATURE_DUAL_SIM
                      if ((reg_nv_is_ehplmn_forbid_enabled((sys_modem_as_id_e_type)mm_sub_id) == FALSE)
#ifdef FEATURE_EQUIVALENT_HPLMN
                      && (reg_sim_per_subs_is_ehplmn(mm_serving_plmn.info.plmn, (sys_modem_as_id_e_type)mm_sub_id))
#else
                      && (sys_plmn_match(reg_sim_per_subs_read_hplmn((sys_modem_as_id_e_type)mm_sub_id), mm_serving_plmn.info.plmn) )
                ...
                case CONGESTTION: 
                  ...
                  default:
                    ...
                    ** --------------------------------------------- */
                      ...
                      if (gprs_attach_attempt_counter <= MAX_ATTEMPT_COUNTER_ATTACH)
                      {
                        gmm_buffer_rej_cause[gprs_attach_attempt_counter-1] = attach_reject_msg.gmm_cause;
                      }
                      for (i=0; i<MAX_ATTEMPT_COUNTER_ATTACH; i++)
                      {
                        if (gprs_attach_attempt_counter == MAX_ATTEMPT_COUNTER_ATTACH && gmm_buffer_rej_cause[i] == NETWORK_FAILURE)
                        {
                          gmm_buffer_rej_cause_network_failure = TRUE; 
                          break;
                        }
...
** ---------------------------- ---------------------------- ---------------------------- ----------------------------*/
  ...
  if((gmm_state == GMM_DEREGISTERED) ||
     ...
     (gmm_state == GMM_SERVICE_REQUEST_INITIATED))
  {
    mm_proc_status_set(SYS_PROC_TYPE_PS_REGISTRATION, FALSE);
  }
         ...
         (gmm_ra_update_type == GMM_COMBINED_RA_LA_UPDATING_WITH_IMSI_ATTACH)))
    {
       if ((gmm_rai_changed( gmm_previous_serving_rai.location_area_code,
           ...
           (((mm_last_procedure_to_update_registration & MM_RAU) != MM_RAU)
            ...
            || (emm_ctrl_data_ptr->TIN == NAS_MM_TIN_GUTI)
            ||  gmm_is_cap_changed()
            || (emm_lte_cs_domain_param_match(gmm_reg_lte_cs_param, gmm_reg_pended_lte_cs_param) == FALSE)
            || (gmm_set_drx_req_pending == NAS_DRX_CFG_REQ_PENDING)
#endif
           ))
       {      
...
/*Reset PS handover RAU initiation flag*/
    ...
    ** ------------------------------------------------------------------------- */
     ...
     */
    if ((!mmsim_sim_valid(SYS_SRV_DOMAIN_CS_ONLY)|| (mm_serving_plmn.ms_op_mode == SYS_SRV_DOMAIN_PS_ONLY)) &&
        (gmm_ra_update_type != GMM_PERIODIC_UPDATING))
    {
  ...
  ** ------------------------------------ */
  ...
  else /* routing_area_updating_attempt_counter >= 5 */
  {
     ...
     **                          the routing area updating fails and the routing area updating attempt counter is equal to 5
      ** ---------------------------- ---------------------------- ----------------------------*/
    if(gmm_t3302_timer_value != mm_get_timeout(TIMER_T3302) && (gmm_plmn_changed(gmm_previous_plmn_with_t3302_value, mm_serving_plmn.info.plmn))&& 
                           !(sys_eplmn_list_equivalent_plmn(gmm_previous_plmn_with_t3302_value) && sys_eplmn_list_equivalent_plmn(mm_serving_plmn.info.plmn)))
    {
      gmm_t3302_timer_value = mm_get_timeout(TIMER_T3302);
  ...
  if (mm_serving_plmn.info.ps_data_suspend)
  {
    ...
    if (mm_reg_waiting_for_reg_cnf)
    {
      ...
      /* Block sending REG_CNF when UE is in RLF state to avoid limited service being reported on UI. */
#ifdef FEATURE_WRLF_SYSTEM_SEL
      else if (is_umts_mode() && (mm_wcdma_rlf_state != WCDMA_RLF_IDLE) )
      {
        MSG_HIGH_DS_0( MM_SUB, "=MM= Block sending REG CNF to CM in this case, as WRLF is in progress \n");
      }
#endif
      else
      {
...
#endif
#ifdef FEATURE_WRLF_SYSTEM_SEL
       /* Block sending SERVICE_IND when UE is in RLF state to avoid limited service being reported on UI. */
       if (is_umts_mode() && (mm_wcdma_rlf_state != WCDMA_RLF_IDLE) )
       {
         MSG_HIGH_DS_0( MM_SUB, "=MM= Block sending service ind to CM in this case, as WRLF is in progress \n");
       }
       else
#endif
      {
      ...
      switch (message->cmd.hdr.message_id)
      {
        case RR_ACTIVATION_IND:
          ...
          ** ------------------------------------------------------------------ */
		  ...
		  mm_ssgccs_handle_outgoing_msg(MM_ROUTING_AREA_UPDATE_FAIL_IND,0,mm_serving_plmn.info.active_rat );
        ...
        case RRC_ABORT_IND:
          ...
          else
          {
            ...
            ** ------------------------------------------------------------------ */
            ...
            mmsend_nas_reg_event_packet(PROC_NAS_RAU_EVENT,PROC_END,PROC_END_LOWER_LAYER_FAILURE,MT_DETACH_TYPE_NONE,MT_DETACH_CAUSE_NONE);
            if(gmm_substate == GMM_ATTEMPTING_TO_UPDATE && mm_serving_cell_valid)
            {
              mm_gmm_reg_failed_cell_id = mm_serving_cell_id;
              gmm_rau_upon_entry_to_a_new_cell = TRUE;
            }			
          }
          break;
            ...
            case GMM_ROUTING_AREA_UPDATE_ACCEPT:
              ...
              ** --------------------------------------------------- */
                    ...
                    message, ra_update_accept_msg ))
              {
                ...
                {
                  gmm_qsh_ota_metric_signaling_log(GMM_ROUTING_AREA_UPDATE_ACCEPT,CN_TO_UE_DIRECTION,
                            ra_update_accept_msg->update_result, (mm_gmm_reject_cause_debug_type)ra_update_accept_msg->gmm_cause);
                }
                else
                {
                  gmm_qsh_ota_metric_signaling_log(GMM_ROUTING_AREA_UPDATE_ACCEPT,CN_TO_UE_DIRECTION,
                                                 ra_update_accept_msg->update_result, MM_GMM_NO_FAILURE_DEBUG);
                }
#endif
                auth_reset_cache_memory(FALSE,NO_TIMER_EXPIRED, RRC_PS_DOMAIN_CN_ID);    
...
#ifdef FEATURE_NAS_REL12_SPEC_CHANGE
                gmm_sync_pmm_state_during_w_to_lg_and_back_to_w_irat = FALSE;
                MSG_HIGH_DS_0(MM_SUB,"Reset gmm_sync_pmm_state_during_w_to_lg_and_back_to_w_irat");
#endif
            ...
            */
                ...
                ** --------------------- --------------------------*/
                ...
                if ((gmm_ra_update_type == GMM_COMBINED_RA_LA_UPDATING) ||
                    (gmm_ra_update_type == GMM_COMBINED_RA_LA_UPDATING_WITH_IMSI_ATTACH)
#ifdef FEATURE_NAS_REL11
                     ||((mm_nv_nas_rel_compliance >= NV_NAS_COMPLIANCE_REL11)&&(gmm_ra_update_type == GMM_PERIODIC_UPDATING) 
                       && (mm_serving_plmn.ms_op_mode == SYS_SRV_DOMAIN_CS_PS) &&(mm_serving_plmn.network_op_mode == NETWORK_OP_MODE_1)
                   ...
                   )
                {
                     ...
                     )
                  {
                    ...
                    gprs_service_attempt_counter = 0; //cr2488
                    mm_stop_timer(TIMER_T3325);
                    MSG_HIGH_DS_0( MM_SUB, "=MM= gprs_service_attempt_counter is reset");
#endif
                    t3330_timeout_counter   = 0;
...
** ---------------------------- ---------------------------- ----------------------------*/
                      ...
                      ** ---------------------------- ---------------------------- ----------------------------*/
                          ...
                          if (gmm_rau_attempt_counter <= MAX_ATTEMPT_COUNTER_RAU && gmm_rau_attempt_counter > 0 )
                          {
                            gmm_buffer_rej_cause[gmm_rau_attempt_counter-1] = ra_update_accept_msg->gmm_cause;
                          }
                          for (i=0; i<MAX_ATTEMPT_COUNTER_RAU; i++)
                          {
                            if (gmm_rau_attempt_counter == MAX_ATTEMPT_COUNTER_RAU && gmm_buffer_rej_cause[i] == NETWORK_FAILURE)
...
#endif
		  {
...
#endif
                  {
                    ...
                    gmm_initiate_routing_area_update();
                        ...
                        && (mm_checkif_rat_pri_list_has_rat(&mm_rat_pri_list_info,SYS_RAT_LTE_RADIO_ACCESS) == TRUE)) && 
                          (gmm_reg_lte_cs_param.voice_domain_pref !=  gmm_reg_pended_lte_cs_param.voice_domain_pref ))
                {
                  MSG_HIGH_DS_0(MM_SUB, "=MM= Start RAU procedure - change in voice domain pref");
                  gmm_set_state(GMM_REGISTERED);
                  gmm_update_ra_type();
                  emm_ctrl_data_ptr->emm_voice_domain_change_pended = NAS_NON_ACQ_REQ_PROCESSING;
                  gmm_initiate_routing_area_update();
                 ...
                 {/* The (gmm_detach_after_rau_pending) and the above case of
                   ...
                   if(mm_timer_status [ TIMER_T3340 ] == TIMER_ACTIVE && gmm_state == GMM_ROUTING_AREA_UPDATING_INITIATED)
                   {
                     gmm_set_state( GMM_REGISTERED );
                   }
            ...
            case GMM_ROUTING_AREA_UPDATE_REJECT:
              ...
              ** ----------------------------------------------------------- */
              ...
              {
#ifdef FEATURE_QSH_EVENT_METRIC
                gmm_qsh_ota_metric_signaling_log(GMM_ROUTING_AREA_UPDATE_REJECT,CN_TO_UE_DIRECTION,
                                                             0xff, (mm_gmm_reject_cause_debug_type)ra_update_reject_msg.gmm_cause);
#endif
                if ((((gmm_get_map_rej_cause7_to_rej_cause14()) 
                     && (!gmm_is_camped_on_hplmn())) || (gmm_get_gsm_map_rej_hplmn_cause7_to_rej_cause14())) 
                     && (ra_update_reject_msg.gmm_cause == GPRS_SERVICES_NOT_ALLOWED))
                { //CMCC GSM Rej7
               ...
               if (gmm_set_drx_req_pending == NAS_DRX_CFG_REQ_PROCESSING)
               {
                ...
                switch (ra_update_reject_msg.gmm_cause) 
                {
                  ...
                  case GPRS_SERVICES_AND_NON_GPRS_SERVICES_NOT_ALLOWED:
                    ...
                    if ((mm_nv_nas_rel_compliance >= NV_NAS_COMPLIANCE_REL12) &&
                        (ra_update_reject_msg.gmm_cause == GPRS_SERVICES_AND_NON_GPRS_SERVICES_NOT_ALLOWED) && 
                          is_umts_mode( )) //cr2576
                    {
                      mm_start_timer(TIMER_T3340,DEFAULT_TIMEOUT);
                    }
                 ...
                 if (gmm_is_dos_mitigation_required_for_ps_domain(GMM_ROUTING_AREA_UPDATE_REJECT, ra_update_reject_msg.gmm_cause))
                 {
                    gmm_perform_dos_mitigation_action(GMM_ROUTING_AREA_UPDATE_REJECT, ra_update_reject_msg.gmm_cause);
                 }
                 else
                 {
                    if (mm_serving_plmn.ms_op_mode == SYS_SRV_DOMAIN_CS_PS)
...
#ifdef FEATURE_DUAL_SIM
                    mm_gmm_set_illegal_non_illegal_cause(ra_update_reject_msg.gmm_cause);
#endif
                    if (mm_reg_waiting_for_reg_cnf)
                    {
                      mmcoord_mmr_reg_cnf.service_state = mm_serving_plmn.info;
                  ...
                  case MS_IDENTITY_CANNOT_BE_DERIVED_BY_THE_NETWORK:
                    ...
                    */
                    if (mm_reg_waiting_for_reg_cnf)
                    {
                     ...
                     else if ((gmm_manual_mode_user_selection_procedure == FALSE) &&
  ...
  */
                    ...
                    */
                    ...
                    else
                    {
                      ...
                      {
                         mmr_service_ind.cause = mm_convert_to_reg_cause(ra_update_reject_msg.gmm_cause );
                      }
                      if ((mm_held_cnm_service_req.present == TRUE || mm_state == MM_WAIT_FOR_RR_CONNECTION_MM) &&
                          (mm_held_cnm_service_req.establishment_cause == (int) MM_RR_EMERGENCY_CALL) &&
                          (ra_update_reject_msg.gmm_cause == NATIONAL_ROAMING_NOT_ALLOWED ||
                  ...
                  default:
                    ...
                    ** ---------------------------------------------------------- */
                    ...
                    (
                      ...
                      if (gmm_rau_attempt_counter <= MAX_ATTEMPT_COUNTER_RAU && gmm_rau_attempt_counter > 0 )
                      {
                        gmm_buffer_rej_cause[gmm_rau_attempt_counter-1] = ra_update_reject_msg.gmm_cause;
                      }
                      for (i=0; i<MAX_ATTEMPT_COUNTER_RAU; i++)
                      {
                        if (gmm_rau_attempt_counter == MAX_ATTEMPT_COUNTER_RAU && gmm_buffer_rej_cause[i] == NETWORK_FAILURE)
                    ...
                    if (((ra_update_reject_msg.gmm_cause >=
                         RETRY_UPON_ENTRY_INTO_A_NEW_CELL_MIN_VALUE) &&
                         (ra_update_reject_msg.gmm_cause <=
                         RETRY_UPON_ENTRY_INTO_A_NEW_CELL_MAX_VALUE)) &&
 ...
 /* Check if GMM is in Limited Service */
  if ((gmm_substate == GMM_LIMITED_SERVICE)||
      ...
      ((gmm_state == GMM_REGISTERED_INITIATED)&& (
	   ...
	   (gmmllc_get_glif_grr_service_req_state() == GLIF_SERVICE_REQUESTED)||
#endif
       (is_umts_mode() && (rrc_connection_status == MMRRC_EST_IN_PROGRESS))))
#ifdef FEATURE_LTE
     ...
     )
  {
    ...
    if ((message->cmd.hdr.message_id == (int)MMR_REG_REQ) &&
       ...
       (is_umts_mode() && (gmm_state == GMM_REGISTERED_INITIATED) && (rrc_connection_status == MMRRC_EST_IN_PROGRESS)))
    {
...
#endif
  else if (mm_waiting_for_stop_mode_cnf == TRUE && gmm_gprs_detach_pending == TRUE)
  {
    MSG_HIGH_DS_0( MM_SUB,"=MM= Do not perform detach as stop cnf is pending");
  }
       ...
       ))
  {
    ...
    */
    ...
    if(gmm_detach_type == GMM_COMBINED_GPRS_IMSI_DETACH || gmm_detach_type == GMM_GPRS_DETACH)
    {
      gprs_attach_upon_entry_to_a_new_cell = FALSE;
      gmm_rau_upon_entry_to_a_new_cell = FALSE;
    }
    ...
    ** -------------------------------------------- */
    ...
    else
    {
      ...
      ** --------------------------------------------------------------------------- */
      if(mm_timer_status[TIMER_T3321] == TIMER_ACTIVE)
      {
        MSG_HIGH_DS_0( MM_SUB,"=MM= Retrying detach within first expiry of T3321, so don't restart it");
      }
      else
      {
...
void gmm_process_detach_request_cause( gmm_mt_detach_request_msg_T *detach_request_msg_p )
{
    ...
    ** ------------------------------------------------------------------ */
    if ((gmm_get_map_rej_cause7_to_rej_cause14())
         && detach_request_msg_p->gmm_cause == GPRS_SERVICES_NOT_ALLOWED 
         && (!gmm_is_camped_on_hplmn()))
    {
      detach_request_msg_p->gmm_cause = GPRS_SERVICES_NOT_ALLOWED_IN_THIS_PLMN;
    }
...
#endif
      gmm_reject_cause = detach_request_msg_p->gmm_cause; 
          ...
          if (mm_serving_plmn.info.reg_service_domain == SYS_SRV_DOMAIN_CS_PS)
          {
            ...
            ** -------------------------------------------------------- */
...
#endif
            {
              mmsim_mark_sim_as_illegal();
#ifdef FEATURE_DUAL_SIM
              mm_gmm_set_illegal_non_illegal_cause(detach_request_msg_p->gmm_cause);
#endif
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (gmm_valid_mt_detach_request_message( message, &detach_request_msg ))
  {
    ...
    {
      gmm_qsh_ota_metric_signaling_log(GMM_DETACH_REQUEST,CN_TO_UE_DIRECTION,
                             detach_request_msg.detach_type, (mm_gmm_reject_cause_debug_type)detach_request_msg.gmm_cause);
    }
    else
    {
      gmm_qsh_ota_metric_signaling_log(GMM_DETACH_REQUEST,CN_TO_UE_DIRECTION,
                                       detach_request_msg.detach_type, MM_GMM_NO_FAILURE_DEBUG);
    }
#endif
#ifdef FEATURE_GSM_GPRS
    if (is_gsm_mode())
    ...
    else if (gmm_state == GMM_ROUTING_AREA_UPDATING_INITIATED)
    {
      ...
      ** ---------------------------------------------------- */
      ...
      /* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
      **24.008: 4.7.5.1.5(g)If the DETACH REQUEST message contains detach type "re-attach not required" and GMM cause #2 "IMSI unknown in HLR", the MS will follow the procedure as described below for the detach type "IMSI detach".
      ...
      **-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
#ifdef FEATURE_NAS_REL9
      if(detach_request_msg.detach_type == GMM_REATTACH_NOT_REQUIRED && detach_request_msg.gmm_cause == IMSI_UNKNOWN_IN_HLR)
      {
        MSG_HIGH_DS_0( MM_SUB, "=MM= Detach request is ignored as it is received with type re-attach not required and cause #2 IN RAU_INITIATED state") ;
      }
      else 
#endif
      if ((detach_request_msg.detach_type == GMM_REATTACH_REQUIRED) ||
...
void gmm_handle_gprs_detach( mm_cmd_type *message )
{
  ...
  sm_cmd_type   *sm_cmd;
#ifdef FEATURE_DUAL_SIM
  boolean mm_send_new_proc_type = FALSE;
#endif
  mmr_service_ind_s_type     mmr_service_ind;
...
#ifdef FEATURE_DUAL_SIM
                mm_proc_status_set(SYS_PROC_TYPE_PS_REGISTRATION, FALSE);
               if((mm_evaluate_proc_type() == SYS_PROC_TYPE_NONE) && (is_wcdma_mode()))
               {
                 mm_send_new_proc_type = TRUE;
               }
#endif
#ifdef FEATURE_GSM_GPRS
                if (is_gsm_mode())
...
#ifdef FEATURE_NAS_ECALL  
                      if (mm_ecall_mode_type == MMR_ECALL_MODE_ECALL_ONLY &&
                          gmm_need_to_perform_ecall_inactivity == TRUE 
                         )
                      {
                         mm_send_new_proc_type = FALSE; 
                         gmm_set_state( GMM_DEREGISTERED );
                ...
                {
                 /*Iif detach accept is received in release in progress , dont send status message */
                  if (is_gsm_mode() || rrc_connection_status != MMRRC_RELEASE_IN_PROGRESS)
                  {
                      mm_send_new_proc_type = FALSE; 
      ...
      switch (message->cmd.timer_expiry.timer_id)
      {
        case TIMER_T3321:
          ...
          ** ---------------------------------------------------------------- */
          ...
          if (mm_ps_detach_type == SYS_PS_DETACH_TYPE_POLICY_CHANGE)
          {
              t3321_timeout_counter = 5;
              MSG_HIGH_DS_0(MM_SUB, "=MM= T3321 timer counter set to 5 due to policy change \n");
          }
          MSG_HIGH_DS_1(MM_SUB, "=MM= T3321 timeout counter = %d", t3321_timeout_counter);
#ifdef FEATURE_DUAL_SIM
...
#endif
          if (gmm_abort_ps_detach &&
              (mm_ps_detach_state == MM_PS_DETACH_STATE_WAITING_TO_SEND_CNF ||
               mm_ps_detach_state == MM_PS_DETACH_STATE_DETACH_REQ_PENDING))
          {
            MSG_HIGH_DS_0(MM_SUB, "=MM= Perform local detach due to high priority user action");
            t3321_timeout_counter = 5;
          }
...
#ifdef FEATURE_DUAL_SIM
   if(mm_send_new_proc_type == TRUE && mm_evaluate_proc_type()==SYS_PROC_TYPE_NONE)
      {
         mm_send_procedure_ind(SYS_PROC_TYPE_NONE_DELAYED);
         mm_send_new_proc_type = FALSE;
      }
      else
      {
...
void gmm_update_ra_type(void)
{
  ...
  else 
  {
    if ((mm_serving_plmn.ms_op_mode == SYS_SRV_DOMAIN_CS_PS) &&
        ...
        (mmsim_sim_valid(SYS_SRV_DOMAIN_CS_ONLY)))
    {
      if(mm_possible_imsi_attach || gmm_pending_imsi_attach)
      {
        gmm_ra_update_type = GMM_COMBINED_RA_LA_UPDATING_WITH_IMSI_ATTACH;    
      }
      else
             ...
             ((((mmcoord_pending_reg_message.cmd.mmr_reg_req.network_selection_mode == 
              ...
              (is_cell_forbidden
               ...
               && !(is_manual_csg_selection_success)
                ...
                (mmcoord_pending_reg_message.cmd.mmr_reg_req.message_header.message_set == MS_MM_REG &&
...
#endif
#ifdef FEATURE_NAS_REL12_SPEC_CHANGE
                 /* Spec 24.008 4.7.5.1 - CR 2553: Trigger RAU in Iu mode to sync PMM state */
                 && !(is_umts_mode() && (gmm_sync_pmm_state_during_w_to_lg_and_back_to_w_irat == TRUE))
#endif
#ifdef FEATURE_DSAC
...
#ifdef FEATURE_NAS_REL12_SPEC_CHANGE
          /* ---------------------------- ---------------------------- ---------------------------- ----------------------------
          ** Spec 24.008 4.7.5.1 - CR 2553:
          ** If the MS performs an inter-system change from PMM-CONNECTED mode in Iu mode to A/Gb mode or S1 mode not due to PS handover, 
          ** and subsequently performs an intersystem change back to Iu mode without sending a RAU message while in A/Gb mode or a TAU message
          ** while in S1 mode, the MS shall initiate a routing area updating procedure.
          ** ---------------------------- ---------------------------- ---------------------------- ----------------------------*/
          if(is_umts_mode() && (gmm_sync_pmm_state_during_w_to_lg_and_back_to_w_irat == TRUE))
          {
            MSG_HIGH_DS_0(MM_SUB,"=MM= W to LorG and back to W IRAT when UE was in PMM connected, RAU Would trigger");
          }
          else
#endif
          {
              ...
              else
              {
                ...
                ** --------------------------------------------------------------- */
                ...
                for(mm_other_as_id = SYS_MODEM_AS_ID_1 ; mm_other_as_id < MAX_NAS_STACKS ; mm_other_as_id++)
                {
                 ...
                 /*If RAI has changed and detach confirm is pending on current sub due to dds switch , force a local detach abandon RAU*/
                 if((mm_ps_detach_state == MM_PS_DETACH_STATE_WAITING_TO_SEND_CNF
                 	...
                 	|| MM_IS_LPLUSL_MODE
                         ...
                         (gmm_force_local_detach_sim[mm_sub_id] == TRUE))
                  {           
                    MSG_HIGH_DS_1(MM_SUB,"=MM= Local detach on SUB %d",mm_sub_id+1);
                    gmm_handle_gprs_detach_low_level_failure();
                    mmsend_nas_reg_event_packet(PROC_NAS_MO_DETACH_EVENT,PROC_END,PROC_END_LOWER_LAYER_FAILURE,MT_DETACH_TYPE_NONE,MT_DETACH_CAUSE_NONE);
                    ps_dreg_started = TRUE;
                  }
...
/*4.7.1.7a Intersystem change from S1 mode to A/Gb mode or S1 mode to Iu mode with ISR activated*/
...
#endif /*defined (FEATURE_GSM) || defined (FEATURE_WCDMA)*/
#endif /* #ifdef FEATURE_GSM_GPRS */ 
          if((gprs_attach_upon_entry_to_a_new_cell == TRUE) &&
              (gmm_state == GMM_DEREGISTERED) &&
              (gmm_substate == GMM_ATTEMPTING_TO_ATTACH) &&
              ((msg_p->cmd.hdr.message_id == (int) RR_SERVICE_IND &&
                ...
                (((msg_p->cmd.hdr.message_id == (int) RRC_SERVICE_IND ||
                  ...
                  ((mm_serving_cell_valid)&&
                   memcmp(&mm_gmm_reg_failed_cell_id,&mm_serving_cell_id,sizeof(uint32)) != 0))
    #ifdef FEATURE_NAS_REL10
                   && ((mm_timer_status[TIMER_T3346] != TIMER_ACTIVE)
    #ifdef FEATURE_NAS_REL11
                   ||(mm_is_rrc_access_class_11_to_15() == TRUE)
          ...
          {
            MSG_HIGH_DS_0(MM_SUB,"=MM= Cell is changed, ATTACH needs to be triggered because of this cell change");
            gmm_initiate_gprs_attach();
          }
          else if(gmm_state == GMM_DEREGISTERED &&
                  gmm_substate == GMM_NORMAL_SERVICE &&
          ...
          else if((gmm_rau_upon_entry_to_a_new_cell == TRUE) &&
                   (gmm_state == GMM_REGISTERED) &&
                   (gmm_substate == GMM_ATTEMPTING_TO_UPDATE) &&
                   ((msg_p->cmd.hdr.message_id == (int) RR_SERVICE_IND &&
                      msg_p->cmd.rr_service_ind.cell_changed) || 
          ...
          {
            MSG_HIGH_DS_0(MM_SUB,"=MM= Cell is changed, RAU needs to be triggered because of this cell change");
            gmm_initiate_routing_area_update();
          }

#ifdef FEATURE_LTE
    if (gmm_is_cap_changed() &&
        (gmm_state == GMM_REGISTERED) &&
        (gmm_substate == GMM_NORMAL_SERVICE))
    {
    ...
    {           
      MSG_HIGH_DS_1(MM_SUB, "=MM= DRX(%d) Set cfg params req pending, Start RAU procedure ",
                                                                   gmm_set_drx_req_pending);
      gmm_update_ra_type();
      gmm_initiate_routing_area_update();
    }
...
#ifdef FEATURE_NAS_REL10
            if ((gmm_state == GMM_REGISTERED || gmm_state == GMM_DEREGISTERED) && 
                (mm_timer_status[TIMER_T3346] == TIMER_ACTIVE) &&
                (gmm_is_plmn_equivalent_t3346() == FALSE) &&
                (gmm_substate == GMM_ATTEMPTING_TO_ATTACH ||
                 gmm_substate == GMM_ATTEMPTING_TO_UPDATE) &&
                ((gmm_plmn_changed( gmm_stored_rai.plmn_id, mm_serving_plmn.info.plmn )) ||
                 (gmm_rai_changed( gmm_stored_rai.location_area_code,
                ...
                (mmcoord_pending_reg_message.cmd.mmr_reg_req.network_selection_mode !=
                    SYS_NETWORK_SELECTION_MODE_LIMITED))
            {
                if ((gmm_state == GMM_DEREGISTERED) && (gmm_substate == GMM_ATTEMPTING_TO_ATTACH))
                {
                   gmm_initiate_gprs_attach();
                }
                else if ((gmm_state == GMM_REGISTERED) && (gmm_substate == GMM_ATTEMPTING_TO_UPDATE))
                {
                   gmm_update_ra_type();
                   gmm_initiate_routing_area_update();
                }
...
void gmm_perform_ecall_inactivity(mm_cmd_type *message )
{
           ...
           (gmm_state == GMM_SERVICE_REQUEST_INITIATED))
  {
    ...
    if(gmm_state == GMM_DEREGISTERED_INITIATED && mm_ecall_mode_type == MMR_ECALL_MODE_ECALL_ONLY) 
    {
      gmm_need_to_perform_ecall_inactivity = TRUE;
    }
    else if (mm_ecall_mode_type == MMR_ECALL_MODE_ECALL_ONLY && ((mm_serving_plmn.ms_op_mode == SYS_SRV_DOMAIN_PS_ONLY) ||
            ((mm_serving_plmn.ms_op_mode == SYS_SRV_DOMAIN_CS_PS) &&
              ((mm_serving_plmn.network_op_mode == NETWORK_OP_MODE_1) || (mm_serving_plmn.network_op_mode == PS_DOMAIN_SYS_INFO_UNAVAILABLE)))))
    {
...
void gmm_handle_set_drx_req(mm_cmd_type *message )
{
  ...
  MSG_HIGH_DS_2(MM_SUB, "=MM= Rx DRX value (%d)  Rx DRX RAT = %d", message->cmd.mmr_set_drx_req.drx_coefficient,message->cmd.mmr_set_drx_req.active_rat);
  ...
  else if ((message->cmd.mmr_set_drx_req.drx_coefficient != SYS_DRX_NOT_SPECIFIED_BY_MS) &&
           (message->cmd.mmr_set_drx_req.drx_coefficient < SYS_DRX_CN_COEFFICIENT_S1_CN6_T32 ||
            message->cmd.mmr_set_drx_req.drx_coefficient > SYS_DRX_CN_COEFFICIENT_S1_CN9_T256))
  {
      MSG_HIGH_DS_1(MM_SUB, "=EMM= Invalid DRX value (%d) recevied, Rejecting DRX_SET_REQ ",
                    message->cmd.mmr_set_drx_req.drx_coefficient);
      mm_send_mmr_set_drx_cnf(FALSE);
  }
  else if (message->cmd.mmr_set_drx_req.client_type == CM_CLIENT_TYPE_DS)
  {
     //emm_ctrl_data_ptr->emm_rat_drx_param_to_be_sent = NAS_SEND_DRX_ON_ALL_RATS;
     (void)gmm_process_paging_cycle_selector(message->cmd.mmr_set_drx_req.drx_coefficient,message->cmd.mmr_set_drx_req.client_type);
     mm_send_mmr_set_drx_cnf(TRUE);
  }
  else if ((gmm_state != GMM_REGISTERED || mm_serving_plmn.gmm_service_state == SERVICE_NOT_AVAILABLE) &&
           ( message->cmd.mmr_set_drx_req.active_rat == SYS_RAT_MAX))
  {
  ...
  else if ((message->cmd.mmr_set_drx_req.active_rat != SYS_RAT_MAX) &&
           (message->cmd.mmr_set_drx_req.active_rat != mm_serving_plmn.info.active_rat))
  {
    MSG_HIGH_DS_0(MM_SUB,"Rejecting MCS DRX Update received with non-active RAT ");
    mm_send_mmr_set_drx_cnf(FALSE);
  }
  else 
  {
    if (message->cmd.mmr_set_drx_req.active_rat == SYS_RAT_MAX ||
        ((message->cmd.mmr_set_drx_req.active_rat == mm_serving_plmn.info.active_rat) &&
         ((message->cmd.mmr_set_drx_req.active_rat ==  SYS_RAT_GSM_RADIO_ACCESS) ||
          (message->cmd.mmr_set_drx_req.active_rat ==  SYS_RAT_UMTS_RADIO_ACCESS) ||
          (message->cmd.mmr_set_drx_req.active_rat ==  SYS_RAT_TDS_RADIO_ACCESS)) &&
         (gmm_drx_coeff_client_rat != SYS_RAT_MAX)))
    {
          ...
          (message->cmd.mmr_set_drx_req.active_rat != SYS_RAT_MAX))
      {
        ...
        if (message->cmd.mmr_set_drx_req.active_rat != SYS_RAT_NONE)
        {
          gmm_drx_coeff_client_rat = message->cmd.mmr_set_drx_req.active_rat;
        }
   ...
   {
      MSG_HIGH_DS_1(MM_SUB,"Rejecting SET_DRX_REQ received from MCS when QMI has already set valid value %d", gmm_drx_coeff_client_rat);
      mm_send_mmr_set_drx_cnf(FALSE);
   }
  }
 }
...
void gmm_handle_registration( mm_cmd_type *message )
{
  ...
  boolean is_combined_rau_needed = FALSE;
  mmr_service_ind_s_type     mmr_service_ind;
                       ...
                       ((gmm_state == GMM_ROUTING_AREA_UPDATING_INITIATED) && gmm_detach_after_rau_pending))
              {
				...
				if ((gmm_state == GMM_DEREGISTERED) &&
				 ((gmm_detach_type == GMM_GPRS_DETACH) || 
				  (gmm_detach_type == GMM_COMBINED_GPRS_IMSI_DETACH)))
				 {
				   if ((mm_serving_plmn.info.reg_service_domain == SYS_SRV_DOMAIN_CS_PS) && (gmm_detach_type == GMM_GPRS_DETACH))
				   {
					gmm_update_mm_serving_plmn_info( mm_serving_plmn.info.sim_state,
                                       SYS_SRV_DOMAIN_CS_ONLY,
                                       FALSE,
      ...
      */
                ...
                else if (is_umts_mode() && (mm_state != MM_IDLE))
                {
                  ...
                  else
                  {
					...
					if((message->cmd.mmr_reg_req.req_service_domain == SYS_SRV_DOMAIN_CS_PS)&&
						((MM_IS_NON_DDS_SUB) ||
						 (mm_state == MM_WAIT_FOR_NETWORK_COMMAND  && mm_timer_status [TIMER_T3240] == TIMER_ACTIVE) ||
						 (mm_state == MM_WAIT_FOR_RR_CONNECTION_LU) || 
						 (mm_state == MM_LOCATION_UPDATE_INITIATED)))
					{
					   mm_change_service_domain();
              ...
              else
              {
                gmm_set_substate( GMM_NO_IMSI );
                   ...
                   #ifdef FEATURE_DUAL_DATA
                    &&
                  ((message->cmd.mmr_reg_req.volte_only == FALSE) ||(mm_serv_change_pending == TRUE))
                   #endif 
                  )
          {
            if (((mm_serving_plmn.info.reg_service_domain == SYS_SRV_DOMAIN_PS_ONLY) ||
            ...
            {
#if defined (FEATURE_GSM) || defined (FEATURE_WCDMA)
#ifndef FEATURE_NAS_REL12_SPEC_CHANGE
      ...
      switch (message->cmd.timer_expiry.timer_id)
      {
        case TIMER_T3310:
          if (gmm_state == GMM_REGISTERED_INITIATED)
          {
           ...
           {
             MSG_HIGH_DS_0(MM_SUB,"Ignore T3310 expiry as PS detach will be processed");
             break;
           }
            /* Check blocking restrictions */
        ...
        case TIMER_T3311:
		  ...
		  else if(mm_is_lai_forbidden_due_to_dos_mitigation((inter_task_lai_T *) &gmm_current_lai))
          {
			  MSG_HIGH_DS_0( MM_SUB, "=MM= LAI forbidden due to DOS mitigation" );
          }
          else if ((gmm_state == GMM_DEREGISTERED) &&
          ...
          else if (message->cmd.timer_expiry.timer_id == TIMER_T3346 &&
          ...
          {
            MSG_HIGH_DS_0( MM_SUB,"=MM= Ignoring T3346 Timer expiry when international data roaming is OFF" );
          }
          else if ((gmm_state == GMM_DEREGISTERED) &&
                   (gmm_substate == GMM_ATTEMPTING_TO_ATTACH))
          {
...
** ---------------------------- ---------------------------- ---------------------------- ----------------------------*/
            ...
            else
            {
              ...
              /*If periodic rau timer expires while 3311 is running and only PS service is available instead of the expected cs_ps, combined rau is pended to reestablish after release*/
              if ((mm_serving_plmn.ms_op_mode == SYS_SRV_DOMAIN_CS_PS) &&
                  (mm_serving_plmn.info.reg_service_domain == SYS_SRV_DOMAIN_PS_ONLY)&&
                  (mm_serving_plmn.network_op_mode == NETWORK_OP_MODE_1) && 
                  (mm_gs_association_established == TRUE) &&
...
FUNCTION GMM_PROCESS_REFRESH_STAGE_IND
...
===========================================================================*/
void gmm_process_refresh_stage_ind
( 
   mm_cmd_type *message
)
{
  if ( (message->cmd.mmr_refresh_stage_ind.refresh_stage == MMGSDI_REFRESH_STAGE_WAIT_FOR_OK_TO_INIT) &&
       (mm_timer_status[TIMER_T3321] == TIMER_ACTIVE) )
  {
    /* GPRS detach procedure will be aborted on the next TIMER_T3321 expiry
       for refresh stage WAIT_FOR_OK_TO_INIT
     */
    t3321_timeout_counter = 5;
  }


  MSG_HIGH_DS_3(MM_SUB,"=MM= refresh stage %d, T3321 timer status %d, T3321 timeout counter %d",
                        message->cmd.mmr_refresh_stage_ind.refresh_stage,
                        mm_timer_status[TIMER_T3321],
                        t3321_timeout_counter);
}
