...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/mm/src/mmutils.c#3 $    $DateTime: 2023/10/08 15:22:21 $ $Author: pwbldsvc $
...
03/02/05   sm      Corrected the call to nas_event_report_payload in mmsend_rat_change_event
...
static byte mm_lu_last_endreason_sim[MAX_AS_IDS];
#define mm_lu_last_endreason mm_lu_last_endreason_sim[mm_as_id]
#else
static byte mm_lu_last_endreason;
#endif

#ifdef FEATURE_DUAL_SIM
extern boolean  mm_gmm_always_on_data_enable_sim[MAX_AS_IDS];
#define mm_gmm_always_on_data_enable mm_gmm_always_on_data_enable_sim[mm_sub_id]
#else
extern boolean  mm_gmm_always_on_data_enable;
#endif
#ifdef FEATURE_LTE
...
#if defined FEATURE_DUAL_SIM  
extern uint8   mm_emm_is_t3247_enabled_sim[MAX_AS_IDS];
#define mm_emm_is_t3247_enabled mm_emm_is_t3247_enabled_sim[mm_sub_id]
...
/* Denotes SIM swap event is processed by one by task (REG/MM) and pending at other task (MM/REG) */
#if defined(FEATURE_TRIPLE_SIM) || defined(FEATURE_SGLTE_DUAL_SIM)
boolean sim_swap_event_handled_by_only_one_task_sim[MAX_NAS_STACKS] = {FALSE, FALSE, FALSE};
#elif defined(FEATURE_DUAL_SIM) || defined(FEATURE_SGLTE)
boolean sim_swap_event_handled_by_only_one_task_sim[MAX_NAS_STACKS] = {FALSE, FALSE};
#endif
...
boolean mm_is_ss_request_rrc_sent_sim[MAX_NAS_STACKS];
#define mm_is_ss_request_rrc_sent mm_is_ss_request_rrc_sent_sim[mm_as_id]
#else
extern boolean        mm_stop_mode_local_detach_peding;
extern boolean mmcoord_svc_cnf_pending;
boolean mm_is_ss_request_rrc_sent;
#endif

sys_modem_as_id_e_type mm_as_id_rrc_abnormal_end_ind;

#define EVENT_NAS_RPT_EXT_VERSION 1
...
extern ULogHandle mm_get_ulog_handle(void);
...
void log_util_log_packet_commit(sys_modem_as_id_e_type as_id, void* ptr);
...
FUNCTION NAME
   ...
   mm_is_single_imei_behavior_applicable
 
DESCRIPTION 
   This function is used to check if a single imei behavior is applicable 
   or not based on the below requirement
   
     Dual IMEI behavior - Default behavior, IMEI to corresponding SIM returned.
     Single IMEI behavior - If both subs camped on (China) TDS, pass single IMEI
                            from NV context 0:
        1. Sub camped on (China) TDS network
        2. Device is a Single IMEI( operator subsidized phone)
   
     Note: Since it is not feasible to check what PLMN the other sub is camped on,
           always return IMEI from NV context 0  when the current sub is camped on
           TDS PLMN
  
DEPENDENCIES
  Policyman XML and api support is needed to detect if UE is configures for single imei

RETURN VALUE
  TRUE  - If Single IMEI behavior is required
  FALSE - otherwise
...
==============================================================================*/
boolean mm_is_single_imei_behavior_applicable
(
  sys_plmn_id_s_type plmn
)
{
  boolean result = FALSE;
  const char  *BooleanName = "nas:SingleIMEI";
  boolean     value;

   /* check if UE is configured for single imei */
  if (policyman_get_subs_boolean_value(BooleanName, (sys_modem_as_id_e_type)mm_sub_id, &value))
  {
    /* check if current camped plmn is TDS PLMN */
    if (reg_mode_is_plmn_td_compliant (plmn))
    {
       result = value;
    }
    MSG_HIGH_DS_1(MM_SUB, "=MM= Is PLMN TDS complaint %d", result);
  }
...
/*====================================================================
Function: gmm_per_subs_always_on_data_enabled

Description: return mm_gmm_always_on_data_enable value

======================================================================*/
#ifdef FEATURE_DUAL_SIM
boolean gmm_per_subs_always_on_data_enabled(sys_modem_as_id_e_type as_id)
{
	return mm_gmm_always_on_data_enable_sim[as_id];
}
#endif

void mm_rrc_est_cause_sent_for_call_set(boolean value)
{
	mm_rrc_est_cause_sent_for_call=value;
}

boolean mm_rrc_est_cause_sent_for_call_get(void)
{
	return mm_rrc_est_cause_sent_for_call;
}

/*===========================================================================
Function: mm_read_imeisv
...
)
{
  ...
  memset((void *)imeisv, 0x0, MAX_IMEI_LEN + 2);

#if defined(FEATURE_DUAL_SIM) && defined(FEATURE_DUAL_IMEI)
#ifdef FEATURE_NAS_SINGLE_IMEI
  sub_id_local = mm_sub_id_stack[mm_sim_get_as_id_from_slot(MMGSDI_SLOT_1)];
  if ( mm_is_single_imei_behavior_applicable(mm_serving_plmn.info.plmn) ) 
  {
...
void mm_read_imei( byte * write_ptr )
{
    ...
    boolean is_single_imei = FALSE;
    if ((write_ptr != NULL) && (mm_is_single_imei_behavior_applicable(mm_serving_plmn.info.plmn) ))
    {
       is_single_imei = TRUE;
       as_id_local = mm_as_id;
       mm_set_as_id(mm_sim_get_as_id_from_slot(MMGSDI_SLOT_1));
    }
#endif
    ...
    case MS_MM_REG :
      {
        switch (primitive->message_id)
        {
          ...
          case MMR_REG_REJECT_IND:
            ...
            mm_msg_info = (byte)(((mmr_reg_reject_ind_s_type*)primitive)->reject_cause);
            break;
          ...
          case MMR_ME_BLOCKED_STATUS_IND:
            break;
#ifdef FEATURE_LTE
          case MMR_NON_ACQ_UPDATE_IND:
            ((mmr_non_acq_update_ind_s_type*)primitive)->as_id = (sys_modem_as_id_e_type)mm_as_id;
            break;
#endif
        ...
        switch (primitive->message_id)
        {
          ...
          case RR_PLMN_LIST_REQ:
            ((rr_plmn_list_req_T*)primitive)->as_id = (sys_modem_as_id_e_type)mm_sub_id;
             mm_msg_info = (byte)(((rr_plmn_list_req_T*)primitive)->trans_id);
            break;
...
#if (defined(FEATURE_DUAL_SIM) && defined(FEATURE_LTE))
            ((rr_irat_allowed_status_ind_T*)primitive)->as_id = (sys_modem_as_id_e_type)mm_sub_id;
#endif
            break;
#ifdef FEATURE_DUAL_SIM
         ...
         case RR_SERVICE_STATUS_IND:
          ...
          case RR_PROCEDURE_IND:
            ((rr_procedure_ind_T*)primitive)->as_id = (sys_modem_as_id_e_type)mm_sub_id;
            mm_msg_info = (byte)(((rr_procedure_ind_T*)primitive)->proc_type);
            break;

          case RR_DDS_SUB_IND:/* AS ID will be set as part of payload */
            mm_msg_info = (byte)(((rr_dds_sub_ind_T*)primitive)->as_id);
            break;
    ...
    case MS_CC_MM_PS :
      ...
      case MS_CM_MM :
      {
        switch (primitive->message_id)
        {
          ...
          case CM_MM_LTE_SUSPEND_CNF: 
            ((cm_mm_lte_suspend_cnf_s_type*)primitive)->as_id = (sys_modem_as_id_e_type)mm_sub_id;
            break; 
#endif
...
void mm_process_diagnostic(mm_cmd_type *msg_p)
{
   ...
   mm_status_msg_T *mm_status = (mm_status_msg_T *)((void *)&msg_p->cmd.hdr);
#else
   (void) msg_p;      /* avoid lint warning */
#endif
   MSG_HIGH_DS_0(MM_SUB,"=MM= MM_STATUS message received");
#ifdef FEATURE_QSH_EVENT_METRIC
   mm_qsh_ota_metric_signaling_log(MM_STATUS,CN_TO_UE_DIRECTION,0xff,(mm_gmm_reject_cause_debug_type)mm_status->reject_cause);
#endif
}
...
#if defined(FEATURE_LTE)
  nv_parm_msg.nv_g2l_support_in_ptm_mode = gmm_nv_geran_to_eutra_measurement_support;
#else
  nv_parm_msg.nv_g2l_support_in_ptm_mode = 0;
#endif
   ...
   if ( mm_held_cnm_service_req.present != FALSE )    /* contains a message */
   {
      ...
      if ((( mm_location_information.location_update_status == UPDATED )&&
          ...
          ((mm_held_cnm_service_req.establishment_cause == (int)MM_RR_EMERGENCY_CALL)&&
           (sd_ss_current_mcc_match_mcc_list_emerg_call_full_srv(&mm_serving_plmn.info.plmn, 
            ...
            * --------------------------------------------*/
            if ((rrc_connection_status == MMRRC_ACTIVE) && (mm_state != MM_IDLE))
            {
               if ((mm_follow_on_proceed)&& (mm_state == MM_LOCATION_UPDATE_INITIATED))
               {
                   ...
                   if (mm_held_cnm_service_req.establishment_cause == (int)MM_RR_SUPPLEMENTARY_SERVICE)
                   {
...
FUNCTION  mm_qsh_ota_metric_signaling_log
...
===========================================================================*/
void mm_qsh_ota_metric_signaling_log
(
  uint8                  message_id,
  uint8                  direction,
...
)
{
  ...
  metric_sig.protocol_discriminator = PROTOCOL_DISCRMINATOR_MM_DEBUG;
  metric_sig.message_type.mm_message_id = 
             (mm_message_id_debug_type)message_id;
  metric_sig.cause_value.mm_cause = mm_cause;
...
FUNCTION  MM_EVALUATE_PROC_TYPE

DESCRIPTION
  This function returns the highest procedure running in NAS
...
===========================================================================*/
sys_proc_type_e_type mm_evaluate_proc_type(void)
{

  sys_proc_type_e_type  proc_type = mm_ds_proc_type;

  if((mm_proc_status[SYS_PROC_TYPE_TEST_LOOPBACK] == TRUE) &&
      (is_umts_mode()))
  {
    proc_type = SYS_PROC_TYPE_TEST_LOOPBACK;
  }
  else if(mm_ds_proc_type == SYS_PROC_TYPE_HIGH_PRIORITY_PS_CALL)
  {
    proc_type = SYS_PROC_TYPE_HIGH_PRIORITY_PS_CALL;
  }
  else if(mm_proc_status[SYS_PROC_TYPE_CS_CALL] == TRUE)
  {
    proc_type = SYS_PROC_TYPE_CS_CALL;
  }
  else if(mm_proc_status[SYS_PROC_TYPE_SUPPLEMENTARY_SERVICE] == TRUE)
  {
    proc_type = SYS_PROC_TYPE_SUPPLEMENTARY_SERVICE;
  }
  else if((mm_proc_status[SYS_PROC_TYPE_HIGH_PRIORITY_PS_CALL] == TRUE) &&
          (is_umts_mode()))
  {
    proc_type = SYS_PROC_TYPE_HIGH_PRIORITY_PS_CALL;
  }
  else if(mm_proc_status[SYS_PROC_TYPE_SMS] == TRUE)
  {
    proc_type = SYS_PROC_TYPE_SMS;
  }
  else if(mm_proc_status[SYS_PROC_TYPE_CS_REGISTRATION] == TRUE)
  {
    proc_type = SYS_PROC_TYPE_CS_REGISTRATION;
  }
  else if(mm_proc_status[SYS_PROC_TYPE_PS_REGISTRATION] == TRUE)
  {
    proc_type = SYS_PROC_TYPE_PS_REGISTRATION;
  }
  else if(mm_proc_status[SYS_PROC_TYPE_PS_CALL_SIGNALING] == TRUE)
  {
...
FUNCTION MM_SET_SMS_PROC_STATUS_TYPE

DESCRIPTION
  This function sets SMS proc type and resets the CS call proc type once UE 
  receives the OTA SMS after receiving MT page.
...
===========================================================================*/
void mm_set_sms_proc_status_type(void)
{
  mm_proc_status_set(SYS_PROC_TYPE_SMS,TRUE);
  if((mm_state == MM_WAIT_FOR_NETWORK_COMMAND) &&
     (mm_serving_plmn.info.active_rat == SYS_RAT_GSM_RADIO_ACCESS))
  {
    mm_proc_status_set(SYS_PROC_TYPE_CS_CALL, FALSE);
  }
}
#endif
...
{
#ifdef FEATURE_DUAL_SIM
  return (byte *)log_alloc_ext(log_code,pkt_size,1,&sub_id_param);
#else
  (void)sub_id_param;
  return (byte *)log_alloc(log_code,pkt_size);
#endif
}
...
FUNCTION NAS_EVENT_REPORT_PAYLOAD

===========================================================================*/
void nas_event_report_payload
(
...
{
#ifdef FEATURE_DUAL_SIM
  event_report_payload_ext(event_id,length,payload,
                EVENT_NAS_RPT_EXT_VERSION,&sub_id_param);
                                  ...
                                  byte   *msg_ptr)  /* L3 message */
{
   ...
   switch (id.set)
   {
      case PD_CC:
         ...
         switch ( id.id )
         {
            ...
            case NAS_OTA_START_CC:
               ...
               log_event = TRUE;
#ifdef FEATURE_DUAL_SIM
               mm_proc_status_set(SYS_PROC_TYPE_CS_CALL,TRUE);
#endif
#ifdef FEATURE_QSH_EVENT_METRIC
#ifdef FEATURE_DISABLED_HABANERO

			   if(id.id == NAS_OTA_ALERTING)
			   {
				 QEVENT(QSH_CLT_MM, MM_QSH_EVENT_CS_CALL_STATUS, MM_QSH_EVENT_CS_CALL_STATUS_VERSION, mm_sub_id,
                       "=EMM= Call status %d", CAFI_KPI_EVENT_IND_SUCCESS);
	             mm_qevent_call_status_set(FALSE);
			   }
#endif
#endif
               break;
      ...
      case PD_SMS:
         ...
         QEVENT(QSH_CLT_MM, MM_QSH_EVENT_CS_CALL_STATUS, MM_QSH_EVENT_CS_CALL_STATUS_VERSION, mm_sub_id,
                       "=EMM= Call status %d", CAFI_KPI_EVENT_IND_REJECT);
#endif
#endif
         mm_set_sms_proc_status_type();
#endif
         switch ( id.id )
         {
...
#endif
#endif
         mm_proc_status_set(SYS_PROC_TYPE_SUPPLEMENTARY_SERVICE,TRUE);
#endif
         switch ( id.id )
         {
            case NAS_OTA_FACILITY:
      ...
      case PD_MM:
         ...
         switch ( id.id )
         {
            ...
            case LOCATION_UPDATING_REQUEST:
              log_event = TRUE;
#ifdef FEATURE_QSH_EVENT_METRIC
              if(id.id == LOCATION_UPDATING_ACCEPT)
              {
                mm_qsh_ota_metric_signaling_log(LOCATION_UPDATING_ACCEPT,direction,0xff, MM_GMM_NO_FAILURE_DEBUG);
              }
#endif
#ifdef FEATURE_DUAL_SIM
              ...
              {
                mm_proc_status_set(SYS_PROC_TYPE_CS_REGISTRATION,TRUE);
              }
#endif
#ifdef FEATURE_QSH_EVENT_METRIC
              if(id.id == LOCATION_UPDATING_ACCEPT)
            ...
            case PAGING_RESPONSE:
               log_event = TRUE;
#ifdef FEATURE_DUAL_SIM
               if((id.id == CM_SERVICE_REQUEST)&&( direction == UE_TO_CN_DIRECTION ))
               {
                 message_type = *(msg_ptr+2) & 0x0F;
                 if(message_type == SHORT_MESSAGE_SERVICE_ESTABLISHMENT)
                 {
                   mm_proc_status_set(SYS_PROC_TYPE_SMS,TRUE);
                 }
                 else if(message_type == SUPPLEMENTARY_SERVICE_ACTIVATION)
                 {
                   mm_proc_status_set(SYS_PROC_TYPE_SUPPLEMENTARY_SERVICE,TRUE);
                 }
                 else
               ...
               else if(id.id == PAGING_RESPONSE)
               {
                 if(is_umts_mode() && 
                    ((mm_last_paging_cause == RRC_PAGE_CAUSE_SMS) ||
                     (mm_last_paging_cause == RRC_PAGE_CAUSE_LOW_PRI_SIGNALLING)))
                 {
                   mm_proc_status_set(SYS_PROC_TYPE_SMS,TRUE);
                 }
                 else
...
#endif
#ifdef FEATURE_QSH_EVENT_METRIC
               if((id.id != AUTHENTICATION_FAILURE) &&
                  (id.id != CM_SERVICE_REJECT) &&
                  (id.id != IDENTITY_REQUEST) &&
                  (id.id != ABORT) &&
                  (id.id != MM_STATUS) &&
                  (id.id != IDENTITY_REQUEST) &&
                  (id.id != TMSI_REALLOCATION_COMMAND))
               {
                 mm_qsh_ota_metric_signaling_log(id.id,direction,0xff, MM_GMM_NO_FAILURE_DEBUG);
               }
#endif
               break;
            default:
      ...
      case PD_GMM:
         switch ( id.id )
         {
            ...
            case GMM_ROUTING_AREA_UPDATE_REJECT:
              log_event = TRUE;
#ifdef FEATURE_QSH_EVENT_METRIC
              if((id.id == GMM_ATTACH_COMPLETE) ||
                 (id.id == GMM_ROUTING_AREA_UPDATE_COMPLETE) ||
                 (id.id == GMM_DETACH_ACCEPT))
              {
                gmm_qsh_ota_metric_signaling_log(id.id,direction,0xff, MM_GMM_NO_FAILURE_DEBUG);
              }
#endif
#ifdef FEATURE_DUAL_SIM
              if((id.id == GMM_ATTACH_REQUEST) ||
                 (id.id == GMM_DETACH_REQUEST) ||
                 (id.id == GMM_ROUTING_AREA_UPDATE_REQUEST))
              {
                mm_proc_status_set(SYS_PROC_TYPE_PS_REGISTRATION,TRUE);
              }
#endif
#ifdef FEATURE_QSH_EVENT_METRIC
              if((id.id == GMM_ATTACH_COMPLETE) ||
            ...
            case GMM_INFORMATION:
               log_event = TRUE;
#ifdef FEATURE_DUAL_SIM
               if(id.id == GMM_SERVICE_REQUEST)
               {
                 mm_proc_status_set(SYS_PROC_TYPE_PS_CALL_SIGNALING,TRUE);
               }
#endif
#ifdef FEATURE_QSH_EVENT_METRIC
               if((id.id != GMM_PTMSI_REALLOCATION_COMMAND) &&
                  (id.id != GMM_AUTHENTICATION_AND_CIPHERING_FAILURE) &&
                  (id.id != GMM_IDENTITY_REQUEST) &&
                  (id.id != GMM_STATUS) &&
                  (id.id != GMM_SERVICE_REQUEST) &&
                  (id.id != GMM_SERVICE_REJECT))
               {
                 gmm_qsh_ota_metric_signaling_log(id.id,direction,0xff, MM_GMM_NO_FAILURE_DEBUG);
               }
#endif
   ...
   if ( TRUE == log_event )
   {
      ...
      event_packet[0] = (byte)  mm_as_id + 1;
      ...
      if ( mm_dual_standby_mode )
      {
        ...
        {
           nas_event_report_payload(EVENT_DS_NAS_MESSAGE_SENT, 4, (void *)event_packet,mm_message_as_id);
        }
        else
        {
           nas_event_report_payload(EVENT_DS_NAS_MESSAGE_RECEIVED, 4, (void *)event_packet,mm_message_as_id);
        }
      }
      else
#endif
      {
        if ( direction == UE_TO_CN_DIRECTION )
        {
           nas_event_report_payload(EVENT_NAS_MESSAGE_SENT, 3, (void *)&id,mm_message_as_id);
        }
        else
        {
           nas_event_report_payload(EVENT_NAS_MESSAGE_RECEIVED, 3, (void *)&id,mm_message_as_id);
        }
      }
                                ...
                                byte   *msg_ptr)
{
   ...
   {
     (void) ULogFront_RawLog(mm_get_ulog_ota_handle(), (const char*) msg_ptr, msg_size);
   }
   ...
   if ( mm_dual_standby_mode  || mm_sglte_mode)
   {
     ...
     ds_log_ptr = (LOG_UMTS_DS_NAS_OTA_MESSAGE_LOG_PACKET_C_type *)
                nas_log_alloc(LOG_UMTS_DS_NAS_OTA_MESSAGE_LOG_PACKET_C, pkt_size,
                                  mm_message_as_id);
     ...
     /* Set packet elements */
     ds_log_ptr->as_id = (uint8) mm_as_id + 1;
     ...
     /* Send the packet */
     log_util_log_packet_commit((sys_modem_as_id_e_type)mm_as_id, ds_log_ptr);
...
#endif
   {
     ...
     log_ptr = (LOG_UMTS_NAS_OTA_MESSAGE_LOG_PACKET_C_type *)
                nas_log_alloc(LOG_UMTS_NAS_OTA_MESSAGE_LOG_PACKET_C, pkt_size,
                mm_message_as_id);
     ...
     /* Send the packet */
     log_util_log_packet_commit((sys_modem_as_id_e_type)mm_as_id,log_ptr);
...
void mmsend_mm_info_event_packet(void)
{
   ...
   {
#ifdef FEATURE_FEMTO_CSG
     nas_event_report_payload(EVENT_DS_PLMN_INFORMATION, 15, (void *)event_packet,mm_message_as_id);
#else
     nas_event_report_payload(EVENT_DS_PLMN_INFORMATION, 10, (void *)event_packet,mm_message_as_id);
#endif 
   }
   else
#endif
   {
#ifdef FEATURE_FEMTO_CSG
     nas_event_report_payload(EVENT_PLMN_INFORMATION, 14, (void *)event_packet,mm_message_as_id);
#else
     nas_event_report_payload(EVENT_PLMN_INFORMATION, 9, (void *)event_packet,mm_message_as_id);
#endif 
   }
...
void mmsend_mm_state_log_packet(void)
{
   ...
   if ( mm_dual_standby_mode  || mm_sglte_mode)
   {
     nas_event_report_payload(EVENT_DS_MM_STATE, 3, (void *)event_packet,mm_message_as_id);
   }
   else
#endif
   {
     nas_event_report_payload(EVENT_MM_STATE, 2, (void *)event_packet,mm_message_as_id);
   }

   /* MM Events */
   /* Log only if GSM or WCDMA. LTE log is incorrect*/
   if (is_gsm_mode() || is_wcdma_mode())
   ...
   if ( mm_dual_standby_mode  || mm_sglte_mode)
   {
     ds_log_ptr = (LOG_UMTS_DS_NAS_MM_STATE_LOG_PACKET_C_type *)
                nas_log_alloc(LOG_UMTS_DS_NAS_MM_STATE_LOG_PACKET_C,
...
#endif
   {
     log_ptr = (LOG_UMTS_NAS_MM_STATE_LOG_PACKET_C_type *)nas_log_alloc(LOG_UMTS_NAS_MM_STATE_LOG_PACKET_C,
        sizeof(LOG_UMTS_NAS_MM_STATE_LOG_PACKET_C_type),mm_message_as_id);
...
void mmsend_nas_ue_static_id_log_packet(void)
{
   ...
   if ( mm_dual_standby_mode || mm_sglte_mode)
   {
     ds_log_ptr = (LOG_UMTS_DS_NAS_UE_STATIC_ID_LOG_PACKET_C_type *)
                nas_log_alloc(LOG_UMTS_DS_NAS_UE_STATIC_ID_LOG_PACKET_C,
                          sizeof(LOG_UMTS_DS_NAS_UE_STATIC_ID_LOG_PACKET_C_type),mm_message_as_id);
...
#endif
   {
     log_ptr = (LOG_UMTS_NAS_UE_STATIC_ID_LOG_PACKET_C_type *)nas_log_alloc(LOG_UMTS_NAS_UE_STATIC_ID_LOG_PACKET_C,
        sizeof(LOG_UMTS_NAS_UE_STATIC_ID_LOG_PACKET_C_type),mm_message_as_id);
...
void mmsend_mm_info_log_packet(void)
{
   ...
   if ( mm_dual_standby_mode  || mm_sglte_mode)
   {
     ds_log_ptr = (LOG_UMTS_DS_MM_INFO_LOG_PACKET_C_type *)
                nas_log_alloc(LOG_UMTS_DS_MM_INFO_LOG_PACKET_C,
                          sizeof(LOG_UMTS_DS_MM_INFO_LOG_PACKET_C_type),mm_message_as_id);
...
#endif
   {
     log_ptr = (LOG_UMTS_MM_INFO_LOG_PACKET_C_type *)nas_log_alloc(LOG_UMTS_MM_INFO_LOG_PACKET_C,
        sizeof(LOG_UMTS_MM_INFO_LOG_PACKET_C_type),mm_message_as_id);
...
void mmsend_nas_ue_dynamic_id_log_packet(uint8 id_type)
{
   ...
   if ( mm_dual_standby_mode || mm_sglte_mode)
   {
     ds_log_ptr = (LOG_UMTS_DS_NAS_UE_DYNAMIC_ID_LOG_PACKET_C_type *)
                nas_log_alloc(LOG_UMTS_DS_NAS_UE_DYNAMIC_ID_LOG_PACKET_C,
                          sizeof(LOG_UMTS_DS_NAS_UE_DYNAMIC_ID_LOG_PACKET_C_type),mm_message_as_id);
...
#endif
   {
     log_ptr = (LOG_UMTS_NAS_UE_DYNAMIC_ID_LOG_PACKET_C_type *)nas_log_alloc(LOG_UMTS_NAS_UE_DYNAMIC_ID_LOG_PACKET_C,
        sizeof(LOG_UMTS_NAS_UE_DYNAMIC_ID_LOG_PACKET_C_type),mm_message_as_id);
...
void mmsend_gmm_state_log_packet(void)
{
   ...
   if ( mm_dual_standby_mode  || mm_sglte_mode)
   {
     nas_event_report_payload(EVENT_DS_GMM_STATE, 3, (void *)event_packet,mm_message_as_id);
   }
   else
#endif
   {
     nas_event_report_payload(EVENT_GMM_STATE, 2, (void *)event_packet,mm_message_as_id);
   }
   ...
   if ( mm_dual_standby_mode  || mm_sglte_mode)
   {
     ds_log_ptr = (LOG_UMTS_DS_NAS_GMM_STATE_LOG_PACKET_C_type *)
                nas_log_alloc(LOG_UMTS_DS_NAS_GMM_STATE_LOG_PACKET_C,
                          sizeof(LOG_UMTS_DS_NAS_GMM_STATE_LOG_PACKET_C_type),mm_message_as_id);
...
#endif
   {
     log_ptr = (LOG_UMTS_NAS_GMM_STATE_LOG_PACKET_C_type *)nas_log_alloc(LOG_UMTS_NAS_GMM_STATE_LOG_PACKET_C,
        sizeof(LOG_UMTS_NAS_GMM_STATE_LOG_PACKET_C_type),mm_message_as_id);
...
event_id_enum_type event_nas = EVENT_DROP_ID;
 ...
 MSG_HIGH_DS_3(MM_SUB, "=MM= NAS sent event type %d  RAT %d  %d ", (byte)nas_event,
   ...
   if ( mm_dual_standby_mode  || mm_sglte_mode)
   {
     nas_event_report_payload(event_nas, 6, (void *)event_packet,mm_message_as_id);
   }
   else
#endif
   {
     nas_event_report_payload(event_nas, 5, (void *)event_packet,mm_message_as_id);
   }


#ifdef FEATURE_QSH_EVENT_NOTIFY_TO_QSH
  qsh_evt = mm_emm_map_nas_reg_event_to_qsh_event(event_nas, event_packet);
  if(mm_is_event_debug_any_failure_enabled(qsh_evt) == TRUE )
  {
    ...
    mm_emm_qsh_client_event_log(MM_QSH_EVENT_FIELD_DEBUG_ANY_FAILURE, event_packet);
  }
  else if(mm_is_qsh_event_valid_and_enabled(qsh_evt) == TRUE)
  {
    mm_emm_qsh_client_event_log(qsh_evt, event_packet);
  }
#endif
}
#ifdef FEATURE_QSH_EVENT_NOTIFY_TO_QSH
void mm_emm_notify_page_event_qsh(void)
{
byte event;
if(mm_is_event_debug_any_failure_enabled(MM_QSH_EVENT_CS_PAGE_IND) == TRUE )
  {
    QSH_LOG(QSH_CLT_MM,QSH_CAT_EVENT,QSH_MSG_TYPE_HIGH,"MM_QSH_EVENT_FIELD_DEBUG_ANY_FAILURE %d",MM_QSH_EVENT_CS_PAGE_IND);
    MSG_HIGH_DS_1(MM_SUB, "=MM= QSH_DEBUG EVENT %d  MM_QSH_EVENT_FIELD_DEBUG_ANY_FAILURE triggered", (int)MM_QSH_EVENT_CS_PAGE_IND);
    mm_emm_qsh_client_event_log(MM_QSH_EVENT_CS_PAGE_IND, &event);
  }
  else if(mm_is_qsh_event_valid_and_enabled(MM_QSH_EVENT_CS_PAGE_IND) == TRUE)
  {
    mm_emm_qsh_client_event_log(MM_QSH_EVENT_CS_PAGE_IND, &event);
  }
}
...
SIDE EFFECTS 
 ...
 void mmsend_timer_expiry_event(byte timer_id)
 {
   ...
   nas_event_report_payload(EVENT_MM_TIMER_EXPIRY, 2, (void *)event_packet,mm_message_as_id);
 }
 /*===========================================================================
...
#ifdef FEATURE_DUAL_SIM
void mm_emm_send_illegal_event_on_current_sub
(
  void
)
{
  ...
  {
     emm_send_illegal_event_for_current_sub();
  }
  else
#endif
  {
     mm_gmm_send_illegal_event_for_current_sub();
  }
}


void mm_emm_send_illegal_event_on_other_sub
(
  void
)
{
  ...
  for( mm_other_as_id = MM_AS_ID_1; mm_other_as_id < (mm_as_id_e_type)MAX_NAS_STACKS ; mm_other_as_id++ )
  {
     if( mm_other_as_id != mm_tmp_as_id )
     {
       ...
       if (mm_serving_plmn.info.active_rat == SYS_RAT_LTE_RADIO_ACCESS)
       {
          emm_send_illegal_event_for_other_sub();
       }
       else
#endif
       {
          mm_gmm_send_illegal_event_on_other_subs();
       }
     }
  }
...
FUNCTION MM_GMM_SEND_ILLEGAL_CAUSE_FOR_CURRENT_SUB
...
void mm_gmm_send_illegal_event_for_current_sub
(
  void
)
{
  if (mm_serving_plmn.info.sim_state != SYS_SIM_STATE_NOT_AVAILABLE)  
  {
    ...
    {
       mm_illegal_me_sub = ILLEGAL_ME_ON_CURRENT_SUB;
    }
    if (mm_serving_plmn.ms_op_mode == SYS_SRV_DOMAIN_CS_ONLY || 
         mm_serving_plmn.ms_op_mode == SYS_SRV_DOMAIN_PS_ONLY)
    {
       MSG_HIGH_DS_3( MM_SUB, "=MM= sending ILLEGAL event on current SUB with cause %d  MS_OP_MODE is %d  SIM State %d", mm_illegal_me_sub, mm_serving_plmn.ms_op_mode, mm_serving_plmn.info.sim_state);
...
FUNCTION MM_GMM_SEND_ILLEGAL_EVENT_ON_OTHER_SUB
...
void mm_gmm_send_illegal_event_on_other_subs
(
  void
)
{
  sys_sim_state_e_type previous_sim_state ;
  previous_sim_state = mm_serving_plmn.info.sim_state;
  if (previous_sim_state != SYS_SIM_STATE_NOT_AVAILABLE)  
  {

    if((mm_serving_plmn.ms_op_mode == SYS_SRV_DOMAIN_CS_PS) ||
       ((mm_serving_plmn.ms_op_mode == SYS_SRV_DOMAIN_CS_ONLY) &&(mm_serving_plmn.info.sim_state == SYS_SIM_STATE_PS_INVALID)))
    {
       if(mm_serving_plmn.info.sim_state == SYS_SIM_STATE_CS_INVALID)
       {
         mm_is_sim_to_be_cs_invalid_after_power_off = TRUE;
       }
       else if(mm_serving_plmn.info.sim_state == SYS_SIM_STATE_PS_INVALID)
       {
    ...
    if (mm_illegal_me_sub != NON_ILLEGAL_ME_ON_OTHER_SUB)  
    {
       mm_illegal_me_sub = ILLEGAL_ME_ON_OTHER_SUB;
    }
    if (previous_sim_state != SYS_SIM_STATE_CS_PS_INVALID)
    {
       MSG_HIGH_DS_4( MM_SUB, "=MM= sending ILLEGAL event on SUB %d with cause %d  MS_OP_MODE is %d  SIM State %d", mm_as_id+1, mm_illegal_me_sub, mm_serving_plmn.ms_op_mode, mm_serving_plmn.info.sim_state);
       (void) mmgsdi_session_manage_illegal_subscription(mm_sim_mmgsdi_info.session_id,
                                                      MMGSDI_SESSION_APP_IS_ILLEGAL);
    }
  }
...
void mm_emm_clear_dos_plmn_counter_forbidden_tai
(
  void
)
{
...
void mm_emm_clear_dos_plmn_counter_forbidden_tai_per_subs
(
...
#ifdef FEATURE_LTE
  emm_ctrl_data_type* emm_ctrl_data_ptr = NULL;
  emm_ctrl_data_ptr = emm_db_get_ctrl_data_per_subs(mm_local_as_id);
  ASSERT( emm_ctrl_data_ptr != NULL );
#endif
  if (mm_emm_max_attempt_counter_dos_mitigation != 0)
  {
...
#if defined(FEATURE_LTE)
#ifdef FEATURE_DUAL_SIM 
    if(mm_subs_capability_mask_sim[mm_local_as_id] & SYS_SYS_MODE_MASK_LTE)
#endif
    {
      emm_clear_forbidden_tai_list(emm_ctrl_data_ptr->rrc_forbidden_list_ptr,TRUE);
      emm_clear_forbidden_tai_list(emm_ctrl_data_ptr->forbidden_for_service_list_ptr,TRUE);
    }
    emm_clear_manual_forbidden_tai_list_per_subs(TRUE, mm_local_as_id);
#endif
  }
}

  void mm_emm_send_legal_event_on_other_subs
  (
    void
  )
  {
    ...
    for( mm_other_as_id = MM_AS_ID_1; mm_other_as_id < (mm_as_id_e_type)MAX_NAS_STACKS ; mm_other_as_id++ )
    {
       if( mm_other_as_id != mm_tmp_as_id )
       {
          ...
          {
             mm_emm_clear_dos_plmn_counter_forbidden_tai();
          }
          if ((mm_illegal_me_sub == ILLEGAL_ME_ON_OTHER_SUB) &&
               ((mm_timer_status[TIMER_T3247] == TIMER_STOPPED && !reg_is_t3245_timer_active((sys_modem_as_id_e_type)mm_other_as_id)) ||
                 (mm_is_sim_to_be_cs_invalid_after_power_off || mm_is_sim_to_be_ps_invalid_after_power_off  ))&&
               ...
               (mm_serving_plmn.info.sim_state == SYS_SIM_STATE_CS_PS_INVALID)))
          {
             (void) mmgsdi_session_manage_illegal_subscription(mm_sim_mmgsdi_info.session_id,
              ...
              MSG_HIGH_DS_2 (MM_SUB, "Sending legal event on SUB %d with cause %d", mm_as_id+1, mm_illegal_me_sub);
          }

       }
    }
    MSG_HIGH_DS_2 (MM_SUB, "Cause on SUB %d is %d",mm_as_id+1, mm_illegal_me_sub);
    mm_set_as_id(mm_tmp_as_id);
  }

  void mm_emm_send_legal_event_on_other_subs_per_subs 
  (
     mm_as_id_e_type mm_local_as_id
  )
  {
    mm_as_id_e_type  mm_other_as_id;
    if (MMCNM_IS_MODE_DUAL_ACTIVE && (mm_mn_sglte_svlte_mode == FALSE))
    {
       return;
    }
    for( mm_other_as_id = MM_AS_ID_1; mm_other_as_id < (mm_as_id_e_type)MAX_NAS_STACKS ; mm_other_as_id++ )
    {
       if( mm_other_as_id != mm_local_as_id)
       {
          if (mm_timer_status_sim[mm_other_as_id][TIMER_T3247] == TIMER_ACTIVE)
          {
             mm_emm_clear_dos_plmn_counter_forbidden_tai_per_subs(mm_other_as_id); 
          }
          if ((mm_illegal_me_sub_sim[mm_other_as_id] == ILLEGAL_ME_ON_OTHER_SUB) &&
               ((mm_timer_status_sim[mm_other_as_id][TIMER_T3247] == TIMER_STOPPED && !reg_is_t3245_timer_active((sys_modem_as_id_e_type)mm_other_as_id)) ||
                 (mm_is_sim_to_be_cs_invalid_after_power_off_sim[mm_other_as_id]|| mm_is_sim_to_be_ps_invalid_after_power_off_sim[mm_other_as_id]))&&
               ((mm_serving_plmn_sim[mm_other_as_id].info.sim_state == SYS_SIM_STATE_NOT_AVAILABLE) ||
               (mm_serving_plmn_sim[mm_other_as_id].info.sim_state == SYS_SIM_STATE_CS_PS_INVALID)))
          {
             (void) mmgsdi_session_manage_illegal_subscription(mm_sim_mmgsdi_info_sim[mm_other_as_id].session_id,
                                                              MMGSDI_SESSION_APP_IS_LEGAL);
              mm_illegal_me_sub_sim[mm_other_as_id] = ILLEGAL_ME_SUB_NO_CAUSE;
              MSG_HIGH_DS_2 (MM_SUB, "Sending legal event on SUB %d with cause %d", mm_other_as_id+1, mm_illegal_me_sub_sim[mm_other_as_id]);
          }

       }
    }
    MSG_HIGH_DS_2 (MM_SUB, "Cause on SUB %d is %d",mm_local_as_id+1, mm_illegal_me_sub_sim[mm_local_as_id]);
  }


boolean mm_emm_is_illegal_me_on_any_sub
(
  sys_modem_as_id_e_type as_id
)
{
  ...
  for( mm_other_as_id = MM_AS_ID_1; mm_other_as_id < (mm_as_id_e_type)MAX_NAS_STACKS ; mm_other_as_id++ )
  {
     if( mm_other_as_id != (mm_as_id_e_type)as_id )
     {
        if (mm_illegal_me_sub_sim[mm_other_as_id] == ILLEGAL_ME_ON_CURRENT_SUB)
        {
           result = TRUE;
           break;
        }
  ...
  MSG_HIGH_DS_3 (MM_SUB, "Is ILLEGAL ME on any SUB = %d ILLEGAL ME SUB = %d Cause = %d", result, mm_other_as_id+1, mm_illegal_me_sub_sim[mm_other_as_id]);
  return result;
}

void mm_gmm_set_non_illegal_cause() 
{
  if (mm_is_multisim_device == TRUE)
  {
    if (mm_serving_plmn.ms_op_mode == SYS_SRV_DOMAIN_CS_ONLY ||
        mm_serving_plmn.ms_op_mode == SYS_SRV_DOMAIN_PS_ONLY || 
        mm_serving_plmn.info.sim_state == SYS_SIM_STATE_CS_PS_INVALID)
    {
      mm_illegal_me_sub = NON_ILLEGAL_ME_ON_OTHER_SUB;
    }
  }
}


void mm_gmm_set_illegal_non_illegal_cause(byte cause)
{
  if ((mm_is_multisim_device == TRUE)
        &&(mm_illegal_me_sub == ILLEGAL_ME_SUB_NO_CAUSE))  
  {
     if (cause == ILLEGAL_ME)
     {
        ...
        if (mm_illegal_me_sub == ILLEGAL_ME_ON_CURRENT_SUB)
        {
           ...
           mm_send_mmr_me_blocked_status_ind(TRUE, mm_as_id);
        }
      }
      else
      {
...
void mm_emm_unblock_me(void)
{
  ...
  ASSERT( emm_ctrl_data_ptr != NULL );
#endif
  mm_emm_send_legal_event_on_other_subs();
#ifdef FEATURE_LTE
  if (mm_serving_plmn.info.active_rat == SYS_RAT_LTE_RADIO_ACCESS)
  {
       emm_send_mmr_me_blocked_status_ind(FALSE);
       *emm_ctrl_data_ptr->emm_illegal_me_sub = ILLEGAL_ME_SUB_NO_CAUSE;
  }
  else
#endif
  {
     mm_send_mmr_me_blocked_status_ind(FALSE, mm_as_id);
     mm_illegal_me_sub = ILLEGAL_ME_SUB_NO_CAUSE;
  }
}
...
#else
void rrc_put_mm_cmd_sub(sys_modem_as_id_e_type as_id, rrc_cmd_type *cmd_buf)
{
   MSG_FATAL_DS(MM_SUB, "=MM= rrc_put_mm_cmd: Not WCDMA!", 0,0,0);
}
#endif void rrc_free_cmd_buf(rrc_cmd_type *cmd_buf)
{
...
#else
void IWCDMA_rrc_put_mm_cmd_sub(interface_t* ptr, sys_modem_as_id_e_type as_id, rrc_cmd_type *cmd_buf)
{
   MSG_FATAL_DS(MM_SUB, "=MM= rrc_put_mm_cmd: Not WCDMA!", 0,0,0);
}
#endif 
...
void mmsend_rat_change_event()
{
  ...
  old_rat = mm_previous_rat;
   ...
   if ( mm_dual_standby_mode  || mm_sglte_mode)
   {
     nas_event_report_payload(EVENT_DS_RAT_CHANGE, 2, (void *)event_packet,mm_message_as_id);
   }
   else
#endif
   {
     nas_event_report_payload(EVENT_RAT_CHANGE, 1, (void *)event_packet,mm_message_as_id);
   }
...
)
{
  ...
  if(request == TRUE)
  {
     nas_event_report_payload(EVENT_SMGMM_REQUEST_SENT,2,(void*)event_payload,mm_message_as_id);
  }
  else
  {
     nas_event_report_payload(EVENT_SMGMM_REJECT_RECEIVED,2,(void*)event_payload,mm_message_as_id);
  }
  ...
  ms_op_mode_local = mm_serving_plmn_sim[stack_id_nas].ms_op_mode;
    ...
    if( is_lte_mode_per_nas_stacks(stack_id_nas))
    {
                                                          ...
                                                          && (emm_ctrl_data_ptr->aborting_conn == FALSE))))
      {
        return SYS_GW_CONN_STATUS_CONN;
	  		  ...
	  		  reg_mode_get_higher_priority_plmn_selection_started((sys_modem_as_id_e_type)mm_sub_id))
      {
    ...
    if(is_umts_mode_per_nas_stacks(stack_id_nas) || is_gsm_mode_per_nas_stacks(stack_id_nas))
    {
      ...
      if(((ms_op_mode_local == SYS_SRV_DOMAIN_PS_ONLY) ||
         ...
         sm_is_pdp_active() &&
#endif
        (gmm_substate_sim[stack_id_nas] == GMM_NORMAL_SERVICE) && (dsumtsps_is_ps_data_available()||
        ((is_gsm_mode_per_nas_stacks(stack_id_nas) && (gmm_is_ps_active_sim[stack_id_nas] == TRUE))
#if defined(FEATURE_LTE)
          || (is_umts_mode_per_nas_stacks(stack_id_nas) && (rrc_connection_status_sim[stack_id_nas] != MMRRC_IDLE) && (mm_rrc_pch_state_info_sim[stack_id_nas].is_in_pch_state != TRUE))
#endif
        )))
      {
 ...
 emm_ctrl_data_type     *emm_ctrl_data_ptr = NULL; 
  ...
  if ( is_umts_mode() || is_gsm_mode())
  {
    ...
    if(
         ...
         sm_is_pdp_active() 
#endif
         && (gmm_substate == GMM_NORMAL_SERVICE) && (dsumtsps_is_ps_data_available()
...
)
{
  ...
  MSG_HIGH_DS_2(MM_SUB, "=MM= mm_check_connection_active returning %d is_in_pch_state : %d",
                                           conn_active, mm_rrc_pch_state_info_sim[stack_id_nas].is_in_pch_state);
#else
  MSG_HIGH_DS_2(MM_SUB, "=MM= mm_check_connection_active returning %d is_in_pch_state : %d",
...
)
{
  ...
  if((((mm_cnm_srv_est_cause_sim[stack_id_nas] == MM_RR_SHORT_MESSAGE) ||
       ...
       ((mm_state_sim[stack_id_nas] == MM_WAIT_FOR_RR_CONNECTION_MM) ||
        ...
        (mm_state_sim[stack_id_nas] == MM_WAIT_FOR_ADDITIONAL_OUTGOING_MM_CONNECTION) ||
...
FUNCTION MM_GET_SUB_CAPABILITY_MASK
...
===========================================================================*/
uint32 mm_get_sub_capability_mask(sys_modem_as_id_e_type sub_id)
{
  return mm_subs_capability_mask_sim[sub_id];
}
#endif
...
void mm_set_nas_bit_mask(uint16 bit_mask)
{
  ...
  mm_client_activated_group = bit_mask & 0x01FF; 
  ...
  else
  {
    ...
    if (mm_nas_changed_group & SYS_NAS_REGISTRATION_INFO_MASK)
    {
      ...
      memscpy(nas_information->reg_nw_info.rplmn.identity, sizeof(sys_plmn_id_s_type), mm_location_information.lai,3);
         ...
         {
           nas_information->reg_nw_info.ecall_inactivity_completed = SYS_DEREG_SUCCESS;
         }
         else if(mm_ecall_dereg_in_wrong_state)
         {
           nas_information->reg_nw_info.ecall_inactivity_completed = SYS_DEREG_ERROR; 
         }
         else
         {
           nas_information->reg_nw_info.ecall_inactivity_completed = SYS_DEREG_NO_RESP; 
         }
...
#ifdef FEATURE_NAS_ECALL
    if (mm_nas_changed_group & SYS_NAS_TIMER_EXPIRY_INFO_MASK)
    {
      nas_information->timer_expiry_info = mm_timer_expiry_info;
      mm_nas_changed_group &= ~SYS_NAS_TIMER_EXPIRY_INFO_MASK;
      mm_timer_expiry_info.timer_bitmask =0;
    }
    if(mm_nas_changed_group & SYS_NAS_INACTIVITY_TIMER_RESUME_INFO_MASK)
    {
      if(mm_ecall_mode_type == MMR_ECALL_MODE_ECALL_ONLY)
      {
        nas_information->ecall_inactivity_timer_response = SYS_INACTIVITY_TIMER_SUCCESS;
      }
      else
      {
        nas_information->ecall_inactivity_timer_response = SYS_INACTIVITY_TIMER_ERROR;
      }
      mm_nas_changed_group &= ~SYS_NAS_INACTIVITY_TIMER_RESUME_INFO_MASK;
    }
#endif
...
/* return - FALSE - LAI not updated, TRUE - LAI updated */
boolean mm_get_updated_lai(inter_task_lai_T *rr_lai, const sys_modem_as_id_e_type as_id)
{
   boolean return_update_status = FALSE;
#ifdef FEATURE_DUAL_SIM
   if( mm_location_information_sim[as_id].location_update_status == UPDATED )
   {
     return_update_status = TRUE;
   }
   rr_lai->PLMN_id.identity[0] = mm_location_information_sim[as_id].lai[0];
   rr_lai->PLMN_id.identity[1] = mm_location_information_sim[as_id].lai[1];
   rr_lai->PLMN_id.identity[2] = mm_location_information_sim[as_id].lai[2];
   rr_lai->location_area_code[0] = mm_location_information_sim[as_id].lai[3];
   rr_lai->location_area_code[1] = mm_location_information_sim[as_id].lai[4];
#else
   if( mm_location_information.location_update_status == UPDATED )
   {
     return_update_status = TRUE;
   }
   rr_lai->PLMN_id.identity[0] = mm_location_information.lai[0];
   rr_lai->PLMN_id.identity[1] = mm_location_information.lai[1];
   rr_lai->PLMN_id.identity[2] = mm_location_information.lai[2];
   rr_lai->location_area_code[0] = mm_location_information.lai[3];
   rr_lai->location_area_code[1] = mm_location_information.lai[4];

#endif

   MSG_HIGH_DS_1(MM_LOCAL_SUB,"=MM= mm_get_updated_lai return_value = %d",return_update_status);
...
void mm_per_nas_stacks_set_nas_bit_mask(uint16 bit_mask, sys_modem_as_id_e_type stack_id_nas)
{
  ...
  {
    rex_enter_crit_sect(&mm_ftd_crit_sect);
    mm_client_activated_group_sim[stack_id_nas] = bit_mask & 0x01FF; 
    rex_leave_crit_sect(&mm_ftd_crit_sect);
  }
  ...
  else
  {
    ...
    else
    {
       ...
       if (mm_nas_changed_group_sim[as_id] & SYS_NAS_REGISTRATION_INFO_MASK)
       {
...
#ifdef FEATURE_NAS_ECALL
         if(mm_idle_substate_sim[as_id] == MM_ECALL_INACTIVE)
         {
           nas_information->reg_nw_info.ecall_inactivity_completed = SYS_DEREG_SUCCESS;
         }
         else if(mm_ecall_dereg_in_wrong_state_sim[as_id])
         {
           nas_information->reg_nw_info.ecall_inactivity_completed = SYS_DEREG_ERROR; 
         }
         else
         ...
         mm_ecall_dereg_in_wrong_state_sim[as_id] = FALSE;
#endif		 
       ...
       {
#ifdef FEATURE_DUAL_SIM
         boolean as_id_match = (as_id == (sys_modem_as_id_e_type)sm_as_id);
#else
...
#endif
#ifdef FEATURE_NAS_ECALL
       if (mm_nas_changed_group_sim[as_id] & SYS_NAS_TIMER_EXPIRY_INFO_MASK)
       {
         nas_information->timer_expiry_info = mm_timer_expiry_info_sim[as_id];
         mm_nas_changed_group_sim[as_id]  &= ~SYS_NAS_TIMER_EXPIRY_INFO_MASK;
         mm_timer_expiry_info_sim[as_id].timer_bitmask &= ~SYS_MM_TIMER_T3242_INFO_MASK;
         mm_timer_expiry_info_sim[as_id].timer_bitmask &= ~SYS_MM_TIMER_T3243_INFO_MASK;
       }
       if(mm_nas_changed_group & SYS_NAS_INACTIVITY_TIMER_RESUME_INFO_MASK)
       {
         if(mm_ecall_mode_type == MMR_ECALL_MODE_ECALL_ONLY)
         ...
         mm_nas_changed_group_sim[as_id] &= ~SYS_NAS_INACTIVITY_TIMER_RESUME_INFO_MASK;
       }
#endif
       result = TRUE;
       rex_leave_crit_sect(&mm_ftd_crit_sect);
...
void mm_perform_ecall_inactivity(void)
{
  ...
  if (mm_state == MM_NULL || mm_state == MM_CONNECTION_ACTIVE || mm_state == MM_WAIT_FOR_RR_CONNECTION_MM  || 
      mm_state == MM_WAIT_FOR_OUTGOING_MM_CONNECTION  || mm_state == MM_WAIT_FOR_ADDITIONAL_OUTGOING_MM_CONNECTION )
  {
...
void mm_pend_lu_rau_on_ll_failure_in_lte()
{

           MSG_HIGH_DS_0( MM_SUB, "=MM= RAU will be triggered due to Lower layer failure in LTE");
...
void mm_log_mm_reg_variables()
{
  ...
  while ((--i>= 0) && (mm_emm_debug_buffer_index - i< 5))
  {
    ...
    {
      MSG_HIGH_DS_3((mm_emm_debug_buffer[i].as_id + 1), "Direction: %d MSG SET: %d ID: %d",
    ...
    {
      MSG_HIGH_DS_3((mm_emm_debug_buffer[i].as_id +1), "Direction: %d UMID: %d EMM state: %d",
           mm_emm_debug_buffer[i].mm_emm_message_direction,
...
#endif  
  {
   ...
   if(i < MAX_AS_IDS)
   {
     emm_ctrl_data_ptr = emm_db_get_ctrl_data_per_subs((mm_as_id_e_type)i);
   }
   else
   {
...
#endif
    MSG_HIGH_DS_6( i+1, "=MM= mm_need_to_process_sim_avail_req/waiting_for_open=%2x, session_type = %d, mm_sim_card_mode=%d, Active Rat=%d(0=gsm, 1=wcdma, 2=lte), mm_waiting_for_deact_cnf =%d, mm_waiting_for_ph_status_change_cnf =%d", 
...
#ifndef FEATURE_LTE
    MSG_HIGH_DS_3( i+1, "=MM= mm_sim_mode_pref=%d, mm_dual_standby_mode=%d, mm_ue_pref_rat = %d", 
                              mm_sim_mode_pref_sim[i], mm_dual_standby_mode, mm_ue_pref_rat_sim[i]);
#endif

    MSG_HIGH_DS_3( i+1, "=MM= mm_waiting_for_stop_mode_cnf=%d, mm_waiting_for_service_cnf=%d, mm_last_active_rat_stopped=%d", 
                  mm_waiting_for_stop_mode_cnf_sim[i], mm_waiting_for_service_cnf_sim[i],
                  mm_last_active_rat_stopped_sim[i]);
    if(i< MAX_AS_IDS)    
    {    
      MSG_HIGH_DS_2( i+1, "=MM= mm_subs_capability=%d, reg_sub_config=%d ", 
                  mm_subs_capability_mask_sim[i], reg_sub_config_sim[i]);  
    }
#ifdef FEATURE_LTE
    if(emm_ctrl_data_ptr != NULL)
    {
      MSG_HIGH_DS_5(i+1,"=EMM= EMM State = %d, EMM Substate = %d,  EMM Connection State = %d, LRRC Active = %d, Last LRRC deact conf status = %d", 
      ...
      MSG_HIGH_DS_5(i+1,"=EMM= LRRC deact req reason =%d, LRRC deact req pending =%d, LRRC aborting conn = %d, Registered serv domain = %d, Ignore GW activation ind = %d", 
...
#endif  
  {
   ...
   else
   {
    ...
    {
         MSG_HIGH_DS_0(i+1,"=MM= CM is waiting for PS_DETACH_CNF");
    }   
    if(mm_waiting_for_deact_cnf_sim[i])
    {
      MSG_HIGH_DS_1(i+1,"=MM= RAT %d (0=GSM, 1=WCDMA, 2=LTE, 3=TDS) was not able to respond to NAS for DEACT_REQ ",mm_serving_plmn_sim[i].info.active_rat);
    }
    else if(mm_waiting_for_stop_mode_cnf_sim[i]) 
    {
      MSG_HIGH_DS_1(i+1,"=MM= RAT %d (0=GSM, 1=WCDMA, 2=LTE, 3=TDS) was not able to respond to NAS for STOP_MODE_REQ/DEACTIVATE_REQ",\
                                                                                                mm_serving_plmn_sim[i].info.active_rat);
    }
    else if(mm_waiting_for_service_cnf_sim[i]) 
    {
      MSG_HIGH_DS_1(i+1,"=MM= RAT %d (0=GSM, 1=WCDMA, 2=LTE, 3=TDS) was not able to respond to NAS for SERVICE_REQ ",mm_serving_plmn_sim[i].info.active_rat);
    }
    else if(mm_waiting_for_ph_status_change_cnf_sim[i]) 
    {
      MSG_HIGH_DS_1(i+1,"=MM= RAT %d (0=GSM, 1=WCDMA, 2=LTE, 3=TDS) was not able to respond to NAS for DEACT_REQ/DEACTIVATE_REQ ",mm_serving_plmn_sim[i].info.active_rat);
    }
    else if((rrc_connection_status_sim[i] == MMRRC_RELEASE_IN_PROGRESS) 
#ifdef FEATURE_LTE
    || (mm_serving_plmn_sim[i].info.active_rat == SYS_RAT_LTE_RADIO_ACCESS &&
        (emm_ctrl_data_ptr != NULL) && 
        (emm_ctrl_data_ptr->aborting_conn))
#endif
    ) 
    {
      MSG_HIGH_DS_1(i+1,"=MM= RAT %d (0=GSM, 1=WCDMA, 2=LTE, 3=TDS) was not able to respond to NAS for ABORT_REQ",mm_serving_plmn_sim[i].info.active_rat);
    }
#ifdef FEATURE_LTE
    else if((mm_serving_plmn_sim[i].info.active_rat == SYS_RAT_LTE_RADIO_ACCESS) && 
            (emm_ctrl_data_ptr != NULL) &&
            (emm_ctrl_data_ptr->rrc_deact_req.deact_cnf_pending))
    {
      MSG_HIGH_DS_0(i+1,"=MM= LTE was not able to respond to NAS for LTE_RRC_DEACT_REQ ");
    }
#endif
    else 
    {
     ...
     if(index>=0)
     {
       ...
       switch(reg_debug_buffer[index].message_id)
       {
          ...
          MSG_HIGH_DS_0(i+1,"=MM= NAS successfully sent CM_SIM_AVAILABLE_CNF to CM");
          break;

          case CM_STOP_MODE_CNF:
          MSG_HIGH_DS_0(i+1,"=MM= NAS successfully sent CM_STOP_MODE_CNF to CM");
          break;

          case CM_PH_STATUS_CHGD_CNF:
          MSG_HIGH_DS_0(i+1,"=MM= NAS successfully sent CM_PH_STATUS_CHGD_CNF to CM");
          break;

          case CM_PS_DETACH_CNF:
          MSG_HIGH_DS_0(i+1,"=MM= NAS successfully sent CM_PS_DETACH_CNF to CM");
          break;

          default:
          MSG_HIGH_DS_2(i+1,"=MM= Last message sent by NAS is message set: %d , message id: %d ", \
...
)
{
  if(!(is_low_prio_debug_buffer_message(message_set,message_id)))
  {
    ...
    if(mm_emm_message_direction == TO_MM)
    {
      MSG_HIGH_DS_2(MM_LOCAL_SUB, "=MM= Received message MS: %d   MSG_ID: %d ", message_set, message_id);
    }
    if(mm_emm_message_direction == FROM_MM)
    {
      MSG_HIGH_DS_2(MM_LOCAL_SUB, "=MM= sent message MS: %d   MSG_ID: %d ", message_set, message_id);
    }
    if (mm_get_ulog_handle() != NULL)
    {
#if defined FEATURE_DUAL_SIM || defined FEATURE_SGLTE
#ifdef FEATURE_LTE
		ULOG_RT_PRINTF_8(mm_get_ulog_handle(),
              "DS: SUB %d =MM= Message Set: %d Message ID: %d mm_state: %d gmm_state: %d emm_state: %d message_direction: %d msg_info: %d",
              as_id + 1,
              message_set,
              message_id,
              mm_state,
              gmm_state,
              EMM_GET_STATE(),
              mm_emm_message_direction,
              mm_msg_info);
#else
		ULOG_RT_PRINTF_7(mm_get_ulog_handle(),
              "DS: SUB %d =MM= Message Set: %d Message ID: %d mm_state: %d gmm_state: %d message_direction: %d msg_info: %d",
              as_id + 1,
              message_set,
              message_id,
              mm_state,
...
#ifdef FEATURE_LTE
		ULOG_RT_PRINTF_7(mm_get_ulog_handle(),
              "=MM= Message Set: %d Message ID: %d mm_state: %d gmm_state: %d emm_state: %d message_direction: %d msg_info: %d",
              message_set,
              message_id,
              mm_state,
              gmm_state,
              ...
              mm_msg_info);
#else
		ULOG_RT_PRINTF_6(mm_get_ulog_handle(),
              "=MM= Message Set: %d Message ID: %d mm_state: %d gmm_state: %d message_direction: %d msg_info: %d",
              message_set,
              message_id,
              mm_state,
              gmm_state,
...
#ifdef FEATURE_QSH_EVENT_METRIC
  if(mm_qsh_debug_metrics.debug_buffer_ptr != NULL)
  {
    qsh_client_metric_log_done_s client_metric_log_done;
    mm_qsh_debug_metrics.debug_buffer_ptr->debug_buffer = mm_emm_debug_buffer[mm_emm_debug_buffer_index];
    qsh_client_metric_log_done_init(&client_metric_log_done);
    client_metric_log_done.client = QSH_CLT_MM;
    client_metric_log_done.metric_id = (qsh_metric_id_t)MM_QSH_METRIC_DEBUG_BUFFER;
    client_metric_log_done.metric_context_id = mm_qsh_debug_metrics.debug_buffer_metric_context_id;
    client_metric_log_done.log_reason = QSH_CLIENT_METRIC_LOG_REASON_EVENT_INTERNAL;
    mm_qsh_debug_metrics.debug_buffer_ptr = 
      (mm_qsh_metric_debug_buffer_s *)qsh_client_metric_log_done(&client_metric_log_done);
  }
#endif
  ...
  if(mm_emm_message_direction == TO_MM)
  {
    mm_check_and_perform_action_invalid_state_detection(as_id, DEBUG_INVALID_STATE_DEBUG_BUFFER_INCOM_MSG);
  }
...
FUNCTION MM_CHECK_AND_PERFORM_ACTION_INVALID_STATE_DETECTION

DESCRIPTION
  This function invokes specific conditional checks based on imput triggers and if those states are inconsistent, then
  a crash (ERR FATAL) is induced.

 Input
   trigger - trigger to invoke specific conditional checks
   as_id   - context id, which is used for Multi-SIM/SGLTE, gets ignored otherwise.

   Output 
     if based on trigger, inconsistent states are found in MM state machine, a crash is induced.
...
void mm_check_and_perform_action_invalid_state_detection
(
  sys_modem_as_id_e_type as_id,
  mm_debug_trigger_enum_T trigger
)
{
    boolean invalid_state_detected = FALSE;

    switch(trigger)
    {
       case DEBUG_INVALID_STATE_POWER_UP:
        {
#if defined FEATURE_DUAL_SIM || defined FEATURE_SGLTE
          if((mm_state_sim[as_id] == MM_NULL) && (rrc_connection_status_sim[as_id] != MMRRC_IDLE))
          {
            invalid_state_detected = TRUE;
          }
#else
          if((mm_state == MM_NULL) && (rrc_connection_status != MMRRC_IDLE))
          {
            invalid_state_detected = TRUE;
          }
#endif
       ...
       case DEBUG_INVALID_STATE_STOP_MODE_CNF:
        {
#if defined FEATURE_DUAL_SIM || defined FEATURE_SGLTE
          if(rrc_connection_status_sim[as_id] != MMRRC_IDLE)
          {
#ifdef FEATURE_WRLF_SYSTEM_SEL
            if(mm_wcdma_rlf_state_sim[as_id] != WCDMA_RLF_IDLE)
            {
              /* In WRLF, for ALT scan, RRC connection is not aborted,
               * It gets aborted when CM sends STOP with IRAT, if UE finds Service in CDMA */
            }
            else
#endif
            {
          ...
          if(mm_timer_status_sim[as_id][ TIMER_MM_REL ] == TIMER_ACTIVE)
          {
            invalid_state_detected = TRUE;
          }
          if(mm_pended_stop_mode_req_sim[as_id].valid == TRUE || mm_stop_mode_req_pended_sim[as_id] == TRUE || mm_nreg_req_received_sim[as_id] == TRUE)
          {
            invalid_state_detected = TRUE;
          }
#else
          ...
          if(mm_pended_stop_mode_req.valid == TRUE || mm_stop_mode_req_pended == TRUE || mm_nreg_req_received == TRUE)
          {
            invalid_state_detected = TRUE;
          }
#endif
       ...
       case DEBUG_INVALID_STATE_DEBUG_BUFFER_INCOM_MSG:
        {
#if defined FEATURE_DUAL_SIM || defined FEATURE_SGLTE
          if((mm_state_sim[as_id] != MM_IDLE) && 
              (mm_state_sim[as_id] != MM_NULL) && 
              (rrc_connection_status_sim[as_id] == MMRRC_IDLE) && 
              ((mm_serving_plmn_sim[as_id].info.active_rat == SYS_RAT_UMTS_RADIO_ACCESS) ||
               (mm_serving_plmn_sim[as_id].info.active_rat == SYS_RAT_TDS_RADIO_ACCESS)))
          {
            invalid_state_detected = TRUE;
          }
#else
       ...
       default:
        {
          ...
          MSG_ERROR_DS_1(MM_SUB, "=MM= Unexpected trigger = %d to detect invalid state: %d", trigger);
        }
        break;

    }
/*
    if(invalid_state_detected == TRUE)
    {
      ERR_FATAL("=MM= Error: Invalid state detected.",0,0,0);
    }
	//Crash point disabled on CS/close to CS PLs
*/
}
...
#if defined FEATURE_DUAL_SIM || defined FEATURE_SGLTE
    ULOG_RT_PRINTF_7(mm_get_ulog_handle(),
            "DS: SUB %d =EMM= umid: %d mm_state: %d gmm_state: %d emm_state: %d message_direction: %d msg_info: %d",
            as_id + 1,
            msg_umid,
            mm_state,
            gmm_state,
...
#else
    ULOG_RT_PRINTF_6(mm_get_ulog_handle(),
            "=EMM= umid: %d mm_state: %d gmm_state: %d emm_state: %d message_direction: %d msg_info: %d",
            msg_umid,
            mm_state,
            gmm_state,
            EMM_GET_STATE(),
...
boolean mm_sub_is_dds_sub(sys_modem_as_id_e_type as_id)
{
  ...
  return (mm_dds_sub == (mm_as_id_e_type)as_id);
#else
...
FUNCTION  mm_clear_pending_attach_req

DESCRIPTION
Clers the pending GMM attach request
...
void mm_clear_pending_attach_req(void)
{
  if(pending_gmm_msg != NULL && pending_gmm_msg_valid == TRUE)
  {
    if((pending_gmm_msg[4] & (0x0F)) == PD_GMM && pending_gmm_msg[5] == GMM_ATTACH_REQUEST)
    {
      pending_gmm_msg_after_release = NULL;
  ...
  if (mm_timer_status[ TIMER_T3302 ] != TIMER_STOPPED)
  {
...
)
{
  ...
  if(found == FALSE)
  {
    if(mm_sim_active_allowed_csg_list.length < SYS_CSG_LIST_MAX_LENGTH)
    {
      ...
      temp_csg = (sys_csg_list_info_s_type*)modem_mem_calloc(1, sizeof(sys_csg_list_info_s_type)*temp_csg_len, MODEM_MEM_CLIENT_NAS);
...
void mmsend_mm_csg_white_list_log_packet(uint8 type, sys_csg_list_s_type *csg_list)
{
   ...
   log_ptr = (LOG_UMTS_NAS_CSG_WHITE_LIST_LOG_PACKET_C_type *)nas_log_alloc(LOG_UMTS_NAS_CSG_WHITE_LIST_LOG_PACKET_C,
        sizeof(LOG_UMTS_NAS_CSG_WHITE_LIST_LOG_PACKET_C_type),mm_message_as_id);
...
void mmsend_mm_csg_available_list_log_packet(sys_detailed_plmn_list_s_type  *available_csg_p)
{
   ...
   log_ptr = (LOG_UMTS_NAS_CSG_AVAILABLE_LIST_LOG_PACKET_C_type *)nas_log_alloc(LOG_UMTS_NAS_CSG_AVAILABLE_LIST_LOG_PACKET_C,
        sizeof(LOG_UMTS_NAS_CSG_AVAILABLE_LIST_LOG_PACKET_C_type), mm_message_as_id);
...
)
{
  ...
  if(mm_sim_allowed_csg_list_read_started == TRUE || mm_sim_operator_csg_list_read_started == TRUE)
  {
    *search_result = REG_MM_PLMN_SEARCH_ABORTED;
    result = FALSE;
  }
  else if(csg_search_type == SYS_CSG_SEARCH_TYPE_LEGACY)
  {
    result = TRUE;
  }
  else
...
/*===========================================================================
FUNCTION  mm_is_csg_app_search_allowed

DESCRIPTION
 This function accepts CSG search type, CSG selection configuration and
 active subscription id as parameters and decides if CSG search initiated
 by CSG App should be allowed.

DEPENDENCIES
  None 

RETURN VALUE
  TRUE: If CSG App search allowed
  FALSE: If CSG App search not allowed

SIDE EFFECTS
  None
===========================================================================*/
boolean mm_is_csg_app_search_allowed
(
  sys_csg_search_type_e_type          csg_search_type,
  sys_csg_selection_config_e_type     config_option,
  sys_modem_as_id_e_type              as_id
)
{
  ...
  else
  {
    ...
    {
      case SYS_CSG_SELECTION_CONFIG_1:
        /* Reject CSG search if OCSGL is present and empty else allow search if:
           a) CSG search type is App initated and at least one entry in the operator
              list is not present in the rejected CSG list and
              timer TIMER_FORBIDDEN_OPERATOR_CSG_LIST is disabled for this csg entry  
           b) CSG search type is User initiated */
        if(
#ifdef FEATURE_DUAL_SIM
           mm_sim_ocsgl_present_sim[as_id] == TRUE
#else
          ...
          )
        {
          if(
#ifdef FEATURE_DUAL_SIM
             mm_sim_active_operator_csg_list_sim[as_id].length < 1
#else
             mm_sim_active_operator_csg_list.length < 1
#endif
            )
          {
            result = FALSE;
          ...
          {
#ifdef FEATURE_DUAL_SIM
            for(i=0; i<mm_sim_active_operator_csg_list_sim[as_id].length; i++)
#else
            for(i=0; i<mm_sim_active_operator_csg_list.length; i++)
#endif
            {
              if((mm_is_csg_in_rejected_list
                  (
#ifdef FEATURE_DUAL_SIM
                    mm_sim_active_operator_csg_list_sim[as_id].csg[i].plmn,
                    mm_sim_active_operator_csg_list_sim[as_id].csg[i].csg_id
#else
                    mm_sim_active_operator_csg_list.csg[i].plmn,
                    mm_sim_active_operator_csg_list.csg[i].csg_id
#endif
                 ...
                 &&
#ifdef FEATURE_DUAL_SIM
                 (mm_sim_active_operator_csg_list_sim[as_id].csg[i].timer_count ==
                                                  MM_TIMER_DISABLED)
#else
                 (mm_sim_active_operator_csg_list.csg[i].timer_count ==
                                                  MM_TIMER_DISABLED)
        ...
        else
        {
          ...
          if(csg_search_type == SYS_CSG_SEARCH_TYPE_APP_INIT)
          {
            if(
#ifdef FEATURE_DUAL_SIM
               mm_sim_active_allowed_csg_list_sim[as_id].length < 1
#else
               mm_sim_active_allowed_csg_list.length < 1
#endif
              )
            {
              result = FALSE;
            ...
            {
#ifdef FEATURE_DUAL_SIM
              for(i=0; i<mm_sim_active_allowed_csg_list_sim[as_id].length; i++)
#else
              for(i=0; i<mm_sim_active_allowed_csg_list.length; i++)
#endif
              {
                if( ( /* Check if the CSG was rejected for Reject Causes #13, #15 or AUTH_AUTHENTICATION_UNACCEPTABLE  */
                      ...
                      (
#ifdef FEATURE_DUAL_SIM
                        mm_sim_active_allowed_csg_list_sim[as_id].csg[i].plmn,
                        mm_sim_active_allowed_csg_list_sim[as_id].csg[i].csg_id
#else
                        mm_sim_active_allowed_csg_list.csg[i].plmn,
                        mm_sim_active_allowed_csg_list.csg[i].csg_id
#endif
                    ...
                    && /* Check if the CSG was rejected for Reject Cause #25 */
#ifdef FEATURE_DUAL_SIM
                    (mm_sim_active_allowed_csg_list_sim[as_id].csg[i].csg_id != SYS_CSG_ID_INVALID)
#else
                    (mm_sim_active_allowed_csg_list.csg[i].csg_id != SYS_CSG_ID_INVALID)
#endif
                  )
...
)
{
  ...
  if(available_csg_p->length > SYS_PLMN_LIST_MAX_LENGTH)
  {
    MSG_ERROR_DS_1(MM_SUB, "=MM= Truncating available length to MAX %d", SYS_PLMN_LIST_MAX_LENGTH);
    available_csg_p->length = SYS_PLMN_LIST_MAX_LENGTH;
  }
...
/*===========================================================================
   ...
   FUNCTION 
	 is_sim_swap_event_handled_by_only_one_task
   
   DESCRIPTION
        Gets the sim_swap_event_handled_by_only_one_task value.
	   
   RETURN VALUE
	 boolean - sim_swap_event_handled_by_only_one_task value
	
 ===========================================================================*/
boolean is_sim_swap_event_handled_by_only_one_task(sys_modem_as_id_e_type local_as_id)
{
  if(IS_NOT_VALID_AS_ID(local_as_id)) return FALSE;
  
  return sim_swap_event_handled_by_only_one_task_sim[local_as_id];
}

/*===========================================================================
	
   FUNCTION 
	 set_sim_swap_event_handled_by_only_one_task
   
   DESCRIPTION
        Sets the sim_swap_event_handled_by_only_one_task value with the given flag (TRUE/FALSE).
	   
   RETURN VALUE
	 None
	
 ===========================================================================*/
void set_sim_swap_event_handled_by_only_one_task(sys_modem_as_id_e_type local_as_id, boolean flag)
{
  if(IS_NOT_VALID_AS_ID(local_as_id)) return;

  sim_swap_event_handled_by_only_one_task_sim[local_as_id] = flag;
  MSG_HIGH_DS_2(MM_SUB, "=MM= set_sim_swap_event_handled_by_only_one_task[%d]=%d", 
            local_as_id, sim_swap_event_handled_by_only_one_task_sim[local_as_id]);
}
...
/*===========================================================================
   ...
   FUNCTION 
	 mm_sim_swap_allowed_csg_list
   
   DESCRIPTION
        Swaps active_allowed_csg_list of the current SUB with the respective data of the given SUB.
        Called when SIM soft swap is triggered to swap NV cache of the SUBs.
	   
   RETURN VALUE
	 None
	
 ===========================================================================*/
void mm_sim_swap_allowed_csg_list
(
  mm_as_id_e_type mm_as_id_target
)
{
  sys_csg_list_s_type mm_sim_allowed_csg_list_current = {0};
  boolean mm_using_allowed_csg_nv_current;
  uint8 i;

  /* Copy current SUB allowed CSG data to temp */
  mm_using_allowed_csg_nv_current = mm_using_allowed_csg_nv;
  if(mm_sim_active_allowed_csg_list.csg != NULL)
  {
    if(mm_sim_active_allowed_csg_list.length > 0)
    {
      mm_sim_allowed_csg_list_current.csg = 
             (sys_csg_list_info_s_type*)modem_mem_calloc(1, sizeof(sys_csg_list_info_s_type) * mm_sim_active_allowed_csg_list.length, MODEM_MEM_CLIENT_NAS);
      mm_check_for_null_ptr((void*) mm_sim_allowed_csg_list_current.csg);
      for(i = 0; i < mm_sim_active_allowed_csg_list.length; i++)
      {
        mm_sim_allowed_csg_list_current.csg[i] = mm_sim_active_allowed_csg_list.csg[i];
      }
    }
    mm_sim_allowed_csg_list_current.length = mm_sim_active_allowed_csg_list.length;
  }

  /* Copy target SUB CSG data to current SUB CSG */
  mm_using_allowed_csg_nv = mm_using_allowed_csg_nv_sim[mm_as_id_target];
  if(mm_sim_active_allowed_csg_list.csg)
  {
    modem_mem_free(mm_sim_active_allowed_csg_list.csg, MODEM_MEM_CLIENT_NAS);
	mm_sim_active_allowed_csg_list.csg = NULL;
  }
  if(mm_sim_active_allowed_csg_list_sim[mm_as_id_target].csg != NULL)
  {
    if(mm_sim_active_allowed_csg_list_sim[mm_as_id_target].length > 0)
    {
      mm_sim_active_allowed_csg_list.csg = 
             (sys_csg_list_info_s_type*)modem_mem_calloc(1, sizeof(sys_csg_list_info_s_type) * mm_sim_active_allowed_csg_list_sim[mm_as_id_target].length, MODEM_MEM_CLIENT_NAS);
      mm_check_for_null_ptr((void*) mm_sim_active_allowed_csg_list.csg);
      for(i = 0; i < mm_sim_active_allowed_csg_list_sim[mm_as_id_target].length; i++)
      {
        mm_sim_active_allowed_csg_list.csg[i] = mm_sim_active_allowed_csg_list_sim[mm_as_id_target].csg[i];
      }
    }
    mm_sim_active_allowed_csg_list.length = mm_sim_active_allowed_csg_list_sim[mm_as_id_target].length;
  }

  /* Copy temp data (current SUB CSG data) to target SUB CSG */
  mm_using_allowed_csg_nv_sim[mm_as_id_target] = mm_using_allowed_csg_nv_current;
  if(mm_sim_active_allowed_csg_list_sim[mm_as_id_target].csg)
  {
    modem_mem_free(mm_sim_active_allowed_csg_list_sim[mm_as_id_target].csg, MODEM_MEM_CLIENT_NAS);
    mm_sim_active_allowed_csg_list_sim[mm_as_id_target].csg = NULL;
  }
  if(mm_sim_allowed_csg_list_current.csg != NULL)
  {
    if(mm_sim_allowed_csg_list_current.length > 0)
    {
      mm_sim_active_allowed_csg_list_sim[mm_as_id_target].csg = 
             (sys_csg_list_info_s_type*)modem_mem_calloc(1, sizeof(sys_csg_list_info_s_type) * mm_sim_allowed_csg_list_current.length, MODEM_MEM_CLIENT_NAS);
      mm_check_for_null_ptr((void*) mm_sim_active_allowed_csg_list_sim[mm_as_id_target].csg);
      for(i = 0; i < mm_sim_allowed_csg_list_current.length; i++)
      {
        mm_sim_active_allowed_csg_list_sim[mm_as_id_target].csg[i] = mm_sim_allowed_csg_list_current.csg[i];
      }
    }
    mm_sim_active_allowed_csg_list_sim[mm_as_id_target].length = mm_sim_allowed_csg_list_current.length;

    /* Free temporary memory created */
    modem_mem_free(mm_sim_allowed_csg_list_current.csg, MODEM_MEM_CLIENT_NAS);
	mm_sim_allowed_csg_list_current.csg = NULL;
  }
}
#endif
...
/*===========================================================================
  ...
  FUNCTION 
	mm_sim_swap_backoff_remaining_info
  
  DESCRIPTION
	Swaps remainig t3246/t3346 value and PLMN info of the current SUB with the respective data of the given SUB.
	Called when SIM soft swap is triggered to swap NV cache of the SUBs.
...
===========================================================================*/
void mm_sim_swap_backoff_remaining_info
(
  mm_as_id_e_type mm_as_id_target
)
{
  mm_as_id_e_type mm_as_id_current = mm_as_id;
  dword mm_t3246_backoff_timer_value_current;
  dword gmm_t3346_backoff_timer_value_current;
  sys_plmn_id_s_type mm_3gpp_t3246_plmn_id_current;
  sys_plmn_id_s_type gmm_3gpp_t3346_plmn_id_current;
  uint8 i;

  /* Find backoff remaining time for current SUB */
  if(mm_timer_status[TIMER_T3246] == TIMER_ACTIVE)
  { 
    mm_t3246_backoff_timer_value = 0;
    /* Find out how much timeout is left */
    mm_t3246_backoff_timer_value = (dword) gs_enquire_timer_ext ( 
                                                 get_mm_timer_queue(), 
                                                 TIMER_T3246,
                                                 TRUE
                                              );
  ...
  /* Change context to target SUB and find backoff remaining time */
  ...
  /* Restore context to current SUB */
  mm_set_as_id(mm_as_id_current);

  mm_t3246_backoff_timer_value_current = mm_t3246_backoff_timer_value;
  gmm_t3346_backoff_timer_value_current = gmm_t3346_backoff_timer_value;
  for(i=0; i<3; i++)
  {
    mm_3gpp_t3246_plmn_id_current.identity[i] = mm_3gpp_t3246_plmn_id.identity[i];
    gmm_3gpp_t3346_plmn_id_current.identity[i] = gmm_3gpp_t3346_plmn_id.identity[i];
  }

  mm_t3246_backoff_timer_value = mm_t3246_backoff_timer_value_sim[mm_as_id_target];
  gmm_t3346_backoff_timer_value = gmm_t3346_backoff_timer_value_sim[mm_as_id_target];
  for(i=0; i<3; i++)
  {
    mm_3gpp_t3246_plmn_id.identity[i] = mm_3gpp_t3246_plmn_id_sim[mm_as_id_target].identity[i];
    gmm_3gpp_t3346_plmn_id.identity[i] = gmm_3gpp_t3346_plmn_id_sim[mm_as_id_target].identity[i];
  }

  mm_t3246_backoff_timer_value_sim[mm_as_id_target] = mm_t3246_backoff_timer_value_current;
  gmm_t3346_backoff_timer_value_sim[mm_as_id_target] = gmm_t3346_backoff_timer_value_current;
  for(i=0; i<3; i++)
  {
    mm_3gpp_t3246_plmn_id_sim[mm_as_id_target].identity[i] = mm_3gpp_t3246_plmn_id_current.identity[i];
    gmm_3gpp_t3346_plmn_id_sim[mm_as_id_target].identity[i] = gmm_3gpp_t3346_plmn_id_current.identity[i];
  }
}
#endif
...
DESCRIPTION
  Handles SIM swap scenario. Swaps MM specific NVs and corresponding caches of current SUB 
...
===========================================================================*/
void mm_handle_sim_swap(mm_as_id_e_type mm_as_id_target)
{
  mm_as_id_e_type mm_as_id_current = mm_as_id;
  boolean auth_gsm_gprs_keys_present_current = FALSE;
  auth_gsm_cipherkey_type auth_gsm_gprs_cipherkey_current;
  byte auth_gsm_gprs_cksn_current = 0;
  gmm_ptmsi_type ptmsi_current;
  gmm_ptmsi_signature_type ptmsi_signature_current;
  routing_area_id_T rai_current;
  gmm_update_status_type ra_update_status_current;
  uint8 i;

  memset(&auth_gsm_gprs_cipherkey_current, 0x00 , sizeof(auth_gsm_gprs_cipherkey_current));
  memset(&ptmsi_current, 0x00 , sizeof(ptmsi_current));
  memset(&ptmsi_signature_current, 0x00 , sizeof(ptmsi_signature_current));
  memset(&rai_current, 0x00 , sizeof(rai_current));

  /* Swap t3246, t3346 backoff remaining timer data for current SUB with target SUB */
#ifdef FEATURE_NAS_REL10
  mm_sim_swap_backoff_remaining_info(mm_as_id_target);
#endif

  /* Swap CSG list for current sub with target SUB */
#ifdef FEATURE_FEMTO_CSG
  mm_sim_swap_allowed_csg_list(mm_as_id_target);
#endif

  /* Swap GSM GPRS Kc CKSN for current sub with target SUB */
  auth_gsm_gprs_keys_present_current = auth_gsm_gprs_keys_present;
  auth_gsm_gprs_cksn_current = auth_gsm_gprs_cksn;
  auth_gsm_gprs_cipherkey_current.value_length = auth_gsm_gprs_cipherkey.value_length;
  if(auth_gsm_gprs_cipherkey.value_length < MAX_AUTH_DATA_LENGTH)
  {
    for(i = 0; i < auth_gsm_gprs_cipherkey.value_length; i++)
    {
      auth_gsm_gprs_cipherkey_current.value_data[i] = auth_gsm_gprs_cipherkey.value_data[i];
    }
  }

  auth_gsm_gprs_keys_present = auth_gsm_gprs_keys_present_sim[mm_as_id_target];
  auth_gsm_gprs_cksn = auth_gsm_gprs_cksn_sim[mm_as_id_target];
  auth_gsm_gprs_cipherkey.value_length = auth_gsm_gprs_cipherkey_sim[mm_as_id_target].value_length;
  if(auth_gsm_gprs_cipherkey_sim[mm_as_id_target].value_length < MAX_AUTH_DATA_LENGTH)
  {
    for(i = 0; i < auth_gsm_gprs_cipherkey_sim[mm_as_id_target].value_length; i++)
    {
      auth_gsm_gprs_cipherkey.value_data[i] = auth_gsm_gprs_cipherkey_sim[mm_as_id_target].value_data[i];
    }
  }

  auth_gsm_gprs_keys_present_sim[mm_as_id_target] = auth_gsm_gprs_keys_present_current;
  auth_gsm_gprs_cksn_sim[mm_as_id_target] = auth_gsm_gprs_cksn_current;
  auth_gsm_gprs_cipherkey_sim[mm_as_id_target].value_length = auth_gsm_gprs_cipherkey_current.value_length;
  if(auth_gsm_gprs_cipherkey_current.value_length < MAX_AUTH_DATA_LENGTH)
  {
    for(i = 0; i < auth_gsm_gprs_cipherkey_current.value_length; i++)
    {
      auth_gsm_gprs_cipherkey_sim[mm_as_id_target].value_data[i] = auth_gsm_gprs_cipherkey_current.value_data[i];
    }
  }

  /* Swap GSM GPRS LOC */
  (void)memscpy(ptmsi_current.digit, PTMSI_SIZE, gmm_stored_ptmsi.digit, PTMSI_SIZE);
  (void)memscpy(ptmsi_signature_current.value, PTMSI_SIGNATURE_SIZE, gmm_stored_ptmsi_signature.value, PTMSI_SIGNATURE_SIZE);
  (void)memscpy((byte *)&rai_current, RAI_SIZE, (byte *)&gmm_stored_rai, RAI_SIZE);
  ra_update_status_current = gmm_update_status;

  (void)memscpy(gmm_stored_ptmsi.digit, PTMSI_SIZE, gmm_stored_ptmsi_sim[mm_as_id_target].digit, PTMSI_SIZE);
  (void)memscpy(gmm_stored_ptmsi_signature.value, PTMSI_SIGNATURE_SIZE, gmm_stored_ptmsi_signature_sim[mm_as_id_target].value, PTMSI_SIGNATURE_SIZE);
  (void)memscpy((byte *)&gmm_stored_rai, RAI_SIZE, (byte *)&gmm_stored_rai_sim[mm_as_id_target], RAI_SIZE);
  gmm_update_status = gmm_update_status_sim[mm_as_id_target];

  (void)memscpy(gmm_stored_ptmsi_sim[mm_as_id_target].digit, PTMSI_SIZE, ptmsi_current.digit, PTMSI_SIZE);
  (void)memscpy(gmm_stored_ptmsi_signature_sim[mm_as_id_target].value, PTMSI_SIGNATURE_SIZE, ptmsi_signature_current.value, PTMSI_SIGNATURE_SIZE);
  (void)memscpy((byte *)&gmm_stored_rai_sim[mm_as_id_target], RAI_SIZE, (byte *)&rai_current, RAI_SIZE);
  gmm_update_status_sim[mm_as_id_target] = ra_update_status_current;

  /* Write current SUB data into NV/EFS */
  mm_write_nv_after_sim_swap();

  /* Change context to target SUB and write data into NV/EFS */
  mm_set_as_id(mm_as_id_target);
  mm_write_nv_after_sim_swap();

  /* Write target SUB PS LOCI into NV. PS LOCI NV item is used by both REG and MM. */
  mm_write_nv_psloci();

  /* Change context to current SUB and write PS LOCI into NV */
  mm_set_as_id(mm_as_id_current);
  mm_write_nv_psloci();

  /* Swap EMM specific NV items */
#ifdef FEATURE_LTE
  emm_handle_sim_swap(mm_as_id_target);
#endif

  /* REG and MM can process sim_available_req in parellel. IMSI need to be swapped by only task (MM or REG) but not by both.
     Whichever task performs swap later, it has to swap NV IMSI. Check if REG already processed SIM swap and swap NV IMSI. */
  NAS_ENTER_CRIT_SECT(mm_sim_swap_crit_sec);
  if(is_sim_swap_event_handled_by_only_one_task((sys_modem_as_id_e_type)mm_as_id))
  {
    /* REG task already processed SIM swap, lets swap NV IMSI also */
    MSG_HIGH_DS_2(MM_SUB, "=MM= SIM swap event is already processed by REG for AS IDs: %d - %d, swap NV IMSI ", mm_as_id, mm_as_id_target);
    mm_swap_nv_imsi(mm_as_id_target);
	set_sim_swap_event_handled_by_only_one_task((sys_modem_as_id_e_type)mm_as_id, FALSE);
    mm_debug_sim_swap_status = MM_DEBUG_SIM_CHANGED_SWAP_DONE_AT_REG_AND_MM;
  }
  else
  {
    /* REG task did not process SIM swap yet, let REG trigger NV IMSI swap */
    MSG_HIGH_DS_2(MM_SUB, "=MM= SIM swap event is not processed by REG yet for AS IDs: %d - %d ", mm_as_id, mm_as_id_target);
    set_sim_swap_event_handled_by_only_one_task((sys_modem_as_id_e_type)mm_as_id, TRUE);
    mm_debug_sim_swap_status = MM_DEBUG_SIM_CHANGED_SWAP_DONE_AT_MM;
  }
...
void mm_check_other_mm_cnm_connections()
{
...

   {
     ...
     for(as_id_ctr = SYS_MODEM_AS_ID_1; as_id_ctr < MAX_NAS_STACKS; as_id_ctr++)
     {
        ...
        {
          mm_set_as_id(as_id_ctr);
          if(mm_state == MM_WAIT_FOR_RR_CONNECTION_MM)
          {
            MSG_HIGH_DS_0(MM_SUB,"Sending MO call reject and aborting connection as MT call already in progress on other sub");
          ...
          else if(mm_csfb_state != MM_CSFB_STATE_NULL) 
          {
            ...
            mm_csfb_rej_pending_call(&mmcoord_pending_reg_message);
          }
#endif
        }
     }
     mm_set_as_id(buffer_mm_as_id);
   }
}
#endif
    ...
    if( mm_other_as_id != mm_tmp_as_id )
    {
      ...
      if(((is_umts_mode() == TRUE )||
              ...
              (mm_plmn_search_in_progress == TRUE) &&
               mm_emm_manual_search_prio_get() != SYS_SRCH_PRIORITY_LOW)
      {
...
FUNCTION  mm_check_plmn_search_in_progress_on_other_subs

DESCRIPTION: 
 
This function returns MM variable mm_plmn_search_in_progress on other sub
...
boolean mm_check_plmn_search_in_progress_on_other_subs(void)
{
  mm_as_id_e_type as_id = MM_OTHER_SUB(mm_as_id);
  return mm_plmn_search_in_progress_sim[as_id];
}
...
FUNCTION  mm_check_mms_progress_on_non_dds_sub

DESCRIPTION: 
 
This function returns MM variable indicates attach on demand due to MMS on other sub
...
boolean mm_check_mms_progress_on_non_dds_sub(void)
{
   boolean result = FALSE;
   mm_as_id_e_type buffer_as_id = mm_as_id;
   
   mm_set_as_id(MM_OTHER_SUB(mm_as_id));
#ifdef FEATURE_LTE
   if (is_lte_mode())
   {
     emm_ctrl_data_type *emm_ctrl_data_ptr = emm_db_get_ctrl_data();
     if ((MM_IS_NON_DDS_SUB) &&
         (emm_ctrl_data_ptr->rrc_active == TRUE) && /*Checking if LTE RAT is actually up or not*/
         ((mm_serving_plmn.ms_op_mode == SYS_SRV_DOMAIN_CS_PS) ||
          (*emm_ctrl_data_ptr->ds_proc_type_ptr == SYS_PROC_TYPE_MMS)))
     {
       MSG_HIGH_DS_0(MM_SUB,"=MM= MMS is progress in LTE ");
       result = TRUE;
     }
   }
   else 
   ...
   if ((is_gsm_mode() || is_umts_mode()) &&
        (MM_IS_NON_DDS_SUB) &&
        (!mm_deep_sleep_alt_scan) && /*Checking if ue not it 1x*/
        (((mm_serving_plmn.ms_op_mode == SYS_SRV_DOMAIN_CS_PS) &&
		  (mmcoord_pending_reg_message.cmd.mmr_reg_req.network_selection_mode != SYS_NETWORK_SELECTION_MODE_LIMITED)) ||
         (mm_ds_proc_type== SYS_PROC_TYPE_MMS)))
   {
     MSG_HIGH_DS_0(MM_SUB,"=MM= MMS is progress in GW ");
     result = TRUE;
   }

   mm_set_as_id(buffer_as_id);
   return result;
}
...
FUNCTION  mm_update_rat_status

DESCRIPTION: 
 
This function checks and update rat status of TD and LTE
  a) On sending service request or mode change indication to access stratum, this function should be called with
     rat priority list sent to access stratum and status as None
  b) On rat deactivation(stop confirmation from L/G/W/T), this function should be called with current rat pri list 
     and status as deactivated
  c) On sending stop mode confirmation to CM, this function should be called with rat priority list as NULL and 
     status as deactivated.
  d) List request is not allowed to change the TDS rat status to deactivated or deact in progress, so 
      If called from list request status parameter should always be set to activated.(Ex. service req does not have TDS 
      and list request has TDS).


For TDS RAT
==========
  1. If rat priority list is not NULL 
       - if T is part of rat priority list then TDS rat status will be set to activated.
       - if T is not part of rat priority list then TDS rat status will be set to deactivated if status sent is deactivated, 
          otherwise TDS status will be set to deactivation in progress.
  2.Otherwise if rat priority list is NULL and status is deactivated then TDS rat status will be set to deactivated.

For LTE RAT on NON DDS SUB
====================
  1. If rat priority list is not NULL 
       - if L is part of rat priority list then emm_send_rrc_other_sub_lte_status_ind will be called with status
           set to activated.
       - if L is not part of rat priority list then emm_send_rrc_other_sub_lte_status_ind will be called with status 
           set to deactivated if status sent is deactivated, otherwise status will be set to deactivation in progress.
  2.Otherwise if rat priority list is NULL and status is deactivated then emm_send_rrc_other_sub_lte_status_ind 
      will be called with status set to deactivated.
...
===========================================================================*/
void mm_update_rat_status
(
  sys_rat_pri_list_info_s_type  *rat_pri_list_info,
  sys_rat_activation_status status
)
{
#ifdef FEATURE_TDSCDMA
  boolean tds_present_in_rat_list = FALSE;
#endif

#if defined(FEATURE_LTE) && defined(FEATURE_DUAL_DATA)
  boolean lte_present_in_rat_list = FALSE;
  emm_ctrl_data_type *emm_ctrl_data_ptr = emm_db_get_ctrl_data_per_subs(MM_OTHER_SUB(mm_as_id));
#endif

  if(rat_pri_list_info != NULL)
  {
#if defined(FEATURE_LTE) && defined(FEATURE_DUAL_DATA)
    lte_present_in_rat_list = 
        mm_checkif_rat_pri_list_has_rat(rat_pri_list_info,SYS_RAT_LTE_RADIO_ACCESS);
#endif
#ifdef FEATURE_TDSCDMA
    tds_present_in_rat_list = 
        mm_checkif_rat_pri_list_has_rat(rat_pri_list_info,SYS_RAT_TDS_RADIO_ACCESS);
#endif
  }
  ...
  if(MM_IS_NON_DDS_SUB)
  {
    if(lte_present_in_rat_list)
    {
      if(status != SYS_RAT_ACTIVATION_STATUS_DEACTIVATED)
      {
        emm_send_rrc_other_sub_lte_status_ind(SYS_RAT_ACTIVATION_STATUS_ACTIVATED);
      }
    }
    else if(status == SYS_RAT_ACTIVATION_STATUS_DEACTIVATED)
    {
      emm_send_rrc_other_sub_lte_status_ind(SYS_RAT_ACTIVATION_STATUS_DEACTIVATED);
    }
    else if((emm_ctrl_data_ptr->last_non_dds_lte_status != SYS_RAT_ACTIVATION_STATUS_DEACTIVATED) &&
            (status != SYS_RAT_ACTIVATION_STATUS_ACTIVATED))
    {
      emm_send_rrc_other_sub_lte_status_ind(SYS_RAT_ACTIVATION_STATUS_DEACTIVATION_IN_PROGRESS);
    }
  }
#endif
#ifdef FEATURE_TDSCDMA
  if(tds_present_in_rat_list)
  {
    if(status != SYS_RAT_ACTIVATION_STATUS_DEACTIVATED)
    {
      mm_tds_rat_status_set(SYS_RAT_ACTIVATION_STATUS_ACTIVATED);
    }
  }
  else if(status == SYS_RAT_ACTIVATION_STATUS_DEACTIVATED)
  {
  ...
  /*
  ** If current status is already deactivated do not move to deactivation in progress
  ** If passed status is activated(list request scenario) do not move to deactivation in progress
  **  as original rat priority list could have TD.(ex. service req has TD but list req does not)
  */
  else if((mm_tds_rat_status != SYS_RAT_ACTIVATION_STATUS_DEACTIVATED) &&
          (status != SYS_RAT_ACTIVATION_STATUS_ACTIVATED))
  {
    mm_tds_rat_status_set(SYS_RAT_ACTIVATION_STATUS_DEACTIVATION_IN_PROGRESS);
  }
#endif
}
#endif
...
)
{
  ...
  for( i=0; ((i< plmn_list_ptr->length) && (i<SYS_PLMN_LIST_MAX_LENGTH)) && ((j>=1) && (j<=71)); i++)
  {
    ...
    nas_event_report_payload( EVENT_NAS_PLMN_FOUND, 71, (void*)event_packet,mm_message_as_id);
  }
}
...
boolean mm_in_conn_management_state(void)
{
...
/*===========================================================================
Function: mm_qsh_dump_data_get()

Description:
MM QSH dump data collection
============================================================================*/
void mm_qsh_dump_data_get
(
  mm_debug_data_s_type *data_ptr,
  mm_as_id_e_type as_id
)
{
#if defined FEATURE_DUAL_SIM || defined FEATURE_SGLTE
  data_ptr->gmm_detach_after_rau_pending_sim_debug = gmm_detach_after_rau_pending_sim[as_id];
  data_ptr->gmm_detach_in_progress_sim_debug = gmm_detach_in_progress_sim[as_id];
  data_ptr->gmm_follow_on_request_sim_debug = gmm_follow_on_request_sim[as_id];
  data_ptr->gmm_gprs_attach_pending_sim_debug = gmm_gprs_attach_pending_sim[as_id];
  data_ptr->gmm_gprs_detach_pending_sim_debug = gmm_gprs_detach_pending_sim[as_id];
  data_ptr->gmm_integrity_protection_activated_sim_debug = gmm_integrity_protection_activated_sim[as_id];
  data_ptr->gmm_is_prev_mode_connected_sim_debug = gmm_is_prev_mode_connected_sim[as_id];
  data_ptr->gmm_is_suspended_sim_debug = gmm_is_suspended_sim[as_id];
  data_ptr->gmm_power_down_active_sim_debug = gmm_power_down_active_sim[as_id];
  data_ptr->gmm_state_sim_debug = (gmm_state_debug_type)gmm_state_sim[as_id];
  data_ptr->gmm_substate_sim_debug = (gmm_substate_debug_type)gmm_substate_sim[as_id];
#ifdef FEATURE_LTE
  data_ptr->gmm_is_lte_advertised_in_last_reg_sim_debug = gmm_is_lte_advertised_in_last_reg_sim[as_id];
  data_ptr->mm_csfb_mt_call_info_ue_identity = (lte_nas_paging_identity_debug_type)mm_csfb_mt_call_info.ue_identity;
  data_ptr->mm_csfb_mt_call_info_mt_csfb_highPriority = mm_csfb_mt_call_info.mt_csfb_highPriority;
  data_ptr->mm_csfb_mt_call_info_debug_is_mt_call_recv_in_conn_state = mm_csfb_mt_call_info.is_mt_call_recv_in_conn_state;
  data_ptr->mm_csfb_state_sim_debug = (mm_csfb_state_debug_type)mm_csfb_state_sim[as_id];
#if defined FEATURE_DUAL_SIM
  data_ptr->mm_service_loss_on_multimode_rat_sim_debug = mm_service_loss_on_multimode_rat_sim[as_id];
#endif
#endif
  data_ptr->mm_waiting_for_regs_complete_txn_sim_debug = mm_waiting_for_regs_complete_txn_get(as_id);
  data_ptr->mm_dual_standby_mode_debug = mm_dual_standby_mode;
  data_ptr->mm_lu_pended_nmo_unknown_sim_debug = mm_lu_pended_nmo_unknown_sim[as_id];
  data_ptr->mm_ps_detach_state_sim_debug = (mm_ps_detach_state_debug_enum_T)mm_ps_detach_state_sim[as_id];
  data_ptr->mm_rlf_stop_mode_pending_sim_debug = (mm_rlf_stop_mode_pending_debug_enum_T)mm_rlf_stop_mode_pending_sim[as_id];
  data_ptr->mm_stop_mode_req_pended_sim_debug = mm_stop_mode_req_pended_sim[as_id];
  data_ptr->mm_subs_capability_mask_sim_debug = mm_subs_capability_mask_sim[as_id];
  data_ptr->mm_waiting_for_abort_cnf_sim_debug = mm_waiting_for_abort_cnf_sim[as_id];
  data_ptr->mm_waiting_for_open_session_cnf_sim_debug = mm_waiting_for_open_session_cnf_sim[as_id];
  data_ptr->mm_wcdma_rlf_state_sim_debug = (wcdma_rlf_state_debug_enum_T)mm_wcdma_rlf_state_sim[as_id];
  data_ptr->mm_wrlf_rat_change_pending_sim_debug = mm_wrlf_rat_change_pending_sim[as_id];
  data_ptr->mmcoord_reg_req_pending_sim_debug = mmcoord_reg_req_pending_sim[as_id];
  data_ptr->call_type_for_rrc_est_req_sim_debug = call_type_for_rrc_est_req_sim[as_id];
#ifdef FEATURE_INTERRAT_PCCO_GTOW
  data_ptr->GtoWCCO_state_sim_debug = (WtoGCCO_in_progress_debug_enum_T)GtoWCCO_state_sim[as_id];
#endif
#ifdef FEATURE_NAS_REL10
  data_ptr->mm_3gpp_t3246_plmn_id_sim_debug = mm_3gpp_t3246_plmn_id_sim[as_id];
#endif
  data_ptr->mm_abort_dual_switch_pending_sim_debug = mm_abort_dual_switch_pending_sim[as_id];
  data_ptr->mm_as_id_debug = (sys_modem_as_id_e_type)mm_as_id;
  data_ptr->mm_cnm_is_active_sim_debug = mm_cnm_is_active_sim[as_id];
  data_ptr->mm_connection_activated_sim_debug = mm_connection_activated_sim[as_id];
  data_ptr->mm_connection_type_sim_debug = (mm_connection_debug_type_T)mm_connection_type_sim[as_id];
#ifdef FEATURE_LTE
  data_ptr->mm_csfb_is_blind_redir_to_lte_req_to_be_sent_sim_debug = mm_csfb_is_blind_redir_to_lte_req_to_be_sent_sim[as_id];
#endif
  data_ptr->mm_deep_sleep_pending_sim_debug = mm_deep_sleep_pending_sim[as_id];
  data_ptr->mm_hard_abort_pending_sim_debug = mm_hard_abort_pending_sim[as_id];
  memscpy((void *) &data_ptr->mm_held_cnm_service_req_sim_debug,sizeof(held_service_request_debug_T),
            (void *) &mm_held_cnm_service_req_sim[as_id], sizeof(held_service_request_T));
  data_ptr->mm_idle_substate_sim_debug = mm_idle_substate_sim[as_id];
  data_ptr->mm_last_active_rat_stopped_sim_debug = mm_last_active_rat_stopped_sim[as_id];
  memscpy((void *) &data_ptr->mm_lu_reject_cause_sim_debug[0],MAX_LU_ATTEMPT_SIZE_DEBUG*sizeof(byte),
            (void *) &mm_lu_reject_cause_sim[as_id][0], MAX_LU_ATTEMPT_SIZE*sizeof(byte));
  data_ptr->mm_lu_start_reason_sim_debug = mm_lu_start_reason_sim[as_id];
  data_ptr->mm_msg_info_debug = mm_msg_info;
  data_ptr->mm_nreg_release_pending_sim_debug = mm_nreg_release_pending_sim[as_id];
  data_ptr->mm_nreg_req_received_sim_debug = mm_nreg_req_received_sim[as_id];
  data_ptr->mm_pended_stop_mode_req_sim_mmr_stop_mode_req = mm_pended_stop_mode_req_sim[as_id].mmr_stop_mode_req;
  data_ptr->mm_pended_stop_mode_req_sim_valid = mm_pended_stop_mode_req_sim[as_id].valid;
  data_ptr->mm_pending_lu_request_sim_debug = mm_pending_lu_request_sim[as_id];
  memscpy((void *) &data_ptr->mm_previous_serving_lai_sim_debug[0],LAI_SIZE*sizeof(byte),
            (void *) &mm_previous_serving_lai_sim[as_id][0], LAI_SIZE*sizeof(byte));
  data_ptr->mm_rat_change_pending_sim_debug = mm_rat_change_pending_sim[as_id];
  data_ptr->mm_rat_handler_status_sim_debug = (mm_rat_handler_status_debug_type)mm_rat_handler_status_sim[as_id];
  data_ptr->mm_rat_pri_list_info_sim_debug = mm_rat_pri_list_info_sim[as_id];

  data_ptr->mm_ready_to_send_reg_cnf_sim_debug = mm_ready_to_send_reg_cnf_sim[as_id];
#ifdef FEATURE_WRLF_SYSTEM_SEL
  data_ptr->mm_received_suitable_search_end_ind_sim_debug = mm_received_suitable_search_end_ind_sim[as_id];
#endif
  data_ptr->mm_reg_waiting_for_reg_cnf_sim_debug = mm_reg_waiting_for_reg_cnf_sim[as_id];
  data_ptr->mm_serv_change_pending_sim_debug = mm_serv_change_pending_sim[as_id];
  data_ptr->mm_serving_cell_id_sim_debug = mm_serving_cell_id_sim[as_id];
  memscpy((void *) &data_ptr->mm_serving_plmn_sim_debug, sizeof(serving_plmn_info_debug_T),
           (void *) &mm_serving_plmn_sim[as_id], sizeof(serving_plmn_info_T));
  data_ptr->mm_state_sim_debug = mm_state_sim[as_id];
#ifdef FEATURE_LTE
  data_ptr->mm_stop_mode_local_detach_peding_sim_debug = mm_stop_mode_local_detach_peding_sim[as_id];
#endif
  data_ptr->mm_stored_imsi_sim_length = mm_stored_imsi_sim[as_id].length;
  memscpy((void *) &data_ptr->mm_stored_imsi_sim_data[0],IMSI_LEN_IN_BYTES*sizeof(byte),
           (void *) &mm_stored_imsi_sim[as_id].data[0], IMSI_LEN_IN_BYTES*sizeof(byte));
#ifdef FEATURE_NAS_REL10
  data_ptr->mm_stored_per_ms_t3212_value_sim_debug = mm_stored_per_ms_t3212_value_sim[as_id];
#endif
  data_ptr->mm_sub_config_sim_debug = mm_sub_config_sim[as_id];
  data_ptr->mm_sub_id_debug = (sys_modem_as_id_e_type)mm_sub_id;
  data_ptr->mm_system_information_sim_debug = mm_system_information_sim[as_id];
#ifdef FEATURE_NAS_REL10
  data_ptr->mm_t3246_backoff_timer_value_sim_debug = mm_t3246_backoff_timer_value_sim[as_id];
#endif
  memscpy((void *) &data_ptr->mm_timer_status_sim_debug[0],sizeof(data_ptr->mm_timer_status_sim_debug),
            (void *) &mm_timer_status_sim[as_id][0], sizeof(mm_timer_status_sim[as_id]));
  data_ptr->mm_waiting_for_deact_cnf_sim_debug = mm_waiting_for_deact_cnf_sim[as_id];
  data_ptr->mm_waiting_for_ph_status_change_cnf_sim_debug = mm_waiting_for_ph_status_change_cnf_sim[as_id];
  data_ptr->mm_waiting_for_service_cnf_sim_debug = mm_waiting_for_service_cnf_sim[as_id];
  data_ptr->mm_waiting_for_stop_mode_cnf_sim_debug = mm_waiting_for_stop_mode_cnf_sim[as_id];
  data_ptr->mmcoord_svc_cnf_pending_sim_debug = mmcoord_svc_cnf_pending_sim[as_id];
  data_ptr->mmode_nv_setting_for_device_mode_debug = mmode_nv_setting_for_device_mode;
#ifdef FEATURE_SPEC_COMPLIANT_WTOG_CELL_CHANGE_ORDER
  data_ptr->WtoGCCO_state_sim_debug = (WtoGCCO_in_progress_debug_enum_T)WtoGCCO_state_sim[as_id];
#endif
  data_ptr->lu_cause_sim_debug = mm_get_lu_cause_per_subs(as_id);
  data_ptr->mm_plmn_search_in_progress_sim_debug = mm_plmn_search_in_progress_sim[as_id];
  data_ptr->mm_waiting_for_list_cnf_type_sim_debug = mm_waiting_for_list_cnf_type_sim[as_id];
  data_ptr->mm_interrat_state_sim_debug = (mm_interrat_change_state_debug_type)mm_interrat_state_sim[as_id];
  data_ptr->mm_service_loss_on_rat_sim_debug = mm_service_loss_on_rat_sim[as_id];
  data_ptr->cs_session_status_sim_debug = (mmrrc_connection_status_debug_T)cs_session_status_sim[as_id];
  data_ptr->mm_cs_domain_possible_reest_sim_debug = mm_cs_domain_possible_reest_sim[as_id];
  data_ptr->mm_ps_domain_possible_reest_sim_debug = mm_ps_domain_possible_reest_sim[as_id];
  data_ptr->pending_gmm_msg_after_release_sim_debug = pending_gmm_msg_after_release_sim[as_id];
  data_ptr->pending_gmm_msg_cause_sim_debug = pending_gmm_msg_cause_sim[as_id];
  //data_ptr->pending_gmm_msg_sim_debug = pending_gmm_msg_sim[as_id];
  data_ptr->pending_gmm_msg_valid_sim_debug = pending_gmm_msg_valid_sim[as_id];
  data_ptr->pending_mm_msg_after_release_sim_debug = pending_mm_msg_after_release_sim[as_id];
  data_ptr->pending_mm_msg_cause_sim_debug = pending_mm_msg_cause_sim[as_id];
  //data_ptr->pending_mm_msg_sim_debug = pending_mm_msg_sim[as_id];
  data_ptr->pending_mm_msg_valid_sim_debug = pending_mm_msg_valid_sim[as_id];
  data_ptr->ps_session_status_sim_debug = (mmrrc_connection_status_debug_T)ps_session_status_sim[as_id];
  data_ptr->rrc_connection_status_sim_debug =(mmrrc_connection_status_debug_T) rrc_connection_status_sim[as_id];
#else
  data_ptr->gmm_detach_after_rau_pending_sim_debug = gmm_detach_after_rau_pending;
  data_ptr->gmm_detach_in_progress_sim_debug = gmm_detach_in_progress;
  data_ptr->gmm_follow_on_request_sim_debug = gmm_follow_on_request;
  data_ptr->gmm_gprs_attach_pending_sim_debug = gmm_gprs_attach_pending;
  data_ptr->gmm_gprs_detach_pending_sim_debug = gmm_gprs_detach_pending;
  data_ptr->gmm_integrity_protection_activated_sim_debug = gmm_integrity_protection_activated;
  data_ptr->gmm_is_prev_mode_connected_sim_debug = gmm_is_prev_mode_connected;
  data_ptr->gmm_is_suspended_sim_debug = gmm_is_suspended;
  data_ptr->gmm_power_down_active_sim_debug = gmm_power_down_active;
  data_ptr->gmm_state_sim_debug = (gmm_state_debug_type)gmm_state;
  data_ptr->gmm_substate_sim_debug = (gmm_substate_debug_type)gmm_substate;
#ifdef FEATURE_LTE
  data_ptr->gmm_is_lte_advertised_in_last_reg_sim_debug = gmm_is_lte_advertised_in_last_reg;
  data_ptr->mm_csfb_mt_call_info_ue_identity = (lte_nas_paging_identity_debug_type)mm_csfb_mt_call_info.ue_identity;
  data_ptr->mm_csfb_mt_call_info_mt_csfb_highPriority = mm_csfb_mt_call_info.mt_csfb_highPriority;
  data_ptr->mm_csfb_mt_call_info_debug_is_mt_call_recv_in_conn_state = mm_csfb_mt_call_info.is_mt_call_recv_in_conn_state;
  data_ptr->mm_csfb_state_sim_debug = (mm_csfb_state_debug_type)mm_csfb_state;
#if defined FEATURE_DUAL_SIM
  data_ptr->mm_service_loss_on_multimode_rat_sim_debug = mm_service_loss_on_multimode_rat;
#endif
#endif
  data_ptr->mm_waiting_for_regs_complete_txn_sim_debug = mm_waiting_for_regs_complete_txn_get(as_id);
  data_ptr->mm_lu_pended_nmo_unknown_sim_debug = mm_lu_pended_nmo_unknown;
  data_ptr->mm_ps_detach_state_sim_debug = (mm_ps_detach_state_debug_enum_T)mm_ps_detach_state;
  data_ptr->mm_rlf_stop_mode_pending_sim_debug = (mm_rlf_stop_mode_pending_debug_enum_T)mm_rlf_stop_mode_pending;
  data_ptr->mm_stop_mode_req_pended_sim_debug = mm_stop_mode_req_pended;
  data_ptr->mm_waiting_for_abort_cnf_sim_debug = mm_waiting_for_abort_cnf;
  data_ptr->mm_waiting_for_open_session_cnf_sim_debug = mm_waiting_for_open_session_cnf;
  data_ptr->mm_wcdma_rlf_state_sim_debug = (wcdma_rlf_state_debug_enum_T)mm_wcdma_rlf_state;
  data_ptr->mm_wrlf_rat_change_pending_sim_debug = mm_wrlf_rat_change_pending;
  data_ptr->mmcoord_reg_req_pending_sim_debug = mmcoord_reg_req_pending;
  data_ptr->call_type_for_rrc_est_req_sim_debug = call_type_for_rrc_est_req;
#ifdef FEATURE_INTERRAT_PCCO_GTOW
  data_ptr->GtoWCCO_state_sim_debug = (WtoGCCO_in_progress_debug_enum_T)GtoWCCO_state;
#endif
#ifdef FEATURE_NAS_REL10
  data_ptr->mm_3gpp_t3246_plmn_id_sim_debug = mm_3gpp_t3246_plmn_id;
#endif
  data_ptr->mm_as_id_debug = (sys_modem_as_id_e_type)mm_as_id;
  data_ptr->mm_cnm_is_active_sim_debug = mm_cnm_is_active;
  data_ptr->mm_connection_activated_sim_debug = mm_connection_activated;
  data_ptr->mm_connection_type_sim_debug = (mm_connection_debug_type_T)mm_connection_type;
#ifdef FEATURE_LTE
  data_ptr->mm_csfb_is_blind_redir_to_lte_req_to_be_sent_sim_debug = mm_csfb_is_blind_redir_to_lte_req_to_be_sent;
#endif
  data_ptr->mm_deep_sleep_pending_sim_debug = mm_deep_sleep_pending;
  data_ptr->mm_hard_abort_pending_sim_debug = mm_hard_abort_pending;
  memscpy((void *) &data_ptr->mm_held_cnm_service_req_sim_debug,sizeof(held_service_request_debug_T),
            (void *) &mm_held_cnm_service_req, sizeof(held_service_request_T));
  data_ptr->mm_idle_substate_sim_debug = mm_idle_substate;
  data_ptr->mm_last_active_rat_stopped_sim_debug = mm_last_active_rat_stopped;
  memscpy((void *) &data_ptr->mm_lu_reject_cause_sim_debug[0],MAX_LU_ATTEMPT_SIZE_DEBUG*sizeof(byte),
            (void *) &mm_lu_reject_cause[0], MAX_LU_ATTEMPT_SIZE*sizeof(byte));
  data_ptr->mm_lu_start_reason_sim_debug = mm_lu_start_reason;
  data_ptr->mm_msg_info_debug = mm_msg_info;
  data_ptr->mm_nreg_release_pending_sim_debug = mm_nreg_release_pending;
  data_ptr->mm_nreg_req_received_sim_debug = mm_nreg_req_received;
  data_ptr->mm_pended_stop_mode_req_sim_mmr_stop_mode_req = mm_pended_stop_mode_req.mmr_stop_mode_req;
  data_ptr->mm_pended_stop_mode_req_sim_valid = mm_pended_stop_mode_req.valid;
  data_ptr->mm_pending_lu_request_sim_debug = mm_pending_lu_request;
  memscpy((void *) &data_ptr->mm_previous_serving_lai_sim_debug[0],LAI_SIZE*sizeof(byte),
            (void *) &mm_previous_serving_lai[0], LAI_SIZE*sizeof(byte));
  data_ptr->mm_rat_change_pending_sim_debug = mm_rat_change_pending;
  data_ptr->mm_rat_handler_status_sim_debug = (mm_rat_handler_status_debug_type)mm_rat_handler_status;
  data_ptr->mm_rat_pri_list_info_sim_debug = mm_rat_pri_list_info;

  data_ptr->mm_ready_to_send_reg_cnf_sim_debug = mm_ready_to_send_reg_cnf;
#ifdef FEATURE_WRLF_SYSTEM_SEL
  data_ptr->mm_received_suitable_search_end_ind_sim_debug = mm_received_suitable_search_end_ind;
#endif
  data_ptr->mm_reg_waiting_for_reg_cnf_sim_debug = mm_reg_waiting_for_reg_cnf;
  data_ptr->mm_serv_change_pending_sim_debug = mm_serv_change_pending;
  data_ptr->mm_serving_cell_id_sim_debug = mm_serving_cell_id;
  memscpy((void *) &data_ptr->mm_serving_plmn_sim_debug, sizeof(serving_plmn_info_debug_T),
           (void *) &mm_serving_plmn, sizeof(serving_plmn_info_T));
  data_ptr->mm_state_sim_debug = mm_state;
#ifdef FEATURE_LTE
  data_ptr->mm_stop_mode_local_detach_peding_sim_debug = mm_stop_mode_local_detach_peding;
#endif
  data_ptr->mm_stored_imsi_sim_length = mm_stored_imsi.length;
  memscpy((void *) &data_ptr->mm_stored_imsi_sim_data[0],IMSI_LEN_IN_BYTES*sizeof(byte),
           (void *) &mm_stored_imsi.data[0], IMSI_LEN_IN_BYTES*sizeof(byte));
#ifdef FEATURE_NAS_REL10
  data_ptr->mm_stored_per_ms_t3212_value_sim_debug = mm_stored_per_ms_t3212_value;
#endif
  data_ptr->mm_sub_id_debug = (sys_modem_as_id_e_type)mm_sub_id;
  data_ptr->mm_system_information_sim_debug = mm_system_information;
#ifdef FEATURE_NAS_REL10
  data_ptr->mm_t3246_backoff_timer_value_sim_debug = mm_t3246_backoff_timer_value;
#endif
  memscpy((void *) &data_ptr->mm_timer_status_sim_debug[0],MAX_MM_TIMER_DEBUG*sizeof(byte),
            (void *) &mm_timer_status[0], (MAX_MM_TIMER_DEBUG+1)*sizeof(byte));
  data_ptr->mm_waiting_for_deact_cnf_sim_debug = mm_waiting_for_deact_cnf;
  data_ptr->mm_waiting_for_ph_status_change_cnf_sim_debug = mm_waiting_for_ph_status_change_cnf;
  data_ptr->mm_waiting_for_service_cnf_sim_debug = mm_waiting_for_service_cnf;
  data_ptr->mm_waiting_for_stop_mode_cnf_sim_debug = mm_waiting_for_stop_mode_cnf;
  data_ptr->mmcoord_svc_cnf_pending_sim_debug = mmcoord_svc_cnf_pending;
  data_ptr->mmode_nv_setting_for_device_mode_debug = mmode_nv_setting_for_device_mode;
#ifdef FEATURE_SPEC_COMPLIANT_WTOG_CELL_CHANGE_ORDER
  data_ptr->WtoGCCO_state_sim_debug = (WtoGCCO_in_progress_debug_enum_T)WtoGCCO_state;
#endif
  data_ptr->lu_cause_sim_debug = mm_get_lu_cause_per_subs(as_id);
  data_ptr->mm_plmn_search_in_progress_sim_debug = mm_plmn_search_in_progress;
  data_ptr->mm_waiting_for_list_cnf_type_sim_debug = mm_waiting_for_list_cnf_type;
  data_ptr->mm_interrat_state_sim_debug = (mm_interrat_change_state_debug_type)mm_interrat_state;
  data_ptr->mm_service_loss_on_rat_sim_debug = mm_service_loss_on_rat;
  data_ptr->cs_session_status_sim_debug = (mmrrc_connection_status_debug_T)cs_session_status;
  data_ptr->mm_cs_domain_possible_reest_sim_debug = mm_cs_domain_possible_reest;
  data_ptr->mm_ps_domain_possible_reest_sim_debug = mm_ps_domain_possible_reest;
  data_ptr->pending_gmm_msg_after_release_sim_debug = pending_gmm_msg_after_release;
  data_ptr->pending_gmm_msg_cause_sim_debug = pending_gmm_msg_cause;
  //data_ptr->pending_gmm_msg_sim_debug = pending_gmm_msg;
  data_ptr->pending_gmm_msg_valid_sim_debug = pending_gmm_msg_valid;
  data_ptr->pending_mm_msg_after_release_sim_debug = pending_mm_msg_after_release;
  data_ptr->pending_mm_msg_cause_sim_debug = pending_mm_msg_cause;
  //data_ptr->pending_mm_msg_sim_debug = pending_mm_msg;
  data_ptr->pending_mm_msg_valid_sim_debug = pending_mm_msg_valid;
  data_ptr->ps_session_status_sim_debug = (mmrrc_connection_status_debug_T)ps_session_status;
  data_ptr->rrc_connection_status_sim_debug = (mmrrc_connection_status_debug_T)rrc_connection_status;
#endif
}
...
Description:
MM EMM QSH Debug buffer for dump collection.
============================================================================*/

void mm_emm_qsh_dump_debug_buffer_get
(
  mm_qsh_debug_buffer_s_type *debug_buffer_ptr
)
{
  uint16 count;
  int32 buffer_index = (int32)mm_emm_debug_buffer_index;

  for(count = 0; count < MM_DEBUG_DUMP_BUFFER_SIZE; count++)
  {
    buffer_index = (int32)buffer_index - 1;
    if(buffer_index < 0)
    {
      buffer_index = MM_DEBUG_MESSAGE_BUFFER_SIZE - 1;
    }
    debug_buffer_ptr[count] = mm_emm_debug_buffer[buffer_index];
  }
}
...
/*===========================================================================
Function: mm_is_serv_req_cause_changed_leading_opt_plmn_srch

Description:
While NAS receives another cm_service_req when it didn't receive any confirmation for already sent rrc_service_req,
This function returns TRUE or FALSE as following:
  =>TRUE:  if service_req_cause in current service_req is changed such that some optimised search will 
           be performed at lower layer or vice versa.
  =>FALSE: Otherwise.

============================================================================*/

boolean mm_is_serv_req_cause_changed_leading_opt_plmn_srch( rrc_mm_service_req_cause_e_type service_req_cause )
{
  boolean result = TRUE;
  if((mm_last_sent_serv_req_params_ptr->service_req_cause == service_req_cause) ||
     (((mm_last_sent_serv_req_params_ptr->service_req_cause == RRC_NORMAL_ACQ) || (mm_last_sent_serv_req_params_ptr->service_req_cause == RRC_MO_PS_CALL)) && 
      ((service_req_cause == RRC_MO_PS_CALL) || (service_req_cause == RRC_NORMAL_ACQ))))
  {
     result = FALSE;
  }
  return result;
...
/*===========================================================================
Function: mm_get_serv_req_cause

Description:
Returns service_req_cause for current service_req.

============================================================================*/

rrc_mm_service_req_cause_e_type mm_get_serv_req_cause(mm_cmd_type *message)
{
  ...
  {
    service_req_cause = RRC_MO_CS_CALL;
  }
#ifdef FEATURE_LTE
  ...
  {
    service_req_cause = RRC_MO_CS_ECALL;
  }
#endif
  else if(gmm_follow_on_request)
  {
    service_req_cause = RRC_MO_PS_CALL;
  }
  else
  {
    service_req_cause = RRC_NORMAL_ACQ;
  }
  return service_req_cause;
}

/*===========================================================================

FUNCTION mm_sub_is_rat_enabled
...
boolean mm_sub_is_rat_enabled(sys_radio_access_tech_e_type rat)
{
#if defined(FEATURE_SGLTE) || defined(FEATURE_DUAL_SIM)
  boolean is_sub_capable = FALSE;

  switch (rat) 
  {
    case SYS_RAT_GSM_RADIO_ACCESS:
      if(mm_subs_capability_mask & SYS_SYS_MODE_MASK_GSM)
      {
        is_sub_capable = TRUE;
      }
      break;
    ...
    #endif
      if(mm_subs_capability_mask & SYS_SYS_MODE_MASK_WCDMA)
      {
        is_sub_capable = TRUE;
      }
      break;
    ...
    #endif
      if(mm_subs_capability_mask & SYS_SYS_MODE_MASK_LTE)
      {
        is_sub_capable = TRUE;
      }
     break;
    ...
    #endif
      if(mm_subs_capability_mask & SYS_SYS_MODE_MASK_TDS)
      {
        is_sub_capable = TRUE;
      }
     break;
    ...
    default:
      MSG_ERROR_DS( MM_SUB, "=MM= Unknown RAT: %d", rat, 0, 0);
      break;
  }


  return is_sub_capable;
#else
  return TRUE;
#endif
}
...
/*===========================================================================
Function: mm_get_is_ehplmn_to_be_camped

Description:
Returns boolean value depending upon ehplmn to be camped or not as a result of current service_req.

============================================================================*/

boolean mm_get_is_ehplmn_to_be_camped(void)
{
  boolean is_ehplmn_to_be_camped;
#ifdef FEATURE_NAS_REL11
  is_ehplmn_to_be_camped = reg_mode_get_hplmn_to_selected(
#if defined FEATURE_DUAL_SIM || defined FEATURE_SGLTE
                                                          (sys_modem_as_id_e_type)mm_as_id
#endif
                                                          );
#else
  is_ehplmn_to_be_camped = FALSE;
#endif
  return is_ehplmn_to_be_camped;
}


/*===========================================================================
Function: mm_per_subs_is_DO_activation_allowed

Description:
.This API will return 0(Continue) if UE is in SRLTE mode, 
  returns 1(Continue and WAIT) if rat priority list does not have T and
  returns 2 otherwise
...
NAS_EXIT_CRIT_SECT(mm_rat_prioritylist_crit_sec);
...
#endif 
  {
    activation_allowed = SYS_ACTIVATION_ALLOWED_CONTINUE;
          ...
          (mm_checkif_rat_pri_list_has_rat(&rat_list,
    ...
    case MS_MM_REG:
      switch (message->cmd.hdr.message_id)
      {
        ...
        case MMR_DDS_SWITCH_IND:
          mm_msg_info = (byte)message->cmd.mmr_dds_switch_ind.target_dds_sub;
          break;
#endif
        case MMR_HIGH_PRIORITY_ACTIVITY_IND:
          mm_msg_info = (byte)message->cmd.mmr_high_priority_activity_ind.local_detach;
          break;
...
/*===========================================================================
Function: mm_check_dual_tds_concurrency

Description:
  This function returns TRUE if detects T + T concurrency

============================================================================*/
boolean mm_check_dual_tds_concurrency
(
  sys_rat_pri_list_info_s_type *rat_priority_list
)
{
  mm_as_id_e_type other_as_id = MM_OTHER_SUB(mm_as_id);
  if ((mm_dual_standby_mode == TRUE) &&
      (mm_checkif_rat_pri_list_has_rat(rat_priority_list,SYS_RAT_TDS_RADIO_ACCESS))&&
      ((mm_tds_rat_status_sim[other_as_id] == SYS_RAT_ACTIVATION_STATUS_ACTIVATED) ||
       (mm_tds_rat_status_sim[other_as_id] == SYS_RAT_ACTIVATION_STATUS_DEACTIVATION_IN_PROGRESS)))
  {
    MSG_HIGH_DS_1(other_as_id+1,"=MM= TDS active status %d", mm_tds_rat_status_sim[other_as_id]);
    return TRUE;
  }
  return FALSE;
}
...
/*===========================================================================
Function: mm_check_ignore_mmr_reg_req

Description:
This function checks if to ignore REG request for avoiding T+T rat concurrency

============================================================================*/
boolean mm_check_ignore_mmr_reg_req(sys_rat_pri_list_info_s_type *rat_priority_list)
{
#ifdef FEATURE_LTE
  emm_ctrl_data_type *emm_ctrl_data_ptr = NULL;
#endif
  /*
    Ignoring REG_REQ in order to avoid the concurrency of T + T:
      Apply following condition only when UE is in Dual Standby Mode.
      If Current SUB REG_REQ is received with T in RAT priority list and other SUB also has T
      then Ignore REG_REQ for retry.
  */
  mm_reg_req_ignored = FALSE;
  if(mm_check_dual_tds_concurrency(rat_priority_list))
  {
    MSG_HIGH_DS_0(MM_SUB, 
      "=MM= Ignored REG request with TDS in rat list as other rat TDS is not deactivated");
    mm_reg_req_ignored = TRUE;
#ifdef FEATURE_LTE
	emm_ctrl_data_ptr = emm_db_get_ctrl_data();
	ASSERT(emm_ctrl_data_ptr != NULL);
	if(emm_ctrl_data_ptr->reg_req_pending_mm_cmd_ptr != NULL)
	{
    ...
    {
#ifdef FEATURE_LTE
     if(EMM_GET_CONNECTION_STATE() == EMM_RELEASING_RRC_CONNECTION_STATE && is_lte_mode())
     {
  ...
  return mm_reg_req_ignored; 
}

/*===========================================================================

FUNCTION mm_resume_ignored_mmr_reg_req
...
===========================================================================*/
void mm_resume_ignored_mmr_reg_req(mm_as_id_e_type as_id)
{
  if (mm_reg_req_ignored_sim[as_id])
  {
    /* Request for last REG REQ on the other SUB */
    mm_send_mmr_reg_req_ignored_ind((sys_modem_as_id_e_type)as_id);
    mm_reg_req_ignored_sim[as_id] = FALSE;
  }
}

#endif

boolean mm_check_if_emcall_allowed(void)
{
  ...
  if((mmcoord_pending_reg_message.cmd.hdr.message_set == MS_MM_REG &&
      ...
      (mmcoord_pending_reg_message.cmd.mmr_reg_req.network_selection_mode == SYS_NETWORK_SELECTION_MODE_LIMITED )
#ifdef FEATURE_LTE
      &&(emm_bit_mask_check(mmcoord_pending_reg_message.cmd.mmr_reg_req.additional_info,(byte)SYS_ADDTNL_INFO_EMERG_CALL) == TRUE)
#endif
       ) ||
       (mm_check_if_cs_cell_access_barred(mm_get_cs_cell_access(PROCEDURE_CS_REGISTRATION)) == TRUE) ||
       ...
       mm_timer_status [TIMER_T3210] == TIMER_STOPPED && 
           ...
           (
#ifdef FEATURE_NAS_REL10
           ((mm_timer_status[TIMER_T3246] == TIMER_ACTIVE) && (mm_is_plmn_equivalent_t3246_plmn())) ||
               ((mm_serving_plmn.ms_op_mode == SYS_SRV_DOMAIN_CS_PS) &&
               (mm_serving_plmn.network_op_mode == NETWORK_OP_MODE_1) &&
...
FUNCTION mm_check_other_sub_call_in_progress

DESCRIPTION 
  This function checks if there is any MO/MT CS call
...
===========================================================================*/
boolean mm_check_other_sub_call_in_progress
(
   mm_as_id_e_type as_id
)
{
   ...
   {
     emm_ctrl_data_type *emm_ctrl_data_ptr = emm_db_get_ctrl_data();
     if (emm_get_lte_process_type(emm_ctrl_data_ptr) == SYS_PROC_TYPE_CS_CALL)
     {
       result = TRUE;
     }
   }
...
#endif
   if ( (is_gsm_mode() || is_umts_mode()) &&
        (mm_evaluate_proc_type() == SYS_PROC_TYPE_CS_CALL) )
   {
     result = TRUE;
   }
...
FUNCTION mm_check_and_abort_sms_conn

DESCRIPTION 
  This function checks if there is any on going SMS that needs to be aborted
...
===========================================================================*/
void mm_check_and_abort_sms_conn
(
  mm_as_id_e_type sub_id
)
{
  mm_as_id_e_type buffer_sub_id = mm_as_id;
  mm_set_as_id(sub_id);

#ifdef FEATURE_LTE
  if (is_lte_mode())
  {
     emm_check_and_abort_sms();
  }
  else
#endif
  /*Abort SMS over CS in GSM. PS SMS should not be aborted. */
  if ( is_gsm_mode() && 
       (mm_evaluate_proc_type() == SYS_PROC_TYPE_SMS) &&
       !mm_sms_is_active )
  {
  ...
  else if (is_umts_mode() && (mm_evaluate_proc_type() == SYS_PROC_TYPE_SMS))
  {
    if (mm_sms_is_active == TRUE)
    {
  ...
  mm_set_as_id(buffer_sub_id);
}
#endif

/*===========================================================================

FUNCTION mm_get_gsm_wcdma_amr_codec_per_subs

DESCRIPTION 
  This function returns the value of global "gsm_wcdma_amr_codec" 
...
===========================================================================*/
byte mm_get_gsm_wcdma_amr_codec_per_subs
(
#if defined FEATURE_DUAL_SIM || defined FEATURE_SGLTE
  sys_modem_as_id_e_type as_id
#else
...
)
{
  ...
  if (IS_NOT_VALID_STACK_ID(as_id)) 
  { 
   MSG_ERROR_DS(MM_SUB,"mm_get_gsm_wcdma_amr_codec_per_subs - Invalid STACK ID: %d ", as_id+1,0,0 ); 
   return 0x0f; 
  }  
#endif

#if defined FEATURE_DUAL_SIM || defined FEATURE_SGLTE
 return gsm_wcdma_amr_codec_preference_sim[as_id];
#else
  return gsm_wcdma_amr_codec_preference;
#endif
}
...
)
{
  ...
  if(IS_NOT_VALID_SUB_ID(sub_id))
  {
    ...
    return NV_NAS_COMPLIANCE_REL11;
  }
  else
  {
...
FUNCTION mm_get_full_srv_req_in_emerg_per_subs

DESCRIPTION 
  This function returns the value of global "full_srv_req_in_emerg" 
...
boolean mm_get_full_srv_req_in_emerg_per_subs( 
#if defined FEATURE_DUAL_SIM || defined FEATURE_SGLTE
  sys_modem_as_id_e_type sub_id
#else
  void
...
)
{
  ...
  if(IS_NOT_VALID_SUB_ID(sub_id))
  {
	MSG_ERROR_DS(MM_SUB, "=MM= mm_get_full_srv_req_in_emerg_per_subs : Invalid AS ID: %d", sub_id+1,0,0);
	return FALSE;
  }
  else
  {
	return full_srv_req_in_emerg_sim[sub_id];
  }
#else
  return full_srv_req_in_emerg;
#endif
}

/*===========================================================================

FUNCTION mm_get_nv_wcdma_hsupa_category_per_subs

DESCRIPTION 
  This function returns the value of global "mm_nv_wcdma_hsupa_category" 
...
extern byte mm_get_nv_wcdma_hsupa_category_per_subs
(
#if defined FEATURE_DUAL_SIM || defined FEATURE_SGLTE
  sys_modem_as_id_e_type sub_id
#else
...
{
  byte mm_nv_wcdma_hsupa_category = MM_RRC_WCMDA_HSUPA_DEFAULT_CATEGORY;
#ifdef FEATURE_SEGMENT_LOADING
  ...
  if(IS_NOT_VALID_SUB_ID(sub_id))
  {
	MSG_ERROR_DS(MM_SUB, "=MM= mm_get_nv_wcdma_hsupa_category_per_subs : Invalid AS ID: %d", sub_id+1,0,0);
	return mm_nv_wcdma_hsupa_category;
  }
  else
  {
#ifdef FEATURE_WCDMA
#ifndef FEATURE_SEGMENT_LOADING
     mm_nv_wcdma_hsupa_category = (byte)rrc_ext_get_wcdma_hsupa_category((sys_modem_as_id_e_type)sub_id); 
#else //#ifndef FEATURE_SEGMENT_LOADING
    if(ptr_W != NULL)
    {
      mm_nv_wcdma_hsupa_category = (byte)IWCDMA_rrc_ext_get_wcdma_hsupa_category(ptr_W, (sys_modem_as_id_e_type)sub_id);
    }
#endif
#endif //#ifdef FEATURE_WCDMA  
  }
...
#ifdef FEATURE_WCDMA
#ifndef FEATURE_SEGMENT_LOADING
     mm_nv_wcdma_hsupa_category = (byte)rrc_ext_get_wcdma_hsupa_category((sys_modem_as_id_e_type)mm_sub_id); 
#else //#ifndef FEATURE_SEGMENT_LOADING
    if(ptr_W != NULL)
    {
      mm_nv_wcdma_hsupa_category = (byte)IWCDMA_rrc_ext_get_wcdma_hsupa_category(ptr_W, (sys_modem_as_id_e_type)mm_sub_id);
    }
#endif
#endif //#ifdef FEATURE_WCDMA  
...
FUNCTION mm_active_rat_per_subs

DESCRIPTION 
  Returns active RAT's per subscription
...
QSH_MDUMP_FN_ATTR
#endif
void mm_active_rat_per_subs(sys_active_rat_info_s_type *active_rat_info)
{
 int i;
  for(i=0;i<MAX_AS_IDS;i++)
  {
#if defined(FEATURE_DUAL_SIM) || defined(FEATURE_TRIPLE_SIM)
    active_rat_info->active_rat[i]=mm_serving_plmn_sim[i].info.active_rat;
#else
    active_rat_info->active_rat[i]=mm_serving_plmn.info.active_rat;
#endif
  }
}

#if defined(FEATURE_DUAL_DATA) && defined(FEATURE_LTE)
/*===========================================================================

FUNCTION mm_get_effective_req_srv_domain

DESCRIPTION 
  Returns effective service domain in case doamin is CS_VOLTE or VOLTE only.
  GSM/UMTS/TDSCDMA will treat CS_VOLTE as CS_only
  LTE will treat CS_VOLTE as CS_PS and VOLTE as PS only
...
===========================================================================*/
sys_srv_domain_e_type  mm_get_effective_req_srv_domain
(
...
{
  sys_srv_domain_e_type effective_domain = srv_domain;
  ...
  switch (srv_domain)
  {
    ...
    case SYS_SRV_DOMAIN_CS_ONLY:
      if ((rat_lte ==TRUE) &&
           (volte_only == TRUE))
      {
        effective_domain = SYS_SRV_DOMAIN_CS_PS;   
      }
      break;
    ...
    case SYS_SRV_DOMAIN_PS_ONLY:
      if ((rat_lte == FALSE) &&
         (volte_only == TRUE))
      {
        effective_domain = SYS_SRV_DOMAIN_NO_SRV;   
      }
      break;
    ...
    case SYS_SRV_DOMAIN_CS_PS:
      if ((rat_lte == FALSE) &&
         (volte_only == TRUE))
      {
        effective_domain = SYS_SRV_DOMAIN_CS_ONLY;   
      }
      break;

    default:
      effective_domain = srv_domain; 
  }
...
FUNCTION mm_compare_ms_op_mode
...
===========================================================================*/
boolean mm_compare_ms_op_mode_per_sub(sys_modem_as_id_e_type as_id, sys_srv_domain_e_type srv_domain)
{
  boolean result = FALSE; 
  ...
  {
    if(emm_ctrl_data_ptr->reg_req_pending_ptr->volte_only == FALSE)        
    {
      if(mm_serving_plmn_sim[as_id].ms_op_mode == srv_domain)
      {
        result = TRUE;
      }
    }
    ...
    {
      if(((srv_domain == SYS_SRV_DOMAIN_CS_VOLTE) &&
           ((emm_ctrl_data_ptr->reg_req_pending_ptr->req_service_domain == SYS_SRV_DOMAIN_CS_PS) ||
            (emm_ctrl_data_ptr->reg_req_pending_ptr->req_service_domain == SYS_SRV_DOMAIN_CS_ONLY))) ||
           ((srv_domain == SYS_SRV_DOMAIN_VOLTE) &&
...
#else
  (void)as_id;
  if(mm_serving_plmn.ms_op_mode == srv_domain)
  {
    result = TRUE;
  }
#endif
...
{
  emm_ctrl_data_type   *emm_ctrl_data_ptr;
  mm_as_id_e_type      mm_sub_id_local,mm_as_id_local;
  MSG_HIGH_DS_0(MM_SUB,"=MM= Disabling CSG as device is Multi SIM");
  emm_ctrl_data_ptr = emm_db_get_ctrl_data();
  mm_sub_id_local = mm_sub_id;
  mm_as_id_local = mm_as_id;
#ifdef FEATURE_DUAL_SIM  
  for(mm_sub_id = MM_AS_ID_1, mm_as_id = MM_AS_ID_1; mm_sub_id < mm_sim_max_subscription; mm_sub_id++, mm_as_id++)
#endif
  {
	 reg_nv_set_csg_support(SYS_CSG_SUPPORT_DISABLED,(reg_as_id_e_type)mm_sub_id);
  ...
  mm_sub_id = mm_sub_id_local;
  mm_as_id = mm_as_id_local;
}
/*===========================================================================

FUNCTION mm_redirection_allowed_status_ind

DESCRIPTION 
  This function returns the value to indicate if redirection is allowed
...
boolean mm_redirection_allowed_status_ind(sys_plmn_id_s_type plmn)
{
...
#endif 
   {
     if(mm_check_plmn_forbidden_national_roaming(tim_plmn))
     {
       return FALSE;
     }
   }
...
#ifdef FEATURE_TDSCDMA
cs_domain_no_response_e_type mm_per_subs_get_cs_domain_no_rsp_type
(
 sys_modem_as_id_e_type as_id
 )
{
	cs_domain_no_response_e_type response_type;
	response_type = CALL_STATE_NORMAL;
#ifdef FEATURE_DUAL_SIM
	if((cm_get_operator_info((sys_modem_as_id_e_type)as_id) == OPERATOR_CMCC) && (mm_state_sim[as_id] == MM_WAIT_FOR_NETWORK_COMMAND) 
		&& (mm_per_subs_is_mt_page_in_progress(as_id) == TRUE) 
		&& (is_ps_tdscdma_mode((sys_modem_as_id_e_type)as_id) == TRUE) 
		&& (mm_cnm_rel_req_received_sim[as_id] == FALSE)
#else
    if((cm_get_operator_info(SYS_MODEM_AS_ID_1) == OPERATOR_CMCC) && (mm_state == MM_WAIT_FOR_NETWORK_COMMAND)
		&& (mm_is_mt_page_in_progress() == TRUE)
		&& (is_tdscdma_mode())
	...
	{
		response_type = CALL_STATE_MT_PAGE_NO_SETUP; 
	}

    MSG_HIGH_DS_1(MM_LOCAL_SUB, "=MM= NAS API for no call response type %d",response_type);
...
boolean mm_is_dos_mitigation_required_for_cs_domain( byte msg_id,byte reject_cause)
{
  boolean dos_mitigation_reqd = FALSE;
  MSG_HIGH_DS_2(MM_SUB,"USIM non gprs counter is -%d and max value of dos mitigation counter is - %d",mm_emm_usim_non_gprs_counter,mm_emm_max_attempt_counter_dos_mitigation);
  if( mm_get_t3247_behavior_enabled()) 
  {
     switch(msg_id)
     {
        case LOCATION_UPDATING_REJECT:
          if(mm_integrity_protected_during_lu == FALSE)
          {
             switch(reject_cause)
             {
                case ILLEGAL_MS:
                ...
                case IMSI_UNKNOWN_IN_HLR:
                  if(mm_emm_usim_non_gprs_counter < mm_emm_max_attempt_counter_dos_mitigation)
                  {
                     dos_mitigation_reqd = TRUE;
                  }
                  break;
                case LA_NOT_ALLOWED:
                case NATIONAL_ROAMING_NOT_ALLOWED:
          ...
          (reg_sim_plmn_forbidden(mm_serving_plmn.info.plmn))
				  	...
				  	{
                         dos_mitigation_reqd = FALSE;
				   	}
                  break;
                default:
                  MSG_HIGH_DS_2(MM_SUB,"DSOS Mitigation is not required for the reject message - %d and cause - %d ",msg_id,reject_cause);
                  break;
           }
          }
            break;
          ...
          if( mm_integrity_protected == FALSE) 
          {
             switch(reject_cause)
             {
                case ILLEGAL_ME:
                  ...
                  break;
                default:
                MSG_HIGH_DS_2(MM_SUB,"DOS Mitigation is not required for the reject message - %d and cause - %d ",msg_id,reject_cause);
                break;
             }
          }
               break;
        case ABORT:
          if( mm_integrity_protected == FALSE) 
          {
            ...
            break;
        case AUTHENTICATION_REJECT:
          if( mm_integrity_protected == FALSE && (mm_emm_usim_non_gprs_counter < mm_emm_max_attempt_counter_dos_mitigation)) 
          {
             dos_mitigation_reqd = TRUE;
          }
          break;
        default:
          MSG_HIGH_DS_1(MM_SUB,"DSOS Mitigation is not required for the  message -  %d ",msg_id);
          break;
     }
  }
  return dos_mitigation_reqd ;
}

void mm_perform_dos_mitigation_action(byte msg_id,byte reject_cause)
{

  if(msg_id == AUTHENTICATION_REJECT )
  {
     mm_emm_usim_non_gprs_counter ++;
	 mm_clear_lu_attempt_counter();
	 MSG_HIGH_DS_2(MM_SUB,"=MM= mm_emm_usim_non_gprs_counter is - %d and LU attempt counter is -  %d",mm_emm_usim_non_gprs_counter,mm_lu_attempt_counter);
     mm_add_forbidden_national_roaming( (inter_task_lai_T *) &mm_serving_lai[0],TRUE);
     mm_add_lai_in_reject_list((inter_task_lai_T *) &mm_serving_lai[0],TRUE);
	 mm_serving_plmn.update_equiv_plmn_list = TRUE;
	 memset(&mm_serving_plmn.equiv_plmn_list, 0x00, sizeof(equiv_plmn_list_s_type));
	 MSG_HIGH_DS_0(MM_SUB,"=MM= Deleted EPLMN list ");
     mm_update_mm_serving_plmn_info( mm_serving_plmn.info.sim_state,
                                     SYS_SRV_DOMAIN_NO_SRV,
                                     FALSE,
                                     SYS_SRV_STATUS_LIMITED,
  ...
  else
  {
     switch(reject_cause)
     {
        ...
        if (mm_reg_waiting_for_reg_cnf)
        {
           MSG_HIGH_DS_0(MM_SUB,"sending MMR REG CNF");
           mmcoord_mmr_reg_cnf.service_state = mm_serving_plmn.info;
        ...
        {
		   mmr_service_ind_s_type   mmr_service_ind;
           MSG_HIGH_DS_0(MM_SUB,"sending MMR SERVICE IND");
           mmr_service_ind.service_state = mm_serving_plmn.info;
           mmr_service_ind.cause = MMR_CAUSE_FAILURE_LAI_NOT_ALLOWED;
           mm_send_mmr_service_ind( &mmr_service_ind );
        }
            break;
        case LA_NOT_ALLOWED:
         ...
         ** --------------------------------------------------------- */
         ...
         /* Store LAI in forbidden_LAI_list_for_regional_provision_of_service */
          mm_add_forbidden_regional_roaming( (inter_task_lai_T *) &mm_serving_lai[0], TRUE);
#endif
           break;
        case NATIONAL_ROAMING_NOT_ALLOWED:
          mm_add_forbidden_national_roaming( (inter_task_lai_T *) &mm_serving_lai[0],TRUE );
        ...
        case PLMN_NOT_ALLOWED:
          ...
          else if ( mm_emm_max_attempt_counter_dos_mitigation > 1 )
          {
             mm_add_plmn_to_fplmn_list( mm_serving_plmn.info.plmn, TRUE); 
          }
          else
          {
...
#ifdef FEATURE_LTE
void cm_update_non_acq_param_to_nas(sys_non_acq_param_s_type non_acq_param)
{
    non_acq_param_change_info_s_type non_acq_param_ind;
    memset((void *)&non_acq_param_ind, 0x00, sizeof(non_acq_param_change_info_s_type));

  if(emm_bit_mask_check(non_acq_param.nonAcqValue,
                            (byte)SYS_NONACQ_INFO_VOICE_DOMAIN_PREF) == TRUE)
  {
    non_acq_param_ind.as_id = non_acq_param.as_id;
    non_acq_param_ind.non_acq_info = SYS_NONACQ_INFO_VOICE_DOMAIN_PREF;
    non_acq_param_ind.voice_domain_pref = non_acq_param.voice_domain_pref;
    mm_post_non_acq_param_change_ind(non_acq_param_ind);
  }
}

/*===========================================================================
Function: mm_post_non_acq_param_change_ind()

Description:
Post a message to own task for non acq params change ind
============================================================================*/

void mm_post_non_acq_param_change_ind(non_acq_param_change_info_s_type param_info)
{
  mm_cmd_type *mm_cmd_p;
  if ((mm_cmd_p = mm_get_cmd_buf()) != NULL)
  {
     /*
     ** Construct the MM_NON_ACQ_PARAM_CHANGE_IND command.
     */
     mm_cmd_p->cmd.non_acq_param_change_ind.message_header.message_set = MS_MM_MM;
     mm_cmd_p->cmd.non_acq_param_change_ind.message_header.message_id = MM_NON_ACQ_PARAM_CHANGE_IND;
   
     PUT_IMH_LEN (sizeof(mm_cmd_p->cmd.non_acq_param_change_ind) - sizeof(IMH_T),
                      &(mm_cmd_p->cmd.non_acq_param_change_ind.message_header));
   
    mm_cmd_p->cmd.non_acq_param_change_ind.as_id = (sys_modem_as_id_e_type)param_info.as_id;
    mm_cmd_p->cmd.non_acq_param_change_ind.non_acq_info = param_info.non_acq_info;
    mm_cmd_p->cmd.non_acq_param_change_ind.voice_domain_pref = param_info.voice_domain_pref;

   MSG_HIGH_DS_0(MM_SUB,"=MM= Send MM_NON_ACQ_PARAM_CHANGE_IND");
...
boolean non_acq_bit_mask_check
(
  uint32 value,
  byte bit_mask
)
...
void mm_emm__max_manual_search_timer_set(sys_arfcn_list_s_type arfcn_list,sys_rat_pri_list_info_s_type rat_pri_list_info)
{
  uint8 i=0,count =0;
  uint32 num_of_rat = rat_pri_list_info.num_items;
  MSG_HIGH_DS_3(MM_SUB,"=EMM= num_of_rat %d wcdma_count %d lte_count %d ",num_of_rat,arfcn_list.wcdma_count,arfcn_list.lte_count); 
  mm_emm_manual_search_timer_value.MaxManualPLMNTimer = 180;
  for(i=0;i<num_of_rat;i++)
  {
    if(rat_pri_list_info.priority_list_info[i].acq_sys_mode == SYS_SYS_MODE_WCDMA && arfcn_list.wcdma_count != 0)
    {
     count++;
    }
	else if(rat_pri_list_info.priority_list_info[i].acq_sys_mode == SYS_SYS_MODE_LTE && arfcn_list.lte_count !=0)
	{
	 count++;
	}
  }
  if(count == num_of_rat)
  {
    mm_emm_manual_search_timer_value.MaxManualPLMNTimer = 20;
	MSG_HIGH_DS_0(MM_SUB,"=EMM= overwriting EFS value to new value of 20 sec as list has valid frequencies ");
  }
}

void mm_emm_manual_search_timer_set(uint8 periodicity,sys_arfcn_list_s_type  arfcn_list,sys_rat_pri_list_info_s_type rat_pri_list_info)
{
  if(periodicity != 0)
  {
    mm_emm_manual_search_timer_value.FirstPLMNTimer = periodicity;
    mm_emm_manual_search_timer_value.MorePLMNTimer = periodicity;
	mm_emm__max_manual_search_timer_set(arfcn_list,rat_pri_list_info);
  }
  else
  {
   	MSG_HIGH_DS_0(MM_SUB,"=EMM= Periodicity provided by CM is not valid, EFS should have been set ");
  }
 
}

void mm_emm_manual_search_param_set( sys_srch_priority_e_type   srch_priority,boolean    enabled)
{
	mm_emm_manual_search_param.srch_priority = srch_priority;
	mm_emm_manual_search_param.enabled = enabled;
}

boolean mm_emm_manual_search_partial_get( void)
{
   MSG_HIGH_DS_1(MM_SUB,"=EMM= partail Search eanbled %d ",mm_emm_manual_search_param.enabled); 
  return mm_emm_manual_search_param.enabled;
}

sys_srch_priority_e_type mm_emm_manual_search_prio_get( void)
{
 MSG_HIGH_DS_1(MM_SUB,"=EMM= Search priority %d ",mm_emm_manual_search_param.srch_priority); 
 return mm_emm_manual_search_param.srch_priority;
}

boolean mm_per_subs_is_ss_in_progress
(
   sys_modem_as_id_e_type as_id
)
{
#if defined FEATURE_DUAL_SIM
   return mm_is_ss_request_rrc_sent_sim[as_id];
#else
   return mm_is_ss_request_rrc_sent;
#endif

}

void mm_set_is_ss_request_rrc_sent
(
   boolean value
)
{
   mm_is_ss_request_rrc_sent = value;
}
#ifdef FEATURE_DISABLED_HABANERO  
#ifdef FEATURE_LTE
/*==============================================================================
...
mm_emm_irat_during_call

==============================================================================*/
	boolean mm_emm_irat_during_call
	(
	   lte_rrc_act_reason_e act_reason
	)
	{
	  boolean irat_during_call_est = FALSE;
	  if (mm_state == MM_WAIT_FOR_RR_CONNECTION_MM && 
		  ...
		  (mm_cnm_srv_est_cause == MM_RR_MO_FULL_SPEECH ||
		   ...
		   mm_cnm_srv_est_cause == MM_RR_CALL_REEST ||
		   mm_cnm_srv_est_cause == MM_RR_EMERGENCY_CALL) &&
		 ((mm_serving_plmn.info.active_rat == SYS_RAT_UMTS_RADIO_ACCESS
			&& (mm_rrc_est_cause_sent_for_call_get()== TRUE)) ||
		  (mm_serving_plmn.info.active_rat == SYS_RAT_GSM_RADIO_ACCESS &&
			(act_reason == LTE_RRC_ACT_IRAT_REDIR))))
	  {
		irat_during_call_est = TRUE;
	  }
	  return irat_during_call_est;
	}
...
mm_set_as_id_rrc_abnormal_call_end_ind
==============================================================================*/
void mm_set_as_id_rrc_abnormal_call_end_ind(sys_modem_as_id_e_type as_id)
{
  mm_as_id_rrc_abnormal_end_ind = as_id;
}

/*==============================================================================
FUNCTION NAME:

mm_get_as_id_rrc_abnormal_call_end_ind
==============================================================================*/
sys_modem_as_id_e_type mm_get_as_id_rrc_abnormal_call_end_ind(void)
{
  return mm_as_id_rrc_abnormal_end_ind;
} 
#ifdef FEATURE_HA_MAINLINE_ONLY_ENABLE
#ifdef FEATURE_DUAL_SIM
boolean mm_is_call_in_progress(sys_modem_as_id_e_type as_id )
{
  boolean result = FALSE;
  if(mm_proc_status_sim[as_id][SYS_PROC_TYPE_CS_CALL] == TRUE)
  {
    result = TRUE;
  }
  MSG_HIGH_DS_1(MM_SUB,"=MM= mm_is_call_in_progress is - %d",result);
  return result;
...
#endif
/*===========================================================================
 FUNCTION MM_MAP_CAUSE
 
 DESCRIPTION
   This function returns the reject cause value based on rat
...
reject_cause_T mm_map_cause()
{
	if(mm_serving_plmn.info.active_rat == SYS_RAT_GSM_RADIO_ACCESS)
	{
		return MM_REJ_ACCESS_CLASS_BLOCKED_GSM;
	}
	else if(mm_serving_plmn.info.active_rat == SYS_RAT_UMTS_RADIO_ACCESS)
	{
		return MM_REJ_ACCESS_CLASS_BLOCKED_WCDMA;
	}
	else if(mm_serving_plmn.info.active_rat == SYS_RAT_TDS_RADIO_ACCESS)
	{
		return MM_REJ_ACCESS_CLASS_BLOCKED_TDSCDMA;
	}
	else if(mm_serving_plmn.info.active_rat == SYS_RAT_LTE_RADIO_ACCESS)
	{
		return MM_REJ_ACCESS_CLASS_BLOCKED_LTE;
	}

	return MM_REJ_ACCESS_CLASS_BLOCKED;
