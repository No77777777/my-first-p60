...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/mm/src/mmsecurity.c#1 $    $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...
ssg_mm_rand_type rand_val_store_mm_sim[MAX_NAS_STACKS];
#define rand_val_store_mm rand_val_store_mm_sim[mm_as_id]
#else
static auth_rand_type rand_value_cache;
static auth_autn_type autn_cache;
static byte  ciphering_key_sequence_number_cache;
ssg_mm_rand_type rand_val_store_mm;
#endif
...
===========================================================================*/
void store_rand_mm(auth_rand_type *p_rand)
{
	rand_val_store_mm.value_length=p_rand->value_length;
	memscpy((void *)&rand_val_store_mm.value_data[0], sizeof(rand_val_store_mm.value_data), 
...
===========================================================================*/
ssg_mm_rand_type get_rand_mm(void)
{
	return rand_val_store_mm;
}

/*===========================================================================
  ...
  for ( ie_count=0; ie_count < mm_no_of_ies; ie_count++ )
  {
    switch (mm_IE_info[ie_count].IEI)
    {
      ...
      case IEI_RAND :
		...
		store_rand_mm(p_rand);
		mm_ssgccs_handle_icoming_msg(AUTHENTICATION_REQUEST,mm_serving_plmn.info.active_rat,CS_DOMAIN_PROCEDURE,(mm_cmd_type *)p_rand);
        break ;
      case IEI_AUTN :
...
)
{
  ...
  *message_buffer_length = 3 ;
  ...
  mm_qsh_ota_metric_signaling_log(AUTHENTICATION_FAILURE,UE_TO_CN_DIRECTION, 0xff,(mm_gmm_reject_cause_debug_type)auth_error);
#endif
...
)
{
  ...
  switch (auth_error)
  {
    case AUTH_NO_ERROR:
	  ...
	  mm_ssgccs_handle_outgoing_msg(MM_AUTHENTICATION_RESPONSE_MSG,NO_PAYLOAD,mm_serving_plmn.info.active_rat );
    ...
    case AUTH_AUTHENTICATION_UNACCEPTABLE:
	  ...
	  mm_ssgccs_handle_outgoing_msg(MM_AUTHENTICATION_FAILURE_MSG,auth_error,mm_serving_plmn.info.active_rat );
...
void mm_perform_reject_actions( rrc_cn_domain_identity_e_type domain_id)
{
  ...
  *--------------------------------------------------------*/
  ...
  if(domain_id == RRC_CS_DOMAIN_CN_ID && mm_is_dos_mitigation_required_for_cs_domain(AUTHENTICATION_REJECT,ILLEGAL_ME))
  {
    is_legacy_required = FALSE;
    mm_perform_dos_mitigation_action(AUTHENTICATION_REJECT,ILLEGAL_ME);
  }
  else
  {

  if(mm_get_t3247_behavior_enabled() && domain_id == RRC_CS_DOMAIN_CN_ID)
  {
  ...
  ** ------------------------------------------------------------------------------------ */
  ...
  if( !((domain_id == RRC_PS_DOMAIN_CN_ID) && 
  	    (gmm_is_dos_mitigation_required_for_ps_domain(GMM_AUTHENTICATION_AND_CIPHERING_REJECT, ILLEGAL_ME))))
  {
  ...
  if(mm_get_t3247_behavior_enabled() && (domain_id == RRC_CS_DOMAIN_CN_ID) && mm_integrity_protected == FALSE)
  {
