...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/mm/src/mm_gstk.c#1 $ $ $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...
void mm_send_gstk_no_service_info(void)
{
  ...
  boolean additional_info_bit_mask =     FALSE;
     ...
     )
  {
    ...
    additional_info_bit_mask = emm_bit_mask_check(mmcoord_pending_reg_message.cmd.mmr_reg_req.additional_info,
                                                     (byte)SYS_ADDTNL_INFO_GEO_DETERMINATION_PHASE);
  }
  MSG_HIGH_DS_4(MM_SUB,"=MM= additional_info_bit_mask = %d, MMSS_GEO_SEARCH = %d, net_sel_mode = %d, mm_follow_on_request = %d",
                              additional_info_bit_mask,
                              is_geo_det_mode, 
...
void mm_send_gstk_mm_idle_info()
{
       ...
       ))
  {
	 ...
	 additional_info_bit_mask = emm_bit_mask_check(mmcoord_pending_reg_message.cmd.mmr_reg_req.additional_info,
                                                                              (byte)SYS_ADDTNL_INFO_GEO_DETERMINATION_PHASE);
     MSG_HIGH_DS_1(MM_SUB,"=MM= additional info says geo determination = %d", additional_info_bit_mask);
#endif
  }
