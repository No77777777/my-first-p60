...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/mm/src/msg_lib_decode_esm.c#1 $
$Author: pwbldsvc $
...
)
{
  ...
  ASSERT(pos != NULL);
  ASSERT(buffer_length != NULL);
  ASSERT(pdn_disconnect_rej != NULL);
  ASSERT(esm_len_in_bits != NULL);
...
)
{
  ...
  ASSERT(pos != NULL);
  ASSERT(buffer_length != NULL);
  ASSERT(bearer_resource_modifi_rej != NULL);
  ASSERT(esm_len_in_bits != NULL);
...
)
{
  ...
  ASSERT(pos != NULL);
  ASSERT(buffer_length != NULL);
  ASSERT(modify_bearer_ctxt_req != NULL);
  ASSERT(esm_len_in_bits != NULL);
...
)
{
  ...
  ASSERT(pos != NULL);
  ASSERT(buffer_length != NULL);
  ASSERT(bearer_resource_alloc_rej != NULL);
  ASSERT(esm_len_in_bits != NULL);
 ...
 )
{
  ...
  word original_buffer_len = 0;
    ...
    ASSERT(pos != NULL);
    ASSERT(buffer_length != NULL);
    ASSERT(deactivate_bearer_context_req != NULL);
    ASSERT(esm_len_in_bits != NULL);
...
)
{
  ...
  ASSERT(pos != NULL);
  ASSERT(buffer_length != NULL);
  ASSERT(def_bearer_context_req != NULL);
  ASSERT(esm_len_in_bits != NULL);
...
{ 
  ...
  nas_event_report_payload(EVENT_LTE_ESM_OTA_INCOMING_MSG, sizeof(event),
  ...
  switch(message_id)
  {
    ...
    case PDN_CONNECTIVITY_REJECT:
      ...
      {
        SKIP_DECODING_ESM(pos, buffer_length, esm_len_in_bits);
        MSG_ERROR_DS_1(MM_SUB, "=NAS_MSG_LIB= PDN CONNECTIVITY REJECT PTI mismatch, expected PTI = %d. Stop decoding",
      ...
      valid_ie = decode_pdn_conn_rej(src,
                                     ...
                                     &esm_len_in_bits);
      MSG_HIGH_DS_1(MM_SUB, "=NAS_MSG_LIB= PDN CONNECTIVITY REJECT decoding PASSED(1)/FAILED(0) = %d", valid_ie);
      if(valid_ie == FALSE)
      {
        SKIP_DECODING_ESM(pos, buffer_length, esm_len_in_bits);
        return FALSE;
    ...
    case ACTIVATE_DEDICATED_EPS_BEARER_CONTEXT_REQUEST:
      ...
      valid_ie = decode_dedicated_eps_bearer_req(src,
                                                 ...
                                                 &esm_len_in_bits);
      MSG_HIGH_DS_1(MM_SUB, "=NAS_MSG_LIB= ACTIVATE DEDICATED EPS BEARER CONTEXT REQUEST decoding PASSED(1)/FAILED(0) = %d", valid_ie);
      if(valid_ie == FALSE)
      {
        SKIP_DECODING_ESM(pos,buffer_length,esm_len_in_bits);
        return FALSE;
    ...
    case DEACTIVATE_EPS_BEARER_CONTEXT_REQUEST:
      ...
      valid_ie = decode_deactivate_eps_bearer_req(src,
                                                  ...
                                                  &esm_len_in_bits);
      MSG_HIGH_DS_1(MM_SUB, "=NAS_MSG_LIB= DEACTIVATE EPS BEARER CONTEXT REQUEST decoding PASSED(1)/FAILED(0) = %d", valid_ie);
      if(valid_ie == FALSE)
      {
        SKIP_DECODING_ESM(pos, buffer_length, esm_len_in_bits);
        return FALSE;
    ...
    case BEARER_RESOURCE_ALLOCATION_REJECT:
      ...
      valid_ie = decode_bearer_resource_alloc_rej(src,
                                                  ...
                                                  &esm_len_in_bits);
      MSG_HIGH_DS_1(MM_SUB, "=NAS_MSG_LIB= BEARER RESOURCE ALLOCATION REJECT decoding PASSED(1)/FAILED(0) = %d", valid_ie);
      if(valid_ie == FALSE)
      {
        SKIP_DECODING_ESM(pos, buffer_length, esm_len_in_bits);
        return FALSE;
    ...
    case BEARER_RESOURCE_MODIFICATION_REJECT:
      ...
      valid_ie = decode_bearer_modify_context_rej(src,
                                                  ...
                                                  &esm_len_in_bits);
      MSG_HIGH_DS_1(MM_SUB, "=NAS_MSG_LIB= BEARER RESOURCE MODIFICATION REJECT decoding PASSED(1)/FAILED(0) = %d", valid_ie);
      if(valid_ie == FALSE)
      {
        SKIP_DECODING_ESM(pos, buffer_length, esm_len_in_bits);
        return FALSE;
    ...
    case MODIFY_EPS_BEARER_CONTEXT_REQUEST:
      ...
      valid_ie = decode_modify_eps_context_req(src,
                                               ...
                                               &esm_len_in_bits);
      MSG_HIGH_DS_1(MM_SUB, "=NAS_MSG_LIB= MODIFY EPS BEARER CONTEXT REQUEST decoding PASSED(1)/FAILED(0) = %d", valid_ie);
      if(valid_ie == FALSE)
      {
        SKIP_DECODING_ESM(pos, buffer_length, esm_len_in_bits);
        return FALSE;
    ...
    case PDN_DISCONNECT_REJECT:
      ...
      valid_ie = decode_pdn_disconnect_rej(src,
                                           ...
                                           &esm_len_in_bits);
      MSG_HIGH_DS_1(MM_SUB, "=NAS_MSG_LIB= PDN DISCONNECT REJECT decoding PASSED(1)/FAILED(0) = %d", valid_ie);
      if(valid_ie == FALSE)
      {
        SKIP_DECODING_ESM(pos, buffer_length, esm_len_in_bits);
        return FALSE;
    ...
    case ESM_NOTIFICATION:
      ...
      if(emm_ctrl_data_ptr->emm_3gpp_spec_ver >= LTE_3GPP_REL9)
      {
        ...
        valid_ie = decode_esm_notification(src,
                                           ...
                                           &esm_len_in_bits);
        MSG_HIGH_DS_1(MM_SUB, "=NAS_MSG_LIB= ESM NOTIFICATION decoding PASSED(1)/FAILED(0) = %d", valid_ie);
        if(valid_ie == FALSE)
        {
          SKIP_DECODING_ESM(pos, buffer_length, esm_len_in_bits);
          return FALSE;
    ...
    case ESM_STATUS:
      ...
      valid_ie = decode_esm_status(src,
                                   ...
                                   &esm_len_in_bits);
      MSG_HIGH_DS_1(MM_SUB, "=NAS_MSG_LIB= ESM STATUS decoding PASSED(1)/FAILED(0) = %d", valid_ie);
      if(valid_ie == FALSE)
      {
        SKIP_DECODING_ESM(pos, buffer_length, esm_len_in_bits);
        return FALSE;
