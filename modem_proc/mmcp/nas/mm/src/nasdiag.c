...
/*======================================================================  
$Header: //components/rel/mmcp.mpss/7.9.0/nas/mm/src/nasdiag.c#1 $ $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $  
...
)
{
  ...
  switch(packet_type)
  {
    case NAS_DIAG_GMM_STATE_INFO_F :
      ...
      break;
    case NAS_DIAG_EMM_STATE_INFO_F :
      if (pkt_len != (uint16) sizeof(DIAG_SUBSYS_NAS_NAS_DIAG_EMM_STATE_INFO_F_req_type))
      {
        ret_status = FALSE;
      }
      else
      {
        req_as_id = ((DIAG_SUBSYS_NAS_NAS_DIAG_EMM_STATE_INFO_F_req_type*)req_ptr)->as_id;
      }
      break;
    case NAS_DIAG_DED_BC_STATE_F :
      if (pkt_len != (uint16) sizeof(DIAG_SUBSYS_NAS_NAS_DIAG_DED_BC_STATE_F_req_type))
      {
        ret_status = FALSE;
      }
      else
      {
        req_as_id = ((DIAG_SUBSYS_NAS_NAS_DIAG_DED_BC_STATE_F_req_type*)req_ptr)->as_id;
      }
      break;
...
/*===========================================================================
Function: nasdiag_emm_state_info
Description:

       This function processes the NAS specific EMM state request packet
       from QXDM and sends the response back.

============================================================================*/

PACKED void *nasdiag_emm_state_info (PACKED void* req_pkt, uint16 pkt_len)
{
#ifdef FEATURE_DUAL_SIM
  emm_ctrl_data_type *emm_ctrl_data_ptr = NULL;
#endif

  DIAG_SUBSYS_NAS_NAS_DIAG_EMM_STATE_INFO_F_rsp_type *rsp_ptr;

  const int rsp_len = sizeof(DIAG_SUBSYS_NAS_NAS_DIAG_EMM_STATE_INFO_F_rsp_type);

  /* Allocate the memory for this */
  rsp_ptr = (DIAG_SUBSYS_NAS_NAS_DIAG_EMM_STATE_INFO_F_rsp_type *)diagpkt_subsys_alloc(DIAG_SUBSYS_NAS, NAS_DIAG_EMM_STATE_INFO_F,
                                 rsp_len );
   if (!rsp_ptr)
  {
    MSG_FATAL_DS(MM_SUB,"diagpkt_subsys_alloc(%d, %d, %d) returned NULL!", 
              DIAG_SUBSYS_NAS, NAS_DIAG_EMM_STATE_INFO_F, rsp_len);
  }
  else
  {
#ifdef FEATURE_DUAL_SIM
   if(nasdiag_validate_req_parameters(pkt_len, req_pkt, NAS_DIAG_EMM_STATE_INFO_F) == TRUE)
   {
		 DIAG_SUBSYS_NAS_NAS_DIAG_EMM_STATE_INFO_F_req_type *req_ptr;
		 req_ptr = (DIAG_SUBSYS_NAS_NAS_DIAG_EMM_STATE_INFO_F_req_type*)req_pkt;
		 MSG_HIGH_DS_1(MM_SUB, "=NAS Diag= req_ptr->as_id %d", req_ptr->as_id);
		 emm_ctrl_data_ptr = emm_db_get_ctrl_data_per_subs(req_ptr->as_id);
		 
		 MSG_HIGH_DS_1(MM_SUB, "=NAS Diag= debug emm_state %d",emm_ctrl_data_ptr->emm_state);
		 if((emm_ctrl_data_ptr->emm_state == EMM_SERVICE_REQUEST_INITIATED)||
		   (emm_ctrl_data_ptr->emm_state == EMM_TRACKING_AREA_UPDATING_INITIATED)||
		   (emm_ctrl_data_ptr->emm_state == EMM_REGISTERED))
		{
		   rsp_ptr->emm_state_diag = TRUE;
		   MSG_HIGH_DS_0(MM_SUB, "=NAS Diag= emm state true");
		}
    }
    else
    {
		  rsp_ptr->emm_state_diag = FALSE;
		  MSG_HIGH_DS_0(MM_SUB, "=NAS Diag= emm state false");
    }
#else
...
/*===========================================================================
Function: nasdiag_ded_bc_state_info
Description:

       This function processes NAS dedicated bearer request packet
       from QXDM and sends the response back.

============================================================================*/

PACKED void *nasdiag_ded_bc_state_info (PACKED void* req_pkt, uint16 pkt_len)
{

  DIAG_SUBSYS_NAS_NAS_DIAG_DED_BC_STATE_F_rsp_type *rsp_ptr;

  const int rsp_len = sizeof(DIAG_SUBSYS_NAS_NAS_DIAG_DED_BC_STATE_F_rsp_type);

  /* Allocate the memory for this */
  rsp_ptr = (DIAG_SUBSYS_NAS_NAS_DIAG_DED_BC_STATE_F_rsp_type *)diagpkt_subsys_alloc(DIAG_SUBSYS_NAS, NAS_DIAG_DED_BC_STATE_F,
                                 rsp_len );
   if (rsp_ptr == NULL)
  {
    MSG_FATAL_DS(MM_SUB,"diagpkt_subsys_alloc(%d, %d, %d) returned NULL!", 
              DIAG_SUBSYS_NAS, NAS_DIAG_DED_BC_STATE_F, rsp_len);
  }
  else
  {
#ifdef FEATURE_DUAL_SIM
    if(nasdiag_validate_req_parameters(pkt_len, req_pkt, NAS_DIAG_DED_BC_STATE_F) == TRUE)
    {
      DIAG_SUBSYS_NAS_NAS_DIAG_DED_BC_STATE_F_req_type *req_ptr;
      req_ptr = (DIAG_SUBSYS_NAS_NAS_DIAG_DED_BC_STATE_F_req_type*)req_pkt;
      if(esm_dc_bearer_exists(req_ptr->as_id))
      {
        rsp_ptr->ded_bc_exists = TRUE;
      }
      MSG_HIGH_DS_1(MM_SUB, "=NAS Diag= req_ptr->as_id %d ", req_ptr->as_id);
     }
     else
     {
       rsp_ptr->ded_bc_exists = FALSE;
     }
#else
    if(esm_dc_bearer_exists())
    {
      rsp_ptr->ded_bc_exists = TRUE;
    }
    else
  ...
  MSG_HIGH_DS_1(MM_SUB, "=NAS debug dedicated bearers exists %d", rsp_ptr->ded_bc_exists);
  return rsp_ptr ;
}
...
static const diagpkt_user_table_entry_type nas_diag_tbl[] =
{
  ...
  {NAS_DIAG_EMM_STATE_INFO_F, NAS_DIAG_EMM_STATE_INFO_F, nasdiag_emm_state_info},
  {NAS_DIAG_DED_BC_STATE_F, NAS_DIAG_DED_BC_STATE_F, nasdiag_ded_bc_state_info},
};
