...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/mm/src/mmtask.c#6 $ $DateTime: 2023/10/08 15:22:21 $ $Author: pwbldsvc $
...
pthread_mutex_t  emm_forbidden_for_service_list_mutex = PTHREAD_MUTEX_INITIALIZER;  
...
#define MM_QSH_METRIC_DEFAULT_CONFIG_COUNT 3
#define MM_QSH_METRIC_SIGNALING_ELEMENT_COUNT 50
#define MM_QSH_METRIC_DEBUG_BUFFER_ELEMENT_COUNT 45
qsh_ext_metric_cfg_s  mm_qsh_metric_default_cfg_arr[MM_QSH_METRIC_DEFAULT_CONFIG_COUNT];
#endif
...
nas_crit_sect_type mm_sim_swap_crit_sec;
#endif
...
static ULogHandle mm_ulog_ota_handle = NULL;
...
mm_cmd_type *mm_cm_get_cmd_buf(cm_mm_cmd_e_T cmd_type)
{
   ...
   switch(cmd_type)
   {
     ...
     case MMCM_LTE_SUSPEND_REQ:
       PUT_IMH_LEN(sizeof(cm_mm_lte_suspend_req_T) - sizeof(IMH_T), &cmd->cmd.hdr) ;
       break;

     case MMCM_LTE_RESUME_IND:
       PUT_IMH_LEN(sizeof(cm_mm_lte_resume_ind_T) - sizeof(IMH_T), &cmd->cmd.hdr) ;
       break;
#endif
	...
	case MMCM_ECALL_REMAINING_TIMER_VALUE:
	  PUT_IMH_LEN(sizeof(cm_mm_ecall_remaining_timer_value_T) - sizeof(IMH_T), &cmd->cmd.hdr) ;
	  break;
	case MMCM_ECALL_INACTIVITY_TRIGGER:
	  PUT_IMH_LEN(sizeof(cm_mm_ecall_inactivity_trigger_T) - sizeof(IMH_T), &cmd->cmd.hdr) ;
	  break;
#endif
...
"/nv/item_files/modem/nas/conn_mode_manual_search_Subscription01\n"\
...
"/nv/item_files/modem/nas/volte_rej_mt_csfb_Subscription01\n"\
...
"/nv/item_files/modem/nas/skip_bearer_ctxt_status_ie\n"\
"/nv/item_files/modem/nas/shared_plmn_emc_attach_retry\n"\
...
"/nv/item_files/modem/nas/lte_nas_estimate_dl_count_from_sqn\n"\
"/nv/item_files/modem/nas/gsm_sel_resel_block_timer\n"\
"/nv/item_files/modem/nas/gsm_sel_resel_block_timer_Subscription01\n"\
"/nv/item_files/modem/nas/disable_gsm_during_fallback_upon_rej15\n"\
"/nv/item_files/modem/nas/disable_gsm_during_fallback_upon_rej15_Subscription01\n"\
"/nv/item_files/modem/nas/emm_unsec_redir_allowed\n"\
"/nv/item_files/modem/nas/emm_unsec_redir_allowed_Subscription01\n"\
"/nv/item_files/modem/nas/prohibit_drx_timer_value\n"\
"/nv/item_files/modem/nas/local_rel_tau_after_csfb_tau\n"\
"/nv/item_files/modem/nas/local_rel_tau_after_nas_sms\n"\
"/nv/item_files/modem/nas/tau_radio_cap_update\n"\
"/nv/item_files/modem/nas/tau_radio_cap_update_Subscription01\n"\
"/nv/item_files/modem/nas/radio_cap_enquiry_timer\n"\
"/nv/item_files/modem/nas/radio_cap_enquiry_timer_Subscription01\n"\
"/nv/item_files/modem/nas/ra_nv_cap_enquiry_skip_opt\n"\
"/nv/item_files/modem/nas/ra_nv_cap_enquiry_skip_opt_Subscription01\n"\
"/nv/item_files/modem/nas/3417_ext_mt_enabled\n"\
"/nv/item_files/modem/nas/radio_cap_enquiry_timer_expiry_ignore\n"\
"/nv/item_files/modem/nas/radio_cap_enquiry_timer_expiry_ignore_Subscription01\n"\
"/nv/item_files/modem/nas/mm_nv_enable_msim_csg\n"\
"/nv/item_files/modem/nas/reattach_after_sync_up_failure\n"\
"/nv/item_files/modem/nas/reattach_after_sync_up_failure_Subscription01\n"\
"/nv/item_files/modem/nas/gmm_disable_forbit_for_alwayson_data\n"\
"/nv/item_files/modem/nas/gmm_disable_forbit_for_alwayson_data_Subscription01\n"\
"/nv/item_files/modem/nas/skip_acb_check_drb_est_ims_registration\n"\
"/nv/item_files/modem/nas/skip_acb_check_drb_est_ims_registration_Subscription01\n"\
"/nv/item_files/modem/nas/t3411_value_during_mmtel_call\n"\
"/nv/item_files/modem/nas/t3411_value_during_mmtel_call_Subscription01\n\
";
...
SIDE EFFECTS
  None
===========================================================================*/
ULogHandle mm_get_ulog_handle(void)
{
  return mm_ulog_handle;
}

/*===========================================================================
FUNCTION mm_get_ulog_ota_handle

DESCRIPTION
  Returns ULOG Handle for NAS OTA messages
...
SIDE EFFECTS
  None
===========================================================================*/
ULogHandle mm_get_ulog_ota_handle(void)
{
  return mm_ulog_ota_handle;
}
...
)
{
  ...
  */
  ...
  MSG_HIGH_DS_4(MM_SUB,"=EMM= isr_efs_flag (status =%d value = %d) volte_persistent_active_flag (status =%d value = %d)",
                    efs_status[0],
                    emm_s1_ctrl_data_ptr->isr_efs_flag,
                    efs_status[1],
                    emm_s1_ctrl_data_ptr->volte_persistent_active_flag);
#ifdef FEATURE_LTE_REL12
  MSG_HIGH_DS_1(MM_SUB,"=EMM= csfb_perform_reg is %d", emm_s1_ctrl_data_ptr->csfb_perform_reg);
#endif
  ...
  if((efs_status[0] = mcfg_fs_read("/nv/item_files/modem/nas/skip_bearer_ctxt_status_ie", 
             &emm_s1_ctrl_data_ptr->skip_bc_status_ie,
             sizeof(boolean), MCFG_FS_TYPE_EFS, (mcfg_fs_sub_id_e_type)mm_as_id )) != MCFG_FS_STATUS_OK)
  {
     emm_s1_ctrl_data_ptr->skip_bc_status_ie = FALSE;
  }

  /* Report to the Dog HB */
  dog_hb_report(DOG_MM_RPT);  
  if((efs_status[1] = mcfg_fs_read("/nv/item_files/modem/nas/emm_combined_proc", 
  ...
  MSG_HIGH_DS_8(MM_SUB,"=EMM= skip_bearer_ctxt_status_ie (status %d value %d) emm_combined_proc_efs_flag (status =%d value = %d) avoid_guti_nas_security_check (status =%d value = %d) geran_cap (status =%d value = %d) ",
                    efs_status[0],
                    emm_s1_ctrl_data_ptr->skip_bc_status_ie,
                    efs_status[1],
                    emm_s1_ctrl_data_ptr->emm_combined_proc_efs_flag,
  ...
  MSG_HIGH_DS_2(MM_SUB,"=EMM= early_call_init (status =%d value = %d) ",
                    efs_status[3],
                    emm_s1_ctrl_data_ptr->early_call_init);
  ...
  if(mm_sim_max_subscription > 1)
  {
    ...
    emm_s2_ctrl_data_ptr->skip_bc_status_ie = emm_s1_ctrl_data_ptr->skip_bc_status_ie;
    emm_s2_ctrl_data_ptr->estimate_dl_count_from_sqn = emm_s1_ctrl_data_ptr->estimate_dl_count_from_sqn;
...
)
{
  ...
  emm_subs_barring_mask emm_subs_mask = EMM_BARRING_NO_MASK;
#ifdef FEATURE_DUAL_SIM  
  for(mm_sub_id = MM_AS_ID_1, mm_as_id = MM_AS_ID_1; mm_sub_id < mm_sim_max_subscription; mm_sub_id++, mm_as_id++)
#endif
  {   
    ...
    emm_ctrl_data_ptr->ds_proc_type_ptr = &mm_ds_proc_type;
#endif
    ...
    emm_ctrl_data_ptr->forbidden_for_service_list_mutex_ptr = &emm_forbidden_for_service_list_mutex;
    ...
    /*FLag to know if RAU is required while moving from L to G/W when TAU is not completed in LTE due to lower layer failure*/
...
#ifdef FEATURE_DUAL_SIM
    emm_ctrl_data_ptr->highest_proc_type = SYS_PROC_TYPE_NONE;
#endif
#if defined(FEATURE_DUAL_SIM) && defined (FEATURE_DUAL_DATA)
    emm_ctrl_data_ptr->esr_tau_pending_conn_rel = FALSE;
    emm_ctrl_data_ptr->paging_conn_est = FALSE;
	emm_ctrl_data_ptr->nas_sms_pending_con_rel = FALSE;
	emm_ctrl_data_ptr->efs_loc_rel_tau_after_nas_sms = FALSE;
    ...
    {
      emm_subs_mask = EMM_BARRING_MASK_SUB1;
    }
	else if (mm_sub_id == MM_AS_ID_2)
	{
      emm_subs_mask = EMM_BARRING_MASK_SUB2;
	}
#endif
  
    rex_def_timer_ex(&emm_ctrl_data_ptr->emm_access_barring_emergency_timer, (rex_timer_cb_type)emm_barring_call_back, (dword)(emm_subs_mask|EMM_EMERGENCY_BARRED)) ;
  
    rex_def_timer_ex(&emm_ctrl_data_ptr->emm_access_barring_mt_access_timer, (rex_timer_cb_type)emm_barring_call_back, (dword)(emm_subs_mask|EMM_MT_ACCESS_BARRED)) ;
  
    rex_def_timer_ex(&emm_ctrl_data_ptr->emm_access_barring_mo_signaling_timer, (rex_timer_cb_type)emm_barring_call_back, (dword)(emm_subs_mask|EMM_MO_SIGNALING_BARRED)) ;
  
    rex_def_timer_ex(&emm_ctrl_data_ptr->emm_access_barring_mo_data_timer, (rex_timer_cb_type)emm_barring_call_back, (dword)(emm_subs_mask|EMM_MO_DATA_BARRED)) ;

    rex_def_timer_ex(&emm_ctrl_data_ptr->emc_backoff_timer, (rex_timer_cb_type)emm_emc_backoff_timer_call_back, (dword)TIMER_EMC_BACKOFF) ;

#ifdef FEATURE_LTE_REL10
    rex_def_timer_ex(&emm_ctrl_data_ptr->emm_access_barring_mo_csfb_timer, (rex_timer_cb_type)emm_barring_call_back, (dword)(emm_subs_mask|EMM_MO_CSFB_BARRED));
    rex_def_timer_ex(&emm_ctrl_data_ptr->emm_access_barring_delay_tolerant_timer, (rex_timer_cb_type)emm_barring_call_back, (dword)(emm_subs_mask|EMM_DELAY_TOLERANT_BARRED));
#endif
...
#ifdef FEATURE_QSH_EVENT_METRIC
   emm_ctrl_data_ptr->qsh_debug_metrics_ptr = &mm_qsh_debug_metrics;
#endif
    ...
    emm_ctrl_data_ptr->last_non_dds_lte_status = SYS_RAT_ACTIVATION_STATUS_DEACTIVATED;
#endif
#endif
    emm_ctrl_data_ptr->abort_ps_detach_ptr  = &gmm_abort_ps_detach;
...
#ifdef FEATURE_DUAL_SIM
    emm_ctrl_data_ptr->is_multisim_device = &mm_is_multisim_device;
    emm_ctrl_data_ptr->emm_illegal_me_sub = &mm_illegal_me_sub;
#endif
    ...
    emm_ctrl_data_ptr->ra_cap_enquiry_nw_ratmask  = 0x00;
...
)
{
...
#ifdef FEATURE_DUAL_SIM
  emm_ctrl_data_type *emm_ctrl_data_s2_ptr = NULL;
#endif
...
#endif
#ifdef FEATURE_DUAL_SIM
  lte_nas_msgr_register(MSGR_NAS_EMM,esm_client,MSGR_ID_NONE,NAS_EMM_PS_CALL_STATUS_CMD) ;
#endif
  ...
  lte_nas_msgr_register(MSGR_NAS_EMM,rrc_client,MSGR_ID_NONE,LTE_RRC_PESUDO_RESEL_LOW_PRIORITY_IND) ;
  ...
  lte_nas_msgr_register(MSGR_NAS_EMM,rrc_client,MSGR_ID_NONE,LTE_RRC_ALL_TRM_PRIO_LOWERED_IND) ;
  lte_nas_msgr_register(MSGR_NAS_EMM,rrc_client,MSGR_ID_NONE,LTE_RRC_CDRX_STATUS_IND) ;
  lte_nas_msgr_register(MSGR_NAS_EMM,rrc_client,MSGR_ID_NONE,LTE_RRC_CONN_EST_TIMER_UPDATE_IND);
  lte_nas_msgr_register(MSGR_NAS_EMM,rrc_client,MSGR_ID_NONE,LTE_RRC_RADIO_CAP_ENQUIRY_IND);
...
#ifdef FEATURE_DUAL_SIM
  mm_set_as_id(MM_AS_ID_2);
  emm_ctrl_data_s2_ptr = emm_db_get_ctrl_data();
  emm_ctrl_data_s2_ptr->cs_rat_available = emm_ctrl_data_ptr->cs_rat_available;
  mm_set_as_id(MM_AS_ID_1);
#endif
...
/*Reset LU initiated flag to flase since 3346 expired.*/
    ...
    case EMM_MT_PAGE_EXT_PRI_TIMER:
      ...
      emm_check_possible_tau_for_rrc_ra_cap_update(emm_ctrl_data_ptr);
#else
#ifdef FEATURE_SGLTE
      ...
      case TIMER_ELS_CONN_REL:
        emm_timer_els_conn_rel_expire(emm_ctrl_data_ptr);
        break;
#endif
      case TIMER_HOLD_TRM_LOCAL_REL:
        emm_timer_hold_trm_local_rel_expiry(emm_ctrl_data_ptr);
        break;
      ...
      case TIMER_LOCAL_REL_MSEARCH:
        emm_timer_local_rel_tau_during_msearch_expiry(emm_ctrl_data_ptr);
        break;
       case TIMER_T3247:
       ...
       case TIMER_PROHIBIT_DS_DRX:
        emm_timer_drx_prohibit_expire(emm_ctrl_data_ptr);
        break;

        case TIMER_RADIO_CAP_ENQ:
  ...
  nas_event_report_payload(EVENT_LTE_EMM_TIMER_EXPIRY,sizeof(event),(void*)&event,mm_message_as_id);
}
...
)
{
...
#if defined FEATURE_DUAL_SIM || defined FEATURE_SGLTE
       && ((mm_subs_capability_mask & SYS_SYS_MODE_MASK_LTE) 
  ...
  else if(sigs & ESM_EMM_CMD_Q_SIG)
  {
    ...
    if((mm_sim_card_mode == MMGSDI_APP_USIM) &&
       (mm_sub_is_rat_enabled(SYS_RAT_LTE_RADIO_ACCESS)|| 
        (((emm_cmd_type*)cmd_ptr)->emm_msgr_cmd.msg_hdr.id != NAS_EMM_PS_CALL_STATUS_CMD &&
         ((emm_cmd_type*)cmd_ptr)->emm_msgr_cmd.msg_hdr.id != NAS_EMM_EPS_BEARER_STATUS_CMD)))
#endif
    ...
    ASSERT(emm_ctrl_data_ptr != NULL);
     ...
     if ((((emm_rrc_cmd_type*)cmd_ptr)->rrc_msgr_cmd.msg_hdr.id)!= LTE_RRC_DL_DATA_IND)
     {
       emm_add_message_to_debug_buffer(((emm_rrc_cmd_type*)cmd_ptr)->rrc_msgr_cmd.msg_hdr.id,
                                         TO_EMM,(sys_modem_as_id_e_type)mm_sub_id);
	 }
  ...
  else if(sigs & GPS_EMM_CMD_Q_SIG)
  {
    ...
    /*Check only if LTE is active  else no need to check for memory
     alocated part, existing handler should send reject to GPS	in case
     LTE is not active*/
...
#if defined (FEATURE_DUAL_SIM)
   rex_init_crit_sect(&mm_sim_swap_crit_sec);
#endif
...
/*===========================================================================
FUNCTION mm_qsh_register

DESCRIPTION
This function call registers MM Task with QSH
...
void mm_qsh_register(void)
{
  ...
  client_reg.client = QSH_CLT_MM;
  client_reg.client_cb_ptr = mm_qsh_debug_callback;
  client_reg.major_ver = MM_QSH_MAJOR_VER;
  client_reg.minor_ver = MM_QSH_MINOR_VER;
  ...
  /* Disable all QSH NAS events by default */
#ifdef FEATURE_DUAL_SIM
  for(sub_id = MM_AS_ID_1; sub_id < MAX_AS_IDS; sub_id++)
    for(idx = 0; idx < MM_QSH_EVENT_MAX; idx++)
      mm_qsh_event_enabled_sim[sub_id][idx] = FALSE;
#else
  for(idx = 0; idx < MM_QSH_EVENT_MAX; idx++)
    mm_qsh_event_enabled[idx] = FALSE;
#endif
#endif

#ifdef FEATURE_QSH_EVENT_METRIC
  qsh_client_metric_cfg_init(mm_qsh_metric_default_cfg_arr,
                          MM_QSH_METRIC_DEFAULT_CONFIG_COUNT);

  //Signaling metric enable for AS_ID_1
  mm_qsh_metric_default_cfg_arr[0].id = MM_QSH_METRIC_SIGNALING;
  mm_qsh_metric_default_cfg_arr[0].subs_id = SYS_MODEM_AS_ID_1;
  mm_qsh_metric_default_cfg_arr[0].fifo.element_size_bytes = 
                                    sizeof(mm_qsh_metric_signaling_s);
  mm_qsh_metric_default_cfg_arr[0].fifo.element_count_total = 
                                MM_QSH_METRIC_SIGNALING_ELEMENT_COUNT;

  //Signaling metric enable for AS_ID_2
  mm_qsh_metric_default_cfg_arr[1].id = MM_QSH_METRIC_SIGNALING;
  mm_qsh_metric_default_cfg_arr[1].subs_id = SYS_MODEM_AS_ID_2;
  mm_qsh_metric_default_cfg_arr[1].fifo.element_size_bytes =
                                     sizeof(mm_qsh_metric_signaling_s);
  mm_qsh_metric_default_cfg_arr[1].fifo.element_count_total =
                                  MM_QSH_METRIC_SIGNALING_ELEMENT_COUNT;

  //Debug buffer metric enable(common for both subs)
  mm_qsh_metric_default_cfg_arr[2].id = MM_QSH_METRIC_DEBUG_BUFFER;
  mm_qsh_metric_default_cfg_arr[2].subs_id = SYS_MODEM_AS_ID_1;
  mm_qsh_metric_default_cfg_arr[2].fifo.element_size_bytes = 
                                 sizeof(mm_qsh_metric_debug_buffer_s);
  mm_qsh_metric_default_cfg_arr[2].fifo.element_count_total = 
                             MM_QSH_METRIC_DEBUG_BUFFER_ELEMENT_COUNT;

  client_reg.metric_info.metric_cfg_count = 
                    MM_QSH_METRIC_DEFAULT_CONFIG_COUNT;
  client_reg.metric_info.metric_cfg_arr_ptr = 
                             &mm_qsh_metric_default_cfg_arr[0];
#endif
#ifdef FEATURE_QSH_DUMP
  client_reg.dump_info.max_size_bytes = 2*sizeof(mm_qsh_dump_data_s_type) +
                                      sizeof(mm_qsh_dump_debug_buffer_s_type);
#endif

  qsh_client_reg(&client_reg);
#ifdef FEATURE_QSH_MDUMP
  qsh_add_thread_id(QSH_CLT_MM);
#endif
}
#endif
...
{
  ULogResult mm_buffer_status, mm_ota_buffer_status;
  ...
  mm_ota_buffer_status = ULogFront_RawInit(&mm_ulog_ota_handle,
                               "NAS_OTA_ULOG",
                               1024, 
                               ULOG_MEMORY_LOCAL,
                               ULOG_LOCK_OS);
  MSG_HIGH_DS_2(MM_SUB,"=MM= ULOG REGISTER mm_buffer_status: %d and mm_ota_buffer_status: %d", mm_buffer_status, mm_ota_buffer_status);
}
...
#if defined(FEATURE_QSH_EVENT_METRIC) || defined(FEATURE_QSH_DUMP) || defined(FEATURE_QSH_EVENT_NOTIFY_TO_QSH) || defined(FEATURE_QSH_MDUMP)
   mm_qsh_register();
#endif
