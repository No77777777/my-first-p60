...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/mm/src/mm_multimode_handler.c#1 $ 
$DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...
extern uint32 mmr_stop_mode_trans_id_sim[MAX_NAS_STACKS];
#define mmr_stop_mode_trans_id mmr_stop_mode_trans_id_sim[mm_as_id]

static boolean mm_lte_redir_during_lu_sim[MAX_AS_IDS]; 
#define mm_lte_redir_during_lu  mm_lte_redir_during_lu_sim[mm_sub_id]

#else
extern uint32 mmr_stop_mode_trans_id;
static boolean mm_lte_redir_during_lu; 
#endif

#if defined(FEATURE_DUAL_SIM) && defined(FEATURE_DUAL_DATA)
mm_cmd_type* mm_irat_pending_reg_cmd_ptr_sim[MAX_AS_IDS];
#define mm_irat_pending_reg_cmd_ptr mm_irat_pending_reg_cmd_ptr_sim[mm_as_id]

mm_csfb_mt_call_info_s_type    mm_csfb_mt_call_info_sim[MAX_AS_IDS];
#define mm_csfb_mt_call_info mm_csfb_mt_call_info_sim[mm_as_id]

mm_gw_identity_param_type mm_mapped_gw_identity_sim[MAX_AS_IDS];
#define mm_mapped_gw_identity mm_mapped_gw_identity_sim[mm_as_id]
#else
/* Used to buffer REG CMD received during GW2L and L2GW IRAT */
mm_cmd_type*     mm_irat_pending_reg_cmd_ptr;
...
extern void mmcoord_process_qsh_message_ind
( 
  mm_cmd_type *cmd_ptr 
);
#endif
...
#ifdef FEATURE_QSH_EVENT_METRIC
lte_nas_message_id mm_qsh_event_nas_message_id = ESM_LAST_UNDEFINED;
int32 mm_qsh_event_rplmn_id = 0;
void mm_emm_notify_rplmn_update_event_qsh
(
...
void mm_set_lte_redir_during_lu(boolean value) 
{
  mm_lte_redir_during_lu = value;
}

boolean mm_get_lte_redir_during_lu(void) 
{
  return mm_lte_redir_during_lu;
}


/*===========================================================================
 
FUNCTION      mm_irat_pending_reg_cmd_ptr_mem_alloc

DESCRIPTION  
 Function allocates memory for mm_irat_pending_reg_cmd_ptr and copies the passed
 mm cmd ptr to it
...
===========================================================================*/
void mm_irat_pending_reg_cmd_ptr_mem_alloc(mm_cmd_type* cmd_ptr)
{
  mm_irat_pending_reg_cmd_ptr = emm_modem_mem_alloc(sizeof(mm_cmd_type),
                                                  emm_db_get_ctrl_data());
  if(mm_irat_pending_reg_cmd_ptr != NULL)
  {
    memscpy((void *)mm_irat_pending_reg_cmd_ptr, sizeof(mm_cmd_type),cmd_ptr, sizeof(mm_cmd_type));
  }
}

/*===========================================================================
 
FUNCTION      mm_irat_pending_reg_cmd_ptr_mem_free

DESCRIPTION  
 Function frees memory for mm_irat_pending_reg_cmd_ptr.
...
===========================================================================*/
void mm_irat_pending_reg_cmd_ptr_mem_free(void)
{
  emm_modem_mem_free(mm_irat_pending_reg_cmd_ptr, emm_db_get_ctrl_data());
  mm_irat_pending_reg_cmd_ptr = NULL;
}


/*===========================================================================

FUNCTION  mm_emm_csfb_domain_change_in_progress

DESCRIPTION: 
  Function to check if domain changing during pending CSFB call
...
===========================================================================*/
boolean mm_emm_csfb_domain_change_in_progress
(
  mm_cmd_type  *cmd_ptr
)
{
  ...
  if((mm_csfb_state == MM_CSFB_STATE_MO_CALL_ORIG) &&
     ...
     (cmd_ptr != NULL) &&
     (cmd_ptr->cmd.hdr.message_set == MS_MM_REG) &&
     (cmd_ptr->cmd.hdr.message_id == (byte)MMR_REG_REQ) &&
     mm_checkif_rat_pri_list_has_rat(&cmd_ptr->cmd.mmr_reg_req.rat_pri_list_info, SYS_RAT_LTE_RADIO_ACCESS) &&
     (emm_ctrl_data_ptr->reg_req_pending_ptr->req_service_domain == cmd_ptr->cmd.mmr_reg_req.req_service_domain) &&
...
void emm_paused_mm_timer (timer_id_T  timer_id)
{
   if (mm_timer_status [ timer_id ] == TIMER_ACTIVE)
   {
     if (gs_modify_timer_ext( get_mm_timer_queue(),
                                  timer_id,
                                  TIMER_MODIFY_PAUSE,
                                  mm_timer_non_deferrable(timer_id)) != GS_SUCCESS)
     {
       MSG_ERROR_DS( MM_SUB, "=MM= Failed to pause timer %d", timer_id,  0, 0);
     }
     else
     {
       MSG_HIGH_DS_1(MM_SUB ,"=MM= Pausing %d", timer_id);
       mm_timer_status[ timer_id ] = TIMER_PAUSED;
     }
   }
}


void emm_resume_paused_timer(timer_id_T  timer_id)
{
  if (mm_timer_status[timer_id] == TIMER_PAUSED)
  {
    if (gs_modify_timer_ext(get_mm_timer_queue(),
                             timer_id,
                             TIMER_MODIFY_RESUME,
                             mm_timer_non_deferrable(timer_id)) != GS_SUCCESS)
    {
      MSG_ERROR_DS(MM_SUB, "Failed to resume timer %d", timer_id, 0, 0);
    }
    else
    {
      MSG_HIGH_DS_1(MM_SUB ,"=MM= Resuming %d", timer_id);
...
#endif
  {
  MSG_HIGH_DS_0(MM_SUB,"Setting GW parameters for intersystem handover from LTE to GW");
...
)
{
  ...
  {
    MSG_HIGH_DS_0(MM_SUB,"=MM= SRVCC w/ emergency voice bearer existing");
...
void mm_emm_process_srvcc_sglte_cs_handover(mm_cmd_type *cmd_ptr)
{
  ...
  MSG_HIGH_DS_0(MM_SUB,"=MM= RR_ACTIVATION_IND cause MM_AS_LTOG_SGLTE_SRVCC_CS_HANDOVER");

  /* set TAU cause to others to start TAU on resuming LTE */
  MSG_HIGH_DS_0(MM_SUB,"=MM= TAU to be performed on LTE resume ");
  ...
  mm_send_mmcnm_srvcc_handover_complete();

  /* restore the original stack id */ 
  mm_set_as_id(current_mm_as_id);
...
)
{
  ...
  memset((void *)&msg,0,sizeof(mmr_stop_mode_req_s_type));
  ...
  /*Ignore RRC_ACTIVATION_IND and abort LTE RRC connection when the following conditions are satisfied
    1] EMM state is EMM-DEREGISTERED, EMM-REGISTERED and aborting connection due to some reasion like 3417 expiry,
       EMM-DEREGISTERD-INITATED or EMM_REGISTERED_INITIATED
  ...
  if((cmd_ptr->cmd.hdr.message_id == RRC_ACTIVATION_IND)
     ...
     ((EMM_GET_STATE() == EMM_DEREGISTERED) || 
      ...
      ((EMM_GET_STATE() == EMM_REGISTERED) && (emm_ctrl_data_ptr->aborting_conn == TRUE)) ||
     ...
     ((cmd_ptr->cmd.rrc_activation_ind.status == MM_AS_INTERRAT_PS_HANDOVER) ||
      ...
      (cmd_ptr->cmd.rrc_activation_ind.status == MM_AS_LTOW_SRVCC_CS_HANDOVER)))
  {
     ...
     (cmd_ptr->cmd.rr_activation_ind.status == MM_AS_LTOG_SGLTE_SRVCC_CS_HANDOVER))
  {
    ...
    *mm_emm_dispatcher_fn_ptr = mm_emm_normal_msg_handler; 
    MSG_HIGH_DS_0(MM_SUB,"=EMM= Change the main routine handler to mm_emm_normal_msg_handler"); 
    return;
  }
#endif
...
#endif
  {
   ...
   {
     MSG_HIGH_DS_1(MM_SUB,"=EMM= L2G CCO indication is received in emm connection state %d ignoring it", 
     ...
     (EMM_GET_STATE() == EMM_DEREGISTERED_INITIATED))
  {
    ...
    {
      MSG_HIGH_DS_0(MM_SUB,"=EMM= NAS performing power off detach ignoring CCO indication");
      return;
    }
  ...
  switch(EMM_GET_STATE())
  {
    ...
    case EMM_REGISTERED_INITIATED:
      ...
      if (emm_ctrl_data_ptr->aborting_conn == TRUE &&
	  	  ...
	  	  emm_ctrl_data_ptr->rrc_deact_req.deact_reason == SYS_STOP_MODE_REASON_DUAL_SWITCH)
	  {
	    MSG_HIGH_DS_0(MM_SUB,"=EMM= Ignoring activation indication after dual switch has been processed");
	  	return;
	  }
      break;
  ...
  if( cmd_ptr->cmd.hdr.message_id == RRC_ACTIVATION_IND)
  {
...
)
    {
      ...
      MSG_HIGH_DS_1(MM_SUB,"=EMM= LTE to TDS i-RAT in progress(cause %d), active RAT is SYS_RAT_TDS_RADIO_ACCESS", local_status);
    }
   else if (TRUE
#ifdef FEATURE_SEGMENT_LOADING
      && get_wcdma_interface()
...
#endif
    {
      ...
      local_status = cmd_ptr->cmd.rrc_activation_ind.status;
      MSG_HIGH_DS_1(MM_SUB,"=EMM= LTE to WCDMA i-RAT in progress(cause %d), active RAT is SYS_RAT_UMTS_RADIO_ACCESS", local_status);
    }
  ...
  else if (cmd_ptr->cmd.hdr.message_id == RR_ACTIVATION_IND)
  {
    ...
    if (mm_csfb_state == MM_CSFB_STATE_MO_CALL_ORIG || mm_csfb_state == MM_CSFB_STATE_MT_CALL_ORIG)
    {
       mm_system_information.sgsnr = TRUE;
    }
    gmm_reset_mcs_client_info();
    MSG_HIGH_DS_1(MM_SUB,"=EMM= LTE to GSM i-RAT in progress(cause %d), active RAT is  SYS_RAT_GSM_RADIO_ACCESS", local_status);
  }
  else
  {
  ...
  switch(local_status)
  {
    ...
    case MM_AS_INTERRAT_PS_HANDOVER:
      MSG_HIGH_DS_0(MM_SUB,"=EMM= received MM_AS_INTERRAT_PS_HANDOVER, sending abort_request to L-RRC");
/* ---------------------------- ---------------------------- ---------------------------- ----------------------------
**24.008: 4.7.5.1
...
** ---------------------------- ---------------------------- ---------------------------- ----------------------------*/
      ...
      if(emm_ctrl_data_ptr->TIN == NAS_MM_TIN_RAT_RELATED_TMSI)
      {
        ...
        {
          MSG_HIGH_DS_0(MM_SUB,"Deactivate ISR due to PDP context update while in LTE");
    ...
    case MM_AS_LTOG_SRVCC_CS_HANDOVER:
      MSG_HIGH_DS_1(MM_SUB,"=EMM= received SRVCC CS HANDOVER to G/W Act Ind status = %d",local_status);
      /* Suspend DS first*/
      if ((gmm_state != GMM_NULL) && (gmm_state != GMM_DEREGISTERED))
      {
        mm_send_cm_ps_data_suspend_ind();
    ...
    case MM_AS_LTOW_SRVCC_CS_PS_HANDOVER:
      MSG_HIGH_DS_0(MM_SUB,"=EMM= received MM_AS_LTOW_SRVCC_CS_PS_HANDOVER");
      reg_service_report(MMR_CAUSE_NOT_SPECIFIED, TRUE, emm_ctrl_data_ptr) ;
    ...
    case MM_AS_LTOG_CCO:
      emm_ctrl_data_ptr->l2gcco_state = EMM_LTOG_CCO_IN_PROGRESS;
      emm_failure_cause.cause_type = LTE_NAS_IRAT_INVALID_STATE;
      MSG_HIGH_DS_0(MM_SUB,"=EMM= received MM_AS_LTOG_CCO set L2GCCO state to IN PROGRESS");
#ifdef FEATURE_SGLTE
        if(MM_IS_IN_SGLTE_MODE )
        {
          MSG_HIGH_DS_0(MM_SUB,"=EMM= SGLTE CCO starting merging..");
          mmsend_mmr_stack_deact_ind();
  ...
  *mm_emm_dispatcher_fn_ptr = emm_process_LTE_to_GW_rat_change; 
  MSG_HIGH_DS_1(MM_SUB,"=EMM= Change the main routine handler to emm_process_LTE_to_GW_rat_change due to %d ",local_status);
  }
...
{
  MSG_HIGH_DS_1(MM_SUB,"=EMM= Set L2GCCO state to INACTIVE, previous cco state %d",
                   emm_ctrl_data_ptr->l2gcco_state);
...
)
{
  ...
  switch(rrc_msgr_cmd->rrc_msgr_cmd.msg_hdr.id)
  {
    ...
    case LTE_RRC_DEACTIVATE_CNF:
      ...
      if(mm_irat_pending_reg_cmd_ptr != NULL)
      {
         if((mm_irat_pending_reg_cmd_ptr->cmd.hdr.message_set == MS_MM_REG) &&
            (mm_irat_pending_reg_cmd_ptr->cmd.hdr.message_id == (byte)MMR_REG_REQ))
         {
    ...
    default:
      MSG_ERROR_DS_1(MM_SUB,"EMM IRAT RRC handler Ignoring message with UMID %d",
                                             rrc_msgr_cmd->rrc_msgr_cmd.msg_hdr.id);
      break;
  }
  ...
  switch(mm_cmd_ptr->cmd.hdr.message_set)
  {
    case MS_GMM_SM:
...
#ifdef FEATURE_SGLTE 
          MSG_HIGH_DS_2(MM_SUB,"=EMM= mm_sglte_current_stack_status  = %d  MM_IS_IN_SGLTE_MODE = %d", 
                      mm_sglte_current_stack_status ,MM_IS_IN_SGLTE_MODE);
#endif
      ...
      && get_tdscdma_interface()
          ...
          if(emm_ctrl_data_ptr->is_srvcc_in_progress == TRUE)
          {
            ...
            {
              MSG_HIGH_DS_0(MM_SUB,"=EMM= Processing Early CSFB Call for SRVCC ");
          ...
          *mm_emm_dispatcher_fn_ptr = mm_emm_normal_msg_handler;
          ...
          {
            if(mm_irat_pending_reg_cmd_ptr->cmd.hdr.message_set==MS_MM_REG &&
               mm_irat_pending_reg_cmd_ptr->cmd.hdr.message_id==MMR_REG_REQ)
            
            {        
                 mm_service_loss_on_rat=mm_serving_plmn.info.active_rat;
            }
           (void)mmcoord_route_mm_emm_messages(mm_irat_pending_reg_cmd_ptr,
                                                emm_ctrl_data_ptr,
        ...
        default:
          MSG_HIGH_DS_1(MM_SUB,"=EMM= Dropping SM CMD %d during L2GW IRAT",
    ...
    case MS_MM_REG:
      switch(mm_cmd_ptr->cmd.hdr.message_id)
      {
        ...
        case MMR_PS_DETACH_REQ:
          ...
          MSG_HIGH_DS_1(MM_SUB,"=EMM= Buffering REG CMD %d rcved during L2GW IRAT", 
                   mm_cmd_ptr->cmd.hdr.message_id);
          mm_irat_pending_reg_cmd_ptr_mem_alloc(mm_cmd_ptr);
          break;

        case MMR_SET_DRX_REQ:
          MSG_HIGH_DS_0(MM_SUB,"=MM= MM received in GW2L inter-rat, return failure");
        ...
        case MMR_DATA_PRIORITY_IND:
          mm_ds_proc_type = mm_cmd_ptr->cmd.mmr_data_prio_ind.proc_type;
          break;
#endif
        ...
        default:
          MSG_HIGH_DS_1(MM_SUB,"=EMM= Dropping REG CMD %d during L2GW IRAT",
                   mm_cmd_ptr->cmd.hdr.message_id);
          break;
      }
      break;
    ...
    case MS_MM_RR:
      if ((mm_cmd_ptr->cmd.hdr.message_id == RR_GMM_GPRS_MULTISLOT_CLASS_IND) ||
          (mm_cmd_ptr->cmd.hdr.message_id == RR_PLMN_RESEL_FAIL_IND) ||
          (mm_cmd_ptr->cmd.hdr.message_id == RR_GMM_GPRS_MS_RA_CAP_REQ) ||
      ...
      {
        MSG_HIGH_DS_2(MM_SUB,"=EMM= Dropping message with message set %d message id %d during L2GW IRAT",
              mm_cmd_ptr->cmd.hdr.message_set, mm_cmd_ptr->cmd.hdr.message_id);
      }
      break;
    case MS_CC_MM:
    case MS_CC_MM_PS:
      switch(mm_cmd_ptr->cmd.hdr.message_id)
      {
        ...
        case MMCNM_REL_REQ:
           ...
           if(emm_ctrl_data_ptr->is_srvcc_in_progress && (mm_state == MM_CONNECTION_ACTIVE))
           {
        ...
        case MMCNM_DATA_REQ:
           MSG_HIGH_DS_0(MM_SUB,"Ignoring MMCNM_DATA_REQ in L2GW I-RAT handler");
           break;
        
        case MMCC_REEST_REQ:
           MSG_HIGH_DS_0(MM_SUB,"Ignoring MMCC_REEST_REQ in L2GW I-RAT handler");
           break;
        
        case MMCNM_UNIT_DATA_REQ:
           MSG_HIGH_DS_0(MM_SUB,"Ignoring MMCNM_UNIT_DATA_REQ in L2GW I-RAT handler");
           break;
        
#ifdef FEATURE_CCBS 
        case MMCNM_PROMPT_RES:
           MSG_HIGH_DS_0(MM_SUB,"Ignoring MMCNM_PROMPT_RES in L2GW I-RAT handler");
           break;
#endif
     ...
     case MS_MM_MM:
       switch(mm_cmd_ptr->cmd.hdr.message_id)
       {
         ...
         case MM_QSH_MESSAGE_IND:
           mmcoord_process_qsh_message_ind(mm_cmd_ptr);
           break;
#endif
         default:
          MSG_ERROR_DS_1(MM_SUB,"Unexpected MM to MM message ID %d",mm_cmd_ptr->cmd.hdr.message_id);
          break;
       }
       break;
       ...
       case MS_CM_MM:
        switch(mm_cmd_ptr->cmd.hdr.message_id)
        {
           case MMCM_MMTEL_CALL_INFO:
             emm_process_mmtel_call_status_info(mm_cmd_ptr,emm_ctrl_data_ptr);
             break;
#ifdef FEATURE_CRAT_MSIM_TEST_FRAMEWORK
           case MMCM_LTE_SUSPEND_REQ:
             emm_process_cm_lte_suspend_req(emm_ctrl_data_ptr);
             break;

           case MMCM_LTE_RESUME_IND:
             emm_process_cm_lte_resume_ind(emm_ctrl_data_ptr);
             break;
#endif
           default:
             MSG_ERROR_DS_1(MM_SUB,"Unexpected CM to MM message ID %d",mm_cmd_ptr->cmd.hdr.message_id);
             break;
        }
        break;
    default:
                                  ...
                                  rrc_sim_update_e_type sim_update_type)
{
  ...
  MSG_HIGH_DS_0(MM_SUB,"Updating the mapped security keys for GW");
...
{
  emm_ctrl_data_type *emm_ctrl_data_ptr = NULL;
  boolean update_rplmn = FALSE;
  emm_ctrl_data_ptr = emm_db_get_ctrl_data();
  update_rplmn = emm_update_rplmn(emm_ctrl_data_ptr);
  ASSERT(emm_ctrl_data_ptr != NULL);
  if(update_rplmn == TRUE)
  {
  ...
  {
    mm_emm_notify_rplmn_update_event_qsh(emm_ctrl_data_ptr->rplmn_info.rplmn);
  }
#endif
}
...
)
{
  if(EMM_GET_STATE() == EMM_REGISTERED)
  {
    ...
    MSG_HIGH_DS_2(MM_SUB,"=EMM= prev radio_cap_update %d new radio_cap_update %d",
                     emm_ctrl_data_ptr->radio_cap_update,radio_cap_update);
    emm_ctrl_data_ptr->radio_cap_update = radio_cap_update;

    emm_ctrl_data_ptr->ra_cap_enquiry_nw_ratmask  = 0x00; /* Reset RAT mask to zero upon new capability change */
  }
}
...
FUNCTION    emm_get_radio_cap_update_state  

DESCRIPTION
  This function returns the radio cap update status.
...
emm_radio_cap_update_enum_T emm_get_radio_cap_update_state
(
  void
)
{
  emm_ctrl_data_type *emm_ctrl_data_ptr = emm_db_get_ctrl_data();
  return emm_ctrl_data_ptr->radio_cap_update;
}
...
void emm_set_state_multimode_rau_failure(lte_nas_emm_cause_type cause)
{
  ...
  MSG_HIGH_DS_1(MM_SUB,"=EMM= GW RAU REJ - Updating EMM w/ cause %d", cause);
  ...
  switch(gmm_state)
  {
    case GMM_SERVICE_REQUEST_INITIATED:
     /*Cosider abnormal failure of service request*/
      ...
      { 
        mm_irat_pending_reg_cmd_ptr_mem_alloc(&mmcoord_pending_reg_message);
      ...
      {
        MSG_HIGH_DS_0(MM_SUB, "Sending GMMSM_ESTABLISH_REJ");
      ...
      else if (gmm_service_request_initiator == MS_GMM_RABM)
      {
        ...
        MSG_HIGH_DS_0(MM_SUB, "Sending GMMRABM_REESTABLISH_REJ");
    ...
    case GMM_REGISTERED:
      ...
      if((mm_serving_plmn.ms_op_mode == SYS_SRV_DOMAIN_CS_ONLY ) &&
	     (mm_serv_change_pending == TRUE) && (gmm_is_suspended == TRUE))
      {
    	gmm_handle_local_gprs_detach();
    ...
    default:
      MSG_HIGH_DS_1(MM_SUB, "default gmm_state = %d", gmm_state);
      break;
...
)
{
   ...
   if (mm_nreg_req_received )
   {
     MSG_HIGH_DS_0(MM_SUB, "MMR_STOP_MODE_REQ pending - Received ACTIVATION_IND");
   }

   if (mm_rat_change_pending)
   {
     MSG_HIGH_DS_0(MM_SUB, "RAT change pending - Ignoring ACTIVATION_IND");
   }
   else if(mm_waiting_for_abort_cnf && mm_serving_plmn.info.active_rat == SYS_RAT_TDS_RADIO_ACCESS)
   {
     MSG_HIGH_DS_0(MM_SUB, "Abort CNF pending in TDS - Ignoring ACTIVATION_IND");
   }
   else if (mm_waiting_for_deact_cnf)
   {
     MSG_HIGH_DS_0(MM_SUB, "Deactivation pending - Ignoring ACTIVATION_IND");
   }
   else if (mm_waiting_for_stop_mode_cnf)
   {
     MSG_HIGH_DS_0(MM_SUB, "RAT Stop request pending - Ignoring ACTIVATION_IND");
   }
   else if (mm_waiting_for_service_cnf)
   {
     MSG_HIGH_DS_0(MM_SUB, "Service or PLMN select request pending - Ignoring ACTIVATION_IND");
   }
   else if ((rrc_connection_status == MMRRC_IDLE) &&
            (mm_waiting_for_list_cnf_type == SYS_NETWORK_SELECTION_MODE_MANUAL)
           )
   {
     MSG_HIGH_DS_0(MM_SUB, "Waiting for LIST CNF - Manual in Idle mode - Ignoring ACTIVATION_IND");
   }
#ifdef FEATURE_WRLF_SYSTEM_SEL
   else if(mm_wcdma_rlf_state != WCDMA_RLF_IDLE)
   {
     MSG_HIGH_DS_1(MM_SUB, "Ignoring LTE_RRC_ACTIVATION_IND message rlf state %d", mm_wcdma_rlf_state);
   }
#endif
   else if(emm_ctrl_data_ptr->l2gcco_state == EMM_LTOG_CCO_ABORTED)
   {
     MSG_HIGH_DS_0(MM_SUB, "Ignoring LTE_RRC_ACTIVATION_IND in CCO aborted state");
   }
   ...
   else if (mm_emm_irat_during_call(act_ind_ptr->act_reason)) 
   {
     MSG_HIGH_DS_0(MM_SUB, "Ignoring ACTIVATION_IND from L-RRC as possible call on GW");
   }
#endif
   else
   {
...
#ifdef FEATURE_DISABLED_HABANERO   	
       case LTE_RRC_ACT_IRAT_BLIND_REDIR:  
#endif
       case LTE_RRC_ACT_IRAT_CELL_RESEL:
       case LTE_RRC_ACT_IRAT_CCO_FAILURE:  
       case LTE_RRC_ACT_IRAT_HO:
         MSG_HIGH_DS_1(MM_SUB, "LTE activation received with cause %d", act_ind_ptr->act_reason);
         ...
         if(MM_SUB_IS_SGLTE)
         {
           MSG_HIGH_DS_2(MM_SUB,"=EMM=MM_IS_IN_SGLTE_MODE =%d mm_sglte_current_stack_status = %d ",MM_IS_IN_SGLTE_MODE,mm_sglte_current_stack_status);
         }
         ...
         */
            ...
            (is_lte_mode() == FALSE))
         {
          ...
          //CSPS to PS when call was ongoing and G2L redirection happenend
         ...
         if ((MM_IS_NON_DDS_SUB) &&
             ...
             ((gmm_state != GMM_DEREGISTERED) && mm_serv_change_pending) &&
             (mmcoord_pending_reg_message.cmd.mmr_reg_req.volte_only == TRUE))
         {
           gmm_detach_type = GMM_GPRS_DETACH;
           mm_serv_change_pending = FALSE;
           gmm_handle_gprs_detach_low_level_failure();
         ...
         if (mmcoord_reg_req_pending)
         {
            ...
            MSG_HIGH_DS_0(MM_SUB, "=EMM= Buffering MMR_REG_REQ pended during IRAT in LTE context ");
            mm_irat_pending_reg_cmd_ptr_mem_alloc(&mmcoord_pending_reg_message);
         }
                 ...
                 ))
         {
           ...
           MSG_HIGH_DS_1(MM_SUB, "=EMM= Buffering Alt Scan Stop mode Req (%d) pended during IRAT in LTE context ",SYS_STOP_MODE_REASON_ALT_SCAN);
           mmcoord_pending_reg_message.cmd.hdr.message_id = MMR_STOP_MODE_REQ;
#ifdef FEATURE_DUAL_SIM
           MSG_HIGH_DS_1(MM_SUB, "=EMM= Buffering Alt Scan/ Dual switch stop mode Req during IRAT in LTE context, mm_abort_dual_switch_pending %d ",mm_abort_dual_switch_pending); 
           if (mm_abort_dual_switch_pending == SYS_STOP_MODE_REASON_DUAL_SWITCH)
		   {
             mmcoord_pending_reg_message.cmd.mmr_stop_mode_req.stop_mode_reason = SYS_STOP_MODE_REASON_DUAL_SWITCH;
           ...
           mmcoord_pending_reg_message.cmd.mmr_stop_mode_req.trans_id = mmr_stop_mode_trans_id;
#ifdef FEATURE_DUAL_SIM
           mmcoord_pending_reg_message.cmd.mmr_stop_mode_req.as_id = (sys_modem_as_id_e_type)mm_as_id;
#endif
           mm_irat_pending_reg_cmd_ptr_mem_alloc(&mmcoord_pending_reg_message);
         }
			...
			if ((emm_ctrl_data_ptr->reg_req_pending_ptr != NULL) &&
             ...
             (act_ind_ptr->act_reason == LTE_RRC_ACT_IRAT_REDIR)
             )  
			{
				mm_set_lte_redir_during_lu(TRUE);  
			}
           ...
           if (rrc_connection_status != MMRRC_IDLE)
           {
             if(pmm_mode == PMM_CONNECTED)
             {
               if(act_ind_ptr->act_reason == LTE_RRC_ACT_IRAT_CELL_RESEL)
               {
                 ...
                 emm_ctrl_data_ptr->tau_cause = TAU_CAUSE_OTHERS;
#ifdef FEATURE_NAS_REL12_SPEC_CHANGE
                 /* Spec 24.008 4.7.5.1 - CR 2553: W to L reselection triggered in PMM connected mode. If UE reselects back to W without triggering 
                  * TAU in S1 mode, then need to trigger RAU in Iu mode to sync PMM state */
                 if((gmm_state == GMM_REGISTERED) || 
                    (gmm_state == GMM_ROUTING_AREA_UPDATING_INITIATED) || 
                    (gmm_state == GMM_SERVICE_REQUEST_INITIATED))
                 {
                   gmm_set_sync_pmm_state_during_w_to_lg_and_back_to_w_irat(TRUE);
                 }
#endif
               }
		 ...
		 mm_set_lte_redir_during_lu(FALSE); 
         ...
         else
         {
            ...
            )
           {
			 ...
			 mm_csfb_rej_pending_call(mm_irat_pending_reg_cmd_ptr);
#ifdef FEATURE_SGLTE
          ...
          else
           {
             ...
             {
               if ((mm_irat_pending_reg_cmd_ptr != NULL) && 
                    (mm_irat_pending_reg_cmd_ptr->cmd.hdr.message_id == (int)MMR_STOP_MODE_REQ))
               {
         ...
         {
           MSG_HIGH_DS_0(MM_SUB,"=EMM= TAU to be performed - Mobility Management for IMS voice termination");
           emm_ctrl_data_ptr->tau_cause = TAU_CAUSE_OTHERS;
           emm_set_and_write_tin_type(NAS_MM_TIN_P_TMSI);
...
#if defined FEATURE_DUAL_SIM && defined FEATURE_DUAL_DATA
		 if (MM_IS_NON_DDS_SUB &&
		     emm_ctrl_data_ptr->local_rel_tau_after_csfb_tau == TRUE &&
		 	 emm_ctrl_data_ptr->second_tau_after_csfb_needed == EMM_CSFB_CON_REL_PEND_ESR &&
		 	 (act_ind_ptr->act_reason == LTE_RRC_ACT_IRAT_REDIR 
#ifdef FEATURE_DISABLED_HABANERO
		 	  || act_ind_ptr->act_reason == LTE_RRC_ACT_IRAT_BLIND_REDIR
#endif
			  )) 
		 {
           emm_ctrl_data_ptr->second_tau_after_csfb_needed = EMM_CSFB_CON_REL_PEND_REDIR;
		 }
		 else
		 {
           emm_ctrl_data_ptr->second_tau_after_csfb_needed = EMM_CSFB_CON_REL_PEND_NONE;
		 }
		 MSG_HIGH_DS_1(MM_SUB,"=EMM= second_tau_after_csfb_needed %d", 
              emm_ctrl_data_ptr->second_tau_after_csfb_needed);
#endif
         ...
         *mm_emm_dispatcher_fn_ptr = mm_process_GW_to_LTE_rat_change;
         MSG_HIGH_DS_1(MM_SUB,"=EMM= MM: Change the main routine handler to mm_process_GW_to_LTE_rat_change due to %d", 
              act_ind_ptr->act_reason);
         break;
   ...
   mm_set_is_ss_request_rrc_sent(FALSE);
}
...
{  
  MSG_HIGH_DS_0(MM_SUB,"EMM registration success, Updating GMM state");
  gmm_set_state( GMM_REGISTERED );
  gmm_set_substate( GMM_NO_CELL_AVAILABLE );
...
)
{
  ...
  switch(reject_cause)
  {
     ...
     **-------------------------------------------------*/ 
      ...
      MSG_HIGH_DS_1(MM_SUB,"GPRS Attempt counter = %d", gprs_attach_attempt_counter);
      gmm_set_state( GMM_DEREGISTERED );
      if (mm_serving_plmn.ms_op_mode == SYS_SRV_DOMAIN_CS_PS)
      {    
        mm_delete_lu_status( ROAMING_NOT_ALLOWED );
...
)
{
  ...
  switch(reject_cause)
  {
     ...
     **-------------------------------------------------*/ 
      ...
      MSG_HIGH_DS_1(MM_SUB, "T3330 timeout counter = %d", t3330_timeout_counter);
...
)
{
  if (emm_get_mmr_attach_failed_ind_status() !=  ATTACH_FAILED_IND_NOT_REQUIRED )
  {
    ...
    MSG_HIGH_DS_0(MM_SUB, "=EMM= MM sent MMR_ATTACH_FAILED_IND");
...
void emm_store_attach_ind_status_type(mmr_reg_req_s_type        *reg_req_ptr)
{
   ...
   else
   {
    ...
    MSG_HIGH_DS_1(MM_SUB, "=EMM= Setting the ATTACH FAILED IND flag Value = %d ",mm_mmr_attach_ind_status);
}
/*===========================================================================
...
void emm_reset_attach_ind_status_type(void)
{
   ...
   mm_mmr_attach_ind_status = ATTACH_FAILED_IND_NOT_REQUIRED;
   MSG_HIGH_DS_0(MM_SUB, "=EMM= resetting the ATTACH FAILED IND flag Value");
}
...
void mm_emm_store_reg_req(mmr_reg_req_s_type* reg_req_ptr,   emm_ctrl_data_type     *emm_ctrl_data_ptr)
{
  ...
  mmcoord_pending_reg_message.cmd.mmr_reg_req.req_service_domain = 
              mm_get_effective_req_srv_domain(mmcoord_pending_reg_message.cmd.mmr_reg_req.req_service_domain, 
                emm_ctrl_data_ptr->reg_req_pending_ptr->volte_only, FALSE);
#endif 
}
...
void mm_emm_store_reg_req_nw_selection_mode(sys_network_selection_mode_e_type   network_selection_mode)
{
  ...
  if(mmcoord_pending_reg_message.cmd.hdr.message_id == MMR_REG_REQ)
  {
...
/* MNK ESM ISR begin*/
         ...
         *mm_emm_dispatcher_fn_ptr = mm_emm_normal_msg_handler;
         ...
         if(MM_SUB_IS_SGLTE)
         {
           MSG_HIGH_DS_2(MM_SUB,"=EMM= mm_sglte_current_stack_status = %d pending msgid = %d  ", 
             mm_sglte_current_stack_status ,mm_irat_pending_reg_cmd_ptr->cmd.hdr.message_id );
         }
#endif
 #ifdef FEATURE_SGLTE 
         if (MM_IS_IN_SGLTE_STATE_4 &&
...
/* MNK ESM ISR END*/
       ...
       else if ((mm_cmd_ptr->cmd.hdr.message_set == MS_MM_RR) &&
                ((mm_cmd_ptr->cmd.hdr.message_id == RR_GMM_GPRS_MULTISLOT_CLASS_IND) ||
                 (mm_cmd_ptr->cmd.hdr.message_id == RR_PLMN_RESEL_FAIL_IND) ||
                 (mm_cmd_ptr->cmd.hdr.message_id == RR_GMM_GPRS_MS_RA_CAP_REQ)))
       {
         MSG_HIGH_DS_0(MM_SUB,"=MM= Received Multislot/MS Radio capbility indication or RR_PLMN_RESEL_FAIL_IND in OOS RLF handler");
         mmcoord_route_mm_message(mm_cmd_ptr,mm_emm_dispatcher_fn_ptr);
       }
       else
       {
         MSG_HIGH_DS_1(MM_SUB,"=EMM= MM: NAS received msg %d during INTER_RAT, this message shall be currently discarded",
                 mm_cmd_ptr->cmd.hdr.message_id);
       }
       break;
     ...
     case MS_MM_REG:
       switch(mm_cmd_ptr->cmd.hdr.message_id)
       {
         ...
         case MMR_PS_DETACH_REQ:
           ...
           mm_irat_pending_reg_cmd_ptr_mem_alloc(command_ptr);
     ...
     case MS_GMM_SM:
       switch(mm_cmd_ptr->cmd.hdr.message_id)
       {
         ...
         case GMMSM_UNITDATA_REQ:
           MSG_HIGH_DS_0(MM_SUB,"Ignoring GMMSM_UNITDATA_REQ in GW2L I-RAT handler");
           break;
#ifdef FEATURE_DUAL_SIM
         ...
         case GMMSM_CLOSE_SESSION_REQ:
           MSG_HIGH_DS_0(MM_SUB,"Ignoring GMMSM_CLOSE_SESSION_REQ in GW2L I-RAT handler");
           break;

         case GMMSM_RAT_RESUME_CNF:
           MSG_HIGH_DS_0(MM_SUB,"Ignoring GMMSM_RAT_RESUME_CNF in GW2L I-RAT handler");
           break;
 
         case GMMSM_FAST_DORMANCY_REQ:
           MSG_HIGH_DS_0(MM_SUB,"Ignoring GMMSM_FAST_DORMANCY_REQ in GW2L I-RAT handler");
           break;
     ...
     case MS_CC_MM_PS:
       switch(mm_cmd_ptr->cmd.hdr.message_id)
       {
         ...
         case MMCNM_DATA_REQ:
           MSG_HIGH_DS_0(MM_SUB,"Ignoring MMCNM_DATA_REQ in GW2L I-RAT handler");
           break;
  
         case MMCC_REEST_REQ:
           MSG_HIGH_DS_0(MM_SUB,"Ignoring MMCC_REEST_REQ in GW2L I-RAT handler");
           break;
  
         case MMCNM_UNIT_DATA_REQ:
           MSG_HIGH_DS_0(MM_SUB,"Ignoring MMCNM_UNIT_DATA_REQ in GW2L I-RAT handler");
           break;
  
#ifdef FEATURE_CCBS 
         case MMCNM_PROMPT_RES:
           MSG_HIGH_DS_0(MM_SUB,"Ignoring MMCNM_PROMPT_RES in GW2L I-RAT handler");
           break;
#endif
...
void emm_multimode_detach(void)
{
  ...
  MSG_HIGH_DS_0(MM_SUB,"=EMM= Moving to DEREGISTERED STATE, Update EPS security_context...");
...
)
{
 ...
 /* Populate EHPLMN list or HPLMN, and RPLMN */
  ...
  if(emm_ctrl_data_ptr->emm_unsec_redir_allowed == UNSECUR_REDIRECTION_TO_GRR_ALLOWED)
  {
    rrc_system_update_req_ptr->not_allow_unsecured_redir = FALSE;
  }
  else
  {
    rrc_system_update_req_ptr->not_allow_unsecured_redir = TRUE;
  }
 #endif
  ...
  {
   #ifdef FEATURE_DISABLED_HABANERO
    MSG_HIGH_DS_1(MM_SUB, "=EMM= Sent LTE_RRC_SYSTEM_UPDATE_REQ not_allow_unsecured_redir %d",rrc_system_update_req_ptr->not_allow_unsecured_redir );
   #else
    MSG_HIGH_DS_0(MM_SUB, "=EMM= Sent LTE_RRC_SYSTEM_UPDATE_REQ not_allow_unsecured_redir");
   #endif
  }
...
)
{
  ...
  {
    MSG_HIGH_DS_1(MM_SUB,"=EMM= Performed MT Detach, Updating GMM/MM Cause %d", cause);
...
void emm_service_loss_on_lte(void)
{
  ...
  {
    MSG_HIGH_DS_0(MM_SUB, "=EMM= Loss of service in LTE");
...
)
{
...
#ifdef FEATURE_NAS_REL12_SPEC_CHANGE
  /*C1-143343 CR  2596*/
  if(emm_get_mm_spec_rel_ver() >= ((LTE_3GPP_REL12)))
  {
  ...
  {
    MSG_HIGH_DS_0(MM_SUB,"=EMM= TAU to be performed - GMM READY STATE");
#ifdef FEATURE_LTE_REL11
    if((emm_ctrl_data_ptr->emm_3gpp_spec_ver >= LTE_3GPP_REL11) &&
...
)
{
  ...
  MSG_HIGH_DS_1(MM_SUB,"=EMM= GW SRV REJ - Updating EMM w/ cause %d", emm_cause);
...
)
{
  ...
  MSG_HIGH_DS_1(MM_SUB,"=EMM= GW MT Detach - Updating EMM w/ cause %d", emm_cause);
  ...
  switch (emm_cause)
  {
    ...
    case LTE_NAS_NOT_AUTHORIZED_FOR_THIS_CSG:
      MSG_HIGH_DS_0(MM_SUB,"=EMM= Resetting attach attempt counter - Multimode detach ");
      emm_ctrl_data_ptr->emm_attach_attempt_counter = 0;
      break ;
    default:
      break;
...
void mm_handle_isr_timer_expiry(emm_ctrl_data_type *emm_ctrl_data_ptr)
{
     ...
     (mm_serving_plmn.info.service_status == SYS_SRV_STATUS_SRV))
  {
       ...
       (mm_serving_plmn.network_op_mode == NETWORK_OP_MODE_3))
    {
       if(mm_state != MM_IDLE)
       {
         MSG_HIGH_DS_0(MM_SUB,"Pending LU as MM state is not IDLE");
...
void  emm_attach_tau_success_update_gw(void)
{
  ...
  {
    MSG_HIGH_DS_0(MM_SUB,"=EMM= Reset GW PS session suspended flag");
  ...
  /* Stop RAU retry timer as TAU accepted, next time after reselection/camping on GW,
     UE needs to do RAU immediately*/
...
void  mm_send_rrc_abort_req_during_wrlf(void)
{
  ...
  MSG_HIGH_DS_0(MM_SUB,"=MM= WCDMA RLF IDLE STATE");
  mm_substate_control(MM_STD_NO_SERVICE);
  gmm_set_substate( GMM_NO_CELL_AVAILABLE );
}
#endif
...
void mm_csfb_set_state(mm_csfb_state_type new_csfb_state)
{
  ...
  if(new_csfb_state == MM_CSFB_STATE_MO_CALL_ORIG)
  {
    if( mm_csfb_service_req_message_ptr == NULL )
    {
      mm_csfb_service_req_message_ptr =
                (mm_cmd_type *)modem_mem_calloc(1, sizeof( mm_cmd_type ),MODEM_MEM_CLIENT_NAS);
  ...
  */
     ...
     (mm_csfb_state == MM_CSFB_STATE_NULL))
  {
    ...
    emm_send_rrc_csfb_call_status_ind(LTE_RRC_CSFB_CALL_STARTED,new_csfb_state);
              ...
              (mm_csfb_state == MM_CSFB_STATE_MT_CALL_CCO)))
  {
    ...
    emm_send_rrc_csfb_call_status_ind(LTE_RRC_CSFB_CALL_ENDED,new_csfb_state);
...
if((new_csfb_state != MM_CSFB_STATE_MT_CALL_ORIG) && (new_csfb_state != MM_CSFB_STATE_MO_CALL_ORIG))    
{
  ...
  MSG_HIGH_DS_1(MM_SUB,"CSFB State set to %d",new_csfb_state);
}
...
void mm_csfb_start_mo_call_orig(mm_cmd_type *msg_p)
{
...
#ifdef FEATURE_QSH_EVENT_METRIC
#ifdef FEATURE_DISABLED_HABANERO
  if (msg_p->cmd.mmcnm_est_req.CNM_service_type == MO_CALL_ESTABLISHMENT  ||
      msg_p->cmd.mmcnm_est_req.CNM_service_type == MO_CALL_ESTABLISHMENT_HALF_SPEECH ||
      msg_p->cmd.mmcnm_est_req.CNM_service_type == MO_CALL_ESTABLISHMENT_FULL_SPEECH ||
      msg_p->cmd.mmcnm_est_req.CNM_service_type == EMERGENCY_CALL_ESTABLISHMENT)
  {
     QEVENT(QSH_CLT_MM, MM_QSH_EVENT_CS_CALL_STATUS, MM_QSH_EVENT_CS_CALL_STATUS_VERSION, mm_sub_id,
             "=EMM= Call status %d Call direction %d", CAFI_KPI_EVENT_IND_START, CM_CALL_DIRECTION_MO);
     mm_qevent_call_status_set(TRUE);
  }
#endif
#endif
  ...
  if(((mm_csfb_state == MM_CSFB_STATE_MO_CALL_ORIG) || 
      ...
      (msg_p->cmd.mmcnm_est_req.CNM_service_type != EMERGENCY_CALL_ESTABLISHMENT))
  {
    MSG_HIGH_DS_1(MM_SUB, "CSFB: Another FB procedure in progress, send EST_REJ - %d", mm_csfb_state);
  ...
  #ifdef FEATURE_SMS_ENHANCED_DOMAIN_SELECTION
	  mm_send_mmcnm_est_rej( EMM_REJECT_CAUSE, EMM_REJ_SERVICE_REQ_FAILURE_CS_DOMAIN_TEMP_NOT_AVAILABLE , MMCC_SMS_CS);
  #else
	  mm_send_mmcnm_est_rej( EMM_REJECT_CAUSE, EMM_REJ_SERVICE_REQ_FAILURE_CS_DOMAIN_TEMP_NOT_AVAILABLE );
  #endif
	}
  /* If T3246 is running and it is not emergency call then reject the ESR procedure */
  else if((emm_ctrl_data_ptr->emm_3gpp_spec_ver >= LTE_3GPP_REL10) &&
     (msg_p->cmd.mmcnm_est_req.CNM_service_type != EMERGENCY_CALL_ESTABLISHMENT) &&
...
** ---------------------------- ---------  ---------------------------- ---------  ---------------------------- --------- */
  ...
  {
    MSG_HIGH_DS_0(MM_SUB,"=EMM= TIMER T3246 is running - ESR cannot be started");
  #ifdef FEATURE_SMS_ENHANCED_DOMAIN_SELECTION
  ...
  else if((msg_p->cmd.mmcnm_est_req.CNM_service_type == SUPPLEMENTARY_SERVICE_ACTIVATION) && 
          (EMM_GET_STATE() == EMM_REGISTERED) &&
          (EMM_GET_SUBSTATE() == EMM_REGISTERED_NO_CELL_AVAILABLE) &&
          (emm_ctrl_data_ptr->rlf_state == TRUE))
  {
    MSG_HIGH_DS_0(MM_SUB,"CSFB: Aborting SUPS request during LTE RLF");
#ifdef FEATURE_SMS_ENHANCED_DOMAIN_SELECTION
    mm_send_mmcnm_est_rej( MM_REJECT_CAUSE, MM_REJ_NO_SERVICE , MMCC_SMS_CS); 
#else
    mm_send_mmcnm_est_rej( MM_REJECT_CAUSE, MM_REJ_NO_SERVICE );
  ...
  else if((emm_ctrl_data_ptr->rrc_active == FALSE ) &&
		  (EMM_GET_STATE() == EMM_REGISTERED) &&
          (EMM_GET_SUBSTATE() == EMM_REGISTERED_NO_CELL_AVAILABLE) )
  {
	  
	MSG_HIGH_DS_0(MM_SUB,"CSFB: Aborting CS call when RRC not active,Abnormal case");
#ifdef FEATURE_SMS_ENHANCED_DOMAIN_SELECTION
    mm_send_mmcnm_est_rej( MM_REJECT_CAUSE, MM_REJ_NO_SERVICE , MMCC_SMS_CS); 
#else
    mm_send_mmcnm_est_rej( MM_REJECT_CAUSE, MM_REJ_NO_SERVICE );
  ...
  {
    MSG_HIGH_DS_0(MM_SUB,"CSFB: Received MO CSFB call");
    ...
    emm_csfb_perform_registration(NAS_EMM_3GPP_CSFB);
#endif
    //Start ESR Procedure
...
{   
  ...
  if(mm_qevent_call_status_get() == TRUE)
  {
    QEVENT(QSH_CLT_MM, MM_QSH_EVENT_CS_CALL_STATUS, MM_QSH_EVENT_CS_CALL_STATUS_VERSION, mm_sub_id,
			     "=EMM= call status %d", CAFI_KPI_EVENT_IND_FAIL);
           ...
           mm_csfb_state == MM_CSFB_STATE_MO_CALL_ORIG)
  {
    ...
    {
      MSG_HIGH_DS_1(MM_SUB,"CSFB: ESR reject with OTA cause type = %d", failure_cause_type.failure_cause.lte_nas_ota_emm_cause);
      switch(failure_cause_type.failure_cause.lte_nas_ota_emm_cause)
      {
        ...
        case LTE_NAS_IMPLICITLY_DETACHED: 
          ...
          /*C1-130466 
            ...
            The UE shall set the EPS update status to EU2 NOT UPDATED (and shall store it according to subclause 5.1.3.3) and
            shall delete any GUTI, last visited registered TAI, TAI list and eKSI. The UE shall enter the state EMM-DEREGISTERED.
            If the service request was initiated for CS fallback, the UE shall select GERAN or UTRAN radio access technology.
            It then proceeds with appropriate MM and CC specific procedures. The EMM sublayer shall not indicate the abort of the service request procedure to the MM sublayer.
          */
          ...
          break;
        /* Added this new cause, with this cause CM will not start silent rediral after silent redial timer expiry and will place the call on GW directly after getting the camped indication frm NAS*/
        case LTE_NAS_EPS_SERVICES_NOT_ALLOWED: 
           move_to_gw = FALSE;
           cause_type = EMM_REJECT_CAUSE;
           cause_value = EMM_REJ_SERVCIE_REQ_FAILURE_EPS_SERVICE_NOT_ALLOWED;
           break;
        ...
        case LTE_NAS_CS_DOMAIN_TEMP_NOT_AVAILABLE:  
           ...
           */
           ...
           {   
             move_to_gw = FALSE; 
             cause_type = EMM_REJECT_CAUSE;
             cause_value = EMM_REJ_SERVICE_REQ_FAILURE_CS_DOMAIN_TEMP_NOT_AVAILABLE;
           }
           else
#endif
           {
    ...
    {
      MSG_HIGH_DS_1(MM_SUB,"CSFB: ESR reject with NON OTA cause type = %d", failure_cause_type.failure_cause.lte_nas_failure_cause);
      switch(failure_cause_type.failure_cause.lte_nas_failure_cause)
      {
        case EMM_T3417_EXT_EXPIRED: 
           if (emm_ctrl_data_ptr->enable_3417_ext_mt_timer == TRUE &&
		       (mm_csfb_state == MM_CSFB_STATE_MO_CALL_ORIG ||
		   	    mm_csfb_state == MM_CSFB_STATE_MT_CALL_ORIG)
		   	   ) 
           {
			 MSG_HIGH_DS_0(MM_SUB, "=EMM= enable_3417_ext_mt_timer enabled");  
             move_to_gw = TRUE;
		   }
		   else if(mm_csfb_state == MM_CSFB_STATE_MO_CALL_ORIG)
           {             
    ...
    if(move_to_gw == TRUE)
    {
      ...
      if((sd_ss_current_mcc_match_mcc_list_emerg_call_full_srv(&emm_ctrl_data_ptr->plmn_service_state->plmn, 
                                                  (sys_modem_as_id_e_type)mm_sub_id) == TRUE) ||
         (full_srv_req_in_emerg == TRUE))
      {
#if defined(FEATURE_DUAL_SIM) || defined(FEATURE_SGLTE)
        byte diag_event[1];
        diag_event[0] = mm_as_id + 1;
    ...
    else
    {
      ...
      else if(mm_csfb_state == MM_CSFB_STATE_MT_CALL_ORIG)
      {
        ...
        mm_csfb_set_state(MM_CSFB_STATE_NULL);
        if ((mm_timer_status[TIMER_RADIO_CAP_ENQ] == TIMER_ACTIVE) || (mm_timer_status[TIMER_RADIO_CAP_ENQ] == TIMER_PAUSED))
        {
          emm_timer_radio_cap_enquiry_expiry(emm_ctrl_data_ptr);
        }
      }
      mm_csfb_set_state(MM_CSFB_STATE_NULL);
...
void mm_csfb_rej_pending_call
(
  mm_cmd_type  *cmd_ptr
)
{  
   reject_cause_T cause_value = MM_REJ_WRONG_STATE;
   emm_ctrl_data_type *emm_ctrl_data_ptr = NULL;
   emm_ctrl_data_ptr = emm_db_get_ctrl_data();
   ASSERT(emm_ctrl_data_ptr != NULL);
  ...
  if(mm_csfb_state == MM_CSFB_STATE_MO_CALL_ORIG)
  {
	 if(mm_emm_csfb_domain_change_in_progress(cmd_ptr) == TRUE)
     {
        cause_value = MM_REJ_DOMAIN_CHNG;
     }
 #ifdef FEATURE_SMS_ENHANCED_DOMAIN_SELECTION
    mm_send_mmcnm_est_rej( MM_REJECT_CAUSE, cause_value, MMCC_SMS_CS);
 #else
    mm_send_mmcnm_est_rej( MM_REJECT_CAUSE, cause_value );
  #endif
    mm_follow_on_request = FALSE;
    mm_csfb_set_state(MM_CSFB_STATE_NULL);
  } 
...
void emm_process_multimode_csfb_failure(emm_ctrl_data_type* emm_ctrl_data_ptr, emm_csfb_failure_cause_type failure_cause_type)
{
  ...
  MSG_HIGH_DS_2(MM_SUB,"ESR failed for CSFB call, ESR type = %d, CSFB failure cause_type = %d", emm_ctrl_data_ptr->esr_type_in_progress,failure_cause_type.failure_type);
  ...
  MSG_HIGH_DS_2(MM_SUB, "CSFB: ESR failed, sending reject to CM for: type= %d cause = %d", cause_type, cause_value);
     ...
     || (emm_ctrl_data_ptr->esr_type_in_progress == NAS_EMM_1X_CSFB_BUFFERED) )
  {
    ...
    {
      if((emm_ctrl_data_ptr->partial_esr == TRUE) &&
         (emm_ctrl_data_ptr->lte_suspend_tau_needed == TRUE))
      {
    ...
    {
      emm_ctrl_data_ptr->lte_suspend_tau_needed = FALSE;
    }
...
void emm_copy_ftai_list_to_manual_ftai_list( emm_ctrl_data_type* emm_ctrl_data_ptr)
{
  ...
  MSG_HIGH_DS_1(MM_SUB,"=EMM= Populating Manual FTAI from FTAI list of length %d",num_of_entries);
...
void mm_set_emm_manual_selection_flag(boolean value)
{
  ...
  MSG_HIGH_DS_1(MM_SUB,"=EMM= EMM Manual user PLMN selection set to %d",value);
...
{ 
  MSG_HIGH_DS_1(MM_SUB,"=MM= MM/GMM Manual user PLMN selection set to %d",value); 
...
{ 
  MSG_HIGH_DS_1(MM_SUB,"=EMM= Set MM PS DETACH STATE - %d", state); 
...
sys_secure_call_e_type mm_emm_get_secure_call_status
(
  void
)
{
  MSG_HIGH_DS_1(MM_SUB,"=EMM= Get secure call status - %d", mm_emm_secure_status_type); 
  return mm_emm_secure_status_type;
}
#endif
/*===========================================================================
...
{
  MSG_HIGH_DS_1(MM_SUB,"=EMM= Set gmm_lte_deact_isr_timer_expired - %d", expired_status); 
...
SIDE EFFECTS
  None
===========================================================================*/
dword emm_get_gmm_t3312_timer_value(void)
{
...
FUNCTION  emm_get_rau_pending_in_conn_mode

DESCRIPTION: This function return the value of mm_rau_pending_in_conn_mode
...
SIDE EFFECTS
  None
===========================================================================*/
boolean emm_get_rau_pending_in_conn_mode(void)
{
  return mm_rau_pending_in_conn_mode;
}

/*===========================================================================

FUNCTION  emm_set_rau_pending_in_conn_mode

DESCRIPTION: This function sets the value of mm_rau_pending_in_conn_mode
...
SIDE EFFECTS
  None
===========================================================================*/
void emm_set_rau_pending_in_conn_mode
(
  boolean rau_pending_in_conn_mode
)
{
  mm_rau_pending_in_conn_mode = rau_pending_in_conn_mode;
}


/*===========================================================================

FUNCTION  emm_process_gmm_drx_cn_coff_s1
...
SIDE EFFECTS
  None
===========================================================================*/
void emm_process_gmm_drx_cn_coff_s1(sys_drx_cn_coefficient_s1_e_type drx_value, cm_client_type_e_type  client_type)
{

  gmm_drx_cn_coeff_s1 = gmm_process_paging_cycle_selector(drx_value, client_type);
  MSG_HIGH_DS_1(MM_SUB, "=MM= Negotiated DRX = %d",gmm_drx_cn_coeff_s1);
...
FUNCTION  emm_set_gmm_drx_coff_client_rat

DESCRIPTION: This function is used by EMM to set "gmm_drx_coeff_client_rat" with client RAT which changed DRX CN value
...
void emm_set_gmm_drx_coff_client_rat(sys_radio_access_tech_e_type drx_rat)
{
  gmm_drx_coeff_client_rat = drx_rat;
}

/*===========================================================================

FUNCTION  emm_get_gmm_drx_coff_client_rat

DESCRIPTION: This function is used by EMM to get the client RAT stored in "gmm_drx_coeff_client_rat" 
to find whether DRX is set by QMI or internal MCS module.
...
sys_radio_access_tech_e_type emm_get_gmm_drx_coff_client_rat(void)
{
  return gmm_drx_coeff_client_rat;
}
...
{ 
  MSG_HIGH_DS_1(MM_SUB,"=EMM= GMM Manual user PLMN selection set to %d",value); 
  gmm_manual_mode_user_selection_procedure = value;
}
...
{
    MSG_HIGH_DS_1(MM_SUB,"=MM= current ue mode = %d ",mm_current_ue_mode);
...
FUNCTION  gmm_set_sync_pmm_state_during_w_to_lg_and_back_to_w_irat

DESCRIPTION: Interface function between GW and LTE to set value for gmm_sync_pmm_state_during_w_to_lg_and_back_to_w_irat
...
#ifdef FEATURE_NAS_REL12_SPEC_CHANGE
void gmm_set_sync_pmm_state_during_w_to_lg_and_back_to_w_irat(boolean value)
{
  MSG_HIGH_DS_1(MM_SUB,"Setting gmm_sync_pmm_state_during_w_to_lg_and_back_to_w_irat to %d",value);
  gmm_sync_pmm_state_during_w_to_lg_and_back_to_w_irat = value;
}
#endif
...
void mm_set_last_requested_network_selection_mode(sys_network_selection_mode_e_type net_sel_mode)
{
   mm_last_requested_network_selection_mode = net_sel_mode;
}


/*===========================================================================

FUNCTION  mm_reg_req_trans_id_get

===========================================================================*/
uint8 mm_reg_req_trans_id_get(void)
{
  return mm_reg_req_trans_id; 
}

/*===========================================================================

FUNCTION  mm_reg_req_trans_id_set

===========================================================================*/
void mm_reg_req_trans_id_set(uint8 trans_id)
{
  mm_reg_req_trans_id = trans_id; 
}

/*===========================================================================

FUNCTION  mm_reg_req_trans_id_reset

===========================================================================*/
void mm_reg_req_trans_id_reset(void)
{
  mm_reg_req_trans_id--; 
}
...
boolean mm_is_sglteplmn_in_sglte_roam(sys_plmn_id_s_type  pPlmnId)
{
  ...
  MSG_HIGH_DS_1(MM_SUB,"=MM= The val of policyman_is_sglte_allowed = %d ",val);
  if (((out_put==POLICYMAN_STATUS_SUCCESS) && (val==TRUE))&&
      ...
      (mm_checkif_rat_pri_list_has_rat(&mm_rat_pri_list_info,SYS_RAT_GSM_RADIO_ACCESS)==TRUE))
   {
     ...
     if ((out_put == POLICYMAN_STATUS_SUCCESS) && (val==TRUE))
       {
         result = TRUE;
         MSG_HIGH_DS_0(MM_SUB,"=MM= Found SGLTE PLMN in SGLTE ROAM mode "); 
       }
      else
      {
        result = FALSE;
...
boolean mm_is_non_sglte_plmn_in_sglte_home(sys_plmn_id_s_type  pPlmnId)
{
  ...
  {
    MSG_HIGH_DS_0(MM_SUB, "=MM= CM_UE_MODE_SGLTE_TEST_ONLY not blocking registration ") ;
    return result;
  }
  if(MM_IS_IN_SGLTE_STATE_4)
  {
    ...
    {
       result = TRUE;
       MSG_HIGH_DS_0(MM_SUB,"=MM= FOUND NON SGLTE PLMN in SGLTE HOME "); 
    }    
  }
  return result;
}
...
boolean mm_process_if_both_stack_has_camped_non_sglte_plmn(mm_as_id_e_type in_as_id)
{
  ...
  if (mm_current_ue_mode == SYS_UE_MODE_SGLTE_TEST_ONLY)
  {
    MSG_HIGH_DS_0(MM_SUB, "CM_UE_MODE_SGLTE_TEST_ONLY not blocking registration..") ;
    return FALSE;
  }
  if (MM_IS_IN_SGLTE_MODE && 
                                                                 ...
                                                                 ==SYS_NETWORK_SELECTION_MODE_MANUAL)))
  {
    ...
    {
      result = TRUE;
      MSG_HIGH_DS_0(MM_SUB,"=MM= In MANUAL Selection Mode Found NON-SGLTE PLMN in SGLTE HOME "); 
    }
  }
     ...
     (mm_serving_plmn_sim[mm_cs_stack_id].info.active_rat==SYS_RAT_GSM_RADIO_ACCESS) )
  {
        ...
        mm_serving_plmn_sim[mm_cs_stack_id].mm_service_state== SERVICE_AVAILABLE)
    {
      ...
      {      
        result = TRUE;
        MSG_HIGH_DS_0(MM_SUB,"=MM= In SGLTE HOME Both Stack camp on NON SGLTE PLMN "); 
     }
    }
  }
...
void mm_if_needed_abort_plmn_search_and_loop_back_to_reg( mmr_reg_req_e_type req_type)
{
     ...
     )
  {
    ...
    MSG_HIGH_DS_0(MM_SUB,"=MM= Aborting PLMN search in state4, as Service req for split in process "); 
...
void mm_abort_service_request_if_needed(void)
{
  ...
  if(emm_ctrl_data_ptr->esr_type_in_progress != NAS_EMM_CSFB_NONE )
  {
        ...
        EMM_GET_CONNECTION_STATE() == EMM_CONNECTED_STATE)
    {
      emm_abort_service_request(emm_ctrl_data_ptr, emm_ctrl_data_ptr->esr_est_conn, TRUE,
                                ...
                                emm_failure_cause);
      emm_update_lte_process_type(emm_ctrl_data_ptr);
    }
    else
    {
  ...
  else if (emm_ctrl_data_ptr->emm_sms_active && (EMM_GET_CONNECTION_STATE()!= EMM_IDLE_STATE))
  {
    if (EMM_GET_STATE() == EMM_SERVICE_REQUEST_INITIATED && mm_timer_status[TIMER_T3417] == TIMER_ACTIVE)
    {
...
#ifdef FEATURE_QSH_EVENT_METRIC
void mm_emm_qsh_client_metric_signaling_log
(
  mm_qsh_signaling_s metric_sig
)
{
  if(mm_qsh_debug_metrics.ota_signaling_ptr != NULL)
  {
    qsh_client_metric_log_done_s client_metric_log_done;
    mm_qsh_debug_metrics.ota_signaling_ptr->metric_sig = metric_sig;
    qsh_client_metric_log_done_init(&client_metric_log_done);
    client_metric_log_done.client = QSH_CLT_MM;
    client_metric_log_done.metric_id = (qsh_metric_id_t)MM_QSH_METRIC_SIGNALING;
    client_metric_log_done.metric_context_id = mm_qsh_debug_metrics.signaling_metric_context_id;
    client_metric_log_done.log_reason = QSH_CLIENT_METRIC_LOG_REASON_EVENT_INTERNAL;
    mm_qsh_debug_metrics.ota_signaling_ptr = 
                  (mm_qsh_metric_signaling_s*)qsh_client_metric_log_done(&client_metric_log_done);
  }
}
#endif
...
FUNCTION  mm_emm_notify_rplmn_update_event_qsh
...
)
{
  ...
  byte i = 0;
  int32 rplmn_id = 0;
  lte_rrc_plmn_s rrc_plmn = emm_convert_nas_plmn_id_to_rrc_plmn_id(rplmn);
  memset(diag_event,0,sizeof(diag_event)/sizeof(byte));
  while (i < LTE_RRC_MAX_MCC_DIGITS)
  {
    rplmn_id = (rplmn_id << 4) | (rrc_plmn.mcc[i++] & 0x0F);
  }
  i = 0;
  while (i < LTE_RRC_MAX_MNC_DIGITS)
  {
    rplmn_id = (rplmn_id << 4) | (rrc_plmn.mnc[i++] & 0x0F);
  }
  if(rplmn_id != mm_qsh_event_rplmn_id)
  {
    mm_qsh_event_rplmn_id = rplmn_id;
    MSG_HIGH_DS_1(MM_SUB, "=MM= DEBUG PRINT QSH_DEBUG EVENT RPLMN UPDATE 0x%x", 
                  mm_qsh_event_rplmn_id);
    emm_event_notify_qsh(MM_QSH_EVENT_RPLMN_UPDATE,diag_event);
  }
}
...
FUNCTION  emm_event_notify_qsh

===================================================*/
void emm_event_notify_qsh
(
  mm_qsh_event_e qsh_evt,
  byte *diag_event
)
{
    MSG_HIGH_DS_1(MM_SUB, "=EMM= EMM QSH event %d",qsh_evt);
...
boolean mm_is_qsh_event_valid_and_enabled
(
  mm_qsh_event_e qsh_evt
)
{
  if(qsh_evt >= MM_QSH_EVENT_MAX)
    return FALSE;

#ifdef FEATURE_DUAL_SIM
  return (mm_qsh_event_enabled_sim[mm_sub_id][(byte)qsh_evt]);
#else
  return (mm_qsh_event_enabled[(byte)qsh_evt]);
#endif

}
boolean mm_is_event_debug_any_failure_enabled
(
  mm_qsh_event_e qsh_evt
)
{
...
#ifdef FEATURE_DUAL_SIM
  return (mm_qsh_event_enabled_sim[mm_sub_id][(byte)MM_QSH_EVENT_FIELD_DEBUG_ANY_FAILURE]);
#else
  return (mm_qsh_event_enabled[(byte)MM_QSH_EVENT_FIELD_DEBUG_ANY_FAILURE]);
#endif
...
#ifdef FEATURE_QSH_EVENT_METRIC
mm_qsh_event_e mm_emm_map_nas_reg_event_to_qsh_event
(
  event_id_enum_type nas_event, byte* event_packet
)
{
  mm_qsh_event_e qsh_evt = MM_QSH_EVENT_MAX;

  switch (nas_event)
  {
    case EVENT_NAS_LAU:
    {
      if (event_packet[3] == PROC_END_REJECTED)
        qsh_evt = MM_QSH_EVENT_LAU_REJECTED;
      else if ((event_packet[3] == PROC_END_TIME_OUT) || (event_packet[3] == PROC_END_LOWER_LAYER_FAILURE))
        qsh_evt = MM_QSH_EVENT_LAU_FAILED;
    }
    break;

    case EVENT_NAS_ATTACH:
    {
      if (event_packet[3] == PROC_END_ACCEPT_PS_ONLY)
        qsh_evt = MM_QSH_EVENT_ATTACH_ACCEPTED_PS_ONLY;
      else if (event_packet[3] == PROC_END_REJECTED)
        qsh_evt = MM_QSH_EVENT_ATTACH_REJECTED;
      else if ((event_packet[3] == PROC_END_TIME_OUT) || (event_packet[3] == PROC_END_LOWER_LAYER_FAILURE))
        qsh_evt = MM_QSH_EVENT_ATTACH_FAILED;
    }
    break;
    ...
    {
      if (event_packet[3] == PROC_END_ACCEPT_PS_ONLY)
        qsh_evt = MM_QSH_EVENT_RAU_ACCEPTED_PS_ONLY;
      else if (event_packet[3] == PROC_END_REJECTED)
        qsh_evt = MM_QSH_EVENT_RAU_REJECTED;
      else if ((event_packet[3] == PROC_END_TIME_OUT) || (event_packet[3] == PROC_END_LOWER_LAYER_FAILURE))
        qsh_evt = MM_QSH_EVENT_RAU_FAILED;
    }
    break;
    ...
    case EVENT_NAS_TAU:
    {
      if(event_packet[2] == PROC_START ||
         event_packet[2] == PROC_RESEND)
      {
        mm_qsh_event_nas_message_id = TRACKING_AREA_UPADTE_REQUEST;
        qsh_evt = MM_QSH_EVENT_EMM_SPECIFIC_SIGNALING;
      }
      else if (event_packet[3] == PROC_END_ACCEPT_PS_ONLY)
        qsh_evt = MM_QSH_EVENT_TAU_ACCEPTED_PS_ONLY;
      else if (event_packet[3] == PROC_END_REJECTED)
        qsh_evt = MM_QSH_EVENT_TAU_REJECTED;
      else if ((event_packet[3] == PROC_END_TIME_OUT) || (event_packet[3] == PROC_END_LOWER_LAYER_FAILURE))
        qsh_evt = MM_QSH_EVENT_TAU_FAILED;
    }
    break;
#endif
    ...
    {
      if(event_packet[2] == PROC_START &&
         event_packet[0] == SYS_RAT_LTE_RADIO_ACCESS)
      {
        mm_qsh_event_nas_message_id = DETACH_REQUEST;
        qsh_evt = MM_QSH_EVENT_EMM_SPECIFIC_SIGNALING;
      }
      else if ((event_packet[3] == PROC_END_TIME_OUT) || (event_packet[3] == PROC_END_LOWER_LAYER_FAILURE))
        qsh_evt = MM_QSH_EVENT_MO_DETACH_FAILED;
    }
    break;
...
#ifndef TEST_FRAMEWORK 
void mm_emm_qsh_client_event_log
(
  mm_qsh_event_e evt,
  byte* event_packet
)
{
  ...
  event_notify_params.client = QSH_CLT_MM;
  event_notify_params.id = evt;
#if defined(FEATURE_DUAL_SIM) || defined(FEATURE_SGLTE)
  event_notify_params.subs_id = (sys_modem_as_id_e_type)mm_as_id;
#endif
  if(evt == MM_QSH_EVENT_EMM_SPECIFIC_SIGNALING)
  {
    event_notify_params.event_data = (void *)&mm_qsh_event_nas_message_id;
  }
  else if(evt == MM_QSH_EVENT_RPLMN_UPDATE)
  {
    event_notify_params.event_data = (void *)&mm_qsh_event_rplmn_id;
  }
  ...
  {
#if defined(FEATURE_DUAL_SIM) || defined(FEATURE_SGLTE)
    case MM_QSH_EVENT_LAU_FAILED:
      QSH_LOG(QSH_CLT_MM,QSH_CAT_EVENT,QSH_MSG_TYPE_HIGH,"AS_ID=%d, LAU_FAILED - end_reason=%d",event_packet[5],event_packet[3]);
      break;

    case MM_QSH_EVENT_LAU_REJECTED:
      QSH_LOG(QSH_CLT_MM,QSH_CAT_EVENT,QSH_MSG_TYPE_HIGH,"AS_ID=%d, LAU_REJECTED - cause=%d",event_packet[5],event_packet[4]);
      break;
    case MM_QSH_EVENT_ATTACH_ACCEPTED_PS_ONLY:
      QSH_LOG(QSH_CLT_MM,QSH_CAT_EVENT,QSH_MSG_TYPE_HIGH,"AS_ID=%d, ATTACH_ACCEPTED_PS_ONLY",event_packet[5]);
      break;

    case MM_QSH_EVENT_ATTACH_FAILED:
      QSH_LOG(QSH_CLT_MM,QSH_CAT_EVENT,QSH_MSG_TYPE_HIGH,"AS_ID=%d, ATTACH_FAILED - end_reason=%d",event_packet[5],event_packet[3]);
      break;

    case MM_QSH_EVENT_ATTACH_REJECTED:
      QSH_LOG(QSH_CLT_MM,QSH_CAT_EVENT,QSH_MSG_TYPE_HIGH,"AS_ID=%d, ATTACH_REJECTED - cause=%d",event_packet[5],event_packet[4]);
      break;

    case MM_QSH_EVENT_RAU_ACCEPTED_PS_ONLY:
      QSH_LOG(QSH_CLT_MM,QSH_CAT_EVENT,QSH_MSG_TYPE_HIGH,"AS_ID=%d, RAU_ACCEPTED_PS_ONLY",event_packet[5]);
      break;

    case MM_QSH_EVENT_RAU_FAILED:
      QSH_LOG(QSH_CLT_MM,QSH_CAT_EVENT,QSH_MSG_TYPE_HIGH,"AS_ID=%d, RAU_FAILED - end_reason=%d",event_packet[5],event_packet[3]);
      break;

    case MM_QSH_EVENT_RAU_REJECTED:
      QSH_LOG(QSH_CLT_MM,QSH_CAT_EVENT,QSH_MSG_TYPE_HIGH,"AS_ID=%d, RAU_REJECTED - cause=%d",event_packet[5],event_packet[4]);
      break;

    case MM_QSH_EVENT_TAU_ACCEPTED_PS_ONLY:
      QSH_LOG(QSH_CLT_MM,QSH_CAT_EVENT,QSH_MSG_TYPE_HIGH,"AS_ID=%d, TAU_ACCEPTED_PS_ONLY",event_packet[5]);
      break;

    case MM_QSH_EVENT_TAU_FAILED:
      QSH_LOG(QSH_CLT_MM,QSH_CAT_EVENT,QSH_MSG_TYPE_HIGH,"AS_ID=%d, TAU_FAILED - end_reason=%d",event_packet[5],event_packet[3]);
      break;

    case MM_QSH_EVENT_TAU_REJECTED:
      QSH_LOG(QSH_CLT_MM,QSH_CAT_EVENT,QSH_MSG_TYPE_HIGH,"AS_ID=%d, TAU_REJECTED - cause=%d",event_packet[5],event_packet[4]);
      break;

    case MM_QSH_EVENT_MO_DETACH_FAILED:
      QSH_LOG(QSH_CLT_MM,QSH_CAT_EVENT,QSH_MSG_TYPE_HIGH,"AS_ID=%d, DETACH_FAILED - end_reason=%d",event_packet[5],event_packet[3]);
      break;
    case MM_QSH_EVENT_EMM_SPECIFIC_SIGNALING:
      QSH_LOG(QSH_CLT_MM,QSH_CAT_EVENT,QSH_MSG_TYPE_HIGH,
                 "EMM specific procedure - message =%d",mm_qsh_event_nas_message_id);
      MSG_HIGH_DS_1(MM_SUB,"EMM specific procedure - message =%d",mm_qsh_event_nas_message_id);
      break;

    case MM_QSH_EVENT_RPLMN_UPDATE:
      QSH_LOG(QSH_CLT_MM,QSH_CAT_EVENT,QSH_MSG_TYPE_HIGH,
                 "RPLMN UPDATED - rplmn=%x",mm_qsh_event_rplmn_id);
      MSG_HIGH_DS_1(MM_SUB,"RPLMN UPDATED - rplmn=%x",mm_qsh_event_rplmn_id);
      break;
    case MM_QSH_EVENT_FIELD_DEBUG_ANY_FAILURE:
      QSH_LOG(QSH_CLT_MM,QSH_CAT_EVENT,QSH_MSG_TYPE_HIGH,"MM_QSH_EVENT_FIELD_DEBUG_ANY_FAILURE");
	  MSG_HIGH_DS_0(MM_SUB, "=MM= NAS_QSH_EVENT_FIELD_DEBUG_ANY_FAILURE has been reported");
	  break;
	case MM_QSH_EVENT_CS_PAGE_IND:
      QSH_LOG(QSH_CLT_MM,QSH_CAT_EVENT,QSH_MSG_TYPE_HIGH,"MM_QSH_EVENT_CS_PAGE_IND");
	  MSG_HIGH_DS_0(MM_SUB, "=MM= MM_QSH_EVENT_CS_PAGE_IND has been reported");
	  break;
    case MM_QSH_EVENT_MAX:
    default:
      break;    
#else
    case MM_QSH_EVENT_LAU_FAILED:
      QSH_LOG(QSH_CLT_MM,QSH_CAT_EVENT,QSH_MSG_TYPE_HIGH,"LAU_FAILED - end_reason=%d",event_packet[3]);
       break;
          
    case MM_QSH_EVENT_LAU_REJECTED:
      QSH_LOG(QSH_CLT_MM,QSH_CAT_EVENT,QSH_MSG_TYPE_HIGH,"LAU_REJECTED - cause=%d",event_packet[4]);
      break;

    case MM_QSH_EVENT_ATTACH_ACCEPTED_PS_ONLY:
      QSH_LOG(QSH_CLT_MM,QSH_CAT_EVENT,QSH_MSG_TYPE_HIGH,"ATTACH_ACCEPTED_PS_ONLY");
      break;
    
    case MM_QSH_EVENT_ATTACH_FAILED:
      QSH_LOG(QSH_CLT_MM,QSH_CAT_EVENT,QSH_MSG_TYPE_HIGH,"ATTACH_FAILED - end_reason=%d",event_packet[3]);
      break;
    
    case MM_QSH_EVENT_ATTACH_REJECTED:
      QSH_LOG(QSH_CLT_MM,QSH_CAT_EVENT,QSH_MSG_TYPE_HIGH,"ATTACH_REJECTED - cause=%d",event_packet[4]);
      break;
    
    case MM_QSH_EVENT_RAU_ACCEPTED_PS_ONLY:
      QSH_LOG(QSH_CLT_MM,QSH_CAT_EVENT,QSH_MSG_TYPE_HIGH,"RAU_ACCEPTED_PS_ONLY");
      break;
    
    case MM_QSH_EVENT_RAU_FAILED:
      QSH_LOG(QSH_CLT_MM,QSH_CAT_EVENT,QSH_MSG_TYPE_HIGH,"RAU_FAILED - end_reason=%d",event_packet[3]);
      break;
    
    case MM_QSH_EVENT_RAU_REJECTED:
      QSH_LOG(QSH_CLT_MM,QSH_CAT_EVENT,QSH_MSG_TYPE_HIGH,"RAU_REJECTED - cause=%d",event_packet[4]);
      break;
    
    case MM_QSH_EVENT_TAU_ACCEPTED_PS_ONLY:
      QSH_LOG(QSH_CLT_MM,QSH_CAT_EVENT,QSH_MSG_TYPE_HIGH,"TAU_ACCEPTED_PS_ONLY");
      break;
    
    case MM_QSH_EVENT_TAU_FAILED:
      QSH_LOG(QSH_CLT_MM,QSH_CAT_EVENT,QSH_MSG_TYPE_HIGH,"TAU_FAILED - end_reason=%d",event_packet[3]);
      break;
    
    case MM_QSH_EVENT_TAU_REJECTED:
      QSH_LOG(QSH_CLT_MM,QSH_CAT_EVENT,QSH_MSG_TYPE_HIGH,"TAU_REJECTED - cause=%d",event_packet[4]);
      break;
    
    case MM_QSH_EVENT_MO_DETACH_FAILED:
      QSH_LOG(QSH_CLT_MM,QSH_CAT_EVENT,QSH_MSG_TYPE_HIGH,"DETACH_FAILED - end_reason=%d",event_packet[3]);
      break;
...
/*===========================================================================
FUNCTION  mm_is_ms_id_changed

DESCRIPTION: 
 
This function checks if MS Identity has changed
  
DEPENDENCIES:
  None

RETURN VALUE
  TRUE: If MS ID type is neither TMSI or IMSI; or
        If MS ID type is TMSI but it doesn't match with the stored TMSI; or
        If MS ID type is IMSI but TMSI is not invalidated

  FALSE: If MS ID type is TMSI and it matches with the stored TMSI;or
         If MS ID type is IMSI and stored TMSI is invalidated

SIDE EFFECTS
  None
===========================================================================*/
boolean mm_is_ms_id_changed
(
  gmm_ie_LV_mobile_id_T  *ms_id_ptr
)
{
  byte invalid_tmsi[TMSI_SIZE];
  boolean result = FALSE;

  ASSERT(ms_id_ptr != NULL);

  memset(invalid_tmsi, 0xFF, TMSI_SIZE);

  /* If the UE is addressed by IMSI, TMSI should have been invalidated */
  if(ms_id_ptr->id_type == (byte)LTE_NAS_EMM_IMSI)
  {
    result = (memcmp((void*)mm_location_information.tmsi, (void*)invalid_tmsi, TMSI_SIZE) != 0);
  }
  /* If the UE is addressed by TMSI, stored TMSI should be identical */
  else if(ms_id_ptr->id_type == (byte)LTE_NAS_EMM_TMSI)
  {
    result = (memcmp((void*)mm_location_information.tmsi, (void*)ms_id_ptr->value, TMSI_SIZE) != 0);
  }
  else
  {
    MSG_ERROR_DS_1(MM_SUB, "=MM= Unexpected MS ID type = %d", ms_id_ptr->id_type);
    result = TRUE;
  }
  return result;
} /* end mm_is_ms_id_changed() */


#ifdef FEATURE_DUAL_SIM
/*===========================================================================
FUNCTION  mm_emm_wlan_call_in_progress

DESCRIPTION: 
 
This function checks if WLAN call is in progress
  
DEPENDENCIES:
  None

RETURN VALUE
  TRUE: If wlan call is in progress, FALSE otherwise

SIDE EFFECTS
  None
===========================================================================*/
boolean mm_emm_wlan_call_in_progress(emm_ctrl_data_type *emm_ctrl_data_ptr)
{
...
#ifdef FEATURE_WRLF_SYSTEM_SEL
/*===========================================================================
FUNCTION  mm_wrlf_clear_gmm_state

DESCRIPTION: 
 
This function clears GMM State when UE recovers in LTE after WRLF.
This function also sends connection abort to W to end WRLF
...
{
  if((mm_wcdma_rlf_state == WCDMA_RLF_STARTED) &&
     (mm_checkif_rat_pri_list_has_rat(rat_pri_list_info,
       mm_wrlf_serving_plmn_ptr->info.active_rat) == FALSE))
  {
    mm_wrlf_clear_gmm_state(TRUE,emm_ctrl_data_ptr);
  }
}
...
FUNCTION MM_SUPL_DOMAIN_SWITCH_IN_PROGESS

DESCRIPTION
  This function checks if SS from PS to CS switch is in progress(UT call failed on IMS and
   CM is performing CS switch)
DEPENDENCIES
  None
...
===========================================================================*/
#if defined(FEATURE_DUAL_SIM) && defined (FEATURE_DUAL_DATA)
boolean mm_emm_supl_domain_switch_in_progress
(
...
{
  /*
  ** On NON DDS SUB if CSFB for SS is in progress with active rat as LTE then
  ** return TRUE if there is no change in requested service domain expect for
  ** volte_only flag.
  */
  if ((mm_csfb_state == MM_CSFB_STATE_MO_CALL_ORIG) &&
      (mm_csfb_service_req_message_ptr->cmd.mmcnm_est_req.
          CNM_service_type == SUPPLEMENTARY_SERVICE_ACTIVATION) &&
      is_lte_mode() &&
      (reg_req_ptr->rat == SYS_RAT_LTE_RADIO_ACCESS) &&
      (emm_ctrl_data_ptr->reg_req_pending_ptr->req_service_domain ==
                                       reg_req_ptr->req_service_domain) &&
      (emm_ctrl_data_ptr->reg_req_pending_ptr->volte_only !=
                                       reg_req_ptr->volte_only))
  {		if(!MM_IS_NON_DDS_SUB && 
                 (mm_rat_pri_list_match(&emm_ctrl_data_ptr->reg_req_pending_ptr->rat_pri_list_info,
       &reg_req_ptr->rat_pri_list_info) == FALSE)&& 
       mm_checkif_rat_pri_list_has_rat(&reg_req_ptr->rat_pri_list_info, SYS_RAT_TDS_RADIO_ACCESS))
              {
                return FALSE;
              }
	else
...
void emm_send_rr_ps_resume_ind(emm_ctrl_data_type  *emm_ctrl_data_ptr)
{
  ...
  for (mm_other_as_id = MM_AS_ID_1; mm_other_as_id < (mm_as_id_e_type)MAX_NAS_STACKS ; mm_other_as_id++ )
  {
    if (mm_other_as_id != mm_tmp_as_id )
    {
      ...
      if((mm_conn_mode_manual_search_enabled_sim[mm_tmp_as_id] == TRUE) &&
...
FUNCTION emm_check_and_abort_sms
...
===========================================================================*/
void emm_check_and_abort_sms
(
  void
)
{
  emm_ctrl_data_type *emm_ctrl_data_ptr = emm_db_get_ctrl_data();
  if (emm_get_lte_process_type(emm_ctrl_data_ptr) == SYS_PROC_TYPE_SMS)
  {
    MSG_HIGH_DS_0(MM_SUB,"=EMM= Abort RRC connection for MT page on this sub");
    emm_rrc_abort_connection(emm_ctrl_data_ptr, FALSE,LTE_RRC_CONN_ABORT_CAUSE_NORMAL) ;
...
emm_set_stop_mode_trand_id
This API sets the transaction ID of the on-going MMR_STOP_MODE_REQ processing

==============================================================================*/

void emm_set_stop_mode_trand_id(uint32 trans_id)
{

  mmr_stop_mode_trans_id = trans_id;
...
mm_emm_send_plmn_search_status_to_mcs


DESCRIPTION
  This function send BPLMN search status to MCS 
...
void mm_emm_send_plmn_search_status_to_mcs(boolean start_status)
{
#ifdef FEATURE_DISABLED_HABANERO

   cxm_mmcp_ind_input_s a;
   cxm_mmcp_ind_output_s b;
   a.subs_id =(sys_modem_as_id_e_type) mm_as_id;

   if(start_status == TRUE)
   {
       a.procedure=CXM_PROC_ACQ_START;
   }
   else
   {
	   a.procedure=CXM_PROC_ACQ_END;

   }
   MSG_HIGH_DS_2(MM_SUB,"=EMM= Sending search status to MCS as (0=end 1= start)= %d on sub = %d",a.procedure,a.subs_id);
   cxm_update_mmcp_procedure_info(&a,&b); 
#endif
}
...
mm_emm_wlan_to_volte_ho_in_progress


DESCRIPTION
  This function check if HO for WLAN to LTE Voice call in progress 
...
boolean mm_emm_wlan_to_volte_ho_in_progress(emm_ctrl_data_type *emm_ctrl_data_ptr,boolean is_pdn_ho_type)
{
  boolean res = FALSE;
  if((is_pdn_ho_type == TRUE ) && 
  	  mm_emm_wlan_call_in_progress(emm_ctrl_data_ptr) == TRUE)
  {
    res = TRUE;
  }
  return res;
...
#ifdef FEATURE_DUAL_SIM
/*===========================================================================
FUNCTION  mm_emm_ims_sms_in_progress

DESCRIPTION: 
 
This function checks if SMS is in progress
  
DEPENDENCIES:
  None

RETURN VALUE
  TRUE: If SMS is in progress, FALSE otherwise

SIDE EFFECTS
  None
===========================================================================*/
boolean mm_emm_ims_sms_in_progress(emm_ctrl_data_type *emm_ctrl_data_ptr)
{
  if(emm_ctrl_data_ptr != NULL &&
    emm_ctrl_data_ptr->wms_sms_status.overall_sms_status == TRUE &&
...
/*===========================================================================
FUNCTION  mm_emm_radio_cap_enquiry_failure

DESCRIPTION: 
 
This function indicates CM to perform forced detach due to capability mismatch 
after MT CSFB call failure 
...
SIDE EFFECTS
  None
===========================================================================*/
void mm_emm_radio_cap_enquiry_failure(void)
{
  if ((mm_timer_status[TIMER_RADIO_CAP_ENQ] == TIMER_ACTIVE) || (mm_timer_status[TIMER_RADIO_CAP_ENQ] == TIMER_PAUSED))
  {
    MSG_HIGH_DS_0(MM_SUB, "=MM= CSFB MT failure- Forced detach due to capability mismatch ");
    gmm_handle_local_gprs_detach();
  }
