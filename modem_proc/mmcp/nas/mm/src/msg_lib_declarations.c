...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/mm/src/msg_lib_declarations.c#1 $
$Author: pwbldsvc $
    ...
    case EPS_MOBILITY_MANAGEMENT_MESSAGES:
      switch (message_id)
      {
        ...
        case EMM_AUTHENTICATION_REQUEST:
          ...
          {
            MSG_ERROR_DS_2(MM_SUB, "=NAS_MSG_LIB EMM_AUTHENTICATION_REQUEST length is %d MAX_LENGTH is %d, truncating it to the MAX length",
                           buffer_length,
                           MAX_EMM_AUTHENTICATION_REQUEST_LEN);
            buffer_length = MAX_EMM_AUTHENTICATION_REQUEST_LEN;
          }
          break;
        ...
        case DOWNLINK_NAS_TRANSPORT:
          if(buffer_length > MAX_DOWNLINK_NAS_TRANSPORT_LEN)
          {
            MSG_ERROR_DS_2(MM_SUB, "=NAS_MSG_LIB= DOWNLINK_NAS_TRANSPORT length is %d MAX_LENGTH is %d, truncating it to the MAX length",
                           buffer_length,
                           MAX_DOWNLINK_NAS_TRANSPORT_LEN);
