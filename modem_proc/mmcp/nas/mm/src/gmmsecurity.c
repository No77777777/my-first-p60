...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/mm/src/gmmsecurity.c#1 $ $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
        ...
        message, &ptmsi_reallocation_command_msg ))
  {
      ...
      gmm_qsh_ota_metric_signaling_log(GMM_PTMSI_REALLOCATION_COMMAND,CN_TO_UE_DIRECTION,
  ...
  switch (auth_error)
  {
    case AUTH_NO_ERROR:
      ...
      */
	  ...
	  mm_ssgccs_handle_outgoing_msg(GMM_AUTHENTICATION_AND_CIPHERING_RESPONSE_MSG,NO_PAYLOAD,mm_serving_plmn.info.active_rat );
    ...
    case AUTH_AUTHENTICATION_UNACCEPTABLE:
      ...
      */
	  ...
	  mm_ssgccs_handle_outgoing_msg(GMM_AUTHENTICATION_AND_CIPHERING_FAILURE_MSG,auth_error,mm_serving_plmn.info.active_rat );
...
)
{
  ...
  if (gmm_get_valid_anc_request_message(message,&incoming_request) == TRUE)
  {
    ...
    /* Spec 24.008, Rel 12 Spec CR 2589
     * An MS capable of both Iu mode and A/Gb mode shall store the received value in the 
     * Ciphering Algorithm IE in the AUTHENTICATION_AND_CIPHERING REQUEST message until 
     * entering state GMM-DEREGISTERED, in order to use it at an inter system change from 
     * Iu mode to A/Gb mode.
     */

    if (is_gsm_mode()) /* !tocheck! why mm_gsm_supported not used here*/
    {
    ...
    else
    {
      ...
      memscpy(&gmm_auth_data, sizeof (gmm_anc_request_message_T),
...
)
{
  ...
  ** ------------------------------------------------------------------ */
  ...
  auth_reset_cache_memory(FALSE,NO_TIMER_EXPIRED,RRC_PS_DOMAIN_CN_ID) ;
   ...
   if (gmm_is_dos_mitigation_required_for_ps_domain(GMM_AUTHENTICATION_AND_CIPHERING_REJECT, ILLEGAL_ME))
  {
     gmm_perform_dos_mitigation_action(GMM_AUTHENTICATION_AND_CIPHERING_REJECT, ILLEGAL_ME);
  }
  else
  {
   ...
   MSG_HIGH_DS_1(MM_SUB,"USIM GPRS Counter is %d ",mm_emm_usim_gprs_counter);
...
)
{
  ...
  {
#ifdef FEATURE_QSH_EVENT_METRIC
    gmm_qsh_ota_metric_signaling_log(GMM_IDENTITY_REQUEST,CN_TO_UE_DIRECTION,
                            incoming_request.identity_type, MM_GMM_NO_FAILURE_DEBUG);
#endif
#ifdef FEATURE_GSM_GPRS
    if (is_gsm_mode())
    {
	...
	temp_identity= mobile_identity.value[0] & 0x07;
	mm_ssgccs_handle_outgoing_msg(GMM_IDENTITY_RESPONSE_MSG,temp_identity,mm_serving_plmn.info.active_rat );
