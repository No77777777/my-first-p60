...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/mm/src/emm_rrc_handler.c#6 $

when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
03/24/15   us     If Timer 3346 is running and EMM substate is EMM_REGISTERED_ATT_TO_UPDATE 
...
byte dl_sqn_for_current_msg_sim[MAX_AS_IDS];
#define dl_sqn_for_current_msg dl_sqn_for_current_msg_sim[mm_as_id]
#else
...
boolean emm_policy_change_pending(lte_rrc_service_ind_s *rrc_service_ind)
{
  ...
  cm_mmode_state_s_type mmode_state;
 
  boolean      non_plmn_selection = TRUE;

  emm_ctrl_data_ptr = emm_db_get_ctrl_data();

  mmode_state = cm_get_mmode_state();
  /*
  ** If NAS is in dual standby mode and CM is in single standby or 
  ** NAS is in single standby mode and CM is in dual standby mode then 
  ** wait for new stop mode request from CM, treat it as policy change pending.
  */
  if (((mm_dual_standby_mode == TRUE) &&
        (mmode_state.standby_pref == SYS_MODEM_DS_PREF_SINGLE_STANDBY)) ||
       ((mm_dual_standby_mode == FALSE) &&
        (mmode_state.standby_pref != SYS_MODEM_DS_PREF_SINGLE_STANDBY)))
  {
    policy_change_reason = CM_EFFECTIVE_POLICY_CHANGE;
  }
  else
  {
     policy_change_reason = cm_effective_policy_changed((sys_modem_as_id_e_type)mm_as_id,SYS_RAT_LTE_RADIO_ACCESS, 
  ...
  if((emm_ctrl_data_ptr->reg_req_pended_indication == TRUE) &&
     ...
     reg_mode_get_higher_priority_plmn_selection_started((sys_modem_as_id_e_type)mm_sub_id) == FALSE)
  ...
  {
      MSG_HIGH_DS_1(MM_SUB, "EMM: Voice Domain Preference change detected voice_domain_pref: %d", non_acq_param.voice_domain_pref);
      if(non_acq_bit_mask_check(non_acq_param.nonAcqValue,
                            (byte)SYS_NONACQ_INFO_VOICE_DOMAIN_PREF) == TRUE)
  {
        emm_ctrl_data_ptr->reg_lte_pended_cs_domain_param_ptr->voice_domain_pref = non_acq_param.voice_domain_pref;
      }
  ...
  else if(mm_timer_status[TIMER_POLICY_CHANGE_PENDING] == TIMER_ACTIVE)
  {
    ...
    pm_chng_pending = TRUE;
           ...
           (emm_ctrl_data_ptr->reg_req_pending_ptr->network_selection_mode 
            ...
            !((sys_plmn_match(emm_ctrl_data_ptr->reg_req_pending_ptr->plmn, 
                              emm_convert_rrc_plmn_id_to_nas_plmn_id(rrc_service_ind->camped_svc_info.selected_plmn)) == TRUE)||
              (sys_eplmn_list_equivalent_plmn(emm_ctrl_data_ptr->reg_req_pending_ptr->plmn) && 
               sys_eplmn_list_equivalent_plmn(emm_convert_rrc_plmn_id_to_nas_plmn_id(rrc_service_ind->camped_svc_info.selected_plmn))))))
  {
    pm_chng_pending = FALSE;
  }
  else
...
)
{
  ...
  if(rrc_service_ind->svc_status == LTE_RRC_SVC_STATUS_AVAILABLE)
  {
    ...
    if(((mm_timer_status[TIMER_T3402] != TIMER_STOPPED) ||
       ...
       &&
       ((reg_service_report != emm_send_reg_cnf) ||
        (sd_ss_current_mcc_match_mcc_list_emerg_call_full_srv(&rrc_reported_plmn, 
...
#endif
#ifdef FEATURE_DUAL_SIM
      && (mm_timer_status[TIMER_EMM_RADIO_RETRY] == TIMER_STOPPED)
#endif
    )
  {
    ...
    switch(emm_ctrl_data_ptr->reg_req_pending_ptr->network_selection_mode)
    {
      ...
      case SYS_NETWORK_SELECTION_MODE_LIMITED_ROUTINE_ACQUISITION:
        ...
        mm_is_sglteplmn_in_sglte_roam(emm_convert_rrc_plmn_id_to_nas_plmn_id(
                   rrc_service_ind->camped_svc_info.selected_plmn));
#endif
        if((((emm_is_plmn_forbidden(srving_tai.plmn, 
                           (sys_modem_as_id_e_type)mm_sub_id) == FALSE)
           &&
...
#endif
        if((((emm_is_plmn_forbidden(tai.plmn, 
                           (sys_modem_as_id_e_type)mm_sub_id) == FALSE)
           &&
           (emm_is_tai_forbidden(tai, emm_ctrl_data_ptr)== FALSE)
#ifdef FEATURE_FEMTO_CSG
...
)
{
  ...
  if(emm_ctrl_data_ptr->reg_req_pending_ptr->network_selection_mode == 
     ...
     emm_ctrl_data_ptr->reg_req_pending_ptr->type ==  MMR_REG_REQ_FULL_RAT_MODE
    )	
  {
    emm_send_reg_camped_ind(SYS_RAT_LTE_RADIO_ACCESS,     
                                        rrc_service_ind->camped_svc_info, FALSE);
...
)
{
  ...
  switch(emm_ctrl_data_ptr->reg_req_pending_ptr->network_selection_mode)
  {
    ...
    case SYS_NETWORK_SELECTION_MODE_LIMITED:
         ...
         (mm_sim_card_mode == MMGSDI_APP_USIM))
      {
        ...
        emm_ctrl_data_ptr->last_rrc_service_ind_ptr->camped_svc_info.ims_emergency_support_enabled = TRUE;
        MSG_HIGH_DS_0(MM_SUB, "=EMM= Forcing IMS EMC support to TRUE due to full srv possible on camped PLMN");
      }
      emm_send_reg_camped_ind(SYS_RAT_LTE_RADIO_ACCESS,     
                                        rrc_service_ind->camped_svc_info, FALSE);
      break;
...
#ifdef FEATURE_LTE_REL9
  if(emm_ctrl_data_ptr->emc_backoff_timer_value > 0 &&
     emm_ctrl_data_ptr->switch_plmn == FALSE &&
  ...
  {
    //reg request pending and camped PLMN is in block PLMN list and there is available PLMN to switch 
    if((emm_bit_mask_check(emm_ctrl_data_ptr->reg_req_pending_ptr->additional_info,
                         (byte)SYS_ADDTNL_INFO_EMERG_CALL) == TRUE)&&
       (emm_ctrl_data_ptr->reg_req_pended_indication == TRUE)&&
       (emm_chk_emc_blocked_plmn_list(tai.plmn,emm_ctrl_data_ptr) == TRUE )&&
       (emm_ctrl_data_ptr->emc_shared_plmn_list.num_plmn > 0))
    {
      //RRC will send service indication with selected PLMN requested by NAS
      emm_send_rrc_switch_plmn_req(emm_ctrl_data_ptr);
    }
    else
    {
      if(emm_ctrl_data_ptr->pdn_conn_req_trans_id != EMM_INVALID_PDN_CONN_REQ_TRANS_ID &&
         emm_check_if_pdn_conn_req_for_emergency_bearer_services(emm_ctrl_data_ptr->pdn_conn_req_ptr->req) && 
         emm_ctrl_data_ptr->emc_srv_status == LTE_NAS_EMC_SRV_PENDING &&
         sys_plmn_match(tai.plmn,emm_ctrl_data_ptr->last_attempted_tai.plmn) != TRUE) 
      {
        emm_init_attach_procedure(EMM_INITIAL_UPDATE, emm_ctrl_data_ptr);
      }
      reg_service_report(MMR_CAUSE_NOT_SPECIFIED, FALSE, emm_ctrl_data_ptr);
  ...
  emm_ctrl_data_ptr->last_rrc_service_ind_ptr->camped_svc_info.
    ims_emergency_support_enabled = rrc_service_ind->camped_svc_info.ims_emergency_support_enabled;
...
)
{
    ...
    switch(emm_ctrl_data_ptr->reg_req_pending_ptr->network_selection_mode)
    {
      ...
      case SYS_NETWORK_SELECTION_MODE_LIMITED_ROUTINE_ACQUISITION:
        if(
           ...
           ||
#endif
           ((((emm_is_plmn_forbidden(current_tai.plmn, 
  ...
  */
                                                         ...
                                                         SYS_SRV_STATUS_NO_SRV)
  {
    ...
    {
      /* if there is not active bearer start Attach procedure */ 
...
#endif
    if (emm_get_rau_pending_in_conn_mode())
    {
      EMM_MOVES_TO_DEREGISTERED_STATE(EMM_DEREGISTERED_NORMAL_SERVICE);
      MSG_HIGH_DS_0(MM_SUB,
              "=EMM= Registration not performed after PSHO in W, start attach procedure");
      /* Start initial type attach procedure */
      emm_init_attach_procedure(EMM_INITIAL_UPDATE, emm_ctrl_data_ptr); 
      return;
    }
  ...
  if(ta_in_tai_list == TRUE)
  {
    ...
    {
      if(emm_ctrl_data_ptr->lte_suspend_tau_needed == TRUE)
      {
    ...
    else if( (*emm_ctrl_data_ptr->set_drx_req_pending_ptr == NAS_DRX_CFG_REQ_PENDING)&&
        (mm_timer_status[TIMER_T3402] != TIMER_ACTIVE) &&
        (mm_timer_status[TIMER_T3411] != TIMER_ACTIVE) &&
        (mm_timer_status[TIMER_PROHIBIT_DS_DRX] != TIMER_ACTIVE)) 
    {
            ...
            (mm_timer_status[TIMER_T3411] != TIMER_ACTIVE))
    {
       ...
       start_tau = TRUE;
            ...
            (mm_timer_status[TIMER_T3411] != TIMER_ACTIVE))
    {
       MSG_HIGH_DS_0(MM_SUB, "=EMM= Initing TAU - change in voice_domain_pref");
       emm_ctrl_data_ptr->tau_cause = TAU_CAUSE_OTHERS;
    ...
    {  
      ...
      {  
        if(emm_ctrl_data_ptr->tau_pending_after_msearch_abort == TRUE)
        {
		  mm_stop_timer(TIMER_LOCAL_REL_MSEARCH);
          emm_ctrl_data_ptr->skip_second_tau_during_msearch = TRUE;
		  mm_start_timer(TIMER_LOCAL_REL_MSEARCH, DEFAULT_TIMEOUT);
          MSG_HIGH_DS_0(MM_SUB, "=EMM= Initing TAU - Low layer failure and Manual search pending");
          start_tau = TRUE;
        }
        else if(ds3gpp_per_subs_is_ps_data_available((sys_modem_as_id_e_type)mm_sub_id) == FALSE)
        {
          MSG_HIGH_DS_0(MM_SUB, "=EMM= Initing TAU - Low layer failure and no data pending, report service to CM as TAU may or may not be a success hereafter");
          start_tau = TRUE;
		  if(emm_ctrl_data_ptr->reg_req_pended_indication == TRUE)
		  {
  ...
  {
	if(ta_in_tai_list==TRUE && emm_ctrl_data_ptr->cell_changed_flag == TRUE)
    {
       emm_ssgccs_cell_change_ind(rrc_service_ind->camped_svc_info.cell_identity, emm_ctrl_data_ptr->plmn_service_state->plmn);
    } 
    ...
    case SYS_NETWORK_SELECTION_MODE_LIMITED:
         ...
         (emm_is_plmn_reg_allowed(rrc_service_ind,emm_ctrl_data_ptr) == TRUE))
      {
        emm_ctrl_data_ptr->plmn_service_state->extend_srv_info = 
                                         SYS_EXTEND_SRV_INFO_FULL_SRV_POSSIBLE;
        emm_ctrl_data_ptr->last_rrc_service_ind_ptr->camped_svc_info.
                                  ims_emergency_support_enabled = TRUE;
		MSG_HIGH_DS_0(MM_SUB, "=EMM= Forcing IMS EMC support to TRUE");
      }
      emm_send_reg_camped_ind(SYS_RAT_LTE_RADIO_ACCESS,     
                                        rrc_service_ind->camped_svc_info, FALSE);             
      break;
...
)
{
  ...
  */
  ...
  else if(emm_is_tai_forbidden(current_tai,emm_ctrl_data_ptr) == TRUE)
  {
	/* Current TAI is Forbidden */
  	emm_failure_cause.cause_type = LTE_NAS_IRAT_INVALID_STATE;
  ...
  {
    /* Current TAI is NOT part of the current TAI list or radio update pending*/
     ...
     emm_ctrl_data_ptr->plmn_service_state->reg_service_domain == SYS_SRV_DOMAIN_CS_PS)
  {
    ...
    */
    if (emm_search_tai_list(&current_tai,emm_ctrl_data_ptr->emm_tai_list_ptr))
    {
      ...
      /*
      ** If reg request pending is set then this means that RLF got triggered and this service request .
      ** was for RLF recovery so send service confirmation to REG.
      */
      if(emm_ctrl_data_ptr->reg_req_pended_indication == TRUE)
      {
        emm_send_reg_camped_ind(SYS_RAT_LTE_RADIO_ACCESS, 
...
#endif
#ifdef FEATURE_FEMTO_CSG
        if (mm_is_csg_in_white_list(current_tai.plmn, &rrc_service_ind->camped_svc_info.csg_info) == FALSE)
        {
          emm_ctrl_data_ptr->plmn_service_state->service_status = 
                                                        SYS_SRV_STATUS_LIMITED;
        }
    ...
    else
    {
      ...
      /* EMM PS detach procedure will be aborted on TAI Change
         in case PS Detach is triggered due to DDS switch or service domain change
      */


    if ((*emm_ctrl_data_ptr->abort_ps_detach_ptr) 
#ifdef FEATURE_DUAL_SIM
       || (*emm_ctrl_data_ptr->dds_switch_pended_ptr)
#endif
...
)
{
  ...
  else
  {
	 ...
	 emm_ctrl_data_ptr->dual_switch_pended == TRUE)
     {
...
)
{
  ...
  */
  ...
  /* Ignore T3346 and allow registration if emergency call is pending and full service is needed to
     make emergency call */
                                ...
                                emm_ctrl_data_ptr) == TRUE))
  {
    ...
    {
        /* emergecy call pending send camp ind to tell the status to CM and PM uses to camp ind to come out of FULL RAT*/
        emm_send_reg_camped_ind(SYS_RAT_LTE_RADIO_ACCESS, 
                               rrc_service_ind->camped_svc_info, FALSE);
    }
...
)
{
  ...
  MSG_HIGH_DS_7(MM_SUB, "=EMM= RRC_SERVICE_IND - TAC %u, Cell ID %u, IMS EMC spt %d Div Duplex %d, Detach Reqd %d, blind_redir_after_csfb %d, search done %d",
                rrc_service_ind->camped_svc_info.tac,
  ...
  MSG_HIGH_DS_1(MM_SUB, "=EMM= RRC_SERVICE_IND - ELS supported flag %d", rrc_service_ind->camped_svc_info.els_supported_flag);
#endif
  if(rrc_service_ind->svc_status == LTE_RRC_SVC_STATUS_AVAILABLE)
  {
    /*reset RLF state*/
...
#ifdef FEATURE_LTE_REL9  
  if((rrc_service_ind->svc_status == LTE_RRC_SVC_STATUS_AVAILABLE)&&
     (emm_ctrl_data_ptr->emc_backoff_timer_value > 0))
  {
    //set up shared PLMN List for EMC attach retry 
    emm_set_shared_emc_supported_plmn_list(rrc_service_ind);

    if(emm_ctrl_data_ptr->switch_plmn == TRUE)
    {
      if (is_plmn_match == FALSE)
      {
        emm_ctrl_data_ptr->switch_plmn = FALSE;
      ...
      else if(emm_ctrl_data_ptr->emc_shared_plmn_list.num_plmn>0)
      {
        //RRC will send service indication with selected PLMN requested by NAS
        emm_send_rrc_switch_plmn_req(emm_ctrl_data_ptr);
        EMM_MOVES_TO_DEREGISTERED_STATE(EMM_DEREGISTERED_LIMITED_SERVICE);
      ...
      {
        //no shared PLMN list, wrap up now
		emm_failure_cause.cause_type = LTE_NAS_IRAT_INVALID_STATE;
        emm_release_attach_pdn_conn_req(emm_ctrl_data_ptr, emm_failure_cause);
        emm_ctrl_data_ptr->switch_plmn = FALSE;
      }
  ...
  MSG_HIGH_DS_2(MM_SUB, "=EMM= tau_pending_gw_attach %d, radio_cap_update_req %d",
  ...
  else if((rrc_service_ind->radio_cap_update_req == TRUE) &&
          ((emm_state == EMM_REGISTERED)|| 
           (emm_state == EMM_SERVICE_REQUEST_INITIATED)))/*allow for collision case SR and radio update  */
  {
#ifdef FEATURE_HA_MAINLINE_ONLY_ENABLE
#ifdef FEATURE_DUAL_SIM
      if(emm_ignore_rrc_ra_cap_update(emm_ctrl_data_ptr) == FALSE)
#endif
#endif
      {
      ...
      (emm_ctrl_data_ptr->reg_req_pended_indication == TRUE)))
  {
    ...
    {
      // Rest available PLMN list to 0 as REG will not use this in service found case
...
#endif 
       /*  CMCC spec:
           After the CSFB CS call is ended, the terminal shall attempt to return to LTE network through autonomous FastReturn.
	       If the LTE network is available, the terminal shall camp on the LTE network and initiate the TAU with IMSI attach.
       */
       if(reg_mode_is_usim_compliant_for_cm((sys_modem_as_id_e_type)mm_as_id) && rrc_service_ind->blind_redir_after_csfb == TRUE)
       {
         emm_ctrl_data_ptr->sg_association_to_be_established = NAS_EMM_SG_ASSOC_REQ_DUE_TO_CS_OR_CSPS_PROC;
       }
    }
...
/*    if(mm_checkif_rat_pri_list_has_rat(&mm_rat_pri_list_info, 
    ...
    mm_wrlf_clear_gmm_state(
       emm_registration_allowance_chk(rrc_service_ind,emm_ctrl_data_ptr),
       emm_ctrl_data_ptr);
#endif
  ...
  if(rrc_service_ind->camped_svc_info.cell_identity != emm_ctrl_data_ptr->last_rrc_service_ind_ptr->camped_svc_info.cell_identity)
  {
  	emm_ctrl_data_ptr->cell_changed_flag = TRUE;
  }
  else
  {
    emm_ctrl_data_ptr->cell_changed_flag = FALSE;
  }
  ...
  if(rrc_service_ind->svc_status == LTE_RRC_SVC_STATUS_AVAILABLE)
  {
       ...
       (emm_ctrl_data_ptr->client_list != NULL))
    {
	   current_tai.plmn = emm_convert_rrc_plmn_id_to_nas_plmn_id(rrc_service_ind->camped_svc_info.selected_plmn) ;
	   current_tai.tac = rrc_service_ind->camped_svc_info.tac ; 
	   if(emm_is_tai_forbidden(current_tai,emm_ctrl_data_ptr) &&
	     ((emm_ctrl_data_ptr->emc_srv_status != LTE_NAS_EMC_SRV_PENDING) &&
  ...
  else /* RRC reports that LTE service is NOT avaliable */
  {
    ...
    switch(emm_state)
    {
      case EMM_DEREGISTERED:
		...
		if(emm_ctrl_data_ptr->mt_detach_info.type == REATTACH_REQUIRED && 
			 emm_ctrl_data_ptr->rlf_state == TRUE && 
			(emm_delete_pended_message_with_msg_id(DETACH_ACCEPT, emm_ctrl_data_ptr) == TRUE))
		{
		  emm_complete_mt_detach(0);
		}		
        EMM_MOVES_TO_DEREGISTERED_STATE(EMM_DEREGISTERED_NO_CELL_AVAILABLE);
      ...
      if((emm_ctrl_data_ptr->rlf_state == FALSE)||
         (emm_ctrl_data_ptr->esr_type_in_progress != NAS_EMM_CSFB_NONE))
        {
          /* EMM will be moved to REGISTERED_NO_CELL_AVAILABLE substate in
           ...
           emm_failure_cause.cause_type = LTE_NAS_IRAT_INVALID_STATE;
          //MNK:CSFB_FAILURE
		    if(emm_ctrl_data_ptr->last_rrc_service_ind_ptr->acq_status == SYS_ACQ_STATUS_NO_RESOURCE)
		  {
			emm_failure_cause.cause.state = EMM_INVALID_STATE_NO_TRM_RESOURCES;
		  }
		  else
		  {
          ...
          if(emm_ctrl_data_ptr->esr_type_in_progress != NAS_EMM_CSFB_NONE)
          {
            ...
            {
              /* Abort RLF if encountered after sending ESR for conenction release, because afetr aborting ESR
                        ESR status will go as ended to LRRC, and in case connection release happens from NW right after
                        RLF window, LRRC will end up honoring the re-direction */
                        
              emm_abort_service_request(emm_ctrl_data_ptr, TRUE, 
                                    TRUE, local_cause,
                                    emm_failure_cause);
			}
			else
  ...
  emm_state = EMM_GET_STATE();
    if(emm_ctrl_data_ptr->aborting_conn == FALSE || emm_reg_container == NULL)
    {
    emm_ctrl_data_ptr->reg_req_pended_indication = FALSE ;
    }
...
)
{
  ...
  ASSERT(emm_ctrl_data_ptr != NULL); 
  MSG_HIGH_DS_6(MM_SUB, "=EMM= LTE_RRC_BARRING_UPDATE_IND - emergency_access_barred %u, skip_acb_for_volte_call %u ac_Barring_r12_present %d,ac barring skip for voice %d,ac barring skip for video %d,ac barring for SMS %d",
...
)
{
  ...
  dword out_nas_count = 0;
  dword nas_count = 0;

  ASSERT(rrc_nas_umts_key_ind != (lte_rrc_nas_umts_key_ind_s *)NULL) ; 
  ASSERT(emm_ctrl_data_ptr != (emm_ctrl_data_type *)NULL) ; 

  if(emm_ctrl_data_ptr->est_nas_count == 0xFFFFFFFF)
  {
    MSG_HIGH_DS_0(MM_SUB, "=EMM= emm_estimate_dl_count - est_nas_count is invalid return FALSE");
    return FALSE;
  }
  ...
  if( context_ptr != NULL )
  {
    ...
    out_nas_count = (( dword) out_count[2] << 16  ) | ( ( dword)out_count[1] << 8 ) | ( dword) out_count[0];
    nas_count = (( dword) context_ptr->nas_dl_cnt[2] << 16  ) | ( ( dword)context_ptr->nas_dl_cnt[1] << 8 ) | ( dword) context_ptr->nas_dl_cnt[0];

    if(out_nas_count <= emm_ctrl_data_ptr->est_nas_count )
    {
      MSG_HIGH_DS_2(MM_SUB, "=EMM= Debug current estimated dl count = %d is not greater than the previous estimated count %d", 
                            out_nas_count,emm_ctrl_data_ptr->est_nas_count);
      return FALSE;
    }
    else
    {
      emm_ctrl_data_ptr->est_nas_count = out_nas_count;
      emm_update_dl_nas_count_bitmap(nas_count, out_nas_count, emm_ctrl_data_ptr);
    }

    if(out_nas_count > nas_count )
    {
	  ...
	  emm_ctrl_data_ptr->last_incoming_sqn = out_count[0];
    }
  }
  ...
  if(mmgsdi_is_test_mode_logging_enabled_for_subscription((sys_modem_as_id_e_type)mm_sub_id))
  {
    MSG_HIGH_DS_6(MM_SUB, "=EMM= [emm_estimate_dl_coun] out_count[0] = %d out_count[1] = %d out_count[2] out_count[3] = %d over_flow_cnt = %d, under_flow_cnt = %d", 
                          out_count[0], out_count[1], out_count[2], out_count[3],
                          over_flow_cnt, under_flow_cnt);
  }
  return TRUE;
...
)
{
  ...
  MSG_ERROR_DS_1(MM_SUB, "=EMM= emm message %d", incloming_message->hdr.msg_id);
  ...
  switch (incloming_message->hdr.msg_id)
  {
    ...
    case TRACKING_AREA_UPDATE_ACCEPT:
      ...
      {
        emm_ssgccs_handle_incoming_msg(incloming_message);
        /* Received TAU accept message. Process the accept message */
        emm_process_tau_accept(&incloming_message->tau_accept,
                               emm_ctrl_data_ptr);
      }
      /* Process TAU Accept from NW in EMM_REGISTRED state */
      else if(emm_state == EMM_REGISTERED)
      {
        emm_process_tau_accept_in_emm_registered_state(&incloming_message->tau_accept,
                                                       emm_ctrl_data_ptr);
      }
      else
      {
    ...
    case DOWNLINK_NAS_TRANSPORT:
      ...
      {
#ifdef FEATURE_DUAL_SIM
        if (mm_emm_ims_sms_in_progress(emm_db_get_ctrl_data_per_subs(MM_OTHER_SUB(mm_as_id))) == TRUE)
        {
          MSG_HIGH_DS_0(MM_SUB, "=EMM= Ignoring DL SMS due to another SMS transaction on other SUB");
		}
		else
#endif
		{
  ...
  {
    if(incloming_message->hdr.msg_id != SECURITY_MODE_COMMAND && (incloming_message->hdr.msg_id !=TRACKING_AREA_UPDATE_ACCEPT))
    {
      emm_ssgccs_handle_incoming_msg(incloming_message);
    }
  }
...
)
{
  ...
  else if((emm_ctrl_data_ptr->estimate_dl_count_from_sqn == TRUE) && 
          (context_ptr != NULL) &&
          (is_smc_msg == FALSE) &&
          ((sec_hdr == INTEGRITY_PROTECT) || (sec_hdr == INTEGRITY_PROTECT_CIPHER)))
  {
    dword dl_nas_count_old = ((dword)prev_dl_cnt[2] << 16  ) | ((dword)prev_dl_cnt[1] << 8 ) | (dword)prev_dl_cnt[0];
    dword dl_nas_count_new = ((dword)dl_security_info.nas_count[2] << 16  ) | ((dword)dl_security_info.nas_count[1] << 8 ) | (dword)dl_security_info.nas_count[0];

    emm_update_dl_nas_count_bitmap(dl_nas_count_old, dl_nas_count_new, emm_ctrl_data_ptr);
  }
  ...
  for (message_index=0; message_index<message_counter; message_index++)
  {
    ...
    switch (protocol_descriminator)
    {
      case EPS_MOBILITY_MANAGEMENT_MESSAGES:
        if(emm_ignore_attach_accept(emm_ctrl_data_ptr) == TRUE)
        {
          attach_accept_successful = FALSE;
          emm_start_detach_on_pti_mismatch(emm_ctrl_data_ptr,
                                                 emm_ctrl_data_ptr->nas_incoming_msg_ptr[1].esm_incoming_msg);
        }
        /* Check for valid flag only for EMM messages*/
        else if(incloming_message_ptr->nas_hdr.valid)
        {
          ...
          */
          if(incloming_message_ptr->esm_incoming_msg.nas_hdr.msg_id != ACTIVATE_DEFAULT_EPS_BEARER_CONTEXT_REQUEST)
          {
            if((EMM_GET_SUBSTATE() != (emm_substate_type)EMM_WAITING_FOR_ESM_RESPONSE))
            {
			  ...
			  else if(emm_ignore_esm_information_request(emm_ctrl_data_ptr,incloming_message_ptr) == TRUE)
			  {
			    continue_processing = FALSE;
                emm_start_detach_on_pti_mismatch(emm_ctrl_data_ptr,
					                             incloming_message_ptr->esm_incoming_msg);
			  }
            }
          }
          else
  ...
  for (status_index=0; status_index<rrc_ul_data_cnf->num_trans_ids; status_index++)
  {
    ...
    if (found_message != (emm_pended_msg_list_type *)NULL)
    {
      ...
      else
      {
        ...
        emm_send_nas_ota_qevent(found_message->nas_out_message.nas_hdr.pd,
                                found_message->nas_out_message.nas_hdr.msg_id);
#endif
        if (status_index > 1)
        {
...
)
{
...
#ifdef FEATURE_DUAL_SIM
  //Do not update rat status based on rat priority list when stop confirmation is pending.
  ...
  if( emm_ctrl_data_ptr->emm_is_conn_abort_pended == TRUE)
  {
       ...
       switch(emm_ctrl_data_ptr->pending_reg_cmd_ptr->cmd.hdr.message_id)
       {
         ...
         case MMR_PLMN_SEARCH_ABORT_REQ:
           emm_send_empty_plmnsearch_cnf(
             &emm_ctrl_data_ptr->pending_reg_cmd_ptr->cmd.mmr_plmn_search_req,REG_MM_PLMN_SEARCH_ABORTED);
           reg_service_report(MMR_CAUSE_NOT_SPECIFIED,FALSE,emm_ctrl_data_ptr);
           break;
         default:
  ...
  if(emm_ctrl_data_ptr->reg_req_pended_indication != TRUE && (emm_ctrl_data_ptr->rlf_state == FALSE))
  {
...
)
{
...
#ifdef FEATURE_DUAL_SIM
  memset((void *)&mt_page_concurrency_config,0,sizeof(cm_mt_page_concurrency_config_s_type));
  mt_page_concurrency_config = cm_msim_get_mt_page_concurrency_config();
  MSG_HIGH_DS_3(MM_SUB, "=EMM= wlan_call_in_progress = %d, is_cs_mt_page_concurrency_allowed = %d, is_ims_mt_page_concurrency_allowed = %d",
                 mm_emm_wlan_call_in_progress(emm_db_get_ctrl_data_per_subs(MM_OTHER_SUB(mm_as_id))),
...
#ifdef FEATURE_DUAL_SIM
      ||((mm_emm_wlan_call_in_progress(emm_db_get_ctrl_data_per_subs(MM_OTHER_SUB(mm_as_id)))) && 
         (mt_page_concurrency_config.is_cs_mt_page_concurrency_allowed == FALSE))
	  ||(mm_emm_ims_sms_in_progress(emm_db_get_ctrl_data_per_subs(MM_OTHER_SUB(mm_as_id))))
#endif
      ))
  {
    MSG_HIGH_DS_0(MM_SUB, "=EMM= Ignoring MT CS Paging during VOLTE or WLAN call");
    return;
  }
    ...
    case LTE_RRC_UE_ID_STMSI:
      if(rrc_page_ind->cn_domain == LTE_RRC_PAGING_CN_DOMAIN_CS)
      {
        ...
        QEVENT(
          ...
          MM_QSH_EVENT_CS_CALL_STATUS_VERSION, 
          mm_sub_id, 
          "=EMM= Call type %d Call direction %d", CAFI_KPI_EVENT_IND_START, CM_CALL_DIRECTION_MT);
	  ...
	  gmm_set_gmm_is_congestion_due_to_rau(FALSE);
        ...
        if(emm_state != EMM_TRACKING_AREA_UPDATING_INITIATED)
        {
#if defined(FEATURE_LTE_REL10) || defined(FEATURE_DUAL_SIM)
          emm_ctrl_data_ptr->responding_to_page = TRUE;
#endif
          ...
          else
          {
            ...
            emm_start_nas_service_request_procedure(emm_ctrl_data_ptr, 
			 ...
			 if((*emm_ctrl_data_ptr->conn_mode_manual_search_ptr == TRUE ) &&
               ...
               emm_ctrl_data_ptr->local_rel_plus_tau_during_msearch_counter < 2)
            {
  ...
  {
    emm_ctrl_data_ptr->paging_conn_est = TRUE;
  }
#endif
...
)
{
  ...
  MSG_HIGH_DS_3(MM_SUB, "=EMM= Rcved RRC PLMN SEARCH CNF - Trans ID %d, Status %d No of PLMNs found %d",
                rrc_plmn_search_cnf_ptr->trans_id,
                rrc_plmn_search_cnf_ptr->status,
                rrc_plmn_search_cnf_ptr->found_plmn_rat_list.length);
#endif  
#ifdef FEATURE_DUAL_DATA
  ...
  if( (mm_emm_is_qrd_device == TRUE || mm_partial_search_allowed == TRUE) && 
	  ...
	  if( mm_emm_is_qrd_device == TRUE && 
		  rrc_plmn_search_cnf_ptr->status == LTE_RRC_PLMN_SEARCH_PARTIAL &&
		  rrc_plmn_search_cnf_ptr->found_plmn_rat_list.length == 0)
...
/*=============================================================================
FUNCTION    emm_process_lte_all_trm_prio_lowered_ind

DESCRIPTION
  This function processes ALL_TRM_PRIO_LOWERED_IND
...
SIDE EFFECTS
  None
=============================================================================*/
void emm_process_lte_all_trm_prio_lowered_ind
(
  emm_ctrl_data_type       *emm_ctrl_data_ptr
)
{
  ASSERT(emm_ctrl_data_ptr != (emm_ctrl_data_type *)NULL);
  MSG_HIGH_DS_0(MM_SUB, "=EMM= Received LTE_RRC_ALL_TRM_PRIO_LOWERED_IND");
#ifdef FEATURE_CRAT_MSIM_TEST_FRAMEWORK
  if (emm_check_if_els_conn_abort_possible(emm_ctrl_data_ptr) == TRUE)
  {
    ...
    emm_reset_esr_tau_pending_conn_rel(LTE_RRC_FORCE_ESR_RESET);
    return;
  } 
#endif
#ifdef FEATURE_DUAL_DATA
...
#endif
  if (mm_timer_status[TIMER_ESR_CONN_REL] == TIMER_ACTIVE)
  {
    mm_stop_timer(TIMER_ESR_CONN_REL);
    emm_perform_esr_conn_rel(emm_ctrl_data_ptr);
  }
#endif
  MSG_HIGH_DS_3(MM_SUB, "=EMM= tau_local_release %d emm_nv_local_rel_tau_dds %d proc_type %d",emm_ctrl_data_ptr->tau_local_release,emm_ctrl_data_ptr->emm_nv_local_rel_tau_dds,emm_get_lte_process_type(emm_ctrl_data_ptr));
  if( emm_ctrl_data_ptr->tau_local_release == EMM_TAU_LOCAL_REL_NONE &&
       emm_ctrl_data_ptr->emm_nv_local_rel_tau_dds == TRUE &&
       emm_get_lte_process_type(emm_ctrl_data_ptr) == SYS_PROC_TYPE_NONE &&
#if defined(FEATURE_DUAL_SIM)
       (MM_IS_NON_DDS_SUB == FALSE  || mm_sim_max_subscription == 1) &&
#endif
       EMM_GET_CONNECTION_STATE()== EMM_CONNECTED_STATE )
  {
    if( emm_is_s1_mode_only(emm_ctrl_data_ptr) == FALSE)
    {
      MSG_HIGH_DS_0(MM_SUB, "=EMM= Perform ESR+TAU on DDS SUB or single	SIM");
      emm_ctrl_data_ptr->esr_tau_pending_conn_rel = TRUE;
      emm_csfb_rsp	csfb_response;
      csfb_response.valid = FALSE;
      emm_start_nas_extended_service_request_procedure(LTE_RRC_EST_CAUSE_MO_CSFB,
                                    LTE_MO_CSFB_1xCSFB,csfb_response,NAS_EMM_3GPP_CSFB);
    }
    if (EMM_GET_STATE() != EMM_SERVICE_REQUEST_INITIATED)
    {
...
#ifdef FEATURE_DUAL_DATA
/*=============================================================================
FUNCTION    emm_perform_esr_conn_rel

DESCRIPTION
  This function checks and performs ESR for connection release
...
)
{
  ...
  {
    if(emm_get_lte_process_type(emm_ctrl_data_ptr) == SYS_PROC_TYPE_NONE &&
       EMM_GET_CONNECTION_STATE() == EMM_CONNECTED_STATE &&
...
/*=============================================================================
FUNCTION    emm_process_lte_rrc_cdrx_status_ind

DESCRIPTION
  This function processes LTE_RRC_CDRX_STATUS_IND
...
SIDE EFFECTS
  None
=============================================================================*/
void emm_process_lte_rrc_cdrx_status_ind
(
  lte_rrc_cdrx_status_ind_s  *rrc_cdrx_status_ind_ptr,
  emm_ctrl_data_type  *emm_ctrl_data_ptr
)
{
  MSG_HIGH_DS_1(MM_SUB, "=EMM= Received LTE_RRC_CDRX_STATUS_IND %d",
                                 rrc_cdrx_status_ind_ptr->status);
  if(rrc_cdrx_status_ind_ptr->status == LTE_RRC_CDRX_UNUSABLE &&
     EMM_GET_STATE() != EMM_SERVICE_REQUEST_INITIATED)
  {
    mm_start_timer(TIMER_ESR_CONN_REL,DEFAULT_TIMEOUT);
  }
  else
  {
    mm_stop_timer(TIMER_ESR_CONN_REL);
...
/*=============================================================================
FUNCTION    emm_process_lte_rrc_conn_est_timer_update_ind

DESCRIPTION
  This function processes conn_est_timer_update_ind from LRRC
...
SIDE EFFECTS
  None
=============================================================================*/
void emm_process_lte_rrc_conn_est_timer_update_ind(lte_rrc_conn_est_timer_update_ind_s* rrc_timer_update_ind,
                                                              emm_ctrl_data_type       *emm_ctrl_data_ptr)
{

  /* barring info mask will represent the type of barring intrested in */
  /* currently interested in MO SIGNALLING whcih has bit value 3 
      so   bit  value                                              |7|6|5|4|3|2|1|0|
      for MO barring only expect barring info as                   |0|0|0|0|1|0|0|0|  --> 0x08
   */
  byte             barring_info_mask = (1<<LTE_RRC_EST_CAUSE_MO_SIGNALING);
  boolean          is_barring_timer_active;
  
  ASSERT(rrc_timer_update_ind != NULL); 
  ASSERT(emm_ctrl_data_ptr != NULL);

  is_barring_timer_active = emm_check_if_barring_timer_running(&emm_ctrl_data_ptr->emm_access_barring_mo_signaling_timer);
  MSG_HIGH_DS_3(MM_SUB, "=EMM= LTE_RRC_CONN_EST_TIMER_UPDATE_IND, Barring info passed = 0x%x, Timer value = 0x%x, barring timer active = %d ",
  	                                                             rrc_timer_update_ind->call_type_bitmask,
  	                                                             rrc_timer_update_ind->barring_timer_value,
  	                                                             is_barring_timer_active);

 /* if barring timer runing then handle only for MO signal, if it not  ignore.
       LRRC should not this indication when timer is not running but in case it send NAS should ignore it.*/
  if ((is_barring_timer_active == TRUE) && (rrc_timer_update_ind->call_type_bitmask == barring_info_mask) )
  {
     /* Calling following fucntion it will take care for restarting the timer 
           and stopping / debarring in case of timer value is 0 */


	 emm_start_connection_delay_timers((byte) rrc_timer_update_ind->call_type_bitmask,
								          (dword) rrc_timer_update_ind->barring_timer_value,
								           emm_ctrl_data_ptr);

	 if(rrc_timer_update_ind->barring_timer_value == 0)
	 {
        /*Process it as timer would have expired and retried the procedure
              No need to go through the psoting timer expiry part as it is being called in same mm task context*/
...
FUNCTION emm_check_selective_rat_cap_enquiry_needed

DESCRIPTION
  This function checks whether capability enquiry is required for the
...
RETURN VALUE
  Returns the enquiry status

SIDE EFFECTS
  None
===========================================================================*/
boolean emm_check_selective_rat_cap_enquiry_needed
( 
...
)
{
  ...
  ASSERT(emm_ctrl_data_ptr != (emm_ctrl_data_type *)NULL);
   ...
   MSG_HIGH_DS_3(MM_SUB,"=MM= Skip NV = %d Initial Attach UE cap = %d Network cap = %d", emm_ctrl_data_ptr->ra_nv_cap_enquiry_skip_opt,emm_ctrl_data_ptr->ue_initial_cap_enq_ratmask, emm_ctrl_data_ptr->nw_initial_cap_enq_ratmask);
  
  if ((emm_ctrl_data_ptr->ra_nv_cap_enquiry_skip_opt == TRUE) &&
      (emm_ctrl_data_ptr->nw_initial_cap_enq_ratmask != 0))
  {
    switch(acq_sys_mode)
    {
      case SYS_SYS_MODE_GSM:
        if (((emm_ctrl_data_ptr->nw_initial_cap_enq_ratmask & (1 << LTE_RRC_CAPABILITIES_RAT_GERAN_CS)) > 0) ||
            ((emm_ctrl_data_ptr->ue_initial_cap_enq_ratmask & (1 << LTE_RRC_CAPABILITIES_RAT_GERAN_CS)) == 0))
        {
          found = TRUE;  /* Capability enquiry required */
        }
        else if (((emm_ctrl_data_ptr->nw_initial_cap_enq_ratmask & (1 << LTE_RRC_CAPABILITIES_RAT_GERAN_CS)) == 0) ||
                 ((emm_ctrl_data_ptr->ue_initial_cap_enq_ratmask & (1 << LTE_RRC_CAPABILITIES_RAT_GERAN_CS)) > 0))
        {
          found = FALSE;
        }
     ...
     case SYS_SYS_MODE_WCDMA:
        if (((emm_ctrl_data_ptr->nw_initial_cap_enq_ratmask & (1 << LTE_RRC_CAPABILITIES_RAT_UTRA_FDD)) > 0) ||
            ((emm_ctrl_data_ptr->ue_initial_cap_enq_ratmask & (1 << LTE_RRC_CAPABILITIES_RAT_UTRA_FDD)) == 0))
        {
          found = TRUE;  /* New Capability enquiry required */
        }
        else if (((emm_ctrl_data_ptr->nw_initial_cap_enq_ratmask & (1 << LTE_RRC_CAPABILITIES_RAT_UTRA_FDD)) == 0) ||
                 ((emm_ctrl_data_ptr->ue_initial_cap_enq_ratmask & (1 << LTE_RRC_CAPABILITIES_RAT_UTRA_FDD)) > 0))
        {
          found = FALSE; /* Skip enquiry for this particular RAT */
        }
        break;
      case SYS_SYS_MODE_TDS:
        found = TRUE;  /* Skip Capability Enquiry for TDS networks */
        break; 
#ifdef FEATURE_LTE
    case SYS_SYS_MODE_LTE:
      if (((emm_ctrl_data_ptr->nw_initial_cap_enq_ratmask & (1 << LTE_RRC_CAPABILITIES_RAT_EUTRA)) > 0) ||
          ((emm_ctrl_data_ptr->ue_initial_cap_enq_ratmask & (1 << LTE_RRC_CAPABILITIES_RAT_EUTRA)) == 0))
      {
        found = TRUE;  /* New Capability enquiry required */
      }
      else if (((emm_ctrl_data_ptr->nw_initial_cap_enq_ratmask & (1 << LTE_RRC_CAPABILITIES_RAT_EUTRA)) == 0) ||
               ((emm_ctrl_data_ptr->ue_initial_cap_enq_ratmask & (1 << LTE_RRC_CAPABILITIES_RAT_EUTRA)) > 0))
      {
        found = FALSE; /* Skip enquiry for this particular RAT */
      }
    ...
    case SYS_SYS_MODE_HDR:
      if (((emm_ctrl_data_ptr->nw_initial_cap_enq_ratmask & (1 << LTE_RRC_CAPABILITIES_RAT_1X)) > 0) ||
          ((emm_ctrl_data_ptr->ue_initial_cap_enq_ratmask & (1 << LTE_RRC_CAPABILITIES_RAT_1X)) == 0))
      {
        found = TRUE;  /* New Capability enquiry required */
      }
      else if (((emm_ctrl_data_ptr->nw_initial_cap_enq_ratmask & (1 << LTE_RRC_CAPABILITIES_RAT_1X)) == 0) ||
               ((emm_ctrl_data_ptr->ue_initial_cap_enq_ratmask & (1 << LTE_RRC_CAPABILITIES_RAT_1X)) > 0))
      {
        found = FALSE; /* Skip enquiry for this particular RAT */
      }
...
#endif
    default :
        MSG_HIGH_DS_1(MM_SUB,"=MM= default rat is %d", acq_sys_mode);
      break;
  }
...
FUNCTION mm_check_rat_present_in_rrc_capability_enq_rat_list

DESCRIPTION
  This function checks whether capability enquiry is done for the
  corresponding acquisition mode.
...
===========================================================================*/
boolean mm_check_rat_present_in_rrc_capability_enq_rat_list
(
  sys_sys_mode_e_type    acq_sys_mode,
  uint8                  rat_mask,
  emm_ctrl_data_type    *emm_ctrl_data_ptr
)
{
  ...
  {
    case SYS_SYS_MODE_GSM:
      if (((emm_ctrl_data_ptr->ra_cap_enquiry_nw_ratmask & (1 << LTE_RRC_CAPABILITIES_RAT_GERAN_CS)) > 0) ||
          (emm_check_selective_rat_cap_enquiry_needed(acq_sys_mode, emm_ctrl_data_ptr) == FALSE))
      {
        found = TRUE;  /* Either Capability enquiry performed in earlier round or skip enquiry */
      }
      else if (((rat_mask & (1 << LTE_RRC_CAPABILITIES_RAT_GERAN_CS)) > 0) || ((rat_mask & (1 << LTE_RRC_CAPABILITIES_RAT_GERAN_PS)) > 0))
      {
        emm_ctrl_data_ptr->ra_cap_enquiry_nw_ratmask  |= (1 << LTE_RRC_CAPABILITIES_RAT_GERAN_CS);
        found = TRUE;
      }
      break;
    case SYS_SYS_MODE_WCDMA:
      if (((emm_ctrl_data_ptr->ra_cap_enquiry_nw_ratmask & (1 << LTE_RRC_CAPABILITIES_RAT_UTRA_FDD)) > 0) ||
          (emm_check_selective_rat_cap_enquiry_needed(acq_sys_mode, emm_ctrl_data_ptr) == FALSE))
      {
        found = TRUE; /*Either Capability enquiry performed in earlier round or skip enquiry */
      }
      else if ((rat_mask & (1 << LTE_RRC_CAPABILITIES_RAT_UTRA_FDD)) > 0)
      {
        emm_ctrl_data_ptr->ra_cap_enquiry_nw_ratmask  |= (1 << LTE_RRC_CAPABILITIES_RAT_UTRA_FDD);
        found = TRUE;
      }
      break;
    case SYS_SYS_MODE_TDS:
...
#ifdef FEATURE_LTE
    case SYS_SYS_MODE_LTE:
      if (((emm_ctrl_data_ptr->ra_cap_enquiry_nw_ratmask & (1 << LTE_RRC_CAPABILITIES_RAT_EUTRA)) > 0) ||
          (emm_check_selective_rat_cap_enquiry_needed(acq_sys_mode, emm_ctrl_data_ptr) == FALSE))
      {
        found = TRUE;  /* Either Capability enquiry performed in earlier round or skip enquiry */
      }
      else if ((rat_mask & (1 << LTE_RRC_CAPABILITIES_RAT_EUTRA)) > 0)
      {
        emm_ctrl_data_ptr->ra_cap_enquiry_nw_ratmask  |= (1 << LTE_RRC_CAPABILITIES_RAT_EUTRA);
        found = TRUE;
      }
      break; 
#endif
    ...
    case SYS_SYS_MODE_NR5G:
      if ((emm_ctrl_data_ptr->ra_cap_enquiry_nw_ratmask & (1 << LTE_RRC_CAPABILITIES_RAT_NR5G)) > 0)
      {
        found = TRUE;  /* Capability enquiry performed in earlier rounds */
      }
      else if (((rat_mask & (1 << LTE_RRC_CAPABILITIES_RAT_NR5G)) > 0) || ((rat_mask & (1 << LTE_RRC_CAPABILITIES_RAT_EUTRA_NR5G)) > 0))
      {
        emm_ctrl_data_ptr->ra_cap_enquiry_nw_ratmask  |= (1 << LTE_RRC_CAPABILITIES_RAT_NR5G);
        found = TRUE;
      }
      break;
#endif
    ...
    case SYS_SYS_MODE_CDMA:
	case SYS_SYS_MODE_HDR:
      if (((emm_ctrl_data_ptr->ra_cap_enquiry_nw_ratmask & (1 << LTE_RRC_CAPABILITIES_RAT_1X)) > 0) ||
            (emm_check_selective_rat_cap_enquiry_needed(acq_sys_mode, emm_ctrl_data_ptr) == FALSE))
      {
        found = TRUE;  /* Either Capability enquiry performed in earlier round or skip enquiry */
      }
      else if ((rat_mask & (1 << LTE_RRC_CAPABILITIES_RAT_1X)) > 0)
      {
        emm_ctrl_data_ptr->ra_cap_enquiry_nw_ratmask  |= (1 << LTE_RRC_CAPABILITIES_RAT_1X);
        found = TRUE;
      }
      break;
#endif
   ...
   MSG_HIGH_DS_2(MM_SUB,"=MM= [Debug] emm_ctrl_data_ptr->ra_cap_enquiry_nw_ratmask = %d found = %d", emm_ctrl_data_ptr->ra_cap_enquiry_nw_ratmask,found);
  return found;
...
FUNCTION emm_update_initial_rat_capability_enquiry

DESCRIPTION
  This function checks whether capability enquiry is done for the
  corresponding acquisition mode during attach procedure.
...
===========================================================================*/
void emm_update_initial_rat_capability_enquiry
(
  uint8                  rat_mask,
  emm_ctrl_data_type    *emm_ctrl_data_ptr
)
{
  ...
  {
    acq_sys_mode = emm_ctrl_data_ptr->reg_req_pending_ptr->rat_pri_list_info.priority_list_info[i].acq_sys_mode;

    switch(acq_sys_mode)
    {
      case SYS_SYS_MODE_GSM:
       emm_ctrl_data_ptr->ue_initial_cap_enq_ratmask  |= (1 << LTE_RRC_CAPABILITIES_RAT_GERAN_CS);

       if ((emm_ctrl_data_ptr->nw_initial_cap_enq_ratmask & (1 << LTE_RRC_CAPABILITIES_RAT_GERAN_CS)) > 0)
       {
         found = TRUE;  /* Capability enquiry performed in earlier round */
       }
       else if (((rat_mask & (1 << LTE_RRC_CAPABILITIES_RAT_GERAN_CS)) > 0) || ((rat_mask & (1 << LTE_RRC_CAPABILITIES_RAT_GERAN_PS)) > 0))
       {
         emm_ctrl_data_ptr->nw_initial_cap_enq_ratmask  |= (1 << LTE_RRC_CAPABILITIES_RAT_GERAN_CS);
         found = TRUE;
       }
       break;
    case SYS_SYS_MODE_WCDMA:
      emm_ctrl_data_ptr->ue_initial_cap_enq_ratmask  |= (1 << LTE_RRC_CAPABILITIES_RAT_UTRA_FDD);
      ...
      {
        emm_ctrl_data_ptr->nw_initial_cap_enq_ratmask  |= (1 << LTE_RRC_CAPABILITIES_RAT_UTRA_FDD);
        found = TRUE;
      }
      break;
    case SYS_SYS_MODE_TDS:
...
#ifdef FEATURE_LTE
    case SYS_SYS_MODE_LTE:
      emm_ctrl_data_ptr->ue_initial_cap_enq_ratmask  |= (1 << LTE_RRC_CAPABILITIES_RAT_EUTRA);
      ...
      {
        emm_ctrl_data_ptr->nw_initial_cap_enq_ratmask  |= (1 << LTE_RRC_CAPABILITIES_RAT_EUTRA);
        found = TRUE;
      }
      break; 
#endif
    ...
    case SYS_SYS_MODE_CDMA:
    case SYS_SYS_MODE_HDR:
      emm_ctrl_data_ptr->ue_initial_cap_enq_ratmask  |= (1 << LTE_RRC_CAPABILITIES_RAT_1X);
      ...
      {
        emm_ctrl_data_ptr->nw_initial_cap_enq_ratmask  |= (1 << LTE_RRC_CAPABILITIES_RAT_1X);
        found = TRUE;
      }
      break;
#endif
    default :
       MSG_HIGH_DS_1(MM_SUB,"=MM= default rat is %d", emm_ctrl_data_ptr->reg_req_pending_ptr->rat_pri_list_info.priority_list_info[i].acq_sys_mode);
      break;
  }

 }

 MSG_HIGH_DS_2(MM_SUB,"=MM= Initial Attach UE cap = %d Network cap = %d", emm_ctrl_data_ptr->ue_initial_cap_enq_ratmask, emm_ctrl_data_ptr->nw_initial_cap_enq_ratmask);

}


/*=============================================================================
FUNCTION    emm_process_rrc_radio_cap_enquiry_ind

DESCRIPTION
  This function processes LTE_RRC_RADIO_CAP_ENQUIRY_IND
...
SIDE EFFECTS
  None
=============================================================================*/
void emm_process_rrc_radio_cap_enquiry_ind
(
  lte_rrc_radio_cap_enquiry_ind_s      *rrc_radio_cap_enquiry_ind,
  emm_ctrl_data_type                   *emm_ctrl_data_ptr
)
{
  ...
  uint32 i;
  boolean is_rat_found = TRUE;
  uint8 current_ratmask;
  emm_state_type emm_state = EMM_GET_STATE();
  ASSERT(emm_ctrl_data_ptr != (emm_ctrl_data_type *)NULL) ; 
  ASSERT(rrc_radio_cap_enquiry_ind != NULL);

  current_ratmask = rrc_radio_cap_enquiry_ind->rat_mask;
  
  MSG_HIGH_DS_2(MM_SUB, "=EMM= Received LTE_RRC_RADIO_CAP_ENQUIRY_IND stored mask = 0x%x  new_mask = 0x%x", emm_ctrl_data_ptr->ra_cap_enquiry_nw_ratmask, current_ratmask );

  if ((emm_state == EMM_REGISTERED_INITIATED) &&
      (emm_ctrl_data_ptr->ra_nv_cap_enquiry_skip_opt == TRUE) &&
      (emm_ctrl_data_ptr->radio_nv_cap_enquiry_timer != 0))
  {
    emm_update_initial_rat_capability_enquiry( current_ratmask, emm_ctrl_data_ptr );
    return;
  }
  
  if (mm_timer_status[TIMER_RADIO_CAP_ENQ] == TIMER_ACTIVE)
  {
    for (i = 0; i < emm_ctrl_data_ptr->reg_req_pending_ptr->rat_pri_list_info.num_items; i++)
    {
      if (mm_check_rat_present_in_rrc_capability_enq_rat_list(emm_ctrl_data_ptr->reg_req_pending_ptr->rat_pri_list_info.priority_list_info[i].acq_sys_mode, 
                                                              current_ratmask, emm_ctrl_data_ptr ) == FALSE)
      {
        is_rat_found = FALSE;
      }
    }
  
    MSG_HIGH_DS_2(MM_SUB, "=EMM= Final stored RRC Mask = 0x%x is_rat_found = %d",emm_ctrl_data_ptr->ra_cap_enquiry_nw_ratmask, is_rat_found);
    
    if (is_rat_found)
    {
      MSG_HIGH_DS_1(MM_SUB, "=EMM= LTE_RRC_RADIO_CAP_ENQUIRY_IND - All RATs capabilities enquired by RRC Mask = 0x%x",emm_ctrl_data_ptr->ra_cap_enquiry_nw_ratmask);
      mm_stop_timer(TIMER_RADIO_CAP_ENQ);
      emm_ctrl_data_ptr->ra_cap_enquiry_nw_ratmask  = 0x00; /* Reset RAT mask to zero upon stopping timer  */
    }
  }
...
)
{
  ...
  switch (rrc_msgr_cmd->rrc_msgr_cmd.msg_hdr.id)
  {
   ...
   case LTE_RRC_NAS_LTE_KEY_IND:
    ...
    case LTE_RRC_ACTIVE_EPS_BEARER_UPDATE_IND:
      ...
      if (MM_IS_NON_DDS_SUB &&
	      emm_ctrl_data_ptr->local_rel_tau_after_csfb_tau == TRUE &&
	  	  emm_ctrl_data_ptr->second_tau_after_csfb_needed == EMM_CSFB_CON_REL_PEND_TAU)
      {
        emm_ctrl_data_ptr->second_tau_after_csfb_needed = EMM_CSFB_CON_REL_PEND_DRB;
	  }
	  MSG_HIGH_DS_1(MM_SUB, "=EMM= second_tau_after_csfb_needed : %d ",emm_ctrl_data_ptr->second_tau_after_csfb_needed);
#endif	  
      emm_update_lte_process_type(emm_ctrl_data_ptr);
      break ;
    case LTE_RRC_CONN_EST_TIMER_UPDATE_IND:
	  emm_process_lte_rrc_conn_est_timer_update_ind(&rrc_msgr_cmd->rrc_msgr_cmd.rrc_conn_est_timer_update_ind,emm_ctrl_data_ptr);		
      break ;
	...
	emm_send_rrc_manual_search_indication(FALSE);
#endif
      }
      break;
    ...
    case LTE_RRC_ALL_TRM_PRIO_LOWERED_IND:
      emm_process_lte_all_trm_prio_lowered_ind(emm_ctrl_data_ptr);
      break;
#ifdef FEATURE_DUAL_DATA
    case LTE_RRC_CDRX_STATUS_IND:
      emm_process_lte_rrc_cdrx_status_ind(
           &rrc_msgr_cmd->rrc_msgr_cmd.rrc_cdrx_status_ind,
           emm_ctrl_data_ptr);
      break;
#endif
    case LTE_RRC_RADIO_CAP_ENQUIRY_IND:
      emm_process_rrc_radio_cap_enquiry_ind(&rrc_msgr_cmd->rrc_msgr_cmd.rrc_radio_cap_enquiry_ind,emm_ctrl_data_ptr);
      break;
    default:
...
FUNCTION  emm_ignore_rrc_ra_cap_update

DESCRIPTION: This function is called by EMM when it receives service ind from 
 RRC for Radio Capability Update 

DEPENDENCIES
  None

RETURN VALUE
  TRUE if RRC TAU request needs to be ignored, else FALSE

SIDE EFFECTS
  None
===========================================================================*/
boolean emm_ignore_rrc_ra_cap_update(emm_ctrl_data_type  *emm_ctrl_data_ptr)
{
  /* Ignore RRC radio Cap Update Indication only if:
     a. It is on nDDS sub
     b. If radio cap update is not pending for other causes (ex: CM service req, or.
        RRC indocation due to PLMN change)
   
     Keep radio cap update as none for now. It will be marked as pending when timer
     expires or when new service request is received- so that IE will be set correctly
     in TAU request.
  */
  if (MM_IS_NON_DDS_SUB &&
      (emm_ctrl_data_ptr->radio_cap_update == EMM_RADIO_CAP_UPDATE_NONE))
  {
    MSG_HIGH_DS_0(MM_SUB, "=EMM= Ignore radio_cap_update on nDDS sub");
    mm_start_timer(TIMER_DELAY_TAU, DEFAULT_TIMEOUT);
    return TRUE;
  }
...
FUNCTION    emm_check_possible_tau_for_rrc_ra_cap_update

DESCRIPTION
  This function starts TAU procedure that was delayed due to Radio Capability
  Update from RRC
...
RETURN VALUE
  None
===========================================================================*/
void emm_check_possible_tau_for_rrc_ra_cap_update
(
  emm_ctrl_data_type      *emm_ctrl_data_ptr
)
{
  ASSERT(emm_ctrl_data_ptr != NULL);
  MSG_HIGH_DS_3(MM_SUB, "=EMM= TIMER_DELAY_TAU expired - EMM state %d, substate %d, RRC status %d", 
  ...
  if((emm_ctrl_data_ptr->rrc_active == TRUE) &&
     ...
     (mm_timer_status[TIMER_T3411]  != TIMER_ACTIVE))
  {
    /* Mark radio cap update as pending when timer expires - so that IE will be set correctly
       in TAU request. */
    emm_ctrl_data_ptr->radio_cap_update = EMM_RADIO_CAP_UPDATE_PENDING;
    emm_ctrl_data_ptr->ra_cap_enquiry_nw_ratmask  = 0x00; /* Reset RAT mask to zero upon new capability change */
...
FUNCTION  emm_check_rrc_ra_cap_update_pending

DESCRIPTION
  This function checks if TAU was delayed for Radio Capability
  Update from RRC and updates REG to set the additional info field in REG request
  correctly
...
RETURN VALUE
  TRUE if REG needs to update the additional info field
  FALSE otherwise
===========================================================================*/
boolean emm_check_rrc_ra_cap_update_pending
(
...
)
{
  ...
  if((mm_subs_capability_mask_sim[as_id] & SYS_SYS_MODE_MASK_LTE) &&
     (mm_timer_status_sim[as_id][TIMER_DELAY_TAU] == TIMER_ACTIVE))
  {  
    mm_stop_timer_per_subs(TIMER_DELAY_TAU, as_id);
    result = TRUE;
  }
