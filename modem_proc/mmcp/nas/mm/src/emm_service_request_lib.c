...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/mm/src/emm_service_request_lib.c#3 $
...
void emm_process_esr_complete(emm_ctrl_data_type* emm_ctrl_data_ptr)
{
  ...
  if(emm_ctrl_data_ptr->esr_type_in_progress == NAS_EMM_3GPP_CSFB)
  {
	...
	mm_stop_timer(TIMER_T3417_EXT_MT);
	   ...
	   emm_ctrl_data_ptr->second_tau_after_csfb_needed ==EMM_CSFB_CON_REL_PEND_NONE)
	{
	  ...
	  MSG_HIGH_DS_2(MM_SUB, "=EMM= local_rel_tau_after_csfb_tau %d second_tau_after_csfb_needed %d",emm_ctrl_data_ptr->local_rel_tau_after_csfb_tau,emm_ctrl_data_ptr->second_tau_after_csfb_needed);
	}
#endif	
  }
...
)
{
  ...
  */
  ...
  MSG_HIGH_DS_4(MM_SUB, "=EMM= efs_loc_rel_tau_after_nas_sms %d emm_nv_force_esr_nondds %d \
                               emm_nv_force_tau_nondds %d emm_sms_active %d",emm_ctrl_data_ptr->efs_loc_rel_tau_after_nas_sms,\
                               emm_ctrl_data_ptr->emm_nv_force_esr_nondds,emm_ctrl_data_ptr->emm_nv_force_tau_nondds,emm_ctrl_data_ptr->emm_sms_active);
  if(emm_ctrl_data_ptr->efs_loc_rel_tau_after_nas_sms == TRUE &&
  	 ...
  	 emm_ctrl_data_ptr->emm_sms_active == TRUE)
  {
	...
	MSG_HIGH_DS_0(MM_SUB,"Set flag to trigger TAU after SMS end");
  }
#endif  
...
#ifdef FEATURE_DISABLED_HABANERO
  QEVENT(QSH_CLT_MM, MM_QSH_EVENT_EMM_PROC, MM_QSH_EVENT_EMM_PROC_VERSION, mm_sub_id,
			"=EMM= MM_QSH_EVENT_EMM_PROC status %d, message type %d", CAFI_KPI_EVENT_IND_START, SERV_REQ);
#endif
#endif
...
#ifdef FEATURE_DISABLED_HABANERO
    QEVENT(QSH_CLT_MM, MM_QSH_EVENT_EMM_PROC, MM_QSH_EVENT_EMM_PROC_VERSION, mm_sub_id,
			"=EMM= MM_QSH_EVENT_EMM_PROC status %d, message type %d", CAFI_KPI_EVENT_IND_FAIL, SERV_REQ);
#endif
#endif
...
)
{
...
#ifdef FEATURE_DISABLED_HABANERO 
  QEVENT(QSH_CLT_MM, MM_QSH_EVENT_EMM_PROC, MM_QSH_EVENT_EMM_PROC_VERSION, mm_sub_id,
			"=EMM= MM_QSH_EVENT_EMM_PROC status %d, message type %d", CAFI_KPI_EVENT_IND_SUCCESS, SERV_REQ);
#endif
#endif
...
#ifdef FEATURE_DUAL_SIM
  //if IMS is registered or UE is CSPS registered(SMS) then start the 300ms timer
  if((emm_ctrl_data_ptr->responding_to_page == TRUE) &&
  ...
  {
    mm_start_timer(EMM_MT_PAGE_EXT_PRI_TIMER,DEFAULT_TIMEOUT);
  }
#endif
    ...
    case EMM_CONNECTED_STATE:
      ...
      else
      {
           ...
           (emm_ctrl_data_ptr->esr_type_in_progress == NAS_EMM_CSFB_NONE))
        {
          ...
          if(emm_ctrl_data_ptr->emm_service_attempt_counter == NAS_THRESHOLD_BACKOFF_SERVICE_ATTEMPTS)
          {
            mm_start_timer(TIMER_T3425,emm_ctrl_data_ptr->service_req_back_off_timer_value);
          }
  ...
  if( EMM_GET_STATE() == EMM_SERVICE_REQUEST_INITIATED ) 
  {
    ...
    switch (cause)
    {
      ...
      case TRANSMISSION_LTE_RRC_UL_DATA_CNF_FAILURE_CTRL_NOT_CONN:
        ...
        {
		  // If TA is non-updated TA, then connection will get aborted after service indication, and ESR will be aborted from there.
          // Simply re-transmit the message here.		  
          mm_stop_timer(TIMER_T3417_EXT);
		  mm_start_timer(TIMER_T3417_EXT,EMM_T3417_EXT_TIMER_DURATION);
		  
		  emm_send_message(&message_ptr->nas_out_message, 
  ...
  *buffer_esr = FALSE;
  ...
  { 
    ...
    case EMM_REGISTERED_INITIATED:
      if((emm_ctrl_data_ptr->csfb_perform_reg == TRUE) &&
         (svc_type == LTE_MO_CSFB_1xCSFB) &&
         (esr_type == NAS_EMM_3GPP_CSFB))
      {
        result = TRUE;
...
)
{
  ...
  */
                                ...
                                emm_ctrl_data_ptr))
  {
    ...
    if((esr_type == NAS_EMM_1xCSFB) || (esr_type == NAS_EMM_3GPP_CSFB))
    {
      emm_ctrl_data_ptr->esr_est_conn = TRUE;
      MSG_HIGH_DS_0(MM_SUB,"=EMM= ESR is sent in new connection");
    }
  }
  ...
  if(esr_type == NAS_EMM_3GPP_CSFB)
  {
    ...
    if(emm_ctrl_data_ptr->enable_3417_ext_mt_timer == TRUE &&
	    svc_type == LTE_MT_CSFB_1XCSFB)
    {
	  MSG_HIGH_DS_0(MM_SUB, "=EMM= enable_3417_ext_mt_timer enabled");	
      mm_stop_timer(TIMER_T3417_EXT_MT) ;
      mm_start_timer(TIMER_T3417_EXT_MT,EMM_T3417_EXT_MT_TIMER_DURATION);
	}
	else
    {
      mm_stop_timer(TIMER_T3417_EXT) ;
...
#endif 
     (mm_timer_status[TIMER_FRLTE] != TIMER_ACTIVE))
     {
         
      	MSG_HIGH_DS_0(MM_SUB, "=EMM= Started TIMER_FRLTE on MO/MT call");
      	mm_start_timer(TIMER_FRLTE,DEFAULT_TIMEOUT);
   ...
   if(
     ...
     (cm_get_operator_info(SYS_MODEM_AS_ID_1) == OPERATOR_CMCC) &&
#endif 
     (mm_timer_status[TIMER_MM_PAGE_RETRY] != TIMER_ACTIVE)&&
        (mm_csfb_state == MM_CSFB_STATE_MT_CALL_ORIG))
     {
       mm_start_timer(TIMER_MM_PAGE_RETRY,DEFAULT_TIMEOUT);
     }
  }
  else if(esr_type == NAS_EMM_1xCSFB)
  {
  ...
  if (emm_ctrl_data_ptr->esr_tau_pending_conn_rel == TRUE)
  {
    emm_send_rrc_force_esr_ind(LTE_RRC_FORCE_ESR_STARTED,emm_ctrl_data_ptr);
  }
#endif
  /* Register connection release client for service request procedure.
     The client needs to be removed upon service reject reception, procedure 
...
void emm_send_esr_if_buffered(emm_ctrl_data_type* emm_ctrl_data_ptr)
{
  ...
  if(emm_ctrl_data_ptr->esr_type_in_progress == NAS_EMM_3GPP_CSFB_BUFFERED)
  {
    emm_ctrl_data_ptr->esr_type_in_progress = NAS_EMM_3GPP_CSFB;
	if(emm_ctrl_data_ptr->enable_3417_ext_mt_timer == TRUE  &&
	   emm_ctrl_data_ptr->pended_esr_msg->emm_outgoing_msg.ext_service_req.service_type == LTE_MT_CSFB_1XCSFB)
	{
		mm_start_timer(TIMER_T3417_EXT_MT,EMM_T3417_EXT_MT_TIMER_DURATION);
	}
	else
...
)          
{
  ...
  if(emm_ctrl_data_ptr->esr_type_in_progress != NAS_EMM_CSFB_NONE)
  {
    MSG_HIGH_DS_0(MM_SUB, "=EMM= CSFB call is aborted, cleared the TIMER and the ESR type and pended ESR");
  }
  if(emm_ctrl_data_ptr->pended_esr_msg != NULL)
  {
    /* Free the memory block */
  	...
  	emm_ctrl_data_ptr->esr_type_in_progress == NAS_EMM_1X_CSFB_BUFFERED)
  {
    ...
    MSG_HIGH_DS_0(MM_SUB, "=EMM= 3GPP2 voice call aborted");
    emm_abort_service_request(emm_ctrl_data_ptr, FALSE, TRUE,
                                    NAS_SIG_CONN_RELEASED, emm_failure_cause);
...
)          
{
 ...
 if(((emm_reject_cs_service_notification(emm_ctrl_data_ptr) == TRUE)&&
  	(emm_is_cs_page_allow_during_ims_resource_reserve(emm_ctrl_data_ptr)== FALSE))
	&& ((cs_service_notification->lcs_ind_exists == FALSE) || (cs_service_notification->lcs_ind != MT_LR)))
  {
