...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/mm/src/mmplmnsel.c#1 $ $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...
void mm_send_service_req( mm_cmd_type *message , sys_radio_access_tech_e_type rat )
{
...
#ifdef FEATURE_DUAL_SIM
   mm_update_rat_status(&message->cmd.mmr_reg_req.rat_pri_list_info,SYS_RAT_ACTIVATION_STATUS_NONE);
#endif
#ifdef FEATURE_WCDMA
   if ((rat == SYS_RAT_UMTS_RADIO_ACCESS) || (rat == SYS_RAT_TDS_RADIO_ACCESS))
   {
      ...
      */
      mm_last_requested_network_selection_mode  =
        message->cmd.mmr_reg_req.network_selection_mode;
     ...
     /* While sending the service request, update the variable so that, we can check later if voice domain pref gets changed with another service request*/
	  ...
	  mm_ssgccs_handle_icoming_msg(MM_MODE_PREF_IND,mm_serving_plmn.info.active_rat,CS_DOMAIN_PROCEDURE,message);
     ...
     */
...
#else
#ifdef FEATURE_NAS_ECALL
        && ((mmcoord_pending_reg_message.cmd.mmr_reg_req.is_emergency_call== FALSE)
              ||  !(mm_ecall_mode_type == MMR_ECALL_MODE_ECALL_ONLY))
#endif
               ...
               )
      {
        rrc_cmd->cmd.rrc_service_req.service_req_cause = RRC_MO_CS_ECALL;
              ...
              (emm_bit_mask_check(message->cmd.mmr_reg_req.additional_info,
                                    (byte)SYS_ADDTNL_INFO_EMERG_CALL) == TRUE) 
#else
              (mmcoord_pending_reg_message.cmd.mmr_reg_req.is_emergency_call == TRUE)
#endif
             && (mm_ecall_mode_type == MMR_ECALL_MODE_ECALL_ONLY)
              )
      {
	  ...
	  /* store some parameters before sending, will be needed later for comparison */ 
      if(( mm_last_sent_serv_req_params_ptr == NULL ) && 
         ((mm_last_sent_serv_req_params_ptr=(mm_service_req_params_T *)modem_mem_calloc(1, sizeof( mm_service_req_params_T ),MODEM_MEM_CLIENT_NAS)) 
                  == NULL))
      {
        MSG_FATAL_DS(MM_SUB, "ERROR: Unable to allocate memory", 0,0,0 );
      }
      memscpy(&(mm_last_sent_serv_req_params_ptr->plmn), sizeof(sys_plmn_id_s_type), &(message->cmd.mmr_reg_req.plmn), sizeof(sys_plmn_id_s_type));
      mm_last_sent_serv_req_params_ptr->csg_id = message->cmd.mmr_reg_req.csg_id;
      mm_last_sent_serv_req_params_ptr->grat_scan_done = message->cmd.mmr_reg_req.grat_scan_done;
      mm_last_sent_serv_req_params_ptr->service_req_cause = rrc_cmd->cmd.rrc_service_req.service_req_cause;
      mm_last_sent_serv_req_params_ptr->scan_scope = message->cmd.mmr_reg_req.scan_scope;
      mm_last_sent_serv_req_params_ptr->trm_timeout_value = message->cmd.mmr_reg_req.trm_timeout;
      mm_last_sent_serv_req_params_ptr->is_ehplmn_to_be_camped = rrc_cmd->cmd.rrc_service_req.is_ehplmn_to_be_camped;
   ...
   if (rat == SYS_RAT_GSM_RADIO_ACCESS)
   {
      ...
      using nw_sel_mode_reset_ind
      */
      mm_last_requested_network_selection_mode  = message->cmd.mmr_reg_req.network_selection_mode;
      ...
      if((mm_follow_on_request)
        ...
        ||(
#ifdef FEATURE_DUAL_SIM
           (mm_last_proc_type_sent == SYS_PROC_TYPE_CS_CALL) &&
#endif
      ...
      /* New cause sent to GRR which asks GRR to camp on unacceptable cell in FULL RAT MODE*/
       else if((message->cmd.mmr_reg_req.message_header.message_set == MS_MM_REG) && 
               (message->cmd.mmr_reg_req.message_header.message_id == MMR_REG_REQ) &&
               ((message->cmd.mmr_reg_req.type == MMR_REG_REQ_FULL_RAT_MODE)
#ifdef FEATURE_LTE
                ||
       ...
       ))
      {
        ...
        rr_plmn_select_req.service_req_cause = RR_MM_FULL_RAT_ACQ;
      }
      else
      {
      ...
      rr_plmn_select_req.scan_scope = message->cmd.mmr_reg_req.scan_scope;

      PUT_IMH_LEN( sizeof( rr_plmn_select_req_T ) - sizeof(IMH_T),
                           &rr_plmn_select_req.message_header );
      MSG_HIGH_DS_2(MM_SUB ,"=MM= MM sent RR_PLMN_SELECT_REQ with service request cause %d scan_scope %d", rr_plmn_select_req.service_req_cause,rr_plmn_select_req.scan_scope);
            ...
            ** ------------------------------------------------------------- */
            if (msg_p->cmd.rrc_service_cnf.service_status == MM_AS_NO_SERVICE)
            {
                 ...
                 (mm_held_cnm_service_req.establishment_cause == (int)MM_RR_EMERGENCY_CALL))
              {
                ...
                MSG_HIGH_DS_0(MM_SUB,"=MM= MM-REJ No service on current cell");
                      ...
                      (mm_serving_plmn.ms_op_mode == SYS_SRV_DOMAIN_CS_ONLY))
                  {
                         ...
                         (mm_held_cnm_service_req.establishment_cause == (int)MM_RR_EMERGENCY_CALL))
                     {
                        ...
                        else if (mm_serving_plmn.info.service_status == SYS_SRV_STATUS_NO_SRV)
                        {
                          ...
                          MSG_HIGH_DS_0(MM_SUB,"=MM= MM-REJ Wrong access class for Emergency call");
                  ...
                  */
                  ...
                  if (((sys_plmn_match(mmcoord_pending_reg_message.cmd.mmr_reg_req.plmn, curr_plmn)) ||
                      (sys_eplmn_list_equivalent_plmn(mmcoord_pending_reg_message.cmd.mmr_reg_req.plmn) && 
                       sys_eplmn_list_equivalent_plmn(mm_serving_plmn.info.plmn))) &&
#ifdef FEATURE_DUAL_SIM 
                      (!((reg_sim_per_subs_int_voice_forbidden(curr_plmn,(sys_modem_as_id_e_type)mm_sub_id )) || 
                         (reg_sim_per_subs_domestic_voice_forbidden(curr_plmn,(sys_modem_as_id_e_type)mm_sub_id ))))
#else
                      (!((reg_sim_int_voice_forbidden( curr_plmn )) || 
                         (reg_sim_domestic_voice_forbidden( curr_plmn ))))
#endif  
                  )
                  {
                     mm_event = MM_STD_NORMAL_SERVICE;
                       ...
                       LAI_SIZE ) == 0 ) )
         {
            ...
            else if (mm_possible_imsi_attach)
            {
               ...
               if (mm_reg_waiting_for_reg_cnf)
               {
                  ...
                  mm_is_camped_on_updated_lai = TRUE;
                  mmcoord_mmr_reg_cnf.service_state = mm_serving_plmn.info;
                  mm_send_mmr_reg_cnf();
                  mm_reg_waiting_for_reg_cnf = FALSE;
                  mm_ready_to_send_reg_cnf   = FALSE;
