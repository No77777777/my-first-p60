...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/mm/src/emm_esm_handler.c#5 $
...
)
{
  ...
  if(release_cause == CONN_REL_LTE_RRC_CONN_REL_PDCP_REL_CAUSED_BY_LTA)
  {
	sig_conn_rel_ind.emm_cause.local_cause_value = EMM_CON_REL_PDCP_OUT_OF_SYNC;
  }
  else
  {
	sig_conn_rel_ind.emm_cause.local_cause_value = EMM_CON_REL_LOCAL_VALUE_NONE;
  }
#endif
  ...
  else
  {
    ...
    event.emm_out_msg = (dword)NAS_ESM_SIG_CONNECTION_RELEASED_IND;
    nas_event_report_payload(EVENT_LTE_EMM_OUTGOING_MSG,sizeof(event),
                      (void*)&event,mm_message_as_id);
  }
...
)
{
  ...
  {
    MSG_HIGH_DS_0(MM_SUB,"=EMM= Processing pended stop_mode DUAL_SWITCH after attach complete failed"); 
      ...
      case CONN_CNF_LTE_RRC_CONN_EST_FAILURE_TAI_CHANGED:
          ...
          )
        {
          ...
          if(emm_ctrl_data_ptr->esr_type_in_progress == NAS_EMM_3GPP_CSFB_BUFFERED)
          {
            ...
            MSG_HIGH_DS_0(MM_SUB, "=EMM= Attach complete failed, ESR is buffered, move to GW");
  ...
  switch(cause)
  {
    ...
    default:
      ...
      else if(cause == CONN_CNF_LTE_RRC_CONN_EST_FAILURE_RF_UNAVAILABLE)
      {
		...
		emm_ctrl_data_ptr->is_syncup_tau_for_local_bear_deact_needed = TRUE;
      }
      ...
      //don't notify ESM layer if LRRC sent cell deprior_is_active
      if (emm_failure_cause.deprior_started == FALSE) 
      {
        emm_build_and_send_failure_ind(local_cause, 
                                     LTE_NAS_NO_FAILURE,
                                     trans_id,
...
{  
...
#endif
#ifdef FEATURE_DUAL_SIM
  boolean   is_pdn_type_ho  = FALSE;
#endif
  emm_pended_msg_list_type * pended_msg = NULL;
  ...
  {
    pended_msg = emm_find_pended_message(emm_data_request->emm_transaction_id, 
                             emm_ctrl_data_ptr);
    MSG_HIGH_DS_4(MM_SUB, "=EMM= Can not process ESM DATA REQ - Dyn mem 0x%x, rrc active %d, reg_req_pending_ptr 0x%x \
		                  pended_msg 0x%x",
	...
	if(pended_msg != NULL)
	{
      emm_delete_pended_message(pended_msg,emm_ctrl_data_ptr);
	}
...
/* Reject further PDN Connectivity requests for a normal PDN when deactivation of
  ...
  if((emm_ctrl_data_ptr->non_emc_bearer_deact_in_progress == TRUE)
     ...
     && (emm_check_if_pdn_conn_req_for_emergency_bearer_services(emm_data_request->esm_out_msg.pdn_connectivity_req.req) == FALSE))
  {
  ...
  if ( (emm_data_request->esm_out_msg.nas_hdr.msg_id == PDN_CONNECTIVITY_REQUEST) &&
       emm_check_if_pdn_conn_req_for_emergency_bearer_services(emm_data_request->esm_out_msg.pdn_connectivity_req.req) )
  {
  ...
  if((emm_data_request->esm_out_msg.nas_hdr.msg_id == PDN_CONNECTIVITY_REQUEST)
     &&
     (emm_data_request->esm_out_msg.pdn_connectivity_req.req == ESM_HANDOVER))
  {
    is_pdn_type_ho = TRUE;    
  }
#endif
    ...
    case EMM_REGISTERED:
...
#ifdef FEATURE_LTE_REL12
             &&(mm_timer_status[TIMER_T3425] == TIMER_STOPPED))
#endif
...
#ifdef FEATURE_DUAL_SIM
             (mm_emm_wlan_to_volte_ho_in_progress(emm_ctrl_data_ptr,is_pdn_type_ho) == TRUE)||
#endif
      ...
      else
      {
...
#ifdef FEATURE_DUAL_SIM
        else if(EMM_GET_SUBSTATE() == (emm_substate_type)EMM_REGISTERED_PLMN_SEARCH && 
        ...
        { 
          ...
          /** Note he PDN connection is pending so on PLMN search and TAU 
          completion send indication to ESM to retry PDN procedures immediately */
    ...
    case EMM_SERVICE_REQUEST_INITIATED:
      ...
      */
              ...
              (emm_ctrl_data_ptr->nas_secure_exchange == TRUE))
      {
		 ...
		 {
			// if CSFB has been started, we will either move to CS RATs or 3gpp2 rat, reject 
			// any ESM procedure at this point
			emm_failure_cause.cause_type = LTE_NAS_IRAT_INVALID_STATE;
        	emm_build_and_send_failure_ind(LTE_NAS_SERVICE_REQ_FAILED_NO_THROTTLE, 
                                       LTE_NAS_NO_FAILURE,
                                       emm_data_request->emm_transaction_id,
        ...
        */
		...
		if(((emm_substate == EMM_REGISTERED_NO_CELL_AVAILABLE) ||
            ...
            (((emm_ctrl_data_ptr->rrc_deact_req.deact_cnf_pending == TRUE) ||
              (emm_ctrl_data_ptr->is_syncup_tau_for_local_bear_deact_needed == TRUE)) &&
              (emm_substate == EMM_REGISTERED_NORMAL_SERVICE)) ||
            (emm_eps_bearer_status_cmd_ptr->esm_local_deact == TRUE)) &&
...
void emm_process_esm_1xcsfb_esr_call_req(emm_1xCSFB_esr_call_req_T* msg_ptr, emm_ctrl_data_type* emm_ctrl_data_ptr)
{
  ...
  {
#ifdef FEATURE_LTE_REL12
    emm_csfb_perform_registration(NAS_EMM_1xCSFB);
#endif
...
void emm_process_esm_1xcsfb_esr_call_abort_req(emm_1xCSFB_esr_call_abort_req_T* msg_ptr,emm_ctrl_data_type* emm_ctrl_data_ptr)
{
  ...
  if( emm_ctrl_data_ptr->esr_est_conn == TRUE)
  {
    emm_rrc_abort_connection(emm_ctrl_data_ptr, FALSE,
                                       LTE_RRC_CONN_ABORT_CAUSE_NORMAL);
  }
...
#ifdef FEATURE_DUAL_SIM
/*===========================================================================
FUNCTION  EMM_PROCESS_ESM_PS_CALL_STATUS_IND
DESCRIPTION
  This function processes ps call status ind  from ESM
...
SIDE EFFECTS
  None
===========================================================================*/
void  emm_process_esm_ps_call_status_cmd
(
  emm_ps_call_status_ind_cmd_T  *cmd_ptr, 
  emm_ctrl_data_type        *emm_ctrl_data_ptr
)
{
  ASSERT(cmd_ptr != NULL);
  ASSERT(emm_ctrl_data_ptr != NULL);
  MSG_HIGH_DS_2(MM_SUB, "=EMM= received NAS_EMM_PS_CALL_STATUS_CMD change from %d to %d",
             emm_ctrl_data_ptr->ps_call_in_progress, cmd_ptr->ps_call_in_progress);
  if(emm_ctrl_data_ptr->esm_proc_retry_ind == TRUE && emm_ctrl_data_ptr->ps_call_in_progress == TRUE && 
  	 cmd_ptr->ps_call_in_progress == FALSE)
  {
    emm_ctrl_data_ptr->esm_proc_retry_ind = FALSE;
  }
  emm_ctrl_data_ptr->ps_call_in_progress = cmd_ptr->ps_call_in_progress;
  ...
  switch(cmd_ptr->emc_srv_status)
  {
    case LTE_NAS_EMC_SRV_NONE:
	  ...
	  if(emm_ctrl_data_ptr->plmn_service_state->service_status != SYS_SRV_STATUS_NO_SRV
	  		&& emm_ctrl_data_ptr->emm_state == EMM_DEREGISTERED_INITIATED
      ...
      if(emm_ctrl_data_ptr->emc_srv_status == LTE_NAS_EMC_SRV_EXIST)
      {
			...
			is_lte_mode())
        {
          /* Set emc srv status now to enable emc attached field in srv cnf/ind
             to REG 
			 1) if MT detach not going as in this case EMM will report limited regional service anyway after
			 detach completion 
			 2) if active RAT is not LTE*/
  ...
  switch (emm_msgr_cmd->emm_msgr_cmd.msg_hdr.id)
  {
      ...
      else 
      {
        if((emm_ctrl_data_ptr->rrc_active == TRUE) &&
           ((EMM_GET_STATE() == EMM_REGISTERED) && 
			...
			(EMM_GET_CONNECTION_STATE() == EMM_RELEASING_RRC_CONNECTION_STATE &&
               MM_IS_NON_DDS_SUB &&
              EMM_GET_SUBSTATE() == EMM_REGISTERED_UPDATE_NEEDED &&
             ...
             (emm_check_mplmn_in_progress(emm_ctrl_data_ptr) == FALSE)))
        {
             ...
             (emm_srv_req_ds_allowance_chk(emm_ctrl_data_ptr) == TRUE))
          {
            if(emm_ctrl_data_ptr->emm_connection_state != EMM_CONNECTED_STATE)
            {
              ...
              nas_event_report_payload(EVENT_LTE_EMM_INCOMING_MSG,sizeof(event),
                                           (void*)&event,mm_message_as_id);
            }
            else
            {
                 ...
                 (EMM_GET_SUBSTATE() == EMM_REGISTERED_LIMITED_SERVICE)))
        {
          MSG_HIGH_DS_0(MM_SUB, "=EMM= SRV REQ fro EMC DRB in LSM - Locally detach and EMC attach");
                    ...
                    if ((EMM_GET_SUBSTATE() == EMM_REGISTERED_LIMITED_SERVICE &&
                  emm_ctrl_data_ptr->reg_req_pended_indication == TRUE &&
                  emm_ctrl_data_ptr->reg_init_rrc_trans_id != 0 ))
          {
            MSG_HIGH_DS_0(MM_SUB, "=EMM= Limited camping pending, reject DRB REQ");
            emm_failure_cause.cause_type = LTE_NAS_IRAT_INVALID_STATE;
			emm_failure_cause.cause.state = EMM_INVALID_STATE_TEMP;
			emm_build_and_send_failure_ind(LTE_NAS_SERVICE_REQ_FAILED_NO_THROTTLE, 
                                         LTE_NAS_NO_FAILURE,
        ...
        else if((emm_ctrl_data_ptr->rrc_active == TRUE) &&
               ((EMM_GET_STATE() == EMM_REGISTERED) && 
               ((((EMM_GET_SUBSTATE() == EMM_REGISTERED_NORMAL_SERVICE) ||
               (EMM_GET_SUBSTATE() == EMM_REGISTERED_ATTEMPTING_TO_UPDATE_MM))&&
			  (emm_ctrl_data_ptr->emm_update_status == EMM_STATUS_NOT_UPDATED)) ||
    ...
    case NAS_EMM_DETACH_CMD:
      ...
      {   
        ...
        nas_event_report_payload(EVENT_LTE_EMM_INCOMING_MSG,
...
)
{
  ...
  else
  {
    ...
    event.emm_out_msg = (dword)NAS_ESM_ACTIVE_EPS_IND;
                                     ...
                                     emm_failure_type            emm_failure_cause)
{
  ...
  else
  {
    ...
    event.emm_out_msg = (dword)NAS_ESM_FAILURE_IND;
    nas_event_report_payload(EVENT_LTE_EMM_OUTGOING_MSG,
                         sizeof(event), (void*)&event,mm_message_as_id);
  }
...
void emm_build_and_send_detach_ind (lte_nas_local_cause_enum_T local_cause)
{
  ...
  else
  {
    ...
    event.emm_out_msg = (dword)NAS_ESM_DETACH_IND;
    nas_event_report_payload(EVENT_LTE_EMM_OUTGOING_MSG,
                         sizeof(event), (void*)&event,mm_message_as_id);
  }
}
...
)
{
  ...
  else
  {
    ...
    event.emm_out_msg = (dword)NAS_ESM_EPS_BEARER_STATUS_IND;
    nas_event_report_payload(EVENT_LTE_EMM_OUTGOING_MSG,
                    sizeof(event), (void*)&event,mm_message_as_id);
  }
...
#ifdef FEATURE_NAS_REL12_SPEC_CHANGE
  /* This function get called only when ISR is active */
  if ((emm_get_mm_spec_rel_ver() >= NV_NAS_COMPLIANCE_REL12) &&
				  (EMM_GET_CONNECTION_STATE()== EMM_CONNECTED_STATE ))
  {
    isr_status_ind.is_ps_ho = TRUE;
  }
#endif
...
void emm_send_unblock_all_apns_ind(void)
{
  ...
  else
  {
    ...
    event.emm_out_msg = (dword)NAS_ESM_UNBLOCK_ALL_APNS_IND;
    nas_event_report_payload(EVENT_LTE_EMM_OUTGOING_MSG,
                      sizeof(event), (void*)&event,mm_message_as_id);
  }
...
FUNCTION    EMM_BUILD_AND_SEND_PROC_RETRY_IND

DESCRIPTION
  This function sends an Procedure Retry Indication to ESM

===========================================================================*/
void emm_build_and_send_proc_retry_ind()
{
  esm_proc_retry_ind_T proc_retry_ind;
  log_lte_nas_emm_outgoing_msg_event_type event;

  memset((void *)&proc_retry_ind,0,sizeof(esm_proc_retry_ind_T));
  msgr_init_hdr((msgr_hdr_s *)&proc_retry_ind.msg_hdr, MSGR_NAS_EMM, NAS_ESM_PROC_RETRY_IND);
  emm_ctrl_data_ptr->esm_proc_retry_ind = FALSE;
  
  if(emm_msgr_send((msgr_hdr_s *)&proc_retry_ind, sizeof(proc_retry_ind)) != E_SUCCESS)
  {
    MSG_FATAL_DS(MM_SUB, "=EMM= emm_msgr_send failed!", 0,0,0 );
  } 
  else
  {
    MSG_HIGH_DS_0(MM_SUB, "=EMM= Sent NAS_ESM_PROC_RETRY_IND");
    event.emm_out_msg = (dword)NAS_ESM_PROC_RETRY_IND;
    nas_event_report_payload(EVENT_LTE_EMM_OUTGOING_MSG,
                         sizeof(event), (void*)&event,mm_message_as_id);
  }
}
