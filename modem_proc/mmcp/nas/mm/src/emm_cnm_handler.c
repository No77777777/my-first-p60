...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/mm/src/emm_cnm_handler.c#1 $
...
)
{
    ...
    )
  {
	...
	else if (mm_per_subs_is_mt_page_in_progress((sys_modem_as_id_e_type)MM_OTHER_SUB(mm_as_id)) == TRUE)
	{
    ...
    else
    {
	  ...
	  MSG_HIGH_DS_0(MM_SUB,"Set flag in connected mode to trigger TAU after SMS end");
#if defined  FEATURE_DUAL_SIM || defined FEATURE_1XSRLTE
      emm_update_lte_process_type(emm_ctrl_data_ptr);
#endif
    }
      ...
      ** to GSM later. This is to remeber if PS data needs to be resumed later */
      ...
      if(mm_csfb_get_state() == MM_CSFB_STATE_MO_CALL_ORIG)
      {
       ...
       if(emm_ctrl_data_ptr->esr_type_in_progress == NAS_EMM_3GPP_CSFB)
       {
         ...
         emm_abort_service_request(emm_ctrl_data_ptr, emm_ctrl_data_ptr->esr_est_conn, 
                     FALSE, LTE_NAS_SERVICE_REQ_FAILED_NO_THROTTLE,
                                                      emm_failure_cause);
       } 
...
)
{
  ...
  is_pended_cp_data = is_cp_data((byte *)&dl_nas_transport_ptr->nas_msg_container.msg_payload[0]);
...
#endif
/*if REL request has been received from CNM,then send release confirmation to cnm to close the transaction and then process this MT SMS*/
#ifdef FEATURE_DUAL_SIM
  if (MMCNM_IS_MODE_DUAL_STANDBY && is_pended_cp_data == TRUE)
  {
