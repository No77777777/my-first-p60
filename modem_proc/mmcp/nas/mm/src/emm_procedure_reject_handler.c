...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/mm/src/emm_procedure_reject_handler.c#1 $
    ...
    case LTE_NAS_CONGESTION: 
      if(emm_ctrl_data_ptr->emm_3gpp_spec_ver >= LTE_3GPP_REL10) 
      {
        ...
        if(msg_id == TRACKING_AREA_REJECT)
        {
          ...
          {
            /* Remember that tau is rejected with T3346 when emergency call was pending */
        ...
        /* R10 Spec 24.301 CR #0905. A UE in CS/PS mode 1 or mode 2
                                    ...
                                    (sys_modem_as_id_e_type)mm_sub_id) == TRUE)))
          {
            ...
            if((emm_ctrl_data_ptr->manual_user_sel == TRUE) ||
               (emm_ctrl_data_ptr->reg_req_pending_ptr->type == MMR_REG_REQ_BETTER_SYSTEM_RESELECTION))
            {
              MSG_HIGH_DS_0(MM_SUB," =MM= Blocking service indication to REG during manual selection or BSR");
            }
            else
			{
 ...
 /* 24.301 5.6.1.5 24.301 */
...
#endif
          if(emm_ctrl_data_ptr->reg_req_pending_ptr->type == MMR_REG_REQ_BETTER_SYSTEM_RESELECTION)
          {
            MSG_HIGH_DS_0(MM_SUB," =MM= Blocking service indication to REG during BSR");
          }
      ...
      case LTE_NAS_IMPLICITLY_DETACHED :
                                 ...
                                 (sys_modem_as_id_e_type)mm_sub_id) == TRUE)))
        {
          ...
          else if(emm_ctrl_data_ptr->reg_req_pending_ptr->type == MMR_REG_REQ_BETTER_SYSTEM_RESELECTION)
          {
            MSG_HIGH_DS_0(MM_SUB," =MM= Blocking service indication to REG during BSR"); 
          }
          else
      ...
      default:
           ...
           (emm_is_registration_for_emc_pdn_conn(emm_ctrl_data_ptr)== TRUE))
        {
         ...
         /* Try getting full service on GW if NW rejected TAU with cause #22 and T3346 is started
  ...
  switch(incoming_message->hdr.msg_id)
  {
    case SERVICE_REJECT:
      ...
      if ((gmm_get_map_rej_cause7_to_rej_cause14()) 
       && incoming_message->mt_emm_status.emm_cause == LTE_NAS_EPS_SERVICES_NOT_ALLOWED 
       && (!gmm_is_camped_on_hplmn()))
      {
        incoming_message->mt_emm_status.emm_cause = LTE_NAS_EPS_SERVICES_NOT_ALLOWED_IN_PLMN;
      }
