...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/mm/src/emm_reg_handler.c#2 $
...
)
{
  ...
  {
    emm_send_empty_plmnsearch_cnf(reg_plmnsearch_ptr,REG_MM_PLMN_SEARCH_ABORTED);
    return;
  }
#ifdef FEATURE_DUAL_SIM
  else if((mm_check_plmn_search_in_progress_on_other_subs() == TRUE) &&
          (reg_plmnsearch_ptr->network_selection_mode != SYS_NETWORK_SELECTION_MODE_MANUAL))
  {
    ...
    return;
          ...
          (reg_plmnsearch_ptr->service_search == SYS_SERVICE_SEARCH_NONE))
  {
    MSG_HIGH_DS_0(MM_SUB, "=EMM= Rejecting PLMN search req as MMS is in progress on Non-DDS sub");
    emm_send_empty_plmnsearch_cnf(reg_plmnsearch_ptr,REG_MM_PLMN_SEARCH_ABORTED_DUE_TO_NO_LOCK);
    return;
  }
  else if ((reg_plmnsearch_ptr->network_selection_mode == SYS_NETWORK_SELECTION_MODE_MANUAL) &&
           (reg_plmnsearch_ptr->service_search == SYS_SERVICE_SEARCH_NONE)&&
  ...
  {
	MSG_HIGH_DS_0(MM_SUB, "=EMM= Rejecting PLMN search req as Timer for MT PS page is running or SR procedure is ongoing");
	emm_send_empty_plmnsearch_cnf(reg_plmnsearch_ptr,REG_MM_PLMN_SEARCH_ABORTED_DUE_TO_NO_LOCK);
	return;
  }
#endif
               ...
               &reg_plmnsearch_ptr->rat_pri_list_info) == TRUE)
  {
  ...
  else if(emm_ctrl_data_ptr->aborting_conn == TRUE && 
           emm_ctrl_data_ptr->reg_req_pended_indication == TRUE &&
          !mm_checkif_rat_pri_list_has_rat(&emm_ctrl_data_ptr->reg_req_pending_ptr->rat_pri_list_info,SYS_RAT_TDS_RADIO_ACCESS) &&
           (mm_tds_rat_status == SYS_RAT_ACTIVATION_STATUS_ACTIVATED || mm_tds_rat_status == SYS_RAT_ACTIVATION_STATUS_DEACTIVATION_IN_PROGRESS))
  {
  MSG_HIGH_DS_0(MM_SUB, "=EMM= Rejecting PLMN search req as service request is pended for DDS switch");
  emm_send_empty_plmnsearch_cnf(reg_plmnsearch_ptr,REG_MM_PLMN_SEARCH_ABORTED_DUE_TO_NO_LOCK);
  return;
  }
#endif
  ...
  if( (*emm_ctrl_data_ptr->conn_mode_manual_search_ptr == TRUE )&&
      (reg_plmnsearch_ptr->network_selection_mode == SYS_NETWORK_SELECTION_MODE_MANUAL)&&
      (reg_plmnsearch_ptr->srch_priority != SYS_SRCH_PRIORITY_LOW))
  {
...
)
{
  ...
  MSG_HIGH_DS_1(MM_SUB, "=EMM= Processing Pended Reg Req reg_req_pend = %d", 
                                   emm_ctrl_data_ptr->reg_req_pended_indication);

  mm_reg_req_trans_id_set(emm_ctrl_data_ptr->reg_req_pending_ptr->trans_id);
  ...
  if (emm_ctrl_data_ptr->reg_req_pended_indication == TRUE)
  {
    ...
    emm_procedure_message_mem_alloc(emm_ctrl_data_ptr);
     ...
     MSG_HIGH_DS_2 (MM_SUB, "=EMM= Pended Req  emm_state = %d, net_sel_mode = %d", 
                                     EMM_GET_STATE(), reg_req_ptr->network_selection_mode);
...
)
{
  ...
  */
  ...
  mmr_reg_cnf_ptr->trans_id = mm_reg_req_trans_id_get();
...
/*===========================================================================
FUNCTION    emm_check_on_demand_call_end

DESCRIPTION
  Function sets the flag to trigger ESR + TAU after on demand call end.
...
SIDE EFFECTS
  None
===========================================================================*/
void emm_check_on_demand_call_end
(
  mmr_reg_req_s_type        *reg_req_ptr, 
  emm_ctrl_data_type *emm_ctrl_data_ptr
)
{
  if(MM_IS_NON_DDS_SUB &&
     emm_ctrl_data_ptr->emm_nv_force_esr_nondds == TRUE &&
     emm_is_s1_mode_only(emm_ctrl_data_ptr) == FALSE &&
     reg_req_ptr->volte_only == TRUE &&
  ...
  {
    MSG_HIGH_DS_0(MM_SUB, "Set flag to start ESR + TAU on demand call end on NONDDS SUB");
...
)
{
  ...
  mm_reg_req_trans_id_set(reg_req_ptr->trans_id);

  MSG_HIGH_DS_4(MM_SUB, "=EMM= MMR_REG_REQ - Srv Domain %d, NW Sel Mode %d, Type %d, Voice domain pref= %d",
  ...
  {
    MSG_HIGH_DS_0(MM_SUB, "=EMM= Resetting lte_suspend_tau_needed flag");
...
#if defined (FEATURE_DUAL_SIM) && defined (FEATURE_TDSCDMA)
  if(mm_check_ignore_mmr_reg_req(&reg_req_ptr->rat_pri_list_info) == TRUE)
  {
   return;
  }
#endif
  ...
  mm_serving_plmn.ms_op_mode = 
       mm_get_effective_req_srv_domain(reg_req_ptr->req_service_domain,
               reg_req_ptr->volte_only,FALSE);
#else 
  ...
  if(reg_req_ptr->type == MMR_REG_REQ_SRV_MODE_UPDATE)
  {
	if((emm_ctrl_data_ptr->reg_req_pending_ptr->network_selection_mode == SYS_NETWORK_SELECTION_MODE_LIMITED) &&
	   (emm_bit_mask_check(emm_ctrl_data_ptr->reg_req_pending_ptr->additional_info,
                         (byte)SYS_ADDTNL_INFO_EMERG_CALL) == TRUE) &&
	   (emm_ctrl_data_ptr->last_rrc_service_ind_ptr->svc_status == LTE_RRC_SVC_STATUS_AVAILABLE))
	{
		mm_stop_timer(TIMER_T3411);
    ...
    reg_service_report = (emm_reg_service_report_type)emm_send_reg_cnf;
	// During inter-RAT from LtoX we might de-allocate EMM pointers in global.
	// If we are processing pended service mode update, re-allocate before processing service request.
	emm_procedure_message_mem_alloc(emm_ctrl_data_ptr);
...
#if defined(FEATURE_DUAL_SIM) && defined (FEATURE_DUAL_DATA)
    emm_check_on_demand_call_end(reg_req_ptr,emm_ctrl_data_ptr);
#endif
    ...
    else
    {
      ...
      if(((conn_state == EMM_CONNECTED_STATE)||
        ...
        ( (emm_ctrl_data_ptr->last_rrc_service_ind_ptr->svc_status == 
         ...
         ((emm_bit_mask_check(reg_req_ptr->additional_info,(byte)SYS_ADDTNL_INFO_EMERG_CALL) == 
           emm_bit_mask_check(emm_ctrl_data_ptr->reg_req_pending_ptr->additional_info,
                                        (byte)SYS_ADDTNL_INFO_EMERG_CALL)) &&
          (emm_bit_mask_check(reg_req_ptr->additional_info,(byte)SYS_ADDTNL_INFO_1XCSFB_CALL_END) == 
           emm_bit_mask_check(emm_ctrl_data_ptr->reg_req_pending_ptr->additional_info,
                                        (byte)SYS_ADDTNL_INFO_1XCSFB_CALL_END))) )										  
...
#if defined(FEATURE_DUAL_SIM) && defined (FEATURE_DUAL_DATA)
      mm_csfb_rej_pending_call(reg_cmd_ptr);
#endif
      if((emm_bit_mask_check(reg_req_ptr->additional_info,
                         (byte)SYS_ADDTNL_INFO_EMERG_CALL) == TRUE) &&
         (reg_req_ptr->network_selection_mode == SYS_NETWORK_SELECTION_MODE_LIMITED) &&
         (emm_bit_mask_check(emm_ctrl_data_ptr->reg_req_pending_ptr->additional_info,
      ...
      switch(reg_req_ptr->network_selection_mode)
      {
        ...
        case SYS_NETWORK_SELECTION_MODE_MANUAL_CAMP_ONLY:
          if(reg_req_ptr->type == MMR_REG_REQ_USER_SELECTION)
          {
           ...
           if(!(
             ...
             (reg_sim_per_subs_domestic_voice_forbidden( reg_req_ptr->plmn,(sys_modem_as_id_e_type)mm_sub_id ))
#else
             (reg_sim_int_voice_forbidden( reg_req_ptr->plmn )) ||
             (reg_sim_domestic_voice_forbidden( reg_req_ptr->plmn ))
#endif
             ))
             {
        ...
        case SYS_NETWORK_SELECTION_MODE_MANUAL_CSG:
          ...
          if (((reg_req_ptr->network_selection_mode == 
                                          SYS_NETWORK_SELECTION_MODE_AUTOMATIC) &&
                (reg_req_ptr->type == MMR_REG_REQ_ACQ_HIGH_PRIOIRTY_SEL_W_O_EPLMN) &&
               (sys_eplmn_list_validate(FALSE) == TRUE)) ||
              (sys_eplmn_list_validate(TRUE) == TRUE))
          {
         ...
         (conn_state == EMM_WAITING_FOR_RRC_CONFIRMATION_STATE))
      {
        if((emm_ctrl_data_ptr->rlf_state == TRUE) &&
           ((radio_cap_update_flag == FALSE) &&
            !((sys_plmn_id_is_undefined(emm_ctrl_data_ptr->rlf_plmn) == FALSE) &&
              (memcmp((void *)&reg_req_ptr->plmn,(void *)&emm_ctrl_data_ptr->rlf_plmn,sizeof(sys_plmn_id_s_type)) != 0) &&
        ...
        else
        {
...
#ifdef FEATURE_LTE_REL9
           || ((emm_bit_mask_check(reg_req_ptr->additional_info,
                         (byte)SYS_ADDTNL_INFO_EMERG_CALL) == TRUE) &&
               reg_req_ptr->network_selection_mode == SYS_NETWORK_SELECTION_MODE_LIMITED)
#endif
		  ...
		  )
          {
            ...
            if (service_change_pending || 
                ((sys_plmn_id_is_undefined(emm_ctrl_data_ptr->rlf_plmn) == FALSE) &&
                 (sys_plmn_id_is_undefined(reg_req_ptr->plmn) == FALSE) &&
                 (reg_req_ptr->network_selection_mode != SYS_NETWORK_SELECTION_MODE_LIMITED) &&
                 (memcmp((void *)&reg_req_ptr->plmn,(void *)&emm_ctrl_data_ptr->rlf_plmn,sizeof(sys_plmn_id_s_type)) != 0)))
            {
               ...
               reg_req_ptr->network_selection_mode == SYS_NETWORK_SELECTION_MODE_LIMITED)
            {
              ...
              emm_abort_attach_procedure();
              emm_ctrl_data_ptr->ignore_attach_pti_mismatch = TRUE;
            }
#ifdef FEATURE_DUAL_SIM
            if(EMM_GET_STATE() == EMM_SERVICE_REQUEST_INITIATED &&
               emm_ctrl_data_ptr->reg_req_pended_indication == TRUE && 
               emm_ctrl_data_ptr->ps_call_in_progress == TRUE &&
               (emm_find_pended_message_with_msg_id(PDN_CONNECTIVITY_REQUEST,emm_ctrl_data_ptr) != NULL))
             {
                MSG_HIGH_DS_0(MM_SUB, "=EMM= PDN CONN REQ is pending - ESM Procedure Retry Needed after TAU");
                emm_ctrl_data_ptr->esm_proc_retry_ind = TRUE;
              }
#endif
            emm_rrc_abort_connection(emm_ctrl_data_ptr, 
                                     FALSE,
...
void emm_local_detach_without_deact(void)
{
    mm_local_cs_detach();
...
)
{
  ...
  MSG_HIGH_DS_2(MM_SUB, "=EMM= Rcved MMR_STOP_MODE_REQ w/ reason %u in EMM state %u",
                ...
                emm_state);
  emm_set_stop_mode_trand_id(reg_cmd_ptr->cmd.mmr_stop_mode_req.trans_id);
  ...
  if (emm_ctrl_data_ptr->dual_switch_pended == TRUE &&
  	  reg_cmd_ptr->cmd.mmr_stop_mode_req.stop_mode_reason == SYS_STOP_MODE_REASON_DUAL_SWITCH)
  {
    emm_ctrl_data_ptr->dual_switch_pended = FALSE;
    MSG_HIGH_DS_0(MM_SUB,"=EMM= Processing pended DUAL switch now.");
  }
#endif
   ...
   *emm_ctrl_data_ptr->is_sim_to_be_ps_invalid_after_power_off = FALSE;
    ...
    for( mm_local_as_id = MM_AS_ID_1; mm_local_as_id < mm_sim_max_subscription; mm_local_as_id ++ )
    {            
          mm_set_as_id(mm_local_as_id);
          emm_ctrl_data_ptr = emm_db_get_ctrl_data();
          if (emm_ctrl_data_ptr->plmn_service_state != NULL)
          {
              /* Send the UNBLOCK on the SUB where it was received originally */
    ...
    MSG_HIGH_DS_1(MM_SUB,"=EMM= emm_illegal_me_sub cause = %d emm_illegal_me_sub cause reset during power off/lpm", *emm_ctrl_data_ptr->emm_illegal_me_sub);
 #endif
  }
    ...
    default:
      ...
      switch(emm_state)
      {
        ...
        case EMM_REGISTERED_INITIATED:
           emm_failure_cause.cause_type = LTE_NAS_IRAT_INVALID_STATE;
#ifdef FEATURE_DUAL_SIM		  
		  if (reg_cmd_ptr->cmd.mmr_stop_mode_req.stop_mode_reason == SYS_STOP_MODE_REASON_DUAL_SWITCH)
		  {
            emm_ctrl_data_ptr->ignore_attach_pti_mismatch = TRUE;	
            if(emm_ctrl_data_ptr->is_detach_req_on_pti_mismatch == TRUE) 
            {
                emm_ctrl_data_ptr->dual_switch_pti = emm_ctrl_data_ptr->pdn_conn_req_ptr->lte_nas_esm_hdr.pti;
            }
			emm_ctrl_data_ptr->reg_req_pending_mm_cmd_ptr = &mmcoord_pending_reg_message;     
            memscpy(emm_ctrl_data_ptr->reg_req_pending_mm_cmd_ptr, sizeof(mm_cmd_type),reg_cmd_ptr,sizeof(mm_cmd_type));
			MSG_HIGH_DS_0(MM_SUB,"=EMM= Mark ignore ATTACH on incorrect PTI to TRUE");
		  }
#endif		
           if((emm_ctrl_data_ptr->rlf_state == TRUE) &&
              (reg_cmd_ptr->cmd.mmr_stop_mode_req.stop_mode_reason == SYS_STOP_MODE_REASON_ALT_SCAN ))
        ...
        case EMM_REGISTERED:
          ...
          {
			// Need to remember to ignore old ATTACH ACCEPT irrespective of RLF state as ATTACH was not sent  
			// For no RLF this will be reset in ATTACH COMPLETE
            emm_ctrl_data_ptr->ignore_attach_pti_mismatch = TRUE;
            if (emm_ctrl_data_ptr->rlf_state == FALSE)
            {
			  ...
			  MSG_HIGH_DS_0(MM_SUB,"=EMM= Stop mode DUAL SWITCH received when ATTACH COMPLETE has not been ACKed by NW");
			  return;
			}            
          }
#endif
...
)
{
    ...
    case EMM_TRACKING_AREA_UPDATING_INITIATED:
      ...
      if(emm_state == EMM_SERVICE_REQUEST_INITIATED)
      {
       ...
       if((emm_ctrl_data_ptr->emm_3gpp_spec_ver < LTE_3GPP_REL12) ||
          (*emm_ctrl_data_ptr->abort_ps_detach_ptr == TRUE)
#ifdef FEATURE_DUAL_SIM
          ||
  ...
  if((emm_ctrl_data_ptr->rrc_active == TRUE) &&
     (emm_ctrl_data_ptr->plmn_service_state->service_status == 
                                                         SYS_SRV_STATUS_SRV) &&
     ((emm_ctrl_data_ptr->emm_update_status == EMM_STATUS_UPDATED) ||
      (emm_state == EMM_REGISTERED_INITIATED) || (emm_state == EMM_TRACKING_AREA_UPDATING_INITIATED)) &&
      ( 
#ifdef FEATURE_LTE_REL11
      (emm_ctrl_data_ptr->emm_3gpp_spec_ver >= LTE_3GPP_REL11) ||
#endif
...
FUNCTION EMM_PROCESS_HIGH_PRIORITY_ACTIVITY_IND

DESCRIPTION
  This function processes MMR_HIGH_PRIORITY_ACTIVITY_IND
...
===========================================================================*/
void emm_process_high_priority_activity_ind
(
  mmr_high_priority_activity_ind_s_type  *high_priority_activity, 
  emm_ctrl_data_type       *emm_ctrl_data_ptr
)
{
  boolean  is_ps_dereg;
  ASSERT(high_priority_activity != NULL);
  ASSERT(emm_ctrl_data_ptr != NULL);
  ASSERT(emm_ctrl_data_ptr->abort_ps_detach_ptr != NULL); 

  is_ps_dereg = high_priority_activity->local_detach;
  MSG_HIGH_DS_1(MM_SUB,"=EMM= Rcved MMR_HIGH_PRIORITY_ACTIVITY_IND  local_detach: %d", is_ps_dereg );
...
} /* end of emm_process_high_priority_activity_ind() */
...
)
{
  ...
  emm_state_type  emm_state = EMM_INVALID_STATE;
  sys_drx_cn_coefficient_s1_e_type   old_drx = emm_get_gmm_drx_cn_coff_s1();
  
  ASSERT(emm_ctrl_data_ptr != NULL); 
  
  MSG_HIGH_DS_2(MM_SUB, "=EMM= Rx DRX value (%d)  DRX RAT = %d", set_drx_req.drx_coefficient, set_drx_req.active_rat);
  ...
  { 
    emm_process_gmm_drx_cn_coff_s1(set_drx_req.drx_coefficient, set_drx_req.client_type);
  ...
  if ((set_drx_req.active_rat == SYS_RAT_MAX) ||
      ((set_drx_req.active_rat == mm_serving_plmn.info.active_rat) &&
       (emm_get_gmm_drx_coff_client_rat() != SYS_RAT_MAX)))
  {
    if (set_drx_req.active_rat != SYS_RAT_NONE)
    {
      emm_set_gmm_drx_coff_client_rat(set_drx_req.active_rat);
    }
       ...
       (mm_timer_status[TIMER_PROHIBIT_DS_DRX] != TIMER_ACTIVE))
    {
      ...
      emm_process_gmm_drx_cn_coff_s1(set_drx_req.drx_coefficient, set_drx_req.client_type);
	  if (emm_get_gmm_drx_cn_coff_s1() == old_drx)
      {
        MSG_HIGH_DS_0(MM_SUB, "=EMM= Current DRX is same as PCS evaluated , send MMR_SET_DRX_CNF ");
        mm_send_mmr_set_drx_cnf(TRUE);
        return;
	  }
      ...
      *emm_ctrl_data_ptr->set_drx_req_pending_ptr = NAS_DRX_CFG_REQ_PENDING;
      ...
      emm_initiate_tau_procedure(EMM_INITIAL_UPDATE, 
              ...
              (mm_timer_status[TIMER_T3402] != TIMER_STOPPED ||
               ...
               mm_timer_status[TIMER_PROHIBIT_DS_DRX] != TIMER_STOPPED)) 
              ||
              (set_drx_req.client_type == CM_CLIENT_TYPE_DS))
    {
  ...
  {
    MSG_HIGH_DS_0(MM_SUB, "=EMM= Send DRX CNF FALSE - Reject SET_DRX_REQ received from MCS when QMI has set valid value ");
    mm_send_mmr_set_drx_cnf(FALSE);
  }
...
FUNCTION  EMM_PROCESS_DATA_PRI_IND

DESCRIPTION
  This function processes the data pri ind from CM/DS
...
void  emm_process_data_prio_ind(mmr_data_priority_ind_s_type mmr_data_prio_ind,
                                emm_ctrl_data_type       *emm_ctrl_data_ptr)
{
  *emm_ctrl_data_ptr->ds_proc_type_ptr = mmr_data_prio_ind.proc_type;
  emm_send_rrc_process_priority(mmr_data_prio_ind.proc_type,
...
)
{
  ...
  if(emm_ctrl_data_ptr->rrc_active == FALSE)
  {
    ...
    {
      emm_add_eplmn_to_backoff_fplmn_list(plmn, timer_count, cause,block_plmn_req_ptr->eplmn_index);
    }
    else
    {
  ...
  else if(emm_ctrl_data_ptr->reg_complete_ack_pending != EMM_PENDING_REG_ACK_NONE)
  {
    ...
    emm_ctrl_data_ptr->pending_block_plmn_req.eplmn_index = block_plmn_req_ptr->eplmn_index;
       ...
       (emm_ctrl_data_ptr->reg_req_pended_indication == FALSE))
    {
      switch(EMM_GET_STATE())
      {
        case EMM_DEREGISTERED:
		  ...
		  MSG_HIGH_DS_0(MM_SUB,"=EMM= Stopping 3411 and resetting attach counter");
          break;
        ...
        case EMM_REGISTERED:
		  ...
		  MSG_HIGH_DS_0(MM_SUB,"=EMM= Stopping 3411 and resetting tau counter");
          break;
...
)
{
  ...
  switch (reg_msg_ptr->cmd.hdr.message_id)
  {
    ...
    case MMR_HIGH_PRIORITY_ACTIVITY_IND:
      emm_process_high_priority_activity_ind(&(reg_msg_ptr->cmd.mmr_high_priority_activity_ind), emm_ctrl_data_ptr) ;
      break;
#ifdef MMOC_API_PH_STAT_CNF_WITH_STATUS
    ...
    case MMR_REFRESH_STAGE_IND:
      emm_process_refresh_stage_ind(reg_msg_ptr->cmd.mmr_refresh_stage_ind , emm_ctrl_data_ptr);
      break;
...
)
{
...
#else
    mmr_plmn_search_cnf_ptr->plmn_search_result = REG_MM_PLMN_SEARCH_SUCCESS;
    mmr_plmn_search_cnf_ptr->found_plmn_list.plmn_list.length = 
                           rrc_plmn_search_cnf_ptr->found_plmn_rat_list.length;
    for(index=0;index<mmr_plmn_search_cnf_ptr->found_plmn_list.plmn_list.length;index++)
    {
      mmr_plmn_search_cnf_ptr->found_plmn_list.plmn_list.info[index] =
                      rrc_plmn_search_cnf_ptr->found_plmn_rat_list.info[index];
    }
#endif  
  ...
  else
  {
    ...
    else if( rrc_plmn_search_cnf_ptr->status == LTE_RRC_PLMN_SEARCH_ABORTED_ON_TRM_FAILURE)   
    {
      mmr_plmn_search_cnf_ptr->plmn_search_result = REG_MM_PLMN_SEARCH_ABORTED_DUE_TO_NO_LOCK;
    }
    else
    {
      mmr_plmn_search_cnf_ptr->plmn_search_result = REG_MM_PLMN_SEARCH_ABORTED;
    }
    if(mm_emm_is_qrd_device == FALSE && mm_partial_search_allowed == FALSE && mm_emm_manual_search_partial_get() == FALSE)
    {
...
FUNCTION EMM_SEND_MMR_ME_BLOCKED_STATUS_IND
...
===========================================================================*/
void emm_send_mmr_me_blocked_status_ind(boolean is_me_blocked)
{
  mmr_me_blocked_status_ind_type reg_me_blocked_ind;
  memset((void *)&reg_me_blocked_ind,0x0,sizeof(mmr_me_blocked_status_ind_type));
  ...
  reg_me_blocked_ind.as_id = (sys_modem_as_id_e_type)mm_as_id; 
  PUT_IMH_LEN( sizeof( mmr_me_blocked_status_ind_type) - sizeof(IMH_T),
                          &reg_me_blocked_ind.message_header );
  MSG_HIGH_DS_1(MM_SUB, "=MM= EMM sent MMR ME BLOCKED IND.status %d", reg_me_blocked_ind.is_me_blocked);
  mm_send_message(&reg_me_blocked_ind.message_header, GS_QUEUE_REG );
}
#endif
...
)
{
  ...
  if(emm_ctrl_data_ptr->emc_srv_status == LTE_NAS_EMC_SRV_ONLY)
  {
    ...
    mmr_service_ind.service_state.eps_nw_feature_support.emc_bs_supported = FALSE;
    if((emm_ctrl_data_ptr->plmn_service_state->eps_nw_feature_support.emc_bs_supported
                                                                             == TRUE) &&
       (emm_ctrl_data_ptr->last_rrc_service_ind_ptr->camped_svc_info.\
                                            ims_emergency_support_enabled  == TRUE))
    {
      mmr_service_ind.service_state.eps_nw_feature_support.emc_bs_supported = TRUE;
    }
  }
#endif 
  ...
  mmr_service_ind.oos_srch_done = emm_ctrl_data_ptr->last_rrc_service_ind_ptr->oos_search_done;
  ...
  MSG_HIGH_DS_3(MM_SUB, "=EMM=  emc_srv_spt = %d emc_attached = %d search done %d",
                mmr_service_ind.service_state.eps_nw_feature_support.emc_bs_supported,
...
)
{
  ...
  */
  ...
  if(emm_ctrl_data_ptr->emc_srv_status == LTE_NAS_EMC_SRV_ONLY)
  {
    ...
    mmr_reg_cnf_ptr->service_state.eps_nw_feature_support.emc_bs_supported = FALSE;
    if((emm_ctrl_data_ptr->plmn_service_state->eps_nw_feature_support.emc_bs_supported
                                                                             == TRUE) &&
       (emm_ctrl_data_ptr->last_rrc_service_ind_ptr->camped_svc_info.\
                                            ims_emergency_support_enabled  == TRUE))
    {
      mmr_reg_cnf_ptr->service_state.eps_nw_feature_support.emc_bs_supported = TRUE;
    }
  }
#endif
  ...
  mmr_reg_cnf_ptr->oos_srch_done = emm_ctrl_data_ptr->last_rrc_service_ind_ptr->oos_search_done;
...
FUNCTION EMM_PROCESS_REFRESH_STAGE_IND
...
===========================================================================*/
void emm_process_refresh_stage_ind
( 
  mmr_refresh_stage_ind_s_type refresh_ind,
  emm_ctrl_data_type           *emm_ctrl_data_ptr
)
{
  if ( (refresh_ind.refresh_stage == MMGSDI_REFRESH_STAGE_WAIT_FOR_OK_TO_INIT) &&
       (emm_ctrl_data_ptr->ps_detach_info.status == EMM_PS_DETACH_CNF_PENDING) &&
       (mm_timer_status[TIMER_T3421] == TIMER_ACTIVE) )
  {
    /* EMM PS detach procedure will be aborted on the next T3421 expiry
       when refresh stage is WAIT_FOR_OK_TO_INIT
     */
    emm_ctrl_data_ptr->emm_detach_attempt_counter = MAX_NUMBER_DETACH_OF_ATTEMPTS;
    MSG_HIGH_DS_3(MM_SUB,"=EMM= refresh stage %d, T3421 timer status %d, detach attempt counter %d",
                          refresh_ind.refresh_stage,
                          mm_timer_status[TIMER_T3421],
...
FUNCTION  EMM_PROCESS_NON_ACQ_PARAM_CHANGE_IND
...
===========================================================================*/
void emm_process_non_acq_param_change_ind
(
  mm_cmd_type              *cmd_ptr,
  emm_ctrl_data_type       *emm_ctrl_data_ptr
)
{
  emm_substate_type emm_substate;
  emm_state_type  emm_state = EMM_INVALID_STATE;
  sys_voice_domain_pref_e_type              prev_voice_domain_pref = emm_ctrl_data_ptr->reg_lte_pended_cs_domain_param_ptr->voice_domain_pref;
  ASSERT(emm_ctrl_data_ptr != (emm_ctrl_data_type *)NULL);
  MSG_HIGH_DS_2(MM_SUB, "=EMM= Previous update voice domain pref %d New voice domain pref %d",
                emm_ctrl_data_ptr->reg_lte_cs_domain_param.voice_domain_pref,
                cmd_ptr->cmd.non_acq_param_change_ind.voice_domain_pref);
  emm_state = EMM_GET_STATE();
  emm_substate = EMM_GET_SUBSTATE();

  /* Do not update pended state to PENDING while state is set to PROCESSING, but update pended parameters. */
    emm_ctrl_data_ptr->reg_lte_pended_cs_domain_param_ptr->is_voice_domain_ie_valid = TRUE;
    emm_ctrl_data_ptr->reg_lte_pended_cs_domain_param_ptr->voice_domain_pref = cmd_ptr->cmd.non_acq_param_change_ind.voice_domain_pref;

  /* Update parameters in REG */
  mm_send_mmr_non_acq_update_ind(gmm_reg_pended_lte_cs_param.voice_domain_pref);
  if(mmcoord_reg_req_pending == TRUE)
  {
    mmcoord_pending_reg_message.cmd.mmr_reg_req.lte_cs_domain_param.voice_domain_pref = cmd_ptr->cmd.non_acq_param_change_ind.voice_domain_pref;
  } 
  else if((emm_ctrl_data_ptr->reg_req_pending_mm_cmd_ptr != NULL) && (emm_ctrl_data_ptr->reg_req_pended_indication == TRUE))
  {
	emm_ctrl_data_ptr->reg_req_pending_mm_cmd_ptr->cmd.mmr_reg_req.lte_cs_domain_param.voice_domain_pref = cmd_ptr->cmd.non_acq_param_change_ind.voice_domain_pref;
  }
  if( (emm_ctrl_data_ptr->reg_lte_cs_domain_param.voice_domain_pref != cmd_ptr->cmd.non_acq_param_change_ind.voice_domain_pref) &&
  	...
  	(emm_state != EMM_TRACKING_AREA_UPDATING_INITIATED))
  {
    ...
    {
      MSG_HIGH_DS_1(MM_SUB, "=EMM= Initing TAU- voice domain pref change, new voice domain value = %d ",
                              cmd_ptr->cmd.non_acq_param_change_ind.voice_domain_pref);
  ...
  else if(prev_voice_domain_pref != cmd_ptr->cmd.non_acq_param_change_ind.voice_domain_pref &&
  	     (emm_state == EMM_REGISTERED_INITIATED || emm_state == EMM_TRACKING_AREA_UPDATING_INITIATED))
  {
    MSG_HIGH_DS_3(MM_SUB, "=EMM= TAU delayed - EMM state %d, voice domain pended %d ongoing voice domain %d",
                    EMM_GET_STATE(),
                    emm_ctrl_data_ptr->emm_voice_domain_change_pended,
                    emm_ctrl_data_ptr->reg_req_pending_ptr->lte_cs_domain_param.voice_domain_pref);
    if((emm_ctrl_data_ptr->reg_req_pending_ptr->lte_cs_domain_param.voice_domain_pref == cmd_ptr->cmd.non_acq_param_change_ind.voice_domain_pref) &&
		emm_ctrl_data_ptr->emm_voice_domain_change_pended == NAS_NON_ACQ_REQ_PENDING)
    {
