...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/mm/src/mmstate.c#3 $    $DateTime: 2022/01/20 01:42:25 $ $Author: pwbldsvc $
...
extern boolean mm_get_lte_redir_during_lu();
...
#define gprs_service_attempt_counter gprs_service_attempt_counter_sim[mm_as_id] /* !tocheck! can it be removed from here */
#endif
...
#ifdef FEATURE_NAS_REL12_SPEC_CHANGE
extern byte    gprs_service_attempt_counter; /* !tocheck! can it be removed from here */
#endif
...
state_event_T mm_std_table[] =
{
...
#ifdef FEATURE_DUAL_SIM
   {  3, 10, MM_STD_RADIO_PREEMPTION,         ACTION_37 },
#endif 
   ...
   {  5, 19, MM_STD_LOW_LEVEL_FAIL_LAI_CHANGE,        ACTION_19 },
...
#ifdef FEATURE_DUAL_SIM
   { 5, 9, MM_STD_RADIO_PREEMPTION,                ACTION_0 },
#endif 
...
#endif 
#ifdef FEATURE_DUAL_SIM
   { 6, 9, MM_STD_RADIO_PREEMPTION,                ACTION_0 },
#endif 
   ...
   {  7, 7, MM_STD_RADIO_PREEMPTION,         ACTION_12 },
#endif
#ifdef FEATURE_CCBS
...
#endif /* end of (FEATURE_MMGPS || FEATURE_CGPS_UMTS_CP) */
#ifdef FEATURE_DUAL_SIM
   { 8, 9, MM_STD_RADIO_PREEMPTION,                ACTION_0 },
#endif 
#endif /*FEATURE_CCBS */
...
#ifdef FEATURE_DUAL_SIM
   {  10, 10, MM_STD_RADIO_PREEMPTION,         ACTION_0 },
#endif 
...
#endif
#ifdef FEATURE_DUAL_SIM
   {  13, 10, MM_STD_RADIO_PREEMPTION,         ACTION_37 },
#endif 
   ...
   { 14, 9, MM_STD_RADIO_PREEMPTION,                ACTION_0 },
#endif
   ...
   { 15, 7, MM_STD_RADIO_PREEMPTION,                  ACTION_12 },
#endif
   ...
   { 17, 9, MM_STD_RADIO_PREEMPTION,                  ACTION_0 },
#endif 
...
#ifdef FEATURE_DUAL_SIM
   { 20, 9, MM_STD_RADIO_PREEMPTION,                ACTION_0 },
#endif 
   ...
   { 21, 9, MM_STD_RADIO_PREEMPTION,                ACTION_0 },
#endif 
   ...
   { 25, 9, MM_STD_RADIO_PREEMPTION,                ACTION_0 },
#endif 
#endif
   /* THIS MUST BE LAST */
...
#endif
   byte           table_size = (sizeof(mm_std_table)/sizeof(state_event_T));/*Klocwork fix*/
 ...
 */
   ...
   while (mm_std_table[entry].event != MM_STD_INVALID_EVENT )
   {
      if ( mm_std_table[entry].current_state == mm_state )
      {
         if (mm_std_table[entry].event ==  mm_event )
         {
            next_state = mm_std_table[entry].next_state;

            action = mm_std_table[entry].action_code;
            entry_found = TRUE;
            break;
         }
      }
   ...
   if (entry_found)
   {
      ...
      switch (action)
      {
         ...
         case ACTION_10:     /* check_lu_fail_action */
              ...
              if (mm_get_lte_redir_during_lu() == FALSE) 
              {
         ...
         case ACTION_18:     /* notify_reestablishment_fail, */
            mm_get_mmcc_rej_cause(mm_std_table[entry].event, &cause_type, &cause_value);
            mm_notify_reestablishment_fail(cause_type,cause_value);
            break;

         case ACTION_19:
            /* Send a establish reject to CNM */
			if (mm_event == MM_STD_LOW_LEVEL_FAIL && mm_cnm_rel_req_received == TRUE)
			{
              MSG_HIGH_DS_0(MM_SUB, "=MM= MMCNM_REL_REQ received during IRAT, MM moving to IDLE, no need to send mmcnm_est_rej");
			  break;
			}
         ...
         case ACTION_37:
		  ...
		  mm_lu_preemption_failure_count++;
          break;
#endif 
      ...
      */
      ...
      if(is_gsm_mode())
      {
        ...
        else if(((mm_csfb_is_blind_redir_to_lte_req_to_be_sent == TRUE)|| mm_timer_status[TIMER_FRLTE] == TIMER_ACTIVE) &&
...
#endif /* FEATURE_GSTK */
#endif /* end of (FEATURE_MMGPS || FEATURE_CGPS_UMTS_CP) */
      if (( current_state != MM_IDLE ) && (mm_state == MM_IDLE))
      {
         ...
         mm_proc_status_set(SYS_PROC_TYPE_CS_REGISTRATION, FALSE);
...
//FR15492
        ...
        else if (mm_need_to_send_no_srv_on_call_end_due_to_dos == TRUE)
        {
         ...
         else if(mm_reg_req_revd_active_call == TRUE)
         {
           ...
           {
#if defined(FEATURE_DUAL_SIM) && defined (FEATURE_DUAL_DATA) && defined (FEATURE_LTE)
            if( (MM_IS_NON_DDS_SUB) && (emm_ctrl_data_ptr->set_sg_flag_on_lau == TRUE))
            {
         ...
         */
               ...
               case TIMER_T3211:
                  ...
                  else
                  {
                     if( mm_timer_status [ TIMER_T3211] == TIMER_STOPPED )
                     {
                       ...
                       {
                         /* Restarting Timer T3211 with smaller value, as LU failed due to lower layer activity */
         ...
         else if (((mm_previous_idle_substate == MM_NORMAL_SERVICE) ||
                  ...
                  (mm_idle_substate == MM_LIMITED_SERVICE))
         {
            ...
            ** paused earlier, resume it now.
            ** -------------------------------------------------------- */
           if ((mm_idle_transition_timer == TIMER_T3211) &&
          ...
          (is_gsm_mode() || is_umts_mode()))
      {
...
#endif 
#ifdef FEATURE_DUAL_SIM
        if((is_gsm_mode()) && (mm_timer_status[TIMER_MM_DUAL_SWITCH] == TIMER_ACTIVE))
        {
           ...
           else if (sm_check_pdp_procedure_pending((sys_modem_as_id_e_type)mm_sub_id))
           {
             MSG_HIGH_DS_0(MM_SUB,"=MM= DUAL SWITCH pended due to PDP signalling in progress "); 
           }
           else
           {
		...
		else if(mm_hard_abort_deep_sleep_pending)
		{
		  if((mm_rat_handler_status == MM_RAT_STOP_TDSCDMA_IN_PROGRESS) || (mm_rat_handler_status == MM_RAT_STOP_WCDMA_IN_PROGRESS) ||
             (mm_rat_handler_status == MM_RAT_STOP_GSM_IN_PROGRESS))
          {
            MSG_HIGH_DS_0(MM_SUB,"=MM= DEEP SLEEP pended due to IRAT in progress "); 
          }
          else
          {
		...
		else if(mm_abort_dual_switch_pending == SYS_STOP_MODE_REASON_DEEP_SLEEP)
        {
          if((mm_rat_handler_status == MM_RAT_STOP_TDSCDMA_IN_PROGRESS) || (mm_rat_handler_status == MM_RAT_STOP_WCDMA_IN_PROGRESS))
          {
            MSG_HIGH_DS_0(MM_SUB,"=MM= DEEP SLEEP pended due to IRAT in progress "); 
                  ...
                  (mm_serving_plmn.mm_service_state == SERVICE_AVAILABLE))
        {
           switch (mm_pending_lu_request)
           {
              ...
              case LTE_IRAT_LU:
				 ...
				 else if ((mmcoord_reg_req_pending) && (mmcoord_pending_reg_message.cmd.hdr.message_set == (int) MS_MM_REG &&
                                                         mmcoord_pending_reg_message.cmd.hdr.message_id == (int) MMR_REG_REQ))
                 {
                   MSG_HIGH_DS_0(MM_SUB,"=MM= REG request pending do not start IRAT LU ");
                 }
                 else if (mm_waiting_for_stop_mode_cnf)
                 {
                   MSG_HIGH_DS_0(MM_SUB,"=MM= STOP CNF pending, do not start IRAT LU ");
                 }
                 else
                 {
                 if ( mm_retry_page_rsp == TRUE && mm_last_page_type != PAGE_TYPE_NONE && mm_timer_status[TIMER_MM_PAGE_RETRY] != TIMER_STOPPED &&
#ifdef FEATURE_DUAL_SIM
...
** ---------------------------- ---------  ---------------------------- ---------  ---------------------------- --------- */
         ...
         !mm_lu_pended_nmo_unknown)                                      
      {
        ...
        if((mm_serving_plmn.mm_service_state == SERVICE_AVAILABLE)&&
           ...
           !((mmcoord_reg_req_pending) && (mmcoord_pending_reg_message.cmd.hdr.message_set == (int) MS_MM_REG &&
                                           mmcoord_pending_reg_message.cmd.hdr.message_id == (int) MMR_REG_REQ)))
        {
          mm_initiate_lu( MM_LU_CAUSE_NORMAL );       
        }
