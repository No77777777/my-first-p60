...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/mm/src/mmregister.c#1 $    $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
      ...
      case MS_TIMER:
         ...
         case TIMER_T3243:
...
#endif
              {
                ...
                if (msg_p->cmd.timer_expiry.timer_id == TIMER_T3242) 
                {
                  mm_timer_expiry_info.timer_bitmask |= SYS_MM_TIMER_T3242_INFO_MASK;
                }
              
                if (msg_p->cmd.timer_expiry.timer_id == TIMER_T3243) 
                {
