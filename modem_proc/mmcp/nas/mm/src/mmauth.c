...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/mm/src/mmauth.c#2 $ $DateTime: 2023/10/08 15:22:21 $ $Author: pwbldsvc $
...
** PS domain GPRS Ciphering Key
*/
auth_gprs_cipherkey_type auth_ps_gprs_cipherkey;

/*
** PS domain GPRS Integrity Key
*/
auth_gprs_integritykey_type auth_ps_gprs_integritykey;
...
** PS domain GPRS Ciphering Key
*/
auth_gprs_cipherkey_type lte_auth_ps_gprs_cipherkey_sim[MAX_AS_IDS] ;
#define lte_auth_ps_gprs_cipherkey lte_auth_ps_gprs_cipherkey_sim[mm_as_id]

/*
** PS domain GPRS Integrity Key
*/
auth_gprs_integritykey_type lte_auth_ps_gprs_integritykey_sim[MAX_AS_IDS]  ;
#define lte_auth_ps_gprs_integritykey lte_auth_ps_gprs_integritykey_sim[mm_as_id]
#else
      ...
      if (total_msg_length >= MIN_GSM_UMTS_SIM_RESPONSE_LENGTH)
      {
        ...
        */
        if (gsm_cipherkey->value_length != GSM_SECURITY_KEY_LENGTH)
        {
          ...
          if(mm_send_auth_rsp_on_invalid_gsm_kc_len)
	  {
...
)
{
  ...
  memscpy((void *)gsm_kc_128bit_in->value_data,MAX_AUTH_DATA_LENGTH,
...
SIDE EFFECTS
  None
===========================================================================*/
boolean auth_is_cs_ckik_derived_from_gsmkc(void)
{
...
/*24.008: 4.1.1.8	Handling of security related parameters at switch on and switch off*/
     ...
     (mm_sim_card_mode == MMGSDI_APP_SIM))
  {
    ...
    if(mm_sim_card_mode == MMGSDI_APP_SIM && cs_hfn == 0 && hfn_threshold == 0 && ps_hfn == 0)
    {
      MSG_HIGH_DS_0(MM_SUB, "=MM= Not deleting CKSN as CS/PS HFN and threshold values are 0 for SIM") ;
    }
    else
    {
...
void auth_update_hfns_to_sim_card( boolean set_threshold )
{
   ...
   if( mm_sim_card_mode == MMGSDI_APP_USIM || mm_sim_card_mode == MMGSDI_APP_SIM)
   {
      ...
      if(mmgsdi_is_test_mode_logging_enabled_for_subscription((sys_modem_as_id_e_type)mm_sub_id))
      {
        MSG_HIGH_DS_6( MM_SUB, "=MM= Auth Current CS HFN: %d %d %d\n =MM= Auth Current PS HFN: %d %d %d", auth_current_hfns.cs_hfn[0],auth_current_hfns.cs_hfn[1],auth_current_hfns.cs_hfn[2],
                                    auth_current_hfns.ps_hfn[0],auth_current_hfns.ps_hfn[1],auth_current_hfns.ps_hfn[2]);
