...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/mm/src/emm_connection_handler.c#1 $

when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
07/25/11   Abhi    If MO CSFB call is not pending then only send release indication to CNM on 
...
emm_reg_container_type emm_reg_container_sim[MAX_AS_IDS] = 
             {(emm_reg_container_type)NULL, (emm_reg_container_type)NULL} ;
#define emm_reg_container emm_reg_container_sim[mm_as_id]


static emm_release_client_type  \
                 emm_release_client_array_sim[MAX_AS_IDS][EMM_RELEASE_CLIENT_ARRAY_MAX_LENGTH];
#define emm_release_client_array emm_release_client_array_sim[mm_as_id]

static word  emm_release_client_array_length_sim[MAX_AS_IDS] = {0, 0} ;
#define emm_release_client_array_length emm_release_client_array_length_sim[mm_as_id]
...
{  
  ...
  //nas_event_report_payload(EVENT_LTE_REG_OUTGOING_MSG,sizeof(event), (void*)&event,mm_message_as_id);
}
...
)
{
  ...
  MSG_HIGH_DS_1(MM_SUB, "=EMM= Reg container setup %d",
                           (reg_req_tobe_executed != (emm_reg_container_type)NULL));
  if(reg_req_tobe_executed != (emm_reg_container_type)NULL)
  {
    mm_reg_req_trans_id_reset();
  }
...
)
{
                                          ...
                                          emm_ctrl_data_ptr) == TRUE))
  {
    ...
    emm_process_unsent_messages(CONN_CNF_LTE_RRC_CONN_EST_FAILURE_ACCESS_BARRED, 
                                emm_failure_cause,
                                emm_ctrl_data_ptr);
...
)
{
  ...
  if((barring_mask & EMM_BARRING_MASK_SUB1) == EMM_BARRING_MASK_SUB1)
  {
    sub_id = MM_AS_ID_1;
	barring_mask = barring_mask^EMM_BARRING_MASK_SUB1;
  }
  else if((barring_mask & EMM_BARRING_MASK_SUB2) == EMM_BARRING_MASK_SUB2)
  {
    sub_id = MM_AS_ID_2;
	barring_mask = barring_mask^EMM_BARRING_MASK_SUB2;
  }
  else
  {
    sub_id = MAX_AS_IDS;
...
)
{
  ...
  if(mm_as_id == MM_AS_ID_1)
  {
    *access_barring_flag |= EMM_BARRING_MASK_SUB1; 
  }
  else if(mm_as_id == MM_AS_ID_2)
  {
    *access_barring_flag |= EMM_BARRING_MASK_SUB2; 
  }
#endif
...
/*===========================================================================
FUNCTION    EMM_CHECK_IF_BARRING_TIMER_RUNNING

DESCRIPTION
  This function takes Timer as arguement and return true or false depending on running
...
boolean emm_check_if_barring_timer_running(rex_timer_type *timer)
{
   ASSERT(timer != (rex_timer_type *)NULL) ; 
   if(rex_get_timer(timer) > 0)
   {
...
)
{
  ...
  /* Resume radio capanility timer upon connestion establishment or failure */
  emm_resume_paused_timer(TIMER_RADIO_CAP_ENQ);
  ...
  switch (rrc_conn_est_cnf->est_status)
  {
...
#endif
      MSG_HIGH_DS_4(MM_SUB, "=EMM= RRC connection est failed - %u, ext_wait_time = %d , deprior_is_active =%d, reg_req_pend = %d",
                    rrc_conn_est_cnf->est_status, emm_ctrl_data_ptr->extended_wait_time,rrc_conn_est_cnf->deprior_is_active,
...
#ifdef FEATURE_CRAT_MSIM_TEST_FRAMEWORK
     /* Send LTE suspend cnf to CM if connection abort was sent due to MO call */
     if (mm_timer_status[TIMER_ELS_CONN_REL] == TIMER_ACTIVE)
     {
       mm_stop_timer(TIMER_ELS_CONN_REL);

       /* If ELS timer is running, check reason for ELS connection abort
        - Due to MPLMN search: reset the flag; no further action.
        - Due to MO call: ELS connection aboet happened due to CM_SUSPEND_REQ. Send CNF to CM. 
       */ 
      ...
      if(emm_ctrl_data_ptr->aborting_conn == TRUE)
      { 
        MSG_HIGH_DS_4(MM_SUB, "=EMM= Rcved CONN CNF as rsp of CONN ABORT - Deact pending %d, reg_req_pend = %d reg_con=%d, %d",
    ...
    default:
      ...
      MSG_FATAL_DS(MM_SUB,"=EMM= Illegal connection status %d",rrc_conn_est_cnf->est_status,0,0) ;
      break ;        
  }
...
)
{
  ...
  MSG_HIGH_DS_4(MM_SUB,
       "=EMM= emm_nv_force_tau_nondds = %d, non_nas_procedure_performed = %d local_rel_tau_after_csfb_tau = %d second_tau_after_csfb_needed = %d",
       emm_ctrl_data_ptr->emm_nv_force_tau_nondds, 
       rrc_conn_rel_ind->non_nas_procedure_performed,
  ...
  if(((rrc_conn_rel_ind->rel_reason == LTE_RRC_CONN_REL_NORMAL) ||
       (rrc_conn_rel_ind->rel_reason == LTE_RRC_CONN_REL_RLF_NONDDS_SPV_TMR_EXPIRY))&&
  ...
  if(rrc_conn_rel_ind->rel_reason != LTE_RRC_CONN_REL_LOCAL )
  {
    emm_ctrl_data_ptr->tau_local_release  = EMM_TAU_LOCAL_REL_NONE;
  }
  MSG_HIGH_DS_1(MM_SUB, "=EMM= TAU Rel cause %d ",emm_ctrl_data_ptr->tau_local_release);
  ...
  /* 24.301 5.5.3.2.2 i - RRC connection failure and no user data pending */
          ...
          ((emm_ctrl_data_ptr->rlf_state == TRUE) && 
           (rrc_conn_rel_ind->rel_reason == LTE_RRC_CONN_REL_ABORTED)))
  {
    ...
    */
    ...
    {
       MSG_HIGH_DS_0(MM_SUB, "=EMM= setting rau_pending_ll_failure flag to remember that RAU needs to be done on IRAT to G/W from Lte");
       emm_ctrl_data_ptr->rau_pending_ll_failure = TRUE;
    }
  }
#ifdef FEATURE_DUAL_DATA
  else if((emm_ctrl_data_ptr->emm_nv_force_tau_nondds == TRUE &&
           MM_IS_NON_DDS_SUB &&
           ((emm_ctrl_data_ptr->paging_conn_est == TRUE) ||
            (rrc_conn_rel_ind->non_nas_procedure_performed == TRUE) ||
            (emm_ctrl_data_ptr->local_rel_tau_after_csfb_tau == TRUE &&
			 ...
			 emm_ctrl_data_ptr->nas_sms_pending_con_rel == TRUE)&&
           rrc_conn_rel_ind->rel_reason == LTE_RRC_CONN_REL_LOCAL) || 
          (emm_ctrl_data_ptr->tau_pending_after_msearch_abort == TRUE && 
           emm_ctrl_data_ptr->aborting_conn == TRUE))
  {
    MSG_HIGH_DS_0(MM_SUB,
     "=EMM= Local connection release set the flag to trigger TAU after camping");
    tau_cause = TAU_CAUSE_LOW_LAYER_FAILURE;
	emm_ctrl_data_ptr->last_tau_type = TAU_CAUSE_LOW_LAYER_FAILURE;
	 if(emm_ctrl_data_ptr->tau_pending_after_msearch_abort == TRUE && 
           emm_ctrl_data_ptr->aborting_conn == TRUE)
    {
     emm_ctrl_data_ptr->hold_tau_trm_for_local_release  = TRUE;
    }
  }
  emm_ctrl_data_ptr->paging_conn_est = FALSE;
  emm_ctrl_data_ptr->nas_sms_pending_con_rel = FALSE;
...
#ifdef FEATURE_DUAL_SIM
  if( (MM_IS_NON_DDS_SUB == FALSE || mm_sim_max_subscription == 1) &&
     rrc_conn_rel_ind->rel_reason == LTE_RRC_CONN_REL_LOCAL &&
     emm_ctrl_data_ptr->tau_local_release  != EMM_TAU_LOCAL_REL_MAC_INACTIVITY)
  {
    MSG_HIGH_DS_0(MM_SUB,
     "=EMM= Local connection release set the flag to trigger TAU after camping on DDS/Single SUB");
    tau_cause = TAU_CAUSE_LOW_LAYER_FAILURE;
	emm_ctrl_data_ptr->last_tau_type = TAU_CAUSE_LOW_LAYER_FAILURE;
    emm_ctrl_data_ptr->tau_local_release  = EMM_TAU_LOCAL_REL_MAC_INACTIVITY;
  }
...
#ifdef FEATURE_CRAT_MSIM_TEST_FRAMEWORK
  /* Send LTE suspend cnf to CM if connection release happened due to ELS abort */
  if (mm_timer_status[TIMER_ELS_CONN_REL] == TIMER_ACTIVE)
  {
     MSG_HIGH_DS_1(MM_SUB, "=EMM= ELS connection release received for reason %d", emm_ctrl_data_ptr->emm_els_conn_abort_reason);
     mm_stop_timer(TIMER_ELS_CONN_REL);
  ...
  if (mm_timer_status[TIMER_HOLD_TRM_LOCAL_REL] == TIMER_ACTIVE)
  {
    mm_stop_timer(TIMER_HOLD_TRM_LOCAL_REL);
  }
  mm_stop_timer(TIMER_LOCAL_REL_MSEARCH);
  /* Connection was aborted */       
  if(emm_ctrl_data_ptr->aborting_conn == TRUE)
  {
    MSG_HIGH_DS_1(MM_SUB,"=EMM= Rcved CONN REL as rsp of CONN ABORT - Deact pending %d",
             emm_ctrl_data_ptr->rrc_deact_req.deact_req_pending);
