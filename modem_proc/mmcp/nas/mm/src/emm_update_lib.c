...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/mm/src/emm_update_lib.c#5 $

when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
10/30/14   us      Correction of Spec version check in handling of Attach Reject with cause LTE_NAS_CONGESTION.  
...
boolean is_nas_emm_attach_tau_timeout_sim[MAX_AS_IDS] = {FALSE, FALSE};
#define is_nas_emm_attach_tau_timeout is_nas_emm_attach_tau_timeout_sim[mm_as_id]

emm_mo_detach_type nas_emm_power_off_detach_type_sim[MAX_AS_IDS] = {INVALID_DETACH_TYPE,INVALID_DETACH_TYPE};
#define nas_emm_power_off_detach_type nas_emm_power_off_detach_type_sim[mm_as_id]

boolean nas_emm_tau_cs_rej_sim[MAX_AS_IDS] = {FALSE, FALSE};
#define nas_emm_tau_cs_rej nas_emm_tau_cs_rej_sim[mm_as_id]

lte_nas_emm_eps_update_val_type nas_emm_tau_cs_rej_tau_type_sim[MAX_AS_IDS] = {TA_UPDATING,TA_UPDATING};
...
)
{
  ...
  emm_send_rrc_reg_status_ind(emm_ctrl_data_ptr,LTE_RRC_REG_FAILED,emm_ctrl_data_ptr->emm_attach_attempt_counter);
#endif
...
#ifdef FEATURE_LTE_REL9 
    if((emm_ctrl_data_ptr->switch_plmn == FALSE) &&
  ...
  else
  {
    ...
    //release attach pdn after max retry counter is reached
    emm_release_attach_pdn_conn_req(emm_ctrl_data_ptr, emm_failure_cause);
...
)
{
  ...
  if(emm_ctrl_data_ptr->esr_type_in_progress == NAS_EMM_3GPP_CSFB_BUFFERED)
  {
    ...
    csfb_failure_cause.failure_cause.lte_nas_failure_cause =  EMM_DETACHED;
    emm_process_extended_service_req_failure(csfb_failure_cause,emm_ctrl_data_ptr);
  }
#endif

  emm_ssgccs_send_attach_failed_ind(cause);
  ...
  if(emm_state == EMM_REGISTERED_INITIATED)
  {
    ...
    switch (cause)
    {
      ...
      case TRANSMISSION_LTE_RRC_UL_DATA_CNF_FAILURE_CTRL_NOT_CONN:
        switch (emm_substate)
        {
          ...
          case EMM_WAITING_FOR_NW_RESPONSE:
            ...
            {
#ifdef FEATURE_DUAL_SIM
              MSG_HIGH_DS_0(MM_SUB, "=EMM= Aborting conn/ DUAL switch pended - Do not start re-attach");
#else
      ...
      case CONN_CNF_LTE_RRC_CONN_EST_FAILURE_CONN_REJECT:
        ...
        if(emm_check_if_pdn_conn_req_for_emergency_bearer_services(emm_ctrl_data_ptr->pdn_conn_req_ptr->req) && 
           emm_ctrl_data_ptr->emc_srv_status == LTE_NAS_EMC_SRV_PENDING)
        {
...
#ifdef FEATURE_DISABLED_HABANERO
	  case CONN_REL_LTE_RRC_CONN_REL_PDCP_REL_CAUSED_BY_LTA:
#endif
...
)
{
  ...
  emm_ssgccs_send_attach_failed_ind(release_cause);
...
)
{
  ...
  */
  ...
  if((gsm_spt == FALSE) && (umts_spt == FALSE) && (tds_spt == FALSE))
  {
    if(emm_ctrl_data_ptr->emm_guti_valid == TRUE)
    {
      ...
      emm_attach_request->guti_type = LTE_NAS_GUTI_TYPE_NATIVE;
           ...
           ||((emm_ctrl_data_ptr->plmn_service_state->sim_state == 
                                               SYS_SIM_STATE_CS_INVALID) &&
              (emm_check_if_pdn_conn_req_for_emergency_bearer_services(emm_ctrl_data_ptr->pdn_conn_req_ptr->req)== FALSE) )
#endif
           )
    {
...
#ifdef FEATURE_LTE_REL9
    else if(emm_check_if_pdn_conn_req_for_emergency_bearer_services(emm_ctrl_data_ptr->pdn_conn_req_ptr->req))
    {
...
#ifdef FEATURE_LTE_REL9
  emm_ctrl_data_ptr->reg_req_pending_ptr->lte_cs_domain_param.is_voice_domain_ie_valid = emm_ctrl_data_ptr->reg_lte_pended_cs_domain_param_ptr->is_voice_domain_ie_valid;
  emm_ctrl_data_ptr->reg_req_pending_ptr->lte_cs_domain_param.voice_domain_pref = emm_ctrl_data_ptr->reg_lte_pended_cs_domain_param_ptr->voice_domain_pref;
...
#ifdef FEATURE_NAS_DELAY_TOLERANT
  ...
  sequence_number = emm_get_nas_transaction_id() ;
   ...
   if(emm_check_if_pdn_conn_req_for_emergency_bearer_services(emm_ctrl_data_ptr->pdn_conn_req_ptr->req) == FALSE)
   {
   ...
   gmm_set_sync_pmm_state_during_w_to_lg_and_back_to_w_irat(FALSE);
#endif
   ...
   if (*emm_ctrl_data_ptr->set_drx_req_pending_ptr == NAS_DRX_CFG_REQ_PENDING)
   {
...
#ifdef FEATURE_DISABLED_HABANERO   
   emm_send_rrc_reg_status_ind(emm_ctrl_data_ptr,LTE_RRC_REG_START, (uint8)emm_ctrl_data_ptr->emm_attach_attempt_counter );
#endif
...
/*===========================================================================
FUNCTION    EMM_IGNORE_ATTACH_ACCEPT

DESCRIPTION
  This function checks if attach accept needs to be ignored based on PTI mismatch.
...
SIDE EFFECTS
  None
===========================================================================*/
boolean emm_ignore_attach_accept
(
  emm_ctrl_data_type *emm_ctrl_data_ptr
)
{
  boolean ignore_result = FALSE;
  lte_nas_incoming_msg_type *emm_message_ptr;
  lte_nas_incoming_msg_type *esm_message_ptr;
  byte  emm_index = 0;
  byte  esm_index = 1;
  ASSERT(emm_ctrl_data_ptr != NULL); 
  ...
  {
    emm_message_ptr = &(emm_ctrl_data_ptr->nas_incoming_msg_ptr[emm_index]);
    if(emm_message_ptr->emm_incoming_msg.hdr.msg_id == ATTACH_ACCEPT )
    {
      esm_message_ptr = &(emm_ctrl_data_ptr->nas_incoming_msg_ptr[esm_index]);
      if((EMM_GET_STATE() == EMM_REGISTERED_INITIATED) &&
	  	(esm_message_ptr->esm_incoming_msg.nas_hdr.msg_id == ACTIVATE_DEFAULT_EPS_BEARER_CONTEXT_REQUEST)&&
         (emm_ctrl_data_ptr->pdn_conn_req_ptr->lte_nas_esm_hdr.pti != 
         esm_message_ptr->esm_incoming_msg.
                 default_bearer_context_req.lte_nas_esm_hdr.pti))
      {
        MSG_HIGH_DS_2(MM_SUB,"ignoring attach accept for PTI mismatch sent pti %d, received %d",
        emm_ctrl_data_ptr->pdn_conn_req_ptr->lte_nas_esm_hdr.pti,
        esm_message_ptr->esm_incoming_msg.default_bearer_context_req.lte_nas_esm_hdr.pti);
        ignore_result = TRUE;
      }
    }
  }
...
/*===========================================================================
FUNCTION    emm_ignore_esm_information_request

DESCRIPTION
  This function checks if esm information request needs to be ignored based on PTI mismatch.
...
SIDE EFFECTS
  None
===========================================================================*/
boolean emm_ignore_esm_information_request
(
  emm_ctrl_data_type *emm_ctrl_data_ptr,
  lte_nas_incoming_msg_type *esm_message_ptr
)
{
  boolean ignore_result = FALSE;
  ASSERT(emm_ctrl_data_ptr != NULL); 
  ASSERT(esm_message_ptr != (lte_nas_incoming_msg_type *)NULL) ; 
  if(emm_ctrl_data_ptr->ignore_attach_pti_mismatch == TRUE)
  {
     if((esm_message_ptr->esm_incoming_msg.nas_hdr.msg_id == ESM_INFORMATION_REQUEST) &&
         (emm_ctrl_data_ptr->pdn_conn_req_ptr->lte_nas_esm_hdr.pti != 
         esm_message_ptr->esm_incoming_msg.esm_info_req.lte_nas_esm_hdr.pti))
     {
        MSG_HIGH_DS_2(MM_SUB,"ignoring esm information request PTI mismatch sent pti %d, received %d",
       emm_ctrl_data_ptr->pdn_conn_req_ptr->lte_nas_esm_hdr.pti,
       esm_message_ptr->esm_incoming_msg.esm_info_req.lte_nas_esm_hdr.pti);
       ignore_result = TRUE;
     }
...
)
{
  ...
  switch (emm_cause)
  {
...
#ifdef FEATURE_DUAL_SIM
         emm_set_illegal_non_illegal_cause(emm_cause);
#endif
       }
      break ;
    ...
    case LTE_NAS_ROAMING_NOT_ALLOWED_IN_THIS_TRACKING_AREA:
	 ...
	 if((emm_cause == LTE_NAS_ROAMING_NOT_ALLOWED_IN_THIS_TRACKING_AREA) &&
		 	 (emm_ctrl_data_ptr->emm_is_cause_12_to_13_mapped == TRUE))
		   {
             MSG_HIGH_DS_0(MM_SUB, "=EMM= NOT deleting EPLMN due to MAPPED 12 to 13 cause ");
			 /* Delete Equivalent PLMN List */
            emm_invalidate_mobility_info(EMM_STATUS_ROAMING_NOT_ALLOWED, 
                                   SYS_SIM_STATE_AVAILABLE, 
                                   emm_ctrl_data_ptr, FALSE) ;
      ...
      emm_ctrl_data_ptr->emm_is_cause_12_to_13_mapped = FALSE;
      break ;
#ifdef FEATURE_LTE_REL10
  ...
  switch (emm_cause)
  {
    case LTE_NAS_IMSI_UNKNOWN_IN_HSS:
      ...
      ** ---------------------------------------------------- */
	  ...
	  if(emm_ctrl_data_ptr->sg_association_to_be_established != NAS_EMM_SG_ASSOC_NOT_REQ)
      {
        MSG_HIGH_DS_0(MM_SUB, "=EMM= Reset SGs assoc flag - LTE_NAS_IMSI_UNKNOWN_IN_HSS");
        emm_ctrl_data_ptr->sg_association_to_be_established = NAS_EMM_SG_ASSOC_NOT_REQ; 
      }
...
)
{
  ...
  {
    case LTE_NAS_IMEI_NOT_ACCEPTED:
	  emm_failure_cause.cause.state = EMM_INVALID_STATE_IMEI_NOT_ACCEPTED_BY_NW;
	  break;
	default:
	  break;
  }
...
** ---------------------------- ---------------------------- ---------------------------- ----------------------------*/
           ...
           if((emm_is_registration_for_emc_pdn_conn(emm_ctrl_data_ptr)== TRUE) &&
              (sd_ss_current_mcc_match_mcc_list_emerg_call_full_srv(&current_lte_plmn, 
                                    (sys_modem_as_id_e_type)mm_sub_id) == TRUE))
           {
             /* Remember that attach is rejected with T3346 when emergency call was pending */
             emm_ctrl_data_ptr->t3346_started_emc_call_pending = TRUE;
           }
         }
         break;
#endif
      ...
      switch (attach_reject->emm_cause)
      {
        ...
        case LTE_NAS_PLMN_NOT_ALLOWED:
        case LTE_NAS_ROAMING_NOT_ALLOWED_IN_THIS_TRACKING_AREA:
          if((attach_reject->emm_cause == LTE_NAS_ROAMING_NOT_ALLOWED_IN_THIS_TRACKING_AREA) &&
		 	 (emm_ctrl_data_ptr->emm_is_cause_12_to_13_mapped == TRUE))
		  {
            MSG_HIGH_DS_0(MM_SUB, "=EMM= NOT deleting EPLMN due to MAPPED 12 to 13 cause ");
		  }
      ...
      if (FALSE == dos_mitigation_reqd)
      {
                ...
                (sys_plmn_match(reg_sim_read_hplmn(), current_lte_plmn))
               ...
               )
                     {
                           if(((emm_ctrl_data_ptr->emm_plmn_block_on_rej_15_8 == TRUE) &&
                              ...
                              (emm_ctrl_data_ptr->eps_storage_supported == FALSE))
   #ifdef FEATURE_LTE_REL12
                            || ((reg_sim_get_eutran_not_allowed((sys_modem_as_id_e_type)mm_sub_id) == TRUE) &&
...
#ifdef FEATURE_LTE_REL12
                             MSG_HIGH_DS_3(MM_SUB, "=EMM= Rel Ver = %d EUTRAN_SIM = %d EUTRAN_NW = %d",
                               emm_ctrl_data_ptr->emm_3gpp_spec_ver, 
                               reg_sim_get_eutran_not_allowed((sys_modem_as_id_e_type)mm_sub_id), 
                               attach_reject->eutran_not_allowed);
#endif
             break;
           default:
             /* Do nothing here for other emm causes */
         ...
         switch (attach_reject->emm_cause)
         {
           case LTE_NAS_TRACKING_AREA_NOT_ALLOWED:
             ...
             emm_add_tai_forbidden_for_service_list(emm_ctrl_data_ptr->forbidden_for_service_list_ptr,
      ...
      switch (attach_reject->emm_cause)
      {
        ...
        case LTE_NAS_NOT_AUTHORIZED_FOR_THIS_CSG:
         ...
         if ((reg_sim_get_eutran_not_allowed((sys_modem_as_id_e_type)mm_sub_id) == TRUE) &&
              (attach_reject->emm_cause == LTE_NAS_NO_SUITABLE_CELLS_IN_TRACKING_AREA) &&
               (emm_ctrl_data_ptr->emm_3gpp_spec_ver >= LTE_3GPP_REL12) &&
             (attach_reject->eutran_not_allowed == TRUE))
         {
      ...
      if(emm_is_registration_for_emc_pdn_conn(emm_ctrl_data_ptr)== TRUE)
      {
        current_lte_plmn = emm_convert_rrc_plmn_id_to_nas_plmn_id(
                            emm_ctrl_data_ptr->last_rrc_service_ind_ptr->camped_svc_info.\
                                                                      selected_plmn);
        EMM_MOVES_TO_DEREGISTERED_STATE(EMM_DEREGISTERED_LIMITED_SERVICE);
         /* Try getting full service on GW if NW rejected Attach with cause #22 and T3346 is started
          while emergency call is pending and full service is needed to make emergency call */
        if((sd_ss_current_mcc_match_mcc_list_emerg_call_full_srv(&current_lte_plmn, 
                                   (sys_modem_as_id_e_type)mm_sub_id) == TRUE) &&
           (attach_reject->emm_cause == LTE_NAS_CONGESTION) && 
...
)
{
  ...
  if ((gmm_get_map_rej_cause7_to_rej_cause14())
    && attach_reject->emm_cause == LTE_NAS_EPS_SERVICES_NOT_ALLOWED 
    && (!gmm_is_camped_on_hplmn()))
  {
    attach_reject->emm_cause = LTE_NAS_EPS_SERVICES_NOT_ALLOWED_IN_PLMN;
  }
  ...
  {
    //add current PLMN to attemped PLMN list and start timer
    emm_add_block_plmn_list_for_emc(emm_ctrl_data_ptr);
    
	// Set the EMC attach capability of cell to FALSE as current PLMN will get added to the back-off list
    emm_ctrl_data_ptr->last_rrc_service_ind_ptr->camped_svc_info.\
                                                 ims_emergency_support_enabled = FALSE;

    //update shared PLMN since last attempted one may be picked from shared PLMN list 
    emm_set_shared_emc_supported_plmn_list(emm_ctrl_data_ptr->last_rrc_service_ind_ptr);

    if(emm_emc_attach_retry_on_shared_plmn(emm_ctrl_data_ptr) == TRUE)
    {
      //EMC retry on shared PLMN
      emm_rrc_abort_connection(emm_ctrl_data_ptr, FALSE,LTE_RRC_CONN_ABORT_CAUSE_NORMAL);
      emm_ctrl_data_ptr->switch_plmn = TRUE;
    }
  }
#endif
  ...
  */
  ...
  if(attach_reject->emm_cause == LTE_NAS_TRACKING_AREA_NOT_ALLOWED &&
  	 ((emm_check_if_map_cause_12_to_13_allow(emm_ctrl_data_ptr)) == TRUE))
  {
    MSG_HIGH_DS_0(MM_SUB, "=EMM= Mapping cause 12 to 13 ");
  ...
  */
  ...
  emm_remove_release_client(emm_attach_release_indication);
#ifdef FEATURE_DISABLED_HABANERO 
  emm_send_rrc_reg_status_ind(emm_ctrl_data_ptr,LTE_RRC_REG_SUCCESS,0);
#endif
#if defined  FEATURE_DUAL_SIM || defined FEATURE_1XSRLTE
  ...
  if(emm_ctrl_data_ptr->esr_type_in_progress == NAS_EMM_3GPP_CSFB_BUFFERED)
  {
    ...
    /* CSFB was buffered progress*/
    failure_cause_type.failure_type = OTA_REJECT_CAUSE;
    failure_cause_type.failure_cause.lte_nas_ota_emm_cause = attach_reject->emm_cause;
    emm_process_extended_service_req_failure(failure_cause_type, emm_ctrl_data_ptr);
  }
#endif
...
)
{
  ...
  emm_ctrl_data_ptr->emm_emc_tai_list_ptr = NULL;
  ...
  switch(emm_attach_accept->eps_attach_result)
  {
    case COMBINED_EPS_IMSI_ATTACH_RESULT:
	  ...
	  if ( emm_ctrl_data_ptr->reg_req_pending_ptr->req_service_domain == SYS_SRV_DOMAIN_PS_ONLY )
      {
        ...
        MSG_HIGH_DS_0(MM_SUB, "=EMM= change attach accept type from combined to PS only ");
      }
      break;
  ...
  if(prev_t3402_value != emm_ctrl_data_ptr->t3402_value)
  {
   ...
   if (emm_attach_accept->emergency_num_list_exists == TRUE)
    ...
    if( emm_ctrl_data_ptr->emm_voice_domain_change_pended == NAS_NON_ACQ_REQ_PROCESSING)
    {
    ...
    emm_ctrl_data_ptr->emm_emc_tai_list_ptr = 
         (lte_nas_emm_tai_lst_info_type *) emm_modem_mem_alloc(sizeof(lte_nas_emm_tai_lst_info_type),
                                              emm_ctrl_data_ptr);
	mm_check_for_null_ptr((void*)emm_ctrl_data_ptr->emm_emc_tai_list_ptr);

	  
	  
	memscpy((void*)emm_ctrl_data_ptr->emm_emc_tai_list_ptr, 
			   sizeof(lte_nas_emm_tai_lst_info_type),
			   (void*)emm_ctrl_data_ptr->emm_tai_list_ptr,
			   sizeof(lte_nas_emm_tai_lst_info_type));

    emm_set_and_write_network_policy(emm_attach_accept->emm_network_policy_exists , emm_attach_accept->emm_unsec_redir_allowed);

    emm_send_rrc_system_update(emm_ctrl_data_ptr);   
	


	emm_modem_mem_free((void *)emm_ctrl_data_ptr->emm_emc_tai_list_ptr,
                        emm_ctrl_data_ptr);
	emm_ctrl_data_ptr->emm_emc_tai_list_ptr = NULL;
    ...
    if (*emm_ctrl_data_ptr->set_drx_req_pending_ptr == NAS_DRX_CFG_REQ_PROCESSING)
    {
...
{
  ASSERT(emm_ctrl_data_ptr != (emm_ctrl_data_type *)NULL) ; 
  if(emm_check_if_pend_conn_abort_during_rlf(emm_ctrl_data_ptr) == FALSE)
  {
...
)
{
     ...
     (emm_ctrl_data_ptr->emc_backoff_timer_value > 0))
  {
    ...
    emm_ctrl_data_ptr->switch_plmn = emm_emc_attach_retry_on_shared_plmn(emm_ctrl_data_ptr);
  }
#endif
  emm_remove_release_client(emm_attach_release_indication);
  if(emm_check_if_pend_conn_abort_during_rlf( emm_ctrl_data_ptr) == FALSE)
...
)
{
  ...
  if((emm_chk_ta_change(emm_ctrl_data_ptr->last_rrc_service_ind_ptr, 
                       emm_ctrl_data_ptr) == TRUE)||
      (reg_sim_plmn_backoff_forbidden(serving_tai.plmn,
                   (sys_modem_as_id_e_type)mm_sub_id) == TRUE))
  {
...
/*===========================================================================
FUNCTION    EMM_START_DETACH_ON_PTI_MISMATCH

DESCRIPTION
  This function starts detach procedure when there is mismatch in PTI after dual swtich
...
SIDE EFFECTS
  None
===========================================================================*/
void emm_start_detach_on_pti_mismatch
(
  emm_ctrl_data_type    *emm_ctrl_data_ptr,
  lte_nas_esm_incoming_msg_type esm_incoming_msg
)
{
  lte_nas_incoming_msg_type *esm_message_ptr = NULL;
  boolean detach_needed =FALSE;
  emm_failure_type               emm_failure_cause;
  ASSERT(emm_ctrl_data_ptr != NULL); 
  memset(&emm_failure_cause,0,sizeof(emm_failure_type));
  if(emm_ctrl_data_ptr->is_detach_req_on_pti_mismatch == TRUE)
  {
    switch (esm_incoming_msg.nas_hdr.msg_id)
    {
		case ACTIVATE_DEFAULT_EPS_BEARER_CONTEXT_REQUEST:
          if(emm_ctrl_data_ptr->dual_switch_pti == esm_incoming_msg.default_bearer_context_req.lte_nas_esm_hdr.pti)
          {
            detach_needed = TRUE;
			 MSG_HIGH_DS_2(MM_SUB,"=EMM= PTI sent before dual switch %d, PTI received from nw after dual swtich %d, same PTI, initiating mo detach", 
                             emm_ctrl_data_ptr->dual_switch_pti,
                             esm_incoming_msg.esm_info_req.lte_nas_esm_hdr.pti);
          }
		  break;
		case ESM_INFORMATION_REQUEST:
          if(emm_ctrl_data_ptr->dual_switch_pti == esm_incoming_msg.esm_info_req.lte_nas_esm_hdr.pti)
          {
            detach_needed = TRUE;
			 MSG_HIGH_DS_2(MM_SUB,"=EMM= PTI sent before dual switch %d, PTI received from nw after dual swtich %d, same PTI, initiating mo detach", 
                             emm_ctrl_data_ptr->dual_switch_pti,
                             esm_incoming_msg.default_bearer_context_req.lte_nas_esm_hdr.pti);
          }
		  break; 
		default :
			 MSG_HIGH_DS_0(MM_SUB,"=EMM= Invalid MSG_ID");
			break;
	}
	if(detach_needed == TRUE)
	{
      emm_ctrl_data_ptr->is_pti_mismatch = TRUE;
      emm_ctrl_data_ptr->dual_switch_pti = ESM_INVALID_PTI;
      emm_failure_cause.cause_type = LTE_NAS_IRAT_INVALID_STATE;
      mm_stop_timer(TIMER_T3410);
      emm_release_attach_pdn_conn_req(emm_ctrl_data_ptr, emm_failure_cause);
...
)
{
  ...
  ASSERT(emm_ctrl_data_ptr != NULL);
  detach_reason = emm_ctrl_data_ptr->detach_reason;  
  ...
  // Remember detach reason for sending GSTK IDLE event, reset this after
  // moving to dereg state and sending the event
  emm_ctrl_data_ptr->detach_reason = detach_reason;
  ...
  else
  {
    ...
    else
    {
      if(emm_ctrl_data_ptr->rrc_active == TRUE)
      {
        //Donot send reg cnf if we are in RLF as REG/CM will send a service req or we are already processing service request
        if((emm_ctrl_data_ptr->rlf_state == TRUE) ||
           ((emm_ctrl_data_ptr->reg_req_pended_indication == TRUE) &&
...
#endif
       || (emm_ctrl_data_ptr->ps_detach_info.ps_detach_req.detach_type == SYS_PS_DETACH_TYPE_POLICY_CHANGE)
     )
  {
...
)
{
  ...
  switch (emm_cause)
  {
    case LTE_NAS_TRACKING_AREA_NOT_ALLOWED:
      ...
      /* Add to frobidden tracking area list */
      emm_add_tai_forbidden_for_service_list(
...
)
{
  ...
  if ((gmm_get_map_rej_cause7_to_rej_cause14())
    ...
    && (!gmm_is_camped_on_hplmn()))
  {
    mt_detach_request->emm_cause = LTE_NAS_EPS_SERVICES_NOT_ALLOWED_IN_PLMN;
  }
...
#ifdef FEATURE_LTE_REL12
          /* 
          ** Delete the pended ESR message to not trigger the emm_extended_service_request_failed
          ** as this will trigger UE moving to GW.
          ** For IMSI Detach collision with ESR request, UE should stay in LTE, perform TAU and then
          ** do ESR after TAU is completed(REL12 - CP-130258/C1-132617)
          */
          if((emm_ctrl_data_ptr->emm_3gpp_spec_ver >= LTE_3GPP_REL12) &&
             (mt_detach_request->detach_req == IMSI_DETACH_MT_DETACH))
          {
            emm_delete_pended_message_with_msg_id(EXT_SERVICE_REQUEST,emm_ctrl_data_ptr);
        }
#endif
        }
      }
  ...
  else
  {
    ...
    emm_build_and_send_detach_ind(LTE_NAS_DETACH_WITH_REATTACH_LTE_NW_DETACH);
  }
...
FUNCTION    EMM_SEND_NAS_OTA_QEVENT

DESCRIPTION
  This function sends QEVENTs based on the scenario for NAS OTA msg
  We pass msg ID and PD 
...
{
#ifdef FEATURE_DISABLED_HABANERO
  if (msg_id == EXT_SERVICE_REQUEST)
  {
    QEVENT(
      QSH_CLT_MM, 
      MM_QSH_EVENT_NAS_OTA_MSG_ACK, 
      MM_QSH_EVENT_NAS_OTA_MSG_ACK_VERSION, 
      mm_sub_id, 
      "=EMM= ACKd PD %d ACKd MSG id %d", pd, msg_id);
  } 
#endif
}
#endif
...
)
{
  ...
  */
  ...
  {
    MSG_HIGH_DS_0(MM_SUB, "=EMM= TAU recovery for CS procedure failure");
  } 
  /*
  ...
  */
  else if((emm_ctrl_data_ptr->emm_update_status == EMM_STATUS_UPDATED) &&
...
#endif
#ifdef FEATURE_DISABLED_HABANERO 
  emm_send_rrc_reg_status_ind(emm_ctrl_data_ptr,LTE_RRC_REG_FAILED,emm_ctrl_data_ptr->emm_tau_attempt_counter);
#endif
  /* TS 24.301 5.5.3.2.6 */
		...
		if(((emm_ctrl_data_ptr->mmtel_call_status.sys_mode == SYS_SYS_MODE_LTE)&&
		   (emm_ctrl_data_ptr->mmtel_call_status.video_call_status == TRUE ||
			...
			((emm_ctrl_data_ptr->t3411_value_during_mmtel_call != 0) &&
			 (emm_ctrl_data_ptr->emm_tau_attempt_counter <= 2) &&
              (emm_is_gcf_enable() == FALSE))
			)
		{
			mm_start_timer(TIMER_T3411, (emm_ctrl_data_ptr->t3411_value_during_mmtel_call*1000)) ;
		}
		else
		{
		    mm_start_timer(TIMER_T3411, EMM_T3411_TIMER_DURATION) ;
...
) 
{
  ...
  if((emm_ctrl_data_ptr->esr_type_in_progress == NAS_EMM_1X_CSFB_BUFFERED) || (emm_ctrl_data_ptr->esr_type_in_progress == NAS_EMM_3GPP_CSFB_BUFFERED))
  {
    ...
    {
      /* CSFB was buffered progress*/
      csfb_failure_cause.failure_type = AS_REJECT_CAUSE;
      csfb_failure_cause.failure_cause.lte_nas_failure_cause = local_cause;
      emm_process_extended_service_req_failure(csfb_failure_cause,emm_ctrl_data_ptr);
    }
  }

  emm_ssgccs_send_tau_failed_ind(cause);
  ...
  if(EMM_GET_STATE() == EMM_TRACKING_AREA_UPDATING_INITIATED)
  {
    ...
    switch (cause)
    {
      ...
      case CONN_CNF_LTE_RRC_CONN_EST_FAILURE_RF_UNAVAILABLE:
        ...
        if(emm_search_tai_list(&current_tai,emm_ctrl_data_ptr->emm_tai_list_ptr)== FALSE)
        {
          ...
          if(emm_ctrl_data_ptr->plmn_service_state->ps_data_suspend == FALSE)
          {
           ...
           (emm_ctrl_data_ptr->radio_cap_update != EMM_RADIO_CAP_UPDATE_NONE))
        { 
          /* TAU due to radio cap change or coming from GW to LTE */
          emm_ctrl_data_ptr->emm_update_status = EMM_STATUS_NOT_UPDATED;
          emm_save_mobility_info(emm_ctrl_data_ptr);
          emm_ctrl_data_ptr->tau_cause = TAU_CAUSE_OTHERS;
          EMM_MOVES_TO_REGISTERED_STATE(EMM_REGISTERED_ATTEMPTING_TO_UPDATE);
          if(emm_ctrl_data_ptr->plmn_service_state->ps_data_suspend == FALSE)
          {
            ...
            mm_send_cm_ps_data_suspend_ind();
                ...
                (emm_lte_cs_domain_param_match(emm_ctrl_data_ptr->reg_req_pending_ptr->lte_cs_domain_param,emm_ctrl_data_ptr->reg_lte_cs_domain_param) == FALSE) ||
                (*emm_ctrl_data_ptr->set_drx_req_pending_ptr == NAS_DRX_CFG_REQ_PENDING) ||
                (*emm_ctrl_data_ptr->set_drx_req_pending_ptr == NAS_DRX_CFG_REQ_PROCESSING) ||
				...
				(emm_ctrl_data_ptr->emm_update_status == EMM_STATUS_ROAMING_NOT_ALLOWED))
        {
          /*UE capability change like voice domain pref change*/
          emm_ctrl_data_ptr->tau_cause = TAU_CAUSE_OTHERS;
          EMM_MOVES_TO_REGISTERED_STATE(EMM_REGISTERED_ATTEMPTING_TO_UPDATE); 
		  MSG_HIGH_DS_1(MM_SUB, "=EMM= Need to trigger TAU after radio timer exp: update status: %d",emm_ctrl_data_ptr->emm_update_status);	
        }
        else if ((emm_ctrl_data_ptr->plmn_service_state->cs_service_status == SYS_SRV_STATUS_LIMITED_REGIONAL) &&
                 (emm_ctrl_data_ptr->plmn_service_state->reg_service_domain == SYS_SRV_DOMAIN_PS_ONLY) &&
                 (emm_ctrl_data_ptr->reg_req_pending_ptr->req_service_domain == SYS_SRV_DOMAIN_CS_PS))
        {
          /*Periodic tau to update CS service status*/
          emm_ctrl_data_ptr->tau_cause = TAU_CAUSE_OTHERS;
          EMM_MOVES_TO_REGISTERED_STATE(EMM_REGISTERED_ATTEMPTING_TO_UPDATE_MM);
        }
        else if(emm_ctrl_data_ptr->emm_update_status == EMM_STATUS_UPDATED)
        {
         /* updated TA, we may need to set TAU cause to periodic here*/
          EMM_MOVES_TO_REGISTERED_STATE(EMM_REGISTERED_NORMAL_SERVICE);  
        }
        mm_stop_timer(TIMER_T3430);
        mm_start_timer(TIMER_EMM_RADIO_RETRY, DEFAULT_TIMEOUT);
      ...
      case TRANSMISSION_LTE_RRC_UL_DATA_CNF_FAILURE_CONN_REL:
        ...
        MSG_HIGH_DS_0(MM_SUB, "=EMM= Conn release before TAU UL Data CNF ");
        emm_ctrl_data_ptr->emm_update_status = EMM_STATUS_NOT_UPDATED;
        EMM_MOVES_TO_REGISTERED_STATE(EMM_REGISTERED_ATTEMPTING_TO_UPDATE);
        mm_stop_timer(TIMER_T3430);
...
)
{
  ...
  /* Get EMM state */
  emm_state = EMM_GET_STATE() ;
  emm_ssgccs_send_tau_failed_ind(release_cause);
  /* 
...
)
{
  ...
  emm_substate = EMM_GET_SUBSTATE() ;
#ifdef FEATURE_DUAL_SIM  
  if((emm_ctrl_data_ptr->tau_cause == TAU_CAUSE_OTHERS) &&
  ...
  {
    emm_ctrl_data_ptr->last_tau_type = TAU_CAUSE_OTHERS;
  }
#endif
  ...
  switch (emm_state)
  {
    ...
    case EMM_REGISTERED:
      ...
      {
		/*manual search in progress and TAU is done after conenction abort*/
        ...
        else if((emm_ctrl_data_ptr->esr_type_in_progress == 
...
)
{
  ...
  {
    MSG_HIGH_DS_0(MM_SUB, "Do not process WMS SMS info when LTE is not part of max cap");
    return;
  }
#if defined(FEATURE_DUAL_SIM) && defined (FEATURE_DUAL_DATA)
  if(emm_ctrl_data_ptr->emm_nv_force_esr_nondds == TRUE &&
     MM_IS_NON_DDS_SUB &&
     emm_is_s1_mode_only(emm_ctrl_data_ptr) == FALSE &&
     ...
     emm_ctrl_data_ptr->rrc_active == TRUE)
  {
    if (emm_ctrl_data_ptr->esr_tau_pending_conn_rel == FALSE &&
       ...
       cmd_ptr->cmd.wms_mm_sms_call_info.overall_sms_status == FALSE)
       {
         MSG_HIGH_DS_0(MM_SUB, "Set flag to start ESR + TAU, SMS end on NONDDS SUB");
         emm_ctrl_data_ptr->esr_tau_pending_conn_rel = TRUE;
       }
	else if (emm_ctrl_data_ptr->esr_tau_pending_conn_rel == TRUE &&
		EMM_GET_STATE() == EMM_SERVICE_REQUEST_INITIATED &&
        emm_ctrl_data_ptr->wms_sms_status.overall_sms_status == FALSE&&
        cmd_ptr->cmd.wms_mm_sms_call_info.overall_sms_status == TRUE)
	   {
	     /* This is a collision case, where ESR has been sent after call end, and during this ESR window, before
	        NW releases the conenction, another VoLTE call gets triggered. Abort on-going ESR & conenction.
	        This is to avoid out of sync with UE and NW. Since at this point IMS might send SIP INVITE to NW, 
	        which will fail due to the ESR that has already been sent to NW*/
...
#endif
  MSG_HIGH_DS_4(MM_SUB,
    "=EMM=sms_over_ip_status %d ,sms_over_nas_status %d ,sms_over_s102 %d overall_sms_status %d",
                ...
                cmd_ptr->cmd.wms_mm_sms_call_info.overall_sms_status);
  ...
  emm_ctrl_data_ptr->wms_sms_status.sms_over_s102_status = 
                  cmd_ptr->cmd.wms_mm_sms_call_info.sms_over_s102;
  emm_ctrl_data_ptr->wms_sms_status.overall_sms_status = 
                  cmd_ptr->cmd.wms_mm_sms_call_info.overall_sms_status;
...
)
{
  ...
  MSG_HIGH_DS_5(MM_SUB, "=EMM= emm_nv_force_esr_nondds %d ,non DDS %d ,S1 only mode %d con state %d MCC check %d",
                emm_ctrl_data_ptr->emm_nv_force_esr_nondds,
                MM_IS_NON_DDS_SUB,
                emm_is_s1_mode_only(emm_ctrl_data_ptr),
                EMM_GET_CONNECTION_STATE(),
				reg_sim_per_subs_check_mcc_same_country(
                  emm_ctrl_data_ptr->plmn_service_state->plmn,
                  (sys_modem_as_id_e_type)mm_as_id));
        ...
        (sys_modem_as_id_e_type)mm_as_id))
  {
    if (emm_ctrl_data_ptr->esr_tau_pending_conn_rel == FALSE &&
		((emm_ctrl_data_ptr->mmtel_call_status.video_call_status == TRUE &&
        cmd_ptr->cmd.cm_mm_mmtel_call_info.video_call_status == FALSE)||
        (emm_ctrl_data_ptr->mmtel_call_status.voice_call_status == TRUE &&
        cmd_ptr->cmd.cm_mm_mmtel_call_info.voice_call_status == FALSE) ||
        (emm_ctrl_data_ptr->mmtel_call_status.sms_status == TRUE &&
        cmd_ptr->cmd.cm_mm_mmtel_call_info.sms_status == FALSE)) &&
        cmd_ptr->cmd.cm_mm_mmtel_call_info.sys_mode == SYS_SYS_MODE_LTE)
    {
      MSG_HIGH_DS_0(MM_SUB, "Set flag to start ESR + TAU, VOLTE/SMS end on NONDDS SUB");
      emm_ctrl_data_ptr->esr_tau_pending_conn_rel = TRUE;
	}
	else if (emm_ctrl_data_ptr->esr_tau_pending_conn_rel == TRUE &&
		     EMM_GET_STATE() == EMM_SERVICE_REQUEST_INITIATED &&
		     ((emm_ctrl_data_ptr->mmtel_call_status.video_call_status == FALSE &&
             cmd_ptr->cmd.cm_mm_mmtel_call_info.video_call_status == TRUE)||
             (emm_ctrl_data_ptr->mmtel_call_status.voice_call_status == FALSE &&
             cmd_ptr->cmd.cm_mm_mmtel_call_info.voice_call_status == TRUE) ||
             (emm_ctrl_data_ptr->mmtel_call_status.sms_status == FALSE &&
             cmd_ptr->cmd.cm_mm_mmtel_call_info.sms_status == TRUE)) &&
             cmd_ptr->cmd.cm_mm_mmtel_call_info.sys_mode == SYS_SYS_MODE_LTE)
	{
	  /* This is a collision case, where ESR has been sent after call end, and during this ESR window, before
	        NW releases the conenction, another VoLTE call gets triggered. Abort on-going ESR & conenction.
  ...
  //on receiving MT data call, abort MPLMN search if in progress on any SUB
  if((emm_ctrl_data_ptr->force_tau_during_manual_search == TRUE) && 
    (emm_mmtel_status(emm_ctrl_data_ptr) == TRUE) && 
    (emm_check_mplmn_in_progress(emm_ctrl_data_ptr)== TRUE))
  {
...
)
{
  ...
  if((eps_active_flag == TRUE) || 
     ((ds3gpp_per_subs_is_ps_data_available((sys_modem_as_id_e_type)mm_sub_id) == TRUE) && (emm_ctrl_data_ptr->tau_pending_after_msearch_abort == FALSE))
     ...
     )
  {
...
#if defined FEATURE_DUAL_SIM && defined FEATURE_DUAL_DATA
    MSG_MED_DS_2(MM_SUB, "=EMM= second_tau_after_csfb_needed: %d local_rel_tau_after_csfb_tau %d",emm_ctrl_data_ptr->second_tau_after_csfb_needed,emm_ctrl_data_ptr->local_rel_tau_after_csfb_tau);
	if (MM_IS_NON_DDS_SUB &&
        ...
        nas_emm_tau_req->eps_update.active_flag == FALSE)
	  {
	...
	MSG_MED_DS_2(MM_SUB, "=EMM= second_tau_after_csfb_needed changed to : %d local_rel_tau_after_csfb_tau %d",emm_ctrl_data_ptr->second_tau_after_csfb_needed,emm_ctrl_data_ptr->local_rel_tau_after_csfb_tau);  
#endif
  ...
  if(emm_is_bc_status_ie_allowed(emm_ctrl_data_ptr))
  {
    ...
    emm_ctrl_data_ptr->bc_status_ie_included = TRUE;
	MSG_HIGH_DS_0(MM_SUB, "=EMM= including eps_bearer_context_status in TAU_REQ");
  }
  else
  {
    emm_ctrl_data_ptr->bc_status_ie_included = FALSE;
    nas_emm_tau_req->eps_bearer_context_status_exists = FALSE ;
	MSG_HIGH_DS_0(MM_SUB, "=EMM= Not including eps_bearer_context_status in TAU_REQ");
  }
       ...
       (emm_current_eps_ta_update == COMBINED_TA_LA_UPDATING))
    {
      if(mm_has_valid_lai() == TRUE)
      {
       ...
       else if(emm_ctrl_data_ptr->emm_3gpp_spec_ver >= LTE_3GPP_REL11)
       {
          ...
          if((emm_ctrl_data_ptr->disable_NRI == FALSE) && (nas_emm_tau_req->tmsi_based_nri.length > 0))
          {
 ...
 /* Set low priority indicator. */
  ...
  /* sending reg status indication for all TAU type  */
#ifdef FEATURE_DISABLED_HABANERO 
  emm_send_rrc_reg_status_ind(emm_ctrl_data_ptr,LTE_RRC_REG_START,(uint8)emm_ctrl_data_ptr->emm_tau_attempt_counter);
#endif 
                                ...
                                emm_ctrl_data_ptr))
  {
    ...
    if(nas_emm_tau_req->eps_update.active_flag == FALSE) 
    {
      emm_ctrl_data_ptr->start_t3440_on_accept = TRUE;
    }
    if (FALSE == emm_connection_setup(conn_est_cause, 
  ...
  {
    emm_send_rrc_radio_cap_tau_ind();
    MSG_HIGH_DS_2(MM_SUB, "=EMM= RA Capability Timer  = %d update status = %d ", emm_ctrl_data_ptr->radio_nv_cap_enquiry_timer, emm_ctrl_data_ptr->radio_cap_update);

    if ((emm_ctrl_data_ptr->radio_nv_cap_enquiry_timer != 0) &&
        (emm_bit_mask_check(emm_ctrl_data_ptr->reg_req_pending_ptr->additional_info,
                                (byte)SYS_ADDTNL_INFO_FORCE_RADIO_CAP_UPDATE)))
    {
      mm_start_timer(TIMER_RADIO_CAP_ENQ, emm_ctrl_data_ptr->radio_nv_cap_enquiry_timer) ;
    }
  }
  ...
  switch(emm_tau_accept_ptr->eps_update_result)
  {
    ...
    case TA_LA_UPDATE_ISR_ACTIVATE:
         ...
         (emm_ctrl_data_ptr->reg_req_pending_ptr->req_service_domain == SYS_SRV_DOMAIN_PS_ONLY ))
      {
        ...
        MSG_HIGH_DS_2(MM_SUB, "=EMM= Ongoing TAU Cause = %d TAU EPS update result change to = %d ",
                              emm_ctrl_data_ptr->emm_ongoing_eps_ta_update,
  ...
  if(prev_t3402_value != emm_ctrl_data_ptr->t3402_value)
  {
    ...
    if((emm_ctrl_data_ptr->emm_3gpp_spec_ver >= LTE_3GPP_REL12) &&
       ((emm_ctrl_data_ptr->emc_srv_status == LTE_NAS_EMC_SRV_NONE)
        ...
        (emm_ctrl_data_ptr->emc_srv_status == LTE_NAS_EMC_SRV_PENDING)
        ||
        /* If UE has both emergency and non-emergency PDN(s) existing and NW indicated that no non-emergency PDN(s)
           exists at NW, then ESM will locally deactivate all non-emergency PDN(s) and stay attached for emergency
           services only upon processing NW bearer status. In that case since emc_srv_status in EMM will be updated
           to EMC_SRV_ONLY after processing TAU Accept, we need to estimate in advance if the UE will eventually be
           attached for emergency bearer services only and not remove the forbidden PLMN from FPLMN list if UE remains
           attached for emergency bearer services only. For this estimation we maintain a bitmap of non-emergency
           default bearers in the UE and using bitwise AND operation with the bitmap of NW bearer context in TAU Accept
           check if NW has any non-emergency PDN(s) existing */  
        ((emm_ctrl_data_ptr->emc_srv_status == LTE_NAS_EMC_SRV_EXIST) &&
         ((emm_tau_accept_ptr->eps_bearer_context_status_exists == FALSE)
          ||
          ((emm_tau_accept_ptr->nw_bc_bm & emm_ctrl_data_ptr->eps_bearer_context_status.non_emc_default_bc_bm) > 0)))))
    {
      /* Remove the registered PLMN from FPLMN lists if necessary for non emc attach*/
      emm_remove_forbidden_plmn(emm_ctrl_data_ptr->last_registered_tai.plmn,
                              emm_ctrl_data_ptr);
    ...
    */
    ...
    if((emm_ctrl_data_ptr->skip_bc_status_ie == TRUE)&&
       (emm_ctrl_data_ptr->bc_status_ie_included == FALSE))
    {
       MSG_HIGH_DS_0(MM_SUB,"=EMM= Skip EPS bearer update");
    }
    else if(emm_tau_accept_ptr->eps_bearer_context_status_exists == TRUE)
    {
    ...
    */
    ...
    if (*emm_ctrl_data_ptr->set_drx_req_pending_ptr == NAS_DRX_CFG_REQ_PROCESSING)
    {
      if (emm_ctrl_data_ptr->emm_rat_drx_param_to_be_sent == NAS_SEND_DRX_ON_ALL_RATS)
      {
        mm_send_mmr_set_drx_cnf(TRUE);
      }
      /* Send DRX update to W-RRC */
   ...
   emm_set_and_write_network_policy(emm_tau_accept_ptr->emm_network_policy_exists, emm_tau_accept_ptr->emm_unsec_redir_allowed);

    /* Inform RRC regarding change of registration info */
    emm_send_rrc_system_update(emm_ctrl_data_ptr) ;
	
   if( emm_ctrl_data_ptr->emm_emc_tai_list_ptr != NULL)
   {
	 emm_modem_mem_free((void *)emm_ctrl_data_ptr->emm_emc_tai_list_ptr,
                        emm_ctrl_data_ptr);
	 emm_ctrl_data_ptr->emm_emc_tai_list_ptr = NULL;
...
)
{
  ...
  emm_ctrl_data_ptr->is_periodic_tau_required = FALSE;
  if ((gmm_get_map_rej_cause7_to_rej_cause14()) 
    && tau_rej_ptr->emm_cause == LTE_NAS_EPS_SERVICES_NOT_ALLOWED 
    && (!gmm_is_camped_on_hplmn()))
  {
    tau_rej_ptr->emm_cause = LTE_NAS_EPS_SERVICES_NOT_ALLOWED_IN_PLMN;
  }
  ...
  {
	  MSG_HIGH_DS_0(MM_SUB, "=EMM= Mapping cause 12 to 13 ");
	  tau_rej_ptr->emm_cause = LTE_NAS_ROAMING_NOT_ALLOWED_IN_THIS_TRACKING_AREA;
	  emm_ctrl_data_ptr->emm_is_cause_12_to_13_mapped = TRUE;
  }
  emm_cancel_authentication();
...
)
{
  ...
  if((emm_ctrl_data_ptr->plmn_service_state->reg_service_domain == SYS_SRV_DOMAIN_CS_PS)&&
      ((emm_ctrl_data_ptr->plmn_service_state->cs_service_status == SYS_SRV_STATUS_LIMITED_REGIONAL)||
       (emm_ctrl_data_ptr->plmn_service_state->cs_service_status == SYS_SRV_STATUS_LIMITED))&&
      (emm_ctrl_data_ptr->plmn_service_state->service_status == SYS_SRV_STATUS_SRV))
  {
    emm_ctrl_data_ptr->plmn_service_state->cs_service_status = SYS_SRV_STATUS_SRV;
  }
...
)
{
  ...
  else
  {
     ...
     emm_clear_forbidden_for_service_list(emm_ctrl_data_ptr->forbidden_for_service_list_ptr, FALSE);
...
)
{
  ...
  else if(update_ftai_list_ptr->ftai_list_action == SYS_RPM_FTAI_LIST_READ)
  {
    ...
    lte_nas_tai_lst1_type tai;
    count = 0;
    status = mcfg_fs_read("/nv/item_files/modem/mmode/lte_rpm_info_2", 
                        &rpm_info, 
...
{
  ASSERT(emm_ctrl_data_ptr != (emm_ctrl_data_type *)NULL) ; 
  MSG_HIGH_DS_1(MM_SUB, "=EMM= Received cmd from CM to trigger TAU for dual RX CSFB: TAU needed flag: %d", emm_ctrl_data_ptr->lte_suspend_tau_needed);

  if(emm_ctrl_data_ptr->lte_suspend_tau_needed == TRUE)
  {
    emm_initiate_tau_procedure_if_needed(emm_ctrl_data_ptr);
...
/*===========================================================================
FUNCTION  emm_process_tau_accept_in_emm_registered_state

DESCRIPTION: 
 
This function handles TAU ACCEPT received in EMM REGISTERED state
...
SIDE EFFECTS
  None
===========================================================================*/
void emm_process_tau_accept_in_emm_registered_state
(
  lte_nas_emm_tau_accept_type *emm_tau_accept_ptr,
  emm_ctrl_data_type          *emm_ctrl_data_ptr
)
{
  ...
  /* If MME retransmits TAU ACCEPT due to failure to receive TAU COMPLETE from UE, UE might receive the retransmitted
     TAU ACCEPT in EMM REGISTERED state. In that case send EMM Status if:
      GUTI IE exists in TAU ACCEPT but received GUTI doesn't match with the stored GUTI or 
      MS Identity IE exists in TAU ACCEPT but received MS Identity doesn't match with the stored MS Identity. 
     Send TAU Complete if either GUTI IE or MS Identity IE exists in TAU ACCEPT.
     Send EMM Status if GUTI and MS Identity IEs don't exist in TAU ACCEPT */

  if(((emm_tau_accept_ptr->guti_type_exists == TRUE) &&
      (memcmp((void*)&emm_ctrl_data_ptr->emm_guti,
            (void*)&emm_tau_accept_ptr->guti,
            sizeof(lte_nas_emm_guti_type)) != 0))
#ifdef FEATURE_LTE_REL11
    ||((emm_tau_accept_ptr->ms_id_exists == TRUE)
       &&
       ((emm_ctrl_data_ptr->emm_3gpp_spec_ver < LTE_3GPP_REL11) ||
        (emm_tau_accept_ptr->ms_id.id_type == (byte)LTE_NAS_EMM_TMSI))
       &&
       (mm_is_ms_id_changed(&emm_tau_accept_ptr->ms_id) == TRUE))
#else
    ||
     ((emm_tau_accept_ptr->ms_id_exists == TRUE) &&
     (mm_is_ms_id_changed(&emm_tau_accept_ptr->ms_id) == TRUE))
  ...
  else  if((emm_tau_accept_ptr->guti_type_exists == TRUE) ||
           (emm_tau_accept_ptr->ms_id_exists == TRUE))
  {
    dword sequence_number = 0;
...
} /* end emm_process_tau_accept_in_emm_registered_state() */
...
)
{
   ...
   *emm_ctrl_data_ptr->is_sim_to_be_cs_invalid_after_power_off = FALSE;
   *emm_ctrl_data_ptr->is_sim_to_be_ps_invalid_after_power_off = FALSE;
   ...
   */
   ...
   /* clear FLAIs if G/W/T is supported on the sub */
...
FUNCTION    EMM_PROCESS_CM_LTE_SUSPEND_REQ

DESCRIPTION
  This function is used to send ELS RRC connection release request to network
  on receiving MO CS or VoLTE call on non-DDS sub.

DEPENDENCIES
  CM should send this command only on DDS sub
...
===========================================================================*/
void  emm_process_cm_lte_suspend_req
(
  emm_ctrl_data_type   *emm_ctrl_data_ptr
)
{
  ...
  /* 
    Send ELS RRC connection release request to RRC only if: 
      a. CM cmd is received on DDS sub and
      b. Sub is ELS capable and is not in idle mode
   
    Loopback cnf to CM if:
      a. CM cmd is received on DDS sub and
      b. Sub is idle or sub is not ELS capable
  */

  if (emm_check_if_els_conn_abort_possible(emm_ctrl_data_ptr) == FALSE)
  {
    /* Loopback cnf to CM */
    emm_send_cm_lte_suspend_cnf();
    return;
  }

  /* Send ELS connection abort to RRC */
  emm_send_rrc_els_conn_abort_req(emm_ctrl_data_ptr, LTE_RRC_ELS_CONN_ABORT_CAUSE_MME_SUSPEND);
  emm_ctrl_data_ptr->emm_els_conn_abort_reason = ELS_ABORT_ON_MO_CALL;

  /*Pend the TAU procedure until LTE_RESUME_IND received from CM*/
  emm_ctrl_data_ptr->lte_suspend_tau_needed = TRUE;
}

/*===========================================================================

FUNCTION    EMM_PROCESS_CM_LTE_RESUME_IND

DESCRIPTION
  This function is used by CM to indicate end of MO call on non-DDS sub and
  for NAS to initiate TAU procedure on DDS sub
...
===========================================================================*/
void  emm_process_cm_lte_resume_ind
(
  emm_ctrl_data_type   *emm_ctrl_data_ptr
)
{
  ASSERT(emm_ctrl_data_ptr != (emm_ctrl_data_type *)NULL) ; 
  MSG_HIGH_DS_0(MM_SUB, "=EMM= Received LTE_RESUME_IND cmd from CM");
...
FUNCTION  emm_timer_els_conn_rel_expire

DESCRIPTION
  This function processes ELS connection release timer expiry.
...
===========================================================================*/
void emm_timer_els_conn_rel_expire
(
  emm_ctrl_data_type *emm_ctrl_data_ptr
)
{
   ...
   MSG_HIGH_DS_0(MM_SUB, "=EMM= Processing ELS connection release timer expiry");
   mm_timer_status[TIMER_ELS_CONN_REL] = TIMER_STOPPED;

   /* check reason for ELS connection abort
        - MPLMN search: send RRC connection abort request.
        - MO call: ELS connection abort happened due to CM_SUSPEND_REQ. Send CNF to CM. 
   */
    emm_update_lte_process_type(emm_ctrl_data_ptr);
   if (emm_ctrl_data_ptr->emm_els_conn_abort_reason == ELS_ABORT_ON_MO_CALL)
   {
      /* Timer expired and REL_IND not received from RRC. Loopback cnf to CM. */
      emm_send_cm_lte_suspend_cnf();

      /* Set TAU pending flag so that TAU is triggered after call end on other sub. */
      emm_ctrl_data_ptr->lte_suspend_tau_needed = TRUE;
      //emm_ctrl_data_ptr->emm_update_status = EMM_STATUS_NOT_UPDATED;
      emm_ctrl_data_ptr->tau_cause = TAU_CAUSE_OTHERS;
   }
   else if (emm_ctrl_data_ptr->emm_els_conn_abort_reason == ELS_ABORT_ON_MPLMN_SRCH)
   {
     MSG_HIGH_DS_0(MM_SUB, "=EMM= No ELS connection release received. Send RRC connection abort for manual plmn serach request");
     emm_rrc_abort_connection(emm_ctrl_data_ptr, FALSE,LTE_RRC_CONN_ABORT_CAUSE_NORMAL) ;
   }

   emm_ctrl_data_ptr->emm_els_conn_abort_reason = ELS_ABORT_NO_CAUSE;
}

#endif

void emm_timer_hold_trm_local_rel_expiry
(
  emm_ctrl_data_type *emm_ctrl_data_ptr
)
{
  ASSERT(emm_ctrl_data_ptr != (emm_ctrl_data_type *)NULL);
  MSG_HIGH_DS_0(MM_SUB, "=EMM= Processing TIMER_HOLD_TRM_LOCAL_REL expiry");
  mm_timer_status[TIMER_HOLD_TRM_LOCAL_REL] = TIMER_STOPPED;
#if defined  FEATURE_DUAL_SIM || defined FEATURE_1XSRLTE
  emm_update_lte_process_type(emm_ctrl_data_ptr);
#endif 
...
FUNCTION  emm_timer_drx_prohibit_expire

DESCRIPTION
  This function processes DRX prohibit timer expiry.
...
void emm_timer_drx_prohibit_expire
(
  emm_ctrl_data_type *emm_ctrl_data_ptr
)
{
  ...
  sys_plmn_id_s_type     rrc_reported_plmn ;
  lte_nas_tai_lst1_type  tai;
  byte current_client_id = (byte)(gmm_client_req_drx_coeff.client_id & 0x0F);
  sys_drx_cn_coefficient_s1_e_type prev_drx = emm_get_gmm_drx_cn_coff_s1();
  ...
  if((emm_ctrl_data_ptr->rrc_active == FALSE) 
     ...
     (emm_ctrl_data_ptr->reg_req_pended_indication == TRUE))
  {
    MSG_HIGH_DS_3(MM_SUB, "=EMM= DS Prohibit timer expiry. rrc active %d svc_status %d reg_pended %d",
                  emm_ctrl_data_ptr->rrc_active,
                  emm_ctrl_data_ptr->last_rrc_service_ind_ptr->svc_status,
                  emm_ctrl_data_ptr->reg_req_pended_indication);
    return;
  ...
  if (current_client_id & (1<<(byte)MM_CLIENT_TYPE_DS))
  {
    ...
    /* if reg is allowed start attach/tau*/
    if((is_reg_allowed == TRUE) &&
       (prev_drx != emm_get_gmm_drx_cn_coff_s1()))
    {
      if((emm_state == EMM_REGISTERED && 
    ...
    {
      MSG_HIGH_DS_0(MM_SUB, "=EMM= Prohibit timer expiry registraion not allowed/DRX value is same");
    }
  }
} /* end of emm_timer_drx_prohibit_expire() */

void emm_timer_local_rel_tau_during_msearch_expiry
(
  emm_ctrl_data_type *emm_ctrl_data_ptr
)
{
  ASSERT(emm_ctrl_data_ptr != (emm_ctrl_data_type *)NULL);
  MSG_HIGH_DS_0(MM_SUB, "=EMM= Processing TIMER_LOCAL_REL_MSEARCH expiry");
  mm_timer_status[TIMER_LOCAL_REL_MSEARCH] = TIMER_STOPPED;
    ...
    if(( EMM_GET_CONNECTION_STATE() == EMM_CONNECTED_STATE )&&
       ...
       (EMM_GET_STATE() == EMM_DEREGISTERED)))
      {
...
#endif
        {
          MSG_HIGH_DS_0(MM_SUB, "=EMM= Abort RRC connection for manual plmn search Progress");
          emm_rrc_abort_connection(emm_ctrl_data_ptr, FALSE,LTE_RRC_CONN_ABORT_CAUSE_NORMAL) ;
        }
        if(emm_ctrl_data_ptr->force_tau_during_manual_search == TRUE &&
           ...
           emm_ctrl_data_ptr->local_rel_plus_tau_during_msearch_counter < 2)
        {
		  ...
		  emm_ctrl_data_ptr->local_rel_plus_tau_during_msearch_counter++;
        }
      }
...
FUNCTION  emm_timer_radio_cap_enquiry_expiry

DESCRIPTION
  This function handles timer expiry when NAS will indicate MMODE to trigger detach. 
...
void emm_timer_radio_cap_enquiry_expiry
(
  emm_ctrl_data_type *emm_ctrl_data_ptr
)
{
   ...
   if (is_lte_mode())
   {
     MSG_HIGH_DS_0(MM_SUB, "=EMM= Processing Radio capability enquiry timer expiry");
     mm_timer_status[TIMER_RADIO_CAP_ENQ] = TIMER_STOPPED;
     if ((mm_csfb_state == MM_CSFB_STATE_MT_CALL_ORIG) || 
         (mm_csfb_state == MM_CSFB_STATE_MO_CALL_ORIG))
     {
       /* When timer expires during ongoine ESR, Restart timer for 5 secs to delay detach */
       mm_start_timer(TIMER_RADIO_CAP_ENQ, 5000) ;
     }
     else if(emm_ctrl_data_ptr->radio_nv_cap_enquiry_timer_expiry_ignore == TRUE)
     {
       MSG_HIGH_DS_0(MM_SUB, "=EMM= ignore the timer expiry");
     }
     else
     {
       /* When timer expires, NAS will indicate MMODE to trigger forced detach*/
