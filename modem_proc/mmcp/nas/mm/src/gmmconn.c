...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/mm/src/gmmconn.c#1 $ $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...
boolean gmm_check_if_stack_deact_in_progess(void);
      ...
      switch (message->cmd.hdr.message_id)
      {
        ...
        case RRC_CLOSE_SESSION_CNF:
          ...
          ** ----------------------------------------------------- */
          ...
          if (gmm_periodic_rau_pending && (mm_deep_sleep_pending == FALSE))
          {
            /* ----------------------------------------------------------------------
        ...
        case MM_DATA_IND:
          switch (message->cmd.mm_data_ind.L3_data[1])
          {
            ...
            case GMM_SERVICE_REJECT:
              if (gmm_valid_service_reject_message( message, &service_reject_msg ))
              {
                ...
                gmm_qsh_ota_metric_signaling_log(GMM_SERVICE_REJECT,CN_TO_UE_DIRECTION,
                                         0xff, (mm_gmm_reject_cause_debug_type)service_reject_msg.gmm_cause);
#endif
                if (gmm_state == GMM_SERVICE_REQUEST_INITIATED)
                {
                  if ((gmm_get_map_rej_cause7_to_rej_cause14())
                       && service_reject_msg.gmm_cause == GPRS_SERVICES_NOT_ALLOWED 
                       && (!gmm_is_camped_on_hplmn()))
                  {
                    service_reject_msg.gmm_cause = GPRS_SERVICES_NOT_ALLOWED_IN_THIS_PLMN;
                  }
                  /* --------------------------------------------------------------
                  ...
                  ** -------------------------------------------------------------- */
...
#ifdef FEATURE_NAS_REL12_SPEC_CHANGE
                  /* Spec 24.008, Section 4.7.13.4, Spec CR 2488
                   ...
                   * shall reset the service request attempt counter, shall stop the timer T3317.
                   */
                  gprs_service_attempt_counter = 0;
                  MSG_HIGH_DS_0( MM_SUB, "=MM= gprs_service_attempt_counter is reset");
#endif
                  ...
                  gmm_service_request_initiator = NULL;
	                ...
	                gmm_reject_cause = service_reject_msg.gmm_cause; 
                  ...
                  ** --------------------------------------------------------- */
                  switch (service_reject_msg.gmm_cause)
                  {
                    ...
                    gprs_service_attempt_counter = 0;
#endif
                  if (gmm_is_dos_mitigation_required_for_ps_domain(GMM_SERVICE_REJECT,service_reject_msg.gmm_cause))
                  {
                     gmm_perform_dos_mitigation_action(GMM_SERVICE_REJECT,service_reject_msg.gmm_cause);
                  }
                  else
                  {
...
#ifdef FEATURE_DUAL_SIM
                      mm_gmm_set_illegal_non_illegal_cause(service_reject_msg.gmm_cause);
#endif
                  ...
                  if (mm_get_t3247_behavior_enabled() && gmm_integrity_protection_activated == FALSE)
                  {
                    ...
                    if (mm_get_t3247_behavior_enabled() && gmm_integrity_protection_activated == FALSE)
                    {
                      ...
                      **************************************************************/
                      ...
                      if ((service_reject_msg.gmm_cause == PLMN_NOT_ALLOWED) &&
                          (mm_get_t3247_behavior_enabled()&& gmm_integrity_protection_activated == FALSE) && 
                          (
#ifdef FEATURE_DUAL_SIM
         ...
         (sys_plmn_match(reg_sim_read_hplmn(), mm_serving_plmn.info.plmn))
                    ...
                    {
                       mmr_service_ind.cause  = MMR_CAUSE_FAILURE_LAI_NOT_ALLOWED;//guard with integrity protection
                    }
                     else
                     {
                        mmr_service_ind.cause = mm_convert_to_reg_cause(
      ...
      switch (message->cmd.timer_expiry.timer_id)
      {
        case TIMER_T3317:
          ...
          ** --------------------------------------- */
          ...
          /* Spec 24.008, Section 4.7.13.5, Spec CRs 2566, 2937
           *
           *   c) T3317 expired
           *
           *   If the MS is in PMM-IDLE mode then the MS shall increment the service request 
           *   attempt counter, abort the procedure and ...
           */

          if((mm_is_rrc_access_class_11_to_15() == FALSE) && (++gprs_service_attempt_counter >= NAS_THRESHOLD_BACKOFF_SERVICE_ATTEMPTS))
          {
            MSG_HIGH_DS_0( MM_SUB, "=MM= gprs_service_attempt_counter reaches max, TIMER_T3325 is started");
            mm_start_timer(TIMER_T3325, mm_emm_service_req_backoff_timer_value);
            
          }
          else
          {
            MSG_HIGH_DS_1( MM_SUB, "=MM= gprs_service_attempt_counter = %d", gprs_service_attempt_counter);
          }
#endif
          break;
         ...
         * On expiry of this TIMER_T3325, reset the service attempt counter
         */
        case TIMER_T3325:
          {
            MSG_HIGH_DS_0( MM_SUB, "=MM= TIMER_T3325 expired");
            gprs_service_attempt_counter = 0; /* todo! ask for spec ENH */
          }
          break;
#endif
  ...
  ** --------------------------------------------------------------------------- */
      ...
      (gmm_state == GMM_SERVICE_REQUEST_INITIATED))
  {
    if (message->cmd.rrc_page_ind.page_ue_id == RRC_PAGE_WITH_PTMSI)
    {
	  ...
	  {
		MSG_HIGH_DS_1( MM_SUB, "=MM= Ignoring MT PS Page in GMM Substate - %d",gmm_substate);  
	  }
      /* -----------------------------------------------------------------------------
      ...
      else if (gmm_state == GMM_REGISTERED && pmm_mode == PMM_IDLE)
      {
        ...
        ** ------------------------------------------------------------------------- */
           ...
           )
        {
	      ...
	      {
	        MSG_HIGH_DS_0( MM_SUB, "=MM= Ignoring MT PS Page On NON DDS sub "); 
	      }
#endif
#ifdef FEATURE_NAS_REL10
...

          {
               ...
               (gmm_substate == GMM_ATTEMPTING_TO_UPDATE))
            {
              ...
              /*  Spec 24.008, Spec CR 2538
               *  If the RAU request is as a paging response, the UE sets the RRC establishment cause 
               *  according to the paging cause.
               */

              /* Setting RRC connection cause for following RAU Req as per above spec ref */

              gmm_rrc_establish_cause = establish_cause;
#endif
#ifdef FEATURE_DUAL_SIM
              if ((message->cmd.rrc_page_ind.paging_cause == RRC_PAGE_CAUSE_SMS) ||
                  (message->cmd.rrc_page_ind.paging_cause == RRC_PAGE_CAUSE_LOW_PRI_SIGNALLING))
              {
...
#ifdef FEATURE_NAS_REL12_SPEC_CHANGE
               /* Re-setting RRC connection cause after above RRC conn req for above RAU is sent */
...
boolean gmm_reestablish_connection( mm_cmd_type *message )
{
  ...
  if (mm_ps_domain_possible_reest &&
      (!((gmm_ra_update_pending || gmm_periodic_rau_pending) &&
         ((gmm_state == GMM_ROUTING_AREA_UPDATING_INITIATED) ||
          (gmm_state == GMM_SERVICE_REQUEST_INITIATED)))) &&
    ...
    if (pending_gmm_msg != NULL)
    {
      ...
      switch (pending_gmm_msg_id)
      {
        ...
        case GMM_DETACH_REQUEST :
          ...
          mm_proc_status_set(SYS_PROC_TYPE_PS_REGISTRATION,TRUE);
#endif
           /*. 4.7.4.1.1 If the detach type information element value indicates "IMSI Detach" the MS shall 
               enter GMM-REGISTERED.IMSI-DETACH_INITIATED, otherwise the MS shall enter the state GMM-DEREGISTERED-INITIATED*/
          if ((gmm_detach_type == GMM_MO_IMSI_DETACH) && (gmm_state != GMM_DEREGISTERED_INITIATED))
          {
            gmm_set_state( GMM_REGISTERED );
...
#ifdef FEATURE_DUAL_SIM
                (rabm_is_rab_active((sys_modem_as_id_e_type)mm_sub_id) == TRUE) ||
#else
                (rabm_is_rab_active() == TRUE) ||
#endif 
                (gmm_state == GMM_SERVICE_REQUEST_INITIATED) ||
                (gmm_state == GMM_ROUTING_AREA_UPDATING_INITIATED) ||
                (gmm_state == GMM_DEREGISTERED_INITIATED) ||
...
** ---------------------------- ----- ---------------------------- ----- ---------------------------- -----*/
              ...
              ** ----------------------------------------------------------*/
                      ...
                      (mm_timer_status[TIMER_T3311] == TIMER_ACTIVE) && (emm_ctrl_data_ptr->TIN == NAS_MM_TIN_GUTI))
              {
				  ...
				  {
				    MSG_HIGH_DS_0( MM_SUB, "=MM= Sending GMMSM_ESTABLISH_REJ as the opmode is CS only");
				    gmm_send_gmmsm_establish_rej(mm_convert_gmm_to_sm_cause(PROTOCOL_ERROR_UNSPECIFIED));
...
|| 
              ...
              {
                MSG_HIGH_DS_1( MM_SUB, "=MM= Sending GMMSM_ESTABLISH_REJ : IRAT inprogress %d", mm_interrat_state);
                gmm_send_gmmsm_establish_rej(mm_convert_gmm_to_sm_cause(PROTOCOL_ERROR_UNSPECIFIED));
              }
#ifdef FEATURE_NAS_REL12_SPEC_CHANGE
              ...
              else if(mm_timer_status[TIMER_T3325] != TIMER_STOPPED)
              {
                MSG_HIGH_DS_0( MM_SUB, "=MM= Sending GMMSM_ESTABLISH_REJ due to TIMER_T3325");
                gmm_send_gmmsm_establish_rej(mm_convert_gmm_to_sm_cause(PROTOCOL_ERROR_UNSPECIFIED));
              }
#endif
              else if (pmm_mode == PMM_CONNECTED)
...
** ---------------------------- ----- ---------------------------- ----- ---------------------------- -----*/
                    ...
                    )
                {
                  ...
                  **************************************************************************/
				  ...
				  {
				    MSG_HIGH_DS_0( MM_SUB, "=MM= Sending GMMSM_ESTABLISH_REJ as the Request is received on non DDS Sub");
				    gmm_send_gmmsm_establish_rej(mm_convert_gmm_to_sm_cause(PROTOCOL_ERROR_UNSPECIFIED));
...
#endif
          else if (gmm_check_if_stack_deact_in_progess()== TRUE)
          {
             MSG_HIGH_DS_0( MM_SUB, "=MM= Sending GMMSM_ESTABLISH_REJ, as stack deact in progress with no ps resumption(cause)");
			 gmm_send_gmmsm_establish_rej(PDP_LOWERLAYER_ERROR);
          }
          else
          {
        ...
        case GMMSM_PS_CALL_STATUS_IND:
          if(message->cmd.gmmsm_ps_call_status_ind.proc_type == SYS_PROC_TYPE_HIGH_PRIORITY_PS_CALL)
          {
            mm_proc_status_set(message->cmd.gmmsm_ps_call_status_ind.proc_type,
                                              message->cmd.gmmsm_ps_call_status_ind.pdp_sig_in_progress);
          }
          else
          {
            gmm_ps_call_in_progess = 
              message->cmd.gmmsm_ps_call_status_ind.pdp_sig_in_progress;
            if((gmm_state != GMM_SERVICE_REQUEST_INITIATED) &&
    ...
    case MS_GMM_TC:
      if (message->cmd.hdr.message_id == GMMTC_UNITDATA_REQ)
      {
        ...
        if ((mm_serving_plmn.gmm_service_state == SERVICE_AVAILABLE) &&
            ((pmm_mode == PMM_CONNECTED) || ((gmm_state == GMM_SERVICE_REQUEST_INITIATED) && (ps_session_status == MMRRC_ACTIVE))))
        {
          /* -------------------------------------
      ...
      switch (message->cmd.hdr.message_id)
      {
        ...
        else if(mm_is_mplmn_search_in_progress() == TRUE)
        {
          ...
          else if(mm_timer_status[TIMER_T3325] != TIMER_STOPPED)
          {
            ...
            rabm_cmd->cmd.gmm_reestablish_rej.as_id = (sys_modem_as_id_e_type)mm_sub_id;
#endif
            MSG_HIGH_DS_0( MM_SUB, "=MM= Sending GMMRABM_REESTABLISH_REJ due to TIMER_T3325" );
            rabm_put_cmd( rabm_cmd );
            /*adding message to the debug buffer*/
            mm_add_message_to_debug_buffer(rabm_cmd->header.message_set, (dword)rabm_cmd->header.cmd_id, FROM_MM
                   ,(sys_modem_as_id_e_type)mm_sub_id);
        ...
        case GMMRABM_RAB_REL_IND:
          ...
          ** spec 24.008 4.7.1.9: 
          ** In case b and c,
          **  -upon an indication from the lower layers that radio accesss bearer(s) is set up, 
          **   the MS shall stop timer T3340 and may send uplink signalling via the existing PS 
          **   signalling connection or user data via radio access bearer(s) 
          **----------------------------------------------------------------------------------------------*/
#ifdef FEATURE_NAS_REL12
...
/* 24.301 5.5.3.3.2 d) */
             ...
             ||((emm_ctrl_data_ptr->emm_3gpp_spec_ver >= LTE_3GPP_REL12) && (gmm_state == GMM_REGISTERED_INITIATED)&&(gmm_attach_type == GMM_COMBINED_GPRS_IMSI_ATTACH)) 
#endif
             )
               ...
               )
             {
                ...
                MSG_HIGH_DS_0(MM_SUB,"=MM= Set SGs association flag - On Open Session cnf on RRC for NMO1");
             }
#endif
            if (gmm_integrity_protection_activated)
            {
        ...
        case RRC_REL_IND:
          ...
          mm_proc_status_set(SYS_PROC_TYPE_PS_CALL_SIGNALING, FALSE);
          mm_proc_status_set(SYS_PROC_TYPE_TEST_LOOPBACK, FALSE);
#endif
          /* ------------------------------------------------------------------------
...
** ---------------------------- ---------------------------- ---------------------------- ----------------------------*/
                   ...
                   )
                {
                  ...
                  ** -------------------------------------------------------------------- */
                  ...
                  else
                  {
                    ...
                    if(mm_waiting_for_stop_mode_cnf && mm_deep_sleep_pending)
                    {
                      MSG_HIGH_DS_0( MM_SUB, "=MM= DIRECT SIGNALLING RE_ESTABLISHEMENT - RAU Pended");
                      gmm_rau_is_required_due_to_dsr = TRUE;
                    }
                    else
                    {
...
#endif
                      || ((mm_stop_mode_req_pended == TRUE) && (mm_timer_status[ TIMER_MM_REL ] == TIMER_ACTIVE))
                      || (mm_deep_sleep_pending == TRUE)
                    )
                  {
                    MSG_HIGH_DS_3( MM_SUB, "=MM= RAU not triggered as T3346 is running = %d OR Reg request is Pending for MM: %d OR STOP MODE REQ is Pended: %d otherwise for EMM",
                                                   mm_timer_status[TIMER_T3346], mmcoord_reg_req_pending, mm_deep_sleep_pending);
                  }
                  else if ((mm_state != MM_IDLE ) &&
                           ((gmm_pending_ra_update_type == GMM_COMBINED_RA_LA_UPDATING) ||
                            (gmm_pending_ra_update_type == GMM_COMBINED_RA_LA_UPDATING_WITH_IMSI_ATTACH)) &&
		...
		else if (gmm_periodic_rau_pending && gmm_state == GMM_REGISTERED && 
			!((gmm_rai_changed( gmm_previous_registration_serving_rai.location_area_code,
                           gmm_previous_registration_serving_rai.routing_area_code,
                           mm_serving_plmn.lac, mm_serving_plmn.rac )) ||
                           (gmm_plmn_changed(gmm_previous_registration_serving_rai.plmn_id,
...
FUNCTION gmm_check_if_stack_deact_in_progess

DESCRIPTION
  This function check if stack is being deactivated with cause related to power down

DEPENDENCIES
  None

RETURN VALUE
  boolean TRUE if cause is related to power down else FALSE
...
===========================================================================*/
 ...
 boolean gmm_check_if_stack_deact_in_progess(void)
 {
   if( mmcoord_pending_reg_message.cmd.hdr.message_id == MMR_STOP_MODE_REQ&&
      ((mmcoord_pending_reg_message.cmd.mmr_stop_mode_req.stop_mode_reason == SYS_STOP_MODE_REASON_MODE_CHANGE )||
       (mmcoord_pending_reg_message.cmd.mmr_stop_mode_req.stop_mode_reason == SYS_STOP_MODE_REASON_POWER_OFF)||
       (mmcoord_pending_reg_message.cmd.mmr_stop_mode_req.stop_mode_reason == SYS_STOP_MODE_REASON_OFFLINE)||
       (mmcoord_pending_reg_message.cmd.mmr_stop_mode_req.stop_mode_reason == SYS_STOP_MODE_REASON_LPM)||
       (mmcoord_pending_reg_message.cmd.mmr_stop_mode_req.stop_mode_reason == SYS_STOP_MODE_REASON_LPM_POWER_OFF)||
       (mmcoord_pending_reg_message.cmd.mmr_stop_mode_req.stop_mode_reason == SYS_STOP_MODE_REASON_MODE_CAP_CHANGE)   
      )
     )
   {   
    return TRUE;   
