...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/mm/src/mmglbl.c#2 $ $DateTime: 2023/10/08 15:22:21 $ $Author: pwbldsvc $
...
{
   forbidden_lai_T        lai[MAX_NO_FORBIDDEN_LAIS];
   byte                    write_ptr;
} forbidden_list_T;
...
extern nas_lai_reject_list_T lai_reject_list_sim[MAX_NAS_STACKS];
...
boolean              mm_reg_req_ignored_sim[MAX_NAS_STACKS];
...
byte                      mm_lu_preemption_failure_count_sim[MAX_NAS_STACKS];
...
boolean                   mm_is_camped_on_updated_lai_sim[MAX_NAS_STACKS];
...
boolean                   mm_integrity_protected_during_lu_sim[MAX_NAS_STACKS];
...
boolean                   mm_gw_intersystem_change_during_additional_mo_conn_sim[MAX_NAS_STACKS];
...
boolean                   mm_manual_mode_irat_ho_in_non_user_sel_cell_sim[MAX_NAS_STACKS];
...
boolean                   mm_hard_abort_deep_sleep_pending_sim[MAX_NAS_STACKS];
...
boolean                    mm_waiting_for_mocall_connect_sim[MAX_NAS_STACKS];
...
uint32 mm_gmm_reg_failed_cell_id_sim[MAX_NAS_STACKS];
...
boolean					mm_user_selection_sim[MAX_NAS_STACKS]; 
...
byte         mm_lu_buffer_rej_cause_sim[MAX_NAS_STACKS][MAX_LU_ATTEMPT_SIZE];
...
sys_network_selection_mode_e_type mm_last_requested_network_selection_mode_sim[MAX_NAS_STACKS];
...
mm_service_req_params_T *mm_last_sent_serv_req_params_ptr_sim[MAX_NAS_STACKS];
...
mmrrc_connection_status_T  mm_backup_rrc_connection_status_sim[MAX_NAS_STACKS];
#define  mm_backup_rrc_connection_status mm_backup_rrc_connection_status_sim[mm_as_id]

mmrrc_connection_status_T  mm_backup_cs_session_status_sim[MAX_NAS_STACKS];
#define  mm_backup_cs_session_status mm_backup_cs_session_status_sim[mm_as_id]

mmrrc_connection_status_T  mm_backup_ps_session_status_sim[MAX_NAS_STACKS];
...
sys_nas_timer_expiry_info_s_type  mm_timer_expiry_info_sim[MAX_NAS_STACKS];
#define  mm_timer_expiry_info  mm_timer_expiry_info_sim[mm_as_id]
#endif

uint8 mm_reg_req_trans_id_sim[MAX_NAS_STACKS];
...
mm_emm_dos_mitigation_feature_config_T mm_emm_dos_mitigation_feature_config_info_sim[MAX_AS_IDS];
#define mm_emm_dos_mitigation_feature_config_info mm_emm_dos_mitigation_feature_config_info_sim[mm_sub_id]

uint8   mm_emm_is_t3247_enabled_sim[MAX_AS_IDS];
#define mm_emm_is_t3247_enabled mm_emm_is_t3247_enabled_sim[mm_sub_id]

uint8  mm_emm_max_attempt_counter_dos_mitigation_sim[MAX_AS_IDS];
#define mm_emm_max_attempt_counter_dos_mitigation mm_emm_max_attempt_counter_dos_mitigation_sim[mm_sub_id]

uint8  mm_emm_max_fplmn_count_dos_mitigation_sim[MAX_AS_IDS];
...
uint8  mm_emm_t3247_test_val_sim[MAX_AS_IDS];
...
byte                      mm_lu_preemption_failure_count;
...
boolean                   mm_is_camped_on_updated_lai;
...
boolean                   mm_integrity_protected_during_lu;
...
boolean                   mm_manual_mode_irat_ho_in_non_user_sel_cell;
boolean					  mm_gw_intersystem_change_during_additional_mo_conn;
...
boolean                   mm_hard_abort_deep_sleep_pending;
boolean                   mm_deep_sleep_alt_scan;

boolean                   mm_waiting_for_csmt_response;
boolean                   mm_waiting_for_mocall_connect;
...
uint32 mm_gmm_reg_failed_cell_id;
...
byte         mm_lu_buffer_rej_cause[MAX_LU_ATTEMPT_SIZE];
...
/* Flag to remember last network selection mode send to RR/RRC */
sys_network_selection_mode_e_type mm_last_requested_network_selection_mode;

/* structure to store some params of last sent rrc service req */
mm_service_req_params_T *mm_last_sent_serv_req_params_ptr;
...
mmrrc_connection_status_T  mm_backup_rrc_connection_status;
mmrrc_connection_status_T  mm_backup_cs_session_status;
mmrrc_connection_status_T  mm_backup_ps_session_status;
byte mm_plmn_search_transaction_id;

#ifdef FEATURE_NAS_ECALL
sys_nas_timer_expiry_info_s_type  mm_timer_expiry_info;
#endif
uint8 mm_reg_req_trans_id;
...
boolean  mm_gmm_always_on_data_enable;
...
byte                      mm_last_page_type;
boolean                   mm_retry_page_rsp = FALSE;

boolean        mm_conn_mode_manual_search_enabled;
boolean        mm_nv_conn_mode_manual_search;

dword          mm_gsm_sel_resel_block_timer;
boolean        mm_disable_g_upon_fallback;
...
boolean mm_is_multisim_device = FALSE;

illegal_me_sub_type mm_illegal_me_sub_sim[MAX_NAS_STACKS] = {ILLEGAL_ME_SUB_NO_CAUSE, ILLEGAL_ME_SUB_NO_CAUSE};
...
#if defined(FEATURE_DUAL_SIM) || defined (FEATURE_SGLTE)
boolean      mm_reg_req__pended_due_to_csg_read_sim[MAX_NAS_STACKS];
#define  mm_reg_req__pended_due_to_csg_read mm_reg_req__pended_due_to_csg_read_sim[mm_as_id]
#else
boolean      mm_reg_req__pended_due_to_csg_read;
#endif
#endif

#if defined  FEATURE_LTE 
#ifdef FEATURE_DUAL_SIM
uint32 mm_extend_lte_disable_duration_sim[MAX_AS_IDS];
...
#if defined(FEATURE_DUAL_SIM)
byte mm_nv_vamos_feature_support_sim[MAX_AS_IDS];
...
#endif
byte                      mm_send_auth_rsp_on_invalid_gsm_kc_len = 0x01;
byte mm_ps_sig_conn_rel_after_ps_sms = 1;

byte mm_del_emm_params_on_gw_ps_reject = 0;
#ifdef FEATURE_NAS_REL11
...
#ifdef FEATURE_NAS_ECALL
#if defined(FEATURE_DUAL_SIM) || defined(FEATURE_SGLTE)
boolean mm_is_lu_triggered_in_ecall_mode_sim[MAX_AS_IDS] = {FALSE, FALSE};
#define  mm_is_lu_triggered_in_ecall_mode  mm_is_lu_triggered_in_ecall_mode_sim[mm_as_id]
#else
boolean mm_is_lu_triggered_in_ecall_mode = FALSE;
#endif
#endif


boolean mm_is_qrd_device = FALSE;
boolean mm_nv_enable_csg = FALSE;
...
ue_nas_rel_compliance_type mm_nv_nas_rel_compliance = NV_NAS_COMPLIANCE_REL11;
boolean                    full_srv_req_in_emerg;
boolean                    emergency_call_after_lu_in_lpm;
boolean                    mm_map_rej_cause7_to_rej_cause14;
boolean                    mm_map_rej_hplmn_cause7_to_rej_cause14;
//CMCC GSM Rej7
#endif
...
/* ---------------------------------------------------------------*/                    
#ifdef FEATURE_DUAL_SIM
byte                      gsm_wcdma_amr_codec_preference_sim[MAX_AS_IDS];
#define gsm_wcdma_amr_codec_preference gsm_wcdma_amr_codec_preference_sim[mm_sub_id]
#else
byte                      gsm_wcdma_amr_codec_preference;
#endif
...
boolean mm_ecall_dereg_in_wrong_state_sim[MAX_NAS_STACKS];
...
uint32 mm_ecall_inactivity_timer_sim[MAX_NAS_STACKS];
...
#ifdef FEATURE_DUAL_SIM
sys_ue_mode_e_type mm_current_ue_mode_sim[MAX_AS_IDS];
...
#ifdef  FEATURE_DUAL_SIM
boolean mm_proc_status_sim[MAX_AS_IDS][SYS_PROC_TYPE_TEST_LOOPBACK + 1];
#define mm_proc_status mm_proc_status_sim[mm_as_id]
sys_proc_type_e_type mm_last_proc_type_sent_sim[MAX_AS_IDS];
#define mm_last_proc_type_sent mm_last_proc_type_sent_sim[mm_as_id]
#endif

#if defined  FEATURE_DUAL_SIM || defined FEATURE_1XSRLTE
sys_proc_type_e_type mm_ds_proc_type_sim[MAX_AS_IDS];
...
sys_pch_state_s_type mm_rrc_pch_state_info_sim[MAX_NAS_STACKS];
#define mm_rrc_pch_state_info mm_rrc_pch_state_info_sim[mm_as_id]
boolean mm_nv_exclude_old_lai_type_ie_sim[MAX_AS_IDS] = {FALSE, FALSE};
...
sys_csg_support_e_type    mm_nv_csg_support_sim[MAX_NAS_STACKS] = {SYS_CSG_SUPPORT_DISABLED, SYS_CSG_SUPPORT_DISABLED};

#else
sys_csg_support_e_type    mm_nv_csg_support_sim[MAX_NAS_STACKS] = {SYS_CSG_SUPPORT_WCDMA, SYS_CSG_SUPPORT_WCDMA};
#endif
...
#ifdef FEATURE_DUAL_SIM
mm_qsh_debug_metrics_s_type mm_qsh_debug_metrics_sim[MAX_AS_IDS];
#define mm_qsh_debug_metrics mm_qsh_debug_metrics_sim[mm_sub_id]
#else
mm_qsh_debug_metrics_s_type mm_qsh_debug_metrics;
#endif
#endif
#ifdef FEATURE_QSH_DUMP
//Global  variables
mm_qsh_dump_tag_as_id_1_s *mm_qsh_dump_tag_0_ptr;
mm_qsh_dump_tag_as_id_2_s *mm_qsh_dump_tag_1_ptr;
mm_qsh_dump_tag_debug_buffer_s *mm_qsh_dump_tag_2_ptr;
#endif
#ifdef FEATURE_DUAL_SIM
#ifdef FEATURE_DUAL_DATA
size_t   mm_max_active_data = 1;
mm_as_id_e_type  mm_dds_sub = MM_AS_ID_1;
#endif
#endif
...
#ifdef FEATURE_QSH_EVENT_METRIC
boolean      mm_qevent_call_in_progress = FALSE;
...
===========================================================================*/
void mm_qevent_call_status_set(boolean status)
{
  mm_qevent_call_in_progress = status;
}
...
===========================================================================*/
boolean mm_qevent_call_status_get()
{  
  return mm_qevent_call_in_progress;
}

#endif    /* FEATURE_QSH_EVENT_METRIC */

void mm_lu_buffer_rej_cause_reset(void)
{
  int i;
  for ( i=0; i< MAX_LU_ATTEMPT_SIZE; i++)
  {
    mm_lu_buffer_rej_cause[i] = 0xFF;
  }
}
void gmm_buffer_rej_cause_reset(void)
{
  int i;
  for ( i=0; i< MAX_ATTEMPT_COUNTER_ATTACH; i++) 
  {
    gmm_buffer_rej_cause[i] = 0xFF;
  }
}
...
void mm_initialise(void)
{
   ...
   mm_gw_intersystem_change_during_additional_mo_conn=FALSE;
   ...
   */
   ...
   memset( &mm_emm_manual_search_param, 0x00, sizeof(mm_emm_manual_search_param_T) );
   mm_emm_manual_search_param.srch_priority = SYS_SRCH_PRIORITY_NORMAL;
   mm_emm_manual_search_param.enabled = 0;
...
#endif
   mm_send_auth_rsp_on_invalid_gsm_kc_len = 0x01;
   mm_ps_sig_conn_rel_after_ps_sms = 1;

#ifdef FEATURE_NAS_REL11
   memset(&mm_nas_nv_sim_hfns,0,sizeof(mm_nas_nv_sim_parameters_T));
#endif
   mm_is_qrd_device = FALSE;    
   mm_nv_enable_csg = FALSE;
#ifdef FEATURE_LTE
   ...
   if (status != MCFG_FS_STATUS_OK)
   {
     ...
     mm_del_emm_params_on_gw_ps_reject = 0;
   }
   else
   {
#ifdef FEATURE_GSM_CB
     ...
     if(mm_nas_nv_items.version >= 15)
     {
        if(mm_nas_nv_items.delete_guti_in_gw_when_lte_not_in_rat_list >= 1)
        {
          mm_del_emm_params_on_gw_ps_reject = 0x01;
        }
        else
        {
          mm_del_emm_params_on_gw_ps_reject  = 0;
     ...
     MSG_HIGH_DS_1(MM_SUB ,"=MM= value of mm_del_emm_params_on_gw_ps_reject = %d ",mm_nas_nv_items.delete_guti_in_gw_when_lte_not_in_rat_list);
     if(mm_nas_nv_items.version >= 17)
     {
        if(mm_nas_nv_items.send_auh_resp_on_invalid_gsm_kc_len >= 1)
        {
          mm_send_auth_rsp_on_invalid_gsm_kc_len = 0x01;
        }
        else
        {
          mm_send_auth_rsp_on_invalid_gsm_kc_len  = 0;
        }

     }
     MSG_HIGH_DS_1(MM_SUB ,"=MM= value of send_auh_resp_on_invalid_gsm_kc_len = %d ",mm_nas_nv_items.send_auh_resp_on_invalid_gsm_kc_len);
                      ...
                      MCFG_FS_TYPE_EFS,(mcfg_fs_sub_id_e_type)MM_AS_ID_1))== MCFG_FS_STATUS_OK) 
   {
     ...
     MSG_HIGH_DS_1(MM_SUB, "MM: setting nas_is_qmss_enabled to %d",nas_is_qmss_enabled);
   }
   else
   {
     nas_is_qmss_enabled = FALSE;
   ...
   { 
     mm_partial_search_allowed = TRUE;
...
#endif
   {
    ...
    mm_previous_serving_rat     = SYS_RAT_NONE;
     ...
     mm_tds_rat_status = SYS_RAT_ACTIVATION_STATUS_DEACTIVATED;
#endif
#endif
     mm_waiting_for_csmt_response     = FALSE;
     mm_waiting_for_mocall_connect     = FALSE;
#ifdef FEATURE_FEMTO_CSG
     mm_reg_req__pended_due_to_csg_read = FALSE;
#endif
     ...
     mm_is_camped_on_updated_lai = FALSE;
   ...
   mm_backup_rrc_connection_status  = MMRRC_IDLE;
   mm_backup_cs_session_status      = MMRRC_IDLE;
   mm_backup_ps_session_status      = MMRRC_IDLE;
...
#endif
#ifdef FEATURE_DUAL_SIM
   for ( i = SYS_PROC_TYPE_CS_CALL; i <= SYS_PROC_TYPE_TEST_LOOPBACK ;i++)
   {
     mm_proc_status[i] = FALSE;
   }
   mm_last_proc_type_sent = SYS_PROC_TYPE_NONE;
   mm_ds_proc_type = SYS_PROC_TYPE_NONE;
#endif
...
#ifdef FEATURE_DUAL_SIM
   mm_is_multisim_device = FALSE;
   mm_illegal_me_sub = ILLEGAL_ME_SUB_NO_CAUSE;
#endif
   mm_conn_mode_manual_search_enabled = FALSE;
   mm_nv_conn_mode_manual_search = FALSE;
   mm_nv_disable_conn_mode_mplmn = FALSE;

   mm_disable_g_upon_fallback = FALSE;

}

#ifdef FEATURE_DUAL_SIM 
  mm_subs_capability_mask_sim[MM_AS_ID_1] = (SYS_SYS_MODE_MASK_LTE|SYS_SYS_MODE_MASK_WCDMA|
                                             SYS_SYS_MODE_MASK_TDS|SYS_SYS_MODE_MASK_GSM);
  mm_subs_capability_mask_sim[MM_AS_ID_2] = SYS_SYS_MODE_MASK_GSM;
#if defined(FEATURE_TRIPLE_SIM)
  mm_subs_capability_mask_sim[MM_AS_ID_3] = SYS_SYS_MODE_MASK_GSM;
#endif
#endif
 ...
 /*mm_timer_t3246_remaining_to_efs*/
    ...
    if((status = mcfg_fs_read("/nv/item_files/modem/mmode/qmss_enabled",
                       ...
                       MCFG_FS_TYPE_EFS,(mcfg_fs_sub_id_e_type)MM_AS_ID_1)) == MCFG_FS_STATUS_OK)
    {
      MSG_HIGH_DS_1(MM_SUB, "MM: setting QRD device setting to %d",mm_is_qrd_device);
    }
    else
    {
      MSG_HIGH_DS_0(MM_SUB, "MM: setting QRD device to false as read failed");
      mm_is_qrd_device = FALSE;
    }
	 if((status = mcfg_fs_read("/nv/item_files/modem/nas/mm_nv_enable_msim_csg",
                       &mm_nv_enable_csg,sizeof(boolean), 
                       MCFG_FS_TYPE_EFS,(mcfg_fs_sub_id_e_type)MM_AS_ID_1)) == MCFG_FS_STATUS_OK)
    {
      MSG_HIGH_DS_1(MM_SUB, "MM: mm_nv_enable_csg set to %d",mm_nv_enable_csg);
    }
    else
    {
      MSG_HIGH_DS_0(MM_SUB, "MM: mm_nv_enable_csg read failed");
#ifdef FEATURE_HA_MAINLINE_ONLY_ENABLE
      mm_nv_enable_csg = TRUE;
#else
	  mm_nv_enable_csg = FALSE;
#endif
    }
...
**************************************************/
...
#endif
  {
   ...
   if (mm_ghdi_nvmem_read(NV_NAS_RELEASE_COMPLIANCE_I,
   ...
   {
     MSG_ERROR_DS(MM_SUB, "=MM=  Wrong value for NAS_REL_COMPLIANCE NV %d, default R 11",
...
void mm_reset_global_variables(void)
{
  ...
  ** ---------------------------- */
  ...
  mm_utran_ps_voice_parameters.is_last_utran_voims_supported = FALSE;
   ...
   mm_is_per_ms_t3212_present   = FALSE;
#endif
  if(mm_timer_status[GSM_EXTENDED_BLOCK_TIMER] == TIMER_ACTIVE)
  {
  ...
  mm_stop_mode_req_pended = FALSE;
  mm_reg_cause            = MMR_CAUSE_NONE;
  mm_integrity_protected_during_lu = FALSE;
  memset( &mm_emm_manual_search_param, 0x00, sizeof(mm_emm_manual_search_param_T) );
  mm_emm_manual_search_param.srch_priority = SYS_SRCH_PRIORITY_NORMAL;
  mm_emm_manual_search_param.enabled = 0;
...
FUNCTION mm_is_srvcc_call_in_progress

DESCRIPTION
  This function tells if a SRVCC call is in progress

DEPENDENCIES
  None

RETURN VALUE
  TRUE if SRVCC call is in progress, FALSE otherwise
...
boolean mm_is_srvcc_call_in_progress
(
  sys_modem_as_id_e_type as_id
)
{
   ...
   return mm_is_cs_connected_due_to_srvcc_sim[as_id];
#else
   (void) as_id;
   return mm_is_cs_connected_due_to_srvcc;
#endif
}
...
===========================================================================*/
void mm_proc_status_set
(
  sys_proc_type_e_type proc_type,
  boolean value
)
{
  if(proc_type <= SYS_PROC_TYPE_TEST_LOOPBACK)
  {
    mm_proc_status[proc_type] = value;
  }
}

/*===========================================================================

FUNCTION MM_LAST_PROC_TYPE_SENT_SET
...
===========================================================================*/
void mm_last_proc_type_sent_set
(
  sys_proc_type_e_type proc_type
)
{
  mm_last_proc_type_sent = proc_type;
}

#ifdef FEATURE_TDSCDMA
 /*===========================================================================

FUNCTION MM_TDS_RAT_STATUS_SET

DESCRIPTION
  This function sets the mm_tds_rat_status
...
{
  if(status != mm_tds_rat_status)
  {
    MSG_HIGH_DS_2(MM_SUB, "=MM= mm_tds_rat_status changing from %d to %d",mm_tds_rat_status,status);
    mm_tds_rat_status = status;
    if(status == SYS_RAT_ACTIVATION_STATUS_ACTIVATED)
    {
      mm_as_id_e_type other_as_id = MM_OTHER_SUB(mm_as_id);
      mm_stop_timer(TIMER_TDS_RAT_DEACTIVATION);
      mm_send_tdsrrc_subscription_id_ind();
      //reset TDS status on other sub
      if(mm_last_active_rat_stopped_sim[other_as_id] == SYS_RAT_TDS_RADIO_ACCESS)
      {
        mm_last_active_rat_stopped_sim[other_as_id] = SYS_RAT_NONE;
      }
    }
    else if(status == SYS_RAT_ACTIVATION_STATUS_DEACTIVATED)
    {
      mm_stop_timer(TIMER_TDS_RAT_DEACTIVATION);
      mm_resume_ignored_mmr_reg_req(MM_OTHER_SUB(mm_as_id));
    }
    else if(status == SYS_RAT_ACTIVATION_STATUS_DEACTIVATION_IN_PROGRESS)
    {
      mm_start_timer(TIMER_TDS_RAT_DEACTIVATION,DEFAULT_TIMEOUT);
    }
  }
}
#endif
...
void mm_invalidate_cs_stack_database(void)
{
  ...
  mm_lu_preemption_failure_count_sim[mm_cs_stack_id] = 0;
  ...
  mm_backup_rrc_connection_status_sim[mm_cs_stack_id] = MMRRC_IDLE;
  mm_backup_cs_session_status_sim[mm_cs_stack_id]     = MMRRC_IDLE;
  mm_backup_ps_session_status_sim[mm_cs_stack_id]     = MMRRC_IDLE;
}
...
/* RESETTING of the variables */
  ...
  mm_integrity_protected_during_lu_sim [src] = FALSE;
  ...
  mm_is_lu_to_be_initiated_t3346_ltogw = FALSE;
#endif
  mm_backup_rrc_connection_status_sim[src] = MMRRC_IDLE;
  mm_backup_cs_session_status_sim[src]     = MMRRC_IDLE;
  mm_backup_ps_session_status_sim[src]     = MMRRC_IDLE;
...
/* memcpy relate varaibles start here */
  ...
  memscpy((void *)&lai_reject_list_sim[dest],sizeof(nas_lai_reject_list_T), 
                   (void *)&lai_reject_list_sim[src],sizeof(nas_lai_reject_list_T));
...
/*###########################################################################*/
 /* All variables related to assigned  start here*/
  ...
  mm_lu_preemption_failure_count_sim[dest] = mm_lu_preemption_failure_count_sim[src];
  ...
  mm_is_camped_on_updated_lai_sim[dest] = mm_is_camped_on_updated_lai_sim[src];
  mm_ef_kc_valid_sim[dest]  = mm_ef_kc_valid_sim[src];
  mm_integrity_protected_sim[dest]  = mm_integrity_protected_sim[src];
  mm_integrity_protected_during_lu_sim[dest]  = mm_integrity_protected_during_lu_sim[src];
  ...
  mm_gmm_reg_failed_cell_id_sim[dest] = mm_gmm_reg_failed_cell_id_sim[src];
#ifdef FEATURE_DSAC
...
/* Flag to remember last network selection mode send to RR/RRC */
  mm_last_requested_network_selection_mode_sim[dest] = mm_last_requested_network_selection_mode_sim[src];
#ifdef FEATURE_GPRS_LAYER3_TX_CONFIRM
...
void mm_read_carrier_specific_efs(void)
{
  ...
  dog_hb_report(DOG_MM_RPT);
  status[2] = mcfg_fs_read("/nv/item_files/modem/nas/suppress_add_update_param",
  ...
  {
    mm_suppress_additional_update_param = TRUE;
  }
  
  MSG_HIGH_DS_6(MM_SUB,"=MM= exclude_old_lai_type_ie (status %d. value %d) extend_lte_disable_duration (status %d. value %d) suppress_additional_update_param (status %d. value %d)",
                       status[0],
...
#ifdef FEATURE_VAMOS 
   status[1] = mcfg_fs_read("/nv/item_files/modem/geran/vamos_support", 
                           &mm_nv_vamos_feature_support,sizeof(vamos_support_e),
                            MCFG_FS_TYPE_EFS,(mcfg_fs_sub_id_e_type)mm_sub_id);
   
   if (status[1] != MCFG_FS_STATUS_OK)
   {
     MSG_HIGH_DS_1(MM_SUB, "=MM= Unable to read NV vamos_support , status = %d",status[1]);
     mm_nv_vamos_feature_support = VAMOS_1;
   }
   MSG_HIGH_DS_2(MM_SUB, "=MM= NV NAS VAMOS FEATURE status,VAMOS FEATURE VALUE : %d, %d",status[1],mm_nv_vamos_feature_support);
#endif 

 /*During refresh,if UE is in DSDS mode then don't use this NV instead enable the feature by default */
#ifdef FEATURE_DUAL_SIM
  if((mm_nv_disable_conn_mode_mplmn == FALSE)&&
		((mmode_nv_setting_for_device_mode == SYS_MODEM_DEVICE_MODE_DUAL_SIM_DUAL_STANDBY) ||
        (mmode_nv_setting_for_device_mode == SYS_MODEM_DEVICE_MODE_TRIPLE_SIM_TRIPLE_STANDBY )) && mm_dual_standby_mode == TRUE)
    {
      mm_conn_mode_manual_search_enabled = TRUE;
    }
#endif
  MSG_HIGH_DS_3(MM_SUB,"=MM= conn_mode_manual_search efs read failed, status: %d, default %d mm_conn_mode_manual_search_enabled %d",
                         status[0],mm_nv_conn_mode_manual_search,mm_conn_mode_manual_search_enabled);
  ...
  status[0] = mcfg_fs_read("/nv/item_files/modem/mmode/sd/full_srv_req_in_emerg", 
  ...
  status[1] = mcfg_fs_read("/nv/item_files/modem/nas/emergency_call_after_lu_in_lpm", 
  ...
  MSG_HIGH_DS_7(MM_SUB,"=MM= NV_FORCE_UE_SGSNR_R99 : %d, NV_FORCE_UE_MSCR_R99_I : %d, NV_NAS_REL_COMPLIANCE : %d full_srv_req_in_emerg (status %d. value %d) emergency_call_after_lu_in_lpm (status %d. value %d)",
                       mm_nv_force_sgsnr_r99,
                       mm_nv_force_mscr_r99,
                       mm_nv_nas_rel_compliance,
                       status[0],
                       full_srv_req_in_emerg,
                       status[1],
                       emergency_call_after_lu_in_lpm);

  /*NV For Mapping PLMN Rejection Cause 7 to Cause 14*/
  status[2] = mcfg_fs_read("/nv/item_files/modem/nas/map_cause7_to_cause14", 
                     &mm_map_rej_cause7_to_rej_cause14, sizeof(boolean),
                     MCFG_FS_TYPE_EFS,(mcfg_fs_sub_id_e_type)mm_sub_id);
  if (status[2] == MCFG_FS_STATUS_OK)
  {
    MSG_HIGH_DS_1(MM_SUB, "=MM= Setting NV map_cause7_to_cause14 to %d",mm_map_rej_cause7_to_rej_cause14);
  }
  else
  {
    mm_map_rej_cause7_to_rej_cause14 = FALSE;
    MSG_HIGH_DS_1(MM_SUB, "=MM= NV read failed, setting NV map_cause7_to_cause14 to default %d",mm_map_rej_cause7_to_rej_cause14);
  }
   status[2] = mcfg_fs_read("/nv/item_files/modem/nas/map_hplmn_cause7_to_cause14", 
						&mm_map_rej_hplmn_cause7_to_rej_cause14, sizeof(boolean),
						MCFG_FS_TYPE_EFS,(mcfg_fs_sub_id_e_type)mm_sub_id);
   if (status[2] == MCFG_FS_STATUS_OK)
   {
	   MSG_HIGH_DS_1(MM_SUB, "=MM= Setting NV map_cause7_to_cause14 to %d",mm_map_rej_hplmn_cause7_to_rej_cause14);
   }
   else
   {
	   mm_map_rej_hplmn_cause7_to_rej_cause14 = FALSE;
	   MSG_HIGH_DS_1(MM_SUB, "=MM= NV read failed, setting NV map_cause7_to_cause14 to default %d",mm_map_rej_hplmn_cause7_to_rej_cause14);
    }
   ...
   dog_hb_report(DOG_MM_RPT);
   status[2] = mcfg_fs_read("/nv/item_files/modem/nas/dos_mitigation_feature_config", 
                            &mm_emm_dos_mitigation_feature_config_info, sizeof(mm_emm_dos_mitigation_feature_config_T),
                            MCFG_FS_TYPE_EFS,(mcfg_fs_sub_id_e_type)mm_sub_id);

   if(status[2] != MCFG_FS_STATUS_OK)
   {
     mm_emm_is_t3247_enabled = FALSE;  
   ...
   MSG_HIGH_DS_4(MM_SUB ,"dos_mitigation_feature_config efs read status: %d, t3247 enabled %d,  max_count value %d, max_entries value %d",
                            status[2], mm_emm_is_t3247_enabled,
                           mm_emm_max_attempt_counter_dos_mitigation,mm_emm_max_fplmn_count_dos_mitigation);

   dog_hb_report(DOG_MM_RPT);
   status[0] = mcfg_fs_read("/nv/item_files/modem/nas/t3247_timer_val_test", 
   ...
   MSG_HIGH_DS_2(MM_SUB ,"t3247_test_timer_val efs read status: %d, timer value (in mins) %d",
                            status[0],mm_emm_t3247_test_val);
  /*GSM extended block timer : FR50563*/
   status[2] = mcfg_fs_read("/nv/item_files/modem/nas/gsm_sel_resel_block_timer", 
                            &mm_gsm_sel_resel_block_timer, sizeof(mm_gsm_sel_resel_block_timer),
                            MCFG_FS_TYPE_EFS,(mcfg_fs_sub_id_e_type)mm_sub_id);

   if(status[2] != MCFG_FS_STATUS_OK)
   {
     mm_gsm_sel_resel_block_timer = 120; /*Default value to 120 minutes*/
   }
    

   MSG_HIGH_DS_2(MM_SUB ,"mm_gsm_sel_resel_block_timer efs read status: %d, timer value (in mins) %d",
                            status[0],mm_gsm_sel_resel_block_timer);


   /*GSM extended block timer : FR50563*/
   status[2] = mcfg_fs_read("/nv/item_files/modem/nas/disable_gsm_during_fallback_upon_rej15", 
                            &mm_disable_g_upon_fallback, sizeof(mm_disable_g_upon_fallback),
                            MCFG_FS_TYPE_EFS,(mcfg_fs_sub_id_e_type)mm_sub_id);

   if(status[2] != MCFG_FS_STATUS_OK)
   {
     mm_disable_g_upon_fallback = FALSE; /*Default value to false*/
   }
    

   MSG_HIGH_DS_2(MM_SUB ,"mm_disable_g_upon_fallback efs read_status: %d,  is_fall_back_disabled=%d",
                            status[0],mm_disable_g_upon_fallback);

#ifdef FEATURE_HA_MAINLINE_ONLY_ENABLE   
   dog_hb_report(DOG_MM_RPT);
   status[0] = mcfg_fs_read("/nv/item_files/modem/nas/gmm_disable_forbit_for_alwayson_data", 
                            &mm_gmm_always_on_data_enable, sizeof(mm_gmm_always_on_data_enable),
                            MCFG_FS_TYPE_EFS,(mcfg_fs_sub_id_e_type)mm_sub_id);

   if(status[0] != MCFG_FS_STATUS_OK)
   {
    mm_gmm_always_on_data_enable = TRUE;
   }

   MSG_HIGH_DS_2(MM_SUB ,"always_on_data efs read status: %d, timer value (in mins) %d",
                            status[0],mm_gmm_always_on_data_enable);
#endif
}
...
#ifdef FEATURE_FEMTO_CSG
  sys_csg_support_e_type mm_nv_csg_support_prev = mm_nv_csg_support;
#endif
...
#ifdef FEATURE_FEMTO_CSG
   if((mm_nv_csg_support != mm_nv_csg_support_prev) && 
   	  (mm_nv_csg_support == SYS_CSG_SUPPORT_DISABLED || mm_nv_csg_support_prev == SYS_CSG_SUPPORT_DISABLED))
  {
    mmsim_update_nv_csg_support_data();
  }
#endif
