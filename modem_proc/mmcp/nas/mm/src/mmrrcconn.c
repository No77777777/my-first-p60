...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/mm/src/mmrrcconn.c#2 $    $DateTime: 2021/08/12 12:01:36 $ $Author: pwbldsvc $
...
boolean mm_rrc_est_cause_sent_for_call_sim[MAX_NAS_STACKS];
...
boolean mm_rrc_est_cause_sent_for_call;
...
boolean mm_dds_switch_while_rrc_conn_active_sim[MAX_NAS_STACKS];
#define mm_dds_switch_while_rrc_conn_active mm_dds_switch_while_rrc_conn_active_sim[mm_as_id]
#else
boolean mm_dds_switch_while_rrc_conn_active;
#endif
...
sys_modem_as_id_e_type mm_tag_rrc_as_id (rrc_cmd_type *cmd)
{  

  sys_modem_as_id_e_type  rrc_as_id = (sys_modem_as_id_e_type)mm_sub_id;
  switch (cmd->cmd_hdr.cmd_id)
  {
    ...
    case RRC_EST_REQ:
      ...
      mm_msg_info = (byte)(cmd->cmd.rrc_est_req.proc_type);
      break;
    ...
    case RRC_PLMN_LIST_REQ:
      ...
      mm_msg_info = (byte)(cmd->cmd.plmn_list_req.trans_id);
      break;
    ...
    case RRC_DS_STATUS_CHANGE_IND:/*NO SIM ID*/
      rrc_as_id = SYS_MODEM_AS_ID_NONE;
      break;
#ifdef FEATURE_DUAL_SIM
   ...
   case RRC_CSG_WHITE_LIST_UPDATE_IND:
    ...
    case RRC_PROCEDURE_IND :
      cmd->cmd.rrc_procedure_ind.as_id = (sys_modem_as_id_e_type)mm_sub_id;
      mm_msg_info = (byte)(cmd->cmd.rrc_procedure_ind.proc_type);
    break;
   case RRC_PSEUDO_LTE_LIST_UPDATE_IND:
     ...
     cmd->cmd.rrc_g_meas_stop_ind.as_id = (sys_modem_as_id_e_type)mm_sub_id;
     break;
#ifdef FEATURE_DUAL_SIM
   ...
   case RRC_ABNORMAL_CALL_END_IND:
       rrc_as_id = mm_get_as_id_rrc_abnormal_call_end_ind();
       cmd->cmd.rrc_subscription_id.as_id = (sys_modem_as_id_e_type)rrc_as_id;
	   MSG_HIGH_DS_1(MM_SUB,"=MM= MM sent RRC_ABNORMAL_CALL_END_IND to RRC as_id %d",rrc_as_id);
       break;
...
void mm_rrc_put_mm_cmd(sys_modem_as_id_e_type rrc_as_id, rrc_cmd_type *cmd)
{
...
#ifdef FEATURE_DUAL_WCDMA
   if(rrc_as_id == SYS_MODEM_AS_ID_NONE)
   {
     ...
     {
       rrc_as_id = SYS_MODEM_AS_ID_1;
       memscpy(rrc_cmd, sizeof(rrc_cmd_type), cmd, sizeof(rrc_cmd_type));

 #ifndef FEATURE_SEGMENT_LOADING
       rrc_put_mm_cmd_sub( rrc_as_id, rrc_cmd );
 #else
       IWCDMA_rrc_put_mm_cmd_sub( ptr_W, rrc_as_id, rrc_cmd );
 #endif
     }
     rrc_as_id = SYS_MODEM_AS_ID_2;
   }
#endif 
   ...
   rrc_put_mm_cmd( cmd );
 #else
   rrc_put_mm_cmd_sub( rrc_as_id, cmd );
 #endif 
 #else
 #ifndef FEATURE_DUAL_WCDMA
   IWCDMA_rrc_put_mm_cmd( ptr_W, cmd );
 #else
   IWCDMA_rrc_put_mm_cmd_sub( ptr_W, rrc_as_id, cmd );
 #endif
 #endif
}
...
void send_cmd_to_umts_rrc( rrc_cmd_type *cmd )
{
   ...
   sys_modem_as_id_e_type  rrc_as_id = SYS_MODEM_AS_ID_1;
   ...
   switch (cmd->cmd_hdr.cmd_id)
   {
      ...
      case RRC_EST_REQ:
         MSG_HIGH_DS_1(MM_SUB,"=MM= MM sent RRC_EST_REQ to RRC proc_type %d",
                                                cmd->cmd.rrc_est_req.proc_type);
              ...
              ! ((mm_conn_mode_manual_search_enabled == TRUE) &&
                  ((mm_check_mplmn_in_progress_on_other_subs() == TRUE))) &&
                   ...
                   ( 
                    ...
                    (gmm_extend_connection_due_to_mt_csfb)|| gmm_force_for_always_on_data==TRUE ))
               {
                 cmd->cmd.open_sess_req.nas_msg.value[2] |= 0x08;
            ...
            if (is_umts_mode() &&
                ...
                !((mm_conn_mode_manual_search_enabled == TRUE) &&
                  ...
                  (
                          ...
                          sm_is_pdp_activation_pending() == TRUE ||
#endif
                          gmm_follow_on_request||gmm_force_for_always_on_data==TRUE))
               {
      ...
      case RRC_SERVICE_REQ:
        MSG_HIGH_DS_2(MM_SUB,"=MM= MM sent RRC_SERVICE_REQ to RRC service request cause %d scan scope %d",cmd->cmd.rrc_service_req.service_req_cause,cmd->cmd.rrc_service_req.scan_scope);
#ifdef FEATURE_LTE
#ifdef TF_UNIT_TEST
         #error code not present
#endif
...
#ifdef FEATURE_DUAL_SIM
         mm_update_rat_status(&cmd->cmd.rrc_change_mode_ind.rat_pri_list_info,
...
#ifdef FEATURE_DUAL_SIM
      case RRC_PROCEDURE_IND:
         MSG_HIGH_DS_1(MM_SUB,"=MM= MM sent RRC_PROCEDURE_IND to RRC proc_type %d",cmd->cmd.rrc_procedure_ind.proc_type);
         break;
#endif

      case RRC_PSEUDO_LTE_LIST_UPDATE_IND:
        MSG_HIGH_DS_1(MM_SUB ,"=MM= MM sent RRC_PSEUDO_LTE_LIST_UPDATE_IND to RRC can_use_pseudo = %d",cmd->cmd.rrc_psuedo_lte_list_update_ind.can_use_pseudo); 
        break;
  case RRC_G_MEAS_STOP_IND:
        MSG_HIGH_DS_1(MM_SUB,"=MM= MM sent RRC_G_MEAS_STOP_IND to RRC with value %d", cmd->cmd.rrc_g_meas_stop_ind.stop_g_meas);
        break;
#ifdef FEATURE_DUAL_SIM
       case RRC_DDS_SUB_IND:
         MSG_HIGH_DS_1(MM_SUB,"=MM= MM sent RRC_DDS_SUB_IND to RRC dds_sub %d", cmd->cmd.rrc_dds_sub_ind.dds_sub_id);
         break;
#endif
       case RRC_ABNORMAL_CALL_END_IND:
			   MSG_HIGH_DS_0(MM_SUB,"=MM= MM sent RRC_ABNORMAL_CALL_END_IND to RRC");
			   break;
...
#ifdef FEATURE_DUAL_SIM
   rrc_as_id = mm_tag_rrc_as_id(cmd);
#endif
/* Add sent messages to the debug buffer */
  mm_add_message_to_debug_buffer(MS_MM_RRC,
   ...
   mm_rrc_put_mm_cmd(rrc_as_id, cmd);
   ...
   switch (cmd->cmd_hdr.cmd_id)
   {
      case RRC_CLOSE_SESSION_REQ:
                  ...
                  (rrc_connection_status == MMRRC_RELEASE_IN_PROGRESS))
         {
           ...
           modem_mem_free(cmd,MODEM_MEM_CLIENT_NAS);
           tdsrrc_free_mm_cmd_buf(tdsrrc_cmd);
      ...
      case RRC_EST_REQ:
         ...
         MSG_HIGH_DS_2(MM_SUB,"=MM= MM sent TDSRRC_EST_REQ secure call status = %d proc_type %d", 
                                              mm_emm_secure_status_type, cmd->cmd.rrc_est_req.proc_type);
         /* inform TDSRRC secure call enabled when MO CSFB, only valid for CS domain */
         if ((mm_emm_secure_status_type == SYS_SECURE_CALL_START)
             &&(RRC_CS_DOMAIN_CN_ID == cmd->cmd.rrc_est_req.cn_domain_id))
         {
           mm_send_rrc_secure_call_ind(RRC_SECURE_VOICE_ENABLE);
         }
#else
         MSG_HIGH_DS_1(MM_SUB,"=MM= MM sent TDSRRC_EST_REQ proc_type %d", cmd->cmd.rrc_est_req.proc_type);
#endif
         rrc_connection_status = MMRRC_EST_IN_PROGRESS;
...
#ifdef FEATURE_DUAL_SIM
       case RRC_PROCEDURE_IND:
         MSG_HIGH_DS_1(MM_SUB,"=MM= MM sent TDSRRC_PROCEDURE_IND to RRC proc_type = %d",cmd->cmd.rrc_procedure_ind.proc_type);
         tdsrrc_cmd->cmd_hdr.cmd_id = TDSRRC_PROCEDURE_IND;
         tdsrrc_cmd->cmd.rrc_procedure_ind = cmd->cmd.rrc_procedure_ind;
         break;
#endif
      default:
   ...
   switch (message->cmd.hdr.message_set)
   {
      case MS_MM_RR:
         switch (message->cmd.hdr.message_id)
         {
            case RR_EST_CNF:
               /*
               Handle RR_EST_CNF in case RRC connection status is EST_IN_PROGRESS otherwise abort RR connection
               */
               if (rrc_connection_status == MMRRC_EST_IN_PROGRESS)
               {
                   rrc_connection_status = MMRRC_ACTIVE;
                   MSG_HIGH_DS_0(MM_SUB, "=MM= RR Connection Established");
                   /*reset after sending out page response for call, no chance for rach failure now*/
                   if (mm_state == MM_WAIT_FOR_NETWORK_COMMAND && mm_timer_status[TIMER_MM_PAGE_RETRY]==TIMER_ACTIVE)
                   {
            ...
            case RR_ABORT_IND:
            case RR_ABORT_CNF:
            if (mm_timer_status[TIMER_MM_PAGE_RETRY]==TIMER_ACTIVE && mm_pending_lu_request == LTE_IRAT_LU
...
/*24.008: 4.1.1.8   Handling of security related parameters at switch on and switch off*/
...
#endif
                  if(pending_gmm_msg_valid == FALSE && pending_mm_msg_valid == FALSE && is_umts_mode()
                     && pending_gmm_msg == NULL && pending_mm_msg == NULL)
                  {
            ...
            case RRC_ABORT_IND:
            case RRC_ABORT_CNF:
			if(mm_dds_switch_while_rrc_conn_active==FALSE)
			{
...
#ifdef FEATURE_LTE
                        /*Perform force detach to recover subsequent MT call after capability mismatch*/
                        mm_emm_radio_cap_enquiry_failure();
#endif
                      }
                      else
                      {
            ...
            case RRC_CLOSE_SESSION_CNF:
              ...
              if( mm_rlf_stop_mode_pending != MM_RLF_STOP_MODE_PENDING_NONE)
              {
                    ...
                    ((message->cmd.hdr.message_id == RRC_CLOSE_SESSION_CNF) && (message->cmd.rrc_close_session_cnf.cn_domain_id == RRC_CS_DOMAIN_CN_ID)))
               {
                  ...
                  if((pending_mm_msg != NULL) && (pending_mm_msg_after_release == FALSE)
                       &&  (mm_cs_domain_possible_reest ==  FALSE)
                  )
                  {
                    modem_mem_free(pending_mm_msg,MODEM_MEM_CLIENT_NAS);
                         ...
                         ((message->cmd.hdr.message_id == RRC_CLOSE_SESSION_CNF) && (message->cmd.rrc_close_session_cnf.cn_domain_id == RRC_PS_DOMAIN_CN_ID)))
               {
                 ...
                 else
                 {
                    ...
                    gmm_cancel_authentication() ;
#ifdef FEATURE_WRLF_SYSTEM_SEL
                    /* Prevent reestablishing PS connection in RLF */
                    if(mm_wcdma_rlf_state == WCDMA_RLF_STARTED)
                    {                   
                       mm_ps_domain_possible_reest = FALSE;
                    }
#endif
                    if((pending_gmm_msg != NULL) && (pending_gmm_msg_after_release == FALSE)
                       &&  (mm_ps_domain_possible_reest ==  FALSE)
                   )
                    {
                      modem_mem_free(pending_gmm_msg,MODEM_MEM_CLIENT_NAS);
...
)
{
   ...
   if(buffer_outgoing_msg == FALSE)
   {
     if(message_id == GMM_DETACH_REQUEST)
     {
       gmm_qsh_ota_metric_signaling_log(GMM_DETACH_REQUEST,UE_TO_CN_DIRECTION,
             (uint8)(((gmm_mo_detach_request_msg_T*)message)->detach_type_spare & 0x07), MM_GMM_NO_FAILURE_DEBUG);
     }
     if(message_id == GMM_AUTHENTICATION_AND_CIPHERING_FAILURE)
     {
       gmm_qsh_ota_metric_signaling_log(GMM_AUTHENTICATION_AND_CIPHERING_FAILURE,UE_TO_CN_DIRECTION,
                             0xff, (mm_gmm_reject_cause_debug_type)(((gmm_auth_ciph_failure_msg_T*)message)->GMM_cause));
     }
     else if(message_id == GMM_STATUS)
     {
       gmm_qsh_ota_metric_signaling_log(GMM_STATUS,UE_TO_CN_DIRECTION,
                             0xff, (mm_gmm_reject_cause_debug_type)(((gmm_mo_gmm_status_msg_T*)message)->gmm_cause));
     }
     else if(message_id == GMM_SERVICE_REQUEST)
     {
       gmm_qsh_ota_metric_signaling_log(GMM_SERVICE_REQUEST,UE_TO_CN_DIRECTION,
                             (uint8)(((gmm_service_request_msg_T*)message)->cksn_service_type & 0x70),MM_GMM_NO_FAILURE_DEBUG);
     }
   }
#endif
                       ...
                       rrc_establish_cause_e_type cause )
{
   ...
   hfn_type hfns;
   mm_rrc_est_cause_sent_for_call_set(FALSE);
   ...
   {
	  mm_rrc_est_cause_sent_for_call_set(TRUE);
...
#ifdef FEATURE_DUAL_SIM
   rrc_est_req_p->proc_type = mm_evaluate_proc_type();
   mm_last_proc_type_sent_set(rrc_est_req_p->proc_type);
#endif
