...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/mm/src/gmmutils.c#2 $ $DateTime: 2021/08/12 12:29:53 $ $Author: pwbldsvc $
    ...
    ** --------------------------------------------------- */
    for (i=0; i < PTMSI_SIZE; i++)
    {
      ...
      if(MM_SUB_IS_SGLTE)
      {
        ...
        gmm_old_ptmsi_sim[MM_SGLTE_STACK_2].digit[i] = 0xFF;
      }
#endif
    }
#ifdef FEATURE_DUAL_SIM
...
/*===========================================================================
FUNCTION gmm_send_gmmsm_irat_ps_service_ind 

DESCRIPTION
  Sends GMMSM_IRAT_PS_SERVICE_IND to SM from GMM
...
SIDE EFFECTS
  None.
===========================================================================*/
void gmm_send_gmmsm_irat_ps_service_ind()
{
  sm_cmd_type * sm_cmd;
  sm_cmd = sm_get_cmd_buf();
  ...
  {
    sm_cmd->header.message_set     = MS_GMM_SM;
    sm_cmd->header.cmd_id          = GMMSM_IRAT_PS_SERVICE_IND;
#ifdef FEATURE_DUAL_SIM
    sm_cmd->cmd.gmmsm_irat_ps_service_ind.as_id = (sys_modem_as_id_e_type)mm_sub_id;
#endif
    sm_put_cmd (sm_cmd);
  }
  else
...
===========================================================================*/
void gmm_send_gmmsm_establish_resend_ind(sm_cm_status_T cause)
{
  sm_cmd_type * sm_cmd;
  sm_cmd = sm_get_cmd_buf();
  ...
  {
    sm_cmd->header.message_set     = MS_GMM_SM;
    sm_cmd->header.cmd_id          = GMMSM_ESTABLISH_RESEND_IND;
    sm_cmd->cmd.gmmsm_establish_resend_ind.cause = cause;
#ifdef FEATURE_DUAL_SIM
    sm_cmd->cmd.gmmsm_establish_resend_ind.as_id = (sys_modem_as_id_e_type)mm_sub_id;
#endif
    MSG_HIGH_DS_0(MM_SUB, "=MM= MM sent GMMSM_ESTABLISH_RESEND_IND");
    sm_put_cmd (sm_cmd);
  }
  else
  {
...
#endif
      else if (gmm_rau_is_required && mm_waiting_for_stop_mode_cnf == FALSE )
      {
        /* Initiate RAU */
...
void gmm_send_nv_params (void)
{
 ...
 #endif
 mcfg_fs_status_e_type qos_status = MCFG_FS_STATUS_EFS_ERR;
 ...
 ** ------------------------------------------------------------------*/
               ...
               (ghdi_nvmem_data_T *)&gmm_nv_multislot_gmsk_power_profile)) != GHDI_SUCCESS)
 {
  ...
  else
  {
   ...
   if((qos_status = mcfg_fs_read("/nv/item_files/modem/data/3gpp/umtsps/qos_support",&qos_support_enabled,sizeof(boolean),
   ...
   {
      MSG_HIGH_DS_1(MM_SUB, "=MM= Setting XID negotiation as FALSE  as read failed, status = %d", qos_status);
      gmm_xid_negotiation_completed = FALSE;
   }
...
#error FEATURE_GPRS_PS_HANDOVER requires FEATURE_REL_6_SUPPORT
  ...
  /* REL 11: EMM Combined Procedures bit should always be set as 1
    ...
    if((mm_checkif_rat_pri_list_has_rat(&mm_rat_pri_list_info, 
                                             SYS_RAT_LTE_RADIO_ACCESS) == TRUE) && emm_is_srvcc_supported(emm_ctrl_data_ptr) == TRUE)
    {
      csn_encode_bit(&encoder, (byte) 1); 
    }
    else
...
void gmm_read_carrier_specific_efs(void)
{
  ...
  status =  mcfg_fs_read("/nv/item_files/modem/nas/prohibit_drx_timer_value",
                          &prohibit_timer_value,
                          sizeof(uint16),
                          MCFG_FS_TYPE_EFS,(mcfg_fs_sub_id_e_type)MM_AS_ID_1);
  if (status != MCFG_FS_STATUS_OK)
  {
     MSG_HIGH_DS_1(MM_SUB ,"=MM= Unable to read NV prohibit DRX timer value, status : %d ",status);
     /*Prohibit timer 60 mins as default value*/
     gmm_client_req_drx_coeff.prohibit_timer_value = 3600000;
  }
  else
  {
    gmm_client_req_drx_coeff.prohibit_timer_value = (uint32)prohibit_timer_value * 1000;
  }
#endif
 ...
 ** ------------------------------------------------------------------*/
  ...
  else if (gmm_gea_support > 127) 
  {
    ...
    /* If GEA1 support is enabled from NV, disable it regardless of 3GPP release*/
    gmm_gea_support &= ~0x01;

  MSG_HIGH_DS_1(MM_SUB, "=MM= GEA Algorithm support value: %d", gmm_gea_support);

  ghdi_clear_hw_classmark_cache( 
#ifdef FEATURE_DUAL_SIM
...
FUNCTION  gmm_qsh_ota_metric_signaling_log
...
===========================================================================*/
void gmm_qsh_ota_metric_signaling_log
(
  uint8                  message_id,
  uint8                  direction,
...
)
{
  ...
  metric_sig.protocol_discriminator = PROTOCOL_DISCRMINATOR_GMM_DEBUG;
  metric_sig.message_type.gmm_message_id = 
             (gmm_message_id_debug_type)message_id;
  metric_sig.cause_value.gmm_cause = gmm_cause;
...
===========================================================================*/
boolean gmm_get_cm_ds_tuneaway_status(void)
{
...
void gmm_reset_old_ptmsi(void)
{
   gmm_old_ptmsi.digit[0] = 0xFF;
   gmm_old_ptmsi.digit[1] = 0xFF;
   gmm_old_ptmsi.digit[2] = 0xFF;
   gmm_old_ptmsi.digit[3] = 0xFF;
}
...
FUNCTION GMM_CHECK_PS_SIGNALLING_CONNECTION_ACTIVE
...
RETURN VALUE
  TRUE:  If PS signalling connection is not released.
  FALSE: If PS signalling connection is already released.
...
boolean gmm_check_ps_signalling_connection_active(sys_modem_as_id_e_type as_id)
{
   boolean result = FALSE;
  ...
  {
    MSG_ERROR_DS(MM_SUB, "=MM= Invalid SUB ID:%D", as_id+1,0,0);
    return FALSE;
  }

  if ((is_umts_mode_per_nas_stacks(as_id)) &&
       (ps_session_status_sim[as_id] == MMRRC_ACTIVE) &&
       (mm_timer_status_sim[as_id][TIMER_T3340] == TIMER_ACTIVE))
  {
    MSG_HIGH_DS_1(MM_SUB, "=MM= Hold Data call till PS signalling connection release SUB%d", as_id+1);
    result = TRUE;
  }
#else
  if ((is_umts_mode()) &&
       (ps_session_status == MMRRC_ACTIVE) &&
       (mm_timer_status[TIMER_T3340] == TIMER_ACTIVE))
  {
    MSG_HIGH_DS_1(MM_SUB, "=MM= Hold Data call till PS signalling connection release SUB%d", as_id+1);
    result = TRUE;
  }
...
boolean gmm_is_camped_on_hplmn(void)
{
  ...
  MSG_HIGH_DS_1(MM_SUB,"=MM= got reject cause 7 on HPLMN is %d", ret_value);
...
FUNCTION gmm_get_map_rej_cause7_to_rej_cause14
...
RETURN VALUE
  TRUE:  If nv_map_rej_cause7_to_rej_cause14 is set.
  FALSE: If nv_map_rej_cause7_to_rej_cause14 is not set.

SIDE EFFECTS
  None

===========================================================================*/
boolean gmm_get_map_rej_cause7_to_rej_cause14(void){
   MSG_HIGH_DS_1(MM_SUB,"=MM= Value of mm_map_rej_cause7_to_rej_cause14 is %d", mm_map_rej_cause7_to_rej_cause14);
   return mm_map_rej_cause7_to_rej_cause14;
}

//CMCC GSM Rej7
boolean gmm_get_gsm_map_rej_hplmn_cause7_to_rej_cause14(void)
{
   boolean gmm_map_rej_hplmn_cause7_to_rej_cause14_gsm;
   MSG_HIGH_DS_1(MM_SUB,"=MM= Value of mm_map_rej_cause7_to_rej_cause14 is %d", mm_map_rej_hplmn_cause7_to_rej_cause14);
   gmm_map_rej_hplmn_cause7_to_rej_cause14_gsm = ((mm_map_rej_hplmn_cause7_to_rej_cause14) && (is_gsm_mode()) && gmm_is_camped_on_hplmn());
   return (gmm_map_rej_hplmn_cause7_to_rej_cause14_gsm);
}


boolean gmm_is_dos_mitigation_required_for_ps_domain( byte msg_id,byte reject_cause)
{
   boolean dos_mitigation_reqd = FALSE;
   MSG_HIGH_DS_2(MM_SUB,"USIM gprs counter is -%d and max value of dos mitigation counter is - %d",mm_emm_usim_gprs_counter,mm_emm_max_attempt_counter_dos_mitigation);
...
void gmm_perform_dos_mitigation_action(byte msg_id,byte reject_cause)
{
   if(msg_id == GMM_AUTHENTICATION_AND_CIPHERING_REJECT )
   {
      ...
      mm_add_forbidden_national_roaming( &gmm_current_lai,TRUE );
      mm_add_lai_in_reject_list( &gmm_current_lai,TRUE );
      if( mm_emm_usim_non_gprs_counter < mm_emm_max_attempt_counter_dos_mitigation)
      {
        mm_emm_usim_non_gprs_counter++;
      }
      MSG_HIGH_DS_4(MM_SUB,"=MM= Authentication Rejected Deleting EPLMNs and .USIM GPRS Counter is -%d , non USIM GPRS Counter is -%d,attach attempt counter is -%d,rau attempt counter is -%d", mm_emm_usim_gprs_counter,mm_emm_usim_non_gprs_counter,gprs_attach_attempt_counter,gmm_rau_attempt_counter);
   ...
   else
   {
      switch(reject_cause)
      {
         ...
         case GPRS_SERVICES_NOT_ALLOWED:
            ...
            mm_add_forbidden_national_roaming( &gmm_current_lai,TRUE );
            mm_add_lai_in_reject_list( &gmm_current_lai,TRUE );
            if((reject_cause !=GPRS_SERVICES_NOT_ALLOWED) &&  (mm_emm_usim_non_gprs_counter < mm_emm_max_attempt_counter_dos_mitigation))
            {
              mm_emm_usim_non_gprs_counter++;
            }
            MSG_HIGH_DS_5(MM_SUB,"=MM= Attach rejected with cause %d. Deleting EPLMNs and .USIM GPRS Counter is -%d , non USIM GPRS Counter is -%d,attach attempt counter is -%d,rau attempt counter is -%d", reject_cause,mm_emm_usim_gprs_counter,mm_emm_usim_non_gprs_counter,gprs_attach_attempt_counter,gmm_rau_attempt_counter);
            mm_serving_plmn.update_equiv_plmn_list = TRUE;
            memset(&mm_serving_plmn.equiv_plmn_list, 0x00, sizeof(equiv_plmn_list_s_type));
            gmm_update_mm_serving_plmn_info( mm_serving_plmn.info.sim_state,
                                             SYS_SRV_DOMAIN_NO_SRV,
            ...
            if (mm_reg_waiting_for_reg_cnf)
            {
               MSG_HIGH_DS_0(MM_SUB,"=MM= Sending MMR REG CNF");
               mmcoord_mmr_reg_cnf.cause = MMR_CAUSE_FAILURE_LAI_NOT_ALLOWED;
               mmcoord_mmr_reg_cnf.service_state = mm_serving_plmn.info;
               mm_send_mmr_reg_cnf();
               mm_reg_waiting_for_reg_cnf = FALSE;
            ...
            {
			   mmr_service_ind_s_type mmr_service_ind;
               MSG_HIGH_DS_0(MM_SUB,"=MM= Sending MMR SERVICe IND");
               mmr_service_ind.cause = MMR_CAUSE_FAILURE_LAI_NOT_ALLOWED;
               mmr_service_ind.service_state = mm_serving_plmn.info;
               mm_send_mmr_service_ind( &mmr_service_ind );
            }
         ...
         case LA_NOT_ALLOWED:
            ...
            mm_add_lai_in_reject_list(&gmm_current_lai,TRUE);
#else
            mm_add_forbidden_regional_roaming( &gmm_current_lai,TRUE );
#endif
                break;
         case NATIONAL_ROAMING_NOT_ALLOWED:
            mm_add_forbidden_national_roaming( &gmm_current_lai,TRUE );
         ...
         if (
            ...
            { 
                ...
                gmm_rau_attempt_counter = 0;
                mmcoord_mmr_reg_cnf.cause = MMR_CAUSE_FAILURE_LAI_NOT_ALLOWED;
                MSG_HIGH_DS_3(MM_SUB,"=MM= Attach rejected with cause %d. Deleting EPLMNs. GPRS Attach attempt Counter is -%d, GPRS RAU Attempt counter is -%d", reject_cause,gprs_attach_attempt_counter,gmm_rau_attempt_counter);
                mm_serving_plmn.update_equiv_plmn_list = TRUE;
                memset(&mm_serving_plmn.equiv_plmn_list, 0x00, sizeof(equiv_plmn_list_s_type));
                mm_add_forbidden_national_roaming( &gmm_current_lai,TRUE);
                mm_add_lai_in_reject_list(&gmm_current_lai,TRUE);
                ...
                MSG_HIGH_DS_2(MM_SUB,"=MM= EMM attach attempt counter -%d, EMM TAU attempt counter -%d.", emm_ctrl_data_ptr->emm_attach_attempt_counter,emm_ctrl_data_ptr->emm_tau_attempt_counter);
         ...
         (sys_plmn_match(reg_sim_read_hplmn(), mm_serving_plmn.info.plmn))
            ...
            {
                mmcoord_mmr_reg_cnf.cause = MMR_CAUSE_FAILURE_LAI_NOT_ALLOWED;
                MSG_HIGH_DS_1(MM_SUB,"=MM= Attach rejected with cause %d. Deleting EPLMNs.", reject_cause);
                mm_serving_plmn.update_equiv_plmn_list = TRUE;
                memset(&mm_serving_plmn.equiv_plmn_list, 0x00, sizeof(equiv_plmn_list_s_type));
                mm_add_forbidden_national_roaming( &gmm_current_lai,TRUE );
                mm_add_lai_in_reject_list( &gmm_current_lai,TRUE );
            ...
            {
                reg_sim_add_plmn_to_gprs_fplmn_list( mm_serving_plmn.info.plmn, TRUE
#ifdef FEATURE_DUAL_SIM
                                                    ,(sys_modem_as_id_e_type)mm_sub_id
#endif
                                                   );
...
boolean gmm_per_subs_is_rau_in_progress
(
   sys_modem_as_id_e_type as_id
)
{
   if (gmm_state_sim[as_id] == GMM_ROUTING_AREA_UPDATING_INITIATED)
   {
   	  return TRUE;
   }
   else
...
FUNCTION GMM_HANDLE_RETX_PDU_TUNEAWAY_FAIL_IND

DESCRIPTION
This function processes TUNEAWAY FAIL INDICATION  Message indicating that a NAS message has not been sent out 
so retx needs to be performed for GMM signalling.
...
void gmm_handle_retx_pdu_tuneaway_fail_ind(void)
{
  MSG_HIGH_DS_0(MM_SUB,"Received RR_GMM_RETX_PDU_TUNEAWAY_FAIL_IND - Retry Procedure");
  ...
  { 
    ...
    {
      MSG_HIGH_DS_0(MM_SUB, "=MM= Retry tmer running and no need to retransmit attach");
    }
    else
    { 
  ...
  { 
    if(mm_timer_status[TIMER_T3311] == TIMER_ACTIVE || mm_timer_status[TIMER_T3302] == TIMER_ACTIVE) 
    {
      MSG_HIGH_DS_0(MM_SUB, "=MM= Retry timer running and no need to retransmit RAU");
    }
    else
    { 
      gmm_initiate_routing_area_update(); 
  ...
  {
    MSG_HIGH_DS_0(MM_SUB, "=MM= Reveived RR_GMM_RETX_PDU_TUNEAWAY_FAIL_IND in unexpected GMM State");
  }
}
...
FUNCTION GMM_RESET_MCS_CLIENT_INFO

DESCRIPTION
This function resets stored value of DRX received from MCS.
...
void gmm_reset_mcs_client_info(void)
{
  byte previous_client_id = (byte)(gmm_client_req_drx_coeff.client_id & 0xF0) >> 4;
  if (previous_client_id & (1<<(byte)MM_CLIENT_TYPE_MCS))
  {
    gmm_client_req_drx_coeff.client_id &= 0x0F;
  }
  gmm_client_req_drx_coeff.client_id = (byte)gmm_client_req_drx_coeff.client_id &(~(1<<(byte)MM_CLIENT_TYPE_QMI));
  gmm_client_req_drx_coeff.mcs_requested_drx = SYS_DRX_CN_COEFFICIENT_S1_MAX;
  gmm_client_req_drx_coeff.default_drx = SYS_DRX_CN_COEFFICIENT_S1_MAX;
  mm_stop_timer(TIMER_PROHIBIT_DS_DRX);
}

/*===========================================================================

FUNCTION GMM_CHECK_LTE_ONLY_CLIENT_DRX_NEGOTIATED

DESCRIPTION
This function checks whether MCS/DS requested DRX value is negotiated in LTE
...
FUNCTION GMM_PROCESS_PAGING_CYCLE_SELECTOR

DESCRIPTION
This function processes DRX value received from client QMI/DS/MCS and evaluates the DRX value to be negotiated with network.
...
sys_drx_cn_coefficient_s1_e_type gmm_process_paging_cycle_selector
(
  sys_drx_cn_coefficient_s1_e_type    client_drx,
  cm_client_type_e_type client_id
)
{
  sys_drx_cn_coefficient_s1_e_type negotiated_drx;
  byte previous_client_id = (byte)(gmm_client_req_drx_coeff.client_id & 0xF0) >> 4;

  MSG_HIGH_DS_3(MM_SUB, "=MM= DS client_drx = %d QMI client_drx = %d , MCS client_drx = %d  ", gmm_client_req_drx_coeff.ds_requested_drx, gmm_client_req_drx_coeff.qmi_requested_drx, gmm_client_req_drx_coeff.mcs_requested_drx);
  MSG_HIGH_DS_4(MM_SUB, "=MM= Current DRX = %d NV DRX  = %d ,Defaulr DRX = %d, Old client ID = %x ", gmm_drx_cn_coeff_s1, gmm_nv_drx_cn_coeff_s1, gmm_client_req_drx_coeff.default_drx, gmm_client_req_drx_coeff.client_id);
  
  if (client_id == CM_CLIENT_TYPE_DS)
  {
    gmm_client_req_drx_coeff.client_id &= 0x0F;
    gmm_client_req_drx_coeff.client_id = (byte)(gmm_client_req_drx_coeff.client_id |(1<<(byte)MM_CLIENT_TYPE_DS));
    gmm_client_req_drx_coeff.client_id |=(byte)(1<<(byte)MM_CLIENT_TYPE_DS)<<4;
    gmm_client_req_drx_coeff.ds_requested_drx = client_drx;
  }
  else if (client_id == CM_CLIENT_TYPE_RM_NETWORK_INTERFACE)
  {
    
    gmm_client_req_drx_coeff.client_id &= 0x0F;
    gmm_client_req_drx_coeff.client_id = (byte)(gmm_client_req_drx_coeff.client_id |(1<<(byte)MM_CLIENT_TYPE_QMI));
    gmm_client_req_drx_coeff.client_id |=(byte)(1<<(byte)MM_CLIENT_TYPE_QMI)<<4;
    gmm_client_req_drx_coeff.qmi_requested_drx = client_drx;
  }
  else if (client_id == CM_CLIENT_TYPE_MCS)
  {
    gmm_client_req_drx_coeff.client_id &= 0x0F;
    gmm_client_req_drx_coeff.client_id = (byte)(gmm_client_req_drx_coeff.client_id |(1<<(byte)MM_CLIENT_TYPE_MCS));
    gmm_client_req_drx_coeff.client_id |=(byte)(1<<(byte)MM_CLIENT_TYPE_MCS)<<4;
    gmm_client_req_drx_coeff.mcs_requested_drx = client_drx;
  }

  if (is_lte_mode())
  {
    if ((client_id == CM_CLIENT_TYPE_MCS) && 
         (previous_client_id & (1<<(byte)MM_CLIENT_TYPE_DS))) 
    {
      if ((gmm_client_req_drx_coeff.ds_requested_drx < gmm_client_req_drx_coeff.default_drx) &&
           (gmm_drx_cn_coeff_s1 <= SYS_DRX_CN_COEFFICIENT_S1_CN6_T32))
      {
        mm_start_timer(TIMER_PROHIBIT_DS_DRX, gmm_client_req_drx_coeff.prohibit_timer_value);
        return SYS_DRX_NOT_SPECIFIED_BY_MS;
      }
      else 
      {
        negotiated_drx = MIN (gmm_client_req_drx_coeff.mcs_requested_drx, gmm_client_req_drx_coeff.default_drx);
      }
    }
    else if (client_id == CM_CLIENT_TYPE_DS)
    {
      client_drx = MIN (gmm_client_req_drx_coeff.ds_requested_drx, gmm_client_req_drx_coeff.qmi_requested_drx);
      if (client_drx > gmm_client_req_drx_coeff.default_drx)
      {
        if(gmm_drx_cn_coeff_s1 == client_drx)
        {
      ...
      else if ((gmm_client_req_drx_coeff.client_id & (1<<(byte)MM_CLIENT_TYPE_MCS)) &&
	  	        (gmm_client_req_drx_coeff.ds_requested_drx != SYS_DRX_NOT_SPECIFIED_BY_MS ))
      {
          negotiated_drx = MIN (client_drx, gmm_client_req_drx_coeff.mcs_requested_drx); 
          negotiated_drx =  MAX ( MIN (negotiated_drx, gmm_client_req_drx_coeff.default_drx), SYS_DRX_CN_COEFFICIENT_S1_CN6_T32);
      }
      else
      {
        negotiated_drx =   MIN (client_drx, gmm_client_req_drx_coeff.default_drx);
      }
    }
    else
    {
      negotiated_drx = client_drx;
    }
  }
  else
  {
  ...
  if ((SYS_DRX_NOT_SPECIFIED_BY_MS == gmm_drx_cn_coeff_s1) &&
      (negotiated_drx == gmm_client_req_drx_coeff.default_drx))
  {
    return SYS_DRX_NOT_SPECIFIED_BY_MS;
  }
  
  MSG_HIGH_DS_1(MM_SUB, "=MM= Negotiated DRX = %d",negotiated_drx);
  
  return negotiated_drx;
}

boolean gmm_is_valid_rej_cause_to_del_emm_params(void) 
{
   boolean is_cause_valid = FALSE;
   switch (gmm_reject_cause)
   {   
       case ILLEGAL_MS:
       case ILLEGAL_ME:
       case GPRS_SERVICES_NOT_ALLOWED: 
   ...
   return is_cause_valid;
}

boolean gmm_is_emm_params_to_be_deleted_on_ps_rej(void)
{
   sys_lte_band_mask_e_type lte_band_mask;
   boolean is_emm_params_to_be_deleted = FALSE;
   lte_band_mask = rfm_get_lte_band_mask();

   MSG_HIGH_DS_1(MM_SUB, "=MM= mm_del_emm_params_on_gw_ps_reject %d", mm_del_emm_params_on_gw_ps_reject);

 #if (LTE_BAND_NUM == 256)
 MSG_HIGH_DS_8(MM_SUB,
   "=MM= : LTE bands 1_64: 0x%08x %08x,LTE bands 65_128: 0x%08x %08x "
   "LTE bands 129_192: 0x%08x %08x,LTE bands 193_256: 0x%08x %08x",
   QWORD_HIGH(lte_band_mask.bits_1_64),
   QWORD_LOW(lte_band_mask.bits_1_64),
   QWORD_HIGH(lte_band_mask.bits_65_128),
   QWORD_LOW(lte_band_mask.bits_65_128),
   QWORD_HIGH(lte_band_mask.bits_129_192),
   QWORD_LOW(lte_band_mask.bits_129_192),
   QWORD_HIGH(lte_band_mask.bits_193_256),
   QWORD_LOW(lte_band_mask.bits_193_256));
#else
  MSG_HIGH_DS_2(MM_SUB, "=MM= band_cap = 0x%08x %08x ",
                  QWORD_HIGH(lte_band_mask),
                  QWORD_LOW(lte_band_mask));
#endif
   
  /*if efs is true && rf capability is having LTE && reject cause is whatever mentioned */
   if ((mm_del_emm_params_on_gw_ps_reject) &&
       (SYS_LTE_BAND_MASK_CHK_IF_EMPTY(&lte_band_mask) == FALSE) && 
       (gmm_is_valid_rej_cause_to_del_emm_params()))
   {
      is_emm_params_to_be_deleted = TRUE;
   }

   return is_emm_params_to_be_deleted;
}
