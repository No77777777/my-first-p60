...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/mm/src/emm_rrc_if.c#2 $
...
(
  ...
  /* valid only if use_timer is TRUE */
  ,uint32 search_time /*!< OOS search timer for LTE in ms */
)
{
  ...
  mm_update_wrlf_state(rat_pri_list,emm_ctrl_data_ptr);
#endif
  ...
  mm_set_last_requested_network_selection_mode(rrc_service_req_ptr->network_select_mode);
  /* RPLMN */
...
/* Populate EHPLMN list or HPLMN */
  ...
  if(rrc_service_req_ptr->ehplmn_camping_allowed == FALSE && (req_plmn != NULL) && reg_mode_skip_eplmn_list(req_plmn, (sys_modem_as_id_e_type)mm_as_id) == TRUE)
  {
    rrc_service_req_ptr->eplmn_list.num_plmns = 0;
	MSG_HIGH_DS_0(MM_SUB, "=EMM= EPLMN doesn't need to be filled as hplmn is prioritized over rplmn");
  }
  else
#endif
  {
...
//  rrc_service_req.rat_pri_list.num_items = 0 ;
  ...
  else if (((reg_req_type == MMR_REG_REQ_FULL_RAT_MODE) &&
           (mm_follow_on_request == FALSE))||
           (rrc_service_req_ptr->network_select_mode == SYS_NETWORK_SELECTION_MODE_LIMITED &&
		    emm_bit_mask_check(additional_info,(byte)SYS_ADDTNL_INFO_GEO_DETERMINATION_PHASE) == TRUE))
  {
    MSG_MED_DS_0(MM_SUB, "=EMM= populating LTE_SERV_REQ_TYPE_FULL_RAT_MODE");
    rrc_service_req_ptr->req_type = LTE_RRC_SERVICE_REQ_TYPE_FULL_RAT_MODE;
  }
  else
  {
  ...
  {
    MSG_HIGH_DS_4(MM_SUB, "EMM: Sent LTE_RRC_SERVICE_REQ scan_scope %d, new_scan %d, use timer %d, timer value %d", 
                  scan_scope, rrc_service_req_ptr->new_scan, rrc_service_req_ptr->use_search_timer, rrc_service_req_ptr->lte_search_timer);
  }
  /*--------LOG RRC SERVICE REQ PACKET------*/
  log_lte_rrc_service_request_info(rrc_service_req_ptr,emm_ctrl_data_ptr);
  /* Free memory */
...
)
{
...
#ifdef FEATURE_DUAL_SIM
  mm_update_rat_status(&plmn_search_req_ptr->rat_pri_list_info,
                                SYS_RAT_ACTIVATION_STATUS_ACTIVATED);
#endif
...
#ifndef FEATURE_DISABLED_HABANERO  
  rrc_plmn_search_req_ptr->priority = SYS_SRCH_PRIORITY_NORMAL;
  ...
  {
	if(plmn_search_req_ptr->incremental_results.enabled == TRUE)
    {
      if((plmn_search_req_ptr->srch_priority == SYS_SRCH_PRIORITY_HIGH)
         ||
         (plmn_search_req_ptr->incremental_results.periodicity == 0))
      {
        if(plmn_search_req_ptr->incremental_results.periodicity == 0)
        {
		...
		else
		{
		  ...
		  rrc_plmn_search_req_ptr->search_timer_value.FirstPLMNTimer = 
                             plmn_search_req_ptr->incremental_results.periodicity;
      	  rrc_plmn_search_req_ptr->search_timer_value.MorePLMNTimer = 
                            plmn_search_req_ptr->incremental_results.periodicity;
		}
        if(plmn_search_req_ptr->max_search_time != 0)
        {
          rrc_plmn_search_req_ptr->search_timer_value.MaxManualPLMNTimer = 
                            plmn_search_req_ptr->max_search_time;
      ...
      else
      {
        ...
        {
		  mm_emm_manual_search_timer_set(plmn_search_req_ptr->incremental_results.periodicity,plmn_search_req_ptr->arfcn_list,plmn_search_req_ptr->rat_pri_list_info);
          rrc_plmn_search_req_ptr->search_timer_value = mm_emm_manual_search_timer_value;	
        }
      }
	...
	mm_emm_manual_search_param_set(plmn_search_req_ptr->srch_priority,plmn_search_req_ptr->incremental_results.enabled);
    rrc_plmn_search_req_ptr->priority = plmn_search_req_ptr->srch_priority;
	rrc_plmn_search_req_ptr->frequency_list = plmn_search_req_ptr->arfcn_list;
	rrc_plmn_search_req_ptr->preferred_plmn_list = plmn_search_req_ptr->preferred_plmn_list;
    MSG_HIGH_DS_2(MM_SUB, "=MM= BST changes length of mcc mode filter =%d incremental_results.enabled=%d",
                      plmn_search_req_ptr->mcc_mode_band_filter.num_items,plmn_search_req_ptr->incremental_results.enabled);
...
/* Populate EHPLMN list or HPLMN */
  ...
  {
    if((plmn_search_req_ptr->network_selection_mode == SYS_NETWORK_SELECTION_MODE_MANUAL)
	   && (plmn_search_req_ptr->srch_priority != SYS_SRCH_PRIORITY_LOW))
    {
      emm_ctrl_data_ptr->plmn_search_in_progress = TRUE;
    }
    mm_set_plmn_search_in_progress_flag(TRUE);
    MSG_HIGH_DS_2(MM_SUB, "=EMM= Sent LTE_RRC_PLMN_SEARCH_REQ scan scope %d srch_priority %d",
                  plmn_search_req_ptr->scan_scope,plmn_search_req_ptr->srch_priority);
  }

  if(((plmn_search_req_ptr->network_selection_mode == SYS_NETWORK_SELECTION_MODE_AUTOMATIC 
  	 && (plmn_search_req_ptr->service_search == SYS_SERVICE_SEARCH_NONE))||
  	 (plmn_search_req_ptr->service_search == SYS_SERVICE_SEARCH_IRAT_PLMN)))
  	{

       mm_emm_send_plmn_search_status_to_mcs(TRUE);
  	}
...
)
{
...
#if defined  FEATURE_DUAL_SIM || defined FEATURE_1XSRLTE
  rrc_conn_est_req.proc_type = emm_get_lte_process_type(emm_ctrl_data_ptr);
  emm_ctrl_data_ptr->highest_proc_type =  rrc_conn_est_req.proc_type;
#endif 
  /*reset the memory*/
  ...
  if(emm_message_ptr->nas_hdr.pd == EPS_MOBILITY_MANAGEMENT_MESSAGES)
  {
    emm_ssgccs_handle_outgoing_msg(&emm_message_ptr->emm_outgoing_msg);
  }
  ...
  {
    MSG_HIGH_DS_1(MM_SUB, "=EMM= Sent LTE_RRC_CONN_EST_REQ w/ cause %d",
                  rrc_conn_est_req.est_cause);
...
#ifdef FEATURE_DUAL_SIM
    if(*emm_ctrl_data_ptr->ds_proc_type_ptr != SYS_PROC_TYPE_NONE)
    {
      emm_send_rrc_process_priority(*emm_ctrl_data_ptr->ds_proc_type_ptr,
       ...
       /** --------------------------------------------
      ** Pause timer TIMER_RADIO_CAP_ENQ if it is already running
      ** -------------------------------------------- */
      emm_paused_mm_timer(TIMER_RADIO_CAP_ENQ);
  }
...
#endif	  
  {
	// If LRRC is being deactivated due to dual switch, do not reset this flag if already was set to TRUE  
	// Need to remember to ignore ATTACH ACCEPT with same PTI once LTE comes online again and UE is re-attaching to NW
...
(
  ...
  mm_csfb_state_type              new_csfb_state
) 
{
  ...
  memset((void *)&rrc_csfb_status_ind,0,sizeof(lte_rrc_csfb_call_status_ind_s));
  ...
  //Send CSFB indication to SSGCCS
  emm_update_ssgccs_csfb_call_status(call_status);

#ifdef FEATURE_CMCC_SECURE_CALL
  if(mm_emm_get_secure_call_status() == SYS_SECURE_CALL_START)
  {
    rrc_csfb_status_ind.secure_call_status = LTE_RRC_CSFB_CALL_SECURE_STATUS_ENABLED;
  }
  else 
  {
    rrc_csfb_status_ind.secure_call_status = LTE_RRC_CSFB_CALL_SECURE_STATUS_DISABLED;
  }
  
#endif

  /* initiate direction to NULL */
  rrc_csfb_status_ind.call_direction = LTE_RRC_CSFB_CALL_DIR_NULL;

  if (new_csfb_state == MM_CSFB_STATE_MT_CALL_ORIG)
  {
    rrc_csfb_status_ind.call_direction = LTE_RRC_CSFB_CALL_DIR_MT;
  }
  else if (new_csfb_state == MM_CSFB_STATE_MO_CALL_ORIG)
  {
    rrc_csfb_status_ind.call_direction = LTE_RRC_CSFB_CALL_DIR_MO;
  }
  
  MSG_HIGH_DS_3(MM_SUB, "=EMM= MO/MT = %d, mm_secure_call_enabled_status = %d, call_status = %d",
            rrc_csfb_status_ind.call_direction,rrc_csfb_status_ind.secure_call_status,call_status );
        ...
        if(index != EMM_FORBIDDEN_TAI_CLEAR_TIMER
             &&!(((stop_mode_reason == SYS_STOP_MODE_REASON_IRAT) ||
               ...
               &&(index == TIMER_T3412 || index == TIMER_T3423 ||
                  index == TIMER_T3442 || index == TIMER_T3346 || index == TIMER_RADIO_CAP_ENQ ||
                  index == TIMER_T3246)) &&
...
)
{
  ...
  reg_sim_plmn_list_s_type* fplmn_list_p =NULL;
  uint32 plmn_index = 0;
...
/* Populate EHPLMN list or HPLMN */
  ...
  fplmn_list_p = (reg_sim_plmn_list_s_type *) emm_modem_mem_alloc(sizeof(reg_sim_plmn_list_s_type),emm_ctrl_data_ptr);
  mm_check_for_null_ptr((void *) fplmn_list_p);

#ifdef FEATURE_DUAL_SIM
  (void)reg_sim_read_fplmn_list(fplmn_list_p,TRUE,(sys_modem_as_id_e_type)mm_as_id );
#else
  (void)reg_sim_read_fplmn_list(fplmn_list_p,TRUE);
#endif

  rrc_system_update_req_ptr->fplmn_list.num_plmns = fplmn_list_p->length;
  if(fplmn_list_p->length > LTE_RRC_MAX_FPLMN_LIST_SIZE)
  {
     MSG_HIGH_DS_1(MM_SUB, "=EMM= FPLMN list too long %d - RRC SYTEM UPDATE REQ",
				   fplmn_list_p->length);
     rrc_system_update_req_ptr->fplmn_list.num_plmns = LTE_RRC_MAX_FPLMN_LIST_SIZE;
  }
  else
  {
    rrc_system_update_req_ptr->fplmn_list.num_plmns = fplmn_list_p->length;
  ...
  for(plmn_index=0; plmn_index<rrc_system_update_req_ptr->fplmn_list.num_plmns; plmn_index++)
  {
	rrc_system_update_req_ptr->fplmn_list.plmn[plmn_index] = 
	  emm_convert_nas_plmn_id_to_rrc_plmn_id(fplmn_list_p->plmn[plmn_index]);
  }
  emm_modem_mem_free(fplmn_list_p, emm_ctrl_data_ptr);
  fplmn_list_p = NULL;
  ...
  else
  {
   ...
   /* send none when there is no EMC tai list   
      Send add when there is EMC tai list and NW send EMC bearer update as true 
      send remove when there is tailist and NW send EMC bearer update as False	  */
  if( emm_ctrl_data_ptr->emm_emc_tai_list_ptr != NULL)
  {
    if(emm_ctrl_data_ptr->plmn_service_state->eps_nw_feature_support.emc_bs_supported == TRUE)
    {
      rrc_system_update_req_ptr->forbidden_tai_list.tai_list_action = LTE_RRC_TAI_LIST_REMOVE;
	}
	else
	{
      rrc_system_update_req_ptr->forbidden_tai_list.tai_list_action = LTE_RRC_TAI_LIST_ADD;
	}
	memscpy((void*)&(rrc_system_update_req_ptr->forbidden_tai_list.tai_list), 
             sizeof(lte_nas_emm_tai_lst_info_type),
            (void*)emm_ctrl_data_ptr->emm_emc_tai_list_ptr,
             sizeof(lte_nas_emm_tai_lst_info_type));
			 
	 MSG_HIGH_DS_2(MM_SUB, "=EMM= Sending EMC action = %d and list length is %d ",
	                          rrc_system_update_req_ptr->forbidden_tai_list.tai_list_action,
	                          emm_ctrl_data_ptr->emm_emc_tai_list_ptr->tai_lst_length);
    
  }
  else
  {
    rrc_system_update_req_ptr->forbidden_tai_list.tai_list_action = LTE_RRC_TAI_LIST_NONE;	
  }
#ifdef FEATURE_DISABLED_HABANERO
...
FUNCTION    EMM_SEND_RRC_REG_STATUS_IND

DESCRIPTION
  This function sends the registration status indication
...
===========================================================================*/
void emm_send_rrc_reg_status_ind
(
  emm_ctrl_data_type *emm_ctrl_data_ptr,
  lte_rrc_reg_status_e reg_status,
  uint8               reg_retry_counter

)
{
#ifdef FEATURE_DISABLED_HABANERO

  lte_rrc_reg_status_ind_s rrc_reg_ind;
  static boolean is_send_fail_or_sucess_required = FALSE;
  ASSERT(emm_ctrl_data_ptr != NULL);
  
   MSG_HIGH_DS_1(MM_SUB, "=EMM= [Debug] sending fail or sucess sending is required = %d ",
                                                         is_send_fail_or_sucess_required );
  if(reg_status == LTE_RRC_REG_START || ( is_send_fail_or_sucess_required == TRUE))
  {
    msgr_init_hdr(&rrc_reg_ind.msg_hdr, MSGR_NAS_EMM, LTE_RRC_REG_STATUS_IND) ;
    rrc_reg_ind.reg_status = reg_status;
    rrc_reg_ind.reg_retry_counter = reg_retry_counter;
  
    rrc_reg_ind.phy_cell_id = emm_ctrl_data_ptr->last_rrc_service_ind_ptr->camped_svc_info.phy_cell_id;
    rrc_reg_ind.earfcn      = emm_ctrl_data_ptr->last_rrc_service_ind_ptr->camped_svc_info.earfcn;
    rrc_reg_ind.plmn_id     = emm_ctrl_data_ptr->last_rrc_service_ind_ptr->camped_svc_info.selected_plmn;
    is_send_fail_or_sucess_required = FALSE;
    if(emm_msgr_send((msgr_hdr_s *)&rrc_reg_ind, sizeof(lte_rrc_reg_status_ind_s)) != E_SUCCESS)
    {
      MSG_FATAL_DS(MM_SUB, "=EMM= Failed to send LTE_RRC_REG_STATUS_IND!", 0,0,0 );
    }
    else
    {
      MSG_HIGH_DS_2(MM_SUB, "=EMM= Sent LTE_RRC_REG_STATUS_IND and reg_status is %d reg_retry_counter %d", rrc_reg_ind.reg_status,rrc_reg_ind.reg_retry_counter);
    }
  }
  if( reg_status == LTE_RRC_REG_START )
  {
	  is_send_fail_or_sucess_required = TRUE;
  }
#endif
...
#ifdef FEATURE_DUAL_SIM
	mm_update_rat_status(rat_pri_list_ptr, SYS_RAT_ACTIVATION_STATUS_NONE);
#endif
  }
...
FUNCTION EMM_SEND_RRC_RADIO_CAP_TAU_IND

DESCRIPTION
  This message is sent when NAS performs TAU with radio cap update flag set to TRUE.
...
===========================================================================*/
void emm_send_rrc_radio_cap_tau_ind
(
  void
)
{
  lte_rrc_radio_cap_tau_ind_s radio_cap_tau;


  ASSERT(emm_ctrl_data_ptr != NULL);

    /* Init */
  memset(&radio_cap_tau,0x0,sizeof(lte_rrc_radio_cap_tau_ind_s));
  msgr_init_hdr((msgr_hdr_s *)&radio_cap_tau.msg_hdr, MSGR_NAS_EMM, LTE_RRC_RADIO_CAP_TAU_IND);

  if(emm_msgr_send((msgr_hdr_s *)&radio_cap_tau, 
               sizeof(lte_rrc_radio_cap_tau_ind_s)) == E_SUCCESS)
  {
    MSG_HIGH_DS_0(MM_SUB, "=EMM= Sent LTE_RRC_RADIO_CAP_TAU_IND");
  }
  else
  {
    MSG_ERROR_DS(MM_SUB, "=EMM= Failed to send LTE_RRC_RADIO_CAP_TAU_IND", 0,0,0 );
  }
}
...
)
{
  ...
  umts_key_rsp.valid_keys = is_key_valid;
...
)
{
  ...
  gsm_key_rsp.valid_keys = is_key_valid;
...
FUNCTION    EMM_SEND_RRC_PROCESS_PRIORITY
...
void emm_send_rrc_process_priority
(
  sys_proc_type_e_type  pri_proc_type,
  lte_trm_client_id_e   trm_client_id
)
{
  ...
  rrc_dsds_trm_priority_req.client_id = trm_client_id;
  rrc_dsds_trm_priority_req.proc_type = pri_proc_type;
  ...
  {
    MSG_HIGH_DS_1(MM_SUB, "=EMM= Sent LTE_RRC_DSDS_TRM_PRIORITY_REQ,  Proc Priority = %d",
                  pri_proc_type);
  }
}
...
FUNCTION    emm_send_multimode_subs_chgd_ind

DESCRIPTION
  This function sends the subs capability to LRRC 
...
===========================================================================*/
void emm_send_rrc_multimode_subs_chgd_ind
(
  mmr_multimode_subs_chgd_req_s_type*  subs_chgd_req_ptr
)
{
  lte_rrc_multimode_subs_chgd_ind_s  rrc_multimode_subs_chgd_ind;
  sys_modem_as_id_e_type as_id;
  msgr_init_hdr(&rrc_multimode_subs_chgd_ind.msg_hdr, MSGR_NAS_EMM, LTE_RRC_MULTIMODE_SUBS_CHGD_IND) ;
  for(as_id = SYS_MODEM_AS_ID_1; as_id < MAX_AS_IDS;as_id++)
  {
    rrc_multimode_subs_chgd_ind.subs_capability_mask[as_id] = subs_chgd_req_ptr->subs_capability_mask[as_id];
  }
  if(emm_msgr_send((msgr_hdr_s *)&rrc_multimode_subs_chgd_ind, sizeof(lte_rrc_multimode_subs_chgd_ind_s)) != E_SUCCESS)
  {
    MSG_FATAL_DS(MM_SUB, "=EMM= emm_msgr_send failed!", 0,0,0 );
  } 
  else
  {
    MSG_HIGH_DS_0(MM_SUB, "=EMM= Sent LTE_RRC_MULTIMODE_SUBS_CHGD_IND");
  }
}

/*===========================================================================

FUNCTION    emm_send_rrc_dds_sub_info_ind

DESCRIPTION
  This function sends the current DDS sub info to LTE RRC 
...
===========================================================================*/
void emm_send_rrc_dds_sub_info_ind
(
  sys_modem_as_id_e_type dds_sub
)
{
  lte_rrc_dds_sub_info_ind_s  rrc_dds_sub_info_ind;
  msgr_init_hdr(&rrc_dds_sub_info_ind.msg_hdr, MSGR_NAS_EMM, LTE_RRC_DDS_SUB_INFO_IND) ;
  rrc_dds_sub_info_ind.dds_sub = dds_sub;
  if(emm_msgr_send((msgr_hdr_s *)&rrc_dds_sub_info_ind, sizeof(lte_rrc_dds_sub_info_ind_s)) != E_SUCCESS)
  {
    MSG_FATAL_DS(MM_SUB, "=EMM= emm_msgr_send failed!", 0,0,0 );
  } 
  else
  {
    MSG_HIGH_DS_0(MM_SUB, "=EMM= Sent LTE_RRC_DDS_SUB_INFO_IND");
  }
}
#if !defined FEATURE_MODEM_ATC 

/*=========================================================================== 
 
FUNCTION  EMM_SEND_MANUAL_SEARCH_IND

DESCRIPTION
  This function sends LTE_RRC_SUBX_PLMN_SRCH_STS_IND
...
===========================================================================*/
void emm_send_rrc_manual_search_indication(boolean search_status)//eMBMS
{

#ifdef FEATURE_DISABLED_HABANERO

  lte_rrc_subx_plmn_srch_sts_ind_s  manual_search_ind;

 

  msgr_init_hdr((msgr_hdr_s *)&manual_search_ind.msg_hdr,
                MSGR_NAS_EMM, 
                LTE_RRC_SUBX_PLMN_SRCH_STS_IND); 
  manual_search_ind.status = search_status;
  manual_search_ind.asubs_id = (sys_modem_as_id_e_type)mm_sub_id;
  mm_set_as_id(MM_OTHER_SUB(mm_as_id));
  
  if( mm_sub_is_rat_enabled(SYS_RAT_LTE_RADIO_ACCESS)  == TRUE )
  {
    MSG_HIGH_DS_1(MM_SUB, "=EMM= Sending LTE_RRC_SUBX_PLMN_SRCH_STS_IND status = %d ",search_status);
    if(emm_msgr_send((msgr_hdr_s *)&manual_search_ind,
               sizeof(lte_rrc_subx_plmn_srch_sts_ind_s)) != E_SUCCESS)
    {
      MSG_ERROR_DS_0(MM_SUB, "=EMM= Failed to send LTE_RRC_SUBX_PLMN_SRCH_STS_IND");
    } 
  }
  
  mm_set_as_id(MM_OTHER_SUB(mm_as_id));
#endif
} /* emm_send_rrc_manual_search_indication */
#endif
#ifdef FEATURE_DUAL_DATA
/*===========================================================================

FUNCTION    emm_send_rrc_other_sub_lte_status_ind

DESCRIPTION
  This function sends the LTE rat status of Non DDS sub to DDS sub
...
===========================================================================*/
void emm_send_rrc_other_sub_lte_status_ind
(
...
)
{
  ...
  emm_ctrl_data_ptr = emm_db_get_ctrl_data();
  if(mm_sub_is_rat_enabled(SYS_RAT_LTE_RADIO_ACCESS) &&
     (emm_ctrl_data_ptr->last_non_dds_lte_status != status))
  {
    lte_rrc_other_sub_lte_status_ind_s  rrc_other_sub_lte_status_ind;
    msgr_init_hdr(&rrc_other_sub_lte_status_ind.msg_hdr, MSGR_NAS_EMM, LTE_RRC_OTHER_SUB_LTE_STATUS_IND);
    rrc_other_sub_lte_status_ind.status = status;
    emm_ctrl_data_ptr->last_non_dds_lte_status = status;
    if(emm_msgr_send((msgr_hdr_s *)&rrc_other_sub_lte_status_ind, sizeof(lte_rrc_other_sub_lte_status_ind_s)) != E_SUCCESS)
    {
      MSG_ERROR_DS_0(MM_SUB, "=EMM= emm_msgr_send failed!");
    } 
    else
    {
      MSG_HIGH_DS_1(MM_SUB, "=EMM= Sent LTE_RRC_OTHER_SUB_LTE_STATUS_IND %d",status);
    }
  }
  mm_set_as_id(MM_OTHER_SUB(mm_as_id));
}
...
FUNCTION emm_send_rrc_force_esr_ind

DESCRIPTION
  This function sends ignore irat stutus ind to LTE RRC
...
===========================================================================*/
void emm_send_rrc_force_esr_ind
(
  lte_rrc_force_esr_status_type_e status,
  emm_ctrl_data_type *emm_ctrl_data_ptr
)
{
  lte_rrc_force_esr_ind_s  rrc_force_esr_ind;

  msgr_init_hdr(&rrc_force_esr_ind.msg_hdr, MSGR_NAS_EMM, LTE_RRC_FORCE_ESR_IND) ;

  rrc_force_esr_ind.status = status;

  if(emm_msgr_send((msgr_hdr_s *)&rrc_force_esr_ind, sizeof(lte_rrc_force_esr_ind_s)) != E_SUCCESS)
  {
    MSG_ERROR_DS(MM_SUB, "=EMM= emm_msgr_send failed!", 0,0,0 );
  }
  else
  {
    MSG_HIGH_DS_1(MM_SUB, "=EMM= Sent LTE_RRC_FORCE_ESR_IND status %d",status);
  }
}
#endif
...
#ifdef FEATURE_LTE_REL9
/*===========================================================================
FUNCTION    emm_send_rrc_switch_plmn_req

DESCRIPTION
  This function processes the expiry of emc_backoff_timer.
...
SIDE EFFECTS
  None
===========================================================================*/
void emm_send_rrc_switch_plmn_req
(
  emm_ctrl_data_type *emm_ctrl_data_ptr
)
{
  lte_rrc_plmn_switch_req_s  rrc_plmn_switch_req;

  ASSERT(emm_ctrl_data_ptr != NULL);

  emm_ctrl_data_ptr->switch_plmn = FALSE;
  msgr_init_hdr(&rrc_plmn_switch_req.msg_hdr, MSGR_NAS_EMM, LTE_RRC_PLMN_SWITCH_REQ) ;

  //pick the first PLMN in list
  rrc_plmn_switch_req.switch_plmn = emm_ctrl_data_ptr->emc_shared_plmn_list.shared_plmn[0].plmn;

  if(emm_msgr_send((msgr_hdr_s *)&rrc_plmn_switch_req, sizeof(lte_rrc_plmn_switch_req_s)) != E_SUCCESS)
  {
    MSG_FATAL_DS(MM_SUB, "=EMM= emm_msgr_send failed!", 0,0,0 );
  }
  else
  {
    MSG_HIGH_DS_0(MM_SUB, "=EMM= Sent LTE_RRC_PLMN_SWITCH_REQ");
  }
...
FUNCTION    EMM_SEND_RRC_ELS_CONN_ABORT_REQ

DESCRIPTION
  This function sends ELS  connection abort signalling request to RRC
...
SIDE EFFECTS
  None
===========================================================================*/
void emm_send_rrc_els_conn_abort_req
(
   emm_ctrl_data_type               *emm_ctrl_data_ptr,
   lte_rrc_els_conn_abort_cause_e   cause
)
{
  lte_rrc_els_conn_abort_req_s els_conn_abort_req;
  emm_connection_state_type emm_connection_state = EMM_IDLE_STATE;
      ...
      (emm_ctrl_data_ptr->aborting_conn == FALSE))
  {
    ...
    msgr_init_hdr((msgr_hdr_s *)&els_conn_abort_req.msg_hdr, MSGR_NAS_EMM, LTE_RRC_ELS_CONN_ABORT_REQ) ;
    els_conn_abort_req.cause = cause;
    
	mm_start_timer(TIMER_ELS_CONN_REL, DEFAULT_TIMEOUT);
	
	emm_update_lte_process_type(emm_ctrl_data_ptr); 
    if(emm_msgr_send((msgr_hdr_s *)&els_conn_abort_req, sizeof(lte_rrc_els_conn_abort_req_s)) != E_SUCCESS)
    {
      MSG_FATAL_DS(MM_SUB, "=EMM= emm_msgr_send failed!", 0,0,0 );
    } 
    else
    {
      MSG_HIGH_DS_0(MM_SUB, "=EMM= Sent LTE_RRC_ELS_CONN_ABORT_REQ");
      emm_ctrl_data_ptr->aborting_conn = TRUE;
      /* If EMM is in connected state, move EMM to releasing state. EMM will move
       back to idle state upon receiving release ind and service ind later.
       If EMM is in waiting rrc cnf state, let EMM stay in this state. EMM will
...
#endif
/*Informs LTE whether to begin or end G measurements FR 50563*/

void emm_send_lrrc_g_meas_stop_ind(boolean status)
{
 if(mm_sub_is_rat_enabled(SYS_RAT_LTE_RADIO_ACCESS)  == TRUE)
  {
   lte_rrc_g_meas_stop_ind_s  lte_rrc_g_meas_stop_ind; // struct
   msgr_init_hdr(&lte_rrc_g_meas_stop_ind.msg_hdr, MSGR_NAS_EMM, LTE_RRC_G_MEAS_STOP_IND) ;
   lte_rrc_g_meas_stop_ind.stop_g_meas = status;
   MSG_HIGH_DS_1(MM_SUB,"=MM= sent LTE_RRC_G_MEAS_STOP_IND to LRRC with stop_g_meas %d",status);
   if(emm_msgr_send((msgr_hdr_s *)&lte_rrc_g_meas_stop_ind, sizeof(lte_rrc_g_meas_stop_ind_s)) != E_SUCCESS)
   {
     MSG_FATAL_DS(MM_SUB, "=EMM= emm_msgr_send failed!", 0,0,0 );
   } 
   else
   {
     MSG_HIGH_DS_1(MM_SUB, "=EMM= Sent LTE_RRC_G_MEAS_STOP_IND status %d", status);
   }
   }
}
