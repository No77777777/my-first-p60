...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/tc/src/tctask.c#1 $    $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...
sys_modem_as_id_e_type tc_as_id;

#ifdef FEATURE_DUAL_WCDMA
byte tc_message_as_id = 1;
#endif

rex_tcb_type* tc_tcb_ptr;

test_loopback_mode4_state_T lb_mode4_state; /* This has the current state of loopback Mode4 operation*/
...
void tc_nas_init( void )
{
  ...
  ** --------------------------------------------------------------*/
  ...
  /* Register with DOG HB new API */
   tc_dog_rpt_var = dog_hb_register_rex((rex_sigs_type) TC_DOGRPT_TIMER_SIG);
#endif
...
{                             
  ...
  //Set the AS ID to default value
  tc_as_id = SYS_MODEM_AS_ID_NONE;
  ...
  for (;;)
  {
    ...
    if (sigs & TC_CMD_Q_SIG)
    {
      ...
      {
#ifdef FEATURE_DUAL_WCDMA
        if(tc_get_as_id(tc_cmd_ptr) == TRUE)
        {
#endif
