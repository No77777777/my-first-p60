...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/tc/src/tcgmm.c#1 $    $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...
void send_gmm_tcpdu(byte mesg_type)
{
  ...
  if (gmm_cmd != NULL)
  {
    ...
    gmm_cmd->cmd.gmmtc_unitdata_req.as_id = tc_as_id;
#endif
  ...
  tc_cmd_type *error_ind /*lint -esym(715, error_ind)*/)
{
  ...
  ** --------------------------------------------------------------------*/
      ...
      rb_test_mode == RB_TEST_MODE_PENDING_CM_CONFIRM) 
  {
    ...
    lb_mode4_state = TEST_LOOPBACK_MODE4_NONE; 
  }
}
...
void tc_close_ue_test_loop(byte* recd_msg, byte tc_length)
{
  ...
  else
  {
    ...
    #error code not present
#endif
    else if((recd_msg[2] & 0x03) == 3)
    {
      // Received Loopback Mode 4 setup message
      tc_current_state = TC_LOOPBACK_MODE4;
    }
...
#ifdef FEATURE_SEGMENT_LOADING
          rab_id = IWCDMA_rrc_find_rab_for_rb_in_est_rabs_sub (ptr_W, tc_as_id, rb_id);
#else
          rab_id = rrc_find_rab_for_rb_in_est_rabs_sub (tc_as_id, rb_id);
#endif
...
#else //FEATURE_DUAL_WCDMA
#ifdef FEATURE_SEGMENT_LOADING
          rab_id = ITDSCDMA_tdsrrc_find_rab_for_rb_in_est_rabs_sub (ptr_T, tc_as_id, rb_id);
#else
          rab_id = tdsrrc_find_rab_for_rb_in_est_rabs_sub (tc_as_id, rb_id);
#endif
#endif  //FEATURE_DUAL_WCDMA
        }
#endif /* FEATURE_TDSCDMA */
    ...
    else if ((tc_current_state == TC_LOOPBACK_MODE4) && (tc_rat_rab_established_on == SYS_RAT_UMTS_RADIO_ACCESS))
    {
       MSG_HIGH_DS_0(TC_SUB,"TC_LOOPBACK_MODE4 is being Activated, De-reg WM");
       //De-register WM
#ifdef FEATURE_WCDMA
        lb_deregister_wm_rlc ();
#endif
       //Set the Mode#4 state so that CLOSE UE Test Loop Complete is sent to NW on confirmation from CM and RRC
       lb_mode4_state = TEST_LOOPBACK_MODE4_PENDING_CM_RRC_CONFIRM;
    }
    
    // Notify CM about loopback mode 
    MSG_HIGH_DS_0(TC_SUB,"Notify CM: UE loopback mode");
    tc_cm_notify_lb_mode((byte)tc_current_state, dummy_dcch_enabled); /*lint !e641 */  

    // Notify RRC about loopback mode 
    MSG_HIGH_DS_0(TC_SUB,"Notify RRC: UE loopback mode");
    (void) tc_rrc_notify_lb_mode((byte)tc_current_state, dummy_dcch_enabled); /*lint !e641 */  
...
{  
  if (tc_current_state != TC_UNKNOWN_MODE)
  {
    ...
    MSG_HIGH_DS_0(TC_SUB,"Changing Test mode to INACTIVE");
     ...
     if( tc_current_state == TC_LOOPBACK_MODE4)
     {
        // Inform CM/DS that LB Mode#4 is opened so that DL data can be Discared at DS
        MSG_HIGH_DS_0(TC_SUB,"Notify CM: UE loopback mode OPEN");
        tc_cm_notify_lb_mode(CMTC_UE_OPEN_LB_MODE4, FALSE); 
        lb_mode4_state = TEST_LOOPBACK_MODE4_OPEN; 
     }
...
{  
  ...
  if( lb_mode4_state != TEST_LOOPBACK_MODE4_NONE)
  {
     MSG_HIGH_DS_1(TC_SUB,"Skipping De-Registering WM RLC ::  lb_mode4_state = %d", lb_mode4_state);
     return;
  }
  ...
  for (rab_index=0; rab_index<MAX_RAB_TO_SETUP; rab_index++)
  {
    if (lb_rab_list[rab_index] != NULL )
    {
      ...
      for (lc_index=0 ; lc_index<num_ul_lc ; lc_index++ )
      {
          ...
          lb_rlc_ul_dereg_params.cb_info[lc_index].wm_reg_ind_cb  = NULL;
          lb_rlc_ul_dereg_params.cb_info[lc_index].bearer_context = NULL;
      }
      ...
      for (lc_index=0 ; lc_index<num_dl_lc ; lc_index++ )
      {
          ...
          lb_rlc_dl_dereg_params.cb_info[lc_index].wm_reg_ind_cb  = NULL;
          lb_rlc_dl_dereg_params.cb_info[lc_index].bearer_context = NULL;
      }
      ...
      if (lb_rlc_ul_dereg_params.nchan > 0)
      {
       ...
       l2_ul_cmd->cmd_hdr.cmd_id = RLC_UL_DEREGISTER_SRVC_REQ;
       l2_ul_cmd->cmd_data.ul_dereg = lb_rlc_ul_dereg_params;
#ifdef FEATURE_DUAL_WCDMA
      ...
      if ( lb_rlc_dl_dereg_params.nchan > 0)
      {
       ...
       l2_dl_cmd->cmd_hdr.cmd_id = RLC_DL_DEREGISTER_SRVC_REQ;
       l2_dl_cmd->cmd_data.dl_dereg = lb_rlc_dl_dereg_params;
#ifdef FEATURE_DUAL_WCDMA
...
#else //FEATURE_DUAL_WCDMA
#ifdef FEATURE_SEGMENT_LOADING
        (void) ITDSCDMA_tdsl2_ul_service_register_sub(ptr_T, tc_as_id, &lb_tdsrlc_ul_dereg_params );
#else
        (void) tdsl2_ul_service_register_sub( tc_as_id, &lb_tdsrlc_ul_dereg_params );
#endif
#endif
      }
...
#else //FEATURE_DUAL_WCDMA
#ifdef FEATURE_SEGMENT_LOADING
        (void) ITDSCDMA_tdsl2_dl_service_register_sub(ptr_T, tc_as_id, &lb_tdsrlc_dl_dereg_params );
#else
        (void) tdsl2_dl_service_register_sub( tc_as_id, &lb_tdsrlc_dl_dereg_params );
#endif
#endif
      }
    }
...
void lb_deregister_rab_wm_rlc(byte rab_index)
  ...
  if( lb_mode4_state != TEST_LOOPBACK_MODE4_NONE)
  {
      MSG_HIGH_DS_2(TC_SUB,"Skipping De-Registering WM RLC for RAB Index = %d :: lb_mode4_state = %d", rab_index, lb_mode4_state);
      return;
  }
