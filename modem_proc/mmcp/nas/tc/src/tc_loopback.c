...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/nas/tc/src/tc_loopback.c#1 $ $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...
void lb_register_rlc_wm( byte rab_id )
{
...
#else
#ifdef FEATURE_SEGMENT_LOADING  
  if ( IWCDMA_rrc_return_lc_info_for_rab_sub(ptr_w, tc_as_id, &(lb_rab_list[rab_id]->lc_info_ptr) ) == RRC_RAB_NOT_FOUND )
  {
    ERR( "Invalid RAB id %d", rab_id, 0, 0 ); 
  }
#else
  if ( rrc_return_lc_info_for_rab_sub(tc_as_id, &(lb_rab_list[rab_id]->lc_info_ptr) ) == RRC_RAB_NOT_FOUND )
  {
    ERR( "Invalid RAB id %d", rab_id, 0, 0 ); 
  }
#endif
  ...
  * -----------------------------------------------------------*/
  ...
  for (i=0; i<lb_rlc_ul_reg_params.nchan; i++)
  {
    ...
    lb_rlc_ul_reg_params.rlc_data[ i ].bearer_context = NULL;
    lb_rlc_ul_reg_params.rlc_data[ i ].wm_reg_ind_cb  = NULL;
  ...
  * -----------------------------------------------------------*/
  ...
  for (i=0; i<lb_rlc_dl_reg_params.nchan; i++)
  {
    ...
    lb_rlc_dl_reg_params.rlc_data[ i ].bearer_context = NULL;
    lb_rlc_dl_reg_params.rlc_data[ i ].wm_reg_ind_cb = NULL;
  }
...
#endif
  {
    ERR_FATAL("NULL Ptr when getting DL CMD BUF", 0,0,0);
  }


#ifdef FEATURE_SEGMENT_LOADING
  if ((l2_ul_cmd = IWCDMA_l2_ul_get_cmd_buf(ptr_w)) == NULL)
#else
  if ((l2_ul_cmd = l2_ul_get_cmd_buf()) == NULL)
#endif
  {
    ERR_FATAL("NULL Ptr when getting UL CMD BUF ", 0,0,0);
  }


    /* Populate DL CMD details */
    l2_dl_cmd->cmd_hdr.cmd_id = RLC_DL_REGISTER_SRVC_REQ;
    l2_dl_cmd->cmd_data.dl_reg = lb_rlc_dl_reg_params;
#ifdef FEATURE_DUAL_WCDMA
   l2_dl_cmd->cmd_hdr.as_id = tc_as_id;
#endif
    
    /* Populate UL CMD details */
    l2_ul_cmd->cmd_hdr.cmd_id = RLC_UL_REGISTER_SRVC_REQ;
    l2_ul_cmd->cmd_data.ul_reg = lb_rlc_ul_reg_params;
#ifdef FEATURE_DUAL_WCDMA
   l2_ul_cmd->cmd_hdr.as_id = tc_as_id;
#endif
    
    
  /* Post a commnad to RLC to Register the DS/RLC watermarks with RLC */
#ifdef FEATURE_SEGMENT_LOADING
    IWCDMA_l2_ul_put_cmd(ptr_w, l2_ul_cmd);
...
#ifdef FEATURE_SEGMENT_LOADING  
  if (ITDSCDMA_tdsrrc_return_lc_info_for_rab_sub( ptr_T, tc_as_id, &(lb_rab_list[rab_id]->tdslc_info_ptr) ) ==
        TDSRRC_RAB_NOT_FOUND)
  {
    ERR( "Invalid RAB id %d", rab_id, 0, 0 ); 
  }
#else
  if (tdsrrc_return_lc_info_for_rab_sub( tc_as_id, &(lb_rab_list[rab_id]->tdslc_info_ptr) ) ==
        TDSRRC_RAB_NOT_FOUND)
  {
    ERR( "Invalid RAB id %d", rab_id, 0, 0 ); 
  }
...
#ifdef FEATURE_SEGMENT_LOADING   
  /* Register the DS/RLC watermarks with RLC */
  (void) ITDSCDMA_tdsl2_ul_service_register_sub( ptr_T, tc_as_id, &lb_tdsrlc_ul_reg_params );
  (void) ITDSCDMA_tdsl2_dl_service_register_sub( ptr_T, tc_as_id, &lb_tdsrlc_dl_reg_params );
#else
  /* Register the DS/RLC watermarks with RLC */
  (void) tdsl2_ul_service_register_sub( tc_as_id, &lb_tdsrlc_ul_reg_params );
  (void) tdsl2_dl_service_register_sub( tc_as_id, &lb_tdsrlc_dl_reg_params );
#endif
#endif
...
void lb_register_dl_lc_rlc_wm(byte rab_id, uint8 dl_lc_id)
{
  ...
  lb_rlc_dl_reg_params.rlc_data[ 0 ].bearer_context = NULL;
  lb_rlc_dl_reg_params.rlc_data[ 0 ].wm_reg_ind_cb = NULL;
...
#ifdef FEATURE_DUAL_WCDMA
     cmd_ptr->cmd.lb_proc_dl_rlc_data_ind.as_id = tc_as_id;
#endif
...
#ifdef FEATURE_MODEM_HEAP      
          dsm_data = modem_mem_calloc(1, total_dsm_data_length * sizeof(uint8), MODEM_MEM_CLIENT_NAS);
#else
...
#ifdef FEATURE_MODEM_HEAP      
    lb_from_rlc_data = (uint8*) modem_mem_calloc(1, max_sdu_size * sizeof(uint8), MODEM_MEM_CLIENT_NAS);
    dsm_data = (uint8*) modem_mem_calloc(1, dsm_item_len * sizeof(uint8), MODEM_MEM_CLIENT_NAS);
#else
...
void lb_mode2_rx_handler (uint32 wm_key)
{
  ...
  while ( dsm_item_ptr != NULL )
  {
    ...
    /* Allocate enough memory from tmc heap */
#ifdef FEATURE_MODEM_HEAP      
    lb_from_rlc_data = (uint8*) modem_mem_calloc(1, dsm_item_len * sizeof(uint8), MODEM_MEM_CLIENT_NAS);
#else
...
)
{
  ...
  MSG_LOW_DS_4(TC_SUB, "tx = %d %d %d tx item len = %d bits", lb_from_rlc_data[0], lb_from_rlc_data[1], lb_from_rlc_data[3], lb_tx_item_size); //TC_DSDS_CHANGES
...
SIDE EFFECTS
  None
===========================================================================*/
boolean tc_get_as_id(tc_cmd_type *cmd_ptr)
{
  boolean status = TRUE;
  ...
  switch (cmd_ptr->header.message_set)
  {
    case MS_RABM_TC:
      switch (cmd_ptr->header.cmd_id)
      {
        case RABMTC_RAB_SETUP_IND :
          as_id = cmd_ptr->cmd.rabm_rab_est_ind.as_id;
          break;

        case RABMTC_RAB_REL_IND:
          as_id = cmd_ptr->cmd.rabm_rab_rel_ind.as_id;
          break;
    ...
    case MS_GMM_TC:
      switch (cmd_ptr->header.cmd_id)
      {
        case GMMTC_ERROR_IND :
          as_id = cmd_ptr->cmd.gmm_error_ind.as_id;
          break;

        case GMMTC_UNITDATA_IND:
          as_id = cmd_ptr->cmd.gmm_unitdata_ind.as_id;
          tc_set_as_id(cmd_ptr->cmd.gmm_unitdata_ind.data, 
                        cmd_ptr->cmd.gmm_unitdata_ind.message_len, as_id);
          break;
    ...
    case MS_MM_TC:
      ...
      {
        case MMTC_ERROR_IND :
          as_id = cmd_ptr->cmd.mm_error_ind.as_id;
          break;

        case MMTC_RAB_SETUP_IND:
        case RABMTC_RAB_SETUP_IND:
          as_id = cmd_ptr->cmd.mm_rab_est_ind.as_id;
          break;
          
        case MMTC_RAB_RELEASE_IND : 
        case RABMTC_RAB_REL_IND :
          as_id = cmd_ptr->cmd.mm_rab_rel_ind.as_id;
          break;

        case MMTC_UNITDATA_IND:
          as_id = cmd_ptr->cmd.mm_unitdata_ind.as_id;
          tc_set_as_id(cmd_ptr->cmd.mm_unitdata_ind.data, 
                        cmd_ptr->cmd.mm_unitdata_ind.message_len, as_id);
          break;
    ...
    case MS_CM_TC:
      switch (cmd_ptr->header.cmd_id)
      {
        case CMTC_CHANGE_TEST_MODE_RSP :
          as_id = cmd_ptr->cmd.cm_change_test_mode_rsp.as_id;
          break;

        case CMTC_STOP_MODE_REQ:
          as_id = cmd_ptr->cmd.cm_stop_mode_req.as_id;
          break;

        case CMTC_CLOSE_LB_READY_IND:
          as_id = cmd_ptr->cmd.cm_close_lb_ready_ind.as_id;
          break;
    ...
    case MS_TDSRRC_TC:
      switch (cmd_ptr->header.cmd_id)
      {
        case RRCTC_TEST_LOOPBACK_CNF :
          as_id = cmd_ptr->cmd.rrc_test_loopback_cnf.as_id;
          break;

        case RRCTC_TEST_LOOPBACK_REJ:
          as_id = cmd_ptr->cmd.rrc_test_loopback_rej.as_id;
          break;
    ...
    case MS_TC_TC:
      switch (cmd_ptr->header.cmd_id)
      {
        case LB_PROC_DL_RLC_DATA_CMD :
          as_id = cmd_ptr->cmd.lb_proc_dl_rlc_data_ind.as_id;
          break;
          
    default:
           as_id = tc_as_id; //Assign tc_as_id for internal messages.
         break;
      }
    break;
    default:
        break;
  } /* End of Message Set IF */

    MSG_HIGH_DS_4(TC_SUB, "=TC= tctask received a message, MS:0x%X   MSG_ID:0x%X  MSG_AS_ID:%d TC_AS_ID:%d",
                cmd_ptr->header.message_set, cmd_ptr->header.cmd_id, as_id, tc_as_id);
                
  if(IS_NOT_VALID_AS_ID(as_id) || (as_id != tc_as_id))
  {
    status = FALSE;
    return status;
  }
...
void tc_set_as_id(byte* tc_data, byte tc_length, sys_modem_as_id_e_type  as_id)
{
  byte ext_buf[MAX_OCTETS_L3_MESSAGE];
  byte recd_msg;
  
  //If its not a valid AS ID, stop processing
  ...
  recd_msg = ext_buf[1];
  if( (ACTIVATE_RB_TEST_MODE ==  recd_msg) && (RB_UNKNOWN_MODE == rb_test_mode) )
  {
    tc_as_id = as_id;
    tc_message_as_id = (byte)(tc_as_id + 1);
  }
}

#endif //#ifdef FEATURE_DUAL_WCDMA    
#endif /* FEATURE_TC */
