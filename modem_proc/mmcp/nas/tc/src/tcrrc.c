...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/tc/src/tcrrc.c#1 $ $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...
void tc_process_rrc_cmd(tc_cmd_type *cmd_ptr)
{
   ...
   switch (cmd_ptr->header.cmd_id)
   {
      case RRCTC_TEST_LOOPBACK_CNF: /* Test loopback confirmation */
         ...
         {
           if( lb_mode4_state == TEST_LOOPBACK_MODE4_PENDING_CM_RRC_CONFIRM)
           {
              lb_mode4_state = TEST_LOOPBACK_MODE4_PENDING_CM_CONFIRM;
           }
           else if (lb_mode4_state == TEST_LOOPBACK_MODE4_PENDING_RRC_CONFIRM)
           {
           ...
           {
                ERR( "Received RRCTC_TEST_LOOPBACK_CNF in unexpected Mode#4 state ", 0,0,0);
           }
         }
         else // For TEST LOOPBACK Mode#1, Mode#2 and Mode#3
         {
            tc_close_ue_test_loop_complete();
         }
         break;
...
boolean tc_notify_rrc(byte mode_type)
{
  ...
  if (is_wcdma_mode())
   ...
   if (rrc_cmd_ptr != NULL)
   {
      ...
      rrc_cmd_ptr->cmd.mode_change.as_id = tc_as_id;
#endif

      MSG_HIGH_DS_1(TC_SUB,"Notify RRC_TC_MODE_CHANGE : Mode :0x%X",mode_type);
      ...
      {
#ifdef FEATURE_DUAL_WCDMA
       IWCDMA_rrc_put_tc_cmd_sub(pi_TC_TASK_W,tc_as_id, rrc_cmd_ptr);
#else
...
#ifdef FEATURE_WCDMA
#ifdef FEATURE_DUAL_WCDMA
       rrc_put_tc_cmd_sub(tc_as_id, rrc_cmd_ptr);
#else
...
#ifdef FEATURE_DUAL_WCDMA
      rrc_cmd_ptr->cmd.test_loopback_ind.as_id = tc_as_id;
#endif

       MSG_HIGH_DS_1(TC_SUB,"Notify RRC_TC_TEST_LOOPBACK_IND : LB Mode :0x%X",lb_mode);
...
boolean tc_rrc_notify_lb_mode(byte lb_mode, boolean dummy_signaling_flag)
{
  switch (lb_mode)
  {
     ...
     case TC_LOOPBACK_MODE4:
        return(tc_notify_lb_mode(RRCTC_LB_MODE4, dummy_signaling_flag));
     default:
