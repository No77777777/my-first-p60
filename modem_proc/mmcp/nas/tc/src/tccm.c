...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/tc/src/tccm.c#1 $ $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...
** ----------------------------------------------------------------------- */
extern void tc_close_ue_test_loop_complete(void);
...
/* -----------------------------------------------------------------------
** FUNCTION     : TC_PROCESS_CMTC_CLOSE_LB_READY_IND()

** DESCRIPTION  : This function processes close lb ready ind from CM
** ----------------------------------------------------------------------- */
LOCAL void tc_process_close_lb_ready_ind(boolean);
...
{  
  ...
  case  CMTC_CLOSE_LB_READY_IND:        /* Process stop mode request */
      tc_process_close_lb_ready_ind(cmd_ptr->cmd.cm_close_lb_ready_ind.close_loopback);
      break;  
...
void tc_cm_notify_lb_mode(byte lb_mode, boolean dummy_sig_active)
{
  switch (lb_mode)
  {
     ...
     case TC_LOOPBACK_MODE4:
          tc_cm_notify_mode(CMTC_UE_LB_MODE4);
          break;
     case CMTC_UE_OPEN_LB_MODE4 :
          tc_cm_notify_mode(CMTC_UE_OPEN_LB_MODE4);
          break;
...
void tc_cm_notify_mode(cmtc_ue_mode_type_T mode_type)
{
  ...
  if ((cm_cmd_ptr = cm_get_cmd_buf()) != NULL) /* Get CM command */
  {
     ...
     cm_cmd_ptr->cmd.tc_change_test_mode_ind.as_id = tc_as_id;  
#endif
...
DESCRIPTION
  This function processes close lb ready ind from CM entity, makes to send
  CLOSE UE TEST LOOP COMPLETE for MODE4
...
===========================================================================*/
void tc_process_close_lb_ready_ind(boolean close_loopback)
{

  MSG_HIGH_DS_2(TC_SUB,"Processing close lb ready ind from CM with status : %d, Current LB Mode4 State : %d", close_loopback, lb_mode4_state);
     
  switch(lb_mode4_state)
  {
     case TEST_LOOPBACK_MODE4_PENDING_CM_RRC_CONFIRM: 
         lb_mode4_state = TEST_LOOPBACK_MODE4_PENDING_RRC_CONFIRM;
       break;
 
     case TEST_LOOPBACK_MODE4_PENDING_CM_CONFIRM:
         lb_mode4_state =  TEST_LOOPBACK_MODE4_CLOSED;
         tc_close_ue_test_loop_complete();
       break;

     default: 
       ERR( "Received CMTC_CLOSE_LB_READY_IND in unexpected Mode4 state ", 0,0,0);
       break; 
  }
}
