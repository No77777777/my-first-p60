...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/cnm/src/cnm_cc_process_specific_msgs2.c#1 $    $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...
VOID_FUNC   CNM_process_status_message( connection_id_T connection_id, boolean syntax_ok, byte cause)
{
         ...
         if(cc_IE_info[IE_count].syntax_ok == TRUE)
         {
           switch (IEI)
           {
               ...
               */
               ...
               if ( cnm_qsh_debug_metrics.ota_signaling_ptr != NULL ) 
               {
                 status_msg_T   status_msg;
                 unsigned int   length;

                 GET_IMH_LEN (length, &(status_msg.msg_head));

#ifdef FEATURE_DUAL_SIM
                 status_msg.as_id = get_as_id();
#endif

#ifdef FEATURE_DUAL_SIM
                 length -= 1;
                 cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.as_id = (byte)status_msg.as_id;
#else
                 cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.as_id = SYS_MODEM_AS_ID_NONE;
#endif
                 cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.protocol_discriminator = PROTOCOL_DISCRMINATOR_CC_DEBUG;
                 cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.message_type = (cnm_message_id_debug_type)STATUS_MSG;
                 cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.direction = CN_TO_UE_DIRECTION;
                 cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.length = length;
                 cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.cause_value = 
                                                                  (cnm_cause_debug_type)(*(cc_IE_info[IE_count].start + 3) & 0x7F);
                 cnm_qsh_metric_log_done();
               }
#endif
   ...
   if ( !syntax_ok )
   {
      ERR("Syntax error in Status Message", 0, 0, 0);
...
VOID_FUNC   CNM_process_user_information_message( connection_id_T connection_id, boolean syntax_ok, byte cause) 
{
...
#ifdef FEATURE_DUAL_SIM 
   mncc_user_user_data_ind.as_id = get_as_id();
#endif
   ...
   if ( syntax_ok )
   {
      ...
      if ( cnm_qsh_debug_metrics.ota_signaling_ptr != NULL ) 
      {
        ...
        GET_IMH_LEN (length, &(mncc_user_user_data_ind.message_header));

#ifdef FEATURE_DUAL_SIM
        length -= 1;
        cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.as_id = (byte)mncc_user_user_data_ind.as_id;
#else
        cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.as_id = SYS_MODEM_AS_ID_1;
#endif
        cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.protocol_discriminator = PROTOCOL_DISCRMINATOR_CC_DEBUG;
        cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.message_type = (cnm_message_id_debug_type)USER_INFORMATION;
        cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.direction = CN_TO_UE_DIRECTION;
        cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.length = length;
        cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.cause_value = (cnm_cause_debug_type)RESERVED;
        cnm_qsh_metric_log_done();
      }
#endif
   }
   ...
   {
      ERR("Syntax error in User Information Message", 0, 0, 0);
...
VOID_FUNC   CNM_process_congestion_control_message( connection_id_T connection_id, boolean syntax_ok, byte cause)
{
   ...
   if ( !syntax_ok )
   {
      ERR("Syntax error in Congestion Control msg",  0, 0, 0);
      CNM_send_status ( connection_id,
                           PD_CC,
                           cause );
   }
...
VOID_FUNC   CNM_process_notify_message( connection_id_T connection_id, boolean syntax_ok, byte cause) 
{
...
#ifdef FEATURE_DUAL_SIM 
   mncc_notify_ind.as_id = get_as_id();
#endif
   ...
   if ( syntax_ok )
   {
      ...
      CNM_send_message( (IMH_T *)&mncc_notify_ind, 
               ...
               if ( cnm_qsh_debug_metrics.ota_signaling_ptr != NULL ) 
               {
                 ...
                 GET_IMH_LEN (length, &(mncc_notify_ind.message_header));

#ifdef FEATURE_DUAL_SIM
                 length -= 1;
                 cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.as_id = (byte)mncc_notify_ind.as_id;
#else
                 cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.as_id = SYS_MODEM_AS_ID_1;
#endif
                 cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.protocol_discriminator = PROTOCOL_DISCRMINATOR_CC_DEBUG;
                 cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.message_type = (cnm_message_id_debug_type)NOTIFY;
                 cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.direction = CN_TO_UE_DIRECTION;
                 cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.length = length;
                 cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.cause_value = (cnm_cause_debug_type)RESERVED;
                 cnm_qsh_metric_log_done();
   ...
   {
      ERR("Syntax error in Notify msg", 0, 0, 0);
      ...
      if (bytes_remaining)
      {
         ...
         if (last_state == CC_STATE_U0_6) /* setup due to recall */
         {
             ...
             if ( cnm_qsh_debug_metrics.ota_signaling_ptr != NULL ) 
             {
               ...
               GET_IMH_LEN (length, &(p_setup_msg->msg_head.IMH));

#ifdef FEATURE_DUAL_SIM
               length -= 1;
               cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.as_id = (byte)p_setup_msg->as_id;
#else
               cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.as_id = SYS_MODEM_AS_ID_1;
#endif
               cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.protocol_discriminator = PROTOCOL_DISCRMINATOR_CC_DEBUG;
               cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.message_type = (cnm_message_id_debug_type)p_setup_msg->msg_head.type;
               cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.direction = UE_TO_CN_DIRECTION;
               cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.length = length;
               cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.cause_value = (cnm_cause_debug_type)RESERVED;
               cnm_qsh_metric_log_done();
             }
...
#endif
                   cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.protocol_discriminator = PROTOCOL_DISCRMINATOR_CC_DEBUG;
                   cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.message_type = 
                      (cnm_message_id_debug_type)p_setup_msg->msg_head.type;
                   cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.direction = UE_TO_CN_DIRECTION;
                   cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.length = length;
                   cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.cause_value = (cnm_cause_debug_type)RESERVED;
                   cnm_qsh_metric_log_done(); 
...
VOID_FUNC CNM_process_mt_call_confirm( IMH_T * message )
{
   ...
   if ( action != 0 )
   {
      ...
      if ( cnm_qsh_debug_metrics.ota_signaling_ptr != NULL ) 
      {
        ...
        GET_IMH_LEN (length, &(call_confirmed_msg.msg_head.IMH));
#ifdef FEATURE_DUAL_SIM
        length -= 1;
        cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.as_id = (byte)call_confirmed_msg.as_id;
#else
        cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.as_id = SYS_MODEM_AS_ID_1;
#endif
        cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.protocol_discriminator = PROTOCOL_DISCRMINATOR_CC_DEBUG;
        cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.message_type = 
                                         (cnm_message_id_debug_type)call_confirmed_msg.msg_head.type;
        cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.direction = UE_TO_CN_DIRECTION;
...
VOID_FUNC CNM_process_user_conn_confirmation( IMH_T * message )
{
   ...
   if ( action != 0 )
   {
      ...
      if ( cnm_qsh_debug_metrics.ota_signaling_ptr != NULL ) 
      {
        ...
        GET_IMH_LEN (length, &(data_req_msg.msg_head.IMH));

#ifdef FEATURE_DUAL_SIM
        length -= 1;
        cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.as_id = (byte)data_req_msg.as_id;
#else
        cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.as_id = SYS_MODEM_AS_ID_1;
#endif
        cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.protocol_discriminator = PROTOCOL_DISCRMINATOR_CC_DEBUG;
        cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.message_type = 
                                  (cnm_message_id_debug_type)data_req_msg.msg_head.type;
        cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.direction = UE_TO_CN_DIRECTION;
        cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.length = length;
        cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.cause_value = (cnm_cause_debug_type)RESERVED;
        cnm_qsh_metric_log_done();
      ...
      switch (cc_state)
      {
          ...
          case CC_STATE_U12:
              ...
              CNM_send_primitive_to_mm(connection_id,
                ...
                if ( cnm_qsh_debug_metrics.ota_signaling_ptr != NULL ) 
                {
                  ...
                  GET_IMH_LEN (length, &(p_release_msg->msg_head.IMH));

#ifdef FEATURE_DUAL_SIM
                  length -= 1;
                  cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.as_id = (byte)p_release_msg->as_id;
#else
                  cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.as_id = SYS_MODEM_AS_ID_1;
#endif
                  cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.protocol_discriminator = PROTOCOL_DISCRMINATOR_CC_DEBUG;
                  cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.message_type = 
                     (cnm_message_id_debug_type)p_release_msg->msg_head.type;
                  cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.direction = CN_TO_UE_DIRECTION;
                  cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.length = length;
                  cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.cause_value = (cnm_cause_debug_type)RESERVED;
                  cnm_qsh_metric_log_done(); 
          ...
          case CC_STATE_U6:
              ...
              if ( cnm_qsh_debug_metrics.ota_signaling_ptr != NULL ) 
              {
                ...
                GET_IMH_LEN (length, &(p_release_complete_msg->msg_head.IMH));

#ifdef FEATURE_DUAL_SIM
                length -= 1;
                cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.as_id = (byte)p_release_complete_msg->as_id;
#else
                cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.as_id = SYS_MODEM_AS_ID_1;
#endif
                cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.protocol_discriminator = PROTOCOL_DISCRMINATOR_CC_DEBUG;
                cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.message_type = 
                     (cnm_message_id_debug_type)p_release_complete_msg->msg_head.type;
                cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.direction = CN_TO_UE_DIRECTION;
                cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.length = length;
                cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.cause_value = (cnm_cause_debug_type)RESERVED;
                cnm_qsh_metric_log_done();
         ...
         case CC_STATE_U9:
              ...
              else if(p_mncc_disc_req->data[cause_length] == IEI_FACILITY)
              {
                ...
                if ( cnm_qsh_debug_metrics.ota_signaling_ptr != NULL ) 
                {
                  ...
                  GET_IMH_LEN (length, &(p_data_req_msg->msg_head.IMH));

#ifdef FEATURE_DUAL_SIM
                  length -= 1;
                  cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.as_id = (byte)p_data_req_msg->as_id;
#else
                  cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.as_id = SYS_MODEM_AS_ID_1;
#endif
                  cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.protocol_discriminator = PROTOCOL_DISCRMINATOR_CC_DEBUG;
                  cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.message_type = 
                            (cnm_message_id_debug_type)p_data_req_msg->msg_head.type;
                  cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.direction = UE_TO_CN_DIRECTION;
                  cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.length = length;
                  cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.cause_value = (cnm_cause_debug_type)RESERVED;
                  cnm_qsh_metric_log_done();
...
/*#endif*/
      ...
      if ( cnm_qsh_debug_metrics.ota_signaling_ptr != NULL ) 
      {
        ...
        GET_IMH_LEN (length, &(p_release_complete->msg_head.IMH));

#ifdef FEATURE_DUAL_SIM
        length -= 1;
        cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.as_id = (byte)p_release_complete->as_id;
#else
        cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.as_id = SYS_MODEM_AS_ID_1;
#endif
        cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.protocol_discriminator = PROTOCOL_DISCRMINATOR_CC_DEBUG;
        cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.message_type = 
           (cnm_message_id_debug_type)p_release_complete->msg_head.type;
        cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.direction = UE_TO_CN_DIRECTION;
        cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.length = length;
        cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.cause_value = (cnm_cause_debug_type)RESERVED;
        cnm_qsh_metric_log_done();  
...
{
    ERR("Unexpected MNCC_NOTIFY_REQ msg received", 0, 0, 0);
...
VOID_FUNC CNM_process_SRVCC_HO_complete_indication(IMH_T *CNM_input_primitive)
{
      ...
      if(MNCNM_IS_SINGLE_STACK)
      {
        ...
        mn_message_as_id = (byte)(mn_as_id + 1);
        mncc_srvcc_ho_compl.as_id = mn_as_id;
      }
  ...
  while ( syntax_ok && IE_count<no_of_IEs )
  {
    ...
    if(cc_IE_info[IE_count].syntax_ok == TRUE)
    {
      switch (IEI)
      {
            ...
            *(p_IE + 1) = IE_len -2;
            ...
            CNM_send_message( (IMH_T *)&cc_est_ind, GS_QUEUE_MN_CM );
               ...
               if ( cnm_qsh_debug_metrics.ota_signaling_ptr != NULL ) 
               {
                 ...
                 GET_IMH_LEN (length, &(cc_est_ind.message_header));

#ifdef FEATURE_DUAL_SIM
                 length -= 1;
                 cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.as_id = (byte)cc_est_ind.as_id;
#else
                 cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.as_id = SYS_MODEM_AS_ID_1;
#endif
                 cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.protocol_discriminator = PROTOCOL_DISCRMINATOR_CC_DEBUG;
                 cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.message_type = (cnm_message_id_debug_type)CC_ESTABLISHMENT;
                 cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.direction = CN_TO_UE_DIRECTION;
                 cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.length = length;
                 cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.cause_value = (cnm_cause_debug_type)RESERVED;
                 cnm_qsh_metric_log_done();
        ...
        default:                            /* this cannot happen! */
          ERR("[ccbs]Unexpected IE in cc_est: IEI = %d",(dword)IEI,0,0);
          syntax_ok = FALSE;
          break;
      }
    }
    ...
    if (action != 0/*A_BAD_EVENT*/)
    {
      ...
      CNM_send_primitive_to_mm( connection_id, PD_CC, &cc_est_conf.msg_head.IMH );
               ...
               if ( cnm_qsh_debug_metrics.ota_signaling_ptr != NULL ) 
               {
                 ...
                 GET_IMH_LEN (length, &(cc_est_conf.msg_head.IMH));

#ifdef FEATURE_DUAL_SIM
                 length -= 1;
                 cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.as_id = (byte)cc_est_conf.as_id;
#else 
                 cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.as_id = SYS_MODEM_AS_ID_1;
#endif
                 cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.protocol_discriminator = PROTOCOL_DISCRMINATOR_CC_DEBUG;
                 cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.message_type = 
                     (cnm_message_id_debug_type)cc_est_conf.msg_head.type;
                 cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.direction = CN_TO_UE_DIRECTION;
                 cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.length = length;
                 cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.cause_value = (cnm_cause_debug_type)RESERVED;
                 cnm_qsh_metric_log_done(); 
  ...
  while ( syntax_ok && (IE_count<no_of_IEs) )
  {
    ...
    if(cc_IE_info[IE_count].syntax_ok == TRUE)
    {
      switch (IEI)
      {
          ...
          default:
            MSG_ERROR_DS_1(MN_SUB,"=MNCNM= [ccbs]Unexpected IE in recall msg: IEI=%d",IEI);
            syntax_ok = FALSE;
            break;
      }
    }
  ...
  if (syntax_ok)
  {
    ...
    CNM_send_message( &recall.message_header, GS_QUEUE_MN_CM );
               ...
               if ( cnm_qsh_debug_metrics.ota_signaling_ptr != NULL ) 
               {
                 ...
                 GET_IMH_LEN (length, &(recall.message_header));

#ifdef FEATURE_DUAL_SIM
                 length -= 1;
                 cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.as_id = (byte)recall.as_id;
#else
                 cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.as_id = SYS_MODEM_AS_ID_1;
#endif
                 cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.protocol_discriminator = PROTOCOL_DISCRMINATOR_CC_DEBUG;
                 cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.message_type = (cnm_message_id_debug_type)RECALL;
                 cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.direction = CN_TO_UE_DIRECTION;
                 cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.length = length;
                 cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.cause_value = (cnm_cause_debug_type)RESERVED;
                 cnm_qsh_metric_log_done();
