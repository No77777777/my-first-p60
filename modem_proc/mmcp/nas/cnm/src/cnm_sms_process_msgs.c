...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/cnm/src/cnm_sms_process_msgs.c#1 $    $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...
boolean CNM_SMS_process_message_type ( mmcnm_data_ind_T * SMS_input_primitive )
{
   ...
   if(SMS_input_primitive != NULL)
   {
       ...
       else
       {
           ...
           MSG_ERROR_DS_1(MN_SUB,"=MNCNM= Received SMS message is too small: message length = %d", length);
       }
   }
   else
   {
...
VOID_FUNC CNM_SMS_process_SMS ( IMH_T * SMS_input_primitive )
{
   ...
   */
       ...
       (SMS_input_primitive->message_id == MMCNM_EST_REJ))
   {
       ...
       if((MNCNM_IS_SINGLE_STACK) && ((mmcnm_est_rej_T *)SMS_input_primitive)->cause_value == AS_REJ_ABORT_RADIO_UNAVAILABLE)
       {
         ...
         CNM_send_MNSMS_ERROR_IND( connection_id, (byte)0, AS_REJ_ABORT_RADIO_UNAVAILABLE, mmcnm_est_rej->cause_value,mmcnm_est_rej->cause_type,mmcnm_est_rej->domain);
#else
...
#ifdef FEATURE_SMS_ENHANCED_DOMAIN_SELECTION
           mmcnm_est_rej = (mmcnm_est_rej_T *) ((void *)SMS_input_primitive);
           CNM_send_MNSMS_ERROR_IND( connection_id, (byte)0, EMM_REJ_TEMP_STATE, mmcnm_est_rej->cause_value, mmcnm_est_rej->cause_type, mmcnm_est_rej->domain);
#else
...
#ifdef FEATURE_SMS_ENHANCED_DOMAIN_SELECTION
           mmcnm_est_rej = (mmcnm_est_rej_T *) ((void *)SMS_input_primitive);
           CNM_send_MNSMS_ERROR_IND( connection_id, (byte)0, EMM_REJ_HARD_FAIL, mmcnm_est_rej->cause_value,mmcnm_est_rej->cause_type, mmcnm_est_rej->domain);
#else
...
#ifdef FEATURE_SMS_ENHANCED_DOMAIN_SELECTION
            mmcnm_est_rej = (mmcnm_est_rej_T *) ((void *)SMS_input_primitive);
            CNM_send_MNSMS_ERROR_IND( connection_id, (byte)0, NORMAL_UNSPECIFIED, mmcnm_est_rej->cause_value, mmcnm_est_rej->cause_type,mmcnm_est_rej->domain);
   ...
   */
   ...
   if (SMS_input_primitive->message_id == MMCNM_REL_IND)
   {
	  ...
	  /* when MT call recived on a SUB, ongoing SMS should be aborted on other SUB  */

	  if(sms_link_control.mode != SMS_LINK_CONTROL_DISABLED  &&
         sms_link_control.cp_ack_deferred == TRUE)
      {
         
         MSG_HIGH_DS_0(MN_SUB,"=MNCNM= Stop TLC1M timer");
...
VOID_FUNC CNM_process_mt_sms ( IMH_T * message, connection_id_T connection_id )
{
   ...
   {
      MSG_ERROR_DS_1(MN_SUB,"=MNCNM= Connection id exceeds the maximum permissible value, connection_id = %d",connection_id);
   }
                                         ...
                                         connection_id_T connection_id )
{
   ...
   */
      ...
      (CNM_check_validity_and_process_short_message(connection_id) == TRUE))
   {
      ...
      /* Set SMS type as normal since NAS doesnt distinguish between regular and 911 SMS*/
      mnsms_est_ind.sms_msg_type = 0;

#ifdef FEATURE_DUAL_SIM 
      mnsms_est_ind.as_id = get_as_id();
#endif
      ...
      if(rpdu_len > length)
      {
        rpdu_len = MIN((byte)length, sizeof(mnsms_est_ind.rpdu.data));
      }
