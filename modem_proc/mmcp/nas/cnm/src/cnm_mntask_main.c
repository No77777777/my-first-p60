...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/cnm/src/cnm_mntask_main.c#1 $    $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...
#if defined(FEATURE_QSH_EVENT_METRIC) || defined(FEATURE_QSH_DUMP)
extern void mn_qsh_cb( qsh_client_cb_params_s  *param );
extern void cnm_qsh_cb( qsh_client_cb_params_s *param );
extern void mn_process_qsh_message_ind( IMH_T *msg_header ); 

#ifdef FEATURE_QSH_EVENT_METRIC
#define CNM_QSH_METRIC_DEFAULT_CONFIG_COUNT 2
#define CNM_QSH_METRIC_DEFAULT_SIGNALING_ELEMENT_COUNT 50 

qsh_ext_metric_cfg_s  cnm_qsh_metric_default_cfg_arr[CNM_QSH_METRIC_DEFAULT_CONFIG_COUNT];
#endif

#endif

static ULogHandle mncnm_ulog_handle = NULL;
...
void mn_qsh_register( void )
{
  ...
  client_reg.client        = QSH_CLT_MN;
  client_reg.client_cb_ptr = mn_qsh_cb;
  client_reg.major_ver     = MN_QSH_MAJOR_VER;
  client_reg.minor_ver     = MN_QSH_MINOR_VER;

  client_reg.cb_action_support_mask = (qsh_action_mask_t)QSH_ACTION_DUMP_COLLECT;

  client_reg.dump_info.max_size_bytes = 2*sizeof(mn_qsh_dump_data_s_type);
...
void cnm_qsh_register( void )
{
  ...
  client_reg.client        = QSH_CLT_CNM;
  client_reg.client_cb_ptr = cnm_qsh_cb;
  client_reg.cb_action_support_mask = QSH_ACTION_NONE;
  client_reg.major_ver     = CNM_QSH_MAJOR_VER;
  client_reg.minor_ver     = CNM_QSH_MINOR_VER;
 
#ifdef FEATURE_QSH_DUMP
  client_reg.cb_action_support_mask = (qsh_action_mask_t)QSH_ACTION_DUMP_COLLECT;

  client_reg.dump_info.max_size_bytes = 2*sizeof(cnm_qsh_dump_data_s_type);
#endif

#ifdef FEATURE_QSH_EVENT_METRIC
  client_reg.cb_action_support_mask = (client_reg.cb_action_support_mask|QSH_ACTION_METRIC_CFG);
  qsh_client_metric_cfg_init(cnm_qsh_metric_default_cfg_arr, CNM_QSH_METRIC_DEFAULT_CONFIG_COUNT);

  //Signaling metric enable for AS_ID_1
  cnm_qsh_metric_default_cfg_arr[0].id = CNM_QSH_METRIC_SIGNALING;
  cnm_qsh_metric_default_cfg_arr[0].subs_id = SYS_MODEM_AS_ID_1;
  cnm_qsh_metric_default_cfg_arr[0].fifo.element_size_bytes = sizeof(cnm_qsh_metric_signaling_s);
  cnm_qsh_metric_default_cfg_arr[0].fifo.element_count_total = CNM_QSH_METRIC_DEFAULT_SIGNALING_ELEMENT_COUNT;

  //Signaling metric enable for AS_ID_2
  cnm_qsh_metric_default_cfg_arr[1].id = CNM_QSH_METRIC_SIGNALING;
  cnm_qsh_metric_default_cfg_arr[1].subs_id = SYS_MODEM_AS_ID_2;
  cnm_qsh_metric_default_cfg_arr[1].fifo.element_size_bytes = sizeof(cnm_qsh_metric_signaling_s);
  cnm_qsh_metric_default_cfg_arr[1].fifo.element_count_total = CNM_QSH_METRIC_DEFAULT_SIGNALING_ELEMENT_COUNT;

  client_reg.metric_info.metric_cfg_count = CNM_QSH_METRIC_DEFAULT_CONFIG_COUNT;
  client_reg.metric_info.metric_cfg_arr_ptr = &cnm_qsh_metric_default_cfg_arr[0];
#endif
...
/*===========================================================================
FUNCTION mncnm_get_ulog_handle

DESCRIPTION
Returns ULOG Handle for MN-CNM task
...
SIDE EFFECTS
  None
===========================================================================*/
ULogHandle mncnm_get_ulog_handle(void)
{
  return mncnm_ulog_handle;
}

/*===========================================================================
FUNCTION mncnm_ulog_register

DESCRIPTION
This function call registers MN-CNM Task with ULOG
...
SIDE EFFECTS
  None
===========================================================================*/
void mncnm_ulog_register(void)
{
  ULogResult result = ULogFront_RealTimeInit(&mncnm_ulog_handle,
                               "NAS_MNCNM_ULOG",
                               512,
                               ULOG_MEMORY_LOCAL,
                               ULOG_LOCK_OS);
  MSG_HIGH_DS_1(MN_SUB,"=MNCNM= ULOG REGISTER RESULT: %d", result);
}
...
void mn_cnm_process_internal_messages(void)
{
     ...
     while((cmd_ptr = mn_cnm_cmd_internal_q_get()) != NULL)
     {
         ...
         {
             ERR("Bad message read, status = %d", (dword) status, 0, 0);
         }   /* if status... */
...
#ifdef FEATURE_QSH_EVENT_METRIC
             case MS_MN_QSH:
                 mn_process_qsh_message_ind( (IMH_T *) msg_header ); 
                 break;
#endif
             case MS_CC_MM:
             case MS_CC_MM_PS:
...
static boolean mn_cnm_validate_and_set_as_id(IMH_T *msg_header)
{
  ...
  if(!MNCNM_IS_SINGLE_STACK)
  {
    mn_message_as_id = (byte)(mn_dsda_as_id + 1);
  }
#endif
#endif
   return success;
...
/*
 * Function name :  get_as_id()
 * -------------
 *
 * Description :
 * -----------
 *
 * Uses : This functions returns the as_id value based on device mode
 * ----
 *
 * 
 * ----------
 *
 * Returns: 
 *        - mn_as_id : if  the mode is single stack
 *          mn_dsda_as_id :  if the mode is DUAL_ACTIVE
 *        
 * --------
 *
 */
...
#ifdef FEATURE_DUAL_SIM
sys_modem_as_id_e_type get_as_id(void)
{
  if(MNCNM_IS_SINGLE_STACK)
  {
  ...
  {
    return (sys_modem_as_id_e_type)mn_dsda_as_id;
  }
#endif
