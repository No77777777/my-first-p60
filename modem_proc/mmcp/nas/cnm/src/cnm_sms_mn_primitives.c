...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/cnm/src/cnm_sms_mn_primitives.c#1 $    $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...
VOID_FUNC CNM_send_MNSMS_ERROR_IND( connection_id_T  connection_id,
                                   ...
                                   reject_type_enum_T reject_type,
                                    ...
                                    )
{
   ...
   p_mnsms_error_ind->permanent_error = ((reject_cause == MM_REJ_INVALID_SIM) ||
										...
										(reject_cause == MM_REJ_ACCESS_CLASS_BLOCKED_GSM)||
                                        (reject_cause == MM_REJ_ACCESS_CLASS_BLOCKED_WCDMA)||
                                        (reject_cause == MM_REJ_ACCESS_CLASS_BLOCKED_TDSCDMA)||
                                        (reject_cause == MM_REJ_ACCESS_CLASS_BLOCKED_LTE)||
   ...
   p_mnsms_error_ind->ota_reject.reject_type = reject_type;
   p_mnsms_error_ind->ota_reject.reject_cause = reject_cause;

#ifdef FEATURE_DUAL_SIM 
   p_mnsms_error_ind->as_id = get_as_id();
#endif
...
#ifdef FEATURE_DUAL_SIM 
   p_mnsms_cpack_ind->as_id = get_as_id();
#endif
