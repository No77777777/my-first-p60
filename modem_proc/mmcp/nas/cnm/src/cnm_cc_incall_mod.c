...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/cnm/src/cnm_cc_incall_mod.c#1 $    $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...
VOID_FUNC   CNM_process_modify_message( connection_id_T connection_id, boolean syntax_ok, byte cause)
{
   ...
   mncc_modify_ind.as_id = get_as_id();
#endif
   ...
   if ( syntax_ok != FALSE )
   {
      ...
      CNM_send_message((IMH_T *)&mncc_modify_ind,
               ...
               if ( cnm_qsh_debug_metrics.ota_signaling_ptr != NULL ) 
               {
                 ...
                 GET_IMH_LEN (length, &(mncc_modify_ind.message_header));

#ifdef FEATURE_DUAL_SIM
                 length -= 1;
                 cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.as_id = (byte)mncc_modify_ind.as_id;
#else
                 ...
                 cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.message_type = (cnm_message_id_debug_type)MODIFY;
...
VOID_FUNC   CNM_process_modify_complete_reject_message( connection_id_T connection_id, boolean syntax_ok, byte cause)
{
   ...
   byte                       IE_length;
#ifdef FEATURE_QSH_EVENT_METRIC
   byte                       modify_comp_rej_cause = RESERVED;
#endif
   dword                      len;
...
#ifdef FEATURE_DUAL_SIM 
   mncc_modify_cnf.as_id = get_as_id();
#endif
   ...
   */
           ...
           case IEI_CAUSE:
              {
                  ...
                  *p_write_point = IEI_CAUSE;
                  p_write_point = ((byte *)(p_write_point)) + IE_length;
#ifdef FEATURE_QSH_EVENT_METRIC
                  modify_comp_rej_cause = *(p_IE + 3 ) & 0x7F;
#endif
              }
              break;
   ...
   if ( syntax_ok )
   {
      ...
      if ( cnm_qsh_debug_metrics.ota_signaling_ptr != NULL ) 
      {
        ...
        GET_IMH_LEN (length, &(mncc_modify_cnf.message_header));

#ifdef FEATURE_DUAL_SIM
        length -= 1;
        cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.as_id = (byte)mncc_modify_cnf.as_id;
#else
        cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.as_id = SYS_MODEM_AS_ID_1;
#endif
        cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.protocol_discriminator = PROTOCOL_DISCRMINATOR_CC_DEBUG;
        cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.message_type = 
                        (cnm_message_id_debug_type)mncc_modify_cnf.message_header.message_id;
        cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.direction = CN_TO_UE_DIRECTION;
        cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.length = length;
        cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.cause_value = modify_comp_rej_cause;
        cnm_qsh_metric_log_done();  
      }
#endif
   ...
   if ( action != 0 )
   {
      ...
      while ((length_processed + sizeof(connection_id_T)) < msg_length )
      {
         ...
         else
         {
            ...
            if ( p_ie_cause[0] == IEI_CAUSE )
            {
               fail = TRUE;
#ifdef FEATURE_QSH_EVENT_METRIC
               reject_cause = *(p_ie_cause + 3) & 0x7F;
#endif
               break;
            }
            else
      ...
      if ( fail == FALSE )
      {
         ...
         if ( bytes_remaining != 0 )
         {
           ...
           if ( cnm_qsh_debug_metrics.ota_signaling_ptr != NULL ) 
           {
             ...
             GET_IMH_LEN (length, &(modify_comp_msg.msg_head.IMH));

#ifdef FEATURE_DUAL_SIM
             length -= 1;
             cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.as_id = (byte)modify_comp_msg.as_id;
#else
             cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.as_id = SYS_MODEM_AS_ID_1; 
#endif
             cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.protocol_discriminator = PROTOCOL_DISCRMINATOR_CC_DEBUG;
             cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.message_type = 
                                      (cnm_message_id_debug_type)modify_comp_msg.msg_head.type;
             cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.direction = UE_TO_CN_DIRECTION;
             cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.length = length;
             cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.cause_value = (cnm_cause_debug_type)RESERVED;
             cnm_qsh_metric_log_done();
      ...
      else
      {
         ...
         if ( cnm_qsh_debug_metrics.ota_signaling_ptr != NULL ) 
         {
           ...
           GET_IMH_LEN (length, &(modify_rej_msg.msg_head.IMH));

#ifdef FEATURE_DUAL_SIM
           length -= 1;
           cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.as_id = (byte)modify_rej_msg.as_id;
#else
           cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.as_id = SYS_MODEM_AS_ID_1;
#endif
           cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.protocol_discriminator = PROTOCOL_DISCRMINATOR_CC_DEBUG;
           cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.message_type = 
                                    (cnm_message_id_debug_type)modify_rej_msg.msg_head.type;
           cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.direction = UE_TO_CN_DIRECTION;
           cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.length = length;
           cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.cause_value = reject_cause;
           cnm_qsh_metric_log_done();
         }
#endif
