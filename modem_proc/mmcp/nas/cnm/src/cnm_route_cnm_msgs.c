...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/cnm/src/cnm_route_cnm_msgs.c#1 $    $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
            ...
            case MMCNM_DATA_IND:
               ...
               if (message_len >= 2) /* message does not contain type IE */
               {
...
#if defined(FEATURE_DUAL_SIM )
                  {
                    ...
                    if(MNCNM_IS_SINGLE_STACK)
                    {
                      /* Below condition is to check if any service is going parallely on other sub in DSDS mode */
                      if( (mn_as_id == SYS_MODEM_AS_ID_NONE) || (local_as_id == mn_as_id))
                      {
                         success = TRUE;
                      }
                      else
...
#endif
                  if((local_as_id > SYS_MODEM_AS_ID_NONE && local_as_id < MAX_AS_IDS) && (success == TRUE))
                  {
                  ...
                  {
                    MSG_ERROR_DS_1(MN_SUB,"=MNCNM= invalid as_id =%d", local_as_id);
                  }
               }
               else
               {
