...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/cnm/src/cnm_ss_process_msgs.c#1 $    $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...
#ifdef FEATURE_DUAL_SIM 
                mncc_facility_ind.as_id = get_as_id();
#endif
...
#ifdef FEATURE_DUAL_SIM 
   mncc_call_hold_cnf.as_id = get_as_id();
#endif
...
#ifdef FEATURE_DUAL_SIM 
   mncc_call_hold_rej.as_id = get_as_id();
#endif
...
#ifdef FEATURE_DUAL_SIM 
   mncc_call_retrieve_cnf.as_id = get_as_id();
#endif
...
#ifdef FEATURE_DUAL_SIM 
   mncc_call_retrieve_rej.as_id = get_as_id();
#endif
...
VOID_FUNC CNM_process_mo_facility ( IMH_T * SS_input_primitive )
{
   ...
   {
      ERR("Unable to send Facility msg", 0, 0, 0);
#if defined(FEATURE_DUAL_SIM )
      if(MNCNM_IS_SINGLE_STACK)
      {
...
VOID_FUNC   CNM_process_mt_facility(  IMH_T *SS_input_primitive )
{
      ...
      if ((type == FACILITY) || (type == RELEASE_COMPLETE))
      {
...
#ifdef FEATURE_DUAL_SIM 
            mnss_facility_ind.as_id = get_as_id();
#endif
...
#ifdef FEATURE_DUAL_SIM 
            GET_IMH_LEN (length, &(((mmcnm_data_ind_T *) ((void *)SS_input_primitive))->msg_head.IMH));
            if(MNCNM_IS_SINGLE_STACK)
            {
         ...
         switch (type)
         {
            case FACILITY:
               ...
               {
                  ERR("Syntax error in Facility msg", 0, 0, 0);
   ...
   {
      MSG_ERROR_DS_1(MN_SUB,"=MNCNM= Connection id should not be %d...ignored", connection_id);
   }
}
