...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/cnm/src/cnm_mntask_lib.c#1 $    $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...
void mn_cnm_put_cmd(mn_cnm_cmd_type *cmd_ptr)
{
   ...
   else
   {
       ...
       MSG_FATAL_MN_DS(MN_SUB,"=MNCNM= NULL Command pointer passed!!",0,0,0);
...
void mn_cnm_put_internal_cmd(mn_cnm_cmd_type *cmd_ptr)
{
   ...
   {
       /* Raise error condition */
       ERR_FATAL("NULL Command pointer passed!!",0,0,0);
