...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/cnm/src/cnm_sms_send_msgs.c#1 $    $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
   ...
   else
   {
      ...
      if ( current_state == SMS_MM_CONN_EST )
      {
        ...
        if(CNM_mt_cp_message[connection_id-8] == NULL)
        {
          ...
          MSG_HIGH_DS_1(MN_SUB,"=MNCNM= ALLOC - CNM_mt_cp_message[%d]", (connection_id-8));
          if(CNM_mt_cp_message[connection_id-8] == NULL)
          {
            MSG_FATAL_MN_DS(MN_SUB,"=MNCNM= Memory allocation failure for CNM_mt_cp_message[%d]", (connection_id-8), 0, 0 );
          }
          else
          {
...
byte CNM_send_cp_data ( connection_id_T connection_id )
{
   ...
   else                             /* its MT */
   {
      ...
      if(message == NULL)
      {
         MSG_FATAL_MN_DS(MN_SUB,"=MNCNM= CNM_mt_cp_message[%d] is NULL", (connection_id-8), 0, 0 );
      }
...
#ifdef FEATURE_DUAL_SIM 
   mnsms_data_ind.as_id = get_as_id();
#endif
...
VOID_FUNC CNM_send_error_indication_to_rp_layer ( connection_id_T connection_id, byte lower_layer_cause )
{
   ...
   CNM_send_MNSMS_ERROR_IND( connection_id, cp_cause_value, lower_layer_cause, 0,0, domain);
#else
