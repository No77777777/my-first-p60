...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/cnm/src/cnm_cc_receive_msg_utils.c#1 $    $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
                            ...
                            byte *last_octet_data_pointer)
{
   ...
   if ( no_of_IEs < MAXNO_IES_IN_MESSAGE )
   {
      ...
      if ( no_of_IEs >= 2 )
      {
         ...
         MSG_LOW_DS_1(MN_SUB,"=MNCNM= IE> %s", DEBUG_identify_IE(IEI));
#else
         MSG_LOW_DS_1(MN_SUB,"=MNCNM= IE> %#X",IEI);
#endif
      }
   ...
   {
      ERR("Too many IEs in message - ignored", 0, 0, 0);
   }
}
...
byte get_no_of_IEs( mn_dsda_as_id_e_type as_id )
{
#if defined(FEATURE_DUAL_SIM) && defined(FEATURE_DUAL_ACTIVE)
  return no_of_IEs_sim[as_id];
#else
  return no_of_IEs;
#endif
}
