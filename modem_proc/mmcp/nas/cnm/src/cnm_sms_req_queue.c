...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/cnm/src/cnm_sms_req_queue.c#1 $    $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
                                        ...
                                        connection_id_T connection_id )
{
   ...
   if ( write_ptr != sms_queue_read_ptr )
   {
      ...
      MSG_HIGH_DS_1(MN_SUB,"=MNCNM= ALLOC - sms_request_queue[%d]", sms_queue_write_ptr );
      ...
      {
           MSG_FATAL_MN_DS(MN_SUB,"=MNCNM= Memory allocation failure for sms_request_queue[%d]", sms_queue_write_ptr, 0, 0 );
      }
   ...
   else
   {
      ...
      if(sms_request_queue[sms_queue_read_ptr] != NULL)
      {
        ...
        *p_connection_id = sms_request_queue[sms_queue_read_ptr]->connection_id;
        ...
        MSG_HIGH_DS_1(MN_SUB,"=MNCNM= FREE - sms_request_queue[%d]", sms_queue_read_ptr);
      }
      else
      {
