...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/cnm/src/cnm_send_status_msg_to_mm.c#1 $    $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
                             ...
                             byte cause)
{
   ...
   if(connection_id < MAXNO_CC_TRANSACTIONS) 
   {
      ...
      *(p_write_point) = (unsigned char)(cause | 0x80);
      ...
      if ( cnm_qsh_debug_metrics.ota_signaling_ptr != NULL ) 
      {
        ...
        cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.cause_value = (cnm_cause_debug_type)cause;
   ...
   {
      MSG_ERROR_DS_1(MN_SUB,"=MNCNM= Received Invalid STATUS_ENQUIRY = %d", connection_id);
   }
}
