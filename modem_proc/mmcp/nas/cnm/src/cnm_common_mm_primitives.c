...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/cnm/src/cnm_common_mm_primitives.c#1 $    $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...
VOID_FUNC CNM_fill_mmcnm_data_req(mmcnm_data_req_T* mmcnm_data_req)
{
  ...
  if(MNCNM_IS_SINGLE_STACK)
  {
     ...
     {
       if( ss_transaction_information[connection_id].as_id != SYS_MODEM_AS_ID_NONE)   
       {
     ...
     {
       if( sm_rl_transaction_information[connection_id].as_id != SYS_MODEM_AS_ID_NONE)
       {
     ...
     {
       MSG_ERROR_DS_1(MN_SUB,"=MNCNM= Invalid PD = %d",(dword) (GET_PD(mmcnm_data_req->data[0])));
     }
  }
#ifdef FEATURE_DUAL_ACTIVE
  else
