...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/cnm/src/cnm_cc_process_msgs.c#1 $    $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
         ...
         switch (CNM_input_primitive->message_id)
         {
            case MMCNM_DATA_IND:
                        ...
                        ((p_mmcnm_data_ind->msg_head.type & 0x80) != 0))
               {
                    ...
                    * for the PD
                    */
                   MSG_ERROR_DS_2(MN_SUB,"=MNCNM= Invalid MSG TYPE or N(SD)=1: %d", connection_id,
            ...
            if ( GS_IS_NOT_UNDEFINED(cc_event) )
            {
              ...
              if (cc_event == CC_MODIFY_event)
              {
               ...
               else
               {
                  ...
                  if((cc_event == CC_PROGRESS_event)&&
                      ((cc_transaction_information_p[connection_id] != NULL) &&
                        ( cc_transaction_information_p[connection_id]->cc_state
                                                                      == CC_STATE_U10 )))
                  {
                    MSG_HIGH_DS_0(MN_SUB,"=MNCNM= Ignoring Progress message during Active call"); /* ignore message */
                  }
                  else
                     ...
                     if (p_IE[1] > 0)
                     {
                        ...
                        if (((p_IE[2]) & 0x60) == STANDARD_GSM )
                        {
                           ...
                           while (((*p_cause_value) & 0x80) == 0)
                           {
                              ...
                              if ((count--) == 0)
                              {
                                 ERR("Bad Cause IE in Status msg", 0, 0, 0);
                                 break;
                              }
                           }
                           p_cause_value++;
