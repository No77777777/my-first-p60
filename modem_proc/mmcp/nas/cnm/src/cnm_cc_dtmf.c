...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/cnm/src/cnm_cc_dtmf.c#1 $    $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...
VOID_FUNC CNM_clear_dtmf_queue(connection_id_T connection_id)
{
  ...
  if(connection_id < MAXNO_CC_TRANSACTIONS)
  {
    ...
    { 
      current_cc_dtmf_state = CNM_get_dtmf_state(connection_id);
       ...
       if(cc_transaction_information_p[connection_id] != NULL)
       {
           ...
           )
         {
           ...
           mncc_start_dtmf_cnf.connection_id = connection_id;
            ...
            mncc_start_dtmf_cnf.as_id = get_as_id();
#endif
                ...
                )  
         {
           ...
           mncc_stop_dtmf_cnf.connection_id = connection_id;
#if defined(FEATURE_DUAL_SIM)
           mncc_stop_dtmf_cnf.as_id = get_as_id();
#endif
                                 ...
                                 dtmf_queue_entry_T *dtmf_queue_entry)
{
    ...
    {
       MSG_HIGH_DS_1(MN_SUB,"=MNCNM= Invalid conn_id = %d passed to CNM_pop_dtmf_queue_entry()", connection_id );
    }
    return(return_value);
...
VOID_FUNC   CNM_process_start_dtmf_ack_message( connection_id_T connection_id, boolean syntax_ok, byte cause)
{
    ...
    mncc_start_dtmf_cnf.dtmf_success = TRUE;
        ...
        * get information of the next IE from global store
        ...
        if (!syntax_ok)
        {
           ERR("Syntax error in Start DTMF Ack Message", 0, 0, 0);
            ...
            && (cc_transaction_information_p[connection_id]->hold_auxiliary_state != HOLD_RETRIEVE_REQUEST))
        {
            CNM_send_message( (IMH_T *)&mncc_start_dtmf_cnf,
               ...
               if ( cnm_qsh_debug_metrics.ota_signaling_ptr != NULL ) 
               {
                 ...
                 GET_IMH_LEN (length, &(mncc_start_dtmf_cnf.message_header));

#ifdef FEATURE_DUAL_SIM
                 length -= 1;
                 cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.as_id = (byte)mncc_start_dtmf_cnf.as_id;
#else
                 cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.as_id = SYS_MODEM_AS_ID_1;
#endif
                 cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.protocol_discriminator = PROTOCOL_DISCRMINATOR_CC_DEBUG;
                 cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.message_type = (cnm_message_id_debug_type)START_DTMF_ACK;
...
VOID_FUNC   CNM_process_start_dtmf_rej_message( connection_id_T connection_id, boolean syntax_ok, byte cause)
{
    ...
    if (CNM_get_dtmf_state(connection_id) == WAIT_FOR_START_DTMF_ACK)
    {
        ...
        {
            MSG_ERROR_DS_2(MN_SUB,"=MNCNM= Removed related STOP DTMF on T336 expired=%d, cmd=%d", connection_id, 
        ...
        * get information of the next IE from global store
...
#ifdef FEATURE_QSH_EVENT_METRIC
                  p_ie_cause = &cc_IE_info[IE_count].IEI;
                  dtmf_rej_cause = *(p_ie_cause + 3) & 0x7F;
#endif
                  break;
        ...
        if (!syntax_ok)
        {
            ERR("Syntax error in Start DTMF Rej Message ", 0, 0, 0);
            CNM_send_status ( connection_id,
                    PD_CC,
                    cause );
        }
...
#endif
                 cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.protocol_discriminator = PROTOCOL_DISCRMINATOR_CC_DEBUG;
                 cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.message_type = (cnm_message_id_debug_type)START_DTMF_REJ;
                 cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.direction = CN_TO_UE_DIRECTION;
                 cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.length = length;
                 cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.cause_value = dtmf_rej_cause;
                 cnm_qsh_metric_log_done(); 
               }
#endif
            ...
            && (cc_transaction_information_p[connection_id]->hold_auxiliary_state != HOLD_RETRIEVE_REQUEST))
        {
            ...
            GS_QUEUE_MN_CM );
               ...
               if ( cnm_qsh_debug_metrics.ota_signaling_ptr != NULL ) 
               {
                 ...
                 GET_IMH_LEN (length, &(mncc_stop_dtmf_cnf.message_header));

#ifdef FEATURE_DUAL_SIM
                 length -= 1;
                 cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.as_id = (byte)mncc_stop_dtmf_cnf.as_id;
#else
                 cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.as_id = SYS_MODEM_AS_ID_1;
#endif
                 cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.protocol_discriminator = PROTOCOL_DISCRMINATOR_CC_DEBUG;
                 cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.message_type = (cnm_message_id_debug_type)STOP_DTMF_ACK;
                 cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.direction = CN_TO_UE_DIRECTION;
                 cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.length = length;
                 cnm_qsh_debug_metrics.ota_signaling_ptr->metric_sig.cause_value = (cnm_cause_debug_type)RESERVED;
                 cnm_qsh_metric_log_done();
...
VOID_FUNC CNM_read_dtmf_queue( connection_id_T connection_id )
{
    ...
    {
        MSG_ERROR_DS_1(MN_SUB,"=MNCNM= DTMF queue empty for connection_id = %d", connection_id);
    }
}
