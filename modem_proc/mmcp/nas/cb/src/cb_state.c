...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/cb/src/cb_state.c#1 $ $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
05/19/15   jbk     Read CB NV items irrespective of BMC feature flag 
...
sys_srv_status_e_type cb_service_sim[MAX_AS_IDS] = {SYS_SRV_STATUS_NO_SRV,SYS_SRV_STATUS_NO_SRV};
#define   cb_service cb_service_sim[cb_as_id]

boolean cb_reg_inactive_sim[MAX_AS_IDS] = {TRUE,TRUE};

#ifdef FEATURE_UMTS_BMC_MSIM
boolean   cb_dds_info_sim[MAX_AS_IDS] = {FALSE,FALSE};
#define   cb_dds_info cb_dds_info_sim[cb_as_id]
#endif
...
sys_srv_status_e_type cb_service = SYS_SRV_STATUS_NO_SRV;
boolean cb_reg_inactive = TRUE;

#ifdef FEATURE_UMTS_BMC_MSIM
boolean  cb_dds_info = FALSE;
#endif

#endif

cb_debug_buffer_s_type cb_debug_buffer[CB_DEBUG_MESSAGE_BUFFER_SIZE];
dword cb_debug_buffer_index = 0;
dword cb_msg_info = 0;
byte cb_scheduling_message[2][CB_BMC_MAX_RLC_DATA_LENGTH];
byte cb_schedule_index = 1;
...
#if defined (FEATURE_UMTS_BMC_MSIM )

static cb_etws_state_type cb_etws_state_sim[MAX_AS_IDS] = {{CB_IDLE_STATE, CB_ETWS_IDLE_INACTIVE },{CB_IDLE_STATE, CB_ETWS_IDLE_INACTIVE }};
#define cb_etws_state cb_etws_state_sim[cb_as_id]

static boolean cb_etws_ctch_drx_timer_running_sim[MAX_AS_IDS] ={FALSE,FALSE};
#define cb_etws_ctch_drx_timer_running cb_etws_ctch_drx_timer_running_sim[cb_as_id]

static cb_etws_idle_substate_id_type cb_etws_idle_substate_sim[MAX_AS_IDS];
#define cb_etws_idle_substate  cb_etws_idle_substate_sim[cb_as_id]

cb_etws_substate_control_type cb_etws_substate_control_sim[MAX_AS_IDS] ={cb_etws_idle_substate_control,cb_etws_idle_substate_control};
#define cb_etws_substate_control  cb_etws_substate_control_sim[cb_as_id]
...
word cb_bmc_schedule_msg_periodicity_sim[MAX_AS_IDS]= {0,0};
#define cb_bmc_schedule_msg_periodicity cb_bmc_schedule_msg_periodicity_sim[cb_as_id]
...
static boolean cb_activate_cb_due_to_service(sys_srv_status_e_type cb_srv_val)
{
  if( cb_srv_val == SYS_SRV_STATUS_SRV )
  {
    return TRUE;
  }
  else
 ...
 */
   ...
   if ((entry_found) &&
       ((next_substate != cb_etws_idle_substate) ||
        (action != CB_ACTION_0)))
   {
      MSG_HIGH_DS_3(CB_SUB, "CB ETWS IDLE substate change from %d to %d, action:%d", cb_etws_idle_substate, next_substate, action );
...
#ifdef FEATURE_NO_CBS_LTD
           if (cb_supported && cb_activate_cb_due_to_service(cb_service))
#else
           if (cb_supported)
#endif
           {
...
)
{
  ...
  MSG_HIGH_DS_1(CB_SUB, "CB ETWS ACTIVE event:%d", cb_event);
  ...
  switch (cb_event)
  {
     ...
     case CB_ETWS_STD_DRX_ACTIVATED:
        ...
        */
        ...
        {
          MSG_HIGH_DS_0(CB_SUB,"CB ETWS DRX timer is not started as its already running ");
        }
       break;
...
)
{
             ...
             (next_cb_state.state != CB_ACTIVE_STATE)))
  {
    ...
    {
      MSG_HIGH_DS_0(CB_SUB,"BMC: BMC is already suspended no need to deactivate the WL1");
  ...
  {
    MSG_HIGH_DS_2(CB_SUB,"CB ETWS state change from %d to %d",cb_etws_state.state,next_etws_state);
    ...
    case MS_WMS_CB :
      switch(cb_incoming_primitive->command.message_header.message_id)
      {
        case CB_SEARCH_LIST_REQ :
          ...
          cb_msg_info = cb_incoming_primitive->command.search_list_req.search_list_length;
          break;
        case CB_DELETE_PAGE_REQ :
          as_id = cb_incoming_primitive->command.delete_page_req.as_id;
          cb_msg_info = cb_incoming_primitive->command.delete_page_req.message_code;
          break;
        ...
        case CB_DUAL_STANDBY_STATUS_IND :
          return FALSE;
        default :
          MSG_FATAL_DS( CB_SUB,"=CB= Unexpected WMS_CB cmd %d", cb_incoming_primitive->command.message_header.message_id, 0,0 );
      }
      break;
    case MS_CB_RR:
      switch(cb_incoming_primitive->command.message_header.message_id)
      {
        case RR_CELL_CHANGE_IND :
          ...
          cb_msg_info = cb_incoming_primitive->command.cell_change_ind.CBCH_present;
          break;
        default :
          MSG_FATAL_DS( CB_SUB,"=CB= Unexpected CB RR cmd %d", cb_incoming_primitive->command.message_header.message_id, 0,0 );
      }
      break;
#ifdef FEATURE_UMTS_BMC
    case MS_RRC_BMC :  
      switch(cb_incoming_primitive->command.message_header.message_id)
      {
        case RRC_CELL_CHANGE_IND :
          ...
          cb_msg_info = cb_incoming_primitive->command.rrc_cell_change_ind.ctch_is_supported;
          break;
        case RRC_ETWS_PRIM_NOTIFICATION_IND:
          as_id = cb_incoming_primitive->command.rrc_etws_prim_notification_ind.as_id; 
          cb_msg_info = cb_incoming_primitive->command.rrc_etws_prim_notification_ind.msg_id[0]<<8 | cb_incoming_primitive->command.rrc_etws_prim_notification_ind.msg_id[1];
          break;  
        default :
          MSG_FATAL_DS( CB_SUB,"=CB= Unexpected RRC BMC cmd %d", cb_incoming_primitive->command.message_header.message_id, 0,0 );
      }
      break;
...
#endif
#else
      if (cb_incoming_primitive->command.message_header.message_id == DL_BMC_BLOCK_IND)
      {
        as_id = cb_incoming_primitive->command.bmc_block_ind.as_id;
      }
      else
      {
        MSG_HIGH_DS_1(CB_SUB,"Unexpected RLC message %d is received",cb_incoming_primitive->command.message_header.message_id);
        as_id = SYS_MODEM_AS_ID_NONE;
      }
#endif
      break;
...
#endif
#else
          as_id = cb_incoming_primitive->command.l1_bmc_dropped_pdu_ind.as_id;
#endif
          break;
        default :
          MSG_FATAL_DS( CB_SUB,"=CB= Unexpected L1_BMC cmd %d", cb_incoming_primitive->command.message_header.message_id, 0,0 );
      }
      break;
#endif/*FEATURE_UMTS_BMC*/
    ...
    case MS_TIMER:
        switch(cb_incoming_primitive->command.message_header.message_id)
        {
          case MID_TIMER_EXPIRY:
            ...
            cb_msg_info = cb_incoming_primitive->command.timer_expiry.timer_id;
            break;
          default:
            MSG_FATAL_DS( CB_SUB,"=CB= Unexpected Timer cmd %d", cb_incoming_primitive->command.message_header.message_id, 0,0 );
        }
       break;
    ...
    case MS_CB_L2   : 
      switch(cb_incoming_primitive->command.message_header.message_id)
      {
        case DL_CB_BLOCK_IND :
          ...
          cb_msg_info = cb_incoming_primitive->command.cb_block_ind.Frame_no;
          break;
        default :
          MSG_FATAL_DS( CB_SUB,"=CB= Unexpected CB_L2 cmd %d", cb_incoming_primitive->command.message_header.message_id, 0,0 );
      }
      break;
    case MS_CB_L1   :
      switch(cb_incoming_primitive->command.message_header.message_id)
      {
        case L1_CB_MISSING_BLOCK_IND :
          ...
          cb_msg_info = cb_incoming_primitive->command.cb_missing_block.fn;
          break;
        case L1_CB_SKIP_CNF :
        ...
        case L1_CB_SCHED_EXPIRY_IND :
          ...
          cb_msg_info = cb_incoming_primitive->command.cb_schedule_expired_ind.channel_ind;
          break;
...
#endif
        default :
          MSG_FATAL_DS( CB_SUB,"=CB= Unexpected CB_L1 cmd %d", cb_incoming_primitive->command.message_header.message_id, 0,0 ); 
      }
      break;
      case MS_REG_CB:
      switch(cb_incoming_primitive->command.message_header.message_id)
      {
        ...
        case REG_SERVICE_STATE_IND:
          ...
          cb_msg_info = cb_incoming_primitive->command.service_state_ind.reg_inactive;
          break;
        case REG_DDS_INFO_IND:
          as_id = cb_incoming_primitive->command.dds_info_ind.as_id;
          cb_msg_info = cb_incoming_primitive->command.dds_info_ind.dds_sub;
          break;
#ifdef FEATURE_TWO_3GPP_SIM
		case REG_3GPP2_STATUS_IND:
          as_id = cb_incoming_primitive->command.cb_3gpp2_stack_status_ind.as_id;
          cb_msg_info = cb_incoming_primitive->command.cb_3gpp2_stack_status_ind.is_stack_active << 8 | cb_incoming_primitive->command.cb_3gpp2_stack_status_ind.active_mode ;
          break;
#endif
      }
      break;
    default :
      MSG_HIGH_DS_1(CB_SUB,"=CB= Unexpected message set %d", cb_incoming_primitive->command.message_header.message_set); 
      break;
  }

  if(IS_NOT_VALID_AS_ID(as_id))
  {
    MSG_HIGH_DS_3(CB_SUB,"=CB= Invalid AS ID:%d - Ignoring message MS: %d   MSG_ID: %d", 
...
/*===========================================================================
  ...
  FUNCTION CB_SEARCH_LIST_SERVICE_STATE_HANDLER
  ...
  ===========================================================================*/
cb_state_type cb_search_list_service_state_handler
(
cb_state_type *cur_state,
byte cb_search_list_length,
cb_as_id_e_type superset_id
)
{
  cb_state_type new_state = *cur_state ;
#if defined(FEATURE_WCDMA_ETWS) && defined(FEATURE_UMTS_BMC)
  byte etws_msg_count = search_list_len - cb_search_list_length;
#endif
  ...
  switch (cur_state->state)
  {
      case CB_IDLE_STATE :
        ...
        switch (cur_state->idle_substate) 
        {
          case CB_INACTIVE_SUBSTATE :
            ...
            MSG_HIGH_DS_0(CB_SUB,"CB: Moved into COLLECTION INITIATED substate") ;
            break ;
          case CB_CBCH_DETECTED_SUBSTATE :
            ...
            if(
#ifdef FEATURE_UMTS_BMC_MSIM
               (cb_check_if_current_sub_to_be_disabled() == TRUE)||
#else
                (mm_dual_standby_mode && (CB_UMTS_MODE == bmc_active_rat))||
#endif
                ((bmc_active_rat == CB_GSM_MODE)
                    &&(cb_search_list_superset == CB_SUBSET)
                    &&(cb_check_if_same_cell(superset_id,cb_as_id) == TRUE)
                    &&(bmc_active_rat == bmc_active_rat_sim[superset_id])))
            {
              MSG_HIGH_DS_0(CB_SUB,"CB: Moved into ACTIVATION PENDING SUBSTATE") ;
...
#endif //ifdef FEATURE_NAS_CBS_DSDS
            {
              ...
              MSG_HIGH_DS_0(CB_SUB,"CB: Moved into ACTIVE state") ;
          ...
          case CB_COLLECTION_INITIATED_SUBSTATE :
            if (cb_search_list_length == 0)
            {
              ...
              MSG_HIGH_DS_0(CB_SUB,"CB: Moved into INACTIVE substate") ;
            }
        ...
        */
        case CB_ACTIVATION_PENDING_SUBSTATE:
          if (cb_search_list_length == 0)
          {
            ...
            MSG_HIGH_DS_0(CB_SUB,"CB: Moved into IDLE, CBCH DETECTED substate") ;
          }
...
#endif //ifdef FEATURE_NAS_CBS_DSDS
          default:
            MSG_FATAL_DS( CB_SUB,"Illigal CB IDLE substate ID",0,0,0) ;
        }
      ...
      case CB_ACTIVE_STATE :
        ...
        break ;
      default:
        MSG_FATAL_DS( CB_SUB,"Illigal CB state ID",0,0,0) ;
    }     /* switch (cur_state->state) */
  return new_state;
}
/*===========================================================================
...
)
{
  ...
  byte i,current_sub;
  cb_search_list_set_type  prev_superset_list[MAX_AS_IDS];
#endif
  byte superset_id = 0;
...
#if defined (FEATURE_DUAL_SIM) && defined(FEATURE_NAS_CBS_DSDS)
  memscpy(prev_superset_list,MAX_AS_IDS,cb_search_list_superset_sim,MAX_AS_IDS);
#endif
                                                         ...
                                                         == (int)CB_SEARCH_LIST_REQ)
  {
    ...
    if (search_list_len != 0)
    {
      ...
      {
        MSG_ERROR_DS_1(CB_SUB,"=CB= Invalid SUB ID:%d in search list req", 
                   cb_incoming_primitive->command.search_list_req.as_id);
      }
    }
#endif

if(cb_dds_info == TRUE && (cb_state.idle_substate == CB_CBCH_DETECTED_SUBSTATE || cb_state.idle_substate == CB_ACTIVATION_PENDING_SUBSTATE) && cb_supported)
{
#if defined (FEATURE_DUAL_SIM) && defined(FEATURE_NAS_CBS_DSDS)
  for(i = CB_AS_ID_1;i < MAX_AS_IDS;i++)
  {
    if(cb_search_list_superset_sim[i] == CB_SUPERSET)
    {
      superset_id = i;
    }
  }
  MSG_HIGH_DS_2(cb_as_id,"CB: cb_search_list_superset_sim[CB_AS_ID_1] = %d cb_search_list_superset_sim[CB_AS_ID_2] = %d ",
  	                                        cb_search_list_superset_sim[CB_AS_ID_1],cb_search_list_superset_sim[CB_AS_ID_2]) ;
  current_sub = cb_as_id;
  for(i = CB_AS_ID_1;i < MAX_AS_IDS;i++)
  {
     /*Checking with prev value to not send additional scheduling req when already in ACTIVE state*/
    if((i != current_sub) && (cb_search_list_superset_sim[i] != CB_NONE)  && (prev_superset_list[i] != cb_search_list_superset_sim[i]))
    {
      cb_as_id = i;
      cb_message_as_id = (byte)(cb_as_id + 1);
      cb_state= cb_search_list_service_state_handler(&cb_state,
                 (search_list_len_sim[i]
#if defined (FEATURE_WCDMA_ETWS) && defined (FEATURE_UMTS_BMC)
                        - number_of_etws_ids_in_search_list()
#endif
                  ),superset_id);
    }
  }
  ...
  new_state = cb_search_list_service_state_handler(&new_state,cb_search_list_length,superset_id);
}
else
{
	new_state = cb_search_list_service_state_handler(&new_state,cb_search_list_length,superset_id);
                                                         ...
                                                         == (int)CB_DELETE_PAGE_REQ)
  {
    ...
    MSG_HIGH_DS_0(CB_SUB,"CB: Received CB_DELETE_PAGE_REQ") ;
  ...
  {
    MSG_HIGH_DS_0(CB_SUB,"CB: Received CB_DELETE_ALL_REQ") ;
  ...
  {
    MSG_HIGH_DS_0(CB_SUB,"Unexpected WMS message is received") ;
  }
  ...
  switch (cur_state->state)
  {
      ...
      switch (cur_state->idle_substate)
      {
        case CB_INACTIVE_SUBSTATE :
          if (cb_supported && cb_activate_cb_due_to_service(cb_service))
          {
            ...
            MSG_HIGH_DS_0(CB_SUB,"CB: Moved into CBCH DETECTED substate") ;
          }
          break ;
        case CB_CBCH_DETECTED_SUBSTATE :
          if ((!cb_supported) || (!cb_activate_cb_due_to_service(cb_service)))
    ...
    case CB_ACTIVE_STATE :
      if ((!cb_supported) || (!cb_activate_cb_due_to_service(cb_service)))
      {
        ...
        MSG_HIGH_DS_1(CB_SUB,"CB: Moved into IDLE, COLLECTION INITIATED substate, bmc_rx_suspended = %d",bmc_rx_suspended) ;
#ifdef FEATURE_UMTS_BMC
            ...
            */
            if(
               ...
               (mm_dual_standby_mode && CB_UMTS_MODE == bmc_active_rat)
                ...
                ||((bmc_active_rat == CB_GSM_MODE)
                    &&(cb_search_list_superset == CB_SUBSET)
                    &&(current_rat == bmc_active_rat_sim[superset_id])
                    &&(cb_supported_sim[superset_id] == TRUE)
                    && cb_check_if_same_cell(superset_id,cb_as_id)))
            {
              MSG_HIGH_DS_0(CB_SUB,"CB: Moved into ACTIVATION PENDING SUBSTATE") ;
              new_state.idle_substate = CB_ACTIVATION_PENDING_SUBSTATE;
            }
        ...
        case CB_ACTIVATION_PENDING_SUBSTATE:
          ...
          */
          if (!cb_supported)
          {
            ...
            MSG_HIGH_DS_0(CB_SUB,"CB: Moved into IDLE, COLLECTION INITIATED substate") ;
          }
          else if(((bmc_active_rat == CB_GSM_MODE)
                    &&(!((cb_search_list_superset == CB_SUBSET)
                         &&(current_rat == bmc_active_rat_sim[superset_id])
                         ...
                         && cb_check_if_same_cell(superset_id,cb_as_id))))
#ifdef FEATURE_UMTS_BMC_MSIM
                   ||(cb_check_if_current_sub_to_be_disabled() == FALSE)
#endif
                 )
          {
            new_state.state = CB_ACTIVE_STATE ;
...
#ifdef FEATURE_DUAL_SIM
#ifdef FEATURE_UMTS_BMC_MSIM
      else if(cb_check_if_current_sub_to_be_disabled() == TRUE)
#else
      else if(mm_dual_standby_mode && current_rat == CB_UMTS_MODE)
#endif
      {
        MSG_HIGH_DS_0(CB_SUB,"CB: Moved into ACTIVATION PENDING SUBSTATE") ;
        new_state.state = CB_IDLE_STATE ;
...
)
{
  ...
  if (cb_supported)
  {
        ...
        memcmp((void *)current_cell_id,(void *)incoming_cell_id, CELL_ID_VALUE_LENGTH))
    {
      ...
      MSG_HIGH_DS_0(CB_SUB,"CB: Cell Changed but Message Cache is not cleared instead each message will be deleted on Dup Det Timer expiry") ;
      ...
      if(CB_BMC_MULTISIM_ENABLE&&
	     (cb_current_wcdma_priority != CRITICAL_PRIORITY))
      {
        cb_current_wcdma_priority = LOW_PRIORITY;
        cb_stop_timer(CB_WCDMA_PRIORITY_THRESHOLD_TIMER,cb_as_id);
        cb_start_timer(CB_WCDMA_PRIORITY_THRESHOLD_TIMER,cb_msim_high_priority_threshold*2000,cb_as_id);
      }
#endif
    }
    ...
    MSG_HIGH_DS_3(CB_SUB,"CB Service ID list:%d  ID-0:%d ID-1:%d", (search_list_len!=0),
  ...
  MSG_HIGH_DS_6(CB_SUB,"BMC: PLMN %X %X %X, LAC %X %X, CELL ID %X",
                               incoming_plmn[0],incoming_plmn[1],incoming_plmn[2],
                               incoming_lac[0],incoming_lac[1],
                               *(dword *)incoming_cell_id) ;
							   
if(cb_dds_info == TRUE && (cb_state.idle_substate == CB_COLLECTION_INITIATED_SUBSTATE || cb_state.idle_substate == CB_ACTIVATION_PENDING_SUBSTATE) && cb_supported)
{
	  ...
	  for(i = CB_AS_ID_1;i < MAX_AS_IDS;i++)
	  {
		if(i != current_sub && cb_search_list_superset_sim[i] != CB_NONE)
		{
		  cb_as_id = i;
		  cb_message_as_id = (byte)(cb_as_id + 1);
		  /*Sending FALSE to not send additional scheduling req when already in ACTIVE state*/
	...
	new_state = cb_cbch_service_state_handler (&new_state,current_rat,TRUE);
}
else
{
	  new_state = cb_cbch_service_state_handler (&new_state,current_rat,TRUE);
...
)
{
  ...
  switch (cur_state->state)
  {
    case CB_IDLE_STATE :
      ...
      MSG_HIGH_DS_0(CB_SUB,"CB: Can not process DL_CB_BLOCK_IND in IDLE state") ;
    ...
    case CB_ACTIVE_STATE :
          ...
          (cb_active_substate != CB_ACTIVE_DRX))
      {
              ...
              (CB_CALCULATE_PAGING_SLOT(cb_current_sfn) == CB_FIFTH_BLOCK)))
        {
          ...
          MSG_HIGH_DS_2(CB_SUB,"CB PSEUDO DRX: Aggressive scan for %d slots till FN:%d",
                         ...
                         cb_incoming_primitive->command.cb_block_ind.Frame_no))
      {
        ...
        MSG_HIGH_DS_0(CB_SUB,"CB: Received invalid CB BLOCK") ;
...
)
{
  ...
  {
      MSG_HIGH_DS_0(CB_SUB,"WCDMA L1 Schedule Expiry indication received in IDLE state") ;
  }
  ...
  {
    MSG_HIGH_DS_0(CB_SUB,"WCDMA L1 Schedule Expiry indication ignored in Improper state") ;
  }
...
)
{
  ...
  switch (cb_incoming_primitive->command.message_header.message_id)
  {
    case DL_CB_BLOCK_IND :
      ...
      MSG_HIGH_DS_1(CB_SUB,"CB: Received DL_CB_BLOCK_IND FN:%d",cb_incoming_primitive->command.cb_block_ind.Frame_no) ;
      ...
      break ;
    default:
      MSG_HIGH_DS_0(CB_SUB,"Unexpected RR message is received") ;
      break ;
  }
...
)
{
  ...
  switch (cb_incoming_primitive->command.message_header.message_id)
  {
    case RR_CELL_CHANGE_IND :
      ...
      MSG_HIGH_DS_1(CB_SUB,"CB: Received RR_CELL_CHANGE_IND, cb_gsm_feature_disabled=%d", cb_gsm_feature_disabled) ;
	  ...
	  if(cb_supported == FALSE && 
			   cb_state.state== CB_IDLE_STATE &&
			   cb_camped_rat != CB_NONE_MODE &&
			   cb_camped_rat != CB_GSM_MODE )
	    {
		  MSG_HIGH_DS_1(CB_SUB,"CB: Ignored CB cell change CTCH (false) camped rat = %d",cb_camped_rat);
	    }
		else
		 {
      /* Process RR message */
...
)
{
  ...
  switch (cb_incoming_primitive->command.message_header.message_id)
  {
    case RRC_CELL_CHANGE_IND :
      ...
      MSG_HIGH_DS_2(CB_SUB,"CB: Received RRC_CELL_CHANGE_IND with N = %d, K = %d ",
      ...
      if (cb_supported)
      {
        ...
        */
        ...
        MSG_HIGH_DS_1(CB_SUB,"BMC: Schedule Message Periodicity: %d",cb_bmc_schedule_msg_periodicity);
...
#ifdef FEATURE_WCDMA_ETWS
      case RRC_ETWS_PRIM_NOTIFICATION_IND:
        MSG_HIGH_DS_0(CB_SUB,"CB ETWS: Received RRC ETWS Primary Indication");
...
)
{
  ...
  switch(timer_id)
  {
    case CB_TIMER_DUP_DET:
        MSG_HIGH_DS_0(CB_SUB,"CB: Timer CB_TIMER_DUP_DET expired");
        cb_update_cache_timer_offsets(0x00);
        break;

#if defined (FEATURE_WCDMA_ETWS) && defined (FEATURE_UMTS_BMC)
    case CB_ETWS_PRIM_DISABLE_CTCH_TIMER:
        MSG_HIGH_DS_0(CB_SUB,"CB ETWS: Timer CB_ETWS_PRIM_DISABLE_CTCH_TIMER expired");
        if (new_state.state == CB_ACTIVE_STATE)
        {
          MSG_HIGH_DS_0(CB_SUB,"CB ETWS: Timer expiry is ignored as non-ETWS CB is Active");
        }
        if(cb_etws_ctch_drx_timer_running)
        {
...
#endif
#ifdef FEATURE_UMTS_BMC_MSIM
    case CB_WCDMA_PRIORITY_THRESHOLD_TIMER:
        MSG_HIGH_DS_0(CB_SUB,"CB: Timer CB_WCDMA_PRIORITY_THRESHOLD_TIMER expired");
        if(CB_BMC_MULTISIM_ENABLE&&(cb_state.state == CB_ACTIVE_STATE) &&
           (cb_active_substate != CB_ACTIVE_DRX))
        {
          if(cb_current_wcdma_priority != LOW_PRIORITY)
          {
            cb_current_wcdma_priority = LOW_PRIORITY;
            cb_send_unscheduled_schedule_req_update_w_priority(cb_as_id);
          }
          else
          {
            MSG_HIGH_DS_1(CB_SUB,"CB: Timer CB_WCDMA_PRIORITY_THRESHOLD_TIMER expired in LOW priority, bumping up priority to HIGH from %d",cb_current_wcdma_priority);
            cb_current_wcdma_priority = HIGH_PRIORITY;
            cb_start_timer(CB_WCDMA_PRIORITY_THRESHOLD_TIMER,cb_msim_high_priority_threshold*1000,cb_as_id);
            cb_send_unscheduled_schedule_req_update_w_priority(cb_as_id);
          }
        }
        break;
#endif
    default :
          MSG_FATAL_DS( CB_SUB,"CB: Unknown timer id expired",0,0,0);
  }    
  ...
  switch (cb_incoming_primitive->command.message_header.message_id)
    {
     case REG_SERVICE_STATE_IND :
#ifdef FEATURE_TWO_3GPP_SIM
	  if(cb_check_if_3GPP2_stack_active(cb_as_id) == TRUE)
	  {
		  MSG_HIGH_DS_2(CB_SUB,"CB: 1x active = %d or Hdr active = %d,no further processing",
		                                        cb_1x_stack_active,cb_hdr_stack_active);
		  if(cb_incoming_primitive->command.service_state_ind.active_rat == SYS_RAT_LTE_RADIO_ACCESS)
		  {
		    cb_reg_inactive = cb_incoming_primitive->command.service_state_ind.reg_inactive;
		  }
	      if(cb_incoming_primitive->command.service_state_ind.reg_inactive == TRUE)
		  {
		    cb_camped_rat =  CB_NONE_MODE;
		  }
...
#endif
      if(cb_incoming_primitive->command.service_state_ind.active_rat == SYS_RAT_GSM_RADIO_ACCESS)
      {
        active_rat=CB_GSM_MODE;
      }
      else if(cb_incoming_primitive->command.service_state_ind.active_rat == SYS_RAT_UMTS_RADIO_ACCESS)
      {
        active_rat=CB_UMTS_MODE;
      }
      else if(cb_incoming_primitive->command.service_state_ind.active_rat == SYS_RAT_LTE_RADIO_ACCESS)
      {
        active_rat = CB_LTE_MODE;
      }
      else
      {
        active_rat = CB_NONE_MODE;
      }

#if defined (FEATURE_NAS_CBS_DSDS) && defined (FEATURE_TWO_3GPP_SIM)
	  cb_camped_rat = active_rat;
	  if(cb_incoming_primitive->command.service_state_ind.reg_inactive == TRUE)
	  {
		cb_camped_rat =  CB_NONE_MODE;
	  }
...
#endif
      if( cb_reg_inactive == cb_incoming_primitive->command.service_state_ind.reg_inactive
#ifdef FEATURE_NO_CBS_LTD
              && ((cb_service  == cb_incoming_primitive->command.service_state_ind.service_state)
              || (bmc_active_rat != active_rat))
#endif
              && (active_rat != CB_LTE_MODE)
         )
      {
        MSG_HIGH_DS_3(CB_SUB,"CB: No change in reg_inactive/service state flag,reg_inactive = %d, service_state = %d, active_rat = %d",cb_reg_inactive,cb_service,active_rat);
      }
      else
      {
        cb_service   = cb_incoming_primitive->command.service_state_ind.service_state;
        cb_reg_inactive = cb_incoming_primitive->command.service_state_ind.reg_inactive;
        MSG_HIGH_DS_3(CB_SUB,"CB: Received REG_SERVICE_STATE_IND reg_inactive = %d, service_state = %d, active_rat = %d",cb_reg_inactive,cb_service,active_rat);

        if(active_rat == CB_GSM_MODE || active_rat == CB_UMTS_MODE)
        {
          new_state = cb_cbch_service_state_handler(&new_state,active_rat,TRUE);
        }
		else if (active_rat == CB_LTE_MODE)
        {
          cb_supported = FALSE; /* no longer receive messages from old RAT */
          new_state = cb_cbch_service_state_handler(&new_state,bmc_active_rat,TRUE); /* clean up current state machine with old RAT*/
          bmc_active_rat = CB_GSM_MODE; /* map LTE to GSM */
        }
        ...
        cb_message_as_id = (byte)(cb_as_id + 1);
        cb_state = cb_cbch_service_state_handler (&cb_state,bmc_active_rat,TRUE);
        cb_as_id = current_sub;
        cb_message_as_id = (byte)(cb_as_id + 1);
#endif
      }
...
#ifdef FEATURE_UMTS_BMC_MSIM
      case REG_DDS_INFO_IND:
         cb_dds_info = cb_incoming_primitive->command.dds_info_ind.dds_sub;
         current_sub = cb_as_id;
         cb_as_id = (cb_as_id == CB_AS_ID_1) ? CB_AS_ID_2 : CB_AS_ID_1;
         if (cb_dds_info_sim[current_sub]==1)
         {
           cb_dds_info_sim[cb_as_id]=0;
         }
         else
         {
           cb_dds_info_sim[cb_as_id]=1;
         }
         cb_as_id=current_sub;
         MSG_HIGH_DS_1(CB_SUB,"CB: Receive REG_DDS_INFO_IND DDS_or_not: %d",cb_dds_info);
         break;
#endif
#ifdef FEATURE_TWO_3GPP_SIM
	  case REG_3GPP2_STATUS_IND:
	  	 cb_process_3gpp2_stack_status_indication(cb_incoming_primitive->command.cb_3gpp2_stack_status_ind.is_stack_active,
		 	                                      cb_incoming_primitive->command.cb_3gpp2_stack_status_ind.active_mode);
...
)
{
  ...
  {
    case CB_IDLE_STATE :
      MSG_HIGH_DS_0(CB_SUB,"L1 Missing Page indication received in IDLE state") ;
      break ;
    case CB_ACTIVE_STATE :
...
)
{
  ...
  {
    case CB_IDLE_STATE :
      MSG_HIGH_DS_0(CB_SUB,"L1 Schedule Expired indication received in IDLE state") ;
      break ;
    case CB_ACTIVE_STATE :
...
#endif
      {
                                     ...
                                     command.cb_schedule_expired_ind.channel_ind)
        {
          ...
          case CBCH_ECBCH:
            ...
            break ;
          default:
            MSG_ERROR_DS_0(CB_SUB,"Unknown channel identifier") ;
            cb_start_gsm_non_drx_mode(CBCH_ECBCH) ;
            break ;
        }
      }
...
)
{
  ...
  switch (cb_incoming_primitive->command.message_header.message_id)
  {
    case L1_CB_MISSING_BLOCK_IND:
      ...
      MSG_HIGH_DS_1(CB_SUB,"CB: Received L1_CB_MISSING_BLOCK_IND FN:%d",
    ...
    case L1_CB_SKIP_CNF:
      ...
      MSG_HIGH_DS_0(CB_SUB,"CB: Received L1_CB_SKIP_CNF ") ;
    ...
    case L1_CB_SCHED_EXPIRY_IND :
      ...
      MSG_HIGH_DS_0(CB_SUB,"CB DRX: Received CB_SCHEDULE_EXPIRED_IND ") ;
    ...
    case L1_CB_SCHEDULING_CNF:
      ...
      MSG_HIGH_DS_0(CB_SUB,"CB DRX: Received L1_CB_SCHEDULING_CNF ") ;
...
)
{
  ...
  MSG_HIGH_DS_1(CB_SUB,"CB: Received DL_RLC_BLOCK_IND in %d state",cur_state->state) ;
  if((cur_state->state == CB_IDLE_STATE)
#ifdef FEATURE_WCDMA_ETWS
     && (cb_etws_state.state == CB_IDLE_STATE)
#endif
     ...
     )
  {
      ...
      MSG_HIGH_DS_1(CB_SUB,"BMC: Received %d new DSM chain(s) ",pdu_cnt_local) ;
      ...
      for (pdu_index = 0; pdu_index < pdu_cnt_local; pdu_index++)
      {
        ...
        MSG_HIGH_DS_1(CB_SUB,"BMC: Received new DSM chain 0x%X",item_ptr) ;
        ...
        if (item_ptr != NULL)
        {
          ...
          if (status == FALSE)
          {
            MSG_HIGH_DS_0(CB_SUB,"CB: Error decoding the RLC parameters");
          ...
          {
            MSG_HIGH_DS_1(CB_SUB,"CB: Received invalid RLC PDU %d",received_sfn) ;
          ...
          MSG_HIGH_DS_3(CB_SUB,"BMC: Expected RLC PDU length is %d at SFN %d and End_SFN = %d ",rlc_pdu_len,received_sfn,received_end_sfn) ;
        ...
        MSG_HIGH_DS_2(CB_SUB,"DSM item rlc_pdu_ptr = %x, item_ptr = %x",rlc_pdu_ptr,item_ptr);
      }
  }
...
)
{
  ...
  switch (cb_incoming_primitive->command.message_header.message_id)
  {
    case DL_BMC_BLOCK_IND:
      ...
      MSG_HIGH_DS_0(CB_SUB,"BMC: Received DL_BMC_BLOCK_IND") ;
      ...
      break ;
    default:
      MSG_HIGH_DS_0(CB_SUB,"Unexpected RLC message is received") ;
      break ;
  }
  ...
  if (bmc_rx_suspended && gs_count_message(GS_QUEUE_RLC_CB) == 0)
  {
    ...
    MSG_HIGH_DS_0(CB_SUB,"BMC: Recovered from the suspend state");
    pdu_cnt = 1;
...
)
{
  ...
  new_state = *cur_state ;
  MSG_HIGH_DS_1(CB_SUB,"BMC: Received WL1 message = %d",cb_incoming_primitive->command.message_header.message_id);
...
)
{
  ...
  {
    cb_add_message_to_debug_buffer(cb_incoming_primitive->command.message_header.message_set,
                                  cb_incoming_primitive->command.message_header.message_id, TO_CB
  ...
  switch (cb_incoming_primitive->command.message_header.message_set)
  {
    ...
    case MS_CB_L1:
      ...
      break ;
    default:
      MSG_HIGH_DS_0(CB_SUB,"Unexpected message set") ;
      break ;
  }
  ...
  if(((cb_state.state != new_state.state) ||
     ...
     (new_state.state == CB_IDLE_STATE))
  {
    MSG_HIGH_DS_0(CB_SUB,"CB: Dynamic memory is freed now");
  ...
  if(current_message != NULL && 
      (current_message->message_header.current_message_descriptor == NULL && current_message->message_header.message_info != NULL))
  {
    MSG_FATAL_DS( CB_SUB,"=CB= current_message_descriptor = NULL even though message_info is not", 0,0,0 );
  }
 ...
 */
   ...
   if ((entry_found) &&
       ((next_substate != cb_active_substate) ||
        (action != CB_ACTION_0)))
   {
      MSG_HIGH_DS_3(CB_SUB, "CB substate change from %d to %d, action:%d", cb_active_substate, next_substate, action );
      ...
      switch (action)
      {
         ...
         default:
            MSG_HIGH_DS_1(CB_SUB, "CB: Action %d not supported", action);
            break;
      }
   }
...
FUNCTION CB_IS_LOW_PRIO_DEBUG_BUFFER_MESSAGE

DESCRIPTION
  This function Filters out the messages that doesn't need to be added in the cb debug buffer.
...
boolean cb_is_low_prio_debug_buffer_message
(
  byte message_set,
  dword message_id
)
...
FUNCTION CB_ADD_MESSAGE_TO_DEBUG_BUFFER
...
===========================================================================*/
void cb_add_message_to_debug_buffer
(
  byte message_set, 
  dword message_id,
  cb_message_direction_type_T cb_message_direction,
  sys_modem_as_id_e_type as_id
)
{
    MSG_HIGH_DS_3(CB_SUB,"MS : %d ,Message ID = %d, AS ID = %d ",message_set,message_id,as_id);
    
    if(!cb_is_low_prio_debug_buffer_message(message_set,message_id))
    {
      if(cb_debug_buffer_index >= CB_DEBUG_MESSAGE_BUFFER_SIZE)
      {
        cb_debug_buffer_index = 0;
      }
      cb_debug_buffer[cb_debug_buffer_index].debug_buffer_type.cb_info_debug_buffer.message_set = message_set;
      cb_debug_buffer[cb_debug_buffer_index].debug_buffer_type.cb_info_debug_buffer.message_id = message_id;
      cb_debug_buffer[cb_debug_buffer_index].present_cb_state= cb_state.state;
      cb_debug_buffer[cb_debug_buffer_index].present_cb_substate= cb_state.idle_substate;
      cb_debug_buffer[cb_debug_buffer_index].cb_message_direction = cb_message_direction;
#if defined FEATURE_DUAL_SIM || defined FEATURE_SGLTE
      cb_debug_buffer[cb_debug_buffer_index].as_id = as_id;
      cb_debug_buffer[cb_debug_buffer_index].msg_info = cb_msg_info;
      cb_msg_info = 0;
#endif

      if (cb_get_ulog_handle() != NULL)
      {
#if defined (FEATURE_DUAL_SIM) || defined(FEATURE_SGLTE)
        ULOG_RT_PRINTF_6(cb_get_ulog_handle(),
                "DS: SUB %d =CB= Message Set: %d Message ID: %d cb_state: %d cb_idle_substate: %d message_direction: %d",
                as_id + 1,
                message_set,
                message_id,
                cb_state.state,
                cb_state.idle_substate,
                cb_message_direction);
#else
        ULOG_RT_PRINTF_5(cb_get_ulog_handle(),
                "=CB= Message Set: %d Message ID: %d cb_state: %d cb_idle_substate: %d message_direction: %d",
                message_set,
                message_id,
                cb_state.state,
                cb_state.idle_substate,
      ...
      cb_debug_buffer_index++;
    }
}

cb_active_substate_id_type cb_active_substate_get(void)
{
  return cb_active_substate;
}

#ifdef FEATURE_UMTS_BMC_MSIM

cb_active_substate_id_type cb_active_substate_get_per_sub(cb_as_id_e_type as_id)
{
  return cb_active_substate_sim[as_id];
}
