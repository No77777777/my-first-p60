...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/cb/src/cb_utils.c#1 $ $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
05/19/15   ps      Fixed compilation warnings 
...
extern void cb_ctch_pdu_decode_error_per_sub(dword pdu_sfn,sys_modem_as_id_e_type as_id);
...
wcdma_priority_type cb_current_wcdma_priority_sim[MAX_AS_IDS] = {LOW_PRIORITY,LOW_PRIORITY,LOW_PRIORITY};
#define  cb_current_wcdma_priority cb_current_wcdma_priority_sim[cb_as_id]
#endif
...
cb_rat_type cb_camped_rat_sim[MAX_AS_IDS]={CB_GSM_MODE,CB_GSM_MODE};
#define cb_camped_rat cb_camped_rat_sim[cb_as_id]

#ifdef FEATURE_UMTS_BMC_MSIM
wcdma_priority_type cb_current_wcdma_priority_sim[MAX_AS_IDS] = {LOW_PRIORITY,LOW_PRIORITY};
#define  cb_current_wcdma_priority cb_current_wcdma_priority_sim[cb_as_id]
#endif
...
cb_rat_type cb_camped_rat = CB_GSM_MODE;
#endif
...
#ifdef FEATURE_UMTS_BMC_MSIM
wcdma_priority_type cb_current_wcdma_priority = LOW_PRIORITY;
#endif
#endif //#ifdef FEATURE_NAS_CBS_DSDS

boolean cb_nv_duplication_detection_disabled = FALSE;
byte cb_msim_high_priority_threshold = CB_WCDMA_PRIORITY_THRESHOLD_TIMEOUT_VALUE_1;

#ifdef FEATURE_UMTS_BMC
#define MAX_PDU_THRESHOLD 128
#if defined(FEATURE_UMTS_BMC_MSIM) && defined(FEATURE_DUAL_SIM) 
static bmc_rlc_state_type bmc_rlc_state_sim[MAX_AS_IDS];
#define bmc_rlc_state bmc_rlc_state_sim[cb_as_id]
#else
static bmc_rlc_state_type bmc_rlc_state;
#endif
#if defined (FEATURE_NAS_CBS_DSDS) && defined (FEATURE_TWO_3GPP_SIM)
boolean cb_1x_stack_active_sim[MAX_AS_IDS]={FALSE,FALSE};
boolean cb_hdr_stack_active_sim[MAX_AS_IDS] = {FALSE,FALSE};
#define cb_1x_stack_active cb_1x_stack_active_sim[cb_as_id]
#define cb_hdr_stack_active cb_hdr_stack_active_sim[cb_as_id]
#else
boolean cb_1x_stack_active = FALSE;
boolean cb_hdr_stack_active = FALSE;
#endif
...
cb_as_id_e_type cb_subs_sim[MAX_AS_IDS] = {CB_AS_ID_1, CB_AS_ID_2};
#define  cb_sub cb_subs_sim[cb_as_id]
#else
cb_as_id_e_type cb_sub = CB_AS_ID_1;  
#endif

#if defined (FEATURE_NAS_CBS_DSDS) && defined (FEATURE_TWO_3GPP_SIM)
extern boolean cb_supported_sim[MAX_AS_IDS];
#define cb_supported cb_supported_sim[cb_as_id]
cb_search_list_set_type  cb_search_list_superset_sim[MAX_AS_IDS];
#else
extern boolean cb_supported;
cb_search_list_set_type  cb_search_list_superset;
#endif
...
#ifdef FEATURE_TWO_3GPP_SIM
extern cb_state_type cb_cbch_service_state_handler
(
...
#ifdef FEATURE_UMTS_BMC_MSIM
extern void dl_dec_register_ctch_pdu_decode_error_cb_msim( void (*cb_ptr)(dword,sys_modem_as_id_e_type ),sys_modem_as_id_e_type as_id);
extern void dl_dec_deregister_ctch_pdu_decode_error_cb_msim(sys_modem_as_id_e_type as_id);
#else
...
void cb_process_mode_change_indication(byte val)
{
  ...
  MSG_HIGH_DS_1(CB_SUB,"CB :Receive REG_MODE_CHANGE_STATE_IND with Subs Active = %d",subs_active);
  if(subs_active==0)
  {
    return;
  }
...
void cb_send_l1_request(void * message_ptr)
{
  ...
  if(message_ptr == NULL)
  {
    MSG_FATAL_DS( CB_SUB, "Unexpected NULL pointer", 0,0,0 );
  }

  MSG_HIGH_DS_2(CB_SUB,"CB: Sending l1 request(%d,%d)",
  ...
  gs_status = gs_send_message(
   ...
   cb_add_message_to_debug_buffer(((IMH_T *)message_ptr)->message_set,
                                  ((IMH_T *)message_ptr)->message_id,
                                  FROM_CB,
#if defined (FEATURE_DUAL_SIM) || defined (FEATURE_TRIPLE_SIM)
  ...
  {
    MSG_ERROR_DS_0(CB_SUB,"CB: Unable to send message to layer 1");
  }
  ...
  for (search_index=0; search_index<search_list_len; search_index++)
  {
    switch(search_lst[search_index].current_message_id_type)
    {
      ...
      case CB_MESSAGE_ID_RANGE :
        ...
        break ;
      default:
        MSG_FATAL_DS( CB_SUB,"Wrong Message Id type identifier (%d)",
...
)
{
  ...
  switch (get_search_direction(current_search_list_entry,message_id))
  {
    ...
    default :
      MSG_FATAL_DS( CB_SUB,"Invalid search direction identifier",0,0,0) ;
  }
  ...
  else
  {
                                    ...
                                    new_list_entry->message_header.message_id))
    {
      ...
      case DOWNWARD_SEARCH :
        ...
        break ;
      case NO_SEARCH:
        /* MSG_ERROR_DS_0(CB_SUB,"CB: Duplication is detected") ; */
        return current_search_list_entry ;
      default :
        MSG_FATAL_DS( CB_SUB,"Invalid search direction identifier",0,0,0) ;
    }
...
)
{
  ...
  new_message_code_ptr->update_number_mask =  (word)((word)1<<(UPDATE_NUM_MASK - update_number)) ;
      ...
      if (next_message_code_ptr->message_code == message_code)
      {
        ...
        switch (next_message_code_ptr->gs_indicator)
        {
          ...
          case CB_LOCATION_AREA_WIDE:
            ...
            break ;
          default:
             MSG_ERROR_DS_1(CB_SUB,"GS indicator is out of range (%d)",next_message_code_ptr->gs_indicator);
             break ;
        } /* switch (next_message_code_ptr->gs_indicator) */
      }
...
#endif
    {
      MSG_FATAL_DS( CB_SUB,"CB: Not enough memory",0,0,0) ;
    }
...
#ifndef FEATURE_MODEM_HEAP   
   gs_free((void *)cmd_ptr );
#else
   modem_mem_free((void *)cmd_ptr, MODEM_MEM_CLIENT_NAS);
#endif
...
)
{
  ...
  MSG_HIGH_DS_2(CB_SUB," ETWS entries are %d (%d) ",etws_id_count,search_list_len);
...
void cb_stop_etws_timer( timer_id_T timer_id )
{
   ...
   {
      MSG_HIGH_DS_1(CB_SUB, "Stopping %d", timer_id);

      gs_status = gs_cancel_timer( GS_QUEUE_CB, CB_TIMER_TAG_AS_ID(timer_id) );

      if (gs_status != GS_SUCCESS)
      {
         MSG_ERROR_DS_0(CB_SUB,"Stop timer failure");
      }
...
void cb_start_etws_timer( timer_id_T timer_id, dword num_millisecs )
{
   ...
   if (num_millisecs == 0)
   {
     MSG_HIGH_DS_1(CB_SUB,"Cannot Start Timer %d with zero time out value",timer_id);
     return;
   }
   ...
   MSG_HIGH_DS_4(CB_SUB, "Start %d, timeout %u:%u, Status = %d",
...
)
{
  ...
  if ((curr_entry=cb_find_etws_message(list, message_id))!= NULL)
  {
     ...
     {
        MSG_HIGH_DS_2(CB_SUB,"CB: ETWS Message (%d, %d) is obsolete and filtered out",
...
)
{
  ...
  {
    MSG_HIGH_DS_0(CB_SUB,"Skip request is empty") ;
  ...
  for (index=0; index<skip_list_length; index++)
  {
    switch (CB_CALCULATE_PAGING_SLOT(skip_list[index]))
    {
      ...
      default:
        ...
        MSG_HIGH_DS_0(CB_SUB,"Invalid FN is to be skiped") ;
...
)
{
  ...
  if (skip_list_length > 0)
  {
    ...
    {
      MSG_HIGH_DS_0(CB_SUB,"CB: Reseting skip_archive[]") ;
    ...
    {
      MSG_ERROR_DS_0(CB_SUB,"Index out of bounds and buffer overflow");
      skip_list_length = 0;
    }
  }
...
)
{
  ...
  if(CB_ERASE_LAST_SCHEDULING != skip_type)
  {
    ...
    if ((skip_list_length=cb_process_skip_list(skip_list_length,skip_list)) == 0)
    {
      ...
      MSG_HIGH_DS_0(CB_SUB,"CB: CB_SKIP_REQ has been stopped") ;
  ...
  MSG_HIGH_DS_1(CB_SUB,"CB: Sending CB_SKIP_REQ request with length == %d",skip_list_length) ;

  for (skip_index=0; skip_index<skip_list_length; skip_index++)
  {
    MSG_HIGH_DS_2(CB_SUB,"CB: CB_SKIP_REQ->FN[%d] == %d",skip_index,skip_list[skip_index]) ;
  }
...
)
{
  ...
  if (page_length > 88)
  {
    ...
    MSG_HIGH_DS_0(CB_SUB,"CB: Message page has been truncated") ;
  }
  ...
  {
      /*converted to sys.h tech type, this type is easily available across module */
    data_ind.current_rat = SYS_RAT_UMTS_RADIO_ACCESS;
  }
  else if (bmc_active_rat == CB_GSM_MODE)
  {
    /*converted to sys.h tech type, this type is easily available across module */
	data_ind.current_rat = SYS_RAT_GSM_RADIO_ACCESS;
  }
  else
  {
     /*This case should not happen but added for precaution */
     data_ind.current_rat = SYS_RAT_NONE;
  }
#endif
  ...
  cb_add_message_to_debug_buffer(data_ind.message_header.message_set,
                                 data_ind.message_header.message_id,
                                 FROM_CB,
#if defined (FEATURE_DUAL_SIM) || defined (FEATURE_TRIPLE_SIM)
                                 ( sys_modem_as_id_e_type)cb_as_id
#else                     
...
#endif
  MSG_HIGH_DS_1(CB_SUB,"CB: CB data indication to WMS, status = %d",status) ;
...
#error code not present
  ...
  */
  ...
  cb_msg_info = message_id;
  cb_add_message_to_debug_buffer(etws_prim_notification_ind.message_header.message_set,
                                 etws_prim_notification_ind.message_header.message_id,
                                 FROM_CB,
#if defined (FEATURE_DUAL_SIM) || defined (FEATURE_TRIPLE_SIM)
                                 ( sys_modem_as_id_e_type)cb_as_id
#else                     
  ...
  MSG_HIGH_DS_2(CB_SUB,"CB ETWS: CB ETWS primary notification to WMS status = %d, Security:%d",status,warning_security_info_present) ;
...
#endif
  {
    MSG_ERROR_DS_0(CB_SUB,"CB: Not enough memory to allocate etws_sec_notification_ind") ;
    return ;
  }
...
#error code not present
  ...
  */
  ...
  cb_msg_info = message_id;
  cb_add_message_to_debug_buffer(etws_sec_notification_ind->message_header.message_set,
                                 etws_sec_notification_ind->message_header.message_id,
                                 FROM_CB,
#if defined (FEATURE_DUAL_SIM) || defined (FEATURE_TRIPLE_SIM)
                                 ( sys_modem_as_id_e_type)cb_as_id
#else                     
...
#endif
  MSG_HIGH_DS_1(CB_SUB,"CB ETWS: CB ETWS Secondary notification to WMS, status = %d",status) ;
...
)
{
  ...
  cb_msg_info = cell_change_ind.cell_change_type;
  cb_add_message_to_debug_buffer(cell_change_ind.message_header.message_set,
                                 cell_change_ind.message_header.message_id,
                                 FROM_CB,
#if defined (FEATURE_DUAL_SIM) || defined (FEATURE_TRIPLE_SIM)
                                 ( sys_modem_as_id_e_type)cb_as_id
#else                     
...
#endif
  MSG_HIGH_DS_1(CB_SUB,"CB: Sent CB Cell Change indication to WMS, status = %d",status) ;
...
)
{
  ...
  else
  {
    while (missing_page->next_missing_page != page_tobe_deleted)
    {
      ...
      {
        MSG_FATAL_DS( CB_SUB,"CB: Existing page could not be found",0,0,0) ;
      }
    }
...
)
{
  ...
  while (list != NULL)
  {
    ...
    if ((!cb_message_is_requested(list->message_header.message_id)) && 
        (list->message_header.message_id != 0xFFFF ))
    {
#if (defined FEATURE_UMTS_BMC_DRX_SUPPORTED) || (defined FEATURE_GSM_CB_DRX_SUPPORTED)
...
#endif          
          {
            cb_stop_timer((timer_id_T)CB_TIMER_DUP_DET,
#if defined (FEATURE_DUAL_SIM) || defined (FEATURE_TRIPLE_SIM)
                           cb_as_id
#else                     
                           CB_AS_ID_1
      ...
      while (cb_received_message_code != NULL)
      {
        ...
        MSG_HIGH_DS_2(CB_SUB,"CB: Message (%d, %d) is obsolete and filtered out",
...
)
{
  ...
  if (process_search_list)
  {
    ...
    MSG_HIGH_DS_1(CB_SUB,"CB: Search list length = %d",search_list_len);
    ...
    {
#if defined (FEATURE_NAS_CBS_DSDS) && defined (FEATURE_TWO_3GPP_SIM)
      if(cb_search_list_superset == CB_SUPERSET || cb_search_list_superset == CB_SUBSET)
      {
        for(i = CB_AS_ID_1;i < MAX_AS_IDS;i++)
        {
          if(i != cb_as_id && 
               (cb_search_list_superset_sim[i] == CB_SUBSET || cb_search_list_superset_sim[i] == CB_SUPERSET))
          {
            cb_search_list_superset_sim[i] = CB_NONE;
          }
        }
      }
      cb_search_list_superset = CB_NONE;
#endif
    }
    else
    {
...
#if defined (FEATURE_NAS_CBS_DSDS) && defined (FEATURE_TWO_3GPP_SIM)
      cb_check_if_search_list_superset();
#endif
    }
...
)
{
  ...
  MSG_HIGH_DS_0(CB_SUB,"CB: Sending CB_DEACTIVATE_REQ request") ;
...
void cb_bmc_deactivate_ctch_monitoring( void )
{
  ...
  if (cmd_ptr == NULL)
  {
    MSG_FATAL_DS( CB_SUB,"BMC: Not enough memory to send WCDMA L1 request",0,0,0) ;
  }
...
#else
  cmd_ptr->cmd.bmc_deactivate_req.as_id = (sys_modem_as_id_e_type)cb_as_id;
  cb_current_wcdma_priority = LOW_PRIORITY;
#endif
#endif
  cb_active_substate_control(CB_STD_DEACTIVATED);

  cb_send_event_ctch_stop();
  cb_add_message_to_debug_buffer(MS_BMC_L1,
                                 CPHY_BMC_DEACTIVATE_REQ,FROM_CB,
#if defined (FEATURE_DUAL_SIM) || defined (FEATURE_TRIPLE_SIM)
                                 ( sys_modem_as_id_e_type)cb_as_id
#else                     
                                 ( sys_modem_as_id_e_type)CB_AS_ID_1
...
#ifdef FEATURE_SEGMENT_LOADING
#ifdef FEATURE_UMTS_BMC_MSIM
  IWCDMA_dl_dec_deregister_ctch_pdu_decode_error_cb_msim(ptr_W_MMCP,(sys_modem_as_id_e_type)cb_as_id); 
  IWCDMA_wl1_put_cmd(ptr_W, cmd_ptr,(sys_modem_as_id_e_type)cb_as_id);
#else
...
#ifdef FEATURE_UMTS_BMC_MSIM
  dl_dec_deregister_ctch_pdu_decode_error_cb_msim((sys_modem_as_id_e_type)cb_as_id);
  wl1_put_cmd(cmd_ptr, (sys_modem_as_id_e_type)cb_as_id) ;
#else
...
)
{
  ...
  {
    MSG_FATAL_DS( CB_SUB,"Received (%d) illigal RAT (GSM-0,UMTS-1)",(int)current_rat,0,0) ;
  }
...
void cb_start_gsm_pseudo_drx_mode(boolean current_cycle_expired)
{
  ...
  {
    MSG_ERROR_DS_0(CB_SUB,"CB PSEUDO DRX: Not enough memory to send L1 Scheduling Request") ;
  ...
  MSG_HIGH_DS_6(CB_SUB,"CB PSEUDO DRX: Sending CB_SCHEDULING_REQ(DRX) Curr:%ld(length %d slots) Next:%ld, Scheduling FN list length is %d (O:%d/P:%d):",
  ...
  {
    MSG_HIGH_DS_6(CB_SUB,"CB PSEUDO DRX: Slot# %lu %lu %lu  FN# %lu %lu %lu",
...
)
{
  ...
  MSG_HIGH_DS_0(CB_SUB,"CB: Sending CB_SCHEDULING_REQ(CB_NON_DRX_MODE) request") ;
...
)
{
  ...
  {
    MSG_ERROR_DS_1(CB_SUB," Received (%d) RAT not supported",current_rat);
  }
#endif
...
)
{
  ...
  cb_msg_info = message_code;
  cb_add_message_to_debug_buffer(duplication_ind.message_header.message_set,
                                 duplication_ind.message_header.message_id,
                                 FROM_CB,
#if defined (FEATURE_DUAL_SIM) || defined (FEATURE_TRIPLE_SIM)
                                 ( sys_modem_as_id_e_type)cb_as_id
#else                     
...
#endif
  MSG_HIGH_DS_1(CB_SUB,"CB: Duplication indication to WMS, Sent status = %d",status) ;
...
)
{
  ...
  cb_msg_info = error_value;
  cb_add_message_to_debug_buffer(cb_error_indication.message_header.message_set,
                                 cb_error_indication.message_header.message_id,
                                 FROM_CB,
#if defined (FEATURE_DUAL_SIM) || defined (FEATURE_TRIPLE_SIM)
                                 ( sys_modem_as_id_e_type)cb_as_id
#else                     
  ...
  MSG_HIGH_DS_2(CB_SUB,"CB: Error(%d) indication to WMS, Sent status = %d",error_value,status) ;
...
)
{
  ...
  if (new_message_ptr != NULL)
  {
    ...
    if (cb_find_message_code(new_message_ptr,message_code) != NULL)
    {
      ...
      MSG_HIGH_DS_3(CB_SUB,"CB: Message (ID %d, Code %d, Page %d) has been cleared",
      ...
      if (wanted_page_info->received_page_indicator == 0)
      {
        ...
        {
          MSG_ERROR_DS_0(CB_SUB,"CB: Could not remove message entry from the list") ;

          return ;
        }

        MSG_HIGH_DS_3(CB_SUB,"CB: Message (ID %d, Code %d, Page %d) has been deleted",
                                         message_id,message_code,page_number) ;
    ...
    {
      MSG_HIGH_DS_2(CB_SUB,"CB: Message (ID %d, Code %d) has not been received yet",
...
)
{
  ...
  if (cmd_ptr == NULL)
  {
    MSG_ERROR_DS_0(CB_SUB,"BMC: Not enough memory to send WCDMA L1 request") ;
    return;
  }
  ...
  MSG_HIGH_DS_1(CB_SUB,"BMC: Skip sequence length is %d :",skip_list_length) ;
  for (dindex=0; dindex<prn_number; dindex++)
  {
    MSG_HIGH_DS_3(CB_SUB,"BMC skip: %d %d %d",skip_list[dindex*3],skip_list[dindex*3 + 1],skip_list[dindex*3 + 2]) ;
  }
  for(prn_number = dindex*3;prn_number < skip_list_length;prn_number++)
  {
    MSG_HIGH_DS_1(CB_SUB,"BMC skip: %d",skip_list[prn_number]) ;
  }
  MSG_HIGH_DS_0(CB_SUB,"BMC: Sending L1_BMC_SKIP_REQ");
  cb_msg_info = (byte)skip_list_length;
  cb_add_message_to_debug_buffer(cmd_ptr->cmd.bmc_skip_req.message_header.message_set,
                                 cmd_ptr->cmd.bmc_skip_req.message_header.message_id,
                                 FROM_CB,
#if defined (FEATURE_DUAL_SIM) || defined (FEATURE_TRIPLE_SIM)
                                 ( sys_modem_as_id_e_type)cb_as_id
#else                     
...
)
{
...
#ifdef FEATURE_UMTS_BMC_MSIM
  cmd_ptr->cmd.bmc_scheduling_req.priority = cb_current_wcdma_priority;
#else
  cmd_ptr->cmd.bmc_scheduling_req.priority = LOW_PRIORITY;
#endif
  ...
  MSG_HIGH_DS_0(CB_SUB,"BMC: Sending CPHY_BMC_SCHEDULING_REQ to WCDMA L1") ;

  cb_send_event_ctch_start(CTCH_NON_DRX_MODE,0);

  cb_msg_info = cmd_ptr->cmd.bmc_scheduling_req.priority;
  cb_add_message_to_debug_buffer(cmd_ptr->cmd.bmc_scheduling_req.message_header.message_set,
                                 cmd_ptr->cmd.bmc_scheduling_req.message_header.message_id,
                                 FROM_CB,
#if defined (FEATURE_DUAL_SIM) || defined (FEATURE_TRIPLE_SIM)
                                 ( sys_modem_as_id_e_type)cb_as_id
#else                     
...
#ifdef FEATURE_SEGMENT_LOADING
#ifdef FEATURE_UMTS_BMC_MSIM
  IWCDMA_dl_dec_register_ctch_pdu_decode_error_cb_msim(ptr_W_MMCP, &cb_ctch_pdu_decode_error_per_sub,(sys_modem_as_id_e_type) cb_as_id);
  IWCDMA_wl1_put_cmd(ptr_W, cmd_ptr,(sys_modem_as_id_e_type)cb_as_id);
#else
...
#else
#ifdef FEATURE_UMTS_BMC_MSIM
  dl_dec_register_ctch_pdu_decode_error_cb_msim(&cb_ctch_pdu_decode_error_per_sub,(sys_modem_as_id_e_type) cb_as_id);
  wl1_put_cmd(cmd_ptr, (sys_modem_as_id_e_type)cb_as_id);
#else
...
#if defined(FEATURE_UMTS_BMC_MSIM) && defined(FEATURE_DUAL_SIM)
  byte as_id_temp =0;
  for (as_id_temp=0;as_id_temp<MAX_AS_IDS;as_id_temp++)
  {
      /*Initialize the rlc_id*/
     bmc_rlc_state_sim[as_id_temp].rlc_lc_id = 0;
     /* Initializes the watermark queue */
     dsm_queue_init( &(bmc_rlc_state_sim[as_id_temp].rlc_rx_wm_item),
                             24000,
                             &(bmc_rlc_state_sim[as_id_temp].rlc_rx_q) ) ;

     bmc_rlc_state_sim[as_id_temp].rlc_wm_reg_flag = FALSE;
  }
...
)
{
  ...
  {
    MSG_FATAL_DS( CB_SUB,"NULL Ptr when sending RLC_DL_REGISTER_SRVC_REQ",0,0,0);
  }
#else
  ...
  else /* a good ptr for RLC Data SAP BCCH data has been received */
  {
    ...
    bmc_rlc_dl_reg_params.rlc_data[ 0 ].rlc_post_rx_func_ptr_para = NULL;

    bmc_rlc_dl_reg_params.rlc_data[ 0 ].wm_reg_ind_cb = NULL;
    bmc_rlc_dl_reg_params.rlc_data[ 0 ].bearer_context = NULL;
...
void bmc_send_rlc_register_wm_cmd_per_sub
(
  /*BMC RLC state pointer*/
  bmc_rlc_state_type     *bmc_rlc_state_ptr,
  /*Call back function pointer for DL WM*/
...
)
{
  ...
  else /* a good ptr for RLC Data SAP BCCH data has been received */
  {
    ...
    bmc_rlc_dl_reg_params.rlc_data[ 0 ].rlc_post_rx_proc_func_ptr = 
                                     from_rlc_post_rx_fn_ptr;
    bmc_rlc_dl_reg_params.rlc_data[ 0 ].rlc_post_rx_func_ptr_para = (void *)&cb_sub;
    bmc_rlc_dl_reg_params.rlc_data[ 0 ].wm_reg_ind_cb = NULL;
    bmc_rlc_dl_reg_params.rlc_data[ 0 ].bearer_context = NULL;
    ...
    l2_dl_cmd->cmd_hdr.cmd_id = RLC_DL_REGISTER_SRVC_REQ;
    l2_dl_cmd->cmd_data.dl_reg = bmc_rlc_dl_reg_params;
    l2_dl_cmd->cmd_hdr.as_id = (sys_modem_as_id_e_type)cb_as_id;
#ifdef FEATURE_SEGMENT_LOADING
    IWCDMA_l2_dl_put_cmd(ptr_W, l2_dl_cmd);
#else
    l2_dl_put_cmd(l2_dl_cmd);
...
)
{
  ...
  MSG_HIGH_DS_3(CB_SUB,"LC id changed from %d to %d register back with WM,old water mark flag = %d",
...
#ifdef FEATURE_UMTS_BMC_MSIM
    bmc_send_rlc_register_wm_cmd_per_sub(&bmc_rlc_state, bmc_process_dsm_data_cb_per_sub);
#else
  ...
  {
    MSG_ERROR_DS_1(CB_SUB,"BMC: Invalid LC ID %d", rlc_lc_id);
  }/*if (rlc_lc_id != 0xFF)*/
...
)
{
  ...
  bmc_rlc_dl_dereg_params.cb_info[ 0 ].wm_reg_ind_cb  = NULL;
  bmc_rlc_dl_dereg_params.cb_info[ 0 ].bearer_context = NULL;
...
#ifdef FEATURE_UMTS_BMC_MSIM
	l2_dl_cmd_ptr->cmd_hdr.as_id = (sys_modem_as_id_e_type)cb_as_id;
#endif
  ...
  {
    MSG_FATAL_DS( CB_SUB, "Cannot get L2 DL cmd buffer",0,0,0);
  }
}
...
)
{
  ...
  if (gs_status != GS_SUCCESS)
  {
    ...
    {
      MSG_HIGH_DS_0(CB_SUB,"BMC: Stop CTCH monitoring due to PDU buffering exceed threshold");
    ...
    MSG_ERROR_DS_0(CB_SUB,"ERROR: Unable to send dl_cb_block_ind message");
  }
  else{
    pdu_cnt = 1;
  }
...
void bmc_process_dsm_data_cb_per_sub
(
   byte rlc_id,
   byte no_pdus,
   void *cb_data
)
{
  ...
  bmc_block_ind.message_header.message_id = (byte)DL_BMC_BLOCK_IND;
  bmc_block_ind.pdu_cnt = pdu_cnt;
  bmc_block_ind.as_id = *(sys_modem_as_id_e_type *)cb_data;
  ...
  if (gs_status != GS_SUCCESS)
  {
    ...
    if (pdu_cnt >= MAX_PDU_THRESHOLD)
    {
      MSG_HIGH_DS_0((bmc_block_ind.as_id + 1),"BMC: Stop CTCH monitoring due to PDU buffering exceed threshold");
      /*Enable the suspend flag*/
      bmc_rx_suspended = TRUE;
      /*Deactivate the CTCH monitoring*/
      cb_bmc_deactivate_ctch_monitoring();
    ...
    MSG_ERROR_DS_0((bmc_block_ind.as_id + 1),"ERROR: Unable to send dl_cb_block_ind message");
  }
  else{
    pdu_cnt = 1;
  }
...
)
{
  ...
  while(pkt_head_ptr != NULL)
  {
    ...
    {
      MSG_HIGH_DS_2(CB_SUB,"CB: Truncating RLC PDU as PDU length %d is beyond the MAX possible length %d",pdu_len,CB_BMC_MAX_RLC_DATA_LENGTH) ;
  ...
  while(item_ptr != NULL)
  {
    if(item_ptr->used)
    {
      ...
      */
      ...
      {
        MSG_HIGH_DS_0(CB_SUB,"CB: Copied RLC PDU to the Truncated length");
...
)
{
  ...
  if (bit_number == 0)
  {
    MSG_FATAL_DS( CB_SUB,"Invalid number of PDU per page",0,0,0) ;
...
void cb_ctch_pdu_decode_error(dword pdu_sfn)
{
  ...
  MSG_HIGH_DS_0(CB_SUB,"CB: Received Dropped PDU Indication from WL1") ;
  ...
  if (gs_status != GS_SUCCESS)
  {
    MSG_ERROR_DS_0(CB_SUB,"CB: Unable to post Dropped PDU indication to ourselves");
  }
}

void cb_ctch_pdu_decode_error_per_sub(dword pdu_sfn,sys_modem_as_id_e_type as_id)
{
  l1_bmc_dropped_pdu_ind_type l1_bmc_dropped_pdu_ind;
  gs_status_T    gs_status;

  MSG_HIGH_DS_0(as_id+1,"CB: Received Dropped PDU Indication from WL1") ;
  l1_bmc_dropped_pdu_ind.message_header.message_set = MS_BMC_L1;
  l1_bmc_dropped_pdu_ind.message_header.message_id  = L1_BMC_DROPPED_PDU_IND;
  l1_bmc_dropped_pdu_ind.pdu_sfn = pdu_sfn;
  l1_bmc_dropped_pdu_ind.as_id = as_id;
  ...
  if (gs_status != GS_SUCCESS)
  {
    MSG_ERROR_DS_0(as_id+1,"CB: Unable to post Dropped PDU indication to ourselves");
  }

#ifdef FEATURE_UMTS_BMC_MSIM
  if(CB_BMC_MULTISIM_ENABLE&&
      cb_active_substate_get_per_sub((cb_as_id_e_type)as_id) != CB_ACTIVE_DRX && 
  	  cb_current_wcdma_priority_sim[as_id] != LOW_PRIORITY)
  {
    cb_current_wcdma_priority_sim[as_id] = LOW_PRIORITY;
    cb_send_unscheduled_schedule_req_update_w_priority((cb_as_id_e_type)as_id);
    cb_stop_timer(CB_WCDMA_PRIORITY_THRESHOLD_TIMER,as_id);
  }
#endif
}
...
void cb_rlc_ctch_pdu_dropped(dword pdu_sfn)
{
  ...
  MSG_HIGH_DS_0(CB_SUB,"CB: Received Dropped PDU Indication from RLC") ;
  l1_bmc_dropped_pdu_ind.message_header.message_set = MS_BMC_L1;
  l1_bmc_dropped_pdu_ind.message_header.message_id  = L1_BMC_DROPPED_PDU_IND;
  l1_bmc_dropped_pdu_ind.pdu_sfn = pdu_sfn;
...
void cb_rlc_ctch_pdu_dropped_per_sub(dword pdu_sfn,sys_modem_as_id_e_type as_id)
{
  l1_bmc_dropped_pdu_ind_type l1_bmc_dropped_pdu_ind;
  gs_status_T    gs_status;

  MSG_HIGH_DS_0(as_id+1,"CB: Received Dropped PDU Indication from RLC") ;
  l1_bmc_dropped_pdu_ind.message_header.message_set = MS_BMC_L1;
  l1_bmc_dropped_pdu_ind.message_header.message_id  = L1_BMC_DROPPED_PDU_IND;
  l1_bmc_dropped_pdu_ind.pdu_sfn = pdu_sfn;
  l1_bmc_dropped_pdu_ind.as_id = as_id;
...
)
{
  ...
  (void) nas_event_report_payload(EVENT_UMTS_NAS_CB_CTCH_START, 10, (void *)event_packet,
                                      cb_message_as_id);
  MSG_HIGH_DS_0(CB_SUB,"CB: Sending event CTCH Start") ;
}
...
void cb_send_event_ctch_stop(void)
{
  ...
  (void) nas_event_report_payload(EVENT_NAS_CB_CTCH_STOP, 2, (void *)event_packet,
                                      cb_message_as_id);
    MSG_HIGH_DS_0(CB_SUB,"CB: Sending event CTCH Stop") ;
}
...
void cb_ctch_pdu_dropped(dword pdu_sfn)
{
  ...
  if(pdu_found == TRUE)
  {
    ...
    (void) nas_event_report_payload(EVENT_UMTS_NAS_CB_BMC_MSG_DECODE_FAIL, 13, (void *)event_packet,
                                        cb_message_as_id);
  }
  MSG_HIGH_DS_2(CB_SUB,"CB: Sending event CB_PDU_DROPPED for SFN:%u if pdu_found = %d is set",pdu_sfn,pdu_found) ;
}
...
void cb_get_nv_items(mm_nas_nv_items_T *mm_nas_nv_items_p)
{
  ...
  {
    MSG_HIGH_DS_1(CB_SUB,"CB: GSM CB feature status 0/1 enable/disable : %d",mm_nas_nv_items_p->cb_gsm_feature_disabled) ;
...
#ifdef FEATURE_UMTS_BMC_MSIM
  if(mm_nas_nv_items_p->version >= 11)
  {
    MSG_HIGH_DS_1(CB_SUB,"CB: cb_msim_high_priority_threshold : %d",mm_nas_nv_items_p->cb_msim_high_priority_threshold) ;
    cb_msim_high_priority_threshold = mm_nas_nv_items_p->cb_msim_high_priority_threshold;
  }
#endif
}
...
log_ptr = (LOG_UMTS_NAS_CB_LOG_PACKET_C_type *)nas_log_alloc(
                         LOG_UMTS_NAS_CB_LOG_PACKET_C, pkt_size, cb_message_as_id);
...
#ifdef FEATURE_DUAL_SIM
log_ptr->as_id = (uint8) (cb_as_id+1);
#else
...
/* Send the packet */
 /* Wrapper Function for log_commit(log_ptr) defined in MM.h and externed in cb_utils.c   */
#ifdef FEATURE_DUAL_SIM
    log_util_log_packet_commit((sys_modem_as_id_e_type)cb_as_id,(void *)log_ptr); 
#else
...
===========================================================================*/
void cb_start_timer( timer_id_T timer_id, dword num_millisecs,cb_as_id_e_type as_id)
{
   gs_status_T    status;

   if (num_millisecs == 0)
   {
     MSG_HIGH_DS_1(as_id+1,"Cannot Start Timer %d with zero time out value",timer_id);
     return;
   }
...
#if defined FEATURE_NAS_CBS_DSDS && defined FEATURE_TWO_3GPP_SIM
   cb_timer_status_sim[as_id][timer_id-1]= TIMER_ACTIVE;
#else
   cb_timer_status[timer_id-1]= TIMER_ACTIVE;
#endif

   MSG_HIGH_DS_3(as_id+1,"Start %d, timeout %lu:%lu",
             timer_id, (dword) (num_millisecs /1000L), (dword)(num_millisecs % 1000L) );


   if ( status != GS_SUCCESS )
   {
      MSG_ERROR_DS_1(as_id+1,"Start timer failure: Status %d", status);
   }
}
...
===========================================================================*/
void cb_stop_timer( timer_id_T timer_id, cb_as_id_e_type as_id)
{
   gs_status_T gs_status = GS_SUCCESS;

#if defined FEATURE_NAS_CBS_DSDS && defined FEATURE_TWO_3GPP_SIM
   if (cb_timer_status_sim[as_id][timer_id-1] != TIMER_STOPPED)
#else
   if (cb_timer_status[timer_id-1] != TIMER_STOPPED)
#endif
   {
      MSG_HIGH_DS_2(as_id+1,"Stopping %d , gs_status=%d", timer_id,gs_status);

      gs_status = gs_cancel_timer( GS_QUEUE_CB, CB_TIMER_TAG_AS_ID(timer_id) );

#if defined FEATURE_NAS_CBS_DSDS && defined FEATURE_TWO_3GPP_SIM
      cb_timer_status_sim[as_id][timer_id-1]= TIMER_STOPPED;
#else
      cb_timer_status[timer_id-1]= TIMER_STOPPED;
#endif
   }
...
)
{
  ...
  MSG_HIGH_DS_1(CB_SUB,"CB: Dup Timer -> Min_offset :%d", min_offset/1000L);
  ...
  if(min_offset != CB_DUP_TIMER_VALUE_INVALID)
  {
    cb_start_timer((timer_id_T)CB_TIMER_DUP_DET,
                   min_offset+num_remaining_ms,
#if defined (FEATURE_DUAL_SIM) || defined (FEATURE_TRIPLE_SIM)
                   cb_as_id
#else                     
                   CB_AS_ID_1
...
#if defined (FEATURE_NAS_CBS_DSDS) && defined (FEATURE_TWO_3GPP_SIM)
void cb_check_if_search_list_superset(void)
{
  byte i = 0;

  for(i = SYS_MODEM_AS_ID_1; i < MAX_AS_IDS; i++)
  {
    if(i-1 != (i+1)%MAX_AS_IDS)
    {
      cb_evaluate_superset(i,(i+1)%MAX_AS_IDS);
    }
  }
}

boolean cb_evaluate_superset_utility(cb_as_id_e_type id1,cb_as_id_e_type id2)
{
  byte j = 0,k = 0,found = 1;
  
  for(k = 0; k < search_list_len_sim[id2]; k++)
  {
    for(j = 0; j < search_list_len_sim[id1]; j++)
    {
      if(search_lst_sim[id2][k].current_message_id_type == search_lst_sim[id1][j].current_message_id_type)
      {
        if(search_lst_sim[id1][j].current_message_id_type == CB_SINGLE_MESSAGE_ID)
        {
          if(search_lst_sim[id1][j].single_message_id.message_id == search_lst_sim[id2][k].single_message_id.message_id)
          {
            found = TRUE;
          }
          else
        ...
        else if(search_lst_sim[id1][j].current_message_id_type == CB_MESSAGE_ID_RANGE)
        {
          if((search_lst_sim[id1][j].message_id_range.min_message_id <= search_lst_sim[id2][k].message_id_range.min_message_id)
                  &&(search_lst_sim[id1][j].message_id_range.max_message_id >= search_lst_sim[id2][k].message_id_range.max_message_id))
          {
            found = TRUE;
          }
          else
  ...
  if(found == TRUE)
  {
    cb_search_list_superset_sim[id1] = CB_SUPERSET;
    cb_search_list_superset_sim[id2] = CB_SUBSET;
  }
  else
  {
    cb_search_list_superset_sim[id1] = CB_NONE;
    cb_search_list_superset_sim[id2] = CB_NONE;
  }

  return found;
}

void cb_evaluate_superset(cb_as_id_e_type as_id1,cb_as_id_e_type as_id2)
{
  cb_as_id_e_type  id1,id2;

  if(search_list_len_sim[as_id1] == 0)
  {
    cb_search_list_superset_sim[as_id1] = CB_NONE;
    return;
  }
  else if(search_list_len_sim[as_id2] == 0)
  {
    cb_search_list_superset_sim[as_id2] = CB_NONE;
    return;
  }
  if(search_list_len_sim[as_id1] >= search_list_len_sim[as_id2])
  {
    id1 = as_id1;
    id2 = as_id2;
  }
  ...
  if(cb_evaluate_superset_utility(id1,id2) == TRUE)
    return;

  (void)cb_evaluate_superset_utility(id2,id1);
}

boolean cb_check_if_same_cell
(
  cb_as_id_e_type as_id1,
  cb_as_id_e_type as_id2
)
{
  boolean temp = FALSE;
  if((memcmp((void *)current_plmn_sim[as_id1],(void *)current_plmn_sim[as_id2],PLMN_VALUE_LENGTH) == 0)
  &&(memcmp((void *)current_lac_sim[as_id1],(void *)current_lac_sim[as_id2],LAC_VALUE_LENGTH) == 0)
  &&(memcmp((void *)current_cell_id_sim[as_id1],(void *)current_cell_id_sim[as_id2],CELL_ID_VALUE_LENGTH) == 0))
  {
    temp = TRUE;
  }
  return temp;
...
#endif
#ifdef FEATURE_UMTS_BMC_MSIM
boolean cb_check_if_emergency_message(word message_id)
{
  if((message_id >= 4352 && message_id <= 4359)
        || (message_id >= 4370 && message_id <= 4399))
  {
    return TRUE;
  }
  return FALSE;
...
void cb_send_unscheduled_schedule_req_update_w_priority(cb_as_id_e_type as_id)
{
  l1_ext_cmd_type  *cmd_ptr;
  cb_as_id_e_type other_as_id = (as_id == CB_AS_ID_1? CB_AS_ID_2 : CB_AS_ID_1);
#ifdef FEATURE_SEGMENT_LOADING
  interface_t *ptr_W = get_wcdma_interface();
#endif
  ...
  if (cmd_ptr == NULL)
  {
    MSG_FATAL_DS( as_id,"BMC: Not enough memory to send WCDMA L1 request",0,0,0) ;
  }
  
  if(cb_state_sim[as_id].state != CB_ACTIVE_STATE && bmc_active_rat_sim[other_as_id] == CB_UMTS_MODE)
  {
	  MSG_HIGH_DS_0(as_id+1,"BMC: Dropping the unscheduled CPHY_BMC_SCHEDULING_REQ as current sub is not in active state");
	  return; 
  }
  cmd_ptr->cmd_hdr.cmd_id = CPHY_BMC_SCHEDULING_REQ ;
  cmd_ptr->cmd.bmc_scheduling_req.scheduling_mode = CTCH_NON_DRX_MODE ;
  cmd_ptr->cmd.bmc_scheduling_req.as_id = (sys_modem_as_id_e_type)as_id;
  cmd_ptr->cmd.bmc_scheduling_req.priority = cb_current_wcdma_priority_sim[as_id];
#if defined FEATURE_UMTS_BMC_DRX_SUPPORTED
  cmd_ptr->cmd.bmc_scheduling_req.bmc_schedule_msg_periodicity = cb_bmc_schedule_msg_periodicity_sim[as_id];
#endif

    MSG_HIGH_DS_1(as_id+1,"BMC: Sending Unscheduled CPHY_BMC_SCHEDULING_REQ to WCDMA L1 with priority = %d",cb_current_wcdma_priority_sim[as_id]) ;

  cb_msg_info = cmd_ptr->cmd.bmc_scheduling_req.priority;
  cb_add_message_to_debug_buffer(MS_BMC_L1,cmd_ptr->cmd_hdr.cmd_id,FROM_CB,( sys_modem_as_id_e_type)cb_as_id);
#ifdef FEATURE_SEGMENT_LOADING
#ifdef FEATURE_UMTS_BMC_MSIM
  IWCDMA_wl1_put_cmd(ptr_W, cmd_ptr,(sys_modem_as_id_e_type)as_id);
#else
  IWCDMA_l1_put_cmd(ptr_W, cmd_ptr);
#endif
#else
#ifdef FEATURE_UMTS_BMC_MSIM
  wl1_put_cmd(cmd_ptr, (sys_modem_as_id_e_type)as_id) ;
#else
  l1_put_cmd(cmd_ptr) ;
#endif
#endif
...
void cb_get_start_pdu_from_rlc(byte *rlc_pdu_ptr, byte sdu_len, sys_modem_as_id_e_type as_id)
{
  byte msg_type=0;
  word message_id=0;
  wcdma_priority_type  prev_priority = cb_current_wcdma_priority_sim[as_id];
  byte rlc_pdu_local[3]={0};
  ...
  memscpy(rlc_pdu_local,3,rlc_pdu_ptr,3);
  cb_bmc_flip_octet_bits_rlc_pdu(rlc_pdu_local, 3);
  msg_type = rlc_pdu_local[CB_BMC_MESSAGE_TYPE_OFFSET];

 if(cb_active_substate_get_per_sub((cb_as_id_e_type)as_id) != CB_ACTIVE_DRX)
 {
  if (msg_type == CBS_MESSAGE_TYPE)
  {
    message_id = (rlc_pdu_local[CB_BMC_MESSAGE_ID_OFFSET] << 8) | 
                                      rlc_pdu_local[CB_BMC_MESSAGE_ID_OFFSET+1] ;

    if(cb_message_is_requested_per_sub(message_id,(cb_as_id_e_type)as_id) && 
	   cb_check_if_emergency_message(message_id) && cb_current_wcdma_priority_sim[as_id] != CRITICAL_PRIORITY)
    {
      cb_current_wcdma_priority_sim[as_id] = HIGH_PRIORITY;        
    }
  }
  else if(msg_type == CBS_SCHEDULE_TYPE && cb_current_wcdma_priority_sim[as_id] != CRITICAL_PRIORITY)
  {
    cb_current_wcdma_priority_sim[as_id] = HIGH_PRIORITY;
  }
  MSG_HIGH_DS_3(as_id+1,"CB: RLC called CB API for msg_type = %d, prev_priority = %d,priority = %d",msg_type,prev_priority,cb_current_wcdma_priority_sim[as_id]);

  if(prev_priority != cb_current_wcdma_priority_sim[as_id] && cb_current_wcdma_priority_sim[as_id]==HIGH_PRIORITY)
  {
    cb_send_unscheduled_schedule_req_update_w_priority((cb_as_id_e_type)as_id);
    if(cb_timer_status_sim[as_id][CB_WCDMA_PRIORITY_THRESHOLD_TIMER-1] != TIMER_ACTIVE)
    {
      cb_start_timer(CB_WCDMA_PRIORITY_THRESHOLD_TIMER,cb_msim_high_priority_threshold*1000,as_id);
    }
  }
 }
 else
 {
   MSG_HIGH_DS_0(as_id+1,"CB: RLC called CB API in DRX mode");
 }
}

boolean cb_check_if_current_sub_to_be_disabled(void)
{
  cb_as_id_e_type other_as_id = CB_AS_ID_2;

  if(cb_as_id == other_as_id)
  {
    other_as_id = CB_AS_ID_1;
  }

  if(bmc_active_rat == CB_UMTS_MODE)
  {
    if(bmc_active_rat_sim[other_as_id] == CB_UMTS_MODE)
    {
      if(cb_dds_info == FALSE && cb_reg_inactive_sim[other_as_id] == FALSE)
      {
        return TRUE;
      }
      else
    ...
    {
#ifdef FEATURE_TWO_3GPP_SIM
	  /* try to minimize impact on existing W_W case, only case impact is 
	       1x HDR, donot touch on 3GPP state variable */
	  if (bmc_active_rat_sim[other_as_id] == CB_1x_HDR_MODE
#if defined (FEATURE_NAS_CBS_DSDS) && defined (FEATURE_TWO_3GPP_SIM)
          && (cb_camped_rat_sim[other_as_id] != CB_LTE_MODE )
#endif
	   )
	  {
        /*if 1x HDR sub is DDS, disable  W depend of active state of 1x */
		if(cb_dds_info == FALSE)
		{
		  /*if  DDS sub, then disable/enable should be depend on  3gpp2 active status */
		  /*if stack is active on current sub then return true for W disable
		      if stack active is FALSE then return FLASE for W disable*/
          return (cb_reg_inactive_sim[other_as_id] == FALSE) ? TRUE : cb_check_if_3GPP2_stack_active(other_as_id);    
		}
		else 
		{
          /*1x is non DDS sub return FALSE as no need to disable W */
		  return FALSE;
...
#endif
      if(cb_reg_inactive_sim[other_as_id] == FALSE)
      {
        return TRUE;
      }
      else
...
===========================================================================*/
boolean cb_message_is_requested_per_sub
(
  word incoming_message_id,
  cb_as_id_e_type as_id
)
{
  byte search_index ;

  for (search_index=0; search_index<search_list_len_sim[as_id]; search_index++)
  {
    switch(search_lst_sim[as_id][search_index].current_message_id_type)
    {
      case CB_SINGLE_MESSAGE_ID :
        if (incoming_message_id == 
                search_lst_sim[as_id][search_index].single_message_id.message_id)
        {
          return TRUE ;
        }
        break ;
      case CB_MESSAGE_ID_RANGE :
        if ((search_lst_sim[as_id][search_index].message_id_range.min_message_id 
                                                     <= incoming_message_id) &&
            (search_lst_sim[as_id][search_index].
                       message_id_range.max_message_id >= incoming_message_id))
        {
          return TRUE ;
        }
        break ;
      default:
        MSG_FATAL_DS( as_id,"Wrong Message Id type identifier (%d)",
                                   (int) search_lst_sim[as_id][search_index].current_message_id_type,0,0) ;
    }
  }
...
FUNCTION    cb_check_if_3GPP2_stack_active

DESCRIPTION   This function return boolean indicating if 3GPP2 stack is active
...
===========================================================================*/
boolean cb_check_if_3GPP2_stack_active
(
  cb_as_id_e_type as_id
)
{
 /*if any of the stack is active return as active*/
  if ((cb_1x_stack_active_sim[as_id]== TRUE)|| (cb_hdr_stack_active_sim[as_id]==TRUE))
  {
     return TRUE;
  }
  else
...
void cb_process_3gpp2_stack_status_indication
( 
...
)
{
   ...
   cb_as_id_e_type current_sub;
   
   if (active_mode == SYS_SYS_MODE_CDMA )
   {
     cb_1x_stack_active = is_stack_active;
	 bmc_active_rat = CB_1x_HDR_MODE;
   }
   else if (active_mode == SYS_SYS_MODE_HDR)
   {
     cb_hdr_stack_active = is_stack_active;
	 bmc_active_rat = CB_1x_HDR_MODE;
   }

   /* for other sub W   */
   current_sub = cb_as_id;
   cb_as_id = (cb_as_id == CB_AS_ID_1) ? CB_AS_ID_2 : CB_AS_ID_1;
   cb_message_as_id = (byte)(cb_as_id + 1);
   cb_state = cb_cbch_service_state_handler (&cb_state,bmc_active_rat,FALSE);
   cb_as_id = current_sub;
   cb_message_as_id = (byte)(cb_as_id + 1);
