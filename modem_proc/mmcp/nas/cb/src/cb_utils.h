...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/cb/src/cb_utils.h#1 $ $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
05/19/15   vm      Order of FN within the same skip list ? GL1/WL1 expects FN list in ascending order  
...
typedef enum cb_search_list_set_tag
{
  CB_NONE,
  CB_SUBSET,
  CB_SUPERSET,
} cb_search_list_set_type ;

extern byte cb_message_as_id;
#define CB_SUB cb_message_as_id
#ifdef FEATURE_TWO_3GPP_SIM
extern boolean cb_check_if_3GPP2_stack_active(cb_as_id_e_type as_id);
extern void cb_process_3gpp2_stack_status_indication
( 
...
extern cb_search_list_set_type  cb_search_list_superset_sim[MAX_AS_IDS];
extern word cb_bmc_schedule_msg_periodicity_sim[MAX_AS_IDS];
#ifdef FEATURE_UMTS_BMC_MSIM
extern wcdma_priority_type cb_current_wcdma_priority_sim[MAX_AS_IDS];
extern boolean   cb_dds_info_sim[MAX_AS_IDS];
#endif

extern boolean cb_1x_stack_active_sim[MAX_AS_IDS];
extern boolean cb_hdr_stack_active_sim[MAX_AS_IDS];
#define cb_1x_stack_active cb_1x_stack_active_sim[cb_as_id]
#define cb_hdr_stack_active cb_hdr_stack_active_sim[cb_as_id]

extern cb_rat_type cb_camped_rat_sim[MAX_AS_IDS];
#define cb_camped_rat cb_camped_rat_sim[cb_as_id]


extern sys_srv_status_e_type cb_service_sim[MAX_AS_IDS];
extern boolean cb_reg_inactive_sim[MAX_AS_IDS];
#define cb_dup_timer_value cb_dup_timer_value_sim[cb_as_id]
#define cb_search_list_superset  cb_search_list_superset_sim[cb_as_id]
...
#define   cb_reg_inactive cb_reg_inactive_sim[cb_as_id]
...
#endif
extern cb_rat_type cb_camped_rat;
extern dword cb_dup_timer_value;
extern cb_search_list_set_type  cb_search_list_superset;
extern word  cb_bmc_schedule_msg_periodicity;
#ifdef FEATURE_UMTS_BMC_MSIM
extern wcdma_priority_type cb_current_wcdma_priority;
extern boolean   cb_dds_info;
#endif
extern sys_srv_status_e_type cb_service;
extern boolean cb_reg_inactive;

extern boolean cb_1x_stack_active ;
extern boolean cb_hdr_stack_active ;
...
#endif
extern byte cb_msim_high_priority_threshold;

#ifdef FEATURE_DUAL_SIM
   
#define MSG_MED_DS( sub , str, p1, p2, p3)\
    MSG_4 (MSG_SSID_DFLT, MSG_LVL_MED,"DS: SUB %d " str , sub, (p1), (p2), (p3))
    
#define MSG_LOW_DS( sub , str, p1, p2, p3)\
    MSG_4 (MSG_SSID_DFLT, MSG_LVL_LOW,"DS: SUB %d " str , sub, (p1), (p2), (p3))
    
#define MSG_ERROR_DS( sub , str, p1, p2, p3)\
    MSG_4 (MSG_SSID_DFLT, MSG_LVL_ERROR,"DS: SUB %d " str , sub, (p1), (p2), (p3))

#define MSG_FATAL_DS( sub , str, p1, p2, p3)\
    ERR_FATAL ("DS: SUB %d " str , sub, (p1), (p2))
...
#define CB_DEBUG_MESSAGE_BUFFER_SIZE      100

typedef enum 
{
  TO_CB,
  FROM_CB
}cb_message_direction_type_T;

typedef struct cb_debug_buffer_s
{ 
  union cbs_debug_buffer
  { 
    struct cb_info
    {
      byte message_set;
      dword message_id;
    } cb_info_debug_buffer;
  } debug_buffer_type;
  byte present_cb_state;
  byte present_cb_substate;
...
//  byte msg_info;
#endif
}cb_debug_buffer_s_type;

extern cb_debug_buffer_s_type cb_debug_buffer[CB_DEBUG_MESSAGE_BUFFER_SIZE];
extern dword cb_debug_buffer_index;
extern dword cb_msg_info;
extern byte cb_scheduling_message[2][CB_BMC_MAX_RLC_DATA_LENGTH];
extern byte cb_schedule_index;
...
extern void bmc_process_dsm_data_cb_per_sub
(
   byte rlc_id,
   byte no_pdus,
   void *cb_data
...
extern cb_etws_substate_control_type cb_etws_substate_control_sim[MAX_AS_IDS];
...
===========================================================================*/
extern void cb_start_timer( timer_id_T timer_id, dword num_millisecs ,cb_as_id_e_type as_id);
...
===========================================================================*/
extern void cb_stop_timer( timer_id_T timer_id ,cb_as_id_e_type as_id);
/*===========================================================================
...
extern void cb_add_message_to_debug_buffer
(
  byte message_set, 
  dword message_id,
  cb_message_direction_type_T cb_message_direction,
...
FUNCTION    cb_check_if_search_list_superset

DESCRIPTION
  This function is to check whether search list of active sub is subset of other subs
...
extern void cb_check_if_search_list_superset(void);
/*===========================================================================

FUNCTION    cb_evaluate_superset

DESCRIPTION
  This function is to evaluate which sub has superset search_list
...
===========================================================================*/
extern void cb_evaluate_superset(cb_as_id_e_type as_id1,cb_as_id_e_type as_id2);
extern boolean cb_evaluate_superset_utility(cb_as_id_e_type id1,cb_as_id_e_type id2);
extern boolean cb_check_if_same_cell
(
...
#endif
#ifdef FEATURE_UMTS_BMC_MSIM
extern void cb_send_unscheduled_schedule_req_update_w_priority(cb_as_id_e_type as_id);
extern boolean cb_check_if_emergency_message(word message_id);
extern boolean cb_check_if_current_sub_to_be_disabled(void);
extern boolean cb_message_is_requested_per_sub
(
  word incoming_message_id,
