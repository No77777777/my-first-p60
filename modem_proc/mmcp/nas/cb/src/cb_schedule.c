...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/cb/src/cb_schedule.c#1 $ $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...
byte cb_bmc_ctch_rf_per_tti_sim[MAX_AS_IDS] = {0};
#define cb_bmc_ctch_rf_per_tti cb_bmc_ctch_rf_per_tti_sim[cb_as_id]

/* Period of CTCH allocation ( N ) */
word cb_bmc_ctch_allocation_period_sim[MAX_AS_IDS] = {0};
#define cb_bmc_ctch_allocation_period cb_bmc_ctch_allocation_period_sim[cb_as_id]

/* CBS frame offset ( K ) */
byte cb_bmc_cbs_frame_offset_sim[MAX_AS_IDS] = {0};
#define cb_bmc_cbs_frame_offset cb_bmc_cbs_frame_offset_sim[cb_as_id]
...
byte cb_skip_block_sim[MAX_AS_IDS];
#define cb_skip_block cb_skip_block_sim[cb_as_id]
...
)
{
  ...
  while (msg_bitmap_ptr)
  {
    ...
    MSG_HIGH_DS_3(CB_SUB,"bitmap:  SFN: %d %d %d", msg_bitmap_ptr->sfn, sfn, msg_bitmap_ptr->recd_flag);
...
{
  byte init_index ;
  MSG_HIGH_DS_0(CB_SUB,"BMC: Initialize the scheduling table");
...
)
{
  ...
  {
    MSG_ERROR_DS_0(CB_SUB,"CB: Not enough memory to save new msg bitmap info received now") ;
    return ;
  }
    ...
    if (skip_len > 0)
    {
      if ((skip_list = (dword *)cb_mem_alloc(CB_SKIP_SEQUENCE_ALLOC_ID,
                                            sizeof(dword)*skip_len))
                                                                     == NULL)
      {
        MSG_ERROR_DS_0(CB_SUB,"CB: Not enough memory to send L1 Skip Request") ;
        return ;
      }
      ...
      MSG_HIGH_DS_0(CB_SUB,"CB: Skipping Unscheduled empty slots");
...
void cb_bmc_start_new_drx_cycle()
{
  ...
  MSG_HIGH_DS_1(CB_SUB,"BMC: Start new DRX cycle with Last SFN: %d",next_last_sfn_sch_prd);
...
{  
  ...
  MSG_HIGH_DS_0(CB_SUB,"BMC: Last SFN of the current DRX cycle");
  //Start the list at entry point
  message_list = basic_cbch_list_entry_point;
...
)
{
  ...
  MSG_HIGH_DS_0(CB_SUB,"Filter Duplicate messages in the current scheduling message");
...
)
{
  ...
  { 
    ...
    {
      MSG_HIGH_DS_3(CB_SUB,"BMC MDT: MSG Type=%d(%d) at SFN=%lu",
                         msg_type,
            ...
            msg_type == RETX_OLD_MESSAGE_WITHIN_SCH_PRD)
  {
    ...
    {
      MSG_HIGH_DS_2(CB_SUB,"BMC MDT: MSG Type=%d at SFN=%lu",
                         msg_type,
  ...
  else if (msg_type == SCHEDULE_MESSAGE)
  {
	...
	if ((tx_index_number == 0) ||
        (scheduling_table[tx_index_number].scheduling_element.new_tx_message.message_id !=
           scheduling_table[tx_index_number-1].scheduling_element.new_tx_message.message_id))
    {
      MSG_HIGH_DS_3(CB_SUB,"BMC Schedule Msg MDT: MSG Type=%d(%d) at SFN=%lu",
                         msg_type,
                         scheduling_table[tx_index_number].element_id,
                         scheduling_table[tx_index_number].scheduling_element.new_tx_message.scheduled_fn);
    }
  ...
  else if(((tx_index_number == 0) ||
             (scheduling_table[tx_index_number].scheduling_element.new_tx_message.message_id !=
              scheduling_table[tx_index_number-1].scheduling_element.new_tx_message.message_id))
              &&(msg_type == NEW_MESSAGE || msg_type == RETX_OLD_MESSAGE_PREV_SCH_PRD) 
              && scheduling_table[tx_index_number].element_id == CB_NO_MESSAGE_TX)
  {
     MSG_HIGH_DS_3(CB_SUB,"BMC MDT:MSG ID = %d, MSG Type=%d(CB_NO_MESSAGE_TX) at SFN=%lu",
                      message_id,msg_type,arrival_sfn);
  }
...
)
{
  ...
  {
     MSG_HIGH_DS_0(CB_SUB,"Network didnot send Future extension bit map");
     return;
  }
  else
  {
     ...
     {
       MSG_HIGH_DS_1(CB_SUB,"These message extensions(%d) are not currently supported",future_extension_ie);
       return;
     }
     else
     {
       ...
       MSG_HIGH_DS_1(CB_SUB,"Length of serial numbers is %d",length_of_serial_number_list);
       index ++;
       for(dindex=0;dindex<length_of_serial_number_list;dindex++)
       {
         ...
         MSG_HIGH_DS_4(CB_SUB,"serial number = %x appended to SFN = %x, CTCH_bs_index = %x, sfn_found = %d",serial_number,sfn,CTCH_bs_index,sfn_found);
       }
     }
  }
}
...
)
{
  ...
  MSG_HIGH_DS_0(CB_SUB,"BMC: Distribute the table across the message list");
                             ...
                             scheduling_index++)
  {
        ...
        scheduling_table[scheduling_index].element_id == CB_OLD_TX)
    {
                          ...
                          scheduling_element.new_tx_message.message_id)) != NULL))
      {
        ...
        {
          MSG_ERROR_DS_0(CB_SUB,"BMC: Unable to allocate memory");
          return;
        }
        ...
        for (retx_index = 0; retx_index < scheduling_period_length; retx_index++)
        {
                ...
                scheduling_index)
          {
            ...
            {
              MSG_ERROR_DS_0(CB_SUB,"BMC: Unable to allocate enough memory");
              return;
            }
...
)
{
  ...
  MSG_HIGH_DS_2(CB_SUB,"BMC: Processing Scheduling table (%lu to %lu)", next_first_sfn_sch_prd, next_last_sfn_sch_prd);
  ...
  {
    MSG_ERROR_DS_0(CB_SUB,"CB: Not enough memory to send L1 Scheduling Request") ;
      ...
      switch (scheduling_table[process_index].element_id)
      {
        ...
        case CB_OLD_TX:
          ...
          else
          {
            ...
            if(cb_check_if_emergency_message(scheduling_table[process_index].scheduling_element.new_tx_message.message_id))
            {
              scheduled_fn_list[scheduling_command_length] |= 0x20000000;  /*CRITICAL priority Mask*/
            }
#endif
          }
          break ;
        case CB_RETX :
          ...
          break ;
        default:
          MSG_ERROR_DS_0(CB_SUB,"Illegal Scheduling Element Type ID") ;
          break;
      }
  ...
  if ((scheduling_command_length == 0) || (scheduled_fn_list[scheduling_command_length - 1] & (~0x70000000)) != next_last_sfn_sch_prd)
  {
    fake_sfn = (dword)(next_last_sfn_sch_prd | 0x80000000);
    scheduled_fn_list[scheduling_command_length++] = fake_sfn;
  }
  else if ((scheduling_command_length == 0) || (scheduled_fn_list[scheduling_command_length - 1] & (~0x70000000)) == next_last_sch_msg_sfn)
  {
  ...
  MSG_HIGH_DS_1(CB_SUB,"BMC: Sending CB_SCHEDULING_REQ(CTCH_DRX_MODE) request with length = %d :",scheduling_command_length) ;
  for (dindex=0; dindex<prn_number; dindex++)
  {
    MSG_HIGH_DS_3(CB_SUB,"BMC: %lu %lu %lu",scheduled_fn_list[dindex*3],scheduled_fn_list[dindex*3 + 1],scheduled_fn_list[dindex*3 + 2]) ;
  }
  for(prn_number = dindex*3;prn_number < scheduling_command_length;prn_number++)
  {
    MSG_HIGH_DS_1(CB_SUB,"BMC: %lu",scheduled_fn_list[prn_number]) ;
  }
...
)
{
  ...
  {
    MSG_HIGH_DS_0(CB_SUB,"BMC: BMC detected Duplicate schedule message");
    return;
  }

  MSG_HIGH_DS_3(CB_SUB,"BMC: Processing Scheduling information,arrival_sfn = %ul, Offset CTCH BS index = %d and Sch Period length = %d",
  ...
  {
    MSG_ERROR_DS_0(CB_SUB,"BMC: Schedule Message is overlapping with current sch period");
    return;
  }
...
#endif
  {
    MSG_ERROR_DS_0(CB_SUB,"CB: Not enough memory to process Bitmap");
    cb_start_umts_non_drx_mode();
    return ;
  }
  ...
  for (dindex=0; dindex<prn_number; dindex++)
  {
    MSG_HIGH_DS_3(CB_SUB,"BMC msgbitmap: %x %x %x",msg_bitmap[dindex*3],msg_bitmap[dindex*3 + 1],msg_bitmap[dindex*3 + 2]) ;
  }
  for(prn_number = dindex*3;prn_number < bitmap_octet_len;prn_number++)
  {
    MSG_HIGH_DS_1(CB_SUB,"BMC msgbitmap: %x",msg_bitmap[prn_number]) ;
  }
...
#endif
  {
    MSG_ERROR_DS_0(CB_SUB,"CB: Not enough memory to process Scheduling Message") ;
    cb_start_umts_non_drx_mode();
#ifndef FEATURE_MODEM_HEAP
  ...
  {
    MSG_HIGH_DS_0(CB_SUB,"Future extension Bitmap is not included");
  }
#endif
  cb_bmc_filter_duplicate_old_message(cb_bmc_scheduling_table, cb_bmc_scheduling_period_length);
  ...
  {
    MSG_ERROR_DS_0(CB_SUB,"Invalid Schedule Message");
    cb_start_umts_non_drx_mode();
#ifndef FEATURE_MODEM_HEAP
    gs_free((void *)msg_bitmap);
#else
...
)
{
  ...
  {
    MSG_HIGH_DS_2(CB_SUB, "BMC: msg_list or sfn_list is  NULL, msg_list is %x and sfn_list is %x",msg_list,sfn_list);
...
)
{
  ...
  {
    MSG_HIGH_DS_0(CB_SUB,"There are no messages in the message list to skip");
    return;
  }
...
)
{
  ...
  else
  {
    ...
    */
    MSG_ERROR_DS_0(CB_SUB,"CB DRX: Unsupported message description");
  }

  parsing_index++;

  MSG_HIGH_DS_3(CB_SUB,"CB DRX:MDT- MSG Type=0x%0x(%d) at FN=%lu",
...
)
{
  switch (channel_ind)
  {
    ...
    case ECBCH :
      ...
      break ;
    default:
      MSG_FATAL_DS( CB_SUB,"Illegal channel type",0,0,0) ;
  }
...
)
{
  ...
  MSG_HIGH_DS_4(CB_SUB,"CB DRX: scheduling_table = %x, scheduling_period_length is %d, Processing Scheduling table (%lu to %lu)",
  ...
  {
    MSG_ERROR_DS_0(CB_SUB,"CB DRX: Not enough memory to send L1 Scheduling Request") ;
    ...
    if (scheduling_table[process_index].element_id != CB_NO_MESSAGE_TX)
    {
      switch (scheduling_table[process_index].element_id)
      {
        ...
        case CB_RETX :
          ...
          break ;
        default:
          MSG_FATAL_DS( CB_SUB,"Illegal Scheduling Element Type ID",0,0,0) ;
      }
  ...
  MSG_HIGH_DS_0(CB_SUB,"CB DRX: Sending CB_SCHEDULING_REQ(CB_DRX_MODE) request") ;
  ...
  MSG_HIGH_DS_1(CB_SUB,"CB DRX: Scheduling sequence length is %d :",scheduling_command_length) ;
  for (dindex=0; dindex<prn_number; dindex++)
  {
    MSG_HIGH_DS_3(CB_SUB,"CB DRX: Slot# %lu %lu %lu",
    ...
    MSG_HIGH_DS_3(CB_SUB,"CB DRX: FN# %lu %lu %lu",
...
)
{
  ...
  */
  ...
  MSG_HIGH_DS_2(CB_SUB,"CB DRX: Begin slot = %d, End Slot = %d",begin_scheduling_slot,end_scheduling_slot);
  ...
  MSG_HIGH_DS_3(CB_SUB,"CB DRX: Processing Scheduling information, Slots %d-%d New#%d",
  ...
  {
    MSG_HIGH_DS_0(CB_SUB,"CB DRX: Duplicate schedule message detected -> discarded");
    return;
  }
#endif
...
#endif
  {
    MSG_ERROR_DS_0(CB_SUB,"CB DRX: Schedule Message is overlapping with current sch period");
    return;
  }
  ...
  MSG_HIGH_DS_6(CB_SUB,"CB DRX: msgbitmap[0-5]- 0X%02X%02X%02X0X%02X%02X%02X",
                 *(scheduling_page_buff+CB_NEW_MSG_BITMAP_OFFSET),
                   ...
                   scheduling_index<number_of_new_messages; scheduling_index++)
  {
    ...
    */
    ...
    {
      MSG_HIGH_DS_0(CB_SUB,"Bitmap already does not have CB slot description") ;
    ...
    {
      MSG_ERROR_DS_0(CB_SUB,"CB DRX: Trying to read beyond the end of message description buffer");
      break;
    }
...
)
{
  ...
  switch (required_blocks)
  {
    ...
    case CB_NOTHING_TO_SKIP:
      return ;
    default:
      MSG_ERROR_DS_1(CB_SUB,"Illigal skip combination (%X)",required_blocks) ;
      return ;
  }
  ...
  for(skip_index=0; skip_index<skip_list_length; skip_index++)
  {
    ...
    cb_skip_block = cb_skip_block | (1 << skip_block_list[skip_index]);
  }
...
)
{
  ...
  switch (missing_paging_blocks)
  {
    ...
    default:
      MSG_FATAL_DS( CB_SUB,"Illigal paging block indicator (%d)",missing_paging_blocks,0,0) ;
      /*lint -e{527} Safe to return if ERR_FATAL does not return */
      return CB_RECEIVING_FIRST_PB_STATE ;

  }

  MSG_ERROR_DS_0(CB_SUB,"CB: No action has been necessary") ;
  return CB_RECEIVING_FIRST_PB_STATE;
