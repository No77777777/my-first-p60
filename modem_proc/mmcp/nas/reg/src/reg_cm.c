...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/reg/src/reg_cm.c#1 $ $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...
)
{
  ...
  if(msg_p)
  {
    if((msg_p->ue_mode >=SYS_UE_MODE_NONE) &&
...
boolean cm_secure_call_req_is_valid 
(
  const cm_secure_call_s_type *msg_p
)
{
  boolean valid = FALSE;
  if(msg_p)
  {
    if((msg_p->type > SYS_SECURE_CALL_NONE) && 
                      (msg_p->type < SYS_SECURE_CALL_MAX))
    {
        valid = TRUE;
    }
  }
...
FUNCTION NAME
  ...
  cm_high_priority_activity_ind_is_valid

==============================================================================*/

boolean cm_high_priority_activity_ind_is_valid
(
 const cm_high_priority_activity_ind_s_type *msg_p
)
{
  boolean valid = FALSE;
...
FUNCTION NAME
  ...
  cm_3GPP2_status_ind_is_valid

==============================================================================*/

boolean cm_3GPP2_status_ind_is_valid
(
 const cm_3gpp2_status_ind_s_type *msg_p
)
{
  boolean valid = FALSE;
  ...
  {
    valid = TRUE;
	if(IS_NOT_VALID_SUB_ID(msg_p->as_id))
	{
      valid = FALSE;
	}
	if(!((msg_p->active_rat == SYS_SYS_MODE_CDMA)||((msg_p->active_rat == SYS_SYS_MODE_HDR))))
	{
       valid = FALSE;
	}
