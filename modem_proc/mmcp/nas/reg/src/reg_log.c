...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/reg/src/reg_log.c#1 $ $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...
)
{
  ...
  if ( mm_dual_standby_mode  || mm_sglte_mode)
  {
    ds_log_ptr = (LOG_UMTS_DS_NAS_REG_LOG_PACKET_C_type *)
                   nas_log_alloc(LOG_UMTS_DS_NAS_REG_LOG_PACKET_C, 
                             sizeof(LOG_UMTS_DS_NAS_REG_LOG_PACKET_C_type),
                             reg_message_as_id);

    if (ds_log_ptr)
    {
      ds_log_ptr->as_id = (uint8) reg_as_id + 1;
      ...
      log_util_log_packet_commit((sys_modem_as_id_e_type)reg_as_id, ds_log_ptr);
    }
  }
  else
#endif
  {
    log_p = (LOG_UMTS_NAS_REG_LOG_PACKET_C_type*)nas_log_alloc(LOG_UMTS_NAS_REG_LOG_PACKET_C,
    ...
    if (log_p)
    {
      ...
      log_util_log_packet_commit((sys_modem_as_id_e_type)reg_as_id, log_p);
    }
  }
}
...
)
{
  ...
  if ( mm_dual_standby_mode || mm_sglte_mode)
  {
    ds_log_ptr = (LOG_UMTS_DS_NAS_EPLMN_LIST_LOG_PACKET_C_type *)
                   nas_log_alloc(LOG_UMTS_DS_NAS_EPLMN_LIST_LOG_PACKET_C, 
...
#endif
  {
    log_p = (LOG_UMTS_NAS_EPLMN_LIST_LOG_PACKET_C_type*)nas_log_alloc(LOG_UMTS_NAS_EPLMN_LIST_LOG_PACKET_C,
...
)
{
  ...
  if (is_added)
  {
    MSG_HIGH_DS_2(REG_LOCAL_SUB, "=REG= PLMN(%d-%d) is added to FPLMN List", mcc, mnc);
  }
  else
  {
    MSG_HIGH_DS_2(REG_LOCAL_SUB, "=REG= PLMN(%d-%d) is removed from FPLMN List", mcc, mnc);
  }
#ifdef FEATURE_DUAL_SIM
  if ( mm_dual_standby_mode )
  {
    ds_log_ptr = (LOG_UMTS_DS_NAS_FPLMN_LIST_UPDATE_LOG_PACKET_C_type *)
                   nas_log_alloc(LOG_UMTS_DS_NAS_FPLMN_LIST_UPDATE_LOG_PACKET_C, 
                             sizeof(LOG_UMTS_DS_NAS_FPLMN_LIST_UPDATE_LOG_PACKET_C_type),
                             reg_message_as_id);

    if (ds_log_ptr)
    {
      ds_log_ptr->as_id = (uint8) as_id + 1;
      /*
...
#endif
  {
    log_p = (LOG_UMTS_NAS_FPLMN_LIST_UPDATE_LOG_PACKET_C_type*)nas_log_alloc(LOG_UMTS_NAS_FPLMN_LIST_UPDATE_LOG_PACKET_C,
...
)
{
  ...
  if ( mm_dual_standby_mode )
  {
    if( fplmn_list_p->length < MAX_NUMBER_FORBIDDEN_PLMNS)
    {
      size_of_alloc = LOG_PTR_SIZE + sizeof(uint8) + sizeof(uint8) + (fplmn_list_p->length * sizeof( sys_plmn_id_s_type));
    }
    else
    {
      size_of_alloc = sizeof(LOG_UMTS_DS_NAS_FPLMN_LIST_LOG_PACKET_C_type);
    }
	
    ds_log_ptr = (LOG_UMTS_DS_NAS_FPLMN_LIST_LOG_PACKET_C_type *)
                   nas_log_alloc(LOG_UMTS_DS_NAS_FPLMN_LIST_LOG_PACKET_C, size_of_alloc,
                                     reg_message_as_id);
...
#endif
  {
    if( fplmn_list_p->length < MAX_NUMBER_FORBIDDEN_PLMNS)
    {
      size_of_alloc = LOG_PTR_SIZE + sizeof(uint8) + (fplmn_list_p->length * sizeof( sys_plmn_id_s_type));
    }
    else
    {
      size_of_alloc = sizeof(LOG_UMTS_NAS_FPLMN_LIST_LOG_PACKET_C_type);
    }
	
    log_p = (LOG_UMTS_NAS_FPLMN_LIST_LOG_PACKET_C_type*)nas_log_alloc(
                         LOG_UMTS_NAS_FPLMN_LIST_LOG_PACKET_C,size_of_alloc,reg_message_as_id);
...
)
{
  ...
  if ( mm_dual_standby_mode || mm_sglte_mode)
  {
    ds_log_ptr = (LOG_UMTS_DS_NAS_HPLMN_SEARCH_TIMER_START_LOG_PACKET_C_type *)
                   nas_log_alloc(LOG_UMTS_DS_NAS_HPLMN_SEARCH_TIMER_START_LOG_PACKET_C, 
...
#endif
  {
    log_p = (LOG_UMTS_NAS_HPLMN_SEARCH_TIMER_START_LOG_PACKET_C_type*)nas_log_alloc(LOG_UMTS_NAS_HPLMN_SEARCH_TIMER_START_LOG_PACKET_C,
...
)
{
  ...
  if ( mm_dual_standby_mode || mm_sglte_mode)
  {
    ds_log_ptr = (LOG_UMTS_DS_NAS_HPLMN_SEARCH_TIMER_EXPIRE_LOG_PACKET_C_type *)
                   nas_log_alloc(LOG_UMTS_DS_NAS_HPLMN_SEARCH_TIMER_EXPIRE_LOG_PACKET_C, 
...
#endif
  {
    log_p = (LOG_UMTS_NAS_HPLMN_SEARCH_TIMER_EXPIRE_LOG_PACKET_C_type*)nas_log_alloc(LOG_UMTS_NAS_HPLMN_SEARCH_TIMER_EXPIRE_LOG_PACKET_C,
  ...
  if ( mm_dual_standby_mode  || mm_sglte_mode)
  {
    uint8 event_packet = (uint8) as_id + 1;
    nas_event_report_payload(EVENT_DS_HPLMN_TIMER_EXPIRED, 1, (void *)&event_packet,reg_message_as_id);
  }
  else
#endif
  {
...
)
{
  ...
  if ( mm_dual_standby_mode || mm_sglte_mode)
  {
    ds_log_ptr = (LOG_UMTS_DS_NAS_HPLMN_SEARCH_START_LOG_PACKET_C_type *)
                   nas_log_alloc(LOG_UMTS_DS_NAS_HPLMN_SEARCH_START_LOG_PACKET_C, 
...
#endif
  {
    log_p = (LOG_UMTS_NAS_HPLMN_SEARCH_START_LOG_PACKET_C_type*)nas_log_alloc(LOG_UMTS_NAS_HPLMN_SEARCH_START_LOG_PACKET_C,
...
)
{
  ...
  if ( mm_dual_standby_mode  || mm_sglte_mode)
  {
    ds_log_ptr = (LOG_UMTS_DS_NAS_HPLMN_SEARCH_END_LOG_PACKET_C_type *)
                   nas_log_alloc(LOG_UMTS_DS_NAS_HPLMN_SEARCH_END_LOG_PACKET_C, 
...
#endif
  {
    log_p = (LOG_UMTS_NAS_HPLMN_SEARCH_END_LOG_PACKET_C_type*)nas_log_alloc(LOG_UMTS_NAS_HPLMN_SEARCH_END_LOG_PACKET_C,
...
)
{
  ...
  {
    log_p = (LOG_UMTS_NAS_PPLMN_LIST_LOG_PACKET_C_type*)nas_log_alloc(LOG_UMTS_NAS_PPLMN_LIST_LOG_PACKET_C,
...
/*=============================================================================
	 	 		Function Name: REG_LOG_LTE_CELL_LIST
	 	 		 
	 	 		Description: This function sends the log packet for LTE cell list database
	 	 		Return Value: cell pointer of the cell which will be logged next
==============================================================================*/
	 	 		 
cell_list_type* reg_log_lte_cell_ist
(
  lte_cell_list_type** lte_cell_list,
  boolean  lte_available,
  sys_modem_as_id_e_type           as_id,
  cell_list_type*          current_cell_ptr,
  uint8 *length_of_list
)
{
  cell_list_type*          temp_cell_id_ptr = NULL;
  lte_cell_list_type *list_ptr= NULL;
  uint8     cell_count=0,i=0;
  LOG_UMTS_NAS_LTE_CELL_LIST_LOG_PACKET_C_type * log_p = NULL;
  uint32 size_of_alloc = 0;
   	 		 
  if(*lte_cell_list == NULL)
  {
    MSG_ERROR_DS_1(REG_LOCAL_SUB, "=REG= LTE IRAT BPLMN : List(%d) is empty ",lte_available);
    return NULL;
  }
	 	 		 
  list_ptr = *lte_cell_list;
  ...
  {
    temp_cell_id_ptr = current_cell_ptr;
  }
  if(((*lte_cell_list)->length - *length_of_list) < LOG_MAX_NUMBER_LTE_CELL_LIST)
  {
    size_of_alloc = LOG_PTR_SIZE+ sizeof(sys_modem_as_id_e_type)+sizeof(uint8)+sizeof( sys_plmn_id_s_type)+ sizeof(uint8)+sizeof(sys_radio_access_tech_e_type)+sizeof(uint32)+(sizeof(log_lte_cell_list)*((*lte_cell_list)->length - *length_of_list));
  }
  else
  {
    size_of_alloc = sizeof(LOG_UMTS_NAS_LTE_CELL_LIST_LOG_PACKET_C_type);
  }
  log_p = (LOG_UMTS_NAS_LTE_CELL_LIST_LOG_PACKET_C_type*)nas_log_alloc(
                LOG_UMTS_NAS_LTE_CELL_LIST_LOG_PACKET_C,size_of_alloc,reg_message_as_id);
  if(log_p)
  {
    log_p->version = (uint8)1;
    log_p->as_id = (uint8)as_id + 1;

    log_p->list_type = lte_available;
    /*populate RAT*/
    log_p->rat = list_ptr->rat;
    log_p->length = list_ptr->length;
    /* Populate PLMN ID */
    log_p->plmn.plmn_id[0] = list_ptr->plmn.identity[0];
    log_p->plmn.plmn_id[1] = list_ptr->plmn.identity[1];
    log_p->plmn.plmn_id[2] = list_ptr->plmn.identity[2];

    while(temp_cell_id_ptr != NULL && cell_count < LOG_MAX_NUMBER_LTE_CELL_LIST)
    {
      log_p->lte_cell_list[cell_count].lac =temp_cell_id_ptr->lac;
      log_p->lte_cell_list[cell_count].cell_id =temp_cell_id_ptr->cell_id;
      log_p->lte_cell_list[cell_count].count = temp_cell_id_ptr->count;
      log_p->lte_cell_list[cell_count].ecount =temp_cell_id_ptr->earfcn.count;
      for(i=0;i< NAS_MAX_EARFCN_COUNT ;i++)
      {
        if(lte_available)
        {
          log_p->lte_cell_list[cell_count].earfcn_nas[i]= temp_cell_id_ptr->earfcn.earfcn_nas[i];
        }
        else
        {
          log_p->lte_cell_list[cell_count].earfcn_nas[i]= INVALID_EARFCN;
        }
      }
      log_p->lte_cell_list[cell_count].timer_count = 0XFFFFFFFF;
      if(!lte_available)
      {
        log_p->lte_cell_list[cell_count].timer_count =   temp_cell_id_ptr->timer_count;
      }
      cell_count++;
  ...
  {
    MSG_ERROR_DS_0(REG_SUB, "=REG= IRAT BPLMN LOGGING reg_log_lte_cell_ist : not able to allocate memory ");
    *lte_cell_list = NULL;
    temp_cell_id_ptr = NULL;
    return temp_cell_id_ptr;
  }
  if(cell_count == LOG_MAX_NUMBER_LTE_CELL_LIST )
  {
    *lte_cell_list = list_ptr;
    *length_of_list += LOG_MAX_NUMBER_LTE_CELL_LIST;
  }
  if(temp_cell_id_ptr == NULL)
  {
   *lte_cell_list = list_ptr->next_ptr;
   *length_of_list = 0;
  }
  return temp_cell_id_ptr;
}