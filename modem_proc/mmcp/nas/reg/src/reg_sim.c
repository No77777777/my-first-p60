...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/reg/src/reg_sim.c#4 $
...
extern uint8  mm_emm_max_fplmn_count_dos_mitigation_sim[MAX_AS_IDS];
#define mm_emm_max_fplmn_count_dos_mitigation mm_emm_max_fplmn_count_dos_mitigation_sim[mm_sub_id]
...
reg_sim_backoff_fplmn_list_info_s_type reg_sim_backoff_fplmn_list_sim[MAX_NAS_STACKS];
#define reg_sim_backoff_fplmn_list reg_sim_backoff_fplmn_list_sim[reg_sub_id]
#else
...
boolean reg_sim_changed_sim[MAX_NAS_STACKS] = {FALSE, FALSE, FALSE};
boolean reg_sim_swapped_sim[MAX_NAS_STACKS] = {FALSE, FALSE, FALSE};
reg_as_id_e_type reg_sim_swapped_target_as_id_sim[MAX_NAS_STACKS] = {REG_AS_ID_1, REG_AS_ID_2, REG_AS_ID_3};
boolean reg_debug_perso_status_sim[MAX_NAS_STACKS] = {FALSE, FALSE, FALSE};
#else
boolean reg_sim_changed_sim[MAX_NAS_STACKS] = {FALSE, FALSE};
boolean reg_sim_swapped_sim[MAX_NAS_STACKS] = {FALSE, FALSE};
reg_as_id_e_type reg_sim_swapped_target_as_id_sim[MAX_NAS_STACKS] = {REG_AS_ID_1, REG_AS_ID_2};
boolean reg_debug_perso_status_sim[MAX_NAS_STACKS] = {FALSE, FALSE};
#endif
#define reg_sim_changed  reg_sim_changed_sim[reg_as_id]
#define reg_sim_swapped  reg_sim_swapped_sim[reg_as_id]
#define reg_sim_swapped_target_as_id reg_sim_swapped_target_as_id_sim[reg_as_id]
#define reg_debug_perso_status reg_debug_perso_status_sim[reg_as_id]
#else
boolean reg_sim_changed = FALSE;
boolean reg_debug_perso_status = FALSE;
#endif
...
)
{
  ...
  switch(reg_sim_card_mode)
  {
    case MMGSDI_APP_SIM:
      ...
      */
      if (reg_sim_read_ens_flag() != FALSE)
      {
  ...
  if (reg_sim_card_mode == MMGSDI_APP_USIM )
  {
    ...
    */
          ...
          MMGSDI_USIM_NASCONFIG,&file_size) == MMGSDI_SUCCESS)
    {
      ...
      ef_nasconfig_p = (byte *)modem_mem_calloc(1, (unsigned int)file_size, MODEM_MEM_CLIENT_NAS);
...
{
#ifdef FEATURE_DUAL_SIM
  MSG_HIGH_DS_1(REG_SUB, "=REG= reg_sim_get_eutran_not_allowed() returns %d",
       reg_sim_nasconfig_sim[as_id].eutran_not_allowed);
  return reg_sim_nasconfig_sim[as_id].eutran_not_allowed;
#else
  (void)as_id;
...
)
{
  ...
  {
    MSG_ERROR_DS_0(REG_SUB, "Invalid AS ID");
    return ;
  }
...
)
{
  ...
  uint32  mnc;
  uint8  i;
  uint8  local_sub_id = (uint8)(as_id+1);
  reg_sim_backoff_fplmn_list_info_s_type *backoff_fplmn_list_ptr;
#ifdef FEATURE_DUAL_SIM
  if(IS_NOT_VALID_AS_ID(as_id))
  {
    MSG_ERROR_DS_0(REG_SUB, "Invalid AS ID");
...
#endif   
#if defined(FEATURE_DUAL_SIM) && defined(FEATURE_DUAL_DATA)
  backoff_fplmn_list_ptr = &reg_sim_backoff_fplmn_list_sim[as_id];
#else
  backoff_fplmn_list_ptr = &reg_sim_backoff_fplmn_list;
#endif

  MSG_HIGH_DS_1(local_sub_id, "=REG= BACKOFF PLMN LIST (length = %d)", backoff_fplmn_list_ptr->length);

  if (backoff_fplmn_list_ptr->length > 0)
  {
    MSG_HIGH_DS_0(local_sub_id, "=REG=  MCC-MNC     REMAINING TIME   CAUSE OF ADDITION");

    for (i = 0; i < backoff_fplmn_list_ptr->length; i++)
    {
      sys_plmn_get_mcc_mnc
      (
        backoff_fplmn_list_ptr->plmn_timer_list[i].plmn,
        &plmn_id_is_undefined,
        &mnc_includes_pcs_digit,
        &mcc,
        &mnc
      ...
      if(mnc_includes_pcs_digit)
      {
        if(backoff_fplmn_list_ptr->plmn_timer_list[i].timer_count == 
                                                 REG_TIMERS_MAX_TIMER_VALUE)
        {
          if(backoff_fplmn_list_ptr->plmn_timer_list[i].cause == 
                                          SYS_BLOCK_PLMN_CAUSE_MAX_REGISTRATION_FAILURE)
          {
            MSG_HIGH_DS_3(local_sub_id, "=REG=  %03lu-%03lu     Permanently   Added for Max registration failure index %d", mcc, mnc,
				 backoff_fplmn_list_ptr->plmn_timer_list[i].eplmn_index);
          }
          else if (backoff_fplmn_list_ptr->plmn_timer_list[i].cause == 
                                          SYS_BLOCK_PLMN_CAUSE_VOICE_NOT_AVAILABLE)
          {
            MSG_HIGH_DS_3(local_sub_id, "=REG=  %03lu-%03lu     Permanently   Added as voice is not available index %d", mcc, mnc,
				 backoff_fplmn_list_ptr->plmn_timer_list[i].eplmn_index);
          }
          else if (backoff_fplmn_list_ptr->plmn_timer_list[i].cause == 
                                          SYS_BLOCK_PLMN_CAUSE_IMS_REGISTRATION_FAILURE)
          {
            MSG_HIGH_DS_3(local_sub_id, "=REG=  %03lu-%03lu     Permanently   Added for IMS REG failure index %d", mcc, mnc,
				 backoff_fplmn_list_ptr->plmn_timer_list[i].eplmn_index);
          }
          else if(backoff_fplmn_list_ptr->plmn_timer_list[i].cause == 
                                          SYS_BLOCK_PLMN_CAUSE_OTHERS)
          {
            MSG_HIGH_DS_3(local_sub_id, "=REG=  %03lu-%03lu     Permanently   Added for other causes index %d", mcc, mnc,
				 backoff_fplmn_list_ptr->plmn_timer_list[i].eplmn_index);
          }
          else if(backoff_fplmn_list_ptr->plmn_timer_list[i].cause == 
                                          SYS_BLOCK_PLMN_CAUSE_PRIORITY )
          {
            MSG_HIGH_DS_3(local_sub_id, "=REG=  %03lu-%03lu     Permanently   Added for DS blocking Priority causes index %d", mcc, mnc,
				 backoff_fplmn_list_ptr->plmn_timer_list[i].eplmn_index);
          }
		  else if(backoff_fplmn_list_ptr->plmn_timer_list[i].cause == 
                                          SYS_BLOCK_PLMN_CAUSE_EMC_IN_LSM )
          {
            MSG_HIGH_DS_3(local_sub_id, "=REG=  %03lu-%03lu     Permanently   Added for EMC call block index %d", mcc, mnc,
				 backoff_fplmn_list_ptr->plmn_timer_list[i].eplmn_index);
          }
        }
        else
        {
          if(backoff_fplmn_list_ptr->plmn_timer_list[i].cause == 
                                            SYS_BLOCK_PLMN_CAUSE_MAX_REGISTRATION_FAILURE)
          {
            MSG_HIGH_DS_4(local_sub_id, "=REG=  %03lu-%03lu     %10lu       Added for Max registration failure index %d", mcc, mnc,
                 backoff_fplmn_list_ptr->plmn_timer_list[i].timer_count,
				 backoff_fplmn_list_ptr->plmn_timer_list[i].eplmn_index);
          }
          else if(backoff_fplmn_list_ptr->plmn_timer_list[i].cause == 
                                            SYS_BLOCK_PLMN_CAUSE_VOICE_NOT_AVAILABLE)
          {
            MSG_HIGH_DS_4(local_sub_id, "=REG=  %03lu-%03lu     %10lu       Added as voice is not available index %d", mcc, mnc,
                 backoff_fplmn_list_ptr->plmn_timer_list[i].timer_count,
				 backoff_fplmn_list_ptr->plmn_timer_list[i].eplmn_index);
          }
          else if(backoff_fplmn_list_ptr->plmn_timer_list[i].cause == 
                                            SYS_BLOCK_PLMN_CAUSE_IMS_REGISTRATION_FAILURE)
          {
            MSG_HIGH_DS_4(local_sub_id, "=REG=  %03lu-%03lu     %10lu       Added for IMS REG failure index %d", mcc, mnc,
                 backoff_fplmn_list_ptr->plmn_timer_list[i].timer_count,
                 backoff_fplmn_list_ptr->plmn_timer_list[i].eplmn_index);
          }
          else if(backoff_fplmn_list_ptr->plmn_timer_list[i].cause == 
                                          SYS_BLOCK_PLMN_CAUSE_OTHERS)
          {
            MSG_HIGH_DS_4(local_sub_id, "=REG=  %03lu-%03lu     %10lu       Added for other causes index %d", mcc, mnc,
                 backoff_fplmn_list_ptr->plmn_timer_list[i].timer_count,
				 backoff_fplmn_list_ptr->plmn_timer_list[i].eplmn_index);
          }
          else if(backoff_fplmn_list_ptr->plmn_timer_list[i].cause == 
                                          SYS_BLOCK_PLMN_CAUSE_PRIORITY )
          {
            MSG_HIGH_DS_4(local_sub_id, "=REG=  %03lu-%03lu     %10lu    Added for DS Blocking Priority causes index %d", mcc, mnc,
                 backoff_fplmn_list_ptr->plmn_timer_list[i].timer_count,
				 backoff_fplmn_list_ptr->plmn_timer_list[i].eplmn_index);
          }
		  else if(backoff_fplmn_list_ptr->plmn_timer_list[i].cause == 
                                          SYS_BLOCK_PLMN_CAUSE_EMC_IN_LSM )
          {
            MSG_HIGH_DS_4(local_sub_id, "=REG=  %03lu-%03lu     %10lu    Added for EMC call block index %d", mcc, mnc,
                 backoff_fplmn_list_ptr->plmn_timer_list[i].timer_count,
				 backoff_fplmn_list_ptr->plmn_timer_list[i].eplmn_index);
          }
        }
      ...
      else
      {
                                                 ...
                                                 REG_TIMERS_MAX_TIMER_VALUE)
        {
          if(backoff_fplmn_list_ptr->plmn_timer_list[i].cause == 
                                          SYS_BLOCK_PLMN_CAUSE_MAX_REGISTRATION_FAILURE)
          {
            MSG_HIGH_DS_3(local_sub_id, "=REG=  %03lu- %02lu     Permanently   Added for Max registration failure index %d", mcc, mnc,
				backoff_fplmn_list_ptr->plmn_timer_list[i].eplmn_index) ;
          }
          else if(backoff_fplmn_list_ptr->plmn_timer_list[i].cause == 
                                          SYS_BLOCK_PLMN_CAUSE_VOICE_NOT_AVAILABLE)
          {
            MSG_HIGH_DS_3(local_sub_id, "=REG=  %03lu- %02lu     Permanently   Added as voice is not available index %d", mcc, mnc,
				backoff_fplmn_list_ptr->plmn_timer_list[i].eplmn_index) ;
          }
          else if(backoff_fplmn_list_ptr->plmn_timer_list[i].cause == 
                                          SYS_BLOCK_PLMN_CAUSE_IMS_REGISTRATION_FAILURE)
          {
            MSG_HIGH_DS_3(local_sub_id, "=REG=  %03lu- %02lu     Permanently   Added for IMS REG failure index %d", mcc, mnc,
                    backoff_fplmn_list_ptr->plmn_timer_list[i].eplmn_index) ;
          }
          else if(backoff_fplmn_list_ptr->plmn_timer_list[i].cause == 
                                       SYS_BLOCK_PLMN_CAUSE_OTHERS)
          {
            MSG_HIGH_DS_3(local_sub_id, "=REG=  %03lu- %02lu     Permanently   Added for other causes index %d ", mcc, mnc,
				backoff_fplmn_list_ptr->plmn_timer_list[i].eplmn_index) ;
          }
          else if(backoff_fplmn_list_ptr->plmn_timer_list[i].cause == 
                                       SYS_BLOCK_PLMN_CAUSE_PRIORITY )
          {
            MSG_HIGH_DS_3(local_sub_id, "=REG=  %03lu-%02lu     Permanently   Added for DS blocking priority causes index %d", mcc, mnc,
				backoff_fplmn_list_ptr->plmn_timer_list[i].eplmn_index) ;
          }
		  else if(backoff_fplmn_list_ptr->plmn_timer_list[i].cause == 
                                       SYS_BLOCK_PLMN_CAUSE_EMC_IN_LSM )
          {
            MSG_HIGH_DS_3(local_sub_id, "=REG=  %03lu-%02lu     Permanently   Added for EMC call block index %d", mcc, mnc,
				backoff_fplmn_list_ptr->plmn_timer_list[i].eplmn_index) ;
          }
        }
        else
        {
          if(backoff_fplmn_list_ptr->plmn_timer_list[i].cause == 
                                            SYS_BLOCK_PLMN_CAUSE_MAX_REGISTRATION_FAILURE)
          {
            MSG_HIGH_DS_4(local_sub_id, "=REG=  %03lu- %02lu     %10lu       Added for Max registration failure index %d", mcc, mnc,
                 backoff_fplmn_list_ptr->plmn_timer_list[i].timer_count,
				 backoff_fplmn_list_ptr->plmn_timer_list[i].eplmn_index);
          }
          else if(backoff_fplmn_list_ptr->plmn_timer_list[i].cause == 
                                            SYS_BLOCK_PLMN_CAUSE_VOICE_NOT_AVAILABLE)
          {
            MSG_HIGH_DS_4(local_sub_id, "=REG=  %03lu- %02lu     %10lu       Added as voice is not available index %d", mcc, mnc,
                 backoff_fplmn_list_ptr->plmn_timer_list[i].timer_count,
				 backoff_fplmn_list_ptr->plmn_timer_list[i].eplmn_index);
          }
          else if(backoff_fplmn_list_ptr->plmn_timer_list[i].cause == 
                                            SYS_BLOCK_PLMN_CAUSE_IMS_REGISTRATION_FAILURE)
          {
            MSG_HIGH_DS_4(local_sub_id, "=REG=  %03lu- %02lu     %10lu       Added for IMS REG failure index %d", mcc, mnc,
                 backoff_fplmn_list_ptr->plmn_timer_list[i].timer_count,
                 backoff_fplmn_list_ptr->plmn_timer_list[i].eplmn_index);
          }
          else if(backoff_fplmn_list_ptr->plmn_timer_list[i].cause == 
                                            SYS_BLOCK_PLMN_CAUSE_OTHERS)
          {
            MSG_HIGH_DS_4(local_sub_id, "=REG=  %03lu- %02lu     %10lu       Added for other causes index %d", mcc, mnc,
                 backoff_fplmn_list_ptr->plmn_timer_list[i].timer_count,
				 backoff_fplmn_list_ptr->plmn_timer_list[i].eplmn_index);
          }
          else if(backoff_fplmn_list_ptr->plmn_timer_list[i].cause == 
                                            SYS_BLOCK_PLMN_CAUSE_PRIORITY )
          {
            MSG_HIGH_DS_4(local_sub_id, "=REG=  %03lu- %02lu     %10lu       Added for DS Blocking priority index %d", mcc, mnc,
                 backoff_fplmn_list_ptr->plmn_timer_list[i].timer_count,
				 backoff_fplmn_list_ptr->plmn_timer_list[i].eplmn_index);
          }
		  else if(backoff_fplmn_list_ptr->plmn_timer_list[i].cause == 
                                            SYS_BLOCK_PLMN_CAUSE_EMC_IN_LSM )
          {
            MSG_HIGH_DS_4(local_sub_id, "=REG=  %03lu- %02lu     %10lu       Added for EMC call block index %d", mcc, mnc,
                 backoff_fplmn_list_ptr->plmn_timer_list[i].timer_count,
				 backoff_fplmn_list_ptr->plmn_timer_list[i].eplmn_index);
          }
        }
...
FUNCTION 
  reg_check_sim_swap_event

DESCRIPTION
  Function that checks whether any SIM swap (due to flex mapping or hard swap) is triggered

RETURN VALUE
  sim_swapped - boolean to indicate sim is swapped or not.

===========================================================================*/
boolean reg_check_sim_swap_event
(
  reg_sim_imsi_s_type sim_imsi,
  reg_nv_imsi_s_type nv_imsi,
  reg_as_id_e_type* reg_as_id_target_p
)
{
  boolean sim_swapped = FALSE;
  reg_as_id_e_type as_id_target = REG_AS_ID_1;
  reg_as_id_e_type sub_id_current = reg_sub_id;
  reg_as_id_e_type as_id_current = reg_as_id;
  reg_nv_imsi_s_type target_nv_imsi;
  boolean nv_read_status;

  MSG_HIGH_DS_2(REG_SUB, "=REG= reg_sim_max_subscription: %d, MAX_AS_IDS: %d", reg_sim_max_subscription, MAX_AS_IDS);
  for(as_id_target = REG_AS_ID_1; (as_id_target < reg_sim_max_subscription) && (as_id_target < MAX_AS_IDS); as_id_target++)
  {
    /* Change context to other sub */
    reg_set_as_id(as_id_target);

    MSG_HIGH_DS_2(REG_SUB, "=REG= check sim swap event with reg_as_id: %d, reg_sub_id: %d", reg_as_id, reg_sub_id);
    if(reg_sub_id != sub_id_current)
    {
      /* If target subscription is already available no need to consider it as a candidate for swap */
      if((reg_sim_card_mode == MMGSDI_APP_SIM) || (reg_sim_card_mode == MMGSDI_APP_USIM))
      {
        MSG_HIGH_DS_0(REG_SUB, "=REG= Subscription already became available. No need to consider it for swap");
        reg_debug_sim_swap_status = REG_DEBUG_SIM_CHANGED_NOT_SWAPPED_SUB_AVAILABLE;
        continue;
      }

      /* Read NV IMSI */
      NAS_ENTER_CRIT_SECT(mm_sim_swap_crit_sec);
      nv_read_status = reg_nv_read_imsi(&target_nv_imsi);
	  NAS_EXIT_CRIT_SECT(mm_sim_swap_crit_sec);

      if (nv_read_status)
      {
        /* Compare current sub SIM IMSI with target sub NV IMSI. They will be same In case of SIM soft swap */
        if(memcmp(sim_imsi.digits, target_nv_imsi.digits, sim_imsi.length) == 0)
        {
          sim_swapped = TRUE;
          *reg_as_id_target_p = reg_as_id;
          MSG_HIGH_DS_2(REG_SUB, "=REG= IMSI matched for swap sim:%d with %d", as_id_current, reg_as_id);
          break;
        }
        else
        {
          MSG_HIGH_DS_0(REG_SUB, "=REG= IMSI is not matched for swap ");
          reg_debug_sim_swap_status = REG_DEBUG_SIM_CHANGED_NOT_SWAPPED_NO_MATCH;
        }
      }
      else
      {
        MSG_HIGH_DS_0(REG_SUB, "=REG= NV read failed for IMSI");
        reg_debug_sim_swap_status = REG_DEBUG_SIM_CHANGED_NOT_SWAPPED_NV_READ_FAILED;
      }
    }
  }
	
  /* Change back context to current SUB */
  reg_set_as_id(as_id_current);

  if(sim_swapped)
  {
    MSG_HIGH_DS_2(REG_SUB, "=REG= SIM swap triggered for AS IDs: %d - %d ", as_id_current, *reg_as_id_target_p);
  }
...
)
{
  ...
  memset(&nv_imsi, 0, sizeof(reg_nv_imsi_s_type));
  reg_debug_perso_status = FALSE;
  if((reg_sim_card_mode == MMGSDI_APP_SIM) || (reg_sim_card_mode == MMGSDI_APP_USIM))
  {
    /* SIM card moe got set before we get it. SIM available request came without sim not available */
    reg_debug_perso_status = TRUE;
	MSG_HIGH_DS_0(REG_SUB, "=REG= sim card mode already set before we process it.");
  }
  ...
  MSG_HIGH_DS_0(REG_SUB, "=REG= reg_sim_load_card_mode called");
      ...
      (session_get_info_ptr.app_state != MMGSDI_APP_STATE_READY_FOR_PERSO)))
  {
	...
	reg_debug_sim_swap_status = REG_DEBUG_SIM_CARD_MODE_NOT_DEFINED;
    MSG_HIGH_DS_2(REG_SUB, "=REG= reg_sim_card_mode=%d, session_get_info_ptr.app_state=%d", reg_sim_card_mode, session_get_info_ptr.app_state);
  ...
  MSG_HIGH_DS_2(REG_SUB, "=REG= sim_read_status=%d, nv_read_status=%d", sim_read_status, nv_read_status);

  if (!sim_read_status)
    reg_debug_sim_swap_status = REG_DEBUG_SIM_IMSI_READ_FAILED;
  else if ((sim_imsi.length == 0) || (sim_imsi.length > REG_SIM_IMSI_MAX_LENGTH))
    reg_debug_sim_swap_status = REG_DEBUG_SIM_IMSI_ILLEGAL;
  else
  {
    if(!nv_read_status)
      reg_debug_sim_swap_status = REG_DEBUG_NV_IMSI_READ_FAILED;
    reg_sim_changed = (boolean)(memcmp(sim_imsi.digits, nv_imsi.digits, sim_imsi.length) != 0);
  }

#if defined(FEATURE_DUAL_SIM)
  /* Check if there is any SIM swap due to flex mapping change OR hard swap. Then swap respective NV contexts */
  reg_sim_swapped = FALSE;
  if(reg_sim_changed && nv_read_status)
  {
    reg_debug_sim_swap_status = REG_DEBUG_SIM_CHANGED;
    /* SIM IMSI is different from NV IMSI. Check if there is any SIM soft swap due to flex mapping change */
    reg_sim_swapped = reg_check_sim_swap_event(sim_imsi, nv_imsi, &reg_sim_swapped_target_as_id);

    MSG_HIGH_DS_2(REG_SUB, "=REG= sim_changed: 1, reg_sim_swapped: %d reg_sim_swapped_target_as_id: %d", reg_sim_swapped, reg_sim_swapped_target_as_id);

    if(reg_sim_swapped)
    {
      /* Swap NV and corresponding cache of current sub with target sub */
      reg_handle_sim_swap(reg_sim_swapped_target_as_id);

      /* SIM soft swap triggered. It shouldn't be treated as new SIM now */
      reg_sim_changed = FALSE;
    }
  }
#endif
...
#endif 
/*Setting default hplmn tag to be TRUE. IF value not present in efs, then perform fast first hplmn search optimization*/
#ifdef FEATURE_NAS_REL12_SPEC_CHANGE
  ...
  */
  switch (req_service_domain)
  {
    ...
    case SYS_SRV_DOMAIN_CS_VOLTE:
      if (reg_sim_state_get() == SYS_SIM_STATE_CS_INVALID)
      {
        allowed_service_domain = SYS_SRV_DOMAIN_VOLTE;
      }
      else if (reg_sim_state_get() == SYS_SIM_STATE_PS_INVALID)
      {
        allowed_service_domain = SYS_SRV_DOMAIN_CS_ONLY;
    ...
    case SYS_SRV_DOMAIN_VOLTE:
      if ((reg_sim_state_get() == SYS_SIM_STATE_PS_INVALID) ||
...
)
{
  ...
  */
  if ( status )
  {
    imsi_p->length = imsi[0];
    MSG_HIGH_DS_1(REG_SUB, "=REG= IMSI LENGTH = %d", imsi_p->length);
...
)
{
  ...
  {
    MSG_ERROR_DS_1(REG_LOCAL_SUB, "=REG= reg_sim_per_subs_read_hplmn- Invalid SUB ID: %d", as_id+1);
    return hplmn;
  }
...
)
{
  ...
  boolean                         status = FALSE;
  mmgsdi_len_type                 file_size; 
  mmgsdi_len_type                 length;
  mmgsdi_len_type                 i, j, k;
  ...
  */
  ...
  hplmn_w_act_list = (reg_sim_plmn_w_act_list_s_type *)modem_mem_calloc(1, sizeof(reg_sim_plmn_w_act_list_s_type), MODEM_MEM_CLIENT_NAS);

   reg_check_for_null_ptr((void*)hplmn_w_act_list );
   if ( hplmn_w_act_list != NULL ) 
   {
    /*
...
)
{
  ...
  */
  if ( !status )
  {
    ...
    if(reg_sim_swapped)
    {
      /* SIM swap triggered and NV swap done by REG task. If it is pending at MM, 
         then sim_swap_event_handled_by_only_one_task flag gets set to TRUE. Once MM done 
         with sim swap it sets sim_swap_event_handled_by_only_one_task flag to FALSE. 
         During handling of SIM swap, MM task will swap PS LOCI data */
      NAS_ENTER_CRIT_SECT(mm_sim_swap_crit_sec);
      if(is_sim_swap_event_handled_by_only_one_task((sys_modem_as_id_e_type)reg_as_id))
      {
        /* MM task did not process SIM swap event yet hence did not swap NV PS location yet.
                  Read PS location from target SUB NV */

        MSG_HIGH_DS_0(REG_SUB, "=REG= SIM swap event handling is in progress at MM. Reading PS LOCI from other SUB NV.");

        /* Change context to target SUB */
        reg_set_as_id(reg_sim_swapped_target_as_id);

        /* Read PS LOCI from target SUB NV*/
      ...
      {
        /* MM task already processed SIM swap. Hence PS location already swapped by MM. */
        status = reg_nv_read_psloci(psloci);
      }
      NAS_EXIT_CRIT_SECT(mm_sim_swap_crit_sec);
    }
  ...
  */
  if ( !status )
  {
    ...
    if(reg_sim_swapped)
    {
      /* SIM swap triggered and NV swap done by REG task. If it is pending at MM, 
         then sim_swap_event_handled_by_only_one_task flag gets set to TRUE. Once MM done 
         with sim swap event it sets sim_swap_event_handled_by_only_one_task flag to FALSE. 
         During handling of SIM swap event, EMM task will swap EPS LOCI data */
      NAS_ENTER_CRIT_SECT(mm_sim_swap_crit_sec);
      if(is_sim_swap_event_handled_by_only_one_task((sys_modem_as_id_e_type)reg_as_id))
      {
        /* MM task did not process SIM swap yet hence did not swap NV EPS location yet.
           Read EPS location from target SUB NV */

        MSG_HIGH_DS_0(REG_SUB, "=REG= SIM swap event handling is in progress at MM. Reading EPS LOCI from other SUB NV.");

        /* Change context to target SUB */
        reg_set_as_id(reg_sim_swapped_target_as_id);
	  
        /* Read EPS LOCI from target SUB NV*/
      ...
      {
        /* MM task already processed SIM swap. Hence EPS location already swapped by EMM. */
        status = reg_nv_read_epsloci(epsloci);
      }
      NAS_EXIT_CRIT_SECT(mm_sim_swap_crit_sec);
    }
...
)
{
  ...
  */
  if((*operator_pplmn_length + *user_pplmn_length) > 0)
  {
    ...
    preferred_plmn_list_p = (reg_sim_plmn_w_act_s_type*)modem_mem_calloc(1, (unsigned int)file_size, MODEM_MEM_CLIENT_NAS);
    ...
    */
    ...
    else
    {
      *preferred_plmn_length = (file_size/REG_SIM_PLMN_LENGTH);
      ...
      preferred_plmn_list_p = (reg_sim_plmn_w_act_s_type*)modem_mem_calloc(1, (unsigned int)((*preferred_plmn_length) * REG_SIM_PLMN_W_ACT_LENGTH),
...
)
{
  ...
  */
  ...
  if (IS_NOT_VALID_SUB_ID(as_id))
  {
    MSG_ERROR_DS_1(REG_LOCAL_SUB, "=REG= reg_sim_read_fplmn_list - Invalid SUB ID: %d", as_id+1);
    return FALSE;
  }
  ...
  */
  ...
  MSG_HIGH_DS_1(REG_LOCAL_SUB,"=REG= FPLMN list length = %d", fplmn_list.length);

  if (
#ifdef FEATURE_DUAL_SIM
     (reg_nv_gcf_flag_get() == FALSE) &&
     ((reg_sim_card_mode_sim[as_id] == MMGSDI_APP_SIM) || (reg_sim_card_mode_sim[as_id] == MMGSDI_APP_USIM))
#else
     (reg_nv_gcf_flag_get() == FALSE) &&
     ...
     */
...
#endif
     {
        ...
        if (reg_sim_dos_mitigation_fplmn_list_sim[as_id].plmn_info[i].is_forbidden)
        {
           ...
           /*Do not increment list size if overwriting an entry in a full list. */
           if (fplmn_list_p->length < REG_SIM_PLMN_LIST_LENGTH)
           {
              fplmn_list_p->length++;
           }
        }
...
#ifdef FEATURE_DUAL_SIM
  if(reg_nv_is_ehplmn_forbid_enabled(as_id) == TRUE)
#else
  if(reg_nv_is_ehplmn_forbid_enabled() == TRUE)
#endif
  {
	  reg_mode_remove_ehplmn_temp_forbidden
	  (
	  	rplmn
#ifdef FEATURE_DUAL_SIM
		,as_id
...
)
{
  ...
  boolean int_voice_roaming_disabled = TRUE; 
  boolean domestic_voice_roaming_disabled = TRUE; 
  ...
  */
  ...
  /*Check if PLMN is allowed for INTERNATIONAL VOICE ROAMING*/
    sys_plmn_get_mcc_mnc
    (
      plmn,
      &plmn_id_is_undefined,
...
if(reg_sim_check_mcc_same_country(plmn) == FALSE &&
   reg_nv_read_voice_roaming() == FALSE)
  {
#ifdef FEATURE_EQUIVALENT_HPLMN
    ...
    for(i =0; i < reg_sim_ehplmn_list.length; i++)
    {
      ...
      {
        int_voice_roaming_disabled = FALSE;
        break;
      }
    }    
    ...
    NAS_EXIT_CRIT_SECT(reg_hplmn_crit_sect);
#endif
    if(int_voice_roaming_disabled)
    {
      if(mnc_includes_pcs_digit)
      {
        MSG_HIGH_DS_2(REG_SUB, "=REG= International Voice roaming disabled, CS+PS not allowed on PLMN %03lu-%03lu", mcc,mnc);
      }
      else
      {
        MSG_HIGH_DS_2(REG_SUB, "=REG= International Voice roaming disabled, CS+PS not allowed on PLMN %03lu-%02lu", mcc,mnc);
      }
      return TRUE;
    }
  }
     /*Check if PLMN is allowed for DOMESTIC ROAMING*/
if(reg_sim_check_mcc_same_country(plmn) == TRUE &&
   reg_nv_read_domestic_voice_roaming() == FALSE)
  {
#ifdef FEATURE_EQUIVALENT_HPLMN
    NAS_ENTER_CRIT_SECT(reg_ehplmn_crit_sect);
    for(i =0; i < reg_sim_ehplmn_list.length; i++)
    {
      ...
      {
        domestic_voice_roaming_disabled = FALSE;
        break;
      }
...
#else
    NAS_ENTER_CRIT_SECT(reg_hplmn_crit_sect);
    if(sys_plmn_match(reg_sim_hplmn,plmn))
    {
      domestic_voice_roaming_disabled = FALSE;
    }
    NAS_EXIT_CRIT_SECT(reg_hplmn_crit_sect);
#endif
    if(domestic_voice_roaming_disabled)
    {
      if(mnc_includes_pcs_digit)
      {
        MSG_HIGH_DS_2(REG_SUB, "=REG= Domestic Voice roaming disabled, CS+PS not allowed on PLMN %03lu-%03lu", mcc,mnc);
      }
      else
      {
        MSG_HIGH_DS_2(REG_SUB, "=REG= Domestic Voice roaming disabled, CS+PS not allowed on PLMN %03lu-%02lu", mcc,mnc);
      }
      return TRUE;
    }
  }
  if(reg_mode_is_ehplmn_forbidden(plmn) == TRUE)
  {
	  return TRUE;
  }
...
FUNCTION NAME
  ...
  reg_sim_domestic_voice_forbidden

==============================================================================*/
boolean reg_sim_domestic_voice_forbidden
(
  sys_plmn_id_s_type      plmn
)
{
  boolean                  status = FALSE;
 #ifdef FEATURE_DUAL_SIM
  MSG_ERROR_DS_0(REG_SUB, "=REG= DUAL SIM: Wrong API called - reg_sim_domestic_voice_forbidden() ");
 #else
  boolean                  plmn_id_is_undefined = FALSE;
  boolean                  mnc_includes_pcs_digit = FALSE;
  uint32                   mcc = 0;
  ...
  MSG_HIGH_DS_0(REG_SUB, "=REG= reg_sim_domestic_voice_forbidden called");
...
)
{
  ...
  {
    MSG_ERROR_DS_1(message_as_id, "=REG= reg_sim_per_subs_plmn_forbidden - Invalid SUB ID: %d", sub_id+1);
    return FALSE;
  }
  ...
  */
  ...
  if(reg_sim_check_mcc_same_country(plmn) == FALSE &&
     reg_nv_read_voice_roaming(sub_id) == FALSE)
  {
#ifdef FEATURE_EQUIVALENT_HPLMN
    NAS_ENTER_CRIT_SECT(reg_ehplmn_crit_sect);
    ...
    if(int_voice_roaming_disabled)
    {
      if(mnc_includes_pcs_digit)
      {
        MSG_HIGH_DS_2(message_as_id, "=REG= International Voice roaming disabled, CS+PS not allowed on PLMN %03lu-%03lu", mcc,mnc);
      }
      else
      {
        MSG_HIGH_DS_2(message_as_id, "=REG= International Voice roaming disabled, CS+PS not allowed on PLMN %03lu-%02lu", mcc,mnc);
      }
      return TRUE;
    }
  }
...
if(reg_sim_check_mcc_same_country(plmn) == TRUE &&
     reg_nv_read_domestic_voice_roaming(sub_id) == FALSE)
  {
#ifdef FEATURE_EQUIVALENT_HPLMN
    NAS_ENTER_CRIT_SECT(reg_ehplmn_crit_sect);
    for(i =0; i < reg_sim_ehplmn_list_sim[sub_id].length; i++)
    {
      if(sys_plmn_match(reg_sim_ehplmn_list_sim[sub_id].plmn[i],plmn))
      {
        domestic_voice_roaming_disabled = FALSE;
        break;
      }
...
#else
    NAS_ENTER_CRIT_SECT(reg_hplmn_crit_sect);
     if(sys_plmn_match(reg_sim_hplmn_sim[sub_id],plmn))
      {
        domestic_voice_roaming_disabled = FALSE;
    }
    NAS_EXIT_CRIT_SECT(reg_hplmn_crit_sect);
    ...
    if(domestic_voice_roaming_disabled)
    {
      if(mnc_includes_pcs_digit)
      {
        MSG_HIGH_DS_2(message_as_id, "=REG= Domestic Voice roaming disabled, CS+PS not allowed on PLMN %03lu-%03lu", mcc,mnc);
      }
      else
      {
        MSG_HIGH_DS_2(message_as_id, "=REG= Domestic Voice roaming disabled, CS+PS not allowed on PLMN %03lu-%02lu", mcc,mnc);
      }
      return TRUE;
    }
  }
  ...
  */
  ...
  if(reg_mode_is_ehplmn_forbidden(plmn,sub_id) == TRUE)
  {
	  return TRUE;
  }
  ...
  if ( status )
  {
    ...
    if(status)
    {
      if(mnc_includes_pcs_digit)
      {
        MSG_HIGH_DS_2(message_as_id,"=REG= PLMN %03lu-%03lu is Forbidden", mcc, mnc);
      }
      else
      {
       MSG_HIGH_DS_2(message_as_id,"=REG= PLMN %03lu- %02lu is Forbidden", mcc, mnc);
      }
    }
  }
  ...
  */
     ...
     reg_nv_read_voice_roaming(sub_id) == FALSE)
  {
    ...
    if(int_voice_roaming_disabled)
    {
      if(mnc_includes_pcs_digit)
      {
        MSG_HIGH_DS_2(sub_id+1, "=REG= International Voice roaming disabled, CS+PS not allowed on PLMN %03lu-%03lu", mcc,mnc);
      }
      else
      {
        MSG_HIGH_DS_2(sub_id+1, "=REG= International Voice roaming disabled, CS+PS not allowed on PLMN %03lu-%02lu", mcc,mnc);
      }
      return TRUE;
    }
  }
...
FUNCTION NAME
  ...
  reg_sim_per_subs_domestic_voice_forbidden

==============================================================================*/
boolean reg_sim_per_subs_domestic_voice_forbidden
(
  sys_plmn_id_s_type      plmn,
  sys_modem_as_id_e_type sub_id
)
{
  ...
  if (IS_NOT_VALID_SUB_ID(sub_id))
  {
    MSG_ERROR_DS_1(REG_SUB, "=REG= reg_sim_per_subs_domestic_voice_forbidden - Invalid SUB ID: %d", sub_id+1);
    return FALSE;
  }
  MSG_HIGH_DS_0(REG_SUB, "=REG=  XDEBUG reg_sim_per_subs_domestic_voice_forbidden called");
  /*
  ** Verify that the PLMN is defined before checking to see if it is
  ** included in the Forbidden PLMN List.
  */
     ...
     reg_nv_read_domestic_voice_roaming(sub_id) == FALSE)
  {
    ...
    if(domestic_voice_roaming_disabled)
    {
      if(mnc_includes_pcs_digit)
      {
        MSG_HIGH_DS_2(sub_id+1, "=REG= Domestic Voice roaming disabled, CS+PS not allowed on PLMN %03lu-%03lu", mcc,mnc);
      }
      else
      {
        MSG_HIGH_DS_2(sub_id+1, "=REG= Domestic Voice roaming disabled, CS+PS not allowed on PLMN %03lu-%02lu", mcc,mnc);
      }
      return TRUE;
    }
  }
...
FUNCTION NAME
  ...
  reg_sim_adjust_eplmn_list_index

==============================================================================*/
void reg_sim_adjust_backoff_list_eplmn_index
(
  uint8 index,
  sys_modem_as_id_e_type as_id
)
{
  ...
  backoff_fplmn_list_ptr = &reg_sim_backoff_fplmn_list;
#endif
  for (j = 0; j < backoff_fplmn_list_ptr->length; j++)
  {
     if((backoff_fplmn_list_ptr->plmn_timer_list[j].eplmn_index > index) &&
        (backoff_fplmn_list_ptr->plmn_timer_list[j].eplmn_index != REG_SIM_BACKOFF_FPLMN_LIST_LENGTH))
     {
       backoff_fplmn_list_ptr->plmn_timer_list[j].eplmn_index -= 1;
     }
  }
}
...
void reg_sim_handle_backoff_fplmn_search_timer_expiry()
{
  ...
  while(i < reg_sim_backoff_fplmn_list.length)
  {
    if(reg_sim_backoff_fplmn_list.plmn_timer_list[i].timer_count == 0)
    {
      index = i;
      /*
      ** Adjust the EPLMN list of all the PLMNs which has index more
      ** than current deleted PLMN as move all the PLMNs up by one
      */
      reg_sim_adjust_backoff_list_eplmn_index(i,(sys_modem_as_id_e_type)reg_sub_id);
  ...
  reg_sim_print_backoff_fplmn_list((sys_modem_as_id_e_type)reg_sub_id);
...
FUNCTION NAME
  ...
  reg_sim_clear_backoff_fplmn_list_with_cause

==============================================================================*/
void reg_sim_clear_backoff_fplmn_list_with_cause
(
  sys_block_plmn_cause_e_type cause
)
{
  uint8 index = 0; 
  MSG_HIGH_DS_1(REG_SUB, "=REG= Clearing backoff plmn list with cause %d", cause);
  while (index < reg_sim_backoff_fplmn_list.length)
  {
    if(reg_sim_backoff_fplmn_list.plmn_timer_list[index].cause == cause)
    {
      reg_sim_remove_plmn_from_backoff_fplmn_list(
            reg_sim_backoff_fplmn_list.plmn_timer_list[index].plmn,
            cause,
            (sys_modem_as_id_e_type)reg_sub_id);
    }
    else
...
)
{
  ...
  for(index = 0; index < backoff_fplmn_list_ptr->length; index++)
  {
    if(sys_plmn_match(backoff_fplmn_list_ptr->plmn_timer_list[index].plmn,plmn))
    {
      if((cause == SYS_BLOCK_PLMN_CAUSE_NONE) ||
         (backoff_fplmn_list_ptr->plmn_timer_list[index].cause == cause))
      {
        found = TRUE;
        break;
      }
  ...
  {
    uint8 j;
    uint8 eplmn_index = backoff_fplmn_list_ptr->plmn_timer_list[index].eplmn_index;
    /*
    ** Mark all the PLMNs to be deleted by setting the EPLMN index to MAX length value
    */
    for(j = 0; j < backoff_fplmn_list_ptr->length; j++)
    {
      if(backoff_fplmn_list_ptr->plmn_timer_list[j].eplmn_index == eplmn_index)
      {
        backoff_fplmn_list_ptr->plmn_timer_list[j].eplmn_index = REG_SIM_BACKOFF_FPLMN_LIST_LENGTH;
      }
    }
    ...
    for(j = 0; j < backoff_fplmn_list_ptr->length; )
    {
      /*
      ** Delete the marked PLMNs, adjust the length and move the remaining PLMNs up.
      ** Move to next entry if PLMNs are not rearranged
      */
      if(backoff_fplmn_list_ptr->plmn_timer_list[j].eplmn_index == REG_SIM_BACKOFF_FPLMN_LIST_LENGTH)
      {
        reg_sim_adjust_backoff_list_eplmn_index(j,as_id);
        backoff_fplmn_list_ptr->length--;
        index = j;
        for (; index < backoff_fplmn_list_ptr->length; index++)
        {
          backoff_fplmn_list_ptr->plmn_timer_list[index] = backoff_fplmn_list_ptr->plmn_timer_list[index + 1];
        }
      }
      else
      {
  ...
  reg_sim_print_backoff_fplmn_list(as_id);
  return found;
}
...
)
{
  ...
  if ( sys_plmn_id_is_undefined(plmn) )
  {
    MSG_ERROR_DS_0(as_id+1, "=REG= Add undefined PLMN to BACKOFF PLMN list");
    return list_updated;
  }

  MSG_HIGH_DS_0(as_id+1, "=REG= Adding PLMN to temp FPLMN list rem time after curr backoff timer expiry %d (ms)");
  /*
  ** Check to see if the PLMN is already contained in the
  ** backoff Forbidden PLMN List.
  */
  if ( !reg_sim_plmn_backoff_forbidden(plmn,as_id) )
  {
    ...
    */
    if ( backoff_fplmn_list_ptr->length < REG_SIM_BACKOFF_FPLMN_LIST_LENGTH )
    {
      backoff_fplmn_list_ptr->length++;
    }
    /*
    ...
    */
    ...
    {
      backoff_fplmn_list_ptr->length = REG_SIM_BACKOFF_FPLMN_LIST_LENGTH;

      for (i = 0; i < (REG_SIM_BACKOFF_FPLMN_LIST_LENGTH - 1); i++)
      {
        backoff_fplmn_list_ptr->plmn_timer_list[i]= backoff_fplmn_list_ptr->plmn_timer_list[i + 1];
      }
    }
    ...
    ** Copy the PLMN into the empty position.
    */
    backoff_fplmn_list_ptr->plmn_timer_list[backoff_fplmn_list_ptr->length - 1].plmn = plmn;
    backoff_fplmn_list_ptr->plmn_timer_list[backoff_fplmn_list_ptr->length - 1].cause = cause;
    backoff_fplmn_list_ptr->plmn_timer_list[backoff_fplmn_list_ptr->length - 1].timer_count = timer;
    backoff_fplmn_list_ptr->plmn_timer_list[backoff_fplmn_list_ptr->length - 1].eplmn_index = eplmn_index;
    /* leave the critical section here */
    rex_leave_crit_sect(&reg_backoff_fplmn_crit_sect);
    list_updated = TRUE;
  }
...
FUNCTION NAME
  ...
  reg_sim_get_backoff_fplmn_info

==============================================================================*/
boolean reg_sim_get_backoff_fplmn_info
(
...
)
{
  ...
  backoff_fplmn_list_ptr = &reg_sim_backoff_fplmn_list;
#endif
  for(j=backoff_fplmn_list_ptr->length;(j>0 && found == FALSE);j--)
  {
    if(sys_plmn_match(backoff_fplmn_list_ptr->plmn_timer_list[j-1].plmn, plmn_timer_info_ptr->plmn))
    {
      found = TRUE;
      *plmn_timer_info_ptr = backoff_fplmn_list_ptr->plmn_timer_list[j-1];
    }
  }
  return found;
}
...
)
{
  ...
  {
    MSG_HIGH_DS_1(as_id+1, "=REG= PLMN added to temp FPLMN list for Max registration failure backoff time %d (ms)",
                    timer);
  }
  else if(cause == SYS_BLOCK_PLMN_CAUSE_VOICE_NOT_AVAILABLE)
  {
    MSG_HIGH_DS_1(as_id+1, "=REG= PLMN added to temp FPLMN list as voice is not available backoff time %d (ms)",
                    timer);
  }
  else if(cause == SYS_BLOCK_PLMN_CAUSE_IMS_REGISTRATION_FAILURE)
  {
    MSG_HIGH_DS_1(as_id+1, "=REG= PLMN added to temp FPLMN list for IMS REG failure backoff time %d (ms)",
                    timer);
  }
  else if(cause == SYS_BLOCK_PLMN_CAUSE_PRIORITY )
  {
    MSG_HIGH_DS_1(as_id+1, "=REG= PLMN added to temp FPLMN list as DS blocking priority backoff time %d (ms)",
                    timer);
  }
  else if(cause == SYS_BLOCK_PLMN_CAUSE_EMC_IN_LSM )
  {
    MSG_HIGH_DS_1(as_id+1, "=REG= PLMN added to temp FPLMN list as EMC call block  time %d (ms)",
                    timer);
  }
  else 
  {
    MSG_HIGH_DS_1(as_id+1, "=REG= PLMN added to temp FPLMN list for other causes backoff time %d (ms)",
                    timer);
  }

  /* 
  ** 1.  If DS blocks with cause "SYS_BLOCK_PLMN_CAUSE_PRIORITY" and timer value is -1, if PLMN exists 
  **    in backoff list already with cause "SYS_BLOCK_PLMN_CAUSE_MAX_REGISTRATION_FAILURE" (NAS blocked the PLMN), 
  **    NAS will delete that entry and block with DS cause "SYS_BLOCK_PLMN_CAUSE_PRIORITY" permanently. 
  ** 2.  If DS or any module blocks already blocked PLMN again with same cause and the timer value is more than remaining time 
  **    for that PLMN in backoff list, then NAS will update the timer count to new value. If the timer value is less than remaining time, 
  **    then block request will be ignored.
  */
  for(index = 0; index < backoff_fplmn_list_ptr->length; index++)
  {
    if(sys_plmn_match(backoff_fplmn_list_ptr->plmn_timer_list[index].plmn,plmn))
    {
      if(cause == SYS_BLOCK_PLMN_CAUSE_PRIORITY  && timer == REG_TIMERS_MAX_TIMER_VALUE &&
         (backoff_fplmn_list_ptr->plmn_timer_list[index].cause == SYS_BLOCK_PLMN_CAUSE_MAX_REGISTRATION_FAILURE))
      {
         MSG_HIGH_DS_0(as_id,"=REG= removing the PLMN to give DS blocking preference");
        (void)reg_sim_remove_plmn_from_backoff_fplmn_list(plmn,
                backoff_fplmn_list_ptr->plmn_timer_list[index].cause,as_id);
        break;
      }
      else if(backoff_fplmn_list_ptr->plmn_timer_list[index].cause == cause)
      {
        if (backoff_fplmn_list_ptr->plmn_timer_list[index].timer_count != REG_TIMERS_MAX_TIMER_VALUE)
        {
          if(timer == REG_TIMERS_MAX_TIMER_VALUE)
          {
            backoff_fplmn_list_ptr->plmn_timer_list[index].timer_count = REG_TIMERS_MAX_TIMER_VALUE;
          }
          else
          {
            rex_timer_cnt_type curr_time = 
              reg_timers_get_rem_backoff_fplmn_search_timer_value((reg_as_id_e_type)as_id);
            if(timer > (curr_time + backoff_fplmn_list_ptr->plmn_timer_list[index].timer_count))
            {
              backoff_fplmn_list_ptr->plmn_timer_list[index].timer_count = (timer - curr_time);
            }
          }
        }
        found = TRUE;
        MSG_HIGH_DS_0(as_id+1, "=REG= PLMN already present in the backoff list");
        break;
      }
    }
  }
  ...
  */
  if ( found == FALSE )
  {
    if ( sys_plmn_id_is_undefined(plmn) )
    {
      MSG_HIGH_DS_0(as_id+1, "=REG= Adding wild card entry to BACKOFF PLMN list");
    }
    /* enter the critical section here */ 
    rex_enter_crit_sect(&reg_backoff_fplmn_crit_sect);
    /*
    ...
    */
    backoff_fplmn_list_ptr->plmn_timer_list[backoff_fplmn_list_ptr->length - 1].cause = cause;
    backoff_fplmn_list_ptr->plmn_timer_list[backoff_fplmn_list_ptr->length - 1].eplmn_index = 
                                                                (uint8)(backoff_fplmn_list_ptr->length - 1);
    /*
    ...
    */
    if(timer != 0 && timer != REG_TIMERS_MAX_TIMER_VALUE)
    {
      reg_timers_start_backoff_fplmn_search_period_timer(
              backoff_fplmn_list_ptr,
              timer, (reg_as_id_e_type)as_id);
    }
    else
    {
      backoff_fplmn_list_ptr->plmn_timer_list[backoff_fplmn_list_ptr->length - 1].timer_count = 
               REG_TIMERS_MAX_TIMER_VALUE;
    }
    list_updated = TRUE;
    /* leave the critical section here */
...
)
{
  ...
  backoff_fplmn_list_ptr = &reg_sim_backoff_fplmn_list;
#endif
  plmn_timer_info_ptr->timer_count = 0;
  length = (uint8)MIN(eplmn_list_p->length, SYS_PLMN_LIST_MAX_LENGTH);
  rex_enter_crit_sect(&reg_backoff_fplmn_crit_sect);

  backoff_fplmn_list_ptr->length = MIN(backoff_fplmn_list_ptr->length,
  ...
  for(i = 0; i < length; i++)
  {
    for(j = 0; j < backoff_fplmn_list_ptr->length; j++)
    {
      if(sys_plmn_match(backoff_fplmn_list_ptr->plmn_timer_list[j].plmn, eplmn_list_p->plmn[i]))
      {
        if((plmn_timer_info_ptr->timer_count < backoff_fplmn_list_ptr->plmn_timer_list[j].timer_count) ||
           (eplmn_backoff_forbidden == FALSE))
        {
          plmn_timer_info_ptr->timer_count = backoff_fplmn_list_ptr->plmn_timer_list[j].timer_count;
          plmn_timer_info_ptr->cause = backoff_fplmn_list_ptr->plmn_timer_list[j].cause;
          plmn_timer_info_ptr->eplmn_index = backoff_fplmn_list_ptr->plmn_timer_list[j].eplmn_index;
        }
...
#endif
  {
	  reg_mode_remove_ehplmn_temp_gprs_forbidden
	  (
	  	rplmn
#ifdef FEATURE_DUAL_SIM
		,as_id
     ...
     */
   ...
   #endif
     {
       MSG_ERROR_DS_0(REG_LOCAL_SUB, "=REG= Add HPLMN to GPRS FPLMN list");
           ...
           if(reg_nv_is_ehplmn_forbid_enabled(as_id) == TRUE)
           {
            reg_mode_add_ehplmn_temp_gprs_forbidden(plmn,as_id);
           }
       return list_updated;
     }
  }
  ...
  */
  ...
  if (!dos_mitigation_req)
  {
     ...
     */
     ...
     {
       MSG_ERROR_DS_0(REG_LOCAL_SUB, "=REG= Add HPLMN to GPRS FPLMN list");
           reg_mode_add_ehplmn_temp_gprs_forbidden(plmn);
       return list_updated;
     }
  }
...
)
{
  ...
  */
  if ( sys_plmn_id_is_undefined(plmn) )
  {
    MSG_ERROR_DS_0(REG_LOCAL_SUB, "=REG= Remove undefined PLMN from DoS FPLMN list");
    return result;
  }
  ...
  */
  ...
  if (reg_sim_find_plmn_in_dos_mitigation_counter_list(plmn, &reg_sim_dos_mitigation_fplmn_list_sim[as_id], &position) )
  {
     ...
     */
     for (; (position < reg_sim_dos_mitigation_fplmn_list_sim[as_id].length-1) && (position < REG_SIM_DOS_MITIGATION_PLMN_LIST_LENGTH-1); position++)
     {
	   ...
	   reg_sim_dos_mitigation_fplmn_list_sim[as_id].plmn_info[position].counter = reg_sim_dos_mitigation_fplmn_list_sim[as_id].plmn_info[position+1].counter;
	   reg_sim_dos_mitigation_fplmn_list_sim[as_id].plmn_info[position].is_forbidden = reg_sim_dos_mitigation_fplmn_list_sim[as_id].plmn_info[position+1].is_forbidden;
     }
     reg_sim_dos_mitigation_fplmn_list_sim[as_id].length = reg_sim_dos_mitigation_fplmn_list_sim[as_id].length - 1;
     result = TRUE;
  }
#else
  if (reg_sim_find_plmn_in_dos_mitigation_counter_list(plmn, &reg_sim_dos_mitigation_fplmn_list, &position) )
  {
     ...
     */
     ...
     reg_sim_dos_mitigation_fplmn_list.length = reg_sim_dos_mitigation_fplmn_list.length-1;
     result = TRUE;
  }
#endif
  MSG_HIGH_DS_4(REG_LOCAL_SUB, "=REG=  reg_sim_remove_plmn_from_dos_mitigation_fplmn_list():PLMN %x %x %x, result %d",
  ...
  */
  if ( !reg_sim_find_plmn_in_dos_mitigation_counter_list(plmn, dos_fplmn_list, &position) )
  {
    ...
    */
    ...
    MSG_HIGH_DS_4(REG_LOCAL_SUB, "=REG= Adding PLMN %x %x %x to temp DoS FPLMN, length = %d ",
  ...
  else
  {
    if (dos_fplmn_list->plmn_info[position].counter < mm_emm_max_attempt_counter_dos_mitigation)
    {
      ...
      MSG_HIGH_DS_4(REG_LOCAL_SUB, "=REG= Incrementing counter for PLMN %x %x %x to temp DoS FPLMN, length = %d ",
                    plmn.identity[0], plmn.identity[1], plmn.identity[2], dos_fplmn_list->length);
      if (dos_fplmn_list->plmn_info[dos_fplmn_list->length - 1].is_forbidden == FALSE)
      {
         dos_fplmn_list->plmn_info[dos_fplmn_list->length - 1].is_forbidden = TRUE;
         list_updated = TRUE;
      }
      ...
      if (dos_fplmn_list->plmn_info[position].counter == mm_emm_max_attempt_counter_dos_mitigation)
      {
         MSG_HIGH_DS_0(REG_LOCAL_SUB, "=REG= Adding temp FPLMN to SIM FPLMN ");
#ifdef FEATURE_DUAL_SIM
...
FUNCTION NAME
  ...
  reg_sim_per_subs_check_mcc_same_country

==============================================================================*/
#ifdef FEATURE_DUAL_SIM
boolean reg_sim_per_subs_check_mcc_same_country
(
  sys_plmn_id_s_type plmn,
  sys_modem_as_id_e_type sub_id
)
...
#ifdef FEATURE_DUAL_SIM
 if(reg_mode_is_ehplmn_gprs_forbidden(plmn,as_id) == TRUE)
#else
 if(reg_mode_is_ehplmn_gprs_forbidden(plmn) == TRUE)
#endif
 {
	 return TRUE;
 }
...
)
{
  ...
  for (i = 0; i < backoff_fplmn_list_ptr->length; i++)
  {
    if ( sys_plmn_match(backoff_fplmn_list_ptr->plmn_timer_list[i].plmn, plmn) ||
         sys_plmn_id_is_undefined(backoff_fplmn_list_ptr->plmn_timer_list[i].plmn))
    {
      found = TRUE;
      if (backoff_fplmn_list_ptr->plmn_timer_list[i].timer_count != REG_TIMERS_MAX_TIMER_VALUE)
      {
        rem_time_value = backoff_fplmn_list_ptr->plmn_timer_list[i].timer_count+curr_timer_count;
      }
      else
      {
        rem_time_value = backoff_fplmn_list_ptr->plmn_timer_list[i].timer_count;
      }
      sys_plmn_get_mcc_mnc
      (
        backoff_fplmn_list_ptr->plmn_timer_list[i].plmn,
      ...
      if(mnc_includes_pcs_digit)
      {
        if(backoff_fplmn_list_ptr->plmn_timer_list[i].cause == SYS_BLOCK_PLMN_CAUSE_MAX_REGISTRATION_FAILURE)
        {
          MSG_HIGH_DS_3(as_id+1, "=REG= PLMN (%03lu-%03lu) is backoff forbidden added for Max registration failure, timer to unblock %u(ms)", 
                 mcc, mnc,rem_time_value);
        }
        else if(backoff_fplmn_list_ptr->plmn_timer_list[i].cause == SYS_BLOCK_PLMN_CAUSE_VOICE_NOT_AVAILABLE)
        {
          MSG_HIGH_DS_3(as_id+1, "=REG= PLMN (%03lu-%03lu) is backoff forbidden added as voice is not available, timer to unblock %u(ms)", 
                 mcc, mnc,rem_time_value);
        }
        else if(backoff_fplmn_list_ptr->plmn_timer_list[i].cause == SYS_BLOCK_PLMN_CAUSE_IMS_REGISTRATION_FAILURE)
        {
          MSG_HIGH_DS_3(as_id+1, "=REG= PLMN (%03lu-%03lu) is backoff forbidden added for IMS REG failure, timer to unblock %u(ms)", 
                 mcc, mnc,rem_time_value);
        }
        else if(backoff_fplmn_list_ptr->plmn_timer_list[i].cause == SYS_BLOCK_PLMN_CAUSE_PRIORITY )
        {
          MSG_HIGH_DS_3(as_id+1, "=REG= PLMN (%03lu-%03lu) is backoff forbidden added DS blocking Priority, timer to unblock %u(ms)", 
                 mcc, mnc,rem_time_value);
        }
		else if(backoff_fplmn_list_ptr->plmn_timer_list[i].cause == SYS_BLOCK_PLMN_CAUSE_EMC_IN_LSM )
        {
          MSG_HIGH_DS_3(as_id+1, "=REG= PLMN (%03lu-%03lu) is backoff forbidden added for EMC call block, timer to unblock %u(ms)", 
                 mcc, mnc,rem_time_value);
        }
        else
        {
          MSG_HIGH_DS_3(as_id+1, "=REG= PLMN (%03lu-%03lu) is backoff forbidden added for other causes, timer to unblock %u(ms)",
                  mcc, mnc,rem_time_value);
        }
      }
      else
      {
        if(backoff_fplmn_list_ptr->plmn_timer_list[i].cause == SYS_BLOCK_PLMN_CAUSE_MAX_REGISTRATION_FAILURE)
        {
          MSG_HIGH_DS_3(as_id+1, "=REG= PLMN (%03lu-%02lu) is backoff forbidden added for Max registration failure, timer to unblock %u(ms)", 
                        mcc, mnc, rem_time_value);
        }
        else if(backoff_fplmn_list_ptr->plmn_timer_list[i].cause == SYS_BLOCK_PLMN_CAUSE_VOICE_NOT_AVAILABLE)
        {
          MSG_HIGH_DS_3(as_id+1, "=REG= PLMN (%03lu-%02lu) is backoff forbidden added as voice is not available, timer to unblock %u(ms)", 
                        mcc, mnc, rem_time_value);
        }
        else if(backoff_fplmn_list_ptr->plmn_timer_list[i].cause == SYS_BLOCK_PLMN_CAUSE_IMS_REGISTRATION_FAILURE)
        {
          MSG_HIGH_DS_3(as_id+1, "=REG= PLMN (%03lu-%02lu) is backoff forbidden added for IMS REG failure, timer to unblock %u(ms)", 
                        mcc, mnc, rem_time_value);
        }
        else if(backoff_fplmn_list_ptr->plmn_timer_list[i].cause == SYS_BLOCK_PLMN_CAUSE_PRIORITY )
        {
          MSG_HIGH_DS_3(as_id+1, "=REG= PLMN (%03lu-%02lu) is backoff forbidden added DS blocking priority, timer to unblock %u(ms)", 
                 mcc, mnc,rem_time_value);
        }
		else if(backoff_fplmn_list_ptr->plmn_timer_list[i].cause == SYS_BLOCK_PLMN_CAUSE_EMC_IN_LSM )
        {
          MSG_HIGH_DS_3(as_id+1, "=REG= PLMN (%03lu-%02lu) is backoff forbidden added for EMC call block, timer to unblock %u(ms)", 
                 mcc, mnc,rem_time_value);
        }
        else
        {
          MSG_HIGH_DS_3(as_id+1, "=REG= PLMN (%03lu-%02lu) is backoff forbidden added for other causes, timer to unblock %u(ms)", 
                        mcc, mnc, rem_time_value);
        }
      }
      break;
...
)
{
  ...
  else if(reg_sim_card_mode_sim[sub_id] != MMGSDI_APP_UNKNOWN)
  {
    NAS_ENTER_CRIT_SECT(reg_hplmn_crit_sect);
...
void reg_sim_imp_variable_prints()
{
  ...
  (
   ...
   else
   {
     ...
     */
#ifdef FEATURE_DUAL_SIM
    status = reg_sim_read_fplmn_list(&fplmn_list, TRUE, (sys_modem_as_id_e_type)reg_sub_id);
#else
    status = reg_sim_read_fplmn_list(&fplmn_list, TRUE);
#endif
    MSG_HIGH_DS_1(REG_SUB, "=REG= Forbidden PLMN list (length = %d)", fplmn_list.length);
    if((status) && (fplmn_list.length > 0))
    {
#ifdef FEATURE_DUAL_SIM
      reg_log_umts_nas_fplmn_list(&fplmn_list,(sys_modem_as_id_e_type)reg_sub_id);
...
)
{
  ...
  MSG_MED_DS_1(stack_id_nas+1, "=REG= Reading ENS Supported Application Flag - %d", reg_sim_ens_flag_sim[stack_id_nas]);
...
FUNCTION NAME
  ...
  reg_sim_session_ok_to_init

==============================================================================*/
void reg_sim_session_ok_to_init
(
  void
)
{
  ...
  MSG_HIGH_DS_2(REG_SUB, "=REG= Reg received MMGSDI refresh event stage = %d, mode = %d", refresh_stage, refresh_mode);
    ...
    switch(refresh_stage)
    {
      case  MMGSDI_REFRESH_STAGE_WAIT_FOR_OK_TO_INIT:
        reg_send_mmr_refresh_stage_ind( refresh_stage );
        MSG_HIGH_DS_0(REG_SUB, "=REG= Reg received sim refresh ok to INIT");
#ifdef FEATURE_SGLTE
        if(!REG_SUB_IS_SGLTE || (reg_as_id == reg_ps_stack_id))
#endif
          reg_sim_session_ok_to_init();
...
)
{
  ...
  memscpy(reg_sim_refresh_fcn_list, sizeof(reg_sim_refresh_fcn_list) , file_list, reg_sim_num_fcn_files * sizeof(mmgsdi_file_enum_type));
...
boolean reg_sim_read_ehplmn_from_sim(reg_sim_plmn_list_s_type* ehplmn_list) 
{
...
)
{
  ...
  if (IS_NOT_VALID_SUB_ID(as_id))
  {
    MSG_ERROR_DS_1(REG_LOCAL_SUB, "=REG= reg_sim_per_subs_is_ehplmn - Invalid AS ID: %d", as_id+1);
    return FALSE;
  }
...
#endif
  MSG_HIGH_DS_0(sub_id+1, "=REG= Low Priority PLMN list cleared!!");
}
...
{  
  ...
  if ((event->evt == MMGSDI_REFRESH_EVT) && 
       (event->data.refresh.mode == MMGSDI_REFRESH_NAA_FCN ||
        ...
        ||
        event->data.refresh.mode == MMGSDI_REFRESH_RESET
	||
	event->data.refresh.mode == MMGSDI_REFRESH_NAA_APP_RESET
	||
	event->data.refresh.mode == MMGSDI_REFRESH_NAA_INIT
	||
	event->data.refresh.mode == MMGSDI_REFRESH_NAA_INIT_FULL_FCN
	||
	event->data.refresh.mode == MMGSDI_REFRESH_RESET_AUTO
	||
	event->data.refresh.mode == MMGSDI_REFRESH_APP_RESET_AUTO
        )  
      )
  {
    do
...
)
{
...
#if defined FEATURE_DUAL_SIM || defined FEATURE_SGLTE
          reg_state_set_gstk_event_flag_per_subs(TRUE, as_id);
#else
  ...
  else if (event->evt == MMGSDI_CARD_INSERTED_EVT)
  {
    as_id = reg_sim_get_as_id_from_slot(event->data.card_inserted.slot);
    if(as_id != SYS_MODEM_AS_ID_NONE)
    {
#ifdef FEATURE_GSTK
      imsi_switch_value = reg_nv_is_imsi_switch_enabled();
      if(imsi_switch_value)
      {
          MSG_HIGH_DS_1(REG_LOCAL_SUB, "=REG= GSTK idle event will be sent after card reset as IMSI_SWITCH value is %d\n ", imsi_switch_value);
  ...
  else if(event->evt == MMGSDI_SIM_BUSY_EVT)
  {
    ...
    if(as_id != SYS_MODEM_AS_ID_NONE)
    {
      reg_send_mmr_sim_busy_ind(event->data.sim_busy.sim_busy, as_id);;
    }
    else
    {
       MSG_HIGH_DS_0(REG_LOCAL_SUB, "=REG= Ignoring SIM_BUSY_EVENT as no session was opened yet");
    }
  }
#endif
#ifdef FEATURE_NAS_ECALL
  if (event->evt == MMGSDI_PROFILE_DATA_EVT)
  {
    ...
    if(as_id != SYS_MODEM_AS_ID_NONE)
    {
      if (event->data.profile_data.profile.profile_type == MMGSDI_PROFILE_TYPE_EMERGENCY)
      {
        ...
        reg_send_mmr_profile_type_ind(event->data.profile_data.profile.profile_type, as_id);
      }
      else
      {
    ...
    {
      MSG_HIGH_DS_0(REG_LOCAL_SUB, "=REG= Ignoring ECALL_PROFILE_DATA_EVENT as no session was opened yet");
    }
  }
#endif
  else
...
boolean reg_sim_check_home_mcc(sys_mcc_type current_mcc)
{
  ...
  boolean home_mnc_includes_pcs_digit;
     ...
     for(i =0; i < reg_sim_ehplmn_list.length; i++)
     {
	   ...
	   MSG_HIGH_DS_1(REG_SUB, "=REG= ehplmn mcc - %d ", home_mcc);
	   if(sys_mcc_same_country(current_mcc, home_mcc))
	   {
		 result = TRUE;
		 break;
	   }
	 ...
	 MSG_HIGH_DS_1(REG_SUB, "=REG= hplmn mcc - %d ", home_mcc);
	 if(sys_mcc_same_country(current_mcc, home_mcc))
	 {
	   result = TRUE;
	 }
...
FUNCTION NAME
  ...
  reg_sim_per_subs_check_home_mcc

==============================================================================*/

boolean reg_sim_per_subs_check_home_mcc
(
...
)
{
  ...
  (void) as_id;
  return reg_sim_check_home_mcc(current_mcc);
#endif

}


boolean reg_sim_is_home_plmn_to_be_camped( sys_srv_domain_e_type  req_service_domain)
{
  byte lrplmnsi = 0x00;
  uint32 i=0;
  sys_mcc_type pm_mcc;
  ...
  boolean pm_valid_mcc = FALSE;
  
  /*if lrplmnsi and (rplmn is home || pm plmn is home) and ef is TRUE*/


  if(reg_nv_home_plmn_to_be_selcted_in_home_country_get() == TRUE)
  {
    if (reg_sim_read(MMGSDI_USIM_LRPLMNSI,&lrplmnsi,1) == TRUE) 
    {
      MSG_HIGH_DS_1(REG_SUB, "=REG= LRPLMNSI value %d ",lrplmnsi);
      if (lrplmnsi == 0x01) 
      { 
          
          rplmn = reg_sim_read_rplmn(req_service_domain);
          status = policyman_get_current_mcc((sys_modem_as_id_e_type)reg_sub_id, &pm_mcc);
          if(POLICYMAN_SUCCEEDED(status))
          {
			pm_valid_mcc = TRUE;
            MSG_HIGH_DS_1(REG_SUB, "=REG= Policyman reported mcc = %d", pm_mcc);
		  }

          if(pm_valid_mcc && (pm_mcc != 1665 && pm_mcc != 0))
           {
              if(reg_sim_check_home_mcc(pm_mcc) == TRUE)
              {
                MSG_HIGH_DS_0(REG_SUB, "=REG= PM MCC equals HOME MCC ");
                result = TRUE;
              }
           }
		   else if(!sys_plmn_id_is_undefined(rplmn) && 
                reg_sim_check_mcc_same_country(rplmn))
		   {
		     MSG_HIGH_DS_0(REG_SUB, "=REG= RPLMN MCC equals HOME MCC ");
             result = TRUE;
		   }
       }
    }
...
)
{
 ...
 if(IS_NOT_VALID_AS_ID(as_id)) 
 {
   MSG_ERROR_DS_0(REG_SUB,"Incorrect AS ID");	 
   return;
 } 	 
...
void reg_sim_perform_merge_operation(void)
{
  ...
  if ( reg_sim_state_sim[reg_ps_stack_id] !=SYS_SIM_STATE_NOT_AVAILABLE  )
  {
    if((reg_sim_state_sim[reg_ps_stack_id]== SYS_SIM_STATE_AVAILABLE) &&
          (reg_sim_state_sim[reg_cs_stack_id]== SYS_SIM_STATE_CS_INVALID))
    {
      reg_sim_state_sim[reg_ps_stack_id] = SYS_SIM_STATE_CS_INVALID;
    }
    else if((reg_sim_state_sim[reg_ps_stack_id]== SYS_SIM_STATE_PS_INVALID) &&
         (reg_sim_state_sim[reg_cs_stack_id]== SYS_SIM_STATE_CS_INVALID))
    {
      reg_sim_state_sim[reg_ps_stack_id] = SYS_SIM_STATE_CS_PS_INVALID;
    }
  }

  if(reg_sim_state_sim[reg_ps_stack_id]== SYS_SIM_STATE_PS_INVALID)
  {
    reg_mode_copy_network_selection_mode(reg_cs_stack_id, reg_ps_stack_id);
  }
...
void reg_sim_perform_split_operation(void)
{
  ...
  if(reg_sim_state_sim[reg_ps_stack_id]== SYS_SIM_STATE_AVAILABLE) 
  {
    reg_sim_state_sim[reg_cs_stack_id] = SYS_SIM_STATE_AVAILABLE;
  }
  else if(reg_sim_state_sim[reg_ps_stack_id]== SYS_SIM_STATE_CS_PS_INVALID) 
  {
    reg_sim_state_sim[reg_ps_stack_id]= SYS_SIM_STATE_PS_INVALID;
    reg_sim_state_sim[reg_cs_stack_id] = SYS_SIM_STATE_CS_INVALID;
  }
  else if(reg_sim_state_sim[reg_ps_stack_id]== SYS_SIM_STATE_PS_INVALID)
  {
  ...
  else if (reg_sim_state_sim[reg_ps_stack_id]== SYS_SIM_STATE_CS_INVALID)
  {
    reg_sim_state_sim[reg_ps_stack_id] = SYS_SIM_STATE_AVAILABLE;
    reg_sim_state_sim[reg_cs_stack_id] = SYS_SIM_STATE_CS_INVALID;
  }
...
/*=============================================================================
     QSH related APIs
==============================================================================*/
void get_dump_reg_mode_sim_vars (reg_debug_data_s_type* ptr, reg_as_id_e_type as_id)
{
    ASSERT (ptr != NULL);
#if defined FEATURE_DUAL_SIM || defined FEATURE_SGLTE
    ptr->reg_sim_cs_rplmn_debug = reg_sim_cs_rplmn_sim[as_id];
    ptr->reg_sim_ens_flag_debug = reg_sim_ens_flag_sim[as_id];
    ptr->reg_sim_hplmn_debug = reg_sim_hplmn_sim[as_id];
    ptr->reg_sim_is_ahplmn_read_debug = reg_sim_is_ahplmn_read_sim[as_id];
    ptr->reg_sim_is_ens_read_debug = reg_sim_is_ens_read_sim[as_id];
    ptr->reg_sim_max_subscription_debug = reg_sim_max_subscription;
    ptr->reg_sim_rplmn_rat_search_order_debug = 
        (reg_sim_mode_pref_debug_e_type)reg_sim_rplmn_rat_search_order_sim[as_id]; 
    ptr->reg_sim_rplmn_read_debug = reg_sim_rplmn_read_sim[as_id];
    ptr->reg_sim_state_sim_debug = reg_sim_state_sim[as_id];
#else
    ptr->reg_sim_cs_rplmn_debug = reg_sim_cs_rplmn;
    ptr->reg_sim_ens_flag_debug = reg_sim_ens_flag;
    ptr->reg_sim_hplmn_debug = reg_sim_hplmn;
    ptr->reg_sim_is_ahplmn_read_debug = reg_sim_is_ahplmn_read;
    ptr->reg_sim_is_ens_read_debug = reg_sim_is_ens_read;
    ptr->reg_sim_rplmn_rat_search_order_debug = 
        (reg_sim_mode_pref_debug_e_type)reg_sim_rplmn_rat_search_order; 
    ptr->reg_sim_rplmn_read_debug = reg_sim_rplmn_read;
    ptr->reg_sim_state_sim_debug = reg_sim_state;
...
)
{
   ...
   MSG_HIGH_DS_4(REG_LOCAL_SUB, "=REG= PLMN %x %x %x present in temp GPRS DoS FPLMN list = %d",
                  plmn_id.identity[0], plmn_id.identity[1], plmn_id.identity[2], result);
   return result;
}
...
)
{
   ...
   MSG_HIGH_DS_0(REG_LOCAL_SUB, "=REG= reg_sim_cleanup_dos_fplmn_list_on_t3247_exp()"); 
...
)
{
   ...
   MSG_HIGH_DS_0(REG_LOCAL_SUB, "=REG= reg_sim_reset_dos_fplmn_list()");

#ifdef FEATURE_DUAL_SIM
   if(IS_NOT_VALID_AS_ID(as_id))
   {
     MSG_ERROR_DS_0(REG_SUB, "=MM= Invalid AS ID ");
     return ;
   }
   fplmn_list = &reg_sim_gprs_fplmn_list_sim[as_id];
#else
...
)
{
   ...
   if (sys_plmn_id_is_undefined(plmn))
   {
     MSG_HIGH_DS_0(REG_LOCAL_SUB, "=REG= Incorrect PLMN received");
     return;
   }
   ...
   MSG_HIGH_DS_4(REG_LOCAL_SUB, "=REG= reg_sim_reset_plmn_counter_in_fplmn_list(): GPRS FPLMN %x %x %x, status %d",
   ...
   MSG_HIGH_DS_4(REG_LOCAL_SUB, "=REG= reg_sim_reset_plmn_counter_in_fplmn_list(): Temp DoS FPLMN %x %x %x, status %d",
                 plmn.identity[0], plmn.identity[1], plmn.identity[2], status);
...
#ifdef FEATURE_LTE 
/*==============================================================================
FUNCTION NAME:  reg_sim_remove_plmn_with_matching_mcc_from_backoff_fplmn_list

DESCRIPTION
  Function removes PLMN(s) present in backoff FPLMN list added for a particular
  cause if the MCC matches with the MCC of the PLMN passed in the function parameter 
  
RETURN VALUE
  None
==============================================================================*/
void reg_sim_remove_plmn_with_matching_mcc_from_backoff_fplmn_list
(
  sys_plmn_id_s_type plmn_id,
  sys_block_plmn_cause_e_type cause
)
{
  uint8 index = 0;
  uint32  mcc1 = 0, mcc2 = 0;

  sys_plmn_get_mcc(plmn_id, &mcc1);

  for(index = 0; index < reg_sim_backoff_fplmn_list.length; index++)
  {
    sys_plmn_get_mcc(reg_sim_backoff_fplmn_list.plmn_timer_list[index].plmn, &mcc2);
...
sys_sim_state_e_type reg_cm_read_sim_state(
		sys_modem_as_id_e_type as_id
		)
{
#ifdef FEATURE_DUAL_SIM
	if(reg_sim_card_mode_sim[as_id] == MMGSDI_APP_SIM ||
		reg_sim_card_mode_sim[as_id] == MMGSDI_APP_USIM)
	{
	  return reg_sim_state_sim[as_id];
	}
#else
	if(reg_sim_card_mode == MMGSDI_APP_SIM ||
		reg_sim_card_mode == MMGSDI_APP_USIM)
	...
	{
	  return SYS_SIM_STATE_NOT_AVAILABLE;
	}
}