...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/reg/src/reg_send.c#1 $ $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...
)
{
  if((service_state->service_status == SYS_SRV_STATUS_SRV) &&
     ...
     (service_state->plmn_service_capability == SYS_SRV_CAPABILITY_CS_SERVICE_ONLY) ||
      ((reg_mode_req_service_domain_get() == SYS_SRV_DOMAIN_CS_VOLTE) &&
...
)
{
  ...
  reg_send_cb_service_ind(SYS_SRV_STATUS_LIMITED,FALSE,active_rat);
#endif
}
...
)
{
  cm_network_list_cnf_s_type *msg = NULL;
      ...
      msg = (cm_network_list_cnf_s_type *)modem_mem_calloc(1, sizeof(cm_network_list_cnf_s_type), MODEM_MEM_CLIENT_NAS);
  ...
  else if(status == REG_MM_PLMN_SEARCH_ABORTED_DUE_TO_NO_LOCK)
  {
    msg->status = REG_CM_NETWORK_LIST_ABORT_ON_TRM_FAILURE;
  }
  else
  {
...
)
{
  ...
  static boolean reg_inactive_previous_sim[MAX_AS_IDS]={TRUE,TRUE};
  #define reg_inactive_previous reg_inactive_previous_sim[reg_sub_id]
  static sys_radio_access_tech_e_type reg_active_rat_previous_sim[MAX_AS_IDS]={SYS_RAT_NONE,SYS_RAT_NONE};
  #define reg_active_rat_previous reg_active_rat_previous_sim[reg_sub_id]
#ifdef FEATURE_NO_CBS_LTD
  static sys_srv_status_e_type reg_service_status_sim[MAX_AS_IDS]={SYS_SRV_STATUS_NO_SRV,SYS_SRV_STATUS_NO_SRV};
  #define reg_service_status_previous reg_service_status_sim[reg_sub_id]
#endif
#else
  static boolean reg_inactive_previous = TRUE;
  static sys_radio_access_tech_e_type reg_active_rat_previous = SYS_RAT_NONE;
#ifdef FEATURE_NO_CBS_LTD
  static sys_srv_status_e_type reg_service_status_previous = SYS_SRV_STATUS_NO_SRV;
#endif
#endif

  reg_service_state_ind_T msg; 
  if((reg_inactive_previous!= reg_inactive) ||
     (reg_active_rat_previous != active_rat)
#ifdef FEATURE_NO_CBS_LTD
     ||(reg_service_status_previous != service_status )
#endif
     )
  {
    ...
    msg.service_state = service_status;
    msg.reg_inactive = reg_inactive;
    msg.active_rat = active_rat;
	reg_msg_info = (((byte)(msg.active_rat)) << 5) | (((byte)(msg.service_state)) << 1) | ((byte)(msg.reg_inactive));
	...
	reg_inactive_previous = reg_inactive;
    reg_active_rat_previous = msg.active_rat;
  }
}
...
#ifdef FEATURE_UMTS_BMC_MSIM
void reg_send_cb_dds_info_ind(boolean dds_info)
{
  cb_reg_dds_info_s_type msg;
  msg.message_header.message_set    = MS_REG_CB;
  msg.message_header.message_id     = (byte)REG_DDS_INFO_IND;
  msg.message_header.message_len_lsb =
    (sizeof(reg_service_state_ind_T) - sizeof(IMH_T)) % 256;
  msg.message_header.message_len_msb =
    (sizeof(reg_service_state_ind_T) - sizeof(IMH_T)) / 256;

  msg.dds_sub = dds_info;
  msg.as_id = (sys_modem_as_id_e_type)reg_as_id;
  MSG_HIGH_DS_0(REG_SUB, "=REG= REG_DDS_INFO_IND");
  reg_send_message(&msg, GS_QUEUE_CB);
}
#endif
...
)
{
  ...
  reg_send_cb_service_ind( service_state.service_status,FALSE,service_state.active_rat);
  ...
  MSG_HIGH_DS_1(REG_SUB, "=REG= CM_SERVICE_CNF scan_status: %d",msg.scan_status);
  reg_send_message(&msg, GS_QUEUE_CM);
}
...
)
{
  ...
  reg_send_cb_service_ind(reg_state_service_status_get(),TRUE,reg_state_active_rat_get());
  ...
  if(reg_state_cm_sim_update_ind_pending_get()) 
  {
    reg_send_cm_sim_update_ind(reg_sim_state_get());
    reg_state_cm_sim_update_ind_pending_set(FALSE);
  }
...
FUNCTION NAME
  ...
  reg_send_cm_me_blocked_status_ind

==============================================================================*/

  void reg_send_cm_me_blocked_status_ind(boolean is_me_blocked)
  {
      cm_me_blocked_status_ind_s_type msg;
      memset(&msg, 0x00, sizeof(cm_me_blocked_status_ind_s_type));
      msg.message_header.message_set	   = MS_CM_REG;
      msg.message_header.message_id	   = (byte)CM_ME_BLOCKED_STATUS_IND;
      msg.message_header.message_len_lsb =
               (sizeof(cm_me_blocked_status_ind_s_type) - sizeof(IMH_T)) % 256;
      msg.message_header.message_len_msb =
              (sizeof(cm_me_blocked_status_ind_s_type) - sizeof(IMH_T)) / 256;
      msg.as_id = (sys_modem_as_id_e_type)reg_sub_id;
      msg.is_me_blocked = is_me_blocked;
      MSG_HIGH_DS_1(REG_SUB, "=REG= CM_ME_BLOCKED_IND status %d", msg.is_me_blocked);
      reg_send_message(&msg, GS_QUEUE_CM);
  }
#endif
...
)
{
  mmr_plmn_search_req_s_type *msg = NULL;
      ...
      msg = (mmr_plmn_search_req_s_type *)modem_mem_calloc(1, sizeof(mmr_plmn_search_req_s_type), MODEM_MEM_CLIENT_NAS);
  ...
  if(mcc_mode_band_filter)
  {
    ...
    /*for Manual search request with type periodic, Cm should sent valid values to NAS, NAS should set other param to some default values if search is not manual*/
  if(msg->network_selection_mode == SYS_NETWORK_SELECTION_MODE_MANUAL && msg->service_search == SYS_SERVICE_SEARCH_NONE)
  {
    msg->srch_priority = reg_state_get_manual_search_priority();
    msg->incremental_results.enabled = reg_state_get_manual_search_increment_search_status();
    msg->incremental_results.periodicity = reg_state_get_manual_search_periodicty();
	memscpy(&msg->preferred_plmn_list, sizeof(sys_preferred_plmn_list_s_type), preferred_plmn_list, sizeof(sys_preferred_plmn_list_s_type));
	memscpy(&msg->arfcn_list, sizeof(sys_arfcn_list_s_type), arfcn_list, sizeof(sys_arfcn_list_s_type));
  msg->max_search_time = max_search_time;
  }
  else 
  {

    MSG_HIGH_DS_0( REG_SUB, "=REG= Set Peridic search  priority to Nomral and other param to invalid" );
    msg->srch_priority = SYS_SRCH_PRIORITY_NORMAL;
	msg->incremental_results.enabled = FALSE;
	msg->incremental_results.periodicity = 0;
	msg->preferred_plmn_list.count = 0;
	msg->arfcn_list.lte_count = 0;
	msg->arfcn_list.wcdma_count = 0;
        msg->max_search_time = 0;
  }
...
SIDE EFFECTS
  None
===========================================================================*/
boolean reg_set_volte_only
(
  sys_srv_domain_e_type srv_domain
)
{
  boolean volte_only = FALSE; 
...
/*===========================================================================
FUNCTION    reg_get_effective_srv_domain
...
SIDE EFFECTS
  None
===========================================================================*/
sys_srv_domain_e_type reg_get_effective_srv_domain
( 
  sys_srv_domain_e_type srv_domain,
  sys_radio_access_tech_e_type rat
)
{
  sys_srv_domain_e_type eff_srv_domain;
  ...
  switch( srv_domain )
  {
    case SYS_SRV_DOMAIN_CS_VOLTE:
      ...
      {
        eff_srv_domain = SYS_SRV_DOMAIN_CS_PS;
      }
      else
      {
        eff_srv_domain = SYS_SRV_DOMAIN_CS_ONLY;
      }
      break;
    ...
    case SYS_SRV_DOMAIN_VOLTE:
      if(rat == SYS_RAT_LTE_RADIO_ACCESS)
      {
        eff_srv_domain = SYS_SRV_DOMAIN_PS_ONLY;
      }
      else
      {
        eff_srv_domain = SYS_SRV_DOMAIN_NO_SRV;
      }
      break;

    default:
      eff_srv_domain = srv_domain;
      break;
  }
  
  return eff_srv_domain;
}
#endif 
/*==============================================================================
...
)
{
...
#if defined(FEATURE_LTE) && defined(FEATURE_DUAL_DATA)
  reg_send_mmr_reg_req_msg.req_service_domain  = 
                             reg_get_effective_srv_domain(req_service_domain, rat);
  reg_send_mmr_reg_req_msg.volte_only = 
               reg_set_volte_only(req_service_domain);
#else
  reg_send_mmr_reg_req_msg.req_service_domain     = req_service_domain;
  reg_send_mmr_reg_req_msg.volte_only = FALSE;
#endif 
 ...
 /*
    Apply the band filtering only if UE is attemping to camp on China CMCC network.
  */
#ifdef FEATURE_NAS_ENABLE_LTE_BAND_FILTERING
  if ( ( (reg_mode_lte_scan_pref_get() ==  REG_MODE_LTE_SCAN_TDD) ||
         (reg_mode_lte_scan_pref_get() ==  REG_MODE_LTE_SCAN_FDD) )&&
       (reg_mode_is_plmn_td_compliant(plmn) == TRUE) )
         
  {
    reg_mode_update_lte_scan_bands(&(reg_send_mmr_reg_req_msg.rat_pri_list_info), reg_mode_lte_scan_pref_get()); 
  }
#endif
  ...
  */
  ...
  reg_send_mmr_reg_req_msg.is_emergency_call = reg_mode_get_emergency_call(); 
#else
  reg_send_mmr_reg_req_msg.is_emergency_call = FALSE;
#endif
...
void  reg_send_mmr_gstk_service(boolean service_available)
{
  ...
  memset(&reg_send_gstk_service,0x00,sizeof(mmr_gstk_service_ind_s_type));
...
)
{
  ...
  reg_send_last_mmr_reg_req();
  reg_mode_filter_available_plmn_list_with_rat_bst(&rat_pri_list_info);
...
)
{
  ...
  {
    MSG_FATAL_DS(REG_SUB, "=REG= Invalid MMR_REG_REQ RAT = %d ",reg_send_mmr_reg_req_msg.rat,0,0);
  }
  MSG_HIGH_DS_1(REG_SUB, "=REG=  volte_only = %d", reg_send_mmr_reg_req_msg.volte_only);
  reg_send_mmr_reg_req_msg.trans_id = reg_state_mmr_trans_id_set();
#ifdef FEATURE_LTE
  reg_send_mmr_reg_req_msg.lte_cs_domain_param = reg_mode_lte_cs_domain_param_get();
#endif

  MSG_HIGH_DS_8(REG_SUB, "=REG= MMR_REG_REQ PLMN(%u-%u) RAT(0-GSM,1-W,2-LTE,3-TDS) = %d netsel %d, type %d grat_scan_done %d trans_id %d trm_timeout %d", 
  	                                    mcc,
  	                                    mnc,
  	                                    reg_send_mmr_reg_req_msg.rat,
  	                                    reg_send_mmr_reg_req_msg.network_selection_mode,
  	                                    reg_send_mmr_reg_req_msg.type,
  	                                    reg_send_mmr_reg_req_msg.grat_scan_done,
  	                                    reg_send_mmr_reg_req_msg.trans_id,
  	                                    reg_send_mmr_reg_req_msg.trm_timeout);
  reg_send_message(&reg_send_mmr_reg_req_msg, GS_QUEUE_MM);
  if(reg_send_mmr_reg_req_msg.type == MMR_REG_REQ_SYSTEM_CHANGE && reg_send_mmr_reg_req_msg.network_selection_mode == SYS_NETWORK_SELECTION_MODE_AUTOMATIC)
  {
    reg_send_mmr_reg_req_msg.type = MMR_REG_REQ_NORMAL;
  }
}
...
)
{
  ...
  memset(&msg, 0x00, sizeof(mmr_sim_available_req_s_type));
...
)
{
  ...
  memset(&msg, 0x00, sizeof(mmr_sim_not_available_req_s_type));
...
)
{
  ...
  memset(&msg, 0x00, sizeof(mmr_stop_mode_req_s_type));
  ...
  msg.trans_id = reg_state_cm_transaction_id_get();
#if defined FEATURE_DUAL_SIM || defined FEATURE_SGLTE
  msg.as_id = (sys_modem_as_id_e_type)reg_as_id;
#endif

  MSG_HIGH_DS_1(REG_SUB, "=REG= MMR_STOP_MODE_REQ sent trans_id 0x%x",msg.trans_id);
  reg_send_message(&msg, GS_QUEUE_MM );
}
...
)
{
  ...
  memset(&msg, 0x00, sizeof(mmr_eq_plmn_change_ind_s_type));
...
)
{
  ...
  memset(&msg, 0x00, sizeof(mmr_plmn_search_abort_req_s_type));
...
)
{
  ...
  memset(&msg, 0x00, sizeof(mmr_ph_status_change_req_s_type));
...
)
{
  ...
  memset(&msg, 0x00, sizeof(mmr_ecall_mode_ind_s_type));
...
)
{
  ...
  memset(&msg, 0x00, sizeof(mmr_profile_type_ind_s_type));
...
)
{
  ...
  memset(&msg, 0x00, sizeof(mmr_file_refresh_ind_s_type));
...
)
{
  ...
  memset(&msg, 0x00, sizeof(mmr_ps_detach_req_s_type));
...
)
{
  ...
  msg = (cm_fplmn_list_ind_s_type *)modem_mem_calloc(1, sizeof(cm_fplmn_list_ind_s_type), MODEM_MEM_CLIENT_NAS);
...
)
{
  ...
  memset(&msg, 0x00, sizeof(mmr_set_drx_req_s_type));
...
)
{
  ...
  memset(&msg, 0x00, sizeof(mmr_block_plmn_req_s_type));
...
)
{
  ...
  memset(&msg,0x00,sizeof(mmr_sim_state_update_ind_s_type));
...
)
{
  ...
  memset(&msg, 0x00, sizeof(cm_sim_state_update_ind_s_type));
...
void reg_send_mmr_clear_lai_reject_list_req( void )
{
  ...
  memset(&msg, 0x00, sizeof(mmr_clear_lai_reject_list_req_s_type));
...
)
{
  ...
  msg.message_header.message_len_msb     =
    ...
    {
      msg.subs_capability_mask[as_id] = subs_capability_mask_ptr[as_id];
    }
 ...
 )
 {
   ...
   msg.target_dds_sub = target_dds_sub;
    msg.always_on_data = always_on_data;
...
#ifdef FEATURE_DUAL_SIM
void reg_send_mmr_data_priority_ind(sys_proc_type_e_type  proc_type)
{
...
void reg_send_mmr_secure_call_ind(sys_secure_call_e_type  sys_secure_call_type)
{
  mmr_secure_call_ind_s_type msg;
  msg.message_header.message_set     = MS_MM_REG;
  msg.message_header.message_id      = (byte)MMR_SECURE_CALL_IND;
  /*lint -e778 */
  msg.message_header.message_len_lsb =
    (sizeof(mmr_secure_call_ind_s_type) - sizeof(IMH_T)) % 256;
  msg.message_header.message_len_msb =
    (sizeof(mmr_secure_call_ind_s_type) - sizeof(IMH_T)) / 256;
  msg.type = sys_secure_call_type;
  msg.as_id = (sys_modem_as_id_e_type)reg_sub_id;
  MSG_HIGH_DS_0(REG_SUB, "=REG= MMR_SECURE_CALL_IND");
  reg_send_message(&msg, GS_QUEUE_MM);
...
)
{
  ...
  memset(&msg, 0x00, sizeof(mmr_net_sel_mode_change_ind_s_type));
...
)
{
  ...
  memset(&msg, 0x00, sizeof(mmr_csg_select_config_update_ind_s_type));
...
)
{
  ...
  memset(&msg, 0x00, sizeof(cm_csg_search_trigger_ind_s_type));
...
)
{
  ...
  memset(&msg, 0x00, sizeof(cm_csg_nw_sel_mode_update_ind_s_type));
...
==============================================================================*/
void reg_send_mmr_refresh_stage_ind 
(
   mmgsdi_refresh_stage_enum_type  stage
)
{
  mmr_refresh_stage_ind_s_type msg;

  msg.message_header.message_set = MS_MM_REG;
  msg.message_header.message_id  = (byte)MMR_REFRESH_STAGE_IND;
  msg.message_header.message_len_lsb =
    (sizeof( mmr_ecall_mode_ind_s_type ) - sizeof(IMH_T)) % 256;
  msg.message_header.message_len_msb =
    (sizeof( mmr_ecall_mode_ind_s_type ) - sizeof(IMH_T)) / 256;

  msg.refresh_stage = stage;

#if defined FEATURE_DUAL_SIM || defined FEATURE_SGLTE
  msg.as_id =(sys_modem_as_id_e_type)reg_as_id;
#endif
  
  MSG_HIGH_DS_0(REG_SUB, "=REG= MMR_REFRESH_STAGE_IND");
  reg_send_message(&msg, GS_QUEUE_MM );
}
 ...
 void reg_send_mmr_high_priority_activity_ind
 (
    boolean local_detach
 )
 {
   mmr_high_priority_activity_ind_s_type msg;

   msg.message_header.message_set = MS_MM_REG;
   msg.message_header.message_id  = (byte)MMR_HIGH_PRIORITY_ACTIVITY_IND;
   msg.message_header.message_len_lsb =
      (sizeof( mmr_high_priority_activity_ind_s_type ) - sizeof(IMH_T)) % 256;
   msg.message_header.message_len_msb =
      (sizeof( mmr_high_priority_activity_ind_s_type ) - sizeof(IMH_T)) / 256;
 
   msg.local_detach = local_detach;
  
#if defined FEATURE_DUAL_SIM || defined FEATURE_SGLTE
   msg.as_id =(sys_modem_as_id_e_type)reg_as_id;
#endif
   MSG_HIGH_DS_0(REG_SUB, "=REG= MMR_HIGH_PRIORITY_ACTIVITY_IND");
   reg_send_message(&msg.message_header, GS_QUEUE_MM );
 }

#ifdef FEATURE_DUAL_SIM

void reg_send_cb_3gpp2_stack_status_ind(  boolean  is_stack_active,sys_sys_mode_e_type active_mode)
{
  cb_reg_3gpp2_stack_status_ind_s_type    msg;
  msg.message_header.message_set    = MS_REG_CB;
  msg.message_header.message_id     = (byte)REG_3GPP2_STATUS_IND;
  msg.message_header.message_len_lsb =
    (sizeof(cb_reg_3gpp2_stack_status_ind_s_type) - sizeof(IMH_T)) % 256;
  msg.message_header.message_len_msb =
    (sizeof(cb_reg_3gpp2_stack_status_ind_s_type) - sizeof(IMH_T)) / 256;

  msg.active_mode = active_mode;
  msg.is_stack_active = is_stack_active;
  msg.as_id = (sys_modem_as_id_e_type)reg_as_id;
  MSG_HIGH_DS_0(REG_SUB, "=REG= REG_3GPP2_STATUS_IND");
  reg_send_message(&msg, GS_QUEUE_CB);
}
...
==============================================================================*/
void reg_send_mmr_t3245_expiry_ind  
(
  void
)
{
  mmr_t3245_expiry_ind_s_type msg;
  memset(&msg, 0x00, sizeof(mmr_t3245_expiry_ind_s_type));
  
  msg.message_header.message_set = MS_MM_REG;
  msg.message_header.message_id  = (byte)MMR_T3245_EXPIRY_IND;
  msg.message_header.message_len_lsb =
    (sizeof( mmr_t3245_expiry_ind_s_type ) - sizeof(IMH_T)) % 256;
  msg.message_header.message_len_msb =
    (sizeof( mmr_t3245_expiry_ind_s_type ) - sizeof(IMH_T)) / 256;
  ...
  MSG_HIGH_DS_0(REG_SUB, "=REG= MMR_T3245_EXPIRY_IND");
  reg_send_message(&msg, GS_QUEUE_MM );
...
/*=============================================================================
     QSH related APIs
==============================================================================*/
void get_dump_reg_mode_send_vars (reg_debug_data_s_type* ptr, reg_as_id_e_type as_id)
{
    ASSERT (ptr != NULL);
#if defined FEATURE_DUAL_SIM || defined FEATURE_SGLTE
    ptr->reg_send_grat_scan_done_debug = reg_send_grat_scan_done_sim[as_id];
    ptr->reg_send_mmr_reg_req_msg_debug = reg_send_mmr_reg_req_msg_sim[as_id];
#else
    ptr->reg_send_grat_scan_done_debug = reg_send_grat_scan_done;
    ptr->reg_send_mmr_reg_req_msg_debug = reg_send_mmr_reg_req_msg;
#endif
}
#endif
