...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/reg/src/reg_state_registering.c#1 $ $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...
)
{
   ...
   if(reg_mode_network_selection_mode_get() == SYS_NETWORK_SELECTION_MODE_MANUAL &&
  	!sys_plmn_match(msg_p->plmn, reg_send_last_reg_req_plmn_get()) &&
	!(reg_eplmn_list_equivalent_plmn(reg_send_last_reg_req_plmn_get()) &&
                          reg_eplmn_list_equivalent_plmn(msg_p->plmn)))
  {  
