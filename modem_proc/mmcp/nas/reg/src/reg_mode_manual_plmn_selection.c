...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/reg/src/reg_mode_manual_plmn_selection.c#1 $ $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...
)
{
  ...
  */
      ...
      (allowed_service_domain != SYS_SRV_DOMAIN_NO_SRV))
  {
    ...
    if(!sys_plmn_id_is_undefined(rplmn) && sys_plmn_match(rplmn,plmn))
     {
  ...
  if (!sys_plmn_id_is_undefined(plmn))
  {
          ...
          (reg_mode_is_rat_present_in_rat_pri_list(pm_chg_pending_initial_rat,rat_pri_list_info,TRUE) ==TRUE))
   {
    ...
    {      
      ...
      else if((network_selection_mode == SYS_NETWORK_SELECTION_MODE_LIMITED_MANUAL_ACQUISITION) &&
      ...
      else if(type == REG_CM_SERVICE_REQ_NORMAL && sys_plmn_match(reg_sim_read_rplmn(reg_mode_req_service_domain_get()),plmn))
      {
        ...
        if(reg_state_get_mcs_pri()) // efs check
        {
          ...
          reg_mode_prioritize_mcs_rat_order(&rat_priority_list,reg_state_get_mcs_low_priority_rat(),reg_state_get_mcs_high_priority_rat());
        }
        initial_rat = rat_priority_list.rat_list[0];
      }
      /* for service request of type System change, always start from rat priority list from CM*/
      else if(type == REG_CM_SERVICE_REQ_SYSTEM_CHANGE)
      {
        initial_rat = rat_priority_list.rat_list[0];
      }
      ...
      if((initial_rat == SYS_RAT_LTE_RADIO_ACCESS) &&
         ((reg_sim_plmn_backoff_forbidden(plmn,(sys_modem_as_id_e_type)reg_sub_id)) ||
          ((type != REG_CM_SERVICE_REQ_USER_SELECTION) &&
           (type != REG_CM_SERVICE_REQ_USER_PREFERRED_PLMN_RAT) &&
#ifdef FEATURE_DUAL_SIM
  ...
  if (!success)
  {
    success = (boolean)( ((reg_mode_network_selection_mode_get() == SYS_NETWORK_SELECTION_MODE_LIMITED_MANUAL_ACQUISITION) &&
                       (service_state.service_status == SYS_SRV_STATUS_LIMITED)) ||
                         (
#ifdef FEATURE_DUAL_SIM
                           reg_sim_per_subs_plmn_forbidden(reg_mode_req_plmn_get(), (sys_modem_as_id_e_type)reg_sub_id) &&
#else
                           reg_sim_plmn_forbidden(reg_mode_req_plmn_get()) &&
#endif
                           reg_mode_network_selection_mode_get() == SYS_NETWORK_SELECTION_MODE_MANUAL &&
                           service_state.service_status == SYS_SRV_STATUS_LIMITED &&
                           reg_state_get_last_cm_service_req_type() != REG_CM_SERVICE_REQ_USER_SELECTION
                         )
                       ) ;
  }
...
void get_dump_reg_mode_manual_plmn_selection_vars (reg_debug_data_s_type* ptr,
                                                   reg_as_id_e_type as_id)
{
     ASSERT (ptr != NULL);
#if defined FEATURE_DUAL_SIM || defined FEATURE_SGLTE
     ptr->reg_mode_manual_plmn_selection_first_attempt_debug = 
        reg_mode_manual_plmn_selection_first_attempt_sim[as_id];
#else
     ptr->reg_mode_manual_plmn_selection_first_attempt_debug = 
        reg_mode_manual_plmn_selection_first_attempt;
#endif
}
#endif
