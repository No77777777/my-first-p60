...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/reg/src/reg_state_idle.c#1 $
          ...
          (reg_state_service_status_get() == SYS_SRV_STATUS_LIMITED_REGIONAL) )
      {
        if (reg_mode_higher_priority_network_available
              (
                ...
                FALSE, /* Set FALSE for legacy HPLMN search */
				NULL
              ))
        {
      ...
      default:
          ...
          if (reg_state_service_status_get() == SYS_SRV_STATUS_SRV)
          {
              ...
              */
              ...
              list = (sys_plmn_rat_list_s_type *)modem_mem_calloc(1, sizeof(sys_plmn_rat_list_s_type), MODEM_MEM_CLIENT_NAS);
              ...
              {
              efs_or_ehplmn_list = (reg_sim_plmn_list_s_type *)modem_mem_calloc(1, sizeof(reg_sim_plmn_list_s_type), MODEM_MEM_CLIENT_NAS);
   ...
   /* HPLMN */
             ...
             for(i = 0; (i < efs_or_ehplmn_list->length && list->length < SYS_PRIORITY_PLMN_LIST_MAX_LENGTH); i++)
              ...
              for(i = 0; i < rat_pri_list_info.num_items; i++)
              {
                ...
                {
                  MSG_HIGH_DS_3(REG_SUB, "=REG= sys_mode = %d band_cap = 0x%08x %08x",
                     rat_pri_list_info.priority_list_info[i].acq_sys_mode,
                     QWORD_HIGH(rat_pri_list_info.priority_list_info[i].band_cap.chgwt_band_cap),
                     QWORD_LOW(rat_pri_list_info.priority_list_info[i].band_cap.chgwt_band_cap));
                }
              }
     ...
     ))  
  {
      ...
      default:
       ...
       /* HPLMN */
              ...
              next_state_p = &reg_state_automatic_search;
#if defined(FEATURE_DUAL_SIM) || defined(FEATURE_SGLTE)
              nas_event_report_payload(EVENT_NAS_LTE_IRAT_SEARCH_STARTED,
                  (uint8)event_size,(void*)diag_event,reg_message_as_id);
#else
              nas_event_report_payload(EVENT_NAS_LTE_IRAT_SEARCH_STARTED,
                                 (uint8)event_size,NULL,reg_message_as_id);
...
(
...
)
 {
  ...
  reg_state_p_type             next_state_p = &reg_state_idle;
  boolean                      dual_mode = FALSE;
  sys_rat_pri_list_info_s_type *rat_pri_list_info = NULL;
  ...
  rat_pri_list_info = (sys_rat_pri_list_info_s_type *)modem_mem_calloc(1, sizeof(sys_rat_pri_list_info_s_type), MODEM_MEM_CLIENT_NAS);
  reg_check_for_null_ptr((void*)rat_pri_list_info );
  
  memscpy(rat_pri_list_info, sizeof(sys_rat_pri_list_info_s_type), 
          reg_mode_rat_pri_list_get(), sizeof(sys_rat_pri_list_info_s_type));

  switch (reg_mode_network_selection_mode_get())
  {
    case SYS_NETWORK_SELECTION_MODE_AUTOMATIC:
    case SYS_NETWORK_SELECTION_MODE_LIMITED_ROUTINE_ACQUISITION: //Doubt
   if ((reg_state_service_status_get() == SYS_SRV_STATUS_SRV) &&
       ...
       (reg_sim_check_mcc_same_country(reg_state_service_plmn_get()) == TRUE) )
      {
        if (reg_mode_higher_priority_network_available
              (
                reg_state_active_rat_get(),
                higher_priority_plmn_list,
                TRUE,  /* Set this flag as TRUE for LTE Only CFL HPLMN search */
				rat_pri_list_info
              ))
        {
    ...
    case SYS_NETWORK_SELECTION_MODE_MANUAL:
    case SYS_NETWORK_SELECTION_MODE_LIMITED_MANUAL_ACQUISITION:
      MSG_ERROR_DS_0(REG_SUB, "=REG= CFL Search Timer expired - manual mode"); 
      break;

    case SYS_NETWORK_SELECTION_MODE_LIMITED:
      MSG_ERROR_DS_0(REG_SUB, "=REG= CFL Search Timer expired - limited mode"); 
      break;

    default:
      MSG_ERROR_DS_1(REG_SUB, "=REG= CFL  Search Timer expired - mode (%d)", reg_mode_network_selection_mode_get()); 
      break;
  }

   modem_mem_free(higher_priority_plmn_list, MODEM_MEM_CLIENT_NAS);
   modem_mem_free(rat_pri_list_info, MODEM_MEM_CLIENT_NAS);

   higher_priority_plmn_list = NULL;
   rat_pri_list_info = NULL;
