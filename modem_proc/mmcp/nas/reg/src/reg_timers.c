...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/reg/src/reg_timers.c#1 $ $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...
static timer_type reg_timers_fast_cfl_search_period_timer_sim[MAX_NAS_STACKS];
#define reg_timers_fast_cfl_search_period_timer  reg_timers_fast_cfl_search_period_timer_sim[reg_as_id]

#else
static timer_type reg_timers_hplmn_search_period_timer;
static timer_type reg_timers_fast_cfl_search_period_timer;
...
#ifdef FEATURE_LTE
static void reg_timers_cfl_hplmn_search_timer_expiration_callback
(
  unsigned long dummy
)
{

  if (dummy == REG_CFL_SEARCH_TIMER)
  {
    reg_timers_enqueue_timer_command
    (
       REG_CFL_SEARCH_TIMER
    );
  }
#if defined FEATURE_DUAL_SIM  
  else if(dummy == REG_CFL_SEARCH_TIMER_SIM_ID_2)
  {
    reg_timers_enqueue_timer_command
    (
       REG_CFL_SEARCH_TIMER_SIM_ID_2
    );
  }
#if defined FEATURE_TRIPLE_SIM_DUAL_SIM
  else if(dummy == REG_CFL_SEARCH_TIMER_SIM_ID_3)
  {
    reg_timers_enqueue_timer_command
    (
       REG_CFL_SEARCH_TIMER_SIM_ID_3
    );
  }
...
#ifdef FEATURE_LTE
/*
** This timer is used for CFL and ACQ DB search for early recovery on HPLMN in LTE
*/

#if defined FEATURE_DUAL_SIM 
  timer_def(&reg_timers_fast_cfl_search_period_timer_sim[SYS_MODEM_AS_ID_2], 
            &nas_timer_group_non_deferrable,
            rex_self(),
            0,
            reg_timers_cfl_hplmn_search_timer_expiration_callback,
            REG_CFL_SEARCH_TIMER_SIM_ID_2);
   timer_def(&reg_timers_fast_cfl_search_period_timer_sim[SYS_MODEM_AS_ID_1], 
            &nas_timer_group_non_deferrable,
            rex_self(),
            0,
            reg_timers_cfl_hplmn_search_timer_expiration_callback,
...
#if defined FEATURE_TRIPLE_SIM_DUAL_SIM
   timer_def(
           &reg_timers_fast_cfl_search_period_timer_sim[SYS_MODEM_AS_ID_3],
           &nas_timer_group_non_deferrable,
           rex_self(),
           0,
           reg_timers_cfl_hplmn_search_timer_expiration_callback,
...
#else
  timer_def(&reg_timers_fast_cfl_search_period_timer, 
            &nas_timer_group_non_deferrable,
            rex_self(),
            0,
            reg_timers_cfl_hplmn_search_timer_expiration_callback,
...
)
{
  ...
  /* In case HPLMN search timer expired, restart HPLMN search timer
  ** instead of CFL search timer in automatic state else restart CFL
     timer.
  */
#ifdef FEATURE_LTE
  if ((timer_get(&reg_timers_hplmn_search_period_timer, T_MSEC) > 0) &&
      (reg_state_get_cfl_hplmn_timer_search_ongoing() == TRUE))
  {
...
)
{
      ...
      (timer_get(&reg_timers_hplmn_search_period_timer, T_MSEC) == 0))
  {
    ...
    */
    ...
    reg_state_set_hplmn_timer_prev_plmn(reg_state_service_plmn_get());
  ...
  /* NAS will run new timer for HPLMN_FREQ scan on configured frequency list and ACQ DB
  **  after acquiring full service on roaming PLMN in LTE in Home MCC
  */
  reg_timers_start_hplmn_cfl_search_period_timer();
}
...
)
{
  ...
  reg_state_set_hplmn_timer_prev_plmn(plmn);
...
)
{
  ...
  if(found_in_avaialble_list || found_in_non_avaialble_list)
  {
    ...
    if(!timer_get(&reg_timers_lte_irat_search_period_timer, T_MSEC))
    {
      ...
      if( reg_timer_short_timer_started == TRUE)
      {
#if defined(FEATURE_DUAL_SIM) || defined(FEATURE_SGLTE)
        nas_event_report_payload(EVENT_NAS_LTE_IRAT_SHORT_TIMER_STARTED,
       ...
       else if(reg_timer_long_timer_started == TRUE)
       {
#if defined(FEATURE_DUAL_SIM) || defined(FEATURE_SGLTE)
        nas_event_report_payload(EVENT_NAS_LTE_IRAT_LONG_TIMER_STARTED,
               (uint8)event_size,(void*)diag_event,reg_message_as_id);
#else
        nas_event_report_payload(EVENT_NAS_LTE_IRAT_LONG_TIMER_STARTED,
                     (uint8)event_size,NULL,reg_message_as_id);
...
#if defined(FEATURE_DUAL_SIM) || defined(FEATURE_SGLTE)
      nas_event_report_payload(EVENT_NAS_HPLMN_IRAT_SEARCH_STARTED,
             (uint8)event_size,(void*)diag_event,reg_message_as_id);
#else
      nas_event_report_payload(EVENT_NAS_HPLMN_IRAT_SEARCH_STARTED,
                         (uint8)event_size,NULL,reg_message_as_id);
...
)
{
  ...
  (void)as_id;
   ...
   {
     MSG_HIGH_DS_1(as_id+1, "=REG= Restart BACKOFF PLMN Search Timer TIMER COUNT %lu", timer);
     (void) rex_clr_timer
     (
       backoff_timer_ptr
     );
   ...
   {
     MSG_HIGH_DS_1(as_id+1, "=REG= Start BACKOFF PLMN Search Timer  TIMER COUNT %lu", timer);
   }
   (void) rex_set_timer
   (
     backoff_timer_ptr,
...
)
{
    ...
    if((curr_timer_count == 0) && 
       (backoff_fplmn_list_info_p->length > 0) &&
       (backoff_fplmn_list_info_p->length < REG_SIM_BACKOFF_FPLMN_LIST_LENGTH))
    {
    ...
    else if((backoff_fplmn_list_info_p->length > 0) &&
            (backoff_fplmn_list_info_p->length < REG_SIM_BACKOFF_FPLMN_LIST_LENGTH))
    {
...
{
  if(rex_get_timer(&reg_timers_update_lte_cap_timer))
  {
...
void reg_timers_start_hplmn_cfl_search_period_timer
(
  void
)
{  
   /* Timer value in EFS is provided in seconds convert to milliseconds*/
  dword timer_count = ((dword) MIN ((reg_nv_hplmn_cfl_search_timer_get()) * 1000,
                                     (reg_nv_cfl_search_config.max_fast_scan_time *1000)));

  MSG_HIGH_DS_1(REG_SUB, "=REG= timer_count = %d", timer_count);

  if (timer_count == 0)
  {
    MSG_HIGH_DS_1(REG_SUB, "=REG= Unexpected fast scan cfl timer value = %d", timer_count);
    return;
  }
  
  if ((!sys_plmn_match(reg_state_prev_plmn_get(), reg_state_service_plmn_get())) ||
       (reg_state_active_rat_get() != SYS_RAT_LTE_RADIO_ACCESS) ||
        (reg_state_prev_tac_get() != reg_state_service_tac_get()))
  {
       reg_cfl_hplmn_search_counter = 0;
       reg_timers_stop_hplmn_cfl_search_period_timer();
       timer_count = ((dword)reg_nv_hplmn_cfl_search_timer_get()) * 1000;
  }

 if ((reg_state_service_status_get() == SYS_SRV_STATUS_SRV) &&
      (reg_state_active_rat_get() == SYS_RAT_LTE_RADIO_ACCESS) &&
      (reg_state_get_cfl_hplmn_timer_search_ongoing() == FALSE) &&
      ((reg_state_roaming_ind_get()== SYS_ROAM_STATUS_ON) ||
      ...
      (reg_sim_check_mcc_same_country(reg_state_service_plmn_get()) == TRUE) )
 {
   ...
   /* Check if the timer is already running */
   if (!timer_get(&reg_timers_fast_cfl_search_period_timer, T_MSEC))
   {
      (void) timer_set
      (
        &reg_timers_fast_cfl_search_period_timer,
        timer_count,
        0,
        T_MSEC
      );

      MSG_HIGH_DS_1(REG_SUB, "=REG= Started HPLMN CFL Search Timer (%d seconds)", (timer_count/1000));   
    }
  }
}
/*==============================================================================
...
==============================================================================*/
void reg_timers_stop_hplmn_cfl_search_period_timer
(
  void
)
{
    ...
    {        
        ...
        MSG_HIGH_DS_0(REG_SUB, "=REG= Stopped HPLMN CFL Search Timer");
    }
}


void reg_timers_start_hplmn_cfl_search_period_timer_10sec
(
  void
)
{
       ...
       (reg_sim_check_mcc_same_country(reg_state_service_plmn_get()) == TRUE) )
  {
    ...
    MSG_HIGH_DS_1(REG_SUB, "=REG= Started HPLMN CFL Search Timer(%d seconds)", (word)(timer_count/1000));  
  }
}
...
void reg_timers_stop_validate_sim_timer(void)
{
  ...
  {
    MSG_HIGH_DS_0(REG_SUB, "=REG= Stopped Validate Sim Timer");
    (void) rex_clr_timer
           (
             &reg_timers_validate_sim_timer
           );
...
#endif
#endif
       if(timer_get(&reg_timers_fast_cfl_search_period_timer, T_MSEC) > 0)
       {
         timer_active = TRUE;
       }
       break;
...
DESCRIPTION
  Function to check whether the HPLMN timer is active
  
RETURN VALUE
  Boolean
  TRUE  = HPLMN timer is active
  FALSE = HPLMN timer is stopped.
===========================================================================*/
boolean reg_timers_is_hplmn_timer_active(void)
{
  if(timer_get(&reg_timers_hplmn_search_period_timer, T_MSEC) == 0)
  {
    return FALSE;
