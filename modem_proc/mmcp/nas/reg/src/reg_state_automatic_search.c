...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/reg/src/reg_state_automatic_search.c#2 $
...
#ifdef FEATURE_LTE
  else if (msg_p->service_search == SYS_SERVICE_SEARCH_HPLMN_FREQ)
  {
  ...
  else 
  {
    ...
    */
    else if (reg_mode_start_higher_priority_plmn_selection(&(msg_p->found_plmn_list.plmn_list)))
    {
     ...
     if(reg_nv_allow_hplmn_timer_restart_value_get((sys_modem_as_id_e_type)reg_as_id) && reg_mode_hplmn_timer_restart(&(msg_p->found_plmn_list.plmn_list)))
      {
        MSG_HIGH_DS_0(REG_SUB,"=REG= HPPLMN search ended before searching whole RAT list");
        reg_timers_stop_hplmn_search_period_timer();
      }
...
{
  reg_state_p_type next_state_p = &reg_state_automatic_search;
  if ((reg_state_get_irat_lte_timer_search_ongoing() == TRUE) || (reg_state_get_cfl_hplmn_timer_search_ongoing() == TRUE ))
  {
...
)
{
  ...
  if ((reg_state_get_irat_hplmn_timer_search_ongoing() == TRUE) || (reg_state_get_cfl_hplmn_timer_search_ongoing() == TRUE ))
  {
    /*starting 10 second timer */
...
)
{
  ...
  if (reg_state_get_cfl_hplmn_timer_search_ongoing() == FALSE)
  {
    /*starting 10 second timer */
    reg_timers_start_hplmn_cfl_search_period_timer_10sec();
  }
...
#ifdef FEATURE_LTE
      if (reg_state_get_cfl_hplmn_timer_search_ongoing() == TRUE)
      {
        reg_timers_start_hplmn_search_timer_10sec();
      }
      else
#endif
      {
        return reg_state_unexpected_hplmn_search_timer_expiration();
      }
      break;
