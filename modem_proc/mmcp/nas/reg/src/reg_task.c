...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/reg/src/reg_task.c#2 $    $DateTime: 2025/11/18 02:39:57 $ $Author: pwbldsvc $
...
#ifdef FEATURE_QSH_DUMP
extern void reg_qsh_debug_callback(qsh_client_cb_params_s * params) REG_QSH_ATTR_UNCOMPRESS;
#endif
/*==============================================================================
...
"/nv/item_files/modem/nas/td_op_device_type\n"\
"/nv/item_files/modem/nas/ehplmn_temp_forbid\n"\
"/nv/item_files/modem/nas/interlace_scan_timer\n"\
"/nv/item_files/modem/nas/lte_fast_cfl_search_timer\n"\
"/nv/item_files/modem/nas/lte_fast_cfl_search_timer_Subscription01\n"\
"/nv/item_files/modem/nas/interlace_scan_timer_Subscription01\n"\
"/nv/item_files/modem/nas/interlace_scan_timer_Subscription02\n"\
"/nv/item_files/modem/nas/extended_preferred_plmn_list1\n"\
"/nv/item_files/modem/nas/extended_preferred_plmn_list2\n"\
"/nv/item_files/modem/nas/allow_hplmn_timer_restart\n\
";
  ...
  reg_check_for_null_ptr((void*)cmd);
    ...
    switch(cmd_type)
    {
      ...
      case CM_SECURE_CALL_REQ:
       cmd->cmd.hdr.message_len_lsb =
         (sizeof(cm_secure_call_s_type) - sizeof(IMH_T)) % 256;
       cmd->cmd.hdr.message_len_msb =
         (sizeof(cm_secure_call_s_type) - sizeof(IMH_T)) / 256;
       break;
#endif
    ...
    case  CM_CSG_SELECT_CONFIG_UPDATE_IND :
     ...
     break;
      ...
      case CM_HIGH_PRIORITY_ACTIVITY_IND:
        cmd->cmd.hdr.message_len_lsb =
            (sizeof(cm_high_priority_activity_ind_s_type) - sizeof(IMH_T)) % 256;
        cmd->cmd.hdr.message_len_msb =
            (sizeof(cm_high_priority_activity_ind_s_type) - sizeof(IMH_T)) / 256;
        break;
     ...
     case CM_3GPP2_STATUS_IND:
        cmd->cmd.hdr.message_len_lsb =
          (sizeof(cm_3gpp2_status_ind_s_type) - sizeof(IMH_T)) % 256;
        cmd->cmd.hdr.message_len_msb =
          (sizeof(cm_3gpp2_status_ind_s_type) - sizeof(IMH_T)) / 256;
      break;
      default:
...
)
{
  ...
  if (POLICYMAN_SUCCEEDED(status))
  {
    ...
    if (POLICYMAN_SUCCEEDED(status))
    {
      reg_sim_max_subscription = (uint8)num_of_sims;
    }
...
#endif
#ifdef FEATURE_DUAL_SIM 
  reg_subs_capability_mask_sim[REG_AS_ID_1] = (SYS_SYS_MODE_MASK_LTE|SYS_SYS_MODE_MASK_WCDMA|
                                             SYS_SYS_MODE_MASK_TDS|SYS_SYS_MODE_MASK_GSM);
  reg_subs_capability_mask_sim[REG_AS_ID_2] = SYS_SYS_MODE_MASK_GSM;
#if defined(FEATURE_TRIPLE_SIM)
  reg_subs_capability_mask_sim[REG_AS_ID_3] = SYS_SYS_MODE_MASK_GSM;
#endif
#endif
...
#ifdef FEATURE_QSH_DUMP
{
  ...
  qsh_client_reg_init(&client_reg_ptr);
  client_reg_ptr.client = QSH_CLT_REG;
  client_reg_ptr.major_ver = REG_QSH_MAJOR_VER;
  client_reg_ptr.minor_ver = REG_QSH_MINOR_VER;
  client_reg_ptr.cb_action_support_mask = QSH_ACTION_DUMP_COLLECT;
  client_reg_ptr.client_cb_ptr = reg_qsh_debug_callback;
  client_reg_ptr.dump_info.max_size_bytes = 2*sizeof(reg_qsh_dump_data_s_type) +
                                      sizeof(reg_qsh_dump_debug_buffer_s_type);
  qsh_client_reg(&client_reg_ptr);
}
#endif
reg_state_reset_plmn_search_transaction_id();
#ifdef FEATURE_DUAL_SIM 
reg_mode_clear_ehplmn_forbidden((sys_modem_as_id_e_type)REG_AS_ID_1);
reg_mode_clear_ehplmn_gprs_forbidden((sys_modem_as_id_e_type)REG_AS_ID_1);
reg_mode_clear_ehplmn_forbidden((sys_modem_as_id_e_type)REG_AS_ID_2);
reg_mode_clear_ehplmn_gprs_forbidden((sys_modem_as_id_e_type)REG_AS_ID_2);
#else
reg_mode_clear_ehplmn_forbidden();
reg_mode_clear_ehplmn_gprs_forbidden();
#endif
trm_register_fallback_ind_cb(TRM_IDLE_COLL_CLIENT_NAS,reg_mode_set_mcs_pri_cb);
}
...
SIDE EFFECTS
  None
===========================================================================*/
ULogHandle reg_get_ulog_handle(void)
{
