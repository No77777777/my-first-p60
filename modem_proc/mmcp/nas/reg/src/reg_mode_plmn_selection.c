...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/reg/src/reg_mode_plmn_selection.c#1 $ $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...
)
{
  ...
  */
          ...
          )
  {
    ...
    if (type == REG_CM_SERVICE_REQ_SYSTEM_CHANGE)
    {
      reg_req_type = MMR_REG_REQ_SYSTEM_CHANGE;
    }
  }
#endif
  ...
  */
  if ((sys_plmn_id_is_undefined(plmn)) &&
      ((!sys_plmn_id_is_undefined(rplmn)) &&
       ...
       ((!reg_sim_per_subs_plmn_forbidden(rplmn, (sys_modem_as_id_e_type)reg_sub_id))||
        (((reg_sim_per_subs_int_voice_forbidden(rplmn, (sys_modem_as_id_e_type)reg_sub_id)) || (reg_sim_per_subs_domestic_voice_forbidden(rplmn, (sys_modem_as_id_e_type)reg_sub_id))) && 
        (reg_mode_network_selection_mode_get() == SYS_NETWORK_SELECTION_MODE_MANUAL))) &&
#else
       ((!reg_sim_plmn_forbidden(rplmn))||
       ((reg_sim_int_voice_forbidden(rplmn) || reg_sim_domestic_voice_forbidden(rplmn)) && 
        (reg_mode_network_selection_mode_get() == SYS_NETWORK_SELECTION_MODE_MANUAL))) &&
#endif
       (allowed_service_domain != SYS_SRV_DOMAIN_NO_SRV)))
  {
    ...
    {
      reg_mode_determine_first_plmn(&plmn, &rplmn_rat_search_order, rat_pri_list_info, FALSE);
    }
    if(sys_plmn_match(plmn, rplmn))
    {
      ...
      if(reg_state_get_mcs_pri()) // efs check
      {
        ...
        reg_mode_prioritize_mcs_rat_order(&rplmn_rat_search_order,reg_state_get_mcs_low_priority_rat(),reg_state_get_mcs_high_priority_rat());
      }
  ...
  else
  {
        ...
        network_selection_mode == SYS_NETWORK_SELECTION_MODE_AUTOMATIC_CAMP_ONLY)
    {
      ...
      */
      ...
      {
        automatic_mode_type = REG_MODE_AUTOMATIC_MODE_IRAT_TO_LTE;
      }
      else
      {
...
)
{
  ...
  scan_info.scan_time = 0;
  rplmn = reg_sim_read_rplmn(req_service_domain);
  reg_mode_plmn_selection_mode = automatic_mode;
  ...
  */
       ...
       (allowed_service_domain != SYS_SRV_DOMAIN_NO_SRV))
  {
    ...
    if(!reg_mode_is_rat_present_in_rat_pri_list(active_rat, rat_pri_list_info, TRUE))
    {
     ...
     if(!sys_plmn_id_is_undefined(rplmn) && sys_plmn_match(rplmn,plmn)) // plmn is rplmn check.. undefined already did before
     {
      if(reg_state_get_mcs_pri()) // efs check
      {
         if (reg_mode_is_rat_present_in_rat_pri_list(reg_state_get_mcs_low_priority_rat(),reg_mode_rat_pri_list_get(),FALSE) && (reg_state_get_mcs_low_priority_rat() != initial_rat))
            {
              reg_mode_deprioritize_rat_pri_list(&rat_search_order,reg_state_get_mcs_low_priority_rat());
            }
        else
            {
              reg_mode_prioritize_rat_pri_list(&rat_search_order,reg_state_get_mcs_high_priority_rat(),2);   
            }
      }
...
)
{
  ...
  boolean reg_rlf_abort_due_to_ehplmn = FALSE;
#endif
...
#if defined(FEATURE_LTE) || defined(FEATURE_WRLF_SYSTEM_SEL)
      /* NAS will ensure to abort the existing RLF after L-RRC reports 
      ** Rakuten high priorty EHPLMN in available PLMN list during suitable search*/

      reg_rlf_abort_due_to_ehplmn = reg_mode_is_avail_plmn_list_contains_higher_priority_ehplmn(available_plmn_list_p);
#endif
      if ((reg_eplmn_list_equivalent_plmn(reg_mode_req_plmn_get()) || 
           sys_plmn_match(reg_mode_req_plmn_get(), reg_sim_read_rplmn(reg_mode_req_service_domain_get()))) 
#if defined(FEATURE_LTE) || defined(FEATURE_WRLF_SYSTEM_SEL)		   
		   && (reg_rlf_abort_due_to_ehplmn == FALSE)
#endif
		 )
      {
        /*
        ...
        reg_mode_plmn_selection_first_attempt = TRUE;
               ...
               (reg_state_is_pwr_on_manual_plmn_selection))
                ...
                && ((reg_mode_plmn_selection_mode != REG_MODE_AUTOMATIC_MODE_SUITABLE_SEARCH) ||
                    (reg_rlf_abort_due_to_ehplmn == TRUE))
#endif
                )
      {
         MSG_HIGH_DS_0(REG_SUB, "=REG= RLF get next available PLMN"); 
#ifdef FEATURE_LTE
         if (reg_mode_fallback_control_get() == TRUE)
         {            
            // Check if all the rats have been processed
...
/*=============================================================================
     QSH related APIs
==============================================================================*/
void get_dump_reg_mode_plmn_selection_vars (reg_debug_data_s_type* ptr,
                                            reg_as_id_e_type as_id)
{    
    ASSERT (ptr != NULL);
#if defined FEATURE_DUAL_SIM || defined FEATURE_SGLTE
    ptr->reg_mode_plmn_selection_first_attempt_debug = 
        reg_mode_plmn_selection_first_attempt_sim[as_id];
    ptr->reg_mode_plmn_selection_mode_debug = 
        (reg_mode_automatic_mode_debug_e_type)reg_mode_plmn_selection_mode_sim[as_id];
    ptr->reg_mode_plmn_selection_reg_req_type_debug = 
        reg_mode_plmn_selection_reg_req_type_sim[as_id];    

#else
    ptr->reg_mode_plmn_selection_first_attempt_debug = 
        reg_mode_plmn_selection_first_attempt;
    ptr->reg_mode_plmn_selection_mode_debug = 
        (reg_mode_automatic_mode_debug_e_type)reg_mode_plmn_selection_mode;
    ptr->reg_mode_plmn_selection_reg_req_type_debug = 
        reg_mode_plmn_selection_reg_req_type;
