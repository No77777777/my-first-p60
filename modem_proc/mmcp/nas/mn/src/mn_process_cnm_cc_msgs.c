...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/mn/src/mn_process_cnm_cc_msgs.c#1 $    $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...
extern void mm_send_rrc_abnormal_call_end_ind(sys_modem_as_id_e_type as_id);
...
VOID_FUNC MN_handle_MNCC_DISC_IND(IMH_T *message_header)
{
  ...
  if(mn_call_information_p[message_ptr->connection_id] != NULL)
  {
   ...
   /* sending abnoraml call end ind to RRC to avoid the FRLTE  when MO/MT call failures with abnormal causes */

       if((cm_cc_cause.present == TRUE) &&
          ((cm_cc_cause.cause_value ==NO_CIRCUIT_CHANNEL_AVAILABLE  )||
           (cm_cc_cause.cause_value ==NETWORK_OUT_OF_ORDER)||
           (cm_cc_cause.cause_value ==TEMPORARY_FAILURE)||
           (cm_cc_cause.cause_value ==SWITCHING_EQUIPMENT_CONGESTION)||
           (cm_cc_cause.cause_value ==REQUESTED_CIRCUIT_CHANNEL_NOT_AVAILABLE)||
           (cm_cc_cause.cause_value ==RESOURCES_UNAVAILABLE_UNSPECIFIED)
           ))
       {
         mm_send_rrc_abnormal_call_end_ind(message_ptr->as_id);
       }
   ...
   else  /* acceptable CC Transaction  */
   {
      ...
      switch(message_ptr->reason)
      {
          ...
          case WCDMA_RAB_RECONFIGURED:
          {
              ...
              {
                MSG_HIGH_DS_1(MN_SUB,"=MNCNM= cannot set modify_in_progress as mn_call_information_p[%d] is NULL",
                connection_id);
              }
#endif /* FEATURE_MULTIMEDIA_FALLBACK */
              break;
...

{
   ...
   else
   {
     ...
     if (s_respond_to_event == TRUE)
     {
       ...
       if (tch_user_connections_store[DEFAULT_RAB_ID].OTA_channel_connected == TRUE)
       {
          MSG_HIGH_DS_1(MN_SUB,"=MNCNM= =MN= BC.ITC  : %d ",mn_call_information_p[pointer->connection_id]->bearer_capability_1.information_transfer_capability );
...

{
   ...
   else
   {
     ...
     {
        MSG_HIGH_DS_1(MN_SUB,"=MNCNM= =MN= BC.ITC  : %d ",call_data->bearer_capability_1.information_transfer_capability );
 ...
 *
 ***********************************************************************/
VOID_FUNC    MN_handle_cs_sig_conn_rel_ind(IMH_T    *message)
{
   ...
   if(sms_link_control.status == SMS_LINK_CONTROL_ACTIVE)   /* Active && Timer is running */
   {
       ...
       MN_put_uasms_link_control_report_ind(sms_link_control.mode, 
#if defined(FEATURE_DUAL_SIM)
                              ((MNCC_CS_SIG_CONN_REL_IND_T *)((void *)message))->as_id,
#endif
...
#if 0 /* This block is related to UUS3 */
      if(mn_call_information_p[connection_id] != NULL)
      {
...
/*        mn_alternate_proc_status.status = MN_SS_ALT_NACKED; */
              ...
              mn_alternate_proc_status.status == MN_SS_ALT_HOLD_ACK_AWAITED)
      {
        ...
        if(mn_alternate_proc_status.status == MN_SS_ALT_NACKED)
        {
          ...
          */
          if((Multicall_Status[(Multicall_Status_index + 1) % 2].connection_id[0] < MAXNO_CC_TRANSACTIONS )&&
		  	(cc_transaction_information_p[Multicall_Status[(Multicall_Status_index + 1) % 2].connection_id[0]] != NULL) &&
