...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/mn/src/mn_process_cm_cc_msgs.c#1 $    $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...
extern void cm_util_set_default_mt_setup_res_per_sub(sys_modem_as_id_e_type asubs_id,cm_mt_setup_res_T *cm_mt_setup_res);
...
VOID_FUNC MN_handle_cm_mt_modify_res(IMH_T *message)
{
   ...
   {
     MSG_ERROR_DS_2(MN_SUB,"=MNCNM= Received Invalid as_id=%d, mn_as_id=%d", message_ptr->as_id,mn_as_id); 
   }
#endif
}
...
/*
 * Function name :  MN_send_setup_resp_after_HO_complete
 * -------------
 *
 * Description :  Handle sending of setup_res after sending HO_complete indication to CM . This is only for SRVCC calls if call is in U6 state
 * -----------     
 *
 *
 *
...
VOID_FUNC MN_send_setup_resp_after_HO_complete(byte local_conn_id, sys_modem_as_id_e_type as_id)
{

    cm_mt_setup_res_T       *message_ptr;

    message_ptr = (cm_mt_setup_res_T *)CNM_alloc(sizeof(cm_mt_setup_res_T));

    MSG_HIGH_0("send_setup_resp_after_HO_complete ind to CM");

    cm_util_set_default_mt_setup_res_per_sub(as_id,message_ptr);

    message_ptr->connection_id = local_conn_id;
    MN_handle_cm_mt_setup_res((IMH_T *)message_ptr);
...
MN_get_cm_mo_call_req(connection_id_T new_connection_id, IMH_T *message_ptr)
{
   ...
   else
   {
    ...
    /* SETUP UUI */
#ifdef FEATURE_UUS
     if(normal_call_message_ptr->mo_mt_user_data.present == FALSE)
     {
        mn_call_information_p[new_connection_id]->user_data_info.present = FALSE;
     }
     else 
     {
        mn_call_information_p[new_connection_id]->user_data_info.present = TRUE;
     }

     if(mn_call_information_p[new_connection_id]->user_data_info.present == TRUE )
     {
         /* Check Implicit Activation */
         if (normal_call_message_ptr->mo_mt_user_data.user_user_type <= UUS1_IMPLICIT)
         {
           if (normal_call_message_ptr->mo_mt_user_data.user_user_data_length <= MAX_USER_USER_DATA_LENGTH)
           {

              mn_call_information_p[new_connection_id]->user_data_info= normal_call_message_ptr->mo_mt_user_data ;
             
           }
           else if(normal_call_message_ptr->mo_mt_user_data.user_user_data_length >  MAX_USER_USER_DATA_LENGTH)
           { 
              /* truncate the length */
              MSG_HIGH_DS_2 (MN_SUB,"=MNCNM= Truncated the user data length : %d -> %d",normal_call_message_ptr->mo_mt_user_data.user_user_data_length,MAX_USER_USER_DATA_LENGTH);
              normal_call_message_ptr->mo_mt_user_data.user_user_data_length = MAX_USER_USER_DATA_LENGTH;
         ...
         { 
           MSG_ERROR_DS_0(MN_SUB,"=MNCNM= other than UUS1_IMPLICIT is not supported");
           mn_call_information_p[new_connection_id]->user_data_info.present = FALSE;
         }
...
VOID_FUNC MN_handle_cm_abort_cc_req(cm_abort_cc_req_T *abort_req)
{
   ...
   {
     MSG_ERROR_DS_2(MN_SUB,"=MNCNM= Received Invalid as_id=%d, mn_as_id=%d", abort_req->as_id,mn_as_id);
...
VOID_FUNC   MN_handle_cm_mo_modify_req(IMH_T    *message)
{
   ...
   else if(MNCNM_IS_SINGLE_STACK)
   {
     ...
     MSG_ERROR_DS_2(MN_SUB,"=MNCNM= Received Invalid as_id=%d, mn_as_id=%d", pointer->as_id,mn_as_id); 
...
VOID_FUNC MN_handle_cm_recall_rsp(IMH_T *message_header)
{
...
#endif  
  {
    ...
    if (send_error)
    {
      ...
      {
        MSG_ERROR_DS_2(MN_SUB,"=MNCNM= [ccbs]Recall res err %d,%d",cid,resp);
    ...
    {
      MSG_ERROR_DS_2(MN_SUB,"=MNCNM= Received Invalid as_id=%d, mn_as_id=%d", msg->as_id,mn_as_id); 
    }
#endif
  }
}
...
VOID_FUNC MN_handle_cm_transfer_srvcc_call_context_res(IMH_T *message_header)
{
  ...
  byte mt_pre_alerting_call_present = FALSE;  /* This variable is used to check whether the call in U6 state or not */
  byte mt_pre_alerting_call_tid = MN_INVALID_CALL_SESSION;
  
  byte call_transfer_success =  TRUE; 
  boolean send_mt_setup_res = FALSE;
...
#endif
   {
      if((message->num_calls > 0) && (message->num_calls <= MAX_NUM_OF_SRVCC_CALLS)) /* Validate the # of calls being transferred in SRVCC HO */
      {
         ...
         for(local_call_cntr = 0; ( (local_call_cntr < message->num_calls) && (call_transfer_success == TRUE)); local_call_cntr++) 
         {
             ...
             if( MN_reserve_call_object(local_conn_id) == TRUE)
             {
                ...
                if((local_conn_id > UNKNOWN_CONNECTION_ID) && (local_conn_id < MAXNO_CC_TRANSACTIONS)) /* Process calls with MT TIDs */
                {
					...
					/* check that the current MO call is in U6 state */
                    else if( cc_transaction_information_p[local_conn_id]->cc_state == CC_STATE_U6 ) 
                    {
                      mt_pre_alerting_call_present = TRUE;
                      mt_pre_alerting_call_tid = local_conn_id;
                    } 
                    else
                    {
                ...
                if(call_transfer_success == TRUE)
                {
				   ...
				   if( (mt_pre_alerting_call_present == TRUE) && (mt_alerting_call_state == CC_STATE_U7) )
                   {
                      MSG_ERROR_DS_0(MN_SUB,"=MNCNM= Error17 MT pre alerting  and MT alerting calls present");
                      call_transfer_success = FALSE;
                   }
         ...
         else
         {
            ...
            else if(mt_pre_alerting_call_present == TRUE)
            {
              send_mt_setup_res = TRUE;
            }
      ...
      if(send_mt_setup_res == TRUE)
      {
         MN_send_setup_resp_after_HO_complete(mt_pre_alerting_call_tid, message->as_id);
         send_mt_setup_res = FALSE;
      }
   ...
   else
   {
      ...
      MN_put_cm_srvcc_ho_complete_ind(FALSE
#ifdef FEATURE_DUAL_SIM
	        ,message->as_id
#endif
     );
...

{
   ...
   if ((speech == TRUE) 
        && (pointer->connection_id < MAXNO_CC_TRANSACTIONS)
        && (cc_transaction_information_p[pointer->connection_id] != NULL)
...

{
   ...
   if((pointer->connection_id < MAXNO_CC_TRANSACTIONS) && mn_call_information_p[pointer->connection_id] != NULL)    
   {
