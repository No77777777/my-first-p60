...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/mn/src/mn_put_cnm_cc_msgs.c#1 $    $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...
extern sys_modem_as_id_e_type get_as_id(void);
...

{
   ...
   byte                    mn_gsm_wcdma_amr_codec_preference = get_gsm_wcdma_codec_pref_from_mm();
   byte                    mn_nv_nas_rel_compliance;
   ...
   else
   {
      ...
      mn_nv_nas_rel_compliance = mm_get_nv_nas_rel_compliance_per_subs(
#ifdef FEATURE_DUAL_SIM
  message_ptr->as_id
#endif
);
...
MSG_HIGH_DS_1(MN_SUB,"=MNCNM= =MM= als_itc_aux_speech while forming CALL CONF : %d ",mn_get_als_itc_aux_speech());
      ...
      if( ( (local_nv_mscr == NV_FORCE_UE_REL99) ||
            ...
            (local_mscr ))) && 
            (mn_nv_nas_rel_compliance >= NV_NAS_COMPLIANCE_REL5) ) 
      {
...
/* Encode WCDMA AMR WB Codec based on NV Bit  */
            if( (mn_gsm_wcdma_amr_codec_preference & 0x08) ) 
            {
...
/* GSM AMR codecs should be included conditionally depending on the 
    NV Item - NV_GSM_AMR_CALL_CONFIG_I */         
            ...
            if( (mn_gsm_wcdma_amr_codec_preference & 0x01) )
            {
              /*GSM AMR NB is supported*/
              message_ptr->data[data_index] |= 0x08; /* bitmap 8...1 */
            }
            if( (mn_gsm_wcdma_amr_codec_preference & 0x04) )
            {
              /*GSM AMR NB is supported*/
              message_ptr->data[data_index] |= 0x10; /* bitmap 8...1 */
            }
            data_index++;
            if( (mn_gsm_wcdma_amr_codec_preference & 0x02) )
            {
                                 ...
                                 ie_cm_cc_invoke_T *cm_cc_invoke)
{
    ...
    if(connection_id <  MAXNO_CC_TRANSACTIONS)
    {
       ...
       if((mn_call_information_p[connection_id]->user_data_info.present) &&
          (mn_call_information_p[connection_id]->user_data_info.user_user_type <= (byte)UUS1_NOT_REQUIRED))
       {
         
          byte   ie_user_data_length;
          ie_user_data_length =  mn_call_information_p[connection_id]->user_data_info.user_user_data_length + 1;  /* including protocol discriminator */
          /* To ensure that memory overflow doesn't occur */
          if((byte)(ie_user_data_length - 1) > MIN((sizeof(mn_call_information_p[connection_id]->user_data_info.user_user_data)),(sizeof(message_ptr.data) - (index + 3))))
          {
            MSG_ERROR_DS_0(MN_SUB,"=MNCNM= Index out of bound and buffer overflow");
          }
          else
          {
            ...
            message_ptr.data[index++] = mn_call_information_p[connection_id]->user_data_info.user_user_protocol_disc;
            memscpy(&message_ptr.data[index], sizeof(message_ptr.data)-index ,mn_call_information_p[connection_id]->user_data_info.user_user_data, ie_user_data_length -1);  /* user-user data length */
...
VOID_FUNC MN_put_MNCC_SETUP_REQ(connection_id_T connection_id)
{
  ...
  message_ptr->as_id
      ...
      if( (call_data_ptr->user_data_info.present) &&
          (call_data_ptr->user_data_info.user_user_type <= (byte)UUS1_IMPLICIT) )
      {
          byte   ie_user_data_length;     
          ie_user_data_length =  call_data_ptr->user_data_info.user_user_data_length + 1;  /* including protocol discriminator */        
              
	      /* To ensure that memory overflow doesn't occur */
          if((byte)(ie_user_data_length - 1) > MIN((sizeof(call_data_ptr->user_data_info.user_user_data)),(sizeof(message_ptr->data) - (data_index + 3))))
          {
             MSG_ERROR_DS_0(MN_SUB,"=MNCNM= Index out of bound and buffer overflow");
          }
          else
          {
             ...
             message_ptr->data[data_index++] = call_data_ptr->user_data_info.user_user_protocol_disc;
    
             message_length++;
    
             memscpy(&message_ptr->data[data_index], sizeof(message_ptr->data)-data_index ,call_data_ptr->user_data_info.user_user_data, ie_user_data_length -1);  /* user-user data length */
    
             data_index += ie_user_data_length -1;
    
             message_length +=   ie_user_data_length -1;
          }
          call_data_ptr->user_data_info.present = FALSE;     /* check this*/
...
MSG_HIGH_DS_1(MN_SUB,"=MNCNM= =MM= als_itc_aux_speech while forming SETUP : %d",mn_get_als_itc_aux_speech());
