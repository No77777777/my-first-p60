...
<$Header: //components/rel/mmcp.mpss/7.9.0/nas/mn/src/mn_timers.c#1 $    $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $>
                     ...
                     byte            sms_timer_id)
{
   ...
   connection_id &= 0x0f;
   as_id = sm_rl_transaction_information[connection_id].as_id;
   ...
   if (sms_timer_id == TR1M)
   {
      ...
      if((as_id  > SYS_MODEM_AS_ID_NONE) && (as_id  < MAX_AS_IDS))
	  {
        ...
        {
          MSG_HIGH_DS_2(MN_SUB," =MNCNM= Received timer f value from WMS %d on as_id = %d" ,sip_config_timer_f_sim[as_id],as_id);
	    }
        num_milli_seconds = ((mo_sms_transport_type == NAS_MSG_TRANSPORT_TYPE_SMS_OVER_IMS) ? (MAX(sip_config_timer_f_sim[as_id],SMS_TR1M_TIMEOUT_VALUE )) : SMS_TR1M_TIMEOUT_VALUE );
	  }
#else 
      if(mo_sms_transport_type == NAS_MSG_TRANSPORT_TYPE_SMS_OVER_IMS)
      {		  
        MSG_HIGH_DS_1(MN_SUB," =MNCNM= Received timer f value from WMS %d " ,sip_config_timer_f);	
	  }
...
VOID_FUNC MN_handle_timeouts(IMH_T *message)
{
   ...
   if(message_ptr->IMH.message_id == MID_TIMER_EXPIRY)
   {
      ...
      else if (message_ptr->timer_id == (MN_SMS_TIMER + TRAM_TIMER_OFFSET))  /* TRAM */
      {
         ...
         return_data = MN_control_SMS_RL(message_ptr->data, sms_TRAM_timer_expiry, NAS_REGULAR_SMS);
      ...
      else if (message_ptr->timer_id == (MN_SMS_TIMER + TR1M_TIMER_OFFSET) )
      {
         ...
         /*Use message type as regular SMS. WMS will use the actual message type. */
         return_data = MN_control_SMS_RL(message_ptr->data, sms_TR1M_timer_expiry, NAS_REGULAR_SMS);
      ...
      else if (message_ptr->timer_id == (MN_SMS_TIMER + TR2M_TIMER_OFFSET) )
      {
          ...
          /*Use deafult message type as regular SMS*/
          return_data = MN_control_SMS_RL(message_ptr->data, sms_TR2M_timer_expiry, NAS_REGULAR_SMS);
