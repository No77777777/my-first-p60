...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/mn/src/mnutils.c#1 $    $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...
extern ULogHandle mncnm_get_ulog_handle(void);
...
boolean mn_log_cs_call_setup_info(uint8 connection_id, uint8 call_type, uint8 orig_direction)
{
   ...
   if ( (MNCNM_IS_SINGLE_STACK) && mm_dual_standby_mode )
   {
     ...
     ds_log_ptr = (LOG_UMTS_DS_CS_CALL_SETUP_INFO_LOG_PACKET_C_type *)
                nas_log_alloc(LOG_UMTS_DS_CS_CALL_SETUP_INFO_LOG_PACKET_C,
     ...
     ds_log_ptr->as_id = (uint8) (mn_as_id + 1);
     ...
     log_util_log_packet_commit((sys_modem_as_id_e_type)mn_as_id, ds_log_ptr);
...
#endif
   {
     ...
     call_info_pkt = (LOG_UMTS_CS_CALL_SETUP_INFO_LOG_PACKET_C_type *)
                      nas_log_alloc(LOG_UMTS_CS_CALL_SETUP_INFO_LOG_PACKET_C, pkt_size,
                                        mn_message_as_id);
	 ...
	 log_util_log_packet_commit((sys_modem_as_id_e_type)SYS_MODEM_AS_ID_NONE, call_info_pkt);
   }
   ...
   pkt_size = 0;
     ...
     if((MNCNM_IS_SINGLE_STACK) && mm_dual_standby_mode)
     {
       ...
       ds_log_ptr = (LOG_UMTS_DS_CS_CALL_RELEASE_INFO_LOG_PACKET_C_type *)
                nas_log_alloc(LOG_UMTS_DS_CS_CALL_RELEASE_INFO_LOG_PACKET_C,
     ...
     call_info_pkt = (LOG_UMTS_CS_CALL_RELEASE_INFO_LOG_PACKET_C_type *)
                    nas_log_alloc(LOG_UMTS_CS_CALL_RELEASE_INFO_LOG_PACKET_C, pkt_size,
                    mn_message_as_id);
   ...
   pkt_size = 0;
     ...
     if((MNCNM_IS_SINGLE_STACK) && mm_dual_standby_mode)
     {
       ...
       ds_log_ptr = (LOG_UMTS_DS_CS_CALL_CHANGE_INFO_LOG_PACKET_C_type *)
                nas_log_alloc(LOG_UMTS_DS_CS_CALL_CHANGE_INFO_LOG_PACKET_C,
     ...
     call_info_pkt = (LOG_UMTS_CS_CALL_CHANGE_INFO_LOG_PACKET_C_type *)
                    nas_log_alloc(LOG_UMTS_CS_CALL_CHANGE_INFO_LOG_PACKET_C, pkt_size,
                    mn_message_as_id);
...
boolean mn_log_cs_connection_bc_info(uint8 connection_id, cm_bearer_capability_T bearer_capability)
{
  ...
  if((MNCNM_IS_SINGLE_STACK) && mm_dual_standby_mode)
  {
   ...
   ds_log_ptr = (LOG_UMTS_DS_NAS_CS_CONNECTION_BC_LOG_PACKET_C_type *)
              nas_log_alloc(LOG_UMTS_DS_NAS_CS_CONNECTION_BC_LOG_PACKET_C,
    ...
    bc_pkt = (LOG_UMTS_NAS_CS_CONNECTION_BC_LOG_PACKET_C_type *)
                    nas_log_alloc(LOG_UMTS_NAS_CS_CONNECTION_BC_LOG_PACKET_C, pkt_size,
                    mn_message_as_id);
  ...
  //  bc_pkt->speech_version_indication = bearer_capability.speech_version_indication;
	...
	log_util_log_packet_commit((sys_modem_as_id_e_type)SYS_MODEM_AS_ID_NONE, bc_pkt);
    }
...
VOID_FUNC mn_log_message(gs_queue_id_T queue_id, IMH_T *message_p)
{
   ...
   */ 
   ...
   if (mncnm_get_ulog_handle() != NULL)
   {
#ifdef FEATURE_DUAL_SIM
     ULOG_RT_PRINTF_4(mncnm_get_ulog_handle(),
              "DS: SUB %d =MNCNM= Message Set: %d Message ID: %d msg_info: %d",
              get_as_id() + 1,
              message_p->message_set,
              message_p->message_id,
              mncnm_msg_info);
#else
     ULOG_RT_PRINTF_3(mncnm_get_ulog_handle(),
              "=MNCNM= Message Set: %d Message ID: %d msg_info: %d",
              message_p->message_set,
              message_p->message_id,
              mncnm_msg_info);
#endif
     ...
     /*  SEE IF ACTIVE CALL INVOLVES DATA OR FAX  */
        for(index=0; index<3; index++)
        {
           ...
           if((Multicall_Status[index].call_activity > CALL_INACTIVE) &&
               (cur_connection_id < MAXNO_CC_TRANSACTIONS)&&                       
...
nas_vote_for_reselection_e_type mn_voice_call_vote_for_reselection(sys_modem_as_id_e_type as_id)
{
   ...
   if(MNCNM_IS_SINGLE_STACK && (as_id != mn_as_id))
   {
     ...
     MSG_HIGH_DS_2(MN_SUB, "mn_voice_call_vote_for_reselection votes %d, CNM single stack and as_id(%d) != mn_as_id)",NAS_VOTE_FOR_RESEL_TRUE,as_id);
   ...
   {
      MSG_HIGH_DS_2(MN_SUB, "mn_voice_call_vote_for_reselection votes %d, DUAL_ACTIVE enabled and as_id = %d, out of bounds",NAS_VOTE_FOR_RESEL_TRUE,as_id);
   ...
   for(index = 0; (index < MAXNO_CC_TRANSACTIONS)&& (proc_finish == FALSE); index++)
   {
         ...
         mn_call_information_p_sim[as_id][index] != NULL)  
     {
       ...
       {
         MSG_HIGH_DS_2(MN_SUB, "mn_voice_call_vote_for_reselection =%d, call state = %d",NAS_VOTE_FOR_RESEL_FALSE,cc_transaction_information_p_sim[as_id][index]->cc_state);
   ...
   for(index = 0; (index < MAXNO_CC_TRANSACTIONS)&& (proc_finish == FALSE); index++)
   {
         ...
         mn_call_information_p[index] != NULL)  
     {
       if( (CC_STATE_U10 != cc_transaction_information_p[index]->cc_state) &&
           (tch_user_connections_store[DEFAULT_RAB_ID].OTA_channel_connected != TRUE))
       {
         MSG_HIGH_DS_2(MN_SUB, "mn_voice_call_vote_for_reselection =%d, call state = %d",NAS_VOTE_FOR_RESEL_FALSE,cc_transaction_information_p[index]->cc_state); 
         ret_val = NAS_VOTE_FOR_RESEL_FALSE;
         proc_finish = TRUE;
       }
     }
   ...
   MSG_HIGH_DS_2(MN_SUB, "mn_voice_call_vote_for_reselection votes %d and as_id = %d",ret_val,as_id);
   return ret_val;
  
}

byte get_gsm_wcdma_codec_pref_from_mm(void)
{
   sys_modem_as_id_e_type local_as_id = SYS_MODEM_AS_ID_NONE;
#ifdef FEATURE_DUAL_SIM
   if(MNCNM_IS_SINGLE_STACK)
...
#endif
#endif
   return mm_get_gsm_wcdma_amr_codec_per_subs(
#ifdef FEATURE_DUAL_SIM
   local_as_id
#endif
   );
...
mn_rrcmn_rab_status_debug_e_type mn_rab_status_get( mn_dsda_as_id_e_type as_id )
{
#if defined(FEATURE_DUAL_SIM) && defined(FEATURE_DUAL_ACTIVE)
  return (mn_rrcmn_rab_status_debug_e_type)mn_rab_status_sim[as_id];
#else
  return (mn_rrcmn_rab_status_debug_e_type)mn_rab_status;
#endif
}


#ifdef FEATURE_NAS_VOC_REL_AFTER_CALL_HOLD
boolean MN_turned_on_vocoder_get( mn_dsda_as_id_e_type as_id )
{
#if defined(FEATURE_DUAL_SIM) && defined(FEATURE_DUAL_ACTIVE)
  return MN_turned_on_vocoder_sim[as_id];
#else
  return MN_turned_on_vocoder;
#endif
}
#endif
#endif
