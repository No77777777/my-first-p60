...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/mn/src/mn_route_sms_msgs.c#1 $    $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...

{
...
#endif
         {
           ...
           else /* abort is set to TRUE */
           {
               for (connection_id = 0; connection_id < MAXNO_SMS_TRANSACTIONS; connection_id++)
               {
                   ...
                   {
                       smr_return_data = MN_control_SMS_RL(connection_id, s_mo_smma_abort_req, NAS_REGULAR_SMS);
   ...
   case UASMS_ABORT_SM_REQ:
      {
...
#endif
         {
           ...
           for (connection_id = 0; connection_id < MAXNO_SMS_TRANSACTIONS; connection_id++)
           {
               smr_return_data = MN_control_SMS_RL(connection_id, s_mo_abort_req, NAS_REGULAR_SMS);
   ...
   case UASMS_SM_LINK_CONTROL_REQ:
      ...
      case UASMS_SM_UPDATE_TIMERF:
      {
		...
		as_id  = SM_RL_UPDATE_TIMERF->as_id;
	    ...
	    {  
		  sip_config_timer_f_sim[as_id] = (SM_RL_UPDATE_TIMERF -> timerf_value)+1000;	//To avoid race conditions, increase timerf vlaue by 1sec
		  MSG_HIGH_DS_2(MN_SUB," =MNCNM= Received timer f value from WMS %d on as_id = %d" ,sip_config_timer_f_sim[as_id],as_id);
		}
#else
		  sip_config_timer_f = (SM_RL_UPDATE_TIMERF -> timerf_value)+1000;
