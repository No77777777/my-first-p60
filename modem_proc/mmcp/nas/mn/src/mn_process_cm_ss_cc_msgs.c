...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/mn/src/mn_process_cm_ss_cc_msgs.c#1 $    $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...
VOID_FUNC   MN_process_cm_2_send(cm_n_send_req_T * message)
{
   ...
   for(index = 0; index < MULTICALL_STATUS_SIZE; index++)
   {
      ...
      if ((connection_id < MAXNO_CC_TRANSACTIONS) && 
          ...
          mn_call_information_p[connection_id] != NULL)  /* reserved connection ID = emergency call */ 
      {
...

{
  ...
  {
    MSG_ERROR_DS_2(MN_SUB,"=MNCNM= Received Invalid as_id=%d, mn_as_id=%d", user_data_req->as_id,mn_as_id); 
  }
#endif
#endif
}
...
byte MN_put_USER_USER_DATA_REQ(connection_id_T connection_id)
{
    ...
    if(Multicall_Status_index >= MULTICALL_STATUS_SIZE)
    {
       ...
       (mn_call_information_p[connection_id]->user_data_info.user_user_type > (byte)UUS1_REQUIRED) &&
       (mn_call_information_p[connection_id]->user_data_info.activated != FALSE))
    {
