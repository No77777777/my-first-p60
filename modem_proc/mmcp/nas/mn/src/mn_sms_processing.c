...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/mn/src/mn_sms_processing.c#1 $    $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
            ...
            case s_mo_sms_req:
               {
                  ...
                  {
                    MN_put_MNIMS_MO_RPDU(sms_msg_type);
...
{
   boolean         success;
   connection_id_T connection_id = 0, counter = 1;
...
MN_get_rp_report(SM_RL_REPORT_REQ_T *SM_RL_REPORT_REQ)
{
   ...
   return_data = MN_control_SMS_RL(SM_RL_REPORT_REQ->connection_id,
       s_report_req, SM_RL_REPORT_REQ->sms_msg_type);
...
boolean MN_validate_sms_as_id(sys_modem_as_id_e_type as_id, word short_message_identifier, boolean send_rsp)
{
  ...
  {
    success = FALSE;
    MSG_ERROR_DS_2(MN_SUB,"=MNCNM= Received Invalid as_id=%d, mn_as_id=%d", as_id,mn_as_id); 
