...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/mn/src/mn_process_cm_ss_msgs.c#1 $    $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...
#define NO_REPLY_TIMER_MIN       5
#define NO_REPLY_TIMER_CS_MAX   30
#define NO_REPLY_TIMER_PS_MAX   30
...
VOID_FUNC MN_handle_cm_release_ss_req(cm_release_ss_req_T *message)
{
   ...
   byte                   *last_octet_component_pointer = NULL;
   int                    size = 0;
   byte                   invoke_id;
   byte                   flag=FALSE;
   ...
   if(message != NULL)
   {
...
#endif
      {
         ...
         else
         {
          ...
          if((message->cm_ss_error.present) && (flag == TRUE))
          {

            invoke_id = message->cm_ss_ref;
...

            {
...

               {
                  ...
                  MN_put_error(message->cm_ss_error,
                               invoke_id,
         ...
         if(flag==TRUE)
         {
           ...
           if(message->cm_ss_error.present)
           {
           ...
           else
           {
             ...
             if(MNCNM_IS_SINGLE_STACK)
             {
               ...
               mn_message_as_id = (byte)(mn_as_id + 1);
                MSG_HIGH_DS_1(MN_SUB,"=MNCNM= =MN= set mn_as_id=%d",mn_as_id); 
             }
#endif
           for (invoke_id = 0; invoke_id < MAXNO_INVOKE_IDS; invoke_id++)
           {
...
MN_handle_cm_activate_ss_req(cm_activate_ss_req_T *message_ptr)
{
   ...
   ss_ForBS_T             ss_ForBS;
   int                    size;
   byte mn_nv_nas_rel_compliance = mm_get_nv_nas_rel_compliance_per_subs(
...
#endif
   {
    ...
    else
    {
      ...
      if ((argument = (byte *)modem_mem_calloc(1, 10, MODEM_MEM_CLIENT_NAS)) !=NULL)
      {
         ...
         if((mn_nv_nas_rel_compliance >= NV_NAS_COMPLIANCE_REL7 && mn_nv_nas_rel_compliance < NV_NAS_COMPLIANCE_REL_MAX) &&
             long_ftn_support)
         {
...
VOID_FUNC MN_handle_cm_register_ss_req(cm_register_ss_req_T *message_ptr)
{
...
#endif
  {
    ...
    /* validating the no_reply_condition_timer value...user can give this value inbetween 5 and 180..So we are rounding off the user given value to muliples of 5  
	   As per the IMS spec, the valid range of no_reply_timer is 5 and 180 where as for CS domain, it is 5 and 30. So to accomidate both, we are rounding off the user given value.
       Suppose if the range is inbetween 30(NO_REPLY_TIMER_CS_MAX) and 180(NO_REPLY_TIMER_PS_MAX), round off to 30.if the timer value is inbetween 5(NO_REPLY_TIMER_MIN), round off to nearest value which is multiple of 5...*/

    /* send an error to upperlayers if validation fails*/
    if (message_ptr->cm_no_reply_condition_timer.present == TRUE)
    {
      byte timer_value= message_ptr->cm_no_reply_condition_timer.timer_value;

      if((timer_value >= NO_REPLY_TIMER_MIN) && (timer_value <= NO_REPLY_TIMER_PS_MAX))
      {
        if(timer_value >NO_REPLY_TIMER_CS_MAX && timer_value <= NO_REPLY_TIMER_PS_MAX)
        {
          message_ptr->cm_no_reply_condition_timer.timer_value  = NO_REPLY_TIMER_CS_MAX;
        }

        else if((timer_value >= NO_REPLY_TIMER_MIN) && (timer_value <= NO_REPLY_TIMER_CS_MAX))
        {
          if((timer_value % NO_REPLY_TIMER_MIN) > 2)
          {
             message_ptr->cm_no_reply_condition_timer.timer_value= (NO_REPLY_TIMER_MIN*(timer_value / NO_REPLY_TIMER_MIN))+ NO_REPLY_TIMER_MIN;
          }
 
          if((timer_value % NO_REPLY_TIMER_MIN) <= 2)
          {
             message_ptr->cm_no_reply_condition_timer.timer_value= (NO_REPLY_TIMER_MIN*(timer_value / NO_REPLY_TIMER_MIN));
          }
        }
      }
      else
      ...
      if ( facility != NULL )
      {
        ...
        if ( registerSS_ARG != NULL )
        {
           ...
           else
           {
              ...
              {
                 size = MN_put_registerSS_ARG(argument, *registerSS_ARG,message_ptr->as_id);
