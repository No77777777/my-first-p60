...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/mn/src/mn_process_smrl_msgs.c#1 $    $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...
extern VOID_FUNC MN_put_MNIMS_MO_RPDU(nas_msg_e_type sms_msg_type);
...

{
   ...
   if(success == TRUE)
   {
      ...
      if (connection_id <=  MAX_MO_CONNECTION_ID) /* error - no conn_ids available */
      {
         ...
         return_data = MN_control_SMS_RL( connection_id, s_mo_sms_req, message_ptr->sms_msg_type );
      ...
      */
...
#endif
      return_data = MN_control_SMS_RL( connection_id, s_mo_smma_req, NAS_REGULAR_SMS );
      ...
      if (return_data.success == TRUE)
      {
         ...
         if(NAS_MSG_TRANSPORT_TYPE_SMS_OVER_IMS == mo_sms_transport_type)
         {
           ...
           MN_put_MNIMS_MO_RPDU(NAS_REGULAR_SMS);
