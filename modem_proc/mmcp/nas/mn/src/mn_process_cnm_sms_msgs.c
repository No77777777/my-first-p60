...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/mn/src/mn_process_cnm_sms_msgs.c#1 $    $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...
MN_handle_MNSMS_DATA_IND(IMH_T *message_header)
{
   ...
   //finding message length based on CNM size calculation 
   message_length = message_length + sizeof(rpdu_T) + sizeof(IMH_T) - sizeof(MNSMS_DATA_IND_T);
   //adding connection_id and as_id to be in sync with MN_get_cp_user_data function
   message_length = message_length + sizeof(connection_id_T) + sizeof(sys_modem_as_id_e_type);
...
MN_handle_MNSMS_ERROR_IND(IMH_T *message_header)
{
   ...
   /*Cast message type to regular. WMS will use the actual message type from its cache.*/
   return_data = MN_control_SMS_RL(message_ptr->connection_id,
       s_cnm_error, NAS_REGULAR_SMS);
       ...
       (return_data.sm_rl_report == TRUE))
   {
      ...
      SM_RL_REPORT_IND.ota_cause.ota_reject_cause = message_ptr->ota_reject.reject_cause;
      SM_RL_REPORT_IND.ota_cause.ota_reject_type  = message_ptr->ota_reject.reject_type;
      ...
      MSG_HIGH_DS_4(MN_SUB,"=MNCNM= Retry Interval for SMS, present = %d, value = %d , lower_layer_ota_reject_cause = %d , ota_reject_type = %d",
                                   SM_RL_REPORT_IND.retry_interval.present, SM_RL_REPORT_IND.retry_interval.value,
                                   SM_RL_REPORT_IND.ota_cause.ota_reject_cause,SM_RL_REPORT_IND.ota_cause.ota_reject_type);
...
MN_handle_MNSMS_EST_IND(IMH_T *message_header)
{
   ...
   //finding message length based on CNM size calculation
   message_length = message_length  + sizeof(IMH_T)  + sizeof(rpdu_T) - sizeof(MNSMS_EST_IND_T);
   //adding connection_id and as_id to be in sync with MN_get_cp_user_data function
   message_length = message_length + sizeof(connection_id_T) + sizeof(sys_modem_as_id_e_type);
...
MN_handle_MNSMS_CPACK_IND(IMH_T *message_header)
{
   ...
   SM_RL_REPORT_IND.ota_cause.ota_reject_cause = 0;
   SM_RL_REPORT_IND.ota_cause.ota_reject_type  = 0;
   
   /* Cast message header to message type */
   message_ptr = (MNSMS_CPACK_IND_T *)message_header;

   connection_id = message_ptr->connection_id;
   return_data = MN_control_SMS_RL(connection_id, s_cnm_ack, NAS_REGULAR_SMS);
   if(return_data.success == TRUE)
   {
