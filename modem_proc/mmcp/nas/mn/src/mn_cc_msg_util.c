...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/mn/src/mn_cc_msg_util.c#1 $    $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...
cm_bearer_capability_T *cm_bearer_capability, connection_id_T conn_id, byte *last_octet_data_ptr)
{
   ...
   {
     MSG_ERROR_DS_1(MN_SUB,"=MNCNM= Invalid IEI=%d",*data_ptr);
   }
   /* add IEI length */
   return (index + 1);
}
