...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/mn/src/mn_cc_processing.c#1 $    $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...
/* Read the Speech Versions Set into the Bitmap */
  ...
  MSG_HIGH_DS_1(MN_SUB,"=MNCNM= NV Settings gsm_wcdma_amr_codec_preference = %d",mn_gsm_wcdma_amr_codec_preference);
...
/* ---------------------------------------------------------------*/
  ...
  MSG_HIGH_DS_1(MN_SUB,"=MNCNM= gsm_wcdma_amr_codec_preference is %d",mn_gsm_wcdma_amr_codec_preference);  

  if((mn_gsm_wcdma_amr_codec_preference & 0x01) == 0) 
  {
      for(index = 0; index < MAX_NUMBER_GSM_SPEECH_CODECS; index++)
      {
  ...
  if((mn_gsm_wcdma_amr_codec_preference & 0x02) == 0)
  {
      for(index = 0; index < MAX_NUMBER_GSM_SPEECH_CODECS; index++)
      {
        if(copy_speech_codec[index] == GSM_FR_SPEECH_VER_5) /* codec with value 8 should not be published*/
  ...
  if((mn_gsm_wcdma_amr_codec_preference & 0x04) == 0)
  {
      for(index = 0; index < MAX_NUMBER_GSM_SPEECH_CODECS; index++)
      {
