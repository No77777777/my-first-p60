...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/services/inc/naslog_v.h#1 $ $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...
{
  byte length;
  sys_plmn_id_s_type plmn[LOG_LTE_RRC_MAX_PLMN_LIST_SIZE];
}lte_nas_plmn_lst_log_type;

typedef PACK(struct)
{
  boolean new_scan;
  boolean use_timer;
}lte_nas_eoos_gwl_scan_log_type;
...
typedef PACK(struct)
{
  ...
  uint64 bits_129_192;
  uint64 bits_193_256;
}lte_nas_bit_mask_256_log_type;

typedef PACK(struct)
{
  uint64  chgwt_band_cap;
  lte_nas_bit_mask_256_log_type lte_band_cap;
}lte_nas_band_cap_u_log_type;

typedef PACK(struct)
{
  byte acq_sys_mode;
  lte_nas_band_cap_u_log_type band_cap;
  boolean bst_rat_acq_required;
  lte_nas_band_cap_u_log_type bst_band_cap;
  uint32 acq_sys_time_interval;
}lte_nas_priority_list_info_log_type;

typedef PACK(struct)
{
  uint32 num_items;
  uint32 next_acq_sys_index;
  lte_nas_priority_list_info_log_type  priority_list_info[LOG_SYS_MAX_ACQ_PREF_RAT_LIST_NUM];
  lte_nas_eoos_gwl_scan_log_type scan_type;
}lte_nas_rat_pri_list_info_log_type;
...
typedef PACK(struct)
{
  ...
  lte_nas_plmn_lst_log_type ehplmn_list;
  lte_nas_tai_lst2_log_type forbidden_ta_list;
  lte_nas_tai_lst2_log_type forbidden_manual_ta_list;
  lte_nas_plmn_lst_log_type eplmn_list;
  boolean scan_is_new;
  boolean use_timer;
  uint32 lte_scan_time;
  byte req_type;
  uint32 csg_id;
  uint32 t_timeout;
  ...
  lte_nas_rat_pri_list_info_log_type rat_pri_list;
}lte_nas_emm_rrc_sevice_request_type;
