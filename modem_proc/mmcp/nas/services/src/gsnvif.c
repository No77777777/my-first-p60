...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/services/src/gsnvif.c#1 $    $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
  ...
  if (cached_slot_id_valid_imei_sim[as_id] == TRUE)
  {
    ...
    if(cached_imei_mapping[as_id] != context_id)
    {
      ...
      for (as_id_local = MM_AS_ID_1 ; as_id_local < (mm_as_id_e_type)MAX_AS_IDS; as_id_local++)
      {
         ...
         {
            MSG_HIGH_DS_3(as_id+1,"=IMEI= Mapping Changed between contexts %d, %d, for context %d",as_id_local,as_id,context_id);
...
#ifdef FEATURE_DUAL_SIM
static boolean cached_hw_classmark_valid_ds[MAX_AS_IDS] = {FALSE,FALSE};

#endif
static boolean cached_hw_classmark_valid = FALSE;

#ifdef FEATURE_DUAL_SIM
ghdi_status_T ghdi_ds_read_hw_classmark(hw_classmark_T *hw_classmark,sys_modem_as_id_e_type as_id)
{
  /* Number of times NVMEM is accessed before cached values are hardcoded to avoid NV read */
  #define NV_READ_RETRIES 3

  static hw_classmark_T cached_hw_classmark_ds_sim[MAX_AS_IDS];
  #define cached_hw_classmark_ds cached_hw_classmark_ds_sim[as_id]
  static byte failure_count_ds_sim[MAX_AS_IDS] = {0,0};
  #define failure_count_ds failure_count_ds_sim[as_id]
  /*-------------------------------------------------------------------
  ...
  **-----------------------------------------------------------------*/
  ...
  rex_enter_crit_sect(&nas_nv_crit_sect);
  is_cached = cached_hw_classmark_valid_ds[as_id];
  rex_leave_crit_sect(&nas_nv_crit_sect);
  NAS_ASSERT_UT(hw_classmark != NULL);

  if (is_cached == FALSE)
  {
    result = ghdi_nvmem_read_per_subs((ghdi_nvmem_parameter_id_T) NV_GSM_A5_ALGORITHMS_SUPPORTED_I,&itemBuffer,as_id);
    ...
    if (result != GHDI_SUCCESS)
    {
      ...
      hw_classmark->A5_4_supported = TRUE;
      ...
      if (failure_count_ds++ > NV_READ_RETRIES)
      {
        // store the hardcoded value as cached value
        rex_enter_crit_sect(&nas_nv_crit_sect);
        if (cached_hw_classmark_valid_ds[as_id] == FALSE)
        {
          cached_hw_classmark_ds = (*hw_classmark);
          // now the cache should be used
          cached_hw_classmark_valid_ds[as_id] = TRUE;
        }
        // otherwise, someone else cached ahead, just use cached value below
        rex_leave_crit_sect(&nas_nv_crit_sect);
      }
    ...
    {
      /* copy whatever is in the item to the cache */
      cached_hw_classmark_ds.A5_1_supported = IS_ALGO_SUPPORTED(itemBuffer.gsm_a5_algorithms_supported & A5_1_SUPPORTED_MASK);
      cached_hw_classmark_ds.A5_2_supported = IS_ALGO_SUPPORTED(itemBuffer.gsm_a5_algorithms_supported & A5_2_SUPPORTED_MASK);
      cached_hw_classmark_ds.A5_3_supported = IS_ALGO_SUPPORTED(itemBuffer.gsm_a5_algorithms_supported & A5_3_SUPPORTED_MASK);
      cached_hw_classmark_ds.A5_4_supported = IS_ALGO_SUPPORTED(itemBuffer.gsm_a5_algorithms_supported & A5_4_SUPPORTED_MASK);
      cached_hw_classmark_ds.A5_5_supported = IS_ALGO_SUPPORTED(itemBuffer.gsm_a5_algorithms_supported & A5_5_SUPPORTED_MASK);
      cached_hw_classmark_ds.A5_6_supported = IS_ALGO_SUPPORTED(itemBuffer.gsm_a5_algorithms_supported & A5_6_SUPPORTED_MASK);
      cached_hw_classmark_ds.A5_7_supported = IS_ALGO_SUPPORTED(itemBuffer.gsm_a5_algorithms_supported & A5_7_SUPPORTED_MASK);

      /* the other values remain hard coded for now */
      cached_hw_classmark_ds.freq_capability = 0;
      cached_hw_classmark_ds.default_rf_power_capability = (byte) CLASS_4;
      cached_hw_classmark_ds.current_rf_power_capability = (byte) CLASS_4;
  ...
  hw_classmark->A5_1_supported = cached_hw_classmark_ds.A5_1_supported;
  hw_classmark->A5_2_supported = cached_hw_classmark_ds.A5_2_supported;
  hw_classmark->A5_3_supported = cached_hw_classmark_ds.A5_3_supported;
  hw_classmark->A5_4_supported = cached_hw_classmark_ds.A5_4_supported;
  hw_classmark->A5_5_supported = cached_hw_classmark_ds.A5_5_supported;
  hw_classmark->A5_6_supported = cached_hw_classmark_ds.A5_6_supported;
  hw_classmark->A5_7_supported = cached_hw_classmark_ds.A5_7_supported;
  hw_classmark->freq_capability = cached_hw_classmark_ds.freq_capability;
  hw_classmark->default_rf_power_capability = cached_hw_classmark_ds.default_rf_power_capability;
  hw_classmark->current_rf_power_capability = cached_hw_classmark_ds.current_rf_power_capability;
...
void ghdi_clear_hw_classmark_cache(
#ifdef FEATURE_DUAL_SIM
...
{
#ifdef FEATURE_DUAL_SIM
		cached_hw_classmark_valid_ds[as_id] = FALSE;
#endif
  cached_hw_classmark_valid = FALSE;
