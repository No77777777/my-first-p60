...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/services/src/gsclassmark.c#1 $    $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...
#ifdef FEATURE_DUAL_SIM
  status = ghdi_ds_read_hw_classmark( &hw_classmark,params_ptrs->as_id);
#else
  status = ghdi_read_hw_classmark(&hw_classmark);
#endif  
...
#ifdef FEATURE_DUAL_SIM
  status = ghdi_ds_read_hw_classmark( &hw_classmark,params_ptr->as_id);
#else
  status = ghdi_read_hw_classmark(&hw_classmark);
#endif
...
)
{
 ...
 include_rel6_IE = TRUE;
  ...
  MSG_HIGH_DS_2( MM_SUB, "=MM= DCS support %1d, PCS support %1d", (int)DCS1800_supported,(int)PCS1900_supported);
#else
  #error code not present
#endif /* FEATURE_GSM_MULTIBAND */
                               ...
                               )
{
  ...
  MSG_HIGH_DS_1(MM_LOCAL_SUB, "=MM= Class Mark Read from Nv is  : %d",gmm_gea_support_sim[as_id]);
...
{
  MSG_HIGH_DS_1(MM_LOCAL_SUB, "=MM= Class Mark Read from Nv is  : %d",gmm_gea_support);
