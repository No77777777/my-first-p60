...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/sm/src/esm_bcm.c#2 $
...
(
  ...
  ,boolean is_ps_ho  /* variable to know if PS ho is going on */
#endif
)
{
  byte index = 0;
...
#ifdef FEATURE_NAS_REL12_SPEC_CHANGE
      /* CR 2316 donnot deact ISR if bearer added after ISR and moved to LTE due to PS HO */
		&&((emm_get_mm_spec_rel_ver() < NV_NAS_COMPLIANCE_REL12)||(is_ps_ho == FALSE))
#endif		
...
#ifdef FEATURE_NAS_REL12_SPEC_CHANGE
	/* CR 2316 donnot deact ISR if bearer added after ISR and moved to LTE due to PS HO */
    else if((emm_get_mm_spec_rel_ver() >= NV_NAS_COMPLIANCE_REL12)&&
		    (mgr_ptr->esm_bc[index] != NULL)&&
		    (mgr_ptr->esm_bc[index]->isr_bearer_ctxt_act_status == CTXT_ACT_AFTER_ISR_IN_GW)&&
		    (is_ps_ho==TRUE))
    {
      mgr_ptr->esm_bc[index]->isr_bearer_ctxt_act_status = CTXT_ACT_BEFORE_ISR;
