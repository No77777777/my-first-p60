...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/sm/src/sm_multimode_handler.c#1 $
...
)
{
  ...
  if(bc_state == ESM_BC_ACTIVE)
  {
    ...
    if(pdp_list[conn_id] == NULL)
    {
      ...
      if(esm_bc->lbi >= MIN_NSAPI_NUM && esm_bc->lbi < ESM_MAX_EPS_BEARERS && pdp_list[nsapi_list[esm_bc->lbi]] == NULL)
      {
        MSG_ERROR_DS_1(SM_SUB,"Primary PDP context/ LBI for con id %d does not exist",nsapi_list[esm_bc->lbi]);        
        return INVALID_ID;
      }	      
      ...
      else
      {
        ...
        pdp_list[conn_id]->linked_ti = (byte)nsapi_list[esm_bc->lbi];
...
)
{
  ...
  sys_modem_as_id_e_type as_id = MSGR_GET_VARIANT(&msg_ptr->cmd.msg_hdr);
  MSGR_CLEAR_VARIANT(msg_ptr->cmd.msg_hdr.id);
    ...
    switch (msg_ptr->cmd.msg_hdr.id)
    {
	  ...
	  case NAS_ESM_FAILURE_IND:
           ...
           (mgr_ptr->on_attach_def_bearer_pending == ESM_BC_ON_ATTACH_DEF_BEARER_NONE))
        {
          ...
          MSG_HIGH_DS_1(SM_SUB,"sig con rel/Failure indication on wrong sub, PS active on SUB %d",sm_as_id+1);
          valid_as_id_received = TRUE;
        }
        break;
  ...
  MSG_HIGH_DS_2(SM_SUB,"sm_esm_check_if_ps_active returning %d attach def bearer status %d",
                     is_ps_active, mgr_ptr->on_attach_def_bearer_pending);
#endif
...
#endif
/*===========================================================================
FUNCTION sm_is_mode_enabled_sub
...
===========================================================================*/
boolean sm_is_mode_enabled_sub(sys_sys_mode_mask_e_type mode)
{
#ifdef FEATURE_DUAL_SIM
  uint32 sub_capability = mm_get_sub_capability_mask((sys_modem_as_id_e_type)sm_as_id);
  if(sub_capability & mode)
  {
    return TRUE;
  }
  return FALSE;
...
#if defined(FEATURE_DUAL_DATA) && !defined(FEATURE_DUAL_WCDMA)
/*===========================================================================
...
sys_modem_as_id_e_type sm_get_multimode_sub(void)
{
  ...
  {
    mode_mask = mm_get_sub_capability_mask((sys_modem_as_id_e_type)sm_mmode_as_id);
    if((mode_mask & SYS_SYS_MODE_MASK_WCDMA) || (mode_mask & SYS_SYS_MODE_MASK_TDS))
    {
