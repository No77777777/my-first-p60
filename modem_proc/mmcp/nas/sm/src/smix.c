...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/sm/src/smix.c#1 $ $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...
** DESCRIPTION  : Helper function that unpacks the Back-off timer IE
** ----------------------------------------------------------------------- */
LOCAL xlate_status_T unpack_gprs_timer_3(byte* ext_buf, uint16 *index,  
                                         pdp_context_info* pdp);
...
)
{
  ...
  qsh_signaling.protocol_discriminator = PROTOCOL_DISCRMINATOR_SM_DEBUG;
  qsh_signaling.message_type.sm_message_id = msg_type;
  qsh_signaling.direction = UE_TO_CN_DIRECTION;
  qsh_signaling.cause_value.sm_cause = SM_UNKNOWN_CAUSE;
#ifdef FEATURE_DUAL_SIM
...
#ifdef FEATURE_QSH_EVENT_METRIC
      qsh_signaling.cause_value.sm_cause = (sm_status_T)pdp->current_cause;
#endif
    break;
...
#error code not present
  ...
  {
    if(!((msg_type == ACTIVATE_PDP_REQUEST || msg_type == ACTIVATE_SEC_PDP_REQUEST) && 
         (pdp->trans_id < MAX_PDP_NUM) && 
...
)
{
  ...
  ** --------------------------------------------------------------*/
  ...
  {
    /*24.008 10.5.6.17*/
    ext_buf[msg_length++] = (IEI_PDP_REQUEST_TYPE | (byte)(pdp->request_type.req_type));
  }
...

{
  ...
  gmm_max_length = gmm_max_L3_msg_length(MM_PS_DOMAIN);
#ifdef FEATURE_REL5
  if ((ue_revision == UE_REV_REL99) && (sm_get_nv_nas_rel_compliance() >= NV_NAS_COMPLIANCE_REL5))
  {
  // Check if Protocol Configuration options field is included
  if (pdp->config_options.valid)
  {
...
)
{
  ...
  sm_qsh_signaling_s qsh_signaling;
  qsh_signaling.protocol_discriminator = PROTOCOL_DISCRMINATOR_SM_DEBUG;
  qsh_signaling.message_type.sm_message_id = *recd_msg;
...
#ifdef FEATURE_QSH_EVENT_METRIC
      if(pdp->current_cause != SM_UNKNOWN_CAUSE)
      {
        qsh_signaling.cause_value.sm_cause = (sm_status_T)pdp->current_cause;
      }
#endif
...
#ifdef FEATURE_QSH_EVENT_METRIC
      qsh_signaling.cause_value.sm_cause = (sm_status_T)pdp->status_message_cause;
#endif
      status = L3_PARSE_SUCCESS;     
    break;
  ...
  ** ------------------------------------------------------------------- */
   ...
   while (msg_index < msg_length)
   {
    ...
    switch (ext_buf[msg_index++])
    {
      ...
      if (((optional_ie_index <= 1)&& (iei_length > 0)) && 
            ((msg_index + iei_length) < msg_length)&&
            ((sm_get_nv_nas_rel_compliance() > NV_NAS_COMPLIANCE_REL10)
              #ifdef FEATURE_LTE
              ||(emm_3gpp_spec_version > LTE_3GPP_REL10_SEP12)
              #endif
             )            
          )
      {
  ...
  ** ------------------------------------------------------------------- */
  ...
  while (msg_index < msg_length)
  {
    ...
    switch (ext_buf[msg_index++])
    {
      ...
      if (((optional_ie_index <= 2)&& (iei_length > 0)) && 
            ((msg_index + iei_length) < msg_length)&&
            ((sm_get_nv_nas_rel_compliance() >= NV_NAS_COMPLIANCE_REL12)
              #ifdef FEATURE_LTE
              ||(emm_3gpp_spec_version >=LTE_3GPP_REL12)
              #endif
             )            
          )
      {
      ...
      ** --------------------------------------------------------------- */
      case IEI_PDP_ADDRESS:
              ...
              ((msg_index + iei_length) < msg_length))
        {
          ...
          pdp->req_pdp_addr = pdp_temp->pdp_addr;
          optional_ie_index = 1;          
        }
        else
        {
...
void pack_qos(byte* ext_buf, qos_T* qos, uint16* msg_length)
{
  ...
  {
   MSG_HIGH_DS_2(SM_SUB, "WCDMA HSUPA category NV (4210) is Set to : %d, NAS REL COMP set to : %d", wcdma_hsupa_category, (byte)nas_rel_compliance);
  ...
  if (ue_revision == UE_REV_REL99)
  {
    ...
    ADD_BYTE_LEN(cur_pos);
#ifdef FEATURE_REL5
    if (nas_rel_compliance >= NV_NAS_COMPLIANCE_REL5)
    {
      //Pack OCTET 13
...
byte unpack_qos (byte* ext_buf, pdp_context_info *pdp, uint16 *msg_length)
{
  ...
  {  
    ...
    MSG_HIGH_DS_5(SM_SUB,"N/W MBR extn donwlink %d N/W GBR extn donwlink %d N/W MBR extn uplink %d N/W GBR extn uplink %d HSUPA category NV (4210) : %d",
                          pdp->qos.ext_max_bit_rate_downlink, pdp->qos.ext_gtd_bit_rate_downlink, pdp->qos.ext_max_bit_rate_uplink, pdp->qos.ext_gtd_bit_rate_uplink, wcdma_hsupa_category);
#endif
  }
  else
  {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  ** ------------------------------------------------ */
  ...
  /* Mask the Re-Attempt Ind Value to ignore spare bits */
  ratc_value = (ext_buf[msg_index++] & 0x01);
  
  pdp->reattempt_ind.valid = TRUE;
  pdp->reattempt_ind.ratc_val = ratc_value;
...
byte unpack_req_qos (byte* ext_buf, pdp_context_info *pdp, uint16 *msg_length)
{
  ...
  if ( ( ie_length > 3 ) && ( ((cur_pos/8 + 7 ) - msg_index) <= ie_length) )
  {
    ...
    if ((ie_length > 0xB )&& ( ((cur_pos/8) - msg_index) <= ie_length) )
    {
     ...
     if(sm_get_wcdma_hsupa_category() >= MM_RRC_WCMDA_DC_HSUPA_CATEGORY)
     {
      if ((ie_length > 14) && ( ((cur_pos/8) - msg_index) <= ie_length) )
      {
        //unpack OCTET 15
