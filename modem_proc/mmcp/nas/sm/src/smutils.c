...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/sm/src/smutils.c#1 $    $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...
#include "sm_qsh_ext.h"
#endif
#include "ds3gpp_api.h"
#include "ULogFront.h"

extern ULogHandle sm_get_ulog_handle(void);
...
boolean sm_log_pdp_event(void)
{
  ...
  for (i=0; i<MAX_PDP_NUM; i++ )
  {
    if (pdp_list[i] != NULL)
    {
        ...
        nas_event_report_payload(EVENT_SM_PDP_STATE, 4, (void *)pdp_event_pkt,sm_message_as_id);
    }
  }
                                     ...
                                     byte cause)
{
  ...
  if (request == TRUE)
  {
    nas_event_report_payload(EVENT_SMGMM_REQUEST_SENT,2,(void*)event_payload,sm_message_as_id);
  }
  else
  {
    nas_event_report_payload(EVENT_SMGMM_REJECT_RECEIVED,2,(void*) event_payload,sm_message_as_id);
  }
}
/*===========================================================================
...
boolean sm_log_pdp_info()
{
  ...
  if ( mm_dual_standby_mode )
  {
   ...
   ds_log_ptr = (LOG_UMTS_DS_PS_CALL_INFO_LOG_PACKET_C_type *)
              nas_log_alloc(LOG_UMTS_DS_PS_CALL_INFO_LOG_PACKET_C, pkt_size,sm_message_as_id);
    ...
    ds_log_ptr->as_id = (uint8) (sm_as_id+1);
    ...
    log_util_log_packet_commit((sys_modem_as_id_e_type)sm_as_id, ds_log_ptr);
  }
  else
#endif
  {
  ...
  call_info_pkt = (LOG_UMTS_PS_CALL_INFO_LOG_PACKET_C_type *)
                    nas_log_alloc(LOG_UMTS_PS_CALL_INFO_LOG_PACKET_C, pkt_size, sm_message_as_id);
  ...
  log_util_log_packet_commit((sys_modem_as_id_e_type)sm_as_id, call_info_pkt);
  }
  return TRUE;
}
...
boolean sm_log_qos_info(pdp_context_info *pdp)
{
  ...
  qos_log_pkt = NULL;
  pkt_size = 0;
  wcdma_hsupa_category = sm_get_wcdma_hsupa_category();
  ...
  if ( mm_dual_standby_mode )
  {
    ...
    ds_log_ptr = (LOG_UMTS_DS_NAS_PS_CONNECTION_QOS_LOG_PACKET_C_type *)
               nas_log_alloc(LOG_UMTS_DS_NAS_PS_CONNECTION_QOS_LOG_PACKET_C, pkt_size,
                                 sm_message_as_id);
  ...
  if(wcdma_hsupa_category >= MM_RRC_WCMDA_DC_HSUPA_CATEGORY)
  {
  ...
  qos_log_pkt = (LOG_UMTS_NAS_PS_CONNECTION_QOS_LOG_PACKET_C_type *)
                   nas_log_alloc(LOG_UMTS_NAS_PS_CONNECTION_QOS_LOG_PACKET_C, pkt_size,
                                     sm_message_as_id);
  ...
  log_util_log_packet_commit((sys_modem_as_id_e_type)sm_as_id, qos_log_pkt);
  }
...
rrc_establish_cause_e_type convert_to_rrc_est_cause(byte traffic_class )
{
  ...
  switch (traffic_class)
  {
    ...
    case SUBSCRIBED_CLASS:
      if ((ue_revision == UE_REV_REL99) && (sm_get_nv_nas_rel_compliance() >= NV_NAS_COMPLIANCE_REL6))
      {
...
boolean sm_validate_qos ( qos_T offered_qos, qos_T min_qos )
{
...
#ifdef FEATURE_REL5
  ue_rev_level_indicator_type ue_revision;
  ue_nas_rel_compliance_type  nas_rel_compliance;
  byte wcdma_hsupa_category; 
  ue_revision = sm_get_revision_level_indicator();
  nas_rel_compliance = sm_get_nv_nas_rel_compliance();
  wcdma_hsupa_category = sm_get_wcdma_hsupa_category();
#endif
  if ( qos_ie_length <= 3 )
  {
 ...
 #ifdef FEATURE_REL5
    if (((ue_revision == UE_REV_REL99) && (nas_rel_compliance >= NV_NAS_COMPLIANCE_REL5)) &&
        is_umts_mode())
    {
...
#ifdef FEATURE_DUAL_SIM
  if (!(sm_check_pdp_procedure_pending((sys_modem_as_id_e_type)sm_as_id)))
#else
  ...
  switch (cmd_ptr->header.message_set)
  {
    ...
    case MS_GMM_SM:
      switch (cmd_ptr->header.cmd_id)
      {
        ...
        case GMMSM_ESTABLISH_RESEND_IND:
          as_id = cmd_ptr->cmd.gmmsm_establish_resend_ind.as_id;
          break;

#ifdef FEATURE_NAS_REL12_SPEC_CHANGE 
        case GMMSM_IRAT_PS_SERVICE_IND:
          as_id = cmd_ptr->cmd.gmmsm_irat_ps_service_ind.as_id;
          break;
#endif
    ...
    case MS_CM_SM:
      switch (cmd_ptr->header.cmd_id)
      {
        ...
        case SMREG_DATA_CALL_ACTIVITY_IND:
          as_id = cmd_ptr->cmd.data_call_activity.as_id; 
          break;
      ...
      break;
#if defined(FEATURE_MODEM_CONFIG_REFRESH) || defined(FEATURE_QSH_EVENT_METRIC)
   case MS_SM_SM:
     switch(cmd_ptr->cmd.qsh_message_ind.IMH.message_id)
     {
#ifdef FEATURE_QSH_EVENT_METRIC
       case SM_QSH_MESSAGE_IND:
        as_id = (sys_modem_as_id_e_type)sm_as_id;
        break;
#endif

       default:
       MSG_ERROR_DS_1(SM_SUB, "=SM= Unknown SM message from SM,Msg id = (%d)",
                                  cmd_ptr->cmd.qsh_message_ind.IMH.message_id);
       break;
...
#endif
  sm_message_as_id = (byte)(sm_as_id + 1);
  return status;
}
...
//PER_SUB_NV_CHNAGES
/*===========================================================================
FUNCTION SM_GET_NV_NAS_REL_COMPLAINCE

DESCRIPTION
  This function calls the appropriate MM/GMM function to get the release complaince
...
===========================================================================*/
ue_nas_rel_compliance_type  sm_get_nv_nas_rel_compliance(void)
{
...
#ifdef FEATURE_SGLTE
  {
    ue_nas_rel_compliance_type rel_compliance_type = NV_NAS_COMPLIANCE_REL5; //Need to check the default value?
    ...
    {
      MSG_ERROR_DS_1(SM_SUB,"ue_nas_rel_compliance_type - Invalid SUB ID: %d", sub_id);
      return rel_compliance_type;
    }
  }
#endif
  return mm_get_nv_nas_rel_compliance_per_subs(stack_id_nas);
#elif defined (FEATURE_SGLTE)
  return mm_get_nv_nas_rel_compliance_per_subs(mm_ps_stack_id);
#else
  return mm_get_nv_nas_rel_compliance_per_subs();
#endif

}
/*===========================================================================
FUNCTION SM_GET_WCDMA_HSUPA_CATEGORY

DESCRIPTION
  This function calls the appropriate MM/GMM function to get the HSUPA Category
  based on the configurations ( MM/GMM gets from RRC for each sub)
...
byte sm_get_wcdma_hsupa_category(void)
{
...
#ifdef FEATURE_SGLTE
  {
    byte wcdma_hsupa_category = MM_RRC_WCMDA_HSUPA_DEFAULT_CATEGORY; //Need to check the default value?
    ...
    {
      MSG_ERROR_DS_1(SM_SUB,"sm_get_wcdma_hsupa_category - Invalid SUB ID: %d", sub_id);
      return wcdma_hsupa_category;
    }
  }
#endif
  return mm_get_nv_wcdma_hsupa_category_per_subs(stack_id_nas);
#elif defined (FEATURE_SGLTE)
  return mm_get_nv_wcdma_hsupa_category_per_subs(mm_ps_stack_id);
#else
  return mm_get_nv_wcdma_hsupa_category_per_subs();
#endif
...
nas_vote_for_reselection_e_type sm_ps_call_vote_for_reselection(sys_modem_as_id_e_type asid)
{
  ...
  byte is_valid_as_id = TRUE;
   ...
   {
#ifdef FEATURE_DUAL_DATA
      if(sm_ps_vote_for_reselection_sim[asid] == TRUE)
#else
...
#ifdef FEATURE_QSH_DUMP
/*===========================================================================
Function sm_qsh_dump_data_get()

Description:
SM QSH dump data collection
============================================================================*/

void sm_qsh_dump_data_get
(
  sm_debug_data_s_type *data_ptr
#ifdef FEATURE_DUAL_DATA
  ,sm_as_id_type_e as_id
#endif
)
{
#ifdef FEATURE_DUAL_DATA
#ifdef FEATURE_LTE
  data_ptr->sm_deact_isr_on_interrat_status_debug = sm_deact_isr_on_interrat_status_sim[as_id];
#endif
  data_ptr->is_multi_pdn_same_apn_debug = ds3gpp_per_subs_get_config_multi_pdn_same_apn((sys_modem_as_id_e_type)as_id);
  data_ptr->is_nw_initiated_qos_support_debug = ds3gpp_per_subs_get_config_umts_nw_init_qos_support((sys_modem_as_id_e_type)as_id);
  data_ptr->timers_pending_debug[0] = timers_pending_sim[as_id][0];
  data_ptr->sm_ps_vote_for_reselection_debug = sm_ps_vote_for_reselection_sim[as_id];
  data_ptr->ps_call_info_ti_debug = ps_call_info_ti_sim[as_id];
  data_ptr->sig_connection_available_debug = sig_connection_available_sim[as_id];
  data_ptr->ps_signalling_conn_state_debug = ps_signalling_conn_state_sim[as_id];
  data_ptr->sm_is_pdp_retranmsit_req_debug = sm_is_pdp_retranmsit_req_sim[as_id];
  data_ptr->cm_initiated_ps_signalling_release_debug = cm_initiated_ps_signalling_release_sim[as_id];
  data_ptr->timer_est_pen_flag_debug = timer_est_pen_flag_sim[as_id];
  data_ptr->sm_abort_cnf_pending_debug = sm_abort_cnf_pending_sim[as_id];
  data_ptr->sm_tlli_debug = sm_tlli_sim[as_id];
  data_ptr->sm_as_id_debug = (sys_modem_as_id_e_type)sm_as_id;
#else
#ifdef FEATURE_LTE
  data_ptr->sm_deact_isr_on_interrat_status_debug = sm_deact_isr_on_interrat_status;
#endif
  data_ptr->is_multi_pdn_same_apn_debug = ds3gpp_per_subs_get_config_multi_pdn_same_apn((sys_modem_as_id_e_type)sm_as_id);
  data_ptr->is_nw_initiated_qos_support_debug = ds3gpp_per_subs_get_config_umts_nw_init_qos_support((sys_modem_as_id_e_type)sm_as_id);
  data_ptr->timers_pending_debug[0] = timers_pending[0];
  data_ptr->sm_ps_vote_for_reselection_debug = sm_ps_vote_for_reselection;
  data_ptr->ps_call_info_ti_debug = ps_call_info_ti;
  data_ptr->sig_connection_available_debug = sig_connection_available;
  data_ptr->ps_signalling_conn_state_debug = ps_signalling_conn_state;
  data_ptr->sm_is_pdp_retranmsit_req_debug = sm_is_pdp_retranmsit_req;
  data_ptr->cm_initiated_ps_signalling_release_debug = cm_initiated_ps_signalling_release;
  data_ptr->timer_est_pen_flag_debug = timer_est_pen_flag;
  data_ptr->sm_abort_cnf_pending_debug = sm_abort_cnf_pending;
  data_ptr->sm_tlli_debug = sm_tlli;
  data_ptr->sm_as_id_debug = (sys_modem_as_id_e_type)sm_as_id;
#endif
}
...
/*===========================================================================
Function: sm_esm_get_qsh_dump_data()

Description:
MM task QSH dump data for for Dump collection action.

============================================================================*/
void sm_esm_get_qsh_dump_data
(
  sm_qsh_dump_tag_debug_data_s *dump_data_ptr, 
  sm_qsh_dump_tag_e tag
)
{
  //Need to add support for as_id_2 when dual data feature flag is going to be enabled
#ifdef FEATURE_DUAL_DATA
  sm_as_id_type_e as_id = SM_AS_ID_1;
#endif
  qsh_client_dump_tag_hdr_init((qsh_dump_tag_hdr_s *)dump_data_ptr,(uint16)tag,
                               sizeof(sm_qsh_dump_tag_debug_data_s));
#ifdef FEATURE_LTE
  esm_qsh_dump_data_get(&(dump_data_ptr->esm_debug_data));
#endif
  sm_qsh_dump_data_get(&(dump_data_ptr->sm_debug_data)
#ifdef FEATURE_DUAL_DATA
                        ,as_id
#endif
                       );
...
/*===========================================================================
Function: sm_get_qsh_dump_debug_buffer()

Description:
SM task QSH dump data for for Dump collection action.

============================================================================*/
void sm_get_qsh_dump_debug_buffer
(
  sm_qsh_dump_tag_debug_buffer_s *dump_data_ptr,
  sm_qsh_dump_tag_e tag
)
{
  uint8 count;
  int32 buffer_index = (int32)sm_esm_debug_buffer_index;

  qsh_client_dump_tag_hdr_init((qsh_dump_tag_hdr_s *)dump_data_ptr,(uint16)tag,
                               sizeof(sm_qsh_dump_tag_debug_buffer_s));

  for(count = 0; count < SM_DEBUG_DUMP_BUFFER_SIZE; count++)
  {
    buffer_index = (int32)buffer_index - 1;
    if(buffer_index < 0)
    {
      buffer_index = SM_DEBUG_MESSAGE_BUFFER_SIZE - 1;
    }
    dump_data_ptr->debug_buffer[count] = sm_esm_debug_buffer[buffer_index];
  }
}

/*===========================================================================
Function: sm_populate_qsh_dump_data()

Description:
SM Task QSH Dump collection action
============================================================================*/
void sm_populate_qsh_dump_data
(
...
)
{
  ...
  if(QSH_DUMP_TAG_ENABLED(param->action_params.dump_collect.dump_tag_mask,SM_QSH_DUMP_TAG_DEBUG_DATA))
  {
    if(remaining_size >= sizeof(sm_qsh_dump_tag_debug_data_s))
    {
      sm_esm_get_qsh_dump_data((sm_qsh_dump_tag_debug_data_s *)dump_ptr, SM_QSH_DUMP_TAG_DEBUG_DATA);
      dump_ptr = dump_ptr + (sizeof(sm_qsh_dump_tag_debug_data_s));
      remaining_size -= sizeof(sm_qsh_dump_tag_debug_data_s);
      total_size_written += sizeof(sm_qsh_dump_tag_debug_data_s);
    }
  }
  if(QSH_DUMP_TAG_ENABLED(param->action_params.dump_collect.dump_tag_mask,SM_QSH_DUMP_TAG_DEBUG_BUFFER))
  {
    if(remaining_size >= sizeof(sm_qsh_dump_tag_debug_buffer_s))
    {
      sm_get_qsh_dump_debug_buffer((sm_qsh_dump_tag_debug_buffer_s *)dump_ptr,SM_QSH_DUMP_TAG_DEBUG_BUFFER);
      dump_ptr = dump_ptr + (sizeof(sm_qsh_dump_tag_debug_buffer_s));
      remaining_size -= sizeof(sm_qsh_dump_tag_debug_buffer_s);
      total_size_written += sizeof(sm_qsh_dump_tag_debug_buffer_s);
    }
  }
...
#ifdef FEATURE_QSH_EVENT_METRIC
/*===========================================================================
Function: sm_post_qsh_message_ind()

Description:
Post a message to own task on QSH call back
============================================================================*/

void sm_post_qsh_message_ind(qsh_client_cb_params_s *param)
{
  sm_cmd_type  *cmd_ptr;
  if( (cmd_ptr = sm_get_cmd_buf()) != NULL)
  {
    /* Construct message */
    cmd_ptr->header.message_set = MS_SM_SM;
    cmd_ptr->cmd.qsh_message_ind.IMH.message_id = SM_QSH_MESSAGE_IND;
   
    PUT_IMH_LEN (sizeof(cmd_ptr->cmd.qsh_message_ind) - sizeof(IMH_T),
                      &(cmd_ptr->cmd.qsh_message_ind.IMH));
   
    cmd_ptr->cmd.qsh_message_ind.param = *param;
    /* Put on SM command queue */
    sm_put_cmd(cmd_ptr);
  }
  else
  {
    MSG_FATAL_DS( SM_SUB, "=SM= ERROR: Unable to allocate memory", 0,0,0 );
  }
}
...
/*===========================================================================
Function: sm_qsh_debug_callback()

Description:
Registered QSH call back for SM task.

Note:
   All the functions that can be called for QSH dump collect should use attribute SM_QSH_ATTR_UNCOMPRESS
   Otherwise functions wont be accessible in error fatal context if they are compressed
============================================================================*/
void sm_qsh_debug_callback(qsh_client_cb_params_s *param)
{
  ASSERT(param != NULL);
  switch(param->action)
  {
    ...
    case QSH_ACTION_METRIC_CFG:
      sm_post_qsh_message_ind(param);
      break;
#endif

#ifdef FEATURE_QSH_DUMP
    case QSH_ACTION_DUMP_COLLECT:
      sm_populate_qsh_dump_data(param);
      break;
#endif
...
FUNCTION  SM_QSH_CLIENT_METRIC_SIGNALING_LOG
...
===========================================================================*/
void sm_qsh_client_metric_signaling_log
(
  sm_qsh_signaling_s metric_sig
)
{
  if(sm_qsh_debug_metrics.ota_signaling_ptr != NULL)
  {
    qsh_client_metric_log_done_s client_metric_log_done;
    sm_qsh_debug_metrics.ota_signaling_ptr->metric_sig = metric_sig;
    qsh_client_metric_log_done_init(&client_metric_log_done);
    client_metric_log_done.client = QSH_CLT_SM;
    client_metric_log_done.metric_id = (qsh_metric_id_t)SM_QSH_METRIC_SIGNALING;
    client_metric_log_done.metric_context_id = sm_qsh_debug_metrics.signaling_metric_context_id;
    client_metric_log_done.log_reason = QSH_CLIENT_METRIC_LOG_REASON_EVENT_INTERNAL;
    sm_qsh_debug_metrics.ota_signaling_ptr = 
                  (sm_qsh_metric_signaling_s*)qsh_client_metric_log_done(&client_metric_log_done);
  }
}
...
)
{
  ...
  {
#if defined FEATURE_DUAL_SIM || defined FEATURE_SGLTE
    ULOG_RT_PRINTF_4(sm_get_ulog_handle(),
          "DS: SUB %d =SM= Message Set: %d Message ID: %d message_direction: %d",
          as_id + 1,
          message_set,
          message_id,
          sm_esm_message_direction);
#else
    ULOG_RT_PRINTF_3(sm_get_ulog_handle(),
        "=SM= Message Set: %d Message ID: %d message_direction: %d",
        message_set,
        message_id,
        sm_esm_message_direction);
#endif
...
#endif
  sm_esm_debug_buffer_index++;
  MSG_HIGH_DS_2(MM_LOCAL_SUB, "ESM: umid: %d message_direction: %d", msg_umid,sm_esm_message_direction);
  if (sm_get_ulog_handle() != NULL)
  {
#if defined FEATURE_DUAL_SIM || defined FEATURE_SGLTE
    ULOG_RT_PRINTF_3(sm_get_ulog_handle(),
          "DS: SUB %d =ESM= umid: %d message_direction: %d",
          as_id + 1,
          msg_umid,
          sm_esm_message_direction);
#else
    ULOG_RT_PRINTF_2(sm_get_ulog_handle(),
          "=ESM= umid: %d message_direction: %d",
          msg_umid,
          sm_esm_message_direction);
#endif
  }
