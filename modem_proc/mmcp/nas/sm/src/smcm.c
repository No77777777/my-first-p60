...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/sm/src/smcm.c#2 $    $DateTime: 2022/02/23 22:57:02 $ $Author: pwbldsvc $
...
extern boolean sm_set_tuneaway_status_gsm(boolean current_ok_for_tuneaway_status);
#endif
#endif
...
/*===========================================================================
FUNCTION SM_PROCESS_DATA_CALL_ACTIVITY_IND 

DESCRIPTION
  This function sets the data call activity status in SM.
...
void sm_process_data_call_activity_ind(smreg_data_call_activity_ind_T *data_call_activity_ind)
{
  MSG_HIGH_DS_2(SM_SUB,"sm_data_call_activity_status changing from %d to %d", 
        sm_data_call_activity_status, data_call_activity_ind->activity);
  sm_data_call_activity_status = data_call_activity_ind->activity;
}


/*===========================================================================
**FUNCTION SM_DATA_CALL_ACTIVITY_STATUS_GET

DESCRIPTION
  This function returns the data call activity status in SM.

===========================================================================*/

sys_data_activity_e_type sm_data_call_activity_status_get(void)
{
  return sm_data_call_activity_status;
}
...
#error code not present
    ...
    case SMREG_DATA_CALL_ACTIVITY_IND:
      sm_process_data_call_activity_ind(&(cmd_ptr->cmd.data_call_activity));
      break;
#endif
    default:
...
void sm_continue_pdp_activation(pdp_context_info* pdp)
{
  ...
  -------------------------------------------------------------------------------------------------------------------------*/
  if (sm_get_nv_nas_rel_compliance() >= NV_NAS_COMPLIANCE_REL12)
  {
...
#if defined(FEATURE_DUAL_SIM) && defined (FEATURE_NAS_FREE_FLOAT)
      if((is_umts_mode() && 
          (pdp->qos.traffic_class == (int)CONVERSATION_CLASS || 
           pdp->qos.traffic_class == (int)STREAMING_CLASS))
#ifdef FEATURE_GSM_GPRS
         ||
         ((pdp->trans_id < MAX_PDP_NUM) && 
          (is_gsm_mode() && sm_max_timer_count[pdp->trans_id] == 0))
#endif
        )
      {
        send_gmm_ps_call_status_ind(SM_PS_HIGH_PRI, TRUE);
        sm_set_tuneaway_status_gsm(FALSE);
      }
      else
      {
...
#if defined(FEATURE_DUAL_SIM) && defined (FEATURE_NAS_FREE_FLOAT)
      if((cur_pdp->req_qos.traffic_class == (int)CONVERSATION_CLASS || 
          cur_pdp->req_qos.traffic_class == (int)STREAMING_CLASS))
      {
        (void)sm_set_tuneaway_status_gsm(FALSE);
        send_gmm_ps_call_status_ind(SM_PS_HIGH_PRI, TRUE);
      }
      else
      {
    ...
    switch (cm_cmd)
    {
      ...
      case SMREG_PDP_ACTIVATE_REJ:
        ...
        * ----------------------------------------------------------------*/
        ...
        MSG_HIGH_DS_3(SM_SUB,"Send PDP_ACTIVATE_REJ to CM TI: %d, Cause:%d, type:%d",
      ...
      case SMREG_PDP_ACTIVATE_SEC_REJ:
        ...
        * ----------------------------------------------------------------*/
        ...
        MSG_HIGH_DS_3(SM_SUB," Send PDP_ACTIVATE_SEC_REJ to CM for TI: %d, Cause:%d, type:%d",
...
#endif
        MSG_HIGH_DS_3(SM_SUB,"Send PDP_MODIFY_REJ to CM for TI: %d, Cause:%d, type:%d",
        ...
        ** -----------------------------------------------------------------*/
        ...
        case SMREG_PDP_NW_MODIFY_REQ:
          ...
          * ----------------------------------------------------------------*/
          ...
          cm_rpt->cmd.pdp_nw_modify_req.pdp_addr = cur_pdp->req_pdp_addr;
          /* -------------------------------------------------------------------
          ** Set the cm_pending_msg to UNKNOWN_CMD, as the CM cmd has been
          ** processed and confirmation/rejection is being sent to the CM task
          ** -----------------------------------------------------------------*/
  ...
  ** ---------------------------------------------------------------------*/
  ...
  else
  {
    ...
    if (status == L3_PARSE_SUCCESS)
    {
      ...
      if(cur_pdp->req_pdp_addr.valid)
      {
        cur_pdp->pdp_addr = cur_pdp->req_pdp_addr; 
      }
