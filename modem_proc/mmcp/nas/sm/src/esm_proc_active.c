...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/sm/src/esm_proc_active.c#1 $
...
)
{
  ...
  if(((failure_ind_ptr != NULL) && 
     ...
     ((esm_get_emc_srv_status() == LTE_NAS_EMC_SRV_PENDING) &&
      (proc_ptr->pending_msg_ptr->esm_out_msg.nas_hdr.msg_id == 
                                                   PDN_CONNECTIVITY_REQUEST) &&
      (emm_check_if_pdn_conn_req_for_emergency_bearer_services(proc_ptr->pending_msg_ptr->esm_out_msg.pdn_connectivity_req.req)) &&
  ...
  //One attempt only for PDN disconnect procedure when DDS switch is in progress
  if((proc_ptr->pending_msg_ptr->esm_out_msg.nas_hdr.msg_id == 
                                                   PDN_DISCONNECT_REQUEST) &&
