...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/sm/src/esm_utils.c#1 $
...
static byte esm_msg_seq_num_sim[MAX_AS_IDS] = {1,1};
#define esm_msg_seq_num esm_msg_seq_num_sim[sm_as_id]
#else
...
extern boolean esm_get_local_deact_flag
(
  void
);
extern void esm_set_local_deact_flag
(
  boolean
);
...
FUNCTION  ESM_QSH_OUTGOING_MESSAGE_LOG

DESCRIPTION
  This function logs the QSH metric for outgoing ESM OTA signaling
...
===========================================================================*/
void esm_qsh_outgoing_message_log
(
  lte_nas_message_id message_type,
  esm_cause_enum_T cause
)
{
  sm_qsh_signaling_s qsh_signaling;
  qsh_signaling.protocol_discriminator = PROTOCOL_DISCRMINATOR_ESM_DEBUG;
  qsh_signaling.message_type.esm_message_id = message_type;
  qsh_signaling.direction = UE_TO_CN_DIRECTION;
  qsh_signaling.cause_value.esm_cause = cause;
...
#ifdef FEATURE_QSH_EVENT_METRIC
  esm_qsh_outgoing_message_log(ACTIVATE_DEFAULT_EPS_BEARER_CONTEXT_ACCEPT,NAS_ESM_NO_FAILURE);
#endif
  event.esm_out_msg = (dword)NAS_EMM_DATA_REQ;
  nas_event_report_payload(EVENT_LTE_ESM_OUTGOING_MSG,
...
#ifdef FEATURE_QSH_EVENT_METRIC
  esm_qsh_outgoing_message_log(ACTIVATE_DEFAULT_EPS_BEARER_CONTEXT_REJECT,
                       out_msg_ptr->esm_out_msg.default_bearer_context_rej.esm_cause.esm_cause);
#endif
  event.esm_out_msg = (dword)NAS_EMM_DATA_REQ;
  nas_event_report_payload(EVENT_LTE_ESM_OUTGOING_MSG,
              sizeof(event), (void*)&event,sm_message_as_id);
...
#ifdef FEATURE_QSH_EVENT_METRIC
  esm_qsh_outgoing_message_log(ACTIVATE_DEDICATED_EPS_BEARER_CONTEXT_REJECT,
                       msg_ptr->esm_cause.esm_cause);
#endif
  event.esm_out_msg = (dword)NAS_EMM_DATA_REQ;
  nas_event_report_payload(EVENT_LTE_ESM_OUTGOING_MSG,
             sizeof(event), (void*)&event,sm_message_as_id);
...
#ifdef FEATURE_QSH_EVENT_METRIC
  esm_qsh_outgoing_message_log(MODIFY_EPS_BEARER_CONTEXT_REJECT,
                       out_msg_ptr->esm_out_msg.modify_eps_bearer_ctxt_rej.esm_cause.esm_cause);
#endif
...
)
{
  ...
  if(cur_state != ESM_BC_INACTIVE)
  {
    ...
    MSG_HIGH_DS_3(SM_SUB, "ESM: Sending DEACT BC REQ - BID %d, Default %d, LBI %d",
                  ...
                  ctxt_ptr->lbi);
    event.esm_out_msg = (dword)MM_CM_DEACT_BEARER_CONTEXT_REQUEST_IND;
    nas_event_report_payload(EVENT_LTE_CM_OUTGOING_MSG,
               sizeof(event), (void*)&event,sm_message_as_id);
  }
  else
  {
...
{  
  ...
  */
  ...
  event.esm_out_msg = (dword)MM_CM_ACT_DRB_RELEASED_IND;
  nas_event_report_payload(EVENT_LTE_CM_OUTGOING_MSG,
              sizeof(event), (void*)&event,sm_message_as_id);
...
)
{
  ...
  cm_pkd_pdn_connectivity_req->pkd_msg_len = (byte)(offset/8);/*Offset will be in bits, convert it to bytes*/
...
#ifdef FEATURE_QSH_EVENT_METRIC
  esm_qsh_outgoing_message_log(PDN_CONNECTIVITY_REQUEST,
                               NAS_ESM_NO_FAILURE);
#endif

  event.esm_out_msg = (dword)NAS_EMM_DATA_REQ;
  nas_event_report_payload(EVENT_LTE_ESM_OUTGOING_MSG, sizeof(event), 
                       (void*)&event,sm_message_as_id);
  esm_modem_mem_free(out_msg_ptr);
...
)
{
  ...
  event.esm_out_msg = (dword)MM_CM_DRB_SETUP_IND;
  nas_event_report_payload(EVENT_LTE_CM_OUTGOING_MSG,
             sizeof(event), (void*)&event,sm_message_as_id);  
...
)
{
 ...
 if(proc_ptr != NULL)
  {
     if( emm_check_if_pdn_conn_req_for_emergency_bearer_services(proc_ptr->pending_msg_ptr->esm_out_msg.pdn_connectivity_req.req) )
	 {
  ...
  if((msg_ptr != NULL) && (msg_ptr->lte_nas_hdr.valid == TRUE)) 
  {
    ...
    {
       cm_pdn_connectivity_rej->reattempt_indicator = msg_ptr->reattempt_indicator;
    }
#endif
  }
  else
...
#ifdef FEATURE_LTE_REL12
    cm_pdn_connectivity_rej->backoff_timer.valid = FALSE;
    cm_pdn_connectivity_rej->reattempt_indicator.valid = FALSE;
#endif
  }
  ...
  event.esm_out_msg = (dword)MM_CM_PDN_CONNECTIVITY_REJECT_IND;
  nas_event_report_payload(EVENT_LTE_CM_OUTGOING_MSG,
             sizeof(event), (void*)&event,sm_message_as_id);
...
)
{
  ...
  event.esm_out_msg = (dword)MM_CM_DRB_REESTABLISH_REJECT_IND;
  nas_event_report_payload(EVENT_LTE_CM_OUTGOING_MSG,
                sizeof(event), (void*)&event,sm_message_as_id);
  ...
  if(def_bearer_req_ptr->lte_nas_esm_hdr.bearer_id >= MIN_NSAPI_NUM)
  {
    if(def_bearer_req_ptr->lte_nas_hdr.valid == TRUE)
    {
      ...
      if(((def_bearer_req_ptr->ti.valid == TRUE) && (sm_check_if_no_conn_exists(conn_id) == FALSE))||
	  	  ((def_bearer_req_ptr->ti.valid & def_bearer_req_ptr->neg_qos.valid) == FALSE))
      {
        ...
        MSG_HIGH_DS_1(SM_SUB, "ESM: PDP context for Connection ID %d exists/ either of TID or QoS is invalid, not updating SM context any further, invalidating TI/ QoS",
                      conn_id);
      }
...
)
{
  ...
  event.esm_out_msg = (dword)MM_CM_ACT_DEDICATED_BEARER_CONTEXT_REQUEST_IND;
  nas_event_report_payload(EVENT_LTE_CM_OUTGOING_MSG,
            sizeof(event), (void*)&event,sm_message_as_id);
...
#ifdef FEATURE_QSH_EVENT_METRIC
  esm_qsh_outgoing_message_log(ACTIVATE_DEDICATED_EPS_BEARER_CONTEXT_ACCEPT,
                               NAS_ESM_NO_FAILURE);
#endif
  event.esm_out_msg = (dword)NAS_EMM_DATA_REQ;
  nas_event_report_payload(EVENT_LTE_ESM_OUTGOING_MSG,
...
)
{
       ...
       (mgr_ptr->esm_bc[index]->eps_bearer_state_ptr->state() == ESM_BC_MODIFY_PENDING)))
    {
      ...
      { 
        ...
        else
        {
          ...
          bearer_status_cmd->context_status.non_emc_default_bc_bm |= 
             (uint16)(1 << (mgr_ptr->esm_bc[index]->eps_bearer_id - ESM_STARTING_BEARER_ID));
        }
      }
  ...
  bearer_status_cmd->esm_local_deact = esm_get_local_deact_flag();
  
  if(esm_get_local_deact_flag() == TRUE)
    esm_set_local_deact_flag(FALSE);
...
)
{
  ...
  nas_event_report_payload(EVENT_LTE_ESM_OUTGOING_MSG,
            sizeof(event), (void*)&event,sm_message_as_id);
#ifdef FEATURE_QSH_EVENT_METRIC
  esm_qsh_outgoing_message_log(DEACTIVATE_EPS_BEARER_CONTEXT_ACCEPT,
                               NAS_ESM_NO_FAILURE);
#endif
  esm_modem_mem_free(out_msg_ptr);
...
#ifdef FEATURE_QSH_EVENT_METRIC
  esm_qsh_outgoing_message_log(BEARER_RESOURCE_ALLOCATION_REQUEST,
                               NAS_ESM_NO_FAILURE);
#endif
  event.esm_out_msg = (dword)NAS_EMM_DATA_REQ;
  nas_event_report_payload(EVENT_LTE_ESM_OUTGOING_MSG,
...
)
{
  ...
  nas_event_report_payload(EVENT_LTE_ESM_TIMER_START, 
                      sizeof(event), &event,sm_message_as_id);
...
)
{
  ...
  if(proc_ptr->timer_ptr != NULL)
  {
    ...
    event.esm_timer_event = (byte)timer_id;
    /* log the event for ConnectionRequest being sent */
    nas_event_report_payload(EVENT_LTE_ESM_TIMER_STOP,
                    sizeof(event), &event,sm_message_as_id);
  }
}
...
#ifdef FEATURE_QSH_EVENT_METRIC
  esm_qsh_outgoing_message_log(PDN_DISCONNECT_REQUEST,
                               NAS_ESM_NO_FAILURE);
#endif
  esm_modem_mem_free(out_msg_ptr);
}
...
)
{
  ...
  event.esm_out_msg = (dword)MM_CM_PDN_DISCONNECT_REJECT_IND;
  nas_event_report_payload(EVENT_LTE_CM_OUTGOING_MSG,sizeof(event), (void*)&event,sm_message_as_id);
...
void esm_build_and_send_pdn_connectivity_abort_rsp(word sdf_id,boolean result)
{
  ...
  event.esm_out_msg = (dword)MM_CM_PDN_CONNECTIVITY_FAILURE_IND;
  nas_event_report_payload(EVENT_LTE_CM_OUTGOING_MSG,sizeof(event), (void*)&event,sm_message_as_id);
...
#ifdef FEATURE_QSH_EVENT_METRIC
  if(out_msg_ptr->esm_out_msg.bearer_resource_modifi_req.esm_cause.valid)
  {
    esm_qsh_outgoing_message_log(BEARER_RESOURCE_MODIFICATION_REQUEST,
           out_msg_ptr->esm_out_msg.bearer_resource_modifi_req.esm_cause.esm_cause);
  }
  else
  {
    esm_qsh_outgoing_message_log(BEARER_RESOURCE_MODIFICATION_REQUEST,
...
)
{
  ...
  event.esm_out_msg = (dword)MM_CM_MODIFY_BEARER_CONTEXT_REQUEST_IND;
  nas_event_report_payload(EVENT_LTE_CM_OUTGOING_MSG,sizeof(event), (void*)&event,sm_message_as_id);
...
)
{
  ...
  event.esm_out_msg = (dword)NAS_EMM_DATA_REQ;
  nas_event_report_payload(EVENT_LTE_ESM_OUTGOING_MSG,sizeof(event), (void*)&event,sm_message_as_id);
#ifdef FEATURE_QSH_EVENT_METRIC
  esm_qsh_outgoing_message_log(MODIFY_EPS_BEARER_CONTEXT_ACCEPT,
                                 NAS_ESM_NO_FAILURE);
#endif
  esm_modem_mem_free(out_msg_ptr);
  ...
  if(bc_info.bearer_id_exist == TRUE && bc_info.pti == dl_msg_pti)
  {
    switch(dl_msg_id)
    {
      case ACTIVATE_DEFAULT_EPS_BEARER_CONTEXT_REQUEST:
        ...
        break;
      case ACTIVATE_DEDICATED_EPS_BEARER_CONTEXT_REQUEST:
        if(bc_info.last_msg_sent == ACTIVATE_DEDICATED_EPS_BEARER_CONTEXT_ACCEPT)
        {
          retx_ul_msg = TRUE;
        }
        break;
      case MODIFY_EPS_BEARER_CONTEXT_REQUEST:
        if(bc_info.last_msg_sent == MODIFY_EPS_BEARER_CONTEXT_ACCEPT)
        {
          retx_ul_msg = TRUE;
        }
        break;
...
)
{
  ...
  e_retval = esm_msgr_send( &msg_ptr->msg_hdr, sizeof(emm_data_req_T) );
  ASSERT(e_retval == E_SUCCESS);
...
)
{
  ...
  {  
    ...
    if(emm_get_3gpp_rel_ver_per_sub((sys_modem_as_id_e_type)sm_as_id) >= LTE_3GPP_REL12)
    {
       cm_bc_mod_rej_ptr->reattempt_indicator = msg_ptr->reattempt_indicator;
    }
#endif
  }
  else
...
#ifdef FEATURE_LTE_REL12
    cm_bc_mod_rej_ptr->backoff_timer.valid = FALSE;
    cm_bc_mod_rej_ptr->reattempt_indicator.valid = FALSE;
#endif
  }
  ...
  event.esm_out_msg = (dword)MM_CM_BEARER_CONTEXT_MODIFY_REJECT_IND;
  nas_event_report_payload(EVENT_LTE_CM_OUTGOING_MSG,sizeof(event), (void*)&event,sm_message_as_id);
...
#ifdef FEATURE_QSH_EVENT_METRIC
  esm_qsh_outgoing_message_log(ESM_INFORMATION_RESPONSE,
                                 NAS_ESM_NO_FAILURE);
#endif
...
)
{
  ...
  nas_event_report_payload(EVENT_LTE_ESM_OUTGOING_MSG,sizeof(event), (void*)&event,sm_message_as_id);
#ifdef FEATURE_QSH_EVENT_METRIC
  esm_qsh_outgoing_message_log(ESM_STATUS,
                               out_msg_ptr->mo_esm_status.esm_cause);
#endif
  /*free heap memory*/
  esm_modem_mem_free(emm_cmd_ptr);
} /* esm_send_esm_status */
...
)
{
  ...
  mem_ptr = modem_mem_calloc(1, size, MODEM_MEM_CLIENT_NAS);
...
)
{
  ...
  if(emm_check_if_pdn_conn_req_for_emergency_bearer_services(msg_ptr->request_type) &&
  ...
  if((pdn_conn_allowed == TRUE) && emm_check_if_pdn_conn_req_for_emergency_bearer_services(msg_ptr->request_type))
  {
  ...
  switch(msg_ptr->id)
  {
    ...
    case MM_CM_1XCSFB_ABORT_RSP:
     /* Log to SM buffer */
      ...
      msgr_set_hdr_variant(msg_ptr,(uint8)sm_as_id);
      break;
...
*/
/*============================================================================*/
void esm_build_and_send_ps_call_status_ind
( 
  void
)
{
  if(esm_get_last_ps_call_status() != esm_check_ps_call_in_progress())
  {
    errno_enum_type                         e_retval;
    emm_ps_call_status_ind_cmd_T* ps_call_status_ind = 
      (emm_ps_call_status_ind_cmd_T *)esm_modem_mem_alloc(sizeof(emm_ps_call_status_ind_cmd_T));
    msgr_init_hdr(&(ps_call_status_ind->msg_hdr),MSGR_NAS_ESM,NAS_EMM_PS_CALL_STATUS_CMD);
    ps_call_status_ind->ps_call_in_progress = esm_check_ps_call_in_progress();
    esm_set_last_ps_call_status(ps_call_status_ind->ps_call_in_progress);
    e_retval = esm_msgr_send( (msgr_hdr_s*)ps_call_status_ind, sizeof(emm_ps_call_status_ind_cmd_T));
    ASSERT(e_retval == E_SUCCESS);
    MSG_HIGH_DS_1(SM_SUB, 
                  "ESM: ESM sent NAS_EMM_PS_CALL_STATUS_CMD ps_call_in_progress is %d",
                  ps_call_status_ind->ps_call_in_progress);
    esm_modem_mem_free(ps_call_status_ind);
  }
}
