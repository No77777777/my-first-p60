...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/sm/src/esmtask.c#3 $
...
#endif
  boolean last_ps_call_status;
  boolean esm_local_deactivation;
} esm_T;


#if defined(FEATURE_DUAL_SIM) && defined(FEATURE_DUAL_DATA)
LOCAL esm_cmd_type *esm_pend_msg_ptr_list_sim[MAX_AS_IDS][ESM_MAX_PENDING_MSGS];
#define esm_pend_msg_ptr_list esm_pend_msg_ptr_list_sim[sm_as_id]

LOCAL esm_T esm_sim[MAX_AS_IDS];
#define esm esm_sim[sm_as_id]
...
FUNCTION  ESM_QSH_INCOMING_MESSAGE_LOG

DESCRIPTION
  This function logs the QSH metric for incoming ESM OTA signaling
...
===========================================================================*/
void esm_qsh_incoming_message_log
(
  esm_cmd_type* esm_cmd_ptr
)
{
  if((esm_cmd_ptr->cmd.msg_hdr.src == MSGR_NAS_EMM) &&
     (esm_cmd_ptr->cmd.msg_hdr.id == NAS_ESM_DATA_IND))
  {
    sm_qsh_signaling_s qsh_signaling;
    lte_nas_esm_incoming_msg_type *in_msg = 
         &esm_cmd_ptr->cmd.esm_data_ind.esm_incoming_msg;
    qsh_signaling.protocol_discriminator = PROTOCOL_DISCRMINATOR_ESM_DEBUG;
    qsh_signaling.message_type.esm_message_id = in_msg->nas_hdr.msg_id;
    qsh_signaling.direction = CN_TO_UE_DIRECTION;
    qsh_signaling.cause_value.esm_cause = NAS_ESM_NO_FAILURE;
#ifdef FEATURE_DUAL_SIM
    qsh_signaling.as_id = sm_as_id;
#endif
    switch(in_msg->nas_hdr.msg_id)
    {
      case ACTIVATE_DEFAULT_EPS_BEARER_CONTEXT_REQUEST:
        if(in_msg->default_bearer_context_req.esm_cause.valid)
        {
          qsh_signaling.cause_value.esm_cause = in_msg->default_bearer_context_req.esm_cause.esm_cause;
        }
        break;
      ...
      case DEACTIVATE_EPS_BEARER_CONTEXT_REQUEST:
        if(in_msg->deactivate_eps_bearer_context_req.esm_cause.valid)
        {
          qsh_signaling.cause_value.esm_cause = in_msg->deactivate_eps_bearer_context_req.esm_cause.esm_cause;
        }
        break;
      case PDN_CONNECTIVITY_REJECT:
        if(in_msg->pdn_connectivity_rej.esm_cause.valid)
        {
          qsh_signaling.cause_value.esm_cause = in_msg->pdn_connectivity_rej.esm_cause.esm_cause;
        }
        break;
      case BEARER_RESOURCE_ALLOCATION_REJECT:
        if(in_msg->bearer_resource_alloc_rej.esm_cause.valid)
        {
          qsh_signaling.cause_value.esm_cause = in_msg->bearer_resource_alloc_rej.esm_cause.esm_cause;
        }
        break;
      case PDN_DISCONNECT_REJECT:
        if(in_msg->pdn_disconnect_rej.esm_cause.valid)
        {
          qsh_signaling.cause_value.esm_cause = in_msg->pdn_disconnect_rej.esm_cause.esm_cause;
        }
        break;
      case BEARER_RESOURCE_MODIFICATION_REJECT:
        if(in_msg->bearer_resource_modifi_rej.esm_cause.valid)
        {
          qsh_signaling.cause_value.esm_cause = in_msg->bearer_resource_modifi_rej.esm_cause.esm_cause;
        }
        break;
      case ESM_STATUS:
        qsh_signaling.cause_value.esm_cause = in_msg->mt_esm_status.esm_cause;
        break;
      default:
        return;
    }
    sm_qsh_client_metric_signaling_log(qsh_signaling);
  }
}
#endif
...
)
{
...
#ifdef FEATURE_QSH_EVENT_METRIC
  esm_qsh_incoming_message_log(esm_cmd_ptr);
#endif
  /*MNK REDESIGN BEGIN*/
     ...
     (esm.esm_context_mgr.on_attach_def_bearer_pending == ESM_BC_ON_ATTACH_DEF_BEARER_PENDING))
  {
    MSG_HIGH_DS_0(SM_SUB, "ESM: received NAS_ESM_PDN_CONNECT_PACKED_CNF from CM");
     ...
     (esm_cmd_ptr->cmd.msg_hdr.id != NAS_ESM_ACTIVE_EPS_IND))
  {
...
#ifdef FEATURE_LTE_REL9
      !emm_check_if_pdn_conn_req_for_emergency_bearer_services(esm_cmd_ptr->cmd.esm_pdn_connectivity_req.request_type) &&
#endif  
      ...
      if((esm_cmd_ptr->cmd.msg_hdr.id == NAS_ESM_PDN_DISCONNECT_REQ) 
         ...
         (esm_cmd_ptr->cmd.esm_pdn_disconnect_req.esm_local_cause.local_cause 
          ...
          esm_cmd_ptr->cmd.esm_pdn_disconnect_req.esm_local_cause.local_cause 
            == ESM_LOCAL_DEACT))
      {
        local_cause = esm_cmd_ptr->cmd.esm_pdn_disconnect_req.esm_local_cause.local_cause;
      }
...
#ifdef FEATURE_DUAL_SIM
  esm_build_and_send_ps_call_status_ind();
#endif
} /* esm_process() */
...
void esm_post_timer_expired(unsigned long param)
{
  ...
  e_status = msgr_register_variant(MSGR_NAS_ESM, &client,
            MSGR_ID_REX,NAS_ESM_TIMER_EXPIRED_TMRI,(uint8)SYS_MODEM_AS_ID_1);
  ASSERT(e_status == E_SUCCESS);

  /* Init the MSGR header*/
  msgr_init_hdr(&cmd.msg_hdr,MS_TIMER,NAS_ESM_TIMER_EXPIRED_TMRI);
#ifdef FEATURE_DUAL_SIM
  msgr_set_hdr_variant(&cmd.msg_hdr,(uint8)SYS_MODEM_AS_ID_1);
#endif
  ...
  e_status = msgr_deregister_variant(MSGR_NAS_ESM, &client,
           NAS_ESM_TIMER_EXPIRED_TMRI,(uint8)SYS_MODEM_AS_ID_1);
  ASSERT(e_status == E_SUCCESS);
...
void esm_post_timer_expired_sub2(unsigned long param)
{
  ...
  e_status = msgr_register_variant(MSGR_NAS_ESM, &client,
             MSGR_ID_REX,NAS_ESM_TIMER_EXPIRED_TMRI,(uint8)SYS_MODEM_AS_ID_2);
  ASSERT(e_status == E_SUCCESS);

  /* Init the MSGR header*/
  msgr_init_hdr(&cmd.msg_hdr,MS_TIMER,NAS_ESM_TIMER_EXPIRED_TMRI);
  msgr_set_hdr_variant(&cmd.msg_hdr,(uint8)SYS_MODEM_AS_ID_2);
  e_status = msgr_send( &cmd.msg_hdr, sizeof(cmd) );
  ASSERT(e_status == E_SUCCESS);
  MSG_HIGH_DS_0(SM_SUB, "ESM: Post NAS_ESM_TIMER_EXPIRED_TMRI to ESM");
  ...
  /* DEREGISTER NAS_ESM_TIMER_EXPIRED_TMRI */
  e_status = msgr_deregister_variant(MSGR_NAS_ESM, &client,
           NAS_ESM_TIMER_EXPIRED_TMRI,(uint8)SYS_MODEM_AS_ID_2);
  ASSERT(e_status == E_SUCCESS);
...
)
{
  ...
  lte_nas_msgr_register(MSGR_NAS_ESM,client_ptr,
                           MSGR_ID_REX,NAS_ESM_PROC_RETRY_IND);
  lte_nas_msgr_register(MSGR_NAS_ESM,client_ptr,
                           MSGR_ID_REX,
...
)
{
...
#endif
  {
	...
	esm.esm_local_deactivation = FALSE;
...
#ifdef FEATURE_DUAL_SIM
        esm_valid_msg = esm_validate_and_set_sm_as_id(
...
byte  esm_update_esm_context_from_sm(const pdp_context_info* pdp_ctxt_ptr)
{
  ...
  if(sm_is_mode_enabled_sub(SYS_SYS_MODE_MASK_LTE) == FALSE)
  {
...
{
  if(sm_is_mode_enabled_sub(SYS_SYS_MODE_MASK_LTE))
  {
    lte_nas_local_cause_T  local_cause;
...
/*==============================================================================
  ...
  FUNCTION:  esm_dedicated_bearer_exists

==============================================================================*/
boolean esm_dc_bearer_exists
(
#ifdef FEATURE_DUAL_SIM
...
)
{
...
#ifdef FEATURE_DUAL_SIM
  if(as_id != esm_get_sm_as_id())
  {
    MSG_HIGH_DS_1(SM_SUB, "invalid sub id %d", as_id);
    return FALSE;
  }
#endif
  ...
  for(index = 0; index < ESM_MAX_EPS_BEARERS; index++)
  {
    ...
    {
       if((esm.esm_context_mgr.esm_bc[index]->default_bearer_ctxt == FALSE)&&
          (esm.esm_context_mgr.esm_bc[index]->eps_bearer_state_ptr->state() !=  ESM_BC_INACTIVE))
       {
        return TRUE;
       }
    }
...
/*===========================================================================
Function: esm_qsh_dump_data_get()

Description:
ESM QSH dump data collection
============================================================================*/
#ifdef FEATURE_QSH_DUMP
void esm_qsh_dump_data_get
(
  esm_debug_data_s_type *data_ptr
)
{
  data_ptr->on_attach_def_bearer_pending = 
     (esm_bc_on_attach_def_bearer_state_debug_T)(esm.esm_context_mgr.on_attach_def_bearer_pending);
  data_ptr->isr_status = esm.esm_context_mgr.isr_status;
  data_ptr->first_def_bearer_ctxt_received = esm.esm_context_mgr.first_def_bearer_ctxt_received;
  data_ptr->cur_pti_index = esm.esm_proc_mgr.cur_pti_index;
#ifdef FEATURE_LTE_REL9
  data_ptr->emc_srv_status = (lte_nas_emc_srv_status_sm_debug_type)(esm.emc_srv_status);
  data_ptr->rel_ver = emm_get_3gpp_rel_ver_per_sub((sys_modem_as_id_e_type)sm_as_id);
#endif
}
#endif
...
void esm_init_bpm_fn_ptr(esm_bearer_resource_proc_T * esm_bp)
{
  esm_bp->request_esm_to_deactivate_ctxts_func_ptr = esm_deactivate_ctxts;
  esm_bp->chk_on_attach_def_bc_setup_ptr = esm_chk_on_attach_def_bc_setup;
}
...
FUNCTION  ESM_CHECK_PS_CALL_IN_PROGRESS

DESCRIPTION
  This function checks if any bearer activation/deactivation or modification is in progress
...
===========================================================================*/
boolean esm_check_ps_call_in_progress(void)
{
  uint8 i;
  for(i = 0; i < ESM_MAX_BP_PROCS; i++)
  {
    if(esm.esm_proc_mgr.esm_bp[i]!= NULL)
    {
      if((esm.esm_proc_mgr.esm_bp[i]->esm_proc_state_ptr->state() == ESM_BP_ACTIVE) ||
         (esm.esm_proc_mgr.esm_bp[i]->esm_proc_state_ptr->state() == ESM_BP_ABORT))
      {
        return TRUE;
      }
    }
  }
  for(i = 0; i < ESM_MAX_EPS_BEARERS; i++)
  {
    if(esm.esm_context_mgr.esm_bc[i] != NULL)
    {
      if(esm.esm_context_mgr.esm_bc[i]->eps_bearer_state_ptr->state() == ESM_BC_ACTIVE_PENDING ||
         esm.esm_context_mgr.esm_bc[i]->eps_bearer_state_ptr->state() == ESM_BC_MODIFY_PENDING)
      {
        return TRUE;
      }
    }
...
FUNCTION  ESM_GET_LAST_PS_CALL_STATUS

DESCRIPTION
  This function returns the ps call status that is last sent to EMM
...
===========================================================================*/
boolean esm_get_last_ps_call_status(void)
{
  return esm.last_ps_call_status;
}


/*===========================================================================

FUNCTION  ESM_SET_LAST_PS_CALL_STATUS

DESCRIPTION
  This function sets last_ps_call_status
...
===========================================================================*/
void esm_set_last_ps_call_status(boolean ps_call_status)
{
  esm.last_ps_call_status = ps_call_status;
}
#endif
...
void esm_release_esm_bp_ptr(esm_bearer_resource_proc_T* proc_ptr)
{
  ASSERT(proc_ptr != NULL);
  esm.esm_proc_mgr.esm_bp[proc_ptr->instance_id]= NULL;
  esm_modem_mem_free(proc_ptr);
}
...
boolean esm_check_pdn_disconnect_sent_by_bid(byte bearer_id)
{
  ...
  {
    if (esm.esm_proc_mgr.esm_bp[index]!= NULL)
    {
...
boolean esm_ignore_mod_due_to_collision(esm_cmd_type *cmd_ptr)
{
  byte index = 0;
  byte bid;
  esm_bearer_context_info_shared_T    bc_info_shared;

  bid = cmd_ptr->cmd.esm_modify_bearer_context_rej.eps_bearer_id;
...
/*===========================================================================
FUNCTION esm_set_local_deact_flag
  
DESCRIPTION
  API to set the local deactivation flag at ESM.
...
===========================================================================*/
void esm_set_local_deact_flag(boolean flag)
{
  MSG_HIGH_DS_1(SM_SUB, "ESM: ESM flag for local deactivation set to %d",flag);
  esm.esm_local_deactivation = flag;
}



/*===========================================================================
FUNCTION esm_get_local_deact_flag
  
DESCRIPTION
  This function fetches the local deactivation flag for ESM
...
===========================================================================*/
boolean esm_get_local_deact_flag(void)
{

  return esm.esm_local_deactivation;
}
#endif /*FEATURE_LTE*/
