...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/sm/src/smgmm.c#1 $    $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...
extern boolean sm_pdp_sig_in_progress_last_sent_sim[MAX_AS_IDS][2];
#define sm_pdp_sig_in_progress_last_sent sm_pdp_sig_in_progress_last_sent_sim[sm_as_id]
#endif
#else
extern uint32 sm_tlli;
#ifdef FEATURE_DUAL_SIM
extern boolean sm_pdp_sig_in_progress_last_sent[2];
#endif
#endif
...
/* -----------------------------------------------------------------------
** FUNCTION     : sm_process_establish_resend_ind()
** DESCRIPTION  : 
   This function resends GMMSM_EST_REQ to GMM inorder to perform pending request 
   immediately instead of waiting for timer expiry.
** ----------------------------------------------------------------------- */
LOCAL void sm_process_establish_resend_ind(gmmsm_establish_resend_ind_T *gprs_ind);


#ifdef FEATURE_NAS_REL12_SPEC_CHANGE 
/* -----------------------------------------------------------------------
** FUNCTION     : sm_process_irat_ps_service_ind()
** DESCRIPTION  : 
   This function locally deatcivates the  PDPs which don't have valid GERAN 
   params (LLC SAPI, Radio Pri)
** ----------------------------------------------------------------------- */
LOCAL void sm_process_irat_ps_service_ind(gmmsm_irat_ps_service_ind_T *irat_ps_srv_ind);
#endif
...
void sm_process_gmm_cmd(sm_cmd_type* cmd_ptr)
{
  ...
  switch (cmd_ptr->header.cmd_id)
  {
    ...
    case GMMSM_ESTABLISH_RESEND_IND:
      ...
      sm_process_establish_resend_ind(&cmd_ptr->cmd.gmmsm_establish_resend_ind);
      break;

#ifdef FEATURE_NAS_REL12_SPEC_CHANGE
    case GMMSM_IRAT_PS_SERVICE_IND:
      
      sm_process_irat_ps_service_ind(&cmd_ptr->cmd.gmmsm_irat_ps_service_ind);
      break;
#endif
...
#ifdef FEATURE_DUAL_SIM
/*===========================================================================
FUNCTION sm_set_tuneaway_status_gsm 

DESCRIPTION
  This function call the GERAN API to set tuneway blocked or unblocked flag
...
boolean sm_set_tuneaway_status_gsm(boolean current_ok_for_tuneaway_status)
{
...
void send_gmm_ok_to_tuneaway_ind(void)
{
  ...
  /* if tuneaway not allowed, block tuneaway for active rat */
  umts_is_ok_to_tuneaway = sm_set_tuneaway_status_gsm(current_ok_for_tuneaway_status);
  send_gmm_ps_call_status_ind(SM_PS_HIGH_PRI, (boolean)!(current_ok_for_tuneaway_status));
...
void send_gmm_smpdu(word mesg_len, byte* l3_buf, boolean pdp_inactive_flag, boolean ps_vote_flag)
{
  ...
  {
#ifdef FEATURE_DUAL_SIM
    send_gmm_ps_call_status_ind(SM_PS_DEFAULT_PRI, TRUE);
#endif
...
void send_gmm_fast_dormancy_req()
{
  ...
  if (gmm_cmd != NULL)
  {
    ...
    {
      send_gmm_ps_call_status_ind(SM_PS_DEFAULT_PRI, FALSE);
    }
#endif
...
===========================================================================*/
void send_gmm_ps_call_status_ind(byte pri_type, boolean pdp_sig_in_progress)
{
  if(sm_pdp_sig_in_progress_last_sent[pri_type] != pdp_sig_in_progress)
  {
    mm_cmd_type * gmm_cmd;
    gmm_cmd = mm_get_cmd_buf();
    if (gmm_cmd != NULL)
    {
      sm_pdp_sig_in_progress_last_sent[pri_type] = pdp_sig_in_progress;
      ...
      if(pri_type == SM_PS_DEFAULT_PRI)
      {
        gmm_cmd->cmd.gmmsm_ps_call_status_ind.proc_type = SYS_PROC_TYPE_PS_CALL_SIGNALING;
      }
      else
      {
        gmm_cmd->cmd.gmmsm_ps_call_status_ind.proc_type = SYS_PROC_TYPE_HIGH_PRIORITY_PS_CALL;
      }
      MSG_HIGH_DS_2( SM_SUB,"Sending PS CALL STATUS IND to GMM pri_type %d pdp_sig_in_progress %d",
                              pri_type, pdp_sig_in_progress);
      mm_put_cmd (gmm_cmd);
    }
    else
    {
...
void sm_process_establish_resend_ind(gmmsm_establish_resend_ind_T *gprs_ind)
{
  word i;
  int pending_msg;
  pending_msg = 0;
...
void sm_process_gprs_rej(gmmsm_establish_rej_T* gprs_rej)
{
  ...
  ** ----------------------------------------------------------------*/
  ...
  else if(sm_data_call_activity_status_get() == SYS_DATA_ACTIVITY_CLEANUP_REQ)
  {
    MSG_HIGH_DS_0(SM_SUB," Aborting sm gmm conn proc with cause PPD_UNKNOWN_REASON  ");
    sm_abort_gmm_conn_proc((cc_cause_T)PPD_UNKNOWN_REASON); 
  }
#endif
  /* --------------------------------------------------------------------
...
//   word stored_trans_id;
  ...
  if( (req_pdp_cxt == NULL) || (rej_resp == NULL) 
#ifdef FEATURE_MBMS
     #error code not present
#endif    
     )
  {
      MSG_ERROR_DS(SM_SUB,"Memory Allocation failed",0,0,0);
      if(req_pdp_cxt != NULL)
      {
...
#ifdef FEATURE_DUAL_SIM
            rej_resp->as_id = (sys_modem_as_id_e_type)sm_as_id;
#endif
            if (cur_pdp == NULL)
            {
          ...
          else
          {
             ...
             ** -------------------------------------------------------------------*/
             ...
             else
             {
               ...
               else if ((recd_msg == REQUEST_SEC_PDP_ACTIVATION) && (sm_get_nv_nas_rel_compliance() >= NV_NAS_COMPLIANCE_REL12) )
               {
  ...
  if (cur_pdp->pdp_state == PDP_INACTIVE_PENDING)
  {
    ...
    if (!cur_pdp->qos_valid )
    {
      ...
      **primary or sec pdp accept because LLC RAB is not activte at this stage.
      ...
      cur_pdp->cause_type  =  SM_INTERNAL_CAUSE;	

      /* Set the cause type as internal cause */
      cur_pdp->current_cause        = (cc_cause_T)SM_INTERNAL_QOS_NOT_ACCEPTED; 
...
void sm_proc_request_pdp_deactivation(pdp_context_info *cur_pdp)
{
  ...
  else
  {
    ...
    if (status == L3_PARSE_SUCCESS)
    {
      ...
      if (!cur_pdp->teardown_ind.ti_flag)
      {
        if (((ue_revision == UE_REV_PRE_REL99) && (sm_get_nv_nas_rel_compliance() <= NV_NAS_COMPLIANCE_REL99)) ||
							cur_pdp->is_primary_pdp == FALSE)
		{
		  //If TI indicator is off for pre-Rel99 NW/ if PDP is secondary then Deactivate single PDP
		...
		{
          cur_pdp->teardown_ind.ti_flag = TRUE;
		  MSG_HIGH_DS_1(SM_SUB,"Single Primary PDP Deactivation for %d TI is OFF, enforcing it to TRUE", cur_pdp->trans_id);
		}
      }     
      if(cur_pdp->teardown_ind.ti_flag == TRUE)
      {
        pdp_context_info *pri_pdp = NULL;
...
)
{
  ...
  if ((con_id = sm_check_pdp_collision(pdp)) != INVALID_ID &&
      ((MIN_MO_TI <= con_id && con_id <= MAX_MO_TI) || 
	  (MIN_MT_TI <= con_id && con_id <= MAX_MT_TI))
	 )
  {
    if (pdp_list[con_id]->pdp_state == PDP_INACTIVE &&
...
)
{
  ...
  for (index = 0; index < MAX_PDP_NUM; index++)
  {
    if (pdp_list_sim[as_id][index] != NULL && 
        pdp_list_sim[as_id][index]->trans_id != INVALID_ID && 
       ((pdp_list_sim[as_id][index]->pdp_state == PDP_ACTIVE_PENDING) || 
       (pdp_list_sim[as_id][index]->pdp_state == PDP_MODIFY_PENDING) ||
       (pdp_list_sim[as_id][index]->pdp_state == PDP_INACTIVE_PENDING)))
    {
      return TRUE;
    }        
  }
...
DESCRIPTION
  This function locally deatcivates the PDPs which dont have valid GERAN params 
...
===========================================================================*/
void sm_process_irat_ps_service_ind(gmmsm_irat_ps_service_ind_T *irat_ps_srv_ind)
{
  word pdp_index;
  for (pdp_index = 0; pdp_index < MAX_PDP_NUM; pdp_index++)
  {
    if ( pdp_list[pdp_index] != NULL && 
         ...
         pdp_list[pdp_index]->nsapi.nsapi < MAX_NSAPI_NUM &&
		 !(pdp_list[pdp_index]->llc_sapi.valid &&
           pdp_list[pdp_index]->radio_pri.valid)
       )
     {
	   MSG_HIGH_DS_3(SM_SUB, "Locally deactivating PDP TI : %d, LLC SAPI :%d, Radio Pri : %d",pdp_list[pdp_index]->trans_id, pdp_list[pdp_index]->llc_sapi.valid, pdp_list[pdp_index]->radio_pri.valid );
	   //Deactivate all secondary PDPs first, if any
       if (pdp_list[pdp_index]->is_primary_pdp)
       {
		  sm_local_deact_all_associated_pdp_ctxt(pdp_list[pdp_index]);
       }
	   sm_perform_local_pdp_deactivation(pdp_list[pdp_index]);
     }
  }
}
#endif
