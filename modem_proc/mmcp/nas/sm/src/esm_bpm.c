...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/sm/src/esm_bpm.c#3 $
...
)
{
  ...
  {
    //Try to find a match;
    if (mgr_ptr->esm_bp[index] == NULL)
    {
      mgr_ptr->esm_bp[index] = (esm_bearer_resource_proc_T *)esm_modem_mem_alloc(sizeof(esm_bearer_resource_proc_T));
      ...
      /* Initialize funtion ptr*/
      esm_init_bpm_fn_ptr(mgr_ptr->esm_bp[index]);
...
)
{
  ...
  esm_release_esm_bp_ptr(proc_ptr);
  proc_ptr = NULL;
} /* esm_bpm_reset_bp() */
...
esm_bearer_resource_proc_T* esm_bpm_find_proc_state_machine(esm_bpm_T* mgr_ptr, word sdf_id, byte lbi)
{
  ...
  for(index = 0; index < ESM_MAX_BP_PROCS; index++)
  {
    if (mgr_ptr->esm_bp[index]!= NULL)
    {
      if( (mgr_ptr->esm_bp[index]->sdf_id == sdf_id) && (mgr_ptr->esm_bp[index]->lbi == lbi) )
      {
        return mgr_ptr->esm_bp[index];
      }
    }
...
esm_bearer_resource_proc_T*  esm_bpm_find_uicc_proc_state_machine(esm_bpm_T* mgr_ptr, byte pti)
{
  ...
  for(index = 0; index < ESM_MAX_BP_PROCS; index++)
  {
    if(mgr_ptr->esm_bp[index]!= NULL)
    {
      if(mgr_ptr->esm_bp[index]->pti == pti)
      {
...
)
{
  ...
  for(index = 0; index < ESM_MAX_BP_PROCS; index++)
  {
    if(mgr_ptr->esm_bp[index]!= NULL)
    {
      if(mgr_ptr->esm_bp[index]->esm_proc_state_ptr->state() != ESM_BP_INACTIVE)
      {
#ifdef FEATURE_LTE_REL9
        if((mgr_ptr->esm_bp[index]->pending_msg_ptr->esm_out_msg.nas_hdr.msg_id 
                                                     == PDN_CONNECTIVITY_REQUEST) &&
           emm_check_if_pdn_conn_req_for_emergency_bearer_services(mgr_ptr->esm_bp[index]->pending_msg_ptr->esm_out_msg.\
                                      pdn_connectivity_req.req) &&
...
)
{
  ...
  switch (cmd_ptr->cmd.msg_hdr.id)
  {
    case NAS_ESM_PDN_CONNECT_PACKED_CNF:
      ...
      switch(cmd_ptr->cmd.esm_pdn_connectivity_pkd_cnf.status)
      {
        ...
        case ESM_MSG_MODIFIED:
          ...
          cmd_ptr->cmd.esm_pdn_connectivity_req.pdn_data.protocol_config_options =
          ...
          {
            cmd_ptr->cmd.esm_pdn_connectivity_req.pdn_data.low_priority_signaling = 
    ...
    case NAS_ESM_DATA_IND:
      event.esm_in_msg = (dword)NAS_ESM_DATA_IND;
      nas_event_report_payload(EVENT_LTE_ESM_INCOMING_MSG,
    ...
    case NAS_ESM_FAILURE_IND:
      event.esm_in_msg = (dword)NAS_ESM_FAILURE_IND;
      nas_event_report_payload(EVENT_LTE_ESM_INCOMING_MSG,
                   sizeof(event), (void*)&event,sm_message_as_id);
    ...
    case NAS_ESM_PDN_DISCONNECT_REQ:
      event.esm_in_msg = (dword)NAS_ESM_PDN_DISCONNECT_REQ;
      nas_event_report_payload(EVENT_LTE_CM_INCOMING_MSG,
                     sizeof(event), (void*)&event,sm_message_as_id);
    ...
    case NAS_ESM_PDN_CONNECTIVITY_ABORT_REQ:
      event.esm_in_msg = (dword)NAS_ESM_PDN_CONNECTIVITY_ABORT_REQ;
      nas_event_report_payload(EVENT_LTE_CM_INCOMING_MSG,
                    sizeof(event), (void*)&event,sm_message_as_id);
    ...
    case NAS_ESM_GET_PDN_CONNECTIVITY_REQ_IND:
      event.esm_in_msg = (dword)NAS_ESM_GET_PDN_CONNECTIVITY_REQ_IND;
      nas_event_report_payload(EVENT_LTE_ESM_INCOMING_MSG,
                        sizeof(event), (void*)&event,sm_message_as_id);
    ...
    case NAS_ESM_BEARER_RESOURCE_MODIFICATION_REQ:
      event.esm_in_msg = (dword)NAS_ESM_BEARER_RESOURCE_MODIFICATION_REQ;
      nas_event_report_payload(EVENT_LTE_CM_INCOMING_MSG, sizeof(event), 
                           (void*)&event,sm_message_as_id);
...
void bpm_get_bp_status(const esm_bpm_T* mgr_ptr)
{
  ...
  for(index = 0; index < ESM_MAX_BP_PROCS; index++)
  {
    if(mgr_ptr->esm_bp[index]!= NULL)
    {
      ...
      MSG_HIGH_DS_2(SM_SUB, "ESM: BP id = %d and pti=%d\n",
                    index, mgr_ptr->esm_bp[index]->pti);
    }
  }
}
...
)
{
  ...
  if(proc_ptr != NULL)
  {
    ...
    if((proc_ptr->esm_proc_state_ptr->state() != ESM_BP_INACTIVE))
    {
  ...
  {
    MSG_ERROR_DS_1(SM_SUB, "ESM: Proc is already released on timer %d expiry",
                  cmd_ptr->cmd.esm_timer_expired_ind.timer_id);
  }

  event.esm_timer_event = (byte)(cmd_ptr->cmd.esm_timer_expired_ind.timer_id);
  nas_event_report_payload(EVENT_LTE_ESM_TIMER_EXPIRY,
...
)
{
  ...
  for(index=0; index < ESM_MAX_BP_PROCS; index++)
  {
    if(mgr_ptr->esm_bp[index]!= NULL)
    {
      if(mgr_ptr->esm_bp[index]->lbi == bearer_id)
...
FUNCTION  ESM_RESEND_PEND_PROC

DESCRIPTION
  This function re-sends pending PDN CONN REQ when receives indication 
  from EMM
...
===========================================================================*/
void esm_resend_pend_proc(esm_bpm_T  * mgr_ptr)
{
  esm_bearer_resource_proc_T* proc_ptr = NULL ;
  byte  index = 0;
  esm_failure_ind_T failure_ind;
  ...
  for(index = 0; index < ESM_MAX_BP_PROCS; index++)
  {
    ...
    if(proc_ptr != NULL)
    {
      if((proc_ptr->esm_proc_state_ptr != NULL && proc_ptr->pending_msg_ptr != NULL) &&
         (proc_ptr->esm_proc_state_ptr->state() == ESM_BP_ACTIVE) &&  
         (proc_ptr->timer_ptr != NULL) &&
         proc_ptr->pending_msg_ptr->esm_out_msg.nas_hdr.msg_id == PDN_CONNECTIVITY_REQUEST)
      {
        memset((void *)&failure_ind,0,sizeof(esm_failure_ind_T));
        /** Updating the cause same as that of the No service case */
        failure_ind.emm_cause.valid = TRUE;
        failure_ind.emm_cause.local_cause = LTE_NAS_SERVICE_REQ_FAILED_NO_THROTTLE;
        failure_ind.emm_failure_cause.cause_type = LTE_NAS_IRAT_INVALID_STATE;
        failure_ind.emm_failure_cause.cause.state = EMM_INVALID_STATE_TEMP;
        MSG_HIGH_DS_1(SM_SUB, "ESM: Resend PDN CONN REQ procedure PTI %d", proc_ptr->pti);
        if(proc_ptr->esm_proc_state_ptr->esm_proc_process_timer_expiry_or_lower_layer_failure(proc_ptr, &failure_ind) == FALSE)
        {
          esm_bpm_release_proc_resources(mgr_ptr, proc_ptr);
          proc_ptr = NULL;
