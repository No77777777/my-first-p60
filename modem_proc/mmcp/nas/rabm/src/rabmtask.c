...
$Header: //components/rel/mmcp.mpss/7.9.0/nas/rabm/src/rabmtask.c#1 $    $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...
#else
    as_id = cmd_ptr->cmd.tc_mode_change_ind.as_id;
...
void rabm_process_rrc_cmd( rabm_cmd_type* cmd_ptr )
{
  ...
  * ----------------------------------------------------------------*/
  if (ue_in_test_mode)
  {
    ...
    //To support Loopback mode#4, DS also needs the RAB notifications from RRC
    //return;
  }
#endif /* FEATURE_TC */
      ...
      else if (rab_list[rab_id].rab_state == RAB_REESTABLISH_PENDING)
      {
        ...
        else
        {
          MSG_ERROR_DS_0( RABM_SUB, "Unable to get a GMM command buffer");
		        ...
		        ( (pdp_list[GET_TRANSID(rab_id)]->pdp_state == PDP_MODIFY_PENDING) && ((pdp_list[GET_TRANSID(rab_id)]->mt_orig_type == TRUE))) ) 
      {
        // If the PDP for this corresponding RAB is ACTIVE, send the
        // CM a RAB-ESTABLISH Indication command.
	MSG_MED_DS_0( RABM_SUB,"Sending a RABM_RAB_ESTABLISH_IND to CM in ACTIVE and MT MODFIY PENDING state");
        send_rabmcm_cmd(rab_id, RABM_RAB_ESTABLISH_IND, (cc_cause_T) REASON_NONE);
        change_state = TRUE;
#ifdef FEATURE_NAS_REL12
        /* Send RAB established ind to GMM so that it stops the timer T3340.*/
...
boolean rabm_rabid_validate_per_sub(sys_modem_as_id_e_type as_id, rab_query_T query_type, byte rab_id)
#else
...
#ifdef FEATURE_DUAL_SIM
      cmd_ptr->cmd.rabm_status_req.as_id = (sys_modem_as_id_e_type)sm_as_id;
#endif	  
    }
...
#ifdef FEATURE_DUAL_WCDMA
      tc_cmd_ptr->cmd.rabm_rab_est_ind.as_id = rabm_cmd_ptr->cmd.rrc_establish_ind.as_id;
#endif
    }
...
#ifdef FEATURE_DUAL_WCDMA
      tc_cmd_ptr->cmd.rabm_rab_est_ind.as_id = rabm_cmd_ptr->cmd.rrc_release_ind.as_id;
#endif
    }
    else
    {
...
#ifdef FEATURE_DUAL_SIM
        (sm_check_pdp_procedure_pending((sys_modem_as_id_e_type)sm_as_id)) ||
#else
        (sm_check_pdp_procedure_pending()) ||
#endif
        rabm_check_procedure_pending_for_SCRI()))
    {
      rab_list[rab_id].rab_state = RAB_REESTABLISH_PENDING;
...

{
  ...
  {
#ifdef FEATURE_DUAL_DATA
    if(rab_list_sim[as_id][rab_id].rab_state == RAB_SERVICE_REQ_SENT) 
#else
    if(rab_list[rab_id].rab_state == RAB_SERVICE_REQ_SENT)         
#endif
    {    
   ...
   MSG_HIGH_DS_2(RABM_SUB, "rabm_pack_uplink_data_status:RAB id %d as id %d",rab_id,as_id);
}
