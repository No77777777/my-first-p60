...
$Header: //components/rel/mmcp.mpss/7.9.0/api/mmtask_v.h#1 $ $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...
typedef struct 
{
 ...
 uint32 user_data;
 nas_mmgsdi_purge_temporary_identity_callback_type mmgsdi_cb;
 sys_modem_as_id_e_type  as_id;
}sim_mm_guti_purge_type_T;
...
typedef enum
{
  ...
  MMCM_ECALL_INACTIVITY_TRIGGER,
  MMCM_ECALL_REMAINING_TIMER_VALUE,
  MMCM_RPM_UPDATE_FTAI_LIST,
#ifdef FEATURE_CRAT_MSIM_TEST_FRAMEWORK
  MMCM_LTE_SUSPEND_REQ,
  MMCM_LTE_RESUME_IND,
#endif
  MMCM_CMD_MAX
} cm_mm_cmd_e_T ;
...
} cm_mm_ecall_inactivity_trigger_T ;

typedef struct
{
  IMH_T message_header;

  sys_ecall_timer_id_e_type timer_id;

  uint32 remaining_timer_value;

  sys_modem_as_id_e_type as_id;

} cm_mm_ecall_remaining_timer_value_T ;
...
typedef struct mm_qsh_message_ind_s
{
  IMH_T                      message_header;
  qsh_client_cb_params_s     param;
}mm_qsh_message_ind_s_type;


typedef struct mm_non_acq_param_change_ind_s
{
  IMH_T                                    message_header;
  sys_modem_as_id_e_type                   as_id;
  sys_nonacq_info_e_type                   non_acq_info;
  sys_voice_domain_pref_e_type             voice_domain_pref;
} mm_non_acq_param_change_ind_s_type;
...
typedef union mm_int_cmd_type_tag {
  ...
  mmr_ps_detach_req_s_type             mmr_ps_detach_req;
   ...
   mmr_refresh_stage_ind_s_type            mmr_refresh_stage_ind;  
   mmr_secure_call_ind_s_type              mmr_secure_call_ind;
   mmr_dds_switch_ind_s_type               mmr_dds_switch_ind;
   mmr_high_priority_activity_ind_s_type   mmr_high_priority_activity_ind;
   mmr_gw_plmn_sel_end_ind_s_type          mmr_gw_plmn_sel_end_ind;
   mmr_t3245_expiry_ind_s_type             mmr_t3245_expiry_ind; 
...
rr_plmn_resel_fail_ind_T         rr_plmn_resel_fail_ind;
   ...
   sim_mm_guti_purge_type_T    gsdi_guti_purge_ind;
   ...
   cm_mm_ecall_inactivity_trigger_T cm_mm_ecall_inactivity_trigger;
   cm_mm_ecall_remaining_timer_value_T  cm_mm_ecall_remaining_timer_value;
   cm_mm_update_ftai_list_T cm_mm_update_ftai_list;
#ifdef FEATURE_CRAT_MSIM_TEST_FRAMEWORK
   cm_mm_lte_suspend_req_T  cm_mm_lte_suspend_req;
   cm_mm_lte_resume_ind_T   cm_mm_lte_resume_ind;
#endif

/* MCFG ==> MM */
   mm_mcfg_refresh_ind_s_type       mcfg_refresh_ind;
/* QSH ==> MM */
   mm_qsh_message_ind_s_type        qsh_message_ind;
   mm_non_acq_param_change_ind_s_type  non_acq_param_change_ind;
} mm_int_cmd_type_T;
...
SIDE EFFECTS
  None
===========================================================================*/
extern boolean mm_is_csg_app_search_allowed
(
  sys_csg_search_type_e_type          csg_search_type,
  sys_csg_selection_config_e_type     config_option,
  sys_modem_as_id_e_type              as_id
