...
/*==============================================================================
 ...
 $Header: //components/rel/mmcp.mpss/7.9.0/api/qmi_mmode_msgr_msg.h#2 $
...
typedef enum {
  MMODE_QMI_VOICE_ECALL_MSD_TRANSMISSION_STATUS_NONE    = 0x00,
    /**< No eCall MSD Transmission Status */

  MMODE_QMI_VOICE_ECALL_MSD_TRANSMISSION_STATUS_SUCCESS = 0x01,
    /**< eCall MSD Transmission Status Success */

  MMODE_QMI_VOICE_ECALL_MSD_TRANSMISSION_STATUS_FAILURE = 0x02,
    /**< eCall MSD Transmission Status Generic Failure */

  MMODE_QMI_VOICE_ECALL_MSD_TRANSMISSION_STATUS_MAX = 0xFFFFFFFF
} mmode_qmi_voice_ecall_msd_transmission_status_e_type;
...
typedef struct
{
  ...
  mmode_qmi_voice_ecall_msd_transmission_status_e_type ecall_msd_transmission_status;

} mmode_qmi_voice_ecall_status_ind_s;

/** @brief Message structure for the data associated 
  with command to send the add-participant operation status from 
  eCall to QMI to AP. 
*/
typedef struct
{
  ...
  mmode_qmi_voice_ecall_status_ind_s ecall_status; 

} mmode_qmi_voice_ecall_status_ind_msg_type;
...
typedef struct
{
 ...
 /** @brief Message structure for the data associated 
  with command to inform VOLTE state to IMS
*/
typedef struct
{
  msgr_hdr_s  msg_hdr;
  ...
  boolean     is_volte_enabled;

} mmode_qmi_volte_status_chg_cmd_msg_type;
...
} mmode_qmi_nas_arfcn_list_ind_msg_type;

/** @brief Message structure for the data associated 
  with Ind to inform PRIORITIZE_LTE
*/
typedef struct
{
...
} mmode_qmi_nas_prioritize_lte_ind_msg_type;
...
typedef struct
{
  ...
  sys_access_service_e_type service_type;

  /*!< Current Barring status*/
  boolean barring_status;    
  uint32  barring_time;
...
enum
{
 ...
 /* QMI_VOICE_CONF_PARTICIPANTS_INFO_CMD  - Interface to pass conference participants info */
  ...
  /* QMI_VOICE_ECALL_STATUS_IND  - Interface to pass eCall status to apps */
  MSGR_DEFINE_UMID(QMI, VOICE, IND, ECALL_STATUS, 0x0C, mmode_qmi_voice_ecall_status_ind_msg_type),
  ...
  MSGR_DEFINE_UMID(QMI, VOICE, IND, VICE_DIALOG, 0x0D, mmode_qmi_voice_viceDialog_info_ind_msg_type),
   ...
   /* QMI_NAS_VOLTE_STATE_CHG_CMD  - Interface to inform VOLTE state to IMS */
  MSGR_DEFINE_UMID(QMI, NAS, CMD, VOLTE_STATE_CHG, 0x0E, mmode_qmi_volte_status_chg_cmd_msg_type),

  /* QMI_NAS_ARFCN_LIST_IND  - Interface to indicate ARFCN list from current active RAT 
    ** QMI receives this message from current RAT */
  MSGR_DEFINE_UMID(QMI, NAS, IND, ARFCN_LIST, 0x0F, mmode_qmi_nas_arfcn_list_ind_msg_type),

  /* QMI_NAS_PRIORITIZE_LTE_IND  - Interface to indicate prioritize lte scan to the active RAT */
  MSGR_DEFINE_UMID(QMI, NAS, IND, PRIORITIZE_LTE, 0x10, mmode_qmi_nas_prioritize_lte_ind_msg_type),
