...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/api/cmll_v.h#1 $
...
/* Type for CM_E911_LTE_FULL_SRV_ADDED_F
*/
typedef struct {
  cm_hdr_type                 hdr;                               /* header */
...
} cm_e911_lte_full_srv_added_type;
...
typedef struct {
  cm_hdr_type           hdr;                                     /* header */
  emcb_s_type           emcb_info;       /* emergency callback mode */
...
typedef struct {
  cm_hdr_type             hdr;                                     /* header */
  multimode_stack_e_type  stack_id;
  /* stack on which CS emerg Scan fail  */
} cm_cs_emerg_scan_fail_type;

/* Type for CM_EMERG_SRCH_FAIL_F
*/
typedef struct {
  cm_hdr_type             hdr;          /* header */
  multimode_stack_e_type  stack_id;     /* stack on which emerg scan failed  */
  sd_ss_mode_pref_e_type  mode_pref;    /* Mode_pref on which ACQ failed */
} cm_emerg_srch_fail_type;
...
typedef struct {
  cm_hdr_type                    hdr;  /* header*/
  uint8                          stack_id;   /* stack indicates which stack was blocked */
...
typedef struct {
  ...
  multimode_stack_e_type         stack_id; /* indicates which stack was blocked */
} cm_suspend_ss_s_type;

typedef struct {
  cm_hdr_type                    hdr;   /* header */
  uint8                          active_stk_bm;
}cm_subs_chg_done_type;

typedef struct {
  cm_hdr_type                    hdr;   /* header */
  uint8                          active_subs;
}cm_mcfg_proceed_with_refresh_type;
...
/* structure to store the information received in QRARB report
*/
...
/* Structure to store Info related to the DS CM_VA_LCH_RSP response
*/
typedef struct cm_va_lch_rsp_s
{
  sys_modem_as_id_e_type asubs_id;
  /* Subs ID for which response is being sent */
  cm_call_hold_state_e_type call_hold_state;
  /* call hold state for which VA is responding */
  uint32 user_data;
  /* User Data sent by CM when command was sent to VA */
}cm_va_lch_rsp_s_type;
...
typedef union cm_generic_rpt_u
{
  ...
  cm_va_lch_rsp_s_type           va_lch_rsp;
    /* Payload associated with DS CM_VA_LCH_RSP response for LCH
    ** command sent to VA.
    */

  cm_qrarb_rpt_s_type            qrarb_info;
...
FUNCTION cm_is_esr_allowed_in_srlte_per_subs
