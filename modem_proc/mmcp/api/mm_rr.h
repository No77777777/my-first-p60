...
$Header: //components/rel/mmcp.mpss/7.9.0/api/mm_rr.h#1 $ $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...
typedef enum
{
  ...
  ,RR_IRAT_ALLOWED_STATUS_IND      = 0x0f
  ...
  ,RR_PREEMPTION_IND = 0x9f
  ,RR_GMM_RETX_PDU_TUNEAWAY_FAIL_IND = 0xA0
  ,RR_PLMN_RESEL_FAIL_IND = 0xA1
...
typedef enum
{
  ...
  RR_MM_MO_PS_CALL,
  RR_MM_NORMAL_ACQ,
  RR_MM_FULL_RAT_ACQ	/*This is supposed to be a full band scan*/
...
/*************************************************************/
/* RR_GMM_RETX_PDU_TUNEAWAY_FAIL_IND primitive                      */
/* FROM : RR                                                 */
/* TO   : GMM                                                */
/* DESCRIPTION :                                             */
/* Indicates that NAS needs to retransmits the last GMM signaling in case GL1 allows tuneaway gap for more than 300 ms  */
/*************************************************************/
...
} rr_gmm_retx_pdu_tuneaway_fail_ind_T;
...
typedef struct
{
  ...
  ** in bplmn search by lower layers*/  
   ...
   /* Below variables are only applicable to Periodic frequency search */ 
...
/*************************************************************/
/* RR_PREEMPTION_IND primitive                              */
/* FROM : RR                                                 */
/* TO   : MM                                                 */
/* DESCRIPTION :                                             */
/* Indicates that existing RR connection is been pre-empted, */
/* due to some high priority activity on other SUB */
/*************************************************************/
typedef struct
{
   IMH_T                   message_header;
...
/*************************************************************/
/* RR_DDS_SUB_IND primitive */
/* FROM : MM   */
/* TO   : RR   */
/* DESCRIPTION :   */
/* Message sent to indicate the DDS sub */
/*************************************************************/
typedef struct
{
  IMH_T                            message_header;
  sys_modem_as_id_e_type as_id; /* New DDS sub */
}rr_dds_sub_ind_T;  
...
typedef struct
{
  ...
  byte                     nv_g2l_support_in_ptm_mode;
...
/*************************************************************
 * RR_IRAT_ALLOWED_STATUS_IND   primitive
 * FROM : MM
 * TO   : RR
 * DESCRIPTION :
 * Used by MM to enable/disable lte reselection, lte redirection, 
 *  wcdma reselection & redirection
 *************************************************************/
typedef struct
{
    IMH_T                        message_header;
    boolean                      lte_reselection_allowed;
    boolean                      lte_redir_allowed;
    boolean                      wcdma_resel_redir_allowed;
    sys_modem_as_id_e_type       as_id;
} rr_irat_allowed_status_ind_T;
...
extern boolean mm_get_updated_lai(inter_task_lai_T *rr_lai, const sys_modem_as_id_e_type as_id);
...
typedef struct
{
   ...
   inter_task_lai_T          lai;     
   
   sys_modem_as_id_e_type    as_id;
} rr_plmn_resel_fail_ind_T;
