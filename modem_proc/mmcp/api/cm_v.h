...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/api/cm_v.h#1 $
...
#define NV_MODE_PREF_ANY_BUT_GSM     ((nv_mode_enum_type)69)

#define NV_MODE_PREF_ANY_BUT_GSM_GPS  ((nv_mode_enum_type)70)


#define NV_MODE_PREF_AMPS_CDMA_HDR_WCDMA_TDS  ((nv_mode_enum_type)71)


#define NV_MODE_PREF_AMPS_CDMA_HDR_GSM_TDS ((nv_mode_enum_type)72)

#define  NV_MODE_HDR_TDS_WCDMA     ((nv_mode_enum_type)73)
/* Service is restricted to HDR, TDS or WCDMA */

#define  NV_MODE_HDR_TDS_WCDMA_LTE    ((nv_mode_enum_type)74)
...
#define CM_MCC_CHINA                460
#define CM_MCC_MACAU                455
#define CM_MCC_JAPAN                440
#define CM_MCC_SOUTH_KOREA          450
#define CM_MCC_HONGKONG             454
#define CM_MCC_NETHERLANDS          204

#define CM_MNC_CHINATEL_CDMA_DEFAULT     3
#define CM_MNC_CHINATEL_3GPP_DEFAULT    11
#define CM_MNC_CHINATEL_3GPP_NETHERLANDS 4
#define CM_MNC_CHINATEL_3GPP_HONGKONG_1  3
#define CM_MNC_CHINATEL_3GPP_HONGKONG_2  4
...
typedef enum cm_va_evt_e {
  CM_VA_EVT_NONE = -1,
    /* Internal */
  CM_VA_EVT_LCH_PROC,
  /* Event to notify LCH to Voice Agent */
  CM_VA_EVT_LCH_CALL_END,
  /* Event to notify VA about voice/vt call end on SUBS
  as LCH can happen for Voice or VT only */
  CM_VA_EVT_MAX
  /* Internal */
}cm_va_evt_e_type;
...
typedef enum
{
 CM_SUPS_TEMPORARY_MODE = 0,
 CM_SUPS_PERMANENT_MODE
}cm_ue_based_sups_mode_e_type;
...
typedef struct
{
  ...
  /* subs id on which LCH procedure is being performed */
  cm_call_hold_state_e_type call_hold_state;
  /* state to which this subs is being moved, active or Hold. */

}cm_va_lch_params_s_type;

typedef struct
{
  sys_modem_as_id_e_type asubs_id;
  /* subs id on which LCH procedure is being performed */
}cm_va_call_end_params_s_type;
...
typedef union
{
  cm_va_lch_params_s_type lch_params;
  /* parameters for Local Call hold */
  cm_va_call_end_params_s_type call_end_params;
  /* parameters for Call end */
}cm_va_evt_param_u_type;
...
typedef enum cm_sub_clnup_reason_e {
  ...
  CM_SUB_CLNUP_REASON_SUBS_NOT_AVAIL_REFRESH,/* 3- subscription is unavailable(UIM triggered) due 
                                              to refresh*/
...
/* Type for Callback registered by Voice agent to CM.
So that CM can inform LCH operation to Voice Agent
cmd_id : Command to VA
Params : Parameters for command being sent
user_data: this would be sent by CM to VA, and VA can send it back in response
                for this command to CM. Later it can be used for any user data that
               CM needs back in response. NULL is also valid Value should be treated
                as Void* so that it can point to any structure

Return Value
True : If VA accepts command, and then CM need to wait for VA response
FALSE : VA did not accepted the command, CM need not wait for VA response */

typedef boolean (cm_va_evt_notif_f_type)(cm_va_evt_e_type , cm_va_evt_param_u_type , uint32 );
...
===========================================================================*/
extern boolean cm_ph_cmd_voc_activity1(
...
SIDE EFFECTS
  None
==========================================================================*/
extern void cm_unlock_hdr_resources1(
...
DESCRIPTION
  ...
  Function that reads the sets  the mcc value read from the SIM
...
DEPENDENCIES
  None
===========================================================================*/
extern void cmmmgsdi_set_imsi_mcc
(
  word mcc
);
...
===========================================================================*/
extern void cmsds_activate_hdr_tunnel_req (
  sys_modem_as_id_e_type  asubs_id
);
...
===========================================================================*/
extern void cmsds_deactivate_hdr_tunnel_req (
  sys_modem_as_id_e_type  asubs_id
);
#endif
...
FUNCTION cmsds_check_if_ok_to_update_acq_status

DESCRIPTION
  This function returns whether it is ok to update the acq_status to TRM_NO_RESOURCE
...
===========================================================================*/
extern boolean cmsds_is_there_ext_volte_or_cellular_call_per_sub
(
  sys_modem_as_id_e_type      as_id
);
/*===========================================================================
...
===========================================================================*/
extern boolean cmsds_is_ext_ims_call_active_per_device(void);
...
DESCRIPTION
  This function returns whether there is active ext volte call on either of the subs
...
===========================================================================*/
extern boolean cmsds_is_ext_volte_call_active_per_device(void);
...
===========================================================================*/
extern cm_call_id_type cmcall_is_there_volte_call(void);
...
FUNCTION cm_va_evt_cb_reg

DESCRIPTION
  registration function for VA to register callback. When LCH procedure would be triggered
  CM will use this callback to send event to Voice Agent
...
===========================================================================*/
boolean cm_va_evt_cb_reg(cm_va_evt_notif_f_type * va_evt_cb_func);
...
===========================================================================*/
extern nv_mode_enum_type      cmutil_map_mode_pref_cm2nv(

       cm_mode_pref_e_type    cm_mode
);
...
===========================================================================*/
extern cm_mode_pref_e_type cmutil_map_mode_pref_nv2cm( nv_mode_enum_type  nv_mode );
...
FUNCTION cmcall_is_there_any_cdma_call_per_sub

DESCRIPTION
  Checks is there any call object allocated for CDMA, considers 1XCSFB calls as well

DEPENDENCIES
  None

RETURN VALUE
  boolean:  TRUE if 1x call is found, FALSE otherwise

SIDE EFFECTS
  none

===========================================================================*/
extern boolean cmcall_is_there_any_cdma_call_per_sub(sys_modem_as_id_e_type  asubs_id);
/*===========================================================================
...
extern sys_ims_reg_state_e_type cmsds_get_ims_voice_reg_status
(
  sys_modem_as_id_e_type    as_id,
  sys_sys_mode_e_type       sys_mode
);
...
===========================================================================*/
cm_call_id_type cmcall_is_there_csfb_call_per_sub
(
  sys_modem_as_id_e_type  asubs_id,
...
extern sys_ims_reg_state_e_type cmsds_get_ims_sms_reg_status
(
  sys_modem_as_id_e_type    as_id,
  sys_sys_mode_e_type       sys_mode
);
...
===========================================================================*/
extern boolean cmefs_get_modem_freq_info_supported( sys_modem_as_id_e_type asubs_id );
...
FUNCTION cmcall_is_there_cs_call_per_sub

DESCRIPTION
Indicates
1. If a Voice or Emergency call is active
2. Call info_type is either GW_CS or CDMA and
3. Call origination is already sent to Protocols (1x or NAS)
...
===========================================================================*/
cm_call_id_type cmcall_is_there_cs_call_per_sub
(
  sys_modem_as_id_e_type  asubs_id,
...
FUNCTION cmmsc_set_force_reg_pending

DESCRIPTION
set force reg pending to desired value
...
/* Structure containing info returned from the
** cm_get_mt_page_concurrency_config API
*/
typedef struct cm_mt_page_concurrency_config_s {
...
} cm_mt_page_concurrency_config_s_type;
