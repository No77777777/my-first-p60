...
$Header: //components/rel/mmcp.mpss/7.9.0/api/mn_uasms.h#1 $ $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...
typedef enum
{
  NAS_REGULAR_SMS,      /**< Normal SMS. */
  NAS_E911_SMS,         /**< SMS to/from 911. */
  NAS_SMS_MAX
} nas_msg_e_type;
...
typedef struct
{
  reject_type_enum_T ota_reject_type;
  reject_cause_T     ota_reject_cause;
}lower_layer_cause_info_T;
...
typedef  struct
{
   ...
   lower_layer_cause_info_T ota_cause;     
   byte     tpdu_length;
//   byte     tpdu[TPDU_SIZE];
   byte     tpdu[RP_USER_DATA_TPDU_LENGTH];
