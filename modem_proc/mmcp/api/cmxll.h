...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/api/cmxll.h#1 $
...
typedef enum {
  ...
  CM_DEREG_FAIL_PENDING_DEACT = 3,
  /* 1x will use this cause code when there is already a deactivate
   * command pending in their queue */

  CM_DEREG_FAIL_ALREADY_DEACT = 4,
  /* 1x will use this cause code when it is already de-activated */
  
  CM_DEREG_SUCCESS            = 5,

}cm_dereg_rsp_cause_e_type;

/* Action for 1x to take upon receiving a cmd from CM with a subscription ID
** that does not match the current 1x sub that 1x has received from MMOC
*/
typedef enum {
  MC_HANDLE_CMD,       /* 1x should handle the cmd normally */
  MC_IGNORE_CMD,       /* 1x should ignore/drop the cmd, but should not crash */
  MC_UNEXPECTED_CMD    /* 1x should crash since the mismatch is never expected */
} cm_mc_sub_diff_action_e_type;
...
/* Type used to pass information to the CM API cm_get_1x_sub_mismatch_action
*/
typedef struct {
  /* name of command sent to 1x
  */
  mc_name_type  cmd;

  /* the current 1x asubs_id, which should be the last value indicated to 1x through MMOC
  */
  sys_modem_as_id_e_type  asubs_id;
} cm_mc_sub_diff_action_s_type;
...
extern void cm_mt_dbm_msg_extn
(
  uint8   dbm_msg_type,                             /* The data burst type */
  uint8   format,  /* Format of the data burst type. Relevant only for SMS */
  const uint8*  data,                                  /* Pointer to the paylaod */
...
SIDE EFFECTS
  None
==========================================================================*/
extern void cm_send_hdr_unlock_rf_cmd(sys_modem_as_id_e_type        asubs_id);
/*========================================================================
...
DESCRIPTION
  ...
  match the current 1x sub (sub which MMOC has told 1x is the 1x sub). This function will
