...
$Header: //components/rel/mmcp.mpss/7.9.0/api/naslog.h#1 $ $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...
#define MAX_NUMBER_FORBIDDEN_PLMNS    128
...
#define LOG_MAX_NUMBER_LTE_CELL_LIST      100
#define LOG_MAX_PPLMN_RATS                3
#define LOG_MAX_EARFCN_NUM                4
#define LOG_SYS_MAX_ACQ_PREF_RAT_LIST_NUM  10
#define LOG_LTE_RRC_MAX_PLMN_LIST_SIZE  40
//#pragma pack(1)
...
typedef PACKED struct PACKED_POST{
  ...
  /*cell Id*/
   uint16 cell_id;
   /*count  for lte avaibale*/
   int8 count;
   /*earfcn count  and its value, if less than 4 then 0xFFFF, valid only for LTE avaiable list*/
   uint8  ecount;
   uint32 earfcn_nas[LOG_MAX_EARFCN_NUM];
   /*FOR LTE not avaiable Value id 0XFFFFFFFF*/
   uint32 timer_count;
...
LOG_RECORD_END
LOG_RECORD_DEFINE(LOG_UMTS_NAS_LTE_CELL_LIST_LOG_PACKET_C)
  ...
  /*Log Version*/
   ...
   /*list type to distinguish LTE Available /Not Available*/
   uint8 list_type;

   /*Rat f Database*/
   uint8 rat;

   /* Length of LTE databse List*/
   uint32  length;
   /* PLMN ID */
   log_plmn_id_type plmn;
	 	 	      
   /*List of LTE avaiable data*/
   log_lte_cell_list   lte_cell_list[LOG_MAX_NUMBER_LTE_CELL_LIST];
