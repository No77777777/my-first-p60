...
$Header: //components/rel/mmcp.mpss/7.9.0/api/tc_cm.h#1 $ $DateTime: 2021/06/15 22:40:27 $ $Author: pwbldsvc $
...
/* --------------------------------------------------------------------------
** Primitive : CMTC_CLOSE_LB_READY_IND, used by CM to notify TC that DS 
**             is Ready for UL/DL loopback for MODE#4 as Water Mark Registartion 
**             with Lowerlayers is done and TC can send CLOSE COMPLETE OTA
** ------------------------------------------------------------------------*/
typedef struct cmtc_close_lb_ready_ind
{
  boolean close_loopback;
  sys_modem_as_id_e_type  as_id;
} cmtc_close_lb_ready_ind_T;
