...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/api/public/cm_gw.h#1 $
...
typedef struct {
    ...
    byte                                 fwd_to_uri_length;
     /**< uri length. */

    char                                 *fwd_to_uri;
     /**< Pointer to uri content. */
} cm_forward_to_addr_uri_s_type;
...
/** Whether the UE requests to establish a new connection to a PDN or to
    ...
    1 0 0 emergency
    1 1 0 handover of emergency bearer services 
*/
typedef enum
{
...
typedef enum
{
  EMM_CON_REL_LOCAL_VALUE_NONE,
  EMM_CON_REL_PDCP_OUT_OF_SYNC
}lte_nas_rel_cause_value_enum_T;
...
typedef struct
{
  ...
  lte_nas_rel_cause_value_enum_T  local_cause_value; 
...
typedef enum
{
  ...
  SM_INTERNAL_QOS_NOT_ACCEPTED
  /* NAS sends this internal cause to CM/DS when NAS initiates PDP deactivation due to QoS Validation Failure*/
}sm_cm_status_T;
...
typedef struct reattempt_ind_s
{
  ...
  byte    ratc_val;                 /* Re-attempt indicator value. */
} reattempt_ind_T;  
