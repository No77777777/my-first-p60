...
/*
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/api/public/policyman.h#2 $
...
typedef enum
{
  ...
  POLICYMAN_ITEM_CA_BAND_COMBOS         = 11,
  POLICYMAN_ITEM_SERVICE_DOMAIN         = 12,
  POLICYMAN_ITEM_IMS_CONFIG             = 13,
  POLICYMAN_ITEM_LTE_FEATURE_MASK       = 14,
} policyman_item_id_t;
...
/**
@brief  Sandbox structure for the sandboxing APIs.
*/
typedef struct policyman_sandbox_t  policyman_sandbox_t;

/*=============================================================================
  Policyman Sandbox APIs
=============================================================================*/

/*-------- policyman_create_sandbox  --------*/
/**
@brief Create a sand box object.

The items returned have had a reference added to them, so once the caller is
done with the item it should call REF_CNT_OBJ_RELEASE_IF() on the pointer to
the object.

@return pSandbox Pointer to the storage of policyman objects needed for sandboxing.
                The caller must call REF_CNT_OBJ_RELEASE_IF() on when it no longer needs
                to sandbox.
*/
...
/**
@brief  Get an item collection which is an attachment to the
           MSGR message. If you call this function YOU MUST call
           ref_cnt_obj_release() on the pCollection after its usage 
           to release the reference.

@param[in]  pMsg Pointer to a MSGR message received from policyman 

@return
  Pointer to the collection of items.
*/
...
/**
@brief  Return the RF band information to be used for scanning.
...
/**
 ...
 @[param[in] mccOnly: If set to TRUE then only MCC will be checked, otherwise whole PLMN will be checked
 @param[in] pSvdPossible: Pointer to boolean to store if PLMN is allowed for SVD operation
 @param[in] subsId: subs ID to check for SVD operation
...
/**
@brief  Return the CA combo to be used.

@param[in]  pItem     Pointer to a POLICYMAN_ITEM_CA_BAND_COMBOS item.
@param[out] caCombo   Pointer to storage for a pointer to the band combos string.
                                  This pointer may become invalid once pItem is released, either directly or by releasing the MSGR message containing it.
                                 Thus, the caller must handle the string (either by copying it into memory it owns or by parsing it) prior to releasing pItem.

@return
  - POLICYMAN_STATUS_SUCCESS if no errors.
  - POLICYMAN_STATUS_ERR_INVALID_ARGS
  - POLICYMAN_STATUS_ERR_INVALID_ITEM_ID if pItem does not point to a
    POLICYMAN_ITEM_CA_BAND_COMBOS item.
*/
...
/**
@brief  Return the LTE feature mask from a POLICYMAN_ITEM_LTE_FEATURE_MASK item.

@param[in]  pItem         Pointer to a POLICYMAN_ITEM_LTE_FEATURE_MASK item.
@param[out] pFeatureMask  Optional pointer to storage for the feature mask
                          (may be NULL).
...
@return
  ...
  - POLICYMAN_STATUS_ERR_INVALID_ARGS
  - POLICYMAN_STATUS_ERR_INVALID_ITEM_ID if pItem does not point to a
          POLICYMAN_ITEM_LTE_FEATURE_MASK item.
*/
...
/**
@brief  Update the specified band combo string for a PLMN.

If the same combo string is desired for multiple PLMNs, this API must be called for each PLMN.

@param[in]  pPlmn       A pointer to the PLMN of the network for which the band combinations
                        are to be updated.
@param[in]  pBandCombos Pointer to the new CA band combo string.
...
/*-------- ca_band_combos_get --------*/
/**
@brief Get the specified band combo string for a subscription

@param[in]  pPlmn         A pointer to the PLMN of the network for which the band
                          combinations are desired.
@param[in]  pBandCombos   Pointer to memory into which the CA band combo string is to be
                          copied.
@param[in]  bandCombosLen Size of the memory pointed to by pBandCombos, in bytes.  The
                          maximum string that can be set is 2304 characters.

@return
  Copies the appropriate string into pBandCombos, ensuring that it is NUL terminated.
  Returns the number of bytes (including the NUL terminator) in the string for this PLMN;
  this can be compared to bandCombosLen to determine if truncation occurred.  There are two
  returns with special meaning:
  0: There is no CA band combo string for the requested PLMN (default will be used).
  1: The CA band combo string for the requested PLMN has explicitly been set to default.
*/
...
/**
@brief  Return the service domain to be used.

@param[in]   pItem               Pointer to a POLICYMAN_ITEM_SERVICE_DOMAIN item.
@param[out] pServiceDomain  Pointer the service domain object.

@return
  - POLICYMAN_STATUS_SUCCESS if no errors.
  - POLICYMAN_STATUS_ERR_INVALID_ITEM_ID if pItem does not point to a
    POLICYMAN_ITEM_SERVICE_DOMAIN item.
*/
/*-------- policyman_get_service_domain --------*/
...
// TODO Maintaining the old IMS Allowed API temporarily
#define policyman_ims_config_get_is_enabled policyman_ims_config_get_ims_allowed

/*-------- policyman_ims_config_get_ims_allowed --------*/ 
/**
@brief  Return whether IMS is allowed.

@param[in]   pItem               Pointer to a POLICYMAN_ITEM_IMS_CONFIG item. 
@param[out]  pImsIsAllowed Pointer to the storage for the IMS allowed value.

@return 
  - POLICYMAN_STATUS_SUCCESS if no errors. 
  - POLICYMAN_STATUS_ERR_INVALID_ITEM_ID if pItem does not point to a 
    POLICYMAN_ITEM_IMS_CONFIG item. 
*/ 
...
/**
@brief  Return the max number of IMS instances.

@param[in]    pItem             Pointer to a POLICYMAN_ITEM_IMS_CONFIG item. 
@param[out]   pMaxImsInstances  Pointer to the max number of IMS instances.
...
*/
typedef enum {
  MRE_COND_ID_CUST_BASE(PM), // Customized condition base for this enum

  PM_COND_ID_SERVICE_STATUS_IN = 100,
  PM_COND_ID_SRV_DOMAIN_HAS,
  PM_COND_ID_SERVING_MCC_IN,
  PM_COND_ID_SERVING_PLMN_IN,
  PM_COND_ID_SERVING_RAT_IN,
  PM_COND_ID_SERVING_BAND_IN,
  PM_COND_ID_REG_REJECT_CAUSE_IN,
  PM_COND_ID_OOS_SCAN_CNT,
  PM_COND_ID_LOCATION_MCC_IN,
  PM_COND_ID_HAVE_SERVICE,
  PM_COND_ID_HAVE_LOCATION,
  PM_COND_ID_HAVE_SERVING_SYSTEM,
  PM_COND_ID_IMSI_PLMN_IN,
  PM_COND_ID_IMSI_MCC_IN,
  PM_COND_ID_SIM_TYPE,
  PM_COND_ID_IMEI,
  PM_COND_ID_SIM_REFRESHED,
  PM_COND_ID_SIMLOCK_ENABLED,
  PM_COND_ID_HAVE_IMSI,
  PM_COND_ID_UE_MODE_IS,
  PM_COND_ID_SVC_MODE_IS,
  PM_COND_ID_NUM_SUBS,
  PM_COND_ID_ENFORCE_FULL_RAT,
  PM_COND_ID_IS_SUBS_DDS,
  PM_COND_ID_USER_DOMAIN_PREF,
  PM_COND_ID_USER_MODE_PREF_CONTAINS,
  PM_COND_ID_USER_MCC_IN,
  PM_COND_ID_NETWORK_SELECTION_MODE,
  PM_COND_ID_VOICE_DOMAIN_PREF_IS,
  PM_COND_ID_CALL_MODE_IS,
  PM_COND_ID_SERVICE_DOMAIN_HAS,
  PM_COND_ID_TIMER_EXPIRED,
  PM_COND_ID_TIMER_IS_RUNNING,
  PM_COND_ID_PHONE_OPERATING_MODE,
  PM_COND_ID_TECH_LOADED,
  PM_COND_ID_SCAN_RESULTS,
  PM_COND_ID_NETWORK_SCAN_ACTIVE,
  PM_COND_ID_VOLTE_ENABLED,
  PM_COND_ID_FDN_STATUS,
  PM_COND_ID_FDN_HAS,
  PM_COND_ID_SPECIALIZATION,
  PM_COND_ID_SUBS_IS_ACTIVE,
  PM_COND_ID_SUBPHONE_CAP_HAS,
  PM_COND_ID_EMBMS_IS_ACTIVE,
  PM_COND_ID_TIME_IN_LPM,
  PM_COND_ID_CURRENT_MCC_IN,
} pm_cond_id_enum_t;
...
*/
typedef enum {
  MRE_ACT_ID_CUST_BASE(PM), // Customized action base for this enum

  PM_ACT_ID_DEVICE_CONFIGURATION      = 100,
  PM_ACT_ID_DEVICE_CONFIGURATION_IF,
  PM_ACT_ID_DEFINE_CONFIG,
  PM_ACT_ID_USE_CONFIG,
  PM_ACT_ID_USE_CONFIG_IF,
  PM_ACT_ID_RAT_CAPABILITY,
  PM_ACT_ID_RAT_CAPABILITY_IF,
  PM_ACT_ID_UE_MODE,
  PM_ACT_ID_UE_MODE_IF,
  PM_ACT_ID_RF_BANDS,
  PM_ACT_ID_RF_BANDS_IF,
  PM_ACT_ID_SVC_MODE,
  PM_ACT_ID_FEATURE,
  PM_ACT_ID_VOICE_DOMAIN_PREF,
  PM_ACT_ID_RAT_ORDER,
  PM_ACT_ID_CALL_MODE,
  PM_ACT_ID_CALL_MODE_IF,
  PM_ACT_ID_VOLTE,
  PM_ACT_ID_SCAN_OPTIMIZATION,
  PM_ACT_ID_FREQ_LIST,
  PM_ACT_ID_FREQ_LIST_IF,
  PM_ACT_ID_SELECT_CONFIG,
  PM_ACT_ID_CA_BAND_COMBOS,
  PM_ACT_ID_SERVICE_DOMAIN,
  PM_ACT_ID_RF_BAND_LIST,
  PM_ACT_ID_DEFINE_FULLRAT_CONFIG,
  PM_ACT_ID_FULLRAT_ENTER,
  PM_ACT_ID_DEFINE_TIMER,
  PM_ACT_ID_EXPIRED_TIMER_HANDLED,
  PM_ACT_ID_TIMER_START,
  PM_ACT_ID_TIMER_STOP,
  PM_ACT_ID_EXCLUDE_TECH,
  PM_ACT_ID_NETWORK_SCAN,
  PM_ACT_ID_IMS_CONFIG,
  PM_ACT_ID_LTE_FEATURE_MASK,
} pm_act_id_enum_t;
