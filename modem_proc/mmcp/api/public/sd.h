...
/**===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/api/public/sd.h#2 $
...
12/22/10   sv      Added MM_STACK_2 for DSDS
...
typedef enum {
  ...
  SD_SS_PREF_UPDATE_REASON_SWITCH_UE_MODE_NO_STK_CHG      = 7,
  ...
  SD_SS_PREF_UPDATE_REASON_DDS_SWITCH_TYPE_TEMP = 12,
    /* DDS switch was preceded by a temporary switch
    ** or a temporary DDS switch*/

  SD_SS_PREF_UPDATE_REASON_PM_UE_MODE_CHG     = 13,
                                   /**< UE Mode changed by policyman */

  SD_SS_PREF_UPDATE_REASON_DUAL_LTE_DDS_SWITCH = 14,
    /* DDS switch in dual-LTE config with no max cap change. Both subs should do acq db scan first. */

  SD_SS_PREF_UPDATE_REASON_USER_SS_PREF_IN_FULL_RAT = 15,
    /* When the User SS Pref are forced over the FULL RAT Object  */
...
*/
  #define ACTIVE_SS_MAIN            BM(MM_STACK_0)

  #define ACTIVE_SS_HYBR_2          BM(MM_STACK_2)

  #define ACTIVE_SS_NONE      (uint8)0


/*~ SENTINEL multimode_stack_e_type.MM_STACK_ALL */
...
typedef enum {
  ...
  SD_SS_MODE_PREF_CDMA_HDR_WCDMA_LTE = BM( SD_MODE_CDMA ) | BM( SD_MODE_HDR ) | \
                                       BM( SD_MODE_WCDMA ) | BM( SD_MODE_LTE ),
                                       /**< Acquire CDMA, HDR, WCDMA, or LTE systems.
                                       */
  ...
  SD_SS_MODE_PREF_CDMA_GSM_TDS   = BM(SD_MODE_CDMA) |  BM(SD_MODE_GSM) | BM(SD_MODE_TDS),

  SD_SS_MODE_PREF_CDMA_WCDMA_TDS = BM(SD_MODE_CDMA) | BM(SD_MODE_WCDMA) | BM(SD_MODE_TDS),

  SD_SS_MODE_PREF_CDMA_TDS        = BM(SD_MODE_CDMA) | BM(SD_MODE_TDS),
  ...
  SD_SS_MODE_PREF_CDMA_GSM_LTE_TDS 
                                  = BM( SD_MODE_CDMA )  | BM( SD_MODE_TDS ) | \
                                  BM( SD_MODE_GSM )  | BM( SD_MODE_LTE),

  SD_SS_MODE_PREF_CDMA_WCDMA_LTE_TDS 
                                  = BM( SD_MODE_CDMA )  | BM( SD_MODE_TDS ) | \
                                  BM( SD_MODE_WCDMA )  | BM( SD_MODE_LTE),
...
/** Enumeration to indicate the SD about fullrat
*/
typedef enum {

  SD_SS_FULLRAT_NONE = -1,       /* For SD internal use */
  SD_SS_FULLRAT_OFF,             /**< UE is not in FULLRAT */
  SD_SS_FULLRAT_ON,              /**< UE is in FULLRAT */
  SD_SS_FULLRAT_NO_CHANGE,       /**< No change in FULLRAT status*/
  SD_SS_FULLRAT_MAX              /* For SD internal use */
...
/** Enumeration of SD power up reasons
*/
typedef enum {

  SD_SS_PWR_UP_REAS_NONE = -1,          /**< For SD internal use */
  SD_SS_PWR_UP_REAS_OPRT_MODE,          /**< Operation mode change */
  SD_SS_PWR_UP_REAS_SUBSC,              /**< Subscription change */
  SD_SS_PWR_UP_REAS_DUAL_STANDBY,       /**< Dual standby change */
  SD_SS_PWR_UP_REAS_SUSP_RESUME,        /**< Suspend/Resume change */
  SD_SS_PWR_UP_REAS_MAX

} sd_ss_pwr_up_reas_e_type;

typedef enum {

  SD_LTE_MODE_NONE = -1,

  SD_LTE_MODE_REMOVED = 0,

  SD_LTE_MODE_ADDED = 1,

  SD_LTE_MODE_CHANGE_MAX
  
} sd_lte_mode_change_ind_e_type;
...
*/
#define  MM_STACK_ALL_REDIR_DELAY_NONE   (0xFFFF)
...
/*
** Enumeration for pending E911 call
*/
typedef enum cm_e911_pending_state_e{  
  CM_E911_PENDING_STATE_NONE = -1,
  CM_E911_PENDING_STATE_OFF,
  CM_E911_PENDING_STATE_ON
}cm_e911_pending_state_e_type;
...
typedef struct {
  ...
  **
  ** A value of MM_STACK_ALL_REDIR_DELAY_NONE indicates that this field should
...
/** Stores previous and current value of active stack mask
*/
typedef struct {

  uint8 old_stack_active_mask;
  uint8 current_stack_active_mask;

} sd_ss_stack_active_mask_s_type;
