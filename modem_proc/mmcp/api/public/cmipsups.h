...
} cmipsups_password_change_ss_req_s_type;
...
typedef struct
{
  ...
  uint16                     retry_duration;
    /*passed on from IMS to AP to retry sups after duration elapses */

  boolean                   is_ext_resp;
 
  cmipsups_ext_resp_s_type  ext_resp;
    /* Extended error response for each sub operation of all forward or call barring*/
...
typedef cmipsups_common_ss_conf_type cmipsups_password_change_ss_conf_type;
