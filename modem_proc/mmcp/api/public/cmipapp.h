...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/api/public/cmipapp.h#5 $
...
#define CMIPAPP_COUNTRY_SPECIFIC_EMERG_SRV_CATEG 0xFFFF
...
typedef enum
{
  ...
  */
  ...
  CMIPAPP_END_CAUSE_OFFLINE = 619,
      /**< Call end cause due to UE going to LPM/APM */

  CMIPAPP_END_CAUSE_DRB_REJ = 620,
  ...
  CMIPAPP_END_CAUSE_PRECONDITION_FAILURE = 625,
      /**< Call end cause due to precondition failures */

  CMIPAPP_END_CAUSE_ACCEPT_MT_CS_PAGE = 626,
      /**< Call end cause due to accepting MT CS Page */
...
typedef enum
{
  ...
  */
  ...
  CMIPAPP_CLIENT_MODIFY_CAUSE_UPGRADE_DUE_TO_VIDEO_CRBT = CM_CALL_MODIFY_CAUSE_UPGRADE_DUE_TO_VIDEO_CRBT,
  /**< Call Upgraded due to Video CRBT ON
  */

  CMIPAPP_CLIENT_MODIFY_CAUSE_DOWNGRADE_DUE_TO_VIDEO_CRBT = CM_CALL_MODIFY_CAUSE_DOWNGRADE_DUE_TO_VIDEO_CRBT
  /**< Call downgraded due to Video CRBT OFF
  */
...
typedef enum
{
  ...
  CMIPAPP_CLIENT_END_CAUSE_SIP_480_TEMP_UNAVAIL = CM_CALL_END_PEER_NOT_REACHABLE,
   ...
   CMIPAPP_CLIENT_END_CAUSE_ALTERNATE_SERVICE = CM_CALL_END_ALTERNATE_SERVICE,
   ...
   CMIPAPP_CLIENT_END_CAUSE_SERVER_UNAVAILABLE = CM_CALL_END_SIP_503_SERVER_UNAVAILABLE,
  ...
  */
  ...
  CMIPAPP_CLIENT_END_CAUSE_CS_FB_NOT_FEASIBLE_IN_ROAM_CS_NW = CM_CALL_END_CSFB_NOT_FEASIBLE_IN_ROAM_CS_NW,
  /* Call ended because of failure on WLAN and roaming CS network is available */
  ...
  CMIPAPP_CLIENT_END_CAUSE_RETRY_ON_IMS_WITHOUT_RTT = CM_CALL_END_CAUSE_RETRY_ON_IMS_WITHOUT_RTT,
  /**< This cause is used by IMS to end the RTT EMC call with Fallback-to-IMS with User Consent. So here, CM would end the call and propagate this cause to Apps. So that UI can trigger the call again with Voice only EMC on IMS domain */

  CMIPAPP_CLIENT_END_CAUSE_CALL_ONGOING_CB_ENABLED = CM_CALL_END_INCOM_REJ_CAUSE_CALL_ONGOING_CB_ENABLED,
  /*End cause to clients in case call is ended when call_barring is enabled*/

  CMIPAPP_CLIENT_END_CAUSE_NOT_ACCEPTABLE_GLOBAL = CM_CALL_END_NOT_ACCEPTABLE_GLOBAL
  /**<Resource identified by request can only generate resp with content not acceptable in SDP*/
...
typedef enum
{
  ...
  CMIPAPP_INCOM_REJ_CAUSE_CALL_ON_OTHER_SUB,
    /**< Indicates user was in another call
    */
  ...
  CMIPAPP_INCOM_REJ_CAUSE_UNWANTED_CALL,
    ...
    */
  CMIPAPP_INCOM_REJ_CAUSE_CALL_BARRING,
  /*Indicates UE cased call_barring is enabled and call_barring is active*/

  CMIPAPP_INCOM_REJ_CAUSE_CALL_BARRING_ANONYMOUS,
  /*Indicates UE cased call_barring is enabled and anonymous_call_barring is active*/

    CMIPAPP_INCOM_REJ_CAUSE_CALL_TYPE_NOT_ALLOWED,
  /*Indicated call is reject as the specific call type is disabled by NV 5280 */
...
/** system mode capability app can support.
*/
typedef enum
{
  CMIPAPP_CAP_SYS_MODE_CDMA = 0,

  CMIPAPP_CAP_SYS_MODE_HDR = 1,

  CMIPAPP_CAP_SYS_MODE_GSM = 2,

  CMIPAPP_CAP_SYS_MODE_WCDMA = 3,

  CMIPAPP_CAP_SYS_MODE_LTE = 4,

#ifdef FEATURE_NR5G_SA
  CMIPAPP_CAP_SYS_MODE_NR5G = 5,
#endif

  CMIPAPP_CAP_SYS_MODE_WLAN = 6,

  CMIPAPP_CAP_SYS_MODE_NO_CHANGE = 7

}cmipapp_cap_sys_mode_e_type;
/*~ SENTINEL  cmipapp_cap_sys_mode_e_type.CMIPAPP_CAP_SYS_MODE_NO_CHANGE*/

/** Bit Masks of system mode capability app can support.
** Combination of system mode capabilities can be created
** by using the macro ADD_SYSMODE
*/
typedef enum
{
  CMIPAPP_CAP_SYS_MODE_MASK_NONE  = 0,

  CMIPAPP_CAP_SYS_MODE_MASK_CDMA = BM(CMIPAPP_CAP_SYS_MODE_CDMA),

  CMIPAPP_CAP_SYS_MODE_MASK_HDR = BM(CMIPAPP_CAP_SYS_MODE_HDR),

  CMIPAPP_CAP_SYS_MODE_MASK_GSM = BM(CMIPAPP_CAP_SYS_MODE_GSM),

  CMIPAPP_CAP_SYS_MODE_MASK_WCDMA = BM(CMIPAPP_CAP_SYS_MODE_WCDMA),

  CMIPAPP_CAP_SYS_MODE_MASK_LTE   = BM(CMIPAPP_CAP_SYS_MODE_LTE),

#ifdef FEATURE_NR5G_SA
  CMIPAPP_CAP_SYS_MODE_MASK_NR5G  = BM(CMIPAPP_CAP_SYS_MODE_NR5G),
#endif

  CMIPAPP_CAP_SYS_MODE_MASK_WLAN  = BM(CMIPAPP_CAP_SYS_MODE_WLAN),

  CMIPAPP_CAP_SYS_MODE_MASK_NO_CHANGE   = BM(CMIPAPP_CAP_SYS_MODE_NO_CHANGE)

}cmipapp_cap_sys_mode_mask_e_type;
/*~ SENTINEL  cmipapp_cap_sys_mode_mask_e_type.CMIPAPP_CAP_SYS_MODE_MASK_NO_CHANGE*/
...
/* This will be passed while IMS sets system mode capability
** for all call types.
*/
typedef struct cmipapp_all_cap_s
{
  cmipapp_cap_sys_mode_mask_e_type   voice_sys_mode;

  cmipapp_cap_sys_mode_mask_e_type   vt_sys_mode;

  cmipapp_cap_sys_mode_mask_e_type   sups_sys_mode;

  cmipapp_cap_sys_mode_mask_e_type   e911_sys_mode;

  cmipapp_cap_sys_mode_mask_e_type   sms_sys_mode;

  cmipapp_cap_sys_mode_mask_e_type   vs_sys_mode;

}cmipapp_all_cap_s_type;
...
typedef struct cmipapp_ind_mo_progr_info_s
{
  ...
  cm_ip_call_prog_info_notif_s_type   prog_info_notif;
...
typedef struct cmipapp_ind_call_end_s
{
  ...
  /* indicates if call can be redialled again in IMS NW 
  */
  ...
  cm_ip_call_composer_info_s_type call_composer_info;
  /* Call composer info
  */
...
/* Indication for DTMF SIP INFO */
typedef struct cmipapp_ind_sip_dtmf_ind_s
{
  /**< Call id */
  cm_call_id_type               call_id;

  /**< DTMF string */
  cm_ip_sip_dtmf_s_type        *dtmf_info;

}cmipapp_ind_sip_dtmf_ind_s_type;

/* Cancel modify infication from IMS */
typedef struct cmipapp_ind_cancel_ims_call_ind_s

{
  ...
  /*Modify result*/
  boolean cancel_call_result;  

  cm_ip_call_cancel_ims_call_req_e_type cancel_ims_call_req_type;
  
}cmipapp_ind_cancel_ims_call_ind_s_type;
...
typedef struct cmipapp_ind_epdn_rat_change_s
{
  cmipapp_sys_mode_e_type  sys_mode;
}cmipapp_ind_epdn_rat_change_s_type;
  ...
  */
  union {
      ...
      */
      ...
      cmipsups_password_change_ss_conf_type  password_change_ss_conf_type;
    ...
    /*Cancel call modify ind */
    cmipapp_ind_cancel_ims_call_ind_s_type   cancel_ims_call_ind;

	/* EPDN RAT change indication */
	cmipapp_ind_epdn_rat_change_s_type       epdn_rat_change;

    /* MT SIP DTMF ind */
    cmipapp_ind_sip_dtmf_ind_s_type     sip_dtmf_ind;
...
typedef struct {
  ...
  uint32              switch_to_codec;
    /* codec to which call has to be switched, multiple codecs are allowed */
...
/* structure for cmipp id allocation
*/
typedef struct {
  cmipapp_call_type_e_type     call_type;
  /* App type
...
** mode
*/
typedef boolean (cmipapp_inform_oprt_mode_sub_f_type) (
...
/*  Call back function called by CM to send SIP DTMF from user.
**  TRUE is returned on successfull operation, FALSE otherwise
*/
typedef boolean (cmipapp_cmd_sip_dtmf_sub_f_type) (
...
/* Call back function for CM to cancel ims call req
**  TRUE is returned on successfull operation, FALSE otherwise
*/

typedef boolean (cmipapp_cmd_cancel_ims_call_req_sub_f_type) (

  const cm_call_id_type           call_id,
    /* call id
    */

  cm_ip_call_cancel_ims_call_req_e_type cancel_ims_call_req,
    /* Cancel call modify type
    */
...
/*~ CALLBACK cmipapp_send_drvcc_f_type
    ONERROR return FALSE */
...
typedef struct {
  ...
  cmipapp_inform_oprt_mode_sub_f_type *cmipapp_inform_oprt_mode_sub;
    /* Callback to inform ipapp of operating mode */
  ...
  cmipapp_cmd_cancel_ims_call_req_sub_f_type              *cmipapp_cmd_cancel_ims_call_req_sub_func_ptr;
  /* Callback for CM to cancel call modify request  */ 
 
  cmipapp_cmd_sip_dtmf_sub_f_type    *cmipapp_cmd_sip_dtmf_sub_func_ptr;
  /* Callback for CM to send DTMF string from AP to IMS */
...
/*===========================================================================
FUNCTION cmipapp_send_cancel_ims_call_req_cmd

DESCRIPTION
  Sends cmd to cancel call modification
 (upgrade/downgrade/cancel hold/cancel resume).
