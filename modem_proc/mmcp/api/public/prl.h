...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/7.9.0/api/public/prl.h#1 $
...
typedef enum
{
  ...
  /* Values 12 - 13 are reserved */
  PRL_ACQ_REC_RESERVED2,
  PRL_ACQ_REC_RESERVED3,

  /* 3GPP generic acquisition type, applicable for IS683E or above */
  PRL_ACQ_REC_3GPP_GENERIC = 14,

  /* GWL acquisition type, applicable for IS683E or above
   ** kept for backward compatibility reasons only. If
   ** PRL_ACQ_REC_3GPP_GENERIC is defined, that should be used 
   **  to represent 3GPP systems
   */
  PRL_ACQ_REC_GWL_GENERIC = 15,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
/* Enumeration of PRL sources.
*/
typedef enum
{
  PRL_SOURCE_INFO_NONE=-1,            /* For internal use only */
  PRL_SOURCE_INFO_UNDETERMINED,       /* PRL is not loaded */
  PRL_SOURCE_INFO_DEFAULT,            /* PRL source is default PRL */
  PRL_SOURCE_INFO_NV,                 /* PRL source is NV */
  PRL_SOURCE_INFO_CARD,               /* PRL source is card */
  PRL_SOURCE_INFO_MAX                 /* For internal use only */
} prl_source_info_e_type;
...
typedef struct
{
  ...
  uint8  band;                              /* Bands -  8bit band-class */
  uint16 uarfcn;                            /* Chans - 16bit UARFCN */
} prli_gwl_generic_rec_type;

#define PRL_MAX_GWL_GENERIC_BAND 49
...
{
  uint8                          version;
    /* version introduced with 3GPP_GENERIC acq record.
       value is set to 0 for GWL_GENERIC records and 1 for 3GPP_GENERIC
    */
  byte                           num_of_rats;      
...
typedef struct
{
  ...
  union
  {
    ...
    /* The following acq record types apply for IS683E PRL or above. */
