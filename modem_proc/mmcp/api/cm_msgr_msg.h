...
/*==============================================================================
 ...
 $Header: //components/rel/mmcp.mpss/7.9.0/api/cm_msgr_msg.h#4 $
...
typedef struct 
{
  ...
  uint32 retry_timer;
  /* A timer when IMS will reattempt registration (secs).  CM uses it to decide the plmn block timer
  ** for non-dds sub.
  */

  uint8 trans_id;
  /**< Transaction ID. IMS should store the transaction id in this indication and
  ** send the same in IMS reg status indication.
  ** Exceptions: 0 for wifi or invalid, 0xFF for 3rd party
  */
...
/** @brief Voice/video media restriction status */
typedef enum
{
  CM_MEDIA_RESTRICT_NONE, /* Voice is not restricted */
  CM_MEDIA_RESTRICT_ROAM, /* Voice is restricted in roam */
  CM_MEDIA_RESTRICT_HOME_ROAM, /* Voice is restricted in home and roam */
  CM_MEDIA_RESTRICT_MAX
}cm_media_restrict_status_e_type;

/** @brief Structure containing IMS voice/video media restriction data. */
typedef struct 
{
  ...
  cm_media_restrict_status_e_type nr5g_voice_restrict_status;
  cm_media_restrict_status_e_type nr5g_video_restrict_status;
  #endif
}cm_media_restriction_status_s_type;
...
typedef struct 
{
  ...
  /**< True VOPS info*/
   ...
   uint8 trans_id;
  /**< Transaction ID. IMS should store the transaction id in this indication 
       and send the same in IMS reg status indication. 
       Exceptions: 0 for wifi or invalid, 0xFF for 3rd party */
...
/** @brief Message structure for the IMS voice/video media restriction status
  indication.
*/ 
typedef struct
{
  ...
  cm_media_restriction_status_s_type data;
  /**< Media restriction status info */
} cm_media_restriction_ind_msg_type;
...
/** Emergency ready on LTE is due to CSFB or not. Values: 
...
/** @brief Message structure for the IMS Handover ready indication.
  This struct provides info required by IMS to perform
  required handover of PDN from WLAN to LTE.
*/
typedef struct
{
  msgr_hdr_s  msg_hdr;
...
} cm_ims_LTE_e911_capabable_for_handover_ind_s_type;
...
{  
  ...
  cm_media_restriction_ind_msg_type cm_media_restriction_ind;
} cm_sds_cmd_u_type;
...
typedef struct 
{
  ...
  /**< Used when AP wants to limit emergency scan 
       to particular RATs */
...
/* Voice call (For all voice call excpet volte & vowlan) status  will be sent as
** 1. CM_VOICE_CALL_STARTED on voice call origination 
**    if there is no prior voice call on asubs_id
** 2. CM_VOICE_CALL_ENDED on voice call end
**    if there is no more voice call on asubs_id
*/
typedef struct
{
  ...
  cm_voice_call_status_e_type 	 voice_call_status; 
  /* Voice call start or end status*/
  
}cm_voice_call_status_ind_s_type;
...
typedef struct
{
  ...
  boolean tune_away_allowed;
  /* TRUE if volte call on stack 2 feature is enabled, FALSE otherwise */
...
/* Structure for CM to send E911 scan status failure IND*/
typedef struct cm_emerg_srch_fail_ind_s
{
  msgr_hdr_s              msg_hdr;
...
/* Structure for CM to notify oprt mode */
typedef struct cm_oprt_mode_s
{                                                                
  msgr_hdr_s              msg_hdr;
...
typedef struct cm_plmn_list_to_avoid_s
{                                                                
  msgr_hdr_s              msg_hdr;
  sys_plmn_id_s_type list_to_avoid[SYS_AVOID_PLMN_LIST_MAX_LENGTH];
  sys_modem_as_id_e_type as_id;
  uint8  num_of_plmns_to_avoid;
} cm_plmn_list_to_avoid_s_type;

typedef struct LTE_emergency_avoid_plmn_list
{                                                                
  msgr_hdr_s              msg_hdr;
  uint8 plmn_list_to_avoid_len;
  sys_plmn_id_s_type plmn_list_to_avoid[SYS_AVOID_PLMN_LIST_MAX_LENGTH];
} LTE_emergency_avoid_plmn_list_type;
...
/* Structure for set t_ssac timer request*/
typedef struct cm_mm_set_ssac_hyst_req_s
{
  msgr_hdr_s              msg_hdr;

  uint16                  t_ssac_val;
 
} cm_mm_set_ssac_hyst_req_s_type;

/* Structure for get t_ssac timer request*/
typedef struct cm_mm_get_ssac_hyst_timer_req_s
{
  msgr_hdr_s              msg_hdr;
 
} cm_mm_get_ssac_hyst_timer_req_s_type;

/* Structure for reporting t_ssac timer to cm clients*/
typedef struct cm_mm_get_ssac_hyst_timer_rsp_s
{
  msgr_hdr_s              msg_hdr;

  uint16                  t_ssac_val;
 
} cm_mm_get_ssac_hyst_timer_rsp_s_type;

typedef struct cm_mm_set_gpp_wifi_cellular_scan_timer_ind_s
{
  msgr_hdr_s              msg_hdr;

  uint16                  gpp_wifi_cellular_scan_timer_val;
 
} cm_set_gpp_wifi_cellular_scan_timer_s_type;
...
/** @brief Message structure for raw rssi
 */
typedef struct
{
  msgr_hdr_struct_type   hdr;     
...
}cm_qmi_cache_reset_msg_type;

typedef struct cm_trm_acq_status_ind_s
{
  msgr_hdr_s hdr;
    /**< Message header */

  sys_acq_status_e_type trm_acq_status;
    /**< SUCCESS: TRM available and acquisition performed
     ** NO_RESOURCE: NO TRM resource
     */

} cm_trm_acq_status_ind_s_type;
...
typedef struct
{
  ...
  boolean is_registered;
  /* Flag to indicate if IMS is Registered for the call_type */
  ...
  cm_call_type_mask_type         call_type_mask; 
  /** Call types associated with this domain. Currently only 
     voice support is requested but added to expand in furture  */

}cm_ims_reg_status_ind_s_type;

/** @brief structure for call composer subject info
**
*/
typedef struct cm_msgr_call_composer_subject_s{

  uint16 *subject_text;
  /**< Subject in UTF16 format*/

  uint32 subject_length;
  /**< Number of UTF16 chars (not in bytes) */

}cm_msgr_call_composer_subject_s_type;

/** @brief structure for call composer picture url
**
*/
typedef struct cm_msgr_call_composer_pic_url_s{

  char *pic_url;
  /**< Picture URL in ASCII format */

}cm_msgr_call_composer_pic_url_s_type;


/** @brief structure of call composer info
**
*/
typedef struct cm_msgr_call_composer_info_s{

  boolean                               info_included;
  /**< Flag to indicate if any call composer info is set by user */

  cm_call_composer_importance_e_type    importance;
  /**< Importance of the call */

  cm_msgr_call_composer_subject_s_type  subject;
  /**< Subject of the call */

  cm_msgr_call_composer_pic_url_s_type  picture_url;
  /**< URL for the uploaded picture */

  cm_call_composer_location_s_type      geo_location;
  /**< User provided geo location  */

  cm_call_composer_org_header_s_type    org_header;
  /**< Organization header of the call */

}cm_msgr_call_composer_info_s_type;
...
typedef struct mt_call_end_auto_reject_modem_ind_s
{
  ...
  */
  ...
  cm_msgr_call_composer_info_s_type  call_composer_info;
  /* Call composer info from invite
  */

  cm_verstat_value_e_type call_verstat;
  /* Indicates MT call verstat value
  */
...
enum
{
  ...
  */
  ...
  MSGR_DEFINE_UMID(MM, CM, IND, EXT_VOLTE_CALL_STATE, 0x36, \
                   cm_ext_volte_call_state_ind_s_type),

  /* MM_CM_SET_SSAC_HYST_TIMER_REQ*/
  MSGR_DEFINE_UMID(MM, CM, REQ, SET_SSAC_HYST_TIMER, 0x37, \
                   cm_mm_set_ssac_hyst_timer_req_s_type),

      /* MM_CM_GET_SSAC_HYST_TIMER_REQ*/
  MSGR_DEFINE_UMID(MM, CM, REQ, GET_SSAC_HYST_TIMER, 0x38, \
                   cm_mm_get_ssac_hyst_timer_req_s_type),

      /* MM_CM_GET_SSAC_HYST_TIMER_RSP*/
  MSGR_DEFINE_UMID(MM, CM, RSP, GET_SSAC_HYST_TIMER, 0x39, \
                   cm_mm_get_ssac_hyst_timer_rsp_s_type),

  MSGR_DEFINE_UMID(MM, CM, IND, AUDIO_RELEASE,0x40, \
                   cm_audio_session_rel_msg_type),
  
  MSGR_DEFINE_UMID(MM, CM, IND, LTE_ACTIVE_STATUS,0x41, \
                   cm_lte_active_status_ind_s_type),
  ...
  MSGR_DEFINE_UMID(MM, CM, IND, MANUAL_SCAN_FAIL, 0x46, \
                   cm_manual_scan_fail_ind_s_type),

  /* MM_CM_RAW_RSSI_IND*/
  MSGR_DEFINE_UMID(MM, CM, IND, RAW_RSSI,0x47, \
                   cm_raw_rssi_msg_type),
  ...
  MSGR_DEFINE_UMID(MM, CM, IND, TRM_PRIORITY, 0x51, \
                   cm_trm_priority_ind_s_type),
  ...
  MSGR_DEFINE_UMID(MM, CM, IND, VOICE_CALL_STATUS, 0x54, \
    cm_voice_call_status_ind_s_type),

  /**< MM_CM_SRLTE_SWITCH_IND Message sent from CM to IMS  */
  MSGR_DEFINE_UMID(MM, CM, IND, SRLTE_SWITCH, 0x55,\
                   cm_srlte_switch_s_type),

   /* MM_CM_TAU_COMPLETE_IND */
  MSGR_DEFINE_UMID(MM, CM, IND, TAU_COMPLETE, 0x56, \
    cm_tau_complete_ind_s_type),

   /* MM_CM_IMS_PREF_CACHE_RESET_IND from CM to QMI-NAS
  */
  MSGR_DEFINE_UMID(MM, CM, IND, IMS_PREF_CACHE_RESET,0x57, \
                   cm_qmi_cache_reset_msg_type),

  /* MM_CM_LL_TRM_ACQ_STATUS_IND from CM to IMS */
  MSGR_DEFINE_UMID(MM, CM, IND, LL_TRM_ACQ_STATUS, 0x58, \
                   cm_trm_acq_status_ind_s_type),

  /* MM_CM_PLMN_AVOID_LIST_IND from CM to LRRC*/
  MSGR_DEFINE_UMID(MM, CM, IND, PLMN_AVOID_LIST, 0x5A, \
                   cm_plmn_list_to_avoid_s_type),
				   

  /* MM_CM_E911_AVOID_PLMN_IND from QMI to CM*/
  MSGR_DEFINE_UMID(MM, CM, IND, E911_AVOID_PLMN, 0x5B, \
                   LTE_emergency_avoid_plmn_list_type),

 /* MM_CM_E911_SRCH_FAIL_IND Message sent from CM to QMI-NAS */
  MSGR_DEFINE_UMID(MM, CM, IND, E911_SRCH_FAIL, 0x60,\
                   cm_emerg_srch_fail_ind_s_type),
  	
 /* MM_CM_OPRT_MODE_IND */
  MSGR_DEFINE_UMID(MM, CM, IND, OPRT_MODE, 0x61, \
                   cm_oprt_mode_s_type),

 /* MM_CM_OPRT_MODE_LPM_IND */
  MSGR_DEFINE_UMID(MM, CM, IND, OPRT_MODE_LPM, 0x62, \
                   cm_oprt_mode_s_type),

  /* MM_CM_IMS_REG_STATUS_IND*/
  MSGR_DEFINE_UMID(MM, CM, IND, IMS_REG_STATUS, 0x63, \
                   cm_ims_reg_status_ind_s_type),
  ...
  /* MM_CM_IMS_LTE_E911_CAPABLE_FOR_HANDOVER_IND */
  /**< Message sent by CMSDS to IMS to start the Handover of PDN from WLAN to LTE during emergency call. 
       This indication should trigger IMS to do the required
  	   handover of the PDN from WLAN to LTE. cm_ims_LTE_e911_capabable_for_handover_ind_s_type contains the 
  	   info required by IMS to start the handover of the PDN (emergency call) from WLAN to IMS.
  	 */
  MSGR_DEFINE_UMID(MM, CM, IND, IMS_LTE_E911_CAPABLE_FOR_HANDOVER, 0x65,
  				 cm_ims_LTE_e911_capabable_for_handover_ind_s_type),

  /* MM_CM_SET_GPP_WIFI_CELLULAR_SCAN_TIMER_IND */
  MSGR_DEFINE_UMID(MM, CM, IND, SET_GPP_WIFI_CELLULAR_SCAN_TIMER, 0x66, \
                   cm_set_gpp_wifi_cellular_scan_timer_s_type),

  /* MM_CM_MEDIA_RESTRICTION_IND Message sent from IMS to CM */
  MSGR_DEFINE_UMID(MM, CM, IND, MEDIA_RESTRICTION,0x67,\
                   cm_media_restriction_ind_msg_type),

  MSGR_DEFINE_UMID(MM, CM, IND, SSAC_ACCESS_BARRING_STATUS, 0x68, \
                   mmode_qmi_nas_access_barring_status_ind_msg_type),
