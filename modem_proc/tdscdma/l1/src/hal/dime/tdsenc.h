...
$Header: //components/rel/tdscdma.mpss/4.3/l1/src/hal/dime/tdsenc.h#1 $ 
...
//#if (!defined(FEATURE_RF_TX_PWR_LIM) && defined(FEATURE_RF_TX_PWR_LIM_REV))
//#error "FEATURE_RF_TX_PWR_LIM_REV requires FEATURE_RF_TX_PWR_LIM"
//#endif

/* + xiaochen 02/21/2011: commented from original code */
#if 0

#if (defined(FEATURE_WCDMA_UL_TFC_PL_REV_2) && !defined(FEATURE_6280_COMPILE))
#error "FEATURE_WCDMA_UL_TFC_PL_REV_2 requires FEATURE_6280_COMPILE"
#endif
...
//#if (defined(FEATURE_WCDMA_LAST_UL_SRB_DATA_REC) && !defined(FEATURE_WCDMA_SUSP_BFI_TWO_FR_BF_ACT))
//#error "FEATURE_WCDMA_LAST_UL_SRB_DATA_REC requires FEATURE_WCDMA_SUSP_BFI_TWO_FR_BF_ACT"
//#endif
...
#define TDSENC_EUL_AND_R99_UL_MUTEX_LOCK()              rex_enter_crit_sect(&tdsenc_eul_and_r99_ul_mutex)
#define TDSENC_EUL_AND_R99_UL_MUTEX_UNLOCK()            rex_leave_crit_sect(&tdsenc_eul_and_r99_ul_mutex)

/* + defang 01/13/2011: commented from original code */
#if 0
#define TDSENC_UL_TRCH_LOG_IDX_ISR_LOCK()               REX_ISR_LOCK(&tdsulcmd_trch_log_indx_crit_sect)
#define TDSENC_UL_TRCH_LOG_IDX_ISR_UNLOCK()             REX_ISR_UNLOCK(&tdsulcmd_trch_log_indx_crit_sect)
#endif
/* - defang 01/13/2011 */

#define TDSENC_EUL_AND_R99_UL_ATOMIC_MUTEX_LOCK()       rex_enter_crit_sect(&tdsenc_eul_and_r99_ul_mutex); REX_DISABLE_PREMPTION();
#define TDSENC_EUL_AND_R99_UL_ATOMIC_MUTEX_UNLOCK()     rex_leave_crit_sect(&tdsenc_eul_and_r99_ul_mutex); REX_ENABLE_PREMPTION(); 

#define TDSENC_EUL_AND_R99_UL_ISR_LOCK()                REX_ISR_LOCK(&tdsenc_eul_and_r99_ul_mutex)
#define TDSENC_EUL_AND_R99_UL_ISR_UNLOCK()              REX_ISR_UNLOCK(&tdsenc_eul_and_r99_ul_mutex)

#define TDSENC_EUL_AND_R99_UL_ISR_ATOMIC_LOCK()         REX_ISR_LOCK(&tdsenc_eul_and_r99_ul_mutex);   REX_DISABLE_PREMPTION();
#define TDSENC_EUL_AND_R99_UL_ISR_ATOMIC_UNLOCK()       REX_ISR_UNLOCK(&tdsenc_eul_and_r99_ul_mutex); REX_ENABLE_PREMPTION(); 


#define TDSENC_UL_ISR_AND_DCH_RECONFIG_LOCK()           REX_ISR_LOCK(&tdsulcmd_dch_reconfig_isr_mutex)
#define TDSENC_UL_ISR_AND_DCH_RECONFIG_UNLOCK()         REX_ISR_UNLOCK(&tdsulcmd_dch_reconfig_isr_mutex)

#if defined(FEATURE_TDSCDMA_TRITON_UL)|| defined(FEATURE_BOLT_MODEM)
...
#endif /*FEATURE_TDSCDMA_TRITON_UL||FEATURE_BOLT_MODEM*/
...
typedef struct
{
  boolean tdsenc_is_init_crit_sect;

  /* Freeze the Tx accumulation to disable the TPC */
  boolean tdsenc_debug_freeze_tx_accum;

  /* Variable used to set the init dpcch pwr and the Rach pwr */
  tdsenc_debug_pwr_values_enum_type tdsenc_debug_tx_pwr_type;

  /*Flag to enable/disable HDET correction */
  boolean tdsenc_debug_disable_hdet_correction;
  ...
  /* DPCH OLPC parameter */
  int16  tdsenc_dpch_init_tx_pwr_dbm;
  int16  tdsenc_dpch_init_tx_pwr_agc_for_revert;
   ...
   */
  boolean tdsenc_reconfig_freeze_flag ;

  uint16 tdsenc_ul_trch_trblk_accum[2][TDSUE_MAX_TRCH];

  uint8 tdsenc_trch_accum_buf_idx;
  ...
  boolean tdsenc_mp_log_dpch_f;   boolean tdsenc_mp_log_erucch_f;
  ...
  #ifdef FEATURE_BOLT_MODEM
  enc_tx_enc_r4r99_dch_cipher_t tdsenc_cipher_key[6];
  enc_tx_r4r99_enc_cipher_ctl_t tdsenc_r4_enc_ciper_ctrl;
  enc_tx_r4r99_enc_cfg_t tdsenc_r4_enc_cfg;
  enc_enc3g_mmap_t tdsenc_r4_enc_params;
  enc_tx_tds_dpch_chintlv_cfg_t tdsenc_r4_chintlv_cfg;
  enc_chintlv_mmap_t tdsenc_r4_chintlv_params;
  enc_mk_cmd_t tdsenc_r4_enc_cmd0;
  uint32 tdsenc_r4_enc_ib_length;
  enc_enc3g_mmap_t tdsenc_erucch_enc_params;
  enc_tx_tds_erucch_chintlv_cfg_t tdsenc_erucch_chintlv_cfg;
  enc_chintlv_mmap_t tdsenc_erucch_chintlv_params;
  enc_mk_cmd_t tdsenc_erucch_enc_cmd1;
  #endif/*FEATURE_BOLT_MODEM*/

  #ifdef TDSCDMA_FEATURE_UL_HWRAM_DBG
  uint32 tdsenc_eramb_dump_buf[ERAMB_DUMP_BUF_SIZE];
  uint32 tdsenc_rmram_dump_buf[RMRAM_DUMP_BUF_SIZE];

  #ifdef FEATURE_BOLT_MODEM
  uint32 tdsenc_wave_input_dump_buf[WAVE_IB_DUMP_BUF_SIZE];
  uint32  tdsenc_erama_dump_buf[ERAMA_DUMP_BUF_SIZE];
  enc_mk_cmd_t tdsenc_cmd0_dump_buf;
  enc_tx_r4r99_enc_cfg_t tdsenc_r4_enc_cfg_dump_buf;
  enc_enc3g_mmap_t tdsenc_r4_enc_params_dump_buf;
  enc_tx_tds_dpch_chintlv_cfg_t tdsenc_r4_chintlv_cfg_dump_buf;
  enc_chintlv_mmap_t dsenc_r4_chintlv_params_dump_buf;
  uint32 tdsenc_eib_dump_buf[EIB_DUMP_BUF_SIZE];  
  #endif/*FEATURE_BOLT_MODEM*/
...
DESCRIPTION
  Kill the PA by calling tdsuldpchctrl_tx_disable()
...
#if 0
/*===========================================================================
FUNCTION        tdsenc_cipher_tm_tb_key_stream
...
SIDE EFFECTS    It calls function tdsenc_gen_cipher_keystream so same
                side effects as that function.
===========================================================================*/


void tdsenc_cipher_tm_tb_key_stream(
...
FUNCTION tdsenc_print_rach_timing_debug
...
===========================================================================*/
void tdsenc_print_rach_timing_debug( void );
...
/*===========================================================================
FUNCTION 
  tdsenc_tx_reg_enable

DESCRIPTION
  This function enable TX when L1 task is first created or after PRACH/DPCH setup.
...
SIDE EFFECTS
  None
==========================================================================*/
void tdsenc_tx_reg_enable(void);


/*===========================================================================
FUNCTION 
  tdsenc_tx_reg_disable

DESCRIPTION
  This function disable TX and initialize ERAMA buffer when PRACH/DPCH tear-down.
...
SIDE EFFECTS
  None
==========================================================================*/
void tdsenc_tx_reg_disable(void);
...
#ifdef TDSCDMA_FEATURE_DIME_DBG_UL_HWRAM
/*=========================================================================
...
/*=========================================================================
FUNCTION tdsenc_eramb_rmram_dump
...
SIDE EFFECTS
  None.
=========================================================================*/
void tdsenc_eramb_rmram_dump(void);
#endif // TDSCDMA_FEATURE_DIME_DBG_UL_HWRAM
...
#ifdef TDSCDMA_FEATURE_DIME_DBG_UL_ENC
/*=========================================================================
FUNCTION tdsenc_enc_program_reg_log

DESCRIPTION
  Print out the enc program register log information.
...
SIDE EFFECTS
  None.
=========================================================================*/
void tdsenc_enc_program_reg_log(void); 
#endif // TDSCDMA_FEATURE_DIME_DBG_UL_ENC

#ifdef FEATURE_BOLT_MODEM
/*=========================================================================

FUNCTION tdsenc_get_wav_ib_pingpong_flag

DESCRIPTION
  This function return which bank wav_ib_r4 is using.

DEPENDENCIES
  None.
  
RETURN VALUE
	pingpong flag for wav_ib_r4
  
SIDE EFFECTS
  None.

=========================================================================*/
uint8 tdsenc_get_wav_ib_pingpong_flag(void);
