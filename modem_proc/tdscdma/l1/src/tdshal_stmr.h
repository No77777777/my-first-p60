...
$Header: //components/rel/tdscdma.mpss/4.3/l1/src/tdshal_stmr.h#1 $
...
#ifdef FEATURE_TDSCDMA_MSGR_VARIANT
#define TDSHAL_STMR_MAX_SUBS 2

typedef enum {
  TDSHAL_STMR_SUB_ID_0,
  TDSHAL_STMR_SUB_ID_1,
} tdshal_stmr_sub_e_type;

typedef struct{
  tdshal_stmr_sub_e_type tds_sub_id;
  vstmr_rtc_id vstmr_tds_rtc_id;
  sys_vfr_e_type tds_vfr_map[TDSHAL_STMR_MAX_SUBS];  
} tdshal_stmr_params_type;

#define TDSHAL_STMR_GET_SUB_ID(sys_as_id)          \
  ((sys_as_id == SYS_MODEM_AS_ID_1) ? TDSHAL_STMR_SUB_ID_0 : TDSHAL_STMR_SUB_ID_1)
#endif

typedef void tdshal_stmr_rxtime_int_isr_type(vstmr_tds_event_tbl_s *event_tbl, void *arg, uint32 pending);

void tdshal_stmr_dump_and_return( uint16* wall_time_subfn, uint16* wall_time_cx8, uint16* rx_time_subfn, uint16* rx_time_cx8 );
...
SIDE EFFECTS
  None
===========================================================================*/
uint32 tdshal_stmr_init
(
 boolean reset
#ifdef FEATURE_TDSCDMA_MSGR_VARIANT
 , sys_modem_as_id_e_type tds_sub_id
#endif
);
...
/*===========================================================================
  ...
  FUNCTION tdshal_stmr_set_vfr
  
  DESCRIPTION
    This function sets the correct vfr indicated by the mapping from the mmoc message.
...
void tdshal_stmr_update_vfr_info(mmoc_vfr_map_s_type* mmoc_vfr_map_s);
#endif
