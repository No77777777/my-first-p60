...
$Header: //components/rel/tdscdma.mpss/4.3/l1/src/tdssrchzz.h#1 $ $DateTime: 2019/08/19 10:51:23 $ $Author: pwbldsvc $
...
#ifdef FEATURE_TDSCDMA_EAMM_OPT
#define TDSSRCHZZ_EAMM_BEST_FD                      100*256
#define TDSSRCHZZ_EAMM_GOOD_CIR                     10*256
#define TDSSRCHZZ_EAMM_BAD_CIR                       3*256
#define TDSSRCHZZ_EAMM_GOOD_RSCP                  -70*256
#define TDSSRCHZZ_EAMM_BAD_RSCP                     -90*256
#define TDSSRCHZZ_EAMM_TH_RSCP_DROP             15*256
#define TDSSRCHZZ_EAMM_COND_BEST_FD()          (tdssrchzz_eamm_info.f_doppler <= TDSSRCHZZ_EAMM_BEST_FD)
#define TDSSRCHZZ_EAMM_COND_TRAN_FD()          (tdssrchzz_eamm_info.f_doppler > TDSSRCHZZ_EAMM_BEST_FD)
#define TDSSRCHZZ_EAMM_COND_BEST_RSCP()       (tdssrchset_active_cell.active_cell_ptr->filt_rscp_x256 >= TDSSRCHZZ_EAMM_GOOD_RSCP)
#define TDSSRCHZZ_EAMM_COND_GOOD_RSCP() \
((tdssrchset_active_cell.active_cell_ptr->filt_rscp_x256 > TDSSRCHZZ_EAMM_BAD_RSCP) \
  && (tdssrchset_active_cell.active_cell_ptr->filt_rscp_x256 < TDSSRCHZZ_EAMM_GOOD_RSCP))
#define TDSSRCHZZ_EAMM_COND_BAD_RSCP()     (tdssrchset_active_cell.active_cell_ptr->filt_rscp_x256 <=TDSSRCHZZ_EAMM_BAD_RSCP)
#define TDSSRCHZZ_EAMM_COND_BEST_CIR()         (tdssrchzz_eamm_info.cir_snr >= TDSSRCHZZ_EAMM_GOOD_CIR)
#define TDSSRCHZZ_EAMM_COND_GOOD_CIR()        ((tdssrchzz_eamm_info.cir_snr > TDSSRCHZZ_EAMM_BAD_CIR) && (tdssrchzz_eamm_info.cir_snr < TDSSRCHZZ_EAMM_GOOD_CIR))
#define TDSSRCHZZ_EAMM_COND_BAD_CIR()          (tdssrchzz_eamm_info.cir_snr <= TDSSRCHZZ_EAMM_BAD_CIR)
#define TDSSRCHZZ_EAMM_RATIO_RSCP_DROPPED()   (tdssrchzz_eamm_info.ratio_rscp > TDSSRCHZZ_EAMM_TH_RSCP_DROP)
#define TDSSRCHZZ_EAMM_SENSOR_STATE_MOBILITY()      (TDSSRCHZZ_EAMM_SENSOR_MOBILITY == tdssrchzz_eamm_info.sensor_state)
#define TDSSRCHZZ_EAMM_SENSOR_STATE_STATIONARY()      (TDSSRCHZZ_EAMM_SENSOR_STATIONARY == tdssrchzz_eamm_info.sensor_state)
#endif
...
typedef struct 
{
 ...
 typedef enum 
 {
   TDSSRCHZZ_EAMM_BAD,   
   TDSSRCHZZ_EAMM_GOOD,
   TDSSRCHZZ_EAMM_TRANSITION,
   TDSSRCHZZ_EAMM_BEST,
   TDSSRCHZZ_EAMM_STATE_NUM
 }tdssrchzz_eamm_state_enum_type;

 typedef enum 
 {   
   TDSSRCHZZ_EAMM_SENSOR_MOBILITY,
   TDSSRCHZZ_EAMM_SENSOR_STATIONARY,
   TDSSRCHZZ_EAMM_SENSOR_STATE_NONE
 }tdssrchzz_eamm_sensor_state_enum_type;

typedef struct 
{
  tdssrchzz_eamm_state_enum_type current_state;
  tdssrchzz_eamm_state_enum_type pre_state;
  tdssrchzz_eamm_sensor_state_enum_type sensor_state;
  int16 ratio_rscp;
  int16 cir_snr;
  int32 f_doppler;
  uint8 drx_index;
  boolean enabled;
}tdssrchzz_eamm_struct_type;
extern tdssrchzz_eamm_struct_type tdssrchzz_eamm_info;
#endif
...
extern boolean tdssrchzz_delay_sleep_for_reacq_in_progress;
...
#define TDSSRCHZZ_CHECK_INTRAF_MEAS_DUE( ) \
        (tdssrchzz_meas_sched_manager.tds_meas_timer_expiry_status & \
        (1<< tdssrchzz_meas_sched_manager.intra_freq.freq_index))
          
#define TDSSRCHZZ_CHECK_INTERF_MEAS_DUE(i) \
        (tdssrchzz_meas_sched_manager.tds_meas_timer_expiry_status & \
        (1<<tdssrchzz_meas_sched_manager.inter_freq[i].freq_index))
        
#ifdef FEATURE_TDSCDMA_EAMM_OPT
#define TDSSRCHZZ_RESET_INTRAF_MEAS_TIMER( ) \
{  tdssrchzz_meas_sched_manager.tds_meas_timer_expiry_status &= \
   (~(1<< tdssrchzz_meas_sched_manager.intra_freq.freq_index)); \
   tdssrchzz_meas_sched_manager.intra_freq.count[TDSSRCHZZ_EAMM_BAD] = 0; }

#define TDSSRCHZZ_RESET_INTERF_MEAS_TIMER(x, y) \
{  tdssrchzz_meas_sched_manager.tds_meas_timer_expiry_status &= \
   (~(1<<tdssrchzz_meas_sched_manager.inter_freq[x].freq_index)); \
   tdssrchzz_meas_sched_manager.inter_freq[x].count[y] = 0; }

#else    
#define TDSSRCHZZ_RESET_INTRAF_MEAS_TIMER( ) \
{  tdssrchzz_meas_sched_manager.tds_meas_timer_expiry_status &= \
   (~(1<< tdssrchzz_meas_sched_manager.intra_freq.freq_index)); \
   tdssrchzz_meas_sched_manager.intra_freq.count = 0; }

#define TDSSRCHZZ_RESET_INTERF_MEAS_TIMER(x) \
{  tdssrchzz_meas_sched_manager.tds_meas_timer_expiry_status &= \
   (~(1<<tdssrchzz_meas_sched_manager.inter_freq[x].freq_index)); \
   tdssrchzz_meas_sched_manager.inter_freq[x].count = 0; }
  #endif
        
#define TDSSRCHZZ_CHECK_IRAT_RSSI_MEAS_DUE( ) \
        (tdssrchzz_meas_sched_manager.irat_meas_timer_expiry_status & TDSSRCHZZ_IRAT_RSSI_MEAS_TIMER)
        
#define TDSSRCHZZ_CHECK_IRAT_BSIC_RC_DUE( ) \
        (tdssrchzz_meas_sched_manager.irat_meas_timer_expiry_status & TDSSRCHZZ_IRAT_BSIC_RC_TIMER)

#define TDSSRCHZZ_RESET_IRAT_RSSI_MEAS_TIMER( ) \
{  tdssrchzz_meas_sched_manager.irat_meas_timer_expiry_status &= (~TDSSRCHZZ_IRAT_RSSI_MEAS_TIMER); \
   tdssrchzz_meas_sched_manager.irat_rssi_count = 0; }

#define TDSSRCHZZ_RESET_IRAT_BSIC_RC_TIMER( ) \
{  tdssrchzz_meas_sched_manager.irat_meas_timer_expiry_status &= (~TDSSRCHZZ_IRAT_BSIC_RC_TIMER); \
   tdssrchzz_meas_sched_manager.irat_bsic_rc_count = 0; }

#define TDSSRCHZZ_RESET_IRAT_BSIC_ID_STATUS( ) \
{  tdssrchzz_meas_sched_manager.irat_meas_timer_expiry_status &= (~TDSSRCHZZ_IRAT_BSIC_ID_TIMER); }
...
typedef struct
{
  ...
  #ifdef FEATURE_TDSCDMA_EAMM_OPT
  uint32 count[TDSSRCHZZ_EAMM_STATE_NUM];
  #else
  uint32 count;
  #endif
...
typedef enum
{
  TDSSRCHZZ_IRAT_ABORT_NONE = 0x0,
  TDSSRCHZZ_IRAT_ABORT_GSM = 0x1,
  #if defined(FEATURE_TDSCDMA_TO_LTE) && defined(FEATURE_TDSCDMA_ABSOLUTE_PRIORITY)  
  TDSSRCHZZ_IRAT_ABORT_LTE = 0x10,
  #endif
  TDSSRCHZZ_IRAT_ABORT_RESEL_EVAL_DONE = 0xFF
}
tdssrchzz_irat_abort_mode;

#ifdef FEATURE_TDSCDMA_T2X_COLLIDE_WITH_SUB2_PICH_OPT
typedef enum
{
  TDSSRCHZZ_IRAT_MEAS_SLEEP,
  TDSSRCHZZ_IRAT_MEAS_WAKEUP,
  TDSSRCHZZ_IRAT_MEAS_RESERVE,
  TDSSRCHZZ_IRAT_MEAS_INVALID
}
tdssrchzz_irat_meas_state_enum_type;

typedef enum
{
  TDSSRCHZZ_IRAT_NONE,
  TDSSRCHZZ_IRAT_RESERVE_SET,
  TDSSRCHZZ_IRAT_DL_WAKEUP
}
tdssrchzz_irat_meas_after_dl_enum_type;

typedef struct
{
  uint16                                  trm_denial;
  boolean                                 is_skipped;
  tdssrchzz_irat_meas_after_dl_enum_type  irat_after_dl_state;
  tdssrchzz_irat_meas_state_enum_type     state;
}tdssrchzz_irat_meas_struct_type;

extern tdssrchzz_irat_meas_struct_type tdssrchzz_irat_meas_para;
#endif
...
typedef struct
{
  ...
  #ifdef FEATURE_TDSCDMA_EAMM_OPT
  //tdssrchzz_eamm_state_enum_type meas_period_mode;  
  uint16 interf_meas_period[TDSSRCHZZ_EAMM_STATE_NUM];
  #else
  ...
  #endif  
  tdssrchzz_irat_abort_mode irat_abort_bitmap;
  boolean sleep_aborted;
  tdssrchzz_meas_lock_info_type meas_lock_info;
}tdssrchzz_meas_sched_manager_struct_type;

extern tdssrchzz_meas_sched_manager_struct_type tdssrchzz_meas_sched_manager;
...
SIDE EFFECTS
  None.
===========================================================================*/
extern tdssrchzz_irat_abort_mode tdssrchzz_drx_meas_scheduler ( void );

/*===========================================================================

FUNCTION     TDSSRCHZZ_IRAT_ABORT_MEAS_DONE

DESCRIPTION
  This function is used to send GSM/LTE meas_done if there is irat abort in scheduler.
...
SIDE EFFECTS
  None.
===========================================================================*/
extern void tdssrchzz_irat_abort_meas_done(tdssrchzz_irat_abort_mode irat_abort_status);
...
FUNCTION     tdssrchzz_update_meas_lock_info
...
void tdssrchzz_update_meas_lock_info(uint16 meas_type, uint16 reason);
#endif /* SRCH_ZZ_H */
