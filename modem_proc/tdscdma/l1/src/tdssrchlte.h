...
$Header: //components/rel/tdscdma.mpss/4.3/l1/src/tdssrchlte.h#1 $ DateTime: $ $Author: pwbldsvc $
...
#if defined(FEATURE_TDSCDMA_IDLEINTERVAL_INFO_SUPPORT) || defined(FEATURE_TDSCDMA_DMO_SUPPORT)
  /* whether current LTE measurement gap is aborted or not, LTE may abort may happen before or after gap end */
...
typedef struct
{
  ...
  boolean is_fat_gap;
  /*Check if TFW cleanup rsp is pending*/
  boolean tfw_cleanup_rsp_pending;
...
extern tdssrchlte_cm_scheduler_struct_type tdssrchlte_cm_scheduler_db;
...
typedef struct
{
  ...
  tdssrchlte_cell_struct_type cm_meas_rsp_cell_list[TDSSRCHLTEDRV_MAX_MEAS_CELLS 
	  + TDSSRCHLTEDRV_MAX_NEW_DETECTED_CELLS];
...
typedef struct
{
  uint16 ta_start_subfn;
  uint16 ta_end_subfn;
  uint16 dmo_blocked_by_ta_count;
  uint16 dmo_blocked_by_sub2_idle_dr;
  boolean gap_opened;
  boolean is_lta;  //if dmo is blocked by LTA
  uint16 gap_start_subfn;
  uint16 gap_end_subfn;
} tdssrchlte_forced_gap_schedule_type;

extern tdssrchlte_forced_gap_schedule_type tdssrchlte_forced_gap_schedule;
#endif
...
SIDE EFFECTS
  None
===========================================================================*/
extern boolean tdssrchlte_cm_scheduler_timer_expiring(uint8 freq_idx, uint32 margin);
...
SIDE EFFECTS
  None
===========================================================================*/
extern uint8 tdssrchlte_cm_scheduler_find_best_freq_idx(uint8 freq_idx_to_exclude);
...
#ifdef FEATURE_TDSCDMA_TO_LTE_FORCED_GAP
extern void tdssrchlte_schedule_forced_gap(void);
#endif

extern void tdssrchlte_send_cleanup_done_signal(void);
/*===========================================================================

FUNCTION  tdssrchlte_get_init_send_status

DESCRIPTION:
  This function returns the status if T2L init request is sent. 
...
SIDE EFFECTS
  None
===========================================================================*/
boolean tdssrchlte_get_init_send_status( void );

/*===========================================================================

FUNCTION  tdssrchlte_cleanup_lte_idle_meas

DESCRIPTION:
  This function returns the status if T2L cleanup is ready in idle state. 
...
SIDE EFFECTS
  None
===========================================================================*/
boolean tdssrchlte_cleanup_lte_idle_meas( void );
#endif /* defined(FEATURE_TDSCDMA_IDLEINTERVAL_INFO_SUPPORT) || defined(FEATURE_TDSCDMA_DMO_SUPPORT) */
