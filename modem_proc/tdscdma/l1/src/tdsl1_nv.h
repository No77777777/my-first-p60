...
Notice that changes are listed in reverse chronological order. 
 ...
 $Header: //components/rel/tdscdma.mpss/4.3/l1/src/tdsl1_nv.h#1 $ $DateTime: 2019/08/19 10:51:23 $ $Author: pwbldsvc $
...
/* default values */
  ...
  #define TDSL1_NV_CONTROL_LEVEL_DEFAULT 4
...
#define TDSSRCHGSM_E3A_OPT_TD_THRESHOLD_FOR_CS_ONLY_DEFAULT -85
#define TDSSRCHGSM_E3A_OPT_GSM_THRESHOLD_FOR_CS_ONLY_DEFAULT -40
...
/*Default values for fallback feature*/
#define TDSL1_DR_DSDS_FALLBACK_CONSECUTIVE_PCH_DEC_SUCCESS_TH 2
#define TDSL1_DR_DSDS_FALLBACK_NO_NEED_RXD_TH 1
#define TDSL1_DR_DSDS_FALLBACK_CONSECUTIVE_PCH_DEC_SUCCESS_IN_BETWEEN_TH 2
#define TDSL1_DR_DSDS_FALLBACK_LACK_OF_RXD_TH 2
#define TDSL1_DR_DSDS_FALLBACK_GLITACH_DESENSE_TH 50  //50%
#define TDSL1_DR_DSDS_FALLBACK_MAX 50
...
typedef PACKED struct PACKED_POST{ 
    uint8   version;
    boolean is_fallback_enabled;                             //Enable/Disable Fallback
    uint8   trigger_fallback_consecutive_crc_error_TH;       //Consecutive PCH CRC error number to trigger fallback
	int16   trigger_fallback_RSCP_TH;                        //RSCP threshold to trigger fallback
	uint8   trigger_fallback_lack_of_RxD_TH;                 //Number of lack of RxD for triggering fallback
	uint8   trigger_fallback_DG_TH;                          //ex., 50<->50%. Desense/Glitch threshold to trigger fallback
	uint8   quit_fallback_max_cycle;                         //Maximum DRX cycle staying in fallback mode before exiting fallback
	uint8   quit_fallback_crc_succ_in_between_TH;            //the number of crc success in between two crc error for quitting fallback 
    uint8   quit_fallback_crc_consecutive_succ_TH;           //IF this many crc consecutive success
    uint8   quit_fallback_no_need_RxD_TH;                    //AND this many no need for RxD, THEN quit fallback mode
    uint16  reserved[4];
} tdsl1_nv_fallback_params_type;
...
typedef PACKED struct PACKED_POST{ 
  ...
  //schedule period factor for inter-f in EAMM mode
  uint8 inter_f_period_factor[4];
  //schedule period factor for t2g rssi in EAMM mode   
  uint8 t2g_rssi_period_factor[4];
  uint8 disable_meas;
...
typedef PACKED struct PACKED_POST{ 
  uint16 level; /* 0x0000 default (power saving mode); 
                   ...
                   0x1000 disable inter-f/irat meas in DRX state*/
  uint16 reserved_1;
  uint16 reserved_2;
  uint16 reserved_3;
...
typedef PACKED struct PACKED_POST{ 
    ...
    uint8 type1_band_mask;
    uint8 type2_band_mask;
...
typedef PACKED struct PACKED_POST{ 
  ...
  uint8 type1_switch_holding_time;
	...
	int8 delta_UL_bandE;
	int8 delta_UL_bandF;
  	int8 reserved[10];  // For future use
...
typedef PACKED struct PACKED_POST{ 
   ...
   int8 tdssrchgsm_e3a_opt_gsm_threshold_for_CS_only;
...
/* TDS error fatal recovery params */
typedef PACKED struct PACKED_POST{
  uint8 version;
  uint8 error_fatal_recovery_enable;
  uint8 reserved[6];
} tdsl1_nv_error_fatal_recovery_params_type;
...
extern tdsl1_nv_fallback_params_type tdsl1_nv_fallback_params;
...
/*===========================================================================
FUNCTION tdsl1_get_drx_t2g_meas_nv_item

DESCRIPTION
  This function reads the NV items and returns the T2G (in DRX state)meas
...
===========================================================================*/
tdsl1_nv_drx_t2g_control_struct_type* tdsl1_get_drx_t2g_meas_nv_item(void);
...
/*===========================================================================
FUNCTION tdsl1_get_error_fatal_recovery_item

DESCRIPTION
  This function reads the NV items and returns error_fatal_recovery_item related
  configuration.
...
SIDE EFFECTS
  None
===========================================================================*/
tdsl1_nv_error_fatal_recovery_params_type* tdsl1_get_error_fatal_recovery_params_nv_item(void);
