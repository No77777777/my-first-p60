...
$PVCSPath: $
$Header: //components/rel/tdscdma.mpss/4.3/l1/src/tdssrchset.h#1 $
...
typedef struct
{
  ...
  uint16 filt_slot_snr;  /*Q6*/
  int32 filt_freq_error;  /*Q8*/
/* used to track the gap between 2 freqErr of the same JDS cell; 
  cannot use ts_rsp_time_stamp, because it's updated for 40 cells;
  cannot use rscp_time_stamp, because rscp_time_stamp only updated for A+M+D set, not R set */
  uint16 freq_err_time_stamp; 
  uint16 meas_gap;  /*ms, it's defined to output gap length into log packet 0xD058, can we delete it?*/
#endif
}
...
typedef struct
{
  ...
  /* TS0 Table is used to store information for iterative channel estimation set of RSCP and 
    ...
    * бн, TS0 Table[127]   is corresponding to cpid 127. Each element includes TS-RSP, RSCP, 
...
typedef struct
{
  ...
  /* non-TS0 Table is used to store information for iterative channel estimation set of  
    ...
    * corresponding to cpid 1,бн, non-TS0 Table[127]   is corresponding to cpid 127. Each 
  ...
  int16 cir_snr;
#ifdef FEATURE_TDSCDMA_MCJD_IC
  boolean non_ts0_mcjdIcenable;
#endif
...
#ifdef FEATURE_TDSCDMA_MCJD_IC
/*1.	Enable MCJD IC if filtered TS-RSP of serving cell < filtered TS-RSP of any interfering iterative cell + Threshold_dB and if global NV setting is IC_ON.*/
#define TDSSRCHSET_TS_RSP_ENABLE_MCJD_THRESHOLD      10*256 
#endif
#endif
...
/*===========================================================================
FUNCTION     TDSSRCHSET_INTER_S_FIND_CELL_IN_MCM

DESCRIPTION
  This function will search one GSM neighbor cell to find whether it
  has an ARFCN and BSIC that match those sent in MCM.

DEPENDENCIES
  None

INPUT PARAMETERS 
  tdssrchset_gsm_cell_struct_type *cell_ptr: GSM neighbor cell.

RETURN VALUE
  Boolean - if TRUE, the GSM neighbor cell is in MCM
                                
SIDE EFFECTS
  None
===========================================================================*/
boolean tdssrchset_inter_s_find_cell_in_mcm
( tdssrchset_gsm_cell_struct_type *cell_ptr, tdsl1_inter_sys_cell_list_struct_type * p_cell_list);
...
/*=========================================================================
 ...
 FUNCTION     :TDSSRCHSET_IS_MCJD_IC_ENABLED
 
 DESCRIPTION  : This function return whether MCJD_IC feature is enabled.
 ...
 SIDE EFFECTS : 
==========================================================================*/
extern boolean tdssrchset_is_mcjd_ic_enabled(void);
#endif
#endif /* TDSSRCH_SET_H */
