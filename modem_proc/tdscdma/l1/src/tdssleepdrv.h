...
$Header: //components/rel/tdscdma.mpss/4.3/l1/src/tdssleepdrv.h#1 $    $DateTime: 2019/08/19 10:51:23 $ $Author: pwbldsvc $

when       who         what, where, why
--------   ---         --------------------------------------------------------
05/05/09   rmak        Moved all content to tdsl1sleep.h
...
11/30/08   saik        Removed extern references to non-existent variables and added reference to global variable tdsslp_FEE_time_in_ms
11/19/08   rmak        Added tdsl1sleep_drv_set_rf_wu_time_sc to allow feedback to sleep_drv for RF WU time
...
09/30/08   mg          Added tdsslp_frame_num_immt_sdt
09/24/08   rgn         Added QXT thresholds
08/15/08   uk          Externed tdsdrx_wtog_bplmn_in_progress flag
...
05/22/08   kps         Changed tdsl1sleep_compute_current_cell_time() to return a boolean.
...
10/10/07   mg          Add tdsslp_ctlr_int_status_type
09/24/07   rmak        Separated TCXO and RF warmup time
                       Updated return type of tdsl1sleep_sleepdrv_set_sleep_timer_1_count()
...
06/29/07   mg          Make tdsslp_drv_ols_has_occured volatile
...
02/28/07   kps         Fix FEATURE_TDSCDMA_L1_MMGPS feature support
01/29/07   mg          Extern the variable tdsslp_drv_multiple_int
01/09/07   rmak        Updated tdsl1sleep_drv_init
12/20/06   rmak        Feature and extern variable cleanup, updated tdsl1sleep_drv_setup
...
11/13/06   sk          GSM timing error fix. Defined extern tdsslp_frc_immt_sdt_Cx8.
10/18/06   mg          Add tdsl1sleep_sleepdrv_get_programmed_coarse_time and format
...
07/20/06   kps         Add tdsslp_get_num_sleep_clks_for_fee() declaration
07/12/06   mg          Shared sleep controller support
07/07/06   mg          Add macros to install/uninstall slp ctlr isr
06/20/06   asr         Added declaration for tdsl1sleep_drv_abort_timeout_handled()
...
04/06/06   mg          Extern tdsl1sleep_drv_compute_gfn_gqsym_at_ols and tdsslp_gqsym_ols
03/20/06   mg          Add SLEEP_DRV_RESET_SLEEP_TIMER_1() macro
02/10/06   mg          Change the tdsl1sleep_compute_current_cell_time declaration.
...
05/31/05   mg          Extern tdsslp_gsm_clk_enabled and sleep_drv_cst_has_occured
...
05/09/05   scm         Add boolean wake_up_for_ctch parameter to tdsl1sleep_drv_setup().
...
11/10/04   asr         Add externs for tdsslp_short_qxt, and functions to set qxt parameters.
10/26/04   asr         Added extern for tdsdrx_ticks_rem_adj_sleep_longer_Cx8 and couple others.
05/12/04   scm         Add SLEEP_LSM_OKTS_SIG to SLOW_REX_TICKS_CONSENSUS.
03/09/04   jac         Added kypd vote in SLOW_REX_TICKS_CONSENSUS
03/01/04   asr         Added TDSSLP_CTLR_BEF_SLEEP_START to tdsslp_ctlr_int_type
...
09/25/03   asr         Added declaration of sleep_drv_ready_for_wake_up(), and tdsslp_drv_slam_gfn(),
                       tdsl1sleep_drv_qsyms_until_gtime_ready(), tdsl1sleep_drv_prepare_for_gtime_snapshot(), and
                       tdsl1sleep_drv_gtime_snapshot().                 
08/07/03    sr         Added function tdsl1sleep_drv_in_sleep() to return the 
                       status of sleep driver.
08/03/03   asr         Declare a tdsl1sleep_drv_exit() function, and sleep_drv_abort_fee().
07/01/03   xfg         Changed for Thin UI build
02/12/03   asr         Declared function tdsl1sleep_drv_adj_sleep_clk_freq()
...
07/03/02   asr         Added sleep_drv_oktcxo and tdsslp_fee_completed, boolean flags
06/11/02   asr,abp     Initial cut

===========================================================================*/

#include "tdsl1sleep.h"
