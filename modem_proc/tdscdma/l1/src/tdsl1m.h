...
$Header: //components/rel/tdscdma.mpss/4.3/l1/src/tdsl1m.h#2 $ $DateTime: 2021/02/04 23:39:42 $ $Author: pwbldsvc $
...
#define TDSL1M_FW_SRCH_SYNCDL_MID_DET_RSP_IRAT_TIMEOUT_MSEC  30 /* 30 ms should be enough for IRAT case*/
...
#define TDSL1M_MAX_BARRED_CELLS (12+8*16) /*Just an arbitary no. 2 * ASET size*/
#else
#define TDSL1M_MAX_BARRED_CELLS 12
...
#define TDSL1M_FW_ERR_BUF_SIZE 10
#define TDSL1M_FW_ERR_TIMER_LEN_MSEC 60000
#define TDSL1M_FW_ERR_CRASH_THRES 3
#endif
...
typedef struct
{
  /* local command header type */
  tdsl1def_l1_local_cmd_hdr_type       hdr;
}tdsl1m_blind_psho_do_meas_type;
...
//jintao add for dl fach race condition
  ...
  ** TDS_DRX_ENTITY_CMD for commands for a particular channel state machine. */
  ...
  tdsl1_dsds_trm_tx_grant_callback_cmd_type l1m_dsds_trm_tx_grant_callback_cmd;
...
tdsl1m_pending_cleanup_proc_cmd_type pending_cleanup_proc_cmd;
  ...
  tdsl1_tdrm_cm_inv_timeout_cmd_type tdsl1_tdrm_cm_inv_timeout_cmd;

  tdsl1m_blind_psho_do_meas_type  tdsl1m_blind_psho_do_meas;
...
typedef struct
{
  ...
  uint32                      wallTime;
  /*! \brief Detected Error Info - tfw_err_type_e converted to bitmask*/
  uint32                     errorInfo;
}tdsl1m_fw_err_info;
#endif 
...
//#endif
  ...
  uint16 last_even_actSfn_multi_sim_cfg_cmd;
  uint16 last_odd_actSfn_multi_sim_cfg_cmd;
  ...
  boolean fw_err_crash_flag;
  uint8 fw_err_buf_idx;
  uint8 fw_err_cnt;
  uint8 fw_err_last_cnt;
  uint8 fw_err_cnt_in_timer;
  tdsl1m_fw_err_info fw_err_info_buf[TDSL1M_FW_ERR_BUF_SIZE];
  rex_timer_type  fw_err_freq_check_timer;
#endif

  boolean fs_acq_in_progress;

  boolean ext_cmd_in_process;
  boolean block_wait_spec_state;
  tfw_state_t new_cfg_tfw_state;
  
  boolean rrc_abort_sig_set_flag;
  #ifdef FEATURE_TDSCDMA_MSGR_VARIANT
  sys_modem_as_id_e_type x2t_irat_as_id;
  #endif
}tdsl1m_global_type;
...
extern boolean tdsl1m_gsm_init_before;
...
#ifdef FEATURE_TDSCDMA_ATLAS
/*===========================================================================

FUNCTION      tdsl1_rsb_send_fw_cmd
...
SIDE EFFECTS
  None.
===========================================================================*/
void tdsl1_rsb_send_fw_cmd(void);
...
SIDE EFFECTS
  None.
===========================================================================*/
boolean tdsl1m_tdrm_request_and_wait_lock(tdsl1_ext_cmd_type*  ext_cmd_ptr, trm_reason_enum_t reason, trm_request_tag_t tag);
...
void tdsl1_ds_grant_cb(trm_client_enum_t client,
  trm_grant_return_enum_type grant, trm_request_tag_t tag);
...
extern uint8 tdsl1m_get_current_call_type(void);
...
FUNCTION    tdsl1m_fw_err_recovery_nv

DESCRIPTION
          return if the NV of fw_error_recovery is enabled or not
...
SIDE EFFECTS
  None
===========================================================================*/
extern uint8 tdsl1m_fw_err_recovery_nv(void);

#ifdef FEATURE_TDSCDMA_FATAL_RECOVERY
/*===========================================================================

FUNCTION    tdsl1m_set_fw_err_timer

DESCRIPTION
          set timer for statistics FW ERROR_IND frequency and save the current err_cnt
...
SIDE EFFECTS
  None
===========================================================================*/
void tdsl1m_set_fw_err_timer(void);
...
SIDE EFFECTS
  None
===========================================================================*/
void tdsl1m_fw_err_cleanup(void);

/*===========================================================================

FUNCTION    tdsl1m_fw_err_timer_expired_cb
...
SIDE EFFECTS
  None
===========================================================================*/
void tdsl1m_fw_err_timer_expired_cb(uint32 unused);

/*===========================================================================

FUNCTION    mcaltds_l1_trigger_fw_err_cmd

DESCRIPTION
          this function is a hack function which is used to trigger FW report FW_ERROR_IND
...
SIDE EFFECTS
  None
===========================================================================*/
void mcaltds_l1_trigger_fw_err_cmd(void);
...
SIDE EFFECTS
  None
===========================================================================*/
void tdsl1_qsh_tc_metrics_init(void);
...
FUNCTION tdsl1m_clear_ext_cmd_in_proc

DESCRIPTION
  clear ext_cmd_in_process flag when receive below ext_cmd.
  stop/suspend/resume_tdscdma_req;cphy_setup; cell_transition
...
===========================================================================*/
void tdsl1m_clear_ext_cmd_in_proc(tdsrrc_l1_cmd_type* cmd_ptr);

/*===========================================================================
FUNCTION  tdsl1m_abort_curr_lock_wait

DESCRIPTION

  After activity based TDRM, RRC would trigger signal to abort L1 lock wait procedure.
  If L1 is waiting for lock, L1 would abort successfully.
  If L1 isn't waiting for lock, L1 just ignre this abort action.
...
===========================================================================*/
void tdsl1m_abort_curr_lock_wait(void);

#ifdef FEATURE_TDSCDMA_T2X_BPLMN_PAGE_RATE_OPT
/*===========================================================================

FUNCTION  tdsl1m_is_channel_set_suspend

DESCRIPTION

  this function is used by DL to check if the initial setup for PCH need to set to suspend state.
...
RETURN VALUE
  ...
  TRUE: suspend PCH when setup; FALSE: normal case, no need to suspend.
...
===========================================================================*/
boolean tdsl1m_is_resume_timline_opt(void);
#endif
