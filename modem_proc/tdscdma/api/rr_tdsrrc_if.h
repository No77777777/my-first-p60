...
$Header: //components/rel/tdscdma.mpss/4.3/api/rr_tdsrrc_if.h#1 $   
...
typedef struct
{
  ...
  /*Indicate GSM return to LTE directly or not after this CSFB call end*/
...
#endif
  /*Set the flag according to current TDS DRX count
   *True:GSM use ACQ_INV for BPLMN search
   *False:GSM use ACQ for BPLMN search
   */
...
/* New type that establishes Connection types */
/* Default NONE, indicates all Redirections not pertaining Reject and for other use cases */
typedef enum
{
  TDSRR_CAUSE_NONE,
  TDSRR_CAUSE_MO_CS_CALL,
  TDSRR_CAUSE_MT_CS_CALL
} tdsrr_est_cause_e_type;
...
typedef struct
{
  ...
  tdsrr_est_cause_e_type            call_est_cause;
