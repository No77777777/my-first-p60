...
$Header: //components/rel/tdscdma.mpss/4.3/api/tds_l1_api.h#1 $    $DateTime: 2019/08/19 10:51:23 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
03/06/15   lhu     added tx pwr statistics api to data service
...
#define TDSSRCH_TX_PWR_DEFAULT               0x8000
...
typedef struct
{
    ...
    /* Tx Power in unit of Q4. RRC gets raw data from lower layer and passes to upper layer
     in order to keep the accuracy. Conversion (divide 16 and store as float number)
     needs to be done at upper layer.
  */
  int16 AgcValue;
  ...
  */
  uint8 paGainState;
  /*Ts[0] is for UPPCH, and ts[1~5] for TS1 to TS5*/
  uint8 timeslot;
