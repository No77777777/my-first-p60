...
/* DEO_IC_NVIC (RPM IC) */
#if (defined (COMPILING_AOP_PROC) || defined (COMPILING_SSC_SDC_PROC))
#ifndef DEO_IC_NVIC
#define DEO_IC_NVIC
#endif /* DEO_IC_NVIC */
#endif /* COMPILING_AOP_PROC || COMPILING_SSC_SDC_PROC */
 ...
 */
     ...
     defined (COMPILING_MODEM_PROC) || \
     defined (COMPILING_SSC_PROC)   || \
     defined (COMPILING_TURING_PROC))
#ifndef DEO_IC_L2VIC
#define DEO_IC_L2VIC
#endif /* DEO_IC_L2VIC */
#endif /* COMPILING_Q6SS_PROC || COMPILING_LPASS_PROC ||
        * COMPILING_MSS_PROC  || COMPILING_MODEM_PROC ||
        * COMPILING_SSC_PROC  || COMPILING_TURING_PROC
        */
...
/* Now get the needed IC driver header file */
    ...
    #if !defined(DEO_GIC_MOCK_TEST)
        #include "VVDRV_qgic.h"
    #else /* DEO_GIC_MOCK_TEST */
        #include "VVTEST_deo_mock_qgic.h"
    #endif /* DEO_GIC_MOCK_TEST */
    #include "apps_proc_common.h"          /* get_cpuid() */
#elif defined(DEO_IC_NVIC)
    #if !defined(DEO_GIC_MOCK_TEST)
        #include "VVDRV_nvic.h"
    #else /* DEO_GIC_MOCK_TEST */
        #include "VVTEST_deo_mock_nvic.h"
    #endif   /* DEO_GIC_MOCK_TEST */
#elif (defined(DEO_IC_L2VIC) && defined (COMPILING_MSS_PROC))
    #if !defined(DEO_GIC_MOCK_TEST)
        #include "MssQ6IntHandler.h" /* at the moment it is the same */
    #else /* DEO_GIC_MOCK_TEST */
        #include "VVTEST_deo_mock_l2vic.h"
    #endif   /* DEO_GIC_MOCK_TEST */
...
#if !defined(DEO_GIC_MOCK_TEST)
    #define DEO_GIC_MOCK(function) function
#endif  /* DEO_GIC_MOCK_TEST */
...
)
{
    ...
    else if (DEO_INT_SECURE_LEVEL_SECURE_GROUP0 == sec_level)
    {
        return HAL_QGIC_SECURE_G0;
    }
...
static uint32 deo_get_gic_targets(deo_interrupt_targets_t targets)
{
    ...
    {
        gic_targets |= HAL_QGIC_TARGET_CPU(get_cpuid());
    }
...
static void deo_interrupt_isr_wrapper(HAL_qgic_InterruptType eInterrupt)
{
     ...
     */
    if ((TRUE == DEO_GIC_MOCK(VVDRV_qgic_isPending)(eInterrupt)) ||
        (TRUE == DEO_GIC_MOCK(VVDRV_qgic_isActive)(eInterrupt)))
    {
...
int32 VVDRV_deo_interrupt_attach_os(const deo_interrupt_t *pInterrupt)
{
    ...
    DEO_GIC_MOCK(VVDRV_qgic_installIsr)(&sInt_info);
...
{
    if (FALSE == DEO_GIC_MOCK(VVDRV_qgic_isEnabled)(int_source))
    {
        DEO_GIC_MOCK(VVDRV_qgic_enable)(int_source);
    }
...
{
    if (TRUE == DEO_GIC_MOCK(VVDRV_qgic_isEnabled)(int_source))
    {
        DEO_GIC_MOCK(VVDRV_qgic_disable)(int_source);
    }
...
{
    DEO_GIC_MOCK(VVDRV_qgic_clearPending)(int_source);
}
...
int32 VVDRV_deo_interrupt_init_os(const deo_interrupt_controller_init_t *config)
{
    ...
    DEO_GIC_MOCK(VVDRV_qgic_init)(&qgicVer);  /* QGIC version not needed */
...
static void deo_interrupt_isr_wrapper(void)
{
    ...
    uint32 eInterrupt = DEO_GIC_MOCK(VVDRV_nvic_GetActiveNumber)();
...
int32 VVDRV_deo_interrupt_attach_os(const deo_interrupt_t *pInterrupt)
{
    ...
    DEO_GIC_MOCK(VVDRV_nvic_InstallIsr)(&nvicInfo);
...
int32 VVDRV_deo_interrupt_enable_os(uint32 int_source)
{
    if (FALSE == DEO_GIC_MOCK(VVDRV_nvic_IsEnable)(int_source))
    {
        DEO_GIC_MOCK(VVDRV_nvic_Enable)(int_source);
    }
...
int32 VVDRV_deo_interrupt_disable_os(uint32 int_source)
{
    if (TRUE == DEO_GIC_MOCK(VVDRV_nvic_IsEnable)(int_source))
    {
        DEO_GIC_MOCK(VVDRV_nvic_Disable)(int_source);
    }
...
void VVDRV_deo_interrupt_clear_os(uint32 int_source)
{
    DEO_GIC_MOCK(VVDRV_nvic_ClearPending)(int_source);
}
...
int32 VVDRV_deo_interrupt_attach_os(const deo_interrupt_t *pInterrupt)
{
    ...
    if (TRUE != DEO_GIC_MOCK(VVDRV_qdsp6_q6l2vicConfigInt)
                                          (pInterrupt->int_source, trigger))
    {
        return DEO_ERROR;
    }

    if (TRUE !=
        DEO_GIC_MOCK(VVDRV_qdsp6_q6l2vicInstallHandler)
                           (pInterrupt->int_source, deo_interrupt_isr_wrapper))
    {
        return DEO_ERROR;
    }
...
int32 VVDRV_deo_interrupt_enable_os(uint32 int_source)
{
    DEO_GIC_MOCK(VVDRV_qdsp6_q6l2vicIntEnable)(int_source);
...
int32 VVDRV_deo_interrupt_disable_os(uint32 int_source)
{
    DEO_GIC_MOCK(VVDRV_qdsp6_q6l2vicIntDisable)(int_source);
...
void VVDRV_deo_interrupt_clear_os(uint32 int_source)
{
    DEO_GIC_MOCK(VVDRV_qdsp6_q6l2vicIntClear)(int_source);
}
