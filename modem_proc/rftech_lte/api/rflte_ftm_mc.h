...
$Header: //components/rel/rftech_lte.mpss/3.11/api/rflte_ftm_mc.h#1 $
...
10/27/15   kr      move tx disable to mc_config
10/28/15   ska     Adding support for successive same band cal with different modem chains
08/25/15   zhw     Added LTE leagcy dev to carrier mapping support
06/29/15   stw     Single Rx Mask added to FTM state machine
06/24/15   stw      LTE_U : Rx Cal Crash Fix due to invalid TDD DISABLE REQ in DL only bands
06/17/15   pkg      Changes to get GRFC settings from RFC and program accordingly for ASM ON/OFF.
04/28/15   sb      [RX/TX split] Fixes to support TX/RX cal
04/08/15   pkg     Changes to support Tx Rx Split Band design.
03/24/15   JJ      Move intra_band ulca flag from ftm to online state machine
03/23/15   JJ      Get rb info api use carrier idx as input param
...
typedef struct
{
   /* Tx waveform info, input from QRCT */
   byte waveform;
   byte num_rbs_pusch;
   byte num_rbs_pucch;
   byte start_pusch_rb_index;
} rflte_ftm_mc_waveform_info;
...
typedef struct
{
   ...
   /* Tx waveform info from QRCT*/
   rflte_ftm_mc_waveform_info waveform_info;
...
void rflte_ftm_mc_sm_set_fw_txagc_start(uint32 carrier_idx, boolean flag);
...
rfcom_lte_band_type rflte_ftm_mc_sm_carrier_get_drx_band(uint32 carrier_idx);
#ifdef FEATURE_RF_HAS_WTR3925
rfm_device_enum_type rflte_ftm_mc_sm_get_carr_drx_device(uint32 carrier_idx);
rfcom_lte_band_type rflte_ftm_mc_sm_carrier_get_prx_band(uint32 carrier_idx);
lm_handle_type rflte_ftm_mc_sm_get_carr_rxlm_buffer(uint32 prx_drx_flag,uint32 carrier_idx);
void rflte_ftm_mc_sm_set_carrier_fw_rxagc_start(boolean flag, uint32 carrier_index);
#endif
...
lm_handle_type rflte_ftm_mc_sm_get_carrier_txlm_buffer(uint32 carrier_index);
rfa_rf_lte_ftm_ul_modulation_type rflte_ftm_mc_sm_get_ul_mod_type(uint32 carrier_idx);
...
uint8 rflte_ftm_mc_sm_get_num_rbs(uint32 carrier_idx);
uint8 rflte_ftm_mc_sm_get_start_rb(uint32 carrier_idx);
...
#ifdef FEATURE_RF_HAS_WTR3925
boolean rflte_ftm_mc_sm_carrier_is_mdsp_rxagc_started(uint32 carrier_idx );
#endif
boolean rflte_ftm_mc_sm_is_mdsp_txagc_started(uint32 carrier_idx);
...
void rflte_ftm_mc_sm_set_waveform_info(uint32 currier_idx, byte waveform, byte num_rbs_pusch, byte num_rbs_pucch, byte start_pusch_rb_index);
rflte_ftm_mc_waveform_info* rflte_ftm_mc_sm_get_waveform_info(uint32 currier_idx);
...
boolean rflte_ftm_mc_chan_modulation(uint32 carrier_idx, rfa_rf_lte_ftm_ul_modulation_type requested_modulation);
...
void rflte_ftm_mc_set_asm_on_off_grfc(rfm_device_enum_type device,
...
uint8 rflte_ftm_mc_sm_get_modem_chain(uint32 carrier_idx);
void rflte_ftm_mc_sm_set_modem_chain(uint8 modem_chain,uint32 carrier_idx);

void rflte_ftm_mc_set_lte_clock_new(rflte_ftm_state_type mcpm_req, rfcom_lte_bw_type lte_bw, boolean is_clock_started);
boolean rflte_ftm_mc_tx_state(uint32 carrier_idx);
...
/*----------------------------------------------------------------------------*/
boolean rflte_ftm_mc_get_intra_band_ul_ca_flag(void);
/*----------------------------------------------------------------------------*/
void rflte_ftm_mc_set_intra_band_ul_ca_flag(boolean flag);
/*----------------------------------------------------------------------------*/
boolean rflte_ftm_set_sm_dev(uint32 carr_index, rf_lte_dev_type dev_type, rfm_device_enum_type device);
/*----------------------------------------------------------------------------*/
rfm_device_enum_type rflte_ftm_get_sm_dev(uint32 carr_index, rf_lte_dev_type dev_type);
/*----------------------------------------------------------------------------*/
boolean ftm_lte_update_ca_mask(uint32 carrier_index, boolean set_flag);
/*----------------------------------------------------------------------------*/
boolean ftm_lte_get_valid_logical_devices(rflte_mc_cell_idx_type carrier_index,
                                          rfcom_lte_band_type band,
                                          rflte_mc_device_info_type *ret_params,
                                          uint16 devices_used_mask);
/*----------------------------------------------------------------------------*/
boolean ftm_lte_validate_logical_devices(rflte_mc_cell_idx_type carrier_index,
                                         rfcom_lte_band_type band,
                                         rflte_mc_device_info_type *input_params,
                                         uint16 devices_used_mask);
/*----------------------------------------------------------------------------*/
boolean ftm_lte_get_devices(rflte_mc_cell_idx_type carr_idx, rflte_mc_device_info_type *device_info);
/*----------------------------------------------------------------------------*/
ftm_rsp_pkt_type ftm_lte_set_devices(void  *ftm_req_data);
/*----------------------------------------------------------------------------*/
void ftm_lte_reset_device_map(void);
/*----------------------------------------------------------------------------*/
uint32 ftm_lte_get_ca_mask(void);
/*----------------------------------------------------------------------------*/
boolean ftm_lte_reset_carr_device_map(uint8 carrier_idx);
/*----------------------------------------------------------------------------*/
boolean rflte_ftm_set_device_map(uint8 old_dev, uint8 new_dev, boolean clear_all_flag);
/*----------------------------------------------------------------------------*/
boolean rflte_ftm_get_device_map(uint8 old_dev, uint8 *new_dev);
/*----------------------------------------------------------------------------*/
boolean rflte_ftm_mc_legacy_device_to_carrier(rfm_device_enum_type legacy_device,
                                              rflte_mc_cell_idx_type *carrier_index,
                                              rf_lte_dev_type *device_type);
/*----------------------------------------------------------------------------*/
void rflte_ftm_mc_build_exit_config_req_table(rfa_rf_lte_config_req_s *rflte_config_req,
                                                                       uint32 carrier_index,
                                                                       boolean is_rx_config);
