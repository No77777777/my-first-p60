...
$Header: //components/rel/rftech_lte.mpss/3.11/api/rfc_lte.h#1 $
...
public:
  ...
  virtual void* mode_config_get(rfm_device_enum_type device, rfc_mode_config_data_enum_type config_data, uint8 alt_path);
  ...
  virtual void *get_fb_path_config(rfm_device_enum_type device_fb, rfcom_lte_band_type band, boolean et_enable);											 
  
  virtual rfm_device_enum_type get_fb_device(rfm_device_enum_type tx_device, rfcom_lte_band_type tx_band, rfcom_lte_band_type rx_band);

  virtual uint32 get_fb_chain(rfm_device_enum_type tx_device, rfcom_lte_band_type tx_band);
  ...
  boolean get_updated_tx_port(rfm_device_enum_type rfm_dev, 
  ...
  void get_band_based_on_list(rflte_mc_band_decision_config_params_s_type* input_list, uint32 list_size);
...
boolean rfc_get_lte_properties (rfc_lte_properties_type **ptr);
  ...
  virtual boolean configure_asm_xsw_port
  ( 
    rfm_device_enum_type device,
    rfcom_lte_band_type band
  );

  virtual boolean generate_xsw_sleep_script
( 
...
private:
  ...
  boolean candidate_band_insert(rflte_mc_band_index_list_s_type* band_list,uint32* band_list_size,rfcom_lte_band_type candidate_band,uint32 associated_list_index);
