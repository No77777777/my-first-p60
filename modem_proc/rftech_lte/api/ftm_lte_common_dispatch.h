...
$Header: //components/rel/rftech_lte.mpss/3.11/api/ftm_lte_common_dispatch.h#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
12/02/15   sml     Phase 2 device status read
08/04/15   krg     FTM command interface for enabling/disabling debug flags during runtime
04/08/15   pkg     Changes to support Tx Rx Split Band design.
03/25/15   jmf     [selftest] Redefine response packets
02/20/15    br     Added FTM interface for FTM_RF_IQ_CAPTURE_ANALYZE_SAMPLES() Command.
01/15/15   JJ      Moved set carrier idx declaration from dispatch.c to header file
...
typedef struct
{
  ...
  int16 range12_ripple;
  /*!< Center of carrier band ripple in dB x1000*/

  int16 range21_ripple;
  /*!< Edge of carrier band ripple in dB x1000*/

  int16 range1_ripple_extreme;
  /*!< Center of carrier band ripple in dB x1000*/

  int16 range2_ripple_extreme;
  /*!< Edge of carrier band ripple in dB x1000*/

  int16 range12_ripple_extreme;
  /*!< Center of carrier band ripple in dB x1000*/

  int16 range21_ripple_extreme;
  /*!< Edge of carrier band ripple in dB x1000*/
...
/*!
  @brief
  Configure FTM LTE Debug Mode Request Packet
*/
typedef PACK(struct)
{
  uint8   cmd;                /*!< Command Identification */
  uint8   sub_sys;            /*!< Sub-system Identification. For instance - 10 for FTM HWTC */
  uint16  mode;               /*!< RF Mode Identification. For Instance - 29 for FTM_LTE_C */
  uint16  ftm_cmd_id;         /*!< FTM Sub Command. Example-  FTM_LTE_CONFIGURE_DEBUG_MODE */
  uint8   debug_en;           /*!< Debug Enable */
  uint32  debug_mask;         /*!< Debug Mask */
  uint32  reserved;           /*!< Reserved*/
}
ftm_lte_debug_mode_request_packet_type;

/*!
  @brief
  Configure FTM LTE Device status emable Request Packet
*/
typedef PACK(struct)
{
  uint8   cmd;                /*!< Command Identification */
...
ftm_lte_device_status_enable_request_packet_type;

/*!
  @brief
  Configure FTM LTE Device status Request Packet
*/
...
typedef PACK(struct)
{
  ...
  uint8   sub_sys;            /*!< Sub-system Identification. For instance - 10 for FTM HWTC */
  uint16  mode;               /*!< RF Mode Identification. For Instance - 29 for FTM_LTE_C */
  uint16  ftm_cmd_id;         /*!< FTM Sub Command. Example-  FTM_LTE_DEVICE_STATUS_READ */
  uint8   device_id;           /*!< device_id */
...
ftm_lte_device_status_read_request_packet_type;


/*----------------------------------------------------------------------------*/
/*!
  @brief
  Configure FTM LTE Debug Mode Response Packet
*/
typedef PACK(struct)
{
  uint8   cmd;            /*!< Command Identification */
  ...
  uint16  mode;           /*!< RF Mode Identification. For Instance - 29 for FTM_LTE_C */
  uint16  ftm_cmd_id;     /*!< FTM Sub Command. Example-  FTM_LTE_CONFIGURE_DEBUG_MODE */
  uint32  ftm_lte_debug_mode_status;   /*!< Status */
}
ftm_lte_debug_mode_response_packet_type;

/*!
  @brief
  Configure FTM LTE Device Read enable Response Packet
*/
...
typedef PACK(struct)
{
  ...
  uint8   sub_sys;        /*!< Sub-system Identification. For instance - 10 for FTM HWTC */
  uint16  mode;           /*!< RF Mode Identification. For Instance - 29 for FTM_LTE_C */
  uint16  ftm_cmd_id;     /*!< FTM Sub Command. Example-  FTM_LTE_DEVICE_STATUS_READ_ENABLE */
  uint32  ftm_lte_debug_mode_status;   /*!< Status */
}
ftm_lte_device_status_enable_response_packet_type;

/*!
  @brief
  Configure FTM LTE Device Read Response Packet
*/
...
typedef PACK(struct)
{
  ...
  uint16  mode;           /*!< RF Mode Identification. For Instance - 29 for FTM_LTE_C */
  uint16  ftm_cmd_id;     /*!< FTM Sub Command. Example-  FTM_LTE_DEVICE_STATUS_READ */
  uint32  ftm_lte_device_read_status;   /*!< Status */
}
ftm_lte_device_status_read_response_packet_type;
...
uint32 ftm_lte_set_carrier_index(void *ftm_req_data);
