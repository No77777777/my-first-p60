...
$Header: //components/rel/rftech_lte.mpss/3.11/api/rflte_nv.h#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
02/07/20   Hari    [CR 2617731] Added TX static parse function prototype
01/27/20   ssl     [CR2610702]Adding suport for RFNV_DATA_TYPE_PEMAX_ADJUST data type
01/27/20   ssl     [CR 2610646] AMPR back off based on UE power class
12/23/19   ssl     Added rfnv_tx_sar_power_limit_control_static_combo_data_type in tx_path_nv_data
10/26/17   sk      Enable LTE B71 support
02/10/17   sk      NV changes for RFNV_DATA_TYPE_TX_MAX_POWER_LIMIT_ADJUST.
06/30/16   kr      FR 36068: Separate Freq. Comp. for LTE40 on QPA4340
05/11/16   rp      MPR/AMPR CA_NS NV override support
05/11/16   kr      pin/pout NV support for LTE 40MHz
03/01/16   mah     Expanded Tx NV tx_cal_chan to accomodate 32-bit EARFCNs.
12/23/15   kga     LTE band specific spur NV support to contain spur data for all path idx( nv containers)
12/14/15   gvn     Static NV re-structuring
11/18/15   cv      Added support for pa bias offset based on earfcn
11/18/15   sg      KW error fixes
...
10/08/15   kr      Fix KW error
09/25/15   kr      Added support for AMPR backoff
09/25/15   sb      FR30081:Apply gain offsets for ASDIV switch position change
09/23/15    pv     Added support for Intra-band ULCA PA swp.
09/02/15   kr      Added support for tx max pwr based EARFCN NV
08/26/15   kab     Multi-Cluster MPR support
8/24/15    kga     Changed TX and RX split order in struct rfnv_lte_hw_path_config_type
08/07/15   sg      FR27572: Add AsDiv SAR backoff support
08/04/15    pv     Added support for NV hook up for Tx Static Data.
07/29/15   kr      conversion of legacy NV CA BW CLASS COMBOS to NV 28874
07/24/15   stw     LTE_U : Bands 252 , 255 added
07/12/15   stw     Parsing of CA BW CLASS COMBOS from NV 28874
06/23/15   sbc     Add support for pa_output_cap_offset 
...
02/12/15   stw     Added unified static to nv table
01/06/15   kab     LTE B32, LTE-U 125,126,127 Support
01/06/15   yzw     Add NV support for RX paths and alt path MTPL
...
#define RFLTE_MAX_ALT_PATH_MTPL_NV_CONTAINERS    16
#define RFLTE_MAX_RX_PATH_NV_CONTAINERS    8  
#define RFLTE_MAX_RX_PATH_LEGACY_NV_CONTAINERS    4
...
#define RFLTE_MAX_ASDIV_SW_POS             2
/* 120 elements */
#define RFLTE_MAX_STATIC_ARRAY_SZ          ( RFLTE_MAX_RX_NV_ELEMENTS * (RFNV_MAX_LNA_GAIN_STATES - 1) )
...
#define RFNV_MAX_NS_SIZE 4

#define RFNV_MAX_POWER_CLASS 5
...
/*!
  ...
  e.g. LTE B1 currently requires to support a max. of 24 AMPR NV
  items.
*/
#define RF_LTE_MAX_AMPR_NVS                24
...
/*!
  @brief
  These indicate the mapping of C0 and C1 Tx NV's to the index of 
  LTE Tx container NV structure
*/
...
/*!
  @brief
  Maximum size of the SAR back-off list (for default/swap antenna position)
*/
#define RFLTE_NV_ANT_POS_BASED_SAR_BACK_OFF_LIST_MAX_SIZE 50
...
typedef struct
{
  /* Flag to indicate NV container is present/detected */
  boolean nv_cont_detected;
  /* Pointer to an array of ASDIV TX gain offset type 
   * of size of number of ASDIV switch positions
   */
  rfnv_asdiv_tx_gain_offset_type *tx_asdiv_gain_offset_ptr;
}rfnv_tx_asdiv_gain_offset_info_type;
...
/*!
   @brief
   Enum definitions for the no of splits in a full band
...
typedef enum 
{
   /*!< Enum definition for zero splits in a full band */
   RFLTE_HW_PATH_CONFIG_NO_SPLIT = 1,

/*!< Enum definition for one split in a full band */
   RFLTE_HW_PATH_CONFIG_1_SPLIT = 3 ,

/*!< Enum definition for two split in a full band */
   RFLTE_HW_PATH_CONFIG_2_SPLIT = 7,

/*!< Enum definition for three split in a full band */
   RFLTE_HW_PATH_CONFIG_3_SPLIT = 15,

/*!< Enum definition for four split in a full band */
   RFLTE_HW_PATH_CONFIG_4_SPLIT = 31,

/*!< Enum definition for five split in a full band */
   RFLTE_HW_PATH_CONFIG_5_SPLIT = 63,

/*!< Enum definition for six split in a full band */
   RFLTE_HW_PATH_CONFIG_6_SPLIT  = 127,

/*!< Enum definition for seven split in a full band */
   RFLTE_HW_PATH_CONFIG_7_SPLIT =  255

} rflte_hw_path_config_num_split_type;

/* Enum for the type of RB allocation */
typedef enum
{
  RFLTE_RB_ALLOC_DEFAULT,
  RFLTE_RB_ALLOC_CONTIG,
  RFLTE_RB_ALLOC_NON_CONTIG,
  RFLTE_RB_ALLOC_MAX = 0x7F
}rflte_rb_alloc_type;
...
{
  RFLTE_NV_BUFFER_CNT,
  RFLTE_NV_BUFFER_ALLOC,
  RFLTE_NV_BUFFER_COPY,
  RFLTE_NV_BUFFER_OPER_MAX
}rflte_nv_buffer_operation;
...
typedef enum 
{
  ...
  /*!> NV updated to NV TLB Pointer are 8 PA State NV with Intra-band ULCA PA SWPts*/
  RF_LTE_EIGHT_PA_STATE_NV_VER_2 = 2,                      /*!> Version 2 */
...
typedef struct
{
  ...
  /*! Split-band configuration for Tx. This field defines the total HW RF splits needed to be supported for Rx on the current band.
  If there are no splits, this value needs to be 1 suggesting 1 HW path for full band, max of 8 splits can be supported. */
  uint8 tx_split;

  /*! Split-band configuration for Rx. This field defines the total HW RF splits needed to be supported for Tx on the current band.
  If there are no splits, this value needs to be 1 suggesting 1 HW path for full band, max of 8 splits can be supported. */
  uint8 rx_split;
...
typedef struct 
{
  ...
  rfcom_txlin_temp_comp_nv_data_type lin_temp_comp_data;
    ...
    /* rise value for PA switch threshold from Linearizer gain state 0 to 1 */
  uint16  intra_band_ulca_pa_rise_0_1;

  /* rise value for PA switch threshold from Linearizer gain state 1 to 2 */
  uint16  intra_band_ulca_pa_rise_1_2;

  /* rise value for PA switch threshold from Linearizer gain state 2 to 3 */
  uint16  intra_band_ulca_pa_rise_2_3;

  /* rise value for PA switch threshold from Linearizer gain state 3 to 4 */
  uint16  intra_band_ulca_pa_rise_3_4;

  /* rise value for PA switch threshold from Linearizer gain state 4 to 5 */
  uint16  intra_band_ulca_pa_rise_4_5;

  /* rise value for PA switch threshold from Linearizer gain state 5 to 6 */
  uint16  intra_band_ulca_pa_rise_5_6;

  /* rise value for PA switch threshold from Linearizer gain state 6 to 7 */
  uint16  intra_band_ulca_pa_rise_6_7;

  
  /* fall value for PA switch threshold from Linearizer gain state 1 to 0 */
  uint16  intra_band_ulca_pa_fall_1_0;

  /* fall value for PA switch threshold from Linearizer gain state 2 to 1 */
  uint16  intra_band_ulca_pa_fall_2_1;

  /* fall value for PA switch threshold from Linearizer gain state 3 to 2 */
  uint16  intra_band_ulca_pa_fall_3_2;

  /* fall value for PA switch threshold from Linearizer gain state 4 to 3 */
  uint16  intra_band_ulca_pa_fall_4_3;

  /* fall value for PA switch threshold from Linearizer gain state 5 to 4 */
  uint16  intra_band_ulca_pa_fall_5_4;

  /* fall value for PA switch threshold from Linearizer gain state 6 to 5 */
  uint16  intra_band_ulca_pa_fall_6_5;

  /* fall value for PA switch threshold from Linearizer gain state 7 to 6 */
  uint16  intra_band_ulca_pa_fall_7_6;
...
/*!
  @brief 
  This module contains structure 
  definitions for the new NV structure for Tx Static Data:
  @{
*/
...
typedef PACK(struct)
{
  /* Combined # of elements */
  rfnv_tx_static_data_type tx_static_data[RF_LTE_MAX_NUM_PA_STATES][LTE_TX_FREQ_COMP_SIZ];
  uint32   num_of_tx_static_data_elements;

}rfnv_tx_static_data_combo_type;
...
typedef PACK (struct)
{
  rfnv_multi_cluster_bw_class_data_type  multi_cluster_bw_data;
  uint8 enable;
} rfnv_multi_cluster_bw_class_data_combo_type;

typedef PACK (struct)
{
  uint8 nv_container_index;
  uint8 ca_ns_number;
  rfnv_lte_ca_ns_ampr_table_type *ca_ns_ampr_table_data[RFNV_TX_STATIC_AMPR_TABLE_SIZE];
  uint8 num_of_ca_ns_ampr_elements;
} rfnv_ca_ns_ampr_backoff_static_data_type;

typedef PACK (struct)
{
  rfnv_ca_ns_ampr_backoff_static_data_type *ca_ns_ampr_static_data[RFLTE_MAX_NS_SIZE];
  uint8 num_of_ca_ns_elements;
  rflte_rb_alloc_type *rflte_ampr_rb_alloc_info[RFLTE_MAX_NS_SIZE];
} rfnv_ca_ns_ampr_backoff_static_data_combo_type;

typedef PACK (struct)
{
  uint8 nv_container_index;
  uint8 ca_ns_number;
  rfnv_lte_ca_ns_mpr_table_type *ca_ns_mpr_table_data[RFNV_TX_STATIC_MPR_TABLE_SIZE];
  uint8 num_of_ca_ns_mpr_elements;
} rfnv_ca_ns_mpr_backoff_static_data_type;

typedef PACK (struct)
{
  rfnv_ca_ns_mpr_backoff_static_data_type *ca_ns_mpr_static_data[RFLTE_MAX_NS_SIZE];
  uint8 num_of_ca_ns_elements;
  rflte_rb_alloc_type *rflte_mpr_rb_alloc_info[RFLTE_MAX_NS_SIZE];
} rfnv_ca_ns_mpr_backoff_static_data_combo_type;

typedef PACK(struct)
{
  rfnv_tx_lin_vs_freq_vs_temp_data_type  temp_comp_data;
  uint8 enable;
} rfnv_tx_lin_vs_freq_vs_temp_data_combo_type;
...
typedef PACK(struct)
{
  rfnv_pemax_adj_static_data_t rfnv_pemax_adj_static_data;
  boolean nv_cont_enabled;
}rfnv_pemax_adj_static_data_combo_t;

typedef PACK (struct)
{
  rfnv_tx_sar_power_limit_control_static_data_t *sar_tx_power_limit_dB10_static_data[RFNV_MAX_POWER_CLASS];
  uint8 num_of_ue_power_class_elements;
} rfnv_tx_sar_power_limit_control_static_combo_data_type;
...
/*!
  @brief
  This module contains structure 
  definitions for the Tx path specifc NV's:
  @{
*/
...
/*!
  @brief
  This structure is kept in NV Pointer Table per LTE Band
  and will keep all the the path specifc Tx related NV's
  which can be slected via NV container value in RFC
...
typedef struct 
{
  ...
  /* XPT path delay offset vs temp vs freq for given Tx Carrier */
  rfnv_tx_delay_offsets_type tx_static;
  /* DPD scaling data */
  ...
  rfnv_pa_swp_adj_val_type pa_swp_data;

  /* NV Related to number of PA States for each chain */
  rfnv_lte_pa_state_nv_type_s pa_state_nv;

  /* offset for QFE PA output cap */
  int8 pa_output_cap_offset; 

  /* NV Related to Tx static Data */
  rfnv_tx_static_data_combo_type   *tx_static_combo_data;
  ...
  /* NV Related to Pemax adjust static data*/
  rfnv_pemax_adj_static_data_combo_t *pemax_adj_static_combo_data;
  
  /*Array of max asdiv switch pos*/
  rfnv_tx_asdiv_gain_offset_info_type tx_asdiv_gain_offsets;

  /* NV Related to AMPR backoff */
  rfnv_ampr_backoff_static_data_combo_type *ampr_backoff_static_combo_data;

  rfnv_tx_sar_power_limit_control_static_combo_data_type *sar_limit_control_static_combo_data;

  rfnv_data_pa_bias_combo_offset_type *pa_bias_offset_data;

  rfnv_multi_cluster_bw_class_data_combo_type *multi_cluster_bw_combo_data;

  /* NV for AMPR backoff for CA_NS type */
  rfnv_ca_ns_ampr_backoff_static_data_combo_type *ca_ns_ampr_backoff_static_combo_data;

  /* NV for MPR backoff for CA_NS type */
  rfnv_ca_ns_mpr_backoff_static_data_combo_type *ca_ns_mpr_backoff_static_combo_data;

  rfnv_tx_lin_vs_freq_vs_temp_data_combo_type *txlin_vs_temp_vs_freq_data;

  tx_pa_state_cal_data_type *tx_pa_state_cal_data_ulca[RFCOM_TXLIN_NUM_LINEARIZERS];

  /* NV Related to Max Tx PWR Limit Adjust static Data */
...
/*!
  @brief
  This structure holds Band , Bandwidth Class
  ...
  uint8                 bw_class
  Bandwidth class associated with band
...
/*!
  @brief
  This structure holds LTE CA Bandwidth Class Combo
  
  @elements:
  rfnv_lte_bw_class_s   dl_ca_combo
  DL CA BW class combo
  
  uint8                 num_ul_ca_combos
  Number of UL CA BW class combos associated with dl_ca_combo
 
  rfnv_lte_bw_class_s*  ul_ca_combos
  UL CA BW class combos associated with dl_ca_combo

*/
typedef PACK (struct)
{
  rfnv_lte_bw_class_s   band_bw_class[NV_MAX_UNIQUE_BAND_COMBOS];
} rfnv_lte_bw_class_combo_s;

/*!
  @brief
  This structure holds LTE CA Bandwidth Class Combination Set.
...
typedef PACK (struct)
{
  rfnv_lte_bw_class_combo_s   dl_ca_combo;
  uint8                       num_ul_ca_combos;
  rfnv_lte_bw_class_combo_s*  ul_ca_combo_ptr;
} rfnv_lte_bw_class_combo_set_s;

/*----------------------------------------------------------------------------*/
/*!
  @brief
  This structure holds LTE CA Bandwidth Class Combinations.
  This structure will cumulatively store DL CA bandwidth class combos and UL CA bandwidth class combos

  @elements:
  uint8 num_bw_class_combo_sets  
  Indicates Number of supported Bandwidth class combination sets where
  each set contains the :
  {DL CA BW Class Combo,number of associated UL CA BW CLass combos and UL CA BW class combos}
  
  rfnv_lte_bw_class_set_s* bw_class_combo_sets
  Each set contains
  {DL CA BW Class Combo,number of associated UL CA BW CLass combos and UL CA BW class combos}
...
{
  uint8 num_bw_class_combo_sets;
  rfnv_lte_bw_class_combo_set_s* bw_class_combo_sets_ptr;
} rfnv_lte_ca_bw_class_combos_s;

/*----------------------------------------------------------------------------*/
/*!
  @brief
  This structure holds the SAR Tx power limits for ASDiv switch positions 0 and 1
...
{
  uint8 band;
  uint16 default_pos_limit_db10_values[RFLTE_SAR_BACK_OFF_SIZE];
  uint16 swap_pos_limit_db10_values[RFLTE_SAR_BACK_OFF_SIZE];
}rfnv_lte_ant_pos_based_sar_back_off_uncompressed_per_band_type;

typedef PACK (struct)
{
  uint8 enable;
  rfnv_lte_ant_pos_based_sar_back_off_uncompressed_per_band_type uncomp_per_band[RFLTE_NV_ANT_POS_BASED_SAR_BACK_OFF_LIST_MAX_SIZE];
}rfnv_lte_ant_pos_based_sar_back_off_uncompressed_type;
...
typedef struct
{
  ...
  uint16 default_pos_limit_db10_values[RFLTE_SAR_BACK_OFF_SIZE];
  uint16 swap_pos_limit_db10_values[RFLTE_SAR_BACK_OFF_SIZE];
}rflte_ant_pos_based_sar_back_off_type;
/*----------------------------------------------------------------------------*/
typedef struct
{
...
} rfnv_rx_cal_internal_data_type;
...
typedef struct
{
   ...
   /* NV Related to RX_GAIN_VS_FREQ : V2 with 32 bit channels */
    ...
    uint32   num_of_tx_asdiv_gain_offset_elements;
    ...
    /* SAR Back-off NV for swap ANT position */
    rflte_ant_pos_based_sar_back_off_type lte_db10_ant_pos_based_sar_back_off;
} rflte_nv_tbl_type;

/*----------------------------------------------------------------------------*/
/*!
  @brief 
   structure to hold 4*uint64 
 */
 #if (LTE_BAND_NUM == 256)
 typedef struct
 {
   ...
   uint64  bits_129_192;
   uint64  bits_193_256;
 } multi_cluster_bit_mask_256;
 #endif

/*----------------------------------------------------------------------------*/
 /*!
   @brief 
     structure to hold LTE MULTI CLUSTER band mask
 */
 typedef PACK (struct)
 {
   #if (LTE_BAND_NUM == 256)
   multi_cluster_bit_mask_256 multi_cluster_band_mask;
   multi_cluster_bit_mask_256 multi_cluster_intra_band_mask;	
   #else
   uint64 multi_cluster_band_mask;
   uint64 multi_cluster_intra_band_mask;
   #endif
 } rfnv_lte_multi_cluster_type;
 ...
 typedef struct
{
   ...
   /* LTE CA BW Class Combos*/
   rfnv_lte_ca_bw_class_combos_s* ca_bw_class_combos_ptr;

   /* LTE MC BW Class combos*/
   rfnv_lte_multi_cluster_type* multi_cluster_bw_class_ptr;

   /* LTE TX GNSS BLANKING */
  rfnv_lte_gnss_blanking_data_combos_s *gnss_blanking_data_combo;
...
void rflte_nv_convert_legacy_ca_support_to_bw_class();
uint8 rflte_nv_convert_bw_class(rflte_mc_bw_class_info_per_band_combo_s* band_combo_ptr);
...
boolean rflte_nv_get_txlin_cal_data
(
  uint8                     band,
...
boolean rflte_nv_get_hdet_vs_agc_nvid
(
  uint8                     band,
  uint32                    tx_nv_container,
  rfnv_item_id_enum_type   *item_code
...
(
  rflte_nv_tbl_type *nv_tbl_ptr, uint32 tx_nv_container
);
...
void
rflte_nv_populate_intra_band_ulca_pa_rise_fall_threshold_nv
(
  rflte_nv_tbl_type            *nv_tbl_type_ptr,
  void                         *rfnv_item_ptr,
  rfnv_lte_pa_state_nv_type_e   nv_pa_states_req,
...
int16
rflte_nv_get_num_active_pa_states_based_on_swpts
(
  rflte_nv_tbl_type* nv_tbl_type_ptr,
  uint32             tx_nv_container,
  boolean            intra_band_ulca_active,
  boolean            *pa_state_active
);


int8
rflte_nv_get_pa_output_cap
(
  rflte_nv_tbl_type       *nv_tbl_type_ptr,
  uint32                   tx_nv_container
);
...
void rflte_nv_populate_mc_band_mask (void);
...
boolean rflte_nv_populate_ca_bw_class_combo_nv( rfnv_item_id_enum_type  item_code,   
                                                rfnv_item_type          *rfnv_item_ptr,
                                                rfnv_item_size_type      nv_item_size );

void *rflte_nv_lte_common_nv_table_get( void );

void *rflte_nv_get_mc_band_mask( void );
...
boolean rflte_nv_process_old_legacy_static_data
(
   rflte_nv_tbl_type *nv_tbl_type_ptr,
   rfnv_item_type          *rfnv_item_ptr,
   uint8 nv_container_idx,
   rfnv_lte_rx_switchpoint_enum_type switch_point_type
);
...
boolean
rflte_nv_populate_rx_cal_chan_data
(
  rflte_nv_tbl_type           *nv_tbl_ptr,
  rfnv_item_type              *rfnv_item_ptr
...
rfcommon_nv_status_type rflte_nv_lte_ant_pos_based_sar_retrieve_nv_items( rex_tcb_type *task_ptr, 
                                                          	          rex_sigs_type task_wait_sig,
                                                          	          void (*task_wait_handler)( rex_sigs_type ) );

boolean rflte_nv_lte_ant_pos_based_sar_copy_rfnv_item( rfnv_item_id_enum_type  item_code,   
                              			       rfnv_item_type          *rfnv_item_ptr,
                               			       rfnv_item_size_type      nv_item_size );
...
boolean rflte_nv_tx_sar_power_limit_control_static_extract(void *temp_data_ptr,
                                                              int32 temp_num_bytes_left,
                                                              uint32 nv_element_size,
                                                              rflte_nv_tbl_type *nv_tbl_type_ptr);


boolean rflte_nv_multi_cluster_bw_class_extract(void *temp_data_ptr,
                                                     uint32 nv_element_size,
                                                     rflte_nv_tbl_type *nv_tbl_type_ptr);

boolean rflte_nv_tx_lin_vs_temp_freq_extract(void *temp_data_ptr,
                                             uint32 nv_element_size,
                                             rflte_nv_tbl_type *nv_tbl_type_ptr);


boolean rflte_nv_query_spur_data_from_nv_container(uint8 nv_container, 
                                                   rfcom_lte_band_type band, 
                                                   rfnv_rx_spur_list_type** spur_list_ptr);


/*----------------------------------------------------------------------------*/
boolean rflte_nv_ca_ns_ampr_backoff_static_extract(void *temp_data_ptr,
                                                   uint32 nv_element_size,
                                                   rflte_nv_tbl_type *nv_tbl_type_ptr);

/*----------------------------------------------------------------------------*/
boolean rflte_nv_ca_ns_mpr_backoff_static_extract(void *temp_data_ptr,
                                                  uint32 nv_element_size,
                                                  rflte_nv_tbl_type *nv_tbl_type_ptr);
...
boolean rflte_nv_pemax_adj_static_data_copy(void  *temp_data_ptr,
                                            uint32 nv_element_size,
                                            rflte_nv_tbl_type *nv_tbl_type_ptr);
boolean rflte_nv_tx_static_data(rfnv_item_id_enum_type  item_code,   
                                rfnv_item_type *rfnv_item_ptr,
                                rfnv_item_size_type nv_item_size);

boolean rflte_nv_extract_variant_marker_nv_data_type(rfnv_item_type* nv_data_ptr,
                                                     uint32 *num_bytes_parsed,
                                                     rfnv_item_size_type nv_item_size,
                                                     uint16 *num_variant_elements);

boolean rflte_nv_iterate_alloc_copy_tx_static_nv_data(rfnv_item_type* nv_data_ptr,
                                                      const uint16 num_elements,
                                                      uint32 bytes_parsed,
                                                      rfnv_item_size_type nv_item_size,
                                                      rflte_nv_buffer_operation oper,
                                                      rflte_common_nv_tbl_type* common_nv_tbl_ptr);
/*----------------------------------------------------------------------------*/                         
/*---------------------------------------------------------------*/
