...
/*==============================================================================
  ...
  $Header: //components/rel/rftech_lte.mpss/3.11/rf/core/inc/rflte_core_hdet.h#1 $
...
06/23/15   skk     Port: Fix for TxAGC Value convertion
...
#define RFLTE_CORE_TXPL_ABS_FBRX_GAIN_ERR_MAX 80
...
/*----------------------------------------------------------------------------*/
uint16* rflte_core_txpl_get_hdet_vs_agc(uint8 sub_id,
                                        uint32 carrier_idx, 
                                        int16 txagc_val);
/*----------------------------------------------------------------------------*/
int16 rflte_core_txpl_convert_hdet_adc_to_txagc(uint8 sub_id,
                                                uint32 carrier_idx, 
                                                uint16 hdet_adc, 
                                                int16 txagc_val, 
...
void rflte_core_txpl_update_mode(uint8 sub_id,
                                 uint32 carrier_idx, 
                                 int16 power_delta);
/*----------------------------------------------------------------------------*/
float rflte_core_txpl_filter_hdet_error(uint8 sub_id,
                                        uint32 carrier_idx, 
                                        int16 hdet_error);
/*----------------------------------------------------------------------------*/
...
void rflte_core_txpl_calc_hdet_err(uint8 sub_id,
                                   uint32 carrier_idx,
...
boolean rflte_core_txpl_calc_corrected_hdet(uint8 sub_id,
                                            uint32 carrier_idx,
                                            lm_handle_type txlm_buffer_idx,
                                            uint8 *phy_rb_block,
                                            uint8 *phy_start_rb,
                                            uint16 *comp_hdet_adc);
