#******************************************************************************
#* $Header: //components/rel/mcfg.mpss/8.1.1/mcfg_gen/scripts/mcfg_generate.pl#1 $
...
sub ReadNextFlag($) {
   ...
   if($flag eq "-xml" or $flag eq "-i" or $flag eq "-o" or $flag eq "-b" or $flag eq "-mbn" or $flag eq "-crm" or $flag eq "--au_aliases") {
      return $flag;
   }
   else {
...
sub ParseArguments {
   ...
   return ($input_file, $output_prefix, $build_root, $xml, $mbn, $crm_buildid, $au_aliases);
}
...
sub Execute {
   ...
   @args or return "Usage is: perl mcfg_generate.pl -i <INPUT FILE> [-o <OUTPUT PREFIX>] [-b <BUILD ROOT>] [-xml] [-mbn] [-crm <crm id>] [--au_aliases <au_aliases>]";

   my ($input_file_name, $output_file_prefix, $build_root, $xmlFormat, $mbn, $crm_buildid, $au_aliases) = @args;
   ...
   if($xmlFormat) {
     ...
     else
     {
        ...
        MCFGWrite::GenerateMcfgSegmentFileXML($xml, $input_file_name, $output_file_prefix, $build_root, $MCFGWrite::GENERIC_C_OUTPUT_FILE_TYPE, 4, undef, $crm_buildid, undef);
