#******************************************************************************
#* $Header: //components/rel/mcfg.mpss/8.1.1/mcfg_gen/scripts/mcfg_def_generate.pl#1 $
...
sub ReadNextFlag($) {
   ...
   if($flag eq "-xml" or $flag eq "-i" or $flag eq "-o" or $flag eq "-b" or $flag eq "-crm"or $flag eq "--au_aliases") {
      return $flag;
   }
   else {
...
sub ParseArguments {
   ...
   return ($input_file, $output_file, $build_root, $xml, $crm_buildid, $au_aliases);
}
...
sub Execute {
   ...
   @args or return "Usage is: perl mcfg_def_generate.pl -i <INPUT FILE(xlsx)> [-o <OUTPUT FILE>] [-b <BUILD ROOT>] [-xml] [-crm <crm_id>] [--au_aliases <au_aliases>]";

   my ($input_file_name, $output_file_name, $build_root, $xmlFormat, $crm_buildid, $au_aliases) = @args;
   ...
   if($xmlFormat) {
      ...
      my ($length_bytes, $num_items) = MCFGWrite::GenerateMcfgSegmentFileXML($xml, $input_file_name, $output_file_name, $build_root, $MCFGWrite::DEFAULT_C_OUTPUT_FILE_TYPE,0,undef,$crm_buildid, undef);
