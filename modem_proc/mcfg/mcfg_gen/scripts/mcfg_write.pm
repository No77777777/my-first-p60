#******************************************************************************
#* $Header: //components/rel/mcfg.mpss/8.1.1/mcfg_gen/scripts/mcfg_write.pm#1 $
...
# Global variables
my $MCFG_VERSION		= '0[xX][0-9A-Fa-f]{8}';
...
my $ATTRIBUTES_LENGTH  = $BYTE + $BYTE;  #Bytes will be stored in little endian format in MBNs.
...
#  LARGE_EFS_HEADER_LENGTH  = 19 Bytes
my $LARGE_EFS_HEADER_LENGTH = $EFS_HEADER_LENGTH+ $INT_16;
...
my $EFS_PAYLOAD_LIMIT = 64*1024; # Max limit(64K) of uint16 variable
my $LARGE_EFS_PAYLOAD_LIMIT = 1*1024*1024; # Max limit(1MB) - compressed size of a worstcase 4MB RFC MBN.
...
sub IsLargeEfs {
   ...
   return ($variant_type == $variantValues->{large_efs} || $variant_type == $variantValues->{slot_large_efs});
}
...
}
{
  ...
  {
    my $alias_str = (shift);
    %au_aliases = split (/[:;]/, $alias_str); 
    #print "au_aliases set to " . Dumper(%au_aliases) . "\n";
  }
  ...
  sub lookup_au_alias($)
  {
    ...
    my %au_aliases = get_au_aliases();
    if (exists($au_aliases{$au}))
    {
      $alias = $au_aliases{$au};
    }
...
# Returns 1 if aliased path exists in fs, 0 otherwise
sub build_alias_present($)
{
  my $au_alias = (shift);
  my $alias_dir = File::Spec->catdir($BUILD_ROOT . 'modem_proc', $au_alias);
...
# Function will update AU folder witin an efs path
# only if an aliased name was given and the directory exists
sub update_path_for_aliased_au($)
{
  my $rel_efs_path = (shift);

  my ($tech_au) = $rel_efs_path =~ /modem_proc[\\|\/]([a-zA-Z0-9]+)/;
  my $tech_alias = lookup_au_alias($tech_au);
  if ($tech_alias ne '' && build_alias_present($tech_alias))
  {
    $rel_efs_path =~ s#(modem_proc[\\|\/])$tech_au#\1$tech_alias#;
    #print "dbg: updated efs path: $rel_efs_path\n";
  }
...
sub parse_nv_prl {
   ...
   printf " ,0x%02X" , ($prl_item_attrib>>8) & 0xFF;
   printf " ,0x%02X" , 0x00;
...
# Scan an efs file and return its size and contents
sub get_efs_contents
{
  my($efs_filename, $item_variant) = @_;
  my $efs_file_buf = '';
  my $efs_read_size = 0;

  $efs_filename = File::Spec->canonpath($efs_filename);

  # MCFG MBN case
  if ($efs_filename =~ /\.$MCFG_POSTFIX$/)
  {
    $efs_file_buf = CompressMBNdata($efs_filename);
    $efs_read_size = length($efs_file_buf);
  }
  else
  {
    open(my $efs_file, "<", $efs_filename)
           or die "Could not open or create temporary file '$efs_filename' $!";

    # Assume the EFS file is a binary file.
    binmode($efs_file);

    # Read the entire EFS file and save the size of the file.
    $efs_read_size = read($efs_file, $efs_file_buf, (stat $efs_filename)[7]);
    
    # Close the EFS file.
    close($efs_file);
  }
  
  if ( (IsLargeEfs($item_variant) && ($efs_read_size >= $LARGE_EFS_PAYLOAD_LIMIT)) ||
       (!IsLargeEfs($item_variant) && ($efs_read_size >= $EFS_PAYLOAD_LIMIT)) )
  {
    die "EFS file too large ($efs_read_size). File: '$efs_filename'\n";
  }
  return ($efs_read_size, $efs_file_buf);
}
...
sub parse_efs_file_bin
{
   ...
   {
     ($efs_read_size, $efs_file_buf) = get_efs_contents($efs_filename, $item_variant);
   }
   ...
   if (IsLargeEfs($item_variant))
   {
     binwrite(pack('I', $efs_payload_length), 4);
   }
   else
   {
...
sub parse_efs_file
{
   ...
   printf "\n /* ".(IsLargeEfs($item_variant)?"Large EFS":"EFS")." file: ";
   ...
   printf " ,0x%02X ,0x%02X ,0x%02X ,0x%02X", $item_variant, ($attrib & 0xFF), (($attrib>>8) & 0xFF), 0x00;
   $byte_count+=8;
   ...
   if (IsLargeEfs($item_variant))
   {
     ...
     printf " ,0x%02X" ,(($efs_payload_length>>16) & 0xFF);
     printf " ,0x%02X" ,(($efs_payload_length>>24) & 0xFF);
     $byte_count += 4;
   }
   else
   {
...
sub create_efs_file
{
   ...
   if(IsLargeEfs($item_variant))
   {
     $mcfg_item_length = $LARGE_EFS_HEADER_LENGTH + $efs_path_length + $efs_type; 
   }
   else
   {
   ...
   if(IsLargeEfs($item_variant))
   {
     ...
     printf " ,0x%02X" ,(($efs_type>>16) & 0xFF);
     printf " ,0x%02X" ,(($efs_type>>24) & 0xFF);
   }
   else
   {
     printf " ,0x%02X" ,($efs_type & 0xFF);
...
sub WriteEFSHeader($$$$$) {
   my ($payload_length, $item_length, $attrib, $filepath, $item_variant) = @_;
   my $filename_length = length($filepath) + $NULL_CHAR_LENGTH; 
...
sub WriteGenericMCFGHeaderBin($$$)
{
  ...
  binwrite( pack('C*', $item_variant, ($attributes & 0xFF), (($attributes>>8) & 0xFF), 0x00));
...
sub CalculateEfsElementLength($$$$) {
   ...
   if($value) {
     ...
     open(UNDP_EFS_FH, "<", "$efs_filename")
         or die "Could not open or create temporary file '$efs_filename' $!";
     binmode(UNDP_EFS_FH);
     $efs_read_size = read(UNDP_EFS_FH, $efs_file_buf, (stat $efs_filename)[7]);
     close(UNDP_EFS_FH);
...
sub WriteMcfgMultiMBNHeader($$$$$$) {
   my ($mcfg_output_file_name, $num_mcfg_items, $length_bytes, $carrier_index, $configuration_type, $max_multimbn_size) = @_;
   
   if(!defined($max_multimbn_size))
   {
...
sub CalculateItemPayloadLength {
   ...
   foreach my $element (@$elements) {
     ...
     elsif($type =~ /^efs$|^large_efs$|^slot_large_efs$/) {
...
sub WriteMcfgItemComment($$$$) {
   ...
   if($item_type =~ /^efs$|^large_efs$|^slot_large_efs$/) {
   }
...
sub GetItemIdentifier($) {
   my ($mcfg_item) = @_;
     ...
     elsif ($mcfg_item->{ItemType} =~ /^efs$|^efs_item$|^large_efs$|^slot_large_efs$/) {
...
sub ExtractEFSFile {
   ...
   if($item->{Variant} == $variantValues->{large_efs})
   {
     $item->{ItemType} = "large_efs"
   }
   elsif($item->{Variant} == $variantValues->{slot_large_efs})
   {
     $item->{ItemType} = "slot_large_efs"
   }
   ...
   my $element = newEFSFile( $item->{ItemType}, $efs_filepath, $efs_filename );
   push( @{$item->{Elements}}, $element );
   $item->{Payload}  = CalculateItemPayloadLength($item->{Elements}, $item->{Attributes});
   if (IsLargeEfs($item->{Variant}))
   {
     $item->{DataSize} = $LARGE_EFS_HEADER_LENGTH + length($efs_filepath) + $item->{Payload};
   }
   else
   {
...
sub ExtractEFSItem($$$) {
   ...
   elsif (IsLargeEfs($item->{Variant}))
   {
      $item->{DataSize} = $LARGE_EFS_HEADER_LENGTH + length($item->{Filepath}) + $item->{Payload};
   }
   else 
   {
...
sub PreProcessItemXML {
   ...
   elsif($itemTag =~ /NvEfsFile|NvLargeEfsFile|NvSlotLargeEfsFile|NvEfsDir/)
   {
     ...
     if (($itemTag =~ /NvLargeEfsFile/ && ($itemAttributeData->{mcfgVariant} != 0x08)) ||
	    ($itemTag =~ /NvSlotLargeEfsFile/ && ($itemAttributeData->{mcfgVariant} != 0x09))||
	    ($itemTag =~ /NvEfsFile/ && ($itemAttributeData->{mcfgVariant} == 0x08)))
     {
       my $MSG = sprintf  "\n== >> Error: Given tag '%s' and variant type '%s' are not matching for item '%s'!", $itemTag, $itemAttributeData->{mcfgVariant}, $id;
       die ($MSG);
     }
   }
   ...
   if(defined($output_type) && ($output_type == $MULTI_MBN_OUTPUT_FILE_TYPE))
   {
     ...
     {
	   my $buildpath = $itemAttributeData->{buildPath};
	   $buildpath =~ s#\\+#/#g;  # convert given path from windows to unix style for further processing
	   my ($vol, $dir, $filename) = File::Spec->splitpath($buildpath);
	   if ($filename =~ /^$MCFG_PREFIX_SW/)
	   {
         $config_type = $MCFG_PREFIX_SW;
	   }
	   elsif ($filename =~ /^$MCFG_PREFIX_HW/)
	   {
         $config_type = $MCFG_PREFIX_HW;
	   }
	   else {
	     die ("Error: Can't determine the MBN type of a MultiMBN entry '%s'", $itemAttributeData->{buildPath})
	   }
	   $original_str = "modem_proc/mcfg/configs";
	   $target_str = "modem_proc/mcfg/bsp/".$config_type."_img/build/".$config_type."/".$crm_buildid."/configs";
	   $target_str =~ s#\\+#/#g;
       $buildpath =~ s#$original_str#$target_str#g;  # if windows specific separator is part of the string then this replace would fail
       $itemAttributeData->{buildPath} = File::Spec->canonpath($buildpath);  # store platform specific path
     }
   }
...
sub ExtractNextMcfgItemXML($$$) {
   ...
   my $handlers = {
      ...
      "NvLargeEfsFile"        => \&ExtractEFSFileXML,
      "NvSlotLargeEfsFile"    => \&ExtractEFSFileXML,
...
sub ExtractNVItemXML($) {
   ...
   if ($item->{ID} > 7232)
   {
     die ("Err: Invalid NV# for NV listing '$item->{ID}'. NVs beyond 7232 should be listed as efs or efs_item types\n");
   }
   elsif ($item->{ID} < 0 )
   {
     die ("Err: Invalid NV# '$item->{ID}' for NV listing. NV values are always positive\n");
   }
   ...
   {
     die ("Err: Invalid XML information listed in '$item->{ID}'");
   }
...
sub ExtractPrlItemXML($) {
   ...
   $prl_filename = update_path_for_aliased_au($prl_filename);
...
sub ExtractEFSFileXML($) {
   ...
   $item->{Filename} = update_path_for_aliased_au($item->{Filename});
   ...
   if (scalar(@{ $itemParsedXML }) > 0)
   {
     ...
     if ($members[0] eq 'INVALID_MCFG_DATA')
     {
       die ("Err: Invalid XML information listed in '$item->{Filepath}'");
     }
   ...
   my $element = newEFSFile( $item->{ItemType}, $item->{Filepath}, $item->{Filename});
...
sub ExtractGroupFileXML($) {
   ...
   $efs_fn_abspath = update_path_for_aliased_au($efs_fn_abspath);

   Log($FILEPATHS_TRACE_ENABLE, "Extracting Group: '$efs_fn_abspath'");
   # read xml group file
   open my $fh, '<', $efs_fn_abspath or die "Can't open '$efs_filename' from path '$efs_fn_abspath'!";
   my $xml_group_content = do { local $/; <$fh> };
...
sub ExtractEFSDirTreeXML($) {
   ...
   $src_dir_root = update_path_for_aliased_au($src_dir_root);
...
sub ExtractEFSItemXML($) {
   ...
   if(IsLargeEfs($item->{Variant}))
   {
     $item->{DataSize} = $LARGE_EFS_HEADER_LENGTH + length($item->{Filepath}) + $sizeInBytes;
   }
   else
   {
...
sub ExtractNextMcfgItemFull($) {
   ...
   my $handlers = {
     "prl"              => \&ExtractPrlItem,
     "efs"              => \&ExtractEFSFile,
     "large_efs"        => \&ExtractEFSFile,
     "slot_large_efs"   => \&ExtractEFSFile,
...
sub GenerateMCFGTargetFrmSourceXML
{
  ...
  my ($mcfg_seg_sz, $num_mcfg_items) = GenerateMcfgSegmentFileXML($xml, $input_file_name, $target_seg_mcfg, $build_root, $BINARY_OUTPUT_FILE_TYPE, 0, undef, $crm_buildid, undef);
...
sub AlignMCFGItems
{
  ...
  if ($boundary_sz =~ /^\d+$/ && $boundary_sz > 0)
  {
    foreach my $mcfg_item (@$mcfg_items) {
      ...
      if ($mcfg_item->{'ItemType'} =~ /^efs$|^large_efs$/)
      {
        if(IsLargeEfs($mcfg_item->{'Variant'}))
        {		
          $temp_len = $LARGE_EFS_HEADER_LENGTH + length( $mcfg_item->{'Elements'}->[0]->{'Filepath'});
        }
		else
        {		
          $temp_len = $EFS_HEADER_LENGTH + length( $mcfg_item->{'Elements'}->[0]->{'Filepath'});
        }
        $padding = GetPadding( $boundary_sz, $temp_len );
...
sub save_digest_to_file {
   my ($digest, $mcfg_output_file_name) = @_;
   my $hash_file, $hash_dir;

   ($hash_file, $hash_dir) = fileparse($mcfg_output_file_name);
   $hash_dir =~ s#configs#configs/debug/hash_comparison/trl_hash_from_mbngen#;
   $hash_dir = File::Spec->canonpath($hash_dir);
   # create dir
   if (! -d $hash_dir)
   {
     mkpath($hash_dir) or die "Error creating dir '$hash_dir': $!";
   }
   # open dir / open file
   $hash_file = File::Spec->catfile( "$hash_dir", "trl_digest");
   open (my $OUT_FILE, '>', $hash_file) or die "Unable to open '$hash_file': $!\n";
   # write hash to file
   binmode($OUT_FILE);
   binwrite($digest, 32, $OUT_FILE);
   # close file
   close($OUT_FILE);
}
...
sub GenerateMcfgSegmentFileXML {
   my ($xml, $mcfg_input_file_name, $mcfg_output_file_name, $build_root, $output_type, $is_byte_aligned, $boundary_sz, $crm_buildid, $max_multimbn_size) = @_;
          ...
          foreach my $item (@items_found) {
            if( IsItemUnique($item) ) {
              $length_bytes += $item->{DataSize};
               ...
               if ($item->{ItemType} =~ /^efs$|^large_efs$|^slot_large_efs$/ && defined($item->{Filepath})) {
   ...
   #  print "size used in mcfg header: length bytes = $length_bytes\n";
      ...
      # write digest to separate file for debugging purposes
      # save_digest_to_file($digest,$mcfg_output_file_name);
   ...
   elsif ($output_type == $MULTI_MBN_OUTPUT_FILE_TYPE) 
   {
      ...
      WriteMcfgMultiMBNHeader($mcfg_output_file_name, $num_mcfg_items, $length_bytes, $mcfg_sheet_data->{"Carrier Index"}, int($mcfg_sheet_data->{"Configuration Type"}), $max_multimbn_size);
      WriteMcfgVersion($mcfg_output_file_name, hex($mcfg_sheet_data->{"Full MCFG Version"}));
