#!/usr/bin/perl
#******************************************************************************
#* $Header: //components/rel/mcfg.mpss/8.1.1/mcfg_gen/scripts/read_apex_xml_dump_nv_xml.pl#1 $
...
my $nvKey = '0'; GetOptions( "item=s" => \$nvKey, "op=s" => \$op_fmt, "version=s" => \$version ); { 
  ...
  if ($nvKey !~ /^\d+$/)
  {
    my @variants = keys %{ $apexTables->{'tblNvItems'} };
    
    #nvKey pass is a path to an item
    foreach my $nvItemKey (@variants)
    {
