#******************************************************************************
#* $Header: //components/rel/mcfg.mpss/8.1.1/mcfg_gen/scripts/mcfg_multi_generate.pl#1 $
...
sub ReadNextFlag($) {
   ...
   if($flag eq "-xml" or $flag eq "-i" or $flag eq "-o" or $flag eq "-b" or $flag eq "-a" or $flag eq "-crm" or $flag eq "-multimbn_limit"or $flag eq "--au_aliases") {
      return $flag;
   }
   else {
      if (not($flag eq '' | $flag =~ /^\s+$/))
      {
...
sub ParseArguments {
   ...
   my $boundary_sz = 4;
   my $crm_buildid = "";
   my $multimbn_limit = 524288; # Default to 512K
   my $au_aliases = "";
   ...
   while($next_flag) {
      ...
      elsif($next_flag eq "-multimbn_limit") {
         $multimbn_limit = $next_argument;
      }
	  elsif($next_flag eq "--au_aliases") {
         $au_aliases = $next_argument;
      }
      else {
         return (); #invalid flag
      }
   ...
   return ($input_file, $output_file, $build_root, $xml, $is_byte_aligned, $boundary_sz, $crm_buildid, $multimbn_limit, $au_aliases);
}
...
sub Execute {
   ...
   @args or return "Usage is: perl mcfg_multi_generate.pl -i <INPUT FILE(xlsx/xml)> [-o <OUTPUT FILE>] [-b <BUILD ROOT>] [-xml] [-a] [-crm <crm_id>] [-multimbn_limit <max_limit>] [--au_aliases <au_aliases>]";

   my ($input_file_name, $output_file_name, $build_root, $xmlFormat, $is_byte_aligned, $boundary_sz, $crm_buildid, $multimbn_limit, $au_aliases) = @args;

   print "\nUsing build root: \"$build_root\"\n\n" if ($build_root);

   $input_file_name = File::Spec->rel2abs($input_file_name);
   MCFGWrite::set_au_aliases($au_aliases) if (length($au_aliases)>0);
   ...
   if($xmlFormat) {
      ...
      my ($length_bytes, $num_items) = MCFGWrite::GenerateMcfgSegmentFileXML($xml, $input_file_name, $output_file_name, $build_root, 
                                                                             $MCFGWrite::MULTI_MBN_OUTPUT_FILE_TYPE, $is_byte_aligned, $boundary_sz, $crm_buildid, $multimbn_limit);
