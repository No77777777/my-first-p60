...
/*==============================================================================
  ...
  $Header: //components/rel/rfdevice_interface.mpss/2.11/api/rfdevice_cmn_type_defs.h#1 $

when       who     what, where, why
--------   ---     -------------------------------------------------------------
05/25/20   up      Store RX and tx path info sub_id specific
12/27/19   up      Added device snapshot feature support
...
08/26/15   hs      Added macros for max number of devices per signal path.
08/24/15   aks     Added structs: rfdevice_trx_notch_info_type, rfdevice_trx_config_chain_params_type
08/17/15   shb     rfdevice_trx interface structure updates for RFC hookup
08/07/15   sma     Populate Tx LUT when requested cal BW is invalid
07/24/15   ry      Added phase to rfdevice_tx_lut_data_type
07/14/15   vv      Added WTR enum definitions for WTR IQ paths and Tx div cfg ratios
06/02/15   zg      Removed temp definition of missing WTR ports
05/22/15   jfc     Temp define of missing WTR ports  
...
/*! Maximum of 2 VCOs connected per WTR ILNA port. 
    which translates to a maximum of 2 RFM_DEVICES per ILNA port*/
#define RFDEVICE_MAX_TRX_DEVICE_PER_SIG_PATH 2

#define RFDEVICE_MAX_TX_DEVICE_PER_SIG_PATH 1

/*! Maximum number of VCO path index*/
#define RFDEVICE_TRX_PATH_IDX_MAX         10
/*----------------------------------------------------------------------------*/

/*! @brief Size of the device params buffer in the rfdevice_trx_config_chain_params_type 
  struct */
#define RFDEVICE_TRX_DEVICE_PARAMS_BUF_SIZE 32

/*! @brief Indicates Total registers for reg dump */
#define RFDEVICE_TRX_3K_MAX_REG_ADDR_SIZE     9216  //needed for snapshot _shirishm_

/*! @brief Indicates Total registers for reg dump */
#define RFDEVICE_TRX_2K_MAX_REG_ADDR_SIZE     3794  //needed for snapshot _shirishm_

/*----------------------------------------------------------------------------*/
/*! 
  @brief 
  Structure containing parameters required by the tech driver 
  to configure notches
*/
typedef struct
{
  uint32 freq_khz;
  uint16 depth;
  uint16 threshold;
}rfdevice_trx_notch_info_type;

/*----------------------------------------------------------------------------*/
/*! 
  @brief 
  Structure containing parameters required by tech and device driver to
  configure Rx/Tx chain 
*/
typedef struct 
{
  /*! @brief Number of notches to be programmed */
  uint16 notch_cnt;

  /*! @brief Number of notches to be programmed */
  rfdevice_trx_notch_info_type notch_array[RFDEVICE_MAX_NOTCHES];

  /*! @brief Final LO Frequency after LIF adjustment */
  uint32 final_lo_freq_khz;

  uint32 device_params[RFDEVICE_TRX_DEVICE_PARAMS_BUF_SIZE];

}rfdevice_trx_config_chain_params_type;
...
typedef enum
{
  ...
  RFDEVICE_TRX_IQ_PATH_DRX,
  RFDEVICE_TRX_IQ_PATH_TX,
  RFDEVICE_TRX_IQ_PATH_MAX,
  RFDEVICE_TRX_IQ_PATH_INVALID =0xFFFFFFFF
...
typedef enum
{
  RFDEVICE_TRX_SUB_1,
  RFDEVICE_TRX_SUB_2,
  RFDEVICE_TRX_SUB_MAX,
  RFDEVICE_TRX_SUB_INVALID = RFDEVICE_TRX_SUB_MAX
} rfdevice_trx_sub_type; 
...
typedef enum
{
  RFDEVICE_TRX_STD_CDMA,  /* CDMA */
  RFDEVICE_TRX_STD_GSM,  /* GSM */
  RFDEVICE_TRX_STD_LTEFDD,  /* LTEFDD */
  RFDEVICE_TRX_STD_LTETDD,  /* LTETDD */
  RFDEVICE_TRX_STD_TDSCDMA,  /* TDSCDMA */
  RFDEVICE_TRX_STD_WCDMA,  /* WCDMA */
  RFDEVICE_TRX_STD_NUM,  /* Num */
  RFDEVICE_TRX_STD_INVALID,  /* Invalid */
} rfdevice_trx_tech_type;

/*----------------------------------------------------------------------------*/
/*!
  @brief
  Enum type to support WTR TX Divide ratios 
...
typedef enum
{
  RFDEVICE_TRX_DIV_CFG0,
  RFDEVICE_TRX_DIV_CFG1,
  RFDEVICE_TRX_DIV_CFG2,
  RFDEVICE_TRX_DIV_CFG_MAX,
  RFDEVICE_TRX_DIV_CFG_INVALID =0xFFFFFFFF
}rfdevice_trx_tx_div_cfg_type;
...
} rfdevice_reg_addr_data_info_type; //needed for snapshot _shirishm_
...
typedef enum
{
  RFDEVICE_XSW_TRIGGER_NONE,
  RFDEVICE_XSW_TRIGGER_WAKEUP,
  RFDEVICE_XSW_TRIGGER_SLEEP,
  RFDEVICE_XSW_TRIGGER_ALL,
} rfdevice_xsw_trigger_enum_type;
...
/*!
  @brief
  Structure to capture tech and path specific transceiver data
 
  @details
  Includes device interface object and calibration information that RFC needs
*/
typedef struct
{
  /*! Device interface object that must be used to access this path after 
    typecasting with appropriate interface class */
  rfdevice_class* dev_obj;

  /*! Pointer to VCO info for the path */
  const rfdevice_trx_vco_info_type* vco_info;

  /*! TRX VCO calibration flag*/
  boolean trx_vco_cal_flag;

} rfdevice_trx_path_data_type;

/*----------------------------------------------------------------------------*/
/*!
  @brief
  Structure to capture the VCO info per tech and band from WTR
 
  @details
  Captures the below info from WTR per tech and band
  - VCO tech-band indepednent info (VCO ID , VCO Type(PRX/DRX/TX))
  - TX Div ratio
 */
typedef struct{

  /*!TRX VCO tech/band independent info*/
  rfdevice_trx_vco_info_type trx_vco_cmn_info;

  /*!TX Div ratio configs*/
  rfdevice_trx_tx_div_cfg_type tx_div_cfg[RFDEVICE_TRX_DIV_CFG_MAX];
  
  /*! TRX VCO calibration flag*/
  boolean trx_vco_cal_flag;

}rfdevice_trx_vco_cal_info_type;
