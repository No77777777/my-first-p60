...
$Header: //components/rel/rfdevice_interface.mpss/2.11/api/rfdevice_papm.h#1 $

when       who    what, where, why
--------   ---   ------------------------------------------------------------------- 
11/07/19   dh    Added interface to include standby delay for QET5102
06/29/17   dh    Updated papm mode enum with RFDEVICE_LDO_MODE
...
#define RFDEVICE_PAPM_INVALID_APT_VOLTAGE 0 
...
typedef struct
{
  ...
  uint16 APT_Vmin; /* APT voltage minimum*/
  uint16 APT_Vmax; /* APT voltage maximum*/ 
...
public:
static boolean RFFE_REG_DUMP_DEBUG_PAPM;
rfdevice_papm();
...
*/
  ...
  /*!
    @brief
    Check if the QET device require standby delay.

    @return
    Return TRUE if standby delay is required
  */  
  virtual boolean is_standby_delay_req();
};
