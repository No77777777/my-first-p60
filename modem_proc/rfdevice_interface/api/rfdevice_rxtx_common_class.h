...
$Header: //components/rel/rfdevice_interface.mpss/2.11/api/rfdevice_rxtx_common_class.h#1 $

when       who     what, where, why
--------------------------------------------------------------------------------
07/21/20   up      Added api to clear sub ID specific state info.
...
07/24/15   zg      Added new api of power_lpm
08/10/15   hm      fix: Incorrect call to destructor
08/03/15   sma     Added ULCA 20+20 Tx LUT power offset 
05/27/15   sd      Added API to determine if Rx Tx can be on separate RATs simulataneously
...
class rfdevice_rxtx_common_class:public rfdevice_class
{
  ...
  virtual uint32 get_device_asic_id() const;
  ...
  /*! Returns fbrx gain state script to set a desired gain state */
  virtual boolean selftest_get_rsb_coeff(   rfm_mode_enum_type tech,
                                            rfcom_band_type_u band,
                                            uint8 fbrx_gain_state,
                                            rfdevice_fbrx_rsb_data_type* rsb_coeff)

  {
    RF_MSG( RF_ERROR, "selftest_get_rsb_coeff not implemented in derived "
                      "common class!");
    return FALSE;
  }
  ...
  virtual boolean operator==(const rfdevice_rxtx_common_class & cmn_rxtx_device) const
  {
  ...
  /*! Get support for rx/tx on separate RATs */
  virtual boolean get_rxtx_simultaneous_rat_support(boolean* is_rxtx_separate_rat)
  {
    RF_MSG( RF_HIGH, "get_rxtx_simultaneous_rat_support not implemented in"
                     " derived common class!");
    return FALSE;
  }
  ...
  virtual boolean clear_sub_id_info
  (
    rfdevice_trx_sub_type sub_id
  )
  {
    RF_MSG( RF_ERROR, "clear_sub_id_info not implemented in derived common class!");
    return FALSE;
  }
