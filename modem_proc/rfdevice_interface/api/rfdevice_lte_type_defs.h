...
$DateTime: 2021/03/09 05:01:20 $ $Author: pwbldsvc $
$Header: //components/rel/rfdevice_interface.mpss/2.11/api/rfdevice_lte_type_defs.h#1 $ 
   
when       who     what, where, why 
--------   ---     -------------------------------------------------------------
12/08/16   ry      Added an enum to indicate the LTE modulation transition
01/19/16   ry      Updated the rfdevice_lte_get_rx_path_info_type API
10/21/15   vkm     Interface to check if band support on a device is over ICI
10/16/15   zg      Added other_rfm_device in spur_mitigation_params
09/12/15   jhap    WTR ICI support: pass the correct CA mode
08/03/15   bm      Renamed the structure rfdevice_lte_single_rb_data_type
                   Also added default_4fmod_script member to this structure
06/14/15   stw     Moved Frequency in Hz to 64 bit for LTE-U
05/19/15   bm      Added support for single_rb_data_type structure
05/05/15   aks     Added structure: rfdevice_lte_dev_cfg_params
...
typedef struct
{
  ...
  /* RFM devices of other band */ 
  rfcom_device_enum_type other_rfm_device_prx;
  rfcom_device_enum_type other_rfm_device_drx;
  ...
  int8 band_home_notch_threshold_list[RFDEVICE_LTE_MAX_NOTCHES];
  int8 band_other_notch_threshold_list[RFDEVICE_LTE_MAX_NOTCHES];
...
typedef enum
{
 RFDEVICE_LTE_MCS_16QAM_TO_HIGHER = 0,
 RFDEVICE_LTE_MCS_HIGHER_TO_16QAM,
 RFDEVICE_LTE_INVALID_MCS_TRANSITION
}
rfdevice_lte_tx_mcs_transition_enum_type;
...
/*! 
  ...
  @brief
  Tech driver passes all the devices which are associated with the
  carrier being tuned. Device driver loops through (N-1) devices in
  the list to determine if PLL has been tuned.
 
  Since PRx and DRx can be on different splits, tech needs to pass in
  both band and rfm_device. 
 
  */
typedef struct
{
  /*! Band to which Rx chain was tuned to */
  rfcom_lte_band_type band;

  /*! RFM device used to represent Rx chain */
  rfm_device_enum_type rfm_device;
}
rfdevice_lte_dev_cfg_params;
/*----------------------------------------------------------------------------*/
