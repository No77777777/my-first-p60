...
10/13/15   bm     Implement a virtual API get_hdet40_scaling_lut
08/03/15   bm     Renamed get_single_rb_scripts() to get_4fmod_scripts()
                  Added new API get_dac_backoff_value() 
07/15/15   bm     Added API to get_hdet40_scaling_lut from WTR driver
...
class rfdevice_trx_lte_tx:public rfdevice_class 
{
  ...
  /*----------------------------------------------------------------------------*/
  virtual
  uint16* get_hdet40_scaling_lut
  (
    rfcom_lte_band_type rf_band,
    uint8* scaling_factor
  )
  {
    return (get_hdet40_scaling_lut(scaling_factor));
  }
  ...
  {
    RF_MSG( RF_ERROR, "get_hdet40_scaling_lut: Derived class does not support this API!" );
    return NULL;
  }
  
 /*----------------------------------------------------------------------------*/
  virtual
  boolean hkadc_override_script
  (
    rfdevice_lte_script_data_type * script_data_ptr,
    rfdevice_lte_tx_mcs_transition_enum_type mcs_trans
  )
  {
    RF_MSG( RF_ERROR, "hkadc_override_script: Derived class does not support this API!" );
    return NULL;
  }
