...
$Header: //components/rel/rfdevice_interface.mpss/2.11/api/rfdevice_pa.h#1 $

when       who    what, where, why
--------   ---   ---------------------------------------------------------------
09/23/15   hzh    Added QFE script logging
06/23/15   dbc    Add set_pa_output_cap()  
...
{
public:
	static boolean RFFE_REG_DUMP_DEBUG_PA;
    rfdevice_pa();
    virtual ~rfdevice_pa();
    ...
    */
    ...
    /*!
      @brief
      interface function setting PA ouput cap value

      @details
      set_pa_output_cap()

      @param pa_params_p
      structure containing RAT/Band/BW/ and PA output Cap offset
     
      @return
      boolean indicating pass or fail
    */
    virtual boolean set_pa_output_cap
    (
      rfdevice_pa_cfg* pa_params_p
    );
