...
/*==============================================================================
  ...
  $Header: //components/rel/rfdevice_interface.mpss/2.11/api/rf_buffer_intf.h#1 $

when       who     what, where, why
--------------------------------------------------------------------------------
11/27/19   mpa     buffer interface data structures modified
08/25/15   aks     Added constants:BYTES_PER_QLINK_WORD, MAX_QLINK_WORDS_PER_PACKET
08/04/15   aks     Added APIs: append_qlink, update_qlink_blk_info
...
typedef struct
{
  uint8 offset; // Indicates where WAIT_ABS is located in the current packet
  int16 fixup_time; //Time relative to Slot boundary/Action time in USTMR units
} rf_buffer_qlink_timer_fixup_type;
...
public:
  ...
  /*! Number of bytes in each Qlink word */
  static const uint16 BYTES_PER_QLINK_WORD= 2;

  /*! Max words per Qlink packet */
  static const uint16 MAX_QLINK_WORDS_PER_PACKET= 32;
...
public:
  ...
  (
    rf_buffer_script_type script = RF_BUFFER_ALL
  )
  {
  ...
  (
    ...
    uint8 rd_delay,
    boolean new_time_ref = FALSE,
    boolean combine_contiguous_writes =FALSE
  )
  {
  ...
  class must implement this function if it contains a SSBI buffer */
  virtual int16 get_ssbi_space_available
  (
    void
  )
  {
  ...
  virtual int16 get_space_available
  (
  ...
  virtual boolean get_rffe_info
  (
    uint16 index,
    rf_buffer_rffe_info_type* rffe_info
  )
  {
  ...
  virtual boolean get_script_execution_time
  (
  ...
  virtual boolean append_qlink
  (
    rf_buffer_transaction_type rd_wr,
    uint32 start_addr,
    uint32 *data,
    uint8 data_len_words,
    int16 delay_ustmr,
    rf_buffer_qlink_timer_fixup_type *timer_fixup_info,
    uint16 timer_fixup_cnt
  )
  {
    RF_MSG(RF_ERROR,"append_qlink() method not implemented");
    return FALSE;
  }

  virtual boolean update_qlink_info
  (
    uint32* qlink_blk_start_addr,
    uint16 qlink_blk_size_bytes
  )
  {
    RF_MSG(RF_ERROR,"update_qlink_info() method not implemented");
    return FALSE;
  }
...
extern "C"
{
  ...
  /*----------------------------------------------------------------------------*/
  extern boolean rf_buffer_clear_script
  (
    rf_buffer_intf* buf_ptr,
    rf_buffer_script_type script
  );
  ...
  /*----------------------------------------------------------------------------*/
  extern boolean rf_buffer_append_ssbi
  (
    rf_buffer_intf* buf_ptr,
    uint8 bus,
    uint8 address,
  ...
  /*----------------------------------------------------------------------------*/
  extern boolean rf_buffer_append_ssbi_with_new_time
  (
    rf_buffer_intf* buf_ptr,
    uint8 bus,
    uint8 address,
  ...
  /*----------------------------------------------------------------------------*/
  extern boolean rf_buffer_append_grfc
  (
    rf_buffer_intf* buf_ptr,
    uint16 grfc_id,
    uint8 start_logic,
  ...
  /*----------------------------------------------------------------------------*/
  extern boolean rf_buffer_append_rffe
  (
    rf_buffer_intf* buf_ptr,
    uint8 slave_id,
    uint8 channel,
  ...
  /*----------------------------------------------------------------------------*/
  extern boolean rf_buffer_append_rffe_multi
  (
    rf_buffer_intf* buf_ptr,
    uint8 slave_id,
    uint8 channel,
  ...
  /*----------------------------------------------------------------------------*/
  extern int16 rf_buffer_get_ssbi_space_available
  (
    rf_buffer_intf* buf_ptr
  );

  /*----------------------------------------------------------------------------*/
  extern int16 rf_buffer_get_space_available
  (
    rf_buffer_intf* buf_ptr,
    rf_buffer_script_type script
  );
  /*----------------------------------------------------------------------------*/
  extern boolean rf_buffer_destroy
  (
    rf_buffer_intf* buf_ptr
  );

  /*----------------------------------------------------------------------------*/
  extern boolean rf_buffer_get_ssbi_info
  (
    rf_buffer_intf* buf_ptr,
    uint16 index,
    rf_buffer_ssbi_info_type* ssbi_info
  ...
  /*----------------------------------------------------------------------------*/
  extern boolean rf_buffer_get_script_execution_time
  (
    rf_buffer_intf* buf_ptr,
    rf_buffer_script_type resource_type,
    uint32 *execution_time
  ...
  /*----------------------------------------------------------------------------*/
  extern boolean rf_buffer_update_qlink_info
  (
    rf_buffer_intf* buf_ptr,
    uint32* qlink_blk_start_addr,
    uint16 qlink_blk_size_bytes
