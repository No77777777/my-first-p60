...
$Header: //components/rel/rfdevice_interface.mpss/2.11/api/rfdevice_trx_wcdma_rx.h#1 $
...
/*! 
  ...
  @details
  API provides an interface to build the AGC preload script.
 
  @param rfm_dev
  enum indicating the current logical device
 
  @param rf_mode
  enum indicating the current technology
 
  @param rf_band
  enum indicating the current wcdma band
 
  @param rx_params
  structure containing the necessary rx parameters - channel & bandwidth
 
  @param exec_type
  enum to indicate direct hardware write or prepare script
 
  @param *script_buffer
  pointer to rf buffer which will contain the tune script
  ...
  @return
  TRUE is tune script preparation is successful else return FALSE 

*/
  virtual boolean get_rxagc_reload_script ( rfm_device_enum_type rfm_dev,
                                            rfcom_mode_enum_type rf_mode,
                                            rfcom_wcdma_band_type rf_band,
                                            rfdevice_wcdma_rx_tune_param_type rx_params,
                                            rf_device_execution_type exec_type,
...
/*!
  ...
  @param band
  Band to which Rx chain is to be tuned to 
...
/*!
  ...
  @param rx_mode
  Enum to indicate if Rx chain is being used by tech for 
  PRx/DRx operation 
