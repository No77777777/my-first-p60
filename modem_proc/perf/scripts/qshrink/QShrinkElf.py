...
$Header: //components/rel/perf.mpss/4.4.2/scripts/qshrink/QShrinkElf.py#1 $
$Date: 2022/10/12 $
...
def initializeArmElfShrinkState(elfFile, outputFile, buildOutputPath, hashFile,qdbFile,
                                logFile, anomalyListFile, iuHashDir):
  ...
  ElfShrinkState.iuHashDir = iuHashDir
...
def runQShrink(elfFile, outputFile,buildOutputPath,hashFile,qdbFile,
               ...
               use_linker_end_symbol, iuHashDir):
  ...
  resultStrs = [ \
    ...
    " ".join(["IU Hash Directory:", iuHashDir]),
  ]
...
def qsr4process():
  ...
  # Buffers holding the IU Content and Trace hash messages
  ElfShrinkState.IU_TRACE_MSG = []
  ElfShrinkState.IU_CONTENT_MSG = []
  ...
  processIUMessages()
  qsr4CreateHashFile()
  del ElfShrinkState.IU_TRACE_MSG #free memory
  del ElfShrinkState.IU_CONTENT_MSG #free memory
...
#   .qsr4 file
# Description
#   Extracts hash strings between Content and IUtrace tags then stores
#   them in IU_CONTENT_MSG and IU_TRACE_MSG global lists
# Return Values:
#=========================================================================

def extractMessages(iuFileName):

  global ElfShrinkState

  iu_content_str_start = -1
  iu_content_str_end = -1

  iu_trace_str_start = -1
  iu_trace_str_end = -1

# Open IU File with hashes
  with open (iuFileName,'rw+') as iuFile:
    # Read all the lines of the file into a variable
    lines = iuFile.readlines()
    # Enumerate the lines and keep a counter of the lines
    for line_number, line in enumerate(lines):
      # In each line look for the IUTraceStrContent or Content tag to add the list of IU Trace messages
      if "<Content>" in line:
        iu_content_str_start = line_number
      if "<\Content>" in line:
        iu_content_str_end = line_number
      if "<IUtrace>" in line:
        iu_trace_str_start = line_number
      if "<\IUtrace>" in line:
        iu_trace_str_end = line_number

  # Make sure we found the proper tags in the file
  if iu_content_str_start == -1: 
    print "Did not find start tag <Content> in " + str(iuFileName)
    return
  if iu_content_str_end == -1:
    print "Did not find end tag <\Content> in " + str(iuFileName)
    return
  if iu_trace_str_end == -1: 
    print "Did not find start tag <IUtrace> in " + str(iuFileName)
    return
  if iu_trace_str_end == -1:
    print "Did not find end tag <\IUtrace> in " + str(iuFileName)
    return

  # Extract all the messages from specefied lines and store them
  ElfShrinkState.IU_CONTENT_MSG.extend(lines[iu_content_str_start + 1 : iu_content_str_end])
  ElfShrinkState.IU_TRACE_MSG.extend(lines[iu_trace_str_start + 1 : iu_trace_str_end])

#=========================================================================
# Function : processIUMessages
#
# Arguments
#
# Description
#   The IU message hash directory is stored as a global. We go through
#   that directory finding all the .qsr4 files
# Return Values:
#=========================================================================

def processIUMessages():
  
  global ElfShrinkState

  # IU hash Message Directory 
  hashDir = ElfShrinkState.iuHashDir

  # If no directory is given skip this
  if hashDir != '':
    # Save the current directory to go back to it later
    cwd = os.getcwd()
    # Change to the iu message hash directory
    os.chdir(hashDir)
    # for every qsr4 file go through it and extract the messages
    for file in glob.glob("*.qsr4"):
...
# all symbol names
  ...
  QCImageVersionRE = re.compile(r'QC_IMAGE_VERSION_STRING_AUTO_UPDATED')
  ...
  QSR4MtraceInternalEndRE= re.compile(r'__QSR_4_0_MTRACE_INTERNAL_END__')
  ...
  ElfShrinkState.QSR4MTRACEInternalEnd = 0 
  ...
  # check for a QSR struct symbol
    ...
    elif QCImageVersionRE.match(entry.name):
      ElfShrinkState.QCImageVersion = getQCImageVersion(entry)

    elif QSR4MtraceInternalEndRE.match(entry.name):
      ElfShrinkState.QSR4MTRACEInternalEnd = entry.value
...
#=========================================================================
# Function : getQCImageVersion
#
# Arguments
#   entry: symbol entry of QC_IMAGE_VERSION_STRING_AUTO_UPDATED
#
# Description
#   
#
# Return Values:
#  QCImageVersion string 
#=========================================================================
def getQCImageVersion(entry):
    shRodata = getSectionHeader(entry.sectionIndex)
    secBytes = getSectionData(entry.sectionIndex)
    offset = entry.value - shRodata.data['sh_addr']
    for idx,strVer in getStringGenerator(secBytes, offset):
      return strVer    
  

#=========================================================================
# Function : processV4InternalMtraceStructs
#
# Arguments
#
# Description
...
#  List of structs
#=========================================================================
def processV4InternalMtraceStructs(start, end):

  global ElfShrinkState
  global QSR_MSG_START, QSR_MSG_SIZE, QSR_MSG_END, QSR_MSG_OFFSET
  MTraceRE = re.compile('(.*?):(.*?):(.*?):(.*?):(.*?):(.*)')
  MTraceHashRE = re.compile('(.*?):::(.*?)::(.*)')

  if (end == 0):
    end = start;

#Read QSR_4_0_MSG section and create hash table and DIAG run time filtering tables
  strPointerOffset = start  - QSR_MSG_START
  strPointerOffsetEnd = end - QSR_MSG_START
  ...
  for hashval,entry in getStringGenerator(ElfShrinkState.bytesDict['QSR_4_0_MSG'], strPointerOffset,strPointerOffsetEnd):
      ...
      # avoid partial strings in future Master Hash load
      string = re.sub('\x0a', r'\\n', string)
      match = MTraceHashRE.match(string)
      if match:
        hashstring = "".join([
                      str(hashval),":|",
                      match.group(2),":::::",
        ...
        string_no_line = entry[entry.find(":"):]
        hash1 = hashlib.md5(string_no_line).hexdigest()[-8:]
  ...
  ElfShrinkState.outputLog.write("".join(["\n","Internal MTRACE Messages processed from address, ", hex(start)," to, ",hex(end), "\n"])) 

#=========================================================================
# Function : processV4ExternalMtraceStructs
#
# Arguments
#
# Description
...
#  List of structs
#=========================================================================
def processV4ExternalMtraceStructs(start, end):
...
#Read QSR_4_0_MSG section and create hash table and DIAG run time filtering tables
  ...
  ElfShrinkState.outputLog.write("".join(["\n","External MTRACE Messages processed from address, ", hex(start)," to, ",hex(end), "\n"])) 
...
#Read QSR_4_0_MSG section and create hash table and DIAG run time filtering tables
 ...
 ################################## 
  if ElfShrinkState.QSR4MTRACEStart!=0:
   if (ElfShrinkState.QSR4MTRACEInternalEnd == 0) or (ElfShrinkState.QSR4MTRACEInternalEnd == ElfShrinkState.QSR4MTRACEStart):
     processV4ExternalMtraceStructs(ElfShrinkState.QSR4MTRACEStart, ElfShrinkState.QSR4MTRACEEnd)
   else:
    processV4InternalMtraceStructs(ElfShrinkState.QSR4MTRACEStart, ElfShrinkState.QSR4MTRACEInternalEnd)
    if ElfShrinkState.QSR4MTRACEInternalEnd < ElfShrinkState.QSR4MTRACEEnd:
      processV4ExternalMtraceStructs(ElfShrinkState.QSR4MTRACEInternalEnd, ElfShrinkState.QSR4MTRACEEnd)
...
def qsr4CreateHashFile():
 ...
 #write Hash File
  ...
  contentStr = [ \
    ...
    " ".join(["<Baseline>",ElfShrinkState.QCImageVersion,"<\Baseline>","\n"]),
    " ".join(["<Content>","\n"]),
    "".join(ElfShrinkState.QSR4_STRINGlist),
    " ".join(["<\Content>","\n"])    
  ]

  if (len(ElfShrinkState.QSR4_MTRACESTRINGlist) !=0) or \
  (len(ElfShrinkState.IU_CONTENT_MSG) !=0) or \
  (len(ElfShrinkState.IU_TRACE_MSG) !=0):    
    contentStr = ["".join(contentStr),
                  " ".join(["<MtraceContent>","\n"])
                      ]

  if len(ElfShrinkState.IU_CONTENT_MSG) !=0:    
    contentStr = ["".join(contentStr),
                       "".join(ElfShrinkState.IU_CONTENT_MSG),
                       ]

  if len(ElfShrinkState.IU_TRACE_MSG) !=0:    
    contentStr = ["".join(contentStr),
                       "".join(ElfShrinkState.IU_TRACE_MSG),
                       ]
