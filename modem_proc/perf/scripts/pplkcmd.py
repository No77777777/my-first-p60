...
class Pplk(object):  # {{{
  ...
  def __init__(self, target, source, env, debug=False, keeplog=True):  # {{{
    ...
    if 'USES_INTERNAL_BUILD' in env:
      run_qcmat4_env = os.environ.get('RUN_QCMAT4')
      
      # this is the general case, as it's undef in env for most user
      if run_qcmat4_env is None:
        self.run_qcmat4 = True
      # override case for specific matching value
      # run_qcmat4_env should be a string, but force it to be one just in case
      elif str(run_qcmat4_env).lower() in ("no", "false", "f", "0"): 
        self.run_qcmat4 = False
      # again with env defined, but match above list, default to run qcmat4
      else:                                            
        self.run_qcmat4 = True
...
# http://docs.python.org/2/library/multiprocessing.html#using-a-pool-of-workers
  ...
  def qshrink(self, inpath, outpath):  # {{{
    ...
    # qshrink stores its output in a fixed directory relative to the build,
    # which other tools depend on; don't arbitrarily move it
    ...
    if not os.path.exists(iu_hash_path):
      iu_hash_path = ""
    ...
    if retcode != 0:
      ...
      for line in stdout.split('\n'):
        if line.startswith('Qshrink Version :'):
          self.p('  %s' % line)
        if line.startswith('QShrink memory saving :'):
          self.p('  %s' % line)
  ...
  def elfmanip(self, inpath, outpath):  # {{{
    ...
    # add q6zip USES flags, with -Q prepended
    flags.extend([k for j in [('-Q', i) for i in self.q6z_flags] for k in j])
  ...
  def qcmat4(self, inpath, outpath):  # {{{
    ...
    retcode, stdout, stderr = self.call(
      [
        self.python,
        pj(self.mypspath,'QCMAT4', 'qcmat.py'),
      ]
      + flags
    )

    if retcode != 0:
      self.p('QCMAT4 failed. Logs below:')
      self.p('stdout: %s' % stdout)
      self.p('stderr: %s' % stderr)

    else:
      self.log('QCMAT4 logs:')
      self.log('stdout: %s' % stdout)
      self.log('stderr: %s' % stderr)
  ...
  def tcmChk(self,inpath, outpath):
    t1 = timer()
    retcode = 0
    ...
    if retcode != 0:
      self.p('TCM check failed. Logs below:')
      self.p('stdout: %s' % stdout)
      self.p('stderr: %s' % stderr)

    else:
      self.log('TCM check logs:')
      self.log('stdout: %s' % stdout)
      self.log('stderr: %s' % stderr)
    ...
    self.p('  TCM check execution time: %s' % t_str(t_diff))
  ...
  def move_up_offset(self, inpath, outpath):  # {{{
    ...
    retcode, stdout, stderr = self.call([
      self.python,
      '-O',
      pj(self.mypspath, 'move_up_offset.py'),
      inpath,
      outpath])

    if retcode != 0:
      self.p('Move up offset failed. Logs below:')
      self.p('stdout: %s' % stdout)
      self.p('stderr: %s' % stderr)

    else:
      self.log('Passed move up offset')
      self.log('stdout: %s' % stdout)
      self.log('stderr: %s' % stderr)
  ...
  def clade_debug_setup(self, inpath, outpath):
    t1 = timer()
    retcode = 0
    # clade_debug_setup.py aborts if there is nothing to do, failing to generate
    # outpath. Copy here to force outpath generation
    #shutil.copyfile(inpath, outpath)
    if os.path.isfile(pj(self.mypspath,'clade_debug_setup.py')) != True:
      return retcode
    self.p('\nRunning CLADE Debug Setup')

    retcode, stdout, stderr = self.call([
      self.python,
      '-O',
      pj(self.mypspath, 'clade_debug_setup.py'),
      outpath,
      outpath])
	  
    if retcode != 0:
      self.log('See CLADE Debug Setup Logs below:')
      self.log('stdout: %s' % stdout)
      self.log('stderr: %s' % stderr)
    else:
      self.log('Ready to Debug CLADE')
      self.log('stdout: %s' % stdout)
      self.log('stderr: %s' % stderr)
    ...
    self.p('  CLADE Debug Setup execution time: %s' % t_str(t_diff))
  ...
  def integrity_check(self, inpath, outpath):
    ...
    retcode, stdout, stderr = self.call([
      self.python,
      '-O',
      pj(self.mypspath, 'integrity_check.py'),
      outpath,
      outpath])
...
##Minidump_elf_editor_____________##
  def mee(self,inpath,outpath):
    t1 = timer()
    retcode = 0
    cur_dir=os.path.dirname(os.path.realpath(__file__))
    mee_path = pj(cur_dir,'..','..','stability','minidump','mee.py')
    xml_path = pj(cur_dir,'..','..','stability','minidump','minidump.xml')
    if (os.path.exists(mee_path)):
      self.p('\nRunning minidump elf editor:')
      ...
      if retcode != 0:
        self.p('Minidump Editor failed. Logs below:')
        self.p('stdout: %s' % stdout)
        self.p('stderr: %s' % stderr)
      ...
      self.p('  Minidump Elf editor execution time: %s' % t_str(t_diff))
    
    else:
      self.p('mee.py tool not found. Skipping Minidump Elf Editor')
...
##Minidump_elf_editor_____________##
  ...
  def manip(self, method, readonly=False):  # {{{
    if self.finalized:
      raise Exception(
    ...
    if method is not 'qcmat4':
        if readonly:
            outpath = '.'.join([self.elfpath] + self.executed_manips)
        else:
...
def main(target, source, env):  # {{{
  ...
  r = p.manip('clade_debug_setup',readonly=True)
  if r != 0:
    return r  
