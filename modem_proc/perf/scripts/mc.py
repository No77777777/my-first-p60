...
class MemoryChecker(object):  # {{{
  ...
  size_template = (
    ...
    ('MPSS secondary Heap', 'mpss_secondary_heap', 0),    
  ...
  def dynamic_or_static(self):  # {{{
    ...
    if self.sym_sizes['amss_heap']['type'] == 'static':
      self.sizes[self.sz_idx['img_sz']] -= \
        self.sym_sizes['amss_heap']['size']
    elif in_range(self.start_address,
                self.budget,
    ...
    if self.sym_sizes['mpss_heap']['type'] == 'static':
        self.sizes[self.sz_idx['img_sz']] -= \
          self.sym_sizes['mpss_heap']['size']
    elif ('mpss_heap' in self.sym_sizes and
    ...
    #change added for new secondary modem heap
    if self.sym_sizes['mpss_secondary_heap']['type'] == 'static':
        self.sizes[self.sz_idx['img_sz']] -= \
          self.sym_sizes['mpss_secondary_heap']['size']
    elif ('mpss_secondary_heap' in self.sym_sizes and
        'start' in self.sym_sizes['mpss_secondary_heap'] and
            'size' in self.sym_sizes['mpss_secondary_heap']):
      if in_range(self.start_address,
                  self.budget,
                  self.sym_sizes['mpss_secondary_heap']['start'],
                  self.sym_sizes['mpss_secondary_heap']['size']):
        self.sizes[self.sz_idx['img_sz']] -= \
          self.sym_sizes['mpss_secondary_heap']['size']

        self.sym_sizes['mpss_secondary_heap']['type'] = 'static'
      else:
        self.sym_sizes['mpss_secondary_heap']['type'] = 'dynamic'
      ...
      if prev_start > 0 and prev_size > 0:
        ...
        if potential_pad >= 0:
          if (
            ...
            dp(''.join([
              'Padding of %8d bytes between segments 0x%08x and 0x%08x' % (
                potential_pad,
                prev_start,
                start,
  ...
  def get_sym_sizes(self):  # {{{
    ...
    sym_sizes['mpss_heap'] = {'start': 0, 'size': 0, 'found': False}
    sym_sizes['mpss_secondary_heap'] = {'start': 0, 'size': 0, 'found': False}
  ...
  def disassemble_mpss_internal_heap_size(self):  # {{{
    hexagon_path = self.hex_path
  ...
  def get_budget(self):  # {{{
    ...
    self.sym_sizes['mpss_heap']['type'] = 'NA'
    self.sym_sizes['amss_heap']['type'] = 'NA'
    self.sym_sizes['mpss_secondary_heap']['type'] = 'NA'	
      ...
      for i in xrange(0, pool_configs.st_size, config_entry_size):
        ...
        if pool_entry_name == 'DEFAULT_PHYSPOOL':
          ...
          budget = sum(pool_entry_addr_pairs[1::2])
           ...
           #change added for new mem_pools for heap CR905104
        elif pool_entry_name == 'AMSS_MEM_HEAP_POOL':
                self.sym_sizes['amss_heap']['start'] = -1,
                self.sym_sizes['amss_heap']['size']  = sum(pool_entry_addr_pairs[1::2])
                self.sym_sizes['amss_heap']['type']  = 'static'
                    
        elif pool_entry_name == 'MODEM_MEM_HEAP_POOL':
                self.sym_sizes['mpss_heap']['start'] = -1,
                self.sym_sizes['mpss_heap']['size']  = sum(pool_entry_addr_pairs[1::2])
                self.sym_sizes['mpss_heap']['type']  = 'static'
				
        elif pool_entry_name == 'MODEM_INTERNAL_MEM_HEAP_POOL':
                self.sym_sizes['mpss_secondary_heap']['start'] = -1,
                self.sym_sizes['mpss_secondary_heap']['size']  = sum(pool_entry_addr_pairs[1::2])
                self.sym_sizes['mpss_secondary_heap']['type']  = 'static'
