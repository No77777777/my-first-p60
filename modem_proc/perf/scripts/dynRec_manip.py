...
#
#-------------------------------------------------------------------------------
#  $Header: //components/rel/perf.mpss/4.4.2/scripts/dynRec_manip.py#1 $
...
# --------  ------  ----    ----------------------------------------------------
# 10/07/15  hzhi    2.3     For CR 872615, add post-link detector for wrong RFC code base.
# 07/16/15  hzhi    2.2     CR 880152: Support EFS RF card scenario.
...
def readDataByAddress(elf, address, size):
 ...
 #Also check for SHT_PROGBITS to ensure this section hold information defined by the program 
 #whose format and meaning solely determined by the program. This also protects against accessing
 #sections of type SHT_NOBITS
  if (section.sh_type == const.sectionTypes.SHT_PROGBITS and address >= section.sh_addr and
...
#{{{{
  ...
  #{
    ...
    #{{
      ...
      if(symbol_va_start_text.st_value < symbol_va_end_text.st_value and
        not symbol_va_start_data.st_value > symbol_va_end_data.st_value):
      #{
        print " "*9 + "-> %d" %j
        print " "*12\
  ...
  #{
    print " "*3 + "-> ! Dynrec feature is enabled but mcfg_dynrec_* API has not been called, please check source code correctness, quit execution."
    elf_handle.updateSymbolValuesByDict(Dict)
    elf_handle.writeOutELF(sys.argv[2])
    sys.exit()
  #}
  ...
  #{
    ...
    # data_ptr -> 'client_id'
    if setDataByAddress(elf_handle, data_ptr,1,client_id) == const.RC_ERROR:
    #{
      # break build process if incorrect compress_rw.lst is in use.
      # this is for CR 843691.
      elf_handle.writeOutELF(sys.argv[2])
      print " "*6 + "-> !!! FATAL ERROR: failed to set symbol: %-40s\t!!! " %curr_entry_name
      print " "*9 + "-> !!! Please check if 'compress_rw.lst' happens to compress 'mcfg_dynrec.o' by mistake. This is a forced termination of build process to prevent fatal runtime crash!!!"
      sys.exit(1)
    #}
