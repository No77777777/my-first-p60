...
$Header: //components/rel/rfdevice_papm.mpss/1.32/api/rfdevice_papm_common.h#1 $
$Author: pwbldsvc $ 

when       who     what, where, why
--------   ---     -------------------------------------------------------------
12/27/19   smi     Add support to get RFFE device register address list
11/07/19   dh      Added get_apt_voltage_min_max() API
...
#define RFDEVICE_PAPM_MAX_REG_SIZE 60
...
public:
  ...
  boolean get_apt_voltage_min_max(rfdevice_papm_misc_info* APT_voltage_info_p);
...
protected:
  ...
  rfdevice_papm_misc_info voltage_info;
