...
Notice that changes are listed in reverse chronological order.
  ...
  $Header: //components/rel/rfdevice_qpa.mpss/1.32/api/rfdevice_qpa_4g.h#1 $
  $DateTime: 2021/03/09 04:50:56 $
  $Author: pwbldsvc $

when       who   what, where, why
--------   ---   ---------------------------------------------------------------
12/27/19   smi   Added support to get RFFE device register address list
01/12/18   dbc   Add support for lpm_self_triggered flag 
01/03/18   dbc   Add support for DPD ports
05/09/16   dbc   Added GSM support for EFS gain & bias scripts
10/08/15   px    Added support for QPA/QASM common tab voting
06/09/15   dbc   Add support for PA Output Cap
...
protected:
  ...
  boolean rfdevice_qpa_4g_process_pa_output_cap_table
  (
    rfdevice_qpa_4g* qpa_device,
    rf_buffer_intf *rf_buff,
    const rfdevice_qpa_4g_reg_data_type* reg_table,
...
boolean update_one_script_reg_addr
(
  const rfdevice_qpa_4g_reg_data_type* script_tbl,
  uint8 size
);

boolean update_ag_list_data_reg_addr
(
  rfdevice_qpa_4g_ag_list_data_type** ag_data_list,
  uint8 num_ports,
  uint8 size
);

boolean update_ag_gain_list_data_reg_addr
(
  rfdevice_qpa_4g_ag_gain_list_data_type** ag_data_list,
  uint8 num_ports,
  uint8 size
);
...
void rfdevice_qpa_4g_calc_output_cap(int8 offset, uint8 autogen_default, uint8 *ret_val, boolean *status);
...
boolean rfdevice_qpa_4g_is_dpd_port(uint8 port_index);
