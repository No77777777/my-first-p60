...
Qualcomm Proprietary
  ...
  $Header: //components/rel/rfdevice_qpa.mpss/1.32/api/rfdevice_qpa_4g_typedef.h#1 $
...
07/17/15   dbc   Add icq_default field to rfdevice_qpa_4g_settings_type (16 Bit ICQ support)
...
typedef struct
{
  ...
  uint8 pa_output_cap_script_size;
  rfdevice_qpa_4g_ag_list_data_type **pa_output_cap;
...
typedef void (*qpa_calc_output_cap_fn_ptr)(int8 offset, uint8 autogen_default, uint8 *ret_val, boolean *status);

typedef uint8 (*qpa_get_efs_data_column_fn_ptr)(uint8 port_index, rfdevice_qpa_4g_tx_bw_data_type lcl_bw);

typedef boolean (*qpa_is_dpd_port_fn_ptr)(uint8 port_index);
...
typedef struct
{
  ...
  qpa_calc_output_cap_fn_ptr calc_output_cap;
  ...
  qpa_is_dpd_port_fn_ptr is_dpd_port;
