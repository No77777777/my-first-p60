...
/*
  ...
  ============================================================================
  $Header: //components/rel/avs.mpss/9.0.c51/elite/vsd/vs/inc/private/vs_i.h#2 $
  $Author: svutukur $
  ============================================================================
*/
...
/* IPA APIs */
#if !defined ( AVS_MPSS_TEST ) && !defined ( WINSIM )
...
#endif  /* #if !defined ( AVS_MPSS_TEST ) && !defined ( WINSIM ) */
...
/**
 * With VS command grouping feature enabled, 4k bytes heap required (for creating job objects) for enabling vocoder session.
 */
#define VS_HEAP_SIZE_V ( 2048 * 2 )
 ...
 */
#define VS_ADSP_CMD_TIMEOUT_DURATION_NS ( 5000000000 )

/* Timeout duration to receive VSS_IMVM_EVT_APPS_START is set to 5 seconds to give sufficient time for Apps to start voice session. 
  In case of timeout, state machine will be completed without completing rest of the actions. This will make sure that call tear-down and subsequent call set up
goes through without any errors. */
#define VS_MEMMAP_CMD_TIMEOUT_DURATION_NS ( 5000000000 )
 ...
 */
#define VS_VOC_TIMING_VFR_START_MARGIN_IN_US     ( 500 )

/*
 * Time difference between the RxRef timer expiry at IMS and the 
 * decoder start at audio
 */
#define VS_VOC_TIMING_RXREF_OFFSET_WITH_DEC_START_IN_US  ( 2000 )
...
#define VS_REPORT_COMMAND_SEND_FAILED( last_cmd_sent, rc ) \
  { MSG_2( MSG_SSID_DFLT, MSG_LEGACY_ERROR,\
                      "VS: ADSP COMMAND SEND FAILED. COMMAND_OPCODE=[0x%08x], rc=[0x%08x]", \
                             last_cmd_sent, rc); } 

#define VS_PANIC_ON_ADSP_COMMAND_TIMEOUT( last_cmd_sent, rc ) \
  { if ( rc ) { ERR_FATAL( "VS: ADSP COMMAND_TIMEOUT (5 seconds): COMMAND_OPCODE=[0x%08x], rc[0x%08x]", \
                            last_cmd_sent, rc, 0 ); } }
...
extern apr_timer_t vs_adsp_command_timeout_timer;
extern bool_t vs_adsp_command_timeout_is_triggered;

extern apr_timer_t vs_adsp_memmap_timeout_timer;
extern bool_t vs_adsp_memmap_timeout_is_trigerred;
extern uint32_t vs_adsp_memmap_timeout_count;

extern uint32_t vs_adsp_last_cmd_sent;
...
typedef enum vs_voc_action_enum_t
{
  ...
  VS_VOC_ACTION_ENUM_PKTEXG_EVENT_ENABLE,
...
typedef struct vs_session_object_t {
  ...
  bool_t is_apps_started;
  /**< Indicates that HLOS has started voice session in ADSP. */
...
uint32_t vs_apps_start_event_handler (
  aprv2_packet_t* packet
);

uint32_t vs_apps_stop_event_handler (
  aprv2_packet_t* packet
);
