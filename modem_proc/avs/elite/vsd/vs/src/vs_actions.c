...
/*
  ...
  $Header: //components/rel/avs.mpss/9.0.c51/elite/vsd/vs/src/vs_actions.c#1 $
  $Author: pwbldsvc $
*/
...
)
{
  ...
  MSG_1( MSG_SSID_DFLT, MSG_LEGACY_LOW,
         "VS_PROFILING:VSS_IMVM_CMD_CREATE_FULL_CONTROL_SESSION(): vs_cmd_ticks_in_us=(%lu)",
         vs_cmd_ticks_in_us );
#endif /* VS_PROFILING */

  session_obj->cmd_ctrl.last_cmd = vs_adsp_last_cmd_sent = VSS_IMVM_CMD_CREATE_FULL_CONTROL_SESSION;

  /* Reset is_apps_started to FALSE when creating MVM session.
     This flag will be set to TRUE (event: VSS_IMVM_EVT_APPS_START) after 
     APPS creates passive MVM session. This flag is checked to make sure 
     that VS does not access PCIe bus before APPS votes for PCIe link up.
   */
  session_obj->is_apps_started = FALSE;
...
)
{
  ...
  vs_cmd_ticks_in_us = (uint64_t)((qurt_sysclock_get_hw_ticks()) *10ull/192ull);
  MSG_1( MSG_SSID_DFLT, MSG_LEGACY_LOW,
         "VS_PROFILING:APRV2_IBASIC_CMD_DESTROY_SESSION(MVM): vs_cmd_ticks_in_us=(%lu)",
         vs_cmd_ticks_in_us );
#endif /* VS_PROFILING */

  session_obj->cmd_ctrl.last_cmd = vs_adsp_last_cmd_sent = APRV2_IBASIC_CMD_DESTROY_SESSION;
  rc = __aprv2_cmd_alloc_send(
         vs_apr_handle, APRV2_PKT_MSGTYPE_SEQCMD_V,
         vs_my_addr, ( (uint16_t) session_obj->header.handle ),
  ...
  /* Reset is_apps_started to FALSE when destroying MVM session.
     This flag will be set to TRUE (event: VSS_IMVM_EVT_APPS_START) after both
     APPS creates passive MVM session and VS creates full control MVM session
     for subsequent calls. This flag is checked to make sure that VS does not
     access PCIe bus before APPS votes for PCIe link up.
   */
  session_obj->is_apps_started = FALSE;
...
)
{
  ...
  vs_cmd_ticks_in_us = (uint64_t)((qurt_sysclock_get_hw_ticks()) *10ull/192ull);
  MSG_1( MSG_SSID_DFLT, MSG_LEGACY_LOW,
         "VS_PROFILING:VSS_IPKTEXG_CMD_REQ_MAILBOX_MEMORY_CONFIG(): vs_cmd_ticks_in_us=(%lu)",
         vs_cmd_ticks_in_us );
#endif /* VS_PROFILING */

  session_obj->cmd_ctrl.last_cmd = vs_adsp_last_cmd_sent = VSS_IPKTEXG_CMD_REQ_MAILBOX_MEMORY_CONFIG;
  rc = __aprv2_cmd_alloc_send(
         vs_apr_handle, APRV2_PKT_MSGTYPE_SEQCMD_V,
         vs_my_addr, ( (uint16_t) session_obj->header.handle ),
         vs_mvm_addr, session_obj->mvm_port,
...
)
{
  ...
  vs_cmd_ticks_in_us = (uint64_t)((qurt_sysclock_get_hw_ticks()) *10ull/192ull);
  MSG_1( MSG_SSID_DFLT, MSG_LEGACY_LOW,
         "VS_PROFILING:VSS_ISTREAM_CMD_CREATE_FULL_CONTROL_SESSION(): vs_cmd_ticks_in_us=(%lu)",
         vs_cmd_ticks_in_us );
#endif /* VS_PROFILING */

  session_obj->cmd_ctrl.last_cmd = vs_adsp_last_cmd_sent = VSS_ISTREAM_CMD_CREATE_FULL_CONTROL_SESSION;
...
)
{
  ...
  vs_cmd_ticks_in_us = (uint64_t)((qurt_sysclock_get_hw_ticks()) *10ull/192ull);
  MSG_1( MSG_SSID_DFLT, MSG_LEGACY_LOW,
         "VS_PROFILING:APRV2_IBASIC_CMD_DESTROY_SESSION(CVS): vs_cmd_ticks_in_us=(%lu)",
         vs_cmd_ticks_in_us );
#endif /* VS_PROFILING */
...
)
{
  ...
  vs_cmd_ticks_in_us = (uint64_t)((qurt_sysclock_get_hw_ticks()) *10ull/192ull);
  MSG_1( MSG_SSID_DFLT, MSG_LEGACY_LOW,
         "VS_PROFILING:VSS_IMVM_CMD_ATTACH_STREAM(): vs_cmd_ticks_in_us=(%lu)",
         vs_cmd_ticks_in_us );
#endif /* VS_PROFILING */

  session_obj->cmd_ctrl.last_cmd = vs_adsp_last_cmd_sent = VSS_IMVM_CMD_ATTACH_STREAM;
  rc = __aprv2_cmd_alloc_send(
         vs_apr_handle, APRV2_PKT_MSGTYPE_SEQCMD_V,
         vs_my_addr, ( ( uint16_t ) session_obj->header.handle ),
         vs_mvm_addr, session_obj->mvm_port,
...
)
{
  ...
  vs_cmd_ticks_in_us = (uint64_t)((qurt_sysclock_get_hw_ticks()) *10ull/192ull);
  MSG_1( MSG_SSID_DFLT, MSG_LEGACY_LOW,
         "VS_PROFILING:VSS_IMVM_CMD_DETACH_STREAM(): vs_cmd_ticks_in_us=(%lu)",
         vs_cmd_ticks_in_us );
#endif /* VS_PROFILING */

  session_obj->cmd_ctrl.last_cmd = vs_adsp_last_cmd_sent = VSS_IMVM_CMD_DETACH_STREAM;
  rc = __aprv2_cmd_alloc_send(
         vs_apr_handle, APRV2_PKT_MSGTYPE_SEQCMD_V,
         vs_my_addr, ( ( uint16_t ) session_obj->header.handle ),
         vs_mvm_addr, session_obj->mvm_port,
...
)
{
  ...
  if ( ( !_rc ) && ( item.ret_value == TRUE ) )
  {
    ...
    vs_cmd_ticks_in_us = (uint64_t)((qurt_sysclock_get_hw_ticks()) *10ull/192ull);
    MSG_1( MSG_SSID_DFLT, MSG_LEGACY_LOW,
           "VS_PROFILING:VSS_INOTIFY_CMD_LISTEN_FOR_EVENT_CLASS(): vs_cmd_ticks_in_us=(%lu)",
           vs_cmd_ticks_in_us );
#endif /* VS_PROFILING */
...
)
{
  ...
  if ( ( !_rc ) && ( item.ret_value == TRUE ) )
  {
    ...
    vs_cmd_ticks_in_us = (uint64_t)((qurt_sysclock_get_hw_ticks()) *10ull/192ull);
    MSG_1( MSG_SSID_DFLT, MSG_LEGACY_LOW,
           "VS_PROFILING:VSS_INOTIFY_CMD_CANCEL_EVENT_CLASS(): vs_cmd_ticks_in_us=(%lu)",
           vs_cmd_ticks_in_us );
#endif /* VS_PROFILING */
...
)
{
  ...
  vs_cmd_ticks_in_us = (uint64_t)((qurt_sysclock_get_hw_ticks()) *10ull/192ull);
  MSG_1( MSG_SSID_DFLT, MSG_LEGACY_LOW,
         "VS_PROFILING:VSS_ISTREAM_CMD_SET_MEDIA_TYPE(): vs_cmd_ticks_in_us=(%lu)",
         vs_cmd_ticks_in_us );
#endif /* VS_PROFILING */

  session_obj->cmd_ctrl.last_cmd = vs_adsp_last_cmd_sent = VSS_ISTREAM_CMD_SET_MEDIA_TYPE;
  rc = __aprv2_cmd_alloc_send(
         vs_apr_handle, APRV2_PKT_MSGTYPE_SEQCMD_V,
         vs_my_addr, ( (uint16_t) session_obj->header.handle ),
         vs_cvs_addr, session_obj->cvs_port,
...
)
{
  ...
  vs_cmd_ticks_in_us = (uint64_t)((qurt_sysclock_get_hw_ticks()) *10ull/192ull);
  MSG_1( MSG_SSID_DFLT, MSG_LEGACY_LOW,
         "VS_PROFILING:VSS_IMVM_CMD_SET_CAL_NETWORK(): vs_cmd_ticks_in_us=(%lu)",
         vs_cmd_ticks_in_us );
#endif /* VS_PROFILING */

  session_obj->cmd_ctrl.last_cmd = vs_adsp_last_cmd_sent =  VSS_IMVM_CMD_SET_CAL_NETWORK;
  rc = __aprv2_cmd_alloc_send(
         vs_apr_handle, APRV2_PKT_MSGTYPE_SEQCMD_V,
         vs_my_addr, ( (uint16_t) session_obj->header.handle ),
         vs_mvm_addr, session_obj->mvm_port,
...
)
{
  ...
  vs_cmd_ticks_in_us = (uint64_t)((qurt_sysclock_get_hw_ticks()) *10ull/192ull);
  MSG_1( MSG_SSID_DFLT, MSG_LEGACY_LOW,
         "VS_PROFILING:VSS_IMVM_CMD_SET_CAL_MEDIA_TYPE(): vs_cmd_ticks_in_us=(%lu)",
         vs_cmd_ticks_in_us );
#endif /* VS_PROFILING */

  session_obj->cmd_ctrl.last_cmd = vs_adsp_last_cmd_sent = VSS_IMVM_CMD_SET_CAL_MEDIA_TYPE;
  rc = __aprv2_cmd_alloc_send(
         vs_apr_handle, APRV2_PKT_MSGTYPE_SEQCMD_V,
         vs_my_addr, ( (uint16_t) session_obj->header.handle ),
         vs_mvm_addr, session_obj->mvm_port,
...
)
{
  ...
  vs_cmd_ticks_in_us = (uint64_t)((qurt_sysclock_get_hw_ticks()) *10ull/192ull);
  MSG_1( MSG_SSID_DFLT, MSG_LEGACY_LOW,
         "VS_PROFILING:VSS_ICOMMON_CMD_SET_VOICE_TIMING_V2(): vs_cmd_ticks_in_us=(%lu)",
         vs_cmd_ticks_in_us );
#endif /* VS_PROFILING */

  session_obj->cmd_ctrl.last_cmd = vs_adsp_last_cmd_sent = VSS_ICOMMON_CMD_SET_VOICE_TIMING_V2;
  rc = __aprv2_cmd_alloc_send(
         vs_apr_handle, APRV2_PKT_MSGTYPE_SEQCMD_V,
         vs_my_addr, ( (uint16_t) session_obj->header.handle ),
         vs_mvm_addr, session_obj->mvm_port,
...
)
{
  ...
  vs_cmd_ticks_in_us = (uint64_t)((qurt_sysclock_get_hw_ticks()) *10ull/192ull);
  MSG_1( MSG_SSID_DFLT, MSG_LEGACY_LOW,
         "VS_PROFILING:VSS_ICOMMON_CMD_SET_VOICE_TIMING_REFERENCE(): vs_cmd_ticks_in_us=(%lu)",
         vs_cmd_ticks_in_us );
#endif /* VS_PROFILING */

  MSG_4( MSG_SSID_DFLT, MSG_LEGACY_MED,
         "VS: TIMING_REFERENCES: handle[0x%08x], tx_ref_timestamp_us[0x%08x], rx_ref_timestamp_us[0x%08x], avtimerOffsetUs[0x%08x] ",
          session_obj->header.handle, voiceTimingReference.tx_ref_timstamp_us, voiceTimingReference.rx_ref_timstamp_us, session_obj->vocoder.timing_config.avtimerOffsetUs );

  session_obj->cmd_ctrl.last_cmd = vs_adsp_last_cmd_sent = VSS_ICOMMON_CMD_SET_VOICE_TIMING_REFERENCE;
  rc = __aprv2_cmd_alloc_send(
         vs_apr_handle, APRV2_PKT_MSGTYPE_SEQCMD_V,
         vs_my_addr, ( (uint16_t) session_obj->header.handle ),
         vs_mvm_addr, session_obj->mvm_port,
...
)
{
  int32_t rc = APR_EOK;
  vs_simple_job_object_t* job_obj = NULL;
  vss_icommon_cmd_set_voice_timing_reference_v2_t voiceTimingReference;
  ...
  voiceTimingReference.frame_type_ref_timestamp_us = 
   session_obj->vocoder.timing_config.frame_type_ref_timestamp_us +
   session_obj->vocoder.timing_config.avtimerOffsetUs;

  voiceTimingReference.enc_packet_ready_margin_us = 
   session_obj->vocoder.timing_config.enc_packet_ready_margin_us;

  MSG_6( MSG_SSID_DFLT, MSG_LEGACY_MED,
         "VS: TIMING_REFERENCES: handle[0x%08x], avtimerOffsetUs[0x%08x], tx_ref_timestamp_us[0x%08x], rx_ref_timestamp_us[0x%08x],"
          "frame_type_ref_timestamp_us [0x%08x], enc_packet_ready_margin_us [%d]",
          session_obj->header.handle, session_obj->vocoder.timing_config.avtimerOffsetUs,voiceTimingReference.tx_ref_timstamp_us, 
          voiceTimingReference.rx_ref_timstamp_us, voiceTimingReference.frame_type_ref_timestamp_us, voiceTimingReference.enc_packet_ready_margin_us );

  session_obj->cmd_ctrl.last_cmd = vs_adsp_last_cmd_sent = VSS_ICOMMON_CMD_SET_VOICE_TIMING_REFERENCE_V2;
  rc = __aprv2_cmd_alloc_send(
         vs_apr_handle, APRV2_PKT_MSGTYPE_SEQCMD_V,
         vs_my_addr, ( (uint16_t) session_obj->header.handle ),
         vs_mvm_addr, session_obj->mvm_port,
         job_obj->header.handle, VSS_ICOMMON_CMD_SET_VOICE_TIMING_REFERENCE_V2,
         &voiceTimingReference, sizeof( voiceTimingReference ) );
  if ( rc )
  {
    VS_REPORT_COMMAND_SEND_FAILED(vs_adsp_last_cmd_sent, rc);
...
)
{
  int32_t rc = APR_EOK;
  vs_simple_job_object_t* job_obj = NULL;
  vss_icommon_cmd_timeline_resync_t voiceTimingResync;

  rc = vs_create_simple_job_object( session_obj->header.handle, &job_obj );
  job_obj->fn_table[ VS_RESPONSE_FN_ENUM_RESULT ] = vs_voc_transition_result_rsp_fn;
  session_obj->session_ctrl.transition_job_handle = job_obj->header.handle;

  voiceTimingResync.resync_type = session_obj->vocoder.timing_config.resync_type;
  voiceTimingResync.network_id = session_obj->vocoder.client_id;
  //voiceTimingResync.vfr_mode = session_obj->vocoder.vfr_config.vfr_mode;
  //voiceTimingResync.vfr_cycle_us = session_obj->vocoder.vfr_config.vfr_cycle_duration_ms*1000;
  /* Offset timing references w.r.t to avtimer. */
  voiceTimingResync.tx_ref_timestamp_us = 
    session_obj->vocoder.timing_config.tx_ref_timestamp_us +
    session_obj->vocoder.timing_config.avtimerOffsetUs;

  voiceTimingResync.rx_ref_timestamp_us = 
   session_obj->vocoder.timing_config.rx_ref_timestamp_us +
   session_obj->vocoder.timing_config.avtimerOffsetUs;

  voiceTimingResync.frame_type_ref_timestamp_us = 
   session_obj->vocoder.timing_config.frame_type_ref_timestamp_us +
   session_obj->vocoder.timing_config.avtimerOffsetUs;

  voiceTimingResync.enc_packet_ready_margin_us = 
   session_obj->vocoder.timing_config.enc_packet_ready_margin_us;

  MSG_6( MSG_SSID_DFLT, MSG_LEGACY_MED,
         "VS: TIMING_RESYNC: handle[0x%08x], resync_type[%d], tx_ref_timestamp_us[0x%08x], rx_ref_timestamp_us[0x%08x],"
          "frame_type_ref_timestamp_us [0x%08x], enc_packet_ready_margin_us [%d]",
          session_obj->header.handle, session_obj->vocoder.timing_config.resync_type,voiceTimingResync.tx_ref_timestamp_us, 
          voiceTimingResync.rx_ref_timestamp_us, voiceTimingResync.frame_type_ref_timestamp_us, voiceTimingResync.enc_packet_ready_margin_us );

  session_obj->cmd_ctrl.last_cmd = vs_adsp_last_cmd_sent = VSS_ICOMMON_CMD_SET_VOICE_TIMELINE_RESYNC;
  rc = __aprv2_cmd_alloc_send(
         vs_apr_handle, APRV2_PKT_MSGTYPE_SEQCMD_V,
         vs_my_addr, ( (uint16_t) session_obj->header.handle ),
         vs_mvm_addr, session_obj->mvm_port,
         job_obj->header.handle, VSS_ICOMMON_CMD_SET_VOICE_TIMELINE_RESYNC,
         &voiceTimingResync, sizeof(voiceTimingResync) );
  if ( rc )
  {
    VS_REPORT_COMMAND_SEND_FAILED(vs_adsp_last_cmd_sent, rc);
    session_obj->session_ctrl.status |= rc;
...
)
{
  ...
  vs_cmd_ticks_in_us = (uint64_t)((qurt_sysclock_get_hw_ticks()) *10ull/192ull);
  MSG_1( MSG_SSID_DFLT, MSG_LEGACY_LOW,
         "VS_PROFILING:VSS_ICOMMON_CMD_SET_VFR_CONFIG(): vs_cmd_ticks_in_us=(%lu)",
         vs_cmd_ticks_in_us );
#endif /* VS_PROFILING */

  session_obj->cmd_ctrl.last_cmd = vs_adsp_last_cmd_sent = VSS_ICOMMON_CMD_SET_VFR_CONFIG;
  rc = __aprv2_cmd_alloc_send(
         vs_apr_handle, APRV2_PKT_MSGTYPE_SEQCMD_V,
         vs_my_addr, ( (uint16_t) session_obj->header.handle ),
         vs_mvm_addr, session_obj->mvm_port,
...
)
{
  ...
  vs_cmd_ticks_in_us = (uint64_t)((qurt_sysclock_get_hw_ticks()) *10ull/192ull);
  MSG_1( MSG_SSID_DFLT, MSG_LEGACY_LOW,
         "VS_PROFILING:VSS_ISTREAM_CMD_RESYNC_CTM(): vs_cmd_ticks_in_us=(%lu)",
         vs_cmd_ticks_in_us );
#endif /* VS_PROFILING */

  session_obj->cmd_ctrl.last_cmd = vs_adsp_last_cmd_sent = VSS_ISTREAM_CMD_RESYNC_CTM;
  rc = __aprv2_cmd_alloc_send(
         vs_apr_handle, APRV2_PKT_MSGTYPE_SEQCMD_V,
         vs_my_addr, ( (uint16_t) session_obj->header.handle ),
         vs_cvs_addr, session_obj->cvs_port,
...
)
{
  ...
  vs_cmd_ticks_in_us = (uint64_t)((qurt_sysclock_get_hw_ticks()) *10ull/192ull);
  MSG_1( MSG_SSID_DFLT, MSG_LEGACY_LOW,
         "VS_PROFILING:VSS_IMVM_CMD_MODEM_START_VOICE(): vs_cmd_ticks_in_us=(%lu)",
         vs_cmd_ticks_in_us );
#endif /* VS_PROFILING */

  session_obj->cmd_ctrl.last_cmd = vs_adsp_last_cmd_sent = VSS_IMVM_CMD_MODEM_START_VOICE;
  rc = __aprv2_cmd_alloc_send(
         vs_apr_handle, APRV2_PKT_MSGTYPE_SEQCMD_V,
         vs_my_addr, ( (uint16_t) session_obj->header.handle ),
         vs_mvm_addr, session_obj->mvm_port,
...
)
{
  ...
  vs_cmd_ticks_in_us = (uint64_t)((qurt_sysclock_get_hw_ticks()) *10ull/192ull);
  MSG_1( MSG_SSID_DFLT, MSG_LEGACY_LOW,
         "VS_PROFILING:VSS_IMVM_CMD_MODEM_STOP_VOICE(): vs_cmd_ticks_in_us=(%lu)",
         vs_cmd_ticks_in_us );
#endif /* VS_PROFILING */

  session_obj->cmd_ctrl.last_cmd = vs_adsp_last_cmd_sent = VSS_IMVM_CMD_MODEM_STOP_VOICE;
  rc = __aprv2_cmd_alloc_send(
         vs_apr_handle, APRV2_PKT_MSGTYPE_SEQCMD_V,
         vs_my_addr, ( (uint16_t) session_obj->header.handle ),
         vs_mvm_addr, session_obj->mvm_port,
...
)
{
  ...
  vs_cmd_ticks_in_us = (uint64_t)((qurt_sysclock_get_hw_ticks()) *10ull/192ull);
  MSG_1( MSG_SSID_DFLT, MSG_LEGACY_LOW,
         "VS_PROFILING:VSS_ISTREAM_CMD_SET_VOC_PARAM(PLC): vs_cmd_ticks_in_us=(%lu)",
         vs_cmd_ticks_in_us );
#endif /* VS_PROFILING */
...
uint32_t vs_voc_action_set_mute ( 
  vs_session_object_t* session_obj,
  vs_param_mute_stream_t* param
)
{
  uint32_t rc =VS_EOK;
  vss_ivolume_cmd_mute_v2_t mute = *(vss_ivolume_cmd_mute_v2_t* )param;

#if defined (VS_PROFILING)
  vs_cmd_ticks_in_us = (uint64_t)((qurt_sysclock_get_hw_ticks()) *10ull/192ull);
  MSG_1( MSG_SSID_DFLT, MSG_LEGACY_LOW,
         "VS_PROFILING:VSS_IVOLUME_CMD_MUTE_V2: vs_cmd_ticks_in_us=(%lu)",
         vs_cmd_ticks_in_us );
#endif /* VS_PROFILING */
  ...
  rc = __aprv2_cmd_alloc_send(
         ...
         vs_my_addr, ( (uint16_t) session_obj->header.handle ),
         vs_cvs_addr, session_obj->cvs_port,
         session_obj->header.handle, VSS_IVOLUME_CMD_MUTE_V2,
         &mute, sizeof( mute ) );
  VS_REPORT_FATAL_ON_ERROR(rc);
...
)
{
  ...
  vs_cmd_ticks_in_us = (uint64_t)((qurt_sysclock_get_hw_ticks()) *10ull/192ull);
  MSG_1( MSG_SSID_DFLT, MSG_LEGACY_LOW,
         "VS_PROFILING:VSS_ISTREAM_CMD_SET_ENC_DTX_MODE(DTX): vs_cmd_ticks_in_us=(%lu)",
         vs_cmd_ticks_in_us );
#endif /* VS_PROFILING */
...
)
{
                     ...
                     cvd_version, sizeof(cvd_version)) >=0 )
  {
    ...
    if( VSS_MEDIA_ID_EVS == session_obj->vocoder.media_id )
    {
      ...
      if( VS_CLIENT_STATE_ENABLED == session_obj->hpcm.client_state )
      {
         EvsparamData->dtx_mode = 0; //hardcode for hpcm enabled case
      }
      else
      {
    ...
    else
    {
      ...
      if( VS_CLIENT_STATE_ENABLED == session_obj->hpcm.client_state )
      {
         *paramData = 0; //hardcode for hpcm enabled case
      }
      else
      {
...
)
{
  ...
  vs_cmd_ticks_in_us = (uint64_t)((qurt_sysclock_get_hw_ticks()) *10ull/192ull);
  MSG_1( MSG_SSID_DFLT, MSG_LEGACY_LOW,
         "VS_PROFILING:VSS_ISTREAM_CMD_CDMA_SET_ENC_MINMAX_RATE(): vs_cmd_ticks_in_us=(%lu)",
         vs_cmd_ticks_in_us );
#endif /* VS_PROFILING */
...
)
{
  ...
  vs_cmd_ticks_in_us = (uint64_t)((qurt_sysclock_get_hw_ticks()) *10ull/192ull);
  MSG_1( MSG_SSID_DFLT, MSG_LEGACY_LOW,
         "VS_PROFILING:VSS_IMVM_CMD_SET_MAX_VAR_VOC_SAMPLING_RATE(): vs_cmd_ticks_in_us=(%lu)",
         vs_cmd_ticks_in_us );
#endif /* VS_PROFILING */

  session_obj->cmd_ctrl.last_cmd = vs_adsp_last_cmd_sent = VSS_IMVM_CMD_SET_MAX_VAR_VOC_SAMPLING_RATE;
  rc = __aprv2_cmd_alloc_send(
     vs_apr_handle, APRV2_PKT_MSGTYPE_SEQCMD_V,
     vs_my_addr, ( (uint16_t) session_obj->header.handle ),
     vs_mvm_addr, session_obj->mvm_port,
...
)
{
  ...
  vs_cmd_ticks_in_us = (uint64_t)((qurt_sysclock_get_hw_ticks()) *10ull/192ull);
  MSG_1( MSG_SSID_DFLT, MSG_LEGACY_LOW,
         "VS_PROFILING:VSS_ITTYOOB_CMD_REGISTER(): vs_cmd_ticks_in_us=(%lu)",
         vs_cmd_ticks_in_us );
#endif /* VS_PROFILING */

  session_obj->cmd_ctrl.last_cmd = vs_adsp_last_cmd_sent = VSS_ITTYOOB_CMD_REGISTER;
  rc = __aprv2_cmd_alloc_send(
     vs_apr_handle, APRV2_PKT_MSGTYPE_SEQCMD_V,
     vs_my_addr, ( (uint16_t) session_obj->header.handle ),
     vs_cvs_addr, session_obj->cvs_port,
...
)
{
  ...
  vs_cmd_ticks_in_us = (uint64_t)((qurt_sysclock_get_hw_ticks()) *10ull/192ull);
  MSG_1( MSG_SSID_DFLT, MSG_LEGACY_LOW,
         "VS_PROFILING:VSS_ITTYOOB_CMD_DEREGISTER(): vs_cmd_ticks_in_us=(%lu)",
         vs_cmd_ticks_in_us );
#endif /* VS_PROFILING */

  session_obj->cmd_ctrl.last_cmd = vs_adsp_last_cmd_sent = VSS_ITTYOOB_CMD_DEREGISTER;
  rc = __aprv2_cmd_alloc_send(
         vs_apr_handle, APRV2_PKT_MSGTYPE_SEQCMD_V,
         vs_my_addr, ( (uint16_t) session_obj->header.handle ),
         vs_cvs_addr, session_obj->cvs_port,
...
)
{
  ...
  MSG_3( MSG_SSID_DFLT, MSG_LEGACY_LOW,
         "VS_PROFILING-PCIe: MAP_MEMORY(): write memmap_table: MODEM_to_ADSP: begin_ticks_us=(%lu), pcie_rtt_us=(%lu), size=(%d)",
         vs_data_begin_ticks_in_us,(vs_data_end_ticks_in_us - vs_data_begin_ticks_in_us), req.size );
#endif /* VS_PROFILING_DATA_PCIE */
  ...
  vs_cmd_ticks_in_us = (uint64_t)((qurt_sysclock_get_hw_ticks()) *10ull/192ull);
  MSG_1( MSG_SSID_DFLT, MSG_LEGACY_LOW,
         "VS_PROFILING:VSS_IMEMORY_CMD_MAP_PHYSICAL(): vs_cmd_ticks_in_us=(%lu)",
         vs_cmd_ticks_in_us );
#endif /* VS_PROFILING */

  session_obj->cmd_ctrl.last_cmd = vs_adsp_last_cmd_sent = opcode;
  rc = __aprv2_cmd_alloc_send(
         vs_apr_handle, APRV2_PKT_MSGTYPE_SEQCMD_V,
         vs_my_addr, ( (uint16_t) session_obj->header.handle ),
         vs_mvm_addr, APR_NULL_V,
...
)
{
  ...
  MSG( MSG_SSID_DFLT, MSG_LEGACY_LOW,
         "vs_voc_action_unmap_memory() memmap_handle set to NULL");
  ...
  session_obj->cmd_ctrl.last_cmd = vs_adsp_last_cmd_sent =  VSS_IMEMORY_CMD_UNMAP;
  rc = __aprv2_cmd_alloc_send(
         vs_apr_handle, APRV2_PKT_MSGTYPE_SEQCMD_V,
         vs_my_addr, ( (uint16_t) session_obj->header.handle ),
         vs_mvm_addr, APR_NULL_V,
...
)
{
  ...
  vs_cmd_ticks_in_us = (uint64_t)((qurt_sysclock_get_hw_ticks()) *10ull/192ull);
  MSG_1( MSG_SSID_DFLT, MSG_LEGACY_LOW,
         "VS_PROFILING:VSS_IAVTIMER_CMD_GET_OFFSET(): vs_cmd_ticks_in_us=(%lu)",
         vs_cmd_ticks_in_us );
#endif /* VS_PROFILING */

  session_obj->cmd_ctrl.last_cmd = vs_adsp_last_cmd_sent = VSS_IAVTIMER_CMD_GET_OFFSET;
  rc = __aprv2_cmd_alloc_send(
     vs_apr_handle, APRV2_PKT_MSGTYPE_SEQCMD_V,
     vs_my_addr, ( (uint16_t) session_obj->header.handle ),
     vs_mvm_addr, APR_NULL_V,
...
)
{
  ...
  session_obj->cmd_ctrl.last_cmd = vs_adsp_last_cmd_sent = VSS_IVPCM_CMD_START_V2;
  rc = __aprv2_cmd_alloc_ext(
         vs_apr_handle, APRV2_PKT_MSGTYPE_SEQCMD_V,
         vs_my_addr, ( (uint16_t) session_obj->header.handle ),
         vs_cvs_addr, session_obj->cvs_port,
...
)
{
  ...
  MSG_1( MSG_SSID_DFLT, MSG_LEGACY_LOW,
         ...
         vs_cmd_ticks_in_us );
#endif /* VS_PROFILING */
  // clear state only if hpcm disable cmd issued by client
  if( VS_CLIENT_STATE_DISABLED == session_obj->hpcm.client_state )
  {
     session_obj->hpcm.txMode = session_obj->hpcm.rxMode = VS_PARAM_UINT8_NOT_SET; 
  }
  session_obj->cmd_ctrl.last_cmd = vs_adsp_last_cmd_sent = VSS_IVPCM_CMD_STOP;
  rc = __aprv2_cmd_alloc_send(
         vs_apr_handle, APRV2_PKT_MSGTYPE_SEQCMD_V,
         vs_my_addr, ( (uint16_t) session_obj->header.handle ),
         vs_cvs_addr, session_obj->cvs_port,
...
)
{
  ...
  vs_cmd_ticks_in_us = (uint64_t)((qurt_sysclock_get_hw_ticks()) *10ull/192ull);
  MSG_1( MSG_SSID_DFLT, MSG_LEGACY_LOW,
         "VS_PROFILING:VSS_IPKTEXG_CMD_SET_MODE(): vs_cmd_ticks_in_us=(%lu)",
         vs_cmd_ticks_in_us );
#endif /* VS_PROFILING */

  session_obj->cmd_ctrl.last_cmd = vs_adsp_last_cmd_sent = VSS_IPKTEXG_CMD_SET_MODE;
  rc = __aprv2_cmd_alloc_send(
         vs_apr_handle, APRV2_PKT_MSGTYPE_SEQCMD_V,
         vs_my_addr, ( (uint16_t) session_obj->header.handle ),
         vs_cvs_addr, session_obj->cvs_port,
...
)
{
  ...
  vs_cmd_ticks_in_us = (uint64_t)((qurt_sysclock_get_hw_ticks()) *10ull/192ull);
  MSG_1( MSG_SSID_DFLT, MSG_LEGACY_LOW,
         "VS_PROFILING:VSS_IPKTEXG_CMD_MAILBOX_SET_CONFIG(): vs_cmd_ticks_in_us=(%lu)",
         vs_cmd_ticks_in_us );
#endif /* VS_PROFILING */

  session_obj->cmd_ctrl.last_cmd = vs_adsp_last_cmd_sent = VSS_IPKTEXG_CMD_MAILBOX_SET_CONFIG;
  rc = __aprv2_cmd_alloc_send(
         vs_apr_handle, APRV2_PKT_MSGTYPE_SEQCMD_V,
         vs_my_addr, ( (uint16_t) session_obj->header.handle ),
         vs_cvs_addr, session_obj->cvs_port,
...
)
{
...
#if defined (VS_PROFILING_DATA_PCIE)
   uint64_t vs_data_begin_ticks1 = (uint64_t)((qurt_sysclock_get_hw_ticks()) *10ull/192ull);
#endif /* VS_PROFILING_DATA_PCIE */

    ipa_dma_device_to_host( session_obj->ipa_ul_handle, &req);

#if defined (VS_PROFILING_DATA_PCIE)
    uint64_t vs_data_end_ticks1 = (uint64_t)((qurt_sysclock_get_hw_ticks()) *10ull/192ull);
    uint32_t enc_req_size = req.size;
  ...
  vs_data_end_ticks_in_us = (uint64_t)((qurt_sysclock_get_hw_ticks()) *10ull/192ull);
    MSG_6( MSG_SSID_DFLT, MSG_LEGACY_LOW,
         "VS_PROFILING-PCIe: PUT_ENC_REQUEST(): write encoding request: MODEM_to_ADSP: begin_ticks_us=(%lu), pcie_rtt_us=(%lu), size=(%d)," 
		 "write_offset update: begin_ticks_us=(%lu), pcie_rtt_us=(%lu), size=(%d)",
         vs_data_begin_ticks1, (vs_data_end_ticks1 - vs_data_begin_ticks1), enc_req_size,
		 vs_data_begin_ticks_in_us, (vs_data_end_ticks_in_us - vs_data_begin_ticks_in_us), req.size );
#endif /* VS_PROFILING_DATA_PCIE */
...
)
{
  ...
  ipa_dma_req_t req;
#if defined (VS_PROFILING_DATA_PCIE)
  uint64_t vs_data_begin_ticks1, vs_data_end_ticks1;
  uint32_t read_offset_size;
#endif
#endif  
  voicemem_cmd_get_iova_t params;
...
#if defined (VS_PROFILING_DATA_PCIE)
    vs_data_begin_ticks1 = (uint64_t)((qurt_sysclock_get_hw_ticks()) *10ull/192ull);
#endif /* VS_PROFILING_DATA_PCIE */

    ipa_dma_host_to_device( session_obj->ipa_ul_handle, &req);

#if defined (VS_PROFILING_DATA_PCIE)
    vs_data_end_ticks1 = (uint64_t)((qurt_sysclock_get_hw_ticks()) *10ull/192ull);
    read_offset_size = req.size;
  ...
  vs_data_end_ticks_in_us = (uint64_t)((qurt_sysclock_get_hw_ticks()) *10ull/192ull);
      MSG_6( MSG_SSID_DFLT, MSG_LEGACY_LOW,
         "VS_PROFILING-PCIe: READ_ENC_MAILBOX(): read encoder_packet: ADSP_to_MODEM: begin_ticks_us=(%lu), pcie_rtt_us=(%lu), size=(%d), "
		 " read_offset update : begin_ticks_us=(%lu), pcie_rtt_us=(%lu), size=(%d)",
         vs_data_begin_ticks_in_us, (vs_data_end_ticks_in_us - vs_data_begin_ticks_in_us), req.size,
	     vs_data_begin_ticks1, (vs_data_end_ticks1 - vs_data_begin_ticks1), read_offset_size );
#endif /* VS_PROFILING_DATA_PCIE */
...
)
{
...
#if defined (VS_PROFILING_DATA_PCIE)
    uint64_t vs_data_end_ticks1 = (uint64_t)((qurt_sysclock_get_hw_ticks()) *10ull/192ull);
	uint32_t pkt_buf_size = req.size;
#endif /* VS_PROFILING_DATA_PCIE */
...
#if defined (VS_PROFILING_DATA_PCIE)
    uint64_t vs_data_begin_ticks2 = (uint64_t)((qurt_sysclock_get_hw_ticks()) *10ull/192ull);
#endif /* VS_PROFILING_DATA_PCIE */

    ipa_dma_device_to_host( session_obj->ipa_dl_handle, &req );  

#if defined (VS_PROFILING_DATA_PCIE)
    uint64_t vs_data_end_ticks2 = (uint64_t)((qurt_sysclock_get_hw_ticks()) *10ull/192ull);
	uint32_t pkt_req_size = req.size;
  ...
  vs_data_end_ticks_in_us = (uint64_t)((qurt_sysclock_get_hw_ticks()) *10ull/192ull);
    MSG_9( MSG_SSID_DFLT, MSG_LEGACY_LOW,
         "VS_PROFILING-PCIe: PUT_DEC_REQUEST(): write decoder_packet: MODEM_to_ADSP: begin_ticks_us=(%lu), pcie_rtt_us=(%lu), size=(%d),"
		 "dec_pkt_req update : begin_ticks_us=(%lu), pcie_rtt_us=(%lu), size=(%d),"
		 "write_offset update : begin_ticks_us=(%lu), pcie_rtt_us=(%lu), size=(%d)",
		 vs_data_begin_ticks1, (vs_data_end_ticks1 - vs_data_begin_ticks1), pkt_buf_size, 
		 vs_data_begin_ticks2, (vs_data_end_ticks2 - vs_data_begin_ticks2), pkt_req_size,
         vs_data_begin_ticks_in_us, (vs_data_end_ticks_in_us - vs_data_begin_ticks_in_us), req.size );
#endif /* VS_PROFILING_DATA_PCIE */
...
)
{
...
#if VS_PROFILING
  MSG_4( MSG_SSID_DFLT, MSG_LEGACY_LOW,
         "VS: FLUSH_ENC_MBOX: encCircBuf:[0x%08x] readOffset_before[%d] writeOffset[%d] reclaimOffset[%d]",
         encCircBuf, (uint32_t)encCircBuf->read_offset, (uint32_t) encCircBuf->write_offset, 
         (uint32_t)session_obj->vocoder.mbox_config.encReclaimOffset );
#endif /* VS_PROFILING */
  ...
  vs_data_end_ticks_in_us = (uint64_t)((qurt_sysclock_get_hw_ticks()) *10ull/192ull);
    MSG_3( MSG_SSID_DFLT, MSG_LEGACY_LOW,
         "VS_PROFILING-PCIe: FLUSH_ENC_MAILBOX(): read encCircBuf->read_offset: ADSP_to_Modem: begin_ticks_us=(%lu), pcie_rtt_us=(%lu), size=(%d)",
         vs_data_begin_ticks_in_us,(vs_data_end_ticks_in_us - vs_data_begin_ticks_in_us), req.size );
#endif /* VS_PROFILING_DATA_PCIE */
...
)
{
...
#if defined (VS_PROFILING_DATA_PCIE)
  uint32_t read_offset = (uint32_t)decCircBuf->read_offset;
  uint32_t reclaim_offset = (uint32_t)session_obj->vocoder.mbox_config.decReclaimOffset; 
  ...
  vs_data_end_ticks_in_us = (uint64_t)((qurt_sysclock_get_hw_ticks()) *10ull/192ull);
    MSG_6( MSG_SSID_DFLT, MSG_LEGACY_LOW,
         "VS: FLUSH_DEC_MBOX: decCircBuf:[0x%08x] readOffset[%d] writeOffset[%d] reclaimOffset[%d] pcie_rtt_read_us = (%lu), size(%d)",
         decCircBuf, read_offset, (uint32_t) decCircBuf->write_offset, 
         reclaim_offset, (vs_data_end_ticks_in_us - vs_data_begin_ticks_in_us), req.size );
...
)
{
  ...
  if ( TRUE == vs_platform_is_fusion )
  {
    ...
    vs_data_end_ticks_in_us = (uint64_t)((qurt_sysclock_get_hw_ticks()) *10ull/192ull);
    MSG_3( MSG_SSID_DFLT, MSG_LEGACY_LOW,
           "VS_PROFILING-PCIe: Initialize_mailobx(): write memmap_table: MODEM_to_ADSP: begin_ticks_us=(%lu), pcie_rtt_us=(%lu), size=(%d)",
           vs_data_begin_ticks_in_us,(vs_data_end_ticks_in_us - vs_data_begin_ticks_in_us), req.size );
#endif 
...
)
{
  ...
  session_obj->cmd_ctrl.last_cmd = vs_adsp_last_cmd_sent = VSS_IVERSION_CMD_GET;
  rc = __aprv2_cmd_alloc_send(
     vs_apr_handle, APRV2_PKT_MSGTYPE_SEQCMD_V,
     vs_my_addr, ( (uint16_t) session_obj->header.handle ),
     vs_mvm_addr, APR_NULL_V,
...
uint32_t vs_voc_action_pktexg_event_enable (
  vs_session_object_t* session_obj
)
{
  int32_t rc = APR_EOK;
  vs_simple_job_object_t* job_obj = NULL;
  vss_istream_cmd_set_pktexg_evt_enable_t pktexg_evt;
  
  if ( FALSE == session_obj->vocoder.pktexg_event_enable )
  {
    MSG( MSG_SSID_DFLT, MSG_LEGACY_MED,
         "vs_voc_action_pktexg_event_enable(): Event based packet exchange is not enabled. Returning.." );
    session_obj->session_ctrl.status = VS_EOK;
    return VS_EIMMEDIATE;
  }
  ...
  pktexg_evt.enable_flag = session_obj->vocoder.pktexg_event_enable;

  session_obj->cmd_ctrl.last_cmd = vs_adsp_last_cmd_sent = VSS_ISTREAM_CMD_SET_PKTEXG_EVT_ENABLE;
  rc = __aprv2_cmd_alloc_send(
         vs_apr_handle, APRV2_PKT_MSGTYPE_SEQCMD_V,
         vs_my_addr, ( (uint16_t) session_obj->header.handle ),
         vs_cvs_addr, session_obj->cvs_port,
         job_obj->header.handle, VSS_ISTREAM_CMD_SET_PKTEXG_EVT_ENABLE,
         &pktexg_evt, sizeof( pktexg_evt ) );
