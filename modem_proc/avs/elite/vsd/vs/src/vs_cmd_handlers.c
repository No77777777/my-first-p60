...
/*
  ...
  $Header: //components/rel/avs.mpss/9.0.c51/elite/vsd/vs/src/vs_cmd_handlers.c#1 $
  $Author: pwbldsvc $
*/
...
/* To keep track of ipa_dma_open, ipa_dma_close, ipa_vote, ipa_devote calls */
ipa_dma_client_type_e vs_ipa_dma_ul_client_type = IPA_DMA_CLIENT_TYPE_MAX;
ipa_dma_client_type_e vs_ipa_dma_dl_client_type = IPA_DMA_CLIENT_TYPE_MAX;
#endif
...
)
{
  ...
  if ( ( timing->resync_type < VSS_RESYNC_TX ) ||
       ( timing->resync_type > VSS_RESYNC_TX_RX )  )
  {
     MSG_1( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
        "SET_PARAM:VS_PARAM_VOC_TIMELINE_RESYNC: Invalid payload: resync_type=(%lu)",
         timing->resync_type);
     return VS_EBADPARAM;
  }
  ...
  if ( timing->resync_type & VSS_RESYNC_RX )
  {
 ...
 // session_obj->vocoder.vfr_config.vfr_mode = timing->vfr_mode;
  ...
  MSG_5( MSG_SSID_DFLT, MSG_LEGACY_HIGH,
        ...
        "frame_type_ref_timestamp_us=(%lu), enc_packet_ready_margin_us: (%d)",
         timing->resync_type ,timing->tx_ref_timestamp_us,timing->rx_ref_timestamp_us, 
         timing->frame_type_ref_timestamp_us,timing->enc_packet_ready_margin_us);
...
static uint32_t vs_voc_set_mute (
  vs_session_object_t* session_obj,
...
)
{
  ...
  MSG_2( MSG_SSID_DFLT, MSG_LEGACY_HIGH,
         "SET_PARAM: VS_PARAM_MUTE_STREAM media_id=(0x%08x), mute_flag=(%d) (0-Disable, 1-Enable)",
         session_obj->vocoder.media_id, param->mute_flag);

  if( TRUE == session_obj->is_stream_ready )
  {
    ( void ) vs_voc_action_set_mute( session_obj , param);
  }
    ...
    else
    {
      ...
      *params->ret_handle = session_obj->header.handle;
      ...
      if( vs_platform_is_fusion )
      {
        ...
        if ( IPA_DMA_CLIENT_TYPE_MAX == vs_ipa_dma_ul_client_type )
        {
          ipa_rc = ipa_dma_open(&ipa_param_ul);
          VS_PANIC_ON_ERROR(ipa_rc);
          session_obj->ipa_ul_handle = ipa_param_ul.ch_handle;
          vs_ipa_dma_ul_client_type = IPA_DMA_CLIENT_TYPE_AUDIO_UL;
        ...
        {
          MSG_1( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
                "VS: CMD_OPEN: ipa_dma_open/ipa_vote for AUDIO_UL already called. client_type=%d", vs_ipa_dma_ul_client_type );
        }
        ...
        if ( IPA_DMA_CLIENT_TYPE_MAX == vs_ipa_dma_dl_client_type )
        {
          ipa_rc = ipa_dma_open( &ipa_param_dl );
          VS_PANIC_ON_ERROR(ipa_rc);
          session_obj->ipa_dl_handle = ipa_param_dl.ch_handle;
          vs_ipa_dma_dl_client_type = IPA_DMA_CLIENT_TYPE_AUDIO_DL;
        ...
        {
          MSG_1( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
                "VS: CMD_OPEN: ipa_dma_open/ipa_vote for AUDIO_DL already called. client_type=%d", vs_ipa_dma_dl_client_type );
        }
#endif
      }
      rc = VS_EOK;
  ...
  for ( ;; )
  {
    ...
    switch ( cmd_params->param_id )
    {
      ...
      case VS_PARAM_ENC_DTX:
        ...
        break;
      case VS_PARAM_MUTE_STREAM:
    	rc = vs_voc_set_mute( session_obj, (vs_param_mute_stream_t*) cmd_params->param_data );
	    break;
...
)
{
  ...
  for ( ;; )
  {
    ...
    if ( ctrl->state == VS_GATING_CMD_STATE_ENUM_EXECUTE )
    {
      ...
      else if( TRUE == vs_adsp_memmap_timeout_is_trigerred )
      {
        seq_obj->state = VS_HELPER_SEQUENCER_ENUM_GOAL_DESTROY;
        MSG_1( MSG_SSID_DFLT, MSG_LEGACY_HIGH,
               "VS: vs_adsp_memmap_timeout_is_trigerred is TRUE; going to GOAL_DESTROY, session_ctrl.state=(%d)", session_obj->session_ctrl.state );
      }
    }
...
)
{
  ...
  for ( ;; )
  {
    ...
    if ( ctrl->state == VS_GATING_CMD_STATE_ENUM_EXECUTE )
    {
      ...
      {
#if (!defined AVS_MPSS_TEST) 	
        if ( IPA_DMA_CLIENT_TYPE_AUDIO_UL == vs_ipa_dma_ul_client_type )
        {
          ipa_rc = ipa_devote(IPA_VOTE_CLIENT_TYPE_AUDIO_UL);
          VS_PANIC_ON_ERROR(ipa_rc);
          ipa_rc = ipa_dma_close(session_obj->ipa_ul_handle );
          VS_PANIC_ON_ERROR(ipa_rc);
          session_obj->ipa_ul_handle = IPA_DMA_CLIENT_TYPE_MAX;
          vs_ipa_dma_ul_client_type = IPA_DMA_CLIENT_TYPE_MAX;
        }
        else
        {
          MSG_1( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
                "VS: CMD_CLOSE: ipa_devote/ipa_dma_close for AUDIO_UL already called. client_type=%d", vs_ipa_dma_ul_client_type );
        }

        if ( IPA_DMA_CLIENT_TYPE_AUDIO_DL == vs_ipa_dma_dl_client_type )
        {
          ipa_rc = ipa_devote( IPA_VOTE_CLIENT_TYPE_AUDIO_DL );
          VS_PANIC_ON_ERROR(ipa_rc);
          ipa_rc = ipa_dma_close(session_obj->ipa_dl_handle );
          VS_PANIC_ON_ERROR(ipa_rc);
          session_obj->ipa_dl_handle = IPA_DMA_CLIENT_TYPE_MAX;
          vs_ipa_dma_dl_client_type = IPA_DMA_CLIENT_TYPE_MAX;
        }
        else
        {
          MSG_1( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
                "VS: CMD_CLOSE: ipa_devote/ipa_dma_close for AUDIO_DL already called. client_type=%d", vs_ipa_dma_dl_client_type );
        }
#endif
      }
    }
...
)
{
  ...
  for ( ;; )
  {
    ...
    for( index = 0; index < VS_MAX_NUM_SESSIONS_V; index++ )
    {
      ...
      session_obj->cmd_ctrl.last_cmd = 0;
...
)
{
  ...
  if( VS_HPCM_TAP_POINT_STREAM_RX == cmd_params->tap_point_id )
  {
    ...
    {
      if(cmd_params->write_buffer->data)
      {
	     mmstd_memcpy( ( void* )session_obj->hpcm.rxWriteBlk.phyAddr, session_obj->hpcm.rxWriteBuf.data_size,
           ( void* )cmd_params->write_buffer->data, cmd_params->write_buffer->data_size );
      }		    
  ...
  if( VS_HPCM_TAP_POINT_STREAM_TX == cmd_params->tap_point_id )
  {
    ...
    if( eventPayload.push_buf_mask & VSS_HPCM_MASK_WRITE_BUFFER )
    {
      if(cmd_params->write_buffer->data)
      {
         mmstd_memcpy( ( void* )session_obj->hpcm.txWriteBlk.phyAddr, session_obj->hpcm.txWriteBuf.data_size, 
           ( void* )cmd_params->write_buffer->data, cmd_params->write_buffer->data_size );	
      }
      cmdPayload.voicemem_handle = session_obj->voicemem_handle;
