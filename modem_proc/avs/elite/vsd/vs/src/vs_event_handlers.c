...
/*
  ...
  $Header: //components/rel/avs.mpss/9.0.c51/elite/vsd/vs/src/vs_event_handlers.c#1 $
  $Author: pwbldsvc $
*/
...
)
{
  ...
  {
    MSG_3( MSG_SSID_DFLT, MSG_LEGACY_HIGH,
           "VSS_IMVM_EVT_APPS_START received from: src_port[0x%08x], dst_port[0x%08x], token[0x%08x]",
            packet->src_port, packet->dst_port, packet->token );
    ...
    session_obj->is_apps_started = TRUE;

    if ( session_obj->session_ctrl.state == VS_VOC_STATE_ENUM_RUN  )
    {
       MSG( MSG_SSID_DFLT, MSG_LEGACY_HIGH,
           "vs_apps_start_event_handler(): Initializing mbox and setting config. ");
       ( void ) vs_voc_action_initialize_mbox( session_obj );
        rc = vs_voc_action_set_mbox_config( session_obj );
        if( rc ) 
        {
           MSG_1( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
               "vs_apps_start_event_handler(): could not set mailbox config. rc 0x%08x", rc);
        }
    }
...
)
{
  ...
  {
    MSG_3( MSG_SSID_DFLT, MSG_LEGACY_HIGH,
           "VSS_IMVM_EVT_APPS_STOP received from: src_port[0x%08x], dst_port[0x%08x], token[0x%08x]",
            packet->src_port, packet->dst_port, packet->token );
...
)
{
  ...
  {
    MSG_3( MSG_SSID_DFLT, MSG_LEGACY_HIGH,
           "VSS_IPKTEXG_EVT_NO_DATA_PACKET received from: src_port[0x%08x], dst_port[0x%08x], token[0x%08x]",
            packet->src_port, packet->dst_port, packet->token );
...
)
{
  uint32_t rc = VS_EOK;
  vs_session_object_t* session_obj = NULL;
  vss_icommon_event_resync_done_t* payload = NULL; 
  ...
  for ( ;; )
  {
    ...
    payload = APRV2_PKT_GET_PAYLOAD( vss_icommon_event_resync_done_t, packet );
    event_param.resync_type = payload->resync_type;
    event_param.status = payload->status;
	//TODO: add token as well in future
    MSG_2( MSG_SSID_DFLT, MSG_LEGACY_HIGH,
...
)
{
  ...
  for ( ;; )
  {
    ...
    if ( (rsp_result->opcode == VSS_ISTREAM_CMD_SET_PKTEXG_EVT_ENABLE) && 
	     (rsp_result->status == VS_EUNSUPPORTED) )
    {
	   simple_obj->status = VS_EOK;
  ...
  if( NULL != session_obj )
  {
    ...
    {
      session_obj->cmd_ctrl.cmd_cnt = session_obj->cmd_ctrl.rsp_cnt = 0;
      vs_signal_run();
    }
  }
...
)
{
  ...
  for ( ;; )
  {
    ...
    MSG_5( MSG_SSID_DFLT, MSG_LEGACY_MED,
             ...
             "mem_address_pcie[MSW] = 0x%08x, mem_address_pcie[LSW] = 0x%08x,  size_bytes[%d]",
