...
/* 
   ...
   $Header: //components/rel/avs.mpss/9.0.c51/elite/vsd/vs/src/vs.c#1 $
   $Author: pwbldsvc $
*/
...
unsigned long long vs_cmd_ticks_in_us_evt_accepted = 0;
#endif /* VS_PROFILING */
 ...
 */
apr_timer_t vs_adsp_command_timeout_timer;
bool_t vs_adsp_command_timeout_is_triggered;

apr_timer_t vs_adsp_memmap_timeout_timer;
bool_t vs_adsp_memmap_timeout_is_trigerred;
uint32_t vs_adsp_memmap_timeout_count = 0;

uint32_t vs_adsp_last_cmd_sent = 0;
 ...
 */
static void vs_adsp_command_timeout_handler (
 void* client_token
)
{

  MSG( MSG_SSID_DFLT, MSG_LEGACY_ERROR, "VS: adsp command timeout triggered !!!" );
  /* TBD: Remove this crash for external/production builds (needs discussion). */
  vs_adsp_command_timeout_is_triggered = TRUE;
  VS_PANIC_ON_ADSP_COMMAND_TIMEOUT(vs_adsp_last_cmd_sent, VS_EPANIC);
  return;

}

/**
 * Timer callback indicating that VSS_IMVM_EVT_APPS_START is not recived from ADSP.
 * In case of timeout, state machine will be completed without completing rest of the actions. 
 * This will make sure that call tear-down and subsequent call set up goes 
 * through without any errors. 
 */
static void vs_adsp_memmap_timeout_handler (
 void* client_token
)
{    

  vs_adsp_memmap_timeout_count++;
  MSG_1( MSG_SSID_DFLT, MSG_LEGACY_ERROR, "VS: adsp memmap timeout triggered !!!. count=(%d)", vs_adsp_memmap_timeout_count );
  vs_adsp_memmap_timeout_is_trigerred = TRUE;
  vs_signal_run();
  return;
...
)
{
  ...
  {
    vs_cmd_ticks_in_us_evt_accepted = (uint64_t)((qurt_sysclock_get_hw_ticks()) * 10ull / 192ull);
    MSG_3( MSG_SSID_DFLT, MSG_LEGACY_LOW,
           "VS_PROFILING: vs_isr_dispatch_fn : opcode=(0x%08x); vs_cmd_ticks_in_us_evt_accepted=(%lu), voice_cmd_pkt_rtt_us=(%lu)",
          packet->opcode, vs_cmd_ticks_in_us_evt_accepted, (vs_cmd_ticks_in_us_evt_accepted - vs_cmd_ticks_in_us) );
  }
  else
  {
    vs_cmd_ticks_in_us = (uint64_t)((qurt_sysclock_get_hw_ticks()) *10ull/192ull);
    MSG_2( MSG_SSID_DFLT, MSG_LEGACY_LOW,
         "VS_PROFILING: vs_isr_dispatch_fn : opcode=(0x%08x); vs_cmd_ticks_in_us=(%lu)",
...
)
{
  ...
  switch ( event_id )
  {
    ...
    {
      MSG_2( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
             "vs_common_send_event(event=0x%08x) is NOT handled. handle=0x%08x",
             event_id, handle );
    }
      break;
  }
...
)
{
  ...
  while( VS_EOK == apr_list_remove_head( &vs_work_pkt_q, (apr_list_node_t**)&work_item ) )
  {
    ...
    switch ( request_id )
    {
      ...
      case VSS_IMVM_EVT_APPS_START:
        rc = vs_apps_start_event_handler( packet );
        break;

      case VSS_IMVM_EVT_APPS_STOP:
        rc = vs_apps_stop_event_handler( packet );
        break;

      case VSS_IPKTEXG_EVT_NO_DATA_PACKET:
        rc = vs_stream_no_data_pkt_event_handler( packet );
        break;

      case VSS_ICOMMON_EVENT_RESYNC_DONE:
...
)
{
  ...
  rc = vs_ssr_pdr_init( );
#if !defined ( AVS_MPSS_TEST ) && !defined ( VS_AUDIOMN_CONFIG )
...
)
{
  ...
  vs_adsp_command_timeout_is_triggered = FALSE;
  rc = apr_timer_create( &vs_adsp_command_timeout_timer,
                         vs_adsp_command_timeout_handler, NULL );
  VS_PANIC_ON_ERROR(rc);

  vs_adsp_memmap_timeout_is_trigerred = FALSE;
  rc = apr_timer_create( &vs_adsp_memmap_timeout_timer,
                         vs_adsp_memmap_timeout_handler, NULL );
  VS_PANIC_ON_ERROR(rc);
...
)
{
  ...
  /** Destroy the timers. */
  ( void ) apr_timer_destroy( vs_adsp_command_timeout_timer );
  ( void ) apr_timer_destroy( vs_adsp_memmap_timeout_timer ); 
