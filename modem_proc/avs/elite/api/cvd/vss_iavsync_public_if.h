...
/*===========================================================================
  ...
  $Header: //components/rel/avs.mpss/9.0.c51/elite/api/cvd/vss_iavsync_public_if.h#1 $
  $Author: pwbldsvc $
...
/** @ingroup cvd_avsync_evtclass_tx
...
#include "apr_pack_end.h"
;
/** @} */ /* end_weakgroup weak_vss_iavsync_evt_tx_timestamp_t */

/** @ingroup cvd_avsync_evt_path_delay
  Notifies the full control client of changes in the value of the Tx and Rx
  voice path delay.

  @apr_msgpayload{vss_iavsync_evt_path_delay_t}
  @table{weak__vss__iavsync__evt__path__delay__t}

  @detdesc
  This event is sent to the full control client under the following
  circumstances:
   - Every time the vocoder is started
   - Subsequently when the value of either Tx or Rx path delay changes (for
     example: upon a device switch or when pre/postprocessing features are
     enabled or disabled)
...
/** @weakgroup weak_vss_iavsync_evt_path_delay_t
@{ */
