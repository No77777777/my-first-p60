...
=============================================================================*/
/*===========================================================================
Copyright (c) 2011, 2015-2016, 2019-2020 Qualcomm Technologies, Inc.  All rights reserved.
...
/*===========================================================================
  ...
  $Header: //components/rel/avs.mpss/9.0.c51/elite/api/cvd/vss_istream_public_if.h#1 $
  $Author: pwbldsvc $
...
@{ */
/** ID of the packet loss concealment (PLC) vocoder parameter.

  @msgpayload{vss_istream_param_plc_t}
  @table{weak__vss__istream__param__plc__t} @newpage
 */
...
{
  uint32_t plc;
    /**< Specifies whether PLC mode is enabled.

         @values
         - 0x00 -- Disable PLC
         - 0x01 -- Enable PLC @tablebulletend */
}
#include "apr_pack_end.h"
;
...
/** ID of the DTX parameter for the vocoder.

  @msgpayload{vss_istream_param_dtx_t}
  @table{weak__vss__istream__param__dtx__t}
*/
...
struct vss_istream_param_dtx_t
{
  uint32_t dtx;
    /**< Specifies whether DTX mode is enabled.
         ...
         @values
         - 0x00 -- Disable DTX
         - 0x01 -- Enable DTX @tablebulletend */
}
#include "apr_pack_end.h"
;
...
/** ID of DTX parameter version 2 for the vocoder.

  This version supports adaptive DTX, which is applicable only for EVS vocoders.

  @msgpayload{vss_istream_param_dtx_v2_t}
  @table{weak__vss__istream__param__dtx__v2__t} @newpage
*/
...
{
  uint32_t version;
    /**< Version of DTX parameter v2.

         @values
         - 0x01 -- Supports adaptive DTX for the EVS vocoder
            - Minimum number of SID frames: 8
            - Maximum number of SID frames: 50

         The maximum and minimum numbers of SID frames are not configurable. */
  ...
  uint32_t dtx_mode;
    /**< Specifies whether DTX mode is enabled.
         ...
         @values
         - 0x00 -- Disable DTX
         - 0x01 -- Enable DTX; the number of SID frames is fixed at 8
         - 0x02 -- Enable adaptive DTX (for EVS vocoders only)@tablebulletend */
}
#include "apr_pack_end.h"
;
...
{
   VSS_ISTREAM_DIRECTION_TX,     /**< Tx path only. */
   VSS_ISTREAM_DIRECTION_RX,     /**< Rx path only. */
   VSS_ISTREAM_DIRECTION_TX_RX   /**< Both Tx and Rx paths. */
...
/** @ingroup cvd_cvs_cmd_set_pktexg_evt_enable
  Enables the event driven mailbox packet exchange mode.
  This command is for full control stream clients only.

  @msgpayload{vss_istream_cmd_set_pktexg_evt_enable_t}
  @table{weak__vss__istream__cmd__set__pktexg__evt__enable_t}
  ...
  None.
*/
#define VSS_ISTREAM_CMD_SET_PKTEXG_EVT_ENABLE ( 0x000133BB )

/* Type definition for vss_istream_cmd_set_pktexg_evt_enable_t. */
typedef struct vss_istream_cmd_set_pktexg_evt_enable_t vss_istream_cmd_set_pktexg_evt_enable_t;

#include "apr_pack_begin.h"
/** @weakgroup weak_vss_istream_cmd_set_pktexg_evt_enable_t
@{ */
/* Payload structure for the VSS_ISTREAM_CMD_SET_PKTEXG_EVT_ENABLE command.
*/
struct vss_istream_cmd_set_pktexg_evt_enable_t
{
  uint8_t enable_flag;
    /**< Vocoder PKTEXG event enable/disable flag.
      *
      *
      *  @values:
      *    - 0: DISABLE (this is default mode).
	  *         Implies time driven mailbox packet exchange mechanism.
      *
      *    - 1: ENABLE.
      *         Implies event driven mailbox packet exchange mechanism.
      */
}
#include "apr_pack_end.h"
;
/** @} *//* end_weakgroup weak_vss_istream_cmd_set_pktexg_evt_enable_t */
