...
=============================================================================*/
/*===========================================================================
Copyright (c) 2011, 2015-2020 Qualcomm Technologies, Inc.  All rights reserved.
...
/*===========================================================================
  ...
  $Header: //components/rel/avs.mpss/9.0.c51/elite/api/cvd/vss_common_public_if.h#1 $
  $Author: pwbldsvc $
...
/** When VSS_MEDIA_ID_AMR_WB_2 is configured, voice packets are
  encoded/decoded as follows:
  - Encoded using the AMR_WB vocoder on the uplink
  - Decoded using the EVS AMR-WB IO decoder on the downlink

  The packet format is the same as the #VSS_MEDIA_ID_AMR_WB packet format.

  @inputfile{group__weakf__media__fmt__amrwb2.tex}
*/
...
/** @ingroup cvd_common_cmd_get_paramv2
  ...
  @apr_msgpayload{vss_icommon_cmd_get_param_v2_t}
   @tablens{weak__vss__icommon__cmd__get__param__v2__t} @newpage
...
struct vss_icommon_cmd_set_voice_timing_v2_t
{
  ...
  uint32_t vsid;
  /**< ID of the voice system @if OEM_only as defined in
       Appendix @xref{chp:VsidOverview}@endif. */
...
/** @ingroup cvd_common_cmd_set_voice_tmg_ref
  ...
  The Vocoder frame reference must be set via #VSS_ICOMMON_CMD_SET_VFR_CONFIG.
*/
#define VSS_ICOMMON_CMD_SET_VOICE_TIMING_REFERENCE_V2 ( 0x000133C1 )

/* Type definition for vss_icommon_cmd_set_voice_timing_reference_t. */
typedef struct vss_icommon_cmd_set_voice_timing_reference_v2_t vss_icommon_cmd_set_voice_timing_reference_v2_t;

#include "apr_pack_begin.h"
/** @weakgroup weak_vss_icommon_cmd_set_voice_timing_reference_v2_t
@{ */
/* Payload structure for the VSS_ICOMMON_CMD_SET_VOICE_TIMING_REFERENCE_V2 command.
*/
struct vss_icommon_cmd_set_voice_timing_reference_v2_t
{
  uint64_t tx_ref_timstamp_us;
  /**< Client's encoder packet exchange time reference (in microseconds).
...
/**
  * This command is sent by VS to set the new time line while in voice call
  */
#define VSS_ICOMMON_CMD_SET_VOICE_TIMELINE_RESYNC (0x000133CC)

#define VSS_RESYNC_TX (1)
#define VSS_RESYNC_RX (2)
#define VSS_RESYNC_TX_RX (3)

/* Type definition for vss_icommon_cmd_timeline_resync_t. */
typedef struct vss_icommon_cmd_timeline_resync_t vss_icommon_cmd_timeline_resync_t;

#include "apr_pack_begin.h"
/** @weakgroup weak_vss_icommon_cmd_timeline_resync_t
@{ */
/* Payload structure for the VSS_ICOMMON_CMD_SET_VOICE_TIMELINE_RESYNC command.
*/

struct vss_icommon_cmd_timeline_resync_t
{
   ...
   uint32_t network_id;
   /**<  Network ID
     *  To be used in future for cal update
     */

   uint64_t tx_ref_timestamp_us;
   /**< Updated encoder packet exchange time reference (absolute time in microseconds).
      *  see SET_VOICE_TIMING_REFERENCE payload for more details
      */

   uint64_t rx_ref_timestamp_us;
   /**< Updated decoder packet exchange time reference (absolute time in microseconds).
    *see SET_VOICE_TIMING_REFERENCE payload for more details
     */

   uint64_t frame_type_ref_timestamp_us;
   /**< Client's time reference to receive the frame type of uplink packet ( absolute time in microseconds).
     * see SET_VOICE_TIMING_REFERENCE_V2 payload for more details
     */
...
/** @} *//* end_addtogroup cvd_common_evt_voice_act_update */

#define VSS_ICOMMON_EVENT_RESYNC_DONE (0x000133CD)

/* Type definition for vss_icommon_evt_voice_activity_update_t. */
typedef struct vss_icommon_event_resync_done_t vss_icommon_event_resync_done_t;

#include "apr_pack_begin.h"
/** @weakgroup weak_vss_icommon_event_resync_done_t
@{ */
/* Payload structure for the event 'VSS_ICOMMON_EVENT_RESYNC_DONE'. */

struct vss_icommon_event_resync_done_t
{
   uint32_t resync_type;
   /* resync type for which ack is sent */
...
#include "apr_pack_end.h"
;
/** @} *//* end_weakgroup weak_vss_icommon_event_resync_done_t */
