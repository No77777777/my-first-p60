...
/*===========================================================================
  ...
  $Header: //components/rel/avs.mpss/9.0.c51/elite/api/cvd/vss_ipktexg_public_if.h#1 $
  $Author: pwbldsvc $

when        who    what, where, why
-------     ---    ----------------------------------------------------------
04/10/19     sw    (Tech Pubs) Edited Doxygen comments for AVS 2.9 (SM8250).
...
<-------- 32 bits -------->
 ...
 -------------------------
| timestamp               |  ---> Set to 0 if not used
 -------------------------
| media_id                |
 -------------------------
| enc_data_size           |
...
struct vss_ipktexg_mailbox_enc_packet_t
{
  ...
  /**< Bitmask that indicates the encoder packet status.
       ...
       - if bit 0 is set -- Indicates a general encoding error, informing the
         client that the packet has been dropped. The data_size is set to 0.
       - If bits 1 to 31 are set - Reserved; set these bits to 0.
       @tablebulletend */
...
/** @ingroup cvd_voc_px_cmd_mbx_mem_cfg
  Sets the address and size information for aDSP mailbox memory.

  @apr_msgpayload {vss_ipktexg_cmd_set_mailbox_memory_config_t}
  @table{weak__vss__ipktexg__cmd__set__mailbox__memory__config__t}

  @detdesc
  This command is applicable only for a fusion platform where the mDSP sits on
  the MDM device and the aDSP is part of the APQ device.
  @par
  The CVD in the APQ-ADSP is responsible for exchanging the aDSP mailbox memory
  configuration with the VS in the MDM-Modem before the voice packet exchange
  starts during a voice call.
  The Modem CVD client (VS) requires this configuration to map the physical
  memory (carved out of the APQ DDR for mailbox packet exchange) with the MVM.
  The memory should be contiguous 80 KB and and dedicated for vocoder packet
  exchanges between the aDSP and Modem.
  @par
  The configuration must be set after bootup as soon as this information is
  available to the HLOS. The HLOS voice driver must also keep track of aDSP
  and MDM PDR/SSR events and trigger the setting of IPA configuration after
  every aDSP/MDM PDR/SSR UP notification.
  @par
  Access control over the mailbox buffer must be configured correctly for the
  APQ-APPS, APQ-ADSP, and PCIe/IPA to access.
...
#include "apr_pack_begin.h"
/** @weakgroup weak_vss_ipktexg_cmd_set_mailbox_memory_config_t
@{ */
...
struct vss_ipktexg_cmd_set_mailbox_memory_config_t
{
  ...
  /**< IO virtual address (understandable to the aDSP) of the mailbox memory
       carved out from the APQ DDR.

       This memory is specifically used for mailbox packet exchanges between
       the aDSP and MDM device. */

  uint64_t mailbox_mem_address_pcie;
  /**< IO virtual address (understandable to PCIe) of the mailbox memory carved
       out from the APQ DDR.

       This memory is specifically used for mailbox packet exchanges between
       the aDSP and MDM. */

  uint32_t mem_size;
  /**< Size(in bytes) of the mailbox memory.

       @values 81920 bytes */
}
#include "apr_pack_end.h"
;
/** @} *//* end_weakgroup weak_vss_ipktexg_cmd_set_mailbox_memory_config_t */

/** @ingroup cvd_voc_px_req_mbx_mem_cfg
  Requests the mailbox memory configuration information for the aDSP
  mailbox memory.

  The VS triggers this request as part of the modem call setup sequence, and
  only for a fusion platform where the mDSP sits on the MDM device and the
  aDSP is part of the APQ device.

  The VS uses the configuration information to map the physical memory (carved
  out from the APQ DDR for mailbox packet exchanges) with the MVM.
  To reduce modem call setup latency, the VS requests this information only
  for the first voice call after bootup or MDM SSR/PDR, and it stores the
...
/** @ingroup cvd_voc_px_rsp_mbx_mem_cfg
  Response to a #VSS_IPKTEXG_CMD_REQ_MAILBOX_MEMORY_CONFIG command.
  ...
  @dependencies
  The CVD must have already received the
  #VSS_IPKTEXG_CMD_SET_MAILBOX_MEMORY_CONFIG information from the HLOS.
  If not, the CVD returns an error response.
*/
...
#include "apr_pack_begin.h"
/** @weakgroup weak_vss_ipktexg_rsp_mailbox_memory_config_t
@{ */
...
{
  uint64_t mailbox_mem_address_adsp;
  /**< IO virtual address (understandable to the aDSP) of mailbox memory
       carved out from the APQ DDR.
  ...
  uint64_t mailbox_mem_address_pcie;
  /**< IO virtual address (understandable to PCIe) of mailbox memory carved
       out from the APQ DDR.
...
#include "apr_pack_end.h"
;
/** @} *//* end_weakgroup weak_vss_ipktexg_rsp_mailbox_memory_config_t */

/** @ingroup cvd_voc_px_evt_no_data_packet
  Event sent from VSM(ADSP) to VS(Modem) indicating that
  ...
  None.
*/
#define VSS_IPKTEXG_EVT_NO_DATA_PACKET ( 0x000133BC )
