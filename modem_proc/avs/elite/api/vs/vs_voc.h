...
/*
  ...
  ============================================================================
  $Header: //components/rel/avs.mpss/9.0.c51/elite/api/vs/vs_voc.h#1 $
  $Author: pwbldsvc $
  ============================================================================
*/
...
/**
 * Resync done event (asynchronous event).
 * It will be sent in response to the runtime resync operation triggered 
 * by the client using VS_PARAM_VOC_TIMELINE_RESYNC paramID. 
 * It indicates that the timeline resync operation is 
 * performed successfully and Audio has shifted to the new timeline.
 *
 */
#define VS_VOC_EVENT_RESYNC_DONE ( 0x000133CA )

typedef struct vs_voc_event_resync_done_t {
  uint32_t status;
  /**< Status of the resync operation.
    *
    *  @values, refer to VS_EXXXX definitions.
    */
  uint32_t resync_type;
    /**< Type of resync which is performed. */ 
} vs_voc_event_resync_done_t;
