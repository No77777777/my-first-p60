...
/*
  ...
  ============================================================================
  $Header: //components/rel/avs.mpss/9.0.c51/elite/api/vs/vs_param.h#1 $
  $Author: pwbldsvc $
  ============================================================================
*/
 ...
 */
#define VS_RESYNC_TX_RX  ( VS_RESYNC_TX | VS_RESYNC_RX )
 ...
 * Dependencies: Vocoder services must be in DISABLED state.
 */
#define VS_PARAM_VOC_TIMING_REFERENCES_V2 ( 0x000133C0 )

typedef struct vs_param_voc_timing_reference_v2_t {
  ...
  uint32_t enc_packet_ready_margin_us;
    /**< 
      ...
      */
} vs_param_voc_timing_reference_v2_t;

/** 
 * Voice Timeline Resync parameter definition.
 * 
 * The client uses VS_COMMON_CMD_SET_PARAM command to exercise this parameter.
 *
 * Dependencies: 
 *  It should be set by the client only after the Voice call has reached steady state.
 *  Its expected to be set when VS is enabled and running.
 *
 *  Audio aligns its VFR with Rx Reference TS, so it needs Rx Reference TS of the new timeline as part of the first resync cmd itself.
 *  Alternatively IMS can populate the Rx Ref TS as part of every resync cmd irrespective of the type of the resync.
 *
 *  Timeline resync can also involve Network change between VoLTE/VoNR/VoWLAN.
 *  VFR config and Vocoder type are still expected to remain same during RAT switch.
 *  This is supported by assuming that Network specific Audio calibration is not required.
 *  In future, if Network specific calibration is introduced for VoLTE/VoNR/VoWLAN 
 *  then client needs to follow typical Handover sequence to perform the RAT switch.
 */

#define VS_PARAM_VOC_TIMELINE_RESYNC ( 0x000133CB )

typedef struct vs_param_voc_timeline_resync_t {
  ...
  uint32_t enc_packet_ready_margin_us;
    /**< 
      ...
      *  Default value is 0us.
      */
} vs_param_voc_timeline_resync_t;

/** 
 * Vocoder packet exchange event enable parameter.
 * 
 * The client uses VS_COMMON_CMD_SET_PARAM command to enable/disable event 
 * based packet exchange in uplink during voice call.
 *
...
/** 
* Parameters to set mute/unmute on downlink/uplink stream
*
* Payload structure for the mute set param is sent by client using 
* VS_COMMON_CMD_SET_PARAM. 
* Mute is applied at the end of stream processing on downlink 
* On uplink, mute is applied right after limiter gain application
*/
#define VS_PARAM_MUTE_STREAM ( 0x0001339E )

typedef struct vs_param_mute_stream_t
{
  uint16_t direction;
    /**< Specifies the path on which Mute mode is set.
      ...
      *   client must send a separate set param on each path.
      */
