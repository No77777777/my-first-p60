...
/**<  @file apm_api.h
      @brief This file contains Audio Processing Manager Commands Data Structures */
...
/** @ingroup apm_cmd_rsp_get_cfg
  ...
  Status field is populated with success code (#MM_EOK) if all
  the parameter ID's being queried returns success code. 
   
  Status field is populated with failure code (#MM_EFAILED) if 
...
/** @ingroup apm_cmd_get_gecko_status_t
  This parameter is used for querying GECKO status. If the
  command can reach the APM, means that GECKO is ready.

  @gpr_hdr_fields
  Opcode -- APM_CMD_GET_GECKO_STATE

  @return
  Response code -- #APM_CMD_RSP_GET_GECKO_STATE
...
/** Definition for the Gecko Not-Ready State */
#define APM_GECKO_STATE_NOT_READY                 0

/** Definition for the Gecko Ready State */
#define APM_GECKO_STATE_READY                     1


/** @ingroup apm_cmd_rsp_get_gecko_status_t
  The response code returned as an acknowledgment for the
  #APM_CMD_GET_GECKO_STATE command.

  The response payload starts with status field which indicates
  the overall status of the #APM_CMD_GET_GECKO_STATE command.

  @gpr_hdr_fields
  Opcode -- APM_CMD_RSP_GET_GECKO_STATE
...
struct apm_cmd_rsp_get_gecko_status_t
{
   ...
   /**< Gecko status.
        ...
        @values
        - #APM_GECKO_STATE_NOT_READY
        - #APM_GECKO_STATE_READY
        @tablebulletend */
}
...
typedef struct apm_cmd_rsp_get_gecko_status_t apm_cmd_rsp_get_gecko_status_t;
 ...
 * Delay is >= 0.
 *
 * Client can leave the port-ids zeros to be autofilled by Gecko.
