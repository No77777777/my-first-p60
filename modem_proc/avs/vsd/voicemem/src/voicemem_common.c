/*
  ...
  $Header: //components/rel/avs.mpss/9.0.c51/vsd/voicemem/src/voicemem_common.c#1 $
  $Author: pwbldsvc $
*/
...
/* Object manager lock. */
static ar_osal_mutex_t voicemem_objmgr_lock;

/* Object manager. */
static vs_objmgr_object_t voicemem_object_table[ VOICEMEM_MAX_OBJECTS_V ];
static vs_objmgr_t voicemem_objmgr;

/* VOICEMEM internal book-keeping heap manager. */
static uint8_t voicemem_internal_heap_pool[ VOICEMEM_INTERNAL_HEAP_SIZE_V ];
static vs_memmgr_type voicemem_internal_heapmgr;
...
{
  ( void ) ar_osal_mutex_lock( voicemem_objmgr_lock );
}


VSD_PRIVATE void voicemem_objmgr_unlock_fn ( void )
{
  ( void ) ar_osal_mutex_unlock( voicemem_objmgr_lock );
}
...
)
{
  ...
  for ( ;; )
  {
    ...
    { /* Allocate memory for the new region object. */
      region_obj = vs_memmgr_malloc( &voicemem_internal_heapmgr,
    ...
    { /* Allocate a new handle for the region object. */
      rc = vs_objmgr_alloc_object( &voicemem_objmgr, &objmgr_obj );
      if ( rc ) break;
      checkpoint = 2;
    }

    /* Link the region object to the handle */
    objmgr_obj->ptr = region_obj;
  ...
  {
  case 2:
    ( void ) vs_objmgr_free_object( &voicemem_objmgr, objmgr_obj->handle );
    /*-fallthru */

  case 1:
    vs_memmgr_free( &voicemem_internal_heapmgr, region_obj );
    /*-fallthru */
...
)
{
  ...
  ( void ) vs_objmgr_free_object( &voicemem_objmgr, region_obj->handle );
  vs_memmgr_free( &voicemem_internal_heapmgr, region_obj );
...
)
{
  ...
  for ( ;; )
  {
    ...
    rc = vs_objmgr_find_object( &voicemem_objmgr, handle, &objmgr_obj );
    if ( rc ) break;

    region_obj = ( ( voicemem_region_object_t* ) objmgr_obj->ptr );
    if ( region_obj == NULL )
    {
      rc = VSD_EFAILED;
      break;
...
VSD_PRIVATE uint32_t voicemem_cmd_init ( void )
{
  ...
  { /* Initialize object manager lock. */
    ( void ) ar_osal_mutex_create( &voicemem_objmgr_lock );
  }
  ...
  { /* Initialize the object manager. */
    ...
    ( void ) vs_objmgr_construct( &voicemem_objmgr, &params );
  }

  { /* Initialize VOICEMEM internal book-keeping heap manager. */
    vs_memmgr_init_heap( &voicemem_internal_heapmgr, (void*) &voicemem_internal_heap_pool,
                          sizeof( voicemem_internal_heap_pool ), NULL);
  }
...
VSD_PRIVATE uint32_t voicemem_cmd_deinit ( void )
{
  ...
  if ( voicemem_init_ref_count == 0 )
  {
    ...
    { /* Destroy the object manager. */
      ( void ) vs_objmgr_destruct( &voicemem_objmgr );
    }

    { /* Destroy the object manager lock. */
      ( void ) ar_osal_mutex_destroy( voicemem_objmgr_lock );
    }
  }
