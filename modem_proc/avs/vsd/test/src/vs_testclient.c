/*
   ...
   $Header: //components/rel/avs.mpss/9.0.c51/vsd/test/src/vs_testclient.c#2 $
   $Author: pwbldsvc $
*/
...
static ar_osal_mutex_t vs_test_int_lock;
static ar_osal_mutex_t vs_test_thread_lock;
static ar_osal_signal_t vs_test_control_event;

static vs_memmgr_type vs_test_heapmgr;
static uint8_t vs_test_heap_pool[ VS_TEST_HEAP_SIZE_V ];

static vs_objmgr_t vs_test_objmgr;
static vs_objmgr_object_t vs_test_object_table[ VS_TEST_MAX_OBJECTS_V ];

static vs_test_gating_control_t vs_test_gating_work_pkt_q;
static gpr_list_t vs_test_nongating_work_pkt_q;
static gpr_list_t vs_test_free_work_pkt_q;
static vs_test_work_item_t vs_test_work_pkts[ VS_TEST_NUM_WORK_PKTS_V ];

static ar_osal_signal_t vs_test_work_event;
static ar_osal_thread_t vs_test_thread;
static uint8_t vs_test_task_stack[ VS_TEST_STACK_SIZE_V ];

static vs_test_session_object_t* vs_test_session_obj = NULL;
static ar_osal_mutex_t vs_test_global_lock;
...
{
  ( void )ar_osal_mutex_lock( vs_test_int_lock );
}


static void vs_test_int_unlock_fn ( void )
{
  ( void )ar_osal_mutex_unlock( vs_test_int_lock );
}


static void vs_test_thread_lock_fn ( void )
{
  ( void )ar_osal_mutex_lock( vs_test_thread_lock );
}


static void vs_test_thread_unlock_fn ( void )
{
  ( void )ar_osal_mutex_unlock( vs_test_thread_lock );
}
...
)
{
  ...
  if ( VS_NULL_V == session_obj->vs_handle ) 
  {
...
)
{
  ...
  for ( ;; )
  {
    ...
    cmd_params = GPR_PKT_GET_PAYLOAD( vs_test_loopback_cmd_t, packet );
...
/**
 * Queues the gpr packet.
 * In case of failure to queue a gpr packet, packet shall be
 * freed by the caller.
 */
...
)
{
  ...
  for ( ;; )
  {
    ...
    rc = gpr_list_remove_head( &vs_test_free_work_pkt_q,
    ...
    rc = gpr_list_add_tail( work_queue, &work_item->link );
    if ( rc )
    {
      MSG_1( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
      ...
      ( void ) gpr_list_add_tail( &vs_test_free_work_pkt_q, &work_item->link );
    }
    else
    {
      /**
...
)
{
  ...
  for ( ;; )
  {
    pkt_params.src_domain_id = VS_NULL_V ;
    pkt_params.src_port = VS_NULL_V;
    pkt_params.dst_domain_id = VS_NULL_V ; 
    pkt_params.dst_port = VS_NULL_V ;
    pkt_params.token = (uint32_t)session_context;
    pkt_params.opcode = cmd_id;
    pkt_params.payload_size = size;
    ...
    {
      MSG_1( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
             "VS Testclient: GPR packet allocation failed with rc=[0x%08x]", rc );
      break;
    }
...
)
{
  ...
  { /* Allocate memory for the VS_TEST object. */
    vs_test_obj = vs_memmgr_malloc( &vs_test_heapmgr, size );
    if ( vs_test_obj == NULL )
    {
      MSG_1( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
    ...
    rc = vs_objmgr_alloc_object( &vs_test_objmgr, &objmgr_obj );
    if ( rc )
    {
      MSG_1( MSG_SSID_DFLT, MSG_LEGACY_ERROR, 
             "vs_test_mem_alloc_object(): Out of objects, rc = (0x%08X)", rc );
      vs_memmgr_free( &vs_test_heapmgr, vs_test_obj );
      return VS_ENORESOURCE;
    }

    /* Use the custom object type. */
    objmgr_obj->ptr = vs_test_obj;
...
)
{
  ...
  ( void ) vs_objmgr_free_object( &vs_test_objmgr, object->header.handle );
  vs_memmgr_free( &vs_test_heapmgr, object );
...
)
{
  ...
  rc = vs_timer_start( session_obj->pktexg_timer, 20000000 );
  ...
  vs_test_session_object_t** ret_session_obj )
{
  ...
  { /* Initialize the simple job object. */
    ...
    rc = vs_timer_create( &session_obj->pktexg_timer, vs_test_pktexg_timer_cb, session_obj );
    ...
    session_obj->vs_handle = VS_NULL_V;
...
)
{
  ...
  if ( vs_test_is_initialized == FALSE ) return VS_EOK;
...
)
{
  ...
  while( TRUE )
  {
    if ( VS_NULL_V == session_obj->vs_handle ) break;
...
)
{
  ...
  if ( VS_NULL_V == session_obj->vs_handle ) return rc;
...
static uint32_t vs_test_process_loopback(gpr_packet_t* packet)
{
  uint32_t rc = VS_EOK;
  vs_test_session_object_t* session_obj = NULL;
...
)
{
  ...
  for ( ;; )
  {
    if ( session_obj->vs_handle == VS_NULL_V )
    {
      rc = VS_EOK;
      break;
    }
...
)
{
  ...
  for ( ;; )
  {
    ...
    if ( VS_EOK == rc )
    {
      rc = vs_timer_stop( session_obj->pktexg_timer );
      ( void ) vs_test_mem_free_object ( ( vs_test_object_t*) simple_obj );
...
static void vs_test_task_process_nongating_work_items ( void )
{
  ...
  while( gpr_list_remove_head( &vs_test_nongating_work_pkt_q,
     ...
     case VS_TEST_GATING_CMD_STATE_FETCH:
       {
          ...
          {
            VS_TEST_PANIC_ON_ERROR ( VS_EUNEXPECTED );
          }
...
{
  ( void )ar_osal_signal_set( vs_test_work_event );
}
...
static void vs_test_worker_thread_fn (
  void* param
)
{
  int32_t rc;

  ar_osal_signal_create( &vs_test_work_event );
  ar_osal_signal_set( vs_test_control_event );

  for ( ;; )
  {
    rc = ar_osal_signal_wait( vs_test_work_event );
    rc |= ar_osal_signal_clear( vs_test_work_event );
    if ( rc ) break;

    vs_test_run( );
  }

  ar_osal_signal_destroy( vs_test_work_event );
  ar_osal_signal_set( vs_test_control_event );
...
static int32_t vs_test_init ( void )
{
  ...
  {  /* Initialize the locks. */
    rc = ar_osal_mutex_create( &vs_test_int_lock );
    rc |= ar_osal_mutex_create( &vs_test_thread_lock );
    rc |= ar_osal_signal_create( &vs_test_control_event );
  }

  { /* Initialize the custom heap. */
    vs_memmgr_init_heap( &vs_test_heapmgr, ( ( void* ) &vs_test_heap_pool ),
                          sizeof( vs_test_heap_pool ), NULL );
  }
  ...
  { /* Initialize the object manager. */
    ...
    rc = vs_objmgr_construct( &vs_test_objmgr, &params );
  }

  { /* Initialize free and nongating work pkt queues. */
    rc = gpr_list_init_v2( &vs_test_free_work_pkt_q, 
                           vs_test_int_lock_fn, vs_test_int_unlock_fn );
    for ( index = 0; index < VS_TEST_NUM_WORK_PKTS_V; ++index )
    {
      ( void ) gpr_list_init_node( ( gpr_list_node_t* ) &vs_test_work_pkts[index] );
      vs_test_work_pkts[index].packet = NULL;
      rc = gpr_list_add_tail( &vs_test_free_work_pkt_q,
                              ( ( gpr_list_node_t* ) &vs_test_work_pkts[index] ) );
    }
    rc = gpr_list_init_v2( &vs_test_nongating_work_pkt_q,
                           vs_test_int_lock_fn, vs_test_int_unlock_fn );
  }
  ...
  { /* Initialize the global session lock. */
    rc = ar_osal_mutex_create( &vs_test_global_lock );
    VS_TEST_PANIC_ON_ERROR ( rc );
  }

  thread_attr.thread_name = "VS_TEST";
  thread_attr.priority = 97;
  thread_attr.stack_size = VS_TEST_STACK_SIZE_V;

  rc = ar_osal_thread_create( &vs_test_thread, &thread_attr,
                          vs_test_worker_thread_fn , NULL );
  VS_TEST_PANIC_ON_ERROR( rc );

  ( void ) ar_osal_signal_wait( vs_test_control_event );
...
static int32_t vs_test_deinit ( void )
{
  ...
  /* Free session object. */
  ( void ) ar_osal_mutex_destroy( vs_test_session_obj->data_lock );
  ( void ) vs_timer_destroy (vs_test_session_obj->pktexg_timer );
  ( void ) vs_test_mem_free_object( (vs_test_object_t*)vs_test_session_obj );

  vs_test_is_initialized = FALSE;
  
  ar_osal_signal_destroy( vs_test_work_event );
  ar_osal_signal_wait( vs_test_control_event );
  
  /* Release gating control structures */
  ( void ) vs_test_gating_control_destroy( &vs_test_gating_work_pkt_q );
  
  /* Release work queue */
  ( void ) gpr_list_clear( &vs_test_free_work_pkt_q );
  ( void ) gpr_list_clear( &vs_test_nongating_work_pkt_q );
  
  /* Deinitialize the object handle table. */
  vs_objmgr_destruct( &vs_test_objmgr );
  
  /* Deinitialize basic OS resources for staging the setup. */
  ( void ) ar_osal_signal_destroy( vs_test_control_event );
  ( void ) ar_osal_mutex_destroy( vs_test_int_lock );
  ( void ) ar_osal_mutex_destroy( vs_test_thread_lock );
  ( void ) ar_osal_mutex_destroy( vs_test_global_lock );
