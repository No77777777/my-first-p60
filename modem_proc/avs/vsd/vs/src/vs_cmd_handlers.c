...
/*
  ...
  $Header: //components/rel/avs.mpss/9.0.c51/vsd/vs/src/vs_cmd_handlers.c#1 $
  $Author: pwbldsvc $
*/
...
extern vs_objmgr_t vs_objmgr;
extern ar_osal_mutex_t vs_service_lock;
...
static uint32_t vs_voc_validate_and_map_media_id (
  vs_session_object_t* session_obj,
  uint32_t media_id
)
{
  ...
  switch ( media_id )
  {
    ...
    {
       session_obj->vocoder.media_id = MEDIA_FMT_ID_AMRNB_MODEM;
    ...
    {
       session_obj->vocoder.media_id = MEDIA_FMT_ID_EAMR;
       rc = vs_vocamr_init( session_obj );
    }
     break;
    ...
    {
       session_obj->vocoder.media_id = MEDIA_FMT_ID_AMRWB;
    ...
    {
       session_obj->vocoder.media_id = MEDIA_FMT_ID_EFR;
    ...
    {
       session_obj->vocoder.media_id = MEDIA_FMT_ID_FR;
    ...
    {
       session_obj->vocoder.media_id = MEDIA_FMT_ID_HR;
    ...
    {
       session_obj->vocoder.media_id = MEDIA_FMT_ID_EVS;
    ...
    {
       session_obj->vocoder.media_id = MEDIA_FMT_ID_4GV_NB_MODEM;
       rc = vs_voc4gv_init( session_obj );
    }
    break;

    case VS_VOCEVRCWB_MEDIA_ID:
    {
      session_obj->vocoder.media_id = MEDIA_FMT_ID_4GV_WB_MODEM;
       rc = vs_voc4gv_init( session_obj );
    }
    break;
    ...
    {
      session_obj->vocoder.media_id = MEDIA_FMT_ID_4GV_NW_MODEM;
       rc = vs_voc4gv_init( session_obj );
    }
    break;
    ...
    {
       session_obj->vocoder.media_id = MEDIA_FMT_ID_V13K;
    ...
    {
      session_obj->vocoder.media_id = MEDIA_FMT_ID_EVRC;
    ...
    {
      session_obj->vocoder.media_id = MEDIA_FMT_ID_G711_ALAW_V2;
      rc = vs_vocg711_v2_init( session_obj );
    }
    break;

    case VS_VOCG711MULAW_V2_MEDIA_ID:
    {
      session_obj->vocoder.media_id = MEDIA_FMT_ID_G711_MLAW_V2;
      rc = vs_vocg711_v2_init( session_obj );
    }
    break;
    ...
    {
      session_obj->vocoder.media_id = media_id;
...
static uint32_t vs_voc_map_amrwb_frame_type (
   vs_pktexg_buffer_t* vs_buffer
)
{
  uint32_t rc = VS_EOK;
  vs_amrwb_codec_mode_t amrwb_codec_mode;
  vs_amrwb_frame_type_t amrwb_frame_type;
  vs_amrwb_2_frame_info_type_t amrwb_2_frame_type;
  uint8_t* frame = NULL;

  frame = ( (uint8_t*)vs_buffer ) + sizeof( vs_pktexg_buffer_t );
  amrwb_frame_type = ( vs_amrwb_frame_type_t )((*frame) >> 4);
  amrwb_codec_mode = ( vs_amrwb_codec_mode_t )((*frame) ^ (amrwb_frame_type << 4));

  MSG_2( MSG_SSID_DFLT, MSG_LEGACY_LOW,"vs_voc_map_amrwb_frame_type(): AMR WB frame type:%d and AMR WB codec mode:%d", amrwb_frame_type , amrwb_codec_mode );

  if( amrwb_frame_type != VS_AMRWB_FRAME_TYPE_SPEECH_GOOD )
  {
    switch( amrwb_frame_type )
    {
      case VS_AMRWB_FRAME_TYPE_SPEECH_PROBABLY_DEGRADED:
      case VS_AMRWB_FRAME_TYPE_SPEECH_BAD:
      case VS_AMRWB_FRAME_TYPE_SID_BAD:
      case VS_AMRWB_FRAME_TYPE_NO_DATA:
        amrwb_2_frame_type = VS_AMRWB_2_NO_DATA;
        vs_buffer->frame_size = 1;
        break;

      case VS_AMRWB_FRAME_TYPE_SPEECH_LOST:
        amrwb_2_frame_type = VS_AMRWB_2_SPEECH_LOST;
        break;

      case VS_AMRWB_FRAME_TYPE_SID_FIRST:
      case VS_AMRWB_FRAME_TYPE_SID_UPDATE:
        amrwb_2_frame_type = VS_AMRWB_2_SID_2_0;
        break;

      default:
        amrwb_2_frame_type = VS_AMRWB_2_INVALID;
        break;
    }
  }
  else 
  {
    if( ( amrwb_codec_mode >= VS_AMRWB_CODEC_MODE_0660 ) && 
         ( amrwb_codec_mode <= VS_AMRWB_CODEC_MODE_2385 ) )
    {
      amrwb_2_frame_type = ( vs_amrwb_2_frame_info_type_t )amrwb_codec_mode;
    }
    else
    {
      amrwb_2_frame_type = VS_AMRWB_2_INVALID;
  ...
  if( VS_AMRWB_2_INVALID == amrwb_2_frame_type)
  {
     MSG( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
        "vs_voc_map_amrwb_frame_type: Received invalid codec mode or frame type\n" );
     return VS_EBADPARAM;
  }
  *frame = amrwb_2_frame_type ;
  MSG_1( MSG_SSID_DFLT, MSG_LEGACY_LOW,"vs_voc_map_amrwb_frame_type(): EVS AMR-WB IO frame type: %d", amrwb_2_frame_type );
  return rc ;
}

static uint32_t vs_voc_map_media_id_vs_to_decoder (
  vs_pktexg_buffer_t* vs_buffer
)
{
  uint32_t rc = VS_EOK;
  uint32_t media_id = vs_buffer->media_id;

  switch ( media_id )
  {
    case VS_VOCAMR_MEDIA_ID:
      vs_buffer->media_id = MEDIA_FMT_ID_AMRNB_MODEM;
     break;

    case VS_VOCEAMR_MEDIA_ID:
      vs_buffer->media_id = MEDIA_FMT_ID_EAMR;
      break;

    case VS_VOCAMRWB_MEDIA_ID:
      vs_buffer->media_id = MEDIA_FMT_ID_AMRWB;
      break;

    case VS_VOCEFR_MEDIA_ID:
      vs_buffer->media_id = MEDIA_FMT_ID_EFR;
      break;

    case VS_VOCFR_MEDIA_ID:
      vs_buffer->media_id = MEDIA_FMT_ID_FR;
      break;

    case VS_VOCHR_MEDIA_ID:
      vs_buffer->media_id = MEDIA_FMT_ID_HR;
      break;

    case VS_VOCEVS_MEDIA_ID:
       vs_buffer->media_id = MEDIA_FMT_ID_EVS;
       break;

    case VS_VOCEVRCB_MEDIA_ID:
      vs_buffer->media_id = MEDIA_FMT_ID_4GV_NB_MODEM;
      break;

    case VS_VOCEVRCWB_MEDIA_ID:
      vs_buffer->media_id = MEDIA_FMT_ID_4GV_WB_MODEM;
      break;

    case VS_VOCEVRCNW_MEDIA_ID:
      vs_buffer->media_id = MEDIA_FMT_ID_4GV_NW_MODEM;
      break;

    case VS_VOCEVRCNW2K_MEDIA_ID:
      vs_buffer->media_id = MEDIA_FMT_ID_EVRC_NW_2K;
      break;

    case VS_VOCQCELP13K_MEDIA_ID:
      vs_buffer->media_id = MEDIA_FMT_ID_V13K;
     break;

    case VS_VOCEVRCA_MEDIA_ID:
      vs_buffer->media_id = MEDIA_FMT_ID_EVRC;
      break;

    case VS_VOCG711ALAW_V2_MEDIA_ID:
      vs_buffer->media_id = MEDIA_FMT_ID_G711_ALAW_V2;
      break;

    case VS_VOCG711MULAW_V2_MEDIA_ID:
      vs_buffer->media_id = MEDIA_FMT_ID_G711_MLAW_V2;
      break;

    case VS_VOCAMRWB_2_MEDIA_ID:
      vs_buffer->media_id = MEDIA_FMT_ID_EVS;
      vs_voc_map_amrwb_frame_type(vs_buffer);
      break;
      
    case VS_VOCDTMF_MEDIA_ID:
      vs_buffer->media_id = MEDIA_FMT_ID_DTMF;
      break;
    ...
    default:
    {
      ...
      MSG_1( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
              "vs_voc_map_codec_mode_vsclient_to_vs(): received unsupported media_id=(0x%08x)", 
                media_id);
    }
      break;
  }
...
static uint32_t vs_voc_map_media_id_encoder_to_vs (
  vs_pktexg_buffer_t* vs_buffer,
  uint32_t voc_media_id
)
{
  uint32_t rc = VS_EOK;
  uint32_t media_id = vs_buffer->media_id ;
  ...
  switch ( media_id )
  {
    case MEDIA_FMT_ID_AMRNB_MODEM:
      vs_buffer->media_id = VS_VOCAMR_MEDIA_ID;
      break;

    case MEDIA_FMT_ID_EAMR:
      vs_buffer->media_id = VS_VOCEAMR_MEDIA_ID;
      break;

    case MEDIA_FMT_ID_AMRWB:
      if (voc_media_id == VS_VOCAMRWB_2_MEDIA_ID)
      {
        vs_buffer->media_id = VS_VOCAMRWB_2_MEDIA_ID;
      }
      else
      {
        vs_buffer->media_id = VS_VOCAMRWB_MEDIA_ID;
      }
      break;

    case MEDIA_FMT_ID_EFR:
       vs_buffer->media_id = VS_VOCEFR_MEDIA_ID;
       break;

    case MEDIA_FMT_ID_FR:
      vs_buffer->media_id = VS_VOCFR_MEDIA_ID;
      break;

    case MEDIA_FMT_ID_HR:
      vs_buffer->media_id = VS_VOCHR_MEDIA_ID;
      break;

    case MEDIA_FMT_ID_EVS:
      vs_buffer->media_id = VS_VOCEVS_MEDIA_ID;
      break;

    case MEDIA_FMT_ID_4GV_NB_MODEM:
      vs_buffer->media_id = VS_VOCEVRCB_MEDIA_ID;
      break;

    case MEDIA_FMT_ID_4GV_WB_MODEM:
      vs_buffer->media_id = VS_VOCEVRCWB_MEDIA_ID;
      break;

    case MEDIA_FMT_ID_4GV_NW_MODEM:
      vs_buffer->media_id = VS_VOCEVRCNW_MEDIA_ID;
      break;

    case MEDIA_FMT_ID_EVRC_NW_2K:
      vs_buffer->media_id = VS_VOCEVRCNW2K_MEDIA_ID;
      break;

    case MEDIA_FMT_ID_V13K:
      vs_buffer->media_id = VS_VOCQCELP13K_MEDIA_ID;
      break;

    case MEDIA_FMT_ID_EVRC:
      vs_buffer->media_id = VS_VOCEVRCA_MEDIA_ID;
      break;

    case MEDIA_FMT_ID_G711_ALAW_V2:
      vs_buffer->media_id = VS_VOCG711ALAW_V2_MEDIA_ID;
      break;

    case MEDIA_FMT_ID_G711_MLAW_V2:
      vs_buffer->media_id = VS_VOCG711MULAW_V2_MEDIA_ID;
      break;
    ...
    default:
    {
       rc = VS_EUNSUPPORTED;
       MSG_1( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
              "vs_voc_map_codec_mode_vs_to_client(): received unsupported media_id=(0x%08x)", 
                media_id);
    }
    break;
  }
...
)
{
  ...
  if( TRUE == session_obj->is_stream_ready )
  {
    if ( session_obj->vocoder.media_id == MEDIA_FMT_ID_EVS)
    {
      ( void ) vs_voc_action_set_evs_dtx( session_obj ); //Param ID for EVS is different from other vocoders, hence special function.
    }
    else
    {
...
)
{
     ...
     )
  {
    MSG_3 ( MSG_SSID_DFLT, MSG_LEGACY_HIGH, 
            "SET_PARAM:VS_PARAM_VOC_TIMING_REFERENCES: vfr_mode is overridden to HARD(1), "
            "actual_vfr_mode=(%d), client_id=(0x%08x), vs_platform_is_fusion=(%d)", 
            timing->vfr_mode, session_obj->vocoder.client_id, vs_platform_is_fusion );
    session_obj->vocoder.vfr_config.vfr_mode = VSS_ICOMMON_VFR_MODE_HARD;
  }
  else
  {
    MSG_3 ( MSG_SSID_DFLT, MSG_LEGACY_HIGH, 
            "SET_PARAM:VS_PARAM_VOC_TIMING_REFERENCES: vfr_mode is overridden to SOFT(0), "
            "actual_vfr_mode=(%d), client_id=(0x%08x), vs_platform_is_fusion=(%d)", 
            timing->vfr_mode, session_obj->vocoder.client_id, vs_platform_is_fusion );
    session_obj->vocoder.vfr_config.vfr_mode = VSS_ICOMMON_VFR_MODE_SOFT;
  }

  MSG_4( MSG_SSID_DFLT, MSG_LEGACY_HIGH,
        "SET_PARAM:VS_PARAM_VOC_TIMING_REFERENCES: read_ref_timestamp_us=(%lu), write_ref_timestamp_us=(%lu)"
...
static uint32_t vs_voc_set_timing_reference_v2 (
  vs_session_object_t* session_obj,
  vs_param_voc_timing_reference_v2_t* timing
)
{
  uint32_t rc = VS_EOK;
  ...
  session_obj->vocoder.timing_config.rx_ref_timestamp_us = timing->write_ref_timestamp_us;
  session_obj->vocoder.vfr_config.vfr_cycle_duration_ms = timing->vfr_cycle_duration_ms;
  session_obj->vocoder.timing_config.frame_type_ref_timestamp_us = timing->frame_type_ref_timestamp_us;
  session_obj->vocoder.timing_config.enc_packet_ready_margin_us = timing->enc_packet_ready_margin_us;
  session_obj->vocoder.vfr_config.vfr_mode = timing->vfr_mode;
  
  MSG_6( MSG_SSID_DFLT, MSG_LEGACY_HIGH,
        "SET_PARAM:VS_PARAM_VOC_TIMING_REFERENCES_V2: read_ref_timestamp_us=(%lu), write_ref_timestamp_us=(%lu), "
        "vfr_mode=(%d) (0-soft, 1-hard), vfr_cycle_duration_ms=(%d), frame_type_ref_timestamp_us=(%lu), enc_packet_ready_margin_us: (%d)",
         timing->read_ref_timestamp_us,timing->write_ref_timestamp_us, timing->vfr_mode, timing->vfr_cycle_duration_ms,
         timing->frame_type_ref_timestamp_us,timing->enc_packet_ready_margin_us);

  return rc;
}


static uint32_t vs_voc_set_timeline_resync (
  vs_session_object_t* session_obj,
  vs_param_voc_timeline_resync_t* timing
)
{
  uint32_t rc = VS_EOK;
  ...
  {
     MSG_1( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
        "SET_PARAM:VS_PARAM_VOC_TIMELINE_RESYNC: client not yet enabled [%d]",
         session_obj->vocoder.client_state);
     return VS_EFAILED;
  }

  if ( ( timing->vfr_mode != VS_VOC_VFR_MODE_SOFT_V ) ||
       ( timing->resync_type == VCPM_RESYNC_NONE ) ||
       ( timing->resync_type > VCPM_RESYNC_TX_RX )  )
  {
     MSG_3( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
        "SET_PARAM:VS_PARAM_VOC_TIMELINE_RESYNC: Invalid payload: resync_type=(%lu) vfr_mode=(%d) (0-soft, 1-hard), vfr_cycle_duration_ms=(%d)",
         timing->resync_type , timing->vfr_mode, timing->vfr_cycle_duration_ms);
     return VS_EBADPARAM;
  }

  session_obj->vocoder.timing_config.is_timing_ref_available = TRUE;
  session_obj->vocoder.timing_config.resync_type = timing->resync_type;
  if ( timing->resync_type & VCPM_RESYNC_TX )
  {
     session_obj->vocoder.timing_config.tx_ref_timestamp_us = timing->tx_ref_timestamp_us;
     session_obj->vocoder.timing_config.frame_type_ref_timestamp_us = timing->frame_type_ref_timestamp_us;
     session_obj->vocoder.timing_config.enc_packet_ready_margin_us = timing->enc_packet_ready_margin_us;
  }
  if ( timing->resync_type & VCPM_RESYNC_RX )
  {
     session_obj->vocoder.timing_config.rx_ref_timestamp_us = timing->rx_ref_timestamp_us;
  }
  session_obj->vocoder.vfr_config.vfr_cycle_duration_ms = timing->vfr_cycle_duration_ms;
  session_obj->vocoder.vfr_config.vfr_mode = timing->vfr_mode;
  session_obj->vocoder.client_id = timing->network_id;
  
  MSG_7( MSG_SSID_DFLT, MSG_LEGACY_HIGH,
        "SET_PARAM:VS_PARAM_VOC_TIMELINE_RESYNC: resync_type=(%lu), read_ref_timestamp_us=(%lu), write_ref_timestamp_us=(%lu), "
        "vfr_mode=(%d) (0-soft, 1-hard), vfr_cycle_duration_ms=(%d), frame_type_ref_timestamp_us=(%lu), enc_packet_ready_margin_us: (%d)",
         timing->resync_type ,timing->tx_ref_timestamp_us,timing->rx_ref_timestamp_us, timing->vfr_mode, timing->vfr_cycle_duration_ms,
         timing->frame_type_ref_timestamp_us,timing->enc_packet_ready_margin_us);
  rc = vs_voc_action_set_voice_timeline_resync(session_obj);
...
)
{
  ...
  rc = vs_voc_validate_and_map_media_id( session_obj, media->media_id );

  return rc;
}

static uint32_t vs_voc_set_tx_dtmf_gen (
  vs_session_object_t* session_obj,
...
)
{
  ...
  {
     MSG( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
        "SET_PARAM:VS_PARAM_TX_DTMF_GEN: stream not yet ready !");
     return VS_EFAILED;
  }
  
  MSG_5( MSG_SSID_DFLT, MSG_LEGACY_HIGH,
        "SET_PARAM:VS_PARAM_TX_DTMF_GEN: low_freq=(%d), high_freq=(%d), duration=(%d), gain(%d), mix_enable(%d)",
         dtmf_info->low_frequency,dtmf_info->high_frequency, dtmf_info->duration, dtmf_info->gain,dtmf_info->mix_enable);

  session_obj->dtmf_info = *dtmf_info;
  rc = vs_voc_action_set_dtmf_gen_to_mailbox_tx(session_obj);
...
)
{
  ...
  ( void ) ar_osal_mutex_lock( vs_service_lock );
  ...
  ( void )ar_osal_mutex_unlock( vs_service_lock );
...
{
  uint32_t rc = VS_EOK;
  uint32_t session_handle = VS_NULL_V;
  ...
  for ( ;; )
  {
    ...
    cmd_params = GPR_PKT_GET_PAYLOAD( vs_hpcm_cmd_enable_t, ctrl->packet );
    if ( NULL == cmd_params )
    {
      VS_REPORT_FATAL_ON_ERROR( VS_EUNEXPECTED );
      rc = VS_EFAILED;
    ...
    if( VS_UNDEFINED_ID_V == session_obj->session_ctrl.status )
    {
      rc = VS_EPENDING;
    }
    else
...
)
{
  ...
  for ( ;; )
  {
    ...
    cmd_params = GPR_PKT_GET_PAYLOAD( vs_hpcm_cmd_disable_t, ctrl->packet );
    if ( NULL == cmd_params )
    {
      VS_REPORT_FATAL_ON_ERROR( VS_EUNEXPECTED );
      rc = VS_EFAILED;
    ...
    if ( ctrl->state == VS_GATING_CMD_STATE_ENUM_EXECUTE )
    {
      ...
      {
        session_obj->session_ctrl.status = GPR_UNDEFINED_ID_V;
        ( void ) vs_voc_action_hpcm_stop( session_obj );
      }
    }

    if( GPR_UNDEFINED_ID_V == session_obj->session_ctrl.status )
    {
      rc = VS_EPENDING;
    }
    else
...
)
{
  ...
  for ( ;; )
  {
    ...
    cmd_params = GPR_PKT_GET_PAYLOAD( vs_hpcm_cmd_close_t, ctrl->packet );
    if ( NULL == cmd_params )
    {
      VS_REPORT_FATAL_ON_ERROR( VS_EUNEXPECTED );
      rc = VS_EFAILED;
...
)
{
  ...
  for ( ;; )
  {
    ...
    if( ( VS_VSID_SUB1 != params->vsid ) &&
     ...
     {
       MSG_2( MSG_SSID_DFLT, MSG_LEGACY_HIGH,
             "VS: CMD_OPEN: VSID=(0x%08x), network_id=(0x%08x)", params->vsid, params->network_id);
     }
    ...
    else
    {
      ...
      *params->ret_handle = session_obj->header.handle;
       ...
       MSG_1( MSG_SSID_DFLT, MSG_LEGACY_HIGH,
              "VS: CMD_OPEN: SUCCESS: handle=(0x%08x)", session_obj->header.handle);
...
)
{
  ...
  for ( ;; )
  {
    ...
    switch ( cmd_params->param_id )
    {
      ...
      case VS_PARAM_VOC_TIMING_REFERENCES_V2:
        rc = vs_voc_set_timing_reference_v2( session_obj, (vs_param_voc_timing_reference_v2_t*)cmd_params->param_data );
        break;
        
      case VS_PARAM_VOC_TIMELINE_RESYNC:
        rc = vs_voc_set_timeline_resync( session_obj, (vs_param_voc_timeline_resync_t*)cmd_params->param_data );
        break;
      ...
      case VS_PARAM_ENC_DTX: //can be removed: Think and confirm
      ...
      case VS_PARAM_TX_DTMF_GEN:
        rc = vs_voc_set_tx_dtmf_gen(session_obj, (vs_vocdtmf_frame_data_t*) cmd_params->param_data);
        break;
      default:
        rc = VS_EUNSUPPORTED;
        break;
...
)
{
  ...
  for ( ;; )
  {
    ...
    cmd_params = GPR_PKT_GET_PAYLOAD( vs_voc_cmd_enable_t, ctrl->packet );
    if ( NULL == cmd_params )
    {
      VS_REPORT_FATAL_ON_ERROR( VS_EUNEXPECTED );
      rc = VS_EFAILED;
    ...
    MSG_2( MSG_SSID_DFLT, MSG_LEGACY_HIGH,
           "VS: CMD_ENABLE: handle=(0x%08x), token=(0x%08x)", cmd_params->handle, cmd_params->token);
    ...
    if ( ctrl->state == VS_GATING_CMD_STATE_ENUM_EXECUTE )
    {
      ...
      if ( FALSE == session_obj->is_vcpm_opened )
      {
        MSG_1( MSG_SSID_DFLT, MSG_LEGACY_HIGH,
               "VS: session Handle=(0X%08X) VCPM graph is not opened, skipping state machine trigger",
               cmd_params->handle );
        rc = VS_EOK;
        break;
      }
  ...
  if( VS_EPENDING != rc )
  {
    ...
    if( session_obj != NULL )
    {
      session_obj->is_vs_enabled = TRUE;
    }
    vs_common_send_event( session_handle, VS_COMMON_EVENT_CMD_RESPONSE,
                          &cmd_rsp, sizeof( vs_common_event_cmd_response_t ) );
...
)
{
  ...
  for ( ;; )
  {
    ...
    cmd_params = GPR_PKT_GET_PAYLOAD( vs_voc_cmd_disable_t, ctrl->packet );
    if ( NULL == cmd_params )
    {
      VS_REPORT_FATAL_ON_ERROR( VS_EUNEXPECTED );
      rc = VS_EFAILED;
    ...
    MSG_2( MSG_SSID_DFLT, MSG_LEGACY_HIGH,
           "VS: CMD_DISABLE: handle=(0x%08x), token=(0x%08x)", cmd_params->handle, cmd_params->token);
...
)
{
  ...
  for ( ;; )
  {
    ...
    cmd_params = GPR_PKT_GET_PAYLOAD( vs_voc_cmd_close_t, ctrl->packet );
    if ( NULL == cmd_params )
    {
      VS_REPORT_FATAL_ON_ERROR( VS_EBADPARAM );
      rc = VS_EFAILED;
    ...
    MSG_2( MSG_SSID_DFLT, MSG_LEGACY_HIGH,
           "VS: CMD_CLOSE: handle=(0x%08x), token=(0x%08x)", cmd_params->handle, cmd_params->token);
    ...
    if ( ctrl->state == VS_GATING_CMD_STATE_ENUM_EXECUTE )
    {
      ...
      {
        MSG_1( MSG_SSID_DFLT, MSG_LEGACY_HIGH,
               "VS: session Handle=(0X%08X) CLOSE cannot be processed as ADSP is Down",
               session_handle );
        rc = VS_EOK;
        VS_RELEASE_LOCK( session_obj->session_lock );
        break;
...
)
{
  ...
  for ( ;; )
  {
    ...
    vs_timer_stop( vs_adsp_timeout_timer );
    for( index1 = 0; index1 < VS_MAX_NUM_SESSIONS_V; index1++ )
    {
      session_obj = vs_session_obj_list[index1];
      if( NULL == session_obj )
      {
         continue;
      }
      ...
      {
         MSG( MSG_SSID_DFLT, MSG_LEGACY_LOW, "vs_voc_cleanup_adsp_reference_cmd_ctrl(): VFR is not allocated, skipping release" );
      }

      //clearing subgraph info and tag ID info
      for ( index2 = 0; index2 < VS_VCPM_MAX_NUM_MODULES ; index2++)
      {
         session_obj->mod_iid_tag_info[index2].mod_iid = VS_VOICE_MOD_DEFAULT_IID;
      }
    
      for(index2 = 0; index2 < VS_VCPM_MAX_NUM_SUBGRAPHS ; index2++)
      {
         session_obj->subgraph_info[index2].is_stream_subgraph = FALSE;
         session_obj->subgraph_info[index2].is_subgraph_started = FALSE;
         session_obj->subgraph_info[index2].sub_graph_id = 0xFFFFFFFF;
      }
...
)
{
  ...
  vs_pktexg_buffer_t* temp_buf = NULL;
  uint8_t* frame=NULL;
  ...
  else
  {
    ...
    {
      /* Request 100us latency vote */
      if ( NULL != vs_npa_sleep_latency_node_handle )
      {
        npa_issue_required_request(vs_npa_sleep_latency_node_handle, 100);
      }
    }
#endif
    ...
    if( TRUE == vs_platform_is_fusion )
    {
      /* Remove 100us latency vote */
      if ( NULL != vs_npa_sleep_latency_node_handle )
      {
        npa_complete_request( vs_npa_sleep_latency_node_handle );
      }
    }
#endif
    if ( ( VS_EOK == rc) && (NULL != *mboxBuffer) )
    {
       (void) vs_voc_map_media_id_encoder_to_vs((vs_pktexg_buffer_t*)*mboxBuffer, session_obj->vocoder.media_id);
       frame = ((uint8_t*)*mboxBuffer) + sizeof( vs_pktexg_buffer_t );
       temp_buf = (vs_pktexg_buffer_t*)*mboxBuffer;
       MSG_3( MSG_SSID_DFLT, MSG_LEGACY_HIGH,
            "vs_get_encoder_buffer_cmd_ctrl(): status_mask[%d] frame data 0x%08x, media ID 0x%08x", (*mboxBuffer)->status_mask, *frame, temp_buf->media_id );
    }
  }
...
)
{
  ...
  else
  {
    ...
    if ( ( TRUE == vs_platform_is_fusion ) && ( FALSE == session_obj->is_stream_ready ) )
    {
      rc = VS_ENOTREADY;
      MSG( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
           "VS: VS_PKTEXG_CMD_RETURN_ENCODER_BUFFER called when is_stream_ready(=0) and vs_platform_is_fusion(=1) ");
    }
    else
    {
#if !defined ( AVS_MPSS_TEST ) && !defined ( WINSIM ) && !defined ( SLEEP_NPA_STUB )
...

{
  ...
  mboxBuffer = (vs_pktexg_buffer_t**)cmd_params->buffer;
  /*if(NULL == session_obj->vocoder.mbox_config.freeDecBufferQ )
  {
    *mboxBuffer= NULL;
  ...
  {
    temp_list = &session_obj->vocoder.mbox_config.freeDecBufferQ;
    decItem = (vs_shmem_block_list_item_t*)(temp_list->dummy.next);
  }*/
  rc = vs_get_list_peak_head( &session_obj->vocoder.mbox_config.freeDecBufferQ,
...
)
{
  ...
  if ( ( TRUE == vs_platform_is_fusion ) && ( FALSE == session_obj->is_stream_ready ) )
  {
    rc = VS_ENOTREADY;
    MSG( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
         "VS: VS_PKTEXG_CMD_PUT_DECODER_BUFFER called when is_stream_ready(=0) and vs_platform_is_fusion(=1) ");
  }
  else
  {
#if !defined ( AVS_MPSS_TEST ) && !defined ( WINSIM ) && !defined ( SLEEP_NPA_STUB )
...
#endif
    rc = vs_voc_map_media_id_vs_to_decoder(mboxBuffer);
    rc = vs_voc_action_put_mailbox_dec_request( session_obj );

    MSG_5( MSG_SSID_DFLT, MSG_LEGACY_HIGH,
           "vs_put_decoder_buffer_cmd_ctrl():  minor version 0x%08x, timestamp_us 0x%08x, status_mask 0x%08x, media_id 0x%08x, frame_size 0x%08x", 
          mboxBuffer->minor_version, mboxBuffer->timestamp_us, mboxBuffer->status_mask, mboxBuffer->media_id, mboxBuffer->frame_size );

    MSG_2( MSG_SSID_DFLT, MSG_LEGACY_LOW,
           "vs_put_decoder_buffer_cmd_ctrl():  size of vs_pktexg_buffer_t 0x%08x, size of mailbox_voc_packet_t 0x%08x", sizeof(vs_pktexg_buffer_t), sizeof(mailbox_voc_packet_t) );
#if !defined ( AVS_MPSS_TEST ) && !defined ( WINSIM ) && !defined ( SLEEP_NPA_STUB )
    if( TRUE == vs_platform_is_fusion )
    {
      /* Remove 100us latency vote */
...
)
{
  ...
  if ( ( TRUE == vs_platform_is_fusion ) && ( FALSE == session_obj->is_stream_ready ) )
  {
    rc = VS_ENOTREADY;
    MSG( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
         "VS: VS_PKTEXG_CMD_FLUSH_BUFFERS called when is_stream_ready(=0) and vs_platform_is_fusion(=1) ");
  }
  else
  {
#if !defined ( AVS_MPSS_TEST ) && !defined ( WINSIM ) && !defined ( SLEEP_NPA_STUB )
...
)
{
  ...
  param_id_ltetty_char_t *tty_payload = NULL;
  ...
  tty_params.dst_port = session_obj->mod_iid_tag_info[VS_MOD_RX_LTETTY].mod_iid ; ;
  tty_params.token = simple_job->header.handle;
  tty_params.opcode = APM_CMD_SET_CFG;
  tty_params.payload_size = sizeof(apm_cmd_header_t) + + sizeof(apm_module_param_data_t) + 
                             sizeof(param_id_ltetty_char_t) ;
  ...
  cmd_header->payload_size = sizeof(apm_module_param_data_t) + sizeof(param_id_ltetty_char_t);

  param_data = (apm_module_param_data_t*)( (uint8_t*)cmd_header + sizeof(apm_cmd_header_t) );
  param_data->module_instance_id = session_obj->mod_iid_tag_info[VS_MOD_RX_LTETTY].mod_iid ;
  param_data->param_id = PARAM_ID_LTETTY_CHAR;
  param_data->param_size = sizeof(param_id_ltetty_char_t);

  tty_payload = (param_id_ltetty_char_t*)( (uint8_t*)param_data + sizeof(apm_module_param_data_t) );
  tty_payload->tty_char = cmd_params->tty_char;
  ...
  if ( rc )
  {
    VS_REPORT_FATAL_ON_ERROR(rc);
    MSG_1( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
           "VS:vs_push_oobtty_char_cmd_ctrl(): Failed to communicate with RX TTY module: rc=0x%08x", rc );
  ...
  MSG( MSG_SSID_DFLT, MSG_LEGACY_HIGH,
          "VS:vs_push_oobtty_char_cmd_ctrl(): Pushed LTE TTY character in downlink" );
