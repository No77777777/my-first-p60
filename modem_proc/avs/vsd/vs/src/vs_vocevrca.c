...
/*
  ...
  ============================================================================
  $Header: //components/rel/avs.mpss/9.0.c51/vsd/vs/src/vs_vocevrca.c#1 $
  $Author: pwbldsvc $
  ============================================================================
*/
...
)
{
  ...
  param_id_vocoder_rate_modulation_param_t *enc_rate;
  uint32_t paramSize = sizeof(param_id_vocoder_rate_modulation_param_t);
  ...
  param_data->param_id = PARAM_ID_VOCODER_RATE_MODULATION;
  param_data->param_size = paramSize;

  enc_rate = (param_id_vocoder_rate_modulation_param_t*)( (uint8_t*)param_data + sizeof(apm_module_param_data_t) );

  switch ( session_obj->vocoder.encoder_rate )
  {
    case VS_VOCEVRCA_AVG_RATE_COP_0:
    case VS_VOCEVRCA_AVG_RATE_COP_4:
      enc_rate->rate_modulation_param = 0x0000;
      break;

     case VS_VOCEVRCA_AVG_RATE_COP_1:
      enc_rate->rate_modulation_param = 0x000F;
       break;

     case VS_VOCEVRCA_AVG_RATE_COP_2:
      enc_rate->rate_modulation_param = 0x0007;
       break;

     case VS_VOCEVRCA_AVG_RATE_COP_3:
      enc_rate->rate_modulation_param = 0x0005;
       break;
  }
  ...
  if ( rc )
  {
    ...
    MSG_1( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
           "VS_VOCEVRCA: Failed to communicate with encoder module while setting encoder rate: rc=0x%08x", rc );
...
)
{
  ...
  session_obj->vocoder.enc_module_id = VS_VOCEVRCA_ENC_MODULE_ID;
  session_obj->vocoder.dec_module_id = VS_VOCEVRCA_DEC_MODULE_ID;
