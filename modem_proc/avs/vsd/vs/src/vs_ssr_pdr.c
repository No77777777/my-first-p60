...
/* 
   ...
   $Header: //components/rel/avs.mpss/9.0.c51/vsd/vs/src/vs_ssr_pdr.c#1 $
   $Author: pwbldsvc $
*/
...
unsigned long long last_vs_adsp_audio_pdr_down_timestamp = 0;
unsigned long long last_vs_adsp_audio_pdr_up_timestamp = 0;

/****************************************************************************
 * EXTERN variables across VS should be defined here.
 ****************************************************************************/

extern ar_osal_signal_t vs_thread_control_event;
...
ar_osal_thread_t vs_pdr_handle;
...
/*Flag to indicate that early PD down notification is received.*/
bool_t vs_early_pd_down = FALSE;
...
static void vs_pdr_adsp_before_shutdown_handler ( void )
{
  ...
  MSG( MSG_SSID_DFLT, MSG_LEGACY_HIGH,
       "VS: handler for adsp_pdr_event_before_shutdown called." );

  vs_adsp_audio_pdr_count++;
  ...
  last_vs_adsp_audio_pdr_down_timestamp = (uint64_t)((qurt_sysclock_get_hw_ticks()) *10ull/192ull);

  vs_cvd_cleanup_is_done = FALSE;
  vs_cvd_service_is_up = FALSE;
  vs_adsp_timeout_is_triggered = FALSE;
  vs_adsp_mbox_memory_config_is_received = FALSE; 
...
static void vs_pdr_adsp_after_powerup_handler ( void )
{
   ...
   * supported and target supports PD-Restart and GPR Service UP/DOWN events.
   *
   * Indicates VCPM Services are actually up post PDR hence made TRUE
   *
   */
  ...
  last_vs_adsp_audio_pdr_up_timestamp = (uint64_t)((qurt_sysclock_get_hw_ticks()) *10ull/192ull);
...
)
{
  ...
  switch (cvd_state)
  {
    ...
    case SERVREG_SERVICE_STATE_EARLY_DOWN:
      vs_cvd_service_is_up = FALSE;
      vs_early_pd_down = TRUE;
      MSG( MSG_SSID_DFLT, MSG_LEGACY_HIGH, 
             "PDR registartion succesfull, curent audio service state is EARLY DOWN" );
      break;
  ...
  for (;;)
  {
    ...
    if ( CVD_REMOTE_SERVICE_STATUS_MASK & signal )
    {
      ...
      switch (cvd_state)
      {
        case SERVREG_SERVICE_STATE_EARLY_DOWN:
          MSG( MSG_SSID_DFLT, MSG_LEGACY_HIGH,
               "VS: recieved SERVREG_SERVICE_STATE_EARLY_DOWN for CVD. " );
        ...
        case SERVREG_SERVICE_STATE_DOWN:
          ...
          if (vs_early_pd_down == TRUE)
          {
             MSG( MSG_SSID_DFLT, MSG_LEGACY_HIGH, "VS: Processing the PDR DOWN after EARLY PDR DOWN notification" );  
             transaction_id = servreg_get_transaction_id( remote_cvd_mon_handle );
             ret = servreg_set_ack( remote_cvd_mon_handle, transaction_id ); 
             MSG_2( MSG_SSID_DFLT, MSG_LEGACY_HIGH,
                 "VS: AUDIO_PDR_DOWN_HANDLING: transactionId[%d], acknowledgement status[0x%08x]",
                 transaction_id, ret );
             vs_early_pd_down = FALSE;
             break;
          } 
          vs_pdr_adsp_before_shutdown_handler();
          break;
...
{
  MSG( MSG_SSID_DFLT, MSG_LEGACY_HIGH,
       "VS: handler for adsp_ssr_event_after_powerup called." );
...
)
{
  ...
  MSG( MSG_SSID_DFLT, MSG_LEGACY_HIGH,
       "VS: handler for adsp_ssr_event_before_shutdown called." );
...
)
{
  ...
  else
  {
    ...
    /* thread attributes*/
     thread_attr.thread_name = VS_PDR_TASK_NAME;
     thread_attr.priority = TASK_PRIORITY( priority );
     thread_attr.stack_size = stack_size;

    rc = (uint32_t)ar_osal_thread_create( &vs_pdr_handle, &thread_attr,vs_pdr, NULL );
