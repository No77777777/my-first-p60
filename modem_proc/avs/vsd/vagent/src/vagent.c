/*
  ...
  $Header: //components/rel/avs.mpss/9.0.c51/vsd/vagent/src/vagent.c#1 $
   $Author: pwbldsvc $
*/
...
static ar_osal_mutex_t vagent_int_lock;
static ar_osal_signal_t vagent_thread_create_event;

static gpr_list_t vagent_work_pkt_q;
static gpr_list_t vagent_free_work_pkt_q;
static vagent_work_item_t vagent_work_pkts[ VAGENT_NUM_WORK_PKTS_V ];

static ar_osal_signal_t vagent_work_event;
static ar_osal_thread_t vagent_thread;
...
{
  ( void )ar_osal_mutex_lock( vagent_int_lock );
}

static void vagent_int_unlock_fn ( void )
{
  ( void )ar_osal_mutex_unlock( vagent_int_lock );
}
...
{
  ( void )ar_osal_signal_set( vagent_work_event );
}
 ...
 */
static uint32_t vagent_queue_gpr_packet (
  gpr_packet_t* packet,
  gpr_list_t* cmd_q
)
{
  ...
  if( packet == NULL )
  {
    VAGENT_REPORT_FATAL_ON_ERROR( VS_EUNEXPECTED );
    return VS_EFAILED;
  }
  ...
  for ( ;; )
  {
    ...
    rc = gpr_list_remove_head( &vagent_free_work_pkt_q, (gpr_list_node_t**)&work_item );
    if ( NULL == work_item )
    {
      VAGENT_REPORT_FATAL_ON_ERROR(VS_ENORESOURCE);
      break;
    }
    ...
    if ( rc )
    {
      MSG_2( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
             "VAGENT: error in queuing GPR packet: opcode[0x%08x], rc[0x%08x]",
             packet->opcode, rc );
      work_item->packet = NULL;
      ( void ) gpr_list_add_tail( &vagent_free_work_pkt_q, &work_item->link );
    }
    else
    {
...
)
{
  ...
  for ( ;; )
  {
    pkt_params.src_domain_id = VAGENT_NULL_V ;
    pkt_params.src_port = VAGENT_NULL_V;
    pkt_params.dst_domain_id = VAGENT_NULL_V ; 
    pkt_params.dst_port = VAGENT_NULL_V ;
    ...
    if ( rc )
    {
      MSG_1( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
             "VAGENT: GPR packet allocation failed with rc=[0x%08x]", rc );
      break;
    }
    ...
    rc = vagent_queue_gpr_packet( packet, &vagent_work_pkt_q );
    if ( rc )
    {
      MSG_1( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
             "VAGENT: GPR packet queuing failed. rc[0x%08x]", rc );
      rc = __gpr_cmd_free( packet );
      VAGENT_REPORT_FATAL_ON_ERROR(rc);
    }
        ...
        if( ( CM_CALL_EVENT_PROGRESS_INFO_IND == call_event ) &&
            ( ( CM_CALL_MODE_INFO_GW_CS != call_info_ptr->mode_info.info_type ) ||
              ( USER_RESUMED != call_info_ptr->mode_info.info.gw_cs_call.call_progress ) ||
               (CM_CONNECTED_CALL_STATUS_CONNECTED == cm_event_params.call_state ) ) )
        {
...
)
{
  ...
  for ( ;; )
  {
    ...
    if ( ( session_obj->vagent_resources > 0 ) &&
         ( VAGENT_VOICE_CLIENT_INVALID == session_obj->current_module ) )
    {
      ...
      session_obj->vagent_resources--;

      MSG_1( MSG_SSID_DFLT, MSG_LEGACY_HIGH,
             "VAGENT: available vagent_num_resources[%d]", session_obj->vagent_resources );
      break;
    }
...
)
{
  ...
  for ( ;; )
  {
    ...
    evt_params = GPR_PKT_GET_PAYLOAD( vagent_eventi_cm_subs_info_t, packet );
    if( evt_params == NULL )
    {
      VAGENT_REPORT_FATAL_ON_ERROR( VS_EBADPARAM );
      break;
    }
...
)
{
  ...
  for ( ;; )
  {
    ...
    evt_params = GPR_PKT_GET_PAYLOAD( vagent_eventi_cm_call_event_t, packet );
    if ( NULL == evt_params )
    {
      VAGENT_REPORT_FATAL_ON_ERROR( VS_EBADPARAM );
      break;
    ...
    MSG_6( MSG_SSID_DFLT, MSG_LEGACY_MED,
           ...
           session_obj->state, evt_params->call_state, session_obj->vagent_resources );
...
)
{
  ...
  for ( ;; )
  {
    ...
    cmd_params = GPR_PKT_GET_PAYLOAD( vagent_ivoice_cmd_resource_request_t, packet );
    if ( NULL == cmd_params )
    {
      VAGENT_REPORT_FATAL_ON_ERROR( VS_EBADPARAM );
      break;
...
)
{
  ...
  for ( ;; )
  {
    ...
    cmd_params = GPR_PKT_GET_PAYLOAD( vagent_ivoice_cmd_resource_release_t, packet );
    if ( NULL == cmd_params )
    {
      VAGENT_REPORT_FATAL_ON_ERROR( VS_EBADPARAM );
      break;
    ...
    MSG_SPRINTF_5( MSG_SSID_DFLT, MSG_LEGACY_HIGH,
                   ...
                   session_obj->asid, session_obj->vsid, session_obj->state,
                   vagent_client_enum_table[cmd_params->client_id], session_obj->vagent_resources );
...
)
{
  ...
  while( gpr_list_remove_head( &vagent_work_pkt_q,
...
static void vagent_worker_thread_fn (
  void* param
)
{
  ...
  ar_osal_signal_create( &vagent_work_event );
  ar_osal_signal_set( vagent_thread_create_event );

  for ( ;; )
  {
    rc = ar_osal_signal_wait( vagent_work_event );
	rc |= ar_osal_signal_clear( vagent_work_event );
    if ( rc )
    {
      break;
    }
  ...
  ar_osal_signal_destroy( vagent_work_event );
  ar_osal_signal_set( vagent_thread_create_event );
...
)
{
  ...
  {  /* Initialize the locks. */
    rc = ar_osal_mutex_create( &vagent_int_lock );
    rc |= ar_osal_signal_create( &vagent_thread_create_event );
  }

  { /* Initialize free and nongating work pkt queues. */
    rc = gpr_list_init_v2( &vagent_free_work_pkt_q, 
                           vagent_int_lock_fn, vagent_int_unlock_fn );
    for ( index = 0; index < VAGENT_NUM_WORK_PKTS_V; ++index )
    {
      ( void ) gpr_list_init_node( ( gpr_list_node_t* ) &vagent_work_pkts[index] );
      vagent_work_pkts[index].packet = NULL;
      rc = gpr_list_add_tail( &vagent_free_work_pkt_q,
                              ( ( gpr_list_node_t* ) &vagent_work_pkts[index] ) );
    }

    rc = gpr_list_init_v2( &vagent_work_pkt_q,
                           vagent_int_lock_fn, vagent_int_unlock_fn );
  }
  ...
  { /* Create the GVA task worker thread. */
    ...
    /* thread attributes*/
    thread_attr.thread_name = VAGENT_TASK_NAME;
    thread_attr.priority = TASK_PRIORITY( priority );
    thread_attr.stack_size = stack_size;

    rc = ar_osal_thread_create( &vagent_thread, &thread_attr,
                            vagent_worker_thread_fn , NULL );
    VAGENT_PANIC_ON_ERROR( rc );

    ( void ) ar_osal_signal_wait( vagent_thread_create_event );
  }
...
)
{
 ...
 /* rc = __aprv2_cmd_register2(
  ...
  for( index = 0; index < VAGENT_MAX_NUM_OF_SUBSCRIPTIONS_V; ++index )
  {
    ...
    vagent_session_obj_list[index].vagent_resources = VAGENT_MAX_NUM_OF_RESOURCES_V;
...
)
{
  ...
  ( void )ar_osal_signal_destroy( vagent_work_event );
  ( void ) ar_osal_signal_wait( vagent_thread_create_event );

  /* Release work queue */
  ( void ) gpr_list_clear( &vagent_free_work_pkt_q );
  ( void ) gpr_list_clear( &vagent_work_pkt_q );

  /* Deinitialize basic OS resources for staging the setup. */
  ( void ) ar_osal_signal_destroy( vagent_thread_create_event );
  ( void ) ar_osal_mutex_destroy( vagent_int_lock );
...
)
{
  ...
  {
    VAGENT_REPORT_FATAL_ON_ERROR( VS_ENOSERVICE );
