/*
   ...
   $Header: //components/rel/avs.mpss/9.0.c51/vsd/vadapter/src/cva_module.c#1 $
   $Author: pwbldsvc $
*/
...
static ar_osal_mutex_t cva_int_lock;
static ar_osal_mutex_t cva_thread_lock;
static ar_osal_signal_t cva_control_event;

static vs_memmgr_type cva_heapmgr;
static uint8_t cva_heap_pool[ CVA_HEAP_SIZE_V ];

static vs_objmgr_t cva_objmgr;
static vs_objmgr_object_t cva_object_table[ CVA_MAX_OBJECTS_V ];

static cva_gating_control_t cva_gating_work_pkt_q;
static gpr_list_t cva_nongating_work_pkt_q;
static gpr_list_t cva_free_work_pkt_q;
static cva_work_item_t cva_work_pkts[ CVA_NUM_WORK_PKTS_V ];

static ar_osal_signal_t cva_work_event;
static ar_osal_thread_t cva_thread;
...
static ar_osal_mutex_t cva_global_lock;
...
{
  ( void )ar_osal_mutex_lock( cva_int_lock );
}

static void cva_int_unlock_fn ( void )
{
  ( void )ar_osal_mutex_unlock( cva_int_lock );
}

static void cva_thread_lock_fn ( void )
{
  ( void )ar_osal_mutex_lock( cva_thread_lock );
}

static void cva_thread_unlock_fn ( void )
{
  ( void )ar_osal_mutex_unlock( cva_thread_lock );
}

static void cva_signal_run ( void )
{
  ( void )ar_osal_signal_set( cva_work_event );
}
 ...
 */
static uint32_t cva_queue_gpr_packet (
  gpr_packet_t* packet,
  gpr_list_t* cmd_q
)
{
  ...
  for ( ;; )
  {
    ...
    rc = gpr_list_remove_head( &cva_free_work_pkt_q, (gpr_list_node_t**)&work_item );
    if ( NULL == work_item )
    {
    ...
    if ( rc )
    {
      MSG_2( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
             "CVA: error in queuing GPR packet: opcode[0x%08x], rc[0x%08x]", packet->opcode, rc );
      work_item->packet = NULL;
      ( void ) gpr_list_add_tail( &cva_free_work_pkt_q, &work_item->link );
    }
    else
    {
      /**
...
)
{
  ...
  rc = cva_queue_gpr_packet( packet, &cva_nongating_work_pkt_q );
...
)
{
  ...
  for ( ;; )
  {
    pkt_params.src_domain_id = CVA_NULL_V ;
    pkt_params.src_port = CVA_NULL_V;
    pkt_params.dst_domain_id = CVA_NULL_V ; 
    pkt_params.dst_port = CVA_NULL_V ;
    ...
    if ( rc )
    {
      MSG_1( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
             "CVA: GPR packet allocation failed with rc=[0x%08x]", rc );
      break;
    }
    ...
    {
      MSG_1( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
             "CVA: GPR packet queuing failed. rc[0x%08x]", rc );
      rc = __gpr_cmd_free( packet );
      CVA_REPORT_FATAL_ON_ERROR(rc);
    }
...
)
{
  ...
  { /* Allocate memory for the CVA object. */
    cva_obj = vs_memmgr_malloc( &cva_heapmgr, size );
    if ( cva_obj == NULL )
    {
      MSG_1( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
    ...
    rc = vs_objmgr_alloc_object( &cva_objmgr, &objmgr_obj );
    if ( rc )
    {
      MSG_1( MSG_SSID_DFLT, MSG_LEGACY_ERROR, 
             "cva_mem_alloc_object(): Out of objects, rc = (0x%08X)", rc );
      vs_memmgr_free( &cva_heapmgr, cva_obj );
      return VS_ENORESOURCE;
    }

    /* Use the custom object type. */
    objmgr_obj->ptr = cva_obj;
...
)
{
  ...
  ( void ) vs_objmgr_free_object( &cva_objmgr, object->header.handle );
  vs_memmgr_free( &cva_heapmgr, object );
  ...
  cva_session_object_t** ret_session_obj )
{
  ...
  { /* Initialize the simple job object. */
    ...
    session_obj->onex_handle = CVA_NULL_V;
    ...
    session_obj->vs_handle = CVA_NULL_V;
...
)
{
  ...
  if ( cva_is_initialized == FALSE ) return VS_EOK;
...
)
{
  ...
  if ( CVA_NULL_V == session_obj->vs_handle ) return VS_EOK;
...
)
{
  ...
  if ( session_obj->vs_handle == CVA_NULL_V ) return VS_EOK;
    ...
    switch ( session_obj->vocoder_type )
    {
      case ONEX_VOC_ID_EVRCA:
        MSG_1( MSG_SSID_DFLT, MSG_LEGACY_LOW,
               "CVA: EVRC-A: DELIVER_UL_PACKET: FrameRate=(%d)", frame_rate );
        break;

      case ONEX_VOC_ID_EVRCB:
        MSG_1( MSG_SSID_DFLT, MSG_LEGACY_LOW,
               "CVA: EVRC-B: DELIVER_UL_PACKET: FrameRate=(%d)", frame_rate );
        break;

      case ONEX_VOC_ID_EVRCNW:
        MSG_1( MSG_SSID_DFLT, MSG_LEGACY_LOW,
               "CVA: EVRC-NW: DELIVER_UL_PACKET: FrameRate=(%d)", frame_rate );
        break;

      case ONEX_VOC_ID_EVRCNW2K:
        MSG_1( MSG_SSID_DFLT, MSG_LEGACY_LOW,
               "CVA: EVRC-NW2K: DELIVER_UL_PACKET: FrameRate=(%d)", frame_rate );
        break;

      case ONEX_VOC_ID_EVRCWB:
        MSG_1( MSG_SSID_DFLT, MSG_LEGACY_LOW,
               "CVA: EVRC-WB: DELIVER_UL_PACKET: FrameRate=(%d)", frame_rate );
        break;

      case ONEX_VOC_ID_QCELP13K:
        MSG_1( MSG_SSID_DFLT, MSG_LEGACY_LOW,
               "CVA: QCELP-13K: DELIVER_UL_PACKET: FrameRate=(%d)", frame_rate );
        break;
    }
...
)
{
  ...
  for( ;; )
  {
    ...
    switch ( session_obj->vocoder_type )
    {
      case ONEX_VOC_ID_EVRCA:
        MSG_1( MSG_SSID_DFLT, MSG_LEGACY_LOW,
               "CVA: EVRC-A: GET_DL_PACKET: FrameRate=(%d)", frame_rate );
        break;

      case ONEX_VOC_ID_EVRCB:
        MSG_1( MSG_SSID_DFLT, MSG_LEGACY_LOW,
               "CVA: EVRC-B: GET_DL_PACKET: FrameRate=(%d)", frame_rate );
        break;

      case ONEX_VOC_ID_EVRCNW:
        MSG_1( MSG_SSID_DFLT, MSG_LEGACY_LOW,
               "CVA: EVRC-NW: GET_DL_PACKET: FrameRate=(%d)", frame_rate );
        break;

      case ONEX_VOC_ID_EVRCNW2K:
        MSG_1( MSG_SSID_DFLT, MSG_LEGACY_LOW,
               "CVA: EVRC-NW2K: GET_DL_PACKET: FrameRate=(%d)", frame_rate );
        break;

      case ONEX_VOC_ID_EVRCWB:
        MSG_1( MSG_SSID_DFLT, MSG_LEGACY_LOW,
               "CVA: EVRC-WB: GET_DL_PACKET: FrameRate=(%d)", frame_rate );
        break;

      case ONEX_VOC_ID_QCELP13K:
        MSG_1( MSG_SSID_DFLT, MSG_LEGACY_LOW,
               "CVA: QCELP-13k: GET_DL_PACKET: FrameRate=(%d)", frame_rate );
        break;
    }
...
)
{
  ...
  if ( ( VS_EOK != rc ) && ( APR_EUNSUPPORTED != rc ) ) 
  {
    MSG_2( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
...
)
{
  ...
  for ( ;; )
  {
    ...
    if( CVA_NULL_V != session_obj->vs_handle )
    {
      flushCmd.handle = session_obj->vs_handle;
...
)
{
  ...
  for( ;; )
  {
    ...
    evt_params = GPR_PKT_GET_PAYLOAD( onex_voc_event_rqst_dtx_mode_t, packet );
    if( evt_params == NULL )
    {
      CVA_REPORT_FATAL_ON_ERROR ( VS_EUNEXPECTED );
      break;
...
)
{
  ...
  for( ;; )
  {
    ...
    evt_params = GPR_PKT_GET_PAYLOAD( onex_voc_event_rqst_min_max_rate_t, packet );
    if( evt_params == NULL )
    {
      CVA_REPORT_FATAL_ON_ERROR ( VS_EUNEXPECTED );
      break;
...
)
{
  ...
  for( ;; )
  {
    ...
    evt_params = GPR_PKT_GET_PAYLOAD( onex_voc_event_rqst_avg_enc_rate_t, packet );
    if( evt_params == NULL )
    {
      CVA_REPORT_FATAL_ON_ERROR ( VS_EUNEXPECTED );
      break;
...
)
{
  ...
  for ( ;; )
  {
    ...
    evt_params = GPR_PKT_GET_PAYLOAD( onex_voc_event_rqst_start_t, packet );
    if( evt_params == NULL )
    {
      CVA_REPORT_FATAL_ON_ERROR ( VS_EUNEXPECTED );
      break;
...
static void cva_task_process_nongating_work_items ( void )
{
  ...
  while( gpr_list_remove_head( &cva_nongating_work_pkt_q,
                               ( ( gpr_list_node_t** ) &work_item ) ) == VS_EOK )
  {
    packet = work_item->packet;
    if ( packet == NULL )
    {
      CVA_PANIC_ON_ERROR ( VS_EUNEXPECTED );
    }
    ...
    switch( packet->opcode )
    {
     ...
     default:
       ...
       rc = cva_queue_gpr_packet ( packet, &cva_gating_work_pkt_q.cmd_q );
       if ( rc )
       {
         MSG_1( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
...
static void cva_worker_thread_fn (
  void* param
)
{
  int32_t rc;

  ar_osal_signal_create( &cva_work_event );
  ar_osal_signal_set( cva_control_event );

  for ( ;; )
  {
    rc = ar_osal_signal_wait( cva_work_event );
    rc |= ar_osal_signal_clear( cva_work_event ); 
    if ( rc ) break;

    cva_run( );
  }

  ar_osal_signal_destroy( cva_work_event );
  ar_osal_signal_set( cva_control_event );
...
static int32_t cva_init ( void )
{
  ...
  {  /* Initialize the locks. */
    rc = ar_osal_mutex_create( &cva_int_lock );
    rc |= ar_osal_mutex_create( &cva_thread_lock );
    rc |= ar_osal_signal_create( &cva_control_event );
  }

  { /* Initialize the custom heap. */
    vs_memmgr_init_heap( &cva_heapmgr, ( ( void* ) &cva_heap_pool ),
                          sizeof( cva_heap_pool ), NULL );
  }
  ...
  { /* Initialize the object manager. */
    ...
    rc = vs_objmgr_construct( &cva_objmgr, &params );
  }

  { /* Initialize free and nongating work pkt queues. */
    rc = gpr_list_init_v2( &cva_free_work_pkt_q, 
                           cva_int_lock_fn, cva_int_unlock_fn );
    for ( index = 0; index < CVA_NUM_WORK_PKTS_V; ++ index  )
    {
      ( void ) gpr_list_init_node( ( gpr_list_node_t* ) &cva_work_pkts[index] );
      cva_work_pkts[index].packet = NULL;
      rc = gpr_list_add_tail( &cva_free_work_pkt_q,
                              ( ( gpr_list_node_t* ) &cva_work_pkts[index] ) );
    }

    rc = gpr_list_init_v2( &cva_nongating_work_pkt_q,
                           cva_int_lock_fn, cva_int_unlock_fn );
  }
  ...
  { /* Initialize the global session lock. */
    rc = ar_osal_mutex_create( &cva_global_lock );
    CVA_PANIC_ON_ERROR( rc );
  }
  ...
  { /* Create the CVA task worker thread. */
    ...
    /* thread attributes*/
    thread_attr.thread_name = CVA_TASK_NAME;
    thread_attr.priority = TASK_PRIORITY( priority );
    thread_attr.stack_size = stack_size;

    rc = ar_osal_thread_create( &cva_thread, &thread_attr,
                             cva_worker_thread_fn , NULL );
    CVA_PANIC_ON_ERROR( rc );

    ( void ) ar_osal_signal_wait( cva_control_event );
  }
...
static int32_t cva_predeinit ( void )
{
  ...
  for( index = 0; index < CVA_MAX_NUM_OF_SESSIONS_V; ++index )
  {
    ...
    ( void ) ar_osal_mutex_destroy( cva_session_obj_list[index]->data_lock );
...
static int32_t cva_deinit ( void )
{
  ...
  ( void )ar_osal_signal_destroy( cva_work_event );
  ( void ) ar_osal_signal_wait( cva_control_event );
  ...
  /* Release work queue */
  ( void ) gpr_list_clear( &cva_free_work_pkt_q );
  ( void ) gpr_list_clear( &cva_nongating_work_pkt_q );

  /* Deinitialize the object handle table. */
  vs_objmgr_destruct( &cva_objmgr );

  /* Deinitialize basic OS resources for staging the setup. */
  ( void ) ar_osal_signal_destroy( cva_control_event );
  ( void ) ar_osal_mutex_destroy( cva_int_lock );
  ( void ) ar_osal_mutex_destroy( cva_thread_lock );
  ( void ) ar_osal_mutex_destroy( cva_global_lock );
...
)
{
  ...
  {
    CVA_REPORT_FATAL_ON_ERROR( VS_ENOSERVICE );
