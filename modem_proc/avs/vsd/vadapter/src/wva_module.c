/*
   ...
   $Header: //components/rel/avs.mpss/9.0.c51/vsd/vadapter/src/wva_module.c#1 $
   $Author: pwbldsvc $
*/
...
static ar_osal_mutex_t wva_int_lock;
static ar_osal_mutex_t wva_thread_lock;
static ar_osal_signal_t wva_control_event;

static vs_memmgr_type wva_heapmgr;
static uint8_t wva_heap_pool[ WVA_HEAP_SIZE_V ];

static vs_objmgr_t wva_objmgr;
static vs_objmgr_object_t wva_object_table[ WVA_MAX_OBJECTS_V ];

static wva_gating_control_t wva_gating_work_pkt_q;
static gpr_list_t wva_nongating_work_pkt_q;
static gpr_list_t wva_free_work_pkt_q;
static wva_work_item_t wva_work_pkts[ WVA_NUM_WORK_PKTS_V ];

static ar_osal_signal_t wva_work_event;
static ar_osal_thread_t wva_thread;
...
static ar_osal_mutex_t wva_global_lock;
...
{
  ( void )ar_osal_mutex_lock( wva_int_lock );
}

static void wva_int_unlock_fn ( void )
{
  ( void )ar_osal_mutex_unlock( wva_int_lock );
}

static void wva_thread_lock_fn ( void )
{
  ( void )ar_osal_mutex_lock( wva_thread_lock );
}

static void wva_thread_unlock_fn ( void )
{
  ( void )ar_osal_mutex_unlock( wva_thread_lock );
}

static void wva_signal_run ( void )
{
  ( void )ar_osal_signal_set( wva_work_event );
}
 ...
 */
static uint32_t wva_queue_gpr_packet (
  gpr_packet_t* packet,
  gpr_list_t* cmd_q
)
{
  ...
  for ( ;; )
  {
    ...
    rc = gpr_list_remove_head( &wva_free_work_pkt_q, (gpr_list_node_t**)&work_item );
    if ( NULL == work_item )
    {
    ...
    if ( rc )
    {
      MSG_2( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
             "WVA: error in queuing GPR packet: opcode[0x%08x], rc[0x%08x]", packet->opcode, rc );
      work_item->packet = NULL;
      ( void ) gpr_list_add_tail( &wva_free_work_pkt_q, &work_item->link );
    }
    else
    {
      /**
...
)
{
  ...
  rc = wva_queue_gpr_packet( packet, &wva_nongating_work_pkt_q );
...
)
{
  ...
  for ( ;; )
  {
    pkt_params.src_domain_id = WVA_NULL_V ;
    pkt_params.src_port = WVA_NULL_V;
    pkt_params.dst_domain_id = WVA_NULL_V ; 
    pkt_params.dst_port = WVA_NULL_V ;
    ...
    if ( rc )
    {
      MSG_1( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
             "WVA: GPR packet allocation failed with rc=[0x%08x]", rc );
      break;
    }
    ...
    {
      MSG_1( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
             "WVA: GPR packet queuing failed. rc[0x%08x]", rc );
      rc = __gpr_cmd_free( packet );
      WVA_REPORT_FATAL_ON_ERROR(rc);
    }
...
)
{
  ...
  { /* Allocate memory for the WVA object. */
    wva_obj = vs_memmgr_malloc( &wva_heapmgr, size );
    if ( wva_obj == NULL )
    {
      MSG_1( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
    ...
    rc = vs_objmgr_alloc_object( &wva_objmgr, &objmgr_obj );
    if ( rc )
    {
      MSG_1( MSG_SSID_DFLT, MSG_LEGACY_ERROR, 
             "wva_mem_alloc_object(): Out of objects, rc = (0x%08X)", rc );
      vs_memmgr_free( &wva_heapmgr, wva_obj );
      return VS_ENORESOURCE;
    }

    /* Use the custom object type. */
    objmgr_obj->ptr = wva_obj;
...
)
{
  ...
  ( void ) vs_objmgr_free_object( &wva_objmgr, object->header.handle );
  vs_memmgr_free( &wva_heapmgr, object );
  ...
  wva_session_object_t** ret_session_obj )
{
  ...
  { /* Initialize the simple job object. */
    ...
    rc = vs_timer_create( &session_obj->pktexg_timer, wva_pktexg_timer_cb, session_obj );
    WVA_PANIC_ON_ERROR( rc );

    session_obj->wcdma_handle = WVA_NULL_V;
    ...
    session_obj->vs_handle = WVA_NULL_V;
...
)
{
  ...
  if ( wva_is_initialized == FALSE ) return VS_EOK;
...
)
{
  ...
  for ( ;; )
  {
    if ( session_obj->vs_handle == WVA_NULL_V )
    {
      rc = VS_EOK;
      break;
    }
...
)
{
  ...
  if ( session_obj->vs_handle == WVA_NULL_V ) return VS_EOK;
...
)
{
  ...
  for ( ;; )
  {
    ...
    if( WVA_NULL_V != session_obj->vs_handle )
    {
...
)
{
  ...
  for ( ;; )
  {
    ...
    evt_params = GPR_PKT_GET_PAYLOAD( wcdma_ivoice_event_request_scr_mode_t, packet );
    if ( NULL == evt_params )
    {
      WVA_REPORT_FATAL_ON_ERROR( VS_EUNEXPECTED );
      rc = VS_EFAILED;
...
)
{
  ...
  for ( ;; )
  {
    ...
    evt_params = GPR_PKT_GET_PAYLOAD( wcdma_ivoice_event_request_codec_mode_t, packet );
    if ( NULL == evt_params )
    {
      WVA_REPORT_FATAL_ON_ERROR( VS_EUNEXPECTED );
      rc = VS_EFAILED;
...
)
{
  ...
  for ( ;; )
  {
    ...
    evt_params = GPR_PKT_GET_PAYLOAD( wcdma_ivoice_event_set_logical_channels_t, packet );
    if ( NULL == evt_params )
    {
      WVA_REPORT_FATAL_ON_ERROR( VS_EUNEXPECTED );
      rc = VS_EFAILED;
...
)
{
  ...
  for ( ;; )
  {
    ...
    evt_params = GPR_PKT_GET_PAYLOAD( wcdma_ivoicel1_event_vfr_notification_t, packet );
    if ( NULL == evt_params )
    {
      WVA_REPORT_FATAL_ON_ERROR( VS_EUNEXPECTED );
      rc = VS_EFAILED;
...
)
{
  ...
  for ( ;; )
  {
    ...
    {
      ( void ) voice_dsm_amr_q_empty( &session_obj->dl_queue );
      WVA_REPORT_FATAL_ON_ERROR ( VS_ENEEDMORE );
      rc = VS_EFAILED;
      break;
    }
...
)
{
  ...
  for ( ;; )
  {
    ...
    evt_params = GPR_PKT_GET_PAYLOAD( wcdma_ivoice_event_request_start_t, packet );
    if ( NULL == evt_params )
    {
      WVA_REPORT_FATAL_ON_ERROR( VS_EUNEXPECTED );
      rc = VS_EFAILED;
...
static void wva_task_process_nongating_work_items ( void )
{
  ...
  while( gpr_list_remove_head( &wva_nongating_work_pkt_q,
                               ( ( gpr_list_node_t** ) &work_item ) ) == VS_EOK )
  {
    ...
    switch( packet->opcode )
    {
      ...
      default:
        ...
        rc = wva_queue_gpr_packet ( packet, &wva_gating_work_pkt_q.cmd_q );
        if ( rc )
        {
          MSG_1( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
  ...
  for ( ;; )
  {
    switch ( ctrl->state )
    {
      case WVA_GATING_CMD_STATE_FETCH:
      {
         ...
         if ( ctrl->packet == NULL )
         {
           WVA_PANIC_ON_ERROR ( VS_EUNEXPECTED );
         }
...
static void wva_worker_thread_fn (
  void* param
)
{
  int32_t rc;

  ar_osal_signal_create( &wva_work_event );
  ar_osal_signal_set( wva_control_event );

  for ( ;; )
  {
    rc = ar_osal_signal_wait( wva_work_event );
    rc |= ar_osal_signal_clear( wva_work_event ); 
    if ( rc ) break;

    wva_run( );
  }

  ar_osal_signal_destroy( wva_work_event );
  ar_osal_signal_set( wva_control_event );
...
static int32_t wva_init ( void )
{
  ...
  {  /* Initialize the locks. */
    rc = ar_osal_mutex_create( &wva_int_lock );
    rc |= ar_osal_mutex_create( &wva_thread_lock );
    rc |= ar_osal_signal_create( &wva_control_event );
   WVA_REPORT_FATAL_ON_ERROR(rc);
  }

  { /* Initialize the custom heap. */
    vs_memmgr_init_heap( &wva_heapmgr, ( ( void* ) &wva_heap_pool ),
                          sizeof( wva_heap_pool ), NULL );
  }
  ...
  { /* Initialize the object manager. */
    ...
    rc = vs_objmgr_construct( &wva_objmgr, &params );
  }

  { /* Initialize free and nongating work pkt queues. */
    rc = gpr_list_init_v2( &wva_free_work_pkt_q, 
                           wva_int_lock_fn, wva_int_unlock_fn );
    for ( index = 0; index < WVA_NUM_WORK_PKTS_V; ++index )
    {
      ( void ) gpr_list_init_node( ( gpr_list_node_t* ) &wva_work_pkts[index] );
      wva_work_pkts[index].packet = NULL;
      rc = gpr_list_add_tail( &wva_free_work_pkt_q,
                              ( ( gpr_list_node_t* ) &wva_work_pkts[index] ) );
    }

    rc = gpr_list_init_v2( &wva_nongating_work_pkt_q,
                           wva_int_lock_fn, wva_int_unlock_fn );
  }
  ...
  { /* Initialize the global session lock. */
    rc = ar_osal_mutex_create( &wva_global_lock );
    WVA_PANIC_ON_ERROR( rc );
  }
  ...
  { /* Create the wva task worker thread. */
    ...
    thread_attr.thread_name = WVA_TASK_NAME;
    thread_attr.priority = TASK_PRIORITY( priority );
    thread_attr.stack_size = stack_size;

    rc = ar_osal_thread_create( &wva_thread, &thread_attr, 
                            wva_worker_thread_fn , NULL );
    WVA_PANIC_ON_ERROR( rc );

    ( void ) ar_osal_signal_wait( wva_control_event );
  }
...
static int32_t wva_predeinit ( void )
{
  ...
  for( index = 0; index < WVA_MAX_NUM_OF_SESSIONS_V; ++index )
  {
    ...
    ( void ) vs_timer_destroy ( wva_session_obj_list[index]->pktexg_timer );
    ( void ) ar_osal_mutex_destroy( wva_session_obj_list[index]->data_lock );
...
static int32_t wva_deinit ( void )
{
  ...
  ar_osal_signal_destroy( wva_work_event );
  ar_osal_signal_wait( wva_control_event );
  ...
  /* Release work queue */
  ( void ) gpr_list_clear( &wva_free_work_pkt_q );
  ( void ) gpr_list_clear( &wva_nongating_work_pkt_q );


  /* Deinitialize the object handle table. */
  ( void ) vs_objmgr_destruct( &wva_objmgr );

  /* Deinitialize basic OS resources for staging the setup. */
  ( void ) ar_osal_signal_destroy( wva_control_event );
  ( void ) ar_osal_mutex_destroy( wva_int_lock );
  ( void ) ar_osal_mutex_destroy( wva_thread_lock );
  ( void ) ar_osal_mutex_destroy( wva_global_lock );
...
)
{
  ...
  {
    WVA_REPORT_FATAL_ON_ERROR( VS_ENOSERVICE );
