/*
   ...
   $Header: //components/rel/avs.mpss/9.0.c51/vsd/vadapter/src/gva_module.c#1 $
   $Author: pwbldsvc $
*/
...
/* When this bit is set in the UL pkt status_mask, it indicates that VFR Resync occurred.
 * GVA is expected to lookout for multiple pkts and pick the latest one.
 */
#define GVA_RESYNC_MASK ( 2 )

/* Number of cycles that GVA needs to read multiple pkts after the occurrence of the VFR Resync.
 */
#define GVA_RESYNC_CYCLES_COUNT ( 1 )
...
static uint32_t gva_gpr_handle;

static ar_osal_mutex_t gva_int_lock;
static ar_osal_mutex_t gva_thread_lock;
static ar_osal_signal_t gva_control_event;

static vs_memmgr_type gva_heapmgr;
static uint8_t gva_heap_pool[ GVA_HEAP_SIZE_V ];

static vs_objmgr_t gva_objmgr;
static vs_objmgr_object_t gva_object_table[ GVA_MAX_OBJECTS_V ];

static gva_gating_control_t gva_gating_work_pkt_q;
static gpr_list_t gva_nongating_work_pkt_q;
static gpr_list_t gva_free_work_pkt_q;
static gva_work_item_t gva_work_pkts[ GVA_NUM_WORK_PKTS_V ];

static ar_osal_signal_t gva_work_event;
static ar_osal_thread_t gva_thread;
...
static ar_osal_mutex_t gva_global_lock;
...
{
  ( void )ar_osal_mutex_lock( gva_int_lock );
}

static void gva_int_unlock_fn ( void )
{
  ( void )ar_osal_mutex_unlock( gva_int_lock );
}

static void gva_thread_lock_fn ( void )
{
  ( void )ar_osal_mutex_lock( gva_thread_lock );
}

static void gva_thread_unlock_fn ( void )
{
  ( void )ar_osal_mutex_unlock( gva_thread_lock );
}

static void gva_signal_run ( void )
{
  ( void )ar_osal_signal_set( gva_work_event );
}
 ...
 */
static uint32_t gva_queue_gpr_packet (
  gpr_packet_t* packet,
  gpr_list_t* cmd_q
)
{
  ...
  for ( ;; )
  {
    ...
    rc = gpr_list_remove_head( &gva_free_work_pkt_q, (gpr_list_node_t**)&work_item );
    if ( NULL == work_item )
    {
    ...
    if ( rc )
    {
      MSG_2( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
             "GVA: error in queuing GPR packet: opcode[0x%08x], rc[0x%08x]", packet->opcode, rc );
      work_item->packet = NULL;
      ( void ) gpr_list_add_tail( &gva_free_work_pkt_q, &work_item->link );
    }
    else
    {
      /**
...
)
{
  ...
  void* payload;
  gpr_cmd_alloc_ext_t pkt_params;

  for ( ;; )
  {
    pkt_params.src_domain_id = GVA_NULL_V ;
    pkt_params.src_port = GVA_NULL_V;
    pkt_params.dst_domain_id = GVA_NULL_V ; 
    pkt_params.dst_port = GVA_NULL_V ;
    pkt_params.token = (uint32_t)session_context;
    pkt_params.opcode = cmd_id;
    pkt_params.payload_size = params_size;
    pkt_params.client_data = 0;
    pkt_params.ret_packet =&packet ;

    rc = __gpr_cmd_alloc_ext(&pkt_params);
    ...
    {
      payload = GPR_PKT_GET_PAYLOAD( void, packet);
      mmstd_memcpy( payload, params_size, cmd_params, params_size );
    }

    /* Queue the command packet for processing. */
    rc = gva_queue_gpr_packet( packet, &gva_nongating_work_pkt_q );
    if ( rc )
    {
      MSG_1( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
             "GVA: GPR packet queuing failed. rc[0x%08x]", rc );
      rc = __gpr_cmd_free( packet );
      GVA_REPORT_FATAL_ON_ERROR(rc);
    }
...
)
{
  ...
  { /* Allocate memory for the GVA object. */
    gva_obj = vs_memmgr_malloc( &gva_heapmgr, size );
    if ( gva_obj == NULL )
    {
      MSG_1( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
    ...
    rc = vs_objmgr_alloc_object( &gva_objmgr, &objmgr_obj );
    if ( rc )
    {
      MSG_1( MSG_SSID_DFLT, MSG_LEGACY_ERROR, 
             "gva_mem_alloc_object(): Out of objects, rc = (0x%08X)", rc );
      vs_memmgr_free( &gva_heapmgr, gva_obj );
      return VS_ENORESOURCE;
    }

    /* Use the custom object type. */
    objmgr_obj->ptr = gva_obj;
...
)
{
  ...
  ( void ) vs_objmgr_free_object( &gva_objmgr, object->header.handle );
  vs_memmgr_free( &gva_heapmgr, object );
  ...
  gva_session_object_t** ret_session_obj )
{
  ...
  { /* Initialize the simple job object. */
    ...
    rc = ar_osal_mutex_create( &session_obj->data_lock );
    ...
    session_obj->vs_handle = GVA_NULL_V;
...
)
{
  ...
  if ( gva_is_initialized == FALSE ) return VS_EOK;
...
)
{
  ...
  if ( GVA_NULL_V == session_obj->vs_handle ) return;
...
)
{
  ...
  param.version = 0x1;
  param.dtx_mode = (uint8_t) session_obj->dtx_mode;
  setParamCmd.handle = session_obj->vs_handle;
  setParamCmd.param_data = (void*)&param;
...
)
{
  ...
  while( TRUE )
  {
    if ( GVA_NULL_V == session_obj->vs_handle ) break;
    ...
    if (tempReadBuf->status_mask & GVA_RESYNC_MASK)
    {
      /* GVA will be shifting to multiple pkt read behavior for these many cycles.*/
       resync_counter = GVA_RESYNC_CYCLES_COUNT;
    }
    if ( ( VOICELOG_IEVENT_VOCODER_STATE_RUNNING == session_obj->vocoder_state ) &&
          ( resync_counter == 0) )
    {
      /* In steady state, GVA is supposed to read only one pkt per VFR cycle,
       * hence breaking out of the loop. */
       break;
    }
    tempReadBuf = NULL;
  }
...
)
{
  ...
  if ( GVA_NULL_V == session_obj->vs_handle ) return rc;
...
)
{
  ...
  if ( session_obj->vs_handle == GVA_NULL_V ) return VS_EOK;
  ...
  for( ;; )
  {
    ...
    if ( NULL != session_obj->vs_read_buf )
    {
      ...
      MSG_5( MSG_SSID_DFLT, MSG_LEGACY_LOW,
             "GVA: AMR-WB: DELIVER_UL_PACKET: Minor version[0x%08x], timestamp_us [0x%08x], status_mask [0x%08x], media ID [0x%08x], frame_size [0x%08x]",
               vsBuffer->minor_version, vsBuffer->timestamp_us, vsBuffer->status_mask, vsBuffer->media_id, vsBuffer->frame_size );
      ...
      if ( ( vsBuffer->frame_size > gsm_buffer->size ) || ( 0 == vsBuffer->frame_size ) )
      {
  ...
  for( ;; )
  {
   ...
   if ( NULL != session_obj->vs_read_buf )
   {
     ...
     MSG_5( MSG_SSID_DFLT, MSG_LEGACY_HIGH,
             "GVA: FR: DELIVER_UL_PACKET: Minor version[0x%08x], timestamp_us [0x%08x], status_mask [0x%08x], media ID [0x%08x], frame_size [0x%08x]",
              vsBuffer->minor_version, vsBuffer->timestamp_us, vsBuffer->status_mask, vsBuffer->media_id, vsBuffer->frame_size );
...
)
{
  ...
  for( ;; )
  {
    ...
    MSG_1(MSG_SSID_DFLT, MSG_LEGACY_HIGH,
           "GVA: (E)AMR: GET_DL_PACKET: Frame=(%d)",
           &frame );
...
)
{
  ...
  if(TRUE == session_obj->is_vs_ready)
  {
    ...
    {	
      ipa_dma_ch_lock( IPA_DMA_CLIENT_TYPE_AUDIO_UL, 0 );
	}
#endif
  ...
  if(TRUE == session_obj->is_vs_ready)
  {
    ...
    if ( TRUE == DalPlatformInfo_IsFusion() )
    {	
      ipa_dma_ch_unlock( IPA_DMA_CLIENT_TYPE_AUDIO_UL );
    }
#endif
  }
...
)
{
  ...
  if ( ( TRUE == session_obj->is_vs_ready ) && ( TRUE == gsm_buffer->flags ) )
  {
    ...
    if ( rc == VS_EOK )
    {
      ...
      if ( TRUE == DalPlatformInfo_IsFusion() )
      {		  
         ipa_dma_ch_lock( IPA_DMA_CLIENT_TYPE_AUDIO_DL, 0 );
      }
#endif
      ...
      if ( TRUE == DalPlatformInfo_IsFusion() )
      {	
        ipa_dma_ch_unlock( IPA_DMA_CLIENT_TYPE_AUDIO_DL );
      }
#endif
...
)
{
  ...
  if ( ( VS_EOK != rc ) && ( VS_EUNSUPPORTED != rc ) ) 
  {
    MSG_2( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
...
)
{
  ...
  for ( ;; )
  {
    ...
    evt_params = GPR_PKT_GET_PAYLOAD( vs_common_event_cmd_response_t, packet );
    if( evt_params == NULL )
    {
      GVA_REPORT_FATAL_ON_ERROR ( VS_EUNEXPECTED );
      break;
...
)
{
  ...
  for ( ;; )
  {
    ...
    if( GVA_NULL_V != session_obj->vs_handle )
    {
      flushCmd.handle = session_obj->vs_handle;
...
)
{
  ...
  for( ;; )
  {
    ...
    evt_params = GPR_PKT_GET_PAYLOAD( vs_voc_event_operating_mode_update_t, packet );
    if( evt_params == NULL )
    {
      GVA_REPORT_FATAL_ON_ERROR ( VS_EUNEXPECTED );
      break;
...
)
{
  ...
  for( ;; )
  {
    ...
    evt_params = GPR_PKT_GET_PAYLOAD( gsm_ivoice_event_request_dtx_mode_t, packet );
    if( evt_params == NULL )
    {
      GVA_REPORT_FATAL_ON_ERROR ( VS_EUNEXPECTED );
      break;
...
)
{
  ...
  for( ;; )
  {
    ...
    evt_params = GPR_PKT_GET_PAYLOAD( gsm_ivoice_event_request_codec_mode_t, packet );
    if( evt_params == NULL )
    {
      GVA_REPORT_FATAL_ON_ERROR ( VS_EUNEXPECTED );
      break;
...
{
  uint32_t rc = VS_EOK;
  gpr_packet_t* packet = ctrl->packet;
  gva_session_object_t* session_obj = NULL;
  ...
  for ( ;; )
  {
    ...
    evt_params = GPR_PKT_GET_PAYLOAD( gsm_ivoice_event_start_request_t, packet );
    if( evt_params == NULL )
    {
      rc = VS_EUNEXPECTED;
      GVA_REPORT_FATAL_ON_ERROR ( rc );
...
)
{
  ...
  for ( ;; )
  {
    ...
    event_params = GPR_PKT_GET_PAYLOAD( vagent_ivoice_event_resource_grant_t, packet );
...
)
{
  ...
  for ( ;; )
  {
    ...
    event_params = GPR_PKT_GET_PAYLOAD( vagent_ivoice_event_resource_revoke_t, packet );
    if( event_params == NULL )
    {
      GVA_REPORT_FATAL_ON_ERROR ( VS_EUNEXPECTED );
      break;
...
static void gva_task_process_nongating_work_items ( void )
{
  ...
  while( gpr_list_remove_head( &gva_nongating_work_pkt_q,
                               ( ( gpr_list_node_t** ) &work_item ) ) == VS_EOK )
  {
    ...
    switch( packet->opcode )
    {
     ...
     default:
       ...
       rc = gva_queue_gpr_packet ( packet, &gva_gating_work_pkt_q.cmd_q );
       if ( rc )
       {
         MSG_1( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
  ...
  for ( ;; )
  {
    switch ( ctrl->state )
    {
      case GVA_GATING_CMD_STATE_FETCH:
      {
        ...
        rc = gpr_list_remove_head( &ctrl->cmd_q,
                                   ( ( gpr_list_node_t** ) &work_item ) );
        if ( rc ) return;

        if( work_item->packet == NULL )
        {
          GVA_PANIC_ON_ERROR ( VS_EUNEXPECTED );
        }
...
static void gva_worker_thread_fn (
  void* param
)
{
  int32_t rc;

  ar_osal_signal_create( &gva_work_event );
  ar_osal_signal_set( gva_control_event );

  for ( ;; )
  {
    rc = ar_osal_signal_wait( gva_work_event );
    rc |= ar_osal_signal_clear( gva_work_event );
    if ( rc ) break;

    gva_run( );
  }

  ar_osal_signal_destroy( gva_work_event );
  ar_osal_signal_set( gva_control_event );
...
)
{
  ...
  rc = gpr_list_init_v2( &p_ctrl->cmd_q,
...
static int32_t gva_init ( void )
{
  ...
  {  /* Initialize the locks. */
    rc = ar_osal_mutex_create( &gva_int_lock );
    rc |= ar_osal_mutex_create( &gva_thread_lock );
    rc |= ar_osal_signal_create( &gva_control_event );
  }

  { /* Initialize the custom heap. */
    vs_memmgr_init_heap( &gva_heapmgr, ( ( void* ) &gva_heap_pool ),
                          sizeof( gva_heap_pool ), NULL);
  }
  ...
  { /* Initialize the object manager. */
    ...
    rc = vs_objmgr_construct( &gva_objmgr, &params );
  }

  { /* Initialize free and nongating work pkt queues. */
    rc = gpr_list_init_v2( &gva_free_work_pkt_q, 
                           gva_int_lock_fn, gva_int_unlock_fn );
    for ( index = 0; index < GVA_NUM_WORK_PKTS_V; ++index )
    {
      ( void ) gpr_list_init_node( ( gpr_list_node_t* ) &gva_work_pkts[index] );
      gva_work_pkts[index].packet = NULL;
      rc = gpr_list_add_tail( &gva_free_work_pkt_q,
                              ( ( gpr_list_node_t* ) &gva_work_pkts[index] ) );
    }
    rc = gpr_list_init_v2( &gva_nongating_work_pkt_q,
                           gva_int_lock_fn, gva_int_unlock_fn );
  }
  ...
  { /* Initialize the global session lock. */
    rc = ar_osal_mutex_create( &gva_global_lock );
    GVA_PANIC_ON_ERROR ( rc );
  }
  ...
  { /* Create the GVA task worker thread. */
    ...
    /* thread attributes*/
    thread_attr.thread_name = GVA_TASK_NAME;
    thread_attr.priority = TASK_PRIORITY( priority );
    thread_attr.stack_size = stack_size;

    rc = ar_osal_thread_create( &gva_thread, &thread_attr,
                            gva_worker_thread_fn , NULL );
    GVA_PANIC_ON_ERROR( rc );

    ( void ) ar_osal_signal_wait( gva_control_event );
  }
...
static int32_t gva_predeinit ( void )
{
  ...
  for( index = 0; index < GVA_MAX_NUM_OF_SESSIONS_V; ++index )
  {
    ...
    ( void ) ar_osal_mutex_destroy( gva_session_obj_list[index]->data_lock );
...
static int32_t gva_deinit ( void )
{
  ...
  ( void )ar_osal_signal_destroy( gva_work_event );
  ( void ) ar_osal_signal_wait( gva_control_event );
  ...
  /* Release work queue */
  ( void ) gpr_list_clear( &gva_free_work_pkt_q );
  ( void ) gpr_list_clear( &gva_nongating_work_pkt_q );

  /* Deinitialize the object handle table. */
  vs_objmgr_destruct( &gva_objmgr );

  /* Deinitialize basic OS resources for staging the setup. */
  ( void ) ar_osal_signal_destroy( gva_control_event );
  ( void ) ar_osal_mutex_destroy( gva_int_lock );
  ( void ) ar_osal_mutex_destroy( gva_thread_lock );
  ( void ) ar_osal_mutex_destroy( gva_global_lock );
...
)
{
  ...
  {
    GVA_REPORT_FATAL_ON_ERROR( VS_ENOSERVICE );
    return VS_ENOSERVICE;
  }
