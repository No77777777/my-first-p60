/*
   ...
   $Header: //components/rel/avs.mpss/9.0.c51/vsd/vadapter/src/tva_module.c#1 $
   $Author: pwbldsvc $
*/
...
static ar_osal_mutex_t tva_int_lock;
static ar_osal_mutex_t tva_thread_lock;
static ar_osal_signal_t tva_control_event;

static vs_memmgr_type tva_heapmgr;
static uint8_t tva_heap_pool[ TVA_HEAP_SIZE_V ];

static vs_objmgr_t tva_objmgr;
static vs_objmgr_object_t tva_object_table[ TVA_MAX_OBJECTS_V ];

static tva_gating_control_t tva_gating_work_pkt_q;
static gpr_list_t tva_nongating_work_pkt_q;
static gpr_list_t tva_free_work_pkt_q;
static tva_work_item_t tva_work_pkts[ TVA_NUM_WORK_PKTS_V ];

static ar_osal_signal_t tva_work_event;
static ar_osal_thread_t tva_thread;
...
static ar_osal_mutex_t tva_global_lock;
...
{
  ( void )ar_osal_mutex_lock( tva_int_lock );
}

static void tva_int_unlock_fn ( void )
{
  ( void )ar_osal_mutex_unlock( tva_int_lock );
}

static void tva_thread_lock_fn ( void )
{
  ( void )ar_osal_mutex_lock( tva_thread_lock );
}

static void tva_thread_unlock_fn ( void )
{
  ( void )ar_osal_mutex_unlock( tva_thread_lock );
}

static void tva_signal_run ( void )
{
  ( void )ar_osal_signal_set( tva_work_event );
}
 ...
 */
static uint32_t tva_queue_gpr_packet (
  gpr_packet_t* packet,
  gpr_list_t* cmd_q
)
{
  ...
  for ( ;; )
  {
    ...
    rc = gpr_list_remove_head( &tva_free_work_pkt_q, (gpr_list_node_t**)&work_item );
    if ( NULL == work_item )
    {
    ...
    if ( rc )
    {
      MSG_2( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
             "TVA: error in queuing GPR packet: opcode[0x%08x], rc[0x%08x]", packet->opcode, rc );
      work_item->packet = NULL;
      ( void ) gpr_list_add_tail( &tva_free_work_pkt_q, &work_item->link );
    }
    else
    {
      /**
...
)
{
  ...
  rc = tva_queue_gpr_packet( packet, &tva_nongating_work_pkt_q );
...
)
{
  ...
  for ( ;; )
  {
    pkt_params.src_domain_id = TVA_NULL_V ;
    pkt_params.src_port = TVA_NULL_V;
    pkt_params.dst_domain_id = TVA_NULL_V ; 
    pkt_params.dst_port = TVA_NULL_V ;
    ...
    if ( rc )
    {
      MSG_1( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
             "TVA: GPR packet allocation failed with rc=[0x%08x]", rc );
      break;
    }
    ...
    {
      MSG_1( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
             "TVA: GPR packet queuing failed. rc[0x%08x]", rc );
      rc = __gpr_cmd_free( packet );
      TVA_REPORT_FATAL_ON_ERROR(rc);
    }
...
)
{
  ...
  if( TRUE == session_obj->is_timer_expiry_for_downlink )
  {
    ...
    rc = vs_timer_start( session_obj->pktexg_timer,
...
)
{
  ...
  { /* Allocate memory for the TVA object. */
    tva_obj = vs_memmgr_malloc( &tva_heapmgr, size );
    if ( tva_obj == NULL )
    {
      MSG_1( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
    ...
    rc = vs_objmgr_alloc_object( &tva_objmgr, &objmgr_obj );
    if ( rc )
    {
      MSG_1( MSG_SSID_DFLT, MSG_LEGACY_ERROR, 
             "tva_mem_alloc_object(): Out of objects, rc = (0x%08X)", rc );
      vs_memmgr_free( &tva_heapmgr, tva_obj );
      return VS_ENORESOURCE;
    }

    /* Use the custom object type. */
    objmgr_obj->ptr = tva_obj;
...
)
{
  ...
  ( void ) vs_objmgr_free_object( &tva_objmgr, object->header.handle );
  vs_memmgr_free( &tva_heapmgr, object );
  ...
  tva_session_object_t** ret_session_obj )
{
  ...
  { /* Initialize the simple job object. */
    ...
    rc = ( uint32_t )ar_osal_mutex_create(  &session_obj->data_lock );
    ...
    rc = vs_timer_create( &session_obj->pktexg_timer, tva_pktexg_timer_cb, session_obj );
    TVA_PANIC_ON_ERROR( rc );

    session_obj->tds_handle = TVA_NULL_V;
    ...
    session_obj->vs_handle = TVA_NULL_V;
...
)
{
  ...
  if ( tva_is_initialized == FALSE ) return VS_EOK;
...
)
{
  ...
  for ( ;; )
  {
    if ( session_obj->vs_handle == TVA_NULL_V )
    {
      rc = VS_EOK;
      break;
    }
...
)
{
  ...
  if ( session_obj->vs_handle == TVA_NULL_V ) return VS_EOK;
...
)
{
  ...
  for ( ;; )
  {
    ...
    if( TVA_NULL_V != session_obj->vs_handle )
    {
...
)
{
  ...
  for ( ;; )
  {
    ...
    evt_params = GPR_PKT_GET_PAYLOAD( tdscdma_ivoice_event_request_scr_mode_t, packet );
    if ( evt_params == NULL )
    {
      TVA_REPORT_FATAL_ON_ERROR ( VS_EUNEXPECTED );
      break;
...
)
{
  ...
  for ( ;; )
  {
    ...
    evt_params = GPR_PKT_GET_PAYLOAD( tdscdma_ivoice_event_request_codec_mode_t, packet );
    if ( evt_params == NULL )
    {
      TVA_REPORT_FATAL_ON_ERROR ( VS_EUNEXPECTED );
      break;
...
)
{
  ...
  for ( ;; )
  {
    ...
    evt_params = GPR_PKT_GET_PAYLOAD( tdscdma_ivoicel2_event_vfr_notification_t, packet );
    if ( evt_params == NULL )
    {
      TVA_REPORT_FATAL_ON_ERROR ( VS_EUNEXPECTED );
      break;
    ...
    rc = vs_timer_start( session_obj->pktexg_timer, ( timing_offset_us*1000 ) );
    TVA_REPORT_FATAL_ON_ERROR(rc);
...
)
{
  ...
  for ( ;; )
  {
    ...
    evt_params = GPR_PKT_GET_PAYLOAD( tdscdma_ivoice_event_set_logical_channels_t, packet );
    if ( evt_params == NULL )
    {
      TVA_REPORT_FATAL_ON_ERROR ( VS_EUNEXPECTED );
      break;
...
)
{
  ...
  for ( ;; )
  {
    ...
    evt_params = GPR_PKT_GET_PAYLOAD( tdscdma_ivoice_event_request_start_t, packet );
    if( evt_params == NULL )
    {
      rc = VS_EUNEXPECTED;
      TVA_REPORT_FATAL_ON_ERROR ( rc );
  ...
  /* If the return code is not VS_EPENDING it confirm that nothing is left for
...
static void tva_task_process_nongating_work_items ( void )
{
  ...
  while( gpr_list_remove_head( &tva_nongating_work_pkt_q,
                               ( ( gpr_list_node_t** ) &work_item ) ) == VS_EOK )
  {
    ...
    switch( packet->opcode )
    {
     ...
     default:
       ...
       rc = tva_queue_gpr_packet ( packet, &tva_gating_work_pkt_q.cmd_q );
       if ( rc )
       {
         MSG_1( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
  ...
  for ( ;; )
  {
    switch ( ctrl->state )
    {
      case TVA_GATING_CMD_STATE_FETCH:
      {
        ...
        {
          TVA_PANIC_ON_ERROR ( VS_EUNEXPECTED );
        }
...
static void tva_worker_thread_fn (
  void* param
)
{
  int32_t rc;

  ar_osal_signal_create( &tva_work_event );
  ar_osal_signal_set( tva_control_event );

  for ( ;; )
  {
    rc = ar_osal_signal_wait( tva_work_event );
    rc |= ar_osal_signal_clear( tva_work_event ); 
    if ( rc ) break;

    tva_run( );
  }

  ar_osal_signal_destroy( tva_work_event );
  ar_osal_signal_set( tva_control_event );
...
static int32_t tva_init ( void )
{
  ...
  {  /* Initialize the locks. */
    rc = ar_osal_mutex_create( &tva_int_lock );
    rc |= ar_osal_mutex_create( &tva_thread_lock );
    rc |= ar_osal_signal_create( &tva_control_event );
    TVA_REPORT_FATAL_ON_ERROR(rc);
  }

  { /* Initialize the custom heap. */
    vs_memmgr_init_heap( &tva_heapmgr, ( ( void* ) &tva_heap_pool ),
                          sizeof( tva_heap_pool ), NULL );
  }
  ...
  { /* Initialize the object manager. */
    ...
    rc = vs_objmgr_construct( &tva_objmgr, &params );
  }

  { /* Initialize free and nongating work pkt queues. */
    rc = gpr_list_init_v2( &tva_free_work_pkt_q, 
                           tva_int_lock_fn, tva_int_unlock_fn );
    for ( index = 0; index < TVA_NUM_WORK_PKTS_V; ++index )
    {
      ( void ) gpr_list_init_node( ( gpr_list_node_t* ) &tva_work_pkts[index] );
      tva_work_pkts[index].packet = NULL;
      rc = gpr_list_add_tail( &tva_free_work_pkt_q,
                              ( ( gpr_list_node_t* ) &tva_work_pkts[index] ) );
    }

    rc = gpr_list_init_v2( &tva_nongating_work_pkt_q,
                           tva_int_lock_fn, tva_int_unlock_fn );
  }
  ...
  { /* Initialize the global session lock. */
    rc = ar_osal_mutex_create( &tva_global_lock );
    TVA_PANIC_ON_ERROR( rc );
  }
  ...
  { /* Create the TVA task worker thread. */
    ...
    thread_attr.thread_name = TVA_TASK_NAME;
    thread_attr.priority = TASK_PRIORITY( priority );
    thread_attr.stack_size = stack_size;

    rc = ar_osal_thread_create( &tva_thread, &thread_attr,
                            tva_worker_thread_fn , NULL );
    TVA_PANIC_ON_ERROR( rc );

    ( void ) ar_osal_signal_wait( tva_control_event );
  }
...
static int32_t tva_predeinit ( void )
{
  ...
  for( index = 0; index < TVA_MAX_NUM_OF_SESSIONS_V; ++index )
  {
    ...
    ( void ) vs_timer_destroy ( tva_session_obj_list[index]->pktexg_timer );
    ( void ) ar_osal_mutex_destroy( tva_session_obj_list[index]->data_lock );
...
static int32_t tva_deinit ( void )
{
  ...
  ar_osal_signal_destroy( tva_work_event );
  ar_osal_signal_wait( tva_control_event );
  ...
  /* Release work queue */
  ( void ) gpr_list_clear( &tva_free_work_pkt_q );
  ( void ) gpr_list_clear( &tva_nongating_work_pkt_q );


  /* Deinitialize the object handle table. */
  ( void ) vs_objmgr_destruct( &tva_objmgr );

  /* Deinitialize basic OS resources for staging the setup. */
  ( void ) ar_osal_signal_destroy( tva_control_event );
  ( void ) ar_osal_mutex_destroy( tva_int_lock );
  ( void ) ar_osal_mutex_destroy( tva_thread_lock );
  ( void ) ar_osal_mutex_destroy( tva_global_lock );
...
)
{
  ...
  {
    TVA_REPORT_FATAL_ON_ERROR( VS_ENOSERVICE );
