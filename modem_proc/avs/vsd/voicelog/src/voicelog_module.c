/*
   ...
   $Header: //components/rel/avs.mpss/9.0.c51/vsd/voicelog/src/voicelog_module.c#1 $
   $Author: pwbldsvc $
*/
...
)
{
  ...
  for ( ;; )
  {
    ...
    ( void )ar_osal_mutex_lock( session_obj->session_lock );
  ...
  if( NULL != session_obj )
  {
    ( void ) ar_osal_mutex_unlock( session_obj->session_lock );
  }
...
)
{
  ...
  if ( NULL == cmd_params )
  {      
    VOICELOG_REPORT_FATAL_ON_ERROR( VS_EBADPARAM );
    return VS_EBADPARAM;
  }
  ...
  if( ( VS_ENORESOURCE != rc ) && ( VS_EOK != rc ) && ( NULL != session_obj ) )
  {
