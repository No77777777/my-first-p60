...
$Header: //components/rel/data.mpss/3.5..2.1/3gpp2/hdrdata/src/hdrds.c#2 $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ( ind_data_ptr != NULL )
  {
    ...
    DS3GPP2_ASSERT (ind_data_size <= sizeof(hdrcom_ind_data_union_type) );
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if ( cmd_ptr == NULL )
  {
    DS_HDR_ERROR_LOG_0(MSG_LEGACY_ERROR, "No command to queue!");
    return;
  }

  DS3GPP2_ASSERT (cmd_size <= sizeof(hdrds_cmd_union_type) );
...
)
{
  ...
  {
     default:
      DS_HDR_ERROR_LOG_1(MSG_LEGACY_ERROR, "MSG for unknown protocol %d",
                    protocol_name);
      break;
  }
} /* hdrds_process_msg() */
...
{
  DS_HDR_ERROR_LOG_1(MSG_LEGACY_ERROR, "IND for protocol %d",
                    protocol_name);
  switch ( protocol_name )
  {
...
)
{
  ...
  default:
      DS_HDR_ERROR_LOG_1(MSG_LEGACY_ERROR, "CMD for unknown protocol %d",
                    protocol_name);
      break;
  }
} /* hdrds_process_cmd() */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS3GPP2_ASSERT( ds_cmd_ptr->hdr.cmd_id == DS_CMD_HDR_EVENT);
  /*lint -save -e826 */
  /*lint -save -e740 */
  event_ptr = (hdrds_event_type* ) ds_cmd_ptr->cmd_payload_ptr;
  /*lint -restore */
  ...
  {
    DS_HDR_ERROR_LOG_0(MSG_LEGACY_ERROR, "Unrecognized event");
  }
