...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp2/hdrdata/src/hdrpacdsif.c#2 $
       ...
       ( ds707_pkt_get_current_data_sess_network() == SYS_SYS_MODE_HDR ))
    {
      ...
      ---------------------------------------------------------------------*/
          ...
          ps_iface_unsubscribe_event_list(
                           hdrpacdsif[curr_iface_index].sn_iface_event_buf,
      ...
      {
        DS_HDR_ERROR_LOG_1(MSG_LEGACY_ERROR, "iface go null fail, errno: %d",
                      ps_errno);
      }
...
/*===========================================================================
FUNCTION      HDRPACDSIF_SN_IFACE_EVENT_CBACK

DESCRIPTION   This is the callback function called when the IFACE event 
              indication is received.  It will queue up a command to be
              processed in the DS task.
...
LOCAL void hdrpacdsif_sn_iface_event_cback
(
ps_iface_type                *iface_ptr,
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  DS_HDR_ERROR_LOG_1(MSG_LEGACY_ERROR, "hdrpacdsif_sn_iface_event_cback():"
                                  " iface event received:%d",
                      event);
  ...
  hdrds_queue_cmd( HDRHAI_PACKET_APPLICATION_CONTROLLER, &hdrpac_cmd ,
    ...
    case IFACE_COMING_UP_EV:
      ...
      break;
    default:
      DS_HDR_ERROR_LOG_1(MSG_LEGACY_ERROR, "Unrecognized iface event received:%d",
                      event);
      break;
  }
}/*hdrpacdsif_sn_iface_event_cback()*/
/*===========================================================================
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (write_sn_ppp_status_to_nv)
  {
    ...
    {
      DS_3GPP2_ERROR_FATAL( "Bad NV read status" );
    }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
    ...
    case HDRPAC_SN_IFACE_COMING_UP_CMD:
      ...
      default:
        ...
        ---------------------------------------------------------------------*/
        DS_HDR_ERROR_LOG_1(MSG_LEGACY_ERROR, "PAC DS IF: Recd Unrecognized cmd:%d",
                      pac_cmd_ptr->name);
        break;
    }  
