...
$Header: //components/rel/data.mpss/3.5..2.1/3gpp2/hdrdata/src/hdrpac_mdm.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
{
  hdrcom_ind_data_union_type *dummy_ind_data = NULL;
  DATA_MDM_ARG_NOT_CONST(ind_data_ptr);
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/  

  dummy_ind_data = (hdrcom_ind_data_union_type*)
    modem_mem_alloc((sizeof(hdrcom_ind_data_union_type)),
        MODEM_MEM_CLIENT_DATA_CRIT);
  
  if (dummy_ind_data == NULL)
  {
    DATA_ERR_FATAL("Memory allocation failed for dummy_ind_data");
    return;
  }
  ...
  modem_mem_free(dummy_ind_data, MODEM_MEM_CLIENT_DATA);
} /* hdrpac_ind_cb() */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if( rlp_up == TRUE )
  {
    ...
    {
      DS_HDR_ERROR_LOG_1(MSG_LEGACY_ERROR, "Invalid AN RLP ID:%d", rlp_id);
    }
  }
  else
  {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
 ...
 hdrmrlpctl_cleanup_all_rlp();
  ...
  /*---------------------------------------------------------------------
  Cleanup TX WM. This will hold for clean up for SN RLP, AN RLP and MRLP
  ---------------------------------------------------------------------*/
  hdrrlp_cleanup_tx_wm();
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (msg_ptr == NULL)
  {
    DS_HDR_ERROR_LOG_0(MSG_LEGACY_ERROR, "NULL msg_ptr passed to hdrpac_msg_cb()");
    return;
  }   
  ...
  {
    DS_HDR_ERROR_LOG_0(MSG_LEGACY_ERROR, "Error Unpacking Msg ID");
  ...
  switch( protocol_id )
  {
    ...
    default:
      ...
      ** to PAC */
      ...
      DS_3GPP2_ERROR_FATAL("Invalid Protocol ID " );
      break;
  }
  ...
  {
    DS_3GPP2_ERROR_FATAL("Not Able to get Stream Config " );
  }
  ...
  switch( app_subtype[stream_id] )
  {
    ...
    case HDRHAI_DEF_PACKET_APP_SN:
      ...
      switch( msg_id )
      {
        ...
        default:
          DS_HDR_ERROR_LOG_1(MSG_LEGACY_ERROR, "Unknown in use Msg ID: %d", msg_id);
          dsm_free_packet( &msg_ptr );
          break;
      }
    break;
  ...
  case HDRHAI_MULTIFLOW_PACKET_APP_AN:
...
#else
      DS_HDR_ERROR_LOG_0(MSG_LEGACY_ERROR, "IS-890/a not supported in this build");
      dsm_free_packet( &msg_ptr );
    ...
    case HDRHAI_3G1X_CSN_APP:
      ...
      DS_HDR_ERROR_LOG_1(MSG_LEGACY_ERROR, "CSNA: 3G1X message (id=%d) received",
                    msg_id);
    ...
    default:
      DS_HDR_ERROR_LOG_1(MSG_LEGACY_ERROR, "Found invalid app subtype %d",
                    app_subtype[stream_id]);
      dsm_free_packet( &msg_ptr );
    break;
  } /* switch app_subtype */
