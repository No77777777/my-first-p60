...
$Header: //components/rel/data.mpss/3.5..2.1/3gpp2/hdrdata/src/hdrpac.c#2 $: //source/qcom/qct/modem/data/1x/hdrdata/main/latest/src/hdrpac.c#26 $
  ...
  case HDRIND_IDLE_AN_GAUP_SLOTTEDMODE:
   ...
   case HDRIND_IDLE_CONNECTION_NOT_ALLOWED:
    ...
    default:
      ...
      ---------------------------------------------------------------------*/
      DS_HDR_ERROR_LOG_1(MSG_LEGACY_ERROR, "PAC: Unrecognized Indication: %d",
                    ind_name);
      break;
  }
} /* hdrpac_process_ind() */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  else
  {
    ...
    {
      DS_HDR_ERROR_LOG_1(MSG_LEGACY_ERROR, "Inval query - strm:%d", stream_id);
    }
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  else
  {
    ...
    {
      DS_HDR_ERROR_LOG_1(MSG_LEGACY_ERROR, "Cant Xon Stream:%d",stream_id);
    }
  }
} /* hdrpac_flow_enable() */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  else
  {
    ...
    {
      DS_HDR_ERROR_LOG_1(MSG_LEGACY_ERROR, "Cant Xoff Stream:%d",stream_id);
    }
...
#else
      DS_HDR_ERROR_LOG_0(MSG_LEGACY_ERROR, "IS-890 not supported in this build");
#endif /* FEATURE_HDR_IS890 */
    }
    ...
    switch ( hdrpac.map_table[i].app_subtype )
    {
      ...
      case HDRHAI_MULTIFLOW_PACKET_APP_SN:
        ...
        switch (session_change_reason)
        {
          ...
          case HDRHAI_SCHG_OPENED_RESTORE_PRIOR_AND_PS:
            ...
            {
              DS_HDR_ERROR_LOG_0(MSG_LEGACY_ERROR, "Invalid switching");
            }
            break;
          default:
            DS_HDR_ERROR_LOG_1(MSG_LEGACY_ERROR, "Invalid reason :%d", session_change_reason);
            break;
        }
      ...
      case HDRHAI_DEF_PACKET_APP_SN:
            ...
            This session open is given to us only when there is a session reconfiguration */
        {
          ...
          {
            DS_HDR_ERROR_LOG_0(MSG_LEGACY_ERROR, "Invalid session change");
          }
        }
#endif /* FEATURE_HDR_QOS */
        break;
      ...
      case HDRHAI_ENHANCED_MULTIFLOW_PACKET_APP_AN:
      case HDRHAI_MULTILINK_MULTIFLOW_PACKET_APP_AN:
        DS_HDR_ERROR_LOG_0(MSG_LEGACY_ERROR, "AN MRLP not supported in this build");
        break;
...
//VIP HAck End
      ...
      default:
        DS_HDR_ERROR_LOG_1(MSG_LEGACY_ERROR, "Invalid app subtype %d found",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch ( pac_cmd_ptr->name )
  {
    ...
    default:
      ...
      ---------------------------------------------------------------------*/
      DS_HDR_ERROR_LOG_1(MSG_LEGACY_ERROR, "PAC: Received Unrecognized command:%d",
                    pac_cmd_ptr->name);
      break;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_HDR_ERROR_LOG_0(MSG_LEGACY_ERROR, "page_monitor_period_ptr is NULL");
    return;
  }
