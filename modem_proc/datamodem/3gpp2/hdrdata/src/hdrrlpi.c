...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp2/hdrdata/src/hdrrlpi.c#2 $
...
)
{
  ...
  switch (cfg_ptr->ver) 
  {
    ...
    default:
      DS_3GPP2_ERROR_FATAL("Unrecognized RLP ver");
      break;
  }
}
...
INLINE boolean hdrrlpi_get_new_buffer( dsm_item_type **new_pkt_item_ptr, uint32 max_bytes)
{
  ...
  if (DSM_LESS_THAN_LINK_LAYER_FEW_FREE_ITEMS(pool_id))
  {
    ...
    {
      DS_RLP_ERROR_LOG_2(MSG_LEGACY_ERROR, "Running low in system memory, "
  ...
  {
     DS_RLP_ERROR_LOG_2(MSG_LEGACY_ERROR, "Could not allocate memory, "
                   "Small_itm_cnt: %d, Large_itm_cnt: %d", 
                   DSM_POOL_FREE_CNT(DSM_DS_SMALL_ITEM_POOL),
                   DSM_POOL_FREE_CNT(DSM_DS_LARGE_ITEM_POOL));
     return FALSE;
...
)
{
  ...
  while (len > 0) 
  {
    ...
    if (!dsm_seek_extract( &tmp_item_ptr,
                           ...
                           &tmp_len))
    {
      DS_3GPP2_ERROR_FATAL("dsm_seek_extract bytes_to_copy:");
    }
    len -= tmp_len;
    ...
    if (len > 0) 
    {
      tmp_off += tmp_len;
      offset = 0;
      if ((item_ptr = (dsm_item_type *)q_next(&rrcb_ptr->state.tx_item_q, &item_ptr->link)) == NULL)
	  {
		DS3GPP2_ASSERT (0);
                return;
	  }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
         ...
         (q_cnt( &rrcb_ptr->state.multilink_nak_q )) )
  {
    ...
    DS3GPP2_ASSERT( nak_entry_ptr != NULL );
    ...
    else /* Delayed Nak */
    {
      ...
      DS3GPP2_ASSERT( quick_repeat_entry_ptr != NULL );
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  DS3GPP2_ASSERT( combo_index < HDRCP_PCP_MAX_COMBOS );
  ...
  /*-------------------------------------------------------------------------
    ...
    fill this up. 
  -------------------------------------------------------------------------*/        
  DS3GPP2_ASSERT(rexmit_info->tx_queue_item_ptr != NULL );  
  txq_ptr = (hdrrlptxq_type*)rexmit_info->tx_queue_item_ptr;
  DS3GPP2_ASSERT(txq_ptr->dsm_chain_ptr != NULL);
  seq_modulus = rrcb_ptr->cfg_ptr->seq_modulus;
  /*-------------------------------------------------------------------------
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Bad RLP header" );
    return (0);
   }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  {
    DS_3GPP2_ERROR_FATAL("Error new handles_built_count:");
    return(0);
  }
  ...
  else
  {
    ...
    {
      DS_3GPP2_ERROR_FATAL("Incorrect tx new data handles_built_count:");
      return (0);
    }
    /*lint -restore suppress code unreachable warning 527*/
  ...
  if (state_ptr->local_state[combo_index].seek_offset == HDRRLPI_TX_ITEM_Q_LOCAL_LEN_EXCL(rrcb_ptr, combo_index))
  {
        ...
        rrcb_ptr->cfg_ptr->packet_stream)
    {
      ...
      if (rrcb_ptr->cfg_ptr->segment_sequencing) 
      {
        ...
        DS3GPP2_ASSERT(tmp_item_ptr != NULL);
  ...
  { 
     /*-------------------------------------------------------------------
       ...
       packet. 
     ----------------------------------------------------------------*/
      DS3GPP2_ASSERT(!((rrcb_ptr->cfg_ptr->enhanced == DSRLP_ENHANCED_VER_EMPA || 
                       rrcb_ptr->cfg_ptr->enhanced == DSRLP_ENHANCED_VER_MMPA) &&
                       rrcb_ptr->cfg_ptr->packet_stream));
 ...
 /*-------------------------------------------------------------------
  ...
  while (payload_in_packet > 0)
  {
    ...
    DS3GPP2_ASSERT( new_pkt_item_ptr->size >= new_pkt_item_ptr->used);
  ...
  {
    DS_3GPP2_ERROR_FATAL("head_item_ptr did not succeed!");
  }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
      ...
      rrcb_ptr->cfg_ptr->packet_stream)
  {
    ...
    DS3GPP2_ASSERT(last_data_unit); /* flush packets always contain the last data unit */
  }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if (pkt_status == TRUE)
  {
     ...
     DS3GPP2_ASSERT(head_item_ptr != NULL);
    /*-----------------------------------------------------------------------
...
)
{
  ...
  DS3GPP2_ASSERT( rrcb_ptr->state.combo_index < HDRCP_PCP_MAX_COMBOS );
    ...
    if ( handles_per_combo[curr_combo_index].combo_id != combo_id )
    {
      if ( handles_per_combo[curr_combo_index].combo_id == HDRCP_PCP_NO_COMBO_ID_ACCEPTED )
      {
        /* 
          ...
          valid combo_index
        */
        DS3GPP2_ASSERT( curr_combo_index == 0 );
      }
      else
      {
        /* 
  ...
  else
  {
    ...
    else
    {
      ...
      */
      DS3GPP2_ASSERT(handles_per_combo[curr_combo_index].combo_id == combo_id);    
    }  
  }
  DS3GPP2_ASSERT( rrcb_ptr->state.combo_index < HDRCP_PCP_MAX_COMBOS );
} 
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    /* Free the DSM item ptr before moving the pkt info ptr to free queue */
    if(pkt_info_ptr->fwd_pkt_ptr != NULL)
    {
      dsm_free_packet( &pkt_info_ptr->fwd_pkt_ptr );
    }
