...
$Header: //components/rel/data.mpss/3.5..2.1/3gpp2/hdrdata/src/dshdr_qos_efsif.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
int dshdr_qos_get_mrlp_rec(dshdr_qos_file_record_type* record, uint32 i)
{
  ...
  {
    DS_HDR_ERROR_LOG_0(MSG_LEGACY_ERROR, "Record is out of range");
    return -1;
  }
...
uint32 dshdr_qos_powerup_init( void )
{
  ...
  {
    DS_HDR_ERROR_LOG_0(MSG_LEGACY_ERROR, "Exiting OUT");
    return 0;
  }
...
)
{
  ...
  while ( DS3GCFGMGR_EFS_TOKEN_PARSE_EOF != (ret_val = ds3gcfgmgr_efs_tokenizer(sm, &from, &to )))
  {
    ...
    {
      DATA_HDR_MSG0("Token Parse Failure");
      break;
    }
    /*------------------------------------------------------------------------
...
/*lint -restore Restore lint error 641*/
    ...
    else if(DS3GCFGMGR_EFS_TOKEN_PARSE_EOL == ret_val)
    {
      ...
      {
        DS_HDR_ERROR_LOG_0(MSG_LEGACY_ERROR, "Invalid Record acquired ");
        field_id = DSHDR_QOS_DEST_IP_ADDRESS;
        continue;
      }
      else
