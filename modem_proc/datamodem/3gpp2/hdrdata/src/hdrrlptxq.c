...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp2/hdrdata/src/hdrrlptxq.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
)
{
   ...
   * the UT compiled
   */
  DS3GPP2_ASSERT( hdrrlp_util_is_task_hdrtx() );
  /*
...
)
{
  ...
  if (new_ptr == NULL)
  {
    ...
    */   
    DS3GPP2_ASSERT ( oldest_rrcb_ptr != NULL );
  ...
  do 
  {
    ...
    else /* more items in queue */
    {
      /*---------------------------------------------------------------------
        ...
        ---------------------------------------------------------------------*/
      DS3GPP2_ASSERT(next_item_ptr != NULL);
                                 ...
                                 rrcb_ptr->cfg_ptr->seq_modulus))
      {
        ...
        else
        {
		  ...
		  DS_RLP_ERROR_LOG_2(MSG_LEGACY_ERROR, " end_seq_num %d is less than req_seq_num %d ",
...
#else
  DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR,"hdrrlptxq_timer_init() is not supported.");
#endif
  return;
}
...
/*--------------------------------------------------------------------------------------------*/
  ...
  DS3GPP2_ASSERT ( signal == HDRL1_TX_RLP_TXQ_SIG  );
