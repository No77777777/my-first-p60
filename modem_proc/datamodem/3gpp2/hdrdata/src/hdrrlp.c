...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp2/hdrdata/src/hdrrlp.c#2 $

when       who        what, where, why
--------   ---        ------------------------------------------------------- 
04/19/16   sd         Fix to clean up Tx watermark only when RLP reset is 
                      triggered in case of HDR.
12/22/14   sd         Fix to skip Quick nak processing for packet discarded as
                      duplicate in RLP reseq_q processing.
  ...
  switch (rrcb_ptr->cfg_ptr->ver)
  {
    case DSRLP_VER_HDR:
      if (log_status(hdrrlp_fr_tx_log.hdr.code))
      {
        ...
        if(stream_header == 0)
        {
            DS_IS707_ERROR_LOG_4(MSG_LEGACY_ERROR, 
    ...
    default:
      DS_3GPP2_ERROR_FATAL("Unrecognized RLP ver:%d sr_id");
      break;
  }
}

...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  -------------------------------------------------------------------------*/
  DS3GPP2_ASSERT( pkt_rrcb_ptr != NULL );
  ...
  if ((item_ptr = (dsm_item_type *)q_check(&rrcb_ptr->state.tx_item_q)) != NULL)
  {
    ...
    DS3GPP2_ASSERT(num_tx_bytes > state_ptr->seek_offset);
...
)
{
  ...
  /* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
        ...
        -----------------------------------------------------------------------*/
          ...
          {
            ps_errno = DS_EFAULT;
            DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR, "Cannot get a valid RSCB Index err:%d", ps_errno );
            return -1;
          }
    ...
    {
      DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "RLP is not in Established state" );
      ps_errno = DS_EFAULT;
      return -1;
    }
...
===========================================================================*/
boolean hdrrlp_process_rlp_pkt
(
  dsrlpi_rlp_ctl_blk_type     *rrcb_ptr,       /* RLP-specific ctl block   */
...
)
{
  ...
  /*-------------------------------------------------------------------------
    Make sure the control block pointer is not NULL
    -------------------------------------------------------------------------*/
  DS3GPP2_ASSERT( fwd_pkt_rrcb_ptr != NULL );
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Could not obtain pkt_info_ptr from free queue" );
    dsm_free_packet(&fwd_pkt_ptr);
    return;
  }
  ...
  {
    DS_IS707_ERROR_LOG_4(MSG_LEGACY_ERROR, "RLP Rx[%d,%d] V[%d,%d] Error extracting RLP header",
                  rrcb_ptr->cfg_ptr->sr_id,
                  "AB"[(rrcb_ptr->route)],
                  rrcb_ptr->cfg_ptr->ver,
                  rrcb_ptr->cfg_ptr->enhanced);
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  -------------------------------------------------------------------------*/
  DS3GPP2_ASSERT(( rlp_id == DSRLPI_HDR_AN_AUTH_SR_ID_VALUE) || 
         ( rlp_id == DSRLP_HDR_SN_RLP_SR_ID_VALUE));

  rscb_index = dsrlp_get_rlp_rscb_index( rlp_id, DSRLP_VER_HDR );
  if(rscb_index == DSRLPI_INVALID_RSCB_INDEX)
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "RLP not initialized for signaling messages!" );
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  DS3GPP2_ASSERT((rlp_id == DSRLP_HDR_SN_RLP_SR_ID_VALUE) ||
  ...
  -------------------------------------------------------------------------*/
  rscb_index = dsrlp_get_rlp_rscb_index( rlp_id, DSRLP_VER_HDR );
  DS3GPP2_ASSERT( rscb_index < DSRLP_MAX_RSCB_ELEMENTS );
  ...
  if(rscb_index >= DSRLP_MAX_RSCB_ELEMENTS )
  {
    DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR, "RLP ID %d not initialized",rlp_id); 
    return;
  }
...
FUNCTION HDRRLP_CLEANUP_TX_WM

DESCRIPTION
  Does the Tx WM cleanup. Note that it should only be called for HDR as the
  clean up is not done by DS707 pkt mgr.
...
)
{
   ...
   if ( rlp_id <= DSRLP_MAX_HDR_MRLP_FLOW_NUMBER - 1 )
   {
    ...
    -----------------------------------------------------------------------*/
     ...
     DATA_RLP_MSG2( MSG_LEGACY_LOW, "Enhanced_cta hdrrlp_enhanced_dormant_bitmap High:0x%x Low:0x%x", hi, lo );
   }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  {
    DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR,"HDR session not open. Not q-ing any msgs");
    return;
  }
       ...
       rlp_index++ )
  {
    ...
    switch ( (rscb_ptr->cfg).stream_id )
    {
      ...
      case HDRHAI_STREAM3:
        ...
        break;
      default:
        DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Invalid RLPID message to be queued" );
        return;
    }
...
#else
  DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "hdrrlp_queue_msg() is not supported.");
#endif
...
#else
  DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "hdrrlp_clear_stats() is not supported.");
#endif
...
#else
  DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "hdrrlp_ind_cb() is not supported.");
#endif
...
)
{
  ...
  switch (ind_name)
  {
    ...
    default:
    {
      DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR, "Invalid indication: %d", ind_name );
      break;
    }
  }
#else
  DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "hdrrlp_process_ind() is not supported.");
#endif
  return;
}/* hdrrlp_process_ind */

...
void hdrrlp_cta_exp_sig_hdlr(void)
{
 ...
 /*-----------------------------------------------------------------------
  ...
  else 
  {
    ...
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "hdrrlp_cta_exp_sig_hdlr() cta go dormant ignored");
  }

#else
  DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "hdrrlp_cta_exp_sig_hdlr() is not supported.");
#endif
  return;
}
