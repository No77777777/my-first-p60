...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp2/hdrdata/src/hdrrsp.c#2 $
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  switch (rlp_id_ptr->ver)
  {
    ...
    default:
      DS_HDR_ERROR_LOG_2(MSG_LEGACY_ERROR, "RSP: unknown flow identity: flow=%d, ver=%d",
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_HDR_ERROR_LOG_0(MSG_LEGACY_ERROR, "RSP: could not get stream for EMPA. Not queueing any msgs");
    dsm_free_packet(&item_ptr);
    return;
  }
  switch(stream_id)
  {
    ...
    case HDRHAI_STREAM3:
      ...
      break;
    default:
      DS_HDR_ERROR_LOG_0(MSG_LEGACY_ERROR, "RSP: EMPA mapped to invalid stream");
      dsm_free_packet(&item_ptr);
      return;
  }
  ...
  if (hdrcp_slp_send_msg(HDRHAI_RTC,                         /* HDR channel */
      ...
      != E_SUCCESS)
  {
    DS_HDR_ERROR_LOG_1(MSG_LEGACY_ERROR,
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch (msg_id)
  {
    ...
    default:		
	  DS_HDR_ERROR_LOG_1(MSG_LEGACY_ERROR, " Invalid Msg_ID : %d ", msg_id);
      DS3GPP2_ASSERT(0);
      break;
  }
}
  ...
  switch (msg_id)
  {
    case HDRRSP_ROUTE_SELECT_MSG_ID:
      ...
      else
      {
        ...
        DS_HDR_ERROR_LOG_1(MSG_LEGACY_ERROR, "RSP: received RouteSelect msg for "
    ...
    case HDRRSP_ACTIVATE_ROUTE_MSG_ID:
      if(hdrmrlpcif_get_two_route_supported_attrib(HDRMRLPCIF_CLIENT_MRLP_SN,
                                    ...
                                    &verno) == E_FAILURE)
      {
	    DS_HDR_ERROR_LOG_0(MSG_LEGACY_ERROR, "RSP: received RouteActivate msg, "
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  DS3GPP2_ASSERT(cmd == PS_HDRRSP_CMD);
  ...
  switch (command)
  {
    ...
    case HDRRSP_CMD_PROCESS_MSG: /* message received */
    {
      ...
      DS3GPP2_ASSERT(NULL != hdrrsp.ops_ptr->process_msg);
    ...
    default:
    {
      DS_3GPP2_ERROR_FATAL("RSP: unknown command:");
      break;
    }
  }
}
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  {
    DS_HDR_ERROR_LOG_0(MSG_LEGACY_ERROR, "Null flow ptr. Unable to clear route prot");
  }
}
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  while (1)
  {
    ...
    DS3GPP2_ASSERT(NULL != hdrrsp.ops_ptr->q_get_item);
    ...
    switch(flow_ptr->route_protocol)
    {
      ...
      case HDRMRLPC_ROUTE_PROT_ID_ROHC:
      {
                            ...
                            ) != 0)
        {
          ...
          DS_HDR_ERROR_LOG_1(MSG_LEGACY_ERROR, "RSP: RoHC De-Compression Error (%d), "
                        "dropping packet!", err);
          dsm_free_packet(&item_ptr);
        }   
      ...
      default:
        DS_3GPP2_ERROR_FATAL("Unknown route protocol");
        break;
    }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  DS3GPP2_ASSERT(rlp_id_ptr->ver == DSRLP_VER_HDR_MULTIFLOW_REVERSE);
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  switch(flow_ptr->route_protocol)
  {
    ...
    default:
      DS_3GPP2_ERROR_FATAL("RSP: flow %d: Unknown route protocol: ");
      break;
  }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if(flow_ptr == NULL)
  {
    DS_HDR_ERROR_LOG_0(MSG_LEGACY_ERROR,"Null Flow Ptr Unable to set Route Protocol");
    return;
  }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  switch (msg_id)
  {
    ...
    case HDRRSP_ACTIVATE_ROUTE_MSG_ID:
    {
      ...
      {
        DS_HDR_ERROR_LOG_1(MSG_LEGACY_ERROR, "RPS: err extracting TransactionID "
      ...
      {
        DS_HDR_ERROR_LOG_2(MSG_LEGACY_ERROR, "RSP: err extracting route from msg "
    ...
    default:
      ...
      DS_HDR_ERROR_LOG_1(MSG_LEGACY_ERROR, "RSP: unrecognized msg 0x%x - discarding",
                    msg_id);
      break;
