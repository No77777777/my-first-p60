...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp2/hdrdata/src/dshdr_an_mgr.c#2 $

when        who    what, where, why
--------    ---    ---------------------------------------------------------- 
04/01/16    sc     Optimize the code to reduce the static memory
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  else
  {
    ...
    DS_HDR_ERROR_LOG_0(MSG_LEGACY_ERROR, "AN CHAP LONG NAI Invalid/never written");
  }
  ...
  if (long_uid_pass)
  {
    ...
    else
    {
      ...
      DS_HDR_ERROR_LOG_0(MSG_LEGACY_ERROR, "AN CHAP Long Passwd Invalid/never written");
    }
  ...
  if (long_uid_pass == FALSE)
  {
    ...
    else
    {
      ...
      ---------------------------------------------------------------------*/
      dshdr_an.auth_info.user_id_len = 0;
      DS_HDR_ERROR_LOG_0(MSG_LEGACY_ERROR, "AN CHAP NAI Invalid or never written");
    }
    ...
    else
    {
      /*---------------------------------------------------------------------
       ...
       --------------------------------------------------------------------*/
      dshdr_an.auth_info.passwd_len = 0;
      DS_HDR_ERROR_LOG_0(MSG_LEGACY_ERROR, "AN CHAP Passwd Invalid or never written");
    }
  }/* if (long_uid_pass == FALSE) */
#ifdef FEATURE_DATA_PPP_DEFAULT_PASSWD
  /*-------------------------------------------------------------------------
  ...
  if( dshdr_an.auth_info.passwd_len == 0 )
  {
    ...
    {
      dshdr_an.auth_info.passwd_len = 0;
      DS_HDR_ERROR_LOG_0(MSG_LEGACY_ERROR,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS3GPP2_ASSERT(ppp_dev == PPP_UM_AN_DEV);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch(ppp_event)
  {
    ...
    default:
      DS_HDR_ERROR_LOG_1(MSG_LEGACY_ERROR, "Unsupported PPP AUTH event %d",
                      ppp_event);
      DS3GPP2_ASSERT(0);
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
...
#else
    if(nv_rtre_control_ext(ds707_curr_subs_id()) == NV_RTRE_CONTROL_NO_RUIM)
  ...
  ret_value = ppp_start( PPP_UM_AN_DEV, 
                         ppp_config_info );
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS3GPP2_ASSERT(stream_id >= HDRHAI_STREAM1);
  ...
  {
    DS_HDR_ERROR_LOG_1(MSG_LEGACY_ERROR, "Invalid AN stream %d", stream_id);
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch( ph_event )
  {
    ...
    default:
      DS_HDR_ERROR_LOG_1(MSG_LEGACY_ERROR, "Unregistered ph event %d rcvd",
                    ph_event);
      break;
  }
                                    ...
                                    cm_rtre_control_e_type rtre_control)
{
  ...
  *support_flag = FALSE;
...
#else
  if (NV_RTRE_CONTROL_USE_RUIM != nv_rtre_control_ext(ds707_curr_subs_id()))
#endif /* FEATURE_DATA_MPSS_AT20_DEV */
#endif
  {
    return;
  ...
  if( dshdr_uim.client_id ==(mmgsdi_client_data_type)INVALID_RUIM_CLIENTID)
  {
    ...
    {
      DS_HDR_ERROR_LOG_0(MSG_LEGACY_ERROR, "RUIM registration failed");
      return;
    }
  ...
  if( dshdr_uim.client_id ==(mmgsdi_client_data_type)INVALID_RUIM_CLIENTID)
  {
    DS_HDR_ERROR_LOG_0(MSG_LEGACY_ERROR, "Invalid UIM Client ID");
    return;
  }
  ...
  if (status != MMGSDI_SUCCESS)
  {
    DS_HDR_ERROR_LOG_0(MSG_LEGACY_ERROR, "RUIM read failure ");
    return;
  }
...
)
{
  ...
  DS_HDR_ERROR_LOG_1(MSG_LEGACY_ERROR, "PPP event %d",
...
)
{
  ...
  switch( cmd_ptr->hdr.cmd_id )
  {
    case DS_CMD_HDR_CDMA_SUBS_AVAIL_EVENT:
      ...
      uim_init_an_hrpd_preference(support_an_hrpd, ds707_curr_subs_id());
#endif /* FEATURE_MMGSDI_SESSION_LIB */
      break;
    ...
    default:
      DS_HDR_ERROR_LOG_1(MSG_LEGACY_ERROR, "Unrecognized cmd %d rcvd",
                    cmd_ptr->hdr.cmd_id);
      break;
  }
                          ...
                          const mmgsdi_cnf_type  *cnf_ptr)
{
  ...
  {
     DS_HDR_ERROR_LOG_0(MSG_LEGACY_ERROR, "Client ID not registered ");
  }
  (void)rex_set_sigs( dsi_get_ds_tcb(), DS_UIM_CMD_SIG );
}
#endif /*FEATURE_RUIM_FUNC_INTERFACE*/
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_3GPP2_ERROR_FATAL( "HDR AN unable to register as CM client");
  }
  else
  {
    /*-----------------------------------------------------------------------
    ...
    {
      DS_HDR_ERROR_LOG_0(MSG_LEGACY_ERROR, "cm_client_ph_reg returned error");
    }
    ...
    {
      DS_HDR_ERROR_LOG_0(MSG_LEGACY_ERROR, "cm_client_act returned error");
    }
  }
} /* dshdr_an_cm_init() */
...
void dshdr_an_mgr_powerup_init( void )
{
  ...
  else
  {
    if( 0 > ppp_event_register( ppp_event_handle,
                                ...
                                PPP_DOWN_EV_MASK ) )
    {
      DS_3GPP2_ERROR_FATAL ("ppp_event_register returned err" );
    }
...
int dshdr_an_set_user_id( uint8 * user_id_ptr, uint8 user_id_len )
{
  ...
  {
    DS_HDR_ERROR_LOG_2(MSG_LEGACY_ERROR, "Failed to set HDR AN user id, "
                  "id 0x%x, len %d",
