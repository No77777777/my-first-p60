...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp2/dsprofile/src/ds_profile_3gpp2_mgr.c#3 $

when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
04/01/16   hr      Not to write to legacyEFS for non KDDI profiles if JCDMA 
                   is enabled
...
/*===========================================================================
FUNCTION      DS_PROFILE_3GPP2_MGR_GET_DB_SUBS_ID

DESCRIPTION  This function returns the internal DB profile 
...
RETURN VALUE  ds_profile_db_subs_type

SIDE EFFECTS  None
===========================================================================*/
ds_profile_db_subs_type ds_profile_3gpp2_mgr_get_db_subs_id
(
  void
)
{
  ds_profile_db_subs_type  ds_profile_db_subs = DB_ACTIVE_SUBSCRIPTION_NONE;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  switch(subs_id) 
  {
    case SYS_MODEM_AS_ID_1:
      ds_profile_db_subs = DB_PROFILE_ACTIVE_SUBSCRIPTION_1;
      break;

    case SYS_MODEM_AS_ID_2:
      ds_profile_db_subs =  DB_PROFILE_ACTIVE_SUBSCRIPTION_2;
      break;

#ifdef FEATURE_TRIPLE_SIM
    case SYS_MODEM_AS_ID_3:
     ds_profile_db_subs =  DB_PROFILE_ACTIVE_SUBSCRIPTION_3;
     break;
#endif
  ...
  if( profiles != NULL ) 
  {
    for ( index=0; index < profiles_count; index++ )
    {
      ...
      DATA_IS707_MSG2( MSG_LEGACY_MED,
                       "index = %d gen_params.profile_number = %d",
                       index, gen_params.profile_number);
      ...
      {             
        DATA_IS707_MSG1( MSG_LEGACY_MED,
                         "Allocating cache for profile %d", 
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  for (index=0; index < profiles_count; index++) 
  {
    ...
    if( (data_sess_profile->profile_type & DB_3GPP2_PROFILE_KDDI) &&
        (data_sess_profile->data_session_profile_id >= DATA_SESSION_JCDMA_PROFILE_MIN) &&
        (data_sess_profile->data_session_profile_id <= DATA_SESSION_JCDMA_PROFILE_MAX) &&
...
)
{
  ...
  return dsi_profile_3gpp2_util_gen_ident_validation( profile );
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /*-------------------------------------------------------------- 
   ...
   --------------------------------------------------------------*/
  ds_profile_3gpp2_util_get_default_params(profile, default_ppp_opts);
...
)
{
  ...
  if( subs < DB_PROFILE_ACTIVE_SUBSCRIPTION_1 ||
      subs > DB_PROFILE_ACTIVE_SUBSCRIPTION_MAX )
  {
...
)
{
  ...
  {
    DS_3GPP2_ERROR_FATAL("ds_profile_3gpp2_mgr_register_event_cb: Unable to register event callback");
  }
