...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp2/ds707/src/dsdctm.c#2 $

when        who    what, where, why
--------    ---    ----------------------------------------------------------
04/01/16   ms      Added throttling mechanism for LCP failure event 
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -- - -*/
      ...
      switch(protocol)
      {
        ...
        case DSDCTM_PPP_PROTOCOL_VSNCP:
        {
          ...
          switch(ppp_fail_reason)
          {
            ...
            case PPP_FAIL_REASON_AUTH_FAILURE:
  ...
  if ( call_status != CM_SS_ORIG_THR_REASON_NONE )
  {
    ...
    if( cm_ss_cmd_orig_thr_update_tbl(
                   ...
                   0) == FALSE)
#else
    if( cm_ss_cmd_orig_thr_update_tbl1(
                   NULL,
                   NULL,
                   ds3g_cm_client_id,
                   CM_SS_ORIG_THR_UPDATE_CURR_SS,
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "DCTM: CM_SS_ORIG_THR_REASON_NONE, "
