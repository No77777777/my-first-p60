...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp2/ds707/src/ds707_ifaceAny_hdlr.c#2 $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_3GPP2_ERROR_FATAL("DSSNetAny instance_ptr is NULL");
    return -1;
  }
  ...
  if(ps_iface_create(any_iface_ptr,
                     ...
                     0) < 0)
  {
    DS_3GPP2_ERROR_FATAL("Could not create Any ps iface");
  }
  ...
  switch(failover_mode)
  {
    ...
    default:
      DS_3GPP2_ERROR_FATAL("Invalid failover type ");
      break;
  } /* switch(failove_mode) */
  ...
  ------------------------------------------------------------------------*/
  instance_ptr->assoc_iface_event_buf = NULL;
  instance_ptr->pref_iface_event_buf = NULL;
  /*-------------------------------------------------------------------------
  ...
  if( (ps_iface_passive_associate( &(instance_ptr->local_iface),
                                   ...
                                   &ps_errno)) < 0)
  {
    DS_3GPP2_ERROR_FATAL("Can't passively assoc ANY iface.");
  }
  /*------------------------------------------------------------------------
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS3GPP2_ASSERT(event < DSSNETANY_MAX_EV);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  /*-------------------------------------------------------------------------
    Register for IFACE UP, IFACE DOWN and IFACE CONFIGURING event.
  -------------------------------------------------------------------------*/
  if (instance_ptr->assoc_iface_event_buf == NULL)
  {
    instance_ptr->assoc_iface_event_buf =
      ...
      ps_iface_subscribe_event_list(
                    instance_ptr->assoc_iface_event_buf,
                         (ps_iface_event_enum_type *)&events,
                          num_events))
  {
    DS_3GPP2_ERROR_FATAL("Could not reg global cback for evts");
    return;
      ...
      ps_iface_subscribe_event_list(
                    instance_ptr->assoc_iface_event_buf,
                         (ps_iface_event_enum_type *)&events,
                          num_events))
  {
    DS_3GPP2_ERROR_FATAL("Could not reg global cback for evt");
    return;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch(instance_ptr->state)
  {
    case DSSNETANY_HANDUP_STATE:
    {
                                      ...
                                      &ps_errno)) < 0)
      {
        if( ps_errno != DS_EWOULDBLOCK )
        {
          /*-----------------------------------------------------------------
            ...
            Check with Marcello.
          -----------------------------------------------------------------*/
          DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "UM bringup failed, posting UM_IFACE_DOWN");
      ...
      if (instance_ptr->pref_iface_event_buf == NULL)
      {
        instance_ptr->pref_iface_event_buf =
                        (void *)ps_iface_alloc_event_handle(
                                instance_ptr->pref_iface_ptr,
                                dssnetAnyi_707_pkt_event_handler_cb,
                                instance_ptr);
      ...
      if( -1 == 
          ps_iface_subscribe_event_list(
                        instance_ptr->pref_iface_event_buf,
                            ...
                            num_events))
      {
        DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR, "Could not reg global cback for evt %d",
      ...
      if(ps_iface_tear_down_cmd(instance_ptr->assoc_iface_ptr,
                                ...
                                NULL) < 0)
      {
        DS_3GPP2_ERROR_FATAL("tear_down() failed when it should have succeeded!");
      }
    }
    /* fall thru */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch(instance_ptr->state)
  {
    ...
    case DSSNETANY_HANDUP_STATE:
    {
      ...
      ---------------------------------------------------------------------*/
      ppp_config.subs_id = ppp_get_subs_id( ds707_curr_subs_id() );

      (void) ppp_start(PPP_UM_SN_DEV,
                       &ppp_config);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch(instance_ptr->state)
  {
    ...
    case DSSNETANY_HANDDOWN_STATE:
    {
      ...
      {
        DS_3GPP2_ERROR_FATAL("Associated Iface is not pref or non-pref Iface");
        return;
      }
      break;
    }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch(instance_ptr->state)
  {
    ...
    case DSSNETANY_FAILOVER_STATE:
    {
      ...
      DS3GPP2_ASSERT(assoc_iface != NULL);
      ...
      {
        DS_IS707_ERROR_LOG_2(MSG_LEGACY_ERROR, "result = %d .ps_errno = %d",
                        result,ps_errno);
        DS3GPP2_ASSERT(0);
        return;
      }
...
)
{
  dssnetAny_sm_cb_type    *instance_ptr;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  DS3GPP2_ASSERT(cmd_ptr != NULL);
  DS3GPP2_ASSERT(cmd_ptr->cmd_payload_ptr != NULL);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  case DSSNETANY_OPENING_STATE:
  {
    ...
    {
      DS_3GPP2_ERROR_FATAL("Preferred Iface is not IPv4 or IPv6 addr family");
      return;
    }
    ...
    if(route_get(NULL,
                 ...
                 &um_iface_ptr ) < 0 )
    {
      DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "route_get: assoc iface not available");
      /*----------------------------------------------------------------------
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_3GPP2_ERROR_FATAL("Invalid instance 0x%p passed into any_bring_up_cmd()");
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(DSSNETANY_SMI_IS_VALID(instance_ptr) == FALSE)
  {
    DS_3GPP2_ERROR_FATAL("Invalid instance 0x%p passed into any_tear_down_cmd()");
  }
...
)
{
  ...
  if(ps_errno == NULL)
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "NULL dss_errno");
    return -1;
  }
  
  if( !(PS_IFACE_IS_VALID(iface_ptr)) )
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Invalid iface_ptr");
    *ps_errno = DS_EINVAL;
    return -1;
  }
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Invalid associated iface_ptr");
    *ps_errno = DS_EINVAL;
    return -1;
  }
