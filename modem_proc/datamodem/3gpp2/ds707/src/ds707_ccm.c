...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp2/ds707/src/ds707_ccm.c#3 $ $DateTime: 2021/02/08 22:33:06 $ $Author: pwbldsvc $
...
static ds707_ccm_flow_control_cmd_enum_type ds707_ccm_out_mem_cmd_fmac;
...
DESCRIPTION   Deprecated. Kept for compilation.
...
DESCRIPTION:
  This function sends a Flow Control command from memory watermark to FMAC.
...
{
  uint8                                i = 0;
  ds707_ccm_flow_control_cmd_enum_type arb_cmd = DS707_CCM_FLOW_OFF;

  if (DS707_CCM_MONITOR_MAX <= wmk)
  {
    DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR, "Invalid watermark index %d",wmk);
    return;
  }

  if ( ds707_ccm_in_cmd[(uint8)wmk] != cmd )
  {
    DATA_IS707_MSG2(MSG_LEGACY_HIGH, "Rcvd cmd %0x for watermark %d",
                    cmd, wmk);
    ds707_ccm_in_cmd[(uint8)wmk] = cmd;
    ds707_ccm_log_memory(cmd);
  }
  else
  {
    DATA_IS707_MSG2(MSG_LEGACY_HIGH, "Duplicate cmd %0x for watermark %d",
                    cmd, wmk);
    return;
  }
  ...
  {
    arb_cmd = (ds707_ccm_flow_control_cmd_enum_type)(arb_cmd | ds707_ccm_in_cmd[i]);
  }

  DS_ENTER_CRIT_SECTION(&ds707_global_crit_sect);
  if ( ds707_ccm_out_mem_cmd_fmac != arb_cmd )
  {
    ds707_ccm_out_mem_cmd_fmac = arb_cmd;
#ifdef FEATURE_HDR_FL_FLOW_CONTROL
    hdrl1_fmac_flow_control_cmd(ds707_ccm_out_mem_cmd_fmac);
#endif
  }
...
)
{
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Could not get config function ptr for FTC_MAC client");
  }
...
)
{
  ...
  if (tmp != ds707_ccm_out_cmd_fmac) {
      ...
      if (ds707_ccm_hdlr_tbl[DS707_CCM_FTC_MAC_FC].ds707_ccm_contrl_fmac_func != NULL )
        ...
        DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Could not get function ptr for FMAC command.");
  }
} /* ds707_ccmi_decide_fc_cmd_to_fmac */
#endif /* FEATURE_DATA_FL_FLOW_CONTROL */
...
)
{
  ...
  {
    DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR, "Invalid RLFC State: %d passed",
                    rlfc_state);
  }
  return;
} /* ds707_set_ccm_rlfc_state */
...
void ds707_ccm_tchup_init (void)
{
  ...
  ds707_ccm_out_mem_cmd_fmac = DS707_CCM_FLOW_OFF;
#endif /* FEATURE_DATA_FL_FLOW_CONTROL */
...
{ 
  ...
  if ( ds707_ccm_hdlr_tbl[DS707_CCM_FTC_MAC_FC].ds707_ccm_contrl_fmac_func != NULL )
      ...
      DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Could not get function for FMAC control.");
...
)
{
  ...
  DATA_IS707_MSG1(MSG_LEGACY_HIGH, 
                    "Monitor_id %d passed to ds707_ccmi_cfcm_event_cb",
                    fc_info->monitor_id);
  if((fc_info->monitor_id == CFCM_MONITOR_CPU)||
     ((fc_info->monitor_id == CFCM_MONITOR_MAX)&& 
      (fc_info->monitors_mask & CFCM_MONITOR_MASK_CPU)))
  {
    switch(fc_info->monitor_data.cpu.cmd)
    {
  ...
  if((fc_info->monitor_id == CFCM_MONITOR_THERMAL_PA)||
     ((fc_info->monitor_id == CFCM_MONITOR_MAX)&& 
      (fc_info->monitors_mask & CFCM_MONITOR_MASK_THERMAL_PA)))
  {
    switch(fc_info->monitor_data.pa.state)
    {
      case DS707_CCM_THERM_PA_OK: 
        /*Handle Normal*/
        DATA_IS707_MSG0(MSG_LEGACY_HIGH,
                        "Received Sensor PA Okay event. Set Flow OFF");
        ccm_flow_cmd = DS707_CCM_FLOW_OFF;
        break;
      case DS707_CCM_THERM_PA_WARN:
        /* Throttle Data */
        DATA_IS707_MSG0(MSG_LEGACY_HIGH,
                        "Received Sensor PA Warning event. Set Flow DOWN");
        ccm_flow_cmd = DS707_CCM_FLOW_DOWN;
        break;
      case DS707_CCM_THERM_PA_WARN2:
        DATA_IS707_MSG0(MSG_LEGACY_HIGH,
                        "Received Sensor PA Warning event.Set Flow DOWN More");
        ccm_flow_cmd = DS707_CCM_FLOW_DOWN_MORE;
        break;
      case DS707_CCM_THERM_PA_WARN3:
        DATA_IS707_MSG0(MSG_LEGACY_HIGH,
                        "Received Sensor PA Warning event. Set Flow Shut DOWN");
        ccm_flow_cmd = DS707_CCM_FLOW_SHUT_DOWN;
        break;
      default:
        DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR,
                        "Invalid sensor event: %d", 
                        fc_info->monitor_data.pa.state);
        break;
    }
    ds707_ccm_therm_flow_control_cmd_hdlr_ex(ccm_flow_cmd,
                                           fc_info->monitor_data.pa.step_timer);
  }
