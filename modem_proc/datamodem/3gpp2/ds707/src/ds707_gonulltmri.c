...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp2/ds707/src/ds707_gonulltmri.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
12/09/14   sc      Prevent go null timer from starting when RAT is eHRPD 
...
#endif
  {
    if (ds707_get_gonulltmri_is_running_flag() == FALSE)
    {
      ...
      else if(ds707_extif_is_pkt_data_sess_on_hdr())
      {
        ...
        {
          DATA_IS707_MSG0(MSG_LEGACY_HIGH, "In eHRPD, dont start gonull timer");
          return;
        }
      ...
      else 
      {
        ...
        DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "The Current Data Session is neither HDR nor 1X");
      ...
      if (gonull_timer > 0)
      {
        ...
        ds3g_timer_start(DS3G_TIMER_ABORT_DORMANT_PPP, gonull_timer);
...
void ds707_gonulltmri_stop(void)
{
  ...
  if (ds707_get_gonulltmri_is_running_flag() == TRUE)
  {
    ...
    ds3g_timer_stop(DS3G_TIMER_ABORT_DORMANT_PPP);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch(ds_cmd_ptr->hdr.cmd_id)
  {
    case DS_CMD_707_ABORT_DORMANT_PPP_TIMER_EXPIRED:
    {
          ...
          (ds707_get_gonulltmri_val() > 0))
      {
        ...
        {      
          ...
          {
            DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Phys link not down, aborting process close dormant PPP timer");
          }
          else
          {
            /*-----------------------------------------------------------------
            ...
            {
              DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR, "iface go null fail, errno: %d", ps_errno);
              break;
            }
          }
        }/* for each iface */  
    ...
    default:
      DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR, "Unknown cmd %d", ds_cmd_ptr->hdr.cmd_id);
      break;
  }
} /* ds707_gonulltmri_process_cmd() */
