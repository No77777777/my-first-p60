...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp2/ds707/src/ds707_pkt_mgr_hdlr.c#3 $ 
...
03/20/15    hr     Fix to process S101 SM status transition before sending
                   bearer tech changed notification
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if (flow_ptr != NULL)
  {
    /* For HDR, TX watermark will be cleaned up when RLP is cleaned up on 
       HDR's indication. This is to avoid the race condition where Tx
       WM is flushed in pkt mgr while the stale value is used in RLP */
    if ( !DS_ISDATA_HDR(tc_state_ptr->so) )
    {
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if (cmd_ptr != NULL)
  {
    ...
    {
      DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "NULL cmd payload ptr" );
      return;
    }
...
)
{
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Could not set PPP session close timer DO value.");
  }
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Could not set PPP session close timer 1X value.");
  }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
 ...
 orig_so = ds707_pkti_get_orig_so();
  ...
  {
    DS_3GPP2_ERROR_FATAL("Bad mobile settings - no SO to orig with");
    return(DS3G_FAILURE);
  }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "NULL pointer passed" );
    return DS3G_IGNORE;
  }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "tc_state_ptr is NULL");
    ret_val = DS3G_REJECT_CALL;
    return ret_val;
  }
...
/*lint -save -e774 Reason for suppressing error 774*/
  ...
  {
    DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR, "conn call with unknown id %d", call_id);
  }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "CALL IDs DO NOT MATCH. STILL CONTINUE");
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -- - -*/
  ...
  {        
    curr_iface_ptr = ds707_pdncntx_get_iface_ptr_from_iface_tbl_index(
                                                       curr_iface_index);
    DS3GPP2_ASSERT(curr_iface_ptr != NULL);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -- - -*/
  ...
  {
    DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR, "Could not end call with id %d", 
                    call_info_cmd_ptr->call_id);
    return;
  }
  ...
  if ( (call_info_cmd_ptr->end_status == CM_CALL_END_OFFLINE) ||
       ...
       (call_info_cmd_ptr->end_status == CM_CALL_END_STANDBY_PREF_CHG)
     )
  { 
    DATA_IS707_MSG1( MSG_LEGACY_HIGH,
...
)
{
  ...
  if (tc_state_ptr == NULL)
  {
    DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR, "get data count unknown id %d",
                    call_id);
    ret_val = FALSE;
  }
  ...
  else
  {
    ...
    if (flow_ptr == NULL)
    {
      DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Invalid flow ptr");
      ret_val = FALSE;
    }
    else
    {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  else
  {
    ...
    */
...
#endif /* FEATURE_EHRPD */
    {
      ...
      else if( ( (old_mode != SYS_SYS_MODE_CDMA) && 
 ...
 /*---------------------------------------------------------------------
  ...
  {
    if(ds3g_timer_get_status(DS3G_TIMER_RETRY_DELAY_ORIG))
    {
      ds707_phys_link_enable_flow(DS_FLOW_707_LOCK_MASK);
    }
  }
...
{
  ds707_pkti_sys_chng_hdlr(ds707_curr_subs_id(), 
                           sys_mode, 
                           SYS_SYS_MODE_NO_SRV);
}
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  ds707_pkti_mgr_hdlr_func_tbl.set_downlink_throughput_running_t_accumulate = NULL;
  ...
  ds707_pkti_mgr_hdlr_func_tbl.reg_tx_wm_non_empty_func_hdlr = NULL;
  ds707_pkti_mgr_hdlr_func_tbl.get_total_ul_cur_wm_cnt_hdlr = NULL;
  /*---------------------------------------------------------------------
...
{
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  DATA_IS707_C_MSG1(DS707_F3_MASK, "get last_good_sys = %d", ds707_last_good_sys);
...
)
{
 ...
 /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (report_p != NULL)
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Already allocated. Check again");
    dss_errno = -1;
    return dss_errno;
  }
  ...
  DS_IS707_ERROR_LOG_2(MSG_LEGACY_ERROR, "Allocating %d bytes for max %d "
  ...
  if(report_p == NULL)
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Mem alloc failure for Iface Info list."
...
)
{
  ...
  /*------------------------------------------------------------------------- 
    ...
    -------------------------------------------------------------------------*/
  current_mode = ds3g_get_current_preferred_mode_ex(ds707_curr_subs_id());
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    current_mode = ds3g_get_current_network_mode_ex(ds707_curr_subs_id());
  }
  else
  {
 ...
 if ((subs_id != DS3GSUBSMGR_SUBS_ID_MIN)) /*use ds3gsubs enum*/
  {
   ...
   /*------------------------------------------------------------------------- 
    ...
    -------------------------------------------------------------------------*/
  duration = ds3gtputmgr_get_actual_tput_refresh_interval(subs_id);
  ...
  if(current_mode == SYS_SYS_MODE_CDMA)
  {
    ...
    ds707_allowed_throughput = 
      ds707_pkt_mgr_util_get_throughput_from_1X_rpc(ds707_rpc_rating);
