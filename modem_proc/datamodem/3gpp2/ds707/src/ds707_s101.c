...
$Header: //components/rel/data.mpss/3.5..2.1/3gpp2/ds707/src/ds707_s101.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
)
{
  ...
  DATA_IS707_MSG4(MSG_LEGACY_HIGH, 
                "S101: S101 Sys Chg: Tunnel Call Allowed = %d, "
                "LCP Throttled: %d, Virt Conn Throttled: %d"
                 "hdr_actv_ho_in_prog: %d",
                tunnel_call_allowed,
                ds707_s101_sm.lcp_throttled,
...
)
{
  ...
  else
  {
    ...
    DATA_IS707_MSG3(MSG_LEGACY_HIGH, 
                             "Tunnel Mode:%d, Prereg:%d, LCP Throttled:%d",
                             ds707_s101_sm.tunnel_mode_val,
                             ds707_s101_sm.prereg_allowed,
                             ds707_s101_sm.lcp_throttled
                             );
    DATA_IS707_MSG3(MSG_LEGACY_HIGH, 
                             "Virt Conn Throttled:%d, "
                             "eHRPD Mode:%d, HDR Session State:%d",
                             ds707_s101_sm.virtual_connection_throttled,
                             ds707_s101_sm.ehrpd_mode,
                             ds707_s101_sm.hdr_sess_open
                             );
    DATA_IS707_MSG2(MSG_LEGACY_HIGH, 
                             "Sys_Mode_LTE:%d, ho_to_L_in_prog:%d",
                             ds707_s101_sm.sys_mode_lte,
...
)
{
  ...
  default:
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "S101: Invalid Transition");
    break;
...
)
{
  ...
  {
    DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR, 
                    "S101: Error: Prereg HO SM transitioning to same state: %d",
                    old_state);
    return;
  }
  ...
  DATA_IS707_MSG2(MSG_LEGACY_HIGH, 
                "S101: S101 prereg H0 SM transitioning from: %d state to: %d state",
                old_state, 
                ds707_s101_prereg_ho_sm.s101_prereg_ho_sm_state);
...
{
  DATA_IS707_MSG1(MSG_LEGACY_HIGH, "S101: Prereg HO SM in %d state", 
                  ds707_s101_prereg_ho_sm.s101_prereg_ho_sm_state);
...
{
  DATA_IS707_MSG2(MSG_LEGACY_HIGH, 
            "S101: eHRPD Mode Change: eHRPD Mode = %d, S101 SM Curr State: %d", 
             is_set, ds707_s101_sm.s101_sm_state);
...
{
  DATA_IS707_MSG2(MSG_LEGACY_HIGH, 
             "S101: LCP Throttle Info Change: LCP Throttled = %d"
             "S101 SM Curr State: %d", 
             is_throttled,ds707_s101_sm.s101_sm_state);
...
)
{
  ...
  switch (prev_tunnel_mode_val)
  {
    ...
    default:
    {  
      DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Invalid prev tunnel mode val");
      break;
    }
  } /* Switch(prev_tunnel_mode_val)  */
...
{
  DATA_IS707_MSG2(MSG_LEGACY_HIGH, 
                "S101: Setting ho_to_l_in_prog to: %d"
                "S101 SM Curr State: %d", 
                ho_to_l_in_prog,
                ds707_s101_sm.s101_sm_state);
...
)
{
  if (DS707_S101_SM_TUNNEL_CALL_ALLOWED == ds707_s101_sm.s101_sm_state)
  {
    DATA_IS707_MSG1(MSG_LEGACY_HIGH, "S101: %d state", 
                       ds707_s101_sm.s101_sm_state);
    return TRUE;
  }
  else
  {
    DATA_IS707_MSG1(MSG_LEGACY_HIGH, 
                  "S101: %d state. Tunnel Call not allowed", 
                  ds707_s101_sm.s101_sm_state);
    return FALSE;
  }
...
{
  DATA_IS707_MSG1(MSG_LEGACY_HIGH, 
                "S101: IRMI response recvd: IRMI Accepted = %d",
                irmi_accepted);
...
)
{
  ...
  DATA_IS707_MSG1(MSG_LEGACY_HIGH, 
                "S101: Recvd PPP Auth %d event in tunnel mode", 
                auth_success);
...
)
{
  ...
  DATA_IS707_MSG2(MSG_LEGACY_HIGH, 
                "S101: Recvd PPP LCP %d event in tunnel mode,"
                "ho_to_L_in_prog:%d", 
                lcp_up,
                ds707_s101_sm.ho_to_L_in_prog);
...
)
{
  ...
  DATA_IS707_MSG2(MSG_LEGACY_HIGH, 
                "S101: Recvd VSNCP %d event in tunnel mode, "
                "ho_to_L_in_prog:%d", 
                vsncp_up,
                ds707_s101_sm.ho_to_L_in_prog);
...
{
  DATA_IS707_MSG2(MSG_LEGACY_HIGH, 
                "S101: HDR Sess Change: HDR Sess State = %d"
                "S101 SM Curr State: %d", 
                sess_open,
                ds707_s101_sm.s101_sm_state);
...
)
{
  ...
  if (FALSE == ds3g_is_hdr_in_srvc(ds707_curr_subs_id()))
  {
    DATA_IS707_MSG0(MSG_LEGACY_HIGH,
    ...
    ds3gdsdif_set_or_clean_null_bearer_reason_info(
                         DS3GDSDIF_SET_NULL_BEARER_REASON,
                         DS3GDSDIF_SYS_NULL_BEARER_REASON_OOS,
                         ds707_curr_subs_id());
    ds_dsd_ext_event_notify(DS_DSD_RADIO_HDR, &dsd_event_info);

    //Check cached radio info and report LTE service if in limited or full srvc
    lte_srv_status = ds3g_get_lte_srv_status(ds707_curr_subs_id());
        ...
        SYS_SRV_STATUS_SRV == lte_srv_status)
    {
      ...
      dsd_event_info.subs_id = 
                  ds3gsubsmgr_subs_id_cm_to_ds3g(ds707_curr_subs_id());              
      ds3gdsdif_set_or_clean_null_bearer_reason_info(SYS_SRV_STATUS_LIMITED == lte_srv_status,
                                                DS3GDSDIF_SYS_NULL_BEARER_REASON_LIMITED_SRVC, 
                                                ds707_curr_subs_id());
  ...
  {
    ds3gdsdif_set_or_clean_null_bearer_reason_info
          (DS3GDSDIF_CLEAN_NULL_BEARER_REASON,
           DS3GDSDIF_SYS_NULL_BEARER_REASON_OOS, ds707_curr_subs_id());
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR,
