...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp2/ds707/src/ds707_so33.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (cfg_info->con[so_index].so == CAI_SO_PPP_PKT_DATA_3G)
  {
    ...
    else
    {
       ...
       memset(dsrlp_blob_cmd_ptr, 0, sizeof(dsrlp_blob_cmd_type));
  ...
  {
    DS_3GPP2_ERROR_FATAL("Only SO 33 uses callback right now");
  }
}

/*===========================================================================
...
)
{
     ...
     )
  {
    ...
    {
      DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Could not parse QOS RLP BLOB!!!");
    }
  }
  else
  {
