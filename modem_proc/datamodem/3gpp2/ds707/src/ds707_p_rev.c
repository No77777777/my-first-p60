...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp2/ds707/src/ds707_p_rev.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_3GPP2_ERROR_FATAL("Unknown system p_rev");
  }
  return (p_rev_index);
} /* ds707_p_rev_get_index() */
...
/*===========================================================================
FUNCTION       DS707_SYNC_ESPM_IND_CBACK_HDLR

DESCRIPTION    Handler to process SYNC and ESPM indications. Processed in DS task.
...
SIDE EFFECTS   None
===========================================================================*/
void ds707_sync_espm_ind_cback_hdlr
(
  ds_cmd_type   *cmd_ptr
)
{
  ds_dsd_event_info_type                 event_info;
  ds707_p_rev_sync_espm_cback_info_type *cb_info = NULL;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

  cb_info = (ds707_p_rev_sync_espm_cback_info_type*)cmd_ptr->cmd_payload_ptr;
  if(cb_info == NULL)
  {
    ASSERT(0);
    return;
  ...
  switch(cb_info->msg_type)
  {
    ...
    case CAI_EXT_SYS_PARM_MSG :
      ...
      event_info.data.cdma_p_rev_ind.so_mask = 
                                  ds707_get_so_mask_from_p_rev(cb_info->p_rev);
      event_info.subs_id = 
  ...
  return;
} /* ds707_sync_espm_ind_cback_hdlr() */
...
{ 
  ds_cmd_type                           *cmd_ptr;
  ds707_p_rev_sync_espm_cback_info_type *cback_info = NULL;
  
  if((cmd_ptr = ds_allocate_cmd_buf(
                       sizeof(ds707_p_rev_sync_espm_cback_info_type))) == NULL)
  {
    ASSERT(0);
    return;
  }
  ...
  {
    DATA_IS707_MSG1(MSG_LEGACY_HIGH, "Sending SYNC-ESPM cmd - %d", 
                    DS_CMD_707_SYNC_ESPM_IND);

    cmd_ptr->hdr.cmd_id = DS_CMD_707_SYNC_ESPM_IND;
    cback_info = (ds707_p_rev_sync_espm_cback_info_type*)
                                         cmd_ptr->cmd_payload_ptr;
    cback_info->msg_type = msg_type;
    cback_info->p_rev = p_rev;
