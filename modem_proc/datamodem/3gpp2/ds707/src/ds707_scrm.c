...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp2/ds707/src/ds707_scrm.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $

when       who        what, where, why
--------   ---        -------------------------------------------------------
04/01/16   sc         Replace all the memcpy and memmove with memscpy and 
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  switch(rate_from_mc)
  {
    ...
    case 5:
      ...
      break;
    default:
      DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR, "Uknown rate %d", rate_from_mc);
...
)
{
  ...
  else if (sch_rate <= CAI_SCH_RATE_RS1_16X_RS2_16X)
  {
    ...
    switch (scrm_sch_rate) 
    {
      ...
      default:
        DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR, "Uknown rate %d", scrm_sch_rate);
  ...
  {
    DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR, "Uknown rate %d", sch_rate);
    /*---------------------------------------------------------------
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "BAD current F-SCH rate");
    return SCRM_SCH_RATE_NONE;
  } 
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "BAD rate");
    return SCRM_SCH_RATE_NONE;
  }
  /*lint -restore*/
...
#endif
      default:
        DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR, "Unknown R-SCH max rate %d",
...
{
  scrm_sch_rate_type    curr_rate;               /* current r-sch rate     */ 
  scrm_sch_rate_type    scrm_rate = SCRM_SCH_RATE_NONE;  
                                                  /* rate to scrm for       */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    ds707_rrm_awaiting_mc  = TRUE;
    queue_status = mc_queue_cmd(ds707_curr_subs_id(), MC_RES_REQ_F);
  ...
  else
  {
    ...
    case DEC_SCH_RATE_1X:
      ...
      case DEC_SCH_RATE_32X:
        ...
        break;
      default:
        DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Invalid RXC SCH rate");
        f_sch_rate = SCRM_SCH_FRAME_8;
        break;
    }
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  -------------------------------------------------------------------------*/
  DS3GPP2_ASSERT(blob_size != NULL);
  DS3GPP2_ASSERT(blob_ptr  != NULL);
