...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp2/ds707/src/ds707_pkt_mgr_util.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $

when        who    what, where, why
--------    ---    ---------------------------------------------------------- 
04/01/16    sd     Fixed throttling info updation on query through QMI WDS. 
06/26/15    sd     Removal of MSG_SPRINTF
04/24/15    sd     Fixed subs_id handling for 3GPP2 in DSDA mode.
03/30/15    hr     Generate generic indications only for up ifaces
...
#define DS_PKT_MGR_EHRPD_FB_APN_FILE      "/eHRPD/fallback_apn_list.txt"
#define DS_PKT_MGR_TMP_EHRPD_FB_APN_FILE  "/eHRPD/fallback_apn_list_tmp.txt"
  ...
  switch (action)
  {
    case DS707_PKT_DORM_ACTION_OK_TO_ORIG:
      switch (reason)
      {
        ...
        default:
          DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR,"Unknown dorm reason %d", reason);
          DS3GPP2_ASSERT(0);
          break;
      } /* switch (reason) */
      break;
    ...
    default:
      DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR,"Unknown dorm action %d", action);
      DS3GPP2_ASSERT(0);
      break;
  } /* switch (action) */
...
)
{
  ...
  if ((ds707_pkti_is_dorm_action_allowed(
      ...
      (ds3g_is_dorm_allowed_in_pkt_mode(SYS_SYS_MODE_CDMA, ds707_curr_subs_id()) == TRUE)        
     )
  {
    return(TRUE);
  }
...
)
{
  ...
  if (ds3g_is_pref_pkt_mode_ex(SYS_SYS_MODE_LTE,ds707_curr_subs_id())  
#ifdef FEATURE_DATA_OPTHO
...

/*===========================================================================
FUNCTION      DS707_PKT_IS_SUBS_ID_VALID

DESCRIPTION   This function validates the subscription value based on 
...
SIDE EFFECTS  None.
===========================================================================*/
boolean ds707_pkt_is_subs_id_valid
(
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if( subs_id <= SYS_MODEM_AS_ID_NONE ||
      subs_id >= SYS_MODEM_AS_ID_NO_CHANGE )
  {
    DATA_3GMGR_MSG1( MSG_LEGACY_ERROR, "subs id invalid: %d",
                     subs_id);
    return FALSE;
  }
...
)
{
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "NULL iface pointer passed in");
    return(FALSE);
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch ( rlp_ver )
  {
    ...
    default:
      DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR, "RLP ver: %d unrecognized", rlp_ver);
      return;
  }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  DS3GPP2_ASSERT(dssnet_instance != NULL);
  /*lint -save -e774 Reason for suppressing error 774*/

  if (dssnet_instance == NULL)
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Invalid dssnet instance ptr");
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS3GPP2_ASSERT(ps_flow_ptr != NULL);
  DS3GPP2_ASSERT(ds_flow_ptr != NULL);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR, "Invalid flow_ptr 0x%x", ps_flow_ptr);
    return NULL;
  }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if (NULL != burst_params_ptr)
  {
    ...
    -----------------------------------------------------------------------*/
    ds3g_burst_enable_flow( phys_link_ptr, 
  ...
  for (i = 1; i < DS707_NUM_PHYS_LINKS; ++i)
  {
    ...
    DS3GPP2_ASSERT (phys_link_ptr != NULL);
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  {
    /*-----------------------------------------------------------------------
      Run bursty flow control algo when disabling flow on Primary phys link
    -----------------------------------------------------------------------*/
    ds3g_burst_disable_flow( phys_link_ptr, 
                             (void *)ds707_wmk_get_pri_wm_ptr(),
                             flow_mask, 
                             &burst_params_ptr->burst_params,
                             ds707_wmk_get_tcp_bidir());
   }
...
void ds707_pkt_mgr_check_coex_data_status(ps_iface_type *iface_ptr)
{
  ...
  if( iface_ptr == NULL )
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Ignore due to invalid iface ptr");
    return;
  }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  *num_ps_flows  = 0;
  ...
  while (ps_flow_handle != NULL)
  {
    ...
    if (!result)
    {
      DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Returning incomplete list of flows");
      return FALSE;
    }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if(ret_val == TRUE)
  {
    ...
    -----------------------------------------------------------------*/
    DATA_IS707_MSG0(MSG_LEGACY_LOW, "eHRPD call");
  }
  return ret_val;
...
)
{
  ...
  if(p_errno == NULL)
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Invalid argument(s) passed.");
    return FAILURE;
  }
  ...
  {
    curr_network_mode = ds3g_get_current_network_mode_ex(ds707_curr_subs_id());
  }
  else
  {
...
)
{
  ...
  *((uint32*)argval_ptr) = dscam_get_cam_timer_value();
  return SUCCESS;
#else
  DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "DS CAM feature not supported.");
  *p_errno = DS_EOPNOTSUPP;
  return FAILURE;
...
)
{
  ...
  *((boolean*)argval_ptr) = dsrdud_is_rdud_enabled();
  return SUCCESS;
#else
  DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "DS RDUD feature not supported.");
  *p_errno = DS_EOPNOTSUPP;
  return FAILURE;
...
)
{
  ...
  if( result == FALSE )
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Could not post SS command info get.");
    *p_errno = DS_ENOMEM;
    return FAILURE;
  }
...
)
{
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Client handler function not registered."
...
)
{
  ...
  /* Sanity check */
  DS3GPP2_ASSERT(ds707_pri_physlink_refcount != 0);
...
)
{
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "TC state ptr is NULL");
    return;
  }
  ...
  if (old_state == tun_phys_link_state)
  {
     DATA_IS707_MSG1(MSG_LEGACY_HIGH, 
                "Transition tun_phys_link_state to: %d"
  ...
  */
  ...
  DATA_IS707_MSG2(MSG_LEGACY_HIGH, 
                "Transition tun_phys_link_state: %d to: %d ",
                old_state, 
                tun_phys_link_state);
...
)
{
  if (NULL == tc_state_ptr )
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "get_tun_phys_link_state: "
  ...
  {
    DATA_IS707_MSG1(MSG_LEGACY_HIGH, 
                  "get_tun_phys_link_state: current state: %d ",
                  tc_state_ptr->tun_phys_link_state);
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch(old_sys)
  {
    ...
    default:
    {
      DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR, "Unhandled old_sys; %d", old_sys);
      break;
    }
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  else
  {
    ...
    DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR, "Bad NV read status %d for DS QCMIP. "
  ...
  if( status == NV_DONE_S )
  {
   ...
   -------------------------------------------------------------------------*/
                ...
                ds707_nv_item.data_pkt_orig_str.num_digits )
    {
      ...
      {
        DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR,"Error in copying Pkt dial string");
      }
      else
      {
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if(val == NULL)
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Null pointer passed in "
  ...
  switch(type)
  {
    ...
    case DS707_CMUX_VAL:
      ...
      {
        DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Invalid no_of_elements in "
                        "ds707_pkt_mgr_get_1x_profile_val");
        break;
      }	
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(val == NULL)
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Null string passed in "
  ...
  switch(type)
  {
    ...
    case DS707_CMUX_VAL:
      ...
      if( num_elements > MAX_CMUX_VAL )
      {
        DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Invalid no_of_elements");
        break;
      }	
    ...
    default:
      DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR,"No match for type %d",type);
  }

  return ret_val; 	
}
/*===========================================================================
FUNCTION DS707_PKT_MGR_UTIL_DPM_REG

DESCRIPTION
  This function will Register Phys link, Iface ptr and FWD wmk with
  data path manager. This function is called immediately after
  posting Iface UP indication to PS.
...
SIDE EFFECTS
  None
===========================================================================*/
boolean ds707_pkt_mgr_util_dpm_reg
(
  ps_iface_type   *iface_ptr
)
{
  ...
  dsm_watermark_type              * fwd_sn_wmk_list = NULL;
  ps_sys_rat_ex_enum_type           current_rat = PS_SYS_RAT_EX_NULL_BEARER;
  ...
  ps_iface_state_enum_type          v4_iface_state = IFACE_STATE_INVALID;
  ps_iface_state_enum_type          v6_iface_state = IFACE_STATE_INVALID;
  ps_dpm_um_reg_iface_info_type     um_reg_iface_info; 
  boolean                           reg_bearer = TRUE;
  uint8                               index; 
  ds707_pdn_context_cb_type       * index_pdn_context_cb = NULL;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if (!PS_IFACE_IS_VALID(iface_ptr)) 
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "ds707_pkt_mgr_util_dpm_reg()"
                                      "NULL Iface pointer received, return");
    return FALSE;  
  }
  pdn_cb_ptr = ds707_pdncntx_get_pdncntx_cb_ptr_from_iface_ptr(iface_ptr);

  if (NULL == pdn_cb_ptr) 
  {
    DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR, "ds707_pkt_mgr_util_dpm_reg()"
                                      "Cannot retrieve PDN CB for iface 0x%x,"
                                      " return", iface_ptr);
     return FALSE;  
  }

  memset(&um_reg_bearer_info,
             0x0,
          sizeof(ps_dpm_um_reg_bearer_info_type));

  memset(&um_reg_iface_info,
             0x0,
  ...
  fwd_sn_wmk_list = ds707_get_wmk_um_SN_fwd_wm_list();
  ps_phys_link_ptr =  PS_IFACEI_GET_PHYS_LINK(iface_ptr);
  if (NULL == fwd_sn_wmk_list
      ||
      NULL == ps_phys_link_ptr) 
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "ds707_pkt_mgr_util_dpm_reg()"
                                      "FWD SN UM watermark list "
                                      "or phys link is NULL"
                                      " return");
    return FALSE;  
  }

  /*-------------------------------------------------------------------------
    Only Route A is supported currently in 3GPP2. So hardcoding to RouteA
  -------------------------------------------------------------------------*/
  l2_to_ps_wm_ptr  =  fwd_sn_wmk_list+DSRSP_ROUTE_A;
  v4_iface_ptr = ds707_pdncntx_get_v4_iface(pdn_cb_ptr);
  v6_iface_ptr = ds707_pdncntx_get_v6_iface(pdn_cb_ptr);
  v4_iface_state = ps_iface_state(v4_iface_ptr);
  v6_iface_state = ps_iface_state(v6_iface_ptr);

  if(FALSE == ds707_pkt_mgr_get_bearer_tech_info(&bearer_tech))
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "ds707_pkt_mgr_util_dpm_reg()"
                                      "Cannot retrieve current nearer tech info,"
                                      " return");
    return FALSE;  
  } 
  if (bearer_tech.data.cdma_type.rat_mask == PS_IFACE_CDMA_NULL_BEARER)
  {
    current_rat = PS_SYS_RAT_EX_NULL_BEARER;
  }
  else if( bearer_tech.data.cdma_type.rat_mask == PS_IFACE_CDMA_1X
      ||
           bearer_tech.data.cdma_type.rat_mask == PS_IFACE_CDMA_FMC)
  {
     current_rat = PS_SYS_RAT_EX_3GPP2_1X;
  }
  else
  {
    ...
    {
       current_rat = PS_SYS_RAT_EX_3GPP2_EHRPD;
    }
    else
    {
      //jc: bug.. fix the inalid case
       current_rat = PS_SYS_RAT_EX_3GPP2_HRPD;
    }

  }
  /*-------------------------------------------------------------------------
    IF EPC system(EHRPD), gets and checks the associated IFACE from pdncb.
    IF NON-EPC system (1X or HRPD), gets the associated IFACE from the
    other PDN block. where MAX PDN INSTANCES is 2.
   
    IF the associated IFACE already registered with DPM(Iface is UP), 
    then register this IFACE with DPM. Otherwise register bearer with DPM.
  -------------------------------------------------------------------------*/
  switch (current_rat)
  {
    case DS_SYS_RAT_EX_3GPP2_HRPD:
    case PS_SYS_RAT_EX_3GPP2_1X:
      for (index = 0; index < DS707_MAX_PDN_INSTANCES; index++)
      {
        index_pdn_context_cb = ds707_get_pdncntx_cb_from_instance(index);
        if (DS707_PDNCNTX_CB_IS_VALID(index_pdn_context_cb)
            &&
            pdn_cb_ptr->this_pdn
               != index_pdn_context_cb)
        {
          if (IFACE_UP == v4_iface_state)
          {
            v6_iface_ptr = ds707_pdncntx_get_v6_iface(index_pdn_context_cb);
            v6_iface_state = ps_iface_state(v6_iface_ptr);    
          }
          else if (IFACE_UP == v6_iface_state)
          {
            v4_iface_ptr = ds707_pdncntx_get_v4_iface(index_pdn_context_cb);
            v4_iface_state = ps_iface_state(v4_iface_ptr);  
          }/*end of for index*/
        }/*end of if*/
      }/*end of for MAX PDN INSTANCES*/        
    /*fall through for regestration*/  
    case DS_SYS_RAT_EX_3GPP2_EHRPD:
    default:
      DATA_IS707_MSG1(MSG_LEGACY_HIGH, "ds707_pkt_mgr_util_dpm_reg()"
                                      "current RAT %d,",current_rat);

      if ( (IFACE_UP == v4_iface_state && IFACE_UP != v6_iface_state)
           ||
           (IFACE_UP != v4_iface_state && IFACE_UP == v6_iface_state))
      {
        reg_bearer = TRUE;  
      }
      else
  ...
  }/*end of switch current_rat*/

  if (TRUE == reg_bearer) 
  {

    um_reg_bearer_info.ps_phys_link_ptr =  ps_phys_link_ptr;
    um_reg_bearer_info.l2_to_ps_wm_ptr  =  l2_to_ps_wm_ptr;
    if ( !(v4_iface_state 
         &
         (IFACE_DISABLED | IFACE_DOWN | IFACE_GOING_DOWN)))
    {
      um_reg_bearer_info.v4_iface_ptr = v4_iface_ptr;
    }
    if ( !(v6_iface_state 
         &
         (IFACE_DISABLED | IFACE_DOWN | IFACE_GOING_DOWN)))
    {
      um_reg_bearer_info.v6_iface_ptr = v6_iface_ptr;
    }
    
    um_reg_bearer_info.rat =   current_rat; 
    if(-1 == ps_dpm_um_reg_bearer(&um_reg_bearer_info))
    {
      DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "ds707_pkt_mgr_util_dpm_reg()"
                                   "Unable to register to PS framework,"
                                   " return");
      return FALSE;  
    } 
    return TRUE;
  ...
  else
  {
    ...
    {
      um_reg_iface_info.registered_iface_ptr = v6_iface_ptr;
      um_reg_iface_info.iface_ptr            = v4_iface_ptr;  
    }
    else if (iface_ptr == v6_iface_ptr)
    {
      um_reg_iface_info.registered_iface_ptr = v4_iface_ptr;
      um_reg_iface_info.iface_ptr            = v6_iface_ptr;           
    }
    //create else
    if(-1 == ps_dpm_um_reg_iface(&um_reg_iface_info))
    {
      DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "ds707_pkt_mgr_util_dpm_reg()"
                                   "Unable to register to PS framework,"
                                   " return");
...
}/* ds707_pkt_mgr_util_dpm_reg() */

/*===========================================================================
FUNCTION DS707_PKT_MGR_UTIL_DPM_DEREG

DESCRIPTION
   This function Deregisters Iface with DPM. This function is called during
  Iface down Ind.
...
SIDE EFFECTS
  None
===========================================================================*/
boolean ds707_pkt_mgr_util_dpm_dereg
(
  ps_iface_type   *iface_ptr
)
{
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  memset(&um_dereg_iface_info,
             0x0,
          sizeof(ps_dpm_um_dereg_iface_info_type));

  iface_addr_family = ps_iface_get_addr_family(iface_ptr);
  if (IFACE_IPV4_ADDR_FAMILY == iface_addr_family) 
  {
    um_dereg_iface_info.v4_iface_ptr =  iface_ptr;
    um_dereg_iface_info.v6_iface_ptr = NULL;
    DATA_IS707_MSG1(MSG_LEGACY_HIGH, "ds707_pkt_mgr_util_dpm_reg()"
                        "Deregistering V4 iface 0x%x with DPM ",iface_ptr);
  }
  else if (IFACE_IPV6_ADDR_FAMILY == iface_addr_family)
  {
    um_dereg_iface_info.v6_iface_ptr =  iface_ptr;
    um_dereg_iface_info.v4_iface_ptr = NULL;
    DATA_IS707_MSG1(MSG_LEGACY_HIGH, "ds707_pkt_mgr_util_dpm_reg()"
                        "Deregistering V6 iface 0x%x with DPM",iface_ptr);
  }
  else
  {
    DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR, "ds707_pkt_mgr_util_dpm_reg()"
                   "Invalid IP address type %d, return",iface_addr_family);  
    return FALSE;
  }
  

  if(-1 == ps_dpm_um_dereg_iface(&um_dereg_iface_info))
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "ds707_pkt_mgr_util_dpm_dereg()"
                                      "Unable to de-register to PS framework,"
                                      " return");  
    return FALSE;
  }
  return TRUE;
}/* ds707_pkt_mgr_util_dpm_dereg() */

/*===========================================================================
FUNCTION DS707_PKT_MGR_UTIL_GET_DO_UL_TPUT_FROM_RAT_MASK

DESCRIPTION
  This function returns max supported throuput in kbps based on RAT mask
  for EVDO uplink
...
SIDE EFFECTS
  None
===========================================================================*/
uint32 ds707_pkt_mgr_util_get_DO_UL_tput_from_rat_mask
(
  uint32 rat_mask
)
{
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /* TODO: check if different rate can be used - hrong */
  ...
  return ret_val;
}/*ds707_pkt_mgr_util_get_DO_UL_tput_from_rat_mask*/

/*===========================================================================
FUNCTION DS707_PKT_MGR_UTIL_GET_DO_DL_TPUT_FROM_RAT_MASK
...
SIDE EFFECTS
  None
===========================================================================*/
uint32 ds707_pkt_mgr_util_get_DO_DL_tput_from_rat_mask
(
  uint32 rat_mask
)
{
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  return ret_val;
}/*ds707_pkt_mgr_util_get_DO_DL_tput_from_rat_mask*/

/*===========================================================================
FUNCTION DS707_PKT_MGR_UTIL_GET_THROUGHPUT_FROM_1X_RPC
...
RETURN VALUE
  UINT32 
===========================================================================*/
uint32 ds707_pkt_mgr_util_get_throughput_from_1X_rpc
(
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/  
  ...
  return rate;
} /* ds707_pkt_mgr_util_get_throughput_from_1X_rpc */

/*===========================================================================
FUNCTION DS707_PKT_MGR_UTIL_REPORT_CONFIGURED_THROUGHPUT

DESCRIPTION
  This function reports max allowed throughput to framework based on 
  the current 3GPP2 bearer technology
...
SIDE EFFECTS
  None
===========================================================================*/
void ds707_pkt_mgr_util_report_configured_throughput
(
  uint32 rat_mask
)
{
  ...
  int16  ps_errno = 0;
  uint32 ul_configured_throughput = 0;
  uint32 dl_configured_throughput = 0;
  ps_sys_configured_throughput_info_type report_val = {0};
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if ( rat_mask == PS_IFACE_CDMA_1X )
  {
    /*----------------------------------------------------------------------
      Hardcoding for now to report max possible value. Acutal value can be 
      retrieved from lower layer RPC rating rpc_get_sch_scrm_rate().
     ---------------------------------------------------------------------*/
    ul_configured_throughput = 
      ds707_pkt_mgr_util_get_throughput_from_1X_rpc(ENC_SCH_16X_RATE);
    dl_configured_throughput = ul_configured_throughput;
  }
  else if (rat_mask != PS_IFACE_CDMA_NULL_BEARER)
  {
    ul_configured_throughput = 
      ds707_pkt_mgr_util_get_DO_UL_tput_from_rat_mask(rat_mask);

    dl_configured_throughput = 
      ds707_pkt_mgr_util_get_DO_DL_tput_from_rat_mask(rat_mask);
  }

  /*----------------------------------------------------------------------
    Report configured throughput info to PS framework when it's none 0. 
    PS framework will handle OOS case.
   ---------------------------------------------------------------------*/
  if( (ul_configured_throughput != 0) ||
      (dl_configured_throughput != 0) )
  {
    report_val.ul_configured_throughput = ul_configured_throughput;
    report_val.dl_configured_throughput = dl_configured_throughput;
    ...
    {
      DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, 
                      "Cannot send configured throughput info to PS framework");
    }    
  }
}/* ds707_pkt_mgr_util_report_configured_dl_throughput */
...
/*===========================================================================
FUNCTION      DS707_PKT_MGR_CONVERT_DS_TO_MCFG_SUBS

DESCRIPTION   
  This function Subs id form DS to MCFG type
...
SIDE EFFECTS  None.
===========================================================================*/
mcfg_fs_sub_id_e_type ds707_pkt_mgr_convert_ds_to_mcfg_subs
(
  ds_sys_subscription_enum_type subs
)
{
  ...
  DATA_IS707_MSG2(MSG_LEGACY_HIGH, 
                  "MCFG subs %d, DS subs %d", mcfg_subs_id, subs);
  return mcfg_subs_id;
}

/*===========================================================================
FUNCTION      DS707_PKT_MGR_GET_FALLBACK_APN_LIST

DESCRIPTION   
  This function allows to get fallback apn list stored in EFS
...
SIDE EFFECTS  None.
===========================================================================*/
int ds707_pkt_mgr_util_get_fallback_apn_list
(
  /* page monitor value, filled in */
  void *argval_ptr, 
...
)
{
  ...
  ds_sys_3gpp2_ehrpd_fallback_apn_list_type *apn_list;

  DATA_IS707_MSG1( MSG_LEGACY_HIGH,
                   "ds707_pkt_mgr_util_set_fallback_apn_list: %d", subs_id);

  if (NULL == ps_errno || NULL == argval_ptr)
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR,
                    "Invalid argument");
    return -1;
  ...
  apn_list = ((ds_sys_3gpp2_ehrpd_fallback_apn_list_type*)argval_ptr);

  apn_list->fallback_apn_list_num = 0;

  if(ds3gcfgmgr_efs_file_init_ex(DS_PKT_MGR_EHRPD_FB_APN_FILE, 
               ds707_pkt_mgr_convert_ds_to_mcfg_subs(subs_id), &efs_db) == -1)
  {
         ...
         != (efs_ret_val = ds3gcfgmgr_efs_tokenizer(&efs_db, &from, &to )))
  {
    ...
    else if(DS3GCFGMGR_EFS_TOKEN_PARSE_SUCCESS == efs_ret_val)
    {
      ...
      if ( (from > to)  || (length > DS_SYS_MAX_APN_LEN))
      {
        DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR,
      ...
      {
        (void)memscpy(apn_list->fallback_apn_list[apn_list->fallback_apn_list_num++].apn_name,
                      length,
                      from,
                      length);
        DATA_IS707_MSG2(MSG_LEGACY_HIGH,
                        "APN[%d] = %s", (apn_list->fallback_apn_list_num - 1),
                        apn_list->fallback_apn_list[apn_list->fallback_apn_list_num - 1].apn_name);
      }
    }
  }
  ...
  *ps_errno = DS_ENOERR;
  return ret;
} /* ds707_pkt_mgr_util_get_fallback_apn_list */


/*===========================================================================
FUNCTION      DS707_PKT_MGR_SET_FALLBACK_APN_LIST

DESCRIPTION   
  This function allows to set fallback apn list stored in EFS
...
SIDE EFFECTS  None.
===========================================================================*/
int ds707_pkt_mgr_util_set_fallback_apn_list
(
  /* page monitor value, filled in */
  void *argval_ptr, 
  ds_sys_subscription_enum_type subs_id, 
...
)
{
  int     ret = 0;
  uint8   index = 0;
  boolean fb_apn_file_exists = TRUE;
  char    temp_buf[DS_SYS_MAX_APN_LEN];
  int     fd;
  int     efs_ret;

  /* structure used for tokenizing the file */
  ds3gcfgmgr_efs_token_type               efs_db; 
  /*store each string read from the efs, might be apn name or pdb label*/

  /* ptrs to start and end of the token */
//  char *from = NULL, *to = NULL; 

  ds_sys_3gpp2_ehrpd_fallback_apn_list_type *apn_list = NULL;
           ...
           ds707_pkt_mgr_convert_ds_to_mcfg_subs(subs_id), &efs_db) == -1)
  {
    DATA_IS707_MSG0(MSG_LEGACY_HIGH, "Error opening file ISN from EFS");
    *ps_errno = DS_EFAULT;
    fb_apn_file_exists = FALSE;
  }

  if(apn_list->fallback_apn_list_num > DS_SYS_EHRPD_FALLBACK_APN_MAX)
  {
    *ps_errno = DS_EDBOVERFLOW;
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR,
                    "Error in APNs Count. Exceeding max apn.");
    return -1;
  }

   /*-------------------------------------------------------------------------
    Create a tmp ehrpd fallback apn file
  -------------------------------------------------------------------------*/
  fd = mcfg_fopen(DS_PKT_MGR_TMP_EHRPD_FB_APN_FILE,
                  MCFG_FS_O_CREAT| MCFG_FS_O_WRONLY,
                  MCFG_FS_DEFFILEMODE,
                  MCFG_FS_TYPE_EFS,
                  ds707_pkt_mgr_convert_ds_to_mcfg_subs(subs_id) );
  if (-1 == fd)
  {
    DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR,
                    "Creating DS_PKT_MGR_TMP_EHRPD_FB_APN_FILE fails Err: %d",
                     mcfg_fs_errno(MCFG_FS_TYPE_EFS));
    return -1;
  }

  while (index < apn_list->fallback_apn_list_num)
  {
    memset(temp_buf, '\0', sizeof(temp_buf));
  
    snprintf(temp_buf,
             sizeof(temp_buf) - 1,
             "%s\n", apn_list->fallback_apn_list[index++].apn_name);
  
    efs_ret = mcfg_fwrite (fd, temp_buf, strlen(temp_buf), MCFG_FS_TYPE_EFS);

  }
  if (-1 == mcfg_fclose(fd, MCFG_FS_TYPE_EFS))
  {
     DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR,
                    " close eHRPD fallback tmp file failed ");
     return -1;
  }

  if(fb_apn_file_exists)
  {
    if (MCFG_FS_STATUS_OK != mcfg_fs_delete(DS_PKT_MGR_EHRPD_FB_APN_FILE, 
	                         MCFG_FS_TYPE_EFS,
                                 ds707_pkt_mgr_convert_ds_to_mcfg_subs(subs_id)))
    {
      DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR,
                      "Efs unlink original eHPRD fb apn file failed");
      return -1;
    }
  }

  if (MCFG_FS_STATUS_OK != mcfg_fs_rename(DS_PKT_MGR_TMP_EHRPD_FB_APN_FILE,
                           DS_PKT_MGR_EHRPD_FB_APN_FILE, 
                           MCFG_FS_TYPE_EFS,
                           ds707_pkt_mgr_convert_ds_to_mcfg_subs(subs_id) ))
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR,
                    " efs rename ehrpd fallback tmp file failed ");
    return -1;
  }

  DATA_IS707_MSG1(MSG_LEGACY_HIGH,
                  "Successfully wrote %d FB APNs in the EFS File ",
                  apn_list->fallback_apn_list_num );
