...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp2/ds707/src/ds707_pkt_mgr_ioctl.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $

when        who    what, where, why
--------    ---    ----------------------------------------------------------
04/01/16    sd     Fixed subs_id handling for 3GPP2 in DSDA mode.
...
#if 0
/*===========================================================================
FUNCTION      DS707_PKT_MGR_PHYS_LINK_IOCTL_FLOW_CTL

DESCRIPTION   Handles the IOCTL to enable or disable flow on all a physlinks
...
SIDE EFFECTS  None
===========================================================================*/
static int ds707_pkt_mgr_phys_link_ioctl_flow_ctl
(
...
/*===========================================================================
FUNCTION      DS707_PKT_MGR_PHYS_LINK_IOCTL_GET_BYTES

DESCRIPTION   Handles the IOCTL to get the allowed bytes for a particular 
              physlink 
...
SIDE EFFECTS  None
===========================================================================*/
static int ds707_pkt_mgr_phys_link_ioctl_get_bytes
(
  ps_phys_link_type        *phys_link_ptr,
  void                     *argval_ptr,
  int16                    *ps_errno
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/  
  switch (ioctl_name)
  {
    ...
    default:
      DATA_DOQOS_MSG1( MSG_LEGACY_HIGH,
...
)
{
  uint8             i;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  DS3GPP2_ASSERT(qos_ioctl_arg_ex != NULL);
  DS3GPP2_ASSERT(iface_ptr != NULL);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  DS3GPP2_ASSERT(ps_flow != NULL);
  DS3GPP2_ASSERT(ioctl_errno != NULL);
  ...
  ----------------------------------------------------------------*/
  DS3GPP2_ASSERT(num_of_specs <=MAX_QOS_OPERATIONS_PER_IOCTL);
  ...
  *ioctl_errno = DS_EBADF;  
  ...
  for (i = 0; i <num_of_specs; i++)
  {
    ...
    {
      DS_QOS_ERROR_LOG_2(MSG_LEGACY_ERROR,
    ...
    --------------------------------------------------------------------*/
    DS3GPP2_ASSERT((ret_val == -1) && (*ioctl_errno == DS_EWOULDBLOCK));
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  for (i = 0; i<num_of_specs; i++)
  {
    if(PS_FLOW_GET_STATE(ps_flow[i]) != FLOW_NULL) //Flow could have gone null and then deleted as part of NON-EPC event handling.
    {
      ...
      DS3GPP2_ASSERT( (ret_val == -1) && (flow_errorno == DS_EWOULDBLOCK));
    }
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/  
  if(ps_errno == NULL)
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Invalid ps_errno");
    return (-1);
  }
  
  if(argval_ptr == NULL)
  {
    DS_QOS_ERROR_LOG_0(MSG_LEGACY_ERROR, "Invalid parameter argval_ptr");
    *ps_errno = DS_EFAULT;
    return (-1);
  }
  ...
  {
    DS_QOS_ERROR_LOG_1(MSG_LEGACY_ERROR, "Invalid Flow 0x%x", ps_flow_ptr);
    *ps_errno = DS_EFAULT;
    return (-1);
  }
  ...
  {
    DS_QOS_ERROR_LOG_0(MSG_LEGACY_ERROR, "Current System Does not support QoS");
  ...
  if(ds707_sec_pkt_mgr_modify_flow(
                                    ...
                                    ps_flow_ptr) == FALSE)
  {
    DS_QOS_ERROR_LOG_1(MSG_LEGACY_ERROR, "Flow 0x%p could not be modified",
                     ps_flow_ptr);
    *ps_errno = DS_EFAULT;
    return (-1);
  }
  ...
  ----------------------------------------------------------------------*/
  DATA_DOQOS_MSG1( MSG_LEGACY_HIGH, "QMOD:Suspending flow:0x%x", ps_flow_ptr);
  ...
  if(ds_flow_ptr == NULL)
  {
    DS_QOS_ERROR_LOG_0(MSG_LEGACY_ERROR, "ds_flow_ptr is NULL ");
    *ps_errno = DS_EFAULT;
    return (-1);
  }  	
...
/*- - - -  - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(!PS_FLOW_IS_VALID(ps_flow_ptr))
  {
    DS_QOS_ERROR_LOG_1(MSG_LEGACY_ERROR, "Invalid PS flow 0x%x", ps_flow_ptr);
    *ps_errno = DS_EINVAL;
    return (-1);
  }
  ...
  if ( (PS_FLOW_GET_STATE(ps_flow_ptr) == FLOW_NULL) ||
       (PS_FLOW_GET_STATE(ps_flow_ptr) == FLOW_GOING_NULL))
  {
    DATA_DOQOS_MSG2( MSG_LEGACY_HIGH,
  ...
  DATA_DOQOS_MSG0( MSG_LEGACY_HIGH,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (argval_ptr == NULL)
  {
    DS_QOS_ERROR_LOG_0(MSG_LEGACY_ERROR, "argval_ptr NULL");
    *ps_errno = DS_EFAULT;
    return(-1);
  }
  ...
  {
    DS_QOS_ERROR_LOG_0(MSG_LEGACY_ERROR, "Error on bundle flows");
    *ps_errno = DS_EINVAL;
    return(-1);
  }
  ...
  {
    DS_QOS_ERROR_LOG_0(MSG_LEGACY_ERROR,
                    "QOS release ioctl is not allowed for network "
                    "initiated QOS flow");
    *ps_errno = DS_EOPNOTSUPP;
    return (-1);
  ...
  ds_put_cmd(cmd_ptr);
  DATA_DOQOS_MSG0( MSG_LEGACY_HIGH, "QOS_RELEASE_EX processed");
  return 0;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DATA_DOQOS_MSG0( MSG_LEGACY_HIGH, "Reservation is OFF");
    ps_flow_suspend_ind( ps_flow_ptr,
                         PS_EIC_NOT_SPECIFIED);
    return 0;
  }
  ...
  DATA_DOQOS_MSG0( MSG_LEGACY_HIGH, "QOS_SUSPEND processed");
  return 0;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  for (i = 0; i< qos_ioctl_arg_suspend_ex->num_flows; i++)
  {
    ...
    {
      DATA_DOQOS_MSG1( MSG_LEGACY_HIGH, "Modify in progress 0x%x",
    ...
    if (PS_FLOW_GET_STATE(qos_ioctl_arg_suspend_ex->flows_ptr[i]) == 
                                                             FLOW_SUSPENDED)
    {
      DATA_DOQOS_MSG0( MSG_LEGACY_HIGH, "Already SUSPENDED");
  ...
  if (num_valid_flows == 0)
  {
    DATA_DOQOS_MSG0( MSG_LEGACY_HIGH, "No valid PS flow");
    return 0;
  }
  ...
  DATA_DOQOS_MSG0( MSG_LEGACY_HIGH, "QOS_SUSPEND_EX processed");
  return 0;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
                  ...
                  (ds707_flow_type *) ps_flow_ptr->client_data_ptr) == TRUE)
  {
    if (ds707_qos_flow_mgr_is_qos_granted_for_flow(
              (ds707_flow_type *) ps_flow_ptr->client_data_ptr) == TRUE)
    {
      DS_QOS_ERROR_LOG_1(MSG_LEGACY_ERROR, "Activate 0x%x", ps_flow_ptr);
  ...
  DATA_DOQOS_MSG0( MSG_LEGACY_HIGH, "QOS_RESUME processed");
  return 0;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (argval_ptr == NULL)
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "argval_ptr NULL");
    *ps_errno = DS_EFAULT;
    return(-1);
  ...
  for (i = 0; i< qos_ioctl_arg_resume_ex->num_flows; i++)
  {
                  ...
                  &setid_non_zero) == TRUE)
    {
      ...
      if (setid_non_zero == FALSE)
      {
        DS_QOS_ERROR_LOG_1(MSG_LEGACY_ERROR, "SETID zero 0x%x",
                         qos_ioctl_arg_resume_ex->flows_ptr[i]);
        /*----------------------------------------------------------
        Suspend the flow
        -----------------------------------------------------------*/
    ...
    else
    {
      /*-------------------------------------------------------------------
        ...
        request and start GRANT TIMER.
      --------------------------------------------------------------------*/
      DATA_DOQOS_MSG0( MSG_LEGACY_LOW,
                     ...
                     (ds707_flow_type *)qos_ioctl_arg_resume_ex->flows_ptr[i]->client_data_ptr) == TRUE)
    {
      if (ds707_qos_flow_mgr_is_qos_granted_for_flow(
                    (ds707_flow_type *)qos_ioctl_arg_resume_ex->flows_ptr[i]->client_data_ptr) == TRUE)
      {
        DS_QOS_ERROR_LOG_1(MSG_LEGACY_ERROR, "Activate 0x%x",
                         qos_ioctl_arg_resume_ex->flows_ptr[i]);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
      ...
      )
  {
    ...
    {
      DS_QOS_ERROR_LOG_0(MSG_LEGACY_ERROR, "Cannot get the RLP ID");
      *ps_errno = DS_EFAULT;
      return -1;
    }
    ...
    {
      DS_QOS_ERROR_LOG_2(MSG_LEGACY_ERROR, "Invalid RMAC ID: %d for this RLP %d",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
     ...
     )
  {
    ...
    if ( ds_flow_ptr == NULL )
    {
      DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Cannot get a valid ds_flow_ptr");
      *ps_errno = DS_EFAULT;
      return -1;
    }
    ...
    {
      DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "NULL Watermark Pointer");
      *ps_errno = DS_EFAULT;
      return -1;
    }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if ( argval_ptr == NULL )
  {
    DS_QOS_ERROR_LOG_0(MSG_LEGACY_ERROR, "Invalid argval_ptr");
    *ps_errno = DS_EFAULT;
    return -1;
  }
  ...
  if ( !(PS_FLOW_IS_VALID(ps_flow_ptr)))
  {
    DS_QOS_ERROR_LOG_0(MSG_LEGACY_ERROR, "Invalid ps_flow_ptr");
    *ps_errno = DS_EFAULT;
    return -1;
  }
     ...
     )
  {
    ...
    if ( ds_flow_ptr == NULL )
    {
      DS_QOS_ERROR_LOG_0(MSG_LEGACY_ERROR, "Cannot get a valid ds_flow_ptr");
      *ps_errno = DS_EFAULT;
      return -1;
    }
    ...
    {
      DS_QOS_ERROR_LOG_0(MSG_LEGACY_ERROR, "Cannot get a valid rev_tc_ptr");
      *ps_errno = DS_EFAULT;
      return -1;
    }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  * ((ps_flow_ioctl_707_inactivity_timer_type*)argval_ptr) = 
  ...
  DATA_DOQOS_MSG1(MSG_LEGACY_MED, "Inactivity timer val = 0x%x",
                  *((ps_flow_ioctl_707_inactivity_timer_type*)argval_ptr));
  return 0;
}
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (ret_val == 0) 
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Iface already down");
    return ret_val;
  }
  if ((ret_val < 0) && 
      (*ps_errno != DS_EWOULDBLOCK))
  {
    DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR, "Iface tear down failed, errno: %d",
                     ps_errno);
    return ret_val;
  }
  else
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Iface tear down in progress");
    return 0;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (ps_iface_addr_family_is_v4(iface_ptr))
  {
    dssnet_instance =  ds707_pkt_mgr_get_dssnet4_sm_cb (iface_ptr);
    {
      ...
      {
        DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR, "ioctl ipcp_dns_opt on iface:0x%x", 
                        iface_ptr);
        *ps_errno = DS_EFAULT;
        return -1;
      }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "ioctl getmdr failed. Null arg ptr");
    *ps_errno = DS_EFAULT;
    return -1;
  }
  return 0;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "ioctl setmdr failed. Null arg ptr");
    *ps_errno = DS_EFAULT;
    return -1;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "ioctl getdorm failed. Null arg ptr");
    *ps_errno = DS_EFAULT;
    return -1;
  }
  return 0;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "ioctl setdorm failed. Null arg ptr");
    *ps_errno = DS_EFAULT;
    return -1;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "ioctl get rlp curr nak fail");
    *ps_errno = DS_EFAULT;
    return -1;
  }
  return 0;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "ioctl set rlp curr nak fail");
    *ps_errno = DS_EFAULT;
    return -1;
  }
  return 0;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "ioctl get rlp default nak fail");
    *ps_errno = DS_EFAULT;
    return -1;
  }
  return 0;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "ioctl set rlp default nak fail");
    *ps_errno = DS_EFAULT;
    return -1;
  }
  return 0;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "ioctl get rlp negotiated nak fail");
    *ps_errno = DS_EFAULT;
    return -1;
  }
  return 0;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ( argval_ptr != NULL )
  {
    ...
    {
      DS_IS707_ERROR_LOG_2(MSG_LEGACY_ERROR, "ioctl set rlp fill rate failed. "
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR,"Error in obataining data sesion ptr");
    return -1;
  }
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "ioctl Get Bearer Technology failed.");
    *ps_errno = DS_EFAULT;
    return -1;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "ioctl sdb query failed. Null arg ptr");
    *ps_errno = DS_EFAULT;
    return -1;
  }
  DATA_IS707_MSG1( MSG_LEGACY_HIGH,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_QOS_ERROR_LOG_0(MSG_LEGACY_ERROR, "ioctl fail, phys_link_ptr NULL");
    *ps_errno = DS_EFAULT;
    return -1;
  }
  ...
  if (PS_FLOW_GET_STATE(ps_flow_ptr) != FLOW_NULL)
  {
    DS_QOS_ERROR_LOG_2(MSG_LEGACY_ERROR, "0x%x in %d state, not in NULL STATE",
  ...
  DATA_DOQOS_MSG0(MSG_LEGACY_HIGH, "QOS_REQUEST processed");
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DATA_DOQOS_MSG1( MSG_LEGACY_MED, "Invalid opcode :0x%x",
                     qos_ioctl_arg_ex->opcode);
    *ps_errno = DS_EFAULT;
    PS_LEAVE_CRIT_SECTION(&global_ps_crit_section);
    return -1; 
...
)
{
  ...
  if ( argval_ptr == NULL )
  {
    DS_QOS_ERROR_LOG_0(MSG_LEGACY_ERROR, "Invalid ArgValPtr Passed");
    if(ps_errno != NULL)
    {
      *ps_errno = DS_EFAULT;
    }
  ...
  DATA_DOQOS_MSG1( MSG_LEGACY_MED,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ( argval_ptr != NULL )
  {
    ...
    else 
    {
      ...
      DATA_DOQOS_MSG1(MSG_LEGACY_HIGH, "Enable HDR slot cycle index params. "
                      "Returning Error: %d", *ps_errno);
    }
  }
  else
  {
    DS_QOS_ERROR_LOG_0(MSG_LEGACY_ERROR, "Slotted Mode - ioctl use hdr slot "
...
)
{
  ...
  *ps_errno = 0;
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "ioctl SET HAT failed. Null arg ptr");
    *ps_errno = DS_EFAULT;
    return -1;
  }
 ...
 *ps_errno = 0;
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "ioctl GET HAT failed. Null arg ptr");
    *ps_errno = DS_EFAULT;
    return -1;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (ds707_pkt_is_dormant(phys_link_ptr) == TRUE)
  {
    ...
    {
      DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR, "ioctl goactive fail, "
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ( ds707_pkti_can_go_dormant ( tc_state_ptr, phys_link_ptr ) == TRUE )
  {
    ...
    if ((ret_val < 0) && ( ioctl_errno != DS_EWOULDBLOCK))
    {
      DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR, "ioctl godorm fail, %d phys_link ret",
                      ioctl_errno);
      *ps_errno = DS_EFAULT;
      return -1;
    }
  ...
  {
    DS_IS707_ERROR_LOG_2(MSG_LEGACY_ERROR, "ioctl godorm fail phys = %d, so = %d",
...
)
{
  ...
  if (ps_errno == NULL) 
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Invalid ps_errno ptr");
    return -1;
  }
  if (!PS_IFACE_IS_VALID(iface_ptr)) 
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Invalid IFACE ptr");
    *ps_errno = DS_EFAULT;
    return -1;
  }
  ...
  if (argval_ptr == NULL) 
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Invalid argval ptr");
    *ps_errno = DS_EFAULT;
    return -1;
  }
  ...
  if (ret_val == -1)
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR,"Error in getting epc handoff ctxt");
  }
  else 
  {
...
)
{
  ...
  if (ret_val == -1)
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR,"Error in setting epc handoff ctxt");
  }
  else 
  {
...
)
{
  ...
  if (!PS_IFACE_IS_VALID(iface_ptr)) 
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Invalid IFACE ptr, fail ioctl");
    *ps_errno = DS_EFAULT;
    return -1;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if((NULL == cmd_ptr) || (NULL == cmd_ptr->cmd_payload_ptr))
  {
   ...
   ds3g_timer_stop(DS3G_TIMER_DUN_CTRL_ENTL);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Invalid MIP");
    *ps_errno = DS_EINVAL;
    return -1;
  }
...
*/
   ...
   case PS_IFACE_IOCTL_707_SET_HYSTERESIS_ACT_TIMER:
    ...
    ---------------------------------------------------------------------*/
    default:
      DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR, "No such 707pkt ioctl: %d", 
                      ioctl_name);
      *ps_errno = DS_EOPNOTSUPP;
      ret_val = -1;
...
//#endif /*DSDA */
  ...
  switch (ioctl_name)
  {
    ...
    case PS_SYS_IOCTL_3GPP2_SET_EHRPD_FALLBACK_APN_LIST:
    {
      ret_val = ds707_pkt_mgr_util_set_fallback_apn_list(argval_ptr, 
                         (ds_sys_subscription_enum_type)subs_id, ps_errno);
      break;
    }
    case PS_SYS_IOCTL_3GPP2_GET_EHRPD_FALLBACK_APN_LIST:
    {
      ret_val = ds707_pkt_mgr_util_get_fallback_apn_list(argval_ptr, 
                         (ds_sys_subscription_enum_type)subs_id, ps_errno);
      break;
    }
    ...
    case PS_SYS_IOCTL_3GPP2_GET_PROFILE_ID_FROM_APN_INFO:
      ret_val = ds3g_get_profile_id_from_apn_info (argval_ptr,
                                                   as_subs_id,
                                                   ps_errno,
                                                   PS_SYS_TECH_3GPP2);
      break;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  *ps_errno = 0;
  ...
  if (phys_link_ptr == NULL)
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "ioctl fail, phys_link_ptr NULL");
    *ps_errno = DS_EFAULT;
    return -1;
  }
...
#ifdef FEATURE_DATA_FLOW_MGMT
      /*---------------------------------------------------------------------
        Ioctl to disable flow on all physlinks associated with a flow
      ---------------------------------------------------------------------*/
#if 0      
    case PS_PHYS_LINK_IOCTL_FLOW_CTL:  
      ret_val = ds707_pkt_mgr_phys_link_ioctl_flow_ctl( phys_link_ptr,
                                                        argval_ptr,
                                                        ps_errno);
      break;

    /*---------------------------------------------------------------------
      Ioctl to get the number of allowed bytes on a particular physlink
    ---------------------------------------------------------------------*/
    case PS_PHYS_LINK_IOCTL_GET_NUM_ALLOWED_BYTES:
      ret_val = ds707_pkt_mgr_phys_link_ioctl_get_bytes( phys_link_ptr,
                                                         argval_ptr,
                                                         ps_errno);
      break;
#endif	  /* If 0 */
      ...
      ---------------------------------------------------------------------*/
    default:
      DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR,"No such 707pkt phys_link_ioctl: %d", 
                      ioctl_name);
      *ps_errno = DS_EOPNOTSUPP;
      ret_val = -1 ;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  *ps_errno = 0;
  ...
  /*lint -restore*/
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "ioctl fail, flow_ptr NULL");
    *ps_errno = DS_EFAULT;
    return -1;
  }
...
{
  ds707_pkt_phys_link_flow_ctl_s_type   *flow_ctl_info_ptr = NULL;
  ds_cmd_type   			*cmd_ptr;
  boolean                                ret_val = -1;
  ds707_local_err_val_e_type             local_err = DS707_LOCAL_ERR_VAL_NONE;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  do 
  {
    if( NULL == ps_errno )
    {
      local_err = DS707_LOCAL_ERR_VAL_0;
      break;
    }
     
    if( NULL == argval_ptr)
    {
      local_err = DS707_LOCAL_ERR_VAL_1;
      *ps_errno = DS_EFAULT;
      break;
    }
         
    cmd_ptr = ds_allocate_cmd_buf(sizeof(ds707_pkt_phys_link_flow_ctl_s_type));

    if((NULL == cmd_ptr) || (NULL == cmd_ptr->cmd_payload_ptr))
    {
      ASSERT(0);
      local_err = DS707_LOCAL_ERR_VAL_2;
      break;
    }

    flow_ctl_info_ptr = 
        (ds707_pkt_phys_link_flow_ctl_s_type*)cmd_ptr->cmd_payload_ptr;

    memscpy(&flow_ctl_info_ptr->flow_ctl_info,
            sizeof(ps_phys_link_ioctl_flow_ctl_type),
            argval_ptr,
            sizeof(ps_phys_link_ioctl_flow_ctl_type));    

    flow_ctl_info_ptr->phys_link_ptr = phys_link_ptr;
     
    cmd_ptr->hdr.cmd_id = DS_CMD_707_PHYS_LINK_DISABLE_FLOW;

    if (flow_ctl_info_ptr->flow_ctl_info.flow_ctl_req == 
        PS_PHYS_LINK_IOCTL_FLOW_ENABLE) 
    {
      cmd_ptr->hdr.cmd_id = DS_CMD_707_PHYS_LINK_ENABLE_FLOW;
    }

    DATA_IS707_MSG2( MSG_LEGACY_HIGH, 
                   "BURSTY_FLOW: Phys_link_ptr: %d, Flow ctl status: %d",
                   flow_ctl_info_ptr->phys_link_ptr, 
                   flow_ctl_info_ptr->flow_ctl_info.flow_ctl_req);
  ...
  if (local_err != DS707_LOCAL_ERR_VAL_NONE) 
  {
    DATA_IS707_MSG1( MSG_LEGACY_ERROR, 
                    "BURSTY_FLOW: IOCTL flow cntrl, local_err: %d",
                    local_err);    
  }
  
  return ret_val;

}/* ds707_pkt_mgr_phys_link_ioctl_flow_ctl */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  do 
  {
    ...
    /*---------------------------------------------------------------------
      Get the bytes allowed for this watermark and pass it on to PS
    ---------------------------------------------------------------------*/
    ((ps_phys_link_ioctl_allowed_num_bytes_type*)argval_ptr)->num_of_bytes = 
       ds3g_burst_get_bytes_allowed((void *)ds707_wmk_get_pri_wm_ptr()); 
  ...
  {
    DATA_IS707_MSG1( MSG_LEGACY_ERROR, 
                    "BURSTY_FLOW: Get allowed bytes, local_err: %d",
                    local_err);   
  }

  return ret_val;
} /* ds707_pkt_mgr_phys_link_ioctl_get_bytes */
#endif /* If 0 */
#endif /* FEATURE_DATA_FLOW_MGMT */
