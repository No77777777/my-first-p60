...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp2/ds707/src/ds707_wmk.c#2 $ 
  $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $

when       who        what, where, why
--------   ---        ------------------------------------------------------- 
03/24/15   sc         Initialize high and low sec WM during allocation
...
typedef struct
{
  ...
  dsm_watermark_type                 um_SN_rev_wmk_list[DSRSP_NUM_ROUTES];
  q_type                             um_SN_rev_q_list[DSRSP_NUM_ROUTES];
  /*Timer for the flow disable*/
  rex_timer_type                     pri_flow_disable_timer;
  /*boolean indicating if flow disable timer is running or not*/
  boolean                            pri_flow_disable_timer_running;
  ps_flow_type                      *ps_flow_ptr[DS707_MAX_IFACES];
#ifdef FEATURE_DATA_FLOW_MGMT
  /*structure that holds burst based paramters*/
  ds707_burst_flow_control_s         burst_params;  
#endif
  boolean                            tcp_bi_directional;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "wmk_buf_ptr or ps_flow_ptr is NULL");
    return;
  }
...
#ifdef FEATURE_DATA_FLOW_MGMT
  ds_707_burst_flow_tx_gone_empty_func(wm_ptr, user_data_ptr);
#endif 
...
/*-------------------------------------------------------------------------- 
  STOP Flow Disable timer if already running
  -------------------------------------------------------------------------*/

  if (is_pri_wmk_buf)
  {
    if (TRUE == ds707_wmk_pri_buf.pri_flow_disable_timer_running)
    {
      (void)rex_clr_timer(&(ds707_wmk_pri_buf.pri_flow_disable_timer));
      ds707_wmk_pri_buf.pri_flow_disable_timer_running = FALSE;
      DATA_IS707_MSG0( MSG_LEGACY_MED, "timer stopped ");
    }
  }
       ...
       (ds707_get_dsm_fc_flag() == TRUE) )
  {
    ...
    if (is_pri_wmk_buf) 
    {
      ds3g_burst_enable_flow( ds707_pkt_get_pri_phys_link_ptr(),
                              &wm_list[route],
                              DS_FLOW_PS_FLOW_TX_WM_MASK|DS_FLOW_PS_FLOW_TX_DNE_WM_MASK,
                              &(ds707_wmk_pri_buf.burst_params.burst_params),
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ( wmk_buf_ptr == NULL )
  {
    DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR, "wmk_buf_ptr not set! No Flow control "
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ( wmk_buf_ptr == NULL )
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "wmk_buf_ptr is NULL");
    return;
  }
  wm_list = wmk_buf_ptr->um_SN_rev_wmk_list;
...
/*===========================================================================
FUNCTION      DS707_PRI_WMK_FLOW_DISABLE_TIMER_CB

DESCRIPTION   Called when the flow_disable_timer expires. This function 
              posts a command to the DS task for necessary handling
...
void ds707_pri_wmk_flow_disable_timer_cb
(
  unsigned long cb_data
)
{
  ds707_pri_wmk_buf_type *wmk_buf_ptr = NULL;
  ds_cmd_type            *ds_cmd_ptr;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  wmk_buf_ptr = (ds707_pri_wmk_buf_type *)cb_data;
  ...
  ds_cmd_ptr = ds_allocate_cmd_buf(sizeof(ds707_pri_wmk_buf_type));
  ...
  if (NULL != ds_cmd_ptr && NULL != ds_cmd_ptr->cmd_payload_ptr)
  {
    /*-----------------------------------------------------------------------
     Populate the DS command buffer and post command to DS
    -----------------------------------------------------------------------*/
    ds_cmd_ptr->hdr.cmd_id = DS_CMD_707_PRI_FLOW_DISABLE_TMR_EXPIRY;
    ds_cmd_ptr->cmd_payload_ptr = wmk_buf_ptr;
    ds_put_cmd( ds_cmd_ptr );
  }
}/*ds707_pri_wmk_flow_disable_timer_cb*/

/*===========================================================================
FUNCTION      DS707_WMK_PROCESS_FLOW_DISABLE_TIMER_EXPIRY_CMD

DESCRIPTION   This function processes the flow disable timer cmd expiry.  
...
SIDE EFFECTS  None.
===========================================================================*/
void ds707_wmk_process_flow_disable_timer_expiry_cmd
(
  ds_cmd_type   *ds_cmd_ptr
)
{
  ds707_pri_wmk_buf_type      * pri_wmk_buf_ptr;
  ps_iface_type               * iface_ptr;
  boolean                       disable_flow_cntrl = FALSE;
  uint8                         curr_ps_flow_index = 0;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

  DATA_IS707_MSG0( MSG_LEGACY_HIGH, 
    "In ds707_wmk_process_flow_disable_timer_expiry_cmd");

  if (ds_cmd_ptr == NULL)
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, 
    "Command pointer NULL in ds707_wmk_process_flow_disable_timer_expiry_cmd");
    return;
  }

  pri_wmk_buf_ptr = (ds707_pri_wmk_buf_type *)(ds_cmd_ptr);

  if ( pri_wmk_buf_ptr == NULL )
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "pri_wmk_buf_ptr is NULL");
    return;
  }

  /*------------------------------------------------------------------- 
  We are here which means the flow_disable_timer has run its course
  Put the timer value to FALSE also .
  ------------------------------------------------------------------*/
  pri_wmk_buf_ptr->pri_flow_disable_timer_running = FALSE;
  ...
  /*-------------------------------------------------------------------- 
    disable_flow_cntrl
    Value: 0: Enable offload flow control
           1: Disable offload flow control
    -------------------------------------------------------------------*/
...
/*-------------------------------------------------------------------------- 
  Start Flow Disable timer if not already running
  -------------------------------------------------------------------------*/
  if ( is_pri_wmk_buf )
  {
    DATA_IS707_MSG2(MSG_LEGACY_HIGH, 
                    "Is timer running %d, timer_val to be configured: %d",                   
                    ds707_wmk_pri_buf.pri_flow_disable_timer_running,
                    ds3g_get_flow_disable_timer());

    if(FALSE == ds707_wmk_pri_buf.pri_flow_disable_timer_running )
    {
      (void) rex_set_timer( &(ds707_wmk_pri_buf.pri_flow_disable_timer),
                               ds3g_get_flow_disable_timer());
      ds707_wmk_pri_buf.pri_flow_disable_timer_running = TRUE;
  ...
  if (is_pri_wmk_buf)
  {
    ds3g_burst_disable_flow( ds707_pkt_get_pri_phys_link_ptr(),
                             (void *)&wm_list[route],
                             DS_FLOW_PS_FLOW_TX_WM_MASK,
                            &(ds707_wmk_pri_buf.burst_params.burst_params),
                             ds707_wmk_get_tcp_bidir());
  }
  else
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(ds_cmd_ptr == NULL)
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Command pointer is NULL");
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(wmk_buf_ptr->allocated == FALSE)
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "wmk_buf_ptr->allocated is FALSE ");
    return;
  }
...
)
{
  ...
  ds707_wmk_pri_buf.tcp_bi_directional = FALSE;
  /*------------------------------------------------------------------------
     Initialize the flow_disable timer.
  ------------------------------------------------------------------------*/
  rex_def_timer_ex(&(ds707_wmk_pri_buf.pri_flow_disable_timer),
                    ds707_pri_wmk_flow_disable_timer_cb,
                    (unsigned long)&ds707_wmk_pri_buf );

  ds707_wmk_pri_buf.pri_flow_disable_timer_running = FALSE;

#ifdef FEATURE_DATA_FLOW_MGMT  
  rex_def_timer_ex(&(ds707_wmk_pri_buf.burst_params.burst_periodic_timer),
                     ds707_burst_cntrl_periodic_timer_cb,
                     NULL );

  rex_def_timer_ex(&(ds707_wmk_pri_buf.burst_params.burst_params.burst_prohibit_timer),
                     ds707_burst_cntrl_prohibit_timer_cb,
                     NULL );

  ds707_wmk_pri_buf.burst_params.burst_periodic_timer_running = FALSE;
  ds707_wmk_pri_buf.burst_params.burst_params.phys_link_ptr = 
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (wmk_buf_ptr==NULL)
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "No free sec wmk to allocate");
    return NULL;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (wm_buf_ptr == NULL)
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Did not find wm entry in wmk buffer "
...
)
{
  ...
  else
  {
    ...
    ds707_wmk_pri_buf.um_SN_rev_wmk_list[0].hi_watermark =
                      ds707_wmk_value.ds707_wmk_pri_dont_exceed_cnt;
    ds707_wmk_pri_buf.tcp_bi_directional = TRUE;
  }
}
...
/*===========================================================================
FUNCTION      DS707_WMK_GET_PRI_WM_PTR

DESCRIPTION   Returns a pointer to the primary watermark

DEPENDENCIES  Return WM pointer for route A

RETURN VALUE  pointer to the primary watermark 

SIDE EFFECTS  None.
===========================================================================*/
dsm_watermark_type* ds707_wmk_get_pri_wm_ptr
( 
  void 
)
{

  return (&ds707_wmk_pri_buf.um_SN_rev_wmk_list[0]);

}/* ds707_wmk_get_pri_wm */

/*===========================================================================
FUNCTION      DS707_WMK_IS_TCP_BIDIR

DESCRIPTION   Returns whether there is tcp bidirectional or not
...
SIDE EFFECTS  None.
===========================================================================*/
boolean ds707_wmk_get_tcp_bidir
(
  void 
)
{
  return ds707_wmk_pri_buf.tcp_bi_directional;
}

#ifdef FEATURE_DATA_FLOW_MGMT
/*===========================================================================
FUNCTION      DS707_WMK_GET_BURST_PARAMS_PTR

DESCRIPTION   Returns a pointer to the burst based params for primary WM

DEPENDENCIES  None

RETURN VALUE  pointer to the burst based paramters

SIDE EFFECTS  None.
===========================================================================*/
ds707_burst_flow_control_s* ds707_wmk_get_burst_params_ptr
(
  void
)
{
  return(&ds707_wmk_pri_buf.burst_params);
}
