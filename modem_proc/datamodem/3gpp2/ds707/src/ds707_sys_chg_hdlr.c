...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp2/ds707/src/ds707_sys_chg_hdlr.c#2 $
  $DateTime: 2019/10/22 02:53:17 $ 
  $Author: pwbldsvc $
  
when       who        what, where, why
--------   ---        -------------------------------------------------------
04/01/16   hr         Reset the sys chg info after all PDNs are down 
                      for eHRPD to 1x transition.      
03/09/15   sc         Send 1x acq notification to DSD while moving 
...
typedef struct
{
  ...
  rex_timer_type                 new_call_attempt_timer; /*time between two 
                                                           call attempts*/
  boolean                        is_defined; /* timer is defined or not */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if( !ds707_nv_util_is_refresh_in_progress(DS3GSUBSMGR_SUBS_ID_1) &&
                     ds_silent_redial_info.is_defined != TRUE)
  {
  ...
  switch( ds_silent_redial_info.sr_state )
  {
    ...
    default:
      /*---------------------------------------------------------------------
        Sanity Check
      ---------------------------------------------------------------------*/
      DS_3GPP2_ERROR_FATAL("Invalid Silent Redial new state value.");
  }/* end switch */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch (ds_cmd_ptr->hdr.cmd_id)
  {
    ...
    default:
      DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR, "Unknown cmd to ds707_sys_chg %d",
...
SIDE EFFECTS  None
===========================================================================*/
void ds707_sys_chg_sm_transition_state
(
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  ------------------------------------------------------------------------*/
  hybr_gw_operational = ds3g_is_lte_hybrid_stack_active(ds707_curr_subs_id());
  ...
  switch (cleanup_trigger)
  {
    case DS707_SYS_CHG_LTE_ATTACH_INIT:
    {
      ...
      ---------------------------------------------------------------*/
      if (!ds3g_msh_cleanup_nonepc_call(ds707_curr_subs_id()))
      {
        DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, 
      ...
      memset((void*)&event_info, 0, sizeof(ds707_cback_event_info_type));
...
void ds707_sys_chg_cache_get_pdn_conn_ind_cmd(ds_cmd_type *cmd_ptr)
{
  ...
  if (NULL == cmd_ptr)
  {
    DS_3GPP2_ERROR_FATAL("ds707_sys_chg: get_pdn_conn_ind_cmd is NULL. ASSERT");
    return;
  }
  ...
  if (NULL == ds707_get_pdn_conn_ind_cmd_ptr)
  {
    ...
    {
      DS_3GPP2_ERROR_FATAL("ds707_sys_chg: can't get cmd buf from DS task. ASSERT");
      return;
    }
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (NULL == ds707_get_pdn_conn_ind_cmd_ptr)
  {
    DS_3GPP2_ERROR_FATAL("ds707_sys_chg: ds707_get_pdn_conn_ind_cmd_ptr is NULL when attempting to replay. DS3GPP2_ASSERT");
    return;
  }
  ...
  if( (cmd_ptr = ds_get_cmd_buf()) == NULL )
  {
    /*------------------------------------------------------------------
      This condition can not be recovered
    -------------------------------------------------------------------*/
    DS_3GPP2_ERROR_FATAL("ds707_sys_chg: can't get cmd buf from DS task");
    return;
  }
