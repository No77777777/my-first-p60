...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp2/ds707/src/ds707_sdb.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $

when        who    what, where, why
--------    ---    ------------------------------------------------------- 
04/01/16    sc     Fix the memory leak when SDB queue cannot accept any
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
    ...
    {
       DS_IS707_ERROR_LOG_2(MSG_LEGACY_ERROR, "invalid event:%d or ptr:%x",
                       event, dbm_info_ptr);
       return;
    }
                                  ...
                                  )
{
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "ds707_sdb_copy_dsm_item input error: "
...
boolean ds707_sdb_process_txq(const ds707_flow_type *flow_ptr)
{
  ...
  if (route == DSRSP_NUM_ROUTES) 
  {
    ...
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "No packets to transmit on SDB/DoS");
    return ret_val;
  }
  ...
  {
    DS_IS707_ERROR_LOG_3(MSG_LEGACY_ERROR, "WM:0x%x has %d packets for route:%d - "
  ...
  if((item_ptr = (dsm_item_type *)q_check(tx_wm_ptr->q_ptr)) == NULL)
  {
    ...
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "No items in Transmit Queue");
    return ret_val;
  }
  ...
  DS3GPP2_ASSERT(meta_size == sizeof(ds1x_meta_info_type));
  ...
  if(item_ptr != NULL)
  {
    ...
    switch(radio_transport)
    {
      case RADIO_TRANSPORT_SDB:
        ...
        DS3GPP2_ASSERT(meta_size == sizeof(ds1x_meta_info_type));
        DS3GPP2_ASSERT(item_ptr != NULL); //We come here only if its not NULL.
...
)
{
  ...
  for(i=0; i<(DS707_NUM_FWD_PS_FLOWS + DS707_NUM_REV_PS_FLOWS);i++)
  {
    ...
    {
      DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR,"PS flow reservation is not active" );
      continue;
    }
    ...
    {
      DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR,"Null Physical Link Pointer" );
      return can_do_sdb;
    }
...
)
{
          ...
          )  
  {
      ...
      if (ds707_sec_pkt_is_qos_available_on_system () != TRUE)
      {
        DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "QoS unavailable");
        return FALSE;
      }      
      ...
      else
      {
        ...
        if (ds707_sec_pkt_rlp_id ( flow_ptr, DSRLP_VER_HDR_MULTIFLOW_REVERSE, 
                                 &rlpid) == FALSE )
        {
          DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Cannot get RLP ID from flow ptr");
          return FALSE;
        }
        if (hdrmrlpcif_get_dos_allowed_attrib(rlpid, &dos_allowed) != E_SUCCESS)
        {
        ...
        if(ds_flow_ptr == NULL)
        {
          DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR, "Null ds_flow for ps_flow %x, ver", flow_ptr);
          return FALSE;
        }
        ...
        {
          DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "RLP is inited. Cant send DoS");
          return FALSE;
        }
...
{
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "CM could not transmit SDB data");
...
)
{
  ...
  /*-------------------------------------------------------------------------
     ...
     )
   {
     ...
     {
       DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "ds707_sdb_clone_dsm_item fails!");
       return;
     }
     /*-------------------------------------------------------------------------
   ...
   else
   {
      ...
      */
     DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "No support for DoS or SDB");
     return;
   }
...
#endif /* FEATURE_QCHAT */
  {
    ...
    {
      DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR, "unknown SDB flags %D. Assume ACH", 
   ...
   if(data_block == NULL)
   {
     DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR, "MAX OUTSTANDING SDB PKTS %d "
...
#else
   if (cm_dbm_cmd_mo_msg3(CAI_SHORT_DATA_BURST,
                          ds707_sdb_cm_dbm_cb,
                          (void *)data_block,
                          ds3g_cm_client_id,
                          &dbm_cmd_info,
   ...
   {
     DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "cm_dbm_cmd_mo_msg2 returned FALSE. "
                     "Not sending DoS / SDB");
   }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_IS707_ERROR_LOG_2(MSG_LEGACY_ERROR, "invalid inputs: buf:%x, buf_len:%d",
                    buf, buf_len);
    return;
  }
  ...
  if ( ds707_pkt_is_dormant (phys_link_ptr) != TRUE )
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Rxed SDB in Non-Dormant state");
  }
  if (ds707_pkt_get_prev_call_so() != CAI_SO_PPP_PKT_DATA_3G)
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Rxed SDB in Non-SO 33 call");
  }
  ...
  {
    DS_IS707_ERROR_LOG_2(MSG_LEGACY_ERROR, "BS:%d & MS:%d SR_ID mismatch",
  ...
  if ((sdb_ctl_byte & DS707_SDB_SO_OMIT_MASK) != DS707_SDB_SO_OMIT_MASK)
  {
    ...
    {
      DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR, "rx'ed SDB for SO %x:Not Supported",
                      sdb_so);
    }
  }
  ...
  -------------------------------------------------------------------------*/
      ...
      {
        DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR, "Invalid size of buf_len:%d", 
                        buf_len);
        return;
      }
  }
...
)
{
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "invalid inputs");
    return;
  }
  ...
  if ( ds707_pkt_is_dormant (phys_link_ptr) != TRUE )
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Rxed DOS in Non-Dormant state");
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "NULL dsm_item_ref_ptr/buf/len_ptr");
    return (-1);
  }
