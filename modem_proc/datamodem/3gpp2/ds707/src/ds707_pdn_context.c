...
$Header: //components/rel/data.mpss/3.5..2.1/3gpp2/ds707/src/ds707_pdn_context.c#3 $
$Author: pwbldsvc $
$DateTime: 2019/12/12 00:03:59 $
                     
when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
04/01/16   sc      Added condition to check the throttle count for the iface along
                   with the iface ptr in configure_vsncp before assert
06/30/15   sd      Block both Ifaces if APN is rejected by the network.
02/23/15   sd      Fix to avoid PPP resync for VSNCP cause code 9 or 11 if the
...
)
{
  ...
  {
    DS_3GPP2_ERROR_FATAL("PS Iface ptr is invalid");
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_3GPP2_ERROR_FATAL("tear_down_cmd_info ptr is invalid");
  }
  ...
  {
    DS_3GPP2_ERROR_FATAL("707 PDN context cb ptr is invalid");
  }
...
)
{
  ...
  if(apn_dest == NULL)
  {
    DS_3GPP2_ERROR_FATAL("APN dest ptr is NULL");
    return 0;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  else
  {
    ...
    {
      DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "memory allocation failed");
      return NULL;
  }
...
) 
{
  ...
  ds_profile_subs_id_enum_type profile_subs_id = ACTIVE_SUBSCRIPTION_1; 
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */

  profile_subs_id = ds707_data_session_get_profile_subs_id(ds707_curr_subs_id());
...
/*-----------------------------------------------------------------------*/
 throttle_info_ptr = 
  ...
  if ( !DS707_THROTTLE_INFO_PTR_IS_VALID(throttle_info_ptr) )
  {
    DS_3GPP2_ERROR_FATAL("invalid throttle_info_ptr.return default deny reason");
    return deny_reason;
  }
  ...
  {
    DS_3GPP2_ERROR_FATAL("invalid iface type; return default deny reason");
    return deny_reason;
  }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if (!DS707_PDNCNTX_CB_IS_VALID(pdncntx_cb_ptr))
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Invalid pdn_cntx_ptr");
    return NULL;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -  */
  ...
  {
    DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR, "Invalid Iface table index %d",
                    iface_tbl_index);
    return NULL;
  }
}
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if( cmd_ptr == NULL )
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Invalid ds cmd ptr!");
    return;
  }
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Invalid command, Ignoring!");
    return;
  }
  ...
  {
    DS_3GPP2_ERROR_FATAL("Bad parameters passed to ds707_pdn_inactivity_timer_process_cmd");
    return;   
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if( !DS707_PDNCNTX_CB_IS_VALID( pdn_context_cb ) )
  {
    DS_3GPP2_ERROR_FATAL("Bad parameters passed to ds707_pdncntx_mpit_timer_process_cmd");
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if( !DS707_PDNCNTX_CB_IS_VALID( pdn_context_cb ) )
  {
    DS_3GPP2_ERROR_FATAL("Bad parameters passed to ds707_pdncntx_inactivity_timer_start");
  }
  ...
  */
                                                ...
                                                &data_sess_profile_info_ptr))
  {
    if(FALSE == ds707_pdncntx_get_requesting_profile(pdn_context_cb,
                                                     &data_sess_profile_info_ptr))
    {
      DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Invalid profile id in PDN context");
      return;
    }
  }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  if(NULL == cmd_ptr)
  {
    DS_3GPP2_ERROR_FATAL("cmd_ptr is NULL");
    return;
  }
  ...
  switch(cmd_ptr->hdr.cmd_id)
  {
    ...
    case DS_CMD_707_IFACE_OUT_USE:
      ...
      {
        DS_3GPP2_ERROR_FATAL("Payload is invalid");
        return;
      }
...
)
{
  ps_iface_type *iface_ptr = NULL;
  uint8                          num_events = 0;
  ps_iface_event_enum_type       events[2];
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  /*---------------------------------------------------------------------
   register for IFACE_UP, IFACE_ROUTEABLE_EV events.
  ---------------------------------------------------------------------*/
  ds707_iface_tbl_ptr->iface_event_buf_ptr = 
                                    (void *)ps_iface_alloc_event_handle(
                                            iface_ptr,
                                            ds707_pdncntx_iface_event_cb,
                                            NULL);
  
  if (ds707_iface_tbl_ptr->iface_event_buf_ptr == NULL) 
  {
    DS_3GPP2_ERROR_FATAL("No PS IFACE Event Buffers Avail");
  }
  num_events = 2; 
  events[0] = IFACE_UP_EV;
  events[1] = IFACE_ROUTEABLE_EV;
  ...
  if( -1 == ps_iface_subscribe_event_list(
                               ...
                               num_events))
  {
    DS_3GPP2_ERROR_FATAL("IFACE_UP_EV/ IFACE_ROUTABLE_EV reg failed");
    return;
  }
}
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  {
    DS_3GPP2_ERROR_FATAL("ncp_started flag is TRUE");
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if (NULL == ncp_config_ptr)
  {
    DS_3GPP2_ERROR_FATAL("ncp_config_ptr is NULL");
    return;
  }
...
#endif
  boolean  is_v4_throttled = FALSE;
  boolean  is_v6_throttled = FALSE;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ( ( !DS707_PDNCNTX_CB_IS_VALID( pdn_context_cb ) ) ||
       ...
       ( ncp_config_ptr == NULL ) )
  {
    DS_3GPP2_ERROR_FATAL("Bad parameters passed to ds707_pdncntx_configure_vsncp");
  }
  ...
  if( PS_IFACE_IS_VALID(v4_iface_ptr) )
  {
    ...
    DS3GPP2_ASSERT(dssnet4_sm_ptr != NULL);
  }
  ...
  if( PS_IFACE_IS_VALID(v6_iface_ptr) )
  {
    ...
    DS3GPP2_ASSERT(dssnet6_sm_ptr != NULL);
  }
#endif /* FEATURE_DATA_PS_IPV6 */
  ...
  if( data_sess_profile_info_ptr->pdn_type == DS707_DATA_SESS_PDN_TYPE_V4_V6 )  
  {
    if( PS_IFACE_IS_VALID(v4_iface_ptr) )
    {
      is_v4_throttled = ds707_pdncntx_is_iface_throttled(
                             data_sess_profile_info_ptr,
                             ps_iface_get_addr_family( v4_iface_ptr ));
    }

    if( PS_IFACE_IS_VALID(v6_iface_ptr) )
    {
      is_v6_throttled = ds707_pdncntx_is_iface_throttled(
                             data_sess_profile_info_ptr,
                             ps_iface_get_addr_family( v6_iface_ptr ));
    }
  }
  ...
  { 
    ...
    {
            /* If any of the iface is throttled then skip it in the request IP type */
      if( (ds3g_is_ipv6_enabled ()) && 
          ( (PS_IFACE_IS_VALID(v4_iface_ptr) == FALSE) ||
            (is_v4_throttled == TRUE) ) 
        )
      {
        ncp_config_ptr->option_vals.req_addr_type = VSNCP_3GPP2_PDN_TYPE_IPV6;
      }
      else if( (is_v6_throttled == TRUE) ||
               (PS_IFACE_IS_VALID(v6_iface_ptr) == FALSE) )
      {
        ncp_config_ptr->option_vals.req_addr_type = VSNCP_3GPP2_PDN_TYPE_IPV4;
      }
      else
      ...
      {
        DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR,"IPV6 NV is disabled, setting VSNCP config "
                                          "to V4 only");
    ...
    else
    {
      ...
      DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR,"Invalid PDN type specified in profile: %d", 
                       data_sess_profile_info_ptr->pdn_type);
      DS3GPP2_ASSERT(0);
    }
    ...
    if ( pdn_context_cb->handover_attach == TRUE )
    {
      ...
      if (handover_ip_config_handle != NULL)
      {
        ...
        {
            DS_3GPP2_ERROR_FATAL("Neither V4 nor V6 IP containers in handover_ip_config_handle are valid");
        }
      ...
      {
        DS_3GPP2_ERROR_FATAL("Configuring VSNCP for Handover attach, with IPv4 addr: 0.0.0.0 ");
      }  
           ...
           DATA_SESS_AUTH_PROTOCOL_PAP_CHAP ) 
    {
      ...
      if(DS3G_PDN_AUTH_CHAP == ds3g_get_config_pdn_level_auth(ds707_curr_subs_id()))
      {
         pdn_level_auth_protocol = DATA_SESS_AUTH_PROTOCOL_CHAP;
      }
      else if(DS3G_PDN_AUTH_PAP == ds3g_get_config_pdn_level_auth(ds707_curr_subs_id()))
      {
         ...
         DATA_SESS_AUTH_PROTOCOL_CHAP == pdn_level_auth_protocol )
    {
      ...
      {
        DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Invalid PDN level auth user id, "
                         "Call failed!" );
        return FALSE;
      }
      ...
      {
        DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Invalid PDN level auth password, "
                         "Call failed!" );
        return FALSE;
      }
      ...
      if ( DATA_SESS_AUTH_PROTOCOL_CHAP == pdn_level_auth_protocol ) 
      {
        ...
        {
          DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Invalid PDN level auth password for CHAP, "
                           "Call failed!" );
          return FALSE;
        }
      }
      ...
      { /* PDN Level Auth Protocol is PAP, password can be NULL for PAP */
        ...
        {
          DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Using a NULL password for PDN level"
                           " PAP authentication" );
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR,"iface 0x%x is inactive", iface_ptr);
    return(0);
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  else
  {
    ...
    memset(&(tear_down_cmd_info_ptr->user_data.tear_down_info),
            0,sizeof(ps_iface_tear_down_client_data_info_type));
    ...
    if(!local_abort &&
      (ds3gsubsmgr_get_temp_dds_subs_id() != SYS_MODEM_AS_ID_NONE) &&
      (ds3gsubsmgr_get_temp_dds_subs_id() != ds707_curr_subs_id()))
    {
      local_abort = TRUE;
    }
    if (local_abort)
...
/*- - - - - - - - - - - - - - - - - -  - - - - - - - - - - - - - - - - - - - */
  ...
  if ( (!PS_IFACE_IS_VALID(iface_ptr)) ||
       (argval_ptr == NULL) )
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR,"invalid IFACE ptr or argval ptr" );
    return -1;
  }
...
/*===========================================================================
FUNCTION DS707_PDNCNTX_SET_IPv6_DNS_ADDR

DESCRIPTION  the function is to set ipv6 dns address to trat iface

RETURN VALUE  0  : if the ipv6 dns addr is set successfully
              -1 : if the ipv6 dns addr does not get processed

DEPENDENCIES  

SIDE EFFECTS  
===========================================================================*/
int ds707_pdncntx_set_ipv6_dns_addr
(
  ps_iface_type 		*this_iface_ptr,
...
)
{
  ...
  {
    memscpy(&this_iface_ptr->v6_net_info.primary_dns,
            sizeof(struct ps_in6_addr),
            &prim_dns_ptr->addr.v6, 
            sizeof(struct ps_in6_addr));
  }
  else
  {
  ...
  if(sec_dns_ptr != NULL)
  {
    memscpy(&this_iface_ptr->v6_net_info.secondary_dns,
            sizeof(struct ps_in6_addr),
            &sec_dns_ptr->addr.v6, 
            sizeof(struct ps_in6_addr));
...
func_exit:
  ...
  {
    DS_3GPP_MSG1_ERROR("ds707_pdncntx_set_ipv6_dns_addr: err %d",
                       local_err);
  }
...
/*===========================================================================
FUNCTION DS707_PDNCNTX_SET_IP_CONFIG

DESCRIPTION  the function is to set some srat ip info to trat iface

RETURN VALUE  0  : if the IP config is processed successfully
              -1 : if the IP config does not get processed

DEPENDENCIES  

SIDE EFFECTS  
===========================================================================*/
int ds707_pdncntx_set_ip_config
(
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  -----------------------------------------------------------------------*/
  if ((config_mask & ACL_HANDOFF_CONFIG_DNS_PCSCF) &&
      (iface_ip_config_ptr->valid_sip_serv_addr_count > 0))
  {    
  ...
  if (iface_ip_config_ptr->ip_addr.type == IFACE_IPV4_ADDR_FAMILY)
  {
	...
	if(config_mask&ACL_HANDOFF_CONFIG_V4_ADDR)
	{
	  /*---------------------------------------------------------------------
  ...
  else if (iface_ip_config_ptr->ip_addr.type == IFACE_IPV6_ADDR_FAMILY)
  {
    ...
    if(config_mask&ACL_HANDOFF_CONFIG_DNS_PCSCF)
    {
      if (-1 == ds707_pdncntx_set_ipv6_dns_addr(ps_iface_ptr, 
	...
	if(config_mask&ACL_HANDOFF_CONFIG_V6_PRI_IID)
	{
	...
	/*Save the v6 prefix info in DS iface*/

    
    if(config_mask&ACL_HANDOFF_CONFIG_V6_PREFIX)
    {
	  iface6_tbl_ptr = ds707_pdncntx_get_iface_tbl_entry_from_iface(ps_iface_ptr);
	  if (iface6_tbl_ptr != NULL)
      {
		iface6_tbl_ptr->v6_prefix_info.is_valid = 1;
		iface6_tbl_ptr->v6_prefix_info.v6_gateway_iid = iface_ip_config_ptr->v6_gateway_iid;
		iface6_tbl_ptr->v6_prefix_info.ip_addr.addr.v6.ps_s6_addr64[0] =
		  iface_ip_config_ptr->ip_addr.addr.v6.ps_s6_addr64[0];
		iface6_tbl_ptr->v6_prefix_info.prefix_valid_life_time = 
		  iface_ip_config_ptr->prefix_valid_life_time;
		iface6_tbl_ptr->v6_prefix_info.prefix_pref_life_time = 
		  iface_ip_config_ptr->prefix_pref_life_time;
	    iface6_tbl_ptr->v6_prefix_info.router_life_time = 
		  iface_ip_config_ptr->router_life_time;

        DATA_IS707_MSG0( MSG_LEGACY_HIGH,
                         "Setting v6 prefix info in 3GPP2 Iface");
	  }
    }
...
func_exit:
  ...
  if(local_err != DS_3GPPI_LOCAL_ERR_VAL_NONE)
  {
    DS_3GPP_MSG1_ERROR("ds707_pdncntx_set_ip_config: err %d",
                       local_err);
  }
...
)
{
  ds707_pdn_context_cb_type *pdncntx_cb_ptr = NULL;
  acl_handoff_ctxt_type     *handoff_cntx_ptr = NULL;
  ps_iface_ip_config_type   *iface_ip_config_ptr = NULL;
  ps_iface_type             *v4_iface_ptr = NULL;
...
#endif /* FEATURE_DATA_OPTHO */
  uint8                            config_mask  = 0;
  acl_handoff_context_config_type *handoff_cntx_config_ptr = NULL;
...
/*- - - - - - - - - - - - - - - - - -  - - - - - - - - - - - - - - - - - - - */
  ...
  handoff_cntx_config_ptr = (acl_handoff_context_config_type*)argval_ptr;

  config_mask = handoff_cntx_config_ptr->config_mask;

  handoff_cntx_ptr = &handoff_cntx_config_ptr->handoff_ctxt;
     ...
     )
  {
    ...
    if (0 == ds707_pdncntx_set_ip_config
                     (v4_iface_ptr, 
                      &(handoff_cntx_ptr->pdn_ip_config.iface_ipv4_config),
                      config_mask))
    {
     ...
     )
  {
	...
	{
	  iface_ip_config_ptr = &(handoff_cntx_ptr->pdn_ip_config.iface_ipv6_config);
	}
	...
	DATA_IS707_MSG3( MSG_LEGACY_HIGH,
                     "Updating IP cfg for V6 Iface:0x%x:%d succeeded? %d",
                     v6_iface_ptr->name,v6_iface_ptr->instance,
                     is_apply_v6_success );
  ...
  ****************************************************************************/
  ...
  {
    if (((NULL == v4_iface_ptr) || 
         (FALSE == handoff_cntx_ptr->pdn_ip_config.iface_ipv4_config.is_ip_cfg_valid) ||
         is_apply_v4_success || v4_in_ppp_resync) &&
        ((NULL == v6_iface_ptr) || 
         (FALSE == handoff_cntx_ptr->pdn_ip_config.iface_ipv6_config.is_ip_cfg_valid) ||
         is_apply_v6_success || v6_in_ppp_resync)
        )
    {
      DATA_IS707_MSG2(MSG_LEGACY_HIGH,
                      "Apply IP cfg succeeded/iface in PPP resync for the "
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "ppp_status_ptr is NULL");
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(!(DS707_PDNCNTX_CB_IS_VALID(pdncntx_cb_ptr)))
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "pdncntx_cb_ptr is invalid");
    return;
  }

  ds707_pkt_mgr_handle_otasn_pdn_down(pdncntx_cb_ptr, ds707_curr_subs_id());
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if (!DS707_PDNCNTX_CB_IS_VALID(pdncntx_cb_ptr))
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "PDN context cb ptr is invalid");
...
DEPENDENCIES
  This function is to be called within PS global crit section
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (!DS707_PDNCNTX_CB_IS_VALID(pdncntx_cb_ptr))
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR,"707 PDN context cb ptr is invalid");
    /*----------------------------------------------------------------------- 
  ...
  if (PS_IFACE_IS_VALID(iface4_ptr))
  {
    ...
    if (NULL == dssnet4_instance)
    {
      DS_3GPP2_ERROR_FATAL("Invalid dssnet4 instance");
      return;
    }
  ...
  if (PS_IFACE_IS_VALID(iface6_ptr))
  {
    ...
    if (NULL == dssnet6_instance)
    {
      DS_3GPP2_ERROR_FATAL("Invalid dssnet6 instance");
      return;
    }
    dssnet6_call_end_reason 
                      = dssnet6_get_instance_call_end_reason(dssnet6_instance);
  ...
  {
    /*---------------------------------------------------------------------
        de -register with Data path manager.
    ---------------------------------------------------------------------*/
    if(is_iface_crit_sect)
    {
      dssnet6_leave_crit_sect();
    }
    PS_LEAVE_CRIT_SECTION(&global_ps_crit_section);
    ds707_pkt_mgr_util_dpm_dereg( iface4_ptr);
    PS_ENTER_CRIT_SECTION(&global_ps_crit_section);
    if(is_iface_crit_sect)
    {
      dssnet6_enter_crit_sect();
    }
    DATA_IS707_MSG2_EX(MSG_LEGACY_HIGH, 
                       subs_id,
  ...
  if (TRUE == post_v6_iface_down)
  {
    ...
    PS_LEAVE_CRIT_SECTION(&global_ps_crit_section);
    ds707_pkt_mgr_util_dpm_dereg( iface6_ptr);
    PS_ENTER_CRIT_SECTION(&global_ps_crit_section);
    if(is_iface_crit_sect)
    {
      dssnet6_enter_crit_sect();
  ...
  {
    /*-----------------------------------------------------------------------
       Notify call end to DS3G event manager.
    -----------------------------------------------------------------------*/
    ds707_pkt_mgr_notify_call_end_event_to_ds3g(iface_ptr,(void*)pdncntx_cb_ptr,subs_id);
    ...
    /*-----------------------------------------------------------------------
       Set pdn_cntx_ptr to NULL
    -----------------------------------------------------------------------*/
    if((ds707_pdncntx_get_v4_iface(pdncntx_cb_ptr) != NULL) &&
        (DS707_IFACE_TBL_IS_VALID(iface4_tbl_ptr)) )
    {
      DATA_IS707_MSG1(MSG_LEGACY_HIGH, 
                      "Setting pdn cntx ptr to NULL for iface tbl 0x%x",
                      iface4_tbl_ptr);
    ...
    /*-----------------------------------------------------------------------
      Set pdn_cntx_ptr to NULL
    -----------------------------------------------------------------------*/
    if ((ds707_pdncntx_get_v6_iface(pdncntx_cb_ptr) != NULL) &&
         (DS707_IFACE_TBL_IS_VALID(iface6_tbl_ptr)) )
    {
      DATA_IS707_MSG1(MSG_LEGACY_HIGH, 
                      "Setting pdn cntx ptr to NULL for iface tbl 0x%x",
                      iface6_tbl_ptr);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  subs_id = ds3gsubsmgr_subs_id_ds_to_cm(PS_IFACE_GET_SUBS_ID(this_iface_ptr));
  	
  ds707_pkt_mgr_handle_otasn_pdn_down(pdncntx_cb_ptr, subs_id);
  ...
  if (last_iface_down == TRUE)
  {
    /*-----------------------------------------------------------------------
       Notify call end to DS3G event manager.
    -----------------------------------------------------------------------*/
    ds707_pkt_mgr_notify_call_end_event_to_ds3g(this_iface_ptr,(void*)pdncntx_cb_ptr,subs_id);
  ...
  {
    DS_3GPP2_ERROR_FATAL("Cannot update the requesting profile id");
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if (ps_errno == NULL) 
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Invalid ps_errno ptr, fail ioctl");
    return -1;
  }
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Heap Memory Allocation failed for PDN throttle info");
    *ps_errno = DS_EFAULT;
    return -1;
  }
...
)
{
 ...
 if ( ehrpd_pdn_ctxt_throttle_ev_info_ptr == NULL )
  ...
  rval = ps_sys_event_ind_ex( PS_SYS_TECH_3GPP2,
...
)
{
  if (!DS707_PDNCNTX_CB_IS_VALID(pdncntx_cb_ptr))
  {
    DS_3GPP2_ERROR_FATAL("PDN context cb ptr is invalid");
  }
...
)
{
  ...
  if (!PS_IFACE_IS_VALID(iface_ptr)) 
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "ds707_pdncntx_iface_reconfig:"
                                       "Invalid IFACE ptr");
    return -1;
  }
...
)
{
  ...
  DATA_IS707_C_MSG1(DS707_F3_MASK, "PDN CB get orig_sys = %d", 
                  pdncntx_cb_ptr->orig_sys);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_3GPP2_ERROR_FATAL("call_end_reason is NULL");
    return FALSE;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (ds707_pdncntx_default_apn_info_ptr == NULL)
  {
    DS_3GPP2_ERROR_FATAL("Memory allocation failed for default APN info structure");
    return;
  }
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Invalid decoded APN");
...
/*===========================================================================
FUNCTION ds707_pdncntx_is_last_iface_for_non_ims_pdn

DESCRIPTION
  This function determines whether the passed IFACE is the last IFACE
  corresponding to active non IMS PDNs.
...
RETURN VALUE 
  None.
==========================================================================*/
boolean ds707_pdncntx_is_last_iface_for_non_ims_pdn
(
  ps_iface_type           *this_iface_ptr,
...
)
{
  ...
  if ( !PS_IFACE_IS_VALID(this_iface_ptr) )
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Invalid IFACE");
    return TRUE;
  }
  ...
  for (index = 0; index < DS707_MAX_IFACES; index++) 
  {
    ...
    if ( !PS_IFACE_IS_VALID(iface_ptr) ||
         (this_iface_ptr == iface_ptr) ||
         ds3gsubsmgr_subs_id_ds_to_cm(PS_IFACE_GET_SUBS_ID(iface_ptr)) != subs_id ||
         !PS_IFACE_IS_IN_USE(iface_ptr) )
    {
      continue;
    }
    ...
    if ( DS707_PDNCNTX_CB_IS_VALID(pdncntx_cb_ptr) &&
        !ds707_pdncntx_is_ims_pdn(pdncntx_cb_ptr, subs_id) )
    {
      ret_val = FALSE;
      break;
    }
  ...
  return ret_val;
}/*ds707_is_last_iface_for_non_ims_pdn*/

/*===========================================================================
FUNCTION ds707_pdncntx_is_ims_pdn

DESCRIPTION
  This function determines whether the passed PDN is an IMS PDN.
...
RETURN VALUE 
  ...
  FALSE - otherwise.
==========================================================================*/
boolean ds707_pdncntx_is_ims_pdn
(
  ds707_pdn_context_cb_type       *pdncntx_cb_ptr,
  sys_modem_as_id_e_type           subs_id
)
{
  ps_iface_type             *v4_iface_p;
  ps_iface_type             *v6_iface_p;
  ps_sys_apn_info_type       apn_info;
  char                      *apn_name = NULL;
  ds707_data_session_profile_info_type *data_sess_profile_info_ptr = NULL;
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  v4_iface_p = ds707_pdncntx_get_v4_iface(pdncntx_cb_ptr);
  v6_iface_p = ds707_pdncntx_get_v6_iface(pdncntx_cb_ptr);

  if ( PS_IFACE_APN_TYPE_IMS == PS_IFACE_GET_APN_TYPE (v4_iface_p) ||
       PS_IFACE_APN_TYPE_IMS == PS_IFACE_GET_APN_TYPE (v6_iface_p) )
  {
    return TRUE;
  }
  ...
  if (DS707_SYS_INFO_EPC_SYSTEM == ds707_pdncntx_get_orig_sys(pdncntx_cb_ptr))
  {
    ...
    if (ds3gdevmgr_get_apn_settings(&apn_info, subs_id))
    {
      if ((apn_info.curr_apn_info[PS_SYS_APN_TYPE_IMS].is_configured == TRUE) && 
          (strncasecmp((const char *)apn_info.
                      curr_apn_info[PS_SYS_APN_TYPE_IMS].apn_name,
                      (const char *)apn_name,
