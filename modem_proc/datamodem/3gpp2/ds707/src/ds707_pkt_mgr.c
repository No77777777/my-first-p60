...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp2/ds707/src/ds707_pkt_mgr.c#5 $ 
  $DateTime: 2023/01/15 20:34:36 $ $Author: pwbldsvc $

when        who    what, where, why
--------    ---    ----------------------------------------------------------
04/01/16    sc     Added code to reset DS707_SYS_CHG_SM state if there are no
                   active calls during a HDR session change
10/08/15    sd     Fixed bearer tech update for HDR if session changes and old
                   session is closed in between.
10/01/15    sd     Changing default value for HRPD IPv6 enabled NV to TRUE.
09/14/15    sd     Changes to have a threshold limit on the number of reverse
                   link ACKs to trigger BP removal.
09/11/15    sd     Fix to enable/disable flow control based on DSM memory 
                   events for data call on 1x.
07/22/15    sd     Fix to avoid allocating a new timer for NV refresh if the 
                   old timer handle is valid.
07/14/15    sd     Fix to ensure PDN compatibilty check for eHRPD profiles in 
                   eHRPD-OOS-eHRPD scenario.
03/22/15    hr     Repoprt configured throughput info to framework when
                   preferred RAT changed to 3GPP2
12/17/14    sc     Prevent traffic channel bringup if retry delay timer is running
12/15/14    sc     Added code to perform graceful PPP termination during hard
                   failure and then update eHRPD availability  
11/17/14    skc    Removing FEATURE_UW_FMC
11/10/14    sd     Data 3GPP2 Async code removal.
...
#include "ds707_burst_flow_control.h"
...
#define DS707_BACK_PRESSURE_ACK_THRESHOLD 10
...
/*-------------------------------------------------------------------------
  Global variable to store the current 1x subs id and also the type of 
  refresh if initiated.
-------------------------------------------------------------------------*/
static ds707_refresh_info_type ds707_refresh_info = {SYS_MODEM_AS_ID_1, 0};

/*-------------------------------------------------------------------------
  Global variable to store the current 1x subs id and also the type of 
  refresh if initiated.
-------------------------------------------------------------------------*/
static boolean ds707_subscription_info[DS3GSUBSMGR_SUBS_ID_MAX];
...
static boolean ds707_pkt_mgr_is_elevator_exit_supported = FALSE;
...
---------------------------------------------------------------------------*/
static boolean ds707_is_auth_internal = TRUE;	
...
/nv/item_files/data/3gpp2/elevator_exit_supported\n\
/nv/item_files/data/3gpp2/dscam_reduced_dormancy_timer\n";

/*---------------------------------------------------------------------------
  Global variables to store the last preferred RAT information in 3GPP2 mode.
---------------------------------------------------------------------------*/
static uint32 last_pref_rat_mask = PS_IFACE_CDMA_NULL_BEARER;
static uint32 last_pref_so_mask  = 0; 
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  DS3GPP2_ASSERT( pri_flow_ptr != NULL);
...
{
  DATA_IS707_MSG1( MSG_LEGACY_LOW, "ds707_is_term_in_progress: %d",
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  */   
  ...
  {        
    ...
    DS3GPP2_ASSERT ( curr_iface_ptr == curr_flow_ptr->ps_iface_ptr );
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if (PS_PHYS_LINK_GET_STATE(phys_link_ptr) == PHYS_LINK_UP)
  {
    ...
    {
      DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR, "Cant tear down phys link - error %d",
                      err_num);
    }
  }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "No dormant call needing re-orig");
  }
  else
  {
#if defined(FEATURE_DATA_IS2000_SDB) || defined(FEATURE_HDR_DOS)
    ...
    else
    {
      ...
      if ((ret_val < 0) && (err_num != DS_EWOULDBLOCK))
      {
        DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR, "PS_IFACE reorig failed %d",
                        err_num);
        for (p = 0; p < DSRSP_NUM_ROUTES; ++p)
        {
          dsm_empty_queue(&ds_flow_ptr_arr[j]->tx_wm_list[p]);
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  {      
    ...
    if(iface_ptr != NULL)
    {
      ...
      {
        DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Couldn't get sec flows");
      }
    #endif
...
)
{
      ...
      )
  {
    ...
    for (i = 1; i < DS707_NUM_PHYS_LINKS; ++i)
    {
      ...
      DS3GPP2_ASSERT (sec_phys_link_ptr != NULL);
...
)
{
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR,"Back pressure data ptr is NULL");
    return;
  }
  else
  {
    new_acks = back_pressure_data_p->tcp_rev_link_ack_counter - 
                 back_pressure_data_p->last_tcp_rev_link_ack_count;
    if ( new_acks <= DS707_BACK_PRESSURE_ACK_THRESHOLD )
    {
      ...
      if ( back_pressure_data_p->tcp_rev_link_ack_counter > 
      	                            DS707_BACK_PRESSURE_ACK_THRESHOLD )
      {
        DATA_IS707_MSG0_EX(MSG_LEGACY_HIGH, subs_id, "Enabling back pressure");
    ...
    else
    {
      ...
      {
        DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR,"Failed to start timer");
      }
    }
  }
...
/*-------------------------------------------------------------------------*/
         ...
         ds707_back_pressure_params.tmr_hndl_dyn_back_pressure )
  {
    ...
    if ( PS_TIMER_INVALID_HANDLE == 
             ds707_back_pressure_params.tmr_hndl_dyn_back_pressure )
    {
      DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Invalid PS timer handle ");
    }
  }
  ...
  switch (event)
  {
    case DS707_CBACK_PHYS_LINK_DOWN_EV:
      if (event_info_ptr->going_dormant == TRUE)
      {
        ...
        ds3g_msh_set_pkt_state(CM_PACKET_STATE_DORMANT, subs_id);
      ...
      --------------------------------------------------------------*/
        ...
        if (enable_flow_flag == TRUE)
        {
          ...
          ---------------------------------------------------------------*/
          DATA_IS707_MSG0_EX( MSG_LEGACY_HIGH,
                              subs_id,
        ...
        ds3geventmgr_notify_ps_call_event_per_sub
        ( 
          subs_id,
          DS3GEVENTMGR_PS_CALL_DORMANT_EV,
          PS_SYS_NETWORK_3GPP2
        );
      ...
      else
      {
        ...
        ds3g_msh_set_pkt_state(CM_PACKET_STATE_NULL, subs_id);
        /*------------------------------------------------------------- 
        ...
        {
          /*---------------------------------------------------------- 
            Here we are processing the PHYS_LINK_DOWN event
            In this case . Stoptimer and notify CM right away. 
          ---------------------------------------------------------*/
   
         if (TRUE == ds3g_timer_get_status_ex(
                         DS3G_DELAY_OTASN_TEARDOWN_TIMER,
                         ds3gsubsmgr_subs_id_cm_to_ds3g(subs_id)))
         {
           ds3g_timer_stop_ex(DS3G_DELAY_OTASN_TEARDOWN_TIMER,
                              ds3gsubsmgr_subs_id_cm_to_ds3g(subs_id));
         }
        ...
        {
          DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "ds707_devmgr_abort() failed");
        }
#endif /* FEATURE_EHRPD */
       ...
       /* Cleanup the last preferred RAT info */
        last_pref_rat_mask = PS_IFACE_CDMA_NULL_BEARER;
        last_pref_so_mask  = 0;          
      }
...
#ifdef FEATURE_DATA_FLOW_MGMT
      /*------------------------------------------------------------- 
        Stop burst hysteresis timer when the last bearer goes down 
      -------------------------------------------------------------*/
      ds707_burst_cntrl_stop_burst_periodic_timer();
#endif /* FEATURE_DATA_FLOW_MGMT */

      break;

    case DS707_CBACK_PHYS_LINK_UP_EV:
      DATA_IS707_MSG1_EX(MSG_LEGACY_HIGH, subs_id, "  %d.  phy link UP - "
...
#endif
      ds3g_msh_set_pkt_state(CM_PACKET_STATE_CONNECT, subs_id);
...
#ifdef FEATURE_DATA_FLOW_MGMT
      /*---------------------------------------------------------------------
        Set burst hysteresis timer when the 1st bearer comes up. This will
        take care of incrementing the unblock_timer_count for each bearer
        upon time expiry. 
      ---------------------------------------------------------------------*/
      ds707_burst_cntrl_set_burst_periodic_timer();
#endif
      ...
      {      
        ...
        if ( PS_IFACE_IS_VALID ( iface_ptr ) )
        {
                 ...
                 (ds707_rmsm_is_rm_iface_up() == TRUE) )
            {
              ...
              queue_status = mc_queue_cmd(subs_id, MC_TUNE_AWAY_CNTL_F); 
              if (queue_status == MC_FREE_QUEUE_EMPTY) 
              {
                DS_3GPP2_ERROR_FATAL( "No MC command links available");
              }
              else if (queue_status != MC_CMD_QUEUED) 
              {
                DS_3GPP2_ERROR_FATAL( "Unknown status return from mc_queue_cmd()");
              }
...
case DS707_CBACK_PHYS_LINK_RESUMING_EV:
      ...
      if(phys_link_ptr != NULL)
      {
        ...
        if( rval == 0 )
        {
          ...
          ds3gflowmgr_notify_ps_call_state_per_sub(
             subs_id, DS3GFLOWMGR_PS_CALL_END_EV);
        }
    ...
    case DS707_CBACK_HANDOFF_EPC_TO_NONEPC:
      ...
      /* If all PDNs are down then reset ds707_sys_chg_sm state to NULL */
      if(ds707_pdnctx_are_all_pdns_down() == TRUE)
      {
        DATA_IS707_MSG0(MSG_LEGACY_HIGH, "Reset sys_chg_sm to NULL state ,all PDNs down");
    ...
    case DS707_CBACK_TUN_PHYS_LINK_DOWN_EV:
    {
      ...
      else
      {
                                                   ...
                                                   subs_id) == TRUE)
        {
          ...
          /*------------------------------------------------------------ 
            this is the case where tunnel is gone, if OTASN timer 
            is running, stop it and notify CM right away
            ---------------------------------------------------------*/
         if (TRUE == ds3g_timer_get_status_ex(
                         DS3G_DELAY_OTASN_TEARDOWN_TIMER,
                         ds3gsubsmgr_subs_id_cm_to_ds3g(subs_id)))
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (ds707_cback_register(DS707_CBACK_PHYS_LINK_COMING_UP_EV,
                          ds707_pkti_event_cback) == FALSE)
  {
    DS_3GPP2_ERROR_FATAL("Could not reg PHYS_LINK_COMING_UP_EV cback");
  }

  if (ds707_cback_register(DS707_CBACK_PHYS_LINK_RESUMING_EV,
                          ds707_pkti_event_cback) == FALSE)
  {
    DS_3GPP2_ERROR_FATAL("Could not reg PHYS_LINK_RESUMING_EV cback");
  }
  ...
  {
    DS_3GPP2_ERROR_FATAL("Could not reg INC CALL cback");
  }

  if (ds707_cback_register(DS707_CBACK_ABORT_SESSION_EV,
                           ds707_pkti_event_cback) == FALSE)
  {
    DS_3GPP2_ERROR_FATAL("Could not reg ABORT SESSION cback");
  }

  if (ds707_cback_register(DS707_CBACK_HOLDDOWN_TIMER_STARTED_EV,
                           ds707_pkti_event_cback) == FALSE)
  {
    DS_3GPP2_ERROR_FATAL("Could not reg HDOWN TIMER START cback");
  }

  if (ds707_cback_register(DS707_CBACK_HOLDDOWN_TIMER_STOPPED_EV,
                           ds707_pkti_event_cback) == FALSE)
  {
    DS_3GPP2_ERROR_FATAL("Could not reg HDOWN TIMER STOP cback");
  }
  ...
  if (ds707_cback_register(DS707_CBACK_HANDOFF_EPC_TO_NONEPC,
                           ds707_pkti_event_cback) == FALSE)
  {
    DS_3GPP2_ERROR_FATAL("Could not reg DS707_CBACK_HANDOFF_EPC_TO_NONEPC ");
  }

  if (ds707_cback_register(DS707_CBACK_HANDOFF_NONEPC_TO_EPC,
                           ds707_pkti_event_cback) == FALSE)
  {
    DS_3GPP2_ERROR_FATAL("Could not reg DS707_CBACK_HANDOFF_NONEPC_TO_EPC ");
  }
  #endif /*FEATURE_EHRPD */
    ...
    */
    ...
    {      
      ...
      curr_iface_ptr = ds707_pdncntx_get_iface_ptr_from_iface_tbl_index(
                                                          curr_iface_index);
      DS3GPP2_ASSERT ( curr_iface_ptr == curr_flow_ptr->ps_iface_ptr );
      DS3GPP2_ASSERT ( PS_IFACE_IS_FLOW_DEFAULT(curr_iface_ptr, 
                                        curr_flow_ptr->ps_flow_ptr ) );
  ...
  for (i = 0; i < DS707_NUM_PHYS_LINKS; ++i)
  {
         ...
         tc_state_ptr->fwd_rlp.flow.ver == DSRLP_VER_HDR_MULTIFLOW_FORWARD)
    {
      ...
      if (item_data_ptr != NULL)
      {
        ...
        item_data_ptr->app_field = 0;
        item_data_ptr->app_ptr = NULL;
		
	//Update app_field and app_ptr to NULL for all dsm chain
        temp_dsm_ptr = item_data_ptr->pkt_ptr;
        while(temp_dsm_ptr != NULL)
        {
          temp_dsm_ptr->app_field = 0;
          temp_dsm_ptr->app_ptr = NULL;
...
)
{
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR,  "Error retrieving IMSI MCC from NV" );
    return FALSE;
  }
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR,  "Error retrieving IMSI MNC from NV" );
    return FALSE;
  }
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR,  "Error retrieving MIN2 from NV");
    return FALSE;
  }
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR,  "Error retrieving MIN1 from NV" );
    return FALSE;
  }
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR," A_key is not active ");
    return FALSE;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_QOS_ERROR_LOG_1(MSG_LEGACY_ERROR, 
  ...
  */
  ...
  /*---------------------------------------------------------------------
    Set Subscription ID for PPP. 
  ---------------------------------------------------------------------*/
  ppp_config_info->subs_id = ppp_get_subs_id( ds707_curr_subs_id() );

  ret_val = ppp_start( PPP_UM_SN_DEV, 
                       ppp_config_info );
...
{
  DATA_IS707_MSG0(MSG_LEGACY_LOW, "get EXIT_TC from CM, enable flow");
  ds707_phys_link_enable_flow(DS_FLOW_707_LOCK_MASK);
...
)
{
  ...
  {
    DS_3GPP2_ERROR_FATAL("CMD PTR NULL");
    return;
  }
  /*---------------------------------------------------------------------- 
  ...
  ds3gdsdif_notify_dsd_roam_status_chg_event(ds707_roaming_ind_ptr->sys_mode, 
                                             ...
                                             roam_flag, DS_SYS_ROAMING_UNSPECIFIED);
...
)
{
  ...
  {
    DS_3GPP2_ERROR_FATAL("ds_cmd_ptr or cmd_payload_ptr is NULL");
    return;
  }
...
#endif
#ifdef FEATURE_DATA_FLOW_MGMT
ds707_burst_flow_control_s       *burst_params_ptr = NULL;
#endif /* FEATURE_DATA_FLOW_MGMT */
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  DATA_MDM_ARG_NOT_CONST(ds_cmd_ptr);
  ...
  if(ds707_data_sess_is_ds_centric_emerg_only_mode())
  {
    ...
    cm_ph_event_info_ptr->as_id == ds707_curr_subs_id() &&
...
#ifdef FEATURE_DATA_FLOW_MGMT
      burst_params_ptr = ds707_wmk_get_burst_params_ptr();

      if (NULL != burst_params_ptr)
      {
        /*---------------------------------------------------------------------
          Run bursty flow control algo when enabling flow on Primary phys link
        ----------------------------------------------------------------------*/
        ds3g_burst_enable_flow(ds707_pkt_get_pri_phys_link_ptr(),
                                ds707_wmk_get_pri_wm_ptr(),
                                DS_FLOW_707_LOCK_MASK, 
                                &burst_params_ptr->burst_params,
                                ds707_wmk_get_tcp_bidir());
      }
#else
...
)
{
  ...
  if((NULL == ds_cmd_ptr) || (NULL == ds_cmd_ptr->cmd_payload_ptr))
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "ds_cmd_ptr or cmd_payload_ptr is NULL");
    return;
  }
  ...
  if(ds707_data_sess_is_ds_centric_emerg_only_mode())
  {
    ...
    cm_ph_event_ptr->as_id == ds707_curr_subs_id() &&
    (
#ifdef FEATURE_DATA_OPTHO
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if (FALSE == iface_in_use)
  {
    ...
    {
      DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "ds707_devmgr_stop() failed");
    }
  }
#endif /* FEATURE_EHRPD */
...
/*===========================================================================
FUNCTION      DS707_PKT_MGR_PROCESS_SUBS_READY_CMD
...
SIDE EFFECTS  None
===========================================================================*/
void ds707_pkt_mgr_process_subs_ready_cmd
(
  ds_cmd_type *ds_cmd_ptr
)
{
  ds707_pkt_subs_notify_cmd_type *notify_info_ptr=NULL;
  ds3gsubsmgr_subs_id_e_type ds3g_subs_id = DS3GSUBSMGR_SUBS_ID_INVALID;

  /* Lint error handler */
  DATA_MDM_ARG_NOT_CONST(ds_cmd_ptr);
  DATA_IS707_MSG0(MSG_LEGACY_HIGH, "Process Subs Ready notification");

  notify_info_ptr = 
  ((ds707_pkt_subs_notify_cmd_type*)(ds_cmd_ptr->cmd_payload_ptr));

  if( (notify_info_ptr == NULL ) || 
      (notify_info_ptr->subs_id <= SYS_MODEM_AS_ID_NONE) ||
      (notify_info_ptr->subs_id >= SYS_MODEM_AS_ID_NO_CHANGE) )
  {
    DATA_IS707_MSG0(MSG_LEGACY_HIGH, "Invalid input. Skip Subs Ready notification");
    return;
  }

  ds3g_subs_id = ds3gsubsmgr_subs_id_cm_to_ds3g(notify_info_ptr->subs_id);

  if(ds3g_subs_id != DS3GSUBSMGR_SUBS_ID_INVALID)
  {
    ds707_subscription_info[ds3g_subs_id] = TRUE;
  }
} /* ds707_pkt_mgr_process_subs_ready_cmd */
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  DATA_IS707_MSG2(MSG_LEGACY_MED, "CM notification sub_cmd %d sub_id %d", 
  ...
  switch (sub_cmd->cm_notify_type)
  {
    ...
    case DS707_SUB_NOT_AVAIL_NOTIFY:
      ds707_pkt_mgr_process_cm_sub_not_avail_cmd(ds_cmd_ptr);
      break;
...
)
{
  ...
  ds3geventmgr_cdma_capability_info_type  *cdma_info_ptr = NULL;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch (event_id)
  {
    ...
    case DS3GEVENTMGR_SUB_NOT_AVAIL_EV:
     ...
     case DS3GEVENTMGR_MMGSDI_SUBSCRIPTION_READY_EV:
       cmd_ptr = ds_allocate_cmd_buf(sizeof(ds707_pkt_subs_notify_cmd_type));
       ...
       if((NULL == cmd_ptr) || (NULL == cmd_ptr->cmd_payload_ptr))
      {
        ...
        cmd_ptr->hdr.cmd_id = DS_CMD_707_SUBSCRIPTION_READY; 
        
        ((ds707_pkt_subs_notify_cmd_type *)(cmd_ptr->cmd_payload_ptr))->subs_id
                 = subs_id;
        ds_put_cmd( cmd_ptr );
      break;
    ...
    case DS3GEVENTMGR_CDMA_CAPABILITY_EV:

      cdma_info_ptr = ( ds3geventmgr_cdma_capability_info_type *)event_info_ptr;
      if(NULL == cdma_info_ptr) 
      {
          DS3GPP2_ASSERT(0);
          return;
      }
      DATA_IS707_MSG2(MSG_LEGACY_HIGH, " Subs ID: %d, CDMA Capability: %d",
                                       subs_id,
                                       cdma_info_ptr->cdma_capable);

      /*----------------------------------------------------------------------- 
        Update the cdma subs ID only when the CDMA capability is TRUE.
      -----------------------------------------------------------------------*/  
      DATA_3GMGR_MSG2(MSG_LEGACY_HIGH,
                    "CDMA subs id - old subs_id[%d], new subs_id[%d] ",
                    ds707_curr_subs_id(),
                    subs_id);
      if(( cdma_info_ptr->cdma_capable == TRUE ) &&
         ( subs_id != ds707_curr_subs_id()) )
      {
        ds707_set_curr_subs_id(subs_id);           
        dsat_set_current_1x_subs_id(subs_id);
        ds707_pkt_mgr_subs_refresh(subs_id);
      }
      break;
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  dsd_event_info.subs_id = 
                ds3gsubsmgr_subs_id_cm_to_ds3g(ds707_curr_subs_id());        
  if (retry_delay_orig_ptr->delay_infinite == TRUE)
  {
    ...
    dsd_event_info.event = DS_DSD_CLIENT_RADIO_RETRY_DELAY_THROTTLED_EV;
  ...
  else if (retry_delay_orig_ptr->delay != 0)
  {
    ...
    ds707_phys_link_disable_flow(DS_FLOW_707_LOCK_MASK );
    ds3g_timer_start( DS3G_TIMER_RETRY_DELAY_ORIG,
                       retry_delay_orig_ptr->delay);
    dsd_event_info.event = DS_DSD_CLIENT_RADIO_RETRY_DELAY_THROTTLED_EV;
    ds_dsd_ext_event_notify(DS_DSD_RADIO_1X, &dsd_event_info);
  }
  else
  {
    ...
    ds3g_timer_stop(DS3G_TIMER_RETRY_DELAY_ORIG);
    dsd_event_info.event = DS_DSD_CLIENT_RADIO_RETRY_DELAY_UNTHROTTLED_EV;
    ds_dsd_ext_event_notify(DS_DSD_RADIO_1X, &dsd_event_info);
    ds707_phys_link_enable_flow(DS_FLOW_707_LOCK_MASK);
  }
}
...
if ( FALSE == hdrcp_smp_session_is_open(HDRHAI_PACKET_APPLICATION_CONTROLLER) ||
    ...
    else
    {
      ...
      else if (hdrcp_stream_stream_is_assigned(HDRHAI_ENHANCED_MULTIFLOW_PACKET_APP_SN))
      {
        ...
        {
          DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "flow prot id not valid, setting so_mask as 0" );
  ...
  {
    DATA_IS707_C_MSG3( DS707_F3_MASK, 
  ...
  /*------------------------------------------------------------------------- 
    Update the last preferred full service RAT information. This information 
    will be used to ensure PDN compatibilty check for eHRPD profiles in 
    eHRPD-OOS-eHRPD scenario.
  -------------------------------------------------------------------------*/
  if( bearer_tech->data.cdma_type.rat_mask != PS_IFACE_CDMA_NULL_BEARER && 
      bearer_tech->data.cdma_type.so_mask != 0 )
  {
    last_pref_rat_mask = bearer_tech->data.cdma_type.rat_mask;
    last_pref_so_mask  = bearer_tech->data.cdma_type.so_mask;
  }
  ...
  *so_mask  = 0;
  ...
  -------------------------------------------------------------------------*/
    ...
    -----------------------------------------------------------------------*/
      ...
      {
        if (FALSE == 
              hdrcp_smp_session_is_valid(HDRHAI_PACKET_APPLICATION_CONTROLLER))
        {
          /*------------------------------------------------------------------- 
            If HDR session is not open notify NULL bearer technology. We have
            already set the rat and so to null bearer initially. Just return 
            TRUE from here
          -------------------------------------------------------------------*/
          DATA_IS707_MSG0(MSG_LEGACY_HIGH, 
                          "ds707_pkt_mgr_get_hdr_sess_info: "
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */  
  ...
  if( bearer_tech == NULL )
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "bearer_tech ptr invalid");
    return FALSE;
  }
  ...
  {
    DATA_IS707_C_MSG4( DS707_F3_MASK, "bearer_tech_info_ex: "
                     "Resetting Old Rat Mask :%d to New Rat Mask :%d  so:0x%x%x",
                     bearer_tech->rat_value,
                     DS_SYS_RAT_EX_NULL_BEARER,
                     QWORD_HIGH(bearer_tech->so_mask),
                     QWORD_LOW(bearer_tech->so_mask));
    bearer_tech->rat_value = DS_SYS_RAT_EX_NULL_BEARER;
  }

  DATA_IS707_MSG4( MSG_LEGACY_MED, 
                   "Bearer Tech info Ex: tech:%d rat:%d so:0x%x%x", 
...
void ds707_pkt_bearer_tech_changed_notification()
{
  ...
  if (  ds707_pkt_mgr_get_bearer_tech_info(&(bearer_tech_change.new_bearer_tech)) == TRUE ) 
  {
                    ...
                    sizeof(ps_iface_bearer_technology_type)))
    {
      ...
      ds707_pkt_mgr_util_report_configured_throughput(
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  switch (ds_cmd_ptr->hdr.cmd_id)
  {
...
#ifdef FEATURE_DATA_FLOW_MGMT
    case DS_CMD_707_PHYS_LINK_DISABLE_FLOW:
    case DS_CMD_707_PHYS_LINK_ENABLE_FLOW:
#endif /* FEATURE_DATA_FLOW_MGMT */
    ...
    case DS_CMD_707_TOGGLE_QNC_ENABLE:
      ...
      {
        DS_3GPP2_ERROR_FATAL("NULL pointer received from ds707_pkt_mgr_get_1x_data_profile_ptr");
        return;
      }
    ...
    case DS_CMD_707_DELAY_OTASN_PDN_TEARDOWN_TIMER_EXPIRED:
      ds707_otasn_timer_expiry_cb_handler(ds_cmd_ptr);
      break;
    ...
    case DS_CMD_707_PRI_FLOW_DISABLE_TMR_EXPIRY:
      ds707_wmk_process_flow_disable_timer_expiry_cmd(ds_cmd_ptr);
      break;
   ...
   case DS_CMD_707_SUBSCRIPTION_READY:
     ds707_pkt_mgr_process_subs_ready_cmd(ds_cmd_ptr);
     break;
...
#ifdef FEATURE_DATA_FLOW_MGMT
    case DS_CMD_707_BURST_PERIODIC_TMR_EXPIRY:
      ds707_burst_cntrl_process_periodic_timer_expiry_cmd(ds_cmd_ptr);
      break;

    case DS_CMD_707_BURST_PROHIBIT_TMR_EXPIRY:
       ds707_burst_cntrl_process_prohibit_timer_expiry_cmd(ds_cmd_ptr);
      break;

    case DS_CMD_707_BURST_WM_GONE_EMPTY:
      ds707_burst_cntrl_gone_empty_cmd_hdlr(ds_cmd_ptr);
      break;
#endif /* FEATURE_DATA_FLOW_MGMT */

    case DS_CMD_707_SYNC_ESPM_IND:
      ds707_sync_espm_ind_cback_hdlr(ds_cmd_ptr);
      break;

    default:
      DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR, "Unknown cmd to ds707_pkt %d",
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  {
    DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR,"unknown signal to ds707_pkt 0x%x", sigs);
    DS3GPP2_ASSERT(0);
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (cm_client_dbm_event_reg(cm_id,
                              ...
                              NULL) != CM_CLIENT_OK)
  {
    DS_3GPP2_ERROR_FATAL("Could not reg with CM");
  }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "DS could not allocate SR_ID");
  }
  else
  {
    DATA_IS707_MSG1( MSG_LEGACY_HIGH, 
  ...
  for (i = 0; i < DS707_MAX_IFACES; i++)
  {
    ...
    if (iface_ptr == NULL)
    {
      DS_3GPP2_ERROR_FATAL("Iface ptr is NULL");
    }
    ...
    if (my_acl_ptr == NULL)
    {
      DS_3GPP2_ERROR_FATAL("ACL ptr is NULL");
    }
...
/*lint -restore */    
    ...
    {
      DS_3GPP2_ERROR_FATAL("Could not create dssnet4");
    }
    /*---------------------------------------------------------------------
    ...
    {
      DS_3GPP2_ERROR_FATAL("Could not register iface_down_ind cb with DSSNET4");
    ...
    if  (ds3g_is_ipv6_enabled()) 
      ...
      ---------------------------------------------------------------------*/
         sm_config_ptr = ds3g_get_ipv6_sm_config_info(ds707_curr_subs_id());
      ip6_sm_ptr         = ds707_pdncntx_get_ip6_sm_ptr(ds707_iface_tbl_ptr);
      if ((NULL == ip6_sm_ptr) || (NULL == sm_config_ptr))
      {
        DS_3GPP2_ERROR_FATAL("ip6_sm_ptr or sm_config is NULL");
        return (rex_sigs_type)0;
      }
      ...
      {
        DS_3GPP2_ERROR_FATAL("dssnet6_sm_ptr is NULL");
        return (rex_sigs_type)0;
      }
      ...
      {
        DS_3GPP2_ERROR_FATAL("Could not create dssnet6");
      }
      ...
      {
        DS_3GPP2_ERROR_FATAL("Could not register iface_down_ind cb with DSSNET6");
        return (rex_sigs_type)0;
      }
    }
#endif /* FEATURE_DATA_PS_IPV6 */
    ...
    {
      DS_3GPP2_ERROR_FATAL("ps_iface_set_addr_family returned error");
      return (rex_sigs_type)0;
    }
  ...
  {
    DS_3GPP2_ERROR_FATAL("could not init 707 RMSM");
  }
    ...
    for (i = 0; i < DS707_MAX_ANY_INSTS; i++)
    {
      ...
      if ((failover_mode != IPV4_ONLY) && (failover_mode != IPV6_ONLY))
      {
        if (dssnetAny_sm_create(dssnetAny_sm_ptr,
                                ...
                                failover_mode) < 0)
        {
          DS_3GPP2_ERROR_FATAL("Could not create dssnetAny SM");
        }
      }
  ...
  -------------------------------------------------------------------------*/
   ...
   for(i = SYS_MODEM_AS_ID_1; i < DS3GSUBSMGR_SUBS_ID_COUNT; i++)
   {
     ...
     ds3geventmgr_event_reg(DS3GEVENTMGR_CDMA_CAPABILITY_EV,
                            DS3GEVENTMGR_CLIENT_ID_3GPP2,
                            &filter_info,
                            ds707_pkt_mgr_ds3g_event_cb,
                            NULL);
  ...
  -------------------------------------------------------------------------*/
   ...
   ds3geventmgr_event_reg(DS3GEVENTMGR_MMGSDI_SUBSCRIPTION_READY_EV,
                          DS3GEVENTMGR_CLIENT_ID_3GPP2,
                          &filter_info,
                          ds707_pkt_mgr_ds3g_event_cb,
                          NULL);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
      ...
      TRUE == ds707_back_pressure_params.data.enabled )
  {
    ...
    -------------------------------------------------------------------------*/
    if ( DSM_QUEUE_PRIORITY_HIGH == dsm_item_get_priority(*dsm_item_ptr_ptr) &&
         ( ppp_get_phase(PPP_UM_SN_DEV) == pppNETWORK ) ) 
    {
      ds707_back_pressure_params.tcp_rev_link_ack_counter++;
      ...
      {
        if(PS_TIMER_SUCCESS != 
               ps_timer_start(ds707_back_pressure_params.tmr_hndl_dyn_back_pressure,
                              ds707_back_pressure_params.data.timer_val))
        {
          DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR,"Failed to start timer ");
        }
        ...
        {
          DATA_IS707_MSG0(MSG_LEGACY_HIGH,
                          "Started BP timer for the first packet");
        }
      }
      
      if ( ds707_back_pressure_params.tcp_rev_link_ack_counter == 
  	                                DS707_BACK_PRESSURE_ACK_THRESHOLD + 1 )
      {
...
)
{
  ...
  {
    DS_IS707_ERROR_LOG_2(MSG_LEGACY_ERROR,"Error in obataining resolved sub id %d "
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  ds707_pdncntx_post_iface_down(this_iface_ptr, reason, FALSE);
...
)
{
  ...
  boolean                        is_ims_call = FALSE;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

  if((NULL == cmd_ptr) || (NULL == cmd_ptr->cmd_payload_ptr))
  {
    DS_3GPP2_ERROR_FATAL("cmd_ptr or cmd_payload_ptr is NULL");
    return;
  }
...
#ifdef FEATURE_MSIM_L_PLUS_L 
  /*--------------------------------------------------------------- 
    Check with DDS switch manager if the call is allowed or not.
   ---------------------------------------------------------------*/
  pdncntx_cb_ptr = ds707_pdncntx_get_pdncntx_cb_ptr_from_iface_ptr(
                                                             this_iface_ptr);
  is_ims_call = ds707_pdncntx_is_ims_pdn(pdncntx_cb_ptr,
    ds3gsubsmgr_subs_id_ds_to_cm(PS_IFACE_GET_SUBS_ID(this_iface_ptr)));
  if ( FALSE == ds3g_dds_switch_hdlr_is_call_allowed(this_iface_ptr,is_ims_call))
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR,"DDS switch in progress."
                                     " call not allowed");
    ds707_post_flow_gonull_iface_down_ind
      (this_iface_ptr,PS_NET_DOWN_REASON_DDS_SWITCH_IN_PROGRESS);
    return;
  }
#endif
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR,"Thermal mitigation in progress."
                                     " call not allowed");
    ...
    -----------------------------------------------------------------------*/
      ...
      if (p_rev_index < (word) DS707_P_REV_6)
      {
        /* if the P_rev is < 6 then it will not be possible to make an SO33
           call */
        ds707_post_flow_gonull_iface_down_ind(this_iface_ptr, 
                             PS_NET_DOWN_REASON_UNSUPPORTED_1X_PREV);
        return;
      }
  ...
  {
    DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR, "Invalid Iface bringup 0x%x",
                     this_iface_ptr);
    ds707_post_flow_gonull_iface_down_ind(this_iface_ptr, 
                         PS_NET_DOWN_REASON_VALIDATION_FAILURE);
    return;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  ds707_pkt_mgr_notify_call_orig_event_to_ds3g(this_iface_ptr);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    local_abort = TRUE;
    end_reason = PS_NET_DOWN_REASON_CLIENT_END;
  }
  ...
  {
    DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR, "Invalid Iface tear down 0x%x",
                     this_iface_ptr);
    DS3GPP2_ASSERT(0);
    return;
  }
  ...
  {
    DS_3GPP2_ERROR_FATAL("dssnet_stop failed");
    return;
  }
} /* ds707_socket_tear_down_cmd() */
/*lint -restore*/
...
)
{
  switch(end_reason)
  {
    ...
    {
      ds707_mode_handler_info.call_end_reason =  
        PS_NET_DOWN_REASON_CM_UNKNOWN_ERROR; 
...
)
{
  ...
  {
    DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR, "Bad NV read status %d", status);
...
)
{
  ...
  DATA_IS707_MSG2_EX( MSG_LEGACY_MED,
                      subs_id,
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  DATA_IS707_MSG1_EX(MSG_LEGACY_HIGH,
                     subs_id,
                     "NV Refresh Event occurred as part of hotswap. Read all NVs for subs_id %d",
                     subs_id);
  
  ds707_refresh_info.nv_refresh_subs_id = subs_id;
  ds707_refresh_info.refresh_type = DS707_REFRESH_TYPE_NV;
  ds707_nv_util_nvrefresh(ds707_refresh_info.curr_subs_id); 
  ...
  ds3geventmgr_set_and_notify_nv_refresh_status(DS3GEVENTMGR_CLIENT_ID_3GPP2, subs_id);
  ds707_refresh_info.refresh_type = DS707_REFRESH_TYPE_NONE;

  return;
} /* ds707_pkt_mgr_nv_refresh */

/*===========================================================================
FUNCTION DS707_PKT_MGR_SUBS_REFRESH

DESCRIPTION    CDMA subs_id change occurred so we need to read all the stored 
               NVs and EFS paths 
...
===========================================================================*/
void ds707_pkt_mgr_subs_refresh(sys_modem_as_id_e_type subs_id)
{
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  DATA_IS707_MSG1(MSG_LEGACY_HIGH, 
                  "Subs ID for CDMA changed. Read all NVs for subs_id %d",subs_id);
  ...
  ds707_refresh_info.curr_subs_id = subs_id;
  ds707_refresh_info.refresh_type = DS707_REFRESH_TYPE_SUBS;
    ds707_nv_util_nvrefresh(ds707_refresh_info.curr_subs_id); 

  /*Debug Mesg*/ 
  DATA_IS707_MSG1(MSG_LEGACY_HIGH, 
                  "Refresh done for subs_id %d",
                   subs_id);
  
  ds707_refresh_info.refresh_type = DS707_REFRESH_TYPE_NONE;
  return;
} /* ds707_pkt_mgr_subs_refresh */
...
sys_modem_as_id_e_type ds707_curr_subs_id()
{
   ...
   -------------------------------------------------------------------------*/
  return ds707_refresh_info.curr_subs_id;
}/* ds707_curr_subs_id */

/*==================================================================================
FUNCTION       DS707_GET_NV_REFRESH_SUBS_ID

DESCRIPTION    Return the subs id on which NV refresh is triggered
 
DEPENDENCIES   None.

RETURN VALUE   subs id (sys_modem_as_id_e_type) 

SIDE EFFECTS   None

=====================================================================================*/
sys_modem_as_id_e_type ds707_get_nv_refresh_subs_id()
{
   /*-------------------------------------------------------------------------
   Return the current subs id  
   -------------------------------------------------------------------------*/
  return ds707_refresh_info.nv_refresh_subs_id;
}/* ds707_curr_subs_id */


/*==================================================================================
FUNCTION       DS707_SET_CURR_SUBS_ID

DESCRIPTION    Sets the current subs id in a global variable
...
=====================================================================================*/
void ds707_set_curr_subs_id(sys_modem_as_id_e_type subs_id)
{
   /*-------------------------------------------------------------------------
   Set the current subs id  
   -------------------------------------------------------------------------*/
  ds707_refresh_info.curr_subs_id = subs_id;
}/* ds707_curr_subs_id */
...
/*==================================================================================
FUNCTION       DS707_GET_REFRESH_TYPE

DESCRIPTION    Return the refresh type in a global variable
 
DEPENDENCIES   None.

RETURN VALUE   Return the current refesh type 
              (ds707_refresh_type) 

SIDE EFFECTS   None

=====================================================================================*/
ds707_refresh_type ds707_get_refresh_type()
{
   /*-------------------------------------------------------------------------
   Return the current subs id  
   -------------------------------------------------------------------------*/
  return ds707_refresh_info.refresh_type;
}/* ds707_curr_subs_id */
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  {  
    ds707_subscription_info[index] = FALSE;
  }
...
)
{
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "MMGSDI ERROR");
    return;
  }

  DS3GPP2_ASSERT( cnf_ptr != NULL );
 ...
 */
    if ( ( ds707_pkt_mgr_get_current_sys_mode() == SYS_SYS_MODE_LTE ) &&
         ( ds3g_get_current_network_mode_ex(ds707_curr_subs_id()) == SYS_SYS_MODE_LTE )&&
         ( ds3g_get_current_srv_status_ex(ds707_curr_subs_id())
                                                == SYS_SRV_STATUS_SRV ) )
    {
...
/*-------------------------------------------------------------------------*/
  ...
  return strlen(nv_item_file_path);
}
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  { 
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Null params");
    return ret_val;
  }
...
/*===========================================================================
FUNCTION      DS707_PKT_IS_EHRPD_LAST_PREF_SYS

DESCRIPTION   Utility function to see if eHRPD was the last preferred system.

DEPENDENCIES  None

RETURN VALUE  TRUE  - if last preferred system was eHRPD
                        FALSE - otherwise

SIDE EFFECTS  None
===========================================================================*/
boolean ds707_pkt_is_ehrpd_last_pref_sys(void)
{
  if(( (last_pref_rat_mask == PS_IFACE_CDMA_EVDO_REVA) && 
       (last_pref_so_mask == PS_IFACE_CDMA_EVDO_EMPA_EHRPD) ) || 
     ( (last_pref_rat_mask == PS_IFACE_CDMA_EVDO_REVB) && 
       (last_pref_so_mask == PS_IFACE_CDMA_EVDO_MMPA_EHRPD) )  )
  {
    return  TRUE;
  }
  else
...
/*===========================================================================
FUNCTION      DS707_PKT_MGR_READ_INTERNAL_AUTH_NV
...
SIDE EFFECTS  None
===========================================================================*/
void ds707_pkt_mgr_read_internal_auth_nv( void )
{
  char item_file_path[] = "/nv/item_files/data/3gpp2/internal_auth";
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */

  DATA_IS707_MSG1(MSG_LEGACY_LOW,
                  "ds707_pkt_mgr_read_internal_auth_nv for subs_id %d", ds707_curr_subs_id());

  if( NV_DONE_S != ds3gcfgmgr_read_efs_nv_ex(item_file_path,
                                             (void*)&ds707_is_auth_internal,
                                             sizeof(boolean),
                                             ds707_curr_subs_id()) )
  {
    DATA_IS707_MSG0(MSG_LEGACY_LOW,
                    "INTERNAL_AUTH read failed, setting default = TRUE");
    ds707_is_auth_internal = TRUE;
  }
  else 
  {
    DATA_IS707_MSG1(MSG_LEGACY_LOW,
                    "INTERNAL_AUTH read returned %d", ds707_is_auth_internal);
  }
  return;
} /* ds3g_read_internal_auth_nv() */

/*===========================================================================
FUNCTION      DS707_PKT_MGR_GET_AUTH_INTERNAL_NV
...
SIDE EFFECTS  None
===========================================================================*/
boolean ds707_pkt_mgr_get_auth_internal_nv( void )
{
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  return ds707_is_auth_internal;
} /* ds3g_get_auth_internal_nv() */

/*===========================================================================
FUNCTION      DS707_PKT_MGR_IS_CDMA_SUBSCRIPTION_READY

DESCRIPTION   Utility function to see if subscription is ready for the subs.
...
SIDE EFFECTS  None
===========================================================================*/
boolean ds707_pkt_mgr_is_cdma_subscription_ready
(
...
)
{
  ...
  return ds707_subscription_info[ds3g_subs_id];
} /* ds707_pkt_mgr_is_cdma_subscription_ready */

/*===========================================================================
FUNCTION      DS3GPP_PDN_CONTEXT_DETERMINE_POLICY_APN_TYPE

DESCRIPTION
  This function checks if the input pdn is IMS/EMERGNECY or not  

PARAMETERS

  ds707_pdn_context_cb_type   -  PDN context 

RETURN VALUE
  ps_iface_apn_enum_type - APN Type
...
SIDE EFFECTS
  None
===========================================================================*/
ps_iface_apn_enum_type  ds707_pkt_mgr_determine_policy_apn_type
(
  ds707_pdn_context_cb_type  *pdncntx_cb_ptr
)
{
 ...
 /* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  do
  {
    ...
    apn_type = PS_IFACE_GET_APN_TYPE (v4_iface_ptr);

    if ( apn_type != PS_IFACE_APN_TYPE_UNSPECIFIED)
    {
      break;
    }

    v6_iface_ptr = ds707_pdncntx_get_v6_iface(pdncntx_cb_ptr);
    apn_type = PS_IFACE_GET_APN_TYPE (v6_iface_ptr);
  ...
  return apn_type;
} /* ds707_pkt_mgr_determine_policy_apn_type */


/*===========================================================================
FUNCTION      DS707_PKT_MGR_NOTIFY_CALL_ORIG_EVENT_TO_DS3G

DESCRIPTION   Function to notify call orig event to DS3G event manager..
...
SIDE EFFECTS  None
===========================================================================*/
void ds707_pkt_mgr_notify_call_orig_event_to_ds3g
(
  ps_iface_type*            this_iface_ptr
)
{
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if (!PS_IFACE_IS_VALID(this_iface_ptr))
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Invalid Iface pointer");
    return;
  }
  ...
  if (!DS707_PDNCNTX_CB_IS_VALID(pdncntx_cb_ptr))
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Invalid PDN");
    return;
  }
  ...
  if( ps_iface_addr_family_is_v4(this_iface_ptr) )
  {
    companion_iface_ptr = ds707_pdncntx_get_v6_iface(pdncntx_cb_ptr);
  }
  else if( ps_iface_addr_family_is_v6(this_iface_ptr) )
  {
    companion_iface_ptr = ds707_pdncntx_get_v4_iface(pdncntx_cb_ptr);
  }

  if ( companion_iface_ptr != NULL &&
       PS_IFACE_IS_VALID(companion_iface_ptr) &&
       PS_IFACE_IS_IN_USE(companion_iface_ptr) )
  {
    return;
  }

  if ( ds707_pdncntx_is_ims_pdn(pdncntx_cb_ptr,subs_id) )
  {
    is_ims_call = TRUE;
  }
  
  /*-----------------------------------------------------------------------
     APN name is not used in case of 1x/HRPD system ( NON - EPC ).
     So pass the APN name only in case of eHRPD
  -----------------------------------------------------------------------*/
  if (DS707_SYS_INFO_EPC_SYSTEM == ds707_pdncntx_get_orig_sys(pdncntx_cb_ptr))
  {
    ds707_pdncntx_get_profile(pdncntx_cb_ptr, &data_sess_profile_info_ptr);

    if(data_sess_profile_info_ptr != NULL)
    {
      apn_name = data_sess_profile_info_ptr->apn_string;
    }
  }
  
  apn_type = ds707_pkt_mgr_determine_policy_apn_type(pdncntx_cb_ptr);

  call_type = ds3g_data_call_activity_get_ps_call_type
              (subs_id,  
               apn_name, 
               is_ims_call,
               apn_type);

  ds3geventmgr_notify_ps_call_event_per_sub_ex
  (
    subs_id,
  ...
  return;
} /* ds707_pkt_mgr_notify_call_orig_event_to_ds3g */

/*===========================================================================
FUNCTION      DS707_PKT_MGR_NOTIFY_CALL_END_EVENT_TO_DS3G

DESCRIPTION   Function to notify call end event to DS3G event manager..
...
SIDE EFFECTS  None
===========================================================================*/
void ds707_pkt_mgr_notify_call_end_event_to_ds3g
(
  ps_iface_type               *this_iface_ptr,
  void                        *pdn_cb_ptr,
  sys_modem_as_id_e_type       subs_id
)
{
  ...
  boolean                               is_ims_call = FALSE;
  char                                 *apn_name = NULL;
  boolean                               is_last_non_ims_pdn = FALSE;
  ds707_pdn_context_cb_type            *pdncntx_cb_ptr = NULL;
  ps_iface_apn_enum_type                apn_type;
  ds3g_data_call_activity_ps_call_type  call_type;
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */

  pdncntx_cb_ptr = (ds707_pdn_context_cb_type*)pdn_cb_ptr;
  ...
  is_last_non_ims_pdn = ds707_pdncntx_is_last_iface_for_non_ims_pdn(this_iface_ptr,subs_id);
  ...
  return;
} /* ds707_pkt_mgr_notify_call_end_event_to_ds3g */

/*===========================================================================
  FUNCTION   DS_707_PKT_MGR_READ_ELEVATOR_EXIT_SUPPORT
  
  DESCRIPTION
    This Function is called to read elevator exit support.
  ...
  SIDE EFFECTS
    None
===========================================================================*/
void ds707_pkt_mgr_read_elevator_exit_support(void)
{

  char item_file_path[] = "/nv/item_files/data/3gpp2/elevator_exit_supported";

/*-------------------------------------------------------------------------
    Read the tethered_nai_prefix from the EFS item.
    If the item is not present or it is invalid, the default value
    will be used.
 -------------------------------------------------------------------------*/
 
  if(MCFG_FS_STATUS_OK == mcfg_fs_read(item_file_path,
                 &ds707_pkt_mgr_is_elevator_exit_supported,
                sizeof(boolean),
                MCFG_FS_TYPE_EFS,
                (mcfg_fs_sub_id_e_type)ds707_curr_subs_id()))
  {
    DATA_IS707_MSG1(MSG_LEGACY_HIGH, "elevator exit enabled, %d",
                  ds707_pkt_mgr_is_elevator_exit_supported);
    return;
  }
  DATA_3GMGR_MSG0(MSG_LEGACY_HIGH,
                    "elvator_exit_support NV read failed");
  return;

}

/*===========================================================================
  FUNCTION   DS_707_PKT_MGR_GET_ELEVATOR_EXIT_SUPPORT
  
  DESCRIPTION
    This Function is called to get elevator exit support from NV.

  DEPENDENCIES
    None.
  
  RETURN VALUE
    mav_ds_tethered_nai_prefix from efs
  
  SIDE EFFECTS
    None
===========================================================================*/
boolean ds707_pkt_mgr_get_elevator_exit_support(void)
{
  return ds707_pkt_mgr_is_elevator_exit_supported;
}
