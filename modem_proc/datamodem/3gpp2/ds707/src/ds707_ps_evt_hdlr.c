...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp2/ds707/src/ds707_ps_evt_hdlr.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if( ds3gsubsmgr_subs_id_ds_to_cm(
         (ds_sys_subscription_enum_type)subs_id) != ds707_curr_subs_id() )
  {
    DATA_IS707_MSG0(MSG_LEGACY_HIGH, "Ignoring event");
    return;
  }
  ...
  switch (event_name)
  {
    ...
    case PS_SYS_EVENT_3GPP_LTE_ATTACH_COMPLETE:
    {
      ...
      {
        DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "NULL event info passed");
        return;
      }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  /* Register for PS SYS event*/
    if(0 != ps_sys_event_reg_ex(tech_type, 
                                event_name, 
                                subs_id,
                                ds707_ps_evt_hdlr_cb,
                                user_data_ptr, 
