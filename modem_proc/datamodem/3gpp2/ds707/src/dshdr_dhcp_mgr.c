...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp2/ds707/src/dshdr_dhcp_mgr.c#2 $
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  {
    DS_IS707_ERROR_LOG_3(MSG_LEGACY_ERROR, "dshdr_dhcp6_status_cb failed. Status:%d handle:%d"
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if (status != DHCP_CLIENT_PROVISIONED)
  {
    DS_IS707_ERROR_LOG_3(MSG_LEGACY_ERROR, "dshdr_dhcp_status_cb failed. Status:%d handle:%d"
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if (dhcp_client_params_init(dhcp_params_ptr,
                              ...
                              0) < 0)
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Unable to init DHCP client params");
    return -1;
  }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if (!dhcp_version_4)
  {
    ...
    if (!dhcp6_handle)
    {
      DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Uninitalized DHCPv6 handle");
      return;
    }
  ...
  else
  {
    ...
    if (!dhcp_handle)
    {
      DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Uninitialized DHCPv4 handle");
      return;
    }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if (!dhcp_version_4)
  {
    ...
    {
      DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Unable to init DHCP6 client params, not triggering DHCP6");
      return;
    }
    ...
    {
      DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR,"ps_iface_set_dhcp_handle failed");
      return;
    }
    else
    {
  ...
  else
  {
    ...
    {
      DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Unable to init DHCP client params, not triggering DHCP4");
      return;
    }
