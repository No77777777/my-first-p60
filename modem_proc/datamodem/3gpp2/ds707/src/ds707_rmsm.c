...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp2/ds707/src/ds707_rmsm.c#2 $

when        who    what, where, why
--------    ---    ---------------------------------------------------------- 
04/01/16    skc    Removing FEATURE_UW_FMC
...
typedef struct
{
  ...
  void *                  um_iface_event_buf_ptr;
  void *                  um_phys_link_event_buf;
  void *                  um_flow_tx_event_buf_ptr;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_3GPP2_ERROR_FATAL("Could not create iface" );
  }
 ...
 (void) ps_iface_dpl_support_flow_logging(&ds707_rmsmi_info.sio_pkt_iface,
  ...
  /*-------------------------------------------------------------------------
    Set the Buffers to NULL. They are allocate when rmsm gets iface
    after routing
  -------------------------------------------------------------------------*/
  ds707_rmsmi_info.um_iface_event_buf_ptr = NULL;
  ds707_rmsmi_info.um_phys_link_event_buf = NULL;
  ds707_rmsmi_info.um_flow_tx_event_buf_ptr = NULL;
...
/**************************************************************************/
 ...
 /*-------------------------------------------------------------------
  ...
  {
    DS_3GPP2_ERROR_FATAL( "Unable to acquire Rm PPP event handle" );
  }
  ...
  {
    DS_3GPP2_ERROR_FATAL( "Unable to acquire Um PPP event handle" );
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(ds707_rmsmi_info.inited != TRUE)
  {
    DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR,"Can't post ev %d, RMSM not inited", 
                     event);
    DS3GPP2_ASSERT(0);
    return;
  }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  if(NULL == ds_cmd_ptr)
  {
    DS_3GPP2_ERROR_FATAL("ds_cmd_ptr is NULL");
    return;
  }
  ...
  if(ds707_rmsmi_info.inited != TRUE)
  {
    DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR,"Can't post cmd %d, RMSM not inited", 
                     cmd);
    DS3GPP2_ASSERT(0);
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
      ...
      ds3gsubsmgr_is_device_mode_multi_sim_standby() == TRUE)
  {
    DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR,
                    "Non-PS subs_id: %d passed in dial_str_cb, failing call",
                     subs_id);
    return NULL;
  }
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Unable to allocate SIOLIB port");
    return(NULL);
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch( ds707_rmsmi_info.state )
  {
    case DS707_RMSM_NULL_STATE:
    {
      ...
      {
        DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR, "ps_sys_event_ind failed %d", 
                        ps_error);
      }
    }
    ...
    case DS707_RMSM_WAITING_FOR_ENTITLEMENT_CHECK_STATE:
    {
      ...
      if(ds707_roaming_is_curr_mode_jcdma() == TRUE)
      {
        if (ds707_get_jcdma_card_info( )== MMGSDI_JCDMA_GRAY_CARD)
        {
          ...
          {
            DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Call Failed. Cleaning SIO");
            (void)ds3g_siolib_set_cd_state( DS3G_SIOLIB_REMOTE_CARRIER_OFF );
            dsat_send_result_code( DSAT_NO_CARRIER );
          ...
          {
             DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR,"Moving to NULL State");
      ...
      {
        DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Iface ptr is NULL");
        if (ds707_rmsmi_info.entitlement_check_req == FALSE)
        {
          DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR,"Call Failed. Cleaning SIO");
          (void) ds3g_siolib_set_cd_state( DS3G_SIOLIB_REMOTE_CARRIER_OFF );
       ...
       -------------------------------------------------------------------*/
       /*-------------------------------------------------------------------
         Allocate all the required event handlers here, as the iface is
         available.         
       -------------------------------------------------------------------*/  
      if (ds707_rmsmi_info.um_iface_event_buf_ptr == NULL)
      {
      
        ds707_rmsmi_info.um_iface_event_buf_ptr =
        ...
        if (ds707_rmsmi_info.um_iface_event_buf_ptr == NULL) 
        {
          DS_3GPP2_ERROR_FATAL("Cannot allocate event buffer");
          return;
        }
      }
       /*-------------------------------------------------------------------
      ...
      {
        /*-----------------------------------------------------------------
          Register for the phys link flow disabled phys_link events.
        -----------------------------------------------------------------*/
        if (ds707_rmsmi_info.um_phys_link_event_buf == NULL)
        {
          ds707_rmsmi_info.um_phys_link_event_buf =
        ...
        num_events = 2; 
        events[0] = PHYS_LINK_FLOW_ENABLED_EV;
        events[1] = PHYS_LINK_FLOW_DISABLED_EV;
        ...
        num_events = 2; 
        events[0] = IFACE_FLOW_ENABLED_EV;
        events[1] = IFACE_FLOW_DISABLED_EV;
        if( -1 == 
        ...
        /*-------------------------------------------------------------------
          Register for the flow_flow enabled and  disabled events.
        -------------------------------------------------------------------*/

        if (ds707_rmsmi_info.um_flow_tx_event_buf_ptr == NULL)
        {
          ds707_rmsmi_info.um_flow_tx_event_buf_ptr =
        ...
        num_events = 2; 
        events[0] = FLOW_TX_ENABLED_EV;
        events[1] = FLOW_TX_DISABLED_EV;
        if( -1 == 
  ...
  switch( ds707_rmsmi_info.state )
  {
    ...
    -------------------------------------------------------------*/
                                                                   ...
                                                                   == TRUE) )
      {
        ...
        {
           DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR,"PHYS LINK PTR IS NULL!");
        }
        else
        {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch( ds707_rmsmi_info.state )
  {
...
#endif /* FEATURE_DS_MOBILE_IP */
    {
       ...
       {
         DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Could not deregister for "
    ...
    case DS707_RMSM_WAITING_FOR_UM_ARBITRATION:
    {
      ...
      else 
      {
        ...
        if (NULL == ds707_rmsmi_info.um_iface_event_buf_ptr)
        {
          ds707_rmsmi_info.um_iface_event_buf_ptr =
                                       (void *) ps_iface_alloc_event_handle(
              ds707_rmsmi_info.IS707pkt_iface_ptr,
        ...
        if( -1 == 
          ps_iface_subscribe_event_list(
                                 ds707_rmsmi_info.um_iface_event_buf_ptr,
                                  ...
                                  num_events))
        {
          DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Disallowing the laptop call. "
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch( ds707_rmsmi_info.state )
  {
    ...
    case DS707_RMSM_WAITING_FOR_UM_UP_SIP_STATE:
    {
      ...
      {
         DATA_IS707_MSG2(MSG_LEGACY_HIGH, 
                        "De-registering for PHYS_LINK_UP_EV for iface: 0x%x::%d",
                         ds707_rmsmi_info.IS707pkt_iface_ptr->name,
                         ds707_rmsmi_info.IS707pkt_iface_ptr->instance );  
		 num_events = 1;
                                                   ...
                                                   DS_CRM_VAL_NETMODEL_MODE )
      {
        ...
        ---------------------------------------------------------------------*/
        ds707_rmsmi_info.rm_ppp_config.subs_id = ppp_get_subs_id( ds707_curr_subs_id() );
        
        if( ppp_start( PPP_RM_DEV, 
                       &ds707_rmsmi_info.rm_ppp_config ) < 0 )
        {
          DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR,
        ...
        {
          DS_3GPP2_ERROR_FATAL( "NULL PPP event handles");
        }
        else
        {
      ...
      else if ( SYS_SYS_MODE_HDR == sys_mode )
      {
        ...
        {
          DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR,"Invalid HRD PROT %d", prot);
        }
      }
#endif /* FEATURE_HDR */
      else
      {
        DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR, "Invalid Operating mode %d",
                        sys_mode);
      }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch( ds707_rmsmi_info.state )
  {
    ...
    case DS707_RMSM_WAITING_FOR_RM_UP_MIP_STATE:
    {
      ...
      if(dev_opts == NULL)
      {
        DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "ds707_rmsmi_process_rm_ppp_up_ev():"
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch( ds707_rmsmi_info.state )
  {
    ...
    {      
      ...
      dpli_log_iface_desc_and_status(&ds707_rmsmi_info.sio_pkt_iface,
                                     ds707_rmsmi_info.IS707pkt_iface_ptr);
      break;
    }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch( ds707_rmsmi_info.state )
  {
    case DS707_RMSM_NULL_STATE:
    {
      DS_3GPP2_ERROR_FATAL("PPP event received while in NULL state ");
      return;
    }
    //lint -fallthrough
    case DS707_RMSM_WAITING_FOR_UM_UP_RELAY_STATE:
...
)
{
  ...
  sio_ioctl_param_type wait_param;
  uint8                          num_events = 0;
  ps_iface_event_enum_type       events[6];
  ...
  switch( new_state )
  {
    ...
    {   
      ...
      if (ds707_rmsmi_info.um_iface_event_buf_ptr != NULL)
      {
        ...
        events[1] = IFACE_DOWN_EV;
        events[2] = IFACE_CONFIGURING_EV;
        events[3] = IFACE_FLOW_ENABLED_EV;
        events[4] = IFACE_FLOW_DISABLED_EV;
        events[5] = IFACE_ADDR_CHANGED_EV;

        ps_iface_unsubscribe_event_list(ds707_rmsmi_info.um_iface_event_buf_ptr,
                                    (ps_iface_event_enum_type *)&events,
                                    num_events);
      }
      /*---------------------------------------------------------------------
      ...
      if (ds707_rmsmi_info.um_phys_link_event_buf != NULL)
      {
        ...
        events[3] = IFACE_PHYS_LINK_UP_EV;
        ps_phys_link_unsubscribe_event_list(
                                 ds707_rmsmi_info.um_phys_link_event_buf,
                                 (ps_iface_event_enum_type *)&events,
                                 num_events);
      ...
      ps_flow_free_event_handle(ds707_rmsmi_info.um_flow_tx_event_buf_ptr);
      ps_phys_link_free_event_handle(ds707_rmsmi_info.um_phys_link_event_buf);
      ps_iface_free_event_handle(ds707_rmsmi_info.um_iface_event_buf_ptr);
      ds707_rmsmi_info.um_flow_tx_event_buf_ptr = NULL;
      ds707_rmsmi_info.um_phys_link_event_buf = NULL;
      ds707_rmsmi_info.um_iface_event_buf_ptr = NULL;
      ...
      ds3gflowmgr_notify_ps_call_state_per_sub(
         ds707_curr_subs_id(), DS3GFLOWMGR_PS_CALL_END_EV);
...
#else
        ds3g_timer_start( DS3G_TIMER_DUN_CTRL_ENTL, 1000);
    ...
    case DS707_RMSM_WAITING_FOR_UM_UP_MIP_STATE:
    {
      ...
      if(ds707_rmsmi_info.um_phys_link_event_buf != NULL)
       {
        DATA_IS707_MSG2(MSG_LEGACY_HIGH, 
                      "De-registering for IFACE_PHYS_LINK_UP_EV"
                      " for iface: 0x%x::%d",
                       ds707_rmsmi_info.IS707pkt_iface_ptr->name,
                        ds707_rmsmi_info.IS707pkt_iface_ptr->instance );
        num_events = 1; 
    ...
    {   
      ...
      num_events = 1; 
      events[0] =IFACE_ADDR_CHANGED_EV;
      ...
      if( -1 == 
             ps_iface_subscribe_event_list(
                    ds707_rmsmi_info.um_iface_event_buf_ptr,
                           ...
                           num_events))
      {
        DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "IP_ADDR_CHAGED ev reg failed" );
         if ( ps_iface_go_null_cmd( ds707_rmsmi_info.IS707pkt_iface_ptr,
      ...
      {
        modem_mem_free(dev_opts, MODEM_MEM_CLIENT_DATA_CRIT);
        DS_3GPP2_ERROR_FATAL( "NULL Rm PPP event handle" );
      }
      else
      {
        (void) ppp_event_register( ds707_rmsmi_info.ppp_rm_event_handle,
      ...
      {
        modem_mem_free(dev_opts, MODEM_MEM_CLIENT_DATA_CRIT);
        DS_3GPP2_ERROR_FATAL( "NULL Um PPP event handle" );
      }
      else
      {
        (void) ppp_event_register( ds707_rmsmi_info.ppp_um_event_handle,
      ...
      else if ( SYS_SYS_MODE_HDR == sys_mode )
      {
        ...
        {
          DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR, "Invalid HDR PROT %d", prot);
        }
      }
#endif /* FEATURE_HDR */
      else
      ...
      /*---------------------------------------------------------------------
        Set Subscription ID for PPP. 
      ---------------------------------------------------------------------*/
      dev_opts->subs_id = ppp_get_subs_id( ds707_curr_subs_id() );
    ...
    case DS707_RMSM_WAITING_FOR_UM_UP_SIP_STATE:
    {
      ...
      {
        if (NULL == ds707_rmsmi_info.um_phys_link_event_buf)
        {
          ds707_rmsmi_info.um_phys_link_event_buf =
                           (void *)ps_phys_link_alloc_event_handle(
                              PS_IFACE_GET_PHYS_LINK(
...
{
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  DS3GPP2_ASSERT(item_head_ptr != NULL && *item_head_ptr != NULL);
  DS3GPP2_ASSERT((device == PPP_RM_DEV) || (device == PPP_UM_SN_DEV));
  DS3GPP2_ASSERT((protocol == PPP_IPCP_PROTOCOL) || (protocol == PPP_LCP_PROTOCOL));
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  DS3GPP2_ASSERT(item_head_ptr != NULL && *item_head_ptr != NULL);
  DS3GPP2_ASSERT(device == PPP_RM_DEV);
  DS3GPP2_ASSERT((protocol == PPP_IPCP_PROTOCOL) || (protocol == PPP_LCP_PROTOCOL));
  DATA_MDM_ARG_NOT_CONST(meta_info_ref_ptr);
  DATA_MDM_ARG_NOT_CONST(item_head_ptr);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  DS3GPP2_ASSERT(ds707_rmsmi_info.state != DS707_RMSM_NULL_STATE);
  ...
  if ( NULL == PS_FLOW_GET_PHYS_LINK(default_flow_ptr) )
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, 
                     "Flow not bound to phys link. Binding it now");
    ps_flow_bind_phys_link(default_flow_ptr,
  ...
  if ( FALSE == ds3g_dds_switch_hdlr_is_call_allowed(this_iface_ptr,is_ims_call))
  {
    ...
    iface_down_ind_info.down_reason = PS_NET_DOWN_REASON_DDS_SWITCH_IN_PROGRESS; 
                                                 ...
                                                 DS_CRM_VAL_NETMODEL_MODE )
  {
    ...
    if ( route_add(ROUTE_DEFAULT,
         ...
         == -1 )
    {
      DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Default route addition failed");
    }
  }
...
)
{
  ...
  DS3GPP2_ASSERT(this_iface_ptr != NULL);
  ...
  if((ds707_rmsmi_info.state == DS707_RMSM_NULL_STATE) || 
     (NULL == phys_link_ptr))
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "No DUN call active");
    return 0;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS3GPP2_ASSERT( this_iface_ptr == ds707_rmsmi_info.IS707pkt_iface_ptr);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS3GPP2_ASSERT( ppp_dev == PPP_RM_DEV );
  switch(ppp_event)
  {
    /*-----------------------------------------------------------------------
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS3GPP2_ASSERT( ppp_dev == PPP_UM_SN_DEV );
  switch(ppp_event)
  {
    /*-----------------------------------------------------------------------
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Error reading tethered NAIs from NV");
    return;
  }
...
)
{
  ...
  {
#endif
    DATA_IS707_MSG0_EX( MSG_LEGACY_MED, subs_id, "RevA WaterMark used");
    /* REVA/REL0 protocol */
...
{
  uint8                          num_events;
  ps_iface_event_enum_type       events[4];
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if(iface_ptr == NULL)
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "iface ptr is NULL" );
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, 
                    "IFACE events reg failed - so returning FALSE" );      
    return FALSE;
  }
  /*-------------------------------------------------------------------
    Register for phys_link events.
    Phys Link UP registration is required for correct SIP falback
     - we donot want to fallback to SIP if we are not able to bring
     the TC IP
  -------------------------------------------------------------------*/
  if (ds707_rmsmi_info.um_phys_link_event_buf == NULL)
  {
  ...
  num_events = 4; 
  events[0] = IFACE_PHYS_LINK_UP_EV;
  events[1] = IFACE_PHYS_LINK_GONE_EV;
  events[2] = PHYS_LINK_FLOW_ENABLED_EV;
  events[3] = PHYS_LINK_FLOW_DISABLED_EV;
  ...
  if( -1 == ps_phys_link_subscribe_event_list(
                  ds707_rmsmi_info.um_phys_link_event_buf ,
                    (ps_iface_event_enum_type *)&events,
                     num_events))
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Phys_link FLOW_ENABLED_EV reg failed");
    /* CALL FAILED */
    return FALSE;
      ...
      ps_flow_subscribe_event_list(
                  ds707_rmsmi_info.um_flow_tx_event_buf_ptr,
                    (ps_iface_event_enum_type *)&events,
                     num_events))
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "FLOW_TX_EV reg failed" );
    /* CALL FAILED */
    return FALSE;
    ...
    ) 
  {
    ...
    if(ps_iface_errno == DS_ENETCLOSEINPROGRESS)
    {
      ...
      if (NULL == ds707_rmsmi_info.um_iface_event_buf_ptr)
      {
        ...
        {
          DS_3GPP2_ERROR_FATAL("Cannot allocate event buffer");
          return DS707_RMSM_BRINGUP_UM_INVALID;
        }
      }
    ...
    else if(ps_iface_errno == DS_EWOULDBLOCK)
    {
      ...
      {
        DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Tearing down up iface - "
...
LOCAL boolean ds707_rmsmi_dereg_ev(ps_iface_type* iface_ptr)
{
  ...
  if(iface_ptr == NULL)
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "iface_ptr is NULL");
  }
  if(ds707_rmsmi_info.um_iface_event_buf_ptr != NULL)
  {
    DATA_IS707_MSG2(MSG_LEGACY_HIGH, 
