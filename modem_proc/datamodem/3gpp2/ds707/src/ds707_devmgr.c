...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp2/ds707/src/ds707_devmgr.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
  
when       who     what, where, why
--------   ---     ------------------------------------------------------- 
02/07/17   fn      Fixes for ehrpd on sub 2 
05/29/15   sc      Added check in throttling mechanism  to prevent redundancy
                   for auth failures 
03/16/15   ms      Added throttling mechanism for LCP failure event 
                   with Auth failure reason.
12/15/14   sc      Added code to perform graceful PPP termination during hard
                   failure and then update eHRPD availability  
07/16/14   vm      Changes to ensure system info is cleared when device manager
...
/** Flag to indicate if DCTM was posted for AUTH failure or not. */
static boolean ds707_devmgr_dctm_auth_fail_posted;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /** Register PCMT */
  ds3g_timer_register(&ds707_devmgr_persist_cb.pcmt_hdl,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Device should be Um");
    return -1;
  }

  if (NULL == data_sess_profile_info_ptr)
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "data_sess_profile_info_ptr is NULL");
    return -1;
  }

   /** Set the variable to false for the next PPP session*/
   ds707_devmgr_dctm_auth_fail_posted = FALSE;
  ...
  switch (ds707_devmgr_cb.state)
  {
    case DEVMGR_CLOSED_STATE:
      ...
      {
        DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Device configure/reg failed");
    ...
    case DEVMGR_CLOSING_STATE:
      ...
      {
        DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "PPP device reconfigure failed");
        return -1;
      }
      ...
      {
        DS_3GPP2_ERROR_FATAL("Transition to restart state failed");
        return -1;
      }
      break;
    ...
    default:
      ...
      {
        DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Device not configured");
        return -1;
      }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(FALSE == ppp_is_device_configured(ds707_devmgr_cb.device))
  {
    ...
    {
      DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Device configure failed");
      return -1;
    }
    ...
    if (0 != ds707_devmgr_ppp_event_register())
    {
      ...
      {
        DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "ppp_device_reset() failed");
      }
      return -1;
    }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "eHRPD failed, do not proceed");
    return -1;
  }
#ifdef FEATURE_DATA_SEND_PPP_CTL_ON_DOS
  /*-----------------------------------------------------------------------
  ...
  /*-----------------------------------------------------------------------
    Store current subs ID.
  -----------------------------------------------------------------------*/
  ppp_dev_config.subs_id = ppp_get_subs_id( ds707_curr_subs_id() );
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Unable to acquire a PPP event handle");
    return -1;
  }
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Failed to register LCP events");
    return -1;
  }
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Failed to register AUTH events");
    return -1;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if(NULL == ds707_devmgr_cb.ppp_event_handle)
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Invalid PPP event handle");
    return FALSE;
  }
  ...
  if(0 != ppp_event_deregister(
                          ...
                          (PPP_DOWN_EV_MASK|PPP_RESYNC_EV_MASK)))
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Failed to de-register LCP events");
    return FALSE;
  }
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Failed to de-register AUTH events");
    return FALSE;
  }
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Failed to de-alloc PPP event handle");
    return FALSE;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Invalid this pointer");
    return FALSE;
  }
  ...
  switch (state)
  {
    ...
    case DEVMGR_LINGER_STATE:
      ...
      {
        DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "PCMT already running");
      }
      break;
                                            ...
                                            ds707_devmgr_cb.ps_phys_link_ptr)))
      {
        ...
        else
        {
          if( ds3gdsdif_is_voice_or_emergency_call_active_on_other_sub(
                  ds3gsubsmgr_subs_id_cm_to_ds3g(ds707_curr_subs_id())) == FALSE )
          {
    ...
    case DEVMGR_CLOSED_STATE:
      ...
      if (NULL != ds707_devmgr_cb.ps_phys_link_ptr)
      {
        ...
        if (0 != ret)
        {
          DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR,"Failed to release physical link %d"
                          , ret);
        }
      }
      ...
      {
        DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "ppp_device_unconfigure() failed");
      }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (0 != ds707_devmgr_device_configure())
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "ds707_devmgr_device_configure() failed");
    return FALSE;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "cmd_ptr is NULL");
    return;
  }
    ...
    case PPP_DOWN_EV:
      ...
      switch (ds707_devmgr_cb.state)
      {
        ...
        case DEVMGR_CLOSING_STATE:
          ...
          /** Post to DCTM if there is LCP timeout or option mismatch or 
              Auth failure during EAP Phase ex:Identity mismatch*/   
          fail_reason =  
               ((ds707_ppp_ev_cmd_type*)cmd_ptr->cmd_payload_ptr)->fail_reason;

          if( (fail_reason == PPP_FAIL_REASON_TIMEOUT) ||
              (fail_reason == PPP_FAIL_REASON_OPTION_MISMATCH) )
          {
            dctm_post_ppp_call_status(PPP_DOWN_EV,
                                      DSDCTM_PPP_PROTOCOL_LCP,
                                      fail_reason);
          }
          /** Handle auth failure only if it wasnt handled as part of 
              auth_event*/ 
          DATA_IS707_MSG1(MSG_LEGACY_HIGH, "ds707_devmgr_dctm_auth_fail_posted = %d", 
                          ds707_devmgr_dctm_auth_fail_posted);
          if( (ds707_devmgr_dctm_auth_fail_posted == FALSE) &&
              (fail_reason == PPP_FAIL_REASON_AUTH_FAILURE) )
          {
            dctm_post_ppp_call_status(PPP_DOWN_EV,
                                      DSDCTM_PPP_PROTOCOL_LCP,
                                      fail_reason);
            ds707_devmgr_dctm_auth_fail_posted = TRUE;
          }
  ...
  switch (((ds707_ppp_ev_cmd_type* ) cmd_ptr->cmd_payload_ptr)->ppp_event)
  {
    case PPP_SUCCESS_EV:
      ...
      switch (ds707_devmgr_cb.state)
      {
        ...
        case DEVMGR_CLOSED_STATE:
          DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Invalid devmgr state");
          return;
    ...
    case PPP_FAILURE_EV:
      ...
      ds707_devmgr_dctm_auth_fail_posted = TRUE;
      DATA_IS707_MSG0(MSG_LEGACY_MED,    
                      "Setting ds707_devmgr_dctm_auth_fail_posted flag true");
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (FALSE == ds3g_timer_get_status(DS3G_TIMER_PCM))
  {
    /** Convert the timeout value to msec */
    ds3g_timer_start(DS3G_TIMER_PCM, 
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  /** Stop the PCM timer */
  ds3g_timer_stop(DS3G_TIMER_PCM);
} /* ds707_devmgr_pcm_timer_stop() */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch ((ds3g_timer_enum_type)timer_id)
  {
    ...
    default:
      DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR, "Bad timer id on callback %d",
                      timer_id);
      return;
  }
