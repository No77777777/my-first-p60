...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp2/ds707/src/ds707_pkt_mgr_hdr.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
---------------------------------------------------------------------------*/
LOCAL boolean ds707_pkti_1x_colocated_with_evdo[DS3GSUBSMGR_SUBS_ID_MAX] = {FALSE};
...
/*lint -save -e774 suppressing error 774*/
       ...
       (call_clean_up == FALSE) )
  {
    ...
    *ppp_resync_ptr  = pkt_info.drs_bit;
    ...
    {
      DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR, "ps phys_link reorig failed %d",
                      err_num);
    }
  }
...
{
  ds_cmd_type                  *cmd_ptr;
  ds707_evdo_coloc_info_type   *coloc_info = NULL;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

  DATA_IS707_MSG2(MSG_LEGACY_MED, 
                  "Coloc info changed, new coloc info: %d for subs %d",
                  evdo_colocated, subs_id);

  cmd_ptr = ds_allocate_cmd_buf(sizeof(ds707_evdo_coloc_info_type));
  if((NULL == cmd_ptr) || (NULL == cmd_ptr->cmd_payload_ptr))
  {
    DS3GPP2_ASSERT(0);
    return;
  ...
  else
  {
    ...
    coloc_info = (ds707_evdo_coloc_info_type *)(cmd_ptr->cmd_payload_ptr);
    coloc_info->evdo_colocated = evdo_colocated;
    coloc_info->subs_id = ds3gsubsmgr_subs_id_cm_to_ds3g(subs_id);
...
(
  ds707_evdo_coloc_info_type   evdo_colocated_info   /* Holds the colocation info    */
)
{
  ds3gsubsmgr_subs_id_e_type  ds_subs_id = DS3GSUBSMGR_SUBS_ID_INVALID;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

  ds_subs_id = evdo_colocated_info.subs_id;
  if(ds3gsubsmgr_is_ds3g_subs_index_valid(ds_subs_id))
  {
    /*-------------------------------------------------------------------------
      Store colocation info in local variable, for later use
    -------------------------------------------------------------------------*/
    ds707_pkti_1x_colocated_with_evdo[ds_subs_id] = 
                                     evdo_colocated_info.evdo_colocated;
  }  
...
{
  ds3gsubsmgr_subs_id_e_type ds_subs = DS3GSUBSMGR_SUBS_ID_INVALID;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

  ds_subs = ds3gsubsmgr_subs_id_cm_to_ds3g(ds707_curr_subs_id());  
  if(ds3gsubsmgr_is_ds3g_subs_index_valid(ds_subs))
  {
    return(ds707_pkti_1x_colocated_with_evdo[ds_subs]);
  }
  else
  {
    return FALSE;
  ...
  if ( ds707_get_pkt_data_sess_state() == DS707_PKT_DATA_SESS_HDR_ACTIVE)
  {
    if (ds707_drs_is_in_mip_call())
    {
      ...
      if (ppp_resync( PPP_UM_SN_DEV,
                      ...
                      NULL) < 0)
      {
        DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Failed to resync PPP on ALN");
      }
    }
    return;
  }
    ...
    *ppp_resync_ptr  = pkt_info.drs_bit;
    ...
    else
    {
      ...
      if ( (ret_val < 0) &&
           (err_num != DS_EWOULDBLOCK))
      {
        DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR, "ps phys link reorig failed %d",
                        err_num);
      }
    }
  }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if(NULL == ds_cmd_ptr)
  {
    DS_3GPP2_ERROR_FATAL("Invalid ds_cmd_ptr passed");
    return;
  }
  ...
  switch (ds_cmd_ptr->hdr.cmd_id)
  {
    ...
    case DS_CMD_707_COLOC_INFO_CHANGED:
      ...
      ds707_pkti_coloc_info_changed_cmd_hdlr(
                *((ds707_evdo_coloc_info_type *)(ds_cmd_ptr->cmd_payload_ptr)));
      break;
#endif //FEATURE_DS_CAM
