...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp2/ds707/src/ds707_epzid_mgr.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
  ...
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
   ...
   -----------------------------------------------------------------------*/
    ...
    -----------------------------------------------------------------------*/
      ...
      {
        DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "EPZID list should be empty. reorig should be true");
      }
    }
  }
  else
...
/*-------------------------------------------------------------------------
  ...
  /* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
    ...
    {
      DS_3GPP2_ERROR_FATAL("phys_link_ptr is NULL");
      return;
    }
    ...
    *ppp_resync_ptr  = pkt_info.drs_bit;
    ...
    {
      DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR, "ps_iface reorig failed %d", err_num);
    }
    else
    {
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
    ...
    {
	    DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR, "Invalid pointer passed: %x", new_epzid_ptr);
      DS3GPP2_ASSERT(0);
      return;
    }
    /*-------------------------------------------------------------
    ...
    switch(ds707_epzid_get_active_epzid_type())
    {
      ...
      default:
	    DS_3GPP2_ERROR_FATAL("Invalid EPZID Type");
	    break;
    }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_3GPP2_ERROR_FATAL("Can't register for phys link coming up event");
  }
  ...
  {
    DS_3GPP2_ERROR_FATAL("Can't register for phys link up event");
  }
  ...
  {
    DS_3GPP2_ERROR_FATAL("Can't register for phys link down event");
  }
  ...
  {
    DS_3GPP2_ERROR_FATAL("Can't register for abort session event");
  }
 ...
 /*-----------------------------------------------------------------
     ...
     !ds707_roaming_is_simplified_jcdma_mode())
  {
    ...
    {
      DS_3GPP2_ERROR_FATAL("Can't register for holddown timer event");
    }
  }
  else
  {
  ...
  {
    DS_3GPP2_ERROR_FATAL("Can't register for sid changed event");
  }
  ...
  {
    DS_3GPP2_ERROR_FATAL("Could not reg NEW_NET_EV cback");
  }
  ds707_epzid_init_list();
} /* ds707_epzid_mgr_init() */

...
LOCAL void ds707_epzid_exit_tc_hdlr(ds_cmd_type *epzid_cmd_ptr)
{
     ...
     !ds707_roaming_is_simplified_jcdma_mode())
  {
    ...
    {
      DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Serious ctrl flow error func only for DS_707_EXIT_TC_CMD");
      return;
    }
