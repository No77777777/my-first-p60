...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp2/ds707/src/ds707_pkt_mgr_phys_link.c#2 $ 
  $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $

when        who    what, where, why
--------    ---    ----------------------------------------------------------
04/01/16    sd     Added dial string validation utility function in DS3G.
...
#ifdef FEATURE_DATA_FLOW_MGMT
/*===========================================================================
FUNCTION      DS707_PKTI_PHYS_LINK_DISABLE_FLOW

DESCRIPTION   This internal API handle PS Iface IOCTL to disable flow on all 
              associated physlinks by calling bursty flow control module
...
SIDE EFFECTS  None
===========================================================================*/
static void ds707_pkti_phys_link_disable_flow
(
  void             *payload_ptr
);

/*===========================================================================
FUNCTION      DS707_PKTI_PHYS_LINK_ENABLE_FLOW

DESCRIPTION   This internal API handle PS Iface IOCTL to enable flow on all 
              associated physlinks by calling bursty flow control module
...
SIDE EFFECTS  None
===========================================================================*/
static void ds707_pkti_phys_link_enable_flow
(
  void             *payload_ptr
);
#endif /* FEATURE_DATA_FLOW_MGMT */
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
   ...
   */
  DS3GPP2_ASSERT(phys_link_ptr == this_phys_link_ptr);
...
#endif /* FEATURE_DATA_OPTHO */
  {
    ...
    if (ds3g_msh_hangup_call_ex((ds707_get_pri_tc_ptr())->call_id,
                             &mode_spec_info,
                             ds707_curr_subs_id()) == DS3G_FAILURE)
    {
      /*-----------------------------------------------------------------------
...
{
  ds3g_dial_val_e_type         dial_str_digits_type;
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  resolved_subs_id = ds707_curr_subs_id();
 ...
 /* This is PRI physlink, put all the SEC physlinks in transient state */
  ...
  else if (DS707_S101_SM_NON_TUNNEL == s101_sm_state)
  {
       ...
       )
    {
      ...
      DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "S101 SM in Non Tunnel state "
...
#endif /* FEATURE_DATA_OPTHO */
  {
    ...
    ds3gflowmgr_notify_ps_call_state_per_sub ( ds707_curr_subs_id(), 
    ...
    dial_str_digits_type = ds3g_proc_dial_str(ds707_pkt_mgr_orig_str,
                                              dial_str,
                                              DS3G_NON_ETSI_CMD_MODE);
    ...
    if ( (dial_str_digits_type == DS3G_DIAL_DIGIT) || 
         (dial_str_digits_type == DS3G_DIAL_EMPTY))
    {
   ...
   ----------------------------------------------------------------------------*/
    ...
    {
      DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Could not originate pkt call");
      ds707_pkti_post_phys_link_down_ev_during_bringup(&event_info);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch(ds_cmd_ptr->hdr.cmd_id)
  {
    ...
    case DS_CMD_707_PHYS_LINK_DISABLE_FLOW:
      ds707_pkti_phys_link_disable_flow(ds_cmd_ptr->cmd_payload_ptr);
      break;

    case DS_CMD_707_PHYS_LINK_ENABLE_FLOW:
      ds707_pkti_phys_link_enable_flow(ds_cmd_ptr->cmd_payload_ptr);
      break;
#endif /* FEATURE_DATA_FLOW_MGMT */

    default:
      DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR,"Unknown cmd %d", 
...
{
  DS3GPP2_ASSERT(phys_link_ptr != 0);
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  {
    DS_3GPP2_ERROR_FATAL("Could not create phys links");
  }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  flow_ctl_info_ptr = (ds707_pkt_phys_link_flow_ctl_s_type *)payload_ptr;
  
  burst_params_ptr = ds707_wmk_get_burst_params_ptr();

  if ((NULL == flow_ctl_info_ptr) || (NULL == burst_params_ptr))
  {
    return;
  }

#if 0
  ds3g_burst_disable_flow( flow_ctl_info_ptr->phys_link_ptr, 
                           (void *)ds707_wmk_get_pri_wm_ptr(),
                           flow_ctl_info_ptr->flow_ctl_info.flow_mask,
                           &burst_params_ptr->burst_params);
#endif

}/* ds707_pkti_phys_link_disable_flow */
...
#if 0   
  ds3g_burst_enable_flow( flow_ctl_info_ptr->phys_link_ptr,
                          ds707_wmk_get_pri_wm_ptr(),
                          flow_ctl_info_ptr->flow_ctl_info.flow_mask, 
                          &burst_params_ptr->burst_params);
#endif

}/* ds707_pkti_phys_link_enable_flow */
