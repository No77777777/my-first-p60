...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp2/ds707/src/ds_3gpp2_vendor.c#2 $
...
)
{
  ...
  if (kind == VERSION_CAPABILITY_IND_KIND)            
  {
    ...
    {    
      DS_IS707_ERROR_LOG_2(MSG_LEGACY_ERROR, "dsm_pushdown_packed error while filling in "
  ...
  switch (kind) 
  {
    ...
    case VERSION_CAPABILITY_REPLY_KIND:
      ...
      {
        DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "NULL pkt_info_ptr passed for VC REPLY KIND packet");
        return;
      }
    ...
    case MAX_PPP_INACTIVITY_TIMER_REPLY_KIND:
      if (NULL == pkt_info_ptr)
      {
        DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "NULL pkt_info_ptr passed for MPIT packet");
        return;
      }
    ...
    case VENDOR_SPECIFIC_REJECT_KIND:
      ...
      {
        DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "NULL pkt_info_ptr or received_pkt_ptr_ptr"
    ...
    default:
      DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR, "TX: Invalid Kind! = %d", kind);
      break;
  } /* kind */
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "ppp_vendor_send failed");
    return;
  }
...
)
{
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Null arguments to "
  ...
  {
    DS_IS707_ERROR_LOG_3(MSG_LEGACY_ERROR, "Unrecognized OUI (0x%x) received, "
                                       "dev %d, proto 0x%x", 
                                       device, protocol, oui);
    dsm_free_packet(&pkt_ptr);
    return;
    ...
    case PPP_LCP_PROTOCOL:   
      ...
      switch( kind )
      {
        ...
        case VERSION_CAPABILITY_IND_KIND:     
          ...
          {
            DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR, "Unexpected PDSN version %d",
                            pdsn_version);
            break;
          }
        ...
        default:
          DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "RX: Unexpected VS packet!");
          /*-----------------------------------------------------------------
            Send VCI reject.
          ------------------------------------------------------------------*/
          ds_3gpp2_send_vendor_packet( device, 
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Unexpected callback, ignoring!");
    return;
  }
...
{  
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  DS3GPP2_ASSERT (device == PPP_UM_SN_DEV);
  DS3GPP2_ASSERT (protocol == PPP_PROTOCOL_LCP);  
  ...
  switch(ppp_event)
  {
    ...
    case PPP_RESYNC_EV:  
      ...
      ---------------------------------------------------------------------*/
      ds3g_timer_stop( DS3G_TIMER_MPIT );
      ...
      if( always_on_cb_ptr->config.is_enabled )
      {
        ...
        ds3g_timer_stop( DS3G_TIMER_LSD );
...
)
{
  ...
  ds3g_timer_register(&(ds_3gpp2_mpit_cb_ptr->mpit_timer),
                       ds_3gpp2_mpit_timeout_cb,
                       DS3G_TIMER_MPIT);
   ...
   ------------------------------------------------------------------------*/
   ds3g_timer_register(&always_on_cb_ptr->lsd_timer,
                        ds_3gpp2_lsd_timeout_cb,
                        DS3G_TIMER_LSD);
#endif /* FEATURE_DATA_3GPP2_ALWAYS_ON */
  return 0;
...
)
{
  ...
  if( always_on_cb_ptr->config.is_enabled && 
      !ds_3gpp2_mpit_cb_ptr->is_enabled )
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Always On enabled without MPIT!");
    return -1;
  }
  ...
  if( ds_3gpp2_mpit_cb_ptr->is_enabled )
  {
    /*-----------------------------------------------------------------------
     ...
     {
       DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Failed to register for 3GPP2 OUI");
       return -1;
     }
     ...
     if (ds_3gpp2_vendor_cb.ppp_event_handle == NULL)
     {
       ...
       --------------------------------------------------------------------*/
       DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Failed to register for PPP events!");
       if(-1 == ppp_vendor_deregister( PPP_UM_SN_DEV, 
                                       VENDOR_SPECIFIC_3GPP2_OUI ) )
       {
         DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Cleanup failed: Unable to "
                                   ...
                                   event_mask ) )
     {
       ...
       if(-1 == ppp_event_dealloc( PPP_UM_SN_DEV,
                                   &(ds_3gpp2_vendor_cb.ppp_event_handle) ) )
       {
         DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Unable to dealloc PPP event!");
       }
       if(-1 == ppp_vendor_deregister( PPP_UM_SN_DEV, 
                                       VENDOR_SPECIFIC_3GPP2_OUI ) )
       {
         DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Unable to deregister 3gpp2 OUI!");
       }
       return -1;
...
int ds_3gpp2_mpit_timer_start(void)
{
  ...
  if ( (TRUE  == ds_3gpp2_vendor_cb.mpit_cb.is_enabled) &&
       ...
       (FALSE == ds3g_timer_get_status( DS3G_TIMER_MPIT )) )
  {
    ds3g_timer_start( DS3G_TIMER_MPIT,
    ...
    {
      ds3g_timer_start( DS3G_TIMER_LSD,
...
{
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if ( TRUE == ds3g_timer_get_status( DS3G_TIMER_MPIT ) )
  {
    ds3g_timer_start( DS3G_TIMER_MPIT,
                       SEC_TO_MSEC(ds_3gpp2_vendor_cb.mpit_cb.timeout) );
#ifdef FEATURE_DATA_3GPP2_ALWAYS_ON 
...
int ds_3gpp2_mpit_timer_stop(void)
{
  ...
  if ( (TRUE  == ds_3gpp2_vendor_cb.mpit_cb.is_enabled) &&
       (0     != ds_3gpp2_vendor_cb.mpit_cb.timeout) &&
       (TRUE == ds3g_timer_get_status( DS3G_TIMER_MPIT )) )
  {
    ...
    if( ( always_on_cb_ptr->config.is_enabled ) &&
        ( always_on_cb_ptr->config.lsd.is_enabled ) &&
        ( 0 != always_on_cb_ptr->lsd_timeout ) &&
        (TRUE == ds3g_timer_get_status( DS3G_TIMER_LSD )) )
    {
...
)
{
  ...
  if( always_on_cb_ptr->is_vci_done )
  {
    ...
    if( 0 != ppp_trigger_lsd( PPP_UM_SN_DEV, &lsd_config ) )
    {
      ...
      ---------------------------------------------------------------------*/
      DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Link Status Determination failed!");
    }
