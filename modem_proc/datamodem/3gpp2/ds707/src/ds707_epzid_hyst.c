...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp2/ds707/src/ds707_epzid_hyst.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
#define EPZIDI_START_TIMER( duration )                                     \
           ...
           {                                                               \
              ds3g_timer_start( DS3G_TIMER_EPZID_HYSTERESIS,  duration ); \
...
#define EPZIDI_STOP_TIMER()                                                \
                          ...
                          {                                                \
                            ds3g_timer_stop( DS3G_TIMER_EPZID_HYSTERESIS);\
 ...
 /*-----------------------------------------------------------------------*/
  ...
  switch(epzidi_hys_state)
  {
     ...
     case EPZIDI_HYS_HT_STATE:
       ...
       break;
    default:
      DS_3GPP2_ERROR_FATAL("Invalid state");
      break;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_3GPP2_ERROR_FATAL("Invalid event_info_ptr passed");
    return;
  }
  ...
  switch(event)
  {
    ...
    default:
      DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR,"Unknown event %d",event);
      DS3GPP2_ASSERT(0);
      break;
  }
...
)
{
  ...
  switch(epzidi_hys_state)
  {
    ...
    case EPZIDI_HYS_HAT_STATE:
      DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR, "Hysteresis Enable -> %d state", epzidi_hys_state);
      break;       
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch(ds_cmd_ptr->hdr.cmd_id)
  {
    ...
    default:
      DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR, "unknown PZID command %d",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR, "Invalid RLP INSTANCE: %d", rlp_instance);
    DS3GPP2_ASSERT(0);
    return;
  }
...
)
{
 ...
 /*------------------------------------------------------------------------*/  
  ...
  {
    DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR, "EPZID List Empty, max_list_len = %d", 
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
   DS_IS707_ERROR_LOG_3(MSG_LEGACY_ERROR,"Invalid EPZID passed: Sid = %d NID = %D, PZID = %d", curr_epzid.sid,curr_epzid.nid,curr_epzid.pzid);
   return FALSE;
  }
   /*-----------------------------------------------------------------------
