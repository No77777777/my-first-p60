...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp2/ds707/src/ds707_data_session_profile.c#3 $ $DateTime: 2019/12/19 04:08:47 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
04/01/16   sd      Fix to read profile from NVRUIM only after subscription 
                   ready event.
07/14/15   sd      Fix to ensure PDN compatibilty check for eHRPD profiles in 
                   eHRPD-OOS-eHRPD scenario.
07/02/15   sd      Pick up non null APN profile for ALWAYS_SHARE_PROFILE
01/27/15   sd      Fixed MIP to SIP fallback handling for OMH profiles in
                   eHRPD mode.
12/18/14   vm      Enter emergency mode on mandatory APN check failure only if
...
/*-------------------------------------------------------------------------- 
 ...
 -------------------------------------------------------------------------*/
static uint8         ds707_data_session_ehrpd_valid_profile
                                     [DS707_DATA_SESS_MAX_SUPPORTED_SUBS] = {0}; 
...
/*===========================================================================
FUNCTION DS707_DATA_SESS_GET_SYS_SUBSID

DESCRIPTION
  This function fetches the profile info ptr based on the subs_id.
DEPENDENCIES
  None
...
sys_modem_as_id_e_type ds707_data_sess_get_sys_subsid
(
  ds_profile_subs_id_enum_type  subs_id 
)
{
  sys_modem_as_id_e_type profile_sys_subsid = SYS_MODEM_AS_ID_NONE;
 /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

  switch (subs_id)
  {
  case ACTIVE_SUBSCRIPTION_1:
    profile_sys_subsid = SYS_MODEM_AS_ID_1;
    break;

  case ACTIVE_SUBSCRIPTION_2:
	  profile_sys_subsid = SYS_MODEM_AS_ID_2;
    break;

#ifdef FEATURE_TRIPLE_SIM
  case ACTIVE_SUBSCRIPTION_3:
	  profile_sys_subsid = SYS_MODEM_AS_ID_3;
    break;
#endif
  default:
    profile_sys_subsid = SYS_MODEM_AS_ID_1;
  ...
  DATA_IS707_MSG1(MSG_LEGACY_LOW,
                   "ds707_data_sess_get_sys_subsid: %d", profile_sys_subsid);
  return profile_sys_subsid;
}

/*===========================================================================
FUNCTION DS707_DATA_SESS_GET_PROFILE_INFO
...
ds707_data_session_profile_info_type** ds707_data_sess_get_profile_info
(
  ds_profile_subs_id_enum_type  subs_id 
)
{
 ...
 /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  return ds707_data_session_profile_ptr;
}


/*===========================================================================
FUNCTION DS707_DATA_SESS_GET_PROFILE_INFO_SYS
...
ds707_data_session_profile_info_type** ds707_data_sess_get_profile_info_sys
(
  void
)
{
...
static void ds707_data_sess_profile_alloc(void)
{
  ...
  ds_profile_subs_id_enum_type           pf_subs = ACTIVE_SUBSCRIPTION_1;
  ds707_data_session_profile_info_type **ds707_data_session_profile_ptr = NULL;
  /*-----------------------------------------------------------------------*/
 
  /*-----------------------------------------------------------------------
    Calculate the number of mandatory profiles required to be allocated 
    for each subscription. This will only include the default and JCDMA
    related subs id. NVRUIM and the persistent profiles in the EFS will
    be allocated when they are read.
  ...
  /*-----------------------------------------------------------------------
    On POWER UP/NV REFRESH allocate the profile pointers for all the 
    available subscriptions with the mandatory profiles. This will ensure 
    that user is able to create/modify/delete profiles on any subscription
    even if it is not the current 1x subscription.
  -----------------------------------------------------------------------*/
  
  for(pf_subs = ACTIVE_SUBSCRIPTION_1; pf_subs < ACTIVE_SUBSCRIPTION_MAX; 
      pf_subs++)
  {
    ds707_data_session_profile_ptr = ds707_data_sess_get_profile_info(pf_subs);
    if(ds707_data_session_profile_ptr == NULL)
    {
      continue;
    }
    
    subs_id = ds707_data_sess_get_sys_subsid(pf_subs);
    if(DS707_REFRESH_TYPE_NV == ds707_get_refresh_type() &&
       subs_id != ds707_get_nv_refresh_subs_id())
    {
      continue;
    }
    
    ds707_data_session_max_profiles[subs_id] = 
                                      ds707_data_session_max_profiles_temp;
    
    DATA_IS707_MSG2 (MSG_LEGACY_HIGH, 
                     "ds707_data_session_max_profiles[%d] = %d",
                     subs_id,
                     ds707_data_session_max_profiles[subs_id]);
    ...
    if ( *ds707_data_session_profile_ptr != NULL )
    {
      modem_mem_free(*ds707_data_session_profile_ptr,MODEM_MEM_CLIENT_DATA_CRIT);
    ...
    *ds707_data_session_profile_ptr = 
    ...
    if(NULL == *ds707_data_session_profile_ptr )
    {
      DS_LEAVE_CRIT_SECTION(&ds707_profile_crit_sect);
      DS_3GPP2_ERROR_FATAL("profile memory allocation failed");
      return;
    }
    
    memset((void *)(*ds707_data_session_profile_ptr), 0, 
           (ds707_data_session_max_profiles[subs_id] * 
              sizeof(ds707_data_session_profile_info_type)) );
    
    ds707_data_session_ehrpd_valid_profile[subs_id] = 0;
  }
        ...
        )
{
  ...
  if(NULL == apn_name || 0 == apn_len || NULL == label_pptr || FALSE == ds707_pkt_is_subs_id_valid(subs_id) )
  {
  ...
  ds707_data_session_profile_ptr = *(ds707_data_sess_get_profile_info_sys());
  
  for ( profile_reg_index = DATA_SESSION_PROFILE_ID_MIN;
         profile_reg_index < ds707_data_session_max_profiles[subs_id];
         profile_reg_index++)
  {
    if ( apn_len == ds707_data_session_profile_ptr[profile_reg_index].apn_string_len )
    {
      if(0 == strncasecmp( ds707_data_session_profile_ptr[profile_reg_index].apn_string,
                  apn_name, apn_len ))
      {
	    if(ds707_data_session_profile_ptr[profile_reg_index].pdn_label_len != 0)
		{		
          *label_pptr = 
            ds707_data_session_profile_ptr[profile_reg_index].pdn_label;
          return ds707_data_session_profile_ptr[profile_reg_index].pdn_label_len;
		}
      }
    }
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ( ( policy_info_ptr->ip_family == IFACE_UNSPEC_ADDR_FAMILY ) ||
       ...
       ( ds707_data_session_profile_ptr[profile_reg_index].pdn_type ==
  ...
  else
  {
    DATA_IS707_MSG4( MSG_LEGACY_LOW,
                   ...
                   ds707_data_session_profile_ptr[profile_reg_index].data_session_profile_id,
                   ds707_data_session_profile_ptr[profile_reg_index].pdn_type,
...
)
{
  ...
  if( ( ds707_data_session_profile_ptr[profile_reg_index].pdn_type ==
        DS707_DATA_SESS_PDN_TYPE_V4_V6 ) ||
      ( ds707_data_session_profile_ptr[profile_reg_index].pdn_type ==
        DS707_DATA_SESS_PDN_TYPE_V4 &&
...
)
{
 ...
 /*------------------------------------------------------------------------*/
  ...
  if  ( from > to )
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR,"Invalid Parameters ");
    return ret_val;
  }
    ...
    case PDN_IP_VERSION_TYPE:
      {
        ...
        {
          DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR,"Undefined PDN Type");
        }
        break;
      }
  ...
  )
{
  ...
  if (profile_id_value < 1000)
  {
    ...
    efs_file_name[39] = profile_id[0];
    efs_file_name[40] = profile_id[1];
    efs_file_name[41] = profile_id[2];
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Profile id value is greater than 1000");
  }
    ...
    else if(DS3GCFGMGR_EFS_TOKEN_PARSE_SUCCESS == ret_val)
    {
      ...
      if (param_name == TRUE)
      {
        ...
        if (ds707_data_sess_get_token_id(from,to,&token_id) < 0)
        {
          ...
          DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Incorrect Param Name" );
        }
        else
        {
...
/*===========================================================================
FUNCTION      ds707_data_session_profile_set_ehrpd_params_ex
...
SIDE EFFECTS  None
===========================================================================*/
void ds707_data_session_profile_set_ehrpd_params_ex
(
  ds707_data_session_profile_id profile_reg_index,
  char *apn_string,
  uint8 apn_string_len,
...
)
{
  uint8 idx = 0;
  ds707_data_session_profile_info_type *ds707_data_session_profile_ptr = NULL;
  ds_profile_subs_id_enum_type pf_subs = 
                              ds707_data_session_get_profile_subs_id(subs_id);

  if( !ds707_pkt_is_subs_id_valid(subs_id) ||
      profile_reg_index < DATA_SESSION_PROFILE_ID_MIN ||
      profile_reg_index >= ds707_data_session_max_profiles[subs_id] )
  {
    DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR,
                     "Incorrect sub_id or profile reg %d value.", 
                     profile_reg_index );
    return;
  }

  if(ds707_data_sess_get_profile_info(pf_subs) != NULL)
  {
    ds707_data_session_profile_ptr = 
          *(ds707_data_sess_get_profile_info(pf_subs));
  }
  if(ds707_data_session_profile_ptr == NULL)
  {
    DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR,
                     "Incorrect ds707_data_session_profile_ptr %d value.", 
                     ds707_data_session_profile_ptr );
    return;
  }
  ...
  {
    ds707_data_session_profile_ptr[profile_reg_index].apn_string_len =
    apn_string_len;

    (void)memscpy(ds707_data_session_profile_ptr[profile_reg_index].apn_string,
                  DS_VSNCP_3GPP2_APN_MAX_VAL_LEN,
                  apn_string,
                  apn_string_len);
  }
  else
  {
    DATA_IS707_MSG2(MSG_LEGACY_HIGH,
        ...
        apn_string_len,
        profile_reg_index);
    ds707_data_session_profile_ptr[profile_reg_index].apn_string_len = 0;
  }

  if ((pdn_label_len > 0) && (pdn_label_len <= DS_VSNCP_3GPP2_APN_MAX_VAL_LEN))
  {
    ds707_data_session_profile_ptr[profile_reg_index].pdn_label_len =
    pdn_label_len;

    (void)memscpy(ds707_data_session_profile_ptr[profile_reg_index].pdn_label,
                  DS_VSNCP_3GPP2_APN_MAX_VAL_LEN,
                  pdn_label,
                  pdn_label_len);
  }
  else
  {
    DATA_IS707_MSG2(MSG_LEGACY_HIGH,
        ...
        pdn_label_len,
        profile_reg_index);
    ds707_data_session_profile_ptr[profile_reg_index].pdn_label_len = 0;
  }

  ds707_data_session_profile_ptr[profile_reg_index].op_pco_id = op_pco_id;
  ds707_data_session_profile_ptr[profile_reg_index].mcc = mcc;
  ds707_data_session_profile_ptr[profile_reg_index].mnc.mnc = mnc;
  ds707_data_session_profile_ptr[profile_reg_index].mnc.mnc_includes_pcs_digit = include_mnc_pcs_digit;


  ds707_data_session_profile_ptr[profile_reg_index].apn_flag = apn_enabled;

  ds707_data_session_profile_ptr[profile_reg_index].pdn_type = pdn_type;
  ds707_data_session_profile_ptr[profile_reg_index].is_pcscf_addr_needed =
    is_pcscf_addr_needed;


  ds707_data_session_profile_ptr[profile_reg_index].v4_dns_addr[PRIMARY_DNS_ADDR].ps_s_addr = 0;
  ds707_data_session_profile_ptr[profile_reg_index].v4_dns_addr[SECONDARY_DNS_ADDR].ps_s_addr = 0;

  ds707_data_session_profile_ptr[profile_reg_index].v6_dns_addr[PRIMARY_DNS_ADDR].ps_s6_addr64[0] = 0;
  ds707_data_session_profile_ptr[profile_reg_index].v6_dns_addr[PRIMARY_DNS_ADDR].ps_s6_addr64[1] = 0;

  ds707_data_session_profile_ptr[profile_reg_index].v6_dns_addr[SECONDARY_DNS_ADDR].ps_s6_addr64[0] = 0;
  ds707_data_session_profile_ptr[profile_reg_index].v6_dns_addr[SECONDARY_DNS_ADDR].ps_s6_addr64[1] = 0;

  //ds707_data_session_profile_ptr[profile_reg_index].v4_dns_addr[PRIMARY_DNS_ADDR].s_addr =
   // v4_pri_dns_addr.s_addr;
  //ds707_data_session_profile_ptr[profile_reg_index].v4_dns_addr[SECENDARY_DNS_ADDR].s_addr =
   // v4_sec_dns_addr.s_addr;

  //ds707_data_session_profile_ptr[profile_reg_index].v6_dns_addr[PRIMARY_DNS_ADDR].s6_addr64[0] =
    //v6_pri_dns_addr.s6_addr64[0];
  //ds707_data_session_profile_ptr[profile_reg_index].v6_dns_addr[PRIMARY_DNS_ADDR].s6_addr64[1] =
   // v6_pri_dns_addr.s6_addr64[1];

  //ds707_data_session_profile_ptr[profile_reg_index].v6_dns_addr[SECENDORY_DNS_ADDR].s6_addr64[0] =
    //v6_sec_dns_addr.s6_addr64[0];
  //ds707_data_session_profile_ptr[profile_reg_index].v6_dns_addr[SECENDORY_DNS_ADDR].s6_addr64[1] =
   // v6_sec_dns_addr.s6_addr64[1];

  ds707_data_session_profile_ptr[profile_reg_index].rat_type = rat_type;
  ds707_data_session_profile_ptr[profile_reg_index].pdn_inactivity_timeout = 0;

  for(idx = 0; idx < PDN_THROTTLE_MAX_TIMERS; idx++)
  {
    ds707_data_session_profile_ptr[profile_reg_index].failure_timer[idx] = failure_timer_vals[idx];
    ds707_data_session_profile_ptr[profile_reg_index].disallow_timer[idx] = disallow_timer_vals[idx];
  }

  ds707_data_session_profile_ptr[profile_reg_index].apn_class = 0;

  ds707_data_session_profile_ptr[profile_reg_index].pdn_level_auth_protocol = 
        pdn_level_auth_protocol;

  if ((pdn_level_user_id_len > 0) && 
      (pdn_level_user_id_len <= PPP_MAX_USER_ID_LEN))
  {
    ds707_data_session_profile_ptr[profile_reg_index].pdn_level_user_id_len =
          pdn_level_user_id_len;

    (void)memscpy(ds707_data_session_profile_ptr[profile_reg_index].pdn_level_user_id,
                  PPP_MAX_USER_ID_LEN,
                  pdn_level_user_id,
                  pdn_level_user_id_len);
  }
  else
  {
    DATA_IS707_MSG2(MSG_LEGACY_HIGH,
        ...
        pdn_level_user_id_len,
        profile_reg_index);
    ds707_data_session_profile_ptr[profile_reg_index].pdn_level_user_id_len = 0;
  }

  if ((pdn_level_auth_password_len > 0) && 
      (pdn_level_auth_password_len <= PPP_MAX_PASSWD_LEN))
  {
    ds707_data_session_profile_ptr[profile_reg_index].pdn_level_auth_password_len =
          pdn_level_auth_password_len;

    (void)memscpy(ds707_data_session_profile_ptr[profile_reg_index].pdn_level_auth_password,
                  PPP_MAX_PASSWD_LEN,
                  pdn_level_auth_password,
                  pdn_level_auth_password_len);
  }
  else
  {
    DATA_IS707_MSG2(MSG_LEGACY_HIGH,
        ...
        pdn_level_auth_password_len,
        profile_reg_index);
    ds707_data_session_profile_ptr[profile_reg_index].pdn_level_auth_password_len = 0;
  }
...
{
  sys_modem_as_id_e_type  subs_id = ds707_curr_subs_id();

  ds707_data_session_profile_set_ehrpd_params_ex( profile_reg_index,
                                                  apn_string,
                                                  apn_string_len,
                                                  pdn_label,
                                                  pdn_label_len,
                                                  ...
                                                  mcc,
                                                  mnc,
                                                  include_mnc_pcs_digit,
                                                  subs_id );
}

/*===========================================================================
FUNCTION      ds707_data_session_profile_set_common_params_ex
...
===========================================================================*/
void ds707_data_session_profile_set_common_params_ex
(
  ds707_data_session_profile_id profile_reg_index,
  ds707_data_session_profile_id profile_id,
...
)
{
  ...
  ds_profile_subs_id_enum_type pf_subs = 
                              ds707_data_session_get_profile_subs_id(subs_id);
  if(pf_subs == ACTIVE_SUBSCRIPTION_MAX)
  {
    return;
  }

  if(TRUE != ds707_pkt_is_subs_id_valid(subs_id))
  {
    return;
  }
  ds707_data_session_profile_ptr = 
  ...
  ---------------------------------------------------------------------------*/
  ds707_data_session_profile_ptr[profile_reg_index].negotiate_dns_server
    = negotiate_dns_server;
  /*---------------------------------------------------------------------------
    set session close timer (i.e. go null timer) for DO and 1X
  ---------------------------------------------------------------------------*/
  ds707_data_session_profile_ptr[profile_reg_index].ppp_session_close_timer_DO
    = ppp_session_close_timer_DO;
  ds707_data_session_profile_ptr[profile_reg_index].ppp_session_close_timer_1X
    = ppp_session_close_timer_1X;
  /*---------------------------------------------------------------------------
    set Linger option (i.e. whether to allow Dormant 2 / Linger state or not)
  ---------------------------------------------------------------------------*/
  ds707_data_session_profile_ptr[profile_reg_index].allow_linger
    = allow_linger;
  /*---------------------------------------------------------------------------
    set PPP lcp and ipcp options
  ---------------------------------------------------------------------------*/
  ds707_data_session_profile_ptr[profile_reg_index].lcp_ack_timeout
    = (uint16)(lcp_ack_timeout);
  ds707_data_session_profile_ptr[profile_reg_index].ipcp_ack_timeout
    = (uint16)(ipcp_ack_timeout);
  ds707_data_session_profile_ptr[profile_reg_index].auth_timeout
    = (uint16)(auth_timeout);
  ds707_data_session_profile_ptr[profile_reg_index].lcp_creq_retry_count
    = lcp_creq_retry_count;
  ds707_data_session_profile_ptr[profile_reg_index].ipcp_creq_retry_count
    = ipcp_creq_retry_count;
  ds707_data_session_profile_ptr[profile_reg_index].auth_retry_count
    = auth_retry_count;
  ds707_data_session_profile_ptr[profile_reg_index].auth_protocol
    = auth_protocol;
  /*---------------------------------------------------------------------------
    set ppp user_id and password lengths as 0
  ---------------------------------------------------------------------------*/
  ds707_data_session_profile_ptr[profile_reg_index].user_id_len = 0;
  ds707_data_session_profile_ptr[profile_reg_index].auth_password_len = 0;
  /*---------------------------------------------------------------------------
    set phys link data rate and data mode options
  ---------------------------------------------------------------------------*/
  ds707_data_session_profile_ptr[profile_reg_index].data_rate
    = data_rate;
  ds707_data_session_profile_ptr[profile_reg_index].data_mode
    = data_mode;
  /*---------------------------------------------------------------------------
   set default values of stateless DHCP parameters
  ---------------------------------------------------------------------------*/
  ds707_data_session_profile_ptr[profile_reg_index].dns_addr_using_dhcp 
    = FALSE;
  ds707_data_session_profile_ptr[profile_reg_index].pcscf_addr_using_dhcp
    = FALSE; 

  /*---------------------------------------------------------------------------
   set default value of APN Type mask
  ---------------------------------------------------------------------------*/
  ds707_data_session_profile_ptr[profile_reg_index].apn_type_mask= 0;
...
)
{
  ...
  ds707_data_session_profile_set_common_params_ex(profile_reg_index,
                                                  profile_id,
                                                  negotiate_dns_server,
                                                  ppp_session_close_timer_DO,
                                                  ppp_session_close_timer_1X,
...
/*-----------------------------------------------------------------------*/
  ...
  {
    DS_3GPP2_ERROR_FATAL("NULL ppp_params_ptr");
  }
       ...
       profile_id++)
  {
    ...
    if (profile_reg_index > DATA_SESSION_PROFILE_ID_MIN)
    {
      ...
      ds707_data_session_profile_ptr[profile_reg_index].profile_type =
        (ds_profile_type_enum_type)( (uint32)PROFILE_COMMON | (uint32)PROFILE_EHRPD );

      /* Set the custom val mask to 0*/
      ds707_data_session_profile_ptr[profile_reg_index].custom_val_mask = 0;

      /* Set the persistence to be TRUE*/
      ds707_data_session_profile_ptr[profile_reg_index].is_persistent = TRUE;

      /* Set the opaque data to 0xFFFFFFFF */
      ds707_data_session_profile_ptr[profile_reg_index].user_app_data =
        0xFFFFFFFF;

      ds707_data_session_profile_ptr[profile_reg_index].clat_enabled = FALSE;

      ds707_data_session_profile_ptr[profile_reg_index].ipv6_delegation = FALSE;

      subs_id = ds707_curr_subs_id();
      ds707_data_session_profile_ptr[profile_reg_index].subs_id = 
        ds707_data_session_get_profile_subs_id(subs_id);
    ...
    {
      DS_IS707_ERROR_LOG_2(MSG_LEGACY_ERROR,
...
void ds707_data_session_international_roaming_profile_init(void)
{
  ...
  */
  ...
  if (profile_reg_index < DATA_SESSION_PROFILE_ID_MIN)
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR,"Invalid Profile ID passed");
    return;
  }
...
)
{
        ...
        profile_id++)
  {
    ...
    if (profile_reg_index > DATA_SESSION_PROFILE_ID_MIN)
    {
      ...
      ds707_data_session_profile_ptr[profile_reg_index].supported_tech_mask =
        TECH_MASK_3GPP2;

      ds707_data_session_profile_ptr[profile_reg_index].ip_version[PRIMARY_DNS_ADDR] 
        = V4V6;

      ds707_data_session_profile_ptr[profile_reg_index].ip_version[SECONDARY_DNS_ADDR] 
        = V4V6;

      ds707_data_session_profile_ptr[profile_reg_index].is_linger_params_valid 
        = FALSE;
      ...
      ds707_data_session_profile_ptr[profile_reg_index].subs_id = 
        ds707_data_session_get_profile_subs_id(ds707_curr_subs_id());
    }
  }
}
...
)
{
  ...
  ------------------------------------------------------------------------*/
  if ( ppp_get_device_constant(PPP_UM_SN_DEV, 
...
DESCRIPTION
  This function sets all the profiles as invalid for all the subscriptions
...
void ds707_data_sess_profile_set_invalid(void)
{
      ...
      pf_subs++)
  {
    ...
    DATA_IS707_MSG1( MSG_LEGACY_HIGH,
                     "Setting all profiles to INVALID for profile = %d", 
                     pf_subs);
...
)
{
  ...
  if (profile_reg_index <= DATA_SESSION_PROFILE_ID_MIN)
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR,"Invalid Profile passed");
    return;
  }
...
/*===========================================================================
FUNCTION      ds707_data_session_set_default_param_ex
...
boolean ds707_data_session_set_default_param_ex
(
  ds707_data_session_profile_id profile_id,
  data_sess_profile_ppp_params_type *ppp_params_ptr,
  sys_modem_as_id_e_type             subs_id
)
{
  ...
  ds_profile_subs_id_enum_type pf_subs = ds707_data_session_get_profile_subs_id(subs_id);
  if(pf_subs == ACTIVE_SUBSCRIPTION_MAX)
  {
    DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR,"Invalid Profile SUBS ID: %d passed", pf_subs);
    return TRUE;
  }
  ...
  if (profile_reg_index < DATA_SESSION_PROFILE_ID_MIN)
  {
    DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR,"Invalid Profile ID: %d passed", profile_id);
    return FALSE;
  }
  ...
  ds707_data_session_profile_ptr[profile_reg_index].subs_id = pf_subs;

  DATA_IS707_MSG3( MSG_LEGACY_LOW,
         "For Profile id %d sys subs id %d Setting subs_id as %d",
          profile_id,
          subs_id,
          ds707_data_session_profile_ptr[profile_reg_index].subs_id);
...
{
  return(ds707_data_session_set_default_param_ex(profile_id, ppp_params_ptr, 
                                                 ds707_curr_subs_id()));
}
/*===========================================================================
...
)
{
  ...
  if(DS707_REFRESH_TYPE_SUBS != ds707_get_refresh_type())
  {
    /*-----------------------------------------------------------------------
    ...
    /*-----------------------------------------------------------------------
      ...
      the profiles yet. Initialize the default profile for all the subs
     -----------------------------------------------------------------------*/
        ...
        pf_subs++)
    {
      ...
      DATA_IS707_MSG1( MSG_LEGACY_HIGH,
                       "Initializing default profile for subs = %d", 
                       pf_subs);
  /*---------------------------------------------------------------------------
  ...
  ds707_data_sess_profile_get_default_ppp_opts(&ds707_default_ppp_params);
      ...
      if (!ds707_data_session_set_default_param_ex(DATA_SESSION_DEFAULT_PROFILE,
                                                   &ds707_default_ppp_params, 
                                                   subs_id))
  {
        DS_3GPP2_ERROR_FATAL("Error initializing default profile");
      }
    }
  }
 ...
 /*-------------------------------------------------------------
  ...
  /* Read profile info if in NV mode. In card mode, read profile from 
       NVRUIM only after subscription ready event. 
       If in card mode, check if suscription ready was received, if so
       go ahead and read the profiles.  */
  ...
  if( nv_status == NV_DONE_S &&
      ( (NV_RTRE_CONTROL_USE_RUIM != nv_item.rtre_config) ||
        ( (NV_RTRE_CONTROL_USE_RUIM == nv_item.rtre_config) &&
          ds707_pkt_mgr_is_cdma_subscription_ready(ds707_curr_subs_id()) ) )
    )
  {
...
)
{
  ...
  int fd = -1; /* EFS File Descriptor */
  char efs_file_name[47]=PROFILE_PATH;
       ...
       tmp_profile_id++)
  {
       ...
       )
    {
      ...
      if (is_persistent)
      {
        ...
        if (-1 == fd)
        {
          DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "EFS Create failed");
          return profile_id;
        }
        /* Close the file as we don't have anything to do*/
        if (-1 == efs_close (fd))
        {
          DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "EFS Close failed");
          return profile_id;
        }
      }
  ...
  if (-1 == profile_id)
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "No available profile");
    return profile_id;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
      ...
      PROFILE_INVALID )
  {
    ...
    {
      DS_3GPP2_ERROR_FATAL("Error creating default profile");
      return FALSE;
    }
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Profile is Invalid");
    return FALSE;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
     ...
     ds707_roaming_is_simplified_jcdma_mode())
  {
       ...
       profile_reg_index++ )
    {
      ...
      if (ds707_data_session_profile_ptr[profile_reg_index].profile_type &
          PROFILE_KDDI)
      {
        ...
        ds707_create_efs_file_name(efs_file_name,profile_id);
        if(ds3gcfgmgr_efs_file_init_ex(efs_file_name, subs_id, &efs_db) != -1)
        {
          DATA_IS707_MSG1(MSG_LEGACY_HIGH,
          ...
          ds707_read_data_session_profile_from_efs(efs_file_name, &efs_db,
              &ds707_data_session_profile_ptr[profile_reg_index],
              profile_id);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if ( ( subs_id == ACTIVE_SUBSCRIPTION_1 ) && 
       (ds707_data_sess_get_sys_subsid(subs_id) != ds707_curr_subs_id()) )
  {
    ds707_data_session_profile_info_ptr = 
                      ds707_data_session_profile_info;
  }
  else if ( ( subs_id == ACTIVE_SUBSCRIPTION_2 ) && 
       (ds707_data_sess_get_sys_subsid(subs_id) != ds707_curr_subs_id()) )
  {
    ds707_data_session_profile_info_ptr = 
                      ds707_data_session_profile_info_secondary_subs;
  ...
  for (profile_reg_index = DATA_SESSION_PROFILE_ID_MIN;
       profile_reg_index < ds707_data_session_max_profiles[ds707_data_sess_get_sys_subsid(subs_id)];
       profile_reg_index++)
  {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if ( ds707_data_sess_get_sys_subsid(subs_id) != ds707_curr_subs_id())
  {
  ...
  -------------------------------------------------------------------------*/
    profile_reg_index = ds707_data_session_is_valid_profile(profile_id, 
                                    ds707_data_sess_get_sys_subsid(subs_id));

if(ds707_data_sess_get_profile_info(subs_id) != NULL)
    {
      ds707_data_session_profile_ptr = 
            *(ds707_data_sess_get_profile_info(subs_id));
    }
    if(ds707_data_session_profile_ptr == NULL)
    {
      DS_LEAVE_CRIT_SECTION(&ds707_profile_crit_sect);
    ...
    if ((profile_reg_index < 0) ||
        (ds707_data_session_profile_ptr[profile_reg_index].subs_id != subs_id))
    {
      DS_IS707_ERROR_LOG_2(MSG_LEGACY_ERROR,
    ...
    DATA_IS707_MSG2(MSG_LEGACY_LOW,
                    ...
                    profile_reg_index,
                    ds707_data_session_profile_ptr[profile_reg_index].subs_id);
    *profile_info_ptr = &ds707_data_session_profile_ptr[profile_reg_index];
...
)
{
  ...
  if( apn_name == NULL || (TRUE != ds707_pkt_is_subs_id_valid(subs_id)))
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR,
                     "ds707_data_session_is_apn_enabled_in_profile: "
                     "NULL apn_name parameter or invalid subs id passed" );
    return FALSE;
  }
  ...
  for ( profile_reg_index = DATA_SESSION_PROFILE_ID_MIN;
        ...
        profile_reg_index++)
  {
    if (PROFILE_EHRPD & ds707_data_session_profile_ptr[profile_reg_index].profile_type)
    {
      if ( apn_len == ds707_data_session_profile_ptr[profile_reg_index].apn_string_len )
      {
        if( strncasecmp( ds707_data_session_profile_ptr[profile_reg_index].apn_string,
                     apn_name, apn_len ) == 0 )
        {
          if ( ds707_data_session_profile_ptr[profile_reg_index].apn_flag == TRUE )
          {
            ret_val = TRUE;
          }
          DS_LEAVE_CRIT_SECTION(&ds707_profile_crit_sect);
...
)
{
  ...
  {
    DATA_IS707_C_MSG0( DS707_F3_MASK,
    ...
    else if(DS3GCFGMGR_EFS_TOKEN_PARSE_SUCCESS == efs_ret_val)
    {
      ...
      else
      {
        ...
        if ( ds707_mandatory_apn_list[i].apn_name == NULL )
        {
          ...
          {
            DS_3GPP2_ERROR_FATAL("profile memory re-allocation failed");
            return;
          }
          else
          {
...
)
{
  ...
  while (  i < DATA_SESSION_MAX_APN_NUM )
  {
    ...
    else
    {
      ...
      if(writebuff == NULL)
      {
        DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "ds707_update_min_apn_efs():"
  ...
  if (MCFG_FS_STATUS_OK != mcfg_fs_delete(efs_file_name,
							...
							(mcfg_fs_sub_id_e_type) ds707_curr_subs_id()))
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "EFS Unlink failed");
    return ;
  }
  ...
  if (-1 == mcfg_fs_rename(tmp_file_name,
			   ...
			   (mcfg_fs_sub_id_e_type)ds707_curr_subs_id()))
  {
    DS_IS707_ERROR_LOG_2(MSG_LEGACY_ERROR, "EFS Rename failed: %d, subs_id 0x%0x",
...
)
{
        ...
        profile_reg_index++)
  {
    if (PROFILE_EHRPD & ds707_data_session_profile_ptr[profile_reg_index].profile_type)
    {
      if ( label_len == ds707_data_session_profile_ptr[profile_reg_index].pdn_label_len )
      {
        if( strncasecmp( ds707_data_session_profile_ptr[profile_reg_index].pdn_label,
                     label_string_ptr, label_len ) == 0 )
        {
          if ( ds707_data_session_profile_ptr[profile_reg_index].apn_flag)
          {
            DATA_IS707_MSG1_EX( MSG_LEGACY_HIGH,
                                subs_id,
...
/*-------------------------------------------------------------------------*/
  ...
  if(
      ds707_data_session_profile_ptr[DATA_SESSION_DEFAULT_PROFILE].data_session_profile_id
      == DATA_SESSION_PROFILE_ID_INVALID)
  {
  /*------------------------------------------------------------------------
  ...
  ------------------------------------------------------------------------*/
  ds707_get_ppp_default_dev_constant(
        &ds707_data_session_profile_ptr[DATA_SESSION_DEFAULT_PROFILE]);

  /*------------------------------------------------------------------------
    get PPP session close timer value and set it in the default strcture
  ------------------------------------------------------------------------*/
  // how to get this default value
  ds707_data_session_profile_ptr[DATA_SESSION_DEFAULT_PROFILE].ppp_session_close_timer_DO =
    ds707_gonulltmr_get_timer(DS707_GONULLTMRI_DO);
  ds707_data_session_profile_ptr[DATA_SESSION_DEFAULT_PROFILE].ppp_session_close_timer_1X =
    ds707_gonulltmr_get_timer(DS707_GONULLTMRI_1X);

  /*------------------------------------------------------------------------
    set the data rate and data mode in the default structure
  ------------------------------------------------------------------------*/
  ds707_data_session_profile_ptr[DATA_SESSION_DEFAULT_PROFILE].data_rate =
    DS707_DATARATE_HIGH;
  ds707_data_session_profile_ptr[DATA_SESSION_DEFAULT_PROFILE].data_mode =
    DS707_DATAMODE_CDMA_HDR;
  /*------------------------------------------------------------------------
    set allow linger as false i.e. linger state is not allowed in case of
    default profile.
  ------------------------------------------------------------------------*/
  ds707_data_session_profile_ptr[DATA_SESSION_DEFAULT_PROFILE].allow_linger
    = FALSE;
  /*------------------------------------------------------------------------
    set linger timeout value to default value (0) for default profile
  ------------------------------------------------------------------------*/
  ds707_data_session_profile_ptr[DATA_SESSION_DEFAULT_PROFILE].linger_timeout_val
    = 0;
  /*------------------------------------------------------------------------
...
)
{
  ...
  {
    DS_3GPP2_ERROR_FATAL("NULL data_session_info_ptr");
    return -1;
  }
  else
  {
...
/*---------------------------------------------------------------------------*/
  ...
  profile_reg_index = ds707_data_session_is_valid_profile(profile_id, ds707_curr_subs_id());
...
#ifdef FEATURE_DATA_PS_IPV6
      else if (ds707_data_session_profile_ptr[profile_reg_index].pdn_type ==
               DS707_DATA_SESS_PDN_TYPE_V6)
      {
  ...
  DATA_IS707_C_MSG1( DS707_F3_MASK,
...
/*-------------------------------------------------------------------------*/
  ...
  if ( (profile_reg_index == DATA_SESSION_PROFILE_ID_INVALID) ||
       ...
       (if_ptr == NULL) )
  {
    DS_IS707_ERROR_LOG_3(MSG_LEGACY_ERROR,
...
/*-------------------------------------------------------------------------*/
  ...
  if ( policy_info_ptr->subs_id == 
         (uint32)ds707_data_session_profile_ptr[profile_reg_index].subs_id )
  {
    retval = TRUE;
  }
  else
...
/*---------------------------------------------------------------------------*/
  ...
  /*-------------------------------------------------------------------------
    ...
    ds707_pkt_is_ehrpd_last_pref_sys() check was added to ensure PDN 
    compatibilty check for eHRPD profiles while UE is OOS/NULL bearer in 
    eHRPD-OOS-eHRPD scenario.
  -------------------------------------------------------------------------*/
  if(ds707_pkt_is_ehrpd_mode_of_operation() ||
     ...
     ds707_pkt_is_ehrpd_last_pref_sys() )
  {
        ...
        profileB_ptr->apn_string_len)
    {
      ...
      {
        DATA_IS707_C_MSG_SPRINTF_3(DS707_F3_MASK,
...
/*-------------------------------------------------------------------------*/
  ...
  DATA_IS707_C_MSG2(DS707_F3_MASK,
  ...
  requesting_profile_reg_index = ds707_data_session_is_valid_profile(requesting_id, ds707_curr_subs_id());
  ...
  requesting_profile_type = 
       ds707_data_session_profile_ptr[requesting_profile_reg_index].profile_type;
  requesting_profile_info = 
       &ds707_data_session_profile_ptr[requesting_profile_reg_index];
  requesting_app_priority =
       ds707_data_session_profile_ptr[requesting_profile_reg_index].app_priority;
...
)
{
  ...
  *auth_password_len = 0;
  ...
  if (profile_id >= (uint8)ds707_data_session_max_profiles[subs_id])
  {
    DATA_IS707_MSG1( MSG_LEGACY_HIGH,
  ...
  else
  {
    memscpy(user_id,
	    PPP_MAX_USER_ID_LEN,
            ds707_data_session_profile_ptr[profile_id].user_id,
            ds707_data_session_profile_ptr[profile_id].user_id_len );

    *user_id_len = ds707_data_session_profile_ptr[profile_id].user_id_len;

    memscpy(auth_password,
	    PPP_MAX_USER_ID_LEN,
            ds707_data_session_profile_ptr[profile_id].auth_password,
            ds707_data_session_profile_ptr[profile_id].auth_password_len );

    *auth_password_len = ds707_data_session_profile_ptr[profile_id].auth_password_len;
  }
    ...
    switch ( config_option )
    {
      case DYNAMIC_LINGER_CONFIG:
      {
        ...
        else
        {
          ...
          DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "The Current Data Session is not HDR or 1X. Setting Linger timer to 0.");
        }
        break;
      } /* case DYNAMIC_LINGER_CONFIG */
...
)
{
  ...
  pf_subs = ds707_data_session_get_profile_subs_id(subs_id);
  if(pf_subs == ACTIVE_SUBSCRIPTION_MAX)
  {
    return ret_val;
  }
  ...
  for (profile_reg_index = DATA_SESSION_PROFILE_ID_MIN;
       ...
       profile_reg_index++)
  {
    if (ds707_data_session_profile_ptr[profile_reg_index].data_session_profile_id
        == profile_id)
    {
...
/*-------------------------------------------------------------------------*/
  ...
  profile_reg_index = ds707_data_session_get_reg_index(profile_id, subs_id);
  ...
  pf_subs = ds707_data_session_get_profile_subs_id(subs_id);
  if(ACTIVE_SUBSCRIPTION_MAX == pf_subs)
  {
    return ret_val;
  }
  ...
  {
    DATA_IS707_MSG2(MSG_LEGACY_MED, "Profile %d is not present and not a valid profile"
                    "subs id %d",
                     profile_id, subs_id);
  }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
 /*--------------------------------
  ...
  DATA_IS707_C_MSG4( DS707_F3_MASK,
...
#else
   if( (NV_RTRE_CONTROL_NO_RUIM != nv_rtre_control_ext(ds707_curr_subs_id())) &&
#endif /* FEATURE_DATA_MPSS_AT20_DEV */
       (!nvruim_data_3gpd_ext_support_per_sub(ds707_curr_subs_id())) &&
       !(NVRUIM_NON_3GPD_CARD == nvruim_check_non_3gpd_cdma_card_per_sub(ds707_curr_subs_id())) )
   {
     DATA_IS707_MSG0( MSG_LEGACY_HIGH,
   ...
   if( nv_status == NV_DONE_S )
   {
     ...
     if ( FALSE == ds707_data_sess_is_nvruim_profile_allocated() ) 
     {
       ...
       *ds707_data_session_profile_ptr = profile_info_realloc;
       memset( (void *)(*ds707_data_session_profile_ptr +
         ds707_data_session_max_profiles[subs_id]),
         0,
         DATA_SESSION_NVRUIM_MAX_PROFILES *
           sizeof(ds707_data_session_profile_info_type) );
       ...
       for ( index = (uint8)ds707_data_session_max_profiles[subs_id];
             index < (uint8)(ds707_data_session_max_profiles[subs_id] + 
                       DATA_SESSION_NVRUIM_MAX_PROFILES);
             index++ ) 
       {
         (*ds707_data_session_profile_ptr)[index].data_session_profile_id =
           DATA_SESSION_PROFILE_ID_INVALID;
         (*ds707_data_session_profile_ptr)[index].profile_type =
           PROFILE_INVALID;
       }
       ds707_data_session_max_profiles[subs_id] += 
                                             DATA_SESSION_NVRUIM_MAX_PROFILES;
     ...
     for(index=0; index<num_valid_profiles; index++)
     {
       ...
       if( nv_status == NV_DONE_S)
       {
         ...
         {
           profile_reg_index = 
              ds707_data_session_get_reg_index( profile_id, ds707_curr_subs_id() );
         }
         ...
         (*ds707_data_session_profile_ptr)[profile_reg_index].data_session_profile_id
           = profile_id;
         (*ds707_data_session_profile_ptr)[profile_reg_index].app_type
           = loc_nv_item.ds_sip_profile.app_type;
         (*ds707_data_session_profile_ptr)[profile_reg_index].app_priority
           = loc_nv_item.ds_sip_profile.app_priority;
         ...
         */
         ...
         --------------------------------------------------------------------*/
         (*ds707_data_session_profile_ptr)[profile_reg_index].allow_linger = FALSE;
         (*ds707_data_session_profile_ptr)[profile_reg_index].linger_timeout_val
           = 0;
         /*--------------------------------------------------------------------
              set PPP lcp and ipcp options
         --------------------------------------------------------------------*/
         ...
         (*ds707_data_session_profile_ptr)[profile_reg_index].auth_protocol = 0;
         /*--------------------------------------------------------------------
           set ppp user_id and password lengths as 0
         --------------------------------------------------------------------*/
         (*ds707_data_session_profile_ptr)[profile_reg_index].user_id_len = 0;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    profile_reg_index = ds707_data_session_get_reg_index( index, ds707_curr_subs_id() );
    if( profile_reg_index == DATA_SESSION_PROFILE_ID_INVALID )
    {
      DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR,
    ...
    if( app_type & ds707_data_session_profile_ptr[profile_reg_index].app_type )
    {
  ...
  if( app_profile_id == DATA_SESSION_PROFILE_ID_INVALID )
  {
         ...
         index < DATA_SESSION_NVRUIM_PROFILE_MIN + num_valid_profiles; index++ )
    {
      ...
      if( ds707_data_session_profile_ptr[profile_reg_index].app_type &
          DEFAULT_PROFILE )
      {
    ...
    {
      DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "App profile id lookup failed." );
    }
  }
  else
  {
...
/*---------------------------------------------------------------------------*/
       ...
       index < DATA_SESSION_NVRUIM_PROFILE_MIN + num_valid_profiles; index++ )
  {
    ...
    if( ds707_data_session_profile_ptr[profile_reg_index].app_type &
        LBS_APP_TYPE )
    {
      if( ds707_data_session_profile_ptr[profile_reg_index].app_priority >
          app_priority_val )
      {
        app_priority_val =
              ds707_data_session_profile_ptr[profile_reg_index].app_priority;
  ...
  if( app_profile_id == DATA_SESSION_PROFILE_ID_INVALID )
  {
    ...
    {
      DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "LBS APP profile lookup failed." );
    }
  }
  else
  {
...
/* Skip the profile arbitration for MIP and MIP to SIP fallback cases */
  ...
  profile_app_type = ds707_data_session_profile_ptr[ profile_reg_index ].app_type;
...
{
  if (ds707_data_session_is_valid_profile(profile_id, ds707_curr_subs_id()) ==
      DATA_SESSION_PROFILE_ID_INVALID)
  {
    return FALSE;
  }
...
)
{
  ...
  else
  {
    ...
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "PPP user ID never written");
  }
  ...
  else
  {
    ...
    -----------------------------------------------------------------------*/
    ppp_auth_info_ptr->passwd_len = 0;
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "PPP password never written" );
#endif /* FEATURE_DATA_PPP_DEFAULT_PASSWD */
  }
...
)
{
  ...
  if( profile_reg_index == DATA_SESSION_PROFILE_ID_INVALID )
  {
    ...
    DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR,"Invalid profile id %d",
                     profile_id );
    return FALSE;
  }
...
)
{
  ...
  if ( ds707_data_session_is_nvruim_profiles_available() &&
       ( ( (ds707_pkt_mgr_get_current_sys_mode() == SYS_SYS_MODE_HDR) &&
           (ds3gdsdif_ehprd_is_pref_system( ds707_curr_subs_id() )
                                                             == FALSE) ) ||
         ( ds707_pkt_mgr_get_current_sys_mode() == SYS_SYS_MODE_CDMA ) ) )
  {
...
void ds707_data_sess_enter_emerg_only_mode(void)
{
  ...
  DATA_IS707_MSG1( MSG_LEGACY_HIGH,
                   ...
                   ds707_is_cm_centric_emerg_only_mode);
    if(!ds3gi_set_cm_emergency_only_mode_ex(TRUE,ds707_curr_subs_id()))
  {
      DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR,
...
)
{
  ...
  DS3GPP2_ASSERT(ds_cmd_ptr != NULL);
  DS3GPP2_ASSERT(ds_cmd_ptr->cmd_payload_ptr != NULL);
...
void ds707_data_sess_exit_emerg_only_mode(void)
{
  ...
  DATA_IS707_MSG1( 
    ...
    if(!ds3gi_set_cm_emergency_only_mode_ex(FALSE,ds707_curr_subs_id()))
  {
      DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_3GPP2_ERROR_FATAL("cmd_payload_ptr is NULL");
    return;
  }
...
)
{
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Invalid PDN throttle counter value");
    return 0;
  }
...
)
{
  ...
  if ((reg_index >= DATA_SESSION_PROFILE_ID_MIN) &&
     (reg_index < ds707_data_session_max_profiles[ds707_curr_subs_id()]))
  {
    ds707_data_session_profile_ptr = *(ds707_data_sess_get_profile_info_sys());
    return ds707_data_session_profile_ptr[reg_index].data_session_profile_id;
  }
  else
  {
...
)
{
  ...
  switch( param_ident )
  {
    ...
    default:
      DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Invalid Param Identifier passed");
      return FALSE;
  }
...
{
  char efs_file_name[47]=PROFILE_PATH;
  char efs_tmp_file_name[47]=PROFILE_TMP_PATH;
  ...
  if (-1 == fd)
  {
    DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR, "Unable to get fd for profile: %d", profile_id);
    return FALSE;
  }
  ...
  if(tempbuff == NULL)
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "ds707_write_profile_to_efs():"
                    "memory allocation failed");
    return FALSE;
  }
                                                ...
                                                )
{
  ...
  if(ds3gcfgmgr_efs_file_init_ex(efs_file_name, ds707_curr_subs_id(), &efs_db) == -1)
  {
    DATA_IS707_MSG0(MSG_LEGACY_HIGH, "Error opening file ISN from EFS");
    return FALSE;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(ds707_data_session_profile_ptr == NULL)
  {
    DS_3GPP2_ERROR_FATAL("profile memory pointer null");
    DS_LEAVE_CRIT_SECTION(&ds707_profile_crit_sect);
    return FALSE;
  }
  max_profiles_ptr = 
    &ds707_data_session_max_profiles[ds707_data_sess_get_sys_subsid(subs_id)];
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ( profile_info_realloc == NULL && 
       ((*max_profiles_ptr) != 0) ) 
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Memory reallocation failed");
    retval = FALSE;
  }
  else
  {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  apn_pkt_mode = ds3g_get_apn_pref_pkt_mode_ex(
                     &apn_name,
                     ds707_curr_subs_id());
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if( ( ( (ds707_pkt_mgr_get_current_sys_mode() == SYS_SYS_MODE_HDR) &&
          (ds3gdsdif_ehprd_is_pref_system(ds707_curr_subs_id()) == FALSE) ) ||
        ( ds707_pkt_mgr_get_current_sys_mode() == SYS_SYS_MODE_CDMA ) ) &&
      ( (current_id == DATA_SESSION_PROFILE_ID_INVALID &&
         requesting_id == DATA_SESSION_PROFILE_ID_INVALID) ||
        (current_id == DATA_SESSION_PROFILE_ID_INVALID &&
...
{
  ds707_data_session_profile_info_type **ds707_data_session_profile_ptr = NULL;
  sys_modem_as_id_e_type                 recvd_subs_id;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */

  recvd_subs_id = ds707_data_sess_get_sys_subsid(subs_id);

  if ( (subs_id == ACTIVE_SUBSCRIPTION_2)&& 
       (recvd_subs_id != ds707_curr_subs_id()) )
  {
    ds707_data_session_profile_ptr = 
      &ds707_data_session_profile_info_secondary_subs;
  }
  ...
  if (*ds707_data_session_profile_ptr != NULL)
  {
    ...
    *ds707_data_session_profile_ptr = NULL;
    ...
    ds707_data_session_max_profiles[recvd_subs_id] = 0;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
        ...
        profile_reg_index++)
  {
    ...
    {
      return ds707_data_session_profile_ptr[profile_reg_index].data_session_profile_id;
    }
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  DATA_IS707_MSG1(MSG_LEGACY_LOW,
                   "ds707_data_session_get_profile_subs_id: %d", ds_profile_subs_id);
...
/*===========================================================================
FUNCTION DS707_DATA_SESSION_GET_PROFILE_ID_FROM_APN_NAME

DESCRIPTION
This function gets the profile number from APN Name that was passed in.

PARAMETERS
  subs_id            - DS profile Subs Id
  *apn               - APN Name
...
RETURN VALUE
  ...
  @dependencies
  caller function need to make sure the memory is already allocated as
  described by arr_size
===========================================================================*/
ds707_data_session_profile_id ds707_data_session_get_profile_id_for_apn_name
(
  ds_profile_subs_id_enum_type   ds_profile_subs_id,
  char                           *apn
)
{
  ds707_data_session_profile_id         profile_reg_index = 0;
  ds707_data_session_profile_info_type *ds707_data_session_profile_ptr = NULL;
  ds707_data_session_profile_id         profile_id = DS_PROFILE_UNSPECIFIED_NUM;
  sys_modem_as_id_e_type                subs_id   = SYS_MODEM_AS_ID_1;
  ds707_data_session_profile_info_type **ds707_data_session_profile_ptr_ptr = NULL;
  /*--------------------------------------------------------------------------*/
  ...
  subs_id = ds707_data_sess_get_sys_subsid(ds_profile_subs_id);

  if (ds707_pkt_is_subs_id_valid(subs_id) == FALSE)
  {
    return profile_id;
  }

  if ((ds707_data_session_profile_ptr_ptr = 
        ds707_data_sess_get_profile_info(ds_profile_subs_id)) != NULL)
  {
    ds707_data_session_profile_ptr = *ds707_data_session_profile_ptr_ptr;
  }
  ...
  {
    if( strncasecmp( ds707_data_session_profile_ptr[profile_reg_index].apn_string,
                     apn, DS_VSNCP_3GPP2_APN_MAX_VAL_LEN ) == 0 )
    {
      profile_id = 
        ds707_data_session_profile_ptr[profile_reg_index].data_session_profile_id;
      break;
  ...
  return profile_id;
} /* ds707_data_session_get_profile_id_for_apn_name*/

/*===========================================================================
FUNCTION DS707_DATA_SESSION_GET_PROFILE_ID_FROM_APN_TYPE

DESCRIPTION
This function gets the profile number from APN Type that was passed in.

PARAMETERS
  subs_id            - DS profile Subs Id
  APn Type           - APN Type Mask
  
RETURN VALUE
  valid number of profile present in the cache
===========================================================================*/
ds707_data_session_profile_id ds707_data_session_get_profile_id_for_apn_type
(
  ds_profile_subs_id_enum_type   ds_profile_subs_id,
  uint64                         apn_type
)
{
  ...
  for ( profile_reg_index = DATA_SESSION_PROFILE_ID_MIN;
        ...
        profile_reg_index++)
  {
    if((ds707_data_session_profile_ptr[profile_reg_index].apn_type_mask & apn_type) != 0 )
    {
      profile_id = 
        ds707_data_session_profile_ptr[profile_reg_index].data_session_profile_id;
  ...
  return profile_id;
} /* ds707_data_session_get_profile_id_for_apn_type*/
