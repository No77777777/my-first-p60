...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp2/ds707/src/ds707_netmodel.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
)
{
  ...
  if( item_head_ptr  == NULL ||
      *item_head_ptr == NULL ||
      ...
      um_opt_ptr     == NULL )
  {
     DS_3GPP2_ERROR_FATAL("NULL arguments passed");
     return RET_ERROR;
  }
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "seek_extract() failed - dumping pkt");
  ...
  while ( pkt_len > 0)
  {
    ...
    {
      DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR, "option length %d < required at least "
                      "2 bytes - dumping pkt",
                      opt_hdr.length);
      dsm_free_packet( item_head_ptr);
      return RET_ERROR;
...
)
{
  ...
  {
     DS_3GPP2_ERROR_FATAL("Invalid DSM item passed");
  ...
  {
    DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR,"Invalid PPP Device %d", out_device);
    dsm_free_packet(item_head_ptr);
    return SNOOP_CB_IGNORE_PKT;
  }
  ...
  if ( TRUE == dsgen_iface_hdlr_use_internal_auth() ||
       ...
       NVRUIM_NON_3GPD_CARD == 
            nvruim_check_non_3gpd_cdma_card_per_sub(ds707_curr_subs_id()) )
  {
    /*-------------------------------------------------------------------------
...
)
{
  ...
  if( out_device >= PPP_MAX_DEV )
  {
    DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR, "Invalid device : out_device %d", 
                     out_device);
    dsm_free_packet(item_head_ptr);
    return SNOOP_CB_IGNORE_PKT;
  }
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Saw LCP C-Ack when NOT in Pseudo "
...
)
{
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Seek extract failed - dumping pkt");
    dsm_free_packet(item_head_ptr);
    return SNOOP_CB_IGNORE_PKT;
  }
...
)
{
  ...
  DS3GPP2_ASSERT((*item_head_ptr)->size > 0 && (*item_head_ptr)->used > 0);
  DS3GPP2_ASSERT(device < PPP_MAX_DEV);
  DS3GPP2_ASSERT(protocol == PPP_CHAP_PROTOCOL);
  DS3GPP2_ASSERT(ppp_cb_array[device].bridge_dev < PPP_MAX_DEV);
  /*lint -restore */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/  
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Laptop NAI cannot be retrieved");
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  DS3GPP2_ASSERT(protocol == PPP_PAP_PROTOCOL);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(item_head_ptr == NULL || *item_head_ptr == NULL)
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "rm_chap_cback called with invalid item!");
    return SNOOP_CB_IGNORE_PKT;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS3GPP2_ASSERT(protocol == PPP_CCP_PROTOCOL || protocol == PPP_ILCCP_PROTOCOL);
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "No memory to send CCP prot-rej!  silently discard");
    dsm_free_packet(item_head_ptr);
    return SNOOP_CB_IGNORE_PKT;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS3GPP2_ASSERT(item_head_ptr != NULL && *item_head_ptr != NULL);
  DS3GPP2_ASSERT(device < PPP_MAX_DEV);
  DS3GPP2_ASSERT(protocol == PPP_LCP_PROTOCOL || protocol == PPP_IPCP_PROTOCOL);
  /*lint -restore */
 /*-------------------------------------------------------------------------
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if( item_head_ptr == NULL || *item_head_ptr == NULL )
  {
    DS_3GPP2_ERROR_FATAL("NULL DSM item passed");
    return SNOOP_CB_IGNORE_PKT;
  }
         ...
         ( PS_IFACE_GET_GRACEFUL_CLOSE(bridge_iface_ptr) == FALSE) ) )
  {
    ...
    {
      /* shouldnt happen */
      DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR,"Error sending T-Ack!");
    }
    else
    {
...
)
{
  ...
  {
    DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR, "You called with bad param %d", behav);
  }
}
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(device != PPP_UM_SN_DEV)
  {
    DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR,"PPP device is not PPP_UM_SN_DEV," 
                     "device: %d", device);
    DS3GPP2_ASSERT(0);
    return FALSE;
  }
