...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp2/ds707/src/ds707_timer.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch ((ds3g_timer_enum_type)timer_id)
  {
    ...
    case DS3G_DELAY_OTASN_TEARDOWN_TIMER:
      cmd_ptr = ds_allocate_cmd_buf(DS_CMD_PAYLOAD_SIZE_ZERO);
      if((NULL == cmd_ptr))
      {
        DS3GPP2_ASSERT(0);
  ...
  switch ((ds3g_timer_enum_type)timer_id)
  {
    ...
    case DS3G_DELAY_OTASN_TEARDOWN_TIMER:
      cmd_ptr->hdr.cmd_id = DS_CMD_707_DELAY_OTASN_PDN_TEARDOWN_TIMER_EXPIRED;
      break;
...
void ds707_timer_init(void)
{
  ...
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ds3g_timer_register(&ds707_cta_timer,
                       ds707_timer_cb,
                       DS3G_TIMER_CTA);

  ds3g_timer_register(&ds707_holddown_timer,
                       ds707_timer_cb,
                       DS3G_TIMER_HOLDDOWN);

#ifdef FEATURE_DS_EPZID_HYSTERESIS
  ds3g_timer_register(&ds707_epzid_hyst_timer,
                       ds707_timer_cb,
                       DS3G_TIMER_EPZID_HYSTERESIS);
#endif /* FEATURE_DS_EPZID_HYSTERESIS */
  
  ds3g_timer_register(&ds707_epzid_delay_timer,
                       ds707_timer_cb,
                       DS3G_TIMER_EPZID_DELAY);

  ds3g_timer_register(&ds707_retry_delay_orig_timer,
                       ds707_timer_cb,
                       DS3G_TIMER_RETRY_DELAY_ORIG);

#ifdef FEATURE_DEDICATED_C2K_MODE_DATA
  ds3g_timer_register(&ds707_pkt_auto_redial_timer,
                       ds707_timer_cb,
                       DS3G_TIMER_PKT_AUTO_REDIAL);
#endif

  ds3g_timer_register(&ds707_abort_dormant_ppp_timer,
                       ds707_timer_cb,
                       DS3G_TIMER_ABORT_DORMANT_PPP);

  ds3g_timer_register(&ds707_dun_timer,
                       ds707_timer_cb,
                       DS3G_TIMER_DUN_CTRL_ENTL);
  
  /*OTASN teardown timer is per subs based*/
  for (sub_idx = 0; sub_idx < DS3GSUBSMGR_SUBS_ID_MAX; sub_idx++)
  {
    ds3g_timer_register_ex(
      (void *)&ds707_delay_otasn_pdn_teardown_timer,
                       ds707_timer_cb,
