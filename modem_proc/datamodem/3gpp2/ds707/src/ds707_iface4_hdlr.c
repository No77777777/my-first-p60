...
$Header: //components/rel/data.mpss/3.5..2.1/3gpp2/ds707/src/ds707_iface4_hdlr.c#2 $
...
--------    ---    ---------------------------------------------------------- 
04/01/16    sd     Fixed LLVM compiler warnings.
...
static void dssnet4i_handle_iface_ev_cback
(
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_3GPP2_ERROR_FATAL("instance_ptr or ps_iface_ptr is NULL");
    return -1;
  }

  if(instance_ptr->this_sm_ptr == instance_ptr)
  {
    DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR, "DSSNet4 SM 0x%x already initialized", 
                     instance_ptr);
    return 0;
  }
  ...
  /*-------------------------------------------------------------------------
    Allocate buffer for Iface and Phys link events.
    Note that the phys link up event is registered only when the common
    dssnet_start() invokes the registration functions
  -------------------------------------------------------------------------*/
  instance_ptr->iface_event_buf =  (void *)ps_iface_alloc_event_handle(
                                          ps_iface_ptr,
                                          dssnet4i_handle_iface_ev_cback,
                                          instance_ptr);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS3GPP2_ASSERT(event < DSSNET4_MAX_EV);
...
)
{
  ...
  {
    DS_3GPP2_ERROR_FATAL("DSSNET4 instance_ptr is NULL");
    return;
  }
  ...
  if (!dshdr_get_mobile_ip_uses_ccoa()) 
  {
    ...
    {
      DS_3GPP2_ERROR_FATAL( "Unable to acquire a MIP event handle");
    }
    else
    {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (instance_ptr == NULL)
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR,  "dssnet4_reg_phys_link_events()"
                     "NULL instance pointer passed, return");
    return -1;
  }
  /*-------------------------------------------------------------------------
    Register for PHYS LINK UP event.
  -------------------------------------------------------------------------*/
  if (instance_ptr->phys_link_event_buf != NULL) 
  {
    num_events = 3; 
    events[0] = IFACE_PHYS_LINK_UP_EV;
    events[1] = IFACE_PHYS_LINK_GONE_EV;
    events[2] = IFACE_PHYS_LINK_DOWN_EV;
    result3 = ps_phys_link_subscribe_event_list(
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR,  "ps_iface_event_cback_reg for "
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (instance_ptr->phys_link_event_buf != NULL)
  {
    ...
    events[1] = IFACE_PHYS_LINK_GONE_EV;
    events[2] = IFACE_PHYS_LINK_DOWN_EV;
    ps_phys_link_unsubscribe_event_list(instance_ptr->phys_link_event_buf,
                                    (ps_iface_event_enum_type *)&events,
                                    num_events);
  }
           ...
           (data_sess_profile_info_ptr) != NULL )
      {
             ...
             (data_sess_profile_info_ptr->pdn_type == DS707_DATA_SESS_PDN_TYPE_V4_V6) )
        {
          ...
          if( PS_IFACE_IS_VALID(instance_ptr->ps_iface_ptr) &&
              !PS_IFACE_IS_IN_USE(instance_ptr->ps_iface_ptr) )
          {
            DATA_IS707_MSG2( MSG_LEGACY_HIGH, 
  ...
  switch(instance_ptr->state)
  {
    ...
    case DSSNET4I_WAITING_FOR_MIP_REG_STATE:
      ...
      else
      {
        ...
        -------------------------------------------------------------------*/
        DS_3GPP2_ERROR_FATAL("Up for SimpleIP, but prov'd for MIP only!");
        break;
      }
#endif /* FEATURE_DS_MOBILE_IP */
    ...
    case DSSNET4I_NET_UP_STATE:
     ...
     default:
      /*---------------------------------------------------------------------
        Sanity Check
      ---------------------------------------------------------------------*/
      DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR,"dssnet4 state machine in" 
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch(instance_ptr->state)
  {
    ...
    case DSSNET4I_NET_UP_STATE:
      ...
      {
        if( ds3gdsdif_is_voice_or_emergency_call_active_on_other_sub(
                                ds3gsubsmgr_subs_id_cm_to_ds3g(ds707_curr_subs_id())) ||
            instance_ptr->local_abort == TRUE )
        {
          mip_reg_sm_local_cleanup();
        }
        else
        {
          dssnet4i_transition_state( instance_ptr,
...
)
{
  ...
  {
    DS_3GPP2_ERROR_FATAL("Invalid ppp_auth_info_ptr pointer");
    return;
  }
  ...
  switch(instance_ptr->state)
  {
    case DSSNET4I_WAITING_FOR_MIP_REG_STATE:
          ...
          )
      {
        ...
        if(ppp_config == NULL)
        {
          DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "dssnet4i_process_mip_failure_ev():"
        ...
        if(ds707_data_session_get_num_valid_profiles_in_nvruim() > 0 &&
           ( ( (ds707_pkt_mgr_get_current_sys_mode() == SYS_SYS_MODE_HDR) &&
               (ds3gdsdif_ehprd_is_pref_system(subs_id) == FALSE) ) ||
             ds707_pkt_mgr_get_current_sys_mode() == SYS_SYS_MODE_CDMA ))
        {
          ...
          ds707_data_session_set_iface_requesting_profile(
                instance_ptr->ps_iface_ptr, data_sess_profile_id, profile_subs_id);
        ...
        {
          DATA_IS707_MSG0_EX( MSG_LEGACY_MED, subs_id, "NV_PPP_USER_ID_I is used.");
        }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch(ppp_event)
  {
    case PPP_UP_EV:
      ...
      if(dev_opts == NULL)
      {
        DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "dssnet4i_handle_ppp_ev_cback():"
    ...
    case PPP_SUBS_LIMITED_TO_V4_EV:
      ...
      if ( !DS707_PDNCNTX_CB_IS_VALID(pdncntx_cb_ptr) )
      {
        DS_3GPP2_ERROR_FATAL("Cannot get PDN Control Block, do not proceed");
        return;
      }
...
/*===========================================================================
FUNCTION DSSNET4I_HANDLE_IFACE_EV_CBACK()

DESCRIPTION
  Called when the a particular event occurs on iface. This posts an appropriate
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  iface_ptr = instance_ptr->ps_iface_ptr;
  ...
  switch (instance_ptr->state)
  {
    case DSSNET4I_CLOSED_STATE:
      ...
      ---------------------------------------------------------------------*/
        ...
        if( instance_ptr->iface_down_ind_cb != NULL &&
            instance_ptr->ps_iface_ptr != NULL )
        {
      ...
      if ( instance_ptr->ppp_event_handle != NULL )
      {
        ...
        {
          DS_3GPP2_ERROR_FATAL( "Unable to dealloc PPP event handle");
        }
    ...
    case DSSNET4I_LINK_OPENING_WITH_SIP_STATE:
             ...
             ds707_pkt_mgr_get_current_sys_mode() == SYS_SYS_MODE_CDMA ) )
      {
        ...
        data_sess_profile_id = sip_profile_index + 
                             DATA_SESSION_NVRUIM_PROFILE_MIN;
        profile_subs_id = ds707_data_session_get_profile_subs_id(subs_id);
        ds707_data_session_set_iface_requesting_profile(
                instance_ptr->ps_iface_ptr, data_sess_profile_id, profile_subs_id);
      }
	...
	---------------------------------------------------------------------*/
        ...
        if( instance_ptr->ppp_event_handle == NULL )
        {
          ...
          {
            DS_3GPP2_ERROR_FATAL( "Unable to acquire a PPP event handle");
          }
        ...
        if ( !( ds707_pdncntx_get_ncp_started( pdncntx_cb_ptr ) ) )        
        {
          ...
          {
            DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "call to ppp_start failed!");
            (void)dssnet4_sm_post_event( instance_ptr,
                                         DSSNET4_LINK_DOWN_EV,NULL);
            return;
          }
	...
	if(ppp_config == NULL)
        {
          DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "dssnet4i_transition_state():"
                          "memory allocation failed for ppp_config");
          dssnet4i_transition_state( instance_ptr, DSSNET4I_CLOSED_STATE);
          break;    
        }
    ...
    case DSSNET4I_NET_UP_STATE:
      ...
      ---------------------------------------------------------------------*/
      if( ( instance_ptr->ps_iface_ptr != NULL ) &&
      	  ( instance_ptr->ps_iface_ptr->v4_net_info.primary_dns.ps_s_addr == 0 ) &&
      ...
      if( (DS707_PDNCNTX_CB_IS_VALID(pdncntx_cb_ptr)) &&
          ( ((PS_IFACE_IS_VALID(iface6_ptr)) &&
             (ps_iface_state(iface6_ptr) != IFACE_COMING_UP)) ||
            (!PS_IFACE_IS_VALID(iface6_ptr)) ) &&
          (ds707_pkt_mgr_get_current_sys_mode() == SYS_SYS_MODE_CDMA)
        ) 
      {
      ...
      /*---------------------------------------------------------------------
        register with Data path manager. 
      ---------------------------------------------------------------------*/
    ...
    case DSSNET4I_LINK_RESYNC_STATE:
      ...
      if(instance_ptr->is_ehrpd_call == TRUE)
      {
        ...
        if ( !DS707_PDNCNTX_CB_IS_VALID(pdncntx_cb_ptr) )
        {
          DS_3GPP2_ERROR_FATAL("Cannot get PDN Instance to set call bringup mode");
          return; 
        }
        /*-------------------------------------------------------------------
   ...
   case DSSNET4I_WAITING_FOR_MIP_DEREG_STATE:
    ...
    case DSSNET4I_LINK_CLOSING_STATE:
        ...
        if( ds707_pdncntx_get_ncp_started( pdncntx_cb_ptr ) )
        {
              ...
              && (ds707_pkt_is_ehrpd_mode_of_operation() == TRUE))
          {
                ...
                )  
            {
                      ...
                      ds707_pdncntx_get_ncp_handle( pdncntx_cb_ptr )) < 0)
              {
                ...
                -----------------------------------------------------------*/ 
                DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "call to ncp_stop failed! "
            ...
            else
            {
              if (ppp_ncp_abort(
                      ds707_pdncntx_get_ncp_handle( pdncntx_cb_ptr )) < 0)
              {
                DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "call to ncp_abort failed!"
                                " Post link_down to cleanup dssnet4");
                (void)dssnet4_sm_post_event( instance_ptr,
                                       DSSNET4_LINK_DOWN_EV,NULL);
                return;
          ...
          else /* NON-EPC or OOS */
          {
            ...
            if (FALSE == ds707_get_has_phone_acquired_signal())
            {
              ...
              if( (PS_IFACE_IS_VALID(instance_ptr->ps_iface_ptr)) &&
                  (! PS_IFACEI_IS_IN_ARBITRATION(instance_ptr->ps_iface_ptr))&&
...
#endif /*FEATURE_EHRPD */
      {
        ...
        {
          DATA_IS707_MSG0_EX(MSG_LEGACY_MED, subs_id, "Current system NON-EPC & "
...
{
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  DS3GPP2_ASSERT( instance_ptr != NULL );
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR, "Invalid cookie 0x%x", 
                    (*(uint8 *)eap_user_data));
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Invalid parameter passed");
    ret_val = FALSE;
  }
  return ret_val;
...
)  
{
 ...
 /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -----------*/
  ...
  ---------------------------------------------------------------------*/
  ppp_config_ptr->subs_id = ppp_get_subs_id( subs_id );
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  *err_num = (sint15) E_FAILURE;
          ...
          != (ret_val = ds3gcfgmgr_efs_tokenizer(efs_db_ptr, &from, &to )))
  {
    ...
    {
      DS3GPP2_ASSERT(DS3GCFGMGR_EFS_TOKEN_PARSE_FAILURE == ret_val);
      *err_num = (sint15) E_FAILURE;
      return -1;
    }
...
)
{
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Invalid parameter");
    return FALSE;
  }
  ...
  {
    DS_IS707_ERROR_LOG_3(MSG_LEGACY_ERROR, "EAP: IMSI invalid. "
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/   
  ...
  if( ( setup_fail == FALSE ) && ( ppp_auth_info_ptr->ehrpd_usim_present ) )
  {
    ...
    -----------------------------------------------------------------------*/
    if(FALSE != ( ds3g_mmgsdi_get_imsi_data_per_subs_id (&ehrpd_imsi , ds707_curr_subs_id()) ) )
    {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if (!(DSSNET4I_IS_VALID(instance_ptr)))
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Invalid dssnet4 instance_ptr");
...
)
{
  ...
  if (instance_ptr == NULL)
  {
    DS_3GPP2_ERROR_FATAL("Instance pointer for dssnet4_set_ps_local_abort is NULL");
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "dssnet4_sm_cb is NULL");
...
)
{
  ...
  if (NULL == dssnet4_sm_cb)
  {
    DS_3GPP2_ERROR_FATAL("dssnet4_sm_cb is NULL. Can't reset call end reason");
  }
...
)
{
  if (!(DSSNET4I_IS_VALID(instance_ptr)))
  {
    DS_3GPP2_ERROR_FATAL("dssnet4_sm_cb is not valid");
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if (!(DSSNET4I_IS_VALID(instance_ptr)))
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "instance_ptr is not valid");
    return FALSE;
  }
