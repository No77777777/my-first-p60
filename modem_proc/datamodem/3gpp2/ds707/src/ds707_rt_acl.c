...
$Header: //components/rel/data.mpss/3.5..2.1/3gpp2/ds707/src/ds707_rt_acl.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
                      
                             EDIT HISTORY FOR FILE

when       who     what, where, why
--------   ---     ----------------------------------------------------------
04/01/16    sd     Pick up non null APN profile for ALWAYS_SHARE_PROFILE
...
10/29/14    niv    Fix to disallow routing on v6 calls when 
                   MIPv6 is enabled 
10/29/14    sd     Clear ds707_omh_profile_id in route ACL while in eHRPD mode.
10/06/14    vm     Fix to not use OMH default profile while data call
                   is on eHRPD.
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if( (!(policy_info_ptr->lookup_only)) )
  {
      ...
      if( ds707_pdn_cntx_cb_ptr == NULL )
      {
            ...
            (!DS707_IFACE_TBL_IS_VALID( ds707_iface_tbl_ptr )) )
        {
          /*-------------------------------------------------------------------
            ...
            For now ASSERT(0)
          -------------------------------------------------------------------*/
          DS_3GPP2_ERROR_FATAL("pdn cb NULL or iface tbl ptr NULL" );
        }
          ...
          )
        {
          ...
          {
            DS_3GPP2_ERROR_FATAL("could not set iface address family");
          }
        ...
        {
          DS_3GPP2_ERROR_FATAL("could not resolve iface addr family");
        }
                       ...
                       &data_sess_profile_info_ptr ) )
        {
          ...
          /*lint -restore*/
          {
            ...
            else
            {
              ...
              if( !other_iface_ptr )
              {
                DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Pointer to iface is NULL" );
              }
              else
              {
                DATA_IS707_MSG3( MSG_LEGACY_HIGH, 
                                 ...
                                 other_iface_ptr->instance, //no issue
              ...
              if( ps_iface_set_addr_family(other_iface_ptr, other_iface_addr_family) 
                  == -1)
              {
                DS_3GPP2_ERROR_FATAL("could not set other iface address family");
              }
    ...
    if (dssnet_set_epc_handoff_ctxt(if_ptr, policy_info_ptr) < 0)
    {
      ...
      -----------------------------------------------------------------------*/
      DS_3GPP2_ERROR_FATAL("Error setting EPC handoff context, fail post proc");
      return;
    }
#endif /* FEATURE_EPC_HANDOFF */
#ifdef FEATURE_DATA_PS_IWLAN_3GPP2
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if( pending_id == DATA_SESSION_ALWAYS_SHARE_PROFILE )
  {
   ...
   DATA_IS707_C_MSG1(DS707_F3_MASK,"Pending ID is %d",pending_id);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  /*----------------------------------------------------------------------------
    For ALWAYS_SHARE_PROFILE, first check DSD preferred system so that only
    one MH will resolve profile. If 3GPP2 is preferred and if there's
    existing PDN up and running, skip pref_sys check, otherwise, search for
    proper profile for first bring up
  ----------------------------------------------------------------------------*/
  if (pending_id == DATA_SESSION_ALWAYS_SHARE_PROFILE)
  {
    mode = ds3g_get_current_network_mode_ex(resolved_subs_id);
    if ( (mode != SYS_SYS_MODE_CDMA) &&
         (mode != SYS_SYS_MODE_HDR) )
    {
      return FALSE;
    }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  -------------------------------------------------------------------------*/
  profile_reg_index = ds707_data_session_is_valid_profile(pending_id, ds707_curr_subs_id());
  
  if( profile_reg_index == DATA_SESSION_PROFILE_ID_INVALID )
  {
    DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR,"Invalid Profile ID %d",pending_id);
    *deny_reason = ACL_DENY_REASON_INVALID_PROFILE_ID;
    return FALSE;
  }
  ...
  {
    DATA_IS707_C_MSG0(DS707_F3_MASK,"HDR session not open");
    return TRUE;
  }
...
ACL_START
  ...
  PASS( ACL_IFNAME_CLASS, REQUIRED_IFACE_TYPE_IS( CDMA_SN_IFACE ) );
  PASS( ACL_PREF_MODE_CLASS,
        IFACE_IS_PREF_MODE_DEF2( ds707_is_pref_pkt_mode, 
                                 SYS_SYS_MODE_CDMA ));
  DENY_WITH_ANY_REASON( SPECIFIED_CONFIG_NOT_SUPPORTED2
  	                (ds707_rt_is_cfg_supported) );
  PASS( ACL_CONFIG_CLASS,
