...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp2/1xrlp/src/dsrlp3.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  *bits_left = *bits_left - RLP3_NAK_HOLE_CNT_LEN;
  ...
  DS3GPP2_ASSERT(num_naks <= RLP3_MAX_NAK_SEGS);
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  /*-------------------------------------------------------------------------
   ...
   {
      DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR, "Malformed frame, fwd_frame_ptr->fr_item_ptr = NULL");
  ...
  if (fwd_frame_ptr->fr_rate == DSRLP_RATE_1)
  {
    ...
    else if (mux_pdu_type == DSRLP_MUX_PDU_5)
    {
      ...
      if ((len_indicator & RLP3_MUXPDU5_LEN_IND_MASK) == 0x00)
      {
          ...
          DS3GPP2_ASSERT(cnt == sizeof(byte));      
      }
      ...
      else if ((len_indicator & RLP3_MUXPDU5_LEN_IND_MASK) == 0x08)
      {
         ...
         Byte   |         6 bits             | 2 bits            |   
          ...
          DS3GPP2_ASSERT(cnt == (sizeof(byte) * 2)); 
    ...
    else
    {
        ...
        -----------------------------------------------------------------*/
        DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR, "Invalid PDU Type");
        dsm_free_packet( &(fwd_frame_ptr->fr_item_ptr));
      ...
      )
    {
                                                ...
                                                == RLP3_FMT_D_DATA_SQI_MASK )
      {
        ...
        {
          DS_RLP_ERROR_LOG_1(MSG_LEGACY_ERROR, "Bad PDU - SW ERR %d", mux_pdu_type);
          dsm_free_packet( &(fwd_frame_ptr->fr_item_ptr));
	  DATA_RLP_MSG0(MSG_LEGACY_LOW, "return RLP_ERASURE");
          return(DSRLPI_RLP_ERASURE);
        }
  ...
  else
  {
      ...
      )
    {
                        ...
                        offset+(RLP3_CTL_FCS_BYTE_LEN*8))) != CRC_16_L_OK)
      {
        /*-------------------------------------------------------------------
         ...
         ------------------------------------------------------------------*/
        DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR, "Bad FCS for INIT frame");
        return (DSRLPI_RLP_ERASURE);
      }
       ...
       )
    {
      ...
      switch (nak_type)
      {
        ...
        default:
          DS_3GPP2_ERROR_FATAL("Unknown NAK Type");
                        ...
                        offset+(RLP3_CTL_FCS_BYTE_LEN*8))) != CRC_16_L_OK)
      {
        ...
        DS_RLP_ERROR_LOG_1(MSG_LEGACY_ERROR, "Bad FCS for NAK frame, type=%d, ",
                      nak_type);
        dsm_free_packet( &(fwd_frame_ptr->fr_item_ptr));
	DATA_RLP_MSG0(MSG_LEGACY_LOW, "return RLP_ERASURE");
        return (DSRLPI_RLP_ERASURE);
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  -----------------------------------------------------------------------*/
  DS3GPP2_ASSERT(frame_len <= DSM_DS_SMALL_ITEM_SIZ);

  item_ptr = dsm_new_buffer(DSM_DS_SMALL_ITEM_POOL);
  if (item_ptr == NULL)
  {
    DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR, "Running out of DSM small items.");
    return (FALSE);
  }
...
/*lint -restore Restore lint error 831*/
  ...
  /*------------------------------------------------------------------------
    Nak rounds fwd & rev.
  ------------------------------------------------------------------------*/
  DS3GPP2_ASSERT(state_ptr->nak_rounds_fwd < (1 << RLP3_SA_NAK_RNDS_FWD_LEN));
  ...
  DS3GPP2_ASSERT(state_ptr->nak_rounds_rev < (1 << RLP3_SA_NAK_RNDS_REV_LEN));
 ...
 /*lint -restore Restore lint error 774*/
  ...
  DS3GPP2_ASSERT((bit_pos - ((bit_pos/(int)BITS_IN_OCTET) * (int)BITS_IN_OCTET)) == 0);
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  {
    DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR, "Called tx_naks when boolean false");
    return FALSE;
  }
...
//  }
                                   ...
                                   &frame_len) == FALSE)
  {
    if (dsrlpi_get_min_frame_and_len(rev_fr_ptr,
                                     ...
                                     &frame_len) == FALSE)
    {
      DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR, "Rev fr too small for NAK frame Tx");
      return(FALSE);
    }
  }
  ...
  if (ret_id == FALSE)
  {
    ...
    DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR,"No naks to send, so returning false.");
    return FALSE;
  }
  ...
  if (dsrlpi_get_best_fr_rate(rev_fr_ptr,bytes_used) == DSRLP_RATE_1_2)
  {
    ...
    DS3GPP2_ASSERT(rev_fr_ptr->fr_rate == DSRLP_RATE_1_2);
  }
...
//  }
      ...
      (rrcb_ptr->state.rnd_trip_fr_cntr == 0))
  {
    ...
    {
      DS3GPP2_ASSERT(rrcb_ptr->state.idle_timer > 0);
