...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp2/1xrlp/src/dsrlpi.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $

when       who        what, where, why
--------   ---        -------------------------------------------------------
04/19/16   sd         Fix to clean up Tx watermark only when RLP reset is 
                      triggered in case of HDR.
07/15/15   sd         Fix to indicate 1X to clear reverse link RLP frame 
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - -  - - - - - - -*/
  ...
  {
    DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR," No frames to rexmit, but in rexmit func");
    return FALSE;
  }

  if (dsrlprtx_get_rexmit_last_seq(rrcb_ptr, &nak_last_seq) == FALSE)
  {
    DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR, " bad last seq info in rtx queue");
  ...
  do
  {
    ...
    else if (next_seq_num == DSRLPSEQ_NO_SEQ)
    {
      ...
      ---------------------------------------------------------------------*/
      DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR, "Need to Rxmit, but txq is empty");
      dsrlprtx_reset(rrcb_ptr);
      return FALSE;
    }
    else
    {
      DATA_RLP_MSG1(MSG_LEGACY_LOW, "Did not find first_seq 0x%x in txq",nak_first_seq);
      /*---------------------------------------------------------------------
      ...
      {
        DATA_RLP_MSG0(MSG_LEGACY_LOW, "Rxmit info no longer in txq");                   
        /*-------------------------------------------------------------------
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  DS3GPP2_ASSERT( (rev_fr_ptr != NULL) &&
                 (fr_len != 0) &&
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  switch ( ver)
  {
    ...
    case DSRLP_VER_HDR_MULTIFLOW_REVERSE:
     ...
     default:
       DS_RLP_ERROR_LOG_1(MSG_LEGACY_ERROR, "Invalid RLP Version %d",(int)ver);
       DS3GPP2_ASSERT( 0);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/  
  if ( rrcb_ptr == NULL )
  {
    DS_RLP_ERROR_LOG_1(MSG_LEGACY_ERROR, "Invalid rrcb_ptr : %d", rrcb_ptr);
    return;     
  }
  ...
  /*------------------------------------------------------------------------
    ...
    Clean up Tx watermark as well in case of HDR as this is not done by 
    DS707 pkt mgr while bringing down the traffic channel.
  -------------------------------------------------------------------------*/
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/  
                             ...
                             == CURR_FRAME_SYNC_MASK)
  {
    switch(state_info_ptr->state)
    {
      ...
      case DSRLPI_ESTABLISHED_STATE:
        ...
        DS_RLP_ERROR_LOG_1(MSG_LEGACY_ERROR, "Invalid SYNC, reset RLP, state=%d",
                      state_info_ptr->state);
        break;

      default:
        DS_RLP_ERROR_LOG_1(MSG_LEGACY_ERROR, "Invalid RLP state %d",
            ...
            == (CURR_FRAME_SYNC_MASK | CURR_FRAME_ACK_MASK))
  {
    switch(state_info_ptr->state)
    {
      ...
      case DSRLPI_RT_MEAS_STATE:
      case DSRLPI_ESTABLISHED_STATE:
        DS_RLP_ERROR_LOG_1(MSG_LEGACY_ERROR, "Invalid SYNC/ACK, drop frame, "
                      "state=%d", state_info_ptr->state);
        break;

      default:
        DS_3GPP2_ERROR_FATAL( "Invalid RLP State");
        break;
    }
    DATA_RLP_MSG2(MSG_LEGACY_MED, 
                                 ...
                                 ==  CURR_FRAME_ACK_MASK)
  {
    switch(state_info_ptr->state)
    {
      ...
      case DSRLPI_ESTABLISHED_STATE:
        DS_RLP_ERROR_LOG_1(MSG_LEGACY_ERROR, "Invalid ACK, drop frame, state=%d",
                      state_info_ptr->state);
        break;
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  {
    DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR, "null dsm item ptr");
    return (DSRLPI_DONE);
  }
  ...
  else
  {
        ...
        if (DSRLPSEQ_GT_SEQ_CHECK(rxed_seq,exp_seq,modulus))
        {
             ...
             )
          {
            ...
            {   /* changed DATA_RLP_MSG2(MSG_LEGACY_HIGH to DATA_RLP_MSG2(MSG_LEGACY_ERROR to help testers. */
                DS_RLP_ERROR_LOG_2(MSG_LEGACY_ERROR, "BURST GAP seq %x Vr %x",
                          curr_info_ptr->seq_num, 
            ...
            else
            {
              ...
              rrcb_ptr->state.big_nak_gap = TRUE;
                /* changed DATA_RLP_MSG2(MSG_LEGACY_HIGH to DATA_RLP_MSG2(MSG_LEGACY_ERROR to help testers. */
              DS_RLP_ERROR_LOG_2(MSG_LEGACY_ERROR, "FCH RDA gap->seq %x Vr %x",
                        curr_info_ptr->seq_num,
                        rrcb_ptr->state.expected_seq_num
                      );
            }
    ...
    if (DSRLPSEQ_GT_SEQ_CHECK(rxed_seq,exp_seq,modulus))
    {
      ...
      {
        DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR, "Could not insert into NAK list");
        rrcb_ptr->state.reset_rlp = TRUE;
      }
    }
  }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  DS3GPP2_ASSERT(fwd_fr_ptr  != NULL); 

  if (fwd_fr_ptr->fr_item_ptr == NULL)
  {
    DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR, "Null ptr to rexmit processing");
    return (DSRLPI_DONE);
  }
      ...
      == TRUE)
  {
    ...
    {
      DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR, "nak remove failed, so reset");
      rrcb_ptr->state.reset_rlp = TRUE;
    }
  }
  else
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if ( state_ptr->tx_item_ptr == NULL)
  {
    ...
    DS3GPP2_ASSERT( state_ptr->tx_item_ptr != NULL);
...
/*-------------------------------------------------------------------------
  ...
  if ( ( payload_in_wm < payload_in_frame)              || 
       ...
       (tx_seq_hi==TRUE))
   {
        ...
        ( rev_fr_ptr->mux_pdu_type == DSRLP_MUX_PDU_2))
    {
      ...
      if ( (tx_seq_hi==TRUE) || (tx_s_seq==TRUE))
      {
        ...
        payload_in_frame       = rev_fr_ptr->max_frame_size 
                               - new_fr_item_ptr->used;
        payload_in_frame       = MIN( payload_in_frame, payload_in_wm);
        DS3GPP2_ASSERT( payload_in_frame < 32);
      ...
      else
      {
        ...
        payload_in_frame       = rev_fr_ptr->max_frame_size 
                               - new_fr_item_ptr->used;
        payload_in_frame       = MIN( payload_in_frame, payload_in_wm);
        DS3GPP2_ASSERT( payload_in_frame < 64);
    ...
    else  /* MuxPDU type 3 and type 5 - format D*/
    {
      ...
      {
        DS_3GPP2_ERROR_FATAL( "Invalid MUX PDU path");
      }
      ...
      if ( rev_fr_ptr->mux_pdu_type == DSRLP_MUX_PDU_3)   
      {
        ...
        payload_in_frame       = rev_fr_ptr->max_frame_size
                               - new_fr_item_ptr->used; 
        payload_in_frame       = MIN( payload_in_frame, payload_in_wm);
        DS3GPP2_ASSERT( payload_in_frame < 256);
  ...
  while( payload_in_frame >0)
  {
    ...
    DS3GPP2_ASSERT( new_fr_item_ptr->size >= new_fr_item_ptr->used );
    ...
    else /* tx_item_ptr != NULL */
    {
      ...
      if( tx_item_len == 0)
      {
        DS_RLP_ERROR_LOG_2(MSG_LEGACY_ERROR, "Zero len item curr=%d dont_ex=%d",
  ...
  else  /* Not rate_1 - FCH frames */
  {
    ...
    {
      DS_RLP_ERROR_LOG_2(MSG_LEGACY_ERROR, " fr_rate mask=%x len=%d Donot match",
  ...
  if ( rev_fr_ptr->fr_rate == DSRLP_RATE_1)
  {
    if( ( rev_fr_ptr->mux_pdu_type == DSRLP_MUX_PDU_1) || 
        ( rev_fr_ptr->mux_pdu_type == DSRLP_MUX_PDU_2))
    {
      DS3GPP2_ASSERT( head_item_ptr->pkt_ptr == NULL);
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  *fr_len  = dsrlp_get_frame_len( rev_fr_ptr->fr_rate,
  ...
  DS3GPP2_ASSERT(rev_fr_ptr->max_frame_size >= *fr_len);
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  DS3GPP2_ASSERT(data_payload_len > 0);
  ...
  if (octet_seq == TRUE)
  {
    DS3GPP2_ASSERT(seg_allowed == TRUE);
  ...
  else if (seg_allowed == TRUE)
  {
    ...
    else if (DSRLPSEQ_GT_SEQ_CHECK(fr_last_seq, nak_last_seq, modulus))
    {
      ...
      ---------------------------------------------------------------------*/
      DS3GPP2_ASSERT(DSRLPSEQ_HAS_S_SEQ(nak_first_seq) ||
             DSRLPSEQ_HAS_S_SEQ(nak_last_seq));
      ...
      (snd_fr_type == RLPI_SND_FR_FMT_C))
  {
    ...
    do
    {
      ...
      {
        DS3GPP2_ASSERT(curr_item_ptr->used == curr_item_ptr->size);
    ...
    if (snd_fr_type == RLPI_SND_FR_FMT_B)
    {
      ...
      {
        DS_RLP_ERROR_LOG_2(MSG_LEGACY_ERROR, "Invalid index for dsrlp_len_table %d, %d", 
                      srvc, mux_pdu);
        return FALSE;
      }
      ...
      -------------------------------------------------------------------*/
      DS3GPP2_ASSERT((curr_item_ptr->used > 0) &&
  ...
  else if ((seg_allowed == FALSE) || (seg_frame_fits == TRUE))
  {
    ...
    -----------------------------------------------------------------------*/
    DS3GPP2_ASSERT( (octet_seq == FALSE) &&
            ((mux_pdu == DSRLP_MUX_PDU_1) || (mux_pdu == DSRLP_MUX_PDU_2)) &&
            ((snd_fr_type == RLPI_SND_FR_FMT_A)     || 
           (snd_fr_type == RLPI_SND_FR_FMT_NONE)) &&
    ...
    -----------------------------------------------------------------------*/
    DS3GPP2_ASSERT((copy_len <= (fr_item_ptr->size - RLP3_DATA_UNSEG_HDR_SIZE)) &&
           ...
           (mux_pdu == DSRLP_MUX_PDU_2))
  {
    ...
    *byte_ptr |= dsrlpi_type_mask_table[mux_pdu][srvc].format_a;
    ...
    if (rev_fr_ptr->fr_rate == DSRLP_RATE_ERASURE)
    {
      DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR, "seg frame not big enough");
  ...
  if (octet_seq == TRUE)
  {
    ...
    {
      DS3GPP2_ASSERT(nak_first_seq == nak_last_seq);
      dsrlprtx_remove_first_element(rrcb_ptr);
    }
    else
    {
