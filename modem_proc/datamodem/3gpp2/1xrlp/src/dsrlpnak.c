...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp2/1xrlp/src/dsrlpnak.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if (dup == NULL)
  {
    DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR, "No nak items in nak free q");
  }
  else
  {
    dup->first_seq = first;
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  while (temp_ptr != NULL)
  {
    ...
    DS3GPP2_ASSERT(DSRLPSEQ_GE_SEQ_CHECK(last_num,first_num,modulus));
             ...
             ((first_equal == TRUE) || (first_num_greater == FALSE)))
    {
      ...
      else if (DSRLPSEQ_HAS_SAME_SEQ(last_num,temp_ptr->last_seq)) 
      {
        ...
        ------------------------------------------------------------------*/
        DS3GPP2_ASSERT(DSRLPSEQ_HAS_S_SEQ(temp_ptr->last_seq));
             ...
             ((last_equal == TRUE) || (last_num_greater == TRUE)))
    {
      ...
      else
      {
        /*------------------------------------------------------------------
          ...
          insert it.
        ------------------------------------------------------------------*/
        DS3GPP2_ASSERT(!DSRLPSEQ_HAS_SAME_SEQ(temp_ptr->first_seq,temp_ptr->last_seq));
    ...
    else
    {
      ...
      ---------------------------------------------------------------------*/
      DS3GPP2_ASSERT((first_num_greater == TRUE) &&
                    (last_num_greater == FALSE) &&
                    (last_equal == FALSE));
      ...
      else 
      {
        ...
        else
        {
          /*----------------------------------------------------------------
            ...
            4->8              6a->6b        4->6,6->6a
          ----------------------------------------------------------------*/
          DS3GPP2_ASSERT(!DSRLPSEQ_HAS_S_SEQ(temp_ptr->first_seq));
...
)
{
  ...
  if ( nak_entry_ptr->curr_rnd <= state_ptr->nak_rounds_rev)
  {
    ...
    else
    {
      if (nak_entry_ptr->curr_rnd >= DSRLP_MAX_NAK_ROUNDS)
      {
        DS_RLP_ERROR_LOG_1(MSG_LEGACY_ERROR, "Array index for naks_per_round_rev array is out of bounds %d",
                      nak_entry_ptr->curr_rnd);
        return FALSE;
      }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  while (first_num != last_num)
  {
    ...
    if (nak_entry_ptr == NULL)
    {
      DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR, "Ran out of nak items in free q");
      rex_leave_crit_sect(&(rrcb_ptr->state.data_path_crit_sect));
      return FALSE;
    }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if (rlpnaki_remove(rrcb_ptr,first_num,last_num,temp_ptr) == FALSE)
  {
    ...
    -----------------------------------------------------------------------*/
    DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR, "Internal mem error in nak remove");
    rex_leave_crit_sect(&(rrcb_ptr->state.data_path_crit_sect));
    return(FALSE);
  }
