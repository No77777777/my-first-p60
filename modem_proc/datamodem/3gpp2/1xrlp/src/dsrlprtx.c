...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp2/1xrlp/src/dsrlprtx.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
typedef struct 
{
  ...
  uint32        next_ptr;           /* points to next  item                    */
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  rlprtx_pool[first_ele].next_ptr = first_ele;
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if (first_ele != last_ele)
  {
    ...
    rlprtx_pool[new_last_ele].next_ptr = first_ele;
  }
  else
  {
    /*-----------------------------------------------------------------------
  ...
  rlprtx_pool[last_ele].next_ptr = last_ele;
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  DS3GPP2_ASSERT(element <= RLPRTX_MAX_ELEMENTS);
  ...
  if (*rtxq_ptr == DSRLPRTX_NO_Q)
  {
    ...
    -----------------------------------------------------------------------*/
    *rtxq_ptr = element;
    rlprtx_pool[element].next_ptr = element;
  ...
  else
  {
    ...
    rlprtx_pool[old_last].next_ptr = element;
    rlprtx_pool[element].next_ptr  = *rtxq_ptr;
  }
