...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp2/1xrlp/src/dsrlp.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
)
{
  ...
  else
  {
    ...
    -----------------------------------------------------------------------*/
    DS_RLP_ERROR_LOG_1(MSG_LEGACY_ERROR,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  for(i=0;i<DSRLP_MAX_RSCB_ELEMENTS;i++)
  {
    ...
    for (p = 0; p < DSRSP_NUM_ROUTES; ++p)
    {
       ...
       * Note: Both these queues are used only if FEATURE_DS_HDR_REVB is defined
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS3GPP2_ASSERT((sr_id < DSRLPI_MAX_CDMA_SR_ID_VALUE) ||
  ...
  for (p = 0; p < DSRSP_NUM_ROUTES; ++p)
  {
    ...
    if( rscb_ptr->is_hdrmrlp_active == TRUE )
    {
      /* If the MRLP state is active, version has to be MRLP Type */
        DS3GPP2_ASSERT( (rscb_ptr->cfg.ver == DSRLP_VER_HDR_MULTIFLOW_FORWARD) ||
                (rscb_ptr->cfg.ver == DSRLP_VER_HDR_MULTIFLOW_REVERSE));

      if(( ver != rscb_ptr->cfg.ver ) ||
           ( sr_id != rscb_ptr->cfg.sr_id))
      {
        DS_RLP_ERROR_LOG_3(MSG_LEGACY_ERROR, "Cant init active "
    ...
    else /* rlp is not active */
    {
      ...
      if(( ver == DSRLP_VER_HDR_MULTIFLOW_FORWARD ) ||
         ( ver == DSRLP_VER_HDR_MULTIFLOW_REVERSE ))
      {
        DS_RLP_ERROR_LOG_3(MSG_LEGACY_ERROR, "Cant init MRLP "
  ...
  for (p = 0; p < DSRSP_NUM_ROUTES; ++p)
  {
    ...
    if(ver == DSRLP_VER_1X_3)
    {
      ...
      DS3GPP2_ASSERT(rrcb_ptr->state.del_fr_id != DSRLPDEL_NO_ARRAY);
    }
#endif
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR, 
  ...
  /*
   ...
   function based on FEATURE_DS_HDR_REVB
  */
#if defined (FEATURE_DS_HDR_REVB)
  ...
  if (dsrlp_blob_ptr->rlp_info_incl)
  {
    ...
    {
      DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR, "RLP BLOB DID NOT PARSE CORRECTLY!!!");
    }
  }
...
)
{
  ...
  switch (ds_cmd_ptr->hdr.cmd_id)
  {
    ...
    default:
      DS_IS707_ERROR_LOG_1(MSG_LEGACY_ERROR, "Unknown RLP cmd %d", ds_cmd_ptr->hdr.cmd_id );
      break;
  }
} /* dsrlp_process_cmd */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS3GPP2_ASSERT(( ver == DSRLP_VER_HDR_MULTIFLOW_FORWARD ) ||
         ( ver == DSRLP_VER_HDR_MULTIFLOW_REVERSE ));
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  ---------------------------------------------------------------------*/
  DS3GPP2_ASSERT(( dsrlpi_rscb_array[rscb_index].is_hdrmrlp_active == TRUE ) &&
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if (rscb_index == DSRLPI_INVALID_RSCB_INDEX)
  {
    ...
    DS_RLP_ERROR_LOG_2(MSG_LEGACY_ERROR, "Did not find RLP %d ver %d",sr_id,ver);
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  switch( ver )
  {
    ...
    default:
      DS_3GPP2_ERROR_FATAL("Cant find RLP ver");
      break;
  }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  DS3GPP2_ASSERT(rscb_ptr->cfg.ver != DSRLP_VER_HDR_MULTIFLOW_REVERSE &&
         rscb_ptr->cfg.ver != DSRLP_VER_HDR_MULTIFLOW_FORWARD);
  ...
  DS3GPP2_ASSERT( fcn_ptr_table != NULL);
  ...
  {
    DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR, "SYNC state Timeout");
    /*-----------------------------------------------------------------------
  ...
  {
    DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR,"Reset due to too many old frames from BS");
    rrcb_ptr->state.reset_rlp = TRUE;          /*Set the reset flag*/
  }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  DS3GPP2_ASSERT(tx_blob_ptr != NULL);
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  /* 1x has only one route */
  DS3GPP2_ASSERT((rscb_ptr->cfg.ver == DSRLP_VER_1X_3) && 
         (rscb_ptr->cfg.ver != DSRLP_VER_HDR_MULTIFLOW_REVERSE) &&
         (rscb_ptr->cfg.ver != DSRLP_VER_HDR_MULTIFLOW_FORWARD));
  rrcb_ptr = &rscb_ptr->rlp_list[DSRSP_ROUTE_A];
  ...
  DS3GPP2_ASSERT( max_to_set_fwd <= DSRLP_MAX_NAK_ROUNDS);
  ...
  -------------------------------------------------------------------------*/
  DS3GPP2_ASSERT( max_to_set_rev <= DSRLP_MAX_NAK_ROUNDS);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  -------------------------------------------------------------------------*/
  DS3GPP2_ASSERT(rscb_index == DSRLP_FIXED_SR_ID_INDEX);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  else
  {
    ...
    -----------------------------------------------------------------------*/
    DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR, "BAD SR ID");
    return(0);
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
     DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR, "invalid rlp_type in timetest rlp logging");
     return;
  }
...
)
{
  ...
  if(rx_wm_ptr == NULL)
  {
    DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR, "No Rx WM for Rx SDB");
    return FALSE;
  }
  ...
  {
    DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR, "SDB Write to non-empty WM");
  }
  ...
  else
  {
    ...
    -----------------------------------------------------------------------*/
    DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR, "No function for SDB Rx");
    return FALSE;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS3GPP2_ASSERT(qos_blob_p != NULL);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS3GPP2_ASSERT(rx_qos_blob_p != NULL);
...
snm_status_type dsrlp_register_snm_initializers(void)
{
  ...
  {
    DS_3GPP2_ERROR_FATAL("Could not register initializer");
  }
...
)
{
  ...
  if( rscb_index == DSRLPI_INVALID_RSCB_INDEX )
  {
    DS_RLP_ERROR_LOG_2(MSG_LEGACY_ERROR,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if( rscb_index == DSRLPI_INVALID_RSCB_INDEX )
  {
    DS_RLP_ERROR_LOG_2(MSG_LEGACY_ERROR, "RLP %d ver %d not found. "
...
#else
  DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR,"dsrlp_any_sig_to_send() not supported");
  return FALSE;
#endif
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if (missed_txc == TRUE)
  {
    DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR, "RLP didn't build frames in time?");
  }
  ...
  {
    DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR, "is_1xrlp_ready is FALSE. 1X RLP not Initalized yet" );
    return;
  }
  ...
  for( fr_num =0; fr_num< rev_fr_blk->max_frames; fr_num++)
  {
       ...
       )
    {
      ...
      {
        DS3GPP2_ASSERT( fcn_ptr_table->tx_init != NULL);
    ...
    {
      DS3GPP2_ASSERT(frame_built == FALSE );
      //frame_built= TRUE;
      rex_leave_crit_sect(&(rrcb_ptr->state.data_path_crit_sect));
      continue;
    }
    ...
    if ( ((rrcb_ptr->state).state   != DSRLPI_ESTABLISHED_STATE) &&
         ((rrcb_ptr->state).state   != DSRLPI_RT_MEAS_STATE))
    {
      DS_RLP_ERROR_LOG_3(MSG_LEGACY_ERROR, "Did not send SYNC/ACK or FCH/DCCH "
    ...
    if( ((rev_fr_ptr->fr_cat_mask) & DSRLP_CAT_CTL_MASK) &&
        ((rrcb_ptr->state).naks_to_send == TRUE))
    {
      DS3GPP2_ASSERT( fcn_ptr_table->tx_nak != NULL && (frame_built == FALSE ));
    ...
    {
      DS3GPP2_ASSERT( fcn_ptr_table->tx_fill != NULL);
      ...
      {
        DS_RLP_ERROR_LOG_1(MSG_LEGACY_ERROR,"Could not construct FILL frame RLP:%d",
                      rscb_ptr->cfg.sr_id);
      }
    }
    ...
    {
      DS3GPP2_ASSERT( fcn_ptr_table->tx_rexmit != NULL);
    ...
    if( (frame_built == FALSE) &&
        ...
        ( ((rrcb_ptr->state).tx_item_ptr != NULL) ||
          (((rscb_ptr->cfg).tx_wm_ptr    != NULL) &&
           ((rscb_ptr->cfg).tx_wm_ptr->current_cnt != 0)) ))
    {
      DS3GPP2_ASSERT( fcn_ptr_table->tx_new_data != NULL);
    ...
    if( frame_built == FALSE)
    {
      ...
      if ((rev_fr_ptr->fr_cat_mask) & DSRLP_CAT_IDLE_MASK)
      {
        ...
        -------------------------------------------------------------------*/
        DS3GPP2_ASSERT( fcn_ptr_table->tx_idle != NULL);
        ...
        else
        {
          ...
          rev_fr_ptr->fr_rate = DSRLP_RATE_BLANK;
          DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR, "Could not make FCH IDLE frame");
        }
      } /* if idle mask */
      rex_leave_crit_sect(&(rrcb_ptr->state.data_path_crit_sect));
      break;
...
#else
  DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR, "dsrlp_build_tx_frames() not supported");
#endif
  return;
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  -------------------------------------------------------------------------*/
  stats_ptr->tx_20ms_fr_cnt++;
  DATA_RLP_MSG1(MSG_LEGACY_MED, " stats_ptr->tx_20ms_fr_cnt %x", stats_ptr->tx_20ms_fr_cnt);
  ...
  while (temp_ptr != NULL)
  {
    ...
    if (DSRLPSEQ_HAS_S_SEQ(first))
    {
      ...
      {
        DS3GPP2_ASSERT(DSRLPSEQ_HAS_SAME_SEQ(first,last));
      }
      else
      {
        DS3GPP2_ASSERT(DSRLPSEQ_GET_NEXT_SEQ(first,modulus) == last);
      }
    }
    ...
    DATA_RLP_MSG2(MSG_LEGACY_MED, 
                  "nak_entry_ptr->first_seq %d nak_entry_ptr->last_seq %d",
                  nak_entry_ptr->first_seq, nak_entry_ptr->last_seq);
    ...
    DATA_RLP_MSG2(MSG_LEGACY_MED, 
                  "Initial Value - nak_entry_ptr->age %d  fr_missed %d",
    ...
    if(nak_entry_ptr->age > fr_missed)
    {
     ...
     {
        DATA_RLP_MSG4(MSG_LEGACY_MED, "Updating Age nak_entry_ptr->age %d "
                      "fr_missed %d nak_entry_ptr->curr_rnd %d  "
                      "nak_entry_ptr->naks_left  %d",
                      nak_entry_ptr->age, fr_missed, nak_entry_ptr->curr_rnd,
        ...
        --------------------------------------------------------------------*/ 
      DS3GPP2_ASSERT(nak_entry_ptr->curr_rnd <= DSRLP_MAX_NAK_ROUNDS);  
      ...
      if (nak_entry_ptr->age == 0)
      {
        ...
        if(nak_entry_ptr->naks_left > 0)
        {
          ...
          DATA_RLP_MSG2(MSG_LEGACY_MED, 
                        "nak_entry_ptr->naks_left %d  fr_missed %d",
                        nak_entry_ptr->naks_left, fr_missed);
        }
    ...
    DATA_RLP_MSG3(MSG_LEGACY_HIGH, " nak_aborted %d  fr_missed %d age %d",
                  nak_aborted, fr_missed, nak_entry_ptr->age);
    /*--------------------------------------------------------------------
...
#else
  DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR, "dsrlp_update_ta_gap() not supported");
#endif
  return;
} /* dsrlp_update_ta_gap */
...
#else
  DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR, "dsrlp_check_sr_id() not supported");
  return FALSE;
#endif
}
...
#else
  DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR, "dsrlp_get_sr_id() not supported");
  return 0;
#endif
}
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /*-----------------------------------------------------------------------
    ...
    if (data_len !=3)
    {
        DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR, "Not enough data to insert Null Hdr");
        return;
    }
    ...
    else if (pkt_len_indicator == 0x0C)
    {
       /*--------------------------------------------------------------------
          ...
          the code.
       --------------------------------------------------------------------*/
       DS_RLP_ERROR_LOG_1(MSG_LEGACY_ERROR, "Invalid Len Indicator %x",pkt_len_indicator);
       return;
    }
    else
    {
    ...
    {
        DS_RLP_ERROR_LOG_2(MSG_LEGACY_ERROR, "offset:%x,Out_ptr:%x;Should not come here",offset,out_pkt_ptr);
    }
#else
  DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR,
...
)
{
  ...
  {
    DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR, "Invalid index into dsrlp_log_id_table");
...
)
{
  ...
  {
    DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR, "Invalid index into dsrlp_len_table");
    return 0;
  }
...
)
{
  if(fr_rate >= DSRLP_RATE_ERASURE ||
     ...
     mux_pdu_type >= DSRLP_MUX_PDU_3)
  {
    DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR, "Invalid index into dsrlp_used_len_table");
    return 0;
  }
...
)
{
  ...
  {
    DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR,"Invalid RSCB Index");
