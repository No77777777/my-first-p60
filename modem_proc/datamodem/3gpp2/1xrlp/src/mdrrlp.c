...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp2/1xrlp/src/mdrrlp.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  {
    DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR, "Invalid Re-xmit, reset RLP");
  ...
  {
    DS_RLP_ERROR_LOG_2(MSG_LEGACY_ERROR, "Re-xmit requested is too old, "
                  "re-xmit/min = %u/%u",
  ...
  {
    DS_RLP_ERROR_LOG_1(MSG_LEGACY_ERROR, "Re-xmit frame not found: %u",
                  first_seq_num);
  ...
  {
    DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR, "Suspicious Re-xmit truncated");
  ...
  {
    DS_RLP_ERROR_LOG_1(MSG_LEGACY_ERROR, "%u re-xmits not queued", frames_to_queue);
    rlp_gen_stats.re_xmits_missed++;
  }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  switch( frame_ptr->gen_data.ctl_len & RLP_GEN_CTL_CHK_MASK)
  {
    ...
    case RLP_REXMIT_UPPR_BITS:
      ...
      *frame_typ = RLP_REXMIT_DATA_FR;
      ...
      {
         DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR,"Rexmit bit set for RLP Type 1 frame");
     ...
     ======================================================================*/
           ...
           RLP_SEGFR_FILL_FR_BITS) 
      {
         ...
         else  /* Fill frame received not in rateset2 */
         {
           ...
           DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR, "Invalid Frame rxed");
    ...
    default:
      ...
      *frame_typ   = RLP_CONTROL_FR;
      return_val   = RLP_CTL_LEN_ERR;
      DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR, "RLP frame with invalid CTL-LEN field");
  } /* switch */
  ...
  {
   seg_fr_cnt            = 0;
   DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR, "Resetting segmented receive");
  ...
  if(( frame_ptr->gen_ctl.ctl_len & RLP_SYNC_MASK) == RLP_SYNC_ENABLED)
  {
    switch( rlp_state_info.state)
    {
      ...
      case RLP_ESTABLISHED_STATE:
        ...
        -------------------------------------------------------------------*/
        DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR, "Invalid SYNC, reset RLP");
        rlp_gen_stats.resets++;
        rlp_reset();
        return;
  ...
  if(( frame_ptr->gen_ctl.ctl_len & RLP_ACK_MASK) == RLP_ACK_ENABLED)
  {
    switch( rlp_state_info.state)
    {
      ...
      case RLP_ESTABLISHED_STATE:
        ...
        -------------------------------------------------------------------*/
        DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR, "ACK when ESTABLISHED, reset RLP");
        rlp_gen_stats.resets++;
        rlp_reset();
        break;
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  {
    DS_RLP_ERROR_LOG_2(MSG_LEGACY_ERROR, "RLP Reset: Max V(N/R), %d/%d",
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  while( nak_ptr != NULL )
  {
    ...
    if( nak_ptr->tx_done )
    {
      ...
      if( rlp_new_or_idle_rcvd )
      {
                 ...
                 (nak_ptr->age ==  RLP_MAX_HOLE_AGE) )
        {
          ...
          {
            DS_RLP_ERROR_LOG_2(MSG_LEGACY_ERROR,"NAK entry too old, seq/age: %u/%u",
    ...
    if( abort )
    {
      if( nak_ptr->seq >= rlp_state_info.needed_seq_num )
      {
        ...
        -------------------------------------------------------------------*/
        DS_RLP_ERROR_LOG_3(MSG_LEGACY_ERROR, "RLP frame aborted: %u, V(N/R): %u/%u",
                      nak_ptr->seq,
  ...
  {
    DS_RLP_ERROR_LOG_1(MSG_LEGACY_ERROR, "rlp_rx_frame_count exceeds the rlp_rx_frame_list boundary %d",
                  rlp_rx_frame_count);
  ...
  for(i = 0; i < rlp_rx_frame_count; i++)
  {
                     ...
                     rlp_state_info.expected_seq_num) )
    {
          ...
          current_seq = (current_seq + 1) % RlpSeqModulus)
      {
        ...
        {
          DS_RLP_ERROR_LOG_2(MSG_LEGACY_ERROR, "No free NAKs V(R)/rx'ed: %u/%u",
                        rlp_state_info.expected_seq_num,
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  else if( GT_SEQ_CHECK( current_seq_num, rlp_state_info.needed_seq_num))
  {
    ...
    if( rlp_reassembly_buf[buf_index].item_ptr != NULL )
    {
      ...
      {
        DS_RLP_ERROR_LOG_3(MSG_LEGACY_ERROR,
  ...
  else
  {
    ...
    ------------------------------------------------------------------------*/
    DS_RLP_ERROR_LOG_3(MSG_LEGACY_ERROR, "Old duplicate: %u, V(N/R): %u/%u",
                  current_seq_num,
                  rlp_state_info.needed_seq_num,
                  rlp_state_info.expected_seq_num);
    (void)dsm_free_buffer(new_item_ptr);
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  {
    DS_3GPP2_ERROR_FATAL( "No TX Watermark item");
  }
  ...
  {
    DS_3GPP2_ERROR_FATAL( "No Post RX Function");
  }
  ...
  {
    DS_3GPP2_ERROR_FATAL( "No Post RX Queue");
  }
  else /* assign the watermark pointer */
  {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_3GPP2_ERROR_FATAL( "Invalid HDLC instance");
  }
  else
  {
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  {
    DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR, "Unknown RLP Type, can't get frame seq num");
  }
  ...
  for(ch_num = 0; ch_num < (fwd_frame_block->num_sup + 1); ch_num++)
  {
    ...
    case RLP_EIGHTH_RATE:
      ...
      if( rlp_traffic_type == RLP_TRAFFIC_TYPE_PRI )
      {
        if( nr_table[ frame_ptr->pri_idle.seq_num] != frame_ptr->pri_idle.nr_crc )
        {
          ...
          if( rlp_state_info.state == RLP_ESTABLISHED_STATE )
          {
            DS_RLP_ERROR_LOG_3(MSG_LEGACY_ERROR, "RLP Idle CRC error: 0x%x, 0x%x, 0x%x",
                 ...
                 ((frame_ptr->pri_idle.rs2_filler & RLP_RS2_IDLE_FILL_MASK) != RLP_RS2_IDLE_LAST_BITS) )
        {
          ...
          if( rlp_state_info.state == RLP_ESTABLISHED_STATE)
          {
            DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR, "RLP RS 2 non-zero filler");
          }
          rlp_pri_frate_errs.idle_fr_err++;
          frame_status = RLP_RX_FRAME_ERR;
          break;
          ...
          else if( last2_idles_same )
          {
            ...
            else /* RLP must Re-sync */
            {
              /*-------------------------------------------------------------
                ...
                RLP and return.
              -------------------------------------------------------------*/
              DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR, "RLP Idle Seq err, Re-Syncing");
      ...
      else /* Invalid Secondary Frame */
      {
        ...
        -------------------------------------------------------------------*/
        DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR, "Invalid Secondary RLP Frame RX'd");
        rlp_state_info.contig_erase++;
        frame_status = RLP_RX_FRAME_ERR;
      }
      break;
    ...
    case RLP_QUARTER_RATE:
      ...
      if( gen_frame_status != RLP_FRAME_OK )
      {
        ...
        {
          DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR, "Control frame with bad CRC rxed");
        ...
        {
          DS_RLP_ERROR_LOG_3(MSG_LEGACY_ERROR, "Frame Err: %u, Rate: %u, Chan: %u",
    ...
    default:
      ...
      ---------------------------------------------------------------------*/
      DS_RLP_ERROR_LOG_1(MSG_LEGACY_ERROR, "Unknown RLP data frame rate: %u",
                    frame_rate);
    ...
    if( frame_status == RLP_RX_FRAME_ERR )
    {
      ...
      if( rlp_state_info.state == RLP_ESTABLISHED_STATE ) 
      {
        ...
        {
          DS_RLP_ERROR_LOG_1(MSG_LEGACY_ERROR, "RLP Reset, Max Erasures: %d",
                        rlp_state_info.contig_erase);
    ...
    if( rlp_state_info.state == RLP_RT_MEAS_STATE )
    {
      ...
      {
        DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR, "Bad 1st Non-ctl frame, Re-sync");
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if( rlp_type == RLP_TYPE_I )
  {
    DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR, "Bitmap NAK is not defined for RLP Type 1");
  }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
      ...
      if( do_bitmap_nak )
      {
        ...
        if( found_first )
        {
          ...
          else
          {
            ...
            {
              DS_RLP_ERROR_LOG_1(MSG_LEGACY_ERROR, "Array index for nak_map out of bounds %d", 
                            nak_map_cnt);
              return;
            }
  ...
  {
    DS_RLP_ERROR_LOG_1(MSG_LEGACY_ERROR, "%u NAKs to send, but NAK list is empty!",
                  naks_to_send_cnt);
  }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
            ...
            rlp_tx_watermark_ptr->current_cnt != 0))
  {
    ...
    if( rlp_tx_data_ptr == NULL)
    {
      ...
      {
        DS_3GPP2_ERROR_FATAL( "No TX data item");
      }
...
#else
  DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR, "ds_process_rlp_rx_frames() not supported");
#endif /* FEATURE_DS */
} /* ds_process_rlp_rx_frames() */
