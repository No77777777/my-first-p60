...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp2/1xrlp/src/dsrlprsq.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
{\
  uint32 temp;\
  DS3GPP2_ASSERT((bytes <= ((seq_mod >> 4)-1)) && (fseq < seq_mod));\
...
{\
  uint32 temp;\
  DS3GPP2_ASSERT((fr_len <= ((seq_mod >> 4)-1)) && (fseq < seq_mod));\
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if (DSRLPSEQ_GT_SEQ_CHECK(needed_seq_num, new_item.last_seq_num, seq_mod))
  {
    ...
    -----------------------------------------------------------------------*/
    DS_RLP_ERROR_LOG_3(MSG_LEGACY_ERROR, "LV(N)= 0x%x > than last "
  ...
  if ( rsq_item_count == 0  ) 
  {
    ...
    -----------------------------------------------------------------------*/
    DS3GPP2_ASSERT((rrcb_ptr->state.reseq_last_item_ptr == NULL) &&
                  (rrcb_ptr->state.reseq_last_chain_ptr== NULL) &&
                  (rrcb_ptr->state.reseq_last_insert_item_ptr == NULL) &&
                  (rrcb_ptr->state.reseq_last_insert_chain_ptr== NULL));
  ...
  else
  {
    ...
    -------------------------------------------------------------------------*/
    DS3GPP2_ASSERT( ( rrcb_ptr->state.reseq_last_chain_ptr != NULL) &&
                   ( rrcb_ptr->state.reseq_last_item_ptr  != NULL) &&
    ...
    else if (rrcb_ptr->state.reseq_last_insert_chain_ptr != NULL) 
    {
      ...
      -------------------------------------------------------------------------*/
      DS3GPP2_ASSERT( walk_item_ptr != NULL);
...
)
{
  ...
  if (walk_item_ptr == NULL) 
  {
    ...
    -----------------------------------------------------------------------*/
    DS3GPP2_ASSERT(last_item_ptr->ptr != NULL);
...
)
{
  ...
  if (DSRLPSEQ_GT_SEQ_CHECK(needed_seq_num, new_item.seq_num, seq_mod)) 
  {
    ...
    DS3GPP2_ASSERT(new_item.ptr != NULL);
    ...
    DS3GPP2_ASSERT(new_item.seq_num == rrcb_ptr->state.needed_seq_num);
  ...
  DS3GPP2_ASSERT(truewalk_item_ptr != NULL);
  ...
  {        
    ...
    {    
      ...
      DS3GPP2_ASSERT((truelast_distance-1)<DSRLPRSQ_MAX_OVERLAP_ITEMS);
  ...
  if (first_item.found) 
  {
    DS3GPP2_ASSERT(first_item.ptr != NULL);
  ...
  if (last_item.found) 
  {
    ...
    DS3GPP2_ASSERT(last_item.ptr != NULL);
  ...
  if (first_item.found) 
  {
    ...
    if (first_item.overlap_bytes > 0)
    {
      ...
      else 
      {
        ...
        -------------------------------------------------------------*/
        DS3GPP2_ASSERT((pullup_bytes == first_item.overlap_bytes) &&
          (new_item.ptr != NULL));
        ...
        (new_item.ptr)->app_field = new_item.seq_num;
        DS3GPP2_ASSERT(new_item.seq_num < seq_mod);
         ...
         !discard_flag) 
  {
    ...
    if (truelast_item.overlap_bytes > 0)
    {
      ...
      else 
      {
        ...
        -------------------------------------------------------------*/
        DS3GPP2_ASSERT(truelast_item.overlap_bytes <= new_item.length);
        ...
        DS3GPP2_ASSERT(( pullup_bytes == truelast_item.overlap_bytes) &&
                      ( new_item.ptr != NULL));
        ...
        --------------------------------------------------------------*/
        new_item.length  = dsm_length_packet(new_item.ptr); 
        DS3GPP2_ASSERT( new_item.length > 0);
  ...
  if (last_item.found && (last_item.ptr!=truelast_item.ptr) && !discard_flag) 
  {
    ...
    if (last_item.overlap_bytes > 0)
    {
      ...
      else 
      {
        ...
        DS3GPP2_ASSERT(pullup_bytes == last_item.overlap_bytes);
  ...
  { 
    ...
    if (first_item.found && !last_item.found) 
    {
      ...
      ---------------------------------------------------------------------*/
      DS3GPP2_ASSERT( first_item.ptr == rrcb_ptr->state.reseq_last_chain_ptr);
    ...
    else if (last_item.found && (!first_item.found)) 
    {
          ...
          (!new_item.lst_set))
      {
        ...
        DS3GPP2_ASSERT(temp_ptr == last_item.ptr);  
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  { 
    ...
    if (DSRLPSEQ_GE_SEQ_CHECK(needed_seq_num, walk_seq_num, seq_mod))
    {
      ...
      DS3GPP2_ASSERT(insert_item_ptr != NULL);
