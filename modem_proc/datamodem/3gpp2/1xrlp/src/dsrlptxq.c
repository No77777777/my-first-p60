...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp2/1xrlp/src/dsrlptxq.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
)
{
  ...
  if ( q_items  >= rrcb_ptr->cfg_ptr->ver_ptr->txq_size ) 
  {
    ...
    DS3GPP2_ASSERT(free_item_ptr != NULL);
...
)
{
  ...
  do
  {
                              ...
                              ))
    {
      ...
      {
        DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR,"Transmitted a zero length rlp frame?");
        end_seq_num = seq_num;
      }
...
)
{
  ...
  do 
  {
    ...
    if ( next_item_ptr == NULL )
    {
      ...
      {
        DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR, "Accessing a zero length rlp frame?");
        end_seq_num = ith_seq_num;
      }
