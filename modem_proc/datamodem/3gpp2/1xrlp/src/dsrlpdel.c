...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp2/1xrlp/src/dsrlpdel.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
{
  DS3GPP2_ASSERT( (rlpdel_id < DSRLPI_MAX_SESSIONS) &&
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  /* Graceful return if reset was executed by 2 different triggers */
  if(rlpdel_fr_detect[rlpdel_id].used == FALSE)
  {
    return;
  }
...
)
{
  ...
  byte      curr_idx;
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  DS3GPP2_ASSERT((rrcb_ptr->state.del_fr_id < DSRLPI_MAX_SESSIONS) &&
                (rlpdel_fr_detect[rrcb_ptr->state.del_fr_id].used == TRUE));

  if (fr_item_ptr == NULL)
  {
    DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR, "Passed null item ptr");
    return;
  }
  ...
  {
    DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR, "too many items in del. fr array");
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  DS3GPP2_ASSERT(rrcb_ptr->state.del_fr_id < DSRLPI_MAX_SESSIONS);
  DS3GPP2_ASSERT(rlpdel_fr_detect[rrcb_ptr->state.del_fr_id].used == TRUE);
  ...
  if(fr_detect_ptr->remove_idx != fr_detect_ptr->add_idx)
  {
    ...
    for(i = 0; i < fr_arr_ptr->index; i++)
    {
      ...
      {
        DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR, "NULL ptr in del frame array");
      }
    }
    fr_arr_ptr->index = 0;
  ...
  if (fr_detect_ptr->del_fr_arr[fr_detect_ptr->add_idx].index > 0)
  {
    ...
    if (fr_detect_ptr->del_fr_arr[fr_detect_ptr->add_idx].index != 0)
    {
      DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR, "NON EMPTY ARRAY...MEM LEAK???");
    }
  }
} /* dsrlpdel_remove() */
#endif /* MSM5000_IRAM_FWD */
