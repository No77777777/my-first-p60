...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp2/1xrlp/src/rlp.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  {
    DS_RLP_ERROR_LOG_1(MSG_LEGACY_ERROR, "NAK Frame FIFO full, attempt: %d",
                  attempt_num);
    return;
  }
  /*-------------------------------------------------------------------------
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  {
    DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR, "No Saved Frames to Re-xmit");
    return;
  }
  /*-------------------------------------------------------------------------
  ...
  if(( frame_ptr->gen_ctl.ctl_len & RLP_SYNC_MASK) == RLP_SYNC_ENABLED)
  {
    switch( rlp_state_info.state)
    {
      ...
      case RLP_ESTABLISHED_STATE:
        ...
        -------------------------------------------------------------------*/
        DS_RLP_ERROR_LOG_1(MSG_LEGACY_ERROR, "Invalid SYNC, reset RLP",
           ...
           i=(i+1) % RLP_SEQ_MODULUS)
      {
        ...
        if( rlp_hole[i].naks)
        {
                   ...
                   rlp_hole[i].age ==  RLP_MAX_HOLE_AGE)
          {
            ...
            ---------------------------------------------------------------*/
            DS_RLP_ERROR_LOG_3(MSG_LEGACY_ERROR, "Seq timeout: %u, V(N/R): %u/%u",
                          i,
                          rlp_state_info.needed_seq_num,
  ...
  else if( GT_SEQ_CHECK( current_seq_num, rlp_state_info.needed_seq_num))
  {
    ...
    while( item_ptr != NULL)
    {
           ...
           current_seq_num)
      {
        ...
        -------------------------------------------------------------------*/
        DS_RLP_ERROR_LOG_3(MSG_LEGACY_ERROR, "Reassy Dup: %u, V(N/R): %u/%u",
                      current_seq_num,
                      rlp_state_info.needed_seq_num,
                      rlp_state_info.expected_seq_num);
        (void)dsm_free_buffer( new_item_ptr);
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  switch( fr_rate)
  {
    ...
    case RLP_EIGHTH_RATE:
      ...
      if( primary_chan)
      {
            ...
            frame_ptr->pri_idle.nr_crc)
        {
          ...
          if( rlp_state_info.state == RLP_ESTABLISHED_STATE)
          {
            DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR, "RLP Idle NR CRC error");
                 ...
                 pri_idle.seq_num     != rlp_saved_eighth.seq_num)
        {
          /*-----------------------------------------------------------------
            ...
            not incremented.
          -----------------------------------------------------------------*/
            DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR, "Idle no match to previous");
            rlp_pri_frate_errs.idle_fr_err++;
            return_value         = RLP_RX_FRAME_ERR;
        }
        else   /* idle frame is good - so far */
        {
          ...
          else if( last2_idles_same)
          {
                              ...
                              rlp_state_info.expected_seq_num))
            {
              ...
              -----------------------------------------------------------*/
              DS_RLP_ERROR_LOG_2(MSG_LEGACY_ERROR, "3 Matching 1/8, now: %d, was: %d",
                            frame_ptr->pri_idle.seq_num,
                            rlp_state_info.expected_seq_num);
              frame_typ            = RLP_IDLE_FR;
              return_value         = RLP_RX_NO_DATA_AVAILABLE;
    ...
    case RLP_FULL_RATE:
                 ...
                 rlp_prifull_table[rlp_mux].format_a)
      {
        /*-----------------------------------------------------------
          ...
          section of IS-707.
        -----------------------------------------------------------*/
        DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR, "Primary Full Rate type err");
    ...
    default:
      ...
      ---------------------------------------------------------------------*/
      DS_3GPP2_ERROR_FATAL( "Unknown RLP data frame rate");
  ...
  {
    DS_RLP_ERROR_LOG_0(MSG_LEGACY_ERROR, "RLP Channel changed, Resync");
    rlp_gen_stats.resets++;
    rlp_reset();
    (void)dsm_free_buffer( rlp_item_ptr);
    return( RLP_RX_NO_DATA_AVAILABLE);
