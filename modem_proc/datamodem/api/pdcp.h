...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/api/pdcp.h#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
/*---------------------------------------------------------------------------
  Following structures are currently used only for WCDMA
---------------------------------------------------------------------------*/
/*---------------------------------------------------------------------------
  Maximum RAB - Put comment
---------------------------------------------------------------------------*/
#define PDCP_MAX_RAB                16

/*---------------------------------------------------------------------------
  Structure to define RB Data rate
---------------------------------------------------------------------------*/
typedef struct
{
  uint8           rb_id;
  ...
  uint32          rb_dl_rate;
  uint32          rb_ul_rate;
  boolean         ul_dacc_status; /* future use */
}pdcp_rrc_rb_data_rate_type;

/*---------------------------------------------------------------------------
  RRC sends following user data to PDCP as part of data rate change
---------------------------------------------------------------------------*/
typedef struct
{
  uint8                       num_rbs;
  pdcp_rrc_rb_data_rate_type  rb_data_rate_info[PDCP_MAX_RAB];
}pdcp_rrc_rb_data_rate_info_type;
...
/*---------------------------------------------------------------------------
  PDCP can send one of the following action while calling MH callback to
  notify data rate indication
---------------------------------------------------------------------------*/
typedef enum
{
  PDCP_DATA_RATE_IND = 0,
  PDCP_RELEASE       = 1,
  PDCP_RELEASE_ALL   = 2,
} pdcp_rrc_data_rate_ind_e_type;

/*---------------------------------------------------------------------------
  RRC Data rate indication callback function pointer type
---------------------------------------------------------------------------*/
typedef void (*pdcp_rrc_data_rate_ind_f_ptr_type)
                  (
                    pdcp_rrc_data_rate_ind_e_type,
                    uint32,
                    uint8,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  Data passed in as part of reg_rb_ind cmd
- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
typedef struct
{
  pdcp_rrc_data_rate_ind_f_ptr_type  func_ptr;
} pdcp_reg_data_rate_ind_req_type;
