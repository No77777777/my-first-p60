...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/api/public/ps_flow.h#2 $
...
struct ps_flow_s
{
  ...
  struct
  {
    ...
    uint64                tx_flow_mask;     /* Mask used to flow control   */
    ...
    struct
    {
      ...
      /*---------------------------------------------------------------------
        Event buffer to register for associated ps_flow events if
        assoc_flow_ptr is non-NULL
      ---------------------------------------------------------------------*/

      void                 * ps_flow_ev_cback_buf;
    } logical_flow;

    /*-----------------------------------------------------------------------
      Event buffer to register for phys link events if phys link is
      FLOW_COUPLED
    -----------------------------------------------------------------------*/

    void   * phys_link_ev_cback_buf;
  ...
  /*-------------------------------------------------------------------------
    DPL control block for flow logging
      ...
      - dropped count : number of packets dropped by DPL for this ps_flow
...
PARAMETERS
  flow_ptr                   : pointer to the flow
  ps_iface_filter_spec_type : pointer to the requested filter spec
  ps_iface_qos_fltr_match_dir_enum_type: requested direction to be checked
