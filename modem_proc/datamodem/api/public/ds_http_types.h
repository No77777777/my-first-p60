...
// TODO: path max len?
#define DS_HTTP_SSL_CERT_PATH_MAX_LEN 256
...
typedef enum
{
  DS_HTTP_SESSION_TYPE_UNSET = 0,
  DS_HTTP_GW_PROV_PRI_SESSION, //MMGSDI_GW_PROV_PRI_SESSION MMGSDI_GW_PROV_SEC_SESSION
  DS_HTTP_GW_PROV_SEC_SESSION,
  DS_HTTP_SESSION_TYPE_MAX
} ds_http_session_type_e_type;
...
typedef struct
{
  ...
  ds_http_session_type_e_type session_type; /* Deprecated */
  ds_http_subscription_e_type subs_id;
  ds_http_pref_ip_e_type      pref_ip_type;
} ds_http_iface_info_s_type;



/* SSL version to use for HTTPS */
/* Note: TLS 1.2 and 1.3 not supported yet; placeholder for future. Current behavior will fall back to TLS 1.1. */
typedef enum
{
  DS_HTTP_SSL_VER_DEFAULT = 0, /* Current default is DS_HTTP_SSL_VER_TLS12 */
  DS_HTTP_SSL_VER_10,
  DS_HTTP_SSL_VER_20,
  DS_HTTP_SSL_VER_30,
...
typedef struct
{
  /* Change SSL version to be used. Default will use highest supported version
      by SSL module */
  ds_http_ssl_version_e_type ssl_version;
  /* Subscribe SSL alerts for warning/suspension/fatal level errors.
      Both mask and cb_fcn need to be set */
  uint8                      ssl_alert_notification_mask;
  ds_http_ssl_alert_cb_fcn   ssl_alert_cb_fcn;
  /* Change the folder path to check for SSL certificates */
  char                       ssl_cert_folder_path[DS_HTTP_SSL_CERT_PATH_MAX_LEN+1];
...
} ds_http_session_config_option_e_type;
