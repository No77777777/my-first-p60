...
/*=============================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/api/public/secssl.h#1 $ 
...
typedef enum
{
  ...
  SECSSL_SES_TLS_PSK_WAIT_EVENT        /**< TLS handshake TLS-PSK data wait event */
...
typedef enum
{
  ...
  SECSSL_SES_TLS_EVENT = 33,
    /**<SSL session to use tlsevent>**/
  SECSSL_SES_CA_CERT_NAME = 34,
    /**< SSL reads given Cert in RootCA and sets as Session cert. */ /* w/o */
  SECSSL_SES_RAM_CA_CERT_ID = 35,
    /**< SSL checks Cert with ID in RAM certs and sets as Session cert. */ /* w/o */
  SECSSL_SES_ENABLE_DYNAMIC_CERTS = 36,
    /**< Enable dynamic certificate usage and fetch. */
  SECSSL_SES_VERIFY_MODE = 37
...
typedef enum
{
  ...
  /*Currently supported in client mode. The server certificate is verified. If the verification process fails, the handshake is immediately terminated with an alert message containing the reason for the verification failure. If no server certificate is sent, because an anonymous cipher is used, SSL_VERIFY_PEER is ignored.*/

  SECSSL_VERIFY_NONE,                  /**< SSL_VERIFY_NONE */
  /*Currently supported in client mode. If not using an anonymous cipher, the server will send a certificate which will be checked. The handshake will be continued regardless of the verification result.*/
...
/**
  For TLS-PSK case client need to sent psk_hint to server.
  psk_key will be used in master secret creation. both should be updated from ClientApp
*/
typedef struct
{
  uint16 psk_hint_len;
  ...
  uint16  psk_key_len;
  uint8* psk_key_data;
}secssl_ses_psk_data_type;

/** Parameter to set SECSSL_SES_CA_CERT_NAME.
   SSL will search for Cert with given name in RootCA folder
and uses it as session cert.
*/
typedef struct
{
  char    *session_ca_cert_name;    /**<  CA root certificates name. */
}secssl_ses_ca_cert_name_data_type;
...
typedef union
{
  ...
  boolean                                 is_tlsevent_pdata;
  secssl_ses_ca_cert_name_data_type       ca_cert_name_pdata;
  uint32                                  ca_cert_id;
...
/*=============================================================================
  Function:    secsslca_add_root_cert
=============================================================================*/
/**
  To add/pass  CA certificates (x509) to SSL from client.
  so that SSL will check/parse  the certificate if its not valid,
  return Error without adding to SSL cert list.

  If the parced certificate is correct, Add to the SSL cert list.
  Note: This API does not has any affect on the current session if this is
        invoked in middle of session.

  @param[in] cert_id a valid cert ID, should be greater than 0
  @param[in] *cert_data,  cert content as buffer in x509 der format  
  @param[in] cert_len, size of *cert_data

  @return
  Returns a X509 cert parse error number to indicate Cert Parse outcome
 
  @dependencies
  None
*/
secerrno_enum_type  secsslca_add_root_cert
( 
  uint32       cert_id,
  uint8        *cert_data,
  uint16       cert_len
...
/*=============================================================================
  Function:    secsslca_remove_root_cert
=============================================================================*/
/**
  To Remove   CA certificates(x509) from SSL from client.
  Note: This API does not has any affect on the current session if this is
        invoked in middle of session.
  ...
  @return
  Returns a error number to indicate Remove cert outcome from SSL Cert List

  @dependencies
  None
*/
secerrno_enum_type secsslca_remove_root_cert ( uint32 cert_id );
