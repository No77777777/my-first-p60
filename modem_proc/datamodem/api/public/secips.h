...
/*===========================================================================
  ...
  $PVCSPath: $
  $Header: //components/rel/data.mpss/3.5..2.1/api/public/secips.h#1 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
03/04/19   ds      IPsec Re-architecture.
07/16/19   fn      DPD Ping session
...
06/29/17   nm      IPSec Re-architecture 
10/01/15   ks      API to reset NATT keep alive timer
08/04/15   nm      Src_port update to SA_info callback
 ...
 */
 ...
 secerrno_enum_type secips_generate_ipsec_info_ex
 (
   void                    *ipsec_handle,
   ip_pkt_info_type        *pkt_info_ptr,
   ps_ipsec_info_type      *ipsec_info,
...
typedef enum
{
  ...
  SECIPS_IKE_FAILURE_CAUSE_REACTIVE_REQ = 17,
  SECIPS_IKE_FAILURE_CAUSE_PCSCF_RESOTRE= 18,
  SECIPS_IKE_FAILURE_CAUSE_REDIRECT_REQ = 19,
...
{
  uint32 subs_id;
}secips_ike_session_ext_info_type;

typedef secipssp_ike_redirect_type secips_ike_redirect_type;

typedef enum
{
  SECIPS_IKE_EXT_INFO_NONE = 0,
  SECIPS_IKE_EXT_NOTIFY_BACKOFF_INFO,
  SECIPS_IKE_EXT_NOTIFY_REDIRECT_INFO,
  SECIPS_IKE_EXT_NOTIFY_UNAUTH_EMER_FALLBACK_INFO,
  SECIPS_IKE_EXT_INFO_MAX
} secips_ike_ext_enum_type;

typedef struct secips_ipsec_backoff_type_s
{
  uint8       backoff_timer;
} secips_ike_backoff_type;

typedef struct secips_ipsec_unauth_emer_fallback_info_type_s
{
  boolean is_fallback_success;
}secips_ike_unauth_emer_fallback_info_type;

typedef struct secips_ipsec_ike_ext_info_type_s
{
  secips_ike_ext_enum_type   ext_info_e;
  union
  {           
    secips_ike_backoff_type    backoff_info;
    secips_ike_redirect_type   redirect_info; 
    secips_ike_unauth_emer_fallback_info_type unauth_emer_fb_info;
  } ext_info_u;
} secips_ipsec_ike_ext_info_type;
...
(
  ...
  secips_ipsec_ike_ext_info_type      ipsec_ext_info
);

/* Ping result data type. */
typedef struct
{
  /* Notify Payload Data */
  uint8                              num_ping;
  ...
  uint16*                            rtt;
  uint16                             avg_rtt;  
} secips_ping_result_info_type;

/* Ping event type. */
typedef void (*secips_ping_cb_f_ptr_type)
(
  void*                               ike_sa_handle,
  secips_ping_result_info_type*       ping_result, 
  secerrno_enum_type                  secips_errno
);
...
/*===========================================================================
FUNCTION          SECIPS_NEW_SESSION_EXT()
===========================================================================*/
/**
  Creates a new IKE handle.
  Client can pass subs_id info for sub specific logging.
  Expected Sub value for sub specific logging are:
  1-Primary Sub
  2-Secondary Sub.
...
secerrno_enum_type  secips_new_session_ext
( 
  secips_ike_sa_handle_type*              ike_sa_handle_ptr,
  secips_cb_f_ptr_type                    ike_cb_fn_ptr,
  secips_ike_session_ext_info_type*       ike_ext_info_ptr
);
...
/*===========================================================================
FUNCTION               SECIPS_INITIATE_IKE_PING
===========================================================================*/
/**
  This function initiates IKE ping mechanism 

  @param ike_sa_handle [in]     - Handle to IKE session.
  @param ping_config   [in]     - configuration for ping session
  @param ping_result_cb [in]    - Function callback pointer for result
  @return
  secerrno_enum_type -- E_SUCCESS or a corresponding failure code.
  ...
  None.
*/
secerrno_enum_type secips_initiate_ike_ping
(
  secips_ike_sa_handle_type        ike_sa_handle,
  const secipssp_ping_config_type* ping_config,
  secips_ping_cb_f_ptr_type        ping_result_cb
);
#endif /* SECIPS_H */
