...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/api/public/dss_iface_ioctl.h#4 $ $DateTime: 2021/05/25 01:37:25 $

when        who    what, where, why
--------    ---    ----------------------------------------------------------
06/24/15    pg     Added to support qos handle when registration to net initiated qos.
05/21/15    pg     Added IOCTL support for PDN detach.
...
typedef enum
{
  ...
  /* Get Path MTU of iface */
  DSS_IFACE_IOCTL_GET_PATH_MTU            = PS_IFACE_IOCTL_GET_PATH_MTU,
  /* Get IPADDR of iface */
  ...
  DSS_IFACE_IOCTL_RAN_ASST_MEDIA_ADAPT_REG     =  PS_PHYS_LINK_IOCTL_RAN_ASST_MEDIA_ADAPT_REG,
  /* Register for RAN assisted media adaptation information */
  DSS_IFACE_IOCTL_RAN_ASST_MEDIA_ADAPT_DEREG   =  PS_PHYS_LINK_IOCTL_RAN_ASST_MEDIA_ADAPT_DEREG,
  /* Deregister for RAN assisted media adaptation information */
  DSS_IFACE_IOCTL_RAN_ASST_MEDIA_ADAPT_UPDATE  =  PS_PHYS_LINK_IOCTL_RAN_ASST_MEDIA_ADAPT_UPDATE,
  ...
  */
  ...
  /* Get OP_RES_PCO LIST */
  DSS_IFACE_IOCTL_GET_OP_RES_PCO_LIST = PS_IFACE_IOCTL_GET_OP_RES_PCO_LIST,

  /* Get MSISDN */
  DSS_IFACE_IOCTL_GET_MSISDN_INFO = PS_IFACE_IOCTL_GET_MSISDN_INFO,
...
/* Enable the slot cycle index mode. This mode will allow for the slot cycle
  ...
  -------------------------------------------------------------------------*/
  DSS_IFACE_IOCTL_WLAN_INITIATE_DPD_CHECK = 
                               PS_IFACE_IOCTL_WLAN_INITIATE_DPD_CHECK,

DSS_IFACE_IOCTL_WLAN_START_DPD_PING      = 
                               PS_IFACE_IOCTL_WLAN_START_DPD_PING,
...
/*-------------------------------------------------------------------------
  ...
  /*-------------------------------------------------------------------------
    This event is used to notify the application of RAN assist media adaptation
    information.
  -------------------------------------------------------------------------*/
  DSS_IFACE_IOCTL_RAN_ASST_MEDIA_ADAPTATION_EV,

  /*-------------------------------------------------------------------------
    This event is used to notify the application of DPD ping statistics.
  -------------------------------------------------------------------------*/
  DSS_IFACE_IOCTL_DPD_PING_INFO_EV,

  /*-------------------------------------------------------------------------
    This event is used to notify the application about discard of packet.
  -------------------------------------------------------------------------*/
  DSS_IFACE_IOCTL_PACKET_DISCARD_EV,
...
/*---------------------------------------------------------------------------
 DSS_IFACE_IOCTL_RAN_ASST_MEDIA_ADAPTATION_EV parameter type
   .
---------------------------------------------------------------------------*/
typedef ps_phys_link_data_flow_direction_enum_type dss_iface_data_flow_direction_enum_type;

typedef ps_phys_link_bit_rate_info_type dss_iface_bit_rate_info_type;

typedef struct
{
  uint32                              ran_asst_handle;
  dss_iface_bit_rate_info_type        bit_rate_info;
  ...
  /**< ROHC. Values:\n
       ...
       -1: ROHC Enable.
  */
}dss_iface_ran_asst_media_adapt_info_type;

/*---------------------------------------------------------------------------
 DSS_IFACE_IOCTL_DPD_PING_INFO_EV parameter type
  -Retrieve the data specifying the changed apn parameters.
---------------------------------------------------------------------------*/
typedef ps_iface_dpd_ping_stat_info_type
  dss_iface_dpd_ping_stat_info_type;
...
/*---------------------------------------------------------------------------
  Argument to be passed with 
  PS_IFACE_IOCTL_GET_OP_RES_PCO_LIST
---------------------------------------------------------------------------*/
typedef ps_iface_op_reserved_pco_list_type
  dss_iface_op_reserved_pco_list_type;

/*---------------------------------------------------------------------------
  Argument to be passed with 
  PS_IFACE_IOCTL_GET_MSISDN_PCO_INFO
---------------------------------------------------------------------------*/
typedef ps_iface_msisdn_info_type dss_iface_msisdn_info_type;
...
/**
  @brief NIQ filter match direction type.
*/
typedef ps_iface_qos_fltr_match_dir_enum_type dss_iface_ioctl_fltr_match_dir_type;
...
typedef struct
{
  ...
  boolean                                       fltr_match_dir_valid;
  /**< Is filter match direction (fltr_match_dir) field valid */
  dss_iface_ioctl_fltr_match_dir_type           fltr_match_dir;
  /**< Direction in which filter matching is done */
...
typedef struct
{
  dss_qos_handle_type                    qos_inst_handle;
  /**< Handle to QOS instance */
  dss_qos_net_initiated_req_handle_type  req_handle;
...
typedef ps_iface_handoff_ancillary_info_type   dss_iface_handoff_ancillary_info_type;
...
typedef struct
{
  ...
  dss_iface_handoff_ancillary_info_type        ancillary_info;
...
typedef union
{
  ...
  dss_iface_ran_asst_media_adapt_info_type     ran_asst_media_adapt_info;
  dss_iface_dpd_ping_stat_info_type            dpd_ping_info;
...
/*---------------------------------------------------------------------------
  Data type used to register for recommended RAN bit rate information
  (DSS_IFACE_IOCTL_RAN_ASST_MEDIA_ADAPTATION_REG). 
---------------------------------------------------------------------------*/
typedef struct
{
...
#define dss_nethandle       app_id
  dss_iface_id_type         iface_id;
  dss_iface_ioctl_event_cb  cback_fn;   /* Bit rate event notification callback */
  ip_filter_type            ip_spec;    /* IP/Port specification*/
  void                     *user_data;  /* User data passed to callback    */

  /* OUTPUT Parameter */
  uint32                    ran_asst_ma_handle;  /* Handle to RAN assit media adaptation */
} dss_iface_ioctl_ran_asst_media_adapt_reg_type;

/*---------------------------------------------------------------------------
  Data type used to deregister for recommended RAN bit rate information
  (DSS_IFACE_IOCTL_RAN_ASST_MEDIA_ADAPTATION_DEREG). dereg the handle when call is brought down.
---------------------------------------------------------------------------*/
typedef struct
{
  sint15                    app_id;     /* Client's App Id                 */
#define dss_nethandle       app_id
  uint32                    ran_asst_ma_handle;   /* Handle to RAN assit media adaptation */
} dss_iface_ioctl_ran_asst_media_adapt_dereg_type;

/*---------------------------------------------------------------------------
  Data type used to set RAN bit rate information
  (DSS_IFACE_IOCTL_RAN_ASST_MEDIA_ADAPTATION_UPDATE)
---------------------------------------------------------------------------*/
typedef struct
{
  sint15                    app_id;     /* Client's App Id                 */
#define dss_nethandle       app_id
  uint32                    ran_asst_ma_handle;     /* Handle to RAN assit media adaptation*/
  dss_iface_bit_rate_info_type  bit_rate_info; /* Bit Rate info*/
} dss_iface_ioctl_ran_asst_media_adapt_update_type;
...
typedef struct
{
  ...
  dss_qos_net_initiated_req_handle_type         req_handle;
  /**< [out] NIQ request handle. it can be used in subsequent
...
/*---------------------------------------------------------------------------
  Argument for DSS_FACE_IOCTL_WLAN_START_DPD_PING
---------------------------------------------------------------------------*/
typedef ps_iface_ioctl_dpd_ping_config_type  dss_iface_ioctl_dpd_ping_config_type;
