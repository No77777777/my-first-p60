...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/api/public/secipssp.h#7 $ $DateTime: 2023/10/16 00:28:16 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
10/13/23   ak      Added plain text support for retransmit IKE packets
02/22/21   fn      Added support for DH group 15-18
02/18/20   ds      Added validation check for allocated ipsec_sa_ptr.
09/16/19   ds      Added entry in IPSEC SA structure to store SP Index/SPD  
                   pointer.
04/05/19   fn      DPD Ping session
09/18/18   js      Added support for handling fragmented dup child_sa req
06/11/18   ak      L+L enhancement for sub specific logging
04/30/18   ak      EMERGENCY_SUPPORT Notify parsing.
04/27/18   shrvn   Added Support for sending EAP_ONLY_AUTHENTICATION payload
04/17/18   js      FR43238: Added support for cert retrieval from HLOS
04/12/18   ak      IKEv2 REDIRECT Support in final IKE_AUTH msg
04/09/18   arun    2181989 : FR 46730: 3GPP - Unauthenticated emergency calls over WiFi" (IPSec)
04/05/18   fn/nm   OCSP support
02/14/18   js      Handle duplicate request msg during CHILD_SA REKEY
02/21/18   ak      Min and max value for timeout of liveness check
01/08/18   ak      BACKOFF_TIMER support 
12/05/17   ak      Use of NW provided liveness check timeout value
11/07/17   nm      PCSCF restore support
05/16/17   nm      PEM cert foramt and CRL support
12/02/16   nm      IKE Fragmentation support 
05/19/16   nm      DeviceID/IMEI FR support
11/23/15   nm      IPSec EAP failure with NULL APN fix
09/18/15   nm      SHA256 support
08/04/15   nm      Dynamic_src port for IKE negotiation support
04/23/15   nm      HTTP_URI Certlookup support
...
#define SECIPSSP_FQDN_MAX_LENGTH          256 
...
/* =================================================================
 ...
 typedef enum
 {
   ...
   SECIPSPKTDEFS_IKE_CV_GD_MODP3072      = 15,
   SECIPSPKTDEFS_IKE_CV_GD_MODP4096      = 16,
   SECIPSPKTDEFS_IKE_CV_GD_MODP6144      = 17,
   SECIPSPKTDEFS_IKE_CV_GD_MODP8192      = 18,
 ...
 typedef enum
 {
   ...
   SECIPSPKTDEFS_IKEV2_NOTIFY_IKE_AUTH_MAX_CONN                 = 8193,
	 ...
	 SECIPSPKTDEFS_IKEV2_NOTIFY_IMEI_NOT_ACCEPTED                 = 11005,    
	 SECIPSPKTDEFS_IKEV2_NOTIFY_UNAUTHENTICATED_EMERGENCY_NOT_SUPPORTED = 11055, 
   ...
   /*IKEv2 Redirect Notify Payloads*/
   SECIPSPKTDEFS_IKEV2_NOTIFY_REDIRECT_SUPPORTED           = 16406,
   SECIPSPKTDEFS_IKEV2_NOTIFY_REDIRECT                     = 16407,
   SECIPSPKTDEFS_IKEV2_NOTIFY_REDIRECTED_FROM              = 16408,
   SECIPSPKTDEFS_IKEV2_NOTIFY_EAP_ONLY_AUTHENTICATION      = 16417,
   SECIPSPKTDEFS_IKEV2_FRAGMENTATION_SUPPORTED            = 16430,
   /* Reserved to IANA - Status Types:	16393 - 40959 */
   /* Private Use - Status Types:		40960 - 65535 */
   SECIPSPKTDEFS_IKEV2_NOTIFY_REACTIVATE_REQ              = 40961,
   SECIPSPKTDEFS_IKEV2_NOTIFY_IMEI                        = 41101,
   SECIPSPKTDEFS_IKEV2_NOTIFY_PCSCF_RESTORE_SUPPORTED     = 41304,
   SECIPSPKTDEFS_IKEV2_NOTIFY_BACKOFF_TIMER               = 41041,
   SECIPSPKTDEFS_IKEV2_NOTIFY_EMERGENCY_SUPPORT           = 41112
 ...
 typedef enum
 {
   SECIPSSA_DEVICE_ID_TYPE_RESERVED = 0,
   SECIPSSA_DEVICE_ID_TYPE_IMEI = 1,
   SECIPSSA_DEVICE_ID_TYPE_IMEISV = 2,
   SECIPSSA_DEVICE_ID_TYPE_MAX
 } secipssa_device_id_enum_type;

 /*  User ID type  */ 
 typedef enum
 {
	 SECIPSSA_USER_ID_TYPE_RESERVED = 0,
	 SECIPSSA_USER_ID_TYPE_IMSI = 0,  
	 SECIPSSA_USER_ID_TYPE_IMEI = 1,
	 SECIPSSA_USER_ID_TYPE_MAX
 } secipssa_user_identity_enum_type;
 
 /* Key information for Simless emergency msk calculation */
 typedef struct
 {
	 uint8*	key_buffer; /**< -- Key */
	 uint16 key_len; /**< -- Key Length */
 } secipssp_key_msk_unauth_emergency_data_type;
 ...
 /**
   * Device identity types and value that will be set from ClientApp
   */
  typedef struct 
  {
    secipssa_device_id_enum_type  device_id_type;
    uint16                        device_id_len;
    uint8*                        device_id_value;
  } secipssp_device_identity;
 ...
 typedef enum 
 {
   IPSEC_REDIRECT_RESERVED                = 0,
   IPSEC_REDIRECT_GATEWAY_IP4_ADDRESS     = 1,
   IPSEC_REDIRECT_GATEWAY_IP6_ADDRESS     = 2,
   IPSEC_REDIRECT_GATEWAY_FQDN            = 3,
   IPSEC_REDIRECT_MAX
 } IPSecRedirectGatewayType;
 
 
 typedef struct secips_ipsec_redirect_type_s
 {
   IPSecRedirectGatewayType      type;
   uint32                        value_len; 
   union 
   {
     uint8                  epdg_fqdn[SECIPSSP_FQDN_MAX_LENGTH];
     struct ps_in_addr      ip4_address;
     struct ps_in6_addr     ip6_address;
   } value;
 } secipssp_ike_redirect_type;
 ...
 typedef struct
 {
   ...
   boolean                http_cert_lookup;  /* HTTP Cert lookup support */
   uint32                 http_lookup_time_sec; /**<- Max time for HTTP lookup */
   secipssp_device_identity device_identity; /**<-device identity value */

    // Below are the new configuration parameters for FRAGMENTATION feature
   boolean                ike_fragmentation; /**<- set to TRUE, to enable ike fragmentation */
   uint32                 ike_ipv4_fragment_size; /**<- max IPv4 ike fragment size */
   uint32                 ike_ipv6_fragment_size; /**<- max IPv6 ike fragment size */
   boolean                disable_crl; /** CRL validation enable/disabling */
   boolean                ike_pcscf_reselection;  /** PCSCF reselection enable/disabling */
   boolean                liveness_chk_enabled;
   uint32                 liveness_max_timeout_period;
   uint32                 liveness_min_timeout_period;
   boolean                disable_ocsp; /** OCSP validation enable/disabling */
   boolean                redirect_supported; /**Redirect support enable/disable*/
   secipssp_ike_redirect_type    redirect_from_info;  /**Redirected from gateway info*/
	 secipssa_user_identity_enum_type user_identity_type;  /** imsi based or imei based */	
	 secipssp_key_msk_unauth_emergency_data_type key_for_msk_unauth_emergency; /** key value for msk generation during unauth emergency*/
   boolean                dyn_cert_retrieval_enabled;  /* Dynamic cert retrieval support */
   uint32                 dyn_cert_retrieval_timer_sec; /* Max time for dynamic cert retrieval */
   boolean                send_eap_only_auth; /** Send/Don't send EAP_ONLY_AUTH payload */
...
{
  uint32              msg_id;
  dsm_item_type**     tx_msg;              /* Current in-flight msg */
  uint8               num_pkts;            /* # of packets if fragmented else 1 */

  uint8               req_frag_length;     /* Request frag size */
  uint8               req_frag_recieved;   /* Recieved frags */
} secipstypes_sa_msg_type;

typedef struct
{
  uint8                              num_ping;
  uint16                             freq_period_ms;
  uint16                             max_time_wait_ms;
  boolean                            set_ka_guard;
}secipssp_ping_config_type;

typedef struct
{
  uint8*      msg;              /* Unencrypted msg cache */
  uint32      msg_len;
} secipstypes_unencr_msg_type;

typedef struct secipssa_isakmp_sa_type_s secipssa_isakmp_sa_type;
 /*---------------------------------------------------------------------------
  * 					  I S A K M P	 S A
  --------------------------------------------------------------------------*/
 /* ISAKMP Security Association.
 */
 struct secipssa_isakmp_sa_type_s
 {
   secipssa_isakmp_sa_type*       this_isakmp_sa_ptr;
   ...
   timer_type*  natt_keepalive_timer;
   ...
   /*Set to True when SECIPSPKTDEFS_IKEV2_FRAGMENTATION_SUPPORTED Notify recv in IKE_INIT rsp. */
   boolean            is_fragmentation_supported;
   ...
   secipssa_device_id_enum_type  device_id_type; /* to store received IMEI type */
   uint8    negotiated_ts_type;    /* to store negotiated TS type*/
   secipstypes_sa_msg_type       last_resp_msg;        /* Last response msg */
	 boolean	is_emergency_apn;  /*If emergency apn, be ready to ignore null IDR in received AUTH messages */
	 boolean  is_unauthenticated_emergency_session; /*unauthenticated emergency session status */
   /*Set to True when EMERGENCY_SUPPORT Notify recv in IKE_AUTH rsp. */
   boolean            is_emergency_supported;
   void*              ping_info;     /* Used to store information relating to ping session */

   secipstypes_unencr_msg_type     unencr_cache_rx;    /* Unencrypted cache for last RX msg */
   secipstypes_unencr_msg_type     unencr_cache_tx;    /* Unencrypted cache for last TX msg */


   /*Set to 1 for Primary sub. Set to 2 for Secondary sub 
   Subs_id info can obtained from xchg_ptr or ike_sa_handle_ptr
   e.g.
   ipsec_cb_ptr = SECIPS_GET_IPSEC_CB_PTR(ike_sa_handle_ptr); OR 
   ikev2_sa_ptr = (secipssa_isakmp_sa_type*) xchg_ptr->isakmp_sa_ptr; 
   */
   uint32   subs_id;
 } ;
...
#define DS_ISAKMP_SA_PTR_IS_VALID(isakmp_sa_ptr)         \
   ((isakmp_sa_ptr != NULL) && 				 \
    (isakmp_sa_ptr->this_isakmp_sa_ptr == isakmp_sa_ptr))				
