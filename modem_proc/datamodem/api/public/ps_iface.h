...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/api/public/ps_iface.h#2 $
  $DateTime: 2019/10/22 02:53:17 $

  when       who what, where, why
  ---------- --- ------------------------------------------------------------
  2014-11-13 cx  IPv6 address cahce optimization
...
#define PS_IFACE_MAX_NUM_PCO  10
...
/*---------------------------------------------------------------------------
TYPEDEF PS_IFACE_CMD_TEAR_DOWN_F_PTR_EX_TYPE
...
PARAMETERS
  this_iface_ptr: Ptr to interface control block on which to operate on.
  app_priority:   priority of requesting application. range is [0,31]
  net_down_reason :   netdown reason of interface.
  client_data:    pointer to any client data that needs to be passed in

RETURN VALUE
  None
---------------------------------------------------------------------------*/
typedef int (*ps_iface_cmd_tear_down_f_ptr_ex_type)
(
  ps_iface_type                 *this_iface_ptr,
  int32                          app_priority,
...
typedef enum
{
  PS_IFACE_OP_RESERVED_PCO_ID1 = 0xFF00,
  PS_IFACE_OP_RESERVED_PCO_ID2 = 0xFF01,
  PS_IFACE_OP_RESERVED_PCO_ID3 = 0xFF02,
  PS_IFACE_OP_RESERVED_PCO_ID4 = 0xFF03,
  PS_IFACE_OP_RESERVED_PCO_ID5 = 0xFF04,
  PS_IFACE_OP_RESERVED_PCO_ID6 = 0xFF05,
  PS_IFACE_OP_RESERVED_PCO_ID7 = 0xFF06,
  PS_IFACE_OP_RESERVED_PCO_ID8 = 0xFF07,
  PS_IFACE_OP_RESERVED_PCO_ID9 = 0xFF08,
  PS_IFACE_OP_RESERVED_PCO_ID10 = 0xFF09
} ps_iface_op_reserved_pco_id_enum_type;
...
typedef struct
{
  ...
  uint16 container_id;
  ps_iface_op_reserved_pco_id_enum_type   pco_id;
}ps_iface_op_reserved_pco_type;


typedef struct
{
  ps_iface_op_reserved_pco_type   pco_info[PS_IFACE_MAX_NUM_PCO];
...
/*===========================================================================
TYPEDEF PS_IFACE_DATA_PATH_CLIENTS_ENUM_TYPE

DESCRIPTION
  Describing different source of request for data path switching.
  Whenever Iface IOCTL called to switch data path appropriate bit will be set or reset. 

NOTE : Enums are not bit masks. They represent bit position in a mask which
       represents a particular client.
===========================================================================*/

typedef enum
{
  PS_IFACE_MIN_HW_PATH        = 0,
  PS_IFACE_DSS_HW_PATH        = PS_IFACE_MIN_HW_PATH,
  PS_IFACE_WDS_HW_PATH        = 1,
  PS_IFACE_DFS_HW_PATH        = 2,
  PS_IFACE_MAX_HW_PATH        = PS_IFACE_DFS_HW_PATH

}ps_iface_data_path_clients_enum_type;

/*===========================================================================
  Default HW bridging is allowed hence set all bits to one
===========================================================================*/
#define PS_IFACE_BRIDGING_ALLOWED (  1 << PS_IFACE_DSS_HW_PATH |    \
                                     1 << PS_IFACE_WDS_HW_PATH |    \
                                     1 << PS_IFACE_DFS_HW_PATH ) 
/*===========================================================================
...
typedef enum
{
  ...
  PS_IFACE_CAPABILITY_HW_CAPABLE       = 0x1,
...
struct ps_iface_s
{
  ...
  struct
  {
    ...
    q_type         event_handle_q;                  /* event callback queue */
    ps_ifacei_v6_addr_type  *primary_v6_addr_ptr; 
                                            /* pointer to primary v6 address */
    ps_ifacei_sec_v6_addr_cache_type *sec_v6_addr_cache_ptr;
                                   /* pointer to secondary v6 address cache */
    ...
    ps_iface_type *back_iface_ptr;     /* ptr to associating interface     */
    ...
    uint64         tx_flow_mask;       /* if !=0, iface is flow_ctl        */
    ...
    boolean        inherit_ip_info;    /* FALSE (default) - local IP info  */
                                       ...
                                       /* associated PS_IFACE              */
    boolean        do_reassembly;      /* perform reassembly in this iface */
    boolean        do_ipsec_processing;/* perform ipsec processing in this */
                                       /* iface (decrypt/authenticate)     */
    ...
    void    * logical_iface_ev_cback_buf_ptr;
    ...
    uint8                           data_path_bridge_mask; /*  Bit based flag for data path bridging*/
  ...
  /*-------------------------------------------------------------------------
    DPL control block for ps_iface
      ...
      - dropped count : number of packets dropped by DPL for this iface
  -------------------------------------------------------------------------*/
  struct
  {
  ...
  ps_iface_op_reserved_pco_list_type *operator_reserved_pco_list_ptr;
  ...
  ps_iface_cmd_tear_down_f_ptr_ex_type tear_down_cmd_f_ptr_ex; /* ext ver of tear down cmd fptr */
...
#define PS_IFACE_SET_ASSOC_IFACE(this_iface, assoc_iface)                \
  (ps_ifacei_set_assoc_iface(this_iface, assoc_iface))
...
/*===========================================================================
FUNCTION PS_IFACE_GET_REASSEMBLY_FLAG()

DESCRIPTION
  This macro gets the reassembly flag for the iface.
...
RETURN VALUE
  None.
===========================================================================*/
#define PS_IFACE_GET_REASSEMBLY_FLAG(this_iface)                \
  ((this_iface)->iface_private.do_reassembly)

/*===========================================================================
FUNCTION PS_IFACE_SET_REASSEMBLY_FLAG()

DESCRIPTION
  This macro Sets the reassembly flag for the iface.
...
RETURN VALUE
  None.
===========================================================================*/
#define PS_IFACE_SET_REASSEMBLY_FLAG(this_iface, value)                \
  ((this_iface)->iface_private.do_reassembly = value)

/*===========================================================================
FUNCTION PS_IFACE_GET_IPSEC_PROCESSING_FLAG()

DESCRIPTION
  This macro gets the ipsec processing flag for the iface.
...
RETURN VALUE
  None.
===========================================================================*/
#define PS_IFACE_GET_IPSEC_PROCESSING_FLAG(this_iface)                \
  ((this_iface)->iface_private.do_ipsec_processing)

/*===========================================================================
FUNCTION PS_IFACE_SET_IPSEC_PROCESSING_FLAG()

DESCRIPTION
  This macro Sets the ipsec processing flag for the iface.
...
RETURN VALUE
  None.
===========================================================================*/
#define PS_IFACE_SET_IPSEC_PROCESSING_FLAG(this_iface, value)                \
  ((this_iface)->iface_private.do_ipsec_processing = value)
...
RETURN VALUE
  ...
  FALSE : otherwise
===========================================================================*/
#define PS_IFACE_GET_CAPABILITY  ps_iface_get_capability
INLINE boolean ps_iface_get_capability
(
  ps_iface_type               * iface_ptr,
  ps_iface_capability_enum_type capability
)
{
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
    ...
    (capability) < PS_IFACE_CAPABILITY_MAX)
  {
    ...
    if(capability == PS_IFACE_CAPABILITY_HW_CAPABLE)
    {
      ...
      base_iface_ptr = PS_IFACE_GET_BASE_IFACE(base_iface_ptr);
     
      return (base_iface_ptr->iface_private.capability_mask & (0x1 << capability));
    }
    else
    {
      return (iface_ptr->iface_private.capability_mask & (0x1 << capability) );
    }
  }
  else
  {
...
#define PS_IFACE_SET_IPV6_GATEWAY_IID(iface_ptr, iid)                 \
  if (PS_IFACE_IS_VALID(iface_ptr))                                   \
  {                                                                   \
    PS_IFACE_GET_V6_PRIMARY_ADDR_PTR(iface_ptr)->gateway_iid = iid;    \
  }    
/*= = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
...
===========================================================================*/
/** @ingroup ps_iface_set_wlan_ipv6_addr
  ...
  @return
   0 -- IP address was applied successfully.\n
  -1 -- Failure.
...
DESCRIPTION
  ...
  This API will always return the first entry in the list that is stored 
  on the specified iface (for backwards compatibility reasons)
  Modehandler need to ensure that the first entry in this list should 
  always contain 0xFF00 for backwards compatibility.
...
PARAMETERS
  this_iface_ptr: ptr to interface control block on which to operate on.
  operator_reserved_pco_ex_info_ptr: pointer to the operator reserved PCO info. 
...
/*===========================================================================
MACRO PS_IFACE_DISBALE_BRIDGING()

DESCRIPTION
  The marco will reset bit corresponding to iface client in given iface.

PARAMETERS
  source:   source of request
  iface ptr: target iface

RETURN VALUE
  none
===========================================================================*/
#define PS_IFACE_DISBALE_BRIDGING(iface_ptr, iface_client)                   \
  if (PS_IFACE_IS_VALID( iface_ptr))                                         \
  {                                                                          \
    (iface_ptr)->iface_private.data_path_bridge_mask &= ~( 1<< iface_client);\
  }

/*===========================================================================
MACRO PS_IFACE_IS_BRIDGING_DISBALED()

DESCRIPTION
  The marco will check for given iface client data path is suspended/unbridged .
...
RETURN VALUE
  none
===========================================================================*/
#define PS_IFACE_IS_BRIDGING_DISBALED(iface_ptr, iface_client)               \
  (0 == ((iface_ptr)->iface_private.data_path_bridge_mask & \
    (1 << iface_client))) ? TRUE: FALSE 

/*===========================================================================
MACRO PS_IFACE_IS_BRIDGING_DISBALED()

DESCRIPTION
  The marco will check for given iface client data path is resumed/bridged .
...
RETURN VALUE
  none
===========================================================================*/
#define PS_IFACE_IS_BRIDGING_ENABLED(iface_ptr, iface_client)               \
  (0 != ((iface_ptr)->iface_private.data_path_bridge_mask & \
    (1 << iface_client))) ? TRUE: FALSE 

/*===========================================================================
MACRO PS_IFACE_ENABLE_BRIDGING()

DESCRIPTION
  The marco will set bit corresponding to iface client in given iface.
...
RETURN VALUE
  none
===========================================================================*/
#define PS_IFACE_ENABLE_BRIDGING(iface_ptr, iface_client)                    \
  if (PS_IFACE_IS_VALID( iface_ptr))                                         \
  {                                                                          \
    (iface_ptr)->iface_private.data_path_bridge_mask |=( 1<< iface_client);  \
  }
...
/*===========================================================================
MACRO PS_IFACE_GET_DATA_PATH_FLAG()

DESCRIPTION
   The macro will return current data path state in given Iface.
...
RETURN VALUE
  none
===========================================================================*/
#define PS_IFACE_GET_DATA_PATH_FLAG(iface_ptr)                               \
  (iface_ptr)->iface_private.data_path_bridge_mask

/*===========================================================================
MACRO PS_IFACE_SET_DATA_PATH_FLAG()
...
RETURN VALUE
  none
===========================================================================*/
#define PS_IFACE_SET_DATA_PATH_FLAG(iface_ptr, bridge_mask)                    \
  (iface_ptr)->iface_private.data_path_bridge_mask = bridge_mask
...
void ps_iface_get_profiles_in_use
(
  int    profile_type, 
  uint32 *prof_3gpp, 
  uint8  *num_prof_3gpp, 
  uint32 *prof_3gpp2, 
  uint8  *num_prof_3gpp2
);
...
boolean ps_iface_check_profile_in_use
(
  uint32  profile_num,
  uint32 *prof_list, 
  uint8  len
...
RETURN VALUE
  NONE
===========================================================================*/
void ps_iface_set_3gpp_profile_id
(
  ps_iface_type *iface_ptr,
  uint32         profile_id
);
