...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/api/public/ps_phys_link.h#3 $ $DateTime: 2021/05/25 01:37:25 $ $Author: pwbldsvc $
...
/*---------------------------------------------------------------------------
  MAX BEARER ID
---------------------------------------------------------------------------*/
#define  PS_PHYS_LINK_MAX_BEARER_EX_ID    64

/*---------------------------------------------------------------------------
  Flow control Internal command type
---------------------------------------------------------------------------*/
typedef enum
{
  PS_PHY_LINK_INTERNAL_CMD_LEGACY_TYPE   = 0,
  PS_PHY_LINK_INTERNAL_CMD_UL_BURST_TYPE = 1
}ps_phys_link_internal_cmd_type;
...
struct ps_phys_link_s
{
  ...
  struct private_info_type
  {
    ...
    q_type                 event_handle_q;/* q of event handles */
    uint32                 ref_cnt;       /* num flows bound to this       */
    uint8                  instance;      /* Phys link instance #          */
    uint64                 tx_flow_mask;  /* if !=0, phys link is flow_ctl */
    phys_link_state_type   state;         /* physical link state           */

    ps_phys_link_capability_mask_type  capability_mask;
#ifdef FEATURE_DATA_FLOW_MGMT
    uint32                 iface_flow_ctl_mask_cnt; /* Increment when Iface is being flow enabled/disable*/
    uint32                 flow_flow_ctl_mask_cnt;  /* Increment when flow is being flow enabled/disable*/
#endif /* FEATURE_DATA_FLOW_MGMT*/
  } phys_private;
  ...
  uint8    bearer_id;
  /*-------------------------------------------------------------------------
    Bearer extension  Id : This would be filled up by framework at the time of physlink bind 
    to iface. If it is default physlink then value will be 0 otherwise new value 
  -------------------------------------------------------------------------*/  
  uint8    bearer_id_ex;
#ifdef FEATURE_LAPP_DATA
  #error code not present
...
}ps_phys_link_bearer_id_ex_type;
...
DESCRIPTION
  This will memset() the private data struct to 0, resets the
  "this_phys_link_ptr" to NULL, and destroys the event queues.

PARAMETERS
  this_phys_link_ptr: Ptr to interface control blocks on which to operate on.
  n_links:            Number of phys links to delete.

RETURN VALUE
  -1 - error in phys link deletion
   0 - on success
...
DESCRIPTION
  To post a RAN assisted media adaptation indication.
