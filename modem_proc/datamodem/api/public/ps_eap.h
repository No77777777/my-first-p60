...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/api/public/ps_eap.h#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $


when        who    what, where, why
--------    ---    ----------------------------------------------------------
06/11/19    fn     Added RCS client
04/30/17    js     Added support for EAP Session-ID to SIM/AKA auth info
...
*/
#define EAP_IDENTITY_MAX_CHAR 512
...
/** The maximum length of the master session key - RFC 4186/4187 @xhyperref{S11,[S11]}
*/
...
/** The maximum random count and length of RAND - RFC 4186/4187 @xhyperref{S11,[S11]}
*/
#define EAP_SIM_MAX_RAND_NO          3

#define EAP_SIM_AKA_MAX_RAND_LEN    16

/** The maximum length of NONCE_MT - RFC 4186/4187 @xhyperref{S11,[S11]}
*/
#define EAP_SIM_MAX_NONCE_MT_LEN    16

/** The maximum length of AUTN - RFC 4186/4187 @xhyperref{S11,[S11]}
*/
#define EAP_AKA_MAX_AUTN            16

/** The maximum length of the Session-ID - RFC 5247 @xhyperref{S11,[S11]}
*/
#define EAP_AKA_MAX_SESSION_ID      ( EAP_SIM_AKA_MAX_RAND_LEN + EAP_AKA_MAX_AUTN + 1 )

#define EAP_SIM_MAX_SESSION_ID      ( (EAP_SIM_MAX_RAND_NO * EAP_SIM_AKA_MAX_RAND_LEN) + \
                                      EAP_SIM_MAX_NONCE_MT_LEN + 1 )

#define EAP_SIM_AKA_MAX_SESSION_ID  ( EAP_AKA_MAX_SESSION_ID > EAP_SIM_MAX_SESSION_ID ? \
                                      EAP_AKA_MAX_SESSION_ID : EAP_SIM_MAX_SESSION_ID )
/** @} */ /* end_addtogroup constants */
...
/** Prototype for the transmit function that the lower layer
  registers to allow the EAP to transmit a packet.
*/
typedef void (*eap_trp_tx_cback_buf_type)
(
  eap_handle_type handle,           /**< Handle to the EAP instance. */
  void           *userdata,         /**< User data */
  uint8*         tx_pkt_buffer,     /**< buffer located in heap and is freed after returning from user */
  uint16         tx_pkt_len         /**< Length of tx pkt buffer */
);
...
/** @brief Auth credentials information - Used to store the msk, emsk
   and session_id currently. 
*/
typedef struct
{
...
typedef struct
{
  ...
  eap_trp_tx_cback_type      trp_tx_f_ptr;        
    /**< Transport Tx function pointer. dsm  */
  eap_trp_tx_cback_buf_type  trp_tx_buf_f_ptr;        
    /**< Transport Tx function pointer.  heap buffer */
...
/** @ingroup eap_input_pkt
  ...
  @vertspace
  @param[in] eap_handle   Handle to the EAP instance.
  @param[in] eap_pkt      EAP input packet buffer.
  @param[in] eap_pkt_len  EAP input packet length
...
===========================================================================*/
/** @ingroup eap_set_result
  Called by the authentication protocol to notify the EAP of the
  authentication process result. This is for clients that do not get EAP
  SUCCESS/FAILURE packets from network. Library will notify the result from
  meta info
