...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/api/public/ps_iface_defs.h#9 $ : //source/qcom/qct/modem/api/datacommon/main/latest/ps_iface_defs.h#41 $ $DateTime: 2022/12/05 04:40:37 $ $Author: pwbldsvc $

when        who    what, where, why
--------    ---    ----------------------------------------------------------
04/12/17    vls    Added new net down reason for redirection not allowed case.
10/31/17    ak     PCSCF Restoration over untrusted WLAN  
12/15/16    vls    Added PS_NET_DOWN reasons for new VZW specific IWLAN failures
04/10/15    fn     Added PS_NET_DOWN reason for ipsec cert failures
...
/*---------------------------------------------------------------------------
  ...
  2 - 3GPP Extra Emergency ifaces, 2 rmnet ifaces, 2 wlan ifaces
  Note : Extra emergency iface is just added to allow IMS client to bring up 
  another emergency call. 3GPP modehandler will bring down existing emergency call on recieving
  second request   
---------------------------------------------------------------------------*/
#define MAX_SYSTEM_IFACES  125
...
typedef enum
{
  ...
  /*-------------------------------------------------------------------------
    See IFACE_PHYS_LNK_MIN_EV to IFACE_PHYS_LINK_EVENT_MAX for next set of 
    range. Add any new physlink events between IFACE_PHYS_LNK_MIN_EV to 
    IFACE_PHYS_LINK_EVENT_MAX
  -------------------------------------------------------------------------*/
  ...
  */
  ...
  /**< Posted by mode handler when modem reserved pco(protocol configuration 
       ...
       purpose.
  */
   IFACE_APN_PARAM_CHANGED_EV                         = 80,
  /**< Posted by mode handler when network updates the APN parameter 
       information. This information is saved on the iface.
  */
  IFACE_EMBMS_CONTENT_DESC_CONTROL_EV                 = 81,
  /**< Posted by mode handler to indicate eMBMS content description settings
  */
  IFACE_EMBMS_SVC_INTEREST_IND_EV                     = 82,
  /**< Posted by mode handler to QMI to indicate eMBMS Service Interest.
  */
  IFACE_RESET_EV                                      = 83,
  /** Posted when modehandler resets iface with ps_ifacei_reset_cmd on
      Active Physical iface. Logical framework is registered for this 
      event and will bring down epc iface.
  */
  IFACE_SET_OFFLOAD_FC_EV                             = 84,
  /**< Posted by mode handler to enable or disable offload flow control.
  */

  IFACE_DPD_PING_INFO_EV                             = 85,
  /**< Posted by mode handler to indicate the statistics 
       associated with the DPD ping exchange on IWLAN_S2B.
  */

  IFACE_NET_CLOSE_IN_PROGRESS_EV                      = 86,
  /**< Posted by ps iface during call bring up request for
       async mode only.
     */
  IFACE_ALL_EV                                        = 87,
  ...
  /*-------------------------------------------------------------------------
    See IFACE_MIN_EV to PHYS_LINK_MAX_EVENT for previous set of physlink
    event range. Any new phys link event goes under below range
  -------------------------------------------------------------------------*/

  PHYS_LINK_EX_MIN_EV                               = IFACE_EVENT_MAX,

  PHYS_LINK_RAN_ASST_MEDIA_ADAPT_INFO_EV            = PHYS_LINK_EX_MIN_EV,
  /**< Posted by mode handler to update RAN assist media adaptation information
       from the network.
  */

#ifdef FEATURE_DATA_FLOW_MGMT
  IFACE_PHYS_LINK_FLOW_CONTROL_EV                   = PHYS_LINK_EX_MIN_EV + 1,
#endif /* FEATURE_DATA_FLOW_MGMT */

  /**< Posted by mode handler to Send Flow control information.
  */

  PHYS_LINK_PACKET_DISCARD_INFO_EV            = PHYS_LINK_EX_MIN_EV + 2,
  /**< Posted by mode handler whenever packet is discarded on the bearer.
  */
  ...
  -------------------------------------------------------------------------*/
  FLOW_MIN_EV                     = PHYS_LINK_EX_MAX_EV,
  ...
  /*---------------------------------------------------------------------------
    Event raised when filter is added to flow in assoc iface. EPC iface
    will install qos reflective filters
  ---------------------------------------------------------------------------*/  
  FLOW_REFLECTIVE_QOS_FILTER_UPDATED_EV   = FLOW_MIN_EV + 16,
  FLOW_MAX_EV
...
DESCRIPTION
  ...
  The type is 2 for all Internal error codes.
  The type is 3 for all CM error codes.
  ...
  The type is 11 for all IWLAN error codes.
...
typedef enum
{
  ...
  PS_NET_DOWN_REASON_PTI_MISMATCH = TYPE_REASON_VAL(6,47),
  /**< The reason is posted by the UE to indicate that the PTI value which 
       is included in the ESM message that the UE receives does not match a 
       PTI in use.	   
       Failure Type: Temporary
       Recovery Mechanism:	Client may attempt a call again */
  PS_NET_DOWN_REASON_ACTIVATION_REJECTED_BCM_VIOLATION = TYPE_REASON_VAL(6,48),
  /**< The reason is posted by the network when UE requested to modify QoS 
       parameters or the Bearer control mode which is not compatible with the 
       selected Bearer control mode. 	  
	   Failure Type: Temporary 
	   Recovery Mechanism:	
	   Client Shall retry after BCM field of PCO is modified by NW  */
  PS_NET_DOWN_REASON_LAST_PDN_DISCONN_NOT_ALLOWED   = TYPE_REASON_VAL(6,49),
  /**< The reason is posted by the network when UE sends PDN disconnect request. 
       This may happen when network and UE are out of sync.
       Failure Type: Temporary
       Recovery Mechanism: Client may attempt a call again */
  ...
  /**< The reason is posted by the network to indicate that the network has 
       ...
       wait for the corresponding time period before retrying the call. If the
       PDN is not throttled, the client may retry immediately.  */
  PS_NET_DOWN_REASON_IP_V4V6_ONLY_ALLOWED       = TYPE_REASON_VAL(6,57),
  ...
  PS_NET_DOWN_REASON_NON_IP_ONLY_ALLOWED        = TYPE_REASON_VAL(6,58),
  ...
  */
  ...
  PS_NET_DOWN_REASON_CALL_PREEMPT_BY_EMERGENCY_APN = TYPE_REASON_VAL(6,127),
  /**< 
      This reason is posted by the UE when the data call is brought down
      due to emergency call bringup on different ePDG. 
            
      Failure Type       : Temporary
      Recovery Mechanism : Client may retry immediately*/

  PS_NET_DOWN_REASON_UE_INIT_DETACH_OR_DISCONNECT = TYPE_REASON_VAL(6,128),
  /**< The reason is posted by the UE when it performs a detache or disconnect 
       PDN action based on TE requirements.
  ...
  PS_NET_DOWN_REASON_INTERNAL_CALL_ENDED          = TYPE_REASON_VAL(2,202),
    /**< 
        ...
        Recovery Mechanism : Client may retry after some time
    */
  /* RESERVED  = TYPE_REASON_VAL(2,203) */
  /* RESERVED  = TYPE_REASON_VAL(2,204) */
  ...
  */
  ...
  /**< 
      This error code is returned when a call bring up is requested with an invalid
      profile number
      Failure Type       : Temporary Failure 
      Recovery Mechanism : Retry the call using a profile number that is created on
                           the Modem.
                           For emergency calls, the profile number passed by application
                           should always be non-zero.
                           For non-emergency calls, if the profile number passed by
                           application is 0, UE will bring up a call using the
                           default profile.
     */ 
...
PS_NET_DOWN_REASON_APN_DISALLOWED_ON_ROAMING      = TYPE_REASON_VAL(2,239),
  /**< 
      ...
      Recovery Mechanism : Retry using an APN name that is
                           ...
                           retry when UE moves back to home network.
                                                                       */
  PS_NET_DOWN_REASON_APN_PARAM_CHANGE               = TYPE_REASON_VAL(2,240),
  /**< 
  ...
  */
  ...
  PS_NET_DOWN_REASON_DDS_CALL_ABORT             = TYPE_REASON_VAL(2,248),
   /**< 
       The error code is returned when DDS switch occurs, so data call is 
       brought down with DDS abort reason.
  ...
  PS_NET_DOWN_REASON_VALIDATION_FAILURE          = TYPE_REASON_VAL(2,249),
  /**< 
  ...
  */
    ...
    */
    ...
    PS_NET_DOWN_REASON_DDS_SWITCH_IN_PROGRESS      = TYPE_REASON_VAL(2,254),
    ...
    PS_NET_DOWN_REASON_CALL_DISALLOWED_IN_ROAMING = TYPE_REASON_VAL(2,255),
  ...
  PS_NET_DOWN_REASON_MO_EXCEPTIONAL_NOT_SUPPORTED      = TYPE_REASON_VAL(2,256),
  ...
  PS_NET_DOWN_REASON_NON_IP_NOT_SUPPORTED      = TYPE_REASON_VAL(2,257),
    /**<  
  ...
  PS_NET_DOWN_REASON_ERR_PDN_NON_IP_CALL_THROTTLED  = TYPE_REASON_VAL(2,258),
    /**< 
  ...
  PS_NET_DOWN_REASON_ERR_PDN_NON_IP_CALL_DISALLOWED = TYPE_REASON_VAL(2,259),
    /**< 
...
PS_NET_DOWN_REASON_IMS_SETTINGS_DISABLED            = TYPE_REASON_VAL(2,269),
  /**< 
     The error code is returned when PDN Connection to a given APN is
     disallowed because IMS is disabled from the IMS client.
     
     Failure Type       : Temporary Failure 
     Recovery Mechanism : Application can retry with the same APN
                          if IMS is re-enabled 
  */
...
/* To map GSM/WCDMA specific call-end reasons from CM */
  ...
  */
  ...
  /**<   This reason is returned if there was a 
         ...
         Recovery Mechanism : Retry after a back-off time period 
  */
  PS_NET_DOWN_REASON_MULTI_PDN_NOT_ALLOWED            = TYPE_REASON_VAL(3,1112),
  /**< 
         This error code is returned when Multiple PDP call Feature is disabled
         Failure Type       : Temporary Failure
         Recovery Mechanism : UE can retry after active PDP call have ended 
  */
  PS_NET_DOWN_REASON_EMBMS_NOT_ENABLED                = TYPE_REASON_VAL(3,1113),
  /**< 
         This error code is returned when data call is brought down because EMBMS  
         is not enabled at RRC layer
         Failure Type       : Temporary Failure
         Recovery Mechanism : UE can retry after EMBMS has been enabled 
  */
  PS_NET_DOWN_REASON_PENDING_REDIAL_CALL_CLEANUP      = TYPE_REASON_VAL(3,1114),
  /**< 
         This error code is returned when data call is brought down if it is found
         in stale state because data call was un-sucessfully transfered during IRAT
         handover
         Failure Type       : Temporary Failure
         Recovery Mechanism : Retry after a back-off time period 
  */

  PS_NET_DOWN_REASON_EMBMS_REGULAR_DEACTIVATION       = TYPE_REASON_VAL(3,1115),
  /**< 
         This error code is returned when EMBMS data call is sucessfully brought
         down 
         Failure Type       : Temporary Failure
         Recovery Mechanism : Client may retry 
  */
  PS_NET_DOWN_REASON_TLB_REGULAR_DEACTIVATION         = TYPE_REASON_VAL(3,1116),
  /**< 
         This error code is returned when Test Loopback data call is sucessfully
         brought down 
         Failure Type       : Temporary Failure
         Recovery Mechanism : Client may retry 
  */
  PS_NET_DOWN_REASON_LOWER_LAYER_REGISTRATION_FAILURE = TYPE_REASON_VAL(3,1117),
  /**< 
         This error code is returned when data call is brought down when lower
         layer registration fails
         Failure Type       : Temporary Failure
         Recovery Mechanism : Retry after a back-off time period 
  */
  ...
  */
  ...
  PS_NET_DOWN_REASON_IRAT_ABORT = TYPE_REASON_VAL(3,1153),
    /**< 
         This reason is posted to indicate that the PDN request failure during 5G/4G IRAT
         Failure Type      : Temporary Failure
         Recovery Mechanism: Client may retry upon full service on target RAT.
*/
...
/* To map HDR specific call-end reasons from CM */
  ...
  PS_NET_DOWN_REASON_HDR_ACC_FAIL              = TYPE_REASON_VAL(3,1514),
   ...
   PS_NET_DOWN_REASON_UNSUPPORTED_1X_PREV   = TYPE_REASON_VAL(3,1515),
   /**< 
       The error code is returned when the prev supported by 1 Base Station is 
       < 6 which is not supported for 1X data call. The UE has to be in 
       footprint of BS which has p-rev >=6 to support this SO33 call.

       Failure Type       :Permanent
       Recovery Mechanism :Move to a BS which supports prev>=6
   */
/* To map technology-agnostic call-end reasons from CM */
  ...
  PS_NET_DOWN_REASON_CM_UNKNOWN_ERROR         = TYPE_REASON_VAL(3,2007),
    /**< 
        This error code is returned when data call is brought down due 
        to some unknown reason from lower layers.
        Failure Type       : Temporary Failure
        Recovery Mechanism : Retry after a back-off time period
    */
...
/* To map some call cmd err reasons from CM */
  ...
  PS_NET_DOWN_REASON_CALL_CONTROL_REJECTED    = TYPE_REASON_VAL(3,2506),
    ...
    */ 
  PS_NET_DOWN_REASON_EMM_DETACHED_PSM    = TYPE_REASON_VAL(3,2507),
    /**< 
        This error code is returned when data call is brought down  
    ...
    */ 
  PS_NET_DOWN_REASON_DUAL_SWITCH              = TYPE_REASON_VAL(3,2508),
    /**< 
        This error code is returned when data call is brought down  
        because Dual Switch from Single Standby to Dual Standby
        Failure Type       : Temporary Failure
        Recovery Mechanism : Retry call origination after some time
    */ 	
  PS_NET_DOWN_REASON_REJECTED_BY_CALL_MANAGER = TYPE_REASON_VAL(3,2509),
    /**< 
        This error code is returned when data call is brought down  
        because call manager reject the data call for some unspecific 
		reasons
        Failure Type       : Temporary Failure
        Recovery Mechanism : Retry call origination after some time
    */
  PS_NET_DOWN_REASON_INVALID_CLASS3_APN  = TYPE_REASON_VAL(3,2510),
     /**<
        This error code is returned when data call is brought down  
        ...
        Recovery Mechanism : Retry call origination after some time
    */
  PS_NET_DOWN_REASON_MPLMN_IN_PROGRESS   = TYPE_REASON_VAL(3,2511),
    /**< 
        This error code is returned when data call is brought down  
        because call manager find there is MPLMN is in progress and
        send ds cmd err as CM_CALL_CMD_ERR_MPLMN_IN_PROGRESS 
        Failure Type       : Temporary Failure
        Recovery Mechanism : Retry call origination after some time
    */
  PS_NET_DOWN_REASON_NO_TRM  = TYPE_REASON_VAL(3,2512),
    /**< 
        This error code is returned when CM rejects the call due to
        TRM unavailability by lower layers.
        Failure Type       : Temporary Failure
        Recovery Mechanism : Retry call origination after some time
    */	
  ...
  /* RESERVED  = TYPE_REASON_VAL(5,5100) */
  PS_NET_DOWN_REASON_IPSEC_GW_UNREACHABLE = TYPE_REASON_VAL(5,5101),
  PS_NET_DOWN_REASON_IPSEC_AUTH_FAILED,
  PS_NET_DOWN_REASON_IPSEC_CERT_INVALID,
  PS_NET_DOWN_REASON_IPSEC_CERT_EXPIRY,

  /* MIP Error codes. */
  /* RESERVED  = TYPE_REASON_VAL(1,64) */
  ...
  PS_NET_DOWN_REASON_MIP_FA_ERR_MISSING_HOME_ADDR               = TYPE_REASON_VAL(1,99),
    /**< 
        ...
        Recovery Mechanism : Check with network provider/Retry call  
                             ...
                             the device with MIP information
    */
  /* RESERVED  = TYPE_REASON_VAL(1,128) */
  ...
  PS_NET_DOWN_REASON_MIP_HA_ERR_ENCAPSULATION_UNAVAILABLE       = TYPE_REASON_VAL(1,139),
    /**< 
        ...
        Recovery Mechanism : Retry call origination after correctly  
                             ...
                             information
    */
  /* RESERVED  = TYPE_REASON_VAL(1,65535) */
  ...
  PS_NET_DOWN_REASON_PPP_ERR_CLOSE_IN_PROGRESS                  = TYPE_REASON_VAL(7,33),
   ...
   PS_NET_DOWN_REASON_NV_REFRESH_IN_PROGRESS                  =   TYPE_REASON_VAL(7,34),
    /**< 
        ...
        Recovery Mechanism : Retry call origination after sometime
    */
  /* RESERVED  = TYPE_REASON_VAL(7,65535) */
  ...
  */
  ...
  PS_NET_DOWN_REASON_IWLAN_S2B_IKEV2_CERT_INVALID = TYPE_REASON_VAL(11,16),
    /**< This error code is returned when the certificate is found to be invalid
         during ipsec processing
         Failure Type       : Permenant Failure
         Recovery Mechanism : Client should retry call and retrieve a new
                              valid certificate after properly provisioning
                              the certificate credentials */

  PS_NET_DOWN_REASON_IWLAN_S2B_IKEV2_CERT_EXPIRY = TYPE_REASON_VAL(11,17),
    /**< This error code is returned when a valid certificate is found 
         to be expired or revoked during iWLAN tunnel bringup
         Failure Type       : Temporary Failure
         Recovery Mechanism : Make new call to retrieve a new valid certificate */
 ...
 PS_NET_DOWN_REASON_IWLAN_S2B_IKEV2_PROTOCOL_FAILURE          = TYPE_REASON_VAL(11,19), 
  ...
  PS_NET_DOWN_REASON_IWLAN_S2B_IKEV2_EAP_AUTH_FAILURE            = TYPE_REASON_VAL(11,24),
  /**<   This reason is returned when data call bringup fails in IKEv2 setup
         due to EAP authentication failure
         configuration      
         Failure Type       : Permanent Failure
         Recovery Mechanism : Retry call origination after correctly  
                              provisioning the device with authentication
  ...
  PS_NET_DOWN_REASON_IWLAN_S2B_PGW_OVERLOAD                      = TYPE_REASON_VAL(11,25),
    /**<   This reason is returned when the PDN request is rejected with PGW overload
           This is classified under minor network failures     
         Failure Type       : Temporary Failure
         Recovery Mechanism : Retry call origination after some time */

  PS_NET_DOWN_REASON_IWLAN_S2B_PERMANENT_FAILURE                 = TYPE_REASON_VAL(11,26),
    /**<   This reason is returned when network rejected with permanent failure     
           This correspond to network permanent failures.
           Without any changes on the network, it is unlikely that the authentication will 
           succeed
         Failure Type       : Permenent Failure
         Recovery Mechanism : Retry call origination after reboot */  
  
  PS_NET_DOWN_REASON_IWLAN_S2B_REQUEST_REJECTED_IF_S2B           = TYPE_REASON_VAL(11,27),
    /**<   This reason is returned when the PDN request is rejected in S2B level
           This is classified under minor network failures               
         Failure Type       : Temporary Failure
         Recovery Mechanism : Retry call origination after some time */  

  PS_NET_DOWN_REASON_IWLAN_S2B_PROTOCOL_ERROR_IF_S2B             = TYPE_REASON_VAL(11,28),
    /**<   This reason is returned when the PDN request is rejected with S2B level 
           protocol error
           This is classified under minor network failures
         Failure Type       : Temporary Failure
         Recovery Mechanism : Retry call origination after some time */  
  
  PS_NET_DOWN_REASON_IWLAN_S2B_NO_SUBSCRIPTION_IF_S2B            = TYPE_REASON_VAL(11,29),
    /**<   The reason is posted when UE requested a service option for which it 
         has no subscription in S2B level
         Failure Type       : Permanent Failure
         Recovery Mechanism : Retry call origination after UICC replacement */  

  PS_NET_DOWN_REASON_IWLAN_S2B_MISSING_OR_UNKNOWN_APN_IF_S2B     = TYPE_REASON_VAL(11,30),
    /**<   This reason is returned when network doesn't accept the incoming APN
         Failure Type       : Permanent Failure
         Recovery Mechanism : Retry call origination after UI?CC replacement */   
  
  PS_NET_DOWN_REASON_IWLAN_S2B_UNSUPPORTED_PDN_TYPE_IF_S2B       = TYPE_REASON_VAL(11,31),
    /**<   This reason is returned when network doesn't support the PDN type
           This correspond to network permanent failures.
           Without any changes on the network, it is unlikely that the authentication will 
           succeed
         Failure Type       : Permenent Failure
  ...
  PS_NET_DOWN_REASON_IWLAN_S2B_RAT_DISALLOWED_IF_S2B             = TYPE_REASON_VAL(11,32),
    /**<   This reason is returned when data call bringup fails 
         due to network not allowing IWLAN RAT in S2B level     
           This correspond to network permanent failures.
           Without any changes on the network, it is unlikely that the authentication will 
           succeed
         Failure Type       : Permenent Failure
  ...
  PS_NET_DOWN_REASON_IWLAN_S2B_EPDG_ADDR_UNREACHABLE             = TYPE_REASON_VAL(11,33), 
  /**<     This reason is returned when data call bringup fails because there are no
           reachable EPDG addresses.
         Failure Type       : Temporary Failure
         Recovery Mechanism : Retry call origination after some time */    
  
  PS_NET_DOWN_REASON_IWLAN_S2B_REACTIVATION_REQ                  = TYPE_REASON_VAL(11,34), 
  /**<     This reason is returned when network wants the UE to re-establish the tunnel 
        and corresponding PDN connection after the current PDN connection is released.
        Failure Type       : Temporary Failure
        Recovery Mechanism : Retry call origination immediately */    

  PS_NET_DOWN_REASON_IWLAN_S2B_CALL_PREEMPT_BY_ANOTHER_APN  =  TYPE_REASON_VAL(11,35),
  /**< 
      This reason is posted by the UE when the data call is brought down
      due to another APN broughtup on different ePDG.
            
      Failure Type       : Temporary
      Recovery Mechanism : Client may retry immediately*/

  PS_NET_DOWN_REASON_IWLAN_S2B_UNAUTHENTICATED_EMERGENCY_UNSUPPORTED =  TYPE_REASON_VAL(11,36),
  /**< 
      This reason is posted by the UE when unauthenticated emergency call is not supported by the ePDG.
            
      Failure Type       : Temporary
      Recovery Mechanism : Client may retry after system status indication indicates that IWLAN becomes 
                           available for usage again per APN.*/

  PS_NET_DOWN_REASON_IWLAN_S2B_IMEI_NOT_ACCEPTED  =  TYPE_REASON_VAL(11,37),
  /**< 
      This reason is posted by the UE when the IMEI is not accepted for unauthenticated emergency call.

      Failure Type       : Temporary
      Recovery Mechanism : Client may retry after system status indication indicates that IWLAN becomes 
                           available for usage again per APN.*/

  PS_NET_DOWN_REASON_IWLAN_S2B_REDIRECT_NOT_ALLOWED        = TYPE_REASON_VAL(11,38),
  /**< 
      This reason is posted by the UE when the data call is brought down
      due to server giving redirect request & redirect of current PDN is 
      not allowed.

      Failure Type       : Temporary
      Recovery Mechanism : Client may retry after some time */

  PS_NET_DOWN_REASON_IWLAN_S2B_NON_3GPP_ACCESS_TO_EPC_NOT_ALLOWED =  TYPE_REASON_VAL(11,39),
  /**< 
      This reason is posted by the UE when the network does not allow non-3GPP
      access to EPC. IWLAN would try with another ePDG on a different PLMN if available
      or IWLAN would throttle calls if it cannot find any.
            
      Failure Type       : Temporary
      Recovery Mechanism : Client may retry after some time.*/ 

  PS_NET_DOWN_REASON_IWLAN_S2B_USER_UNKNOWN  =  TYPE_REASON_VAL(11,40),
  /**< 
      This reason is posted by the UE when the user identified by the IMSI is
      unknown to the network. IWLAN would try with another ePDG on a different PLMN if available
      or IWLAN would throttle calls if it cannot find any.

      Failure Type       : Temporary
      Recovery Mechanism : Client may retry after some time. */

  PS_NET_DOWN_REASON_IWLAN_S2B_AUTHORIZATION_REJECTED  =  TYPE_REASON_VAL(11,41),
  /**< 
      This reason is posted by the UE when the user is not authorized by the network.
      IWLAN would try with another ePDG on a different PLMN if available
      or IWLAN would throttle calls if it cannot find any.

      Failure Type       : Temporary
      Recovery Mechanism : Client may retry after some time. */

  PS_NET_DOWN_REASON_IWLAN_S2B_ILLEGAL_ME  =  TYPE_REASON_VAL(11,42),
  /**< 
      This reason is posted by the UE when the mobile equipment (ME) used is not accepted by the network.
      IWLAN would try with another ePDG on a different PLMN if available
      or IWLAN would throttle calls if it cannot find any.
...
typedef enum
{
  ...
  PS_IFACE_APN_TYPE_INTERNET     = 1,
  PS_IFACE_APN_TYPE_IMS          = 2,
  PS_IFACE_APN_TYPE_VSIM         = 3,
  PS_IFACE_APN_TYPE_MMS           = 4,
  PS_IFACE_APN_TYPE_DUN           = 5,
  PS_IFACE_APN_TYPE_SUPL          = 6,
  PS_IFACE_APN_TYPE_HIPRI         = 7,
  PS_IFACE_APN_TYPE_FOTA          = 8,
  PS_IFACE_APN_TYPE_CBS           = 9,
  PS_IFACE_APN_TYPE_IA            = 10,
  PS_IFACE_APN_TYPE_EMERGENCY     = 11,
  PS_IFACE_APN_TYPE_MAX 
...
/**< This mask value is set when OP Reserved PCO changes compared to 
     previously configured value */
#define PS_IFACE_EXT_IP_CFG_MASK_OP_RES_PCO_LIST  (0x80000)

/**< This mask value is set when MSISDN PCO changes compared to 
     previously configured value */
#define PS_IFACE_EXT_IP_CFG_MASK_MSISDN_PCO       (0x100000)

/**< This mask value is set when ePDG address changes compared to 
     previously configured value */
#define PS_IFACE_EXT_IP_CFG_MASK_EPDG_ADDR       (0x200000)

/**< This mask value is set when WIFI IP address changes compared to 
     previously configured value */

#define PS_IFACE_EXT_IP_CFG_MASK_WIFI_ADDR       (0x400000)
...
typedef enum
{
  PS_IFACE_DATA_RATE_TYPE_BPS,
  PS_IFACE_DATA_RATE_TYPE_KBPS,
  PS_IFACE_DATA_RATE_TYPE_MBPS,
  PS_IFACE_DATA_RATE_TYPE_GBPS
} ps_iface_data_bearer_rate_enum_type;

typedef struct
{
  ps_iface_data_bearer_rate_enum_type rate_type;
  
  int64 max_tx_bearer_data_rate;      /*Max Tx bearer data rate*/
  int64 max_rx_bearer_data_rate;      /*Max Rx bearer data rate*/
  int64 avg_tx_bearer_data_rate;      /*Average Tx bearer data rate*/
  int64 avg_rx_bearer_data_rate;      /*Average Rx bearer data rate*/
  int64 current_tx_bearer_data_rate;  /*Current Tx bearer data rate*/
  int64 current_rx_bearer_data_rate;  /*Current Rx bearer data rate*/
} ps_iface_data_bearer_rate_ex;
...
typedef struct
{
  ...
  ps_iface_data_bearer_rate_ex data_rate_ex; /*Extended Data Rate Type*/
...
/*---------------------------------------------------------------------------
   Enum for direction to use during filter match. TFT packet filter direction defined (per 3GPP TS24.008 10.5.6.12).
---------------------------------------------------------------------------*/
typedef enum
{
  PS_QOS_FILTR_DIR_BIDIR        = 0x0,     
  /* Filter match in Both direction (Bi-Directional) */
  PS_QOS_FILTR_DIR_DL           = 0x1, 
  /* Filter match in downlink direction                      */
  PS_QOS_FILTR_DIR_UL           = 0x2,   
  /* Filter match in uplink direction                          */
  PS_QOS_FILTR_DIR_UL_OR_DL     = 0x3, 
  /* Filter match in either RX or TX directions or both */
  PS_QOS_FILTR_DIR_MAX          = 0x4, 
} ps_iface_qos_fltr_match_dir_enum_type;
...
typedef struct
{
   ...
   ps_iface_qos_fltr_match_dir_enum_type        fltr_match_dir;
...
typedef enum
{
  PS_IFACE_HANDOFF_ANCILLARY_INFO_NONE            = 0x0,
  PS_IFACE_HANDOFF_ANCILLARY_INFO_WITH_N26_FLAG   = 0x1,
  PS_IFACE_HANDOFF_ANCILLARY_INFO_MAX         = 0xFFFFFFFF
  
}ps_iface_handoff_ancillary_info_mask_enum_type;

typedef struct
{
  ps_iface_handoff_ancillary_info_mask_enum_type    ancillary_info_mask;
}ps_iface_handoff_ancillary_info_type;

typedef struct
{
  ds_sys_rat_ex_enum_type  srat;
  ds_sys_rat_ex_enum_type  trat;
  ps_iface_handoff_ancillary_info_type  ancillary_info;
...
{
  ps_flow_type         *  flow_ptr;
  uint8                   next_hdr_protocol;
  uint32                  dst_port;  
  ip_version_enum_type    ip_vsn;
} ps_iface_reflective_qos_filter_type;

/**
  @brief Structure to be used with PHYS_LINK_RAN_ASST_MEDIA_ADAPT_INFO_EV
         and PS_PHYS_LINK_IOCTL_GET_RAN_ASSIT_MEDIA_ADAPT_INFO
*/
typedef enum{
  PS_PHYS_LINK_DATA_FLOW_UPLINK   = 0x01,
  PS_PHYS_LINK_DATA_FLOW_DOWNLINK = 0x02
}ps_phys_link_data_flow_direction_enum_type;

typedef struct
{
  ps_phys_link_data_flow_direction_enum_type   direction;
  /**< Direction.
  */

  uint8                                        bit_rate;
  /**< Index for bit rate. Range from 0 to 63 */
}ps_phys_link_bit_rate_info_type;

typedef struct
{
  ps_phys_link_bit_rate_info_type     bit_rate_info;

  boolean                             is_rohc_enabled;
  /**< ROHC. Values:\n
       -0: ROHC Disable.
       -1: ROHC Enable.
  */
}ps_phys_link_ran_asst_media_adapt_info_type;

/**
  @brief Structure to be used with IFACE_DPD_PING_INFO_EV
*/
#define PS_IFACE_RTT_LENGTH_MAX  255
typedef struct
{
  uint8       num_pings;
  uint8       pkts_loss;
  uint16      avg_rtt;                       /**< Average RTT in ms */
  uint16      rtt[PS_IFACE_RTT_LENGTH_MAX];  
  /**< RTT in ms for each ping. Length of this array is num_pings. 
       0xFFFF indicates loss packets */
  void      * user_data_ptr;
...
typedef enum
{
  PS_PHYS_LINK_DATA_TRAFFIC_NONE           = 0x0,
  PS_PHYS_LINK_DATA_TRAFFIC_TCP_BI_DIR     = 0x1
}ps_phys_link_data_traffic_enum_type;

typedef enum
{
  PS_PHYS_LINK_FLOW_CTL_REASON_NONE           = 0x0,
  PS_PHYS_LINK_FLOW_CTL_REASON_UNSPECIFIED    = PS_PHYS_LINK_FLOW_CTL_REASON_NONE,
  PS_PHYS_LINK_FLOW_CTL_REASON_PROHIBITED_TIMER_EXP  = 0x1,
  PS_PHYS_LINK_FLOW_CTL_REASON_PERIODIC_TIMER_EXP    = 0x2

}ps_phys_link_flow_ctl_reason_enum_type;


typedef struct 
{
  uint64                                  flow_mask;
  uint32                                  allowed_num_of_bytes;
  boolean                                 ack_required;
  boolean                                 is_flow_enable;
  ps_phys_link_data_traffic_enum_type     traffic_type;
  ps_phys_link_flow_ctl_reason_enum_type  flow_ctl_reason;
  
} ps_phys_link_flow_ctl_cmd_params_type;
#endif /* FEATURE_DATA_FLOW_MGMT */
...
typedef union
{
  ...
  ps_iface_reflective_qos_filter_type     reflective_qos_fltr_info;
 ...
 /**< Event info for events: IFACE_EMBMS_AVAILABLE_SAI_LIST_EV*/
  ...
  /**< Event info associated with IFACE_EMBMS_CONTENT_DESC_CONTROL_EV */

  ps_iface_embms_svc_interest_ind_type embms_svc_interest_ind_info;
  /**< Event info associated with IFACE_EMBMS_SVC_INTEREST_IND_EV */

  boolean disable_offload_fc;
  /**< Event info associated with IFACE_SET_OFFLOAD_FC_EV
  ...
  ps_phys_link_ran_asst_media_adapt_info_type  ran_asst_info;
    ...
    ps_iface_dpd_ping_stat_info_type          *dpd_ping_stat_info;
...
/*===========================================================================
MACRO PS_PHYS_LINK_GET_BEARER_EX_ID()

DESCRIPTION
  Returns the bearer ex id associated with a phys link.
...
===========================================================================*/
#define PS_IFACE_GET_BEARER_ID_EX_FROM_PHYSLINK(ps_iface_ptr, phys_link_ptr)                            \
  ps_iface_get_bearer_id_ex_from_physlink( ps_iface_ptr, phys_link_ptr)
