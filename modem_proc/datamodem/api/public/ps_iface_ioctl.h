...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/api/public/ps_iface_ioctl.h#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $

when        who    what, where, why
--------    ---    ----------------------------------------------------------
2015-21-05  pg  Added IOCTL support for PDN detach.
04/23/15    wc     Added queueing delay ioctls
...
#define PS_IFACE_IOCTL_MAX_COUNTRY_CODE_LEN  2   
/**< Must be the same as PS_SYS_MAX_COUNTRY_CODE_LEN */
...
typedef enum
{
  ...
  PS_IFACE_IOCTL_GET_PATH_MTU             = 15, /* Get Data Path MTU of Iface */ 
  ...
  PS_IFACE_IOCTL_GET_PDN_TEARDOWN_CONFIG = 69, /* Get the PDN tear down 
                                                  configuration*/  
  PS_IFACE_IOCTL_GET_OP_RES_PCO_LIST     = 70, /* Get operator reserved pco List*/

  PS_IFACE_IOCTL_GET_MSISDN_INFO         = 71, /* Get MSISDN info*/
  PS_IFACE_IOCTL_GET_FLOW_LIST_FROM_FLTR = 72, 
  /* Get Phys link from application requested iface QoS filters */
  /*-------------------------------------------------------------------------
  ...
  PS_IFACE_IOCTL_GET_COMPANION_IFACE      = 133, 
  /**< Query the companion iface(other iface)*/

  PS_IFACE_IOCTL_GET_UL_PACKET_DELAY_METRICS   = 135, /* Get UL delay metrics */
  PS_IFACE_IOCTL_RESET_UL_PACKET_DELAY_METRICS = 136, /* Reset UL delay metics*/
  PS_IFACE_IOCTL_KEEP_ALIVE                    = 137, /* Keep alive ioctl */
  PS_IFACE_IOCTL_FILTER_MATCH_REQUEST          = 138, /* Filter match ioctl */
  PS_IFACE_IOCTL_GET_3GPP_COMPANION_IFACE      = 139, 
  PS_IFACE_IOCTL_SET_TCP_LEVEL_TRM_REQUEST     = 140,
  /**< Query the companion iface of 3gpp*/
  /*-------------------------------------------------------------------------
  ...
  PS_IFACE_IOCTL_EMBMS_SVC_INTEREST = 909, /**< Service Interest */
  ...
  PS_IFACE_IOCTL_WLAN_GET_COUNTRY_CODE    = 1004, 
  /**< Query the country code */
  PS_IFACE_IOCTL_WLAN_INITIATE_DPD_CHECK = 1005, 
  /**< Application wants to intitiate DPD check */
  PS_IFACE_IOCTL_WLAN_START_DPD_PING = 1006, 
  /**< Application wants to start DPD pings */
...
/**
  @brief Rx and Tx filter spec type
*/
typedef struct
{
...
typedef struct
{
  ps_iface_filter_spec_type                   net_initiated_qos_spec;
  /**< QOS specification */
...
/**
@brief Data type for specifying the parameters for
PS_IFACE_IOCTL_QOS_NET_INITIATED_REAL_TIME_CHECK and 
PS_IFACE_IOCTL_GET_FLOW_LIST_FROM_FLTR

@see PS_IFACE_IOCTL_QOS_NET_INITIATED_REAL_TIME_CHECK
@see PS_IFACE_IOCTL_GET_FLOW_LIST_FROM_FLTR
*/
typedef struct
{
  ps_iface_filter_spec_type                   net_initiated_qos_spec;
...
typedef struct
{
  ps_iface_filter_spec_type                  fltr_spec;
  /**< QOS specification */
  list_type                                  matching_flow_list;
  /**< [out] List of ps_iface_ioctl_matching_flow_type *. */
  ps_iface_qos_fltr_match_dir_enum_type      fltr_match_dir;
  /**< Direction for filter matching */
} ps_iface_ioctl_fltr_check_type;
...
typedef enum
{
  PS_IFACE_IOCTL_DATA_PATH_HARDWARE = 0, /* Hardware Data Path */
  PS_IFACE_IOCTL_DATA_PATH_SOFTWARE = 1,  /* Software Data Path */
  PS_IFACE_IOCTL_DATA_PATH_MAX
} ps_iface_ioctl_data_path_enum_type;

typedef enum
{
  PS_IFACE_IOCTL_CLIENT_MIN  = 0,
  PS_IFACE_IOCTL_CLIENT_DSS  = PS_IFACE_IOCTL_CLIENT_MIN, 
  PS_IFACE_IOCTL_CLIENT_WDS  = 1,
  PS_IFACE_IOCTL_CLIENT_DFS  = 2,
  PS_IFACE_IOCTL_CLIENT_MAX  = PS_IFACE_IOCTL_CLIENT_DFS  
} ps_iface_ioctl_clients_enum_type;
...
typedef struct
{
  ...
  ps_iface_ioctl_clients_enum_type    client_type;
...
/*---------------------------------------------------------------------------
  Queueing delay metrics related
---------------------------------------------------------------------------*/
typedef struct
{
  uint32  max;
  ...
  uint32  avg;
  uint32  num_samples;
} ps_iface_ioctl_delay_metric_type;

typedef struct
{
  uint8                             bearer_id;
  ps_iface_ioctl_delay_metric_type  rm_l2_queueing_delay;
  ps_iface_ioctl_delay_metric_type  l2_tx_delay;
} ps_iface_ioctl_per_bearer_ul_packet_delay_metrics_type;

/* Argument to be passed with PS_IFACE_IOCTL_GET_UL_PACKET_DELAY_METRICS */
typedef struct
{
  uint8     num_bearers;
  ps_iface_ioctl_per_bearer_ul_packet_delay_metrics_type
            bearer_ul_packet_delay_metrics_arr[DS_SYS_MAX_BEARER];
} ps_iface_ioctl_ul_packet_delay_metrics_type;
...
/**
  @brief Argument to be passed with 
         PS_IFACE_IOCTL_WLAN_GET_COUNTRY_CODE.
*/
typedef struct
{
  uint8  country_code[PS_IFACE_IOCTL_MAX_COUNTRY_CODE_LEN];
} ps_iface_ioctl_country_code_type;
...
{
  uint8       num_pings;
  uint16      frequency;      /**< Frequency in ms */
  boolean     set_ka_guard;
  uint16      max_wait_time;  /**< time in ms, default 1000 ms */
  void      * user_data_ptr;
} ps_iface_ioctl_dpd_ping_config_type;
