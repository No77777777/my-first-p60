...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/api/public/ds_flow_control.h#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
/**< The DS_FLOW_UMTS_MASK is used to enable and disable flow for 
     PS data calls brought up on G/W/T/LTE.
     Enable Flow  -
                 1. When a PS call is brought up successfully
                 2. RAB reestablishment indication is received
                 3. Resume Indication is received 
                 4. If the data is purged from the water marks on G/W/TD,
                    This is not used for LTE data.
     Disable Flow -
                 1. When a PS call is in the process of tearing down
                 2. RAB reestablishment request is sent out
                 3. Suspend Indication is received and there is data in the watermark
                 4. If the data high water mark is hit.
     This is triggered even when there is no DATA in the WM's. 
*/ 
...
/**< The DS_FLOW_LTE_PDCP_MASK is used to enable and disable flow on LTE 
     Enable Flow  - When the data in the DS WM's goes below the LOW_THRESHOLD
     Disable Flow - When the data in the DS WM's exceeds the HIGH_THRESHOLD.
     This is not triggered when there is no DATA in the WM's.
*/  
#define DS_FLOW_LTE_PDCP_MASK     DS_FLOW_LL_MASK  /**< LTE PDCP mask. */
#define DS_FLOW_LTE_PDCP_HGH_MASK DS_FLOW_LTE_PDCP_MASK /* PDCP high watermark flow mask*/
#define DS_FLOW_RLC_MASK          DS_FLOW_LL_MASK  /**< RLC mask. */
#define DS_FLOW_SNDCP_MASK        DS_FLOW_LL_MASK  /**< SNDCP mask. */


/**< The DS_FLOW_UMTS_RMSM_MASK  is used to enable and disable flow for 
     DUN calls and in Test Loopback B(TLB) mode.
     Enable Flow  - When a DUN/TLB call is brought up successfully
     Disable Flow - When a DUN/TLB call is in the process of tearing down
     This is triggered even when there is no DATA in the WM's. 
*/  
...
/**< The DS_FLOW_GCSD_MASK is used to enable and disable flow for 
     CS data calls on GSM.
     Enable Flow  -
                 1. When a GSM CS call is brought up 
                 2. If the queued SIO data gow below the LOW_THRESHOLD
                    When data is present
     Disable Flow -
                 1. When a GSM CS call is in the process of tearing down
                 2. If the queued SIO data goes above the HIGH_THRESHOLD
                    When data is present
     This is triggered even when there is no DATA in the WM's. 
*/ 
...
/**< The DS_FLOW_WCDMACS_MASK is used to enable and disable flow for 
     CS data calls on WCDMA/TD.
     Enable Flow  -
                 1. When a WCDMA CS call is brought up successfully 
                 2. If the queued SIO data gow below the LOW_THRESHOLD
                    When data is present
     Disable Flow -
                 1. When a WCDMA CS call is torn down successfully
                 2.  If the queued SIO data goes above the HIGH_THRESHOLD
                    When data is present
     This is triggered even when there is no DATA in the WM's. 
*/ 
#define DS_FLOW_WCDMACS_MASK      0x00010000  /**< WCDMA CS data flow mask. */
#define DS_FLOW_SWITCH_MASK       0x00020000  /**< Phylink disable/enable depends
                                                                                        on iface enable/disable . */
...
#define DS_FLOW_PS_FLOW_TX_HGH_WM_MASK   DS_FLOW_PS_FLOW_TX_WM_MASK /**< Mask for transmit watermark(HIGH) 
                                                   associated with a PS flow. */

#define DS_FLOW_NO_SERVICE_MASK   0x20000000  /**< Mask used to control flow 
                                                   during service 
                                                   interruption. */
/**< The DS_FLOW_COEX_MASK is used only in data+data scenario
     Enable Flow  -
                 1. When MMS call ended on the non-DDS sub
                 2. No coex issue has been detected
     Disable Flow on the DDS sub -
                    When there is a MMS call on the non-DDS sub and a
                    coex issue has been detected
     This is triggered even when there is no DATA in the WM's. 
*/ 
#define DS_FLOW_COEX_MASK         0x40000000  /**< Mask for DSDA Co-ex issue */
#define DS_FLOW_PHYS_LINK_FLOW_MASK      0x80000000  /**< Phylink disable/enable depends                                                                                             on flow enable/disable  */

/* -------------------------------------------------------------------------------
  Here onwards All Mask values are more than 32 bit, They should be set into uint64 variables. 
 --------------------------------------------------------------------------------*/
#define DS_FLOW_PHYS_LINK_IFACE_MASK     0x100000000  /**< UINT64 MASK: Phylink disable/enable depends
                                                                                        on iface enable/disable . */
#define DS_FLOW_LTE_PDCP_DNE_MASK        0x200000000  /**< UINT64 MASK: PDCP DNE watermark flow mask */

#define DS_FLOW_PS_FLOW_TX_DNE_WM_MASK   0x400000000  /**< UINT64 MASK: Mask for transmit watermark(DNE) 
                                                   associated with a PS flow */

#define DS_FLOW_RRC_INACTIVE_MASK        0x0800000000 /**< UINT64 MASK: RRC goes inactive */

#define DS_FLOW_UNSUPPORTED_CFG_MASK     0x1000000000 /**< UINT64 MASK: Disable flow for unsupported config */
