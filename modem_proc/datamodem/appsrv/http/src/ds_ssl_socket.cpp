  ...
  ssl_version(DS_HTTP_SSL_VER_DEFAULT)
{
  ...
  cert_path   = request->ssl_config_info.ssl_cert_folder_path;
}
  ...
  ssl_version(DS_HTTP_SSL_VER_DEFAULT)
{
  ...
  cert_path   = active_request->ssl_config_info.ssl_cert_folder_path;
}
...
bool ds_ssl_socket::validate_ssl_config()
{
  // return ( convert_ssl_ver_from_config(active_request->ssl_config_info.ssl_version) == convert_ssl_ver_from_config(ssl_version) &&
  //          ASString(active_request->ssl_config_info.ssl_cert_folder_path) == cert_path );

  if( convert_ssl_ver_from_config(active_request->ssl_config_info.ssl_version) != convert_ssl_ver_from_config(ssl_version) ||
      ASString(active_request->ssl_config_info.ssl_cert_folder_path) != cert_path )
  {
    DATA_APPSRV_MSG_SPRINTF_5(
                              MSG_LEGACY_ERROR,
                              "ssl::validate_ssl_config - req %d: SSL version mismatch %d,%d or cert path \"%s\",\"%s\"",
                              active_request->request_id,
                              ssl_version,
                              active_request->ssl_config_info.ssl_version,
                              cert_path.c_str(),
                              active_request->ssl_config_info.ssl_cert_folder_path
                              );
    return false;
  }
  return true;
