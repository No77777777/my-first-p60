...
const char DS_HTTP_VERSION_PREFIX[]                     = "http/";
...
const uint32 DS_HTTP_VER_PREFIX_STR_LEN        = 5;
...
bool ds_http_response::decode_header()
{
  ...
  ASString       getter;
  char           ver_prefix[DS_HTTP_VER_PREFIX_STR_LEN + 1] = {0};
  uint32         ver_major = 0;
  uint32         ver_minor = 0;
  receive_status = DS_HTTP_RECEIVE_COMPLETE;
  ...
  iss.get_next(getter, ' ');
  if((3 != sscanf(getter.c_str(), "%5s%lu%*[.]%lu", ver_prefix, &ver_major, &ver_minor)) || \
     (0 != strncasecmp(ver_prefix, DS_HTTP_VERSION_PREFIX, DS_HTTP_VER_PREFIX_STR_LEN)) || \
     (1 != ver_major))
  {
    DATA_APPSRV_MSG_SPRINTF_1(MSG_LEGACY_ERROR, "decode_header - Not http 1.X! HTTP version: %s", getter.c_str());
    return false;
  }
