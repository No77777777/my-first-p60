...
struct ds_http_redirect_cache
{
  ...
  explicit ds_http_redirect_cache(ASIStream& istream);
  ds_http_redirect_cache(
...
: ip_addr_type(DSS_AF_INET), ip_addr(0)
{ }
...
void update_permanent_uri_cache()
{
  ...
  ASOFStream ofstream(DS_HTTP_REDIRECT_CACHE_TEMP_PATH, MCFG_FS_O_CREAT | MCFG_FS_O_TRUNC | MCFG_FS_O_WRONLY, MCFG_FS_DEFFILEMODE);
  ...
  for(redirect_iterator redirect_it = redirect_cache_list.begin(); redirect_cache_list.end() != redirect_it; ++redirect_it)
  {
    ...
    if( ofstream.fail() )
    {
      ...
      ofstream.close_file();
      mcfg_fs_delete(DS_HTTP_REDIRECT_CACHE_TEMP_PATH, MCFG_FS_TYPE_EFS, MCFG_FS_SUBID_NONE); // Erase temp file
      return;
    }
  }
  ...
  if( 0 != mcfg_fs_rename(DS_HTTP_REDIRECT_CACHE_TEMP_PATH, DS_HTTP_REDIRECT_CACHE_PATH, MCFG_FS_TYPE_EFS, MCFG_FS_SUBID_NONE) )
  {
    DATA_APPSRV_MSG0(MSG_LEGACY_ERROR, "update_permanent_uri_cache - Failed to overwrite original file with temp file");
    mcfg_fs_delete(DS_HTTP_REDIRECT_CACHE_TEMP_PATH, MCFG_FS_TYPE_EFS, MCFG_FS_SUBID_NONE); // Erase the temp file if rename fails
    return;
  }
}
...
void append_permanent_uri_cache(const ds_http_redirect_cache* uri_cache)
{
  ASOFStream ofstream(DS_HTTP_REDIRECT_CACHE_PATH, MCFG_FS_O_CREAT | MCFG_FS_O_APPEND | MCFG_FS_O_WRONLY, MCFG_FS_DEFFILEMODE);
...
void ds_http_init_uri()
{
  ...
  ASIFStream ifstream(DS_HTTP_REDIRECT_CACHE_PATH, MCFG_FS_O_RDONLY, 0);
  bool changed = false;
  ...
  do
  {
    ...
    else if( ifstream.fail() )
    {
      ...
      // ifstream.close();
      // mcfg_fs_delete(DS_HTTP_REDIRECT_CACHE_TEMP_PATH, MCFG_FS_TYPE_EFS, MCFG_FS_SUBID_NONE); // Erase corrupt file
      return;
    }
