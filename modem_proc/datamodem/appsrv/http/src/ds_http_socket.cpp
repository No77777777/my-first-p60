...
void ds_http_socket::resolve_ssl_suspension(const bool cont)
{
  // do nothing
}
...
bool ds_http_socket::validate_ssl_config()
{
  return true;
}
...
void ds_http_socket::http_create_socket()
{
  ...
  //create socket
  sockfd = dss_socket(dsnet_handle_id, ip_addr_info.ip_addr_type, DSS_SOCK_STREAM, PS_IPPROTO_TCP, &err);
  ...
  if(0 != active_request->reserved_src_port)
  {
    ...
    if (ip_addr_info.ip_addr_type == DSS_AF_INET)
    {
...
void ds_http_socket::http_connect()
{
  ...
  if(DSS_AF_INET == ip_addr_info.ip_addr_type)
  {
  ...
  else // if(DSS_AF_INET6 == ip_addr_info.ip_addr_type)
  {
...
void ds_http_socket::http_connected()
{
  ...
  if(persistent && 0 != timeout_length)
  {
    ...
    else
    {
      ...
      // If SSL socket, validate SSL config matches previous request.
      // Regular socket will always return true.
      if( validate_ssl_config() )
      {
      socket_status = SOCKET_STATUS_SEND;
      active_request->reset_send_status();
      http_send();
      ...
      {
        DATA_APPSRV_MSG0(MSG_LEGACY_MED, "http_connected - SSL config mismatch, reestablishing SSL connection");
        socket_status = SOCKET_STATUS_CLOSE;
        http_close();
      }
    }
