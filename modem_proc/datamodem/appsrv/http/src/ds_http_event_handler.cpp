                       ...
                       )
{
  ...
  if(PS_SYS_MAX_AVAIL_SYS < sys_info->num_avail_sys)
  {
    ASSERT(0);
    return;
  }
  ...
  for(uint32 i = 0; i < sys_info->num_avail_sys; ++i)
  {
    ...
    DATA_APPSRV_MSG5(MSG_LEGACY_HIGH, "ds_http_ps_sys_cb - [%d / %d] pref:%d rat:%d so:%d", i, sys_info->num_avail_sys, preferred_network, rat, so);
