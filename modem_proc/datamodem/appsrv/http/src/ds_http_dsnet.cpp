...
--------    ---    ----------------------------------------------------------
11/10/16    ml     PDN override for home / roaming
...
static uint32 ds_http_query_pdp_type(ds_profile_hndl_type profile_handle, ds_profile_3gpp_param_enum_type query_param);
...
bool ds_http_dsnet::check_dsnet_handle(sint15 dsnet_handle) const
{
  return dsnet_handle == dsnet_handle_id;
}


bool ds_http_dsnet::check_dns_handle(dss_dns_session_mgr_handle_type dns_handle) const
{
  return dns_handle == dns_session_handle;
}
...
{
  DATA_APPSRV_MSG2(MSG_LEGACY_MED, "add_request - rid:%d status:%d", request->request_id, dsnet_status);
  switch(dsnet_status)
  {
...
void ds_http_dsnet::retry_dns_dual_ip()
{
  if(!is_dual_ip)
    return;
  ...
  if( DSS_ERROR == dss_last_netdownreason(dsnet_handle_id, &reason, &dss_errno) )
    return;

  switch(reason)
  {
    // IPv4 failures
    case DSS_NET_DOWN_REASON_ERR_PDN_IPV4_CALL_DISALLOWED: // fallthrough
    case DSS_NET_DOWN_REASON_IP_V6_ONLY_ALLOWED:           // fallthrough
    // IPv6 failures
    case DSS_NET_DOWN_REASON_IP_V4_ONLY_ALLOWED:           // fallthrough
    case DSS_NET_DOWN_REASON_ERR_PDN_IPV6_CALL_DISALLOWED: // fallthrough
    case DSS_NET_DOWN_REASON_IPV6_ERR_PREFIX_UNAVAILABLE:  // fallthrough
    case DSS_NET_DOWN_REASON_IPV6_ERR_HRPD_IPV6_DISABLED:  // fallthrough
    case DSS_NET_DOWN_REASON_IPV6_DISABLED:                // fallthrough

    // IP version mismatch
    case DSS_NET_DOWN_REASON_IP_VERSION_MISMATCH:
      ds_http_dual_ip_retry(dsnet_wait_queue, iface_info, DS_HTTP_ERROR_NETWORK_DOWN);
      break;

    // TODO: include these errors?
    // case DSS_NET_DOWN_REASON_ERR_PDN_IPV4_CALL_THROTTLED:
    // case DSS_NET_DOWN_REASON_ERR_PDN_IPV6_CALL_THROTTLED:
...
bool ds_http_dsnet::ds_http_init_dsnet()
{
  ...
  if(DSS_AF_ANY == ip_family)
  {
    ip_family  = iface_info.pref_ip_type;
    is_dual_ip = true;
  }
...
bool ds_http_dsnet::ds_http_open_dns_session()
{
  ...
  {
    DATA_APPSRV_MSG1(MSG_LEGACY_ERROR, "ds_http_open_dns_session - Failed to set dns net policy config, error %d", err);
    ds_http_close_dns_session();
    return false;
  }
  DATA_APPSRV_MSG1(MSG_LEGACY_MED, "ds_http_open_dns_session - Created DNS Session %d", dns_session_handle);
  return true;
}
...
void ds_http_dsnet::ds_http_create_dns_query(ds_http_request* request)
{
  ...
  if(NULL == active_query)
  {
    ...
    if(!dns_query->start_query(dns_session_handle, ip_family))
    {
      ...
      if(is_dual_ip)
        dns_query->retry_dns_dual_ip(iface_info);
                                     ...
                                     )
{
  ...
  do
  {
    ...
    for(uint16 i = 0; i < num_records; i++)
    {
      ...
      {
        /* DNS IP type needs to match dsnet IP type */
      ...
      if (result_buf_ptr[i].ai_family == DSS_AF_INET)
      {
        ...
        addr_info.ip_addr_type         = DSS_AF_INET;
        found                          = true;
        break;
      }
      else if(result_buf_ptr[i].ai_family == DSS_AF_INET6)
      {
        ...
        addr_info.ip_addr_type          = DSS_AF_INET6;
        found                           = true;
        break;
      }
    }
  ...
  if(!found && is_dual_ip)
  {
    active_query->retry_dns_dual_ip(iface_info);
  }
...
uint32 ds_http_query_pdp_type(ds_profile_hndl_type profile_handle, ds_profile_3gpp_param_enum_type query_param)
{
  ds_profile_info_type    profile_info;
  ds_profile_status_etype res     = DS_PROFILE_REG_RESULT_FAIL;
  uint32                  ret_val = 0;

  if(query_param != DS_PROFILE_3GPP_PROFILE_PARAM_PDP_CONTEXT_PDP_TYPE &&
     query_param != DS_PROFILE_3GPP_PROFILE_PARAM_LTE_ROAMING_PDP_TYPE &&
     query_param != DS_PROFILE_3GPP_PROFILE_PARAM_OVERRIDE_HOME_PDP_TYPE)
  {
    DATA_APPSRV_MSG1(MSG_LEGACY_ERROR, "ds_http_query_pdp_type - Invalid PDP query param %d", query_param);
    return 0xff; // PDP MAX value
  }

  memset(&profile_info, 0, sizeof(ds_profile_info_type));
  profile_info.buf = &ret_val;
  profile_info.len = sizeof(uint32);

  res = ds_profile_get_param (profile_handle,
                              (ds_profile_identifier_type)query_param,
                              &profile_info);

  if ( DS_PROFILE_REG_RESULT_SUCCESS != res || profile_info.len > sizeof(uint32) )
  {
    DATA_APPSRV_MSG4(
                     MSG_LEGACY_ERROR,
                     "ds_http_query_pdp_type - Get profile param %d failed with err:%d or invalid param len %d : %d",
                     query_param,
                     res,
                     sizeof(uint32),
                     profile_info.len
  ...
  DATA_APPSRV_MSG2(MSG_LEGACY_MED, "ds_http_query_pdp_type - PDP info for %d, result:%d", query_param, ret_val);
  return ret_val;
}
...
int ds_http_resolve_ip_type(const ds_http_dsnet_iface_info& iface_info)
{
  ...
  int                     ret_val        = iface_info.pref_ip_type;

  /* For WLAN LB, use IPv4v6 */
  if (DS_HTTP_IFACE_WLAN_LB == iface_info.iface_type)
    return DSS_AF_ANY;
  ...
  if ( DS_PROFILE_REG_RESULT_SUCCESS != res || profile_handle == NULL )
  {
  ...
  do
  {
    bool pdp_override_set = true;

    /* Query ANDSF for roaming status if iface is 3GPP */
    if( DS_HTTP_IFACE_3GPP == iface_info.iface_type &&
        ds_andsf_get_roaming_status( (ds_sys_subscription_enum_type) iface_info.get_subs_id() ) )
    {
      /* Check roaming PDP type */
      switch( (ds_profile_3gpp_roaming_pdp_type_enum_type) ds_http_query_pdp_type(
                                                                                  profile_handle,
                                                                                  DS_PROFILE_3GPP_PROFILE_PARAM_LTE_ROAMING_PDP_TYPE
                                                                                  ) )
      {
        case DS_PROFILE_3GPP_ROAMING_PDP_IPV4V6:
          ret_val = DSS_AF_ANY;
          break;

        case DS_PROFILE_3GPP_ROAMING_PDP_IP:
          ret_val = DSS_AF_INET;
          break;

        case DS_PROFILE_3GPP_ROAMING_PDP_IPV6:
          ret_val = DSS_AF_INET6;
          break;

        case DS_PROFILE_3GPP_ROAMING_PDP_MAX:
        default:
          pdp_override_set = false;
          break;
      }
    }
    else
    {
      /* Check home override PDP type */
      switch( (ds_profile_3gpp_override_home_pdp_type_enum_type) ds_http_query_pdp_type(
                                                                                        profile_handle,
                                                                                        DS_PROFILE_3GPP_PROFILE_PARAM_OVERRIDE_HOME_PDP_TYPE
                                                                                        ) )
      {
        case DS_PROFILE_3GPP_OVERRIDE_HOME_PDP_IPV4V6:
          ret_val = DSS_AF_ANY;
          break;

        case DS_PROFILE_3GPP_OVERRIDE_HOME_PDP_IP:
          ret_val = DSS_AF_INET;
          break;

        case DS_PROFILE_3GPP_OVERRIDE_HOME_PDP_IPV6:
          ret_val = DSS_AF_INET6;
          break;

        case DS_PROFILE_3GPP_OVERRIDE_HOME_PDP_MAX:
        default:
          pdp_override_set = false;
          break;
      }
    ...
    // No override set, continue with regular IP type check
    switch( (ds_profile_3gpp_pdp_type_enum_type) ds_http_query_pdp_type(
                                                                        profile_handle,
