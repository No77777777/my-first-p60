...
static ds_http_dsnet* ds_http_get_secondary_dsnet_by_session_id(const uint32 session_id);
...
static void ds_http_resolve_auth_helper(ds_http_dsnet* http_dsnet, uint32 request_id, ds_http_auth_info_s_type* auth_info);
static void ds_http_resolve_ssl_suspension_helper(ds_http_dsnet* http_dsnet, uint32 request_id, bool ssl_cont);
static void ds_http_append_chunk_to_request_helper(
...
void ds_http_session_cleanup_handler(uint32* data_ptr)
{
  ...
  ds_http_dsnet* http_dsnet_secondary = session_info->get_dsnet_secondary();
  ds_appsrv_free(session_info);

  if( ( NULL != http_dsnet && http_dsnet->dsnet_inactive() ) ||
      ( NULL != http_dsnet_secondary && http_dsnet_secondary->dsnet_inactive() ) )
  {
    ds_http_event_remove_dsnet();
  }
...
ds_http_dsnet* ds_http_get_secondary_dsnet_by_session_id(const uint32 session_id)
{
  ASLockGuard lock(session_crit_sect);
  ds_http_session_info* session_info = get_session_info(session_id);
  if(NULL != session_info)
    return session_info->get_dsnet_secondary();
...
ds_http_dsnet* ds_http_get_dsnet_by_dsnet_handle(const sint15 dsnet_handle_id)
{
  for(dsnet_iterator it = dsnet_list.begin(); it != dsnet_list.end(); ++it)
  {
    if( (*it)->check_dsnet_handle(dsnet_handle_id) )
    {
      return *it;
    }
  }
...
void ds_http_remove_request(const uint32 session_id, const uint32 request_id)
{
  ...
  http_dsnet = ds_http_get_secondary_dsnet_by_session_id(session_id);
  if(NULL != http_dsnet)
  {
    http_dsnet->remove_request(request_id);
  }
...
{
  ASLockGuard dsnet_lock(dsnet_crit_sect);
  ds_http_resolve_auth_helper( ds_http_get_dsnet_by_session_id(session_id), request_id, auth_info );
  ds_http_resolve_auth_helper( ds_http_get_secondary_dsnet_by_session_id(session_id), request_id, auth_info );
}

void ds_http_resolve_auth_helper(ds_http_dsnet* http_dsnet, uint32 request_id, ds_http_auth_info_s_type* auth_info)
{
  if(NULL == http_dsnet)
    return;
...
{
  ASLockGuard dsnet_lock(dsnet_crit_sect);
  ds_http_resolve_ssl_suspension_helper( ds_http_get_dsnet_by_session_id(session_id), request_id, ssl_cont );
  ds_http_resolve_ssl_suspension_helper( ds_http_get_secondary_dsnet_by_session_id(session_id), request_id, ssl_cont );
}

void ds_http_resolve_ssl_suspension_helper(ds_http_dsnet* http_dsnet, uint32 request_id, bool ssl_cont)
{
  if(NULL == http_dsnet)
    return;
...
{
  ASLockGuard dsnet_lock(dsnet_crit_sect);
  ds_http_append_chunk_to_request_helper( ds_http_get_dsnet_by_session_id(session_id), request_id, chunked_content );
  ds_http_append_chunk_to_request_helper( ds_http_get_secondary_dsnet_by_session_id(session_id), request_id, chunked_content );
}

void ds_http_append_chunk_to_request_helper(
                                            ds_http_dsnet*                 http_dsnet,
                                            const uint32                   request_id,
                                            const ds_http_chunked_content& chunked_content
                                            )
                              ...
                              )
{
  ...
  for(dsnet_iterator it = dsnet_list.begin(); it != dsnet_list.end(); ++it)
  {
    if( (*it)->check_dns_handle(dns_session_handle) )
    {
      http_dsnet = *it;
      break;
    }
...
uint32 ds_http_get_last_netdownreason(uint32 session_id, dss_net_down_reason_type *reason)
{
  ...
  if( NULL == http_dsnet || DSS_ERROR == dss_last_netdownreason(http_dsnet->get_dsnet_handle_id(), reason, &dss_errno) )
  {
    return DS_HTTP_ERROR;
  }
...
bool ds_http_dual_ip_retry(
                           ASList<ds_http_request*>& request_list,
                           ds_http_dsnet_iface_info  iface_info,
                           sint15                    errval
                           )
{
  DATA_APPSRV_MSG0(MSG_LEGACY_HIGH, "ds_http_dual_ip_retry - DEBUG: Starting dual ip retry");
  if( request_list.empty() )
    return false;

  if(iface_info.pref_ip_type == DSS_AF_INET6)
    iface_info.pref_ip_type = DSS_AF_INET;
  else
    iface_info.pref_ip_type = DSS_AF_INET6;

  // No need to lock dsnet_crit_sect since call stack should have already locked it
  // ASLockGuard dsnet_lock(dsnet_crit_sect);
  ...
  if(NULL == http_dsnet)
  {
    ...
    if(NULL == http_dsnet)
    {
      DATA_APPSRV_MSG0(MSG_LEGACY_ERROR, "ds_http_dual_ip_retry - Failed to allocate memory for http dsnet");
      return false;
    }
    else if(!dsnet_list.push_back(http_dsnet))
    {
      ds_appsrv_free(http_dsnet);
      DATA_APPSRV_MSG0(MSG_LEGACY_ERROR, "ds_http_dual_ip_retry - Failed to allocate memory for dsnet list");
      return false;
    }

    DATA_APPSRV_MSG6(
                     MSG_LEGACY_HIGH,
                     "ds_http_dual_ip_retry - [Input iface:%d profile:%d subs:%d] [Session iface:%d profile:%d subs:%d]",
                     iface_info.iface_type,
                     iface_info.profile_id,
                     iface_info.subs_id,
                     http_dsnet->iface_info.iface_type,
  ...
  {
    ds_http_request* request = request_list.front();
    request_list.pop_front();

    // Check if already retried
    if( request->get_dual_ip_retry_status() )
    {
      request->notify_error(errval);
      ds_appsrv_free(request);
      continue;
    }

    request->set_dual_ip_retry_status(true);
    http_dsnet->add_request(request);

    { // Scoping to guarantee session_crit_sect to be released
      ASLockGuard lock(session_crit_sect);
      ds_http_session_info* session_info = get_session_info(request->session_id);
      if(NULL == session_info)
        continue;

      if(NULL == session_info->get_dsnet_secondary())
      {
        session_info->set_dsnet_secondary(http_dsnet);
      }
    }
  }
