...
: iface_type(DS_HTTP_IFACE_MAX), profile_id(0), subs_id(DS_HTTP_DEFAULT_SUBS), pref_ip_type(DSS_AF_INET)
{
  if(NULL == iface)
    return;
  ...
  switch(iface->pref_ip_type)
  {
    case DS_HTTP_PREF_IPV6:
      pref_ip_type = DSS_AF_INET6;
      break;

    case DS_HTTP_PREF_IPV4: // fallthrough
    default:
      pref_ip_type = DSS_AF_INET;
      break;
  }
}
...
{
  if(subs_id != rhs.subs_id || iface_type != rhs.iface_type || pref_ip_type != rhs.pref_ip_type)
    return false;
