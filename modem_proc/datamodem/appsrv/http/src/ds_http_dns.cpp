...
: dns_query_handle(DSS_ERROR), is_dual_ip(false)
{
  request_queue.push_back(request);
}


ds_http_dns_query::ds_http_dns_query(ds_http_request* request, bool dual_ip)
: dns_query_handle(DSS_ERROR), is_dual_ip(dual_ip)
{
  request_queue.push_back(request);
}
...
bool ds_http_dns_query::retry_dns_dual_ip(const ds_http_dsnet_iface_info& iface_info)
{
  return ds_http_dual_ip_retry(request_queue, iface_info, DS_HTTP_ERROR_DNS);
}
