...
bool ASXML::XMLElementCondition::validateElement(const XMLElement* elem, bool& flag) const
{
  ...
  {
    if(is_required)
      XMLDecoderException::get_instance().raise(flag, ERR_MSG_INVALID_TAG, element_name);
  ...
  else
  {
    ...
    {
      XMLDecoderException::get_instance().raise(flag, ERR_MSG_CHILD_LOW, element_name);
      return false;
    }

    if(check_max_children && max_expected_children < elem->children_size())
    {
      XMLDecoderException::get_instance().raise(flag, ERR_MSG_CHILD_HIGH, element_name);
      return false;
    }

    if(expected_num_attributes != elem->attributes_size())
    {
      XMLDecoderException::get_instance().raise(flag, ERR_MSG_ATTR_COUNT, element_name);
      return false;
    }
    ...
    {
      XMLDecoderException::get_instance().raise(flag, ERR_MSG_EMPTY_CONTENT, element_name);
      return false;
    }
  }
