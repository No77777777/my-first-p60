...
: is_api_override(false), data_roaming_option(false), flow_type(type)
{
  ...
  switch(flow_type)
  {
    case FD_SERVICE_BASED:
    {
      XMLElementCondition(NULL, true, true, 3, 6).validateElement(elem, flag);
  ...
  element_validator.addRule(XMLElementCondition(ELEM_NAME_DATA_ROAMING_OPTION, false, true));
  parse_fcn_ptrs.push_back(&ANDSF_FlowDistribution::parse_data_roaming_option);
...
void ASXML::ANDSF_FlowDistribution::parse_data_roaming_option(const XMLElement* elem, bool& flag)
{
  data_roaming_option = str_to_bool(elem->get_text());
}
