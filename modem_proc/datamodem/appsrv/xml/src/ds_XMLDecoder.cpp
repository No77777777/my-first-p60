...
: root(NULL), flag(false)
{
  init( ASString(xml_content, len) );
}


void ASXML::XMLDecoder::init(const ASString& xml_content)
{
  decode(xml_content);
...
void ASXML::XMLDecoder::decode(const ASString& raw_bytes)
{
  uint32   skip_count = 0;
  ASString token;

  DATA_APPSRV_MSG1(MSG_LEGACY_MED, "XMLDecoder::XMLDecoder - parsing xml file of size %d", raw_bytes.size());
    ...
    else
    {
      ...
      else
      {
        if(NULL == cur_element)
        {
          ...
          XMLDecoderException::get_instance().raise(flag, ERR_MSG_INVALID_STATE, "NULL cur_element");
          return;
        }
