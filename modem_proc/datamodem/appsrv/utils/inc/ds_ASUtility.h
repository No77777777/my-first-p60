...
const uint16 DS_UTILS_UINT32_HEX_LEN = 8;

bool mac_addr_to_bin(const ASString& mac_addr_str, uint8 mac_addr[DS_APPSRV_UTIL_WLAN_MAC_ADDR_LEN]);

bool get_file_content(const char* file_path, ASBuffer& buffer, mcfg_fs_sub_id_e mcfg_subs_id=MCFG_FS_SUBID_NONE);
bool copy_file(const char* orig_file_path, const char* copy_file_path, mcfg_fs_sub_id_e mcfg_subs_id=MCFG_FS_SUBID_NONE);
