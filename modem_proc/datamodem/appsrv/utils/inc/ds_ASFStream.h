...
public:
  ASFileBuffer();
  ASFileBuffer(const char* path, int oflag, int mode, mcfg_fs_sub_id_e mcfg_subs_id);
  ~ASFileBuffer();
  ...
  bool open_file(const char* path, int oflag, int mode, mcfg_fs_sub_id_e mcfg_subs_id);
  void close_file();
  bool is_open();
...
public:
  ...
  ASIFStream(
             const char* path,
             int oflag,
             int mode                      = MCFG_FS_DEFFILEMODE,
             mcfg_fs_sub_id_e mcfg_subs_id = MCFG_FS_SUBID_NONE
             );
  ~ASIFStream();
