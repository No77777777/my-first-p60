...
ASFileBuffer::ASFileBuffer(const char* path, int oflag, int mode, mcfg_fs_sub_id_e mcfg_subs_id)
: filedes(-1), file_size(0)
{
  open_file(path, oflag, mode, mcfg_subs_id);
}
...
bool ASFileBuffer::open_file(const char* path, int oflag, int mode, mcfg_fs_sub_id_e mcfg_subs_id)
{
  // A file is already open
  if(-1 != filedes)
    return true;

  filedes = mcfg_fopen(path, oflag, mode, MCFG_FS_TYPE_EFS, mcfg_subs_id);
  ...
  if(0 != mcfg_fstat(filedes, &file_stat, MCFG_FS_TYPE_EFS))
  {
    return false;
  }
...
void ASFileBuffer::close_file()
{
  if(-1 != filedes)
  {
    mcfg_fclose(filedes, MCFG_FS_TYPE_EFS);
  }
...
bool ASFileBuffer::write(const char* buf, uint32 len)
{
  ...
  {
    int32 written = mcfg_fwrite(filedes, (void*)(buf + total_write), len - total_write, MCFG_FS_TYPE_EFS);
    if(0 >= written)
    {
      return false;
    }
...
bool ASFileBuffer::read_file()
{
  ...
  {
    // read file
    int32 read = mcfg_fread(filedes, buffer, buffer_size, MCFG_FS_TYPE_EFS);
...
ASIFStream::ASIFStream(const char* path, int oflag, int mode, mcfg_fs_sub_id_e mcfg_subs_id)
{
  init(&ifstream);
  open_file(path, oflag, mode, mcfg_subs_id);
}
...
void ASIFStream::open_file(const char* path, int oflag, int mode, mcfg_fs_sub_id_e mcfg_subs_id)
{
  if( ifstream.open_file(path, oflag, mode, mcfg_subs_id) )
  {
    clear_state();
  }
  else
...
ASOFStream::ASOFStream(const char* path, int oflag, int mode, mcfg_fs_sub_id_e mcfg_subs_id)
{
  init(&ofstream);
  open_file(path, oflag, mode, mcfg_subs_id);
}
...
void ASOFStream::open_file(const char* path, int oflag, int mode, mcfg_fs_sub_id_e mcfg_subs_id)
{
  if( ofstream.open_file(path, oflag, mode, mcfg_subs_id) )
  {
    clear_state();
  }
  else
