...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp/rmsm/src/dsumts_rmsmi.c#3 $ 
$DateTime: 2021/02/08 22:33:06 $ $Author: pwbldsvc $

when        who    what, where, why
--------    ---    ----------------------------------------------------------
06/14/17    skc    Resolved escape sequence issue in CDMA mode.
...
#define DS_3GPP_RMSMI_IFACE_EV_COUNT 5

static ps_iface_event_enum_type iface_ev_p[DS_3GPP_RMSMI_IFACE_EV_COUNT] = 
       {IFACE_UP_EV,IFACE_DOWN_EV, IFACE_ACTIVE_OUT_OF_USE_EV,
         IFACE_FLOW_ENABLED_EV,IFACE_FLOW_DISABLED_EV};

#define DS_3GPP_RMSMI_PHYS_LINK_EV_COUNT 2

static ps_iface_event_enum_type phys_link_ev_p[DS_3GPP_RMSMI_PHYS_LINK_EV_COUNT] = 
       {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
   ...
   DS3GPP_ASSERT(rmsmi_info_ptr != NULL);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
        ...
        sm_inst < DSUMTS_RMSM_MAX_SM_INSTANCES; sm_inst++)
  {
    ...
    DS3GPP_ASSERT(sm_info_ptr != NULL);
...
)
{
  ...
  {
    DS_3GPP_MSG2_LOW("PDP profile  %d access error:%d fail call",
                        rmsmi_info_ptr->profile_id, pdp_result);
    *result_ptr = -1;
  }
  ...
  {
    DS_3GPP_MSG1_LOW("Context Specified %d is secondary. Fail call setup",
                       rmsmi_info_ptr->profile_id);
    *result_ptr = -1;
  }
  DS_3GPP_MSG2_MED("PDP profile  %d access error:%d",
                        rmsmi_info_ptr->profile_id, pdp_result);
...
#ifdef FEATURE_QSH_MDUMP
/*===========================================================================
FUNCTION dsumts_rmsmi_qsh_mdump_collect
...
===========================================================================*/ 
QSH_MDUMP_FN_ATTR void dsumts_rmsmi_qsh_mdump_collect()
{
  qsh_mdump_collect_high(&dsumts_rmsmi_info, sizeof(dsumts_rmsmi_info_type));
}
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
       ...
       rm_dev_instance++)
  {
    ...
    rex_def_timer_ex( &(rmsmi_info_ptr->stop_ppp_delay_timer),
                      ds_3gpp_rmsm_ip_common_stop_ppp_delay_timer_cb , 
                      (unsigned long )rmsmi_info_ptr );

    rex_def_timer_ex( &(rmsmi_info_ptr->ppp_up_delay_timer),
    ...
    {
      DS_3GPP_MSG0_LOW("Couldn't get PPP event handler");
    }
          ...
          sm_instance++, i++)
    {
      ...
      if (sm_info_ptr == NULL)
      {
        DS_3GPP_MSG2_LOW( "Could not allocate memory for sm instance %d, call instance %d", 
                            sm_instance, 
                            rm_dev_instance);
        DS3GPP_ASSERT(0);
	return FALSE;
      ...
      iface_instance = ps_iface_create(&sm_info_ptr->sio_iface, 
                                       ...
                                       1);
      DS3GPP_ASSERT(iface_instance >= 0);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_3GPP_MSG1_LOW("HANDLE_ATD_ON_RM: failed, RM is locked already,"
  ...
  {
    DS_3GPP_MSG1_LOW("HANDLE_ATD_ON_RM: success, RM lock acquired, Rm Device [%d]", 
             rm_dev_instance);
  }
#endif /* FEATURE_DATA_TE_MT_PDP */

  DS_3GPP_MSG2_HIGH("HANDLE_ATD_ON_RM: Rm Device [%d], Rcvd dial string for  subs id %d",
                     rm_dev_instance, ps_subs_id);
  ...
  {
    DS_3GPP_ERROR_LOG_0_EX(ps_subs_id, "Unable to allocate SIOLIB port");
    (void)ds3g_siolib_ex_set_cd_state (DS3G_SIOLIB_REMOTE_CARRIER_OFF,port_id);
    dsat_send_result_code_ex( DSAT_NO_CARRIER,port_id );
#ifdef FEATURE_DATA_TE_MT_PDP
    dsumts_rmsm_unlock_rm();
#endif
  ...
  else
  {
    ...
    {
      DS_3GPP_MSG0_LOW("Couldn't get default profile, failing call");
  ...
  DS_3GPP_MSG2_HIGH("Profile:%d valid & primary: using it, PDP type %d",
            rmsmi_info_ptr->profile_id, pdp_type);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_3GPP_MSG0_LOW("RMSMI_PROFILE_MATCH: invalid input params, returning!");
    return FALSE;
  }
  ...
  for ( i = 0; i < tmp_num_of_profiles; i++ ) 
  {
    ...
    if ( ( result == DS_UMTS_PDP_SUCCESS ) && ( valid_flag ) ) 
    {
      ...
      {
        DS_3GPP_MSG1_LOW("RMSMI_PROFILE_MATCH: PDP profile access error [%d] fail call",
                 result);
        return FALSE;
      }
#ifdef FEATURE_SECONDARY_PDP
      ...
      {
        DS_3GPP_MSG0_LOW("RMSMI_PROFILE_MATCH: Context Specified is secondary. Fail call setup");
        return FALSE;
      }
#endif
      else
      {
        ...
        {
          DS_3GPP_MSG0_LOW("RMSMI_PROFILE_MATCH: Changing PDP_TYPE as IPV4 as call with IPV4V6 is not possible");
        ...
        if (( 0 == strlen( (char *)apn ) )|| 
               ( 0 == strncasecmp( (char *)encoded_apn_name, (char *)incom_apn_name,
    ...
    {
      DS_3GPP_MSG2_LOW("MT_RING_HDLR: skipping to next profile, result [%d], valid_flag [%d]",
               result, valid_flag);
      continue;
    }
...
{ 
  ...
  {
    DS_3GPP_ERROR_LOG_0("HANDLE_ANS_ON_RM: Unable to allocate SIOLIB port");
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if ( mt_info_ptr == NULL ) 
  {
    DS_3GPP_MSG0_LOW("RMSMI_MT_RING_TE: invalid input params, returning!");
    return FALSE;
  }
  ...
  {
    DS_3GPP_MSG3_LOW("RMSMI_MT_RING_TE: no access ctrl is active, RING_TE/AUTO-ANS!", 
           rm_dev_instance, mt_info_ptr->apn_name[0], mt_info_ptr->pdp_type);
    goto RING_TE;
  }
  ...
  for (profile_id = 1; profile_id <= DS_UMTS_MAX_PDP_PROFILE_NUM; profile_id++) 
  {
    ...
    if( result != DS_UMTS_PDP_SUCCESS ) 
    {
      DS_3GPP_MSG1_LOW("RMSMI_MT_RING_TE: PDP profile access error [%d], fail call",
             result);
      return FALSE;
    }
    ...
    default:
      DS_3GPP_MSG1_LOW("RMSMI_MT_RING_TE: unknown access control method [%d]",
                access_ctrl_flag);
      DS3GPP_ASSERT(0);
      return FALSE;
    }/* switch() */
  ...
  if (!perm_list_match) 
  {
    switch( active_access_ctrl )
    {
      case DSAT_PDP_ACCESS_CONTROL_REJECT:
        if ( rej_list_sz == 0 )
        {
          DS_3GPP_MSG0_LOW("RMSMI_MT_RING_TE: ACCESS_CTRL_REJ, rej_list_sz = 0, RING TE");
        }
        break; /* DSAT_PDP_ACCESS_CONTROL_REJECT */
      ...
      case DSAT_PDP_ACCESS_CONTROL_PERMISSION:
        ...
        {
          DS_3GPP_MSG0_LOW("MT_HANDLE_RING_IND: ACCESS_CTRL_PERM, perm_list_sz <> 0, no match, RING TE!");
        }
        break; /* DSAT_PDP_ACCESS_CONTROL_PERMISSION */
      ...
      default:
        DS_3GPP_ERROR_LOG_1("MT_RING_HDLR: unknown access control method [d]",
                  access_ctrl_flag);
        DS3GPP_ASSERT(0);
        return FALSE;
    }/* switch() */
...
RING_TE:
  /*-----------------------------------------------------------------------
    ...
    pdp-ip or pdp-ppp
  -------------------------------------------------------------------------*/
  DS_3GPP_MSG2_MED ("RMSMI_MT_RING_TE: incom_ds_pdp_type %d!", incom_ds_pdp_type);  
  if(incom_ds_pdp_type == DS_UMTS_PDP_PPP )
  {
    rmsmi_info_ptr->sm_type = DSUMTS_RMSM_SM_TYPE_PDP_PPP ;
    rmsmi_info_ptr->mt_index = mt_info_ptr->mt_index;
    DS_3GPP_MSG0_LOW("RMSMI_MT_RING_TE: PDP-PPP type call/State machine");
  }
            ...
            (incom_ds_pdp_type == DS_UMTS_PDP_IPV4V6))
  {
    rmsmi_info_ptr->sm_type = DSUMTS_RMSM_SM_TYPE_PDP_IP ;
    rmsmi_info_ptr->mt_index = mt_info_ptr->mt_index;
    DS_3GPP_MSG0_LOW("RMSMI_MT_RING_TE: PDP-IP type call/State machine");
  }
  else
  {
    DS_3GPP_MSG2_LOW("RMSMI_MT_RING_TE: Invalid PDP type [%d] incom_ds_pdp_type %d!",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_3GPP_MSG1_HIGH( "Call aborted for port %d", port_id);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS3GPP_ASSERT(cb_data != NULL);

  port_id = (ds3g_siolib_port_e_type)(int)cb_data;
...
------------------------------------------------------------------------*/
  ...
  {
    DS_3GPP_MSG0_LOW( "DTR assert, ignoring");
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch( event )
  {
    ...
    case IFACE_DOWN_EV:
    {
      DS_3GPP_MSG2_MED ("arb_close_in_progress : %d, Iface state: %d",
                         sm_info_ptr->arb_close_in_progress,
                         event_info.iface_down_info.state);
      if ( sm_info_ptr->arb_close_in_progress )
      {
        DS_3GPP_MSG0_LOW( "In arb, UM iface down ev, posting UM_IFACE_DOWN_ARB_EV");
        dsumts_rmsm_post_event( &inst, UMTS_RMSM_UM_IFACE_DOWN_ARB_EV );
      }
      else if( event_info.iface_down_info.state != IFACE_DOWN )
      {
        DS_3GPP_MSG0_LOW( "UM iface down, posting UMTS_RMSM_UM_IFACE_DOWN_EV");
        dsumts_rmsm_post_event( &inst, UMTS_RMSM_UM_IFACE_DOWN_EV );
      }
      else
      {
        DS_3GPP_MSG1_LOW( "Ignoring event %d", event);
      }
  ...
  switch( event )
  {
    case PHYS_LINK_FLOW_ENABLED_EV:    
      ...
      /*---------------------------------------------------------------------
        ...
        laptop flow will be enabled by rx_wm lo water function.
      ---------------------------------------------------------------------*/
      DS_3GPP_MSG0_LOW( "PHYS link flow enabled,"
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
 ...
 //Initialize timer value to 0 before reading from NV
  ...
  DS_3GPP_MSG1_HIGH(" smd_notify_delay_timer value : %d,", ds_smem_tx_delay);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS3GPP_ASSERT(user_data_ptr != NULL);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_3GPP_MSG1_LOW( "rmsmi_info_ptr->instance is invalid: "
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /*lint -e730 */
  DS3GPP_ASSERT( ppp_dev == PPP_RM_DEV ||
  ...
  DS_3GPP_MSG3_MED( "Recd %d ppp event ; pdp type %d for protocol %d ",
                     ppp_event, pdp_type, protocol);
  ...
  else if (protocol == PPP_PROTOCOL_LCP && pdp_type == DS_UMTS_PDP_IPV4V6)
  {
    ...
    {
      DS_3GPP_MSG0_LOW("LCP Up/Resync event received during dual IP call, ignoring");
      return;
    }
    inst.sm_instance =  DSUMTS_RMSM_ANY_SM_INSTANCE;
  }
...
)
{
  if (rm_dev_instance <= DSUMTS_RMSM_INVALID_RM_DEV_INSTANCE ||
         rm_dev_instance >= DSUMTS_RMSM_MAX_RM_DEV_INSTANCES)
  {
    DS_3GPP_MSG1_LOW("Invalid Rm Dev instance %d",rm_dev_instance);
    return NULL;
  }
...
)
{
  ...
  {
    DS_3GPP_MSG0_LOW("FAIL modem_mem_alloc for dsumts_rmsmi_get_um_iface_ptr");
    return NULL;
  }
  ...
  /*-------------------------------------------------------------------------
    Copy APN for DUN call case.
  -------------------------------------------------------------------------*/
  strlcpy((char *)acl_policy_info->apn.name, 
          (const char*)rmsmi_info_ptr->profile_context_info.apn,
          DS_UMTS_MAX_APN_STRING_LEN+1);
  acl_policy_info->apn.length = strlen((char*)rmsmi_info_ptr->
                                        profile_context_info.apn);
  ...
  {
    DS_3GPP_ERROR_LOG_0_EX(rmsmi_info_ptr->subs_id,  
                           "lookup: FAILED route look up by policy!");
    modem_mem_free((void *)acl_policy_info, MODEM_MEM_CLIENT_DATA_CRIT );
    return NULL;
  }
  DS_3GPP_MSG3_HIGH( "lookup: UM Iface [0x%x:%x], "
                     " prio_mask [0x%x]", 
                        um_iface_ptr->name, 
...
)
{
  ...
  {
    DS_3GPP_MSG1_LOW("Unsupported PDP type:%d",
  ...
  DS_3GPP_MSG2_LOW("bring_up_iface_by_policy: ret [%d], errno [%d]", 
                    result, *ps_errno);
...
)
{
  ...
  if(0 != ps_phys_link_ioctl(PS_IFACE_GET_PHYS_LINK(
                             ...
                             &err_no))
  {
    DS_3GPP_MSG0_LOW("Cannot get the Call ID. ");
  }
  //Register with FC
...
)
{
  ...
  {
    DS_3GPP_MSG1_LOW("Cannot get the Call ID.%d", call_id);
  }
  //Deregister with FC
...
)
{
  ...
  if(index >= MAX_WM_CNT) {
      DS_3GPP_MSG0_LOW ("Max WM count reached. Cannot report statistics");
      return;
  }
  ...
  case WM_DIR_PS_SIO_DL:
    DS_3GPP_MSG1_MED("Reporting statistics for WM_DIR_PS_SIO_DL call_id %d",
                      call_id);
  ...
  case WM_DIR_PS_SIO_UL:
    DS_3GPP_MSG1_MED("Reporting statistics for WM_DIR_PS_SIO_UL call_id %d",
                      call_id);
    wm_stat_ptr->wm_info[index].tech_id=UNKNOWN_TECH;
...
)
{
  ...
  if(rmsmi_info_ptr == NULL)
  {
    DS_3GPP_MSG0_LOW("Rmsmi Info pointer is NULL. Assert");
	return;
  }
  ...
  switch (rmsmi_info_ptr->sm_type)
  {
    ...
    {
        DS_3GPP_MSG1_LOW ("Invalid RmSm IP Common state %d to "
    ...
    {
        DS_3GPP_MSG1_LOW("Invalid RmSM PPP state %d to "
                           "receive Entitlement Check Complete",
                            sm_ptr->pdp_specific_info.pdp_ppp.state);
      return;
    }
  ...
  default: 
    DS_3GPP_MSG1_LOW("Invalid state machine type %d",
  ...
  if( result == FALSE )
  {
    DS_3GPP_MSG0_LOW("Entitlement check Fail. Terminate call");
    dsumts_rmsm_post_event( &inst, UMTS_RMSM_RM_PPP_DOWN_EV );
    return;
  }
 ...
 )
{
  ...
  switch (rmsmi_info_ptr->sm_type)
  {
    case DSUMTS_RMSM_SM_TYPE_PDP_IP:
      ...
      {
        DS_3GPP_MSG1_LOW("Invalid state %d to receive Redial "
                           "Check Complete",
                            rmsmi_info_ptr->ds_3gpp_rmsm_ip_common_info_ptr->state);
        return;
      }
...
#if defined(FEATURE_DATA_WCDMA_PS) || defined(FEATURE_GSM_GPRS)
    case DSUMTS_RMSM_SM_TYPE_PDP_PPP:
        DS_3GPP_MSG1_LOW("Unsupported state machine type for redial check %d",
                  rmsmi_info_ptr->sm_type);
        return;
#endif /* FEATURE_DATA_WCDMA_PS || FEATURE_GSM_GPRS */
  ...
  if( result == FALSE )
  {
    DS_3GPP_MSG0_LOW("Redial check Fail. Terminate call");
...
)
{
  ...
  DS_3GPP_MSG1_MED(" IS DUN callc for CM call id %d",cm_call_id);
  ...
  if(!bearer_context_p->ds_bearer_context_dyn_p->is_default)
  {
    ...
    DS_3GPP_MSG0_LOW("bearer is non-default, so returning FALSE");
    return FALSE;
  }
  ...
  {
    DS_3GPP_MSG1_LOW("No PDN context associated with call_id %d",
                       cm_call_id);
    return FALSE;
  }
  /*-------------------------------------------------------------------------
  ...
  {
    DS_3GPP_MSG0_LOW("Not a valid RMSM instance, return FALSE");
    return FALSE;
  }
...
)
{
  ...
  DS_3GPP_MSG2_HIGH("SET RM instance %d SIO Port Id is %d",
                     rmsmi_info_ptr->instance, port_id);
}
...
{
  DS_3GPP_MSG1_MED("Rm Dev %d", rm_dev);  
  if (rm_dev == DSUMTS_RMSM_RM_DEV_INSTANCE_0)
  {
    return PPP_RM_DEV;
  }
  ...
  {
    DS_3GPP_MSG1_LOW("Invalid Rm Dev %d", rm_dev);
...
)
{
  ...
  reg_val = ds_3gpp_iface_reg_ev(iface_ev_p,
                                 DS_3GPP_RMSMI_IFACE_EV_COUNT,
                                 dsumts_rmsmi_um_event_handler_cb,
                                 sm_ptr->um_iface_ptr,
                                 sm_ptr);
  if(!reg_val)
  {
    DS_3GPP_MSG0_LOW("IFACE_EV Registration Failed");
    retval = FALSE;
  }
  ...
  {
    DS_3GPP_MSG0_LOW("null phys link during ev reg");
  }


  /*-----------------------------------------------------------------------
    Register for  ps phys link events 
  -----------------------------------------------------------------------*/
  reg_val = ds_3gpp_phys_link_reg_ev(phys_link_ev_p,
                                     DS_3GPP_RMSMI_PHYS_LINK_EV_COUNT,
                                     dsumts_rmsmi_phys_link_um_event_handler_cb,
  ...
  if(!reg_val)
  {
    DS_3GPP_MSG0_LOW("PHYS_LINK_FLOW_ENABLED_EV Registration Failed");
    retval = FALSE;
  }

  DS_3GPP_MSG2_MED( "Registering for IFACE_EVENTS/PHYS_LINK events for , "
                    "um_iface_ptr: %x , result %d ", 
                    sm_ptr->um_iface_ptr, retval);
...
)
{
  ...
  DS_3GPP_MSG1_HIGH( "dereg rmsmi iface cb events for iface 0x%x",
                      sm_ptr->um_iface_ptr);
  ...
  if ( sm_ptr->um_phys_lnk_ptr == NULL )
  {
    DS_3GPP_MSG0_LOW( "null phys link ptr during dereg" );
  }
...
)
{
  ...
  for (rm_dev = 0; rm_dev < DSUMTS_RMSM_MAX_RM_DEV_INSTANCES; rm_dev++ )
  {
    ...
    if (rmsmi_info_ptr == NULL)
    {
      DS_3GPP_MSG1_LOW( "Cannot allocate Rm Dev Instance: %d ",rm_dev ); 
      continue;
    }

    if (rmsmi_info_ptr->port_id != DS3G_SIOLIB_PORT_NONE)
    {
      DS_3GPP_MSG2_LOW( "Rm Dev %d has an active data session. Port Id: %d ",
                         rm_dev, rmsmi_info_ptr->port_id ); 
    }
    else
    {
      DS_3GPP_MSG1_LOW( "Rm Dev %d is free, use this instance for data session ",
                         rm_dev); 
      break;
    }
  } 
  
  DS_3GPP_MSG2_HIGH( "Rm Dev being returned: %d for port id", 
                      rm_dev,
...
{
  boolean result = TRUE;
      DS_3GPP_ERROR_LOG_1("Rm Dev d", rm_dev);
...
)
{
  ...
  {
    DS_3GPP_MSG1_LOW( "Invalid PPP Dev %d", ppp_dev);
    return FALSE;
  }
...
)
{
  dsumts_rmsmi_info_type *rmsmi_info_ptr = NULL;  
  dsumts_rmsm_rm_dev_instance_type rm_dev;  
  ds3g_siolib_port_e_type port_id = ds3g_siolib_get_modem_port();
