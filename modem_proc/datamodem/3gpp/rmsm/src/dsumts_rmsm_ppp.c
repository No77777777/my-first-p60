...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp/rmsm/src/dsumts_rmsm_ppp.c#2 $ 
...
/*===========================================================================
  FUNCTION DSUMTS_RMSM_PPP_FULL_SRV_CNF_CB()
  ...
  SIDE EFFECTS
    None
===========================================================================*/
LOCAL void dsumts_rmsm_ppp_full_srv_cnf_cb
(
   void     *cb_data,
   boolean   cb_result
)
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_3GPP_MSG0_LOW("dsumts_rmsm_ppp_full_srv_cnf_cb received");
  ...
  for (rmsm_inst = 0; rmsm_inst < DSUMTS_RMSM_MAX_SM_INSTANCES ; rmsm_inst++)
  {
    ...
    if (sm_ptr != NULL) {
      ...
      if(sm_ptr->pdp_specific_info.pdp_ppp.state == DSUMTS_RMSM_IP_WAITING_FULL_SRV_CNF_STATE)
      {
        if (cb_result)
        {
         /*-----------------------------------------------------------------
         Transition the state to mode online cnf state which would 
         bring up the um packet call
         -----------------------------------------------------------------*/
          dsumts_rmsm_ppp_transition_state(sm_ptr,
                                           DSUMTS_RMSM_PPP_FULL_SRV_CNF_STATE);
        }
        else
        {
          /*-------------------------------------------------------------------
...
}/*dsumts_rmsm_ppp_full_srv_cnf_cb*/


/*===========================================================================
  FUNCTION DSUMTS_RMSM_PPP_CLOSE_PRE_ATTACH()
  ...
  SIDE EFFECTS
    None
===========================================================================*/
LOCAL boolean dsumts_rmsm_ppp_close_pre_attach
(
   dsumts_rmsmi_sm_info_type        *sm_ptr 
)
{
  ...
  do
  {
    ...
    if (ds_3gpp_pre_attach_hdlr_active(rmsmi_info_ptr->subs_id))
    {
      cb_ptr = (dsumts_rmsmi_info_type *)
                   ds_3gpp_pre_attach_hdlr_get_cb_data(dsumts_rmsm_ppp_full_srv_cnf_cb);
          ...
          (cb_ptr == rmsmi_info_ptr))
      {
        ...
        for (rmsm_inst=0; rmsm_inst < DSUMTS_RMSM_MAX_SM_INSTANCES; rmsm_inst++)
        {
          if(rmsmi_info_ptr->sm_info_ptr[rmsm_inst]->pdp_specific_info.pdp_ppp.state != 
...
}/*dsumts_rmsm_ppp_close_pre_attach*/
...
boolean dsumts_rmsm_ppp_sm_init( dsumts_rmsmi_sm_info_type *sm_ptr)
{
  ...
  {
    DS_3GPP_MSG0_LOW("Couldn't get UM PPP event handle");
    return FALSE;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch( event )
  {
    ...
    default:
      DS_3GPP_MSG3_LOW( "Invalid UMTS RMSM SM instance:%d, Rm Dev instance:%d,"
                          "event %d, ignoring", 
...
)
{
  ...
  DS_3GPP_MSG3_HIGH( "Recd RM_WANTS_PKT_CALL_EV, state %d for RM DEV inst %d, SM inst %d",
  ...
  switch( sm_ptr->pdp_specific_info.pdp_ppp.state )
  {
    case DSUMTS_RMSM_PPP_NULL_STATE:
      ...
      -------------------------------------------------------------------------*/
      DS3GPP_ASSERT( !rmsmi_info_ptr->is_mt );
#endif /*FEATURE_DATA_TE_MT_PDP*/
      ...
      if (ps_sys_event_ind_ex(PS_SYS_TECH_3GPP,
                              ...
                              &ps_errno) == -1)
      {
        DS_3GPP_MSG1_LOW("Error in sending sys_event ent_chk. ps_errno: %d",
                            ps_errno);
   ...
   case DSUMTS_RMSM_PPP_WAITING_FOR_ENTITLEMENT_CHECK_STATE:
      ...
      dsumts_rmsm_ppp_transition_state(sm_ptr, 
                                       DSUMTS_RMSM_PPP_WAITING_FULL_SRV_CNF_STATE);
    break;
    ...
    case DSUMTS_RMSM_PPP_RINGING_TE:
      ...
      -------------------------------------------------------------------------*/
      DS_3GPP_MSG2_LOW("RMSM_PPP_PKT_CALL_EV: check RMSM  is_locked [%d] is_mt %d", 
              rmsmi_info_ptr->is_locked,
              rmsmi_info_ptr->is_mt);
      DS3GPP_ASSERT( rmsmi_info_ptr->is_locked );
      DS3GPP_ASSERT( rmsmi_info_ptr->is_mt );
    ...
    case DSUMTS_RMSM_PPP_WAITING_FOR_UM_DOWN_STATE:
      ...
      ---------------------------------------------------------------------*/
      DS_3GPP_MSG1_LOW("Unexpected RM_WANTS_PKT_CALL_EV in %d state",
                         sm_ptr->pdp_specific_info.pdp_ppp.state);
      break;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch( sm_ptr->pdp_specific_info.pdp_ppp.state )
  {
    ...
    case DSUMTS_RMSM_PPP_WAITING_FOR_ENTITLEMENT_CHECK_STATE:
    case DSUMTS_RMSM_PPP_FULL_SRV_CNF_STATE:
      dsumts_rmsm_ppp_transition_state( sm_ptr,
                                        DSUMTS_RMSM_PPP_NULL_STATE );
    ...
    case DSUMTS_RMSM_PPP_WAITING_FOR_UM_DOWN_STATE:
      ...
      ---------------------------------------------------------------------*/
      DS_3GPP_MSG1_LOW( "Ignoring RM_IFACE_DOWN_EV in %d state",
                sm_ptr->pdp_specific_info.pdp_ppp.state);
      break;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch( sm_ptr->pdp_specific_info.pdp_ppp.state )
  {
    case DSUMTS_RMSM_PPP_WAITING_FOR_UM_UP_STATE:
      ...
      {
        DS_3GPP_MSG2_LOW("Memory allocation failure for client:%d, size:%d",
      ...
      {
        DS_3GPP_MSG1_HIGH("Setting CHAP/PAP based on profile auth %d",
                           pdata.auth_type);
      ...
      ---------------------------------------------------------------------*/
      dsumts_rmsm_ppp_rm_ppp_config_ptr->subs_id = 
                                  ppp_get_subs_id( rmsmi_info_ptr->subs_id );

      if ( ppp_start( ppp_dev, 
                      dsumts_rmsm_ppp_rm_ppp_config_ptr ) < 0 )
      {
        DS_3GPP_MSG0_LOW("RM PPP config failed, posting RM_IFACE_DOWN");
        dsumts_rmsm_post_event( &inst, UMTS_RMSM_RM_PPP_DOWN_EV );
        break;
      }
      ...
      if ( dsUmtsPdnCntx_configurePdpPpp(sm_ptr->um_iface_ptr, 
                                         ppp_dev) < 0 )
      {
        DS_3GPP_MSG0_LOW("UM PPP config failed, posting RM_IFACE_DOWN");
        dsumts_rmsm_post_event( &inst, UMTS_RMSM_RM_PPP_DOWN_EV );
        break;
      }
    ...
    case DSUMTS_RMSM_PPP_WAITING_FOR_UM_DOWN_STATE:
      ...
      /*---------------------------------------------------------------------
        Ignore event in all other states.
      ---------------------------------------------------------------------*/
      DS_3GPP_MSG1_LOW( "Ignoring UM_IFACE_UP_EV in %d state",
                sm_ptr->pdp_specific_info.pdp_ppp.state);
      break;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch( sm_ptr->pdp_specific_info.pdp_ppp.state )
  {
    ...
    case DSUMTS_RMSM_PPP_WAITING_FOR_UM_DOWN_STATE:
      ...
      {
        DS_3GPP_MSG0_LOW( "MT-UMSM is cleared in UM up no need to do anything");
      }
      else
      {
        DS_3GPP_MSG1_LOW("PPP_PROCESS_RM_IFACE_DOWN: wrong state [%d]", sm_ptr->pdp_specific_info.pdp_ppp.state);
        DS3GPP_ASSERT(0);
        return;
      }
    ...
    case DSUMTS_RMSM_PPP_NULL_STATE:
      ...
      /*---------------------------------------------------------------------
        Ignore event in all other states.
      ---------------------------------------------------------------------*/
      DS_3GPP_MSG1_LOW( "Ignoring UM_IFACE_DOWN_EV in %d state",
                sm_ptr->pdp_specific_info.pdp_ppp.state);
      break;
...
)
{
  ...
  DS_3GPP_MSG2_MED("Entered Process Entitlement Check Failed CMd handling, "
                   "Rm Dev instance: %d in PPP state", 
  ...
  switch (sm_ptr->pdp_specific_info.pdp_ppp.state)
  {
    ...
    default:
      /*Ignore entitlement failed Ev in other state */ 
      break;
  }
...
)
{
  ...
  DS_3GPP_MSG0_LOW("RMSM_PPP_UM_MT_PKT_CALL: UM wants MT PPP pkt call");
  ...
  switch( sm_ptr->pdp_specific_info.pdp_ppp.state )
  {
    case DSUMTS_RMSM_PPP_NULL_STATE:
      ...
      {
        DS_3GPP_MSG0_LOW("RMSM_PPP_MT_PKT_CALL: unable to set MT flag");
      }
      dsumts_rmsm_ppp_transition_state( sm_ptr,
    ...
    default:
      DS_3GPP_MSG1_LOW("RMSM_PPP_UM_MT_PKT_CALL: UM_WANTS_MT_PKT_CALL_EV is invalid in state [%d]",
                sm_ptr->pdp_specific_info.pdp_ppp.state);
  }/* switch(state) */
...
)
{
  ...
  DS_3GPP_MSG0_LOW("PPP_PROCESS: RM rejects MT PPP pkt call");

  /*------------------------------------------------------------------------- 
    Bug trap: 
    (1) check that RMSM is marked for MT
  -------------------------------------------------------------------------*/
  DS3GPP_ASSERT( rmsmi_info_ptr || rmsmi_info_ptr->is_mt );
...
)
{
  ...
  DS_3GPP_MSG0_LOW("PPP_PROCESS: RM Um locked ");
  ...
  switch( sm_ptr->pdp_specific_info.pdp_ppp.state )
  {
    ...
    -------------------------------------------------------------------------*/
     ...
     case DSUMTS_RMSM_PPP_WAITING_FOR_UM_DOWN_STATE:
      ...
      break;
    default:
      DS_3GPP_MSG0_LOW("Um down received in incorrect state: %d ");
      break;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_3GPP_MSG1_HIGH( "Recd UM_PPP_DOWN_EV, state %d", 
                      sm_ptr->pdp_specific_info.pdp_ppp.state );
  ...
  switch( sm_ptr->pdp_specific_info.pdp_ppp.state )
 {
   case DSUMTS_RMSM_PPP_UM_RM_UP_STATE:
     ...
     {
       DS_3GPP_MSG0_LOW("ppp_stop failed!");
     }
     break;
...
)
{
  ...
  ds_umts_pdp_auth_type   pdata;
  char                    passwd_info[PPP_MAX_PASSWD_LEN] = {0};
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch( new_state )
  {
    case DSUMTS_RMSM_PPP_NULL_STATE:
    {
      ...
      {
        DS_3GPP_MSG0_LOW("No further cleanup action needed. Reset ent_chk_reg flag");
      ...
      if(dsumts_rmsm_ppp_close_pre_attach(sm_ptr))
      {
      ...
      {
        DS_3GPP_MSG0_LOW("RMSM_IP_TRANS_STATE: unable to reset MT flag");
      }
      ...
      if(ds3g_siolib_ex_change_mode( SIO_DS_AUTODETECT_MODE, NULL, NULL, NULL,
                                          rmsmi_info_ptr->port_id)
    ...
    case DSUMTS_RMSM_PPP_WAITING_FULL_SRV_CNF_STATE:
        ...
        /*-------------------------------------------------------------------- 
          ...
          DSUMTS_RMSM_PPP_MODE_ONLINE_CNF_STATE

          If ds_3gpp_pre_attach_hdlr_call_orig_req returns FALSE and
          cb_ev_active is FALSE
          RMSM PPP state can not be brought up and needs to be brought down

          if ds_3gpp_pre_attach_hdlr_call_orig_req returns FALSE and
          cb_ev_active is TRUE wait for the cb event

          Also Read authentication info from profile. This info will be used for 
          authentication negotiation with PC
          --------------------------------------------------------------------*/
    ...
    case DSUMTS_RMSM_PPP_WAITING_FOR_UM_UP_STATE:
      ...
      ps_result = dsumts_rmsmi_bring_up_um_iface_by_policy( sm_ptr, 
                                                     &sm_ptr->um_iface_ptr, 
                                                     &ps_iface_errno );
      
      DS_3GPP_MSG3_HIGH( "route lookup ret Um Iface [0x%x:%x], try to bring-up by policy"
                         "errpr no %d", 
      ...
      if ( ps_result  == 0 )
      {
        DS_3GPP_MSG1_LOW("UM i/f 0x%x already up", 
                  sm_ptr->um_iface_ptr);
        DS3GPP_ASSERT(0);
      }
    ...
    case DSUMTS_RMSM_PPP_RINGING_TE:
      ...
      if ( dsat_cgauto_val == 1 ) 
      {
        ...
        DS_3GPP_MSG0_LOW("RMSM_PPP_TRANS_STATE: Rm ready for auto-ans! RING TE and ANS immediately");
        /*-------------------------------------------------------------------------
      ...
      {
        DS_3GPP_MSG0_LOW("RMSM_PPP_TRANS_STATE: Rm is not set for auto-ans! reg ANS+HUP cb");
      }
      ...
      if ( result == DSAT_BUSY ) 
      {
        DS_3GPP_MSG0_LOW("RMSM_PPP_TRANS_STATE: cannot reg ANS+HUP CBs, reject MT call!");
      ...
      else if ( result == DSAT_ERROR ) 
      {
        ...
        DS_3GPP_MSG0_LOW("RMSM_PPP_TRANS_STATE: err! trying to reg hdlr when +CGAUTO=1");
        return;
      }
      ...
      {
        DS_3GPP_MSG0_LOW("RMSM_PPP_TRANS_STATE: err! unknown ret code");
        return;
      }

      ds3g_siolib_set_answer_callbacks( &dsumts_rmsm_ppp_siolib_func_tbl );

      DS_3GPP_MSG0_LOW("RMSM_PPP_TRANS_STATE: RING TE! with SIOLIB auto-ans set");
      ds3g_siolib_alert_user( DSAT_RING, 
    ...
    default:
      DS_3GPP_MSG1_LOW ( "Invalid state %d transition", new_state);
      DS3GPP_ASSERT(0);
      break;
