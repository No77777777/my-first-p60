...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp/rmsm/src/ds_3gpp_rmsm_ip_common.c#3 $ 
  $DateTime: 2022/04/21 04:32:00 $ $Author: pwbldsvc $
...
/*---------------------------------------------------------------------------
  STOP PPP delay timer is used to delay the PPP tear down in a dual IP DUN call
  scenario where one of the IP has been rejected by the network before config
  request for the other IP was received from the thethered PC. In such a case,
  we will start this timer and upon its expiry, if PC fails to send the config
  reuest for the other IP, PPP will be torn down.
---------------------------------------------------------------------------*/
#define DS_3GPP_RMSM_IP_COMMON_STOP_PPP_DELAY_TIME  1000 

/*---------------------------------------------------------------------------
  For a dual IP profile ,if a v4/v6 only call is initiated at TE, instead of 
  waiting indefinitely for both the config packets to arrive, if a particular
  config packet is received we kick start PPP_UP_DELAY timer and on its 
  expiry, go ahead and set up the PPP for the received IP type 
---------------------------------------------------------------------------*/
...
)
{
  ...
  DS_3GPP_MSG1_MED("Configure PPP callback invoked, RMSM info ptr: %x", 
                    this_ptr);
  ...
  if (protocol == PPP_PROTOCOL_IPCP)
  {
    ...
    if (sm_info_ptr == NULL) 
    {
      DS_3GPP_MSG0_LOW("sm_info_ptr = NULL");
      return FALSE;
    }
    ...
    DS_3GPP_MSG3_HIGH("IP ADDR:: %1x ,P DNS:%lx S DNS:%lx",
      ps_ntohl(ip_addr),
  ...
  else if (protocol == PPP_PROTOCOL_IPV6CP)
  {
    ...
    DS_3GPP_MSG4_HIGH("Local IID::%lx%lx, Global IID::%lx%lx", 
                      QWORD_HIGH(ps_ntohll(rm_iid)),
                      QWORD_LOW(ps_ntohll(rm_iid)),
                      QWORD_HIGH(ps_ntohll(te_iid)),
                      QWORD_LOW(ps_ntohll(te_iid))); 
  ...
  DS_3GPP_MSG3_HIGH("Protocol being configured to passive: %d"
     ...
     )
  {
    ...
    if ( (DSUMTS_RMSMI_GET_PDP_TYPE(rmsmi_info_ptr) != DS_UMTS_PDP_IPV4V6) ||
       ...
       )
    {
      ...
      if (DSUMTS_RMSMI_GET_PDP_TYPE(rmsmi_info_ptr) == DS_UMTS_PDP_IPV4V6)
      {
        if(0 != rex_get_timer(&(rmsmi_info_ptr->ppp_up_delay_timer)))
        {
          DS_3GPP_MSG0_HIGH("PPP UP delay timer cleared.");
          (void)rex_clr_timer(&(rmsmi_info_ptr->ppp_up_delay_timer));
        }
      }
      	   ...
      	   !ppp_inject_packet(ppp_dev, proto, *ppp_pkt_ptr) )
      {
        *ppp_pkt_ptr= NULL;
         DSUMTS_LEAVE_CRIT_SECTION( crit_sec_ptr );     
         DS_3GPP_MSG0_LOW("ppp pkt injected ");
      ...
      {
        DSUMTS_LEAVE_CRIT_SECTION( crit_sec_ptr );
        DS_3GPP_MSG0_LOW("ppp pkt error ");
        return FALSE;
      }
    }
    else
  ...
  {
    DS_3GPP_MSG1_LOW("Not injecting PPP pkt. PPP mode: %d", 
...
)
{
  ...
  ds_umts_pdp_type_enum_type pdp_type = DS_UMTS_PDP_IP;

  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

  DS_3GPP_MSG2_MED("Received  ds_3gpp_rmsm_ip_common_rm_ipcp_cfg_req_cb, device: %d"
                     "protocol: %d", device, protocol ); 

  /*lint -e730 -esym(613, item_head_ptr)*/
  if ( NULL == item_head_ptr || 
       NULL == *item_head_ptr ||       
       ((*item_head_ptr)->size <= 0  || (*item_head_ptr)->used <= 0) ||
       device >= PPP_MAX_DEV || 
  ...
  DS_3GPP_MSG2_HIGH("IPCP packet for PDP type %d received in state %d",
                     pdp_type, info_ptr->state);
  ...
  {
    /* Freeing previous IPCP cfg req pkt */
  ...
  {
    DSUMTS_BRANCH_LEAVE_CRIT_SECTION( &(info_ptr->v4_info.ipi_sm_info_crit_sec) );
    DS_3GPP_MSG0_LOW("Error Parsing IPCP Config Req Packet, freeing it");
    dsm_free_packet(item_head_ptr);
  ...
  if(info_ptr->v4_info.send_ipcp_naks_flg )
  {
    ...
    DS_3GPP_MSG1_HIGH(" Initiating IPCP NAK delay timeout of %d ms",
                        DS_3GPP_RMSM_IP_COMMON_IPCP_RES_DELAY_TIME);
  }
...
)
{
  ...
  {
    /*Freeing previous IPV6CP cfg req pkt */
  ...
  {
    DSUMTS_BRANCH_LEAVE_CRIT_SECTION( &(info_ptr->v6_info.ipi_sm_info_crit_sec) );
    DS_3GPP_MSG0_LOW("Error Parsing IPV6CP Config Req Packet, freeing it");
    dsm_free_packet(item_head_ptr);
    dsumts_rmsm_post_event(&inst, UMTS_RMSM_RM_PPP_DOWN_EV);
  ...
  if(info_ptr->v6_info.send_ipv6cp_naks_flg )
  {
    ...
    DS_3GPP_MSG1_HIGH(" Initiating IPV6CP NAK delay timeout of %d ms",
                        DS_3GPP_RMSM_IP_COMMON_IPV6CP_RES_DELAY_TIME);
  }
  ...
  DS_3GPP_MSG1_HIGH("Rx IPV6CP CReq: posting UMTS_RMSM_RM_BRING_UP_UM_IFACE_EV, "
                      "RmSM IP Common State: %d ",
                       info_ptr->state);
...
)
{
  ...
  /*-------------------------------------------------------------------------
   ...
   while ( pkt_len > 0)
   {
     ...
     {
       DS_3GPP_MSG0_MED("Invalid opt_hdr received");
       DSUMTS_BRANCH_LEAVE_CRIT_SECTION( &(info_ptr->v4_info.ipi_sm_info_crit_sec) );
       return -1;
     }
 
     DS_3GPP_MSG7_MED("opt_hdr.type: %d" 
                      " IP address in opt:%lx"
                      " primary DNS address in opt:%lx"
                      " secondary DNS address in opt:%lx"
                      " primary NBNS address in opt:%lx"
                      " secondary NBNS address in opt:%lx",
                      opt_hdr.type,
                      ...
                      info_ptr->ppp_info.ipcp_info.secondary_nbns, 0);
    ...
    switch(opt_hdr.type)
    {
      ...
      case IPCP_1ADDRESS:
        ...
        DS_3GPP_MSG1_LOW("IP address in opt:%lx", 
                           info_ptr->ppp_info.ipcp_info.ip_address);
        break;
      ...
      case IPCP_PRIMARY_DNS:
        ...
        DS_3GPP_MSG1_LOW("primary DNS address in opt:%lx", 
                           info_ptr->ppp_info.ipcp_info.primary_dns);
        break;
      ...
      case IPCP_SECONDARY_DNS:
        ...
        DS_3GPP_MSG1_LOW("secondary DNS address in opt:%lx", 
                           info_ptr->ppp_info.ipcp_info.primary_dns);
        break;
      ...
      case IPCP_PRIMARY_NBNS:
        if( ds_3gpp_nv_manager_get_nbns_enabled(rmsmi_info_ptr->subs_id))  
        {
          ...
          DS_3GPP_MSG1_LOW("primary NBNS address in opt:%lx",
                             info_ptr->ppp_info.ipcp_info.primary_nbns);
        }
      break;
      ...
      case IPCP_SECONDARY_NBNS:
        if( ds_3gpp_nv_manager_get_nbns_enabled(rmsmi_info_ptr->subs_id))  
        {
          ...
          DS_3GPP_MSG1_LOW("secondary NBNS address in opt:%lx",
                             info_ptr->ppp_info.ipcp_info.secondary_nbns);        
        }
      break;
...
)
{
   ...
   DS_3GPP_MSG2_HIGH("Parsing IPV6CP Config REq_PKT, rmsmi_info_ptr: %x ipv6cp id:%d", 
                    rmsmi_info_ptr,cfg_hdr.identifier);
...
)
{
  ...
  DS_3GPP_MSG3_MED("Auth info from Rm PPP, rmsmi_info_ptr:%x, Auth type(%d) valid flags %x", 
                     rmsmi_info_ptr,
                     info_ptr->ppp_info.auth_info.auth_type,
                     info_ptr->ppp_info.valid_fields);
...
)
{
  ...
  if (inst_ptr == NULL) 
  {
    DS_3GPP_MSG0_LOW("inst_ptr = NULL");
    return;
  }
  ...
  {
    DS_3GPP_MSG1_LOW("Wrong PDP Type: %d, Expected V4. Returning",
  ...
  if((dsumts_rmsm_ip_get_state(sm_info_ptr) != DSUMTS_RMSM_IP_WAITING_FOR_UM_UP_STATE)&&
     (dsumts_rmsm_ip_get_state(sm_info_ptr) != DSUMTS_RMSM_IP_WAITING_FULL_SRV_CNF_STATE) &&
     (dsumts_rmsm_ip_get_state(sm_info_ptr) != DSUMTS_RMSM_IP_FULL_SRV_CNF_STATE))

  {
    DS_3GPP_MSG1_LOW("IPCP rsp gen timer fired in wrong state:%d ignoring",
             rmsmi_info_ptr->ds_3gpp_rmsm_ip_common_info_ptr->state);
    return;
  }
  ...
  if( sizeof(struct config_hdr) <= DSM_POOL_ITEM_SIZE(DSM_DS_SMALL_ITEM_POOL))
  {
    ...
    {
    /*-----------------------------------------------------------------------
        Since we ran out of small DSM items. We should not reach here 
        since after we hit low threshold on Small DSM items, RLC will release 
        all the DSM items in its queues.
    -----------------------------------------------------------------------*/
        DS_3GPP_MSG0_LOW("failed to DSM item for IPCP config HDR");
        return;
    }
  }
  ...
  *cp++ = IPCP_1ADDRESS_SIZ;
                              ...
                              opt_len, DSM_DS_SMALL_ITEM_POOL) != opt_len)
  {
    ...
    DS_3GPP_MSG0_LOW("failed to push Pri DNS option to DSM item");
    return;
  }
  ...
  *cp++ = IPCP_1ADDRESS_SIZ;
                              ...
                              opt_len, DSM_DS_SMALL_ITEM_POOL) != opt_len)
  {
    dsm_free_packet(&reply_bp);
    DSUMTS_RMSMI_MEM_FREE(ipcp_buf,MODEM_MEM_CLIENT_DATA);
    DS_3GPP_MSG0_LOW("failed to push Sec DNS option to DSM item");
    return;
  }
  ...
  -----------------------------------------------------------------------*/
    ...
    *cp++ = IPCP_1ADDRESS_SIZ;
                              ...
                              opt_len, DSM_DS_SMALL_ITEM_POOL) != opt_len)
    {
      dsm_free_packet(&reply_bp);
      DSUMTS_RMSMI_MEM_FREE(ipcp_buf,MODEM_MEM_CLIENT_DATA);
      DS_3GPP_MSG0_LOW("failed to push Pri NBNS option to DSM item");
      return;
    }
    ...
    *cp++ = IPCP_1ADDRESS_SIZ;
                                ...
                                opt_len, DSM_DS_SMALL_ITEM_POOL) != opt_len)
    {
      dsm_free_packet(&reply_bp);
      DSUMTS_RMSMI_MEM_FREE(ipcp_buf,MODEM_MEM_CLIENT_DATA);
      DS_3GPP_MSG0_LOW("failed to push Sec NBNS option to DSM item");
      return;
    }
  }
  ...
  DS_3GPP_MSG2_HIGH("IPCP delayed NAK sent;Bogus IP PDNs :%lx Bogus IP SDNs:%lx",
...
)
{
  ...
  if( DSUMTS_RMSMI_GET_PDP_TYPE(rmsmi_info_ptr) != DS_UMTS_PDP_IPV6 &&
      DSUMTS_RMSMI_GET_PDP_TYPE(rmsmi_info_ptr) != DS_UMTS_PDP_IPV4V6)
  {
    DS_3GPP_MSG1_LOW("Wrong PDP Type: %d, Expected V6. Returning",
                       DSUMTS_RMSMI_GET_PDP_TYPE(rmsmi_info_ptr));
    return;
  }
  ...
  if((dsumts_rmsm_ip_get_state(sm_ptr) != DSUMTS_RMSM_IP_WAITING_FOR_UM_UP_STATE) &&
     (dsumts_rmsm_ip_get_state(sm_ptr) != DSUMTS_RMSM_IP_WAITING_FULL_SRV_CNF_STATE)&&
     (dsumts_rmsm_ip_get_state(sm_ptr) != DSUMTS_RMSM_IP_FULL_SRV_CNF_STATE))
  {
    DS_3GPP_MSG1_LOW("IPv6CP rsp gen timer fired in wrong state:%d ignoring",
             rmsmi_info_ptr->ds_3gpp_rmsm_ip_common_info_ptr->state);
    return;
  }
  ...
  *cp++ = IPV6CP_IFACE_ID_OPTION_SIZE;
  ...
  if(pppi_outgoing(ppp_dev, protocol, &reply_bp, NULL) == -1)
  {
   /*---------------------------------------------------------------------
     pppi_outgoing() frees the packet in case of an error
   ---------------------------------------------------------------------*/
   DS_3GPP_MSG0_LOW( "pppi_outgoing() Error");
  }

  DSUMTS_RMSMI_MEM_FREE(ipv6cp_buf,MODEM_MEM_CLIENT_DATA);
  DS_3GPP_MSG2_LOW("IPv6CP delayed NAK sent IID :%lx%lx",
                   QWORD_HIGH(iid),
                   QWORD_LOW(iid));
...
)
{
  ...
  {
    DS_3GPP_MSG1_LOW("PDP Type is: %d, Expected V4 or V6", 
                        DSUMTS_RMSMI_GET_PDP_TYPE(rmsmi_info_ptr));
    ;
  }
...
)
{
  ...
  DS_3GPP_MSG2_MED("Entered Process Entitlement Check Failed CMd handling, "
                   "Rm Dev instance: %d state %d", inst_ptr->rm_dev_instance,
                   rmsmi_info_ptr->ds_3gpp_rmsm_ip_common_info_ptr->state);
...
)
{
  ...
  dsumts_rmsmi_sm_info_type* sm_info_ptr = NULL;
  dsumts_rmsm_sm_instance_type sm_instance = DSUMTS_RMSM_INVALID_SM_INSTANCE;
  /*----------------------------------------------------------------------*/
  ...
  DS_3GPP_MSG5_MED("Entered dsumts_rmsmi_transition_state,"
                   " rm_dev_instance: %d, pdp type %d"
                   " Changing the State of RmSm IP common from %d to %d for port:%d",
                     rm_dev_instance, 
                     pdp_type,
                     rmsmi_info_ptr->ds_3gpp_rmsm_ip_common_info_ptr->state,
                     new_state,
  ...
  switch (new_state)
  {
    case DS_3GPP_RMSM_IP_COMMON_NULL_STATE:
    {
      ...
      {
        DS_3GPP_MSG0_LOW("No further cleanup action needed. "
      ...
      {
        DS_3GPP_MSG0_LOW("Error setting CD State");
      }
      ...
      if (ds3g_siolib_ex_change_mode
           ...
           == DS3G_SIOLIB_NO_PORT_OPEN)
      {
        DS_3GPP_MSG0_LOW("Error changing mode to autodetect");
      }
      ...
      for (sm_instance = DSUMTS_RMSM_V4_SM_INSTANCE;
             sm_instance < DSUMTS_RMSM_MAX_SM_INSTANCES; sm_instance++)
      {
      ...
      (void)rex_clr_timer(&(rmsmi_info_ptr->stop_ppp_delay_timer));
      ...
      if (pdp_type == DS_UMTS_PDP_IPV4 || pdp_type == DS_UMTS_PDP_IPV4V6  )
      {
         ...
         if (retval == -1)
         {
           DS_3GPP_MSG0_LOW("Unable to deregister IPCP events");
         }
      ...
      if (pdp_type == DS_UMTS_PDP_IPV6 || pdp_type == DS_UMTS_PDP_IPV4V6)
      {
        ...
        if (retval == -1)
        {
          DS_3GPP_MSG0_LOW("Unable to deregister IPV6CP events");
        }
      ...
      if (retval == -1)
      {
        DS_3GPP_MSG0_LOW("Unable to deregister LCP events");
      }
      ...
      if (retval == -1)
      {
        DS_3GPP_MSG0_LOW("PPP Abort failed");
      }
     ...
     rmsmi_info_ptr->mt_te_ring = FALSE;
      ...
      {
       DS_3GPP_MSG0_LOW("RMSM_IP_TRANS_STATE: unable to "
                          "reset MT flag");
      }
    ...
    case DS_3GPP_RMSM_IP_COMMON_RINGING_TE_STATE:
    {
      ...
      {
        DS_3GPP_MSG0_LOW("RMSM_IP_TRANS_STATE: Rm ready for auto-ans! "
      ...
      {
        DS_3GPP_MSG0_LOW("RMSM_IP_TRANS_STATE: Rm is not set "
      ...
      if ( result == DSAT_BUSY ) 
      {
        DS_3GPP_MSG0_LOW("RMSM_IP_TRANS_STATE: cannot reg "
      ...
      else if ( result == DSAT_ERROR ) 
      {
        ...
        DS_3GPP_MSG0_LOW("RMSM_IP_TRANS_STATE: err! trying to register when +CGAUTO=1");
        return;
      }
      else if ( result == DSAT_OK ) 
      {
        DS_3GPP_MSG0_LOW("RMSM_IP_TRANS_STATE: ANS and HUP registered"
                          "successfully!");
      }
      else
      {
        DS_3GPP_MSG0_LOW("RMSM_IP_TRANS_STATE: err! unknown ret code");
        return;
      }
      ...
      DS_3GPP_MSG0_LOW("RMSM_IP_TRANS_STATE: RING TE! "
    ...
    else
    {
     ...
     {
       DS_3GPP_MSG0_LOW ("MT call no PPP server");
      ...
      rmsmi_info_ptr->rm_ppp_config_ptr->auths_peer = FALSE;
       ...
       {
         DS_3GPP_MSG1_LOW ("Error PDP Auth profile read: %d,using CHAP/PAP"
       ...
       {
         DS_3GPP_MSG0_LOW("Setting CHAP/PAP based on profile auth");
...
#endif /* FEATURE_DATA_TE_MT_PDP */
     {
       ...
       if (rr_dtm_get_max_LAPDm_octets() != 0) {
         ...
         DS_3GPP_MSG0_HIGH("Using PAP protocol for GSM DTM");
       }
       else
#endif /* FEATURE_GSM_DTM */
       {
         if (ds_umts_get_pdp_profile_auth_info_per_subs((uint16)rmsmi_info_ptr->profile_id,
                             dsumts_subs_mgr_get_subs_id(rmsmi_info_ptr->subs_id),
                                                &pdata) == DS_UMTS_PDP_SUCCESS )
         {
           DS_3GPP_MSG1_LOW("Setting CHAP/PAP based on profile auth %d",
                              pdata.auth_type);
           if (pdata.auth_type == DS_UMTS_PDP_AUTH_PAP)
           {
             rmsmi_info_ptr->rm_ppp_config_ptr->lcp_info.
      ...
      ---------------------------------------------------------------------*/
      rmsmi_info_ptr->rm_ppp_config_ptr->subs_id = 
                                     ppp_get_subs_id( rmsmi_info_ptr->subs_id );

      if ( ppp_start( ppp_dev, 
                      rmsmi_info_ptr->rm_ppp_config_ptr ) < 0 )
      {
        DS_3GPP_MSG0_LOW("RM PPP config failed, posting RM_PPP_DOWN");
        dsumts_rmsm_post_event( &inst , UMTS_RMSM_RM_PPP_DOWN_EV );
        break;
      }
      ...
      if (retval == -1)
      {
        DS_3GPP_MSG0_LOW("Unable to register for LCP events"); 
        dsumts_rmsm_post_event( &inst , UMTS_RMSM_RM_PPP_DOWN_EV );
        break;
      }
          ...
          pdp_type == DS_UMTS_PDP_IPV4V6)
      {
        ...
        if (retval == -1)
        {
          DS_3GPP_MSG0_LOW("Unable to register for IPCP events"); 
          dsumts_rmsm_post_event( &inst , UMTS_RMSM_RM_PPP_DOWN_EV );
          break;
        }
          ...
          pdp_type == DS_UMTS_PDP_IPV4V6)
      {
        ...
        if (retval == -1)
        {
          DS_3GPP_MSG0_LOW("Unable to register for IPV6CP events"); 
          dsumts_rmsm_post_event( &inst , UMTS_RMSM_RM_PPP_DOWN_EV );
          break;
        }
...
/*===========================================================================
 FUNCTION   DS_3GPP_RMSM_IP_COMMON_STOP_PPP

 DESCRIPTION
 
   This function will be called when ppp_stop needs to be triggered.
...
===========================================================================*/
LOCAL void ds_3gpp_rmsm_ip_common_stop_ppp
( 
  dsumts_rmsmi_info_type     *rmsmi_info_ptr
)
{
  ds_3gpp_rmsm_ip_common_info_type* info_ptr = NULL;
  ppp_dev_enum_type                 ppp_dev = PPP_MAX_DEV;
  ...
  return;
} /* ds_3gpp_rmsm_ip_common_stop_ppp */

/*===========================================================================
 FUNCTION   DS_3GPP_RMSM_IP_COMMON_STOP_PPP_DELAY_TIMER_CMD_HDLR

 DESCRIPTION
   
   This function will be called when the STOP_PPP_DELAY_TIMER expires
...
===========================================================================*/
LOCAL void ds_3gpp_rmsm_ip_common_stop_ppp_delay_timer_cmd_hdlr
( 
  dsumts_rmsm_instance_type* inst_ptr
)
{
  ...
  ds_3gpp_rmsm_ip_common_stop_ppp(rmsmi_info_ptr);
  return;
}
...
)
{
  ...
  if (DSUMTS_RMSMI_GET_PDP_TYPE(rmsmi_info_ptr) == DS_UMTS_PDP_IPV4V6)
  {
     ...
     {
       inst_ptr->sm_instance = DSUMTS_RMSM_V4_SM_INSTANCE;
     }
     else if(!IPCP_CREQ_RECD(info_ptr) && IPV6CP_CREQ_RECD(info_ptr))
     {
       inst_ptr->sm_instance = DSUMTS_RMSM_V6_SM_INSTANCE;
     }
     else
     {
       DS_3GPP_MSG0_LOW("Both config Packets Received!");
       return;
     }  
  }
  else
  {
    DS_3GPP_MSG0_LOW("Wait Delay Timer triggered for Incorrect PDP type!");
    return;
  }
  sm_info_ptr = dsumts_rmsmi_get_sm_info_ptr(inst_ptr);
  ...
  if(V6_SM_INSTANCE(inst_ptr->sm_instance))
  {
  	   ...
  	   !ppp_inject_packet(ppp_dev, proto, *ppp_pkt_ptr) )
	{
	  *ppp_pkt_ptr= NULL;
	  ...
	  dsumts_rmsm_post_event( inst_ptr , UMTS_RMSM_RM_PPP_CONFIGURED_EV );
	}
	else
	{
	  DSUMTS_LEAVE_CRIT_SECTION( crit_sec_ptr );
...
)
{
  ...
  if(info_ptr == NULL)
  {
    DS_3GPP_MSG0_LOW("MEM ALLOC Failure: RmSm ip common Info ptr is NULL during init");
    return FALSE;
  }
...
)
{
  ...
  DS_3GPP_MSG0_LOW("RMSM_PROCESS_MT: UM wants MT PDP-IP pkt call");
  ...
  switch( rmsmi_info_ptr->ds_3gpp_rmsm_ip_common_info_ptr->state )
  {
    case DS_3GPP_RMSM_IP_COMMON_NULL_STATE:
      ...
      {
        DS_3GPP_MSG0_LOW("RMSM_IP_MT_PKT_CALL: unable to set MT flag");
      }
      DS_3GPP_MSG0_LOW("RMSM_IP_PKT_CALL_EV: transition to RMSM_IP_RINGING_TE");
      ds_3gpp_rmsm_ip_common_transition_state( inst_ptr->rm_dev_instance, 
...
)
{
  ...
  -------------------------------------------------------------------------*/
  if ( (!rmsmi_info_ptr) || (!rmsmi_info_ptr->is_mt) )
  {
    return;
  }
...
)
{
  ...
  { 
    DS_3GPP_MSG0_LOW("Sm Instance Invalid !");
    return;
  }
  switch (rmsmi_info_ptr->ds_3gpp_rmsm_ip_common_info_ptr->state)
  {
...
DS_3GPP_MSG3_MED("Handle Rm Wants pkt call EV, "
  ...
  switch( rmsmi_info_ptr->ds_3gpp_rmsm_ip_common_info_ptr->state )
  {
    ...
    case DS_3GPP_RMSM_IP_COMMON_RINGING_TE_STATE:
      ...
      -------------------------------------------------------------------------*/
      DS_3GPP_MSG2_HIGH("RMSM_IP_PKT_CALL_EV: check RMSM is locked[%d], is_mt [%d]", 
 ...
 DS_3GPP_MSG3_MED("Handle Um Iface Down EV, "
  ...
  switch (rmsmi_info_ptr->ds_3gpp_rmsm_ip_common_info_ptr->state)
  {
    case DS_3GPP_RMSM_IP_COMMON_WAITING_FOR_PPP_CONFIGURED_STATE:
    {
      ...
      {
        redial_chk_info.net_down_reason = PS_NET_DOWN_REASON_VALIDATION_FAILURE;
      }
    ...
    case DS_3GPP_RMSM_IP_COMMON_PPP_CONFIGURED_STATE:
    {
             ...
             DS_3GPP_RMSM_IP_COMMON_WAITING_FOR_PPP_CONFIGURED_STATE )
      {
        ...
        dsumtsps_mt_umsm_post_ev( rmsmi_info_ptr->mt_index, 
                      ...
                      DS_3GPP_RMSM_IP_COMMON_PPP_CONFIGURED_STATE )
      {
        DS_3GPP_MSG1_LOW("IP_PROCESS_UM_IFACE_DOWN: wrong state [%d]",
                  rmsmi_info_ptr->ds_3gpp_rmsm_ip_common_info_ptr->state);
        return;
      }
      ...
      companion_sm_info_ptr = dsumts_rmsmi_get_companion_sm_ptr(inst_ptr);

      if (V4_SM_INSTANCE(inst_ptr->sm_instance))
      {
        /*---------------------------------------------------------------------
          If Iface down recd for IPv4 and IPv6CP is received and 
          iface down has not been recd for IPv6 iface then donot tear DUN call
        ---------------------------------------------------------------------*/
             ...
             companion_sm_info_ptr->recd_um_down == FALSE )
        {
          ...
          if ( PS_IFACE_IS_VALID(companion_sm_info_ptr->um_iface_ptr) &&
               PS_IFACEI_GET_STATE(companion_sm_info_ptr->um_iface_ptr) == IFACE_UP )
          {
            ds_3gpp_rmsm_ip_common_reset_ipcp_creq_recd(info_ptr);
            ppp_set_mode( ppp_dev, PPP_INTERNAL_MODE );
        ...
        else if( (DSUMTS_RMSMI_GET_PDP_TYPE(rmsmi_info_ptr) == 
                                                      DS_UMTS_PDP_IPV4V6) &&
                  companion_sm_info_ptr != NULL &&
                  companion_sm_info_ptr->recd_um_down == FALSE)
        {          
          DS_3GPP_MSG0_HIGH("PPP stop delay timer started.");
          (void)rex_set_timer(&(rmsmi_info_ptr->stop_ppp_delay_timer),
               (rex_timer_cnt_type)DS_3GPP_RMSM_IP_COMMON_STOP_PPP_DELAY_TIME);
        }
...
)
{
  ...
  if(info_ptr == NULL)
  {
    DS_3GPP_MSG0_LOW("Info ptr is NULL !");
    return;
  }
  ...
  DS_3GPP_MSG3_MED("Handle PPP Up, rm_dev_instance: %d, sm_instance: %d in state %d",
                    inst_ptr->rm_dev_instance, 
                    inst_ptr->sm_instance,
                    rmsmi_info_ptr->ds_3gpp_rmsm_ip_common_info_ptr->state);
  ...
  switch (rmsmi_info_ptr->ds_3gpp_rmsm_ip_common_info_ptr->state)
  {
    ...
    case DS_3GPP_RMSM_IP_COMMON_WAITING_FOR_PPP_CONFIGURED_STATE:
    {
      DS_3GPP_MSG2_HIGH("Recieving PPP up in Invalid state, rm_dev_instance: %d, sm_instance: %d",
                       inst_ptr->rm_dev_instance,
                       inst_ptr->sm_instance);
      if ((DSUMTS_RMSMI_GET_PDP_TYPE(rmsmi_info_ptr) == DS_UMTS_PDP_IPV4V6) &&
         (ds_3gpp_nv_manager_get_allow_single_IP_call_dualIP_pf(rmsmi_info_ptr->subs_id)))
      {
        /* -------------------------------------------------------------------------------
        ...
        if((IPCP_CREQ_RECD(info_ptr) && !IPV6CP_CREQ_RECD(info_ptr)) ||
	      (!IPCP_CREQ_RECD(info_ptr) && IPV6CP_CREQ_RECD(info_ptr)))
	    {
          DS_3GPP_MSG0_HIGH("PPP UP delay timer started.");
          (void)rex_set_timer(&(rmsmi_info_ptr->ppp_up_delay_timer),
          (rex_timer_cnt_type)ds_3gpp_nv_manager_get_ppp_wait_timer(rmsmi_info_ptr->subs_id));
		  }
	    }
      }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  switch( cmd )
  {
    ...
    case DS_CMD_3GPP_RMSM_STOP_PPP_DELAY_TIMER:
      ds_3gpp_rmsm_ip_common_stop_ppp_delay_timer_cmd_hdlr(inst_ptr);
      break;
    ...
    default:
      DS_3GPP_ERROR_LOG_3("Invalid UMTS RMSM sm instance: d, "
...
)
{
  ...
  {
    DS_3GPP_MSG0_LOW("RmSmi info ptr is NULL");
    return DS_3GPP_RMSM_IP_COMMON_NULL_STATE;
  }
...
/*===========================================================================
 FUNCTION   DS_3GPP_RMSM_IP_COMMON_STOP_PPP_DELAY_TIMER_CB
...
===========================================================================*/
void ds_3gpp_rmsm_ip_common_stop_ppp_delay_timer_cb
( 
  unsigned long  callback_data
)
{
  ...
  DS_3GPP_MSG0_HIGH("PPP stop delay timer expired.");
  ...
  {
    cmd_ptr->hdr.cmd_id = DS_CMD_3GPP_RMSM_STOP_PPP_DELAY_TIMER;
    data_ptr = (dsumts_rmsm_instance_type*)cmd_ptr->cmd_payload_ptr;
    data_ptr->rm_dev_instance = rmsmi_info_ptr->instance;
    ds_put_cmd(cmd_ptr);
  }
  ...
  return;
} /* ds_3gpp_rmsm_ip_common_stop_ppp_delay_timer_cb */
...
)
{
  ...
  DS_3GPP_MSG0_HIGH("PPP wait delay timer expired.");
