...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp/rmsm/src/dsumts_rmsm.c#2 $ 
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (FALSE == result)
  {
    DS_3GPP_MSG0_LOW("RMSM initialization failed ");
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  default:
    DS_3GPP_MSG1_LOW("invalid SM type :%d",rmsmi_info_ptr->sm_type);
  }
   ...
   rmsm_dun_ctrl_param_ptr
    ...
    if(rmsmi_info_ptr->sm_type == DSUMTS_RMSM_SM_TYPE_PDP_IP )
    {
      ...
      switch (rmsm_ip_state)
      {
        ...
        default:
          DS_3GPP_MSG1_LOW("dun_ctrl_resp called in invalid RMSM state: %d",
                                rmsm_ip_state);
          return ;
      }
    }
    ...
    {
      DS_3GPP_MSG0_LOW("Invalid SM type");
      return;
    }
  }
...
)
{
  ...
  DS_3GPP_MSG0_LOW("RMSM_IS_RM_AVAIL: Rm available!");
...
)
{
  ...
  DS_3GPP_MSG0_LOW("RMSM_RELEASE_RM: Rm released!");
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_3GPP_MSG0_LOW("RMSM_MT_HANGUP_CB: TE hung-up");
  rmsmi_info_ptr = dsumts_rmsmi_get_rmsmi_info_ptr(inst.rm_dev_instance);
...
)
{
  ...
  {
    DS_3GPP_ERROR_FATAL("RMSM_MT_ANS_CB: ans_cb_data validation failed!");
    return NULL;
  }
  ...
  DS_3GPP_MSG0_LOW("RMSM_MT_ANS_CB: dereg dsat PDP and SIOLIB auto-ans hdlrs");
