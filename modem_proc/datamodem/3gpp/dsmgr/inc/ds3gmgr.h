...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp/dsmgr/inc/ds3gmgr.h#4 $ $DateTime: 2021/04/12 20:04:55 $ $Author: pwbldsvc $
...
===========================================================================*/
#ifndef FEATURE_DATA_NR5G_SA
  #define SYS_SYS_MODE_NR5G        12 
  #define SYS_SYS_MODE_MASK_NR5G  ( 1<< ((int)(SYS_SYS_MODE_NR5G)) )
...
typedef enum
{
  ...
  DS_PKT_SYS_MODE_5G    =  SYS_SYS_MODE_NR5G,
...
#define DS_3GPP_IS_UMTS_CURRENT_MODE(call_mode) \
 (                                              \
     (call_mode == SYS_SYS_MODE_WCDMA) ||       \
     (call_mode == SYS_SYS_MODE_GSM)   ||       \
     (call_mode == SYS_SYS_MODE_TDS)            \
...
/*---------------------------------------------------------------------------
  Structure for Subscription not available info
---------------------------------------------------------------------------*/
typedef struct
{
  uint8          subs_id_mask;
    /**< subs mask ind which subs are no longer available */

  uint8         *sub_clnup_reason_ptr;
  /**< pointer to structure that indications the clnup reason */

} ds3g_cm_sub_not_avail_info_type;
...
typedef struct
{
 ...
 /*-------------------------------------------------------------------------
  ...
  boolean (*bearer_reactivate_handler)
                  (
                    cm_call_id_type            call_id,
                    cm_call_mode_info_s_type  *mode_info_ptr,
                    cm_act_bearer_rsp_s_type  *cm_act_res_p,
  ...
  /*------------------------------------------------------------------------- 
    ...
    -------------------------------------------------------------------------*/
  void(*request_modem_power_stats)
                  (
                    sys_modem_as_id_e_type subs_id
                  );
  /*-------------------------------------------------------------------------
  ...
  -------------------------------------------------------------------------*/
  void(*reg_tx_wm_non_empty_func_hdlr)
                  (
                    sys_modem_as_id_e_type subs_id
                  );
  /*-------------------------------------------------------------------------
    Handler to get total ul current wm cnt
  -------------------------------------------------------------------------*/
  uint64(*get_total_ul_cur_wm_cnt_hdlr)
                  (
                    sys_modem_as_id_e_type subs_id
                  );
...
SIDE EFFECTS  None
===========================================================================*/
boolean ds3g_is_hdr_in_srvc(sys_modem_as_id_e_type subs_id);
