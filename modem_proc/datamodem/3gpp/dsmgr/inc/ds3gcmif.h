...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp/dsmgr/inc/ds3gcmif.h#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
/*------------------------------------------------------------------------- 
  Function ptr to a function that will be called when CM_PH_INFO from CM
  is received.
  Clients/Modules which will be interested in getting the CM PH info will
  register and deregister when they no longer need it.
 
  Primarily this is needed for pre attach hdlr to save the CM PH info
  -------------------------------------------------------------------------*/

#define DS3G_CM_PH_INFO_MAX_CB_CLIENT 3

typedef void (*ds3g_cm_ph_cb_fn_ptr)
(
...
typedef struct
{
  ds3g_cm_ph_cb_fn_ptr   cm_ph_cb_fn;
}ds3g_cm_ph_cb_info_type;


typedef struct
{
  uint8                       cb_cnt;
  ds3g_cm_ph_cb_info_type    *cm_ph_cb_info[DS3G_CM_PH_INFO_MAX_CB_CLIENT];
}ds3g_cm_ph_cb_info;

/*---------------------------------------------------------------------------
  Structure to store info for TAC/LAC
---------------------------------------------------------------------------*/
typedef enum
{
  DS3G_TAC_INFO  = 0,
  DS3G_LAC_INFO  = 1
}ds3g_cm_tac_or_lac_info_enum_type;

typedef struct
{
  sys_lac_type                       tac_or_lac;
  ds3g_cm_tac_or_lac_info_enum_type  tac_or_lac_enum_info;
}ds3g_cm_tac_or_lac_info_type;

typedef struct
{
  sys_modem_as_id_e_type             subs_id;
  ds3g_cm_tac_or_lac_info_type       tac_lac_info;
}ds3g_cm_tac_lac_info_type;
