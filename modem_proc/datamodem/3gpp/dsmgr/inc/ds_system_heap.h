...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp/dsmgr/inc/ds_system_heap.h#2 $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DATA_MSG3_MED("modem_mem_alloc 0x%x:FILE %s:LINE %d",(unsigned int)tmp,file_name,line_number);
  return tmp;
...
)
{
  if (NULL != mem_ptr_ptr)
  {
    DATA_MSG3_MED("modem_mem_free 0x%x:FILE %s:LINE %d",(unsigned int)mem_ptr_ptr,
                                                   file_name,
                                                   line_number);
    if(NULL != mem_ptr_ptr )
    {
