...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp/dsmgr/src/ds3gsiolib.c#7 $ $DateTime: 2021/02/08 22:33:06 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ------------------------------------------------------------
08/24/20   ks      Removed DS flag FEATURE_DATA_ATCOP.
...
08/25/16   skc     Fixed issue related to cd assert for DUN call.
...
/*---------------------------------------------------------------------------
  Struct for DS_CMD_SIO_TIMER_EXPIRED
---------------------------------------------------------------------------*/
typedef struct
{
  unsigned long  timer_id;
} ds3g_siolib_timer_exp_cmd_type;
...
)
{
  ...
  ds3g_siolib_timer_exp_cmd_type  *timer_expired_cmd_ptr = NULL;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  cmd_ptr = ds_allocate_cmd_buf(sizeof(ds3g_siolib_timer_exp_cmd_type));
  ...
  cmd_ptr->hdr.cmd_id = DS_CMD_SIO_TIMER_EXPIRED;
  ds_put_cmd( cmd_ptr );
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_3GPP_ERROR_FATAL("input port is DS3G_SIOLIB_PORT_NONE");
    return;
  }
...
LOCAL void ds3g_siolib_dtr_changed_cb( void* cb_data )
{
  ...
  ds3g_siolib_port_e_type         port = (ds3g_siolib_port_e_type)(int)cb_data;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(port > DS3G_SIOLIB_PORT_MAX || port <= DS3G_SIOLIB_PORT_NONE)
  {
    ...
    DS_SIO_MSG0_LOW("DTR CB on invalid port");
    goto func_exit;
  }
  ...
  if (NULL != dsm_item_ptr)
  {
    ...
    else
    {
      ...
      DS_SIO_MSG0_LOW("Failed to pushdown DSM item for DTR event");
  ...
  else
  {
    ...
    DS_SIO_MSG0_LOW("Failed to allocate DSM item for DTR event");
  }
  ...
  DS_SIO_MSG5_MED
  ( "ds3g_siolib_dtr_changed_cb: port index %d port %d "
    "ds3g_siolib_current_ring_type %d port_is_allocated %d "
  ...
  if(((DS3G_SIOLIB_RING_TYPE_NONE == ds3g_siolib_current_ring_type) &&
       ...
       ds3g_siolib_dtr_handling_pending_flag_is_set(port)) 
  {
    DS_SIO_MSG1_LOW( "Invalid DTR change: Port Number %d ", port);
...
func_exit:
  ...
  {
    DS_3GPP_ERROR_LOG_1("ds3g_siolib_dtr_changed_cb: err d",
                       local_err);
  }
  return;
}
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
 ...
 ------------------------------------------------------------------------*/
  ...
  ds3g_timer_register( &ds3g_siolib_ring_timer[DS3G_SIOLIB_PORTID_0],
                        ds3g_siolib_timer_cb,
                        DS3G_TIMER_RING_TE_1);

  ds3g_timer_register( &ds3g_siolib_dcd_wink_timer[DS3G_SIOLIB_PORTID_0],
                        ds3g_siolib_timer_cb,
                        DS3G_TIMER_WINK_DCD_1);

  ds3g_timer_register( &ds3g_siolib_ring_timer[DS3G_SIOLIB_PORTID_1],
                        ds3g_siolib_timer_cb,
                        DS3G_TIMER_RING_TE_2);

  ds3g_timer_register( &ds3g_siolib_dcd_wink_timer[DS3G_SIOLIB_PORTID_1],
                        ds3g_siolib_timer_cb,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_3GPP_ERROR_FATAL("open port is DS3G_SIOLIB_PORT_NONE");
  ...
  else
  {
    ...
    */
    ...
    switch(port_id) 
    {
      ...
      #error code not present
#endif /* FEATURE_DATA_MUX */
        DS_SIO_MSG1_MED("ds3g_siolib_open port_id : %d", port_id);
      ...
      case SIO_PORT_SMD_DATA11:
       ...
       if ( NV_DONE_S == nv_status )
       {
         if( (nv_item.disable_cm_call_type & CM_CALL_TYPE_VT_MASK) ||
             (nv_item.disable_cm_call_type & CM_CALL_TYPE_CS_DATA_MASK))
         {
           state->serial_info.stream_id = SIO_NO_STREAM_ID;
           return DS3G_SIOLIB_NO_PORT_OPEN; 
         }
       }
       DS_SIO_MSG1_MED("ds3g_siolib_open port_id CSVT/SMD11 : %d", port_id);
        /* Register for DTR event with DS3G_SIO port */
        ioctl_param.enable_dte_ready_event_ext.cb_func = ds3g_siolib_dtr_changed_cb;
        ioctl_param.enable_dte_ready_event_ext.cb_data = (void*)open_port;
        break;
      ...
      default:
        DS_3GPP_ERROR_FATAL("Invalid Port being opened in ds3g_siolib_open");
        return(ret_val);
    }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(port_id == DS3G_SIOLIB_PORT_NONE)
  {
    DS_3GPP_ERROR_FATAL("input port is equal to DS3G_SIOLIB_PORT_NONE");
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(port_id == DS3G_SIOLIB_PORT_NONE)
  {
    DS_3GPP_ERROR_FATAL("input port equal to DS3G_SIOLIB_PORT_NONE");
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_3GPP_ERROR_FATAL("pending_close_port==DS3G_SIOLIB_PORT_NONE");
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_3GPP_ERROR_FATAL("siolib_modem_port==SIOLIB_PORT_NONE");
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (port_id <= DS3G_SIOLIB_PORT_NONE ||
      port_id >= DS3G_SIOLIB_PORT_MAX )
  {
    DS_SIO_MSG0_ERROR("siolib_modem_port == DS3G_SIOLIB_PORT_NONE");
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if (port_id <= DS3G_SIOLIB_PORT_NONE ||
      port_id >= DS3G_SIOLIB_PORT_MAX )
  {
    DS_3GPP_ERROR_FATAL("siolib_modem_port== DS3G_SIOLIB_PORT_NONE");
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (port_id <= DS3G_SIOLIB_PORT_NONE ||
      port_id >= DS3G_SIOLIB_PORT_MAX )
  {
    DS_3GPP_ERROR_FATAL("port_id IS INVALID");
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /*-------------------------------------------------------------------------
    ...
    command id.
  -------------------------------------------------------------------------*/
  DS_SIO_MSG1_LOW( "ds3g sio process cmd id [%d]", cmd_ptr->hdr.cmd_id );
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
 ...
 ------------------------------------------------------------------------*/     
  ...
  if( !call_answered && duration )
  {
    ...
    ds3g_timer_start( DS3G_TIMER_RING_TE_1, duration );
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
 ...
 ------------------------------------------------------------------------*/     
  ...
  if( ds3g_timer_get_status( DS3G_TIMER_RING_TE_1 ) )
  {
    ds3g_timer_stop( DS3G_TIMER_RING_TE_1 );
  }
...
#if   defined (FEATURE_DATA_A2_MDM)
  ...
  case DS3G_SIOLIB_UNSOLICTED_ACC:
    if ( tx_port == DS3G_SIOLIB_DATA_PORT && tx_ptr != NULL) 
    {
            ...
            port_count < DS3G_SIOLIB_PORT_MAX; port_count++) 
      {
        ...
        DS_SIO_MSG3_LOW("Port:%d, DTR%d, initDTR%d",port_count,dtr_status,state->is_dtr_high);
            ...
            ( dtr_status == TRUE) )
        {
          ...
          else
          {
            ...
            if (sio_info_ptr != NULL)
            {
              /* Send URC if the port is not blocked and no other port is active concurrently */
              /* Queue the URC if this port is blocked */
              /* Queue the URC if this port is not blocked but other port may active 
                 in case of multi-port configuration. Sending URC may effect the processing 
                 of AT Command on another port. */
              if( FALSE == block_indications_flag &&
                  DS3G_SIOLIB_PORT_NONE == ds3g_siolib_get_active_port() )
              {
    ...
    {
      /* Send URC if we have received any AT command to this port */
      if (TRUE == dsatcmdp_is_sio_info_inited(tx_port))
      {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_SIO_MSG1_MED( "ds3g_siolib_is_external_port_free: ds3g_siolib_port_allocated[i] %d ", 
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(ds3g_siolib_active_port == DS3G_SIOLIB_PORT_NONE)
  {
    DS_3GPP_ERROR_FATAL("siolib_active_port is DS3G_SIOLIB_PORT_NONE");
    return;
  }
...
DESCRIPTION   This function processes the DS_CMD_SIO_TIMER_EXPIRED. It 
              determines which timer has expired, and calls the appropriate
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(cmd_ptr->hdr.cmd_id != DS_CMD_SIO_TIMER_EXPIRED)
  {
  ...
  timer_expired_cmd_ptr =
    (ds3g_siolib_timer_exp_cmd_type*)cmd_ptr->cmd_payload_ptr;

  /*-------------------------------------------------------------------------
    Call the appropriate function, based on the timer that expired.
  -------------------------------------------------------------------------*/
  switch(timer_expired_cmd_ptr->timer_id)
  {
    ...
    default:
      DATA_3GMGR_MSG1(MSG_LEGACY_LOW, "Invalid timer_id %d",
                      timer_expired_cmd_ptr->timer_id);
      break;
  } /* switch( timer_expired_cmd_ptr->timer_id ) */
...
DESCRIPTION
  This function collects the mdump for DS3G SIO LIB variables
...
{
  qsh_mdump_collect_high(&ds3g_siolib_modem_port, sizeof(ds3g_siolib_port_e_type));
  qsh_mdump_collect_high(&ds3g_siolib_active_port, sizeof(ds3g_siolib_port_e_type));
  qsh_mdump_collect_high(&ds3g_siolib_tx_port, sizeof(ds3g_siolib_port_e_type));
  qsh_mdump_collect_high(&ds3g_siolib_dtr_sig_cntr, sizeof(uint8));
  qsh_mdump_collect_high(&ds3g_siolib_state, DS3G_SIOLIB_PORT_MAX * sizeof(ds3g_siolib_state_info_type));
}
#endif /* FEATURE_QSH_MDUMP */
