...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp/dsmgr/src/ds3g_qmi_nas_client.c#2 $
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if(!nas_svc_obj)
  {
    DS_3GPP_ERROR_FATAL("Unable to get NAS service object");
    return;
  }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  {
    DATA_3GMGR_MSG0(MSG_LEGACY_LOW,"Notifier release failed");
    ds3g_qmi_nas_client_notifier_handle = NULL;
    return;
  }
  ...
  if(QMI_NO_ERR != qmi_client_register_error_cb(ds3g_qmi_nas_client_handle,
                                                ...
                                                &ds3g_qmi_nas_client_client_cb_data))
  {
    DATA_3GMGR_MSG0(MSG_LEGACY_LOW,"QMI NAS register error callback failed");
  }
  ...
  {
    DS_3GPP_ERROR_FATAL("Couldn't get data_block_ptr memory");
    return;
  }
                               ...
                               &ind_reg_txn_hdl))
  {
    ...
    modem_mem_free(nas_get_sys_info_resp_msg,MODEM_MEM_CLIENT_DATA);
    DS_3GPP_ERROR_FATAL("Send NAS indication register request failed");
    return;
  }
  ...
  {
    DS_3GPP_ERROR_FATAL("Couldn't get data_block_ptr memory. Free mem 0xx");
    return;
  }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  {
    DATA_3GMGR_MSG0(MSG_LEGACY_LOW,"Invalid callback data");
    return;
  }
  ...
  if(msg_id == QMI_NAS_SYS_INFO_IND_MSG_V01)
  {
    ...
    if(sys_info_ind->gsm_srv_status_info_valid == 1)
    {
      if(sys_info_ind->gsm_srv_status_info.srv_status == SYS_SRV_STATUS_SRV)
      {
        ...
        if(sys_info_ind->gsm_sys_info_valid == 1)
        {
          ...
          if(sys_info_ind->gsm_sys_info.common_sys_info.srv_domain_valid == 1)
          {
            ...
            DATA_3GMGR_MSG6(MSG_LEGACY_LOW,
                            ...
                            " srv_status:%d, srv_domain:%d, rat_mask:%d",
                            DS_DSD_RADIO_GSM,
          ...
          else
          {
            ...
            ---------------------------------------------------------------------*/
            DATA_3GMGR_MSG0(MSG_LEGACY_LOW,"GSM srv domain invalid");
          }
        ...
        else
        {
          ...
          ---------------------------------------------------------------------*/
          DATA_3GMGR_MSG0(MSG_LEGACY_LOW,"GSM sys info invalid");
        }
      }/* if(srv_status == SYS_SRV_STATUS_SRV)*/
      else
      {
    ...
    else
    {
      ...
      ---------------------------------------------------------------------*/
      DATA_3GMGR_MSG0(MSG_LEGACY_LOW,"GSM service status info invalid");
    }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  {
    DATA_3GMGR_MSG2(MSG_LEGACY_LOW,"Invalid msg_id=%u. Free mem 0x%x",
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  {
    DATA_MSG0(MSG_LEGACY_LOW,"Invalid callback data");
    return;
  }
  ...
  switch(error)
  {
    ...
    case QMI_SERVICE_ERR:
      DATA_3GMGR_MSG0(MSG_LEGACY_LOW,"QMI NAS service is down");
      /*-----------------------------------------------------------------------
        Release the client.
      ------------------------------------------------------------------------*/
      if(QMI_NO_ERR != qmi_client_release(ds3g_qmi_nas_client_handle))
      {
        DATA_3GMGR_MSG0(MSG_LEGACY_LOW,"Client release failed");
        return;
      }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  DATA_3GMGR_MSG1(MSG_LEGACY_LOW, "dsd_sync_enabled: %d",
