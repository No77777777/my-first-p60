...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp/dsmgr/src/ds3gmmgsdiif.c#4 $ $DateTime: 2024/05/29 04:41:54 $ $Author: pwbldsvc $
...
/* IMSI_M info table */
static ds3g_mmgsdi_imsi_m_info_type
  ds3g_mmgsdi_imsi_m_info[DS3GSUBSMGR_SUBS_ID_MAX] = {{{NULL}}};

#ifdef FEATURE_EHRPD
/* ACL response object */
static ds3g_mmgsdi_acl_info_type ds3g_mmgsdi_acl_info[DS3GSUBSMGR_SUBS_ID_MAX] = {0};
#endif /* FEATURE_EHRPD */
...
/*----------------------------------------------------------------------------
  Cache for storing UE EFacc values for each GW session
-----------------------------------------------------------------------------*/
static uint32 ds3g_mmgsdi_efacc_class_value[MAX_DS3G_MMGSDI_SESSIONS] = {0};

/*----------------------------------------------------------------------------
  Cache that tracks when subscription ready indication comes in
  from both UIM and CM.
-----------------------------------------------------------------------------*/
...
boolean ds3g_mmgsdi_read_iccid
(
  mmgsdi_session_type_enum_type session_type
);
...
void ds3g_mmgsdi_read_iccid_from_session_type
(
  mmgsdi_session_type_enum_type session_type
);

boolean ds3g_mmgsdi_check_and_update_iccid_per_sub
(
...
void ds3g_mmgsdi_reset_mmgsdi_info
(
  uint8  index
);

void ds3g_mmgsdi_reset_and_notify
(
  uint8  index
);
...
/*===========================================================================
FUNCTION  DS3G_MMGSDI_GET_IMSI_M_INFO_FROM_SESSION_ID

DESCRIPTION
  This function fetches the IMSI_M info based on Session Id
...
SIDE EFFECTS
  None.
===========================================================================*/
ds3g_mmgsdi_imsi_m_info_type* ds3g_mmgsdi_get_imsi_m_info_from_session_id
(
  mmgsdi_session_id_type  session_id
);

/*===========================================================================
FUNCTION      DS3G_MMGSDI_GET_IMSI_INFO_FROM_SESSION_TYPE
...
/*===========================================================================
FUNCTION  DS3G_MMGSDI_GET_IMSI_M_INFO_FROM_SESSION_TYPE

DESCRIPTION
  This function fetches the IMSI_M info based on Session Type
...
SIDE EFFECTS
  None.
===========================================================================*/
ds3g_mmgsdi_imsi_m_info_type* ds3g_mmgsdi_get_imsi_m_info_from_session_type
(
  mmgsdi_session_type_enum_type  session_type
);
...
(
  ds3g_mmgsdi_event_info_type   *mmgsdi_event_info_ptr
);
...
FUNCTION DS3G_MMGSDI_NOTIFY_IMSI_M_INFO_EV

DESCRIPTION
  This function checks if valid IMSI_M info is availaible and notify
...
===========================================================================*/
void ds3g_mmgsdi_notify_imsi_m_info_ev
(
  sys_modem_as_id_e_type         subs_id,
  mmgsdi_app_enum_type           app_type,
  ds3g_mmgsdi_imsi_m_info_type  *imsi_m_info_ptr
);
...
FUNCTION DS3G_MMGSDI_RESET_IMSI_M_INFO

DESCRIPTION
  This function resets the IMSI_M info.
...
void ds3g_mmgsdi_reset_imsi_m_info
(
  sys_modem_as_id_e_type         subs_id,
  mmgsdi_app_enum_type           app_type,
  ds3g_mmgsdi_imsi_m_info_type  *imsi_m_info_ptr
...
#ifdef FEATURE_QSH_MDUMP
/*===========================================================================
FUNCTION ds3g_mmgsdi_qsh_mdump_collect
...
===========================================================================*/ 
QSH_MDUMP_FN_ATTR 
void ds3g_mmgsdi_qsh_mdump_collect()
{
  qsh_mdump_collect_high(&ds3g_mmgsdi_subscription_ready_status, 
  	MAX_DS3G_MMGSDI_SESSIONS * sizeof(ds3g_mmgsdi_subscription_ready_bit_mask_type));
}
...
)
{
  ...
  uint8                          i = 0;
  mmgsdi_session_type_enum_type    session_type[MAX_DS3G_MMGSDI_SESSIONS] =
  ...
  mmgsdi_session_type_enum_type    isim_session_type[MAX_DS3G_MMGSDI_NON_PROV_SESSIONS] =
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  for ( i = 0; i < MAX_DS3G_MMGSDI_SESSIONS; i++ )
  {
    ...
    ds3g_mmgsdi_info.ds3g_mmgsdi_session_info[i].iccid_len = 0xFF;
    memset( ds3g_mmgsdi_info.ds3g_mmgsdi_session_info[i].iccid,
            0,
  ...
  for (i=0; i< DS3GSUBSMGR_SUBS_ID_MAX; i++)
  {
    memset(ds3g_mmgsdi_imsi_m_info[i].imsi_m_response,0,DS3G_MMGSDI_IMSI_M_LEN);

    ds3g_mmgsdi_imsi_m_info[i].ready_status = FALSE;
  }
  ...
  for (i=0; i< DS3GSUBSMGR_SUBS_ID_MAX; i++)
  {
    ds3g_mmgsdi_acl_info[i].acl_response = NULL;
    ds3g_mmgsdi_acl_info[i].acl_response_len = 0;
  }
  ...
  for (i=0; i< DS3GSUBSMGR_SUBS_ID_MAX; i++)
  {
    ds3g_mmgsdi_acl_info[i].acl_read_from_usim = ds3g_get_ehrpd_auth_in_usim(i);
  /*---------------------------------------------------
    If acl read mode is S/W then don't read ACL from
    card and mark the initialization flag to TRUE.
  ---------------------------------------------------*/
    if( ds3g_mmgsdi_acl_info[i].acl_read_from_usim == FALSE )
    {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (NULL == cnf_ptr)
  {
    DS_3GPP_ERROR_FATAL("cnf_ptr is NULL");
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  cm_subs_id = ds3g_mmgsdi_get_sub_id_from_session_id(event->session_id);
...
/*===========================================================================
FUNCTION DS3G_MMGSDI_PROCESS_SUBS_READY_ACTIONS

DESCRIPTION
  Function to process all of the actions required once both subscription
  ready event and subs id info has been received for USIM session to process.
...
===========================================================================*/
void ds3g_mmgsdi_process_subs_ready_actions
(
  ds3g_mmgsdi_event_info_type   *mmgsdi_event_info_ptr
)
{
  ...
  if(ds3gsubsmgr_is_ds3g_subs_index_valid(ds3g_subs_id) && 
     (ds3g_mmgsdi_acl_info[ds3g_subs_id].acl_read_from_usim == TRUE))
  {
  ...
  if (MMGSDI_SUCCESS == ret_val)
  {
    ...
    {
      DATA_3GMGR_MSG0(MSG_LEGACY_LOW, "ICCID hasn't changed");
    }
  }
  else
  {
  ...
  #endif /* FEATURE_DATA_RPM */
  ds3g_mmgsdi_read_iccid_from_session_type(session_type);
  #ifdef DATA_PS_DATA_OFF
  ds3g_mmgsdi_read_efpsdataoff(session_type);
  #endif

  return;
} /* ds3g_mmgsdi_process_subs_ready_actions */
/*===========================================================================
...
)
{
 ...
 /*- - - - - - - - - - - - - - -- - - - - -- - - - - - - - - - - - - - - */
  ...
  {
    DATA_3GMGR_MSG1(MSG_LEGACY_LOW,"Invalid ds3g mmgsdi status usim mask %d ",
                    mask);
...
func_exit:
  ...
  {
    DS_3GPP_ERROR_LOG_1_EX(subs_id, "ds3g_mmgsdi_subs_ready_to_process_usim: err d",
                       local_err);
  }
  return ret_val;
...
/*===========================================================================
FUNCTION DS3G_MMGSDI_SUBS_READY_TO_PROCESS_CSIM

DESCRIPTION
  Function to determine if both subscription ready event and subs id info
  has been received for CSIM session to process.
...
===========================================================================*/
boolean ds3g_mmgsdi_subs_ready_to_process_csim
(
  uint8                          mask,
  mmgsdi_session_type_enum_type  session_type
)
...
/*- - - - - - - - - - - - - - -- - - - - -- - - - - - - - - - - - - - - */
  ...
  DATA_3GMGR_MSG2_EX(MSG_LEGACY_MED,
          subs_id,
          "ds3g_mmgsdi_subs_ready_to_process_csim, mask %d session type %d",
          mask, session_type);

  if ((mask != DS3G_MMGSDI_CSIM_SUBSCRIPTION_READY_MASK) &&
      (mask != DS3G_MMGSDI_CSIM_SUBS_ID_READY_MASK))
  {
    DS_3GMGR_ERROR_LOG_1_EX(MSG_LEGACY_ERROR, subs_id,
                    "Invalid ds3g mmgsdi status csim mask d ",
                    mask);
    return FALSE;
  }
    ...
    )
  {
    ...
    ready_mask = DS3G_MMGSDI_CSIM_SUBSCRIPTION_READY_MASK |
                 DS3G_MMGSDI_CSIM_SUBS_ID_READY_MASK;
  ...
  return FALSE;
} /* ds3g_mmgsdi_subs_ready_to_process_csim */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS3GPP_ASSERT(NULL != cmd_ptr);
  DS3GPP_ASSERT(NULL != cmd_ptr->cmd_payload_ptr);
  ...
  if ( ds3g_mmgsdi_get_session_id_idx( session_id, &index ) == TRUE )
  {
     ...
     switch ( session_type )
     {
       ...
       case MMGSDI_1X_PROV_TER_SESSION:
         ...
         if(ds3g_mmgsdi_subs_ready_to_process_csim(
                           DS3G_MMGSDI_CSIM_SUBSCRIPTION_READY_MASK,
                           session_type) == FALSE)
         {
           DATA_3GMGR_MSG0_EX(MSG_LEGACY_HIGH,
                              subs_id,
                              "subs not ready to process csim session");
         }
      ...
      case MMGSDI_GW_PROV_TER_SESSION:
         ...
         /* Ensure that any logic that require both the USIM to be ready AND the
            SUB ID to be updated gets added into the ds3g_mmgsdi_process_subs_ready_actions
            function call.  The rest of the logic can still continue since it 
            only relies on the session and not the sub id*/
         if(ds3g_mmgsdi_subs_ready_to_process_usim(
                           DS3G_MMGSDI_USIM_SUBSCRIPTION_READY_MASK,
                           session_type) == TRUE)
         {
           ds3g_mmgsdi_process_subs_ready_actions(mmgsdi_event_info_ptr);
         }
         else
         {
           DATA_3GMGR_MSG0_EX(MSG_LEGACY_HIGH,
        ...
        /* Below actions rely only on the session and don't care whether sub id
           is updated or not */
        /* Read register for file refresh */
         ds3g_mmgsdi_3gpp_register_for_refresh(session_type);
...
)
{
  ...
  mmgsdi_refresh_enforcement_policy_type *refresh_enf_pol_ptr = NULL;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /*  No need to vote NOK for emergency calls by DS since it will be handled
      by CM */

  if( mmgsdi_event_info_ptr->source == MMGSDI_REFRESH_SOURCE_ECALL )
  {
    /* Voting OK to Refresh if source for refresh is ECALL */
    voting_ok_to_refresh_flag = TRUE;
    voting_result = TRUE;
  }
  else if( refresh_enf_pol_ptr->is_enforcement_present == TRUE )
  {
	...
	/* voting OK for voice calls as per VZW REQ if criteria met*/
	if( (refresh_enf_pol_ptr->data.force_refresh_voice_call == TRUE) && 
		 ds_3gpp_enforce_vote_criteria_for_voice_call(cm_subs_id))
    {
      voting_ok_to_refresh_flag = TRUE;
      voting_result = TRUE;
    }
  }
  else if( ds3g_get_refresh_vote_ok_info(cm_subs_id) == TRUE )
  {
  ...
  /* If voting_ok_to_refresh flag is set due to any of the above conditions,
     then vote now. Else fallback to legacy handling */
  if( voting_ok_to_refresh_flag )
  {
    mmgsdi_status = ds3g_mmgsdi_ok_to_refresh( voting_result, session_type );
    if (mmgsdi_status != MMGSDI_SUCCESS)
    {
      DATA_3GMGR_MSG2(MSG_LEGACY_LOW,
        ...
        switch( fcn_indication.file_list[index] )
        {
          ...
          case MMGSDI_USIM_ACL:
            ...
            /*-----------------------------------------------------
              ...
              from card.
            -----------------------------------------------------*/
            if( ds3g_get_ehrpd_auth_in_usim(subs_id) == TRUE )
            {
              /* Check if ACL service is enabled, and then read ACL  */
...
)
{
  ...
  ds3g_mmgsdi_imsi_m_info_type  *imsi_m_info_ptr = NULL;
  ds3g_mmgsdi_event_info_type  *mmgsdi_event_info_ptr = NULL;
  mmgsdi_session_id_type         session_id;
  mmgsdi_app_enum_type           app_type;
       ...
       ( MMGSDI_READ_CNF == read_cnf_ptr->response_header.response_type ) )
  {
    ...
    -----------------------------------------------------------------------*/
      ...
      else
      {
        ...
        event_type = DS3G_MMGSDI_IMSI_INFO_EVENT;
      }

      else if ( MMGSDI_APP_CSIM == app_type || MMGSDI_APP_RUIM == app_type)
      {
        imsi_m_info_ptr = ds3g_mmgsdi_get_imsi_m_info_from_session_id
                          (session_id);
        if ( NULL == imsi_m_info_ptr)
        {
          DATA_3GMGR_MSG0(MSG_LEGACY_LOW, "IMSI_M info ptr is NULL ");
          return;
        }

        /* Initialize the IMSI_M info table */
        memset( imsi_m_info_ptr->imsi_m_response,
                0,
                DS3G_MMGSDI_IMSI_M_LEN );
        /*-----------------------------------------------------------------------
          The reading from IMSI_M efs was successful.  This copies the data from
          the mmgsdi response
        -----------------------------------------------------------------------*/
        if ( read_cnf_ptr->read_data.data_len <= DS3G_MMGSDI_IMSI_M_LEN )
        {
          memscpy( imsi_m_info_ptr->imsi_m_response,
                   DS3G_MMGSDI_IMSI_M_LEN,
                   read_cnf_ptr->read_data.data_ptr,
                   read_cnf_ptr->read_data.data_len );
        }
        ...
        event_type = DS3G_MMGSDI_IMSI_M_INFO_EVENT;
      }
      else
      {
        DATA_3GMGR_MSG1(MSG_LEGACY_LOW, "Invalid app type %d",app_type);
        return;
      }
      ...
      mmgsdi_event_info_ptr->event_type = event_type;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if( NULL == cnf_ptr )
  {
    DATA_3GMGR_MSG0(MSG_LEGACY_LOW, "ds3g_mmgsdi_get_iccid_cb: cnf_ptr NULL");
    return;
  }
       ...
       ( MMGSDI_READ_CNF == read_cnf_ptr->response_header.response_type ) )
  {
         ...
         ( MMGSDI_SUCCESS == read_cnf_ptr->response_header.mmgsdi_status ) )
    {
      ...
      {   
        ...
        {
          memscpy( ds3g_mmgsdi_info.ds3g_mmgsdi_session_info[index].iccid,
                   DS3G_MMGSDI_ICCID_LEN,
                   read_cnf_ptr->read_data.data_ptr,
                   read_cnf_ptr->read_data.data_len );
          ds3g_mmgsdi_info.ds3g_mmgsdi_session_info[index].iccid_len = 
          read_cnf_ptr->read_data.data_len;
        }
        else
        {
      ...
      {
        DATA_3GMGR_MSG0(MSG_LEGACY_LOW,"session_id not matched");
        return;
      }
...
FUNCTION  DS3G_MMGSDI_IMSI_M_INFO_EVENT_PROCESS

DESCRIPTION
  This function is the handler function for DS3G_MMGSDI_IMSI_M_INFO_EVENTS
...
===========================================================================*/
void ds3g_mmgsdi_imsi_m_info_event_process
(
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  subs_id = ds3g_mmgsdi_get_sub_id_from_session_id(session_id);
  imsi_m_info_ptr = ds3g_mmgsdi_get_imsi_m_info_from_session_id(session_id);
  if ( imsi_m_info_ptr != NULL )
  {
    imsi_m_info_ptr->ready_status = TRUE;
    ds3g_mmgsdi_notify_imsi_m_info_ev(subs_id,app_type,imsi_m_info_ptr);
  }
} /* ds3g_mmgsdi_imsi_m_info_event_process */

/*===========================================================================
FUNCTION  DS3G_MMGSDI_ICCID_INFO_EVENT_PROCESS

DESCRIPTION
  This function is the handler function for DS3G_MMGSDI_ICCID_INFO_EVENT
...
===========================================================================*/
void ds3g_mmgsdi_iccid_info_event_process
(
  ds_cmd_type  *cmd_ptr
)
{
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if( ds3g_mmgsdi_get_session_id_idx( session_id, &index ) == TRUE )
  {
    ...
    event_info.iccid_info.iccid_len = 
      ds3g_mmgsdi_info.ds3g_mmgsdi_session_info[index].iccid_len;
	
    memscpy( event_info.iccid_info.iccid,
             event_info.iccid_info.iccid_len,
             ds3g_mmgsdi_info.ds3g_mmgsdi_session_info[index].iccid,
             ds3g_mmgsdi_info.ds3g_mmgsdi_session_info[index].iccid_len );

    event_info.iccid_info.is_valid = TRUE;
    event_info.iccid_info.subs_id = 
    ...
    if(ds3geventmgr_set_event_info(DS3GEVENTMGR_MMGSDI_ICCID_INFO_EV,
                                   (void *)&event_info,&filter_info) == FALSE)
    {
      DATA_3GMGR_MSG0(MSG_LEGACY_LOW,
                      "Unable to provide ICCID info to registered clients");
    }
  }
  else
  {
  ...
  return;
}/* ds3g_mmgsdi_iccid_info_event_process */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DATA_3GMGR_MSG0_EX(MSG_LEGACY_MED, 
                     subs_id, 
                     "Sending Request for Reading IMSI");
...
/*==============================================================================
FUNCTION  DS3G_MMGSDI_READ_ICCID

DESCRIPTION
  Get ICCID info after receiving Subscription Ready Events from GW sessions
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DATA_3GMGR_MSG0_EX(MSG_LEGACY_MED, 
                     subs_id,
                     "Sending Request for Reading ICCID");
  ...
  /* Read the data */
  mmgsdi_status = ds3g_mmgsdi_read_transparent( iccid_access,
  ...
  {
    DATA_3GMGR_MSG2(MSG_LEGACY_LOW,
                    "Failed to read ICCID using session type [%d], status [%d]",
                    session_type, mmgsdi_status);
    return FALSE;
  }

  return TRUE;
}/* ds3g_mmgsdi_read_iccid */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  subs_id = ds3g_mmgsdi_get_sub_id_from_session_type(non_prov_session_type);
  ...
  DATA_3GMGR_MSG0_EX(MSG_LEGACY_MED,
                     subs_id,
                     "Sending Request for Reading IMPI");
...
)
{
  ...
  DATA_3GMGR_MSG2_EX(MSG_LEGACY_MED,
                     subs_id,
                     "App type[%d] Session id =[%d] from the table",
                     app_type, 
                     session_id);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_3GMGR_ERROR_LOG_2_EX(MSG_LEGACY_ERROR, subs_id, "Improper command confirmation cnf [d] "
                    "response_type [d]",
                    cnf,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (!ds3g_mmgsdi_get_arr_index_from_session_type(session_type,&index))
  {
    DATA_3GMGR_MSG1(MSG_LEGACY_LOW,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/  
  ...
  if (efacc_class == NULL)
  {
    DATA_3GMGR_MSG0(MSG_LEGACY_LOW,"EFACC CLASS PTR is NULL");
    local_err = DS_LOCAL_ERR_VAL_0;
    goto func_exit;
  }
  ...
  *efacc_class = 0;
  ...
  if(session_type == MMGSDI_MAX_SESSION_TYPE_ENUM )
   {
     DATA_3GMGR_MSG0(MSG_LEGACY_LOW,
                     "Invalid Session type (MMGSDI_MAX_SESSION_TYPE_ENUM)");
     local_err = DS_LOCAL_ERR_VAL_2;
     goto func_exit;
   }
  ...
  {
    DATA_3GMGR_MSG1(MSG_LEGACY_LOW,
                    "Error: Invalid session type: 0x%x",
                    session_type);
  ...
  if((ds3g_mmgsdi_efacc_class_value[index] & ACC_CLASS_11_15_MASK) != 0) 
  {
    DATA_3GMGR_MSG2_EX(MSG_LEGACY_HIGH,
                      subs_id,
                      "efacc class is enabled for session type [%d] "
                      "and efacc class %d",
                      session_type, 
                      ds3g_mmgsdi_efacc_class_value[index]);
...
func_exit:
  if(local_err != DS_LOCAL_ERR_VAL_NONE)
  {
    DS_3GPP_ERROR_LOG_1_EX(subs_id, "ds3g_mmgsdi_get_efacc_class: err d",
                       local_err);
  }
  return ret_val;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(session_type == MMGSDI_MAX_SESSION_TYPE_ENUM )
  {
    DS_3GPP_ERROR_LOG_1_EX(subs_id, "Invalid Session type:d", session_type);
    return FALSE;
  }
...
SIDE EFFECTS
  None.
===========================================================================*/
void ds3g_mmgsdi_3gpp_register_for_refresh
(
  mmgsdi_session_type_enum_type session_type
)
{
  ...
  subs_id = ds3g_mmgsdi_get_sub_id_from_session_type(session_type);
  DATA_3GMGR_MSG1_EX(MSG_LEGACY_MED,subs_id,"app_type:%d",app_type);
  
  if ( app_type == MMGSDI_APP_USIM )
  {
    DATA_3GMGR_MSG0(MSG_LEGACY_LOW, "Registers UMTS file change from USIM");
    file_list[0] = MMGSDI_USIM_ACC; /* EF for Access  Control Class */
#ifdef DATA_PS_DATA_OFF
    file_list[1] = MMGSDI_USIM_PS_DATA_OFF; /* EF for PS Data Off */    
  ...
  {
    DATA_3GMGR_MSG0(MSG_LEGACY_LOW, "Registers UMTS file change from USIM");
    file_list[0] = MMGSDI_GSM_ACC; /* EF for Access Control Class */
    refresh_files.num_files = 1;
  }
  else
  {
...
}/* ds3g_mmgsdi_3gpp_register_for_refresh */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
    ...
    else
  {
      DATA_3GMGR_MSG1_EX(MSG_LEGACY_HIGH, 
                        subs_id,
                        "EF-AD has invalid MNC length: 0x%x",
          ...
          impi_info_ptr->impi_info.data_ptr[idx] != '@')
  {
         ...
         impi_info_ptr->impi_info.data_ptr[idx] - '0' > 9 ) 
    {
      DATA_3GMGR_MSG1_EX(MSG_LEGACY_HIGH, 
                         ...
                         "impi data is not a digit, idx is %d", 
                         idx);
      return FALSE;
    }
    idx++; 
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (!ds3g_mmgsdi_check_if_impi_contain_imsi(impi_info_ptr, &imsi_len, subs_id))
  {
    DATA_3GMGR_MSG0_EX(MSG_LEGACY_HIGH, 
                       subs_id,
                       "ds3g_mmgsdi_format_imsi_from_impi, "
                       "impi does not contain imsi" );
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  else
  {
    ...
    else
    {
      DATA_3GMGR_MSG0_EX(MSG_LEGACY_HIGH, 
                         subs_id,
                         "IMSI data is not ready for querying");
      return FALSE;
    }
  }
  return TRUE;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  else
  {
    ...
    else
    {
      DATA_3GMGR_MSG0_EX(MSG_LEGACY_HIGH, 
                         subs_id,
                         "IMPI data is not ready for querying");
      return FALSE;
    }
  }
  return TRUE;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
         ...
         ds3g_mmgsdi_info.ds3g_mmgsdi_session_info[index].session_id) )
    {
      ...
      switch ( session_type )
      {
        ...
        case MMGSDI_1X_PROV_TER_SESSION:
          ...
          /* Invalidate the IMSI_M  */

          imsi_m_info_ptr = ds3g_mmgsdi_get_imsi_m_info_from_session_type (session_type);

          if (imsi_m_info_ptr == NULL)
          {
            DATA_3GMGR_MSG1(MSG_LEGACY_LOW,
                          "IMSI_M info ptr is NULL for session type[%d]",
                          session_type);
            return;
          }
          ...
          if (ds3gsubsmgr_is_subs_id_valid(subs_id))
          {
            ds3g_mmgsdi_reset_imsi_m_info(subs_id,
                                          mmgsdi_event_info_ptr->app_type,
                                          imsi_m_info_ptr);
          }
        ...
        case MMGSDI_GW_PROV_TER_SESSION:
          ...
          if (ds3gsubsmgr_is_ds3g_subs_index_valid(subs_indx))
          {
            ...
            {
#ifdef FEATURE_EHRPD
              ds3g_mmgsdi_acl_info[subs_indx].acl_response = NULL;
              ds3g_mmgsdi_acl_info[subs_indx].acl_response_len = 0;
#endif /* FEATURE_EHRPD */
            }
          }
  ...
  return;
} /* ds3g_mmgsdi_card_refresh_event_process */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  for ( index = 0; index < MAX_DS3G_MMGSDI_SESSIONS; index++ )
  {
      ...
      switch ( session_type )
      {
        ...
        case MMGSDI_GW_PROV_TER_SESSION:
          ...
          if (ds3gsubsmgr_is_ds3g_subs_index_valid(subs_indx))
          {
            ...
            ds3g_mmgsdi_reset_efpsdataoff(subs_id);
#ifdef FEATURE_EHRPD
            ds3g_mmgsdi_acl_info[subs_indx].acl_response = NULL;
            ds3g_mmgsdi_acl_info[subs_indx].acl_response_len = 0;
#endif /* FEATURE_EHRPD */
	...
	/*-------------------------------------------------------------------
          Gererate the DS3GEVENTMGR_MMGSDI_ICCID_INFO_EV
        -------------------------------------------------------------------*/
         ds3g_mmgsdi_reset_and_notify(index);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch( mmgsdi_event_info_ptr->event_type )
  {
    ...
    case DS3G_MMGSDI_SUBSCRIPTION_READY_EVENT:
      ...
      ds3g_mmgsdi_reset_cc_sp_throttle_if_required( mmgsdi_event_info_ptr );
      break;
    ...
    case DS3G_MMGSDI_ICCID_INFO_EVENT:
      ds3g_mmgsdi_iccid_info_event_process( cmd_ptr );
      break;

    case DS3G_MMGSDI_IMSI_M_INFO_EVENT:
      ds3g_mmgsdi_imsi_m_info_event_process( cmd_ptr );
      break;

    case DS3G_MMGSDI_USIM_EFPSDATAOFF_READ_CNF_EVENT:
      ds3g_mmgsdi_efpsdataoff_update_cache( cmd_ptr );
      break;

    default:
      DATA_3GMGR_MSG1(MSG_LEGACY_LOW,
                      "Unrecognized event type [%d] rcvd",
                      mmgsdi_event_info_ptr->event_type);
      break;
  }
  return;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
       ...
       ( MMGSDI_WRITE_CNF == cnf_ptr->write_cnf.response_header.response_type ) )
  {
         ...
         ( MMGSDI_SUCCESS == cnf_ptr->write_cnf.response_header.mmgsdi_status ) )
    {
      ...
      DATA_3GMGR_MSG2_EX(MSG_LEGACY_MED,
                         subs_id,
                         "ds3g_mmgsdi_write_transparent_cb client ref:%d "
                         "status:%d",
                         cnf, 
                         status);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
         ...
         cnf_ptr->srv_available_cnf.response_header.response_type ) )
  {
           ...
           cnf_ptr->srv_available_cnf.response_header.mmgsdi_status ) )
    {
      ...
      DATA_3GMGR_MSG2_EX(MSG_LEGACY_MED,
                         subs_id,
                         "ds3g_mmgsdi_service_available_cb client ref:%d "
                         "status:%d",
                         cnf, 
                         status);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DATA_3GMGR_MSG2_EX(MSG_LEGACY_MED, 
                     subs_id,
                     "Sending Read Transparent on Session "
                     "Type [%d]for Session Id [%d] ", 
                     session_type, 
                     session_id);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if( (( subs_id == SYS_MODEM_AS_ID_NONE ) ||
      ...
      ( ds3gsubsmgr_is_device_multi_sim_mode() == TRUE))
  {
...
)
{
  mmgsdi_return_enum_type  mmgsdi_status = MMGSDI_ERROR;
  mmgsdi_session_id_type   session_id = DS3G_MMGSDI_INVALID_SESSION_ID;
  rex_sigs_type            sig_mask[PS_MAX_SIG_ARR_LEN];
  sys_modem_as_id_e_type   subs_id = SYS_MODEM_AS_ID_NONE;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  /* check client id */
  if ( ds3g_mmgsdi_info.client_id == DS3G_MMGSDI_INVALID_CLIENT_ID )
  ...
  /* Get the corresponding session id for session type */
  if ( ds3g_mmgsdi_get_session_id( session_type, &session_id )
         == TRUE )
  {
    DATA_3GMGR_MSG2(MSG_LEGACY_LOW,
  ...
  {
    /* Try to use primary 1X as fallback. */
    if ( ds3g_mmgsdi_get_session_id( MMGSDI_1X_PROV_PRI_SESSION , &session_id )
           == TRUE )
      {
        DATA_3GMGR_MSG2(MSG_LEGACY_LOW,
                      "Can't find session id for session_type %d but using session id [%d] for 1X_PROV_PRI",
                      session_id, session_type );
    }
    else
    {
        DATA_3GMGR_MSG1(MSG_LEGACY_LOW,
                        "Failed to find a session id using session type [%d] nor 1X_PROV_PRI",
                        session_type);
        return mmgsdi_status;
    }
  }
  ...
  PS_SET_SIG(sig_mask, PS_UIM_CMD_SIGNAL);
  (void)PS_WAIT(sig_mask);                       /* Wait for completion*/
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_3GPP_ERROR_FATAL("RCINIT Lookup for ps tcb returned NULL");
  }
#else
  ps_tcb_ptr = &ps_tcb;
#endif
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_3GMGR_ERROR_LOG_1(MSG_LEGACY_ERROR, "Invalid client Id:d", client_id);
    return FALSE;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(!ret_val)
  {
    DS_3GMGR_ERROR_LOG_1_EX(MSG_LEGACY_ERROR, subs_id, "session_type:d not found",
                    session_type);
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  subs_id = ds3g_mmgsdi_get_sub_id_from_session_type(*session_type_ptr);
  ...
  if(!ret_val)
  {
    DS_3GMGR_ERROR_LOG_1_EX(MSG_LEGACY_ERROR, subs_id, "session_id:d not found", session_id);
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DATA_3GMGR_MSG0_EX(MSG_LEGACY_MED, 
                     subs_id,
                     "Sending Request for Reading ACL");
  ...
  if(ds3gsubsmgr_is_subs_id_valid(subs_id) && 
      (ds3g_mmgsdi_acl_info[subs_id].acl_response != NULL))
  {
    modem_mem_free( ds3g_mmgsdi_acl_info[subs_id].acl_response,
                       MODEM_MEM_CLIENT_DATA_CRIT );
    ds3g_mmgsdi_acl_info[subs_id].acl_response_len = 0;
    ds707_acl_info = ds707_data_session_get_acl_info_ptr();
    ds707_acl_info->num_apn = -1;
  }
  /* Read the data */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DATA_3GMGR_MSG0_EX(MSG_LEGACY_MED, 
                     subs_id,
                     "Sending Request for App Capabilities");
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
         ...
         read_cnf_ptr->response_header.response_type ) )
  {
         ...
         ( MMGSDI_SUCCESS == read_cnf_ptr->response_header.mmgsdi_status ) )
    {
      ...
      ds3g_mmgsdi_acl_info[subs_id].acl_response =
        modem_mem_alloc( read_cnf_ptr->read_data.data_len,
                         MODEM_MEM_CLIENT_DATA_CRIT );

      if ( ds3g_mmgsdi_acl_info[subs_id].acl_response == NULL )
      {
      ...
      memset( ds3g_mmgsdi_acl_info[subs_id].acl_response,
              0,
              read_cnf_ptr->read_data.data_len );
    /*-----------------------------------------------------------------------
      ...
      the mmgsdi response
    -----------------------------------------------------------------------*/
     memscpy( ds3g_mmgsdi_acl_info[subs_id].acl_response,
              read_cnf_ptr->read_data.data_len,
              read_cnf_ptr->read_data.data_ptr,
              read_cnf_ptr->read_data.data_len );
     ...
     -----------------------------------------------------------------------*/
      ds3g_mmgsdi_acl_info[subs_id].acl_response_len =
         read_cnf_ptr->read_data.data_len;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
         ...
         app_capabilities_cnf_ptr->response_header.response_type ) )
  {
           ...
           app_capabilities_cnf_ptr->response_header.mmgsdi_status ) )
    {
      ...
      ds3g_mmgsdi_acl_info[subs_id].acl_is_enabled =
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if( ds3g_mmgsdi_acl_info[subs_id].acl_response == NULL )
  {
    DATA_3GMGR_MSG1(MSG_LEGACY_FATAL, 
                    "ds3g_mmgsdi_parse_acl(): Unexpected "
                    "ds3g_mmgsdi_acl_info[%d].acl_response is NULL ",
                    subs_id);
...
/*lint -save -e661 suppressing error 661*/
  if( ds3g_mmgsdi_acl_info[subs_id].acl_response_len > 0 )
  {
    /* Extract number of APNs on ACL */
    num_apn = ds3g_mmgsdi_acl_info[subs_id].acl_response[offset];
    offset += sizeof(uint8);
    ...
    for( apn_counter = 0; apn_counter < num_apn; apn_counter++ )
    {
      ...
      /* Extract type field */
      if( ds3g_mmgsdi_acl_info[subs_id].acl_response[offset] != 0xDD )
      {
        DATA_3GMGR_MSG1(MSG_LEGACY_LOW,
                        "APN TLV has invalid Tag/Type Value: 0x%x, should be "
                        "0xDD to indicate start of new APN",
                        ds3g_mmgsdi_acl_info[subs_id].acl_response[offset]);
        parse_error = TRUE;
        local_err = DS_LOCAL_ERR_VAL_0;
        break;
      }
      ...
      apn_len = ds3g_mmgsdi_acl_info[subs_id].acl_response[offset];
      offset += sizeof(uint8);
      ...
      else if( (uint32)(apn_len + offset) > (uint32)ds3g_mmgsdi_acl_info[subs_id].acl_response_len )
      {
        DATA_3GMGR_MSG0(MSG_LEGACY_LOW,
      ...
      {
        DATA_3GMGR_MSG0(MSG_LEGACY_FATAL,
    ...
    if( parse_error )
    {
      DS_3GMGR_ERROR_LOG_0(MSG_LEGACY_ERROR, "ds3g_mmgsdi_parse_acl(): "
...
/*lint -restore Restore lint error 661*/
  ...
  if(local_err != DS_LOCAL_ERR_VAL_NONE)
  {
    DS_3GPP_ERROR_LOG_1("ds3g_mmgsdi_parse_acl: err d",
                        local_err);
   }
  return;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if( ds3g_mmgsdi_acl_info[subs_id].acl_is_enabled == TRUE )
  {
    /* Read ACL from USIM */
...
{
  uint8 i;
  ds3g_mmgsdi_event_info_type   mmgsdi_event_info;
  sys_modem_as_id_e_type        prev_subs_id;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

  memset(&mmgsdi_event_info, 0, sizeof(ds3g_mmgsdi_event_info_type));
  ...
  for ( i = 0; i < MAX_DS3G_MMGSDI_SESSIONS; i++ )
  {
    if ( ds3g_mmgsdi_info.ds3g_mmgsdi_session_info[i].session_type
          == session_type )
    {
      prev_subs_id = ds3g_mmgsdi_info.ds3g_mmgsdi_session_info[i].sub_id;
      ds3g_mmgsdi_info.ds3g_mmgsdi_session_info[i].sub_id = sub_id;
      mmgsdi_event_info.session_id = ds3g_mmgsdi_info.ds3g_mmgsdi_session_info[i].session_id;
      mmgsdi_event_info.slot = ds3g_mmgsdi_info.ds3g_mmgsdi_session_info[i].slot;
      mmgsdi_event_info.app_type = ds3g_mmgsdi_info.ds3g_mmgsdi_session_info[i].app_type;
      mmgsdi_event_info.event_type = DS3G_MMGSDI_SUBSCRIPTION_READY_EVENT;
      DATA_3GMGR_MSG4(MSG_LEGACY_LOW, 
                      "Subscription ID stored. Sub_id = %d, "
                      "Session_id = %d, session_type = %d, slot = %d", 
                      sub_id, 
                      ds3g_mmgsdi_info.ds3g_mmgsdi_session_info[i].session_id,
                      ds3g_mmgsdi_info.ds3g_mmgsdi_session_info[i].session_type,
                      ds3g_mmgsdi_info.ds3g_mmgsdi_session_info[i].slot
                     );
      ...
      if(ds3g_mmgsdi_subs_ready_to_process_usim(
         ...
         == TRUE)
      {
        ds3g_mmgsdi_process_subs_ready_actions(&mmgsdi_event_info);
      }
      else
      {
        DATA_3GMGR_MSG0_EX(MSG_LEGACY_HIGH,
                          sub_id, 
                          "ds3g_mmgsdi_update_sub_id - "
                          "subs not ready to process usim session");
        }

      /* Rest of the logic below does not care whether SUBSCRIPTION_READY
         has been received yet or not.  It only cares about the 
         sub id */
      if(ds3g_mmgsdi_subs_ready_to_process_csim(
                           DS3G_MMGSDI_CSIM_SUBS_ID_READY_MASK,
                           session_type) == FALSE)
      ...
      if (prev_subs_id != sub_id)
      {
        ds3g_mmgsdi_reset_cc_sp_throttle_if_required(&mmgsdi_event_info);
      }
      return TRUE;
    }
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if( !ret_val )
  {
    DS_3GMGR_ERROR_LOG_1_EX(MSG_LEGACY_ERROR, sub_id,
...
/*===========================================================================
FUNCTION DS3G_MMGSDI_GET_CDMA_SESSION_TYPE_FROM_SUB_ID

DESCRIPTION
  Get the cdma session type from mmgsdi table provided subscription id
...
SIDE EFFECTS
  None
===========================================================================*/
boolean ds3g_mmgsdi_get_cdma_session_type_from_sub_id
(
  sys_modem_as_id_e_type         sub_id,
  mmgsdi_session_type_enum_type* session_type_ptr
)
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ( NULL == session_type_ptr )
  {
    DS_3GMGR_ERROR_LOG_0_EX(MSG_LEGACY_ERROR, sub_id,
                    "ds3g_mmgsdi_get_cdma_session_type_from_sub_id invalid inputs");
    return ret_val;
  }
  ...
  {
    if (( ds3g_mmgsdi_info.ds3g_mmgsdi_session_info[i].sub_id == sub_id ) &&
        ( ds3g_mmgsdi_info.ds3g_mmgsdi_session_info[i].session_type == MMGSDI_1X_PROV_PRI_SESSION 
          ||
          ds3g_mmgsdi_info.ds3g_mmgsdi_session_info[i].session_type == MMGSDI_1X_PROV_SEC_SESSION 
#ifdef FEATURE_TRIPLE_SIM 
          || 
          ds3g_mmgsdi_info.ds3g_mmgsdi_session_info[i].session_type == MMGSDI_1X_PROV_TER_SESSION 
#endif /* FEATURE_TRIPLE_SIM */
          ) &&
        ( ds3g_mmgsdi_info.ds3g_mmgsdi_session_info[i].is_operational ))
    {
      *session_type_ptr  = ds3g_mmgsdi_info.ds3g_mmgsdi_session_info[i].session_type;
      ret_val = TRUE;
      break;
    }
  }
  ...
  {
    DS_3GMGR_ERROR_LOG_1_EX(MSG_LEGACY_ERROR, sub_id,
                "ds3g_mmgsdi_get_cdma_session_type_from_sub_id sub_id:d session not found",
                sub_id);
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DATA_3GMGR_MSG2_EX(MSG_LEGACY_MED,
                     subs_id,
                     "Session Id: %d,IMSI Info: %0xx",
                     session_id, 
                     &ds3g_mmgsdi_imsi_info[subs_id]);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DATA_3GMGR_MSG2_EX(MSG_LEGACY_MED,
                     ...
                     "Session Id: %d,IMSI Info: %0xx",
                     session_id, 
                     &ds3g_mmgsdi_imsi_m_info[subs_id]);

  return &ds3g_mmgsdi_imsi_m_info[subs_id];
} /* ds3g_mmgsdi_get_imsi_m_info_from_session_id */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DATA_3GMGR_MSG3(MSG_LEGACY_LOW,
                  "Subs Id: %d,Session_type: %d,IMSI Info: 0x%x",
                  subs_id, session_type, &ds3g_mmgsdi_imsi_m_info[subs_id]);

  return &ds3g_mmgsdi_imsi_m_info[subs_id];
} /* ds3g_mmgsdi_get_imsi_m_info_from_session_type */
...
/*===========================================================================
FUNCTION DS3G_MMGSDI_READ_ICCID_FROM_SESSION_TYPE

DESCRIPTION
  This function reads the ICCID info from SIM only when both subscription
  ready and CM subs_id from phone event are received.
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ( (ds3g_mmgsdi_info.ds3g_mmgsdi_session_info[index].is_operational == TRUE 
          && ds3gsubsmgr_is_subs_id_valid(subs_id) == TRUE))
  {
    if (ds3g_mmgsdi_check_and_update_iccid_per_sub(subs_id , session_type) 
          == FALSE)
    {
      if(ds3g_mmgsdi_read_iccid(session_type) == FALSE)
      {
        DS_3GMGR_ERROR_LOG_1_EX(MSG_LEGACY_ERROR, subs_id,
                        "Failed to read iccid info for session type [d]",
                        session_type);
      }
    }
  }
  return;
} /* ds3g_mmgsdi_read_iccid_from_session_type() */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  subs_id = ds3g_mmgsdi_get_sub_id_from_session_id(
                      ds_mmgsdi_event_info->session_id);
  ...
  for ( i = 0; i < MAX_DS3G_MMGSDI_NON_PROV_SESSIONS; i++ )
  {
       ...
       )
    {
      DATA_3GMGR_MSG2_EX(MSG_LEGACY_HIGH,
                         subs_id,
                         "ISIM OK to refresh with session_id [%d], " 
                         "session type [%d]",
                         ds3g_mmgsdi_isim_info[i].session_id,
                         ds3g_mmgsdi_isim_info[i].session_type);
      ...
      if ( mmgsdi_status != MMGSDI_SUCCESS)
      {
        DS_3GMGR_ERROR_LOG_1(MSG_LEGACY_ERROR, "Failed to call MMGSDI OK to Refresh "
                        "[d]", mmgsdi_status);
      }
    }
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
     DATA_3GMGR_MSG0_EX(MSG_LEGACY_HIGH, subs_id, "No ISIM App found");
     modem_mem_free(mmgsdi_available_apps_ptr, MODEM_MEM_CLIENT_DATA_CRIT);
     return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (MMGSDI_SUCCESS == status)
  {
    if (MMGSDI_SESSION_OPEN_CNF == cnf)
    {
      ...
      subs_id = ds3g_mmgsdi_get_sub_id_from_slot(cnf_ptr->session_open_cnf.slot_id);
      DATA_3GMGR_MSG3_EX(MSG_LEGACY_HIGH,
                        subs_id,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  subs_id = ds3g_mmgsdi_get_sub_id_from_session_id(event->session_id);
  DATA_3GMGR_MSG3_EX(MSG_LEGACY_HIGH,
                     subs_id,
                     "ds3g_mmgsdi_non_prov_isim_session_evt_reg_cb(): "
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_3GMGR_ERROR_LOG_1_EX(MSG_LEGACY_ERROR, subs_id,"Invalid ds3g mmgsdi status mask d ",
                    mask);
    return FALSE;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /*---------------------------------------------------
    Reading nv item 06862
  ---------------------------------------------------*/
  ds3g_mmgsdi_acl_info[subs_id].acl_read_from_usim = ds3g_get_ehrpd_auth_in_usim(subs_id);

  /*---------------------------------------------------
    If acl read mode is S/W then don't read ACL from
    card and mark the initialization flag to TRUE.
  ---------------------------------------------------*/
  if( ds3g_mmgsdi_acl_info[subs_id].acl_read_from_usim == FALSE )
  {
    ds3g_mmgsdi_acl_process_done();
  }
#endif /* FEATURE_EHRPD */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  memset(&event_info,0,sizeof(ds3geventmgr_event_info_type));
  event_info.imsi_info.app_type = app_type;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  memset(&event_info,0,sizeof(ds3geventmgr_event_info_type));
  event_info.imsi_m_info.app_type = app_type;

  if ( imsi_m_info_ptr->ready_status )
  {
    event_info.imsi_m_info.is_valid = TRUE;
    memscpy(&event_info.imsi_m_info.imsi_m_data,
            DS3G_MMGSDI_IMSI_M_LEN,
            imsi_m_info_ptr->imsi_m_response,
            DS3G_MMGSDI_IMSI_M_LEN);
  }
  else
  {
    event_info.imsi_m_info.is_valid = FALSE;
  }
  
  if(ds3geventmgr_set_event_info(DS3GEVENTMGR_MMGSDI_IMSI_M_INFO_EV,
                                  (void *)&event_info,&filter_info) == FALSE)
  {
    DATA_3GMGR_MSG0(MSG_LEGACY_LOW,
                    "Unable to provide IMSI_M info to registered clients");
  } 
  return;
} /* ds3g_mmgsdi_notify_imsi_m_info_ev */
...
/*===========================================================================
FUNCTION      DS3G_MMGSDI_CHECK_AND_UPDATE_ICCID_PER_SUB

DESCRIPTION   Update the mmgsdi info cache with ICCID value for the sub id

DEPENDENCIES  None.

RETURN VALUE  TRUE: if sub is found in the cache
			   FALSE: if sub id is not found in the cache.
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ( found )
  {
    for(i = 0 ; i < MAX_DS3G_MMGSDI_SESSIONS && i != index ; i++)
    {
      if ( ds3g_mmgsdi_info.ds3g_mmgsdi_session_info[i].session_type
            == session_type )
      {
      	ds3g_mmgsdi_info.ds3g_mmgsdi_session_info[i].iccid_len = 
	  ds3g_mmgsdi_info.ds3g_mmgsdi_session_info[index].iccid_len;

	memscpy( ds3g_mmgsdi_info.ds3g_mmgsdi_session_info[i].iccid,
	         ds3g_mmgsdi_info.ds3g_mmgsdi_session_info[i].iccid_len,
	         ds3g_mmgsdi_info.ds3g_mmgsdi_session_info[index].iccid,
	         ds3g_mmgsdi_info.ds3g_mmgsdi_session_info[index].iccid_len);	           
	break;        
  ...
  return FALSE;
} /* ds3g_mmgsdi_check_and_update_iccid_per_sub */

/*===========================================================================
FUNCTION      DS3G_MMGSDI_RESET_MMGSDI_INFO

DESCRIPTION   Reset  the mmgsdi info cache for the given index 
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/ 
  ...
  ds3g_mmgsdi_info.ds3g_mmgsdi_session_info[index].app_type = MMGSDI_APP_NONE;
    
  ds3g_mmgsdi_info.ds3g_mmgsdi_session_info[index].is_operational = FALSE;
   
  ds3g_mmgsdi_info.ds3g_mmgsdi_session_info[index].sub_id = SYS_MODEM_AS_ID_NONE;
    
  ds3g_mmgsdi_info.ds3g_mmgsdi_session_info[index].iccid_len = 0xFF;
  memset( ds3g_mmgsdi_info.ds3g_mmgsdi_session_info[index].iccid,
          0,
	  DS3G_MMGSDI_ICCID_LEN );

  ds3g_mmgsdi_subscription_ready_status[index].subscription_ready_status = 0;
} /* ds3g_mmgsdi_reset_mmgsdi_info */

/*===========================================================================
FUNCTION      DS3G_MMGSDI_RESET_AND_NOTIFY

DESCRIPTION   Reset the ICCID info and generate the DS3GEVENTMGR_MMGSDI_ICCID_INFO_EV event
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  event_info.iccid_info.is_valid = FALSE;
  event_info.iccid_info.subs_id = 
    ds3g_mmgsdi_info.ds3g_mmgsdi_session_info[index].sub_id;
...
)
{
  ...
  do
  {
    ...
    {
      DS_3GMGR_ERROR_LOG_1_EX(MSG_LEGACY_ERROR, subs_id,
                        "Error: Invalid session id: 0xx", session_id);
      break;
    }
   ...
   ----------------------------------------------------------------------*/
    ...
    {
      DS_3GMGR_ERROR_LOG_0_EX(MSG_LEGACY_ERROR, subs_id, "Error: Out of memory");
      break;
    }
                                       ...
                                       tag_value_p))
    {
      DATA_3GMGR_MSG1_EX(MSG_LEGACY_HIGH,
                         subs_id,
                         "Tag Id: %x configured in the USIM NAS Config file",
                         tag_id);
      ret_val = TRUE;
    }
  } while (0); 
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if((input_file_stream == NULL) || (offset == NULL) ||
     (input_file_length == 0))
  {
    DS_3GPP_ERROR_LOG_3_EX(subs_id, "Input_file_stream:0xx, Offset:0xx,"
                         "Input_file_length: d",
                         input_file_stream, offset, input_file_length);
    return tag_length;
  }
  ...
  if(first_byte == 0xFF)
  {
    DS_3GPP_ERROR_LOG_0_EX(subs_id, "Invalid number of length octets");
    return tag_length;
  }
  else
  {
    ...
    { 
      ...
      {
        DS_3GPP_ERROR_LOG_2_EX(subs_id, "Too many length octets: Input_file_length:d,"
                           "Number of octets:d",
  ...
  {
    DS_3GPP_ERROR_LOG_0_EX(subs_id, "Tag length and offset is greater than file length");
    tag_length = 0;
  }
  
  DATA_3GMGR_MSG1(MSG_LEGACY_LOW,"Tag length returned is %d",tag_length);
  return tag_length;
...
)
{
  ...
  {
    DATA_3GMGR_MSG2(MSG_LEGACY_LOW,
                    "input_file_stream is 0x%x,output_tag_value_p is 0x%x",
                    input_file_stream,
                    output_tag_value_p);
    return ret_val;
  }
  ...
  {
    DATA_3GMGR_MSG2(MSG_LEGACY_LOW,
                    "input_file_stream is 0x%x, input_file_length is %d",
                    *input_file_stream,
                    input_file_length);
    ...
    if (current_tag_id < DS3G_MMGSDI_TAG_LOW_PRIORITY_SIG || 
        current_tag_id > DS3G_MMGSDI_TAG_SM_RETRY_RAT_CHANGE)
    {
      DS_3GPP_ERROR_LOG_1_EX(subs_id, "Invalid tag found:0xx", current_tag_id);
      return ret_val;
    }
    ...
    if (tag_length == 0)
    {
      DS_3GPP_ERROR_LOG_0_EX(subs_id, "Invalid tag length");
      return ret_val;
    }
    ...
    switch (current_tag_id)
    {
      ...
      case DS3G_MMGSDI_TAG_LOW_PRIORITY_SIG:
        ...
        {
          /*------------------------------------------------------------------- 
            Tag length for SM Retry at RAT Change and SM Retry Timer should be
            1 byte
            -------------------------------------------------------------------*/
          if (tag_length == 1)
          {
          ...
          {
            DS_3GPP_ERROR_LOG_0_EX(subs_id, "Invalid tag length for a known tag");
          }
          tag_set = TRUE;
        }
        else
...
{
  DATA_3GMGR_MSG0_EX(MSG_LEGACY_HIGH, subs_id, " REset IMSI data");
...
{
  DATA_3GMGR_MSG0_EX(MSG_LEGACY_HIGH,subs_id," REset IMPI data");
...
)
{
  ...
  imsi_m_info_ptr->ready_status = FALSE;

  ds3g_mmgsdi_notify_imsi_m_info_ev(subs_id,
                                    app_type,
                                    imsi_m_info_ptr);
} /* ds3g_mmgsdi_reset_imsi_m_info */

/*===========================================================================
FUNCTION ds3g_mmgsdi_is_cdma_subscription_ready

DESCRIPTION
  Function to get the staus if subscription ready is received for a CDMA subs.

DEPENDENCIES
  None

RETURN VALUE
  TRUE:  If subscription ready was received
  FALSE: Otherwise.

SIDE EFFECTS
  None
===========================================================================*/
boolean ds3g_mmgsdi_is_cdma_subscription_ready
(
  sys_modem_as_id_e_type         subs_id
)
{
  uint8                          index;
  mmgsdi_session_type_enum_type  session_type;
  uint8                       ready_mask = DS3G_MMGSDI_SUBSCRIPTION_READY_MASK;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /* CDMA subs and session have one to one mapping */
  switch(subs_id)
  {
  ...
  return FALSE;  
} /* ds3g_mmgsdi_is_cdma_subscription_ready */
