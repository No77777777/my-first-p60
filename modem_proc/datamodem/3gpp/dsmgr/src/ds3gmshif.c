...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp/dsmgr/src/ds3gmshif.c#3 $ $DateTime: 2020/04/13 07:37:54 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
04/01/16   sd      Added call_id as a parameter in the new call end command 
                   callback from CM.
...
)
{
  ...
  DATA_3GMGR_MSG0_EX(MSG_LEGACY_HIGH,subs_id,"Rx'ed DS_COMPLETE_LL_CONNECT_CMD");
  ...
  if (ds3gi_call_info[call_inst].call_state == DS3G_WAIT_FOR_LL_CONNECT_STATE)
  {
    ...
    if( hdlr_tbl_ptr->complete_ll_connect_handler != NULL )
    {
      ...
      else
      {
        ...
        if (cm_ret_val == FALSE)
        {
          DS_3GPP_ERROR_FATAL("CM end call failed");
          return ret_val;
        }
      } 
...
)
{
  ...
  DATA_3GMGR_MSG0_EX(MSG_LEGACY_HIGH,
                     subs_id,
                     "Rx'ed DS_COMPLETE_LL_DISCONNECT_CMD");
  ...
  if (ds3gi_call_info[call_inst].call_state == DS3G_WAIT_FOR_LL_DISCONNECT_STATE)
  {
    ...
    if( hdlr_tbl_ptr->complete_ll_disconnect_handler != NULL )
    {
      ...
      DATA_3GMGR_MSG0_EX(MSG_LEGACY_HIGH, 
                         subs_id,
                         "Sending End Call command to CM");
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -- - -*/
  ...
  DATA_3GMGR_MSG1_EX(MSG_LEGACY_HIGH, 
                     subs_id,
                     "Rx'ed CM_CALL_RAB_REESTAB_REQ_CMD, call_inst: %d", 
                     call_inst);
  ...
  if( ds3gi_call_info[call_inst].call_state == DS3G_CALL_ACTIVE_CALL_STATE )
  {
    ...
    {
      cm_ret_val = ds_3gpp_loopback_rab_reestab_req(rab_params);
    }
    else
    {
    ...
    if (cm_ret_val == FALSE)
    {
      DS_3GPP_ERROR_FATAL("CM call cmd RAB Re-establish failed");
      return(ret_val);
    }
    else
    {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -- - -*/
  ...
  if(!(IS_IN_DS_TASK()))
  {
    DS_3GPP_ERROR_FATAL("Not executing in ds task");
    return FALSE;
  }
  ...
  if (call_inst >= DS3GI_MAX_NUM_CALLS)
  {
	DS_3GMGR_ERROR_LOG_1(MSG_LEGACY_ERROR,
                       "Can't confirm incoming call setup - Bad call ID %d", cm_call_id);
    setup_params_ptr->cc_cause.cause_value = INCOMPATIBLE_DESTINATION;
	return DS3G_FAILURE;
  }
  ...
  if (cm_ret_val == FALSE)
  {
    DS_3GPP_ERROR_FATAL("CM call cmd setup res failed");
    return DS3G_FAILURE;
  }
  else
  {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -- - -*/
  ...
  {
    DS_3GMGR_ERROR_LOG_0_EX(MSG_LEGACY_ERROR, subs_id, "ATA rx'ed, but phone is not RINGING");
  }
  modem_mem_free((void *)ans_params, MODEM_MEM_CLIENT_DATA_CRIT );
  modem_mem_free((void *)cause_params, MODEM_MEM_CLIENT_DATA_CRIT );
  return(ret_val);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -- - -*/
 ...
 /*---------------------------------------------------------------------- 
  ...
  ds_3gpp_plmn_sys_sel_clr_retry_sm(subs_id);

  ds_3gpp_plmn_sys_sel_set_retry_type(retry_type,
                                      subs_id);

  ds_3gpp_plmn_sys_sel_set_sys_sel_pref_params(&sys_sel_pref_params,
                                               subs_id);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -- - -*/
  ...
  if (sys_sel_cb_f_ptr == NULL)
  {
    DS_3GPP_MSG0_HIGH_EX(subs_id,"sys_sel_cb_f_ptr is NULL");
  }
...
)
{
  ...
  if (sys_sel_cb_f_ptr == NULL)
  {
    DS_3GPP_MSG0_HIGH_EX(plmn_blocking_info.asubs_id, "sys_sel_cb_f_ptr is NULL");
  }

  DS_3GPP_ERROR_LOG_3_EX(plmn_blocking_info.asubs_id, 
                      "Blocking Plmn for timer:d subs_id:d req_pref:d",
  ...
  {
    DS_3GPP_ERROR_LOG_0_EX(plmn_blocking_info.asubs_id, 
                           "Error while invoking CM API to block PLMN");
    return ret_val;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -- - - - */
  ...
  {
    DS_3GPP_MSG3_MED_EX(subs_id, "PLMN being inspected is zero",0,0,0);
    return FALSE;
  }
  DS_3GPP_MSG1_HIGH_EX(subs_id, 
                       "ds3g_msh_enable_lte_plmn_blocking_on_plmn_id_ex:preference :%d", 
                       request_pref);
 ...
 ----------------------------------------------------------------------*/
  ...
  {
    DS_3GPP_ERROR_LOG_1_EX(subs_id, "Error in Sub id d while sending detach"
  ...
  if(cm_ph_cmd_detach_and_plmn_blocking(sys_sel_cb_f_ptr, 
                                        ...
                                        plmn_blocking_info) != TRUE)
  {
    DS_3GPP_ERROR_LOG_3_EX(subs_id, "Error while invoking CM API to unblock PLMN", 
                        0,0,0);
    return FALSE;
  }
...
{
  return ds3g_msh_enable_lte_plmn_blocking_on_plmn_id_ex2
           (plmn_id, CM_PLMN_BLOCKING_PREF_PLMN_UNBLOCKING_ONLY, subs_id);
}
/*===========================================================================
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -- - - - */
  ...
  return ds3g_msh_enable_lte_plmn_blocking_on_plmn_id_ex(current_plmn_id, subs_id);
}
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -- - - - */
  ...
  ret_val = cm_mm_call_cmd_pdn_connectivity_abort_req(ds3gi_cm_call_cmd_cb,
                                                     (void*)(uint32)(abort_params.sdf_id),
                                                      ds3g_cm_client_id,
                                                      &abort_params);
  ...
  if(ret_val != DS3G_SUCCESS)
  {
    DATA_3GMGR_MSG0_EX(MSG_LEGACY_HIGH,
                       subs_id,
                       "Could not abort PDN connection request");
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -- - - - */
  ...
  /*------------------------------------------------------------------------
    If voice call is active on other sub, 
    then set the local deactivation to TRUE. This helps in tearning down 
    the PDN locally as this sub would not have TRM to deactivate it OTA.
  ------------------------------------------------------------------------*/
        ...
        (ds3gsubsmgr_subs_id_cm_to_ds3g(subs_id)))
  {
    ...
    disconnect_params.esm_local_cause.local_cause = ESM_LOCAL_DEACT;
  }
  ...
  if (ds_3gpp_loopback_is_enabled())
  {
    ret_val = ds_3gpp_loopback_hdlr_pdn_disconnect_req(disconnect_params.eps_bearer_id,
                                  &disconnect_params);
  }
  else
  {
  ...
  if(ret_val != DS3G_SUCCESS)
  {
    DATA_3GMGR_MSG0_EX(MSG_LEGACY_HIGH,
                       subs_id,
                       "Could not send PDN disconnect request");
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -- - - - */
  ...
  DATA_3GMGR_MSG0_EX(MSG_LEGACY_HIGH,
                     subs_id,
                     "Initiate PDN connectivity request");
  ...
  if (call_info_p == NULL)
  {
    DS_3GPP_ERROR_LOG_0_EX(subs_id, "call_info_p is NULL");
    return retVal;
  }
  ...
  if( hdlr_tbl_ptr != NULL )
  {
    if( hdlr_tbl_ptr->originate_call_handler_ext != NULL )
    {
      ...
      if(hdlr_result == DS3G_SUCCESS)
      {
        ...
        {
          retVal = \
            ds_3gpp_loopback_pdn_connectivity_req(ds3gi_cm_call_cmd_cb,
        ...
        {  
          ...
          DATA_3GMGR_MSG1_EX(MSG_LEGACY_HIGH,
                             subs_id,
                             "Call origination with SDF_ID %d",
                             lte_orig_params->sdf_id);
        }
        if(retVal == DS3G_FAILURE)
        {
          DATA_3GMGR_MSG0_EX(MSG_LEGACY_HIGH,
                             subs_id,
                             "Call origination with CM failed");
        }
      }
      else
      {
...
)
{
  ...
  if (ds_3gpp_loopback_is_enabled())
  {
     ret_val= ds_3gpp_loopback_res_alloc_req(bearer_alloc_params);
  }
  else
  {
    ret_val = cm_mm_call_cmd_res_alloc_req(ds3gi_cm_call_cmd_cb,
                                        (void*)(uint32)(bearer_alloc_params->sdf_id),
                                        ds3g_cm_client_id,
                                        bearer_alloc_params);
  }
  if(ret_val != DS3G_SUCCESS)
  {
    DS_3GMGR_ERROR_LOG_0_EX(MSG_LEGACY_ERROR, subs_id,
                            "bearer alloc request failed in ds3g");
  }
  return ret_val;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -- - - - */
  ...
  ret_val = cm_mm_call_cmd_bearer_res_mod_req(ds3gi_cm_call_cmd_cb,
                                      (void*)(uint32)(bearer_modify_params->sdf_id),
                                      ds3g_cm_client_id,
                                      bearer_modify_params);
  if(ret_val != DS3G_SUCCESS)
  {
    DS_3GMGR_ERROR_LOG_0_EX(MSG_LEGACY_ERROR, subs_id,
                            "bearer modify request failed in ds3g");
  }
  return ret_val;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -- - - - */
  ...
  DATA_3GMGR_MSG1_EX(MSG_LEGACY_HIGH,
                     subs_id,
                     "Abort Bearer Request, subs_id:%d", 
                     subs_id);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -- - - - */
  ...
  {
    DS_3GMGR_ERROR_LOG_0_EX(MSG_LEGACY_ERROR, subs_id, "Can't orig, incoming call in progress");
    return ret_val;
  }
  ...
  if( hdlr_tbl_ptr != NULL )
  {
    if( hdlr_tbl_ptr->originate_call_handler != NULL )
    {
      ...
      if( hdlr_result == DS3G_SUCCESS )
      {
        ...
        if(ds3gsubsmgr_is_device_multi_sim_mode() == TRUE) 
        {
          ...
          if( ds3gsubsmgr_is_subs_id_valid(subs_id) )
          {
            ...
            if (ds_3gpp_loopback_is_enabled())
            {
              cm_ret_val = ds_3gpp_loopback_hdlr_call_cmd_orig_cc_per_subs(gw_ps_orig_params_ptr,subs_id,&ds3gi_call_info[call_inst].call_id);
            }
            else
            {
            ...
            (ds3gi_call_info[call_inst].call_id == CM_CALL_ID_INVALID))
        {
          ...
          -----------------------------------------------------------------*/
          DS_3GMGR_ERROR_LOG_0_EX(MSG_LEGACY_ERROR, subs_id, "CM failed DS orig");
        }
        else
        {
          /*-----------------------------------------------------------------
      ...
      else
      {
        ...
        DS_3GMGR_ERROR_LOG_0_EX(MSG_LEGACY_ERROR, subs_id, "Can't orig, Mode-sp. Hdlr error");
      }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -- - -*/
  ...
  if(ds3gi_call_info[call_inst].call_state == DS3G_END_CALL_STATE)
  {
    DATA_MSG0_EX(MSG_LEGACY_HIGH,
                 subs_id,
                 "A call_end is already pending. Return Success");
    return DS3G_SUCCESS;
    ...
    if( hdlr_tbl_ptr->user_end_handler != NULL )
    {
      ...
      else
      {
       ...
       /*------------------------------------------------------------------
        ...
        if (ds_3gpp_loopback_is_enabled())
        {
          cm_ret_val = ds_3gpp_loopback_hdlr_call_cmd_end(&end_params);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  DATA_3GMGR_MSG1_EX(MSG_LEGACY_HIGH,
                     subs_id,
                     "Rx'ed CM_PDP_MODIFY_REQ_CMD for call_id %d", 
                     call_id);
  ...
  if(ds3gi_call_info[call_inst].call_state == DS3G_CALL_ACTIVE_CALL_STATE )
  {
    ...
    if (cm_ret_val == FALSE)
    {
      DS_3GPP_ERROR_FATAL("CM call cmd PDP MODIFY REQUEST failed");
      return(DS3G_FAILURE);
    }
    else
    {
  ...
  else
  {
    DATA_3GMGR_MSG0_EX(MSG_LEGACY_HIGH, 
                       subs_id,
                       "Rx'ed MODIFY PDP REQ:Call not active");
  }
...
)
{
  ...
  DATA_3GMGR_MSG0_EX(MSG_LEGACY_HIGH, 
                     subs_id,
                     "Sending PS DATA AVAILABLE command to CM");
  ...
  if (cm_ret_val == FALSE)
  {
    DS_3GPP_ERROR_FATAL("CM call cmd ps_data_available failed");
    return;
  }
} /* ds3g_msh_ps_data_available_ind() */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -- - -*/
  ...
  cm_ret_val = cm_ph_cmd_packet_state1(NULL,            /* cb func          */
                                      NULL,            /* data block ptr   */
                                      ds3g_cm_client_id, 
                                      pkt_state,
                                      subs_id);
  ...
  if (cm_ret_val == FALSE)
  {
    DS_3GPP_ERROR_FATAL("CM cmd_pkt_state failed");
    return;
  }
...
)
{
  ...
  ds3g_cm_client_id = ds3g_get_cm_client_id();
  cm_ret_val = cm_mm_call_cmd_ps_sig_release_per_subs(ds3gi_cm_call_cmd_cb,
                                              NULL,
                                                      ds3g_cm_client_id,
                                                      subs_id
                                             );
  if (cm_ret_val == FALSE)
  {
    DS_3GMGR_ERROR_LOG_0_EX(MSG_LEGACY_ERROR, subs_id,"CM call cmd ps_sig_release failed");
...
)
{
  ...
  DATA_3GMGR_MSG3(MSG_LEGACY_HIGH,
                     "call type: %d, call mode: %d, call id: %d",
                     (int)call_type, 
                     (int)sys_mode, 
                     cm_call_id);
  switch(sys_mode)
...
#else
        DS_3GMGR_ERROR_LOG_0(MSG_LEGACY_ERROR, "UMTS CSD is not supported ");
        result =  DS3G_FAILURE;
...
#else
        DS_3GMGR_ERROR_LOG_0(MSG_LEGACY_ERROR, "UMTS PS is not supported ");
        result = DS3G_FAILURE;
...
SIDE EFFECTS  None.
===========================================================================*/
boolean ds3g_msh_cleanup_nonepc_call
(  
  sys_modem_as_id_e_type    subs_id 
)
{
  ...
  DATA_3GMGR_MSG0_EX(MSG_LEGACY_MED, 
                     subs_id,
                     "Calling cm_ac_cmd_1xPPP_nw_cleanup()");

  ds3g_cm_client_id = ds3g_get_cm_client_id();

  return cm_ac_cmd_PPP_nw_cleanup1(ds3gi_cm_ac_cmd_cb, ds3gi_cm_ac_state_cb,
                                  NULL, ds3g_cm_client_id, subs_id);
