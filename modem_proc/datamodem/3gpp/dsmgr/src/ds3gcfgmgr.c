...
/*===========================================================================
  ...
  $PVCSPath: $
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp/dsmgr/src/ds3gcfgmgr.c#2 $ 
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  efs_conf_file_size =
    ...
    ds_epc_efs_item_file_path_size();
 ...
 /*-------------------------------------------------------------------------
  EPC specific items
  ds_epc_update_data_config_info function, will always NULL terminate
  file_paths_buf_ptr. In case of success return value is the size of string
  copied to buf_ptr.
 -------------------------------------------------------------------------*/
  result = ds_epc_update_data_config_info(file_paths_buf_ptr +
                                           next_file_paths_buf_write_pos);

  if ( (result == -1) ||
     ((uint32)result > ds_epc_efs_item_file_path_size()) )
  {
    DATA_3GMGR_MSG0(MSG_LEGACY_LOW,
                    "Error storing epc efs item file paths in buffer");
    ds_system_heap_mem_free(file_paths_buf_ptr);
    return -1;
  }
  next_file_paths_buf_write_pos += result;
  ...
  if(FALSE == conf_file_valid)
  {
    ...
    length = strlen(file_paths_buf_ptr);    
    result = ds3gcfgmgr_write_efs_file(config_fd,
                      file_paths_buf_ptr,
                                       length,
                                       MCFG_FS_TYPE_EFS);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_ERROR_LOG_2_EX(MSG_LEGACY_ERROR, subs_id, "NV Read Failed Item d Code d",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if( ds3gcfgmgr_nv_command.status != NV_DONE_S )
  {
    DS_3GMGR_ERROR_LOG_2_EX(MSG_LEGACY_ERROR, subs_id, "NV Write Failed Item d Code d",
                    ds3gcfgmgr_nv_command.item, ds3gcfgmgr_nv_command.status);
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if((temp_buf.st_size) != (item_size))
  {
    DATA_3GMGR_MSG_SPRINTF_3(MSG_LEGACY_ERROR,
                    ...
                    "item_size:%d temp_buf.st_size:%d . Mismatched sizes!",
  ...
  {
    DS_3GMGR_ERROR_LOG_1_EX(MSG_LEGACY_ERROR, subs_id, "mcfg_fs_read error:d",
                    read_status);
    ret_val = NV_FAIL_S;
  }
  else
...
/*===========================================================================
FUNCTION      DS3GCFGMGR_WRITE_EFS_FILE

DESCRIPTION   This function writes into the EFS file.  EFS write has a upper 
              bound of 8kb for one write. As file can outgrow this limit, 
              this function calls MCFG write in a loop for the pending data, 
              not written in the previous call.

DEPENDENCIES  None

RETURN VALUE  Length of buffer written into EFS or
              -1 if failure to write

SIDE EFFECTS  None
===========================================================================*/
int ds3gcfgmgr_write_efs_file
(
  int             fd,
  char           *buf,
  uint32          length,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  while(length > 0)
  {
    ...
    {
      DATA_3GMGR_MSG0(MSG_LEGACY_LOW,
                      "Failed to write efs file paths into file");
      ret_val = -1;
      break;
    }
  
    temp_buf = temp_buf + result;
    length = length - result;
    ret_val = ret_val + result;
  }

  return ret_val;  
} /* ds3gcfgmgr_write_efs_file() */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
         ...
         DS3GCFGMGR_EFS_TOKEN_PARSE_EOL != ret_val )
  {
    ...
    {
      DS_3GMGR_ERROR_LOG_0(MSG_LEGACY_ERROR,"Token Parse Failure");
      break;
    }
...
void ds3gcfgmgr_read_fast_dormancy_enabled_flag_from_efs( void )
{
  ...
  int     fd =0, efs_ret = 0;
  int     result;
  uint8   efs_read_val = 0;
  boolean enabled_flag = TRUE;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ( 0 != result )
  {
    DS_3GMGR_ERROR_LOG_1(MSG_LEGACY_ERROR, "Error closing EFS file error:d",
