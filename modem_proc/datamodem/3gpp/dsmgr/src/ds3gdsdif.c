...
Copyright (c) 2010 - 2019 by Qualcomm Technologies, Incorporated.  All Rights Reserved.
===========================================================================*/
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp/dsmgr/src/ds3gdsdif.c#6 $
...
/*---------------------------------------------------------------------------
   Saved Last SRVCC Voice call
---------------------------------------------------------------------------*/
static ds3gdsdif_voice_call_info_type ds3gdsdif_last_srvcc_voice_call_info
                                      [DS3GSUBSMGR_SUBS_ID_COUNT] = {0};

/*---------------------------------------------------------------------------
   Saved Last SRVCC Emergency call
---------------------------------------------------------------------------*/
static ds3gdsdif_voice_call_info_type ds3gdsdif_last_srvcc_emerg_call_info
                                      [DS3GSUBSMGR_SUBS_ID_COUNT] = {0};
...
/*---------------------------------------------------------------------------
  Flag to indicate WLAN availability
---------------------------------------------------------------------------*/
static boolean ds3gdsdif_wlan_avail[DS3GSUBSMGR_SUBS_ID_MAX] = {0};

/*---------------------------------------------------------------------------
  NULL bearer reason info. This would be in NULL Bearer system status event
---------------------------------------------------------------------------*/
static uint64 ds3gdsdif_null_bearer_info    [DS3GSUBSMGR_SUBS_ID_MAX] = {0};

/*---------------------------------------------------------------------------
  Prev NULL bearer reason info. This would be in NULL Bearer system status event
---------------------------------------------------------------------------*/
static uint64 ds3gdsdif_prev_null_bearer_info[DS3GSUBSMGR_SUBS_ID_MAX] = {0};


#ifndef FEATURE_MSIM_L_PLUS_L 

/*---------------------------------------------------------------------------
  Flag to indicate WLAN capability
---------------------------------------------------------------------------*/
static boolean ds3gdsdif_wlan_cap[DS3GSUBSMGR_SUBS_ID_MAX] = {0};

/*---------------------------------------------------------------------------
  Flag to indicate whether WLAN capability has been notified
---------------------------------------------------------------------------*/
static boolean ds3gdsdif_wlan_cap_notified[DS3GSUBSMGR_SUBS_ID_MAX] = {0};
      ...
      subs_indx++)
  {
    if(create_subs_instance[subs_indx] == TRUE)
    {
      if(ds3gdsdif_radio_info[subs_indx] == NULL)
      {
        ...
        {
          DS_3GPP_ERROR_FATAL("Error while creating radio info instance. return.");
        }
        else
        {
          DATA_MSG1(MSG_LEGACY_LOW,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DATA_MSG2_EX(MSG_LEGACY_MED, 
               subs_index,
               "notify wlan status, subs id:%d, wlan available:%d ",
               subs_index,
               wlan_available);

  dsd_event_info.subs_id = subs_index;
  ...
  if ( wlan_available )
  {
    ...
    memscpy(dsd_event_info.data.wlan_param_info.hessid,
	    PS_SYS_MAX_HESSID_LEN,
            wlan_info->hessid,
            PS_SYS_MAX_HESSID_LEN);
  
    memscpy(dsd_event_info.data.wlan_param_info.country_code,
            PS_SYS_MAX_COUNTRY_CODE_LEN,
            wlan_info->country_code,
            PS_SYS_MAX_COUNTRY_CODE_LEN);

    DATA_MSG6(MSG_LEGACY_LOW, "notify wlan status hessid %x:%x:%x:%x:%x:%x ",
              dsd_event_info.data.wlan_param_info.hessid[0],
              dsd_event_info.data.wlan_param_info.hessid[1],
              dsd_event_info.data.wlan_param_info.hessid[2],
              dsd_event_info.data.wlan_param_info.hessid[3],
              dsd_event_info.data.wlan_param_info.hessid[4],
              dsd_event_info.data.wlan_param_info.hessid[5]);
    ...
    DATA_3GMGR_MSG_SPRINTF_2(MSG_LEGACY_HIGH,
                             "Country Code: %d, %d",
                    dsd_event_info.data.wlan_param_info.country_code[0],
                    dsd_event_info.data.wlan_param_info.country_code[1]);
...
/*===========================================================================
FUNCTION       DS3GDSDIF_NOTIFY_WLAN_CAP_INFO

DESCRIPTION    This function sends wlan capability event to clients
...
SIDE EFFECTS   None.
===========================================================================*/
void ds3gdsdif_notify_wlan_cap_info
(
  ds3gsubsmgr_subs_id_e_type      subs_index,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DATA_3GMGR_MSG2_EX(MSG_LEGACY_MED,
                     subs_index,
                     "ds3gdsdif_notify_wlan_cap_info, "
                     "subs_id: %d, wlan_capable: %d", 
                     subs_index, wlan_capable);
  ...
  event_info.wlan_capability_info.wlan_capable = wlan_capable;

  ds3geventmgr_set_event_info(DS3GEVENTMGR_WLAN_CAPABILITY_EV,
                             (void*)&event_info,
                             &filter_info);
...
#ifndef FEATURE_MSIM_L_PLUS_L 
/*===========================================================================
FUNCTION       DS3GDSDIF_CHECK_AND_NOTIFY_WLAN_CAPABLE

DESCRIPTION    This function checks if wlan capability is to be notified
...
SIDE EFFECTS   None.
===========================================================================*/
void ds3gdsdif_check_and_notify_wlan_capable
(
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DATA_3GMGR_MSG1_EX(MSG_LEGACY_MED,
                     ds3g_subs_id,
                     "ds3gdsdif_check_and_notify_wlan_capable, "
                     "subs_id: %d", 
                     ds3g_subs_id);

  if (ds3gdsdif_wlan_cap_notified[ds3g_subs_id] == FALSE)
  {
    /*-------------------------------------------------------------------------
      Initially, always notify wlan capable on sub 1 and not capable on other
      subs. Enforce this rule for now since currently only one sub can support
      vowifi at a time.
    -------------------------------------------------------------------------*/
    if (ds3g_subs_id == DS3GSUBSMGR_SUBS_ID_1)
    {
      ds3gdsdif_wlan_cap[ds3g_subs_id] = TRUE;
      ds3gdsdif_notify_wlan_cap_info(ds3g_subs_id, TRUE); 
    }
    else
    {
      ds3gdsdif_wlan_cap[ds3g_subs_id] = FALSE;
      ds3gdsdif_notify_wlan_cap_info(ds3g_subs_id, FALSE);
    }
    ds3gdsdif_wlan_cap_notified[ds3g_subs_id] = TRUE;
  }
  else
  {
    subs_capability = ds3gsubsmgr_get_subs_capability_for_subs_index(ds3g_subs_id);

    /*-------------------------------------------------------------------------
      When wlan becomes capable on one sub, before notifying wlan capable on
      this sub, always notify wlan not capable on other subs first. Also make
      sure no duplicate notifications are sent.
    -------------------------------------------------------------------------*/
    if ( ( ( subs_capability & SYS_SYS_MODE_MASK_LTE ) == SYS_SYS_MODE_MASK_LTE ) &&
           ( ds3gdsdif_wlan_cap[ds3g_subs_id] == FALSE ) )
    {
      for( subs_index = DS3GSUBSMGR_SUBS_ID_MIN;
           subs_index < DS3GSUBSMGR_SUBS_ID_COUNT; subs_index++ )
      {
        if ( subs_index != ds3g_subs_id && ds3gdsdif_wlan_cap[subs_index] == TRUE ) 
        {
          ds3gdsdif_wlan_cap[subs_index] = FALSE;
          ds3gdsdif_notify_wlan_cap_info(subs_index, FALSE); 
        }
      }
      ds3gdsdif_wlan_cap[ds3g_subs_id] = TRUE;
      ds3gdsdif_notify_wlan_cap_info(ds3g_subs_id, TRUE);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DATA_MSG1_EX(MSG_LEGACY_HIGH, 
               event_info_ptr->subs_capability.subs_id,
               "ds3gdsdif_subsmgr_event_cb event_id:%d",
               event_id);
  ...
  switch(event_id)
  {
    ...
    case DS3GSUBSMGR_SUBS_CAPABILITY_CHG_EV:
      /* Notify subs capability for subs_id */
      ds_dsd_ext_notify_subs_capability_chg(
         event_info_ptr->subs_capability.curr,
         event_info_ptr->subs_capability.subs_id);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
     ...
     (call_event == CM_CALL_EVENT_END))
  {
    ...
    if(gsm_stack_info_ptr->cell_srv_ind.dtm_supp == SYS_DTM_SUPPORT_NOT_AVAIL)
    {
      ...
      /*-----------------------------------------------------------------------
         This is to handle voice call connected/end event. If voice call
         connected/ended and DTM not supported, parse cached stack info.
      -----------------------------------------------------------------------*/
...
/*===========================================================================
FUNCTION     DS3GDSDIF_PROCESS_CDMA_VOICE_CALL_EVENT

DESCRIPTION  This function process the CDMA voice call event.
...
SIDE EFFECTS  None
===========================================================================*/
void ds3gdsdif_process_cdma_voice_call_event
(
  sys_modem_as_id_e_type  asubs_id,
  cm_call_event_e_type    call_event
)
{
  uint8                          cdma_stack;
  ds3g_cm_ss_stack_info_type    *cdma_stack_info_ptr = NULL;
  ds_3gppi_local_err_val_e_type  local_err =  DS_3GPPI_LOCAL_ERR_VAL_NONE; 
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

  do
  {
    /* Get the stack serving CDMA */
    cdma_stack = ds3gstkproc_get_stack_id_from_radio(asubs_id, SYS_SYS_MODE_CDMA);
    if(cdma_stack >= CM_NO_STACKS)
    {
    ...
    /* Get CDMA stack info */
    ds3gstkproc_get_stack_info_from_stack_id(
       ds3gsubsmgr_subs_id_cm_to_ds3g(asubs_id), cdma_stack, &cdma_stack_info_ptr);

    if(NULL == cdma_stack_info_ptr)
    {
    ...
    {
      /*-----------------------------------------------------------------------
         This is to handle voice call connected event. If voice call
         connected, parse cached stack info.
      -----------------------------------------------------------------------*/
      ds3gstkproc_process_cached_stack_info(asubs_id);
    }
  }while(0);
  ...
  if (local_err != DS_3GPPI_LOCAL_ERR_VAL_NONE)
  {
    DATA_3GMGR_MSG2(MSG_LEGACY_MED,
                    "ds3gdsdif_process_cdma_voice_call_event "
                    "returns err_val:d for stack_id:d ",
                    local_err,
                    cdma_stack);
  }
...
)
{
  ...
  DATA_MSG2_EX(MSG_LEGACY_MED,
               ds3g_subs_id,
               "ds3gdsdif_is_voice_call_active_with_call_id:%d, ret_val:%d", 
               call_id, 
               ret_val);
  return ret_val;
...
/*===========================================================================
FUNCTION       ds3gdsdif_is_voice_srvcc_call

DESCRIPTION    This function checks if voice call handoff type is SRVCC

DEPENDENCIES   None.

RETURN VALUE   TRUE: if handoff type is SRVCC
               FALSE: if handoff is not of SRVCC type

SIDE EFFECTS   None.
===========================================================================*/
boolean ds3gdsdif_is_voice_srvcc_call
(
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if( voice_call_inst_p != NULL )
  {
    DATA_MSG0_EX(MSG_LEGACY_MED,
                 ds3g_subs_id,
                 "ds3gdsdif_create_voice_call_instance: already exists");
    return NULL;
  }
  ...
  if( voice_call_inst_p != NULL )
  {
    ...
    voice_call_inst_p->csfb_type = CM_CALL_CSFB_TYPE_NONE;
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if( emerg_call_inst_p != NULL )
  {
    ...
    emerg_call_inst_p->csfb_type = CM_CALL_CSFB_TYPE_NONE;
  ...
  switch( msg_ptr->call_status_e )
  {
    case LTE_RRC_CSFB_CALL_STARTED:
      ...
      if( voice_call_state.is_call_active == FALSE &&
          msg_ptr->call_direction == LTE_RRC_CSFB_CALL_DIR_MT )
      {
        voice_call_inst_p
        ...
        if( voice_call_inst_p )
        {
          ...
          voice_call_inst_p->csfb_type = CM_CALL_CSFB_TYPE_PPCSFB;

          /* Start MT_CSFB_ESTAB_HYSTERESIS timer */
          if( ! ds3g_timer_get_status_ex(DS3G_TIMER_MT_CSFB_ESTAB_HYSTERESIS,
                                         subs_index) )
          {
            ds3g_timer_start_ex( DS3G_TIMER_MT_CSFB_ESTAB_HYSTERESIS,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  timer_id
    = (ds3g_timer_enum_type)DS3G_TIMER_ENUM_FROM_TIMER_ID(timer_id_per_subs,
                                                          ds3g_subs_id);
...
#ifdef FEATURE_QSH_MDUMP
/*===========================================================================
FUNCTION ds3gdsdif_qsh_mdump_collect
...
===========================================================================*/ 
QSH_MDUMP_FN_ATTR void ds3gdsdif_qsh_mdump_collect()
{
  qsh_mdump_collect_high(ds3gdsdif_voice_call_info, 
  	DS3GSUBSMGR_SUBS_ID_COUNT * DS3GDSDIF_VOICE_CALL_COUNT *sizeof(ds3gdsdif_voice_call_info_type));
}
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
      ...
      subs_indx++)
  {
    ...
    ds3gdsdif_last_srvcc_voice_call_info[subs_indx].state = 
                           DS3GDSDIF_VOICE_CALL_INACTIVE;
    ds3gdsdif_last_srvcc_emerg_call_info[subs_indx].state = 
                           DS3GDSDIF_VOICE_CALL_INACTIVE;
  }
  ...
  if(ds3gsubsmgr_event_reg(DS3GSUBSMGR_DEVICE_MODE_EV,
                           ...
                           &client_id) == FALSE)
  {
    DS_3GPP_ERROR_FATAL("Device mode ev registration failed.");
    return;
  }
  ...
  if(ds3gsubsmgr_event_reg(DS3GSUBSMGR_DDS_CHANGE_EV,
                           ...
                           &client_id) == FALSE)
  {
    DS_3GPP_ERROR_FATAL("DDS change ev registration failed.");
    return;
  }
  ...
  if(ds3gsubsmgr_event_reg(DS3GSUBSMGR_MODE_PREF_CHG_EV,
                           ...
                           &client_id) == FALSE)
  {
    DS_3GPP_ERROR_FATAL("mode pref change ev registration failed.");
    return;
  }
  ...
  if(ds3gsubsmgr_event_reg(DS3GSUBSMGR_SUBS_CAPABILITY_CHG_EV,
                           ...
                           &client_id) == FALSE)
  {
    DS_3GPP_ERROR_FATAL("Subs capability change ev registration failed.");
    return;
  }
  ...
  if( ds_dsd_ext_event_reg_client(DS_DSD_EXT_CLIENT_DATA,
                                  ...
                                  NULL) == FALSE)
  {
    DS_3GPP_ERROR_FATAL("Could not register with DSD for pref sys change ind");
  }

  if(ds_dsd_ext_reg_ss_event_process_complete(
       ds3gdsdif_ss_event_complete_hdlr_cb)== FALSE)
  {
    DS_3GPP_ERROR_FATAL("Could not register with DSD for SS event complete");
  }
  ...
  if ( 0 != ps_sys_event_reg(PS_SYS_TECH_WLAN,
                             ...
                             &ps_errno))
  {
    DS_3GPP_ERROR_FATAL("Register for WLAN status change event failed");
  }
#endif /*FEATURE_DATA_WLAN_MAPCON*/
      ...
      subs_indx++)
  {
    ...
    ds3g_timer_register_ex( (void*)&ds3gdsdif_mt_csfb_estab_hyst_timer[subs_indx],
                            ds3gdsdif_mt_csfb_estab_hyst_timer_cb,
  ...
  -------------------------------------------------------------------------*/
  ds3gmsgr_reg_msgr_cb(LTE_RRC_CSFB_CALL_STATUS_IND,
                                  ds3gdsdif_csfb_call_status_ind_hdlr);

#ifdef FEATURE_LAPP
  ds3g_dsd_nr5g_proc_init();
#endif 
  return;
} /* ds3gdsdif_init() */
...
/*===========================================================================
FUNCTION      DS3GDSDIF_SET_ATTACH_APN_INFO

DESCRIPTION   This function extracts the APN name and Emergency call supported info
              to pass it to MPPM and stores it in the cache
...
RETURN VALUE  TRUE: if the cache is update
              FALSE: otherwise
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  return ret_val;
} /* ds3gdsdif_set_attach_apn_info */

/*===========================================================================
FUNCTION      DS3GDSDIF_VALIDATE_ATTACH_APN_INFO
...
SIDE EFFECTS  None.
===========================================================================*/
boolean ds3gdsdif_validate_attach_apn_info
(
  ds3gsubsmgr_subs_id_e_type          ds3g_subs_id,
  boolean                             attach_in_progress,
  ds3geventmgr_attach_prof_info_type *attach_prof_info
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  return ret_val;
} /* ds3gdsdif_validate_attach_apn_info */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(FALSE == ds3gdsdif_validate_attach_apn_info(ds3g_subs_id,
                                            attach_in_progress,
                                            attach_prof_info))
  {
  ...
  {
    DS_3GMGR_ERROR_LOG_0_EX(MSG_LEGACY_ERROR, subs_id,
  ...
  /*-------------------------------------------------------------------------
    Generate the LTE attach init event with the 'in_progress' = FALSE if
    1. The flag is TRUE before and
    2. apn name if different from the current one.
  -------------------------------------------------------------------------*/
  if ( TRUE == ds3gdsdif_lte_attach_in_progress[ds3g_subs_id].in_progress &&
       TRUE == attach_in_progress &&
       strncasecmp((const char *)ds3gdsdif_lte_attach_in_progress
                    [ds3g_subs_id].prof_info.apn_name,
                    (const char *) attach_prof_info->apn_name,
                    DS_SYS_MAX_APN_LEN) != 0 )
  {
    memscpy(event_info_ptr, sizeof(ds3geventmgr_attach_info_type),
            ...
            sizeof(ds3geventmgr_attach_info_type));
    event_info_ptr->lte_attach_info.in_progress = FALSE;

    if(ds3geventmgr_set_event_info(DS3GEVENTMGR_LTE_ATTACH_INIT_EV,
                                  (void *)event_info_ptr,
                                  &filter_info) == FALSE)
    {
      DATA_3GMGR_MSG0_EX(MSG_LEGACY_HIGH,
                         ds3g_subs_id,
...
/*===========================================================================
FUNCTION      DS3GDSDIF_GET_LAST_SRVCC_VOICE_CALL_STATE

DESCRIPTION   Returns the saved last SRVCC voice call state for the given subs_id This 
              function was created to retrieve the last SRVCC call which ended 
              while RAU/TAU was in progress. Using the state of this last saved 
              voice call info, the CM SS SRV event processing can decide 
              whether to declare NULL bearer or GSM full service.
...
ds3gdsdif_voice_call_state_type ds3gdsdif_get_last_srvcc_voice_call_state
(
  ds3gsubsmgr_subs_id_e_type  ds3g_subs_id
)
{
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch( ds3gdsdif_last_srvcc_voice_call_info[ds3g_subs_id].state )
  {
  ...
  if( call_active )
  {
    ...
    voice_call_state.info_p = &(ds3gdsdif_last_srvcc_voice_call_info[ds3g_subs_id]);
  }
  else if( call_inactive )
  {
  ...
  return voice_call_state;
}/* ds3gdsdif_get_last_srvcc_voice_call_state */

/*===========================================================================
FUNCTION      ds3gdsdif_is_voice_or_emergency_call_active

DESCRIPTION   Returns TRUE if the voice call or emergency call is active for 
the given subs_id
...
RETURN VALUE  TRUE : if voice/emergency call is active 
              FALSE : otherwise 

SIDE EFFECTS  None.
===========================================================================*/
boolean ds3gdsdif_is_voice_or_emergency_call_active
(
  ds3gsubsmgr_subs_id_e_type  subs_id
)
{
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  voice_call_state = ds3gdsdif_get_voice_call_state(subs_id);
  emerg_call_state = ds3gdsdif_get_emerg_call_state(subs_id);

  if((voice_call_state.is_call_active == TRUE && voice_call_state.info_p && 
      voice_call_state.info_p->radio != SYS_SYS_MODE_WLAN) ||
     (emerg_call_state.is_call_active == TRUE && emerg_call_state.info_p && 
      emerg_call_state.info_p->radio != SYS_SYS_MODE_WLAN))
  {
    ret_val = TRUE;
  }
...
/*===========================================================================
FUNCTION      ds3gdsdif_is_voice_or_emergency_call_active_on_sys_mode

DESCRIPTION   Returns TRUE if the voice call or emergency call is active for 
              the given subs_id and call is made over the specified radio
.
PARAMETERS    ds3g_subs_id: subscription id
              sys_mode:  radio that the call is made over 

DEPENDENCIES  None.

RETURN VALUE  TRUE : if voice/emergency call is active on specified radio
              FALSE : otherwise 

SIDE EFFECTS  None.
===========================================================================*/
boolean ds3gdsdif_is_voice_or_emergency_call_active_on_sys_mode
(
  ds3gsubsmgr_subs_id_e_type  subs_id,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if((voice_call_state.is_call_active == TRUE &&
      ...
      voice_call_state.info_p->radio == sys_mode) ||
     (emerg_call_state.is_call_active == TRUE &&
      emerg_call_state.info_p && 
      emerg_call_state.info_p->radio == sys_mode))
  {
    ret_val = TRUE;
  }

  return ret_val;
} /* ds3gdsdif_is_voice_or_emergency_call_active_on_sys_mode */

/*===========================================================================
FUNCTION      ds3gdsdif_is_voice_or_emergency_call_active_on_other_sub
...
SIDE EFFECTS  None.
===========================================================================*/
boolean ds3gdsdif_is_voice_or_emergency_call_active_on_other_sub
(
  ds3gsubsmgr_subs_id_e_type  subs_id
)
{
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  other_subs_id = DS3GDSDIF_GET_OTHER_SUBS_INDX(subs_id);

  if (ds3gdsdif_is_voice_or_emergency_call_active (other_subs_id))
  {
    DATA_3GMGR_MSG0_EX(MSG_LEGACY_MED, ds3gsubsmgr_subs_id_ds3g_to_cm(subs_id),
                   "Voice call on other sub");
    ret_val = TRUE;
  }

  return ret_val;
}/* ds3gdsdif_is_voice_or_emergency_call_active_on_other_sub */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /* ---------------------------------------------------------------------
  If the voice call info is still NULL at this point, it is possible that 
  the voice call ended and hence got removed. Check the last saved voice call 
  state to make a decision about NULL bearer 
  ----------------------------------------------------------------------- */
  if (voice_call_state.info_p == NULL)
  {
    voice_call_state 
      = ds3gdsdif_get_last_srvcc_voice_call_state(ds3g_subs_id);
  }
...
/*===========================================================================
FUNCTION      DS3GDSDIF_GET_NON_LTE_VOICE_CALL_STATE

DESCRIPTION   Returns the voice call state for the given subs_id that is not 
              in a call with LTE sys mode.  This function was created to
              handle the multiple SRVCC call case where there may be several
              SRVCC calls but only one has officially transferred the
              sys mode to a non LTE system.
...
SIDE EFFECTS  None.
===========================================================================*/
ds3gdsdif_voice_call_state_type ds3gdsdif_get_non_lte_voice_call_state
(
  ds3gsubsmgr_subs_id_e_type  ds3g_subs_id
)
{
  ...
  ds3gdsdif_voice_call_state_type    voice_call_state;
  boolean                            call_active = FALSE;
  uint8                              call_active_idx = 0;
  boolean                            call_inactive = FALSE;
  uint8                              call_inactive_idx = 0;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    voice_call_info_p = ds3gdsdif_voice_call_info[ds3g_subs_id][index];
    if( voice_call_info_p != NULL && voice_call_info_p->radio != SYS_SYS_MODE_LTE)
    {
      switch( voice_call_info_p->state )
      {
        case DS3GDSDIF_VOICE_CALL_ORIG:
        ...
        case DS3GDSDIF_VOICE_CALL_MT_CSFB_PAGED:
          call_active = TRUE;
          call_active_idx = index;
          break;

        case DS3GDSDIF_VOICE_CALL_INACTIVE_PENDING:
          call_inactive = TRUE;
          call_inactive_idx = index;
          break;
  ...
  if( call_active )
  {
    voice_call_state.is_call_active = TRUE;
    voice_call_state.info_p
      = ds3gdsdif_voice_call_info[ds3g_subs_id][call_active_idx];
  }
  else if( call_inactive )
  {
    voice_call_state.is_call_active = FALSE;
    voice_call_state.info_p
      = ds3gdsdif_voice_call_info[ds3g_subs_id][call_inactive_idx];
  }
  else
  {
    voice_call_state.is_call_active = FALSE;
  ...
  return voice_call_state;
}/* ds3gdsdif_get_non_lte_voice_call_state() */

/*===========================================================================
FUNCTION      DS3GDSDIF_GET_LAST_SRVCC_EMERG_CALL_STATE

DESCRIPTION   Returns the saved last SRVCC emergency call state for the given subs_id  This 
              function was created to retrieve the last SRVCC call which ended 
              while RAU/TAU was in progress. Using the state of this last saved 
              voice call info, the CM SS SRV event processing can decide 
              whether to declare NULL bearer or GSM full service.
...
ds3gdsdif_voice_call_state_type ds3gdsdif_get_last_srvcc_emerg_call_state
(
  ds3gsubsmgr_subs_id_e_type  ds3g_subs_id
)
{
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch( ds3gdsdif_last_srvcc_emerg_call_info[ds3g_subs_id].state )
  {
    case DS3GDSDIF_VOICE_CALL_ORIG:
    case DS3GDSDIF_VOICE_CALL_CONNECTED:
    case DS3GDSDIF_VOICE_CALL_MT_CSFB_PAGED:
  ...
  {
    emerg_call_state.is_call_active = TRUE;
    emerg_call_state.info_p = &(ds3gdsdif_last_srvcc_emerg_call_info[ds3g_subs_id]);
  }
  else if( call_inactive )
  {
  ...
  return emerg_call_state;
}/* ds3gdsdif_get_last_srvcc_emerg_call_state */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /* If the emerg call info is still NULL at this point, it is possible that 
  the emerg call ended and hence got removed. Check the last saved emerg call 
  state to make a decision about NULL bearer */
  if (emerg_call_state.info_p == NULL)
  {
    emerg_call_state 
      = ds3gdsdif_get_last_srvcc_emerg_call_state(ds3g_subs_id);
  }
...
/*===========================================================================
FUNCTION      DS3GDSDIF_GET_NON_LTE_EMERG_CALL_STATE

DESCRIPTION   Returns the emergency call state for the given subs_id that is
              not in a call with LTE sys mode.  This function was created to
              handle the multiple SRVCC call case where there may be several
              SRVCC calls but only one has officially transferred the
              sys mode to a non LTE system.
...
SIDE EFFECTS  None.
===========================================================================*/
ds3gdsdif_voice_call_state_type ds3gdsdif_get_non_lte_emerg_call_state
(
  ds3gsubsmgr_subs_id_e_type  ds3g_subs_id
)
{
  ...
  ds3gdsdif_voice_call_state_type    emerg_call_state;
  boolean                            call_active = FALSE;
  uint8                              call_active_index = 0;
  boolean                            call_inactive = FALSE;
  uint8                              call_inactive_index = 0;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    emerg_call_info_p = ds3gdsdif_emerg_call_info[ds3g_subs_id][index];
    if( emerg_call_info_p != NULL && emerg_call_info_p->radio != SYS_SYS_MODE_LTE)
    {
      switch( emerg_call_info_p->state )
      {
        case DS3GDSDIF_VOICE_CALL_ORIG:
        case DS3GDSDIF_VOICE_CALL_CONNECTED:
          call_active = TRUE;
          call_active_index = index;
          break;

        case DS3GDSDIF_VOICE_CALL_INACTIVE_PENDING:
          call_inactive = TRUE;
          call_inactive_index = index;
          break;
  ...
  if( call_active )
  {
    emerg_call_state.is_call_active = TRUE;
    emerg_call_state.info_p
      = ds3gdsdif_emerg_call_info[ds3g_subs_id][call_active_index];
  }
  else if( call_inactive )
  {
    emerg_call_state.is_call_active = FALSE;
    emerg_call_state.info_p
      = ds3gdsdif_emerg_call_info[ds3g_subs_id][call_inactive_index];
  }
  else
  {
    emerg_call_state.is_call_active = FALSE;
  ...
  return emerg_call_state;
}/* ds3gdsdif_get_non_lte_emerg_call_state() */


/*===========================================================================
FUNCTION      DS3GDSDIF_RESET_LAST_SRVCC_VOICE_EMERG_CALL_STATE

DESCRIPTION   Resets the last voice/emergency call state for the given subs_id 
...
void ds3gdsdif_reset_last_srvcc_voice_emerg_call_state
(
  ds3gsubsmgr_subs_id_e_type  ds3g_subs_id
)
{
  memset( &(ds3gdsdif_last_srvcc_voice_call_info[ds3g_subs_id]), 0, 
          sizeof(ds3gdsdif_voice_call_info_type));
  ds3gdsdif_last_srvcc_voice_call_info[ds3g_subs_id].state = 
          DS3GDSDIF_VOICE_CALL_INACTIVE;
  
  memset( &(ds3gdsdif_last_srvcc_emerg_call_info[ds3g_subs_id]), 0, 
          sizeof(ds3gdsdif_voice_call_info_type));
  ds3gdsdif_last_srvcc_emerg_call_info[ds3g_subs_id].state = 
          DS3GDSDIF_VOICE_CALL_INACTIVE;
} /* ds3gdsdif_reset_last_srvcc_voice_emerg_call_state */

/*===========================================================================
FUNCTION      DS3GDSDIF_FIND_SRVCC_CSFB_VOICE_CALL

DESCRIPTION   Returns searches for a SRVCC voice call other than the passed 
              call id for the given subs_id This function was created because 
              if there is another srvcc or csfb voice call in the system then 
              all voice calls after that should be treated the same way as a 
              srvcc or csfb voice call. This function is called when a voice 
              call connect event is received and if a srvcc/csfb voice call 
              is found in the system, the ho_type and csfb_type fields will 
              be copied over.
...
ds3gdsdif_voice_call_state_type ds3gdsdif_find_srvcc_csfb_voice_call
(
  ds3gsubsmgr_subs_id_e_type  ds3g_subs_id,
  uint8                       call_id
)
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  for( index=0; index < DS3GDSDIF_VOICE_CALL_COUNT; index++)
  {
    ...
    if( voice_call_info_p != NULL )
    {
      if (voice_call_info_p->call_id != call_id &&
          (ds3gdsdif_is_voice_srvcc_call(voice_call_info_p->voice_ho_type) ||
           voice_call_info_p->csfb_type == CM_CALL_CSFB_TYPE_PPCSFB))
      {
        voice_call_state.info_p = voice_call_info_p;
        break;
      }
    }
  }
  ...
  if (voice_call_state.info_p != NULL)
  {
    switch( (voice_call_state.info_p)->state )
    {
      case DS3GDSDIF_VOICE_CALL_ORIG:
      case DS3GDSDIF_VOICE_CALL_CONNECTED:
      case DS3GDSDIF_VOICE_CALL_MT_CSFB_PAGED:
  ...
  {
    /*-------------------------------------------------------------------
     If no SRVCC/CSFB voice call is found, check the last saved SRVCC/CSFB 
     voice call cache
    -------------------------------------------------------------------*/
    voice_call_state
      = ds3gdsdif_get_last_srvcc_voice_call_state(ds3g_subs_id);
  ...
  return voice_call_state;
}/* ds3gdsdif_find_srvcc_csfb_voice_call */

/*===========================================================================
FUNCTION      DS3GDSDIF_FIND_SRVCC_CSFB_EMERG_CALL

DESCRIPTION   Returns searches for a SRVCC/CSFB emergency call other than the passed 
              call id for the given subs_id This function was created because 
              if there is another srvcc or csfb voice call in the system then 
              all voice calls after that should be treated the same way as a 
              srvcc or csfb voice call. This function is called when a voice 
...
ds3gdsdif_voice_call_state_type ds3gdsdif_find_srvcc_csfb_emerg_call
(
  ds3gsubsmgr_subs_id_e_type  ds3g_subs_id,
  uint8                       call_id
)
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  for( index=0; index < DS3GDSDIF_EMERG_CALL_COUNT; index++)
  {
    ...
    if( emerg_call_info_p != NULL )
    {
      if (emerg_call_info_p->call_id != call_id &&
          (ds3gdsdif_is_voice_srvcc_call(emerg_call_info_p->voice_ho_type) ||
           emerg_call_info_p->csfb_type == CM_CALL_CSFB_TYPE_PPCSFB))
      {
        emerg_call_state.info_p = emerg_call_info_p;
        break;
      }
    }
  }
  ...
  if (emerg_call_state.info_p != NULL)
  {
    switch( (emerg_call_state.info_p)->state )
    {
      case DS3GDSDIF_VOICE_CALL_ORIG:
      case DS3GDSDIF_VOICE_CALL_CONNECTED:
      case DS3GDSDIF_VOICE_CALL_MT_CSFB_PAGED:
  ...
  {
    /*-------------------------------------------------------------------
     If no SRVCC/CSFB emergency call is found, check the last saved SRVCC/CSFB 
  ...
  return emerg_call_state;
}/* ds3gdsdif_find_srvcc_csfb_emerg_call */

/*===========================================================================
FUNCTION      DS3GDSDIF_FIND_SRVCC_CSFB_CALL

DESCRIPTION   Returns searches for a SRVCC/CSFB call other than the passed 
              call id for the given subs_id This function was created because 
              if there is another srvcc or csfb voice call in the system then 
              all voice calls after that should be treated the same way as a 
              srvcc or csfb voice call. This function is called when a voice 
...
ds3gdsdif_voice_call_state_type ds3gdsdif_find_srvcc_csfb_call
(
  ds3gsubsmgr_subs_id_e_type  ds3g_subs_id,
  uint8                       call_id
)
{
  ...
  voice_call_state = ds3gdsdif_find_srvcc_csfb_voice_call(ds3g_subs_id, call_id);

  if (voice_call_state.info_p == NULL)
  {
    voice_call_state = ds3gdsdif_find_srvcc_csfb_emerg_call(ds3g_subs_id, call_id);
  }

  return voice_call_state;
} /* ds3gdsdif_find_srvcc_csfb_call */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(dsd_radio != DS_DSD_RADIO_INVALID)
  {
    ...
    {
      DATA_3GMGR_MSG0_EX(MSG_LEGACY_MED,
                         as_id,
...
/*===========================================================================
FUNCTION      DS3GDSDIF_NOTIFY_DSD_RAT_EXPANSION

DESCRIPTION   This function notifies DSD about RAT expansion in progress.

PARAMETERS    as_id: Subs_id
              rat_exp_in_prog: RAT expansion in progress
...
SIDE EFFECTS  None.
===========================================================================*/
void ds3gdsdif_notify_dsd_rat_expansion
(
  sys_modem_as_id_e_type  as_id,
  boolean                 rat_exp_in_prog
)
{
  ds_dsd_event_info_type  dsd_event_info;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  dsd_event_info.event = DS_DSD_CLIENT_RADIO_RAT_EXPANSION_EV;
  dsd_event_info.subs_id = ds3gsubsmgr_subs_id_cm_to_ds3g(as_id);
  dsd_event_info.data.rat_expansion_info.in_progress = rat_exp_in_prog;

  /* RAT expansion in progress is needed only in case of SVLTE+G case.
     1x is used as default sys mode to satisfy the function declaration.
     It is just a dummy here.*/
  ds_dsd_ext_event_notify(DS_DSD_RADIO_1X, &dsd_event_info);

  return;
} /* ds3gdsdif_notify_dsd_rat_expansion() */
...
PARAMETERS    as_id: Subs_id
              ...
              num_configured_scells: number of configured scells
...
)
{
  ds_dsd_event_info_type  dsd_event_info;
#ifdef FEATURE_DATA_4_5G
  ds_eps_lte_mod_mimo_s   curr_mimo_mod;
#endif
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  memset(&dsd_event_info,0,sizeof(dsd_event_info));
...
#ifdef FEATURE_DATA_4_5G
  dsd_event_info.data.lte_ex_so_info.num_configured_scells = 
                                                       num_configured_scells;
  dsd_event_info.data.lte_ex_so_info.connect_mode = 
                      ds_eps_pdn_cntxt_ex_get_connect_mode(dsd_event_info.subs_id);

  /* Also send mimo mod info which will be used for so mask calculation */
  memset(&curr_mimo_mod, 0, sizeof(ds_eps_lte_mod_mimo_s));
  ds_eps_pdn_cntxt_ex_get_mimo_mod(dsd_event_info.subs_id,
                                   &curr_mimo_mod);
  dsd_event_info.data.lte_ex_so_info.dl_256_qam = curr_mimo_mod.dl_256_qam;
  dsd_event_info.data.lte_ex_so_info.ul_64_qam = curr_mimo_mod.ul_64_qam;
  dsd_event_info.data.lte_ex_so_info.mimo_4_layers = curr_mimo_mod.mimo_4_layers;
#endif
...
#ifdef FEATURE_DATA_4_5G
/*===========================================================================
FUNCTION      DS3GDSDIF_NOTIFY_DSD_LTE_MIMO_MOD_EVENT

DESCRIPTION   This function notifies DSD about LTE MIMO MOD events

PARAMETERS    as_id: Subs_id
              dl_256_qam: dl 256 qam availability
              ul_64_qam:  ul 64 qam availability
              mimo_4_layers: 4 layered mimo availability
...
SIDE EFFECTS  None.
===========================================================================*/
void ds3gdsdif_notify_dsd_lte_mimo_mod_event
(
  sys_modem_as_id_e_type  as_id,
  boolean                 dl_256_qam,
  boolean                 ul_64_qam,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  dsd_event_info.event = DS_DSD_CLIENT_RADIO_LTE_EX_SO_CHANGED_EV;
  dsd_event_info.subs_id = ds3gsubsmgr_subs_id_cm_to_ds3g(as_id);
  dsd_event_info.data.lte_ex_so_info.lte_so_ex_bit_msk = DS_DSD_LTE_EX_SO_MOD_MIMO_MSK;
  dsd_event_info.data.lte_ex_so_info.dl_256_qam = dl_256_qam;
  dsd_event_info.data.lte_ex_so_info.ul_64_qam = ul_64_qam;
  dsd_event_info.data.lte_ex_so_info.mimo_4_layers = mimo_4_layers;
  dsd_event_info.data.lte_ex_so_info.connect_mode = 
                        ds_eps_pdn_cntxt_ex_get_connect_mode(dsd_event_info.subs_id);

  /* Also send ca info which will be used for so mask calculation */
  dsd_event_info.data.lte_ex_so_info.num_configured_scells =
        ds_eps_pdn_cntxt_ex_get_num_configured_scells(dsd_event_info.subs_id);
...
/*===========================================================================
FUNCTION      DS3GDSDIF_NOTIFY_DSD_LTE_MODE_CHG_EVENT

DESCRIPTION   This function notifies DSD that LTE is 
              entering or exitting connect mode

PARAMETERS    as_id: Subs_id
              connect_mode: whether LTE is in connect mode
...
SIDE EFFECTS  None.
===========================================================================*/
void ds3gdsdif_notify_dsd_lte_mode_chg_event
(
  sys_modem_as_id_e_type  as_id,
  boolean                 entering_connect_mode
)
{
  ds_dsd_event_info_type  dsd_event_info;
  ds_eps_lte_mod_mimo_s   curr_mimo_mod;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  dsd_event_info.subs_id = ds3gsubsmgr_subs_id_cm_to_ds3g(as_id);
  dsd_event_info.data.lte_ex_so_info.lte_so_ex_bit_msk = 
                                DS_DSD_LTE_EX_SO_CONNECT_MODE_MSK;
  dsd_event_info.data.lte_ex_so_info.connect_mode = entering_connect_mode;
  /* Cache the connect mode status which will be used for so mask calculation */
  #ifdef FEATURE_DATA_4_5G
  ds_eps_pdn_cntxt_ex_set_connect_mode(entering_connect_mode, dsd_event_info.subs_id);
  #endif
  /* Also send mimo mod info which will be used for so mask calculation */
  memset(&curr_mimo_mod, 0, sizeof(ds_eps_lte_mod_mimo_s));
  #ifdef FEATURE_DATA_4_5G
  ...
  #endif
  ds_dsd_ext_event_notify(DS_DSD_RADIO_LTE, &dsd_event_info);
} /* ds3gdsdif_notify_dsd_lte_mode_chg_event */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  dsd_event_info.data.roaming_status_info.roaming_type = roaming_type;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DATA_MSG9_EX(MSG_LEGACY_HIGH,
            ...
            "new_pref_rat_mask:%d new_pref_so_mask:%x%x",
            subs_id, old_pref_sys, new_pref_sys,
            new_event_info_p->dsd_radio_info[new_pref_index].changed_fields,
            new_event_info_p->dsd_radio_info[new_pref_index].srv_status,
            new_event_info_p->dsd_radio_info[new_pref_index].srv_domain,
            new_event_info_p->dsd_radio_info[new_pref_index].rat_mask,
            (uint32)(new_event_info_p->dsd_radio_info[new_pref_index].so_mask >> 32),
            (uint32)(new_event_info_p->dsd_radio_info[new_pref_index].so_mask));
...
)
{
  ...
  ds3gdsdif_voice_call_info_type  *last_srvcc_call_info_ptr = NULL;
  uint8                            index = 0;
  ds3gdsdif_voice_call_info_type  *voice_call_info_p = NULL;
  ds3gdsdif_voice_call_state_type  last_srvcc_csfb_voice_call_state;
  ds3geventmgr_call_status_info_type voice_call_info;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  memset( (void*)&eventmgr_input, 0, sizeof(ds3geventmgr_filter_type ));
  memset ( (void*)&voice_call_info, 0, sizeof(ds3geventmgr_call_status_info_type));
  ...
  eventmgr_input.tech = PS_SYS_TECH_ALL;
  eventmgr_input.ps_subs_id = 
    (ps_sys_subscription_enum_type)ds3gsubsmgr_subs_id_cm_to_ds(call_info_cmd_ptr->subs_id);

  DATA_MSG7_EX(MSG_LEGACY_HIGH,
               subs_indx,
               "ds3gdsdif_process_voice_call_event_cmd: "
               "event:%d call_type:%d call_id:%d sys_mode:%d "
               "voice_ho_type:%d subs_id:%d  csfb_type:%d",
               ...
               call_info_cmd_ptr->csfb_type);

  voice_call_info.sys_mode = call_info_cmd_ptr->call_sys_mode;
  ...
  switch(call_info_cmd_ptr->call_type)
  {
    ...
    case CM_CALL_TYPE_EMERGENCY:
      ...
      else if (call_info_cmd_ptr->call_event == CM_CALL_EVENT_EMERGENCY_FLASHED)
      {
        /*---------------------------------------------------------------------
          1. Delete voice call instance with the current call id
          2. Delete existing emergency call instance (since only 1 emergency call
             is supported)
          3. Create new emergency call instance with the current call id
        ---------------------------------------------------------------------*/
        ds3gdsdif_delete_voice_call_instance(subs_indx, 
                                             call_info_cmd_ptr->call_id);
        emerg_call_state = ds3gdsdif_get_emerg_call_state(subs_indx);
        if (emerg_call_state.info_p != NULL)
        {
          ds3gdsdif_delete_emerg_call_instance(subs_indx, 
  ...
  switch(call_info_cmd_ptr->call_event)
  {
    ...
    case CM_CALL_EVENT_INCOM:
      ...
      call_info_ptr->csfb_type = call_info_cmd_ptr->csfb_type;

      voice_call_info.status = DS3GEVENT_CALL_STATE_CONNECTING;
      /*---------------------------------------------------------------------
    ...
    case CM_CALL_EVENT_CONNECT:
      ...
      {
        voice_call_info.status = DS3GEVENT_CALL_STATE_CONNECTED;
        ...
        if (call_info_ptr->csfb_type == CM_CALL_CSFB_TYPE_NONE)
        {
          call_info_ptr->csfb_type = call_info_cmd_ptr->csfb_type;
        }

        last_srvcc_csfb_voice_call_state = 
           ds3gdsdif_find_srvcc_csfb_call(subs_indx, call_info_ptr->call_id);

        if(last_srvcc_csfb_voice_call_state.info_p != NULL)
        {
          call_info_ptr->csfb_type = (last_srvcc_csfb_voice_call_state.info_p)->csfb_type;
          call_info_ptr->voice_ho_type = (last_srvcc_csfb_voice_call_state.info_p)->voice_ho_type;
        }
        ...
        {
          /*-------------------------------------------------------------------
            If voice call is on CDMA, process cached SS event.
          -------------------------------------------------------------------*/          
          ds3gdsdif_process_cdma_voice_call_event(
    ...
    case CM_CALL_EVENT_HO_COMPLETE:
       /* ------------------------------------------------------------------
         Update the radio info with sys-mode and process the handoff type.
         Ho-type in the voice-call instance should be retainded to enable
         fast return to LTE later when call has ended. So update the ho-
         type only if SRVCC
       --------------------------------------------------------------------*/
      call_info_ptr->radio = call_info_cmd_ptr->call_sys_mode;
       
      if(ds3gdsdif_is_voice_srvcc_call(call_info_cmd_ptr->call_voice_ho_type))
      {
        ...
        for( index=0; index < DS3GDSDIF_VOICE_CALL_COUNT; index++)
        {
          voice_call_info_p = ds3gdsdif_voice_call_info[subs_indx][index];
          if( voice_call_info_p != NULL && 
              (voice_call_info_p->state == DS3GDSDIF_VOICE_CALL_ORIG || 
               voice_call_info_p->state == DS3GDSDIF_VOICE_CALL_CONNECTED))
          {
            if (voice_call_info_p->call_id != call_info_cmd_ptr->call_id)
            {
              voice_call_info_p->radio = call_info_cmd_ptr->call_sys_mode;
              voice_call_info_p->voice_ho_type = call_info_cmd_ptr->call_voice_ho_type;
            }
          }
        }
        ...
        for( index=0; index < DS3GDSDIF_EMERG_CALL_COUNT; index++)
        {
          voice_call_info_p = ds3gdsdif_emerg_call_info[subs_indx][index];
          if( voice_call_info_p != NULL && 
              (voice_call_info_p->state == DS3GDSDIF_VOICE_CALL_ORIG || 
               voice_call_info_p->state == DS3GDSDIF_VOICE_CALL_CONNECTED))
          {
      ...
      /*--------------------------------------------------------------------------
      If the call sys mode/radio of the voice call state gets updated to GSM no 
      matter what RAT it came from originally, then we process the gsm voice call
      event, but if the radio is some other RAT, there is no need to process the
      gsm voice call event
      --------------------------------------------------------------------------*/
      if(SYS_SYS_MODE_GSM == call_info_cmd_ptr->call_sys_mode)
      {
        ds3gdsdif_process_gsm_voice_call_event(
                             ds3gsubsmgr_subs_id_ds3g_to_cm(subs_indx),
                             call_info_cmd_ptr->call_event);
    ...
    case CM_CALL_EVENT_END:
      if(call_info_ptr->call_id == call_info_cmd_ptr->call_id)
      {
        voice_call_info.status = DS3GEVENT_CALL_STATE_IDLE;
        /* cache call info (radio and state) */
        ...
        /*-------------------------------------------------------------------
          If voice call is on CDMA, process cached SS event.
        -------------------------------------------------------------------*/ 
        else if((call_info_cmd_ptr->call_sys_mode == SYS_SYS_MODE_CDMA) ||
           (ds3gdsdif_get_preferred_sys(subs_indx) == SYS_SYS_MODE_CDMA))
        {
          call_info_ptr->radio = call_info_cmd_ptr->call_sys_mode;
        ...
        /* Save the SRVCC call info which has HOed from L 2 G and is being ended so 
        that it can be used during the processing of CM SS SRV change event to decide 
        whether to declare NULL bearer or not */
        
        if (!emergency_call)
        {
          last_srvcc_call_info_ptr = 
              &(ds3gdsdif_last_srvcc_voice_call_info[subs_indx]);
        }
        else
        {
          last_srvcc_call_info_ptr = 
              &(ds3gdsdif_last_srvcc_emerg_call_info[subs_indx]);
        }
        
        if (call_info_ptr->radio == SYS_SYS_MODE_GSM &&
            (call_info_ptr->voice_ho_type == SYS_VOICE_HO_SRVCC_L_2_G ||
             call_info_ptr->csfb_type == CM_CALL_CSFB_TYPE_PPCSFB))
        {
          DATA_MSG2_EX(MSG_LEGACY_HIGH, 
                      subs_indx,
                      "Saving last SRVCC/CSFB voice call call id:%d "
                      "call state: %d", 
                      call_info_ptr->call_id, 
                      call_info_ptr->state);
          memscpy(last_srvcc_call_info_ptr, sizeof(ds3gdsdif_voice_call_info_type),
                  call_info_ptr, sizeof(ds3gdsdif_voice_call_info_type));
        }
        ...
        if((call_info_cmd_ptr->call_type == CM_CALL_TYPE_VOICE)   ||
           ...
           ((emergency_call && ds3g_is_ph_state_emergency_cb_mode(subs_indx) == FALSE)))
        {
        ...
        else if((ds3g_is_ph_state_emergency_cb_mode(subs_indx) == FALSE))
        {
  ...
  /*-----------------------------------------------------------------------
    For regular voice calls always post DS3GEVENT here. For emergency calls
    post DS3GEVENT only for RAT change. For other emrgency call states
    rely on MM_CM_EMERGENCY_MODE_STATUS_IND from CM to post DS3GEVENT.
  -----------------------------------------------------------------------*/

  if (emergency_call == FALSE)
  {
    (void)ds3geventmgr_set_event_info(DS3GEVENTMGR_VOICE_CALL_STATUS_EV,
                                     (void *)&voice_call_info,
                                     &eventmgr_input);
    ds3gtputmgr_downlink_throughput_report_hdlr(call_info_cmd_ptr->subs_id);
    DATA_3GMGR_MSG1_EX(MSG_LEGACY_LOW,
                       ds3gsubsmgr_subs_id_cm_to_ds3g(call_info_cmd_ptr->subs_id),
                       "W2L Voice call set event status: %d ",
                       voice_call_info.status);
  }
  else
  {
    if (call_info_cmd_ptr->call_event == CM_CALL_EVENT_RAT_CHANGE)
    {
      (void)ds3geventmgr_set_event_info(DS3GEVENTMGR_EMERGENCY_CALL_STATUS_EV,
                                        (void *)&voice_call_info,
                                        &eventmgr_input);
    }
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DATA_3GMGR_MSG3_EX(MSG_LEGACY_HIGH,
                    subs_index,
                    "ds3gdsdif_process_wlan_svc_chg_cmd: wlan_avail:%d,"
                    "wqe supported:%d, wlan_assoc_type:%d",
                    wlan_info->is_wlan_available, 
                    wlan_info->wqe_status,
                    wlan_info->wlan_assoc_type);

  for( subs_index = DS3GSUBSMGR_SUBS_ID_MIN;
       subs_index < DS3GSUBSMGR_SUBS_ID_COUNT; subs_index++ )
  {
    if (wlan_info->wlan_assoc_type == PS_SYS_WLAN_ASSOC_STA) 
    {
      ds3gdsdif_notify_wlan_status(subs_index, 
                                   wlan_info, 
                                   wlan_info->is_wlan_available);
      ds3gdsdif_wlan_avail[subs_index] = wlan_info->is_wlan_available;
    }
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if (radio_ptr == NULL)
  {
    DS_ERROR_LOG_0_EX(MSG_LEGACY_ERROR, asubs_id, "radio_ptr NULL");
  ...
  {
    DS_ERROR_LOG_1_EX(MSG_LEGACY_ERROR, asubs_id,
  ...
  {
    DS_ERROR_LOG_2_EX(MSG_LEGACY_ERROR, asubs_id,
              "pref radio sys_mode:d not found on stackd, return no service",
...
} /* dsd3gif_get_srv_status_from_ds3g_radio_cache() */


/*===========================================================================
FUNCTION      DS3GDSDIF_GET_SRV_DOMAIN_FROM_DS3G_RADIO_CACHE

DESCRIPTION   Utility function to get the srv domain from ds3g stk table.

DEPENDENCIES  None.

RETURN VALUE  The srv domain of the pref radio from the ds3g radio cache

SIDE EFFECTS  None.
===========================================================================*/
sys_srv_domain_e_type dsd3gif_get_srv_domain_from_ds3g_radio_cache
(
  sys_modem_as_id_e_type       asubs_id,
  ds_dsd_ext_radio_info_type  *radio_ptr
)
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  -----------------------------------------------------------------------*/
  if((cached_stack_info_ptr->sys_mode != radio_ptr->system_mode) &&
     ((cached_stack_info_ptr->srv_domain> SYS_SRV_DOMAIN_NO_SRV)
      && (cached_stack_info_ptr->srv_domain < SYS_SRV_DOMAIN_MAX)))
  {
    DS_ERROR_LOG_2_EX(MSG_LEGACY_ERROR, asubs_id,
              "pref radio sys_mode:d not found on stackd, return no service",
              cached_stack_info_ptr->sys_mode,
  ...
  return cached_stack_info_ptr->srv_domain;
} /* dsd3gif_get_srv_domain_from_ds3g_radio_cache() */
...
/*===========================================================================
FUNCTION       DS3GDSDIF_GET_NULL_BEARER_REASON_INFO

DESCRIPTION    This function returns the null bearer reason info.

DEPENDENCIES   None.

RETURN VALUE   Null bearer reason info.

SIDE EFFECTS   None.
===========================================================================*/
uint64 ds3gdsdif_get_null_bearer_reason_info
(
  sys_modem_as_id_e_type             asubs_id
)
{
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  subs_index = ds3gsubsmgr_subs_id_cm_to_ds3g(asubs_id);
  if ( ds3gsubsmgr_is_ds3g_subs_index_valid(subs_index))
  {
    return ds3gdsdif_null_bearer_info[subs_index];
  }

  return NULL;
} /* ds3gdsdif_get_null_bearer_reason_info */
/*===========================================================================
FUNCTION       DS3GDSDIF_GET_NULL_BEARER_REASON_INFO

DESCRIPTION    This function returns the prev null bearer reason info.
...
SIDE EFFECTS   None.
===========================================================================*/
uint64 ds3gdsdif_get_prev_null_bearer_reason_info
(
  sys_modem_as_id_e_type             asubs_id
)
{
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ( ds3gsubsmgr_is_ds3g_subs_index_valid(subs_index))
  {
    return ds3gdsdif_prev_null_bearer_info[subs_index];
  }

  return NULL;
} /* ds3gdsdif_get_prev_null_bearer_reason_info */
/*===========================================================================
FUNCTION       DS3GDSDIF_COMPARE_NULL_BEARER_REASON_INFO

DESCRIPTION    This function compare the null bearer reason info.
...
SIDE EFFECTS   None.
===========================================================================*/
boolean ds3gdsdif_compare_null_bearer_reason_info
(
  sys_modem_as_id_e_type             asubs_id
)
{
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    return(ds3gdsdif_null_bearer_info[subs_index] !=
           ds3gdsdif_prev_null_bearer_info[subs_index]);
  }
  return FALSE;
}/* ds3gdsdif_compare_null_bearer_reason_info */

/*===========================================================================
FUNCTION       DS3GDSDIF_SET_OR_CLEAN_NULL_BEARER_REASON_INFO

DESCRIPTION    This function set/clean null bearer reason bit
...
SIDE EFFECTS   None.
===========================================================================*/
void ds3gdsdif_set_or_clean_null_bearer_reason_info
(
  ds3gdsdif_null_bearer_reason_action_type  action,
  uint64                                    reason_bit,
  sys_modem_as_id_e_type                    subs_id
)
{
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /*set null bearer reason bit*/
  if (DS3GDSDIF_SET_NULL_BEARER_REASON == action)
  { 
    ds3gdsdif_null_bearer_info[subs_index] |= reason_bit;
  }
  /*clean null bearer reason bit*/
  else
  {
    ds3gdsdif_null_bearer_info[subs_index] &= (~reason_bit);
  }
  ...
  DS_3GPP_MSG5_HIGH_EX( subs_id,
                     "null_bearer_reason_info: action:%d, reason bit:%x%x, current NULL bearer reason:%x%x",
                     action, 
                     QWORD_HIGH(reason_bit),
                     QWORD_LOW(reason_bit),
                     QWORD_HIGH(ds3gdsdif_null_bearer_info[subs_index]),
                     QWORD_LOW(ds3gdsdif_null_bearer_info[subs_index]));

  return;
}/* ds3gdsdif_set_or_clean_null_bearer_reason_info */

/*===========================================================================
FUNCTION       DS3GDSDIF_CHECK_VOICE_CALL_NULL_BEARER_REASON

DESCRIPTION    This function is to check whether to set/clean 
               DS3GDSDIF_SYS_NULL_BEARER_REASON_VOICE_ON_OTHER_SUB and 
               DS3GDSDIF_SYS_NULL_BEARER_REASON_VOICE_ON_SAME_SUB 
               Null bearer reason bit.
...
SIDE EFFECTS   None.
===========================================================================*/
void ds3gdsdif_check_voice_call_null_bearer_reason
(
  sys_sys_mode_e_type         sys_mode,
  boolean                     ps_data_suspend,
  sys_srv_status_e_type      *srv_status_ptr,
  sys_modem_as_id_e_type      subs_id
)
{
  ds3gsubsmgr_subs_id_e_type ds3g_subs_id = DS3GSUBSMGR_SUBS_ID_INVALID;
  uint64                     null_bearer_reason = DS3GDSDIF_SYS_NULL_BEARER_REASON_NONE;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ds3g_subs_id = ds3gsubsmgr_subs_id_cm_to_ds3g(subs_id);
  if (!ds3gsubsmgr_is_ds3g_subs_index_valid(ds3g_subs_id))
  {
    ...
    case SYS_SYS_MODE_LTE:
      {
        if (ps_data_suspend == TRUE)
        {
          if (ds3gsubsmgr_get_device_mode() == 
                SYS_MODEM_DEVICE_MODE_DUAL_SIM_DUAL_STANDBY)
          {
            if (ds3gdsdif_is_voice_or_emergency_call_active_on_other_sub(ds3g_subs_id))
            {
              *srv_status_ptr = SYS_SRV_STATUS_LIMITED;  
              null_bearer_reason |= DS3GDSDIF_SYS_NULL_BEARER_REASON_VOICE_ON_OTHER_SUB;
            }
            else if (ds3gdsdif_is_voice_or_emergency_call_active_on_sys_mode (ds3g_subs_id,
                                                                              SYS_SYS_MODE_CDMA))
            {
              /* For the 1xSRLTE case, when 1x call is up, ps_data_suspend will
                 be TRUE for the entirety of the voice call, and DS should
                 send null bearer */
              *srv_status_ptr = SYS_SRV_STATUS_LIMITED;
              null_bearer_reason |= DS3GDSDIF_SYS_NULL_BEARER_REASON_VOICE_ON_SAME_SUB;
            }
          }
          else
          {
            if (ds3gdsdif_is_voice_or_emergency_call_active_on_sys_mode (ds3g_subs_id,
                                                                         SYS_SYS_MODE_CDMA))
            {
              /* For the 1xSRLTE case, when 1x call is up, ps_data_suspend will
                 be TRUE for the entirety of the voice call, and DS should
  ...
  ds3gdsdif_set_or_clean_null_bearer_reason_info(
                           (null_bearer_reason & DS3GDSDIF_SYS_NULL_BEARER_REASON_VOICE_ON_OTHER_SUB) != 0,
                                                 DS3GDSDIF_SYS_NULL_BEARER_REASON_VOICE_ON_OTHER_SUB,
                                                 subs_id);

  ds3gdsdif_set_or_clean_null_bearer_reason_info(
                           (null_bearer_reason & DS3GDSDIF_SYS_NULL_BEARER_REASON_VOICE_ON_SAME_SUB) != 0,
                                                 DS3GDSDIF_SYS_NULL_BEARER_REASON_VOICE_ON_SAME_SUB,
                                                 subs_id);

  return;
}/* ds3gdsdif_check_voice_call_null_bearer_reason */

/*===========================================================================
FUNCTION      DS3GDSDIF_NOTIFY_DSD_LTE_ATTACH_PENDING

DESCRIPTION   This function notifies DSD about LTE in attach pending state
...
SIDE EFFECTS  None.
===========================================================================*/
void ds3gdsdif_notify_dsd_lte_attach_pending
(
  sys_modem_as_id_e_type  subs_id
)
{
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  dsd_event_info.event = DS_DSD_CLIENT_RADIO_ATTACH_PENDING_EV;
  dsd_event_info.subs_id = ds3gsubsmgr_subs_id_cm_to_ds3g(subs_id);

  ds_dsd_ext_event_notify(DS_DSD_RADIO_LTE, &dsd_event_info);

  DS_3GPP_MSG0_HIGH_EX(subs_id,
                      "ds3gdsdif_notify_dsd_lte_attach_pending");

} /* ds3gdsdif_notify_dsd_lte_attach_pending */

/*===========================================================================
FUNCTION      DS3GDSDIF_UPDATE_PREV_NULL_BEARER_REASON

DESCRIPTION   This function update previous null bearer reason with current 
              one, after null bearer reason change notification is sent.
...
SIDE EFFECTS  None.
===========================================================================*/
void ds3gdsdif_update_prev_null_bearer_reason
(
  ds3gsubsmgr_subs_id_e_type  subs_index
)
{
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  ds3gdsdif_prev_null_bearer_info[subs_index] =
                                    ds3gdsdif_null_bearer_info[subs_index];
  DS_3GPP_MSG4_HIGH_EX(subs_index,
                     "ds3gdsdif_update_prev_null_bearer_reason: prev NULL bearer reason:%x%x, curr NULL bearer reason:%x%x",
                     QWORD_HIGH(ds3gdsdif_prev_null_bearer_info[subs_index]),
                     QWORD_LOW(ds3gdsdif_prev_null_bearer_info[subs_index]),
                     QWORD_HIGH(ds3gdsdif_null_bearer_info[subs_index]),
                     QWORD_LOW(ds3gdsdif_null_bearer_info[subs_index]));
  return;
} /* ds3gdsdif_update_prev_null_bearer_reason */
