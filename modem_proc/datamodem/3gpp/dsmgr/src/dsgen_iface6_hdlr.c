...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp/dsmgr/src/dsgen_iface6_hdlr.c#4 $
  $Author: pwbldsvc $ $DateTime: 2021/02/08 22:33:06 $
...
12/20/10    op     Removed an DS3GPP_ASSERT(0) in dssnet6i_process_net_up_ev
...
static void dssnet6i_handle_iface_ev_cback
(
...
static void dssnet6i_process_rs_ra_start_ev
(
  dssnet6_sm_cb_type *instance_ptr            /* dssnet6 instance ptr      */
);
...
static void dssnet6i_notify_rs_ra_event
(
  ds_sys_subscription_enum_type  ds_subs_id,
  ds3geventmgr_rs_ra_enum_type   rs_ra_info
);

static void ds3gnet6i_check_and_notify_rs_ra_event
(
  dssnet6_sm_cb_type *instance_ptr            /* dssnet6 instance ptr      */
);
static void dssnet6i_notify_ipv6_iface_up_event
(
  ds_sys_subscription_enum_type   ds_subs_id,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_3GPP_ERROR_FATAL("Invalid Instance ptr or Iface pointer");
    return -1;
  }
  ...
  instance_ptr->rs_start_sent    = FALSE;
  ...
  {
    DS_3GPP_ERROR_FATAL("Invalid primary or secondary dns pointer");
    return -1;
  }
  ...
  {
    DS_3GPP_ERROR_FATAL("ps_iface_set_addr_family() failed, returned -1");
    return -1;
  }
  ...
  -----------------------------------------------------------------------*/
  instance_ptr->iface_event_buf  = 
                         (void *) ps_iface_alloc_event_handle(
                              ps_iface_ptr,
                              dssnet6i_handle_iface_ev_cback,
                              instance_ptr);

  instance_ptr->phys_link_event_buf = (void *)ps_phys_link_alloc_event_handle(
                              PS_IFACE_GET_PHYS_LINK(ps_iface_ptr),
                              dssnet6i_handle_phys_link_ev_cback,
                              instance_ptr); 
  if (instance_ptr->iface_event_buf == NULL
      ||
      instance_ptr->phys_link_event_buf == NULL) 
  {
  ...
  if (-1 == ip6_sm_ind_cback_reg( instance_ptr->ip6_instance,
                            ...
                            instance_ptr))
  {
    DS_3GPP_ERROR_FATAL("ip6_sm_ind_cback_reg() returned -1");
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS3GPP_ASSERT(event < DSSNET6_MAX_EV);
  ...
  switch (event)
  {
    ...
    case DSSNET6_RS_RA_START_EV:
      cmd_id = DS_CMD_DSSNET6_RS_RA_START_EV;
      break;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if((NULL == cmd_ptr) || (NULL == cmd_ptr->cmd_payload_ptr))
  {
    DS_3GPP_ERROR_FATAL("cmd_ptr or cmd_payload_ptr is NULL");
    return;
  }
      ...
      cmd_ptr->hdr.cmd_id == DS_CMD_DSSNET6_NET_ABORTED_EV)
  {
    ...
    DS3GPP_ASSERT(v6_call_status_ptr->dssnet_instance != NULL);
  ...
  switch (cmd_ptr->hdr.cmd_id)
  {
    ...
    case DS_CMD_DSSNET6_RS_RA_START_EV:
      dssnet6i_process_rs_ra_start_ev(instance_ptr);
      break;
...
)
{
  int result;
  int8                          num_events = 0;
  ps_iface_event_enum_type      events[3];
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    num_events = 3; 
    events[0] = IFACE_PHYS_LINK_UP_EV;
    events[1] = IFACE_PHYS_LINK_DOWN_EV;
    events[2] = IFACE_PHYS_LINK_GONE_EV;
  ...
  result = ps_phys_link_subscribe_event_list(
                         instance_ptr->phys_link_event_buf,
...
{
  uint8                          num_events = 0;
  ps_iface_event_enum_type       events[1];
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
/*lint -save -e641*/
/*---------------------------------------------------------------------------
...
---------------------------------------------------------------------------*/
  ...
  if (PS_IFACE_GET_NUM_SEC_FLOWS(instance_ptr->ps_iface_ptr) == 0)
  {
    ...
    num_events = 1; 
    events[0] = IFACE_FLOW_DELETED_EV;
    ps_iface_unsubscribe_event_list(instance_ptr->iface_event_buf,
                                  (ps_iface_event_enum_type *)&events,
                                  num_events);
#ifdef FEATURE_DATA_IS707
...
#endif /* FEATURE_DATA_IS707 */
#ifdef FEATURE_DATA_IS707
  ps_iface_net_down_reason_type call_end_reason = PS_NET_DOWN_REASON_NOT_SPECIFIED;
               ...
               (data_sess_profile_info_ptr->pdn_type == DS707_DATA_SESS_PDN_TYPE_V4_V6) )
          {
            ...
            if(!PS_IFACE_IS_IN_USE(instance_ptr->ps_iface_ptr))
            {
              DATA_3GMGR_MSG2( MSG_LEGACY_LOW,
                               ...
                               instance_ptr->ps_iface_ptr->name, // clang_sa_ignore [core.NullDereference]
                               instance_ptr->ps_iface_ptr->instance ); // clang_sa_ignore [core.NullDereference]
  ...
  switch (instance_ptr->state)
  {
    ...
    case DSSNET6I_WAITING_FOR_NET_UP_STATE:
      ...
      if (ip6_sm_post_event(instance_ptr->ip6_instance, IP6_SM_ABORT_EV, NULL) 
          == -1)
      {
        DS_3GPP_ERROR_FATAL("ip6_sm_post_event() failed returned -1");
      }
      /* Fall through */
    ...
    case DSSNET6I_LINK_CLOSING_STATE:
      {
        ...
        else
        {
          num_events = 1; 
          events[0] = IFACE_FLOW_DELETED_EV;
          ps_iface_subscribe_event_list(instance_ptr->iface_event_buf,
                                        (ps_iface_event_enum_type *)&events,
                                        num_events);
  ...
  switch (instance_ptr->state)
  {
    ...
    case DSSNET6I_WAITING_FOR_NET_DOWN_STATE:
      ...
      DS_3GMGR_ERROR_LOG_2_EX(MSG_LEGACY_ERROR, subs_id,
                      "Got unexpected PHY_LINK_DOWN_EV for DSSNet6 0xx in d state",
                      instance_ptr,
                      instance_ptr->state);
      break;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if (NULL == instance_ptr)
  {
    DS_3GPP_ERROR_FATAL("Input parameters are NULL");
    return;
  }
  ...
  switch (instance_ptr->state)
  {
    ...
    case DSSNET6I_NET_UP_STATE:
      DS_3GMGR_ERROR_LOG_2_EX(MSG_LEGACY_ERROR, subs_id,
                      "DSSNet6 0xx got unexpected LINK_UP_EV in d state",
                      instance_ptr,
                      instance_ptr->state);
      DS3GPP_ASSERT(0);
      break;
  ...
  switch (instance_ptr->state)
  {
    case DSSNET6I_CLOSED_STATE:
      ...
      if (FALSE == instance_ptr->use_ppp)
      {
        ...
        ds_subs_id = PS_IFACE_GET_SUBS_ID(instance_ptr->ps_iface_ptr);

        if(ds_pdn_cntxt_get_ipv6_iface_up_in_progress(instance_ptr->ps_iface_ptr))
        {
          dssnet6i_notify_ipv6_iface_up_event( 
                                 ds_subs_id,
                                 DS3GEVENTMGR_IPV6_IFACE_UP_END);
          ds_pdn_cntxt_set_ipv6_iface_up_in_progress(instance_ptr->ps_iface_ptr,FALSE);
        }
    ...
    case DSSNET6I_WAITING_FOR_NET_UP_STATE:
      ...
      if (ip6_sm_post_event(instance_ptr->ip6_instance, IP6_SM_ABORT_EV, NULL) 
          == -1)
      {
        DS_3GMGR_ERROR_LOG_1_EX(MSG_LEGACY_ERROR, 
                        ds3gsubsmgr_subs_id_ds_to_cm(ds_subs_id),
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  ds3gnet6i_check_and_notify_rs_ra_event(instance_ptr);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch (instance_ptr->state)
  {
    ...
    case DSSNET6I_LINK_RESYNC_STATE:
      /* call registered callback if it is 3GPP and callback exists
       use_ppp is FALSE means 3GPP*/
      if ((!instance_ptr->use_ppp) && (instance_ptr->event_cb != NULL))
      {
        instance_ptr->event_cb(instance_ptr,
                               DSSNET6_NET_DOWN_EV,
                               instance_ptr->state, 
...
/*===========================================================================
FUNCTION DSSNET6I_PROCESS_RS_RA_START_EV()

DESCRIPTION
  This function processes the DSSNET6_RS_RA_START_EV.
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  -------------------------------------------------------------------------*/
  subs_indx = ds3gsubsmgr_subs_id_ds_to_ds3g(ds_subs_id);
  if( ds3gsubsmgr_is_ds3g_subs_index_valid(subs_indx) == FALSE )
  {
    return;
  }

  instance_ptr->rs_start_sent = TRUE;
  dssnet6i_notify_rs_ra_event( ds_subs_id,
                               DS3GEVENTMGR_RS_RA_START);
}
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS3GPP_ASSERT(ppp_dev == PPP_UM_SN_DEV);
  switch (ppp_event)
  {
    case PPP_UP_EV:
    ...
    case PPP_SUBS_LIMITED_TO_V6_EV:
      ...
      if ( !DS707_PDNCNTX_CB_IS_VALID(pdncntx_cb_ptr) )
      {
        DS_3GPP_ERROR_FATAL("Cannot get PDN Control Block, do not proceed");
      }
      ...
      {
        DS_3GMGR_ERROR_LOG_1(MSG_LEGACY_ERROR, "Event action d not supported for "
...
/*===========================================================================
FUNCTION DSSNET6I_HANDLE_IFACE_EV_CBACK()

DESCRIPTION
  Called when the registered IFACE EVENT occurs.  This posts an appropriate
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch (ind)
  {
    ...
    case IP6_SM_SOL_IND:
      (void) dssnet6_sm_post_event(instance_ptr, DSSNET6_RS_RA_START_EV,NULL);
       break;
...
)
{
 ...
 boolean                        is_third_party_ho = FALSE;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch (instance_ptr->state)
  {
    case DSSNET6I_CLOSED_STATE:
      ...
      {
       profile_index = (ds_profile_num_type)instance_ptr->ps_iface_ptr->iface_private.profile_id_3gpp;
       ds_3gpp_iwlan_to_lte_ho_reset_ho_info(profile_index);
       DATA_3GMGR_MSG2(MSG_LEGACY_HIGH,
              "DSSNet6 0x%x reset iwlan ho info for Profile: %d", instance_ptr, profile_index);	
      }
      /*---------------------------------------------------------------------
         ...
         ------------------------------------------------------------------*/
          ...
          if(instance_ptr->iface_down_ind_cb != NULL)
          {
            DATA_MSG3(MSG_LEGACY_HIGH,"V6 iface 0x%x:%d (0x%x) is down. Calling iface_down_ind_cb",
                      instance_ptr->ps_iface_ptr->name, // clang_sa_ignore [core.NullDereference]
                      instance_ptr->ps_iface_ptr->instance, // clang_sa_ignore [core.NullDereference]
                      instance_ptr->ps_iface_ptr); // clang_sa_ignore [core.NullDereference]
            instance_ptr->iface_down_ind_cb(instance_ptr->ps_iface_ptr,
                                        instance_ptr->call_end_reason,
                                        TRUE);
          }
          ...
          -----------------------------------------------------------------*/
          num_events = 3; 
          events[0] = IFACE_PHYS_LINK_GONE_EV;
          events[1] = IFACE_PHYS_LINK_UP_EV;
          events[2] = IFACE_PHYS_LINK_DOWN_EV;
          ps_phys_link_unsubscribe_event_list(
                         instance_ptr->phys_link_event_buf,
                                      (ps_iface_event_enum_type *)&events,
                                      num_events);
    ...
    case DSSNET6I_LINK_OPENING_STATE:
      ...
      -----------------------------------------------------------------*/
        ...
        num_events = 1; 
        events[0] = IFACE_PHYS_LINK_DOWN_EV;
        ps_phys_link_unsubscribe_event_list(
                           instance_ptr->phys_link_event_buf,
                                    (ps_iface_event_enum_type *)&events,
                                    num_events);
...
#endif /* FEATURE_DATA_IS707 */
      {
        ...
        if(ppp_config == NULL)
        {
          DS_3GMGR_ERROR_LOG_0(MSG_LEGACY_ERROR, "dssnet6i_transition_state():"
    ...
    case DSSNET6I_WAITING_FOR_NET_UP_STATE:
      ...
      if (instance_ptr->use_ppp == FALSE && 
          ds3g_is_optimized_handoff_ip_config_enabled
            (PS_IFACE_GET_SUBS_ID(instance_ptr->ps_iface_ptr)) == TRUE)
      {
        if ((boolean)(uint32)(instance_ptr->user_data_p) == TRUE)
        {
          DATA_3GMGR_MSG0(MSG_LEGACY_HIGH,
                        "iWLAN to LTE HO. Setting IP6 SM to passive start");

          if ((ip6_sm_set_passive(instance_ptr->ip6_instance)) == -1)
          {
            DS_3GMGR_ERROR_LOG_0(MSG_LEGACY_ERROR,
                          "ip6_sm_set_passive() failed ");
          }
        }
      }

	  is_third_party_ho = dssnet6i_is_third_party_ho(instance_ptr);
#ifdef FEATURE_DATA_OPTHO
      if ((instance_ptr->use_ppp) && 
          ...
          (ds707_s101_query_sm_state() != DS707_S101_SM_NON_TUNNEL))
      {
        ...
        if ((ip6_sm_set_passive(instance_ptr->ip6_instance)) == -1)
        {
          DS_3GPP_ERROR_FATAL("ip6_sm_set_passive() failed ");
        }
      }
#endif /* FEATURE_DATA_OPTHO */
      ...
      DS_3GMGR_ERROR_LOG_1(MSG_LEGACY_ERROR,"Prefix Delegation type returned d",
                      dssnet6i_get_ipv6_prefix_delegation_mode(instance_ptr->ps_iface_ptr));

      if ((ip6_sm_post_event( instance_ptr->ip6_instance,
                              IP6_SM_START_EV, NULL)) == -1)
      {
        DS_3GPP_ERROR_FATAL("ip6_sm_post_event() failed ");
      }

      if (is_third_party_ho)
      {
        dssnet6i_set_prefix_for_third_party_ho(instance_ptr);
      }
      if (!instance_ptr->use_ppp)
      {
        ds_3gpp_iwlan_to_lte_ho_config_v6_params(instance_ptr);
      }

      dssnet6i_apply_v6_prefix_from_iface(instance_ptr);
      break;
    ...
    case DSSNET6I_NET_UP_STATE:    
      ...
      if( (DS707_PDNCNTX_CB_IS_VALID(pdncntx_cb_ptr)) &&
          ( ((PS_IFACE_IS_VALID(iface4_ptr)) &&
             (ps_iface_state(iface4_ptr) != IFACE_COMING_UP)) ||
            (!PS_IFACE_IS_VALID(iface4_ptr)) ) &&
	...
	queue_status = mc_queue_cmd(ds707_curr_subs_id(), MC_TUNE_AWAY_CNTL_F);
      ...
      {
       /*-----------------------------------------------------------------
         register with Data path manager before IFACE UP ind.
       ---------------------------------------------------------------------*/
       ds707_pkt_mgr_util_dpm_reg( instance_ptr->ps_iface_ptr);
      ...
      if (instance_ptr->pdsn_as_proxy_ipv6_dns_server)
      {
        ...
        instance_ptr->ps_iface_ptr->v6_net_info.primary_dns = peer_addr.addr.v6; // clang_sa_ignore [core.NullDereference]
        instance_ptr->ps_iface_ptr->v6_net_info.secondary_dns = peer_addr.addr.v6; // clang_sa_ignore [core.NullDereference]
      }
    ...
    case DSSNET6I_LINK_RESYNC_STATE:
      ...
      if (instance_ptr->use_ppp)
      {
        ...
        if(instance_ptr->is_ehrpd_call == TRUE)
        {
          ...
          if ( !DS707_PDNCNTX_CB_IS_VALID(pdncntx_cb_ptr) )
          {
            DS_3GPP_ERROR_FATAL("Cannot get PDN Instance to set call bringup mode");
            return; 
          }
          /*-------------------------------------------------------------------
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  ---------------------------------------------------------------------*/
#ifdef FEATURE_DATA_IS707
  ppp_config_ptr->subs_id = ppp_get_subs_id( ds707_curr_subs_id() );
#endif /*FEATURE_DATA_IS707*/
  ...
  {
    DS_IS707_ERROR_LOG_2(MSG_LEGACY_ERROR," resolved sub id d is not supported"
  ...
  if (PS_IFACE_IS_VALID(instance_ptr->ps_iface_ptr))
  {
    instance_ptr->ps_iface_ptr->v6_net_info.primary_dns 
  ...
  {
    DATA_3GMGR_MSG0(MSG_LEGACY_LOW,"Invalid iface ptr");
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  if (instance_ptr == NULL)
  {
    DS_3GPP_ERROR_FATAL("Instance pointer for dssnet6_set_ps_local_abort is NULL");
  }
  else
  {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if (NULL == dssnet6_sm_cb)
  {
    DS_3GPP_ERROR_FATAL("dssnet6_sm_cb is NULL. Can't reset call local abort");
    return;
  }
  DATA_3GMGR_MSG1(MSG_LEGACY_HIGH, 
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if (!(DSSNET6I_IS_VALID(instance_ptr)))
  {
    DS_3GPP_ERROR_FATAL("dssnet6_sm_cb is not valid");
    return;
  }
...
/*=========================================================================
FUNCTION       DSSNET6I_NOTIFY_RS_RA_EVENT

DESCRIPTION    This function notifies the RS/RA event
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  filter_info.ps_subs_id = (ps_sys_subscription_enum_type) ds_subs_id;

  event_info.rs_ra_info.rs_ra_type = rs_ra_info;

  if(ds3geventmgr_set_event_info(DS3GEVENTMGR_RS_RA_EV,
                                 (void *)&event_info,&filter_info) == FALSE)
  {
    DATA_3GMGR_MSG0(MSG_LEGACY_LOW,
                    "Unable to provide Last RS event to clients");
  }
} /* dssnet6i_notify_rs_ra_event */
/*=========================================================================
FUNCTION       DSSNET6I_NOTIFY_IPV6_IFACE_UP_EVENT
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  event_info.ipv6_iface_up_info.ipv6_iface_up_type = ipv6_iface_up_info;

  if(ds3geventmgr_set_event_info(DS3GEVENTMGR_IPV6_IFACE_BRINGUP_EV,
                                 (void *)&event_info,&filter_info) == FALSE)
  {
    DATA_3GMGR_MSG0(MSG_LEGACY_LOW,
                    "Unable to provide Last RS event to clients");
...
/*=========================================================================
FUNCTION       DS3GNET6I_CHECK_AND_NOTIFY_RS_RA_EVENT

DESCRIPTION    This function checks if RS/RA event with type 
               DS3GEVENTMGR_RS_RA_END is to be notified for given instance
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ( TRUE == instance_ptr->rs_start_sent )
  {
    dssnet6i_notify_rs_ra_event( ds_subs_id,
                                 DS3GEVENTMGR_RS_RA_END);
    instance_ptr->rs_start_sent = FALSE;
  }
  /*it mean iface up or down */
  if(ds_pdn_cntxt_get_ipv6_iface_up_in_progress(instance_ptr->ps_iface_ptr))
  {
    dssnet6i_notify_ipv6_iface_up_event( 
                                 ds_subs_id,
...
} /* ds3gnet6i_check_and_notify_rs_ra_event */
