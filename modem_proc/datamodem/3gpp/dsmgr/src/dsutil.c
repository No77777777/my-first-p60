...
$Header: //components/rel/data.mpss/3.5..2.1/3gpp/dsmgr/src/dsutil.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
/*==============================================================================
FUNCTION DS_UTIL_CALCULATE_THROUGHPUT_MOVING_AVG_ULDL
...
SIDE EFFECTS
  None
===========================================================================*/
uint32 ds_util_calculate_throughput_moving_avg_uldl
(
  uint64  prev_avg_throughput,
  uint64  prev_total_rx_bytes,
  uint64  current_total_rx_bytes,
  uint64  sampling_interval,
  uint64  moving_avg_coeff
)
{
  uint64 current_rate =  0; 
  uint64 num_rx_bytes =  0;
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if (prev_avg_throughput == 0)
  {
    new_avg = current_rate;
  }
  else 
  {

    new_avg =  (((moving_avg_coeff* current_rate)+
                 ((MAX_UL_DL_FREQUENCY - moving_avg_coeff)*prev_avg_throughput)) 
                / MAX_UL_DL_FREQUENCY);
  }

  return (uint32)new_avg;
...
/*===========================================================================
FUNCTION      DS_UTIL_HEXSTRTOI

DESCRIPTION   This function will convert hex string to interger value.
...
SIDE EFFECTS  None.
===========================================================================*/
uint32 ds_util_hexstrtoi( char *str )
{
  uint32  n = 0;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
...
/*===========================================================================
FUNCTION      DS_UTIL_DECSTRTOI

DESCRIPTION   This function will convert dec string to interger value.
...
SIDE EFFECTS  None.
===========================================================================*/
uint32 ds_util_decstrtoi( char *str )
{
  uint32  n = 0;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
