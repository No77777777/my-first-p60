...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp/dsmgr/src/ds3gsiolib_ex.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_3GPP_ERROR_FATAL("NULL passed for alloc_info_ptr");
    return return_port;
  }  
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_3GPP_ERROR_FATAL("Port Id is DS3G_SIOLIB_PORT_NONE");
    return result;
  }
  ...
  if(NULL == state)
  {
    DS_3GPP_ERROR_FATAL(" InValid Port State");
    return result;
  } 
  /*-------------------------------------------------------------------------
  ...
  {
    DS_3GPP_ERROR_FATAL("Callback func. ptr = NULL in SIOLIB");
    return FALSE;
  }
  return result;
                                     ...
                                     DSAT_BASIC_N_C_IDX,0,0,NUM_TYPE) == 2 )
       {
         switch (port_index)
         {
            case DS3G_SIOLIB_PORTID_0:
              ds3g_timer_start( DS3G_TIMER_WINK_DCD_1, DS3G_SIOLIB_DCD_WINK_INTERVAL);
              ds3g_siolib_port_signal_status[port_index].dcd_winking = ON;
              break;
            case DS3G_SIOLIB_PORTID_1:
              ds3g_timer_start( DS3G_TIMER_WINK_DCD_2, DS3G_SIOLIB_DCD_WINK_INTERVAL);
              ds3g_siolib_port_signal_status[port_index].dcd_winking = ON;
              break;
            default:
              /*do nothing*/
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (port_id <= DS3G_SIOLIB_PORT_NONE || 
	    port_id >= DS3G_SIOLIB_PORT_MAX)
  {
    DS_3GPP_ERROR_FATAL("port_id invalid");
	return;
  }
      ...
      (state->serial_info.close_in_progress == FALSE) )
  {
    ...
    if ( stream_mode == SIO_DS_AUTODETECT_MODE ) 
    {
      ...
      if ( ( rx_wm_ptr == NULL ) && ( tx_wm_ptr == NULL ) )
      {
       ...
       {
         DS_3GPP_ERROR_FATAL("WM pointers not null in autodetect mode ");
         return DS3G_SIOLIB_NO_PORT_OPEN;
       }
    }
    else
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(port_id <= DS3G_SIOLIB_PORT_NONE ||
	 port_id >= DS3G_SIOLIB_PORT_MAX)
  {
    DS_3GPP_ERROR_FATAL("port id ==DS3G_SIOLIB_PORT_NONE");
    return ret_val;
  }
...
DESCRIPTION
  This function controls the enabling or disabling of inbound flow ctl on a 
  specified port.
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ( state->serial_info.stream_id != SIO_NO_STREAM_ID )
  {
    ...
    DS_SIO_MSG2_LOW("Call Msk: 0x%x FlowMsk: 0x%x",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (port_id <= DS3G_SIOLIB_PORT_NONE || 
	    port_id >= DS3G_SIOLIB_PORT_MAX)
  {
    DS_SIO_MSG0_LOW("Port id = PORT_NONE");
  ...
  if(NULL == state)
  {
    DS_SIO_MSG0_LOW(" InValid Port State");
  ...
  {
    DS_SIO_MSG1_LOW("Checking CD w/bad stream id %d", state->serial_info.stream_id);
...
func_exit:
  ...
  {
    DS_SIO_MSG1_ERROR("ds3g_siolib_ex_is_cd_asserted: err %d",
                       local_err);
  }
  return( ret_val );
