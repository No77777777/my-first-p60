...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp/dsmgr/src/ds3gcmif.c#7 $ $DateTime: 2023/02/15 03:35:59 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
04/01/16   sd      Fixed throttling info updation on query through QMI WDS.
02/23/15   sd      Rectified throttle info update handling in CM SS event.
01/28/15   sd      Added call_id as a parameter in the new call end command
                   callback from CM.
...
#include "ds3g_dds_switch_hdlr.h"
...
static ds3g_cm_ph_cb_info ds3gi_cm_ph_cb_info[DS3GSUBSMGR_SUBS_ID_COUNT];
...
/*---------------------------------------------------------------------------
  Cache to store CM PH event RAT disabled mask per sub
---------------------------------------------------------------------------*/
static uint32 ds3g_cm_rat_disabled_mask[DS3GSUBSMGR_SUBS_ID_COUNT] = {0};
...
#define DS3G_RAT_DISABLED_MASK_CONTAIN(mask, sys_mode)  \
        ( ((mask & (1<< ((uint32)sys_mode))) != 0) ? TRUE : FALSE )


#define DSMGR_SET_ESM_CAUSE(info, cause_code)  \
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  for (instance = 0; instance < DS3GI_MAX_NUM_CALLS; instance++)
  {
    if (ds3gi_call_info[instance].call_state != DS3G_IDLE_CALL_STATE)
    {
      ...
      {
        DS_3GMGR_ERROR_LOG_2_EX(MSG_LEGACY_ERROR, 
                        ds3gi_call_info[instance].subs_id,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_3GPP_ERROR_FATAL("Null pointer for cm_mm_call_info_s_type");
    return(FALSE);
  }
  ...
  else if (call_event == CM_CALL_EVENT_UNBLOCK_ALL_APN)
  {
    result = TRUE;
  }
...
)
{
  ...
  for (instance = 0; instance < DS3GI_MAX_NUM_CALLS; instance++)
  {
         ...
         (ds3gi_call_info[instance].call_mode == call_mode)  )
    {
      ...
      {
        DS_3GPP_ERROR_FATAL("Call_id is invalid");
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if( (cmd_ptr == NULL) || (cmd_ptr->cmd_payload_ptr == NULL) )
  {
    DS_3GPP_ERROR_FATAL("ds3gi_end_call has NULL cmd ptr");
    return;
  }

  call_info_cmd_ptr = (ds3g_call_info_cmd_type*)cmd_ptr->cmd_payload_ptr;
  if (!ds3gsubsmgr_is_subs_id_valid(call_info_cmd_ptr->subs_id))
  {
    return;
  }  
  ...
  ds3gi_call_info[call_inst].call_end_recd = TRUE;
  ...
  else if(call_info_cmd_ptr->end_type == DS_CM_CALL_END_EXTERNAL)
  {
    ...
    */
       ...
       ds3gi_call_info[call_inst].call_state != DS3G_END_CALL_STATE )
    {
      ...
      DS_3GMGR_ERROR_LOG_3_EX(MSG_LEGACY_ERROR,
      ...
      switch(ds3gi_call_info[call_inst].call_mode)
      {
       ...
       ds3gi_call_info[call_inst].call_state == DS3G_END_CALL_STATE)
    {
      DATA_3GMGR_MSG2_EX(MSG_LEGACY_HIGH,
				         call_info_cmd_ptr->subs_id,
  ...
  DATA_3GMGR_MSG1_EX(MSG_LEGACY_HIGH, 
                     call_info_cmd_ptr->subs_id,
                     "Data call ended for call_id %d",
                     call_info_cmd_ptr->call_id);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DATA_3GMGR_MSG1_EX(MSG_LEGACY_HIGH, 
	                 event_ptr->asubs_id,
	                 "ds3gi_cm_ph_event_cb() for event: %d",
	                 ph_event);
...
#ifdef FEATURE_MSIM_L_PLUS_L 
    case CM_PH_EVENT_DDS_SWITCH_ALLOWED:
#endif 
      notify = TRUE;
      break;
  ...
  if( notify == TRUE )
  {
    ...
    ph_ev_info_ptr->rat_disabled_mask  = event_ptr->rat_disabled_mask;
    ...
    ph_ev_info_ptr->dds_switch_type    = event_ptr->dds_switch_type;
    ph_ev_info_ptr->test_control_type  = event_ptr->test_control_type;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_3GPP_ERROR_FATAL("subs_info_ptr = NULL");
    return;
  }
  ...
  switch ( subs_event )
  {
    case CM_PH_EVENT_SUBSCRIPTION_PREF_INFO:
      DATA_3GMGR_MSG3_EX(MSG_LEGACY_MED,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
     ...
     (call_info_ptr->call_type == CM_CALL_TYPE_VT))
  {
    switch(call_event)
    {
      ...
      case CM_CALL_EVENT_EMERGENCY_FLASHED:
        ...
        data_block_ptr->csfb_type          = call_info_ptr->csfb_type;
 ...
 -----------------------------------------------------------------*/
  ...
  {
    DATA_3GMGR_MSG0(MSG_LEGACY_LOW, "CM_CALL_EVENT_UNBLOCK_ALL_APN received");
  }
  ...
  if(cmd_id  != DS_MIN_CMD)
  {
    ...
    memscpy(&(call_info_cmd_ptr->lte_nas_local_cause),
              sizeof(lte_nas_local_cause_T),
              &(call_info_ptr->mode_info.info.lte_call.esm_local_cause),
              sizeof(lte_nas_local_cause_T));
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DATA_3GMGR_MSG6_EX(MSG_LEGACY_HIGH,
                     ss_info_ptr->asubs_id,
                     "ss event cb: Event:%d asubs_id:%d num_stacks:%d "
                     "changed_fields_subs:0x%x%x is_stack2_operational:%d",
                     ss_event, 
                     ss_info_ptr->asubs_id,
                     ss_info_ptr->number_of_stacks,
                     QWORD_HIGH(ss_info_ptr->changed_fields_subs),
                     QWORD_LOW(ss_info_ptr->changed_fields_subs),
                     ss_info_ptr->is_stack2_operational);

  DATA_3GMGR_MSG5_EX(MSG_LEGACY_HIGH,
                     ss_info_ptr->asubs_id,
  ...
  for(stack_idx=0; stack_idx < ss_info_ptr->number_of_stacks; stack_idx++)
  {
    ...
    DATA_3GMGR_MSG9_EX(MSG_LEGACY_HIGH,
                       ss_info_ptr->asubs_id,
                       "ss event cb: stack[%d] is_operational:%d changed_fields:0x%x%x "
                       "changed_fields2:0x%x%x sys_mode:%d srv_status:%d srv_domain:%d ",
                        stack_idx,
                        ss_info_ptr->stack_info[stack_idx].is_operational,
                        QWORD_HIGH(ss_info_ptr->stack_info[stack_idx].changed_fields),
                        QWORD_LOW(ss_info_ptr->stack_info[stack_idx].changed_fields),
                        QWORD_HIGH(ss_info_ptr->stack_info[stack_idx].changed_fields2),
                        QWORD_LOW(ss_info_ptr->stack_info[stack_idx].changed_fields2),
    ...
    DATA_3GMGR_MSG7_EX(MSG_LEGACY_HIGH,
                       ss_info_ptr->asubs_id,
                       "ss event cb: stack[%d]"
                       "effective_srv_domain:%d  roam_status: %d"
                       " hs_ind:%d hs_call_status:%d "
  ...
  if (ss_event == CM_SS_EVENT_SRV_CHANGED)
  {
    ...
    if (ds707_s101_is_hdr_actv_ho_in_prog() == TRUE &&
        ds3g_is_hdr_in_srvc(ss_info_ptr->asubs_id) == TRUE
       )
    {
      /*----------------------------------------------------------------------
  ...
  if(ss_event == CM_SS_EVENT_SRV_CHANGED)
  {
    ...
    for(stack_idx=0; stack_idx < ss_info_ptr->number_of_stacks; stack_idx++)
    {
         ...
         (DS_3GPP_IS_3GPP_CURRENT_MODE(ss_info_ptr->stack_info[stack_idx].sys_mode)))
      {
        if(ss_info_ptr->stack_info[stack_idx].srv_status == SYS_SRV_STATUS_SRV ||
           (ss_info_ptr->stack_info[stack_idx].srv_status == SYS_SRV_STATUS_LIMITED &&
            ss_info_ptr->stack_info[stack_idx].sys_mode == SYS_SYS_MODE_LTE))
        {
    ...
    /*------------------------------------------------------------------------
      Process the tac/lac info
    -------------------------------------------------------------------------*/
    ds3gstkproc_process_tac_lac_info(ss_info_ptr);                       
  }
  ...
  if (ss_event == CM_SS_EVENT_SRV_CHANGED)
  {
    ds_autoreg_notify_ss_ev_info(ss_info_ptr);
  }
  ...
  if( call_cmd_err != CM_CALL_CMD_ERR_NOERR )
  {
    ...
    switch( call_cmd )
    {
      ...
      case CM_CALL_CMD_PDN_DISCONNECT_REQ:
        ...
        if(call_inst >= DS3GI_MAX_NUM_CALLS)
        {
          DS_3GPP_ERROR_FATAL("call_inst is greater than maximum calls");
          return;
        }
        ...
        else
        {
          ...
          else if (call_cmd_err == CM_CALL_CMD_ERR_CALL_CONTROL_REJECTED)
          {
            call_info_cmd_ptr->end_status = CM_CALL_END_CC_REJECT;
          }
          else
          {
       ...
       case CM_CALL_CMD_RAB_REESTAB:
        if(ds3gsubsmgr_is_device_mode_single_sim() == FALSE)
        {
          ...
          else
          {
            ...
            sys_mode
              = ds3g_get_current_network_mode_ex(ds3gi_call_info[call_inst].subs_id);
      ...
      case CM_CALL_CMD_PS_DATA_AVAILABLE:
            ...
            subs_id_ptr = (sys_modem_as_id_e_type*)cmd_ptr->cmd_payload_ptr;
            *subs_id_ptr = (sys_modem_as_id_e_type)(int)data_block_ptr;
            ds_put_cmd( cmd_ptr );
        break;
      ...
      case CM_CALL_CMD_PDN_CONNECTIVITY_REQ:
        ...
        else
        {
          ...
          call_info_cmd_ptr->call_cmd_err = call_cmd_err;
          call_info_cmd_ptr->call_type = CM_CALL_TYPE_PS_DATA;
          /* We are not populating subs_id here */
  ...
  if( call_cmd_err != CM_CALL_CMD_ERR_NOERR )
  {
    ...
    switch( call_cmd )
    {
      ...
      case CM_CALL_CMD_ANSWER:
        ...
        {
          DATA_3GMGR_MSG2_EX(MSG_LEGACY_HIGH,
                             ds3gi_call_info[call_inst].subs_id,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DATA_3GMGR_MSG3(MSG_LEGACY_MED,
                  ...
                  (sys_modem_as_id_e_type)(int)data_block_ptr, ss_cmd, ss_cmd_err);
...
{
  ds_cmd_type                      *cmd_ptr = NULL;
  ds3g_cm_sub_not_avail_info_type  *subs_info_ptr = NULL;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  cmd_ptr = ds_allocate_cmd_buf(sizeof(ds3g_cm_sub_not_avail_info_type));
  if( (cmd_ptr == NULL) || (cmd_ptr->cmd_payload_ptr == NULL) )
  {
    DS3GPP_ASSERT(0);
    return FALSE;
  ...
  cmd_ptr->hdr.cmd_id = DS_CMD_CM_PH_SUB_NOT_AVAIL;
  subs_info_ptr = (ds3g_cm_sub_not_avail_info_type*)cmd_ptr->cmd_payload_ptr;

  subs_info_ptr->subs_id_mask = subs_id_mask;
  subs_info_ptr->sub_clnup_reason_ptr = sub_clnup_reason_ptr;
...
/*===========================================================================
FUNCTION      DS3GI_CM_PROCESS_SUB_NOT_AVAIL_NOTIFY

DESCRIPTION   This function processes the SUB_NOT_AVAIL indication from CM 
              and performs the proper cleanup. 
...
SIDE EFFECTS  None
===========================================================================*/
void ds3gi_cm_process_sub_not_avail_notify
(
  ds_cmd_type  *cmd_ptr
)
{
  sys_modem_as_id_e_type            subs_id = SYS_MODEM_AS_ID_NONE;
  ds3geventmgr_filter_type          filter_info;
  ds3g_cm_sub_not_avail_info_type  *sub_info_ptr = NULL;
  uint8                             subs_id_mask;
  uint8                            *sub_clnup_reason_ptr = NULL;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  sub_info_ptr = (ds3g_cm_sub_not_avail_info_type*)cmd_ptr->cmd_payload_ptr;
  subs_id_mask = sub_info_ptr->subs_id_mask;
  sub_clnup_reason_ptr = sub_info_ptr->sub_clnup_reason_ptr;
  ...
  /*-------------------------------------------------------------------------- 
    CM sends the reason CM_SUB_CLNUP_REASON_SUBS_NOT_AVAIL in legacy
    subs_not_avail case (No OTASN PDN)
      - Legacy behavior applies
    and,
    when LPM is triggered while subs_not_avail is suspended (OTASN case)
     - In this case MH uses the enum 0 to check if CM wants to fallback to legacy
      scenario. If yes, MH resumes processing of subs_not_avail imm'dly
     - The OTA timer status (if running or not) is checked in the individual
       function calls made .
   
     The below if check is for the case when
        - subs_not_avail is suspended due to OTA timer
        - LPM mode is triggered . In this case, CM will give subs_not_avail CB
          with CM_SUB_CLNUP_REASON_SUBS_NOT_AVAIL and MH should inititate
          graceful cleanup (resume the subs_not_avail)
  ---------------------------------------------------------------------------*/
  if ( ((uint8)*sub_clnup_reason_ptr == CM_SUB_CLNUP_REASON_SUBS_NOT_AVAIL) &&
       (ds3geventmgr_is_sub_not_avail_processing_in_progress(subs_id) == TRUE))
  {
   #ifdef FEATURE_DATA_IS707
     ds707_pkt_mgr_fallback_to_legacy_subs_not_avail_procedure(subs_id);
   #endif /*FEATURE_DATA_IS707*/
    ds_3gpp_pdn_cntxt_fallback_to_legacy_subs_not_avail_proc(subs_id);
  ...
  ds3g_mmgsdi_clear_subs_id_ready_mask(subs_id);
    ...
    if(ds3geventmgr_set_event_info(DS3GEVENTMGR_SUB_NOT_AVAIL_EV,
                                   ...
                                   &filter_info) == FALSE)
    {
      DS_3GMGR_ERROR_LOG_0_EX(MSG_LEGACY_ERROR, subs_id, "Unable to send SUB_NOT_AVAIL EV");
  }

  return;
} /* ds3gi_cm_process_sub_not_avail_notify() */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if((msgrtype == MM_CM_PLMN_LIST_CHANGE_IND) && (dsmsg_ptr != NULL))
  {
    DATA_3GMGR_MSG0_EX(MSG_LEGACY_HIGH,
                       subs_id,
                       "Processing MM_CM_PLMN_LIST_CHANGE_IND");
...
/*===========================================================================
FUNCTION      DS3GI_LB_INIT

DESCRIPTION   This function initializes the Data Services Loopback Manager
              client, and registers for cm ss,call events of interest, namely:
...
SIDE EFFECTS  None
===========================================================================*/
void ds3gi_lb_init( void )
{
  lb_client_status_e_type  ret_val;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if (ds_3gpp_loopback_is_enabled())
  {
    ret_val = ds_3gpp_loopback_client_ss_call_reg( ds3gi_cm_ss_event_cb,
                                        LB_CLIENT_SS_EVENT_REG,
                                        CM_SS_EVENT_SRV_CHANGED);
    if(ret_val != LB_CLIENT_OK)
    {
      DATA_3GMGR_MSG1(MSG_LEGACY_LOW,
                      "DS unable to register LB SS EVENT  CM_SS_EVENT_SRV_CHANGED, "
                      "error %d",(int)(ret_val));
    }

    ret_val = ds_3gpp_loopback_client_call_reg(ds3gi_cm_call_event_cb,
                           LB_CLIENT_EVENT_REG,
                           CM_CALL_EVENT_GET_PDN_CONN_IND
                         );
    if(ret_val != LB_CLIENT_OK)
    {
      DATA_3GMGR_MSG1(MSG_LEGACY_LOW,
                      "DS unable to register LB CM_CALL_EVENT_GET_PDN_CONN_IND, "
                      "error %d",(int)(ret_val));
    }
    ...
    {
      DATA_3GMGR_MSG1(MSG_LEGACY_LOW,
                      "DS unable to register LB CM_CALL_EVENT_ACT_BEARER_IND"
                      "error %d",(int)(ret_val));
    }
    ...
    {
      DATA_3GMGR_MSG1(MSG_LEGACY_LOW,
                      "DS unable to register LB CM_CALL_EVENT_CONNECT"
                      "error %d",(int)(ret_val));
    }
    ...
    {
      DATA_3GMGR_MSG1(MSG_LEGACY_LOW,
                      "DS unable to register LB CM_CALL_EVENT_RAB_REESTAB_IND"
                      "error %d",(int)(ret_val));
    }
    ...
    if(ret_val != LB_CLIENT_OK)
    {
      DATA_3GMGR_MSG1(MSG_LEGACY_ERROR,
                      "DS unable to register LB CM_CALL_EVENT_RAB_REL_IND"
                      "error %d",(int)(ret_val));
    }
    ...
    {
      DATA_3GMGR_MSG1(MSG_LEGACY_ERROR,
                      "DS unable to register LB CM_CALL_EVENT_RAB_REESTAB_REJ"
                      "error %d",(int)(ret_val));
    }
    ...
    {
      DATA_3GMGR_MSG1(MSG_LEGACY_LOW,
                      "DS unable to register LB CM_CALL_EVENT_END"
                      "error %d",(int)(ret_val));
    }
    return;
  }
...
/*===========================================================================
FUNCTION      DS3GI_PROCESS_RAT_EXPANSION

DESCRIPTION   This function process the RAT expansion.
...
SIDE EFFECTS  None
===========================================================================*/
void ds3gi_process_rat_expansion
(
...
)
{
  ...
  cm_rat_expansion_status_e_type  rat_expansion_status;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if( ph_info_ptr == NULL )
  {
    return;
  ...
  old_mode_pref = ds3gsubsmgr_get_mode_pref_for_subs_id(ph_info_ptr->as_id);
  ds3g_subs_id = ds3gsubsmgr_subs_id_cm_to_ds3g( ph_info_ptr->as_id );

  if( ds3gsubsmgr_is_ds3g_subs_index_valid(ds3g_subs_id) )
  {
    /*---------------------------------------------------------------------
      Check if RAT expansion is in progress or not
    ---------------------------------------------------------------------*/
    rat_expansion_status =
      cm_get_rat_expansion_status( old_mode_pref,
                                   ph_info_ptr->mode_pref,
                                   ds3g_cm_rat_disabled_mask[ds3g_subs_id],
                                   ph_info_ptr->rat_disabled_mask );

    DATA_3GMGR_MSG1_EX(MSG_LEGACY_HIGH,
                       ph_info_ptr->as_id, 
                       "rat_expansion_status:%d",
					   rat_expansion_status);

    /*-------------------------------------------------------------------
      Cache RAT disabled mask
    -------------------------------------------------------------------*/
    ds3g_cm_rat_disabled_mask[ds3g_subs_id] =
      ph_info_ptr->rat_disabled_mask;

    /*-------------------------------------------------------------------
      Send notification to DSD via DSDIF regarding RAT expansion
    -------------------------------------------------------------------*/
    switch( rat_expansion_status )
    {
      case CM_RAT_EXPANSION_STATUS_ACTIVATED:
        ds3gdsdif_notify_dsd_rat_expansion(ph_info_ptr->as_id, TRUE);
        break;

      case CM_RAT_EXPANSION_STATUS_DEACTIVATED:
        ds3gdsdif_notify_dsd_rat_expansion(ph_info_ptr->as_id, FALSE);
        break;
...
#ifdef FEATURE_QSH_MDUMP
/*===========================================================================
FUNCTION ds3gi_qsh_mdump_collect
...
===========================================================================*/ 
QSH_MDUMP_FN_ATTR void ds3gi_qsh_mdump_collect()
{
  qsh_mdump_collect_high(&ds3gi_plmn_id, 
  	DS3G_MAX_SUBS * sizeof(sys_plmn_id_s_type));
}
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  else
  {
    ...
    {
      DS_3GPP_ERROR_FATAL("ds3gi_reg_ac_event error!");
    }
#endif
#endif /* FEATURE_EHRPD */
    ...
    {
      DS_3GPP_ERROR_FATAL("DS unable to register CM_CALL_EVENT_EXIT_TC with CM");
      return;
    }
#endif /* FEATURE_DATA_IS707 */
    ...
    ds3gmsgr_reg_msgr_cb(MM_CM_PLMN_LIST_CHANGE_IND,
                                     (ds3g_msgr_client_func_ptr)ds3gi_cm_plmn_list_change_ind_msg_hdlr);
  }

  if (CM_CLIENT_OK != cm_client_ph_reg(ds3gi_cm_client_id,
                                       ds3gi_cm_ph_event_cb,
                                       CM_CLIENT_EVENT_REG,
  ...
  {
    DS_3GMGR_ERROR_LOG_0(MSG_LEGACY_ERROR, 
                    "Unable to register for Test Control event"
                   );
  }
  ...
  if (CM_CLIENT_OK != cm_client_ph_reg(ds3gi_cm_client_id,
                                       ds3gi_cm_ph_event_cb,
                                       CM_CLIENT_EVENT_REG,
                                       CM_PH_EVENT_DDS_SWITCH_ALLOWED,
                                       CM_PH_EVENT_DDS_SWITCH_ALLOWED,
                                       NULL))
  {
    DS_3GMGR_ERROR_LOG_0(MSG_LEGACY_ERROR,
                    "Unable to register for DDS switch allowed event"
                   );
  }
#endif
 ...
 /*-----------------------------------------------------------------------
  ...
  -----------------------------------------------------------------------*/
  ds3g_cm_ph_cmd_cb_f_ptr = ds3g_cm_ph_cb;

  for (subs_index = DS3GSUBSMGR_SUBS_ID_MIN;subs_index < DS3GSUBSMGR_SUBS_ID_MAX;subs_index++)
  {
    memset(&ds3gi_cm_ph_cb_info[subs_index],0,sizeof(ds3g_cm_ph_cb_info));
  }
...
{
  sys_sys_mode_e_type  call_mode = SYS_SYS_MODE_NO_SRV;
  ds_call_type_e_type  call_type = DS_CALL_TYPE_CKT;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  call_mode = call_inst_ptr->call_mode;
  call_type = call_inst_ptr->call_type;

  if( (call_mode  < SYS_SYS_MODE_NO_SRV) ||
      (call_mode >= SYS_SYS_MODE_MAX) ||
      (call_type >= DS_NUM_CALL_TYPES) ||
      (call_type == DS_CALL_TYPE_UNSPECIFIED))
  {
     DS_3GMGR_ERROR_LOG_2_EX(MSG_LEGACY_ERROR, call_inst_ptr->subs_id,
  ...
  {
    return (ds3gi_hdlr_tbl[call_mode][call_type]);
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  for (instance = 0; instance < DS3GI_MAX_NUM_CALLS; instance++)
  {
        if (ds3gi_call_info[instance].call_id == CM_CALL_ID_INVALID)
        {
          if(ds3gi_call_info[instance].call_state != DS3G_IDLE_CALL_STATE)
          {
            DS_3GPP_ERROR_FATAL("Call is invalid but call state is not idle");
            return DS3GI_INVALID_CALL_INSTANCE;
          }
          break;
        }
  ...
  if( instance < DS3GI_MAX_NUM_CALLS )
  {
    ...
    ds3gi_call_info[instance].call_end_recd = FALSE;
    ds3gi_call_info[instance].call_time_orig = time_get_uptime_secs();
    return instance;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if(instance >= DS3GI_MAX_NUM_CALLS)
  {
    DS_3GPP_ERROR_FATAL("call instance greater than maximum allowed");
    return;
  }
  ...
  ds3gi_call_info[instance].call_end_recd = FALSE;
  ds3gi_call_info[instance].call_time_orig = 0;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (instance == DS3GI_MAX_NUM_CALLS)
  {
    DS_3GMGR_ERROR_LOG_1(MSG_LEGACY_ERROR, "Call_id is not found d", cm_call_id);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (call_inst >= DS3GI_MAX_NUM_CALLS)
  {
    DATA_3GMGR_MSG1_EX(MSG_LEGACY_HIGH, 
                       call_info_cmd_ptr->subs_id,
                       "CM_CALL_END_CMD with bad call_id %d",
                       call_info_cmd_ptr->call_id);
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(cc_info_ptr == NULL)
  {
    DS_3GPP_ERROR_FATAL("NULL passed for cc_info_ptr");
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_3GPP_ERROR_FATAL("Calls active in other modes");
    return;
  }
  ...
  else if( !ds3gi_call_limit_info[inc_call_mode].cs_and_ps_allowed_flag )
  {
    ...
    else
    {
      ...
      -----------------------------------------------------------------------*/
      DS_3GMGR_ERROR_LOG_1(MSG_LEGACY_ERROR,
        ...
        if(ds3gi_call_info[force_down_call_inst].call_state == DS3G_ORIG_CALL_STATE)
        {
          ...
          if((hdlr_tbl_ptr->force_call_down_handler != NULL) )
          {
                                          ...
                                          call_info_cmd_ptr->subs_id))
            {
              ...
              if (cm_ret_val == FALSE)
              {
                DS_3GPP_ERROR_FATAL("CM call cmd end failed");
                return DS3G_FAILURE;
              }
              else
              {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_3GPP_ERROR_FATAL("Heap Memory Allocation failed");
    return;
  }
        ...
        DS3G_CALL_IS_ETSI_MODE( inc_call_mode ) ))
  {
    ...
    if( DS3G_SUCCESS != ds3gi_manage_terminating_calls( cmd_ptr,
                                          ...
                                          call_info_cmd_ptr->subs_id))
    {
      DS_3GMGR_ERROR_LOG_0_EX(MSG_LEGACY_ERROR, 
                              call_info_cmd_ptr->subs_id, 
                              "Cannot manage terminating call");
  ...
  if( cc_info->accept_inc_call )
  {
    ...
    if( !cc_info->switch_inc_call )
    {
      ...
      --------------------------------------------------------------------*/
        if( hdlr_tbl_ptr != NULL &&
            hdlr_tbl_ptr->incoming_call_handler != NULL )
        {
          /*------------------------------------------------------------------
        ...
        } /* if( hdlr_tbl_ptr != NULL && hdlr_tbl_ptr->incoming_call_handler != NULL ) */
        else
        {
          DATA_3GMGR_MSG0(MSG_LEGACY_LOW,
  ...
  if ( cc_info->action != DS3G_REJECT_CALL )
  {
    ...
    {
      modem_mem_free( cc_info, MODEM_MEM_CLIENT_DATA_CRIT );
      DS_3GPP_ERROR_FATAL("call instance value greater than Maximum calls");
      return;
    }
    ds3gi_call_info[inc_call_inst].call_id = call_info_cmd_ptr->call_id;
    ds3gi_call_info[inc_call_inst].call_type = inc_call_type;
  ...
  if( cc_info->action == DS3G_ANSWER_CALL )
  {
    ...
    {
      modem_mem_free( cc_info, MODEM_MEM_CLIENT_DATA_CRIT );
      DS_3GPP_ERROR_FATAL("CM call cmd answer failed");
      return;
    }
  }
  else if( cc_info->action == DS3G_ALERT_USER )
  {
    ...
    DATA_3GMGR_MSG0_EX(MSG_LEGACY_HIGH, 
                       call_info_cmd_ptr->subs_id,
                       "Mode-Sp. Hdlr says alert user");
    
    if (inc_call_inst >= DS3GI_MAX_NUM_CALLS)
    {
       modem_mem_free( cc_info, MODEM_MEM_CLIENT_DATA_CRIT );
       DS_3GPP_ERROR_FATAL("call_instance greater than Maximum calls");
       return;
    }
  ...
  else if( cc_info->action == DS3G_REJECT_CALL )
  {
    ...
    DATA_3GMGR_MSG0_EX(MSG_LEGACY_HIGH, 
		               call_info_cmd_ptr->subs_id,
		               "Mode-Sp. Hdlr rejected incoming call");
  ...
  else if( cc_info->action == DS3G_DO_NOTHING )
  {
    ...
    {
      modem_mem_free( cc_info, MODEM_MEM_CLIENT_DATA_CRIT );
      DS_3GPP_ERROR_FATAL("call instance is greater than maximum calls");
      return;
    }
    /*-----------------------------------------------------------------
      Set the call state to indicate that we are waiting for the call
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DATA_3GMGR_MSG0_EX( MSG_LEGACY_HIGH,
                      call_info_cmd_ptr->subs_id,
                      "Rx'ed CM_CALL_INCOMING_PAGE_CMD" );
  ...
  if ( page_response == DS3G_ANSWER_CALL )
  {
    DATA_3GMGR_MSG0_EX(MSG_LEGACY_HIGH, 
		               call_info_cmd_ptr->subs_id,
		               "Mode handler accepts incoming page");
    ...
    if (cm_ret_val == FALSE)
    {
      DS_3GMGR_ERROR_LOG_0_EX(MSG_LEGACY_ERROR, call_info_cmd_ptr->subs_id, 
  ...
  else if( page_response == DS3G_REJECT_CALL )
  {
    ...
    DATA_3GMGR_MSG0_EX(MSG_LEGACY_HIGH, 
		               call_info_cmd_ptr->subs_id,
		               "Mode handler rejects incoming page");
    cm_ret_val = cm_mm_call_cmd_incoming_page_resp( ds3gi_cm_call_cmd_cb,
                                                    NULL,
                                                    ds3g_cm_client_id,
                                                    FALSE );
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (ds3gi_get_call_mode(cmd_ptr) != ds3gi_call_info[call_inst].call_mode)
  {
    DS_3GMGR_ERROR_LOG_2_EX(MSG_LEGACY_ERROR, 
                    ds3gi_call_info[call_inst].subs_id,
                    "CM_CALL_CONN with bad mode d wanted d",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DATA_3GMGR_MSG1_EX(MSG_LEGACY_HIGH,
                     call_info_cmd_ptr->subs_id,
                     "Rx'ed CM_CALL_SETUP_CMD, call id: %d",
                     call_info_cmd_ptr->call_id);
  ...
  {
    DS_3GPP_ERROR_FATAL(" call mode is not in ETSI mode");
    return;
  }
  ...
  if( DS3G_SUCCESS != ds3gi_manage_terminating_calls( cmd_ptr,
                                          ...
                                          call_info_cmd_ptr->subs_id))
  {
    DATA_3GMGR_MSG0(MSG_LEGACY_LOW,"Cannot manage terminating call");
    modem_mem_free( cc_info, MODEM_MEM_CLIENT_DATA_CRIT );
  ...
  if ( cc_info->action != DS3G_REJECT_CALL )
  {
    ...
    if( DS3GI_INVALID_CALL_INSTANCE != inc_call_inst )
    {
      for(call_inst_itr=0;call_inst_itr<DS3GI_MAX_NUM_CALLS;call_inst_itr++)
      {
        ...
        {
          DS_3GMGR_ERROR_LOG_1_EX(MSG_LEGACY_ERROR, call_info_cmd_ptr->subs_id,
...
func_exit:
  ...
  {
    DS_3GPP_ERROR_LOG_1_EX(call_info_cmd_ptr->subs_id, 
                           "ds3gi_process_cm_call_setup_cmd: err d",
                           local_err);
  }
  return;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (call_inst >= DS3GI_MAX_NUM_CALLS)
  {
...
#endif /* FEATURE_MULTIMEDIA_FALLBACK */
    {
      DATA_3GMGR_MSG0_EX(MSG_LEGACY_HIGH, 
                         call_info_cmd_ptr->subs_id,
                         "CM_CALL_CONF_CMD has unknown call id");
      return;
    }
  }
  else
...
)
{
  ...
  uint32                        call_inst = DS3GI_DEFAULT_CALL_INSTANCE;
  boolean                       cm_ret_val = FALSE;
  sys_sys_mode_e_type           inc_call_mode = SYS_SYS_MODE_NONE;
  cm_call_mode_info_e_type      inc_info_type;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -- - -*/
  ...
  inc_info_type = call_info_cmd_ptr->mode_info.info_type;

  if (inc_info_type != CM_CALL_MODE_INFO_GW_PS )
  {
    DS_3GMGR_ERROR_LOG_0_EX(MSG_LEGACY_ERROR, call_info_cmd_ptr->subs_id, 
                    "Info_type is not GW_PS, return");
    return;
  }
  else
  {
    inc_call_mode = SYS_SYS_MODE_GSM;
    DATA_3GMGR_MSG0(MSG_LEGACY_LOW, "Setting inc_call_mode to GSM");
  ...
  if (ds3gi_call_info[call_inst].call_state == DS3G_CALL_ACTIVE_CALL_STATE)
    {
     ...
     hdlr_tbl_ptr = ds3gi_hdlr_tbl[inc_call_mode][DS_CALL_TYPE_PKT];
  ...
  if ( action != DS3G_REJECT_CALL )
  {
    ...
    if (cm_ret_val == FALSE)
    {
      DS_3GPP_ERROR_FATAL("CM call cmd modify resp failed");
      return;
    }
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -- - -*/
  ...
  DATA_3GMGR_MSG1_EX(MSG_LEGACY_HIGH, 
                     call_info_cmd_ptr->subs_id,
                     "Rx'ed CM_CALL_PROGRESS_CMD, call id: %d",
                     call_info_cmd_ptr->call_id);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DATA_3GMGR_MSG0_EX(MSG_LEGACY_HIGH, 
                     call_info_cmd_ptr->subs_id,
                     "Rx'ed CM_CALL_EVENT_PDN_CONN_REJ_IND");
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DATA_3GMGR_MSG0_EX(MSG_LEGACY_HIGH, 
                     call_info_cmd_ptr->subs_id,
                     "Rx'ed CM_CALL_EVENT_PDN_CONN_FAIL_IND");
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_3GPP_ERROR_FATAL("Attach request sequence number is 0,ASSERT !");
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  call_activate_response =
    (cm_act_bearer_rsp_s_type *)modem_mem_alloc
        (sizeof(cm_act_bearer_rsp_s_type),
         MODEM_MEM_CLIENT_DATA);

  if (call_activate_response == NULL)
  {
    DS_3GPP_ERROR_FATAL("Memory Allocation Failed.");
    local_err = MSG_LEGACY_LOW;
    goto func_exit;
  }
  ...
  /* using cmd_ptr->cmd.call_info.subs_id 
   which might be filled by CM, have to confirm that */
  ...
  for(call_inst_itr=0;call_inst_itr<DS3GI_MAX_NUM_CALLS;call_inst_itr++)
  {
    if(ds3gi_call_info[call_inst_itr].call_id == call_info_cmd_ptr->call_id)
    {
       ...
       if( hdlr_tbl_ptr->bearer_reactivate_handler == NULL )
       {
          DATA_3GMGR_MSG0(MSG_LEGACY_LOW, "reactivate bearer ind handler not registered");
          local_err = DS_LOCAL_ERR_VAL_5;
          goto func_exit;
       }

      /*Scenario where call is already activated indicates dirction change */
       action = hdlr_tbl_ptr->bearer_reactivate_handler(
             ds3gi_call_info[call_inst_itr].call_id,
             &(call_info_cmd_ptr->mode_info),
             call_activate_response,
             call_info_cmd_ptr->subs_id);

       call_activate_response->is_accept = action;
       ...
       /*-------------------------------------------------------------------
         Bearer configuration changed. We are done processing this config
         change. We can return from this function now.
       --------------------------------------------------------------------*/
  ...
  if( inc_call_inst >= DS3GI_MAX_NUM_CALLS )
  {
	...
	DSMGR_SET_ESM_CAUSE(lte_info_ptr, NAS_ESM_INSUFFICIENT_RESOURCES);
  ...
  if(action == DS3G_SUCCESS)
  {
    if( inc_call_inst < DS3GI_MAX_NUM_CALLS )
    {
      ...
      if (ds_3gpp_loopback_is_enabled())
      {
        cm_ret_val = ds_3gpp_loopback_act_bearer_rsp(
                                        ds3gi_call_info[inc_call_inst].call_id,
                                        call_activate_response
                                      );
      }
    ...
    {
      modem_mem_free((void*)call_activate_response, MODEM_MEM_CLIENT_DATA);
      DS_3GPP_ERROR_FATAL("CM call cmd act bearer rsp failed");
      return;
    }
  }
  ...
  if(action == DS3G_FAILURE)
  {
    DATA_3GMGR_MSG0_EX(MSG_LEGACY_HIGH, 
                       call_info_cmd_ptr->subs_id,
                       "Mode Hdlr rejected the call");
    call_activate_response->is_accept = FALSE;

    if(lte_info_ptr->esm_cause.valid)
    {
	  DSMGR_SET_ESM_CAUSE(call_activate_response, 
	                      lte_info_ptr->esm_cause.esm_cause);

    }
    else
    {
      DATA_3GMGR_MSG0(MSG_LEGACY_LOW,"ESM Cause code is not valid");
      local_err = DS_LOCAL_ERR_VAL_2;
    }
...
func_exit:
  ...
  {
    DS_3GPP_ERROR_LOG_1_EX(call_info_cmd_ptr->subs_id,
                           "ds3gi_process_cm_call_act_bearer_ind_cmd: err d",
                           local_err);
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DATA_3GMGR_MSG0_EX(MSG_LEGACY_HIGH, 
                     call_info_cmd_ptr->subs_id,
                     "Rx'ed CM_CALL_EVENT_RES_ALLOC_REJ_IND");
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DATA_3GMGR_MSG0_EX(MSG_LEGACY_HIGH, 
                     call_info_cmd_ptr->subs_id,
                     "Rx'ed CM_CALL_EVENT_RES_ALLOC_FAIL_IND");
  ...
  {
    DS_3GMGR_ERROR_LOG_1_EX(MSG_LEGACY_ERROR, call_info_cmd_ptr->subs_id,
                    "Received RES ALLOC Fail Ind in wrong moded",
                    inc_call_mode);
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DATA_3GMGR_MSG0_EX(MSG_LEGACY_HIGH, 
                     call_info_cmd_ptr->subs_id,
                     "Rx'ed CM_CALL_EVENT_BEARER_MOD_REJ_IND");
  ...
  {
    DS_3GMGR_ERROR_LOG_1_EX(MSG_LEGACY_ERROR, call_info_cmd_ptr->subs_id,
                    "Received Bearer Mod Rej Ind in wrong moded",
                    inc_call_mode);
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DATA_3GMGR_MSG0_EX(MSG_LEGACY_HIGH, 
                     call_info_cmd_ptr->subs_id,
                     "Rx'ed CM_CALL_EVENT_BEARER_MOD_IND");
  ...
  if(inc_info_type != CM_CALL_MODE_INFO_LTE)
  {
    DATA_3GMGR_MSG1(MSG_LEGACY_LOW,
                    "Received Bearer Mod Ind in wrong info_type %d",
                    inc_info_type);
  ...
  if(action == DS3G_SUCCESS)
  {
    ...
    if (cm_ret_val == FALSE)
    {
      DS_3GPP_ERROR_FATAL("CM call cmd bearer mod accept rsp failed");
  ...
  if(action == DS3G_FAILURE)
  {
    ...
    if (cm_ret_val == FALSE)
    {
      DS_3GPP_ERROR_FATAL("CM call cmd bearer mod reject rsp failed");
      modem_mem_free((void *)call_mod_resp, MODEM_MEM_CLIENT_DATA_CRIT );
      local_err = DS_LOCAL_ERR_VAL_3;
      goto func_exit;
    }
...
func_exit:
  ...
  {
    DS_3GPP_ERROR_LOG_1_EX(call_info_cmd_ptr->subs_id,
                           "ds3gi_process_cm_call_bearer_mod_ind_cmd: err d",
                           local_err);
  }
  return;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DATA_3GMGR_MSG0_EX(MSG_LEGACY_HIGH, 
                     subs_id,
                     "Rx'ed CM_SS_3GPP_CNTXT_TRANSFER_CMD");
  ...
  if(!ds3gsubsmgr_is_subs_id_valid(subs_id))
  {
    ...
    {
      DS_3GMGR_ERROR_LOG_2_EX(MSG_LEGACY_ERROR, subs_id,
    ...
    if( hdlr_tbl_ptr->rab_rel_handler != NULL )
    {
      ...
      hdlr_result =
        hdlr_tbl_ptr->rab_rel_handler(ds3gi_call_info[call_inst].call_id,
                                      &end_params,
                                      call_info_cmd_ptr->subs_id,
                                      &call_info_cmd_ptr->lte_nas_local_cause);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -- - -*/
  ...
  DATA_3GMGR_MSG2_EX(MSG_LEGACY_HIGH, 
                     call_info_cmd_ptr->subs_id,
                     "Rx'ed CM_CALL_RAB_REESTAB_IND_CMD, DL DIR: %d, UL DIR: %d",
                     call_info_cmd_ptr->mode_info.info.lte_call.dl_bearer,
                     call_info_cmd_ptr->mode_info.info.lte_call.ul_bearer);
#else
  DATA_3GMGR_MSG0_EX(MSG_LEGACY_HIGH, 
                     call_info_cmd_ptr->subs_id,
                     "Rx'ed CM_CALL_RAB_REESTAB_IND_CMD");
#endif /* FEATURE_LAPP */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DATA_3GMGR_MSG0_EX(MSG_LEGACY_HIGH, 
                     call_info_cmd_ptr->subs_id,
                     "Rx'ed CM_CALL_EVENT_RAB_REESTAB_REJ_CMD");
  ...
  if (ds3gi_call_info[call_inst].call_state == DS3G_CALL_ACTIVE_CALL_STATE)
  {
    ...
    if( hdlr_tbl_ptr->rab_re_estab_rej_handler != NULL )
    {
      ...
      switch (call_info_cmd_ptr->mode_info.info_type)
      {
        ...
        DATA_3GMGR_MSG1_EX(MSG_LEGACY_HIGH, 
                           call_info_cmd_ptr->subs_id,
                           "CM_CALL_EVENT_RAB_REESTAB_REJ_CMD "
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -- - -*/
  ...
  DATA_3GMGR_MSG0_EX(MSG_LEGACY_HIGH, 
                     call_info_cmd_ptr->subs_id,
                     "Rx'ed CM_CALL_EVENT_RAB_REESTAB_FAIL_CMD");
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DATA_3GMGR_MSG4_EX(MSG_LEGACY_HIGH,
                     ss_event_info_ptr->asubs_id,
                     "ds3gi_process_cm_ss_srv_chg_cmd: asubs_id:%d num_stacks:%d "
                     "changed_fields_subs:0x%x%x",
                     ss_event_info_ptr->asubs_id,
                     ss_event_info_ptr->number_of_stacks,
                     QWORD_HIGH(ss_event_info_ptr->changed_fields_subs),
                     QWORD_LOW(ss_event_info_ptr->changed_fields_subs));
  ...
  for(stack_idx=0; stack_idx<ss_event_info_ptr->number_of_stacks; stack_idx++)
  {
    ...
    DATA_3GMGR_MSG5_EX(MSG_LEGACY_HIGH,
                       ...
                       "changed_fields:0x%x%x changed_fields2:0x%x%x",
                       stack_idx,
                       QWORD_HIGH(ss_event_info_ptr->stack_info[stack_idx].changed_fields),
                       QWORD_LOW(ss_event_info_ptr->stack_info[stack_idx].changed_fields),
                       QWORD_HIGH(ss_event_info_ptr->stack_info[stack_idx].changed_fields2),
                       QWORD_LOW(ss_event_info_ptr->stack_info[stack_idx].changed_fields));
  }
...
/*===========================================================================
FUNCTION      DS3GI_DISPATCH_CM_PH_CB

DESCRIPTION   This function dispatches cm ph info cb function ptr
...
)
{
  ...
  DS_3GPP_MSG0_HIGH_EX(ph_info_ptr->as_id, "In ds3gi_dispatch_cm_ph_cb");
  /*-------------------------------------------------------------------- 
    Dispatch cb's to all appropriate clients
    If ph info ptr is received as NULL, it means CM PH GET resulted in
    a cb error, IT is the clients responsibility to handle this case
   --------------------------------------------------------------------*/
  cb_cnt = ds3gi_cm_ph_cb_info[ph_info_ptr->as_id].cb_cnt;

  for (cb_loop_cnt = 0; cb_loop_cnt < cb_cnt; cb_loop_cnt++)
  {
     if((ds3gi_cm_ph_cb_info[ph_info_ptr->as_id].cm_ph_cb_info[cb_loop_cnt] != NULL) &&
        (ds3gi_cm_ph_cb_info[ph_info_ptr->as_id].cm_ph_cb_info[cb_loop_cnt]->cm_ph_cb_fn != NULL))
     {
       ds3gi_cm_ph_cb_info[ph_info_ptr->as_id].cm_ph_cb_info[cb_loop_cnt]->cm_ph_cb_fn(ph_info_ptr);
     }
  }

}/*ds3gi_dispatch_cm_ph_cb*/
...
)
{
  ...
  cm_mode_pref_e_type       old_mode_pref;
  sys_oprt_mode_e_type      old_oprt_mode;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (!ds3gsubsmgr_is_subs_id_valid(ph_info_ptr->as_id))
  {
    return;
  }  
  old_mode_pref = ds3gsubsmgr_get_mode_pref_for_subs_id(ph_info_ptr->as_id);
  old_oprt_mode = ds_3gpp_cm_ph_info_oprt_mode;
  /*-------------------------------------------------------------------------
    Get the CDMA subscription mode. 
  -------------------------------------------------------------------------*/
#ifdef FEATURE_DEDICATED_C2K_MODE_DATA
  if(ds3gsubsmgr_is_device_multi_sim_mode() == TRUE)
  {
    curr_mode = ds3g_get_current_network_mode_ex(ds707_curr_subs_id());
  }
  else
  {
    curr_mode = ds3g_get_current_network_mode_ex(ph_info_ptr->as_id);
  }
#endif /* FEATURE_DEDICATED_C2K_MODE_DATA */
  ...
  switch( ph_info_ptr->ph_event )
  {
    case CM_PH_EVENT_OPRT_MODE:
    {
      ...
      if(ph_info_ptr->oprt_mode != ds_3gpp_cm_ph_info_oprt_mode)
      {
        DATA_3GMGR_MSG3_EX(MSG_LEGACY_HIGH,
                           ph_info_ptr->as_id,
                           "oprt_mode change: Old_mode:%d, new_mode:%d, old_mode_pref: %d",
        ...
        ds_3gpp_process_oprt_mode_chg_cmd(ph_info_ptr->oprt_mode,
                                          ph_info_ptr->as_id);
        ds_umts_tlb_handle_oprt_mode_ev(ph_info_ptr->oprt_mode, 
                                        ph_info_ptr->as_id,
                                        ph_info_ptr->test_control_type);
      }

      /*---------------------------------------------------------------------
        Notify subsmgr if mode preference has changed
      ---------------------------------------------------------------------*/
      if(ph_info_ptr->mode_pref != old_mode_pref)
      {
      ...
      /*---------------------------------------------------------------------
        Process RAT expansion
      ---------------------------------------------------------------------*/
      ds3gi_process_rat_expansion(ph_info_ptr);
      /*---------------------------------------------------------------------
        If the oprt mode is ONLINE, reset the LPM bits
      ---------------------------------------------------------------------*/
      if (SYS_OPRT_MODE_ONLINE == ph_info_ptr->oprt_mode) 
      {
        ds3geventmgr_clear_lpm_pwr_off_status();
        if(SYS_OPRT_MODE_LPM == old_oprt_mode)
        {
          ds3geventmgr_set_mode_online_evt(ph_info_ptr->as_id);
        }
      }
    }
    break;
    ...
    case CM_PH_EVENT_INFO:
    {
      ...
      if(((curr_mode == SYS_SYS_MODE_CDMA) ||
         ...
         ((ph_info_ptr->device_mode == SYS_MODEM_DEVICE_MODE_SINGLE_SIM) ||
           (ph_info_ptr->as_id == ds707_curr_subs_id())))
      {
        ph_info_ptr->emerg_to_normal_mode = FALSE;
      }
#endif /* FEATURE_DEDICATED_C2K_MODE_DATA */
      ...
      /*-------------------------------------------------------------------- 
        Dispatch CM PH info for all registered clients
        --------------------------------------------------------------------*/
      ds3gi_dispatch_cm_ph_cb(ph_info_ptr);
    ...
    case CM_PH_EVENT_TEST_CONTROL_TYPE:
      ds_umts_tlb_handle_test_control_ev(ph_info_ptr->test_control_type, 
                                         ph_info_ptr->as_id);
      break;
...
#ifdef FEATURE_DATA_IS707
      /*----------------------------------------------------------------
        Tear down 3GPP2 Ifaces on DDS switch allowed indication from CM.
      ----------------------------------------------------------------*/
      if(ph_info_ptr->as_id == ds707_curr_subs_id())
      {
        ds707_pkt_mgr_local_abort(ph_info_ptr->as_id, 
                                  PS_NET_DOWN_REASON_DDS_CALL_ABORT);
      }
#endif /* FEATURE_DATA_IS707 */

      ds3g_dds_switch_hdlr_dds_switch_allowed_event();
#endif
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DATA_3GMGR_MSG0_EX(MSG_LEGACY_HIGH, 
                     call_info_cmd_ptr->subs_id,
                     "Rx'ed CM_CALL_EVENT_PDP_MODIFY_IND");
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DATA_3GMGR_MSG0_EX(MSG_LEGACY_HIGH, 
                     call_info_cmd_ptr->subs_id,
                     "Rx'ed CM_CALL_EVENT_PDP_PROMOTE_IND");
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  mode = ds3g_get_current_network_mode_ex((*subs_id_ptr));
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch ( standby_pref )
  {
    case SYS_MODEM_DS_PREF_SINGLE_STANDBY:
      ...
      /* SET PS SUBS id */
      ds3gsubsmgr_set_def_data_subs_id_ex( ph_info_ptr->default_data_subs,
                                           DDS_CAUSE_PERMANENT );
      /* SET Voice SUBS id */
      ...
      DATA_MSG1_EX(MSG_LEGACY_HIGH, 
                   ps_subs_id,
                   "Current ps sub id is %d", ps_subs_id);
    ...
    case SYS_MODEM_DS_PREF_TRIPLE_STANDBY:
      ...
      DATA_3GMGR_MSG3_EX(MSG_LEGACY_MED,
                         ps_subs_id,
                         "Received Standby Pref %d "
                         "voice sub id[%d], data sub id[%d]",
                         standby_pref,
                         ph_info_ptr->default_voice_subs,
                         ph_info_ptr->default_data_subs);
      ...
      /* SET PS SUBS id */
      ds3gsubsmgr_set_def_data_subs_id_ex( new_subs_id,
                                           ph_info_ptr->dds_switch_type );
      /* SET Voice SUBS id */
      ds3gsubsmgr_set_def_voice_subs_id(ph_info_ptr->default_voice_subs);
      ds3gsubsmgr_set_standby_pref(standby_pref);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DATA_3GMGR_MSG9_EX(MSG_LEGACY_MED,
                    ps_subs_id,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  DATA_3GMGR_MSG0_EX(MSG_LEGACY_MED, 
                     subs_id_msk,
                     "cm_data_packet_state_notify() called");
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DATA_3GMGR_MSG4_EX(MSG_LEGACY_MED,
                    subs_id,
                    "sys_change_handler. old_mode:%d, new_mode:%d "
...
)
{
  ...
  boolean                     ignore_cm_plmn_info = FALSE;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /*-------------------------------------------------------------------------
    Return early if PLMN ID passed to this function is zero
  --------------------------------------------------------------------------*/
  if(memcmp(&plmn_info.plmn_id, &zero_plmn, sizeof(sys_plmn_id_s_type)) == 0)
  {    
    return;
  }
  ...
  memset(&old_plmn_id, 0, sizeof(sys_plmn_id_s_type));
  ...
  if ((ds3gdsdif_is_lte_attach_in_progress(plmn_info.subs_id)) && 
      ((plmn_info.changed_fields & CM_SS_SYS_ID_MASK) == 0))
  {
    ignore_cm_plmn_info = TRUE;
  } 

  ds_subs_id = ds3gsubsmgr_subs_id_cm_to_ds3g(subs_id);

  if ((DS3GSUBSMGR_SUBS_ID_INVALID != ds_subs_id) &&
      (ignore_cm_plmn_info == FALSE))
  {
    memscpy(&old_plmn_id, sizeof(sys_plmn_id_s_type),
    ...
    /*-----------------------------------------------------------------------
     ...
     1. Network override set on RAT x on PLMN 1
     2. Context transfer to RAT y on PLMN2
     3. Context transfer back to RAT x on PLMN2.
     4. From DS perspective PLMN hasn't changed across RATs. 
     5. However, in this case throttling on RAT x should be cleared since
        PLMN has changed from PLMN1 to PLMN2 on RAT x. 
    -----------------------------------------------------------------------*/
    else
    {
      ds_3gpp_pdn_throt_clr_on_plmn_change(sys_mode, new_plmn_id, subs_id);
    }
    /*---------------------------------------------------------------------
      Generate PLMN ID change event.
    ----------------------------------------------------------------------*/

    memset(&filter_info, 0, sizeof(ds3geventmgr_filter_type));
    filter_info.ps_subs_id = (ps_sys_subscription_enum_type)ds3gsubsmgr_subs_id_cm_to_ds(subs_id);
    filter_info.tech = PS_SYS_TECH_ALL;
    ds3geventmgr_set_event_info(DS3GEVENTMGR_PLMN_CHG_EV,NULL,&filter_info);
  }
  return;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  else
  {
    ...
    plmn_info.changed_fields = changed_fields;
...
/*===========================================================================
FUNCTION      DS3G_CLEAR_CURRENT_PLMN_ID

DESCRIPTION   This function clears the current_plmn_id.
...
SIDE EFFECTS  None
===========================================================================*/
void ds3g_clear_current_plmn_id
(
  sys_modem_as_id_e_type subs_id
)
{
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(ds3gsubsmgr_is_ds3g_subs_index_valid(ds_subs_id))
  {
    memset(&ds3gi_plmn_id[ds_subs_id], 0, sizeof(sys_plmn_id_s_type));
  }
  return;
} /* ds3g_clear_current_plmn_id */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
                                                       ...
                                                       call_type++)
  {
    ...
    if( hdlr_tbl_ptr != NULL )
    {
      ...
      {
        DATA_3GMGR_MSG0_EX(MSG_LEGACY_LOW, subs_id, "Suspend Data Flow");
        /*-------------------------------------------------------------------
          Invoke the mode-specific handler function.
        -------------------------------------------------------------------*/
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
                                                       ...
                                                       call_type++)
  {
    ...
    if( hdlr_tbl_ptr != NULL )
    {
      ...
      {
        DATA_3GMGR_MSG0_EX(MSG_LEGACY_HIGH, subs_id, "Resume Data Flow");
        /*-------------------------------------------------------------------
          Invoke the mode-specific handler function.
        -------------------------------------------------------------------*/
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_3GMGR_ERROR_LOG_2(MSG_LEGACY_ERROR,"ds3gi_cm_ac_cmd_cb: ac_cmd:d err: d",
                    ac_cmd, ac_cmd_err);
    /*-----------------------------------------------------------------------
      Some type of error occurred, take action based on the the type of
      command.
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DATA_3GMGR_MSG1_EX(MSG_LEGACY_HIGH, 
                    subs_id, 
                    "ds3gi_set_cm_emergency_only_mode:%d",
                    is_emergency_only);
...
SIDE EFFECTS  None
===========================================================================*/
boolean ds3g_is_ph_state_emergency_cb_mode
(
...
/*===========================================================================
FUNCTION      ds3g_cm_ph_cb

DESCRIPTION   Callback function to be called by CM
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DATA_3GMGR_MSG2(MSG_LEGACY_HIGH, "Recd Err code %d for ph cmd %d",
                                   ph_cmd_err,
                                   ph_cmd);
  switch(ph_cmd)
  {
     ...
     case CM_PH_CMD_PLMN_BLOCKING:
       ...
       else if ((ph_cmd == CM_PH_CMD_PLMN_BLOCKING)       &&
                (ph_cmd_err == CM_PH_CMD_ERR_IN_USE_S))
       {
         ds_3gpp_apn_switch_mgr_perform_cleanup_on_detach_retry_err
                                                (data_block_ptr);
       }
       else
       {
          cmd_ptr->hdr.cmd_id = DS_CMD_3GPP_SYS_SEL_PREF_ERR;
          /*data_block_ptr has the subs_id*/
          cmd_ptr->cmd_payload_ptr = data_block_ptr;
       }
     ...
     case CM_PH_CMD_OPRT_MODE:
      ...
      if(ph_cmd_err == CM_PH_CMD_ERR_NOERR)
      {
        cmd_ptr->hdr.cmd_id = DS_CMD_3GPP_CM_PH_OPRT_MODE_CB_NO_ERR;
      }
      else
      {
        cmd_ptr->hdr.cmd_id = DS_CMD_3GPP_CM_PH_OPRT_MODE_CB_ERR;
      }
    ...
    case CM_PH_CMD_INFO_GET:
      ...
      if(ph_cmd_err == CM_PH_CMD_ERR_NOERR)
      {
        cmd_ptr->hdr.cmd_id = DS_CMD_3GPP_CM_PH_INFO_CB_NO_ERR;
      }
      else
      {
        cmd_ptr->hdr.cmd_id = DS_CMD_3GPP_CM_PH_INFO_CB_ERR;
      }
	    break;
...
/*===========================================================================
FUNCTION      ds3g_get_cm_ph_cb_fn_ptr
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DATA_3GMGR_MSG0_EX(MSG_LEGACY_HIGH,subs_id,"ds3g_process_flow_disable_ex");
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DATA_3GMGR_MSG0_EX(MSG_LEGACY_HIGH, subs_id, "ds3g_process_flow_enable_ex");
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DATA_3GMGR_MSG1(MSG_LEGACY_LOW, "ds3g_get_total_uldl_wm_cnt_ex. subs_id:%d",
                  subs_id);
...
/*===========================================================================
FUNCTION      DS3G_GET_TOTAL_UL_CUR_WM_CNT_EX

DESCRIPTION   Mode specific get total UL current wm cnt handler is called.
...
SIDE EFFECTS  None
===========================================================================*/
uint64 ds3g_get_total_ul_cur_wm_cnt_ex
(
  sys_modem_as_id_e_type subs_id
)
{
  uint8                call_type;
  sys_sys_mode_e_type  mode;
  uint64               total_ul_wm_cnt = 0;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if(!ds3gsubsmgr_is_subs_id_valid(subs_id))
  {
    return total_ul_wm_cnt;
  }

  DATA_3GMGR_MSG0_EX(MSG_LEGACY_HIGH,subs_id,"ds3g_get_total_ul_cur_wm_cnt_ex");
  ...
  {
    if ((ds3gi_hdlr_tbl[mode][call_type] != NULL) &&
        (ds3gi_hdlr_tbl[mode][call_type]->get_total_ul_cur_wm_cnt_hdlr != NULL))
    {
      total_ul_wm_cnt += 
        ds3gi_hdlr_tbl[mode][call_type]->get_total_ul_cur_wm_cnt_hdlr( subs_id );
    } /* hdlr func not NULL */
  ...
  return total_ul_wm_cnt;
} /* ds3g_get_total_ul_cur_wm_cnt_ex() */

/*===========================================================================
FUNCTION      DS3G_PROCESS_REG_TX_WM_NON_EMPTY_FUNC_EX

DESCRIPTION   Mode specific reg tx wm non empty func handler is called.
...
SIDE EFFECTS  None
===========================================================================*/
void ds3g_process_reg_tx_wm_non_empty_func_ex
(
  sys_modem_as_id_e_type subs_id
)
{
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DATA_3GMGR_MSG0_EX(MSG_LEGACY_HIGH,subs_id,"ds3g_get_total_uldl_wm_cnt_ex");
  ...
  {
    if ((ds3gi_hdlr_tbl[mode][call_type] != NULL) &&
        (ds3gi_hdlr_tbl[mode][call_type]->reg_tx_wm_non_empty_func_hdlr != NULL))
    {
      ds3gi_hdlr_tbl[mode][call_type]->reg_tx_wm_non_empty_func_hdlr( subs_id );
    } /* hdlr func not NULL */
  } /* call type loop */

  return;
} /* ds3g_process_reg_tx_wm_non_empty_func_ex() */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DATA_3GMGR_MSG0_EX(MSG_LEGACY_HIGH, 
                     call_info_cmd_ptr->subs_id,
                     "Rx'ed DS_CMD_CM_PDN_CONNECTIVITY_REQ_ERR");
  ...
  else
  {
    ...
    DATA_3GMGR_MSG1_EX(MSG_LEGACY_HIGH, 
                       call_info_cmd_ptr->subs_id,
                       "Data call ended for sdf_id 0x%x",
...
/*===========================================================================
FUNCTION      ds3g_send_tlb_close_ready_ind

DESCRIPTION   This function is used to give TLB close ready indication to CM
...
SIDE EFFECTS  None
===========================================================================*/
void ds3g_send_tlb_close_ready_ind
(
...
)
{
  ...
  cm_ph_cmd_close_lb_ready_per_subs(NULL,
                                    NULL,
                                    client_id,
                                    close_loopback,
                                    subs_id);
  DATA_3GMGR_MSG0_EX(MSG_LEGACY_HIGH, subs_id, "Sent close ready to CM"); 
} /* ds3g_send_tlb_close_ready_ind *//*===========================================================================
FUNCTION      ds3g_read_cm_current_oprt_mode

DESCRIPTION   This function is used to read the current operating mode 
              reported by CM

DEPENDENCIES  None

RETURN VALUE  ds3g_read_cm_current_oprt_mode

SIDE EFFECTS  None
===========================================================================*/
sys_oprt_mode_e_type ds3g_read_cm_current_oprt_mode
(
  sys_modem_as_id_e_type subs_id
)
{
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  DS_3GPP_MSG1_MED("Curr OPRT mode is %d",ds_3gpp_cm_ph_info_oprt_mode);
  return ds_3gpp_cm_ph_info_oprt_mode;
}

/*===========================================================================
FUNCTION      ds3g_set_sys_set_sys_sel_pref

DESCRIPTION   This function is used to set system selection preference to CM

DEPENDENCIES  None
 
PARAMETERS    sys selection ptr to be sent to CM 
              retry_on_failure - TRUE  :- CB FN to be popuklated
                                 FALSE :- CB FN is NULL
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  if (retry_on_failure)
  {
    sys_sel_cb_f_ptr = (cm_ph_cmd_cb_f_type *)ds3g_get_cm_ph_cb_fn_ptr();
  }
  ...
  if(cm_ph_cmd_sys_sel_pref_5(
                 ...
                 subs_id,
                 (uint16)0,
                 sys_sel_pref_params_ptr)!= TRUE)
  {
    DS_3GPP_ERROR_LOG_0_EX(subs_id, "CM sys sel set failed");
  }

}/*ds3g_set_sys_sel_pref*/

/*===========================================================================
FUNCTION      ds3g_get_cm_ph_info

DESCRIPTION   This function is used to get the latest Phone info from CM 
...
RETURN VALUE  Boolean  - True -  IF CM PH info is requested successfully 
                         False - OTherwise 

SIDE EFFECTS  None
===========================================================================*/
boolean  ds3g_get_cm_ph_info
(
   ds3g_cm_ph_cb_fn_ptr          cb_fn_ptr,
   sys_modem_as_id_e_type        subs_id
)
{
  ...
  do
  {
    ...
    /*----------------------------------------------------------------------- 
      Now allocate the cm_ph_cb_fn_data
      -----------------------------------------------------------------------*/
    if (ds3gi_cm_ph_cb_info[subs_id].cb_cnt >= DS3G_CM_PH_INFO_MAX_CB_CLIENT)
    {
      err_val++;
      break;
    }

    tmp_cb_cnt = ds3gi_cm_ph_cb_info[subs_id].cb_cnt;

    /*----------------------------------------------------------------------------- 
      Code to check if the function ptr is already registered
      -----------------------------------------------------------------------------*/
    if (tmp_cb_cnt > 0)
    {
      for (loop_cnt = 0; loop_cnt < tmp_cb_cnt; loop_cnt++)
      {
        if((ds3gi_cm_ph_cb_info[subs_id].cm_ph_cb_info[loop_cnt] != NULL) &&
           (ds3gi_cm_ph_cb_info[subs_id].cm_ph_cb_info[loop_cnt]->cm_ph_cb_fn == cb_fn_ptr))
        {
          err_val++;
          break;
        }
    ...
    ds3gi_cm_ph_cb_info[subs_id].cm_ph_cb_info[tmp_cb_cnt] = 
        (ds3g_cm_ph_cb_info_type *)modem_mem_alloc(sizeof(ds3g_cm_ph_cb_info_type),
                                                 MODEM_MEM_CLIENT_DATA);

    if (ds3gi_cm_ph_cb_info[subs_id].cm_ph_cb_info[tmp_cb_cnt] == NULL)
    {
      err_val++;
      break;
    }

    ds3gi_cm_ph_cb_info[subs_id].cm_ph_cb_info[tmp_cb_cnt]->cm_ph_cb_fn = cb_fn_ptr;
                                                            
    ds3gi_cm_ph_cb_info[subs_id].cb_cnt++;

    sys_sel_cb_f_ptr = (cm_ph_cmd_cb_f_type *)ds3g_get_cm_ph_cb_fn_ptr();

    cm_ph_cmd_get_ph_info_per_subs(sys_sel_cb_f_ptr, 
                                   NULL,
                                   ds3g_get_cm_client_id(),
                                   subs_id);
  ...
  DS_3GPP_MSG1_HIGH_EX(subs_id, "ds3g_get_cm_ph_info returns %d",err_val);
  return ret_val;
}/*ds3g_get_cm_ph_info*/

/*===========================================================================
FUNCTION      ds3g_get_cm_ph_info_cb_err

DESCRIPTION   Called when cm ph info reports error 
              Look through ds3gi_cm_ph_cb_info and dispatch error to all
              registered clients
...
SIDE EFFECTS  None
===========================================================================*/
void ds3g_get_cm_ph_info_cb_err
(
  void
)
{
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  DS_3GPP_ERROR_LOG_0("CM PH info cb err");
  return;
}/*ds3g_get_cm_ph_info_cb_err*/

/*===========================================================================
FUNCTION      ds3g_get_cm_ph_info_cb_no_err

DESCRIPTION   Called when cm ph info reports no error 
...
SIDE EFFECTS  None
===========================================================================*/
void ds3g_get_cm_ph_info_cb_no_err
(
  void
)
{
  ...
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  DS_3GPP_MSG0_HIGH("No error for cm ph info");
  return;
}/*ds3g_get_cm_ph_info_cb_no_err*/


/*===========================================================================
FUNCTION      ds3g_dereg_cm_ph_info

DESCRIPTION   This function is used to deregister and stop receiving 
              CM PH info 
...
RETURN VALUE  Boolean  - True -  IF CM PH info is deregistered successfully 
                         False - If deregistration fails

SIDE EFFECTS  None
===========================================================================*/
boolean  ds3g_dereg_cm_ph_info
(
   ds3g_cm_ph_cb_fn_ptr          cb_fn_ptr,
   sys_modem_as_id_e_type        subs_id
)
{
  ...
  do
  {
    ...
    /*------------------------------------------------------------------------------ 
      If loopcnt >= tmp_cb_cnt, no match found and hence no registration
      -----------------------------------------------------------------------------*/
    if(loop_cnt >= tmp_cb_cnt)
    {
      err_val++;
    ...
    ds3gi_cm_ph_cb_info[subs_id].cm_ph_cb_info[loop_cnt]->cm_ph_cb_fn = NULL;
    modem_mem_free(ds3gi_cm_ph_cb_info[subs_id].cm_ph_cb_info[loop_cnt],
                   MODEM_MEM_CLIENT_DATA);

    ds3gi_cm_ph_cb_info[subs_id].cm_ph_cb_info[loop_cnt] = NULL;
    ds3gi_cm_ph_cb_info[subs_id].cb_cnt--;
  ...
  DS_3GPP_MSG1_HIGH_EX(subs_id, "ds3g_dereg_cm_ph_info returns %d",err_val);
  return ret_val;
}/*ds3g_dereg_cm_ph_info*/


/*===========================================================================
FUNCTION      DS3G_TAC_OR_LAC_CHANGE_CMD_HDLR

DESCRIPTION 
  This function is used to notify tac/lac change to registered clients
...
SIDE EFFECTS 
  None
===========================================================================*/
void ds3g_tac_or_lac_change_cmd_hdlr
(
  ds_cmd_type  *cmd_ptr
)
{
  ds3g_cm_tac_lac_info_type    *tac_lac_info_ptr = NULL;
  ds3geventmgr_filter_type      filter_info = {0};
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if( (NULL == cmd_ptr) || (NULL == cmd_ptr->cmd_payload_ptr) )
  {
  ...
  tac_lac_info_ptr = (ds3g_cm_tac_lac_info_type *)cmd_ptr->cmd_payload_ptr;

  /*---------------------------------------------------------------------
    Generate TAC/LAC change event.
  ----------------------------------------------------------------------*/
  filter_info.tech       = PS_SYS_TECH_3GPP;
  filter_info.ps_subs_id = (ps_sys_subscription_enum_type)
                            ds3gsubsmgr_subs_id_cm_to_ds(
                                                tac_lac_info_ptr->subs_id);

  if(ds3geventmgr_set_event_info(DS3GEVENTMGR_TAC_OR_LAC_INFO_EV,
                                (void *)&(tac_lac_info_ptr->tac_lac_info), 
                                &filter_info) == FALSE)
  {
    DATA_3GMGR_MSG0(MSG_LEGACY_LOW,
                    "Unable to provide TAC/LAC info to registered clients");
  } 
  return;
}/* ds3g_tac_or_lac_change_cmd_hdlr */
