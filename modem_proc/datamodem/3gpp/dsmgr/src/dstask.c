...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp/dsmgr/src/dstask.c#3 $ $DateTime: 2019/12/12 00:03:59 $ $Author: pwbldsvc $
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  rval = ds3gmsgr_msg_send(DS_UT_DSTASK_SYNC_CNF, MSGR_DS_MSGRRECV,&msg, sizeof(msg));
  DS3GPP_ASSERT(rval == TRUE);
  return;
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  {
    rval = ds3gmsgr_msg_send_ext(DS_UT_LTE_TLB_DS_CTL_READY_CNF,
                                 MSGR_DS_MSGRRECV,
                                 &msg,
                                 sizeof(msg),
                                 subs_id);
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  {
    (void)ds3gmsgr_msg_send_ext(DS_UT_3GPP_BEARER_QOS_INFO_IND,
                                MSGR_DS_MSGRRECV,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_3GMGR_C_MSG3(DS3G_F3_MASK, "Putting cmd %d into DS Cmd buffer - "
...
/*-------------------------------------------------------------------------
 ...
 /*-------------------------------------------------------------------------
  ...
  ds_3gpp_loopback_hdlr_read_efs();
#endif /* FEATURE_DATA_TEST_LOOPBACK */
...
#ifdef FEATURE_DATA_LTE_OFFTARGET_TEST
  ds3gmsgr_reg_msgr_cb(DS_UT_DSTASK_SYNC_REQ, 
                                   dsi_ut_sync_req_hdlr);
  ds3gmsgr_reg_msgr_cb(LTE_TLB_DS_CTL_READY_IND, 
                                   dsi_ut_ds_close_ready_hdlr);
  ds3gmsgr_reg_msgr_cb(DS_3GPP_BEARER_QOS_INFO_IND, 
                                   dsi_ut_ds_bearer_qos_info_ind_hdlr);
#endif
 ...
 /*-------------------------------------------------------------------------
  ...
  {
    DATA_3GMGR_MSG0(MSG_LEGACY_LOW," DS task is ready");
  }
#else
  /*-------------------------------------------------------------------------
