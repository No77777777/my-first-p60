...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp/dsmgr/src/ds3gmgr.c#3 $ $DateTime: 2019/12/12 00:03:59 $ $Author: pwbldsvc $
...
/*---------------------------------------------------------------------------
  Maximum number of PS Iface events registered by DS3G
   - IFACE_UP_EV
   - IFACE_DOWN_EV
---------------------------------------------------------------------------*/
#define DS3G_MAX_PS_IFACE_EV_REG 2
...
/*---------------------------------------------------------------------------
  cache to store IMS PDN Status
---------------------------------------------------------------------------*/
static uint32 ds3g_ims_pdn_status_mask = 0;
...
/*===========================================================================
FUNCTION      DS3G_PS_IFACE_EV_CB

DESCRIPTION   This function gets called when iface up/down event is posted,
              called for all ifaces
...
SIDE EFFECTS  None
===========================================================================*/
static void ds3g_ps_iface_ev_cb
(
...
)
{
  ...
  ds3g_ps_iface_ev_cmd_type  *iface_ev_cmd_p = NULL;
  ps_iface_name_enum_type     iface_name = PS_IFACE_INVALID_NAME;
  ps_iface_apn_enum_type      iface_apn_type = PS_IFACE_APN_TYPE_UNSPECIFIED;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

  /*-------------------------------------------------------------------------
    Validate event
  -------------------------------------------------------------------------*/
  if( (event != IFACE_UP_EV ) && (event != IFACE_DOWN_EV) )
  {
    DATA_3GMGR_MSG1(MSG_LEGACY_LOW,
                    "Cback invoked for a wrong event; %d", event);
    return;
  }
  ...
  {
    DATA_3GMGR_MSG0(MSG_LEGACY_LOW, "Null iface ptr");
    return;
  }

  iface_name = PS_IFACE_GET_NAME(this_iface_ptr);
  iface_apn_type = PS_IFACE_GET_APN_TYPE(this_iface_ptr);

  DATA_3GMGR_MSG4(MSG_LEGACY_MED,
                  "ds3g_ps_iface_ev_cb: iface:0x%x:%d apn_type:%d event:%d",
                  iface_name, this_iface_ptr->instance, iface_apn_type, event);

  /*-------------------------------------------------------------------------
    Handling of APN_Type: IMS events
  -------------------------------------------------------------------------*/

  /*-------------------------------------------------------------------------
    Ignore if iface is not of EPC_IFACE type OR
    If event is IFACE_UP and APN Type is not IMS
    Note: When IFACE_DOWN event comes, APN_Type field is cleared. So we
    would need to post a command. This should not have a side-effect since
    we are just clearing the mask.
  -------------------------------------------------------------------------*/
  if( (iface_name != EPC_IFACE) ||
      ((event == IFACE_UP_EV) && (iface_apn_type != PS_IFACE_APN_TYPE_IMS)) )
  {
    return;
  }

  /*-------------------------------------------------------------------------
    Allocate DS_CMD_NOTIFY_IMS_PDN_STATUS command and post it on DS task
  -------------------------------------------------------------------------*/
  cmd_ptr = ds_allocate_cmd_buf(sizeof(ds3g_ps_iface_ev_cmd_type));
  if( (cmd_ptr == NULL) || (cmd_ptr->cmd_payload_ptr == NULL) )
  {
    return;
  }

  cmd_ptr->hdr.cmd_id = DS_CMD_NOTIFY_IMS_PDN_STATUS;
  iface_ev_cmd_p = (ds3g_ps_iface_ev_cmd_type*)cmd_ptr->cmd_payload_ptr;
  iface_ev_cmd_p->iface_ptr = this_iface_ptr;
  iface_ev_cmd_p->event = event;
...
/*===========================================================================
FUNCTION      DS3G_PROCESS_NOTIFY_IMS_PDN_STATUS_CMD

DESCRIPTION   This function notifies IMS PDN status message via msgr
...
SIDE EFFECTS  None
===========================================================================*/
static void ds3g_process_notify_ims_pdn_status_cmd
(
  ds_cmd_type  *cmd_ptr
)
{
  ...
  ds_ims_pdn_info_msg_s       ims_pdn_info_msg = {{0}};
  sys_modem_as_id_e_type      cm_subs_id = SYS_MODEM_AS_ID_NONE;
  uint32                      prev_ims_pdn_status_mask = 0;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /*-------------------------------------------------------------------------
    Validate iface ptr
  -------------------------------------------------------------------------*/
  if(iface_ev_cmd_p->iface_ptr == NULL)
  {
    DATA_3GMGR_MSG0(MSG_LEGACY_LOW, "Null iface ptr");
    return;
  }
  ...
  -------------------------------------------------------------------------*/
  /* Get Subs id from Iface */
  cm_subs_id = ds3gsubsmgr_subs_id_ds_to_cm(
                 PS_IFACE_GET_SUBS_ID(iface_ev_cmd_p->iface_ptr));

  DATA_3GMGR_MSG3_EX(MSG_LEGACY_MED,
                     cm_subs_id,
                     "IMS Iface event:%d instance:%d current IMS PDN status:%d",
                     iface_ev_cmd_p->event,
                     iface_ev_cmd_p->iface_ptr->instance,
                     ds3g_ims_pdn_status_mask);
  switch( iface_ev_cmd_p->event )
  {
    case IFACE_UP_EV:
    {
      prev_ims_pdn_status_mask = ds3g_ims_pdn_status_mask;

      /* Set IMS PDN status mask for the corresponding instance  */
      ds3g_ims_pdn_status_mask |= (0x1 << iface_ev_cmd_p->iface_ptr->instance);

      /* Notify MSGR only if previously not notified */
      if( (prev_ims_pdn_status_mask == 0) && (ds3g_ims_pdn_status_mask > 0) )
      {
        notify = TRUE;
      }
    }
    ...
    case IFACE_DOWN_EV:
    {
      ...
      /* Clear IMS PDN status mask for the corresponding instance */
      ds3g_ims_pdn_status_mask &= ~(0x1 << iface_ev_cmd_p->iface_ptr->instance);

      /* Notify MSGR only if all IMS PDNs are down */
      if( (prev_ims_pdn_status_mask > 0) && (ds3g_ims_pdn_status_mask == 0) )
      {
        notify = TRUE;
      }
  ...
  /*-----------------------------------------------------------------------
    Notify Clients using MSGR
  -----------------------------------------------------------------------*/
  if(notify)
  {
    if( iface_ev_cmd_p->event == IFACE_UP_EV )
    {
      ims_pdn_info_msg.ims_pdn_status = DS_PDN_STATUS_UP;
    }
    else
    {
      ims_pdn_info_msg.ims_pdn_status = DS_PDN_STATUS_DOWN;
    }
    ...
    DATA_3GMGR_MSG1(MSG_LEGACY_LOW,
                    "Notify IMS PDN Status:%d using MSGR",
                    ims_pdn_info_msg.ims_pdn_status);

    /* Send message using MSGR */
    ds3gmsgr_msg_send_ext(DS_3GPP_IMS_PDN_INFO_IND,
                          MSGR_DS_3GPP,
                          (msgr_hdr_struct_type*)(&ims_pdn_info_msg),
                          sizeof(ds_ims_pdn_info_msg_s),
                          cm_subs_id);
  }

  return;
} /* ds3g_process_notify_ims_pdn_status_cmd() */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /*-------------------------------------------------------------------------
    Initialize ds3g throughput mgr
  -------------------------------------------------------------------------*/
  ds3gtputmgr_init();
  ...
  for ( i = PS_SYS_PRIMARY_SUBS; i < PS_SYS_SUBS_MAX; i++ )
  {
    if( 0 != ps_sys_event_reg_ex( PS_SYS_TECH_ALL,
                               ...
                               &ps_errno ) )
    {
      DS_3GMGR_ERROR_LOG_0(MSG_LEGACY_ERROR, "Couldn't register for "
                                       "PS_SYS_EVENT_LOW_LATENCY_TRAFFIC_STATUS!");
    }
  }
...
void ds3g_process_init_complete_cmd(void)
{
  ...
  uint8                     index = 0;
  void                     *ps_iface_ev_buf_ptr = NULL;
  ps_iface_event_enum_type  ps_iface_ev_list[DS3G_MAX_PS_IFACE_EV_REG];
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  ds3gtputmgr_uldl_timer_init();
#ifdef FEATURE_DATA_RAVE_SUPPORT
  ds3gtputmgr_uplink_throughput_init();
#endif
  ds3gtputthres_init();
  /*-------------------------------------------------------------------------
    Register for all IFACE UP and DOWN events
  -------------------------------------------------------------------------*/
  ps_iface_ev_buf_ptr = ps_iface_alloc_event_handle(NULL,
                                                    ds3g_ps_iface_ev_cb,
                                                    NULL);
  if( ps_iface_ev_buf_ptr == NULL )
  {
    DATA_3GMGR_MSG0(MSG_LEGACY_LOW,
                    "Unable to allocate buffer for Iface event listener");
  }
  else
  {
    ps_iface_ev_list[0] = IFACE_UP_EV;
    ps_iface_ev_list[1] = IFACE_DOWN_EV;

    ps_iface_reg_result = ps_iface_subscribe_event_list(
                              ps_iface_ev_buf_ptr,
                              (ps_iface_event_enum_type*)ps_iface_ev_list,
                              DS3G_MAX_PS_IFACE_EV_REG);

    if(ps_iface_reg_result != 0)
    {
      DATA_3GMGR_MSG0(MSG_LEGACY_LOW,
                    "Unable to register Iface event listener. "
                    "Freeing cb buffer allocated");
      if(ps_iface_ev_buf_ptr != NULL)
      {
...
/* In case of loopback mode  register for loopback module cm events */
...
/*===========================================================================
FUNCTION      DS3G_PROCESS_MSGR_CMDS

DESCRIPTION   This function processes the MSGR cmd posted by MSGR cback and
              invokes the respective cback function of various DS clients

PARAMETERS    cmd_ptr : Payload containing msg information
...
===========================================================================*/
void ds3g_process_msgr_cmd
(
  ds_cmd_type  *cmd_ptr
)
{
  ds3gmsgr_cmd_type         *rcvd_msg_info_ptr = NULL;
  ds3g_msgr_client_func_ptr   client_cback_func_ptr = NULL;
  sys_modem_as_id_e_type subs_id = SYS_MODEM_AS_ID_NONE;
  msgr_hdr_s   *msgr_hdr_ptr = NULL;
  msgr_umid_type umid = 0;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  if( (NULL == cmd_ptr) || (NULL == cmd_ptr->cmd_payload_ptr) )
  {
    DS_3GPP_ERROR_FATAL("Invalid CMD ptr or CMD payload ptr, ASSERTING !!!");
    return;
  }

  rcvd_msg_info_ptr = (ds3gmsgr_cmd_type *)cmd_ptr->cmd_payload_ptr;

  client_cback_func_ptr = (ds3g_msgr_client_func_ptr)rcvd_msg_info_ptr->user_data;

  msgr_hdr_ptr = (msgr_hdr_s*)&rcvd_msg_info_ptr->payload.hdr;

  #ifdef FEATURE_MSIM_L_PLUS_L
  subs_id = MSGR_GET_VARIANT(msgr_hdr_ptr);
  umid = (msgr_umid_type)(msgr_hdr_ptr->id);
  MSGR_CLEAR_VARIANT(msgr_hdr_ptr->id);

  #else
  subs_id = INST_ID_TO_SYS_AS_ID(msgr_hdr_ptr->inst_id);
  #endif

  DATA_3GMGR_MSG3(MSG_LEGACY_LOW,
                  "Client cback function MSG: umid %d,subs_id %d,umid after clear %d",
                  umid,
                  subs_id,
                  msgr_hdr_ptr->id
                  );

  if(NULL != client_cback_func_ptr)
  {
    if(FALSE == client_cback_func_ptr(rcvd_msg_info_ptr->payload.hdr.id,
                    subs_id,
                   (const msgr_hdr_struct_type *)&rcvd_msg_info_ptr->payload))
    {
      DATA_3GMGR_MSG2(MSG_LEGACY_LOW,
                      "Cback invocation returned error for msg ID: %d,subs_id %d",
                      rcvd_msg_info_ptr->payload.hdr.id,
                      subs_id);
    }
  }
  else
  {
    DATA_3GMGR_MSG2(MSG_LEGACY_LOW,
                    "Client cback function not valid for MSG: %d,subs_id %d",
                    rcvd_msg_info_ptr->payload.hdr.id,
                    subs_id);
  }
  
  return;
}/*ds3g_process_msgr_cmd*/
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch( cmd_ptr->hdr.cmd_id )
  {
    ...
    case DS_CMD_DS3GTPUTMGR_TIMER_EXPIRED:
      ds3gtputmgr_process_timer_expired_cmd( cmd_ptr );
      break;

    case DS_CMD_NOTIFY_IMS_PDN_STATUS:
      ds3g_process_notify_ims_pdn_status_cmd( cmd_ptr );
      break;

    case DS_CMD_SIO_TIMER_EXPIRED:
   ...
   case DS_CMD_CM_CALL_EV_MOD_BEARER_IND:
    ...
    case DS_CMD_CM_PH_SUB_NOT_AVAIL:
      ds3gi_cm_process_sub_not_avail_notify( cmd_ptr );
      break;
#ifdef FEATURE_EHRPD
    ...
    case DS_CMD_CM_TAC_OR_LAC_CHG:
      ds3g_tac_or_lac_change_cmd_hdlr(cmd_ptr);
      break;
    ...
    case DS_CMD_DSD_APM_DEL_ATTACH_PROFILES:
    case DS_CMD_DSD_APM_SET_ATTACH_PDN_LIST:
    ...
    case DS_CMD_PROFILE_DB_DEVICE_MODE_EXT:
      ...
      break;
    case DS_CMD_PROCESS_REPORTING_INTERVAL_CMD:
    {
      ds3g_process_reporting_interval_cmd(cmd_ptr);
      break;
    }
    ...
    case DS_CMD_PROCESS_ULDL_TIMER_CMD:
    case DS_CMD_PROCESS_DL_TPUT_TIMER_CMD:
    {
      ds3gtputmgr_process_ds_cmd(cmd_ptr);
      break;
    }

#ifdef FEATURE_MSIM_L_PLUS_L
    case DS_CMD_DS3G_DDS_SWITCH_REQ:
      ds3g_dds_switch_hdlr_process_dds_switch_req_cmd( cmd_ptr );
      break;

    case DS_CMD_DS3G_DDS_SWITCH_ALLOWED_ERR:
      ds3g_dds_switch_hdlr_process_dds_switch_allowed_err_cmd();
      break;

    case DS_CMD_DS3G_DUAL_STANDBY_PREF_ERR:
      ds3g_dds_switch_hdlr_process_dds_switch_err_cmd();
      break;
#endif
    case DS_CMD_DS3G_PS_REG_REQ:
      ds3g_data_call_process_ps_reg_req(cmd_ptr);
      break;
	case DS_CMD_DS3G_MO_DATA_PROTECT_TIMER_EXPIRED:
	  ds3gtrmmgr_process_mo_data_protect_timer_expired_cmd(cmd_ptr);
	  break;
    ...
    case DS_CMD_DSSNET6_RS_RA_START_EV:
      dssnet6_sm_process_event(cmd_ptr);
      break;
#endif /* FEATURE_DATA_PS_IPV6 */

#ifdef FEATURE_DATA_DIAG_SUPPORT
  case DS_CMD_DIAG_REQ:
      ds3gdiagmgr_process_ds_cmd(cmd_ptr);
      break;
#endif /*FEATURE_DATA_DIAG_SUPPORT*/

#ifdef FEATURE_DS_QSH_INTERFACE
    case DS_CMD_PROCESS_QSH_CMD:
	  ds3g_qsh_if_cmd_hdlr(cmd_ptr);
	  break;
#endif /* FEATURE_DS_QSH_INTERFACE */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DATA_3GMGR_MSG2(MSG_LEGACY_LOW,
                    "No hdlr regd for mode:%d to refresh act tput for subs id:%d",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DATA_3GMGR_MSG1(MSG_LEGACY_LOW,
                    "downlink throughput estimation: No service on ds3g ID %d",
                    ds3g_subs_id);
    return;
  }
  ...
  {
    DATA_3GMGR_MSG2(MSG_LEGACY_LOW,
                    "No hdlr regd for mode:%d for subs id:%d",
                    current_mode, ds3g_subs_id);
  }
  return;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ((hdlr_tbl_ptr != NULL) &&
      (hdlr_tbl_ptr->set_downlink_throughput_running_t_accumulate != NULL))
  {
    hdlr_tbl_ptr->set_downlink_throughput_running_t_accumulate(
                                          cm_subs_id, running_t_accumulate );
  }
  }
  return;
} /* ds3g_set_downlink_throughput_running_t_accumulate() */

/*===========================================================================
FUNCTION      DS3G_GET_DL_TPUT_T_ACCUMUATE_VALUE

DESCRIPTION   This function invokes the mode handlers to to provide
              t_accumulate value from the mode handler
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  *t_accumuate = 0;
  ...
  if (current_mode == SYS_SYS_MODE_NO_SRV)
  {
    DATA_3GMGR_MSG1(MSG_LEGACY_LOW, "act tput: No service on ds3g  subs id:%d",
                                      ds3g_subs_id);
    return;
  }
  ...
  if( ( current_mode == SYS_SYS_MODE_NONE ) ||
      ( current_mode >= SYS_SYS_MODE_MAX ))
  {
    DATA_3GMGR_MSG1(MSG_LEGACY_LOW, "Invalid current mode %d ",
                    current_mode);
    return;
  }
...
{
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  DATA_3GMGR_MSG1_EX(MSG_LEGACY_HIGH,as_id, "setting CS subs_id to %d", as_id);
...
SIDE EFFECTS  None
===========================================================================*/
sys_srv_status_e_type ds3g_get_lte_srv_status
(
  sys_modem_as_id_e_type      subs_id
)
{
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  -------------------------------------------------------------------------*/
  if(sys_plmn_match(plmn_id_info_p->hplmn, plmn_id_info_p->rplmn) == TRUE )
  {
    is_vplmn = FALSE;
  }
  ...
  {
    if(sys_plmn_match(plmn_id_info_p->ehplmn_list.plmn[index], 
                      plmn_id_info_p->rplmn) == TRUE )
    {
      is_vplmn = FALSE;
    }
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  else
  {
    if(NULL != prof_context)
    {
      ...
      {
        if(ds_3gpp_apn_switch_hdlr_enabled(subs_id))
        {
          ds3gmgr_check_and_copy_apn_for_DUN_call(prof_context, subs_id);
        }
      ...
      {
        DS_LTE_MSG1_ERROR_EX(subs_id, "Unable to extract context for Attach Prof: d, "
                          "No TRAT indication sent",profile_id);
      }
    }
    else
    {
      DS_LTE_MSG1_ERROR_EX(subs_id, "Invalid Profile context passed: 0xx",prof_context);
    }
  }
  return ret_val;
...
/*===========================================================================
FUNCTION ds3gmgr_check_and_copy_apn_for_DUN_call

DESCRIPTION
  This function will check if apn in the profile is NULL.
  If apn is null then for DUN call cases we need to extract
  apn from the userid field.
...
===========================================================================*/
void ds3gmgr_check_and_copy_apn_for_DUN_call
(
  ds_umts_pdp_profile_type *prof_context,
  sys_modem_as_id_e_type    subs_id
)
{
  ...
  if(prof_context == NULL ||
     strncmp((char*)prof_context->context.apn,
             (char*)"\0", DS_UMTS_MAX_APN_STRING_LEN+1)  != 0)
  {
    return;
  }

  user_apn = (byte*) ds_3gpp_apn_switch_hdlr_get_user_apn(subs_id);
  if(user_apn == NULL)
  {
    return;
  }

  if(strncmp((char*)prof_context->context.apn,
             (char*)user_apn, DS_UMTS_MAX_APN_STRING_LEN+1)  != 0)
  {
    strlcpy((char *)prof_context->context.apn,
            (const char*)user_apn,
            DS_UMTS_MAX_APN_STRING_LEN+1);
  }
}  /* ds3gmgr_check_and_copy_apn_for_DUN_call */
/*===========================================================================
FUNCTION DS3G_IS_ICMP_SENDING_ALLOWED

DESCRIPTION
  This function will query 3GPP MH if the icmp error sending is allowed or not
  and reply back to clients.
...
===========================================================================*/
boolean ds3g_is_icmp_sending_allowed
(
  ps_iface_type  *iface_ptr
)
{
  return ds_3gpp_pdn_context_is_icmp_error_sending_allowed(iface_ptr);
}
