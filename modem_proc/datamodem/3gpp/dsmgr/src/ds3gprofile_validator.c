...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp/dsmgr/src/ds3gprofile_validator.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  else
  {
    ...
    while(index <= label_count)
    {
      ...
      {
        DS_3GMGR_ERROR_LOG_0(MSG_LEGACY_ERROR, "Error decoding. Avoiding buf overrun on Src Str");
        ret_val = FALSE;
        break;
      }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  do
  {
    ...
    DS_3GPP_MSG2_MED_EX(subs_id,
                        "Network Mode: %d and Emergency service support: %d",
                        network_mode, 
...
/*===========================================================================
FUNCTION DS3G_PROFILE_VALIDATOR_IS_APN_NAME_FORBIDDEN
...
PARAMETERS 
  pdn_context_ptr: PDN who APN name needs to be validated 
  apn_name: APN name to be validated 
  subs_id: Current active PS subs
...
===========================================================================*/
boolean ds3g_profile_validator_is_apn_name_forbidden
(
  ds_pdn_context_s         *pdn_context_ptr,
  char                     *apn_name,
  sys_modem_as_id_e_type    subs_id
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  do
  {
    DS_3GPP_MSG1_MED_EX(subs_id, " apn_name %s", apn_name);
  ...
  return ret_val;
} /* ds3g_profile_validator_is_apn_name_forbidden */
