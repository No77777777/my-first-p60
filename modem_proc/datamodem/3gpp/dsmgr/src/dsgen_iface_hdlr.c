...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp/dsmgr/src/dsgen_iface_hdlr.c#2 $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS3GPP_ASSERT( PS_IFACE_IS_VALID(ret_iface_ptr) );
...
#else
    DS_3GPP_ERROR_FATAL("FEATURE_DATA_IS707 not defined");
#endif /* FEATURE_DATA_IS707 */
  }
...
#else
    {
      DS_3GPP_ERROR_FATAL("IFACE passed to dssnet_stop is not CDMA");
    }
#endif /* FEATURE_DATA_IS707 */
  }
...
)
{
  ...
  else
  {
    DS_3GMGR_ERROR_LOG_1(MSG_LEGACY_ERROR,
                    "dssnet_ppp_setup_ncp() bad args config_ptr 0xp",
                    ppp_config_ptr);
    DS3GPP_ASSERT(0);
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (ds707_is_policy_handoff_class_epc(acl_policy_info_ptr))
  {
    ...
    {
      DS_3GMGR_ERROR_LOG_0_EX(MSG_LEGACY_ERROR, subs_id, "Could not transfer EPC handoff ctxt. "
...
{
  return ds707_pkt_mgr_get_auth_internal_nv();
}
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (0 != ppp_ncp_get_negotiated_param (ncp_handle, 
                                         ...
                                         sizeof(selected_bcm)))
  {
    DS_IS707_ERROR_LOG_0(MSG_LEGACY_ERROR, "Failed to get S-BCM value");
    /*-------------------------------------------------------------------------
