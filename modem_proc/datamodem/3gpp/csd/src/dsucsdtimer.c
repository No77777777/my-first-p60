...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp/csd/src/dsucsdtimer.c#2 $ 
...
ds_ucsd_timer_init(void)
{
   ...
   {                         
      ...
      ds_ucsd_timer_array[i].next_ptr    = DS_UCSD_TIMER_NOT_IN_LIST_INDEX;
   } 
}
...
)
{
   ...
   register uint8 prev;
   register uint8 next_ptr;

   if (id >= DS_UCSD_MAX_TIMERS)
   {
      DATA_CSD_MSG1_LOW("Bad timer ID = %d",id);
      return;
   }
   if ( ds_ucsd_timer_array[id].next_ptr != DS_UCSD_TIMER_NOT_IN_LIST_INDEX)
   {
      ...
      */
      ...
      while ((index != DS_UCSD_TIMER_NULL_INDEX) && (index < DS_UCSD_MAX_TIMERS) && (index != id))
      {
         prev  = index;
         index = ds_ucsd_timer_array[index].next_ptr;
      }
   
      if ((index != DS_UCSD_TIMER_NULL_INDEX) && (index < DS_UCSD_MAX_TIMERS))
      {
         next_ptr = ds_ucsd_timer_array[index].next_ptr;
      
         /* Increment the next item in the list by the cancelled timers
         ** counter value if there is one...
         */
         if ((next_ptr != DS_UCSD_TIMER_NULL_INDEX) && (next_ptr < DS_UCSD_MAX_TIMERS))
         {
            ds_ucsd_timer_array[next_ptr].counter += ds_ucsd_timer_array[index].counter;
         }
      
         /* Take the timer out of the list */
         if ((prev != DS_UCSD_TIMER_NULL_INDEX) && (prev < DS_UCSD_MAX_TIMERS))
         {
            ds_ucsd_timer_array[prev].next_ptr = next_ptr;
         }
         else
         {
            ds_ucsd_timer_list_front_index = next_ptr;
         }
      }
   }
   ...
   ds_ucsd_timer_array[id].next_ptr    = DS_UCSD_TIMER_NOT_IN_LIST_INDEX;
}
...
)
{
   ...
   if (msec < 0)
   {
      DATA_CSD_MSG1_LOW("Bad timer msec value = %d",msec);
      return;
   }
   ...
   */
   ...
   while ((index != DS_UCSD_TIMER_NULL_INDEX) && (index < DS_UCSD_MAX_TIMERS)
   ...
   if (prev == DS_UCSD_TIMER_NULL_INDEX)
   {
      ...
      */
      if ((ds_ucsd_timer_list_front_index != DS_UCSD_TIMER_NULL_INDEX)
            && (ds_ucsd_timer_list_front_index < DS_UCSD_MAX_TIMERS))
      {
      ...
      ds_ucsd_timer_array[id].next_ptr = ds_ucsd_timer_list_front_index;
   ...
   */
   ...
   {
      ds_ucsd_timer_array[prev].next_ptr = id;
      ds_ucsd_timer_array[id].next_ptr = DS_UCSD_TIMER_NULL_INDEX;
   }
   ...
   */
   ...
   {      
      next_ptr = ds_ucsd_timer_array[prev].next_ptr;
      
      /* Subtract new timer counter value from the next one in the
      ** list to keep the counters differential
      */
      ds_ucsd_timer_array[next_ptr].counter -= counter;
      
      /* Link in new timer */
      ds_ucsd_timer_array[id].next_ptr = next_ptr;
      ds_ucsd_timer_array[prev].next_ptr = id;
   }
}
      ...
      */
      if (--ds_ucsd_timer_array[index].counter == 0)
      {
               ...
               (ds_ucsd_timer_array[index].counter == 0))             
         {
            ...
            index = ds_ucsd_timer_array[index].next_ptr;
            ds_ucsd_timer_array[old_index].next_ptr = DS_UCSD_TIMER_NOT_IN_LIST_INDEX;
         }                             
