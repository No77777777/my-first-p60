...
Copyright (c) 2003 - 2015 Qualcomm Technologies, Incorporated.  All Rights Reserved.
...
/*===========================================================================
  ...
  $PVCSPath:$
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp/csd/src/dswcsdv80.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  for ( i = 0; i < dswcsdv80_rate_table_size; i++ )
  {
     ...
     {
        DATA_CSD_MSG1_HIGH( "Found V.80 OTA rate, 0x%x",
  ...
  DATA_CSD_MSG1_LOW( "Cannot find V.80 OTA rate: FNUR=%d", fnur);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DATA_CSD_MSG1_LOW("no memory for V.80 CMD: small pool=%d",
  ...
  switch(cmd)
  {
    case DSWCSDV80_RATEH_CMD:
      ...
      DATA_CSD_MSG2_LOW( "Send Tx=0x%x Rx=0x%x rate info to DTE", rate, rate);
      break;

    default:
      DATA_CSD_MSG0_LOW("Bad V.80 cmd towards DTE");
  ...
  if ( ( sio_status = ds3g_siolib_ex_setup_tx( new_item, FALSE, port_id ) ) !=
    DS3G_SIOLIB_OK )
  {
    DATA_CSD_MSG1_ERROR( "SIO is not open: status=%d", sio_status);
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DATA_CSD_MSG0_LOW("memory allocation failed");
    return;
  }
  ...
  if(pkt_ptr == NULL)
  {
    DATA_CSD_MSG0_LOW("Ptr to DSM pkt chain is NULL" );
  ...
  while (item_ptr != NULL)
  {
    ...
    for (i=0; i<item_used; i++)
    {
      ...
      {
        #ifdef V80_DEBUG
          DATA_CSD_MSG1_ERROR("Found first TC 0x%x",c);
        #endif
        process = TRUE;
        break;
      }
    ...
    else
    {
      ...
      ---------------------------------------------------------------------*/
      DS3GPP_ASSERT(item_ptr->size < DSWCSDV80_DATA_FRAGMENT_SIZE);

      #ifdef V80_DEBUG
        DATA_CSD_MSG2_LOW("pkt chain has TC in pkt_cnt=%d, byte=%d",pkt_cnt,i);
      #endif
      ...
      if(item_ptr->pkt_ptr != NULL)
      {
        ...
        #ifdef V80_DEBUG
          DATA_CSD_MSG0_LOW("Not last item in chain");
        #endif
      }
      ...
      do
      {
        ...
        if((curr_index != 0xFF) && (curr_index < 4))
        {

          #ifdef V80_DEBUG
            DATA_CSD_MSG1_LOW("Found current TC=0x%x",item_ptr->data_ptr[i]);
          #endif
          ...
          else
          {
            ...
            next_index = 0xFF;
            #ifdef V80_DEBUG
              DATA_CSD_MSG2_LOW("last octet is a TC, data_ptr[%d]=0x%x", i,
          ...
          ---------------------------------------------------------------------*/
          DS3GPP_ASSERT(s < DSWCSDV80_DATA_FRAGMENT_SIZE);

          if((next_index != 0xFF) && (next_index < 4))
          {
            #ifdef V80_DEBUG
              DATA_CSD_MSG1_LOW("Found double TC next=0x%x",item_ptr->data_ptr[i+1]);
            #endif
      ...
      {
        #ifdef V80_DEBUG
          DATA_CSD_MSG2_ERROR("DL: old item big enough s=%d, size=%d",s,
                                                   item_ptr->size);
        #endif
        /*-------------------------------------------------------------------
      ...
      {
        #ifdef V80_DEBUG
          DATA_CSD_MSG1_LOW("Calling pushdown_tail w/size=%d",s);
        #endif
        /*-------------------------------------------------------------------
                                      ...
                                      DSM_DS_SMALL_ITEM_POOL) ) != s)
        {
          ...
          DATA_CSD_MSG2_LOW("Not enough memory for V80 to DTE: %d != %d", s, size);
        }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DATA_CSD_MSG0_LOW("NULL ptr into dswcsd_v80_from_dte()");
    return (item_became_chain);
  }
  DS3GPP_ASSERT (NULL != *item_ptr_passed);
  
  item_ptr = *item_ptr_passed;

  if(item_ptr == NULL)
  {
    DATA_CSD_MSG0_LOW("dswcsd_v80_from_dte() got NULL item");
    return (item_became_chain);
  }
  ...
  if(dswcsdv80_last_octet == TRUE)
  {
    ...
    backup = 1;
    #ifdef V80_DEBUG
      DATA_CSD_MSG0_LOW("Last octet was <EM>");
    #endif
  }
  else
  {
    ...
    for (i=0; i<item_ptr->used; i++)
    {
      ...
      {
        #ifdef V80_DEBUG
          DATA_CSD_MSG1_ERROR("Found first <EM> 0x%x",item_ptr->data_ptr[i]);
        #endif
        process = TRUE;
        break;
      }
  ...
  if(process == TRUE)
  {
    ...
    do
    {
          ...
          (dswcsdv80_last_octet == TRUE) )
      {
        ...
        else
        {
          ...
          else
          {
            ...
            dswcsdv80_last_octet = TRUE;
            #ifdef V80_DEBUG
              DATA_CSD_MSG2_ERROR("last octet is EM used:%d == i:%d",
                                  item_ptr->used,i+1);
            #endif
            break;
          }
        ...
        if( (next_octet >= 0xB0) && (next_octet <= 0xC0) )
        {
          ...
          if( next_octet == (uint8)DSWCSDV80_RATEH_CMD )
          {
            ...
            #ifdef V80_DEBUG
              DATA_CSD_MSG2_LOW("Found inband cmd 0x%x, rm %d octets",
                         next_octet,(4-backup));
            #endif
          }
          else
        ...
        else
        {
          ...
          -----------------------------------------------------------------*/
          if( DSWCSDV80_DATA_FRAGMENT_SIZE <= (s +1) )
          {
            DS3GPP_ASSERT(0);
            return (item_became_chain);
          }
          switch(next_octet)
          {
            ...
            default:
             ...
             -------------------------------------------------------*/
              DATA_CSD_MSG0_LOW("Single <EM>, remove it and next octet");
              break;
          ...
          i += 2-backup;
            ...
            {
              DATA_CSD_MSG2_LOW("Found TC 0x%x, insert 0x%x",
                      next_octet, dswcsdv80_from_dte_scratch[(s-1)]);
            }
            else
            {
              DATA_CSD_MSG3_LOW("Found TC 0x%x, insert 0x%x 0x%x",
                        next_octet, 
      ...
      else
      {
        ...
        -----------------------------------------------------------*/
        DS3GPP_ASSERT( DSWCSDV80_DATA_FRAGMENT_SIZE > s );
    ...
    if(s>item_ptr->size)
    {
      ...
      #ifdef V80_DEBUG
        DATA_CSD_MSG2_LOW("UL data to big for DSM item %d>%d",s,item_ptr->size);
      #endif
                                    ...
                                    DSM_DS_SMALL_ITEM_POOL) ) != s)
      {
        ...
        DATA_CSD_MSG2_LOW("Not enough memory for V80 from DTE: %d != %d", s, size);
      }
      else
      {
    ...
    else
    {
      ...
      if(s == 0)
      {
        ...
        -------------------------------------------------------------------*/
        #ifdef V80_DEBUG
          DATA_CSD_MSG0_LOW("Item became empty dswcsd_v80_from_dte()"); 
        #endif
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  while ( sdu_byte_cnt < dswcsd_call_info.bytes_per_sdu )
  {
    ...
    if ( dswcsd_sio_dsm_item_ptr == NULL )
    {
      ...
      {
        #ifdef V80_DEBUG
          DATA_CSD_MSG0_LOW("Data from SIO for SDU");
        #endif
        ...
        if(dswcsd_sio_dsm_item_ptr == NULL)
        {
          ...
          -----------------------------------------------------------------*/
          #ifdef V80_DEBUG
            DATA_CSD_MSG0_LOW("Item became empty build w/flag");
          #endif
          continue;
        }
      ...
      else
      {
        ...
        {
          DATA_CSD_MSG2_LOW("out of memory: small=%d large=%d",
         ...
         octets_left_for_sdu)
    {
      ...
      DATA_CSD_MSG0_ERROR("Failed to obtain enough memory");
    }
    else
    {
      /*---------------------------------------------------------------------
    ...
    if ( dswcsd_dsm_payload_size == 0 )
    {
      if(item_became_chain == TRUE)
      {
        ...
        dswcsd_set_dsm_payload_size(dswcsd_dsm_payload_size);
        #ifdef V80_DEBUG
          DATA_CSD_MSG1_LOW("V.80 chained the item in UL, pkt_ptr->size=%d",
