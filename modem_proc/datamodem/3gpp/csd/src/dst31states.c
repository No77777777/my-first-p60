...
/*=============================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp/csd/src/dst31states.c#2 $
  ...
  */
  switch (curr_state)
  {
    ...
    case DST31_WAIT_FOR_ORDER_STATE:
      ...
      {
        DATA_CSD_MSG0_LOW("+FIT timer expired!");
    ...
    case DST31_FTM_STATE:
      ...
      /* Need to wait until we are notified that SIO is in online-data mode */
      {
        ...
        DATA_CSD_MSG0_LOW( "Wait for ONLINE_DATA mode"); 
    ...
    case DST31_FRM_STATE:
      if (t31_ctrl_blk->flags.timeout || t31_ctrl_blk->flags.at_cmd_abort)
      {
        if (t31_ctrl_blk->flags.timeout == TRUE)
        {
          DATA_CSD_MSG0_LOW("+FCL timer expired!");
        }
        else
        {
          DATA_CSD_MSG0_LOW("Command aborted by DTE");
        }
    ...
    default: 
     DATA_CSD_MSG1_LOW("Invalid state=%d", curr_state);
  } /* switch */
...
)
{
  ...
  */
  DATA_CSD_MSG0_LOW( "dst31i_process_idle_state() erroneously accessed!");
} /* dst31i_process_idle_state */
    ...
    { 
          ...
          (dl_data_type == DS_FA_DATA_BCS))
      {
        ...
        */
        ...
        DATA_CSD_MSG1_LOW("SYNC::hold pending data, cnt=%d", 
          dsm_length_packet(t31_ctrl_blk->pending_dl_data_ptr));
      }
      else
      {
        DATA_CSD_MSG2_HIGH("Unexpected RX data in SYNC state-- Data deleted, cnt=%d type=%d",
...
)
{
  ...
  */
    ...
    if (dsgcsdfa_is_idle() == TRUE)
    {
      ...
      */
      ...
      if (fa_data_ptr != NULL)
      {
        ...
        {
          DATA_CSD_MSG1_LOW( "Freeing RX data in FA_IDLE_STATE, type=%d", fa_data_type);
        }
  ...
  if (fa_stat == DS_FA_FAILURE)
  {
    DATA_CSD_MSG0_LOW("WFO::FA failure detected");
  ...
  */
  ...
  else
  {
    ...
    */
    ...
    DATA_CSD_MSG1_LOW( "+FTH::send %d HDLC frame bytes", ul_byte_cnt);
    if( t31_ctrl_blk->tps_ul_wm_ptr )
    {
      DATA_CSD_MSG3_HIGH("TPS-UL:Qcnt=%d Qhi=%d Qlo=%d", 
    ...
    */
    if (frame_stat == DST31_FRAME_COMPLETE)
    {
      if (t31_ctrl_blk->flags.fth_last_frame == TRUE)
      {
        ...
        ** completed.
        */
        DATA_CSD_MSG1_HIGH("+FTH::complete TX_bytes=%d",
    ...
    */
    if (t31_ctrl_blk->flags.fth_preamble_active == TRUE)
    {
      ...
      if (t31_ctrl_blk->byte_cnts.fth_preamble_cnt < DST31_FTH_MAX_PREAMBLE_SIZE)
      {
        ...
        DATA_CSD_MSG2_LOW("+FTH: send %d pre-flags, cnt=%d",
      ...
      else
      {
        ...
        DATA_CSD_MSG0_LOW("+FTH: preamble complete");
      }
    }
    else if (t31_ctrl_blk->pending_ul_data_ptr)
    {
      ...
      DATA_CSD_MSG1_LOW("+FTH::process pending data, cnt=%d", 
      ...
      */
      if (sio_data_ptr && (sio_data_ptr->used > 0))
      {
        ...
        */
        ...
        DATA_CSD_MSG1_LOW("+FTH::process %d data bytes",ul_byte_cnt);
        ...
        */
        if (sio_data_ptr && (sio_data_ptr->used > 0))
        {
          ...
          DATA_CSD_MSG2_LOW("+FTH::hold pending data, cnt=%d fs=%d",
        ...
        */
        if (frame_stat == DST31_FRAME_COMPLETE)
        {
          ...
          DATA_CSD_MSG2_HIGH("+FTH::send frame OTA, cnt=%d lf=%d",
        ...
        else if (frame_stat & (DST31_FRAME_INCOMPLETE | DST31_FRAME_READY)) /*lint !e655 */
        {
          ...
          */
          DATA_CSD_MSG0_LOW( "+FTH::wait on frame, send flag!");
      ...
      else if (t31_ctrl_blk->flags.wait_for_first_data_byte == TRUE)
      {
        ...
        ** to start coming in.
        */
        DATA_CSD_MSG0_LOW( "+FTH::no SIO data, send HDLC flag!");
...
)
{
  ...
  */
  if (t31_ctrl_blk->pending_dl_data_ptr != NULL)
  {
    ...
    DATA_CSD_MSG1_LOW( "+FRH::process pending data, cnt=%d", dl_data_ptr->used);
  }
  else
  {
  ...
  if ((fa_stat == DS_FA_FAILURE) || (dl_data_type == DS_FA_DATA_MSG))
  {
    ...
    if (fa_stat == DS_FA_FAILURE)
    {
      DATA_CSD_MSG0_LOW("+FRH::FA failure detected");
    ...
    {
      DATA_CSD_MSG0_LOW( "+FRH::Invalid page data, discarded");
  ...
  else 
  {
    ...
    */
    if (t31_ctrl_blk->flags.frh_wait_for_fa_idle == TRUE)
    {
      ...
      { 
        DATA_CSD_MSG1_LOW("Dumping +FRH data, cnt=%d",
      ...
      {
        DATA_CSD_MSG0_LOW("FA going IDLE, Exit +FRH state");
    ...
    */
    if (dl_data_ptr && (dl_data_ptr->used > 0))
    {
      ...
      */
      ...
      {
        DATA_CSD_MSG2_LOW("+FRH::frame_sz=%d lf=%d",
      ...
      */
      if (dl_data_ptr && (dl_data_ptr->used > 0))
      {
        ...
        DATA_CSD_MSG1_LOW("+FRH::hold pending data, cnt=%d",
                dl_data_ptr->used);
      }
      else
      {
    ...
    */
    ...
    
    {
      ...
      {
        DATA_CSD_MSG0_LOW("+FRH::SIO TX error to DTE");
      }
      else
      {
             ...
             (frame_stat == DST31_FRAME_COMPLETE))
    {
      if (frame_stat == DST31_FRAME_COMPLETE)
      {
        DATA_CSD_MSG1_HIGH("+FRH::complete RX_bytes=%d",
        ...
        */
        ...
        {
          DATA_CSD_MSG0_LOW("+FRH: Waiting on FA to go IDLE");
      ...
      else
      {
        ...
        */
        DATA_CSD_MSG1_HIGH("+FRH: toss bad frame, cnt=%d",
                 t31_ctrl_blk->io_data.cnt);

        dsat_result = (dsm_length_packet(dl_data_ptr) == 0)
                        ? DSAT_OK : DSAT_ERROR;
 
        DATA_CSD_MSG2_LOW("Dumping +FRH data, cnt=%d rc=%d",
  ...
  if (t31_ctrl_blk->flags.tps_flowctl_asserted == FALSE)
  {
    ...
    */
    if (t31_ctrl_blk->pending_ul_data_ptr)
    {
      ...
      DATA_CSD_MSG1_LOW("+FTM::process pending data, cnt=%d",
               sio_data_ptr->used);
    }
    else
    {
    ...
    */
    if (sio_data_ptr && (sio_data_ptr->used > 0))
    {
      ...
      */
          ...
          (sio_data_ptr && (sio_data_ptr->used > 0)))
      {
        ...
        DATA_CSD_MSG1_LOW("+FTM::hold pending data, cnt=%d",
         dsm_length_packet(sio_data_ptr));
      }
      else
      {
    ...
    else if (t31_ctrl_blk->flags.wait_for_first_data_byte == TRUE)
    {
      ...
      ** up while we wait.
      */
      DATA_CSD_MSG0_LOW("+FTM::no SIO data, send '1' bits");
    ...
    */
    if (frame_stat & (DST31_FRAME_EMPTY | DST31_FRAME_CORRUPT)) /*lint !e655 */
    {
      if (frame_stat & DST31_FRAME_EMPTY) /*lint !e655 */
      {
        ...
        {
          DATA_CSD_MSG0_LOW("+FTM::no SIO data, send '0' bits");
      ...
      {
        DATA_CSD_MSG0_LOW("+FTM::corrupt message frame");
      }
    }
    ...
    if (frame_stat & (DST31_FRAME_READY | DST31_FRAME_COMPLETE)) /*lint !e655 */
    {
      ...
      */
      ...
      DATA_CSD_MSG1_LOW("+FTM::send %d fax data byte(s)",
    ...
    */
    if (frame_stat & (DST31_FRAME_EMPTY | DST31_FRAME_COMPLETE)) /*lint !e655 */
    {
      ...
      {
        DATA_CSD_MSG2_LOW("+FTM::page %d complete, fs=%d",
          t31_ctrl_blk->byte_cnts.page_cnt-1,frame_stat);
      }

      t31_ctrl_blk->byte_cnts.page_cnt++;
        
      DATA_CSD_MSG4_LOW("+FTM::SIO_byte_cnt=%d, +FTM::DLEs removed=%d, \
...
)
{
  ...
  */
  if (t31_ctrl_blk->pending_dl_data_ptr)
  {
    ...
    DATA_CSD_MSG1_LOW( "+FRM::process pending data, cnt=%d", dl_data_ptr->used);
  }
  else
  {
    /* Check for data on the watermark */
  ...
  if ((fa_stat != DS_FA_FAILURE) || (dl_data_type == DS_FA_DATA_MSG))
  {
    ...
    if (dl_data_ptr || (fa_stat == DS_FA_SWITCHING_TO_IDLE))
    {
      ...
      if (dl_data_ptr && (dl_data_ptr->used > 0))
      {
        ...
        */
        ...
        if (dl_byte_cnt > 0)
        {
          ...
          DATA_CSD_MSG1_LOW( "+FRM::hold pending data, cnt=%d", dl_byte_cnt);
        }
        else
        {
          /* We are done with the packet so free it */
      ...
      */
      ...
      if (frame_stat & (DST31_FRAME_READY | DST31_FRAME_COMPLETE)) /*lint !e655 */
      {
        ...
        if (sio_data_ptr && (sio_data_ptr->used > 0))
        {
          ...
          DATA_CSD_MSG1_LOW( "+FRM::receiving %d fax data bytes", 
            sio_data_ptr->used);
          ...
          {
            DATA_CSD_MSG0_LOW("+FRM::SIO TX error to DTE");
          }
        }
      ...
      */
      if (dl_data_ptr && (dl_data_ptr->used > 0))
      {
        ...
        DATA_CSD_MSG1_LOW("+FRM::hold pending data, cnt=%d",
              dl_data_ptr->used);
      }
      else
      {
    ...
    */
    if (fa_stat == DS_FA_SWITCHING_TO_IDLE)
    {
      ...
      if (t31_ctrl_blk->byte_cnts.page_cnt++ > 0)
      {
        DATA_CSD_MSG1_LOW("+FRM::page %d processing complete",
          t31_ctrl_blk->byte_cnts.page_cnt-1);
      }

      DATA_CSD_MSG4(MSG_LEGACY_LOW,"+FRM::OTA_byte_cnt=%d, +FRM::DLEs   added=%d, \
  ...
  else
  {
    ...
    {
      DATA_CSD_MSG0_LOW("+FRM::Invalid BCS data rx'd, data discarded");
    ...
    {
      DATA_CSD_MSG0_LOW("+FRM::FA failure detected");
...
)
{
  ...
  if (t31_ctrl_blk->flags.timeout == TRUE)
  {
    DATA_CSD_MSG0_LOW("+FTS::+FTS timer expired!");
  ...
  */
  if (dsgcsdfa_process_uplink(NULL, DS_FA_DATA_NONE) == DS_FA_FAILURE)
  {
    DATA_CSD_MSG0_LOW("+FTS::FA failure detected");
...
)
{
  ...
  if (t31_ctrl_blk->flags.timeout == TRUE)
  {
    DATA_CSD_MSG0_LOW("+FRS::+FRS timer expired!");
