...
/*=============================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp/csd/src/dst31hdlc.c#2 $
...
)
{
  ...
  DATA_CSD_MSG1_LOW("HDLC-ZBI on %d bytes of data", ibuf_len);
...
#if 0 /* Debug use only */
    DATA_CSD_MSG2_LOW("HDLC-ZBI processing byte[%2d]=0x%x\n", byte_idx,ibyte);
#endif
  ...
  DATA_CSD_MSG1_LOW("HDLC-ZBI output buffer len=%d", ocnt);
...
)
{
  ...
  /* Safety Net */
  DS3GPP_ASSERT(ibuf != NULL );
  DS3GPP_ASSERT(obuf != NULL );
  DS3GPP_ASSERT(obuf_cnt != NULL );

  DATA_CSD_MSG1_LOW("HDLC-ZBD on %d bytes of data", dsm_length_packet(*ibuf));
  ...
  while (frame_complete == FALSE)
  {
    ...
    */
        ...
        ((tbyte = dsm_pull8(ibuf)) == -1))
    {
      if (i_limit == DST31_FRH_MAX_BYTES_PER_20MS)
      {
        DATA_CSD_MSG1_LOW( "HLDC-ZBD reached limit=%d", DST31_FRH_MAX_BYTES_PER_20MS);
      }
      break;
    }
    ...
    */
    ...
    {
      DATA_CSD_MSG3_LOW("ibyte=0x%02x obyte=0x%02x fc=%d",
        ...
        if (ones_cnt == DST31_MAX_CONSECUTIVE_FLAG_ONE_BITS)
        {
          ...
          switch (zbd_state)
          {
            case DST31_FRH_HDLC_FIND_FLAG_STATE:
              ...
              */
              ...
              zbd_flag_cnt = 1;
              DATA_CSD_MSG1_LOW("Found opening flag, data=0x%x", tbyte);
              break;
              ...
              */
              ...
              if (zbd_flag_cnt == DST31_CARRIER_DETECT_CNT)
              {
                *carrier_detected = TRUE;
                DATA_CSD_MSG1_LOW("+FRH carrier detected, fc=%d",zbd_flag_cnt);
              }

              DATA_CSD_MSG1_LOW("Toss consecutive flags fc=%d", zbd_flag_cnt);
              break;
            ...
            case DST31_FRH_HDLC_DATA_STATE:
              ...
              */
              DATA_CSD_MSG2_LOW("Found closing flag, fc=%d val=0x%x",
                zbd_flag_cnt,tbyte);

              DATA_CSD_MSG2_LOW("HDLC::PUSH back 0x%02x 0x%02x\n",
            ...
            default:
              /* Should NEVER happen */
              DATA_CSD_MSG1_LOW( "Unknown HDLC state, val=%d", zbd_state);
          } /* switch */
          ...
          */
          if (zbd_state == DST31_FRH_HDLC_FLAG_STOP_STATE)
          {
            ...
            {
              DATA_CSD_MSG0_LOW("<<*** NULL HDLC frame ***>>");
            }
      ...
      */
      if ((ofill_cnt >= DST31_MAX_BITS_PER_BYTE) || flag_found)
      {
        if (zbd_state == DST31_FRH_HDLC_FLAG_START_STATE)
        {
          ...
          */
          if (flag_found == FALSE)
          {
            ...
            */ 
            DATA_CSD_MSG1_LOW("+FRH now in DATA state, val=0x%x",obyte);
        ...
        {
          DATA_CSD_MSG2_LOW("HDLC->add val=0x%02x cnt=%d",
  ...
  */
...
#if 0 /* Debug use only */
  DATA_CSD_MSG7(MSG_LEGACY_LOW,"HDLC-ZBD reentrant variables: zbd_oshift_start   =%d,\
  ...
  if (frame_complete)
  {
    DATA_CSD_MSG1_LOW( "HDLC-ZBD frame_size=%d", ocnt);
  }
