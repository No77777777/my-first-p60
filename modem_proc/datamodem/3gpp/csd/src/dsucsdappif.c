...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp/csd/src/dsucsdappif.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
/*-------------------------------------------------------------------------*/
   ...
   if ( index >= MAX_UCSD_CALL_TBL_ENTRIES ) 
   {
     ...
     DATA_CSD_MSG1_LOW( "Cannot find CS call instance: %d", inst_id );
   }
   return ( cm_call_id );
...
)
{
  ...
  *client_pptr = NULL;
  ...
  DATA_CSD_MSG1_LOW( "Allocated UCSD client index: %d",index);
...
)
{
  ...
  {
    DATA_CSD_MSG1_LOW("Invalid client identifier specified: %d",client_id);
    return FALSE;
  }
...
)
{
  ...
  if( status )
  {
    ...
    DATA_CSD_MSG1_LOW( "Released UCSD client: %d",client_id);
  }
...
)
{
  ...
  {
    DATA_CSD_MSG2_LOW("Invalid call type specified: %02X   permitted: %02X",
  ...
  for( index=0; index<DSUCSD_API_MAX_CLIENTS; index++ )
  {
        ...
        IS_CALL_TYPE(call_type, c_ptr->info.call_types) )
    {
      ...
      if( NULL != c_ptr->info.call_event_cb )
      {
        ...
        DATA_CSD_MSG2_MED( "Posted UCSD event: evt=%d clnt=%d",
...
)
{
  ...
  {
    DATA_CSD_MSG0_LOW("Registration request before UCSD client support init");
  ...
  {
    DATA_CSD_MSG2_LOW("Invalid call types bitmask specified: %02X   permitted: %02X",
  ...
  if( ( TRUE == status ) && ( NULL != client_ptr ) )
  {
    ...
    DATA_CSD_MSG3_LOW("Registered UCSD client[%d]:  types=0x%02X cb=0x%p",
...
)
{
  ...
  DATA_CSD_MSG2_LOW("ds_ucsd_api_deregister_client UCSD client[%d] status[%d]",
                  client_id,status);
...
/*-------------------------------------------------------------------------*/
  ...
  {
    DATA_CSD_MSG1_LOW("Invalid call mode specified: %d",call_mode);
    return DSUCSD_INVALID_INST_ID;
  }
  if( TRUE !=
      ds_ucsd_validate_cbst_values( speed, name, connection_element ) )
  {
    DATA_CSD_MSG3_LOW("Unsupported call configuraiton specified: %d %d %d",
  ...
  {
    DATA_CSD_MSG1_LOW("Unsupported WAIUR for current mode specified: %d",
               waiur);
    return DSUCSD_INVALID_INST_ID;
  }
  if( NULL == dial_string )
  ...
  {
    DATA_CSD_MSG2_LOW("Dialing string exceeds max length: %d<%d",
  ...
  if ( inst_id != DSUCSD_INVALID_INST_ID )
  {
    ...
    if ( (cmd_ptr != NULL) && (cmd_ptr->cmd_payload_ptr != NULL) )
    {
      ...
      DATA_CSD_MSG1_HIGH( "UCSD Application originates a call, client id [%d]",
                      client_id);
     ...
     memscpy( (char*)ds_ucsd_app_dial_string,
      ...
      if(uus_info->present == TRUE)
      {
        ...
        else
        {
           ...
           DATA_CSD_MSG2_LOW( "UUS1 invalid payload size, 0<(size=%d)<=%d", 
  ...
  {
   DATA_CSD_MSG1_LOW( "Cannot originate call: no call instance [%d]",
                     inst_id);
  }
...
)
{
  ...
  {
    DATA_CSD_MSG1_LOW("Invalid call instance identifier specified: %d",inst_id);
    return FALSE;
  }
  if( VALIDATE_CALL_TYPE(call_type, DSUCSD_API_CALL_TYPES_SCUDIF_MASK) )
  {
    DATA_CSD_MSG2_LOW("Invalid call type bitmask specified: %02X   permitted: %02X",
  ...
  if ( (cmd_ptr != NULL) && (cmd_ptr->cmd_payload_ptr != NULL) )
  {
    DATA_CSD_MSG1_HIGH( "UCSD Application confirms a call client id[%d]",
                     client_id);
...
/*-------------------------------------------------------------------------*/
  ...
  if ( (cmd_ptr != NULL) && (cmd_ptr->cmd_payload_ptr != NULL) )
  {
    DATA_CSD_MSG1_HIGH( "UCSD Application answers a call for client id[%d]",
                    client_id);
...
/*-------------------------------------------------------------------------*/
  ...
  if ( (cmd_ptr != NULL) && (cmd_ptr->cmd_payload_ptr != NULL ) )
  {
    DATA_CSD_MSG1_HIGH( "UCSD Application ends a call for client_id [%d]", 
                     client_id);
...
)
{
  ...
  DATA_CSD_MSG1_LOW( "Invoked ds_ucsd_api_set_rlp(%d)",client_id);
  ...
  {
    DATA_CSD_MSG0_LOW( "Failed to update RLP parameter set");
    result = FALSE;
  }
...
)
{
  ...
  DATA_CSD_MSG1_LOW( "Invoked ds_ucsd_api_get_rlp(%d)",client_id);
  ...
  {
    DATA_CSD_MSG0_LOW( "Failed to get RLP parameter sets");
    result = FALSE;
  }
...
)
{
  ...
  else
  {
        ...
        (inst_id == ds_ucsd_last_call_info.inst_id) )
    {
      ...
      callstats_ptr->present = TRUE;
      DATA_CSD_MSG3_HIGH("ds_ucsd_last_call_info: tx=%d rx=%d active=%d",
