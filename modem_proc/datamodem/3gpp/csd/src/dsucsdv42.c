...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp/csd/src/dsucsdv42.c#3 $ $DateTime: 2020/11/16 21:05:49 $ $Author: pwbldsvc $
   ...
   if (compression_request != V42_NO_COMPRESSION)
   {
      if (Rlp_process_uplink)
      {
         ...
         if ((EmptyCount > EMPTY_FLUSH_THRESHOLD) && (!Flushed))
         {
            V42_flush();
            EmptyCount = 0;
            DATA_CSD_MSG0_LOW("V42bis: Flushing...");
         }
         else
         {
            EmptyCount++;
      ...
      {
         ret_value = V42_ERROR;
         DATA_CSD_MSG0_LOW("V42bis:Fatal Error!");
      }
   }
