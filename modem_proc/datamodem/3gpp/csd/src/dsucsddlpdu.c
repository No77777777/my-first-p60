...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp/csd/src/dsucsddlpdu.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
)
{
   ...
   DATA_CSD_MSG3_LOW("Writing PDU OoS at Downlink List: %d[%d] 0x%x",
...
ds_ucsd_DownlinkPDUMgr_DeleteUnAcknowledgedFrames (void)
{
   ...
   {
      DATA_CSD_MSG2_LOW("DLPDUMgr: DelUnACKFrames, illegal VE=%d or VR=%d",
   ...
   do 
   {
      if (CurrentList->State[Index] == PDU_STATE_RCVD)
      {
         ...
         CurrentList->State[Index] = PDU_STATE_IDLE; // clang_sa_ignore [core.NullDereference]       
         /* Free DSM item */
...
)
{
   ...
   if ( (_Ns < 0) || 
        ...
        (CurrentList->VR > get_current_rlp_max_frame_number()) )
   {
      DATA_CSD_MSG2_LOW("DLPDUMgr: MarkMissFrmForSREJ, illegal N(r)=%d or VR=%d",
                        _Ns,CurrentList->VR);
...
ds_ucsd_DownlinkPDUMgr_CheckSREJToSend (void)
{
  ...
  DATA_CSD_MSG2_LOW("In ds_ucsd_DownlinkPDUMgr_CheckSREJToSend: VR=%d VE=%d",
   ...
   if ( (CurrentList->VE > get_current_rlp_max_frame_number()) ||
        (CurrentList->VR > get_current_rlp_max_frame_number()) )
   {
      DATA_CSD_MSG2_LOW("DLPDUMgr: CheckSREJToSend, illegal VE=%d or VR=%d",
                     CurrentList->VE,CurrentList->VR);
      WRITE_EVENT_TO_L2R (RLP_ERROR_IND);
   }
   else
...
)
{
   ...
   {
     DATA_CSD_MSG2_HIGH("Disabling TX Flow Control: %d < %d",
                RLP_DSM_MEM_LEVEL_MANY, free_cnt);
...
)
{
   ...
   if (List != NULL)
   {
      ...
      DATA_CSD_MSG1_LOW("Initializing Downlink List: %d",List->Type);
