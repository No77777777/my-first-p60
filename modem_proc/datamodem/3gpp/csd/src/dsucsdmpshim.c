...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp/csd/src/dsucsdmpshim.c#2 $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_3GPP_MSG0_LOW ("Unable to allocate GCSD command buffer");
  }
  else
  {
...
)
{
  ...
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
   DATA_CSD_MSG0_LOW("In ds_ucsd_mpshim_register_tdscdma_rlc");
#ifdef FEATURE_SEGMENT_LOADING
   if(ds_3gpp_csd_tds_tbl == NULL)
   {
     DATA_CSD_MSG0_LOW("In ds_ucsd_mpshim_register_tdscdma_rlc TDS not active");
     return ( DS3G_FAILURE );
   }
#endif /* FEATURE_SEGMENT_LOADING */
  /*-------------------------------------------------------------------------
...
#endif /* FEATURE_SEGMENT_LOADING */
  {
    DS_3GPP_MSG1_LOW("UL WM registration failed lc_id:%d", 
              lc_info_ptr->ul_lc_id[ 0 ]);
  }
...
#endif /* FEATURE_SEGMENT_LOADING */
  {
    DS_3GPP_MSG1_LOW("DL WM registration failed lc_id:%d", 
              lc_info_ptr->dl_lc_id[ 0 ]);
  } 
...
)
{
  ...
  DATA_CSD_MSG0_LOW("In ds_ucsd_mpshim_deregister_tdscdma_rlc");
#ifdef FEATURE_SEGMENT_LOADING
  if(ds_3gpp_csd_tds_tbl == NULL)
  {
    DATA_CSD_MSG0_LOW("In ds_ucsd_mpshim_register_tdscdma_rlc TDS not active");
...
#endif /* FEATURE_SEGMENT_LOADING */
  {
    DS_3GPP_MSG1_LOW("DL WM deregistration failed lc_id:%d", 
              ds_ucsd_rlc_dl_dereg_params.rlc_id[ 0 ]);
  }
  return TRUE ;
...
/*-------------------------------------------------------------------------*/
   ...
   DS3GPP_ASSERT( NULL != lc_info_ptr );
  
   DATA_CSD_MSG0_LOW("In ds_ucsd_mpshim_register_wcdma_rlc");
   
#ifdef FEATURE_SEGMENT_LOADING
   if(ds_3gpp_csd_wcdma_tbl == NULL)
   {
     DATA_CSD_MSG0_LOW("In ds_ucsd_mpshim_register_wcdma_rlc WCDMA not active");
     return ( DS3G_FAILURE );
   }
#endif /* FEATURE_SEGMENT_LOADING */
   ...
   /* Set UL and DL WM reg cbacks to NULL */
   rlc_ul_reg_params.rlc_data[ 0 ].bearer_context = NULL;
   rlc_ul_reg_params.rlc_data[ 0 ].wm_reg_ind_cb = NULL;
   
   rlc_dl_reg_params.rlc_data[ 0 ].bearer_context = NULL;
   rlc_dl_reg_params.rlc_data[ 0 ].wm_reg_ind_cb = NULL;
...
#ifdef FEATURE_DUAL_WCDMA
     l2_dl_cmd_ptr->cmd_hdr.as_id = subs_id;
#endif
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DATA_CSD_MSG0_LOW("In ds_ucsd_mpshim_deregister_wcdma_rlc");
   
#ifdef FEATURE_SEGMENT_LOADING
   if(ds_3gpp_csd_wcdma_tbl == NULL)
   {
     DATA_CSD_MSG0_LOW("In ds_ucsd_mpshim_deregister_wcdma_rlc WCDMA not active");
     return ( FALSE );
   }
...
/*-------------------------------------------------------------------------*/
  ...
  DS3GPP_ASSERT( NULL != datapath_ptr );
...
/*-------------------------------------------------------------------------*/
   ...
   DS3GPP_ASSERT( NULL != call_desc_ptr );
   DS3GPP_ASSERT( NULL != chan_parms_ptr );

   DATA_CSD_MSG0_LOW( "Connecting CSD data path (mpshim)");
   ...
   */
       ...
       (call_desc_ptr->bearer_cap.sync_async == ASYNCHRONOUS) )
   {
     ...
     {
        DATA_CSD_MSG0_LOW("RCINIT Lookup for gcsd tcb returned NULL");
     }
#else
...
/*-------------------------------------------------------------------------*/
  ...
  DATA_CSD_MSG1_MED("In ds_ucsd_mpshim_call_cleanup: call ID=%d",
          call_desc_ptr->cm_call_id);
  ...
  if( TRUE == call_was_connected )
  {
    ...
    {
      DATA_CSD_MSG0_LOW("Problem disconnecting data path");
      result = DS3G_FAILURE;
    }
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS3GPP_ASSERT( NULL != call_desc_ptr );
  DS3GPP_ASSERT( NULL != chan_params_ptr );
  
  DATA_CSD_MSG3_MED ("Handover change config: dir=%d datarate=%d SDUsize=%d",
           direction,
  ...
  default:
    DATA_CSD_MSG1_LOW( "Unsupported handover direction: %d",
               direction );
    return FALSE;
  }
...
/*-------------------------------------------------------------------------*/
   ...
   {
     DATA_CSD_MSG0_LOW("Registered with NAS/MN for handover notification");
   }
