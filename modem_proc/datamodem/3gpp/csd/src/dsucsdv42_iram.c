...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp/csd/src/dsucsdv42_iram.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...

{
   ...
   {
     DATA_CSD_MSG2_LOW("NULL ptr input to find_match()2: %p, %p",ds_v42_refill_hash_flag
...
)
{
   ...
   {
     DATA_CSD_MSG1_LOW("ds_v42_next_code is a NULL ptr: %p",ds_v42_next_code);
     return;     
   }
...
)
{
   ...
   {
       DATA_CSD_MSG2_LOW("V42 table init failed: hash tbl ptr %p, prefix tbl ptr %p",
            tmp_hash_table_ptr,
