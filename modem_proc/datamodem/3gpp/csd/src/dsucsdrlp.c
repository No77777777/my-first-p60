...
Copyright (c) 2000 - 2020 Qualcomm Technologies, Incorporated and/or its subsidiaries.
All Rights Reserved.

Communications Consultants Worldwide Limited (C) 2000-2020.
...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp/csd/src/dsucsdrlp.c#3 $ $DateTime: 2020/11/16 21:05:49 $ $Author: pwbldsvc $
...
short rlp_read_UI_data (byte* Data)
{
   ...
   {
      DATA_CSD_MSG2_LOW("rlp_read_UI_data read problem: %d!=%d",
...
short rlp_send_UI_data (const byte* Data)
{
   ...
   if (UI_State == STATE_IDLE)
   {
      ...
      {
         DATA_CSD_MSG2_LOW("rlp_send_UI_data write problem: %d!=%d",
                   RLP_CURRENT_PDU_SIZE, cnt);
         return RLP_ERROR;
      }
...
short rlp_XID_send (const XID_Params_T* SendXID)
{
   ...
   {
      DATA_CSD_MSG1_LOW("XID_C_State!= STATE_IDLE or SendXID== NULL %p",SendXID);
      return RLP_ERROR;
   }
}
...
void rlp_main (void)
{
   ...
   */
   ...
   {
      DATA_CSD_MSG1_LOW("RLP in state %d exited without reading frame",RlpCurrentState);
  ...
  if ( TRUE == ds_ucsd_get_rlp_error_handling() )
  {
    ...
    if (TIMER_RUNNING (TIMERID_ADM_IDLE))
    {
      ...
      if (TIMER_EXPIRED (TIMERID_ADM_IDLE))
      {
        ...
        DATA_CSD_MSG0_HIGH("RLP: ADM state idle timer expired");
    ...
    else
    {
      ...
      if ( 0 < current_RLP_params->rlp_adm_idle_timer )
      {
        ...
        DATA_CSD_MSG0_HIGH("RLP: ADM state idle timer started");
      }
...
static void ActionPendingConnectionRequest (DSI_Event_T Event)
{
   ...
   if ((ds_ucsd_get_rlp_process_uplink()) && (ds_HardwareTxReady()))
   {
      ...
      if (TIMER_EXPIRED (TIMERID_T1))
      {
         ...
         {
            DATA_CSD_MSG0_LOW("RLP: Start ABM failed to complete");
...
static void ActionConnectionEstablished (DSI_Event_T Event)
{
   ...
   switch (Event)
   {
      ...
      case RLP_REMAP_REQ:
         ...
         if (status == RLP_ERROR)
         {
            DATA_CSD_MSG0_LOW("RLP: Problem on REMAP request");
            ds_ucsd_set_rlp_error_handling(TRUE);
...
static void ActionDiscInitiated (DSI_Event_T Event)
{
   ...
   if ((ds_ucsd_get_rlp_process_uplink()) && (ds_HardwareTxReady()))
   {
      ...
      if (TIMER_EXPIRED (TIMERID_T1))
      {
         ...
         {
            DATA_CSD_MSG0_LOW("RLP: Disconnect failed to complete");
            ds_ucsd_set_rlp_error_handling(TRUE);
...
boolean ProcessXIDCommand (dsm_item_type* PDU, XID_Params_T* Current)
{
   ...
   {
      DATA_CSD_MSG1_LOW("Illegal RLP version received = %d", ProposedXID.RLPVersion);
   }
...
boolean ProcessXIDResponse (dsm_item_type * PDU, XID_Params_T* Xid)
{
  ...
  {
    DATA_CSD_MSG0_LOW("Aborting ProcessXIDResponse on XID decode failure");
    return FALSE;
  }
        ...
        if ((XID_C_State == STATE_WAIT) && (PF))
        {
            ...
            if ( TRUE == ProcessXIDResponse (Frame->Data, &Rlp_CurrentXID) )
            {
              ...
              if (terminate)
              {
                ...
                DATA_CSD_MSG0_ERROR("RLP: Compression mode changed in balanced mode");
                RlpCurrentState  = RLPSTATE_ADM_ATTACHED;
                ds_ucsd_set_rlp_error_handling(TRUE);
              }
              else
...
static void CheckXIDTimeout(void)
{
   if (TIMER_EXPIRED (TIMERID_TXID))
   {
      ...
      {
        DATA_CSD_MSG0_LOW("RLP: XID negotiation failed to complete");
        ds_ucsd_set_rlp_error_handling(TRUE);
         ...
         else /* Frame is I + S or S */
         {
            ...
            */
            if ( (Nr > get_current_rlp_max_frame_number()) ||
                 (Nr < 0) )
            {
               DATA_CSD_MSG1_LOW("RLP: received invalid N(R)=%d",Nr);
               status = RLP_ERROR;
            }
         } /* Frame is I + S or S */
      } 
   ...
   else  /* Supervisory only frame */
   {
      ...
      */
             ...
             (Last_tx_nr != nr) ) )
      {
         ...
         DS3GPP_ASSERT (NULL != TxFrame.Data);
