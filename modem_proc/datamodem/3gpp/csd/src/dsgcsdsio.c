...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp/csd/src/dsgcsdsio.c#3 $ $DateTime: 2020/11/16 21:05:49 $ $Author: pwbldsvc $
...
ds_gcsd_setup_online_modem_control (void)
{
  ...
  */
  ...
  if( v24_state_changed )
  {
    ...
    {
      DATA_CSD_MSG3_LOW("Enable SIO Flow: HighWm=%d CTS=%d CTS(Saved)=%d",
              SIO_high_WM_state, v24_control.CTS, Saved_V24_control.CTS);
      SET_INBOUND_FLOW( TRUE ); /*lint !e506 */
    }
    else
    {
      DATA_CSD_MSG3_LOW("Disable SIO Flow: HighWm=%d CTS=%d CTS(Saved)=%d",
              SIO_high_WM_state, v24_control.CTS, Saved_V24_control.CTS);
  ...
  {
    DATA_CSD_MSG3_LOW("Skipping WM flow control op: HighWm=%d CTS=%d CTS(Saved)=%d",
            SIO_high_WM_state, v24_control.CTS, Saved_V24_control.CTS );
  }
