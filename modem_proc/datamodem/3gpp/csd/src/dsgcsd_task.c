...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp/csd/src/dsgcsd_task.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
{
   DS3GPP_ASSERT( NULL != bearer_cap_ptr );
...
void ds_gcsd_init_s30_timer_value(void)
{
  ...
  DATA_CSD_MSG1_HIGH("User data inactivity timer: %d sec",
...
void ds_gcsd_s30_timer_control(uint32 data_byte_count)
{
   ...
   DATA_CSD_MSG1_HIGH("User data inactivity timer control, byte count(%d)",
                   data_byte_count);
   ...
   if (cmd != NULL)
   {
      ...
      switch (cmd->hdr.cmd_id)
      {
         ...
         */   
         ...
         /* Should never happen */  
         default:
            DATA_CSD_MSG1_LOW("GCSD received illegal message ",cmd->hdr.cmd_id);
            break;
      }
   }
   return event;
