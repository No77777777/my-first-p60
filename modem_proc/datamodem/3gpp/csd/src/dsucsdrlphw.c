...
/*===========================================================================
  ...
  $PVCSPath:$
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp/csd/src/dsucsdrlphw.c#3 $ $DateTime: 2020/11/16 21:05:49 $ $Author: pwbldsvc $
...
)
{
   ...
   DS3GPP_ASSERT (NULL != *rlp_frame_buf);
...
)
{
   ...
   DS3GPP_ASSERT (NULL != *rlp_frame_buf);
   DS3GPP_ASSERT (0 == (*rlp_frame_buf)->used);  /* Ensure clean packet */  
...
void ds_HardwareRLPTx (void)
{
   ...
   */
   ...
   {
      DATA_CSD_MSG0_LOW("UCSD NT TX: Ds_ucsd_nt_get_ul_ota_buf_cb failed");
      return;
   }
   ...
   if (!rlp_tx_ready)
   {
      ...
      switch (Ds_ucsd_nt_bytes_per_sdu)
      {
         ...
         default:
            ...
            DATA_CSD_MSG1_LOW("Invalid UMTS NT TX frame size %d",
                             Ds_ucsd_nt_bytes_per_sdu);
            break;
        ...
        (ds_ucsd_nt_cb_info->read_dl_ota_data_cb(&radio_rx_data)))
   {
      ...
      switch (Ds_ucsd_nt_bytes_per_sdu)
      {
         ...
         default:
            ...
            DATA_CSD_MSG1_LOW("Invalid UMTS NT RX frame size %d",
                             Ds_ucsd_nt_bytes_per_sdu);
            break;
...
short ds_HardwareReceiveRLPFrame (RLP_Frame_T* Frame)
{
  ...
  if (rlp_rx_frame_available)
  {
...
#ifdef BOA_APPS
      {
        ...
        switch ( header_size  )
        {
          ...
          case RLP_HDR_SIZE_VER_2:
            ...
            break;
          default:
            DATA_CSD_MSG1_LOW("Bad RLP Header Size (%d)", header_size);
            break;
        }
    ...
    {
      DATA_CSD_MSG1_LOW("RLP: RX FCS failure, fs=%d",frame_size);
   ...
   /* Process Header bytes */
   {
      ...
      switch ( rlp_hdr_size )
      {
        ...
        case RLP_HDR_SIZE_VER_2:
          ...
          break;
        default:
          DATA_CSD_MSG1_LOW("Bad RLP Header Size (%d)", rlp_hdr_size);
          break;
      }
