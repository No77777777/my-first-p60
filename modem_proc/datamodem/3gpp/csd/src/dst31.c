...
/*=============================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp/csd/src/dst31.c#2 $
...
)
{
  ...
  if( t31_ctrl_blk->sio_dl_wm_ptr )
  {
    if (t31_ctrl_blk->sio_dl_wm_ptr->current_cnt == 0)
    {
      ...
      DATA_CSD_MSG0_HIGH("SIO TX flush completed");
    }
    else
    {
      if (ds3g_siolib_ex_flush_tx(dst31i_sio_flush_tx_cb, 
        ...
        {
          DATA_CSD_MSG0_LOW("Error flushing SIO TX queue");
        }

      DATA_CSD_MSG1_LOW("SIO data still pending, flushing %d bytes",
...
#endif
{
  ...
  if (t31_ctrl_blk->flags.sio_flowctl_asserted == FALSE)
  {
    ...
    DATA_CSD_MSG0_LOW( "SIO-UL:Flow-Control ASSERTED");
    if( t31_ctrl_blk->sio_ul_wm_ptr )
    {
      DATA_CSD_MSG3_MED("SIO-UL:Qcnt=%d Qhi=%d Qlo=%d", 
...
#endif
{
      ...
      (t31_ctrl_blk->flags.sio_flowctl_asserted == TRUE))
  {
    ...
    DATA_CSD_MSG0_LOW( "SIO-UL:Flow-Control DEASSERTED");
    if( t31_ctrl_blk->sio_ul_wm_ptr )
    {
      DATA_CSD_MSG3_MED("SIO-UL:Qcnt=%d Qhi=%d Qlo=%d", 
                  t31_ctrl_blk->sio_ul_wm_ptr->current_cnt,
...
#endif
{
  ...
  if (t31_ctrl_blk->flags.tps_flowctl_asserted == FALSE)
  {
    ...
    DATA_CSD_MSG0_LOW("TPS-UL:Flow-Control ASSERTED");
    if( t31_ctrl_blk->tps_ul_wm_ptr )
    {
      DATA_CSD_MSG3_MED("TPS-UL:Qcnt=%d Qhi=%d Qlo=%d", 
...
#endif
{
  ...
  if (t31_ctrl_blk->flags.tps_flowctl_asserted == TRUE)
  {
    ...
    DATA_CSD_MSG0_LOW("TPS-UL:Flow-Control DEASSERTED");
    if( t31_ctrl_blk->tps_ul_wm_ptr )
    {
      DATA_CSD_MSG3_MED("TPS-UL:Qcnt=%d Qhi=%d Qlo=%d", 
                  t31_ctrl_blk->tps_ul_wm_ptr->current_cnt,
...
)
{
  ...
  DATA_CSD_MSG0_LOW("Resetting T31 context");
  ...
  */
  ...
  {
    DATA_CSD_MSG0_LOW("FINAL AT result DSAT_ERROR for pending cmd's");
  ...
  if (end_of_call == TRUE)
  {
    ...
    DATA_CSD_MSG5(MSG_LEGACY_HIGH,"Total Fax HDLC TX bytes=%d, Total Fax HDLC RX bytes=%d,\
    ...
    DATA_CSD_MSG0_LOW("Session Complete");
  }
...
)
{
  ...
  */
  if (dsat_result_type == DST31_DSAT_RESULT_FINAL)
  {
    ...
    {
      DATA_CSD_MSG1_LOW("FINAL AT result via async, RC=%d", dsat_result);
      (*t31_ctrl_blk->dsat_callback)(dsat_result);
    }
    else
    {
      /* This is used as a catch all if the callback isn't defined */
      DATA_CSD_MSG1_LOW("FINAL AT result no async, RC=%d", dsat_result);
      dsat_send_intermediate_result_code(dsat_result);
    }
  }
  else
  {
    DATA_CSD_MSG1_LOW( "Send inter-AT result, RC=%d", dsat_result);
    ...
    */
    ...
    DATA_CSD_MSG0_LOW( "Scheduling SIO TX FLUSH");
    if (ds3g_siolib_ex_flush_tx(dst31i_sio_flush_tx_cb,
  ...
  */
      ...
      (t31_ctrl_blk->sio_mode != new_sio_mode))
  {
    ...
    */
    ...
    {
      DATA_CSD_MSG0_LOW("Request ONLINE_CMD mode change");
      ds_cmd = DS_CMD_ENTER_ONLINE_CMD_TX_FLUSH;
    }
    else
    {
      DATA_CSD_MSG0_LOW("Request ONLINE_DATA mode change");
    ...
    {
      DS_3GPP_MSG1_LOW("dst31i_send_ds_cmd() failed, cmd=%d", (int)ds_cmd); /*lint !e571 */
    }
...
)
{
  ...
  {
    DATA_CSD_MSG2_LOW("+FTH: HDLC-EOF type=0x%x cnt=%d", 
    ...
    DATA_CSD_MSG1_LOW("+FTH::calculated FCS=0x%x",io_data_ptr->fcs);
  ...
  switch (io_data_ptr->dle_data_state)
  {
    case DST31_FIND_DLE_STATE:
      if (data_byte == DST31_DLE_CHAR)
      {
        ...
        DATA_CSD_MSG0_LOW("found <DLE> new state=DST31_DLE_START_STATE");
        io_data_ptr->dle_data_state = DST31_DLE_START_STATE;
      }
      else
      {
        /* Append the data into the buffer and keep searching for a <DLE> */
        DATA_CSD_MSG1_LOW( "buffering raw data, val=0x%x", data_byte);
    ...
    case DST31_DLE_START_STATE:
      switch (data_byte)
      {
        case DST31_ETX_CHAR:
          ...
          DATA_CSD_MSG0_LOW("found <DLE><ETX>, end-of-data");
          break;
        ...
        case DST31_DLE_CHAR:
          ...
          DATA_CSD_MSG0_LOW("replace <DLE><DLE> -> <DLE>");
        ...
        case DST31_SUB_CHAR:
          ...
          DATA_CSD_MSG0_LOW("replace <DLE><SUB> -> <DLE><DLE>");
        ...
        default:
          ...
          DATA_CSD_MSG1_LOW("remove invalid <DLE><0x%x> sequence", data_byte);
    ...
    case DST31_DLE_STOP_STATE:
      DATA_CSD_MSG2_LOW("DLE filtering complete, bc=%d +DLEs=%d",
        io_data_ptr->cnt, t31_ctrl_blk->byte_cnts.dle_bytes);
      break;

    default:
      DATA_CSD_MSG2_LOW("invalid DLE state, state=%d cnt=%d",
...
)
{
  ...
  switch (io_data_ptr->dle_data_state)
  {
    case DST31_FIND_DLE_STATE:
      if (data_byte == DST31_DLE_CHAR)
      {
        DATA_CSD_MSG0_LOW("found <DLE>, goto DST31_DLE_START_STATE");
        io_data_ptr->dle_data_state = DST31_DLE_START_STATE;
      }
      else
      {
    ...
    case DST31_DLE_START_STATE:
      switch (data_byte)
      {
        case DST31_DLE_CHAR:
          ...
          if (t31_ctrl_blk->at_params.fdd == 1)
          {
            DATA_CSD_MSG0_LOW("replace <DLE><DLE> -> <DLE><SUB><DLE><SUB>");
            io_data_ptr->buf[io_data_ptr->cnt++] = DST31_DLE_CHAR;
          ...
          {
            DATA_CSD_MSG0_LOW("replace <DLE><DLE> -> <DLE><DLE><DLE><DLE>");
            io_data_ptr->buf[io_data_ptr->cnt++] = DST31_DLE_CHAR;
            io_data_ptr->buf[io_data_ptr->cnt++] = DST31_DLE_CHAR;
            io_data_ptr->buf[io_data_ptr->cnt++] = DST31_DLE_CHAR;
            io_data_ptr->buf[io_data_ptr->cnt++] = DST31_DLE_CHAR;
          ...
          DATA_CSD_MSG0_LOW("goto DST31_FIND_DLE_STATE");
          io_data_ptr->dle_data_state = DST31_FIND_DLE_STATE;
          break;
        ...
        default:
          ...
          */
          DATA_CSD_MSG1_LOW("inserting <DLE><DLE><0x%x>", data_byte);
          io_data_ptr->buf[io_data_ptr->cnt++] = DST31_DLE_CHAR;
          io_data_ptr->buf[io_data_ptr->cnt++] = DST31_DLE_CHAR;
          io_data_ptr->buf[io_data_ptr->cnt++] = data_byte;
...
)
{
  ...
  if ((pkt_sz = (uint16)dsm_length_packet(dsm_ptr)) > 0)
  {
    ...
    for (i = 0; i < pkt_sz; i++)
    {
      DATA_CSD_MSG2_LOW("  -->buf[%d]=0x%x", i,zbuf[i]);
    }
  }
...
)
{
  ...
  DATA_CSD_MSG0_LOW("Protocol Error, session terminated!");
  ...
  {
    DS_3GPP_MSG1_LOW("dst31i_send_ds_cmd() failed, cmd=%d",
      (int)DS_CMD_3GPP_GCSD_DISCONNECT);
  }
#else
  DATA_CSD_MSG1_HIGH("Protocol Error, session terminated!,"
...
)
{
  ...
  DATA_CSD_MSG1_HIGH( "%d detected", fa_stat);
  ...
  switch (fa_stat)
  {
    ...
    default:
      DATA_CSD_MSG1_LOW("unknown FA status, value=%d", fa_stat);
   } /* switch */
...
)
{
  ...
  /* Safety net */
  DS3GPP_ASSERT(in_data_ptr != NULL);
  DS3GPP_ASSERT(out_data_ptr != NULL);
  DS3GPP_ASSERT(last_frame_ptr != NULL);
  DS3GPP_ASSERT(fcs_stat_ptr != NULL);
  ...
  *fcs_stat_ptr = DST31_FCS_CALC_PENDING;
      ...
      ((in_byte_cnt + io_data_ptr->cnt) < ARR_SIZE(io_data_ptr->buf)))
  {
    ...
    */
    ...
    DATA_CSD_MSG2_LOW("+FRH: frame_stat=%d frame_sz=%d",
    ...
    if (frame_stat == DST31_FRAME_COMPLETE) 
    {
      if (io_data_ptr->cnt >= DST31_HDLC_MIN_FRAME_SZ) 
      {
        ...
        */
        ...
        DATA_CSD_MSG2_LOW("+FRH::frame_id=0x%x FCS=0x%x",
      ...
      {
        DATA_CSD_MSG0_LOW("+FRH: invalid HDLC frame rx'd, toss and reset");
  ...
  else if (in_byte_cnt)
  {
    ...
    */
    DATA_CSD_MSG2_LOW("+FRH::buffer overflow, rx_bytes=%d buf_sz=%d",
      (in_byte_cnt + io_data_ptr->cnt), ARR_SIZE(io_data_ptr->buf));
...
)
{
      ...
      ((in_byte_cnt + io_data_ptr->cnt) < ARR_SIZE(io_data_ptr->buf)))
  {
    ...
    */
    ...
    {
      DATA_CSD_MSG0_LOW("+FTH: HDLC-SOF");
    }
  ...
  else if (in_byte_cnt)
  {
    ...
    */
    DATA_CSD_MSG2_LOW("+FTH::buffer overflow, rx_bytes=%d buf_sz=%d",
      (in_byte_cnt + io_data_ptr->cnt), ARR_SIZE(io_data_ptr->buf));
...
)
{
      ...
      ((in_byte_cnt + io_data_ptr->cnt) < ARR_SIZE(io_data_ptr->buf)))
  {
    ...
    */
    ...
    {
      DATA_CSD_MSG0_LOW("+FRM::found NULL message frame!");
  ...
  else if (in_byte_cnt)
  {
    ...
    ** toss the data.
    */
    DATA_CSD_MSG2_LOW("+FRM::buffer overflow, rx_bytes=%d buf_sz=%d",
      (in_byte_cnt + io_data_ptr->cnt), ARR_SIZE(io_data_ptr->buf));
...
)
{
    ...
    */
    ...
    if (frame_stat & (DST31_FRAME_COMPLETE | DST31_FRAME_READY)) /*lint !e655 */
    {
      ...
      */
      if (io_data_ptr->cnt > 0)
      {
        DATA_CSD_MSG1_LOW("+FTM::push %d bytes",io_data_ptr->cnt);
  ...
  else if (in_byte_cnt)
  {
    ...
    ** layer to return an ERROR condition to the DTE.
    */
    DATA_CSD_MSG2_LOW("+FTM::buffer overflow, msg_cnt=%d buf_sz=%d",
      (in_byte_cnt + io_data_ptr->cnt), ARR_SIZE(io_data_ptr->buf));
...
)
{
  ...
  */
  if (new_state != t31_ctrl_blk->curr_state)
  {
    ...
    DATA_CSD_MSG3_HIGH("Change state: last=%d curr=%d new=%d", 
    ...
    */
    ...
    if (new_state == DST31_IDLE_STATE)
    {
      ...
      DATA_CSD_MSG0_LOW("Now in IDLE state");
    }
  ...
  {
    DATA_CSD_MSG1_LOW("Erroneous state transition state=%d",new_state);
  }
} /* dst31i_change_state */
...
)
{
  ...
  DATA_CSD_MSG1_LOW("Terminate state, set_fa_to_idle=%d", set_fa_to_idle);
...
)
{
  ...
  {
    DATA_CSD_MSG1_LOW("push error, invalid size=%d",data_size);
...
{  
  ...
  DATA_CSD_MSG0_HIGH("powerup-init completed");
...
)
{
  ...
  DATA_CSD_MSG0_LOW("AT command aborted by DTE");
...
)
{
  ...
  DS3GPP_ASSERT( NULL != param_ptr );
  
  DATA_CSD_MSG0_LOW("Updating the FPS paramaters form ATCOP");
...
)
{
  ...
  */
    ...
    if (DST31_IN_VALID_RX_STATE(t31_ctrl_blk->curr_state))
    {
      ...
      */
      ...
      if (sio_data_ptr)
      {
        ...
        DATA_CSD_MSG2_HIGH("DTE aborted action, st=%d qcnt=%d",
        ...
        else
        {
          ...
          */
          DATA_CSD_MSG0_LOW("FaxAppl aborting call. Too long for carrier setup");
  ...
  {   
    ...
    if (dl_data_ptr)
    { 
      DATA_CSD_MSG2_LOW("Unexpected RX data in TX state,"
    ...
    {
      DATA_CSD_MSG0_LOW("FA failure during downlink");
  ...
  if (t31_ctrl_blk->flags.set_fa_to_idle == TRUE)
  {
    ...
    */
    ...
    else
    {
      ...
      */
      DATA_CSD_MSG0_LOW("WFO::UL data pending, FA not set to IDLE");
...
)
{
  ...
  /* Safety net */
  DS3GPP_ASSERT(event_ptr != NULL );

  DATA_CSD_MSG1_HIGH("received %d", event_ptr->hdr.cmd_id);
  ...
  switch (event_ptr->hdr.cmd_id)
  {
    ...
    case DS_GCSD_CONNECT_MT_REQ:
      ...
      {
        DATA_CSD_MSG0_LOW("received DS_GCSD_CONNECT_MO_REQ");
      }
      else
      {
        DATA_CSD_MSG0_LOW("received DS_GCSD_CONNECT_MT_REQ");
      }
...
)
{
  ...
  {
    DATA_CSD_MSG1_HIGH("TIMER expiration detected, type=%d",
    ...
    switch (t31_ctrl_blk->timer_type)
    {
      case DST31_DCN_TIMER_TYPE:
        DATA_CSD_MSG0_LOW("DCN complete, go IDLE");
      ...
      case DST31_CD_TIMER_TYPE:
        DATA_CSD_MSG0_LOW("CD timeout, send CONNECT");
      ...
      */
      ...
      default:
        DATA_CSD_MSG1_LOW("Unknown/Invalid timer detected, type=%d", 
          t31_ctrl_blk->timer_type);
    } /* switch */
  }
} /* dst31_process_signals */
...
)
{
  ...
  DATA_CSD_MSG0_LOW("Call type is %d", t31_ctrl_blk->call_type);
...
)
{
  ...
  DATA_CSD_MSG0_LOW("Call CONNECTED initialization");
 ...
 DATA_CSD_MSG0_LOW("Call CONNECTED init complete");
} /* dst31_call_connect_init */
