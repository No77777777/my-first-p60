...
Initialization and Sequencing Requirements:
 ...
 - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  Copyright (c) 2002 - 2015 Qualcomm Technologies Incorporated. All Rights Reserved
...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp/csd/src/dsgcsdfatrans.c#2 $
...
{ \
  DATA_CSD_MSG3_HIGH("f[0]=0x%x f[1]=0x%x f[2]=0x%x", \
    frame[0] & 0xff,frame[1] & 0xff,frame[2] & 0xff); \
  DATA_CSD_MSG3_HIGH("f[3]=0x%x f[4]=0x%x f[5]=0x%x",\
    frame[3] & 0xff,frame[4] & 0xff,frame[5] & 0xff); \
  DATA_CSD_MSG3_HIGH("f[6]=0x%x f[7]=0x%x", \
...
)
{
  ...
  while (TRUE) /*lint !e716*/
  {
    ...
    */
    ...
    DATA_CSD_MSG3_LOW("B1=0x%x B2=0x%x NewByte=0x%x", 
    ...
    DS3GPP_ASSERT(push_count == sizeof(new_byte));
...
)
{
       ...
       (ctrl_blk_ptr->fa_state_trans == FA_STATE_TRANS_MSG_REC) || override))
  {
    if (override == TRUE)
    {
      DATA_CSD_MSG2_LOW("OVERRIDE: Send %d SYNC frames, cnt=%d", 
                  ui16_num_frames, ctrl_blk_ptr->uplink_wm_ptr->current_cnt);
    }

    DATA_CSD_MSG2_LOW("Sending %d SYNC frame(s) cnt=%d", 
               ui16_num_frames, ctrl_blk_ptr->uplink_wm_ptr->current_cnt);
    ...
    if (dsm_item_ptr)
    {
      ...
      DATA_CSD_MSG3_LOW("TPS-UL:Qcnt=%d Qhi=%d Qlo=%d", 
  ...
  if (dsm_item_ptr)
  {
    if (f_sync_first)
    {
      ...
      ** STATUS, there should only be one STATUS behind it. */
      DATA_CSD_MSG1_LOW("Sending %d SYNC+STATUS frame(s)", ui16_num_frames);
    }
    else
    {
      DATA_CSD_MSG1_LOW("Sending %d STATUS frame(s)", ui16_num_frames);
    }
    ...
    ** if we're in BCS_REC.  It's INFO_FIELD_IDLE in the other states. */
    do
    {
      ...
      {
        DATA_CSD_MSG1_LOW("Exiting loop, array overrun %d", ui16_count);
        break;
      }
  ...
  */
  if (ctrl_blk_ptr->f_synchronized == TRUE)
  {
    if (ctrl_blk_ptr->bits_to_shift != -1)
    {
      ...
      {
        DATA_CSD_MSG3_LOW("SYNC[0] wrong: fp[0]=0x%x sync[0]=0x%x idx=%d",
      ...
      */
      if (ctrl_blk_ptr->bits_to_shift == -1)
      {
        ...
        */
        ...
        DATA_CSD_MSG3_HIGH("SYNC Found, idx=%d bits=%d val=0x%x",
...
)
{
  ...
  DATA_CSD_MSG0_LOW("Look for SYNC frame");
  ...
  while (TRUE) /*lint !e716*/
  {
    ...
    if (bytes_available < FRAME_SIZE)
    {
      ...
      DATA_CSD_MSG1_LOW("Did not find SYNC! bytes=%d",bytes_available);
      return(FALSE);
    }
    ...
    else
    {
      ...
      {
        DATA_CSD_MSG1_LOW("SYNC officially lost, hyst=%d",
          FA_SYNC_FRAME_LOSS_HYSTERISIS);
...
)
{
  ...
  DATA_CSD_MSG0_LOW("In BCS_TRA trying to get SYNC'ed");
      ...
      !ctrl_blk_ptr->f_goto_idle)
  {
    ...
    ** just lost sync.  Start up the sync loss timer. */
    DATA_CSD_MSG1_LOW("Start %dmsec BCS_TRA sync loss timer", 
      SYNC_LOSS_DURATION);
  ...
  ** the number of OTA bytes we're gonna need to have just one BCS byte. */
  ...
  while (TRUE)  /*lint !e716*/
  {
    ...
    if (bytes_available < FRAME_SIZE)
    {
      ...
      FA_FRAME_DUMP(frame_ptr, FALSE);  /*lint !e506 !e774*/
      DATA_CSD_MSG1_LOW("Incomplete frame, bytes=%d!", bytes_available);
      break;
    }
    ...
    DATA_CSD_MSG3_LOW("bcs_data=0%x sync?=%d status?=%d",
    ...
    if (found_sync || found_status)
    {
      ...
      *f_synchronized = TRUE;
      ...
      if (found_sync)
      {
        DATA_CSD_MSG0_LOW("Found SYNC frame!");
      }
      else
      {
        DATA_CSD_MSG2_LOW("Found STATUS frame! typ=%d data=%x", 
      ...
      if ((rex_get_timer(ctrl_blk_ptr->timer_ptr) != 0) &&
          (ctrl_blk_ptr->timer_type == FA_SYNC_LOSS_TIMER_TYPE))
      {
        DATA_CSD_MSG0_LOW("Stopping SYNC loss timer");
  ...
  ** received frames are crap, and zeros seem as benign as anything. */
  ...
  DATA_CSD_MSG1_HIGH("Giving %d fake BCS bytes to T31", 
  ...
  switch (ctrl_blk_ptr->data_rate)
  {
    ...
    default:
      ...
      ** an error meaning T31 should abort the call. */
      DATA_CSD_MSG1_ERROR("Bad OTA rate: %d", ctrl_blk_ptr->data_rate);
  ...
  ** ui16_frames_per_bcs_byte = OTA Rate / 300 bps / 8 (FRAME_SIZE) bytes */
  switch (ctrl_blk_ptr->data_rate)
  {
    ...
    default:
      ...
      ** rate is.  Just say 1 STATUS frame for the hell of it, and return
      ** an error meaning T31 should abort the call. */
      DATA_CSD_MSG1_LOW("Bad OTA rate: %d", ctrl_blk_ptr->data_rate);
...
)
{
  ...
  {
    DATA_CSD_MSG1_LOW("Same state transition, state=%d", new_state);
    return(DS_FA_SUCCESS);
  }
      ...
      (ctrl_blk_ptr->fa_state_trans == FA_STATE_TRANS_MSG_TRA))
  {
    ...
    {
      DATA_CSD_MSG0_LOW("End TRAINING phase");
    ...
    {
      DATA_CSD_MSG1_LOW("Page Complete: Total Rx MSG bytes: %d", 
                  ctrl_blk_ptr->ui32_msg_byte_count);
      if (T31_fake_train_cnt > 0)
      {
        DATA_CSD_MSG1_LOW("Page Complete: Fake Train bytes: %d", 
                    T31_fake_train_cnt);
      }
      if (T31_real_train_cnt > 0)
      {
        DATA_CSD_MSG1_LOW("Page Complete: Real Train bytes: %d", 
                    T31_real_train_cnt);
      }
      if (TPS_byte_cnt > 0)
      {
        DATA_CSD_MSG1_LOW("Page Complete: TPS Rx MSG bytes: %d", 
                    TPS_byte_cnt);
      }
      if (T31_byte_cnt > 0)
      {
        DATA_CSD_MSG1_LOW("Page Complete: T31 Rx MSG bytes: %d", 
                    T31_byte_cnt);
      }
    }
    else
    {
      DATA_CSD_MSG1_LOW("Page Complete: Total Tx MSG bytes: %d", 
                  ctrl_blk_ptr->ui32_msg_byte_count);
      if (T31_fake_train_cnt > 0)
      {
        DATA_CSD_MSG1_LOW("Page Complete: Fake Train bytes: %d", 
      ...
      if (TPS_byte_cnt > 0)
      {
        DATA_CSD_MSG1_LOW("Page Complete: TPS Tx MSG bytes: %d", 
                    TPS_byte_cnt);
      }
      if (T31_byte_cnt > 0)
      {
        DATA_CSD_MSG1_LOW("Page Complete: T31 Tx MSG bytes: %d", 
                    T31_byte_cnt);
      }
    }
  }
  ...
  switch (new_state)
  {
    ...
    case FA_STATE_TRANS_MSG_REC:
      if (ctrl_blk_ptr->i16_bytes_between_sync_frames == UNKNOWN_DATA_PER_SYNC)
      {
        ...
        ** be considered fatal.  Let's just assume it's not a goofy rate. */
        DATA_CSD_MSG0_LOW("Unknown fax rate! Assume standard!");
      ...
      {
        DATA_CSD_MSG0_LOW("Transmitting fax PAGE Data");
      }
      else
      {
        DATA_CSD_MSG0_HIGH("Transmitting fax TRAINING Data");
      }
    ...
    case FA_STATE_TRANS_MSG_TRA:
      if (ctrl_blk_ptr->i16_bytes_between_sync_frames == UNKNOWN_DATA_PER_SYNC)
      {
        ...
        ** entering the MSG phase, and we don't know the fax rate.  This could
        ** be considered fatal.  Let's just assume it's not a goofy rate. */
        DATA_CSD_MSG0_HIGH("Unknown fax rate!! Assume standard!");
        ctrl_blk_ptr->i16_bytes_between_sync_frames = DATA_FRAMES_ONLY;
      }
      ...
      {
        DATA_CSD_MSG1_LOW("Start %dmsec TCF timer", TRAINING_DURATION);
    ...
    default:
      ...
      DATA_CSD_MSG0_LOW("Bad fax adapter state!");
      break;
  }
...
{
  DATA_CSD_MSG1_LOW ("Invalid T31 data in state: state_num=%d", 
...
)
{
  ...
  {
    DATA_CSD_MSG0_LOW("FA: now in MSG_TRA, notify remote FA");
...
)
{
  ...
  DS3GPP_ASSERT(fax_uplink_data != NULL);
...
)
{
  ...
  DATA_CSD_MSG1_LOW("Sending %d BCS bytes", dsm_length_packet(fax_uplink_data));
  if( ctrl_blk_ptr->uplink_wm_ptr )
  {
    DATA_CSD_MSG3_LOW("TPS-UL:Qcnt=%d Qhi=%d Qlo=%d", 
               ctrl_blk_ptr->uplink_wm_ptr->current_cnt,
  ...
  ** generate lots of bad data to force loss of synchronization. */
  if (memcmp(fax_uplink_data->data_ptr, "Lose Sync Now", 13) == 0)
  {
    ...
    for (i16_bcs_byte = 0; i16_bcs_byte < 5; i16_bcs_byte++)
    {
      ...
      {
        DS_3GPP_ERROR_LOG_0("DSM item pool exhausted");
      }
      else
      {
  ...
  {
    DATA_CSD_MSG1_LOW("FA-BCS: data=0x%x", i16_bcs_byte);
...
)
{
    ...
    if (fax_uplink_data != NULL)
    {
      if (ctrl_blk_ptr->i16_bytes_between_sync_frames == DATA_FRAMES_ONLY)
      {
        ...
        ** don't add SYNC frames to the DATA.  Just send to GCSD-TPS. */
        ...
        DATA_CSD_MSG1_LOW("%d DATA bytes to TPS", ui16_dsm_length);
        ...
        while (ret_val == DS_FA_UPLINK_COMPLETE)
        {
          ...
          if (ui16_num_bytes < ctrl_blk_ptr->i16_bytes_to_next_sync)
          {
            ...
            ** Just send what we got from T31 right down protocol stack. */
            ...
            DATA_CSD_MSG1_LOW("2:: %d DATA bytes to TPS", ui16_num_bytes);
                   ...
                   (uint16)ctrl_blk_ptr->i16_bytes_to_next_sync)
          {
            ...
            ** SYNC frame right down the protocol stack. */
            ...
            DATA_CSD_MSG1_LOW("%d DATA bytes AND SYNC to TPS", ui16_num_bytes);
          ...
          else
          {
            ...
            ** so one... */
            ...
            DATA_CSD_MSG1_LOW("%d DATA bytes AND SYNC to TPS.",
                       ctrl_blk_ptr->i16_bytes_to_next_sync);
  ...
  else
  {
    ...
    {
      DATA_CSD_MSG1_LOW("remote not ready, %d UL bytes pending", 
  ...
  while (ctrl_blk_ptr->fa_state_trans == FA_STATE_TRANS_IDLE)
  {
    if (!*f_synchronized)
    {
      ...
      *f_synchronized = look_for_sync_frame(dsm_ptr, ctrl_blk_ptr);
      if (*f_synchronized)
      {
        DATA_CSD_MSG0_LOW("NOW synchronized in IDLE");
      }
    }
    ...
    ** frame's worth of data to look at, and this extract will also fail. */
    ...
    if (better_dsm_extract(*dsm_ptr, &frame_ptr, FRAME_SIZE) == FRAME_SIZE)
    {
      if (is_it_sync_frame(frame_ptr, ctrl_blk_ptr))
      {
        ...
        {
          DATA_CSD_MSG0_LOW("FA in SYNC with REMOTE FA");
          DATA_CSD_MSG1_LOW("delay %dms for CED processing",CED_TONE_DELAY);
	...
	DATA_CSD_MSG1_HIGH("Remote FA in %d",frame_type);
        ...
        switch (frame_type)
        {
          ...
          case FA_STATE_TRANS_BCS_REC:
            ...
            ** BCS_TRA.  We are now receiving BCS data from remote FA. */
            ...
            if (ctrl_blk_ptr->f_send_connect == TRUE)
            {
              DATA_CSD_MSG0_LOW("Remote FA in BCS_REC: send CONNECT");
          ...
          default:
            ...
            DATA_CSD_MSG1_LOW("In IDLE got bad remote FA STATUS: frame_type=%d",
                        frame_type);
            break;
        } /* switch */
      }
      else
      {
        ...
        DATA_CSD_MSG0_HIGH("SYNC loss in IDLE");
        FA_FRAME_DUMP(frame_ptr, TRUE);  /*lint !e506 !e774*/
        *f_synchronized = FALSE;
        ...
        if ((ctrl_blk_ptr->f_send_connect == TRUE) &&
            (ctrl_blk_ptr->f_call_is_MO == FALSE))
        {
          DATA_CSD_MSG0_LOW("Resetting SYNC frame hysterisis");
          ctrl_blk_ptr->sync_hysterisis_cnt = 0;
        }
      }
  ...
  while (TRUE)  /*lint !e716*/
  {
    ...
    ** frame's worth of data to look at, and this extract will also fail. */
    ...
    if (cnt  < FRAME_SIZE)
    {
      ...
      ** enough data to play with either way.  So get outta here. */
      FA_FRAME_DUMP(frame_ptr, FALSE);  /*lint !e506 !e774*/
      DATA_CSD_MSG1_LOW("Incomplete frame, bytes=%d!", cnt);
      break;
    }
    ...
    else
    {
      ...
      DATA_CSD_MSG0_LOW("SYNC lost in BCS_REC downlink");
      FA_FRAME_DUMP(frame_ptr, TRUE);  /*lint !e506 !e774*/
      *f_synchronized = FALSE;
    }
  }
  ...
  ** Also leaves if we run out of data. */
      ...
      */
          ...
          (ctrl_blk_ptr->f_start_dcs_timer == TRUE))
      {
        if (ctrl_blk_ptr->f_start_dcs_timer == TRUE)
        {
          ...
          */
          DATA_CSD_MSG1_LOW("Start short %dmsec DCS->TCF timer",
    ...
    */
    ...
    if (is_it_sync_frame(frame_ptr, ctrl_blk_ptr))
    {
      ...
      ** to IDLE and throw away the frame. */
      DATA_CSD_MSG0_HIGH("going IDLE in BCS_TRA");
      ...
      */
      if (ctrl_blk_ptr->f_start_dcs_timer == TRUE)
      {
        DATA_CSD_MSG1_LOW("Start %dmsec DCS->TCF timer", DCS_TO_TRAIN_DELAY);
    ...
    {
      DATA_CSD_MSG3_LOW("Found STATUS frame=0x%x in bcs_tra, dsmLen=%d bytes_per_bcs=%d",
      ...
      if (dsm_length_packet(*dsm_ptr) >= ui16_bytes_per_bcs_byte)
      {
        ...
        ** process "implementation dependent". */
        ...
        if (frame_type != FA_STATE_TRANS_BCS_REC)
        {
         ...
         DATA_CSD_MSG1_LOW("Remote FA: frame_type=%d, should be: BCS_REC",
                      frame_type);
        ...
        DATA_CSD_MSG1_LOW("Putting bcs_data=0x%x into dsm_item", ui8_bcs_data);
    ...
    else
    {
      ...
      *f_synchronized = FALSE;
      ...
      {
        DATA_CSD_MSG0_LOW("SYNC lost in BCS_TRA downlink");
      }
      else
      {
        DATA_CSD_MSG0_LOW("Cannot find SYNC in BCS_TRA downlink");
        ret_val = DS_FA_FAILURE;
        break;
      }
  ...
  if (*fax_downlink_data != NULL)
  {
    *fax_type_of_data = DS_FA_DATA_BCS;
    ...
    DATA_CSD_MSG1_LOW("Returning %d BCS bytes to T31", 
  ...
  while (TRUE)  /*lint !e716*/
  {
    if (!*f_synchronized)
    {
      ...
      *f_synchronized = look_for_sync_frame(dsm_ptr, ctrl_blk_ptr);
      ...
      {
        DATA_CSD_MSG0_HIGH("SYNC found in MSG_REC");
      }
    }
    ...
    ** frame's worth of data to look at, and this extract will also fail. */
    ...
    if (better_dsm_extract(*dsm_ptr, &frame_ptr, FRAME_SIZE) == FRAME_SIZE)
    {
      if (!is_it_sync_frame(frame_ptr, ctrl_blk_ptr))
      {
          ...
          switch (frame_type)
          {
            ...
            case FA_STATE_TRANS_MSG_TRA:
              ...
              ** sending him DATA frames on the uplink.  Set flag. */
              ...
              {
                DATA_CSD_MSG0_LOW("remote FA now in MSG_TRA");
              }
            ...
            case FA_STATE_TRANS_BCS_REC:
              ...
              DATA_CSD_MSG1_LOW("MSG_TRA: trailing BCS frame: type=%d", frame_type);
              break;
            ...
            default:
              ...
              ** so do nothing. */
              DATA_CSD_MSG1_LOW("MSG_TRA: Bad remote STATUS frame: st=%d", frame_type);
              break;
          }
        }
        else
        {
          /* Crap!  It's neither SYNC nor STATUS frame.  Gotta start over. */
          DATA_CSD_MSG0_LOW("SYNC lost in MSG_REC downlink");
          FA_FRAME_DUMP(frame_ptr, TRUE);  /*lint !e506 !e774*/
          *f_synchronized = FALSE;
        }
      }
  ...
  *fax_type_of_data  = DS_FA_DATA_NONE;
  ...
  if (*f_synchronized)
  {
    ...
    ** or change state. */
        ...
        if (is_it_status_frame(data_ptr, &frame_type, &ui8_data))
        {
              ...
              (frame_type != FA_STATE_TRANS_BCS_REC))
          {
            ...
            ** state we're in.  Let's just go IDLE and hope for the best. */
            DATA_CSD_MSG1_LOW("MSG_TRA: bad remote FA STATUS: ft=%d",
                        frame_type);
        ...
        else
        {
          if (ctrl_blk_ptr->f_training_phase == TRUE) 
          {
            ...
            if (frame_value == 0)
            {
              ...
              DATA_CSD_MSG0_LOW("SYNC lost, RX TRAINING data");
          ...
          {
            FA_FRAME_DUMP(data_ptr, TRUE);  /*lint !e506 !e774*/
            DATA_CSD_MSG0_LOW("SYNC lost, RX PAGE data");
      ...
      ** a rate equivalent to the incoming data rate. */
      if (ctrl_blk_ptr->f_training_phase)
      {
        ...
        for (i = 0 ; i < frame_cnt; i++)
        {
          ...
          {
            DATA_CSD_MSG1_LOW("Generating %d train bytes for DTE", cnt);
    ...
    {
      DATA_CSD_MSG1_LOW("Generating %d train bytes for DTE", 
        ui16_data_per_frame);
    }
#endif
  }
    ...
    ** to IDLE or have a DSM item allocation failure. */
    while (TRUE)  /*lint !e716*/
    {
      ...
      if (is_it_sync_frame(data_ptr, ctrl_blk_ptr))
      {
        ...
        DS3GPP_ASSERT(bytes_available == FRAME_SIZE);

        DATA_CSD_MSG0_LOW("MSG_TRA found SYNC_FRAME");
             ...
             DATA_FRAMES_ONLY) || ctrl_blk_ptr->f_got_sync_last_time)
        {
          ...
          ** Fax data reception has officially ended.  :( */
          DATA_CSD_MSG0_LOW("going IDLE in MSG_TRA");
  ...
  if (*fax_downlink_data != NULL)
  {
    ...
    *fax_type_of_data = DS_FA_DATA_MSG;
    ...
    DATA_CSD_MSG1_LOW("Returning %d DATA bytes to T31", ui16_dsm_length);
    T31_byte_cnt += ui16_dsm_length;
...
{
  DATA_CSD_MSG0_LOW("FA initialization started");

  if (ctrl_blk_ptr->uplink_buf_dsm_ptr != NULL)
  {
    /* Indicates memory leak in previous call. */
    DATA_CSD_MSG0_LOW("uplink_buf_dsm_ptr not NULL on call init");
  ...
  {
    /* Indicates memory leak in previous call. */
    DATA_CSD_MSG0_LOW("downlink_buf_dsm_ptr not NULL on call init");
  ...
  DATA_CSD_MSG0_HIGH("FA initialization completed. FA waiting to synchronize!");
...
)
{
  ...
  ** rest while in the next state. */
  ...
  if ((ret_val != DS_FA_UPLINK_COMPLETE) && (ret_val != DS_FA_SUCCESS) &&
      (ret_val != DS_FA_UPLINK_DATA_QUEUED))
  {
    DATA_CSD_MSG1_HIGH("process_uplink returns: %d", (word)ret_val);
  }
...
)
{
  ...
  ** correctly and passing it up to T31 like we should. */
  ...
  {
    DATA_CSD_MSG2_HIGH("DL buffer %d > %d bytes", plen,MAX_DOWNLINK_BYTES_TO_PROCESS);
  } 
  ...
  ** process the rest while in the next state. */
  ...
  {
    DATA_CSD_MSG1_LOW("process_downlink returns: %d", (word)ret_val);
  }
...
)
{
  ...
  DATA_CSD_MSG0_LOW("Requested change to IDLE state");
  ...
  if (ctrl_blk_ptr->fa_state_trans != FA_STATE_TRANS_IDLE)
  {
    if (ctrl_blk_ptr->fa_state_trans == FA_STATE_TRANS_MSG_REC)
    {
      ...
      ** state change to IDLE, as specified in GSM 03.45, 5.2.3.4. */
      DATA_CSD_MSG0_LOW("Page done, send 10 SYNCS");
    ...
    if (ctrl_blk_ptr->uplink_buf_dsm_ptr != NULL)
    {
      ...
      DATA_CSD_MSG0_HIGH("Discarding buffered uplink data!");
      dsm_free_packet(&ctrl_blk_ptr->uplink_buf_dsm_ptr);
    }
    ...
    if (ctrl_blk_ptr->downlink_buf_dsm_ptr != NULL)
    {
      ...
      DATA_CSD_MSG0_LOW("Discarding buffered downlink data!");
      dsm_free_packet(&ctrl_blk_ptr->downlink_buf_dsm_ptr);
    }
    ...
    */
    ...
    {
      DATA_CSD_MSG0_LOW("queue DCN closing sync frames");
    ...
    if (ret_val == DS_FA_SUCCESS)
    {
      DATA_CSD_MSG0_LOW("FA now in IDLE state");
    }
  }
...
)
{
  ...
  ** we store away the interesting data in our control block for use later. */
  ...
  DATA_CSD_MSG1_LOW("Parsing BCS frame of %d bytes", ui16_data_count);
  ...
  if ((ui8_data_ptr[0] == 0xFF) && (ui8_data_ptr[1] == 0x13))
  {
    ...
    {
      DATA_CSD_MSG1_LOW("HDLC FrameID=0x%x",
      ...
      if ((*(ui8_data_ptr + ui16_offset) & HDLC_FCF_MASK) == HDLC_DCS_FRAME)
      {
        ...
        DATA_CSD_MSG0_LOW("Start TRAINING phase");
        ...
        if ((ui16_offset + sizeof(ui8_hdlc_byte)) > ui16_data_count)
        {
          ...
          DATA_CSD_MSG0_LOW("No rate byte in DCS frame!");
          return fa_stat;
        }
        ...
        ** rate bits we just got.  Pop out once we find it. */
        ...
        DATA_CSD_MSG_SPRINTF_1(MSG_LEGACY_HIGH," Fax Rate: %s",
                           fax_rate_array[ui16_offset].name_str);
        DATA_CSD_MSG2_LOW( "t_nulls=%d frame_sz=%d", 
                       ctrl_blk_ptr->ui16_max_training_nulls,
                       ctrl_blk_ptr->ui16_data_per_frame);
        ...
        if (!f_uplink_frame)
        {
          ...
          ** a timer, so we don't fail it. */
          DATA_CSD_MSG1_LOW("Waiting to start %dmsec DCS->TCF timer", DCS_TO_TRAIN_DELAY);
      ...
      {
        DATA_CSD_MSG0_LOW("DCN - disconnect detected");
  ...
  switch (ctrl_blk_ptr->fa_state_trans)
  {
    case FA_STATE_TRANS_IDLE:
      ...
      ** some training. */
      ...
      {
        DATA_CSD_MSG0_LOW("DCS->TCF delay complete. To MSG_TRA state");
    ...
    case FA_STATE_TRANS_BCS_TRA:
      ...
      ** endure loss of synchronization before going IDLE. */
      DATA_CSD_MSG0_LOW("Sync loss timeout in BCS_TRA. To IDLE state");
    ...
    case FA_STATE_TRANS_MSG_TRA:
      ...
      ** DTE, so return to idle. */
      DATA_CSD_MSG0_LOW("Training complete in MSG_TRA. To IDLE state");
    ...
    default:
      ...
      DATA_CSD_MSG1_LOW("Timeout state: invalid state=%d!",
                  ctrl_blk_ptr->fa_state_trans);
      break;
  }
