...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp/csd/src/dsucsdnt.c#2 $
...
)
{
   ...
   if (Sio_rx_saved_byte_count)
   {
      ...
      {
         DATA_CSD_MSG3_LOW("No room for prior saved data %d %d %d",
...
{                       
   ...
   */ 
   if (status_byte_changed || num_chars_read)
   {
      ...
      {
         DS_3GPP_MSG0_LOW ("*pdu_ptr returned NULL ");
         RetVal = FALSE;
         return RetVal;
      }
      ...
      */
      ...
      else
      {
         if (peek_and_trim)
         {
            ...
            {
               DATA_CSD_MSG2_ERROR("dsm_peek_byte failure: offset=%d, size=%d",pdu_byte,
                          dsm_length_packet(*pdu_ptr));
            }
...
)
{
   ...
   DS3GPP_ASSERT( NULL != pWriteToBuffer );
...
)
{
   ...
   DS3GPP_ASSERT( NULL != rlp_params_ptr );
   ...
   */
   ...
   {
      DS_3GPP_ERROR_LOG_0("UCSD NT: Not all reqd callbacks registered");
   }
   ...
   DATA_CSD_MSG2_HIGH("In ds_ucsd_nt_init_call_data: PDU Type=%d, SupDupTxFrm=%d",
             ds_ucsd_get_CurrentPDUType(),
             Ds_ucsd_nt_suppress_duplicate_tx_frames);
} /* ds_ucsd_nt_init_call_data */
      ...
      */  
      ...
      default:
         ...
         if ((V42_ERROR == status) || (RLP_ERROR == status))
         {
            ...
            DATA_CSD_MSG0_LOW("NT:Fatal RLP Error");
      ...
      default:
         ...
         if( TIMER_RUNNING( TIMERID_MO_FLUSH ) )
         {
           ...
           if( TIMER_EXPIRED( TIMERID_MO_FLUSH ) )
           {
             ...
             DATA_CSD_MSG0_MED( "L2R MO terminate flush timer expired");
         ...
         else
         {
           ...
           DATA_CSD_MSG1_MED( "L2R MO terminate flush timer started: timer=%d ms",
...
)
{
   ...
   */
   if (!Ds_ucsd_nt_init_call_data_called)
   {
      ...
      */
      if (event == (DSI_Event_T)DS_UMTS_CSD_DISCONNECT_AFTER_FLUSH_EVENT)
      {
         ...
         {
            DATA_CSD_MSG0_LOW("UCSD NT: Not able to respond to DISC_AFTER_FLUSH");
         }
      }
        
      /* DATA_CSD_MSG1_ERROR if we get an event before stack is initialized. 
      ** We will get the DISCONNECT event in the 
      ** case where the call is terminated by the stack, so don't 
      ** bother printing out DATA_CSD_MSG1_ERROR for this case. 
      */
      else if ( (event != (DSI_Event_T)DS_UMTS_CSD_NULL_EVENT) &&
                (event != (DSI_Event_T)DS_UMTS_CSD_DISCONNECT_EVENT) )
      {
         DATA_CSD_MSG1_LOW( "UCSD NT: Event %d arrived before NT init - ignoring",
                    event);
      }
   ...
   {
      DS_3GPP_MSG0_LOW("UCSD NT: Cannot process both TX and RX data");
   }
