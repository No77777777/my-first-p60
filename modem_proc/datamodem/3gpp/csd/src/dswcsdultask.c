...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp/csd/src/dswcsdultask.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DATA_CSD_MSG0_LOW("RCINIT Lookup for dswcsd_ul tcb returned NULL");
  }
#else
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  while( (cmd_ptr = (dswcsd_cmd_type *)q_get( &dswcsd_ul_cmd_q )) != 
         NULL )
  {
    DATA_CSD_MSG1_LOW("Rx'ed CS Data UL Task command %d", cmd_ptr->hdr.cmd_id);
    ...
    switch( cmd_ptr->hdr.cmd_id )
    {
      ...
      case DSWCSD_CONNECT_CALL_CMD:
           ...
           DATA_CSD_MSG3_MED("dswcsd_from_sio_q_cnt %d, dswcsd_dsm_payload_size %d, sdus_per_tti*bytes_per_sdu %d", 
                          sio_q_cnt, 
      ...
      case DSWCSD_V80_CONNECT_CALL_CMD:
           ...
           DATA_CSD_MSG1_LOW("CS data call using V.80 SAM; V.80 SYN1 using %d",
      ...
      default:
           DATA_CSD_MSG1_LOW("Invalid CS Data UL Task command: %d", 
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  for( ;; )
  {
    ...
    DATA_CSD_MSG2_LOW("req_sigs = 0x%x set_sigs = 0x%x", requested_sigs, set_sigs);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DATA_CSD_MSG1_LOW("No free memory blocks available. Cmd buffer of size : %d",
