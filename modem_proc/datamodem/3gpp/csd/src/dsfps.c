...
/*=============================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp/csd/src/dsfps.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
)
{
  ...
  /* Safety net */
  DS3GPP_ASSERT(dsat_callback != NULL);
  ...
  if ((cmd_ptr = ds_gcsd_get_cmd_buf()) != NULL)
  {
    ...
    DATA_CSD_MSG1_MED("Sending command %d to the GCSD-FPS", cmd);
  ...
  {
    DS_3GPP_MSG1_LOW("ds_gcsd_get_cmd_buf() failure, cmd=%d", (int)cmd);
  }
} /* dsfps_at_exec_action_cmd */
