...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp/csd/src/dsucsdulpdu.c#3 $ $DateTime: 2020/11/16 21:05:49 $ $Author: pwbldsvc $
...
void ds_ucsd_UplinkPDUMgr_AdvanceVAtoNR (short _Nr)
{
   ...
   {
      DATA_CSD_MSG2_LOW("ULPDUMgr: AdvanceVAtoNR, illegal N(r)=%d or VA=%d",
                              _Nr,CurrentList->VA);
...
short ds_ucsd_UplinkPDUMgr_SREJDataToTransmit (void)
{
   ...
   if (CurrentList->VA != CurrentList->VS)
   {
      ...
      if ( (CurrentList->VS > get_current_rlp_max_frame_number()) ||
           (CurrentList->VA > get_current_rlp_max_frame_number()) )
      {
         DATA_CSD_MSG2_LOW("ULPDUMgr: SREJDataToTransmit, illegal VS=%d or VA=%d",
...
)
{
   ...
   if (List != NULL)
   {
      ...
      DATA_CSD_MSG1_LOW("Initializing Uplink List: %d",List->Type);
