...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp/csd/src/dsucsdv42c.c#3 $ $DateTime: 2020/11/16 21:05:49 $ $Author: pwbldsvc $
...
void       compress(void)
{
   ...
   if( !V42_get_terminate_flag() )
   {
     if (0 ==
         ds_ucsd_V24_interface_handler_byte_oriented(&InputTxBuffer,
                                                     MAX_TX_BUF_BYTES) )
     {
       DATA_CSD_MSG0_LOW("V42 compress input buffer empty");
     }
   }
   ...
   while (1)  /*lint !e716 */
   {
      ...
      {
         ds_v42_c_buffer_write_error = FALSE;
         DATA_CSD_MSG0_LOW("v42encode: buffer write error");
         return;
      }
      ...
      if ( 1 != dsm_pullup(&InputTxBuffer, &RxWord, 1) )
      {
         ...
         DATA_CSD_MSG0_LOW("V42 compress exiting - input buffer empty");
         return;
      }
