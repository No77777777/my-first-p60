...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp/csd/src/dsucsdv42d.c#3 $ $DateTime: 2020/11/16 21:05:49 $ $Author: pwbldsvc $
...
void            init_decoder(void)
{
   ...
   {
     DS_3GPP_MSG2_LOW( "Heap Memory Allocation failed for client: %d size: %d" ,
...
void           v42decode()
{
   ...
   while (1)   /*lint !e716 */
   {
      ...
      if (new_code > ds_v42_d_max_code)
      {
         ...
         DATA_CSD_MSG2_LOW("D_ERROR_IND (code:%x max:%x)",new_code,ds_v42_d_max_code);
      ...
      if (ds_v42_d_mode == COMPRESSED_MODE)
      {
         ...
         if (new_code < 3)
         {
            ...
            if (new_code == STEPUP)
            {
               ...
               {
                  DATA_CSD_MSG0_LOW("decode: stepup error - codeword size exceeded!. D_ERROR_IND 2");
      ...
      if (ds_v42_d_mode == COMPRESSED_MODE)
      {
         ...
         if (code >= FIRST_CODE)
         {
            ...
            {    
               DATA_CSD_MSG1_LOW("V42error:code 0x%X not found in dictionary", code);
