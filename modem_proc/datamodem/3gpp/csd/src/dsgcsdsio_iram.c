...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp/csd/src/dsgcsdsio_iram.c#3 $ $DateTime: 2020/11/16 21:05:49 $ $Author: pwbldsvc $
...
)
{
   ...
   */
   ...
   if (output_data_valid)
   {
      ...
      DS3GPP_ASSERT (Curr_tx_dsm_item != NULL);
      ...
      if (!Suspend_data_to_sio) 
      {
         ...
         while (Curr_tx_dsm_item != NULL)
         {
           ...
           if (ds3g_siolib_ex_setup_tx (tmp_dsm_ptr, 
            ...
            {
               DATA_CSD_MSG1_ERROR("GCSD: Unable to transmit to SIO port %d", 
                               (int)ds3g_siolib_ex_get_csd_port());
            }
         }
      }         
   }
