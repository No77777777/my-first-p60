...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp/csd/src/dsgcsdl1if.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
)
{
  ...
  if (error)
  {
     DATA_CSD_MSG1_LOW("Error status on L1 RX callback=%d",status);
  }
  ...
  if (BUF_IS_FULL (Gcsd_l1_rx_buf_mgr))
  {
    *next_buf = &Gcsd_l1_default_rx_buf;
    if (!prev_overrun)
    {
      DATA_CSD_MSG0_LOW("GCSD L1 RX overrun");
    }
    prev_overrun++;
  }
  else
  {
    ...
    *next_buf = &Gcsd_l1_rx_buf[Gcsd_l1_rx_buf_mgr.write_index];
    ...
    if (prev_overrun)
    {
      DATA_CSD_MSG1_LOW("GCSD L1 RX overrun cleared %d",prev_overrun);
      prev_overrun = 0;
    }
...
{   
   ...
   if (BUF_IS_EMPTY (Gcsd_l1_tx_buf_mgr))
   {
      *curr_buf = Gcsd_l1_default_tx_buf;
      if (!prev_underrun)
      {
         DATA_CSD_MSG0_LOW("GCSD L1 TX underrun");
      }
      prev_underrun++;
   }
   else
   {
      ...
      *curr_buf = Gcsd_l1_tx_buf[Gcsd_l1_tx_buf_mgr.read_index];
      ...
      if (prev_underrun)
      {
         DATA_CSD_MSG1_LOW("GCSD L1 TX underrun cleared=%d",prev_underrun);
         prev_underrun = 0;
      }
   }
...
)
{
   ...
   {
      DATA_CSD_MSG0_LOW("ds_gcsd_write_to_channel_coder: Tx Overrun!");
      return FALSE;
   }
...
)
{
   ...
   {
      DATA_CSD_MSG0_LOW("Buffer pointer mismatch");
      return FALSE;
   }
   
   if (BUF_IS_FULL (Gcsd_l1_tx_buf_mgr))
   {
      DATA_CSD_MSG0_LOW("GCSD TX buffer full");
      return FALSE;
   }
