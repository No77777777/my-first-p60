...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp/csd/src/dsgcsdra1.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
   ...
   *rx_error = tmp_rx_error;
   ...
   if (e_receive_sync_pattern)
   {
      ...
      switch (Gcsd_curr_data_rate)
      {
         ...
         default:
            DS_3GPP_MSG1_LOW ("GCSD RA1'RX: Unsupported user rate = %d",(int)Gcsd_curr_data_rate);
            break;
      }
   ...
   else
   {
      ...
      {         
         ...
         default:
            DS_3GPP_MSG1_LOW ("GCSD RA1'RX: Unsupported user rate = %d",
...
)
{
   ...
   if (e_send_sync_pattern)
   {
      ...
      switch (Gcsd_curr_data_rate)
      {
         ...
         default:
            ...
            DS_3GPP_ERROR_LOG_1 ("GCSD RA1'TX: Unsupported user rate = d",(int)Gcsd_curr_data_rate);
            break;
      }
   } 
   ...
   else /* Don't send sync pattern, send user data */
   {
      switch (Gcsd_curr_data_rate)
      {
         ...
         default:
            DS_3GPP_MSG1_LOW ("GCSD RA1'TX: Unsupported user rate = %d",(int)Gcsd_curr_data_rate);
            break;
      }
   }
