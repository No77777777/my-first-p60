...
DESCRIPTION
  ...
  Copyright (c) 2003 - 2017 by Qualcomm Technologies, Incorporated.
  All Rights Reserved.
...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp/csd/src/dsucsdhdlr.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
#define SET_API_INTERFACES(info_ptr)               \
    ...
    DATA_CSD_MSG0_MED("UCSD Routing: call_ctrl=API, datapath=SIO");
#else
/* Assign API datapath routing.*/
#define SET_API_INTERFACES(info_ptr)               \
    info_ptr->call_ctrl_iface = DS_UCSD_IFACE_API; \
    info_ptr->datapath_iface  = DS_UCSD_IFACE_API; \
    DATA_CSD_MSG0_MED("UCSD Routing: call_ctrl=API, datapath=API");
...
)
{
   ...
   DATA_CSD_MSG3_HIGH("CBST validate: SPEED=%d,NAME=%d,CE=%d",speed,name,ce);
   for( i = 0; i < (uint32)AT_speed_table_size; i++ )
   {
      if( (uint32)AT_speed_table[i].speed == speed )
      {
       ...
       {
         DATA_CSD_MSG1_LOW("+CBST: speed %d won't support RDI ITC",speed);
         rc = FALSE;
       }
            ...
            (name == ATCOP_ASYNC_RDI_DATA) )
       {
         ...
         {
           DATA_CSD_MSG1_LOW("+CBST: speed %d won't support ASYNC data",speed);
           rc = FALSE;
         }
       }
                 ...
                 (name == ATCOP_SYNC_RDI_DATA) )
       {
         ...
         {
           DATA_CSD_MSG1_LOW("+CBST: speed %d won't support SYNC data",speed);
           rc = FALSE;
         }
       }
       else
       {
         DATA_CSD_MSG1_LOW("+CBST: unknown name (sync/async) setting",name);
         rc = FALSE;
       }
       ...
       */
       if (ce == ATCOP_NON_TRANSPARENT_DATA)
       {
         ...
         {
           DATA_CSD_MSG1_LOW("+CBST: speed %d won't support NT data",speed);
           rc = FALSE;
         }
       }
       else if (ce == ATCOP_TRANSPARENT_DATA)
       {
         ...
         {
          DATA_CSD_MSG1_LOW("+CBST: speed %d won't support T data",speed);
           rc = FALSE;
         }
       }
       else
       {
         DATA_CSD_MSG1_LOW("+CBST: unknown CE (T/NT) setting",ce);
         rc = FALSE;
       }
   ...
   {
      DATA_CSD_MSG1_LOW("+CBST: speed value %d not supported",speed);
      rc = FALSE;
   }
   ...
   {
     DATA_CSD_MSG2_LOW("+CBST: name %d and ce %d is not supported",name ,ce);
     rc = FALSE;
   }
...
if( NULL == data_bc_ptr ||
  ...
  *speed_ptr = 0;
  ...
  DATA_CSD_MSG3_HIGH ("CBST decode: SPEED=%d,NAME=%d,CE=%d",*speed_ptr,*name_ptr,*ce_ptr);
...
)
{
  ...
  uint8 mode;
  DATA_CSD_MSG2_HIGH("VT call on subs id %d, network mode %d internal mode",
                  (int)ds_ucsd_subs_id,
                  (int)ds_ucsd_network_mode);
#ifdef FEATURE_DUAL_SIM
  ...
  {
#ifdef FEATURE_3GPP_CSFB
      DATA_CSD_MSG1_LOW("CSFB:setting mode to internal mode %d",
...
/*-------------------------------------------------------------------------*/
  ...
  if ( index < MAX_UCSD_CALL_TBL_ENTRIES ) 
  {
    ...
    else
    {
      ...
      DATA_CSD_MSG1_LOW("Allocated CS Data call instance ID = %d", 
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DATA_CSD_MSG1_LOW("Invalid instance ID: %d",inst_id);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DATA_CSD_MSG1_LOW("Could not find call instance in table: %d",
               inst_id);
    result = FALSE;
  }
  else
...
)
{
  ...
  {
    DATA_CSD_MSG0_LOW("Invalid Table Index");
    return;
  }
...
/*-------------------------------------------------------------------------*/
   ...
   DATA_CSD_MSG1_LOW(".Default OTA rate to %d",ota_rate);
...
/*-------------------------------------------------------------------------*/
  ...
  DATA_CSD_MSG1_LOW("NV_DATA_INCOMING_CSD_CALL_IS_INTERNAL_I is %d",
           incoming_internal_csd_call_enabled);
...
/*-------------------------------------------------------------------------*/
  ...
  DATA_CSD_MSG5_MED("RLP_VERSION_GSM: %d RLP_VERSION_WCDMA: %d RECOVERY_MODE_SREJ %d"
                 " RLP_ADM_IDLE_TIMER: %d, NT_CONNECT_STATE_I: %d",
                 rlp_params->rlp_ver_gsm,
                 rlp_params->rlp_ver_wcdma,
...
)
{
  ...
  DATA_CSD_MSG1_LOW("WAIUR value from ATCOP=%d",chsn_val);
  return result;
...
)
{
   ...
   if( NULL == service_ptr || NULL == chosen_Capability )
   {
     DS3GPP_ASSERT(0);
     return FALSE;
   }
   ...
   if ( valuesOK == FALSE ) 
   {
      DATA_CSD_MSG1_LOW( "Invalid Speed value: %d", atCmdVal);
   }
   ...
   if (valuesOK)
   {
      ...
      for( i = 0; i < AT_name_table_size; i++ )
      {
         if( AT_name_table[i].name == atCmdVal )
         {
            ...
            {
              DATA_CSD_MSG1_LOW("Cannot map ITC: %d",
                         tempChosen_capability.chosen_bc.itc);
            }
            break;
         }
      ...
      if ( valuesOK == FALSE ) 
      {
         DATA_CSD_MSG1_LOW("Invalid Name value: %d", atCmdVal);
      }
   }
   ...
   if (valuesOK)
   {
      ...
      if ( valuesOK == FALSE ) 
      {
         DATA_CSD_MSG1_LOW("Invalid CE value: %d", atCmdVal);
      }
   }
   ...
   if (valuesOK)
   {
      ...
      */
      if ( DS_UCSD_IS_WCDMA_TDS_MODE(ds_ucsd_network_mode) )
      {
         ...
         {
            valuesOK = FALSE;
            DATA_CSD_MSG1_LOW("Speed setting %d not valid in WCDMA mode",
      ...
      ** speed setting
      ...
      else if (ds_ucsd_network_mode == SYS_SYS_MODE_GSM)
      {
        ...
        {       
          ...
          {
            valuesOK = FALSE;
            DATA_CSD_MSG1_LOW("Speed setting %d not valid in GSM mode",
                       service_ptr->cbst_info.speed);
          }
        }
      }
      ...
      */
      ...
      {
        valuesOK = FALSE;
        DATA_CSD_MSG1_LOW("Mode %d invalid for UMTS CSD call",
                   ds_ucsd_network_mode);
      }
   }
   ...
   */
   if (valuesOK)
   {
          ...
          tempChosen_capability.chosen_bc.connection_element )
     {
       ...
       DATA_CSD_MSG3_HIGH("CS ASYNC +CHSN=%d, FNUR=%d, WAIUR=%d",
...
{
  if( NULL == call_info_ptr || NULL == gw_cs_orig_params )
  {
    DS3GPP_ASSERT(0);
    return;
  }
...
)
{
  ...
  if ( call_info_ptr->uus_info.present == TRUE )
  {
    ...
    {
      DATA_CSD_MSG1_LOW("Invalid H.323 addr id = %d",call_info_ptr->uus_info.uus_id);
      return;
    }
...
{
  DATA_CSD_MSG0_LOW("In ds_ucsd_configure_l2_watermarks");
...
{
   DATA_CSD_MSG1_LOW("UCSD signal handler incorrectly called, sigs=%lx",set_sigs);
}
...
/*-------------------------------------------------------------------------*/
   ...
   DATA_CSD_MSG2_MED("In ds_ucsd_connect_l2_interface: RABId %d; isHO %d",
           rab_id, is_handover);
...
#endif /* FEATURE_SEGMENT_LOADING */
   {
...
#endif /* FEATURE_SEGMENT_LOADING */
     {
       DATA_CSD_MSG1_LOW("Invalid RAB id %d", rab_id);
       return ( DS3G_FAILURE );
     }
     if ( ( tds_lc_info.num_ul_lc_ids_for_rab > 1 ) ||
          ( tds_lc_info.num_dl_lc_ids_for_rab > 1 ) )
     {
       DATA_CSD_MSG3_LOW("More than 1 LC id: RABid %d; UL %d; DL %d",
                  rab_id,
    ...
    if ( tdsrrc_return_cs_data_call_parms( tds_lc_info.rab_id, 
...
#endif /* FEATURE_SEGMENT_LOADING */
     {
       DATA_CSD_MSG1_LOW("Failed to get RAB info for RAB ID %d",
                  tds_lc_info.rab_id);
     ...
     {
       DATA_CSD_MSG1_ERROR("Invalid SDU size: %d bits", tds_cs_data_parms.sdu_size);
   ...
   {
     DATA_CSD_MSG0_LOW("In ds_ucsd_connect_l2_interface WCDMA not active");
     return ( DS3G_FAILURE );
   }
#endif /* FEATURE_SEGMENT_LOADING */
  ...
  if ( rrc_return_cs_data_call_parms_sub(subs_id, lc_info.rab_id, 
     ...
     {
       DATA_CSD_MSG1_ERROR( "Invalid SDU size: %d bits", cs_data_parms.sdu_size);
   ...
   if ( SYS_SYS_MODE_TDS ==  ds_ucsd_network_mode)
   {
     ...
     {
       DATA_CSD_MSG0_LOW("UCSD mode is TDS, but RRC_is_tds_active returned FALSE");
       return FALSE;
     }
   }
   else
   ...
   case 2:
     ...
     else
     {
       ...
       {
         DATA_CSD_MSG2_LOW("No. of bytes_per_sdu:%d for this call info is invalid for SDUS_PER_TTI=%d", 
   ...
   default:
     DATA_CSD_MSG1_LOW("Bad WCDMA Async SDUS_PER_TTI=%d",sdus_per_tti);
     return ( DS3G_FAILURE );
   }
   ...
   DATA_CSD_MSG3_HIGH( "Channel config: %d bytes per SDU, %d SDU per TTI, data rate %d",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
           ...
           type++ )
      {
        ...
        if( 0 != (ds_ucsd_call_tbl_ptr[ index ]->call_type & MAKE_CALL_TYPE( type ) ) )
        {
              ...
              ( ds_ucsd_call_limit_info[ type ] <= call_type_count[ type ] ) )
          {
            ...
            DATA_CSD_MSG3_LOW("Concurrent limits exceeded: call ID %d inst ID %d type %d",
  ...
  if( TRUE == result )
  {
    ...
    {
      DATA_CSD_MSG1_LOW("Total no of supported call instances exceeded calllimit[%d]",
                      call_limit);
      result = FALSE;
    }
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (NULL == bc_ptr || NULL == mode_info_ptr || 
      NULL == call_info_pptr )
  {
     DS3GPP_ASSERT(0);
     return FALSE;
                                              ...
                                              &switch_ptr ) )
  {
    ...
    if( NULL != switch_ptr )
    {
      ...
      DATA_CSD_MSG3_HIGH( "Switching incoming call ID %d to prev call ID: %d inst ID: %d",
      ...
      {
        DATA_CSD_MSG1_LOW( "Failed to end the call on switch: %d", prev_call_id);
      }
       ...
       call_instance++)
  {
        ...
        (ds_ucsd_call_tbl_ptr[call_instance]->cm_call_id != CM_CALL_ID_INVALID) )
    {
...
#endif /* FEATURE_DUAL_SIM */
      {
        ...
        {
          DATA_CSD_MSG1_LOW("No CSD Support in GSM, Cleanup Call_id:%d",
         ...
         {
            DATA_CSD_MSG2_LOW("Failed to end the call: "
        ...
        DATA_CSD_MSG3_HIGH("Changing mode for call id:%d :old mode:%d to new mode %d",
                   ds_ucsd_call_tbl_ptr[call_instance]->cm_call_id,
                   new_mode,old_mode);
        ...
        if( !ds3g_msh_set_new_call_mode(ds_ucsd_call_tbl_ptr[call_instance]->cm_call_id,
                                        new_mode) )
        {
          DATA_CSD_MSG1_LOW("Mode set failed for inst :%d", call_instance);
        }
      }
    }
  }
...
/* Query GSM L1 for channel characteristics */
  ...
  default:
    DATA_CSD_MSG1_LOW("Unsupported GSM L1 mode: %d", chmode);
    result = FALSE;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DATA_CSD_MSG2_LOW("NAS(MN)->UCSD notification: reason=%d RAB=%d",
           reason, rab_id);
...
/*-------------------------------------------------------------------------*/
  ...
  DATA_CSD_MSG2_LOW("Handover update stack interface: reason=%d, result %d",
           data_ptr->reason, result);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  else
  {
    ...
    DATA_CSD_MSG1_LOW("Process handover cmd: reason=%d",reason);
        ...
        (call_info_ptr->state == DS_UCSD_CALL_STATE_CONNECT_LL) )
    {
      ...
      if( NULL == data_bc_ptr )
      {
        DATA_CSD_MSG1_LOW("Cannot determine data bearer configuration: inst %d",
...
/*-------------------------------------------------------------------------*/
       ...
       ds_ucsd_find_call_instance( data_ptr->inst_id )) )
  {
    ...
    */
    DATA_CSD_MSG1_LOW( "Invalid CS Data call instance ID %d in APP_ORIG_CALL cmd."
                     " End the call",
                     data_ptr->inst_id);
  ...
  else
  {
    ...
    if( ds_ucsd_set_call_mode_and_subs_id(
                ...
                &ds_ucsd_subs_id) )
    {
      DATA_CSD_MSG2_HIGH("VT call on subs id %d mode %d", 
            (int)ds_ucsd_subs_id,
            (int) ds_ucsd_network_mode);
      /* set the current subs_id in ds3g for CS call origination */
      ds3g_set_cs_subs_id(ds_ucsd_subs_id);
    ...
    {
      DATA_CSD_MSG1_LOW("Invalid subs id = %d", ds_ucsd_subs_id);
      end_call = TRUE;
    }
#else
    ...
    if( INVALID_3GPP_CS_SYS_MODE(ds_ucsd_network_mode) )
    {
      ...
      ds_ucsd_network_mode = SYS_SYS_MODE_WCDMA;
#endif /* FEATURE_SEGMENT_LOADING */
      DATA_CSD_MSG1_LOW("CSFB: setting mode to %d",(int)ds_ucsd_network_mode);
#endif /* FEATURE_3GPP_CSFB*/
    }

    /* Determine composite call type based on call mode */
    call_info_ptr->call_mode = data_ptr->call_mode;
    DATA_CSD_MSG1_HIGH("Initiating CS data call in  n/w mode %d",
                   ds_ucsd_network_mode );
    ...
    */
    ...
    if ( DS3G_FAILURE ==
         ds3g_msh_initiate_call( DS_CALL_TYPE_CKT,
                                 ...
                                 call_info_ptr ) )
    {
      DATA_CSD_MSG1_LOW("Failed to initiate the call: CS Data call instance ID = %d", 
                data_ptr->inst_id);
  ...
  if ( end_call == TRUE ) 
  {
    ...
    (void) ds_ucsd_app_notify_client(
...
{ 
  ...
  if( MAX_UCSD_CALL_TBL_ENTRIES > index )
  {
    ...
    if( TRUE == data_ptr->result_params.reject)
    {
      ...
      if( TRUE == cause_ptr->cc_cause_present )
      {
        ...
        DATA_CSD_MSG1_LOW("call rejected using cause  %d",
                         setup_params.cc_cause.cause_value);
      }
    }
    else
    {
                                               ...
                                               &setup_params ) )
    {
      ...
      DATA_CSD_MSG1_LOW( "Failed to confirm call setup: CM call ID = %d", 
                 call_info_ptr->cm_call_id);
    }
  ...
  {
    DATA_CSD_MSG1_LOW( "Invalid CM call instance ID %d in APP_CONFIRM_CALL cmd. End the call",
                     data_ptr->inst_id);
    end_call = TRUE;
  }
  ...
  if( end_call )
  {
    ...
    /* Notify API client of event */
    (void)ds_ucsd_app_notify_client(data_ptr->client_id, &event);
...
{ 
      ...
      ds_ucsd_find_call_instance( data_ptr->inst_id )) )
  {
    ...
    {
      DATA_CSD_MSG0_LOW("Attempt to answer call not in ALERTING state");
    ...
    else
    {
      ...
      {
        /* If CM fails to answer the call, DSMGR will end the call */
        DATA_CSD_MSG2_LOW( "Failed to answer the call: CM call ID = %d; "
  ...
  if( end_call )
  {
    ...
    DATA_CSD_MSG1_LOW("Invalid CS Data call instance ID %d in APP_ANSWER_CALL cmd. "
               "End the call", 
               data_ptr->inst_id );
...
/*-------------------------------------------------------------------------*/
      ...
      ds_ucsd_find_call_instance( data_ptr->inst_id )) )
  {
    ...
    if ( ds3g_msh_hangup_call_ex( call_info_ptr->cm_call_id, NULL,
      ...
      {
        DATA_CSD_MSG2_LOW( "Failed to end the call: CM call ID = %d; CS Data call instance ID = %d", 
                   call_info_ptr->cm_call_id, 
                   call_info_ptr->inst_id );
  ...
  {
    DATA_CSD_MSG1_LOW( "Invalid CS Data call instance ID %d in APP_END_CALL cmd. End the call",
                     data_ptr->inst_id);
  ...
  if ( end_call == TRUE ) 
  {
    ...
    {
      DATA_CSD_MSG0_LOW("No client notified for call end");
    }
  }
...
{ 
  ...
  {
    DATA_CSD_MSG1_LOW("Cannot find call instance for CM call ID: %d",
               call_info_cmd_ptr->call_id );
    return;
  }
  ...
  {
    DATA_CSD_MSG0_LOW("Received UUS1 data");
  }
  /*-------------------------------------------------------------------------
...
{ 
  ...
  if( call_info_ptr->uus1_data.mt_user_data.present == TRUE )
  {
    DATA_CSD_MSG0_LOW("UUS1 data congestion ind has data");
  }

  DATA_CSD_MSG0_LOW("Should end CSD call");
#endif /* FEATURE_UUS */
}
...
{
  if( NULL == call_info_ptr || NULL == event_ptr )
  {
    DS3GPP_ASSERT(0);
    return;
  }
  ...
  if( DSUCSD_INVALID_CLIENT_ID != call_info_ptr->client_id )
  {
    ...
    {
      DATA_CSD_MSG2_LOW("No client notified for call event: clientID=%d CM event=%d",
  ...
  else
  {
    ...
    if( FALSE ==
        ds_ucsd_app_notify_clients_by_type( (uint8)call_info_ptr->call_type,
                                            event_ptr ) )
    {
      DATA_CSD_MSG1_LOW("No client notified for call event: CM event=%d",
...
/*-------------------------------------------------------------------------*/
  ...
  DATA_CSD_MSG1_MED("ds_ucsd_call_end_common_cleanup: inst ID=%d", inst_id);

  /* Find call instance in the call table */
  if( MAX_UCSD_CALL_TBL_ENTRIES <=
      (index = ds_ucsd_find_call_instance( inst_id )) )
  {
    DATA_CSD_MSG1_LOW("Cannot find call instance for inst ID: %d", inst_id);
    return FALSE;
  }
  call_info_ptr = DSUCSD_GET_CALL_INFO( index );
        ...
        if( DS_UCSD_IFACE_SIO == call_info_ptr->call_ctrl_iface )
        {
          if( FALSE ==
              ds3g_siolib_ex_deregister_callback_func (
                                  ...
                                  call_info_ptr->sio_port_id) )
          {
            DATA_CSD_MSG0_LOW("Error in SIOLIB deregister");
          }
        }
        port_index = ds3g_siolib_get_index_from_allocated_port(
                                           call_info_ptr->sio_port_id);
  ...
  {
    DATA_CSD_MSG0_LOW("Specified instance ID not in use");
  }
...
)
{
  ...
  DATA_CSD_MSG1_LOW("Got %d",cmd_ptr->hdr.cmd_id);
  ...
  case DS_CMD_3GPP_GCSD_DISCONNECT:
    if (cmd_ptr->cmd_payload_ptr != NULL)
    {
      ...
      if ( ds3g_msh_hangup_call_ex( ucsd_call_info_ptr->cm_call_id, NULL,
        ...
        {
          DATA_CSD_MSG2_LOW("Failed to end the call: CM call ID = %d"
                          "CS Data call instance ID = %d", 
  ...
  case DS_CMD_3GPP_GCSD_RX_DCD_ON:
    if (cmd_ptr->cmd_payload_ptr != NULL)
    {
      ...
      {
        DATA_CSD_MSG1_LOW("Aborting connect, CM Call ID invalid!: %d",
                        call_id);
      }
    }
    break;  
  ...
  default:
    DATA_CSD_MSG1_LOW("Unexpected command to UCSD handlers = %d",cmd_ptr->hdr.cmd_id);
    break;
  }
} /* ds_ucsd_process_cmds() */
...
ds_ucsd_call_hangup_cb (void)
{
  ...
  if( MAX_UCSD_CALL_TBL_ENTRIES != index )
  {
    ...
    DATA_CSD_MSG2_LOW("HANGUP cb called, ending call: CM call ID %d  inst ID %d",
  ...
  {
    DATA_CSD_MSG0_LOW("HANGUP cb cannot find external call instance");
  }
...
ds_ucsd_call_answer_cb (void)
{
  ...
  if( MAX_UCSD_CALL_TBL_ENTRIES == index )
  {
    ...
    DATA_CSD_MSG0_LOW( "Call instance not in ALERTING state, rejecting answer");
  ...
  else
  {
    ...
    DATA_CSD_MSG0_LOW("ANSWER cb called");
    call_info_ptr->state = DS_UCSD_CALL_STATE_ANSWERED;
    ...
    {
      DATA_CSD_MSG2_LOW("Failed to answer the call: CM call ID = %d; CS Data call instance ID = %d", 
                 call_info_ptr->cm_call_id, 
                 call_info_ptr->inst_id);
...
/*-------------------------------------------------------------------------*/
   ...
   if( NULL == data_bc_ptr || NULL == call_info_ptr )
   {
     DS3GPP_ASSERT(0);
     return DS3G_FAILURE;
   }  

   DATA_CSD_MSG1_HIGH("Connecting CSD data path: RABid=%d", rab_id );
   ...
   if ( DS_UCSD_IS_WCDMA_TDS_MODE(ds_ucsd_network_mode) )
   {
     ...
     {
       DATA_CSD_MSG0_LOW("Problem registering with RLC, connection aborted");
       return rlc_result;
     }
     ...
     if( DS_UCSD_IFACE_SIO == call_info_ptr->datapath_iface ) 
     {
       ...
       if ( data_bc_ptr->sync_async == SYNCHRONOUS )
       {
         ...
         if ( ( wcsd_cmd_ptr = dswcsd_ul_get_cmd_buf() ) != NULL ) 
         {
           ...
           #ifdef FEATURE_DUAL_SIM
             wcsd_cmd_ptr->hdr.subs_id = call_info_ptr->subs_id;
           #else
             wcsd_cmd_ptr->hdr.subs_id = ds3g_get_cs_subs_id();
           #endif /* FEATURE_DUAL_SIM */
         ...
         {
           DS_3GPP_MSG0_LOW( "Cannot get WCSD UL cmd buffer");
         }
         ...
         {
           DS_3GPP_MSG0_LOW( "Cannot get WCSD DL cmd buffer");
         }
   ...
   if ( ds_ucsd_network_mode == SYS_SYS_MODE_GSM )
   {
     ...
     if( TRUE !=
         ds_ucsd_get_GSM_channel_info(call_info_ptr) ) 
     {
       DATA_CSD_MSG0_LOW("Failed to get valid channel characteristics: Releasing the call");
       return ( DS3G_FAILURE );
     }
   }
          ...
          ( data_bc_ptr->sync_async == ASYNCHRONOUS ) ) )
   {
     ...
     ------------------------------------------------------------------*/
     SET_CALL_DESCRIPTOR( (&call_desc), call_info_ptr, data_bc_ptr );
     (void)ds_ucsd_mpshim_connect_data_path( &call_desc,
                                           &call_info_ptr->chan_info );
...
)
{
    ...
    if( DS_UCSD_IFACE_SIO == call_info_ptr->call_ctrl_iface )
    {
      if (FALSE ==
          ds3g_siolib_ex_register_callback_func (&ds_ucsd_siolib_func_tbl,
                                                 call_info_ptr->sio_port_id))
      {
        DATA_CSD_MSG0_LOW("Unable to reg SIOLIB funcs");
        result = FALSE;
      }
    }
  }
  ...
  {
    DATA_CSD_MSG0_LOW("Unable to allocate SIOLIB port");
    result = FALSE;
  }
...
)
{
  ...
  DATA_CSD_MSG3_MED("Registering command handlers with ATCOP: inst=%d dir=%d reg_rc=%d",
  ...
  case  DSAT_ERROR:
    DATA_CSD_MSG0_LOW("ATCOP rejected registration: +CGAUTO=2");
    break;
  case  DSAT_BUSY:
    DATA_CSD_MSG0_LOW("ATCOP rejected registration: MT PDP registered");
    break;

  default:
    DATA_CSD_MSG0_LOW("Unsupported ATCOP return code");
    break;
  }
...
)
{
  ...
  if( is_call_connecting )
  {
    ...
    {
      DATA_CSD_MSG1_LOW("Getting SIO_IOCTL_SET_PACKET_SIZE = %d",
    ...
    {
      DATA_CSD_MSG1_LOW("Error getting SIO_IOCTL_SET_PACKET_SIZE: %d",
                 sio_ioctl_param.packet_size );
    }
  ...
  DATA_CSD_MSG1_LOW("Setting SIO_IOCTL_SET_PACKET_SIZE = %d",
           sio_ioctl_param.packet_size);
...
)
{
   ...
   if ( call_info_ptr != NULL )
   {
      DATA_CSD_MSG3_HIGH( "Call ID handler: CM call ID=%d; CS inst ID=%d type=%d", 
                call_id, 
              call_info_ptr->inst_id, 
              call_info_ptr->call_type );
    ...
    {
      DATA_CSD_MSG1_LOW("Unable to set call inst for cm_call_id : %d",
                 call_id);
      return;
    }
   }
   else
   {
    ...
    DATA_CSD_MSG2_LOW("Invalid call table entry [0x%p]. End the call: CM call ID = %d", 
      ...
      {
        DATA_CSD_MSG0_LOW("Failed to end the call" );
      }
   }
} /* ds_ucsd_call_id_handler() */
...
)
{
  ...
  */
      ...
      (index = ds_ucsd_find_call_instance_by_port( DS3G_SIOLIB_PORT_MAX )) )
  {
    ...
    {
      DATA_CSD_MSG0_LOW("Could not find external call port "
  ...
  if( MAX_UCSD_CALL_TBL_ENTRIES != index )  
  {
    ...
    DATA_CSD_MSG2_LOW("DTR changed signal handler: status=%d index=%d",
             dtr_status, index);
   ...
   */
   ...
   {
      DATA_CSD_MSG0_LOW("Ignoring DTR drop-FAX not connected");
      return;
   }
...
)
{
  ...
  DATA_CSD_MSG2_LOW("Call confirm handler: CM call ID=%d type=%d",
            call_id,
            curr_call_type);
  ...
  {
    DATA_CSD_MSG1_LOW("Cannot find call instance for CM call ID: %d",call_id );
...
#error code not present
#endif /* FEATURE_MULTIMEDIA_FALLBACK */
  {
    ...
    DATA_CSD_MSG0_LOW("Conf Data BC structure:");
    ...
    if ( bc_ok ) 
    {
       ...
       if(! IS_VOICE_CALL( curr_call_type ) ) /*lint !e655 */
       {
         ...
         {
           DATA_CSD_MSG0_LOW("CS Data call confirmed");
         }
         else
         {
#ifdef FEATURE_MULTIMEDIA_FALLBACK
           #error code not present
#else
           DATA_CSD_MSG0_LOW( "Missing CS Data call instance");
           return (DS3G_FAILURE);
#endif /* FEATURE_MULTIMEDIA_FALLBACK */
         }
     ...
     {
        DATA_CSD_MSG0_LOW("CS Data bearer validation failed");
     }
   }
...
)
{
   ...
   DATA_CSD_MSG3_HIGH( "Call progress handler: CM call ID=%d type=%d progress=%d",
...
/*-------------------------------------------------------------------------*/
  ...
  call_info_ptr = DSUCSD_GET_CALL_INFO( index );
   ...
   DATA_CSD_MSG2_HIGH( "Call connected handler: CM call ID=%d type=%d",
             call_id,
             curr_call_type);
...
#endif /* FEATURE_MULTIMEDIA_FALLBACK */
   {
     ...
     */
     ...
     if( TRUE != ds_ucsd_connect_data_path( mode_info_ptr->info.gw_cs_call.rab_id,
                                            ...
                                            call_info_ptr ) )
     {
       DATA_CSD_MSG0_LOW("Problem connecting external interface data path" );
       return DS3G_FAILURE;
     }
     else
     {
       ...
       DATA_CSD_MSG1_HIGH("CS Data call connected, delay=%d", *delay_connect);
     }
   } /* IS_VOICE_CALL() */
...
{  
  ...
  if( NULL == end_params )
  {
   ...
   DATA_CSD_MSG2_HIGH("Complete LL connect handler for call id %d, subs id %d",
                  call_id,(int) subs_id );
  /* Find call instance in the call table */
  if( MAX_UCSD_CALL_TBL_ENTRIES <=
      (index = ds_ucsd_find_call_instance( DSUCSD_XLATE_CALLID( call_id ))) )
  ...
  {
    DATA_CSD_MSG0_LOW("Subs_id Mismatch");
    return DS3G_FAILURE;
  }
#endif /* FEATURE_DUAL_SIM */
      ...
      */
      ...
      else  /* Non-transparent */
      {
         ...
         switch ( ds_ucsd_mpshim_get_rlp_dc_state() )
         {
            ...
            default:
              DATA_CSD_MSG1_LOW("Bad compression mode=%d",
...
)
{
   ...
   DATA_CSD_MSG2_LOW("Call setup handler: CM call ID=%d mode %d",
                   call_id,
                   ds_ucsd_network_mode);
   ...
   if (mode_info_ptr->info.gw_cs_call.bearer_capability_1.present == TRUE)
   {
     ...
     if(data_bc_ptr == NULL)
     {
       DATA_CSD_MSG0_LOW("data_bc_ptr is NULL, CSD call rejected" );
       REJECT_CALL( setup_res_params, INCOMPATIBLE_DESTINATION );
       return DS3G_FAILURE; 
     }

     DATA_CSD_MSG2_HIGH("Current call_type = %d, subs_id = %d, Incoming Setup BC struct:",
   ...
   */
...
#endif /* FEATURE_MULTIMEDIA_FALLBACK */
     {
       ...
       if ( NV_DONE_S == nv_status )
       {
              ...
              IS_VIDEO_CALL( data_call_type ) ) 
         {
           ...
           DATA_CSD_MSG0_LOW("Incoming VT CSD call rejected, NV_DISABLE_CM_CALL_TYPE_I");
           REJECT_CALL( setup_res_params, INCOMPATIBLE_DESTINATION );
           return DS3G_FAILURE; 
         }
       }
       ...
       {
         DATA_CSD_MSG1_LOW("NV read failure NV status = %d",nv_status);
       }
                                           ...
                                           &call_info_ptr ) )
       {
         ...
         DATA_CSD_MSG1_LOW("CS Data call instance unavailable for CM call ID: %d ",
                    call_id);
       ...
       DATA_CSD_MSG0_LOW("Outgoing Setup BC struct:");
       ds_ucsd_print_bc_struct( data_bc_ptr );
           ...
           ds_ucsd_app_notify_clients_by_type( (uint8)data_call_type, &event ) )
       {
         ...
         DATA_CSD_MSG0_LOW("No client notified for call setup");
         *delay_rsp = FALSE;

         /* Assume external call control and datapath */
         call_info_ptr->call_ctrl_iface = DS_UCSD_IFACE_SIO;
         call_info_ptr->datapath_iface  = DS_UCSD_IFACE_SIO;
         DATA_CSD_MSG0_LOW("UCSD Routing: call_ctrl=SIO, datapath=SIO");
             ...
             IS_VIDEO_CALL( data_call_type ) ) /*lint !e655 */
         {
           ...
           DATA_CSD_MSG0_LOW("CS Data VT call setup failed, "
   ...
   else
   {
     ...
     REJECT_CALL( setup_res_params, INCOMPATIBLE_DESTINATION );
     DATA_CSD_MSG0_LOW("CS Data call setup failed - incompatible destination" );
   }  
#ifdef FEATURE_MULTIMEDIA_FALLBACK
   #error code not present
#endif /* FEATURE_MULTIMEDIA_FALLBACK */
...
)
{
  ...
  if ( NULL == end_params || NULL == delay_disconnect ) 
  {
    DS3GPP_ASSERT(0);
    return;
  }
  ...
  call_info_ptr = DSUCSD_GET_CALL_INFO( index );
   ...
   DATA_CSD_MSG2_MED( "User end handler: CM call ID=%d type=%d",
             call_id,
             curr_call_type );
...
)
{
  if ( NULL == end_params ) 
  {
   ...
   DATA_CSD_MSG0_LOW("Complete LL disconnect handler");
...
/*-------------------------------------------------------------------------*/
  ...
  DATA_CSD_MSG2_HIGH( "Call ended handler: CM call ID=%d type=%d",
            call_info_ptr->cm_call_id,
            curr_call_type );
  ...
  if( DS3G_SUCCESS != ds_ucsd_mpshim_call_cleanup( &call_desc,
                                                   call_was_connected ) )
  {
    DATA_CSD_MSG0_LOW("Problem in shim layer cleanup");
  }
  ...
  {
    DATA_CSD_MSG0_LOW("Disconnecting datapath");
    if( TRUE != ds_ucsd_disconnect_data_path( data_bc_ptr,
                                              call_info_ptr,
                                              TRUE ) )
    {
      DATA_CSD_MSG0_LOW("Problem disconnecting external interface datapath");
    }
  }
          ...
          (DS_UCSD_CALL_STATE_CONNECT    == call_info_ptr->state) )
      {
        ...
        if( DS3G_SIOLIB_OK !=
            ds3g_siolib_ex_change_mode( SIO_DS_AUTODETECT_MODE,
                                     ...
                                     call_info_ptr->sio_port_id) )
        {
          DATA_CSD_MSG0_LOW("Failed to set SIO to AUTODETECT mode");
        }
      }
    }
  } /* DS_UCSD_IFACE_SIO == datapath_iface */
  ...
  if ( sio_q_cnt > 0 )
  {
    DATA_CSD_MSG1_LOW("Cleaning up UL dswcsd_from_sio_q_cnt(%d)",
              sio_q_cnt);
    dswcsd_ul_cleanup();
  }  
#endif /* FEATURE_DATA_WCDMA_CS */
  ...
  {
    DATA_CSD_MSG0_LOW("Call ended handler: problem in common cleanup");
  }
#ifdef FEATURE_DUAL_SIM
...
/*-------------------------------------------------------------------------*/
  ...
  else
  {
    ...
    DATA_CSD_MSG0_LOW("AT escape sequence handler");
...
/*-------------------------------------------------------------------------*/
  ...
  if ( NULL == from_sio_wm_ptr ||
       NULL == to_sio_wm_ptr   ||
       NULL == mode_hdlr_sio_rx_func_ptr) 
  {
    DS3GPP_ASSERT(0);
    return;
  }
  ...
  else
  {
    ...
    DATA_CSD_MSG0_LOW("Return to online data handler");
...
/*-------------------------------------------------------------------------*/
  ...
  else
  {
    ...
    DATA_CSD_MSG0_LOW("Return to online data complete handler");
...
)
{
  ...
  uint8 index;
   ...
   DATA_CSD_MSG2_HIGH( "User answer handler: CM call ID=%d",
             call_id,
             call_info_ptr->call_type);
...
ds_ucsd_auto_answer_handler ( void )
{
  ...
  {
    /* Call instance in bad state, reply to external host */
    DATA_CSD_MSG0_LOW("Call instance not in ALERTING state, rejecting auto answer");
    dsat_send_result_code( DSAT_NO_CARRIER );
  }
  else
  {
    ...
    DATA_CSD_MSG2_HIGH( "Auto answer handler: CM call ID %d  inst ID %d",
              call_info_ptr->cm_call_id, call_info_ptr->inst_id);
    call_info_ptr->state = DS_UCSD_CALL_STATE_ANSWERED;
    ...
    {
      DATA_CSD_MSG2_LOW("Failed to answer the call: CM call ID = %d; CSD inst ID = %d",
                 call_info_ptr->cm_call_id, 
                 call_info_ptr->inst_id );
...
/*-------------------------------------------------------------------------*/
  ...
  else
  {
    ...
    DATA_CSD_MSG0_LOW("Return to online cmd mode complete handler");
...
)
{
   ...
   if ( NULL == call_info_ptr || NULL == data_bc_ptr) 
   {
     DS3GPP_ASSERT(0);
     return FALSE;
   }
   ...
   if( TRUE == call_info_ptr->neg_cap.present )
   {
     ...
     DATA_CSD_MSG0_LOW("Orig BC structure:");
     ds_ucsd_print_bc_struct( data_bc_ptr );
   ...
   {
     DATA_CSD_MSG0_LOW("Bearer capability not initialized, cannot originate");
     rc = FALSE;
   }
   ...
   if ( rc == TRUE ) 
   {
     if ( data_bc_ptr->sync_async == SYNCHRONOUS )
     {
       DATA_CSD_MSG1_LOW( "Sync CS Data call originated: FNUR=%d", 
                 data_bc_ptr->fixed_network_user_rate);
     ...
     {
       DATA_CSD_MSG1_LOW("Async CS Data call originated: FNUR=%d", 
                 data_bc_ptr->fixed_network_user_rate);
     }
   } /* rc == TRUE */
...
) 
{
   ...
   if ( NULL == dial_string           || 
        NULL == mode_spec_info_ptr    ||
        NULL == cdma_orig_params_ptr  ||
        NULL == cdma_params_changed   ||
        NULL == gw_cs_orig_params_ptr ||
        NULL == gw_cs_params_changed  ||
        NULL == gw_ps_orig_params_ptr ||
        NULL == gw_ps_params_changed  ||
        NULL == cm_srv_type ) 
   {
     DS3GPP_ASSERT(0);
     return DS3G_FAILURE;
   }
   DATA_CSD_MSG0_HIGH("Originate call handler");
   ...
   */
   {
                                  ...
                                  &chosen_capability ) )
     {
       ...
       */
       ...
       if ( NV_DONE_S == nv_status )
       {
         ...
         {
           /* Reject the incoming VT data call */
           DATA_CSD_MSG0_LOW("Originating VT CSD call rejected, NV_DISABLE_CM_CALL_TYPE_I");
           /* New call of this type cannot be supported. */
     ...
     {
       rc = DS3G_FAILURE;
       DATA_CSD_MSG0_LOW("Can't originate call: invalid bearer service type settings");
     } /* ds_ucsd_get_chosen_cap */
   }
...
)
{
  ...
  DATA_CSD_MSG1_LOW("CNTXT_TRANSFER_IND in sys mode:%d",
             sys_mode);
...
)
{
       ...
       NULL == end_params ) 
  {
   ...
   DATA_CSD_MSG2_HIGH("Incoming call handler: CM call ID=%d type=%d",
             call_id,
             curr_call_type );
   ...
   if ( ans_params->ans_params.gw_cs_ans.accept == TRUE )
   {
     ...
     if( TRUE !=
         ds_ucsd_decode_cbst_values( data_bc_ptr,
                                     ...
                                     &call_info_ptr->bearer_params.cbst_info.connection_element ))
     {
       DATA_CSD_MSG0_LOW("Failed to decode +CBST values"); 
     }
     ...
     {
       /* Now alert the user */
       DATA_CSD_MSG0_HIGH("Incoming CS Data call accepted" );
     ...
     else
     {
       ...
       DATA_CSD_MSG0_ERROR("Incoming CS Data call rejected with USER_BUSY cause" );
       ...
       {
         /* If CM fails to answer the call, DSMGR will end the call */
         DATA_CSD_MSG2_LOW("Failed to answer the call: "
                    "CM call ID = %d; CS Data call instance ID = %d", 
                    call_info_ptr->cm_call_id, 
                    call_info_ptr->inst_id);
         ...
         if( TRUE != ds_ucsd_call_end_common_cleanup( call_info_ptr->inst_id ) )
         {
           DATA_CSD_MSG0_LOW("Incoming call handler: problem in common cleanup");
         }
   ...
   {
     DATA_CSD_MSG0_LOW( "Incoming CS Data call rejected");
     return ( DS3G_REJECT_CALL );
   } /* accept == TRUE */
} /* ds_ucsd_incoming_call_handler() */
  ...
  if ( DS_UCSD_IS_WCDMA_TDS_MODE(ds_ucsd_network_mode) )
  {
   ...
   if( (ds_3gpp_csd_tds_tbl != NULL) && 
...
#endif /* FEATURE_SEGMENT_LOADING */
    {
      if( TRUE != ds_ucsd_mpshim_deregister_tdscdma_rlc(subs_id) )
      {
        DATA_CSD_MSG0_LOW("Problem releasing TD_SCDMA RLC interface");
        return FALSE;
      }
    }
    else 
    ...
    /* WCDMA CS Data */
    {
      ...
      if( TRUE != ds_ucsd_mpshim_deregister_wcdma_rlc(subs_id) )
      {
        DATA_CSD_MSG0_LOW("Problem deregistering from L2");
      }
    }
   /* Check for external datapath */
   if( DS_UCSD_IFACE_SIO == call_info_ptr->datapath_iface )
...
/*-------------------------------------------------------------------------*/
   ...
   {
     DATA_CSD_MSG1_LOW( "Cannot find call instance for CM call ID: %d",exist_call_id);
     return DS3G_REJECT_CALL;
   }
   call_info_ptr = DSUCSD_GET_CALL_INFO( index );
   ...
   DATA_CSD_MSG3_HIGH("Switch call handler: incomID=%d existID=%d type=%d",
             inc_call_id,
             exist_call_id,
             curr_call_type );
...
/*-------------------------------------------------------------------------*/
   ...
   DATA_CSD_MSG2_HIGH( "Force call down handler: callID=%d type=%d",
             exist_call_id,
             curr_call_type);
...
)
{
  ...
  if( MAX_UCSD_CALL_TBL_ENTRIES <=
      (index = ds_ucsd_find_call_instance( inst_id )) )
  {
    DATA_CSD_MSG1_LOW("Cannot find call instance: %d",inst_id);
    return FALSE;
  }
  call_info_ptr = DSUCSD_GET_CALL_INFO( index );
  ...
  *tx_to_network_byte_cnt   = call_info_ptr->tx_data_counter;
  DATA_CSD_MSG2_LOW("ds_ucsd_get_data_counters: tx=%d rx=%d",
...
{  
  ...
  {
    DATA_CSD_MSG0_LOW("NULL pointer parameter specified");
    return FALSE;
  }
...
/*-------------------------------------------------------------------------*/
  ...
  if ( NULL == dial_string || NULL == modifiers)
  {
    DS3GPP_ASSERT(0);
    return NULL;
  } 
  ...
  {
    DATA_CSD_MSG1_LOW("Invalid subs_id provided: %d", subs_id);
    return NULL;
  }
#endif /* FEATURE_DUAL_SIM */
  ...
  if ( inst_id != NO_CALL_ID ) 
  {
    ...
    DATA_CSD_MSG1_HIGH( "Initiating CS data call in mode %d",ds_ucsd_network_mode );
    ...
    else
    {
      ...
      /* Perform common call end cleanup. */
      (void)ds_ucsd_call_end_common_cleanup( inst_id );
    }
  }
  else
  {
...
ds_ucsd_hdlr_init (void)
{
  ...
  DATA_CSD_MSG0_HIGH("Initialize UMTS CS Data handlers");
#ifdef FEATURE_SEGMENT_LOADING
   ...
   {
     DATA_3GMGR_MSG0(MSG_LEGACY_LOW, "Neither TDS nor WCDMA active");
   }
#endif /* FEATURE_SEGMENT_LOADING  */
...
)
{
  ...
  {
    DATA_CSD_MSG1_LOW("Cannot find call instance for CM call ID: %d",cm_call_id );
    return FALSE;
  }
  call_info_ptr = DSUCSD_GET_CALL_INFO( index );
  ...
  */
  ...
  if ( (DS_UCSD_IFACE_SIO == call_info_ptr->call_ctrl_iface) &&
       (DS_UCSD_IFACE_SIO == call_info_ptr->datapath_iface) )
  {
    DATA_CSD_MSG0_LOW("Call is an ATCoP call");
    return TRUE;
  }
  else
  {
    DATA_CSD_MSG0_LOW("call is initiated by API");
    return FALSE;
  }
...
uint8 ds_ucsd_get_call_inst_from_call_id(cm_call_id_type cm_call_id)
{
  ...
  {
    DATA_CSD_MSG1_LOW("Invlaid CM Call_id passed: %d", cm_call_id);
