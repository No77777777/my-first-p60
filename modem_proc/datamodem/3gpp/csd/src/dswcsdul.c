...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp/csd/src/dswcsdul.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DATA_CSD_MSG1_LOW("Freeing DSM with data on cleanup: %d", 
  ...
  if ( !dswcsd_sio_inbound_flow_enabled )
  {
    ...
    DATA_CSD_MSG0_LOW( "Enabled inbound SIO flow");
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DATA_CSD_MSG0_LOW("CS Data UL clean_up");
  ...
  if ( NULL != dswcsd_sio_dsm_item_ptr )
  {
    DATA_CSD_MSG1_MED("Freeing DSM with data on cleanup: %d", 
             dsm_length_packet(dswcsd_sio_dsm_item_ptr));
    dsm_free_packet( &dswcsd_sio_dsm_item_ptr );
    dswcsd_sio_dsm_item_ptr = NULL;
  }
  ...
  if ( !dswcsd_sio_inbound_flow_enabled )
  {
    ...
    DATA_CSD_MSG0_MED("Enabled inbound SIO flow");
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DATA_CSD_MSG1_LOW(" Number of bytes_per_sdu for this call info is invalid %d",
         dswcsd_call_info.bytes_per_sdu);
    return NULL;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  while ( sdu_byte_cnt < dswcsd_call_info.bytes_per_sdu )
  {
    ...
    if ( dswcsd_sio_dsm_item_ptr == NULL )
    {
      ...
      {
        DATA_CSD_MSG1_LOW("dswcsd_sio_dsm_item_ptr is null and SIO_q has no data resetting dswcsd_from_sio_q_cnt to 0 earlier value %d",
    ...
    if ( dswcsd_dsm_payload_size == 0 )
    {
      if(item_became_chain == TRUE)
      {
        ...
        dswcsd_dsm_payload_size = (uint16)dsm_length_packet( dswcsd_sio_dsm_item_ptr );
        #ifdef V80_DEBUG
          DATA_CSD_MSG1_LOW("V80 chained the item in UL, pkt_ptr->size=%d",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DATA_CSD_MSG2_MED("CS tx status: siocnt=%d, rlccnt=%d", dswcsd_from_sio_q_cnt/80, 
  ...
  {
    DATA_CSD_MSG0_LOW("Don't post to RLC watermark");
...

  {
    ...
    else
    {
...

      {
        ...
        DATA_CSD_MSG2_LOW("Not enough data to tx: cnt=%d < %d",
                 dswcsd_from_sio_q_cnt,
  ...
  if ( sdu_cnt == dswcsd_call_info.sdus_per_tti ) 
  {
    for ( sdu_cnt = 0; sdu_cnt < dswcsd_call_info.sdus_per_tti; sdu_cnt++ ) 
    {
      if ( sdu_ptrs[ sdu_cnt ] != NULL )
      {
        ...
        {
          DATA_CSD_MSG1_LOW("Could not extract %d bytes from UL SDU log",
                    dswcsd_call_info.bytes_per_sdu);
        }
        ds_ucsd_log_tx_data( logbuf,
                             (ds_ucsd_frame_size_T)dswcsd_call_info.bytes_per_sdu );

        DATA_CSD_MSG1_LOW( "CS Data UL: send %d bytes to RLC",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ( *dsm_item_ptr != NULL ) 
  {
    ...
    */ 
    ...
    {
      dsm_free_packet (dsm_item_ptr);
      DATA_CSD_MSG2_LOW("CSD SIO Tx queue full, dropping packet: %d >= %d",
    ...
    else
    {
      ...
      DATA_CSD_MSG1_MED("CSD SIO Tx queue cnt=%d", dswcsd_from_sio_q_cnt);
        ...
        if ( 
             ...
             ( dswcsd_from_sio_q_cnt >= 
               ( dswcsd_call_info.sdus_per_tti * 
                 dswcsd_call_info.bytes_per_sdu ) ) )
        {
          DATA_CSD_MSG1_MED("CSD SIO TX post signal SIO q cnt %d", dswcsd_from_sio_q_cnt);
          ...
          ( dswcsd_from_sio_q_cnt >= FROM_SIO_Q_HI_CNT ) )
      {
        ...
        DATA_CSD_MSG0_LOW("Disabled inbound SIO flow");
      }
      ...
      {
        DATA_CSD_MSG1_LOW( "Problem updating data traffic counters: call ID=%d",
...
)
{
  ...
  {
    DATA_CSD_MSG1_LOW("Index: %d passed for dswcsd_known_pattern is invalid",
               index);
    return -1;
  }
...
/*-------------------------------------------------------------------------*/
  ...
  DATA_CSD_MSG1_HIGH("NV_DATA_QMSS_CSVT_ENABLE is set to %d",
