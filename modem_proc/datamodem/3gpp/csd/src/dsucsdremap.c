...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp/csd/src/dsucsdremap.c#3 $ $DateTime: 2020/11/16 21:05:49 $ $Author: pwbldsvc $
...
)
{
   ...
   DATA_CSD_MSG0_LOW( "In ds_read_remap_pending_buffer");
   ...
   if (NULL != Buffer)
   {
     ...
     if( 0 < size)
     {
       DATA_CSD_MSG1_LOW("Extracted data from Remap buffer: size=%d",size);

       if(size != dsm_pushdown_tail( &RmPDU, PDU, size, DSM_ITEM_POOL(RmPDU) ))
       {
         DATA_CSD_MSG1_LOW("DSM pushdown size mismatch %d",size);
       }
   ...
   DATA_CSD_MSG1_LOW("Read Remap buffer returns: %d", PDUsize);
   return PDUsize;
}
