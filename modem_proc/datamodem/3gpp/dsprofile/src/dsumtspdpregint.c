...
Copyright (c) 2006-2018 Qualcomm Technologies, Incorporated.  All Rights Reserved.
...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp/dsprofile/src/dsumtspdpregint.c#4 $ $DateTime: 2020/01/10 08:51:15 $ $Author: pwbldsvc $
...
---------------------------------------------------------------------------*/
#define DS_UMTS_PDP_CHECK_INIT_FLAG(subs_id) {\
  if( !ds_umts_pdp_registry_init_flag )\
    {\
      DS_3GPP_ERROR_LOG_0_EX(subs_id, "PDP registry not initialized");\
...
---------------------------------------------------------------------------*/
#define  DS_UMTS_PDP_CHECK_FILTER_ID(filter_id,subs_id) {\
  if( (filter_id == 0) || \
      (filter_id > DS_UMTS_MAX_TFT_PARAM_SETS) )\
    {\
      DS_3GPP_ERROR_LOG_1_EX(subs_id,  "Invalid filter identifier:d ",filter_id);\
...
---------------------------------------------------------------------------*/
#define DS_UMTS_PDP_CHECK_OUT_BUFF_LEN(val,max,subs_id) {\
  if( max > val ) \
    {\
      DS_3GPP_ERROR_LOG_2_EX(subs_id, "out buffer parm short :V:d M:d",val,max);\
...
---------------------------------------------------------------------------*/
#define DS_UMTS_PDP_CHECK_IN_BUFF_LEN(val,max,subs_id) {\
  if( val > max ) \
    {\
      DS_3GPP_ERROR_LOG_2_EX(subs_id, "Input buff > Max allowed V:d M :d",val,max);\
...
#define DS_UMTS_PDP_ALLOC_LOCAL_PDP_PROFILE(PTR) {\
  ...
  if(NULL == PTR)\
  {\
    DS_3GPP_ERROR_LOG_2("Memory allocation failure for client:d, size:d",\
...
/*---------------------------------------------------------------------------
  Structure to store AP default profile
---------------------------------------------------------------------------*/
typedef struct
{
  boolean  embd_profile_valid;
  byte     embd_profile_num;
  boolean  teth_profile_valid;
  byte     teth_profile_num;
} ds_umts_pdp_ap_def_profile_type;

/*---------------------------------------------------------------------------
  Cached value of AP profile number for embedded calls.
---------------------------------------------------------------------------*/
static ds_umts_pdp_ap_def_profile_type  ap_def_profile_info[DS_SUBSCRIPTION_MAX];
...
)
{
  ...
  {
    DS_3GPP_ERROR_LOG_1_EX(subs_id, "Profile_number:d not present in cache",
                       profile_number);
...
/*===========================================================================
FUNCTION DS_UMTS_PDP_CHECK_SUBSCRIPTION_ID

DESCRIPTION
This function checks the validity of subscription id.
...
)
{
  ...
  {
    DS_3GPP_ERROR_LOG_0_EX(subs_id, "Active subscription ID is invalid");
...
)
{
  ...
  {
    DS_3GPP_ERROR_LOG_2_EX(subs_id, "File write failed result=d errno:d",
                       result, mcfg_fs_errno(MCFG_FS_TYPE_EFS));
    return FALSE;
  }

  /*-------------------------------------------------------------------------
    update KAMGR for change in default profile.
  -------------------------------------------------------------------------*/
  if(profile_number != (uint16)cached_embedded_call_profile_number[subs_id])
  {
    ds_3gpp_kamgr_default_profile_chg_cback((sys_modem_as_id_e_type)subs_id,
                                            profile_number);
  }
  /*-------------------------------------------------------------------------
...
/*===========================================================================
FUNCTION DS_UMTS_UPDATE_AP_EMBEDDED_CALL_PROF_NUM

DESCRIPTION
  This function updates the AP def profile number.
...
SIDE EFFECTS
  None
===========================================================================*/
boolean ds_umts_update_ap_embedded_call_prof_num
(
  ds_umts_pdp_subs_e_type  subs_id,
  uint16                   profile_number
)
{
  ...
  /*-------------------------------------------------------------------------
    Open the file for write
  -------------------------------------------------------------------------*/
  result = mcfg_fs_write(DS_3GPP_PROFILE_AP_EMBD_PROF_NUM,
                         (void *)(&profile_num),
                         sizeof(byte),
                         MCFG_FS_O_CREAT | MCFG_FS_O_TRUNC | MCFG_FS_O_RDWR,
                         MCFG_FS_DEFFILEMODE,
  ...
  /*-------------------------------------------------------------------------
    Set the cached embedded call profile_number for later reads
  -------------------------------------------------------------------------*/
  if( DS_UMTS_PDP_SUCCESS == ds_umts_pdp_check_profile_number(profile_number,
                                                              subs_id) )
  {
    if( ap_def_profile_info[subs_id].embd_profile_num != profile_number )
    {
      ap_def_profile_info[subs_id].embd_profile_valid = TRUE;
      ap_def_profile_info[subs_id].embd_profile_num = (byte)profile_number;
    }
  }
...
/*===========================================================================
FUNCTION DS_UMTS_UPDATE_AP_DEF_TETHERED_PROF_NUM
...
SIDE EFFECTS
  None
===========================================================================*/
boolean ds_umts_update_ap_def_tethered_prof_num
(
  ds_umts_pdp_subs_e_type  subs_id,
  uint16                   profile_number
)
{
  ...
  /*-------------------------------------------------------------------------
    Open the file for write
  -------------------------------------------------------------------------*/
  result = mcfg_fs_write(DS_3GPP_PROFILE_AP_TETH_PROF_NUM,
                         (void *)(&profile_num),
                         sizeof(byte),
                         MCFG_FS_O_CREAT | MCFG_FS_O_TRUNC | MCFG_FS_O_RDWR,
                         MCFG_FS_DEFFILEMODE,
  ...
  if( DS_UMTS_PDP_SUCCESS == ds_umts_pdp_check_profile_number(profile_number,
                                                              subs_id) )
  {
    if( ap_def_profile_info[subs_id].teth_profile_num != profile_number )
    {
      ap_def_profile_info[subs_id].teth_profile_valid = TRUE;
      ap_def_profile_info[subs_id].teth_profile_num = (byte)profile_number;
    }
  }
...
/*===========================================================================
FUNCTION DS_UMTS_IS_PROFILE_MODEM_DEFAULT_PROFILE

DESCRIPTION
  This function checks if the profile number is modem_def_profile

PARAMETERS 
 profile_number : profile number of the profile you want to check 
 subs_id : Subscription ID
...
RETURN VALUE
  TRUE       : profile number clear successfully
  FALSE      : Error in updating profile number

SIDE EFFECTS
  None
===========================================================================*/
boolean ds_umts_is_profile_modem_default_profile
(
  ds_profile_db_profile_num_type  profile_number,
  ds_umts_pdp_subs_e_type         subs_id
)
{
  char     profile_name[DS_UMTS_MAX_PROFILE_NAME_LEN] = {0};
  boolean  ret_val = FALSE; 
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if (ds_umts_get_pdp_profile_name_internal
       ( 
         (uint16)profile_number,
         subs_id,
         (byte *)&profile_name[0],
  ...
  {
    /*------------------------------------------------------------ 
      Check if profile name is modem default profile
      -----------------------------------------------------------*/
    if (strncasecmp ( profile_name,
                      (const char*)DS_3GPP_PROFILE_MODEM_AP_DEF_PROF_SIG,
                       DS_UMTS_MAX_PROFILE_NAME_LEN) == 0)
    {
      ret_val =  TRUE;
    }
  }
  ...
  return ret_val;
}/*ds_umts_is_profile_modem_default_profile*/
/*===========================================================================
FUNCTION DS_UMTS_CLEAR_MODEM_EMBEDDED_CALL_PROF_NUM

DESCRIPTION
  This function clears the modem embedded profile number.
...
SIDE EFFECTS
  None
===========================================================================*/
boolean ds_umts_clear_modem_embedded_call_prof_num
(
  ds_umts_pdp_subs_e_type  subs_id
)
{
...
/*-------------------------------------------------------------------------
  ...
  /*-------------------------------------------------------------------------
    Open the file for write
  -------------------------------------------------------------------------*/
  result = mcfg_fs_delete(ds_umts_pdp_fully_qual_embedded_call_file_name,
                          MCFG_FS_TYPE_EFS,
                          (mcfg_fs_sub_id_e_type)subs_id);

  if ( result != MCFG_FS_STATUS_OK )
  {
    DS_3GPP_MSG2_LOW("File deletion result=d errno:d",
                       result, mcfg_fs_errno(MCFG_FS_TYPE_EFS));
    return FALSE;
  }

  /*----------------------------------------------------------------------
     Reset global cache and already read variable
     --------------------------------------------------------------------*/
  cached_embedded_call_profile_number[subs_id] = 0;
  embedded_call_profile_number_read[subs_id]  = FALSE;

  return TRUE;
}/*ds_umts_clear_modem_embedded_call_prof_num*/

/*===========================================================================
FUNCTION DS_UMTS_CLEAR_AP_EMBEDDED_CALL_PROF_NUM

DESCRIPTION
  This function clear the AP def embedded profile number.
...
SIDE EFFECTS
  None
===========================================================================*/
boolean ds_umts_clear_ap_embedded_call_prof_num
(
  ds_umts_pdp_subs_e_type  subs_id
)
{
  ...
  /*-------------------------------------------------------------------------
    Open the file for write
  -------------------------------------------------------------------------*/
  result = mcfg_fs_delete(DS_3GPP_PROFILE_AP_EMBD_PROF_NUM,
                          MCFG_FS_TYPE_EFS,
                          (mcfg_fs_sub_id_e_type)subs_id);
  if ( result != MCFG_FS_STATUS_OK )
  {
  ...
  /*-------------------------------------------------------------------------
    Set the cached embedded call profile_number for later reads
  -------------------------------------------------------------------------*/
  ap_def_profile_info[subs_id].embd_profile_valid = FALSE;
  ap_def_profile_info[subs_id].embd_profile_num = 0;

  return TRUE;
}

/*===========================================================================
FUNCTION DS_UMTS_CLEAR_MODEM_TETHERED_CALL_PROF_NUM

DESCRIPTION
  This function clears the modem tethered profile number.
...
SIDE EFFECTS
  None
===========================================================================*/
boolean ds_umts_clear_modem_tethered_call_prof_num
(
  ds_umts_pdp_subs_e_type  subs_id
)
{
  ...
  -------------------------------------------------------------------------*/
  result = mcfg_fs_delete(ds_umts_pdp_fully_qual_tethered_call_file_name,
                          MCFG_FS_TYPE_EFS,
                          (mcfg_fs_sub_id_e_type)subs_id);
  ...
  return TRUE;
}/*ds_umts_clear_modem_tethered_call_prof_num*/
/*===========================================================================
FUNCTION DS_UMTS_CLEAR_AP_DEF_TETHERED_PROF_NUM

DESCRIPTION
  This function clear the AP def tethered profile number.
...
RETURN VALUE
  TRUE       : profile number cleared successfully
  FALSE      : Error in updating profile number

SIDE EFFECTS
  None
===========================================================================*/
boolean ds_umts_clear_ap_def_tethered_prof_num
(
  ds_umts_pdp_subs_e_type  subs_id
)
  {
  ...
  /*-------------------------------------------------------------------------
    Open the file for write
  -------------------------------------------------------------------------*/
  result = mcfg_fs_delete(DS_3GPP_PROFILE_AP_TETH_PROF_NUM,
                          MCFG_FS_TYPE_EFS,
                          (mcfg_fs_sub_id_e_type)subs_id);
  if ( result != MCFG_FS_STATUS_OK )
  {
  ...
  /*-------------------------------------------------------------------------
    Set the cached tethered call profile_number for later reads
  -------------------------------------------------------------------------*/
  ap_def_profile_info[subs_id].teth_profile_valid = FALSE;
  ap_def_profile_info[subs_id].teth_profile_num = 0;

  return TRUE;
}

/*===========================================================================
FUNCTION DS_UMTS_CLEAR_MODEM_EMBEDDED_AND_TETHERED_PROFILES
...
SIDE EFFECTS
  None
===========================================================================*/
void ds_umts_clear_modem_embedded_and_tethered_profiles
(
...
/*- - - - - -  - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  subs_id = dsumts_subs_mgr_get_subs_id(as_id);
  ...
  if (!ds_umts_clear_modem_embedded_call_prof_num(subs_id))
  {
    DS_3GPP_MSG1_LOW("Unable to clear modem embedded profile from EFS"
                          "for subs", subs_id);
  }

  if (!ds_umts_clear_modem_tethered_call_prof_num(subs_id))
  {
    DS_3GPP_MSG1_LOW("Unable to clear modem tethered profile from EFS"
                          "for subs", subs_id);
  }

  return ;
}/*ds_umts_clear_modem_embedded_and_tethered_profiles*/
/*===========================================================================
...
)
{
  ...
  {
    DS_3GPP_ERROR_LOG_0_EX(subs_id, "Copying profile name(strlcpy) failed");
    return FALSE;
  }
  /*lint +e534*/
...
)
{
  ...
  if ( result == -1)
  {
    DS_3GPP_ERROR_LOG_2_EX(subs_id, "efs_unlink failed for fd=d errno=d.",
                       result, mcfg_fs_errno(MCFG_FS_TYPE_EFS));
    return FALSE;
  }
...
SIDE EFFECTS
  None
===========================================================================*/
boolean ds_umts_get_pdp_profile_context_info_from_cache
(
  uint16                     profile_number,
  ds_umts_pdp_subs_e_type    subs_id,
...
ds_umts_pdp_profile_status_etype ds_umts_pdp_registry_init_internal(void)
{
  ...
  memset(ap_def_profile_info,
         0,
         sizeof(ds_umts_pdp_ap_def_profile_type)*DS_SUBSCRIPTION_MAX);
  ...
  if ((dirp == NULL) || (EFSDIR *)-1 == dirp)
  {
    ...
    {
      DS_3GPP_ERROR_LOG_0("Could not create pdp_profiles directory");
    }
  }
...
)
{
  ...
  -------------------------------------------------------------------------*/
  DS_UMTS_PDP_CHECK_INIT_FLAG(subs_id);
  ...
  if( status != TRUE )
  {
    DS_3GPP_ERROR_LOG_1_EX(subs_id, "PDP profile access error:d",status);
...
{
  DS_3GPP_ERROR_LOG_0_EX(subs_id, "Format not supported");
...
{
  boolean  status;
  ds_umts_pdp_profile_type        *pdp_local_profile_ptr = NULL;
  ...
  if(  ds_umts_pdp_check_profile_number(profile_number, subs_id) ==
     DS_UMTS_PDP_INVALID_PROFILE_NUM )
  {
    ret_val = DS_UMTS_PDP_INVALID_PROFILE_NUM;
  ...
  if(ds_umts_pdp_check_subscription_id(subs_id) ==
     DS_UMTS_PDP_INVALID_SUBS_ID)
  {
     ret_val = DS_UMTS_PDP_INVALID_SUBS_ID;
  ...
  DS_UMTS_PDP_CHECK_OUT_BUFF_LEN(profile_name_buf_len,
        DS_UMTS_MAX_PROFILE_NAME_LEN,subs_id);

  /*-------------------------------------------------------------------
     Allocate memory on heap, sanitize it
    ------------------------------------------------------------------*/
  pdp_local_profile_ptr =(ds_umts_pdp_profile_type*)
                              modem_mem_alloc(sizeof(ds_umts_pdp_profile_type),
                              MODEM_MEM_CLIENT_DATA); 
  
  if (pdp_local_profile_ptr == NULL)
  {
  ...
  status = ds_umts_get_pdp_profile_all_info_from_cache(
                                            profile_number,
                                            subs_id,
                                            pdp_local_profile_ptr);
  if( status != TRUE )
  {
    ret_val = DS_UMTS_PDP_ACCESS_ERROR;
  ...
  strlcpy( (char*)profile_name_buf,
          (char*)pdp_local_profile_ptr->profile_name,
          profile_name_buf_len);
  /*lint +e534*/

  /*------------------------------------------------------------------- 
    If we have reached here, then there was no error, return success
    -------------------------------------------------------------------*/
...
func_exit:
  if (pdp_local_profile_ptr != NULL)
  {
     modem_mem_free((void *)pdp_local_profile_ptr,
                     MODEM_MEM_CLIENT_DATA );
  }

  if (local_err != DS_LOCAL_ERR_VAL_NONE)
  {
    DS_3GPP_ERROR_LOG_1_EX(subs_id, "ds_umts_get_pdp_profile_name_internal error:d",
                       local_err);
  }
...
{
  DS_3GPP_ERROR_LOG_0_EX(subs_id, "PDP profile API not supported");
...
/*===========================================================================
FUNCTION DS_UMTS_GET_PDP_PROFILE_APN_TYPE

DESCRIPTION
  This function retrieves the APN Type Mask specified in the profile.

PARAMETERS
  profile_number  : profile number
  subs_id         : subscription id.
  apn_type_mask   : Pointer to the APN Type Mask
...
SIDE EFFECTS
  None
===========================================================================*/
ds_umts_pdp_profile_status_etype ds_umts_get_pdp_profile_apn_type
(
  uint16                  profile_number,
  ds_umts_pdp_subs_e_type subs_id,
...
)
{
  ...
  DS_UMTS_PDP_TASKLOCK_NON_L4();
  *apn_type_mask = DS_UMTS_PDP_PROFILE_CACHE(subs_id, cache_index)->context.apn_type_mask;

  DS_UMTS_PDP_TASKFREE_NON_L4();

  return DS_UMTS_PDP_SUCCESS;
} /* ds_umts_get_pdp_profile_apn_type */
...
)
{
  ...
  ------------------------------------------------------------------------*/
  DS_UMTS_PDP_CHECK_OUT_BUFF_LEN(apn_buf_len,DS_UMTS_MAX_APN_STRING_LEN,subs_id);
...
)
{
  ...
  -------------------------------------------------------------------*/
  DS_UMTS_PDP_CHECK_OUT_BUFF_LEN(username_buf_len,
...
)
{
  ...
  /*-------------------------------------------------------------------
    ...
    Check the length of the output buffer here.
  -------------------------------------------------------------------*/
  DS_UMTS_PDP_CHECK_OUT_BUFF_LEN(password_buf_len,
                                 DS_UMTS_MAX_QCPDP_STRING_LEN,
                                 subs_id);
...
)
{
  ...
  -------------------------------------------------------------------*/
  DS_UMTS_PDP_CHECK_FILTER_ID(filter_id, subs_id);
...
)
{
...
#if 0
  /*-------------------------------------------------------------------
    Check if AP embedded call profile number is available
  -------------------------------------------------------------------*/
  if( DS_UMTS_PDP_SUCCESS ==
        ds_umts_get_pdp_ap_profile_num_for_embedded_calls_internal(
          subs_id, profile_number) )
  {
    DS_3GPP_MSG1_LOW("AP default embedded profile:%d",*profile_number);
  }
  #endif
  if( ds3gdevmgr_get_internet_prof_from_apn_info(
      profile_number, dsumts_subs_mgr_get_as_id(subs_id)))
  {
    return DS_UMTS_PDP_SUCCESS;
  }
  /*-------------------------------------------------------------------
  ...
  if( (0 != cached_embedded_call_profile_number[subs_id]) &&
           (embedded_call_profile_number_read[subs_id]) )
  {
  ...
  else
  {
    ...
    {
      DS_3GPP_MSG2_LOW("file read failed result:%d errno:%d ",
                         efs_result, mcfg_fs_errno(MCFG_FS_TYPE_EFS));
      *profile_number = DS_3GPP_DEF_EMBEDDED_PROFILE;
      return DS_UMTS_PDP_SUCCESS;
    }
...
)
{
...
#if 0
  /*-------------------------------------------------------------------
    Check if AP tethered call profile number is available
  -------------------------------------------------------------------*/
  if( DS_UMTS_PDP_SUCCESS ==
        ds_umts_get_pdp_ap_profile_num_for_tethered_calls_internal(
          subs_id, profile_number) )
  {
    DS_3GPP_MSG1_LOW("AP default tethered profile:%d",*profile_number);
  }
 #endif
  if( ds3gdevmgr_get_internet_prof_from_apn_info(
         profile_number, dsumts_subs_mgr_get_as_id(subs_id)))
  ...
  if( (0 != cached_tethered_call_profile_number[subs_id]) &&
           (tethered_call_profile_number_read[subs_id]) )
  {
  ...
  else
  {
    ...
    if ( efs_result != MCFG_FS_STATUS_OK )
    {
      DS_3GPP_MSG2_LOW("file read failed result:%d errno",
                         efs_result, mcfg_fs_errno(MCFG_FS_TYPE_EFS));
      *profile_number = DS_3GPP_DEF_TETHERED_PROFILE;
      return DS_UMTS_PDP_SUCCESS;
    }
...
/*===========================================================================
FUNCTION DS_UMTS_GET_PDP_AP_PROFILE_NUM_FOR_EMBEDDED_CALLS_INTERNAL

DESCRIPTION
  This function retrieves the AP profile number specififed for embedded
  data calls ( embedded calls ).  The cached value is return if available;
  otherwise, the read is done from the EFS file.
...
{
  mcfg_fs_status_e_type mcfg_fs_result = 0;
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /*-------------------------------------------------------------------
    Check if cached embedded call profile number is available
  -------------------------------------------------------------------*/
  if( (0 != ap_def_profile_info[subs_id].embd_profile_num) &&
      (ap_def_profile_info[subs_id].embd_profile_valid) )
  {
    /*-------------------------------------------------------------------
    if the 3gpp profile cache is invalid, this means the stale profile 
    number exits in the EFS. So go ahead and delete it from 
    both the AP cache and the EFS .
    -------------------------------------------------------------------*/
    if (!ds_3gpp_profile_cache_is_entry_valid
         (
           (ds_profile_db_profile_num_type)
           ap_def_profile_info[subs_id].embd_profile_num,
            subs_id
         ) ||
          !ds_umts_is_profile_modem_default_profile
           (
    ...
    {
      if( !ds_umts_clear_ap_embedded_call_prof_num(subs_id))
      {
        DS_3GPP_MSG0_LOW("failure clearing ap embedded cache");
      }
      return DS_UMTS_PDP_FAIL;
    }
    else
    {
    *profile_number = ap_def_profile_info[subs_id].embd_profile_num;
    }
  }
  else
  {
    mcfg_fs_result = mcfg_fs_read( DS_3GPP_PROFILE_AP_EMBD_PROF_NUM,
                                   (void *)profile_number,
                                   sizeof(byte),
                                   MCFG_FS_TYPE_EFS,
                                   (mcfg_fs_sub_id_e_type)subs_id );
    if ( mcfg_fs_result != MCFG_FS_STATUS_OK )
    {
      DS_3GPP_MSG2_LOW("file read failed result:%d errno:%d ",
                       mcfg_fs_result, mcfg_fs_errno(MCFG_FS_TYPE_EFS));
      return DS_UMTS_PDP_ACCESS_ERROR;
    }

    /*-------------------------------------------------------------------
      Set the cached AP embedded call profile_number for later reads
    -------------------------------------------------------------------*/
    ap_def_profile_info[subs_id].embd_profile_num = *profile_number;
    ap_def_profile_info[subs_id].embd_profile_valid = TRUE;
  }
...
/*===========================================================================
FUNCTION DS_UMTS_GET_PDP_AP_PROFILE_NUM_FOR_TETHERED_CALLS_INTERNAL
...
)
{
  ...
  /*-------------------------------------------------------------------
    Check if cached embedded call profile number is available
  -------------------------------------------------------------------*/
  if( (0 != ap_def_profile_info[subs_id].teth_profile_num) &&
      (ap_def_profile_info[subs_id].teth_profile_valid) )
  {
    /*-------------------------------------------------------------------
    if cache_index return <= 0 , that means the profile is invalid and only 
    the stale profile number exits in the EFS. So go ahead and delete it from 
    both the cache and the EFS .
    -------------------------------------------------------------------*/
    if (!ds_3gpp_profile_cache_is_entry_valid
         (
           (ds_profile_db_profile_num_type)
           ap_def_profile_info[subs_id].teth_profile_num,
            subs_id
         ) ||
          !ds_umts_is_profile_modem_default_profile
           (
    ...
    {
      if( !ds_umts_clear_ap_def_tethered_prof_num(subs_id))
      {
        DS_3GPP_MSG0_LOW("failure clearing ap tethered cache");
      }
      return DS_UMTS_PDP_FAIL;
    }
    else
    {
    *profile_number = ap_def_profile_info[subs_id].teth_profile_num;
    }
  }
  else
  {
    mcfg_fs_result = mcfg_fs_read( DS_3GPP_PROFILE_AP_TETH_PROF_NUM,
                                   (void *)profile_number,
                                   sizeof(byte),
                                   MCFG_FS_TYPE_EFS,
                                   (mcfg_fs_sub_id_e_type)subs_id );
    ...
    /*-------------------------------------------------------------------
      Set the cached AP tethered call profile_number for later reads
    -------------------------------------------------------------------*/
    ap_def_profile_info[subs_id].teth_profile_num = *profile_number;
    ap_def_profile_info[subs_id].teth_profile_valid = TRUE;
  }
...
/*===========================================================================
FUNCTION DS_UMTS_GET_PDP_PDN_LABEL_INTERNAL

DESCRIPTION
  This function retrieves "pdn label" for a profile

PARAMETERS
  profile_number  : profile number
  subs_id :Subscription id.
  pdn_label   : pointer to store the retrived profile label.
...
===========================================================================*/
ds_umts_pdp_profile_status_etype
  ds_umts_get_pdp_pdn_label_internal
(
...
)
{
  ...
  /*-------------------------------------------------------------------
    Read the pdn label info from the cache table.
  -------------------------------------------------------------------*/

  DS_UMTS_PDP_TASKLOCK_NON_L4();
  (void)memscpy(pdn_label,
          (DS_3GPP_MAX_PDN_LABLE_LENTH +1),
          DS_UMTS_PDP_PROFILE_CACHE(subs_id, cache_index)->pdn_label,
          sizeof(DS_UMTS_PDP_PROFILE_CACHE(subs_id, cache_index)->pdn_label));
...
DESCRIPTION
  This function sets "pdn label" for a profile
...
===========================================================================*/
ds_umts_pdp_profile_status_etype
  ds_umts_set_pdp_pdn_label_internal
(
...
)
{
  ...
  DS_3GPP_ERROR_LOG_0("PDP registry API not supported");
  return DS_UMTS_PDP_API_NOT_SUPPORTED;
#else
  int8 cache_index = -1;
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
...
)
{
  ...
  switch( family )
  {
    case DS_UMTS_EMBEDDED_PROFILE_FAMILY:
      ...
      if (ds_umts_update_embedded_call_prof_num(subs_id,profile_number) &&
          ds_umts_update_default_tethered_prof_num(subs_id,profile_number))
      {
...
/*===========================================================================
FUNCTION DS_UMTS_SET_AP_DEF_PROFILE_NUMBER_PER_SUBS_INTERNAL

DESCRIPTION
  This function sets the AP default profile number for tethered or embedded
  family for the specified subscription. The value is written to either
  embedded_call_prof_nums_dsds or tethered_call_prof_nums_dsds file.
...
===========================================================================*/
ds_umts_pdp_profile_status_etype
ds_umts_set_ap_def_profile_number_internal
(
  ds_umts_profile_family_e_type  family,
  ds_umts_pdp_subs_e_type        subs_id,
  uint16                         profile_number
)
{
  ...
  {
    case DS_UMTS_EMBEDDED_PROFILE_FAMILY:
      if(ds_umts_update_ap_embedded_call_prof_num(subs_id,profile_number))
      {
         status = DS_UMTS_PDP_SUCCESS;
      }
      break;
    ...
    case DS_UMTS_TETHERED_PROFILE_FAMILY:
      if(ds_umts_update_ap_def_tethered_prof_num(subs_id,profile_number))
      {
         status = DS_UMTS_PDP_SUCCESS;
      }
      break;
...
/*===========================================================================
FUNCTION DS_UMTS_CLEAR_AP_DEF_PROFILE_NUMBER_PER_SUBS_INTERNAL

DESCRIPTION
  This function resets the AP default profile number for tethered or embedded
  family for the specified subscription.
...
===========================================================================*/
ds_umts_pdp_profile_status_etype
ds_umts_clear_ap_def_profile_number_internal
(
  ds_umts_profile_family_e_type  family,
  ds_umts_pdp_subs_e_type        subs_id
)
...
/*===========================================================================
FUNCTION DS_UMTS_GET_AP_DEF_PROFILE_NUMBER_INTERNAL

DESCRIPTION
  This function gets the AP default profile number for the specified
  subscription. It gets the value specified to the
  embedded_call_prof_nums_dsds file or tethered_call_prof_nums_dsds file.
...
===========================================================================*/
ds_umts_pdp_profile_status_etype
ds_umts_get_ap_def_profile_number_internal
(
  ds_umts_profile_family_e_type   family,
  ds_umts_pdp_subs_e_type         subs_id,
  uint16                         *profile_number
)
{
  ...
  *profile_number = 0;
  ...
  {
    case DS_UMTS_EMBEDDED_PROFILE_FAMILY:
      status = ds_umts_get_pdp_ap_profile_num_for_embedded_calls_internal(
                 subs_id, (byte *)profile_number);
      break;

    case DS_UMTS_TETHERED_PROFILE_FAMILY:
      status = ds_umts_get_pdp_ap_profile_num_for_tethered_calls_internal(
                 subs_id, (byte*)profile_number);
      break;
...
/*===========================================================================
FUNCTION DS_UMTS_GET_PROFILE_NUM_FROM_APN_NAME
...
===========================================================================*/
uint16 ds_umts_get_profile_num_from_apn_name
(
  char                    *apn,
  ds_umts_pdp_subs_e_type  subs_id
)
{
  ...
  for (cache_index = 1; cache_index <= DS_UMTS_MAX_PDP_PROFILE_NUM; cache_index++)
  {
    ds_umts_pdp_profile_ptr = DS_UMTS_PDP_PROFILE_CACHE(subs_id, cache_index);
    ...
    {
      if (strncasecmp(apn,
                      (const char*)ds_umts_pdp_profile_ptr->context.apn,
                      DS_UMTS_MAX_PROFILE_NAME_LEN) == 0)
      {
        profile_num = ds_umts_pdp_profile_ptr->context.pdp_context_number;
        break;
      }
    }
  }
  ...
  return profile_num;
} /* ds_umts_get_profile_num_from_apn_name */

/*===========================================================================
FUNCTION DS_UMTS_GET_PROFILE_NUM_FROM_APN_TYPE
...
RETURN VALUE
  ...
  described by arr_size
===========================================================================*/
uint16 ds_umts_get_profile_num_from_apn_type
(
  uint64                   apn_type_mask,
  ds_umts_pdp_subs_e_type  subs_id
)
{
  ...
  for (cache_index = 1; cache_index <= DS_UMTS_MAX_PDP_PROFILE_NUM; cache_index++)
  {
    ...
    if (ds_umts_pdp_profile_ptr != NULL && ds_umts_pdp_profile_ptr->context.valid_flg)
    {
      if ((ds_umts_pdp_profile_ptr->context.apn_type_mask & apn_type_mask) != 0)
      {
        profile_num = ds_umts_pdp_profile_ptr->context.pdp_context_number;
        break;
      }
  ...
  return profile_num;
} /* ds_umts_get_profile_num_from_apn_type */
...
/*===========================================================================
FUNCTION DS_UMTS_PDP_CONTEXT_GET_INTERNET_APN_NAME

DESCRIPTION
  This function retrieves the Internet APN Name

PARAMETERS
  internet_apn_p: Pointer to the Internet APN which will
...
SIDE EFFECTS
  None
===========================================================================*/
boolean ds_umts_pdp_context_get_apn_name
(
  byte                     *apn_p,
  ds_umts_pdp_subs_e_type   subs_id,
  uint32                    apn_type
)
{
  ...
  uint8                                  profile_count = 0;
  ds_umts_pdp_context_type  		     profile_context;
  int                                    i = 0; 
  boolean                                ret_val = FALSE;
  ...
  do
  {
    ...
    memset(&profile_context, 0, sizeof(ds_umts_pdp_context_type));

    /*------------------------------------------------------------------------------
      If want to get apn with default APN type, first check default profile, if it
      is default APN type, return it right away. Otherwise, will loop through
      all valid profile and get first matched apn.
    ------------------------------------------------------------------------------*/
    if (apn_type == DS_APN_TYPE_MASK_DEFAULT &&
        ds_umts_get_default_profile_number_internal(
                            DS_UMTS_EMBEDDED_PROFILE_FAMILY,
                            (ds_umts_pdp_subs_e_type)subs_id,
                            &profile_no))
    {
      result = ds_umts_get_pdp_profile_context_info_per_subs( profile_no, 
                                                              subs_id, 
                                                              &profile_context);

      if (result == DS_UMTS_PDP_SUCCESS &&
          (profile_context.apn_type_mask & DS_APN_TYPE_MASK_DEFAULT) != 0)
      {
        memscpy(apn_p, DS_SYS_MAX_APN_LEN,
                profile_context.apn, DS_SYS_MAX_APN_LEN);  
    ...
    profile_count = 
      ds_3gpp_profile_cache_get_valid_profile_numbers(
        dsumts_subs_mgr_get_subs_id((sys_modem_as_id_e_type)subs_id), 
        profile_nums_arr, DS_UMTS_MAX_PDP_PROFILE_NUM);

    for (i=0; i< profile_count; i++)
    {
      result = ds_umts_get_pdp_profile_context_info_per_subs( profile_nums_arr[i], 
                                                              subs_id, 
                                                              &profile_context);
      ...
      if ((profile_context.apn_type_mask & apn_type) != 0)
      {
        memscpy(apn_p, DS_SYS_MAX_APN_LEN,
                profile_context.apn, DS_SYS_MAX_APN_LEN);  
