...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp/dsprofile/src/dsumts_subs_mgr.c#2 $
...
)
{
  ...
  if (fs_res == -1)
  {
    DS_3GPP_ERROR_LOG_3_EX(subs_id, "efs_write failed for fd=d errno=d", fd, efs_errno, 0);
    return FALSE;
  }
  /*lint -e737*/
  else if (fs_res != nbyte)
  {
    DS_3GPP_ERROR_LOG_3_EX(subs_id, "efs_write wrote wrong num of bytes=d errno=d", fs_res, efs_errno, 0);
    return FALSE;
  }
  /*lint +e737*/
  return TRUE;
...
)
{
  ...
  if (fs_res == -1)
  {
    DS_3GPP_ERROR_LOG_3_EX(subs_id, "efs_read failed for fd=d errno=d", fd, efs_errno, 0);
    return FALSE;
  }
  /*lint -e737*/
  else if (fs_res != nbyte)
  {
    DS_3GPP_ERROR_LOG_3_EX(subs_id, "efs_read read wrong num of bytes=d errno=d", fs_res, efs_errno, 0);
    return FALSE;
  }
  /*lint +e737*/
  return TRUE;
...
)
{
  ...
  {
    DS_3GPP_ERROR_LOG_3_EX(subs_id, "Invalid arguments passed.", 0, 0, 0);
    return FALSE;
  }
...
-----------------------------------------------------------------------------*/
       ...
       buffer_index++)
  {
         ...
         *( buffer + buffer_index ) == ',')
    {
      ...
      {
        DS_3GPP_ERROR_LOG_3_EX(subs_id, "Profile number is invalid"
  ...
  {
    DS_3GPP_ERROR_LOG_3_EX(subs_id, "Invalid EFS contents. num_profiles = d, prof_num_count = d",
...
)
{
  ...
  if ( fs_handle == -1)
  {
    DS_3GPP_ERROR_LOG_3_EX(subs_id, "efs_open failed for d errno=d.", fs_handle, efs_errno, 0);
    return FALSE;
  }
  ...
  {
    DS_3GPP_ERROR_LOG_3_EX(subs_id, "efs_close failed for d errno = d", fs_handle, efs_errno, 0);
    return FALSE;
  }
...
static boolean dsumts_subs_mgr_create_and_init_mapping_info_for_embedded( void )
{
                       ...
                       DS_UMTS_EMBEDDED_PROFILE_FAMILY)) )
  {
    ...
    if ( fs_handle == -1)
    {
      DS_3GPP_ERROR_LOG_3_EX(subs_id, "efs_open failed for fd=d errno=d.", fs_handle, efs_errno, 0);
      return FALSE;
    }
    ...
    if (efs_close (fs_handle) == -1)
    {
      DS_3GPP_ERROR_LOG_3_EX(subs_id, "efs_close failed for fd=d errno=d", fs_handle, efs_errno, 0);
      return FALSE;
    }
#endif
...
DESCRIPTION
  ...
  SYS_MODEM_AS_ID_1 : DS_PROFILE_ACTIVE_SUBSCRIPTION_1
  SYS_MODEM_AS_ID_2 : DS_PROFILE_ACTIVE_SUBSCRIPTION_2
  SYS_MODEM_AS_ID_3 : DS_PROFILE_ACTIVE_SUBSCRIPTION_3
  other             : DS_PROFILE_ACTIVE_SUBSCRIPTION_NONE
...
===========================================================================*/
ds_profile_subs_etype dsumts_subs_mgr_get_profile_subs_id
(
...
)
{
  switch(as_id) {
    ...
    case SYS_MODEM_AS_ID_3:
     return DS_PROFILE_ACTIVE_SUBSCRIPTION_3;
#endif

    default:
      return DS_PROFILE_ACTIVE_SUBSCRIPTION_NONE;
  }
}/*  dsumts_subs_mgr_get_profile_subs_id */
...
===========================================================================*/
ds_profile_subs_etype dsumts_subs_mgr_get_ds_profile_subs_id
(
...
FUNCTION DSUMTS_SUBS_MGR_GET_SUBS_ID_FROM_POLICY_SUBS_ID
...
)
{
  ...
  if(as_id == SYS_MODEM_AS_ID_NONE) {
    DS_3GPP_ERROR_LOG_3_EX(subs_id, "Active subscription is None",0,0,0);
    return;
  }
  ...
  if (result == DS_UMTS_PDP_SUCCESS) {
    ...
    DS_3GPP_ERROR_LOG_3_EX(subs_id, "Could not get default profile error: d",result,0,0);
    return;
  }
...
)
{
  ...
  {
    DS_3GPP_ERROR_LOG_3_EX(subs_id, "Active subscription ID is invalid",0,0,0);
    return DS_UMTS_PDP_FAIL;
  }
  ...
  if ( fs_handle == -1)
  {
    DS_3GPP_ERROR_LOG_3_EX(subs_id, "efs_open failed fd=d errno=d.", fs_handle, efs_errno, 0);
    return DS_UMTS_PDP_ACCESS_ERROR;
  }
 /*--------------------------------------------------------------------------
  ...
  if (file_size == -1)
  {
    DS_3GPP_ERROR_LOG_3_EX(subs_id,  "efs_lseek failed for d errno=d", fs_handle, efs_errno, 0 );
    return DS_UMTS_PDP_ACCESS_ERROR;
  }
 ...
 /*--------------------------------------------------------------------------
  ...
  {
    DS_3GPP_ERROR_LOG_3_EX(subs_id,  "config :file_pos/struct_size mismatch ",
               file_size, 0, 0 );
  ...
  {
    DS_3GPP_ERROR_LOG_3_EX(subs_id,  "config :fs_truncate failed fd=d errno=",
...
)
{
  ...
  if (NULL == profile_num)
  {
    DS_3GPP_ERROR_LOG_3_EX(subs_id, "Invalid profile_num ptr", 0,0,0);
    return DS_UMTS_PDP_FAIL;
  }
  ...
  if(dsumts_subs_mgr_cache_value_valid[DSUMTS_SUBS_MGR_EMBEDDED_FAMILY_INDEX]
    ...
    DS_3GPP_ERROR_FATAL("Cache values not valid");
  }
  return DS_UMTS_PDP_SUCCESS;
}/* dsumts_subs_mgr_get_default_profile_num_per_subs_for_embedded */
...
)
{
  ...
  if(dsumts_subs_mgr_cache_value_valid[DSUMTS_SUBS_MGR_TETHERED_FAMILY_INDEX] == TRUE)
    ...
    DS_3GPP_ERROR_FATAL(" Tethered cache value is not valid");
  }
  return DS_UMTS_PDP_SUCCESS;
}/* dsumts_subs_mgr_get_default_profile_num_per_subs_for_tethered */
