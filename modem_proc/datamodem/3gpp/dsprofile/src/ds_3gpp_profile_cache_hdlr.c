...
---------------------------------------------------------------*/
/* These profile-counters are per device not per subs */
static uint8 transient_profile_cnt  = 0;
static uint8 persistent_profile_cnt = 0;

/*===========================================================================
FUNCTION DS_3GPP_PROFILE_CACHE_CHECK_UMTS_QOS_PARAMS_WITH_DB
...
SIDE EFFECTS
  None
===========================================================================*/
ds_3gpp_profile_cache_update_enum_type ds_3gpp_profile_cache_check_umts_qos_params_with_db
(
...
/*===========================================================================
FUNCTION DS_3GPP_PROFILE_CACHE_GET_ROAMING_PDP_TYPE

DESCRIPTION
  The function converts PDP type to Roaming PDP type
...
===========================================================================*/
ds_profile_db_3gpp_roaming_pdp_protocol
  ds_3gpp_profile_cache_get_roaming_pdp_type
(
  ds_umts_pdp_type_enum_type pdp_type
)
{
  ds_profile_db_3gpp_roaming_pdp_protocol  roaming_pdp_type;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

  switch( pdp_type )
  {
    case DS_UMTS_PDP_IPV4:
      roaming_pdp_type = DB_3GPP_ROAMING_IPV4;
      break;
    case DS_UMTS_PDP_IPV6:
      roaming_pdp_type = DB_3GPP_ROAMING_IPV6;
      break;
    case DS_UMTS_PDP_IPV4V6:
      roaming_pdp_type = DB_3GPP_ROAMING_IPV4V6;
      break;
    default:
      roaming_pdp_type = DB_3GPP_ROAMING_IP_MAX;
      break;
  }
  return roaming_pdp_type;
}
...
/*===========================================================================
FUNCTION DS_3GPP_PROFILE_CACHE_GET_PDP_TYPE_FRM_ROAMING_PDP

DESCRIPTION
  The function converts Roaming PDP type to PDP type
...
===========================================================================*/
ds_umts_pdp_type_enum_type
  ds_3gpp_profile_cache_get_pdp_type_frm_roaming_pdp
(
  ds_profile_db_3gpp_roaming_pdp_protocol  roaming_pdp_type
)
{
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_3GPP_ERROR_LOG_1("Profile number:d invalid",profile_number);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_3GPP_ERROR_LOG_1_EX(subs_id, "cache_get_umts_pdp_profile_ptr: internal err status d",
                       local_err);
  }
  return FALSE;
} /*ds_3gpp_profile_cache_get_umts_pdp_profile_ptr*/
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if (ds_umts_pdp_profile_ptr == NULL)
  {
    DS_3GPP_ERROR_LOG_0_EX(subs_id, "ds_umts_pdp_profile_ptr is NULL");
    return FALSE;
  }
  ...
  *ds_umts_pdp_profile_ptr =
  ...
  if (*ds_umts_pdp_profile_ptr == NULL)
  {
    DS_3GPP_ERROR_LOG_0_EX(subs_id, "modem_mem_alloc failed to allocate memory for new profile");
    return FALSE;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_3GPP_MSG1_HIGH("get_free_cache_index: free cache index: %d", cache_index);
  ...
  return cache_index;
} /*ds_3gpp_profile_cache_get_free_cache_index*/
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  do
  {
    ...
    return ds_umts_pdp_profile_ptr->profile_number;
  } while (0);

  if(local_err != DS_3GPPI_LOCAL_ERR_VAL_NONE)
  {
    DS_3GPP_ERROR_LOG_1_EX(subs_id, "get_cache_profile_num_from_index: internal err status: d",
                       local_err);
  }
  return -1;
} /*ds_3gpp_profile_cache_get_cache_profile_num_from_index*/
...
===========================================================================*/
ds_3gpp_profile_cache_update_enum_type ds_3gpp_profile_cache_write_mnc
(
  ds_profile_db_profile_num_type             profile_number,
  const ds_profile_db_3gpp_tech_params_type *ds_3gpp_tech_params_ptr,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    return DS_3GPP_PROFILE_CACHE_UPDATE_WITH_ERROR;
  }
  ...
  if (tech_mask != EPC_PROFILE_TECH_MASK)
  {
    ...
    if (memcmp((void *)&ds_umts_pdp_profile_ptr->mnc,
            (void *)&ds_3gpp_tech_params_ptr->tech_common_params.mnc,
            sizeof(ds_umts_3gpp_mnc_type)) == 0)
    {
      return DS_3GPP_PROFILE_CACHE_UPDATE_WITH_NO_CHANGE;
    }
  ...
  return DS_3GPP_PROFILE_CACHE_UPDATE_SUCCESS;
}
...
===========================================================================*/
ds_3gpp_profile_cache_update_enum_type ds_3gpp_profile_cache_write_mcc
(
  ds_profile_db_profile_num_type             profile_number,
  const ds_profile_db_3gpp_tech_params_type *ds_3gpp_tech_params_ptr,
  const ds_profile_db_gen_params_type       *gen_params,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (tech_mask != EPC_PROFILE_TECH_MASK)
  {
    ...
    if (memcmp((void *)&ds_umts_pdp_profile_ptr->mcc,
...
===========================================================================*/
ds_3gpp_profile_cache_update_enum_type ds_3gpp_profile_cache_write_apn_bearer
(
  ds_profile_db_profile_num_type             profile_number,
  const ds_profile_db_3gpp_tech_params_type *ds_3gpp_tech_params_ptr,
  const ds_profile_db_gen_params_type       *gen_params,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (memcmp((void *)&ds_umts_pdp_profile_ptr->apn_bearer,
...
===========================================================================*/
ds_3gpp_profile_cache_update_enum_type ds_3gpp_profile_cache_write_src_stat_desc_min
(
  ds_profile_db_profile_num_type             profile_number,
  const ds_profile_db_3gpp_tech_params_type *ds_3gpp_tech_params_ptr,
  const ds_profile_db_gen_params_type       *gen_params,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (memcmp((void *)&ds_umts_pdp_profile_ptr->src_stat_desc_min,
...
===========================================================================*/
ds_3gpp_profile_cache_update_enum_type ds_3gpp_profile_cache_write_src_stat_desc_req
(
  ds_profile_db_profile_num_type             profile_number,
  const ds_profile_db_3gpp_tech_params_type *ds_3gpp_tech_params_ptr,
  const ds_profile_db_gen_params_type       *gen_params,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (memcmp((void *)&ds_umts_pdp_profile_ptr->src_stat_desc_req,
...
===========================================================================*/
ds_3gpp_profile_cache_update_enum_type ds_3gpp_profile_cache_write_apn_class
(
  ds_profile_db_profile_num_type             profile_number,
  const ds_profile_db_3gpp_tech_params_type *ds_3gpp_tech_params_ptr,
  const ds_profile_db_gen_params_type       *gen_params,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(tech_mask != EPC_PROFILE_TECH_MASK)
  {
    ...
    if(memcmp((void *)&ds_umts_pdp_profile_ptr->apn_class,
...
===========================================================================*/
ds_3gpp_profile_cache_update_enum_type ds_3gpp_profile_cache_write_inactivity_timer_val
(
  ds_profile_db_profile_num_type             profile_number,
  const ds_profile_db_3gpp_tech_params_type *ds_3gpp_tech_params_ptr,
  const ds_profile_db_gen_params_type       *gen_params,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (memcmp((void *)&ds_umts_pdp_profile_ptr->inactivity_timer_val,
...
===========================================================================*/
ds_3gpp_profile_cache_update_enum_type ds_3gpp_profile_cache_write_operartor_reserved_pco
(
  ds_profile_db_profile_num_type             profile_number,
  const ds_profile_db_3gpp_tech_params_type *ds_3gpp_tech_params_ptr,
  const ds_profile_db_gen_params_type       *gen_params,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (tech_mask != EPC_PROFILE_TECH_MASK)
  {
     ...
     if (memcmp((void *)&ds_umts_pdp_profile_ptr->operator_reserved_pco,
...
===========================================================================*/
ds_3gpp_profile_cache_update_enum_type ds_3gpp_profile_cache_write_apn_disable_flag
(
  ds_profile_db_profile_num_type             profile_number,
  const ds_profile_db_3gpp_tech_params_type *ds_3gpp_tech_params_ptr,
  const ds_profile_db_gen_params_type       *gen_params,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (tech_mask != EPC_PROFILE_TECH_MASK)
  {
    ...
    if (memcmp((void *)&ds_umts_pdp_profile_ptr->apn_disable_flag,
...
/*===========================================================================
FUNCTION DS_3GPP_PROFILE_CACHE_WRITE_PDN_LABEL

DESCRIPTION
  The function updates the PDN_LABEL value to cache
...
===========================================================================*/
ds_3gpp_profile_cache_update_enum_type ds_3gpp_profile_cache_write_pdn_label
(
  ds_profile_db_profile_num_type             profile_number,
  const ds_profile_db_3gpp_tech_params_type *ds_3gpp_tech_params_ptr,
  const ds_profile_db_gen_params_type       *gen_params,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (strncasecmp((char *)ds_umts_pdp_profile_ptr->pdn_label,
          (char *)ds_3gpp_tech_params_ptr->pdn_label_3gpp,
          sizeof(ds_3gpp_tech_params_ptr->pdn_label_3gpp)) == 0)
  {
    return DS_3GPP_PROFILE_CACHE_UPDATE_WITH_NO_CHANGE;
  }
    /*-------------------------------------------------------------------
    Reset the previous pdn label before updating new profile pdn label.
  -------------------------------------------------------------------*/
  memset((void *)ds_umts_pdp_profile_ptr->pdn_label,
         '\0',
         DS_3GPP_MAX_PDN_LABLE_LENTH+1);

  strlcpy((char *)ds_umts_pdp_profile_ptr->pdn_label,
          (char *)ds_3gpp_tech_params_ptr->pdn_label_3gpp,
          sizeof(ds_3gpp_tech_params_ptr->pdn_label_3gpp));
...
===========================================================================*/
ds_3gpp_profile_cache_update_enum_type ds_3gpp_profile_cache_write_im_cn_flag
(
  ds_profile_db_profile_num_type             profile_number,
  const ds_profile_db_3gpp_tech_params_type *ds_3gpp_tech_params_ptr,
  const ds_profile_db_gen_params_type       *gen_params,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(memcmp((void *)&ds_umts_pdp_profile_ptr->im_cn_flag,
...
===========================================================================*/
ds_3gpp_profile_cache_update_enum_type ds_3gpp_profile_cache_write_request_pcscf_address_using_dhcp_flag
(
  ds_profile_db_profile_num_type             profile_number,
  const ds_profile_db_3gpp_tech_params_type *ds_3gpp_tech_params_ptr,
  const ds_profile_db_gen_params_type       *gen_params,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (tech_mask != EPC_PROFILE_TECH_MASK)
  {
    ...
    if(memcmp((void *)&ds_umts_pdp_profile_ptr->request_pcscf_address_using_dhcp_flag,
            (void *)&ds_3gpp_tech_params_ptr->tech_common_params.
            request_pcscf_address_using_dhcp_flag, sizeof(boolean)) == 0)
    {
      return DS_3GPP_PROFILE_CACHE_UPDATE_WITH_NO_CHANGE;
    }
...
===========================================================================*/
ds_3gpp_profile_cache_update_enum_type ds_3gpp_profile_cache_write_request_dns_address_using_dhcp_flag
(
  ds_profile_db_profile_num_type             profile_number,
  const ds_profile_db_3gpp_tech_params_type *ds_3gpp_tech_params_ptr,
  const ds_profile_db_gen_params_type       *gen_params,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (tech_mask != EPC_PROFILE_TECH_MASK)
  {
    ...
    if (memcmp((void *)&ds_umts_pdp_profile_ptr->request_dns_address_using_dhcp_flag,
            (void *)&ds_3gpp_tech_params_ptr->tech_common_params.
            request_dns_address_using_dhcp_flag, sizeof(boolean)) == 0)
    {
      return DS_3GPP_PROFILE_CACHE_UPDATE_WITH_NO_CHANGE;
    }
...
===========================================================================*/
ds_3gpp_profile_cache_update_enum_type ds_3gpp_profile_cache_write_request_pcscf_address_flag
(
  ds_profile_db_profile_num_type             profile_number,
  const ds_profile_db_3gpp_tech_params_type *ds_3gpp_tech_params_ptr,
  const ds_profile_db_gen_params_type       *gen_params,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (tech_mask != EPC_PROFILE_TECH_MASK)
  {
    ...
    if(memcmp( (void *)&ds_umts_pdp_profile_ptr->request_pcscf_address_flag,
            (void *)&ds_3gpp_tech_params_ptr->tech_common_params.
            request_pcscf_address_flag,sizeof(boolean)) == 0)
    {
      return DS_3GPP_PROFILE_CACHE_UPDATE_WITH_NO_CHANGE;
    }
...
===========================================================================*/
ds_3gpp_profile_cache_update_enum_type ds_3gpp_profile_cache_write_auth_type
(
  ds_profile_db_profile_num_type             profile_number,
  const ds_profile_db_3gpp_tech_params_type *ds_3gpp_tech_params_ptr,
  const ds_profile_db_gen_params_type       *gen_params,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (tech_mask != EPC_PROFILE_TECH_MASK)
  {
    ...
    if(memcmp((void*)&ds_umts_pdp_profile_ptr->auth.auth_type,
             (void *)&ds_3gpp_tech_params_ptr->
             tech_common_params.auth_info.auth_type,
             sizeof(ds_profile_db_auth_protocol)) == 0)
    {
      return DS_3GPP_PROFILE_CACHE_UPDATE_WITH_NO_CHANGE;
    }
...
===========================================================================*/
ds_3gpp_profile_cache_update_enum_type ds_3gpp_profile_cache_write_auth_password
(
  ds_profile_db_profile_num_type             profile_number,
  const ds_profile_db_3gpp_tech_params_type *ds_3gpp_tech_params_ptr,
  const ds_profile_db_gen_params_type       *gen_params,
...
)
{
  ...
  boolean                    case_sensitive = 
                                 ds_3gpp_nv_manager_ex_get_case_sensitive(as_subs_id);
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if( !ds_3gpp_profile_cache_get_umts_pdp_profile_ptr(profile_number,
                                                      ds_3gpp_tech_params_ptr,
                                                      gen_params,
  ...
  as_subs_id = dsumts_subs_mgr_get_as_id(
                 dsumts_subs_mgr_get_subs_id_from_db_subs_id(
  ...
  if (tech_mask != EPC_PROFILE_TECH_MASK)
  {
    ...
    if ((!case_sensitive &&
          (strncasecmp( (char *) ds_umts_pdp_profile_ptr->auth.password,
             (char *)ds_3gpp_tech_params_ptr->tech_common_params.auth_info.password,
               sizeof(ds_umts_pdp_profile_ptr->auth.password)) == 0)) ||
          (case_sensitive &&
          (strcmp( (char *) ds_umts_pdp_profile_ptr->auth.password,
    ...
    memset((void *)ds_umts_pdp_profile_ptr->auth.password,0,
...
===========================================================================*/
ds_3gpp_profile_cache_update_enum_type ds_3gpp_profile_cache_write_auth_username
(
  ds_profile_db_profile_num_type             profile_number,
  const ds_profile_db_3gpp_tech_params_type *ds_3gpp_tech_params_ptr,
  const ds_profile_db_gen_params_type       *gen_params,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (tech_mask != EPC_PROFILE_TECH_MASK)
  {
    ...
    if ((!case_sensitive &&
        (strncasecmp( (char *) ds_umts_pdp_profile_ptr->auth.username,
             (char *)ds_3gpp_tech_params_ptr->tech_common_params.
             auth_info.username,
             sizeof(ds_umts_pdp_profile_ptr->auth.username)) == 0)) ||
        (case_sensitive &&
        (strcmp( (char *) ds_umts_pdp_profile_ptr->auth.username,
             (char *)ds_3gpp_tech_params_ptr->tech_common_params.
...
===========================================================================*/
ds_3gpp_profile_cache_update_enum_type ds_3gpp_profile_cache_write_umts_qos_req
(
  ds_profile_db_profile_num_type             profile_number,
  const ds_profile_db_3gpp_tech_params_type *ds_3gpp_tech_params_ptr,
  const ds_profile_db_gen_params_type       *gen_params,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if((ds_3gpp_profile_cache_check_umts_qos_params_with_db(
    &ds_umts_pdp_profile_ptr->qos_request_umts,&ds_3gpp_tech_params_ptr->qos_request_umts,
    is_extended) == DS_3GPP_PROFILE_CACHE_UPDATE_WITH_NO_CHANGE) &&
...
===========================================================================*/
ds_3gpp_profile_cache_update_enum_type ds_3gpp_profile_cache_write_umts_qos_req_extended
(
  ds_profile_db_profile_num_type             profile_number,
  const ds_profile_db_3gpp_tech_params_type *ds_3gpp_tech_params_ptr,
  const ds_profile_db_gen_params_type       *gen_params,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  memset((void *)&ds_umts_pdp_profile_ptr->qos_request_umts, 0,
         sizeof(ds_umts_umts_qos_params_type) - sizeof(boolean));

  ds_umts_pdp_profile_ptr->qos_request_umts.valid_flg =
    ds_3gpp_tech_params_ptr->is_valid.qos_req_umts;

  ds_3gpp_profile_cache_copy_umts_qos_params_from_db(
...
===========================================================================*/
ds_3gpp_profile_cache_update_enum_type ds_3gpp_profile_cache_write_umts_qos_min
(
  ds_profile_db_profile_num_type             profile_number,
  const ds_profile_db_3gpp_tech_params_type *ds_3gpp_tech_params_ptr,
  const ds_profile_db_gen_params_type       *gen_params,
...
===========================================================================*/
ds_3gpp_profile_cache_update_enum_type ds_3gpp_profile_cache_write_umts_qos_min_extended
(
  ds_profile_db_profile_num_type             profile_number,
  const ds_profile_db_3gpp_tech_params_type *ds_3gpp_tech_params_ptr,
  const ds_profile_db_gen_params_type       *gen_params,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if((ds_3gpp_profile_cache_check_umts_qos_params_with_db(&ds_umts_pdp_profile_ptr
    ->qos_minimum_umts,&ds_3gpp_tech_params_ptr->qos_minimum_umts, is_extended) ==
  ...
  memset((void *)&ds_umts_pdp_profile_ptr->qos_minimum_umts, 0,
         sizeof(ds_umts_umts_qos_params_type) - sizeof(boolean));
...
{
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if (ds_umts_gprs_qos_param == NULL || db_gprs_qos_param == NULL)
  {
    DS_3GPP_ERROR_LOG_2("UMTS gprs qos param d or Profile DB GPRS param d is NULL",
                       ds_umts_gprs_qos_param, db_gprs_qos_param);
    return;
  }
...
/*===========================================================================
FUNCTION DS_3GPP_PROFILE_CACHE_CHECK_GPRS_QOS_PARAMS_WITH_DB

DESCRIPTION
  The function to check if profile parameter can be updated or not
...
RETURN VALUE
 ds_3gpp_profile_cache_update_enum_type
 DS_3GPP_PROFILE_CACHE_UPDATE_SUCCESS - if param value can be updated successfully
 DS_3GPP_PROFILE_CACHE_UPDATE_WITH_NO_CHANGE - if param new value is same as old param value
 DS_3GPP_PROFILE_CACHE_UPDATE_WITH_ERROR - if something else is wrong with the call
...
===========================================================================*/
ds_3gpp_profile_cache_update_enum_type ds_3gpp_profile_cache_check_gprs_qos_params_with_db
(
  ds_umts_gprs_qos_params_type                       *ds_umts_gprs_qos_param,
  const ds_profile_db_3gpp_umts_gprs_qos_params_type *db_gprs_qos_param
)
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(memcmp((void *)&ds_umts_gprs_qos_param->precedence,
          (void *)&db_gprs_qos_param->precedence,
          sizeof(uint32)) != 0)
  {
    return DS_3GPP_PROFILE_CACHE_UPDATE_SUCCESS;
  }

  if(memcmp((void *)&ds_umts_gprs_qos_param->delay,
          (void *)&db_gprs_qos_param->delay,
          sizeof(uint32)) != 0)
  {
    return DS_3GPP_PROFILE_CACHE_UPDATE_SUCCESS;
  }

  if(memcmp((void *)&ds_umts_gprs_qos_param->reliability,
          (void *)&db_gprs_qos_param->reliability,
          sizeof(uint32)) != 0)
  {
    return DS_3GPP_PROFILE_CACHE_UPDATE_SUCCESS;
  }

  if(memcmp((void *)&ds_umts_gprs_qos_param->peak,
          (void *)&db_gprs_qos_param->peak,
          sizeof(uint32)) != 0)
  {
    return DS_3GPP_PROFILE_CACHE_UPDATE_SUCCESS;
  }

  if(memcmp((void *)&ds_umts_gprs_qos_param->mean,
          (void *)&db_gprs_qos_param->mean,
          sizeof(uint32)) != 0)
  {
    return DS_3GPP_PROFILE_CACHE_UPDATE_SUCCESS;
  }
...
{
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if (ds_umts_qos_param == NULL || db_umts_qos_param == NULL)
  {
    DS_3GPP_ERROR_LOG_2("UMTS gprs qos param d or Profile DB GPRS param d is NULL",
                       ds_umts_qos_param, db_umts_qos_param);
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if (ds_umts_qos_param == NULL || db_umts_qos_param == NULL)
  {
    DS_3GPP_ERROR_LOG_2("UMTS gprs qos param d or Profile DB umts param d is NULL",
                       ds_umts_qos_param, db_umts_qos_param);
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(memcmp((void *)&ds_umts_qos_param->traffic_class,
          (void *)&db_umts_qos_param->traffic_class,
          sizeof(ds_umts_qos_tclass_enum_type)) != 0)
  {
    return DS_3GPP_PROFILE_CACHE_UPDATE_SUCCESS;
  }

  if(memcmp((void *)&ds_umts_qos_param->max_ul_bitrate,
          (void *)&db_umts_qos_param->max_ul_bitrate,
          sizeof(uint32)) != 0)
  {
    return DS_3GPP_PROFILE_CACHE_UPDATE_SUCCESS;
  }

  if(memcmp((void *)&ds_umts_qos_param->max_dl_bitrate,
          (void *)&db_umts_qos_param->max_dl_bitrate,
          sizeof(uint32)) != 0)
  {
    return DS_3GPP_PROFILE_CACHE_UPDATE_SUCCESS;
  }

  if(memcmp((void *)&ds_umts_qos_param->gtd_ul_bitrate,
          (void *)&db_umts_qos_param->gtd_ul_bitrate,
          sizeof(uint32)) != 0)
  {
    return DS_3GPP_PROFILE_CACHE_UPDATE_SUCCESS;
  }

  if(memcmp((void *)&ds_umts_qos_param->gtd_dl_bitrate,
          (void *)&db_umts_qos_param->gtd_dl_bitrate,
          sizeof(uint32)) != 0)
  {
    return DS_3GPP_PROFILE_CACHE_UPDATE_SUCCESS;
  }

  if(memcmp((void *)&ds_umts_qos_param->dlvry_order,
          (void *)&db_umts_qos_param->dlvry_order,
          sizeof(ds_umts_qos_dorder_enum_type)) != 0)
  {
    return DS_3GPP_PROFILE_CACHE_UPDATE_SUCCESS;
  }

  if(memcmp((void *)&ds_umts_qos_param->max_sdu_size,
          (void *)&db_umts_qos_param->max_sdu_size,
          sizeof(uint32)) != 0)
  {
    return DS_3GPP_PROFILE_CACHE_UPDATE_SUCCESS;
  }

  if(memcmp((void *)&ds_umts_qos_param->sdu_err,
          (void *)&db_umts_qos_param->sdu_err,
          sizeof(ds_umts_qos_sdu_error_enum_type)) != 0)
  {
    return DS_3GPP_PROFILE_CACHE_UPDATE_SUCCESS;
  }

  if(memcmp((void *)&ds_umts_qos_param->res_biterr,
          (void *)&db_umts_qos_param->res_biterr,
          sizeof(ds_umts_qos_res_ber_enum_type)) != 0)
  {
    return DS_3GPP_PROFILE_CACHE_UPDATE_SUCCESS;
  }

  if(memcmp((void *)&ds_umts_qos_param->dlvr_err_sdu,
          (void *)&db_umts_qos_param->dlvr_err_sdu,
          sizeof(ds_umts_qos_sdu_dlvr_enum_type)) != 0)
  {
    return DS_3GPP_PROFILE_CACHE_UPDATE_SUCCESS;
  }

  if(memcmp((void *)&ds_umts_qos_param->trans_delay,
          (void *)&db_umts_qos_param->trans_delay,
          sizeof(uint32)) != 0)
  {
    return DS_3GPP_PROFILE_CACHE_UPDATE_SUCCESS;
  ...
  if(memcmp((void *)&ds_umts_qos_param->thandle_prio,
          (void *)&db_umts_qos_param->thandle_prio,
          sizeof(uint32)) != 0)
  {
    return DS_3GPP_PROFILE_CACHE_UPDATE_SUCCESS;
  ...
  if (!is_extended && (memcmp((void *)&ds_umts_qos_param->sig_ind,
          (void *)&db_umts_qos_param->sig_ind,
          sizeof(boolean)) != 0))
  {
    return DS_3GPP_PROFILE_CACHE_UPDATE_SUCCESS;
...
{
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if (ds_umts_pdp_profile_ptr == NULL || ds_3gpp_tech_params_ptr == NULL)
  {
    DS_3GPP_ERROR_LOG_2("Cache ptr d or Tech params ptr d is NULL",
...
/*===========================================================================
FUNCTION DS_3GPP_PROFILE_CACHE_CHECK_LTE_QOS_PARAMS_WITH_DB
...
===========================================================================*/
ds_3gpp_profile_cache_update_enum_type ds_3gpp_profile_cache_check_lte_qos_params_with_db
(
  ds_umts_pdp_profile_type                  *ds_umts_pdp_profile_ptr,
  const ds_profile_db_3gpp_tech_params_type *ds_3gpp_tech_params_ptr
)
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(ds_umts_pdp_profile_ptr->qos_request_lte.valid_flg !=
    ds_3gpp_tech_params_ptr->is_valid.qos_req_lte)
  {
    return DS_3GPP_PROFILE_CACHE_UPDATE_SUCCESS;
  }

  if(memcmp((void *)&ds_umts_pdp_profile_ptr->qos_request_lte.qci,
          (void *)&ds_3gpp_tech_params_ptr->qos_request_lte.qci,
          sizeof(ds_3gpp_lte_qci_e_type)) != 0)
  {
    return DS_3GPP_PROFILE_CACHE_UPDATE_SUCCESS;
  }

  if(memcmp((void *)&ds_umts_pdp_profile_ptr->qos_request_lte.g_dl_bit_rate,
          (void *)&ds_3gpp_tech_params_ptr->qos_request_lte.g_dl_bit_rate,
          sizeof(uint32)) != 0)
  {
    return DS_3GPP_PROFILE_CACHE_UPDATE_SUCCESS;
  ...
  if(memcmp((void *)&ds_umts_pdp_profile_ptr->qos_request_lte.max_dl_bit_rate,
          (void *)&ds_3gpp_tech_params_ptr->qos_request_lte.max_dl_bit_rate,
          sizeof(uint32)) != 0)
  {
    return DS_3GPP_PROFILE_CACHE_UPDATE_SUCCESS;
  ...
  if(memcmp((void *)&ds_umts_pdp_profile_ptr->qos_request_lte.g_ul_bit_rate,
          (void *)&ds_3gpp_tech_params_ptr->qos_request_lte.g_ul_bit_rate,
          sizeof(uint32)) != 0)
  {
    return DS_3GPP_PROFILE_CACHE_UPDATE_SUCCESS;
  ...
  if(memcmp((void *)&ds_umts_pdp_profile_ptr->qos_request_lte.max_ul_bit_rate,
          (void *)&ds_3gpp_tech_params_ptr->qos_request_lte.max_ul_bit_rate,
          sizeof(uint32)) != 0)
  {
    return DS_3GPP_PROFILE_CACHE_UPDATE_SUCCESS;
...
===========================================================================*/
ds_3gpp_profile_cache_update_enum_type ds_3gpp_profile_cache_write_lte_qos_req
(
  ds_profile_db_profile_num_type             profile_number,
  const ds_profile_db_3gpp_tech_params_type *ds_3gpp_tech_params_ptr,
  const ds_profile_db_gen_params_type       *gen_params,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (ds_3gpp_profile_cache_check_lte_qos_params_with_db(ds_umts_pdp_profile_ptr,
     ds_3gpp_tech_params_ptr) == DS_3GPP_PROFILE_CACHE_UPDATE_WITH_NO_CHANGE)
  {
    return DS_3GPP_PROFILE_CACHE_UPDATE_WITH_NO_CHANGE;
  }
...
===========================================================================*/
ds_3gpp_profile_cache_update_enum_type ds_3gpp_profile_cache_write_gprs_qos_req
(
  ds_profile_db_profile_num_type             profile_number,
  const ds_profile_db_3gpp_tech_params_type *ds_3gpp_tech_params_ptr,
  const ds_profile_db_gen_params_type       *gen_params,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (ds_3gpp_profile_cache_check_gprs_qos_params_with_db(&ds_umts_pdp_profile_ptr
      ->qos_request_gprs,&ds_3gpp_tech_params_ptr->qos_request_gprs) ==
      DS_3GPP_PROFILE_CACHE_UPDATE_WITH_NO_CHANGE && (ds_umts_pdp_profile_ptr->qos_request_gprs.valid_flg ==
      ds_3gpp_tech_params_ptr->is_valid.qos_req_gprs))
  {
    return DS_3GPP_PROFILE_CACHE_UPDATE_WITH_NO_CHANGE;
  }

  memset((void *)&ds_umts_pdp_profile_ptr->qos_request_gprs, 0,
...
===========================================================================*/
ds_3gpp_profile_cache_update_enum_type ds_3gpp_profile_cache_write_gprs_qos_min
(
  ds_profile_db_profile_num_type             profile_number,
  const ds_profile_db_3gpp_tech_params_type *ds_3gpp_tech_params_ptr,
  const ds_profile_db_gen_params_type       *gen_params,
...
===========================================================================*/
ds_3gpp_profile_cache_update_enum_type ds_3gpp_profile_cache_write_subscription_id
(
  ds_profile_db_profile_num_type             profile_number,
  const ds_profile_db_3gpp_tech_params_type *ds_3gpp_tech_params_ptr,
  const ds_profile_db_gen_params_type       *gen_params,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (memcmp((void *)&ds_umts_pdp_profile_ptr->subscription_id,
     (void *)&gen_params->subscription_id,
        sizeof(ds_profile_db_subs_type))==0)
  {
    return DS_3GPP_PROFILE_CACHE_UPDATE_WITH_NO_CHANGE;
  }
...
ds_3gpp_profile_cache_update_enum_type ds_3gpp_profile_cache_write_profile_name
(
  ds_profile_db_profile_num_type             profile_number,
  const ds_profile_db_3gpp_tech_params_type *ds_3gpp_tech_params_ptr,
  const ds_profile_db_gen_params_type       *gen_params,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (strncasecmp((char *)ds_umts_pdp_profile_ptr->profile_name,(char *)
          gen_params->profile_name,
          sizeof(gen_params->profile_name)) == 0)
  {
    return DS_3GPP_PROFILE_CACHE_UPDATE_WITH_NO_CHANGE;
  }
    /*-------------------------------------------------------------------
...
===========================================================================*/
ds_3gpp_profile_cache_update_enum_type ds_3gpp_profile_cache_write_pdp_type
(
  ds_profile_db_profile_num_type             profile_number,
  const ds_profile_db_3gpp_tech_params_type *ds_3gpp_tech_params_ptr,
  const ds_profile_db_gen_params_type       *gen_params,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (tech_mask != EPC_PROFILE_TECH_MASK)
  {
    ...
    if (memcmp((void *)&ds_umts_pdp_profile_ptr->context.pdp_type,
                (void *)&ds_3gpp_tech_params_ptr->pdp_context.pdp_type,
                sizeof(ds_umts_pdp_type_enum_type)) == 0)
    {
      return DS_3GPP_PROFILE_CACHE_UPDATE_WITH_NO_CHANGE;
    }
  ...
  {
    ds_umts_pdp_profile_ptr->context.valid_flg = TRUE;
    if ((ds_3gpp_tech_params_ptr->tech_common_params.pdp_type == DB_PDP_TYPE_IPV4 &&
        ds_umts_pdp_profile_ptr->context.pdp_type == DS_UMTS_PDP_IPV4 ) ||
        (ds_3gpp_tech_params_ptr->tech_common_params.pdp_type == DB_PDP_TYPE_IPV4V6 &&
        ds_umts_pdp_profile_ptr->context.pdp_type == DS_UMTS_PDP_IPV4V6 ) ||
        (ds_3gpp_tech_params_ptr->tech_common_params.pdp_type == DB_PDP_TYPE_IPV6 &&
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  pdp_type = ds_3gpp_profile_cache_get_pdp_type_frm_roaming_pdp(
               ds_3gpp_tech_params_ptr->lte_pdp_type);

  if (memcmp((void *)&ds_umts_pdp_profile_ptr->context.lte_roaming_pdp_type,
             (void *)&pdp_type,
             sizeof(ds_umts_pdp_type_enum_type)) == 0)
  {
    return DS_3GPP_PROFILE_CACHE_UPDATE_WITH_NO_CHANGE;
  ...
  memset(&(ds_umts_pdp_profile_ptr->context.lte_roaming_pdp_type),
         0,
         sizeof(ds_umts_pdp_type_enum_type));

  ds_umts_pdp_profile_ptr->context.lte_roaming_pdp_type = pdp_type;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  pdp_type = ds_3gpp_profile_cache_get_pdp_type_frm_roaming_pdp(
               ds_3gpp_tech_params_ptr->umts_pdp_type);

  if (memcmp((void *)&ds_umts_pdp_profile_ptr->context.umts_roaming_pdp_type,
             (void *)&pdp_type,
             sizeof(ds_umts_pdp_type_enum_type)) == 0)
  {
    return DS_3GPP_PROFILE_CACHE_UPDATE_WITH_NO_CHANGE;
  ...
  memset(&(ds_umts_pdp_profile_ptr->context.umts_roaming_pdp_type),
         0,
         sizeof(ds_umts_pdp_type_enum_type));

  ds_umts_pdp_profile_ptr->context.umts_roaming_pdp_type = pdp_type;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (memcmp((void *)&ds_umts_pdp_profile_ptr->context.override_home_pdp_type,
             (void *)&pdp_type,
             sizeof(ds_umts_pdp_type_enum_type)) == 0)
  {
    return DS_3GPP_PROFILE_CACHE_UPDATE_WITH_NO_CHANGE;
...
/*===========================================================================
FUNCTION DS_3GPP_PROFILE_CACHE_WRITE_PDP_CONTEXT_TYPE
...
===========================================================================*/
void ds_3gpp_profile_cache_write_pdp_context_type
(
  ds_profile_db_3gpp_tech_params_type *ds_3gpp_tech_params_ptr,
  ds_umts_pdp_profile_type            *ds_umts_pdp_profile_ptr
)
...
===========================================================================*/
ds_3gpp_profile_cache_update_enum_type ds_3gpp_profile_cache_write_pdp_context_apn
(
  ds_profile_db_profile_num_type             profile_number,
  const ds_profile_db_3gpp_tech_params_type *ds_3gpp_tech_params_ptr,
  const ds_profile_db_gen_params_type       *gen_params,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (strncmp((char *)ds_umts_pdp_profile_ptr->context.apn,
          gen_params->apn_name,sizeof(gen_params->apn_name)) == 0)
  {
    return DS_3GPP_PROFILE_CACHE_UPDATE_WITH_NO_CHANGE;
  }
...
===========================================================================*/
ds_3gpp_profile_cache_update_enum_type ds_3gpp_profile_cache_write_pdp_context_ipv4_addr_alloc
(
  ds_profile_db_profile_num_type             profile_number,
  const ds_profile_db_3gpp_tech_params_type *ds_3gpp_tech_params_ptr,
  const ds_profile_db_gen_params_type       *gen_params,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (memcmp((void *)&ds_umts_pdp_profile_ptr->context.ipv4_addr_alloc,
          (void *)&ds_3gpp_tech_params_ptr->pdp_context.ipv4_addr_alloc,
          sizeof(ds_umts_pdp_ipv4_addr_alloc_e_type))==0)
  {
    return DS_3GPP_PROFILE_CACHE_UPDATE_WITH_NO_CHANGE;
  }
...
===========================================================================*/
ds_3gpp_profile_cache_update_enum_type ds_3gpp_profile_cache_write_pdp_context_d_comp
(
  ds_profile_db_profile_num_type             profile_number,
  const ds_profile_db_3gpp_tech_params_type *ds_3gpp_tech_params_ptr,
  const ds_profile_db_gen_params_type       *gen_params,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  ds_umts_pdp_profile_ptr->context.valid_flg = TRUE;
  if (memcmp((void *)&ds_umts_pdp_profile_ptr->context.d_comp,
...
===========================================================================*/
ds_3gpp_profile_cache_update_enum_type ds_3gpp_profile_cache_write_pdp_context_h_comp
(
  ds_profile_db_profile_num_type             profile_number,
  const ds_profile_db_3gpp_tech_params_type *ds_3gpp_tech_params_ptr,
  const ds_profile_db_gen_params_type       *gen_params,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (memcmp((void *)&ds_umts_pdp_profile_ptr->context.h_comp,
                (void *)&ds_3gpp_tech_params_ptr->pdp_context.h_comp,
                sizeof(ds_umts_pdp_header_comp_e_type)) == 0)
  {
    return DS_3GPP_PROFILE_CACHE_UPDATE_WITH_NO_CHANGE;
  }
...
===========================================================================*/
ds_3gpp_profile_cache_update_enum_type ds_3gpp_profile_cache_write_pdp_context_secondary_flag
(
  ds_profile_db_profile_num_type             profile_number,
  const ds_profile_db_3gpp_tech_params_type *ds_3gpp_tech_params_ptr,
  const ds_profile_db_gen_params_type       *gen_params,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (memcmp((void *)&ds_umts_pdp_profile_ptr->context.secondary_flag,
...
===========================================================================*/
ds_3gpp_profile_cache_update_enum_type ds_3gpp_profile_cache_write_pdp_context_number
(
  ds_profile_db_profile_num_type             profile_number,
  const ds_profile_db_3gpp_tech_params_type *ds_3gpp_tech_params_ptr,
  const ds_profile_db_gen_params_type       *gen_params,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  ds_umts_pdp_profile_ptr->context.valid_flg = TRUE;
  if (memcmp((void *)&ds_umts_pdp_profile_ptr->context.pdp_context_number,
...
===========================================================================*/
ds_3gpp_profile_cache_update_enum_type ds_3gpp_profile_cache_write_access_ctrl_flag
(
  ds_profile_db_profile_num_type             profile_number,
  const ds_profile_db_3gpp_tech_params_type *ds_3gpp_tech_params_ptr,
  const ds_profile_db_gen_params_type       *gen_params,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(memcmp((void *)&ds_umts_pdp_profile_ptr->context.access_ctrl_flag,
          (void *)&ds_3gpp_tech_params_ptr->pdp_context.access_ctrl_flag,
          sizeof(ds_umts_pdp_access_ctrl_e_type)) == 0)
  {
    return DS_3GPP_PROFILE_CACHE_UPDATE_WITH_NO_CHANGE;
  }
...
===========================================================================*/
ds_3gpp_profile_cache_update_enum_type ds_3gpp_profile_cache_write_v4_dns_addr_primary
(
  ds_profile_db_profile_num_type             profile_number,
  const ds_profile_db_3gpp_tech_params_type *ds_3gpp_tech_params_ptr,
  const ds_profile_db_gen_params_type       *gen_params,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (tech_mask != EPC_PROFILE_TECH_MASK)
  {
    ...
    if (ds_umts_pdp_profile_ptr->dns_addr.primary_dns_addr.ip_vsn ==
     (ds_ip_version_enum_type)ds_3gpp_tech_params_ptr->tech_common_params.dns_addr.primary_ip_version &&
      memcmp((void *)&ds_umts_pdp_profile_ptr->dns_addr.primary_dns_addr.pdp_addr.pdp_addr_ipv4,
             (void *)&ds_3gpp_tech_params_ptr->tech_common_params.dns_addr.primary_addr.v4.addr,
             sizeof(ds_profile_db_ip_addr_v4_type)) == 0)
...
===========================================================================*/
ds_3gpp_profile_cache_update_enum_type ds_3gpp_profile_cache_write_pdp_context_primary_id
(
  ds_profile_db_profile_num_type             profile_number,
  const ds_profile_db_3gpp_tech_params_type *ds_3gpp_tech_params_ptr,
  const ds_profile_db_gen_params_type       *gen_params,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(memcmp((void *)&ds_umts_pdp_profile_ptr->context.primary_id,
...
===========================================================================*/
ds_3gpp_profile_cache_update_enum_type ds_3gpp_profile_cache_write_v4_dns_addr_secondary
(
  ds_profile_db_profile_num_type             profile_number,
  const ds_profile_db_3gpp_tech_params_type *ds_3gpp_tech_params_ptr,
  const ds_profile_db_gen_params_type       *gen_params,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (tech_mask != EPC_PROFILE_TECH_MASK)
  {
    ...
    if (ds_umts_pdp_profile_ptr->dns_addr.secondary_dns_addr.ip_vsn ==
     (ds_ip_version_enum_type)ds_3gpp_tech_params_ptr->tech_common_params.dns_addr.secondary_ip_version &&
      memcmp((void *)&ds_umts_pdp_profile_ptr->dns_addr.secondary_dns_addr.pdp_addr.pdp_addr_ipv4,
             (void *)&ds_3gpp_tech_params_ptr->tech_common_params.dns_addr.secondary_addr.v4.addr,
             sizeof(ds_profile_db_ip_addr_v4_type)) == 0)
...
===========================================================================*/
ds_3gpp_profile_cache_update_enum_type ds_3gpp_profile_cache_write_v6_dns_addr_primary
(
  ds_profile_db_profile_num_type             profile_number,
  const ds_profile_db_3gpp_tech_params_type *ds_3gpp_tech_params_ptr,
  const ds_profile_db_gen_params_type       *gen_params,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (tech_mask != EPC_PROFILE_TECH_MASK)
  {
    ...
    if ((ds_umts_pdp_profile_ptr->dns_addr.primary_dns_addr.ip_vsn ==
       (ds_ip_version_enum_type)ds_3gpp_tech_params_ptr->tech_common_params.dns_addr.primary_ip_version) &&
        (memcmp((void *)&ds_umts_pdp_profile_ptr->dns_addr.primary_dns_addr.pdp_addr.
     pdp_addr_ipv6.in6_u, (void *)&ds_3gpp_tech_params_ptr->tech_common_params.
     dns_addr.primary_addr.v6, sizeof(ds_profile_db_ip_addr_v6_type)) == 0))
    {
      return DS_3GPP_PROFILE_CACHE_UPDATE_WITH_NO_CHANGE;
    }
...
===========================================================================*/
ds_3gpp_profile_cache_update_enum_type ds_3gpp_profile_cache_write_v6_dns_addr_secondary
(
  ds_profile_db_profile_num_type             profile_number,
  const ds_profile_db_3gpp_tech_params_type *ds_3gpp_tech_params_ptr,
  const ds_profile_db_gen_params_type       *gen_params,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (tech_mask != EPC_PROFILE_TECH_MASK)
  {
    ...
    if ((ds_umts_pdp_profile_ptr->dns_addr.secondary_dns_addr.ip_vsn ==
         (ds_ip_version_enum_type)ds_3gpp_tech_params_ptr->tech_common_params.dns_addr.secondary_ip_version) &&
        (memcmp((void *)&ds_umts_pdp_profile_ptr->dns_addr.secondary_dns_addr.pdp_addr.
            pdp_addr_ipv6.in6_u,(void *)&ds_3gpp_tech_params_ptr->tech_common_params.
            dns_addr.secondary_addr.v6, sizeof(ds_profile_db_ip_addr_v6_type)) == 0))
    {
      return DS_3GPP_PROFILE_CACHE_UPDATE_WITH_NO_CHANGE;
    }
...
ds_3gpp_profile_cache_update_enum_type ds_3gpp_profile_cache_write_pdp_context_v6_pdp_addr
(
  ds_profile_db_profile_num_type             profile_number,
  const ds_profile_db_3gpp_tech_params_type *ds_3gpp_tech_params_ptr,
  const ds_profile_db_gen_params_type       *gen_params,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(ds_umts_pdp_profile_ptr->context.pdp_addr.ip_vsn ==
    (ds_ip_version_enum_type)ds_3gpp_tech_params_ptr->pdp_context.pdp_addr.ip_vsn &&
     memcmp((void *)&ds_umts_pdp_profile_ptr->context.pdp_addr.
                                              pdp_addr.pdp_addr_ipv6.in6_u, 
            (void *)&ds_3gpp_tech_params_ptr->pdp_context.
                                              pdp_addr.addr.v6, 
            sizeof(  ds_umts_pdp_profile_ptr->context.pdp_addr.
                                              pdp_addr.pdp_addr_ipv6.in6_u)) == 0)
  {
    return DS_3GPP_PROFILE_CACHE_UPDATE_WITH_NO_CHANGE;
  }
  ...
  memscpy((void *)&ds_umts_pdp_profile_ptr->context.pdp_addr.
	  ...
	  sizeof(  ds_umts_pdp_profile_ptr->context.pdp_addr.
                                            pdp_addr.pdp_addr_ipv6.in6_u),
          (void *)&ds_3gpp_tech_params_ptr->pdp_context.pdp_addr.addr.v6,
          sizeof(  ds_3gpp_tech_params_ptr->pdp_context.pdp_addr.addr.v6));
...
===========================================================================*/
ds_3gpp_profile_cache_update_enum_type ds_3gpp_profile_cache_write_pdp_context_v4_pdp_addr
(
  ds_profile_db_profile_num_type             profile_number,
  const ds_profile_db_3gpp_tech_params_type *ds_3gpp_tech_params_ptr,
  const ds_profile_db_gen_params_type       *gen_params,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (ds_umts_pdp_profile_ptr->context.pdp_addr.ip_vsn ==
    (ds_ip_version_enum_type)ds_3gpp_tech_params_ptr->pdp_context.pdp_addr.ip_vsn &&
      memcmp((void *)&ds_umts_pdp_profile_ptr->context.pdp_addr.pdp_addr.pdp_addr_ipv4,
     (void *)&ds_3gpp_tech_params_ptr->pdp_context.pdp_addr.addr.v4,sizeof(uint32)) == 0)
...
===========================================================================*/
ds_3gpp_profile_cache_update_enum_type ds_3gpp_profile_cache_write_emergency_flag
(
  ds_profile_db_profile_num_type             profile_number,
  const ds_profile_db_3gpp_tech_params_type *ds_3gpp_tech_params_ptr,
  const ds_profile_db_gen_params_type       *gen_params,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (memcmp((void *)&ds_umts_pdp_profile_ptr->emergency_calls_are_supported,
...
===========================================================================*/
ds_3gpp_profile_cache_update_enum_type ds_3gpp_profile_cache_write_linger_params
(
  ds_profile_db_profile_num_type             profile_number,
  const ds_profile_db_3gpp_tech_params_type *ds_3gpp_tech_params_ptr,
  const ds_profile_db_gen_params_type       *gen_params,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (tech_mask != EPC_PROFILE_TECH_MASK)
  {
    ...
    if ((ds_umts_pdp_profile_ptr->iface_linger_params.valid_flag ==
        ds_3gpp_tech_params_ptr->tech_common_params.iface_linger_params.is_valid) &&
        (ds_umts_pdp_profile_ptr->iface_linger_params.allow_linger_flag ==
        ds_3gpp_tech_params_ptr->tech_common_params.iface_linger_params.linger_params.allow_linger_flag) &&
        memcmp((void *)&ds_umts_pdp_profile_ptr->iface_linger_params.linger_timeout_val,
        (void *)&ds_3gpp_tech_params_ptr->tech_common_params.iface_linger_params
        .linger_params.linger_timeout_val,sizeof(uint16)) == 0)
    {
      return DS_3GPP_PROFILE_CACHE_UPDATE_WITH_NO_CHANGE;
    }
...
===========================================================================*/
ds_3gpp_profile_cache_update_enum_type ds_3gpp_profile_cache_write_tft_filter_id1
(
  ds_profile_db_profile_num_type             profile_number,
  const ds_profile_db_3gpp_tech_params_type *ds_3gpp_tech_params_ptr,
  const ds_profile_db_gen_params_type       *gen_params,
...
{
  ds_3gpp_profile_cache_update_enum_type  result
                                            = DS_3GPP_PROFILE_CACHE_UPDATE_WITH_ERROR;
  ds_umts_pdp_profile_type               *ds_umts_pdp_profile_ptr = NULL;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  result = DS_3GPP_PROFILE_CACHE_UPDATE_WITH_NO_CHANGE;
  do {
    if(ds_umts_pdp_profile_ptr->tft[tft_index].valid_flg !=
      ds_3gpp_tech_params_ptr->is_valid.tft[tft_index])
    {
      result = DS_3GPP_PROFILE_CACHE_UPDATE_SUCCESS;
      break;
    }

    if (memcmp((void *) &ds_umts_pdp_profile_ptr->tft[tft_index].filter_id,
    ...
    if(memcmp((void *)&ds_umts_pdp_profile_ptr->tft[tft_index].eval_prec_id,
    ...
    if(memcmp((void *)&ds_umts_pdp_profile_ptr->tft[tft_index].prot_num,
    ...
    if(memcmp((void *)&ds_umts_pdp_profile_ptr->tft[tft_index].src_addr_mask.mask.mask,
    ...
    if(memcmp((void *)&ds_umts_pdp_profile_ptr->tft[tft_index].src_addr_mask.mask.prefix_len,
    ...
    if(memcmp((void *)&ds_umts_pdp_profile_ptr->tft[tft_index].src_addr_mask.ip_vsn,
           (void *)&ds_3gpp_tech_params_ptr->tft[tft_index].src_addr_mask_ip_ver,
           sizeof(ds_profile_db_ip_version)) != 0)
    {
      result = DS_3GPP_PROFILE_CACHE_UPDATE_SUCCESS;
      break;
    }
    ...
    if(memcmp((void *)&ds_umts_pdp_profile_ptr->tft[tft_index].src_addr_mask.address.
    ...
    if(memcmp((void *)&ds_umts_pdp_profile_ptr->tft[tft_index].src_addr_mask.address.
           ...
           sizeof(ds_profile_db_ip_addr_v6_type)) != 0)
    {
      result = DS_3GPP_PROFILE_CACHE_UPDATE_SUCCESS;
      break;
    }

    if(memcmp((void *)&ds_umts_pdp_profile_ptr->tft[tft_index].dest_port_range,
           (void *)&ds_3gpp_tech_params_ptr->tft[tft_index].dest_port_range,
           sizeof(ds_umts_port_range_type)) != 0)
    {
      result = DS_3GPP_PROFILE_CACHE_UPDATE_SUCCESS;
      break;
    }

    if(memcmp((void *)&ds_umts_pdp_profile_ptr->tft[tft_index].src_port_range,
    ...
    if(memcmp((void *)&ds_umts_pdp_profile_ptr->tft[tft_index].ipsec_spi,
    ...
    if(memcmp((void *)&ds_umts_pdp_profile_ptr->tft[tft_index].tos_mask,
    ...
    if(memcmp((void *)&ds_umts_pdp_profile_ptr->tft[tft_index].flow_label,
...
===========================================================================*/
ds_3gpp_profile_cache_update_enum_type ds_3gpp_profile_cache_write_tft_filter_id2
(
  ds_profile_db_profile_num_type             profile_number,
  const ds_profile_db_3gpp_tech_params_type *ds_3gpp_tech_params_ptr,
  const ds_profile_db_gen_params_type       *gen_params,
...
===========================================================================*/
ds_3gpp_profile_cache_update_enum_type ds_3gpp_profile_cache_write_max_pdn_conn_per_block
(
  ds_profile_db_profile_num_type             profile_number,
  const ds_profile_db_3gpp_tech_params_type *ds_3gpp_tech_params_ptr,
  const ds_profile_db_gen_params_type       *gen_params,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (memcmp((void *)&ds_umts_pdp_profile_ptr->max_pdn_conn_per_blk,
...
===========================================================================*/
ds_3gpp_profile_cache_update_enum_type ds_3gpp_profile_cache_write_pdn_conn_timer
(
  ds_profile_db_profile_num_type             profile_number,
  const ds_profile_db_3gpp_tech_params_type *ds_3gpp_tech_params_ptr,
  const ds_profile_db_gen_params_type       *gen_params,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (memcmp((void *)&ds_umts_pdp_profile_ptr->max_pdn_conn_time,
...
===========================================================================*/
ds_3gpp_profile_cache_update_enum_type ds_3gpp_profile_cache_write_pdn_req_wait_timer
(
  ds_profile_db_profile_num_type             profile_number,
  const ds_profile_db_3gpp_tech_params_type *ds_3gpp_tech_params_ptr,
  const ds_profile_db_gen_params_type       *gen_params,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (memcmp((void *)&ds_umts_pdp_profile_ptr->pdn_req_wait_time,
...
===========================================================================*/
ds_3gpp_profile_cache_update_enum_type ds_3gpp_profile_cache_write_user_app_data
(
  ds_profile_db_profile_num_type             profile_number,
  const ds_profile_db_3gpp_tech_params_type *ds_3gpp_tech_params_ptr,
  const ds_profile_db_gen_params_type       *gen_params,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (tech_mask != EPC_PROFILE_TECH_MASK)
  {
    ...
    if (memcmp((void *)&ds_umts_pdp_profile_ptr->user_app_data,
...
===========================================================================*/
ds_3gpp_profile_cache_update_enum_type ds_3gpp_profile_cache_write_roaming_disallowed
(
  ds_profile_db_profile_num_type                profile_number,
  const ds_profile_db_3gpp_tech_params_type    *ds_3gpp_tech_params_ptr,
  const ds_profile_db_gen_params_type          *gen_params,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (memcmp((void *)&ds_umts_pdp_profile_ptr->roaming_disallowed,
...
===========================================================================*/
ds_3gpp_profile_cache_update_enum_type ds_3gpp_profile_cache_write_pdn_discon_wait_time
(
  ds_profile_db_profile_num_type                profile_number,
  const ds_profile_db_3gpp_tech_params_type    *ds_3gpp_tech_params_ptr,
  const ds_profile_db_gen_params_type          *gen_params,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (memcmp((void *)&ds_umts_pdp_profile_ptr->pdn_discon_wait_time,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if( local_err != DS_3GPPI_LOCAL_ERR_VAL_NONE)
  {
    DS_3GPP_MSG2_HIGH( "Prf Cache Entry Delete: return status %d local err %d",
                       ret_val, local_err);
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    is_profile_valid =
                    ds_3gpp_profile_cache_check_profile_number(profile_number);
    if (is_profile_valid != DS_UMTS_PDP_SUCCESS)
    {
      local_err = DS_3GPPI_LOCAL_ERR_VAL_0;
      break;
    }
    /* Check whether the max transient and persistent profiles haven't reached.
	*/
    if (transient_profile) 
    {
      if (transient_profile_cnt >= DS_UMTS_MAX_PDP_TRANSIENT_PROFILE_NUM)
      {
        local_err = DS_3GPPI_LOCAL_ERR_VAL_1;
        break;
      }
    ...
    // Try to allocate the memory to hold the new profile
    ...
    if (NULL == ds_umts_pdp_profile_ptr)
    {
      local_err = DS_3GPPI_LOCAL_ERR_VAL_4;
      break;
    }
  ...
  if( local_err != DS_3GPPI_LOCAL_ERR_VAL_NONE)
  {
    DS_3GPP_ERROR_LOG_2 ("profile_cache_entry_create: ret_val d local err d",
                        ret_val, local_err);
...
/* Set requested UMTS QoS parm, traffic class to Interactive for Gobi2 UMTS builds - CR 155084 */
  ...
  ds_umts_pdp_profile_ptr->context.apn_type_mask = 0;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (ds_umts_pdp_profile_ptr == NULL)
  {
    DS_3GPP_MSG0_LOW("Cache Ptr is NULL, Entry does not exist");
    return ret_val;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (ret_val != 1)
  {
    DS_3GPP_C_MSG3(DS_3GPP_F3_MASK, "prf cache entry invalid: subs %d, prf number %d, valid = %d",
                   subs_id, profile_number, ret_val);
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  /*--------------------------------------------------------------------------- 
    internal utility: caller required to pass valid ptrs
  ---------------------------------------------------------------------------*/
  if (ds_umts_pdp_profile_ptr == NULL || tech_specific_params_ptr == NULL)
  {
    DS_3GPP_ERROR_LOG_2("Cache ptr d or Tech params ptr d is NULL",
                       ds_umts_pdp_profile_ptr, tech_specific_params_ptr);
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(ds_umts_pdp_profile_ptr == NULL)
  {
    DS_3GPP_ERROR_LOG_3_EX(subs_id, "convert_profile_db_params: local err d prf d subs_id d",
                       local_err, profile_number, subs_id);
    return NULL;
  }
  ...
  */
  ...
  ds_3gpp_profile_cache_write_pdp_context_type(ds_3gpp_tech_params_ptr,ds_umts_pdp_profile_ptr);
  ...
  memscpy((void *)&ds_umts_pdp_profile_ptr->context.lte_roaming_pdp_type,
          sizeof(ds_umts_pdp_type_enum_type),
          (void *)&ds_3gpp_tech_params_ptr->lte_pdp_type,
          sizeof(ds_umts_pdp_type_enum_type));

  memscpy((void *)&ds_umts_pdp_profile_ptr->context.umts_roaming_pdp_type,
          sizeof(ds_umts_pdp_type_enum_type),
          (void *)&ds_3gpp_tech_params_ptr->umts_pdp_type,
          sizeof(ds_umts_pdp_type_enum_type));
  ...
  memscpy((void *)&ds_umts_pdp_profile_ptr->context.apn_type_mask,
          sizeof(uint64),
          (void *)&gen_params_ptr->apn_type_mask,
          sizeof(uint64));
  ...
  memscpy((void *)&ds_umts_pdp_profile_ptr->attach_flag,
           sizeof(boolean),
           (void *)&gen_params_ptr->is_attach,
           sizeof(boolean));
  ...
  memscpy((void *)ds_umts_pdp_profile_ptr->pco_id_list,
          sizeof(ds_umts_pdp_profile_ptr->pco_id_list),
          (void *)ds_3gpp_tech_params_ptr->pco_id_list,
          sizeof(ds_3gpp_tech_params_ptr->pco_id_list));

  memscpy((void *)&ds_umts_pdp_profile_ptr->request_msisdn_flag,
          sizeof(boolean),
          (void *)&ds_3gpp_tech_params_ptr->request_msisdn_flag,
          sizeof(boolean));

  memscpy((void *)&ds_umts_pdp_profile_ptr->pdn_label,
          DS_3GPP_MAX_PDN_LABLE_LENTH +1,
          (void *)&ds_3gpp_tech_params_ptr->pdn_label_3gpp,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (ds_umts_pdp_profile_ptr == NULL)
  {
    DS_3GPP_ERROR_LOG_3_EX(subs_id,  "profile_cache_write_readonly_gen_params fail: "
                        "local err d prf d subs_id d",
                        local_err, profile_number, subs_id);
  ...
  ds_umts_pdp_profile_ptr->profile_client = gen_params_ptr->profile_client;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (ds_umts_pdp_profile_ptr == NULL)
  {
    DS_3GPP_ERROR_LOG_3_EX(subs_id,  "convert_to_profile_db fails: "
                        "local err d prf d cache subs_id d",
                        local_err, profile_number, subs_id);
    /*ret_val has not been changed, re-confirm anyways */
  ...
  gen_params_ptr->profile_client = ds_umts_pdp_profile_ptr->profile_client;
 ...
 -------------------------------------------------------------------------*/
  ...
  ds_3gpp_tech_params_ptr->lte_pdp_type =
    ds_3gpp_profile_cache_get_roaming_pdp_type(
       ds_umts_pdp_profile_ptr->context.lte_roaming_pdp_type);

  ds_3gpp_tech_params_ptr->umts_pdp_type =
    ds_3gpp_profile_cache_get_roaming_pdp_type(
       ds_umts_pdp_profile_ptr->context.umts_roaming_pdp_type);
...
/*---------------------------------------------------------------------------------
  ...
  memscpy((void *)&gen_params_ptr->apn_type_mask,
          sizeof(uint64),
          (void *)&ds_umts_pdp_profile_ptr->context.apn_type_mask,
          sizeof(uint64));
  ...
  memscpy((void *)&gen_params_ptr->is_attach,
          sizeof(boolean),
          (void *)&ds_umts_pdp_profile_ptr->attach_flag,
          sizeof(boolean));
  ...
  memscpy((void *)ds_3gpp_tech_params_ptr->pco_id_list,
          sizeof(ds_3gpp_tech_params_ptr->pco_id_list),
          (void *)ds_umts_pdp_profile_ptr->pco_id_list,
          sizeof(ds_umts_pdp_profile_ptr->pco_id_list));

  memscpy((void *)&ds_3gpp_tech_params_ptr->request_msisdn_flag,
          sizeof(boolean),
          (void *)&ds_umts_pdp_profile_ptr->request_msisdn_flag,
          sizeof(boolean));

  memscpy((void *)&ds_3gpp_tech_params_ptr->pdn_label_3gpp,
          DS_PROFILE_DB_MAX_APN_NAME_LEN + 1,
          (void *)&ds_umts_pdp_profile_ptr->pdn_label,
          DS_3GPP_MAX_PDN_LABLE_LENTH +1);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  do
  {
    ...
    if (ds_umts_pdp_profile_ptr != NULL)
    {
      ...
      is_set = (ds_umts_pdp_profile_ptr->epc_profile_tech_mask & 
                           (0x01 << (COMMON_TECH_PARAM_MAX - ident)))? TRUE: FALSE;
      *is_tech_param_set = is_set;
    }
    else
    {
      local_err = DS_3GPPI_LOCAL_ERR_VAL_4;
  ...
  {
    DS_3GPP_MSG3_MED("Is Tech Param Set: local err %d prf %d is_set %d",
                     local_err, profile_number, is_set );
  }

  return ret_val;
} /*ds_3gpp_profile_cache_is_epc_tech_param_set*/
...
===========================================================================*/
ds_3gpp_profile_cache_update_enum_type ds_3gpp_profile_cache_write_ipv6_delegation
(
  ds_profile_db_profile_num_type             profile_number,
  const ds_profile_db_3gpp_tech_params_type *ds_3gpp_tech_params_ptr,
  const ds_profile_db_gen_params_type       *gen_params,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (memcmp((void *)&ds_umts_pdp_profile_ptr->ipv6_prefix_delegation_flag,
...
===========================================================================*/
ds_3gpp_profile_cache_update_enum_type ds_3gpp_profile_cache_write_roaming_ho_iwlan_to_lte
(
  ds_profile_db_profile_num_type             profile_number,
  const ds_profile_db_3gpp_tech_params_type *ds_3gpp_tech_params_ptr,
  const ds_profile_db_gen_params_type       *gen_params,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (memcmp((void *)&ds_umts_pdp_profile_ptr->roaming_ho_iwlan_to_lte,
          (void *)&ds_3gpp_tech_params_ptr->roaming_ho_iwlan_to_lte,
          sizeof(boolean)) == 0)
  {
    return DS_3GPP_PROFILE_CACHE_UPDATE_WITH_NO_CHANGE;
...
===========================================================================*/
ds_3gpp_profile_cache_update_enum_type ds_3gpp_profile_cache_write_roaming_ho_lte_to_iwlan
(
  ds_profile_db_profile_num_type              profile_number,
  const ds_profile_db_3gpp_tech_params_type  *ds_3gpp_tech_params_ptr,
  const ds_profile_db_gen_params_type        *gen_params,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (memcmp((void *)&ds_umts_pdp_profile_ptr->roaming_ho_lte_to_iwlan,
          (void *)&ds_3gpp_tech_params_ptr->roaming_ho_lte_to_iwlan,
          sizeof(boolean)) == 0)
  {
    return DS_3GPP_PROFILE_CACHE_UPDATE_WITH_NO_CHANGE;
...
===========================================================================*/
ds_3gpp_profile_cache_update_enum_type ds_3gpp_profile_cache_write_failure_timer
(
  ds_profile_db_profile_num_type             profile_number,
  const ds_profile_db_3gpp_tech_params_type *ds_3gpp_tech_params_ptr,
  const ds_profile_db_gen_params_type       *gen_params,
...
/*===========================================================================
FUNCTION DS_3GPP_PROFILE_CACHE_WRITE_OP_RESERVED_PCO_ID_LIST

DESCRIPTION
  The function updates the cache with the operator reserved pco id list
...
===========================================================================*/
ds_3gpp_profile_cache_update_enum_type  ds_3gpp_profile_cache_write_op_reserved_pco_id_list
(
  ds_profile_db_profile_num_type             profile_number,
  const ds_profile_db_3gpp_tech_params_type *ds_3gpp_tech_params_ptr,
  const ds_profile_db_gen_params_type       *gen_params,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (memcmp((void *)&ds_umts_pdp_profile_ptr->pco_id_list,
          (void *)&ds_3gpp_tech_params_ptr->pco_id_list,
          sizeof(ds_3gpp_tech_params_ptr->pco_id_list)) == 0)
  {
    return DS_3GPP_PROFILE_CACHE_UPDATE_WITH_NO_CHANGE;
  }
...
/*===========================================================================
FUNCTION DS_3GPP_PROFILE_CACHE_WRITE_MSISDN_REQUEST_FLAG

DESCRIPTION
  The function updates the cache with the msisdn request flag
...
===========================================================================*/
ds_3gpp_profile_cache_update_enum_type   ds_3gpp_profile_cache_write_msisdn_request_flag
(
  ds_profile_db_profile_num_type             profile_number,
  const ds_profile_db_3gpp_tech_params_type *ds_3gpp_tech_params_ptr,
  const ds_profile_db_gen_params_type       *gen_params,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (memcmp((void *)&ds_umts_pdp_profile_ptr->request_msisdn_flag,
          (void *)&ds_3gpp_tech_params_ptr->request_msisdn_flag,
          sizeof(boolean)) == 0)
  {
    return DS_3GPP_PROFILE_CACHE_UPDATE_WITH_NO_CHANGE;
...
/*===========================================================================
FUNCTION DS_3GPP_PROFILE_CACHE_WRITE_ATTACH_FLAG

DESCRIPTION
  The function updates the attach flag
...
===========================================================================*/
ds_3gpp_profile_cache_update_enum_type ds_3gpp_profile_cache_write_attach_flag
(
  ds_profile_db_profile_num_type             profile_number,
  const ds_profile_db_3gpp_tech_params_type *ds_3gpp_tech_params_ptr,
  const ds_profile_db_gen_params_type       *gen_params,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (memcmp((void *)&ds_umts_pdp_profile_ptr->attach_flag,
          (void *)&gen_params->is_attach,
          sizeof(boolean)) == 0)
  {
    return DS_3GPP_PROFILE_CACHE_UPDATE_WITH_NO_CHANGE;
  }

  memscpy((void *)&ds_umts_pdp_profile_ptr->attach_flag, sizeof(boolean),
          (void *)&gen_params->is_attach,
          sizeof(boolean));
...
/*===========================================================================
FUNCTION DS_3GPP_PROFILE_CACHE_GET_PERSISTENT_COUNT

DESCRIPTION
  This function returns the persistent profile count
...
SIDE EFFECTS
  None
===========================================================================*/
uint16 ds_3gpp_profile_cache_get_persistent_count
(
   void
)
{
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  return persistent_profile_cnt;  
}/*ds_3gpp_profile_cache_get_persistent_count*/
/*===========================================================================
...
===========================================================================*/
ds_3gpp_profile_cache_update_enum_type ds_3gpp_profile_cache_write_clat_enabled
(
  ds_profile_db_profile_num_type             profile_number,
  const ds_profile_db_3gpp_tech_params_type *ds_3gpp_tech_params_ptr,
  const ds_profile_db_gen_params_type       *gen_params,
...
/*===========================================================================
FUNCTION DS_3GPP_PROFILE_CACHE_WRITE_APN_TYPE

DESCRIPTION
  The function updates the APN Type in Cache

PARAMETERS:
  profile_number           -- Profile number passed by profile DB.
  ds_3gpp_tech_params_ptr  -- 3GPP Tech Params Ptr
  gen_params               -- General Params
  tech_mask                -- Technology Mask
...
===========================================================================*/
ds_3gpp_profile_cache_update_enum_type ds_3gpp_profile_cache_write_apn_type
(
  ds_profile_db_profile_num_type             profile_number,
  const ds_profile_db_3gpp_tech_params_type *ds_3gpp_tech_params_ptr,
  const ds_profile_db_gen_params_type       *gen_params,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (memcmp((void *)&ds_umts_pdp_profile_ptr->context.apn_type_mask,
          (void *)&gen_params->apn_type_mask,
          sizeof(uint64)) == 0)
  {
    return DS_3GPP_PROFILE_CACHE_UPDATE_WITH_NO_CHANGE;
  }
...
/*===========================================================================
FUNCTION DS_3GPP_PROFILE_CACHE_GET_PROFILE_NUM_FOR_APN

DESCRIPTION
  This function checks if there is a profile with given APN name and
...
RETURN VALUE
  0               : profile not found for the APN name
  profile_num > 0 : Profile found for the APN name

SIDE EFFECTS
  None
===========================================================================*/
uint16 ds_3gpp_profile_cache_get_profile_num_for_apn
(
  ds_umts_pdp_subs_e_type  subs_id,
  byte                    *apn_name
)
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_3GPP_MSG1_HIGH("ds_3gpp_profile_cache_get_profile_num_for_apn subs_id:%d",
                    subs_id);
       ...
       cache_index++)
  {
    ...
    if (ds_umts_pdp_profile_ptr != NULL &&
        ds_umts_pdp_profile_ptr->context.valid_flg == TRUE)
    {
      if( strncasecmp((char*)ds_umts_pdp_profile_ptr->context.apn,
                      (char*)apn_name,
  ...
  return profile_num;
} /* ds_3gpp_profile_cache_get_profile_num_for_apn */
