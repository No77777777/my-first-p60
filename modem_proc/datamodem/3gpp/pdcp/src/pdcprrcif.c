...
Copyright (c) 2005 - 2015 Qualcomm Technologies, Incorporated.  All Rights Reserved.
...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp/pdcp/src/pdcprrcif.c#3 $ $DateTime: 2021/02/08 22:33:06 $ $Author: pwbldsvc $
...
/*---------------------------------------------------------------------------
  Global to store the Data rate ind CB function pointer
---------------------------------------------------------------------------*/
static pdcp_reg_data_rate_ind_req_type pdcp_reg_data_rate_ind_req_info ={NULL};
...
===========================================================================*/
static uint8 pdcp_get_new_pdcp_inst( void )
{
  uint8 pdcp_inst;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  for (pdcp_inst = 0; pdcp_inst < MAX_PDCP_INST; pdcp_inst++)
  {
    ...
    {
        if (
            (pdcp_ctl_blk[pdcp_inst].rlc_state.rlc_dl_id == rlc_dl_id)
          &&(pdcp_ctl_blk[pdcp_inst].rlc_state.rlc_ul_id == rlc_ul_id)
...
)
{
  ...
  uint16                            index;
  uint8                             comp_idx = MAX_COMP_ENG_TYPES;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
...
{
  uint8              pdcp_inst;
  rohc_ioctl_e_type  rohc_ioctl_type = ROHC_IOCTL_CONTEXT_REINITIALIZATION;
...
{
  uint8              pdcp_inst;
  rohc_ioctl_e_type  rohc_ioctl_type = ROHC_IOCTL_SWITCH_DECOMP_MODE;
  errno_enum_type    rohc_ioctl_errno;
...
)
{
  ...
  /*---------------------------------------------------------------------
      DL WMs based on DL Data rate indicated by lower layers

  =======================================================================
    DATA RATE                  WM_LO     WM_HI     WM_DNE     DNE_QCNT
                                                             (WM_DNE/500)
  =======================================================================
    NON-HS (<= 1.25Mbps)       1600      40000     49000         98

    > 1.25Mbps && <= 1.8Mbps   40000     80000     125000        250

    > 1.8Mbps && <= 3.6Mbps    60000     85000     150000        300

    > 3.6Mbps && <= 7.2Mbps    120000    170000    300000        600

    > 7.2Mbps && <= 42Mbps
      ( Feature REL8 defined ) 400000    800000    1000000       2000
      ( Feature MIMO defined)  200000    800000    900000        1800
      ( Default )              200000    600000    700000        1400

    > 42 Mbps                  216000    540000    675000        1350
  =======================================================================
  ---------------------------------------------------------------------*/
  ...
  /*-------------------------------------------------------------------------
    WM levels for non-HS rates.
  -------------------------------------------------------------------------*/
  if( rab_dl_rate <= DSWPSD_DL_RATE_NON_HS )
  {
    dsm_set_low_wm    (wm_ptr, DSUMTSPS_DL_WM_DEFAULT_LO);
    dsm_set_hi_wm     (wm_ptr, DSUMTSPS_DL_WM_DEFAULT_HI);

    if (wm_ptr->current_cnt <  wm_ptr->lo_watermark) 
    {
      dsm_set_dne (wm_ptr, DSUMTSPS_DL_WM_DEFAULT_DNE);
      dsm_set_dne_q_cnt (wm_ptr, DSUMTSPS_DL_WM_DEFAULT_DNE_QCNT);
    }
    else 
    {
      if(DSUMTSPS_DL_WM_DEFAULT_DNE > wm_ptr->dont_exceed_cnt)
      {
    dsm_set_dne       (wm_ptr, DSUMTSPS_DL_WM_DEFAULT_DNE);
    dsm_set_dne_q_cnt (wm_ptr, DSUMTSPS_DL_WM_DEFAULT_DNE_QCNT);
  }
  ...
  -------------------------------------------------------------------------*/
  else if( (rab_dl_rate >  DSWPSD_DL_RATE_NON_HS)  &&
           (rab_dl_rate <= DSWPSD_DL_RATE_1_8M) )
  {
    dsm_set_low_wm    (wm_ptr, DSWPSD_DL_WM_DEFAULT_HS_LO);
    dsm_set_hi_wm     (wm_ptr, DSWPSD_DL_WM_DEFAULT_HS_HI);

    if (wm_ptr->current_cnt <  wm_ptr->lo_watermark) 
    {
      dsm_set_dne       (wm_ptr, DSWPSD_DL_WM_DEFAULT_HS_DNE);
      dsm_set_dne_q_cnt (wm_ptr, DSWPSD_DL_WM_DEFAULT_HS_DNE_QCNT);
    }
    else 
    {
      if(DSWPSD_DL_WM_DEFAULT_HS_DNE > wm_ptr->dont_exceed_cnt)
      {
    dsm_set_dne       (wm_ptr, DSWPSD_DL_WM_DEFAULT_HS_DNE);
    dsm_set_dne_q_cnt (wm_ptr, DSWPSD_DL_WM_DEFAULT_HS_DNE_QCNT);
  }
  ...
  -------------------------------------------------------------------------*/
  else if( (rab_dl_rate >  DSWPSD_DL_RATE_1_8M)  &&
           (rab_dl_rate <= DSWPSD_DL_RATE_3_6M) )
  {
    dsm_set_low_wm    (wm_ptr, DSWPSD_DL_WM_DEFAULT_HS_P1_LO);
    dsm_set_hi_wm     (wm_ptr, DSWPSD_DL_WM_DEFAULT_HS_P1_HI);

    if (wm_ptr->current_cnt <  wm_ptr->lo_watermark) 
    {
    dsm_set_dne       (wm_ptr, DSWPSD_DL_WM_DEFAULT_HS_P1_DNE);
    dsm_set_dne_q_cnt (wm_ptr, DSWPSD_DL_WM_DEFAULT_HS_P1_DNE_QCNT);
  }
    else 
    {
      if(DSWPSD_DL_WM_DEFAULT_HS_P1_DNE > wm_ptr->dont_exceed_cnt)
      {
        dsm_set_dne       (wm_ptr, DSWPSD_DL_WM_DEFAULT_HS_P1_DNE);
        dsm_set_dne_q_cnt (wm_ptr, DSWPSD_DL_WM_DEFAULT_HS_P1_DNE_QCNT);
      }
  ...
  /*-------------------------------------------------------------------------
    WM levels for upto 7.2Mbps
  -------------------------------------------------------------------------*/
  else if( (rab_dl_rate >  DSWPSD_DL_RATE_3_6M)  &&
           (rab_dl_rate <= DSWPSD_DL_RATE_7_2M) )
  {
    dsm_set_low_wm    (wm_ptr, DSWPSD_DL_WM_DEFAULT_HS_P2_LO);
    dsm_set_hi_wm     (wm_ptr, DSWPSD_DL_WM_DEFAULT_HS_P2_HI);

    if (wm_ptr->current_cnt <  wm_ptr->lo_watermark) 
    {
      dsm_set_dne       (wm_ptr, DSWPSD_DL_WM_DEFAULT_HS_P2_DNE);
      dsm_set_dne_q_cnt (wm_ptr, DSWPSD_DL_WM_DEFAULT_HS_P2_DNE_QCNT);
    }
    else 
    {
      if(DSWPSD_DL_WM_DEFAULT_HS_P2_DNE > wm_ptr->dont_exceed_cnt)
      {
    dsm_set_dne       (wm_ptr, DSWPSD_DL_WM_DEFAULT_HS_P2_DNE);
    dsm_set_dne_q_cnt (wm_ptr, DSWPSD_DL_WM_DEFAULT_HS_P2_DNE_QCNT);
  }
  ...
  /*-------------------------------------------------------------------------
    WM levels for rab rates upto 42Mbps
  -------------------------------------------------------------------------*/
  else if( (rab_dl_rate >  DSWPSD_DL_RATE_7_2M)  &&
           (rab_dl_rate <= DSWPSD_DL_RATE_42M) )
  {
    dsm_set_low_wm    (wm_ptr, DSWPSD_DL_WM_DEFAULT_HSPA_PLUS_LO);
    dsm_set_hi_wm     (wm_ptr, DSWPSD_DL_WM_DEFAULT_HSPA_PLUS_HI);

    if (wm_ptr->current_cnt <  wm_ptr->lo_watermark) 
    {
    dsm_set_dne       (wm_ptr, DSWPSD_DL_WM_DEFAULT_HSPA_PLUS_DNE);
    dsm_set_dne_q_cnt (wm_ptr, DSWPSD_DL_WM_DEFAULT_HSPA_PLUS_DNE_QCNT);
  }
    else 
    {
      if(DSWPSD_DL_WM_DEFAULT_HSPA_PLUS_DNE > wm_ptr->dont_exceed_cnt)
      {
        dsm_set_dne       (wm_ptr, DSWPSD_DL_WM_DEFAULT_HSPA_PLUS_DNE);
        dsm_set_dne_q_cnt (wm_ptr, DSWPSD_DL_WM_DEFAULT_HSPA_PLUS_DNE_QCNT);
      }
  ...
  /*-------------------------------------------------------------------------
    WM levels for rab rates greater than 42Mbps
  -------------------------------------------------------------------------*/
  else if( rab_dl_rate > DSWPSD_DL_RATE_42M )
  {
    dsm_set_low_wm (wm_ptr, DSWPSD_DL_WM_DEFAULT_3C_HSDPA_LO);
    dsm_set_hi_wm  (wm_ptr, DSWPSD_DL_WM_DEFAULT_3C_HSDPA_HI);
    ...
    {
      if(DSWPSD_DL_WM_DEFAULT_3C_HSDPA_DNE > wm_ptr->dont_exceed_cnt)
      {
    dsm_set_dne    (wm_ptr, DSWPSD_DL_WM_DEFAULT_3C_HSDPA_DNE);
        dsm_set_dne_q_cnt (wm_ptr, DSWPSD_DL_WM_DEFAULT_3C_HSDPA_DNE_QCNT);
  }
    }
  }


  DATA_MSG3_HIGH("WM levels after update - Lo: %d, Hi: %d, DNE: %d",
                 wm_ptr->lo_watermark,
                 wm_ptr->hi_watermark,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(cfg_req == NULL)
  {
    DATA_MSG0_ERROR("cfg_req ptr is NULL");
    return ;
  }
  ...
  /*-------------------------------------------------------------------------
    Invoke callback function only for the release PDCP actions.
    Data rates SETUP_PDCP and RECONFIG_PDCP will be handled via
    pdcp_rrc_data_rate_ind_cmd_hdlr
  -------------------------------------------------------------------------*/
    if( pdcp_reg_data_rate_ind_req_info.func_ptr != NULL )
    {
      if( cfg_req->action == RELEASE_PDCP )
      {
        pdcp_reg_data_rate_ind_req_info.func_ptr(PDCP_RELEASE,
                                                 0,
                                                 cfg_req->rlc_ul_id,
                                                 0);
      }
      else if( cfg_req->action == RELEASE_ALL_PDCP )
      {
        pdcp_reg_data_rate_ind_req_info.func_ptr(PDCP_RELEASE_ALL,
                                                 0,
                                                 cfg_req->rlc_ul_id,
                                                 0);
      }
    ...
    {
      DATA_MSG0_MED("NULL RB Ind CB func ptr. NV73689 may not be set");
    }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (pdcp_is_rohc_cfg(pdcp_info))
  {
    ...
    if (is_mcast_call == FALSE)
    {
      ...
      {
        DATA_MSG2_ERROR("ROHC parameter = %d %d",
                        rohc_info.rohc_dl_max_cid,rohc_info.rohc_ul_max_cid);
        return FALSE;
      }
    }
    else
...
/*===========================================================================
FUNCTION PDCP_REG_RRC_DATA_RATE_IND

DESCRIPTION  This function registers for Data rate Indications sent by
             RRC to PDCP module.
...
SIDE EFFECTS
  None
===========================================================================*/
boolean pdcp_reg_rrc_data_rate_ind
(
  pdcp_rrc_data_rate_ind_f_ptr_type  data_rate_ind_func_ptr
)
{
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
    /* set data rate ind function ptr */
    pdcp_reg_data_rate_ind_req_info.func_ptr = data_rate_ind_func_ptr;

  return TRUE;
} /* pdcp_reg_rrc_data_rate_ind */

/*===========================================================================
FUNCTION PDCP_DEREG_DATA_RATE_IND

DESCRIPTION  This function de-registers for Data Rate Indications sent by
             RRC to PDCP module.
...
RETURN VALUE
  True  - De-Registration succeeded
  False - De-Registration failed

SIDE EFFECTS
  None
===========================================================================*/
boolean pdcp_dereg_data_rate_ind
(
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /* set data rate ind function pointer to NULL */
  pdcp_reg_data_rate_ind_req_info.func_ptr = NULL;

  return TRUE;
} /* pdcp_dereg_data_rate_ind */

/*===========================================================================
FUNCTION PDCP_RRC_DATA_RATE_IND_CMD_HDLR

DESCRIPTION
  This function process the PDCP_RRC_DATA_RATE_IND command.

PARAMETERS
  cmd_ptr: the CMD itself being processed by comp-task
...
===========================================================================*/
void pdcp_rrc_data_rate_ind_cmd_hdlr
(
  const comp_cmd_type  *cmd_ptr                      /* Pointer to command */
)
{
  pdcp_rrc_rb_data_rate_info_type  *data_rate_info_p = NULL;
  uint8                             index = 0;
  uint8                             pdcp_inst;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if(cmd_ptr == NULL)
  ...
  data_rate_info_p = (pdcp_rrc_rb_data_rate_info_type*)
                      (&cmd_ptr->cmd.pdcp_rrc_data_rate_info);

  if(data_rate_info_p == NULL)
  {
    DATA_MSG0_ERROR("data_rate_info_p ptr is NULL");
    return ;
  }

  /*Validate subscription id*/
  if(!ds3gsubsmgr_is_subs_id_valid(cmd_ptr->hdr.subs_id))
  {
    return ;
  }

  if( cmd_ptr->hdr.cmd_id != PDCP_RRC_DATA_RATE_IND )
  {
    DATA_MSG1_ERROR("invalid cmd id in rrc_data_rate_ind_cmd_hdlr %d",
                    cmd_ptr->hdr.cmd_id);
    DS3GPP_ASSERT(0);
    return;
  }

  DATA_MSG1_HIGH("pdcp_rrc_data_rate_ind_cmd_hdlr num_rbs:%d",
                 data_rate_info_p->num_rbs);

  /*-------------------------------------------------------------------------
    Update DL data rate in the PDCP control block and update DL WMs based
    on new data rate indication
  -------------------------------------------------------------------------*/
  for( index = 0; index < data_rate_info_p->num_rbs; index++ )
  {
    pdcp_inst
      = pdcp_get_pdcp_inst_given_lc_id(
          cmd_ptr->hdr.subs_id,
          (uint8)(data_rate_info_p->rb_data_rate_info[index].rlc_dl_id),
          (uint8)(data_rate_info_p->rb_data_rate_info[index].rlc_ul_id));
    ...
    /*-----------------------------------------------------------------------
      Cache the DL rate
    -----------------------------------------------------------------------*/
    pdcp_ctl_blk[pdcp_inst].dl_data_rate
      = data_rate_info_p->rb_data_rate_info[index].rb_dl_rate;

    /*-----------------------------------------------------------------------
      Update DL watermarks only in PDCP_UP state
    -----------------------------------------------------------------------*/
      if (pdcp_ctl_blk[pdcp_inst].pdcp_to_ps_wm != NULL)
      {
        DATA_MSG0_HIGH("data rate ind, updating pdcp_to_ps rx WM");
        pdcp_update_dl_wm_levels(pdcp_ctl_blk[pdcp_inst].dl_data_rate,
                                 pdcp_ctl_blk[pdcp_inst].pdcp_to_ps_wm);
      }

    //DATA_MSG0_HIGH("data rate ind, updating rlc rx WM");
      pdcp_update_dl_wm_levels(
         pdcp_ctl_blk[pdcp_inst].dl_data_rate,
         &(pdcp_ctl_blk[pdcp_inst].rlc_state.rlc_rx_wm_item));
    ...
    /*-----------------------------------------------------------------------
      Invoke callback function only if registered
    -----------------------------------------------------------------------*/
    if( pdcp_reg_data_rate_ind_req_info.func_ptr != NULL )
    {
      pdcp_reg_data_rate_ind_req_info.func_ptr(
         PDCP_DATA_RATE_IND,
         data_rate_info_p->rb_data_rate_info[index].rb_id,
         data_rate_info_p->rb_data_rate_info[index].rlc_ul_id,
         data_rate_info_p->rb_data_rate_info[index].rb_ul_rate);
    }
    else
    {
      DATA_MSG0_MED("NULL Data rate Ind CB func ptr. NV73689 set to inactive");
    }
  }
