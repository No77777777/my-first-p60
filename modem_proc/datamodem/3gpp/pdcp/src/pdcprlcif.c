...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp/pdcp/src/pdcprlcif.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
void pdcp_dl_wm_reg_ind_cb
(
  rlc_lc_id_type        lc_id,
  rlc_wm_reg_rsp_status status,
  void*                 bearer_cntxt_p
);
...
{  
  ...
  DATA_MSG1_HIGH("In pdcp_dereg_wm_with_tdrlc pdcp_inst=%d",
                   pdcp_instance);
#ifdef FEATURE_SEGMENT_LOADING
  if(ds_3gpp_ps_tds_tbl == NULL)
  {
...
)
{
  ...
  -------------------------------------------------------------------------*/
  if (ds_3gpp_loopback_is_enabled() && (!ds_3gpp_l2_loopback_is_enabled()))
  {
    ds_3gpp_loopback_l2_reg_ul_wm((uint8)rlc_ul_id,tx_wm_ptr);
  }
  else
  {
  #ifdef FEATURE_SEGMENT_LOADING
  ...
  #endif /* FEATURE_SEGMENT_LOADING */
    {
      DS_3GPP_ERROR_LOG_0_EX(subs_id, "NULL Ptr when sending RLC_UL_REGISTER_SRVC_REQ");
    }
    else /* a good ptr for RLC Data SAP BCCH data has been received */
    {
      ...
      pdcp_rlc_ul_reg_params.rlc_data[ 0 ].bearer_context = (void*)bearer_cntxt_p;
      pdcp_rlc_ul_reg_params.rlc_data[ 0 ].wm_reg_ind_cb = NULL;
  ...
  {
    ds_3gpp_loopback_l2_reg_dl_wm((uint8)rlc_dl_id,rx_wm_ptr);
  }
  else
  {
  #ifdef FEATURE_SEGMENT_LOADING
    ...
    else /* a good ptr for RLC Data SAP BCCH data has been received */
    {
      ...
      pdcp_rlc_dl_reg_params.rlc_data[ 0 ].bearer_context = (void*)bearer_cntxt_p;
      pdcp_rlc_dl_reg_params.rlc_data[ 0 ].wm_reg_ind_cb = pdcp_dl_wm_reg_ind_cb;
...
)
{
  ...
  DATA_MSG1_HIGH("In pdcp_dereg_wm_with_rlc pdcp_inst=%d",
                  pdcp_instance);
  ...
  if(pdcp_validate_pdcp_inst(pdcp_instance) == FALSE)
  {
    ...
    validate subs id
    -------------------------------------------------------------------------*/
    if(ds3gsubsmgr_is_subs_id_valid(pdcp_ctl_blk[pdcp_instance].subs_id) == FALSE)
    {
      return;
    }
#endif
  ...
  {
    DS_3GPP_ERROR_LOG_0("Cannot get L2 UL cmd buffer");
  }
...
#ifdef FEATURE_DUAL_WCDMA
    l2_dl_cmd_ptr->cmd_hdr.as_id = pdcp_ctl_blk[pdcp_instance].subs_id;
#endif
  ...
  {
    DS_3GPP_ERROR_LOG_0("Cannot get L2 DL cmd buffer");
  }
...
)
{
  ...
  if((pdcp_cmd_ptr = comp_get_cmd_buf()) == NULL)
  {
    ...
    -----------------------------------------------------------------------*/
    DS_3GPP_ERROR_LOG_0("No items on PDCP Task free cmd q");
    PDCP_LEAVE_CRIT_SECTION ( &pdcp_crit_section );
    return;
  }
...
)
{
  ...
  DATA_MSG2_HIGH("pdcp_to_rlc_hi_water_cb, wm= %d, pdcp_inst = %d",
                  wm, pdcp_inst);
  /*-------------------------------------------------------------------------
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if( pdcp_instance >= MAX_PDCP_INST)
  {
    DATA_MSG1_ERROR("pdcp_instance %d is out of range", pdcp_instance);
    return;
  }
...
#endif /* FEATURE_WCDMA_NEW_L2_INTERFACE */
  {
	DS_3GPP_ERROR_LOG_3("NULL Ptr when sending RLC_UL_REGISTER_SRVC_REQ",0,0,0);
  }
  else 
  {
...
/*===========================================================================
FUNCTION PDCP_DL_WM_REG_IND_CB

DESCRIPTION This is the callback function for the PDCP to indicate DL watermrk registration
...
{
  ds_bearer_context_s* temp;
  temp = (ds_bearer_context_s*)bearer_cntxt_p;

  if ( status == RLC_WM_REQ_SUCCESS && 
       ds_bearer_cntx_validate_bearer_context(temp) )
  {
    ds_umts_tlb_wm_reg_cnf(temp->ds_bearer_context_dyn_p->nsapi,
                           temp->ds_bearer_context_dyn_p->subs_id);
  }
  else
  {
    DATA_MSG0_ERROR("Invalid bearer_context in cb or reg_status is false");
  }
