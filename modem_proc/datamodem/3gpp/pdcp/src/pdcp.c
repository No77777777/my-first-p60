...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp/pdcp/src/pdcp.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_3GPP_ERROR_LOG_0("pdcp_init_rlc_state failed");
  }
...
{
  dsm_watermark_type  *pdcp_to_ps_wm_ptr = NULL;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  pdcp_to_ps_wm_ptr = pdcp_ctl_blk[pdcp_inst].pdcp_to_ps_wm;

  /*-------------------------------------------------------------------------
    Set up PDCP->PS watermark - used for both PDP-IP & PDP-PPP stacks
    Lo, HI and DNE counts are setup in Mode Handler.
  -------------------------------------------------------------------------*/
  pdcp_to_ps_wm_ptr->gone_empty_func_ptr = NULL;
 // (pdcp_ctl_blk[pdcp_inst].pdcp_to_ps_wm)->non_empty_func_ptr   = NULL;
  pdcp_to_ps_wm_ptr->lowater_func_ptr = NULL;
  pdcp_to_ps_wm_ptr->hiwater_func_ptr = NULL;

  if (pdcp_ctl_blk[pdcp_inst].pdcp_state == PDCP_UP)
  {
    pdcp_to_ps_wm_ptr->lowater_func_ptr = pdcp_pdcp_to_ps_flow_enable_cb;
    pdcp_to_ps_wm_ptr->lowater_func_data = (void*) pdcp_inst;

    pdcp_to_ps_wm_ptr->hiwater_func_ptr = pdcp_pdcp_to_ps_flow_disable_cb;
    pdcp_to_ps_wm_ptr->hiwater_func_data = (void*) pdcp_inst;
  }
...
)
{
  ...
  if(cmd_ptr == NULL)
  {
    DS_3GPP_ERROR_LOG_0_EX(subs_id, "cmd_ptr passed is NULL");
    return FALSE;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  bearer_cntxt_p = (ds_bearer_context_s *)rx_func_para;
  ...
  {
    DATA_MSG2_ERROR("tx_wm_ptr = %d, rx_wm_ptr = %d", tx_wm_ptr, rx_wm_ptr);
    PDCP_BRANCH_LEAVE_CRIT_SECTION ( &pdcp_crit_section );
    return FALSE;
  }
  ...
  if (pdcp_ctl_blk[pdcp_inst].pdcp_state == PDCP_TRANSPARENT)
  {
    ...
    if( (ds_3gpp_ps_tds_tbl != NULL) &&
        (cmd_ptr->cmd.pdcp_reg_wm.bearer_call_mode == SYS_SYS_MODE_TDS) )
#else
    if(cmd_ptr->cmd.pdcp_reg_wm.bearer_call_mode == SYS_SYS_MODE_TDS)
#endif /* FEATURE_SEGMENT_LOADING */
    {
...
#endif /* FEATURE_TDSCDMA */
    {
      ...
      if( (ds_3gpp_ps_wcdma_tbl != NULL) &&
          (cmd_ptr->cmd.pdcp_reg_wm.bearer_call_mode == SYS_SYS_MODE_WCDMA) )
#else
      if(cmd_ptr->cmd.pdcp_reg_wm.bearer_call_mode == SYS_SYS_MODE_WCDMA)
#endif /* FEATURE_SEGMENT_LOADING */
      {
...
#ifdef FEATURE_SEGMENT_LOADING
      if( (ds_3gpp_ps_tds_tbl != NULL) &&
          (cmd_ptr->cmd.pdcp_dereg_wm.bearer_call_mode == SYS_SYS_MODE_TDS) )
#else
      if(cmd_ptr->cmd.pdcp_dereg_wm.bearer_call_mode == SYS_SYS_MODE_TDS)
#endif /* FEATURE_SEGMENT_LOADING */
      {
...
#ifdef FEATURE_SEGMENT_LOADING
        if( (ds_3gpp_ps_wcdma_tbl != NULL) &&
            (cmd_ptr->cmd.pdcp_dereg_wm.bearer_call_mode == SYS_SYS_MODE_WCDMA) )
#else
        if(cmd_ptr->cmd.pdcp_dereg_wm.bearer_call_mode == SYS_SYS_MODE_WCDMA)
#endif /* FEATURE_SEGMENT_LOADING */
        {
...
)
{
  ...
  DATA_MSG3_HIGH("pdcp set flow mask, pdcp_inst: %d, flow_dir: %d, flow_mask: %d",
