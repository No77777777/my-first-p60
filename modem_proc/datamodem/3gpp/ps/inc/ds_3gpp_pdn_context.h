...
$Header: //components/rel/data.mpss/3.5..2.1/3gpp/ps/inc/ds_3gpp_pdn_context.h#7 $
...
#define DS_3GPP_OPER_RESERVED_PCO_LIST_MASK       (0x400)
...
#define DS_MAX_APN_PLMN_SUPPORTED (DS_MAX_APN_NUM-1)*DS_3GPP_MAX_PLMN_SUPPORTED*DS_MAX_THROT_RAT



/*---------------------------------------------------------------------------
  Hysteresis timer in milli-secs. Upon expiry of this timer, PDN connection
  will be terminated if no client is using it.
---------------------------------------------------------------------------*/
#define DS_3GPP_UNUSED_PDN_TIMER_VAL 2000 
...
#define DS_3GPP_APM_SET_IS_TRANSFER_IN_PROGRESS_FLAG(payload, value, subs_id) \
  payload.ind_field_mask |= DS_DSD_IND_CNTXT_TRANSFER_PROGRESS_MASK; \
  payload.is_transfer_progress = value; \
  payload.subs_id = subs_id;
...
#define DS_3GPP_IS_PDN_CONTEXT_STATE_AWAITING_BEARER_ALLOC(pdn_p)  \
...
#define DS_3GPP_BEARER_PHYS_LINK_EV_COUNT 1
...
typedef struct 
{
  ...
  boolean               ipv6_iface_up_in_progess;
  dssnet6_sm_prefix_info_type    v6_prefix_info; /*Added for optimized IP handover*/
#endif /* FEATURE_DATA_PS_IPV6 */
...
typedef struct
{
  ...
  uint64                prev_rxed_data_pkt_cnt;
}ds_pdn_throughput_stats;
...
{  
  ...
  ds_umts_pdp_type_enum_type default_pdp_type;
  ...
  rex_timer_type             tcp_retrans_protect_timer;
                             /*!< Use to raise trm priority for protecting tcp retransmit*/
  ...
  boolean                  force_ota_teardown;
                                       ...
                                       to be forced*/
  boolean                  modem_pco_req;
                                      /* This is set when modem app like ATP requests 
                                         OP PCO to be sent out OTA*/

  boolean                  is_always_shared;
                           /* Flag indicating if this PDN is being used as the
                              always share PDN */
  ...
  rex_timer_type           unused_pdn_timer;
   /* Hysteresis timer in milli-secs. Upon expiry of this timer, PDN connection
      will be terminated if no client is using it */

  boolean                  needs_connection_retry;
                           /* Used to determine if this default PDN needs connection
                           retry after IRAT to LTE is successful. This caters to a 
                           specific scenario where the attach APN is not cleaned up
                           from the APN table because the IPv6 iface is not down yet.*/
  ds_pdn_throughput_stats  throughput_stats;
 /*!< cached bearer tech change used during sys chng hndling */
  ps_iface_ioctl_bearer_tech_changed_type  cached_bearer_tech;
#ifdef FEATURE_DATA_WLAN_MAPCON
  ds_sys_system_status_info_type           cached_bearer_tech_ex;
#endif /* FEATURE_DATA_WLAN_MAPCON */

  boolean                                  out_of_use_in_progress;
  /* For race condition, detach and AP tearing down iface may happen at the same time;
     it's used to indicate IFACE_OUT_OF_USE cmd is in progress and CALL END EVENT 
     should be sent to ds3g in bearerDownIndCB */

  boolean                                  epco_enabled;
   /* Flag to indicate if EPCO is to be used for this PDN.
      By default it will be FALSE */

  boolean                                  nw_ps_data_off_supported;
  /* Flag to indicate whether PS Data Off is supported by both UE and NW */
  uint64                                   prev_rx_bytes_for_bidir;
  /* Cached total prev received bytes per PDN from iface statistic, 
     this is required to calculate the DL throughput in 2s to indicate Bi-Dir session*/
...
#ifdef FEATURE_QSH_MDUMP
/*===========================================================================
FUNCTION ds_pdn_cntxt_qsh_mdump_collect
...
===========================================================================*/ 
QSH_MDUMP_FN_ATTR 
void ds_pdn_cntxt_qsh_mdump_collect
(
  void
);
#endif
...
PARAMETERS   
  *protocol_config_options_p  - Ptr to protocol config
...
(
  ds_3gpp_protocol_cfg_options_s  *protocol_config_options_p,
...
PARAMETERS
  ...
  ignore_nw_override - boolean indicating whether or not to use network
                       override information.
  pref_mode      - CM sys mode
...
(
  sys_modem_as_id_e_type           subs_id,
  ds_umts_pdp_context_type        *context_info_p,
  boolean                          roaming_status,
  boolean                          ignore_nw_override,
...
/*===========================================================================
FUNCTION DS_3GPP_PDN_CNTXT_CHECK_ACTIVE_PDN_WITH_INPUT_PROFILENUM

DESCRIPTION
  Function to check if there is any PDN active with the passed
  profile number.

PARAMETERS
  profile_num - Profile number passed in Dial String

DEPENDENCIES
  None.

RETURN VALUE
  TRUE: If the passed profile num has an active PDN 
  FALSE: otherwise
...
boolean ds_3gpp_pdn_cntxt_check_active_pdn_with_input_profilenum
(
   uint16                   profile_num,
   sys_modem_as_id_e_type   subs_id   
);
...
/*===========================================================================
FUNCTION DS_3GPP_PDN_CNTXT_CHECK_ACTIVE_PDN_WITH_INPUT_APN

DESCRIPTION
  Function to check if there is any PDN active with the input
  APN name and pdp type.
...
RETURN VALUE
  TRUE: If any active PDN apn is same as input APN 
  FALSE: otherwise
...
boolean ds_3gpp_pdn_cntxt_check_active_pdn_with_input_apn
(
  byte*                       apn,
  ds_umts_pdp_type_enum_type  input_pdp_type,
  sys_modem_as_id_e_type      subs_id
);
...
(
  ds_3gpp_protocol_cfg_options_s  *protocol_config_options_p, /* Ptr to PCO */
...
/*===========================================================================
FUNCTION DS_3GPP_PDN_CNTXT_IS_APN_THROTTLED_UNALLOCATED_BEARER

DESCRIPTION
  This function checks if an apn is throttled for the unallocated
  bearer (i.e for the first iface of a DUAL ip call)
...
RETURN VALUE
  True -  if iface is throttled for unallocated bearer
  False - otherwise
...
===========================================================================*/
boolean 
ds_3gpp_pdn_cntxt_is_apn_throttled_unallocated_bearer
(
  ds_pdn_context_s    *pdn_cntx_p,
  ps_iface_type       *iface_p
);
...
/*===========================================================================
FUNCTION  DS_3GPP_PDN_CNTXT_IS_IMS_PDN

DESCRIPTION
  This function queries whether the requested PDN is type of IMS or not
...
===========================================================================*/
boolean ds_3gpp_pdn_cntxt_is_ims_pdn
(
  void *pdn_cntx_p
);

/*===========================================================================
FUNCTION  DS_3GPP_PDN_CNTXT_IS_TARGET_PDN

DESCRIPTION
  This function queries whether the requested PDN is certain type of PDN

PARAMETERS
  *pdn_cntxt_p  -  pointer to PDN context
  type          -  target type to be checked
...
===========================================================================*/
boolean ds_3gpp_pdn_cntxt_is_target_type_pdn
(
  ds_pdn_context_s      *pdn_cntxt_p,
  ps_sys_apn_enum_type  type
);
/*===========================================================================
...
PARAMETERS 
  *protocol_config_options_p     - config option that has to be filled
  *profile_p,                    - PDP profile
   collision_pco_id              - the collision PCO id for which filling
                                   would be skipped
...
/*======================================================================== 
 FUNCTION DS_3GPP_PDN_CNTXT_IS_ANY_EPDN_AWAITING_BEARER_ALLOCATION

DESCRIPTION
  This function returns the status, if  there is a emergency pdn waiting for
...
RETURN VALUE
  TRUE  - If there is at least one EPDN in AWAITING_BEARER_ALLOC state
  FALSE - otherwise
SIDE EFFECTS
  None
 =======================================================================*/
ds_pdn_context_s* ds_3gpp_pdn_cntxt_is_any_epdn_awaiting_bearer_allocation(void);
...
/*===========================================================================
FUNCTION DS_3GPP_PDN_CONTEXT_TAG_DEF_PDN_FOR_CONNECTION_RETRY

DESCRIPTION
  This function is used to tag default PDN context for connection retry.after
  IRAT to LTE is successful. This caters to a specific scenario where the 
  attach APN is not cleaned up from the APN table because the IPv6 iface is not
  down yet. Thus we will attempt to bring up the tagged default PDNs when the
  IPv6 Iface is completely down.
...
SIDE EFFECTS
  None
===========================================================================*/
void ds_3gpp_pdn_cntxt_tag_def_pdn_for_connection_retry
(
  sys_modem_as_id_e_type subs_id
);

/*===========================================================================
FUNCTION DS_3GPP_PDN_CONTEXT_IS_DEF_PDN_TAGGED_FOR_CONNECTION_RETRY

DESCRIPTION
  This function is used to find out if there are any default PDN(s) tagged for
  connection retry after IRAT
...
RETURN VALUE
  boolean - TRUE if a tagged default PDN is found, FALSE otherwise.

SIDE EFFECTS
  None
===========================================================================*/
boolean ds_3gpp_pdn_cntxt_is_def_pdn_tagged_for_connection_retry
(
  sys_modem_as_id_e_type subs_id
);

/*===========================================================================
FUNCTION DS_3GPP_PDN_CNTX_CACHE_BEARER_TECH_INFO

DESCRIPTION
  This function posts IFACE bearer tech event to all valid
  Iface pointers for a single PDN context.
...
SIDE EFFECTS
  None
===========================================================================*/
void ds_3gpp_pdn_cntx_cache_bearer_tech_info
(
  ds_pdn_context_s	*pdn_cntxt_p,
  void		        *bearer_tech_chg_info_p
);

/*===========================================================================
FUNCTION DS_3GPP_PDN_CNTX_CACHE_BEARER_TECH_EX_INFO
...
SIDE EFFECTS
  None
===========================================================================*/
void ds_3gpp_pdn_cntx_cache_bearer_tech_ex_info
(
  ds_pdn_context_s  *pdn_cntxt_p,
  void		    *bearer_tech_chg_info_p
);
...
/*===========================================================================
FUNCTION DS_3GPP_PDN_CNTX_POST_IFACE_BEARER_TECH_EVT_GENERIC
...
PARAMETERS
 subs_id ------------- subscription id.
 tech_change_info_p -- bearer tech change info pointer.
 bearer_tech_ex_p ---- bearer tech ex info pointer.
...
SIDE EFFECTS
  None
===========================================================================*/
void ds_3gpp_pdn_cntx_post_iface_bearer_tech_evt_generic
(
  sys_modem_as_id_e_type	            subs_id,
  ps_iface_ioctl_bearer_tech_changed_type   *tech_change_info_p,
...
/*===========================================================================
FUNCTION DSPDNCNTX_FILL_MSIDSN_CONTAINER

DESCRIPTION
  This function fills the MSISDN PCO container
...
===========================================================================*/
void ds_pdn_cntxt_fill_msisdn_support_container
(
  ds_3gpp_protocol_cfg_options_s  *protocol_config_options_p,
  sys_modem_as_id_e_type           subs_id
);
...
/*===========================================================================
FUNCTION ds_3gpp_pdn_cntx_validate_assert

DESCRIPTION
  This function checks if the pdn context is valid and asserts in error scenarios
...
void ds_3gpp_pdn_cntx_validate_assert
(
  ds_pdn_context_s* pdn_cntx_p
);
...
/*=====================================
FUNCTION ds_pdn_context_emergengy_sub_match

DESCRIPTION
  Iface 14 (V4) and 15 (V6 if applicable) are reserved for Emergency PDN on Sub1
  Iface 16 (V4) and 17 (V6 if applicable) are reserved for Emergency PDN on Sub2
  This function would check if the iface is the right one for the sub for EPDN. 
  It would be called during the routing procedure.

PARAMETERS   
  iface_p - Pointer to the iface
  subs_id - the call sub id
    
DEPENDENCIES
  None.

RETURN VALUE
  TRUE:  the iface is the right one for the call sub
  FALSE: the iface is the wrong one for the call sub

SIDE EFFECTS
  None
=====================================*/
boolean ds_pdn_context_emergengy_sub_match
(
  ds_3gpp_iface_s                  *iface_p,
  sys_modem_as_id_e_type           subs_id
);
...
/*===========================================================================
FUNCTION DS_3GPP_PDN_CNTXT_TEARDOWN_DOWN_ALL_PDNS

DESCRIPTION
  This function tears down the all the PDN connections when AP requests UE
  to detached as a part of attach profile Update
...
SIDE EFFECTS
  None
===========================================================================*/
void ds_3gpp_pdn_cntxt_tear_down_all_pdns 
(
  sys_modem_as_id_e_type  subs_id
);

/*===========================================================================
FUNCTION ds_3gpp_pdn_cntxt_lpm_teardown

DESCRIPTION
  This function is used to teardown calls when 
  LPM Online transition occurs
...
===========================================================================*/
void ds_3gpp_pdn_cntxt_lpm_teardown(void);

/*===========================================================================
FUNCTION ds_3gpp_pdn_cntxt_local_teardown

DESCRIPTION
  This function is used to teardown calls when DDS switch occurs
PARAMETERS
  void
...
DESCRIPTION
  ...
  using the passed profile number for the specified subscription
...
DESCRIPTION
  ...
  same in profiles and from the app and returns the conflict pco id
...
===========================================================================*/
uint16 ds_pdn_cntxt_check_op_pco_collision
(
  ds_umts_pdp_profile_type     *profile_p,
  void                         *user_data_to_query_pco,
  sys_modem_as_id_e_type        subs_id  
...
/*===========================================================================
FUNCTION dsPdnContext_any_other_pdn_diff_pf_num_active

DESCRIPTION
 This function checks if there is any other valid PDN UP for the passed
 subs_id and having profile number other than passed prof num
...
RETURN VALUE
  TRUE, if there is an active PDN other than the profile passed in.
  FALSE, otherwise

SIDE EFFECTS
===========================================================================*/
boolean dsPdnContext_any_other_pdn_diff_pf_num_active
(
...
/*===========================================================================
FUNCTION  DS_3GPP_PDN_CNTXT_RESOLVE_ALWAYS_SHARE_PROFILE

DESCRIPTION
  This function resolves the always share profile 0xFE to a valid profile

PARAMETERS
  subs_id - subscription for the PDN
  resolved_pdn_apn - ptr for the resolved pdn's APN to be populated
  policy_info_ptr - ptr to the policy
 
DEPENDENCIES
  None.

RETURN VALUE 
  uint32 - the resolved profile number

SIDE EFFECTS
  None.

===========================================================================*/
uint32 ds_3gpp_pdn_cntxt_resolve_always_share_profile
(
  sys_modem_as_id_e_type   subs_id,
  byte                    *resolved_pdn_apn,
  acl_policy_info_type    *policy_info_ptr
);

/*===========================================================================
FUNCTION  DS_3GPP_PDN_CNTXT_SET_RESOLVED_ALWAYS_SHARE_PROF_NUM

DESCRIPTION
  This function sets the global for the resolved always share profile

PARAMETERS
  resolved_prof_num - profile num to be stored
...
===========================================================================*/
void ds_3gpp_pdn_cntxt_set_resolved_always_share_prof_num
(
  uint32 resolved_prof_num
);
...
DESCRIPTION
  This function gets the global for the resolved always share profile
...
RETURN VALUE 
  uint32 - cached always share profile number

SIDE EFFECTS
  None.

===========================================================================*/
uint32 ds_3gpp_pdn_cntxt_get_resolved_always_share_prof_num(void);
...
/*===========================================================================
FUNCTION ds_3gpp_pdn_cntxt_accept_nw_provided_ip_type

DESCRIPTION
  This function checks whether the Network provided IP Type should be
  accepted. If the Network Provided IP Type cannot be accepted, UE
  will send a Bearer Activation Reject to the Network.
...
RETURN VALUE
  TRUE, if NW provided IP Type can be accepted
  FALSE, otherwise

SIDE EFFECTS
  None
===========================================================================*/
boolean ds_3gpp_pdn_cntxt_accept_nw_provided_ip_type
(
  ds_pdn_context_s          *pdn_cntx_p,
  ds_apn_ip_support_type_e   ip_support
);
...
/*===========================================================================
  FUNCTION DS_3GPP_PDN_CNTX_APN_TYPE_CHG_CMD_HDLR

  DESCRIPTION
  This function handles the apn type change, It extracts the apn type
  for the profile number passed and sends both the old and new apn as
  an event to ds3gmgr
...
===========================================================================*/
void ds_3gpp_pdn_cntx_apn_type_chg_cmd_hdlr
(
  const ds_cmd_type      *cmd_ptr
);

/*===========================================================================
  FUNCTION DS_3GPP_PDN_CNTX_GET_ANY_ACTIVE_PDN

  DESCRIPTION
  This function returns pointer for a valid PDN active for a given subs_id
  ...
  RETURN VALUE 
  Pointer to active PDN or NULL.
   
  SIDE EFFECTS 
  None.
  
===========================================================================*/
ds_pdn_context_s *ds_3gpp_pdn_cntxt_get_any_active_pdn
(
  sys_modem_as_id_e_type subs_id
);
...
/*===========================================================================
FUNCTION  ds_3gpp_pdn_cntxt_allocate_pdn_and_bringup_ifaces

DESCRIPTION
  This functions allocates a new PDN context and new DS ifaces and brings up
  the ifaces. This function is used by KAMGR.
...
===========================================================================*/
boolean ds_3gpp_pdn_cntxt_allocate_pdn_and_bringup_ifaces
(
  uint16                    profile_no,
  ps_iface_type           **iface,
  ds_umts_pdp_profile_type *prof_context,
...
/*===========================================================================
FUNCTION ds_3gpp_pdn_cntxt_is_bearer_deactivated_by_network
...
boolean ds_3gpp_pdn_cntxt_is_bearer_deactivated_by_network
(
   ds_pdn_context_s               *pdn_context_p,
   void                           *bearer_context_p
);
...
/*===========================================================================
FUNCTION ds_3gpp_pdn_context_set_inactivity_polling_timers

DESCRIPTION
  This function sets the inactivity and polling timer for a
  single pdn context.

PARAMETERS
  pdn_context_p (IN) :  PDN context object
  enable_timers:  flag that determines whether timers are
...
===========================================================================*/
void ds_3gpp_pdn_context_set_inactivity_polling_timers
(
   ds_pdn_context_s              *pdn_cntx_p,
   sys_modem_as_id_e_type         cm_sub_id, 
   boolean                        enable_timers
);

/*===========================================================================
FUNCTION ds_3gpp_pdn_context_set_inactivity_polling_timers_all_pdns

DESCRIPTION
  This function sets the inactivity and polling timer for each
  pdn context.  It will cycle through all PDN contexts and set
  the inactivity and polling timer for every PDN that is both up
  and belonging to the sub_id that we are looking at
...
===========================================================================*/
void ds_3gpp_pdn_context_set_inactivity_polling_timers_all_pdns
(
   sys_modem_as_id_e_type         cm_sub_id,
   boolean                        enable_timers
...
/*===========================================================================
FUNCTION DS_3GPP_PDN_CNTXT_IS_PROFILE_ID_BLOCKED_ON_CURRENT_PLMN

DESCRIPTION
   The functions checks if the passed profile id is blocked on the
   current plmn
...
===========================================================================*/
boolean ds_3gpp_pdn_cntxt_is_profile_id_blocked_on_current_plmn
(
...
/*===========================================================================
FUNCTION      DS3GPP_PDN_CONTEXT_IS_IMS_OR_EMERGNECY_PDN
...
PARAMETERS
  ...
  ds_pdn_context_s        -   PDN context 
  sys_modem_as_id_e_type  -   subscription Id to check the ifaces
                              active on that subscription

RETURN VALUE
  boolean TRUE  : if input PDN is IMS
          FALSE : otherwise
...
SIDE EFFECTS
  None
===========================================================================*/
boolean ds3gpp_pdn_context_is_ims_or_emergency_pdn
(
  ds_pdn_context_s       *pdn_cntx_p,
  sys_modem_as_id_e_type  subs_id
);
...
/*===========================================================================
FUNCTION      DS3GPP_PDN_CONTEXT_IS_IMS_PDN

DESCRIPTION
  This function checks if the input pdn is IMS or not  
...
SIDE EFFECTS
  None
===========================================================================*/
boolean ds3gpp_pdn_context_is_ims_pdn
(
  ds_pdn_context_s       *pdn_cntx_p,
  sys_modem_as_id_e_type  subs_id
);
...
/*===========================================================================
FUNCTION      DS_3GPP_PDN_CONTEXT_MODEM_DEF_PROFILE_REDIAL

DESCRIPTION This function evaluates whether redial will be performed based 
            on conditions described below. 
   

PARAMETERS

  new_apn_name_p          -  pointer to the apn name which is to be checked
                             whether it is in attach apn list or not
  new_pdp_type            -  new pdp type of the profile whose parameter was
                             changed
  sys_modem_as_id_e_type  -  subscription Id
  prof_num                -  profile number to check is it is modem_def_profile

RETURN VALUE
  boolean TRUE  : returns TRUE only if the profile is in the active attach
                  profile list and the profile is modem_def_profile and
                  apn_param_change nv is disabled
          FALSE : otherwise
...
SIDE EFFECTS
  None
===========================================================================*/
boolean ds_3gpp_pdn_context_modem_def_profile_redial
(
  char                             *new_apn_name_p,
...
/*===========================================================================
FUNCTION    DS_3GPP_PDN_CNTXT_PERFORM_NETWORK_OVERRIDE_CLEANUP

DESCRIPTION
  This performs action of bringing down the iface if the IP mismatches due to
  NW having overriden the IP 
 
  This fucntion is called as part of validating ifaces when iface is going up
  for both emergency and non-emergency iface bringup.
...
RETURN VALUE
 TRUE:    If ip is overriden with a different IP type, return TRUE and bring down
          the iface.
 FALSE : NO NW override IP OR matched with the NW override IP .


SIDE EFFECT
  None.
===========================================================================*/
boolean ds_3gpp_pdn_cntxt_perform_network_override_cleanup
(
  ps_iface_type         *iface_p
);
...
SIDE EFFECTS
  None.
===========================================================================*/
void ds_3gpp_pdn_context_cleanupDSIface
(
  ds_3gpp_iface_s           *ds3gpp_iface_p
);

/*===========================================================================
FUNCTION DS_3GPP_PDN_CONTEXT_IS_ICMP_ERROR_SENDING_ALLOWED

DESCRIPTION
  This function will check if error sending is allowed or not.
  Error sending is not allowed if all the following conditions are met
  1. If iface belongs to IMS PDN
  2. IMS PDN is on nDDS
  3. RAB is released
...
RETURN VALUE
  TRUE: If error sending is allowed
  FALSE: Otherwise 
...
===========================================================================*/
boolean ds_3gpp_pdn_context_is_icmp_error_sending_allowed
(
  ps_iface_type  *iface_ptr
);
/*===========================================================================
FUNCTION DS_PDN_CNTXT_SET_IPV6_IFACE_UP_IN_PROGRESS

DESCRIPTION
  try to set ipv6_iface_up_in_progress
...
SIDE EFFECTS
  None
===========================================================================*/
void ds_pdn_cntxt_set_ipv6_iface_up_in_progress
(
  ps_iface_type       *ps_v6_iface_ptr,
  boolean              ipv6_iface_up_in_progress
);

#ifdef FEATURE_LAPP
/*===========================================================================
FUNCTION	   DS_3GPP_PDN_CNTXT_IFACE_IS_COMING_UP

DESCRIPTION    
  This function checks the PDN context to see if there is any iface in
  coming up state.
...
RETURN VALUE   
  TRUE  -- if atleast one of the 2 ifaces are in coming up state
  FALSE -- Otherwise

SIDE EFFECTS   
  NONE
===========================================================================*/
boolean ds_3gpp_pdn_cntxt_iface_is_coming_up
(
...
/*===========================================================================
FUNCTION     DS_3GPP_PDN_CNTXT_GET_PDN_CNTXT_FROM_EPS_ID

DESCRIPTION    
  This util function is used to get the pdn context corresponding to an
  EPS ID
...
RETURN VALUE   
  pdn context pointer corresponding to the EPS bearer ID

SIDE EFFECTS   
  NONE
===========================================================================*/
ds_pdn_context_s* ds_3gpp_pdn_cntxt_get_pdn_cntxt_from_eps_id
(
  eps_bearer_id_T                      eps_id,
  sys_modem_as_id_e_type               subs_id
);
...
/*===========================================================================
FUNCTION     DS_3GPP_PDN_CNTXT_GET_V4_PS_IFACE_FROM_EPS_ID

DESCRIPTION    
  This util function is used to get the v4 and v6 PS ifaces associated to a
  particular bearer
 
PARAMETERS 
  eps_id           -- EPS ID of the bearer
  ps_iface_v4_ptr  -- v4 ps iface pointer
  ps_iface_v6_ptr  -- v6 ps iface pointer
  subs_id          -- Subscription id  
...
SIDE EFFECTS   
  NONE
===========================================================================*/
void  ds_3gpp_pdn_cntxt_get_ps_iface_from_eps_id
(
  eps_bearer_id_T                      eps_id,
  ps_iface_type                       **ps_iface_v4_ptr,
  ps_iface_type                       **ps_iface_v6_ptr,
  sys_modem_as_id_e_type               subs_id
);
...
===========================================================================*/
boolean ds_pdn_cntxt_dhcpRequestExtConfig
(
...
===========================================================================*/
boolean ds_pdn_cntxt_dhcpStartV4AddrAlloc
(
  ds_3gpp_iface_s   	*ds3gpp_iface_p,	   /* ptr to 3GPP iface tbl  */
  ds_pdn_context_s  	*pdn_cntx_p 		/* call instance for the call*/
);
...
/*===========================================================================
FUNCTION DS_PDN_CNTXT_GET_IPV6_IFACE_UP_IN_PROGRESS

DESCRIPTION
  try to get ipv6_iface_up_in_progress
...
SIDE EFFECTS
  None
===========================================================================*/
boolean ds_pdn_cntxt_get_ipv6_iface_up_in_progress
(
  ps_iface_type       *ps_v6_iface_ptr
);

/*===========================================================================
FUNCTION        DS_3GPP_PDN_CONTEXT_NOTIFY_CALL_END

DESCRIPTION 
  Send DS3GEVENTMGR_PS_CALL_END_EV to DS3G. This event indicates 
  that data call for the PDN is ended.
...
SIDE EFFECTS
  None
===========================================================================*/
void ds_3gpp_pdn_context_notify_call_end
(
   ds_pdn_context_s                  *pdn_cntx_ptr,
   sys_modem_as_id_e_type             subs_id 
);
...
/*===========================================================================
FUNCTION DS_3GPP_PDN_CNTX_TEARDOWN_PDN_USING_PROFILE

DESCRIPTION
  This function tears down the PDN Connections using the given profile number
...
SIDE EFFECTS
  None
===========================================================================*/
boolean ds_3gpp_pdn_cntx_teardown_pdn_using_profile
(
  uint32  			 profile_number,
  sys_modem_as_id_e_type         subs_id,
  ps_iface_net_down_reason_type  ps_netdown_reason
);

/*===========================================================================
FUNCTION    DS_3GPP_PDN_CONTEXT_CLEANUPPSIFACE

DESCRIPTION
  Resets certain ds3gpp_iface fields

PARAMETERS
  ps_iface_ptr   - Pointer to the PS iface that is to be cleaned up.
  subs_id - subscription ID
  down_reason - PS netdown reason
...
SIDE EFFECTS
  None.
===========================================================================*/
void ds_3gpp_pdn_context_cleanupPSIface
(
...
/*===========================================================================
FUNCTION      ds_pdn_context_get_emergency_iface_addr_family

DESCRIPTION
  This function returns the address family of the emergency iface.
...
RETURN VALUE  IFACE_IPV6_ADDR_FAMILY: For v6 Iface.
              IFACE_IPV4_ADDR_FAMILY: For v4 Iface.

SIDE EFFECTS  None.
===========================================================================*/
ps_iface_addr_family_type ds_pdn_context_get_emergency_iface_addr_family
(
  ds_3gpp_iface_s *iface_p
);

/*===========================================================================
FUNCTION DS_3GPP_PDN_CONTEXT_EMERGENCY_AND_INTERNET_PDN_UP

DESCRIPTION
  This function loops over all PDN contexts to see if emergency is up and
  internet PDN is only non-emergency PDN that is up.
...
RETURN VALUE
  TRUE, If Emergency APN is up and for non-emergency, only internet PDN is up.
  FALSE, otherwise

SIDE EFFECTS
===========================================================================*/
boolean ds_3gpp_pdn_context_emergency_and_internet_pdn_up
(
  sys_modem_as_id_e_type  subs_id,
  char                   *internet_apn
);
...
/*===========================================================================
FUNCTION  DS_3GPP_ENFORCE_EVALUTE_FOR_VOICE_CALL

DESCRIPTION
  As per VZW  requirements , during refresh enforcement event if only
  value 4 (force refresh during voice call bit) is set,
  DS MH should vote positive if only IMS PDN exists and have active voice/video call
  
  Note: As per requirement The voice or video call can be either circuit switched (voice) 
  or VoLTE (voice) or LVC (video) or VoWiFi (voice or video).
  
PARAMETERS
  subs_id      - subs id 
  
RETURN VALUE 
  FALSE : if voting is Not oK
  TRUE  : if voting is ok
...
boolean ds_3gpp_enforce_vote_criteria_for_voice_call
(
  sys_modem_as_id_e_type      subs_id
);
