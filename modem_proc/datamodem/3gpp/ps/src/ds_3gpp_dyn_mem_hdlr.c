...
$Header: //components/rel/data.mpss/3.5..2.1/3gpp/ps/src/ds_3gpp_dyn_mem_hdlr.c#2 $
...
/*-------------------------------------------------------------------------*/
  ...
  {
    DS_3GPP_ERROR_LOG_0_EX(subs_id, "Invalid index passed");
    return rval;
  }
...
/*------------------------------------------------------------------------- 
 ...
 --------------------------------------------------------------------------*/
     ...
     (bearer_context_p->ds_bearer_context_dyn_p != NULL))
  {
    ...
    DS_3GPP_MSG1_LOW("ds_3gpp_dyn_mem_hdlr_free_bearers_dyn_mem: "
                      "status [%d] for UMTS bearer cnxt",
                      rval1);
    ...
    DS_3GPP_MSG1_LOW("ds_3gpp_dyn_mem_hdlr_free_bearers_dyn_mem: "
                      "status [%d] for LTE eps bearer cnxt",
                      rval2);
  ...
  {
		DS_3GPP_ERROR_LOG_0_EX(subs_id, "Bearer context is invalid");
  }
} /* ds_3gpp_dyn_mem_hdlr_free_bearers_dyn_mem */
...
/*-------------------------------------------------------------------------*/
  ...
  if(rval == FALSE )
  {
    DS_3GPP_ERROR_LOG_0_EX(subs_id, "Error in freeing UMTS bearer flow manager");
  }
  return;
...
/*-------------------------------------------------------------------------*/
  ...
  {
    DS_3GPP_ERROR_LOG_0_EX(subs_id, "Index out of bounds");
    return ret_val;
  }
...
/*-------------------------------------------------------------------------*/
  ...
  if(rval == FALSE )
  {
    DS_3GPP_ERROR_LOG_0_EX(subs_id, "Error in freeing eps bearer flow manager");
  }           
#endif
...
/*-------------------------------------------------------------------------*/
  ...
  rex_undef_timer(&(pdn_context_p->ds_pdn_context_dyn_p->tcp_retrans_protect_timer));

  rex_undef_timer(&(pdn_context_p->ds_pdn_context_dyn_p->unused_pdn_timer));
...
/*-------------------------------------------------------------------------*/
  if((bearer_context_p == NULL) || 
     (bearer_context_p->ds_bearer_context_dyn_p == NULL))
  {
    DS_3GPP_ERROR_LOG_0("Invalid bearer context passed");
    return;
  }
  ...
  if(ds_3gpp_pdn_cntxt_get_subs_id_from_pdn_internal(pdn_context_p, &subs_id)== FALSE)
  {
    return;
  }
  ...
  -----------------------------------------------------------------------------*/
   ...
   if(free_all_dyn_mem)
   {
     ...
     DS_3GPP_MSG0_HIGH_EX(subs_id, "Free all dyn mem");
...
/*-------------------------------------------------------------------------*/
  ...
  if(rval == FALSE )
  {
    DS_3GPP_ERROR_LOG_0_EX(subs_id, "Error in freeing eps flow manager");
  }          
#endif
...
/*-------------------------------------------------------------------------*/
  ...
  if(rval == FALSE )
  {
    DS_3GPP_ERROR_LOG_0_EX(subs_id, "Error in freeing umts pdn context");
  }
  return;
