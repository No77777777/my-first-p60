...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp/ps/src/comptask.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_3GPP_ERROR_FATAL("q_init comp_cmd_q failed");
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  cmd_ptr->hdr.subs_id = ds3g_get_ps_subs_id();
    ...
    switch( cmd_ptr->hdr.cmd_id )
    {
      ...
      /*-----------------------------------------------------------------------
        When RRC notifies RB data rate indication to COMP.
      -----------------------------------------------------------------------*/
      case PDCP_RRC_DATA_RATE_IND:
        pdcp_rrc_data_rate_ind_cmd_hdlr( cmd_ptr );
        break;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DATA_MSG0_LOW("comp_get_cmd_buf");
  /*-------------------------------------------------------------------------
  ...
  if(cmd_ptr == NULL)
  {
    DS_3GPP_ERROR_FATAL("comp_get_cmd_buf: Memory allocation failed");
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS3GPP_ASSERT( sig_handler != NULL );
...
comp_sig_enum_type comp_reserve_to_comp_tx_sig(void)
{
    ...
    DS_3GPP_ERROR_FATAL("Cannot reserve tx signal!");
