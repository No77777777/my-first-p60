...
$Header: //components/rel/data.mpss/3.5..2.1/3gpp/ps/src/ds_3gppi_utils.c#3 $
...
)
{
  ...
  if( ds_3gpp_nv_manager_get_nv_umts_rel_version(subs_id) >= 
  ...
  return FALSE;
} /* ds_3gppi_is_rel_11_or_above_supported */
...
)
{
  if( ds_3gpp_nv_manager_get_nv_umts_rel_version(subs_id) >= 
      ...
      ds_3gpp_nv_manager_get_nv_lte_rel_version(subs_id) >= LTE_3GPP_REL12 )
  {
    return TRUE;
  }

  return FALSE;
} /* ds_3gppi_is_rel_12_or_above_supported */
...
{
#ifndef FEATURE_RELEASE_13_UNSUPP
  if( ds_3gpp_nv_manager_get_nv_umts_rel_version(subs_id) >= 
                                                 NV_NAS_COMPLIANCE_REL13  ||
      ds_3gpp_nv_manager_get_nv_lte_rel_version(subs_id) >= LTE_3GPP_REL13 )
  {
    return TRUE;
  }
#endif /* FEATURE_RELEASE_13_UNSUPP */
  return FALSE;
} /* ds_3gppi_is_rel_13_or_above_supported */
...
{
#ifdef FEATURE_RELEASE_14
  if( ds_3gpp_nv_manager_get_nv_umts_rel_version(subs_id) >= 
                                                 NV_NAS_COMPLIANCE_REL14  ||
      ds_3gpp_nv_manager_get_nv_lte_rel_version(subs_id) >= LTE_3GPP_REL14 )
  {
    return TRUE;
  }
#endif /* FEATURE_RELEASE_14 */
  return FALSE;
} /* ds_3gppi_is_rel_14_or_above_supported */
...
)
{
  ...
  if( ds_3gpp_nv_manager_get_nv_umts_rel_version(subs_id) >= 
                                                 NV_NAS_COMPLIANCE_REL15  ||
      ds_3gpp_nv_manager_get_nv_lte_rel_version(subs_id) >= LTE_3GPP_REL15 )
  {
    return TRUE;
  }
...
} /* ds_3gppi_is_rel_15_or_above_supported */
...
{
   return(ds_3gppi_is_rel_11_or_above_supported(subs_id) && 
         (!ds_3gppi_is_rel_12_or_above_supported(subs_id)));
}  /*ds_3gppi_only_rel_11_supported */

/*===========================================================================
FUNCTION ds_3gpp_is_lte_only_rel_11_or_below_supported

DESCRIPTION
  Determine if UE support LTE release 11 or below versions
...
{
#ifdef FEATURE_DATA_LTE
  if(ds_3gpp_nv_manager_get_nv_lte_rel_version(subs_id) < LTE_3GPP_REL11_MAX)
  {
    return TRUE;
  }
#endif /* FEATURE_DATA_LTE */
  ...
  return FALSE;
} /* ds_3gppi_is_lte_only_rel_11_or_below_supported */
...
{
  int result = -1;
  DS3GPP_ASSERT(info_p != NULL);
  DS3GPP_ASSERT(path != NULL);
...
)
{
  ...
  if (!val)
  {
    DS_3GPP_ERROR_LOG_0("val ptr is NULL");
      goto err;
  }
  ...
  if (!info_p)
  {
    DS_3GPP_ERROR_LOG_1("Failed to get item info, item d", (int)item);
  }
...
)
{
  ...
  if ((ret_val <=0)|| (attach_fb_info.fallback_enabled == FALSE))
  {
    ...
    DS_3GPP_MSG0_HIGH_EX(subs_id,
           "LTE Attach Dual IP fall back not set. Disabling feature");
  ...
  else
  {
    /*-----------------------------------------------------------------------
      NV read successful
      -----------------------------------------------------------------------*/
    DS_3GPP_MSG1_HIGH_EX(subs_id, "LTE Attach Dual IP fall back NV set to %d",
...
)
{
 ...
 /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ((from == NULL) || (to == NULL) || (from > to))
  {
    DS_3GPP_MSG2_HIGH_EX(subs_id,
                     "apn disconnect order list:Store APN identifier failed."
  ...
  if (identifier_type == DS_PROFILE_3GPP_PROFILE_PARAM_PDP_CONTEXT_APN)
  {
    ...
    DS_3GPP_MSG2_MED_EX(subs_id,
                      "apn disconnect order list:Identifier type:%d,"
  ...
  for (idx =0; idx<DS_3GPP_MAX_APN_IDENTIFIERS;idx++)
  {
    ...
    if (strncasecmp((char*)apn_disconn_order_list_p->identifier,
                    (char*)from,str_len)==0)
    {
      DS_3GPP_MSG1_MED_EX(subs_id, "Matching Identifier found at index %d",idx);
      break;
    }
  }
  /*----------------------------------------------- 
 ...
 /*--------------------------------------------------------------------------
  ...
  {
    DS_3GPP_MSG0_HIGH_EX(subs_id, "apn disconnect order list: Mem alloc failed");
    return ret_val;
  }
...
void ds_3gppi_utils_read_apn_disconn_order_list_frm_efs(sys_modem_as_id_e_type subs_id)
{
  ...
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  DS_3GPP_MSG0_MED_EX(subs_id, "Read APN disconn config from EFS");
  ...
  ---------------------------------------------------------------------------*/
  if(ds3gcfgmgr_efs_file_init_ex(file_name, subs_id, &efs_db) == -1)
  {
    DS_3GPP_MSG0_HIGH_EX(subs_id, "apn disconnect order list: Error reading list from EFS");
    return;
  }
  /*----------------------------------------------------
          ...
          != (ret_val = ds3gcfgmgr_efs_tokenizer(&efs_db, &from, &to )))
  {
    ...
    {
      DS_3GPP_ERROR_LOG_0_EX(subs_id, "Token Parse Failure");
      break;
    }
    /*------------------------------------------------------------------------- 
    ...
    else if(DS3GCFGMGR_EFS_TOKEN_PARSE_SUCCESS == ret_val)
    {
      ...
      {
        DS_3GPP_MSG1_HIGH_EX(subs_id, 
                             "apn disconnect order list: Cannot accept more"
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
     ...
     from > to)
  {
    DS_3GMGR_ERROR_LOG_3_EX(MSG_LEGACY_ERROR, subs_id,
                    "Invalid Parameters from: 0xx to: 0xx Queue ptr: 0xx",
  ...
  if(-1 == ret_val)
  {
    DS_3GMGR_ERROR_LOG_0_EX(MSG_LEGACY_ERROR, subs_id, "Memeory allocation failure !!!!");
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_3GMGR_ERROR_LOG_3_EX(MSG_LEGACY_ERROR, subs_id,
                    "Invalid Input Params from:0xx to:0xx compare string:0xx",
                    from, to, comp_string);
    return ret_val;
  }
  ...
  {
    DS_3GMGR_ERROR_LOG_0_EX(MSG_LEGACY_ERROR, subs_id, "Invalid Token ID");
    return ret_val;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_3GMGR_ERROR_LOG_3_EX(MSG_LEGACY_ERROR, subs_id,
                    "Invalid Input Params, File Name: 0xx "
                    "Compare String: 0xx EFS queue 0xx", 
  ...
  {
    DATA_3GMGR_MSG0_EX(MSG_LEGACY_HIGH, subs_id,
                       "Failure reading PDN throttling cnfg");
    return return_val;
  }
          ...
          != (ret_val = ds3gcfgmgr_efs_tokenizer(&efs_db, &from, &to)))
  {
    ...
    else if(DS3GCFGMGR_EFS_TOKEN_PARSE_SUCCESS == ret_val)
    {
      ...
      else 
      {
        ...
        {
          DS_3GMGR_ERROR_LOG_0_EX(MSG_LEGACY_ERROR, subs_id,
                                  "Failure filling token contents");
          return return_val;
        }
        /*-------------------------------------------------------------------
...
/*----------------------------------------------------------------------*/
  ...
  for (i = 0; i < (int)SYS_SYS_MODE_MAX; ++i) {
    ...
    case SYS_SYS_MODE_NR5G:
      ds_3gppi_mode_index[i] = (int)DS_3GPPI_SYS_MODE_5G;
      break;
#endif /* FEATURE_LAPP */
#endif
#ifdef FEATURE_WCDMA
...
/*-------------------------------------------------------------------------*/
  ...
  {
    DS_3GPP_ERROR_LOG_0("Failed to allocate memory");
    return -1;
  }
  ...
  for (i = 0; i < DS_3GPP_CFG_MAX; i ++)
  {
         ...
         info_p, nv_item_file_path, DS_3GPPI_CFG_MAX_PATH_SIZE) == TRUE)
    {
      ...
      {
        DS_3GPP_ERROR_LOG_1("Fail to copy all files path in buffer file_paths_buf_len d",
                            file_paths_buf_len);
        return -1;
      }
    ...
    {
      DS_3GPP_ERROR_LOG_1("ds_3gppi_cfg_get_item_path failed for item d skipping",
                          i);
    }
  }
...
)
{
     ...
     (ret_mnc == NULL))
  {
    DS_3GPP_ERROR_LOG_3_EX(subs_id,
                           "Input param error:PLMN ID:0xx, ret_mcc:0xx, "
                           "ret_mnc:0xx",current_plmn_id,ret_mnc,ret_mcc);
    return;
  }
  ...
  *ret_mcc = mcc;
  ...
  DS_3GPP_MSG1_HIGH_EX(subs_id, "MCC returned is %d",mcc);
  ...
  DS_3GPP_MSG3_HIGH_EX(subs_id, "MNC: %d%d%d",mnc_digit[0],mnc_digit[1],mnc_digit[2]);
  if (mnc_digit[2] == 0xF)
  {
  ...
  *ret_mnc = mnc;
  ...
  DS_3GPP_MSG1_HIGH_EX(subs_id, "MNC returned is %d",mnc);
   return;
...
)
{
  ...
  if ( (ret_val = ds3g_mmgsdi_read_nas_config_file(tag_id, subs_id, tag_value_p)) 
         == FALSE)
  {    
    ...
    if (efnasconfig != NULL)
    {
      if(!ds3g_mmgsdi_read_nas_config_tag((uint8 *)efnasconfig, 
                                           DS_3GPP_EFNAS_CONFIG_SIZE,
                                           tag_id, 
                                           subs_id, 
                                           tag_value_p))
      {
        DS_3GPP_ERROR_LOG_1_EX(subs_id, "Requested tag 0xx was not found, Exit", tag_id);
      }
      else
      {
        ret_val = TRUE;
  ...
  return ret_val;
} /* ds_3gpp_read_read_nas_config_file */
...
{
  ps_sys_pdp_enum_type ps_sys_pdp_type = PS_SYS_PDP_TYPE_IPV4;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/ 
  
  switch(pdn_pdp_type)
  {
    case DS_UMTS_PDP_IPV4:
         ps_sys_pdp_type = PS_SYS_PDP_TYPE_IPV4;
         break;
    case DS_UMTS_PDP_IPV6:
         ps_sys_pdp_type = PS_SYS_PDP_TYPE_IPV6;
         break;
    case DS_UMTS_PDP_IPV4V6: 
         ps_sys_pdp_type = PS_SYS_PDP_TYPE_IPV4V6;
         break;
    case DS_UMTS_PDP_PPP:
         ps_sys_pdp_type = PS_SYS_PDP_TYPE_PPP;
         break;
    default:
         break;
   }/* switch */

  return ps_sys_pdp_type;
}/*ds_3gpp_map_ds_umts_pdp_type_to_ps_sys_pdp_type*/
