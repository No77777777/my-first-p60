...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp/ps/src/ds_um_lo.c#2 $
...
dsumtsps_loopback_compute_checksum(unsigned char * pkt, long lpkt, long offset)
{
    ...
    if (!pkt || (lpkt == 0) || (offset > lpkt)) {
      DS_3GPP_MSG0_LOW("dsumtsps_compute_checksum sanity check fail");
      return 0;
    }
...
dsumtsps_loopback_compute_icmp_checksum(unsigned char * pkt, long lpkt)
{
    ...
    if (!pkt || (lpkt < IP_HDR_LENGTH + ICMP_CHECKSUM_OFFSET + 2)) {
      DS_3GPP_MSG0_LOW("dsumtsps_compute_icmp_checksum sanity check fail");
      return 0;
    }
...
dsumtsps_loopback_compute_udp_checksum(unsigned char * pkt, long lpkt)
{
    ...
    if (tmp_pkt == NULL)
    {
      DS_3GPP_MSG0_LOW("dsumtsps_loopback_compute_udp_checksum memory allocation failure");
      return 0;
    }
    ...
    {
      DS_3GPP_MSG0_LOW("dsumtsps_compute_udp_checksum sanity check fail");
      return 0;
    }
...
dsumtsps_loopback_compute_icmp6_checksum(unsigned char * pkt, long lpkt)
{
    ...
    if (!pkt || (lpkt < IPV6_HDR_LENGTH + ICMP_CHECKSUM_OFFSET + 2)) {
      DS_3GPP_MSG0_LOW("dsumtsps_compute_icmp6_checksum sanity check fail");
      return 0;
    }
    ...
    DS_3GPP_MSG1_LOW("Payload Length %d", ph.length);
    ph.protocol = (uint8)PS_IPPROTO_ICMP6;

    dsm_item_ptr = dsm_new_buffer(DSM_DS_LARGE_ITEM_POOL);
    if(dsm_item_ptr == NULL)
    {
      DS_3GPP_MSG0_LOW("Ran out of DSM buffers, cannot compute icmpv6 checksum");
      return 0;
    }
...
)
{
  ...
  if (((ip6_base_hdr_type*)data_ptr)->next_hdr == 58)
  {
    ...
    if (((dsumtsps_icmp6_gen_format*)(data_ptr + sizeof(ip6_base_hdr_type)))->type 
                                                                      == 133)
    {
      DS_3GPP_MSG0_LOW("RoSol msg received, composing Ro Ad.");
  ...
  else
  {
    ...
    if((((ip6_base_hdr_type*)data_ptr)->next_hdr == 17) || 
       ((((ip6_base_hdr_type*)data_ptr)->next_hdr == 44) &&
       (((ds_3gpp_ip6_ext_hdr_type*)data_ptr)->prot == 17)))
    {
      memscpy(&src_addr,
              sizeof(struct ps_in6_addr),
              &(((ip6_base_hdr_type*)data_ptr)->dst_addr),
...
{
  DS_3GPP_MSG0_LOW("Low watermark hit");
...
{
  DS_3GPP_MSG0_LOW("High watermark hit");
...
)
{
 ...
 /*--------------------------------------------------------------------------
  ...
  if (data_ptr == NULL)
  {
     DS_3GPP_MSG0_LOW("dsumtsps_loopback_convert_packet memory allocation failed");  
     bRet = FALSE;
     return bRet;
  }
  ...
  /*-------------------------------------------------------------------------
    Modify the packet and put it back into a DSM item to loop it back:
     ...
     - If this is an ICMP pkt, assume that it's a ping request and change 
       it to ping response 
  -------------------------------------------------------------------------*/
  //DS_3GPP_MSG0_LOW("ip tx loopback um data cb ");    
  /*-------------------------------------------------------------------------
  ...
  if ( (((dsumtsps_ip_format*)data_ptr)->bProt == 17) &&
    ...
    {      
      DS_3GPP_MSG0_LOW("UDP checksum verification failed!!pkt corrupted");
      bRet = FALSE;
    }
  }
  ...
  if (((dsumtsps_ip_format*)data_ptr)->bProt == 1) {    
    ...
    {
      DS_3GPP_MSG0_LOW("ICMP checksum verification failed!!pkt corrupted");
      bRet = FALSE;
    }
  } 
  ...
  if ((((dsumtsps_ip_format*)data_ptr)->bProt == 17) &&
      ...
      (dsumtsps_loopback_udp_chksum_verif == TRUE) ) {
    DS_3GPP_MSG3_LOW("UDP checksum verification failed!! orig=%d new=%d",orig_chksum,
             new_chksum,0);
    bRet = FALSE;
  }
...
)
{
  ...
  if (*item_ptr == NULL)
  {
    DS_3GPP_MSG0_LOW("NULL DSM item from dsm_pushdown");
    /*lint -e506 -e774*/
    PS_META_INFO_FREE(&meta_info_ptr);
    /*lint +e506 +e774*/
    return;
...
{   
    ...
    if(pdn_cntx_p->ds_pdn_context_dyn_p->def_bearer_context_ptr == NULL)
    {
        ...
        if(ret_val != 0)
        {
            DS_3GPP_MSG1_LOW("binding flow to phys link failed:error=%d",ret_val);
            DS3GPP_ASSERT(0);
            return;
    ...
    DS_3GPP_MSG3_LOW("iface up cmd hdlr PDN cntx Iface:0x%x :info:%lx",\
    ...
    if(ps_iface_set_addr(iface_p, &ip_addr)!=0)
    {
       DS_3GPP_MSG1_HIGH_EX(subs_id,
        "LPBK: Setting address failed for iface_ptr: %d",iface_p);
    }
...
// Loopback specific Water mark for Non LTE Loopback RAT
    ...
    (void) ps_phys_link_set_link_protocol(&(bearer_context_p->ds_bearer_context_dyn_p->  
                                          phys_link), PS_PHYS_LINK_LINK_PROTOCOL_NONE,
                                          protocol_handle);
    ...
    ps_phys_link_enable_flow(&(bearer_context_p->ds_bearer_context_dyn_p->phys_link),
                             DS_FLOW_LTE_PDCP_MASK|DS_FLOW_LTE_PDCP_DNE_MASK|DS_FLOW_UMTS_MASK);
