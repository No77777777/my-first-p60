...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp/ps/src/dsumtspsmthdlr.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
)
{
  ...
  {
    DS_3GPP_MSG1_LOW("MT_UMSM_CLEAN_UP: invalid index, mt_index [%d]",
                        mt_index);
    return;
  }

  DS_3GPP_MSG1_LOW("MT_UMSM_CLEAN_UP: cleaning up MT-UMSM, mt_index [%d]",
                     mt_index);
  ...
  {
    DS_3GPP_MSG1_LOW("Timer ptr is NULL, mt_index %d ", mt_index);
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if ( !DSUMTSPS_MT_INDEX_IS_VALID ( mt_index ) ) 
  {
    DS_3GPP_MSG1_LOW("MT_UMSM_TRANS_STATE: invalid mt_index [%d]", 
                        mt_index);
    DS3GPP_ASSERT(0);
    return;
  }

  DS_3GPP_MSG3_LOW("MT_UMSM_TRANS_STATE: mt_index [%d], new MT-UMSM state [%d]\
  ...
  switch (new_um_state) 
  {
    ...
    DS_3GPP_MSG0_LOW("UMSM_TRANSITION_STATE: MT_UM_BRINGING_RMSM_UP_STATE\
  ...
  case DSUMTSPS_MT_UMSM_NULL_STATE:
    ...
    DS_3GPP_MSG0_LOW("UMSM_TRANSITION_STATE: MT_UM_NULL_STATE entry actions completed!");
    break;
  default:
    DS_3GPP_MSG1_LOW("Unhandled UMSM_TRANSITION_STATE: %d", new_um_state);
  }
  return;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if ( !DSUMTSPS_MT_INDEX_IS_VALID ( mt_index ) ) 
  {
    DS_3GPP_MSG1_LOW("MT_UMSM_EV_HDLR: invalid mt_index %d", 
                        mt_index);
    DS3GPP_ASSERT(0);
    return;
  }

  DS_3GPP_MSG2_LOW("MT_UMSM_EV_HDLR: current MT-UMSM state [%d], event posted [%d]",
  ...
  case MT_UMSM_CALL_RING_EV:
    ...
    {
      DS_3GPP_MSG2_LOW("MT_UMSM_EV_HDLR: trap, MT-UMSM invalid state [%d], mt_index [%d]!",
  ...
  case MT_UMSM_CALL_REJECT_EV:
    ...
    if ( dsumtsps_mt_timer_table_ptr[mt_index]->um_state == DSUMTSPS_MT_UMSM_BRINGING_RM_UP_STATE )
    {
      ...
      DS_3GPP_MSG0_LOW("MT_UMSM_EV_HDLR: MT_UM_BRINGING_RMSM_UP_STATE exit actions completed!");
      dsumtsps_mt_umsm_transition_state ( mt_index, 
                                          DSUMTSPS_MT_UMSM_NULL_STATE, event_data );
    }
    else
    {
      DS_3GPP_MSG2_LOW("MT_UMSM_EV_HDLR: trapped REJECT_EV, \
  ...
  case MT_UMSM_CALL_ANS_TIMEOUT_EV:
    ...
    {
      DS_3GPP_MSG2_LOW("MT_UMSM_EV_HDLR: trap TIMEOUT_EV, \
  ...
  default:
    DS_3GPP_MSG1_LOW("MT_PPP_EV_HDLR: Unknown event:%d",event);
    DS3GPP_ASSERT(0);
    return;
...
)
{
  ...
  {
    DS_3GPP_MSG1_LOW("Invalid PDP type, local_pdp_type = %d",
                        (int)local_pdp_type);
    DS3GPP_ASSERT(0);
  }
  return nas_pdp_type;
...
)
{
  ...
  {
    DS_3GPP_ERROR_FATAL("No Ioctl info available");
    *ps_errno = DS_EFAULT;
    return -1;
  ...
  {
    DS_3GPP_MSG0_LOW("PPP socket calls are not supported, reg MT failed");
    *ps_errno = DS_EFAULT;
    return -1;
  }
  ...
  {
    DS_3GPP_MSG1_LOW("Unsupported type (%d) of socket call, reg MT failed", 
                        pdp_type);
    *ps_errno = DS_EFAULT;
    return -1;
  }
  ...
  for(i=0;i<DSUMTSPS_MT_MAX_CB; i++)
  {
    if(dsumtsps_mt_reg_table_ptr[i] == NULL)
    {
      ...
      DS_3GPP_MSG2_LOW("Reg MT cb, handle %d, pdp_type %d",
  ...
  -------------------------------------------------------------------------*/
    DS_3GPP_MSG2_LOW("Could not get an idx in MT table, base_val %d,i %d", 
...
)
{
  ...
  if(callback_info == NULL)
  {
    DS_3GPP_ERROR_FATAL("No Ioctl info for de-registering cb");
    *ps_errno = DS_EFAULT;
    return -1;
  }
  /*-------------------------------------------------------------------------
  ...
  {
    DS_3GPP_MSG1_LOW("Invalid handle %d for de-registering MT callback", 
      ...
      (dsumtsps_mt_reg_table_ptr[index]->handle == callback_info->handle) )
  {
    ...
    DS_3GPP_MSG3_LOW("De-reg MT cb, handle %d, base_val %d, idx %d", 
                  callback_info->handle, dsumtsps_mt_handle_base_val, index);
    return 0;
  }
  else
  {
    DS_3GPP_MSG1_LOW("Invalid handle value %d", callback_info->handle); 
    *ps_errno = DS_EFAULT;
    return -1;
  }
...
{
  DS_3GPP_MSG0_LOW("dsumtsps_mt_handle_timer_cb reg not used. Return");
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  *data_ptr = (uint8)cb_param;
  ...
  DS_3GPP_MSG2_LOW("RM_ANS_TIMEOUT: MT timer expired,\
...
)
{
  ...
  DS_3GPP_MSG0_LOW("MTHDLR_MATCH_EMB_PROFILE: PDP-IP call!");
  ...
  for(i=0;i<DSUMTSPS_MT_MAX_CB; i++)
  {
       ...
       (dsumtsps_mt_reg_table_ptr[i]->pdp_type == nas_pdp_type))
    {
      ...
      DS_3GPP_MSG0_LOW("MTHDLR_MATCH_EMB_PROFILE: APN and PDP type matched!");
      ...
      if(app_registered == FALSE)
      {
           ...
           if( dsumtsps_mt_timer_table_ptr[index] == NULL )
           {
             ...
             if((dsumtsps_mt_timer_table_ptr[index]->mt_timer_ptr =
                   rex_create_timer_ex(dsumtsps_mt_handle_timer_cb,
                     (uint32)(index))) != NULL )
             {
               DS_3GPP_MSG3_LOW("MTHDLR_MATCH_PROFILE: Start tmr for MT, callid %d, mt_idx %d, idx %d",
                   call_id, i, index);
             ...
             {
               DS_3GPP_MSG0_LOW("MTHDLR_MATCH_PROFILE: Tmr ptr for rex timer is NULL"); 
        ...
        {
          DS_3GPP_MSG0_LOW("MTHDLR_MATCH_PROFILE: No index available for MT timer");
  ...
  default:
    DS_3GPP_MSG1_LOW("MTHDLR_MATCH_EMB_PROFILE: Invalid PDP Type! ds_pdp_type:%d",
                        ds_pdp_type);
    return FALSE;
...
)
{
  ...
  if( (cmd_ptr == NULL) || (cmd_ptr->cmd_payload_ptr == NULL) )
  {
    DS_3GPP_MSG0_LOW( MSG_LEGACY_LOW, "NULL pointer passed" );
    return FALSE;
  }
  ...
  case DS_UMTS_PDP_PPP:
    ...
    DS_3GPP_MSG1_LOW("MT_REG_TBL_STORE_INFO: pdp_type [%d], PDP activ req, store in MT tbl",
                      ds_pdp_type);
  ...
  for(index = 0; index < DSUMTSPS_MT_MAX_CB; index ++)
  {
    if(dsumtsps_mt_timer_table_ptr[index] == NULL)
    {
      ...
      if(dsumtsps_mt_timer_table_ptr[index] == NULL)
      {
        DS_3GPP_MSG3_LOW("Memory allocation failure for client:%d, size:%d",
      ...
      *mt_index = index;
      ...
      {
        DS_3GPP_MSG0_LOW("apn->valid flag not set, apn_name not\
                           filled in table");
      }
      /*lint +e534*/
      ...
      DS_3GPP_MSG3_LOW("Stored in table mt_index %d call_id %d\
       ...
       if((dsumtsps_mt_timer_table_ptr[index]->mt_timer_ptr =
             rex_create_timer_ex(dsumtsps_mt_ans_timeout_cb,
               (uint32)(index))) != NULL )
       {
         DS_3GPP_MSG2_LOW("MT_REG_TBL_STORE_INFO: Start tmr for MT, cm_call_id [%d], mt_index [%d]",
                            call_id, index);
         (void) rex_set_timer(dsumtsps_mt_timer_table_ptr[index]->mt_timer_ptr,
       ...
       {
         DS_3GPP_MSG0_LOW("MT_REG_TBL_STORE_INFO: Tmr ptr for rex timer is NULL"); 
         ds_system_heap_mem_free(dsumtsps_mt_timer_table_ptr[index]);
  ...
  if(index == DSUMTSPS_MT_MAX_CB)
  {
    DS_3GPP_MSG0_LOW("MT_REG_TBL_STORE_INFO: No index available for MT timer");
    return FALSE;
  }
  ...
  default:
    DS_3GPP_MSG1_LOW("MT_REG_TBL_STORE_INFO: Invalid PDP Type! ds_pdp_type [%d]",
                        ds_pdp_type);
    return FALSE;
...
)
{
  ...
  {
    DS_3GPP_MSG1_LOW("MT_GET_MT_INFO: invalid input params, mt_index [%d]",
                        mt_index );
    return FALSE;
  }
  ...
  DS_3GPP_MSG3_LOW("MT_GET_MT_INFO: pdp_type %d apn_len %d rmsm_inst %d",
...
)
{
  ...
  {
     DS_3GPP_MSG0_LOW("Umts PDN cntxt passed is not valid");
     return call_id;
   }
#endif /* FEATURE_DATA_TE_MT_PDP */
  ...
  { 
    ...
    if (iface_ptr == NULL )
    {
      DS_3GPP_MSG0_LOW("No valid iface pointer in mt_check_call_orig");
      return call_id;
    }
  }
  /*-------------------------------------------------------------------------
  ...
  {
     DS_3GPP_MSG1_LOW("encoded_app_len is %d that greater than DS_UMTS_MAX_APN_STRING_LEN", 
                      encoded_apn_len);
     return call_id;
  }
  ...
  {
    DS_3GPP_MSG2_LOW(" Unknown PDP type in profile, profile %d, PDP type %d", 
  ...
  for(j=0;j<DSUMTSPS_MT_MAX_CB; j++)
  {
    if(dsumtsps_mt_timer_table_ptr[j] != NULL)
    {
      ...
      -----------------------------------------------------------------------*/
      DATA_3GPP_MSG_SPRINTF_3(MSG_LEGACY_LOW,"dsumtsps_mt_check_call_orig"
      ...
      if(((dsumtsps_mt_timer_table_ptr[j]->pdp_type == pdp_type)||
        ...
        ((strncasecmp((char *)dsumtsps_mt_timer_table_ptr[j]->apn_name, 
                     (char *)encoded_apn,
                     DS_UMTS_MAX_APN_STRING_LEN+1))== 0 ||
         (( 0 == strlen( (char *)apn_name ) ))))
      {
        DS_3GPP_MSG1_LOW("MT call orig, delete rex timer, call_id %d",
  ...
  DS_3GPP_MSG1_LOW("Mt_check_call_orig, returning call_id %d", call_id);
  return call_id;
...
)
{
  ...
  {
    DS_3GPP_MSG1_LOW("dsumtsps_mt_send_reject: invalid index, mt_index [%d]",
                        index);
    return;
  }
  call_id = dsumtsps_mt_timer_table_ptr[index]->call_id;

  DS_3GPP_MSG2_LOW("MT_SEND_REJECT: mt_index [%d], call id %d HANGUP_CALL()!", 
                    index, call_id);
                                                             ...
                                                             == DS3G_FAILURE)
  {
    ...
    -----------------------------------------------------------------------*/
    DS_3GPP_MSG1_LOW("MT_SEND_REJECT: Call already ended call_id :%d", 
                       call_id);
  }
