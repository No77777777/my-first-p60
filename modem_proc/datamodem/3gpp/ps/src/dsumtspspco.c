...
Copyright (c) 2002-2016 Qualcomm Technologies, Incorporated.  All Rights Reserved.
...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp/ps/src/dsumtspspco.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
func_exit:
  ...
  {
    DS_3GPP_ERROR_LOG_1("dswpsd_ppp_gen_pap_auth_req_pkt: err d", local_err);
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(pkt_buff_ptr == NULL || challenge_ptr == NULL || sys_name_ptr == NULL)
  {
    DS3GPP_ASSERT(0);
    chap_pkt_len = -1;
    //"Null ptr: pkt_buff_ptr %p, challenge_ptr %p, sys_name_ptr %p",
    local_err = DS_3GPPI_LOCAL_ERR_VAL_0;
    goto func_exit;
  } 
  ...
  *cp++ = challenge_len; 
  ...
  DS_3GPP_MSG2_HIGH("chap challenge len :%d chap sys name len :%d",
                    challenge_len, sys_name_len);
...
func_exit:
  if ( local_err != DS_3GPPI_LOCAL_ERR_VAL_NONE)
  {
    DS_3GPP_ERROR_LOG_1("dswpsd_ppp_gen_chap_challenge_pkt: err d", local_err);
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
                                                        ...
                                                        sys_name_ptr == NULL)
  {
    DS3GPP_ASSERT(0);
    ret_val = -1;
    //"Null ptr: pkt_buff_ptr %p, challenge_ptr %p,"
    //" secret_ptr %p, sys_name_ptr %p",
    local_err = DS_3GPPI_LOCAL_ERR_VAL_0;
    goto func_exit;
  } 
  ...
  DS_3GPP_MSG3_HIGH_EX(subs_id,
                       "Generating CHAP Res pkt: CHAP Chal :%d Sec :%d Nam:%d",
  ...
  DS_3GPP_MSG2_HIGH_EX(subs_id, "Chap Chal res pkt len :%d pkt len %d",
                       chap_response_length,
                       pkt_buff_len);
  ...
  *bp++ = identifier;
  ...
  DS_3GPP_MSG0_HIGH_EX(subs_id, "Starting the response generation");
  ...
  *cp++   = MD5_DIGEST_SIZE;                  /* Fill in value size field  */
  ...
  ret_val = (int)chap_response_length;

func_exit:
  if ( local_err != DS_3GPPI_LOCAL_ERR_VAL_NONE)
  {
    DS_3GPP_ERROR_LOG_1_EX(subs_id, "ppp_gen_chap_response_pkt: err d", local_err);
  }
  return ret_val;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS3GPP_ASSERT(0);
    //"Null ptr: pkt_buff_ptr %p, hash_ptr %p, "
    //"responder_name_ptr %p",
    ret_val = -1;
    local_err = DS_3GPPI_LOCAL_ERR_VAL_0;
    goto func_exit;
  } 

  DS_3GPP_MSG0_HIGH_EX(subs_id, "constructing CHAP Res pkt from parms passed");
  /*-------------------------------------------------------------------------
  ...
  DS_3GPP_MSG2_HIGH_EX(subs_id, "hash len:%d resp len:%d",
                       hash_len,responder_name_len);
  ...
  DS_3GPP_MSG2_HIGH_EX(subs_id, "Chap Chal res pkt len :%d pkt len",
                    chap_response_length,
                    pkt_buff_len);
...
func_exit:
  if ( local_err != DS_3GPPI_LOCAL_ERR_VAL_NONE)
  {
    DS_3GPP_ERROR_LOG_1_EX(subs_id, "ppp_fill_chap_response_pkt: err d",
                           local_err);
  }
...
{
  int       dns_pkt_len = -1;                    /* Length of IPCP packet */  
...
func_exit:
  if ( local_err != DS_3GPPI_LOCAL_ERR_VAL_NONE)
  {
    DS_3GPP_ERROR_LOG_3("dsumtsps_gen_ipcp_req_pkt: err d req pkt len d pkt len d",
                       local_err, dns_pkt_len, protocol_pkt_len);
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if( protocol_pkt_ptr == NULL ||
      pcscf_v4_addr_array == NULL ||
      num_v4_pcscf_addresses == NULL ||
      pcscf_v6_addr_array == NULL ||
      num_v6_pcscf_addresses == NULL)
  {
    local_err = DS_3GPPI_LOCAL_ERR_VAL_0;
    goto func_exit;
  }
  ...
  if(protocol_pkt_len  == 0)
  {
   /* address is v6, store it in the v6 array */
    ...
    {
      status = FALSE;
      //"More than max pcscf addresses not supported");
      local_err = DS_3GPPI_LOCAL_ERR_VAL_1;
      goto func_exit;
    }
  ...
  {
    status = FALSE;
    //"Wrong pcscf address, incorrect length!");//
    local_err = DS_3GPPI_LOCAL_ERR_VAL_2;
    goto func_exit;
  }
...
func_exit:
  if ( local_err != DS_3GPPI_LOCAL_ERR_VAL_NONE)
  {
    DS_3GPP_ERROR_LOG_2_EX(subs_id, "decode_v6_pcscf_pkt: err d num_addr d",
                       local_err, num_addr);
  }

  return status;
} /*ds3gpp_decode_v6_pcscf_pkt*/
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  -------------------------------------------------------------------------*/
  if( NULL == protocol_pkt_ptr ||
      NULL == pcscf_v4_addr_array ||
      NULL == num_v4_pcscf_addresses)
  {
  ...
  {
    //"Wrong pcscf address, incorrect v4 length!");
    local_err = DS_3GPPI_LOCAL_ERR_VAL_1;
    goto func_exit;
  }
...
func_exit:
  if ( local_err != DS_3GPPI_LOCAL_ERR_VAL_NONE)
  {
    DS_3GPP_ERROR_LOG_2_EX(subs_id, "ds3gpp_decode_v4_pcscf_pkt: err d num_addr d",
                       local_err, num_addr);
  }
  return status;
}
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if( NULL == protocol_pkt_ptr ||
      NULL == pcscf_addr_array ||
      NULL == num_pcscf_addresses)
  {
    local_err = DS_3GPPI_LOCAL_ERR_VAL_0;
    goto func_exit;
  }
...
func_exit:
  if ( local_err != DS_3GPPI_LOCAL_ERR_VAL_NONE)
  {
    DS_3GPP_ERROR_LOG_2("dsumtsps_decode_pcscf_pkt: err d num_addr d",
                       local_err, num_addr);
  }
  ...
  while (protocol_pkt_len > 0) 
  {
    ...
    switch (config_type) 
    {
      case CONFIG_REQ:
        ...
        DS_3GPP_MSG0_HIGH_EX(subs_id, "CONFIG_REQ IPCP packet recvd");
        /*-------------------------------------------------------------------
        ...
        while (length) 
        {
          ...
          {
            ret_val = DS3G_FAILURE;
            //"Zero option length in IPCP pkt");
            local_err = DS_3GPPI_LOCAL_ERR_VAL_4;
            goto func_exit;
          }
          ...
          switch (opt_type)
          {
            case IPCP_1ADDRESS:
              ...
              {
                ret_val = DS3G_FAILURE;
                //"Invalid IPCP ADDRESS Option Len %d",
                //opt_len);
                local_err = DS_3GPPI_LOCAL_ERR_VAL_5;
                goto func_exit;
              }
            ...
            default:
              DS_3GPP_MSG1_HIGH_EX(subs_id, "Option type not supported %d",
                                   opt_type);
      ...
      case CONFIG_NAK:
         ...
         DS_3GPP_MSG0_HIGH_EX(subs_id, "CONFIG NAK IPCP packet recvd.");
        /*-------------------------------------------------------------------
        ...
        {
          DS_3GPP_MSG1_HIGH_EX(subs_id, "Invalid Identifier received %d",
                               identifier);
          ...
          switch (opt_type)
          {
            case IPCP_PRIMARY_DNS:
              ...
              else
              {
                ...
                protocol_pkt_ptr +=4;
                length -= opt_len;
                DS_3GPP_MSG1_HIGH_EX(subs_id, "CONFIG NACK: Received PDNS %lx",
            ...
            case IPCP_PRIMARY_NBNS:
              ...
              else
              {
                ...
                protocol_pkt_ptr +=4;
                length -= opt_len;
                DS_3GPP_MSG1_HIGH_EX(subs_id, "CONFIG NACK: Received PNBNS %lx",
            ...
            case IPCP_SECONDARY_NBNS:
              ...
              else
              {
                ...
                protocol_pkt_ptr +=4;
                length -= opt_len;
                DS_3GPP_MSG1_HIGH_EX(subs_id, "CONFIG NACK: Received SNBNS %lx",
            ...
            case IPCP_SECONDARY_DNS:
              ...
              else
              {
                ...
                protocol_pkt_ptr +=4;
                length -= opt_len;
                DS_3GPP_MSG1_HIGH_EX(subs_id, "CONFIG NACK: Received SDNS %lx",
                         (unsigned long)ipcp_options->secondary_dns);
              }
            break;

            default:
              DS_3GPP_MSG1_HIGH_EX(subs_id,"Option type not supported %d",opt_type);
              protocol_pkt_ptr += (opt_len - 2);
              length -= opt_len;
          }
        } /* while length */
      ...
      case CONFIG_ACK:
        ...
        DS_3GPP_MSG0_HIGH_EX(subs_id, "CONFIG ACK IPCP packet recvd. ");
        /*-------------------------------------------------------------------
          Check if the identifer matches the value stored in the call table. 
          If not, discard the packet.
        -------------------------------------------------------------------*/
          ...
          switch (opt_type)
          {
            ...
            case IPCP_PRIMARY_NBNS:
              ...
              else
              {
                ...
                {
                  DS_3GPP_MSG1_HIGH_EX(subs_id,
                                "CONFIG ACK: Received non-zero PNBNS %lx",
                                (unsigned long)dns_val);
            ...
            case IPCP_SECONDARY_NBNS:
              ...
              else
              {
                ...
                {
                  DS_3GPP_MSG1_HIGH_EX(subs_id,
                               "CONFIG ACK: Received non-zero SNBNS %lx",
                               (unsigned long)dns_val);
            ...
            default:
              DS_3GPP_MSG1_HIGH_EX(subs_id,
                        "Option type not supported %d",opt_type);
              protocol_pkt_ptr += (opt_len - 2);
              length -= opt_len;
          }
        } /* while (length) */
      ...
      case CONFIG_REJ:
        ...
        DS_3GPP_MSG0_HIGH_EX(subs_id, "CONFIG REJ IPCP packet recvd. -Ignore");
        /*-------------------------------------------------------------------
          Check if the identifer matches the value stored in the call table. 
          If not, discard the packet.
        -------------------------------------------------------------------*/
        ...
        {
          DS_3GPP_MSG1_HIGH_EX(subs_id, 
                     "Invalid Identifier received %d",identifier);
          protocol_pkt_ptr += length;
          continue;
        }
        /*-------------------------------------------------------------------
        ...
        while (length) 
        {
          ...
          -----------------------------------------------------------------*/
          opt_type = *protocol_pkt_ptr++;
          DS_3GPP_MSG1_HIGH_EX(subs_id, "CONFIG_REJ: Opt Type : %d", opt_type);
      ...
      default:
        DS_3GPP_MSG1_HIGH_EX(subs_id, 
            "Invalid IPCP code : Ignore it ",config_type);
        protocol_pkt_ptr += length;
...
func_exit:
  if ( local_err != DS_3GPPI_LOCAL_ERR_VAL_NONE)
  {
    DS_3GPP_ERROR_LOG_2_EX(subs_id, "dsumtsps_decode_ipcp_pkt: err d opt_len",
                       local_err,
                       opt_len);
  }
...
(
  ...
  ds3gsubsmgr_subs_id_e_type   subs_index  /*subscription index   */
)
{
...
)
{
  ...
  if(protocol_pkt_ptr == NULL || num_ipv4_dns == NULL || 
                                      ipv4cp_options == NULL)
  {
    DS_3GPP_MSG0_HIGH_EX(subs_index, 
                         "Invalid arguments while decoding ipcp options");
    return;
  }
  if(protocol_pkt_len != DSUMTS_IPV4_ADDR_LEN)
  {
    DS_3GPP_MSG0_HIGH_EX(subs_index, "Wrong IPV4 DNS Address");
    return;
  }

  DS_3GPP_MSG1_MED_EX(subs_index,
                      "Number of IPv4 DNS address in container: %d", 
                      *num_ipv4_dns);
  /*-----------------------------------------------------------------------
  ...
  if(*num_ipv4_dns == 0)
  {
    if( (dns_addr = get32((byte *)protocol_pkt_ptr)) != 0 )
    {
      ...
      DS_3GPP_MSG1_HIGH_EX(subs_index,
                           "Decoded primary v4 DNS address %x",
                           dns_addr);
    }
    (*num_ipv4_dns)++;
  }
  else if(*num_ipv4_dns == 1)
  {
    if( (dns_addr = get32((byte *)protocol_pkt_ptr)) != 0 )
    {
      ...
      DS_3GPP_MSG1_HIGH_EX(subs_index,
                           "Decoded secondary v4 DNS address %x",
                           dns_addr);
    }
    (*num_ipv4_dns)++;
  }    
  ...
  {
    DS_3GPP_MSG0_HIGH_EX(subs_index, 
        "More Than Two IPv4 DNS addresses present, Ignoring");
  }
}
...
)
{
  ...
  uint16                            pco_index = 0;
   ...
   if(protocol_pkt_ptr == NULL  )
  {
    DS_3GPP_MSG0_HIGH_EX(subs_id, "protocol_pkt_ptr is Nulll ");
    return FALSE;
  }
  ...
  {
    DS_3GPP_MSG1_HIGH_EX(subs_id, 
      "Invalid decoding operator reserved pco options- protocol pkt length %d",
      protocol_pkt_len);
    return FALSE;
  }
...
*/
  ...
  */
  ...
  /* Indicates this is not modem operator reserved pco */
  if (profile_pco_id_configured) 
  {
     /*Break when matching pco id is found or pco id 0 is found */
     while(pco_index < DS_3GPP_OP_RESERVED_PCO_ID_LIST_MAX) 
     {
        if ((protocol_id == operator_reserved_pco_list_ptr
                              ->pco_info[pco_index].pco_id ) ||
     ...
     {
        DS_3GPP_MSG0_HIGH_EX(subs_id, "Reached Max Number of Pcos");
        return FALSE;   
     }

     DS_3GPP_MSG1_HIGH_EX(subs_id, "Filling PCO Index %d",pco_index);
  ...
  /* Set Modem Operator Reserved Pco */
