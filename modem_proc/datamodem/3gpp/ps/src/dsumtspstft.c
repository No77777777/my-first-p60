...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp/ps/src/dsumtspstft.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ( num_filters == 0)
  {
    DS_3GPP_MSG0_LOW("ERROR: Zero Rx filters received");
    ret_val = FALSE;
  ...
  while (filter_offset > 0) 
  {
    if(ps_flow_get_rx_fltr_by_handle(flow_ptr,
                                     ...
                                     &new_handle) == FALSE)
    {
      DS_3GPP_MSG1_LOW("Could not get RX filter by handle for phys link %d",
                         flow_ptr);
      ret_val = FALSE;
  ...
  while ((num_filters > 0) && (filter_count < MAX_PACKET_FILTERS))
  {
    ...
    if ( fltr_len == 0 )
    {
      DS_3GPP_MSG1_LOW("Invalid filter Components, Filter Length: %d",
                          fltr_len);
      ret_val = FALSE;
    ...
    DS3GPP_ASSERT( fltr_len < MAX_FILTER_CONTENT_LENGTH );
    tft_len += fltr_len;
...
func_exit:
  ...
  {
    DS_3GPP_ERROR_LOG_1("dsumtsps_fill_v4_tft_params_for_sec: err d",
                        local_err);
  }
...
)
{
  ...
  if ( cmd_id == DS_CM_CALL_INCOM_CMD ) /* MT secondary request  */
  {
    if ( op_code != DS_UMTS_TFT_OPCODE_CREATE )
    {
      *cause = SM_SEMANTIC_ERR_IN_TFT;
      DS_3GPP_ERROR_LOG_1_EX(subs_id, "Invalid opcode received in MT secondary",
                          op_code);
      return FALSE;
    }
    ...
    if ( 0 == recvd_num_filters )
    {
      *cause = SM_SYNTACTIC_ERR_IN_TFT;
      DS_3GPP_ERROR_LOG_0_EX(subs_id, "Num of filters cannot be 0 for opcode CREATE");
      return FALSE;
    }
  ...
  else if ( cmd_id == DS_CM_CALL_NW_MODIFY_REQ )
  {
    ...
    {
      *cause = SM_SEMANTIC_ERR_IN_TFT;
      DS_3GPP_ERROR_LOG_0_EX(subs_id, "Opcode DELETE TFT is not expected in MODIFY REQ");
      return FALSE;
    }
    ...
    if ( op_code != DS_UMTS_TFT_OPCODE_NO_TFT )
    {
      ...
      {
        *cause = SM_SYNTACTIC_ERR_IN_TFT;
        DS_3GPP_ERROR_LOG_1_EX(subs_id, "Num of filters cannot be 0 for opcode d",
                            op_code);
        return FALSE;
      }
    }
  ...
  switch ( op_code )
  {
    ...
    case DS_UMTS_TFT_OPCODE_ADD:
    {
      ...
      for ( i=0;i< filter_ids.tx_num_filters;++i)
      {
        for ( j=i+1;j<filter_ids.tx_num_filters;++j)
        {
          ...
          {
            DS_3GPP_ERROR_LOG_1_EX(subs_id, "Create/Add filter with Tx identical filter ids d",
      ...
      for ( i=0;i< filter_ids.rx_num_filters;++i)
      {
        for ( j=i+1;j<filter_ids.rx_num_filters;++j)
        {
          ...
          {
            DS_3GPP_ERROR_LOG_1_EX(subs_id, "Create/Add filter with Rx identical filter ids d",
    ...
    case DS_UMTS_TFT_OPCODE_DEL_FILTER:
    {
      ...
      {
        *cause = SM_SEMANTIC_ERR_IN_TFT;
        DS_3GPP_ERROR_LOG_0_EX(subs_id, "Num of filters cannot be 0 for opcode DELETE PACKET FILTER");
         return FALSE;
      }
    }
      break;
    case DS_UMTS_TFT_OPCODE_NO_TFT:
    {
      ...
      {
        *cause = SM_SYNTACTIC_ERR_IN_TFT;
        DS_3GPP_ERROR_LOG_1_EX(subs_id, "Number of filters d received for opcode NO TFT",
                            op_code);
        return FALSE;
      }
    }
  ...
  {
    DS_3GPP_ERROR_LOG_1_EX(subs_id, "ERROR: Max filters reached d",
                        total_num_filters);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /* Need to differentiate between IPV4 and IPV6 */
    if ( mode_info_ptr->info.gw_ps_call.tft.valid == TRUE )
    {
      ...
      {
        DS_3GPP_MSG0_LOW("Num filter more than MAX");
       *cause = SM_SEMANTIC_ERR_IN_TFT;
       ret_val = FALSE;
       local_err = DS_3GPPI_LOCAL_ERR_VAL_0;
       goto func_exit;
        ...
        switch( direction )
        {
          case FILTER_UPLINK :
            ...
            &(tx_fltr->list_ptr[tx_fltr->num_filters]),  
                         ...
                         cause) )
            {
              DS_3GPP_MSG0_LOW(" FILTER_UPLINK Invalid TFT Params");
              ret_val = FALSE;
              local_err = DS_3GPPI_LOCAL_ERR_VAL_1;
              goto func_exit;
            }
          ...
          case FILTER_PRE_REL7:
            ...
            &(rx_fltr->list_ptr[rx_fltr->num_filters]),
                         ...
                         cause) )
            {
              DS_3GPP_MSG0_LOW(" FILTER_DOWNLINK/FILTER_PRE_REL7 Invalid TFT Params");
              ret_val = FALSE;
              local_err = DS_3GPPI_LOCAL_ERR_VAL_2;
              goto func_exit;
            }
          ...
          case FILTER_BIDIRECTIONAL:
            ...
            &(tx_fltr->list_ptr[tx_fltr->num_filters]),
                         ...
                         cause))
            {
              DS_3GPP_MSG0_LOW("FILTER_BIDIRECTIONAL, DOWNLINK  Invalid TFT Params");
              ret_val = FALSE;
              local_err = DS_3GPPI_LOCAL_ERR_VAL_3;
              goto func_exit;
            }
            ...
            &(rx_fltr->list_ptr[rx_fltr->num_filters]),
                         ...
                         cause) )
            {
              DS_3GPP_MSG0_LOW("FILTER_BIDIRECTIONAL, FILTER_UPLINK Invalid TFT Params");
              ret_val = FALSE;
          ...
          /* We need to return ERROR this should not happen */
	        DS_3GPP_ERROR_FATAL("Invalid tft filter direction");
        }/* switch (direction)*/
    ...
    {
      DS_3GPP_MSG0_LOW("Invalid filters received");
      *cause = SM_SYNTACTIC_ERR_IN_TFT;
      ret_val = FALSE;
      local_err = DS_3GPPI_LOCAL_ERR_VAL_4;
      goto func_exit;
...
func_exit:
  if(local_err != DS_3GPPI_LOCAL_ERR_VAL_NONE)
  {
    DS_3GPP_ERROR_LOG_1("dsumtsps_conv_network_tft_app_tft: err d",
                       local_err);
  }
  ...
  while( fltr_len < (filter_list.content_length -1))
  {
    ...
    else if(filter_list.filter_contents[fltr_len] == DS_UMTS_TFT_TYPE_OF_SRVC)
    {
      ...
      else
      {
        ...
        DS_3GPP_ERROR_LOG_0("Invalid TOS");
      }
    }
    /*-----------------------------------------------------------------------
    ...
    else
    {
      *cause = SM_SEMANTIC_ERR_IN_FILTER;
      ret_val = FALSE;
      //"Invalid Params in TFT fltr len= %d, content= %d",
      //fltr_len,filter_list.filter_contents[fltr_len]);
      temp_num = filter_list.filter_contents[fltr_len];
      local_err = DS_3GPPI_LOCAL_ERR_VAL_2;
      goto func_exit;
    }
  }
...
-----------------------------------------------------------------------------*/
  ...
  if ( temp_hdr_prot != PS_NO_NEXT_HDR )
  {
    ...
    case PS_IPPROTO_ICMP:
    {
      ...
      break;
    default:
      DS_3GPP_ERROR_LOG_1("Invalid next header protocol d",temp_hdr_prot);
      *cause = SM_SEMANTIC_ERR_IN_FILTER;
      ret_val = FALSE;
      //"Invalid next header protocol %d",
      //temp_hdr_prot);
      temp_num = (int)temp_hdr_prot;
      local_err = DS_3GPPI_LOCAL_ERR_VAL_3;
      goto func_exit;
    }
  }
...
func_exit:
  if(local_err != DS_3GPPI_LOCAL_ERR_VAL_NONE)
  {
    DS_3GPP_ERROR_LOG_3("dsumtsps_fill_v4_tft_params_for_sec: err d temp d fltr len d",
                       local_err, temp_num, fltr_len);
  }
...
)
{
  ...
  DS_3GPP_MSG1_HIGH_EX(subs_id, "Modify is for opdcode %d",
  ...
  if ( recvd_fltr_ids.rx_num_filters )
  {
    ...
    for (i=0;i < recvd_fltr_ids.rx_num_filters;++i)
    {
      for (replaced = FALSE,j=0;j < curr_fltr_ids->rx_num_filters; ++j)
      {
        ...
        {
          DS_3GPP_MSG1_HIGH_EX(subs_id, "Rx Filter id %d is getting replaced ",
                               recvd_fltr_ids.rx_filter_ids[i]);
      ...
      if (!replaced)
      {
        DS_3GPP_MSG1_HIGH_EX(subs_id,
                             "Rx Filter %d not found for replace, ADDING it",
                             recvd_fltr_ids.rx_filter_ids[i]);
        memscpy((void *)&(mod_rx_fltr->list_ptr[mod_rx_fltr->num_filters]),
	        sizeof(mod_rx_fltr->list_ptr[mod_rx_fltr->num_filters]),
                (const void *)&(recvd_params->rx.fltr_template.list_ptr[i]),
  ...
  if ( recvd_fltr_ids.tx_num_filters )
  {
    ...
    for (i=0;i < recvd_fltr_ids.tx_num_filters;++i)
    {
      for (replaced = FALSE,j=0;j < curr_fltr_ids->tx_num_filters; ++j)
      {
        ...
        {
          DS_3GPP_MSG1_HIGH_EX(subs_id, "Tx Filter id %d is getting replaced ",
                               recvd_fltr_ids.tx_filter_ids[i]);
      ...
      {
        DS_3GPP_MSG1_HIGH_EX(subs_id,
                             "Tx Filter %d not found for replace, ADDING it",
                             recvd_fltr_ids.tx_filter_ids[i]);
...
)
{
  ...
  DS_3GPP_MSG0_HIGH_EX(subs_id, "Modify is for OPCODE_DELETE");
  ...
  for (i=0;i < recvd_fltr_ids.rx_num_filters;++i)
  {
    for (j=0;j < curr_fltr_ids->rx_num_filters; ++j)
    {
      ...
      {
        DS_3GPP_MSG3_HIGH_EX(subs_id, "Rx filter id %d is getting deleted",
  ...
  for (i=0;i < recvd_fltr_ids.tx_num_filters;++i)
  {
    for (j=0;j < curr_fltr_ids->tx_num_filters; ++j)
    {
      ...
      {
        DS_3GPP_MSG1_HIGH_EX(subs_id, "Tx filter id %d is getting deleted",
                           recvd_fltr_ids.tx_filter_ids[i]);
...
)
{
  ...
  DS_3GPP_MSG0_HIGH_EX(subs_id, "Modify is for OPCODE_CREATE");
  /* ------------------------------------------------------------------
  ...
  DS_3GPP_MSG2_HIGH_EX(subs_id,
               "Replaced with received %d tx filters and %d rx filters",
               curr_fltr_ids->tx_num_filters,
               curr_fltr_ids->rx_num_filters);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS3GPP_ASSERT( (recvd_params != NULL ) &&( mod_qos_params != NULL ));
  DS3GPP_ASSERT ( curr_fltr_ids != NULL );
  DS3GPP_ASSERT ( (flow_ptr != NULL) &&(iface_ptr !=NULL));
  DS3GPP_ASSERT ( cause != NULL);
  ...
  DS_3GPP_MSG3_HIGH_EX(subs_id,
          "In dsumtsps_mt_modify_filter_params with current %d rcvd %d ",
          curr_fltr_ids->rx_num_filters,
  ...
  if ( recvd_fltr_ids.op_code == (byte)DS_UMTS_TFT_OPCODE_ADD)
  {
    ...
    if ( found_match )
    {
      if ( recvd_fltr_ids.op_code == (byte)DS_UMTS_TFT_OPCODE_ADD )
      {
        *cause = SM_SYNTACTIC_ERR_IN_FILTER;
        ret_val = FALSE;
        //"Duplicate Filter id %d in new request Operation ADD Failed",
        //found_match);
        local_err = DS_3GPPI_LOCAL_ERR_VAL_0;
        goto func_exit;
      }
  ...
  while ( handle != NULL )  /* Get Rx Filter by handle  */
  {
                                     ...
                                     &new_handle) == FALSE)
    {
      *cause = SM_ACTIVATION_REJECTED_UNSPECIFIED;
      ret_val = FALSE;
      //"Could not get RX filter by handle for phys link %p",
      //flow_ptr);
      local_err = DS_3GPPI_LOCAL_ERR_VAL_1;
      goto func_exit;
    }
  ...
  DS_3GPP_MSG1_HIGH_EX(subs_id,"Retreived %d Rx filters from PS ",
                       mod_rx_fltr->num_filters);
  ...
  DS_3GPP_MSG1_HIGH_EX(subs_id, "Retreived %d Tx filters from PS ", 
                       mod_tx_fltr->num_filters);
  ...
  case DS_UMTS_TFT_OPCODE_DEL_FILTER:
  {
    ...
    break;
  default:
    DS_3GPP_ERROR_LOG_1_EX(subs_id, "Invalid Operation received d ",
                           recvd_fltr_ids.op_code);
    *cause = SM_SYNTACTIC_ERR_IN_TFT;
    break;
  }
...
func_exit:
  if(local_err != DS_3GPPI_LOCAL_ERR_VAL_NONE)
  {
    DS_3GPP_ERROR_LOG_2_EX(subs_id, "dsumtsps_mt_modify_filter_params: err d flow ptr 0xx",
                       local_err, flow_ptr);
  }
...
)
{
  ...
  ip_filter_type       *filter;
  uint8                tft_len = 1, fltr_len = 0, ip6_addr_len,ip6_prefix_len ;
  uint16               temp_portnum, temp_portrng; 
                                     /* to convert network to host order   */
  uint8                filter_id;    /* Filter Identifier                  */
  void                 *handle;      /* Handle to the filter               */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  subs_id = ds3gsubsmgr_subs_id_ds_to_cm(
              PS_IFACE_GET_SUBS_ID(PS_FLOW_GET_IFACE(flow_ptr)));
  ...
  {
    ret_val = FALSE;
    //"ERROR: Zero Rx filters received ");
    local_err = DS_3GPPI_LOCAL_ERR_VAL_0;
    goto func_exit;
  }
  ...
  while (num_filters > 0)
  {
    ...
    { 
     ...
     DS_3GPP_MSG2_HIGH_EX(subs_id, " Filter %d : SRC ADDR : %d ",
                   filter_count, filter->ip_hdr.v4.src.addr.ps_s_addr);
     DS_3GPP_MSG2_HIGH_EX(subs_id, "Filter %d : SNET MSK : %d",
                         filter_count, 
    ...
    {   
      ...
      DS_3GPP_MSG2_HIGH_EX(subs_id, "Filter %d : NXT HDR PROTOCOL :%d ",
                          filter_count, 
      ...
      {     
        ...
        if ( filter->next_prot_hdr.tcp.field_mask & (int)IPFLTR_MASK_TCP_DST_PORT )
        {
          if ( filter->next_prot_hdr.tcp.dst.range == 0 )
          {
            ...
            DS_3GPP_MSG2_HIGH_EX(subs_id, "Filter %d : SINGLE TCP DEST PORT: %d",
                               filter_count, temp_portnum);
          }
          else
          {
            ...
            DS_3GPP_MSG3_HIGH_EX(subs_id, " Filter %d : TCP DEST PORT RANGE: %d, %d ",
                       filter_count, temp_portnum, temp_portnum+temp_portrng);
          }
        } /* if (DST PORT) */
        ...
        if ( filter->next_prot_hdr.tcp.field_mask & (int)IPFLTR_MASK_TCP_SRC_PORT )
        {
          if ( filter->next_prot_hdr.tcp.src.range == 0 )
          {
            ...
            DS_3GPP_MSG2_HIGH_EX(subs_id, "Filter %d: SINGLE TCP SRC PORT: %d",
                               filter_count, temp_portnum);
          }
          else
          {
            ...
            DS_3GPP_MSG3_HIGH_EX(subs_id, " Filter %d : TCP SRC PORT RANGE: %d, %d ",
                       filter_count, temp_portnum, temp_portnum+temp_portrng);
          }
        }
      }
      ...
      {       
        ...
        if ( filter->next_prot_hdr.udp.field_mask & (int)IPFLTR_MASK_UDP_DST_PORT )
        {
          ...
          {   
            ...
            DS_3GPP_MSG2_HIGH_EX(subs_id, "Filter %d: SINGLE UDP DEST PORT: %d",
                               filter_count, temp_portnum);
          }
          else
          {
            ...
            DS_3GPP_MSG3_HIGH_EX(subs_id, " Filter %d : UDP DEST PORT RANGE: %d, %d ",
                       filter_count, temp_portnum, temp_portnum+temp_portrng);
          }
        }
        ...
        if ( filter->next_prot_hdr.udp.field_mask & (int)IPFLTR_MASK_UDP_SRC_PORT )
        {
          ...
          {   
            ...
            DS_3GPP_MSG2_HIGH_EX(subs_id, "Filter %d: SINGLE UDP SRC PORT: %d",
                               filter_count, temp_portnum);
          }
          else
          {
            ...
            tft_params_ptr->filter_list[filter_count].\
             ...
             DS_3GPP_MSG3_HIGH_EX(subs_id," Filter %d : UDP SRC PORT RANGE: %d, %d ",
                       filter_count, temp_portnum, temp_portnum+temp_portrng);
          }
        }
      } /* if(PS_IPPROTO_UDP) */
    ...
    if ( filter->ip_hdr.v6.field_mask & (int)IPFLTR_MASK_IP6_TRAFFIC_CLASS )
    {
      ...
      DS_3GPP_MSG3_HIGH_EX(subs_id, " Filter %d : TOS VAL: %d, TOS MASK: %d ",
                 filter_count, filter->ip_hdr.v4.tos.val, 
                 filter->ip_hdr.v4.tos.mask);
    }
    ...
    if ( filter->ip_hdr.v6.field_mask & (int)IPFLTR_MASK_IP6_FLOW_LABEL )
    {
      ...
      DS_3GPP_MSG3_HIGH_EX(subs_id, " Filter %d : Flow Label %d ",
    ...
    {
      ret_val = FALSE;
      //"Invalid filter Components, Filter Length: %d",
      //fltr_len);
      local_err = DS_3GPPI_LOCAL_ERR_VAL_3;
      goto func_exit;
    }
    ...
    {
      DS_3GPP_MSG1_HIGH_EX(subs_id, "Filter Component Length: %d ",fltr_len);
    }
  ...
  DS_3GPP_MSG1_HIGH_EX(subs_id, "Total Filter Length: %d",tft_len);
...
func_exit:
  if(local_err != DS_3GPPI_LOCAL_ERR_VAL_NONE)
  {
    DS_3GPP_ERROR_LOG_3_EX(subs_id, 
                       "dsumtsps_fill_v6_tft_params_for_sec: err d flow ptr "
                       " 0xx fltr len d",
                       local_err, flow_ptr, fltr_len);
  }
