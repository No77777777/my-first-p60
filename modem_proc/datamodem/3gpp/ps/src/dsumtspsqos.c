...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp/ps/src/dsumtspsqos.c#3 $ $DateTime: 2021/02/08 22:33:06 $ $Author: pwbldsvc $
...
)
{
  ...
  { 
    ...
    else
    {
      ...
      {
        DS_3GPP_ERROR_LOG_0("Invalid exponent value recived. setting SDU ERR to subscribe");
        temp_sdu_err = DS_UMTS_QOS_SDU_ERR_SUBSCRIBE;
      }
    }
  }
  ...
  {
    DS_3GPP_ERROR_LOG_1("Invalid SDU ERR parameter : d",temp_sdu_err);
...
)
{
  ...
  DS3GPP_ASSERT ( ip_flow != NULL );
  DS3GPP_ASSERT ( umts_qos != NULL );
...
)
{
  ...
  DS_3GPP_MSG1_HIGH("app_qos->field_mask : 0x%x", app_qos->field_mask);
  ...
  if ( app_qos->field_mask & (int)QOS_MASK_RX_FLOW )
  {
    DS_3GPP_MSG0_LOW("Secondary Modification : Processing RX Req QOS Flow Spec");
    /*-----------------------------------------------------------------------
  ...
  else
  {
    ...
    -----------------------------------------------------------------------*/
    DS_3GPP_MSG0_LOW("Secondary Modification :No Change in RX Req QOS Flow Spec");
  ...
  {
    DS_3GPP_MSG0_LOW("Secondary Modification : Processing TX Req QOS Flow Spec");
    /*-----------------------------------------------------------------------
      Convert the Data Rate parameter of required set into UMTS specific:
      DL Max Data Rate, DL Guartanteed Data Rate
    -----------------------------------------------------------------------*/
  ...
  else
  {
    /*-----------------------------------------------------------------------
      ...
      Get the current flow parameters and populate the required set.
    -----------------------------------------------------------------------*/
    DS_3GPP_MSG0_LOW("Secondary Modification :No Change in TX Req QOS Flow Spec");
  ...
  if ( app_qos->field_mask & (int)QOS_MASK_RX_MIN_FLOW )
  {
    DS_3GPP_MSG0_LOW("Secondary Modification : Processing RX Min QOS Flow Spec");
    /*-----------------------------------------------------------------------
  ...
  else
  {
    ...
    -----------------------------------------------------------------------*/
    DS_3GPP_MSG0_LOW("Secondary Modification :No Change in RX Min QOS Flow Spec");
  ...
  {
    DS_3GPP_MSG0_LOW("Secondary Modification : Processing TX Min QOS Flow Spec");
  ...
  else
  {
    /*-----------------------------------------------------------------------
      ...
      Get the current flow parameters and populate the minimum set.
    -----------------------------------------------------------------------*/
    DS_3GPP_MSG0_LOW("Secondary Modification : No Change in TX Min QOS Flow Spec");
...
)
{
  ...
  DS_3GPP_MSG1_HIGH("app_qos_ptr->field_mask: 0x%x ", app_qos_ptr->field_mask);
  ...
  if ( app_qos_ptr->field_mask & (int)QOS_MASK_RX_FLOW )
  {
    DS_3GPP_MSG0_LOW("PRI_QOS_MODIFY_FLOW_PARAM_PROCESS:Processing RX Req QOS Flow ");
    /*-----------------------------------------------------------------------
      Convert the Traffic Class parameter of required set into UMTS specific.
    -----------------------------------------------------------------------*/
    dsumtsps_process_traffic_class (
  ...
  else
  {
    /*-----------------------------------------------------------------------
      ...
      Get the current flow parameters and populate the required set.
    -----------------------------------------------------------------------*/
    DS_3GPP_MSG0_LOW("PRI_QOS_MODIFY_FLOW_PARAM_PROCESS:No Change in RX Req QOS Flow ");
    rx_flow_params = PS_FLOW_GET_RX_REQUESTED_FLOW(flow_ptr);
  ...
  {
    DS_3GPP_MSG0_LOW("PRI_QOS_MODIFY_FLOW_PARAM_PROCESS: Processing TX Req QOS Flow ");
    /*-----------------------------------------------------------------------
      Convert the Data Rate parameter of required set into UMTS specific:
      DL Max Data Rate, DL Guartanteed Data Rate
    -----------------------------------------------------------------------*/
  ...
  else
  {
    /*-----------------------------------------------------------------------
      ...
      Get the current flow parameters and populate the required set.
    -----------------------------------------------------------------------*/
    DS_3GPP_MSG0_LOW("PRI_QOS_MODIFY_FLOW_PARAM_PROCESS:No Change in TX Req QOS Flow");
  ...
  if ( app_qos_ptr->field_mask & (int)QOS_MASK_RX_MIN_FLOW )
  {
    DS_3GPP_MSG0_LOW("PRI_QOS_MODIFY_FLOW_PARAM_PROCESS: Processing RX Min QOS Flow");
    /*-----------------------------------------------------------------------
      Convert the Traffic Class parameter of minimum set into UMTS specific.
    -----------------------------------------------------------------------*/
    dsumtsps_process_traffic_class (
  ...
  else
  {
    /*-----------------------------------------------------------------------
      ...
      Get the current flow parameters and populate the minimum set.
    -----------------------------------------------------------------------*/
    DS_3GPP_MSG0_LOW("PRI_QOS_MODIFY_FLOW_PARAM_PROCESS: No Change in RX Min QOS Flow");
  ...
  {
    DS_3GPP_MSG0_LOW("PRI_QOS_MODIFY_FLOW_PARAM_PROCESS: Processing TX Min QOS Flow");
  ...
  else
  {
    /*-----------------------------------------------------------------------
      ...
      Get the current flow parameters and populate the minimum set.
    -----------------------------------------------------------------------*/
    DS_3GPP_MSG0_LOW("PRI_QOS_MODIFY_FLOW_PARAM_PROCESS: No Change in TX Min QOS Flow");
...
)
{
      ...
      flow_ptr->data_rate.format_type == DATA_RATE_FORMAT_MIN_MAX_TYPE)
  {
    if (flow_ptr->data_rate.format.min_max.max_rate > QOS_MAX_BIT_RATE )
    {
      ...
      DS_3GPP_ERROR_LOG_1("max data rate exceeds theoritical max: d",
...
)
{
  ...
  {
    DS_3GPP_ERROR_LOG_0("Cannot convert Net Allocated QOS into App QOS since it is NULL");
  }
  else
  {
...
{   
         ...
         umts_qos_ptr == NULL)
    {
      DS_3GPP_ERROR_LOG_2_EX(subs_id,
                "CONV_NETW_QOS_TO_UMTS_QOS: Invalid Netw QoS Ptr 0xx "
                "or UMTS QoS Blob Ptr x",
                netw_qos_ptr, umts_qos_ptr);
      return FALSE;
    }

    DS_3GPP_MSG0_HIGH_EX(subs_id,
        "CONV_NETW_QOS_TO_UMTS_QOS: Starting QoS conversion routine");
    ...
    else
    {
        ...
        default:
            DS_3GPP_MSG1_HIGH_EX(subs_id,
                "CONV_NETW_QOS_TO_UMTS_QOS: Max SDU Size is out of range:%d, \
    ...
    if(umts_qos_ptr->traffic_class == DS_UMTS_QOS_TCLASS_INTR)
    {
      ...
      ----------------------------------------------------------------------*/
      DS_3GPP_MSG1_HIGH_EX(subs_id, 
             "converting the signalling indication parameter: sig_ind - %d", 
    ...
    DS_3GPP_MSG0_HIGH_EX(subs_id,
        "CONV_NETW_QOS_TO_UMTS_QOS: End of QoS conversion routine");
...
{   
    ...
    if ( !DSUMTSPS_QOSI_IS_QOS_BLOB_VALID( umts_qos_ptr ) ||
         app_qos_ptr == NULL )
    {
      DS_3GPP_ERROR_LOG_2("CONV_UMTS_QOS_TO_APP_QOS: invalid UMTS QoS Blob Ptr 0xx"
                         " or App Qos Ptr 0xx",
                         umts_qos_ptr, app_qos_ptr);
      return FALSE;
    }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  app_tx_ip_flow_ptr = &app_qos_ptr->tx_ip_flow;
    ...
    if ( !DSUMTSPS_QOSI_IS_QOS_BLOB_VALID( gprs_qos_ptr ) ||
         app_qos_ptr == NULL )
    {
      DS_3GPP_ERROR_LOG_2("CONV_UMTS_QOS_TO_APP_QOS: Invalid UMTS QoS Blob Ptr 0xp"
                         " or App Qos Ptr 0xp",
                         gprs_qos_ptr, app_qos_ptr);
      return FALSE;
    }
...
)
{
    ...
    {
      DS_3GPP_ERROR_LOG_2_EX(subs_id,
                         "CONV_NETW_QOS_TO_GPRS_QOS: Invalid Netw QoS Ptr 0xx"
                         " or UMTS QoS Blob Ptr",
                         netw_qos_ptr, gprs_qos_ptr);
      return FALSE;
    }

    DS_3GPP_MSG0_HIGH_EX(subs_id,
        "CONV_NETW_QOS_TO_GPRS_QOS: Starting QoS conversion routine");
...
)
{
  ...
  /*-----------------------------------------------------------------------
    Data rate = 63 * 1 kbps,
    giving a range of 1kbps to 63kbps in 1kbps increments
  -----------------------------------------------------------------------*/
  ...
  /*-------------------------------------------------------------------------
    Data rate = 64 + ( 8 bit binary encoded value - 0x40)* 8 kbps,
  ...
  /*---------------------------------------------------------------------------
    Data rate = 576 + ( 8 bit binary encoded value - 0x80)* 64 kbps,
