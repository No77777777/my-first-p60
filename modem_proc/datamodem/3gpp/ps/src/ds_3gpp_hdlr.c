...
/*===========================================================================
  ...
  Copyright (c) 2008-2020 Qualcomm Technologies Incorporated. All Rights Reserved
...
$Header: //components/rel/data.mpss/3.5..2.1/3gpp/ps/src/ds_3gpp_hdlr.c#10 $
...
/*===========================================================================
FUNCTION DS3GPP_GET_SYS_CHANGE_HDLR

DESCRIPTION
This is ensures to register only one sys-change-hdlr for all 3gpp RATs
...
===========================================================================*/
void* ds3gpp_get_sys_change_hdlr(void);
...
/*===========================================================================
FUNCTION DS_3GPP_PROCESS_APN_PREF_SYS_CMD
 
DESCRIPTION
  This function is used to handle the event when the APN preferred system
  has changed
...
===========================================================================*/
static void ds_3gpp_process_apn_pref_sys_cmd
(
...
#ifdef FEATURE_QSH_MDUMP
/*===========================================================================
FUNCTION ds3gpp_qsh_mdump_collect
...
===========================================================================*/ 
QSH_MDUMP_FN_ATTR 
void ds3gpp_qsh_mdump_collect()
{
  qsh_mdump_collect_high(&ds3gpp_info, sizeof(ds3gpp_info_type));
  qsh_mdump_collect_high(&ds_3gpp_nv_refresh_in_progress, 
  	SYS_MODEM_AS_ID_MAX * sizeof(boolean));
}
...
)
{
  ...
  {
    DS_3GPP_MSG3_HIGH_EX(subs_id, "Max PDN Conn Per Blk changed for profile %d, "
...
function_exit:
  ...
  {
    DS_3GPP_ERROR_LOG_1_EX(subs_id, "ds_3gpp_max_pdn_conn_per_blk_chg_hdlr: err d",
                       local_err);
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(ds_3gpp_pdn_limit_is_enabled(subs_id))
  {
    DS_3GPP_MSG3_HIGH_EX(subs_id, "PDN Req Wait Time changed for profile %d, "
...
function_exit:
  ...
  if(local_err != DS_3GPPI_LOCAL_ERR_VAL_NONE)
  {
    DS_3GPP_ERROR_LOG_2_EX(subs_id, "req_wait_time_chg_hdlr: err d - Mem alloc fail/Error "
                       "retrieving params for prof d",
                       local_err, profile_num);
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_3GPP_MSG3_HIGH_EX(subs_id, "Max PDN Conn Time changed for profile %d, "
...
function_exit:
  ...
  if(local_err != DS_3GPPI_LOCAL_ERR_VAL_NONE)
  {
    DS_3GPP_ERROR_LOG_2_EX(subs_id, "max_pdn_conn_time_chg_hdlr: err d - mem alloc fail/"
                       "retrieve prof:d param fail",
                       local_err, profile_num);
  }
...
/*===========================================================================
FUNCTION DS_3GPP_APN_PREF_SYS_CHG_CB_HDLR
 
DESCRIPTION
  This function is used to handle the PS event that indicates a preferred
  system change has occurred for an IMS APN.  
...
===========================================================================*/
void ds_3gpp_apn_pref_sys_chg_cb_hdlr
(
...
)
{
  ...
  ds_3gpp_apm_apn_pref_sys_cmd_type *data_ptr  = NULL;
  ds_3gppi_local_err_val_e_type      local_err = DS_3GPPI_LOCAL_ERR_VAL_NONE;
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if( PS_SYS_TECH_ALL != tech_type || 
      PS_SYS_EVENT_APN_PREF_SYS_CHANGE != event_name )
  {
    local_err = DS_3GPPI_LOCAL_ERR_VAL_1;
    goto func_exit;
  }

  cmd_ptr = ds_allocate_cmd_buf(sizeof(ds_3gpp_apm_apn_pref_sys_cmd_type));
  ...
  cmd_ptr->hdr.cmd_id = DS_CMD_3GPP_APN_PREF_SYS_CHG;
  data_ptr = (ds_3gpp_apm_apn_pref_sys_cmd_type*)cmd_ptr->cmd_payload_ptr;
  data_ptr->dsd_subs_id = subscription_id;

  memscpy(&(data_ptr->apn_pref_sys), 
          sizeof(ps_sys_apn_pref_sys_type),
          event_info_ptr, sizeof(ps_sys_apn_pref_sys_type));

  ds_put_cmd_int(cmd_ptr);

func_exit:
  if(local_err != DS_3GPPI_LOCAL_ERR_VAL_NONE)
  {
    DS_3GPP_ERROR_LOG_1_EX(ds3gsubsmgr_subs_id_ds_to_cm(
                           (ds_sys_subscription_enum_type)subscription_id),
                           "ds_3gpp_apn_pref_sys_chg_cb_hdlr err d",
                           local_err);
  }
  return;
} /* ds_3gpp_apn_pref_sys_chg_cb_hdlr */
...
===========================================================================*/
void ds_3gpp_process_apn_pref_sys_cmd
(
  const ds_cmd_type  *cmd_ptr
)
{
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_3GPP_ERROR_LOG_0("ds_3gpp_process_apn_pref_sys_cmd: "
                                "Null ptr passed return");
    return;
  }
  ...
  cm_subs_id =
    ds3gsubsmgr_subs_id_ds_to_cm
      ((ds_sys_subscription_enum_type)data_ptr->dsd_subs_id);
  ...
  /* Preferred system is WLAN or IWLAN so LTE attach is not allowed */
  if (data_ptr->apn_pref_sys.pref_sys != DS_SYS_APN_PREF_SYS_WWAN) 
  {
    ds_3gpp_pdn_throt_set_global_throt_mask(
                                         data_ptr->apn_pref_sys.apn, 
                                         cm_subs_id, 
                                         DS_3GPP_PDN_THROT_APN_PREF_SYS_MASK);
    ds_3gpp_pdn_throt_advertise_blocked_apns_current_plmn(cm_subs_id);
    ds_3gpp_pdn_cntxt_disable_lte_if_attach_profile_unavailable(cm_subs_id);
  }
  else
  {
    /* Preferred system is WWAN so LTE attach is allowed -
       reset throttling mask for pref sys */
    ...
    {
      ds_3gpp_pdn_throt_cleanup_apn_index_entry_if_needed(apn_index_ptr, FALSE,
                                                          cm_subs_id);
      ds_3gpp_pdn_throt_advertise_blocked_apns_current_plmn(cm_subs_id);
      if (ds_dsd_apm_is_apn_in_attach_pdn_list(cm_subs_id, 
                                          (char *)data_ptr->apn_pref_sys.apn))
      {    
        ds_3gpp_pdn_cntxt_enable_lte_if_attach_profile_available(cm_subs_id);
      }
    } 
    else 
    {
      DS_3GPP_ERROR_LOG_0_EX(cm_subs_id, "ds_3gpp_process_apn_pref_sys_cmd:"
                                  " failed to get apn index ptr");
    }
    #ifdef FEATURE_DATA_LTE
    DS_3GPP_ERROR_LOG_1_EX(cm_subs_id,
                         "IMS API override: preferred system:%d check attach pending conditions",
                         data_ptr->apn_pref_sys.pref_sys);
    /*Pref system for IMS PDN is WWAN, Check if can enter attach pending*/
    ds_eps_pdn_cntxt_ex_check_attach_pending(cm_subs_id);
    #endif /*FEATURE_DATA_LTE*/
  }
  return;
} /* ds_3gpp_process_apn_pref_sys_cmd */
...
)
{
  ...
  {
    DS_3GPP_ERROR_LOG_1_EX(subs_id, "Invalid profile_num d ", profile_num);
     return;
  }

  DS_3GPP_MSG2_HIGH_EX(subs_id, "APN Disabled flag changed to %d for profile %d ", 
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_3GPP_MSG1_LOW("Invalid profile %d ", profile_num);
    return;
  }
  ...
  ds_3gpp_kamgr_process_apn_name_chg(profile_num,subs_id);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DATA_3GPP_MSG_SPRINTF_3_EX(MSG_LEGACY_MED,
                             subs_id,
                             "Profile status: %d. "
  ...
  if (apn_disable_flag)
  {
    ...
    {
      ds_3gpp_pdn_throt_set_global_throt_mask
        (prof_apn_name, subs_id, DS_3GPP_PDN_THROT_APN_DISABLED_MASK);
    ...
    ds_3gpp_pdn_cntx_teardown_pdn_per_profile
        (profile_num, PS_NET_DOWN_REASON_INTERNAL_APN_DISABLED,subs_id);
  }
  else
  {
    if (profile_status == DS_UMTS_PDP_SUCCESS)
  ...
  if(ds_3gpp_cfg_get_subsid(DS_3GPP_CFG_PROFILE_LIST, 
                    ...
                    (sizeof(uint16) * DS_3GPP_CFG_PROFILES_MAX),
                     subs_id) == -1)
  {
    //"Cannot read EFS Config item DS_3GPP_CFG_PROFILE_LIST");
    local_err = DS_3GPPI_LOCAL_ERR_VAL_0;
    goto func_exit;
  }
  ...
  {
    //"Min Profile List has %d profiles",
    //num_profiles);
    local_err = DS_3GPPI_LOCAL_ERR_VAL_1;
    temp_num = num_profiles;
    goto func_exit;
  }
  ...
  for(index = 1; index <= num_profiles; index++)
  {
    ...
    DS_3GPP_MSG1_HIGH_EX(subs_id, "profile index %d",index);
   ...
   DS_3GPP_MSG3_HIGH_EX(subs_id, "profile match %d for profile %d . disabled_profile_cnt %d ",
  ...
  if(profile_match)
  {
    ...
    else
    {
      ...
      */
      //"Can't enable LTE selection for profile :%d,",
      //profile_num);
      local_err = DS_3GPPI_LOCAL_ERR_VAL_2;
      temp_num = profile_num;
      goto func_exit;
    }
  }
  else
  {
    //"Profile %d not in min APN list ",
    //profile_num);
...
func_exit:
  if(local_err != DS_3GPPI_LOCAL_ERR_VAL_NONE)
  {
    DS_3GPP_ERROR_LOG_2_EX(subs_id, "ds_3gpp_process_apn_disabled_cmd: err d temp d",
                       local_err, temp_num);
  }
...
)
{
  ...
  if (ds3gpp_validate_per_subs_info_hndl(hndl))
  {
    ...
    DS_3GPP_MSG2_MED_EX(subs_id, "current_ds_lte_flag for Subs id(CM): %d is %d",
...
)
{
  ...
  DS_3GPP_MSG2_HIGH_EX(subs_id, "Roaming Disallowed flag changed for profile %d, "
...
)
{
 ...
 /*----------------------------------------------------------------------*/
  ...
  if(old_apn == NULL)
  {
    DS_3GPP_MSG0_LOW("New apn is NULL");
    return;
  }

  DATA_3GPP_MSG_SPRINTF_2_EX(MSG_LEGACY_HIGH,
                             subs_id,
                             "APN name chg hdlr old_apn_name:%s profile_id:%d",
                             old_apn,profile_num);
  ...
  apn_name_chg_info_ptr->old_apn_type_mask = old_apn_type_mask;
...
/*===========================================================================
FUNCTION DS_3GPP_APN_TYPE_CHG_HDLR
 
DESCRIPTION
  This function is invoked when the APN type is changed.
...
===========================================================================*/
static void ds_3gpp_apn_type_chg_hdlr
(
  uint16                      profile_num,
  sys_modem_as_id_e_type      subs_id,
  uint64                      old_apn_type_mask
...
{
  ds_cmd_type                           *cmd_ptr = NULL; 
  ds_profile_db_apn_type_chg_info_type  *apn_type_chg_info_ptr = NULL;
 /*-------------------------------------------------------------------------------*/
  DS_3GPP_MSG2_HIGH_EX(subs_id, "ds_3gpp_apn_type_chg_hdlr profile num %d," "old apn type_mask %d",profile_num,
                    old_apn_type_mask);

  cmd_ptr = ds_allocate_cmd_buf(sizeof(ds_profile_db_apn_type_chg_info_type));
  if( cmd_ptr == NULL || cmd_ptr->cmd_payload_ptr == NULL )
  {
    DS3GPP_ASSERT(0);
    return;
  ...
  cmd_ptr->hdr.cmd_id = DS_CMD_3GPP_PROFILE_DB_APN_TYPE_CHANGE;
  apn_type_chg_info_ptr = (ds_profile_db_apn_type_chg_info_type *)cmd_ptr->cmd_payload_ptr;
  apn_type_chg_info_ptr->profile_num = profile_num;
  apn_type_chg_info_ptr->old_apn_type_mask = old_apn_type_mask;
  apn_type_chg_info_ptr->subs_id = subs_id;
  ds_put_cmd_ext(cmd_ptr);
...
)
{
  ...
  /*----------------------------------------------------------------------*/
  DS_3GPP_MSG2_HIGH_EX(subs_id, "PDP type chged,Profile_Number:%d New PDP type:%d",
...
)
{
  ...
  if (ds3gpp_validate_per_subs_info_hndl(hndl))
  {
    ...
    DS_3GPP_MSG2_LOW("Current internal sys mode for Subs id(CM): %d is %d",
                      subs_id, mode);
  }
...
)
{
  ...
  hndl = ds3gpp_get_per_subs_info_hndl(subs_id);
#ifdef FEATURE_LAPP
  network_mode = ds3gpp_hdlr_ex_override_sys_mode(network_mode, subs_id);  
#endif

  if (ds3gpp_validate_per_subs_info_hndl(hndl))
  {
    /*ideally this function should be called only once for all RATs*/
...
)
{
  ...
  DS_3GPP_MSG1_HIGH_EX(subs_id, "ds_3gpp_process_oprt_mode_chg_cmd: Invoking registered "
...
)
{
  ...
  if (ds3gpp_validate_per_subs_info_hndl(hndl))
  {
    DS_3GPP_MSG2_MED_EX(subs_id, "Last full srv mode for Subs id(CM): %d is "
...
)
{
  ...
  if (ds3gpp_validate_per_subs_info_hndl(hndl))
  {
    /*let ds3g print service status info */
...
)
{
  ...
  sint15                                     ps_errno;
#endif /* FEATURE_DATA_WLAN_MAPCON */
  ds_sys_system_status_info_type	    *bearer_tech_ex_ptr=NULL;
  ds3gpp_per_subs_info_type                 *hndl = NULL;
  boolean                                    advertise_empty_list = FALSE;
  sys_plmn_id_s_type                         current_camped_plmn;
  ds_3gppi_local_err_val_e_type              local_err = DS_3GPPI_LOCAL_ERR_VAL_NONE;
  sys_sys_mode_e_type                        curr_sys_mode = new_mode;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
...
#ifdef FEATURE_LAPP  
  new_mode = ds3gpp_hdlr_ex_override_sys_mode(new_mode, subs_id);
#endif /* FEATURE_LAPP */
...
{
  DS_3GPP_MSG0_LOW("Sys change handler invalid feature WCDMA undefined");
  local_err = DS_3GPPI_LOCAL_ERR_VAL_0;
  goto func_exit;
}
#endif /* FEATURE_DATA_WCDMA_PS */
...
{
  DS_3GPP_MSG0_LOW("Sys change handler invalid feature GSM_GPRS undefined");
  local_err = DS_3GPPI_LOCAL_ERR_VAL_1;
  goto func_exit;
}
#endif /* FEATURE_GSM_GPRS */
...
{
  DS_3GPP_MSG0_LOW("Sys change handler invalid feature TDSCDMA undefined");
  local_err = DS_3GPPI_LOCAL_ERR_VAL_2;
  goto func_exit;
}
#endif /* FEATURE_TDSCDMA */
 ...
 /*-------------------------------------------------------------------------
  ...
  if ( (
       ...
       (ds3gpp_disconnect_unused_pdn_gwt_allowed(subs_id,new_mode,old_mode)))
  {
    DS_3GPP_MSG0_HIGH_EX(subs_id, "Tearing down calls on G/W/T RAT");
  ...
  if(DS_3GPP_IS_3GPP_CURRENT_MODE(new_mode))
  {
    ...
    if(is_ps_srv_domain_pref == TRUE)
    {
      ...
      /*------------------------------------------------------------------------- 
        If old_srv_domain is SYS_SRV_DOMAIN_CS_ONLY or SYS_SRV_DOMAIN_NO_SRV 
		new srv_domain is PS srv domain
         ...
         (hndl->old_srv_domain  == SYS_SRV_DOMAIN_NO_SRV))
      {
        ...
        ds_3gpp_dam_clear_throttling(subs_id);
      }
    /* Reset Service request throttling on RAT change only for a specific config */
      ...
      if (ds_3gpp_nv_manager_get_enable_pdn_throt_per_rat(subs_id))
      {
        if (curr_srv_status == SYS_SRV_STATUS_SRV || 
            new_mode == SYS_SYS_MODE_LTE)
        {
          ds_3gpp_pdn_throt_sm_advertise_blocked_apns_to_external_clients(subs_id);
        }
      }
      ...
      if(curr_srv_status == SYS_SRV_STATUS_SRV)
      {
        ...
        ---------------------------------------------------------------------*/
        bearer_tech_ex_ptr = &bearer_tech_ex;
        if( ds_3gpp_bearer_cntxt_get_bearer_type_ex( new_mode,
                                                     bearer_tech_ex_ptr,
                                                     &ps_errno,
                                                     subs_id ) != 0)
        {
          DS_3GPP_MSG0_LOW("Could not fetch bearer tech ex info");
          local_err = DS_3GPPI_LOCAL_ERR_VAL_3;
          /* This will make sure the bearer-tech-ex event wont be posted */
          bearer_tech_ex_ptr = NULL;
        }
#endif /* FEATURE_DATA_WLAN_MAPCON */
       ...
       ----------------------------------------------------------------------*/
        ds_3gpp_pdn_cntx_post_iface_bearer_tech_evt_generic(subs_id,
		                                   &(tech_change_info),
		                                   bearer_tech_ex_ptr,
                                           NULL);
...
/*If new mode is wcdma or lte or old mode is wcdma or lte and the new mode 
  is in full service; then check if the uldl throughput timer needs to be
  stopped and restarted in new mode */
  if((new_mode==SYS_SYS_MODE_LTE || old_mode == SYS_SYS_MODE_LTE  ||
     new_mode==SYS_SYS_MODE_WCDMA || old_mode == SYS_SYS_MODE_WCDMA) &&
     (curr_srv_status == SYS_SRV_STATUS_SRV))
  {
  ...
  /*----------------------------------------------------------------------- 
	...
	Do not update the old srv_domain if the old srv_domain was
	SYS_SRV_DOMAIN_CAMPED 
    -----------------------------------------------------------------------*/
  if(srv_domain != SYS_SRV_DOMAIN_CAMPED)
  {
    hndl->old_srv_domain = srv_domain;
    DS_3GPP_MSG1_LOW("Updating old srv domain to %d",hndl->old_srv_domain);
  }
  ...
  /*When sys mode changes from W or changes to W, call function to check criteria for 
    downlink throughput report for W RRC*/
  if(new_mode==SYS_SYS_MODE_WCDMA || old_mode == SYS_SYS_MODE_WCDMA)
  {
    ds3gtputmgr_downlink_throughput_report_hdlr(subs_id);
  }

func_exit:
  if(local_err != DS_3GPPI_LOCAL_ERR_VAL_NONE)
  {
    DS_3GPP_ERROR_LOG_1_EX(subs_id, "ds3gpp_sys_change_handler: err d",
                       local_err);
  }
...
)
{
  ...
  ds_3gppi_local_err_val_e_type      local_err = DS_3GPPI_LOCAL_ERR_VAL_NONE;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  DS3GPP_ASSERT(call_info_ptr != NULL);
  ...
  switch(call_info_ptr->call_event)
  {
    ...
    {
      DS_3GPP_MSG0_HIGH_EX(subs_id, "Processing fast dormancy status CM callback");
      /*-------------------------------------------------------------------------
      ...
      switch(call_info_ptr->mode_info.info.gw_ps_call.ps_sig_rel_status)
      {
        ...
        default:
          //"ds_3gpp_ps_data_generic_ind_hdlr: Invalid ps_sig_rel_status %d", 
          //call_info_ptr->mode_info.info.gw_ps_call.ps_sig_rel_status);
          temp_num = (int)call_info_ptr->mode_info.info.gw_ps_call.ps_sig_rel_status;
    ...
    default:
      //"ds_3gpp_ps_data_generic_ind_hdlr :Unhandled call event %d",
      //call_info_ptr->call_event);
      temp_num = (int)call_info_ptr->call_event;
  }

  if(local_err != DS_3GPPI_LOCAL_ERR_VAL_NONE)
  {
    DS_3GPP_ERROR_LOG_2_EX(subs_id, "ds_3gpp_ps_data_generic_ind_handler: err d temp d",
                       local_err, temp_num);
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS3GPP_ASSERT(orig_info_ptr != NULL );
  ...
  DS_3GPP_MSG2_HIGH_EX(subs_id, "3GPP Call_Orig_hdlr_ext bearer cntx 0x%x inst %d",
...
/*===========================================================================
FUNCTION  DS3GPP_CALL_ENDED_HDLR
...
)
{
  ...
  /*-------------------------------------------------------------------------- 
    If call end is received for the OTASN PDN while Timer is running,
    cleanup here
    -----------------------------------------------------------------------*/
    pdn_cntx_p = bearer_cntx_p->ds_bearer_context_dyn_p->pdn_context_ptr;
    ds_3gpp_pdn_cntxt_process_otasn_timer_cleanup(pdn_cntx_p, FALSE ,subs_id);
  ...
  network_mode = ds3gpp_get_current_network_mode(subs_id);
   ...
   if( !DS_3GPP_IS_3GPP_CURRENT_MODE(network_mode))
  {
    DS_3GPP_MSG3_LOW("DS 3GPP Invalid network_mode:%d",network_mode,0,0);
    local_err = DS_3GPPI_LOCAL_ERR_VAL_2;
    goto func_exit;
  }
  ...
  switch (network_mode)
  {
    ...
    case SYS_SYS_MODE_LTE:
      DS_3GPP_MSG2_MED_EX(subs_id, "MH Client: Deregistering from FC for call id %d network_mode = %d",
              callid, network_mode);
      fc_post_deregister_cmd_wm_trace(callid, UNIFIED_3GPP_MODE_HANDLER_CLIENT);
      break;

    default:
      DS_3GPP_MSG1_LOW("MH: Not deregistering for UNKNOWN tech, call id %d",
              callid);
      local_err = DS_3GPPI_LOCAL_ERR_VAL_3;
      break;
  }
...
func_exit:
  if(local_err != DS_3GPPI_LOCAL_ERR_VAL_NONE)
  {
    DS_3GPP_ERROR_LOG_3_EX(subs_id, "ds3gpp_call_ended_hdlr: err d nw mode d subs id d",
                       local_err, network_mode, subs_id);
  }
...
)
{
  ...
  {
    DS_3GPP_ERROR_FATAL(" Incorrect mode_info in ds3gpp_call_id_hdlr");
  }
...
)
{
  ...
  {
    DS_3GPP_MSG1_HIGH_EX(subs_id, "MT call end with call id: %d",call_id);
  ...
  DS3GPP_ASSERT(bearer_cntx_p != NULL);
...
)
{
  ...
  sys_sys_mode_e_type              mode = SYS_SYS_MODE_NONE;
  boolean                          per_plmn_throttling = FALSE;
  ds_3gppi_local_err_val_e_type    local_err = DS_3GPPI_LOCAL_ERR_VAL_NONE;
  /*-----------------------------------------------------------------------*/
  ...
  /*----------------------------------------------------------------------------
    Query the NV setting for whether PLMN throttling per plmn is enabled.
  ----------------------------------------------------------------------------*/
  per_plmn_throttling =
                  ds_3gpp_nv_manager_get_nv_rel_10_throttling_per_plmn(subs_id);

  /*--------------------------------------------------------------------------
    Check if Per PLMN throttling is enabled & if sys mode is not LTE, then
    plmn throttling will be disabled.
  --------------------------------------------------------------------------*/
  if ((per_plmn_throttling == TRUE) && (mode != SYS_SYS_MODE_LTE)) 
  {
    //DS_3GPP_MSG1_HIGH_EX(subs_id, "Per PLMN Throttling was enabled, and mode is not LTE: %d", mode);
    local_err = DS_3GPPI_LOCAL_ERR_VAL_0;
    goto func_exit;
  }
  ...
  if(sm_ptr != NULL)
  {
    if(ds_3gpp_srv_req_throt_get_state(sm_ptr, &ret_val, subs_id) == TRUE)
    {
      ret_val = !ret_val;
      //DS_3GPP_MSG1_MED_EX(subs_id, "Throt SM consulted. Return %d to NAS",ret_val);
      local_err = DS_3GPPI_LOCAL_ERR_VAL_1;
    }
    else
    {
      //DS_3GPP_MSG0_HIGH_EX(subs_id, "Func to query throt state ret FALSE. Allow srv req");
      local_err = DS_3GPPI_LOCAL_ERR_VAL_2;
      /*------------------------------------------------------------------------
  ...
  {
    //DS_3GPP_MSG0_HIGH_EX(subs_id, "Srv req throt sm not avail. Allow srv req");
    local_err = DS_3GPPI_LOCAL_ERR_VAL_3;
  }
...
func_exit:
  if(local_err != DS_3GPPI_LOCAL_ERR_VAL_NONE)
  {
    DS_3GPP_ERROR_LOG_2_EX(subs_id, "ds3gpp_per_subs_is_service_req_allowed_on_plmn: err d ret_val d",
                       local_err, ret_val);
  }
 ...
 /*------------------------------------------------------------------------- 
     ...
     (bearer_context_p->ds_bearer_context_dyn_p->call_mode == SYS_SYS_MODE_LTE))
  {
    DS_3GPP_MSG0_HIGH_EX(subs_id,
                         "Bearer context's call mode is LTE, return SUCCESS");
...
)
{
  ...
  {
    DS_3GPP_ERROR_LOG_0_EX(subs_id, "ds3gpp_bearer_modify_ind_hdlr: mode_info_ptr NULL");
    return DS3G_FAILURE;
  }
  ...
  {
    DS_3GPP_MSG0_MED_EX(subs_id, "Updating the internal 3GPP network mode to LTE");
...
)
{
  ...
  else if(curr_srv_status == SYS_SRV_STATUS_PWR_SAVE &&
          ds_3gpp_nv_manager_get_enforce_ps_data_avail(subs_id) && is_ims_pdn)
  {
    ret_val = TRUE;
  }
  else
  {
   ...
   DS_3GPP_MSG1_HIGH_EX(subs_id, "ds3gpp_is_ps_data_avail_ind_allowed "
                        "on power Save %d", ret_val);
   return ret_val;
...
)
{
  ...
  DS_3GPP_MSG1_HIGH_EX(subs_id, "Data Suspend Ind Hdlr Called on sub %d",
                       subs_id);
  ...
  if (!ds3gpp_validate_per_subs_info_hndl(hndl))
  {
    DS_3GPP_MSG1_LOW("Per Subs Hndl is Invalid for cm subs id: %d",subs_id);
    return;
  }
  ...
  else if (hndl->sent_ps_data_available == FALSE)
  {
    ...
    */
    DS_3GPP_MSG2_LOW("Data already suspended,Updating phys_link_flow %d,subs_id:%d",
                      FALSE,subs_id);
...
)
{
  ...
  ds3gpp_per_subs_info_type*       hndl = NULL;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  DS_3GPP_MSG1_HIGH_EX(subs_id, "Data Resume Ind Hdlr Called on sub %d",
                    subs_id);
  ...
  {
    DS_3GPP_MSG0_HIGH_EX(subs_id,
        "Data Resume Ind Hdlr Called when data_suspend is false");
    return;
  }
  ...
  {
    DS_3GPP_ERROR_LOG_1_EX(subs_id, "Resume Ind: invalid network_mode:d",network_mode);
  }
       ...
       bearer_cntx_inst++ )
  {
    ...
    else
    {
      ...
      if(ds_3gpp_pdn_cntx_validate_pdn_context_internal(pdn_cntxt_p))
      {
        ...
        {
          DS_3GPP_MSG0_MED_EX(subs_id, "PDN is emergency. Do not flow control.");
          disable_flow = FALSE;
        }
        ...
        if (current_nw_mode == SYS_SYS_MODE_LTE)
        {
          ...
          {
            DS_3GPP_MSG1_MED_EX(subs_id, "Do not Tear down v4 apn type: %d",
                         PS_IFACE_GET_APN_TYPE(&(ds3gpp_iface_v4_p->ps_iface)));
            disable_flow = FALSE;
          }
          ...
          if (ds3gpp_iface_v6_p != NULL && 
              PS_IFACE_GET_APN_TYPE(&(ds3gpp_iface_v6_p->ps_iface))
                 == PS_IFACE_APN_TYPE_IMS)
          {
            DS_3GPP_MSG1_MED_EX(subs_id, "Do not Tear down v6 apn type: %d,",
                         PS_IFACE_GET_APN_TYPE(&(ds3gpp_iface_v6_p->ps_iface)));
            disable_flow = FALSE;
          }
    #endif /* FEATURE_DATA_PS_IPV6 */
          ...
          if (ds_3gpp_pdn_cntxt_is_pdn_third_party_ims(pdn_cntxt_p,subs_id) 
              == TRUE)
          {
            DS_3GPP_MSG0_MED_EX(subs_id, "PDN APN matches with IMS PDN APN in device "
    ...
    if( disable_flow )
    {
      ds_3gpp_bearer_cntxt_disable_flow(bearer_cntx_p,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
       ...
       bearer_cntx_inst++ )
  {
    ...
    if( ds_bearer_cntxt_validate_bearer_subscription_internal(bearer_cntx_p, subs_id) )
    {
      ds_3gpp_bearer_cntxt_enable_flow(bearer_cntx_p, 
                                       DS_FLOW_COEX_MASK, FALSE);
    }
  }
  return;
...
/*===========================================================================
FUNCTION  DS3GPP_GET_TOTAL_UL_CUR_WM_CNT_HDLR

DESCRIPTION
  This function returns total current UL WM count for all active bearers
  in input subs_id
...
===========================================================================*/
uint64 ds3gpp_get_total_ul_cur_wm_cnt_hdlr
(
  sys_modem_as_id_e_type  subs_id
)
{
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
       ...
       bearer_cntx_inst++ )
  {
    ...
    if( ds_bearer_cntxt_validate_bearer_subscription_internal(bearer_cntx_p, subs_id) )
    {
      total_ul_cur_wm_cnt += bearer_cntx_p->tx_wm.wm_item.current_cnt;
    }
  }
  return total_ul_cur_wm_cnt;
} /* ds3gpp_get_total_uldl_wm_cnt_hdlr() */
/*===========================================================================
FUNCTION DS_3GPP_REG_TX_WM_NON_EMPTY_FUNC_HDLR

DESCRIPTION
  This function regiseter tx wm non_empty_func_ptr for all up bearers 
...
===========================================================================*/
void ds_3gpp_reg_tx_wm_non_empty_func_hdlr
(
  sys_modem_as_id_e_type  subs_id
)
{
  ...
  for (index = 0; index < DS_3GPP_MAX_BEARER_CONTEXT; index++)
  {
    ...
    {
      /*---------------------------------------------------------------------
        If sys mode is G, then no need to check if the bearer is up or not.
        As in case of G, bearer may not be in up state during data transfer.
      ---------------------------------------------------------------------*/
      ...
      {  
        /*-------------------------------------------------------------------
          Register tx wm non_empty_func_ptr .     
        -------------------------------------------------------------------*/
        (bearer_context_p->tx_wm.wm_item).
        non_empty_cb_ptrs[0]  = (wm_cb_type)ds_bearer_cntxt_non_empty_func_cb;
        (bearer_context_p->tx_wm.wm_item).
        non_empty_cb_data[0] = (void*) bearer_context_p;
      }     
    }
  }
} /* ds_3gpp_reg_tx_wm_non_empty_func_hdlr */
/*===========================================================================
...
)
{
  ...
  {
    DS_3GPP_MSG1_LOW("Current mode %d not a valid 3GPP mode."
  ...
  if(num_bytes_reqd ==0)
  {
    DS_3GPP_MSG1_LOW("%d Bytes reqd to report t'put Ind", num_bytes_reqd);
    local_err = DS_3GPPI_LOCAL_ERR_VAL_2;
    goto func_exit;
  }
  ...
  if(report_p == NULL)
  {
    DS_3GPP_MSG0_LOW("Mem Alloc to report t'put Ind failed");
    local_err = DS_3GPPI_LOCAL_ERR_VAL_3;
    goto func_exit;
  }
  ...
  if(FALSE == ds_3gpp_report_all_iface_throughput_info(cm_subs_id, report_p,
                                                       num_active_ifaces))
  {
    DS_3GPP_MSG1_LOW("reporting Throughput Info for CM subs ID failed:%d", 
                      cm_subs_id);
    local_err = DS_3GPPI_LOCAL_ERR_VAL_4;
    goto func_exit;
  }
  ...
  {
    DS_3GPP_MSG0_LOW("Info List generated is empty");
  ...
  else
  {
    ...
    (ps_sys_subscription_enum_type)ds3gsubsmgr_subs_id_ds3g_to_ds(ds3g_subs_id),
                              ...
                              &dss_errno))
    {
      DS_3GPP_MSG0_LOW("Sending Throughput Info Ind failed");
...
func_exit:
  ...
  if(local_err != DS_3GPPI_LOCAL_ERR_VAL_NONE)
  {
    DS_3GPP_ERROR_LOG_3_EX(ds3g_subs_id, 
                       "ds_3gpp_report_throughput_info_hdlr: err d subs id d "
                       " sys mode d",
                       local_err, ds3g_subs_id, current_mode);
  }
...
)
{
  ...
  ds_3gppi_local_err_val_e_type    local_err = DS_3GPPI_LOCAL_ERR_VAL_NONE;
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  DS_3GPP_MSG0_LOW(" In ds3gpp_ps_data_fail_ind()");
  ...
  if (hndl->data_suspend_flag == FALSE)
  {
    DS_3GPP_MSG0_LOW("PS Data Fail Ind received when not in suspended state, ignoring");
    local_err = DS_3GPPI_LOCAL_ERR_VAL_1;
    goto func_exit;
  }
  ...
  {
    DS_3GPP_MSG0_LOW("Unsolicited PS Data Fail Ind, ignoring");
    local_err = DS_3GPPI_LOCAL_ERR_VAL_2;
    goto func_exit;
  }
...
func_exit:
  if(local_err != DS_3GPPI_LOCAL_ERR_VAL_NONE)
  {
    DS_3GPP_ERROR_LOG_1_EX(subs_id, "ds3gpp_ps_data_fail_ind_hdlr: err d",
                       local_err);
  }
...
)
{
  ...
  *delay_connect = FALSE;
  ...
  {
    DS_3GPP_ERROR_LOG_1_EX(subs_id, "Suspended:Invalid Call Conn Cmd for call_id:d",call_id);
    return DS3G_FAILURE;
  }

  if ( mode_info_p == NULL )
  {
    DS_3GPP_ERROR_FATAL(" mode_info_p = NULL in call_connected_hdlr");
    return DS3G_FAILURE;
  }
  ...
  {
    DS_3GPP_MSG0_HIGH_EX(subs_id, "Bearer in pending redial state! Ignore");
    return DS3G_FAILURE;
  }
    ...
    )
  {
    ...
    if(bearer_cntx_p->ds_bearer_context_dyn_p->is_default)
    {
      ps_phys_link_type *phys_link = &(bearer_cntx_p->ds_bearer_context_dyn_p->phys_link);
         ...
         ) != DS3G_SUCCESS)
      {
        ...
        DATA_3GPP_MSG_SPRINTF_1_EX(MSG_LEGACY_HIGH,subs_id,
                                   "Call connect hdlr Returns FAILURE pdn 0x%x",pdn_cntx_p);
        return DS3G_FAILURE;
      }
    }
    }
    else
    {
         ...
         (call_id, mode_info_ptr_i,NULL, subs_id) != DS3G_SUCCESS)
      {
        ...
        DATA_3GPP_MSG_SPRINTF_1_EX(MSG_LEGACY_HIGH,subs_id,
                                   "Call connect hdlr Returns FAILURE non-def call id %d",call_id);
        return DS3G_FAILURE;
      }
    }
  ...
  {
    DS_3GPP_ERROR_LOG_3_EX(subs_id, "pdn cntx:x Invalid network_mode:d",
  ...
  DS_3GPP_MSG1_MED_EX(subs_id, "MH Client: CALL_ID values (%d)",callid);
  ...
  {
    case SYS_SYS_MODE_WCDMA:
      DS_3GPP_MSG1_MED_EX(subs_id, "MH Client: Registering with FC for UMTS_TECH, call id %d",
              callid);
    ...
    case SYS_SYS_MODE_GSM:
      DS_3GPP_MSG1_MED_EX(subs_id, "MH: Registering with FC for GSM_TECH, call id %d",
              callid);
    ...
    default:
      DS_3GPP_MSG2_MED_EX(subs_id, "MH: Not registering for UNKNOWN tech, call id %d mode %d",
              callid, network_mode);
      break;
  }
#endif /* FEATURE_QXDM_WM_TRACING */
...
)
{
  ...
  /*------------------------------------------------------------------------- 
    ...
    -------------------------------------------------------------------------*/
  duration = ds3gtputmgr_get_actual_tput_refresh_interval(ds3gsubs_id);

  if (duration == 0)
  {
    DS_3GPP_MSG1_HIGH_EX(cm_subs_id, "act t'put: DS3G subs id %d timer "
  ...
  DS_3GPP_MSG1_MED_EX(cm_subs_id, "act t'put: periodicity %d msec",
                      sample_interval);
  ...
  if(!DS_3GPP_IS_3GPP_CURRENT_MODE(current_mode))
  {
    DS_3GPP_MSG2_MED_EX(cm_subs_id,
                        "Not a valid sys mode %d on Subs id(CM) %d",
                        current_mode, cm_subs_id);
    return;
  }
  ...
  if(!ds_3gpp_bearer_context_refresh_actual_throughput(cm_subs_id,
                                                       sample_interval))
  {
    DS_3GPP_MSG1_MED_EX(cm_subs_id, 
             "No active bearers on Subs id(CM) %d to refresh Act tput"
             "values ", cm_subs_id);
  }
...
)
{
  ...
  ds_pdn_context_s                 *pdn_cntx_p;
  ds3g_call_info_cmd_type          *call_info_cmd_ptr = NULL;
  ds3g_incom_e_type                ret_val = DS3G_IGNORE;
  int                              temp_num = -1;
  ds_3gppi_local_err_val_e_type    local_err = DS_3GPPI_LOCAL_ERR_VAL_NONE;
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_3GPP_MSG2_HIGH_EX(subs_id, "Incoming call handler, call id %d, pdp group id %d",
  ...
  {
    DS_3GPP_MSG1_LOW("INCOM_CALL_HDLR: call with call id[%d] already present",
                       call_id);
  ...
  else
  {
    ...
    if(!ds_3gpp_pdn_cntx_validate_pdn_context(pdn_cntx_p))
    {
      DS_3GPP_MSG2_LOW("pdn context does not exist: group_id=%d call_id=%d",
                          ps_call_info->pdp_group_id,
                          call_id);
      end_params->end_params.gw_ps_end.cause = SM_UNKNOWN_PDP_CONTEXT;
      ret_val = DS3G_REJECT_CALL;
      temp_num = ps_call_info->pdp_group_id;
      local_err = DS_3GPPI_LOCAL_ERR_VAL_2;
      goto func_exit;
    }
    ...
    {
      DS_3GPP_MSG0_LOW("Got incoming call while PDN going down - Rejecting");
      ret_val = DS3G_REJECT_CALL;
      local_err = DS_3GPPI_LOCAL_ERR_VAL_3;
      goto func_exit;
    }
    ...
    if(ds_pdn_cntxt_store_bearer_context(pdn_cntx_p,(void *)bearer_cntx_p) != \
                                                DS3G_SUCCESS)
    {
      DS_3GPP_MSG0_LOW("Could not store the allocated bearer");
    ...
    {
      DS_3GPP_MSG0_LOW("Umts Bearer context passed is Invalid");
      ret_val = DS3G_REJECT_CALL;
      local_err = DS_3GPPI_LOCAL_ERR_VAL_5;
      goto func_exit;
    }
    ...
    {
      DS_3GPP_MSG0_HIGH_EX(subs_id, "PCO present in secondary activation request");      
    ...
    if (ds_flow_manager_nw_bearer_activation_req(pdn_cntx_p,
                                           ...
                                           DS3G_SUCCESS)
    {
      DS_3GPP_MSG0_LOW("Flow manager bearer act req failed");
      ds_bearer_cntxt_free_bearer_inst(bearer_cntx_p);
      if(ds_pdn_cntxt_release_bearer_context(pdn_cntx_p,(void *)bearer_cntx_p) !=
                                                   DS3G_SUCCESS)
      {
        DS_3GPP_MSG0_HIGH_EX(subs_id,"could not release the allocated bearer");
      }
      ds_bearer_cntxt_return_bearer_to_free_pool(bearer_cntx_p);
      ret_val = DS3G_REJECT_CALL;
      local_err = DS_3GPPI_LOCAL_ERR_VAL_6;
...
func_exit:
  if(local_err != DS_3GPPI_LOCAL_ERR_VAL_NONE)
  {
    DS_3GPP_ERROR_LOG_3_EX(subs_id, "dsumts_incoming_call_handler: err d  call id d temp d",
                       local_err, call_id, temp_num);
  }
...
)
{
  ...
  {
    DS_3GPP_MSG1_HIGH_EX(subs_id, "Null input parameters, call id: %d",call_id);
    return;
  }
  ...
  if (!ds_pdn_cntxt_validate_pdn_subscription(pdn_cntx_p, subs_id))
  {
    DS_3GPP_ERROR_LOG_0_EX(subs_id, "PDN Subscription Validation failed");
    answer_params->ans_params.gw_ps_ans.accept = FALSE;
    return;
  }
...
)
{
  ...
  DS_3GPP_MSG2_HIGH_EX(subs_id,
                       "CNTXT_TRANSFER_IND_HDLR Called ds_3gpp_sys_mode:%d",
                       mode,
                       target_rat);
  ...
  -----------------------------------------------------------------------*/
  if (target_rat == SYS_SYS_MODE_WCDMA ||
      target_rat == SYS_SYS_MODE_GSM ||
...
void* ds3gpp_get_sys_change_hdlr(void)
{
  static boolean is_sys_chg_reg;

  if(FALSE == is_sys_chg_reg)
  {
    is_sys_chg_reg = TRUE;
    return ds3gpp_sys_change_handler;
  }

  return NULL;
} /* ds3gpp_get_sys_change_hdlr */
...
rex_sigs_type dsumts_hdlr_init( void )
{
  ...
  */
    ...
    -------------------------------------------------------------------------*/
    hdlr_tbl_p->sys_change_handler        =  ds3gpp_get_sys_change_hdlr();
  ...
  */
    ...
    hdlr_tbl_p->set_downlink_throughput_running_t_accumulate = NULL;
    ...
    hdlr_tbl_p->reg_tx_wm_non_empty_func_hdlr = ds_3gpp_reg_tx_wm_non_empty_func_hdlr;
	hdlr_tbl_p->get_total_ul_cur_wm_cnt_hdlr = ds3gpp_get_total_ul_cur_wm_cnt_hdlr;
  ...
  */
    ...
    hdlr_tbl_p->context_mod_ind_handler = ds_3gpp_nw_context_modify_ind_hdlr;

    hdlr_tbl_p->nw_modify_req_handler = ds_3gpp_nw_context_modify_req_hdlr;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /*-------------------------------------------------------------------------
    Get the network mode for the bearer. If bearer_context_p is NULL (dedicated bearer),
    get the mode from the default bearer.
  -------------------------------------------------------------------------*/
  if (ds_bearer_cntx_validate_bearer_context(bearer_context_p))
  {
  ...
  else
  {
    ...
    {
      def_bearer_context_p = ds_bearer_cntxt_get_bearer_from_eps_id(
                               mode_info_ptr_i->info.lte_call.lbi, subs_id);

      if (!ds_bearer_cntx_validate_bearer_context(def_bearer_context_p))
      {
        return DS3G_FAILURE;
      }
      call_mode = def_bearer_context_p->ds_bearer_context_dyn_p->call_mode;
    }
    else
    {
      return DS3G_FAILURE;
  ...
  {
    DS_3GPP_ERROR_LOG_1_EX(subs_id, "DS 3GPP Invalid call_mode:d",call_mode);

    /*-----------------------------------------------------------------------
      Store the call_id here, otherwise bearer can't be cleaned up later
      when CM sends CALL_END for this call.
    -----------------------------------------------------------------------*/
    ...
    {
      DS_3GPP_ERROR_LOG_1_EX(subs_id, "Couldn't find bearer corresponding to SDF ID 0xx."
                         "Not storing call_id before rejecting",
  ...
  if (ds3gpp_get_ps_data_suspend_flag(subs_id) == TRUE)
  {
    DS_3GPP_ERROR_LOG_1_EX(subs_id, "Suspended:Invalid bearer activate ind call_id:d",call_id);
    //return DS3G_FAILURE;
  }
  ...
  if(mode_info_ptr_i->info.lte_call.lbi == ESM_INVALID_BEARER_ID)
  {
    if (bearer_context_p == NULL)
    {
      DS_3GPP_MSG1_HIGH_EX(subs_id, "Cannot find bearer corresponding to the SDF ID 0x%x,\
  ...
  DS_3GPP_MSG1_MED_EX(subs_id, "MH Client: CALL_ID value (%d)",callid);
  ...
  {
    case SYS_SYS_MODE_LTE:
      DS_3GPP_MSG1_MED_EX(subs_id, "MH Client: Registering with FC for LTE_TECH, call id %d",
              callid);
    ...
    default:
      DS_3GPP_MSG2_MED_EX(subs_id, "Not registering with FC for call_id: %d, mode %d",
                        callid, call_mode);
      break;
  }
#endif /* FEATURE_QXDM_WM_TRACING */
...
)
{
  ...
  if(bearer_context_p == NULL)
  {
    DS_3GPP_MSG1_HIGH_EX(subs_id, "Cannot locate bearer from SDF ID 0x%x while proc PDN conn Rej",
  ...
  {
    DS_3GPP_ERROR_LOG_2_EX(subs_id, "Invalid bearer call_mode:d, changing it to d",
                       (bearer_context_p->ds_bearer_context_dyn_p->call_mode), 
                        SYS_SYS_MODE_LTE);
    bearer_context_p->ds_bearer_context_dyn_p->call_mode = SYS_SYS_MODE_LTE;    
  }
  ...
  {
    DS_3GPP_ERROR_LOG_1_EX(subs_id, "Invalid Attach SM ptr for cm sub id d",subs_id);
    return;
  }

  /*-------------------------------------------------------------------------
    FOR GW originated
    if we have a cached pdn connectivity request and if it is GW
    originated and if the previous request was rejected because of a
    sequence number mismatch,don't tear down the PDN context,
    use the same profile parameters.Also ensure that the client is in_use
   
    For non GW originated
   
    1. Check if the reject is for 
  ...
  if( TRUE == mode_info_ptr->info.lte_call.esm_local_cause.valid)
  {
    DS_3GPP_MSG1_HIGH_EX(subs_id, "DEBUG_ATTACH: Local ESM cause code : %d",
                      mode_info_ptr->info.lte_call.esm_local_cause.local_cause);
  }
  ...
  {
    DS_3GPP_MSG3_HIGH_EX(subs_id, "Re originating cached PDN conn request:%d,cached_paging_info:%d on Subs Id(CM)"
                      "%d", attach_sm_ptr->cached_seq_num,attach_sm_ptr->cached_paging_response,
                      subs_id);
    DS_3GPP_MSG3_HIGH_EX(subs_id, "Setting the pdn context and bearer context to PEND_REDIAL",
                      0,0,0);
...
)
{
  ...
  if(bearer_context_p == NULL)
  {
    DS_3GPP_MSG1_HIGH_EX(subs_id, "Cannot locate bearer from SDF ID: %d while proc PDN conn Fail",
               mode_info_ptr->info.lte_call.sdf_id);
    return;
  }
...
{ 
  ...
  if( (cmd_ptr == NULL) || (cmd_ptr->cmd_payload_ptr == NULL) )
  {
    DS_3GPP_ERROR_LOG_0("NULL pointer passed" );
    return;
  }
  call_info_cmd_ptr = (ds3g_call_info_cmd_type*)cmd_ptr->cmd_payload_ptr;

  DS_3GPP_MSG0_HIGH_EX(call_info_cmd_ptr->subs_id,
                       "In ds3gpp_eps_pdn_conn_req_err_hdlr" );
  ...
  DS_3GPP_MSG0_HIGH_EX(call_info_cmd_ptr->subs_id,
                       "calling ds_bearer_cntxt_deactivate_cnf" );
  ds_bearer_cntxt_deactivate_cnf
  (
    bearer_context_p,
    call_info_cmd_ptr,
...
)
{
  ...
  {
    DS_3GPP_ERROR_LOG_1_EX(subs_id, "DS 3GPP Invalid network_mode:d,\
...
rex_sigs_type dseps_hdlr_init( void )
{
  ...
  if (hdlr_tbl_p)
  {
    ...
    hdlr_tbl_p->set_downlink_throughput_running_t_accumulate = ds_eps_dl_estimated_throughput_set_running_t_accumulate;
    hdlr_tbl_p->mode_pref_change_handler = ds_3gpp_mode_pref_change_hdlr;
...
)
{
  ...
  if (ds3gpp_validate_per_subs_info_hndl(hndl))
  {
    ...
    DS_3GPP_MSG2_MED_EX(subs_id, "Attach initiated flag for Subs Id(CM): %d is %d",
                      subs_id, flag_val);
  }
  return;
...
)
{
  ...
  if (ds3gpp_validate_per_subs_info_hndl(hndl))
  {
    ...
    DS_3GPP_MSG2_MED_EX(subs_id, "Attach complete flag for Subs Id(CM): %d is %d",
                      subs_id, flag_val);
  }
...
)
{
  ...
  if (ds3gpp_validate_per_subs_info_hndl(hndl))
  {
    ...
    DS_3GPP_MSG2_MED_EX(subs_id, "Attach complete flag for cm subs id: %d is %d",
                      subs_id, flag);
  }
...
)
{
  ...
  if (ds3gpp_validate_per_subs_info_hndl(hndl))
  {
    ...
    DS_3GPP_MSG2_MED_EX(subs_id, "PS Data Suspend flag for cm subs id: %d is %d",
                      subs_id, flag_val);
  }
...
)
{
  ...
  if (ds3gpp_validate_per_subs_info_hndl(hndl))
  {
    ...
    DS_3GPP_MSG2_MED_EX(subs_id, "PS Data available flag for subs id: %d is %d",
                      subs_id, flag_val);
  }
...
)
{
  ...
  if (ds3gpp_validate_per_subs_info_hndl(hndl))
  {
    ...
    DS_3GPP_C_MSG2(DS_3GPP_F3_MASK, "PS Data Available flag for Subs Id(CM): %d is %d",
                      subs_id, flag);
  }
...
)
{
  ...
  do
  {
    ...
    {
      DS_3GPP_MSG1_LOW("ds_3gpp_event_register_cb: Event registration failed. "
  ...
  /*Check the error code. If there is any error, return failure.*/
  if(local_err != DS_3GPPI_LOCAL_ERR_VAL_NONE)
  {
    DS_3GPP_ERROR_LOG_2("ds_3gpp_event_register_cb:  err d evt d - cb ptr NULL"
                       "/invalid ev/buf alloc fail",
                        local_err, event);
    return DS3G_FAILURE;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {    
    ...
    {
      DS_3GPP_ERROR_LOG_0("ds_3gpp_process_oprt_mode_chg_cmd: NULL "
                         "event_cback_f_ptr");
      continue;
    }
...
)
{
  ...
  {
    DS_3GPP_ERROR_LOG_0("cb_func_tbl_p is NULL");
    return;
  }
  ...
  cb_func_tbl_p->apn_type_change_handler = ds_3gpp_apn_type_chg_hdlr;
...
rex_sigs_type ds3gpp_hdlr_init( void )
{
  ...
  /* wait_timer list is initialized in roaming module */
  ds_3gpp_roaming_hdlr_powerup_init();
  ds_3gpp_config_mgr_powerup_init();
  ds_3gpp_cc_sp_init();
  ds3g_generic_config_powerup_init();
  return requested_sigs;
}
/*===========================================================================
...
)
{
  ...
  ds_3gpp_nv_manager_ex_init_per_subs_id(subs_id);
#ifdef FEATURE_DATA_LTE
  ds_dsd_apm_init_per_subsid(subs_id);
  ds_3gpp_pdn_limit_powerup_init(subs_id);
  ds_3gpp_dam_read_efs_per_subs_id(subs_id);
#endif
#ifdef FEATURE_DATA_RPM
  ...
  ds3g_generic_config_powerup_init_per_subs_id(subs_id);
  ds_3gpp_config_mgr_powerup_init_per_subs_id(subs_id);
}
...
/*===========================================================================
FUNCTION DS3GPP_HDLR_EVENT_REG
...
/* ------------------------------------------------ */
  ...
  {
    DS_3GPP_MSG0_LOW("Registration for DDS mode EV failed !");
    local_err = DS_3GPPI_LOCAL_ERR_VAL_0;
    goto func_exit;
  }
  ...
  if (ds3gsubsmgr_event_reg(DS3GSUBSMGR_DEVICE_MODE_EV, 
                            ...
                            NULL, &client_id_ds3gsubsmgr ) == FALSE) 
  {
    DS_3GPP_MSG0_LOW("Registration for Device mode EV failed !");
    local_err = DS_3GPPI_LOCAL_ERR_VAL_1;
    goto func_exit;
  }
                                                                ...
                                                                subs_id++)
  {
    ...
    if (ds3geventmgr_event_reg(DS3GEVENTMGR_MODE_LPM_EV,
                               ...
                               NULL) == FALSE)
    {
      DS_3GPP_MSG0_LOW("Registration for MODE LPM EV failed !");
      local_err = DS_3GPPI_LOCAL_ERR_VAL_2;
      goto func_exit;
    }
    ...
    if (ds3geventmgr_event_reg(DS3GEVENTMGR_SUB_NOT_AVAIL_EV,
                               ...
                               NULL) == FALSE)
    {
      DS_3GPP_MSG0_LOW("Registration for CARD REFRESH EV failed !");
      local_err = DS_3GPPI_LOCAL_ERR_VAL_3;
      goto func_exit;
    }
    ...
    if(ds3geventmgr_event_reg(DS3GEVENTMGR_NV_REFRESH_EV,
                              ...
                              NULL) == FALSE)
    {
      DS_3GPP_MSG0_LOW("Registration for NV REFRESH EV failed !");
      local_err = DS_3GPPI_LOCAL_ERR_VAL_4;
      goto func_exit;
    }
    ...
    if(ds3geventmgr_event_reg(DS3GEVENTMGR_THERMAL_MITIGATION_EV,
                              ...
                              NULL) == FALSE)
    {
      DS_3GPP_MSG0_LOW("Registration for THERMAL MITIGATION EV failed !");
      local_err = DS_3GPPI_LOCAL_ERR_VAL_5;
      goto func_exit;
    }
    if (ds3geventmgr_event_reg(DS3GEVENTMGR_LATENCY_INFO_EV,
                               ...
                               NULL) == FALSE)
    {
      DS_3GPP_MSG0_LOW("Registration for Latency Info EV failed !");
      local_err = DS_3GPPI_LOCAL_ERR_VAL_6;
      goto func_exit;
    }
    ...
    if (ds3geventmgr_event_reg(DS3GEVENTMGR_PLMN_LIST_CHG_EV,
                               ...
                               NULL) == FALSE)
    {
      DS_3GPP_MSG0_LOW("Registration for PLMN LIST CHG EV failed !");
      local_err = DS_3GPPI_LOCAL_ERR_VAL_7;
      goto func_exit;
    }

    if (ds3geventmgr_event_reg(DS3GEVENTMGR_APM_SET_ATTACH_PROFILE_EV,
                               DS3GEVENTMGR_CLIENT_ID_3GPP,
                               &filter_info,
                               ds3gpp_ds3g_event_cb,
                               NULL) == FALSE)
    {
      DS_3GPP_MSG0_LOW("Registration for set attach profile ev failed !");
...
func_exit:
  if(local_err != DS_3GPPI_LOCAL_ERR_VAL_NONE)
  {
    DS_3GPP_ERROR_LOG_1_EX(subs_id, "ds_3gpp_hdlr_event_reg: err d",
                       local_err);
    DS3GPP_ASSERT(0);
  }
...
)
{
  ...
  if (cmd_ptr == NULL)
  {
    DS_3GPP_ERROR_LOG_0("cmd ptr is NULL");
    return;
  }
  ...
  DS_3GPP_MSG0_MED_EX(*subs_id_p, "ds_3gpp_stop_sending_data_avail");
...
)
{
  if ( cmd_ptr == NULL )
  {
    DS_3GPP_ERROR_FATAL("null cmd_ptr");
    return;
  }
  ...
  switch( cmd_ptr->hdr.cmd_id )
  {
    ...
    case DS_CMD_3GPP_RMSM_STOP_PPP_DELAY_TIMER:
...
#endif /*FEATURE_DATA_FLOW_MGMT */
    case DS_CMD_3GPP_UNUSED_PDN_TMR_EXP_CMD:
    case DS_CMD_3GPP_PDN_POLLING_TMR_EXP:
    case DS_CMD_3GPP_PDN_APN_PARAM_CHG_TMR_EXP:
    case DS_CMD_3GPP_PRE_ATTACH_TIMER_EXP:
    case DS_CMD_3GPP_PRE_ATTACH_SYS_INFO:
    case DS_CMD_3GPP_APN_SWITCH_SYS_INFO:
    case DS_CMD_3GPP_APN_SWITCH_DETACH_OR_DISCONNECT:
    ...
    case DS_CMD_3GPP_OTASN_PDN_TMR_EXP:
    ...
    case DS_CMD_3GPP_PROC_WRRC_DATA_RATE_IND:
    case DS_CMD_3GPP_CONFIGURE_ULDL_DATA_RATE:
#ifdef FEATURE_DATA_RAVE_SUPPORT
     case DS_CMD_3GPP_UPLINK_DATA_RATE_IND:
#endif
  case DS_CMD_3GPP_DOWNLINK_ESTM_DATA_RATE_IND:
#ifdef FEATURE_DATA_LTE
    ...
    case DS_CMD_3GPP_LB_UL_RAB_DEREG_CNF:
    case DS_CMD_3GPP_SYS_SEL_PREF_ERR:
    case DS_CMD_3GPP_SYS_SEL_PREF_NO_ERR:
    case DS_CMD_3GPP_CM_PH_OPRT_MODE_CB_ERR:
    case DS_CMD_3GPP_CM_PH_OPRT_MODE_CB_NO_ERR:
    case DS_CMD_3GPP_CM_PH_INFO_CB_ERR:
    case DS_CMD_3GPP_CM_PH_INFO_CB_NO_ERR:
    ...
    case DS_CMD_3GPP_PROFILE_DB_APN_TYPE_CHANGE:
    case DS_CMD_3GPP_PROFILE_DB_APN_DISABLE:
    case DS_CMD_3GPP_PROFILE_PERSISTENT_OPTIMIZER:
    ...
    case DS_CMD_3GPP_APM_WRITE_ATTACH_PROFILE_INFO:
    case DS_CMD_3GPP_PLMN_LIST_CHG:
    case DS_CMD_3GPP_APN_NAME_CHG_CMD:
    case DS_CMD_3GPP_DL_TPUT_INIT_TIMER:
    case DS_CMD_3GPP_WRITE_HPLMN_LIST:
    case DS_CMD_3GPP_INVOKE_IFACE_EV_CB_HDLR:
    case DS_CMD_3GPP_APN_PREF_SYS_CHG:
    case DS_CMD_3GPP_DPM_INFO_CNF:
#ifdef FEATURE_LAPP_UFS
    #error code not present
#endif /* FEATURE_LAPP_UFS  */ 

#ifdef FEATURE_DATA_FLOW_MGMT
    case DS_CMD_3GPP_FLOW_CTL_CMD:
    case DS_CMD_3GPP_LOG_FLOW_CTRL_LATENCY:
#endif /*FEATURE_DATA_FLOW_MGMT */
    case DS_CMD_3GPP_NON_EMPTY_CB_CMD:
    case DS_CMD_3GPP_UDT_BARRING_TIMER_EXPIRED:
    case DS_CMD_3GPP_IFACE_TCP_LEVEL_TRM_REQ_INFO:
    case DS_CMD_3GPP_PDN_TCP_RETRANS_PROTECT_TMR_EXP:
    case DS_CMD_3GPP_OMADM_SET_PSDATAOFF_INFO:
    case DS_CMD_3GPP_PHYS_LINK_EV_CB:
   ...
   (defined(FEATURE_GSM) && defined(FEATURE_DATA_GCSD)))
    ...
    case DS_CMD_3GPP_GSNDCP_DEREG_WM_CNF:
      ds_gprs_bearer_cntxt_process_gsndcp_ev_cmd(cmd_ptr);
      break;
#endif /* FEATURE_GSM_GPRS */

#if defined (FEATURE_DATA_WLAN_MAPCON) && defined ( FEATURE_DATA_LTE)
    case DS_CMD_3GPP_KAMGR_SYS_INFO:
    case DS_CMD_3GPP_KAMGR_TIMER_EXPIRY_CB:
    case DS_CMD_3GPP_KAMGR_QMI_BRING_UP_IND:
    case DS_CMD_3GPP_KAMGR_QMI_TEAR_DOWN_IND:
    case DS_CMD_3GPP_KAMGR_DEFAULT_APN_CHG_IND:
      ds_3gpp_kamgr_process_cmd(cmd_ptr);
      break;
    ...
    case DS_CMD_3GPP_SET_UL_TPUT_THRES_CRITERIA:
    case DS_CMD_3GPP_SET_DL_TPUT_THRES_CRITERIA:
    {
      ds3gtputthres_process_ds_cmd(cmd_ptr);
      break;
    }
    case DS_CMD_3GPP_APPLY_V6_PREFIX_IND:
      ds_3gpp_net6i_apply_v6_prefix_from_iface_cmd(cmd_ptr->cmd_payload_ptr);
      break;
    default:
...
)
{
  ...
  DS_3GPP_MSG1_HIGH_EX(subs_id, "DS3GPP Nv Refresh for SUB ID %d",subs_id);

  ds_3gpp_nv_refresh_in_progress[subs_id] = TRUE;
  
  /* Reset network override table */
  ds_3gpp_network_override_hdlr_reset(subs_id,FALSE, FALSE);  

  ds_3gpp_config_mgr_init_data_3gpp_cfg_per_sub(subs_id);
  ...
  /*Reset global throttling masks*/
  ds_3gpp_pdn_throt_unblock_throttling_on_nv_refresh(subs_id);

  /*Reset pdn throttling*/
  ds_3gpp_pdn_throt_reset_pdn_throttle(subs_id,rat_mask);
  ...
  //Do the reading of the device-settings after NVs are read from New MBN.....
  ds_3gpp_pdn_throt_read_device_settings_info(subs_id);

  /* read the ims-settings after NVs are read from New MBN */
  ds_3gpp_pdn_throt_mgr_read_ims_settings_info(subs_id);
	
  /*reset and initialize Service request throttling*/
  ds_3gpp_srv_req_throt_go_null(subs_id, TRUE);
  ...
  ds_3gpp_dam_refresh_per_subs_id(subs_id);
  ds_3gpp_dam_read_efs_per_subs_id(subs_id);
  
  /*----------------------------------------------------------------------------------
   NV refresh, Exit LTE Attach pending state, clean null bearer reason bit if it's set
  ----------------------------------------------------------------------------------*/
  ds_eps_pdn_cntxt_ex_reset_attach_pending(subs_id);
#endif
  ...
  ds_3gpp_config_mgr_roam_hdlr_clear_per_subs_id(subs_id);
  ...
  ds_3gpp_kamgr_dealloc_kamgr_instance( subs_id );
  ds_3gpp_kamgr_read_efs_info ( subs_id );
  ...
  ds3gpp_device_ev_hdlr_clear_omadm_ps_data_off_service_info_entry(subs_id);
  ds3geventmgr_set_and_notify_nv_refresh_status(DS3GEVENTMGR_CLIENT_ID_3GPP,
                                                subs_id);

  ds_3gpp_nv_refresh_in_progress[subs_id] = FALSE;

  DS_3GPP_MSG1_HIGH_EX(subs_id, "DS3GPP Nv Refresh for SUB ID Done %d",subs_id);

  return;
}
/*===========================================================================
FUNCTION  DS_3GPP_HDLR_REPORT_CONFIGURED_ULDL_TPUT

DESCRIPTION
 This functions is invoked a part of configured tput commands process.
 This function inturn invokes throughputmager toinform the configureddata rates.
...
===========================================================================*/
void ds_3gpp_hdlr_report_configured_uldl_tput
(
   sys_modem_as_id_e_type  subs_id,
...
)
{
  ...
  ds3gtputmgr_report_uldl_configured_throughput_response(
     ds3gsubsmgr_subs_id_cm_to_ds3g(subs_id),
     uplink_configured_kbps,
     downlink_configured_kbps);
}
/*===========================================================================
FUNCTION      DS_3GPP_HDLR_REPORT_ACHIEVABLE_DATE_RATE

DESCRIPTION   This function starts the timer value based on the frequency and 
              inturn updates the lower layer with timer value and start/stop 
...
SIDE EFFECTS  None
===========================================================================*/
void ds_3gpp_hdlr_report_achievable_date_rate
( 
  sys_modem_as_id_e_type         subs_id,
  uint32                         frequency
)
{
  ...
  boolean                     any_wcdma_bearer_up=FALSE;
  ds3gsubsmgr_subs_id_e_type  ds3g_subs_id = DS3GSUBSMGR_SUBS_ID_INVALID;
  uint32                      timer_val = 0;
  boolean                     lte_ml1_status= FALSE;
  boolean                     need_to_report = FALSE;
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  timer_val = ds_3gpp_nv_manager_get_uldl_tput_t_tmeas(subs_id);

  ds3g_subs_id = ds3gsubsmgr_subs_id_cm_to_ds3g(subs_id);


  any_lte_bearer_up = ds_3gpp_bearer_cntxt_check_if_any_bear_up_sys_mode(
                         subs_id,
                         SYS_SYS_MODE_LTE);

  any_wcdma_bearer_up = ds_3gpp_bearer_cntxt_check_if_any_bear_up_sys_mode(
                         subs_id,
                         SYS_SYS_MODE_WCDMA);

  lte_ml1_status = ds_eps_lte_ml1_is_connected(subs_id);


  /*Handle during irat and call tear down*/
  /*If there is no LTE bearer up and LTE achievable in progress; stop 
    uldl timer and inform lte lower  layer*/
  if((!any_lte_bearer_up || !lte_ml1_status) && 
     ds3gtputmgr_get_lte_achievable_in_progress(ds3g_subs_id) )
  {
#ifdef FEATURE_DATA_LTE
        ds_eps_througput_uldl_stop_ind(subs_id);
#endif
        ds3gtputmgr_reset_lte_achievable_in_progress(ds3g_subs_id);
        /*Inform lte lower layer to stop*/
        need_to_report = FALSE;
  }

  /*If there is no WCDMA bearer up and WCDMA achievable in progress; 
    stop uldl timer and inform WCDMA lower layer*/

  if(!any_wcdma_bearer_up && 
     ds3gtputmgr_get_wcdma_achievable_in_progress(ds3g_subs_id) )
  {
#ifdef FEATURE_DATA_WCDMA_PS
        ds_wcdma_achievable_tput_startstop(FALSE);
#endif
        ds3gtputmgr_reset_wcdma_achievable_in_progress(ds3g_subs_id);
        /*Inform wcdma lower layer to stop*/
        need_to_report = FALSE;
  }
  ...
  /*Scenario frequency update or frequency initialize from app*/
  /*If there is atleast one LTE bearer up and LTE achievable in progress 
    is not set ; start uldl timer and inform lte lower  layer*/

    if(any_lte_bearer_up && lte_ml1_status)
    {
      if(!ds3gtputmgr_get_lte_achievable_in_progress(ds3g_subs_id))
      {
#ifdef FEATURE_DATA_LTE
        ds_eps_througput_uldl_start_ind(subs_id,timer_val,frequency);
#endif
        ds3gtputmgr_set_lte_achievable_in_progress(ds3g_subs_id);
      }
      /*Inform lte lower layer*/
      need_to_report = TRUE;
    }

    /*If there is atleast one WCDMA bearer up and WCDMA achievable 
      in progress is not set ; start uldl timer and inform lte
      lower  layer*/

    if(any_wcdma_bearer_up)
    {
      if(!ds3gtputmgr_get_wcdma_achievable_in_progress(ds3g_subs_id))
      {
#ifdef FEATURE_DATA_WCDMA_PS
        ds_wcdma_achievable_tput_startstop(TRUE);
#endif
        ds3gtputmgr_set_wcdma_achievable_in_progress(ds3g_subs_id);
      }
      /*Inform wcdma lower layer*/
      need_to_report = TRUE;
    }
  }
  else
  {
    /*If the frequency is zero and LTE achievable in progress; stop 
      uldl timer and inform lte lower  layer*/
...
#endif
        ds3gtputmgr_reset_lte_achievable_in_progress(ds3g_subs_id);
        /*Inform lte lower layer to start*/
        need_to_report = FALSE;
      }

    /*If the frequency is zero and WCDMA achievable in progress; stop 
      uldl timer and inform wcdma lower  layer*/
  ...
  ds3gtputmgr_set_throughput_report_status(DS3G_TPUT_CLIENT_QMI,
                                           ds3g_subs_id,
                                           need_to_report);
  return;
}/*ds_3gpp_hdlr_report_achievable_date_rate*/

#ifdef FEATURE_DATA_RAVE_SUPPORT
/*===========================================================================
FUNCTION      DS_3GPP_HDLR_GET_UPLINK_STATUS

DESCRIPTION   This function is invoked to get the uplink status in wcdma and 
              lte rat
...
SIDE EFFECTS  None
===========================================================================*/
uint32 ds_3gpp_hdlr_get_current_uplink_status
(
  sys_modem_as_id_e_type       subs_id
)
{
  boolean   any_lte_bearer_up        = FALSE;
  boolean   any_wcdma_bearer_up      = FALSE;
  boolean   any_lte_bearer_dormant   = FALSE;
  boolean   any_wcdma_bearer_dormant = FALSE;
  ...
  any_lte_bearer_dormant = ds_3gpp_bearer_cntxt_check_if_any_bear_dormant_sys_mode(
                             subs_id,
                             SYS_SYS_MODE_LTE);

  any_wcdma_bearer_dormant = ds_3gpp_bearer_cntxt_check_if_any_bear_dormant_sys_mode(
                               subs_id,
                               SYS_SYS_MODE_WCDMA);


    if(any_lte_bearer_dormant || any_wcdma_bearer_dormant)
    {
      return DS3G_REPORT_DISABLED_ALL_CALLS_DORMANT;
    }
    else
    {
      return DS3G_REPORT_DISABLED_NO_DATA_CALL;
    }
}
#endif
/*===========================================================================
FUNCTION      DS_3GPP_HDLR_QUERY_ULDL_THROUGHPUT_REQUEST

DESCRIPTION   This function querys the lowerlayer for achievable data rate 
              in case of lte and wcdma rats 
...
SIDE EFFECTS  None
===========================================================================*/
void ds_3gpp_hdlr_query_uldl_throughput_request
(
   boolean                   query_achievable,
   sys_modem_as_id_e_type    subs_id
)
{
  ds3gsubsmgr_subs_id_e_type  ds3g_subs_id;
  uint32       achievable_ul_data_rate = 0;
  uint32       achievable_dl_data_rate = 0;
  ...
  if(query_achievable)
  {
    ...
    {
  #ifdef FEATURE_DATA_LTE
       ds_eps_througput_uldl_query_req(subs_id);
  #endif
    }
    else if(ds3gtputmgr_get_wcdma_achievable_in_progress(ds3g_subs_id))
    {
  #ifdef FEATURE_DATA_WCDMA_PS
       achievable_ul_data_rate = ds_wcmda_get_achievable_ul_data_rate();
       achievable_dl_data_rate = ds_wcmda_get_achievable_dl_data_rate();
  #endif
       ds3gtputmgr_report_uldl_throughput_response(ds3g_subs_id,
...
)
{
  ...
  ds_umts_pdp_profile_status_etype     result;  
  ds_3gpp_downlink_estimated_data_ind  *downlink_est_data_ptr = NULL;
  ps_sys_dl_throughput_info_type       dl_tput_info;
  ds_bearer_dpm_cb_info_s_type         *dpm_info_ptr = NULL;

#ifdef FEATURE_DATA_FLOW_MGMT
  ds_bearer_wm_flow_control_info_type    *wm_flow_info_ptr = NULL;
#endif /*FEATURE_DATA_FLOW_MGMT */
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

  if ( cmd_ptr == NULL )
  {
    DS_3GPP_ERROR_FATAL("PROC_PS_CMD: null cmd_ptr!");
    return;
  }
  ...
  switch(cmd_ptr->hdr.cmd_id)
  {
    ...
    case DS_CMD_3GPP_INVOKE_IFACE_EV_CB_HDLR:
      ds_3gpp_iface_ev_cback_cmd_hdlr(cmd_ptr);
      break;
    ...
    case DS_CMD_3GPP_RPM_SAMPLING_TIMER_EXP:
      ds_3gpp_rpm_sampling_tmr_exp_hdlr(cmd_ptr);
      break;
    ...
    case DS_CMD_3GPP_PDN_THROT_UNBLOCK_APN_TIMER_EXP:
      if(cmd_ptr->cmd_payload_ptr != NULL)
      {
        ...
        ds_pdn_throt_spec_unblock_apn_timer_exp_cmd_hdlr
    ...
    case DS_CMD_3GPP_PDN_THROT_IMSI_INFO_AVAIL:
      if(cmd_ptr->cmd_payload_ptr != NULL)
      {
       ...
       ds_3gpp_rpm_read_sim_info(subs_id);
        ...
        ds_pdn_throt_spec_imsi_info_avail_cmd_hdlr(subs_id);
#ifdef FEATURE_DATA_LTE
        if(ds_3gpp_pdn_limit_is_enabled(subs_id))
        {
    ...
    case DS_CMD_3GPP_OTASN_PDN_TMR_EXP:
      if(cmd_ptr->cmd_payload_ptr != NULL)
      {
        ds_3gpp_pdn_cntxt_otasn_delay_timer_exp_cmd_hdlr
          (FALSE , (sys_modem_as_id_e_type)
           (*((sys_modem_as_id_e_type*)cmd_ptr->cmd_payload_ptr)));
      }
      break;
#endif /*FEATURE_DATA_REL10*/
    case DS_CMD_3GPP_INIT_AFTER_POWERUP:
...
#if defined (FEATURE_DATA_WLAN_MAPCON) && defined (FEATURE_DATA_LTE) 
      ds_3gpp_kamgr_read_efs_info(ds3g_get_ps_subs_id());
#endif
      ...
      /* Retrieve the initial config from the Policy Manager. */
      ds3gsubmgr_read_policyman_cfg();
      break;
    ...
    case DS_CMD_3GPP_PDN_THROT_CLEAR_APN_REJ_T3402_TMR:
      if(cmd_ptr->cmd_payload_ptr != NULL)
      {
        ds_3gpp_dam_t3402_timer_expiry_cmd_hdlr(cmd_ptr);
      }
      break;
    ...
    case DS_CMD_3GPP_PROFILE_DB_PARAM_CHG:
      if(cmd_ptr->cmd_payload_ptr != NULL)
      {
        ...
        { 
          ...
          if( result != DS_UMTS_PDP_SUCCESS)
          {
             DS_3GPP_ERROR_LOG_1_EX(subs_id, "Error in retrieving definition for prof number:d",
                         param_chg_ptr->profile_num);
          }
        }
      }
    ...
    case DS_CMD_3GPP_LB_DL_RAB_REG_CNF:
    case DS_CMD_3GPP_LB_UL_RAB_DEREG_CNF:
       ds_3gpp_loopback_hdlr_process_cmds(cmd_ptr);
       break;
#endif
    ...
    case DS_CMD_3GPP_SYS_SEL_PREF_ERR:
     if ( cmd_ptr->cmd_payload_ptr != NULL)
     {
       ds_3gpp_plmn_sys_sel_pref_err_cmd_hdlr(cmd_ptr->cmd_payload_ptr);
     }
      break;

    case DS_CMD_3GPP_SYS_SEL_PREF_NO_ERR:
      ds_3gpp_plmn_sys_sel_pref_no_err_cmd_hdlr(cmd_ptr->cmd_payload_ptr);
 #ifdef FEATURE_DATA_LTE
    ...
    case  DS_CMD_3GPP_CM_PH_INFO_CB_ERR:
      ds3g_get_cm_ph_info_cb_err();
      break;
    
    case DS_CMD_3GPP_CM_PH_INFO_CB_NO_ERR:
      ds3g_get_cm_ph_info_cb_no_err();
      break;
    ...
    case DS_CMD_3GPP_CONFIGURE_ULDL_DATA_RATE:
       if(cmd_ptr->cmd_payload_ptr != NULL)
       {
         configured_data_info_ptr = 
           (ds_3gpp_configured_datarate_type*)cmd_ptr->cmd_payload_ptr;
         if(configured_data_info_ptr != NULL)
         {
           ds_3gpp_hdlr_report_configured_uldl_tput(configured_data_info_ptr->subs_id,
                                                    configured_data_info_ptr->uplink_configured_kbps,
                                                    configured_data_info_ptr->downlink_configured_kbps);
         }
       }
    break;
#ifdef FEATURE_DATA_RAVE_SUPPORT
    case DS_CMD_3GPP_UPLINK_DATA_RATE_IND:
      if(cmd_ptr->cmd_payload_ptr != NULL)
      {
        ds_wcdma_uplink_data_rate_ind_hdlr(cmd_ptr->cmd_payload_ptr);
      }
      break;
#endif
    case DS_CMD_3GPP_DOWNLINK_ESTM_DATA_RATE_IND:
    {
      if(cmd_ptr->cmd_payload_ptr != NULL)
      {
        downlink_est_data_ptr = 
          (ds_3gpp_downlink_estimated_data_ind*)cmd_ptr->cmd_payload_ptr;
        if(downlink_est_data_ptr != NULL)
        {
          ds3gtputmgr_process_downlink_throughput_estimation_ind(downlink_est_data_ptr->subs_id,
                                                downlink_est_data_ptr->data_rate_in_kbps,
                                                downlink_est_data_ptr->confidence,
                                                downlink_est_data_ptr->data_freeze,
                                                downlink_est_data_ptr->moved_out
                                               );
        }
      }
      break;
    ...
    case DS_CMD_3GPP_PROFILE_PERSISTENT_OPTIMIZER:
    {
      ds_3gpp_profile_persistent_optimizer_cmd_hdlr(cmd_ptr);
      break;
    }
    ...
    case DS_CMD_3GPP_PROFILE_DB_APN_TYPE_CHANGE:
    {
      ds_3gpp_pdn_cntx_apn_type_chg_cmd_hdlr(cmd_ptr);
      break;
    }

    case DS_CMD_3GPP_APN_PREF_SYS_CHG:
    {
      ds_3gpp_process_apn_pref_sys_cmd(cmd_ptr);
      break;
    }
#ifdef FEATURE_DATA_LTE
    case DS_CMD_3GPP_PROFILE_DB_APN_DISABLE:
...
#ifdef FEATURE_DATA_WCDMA_PS
    case DS_CMD_3GPP_PROC_WRRC_DATA_RATE_IND:
      ds_wcdma_bearer_cntxt_proc_wrrc_data_rate_ind_cmd ( cmd_ptr );
      break;
#endif /* FEATURE_DATA_WCDMA_PS */
    ...
    case DS_CMD_3GPP_FLOW_CTL_CMD:
    {
      ...
      {
        wm_flow_info_ptr = 
          (ds_bearer_wm_flow_control_info_type *)cmd_ptr->cmd_payload_ptr;
        if (NULL != wm_flow_info_ptr)
        {
          ds_3gpp_burst_hdlr_process_flow_ctl_cmd(wm_flow_info_ptr->bearer_context_p, 
                                                    wm_flow_info_ptr->flow_enable,
                                                    wm_flow_info_ptr->flow_mask);
        }
      }
      break;
    }/* DS_CMD_3GPP_WM_FLOW_INFO */

    case DS_CMD_3GPP_LOG_FLOW_CTRL_LATENCY:
    {
      if (cmd_ptr->cmd_payload_ptr != NULL)
      {       
        ds_3gpp_burst_flow_log_latency_ev(
           (ds3g_burst_based_flow_control_s *)cmd_ptr->cmd_payload_ptr);
      }
      break; 
    }/* DS_CMD_3GPP_LOG_FLOW_CTRL_LATENCY */
#endif /*FEATURE_DATA_FLOW_MGMT */
    case DS_CMD_3GPP_DPM_INFO_CNF:
    {
      if (cmd_ptr->cmd_payload_ptr != NULL)
      {
        dpm_info_ptr = 
            (ds_bearer_dpm_cb_info_s_type *)cmd_ptr->cmd_payload_ptr;

        if (NULL != dpm_info_ptr)
        {
           ds_bearer_cntxt_dpm_ev_cb_hdlr(dpm_info_ptr->dpm_event, 
                                          &(dpm_info_ptr->dpm_info));
        }
      }
      break;

    }/* DS_CMD_3GPP_DPM_INFO_CNF */

    case DS_CMD_3GPP_UDT_BARRING_TIMER_EXPIRED:
    {
      ds_eps_udt_mgr_process_barring_timer_exp(cmd_ptr);
      break;
    }

    case DS_CMD_3GPP_OMADM_SET_PSDATAOFF_INFO:
      ds3gpp_device_ev_hdlr_update_omadm_ps_data_off_cache(cmd_ptr);
      break;

    case DS_CMD_3GPP_PHYS_LINK_EV_CB:
      ds_3gpp_phys_link_ev_cback_hdlr(cmd_ptr);
      break;
    ...
    case DS_CMD_3GPP_APN_SWITCH_DETACH_OR_DISCONNECT:
    {
      if(cmd_ptr->cmd_payload_ptr != NULL)
      {
        ds_3gpp_apn_switch_mgr_perform_detach_or_disconnect_action
           (*((sys_modem_as_id_e_type*)cmd_ptr->cmd_payload_ptr));
      }
      break;
    }
    ...
    default:
      DS_3GPP_MSG1_HIGH_EX(subs_id, "Unexpected cmd: %d, in ds3gpp_processPSCmd, ignored.",
               cmd_ptr->hdr.cmd_id);
      break;
  } /* switch( cmd_id ) */
...
)
{
  ...
  if (bearer_context_p == NULL)
  {
    DS_3GPP_ERROR_FATAL("NULL bearer context pointer for WM tracing.");
  }
...
)
{
  ...
  if( ps_errno == NULL )
  {
    DS_3GPP_MSG0_LOW("ps_errno is NULL");
    ret_val = -1;
    local_err = DS_3GPPI_LOCAL_ERR_VAL_0;
    goto func_exit;
  }
  ...
  if (arg_val_ptr == NULL)
  {
    DS_3GPP_MSG0_LOW("arg_val_ptr is NULL");
    *ps_errno = DS_EFAULT;
    ret_val = -1;
    local_err = DS_3GPPI_LOCAL_ERR_VAL_1;
    goto func_exit;
      ...
      ( pdn_context_ptr != NULL ) )
  {
    ...
    {
      DS_3GPP_MSG0_LOW("PDN context is not in use.(v4)");
           ...
           ( pdn_context_ptr != NULL ))
  {
    ...
    {
      DS_3GPP_MSG0_LOW("PDN context is not in use.(v6)");
      *ps_errno = DS_ENETDOWN;
      ret_val = -1;
      local_err = DS_3GPPI_LOCAL_ERR_VAL_3;
      goto func_exit;
  ...
  {
    DS_3GPP_MSG0_LOW("APN is not matched in any pdn context either v4 or v6");
    *ps_errno = DS_ENETDOWN;
    ret_val = -1;
    local_err = DS_3GPPI_LOCAL_ERR_VAL_4;
    goto func_exit;
  ...
  {
    DS_3GPP_MSG1_MED_EX(subs_id, "bearer_id = %d", nsapi_info_p->nsapi[temp_index]);
  }
  return 0;
...
func_exit:
  if(local_err != DS_3GPPI_LOCAL_ERR_VAL_NONE)
  {
    DS_3GPP_ERROR_LOG_1_EX(subs_id, "ds3gpp_get_nsapi_from_apn: err d",
                       local_err);
  }
...
)
{
  ...
  DS_3GPP_MSG2_MED_EX(subs_id, "Current Pref mode %d Is BSR in Progress? %d",
  ...
  if(bsr_status == FALSE)
  {
    ...
    {
      DS_3GPP_MSG0_MED_EX(subs_id, "Tearing down PDN Connections");
      ds_3gpp_pdn_cntxt_tear_down(subs_id);
    }
  }
  else
  {
    DS_3GPP_MSG0_MED_EX(subs_id, "BSR compuation in progress.Do not teardown"
                     "PDN connections");
  }
...
)
{
  ...
  {
    DS_3GPP_MSG0_HIGH_EX(subs_id, "No need to process PLMN change as new PLMN is NULL");
    return;
  }
  ...
  ds_3gpp_pdn_process_new_plmn(subs_id, new_plmn_id,
                              ds3gpp_get_current_network_mode(subs_id));
  ds_3gpp_pdn_cntxt_disable_lte_if_attach_profile_unavailable(subs_id);
  ds_eps_udt_mgr_plmn_chg_hdlr(subs_id);
...
{
  ds_3gpp_pdn_throt_clr_on_plmn_change(network_mode, plmn_id, subs_id);
  ds_3gpp_roaming_inform_new_plmn_info(subs_id, plmn_id);
  ds_3gpp_pdn_throt_advertise_blocked_apns_with_plmn (subs_id, plmn_id);
...
)
{
 if (!DS_3GPP_IS_3GPP_CURRENT_MODE(new_pref_mode))
 {
   DS_3GPP_MSG1_HIGH_EX(subs_id,
                        "Non-3gpp mode in mode_pref_change: %d, ignoring",
                        new_pref_mode);
   return;
 }
...
)
{
  ...
  if (ds3gpp_validate_per_subs_info_hndl(hndl))
  {
    ...
    DS_3GPP_MSG3_HIGH_EX(subs_id, "Context transfer Target Rat is %d and cntxt transfer "
...
)
{
  ...
  if (ds3gpp_validate_per_subs_info_hndl(hndl))
  {
    ...
    DS_3GPP_MSG2_MED_EX(subs_id, "Context Transfer Info received: %d for Subs Id(CM): %d",
                      flag, subs_id);
  }
...
)
{
  ...
  {
    DS_3GPP_ERROR_LOG_1("Subs info hndl:0xx is not valid",hndl);
  }
...
#endif /* FEATURE_TDSCDMA */
    default:
      DS_3GPP_ERROR_LOG_1 ("Invalid sys mode passed: d", 
                          mode );
      break;
  }
...
)
{
  ...
  /*-----------------------------------------------------------------------
    Register for VoLTE switch status indication
  -----------------------------------------------------------------------*/
      ds3gmsgr_reg_msgr_cb(DS_APPSRV_AUTOREG_VOLTE_SWITCH_STATUS_IND,
                       (ds3g_msgr_client_func_ptr)ds3gpp_device_ev_hdlr_volte_switch_status_ind);
  ...
  if (local_err != DS_3GPPI_LOCAL_ERR_VAL_NONE)
  {
    DS_3GPP_ERROR_LOG_1_EX(subs_id, "per_subs_info_init: err d - mem alloc fails",
                       local_err);
  }
...
)
{
  ...
  switch (event_id)
  {
    case DS3GSUBSMGR_DDS_CHANGE_EV:
    {
      ...
      {
        ds_3gpp_pdn_cntxt_local_teardown(
    ...
    case DS3GSUBSMGR_DEVICE_MODE_EV:
    {
      switch (event_info_ptr->device_mode.curr)
      {
        case SYS_MODEM_DEVICE_MODE_SINGLE_SIM:
        {
          ...
          ds_3gpp_pre_attach_hdlr_init(curr_subs_id);
          ds_3gpp_apn_switch_hdlr_init(curr_subs_id);
#ifdef FEATURE_DATA_LTE
...
#if defined (FEATURE_DATA_WLAN_MAPCON) && defined (FEATURE_DATA_LTE) 
          ds_3gpp_kamgr_read_efs_info(curr_subs_id);
#endif
          ds_3gpp_apn_pref_sys_per_subs_init(curr_subs_id);
        ...
        case SYS_MODEM_DEVICE_MODE_TRIPLE_SIM_TRIPLE_STANDBY:
        {
          for (i=0; i< DS3GSUBSMGR_SUBS_ID_MAX; i++)
          {
            ...
            ds_3gpp_pre_attach_hdlr_init(i);
            ds_3gpp_apn_switch_hdlr_init(i);
#ifdef FEATURE_DATA_LTE
...
#if defined (FEATURE_DATA_WLAN_MAPCON) && defined (FEATURE_DATA_LTE) 
            ds_3gpp_kamgr_read_efs_info(i);
#endif
            ds_3gpp_apn_pref_sys_per_subs_init(i);
            ds3gpp_device_ev_hdlr_init_after_powerup();
          }
          break;
        } /* case SYS_MODEM_DEVICE_MODE_DUAL_SIM_DUAL_STANDBY */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    //"Invalid input data, Sub ID Mask not provided");
    local_err = DS_3GPPI_LOCAL_ERR_VAL_0;
    goto func_exit;
  }
  ...
  /* Only thermal mitigation event has a payload passed */
  ...
  DS_3GPP_MSG1_HIGH_EX(subs_id, "Processing DS3G event on subs id %d", subs_id);
  ...
  switch (event_id) 
  {
    case DS3GEVENTMGR_MODE_LPM_EV:
      ...
      if(ds3geventmgr_set_lpm_pwr_off_status(
         ...
         == FALSE)
      {
        DS_3GPP_MSG1_HIGH_EX(subs_id, "Cannot set lpm pwr off %d Bit msk",
                          DS3GEVENTMGR_LPM_PWROFF_3GPP_CTRL_BIT_MSK);
      }
    ...
    case DS3GEVENTMGR_SUB_NOT_AVAIL_EV:
      ...
      if(ds3geventmgr_set_sub_not_avail_status
                                (DS3GEVENTMGR_SUB_NOT_AVAIL_3GPP_CTRL_BIT_MSK,
                                 subs_id) == FALSE)
      {
        DS_3GPP_MSG2_HIGH_EX(subs_id, "Cannot set SUB_NOT_AVAIL %d Bit msk on subs %d",
                          DS3GEVENTMGR_SUB_NOT_AVAIL_3GPP_CTRL_BIT_MSK,
                          subs_id);
      }
      ...
      else
      {
        ...
        DS_3GPP_MSG2_HIGH_EX(subs_id, "Network mode %d is not a 3GPP mode. Not processing "
    ...
    case DS3GEVENTMGR_LATENCY_INFO_EV:
      if(event_info_ptr == NULL)
      {
         //"ds3gpp_ds3g_event_cb: event_info_ptr NULL");
         local_err = DS_3GPPI_LOCAL_ERR_VAL_3;
         goto func_exit;
      }

      any_lte_bearer_up = ds_3gpp_bearer_cntxt_check_if_any_bear_up_sys_mode
                            (subs_id,SYS_SYS_MODE_LTE);
      any_lte_bearer_dormant = ds_3gpp_bearer_cntxt_check_if_any_bear_dormant_sys_mode
                                 (subs_id,SYS_SYS_MODE_LTE);
      if ( any_lte_bearer_up || any_lte_bearer_dormant) 
      {
        ...
        ds_mgr_low_latency_info.traffic_state = 
                    (ds_low_latency_traffic_state_enum_type)ps_latency_info->traffic_state;
        ...
        DS_3GPP_MSG1_HIGH_EX(subs_id, "ds_3gpp_low_latency_ind_cmd: traffic state:%d",
                            ds_mgr_low_latency_info.traffic_state);
   ...
   case DS3GEVENTMGR_APM_NEW_ATTACH_PROFILE_EV:
    case DS3GEVENTMGR_APM_SET_ATTACH_PROFILE_EV:
#ifdef FEATURE_DATA_LTE
      /*Enables each plmn id unblocked if profile
        available on plmn id*/
      ds_3gpp_pdn_cntxt_enable_lte_each_plmnid(subs_id); 
      /*Disables current plmnid if profile available on plmn id*/
#endif
     break;
    default:
      //"Unrecognized DS3G event %d on subs id %d", 
      //event_id, subs_id);
      local_err = DS_3GPPI_LOCAL_ERR_VAL_4;
      goto func_exit;
      break;
...
func_exit:
  if(local_err != DS_3GPPI_LOCAL_ERR_VAL_NONE)
  {
    DS_3GPP_ERROR_LOG_2_EX(subs_id, "ds3gpp_ds3g_event_cb: err d event id d",
                       local_err, event_id);
  }
...
/*===========================================================================
FUNCTION ds_3gpp_hdlr_is_ue_in_ac11to15

DESCRIPTION
  This function checks whether the ue is in ac 11 to 15 on the current 
  subscription. This function returns true if 
...
===========================================================================*/
boolean ds_3gpp_hdlr_is_ue_in_ac11to15
(
   sys_modem_as_id_e_type   subs_id
)
{
   ...
   return ds_3gpp_hdlr_is_ue_in_ac11to15_on_plmnid(subs_id,current_plmn_id);

}
/*===========================================================================
FUNCTION ds_3gpp_hdlr_is_ue_in_ac11to15_on_plmnid

DESCRIPTION
  This function checks whether the ue is in ac 11 to 15 on the current 
  subscription in the plmn id specified. This function returns true if 
  one of the  below scenarios is True 
...
boolean ds_3gpp_hdlr_is_ue_in_ac11to15_on_plmnid
(
   sys_modem_as_id_e_type   subs_id,
   sys_plmn_id_s_type       current_plmn_id
)
{   
  ...
  {
    DS_3GPP_ERROR_LOG_0_EX(subs_id, "plmn_id passed is zero");
    return ue_ac_11to15;
  }
  ...
  do
  {
    ...
    if(ds3g_mmgsdi_get_efacc_class(subs_id,&efacc_class))
    {
      ...
      if ((efacc_class & DS_3GPP_AC_CLASS_12_to_14) != 0)
      {
        ...
        {
          DS_3GPP_MSG0_HIGH_EX(subs_id, "AC 12-14 and MCC match");
          ue_ac_11to15 = TRUE;
          break;
        }
      ...
      if((efacc_class & DS_3GPP_AC_CLASS_11_or_15)!= 0)
      {
        ...
        {
          DS_3GPP_MSG0_HIGH_EX(subs_id, "EHPLMN match");
          ue_ac_11to15 = TRUE;
          break;
        }
        /*---------------------------------------------------------- 
        ...
        {
          DS_3GPP_MSG0_HIGH_EX(subs_id, "Home PLMN match");
          ue_ac_11to15 = TRUE;
          break;
        }
      }
...
/*===========================================================================
FUNCTION DS_3GPP_APN_PREF_SYS_PER_SUBS_INIT

DESCRIPTION
  This function registers for the APN Preferred System Indication from 
  PS framework.
...
SIDE EFFECTS
  None.
===========================================================================*/
void ds_3gpp_apn_pref_sys_per_subs_init
(
  sys_modem_as_id_e_type subs_id
)
{
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
    ...
    ret_val = ps_sys_event_reg_ex( PS_SYS_TECH_ALL,
                                   PS_SYS_EVENT_APN_PREF_SYS_CHANGE,
    ...
    if( 0 > ret_val )
    {
      DS_3GMGR_ERROR_LOG_3_EX(MSG_LEGACY_ERROR, subs_id,
                       "ds_3gpp_apn_pref_sys_per_subs_init: Error while "
                       "registering for PS_SYS_EVENT_APN_PREF_SYS_CHANGE "
                       "event, return value d and error d, "
                       "PS subs index: d ",
                       ret_val, ps_errno, subs_id);
    }
  
#endif /* FEATURE_DATA_WLAN_MAPCON */
  return;
} /* ds_3gpp_apn_pref_sys_per_subs_init */

/*===========================================================================
FUNCTION       DS_3GPP_GET_BIT_RATE_QUERY_SUPPORT_CONFIG_INFO

DESCRIPTION    This utility function is invoked via PS_SYS IOCTL when AP
               invokes the QMI API to get query bit rate support config
...
int ds_3gpp_get_bit_rate_query_support_config_info
(
  void                    *arg_val_ptr,
  sys_modem_as_id_e_type   subs_id,
  int16                   *ps_errno
...
{
  ps_sys_ioctl_3gpp_ran_asst_support_config_type         *query_bit_rate_config;
  ...
  query_bit_rate_config = (ps_sys_ioctl_3gpp_ran_asst_support_config_type*)arg_val_ptr;
  memset(query_bit_rate_config,
         0,
         sizeof(ps_sys_ioctl_3gpp_ran_asst_support_config_type));
  memset(&config,
         0,
         sizeof(ds_3gpp_ran_bit_data_rate_support_config));

  ret = ds3gpp_get_bit_data_rate_supp_config(subs_id, &config);
  if (ret == E_SUCCESS )
  {
    query_bit_rate_config->bit_rate_update_support = config.query_bit_data_rate_support;
    *ps_errno = DS_ENOERR;
    ret_val = 0;
  }
  else
...
func_exit:
  return ret_val;
} /* ds_3gpp_get_bit_rate_query_support_config_info */
