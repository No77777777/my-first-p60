...
$Header: //components/rel/data.mpss/3.5..2.1/3gpp/umtsps/src/ds_umts_bearer_context.c#4 $
...
#define DS_UMTS_BEARER_CTXT_VFR_CALL_NO_ARG_PER_BEARER(rval, func, bearer_context_p )  \
            DS_3GPPI_VFR_CALL_NO_ARG_PER_BEARER(umts_bearer_context_ftbl, rval, func, bearer_context_p)
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch(map_rule)
  {
    ...
    case CALL_END_DS_CAUSE_CODES_TO_PS_IFACE_CAUSE_CODES:
      ...
      {
        case CM_CALL_END_INTERNAL_ERROR:
        *ps_indr_output_ptr = PS_NET_DOWN_REASON_CM_UNKNOWN_ERROR;
        break;
        ...
        default:
          *ps_indr_output_ptr = PS_NET_DOWN_REASON_NOT_SPECIFIED;
      }
   ...
   and SM_INTERNAL_CAUSE (mode_info_ptr->info.gw_ps_call.pdp_cause_type) */
              ...
              case SM_ABORT_SERVICE_NOT_AVAILABLE:
                *ps_indr_output_ptr = PS_NET_DOWN_REASON_SM_ABORT_SERVICE_NOT_AVAILABLE;
                break;
               /* Internal cause code to indicate QoS is not accepted at NAS*/
              case SM_INTERNAL_QOS_NOT_ACCEPTED:
               *ps_indr_output_ptr = PS_NET_DOWN_REASON_QOS_NOT_ACCEPTED;
                break;
              default:
                *ps_indr_output_ptr = PS_NET_DOWN_REASON_NOT_SPECIFIED;
                DS_3GPP_ERROR_LOG_1_EX(subs_id,  "unknown SM_INTERNAL cause-code [d]", 
...
)
{
  ...
  if (!ds3gsubsmgr_is_subs_id_valid(subs_id))
  {
   ...
   (void)ds_bearer_flow_manager_go_null_flow(bearer_context_p);

    /* Release the memory for the flow context
       and clear the array entries for this bearer do this from a
       flow context context */
   (void)ds_bearer_flow_manager_delete_all_flow_context
   ...
   ---------------------------------------------------------------------*/
   ps_phys_link_gone_ind_ex(&(bearer_context_p->ds_bearer_context_dyn_p->phys_link), 
                                                                              ps_eic);
   ps_phys_link_enable_flow(&(bearer_context_p->ds_bearer_context_dyn_p->phys_link), 
                                                                   DS_FLOW_UMTS_MASK);
   ...
   DS_3GPP_MSG1_HIGH_EX(subs_id,
                        "Physlink gone, BEARER DOWN IND CB for bearer :%x",
                        bearer_context_p);
...
)
{
  ...
  ret_val = ps_phys_link_up_cmd( &(bearer_context_p->ds_bearer_context_dyn_p->
                                   phys_link),
                                 &ps_errno,
                                 NULL );
...
/*------------------------------------------------------------------------*/
  ...
  DS_3GPP_MSG2_HIGH("Bearer go_null hdlr: call id:%d Bearer Inst:%d",
  ...
  {
     ds_bearer_context_wm_s *tx_watermark_p = &(bearer_context_p->tx_wm);
     DS_3GPP_MSG1_HIGH("Default down. Dedicated up %d",
  ...
  if(ds_3gpp_pdn_cntx_validate_pdn_context(pdn_context_p))
  {
    ...
    {
      ps_phys_link_type *phys_link = &(def_bearer_cntxt_p->
                                         ds_bearer_context_dyn_p->phys_link);
      //Check to see if default is not dormant.
      if ((PS_PHYS_LINK_GET_STATE(phys_link) == PHYS_LINK_DOWN) &&            
            (def_bearer_cntxt_p->ds_bearer_context_dyn_p->state ==  // clang_sa_ignore [core.NullDereference]
             DS_BEARER_CONTEXT_STATE_DOWN) && 
            (pdn_context_p->ds_pdn_context_dyn_p->num_active_bearer_context == 1)) // clang_sa_ignore [core.NullDereference]
       {     
         DS_3GPP_MSG1_HIGH("Freeing the default bearer %d",def_bearer_cntxt_p);
         dsUmtsBearerCntxt_Free(
            def_bearer_cntxt_p,
            pdn_context_p,
            DS_3GPP_PS_IFACE_NET_DOWN_REASON(down_reason),
...
)
{
  ...
  cm_call_mode_info_s_type           *mode_info_p = NULL;
  boolean                            rel12_plus = FALSE;
  ds_pdn_throt_spec_parsed_backoff_info_s  parsed_info;
  timer_3gpp_type3                         backoff_timer;
  reattempt_ind_T                          reattempt_indicator;
  ds_pdn_throt_rule_info                   throt_rule_info;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if(call_info_p == NULL)
  {
    return;
  ...
  memset(&throt_rule_info, 0, sizeof(ds_pdn_throt_rule_info));
  mode_info_p = (cm_call_mode_info_s_type*)modem_mem_alloc(
  ...
  /*---------------------------------------------------------------------------
    For IP fallback use case, we might have requested IPv6 only connection
    over the air eventhough profile says PDP type was v4v6.
    Below function takes care of removing the cached Iface from PDN context tbl
    and updating the APN IP support information. 
  ---------------------------------------------------------------------------*/
  if(ds_3gpp_ip_fallback_is_ip_fallback_active(subs_id))
  {
    if(!ds_3gpp_ip_fallback_update_pdn_cntxt_info(pdn_context_p, subs_id))
    {      
      goto func_exit;
    }
  }/* ds_3gpp_ip_fallback_is_ip_fallback_active */
      ...
      rel12_plus)
  {
    ...
    rat_mask |= ds_3gpp_pdn_throt_sm_sys_mode_to_rat_mask(sys_sys_mode);
  }
  ...
  {
    DS_3GPP_ERROR_LOG_1_EX(subs_id, "Trapped, trying to end already ended call. Bearer instance = "
                       "d, returning...",
      (uint32)(bearer_context_p->ds_bearer_context_dyn_p->client_data_ptr));
      goto func_exit;
  }
 ...
 /*----------------------------------------------------------------------------------- 
  ...
  if(ds_3gpp_pdn_cntxt_is_bearer_deactivated_by_network(pdn_context_p,
                                                        (void *)bearer_context_p))

  {
     ds_3gpp_network_override_hdlr_set_overridden_ip
                      (pdn_context_p->ds_pdn_context_dyn_p->pdp_profile.context.apn,
                       ...
                       call_info_p->mode_info.info.gw_ps_call.reattempt_indicator,
                       FALSE,
                       TRUE,
                       subs_id);
  }
  ...
  if ( call_info_p->end_type == DS_CM_CALL_END_INTERNAL )
  {
    ...
    if( !dsUmtsBearerCntx_map_cause_codes( 
                 ...
                 subs_id ) )
    {
      DS_3GPP_MSG1_LOW("CALL_ENDED_HDLR: Mapping failed for CALL_END_INTERNAL!, call id =%d",
                call_info_p->call_id);
    }
    ...
    {
      down_reason.reason.ps_iface_down_reason = PS_NET_DOWN_REASON_MPLMN_IN_PROGRESS;
    }
    else 
    {
      if( !dsUmtsBearerCntx_map_cause_codes( 
    ...
    do
    {
           ...
           (bearer_context_p->ds_bearer_context_dyn_p->down_reason_ps_eic != PS_EIC_NOT_SPECIFIED) )
      {
        ...
        bearer_context_p->ds_bearer_context_dyn_p->down_reason_ps_eic = PS_EIC_NOT_SPECIFIED;
                        ...
                        subs_id ) )
      {
        DS_3GPP_MSG1_LOW("CALL_ENDED_HDLR: Mapping failed for CALL_END_EXTERNAL!, call id=%d",
                            call_info_p->call_id);
      }
      ...
      {
        down_reason.reason_T = DS_3GPP_PS_IFACE_NET_DOWN_REASON_T;
        down_reason.reason.ps_iface_down_reason = bearer_context_p->
                                           ds_bearer_context_dyn_p->down_reason;
      ...
      /*----------------------------------------------------------------------------- 
        If cm_call_end_cause is set to CLIENT_END, No need to perform any kinds of
        throttling below, simply break
      -----------------------------------------------------------------------------*/
      if(cm_call_end_cause == CM_CALL_END_CLIENT_END)
      {
        DS_3GPP_MSG0_HIGH("Client end, no need to perform throttling checks");
        break;
      }
      
      backoff_timer = mode_info_p->info.gw_ps_call.backoff_timer;
      reattempt_indicator = mode_info_p->info.gw_ps_call.reattempt_indicator;
   
      /*-------------------------------------------------------------------- 
      Fetch the parsed info
      -------------------------------------------------------------------*/
      ds_pdn_throt_spec_parse_backoff_info
        (backoff_timer, reattempt_indicator, subs_id,
         down_reason.reason.ps_iface_down_reason, &parsed_info,
         SYS_SYS_MODE_WCDMA);

      throt_rule_info.rejected_pdn_cntx_p = pdn_context_p;
      throt_rule_info.mode_info_p = mode_info_p;
      throt_rule_info.sys_mode = bearer_context_p->ds_bearer_context_dyn_p->call_mode;
      throt_rule_info.down_reason = down_reason.reason.ps_iface_down_reason;
      throt_rule_info.parsed_info_p = &parsed_info;

      /*-----------------------------------------------------------------------
        Update network override table with the IP type allowed for the
        next PDN connection if -
        1. If IP fallback is configured in EFS and
        2. UE is rejected with a fallback cause code that is configured in
           the XML other than CC #50 (For CC 50, we want apply spec throttling)
      -----------------------------------------------------------------------*/        
      if (ds_3gpp_ip_fallback_is_ip_fallback_active(subs_id) && 
         (throt_rule_info.down_reason != 
          PS_NET_DOWN_REASON_IP_V4_ONLY_ALLOWED) &&
          ds_3gpp_cfg_mgr_is_down_reason_configured_in_fallback_db(
              throt_rule_info.down_reason, subs_id))
      {
        ds_3gpp_network_override_hdlr_set_overridden_ip(
               throt_rule_info.rejected_pdn_cntx_p->ds_pdn_context_dyn_p->\
               pdp_profile.context.apn,
               DS_3GPP_PDN_THROT_PDP_IPV4,
               DS_3GPP_PDN_THROT_CONN_REJ,
               DS_3GPP_THROT_RAT_MASK_UMTS,
               reattempt_indicator,
               TRUE,
               FALSE,
      ...
      ds_pdn_throt_rule_notify(&throt_rule_info);

      if(ds_3gpp_nv_manager_is_rel_10_or_above_supported(subs_id))
      {
  ...
  {
    DS_3GPP_ERROR_LOG_1_EX(subs_id, "CALL_ENDED_HDLR: Unknown PDP Cause Type recvd from CM/SM!, call id=d",
              call_info_p->call_id);
  } 
  ...
  func_exit:
    modem_mem_free(mode_info_p, MODEM_MEM_CLIENT_DATA);
    return;
...
)
{
  ...
  DS_3GPP_MSG2_HIGH("Setting Physlink Pointer 0x%p to bearer_id %d",
  ...
  if ( !dsUmtsBearerCntxt_conv_qos_netw_to_umts( netw_qos_p,umts_qos_p ) )
  {
    ...
    DS_3GPP_MSG1_LOW("Unable to convert Netw to UMTS QoS, Bearer Inst:%d",
              (uint32)bearer_context_p->ds_bearer_context_dyn_p->client_data_ptr);
    ds_bearer_cntxt_set_bearer_state(bearer_context_p,
  ...
  if ( !dsUmtsBearerCntxt_conv_qos_netw_to_gprs( netw_qos_p,gprs_qos_p ) )
  {
    ...
    DS_3GPP_MSG1_LOW("Unable to convert Netw to GPRS QoS, Bearer Inst:%d",
              (uint32)bearer_context_p->ds_bearer_context_dyn_p->client_data_ptr);
    ds_bearer_cntxt_set_bearer_state(bearer_context_p,
                                     DS_BEARER_CONTEXT_STATE_GOING_DOWN);
    return DS3G_FAILURE;
  ...
  DS_3GPP_MSG1_HIGH("In activateCnf Netw QoS converted to UMTS & GPRS, Bearer Context Inst:%d",
           (uint32)bearer_context_p->ds_bearer_context_dyn_p->client_data_ptr);
  ...
  {    
    ...
    if(result == FALSE)
    {
      DS_3GPP_MSG0_LOW("Bind Flow failed for bearer");
    }
  }
...
)
{
  ds_pdn_context_s                  *pdn_context_p = NULL;
  sys_modem_as_id_e_type             subs_id = SYS_MODEM_AS_ID_NONE;
  uint32                             rat_mask = DS_3GPP_THROT_RAT_MASK_UMTS;
  boolean                          is_attach_apn = FALSE;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if(bearer_context_p->ds_bearer_context_dyn_p->state == DS_BEARER_CONTEXT_STATE_UP)
  {
    ...
    if(PS_PHYS_LINK_GET_STATE(&(bearer_context_p->ds_bearer_context_dyn_p->phys_link))
                                                                        != PHYS_LINK_UP)
    {
  ...
  if(ds_3gpp_nv_manager_is_rel_10_or_above_supported(subs_id))
  {
    ...
    ds_3gpp_pdn_throt_handle_nw_inited_unblock_apn
      (bearer_context_p->ds_bearer_context_dyn_p->apn, subs_id, rat_mask);

  
    if (ds_dsd_apm_is_apn_in_attach_pdn_list(
          subs_id, (char*)bearer_context_p->ds_bearer_context_dyn_p->apn))
    {
      is_attach_apn = TRUE;
    }
  ...
  {
    ds_3gpp_rpm_handle_pdp_act_accept((pdn_context_p->ds_pdn_context_dyn_p
                                         ->pdp_profile.context.apn),
                                        subs_id);
  }
  ...
  if(ds_3gpp_throt_sm_is_cc_sp_throttling_enabled(subs_id))
  {
    ...
    {
      if(!ds_3gpp_cc_sp_throt_is_attach_apn_def_throttled(subs_id))
    {
      ...
      if(!ds_3gpp_cc_sp_is_plmn_unblock_timer_running(subs_id))
      {
        ds_3gpp_cc_sp_throt_set_plmn_unblock_timer(subs_id, throt_tmr_val);
      }
    }
    }
    ds_3gpp_pdn_throt_clear_throttle_state(pdn_context_p->ds_pdn_context_dyn_p
                                          ->pdp_profile.context.apn,
                                           DS_UMTS_PDP_IPV4V6,
                                           subs_id, DS_3GPP_THROT_RAT_MASK_UMTS,
...
{
  #if !defined(FEATURE_LAPP) || !defined(FEATURE_NAS_REL7)  
  uint32 max_bitrate;
  #endif
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  -------------------------------------------------------------------------*/
  ds_umts_bearer_flow_manager_convert_app_flow_to_nw_flow(
     req_qos->max_dl_bitrate,
    &(orig_params_ptr->qos.max_bit_rare_downlink),
    &(orig_params_ptr->qos.ext_max_bit_rate_downlink),
    &(orig_params_ptr->qos.ext2_max_bit_rate_downlink),
    &(orig_params_ptr->extended_qos.max_bit_rate_dl),
    &(orig_params_ptr->extended_qos.unit_max_bit_rate),
     subs_id);
  ...
  ds_umts_bearer_flow_manager_convert_app_flow_to_nw_flow(
    ...
    &(orig_params_ptr->qos.ext2_gtd_bit_rate_downlink),
    &(orig_params_ptr->extended_qos.gbr_dl),
    &(orig_params_ptr->extended_qos.unit_gbr),
     subs_id);
  ...
  ds_umts_bearer_flow_manager_convert_app_flow_to_nw_flow(
    ...
    &(orig_params_ptr->qos.ext2_max_bit_rate_uplink),
    &(orig_params_ptr->extended_qos.max_bit_rate_ul),
    &(orig_params_ptr->extended_qos.unit_max_bit_rate),
     subs_id);
  ...
  ds_umts_bearer_flow_manager_convert_app_flow_to_nw_flow(
    ...
    &(orig_params_ptr->qos.ext2_gtd_bit_rate_uplink),
    &(orig_params_ptr->extended_qos.gbr_ul),
    &(orig_params_ptr->extended_qos.unit_gbr),
     subs_id);
...
)
{
  ...
  ds_umts_bearer_flow_manager_convert_app_flow_to_nw_flow(
    ...
    &(orig_params_ptr->minimum_qos.ext2_max_bit_rate_downlink),
    &(orig_params_ptr->extended_qos.max_bit_rate_dl),
    &(orig_params_ptr->extended_qos.unit_max_bit_rate),
     subs_id);
  ...
  ds_umts_bearer_flow_manager_convert_app_flow_to_nw_flow(
    ...
    &(orig_params_ptr->minimum_qos.ext2_gtd_bit_rate_downlink),
    &(orig_params_ptr->extended_qos.gbr_dl),
    &(orig_params_ptr->extended_qos.unit_gbr),
     subs_id);
  ...
  ds_umts_bearer_flow_manager_convert_app_flow_to_nw_flow(
    ...
    &(orig_params_ptr->minimum_qos.ext2_max_bit_rate_uplink),
    &(orig_params_ptr->extended_qos.max_bit_rate_ul),
    &(orig_params_ptr->extended_qos.unit_max_bit_rate),
     subs_id);
  ...
  ds_umts_bearer_flow_manager_convert_app_flow_to_nw_flow(
    ...
    &(orig_params_ptr->minimum_qos.ext2_gtd_bit_rate_uplink),
    &(orig_params_ptr->extended_qos.gbr_ul),
    &(orig_params_ptr->extended_qos.unit_gbr),
     subs_id);
...
)
{
  ...
  else
  {
    ...
    {
      DS_3GPP_ERROR_LOG_0_EX(subs_id, "Missing UMTS Bearer Flow Manager!");
      return DS3G_FAILURE;
    }
  ...
  {
    DS_3GPP_MSG0_LOW("AT+CGEQREQ specified: filling user req qos");
    dsUmtsBearerCntxt_fill_orig_r99_qos_params(req_qos,                                               
                                               orig_params_ptr,
                                               subs_id
                                               );
    /*-------------------------------------------------------------------------
  ...
  if( (req_qos_r9798 != NULL) && (req_qos_r9798->valid_flg) )
  {
    ...
    -----------------------------------------------------------------------*/
    DS_3GPP_MSG0_LOW("AT+CGQREQ specified: filling user req qos");
  ...
  {
    DS_3GPP_MSG0_LOW("AT+CGEQMIN specified: filling Minimum qos");
    dsUmtsBearerCntxt_fill_origination_r99_min_qos_params(
                  min_qos,
                  orig_params_ptr,
                  subs_id
  ...
  if( (min_qos_r9798 != NULL) && (min_qos_r9798->valid_flg) )
  {
    /*-----------------------------------------------------------------------
      ...
      Fill parameters as-is from atcop. No further encoding is required
    -----------------------------------------------------------------------*/
    DS_3GPP_MSG0_LOW("AT+CGQREQ specified: filling Minimum qos");
...
{  
  ...
  phys_link = &(bearer_context_p->ds_bearer_context_dyn_p->phys_link);
  /*-------------------------------------------------------------------------
...
)
{
  ...
  DS_3GPP_MSG1_HIGH("In ds_umts_bearer_cntxt_transfer_setup_hdlr for bearer_inst:%d", 
                    (uint32)(bearer_context_p->ds_bearer_context_dyn_p->client_data_ptr));
  ...
  {    
    ...
    DS_3GPP_MSG2_HIGH("Using nsapi %d for BearerInst:%d",
...
===========================================================================*/
LOCAL void dsUmtsBearerCntxt_getEndParams
(
  ds_bearer_context_s              *bearer_context_p,
...
)
{
  ...
  DS3GPP_ASSERT(ds3gi_call_info != NULL);
  ...
  DS_3GPP_MSG2_HIGH("User End Hdlr: call id:%d BearerInst:%d",
		  bearer_context_p->ds_bearer_context_dyn_p->cm_call_id,\
		  (uint32)bearer_context_p->ds_bearer_context_dyn_p->client_data_ptr);
  ...
  *delay_disconnect = FALSE;
  ...
  if(bearer_context_p->ds_bearer_context_dyn_p->down_reason == 
...
LOCAL boolean dsUmtsBearerCntxt_bringUp(void                 *info_p,
                                  ...
                                  boolean                    useDefaultDialStr)
{  
  ds3g_dial_val_e_type        dial_str_digits_type = DS3G_DIAL_MAX;
  ...
  if(! ds_3gpp_pdn_cntxt_get_subs_id_from_pdn(pdn_cntx_p,&subs_id))
  {
    ...
    MSG_SPRINTF_1(MSG_SSID_DS, MSG_LEGACY_HIGH, "dsBearerCntxt_bringUp: dial_str %s",
    ...
    if( pdn_cntx_p->ds_pdn_context_dyn_p->state == DS_PDN_CONTEXT_STATE_ANSWER) 
    {
      ...
      {
        DS_3GPP_MSG2_LOW("Could not Answer pkt call Mode: %d Bearer Inst:%d",\
             network_mode, call_info_p->call_instance);
        error_flg = TRUE;
      }
    }
    else
    {
      ...
      -------------------------------------------------------------------*/
      dial_str_digits_type = ds3g_proc_dial_str(dsatumts_pkt_orig_str,   
                                                dsatumts_pkt_orig_str,
                                                DS3G_ETSI_CMD_MODE);
      
      if ( !( ( dial_str_digits_type == DS3G_DIAL_DIGIT ) ||
              ( dial_str_digits_type == DS3G_DIAL_ASCII ) ) )  
      {  
        error_flg = TRUE;
        DS_3GPP_ERROR_LOG_1_EX(subs_id, "PHYS_LINK_UP_CMD_HDLR: ABORTING UMTS Pkt Call, INVALID dial string, call_instance:d",
      ...
      if(error_flg!= TRUE)
      {
        ...
        {
          DS_3GPP_MSG2_LOW("Could not Originate pkt call Mode: %d Bearer Inst:%d",\
                    network_mode, call_info_p->call_instance);
          error_flg = TRUE;
        }
      }
    ...
    if (error_flg)
    {
      ...
      DS_3GPP_ERROR_LOG_1_EX(subs_id, "Error bringing up Bearer Context for Call Inst d",
                call_info_p->call_instance);
...
/*===========================================================================
FUNCTION DSUMTSBEARERCNTXT_GET_UL_PACKET_METRICS

DESCRIPTION
  This function gets the UL packet metrics for UMTS bearers

PARAMETERS
  ds_bearer_context_s  - Bearer context pointer
  ps_iface_ioctl_per_bearer_ul_packet_delay_metrics_type - Struct to store
                                                           UL packet metrics
...
===========================================================================*/
boolean dsUmtsBearerCntxt_get_ul_packet_metrics
(
...
/*===========================================================================
FUNCTION DSUMTSBEARERCNTXT_RESET_UL_PACKET_METRICS

DESCRIPTION
  This function resets the UL packet metrics for UMTS bearers
...
===========================================================================*/
boolean dsUmtsBearerCntxt_reset_ul_packet_metrics
(
   ds_bearer_context_s             *bearer_context_ptr
)
{
   boolean                          ret_val = FALSE;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/  
  DS_UMTS_BEARER_CTXT_VFR_CALL_NO_ARG_PER_BEARER
  (
    ret_val,
...
for(subs_indx = DS3GSUBSMGR_SUBS_ID_MIN;
  ...
  DS_BEARER_CTXT_VF_REG(SYS_SYS_MODE_WCDMA, redial,\
                        dsUmtsBearerCntxt_redialHdlr);  
  DS_BEARER_CTXT_VF_REG(SYS_SYS_MODE_WCDMA, get_ul_packet_metrics, \
                        dsUmtsBearerCntxt_get_ul_packet_metrics);
  DS_BEARER_CTXT_VF_REG(SYS_SYS_MODE_WCDMA, reset_ul_packet_metrics, \
                        dsUmtsBearerCntxt_reset_ul_packet_metrics);
#ifdef FEATURE_DATA_WLAN_MAPCON
...
)
{
     ...
     (bearer_context_p->ds_bearer_context_dyn_p != NULL))
  {
    ...
    {
       DS_3GPP_MSG0_HIGH("Freeing Wcdma bearer inst");
...
)
{
  ...
  DS_3GPP_MSG3_MED("Updating Actual throughput for Bearer 0x%x bearer call "
                    "mode %d, sampling interval %d",bearer_context_p,
                    bearer_context_p->ds_bearer_context_dyn_p->call_mode,
                    sampling_interval);
  ...
  DS_3GPP_MSG3_HIGH("Prev UL Bytes Cnt transf %lu WM's byte cnt %lu "
                   "Calculated Avg actual t'put %lu kbps",
                   prev_tx_wm_cnt,
                   current_tx_wm_cnt,
                   bearer_context_p->ds_bearer_context_dyn_p->
                          bearer_throughput_stats.actual_ul_rate);
...
boolean dsUmtsBearerCntxt_allocate_umts_bearer(int index)
{
  ...
  if (NULL != umts_bearer_context_tbl[loop_index].ds_umts_bearer_context_dyn_p)
  {
   ...
   DS_3GPP_MSG1_HIGH("Allocated UMTS bearer at index %d", loop_index);
...
===========================================================================*/
void dsUmtsBearerCntxt_examine_if_throt_needed
(
...
)
{
  ...
  do
  {
    if((call_info_ptr == NULL) || (umts_call_info_ptr == NULL) || 
       (rab_throt_info_ptr == NULL))
    {
      break;    
    }
  ...
  --------------------------------------------------------------------------*/
    ...
    DS_3GPP_MSG2_HIGH("Rej cause: %d. Back off needed/note %d for the cause",
...
)
{
  ...
  DS_3GPP_MSG2_HIGH("data rate: %d : allowed max data rate :%d",
                   data_rate,max_bit_rate);
...
)
{
  ...
  ppp_config_info_p->rx_signal      = bearer_context_p->
                                            ds_bearer_context_dyn_p->rx_sig;
...
)
{
  ...
  DS_3GPP_MSG2_MED("Traffic class of bearer is %d system_rat %d",
                   traffic_class, system_rat);
...
)
{
  ...
  if (!bearer_context_p->ds_bearer_context_dyn_p->is_transferable_to_GW)
  {
    ...
    DS_3GPP_ERROR_LOG_0_EX(subs_id, "is_transferable_to_GW is false NAS will handle this case.");
    return;  
  }
#ifdef FEATURE_DATA_LTE
  ...
  {      
     DS_3GPP_ERROR_LOG_1_EX(subs_id, "QOS -- L-> GW continuity is not supported, ending call id:d",
                              bearer_context_p->ds_bearer_context_dyn_p->cm_call_id);
