...
$Header: //components/rel/data.mpss/3.5..2.1/3gpp/umtsps/src/ds_gprs_bearer_context.c#3 $
...
/*===========================================================================
FUNCTION DS_GPRS_BEARER_CONTXT_REG_WM_CB

DESCRIPTION
  This function is called when the GSNDCP will invoke registration callback 
...
===========================================================================*/
void ds_gprs_bearer_cntxt_reg_wm_cb
(
  void    *callback_data,
  boolean  cnf_status
);
...
/*===========================================================================
FUNCTION DS_GPRS_BEARER_CONTXT_DEREG_WM_CB

DESCRIPTION
  This function is called when the GSNDCP will invoke deregistration callback 
  after deregistration process
...
===========================================================================*/
void ds_gprs_bearer_cntxt_dereg_wm_cb
(
  void    *callback_data,
  boolean  cnf_status
);
...
)
{
  ...
  {
    DS_3GPP_MSG2_HIGH("Bearer ps handle is not valid: "
                      "original handle %d, current hanlde %d. "
...
SIDE EFFECTS
  None.
===========================================================================*/
uint64 ds_gprs_bearer_cntxt_get_so_mask_for_egprs_support
(
...
)
{
  ...
  {
    /*-----------------------------------------------------------------------
      No free command buffers available, return.
    -----------------------------------------------------------------------*/
    return;
  }
  ...
  sndcp_cmd->cmd.gsndcp_reg_wm.reg_wm_func_ptr = ds_gprs_bearer_cntxt_reg_wm_cb;
  sndcp_cmd->cmd.gsndcp_reg_wm.reg_wm_func_parm = (void*) bearer_context_p;
  sndcp_cmd->cmd.gsndcp_reg_wm.dereg_wm_func_ptr = ds_gprs_bearer_cntxt_dereg_wm_cb;
  sndcp_cmd->cmd.gsndcp_reg_wm.dereg_wm_func_parm = (void*) bearer_context_p;
...

LOCAL void ds_gprs_bearer_cntxt_register_wm
(
  ds_bearer_context_s  *bearer_cntxt_p
)
{
  ds_gprs_bearer_cntxt_register_wm_with_sndcp(&(bearer_cntxt_p->rx_wm.wm_item),
  	                                          NULL,
  	                                          &(bearer_cntxt_p->tx_wm.wm_item),
  	                                          bearer_cntxt_p);

  /* Call WM reg_cnf immediately as there is no cnf mechanism from SNDCP
        If NW Init QoS is supported on GPRS, we can modify this */
  ds_umts_tlb_wm_reg_cnf(bearer_cntxt_p->ds_bearer_context_dyn_p->nsapi,
...
)
{
  ...
  sndcp_cmd->cmd.gsndcp_dereg_wm.reg_wm_func_ptr = ds_gprs_bearer_cntxt_reg_wm_cb;
  sndcp_cmd->cmd.gsndcp_dereg_wm.reg_wm_func_parm = (void*) bearer_context_p;
  sndcp_cmd->cmd.gsndcp_dereg_wm.dereg_wm_func_ptr = ds_gprs_bearer_cntxt_dereg_wm_cb;
  sndcp_cmd->cmd.gsndcp_dereg_wm.dereg_wm_func_parm = (void*) bearer_context_p;
...
{
  DS_3GPP_MSG0_LOW("no functionality here");
  return;
}  /*  ds_gprs_bearer_cntxt_reg_wm_cb()  */
...
{
  ds_cmd_type  *cmd_ptr = NULL;                 /* DS Task message */
  ds_bearer_context_s *bearer_context_p = (ds_bearer_context_s*)callback_data;
  ds_gprs_gsndcp_reg_cnf_ev_type *reg_cnf_ev_ptr = NULL;

  DS_3GPP_MSG0_HIGH("ds_gprs_bearer_cntxt_dereg_wm_cb");
  ...
  cmd_ptr->hdr.cmd_id = DS_CMD_3GPP_GSNDCP_DEREG_WM_CNF;
  ...
  return;
}  /*  ds_gprs_bearer_cntxt_dereg_wm_cb()  */
...
DESCRIPTION
  ...
  This function processes the GSNDCP invoked deregistration callback 
  after deregistration process
...
===========================================================================*/
void ds_gprs_bearer_cntxt_process_dereg_wm_hdlr
(
  ds_bearer_context_s *bearer_context_p,
  boolean            cnf_status
)
{
  ...
  DS_3GPP_MSG0_HIGH("ds_gprs_bearer_cntxt_process_dereg_wm_hdlr");
  if(!ds_bearer_cntx_validate_bearer_context(bearer_context_p))
  {
    return;
  }

  if(cnf_status == TRUE && bearer_context_p->ds_bearer_context_dyn_p
  ...
  else if(cnf_status == FALSE)
  {
     DS_3GPP_MSG0_HIGH("dereg wm failed");
    /*     To DO -> Need to check what should be done       */
  }

}  /*  ds_gprs_bearer_cntxt_process_dereg_wm_hdlr()  */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/  
  ...
  {
    DS_GPRS_MSG1_HIGH("Release GPRS Radio Layer,set bearer Inst:0x%x to UP_DORMANT",
             bearer_context_p);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/  
  ...
  if(bearer_context_p->ds_bearer_context_dyn_p->is_ho2GPRS_in_progress == FALSE)
  {
    DS_GPRS_MSG1_HIGH("Suspend GPRS Radio Layer , Bearer Inst:0x%x to UP DORMANT",
                       bearer_context_p);
...
)
{
  ...
  {
    DS_GPRS_MSG1_HIGH(" Resume Reg req called in wrong state %d",
                        bearer_context_p->ds_bearer_context_dyn_p->state);
    return;
  }
  ...
  if ( cnf_status == TRUE )
  {
    ...
    {
      DS_GPRS_MSG0_HIGH("DS_3GPP_DATA_SHARK_ENABLE NV enabled. "
    ...
    /*---------------------------------------------------------------------  
      Send the active ind here as in case of GSM, there is no RAB-REESTAB
    --------------------------------------------------------------------*/  
  ...
  else
  {
    ...
    -----------------------------------------------------------------------*/
    while((item_ptr =
             (dsm_item_type*)dsm_dequeue(&(bearer_context_p->rx_wm.wm_item)))
             !=  NULL)
    {
    ...
    /*---------------------------------------------------------------------
      ...
      clean up the call.
    -----------------------------------------------------------------------*/    
    phys_link_p = &(bearer_context_p->ds_bearer_context_dyn_p->phys_link);
    ...
    {
      if (bearer_context_p->ds_bearer_context_dyn_p->is_default) // clang_sa_ignore [core.NullDereference]
      {
        /*-------------------------------------------------------------------
...
/*--------------------------------------------------------------------------*/
  ...
  DS_GPRS_MSG2_HIGH("Resuming connection to GPRS Bearer Inst:%d :PDP Type:%d ",
              (uint32)bearer_context_p->ds_bearer_context_dyn_p->client_data_ptr,
                      pdp_type);
...
{  
  ...
  if((ds3gpp_get_ps_data_suspend_flag(subs_id) == TRUE) ||
     ...
     (bearer_context_p->ds_bearer_context_dyn_p->state != 
      DS_BEARER_CONTEXT_STATE_UP_DORMANT_REESTAB)))
  {
    DS_GPRS_MSG1_HIGH(" Reg req called in wrong state %d",
                        bearer_context_p->ds_bearer_context_dyn_p->state);
    return;
  }
  ...
  if ( cnf_status == TRUE )
  {
    ...
    {
      DS_3GPP_ERROR_LOG_0_EX(subs_id, "Bearer activation failed for GPRS, "
                          "Freeing resources");
      ret_val = ps_phys_link_down_cmd(&(bearer_context_p->ds_bearer_context_dyn_p->
                                      phys_link), &ps_errno,
                                      NULL);
      if((ret_val == 0) || (ps_errno != DS_EWOULDBLOCK))
      {
...
/*--------------------------------------------------------------------------*/
  ...
  pdp_type = ((ds_pdn_context_s*)(bearer_context_p->ds_bearer_context_dyn_p->
                                  pdn_context_ptr))->
    ds_pdn_context_dyn_p->pdp_profile.context.pdp_type;
  DS_GPRS_MSG2_HIGH("GORS activate CNF, PDP Type: %d Bearer Inst:%d",
             pdp_type, (uint32)bearer_context_p->ds_bearer_context_dyn_p->
            client_data_ptr);
...
)
{
  ...
  DS_3GPP_MSG3_HIGH("GERAN LL reports Current tx rate %d bps current rx %d "
                    "bps fore bearer inst 0x%x in GPRS",
                    geran_data_rate.current_tx_bearer_data_rate,
...
{   
  ...
  DS_UMTS_BEARER_CTXT_VF_REG(SYS_SYS_MODE_GSM, register_wm, \
  	                         ds_gprs_bearer_cntxt_register_wm);
}
...
)
{
  ...
  switch(cmd_ptr->hdr.cmd_id)
  {
    ...
    case DS_CMD_3GPP_GSNDCP_DEREG_WM_CNF:
      ds_gprs_bearer_cntxt_process_dereg_wm_hdlr(bearer_context_p,
                                                         cnf_status);
      break;
...
}  /* ds_gprs_bearer_cntxt_process_gsndcp_ev_cmd() */
...
)
{
  ...
  switch (egprs_support)
  {
    ...
    case SYS_EGPRS_SUPPORT_AVAIL:
      DS_GPRS_MSG0_LOW("egprs support available");
