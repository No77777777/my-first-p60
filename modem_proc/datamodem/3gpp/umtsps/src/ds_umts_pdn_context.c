...
$Header: //components/rel/data.mpss/3.5..2.1/3gpp/umtsps/src/ds_umts_pdn_context.c#3 $
...
PARAMETERS  
  *orig_params_p   - Ptr to  Call origination parameters
  ...
  protocol config  - Protocol configuration container information
...
)
{
  ...
  {
    DS_3GPP_MSG0_HIGH("Invalid input parameters in dsUmtsPdnCntx_verifyPdpType");
    return FALSE;
  }
  ...
  rel12_plus_support = ds_3gppi_is_rel_12_or_above_supported(subs_id);
  ...
  {
    if(!ds_3gpp_ip_fallback_update_pdn_cntxt_info(pdn_cntx_p, subs_id))
    {            
      return FALSE;
    }
  ...
  {
    DS_3GPP_MSG2_HIGH("Received ESM cause %d for PDP type %d",
    ...
    switch(pdp_type_ctx_req)
    {
      ...
      case DS_UMTS_PDP_IPV4:
        if(mode_info_ptr->info.gw_ps_call.cause.ext_cause == 
          ...
          ds_3gpp_network_override_hdlr_set_overridden_ip(
             ...
             mode_info_ptr->info.gw_ps_call.reattempt_indicator,
             FALSE,
             FALSE,
             subs_id);
      	}
      ...
      case DS_UMTS_PDP_IPV4V6:
        ...
        {
          DS_3GPP_MSG1_LOW("Unhandled cause code %d",
                     mode_info_ptr->info.gw_ps_call.cause.ext_cause);
          break;
        }
  ...
  if ((pdn_cntx_p->ds_pdn_context_dyn_p->keep_alive_pdn == FALSE) &&
      (ds_3gpp_pdn_cntxt_accept_nw_provided_ip_type (pdn_cntx_p, ip_support)
       == FALSE))
  {
  ...
  /*------------------------------------------------------------------------
    If IP fallback is active and if there is a cached iface in the IP
    fallback DB, then -      
    - Client requested both v4 and v6 call, but due to IP fallback, we
      requested IPV6 only over the air.      
    Send call failure on the cached iface as network accepted the PDN
    connection on the iface we requested over the air.
  ------------------------------------------------------------------------*/
  if (ds_3gpp_ip_fallback_is_ip_fallback_active(subs_id))
  {
    DS_3GPP_MSG0_HIGH("Clear cached Client call");
    ds_3gpp_ip_fallback_clear_cached_client_call(pdn_cntx_p, 
  ...
  if(pdp_type_org == 0)
  {
    if( pdp_type_num ==  0x01)
    {
      ...
      {
        DS_3GPP_MSG0_HIGH("Received unexpected pdp type PPP.");
        return DS3G_FAILURE;
      }
    }
    else
  ...
  else if(pdp_type_org == 0x01)
  {
        ...
        ( pdp_type_num !=  DS_3GPP_PDP_TYPE_IPV4V6_NUM) )
    {
      if(ip_support != DS_IPV4_ONLY &&
         ip_support != DS_IPV4_SINGLE_BEARER)
      {
        DS_3GPP_MSG2_HIGH("IPV4 ret when req was diff. Req: %d, Ret: %d. Rej call",
                pdn_cntx_p->ds_pdn_context_dyn_p->pdp_profile.context.pdp_type, 
                           ip_support);
        return DS3G_FAILURE;
      }
    ...
    else if( pdp_type_num ==  DS_3GPP_PDP_TYPE_IPV6_NUM)   
    {
      if(ip_support != DS_IPV6_ONLY &&
         ip_support != DS_IPV6_SINGLE_BEARER)
      {
        DS_3GPP_MSG2_HIGH("IPV6 ret when req was diff. Req: %d, Ret: %d. Rej call",
                pdn_cntx_p->ds_pdn_context_dyn_p->pdp_profile.context.pdp_type, 
                           ip_support);
        DS_3GPP_MSG0_HIGH("Asserting Now...");
        DS3GPP_ASSERT(0);
        return DS3G_FAILURE;
      }
    ...
    else if( pdp_type_num ==  DS_3GPP_PDP_TYPE_IPV4V6_NUM)   
    {
      ...
      {
        DS_3GPP_MSG2_HIGH("IPV4V6 ret when req was diff. Req: %d, Ret: %d. Rej call",
                pdn_cntx_p->ds_pdn_context_dyn_p->pdp_profile.context.pdp_type, 
                           ip_support);
        return DS3G_FAILURE;
      }
  ...
  if( pdp_type_ctx_req == DS_UMTS_PDP_PPP &&
      pdp_type_ctx_accept != DS_UMTS_PDP_PPP ) 
  {
    DS_3GPP_ERROR_LOG_0_EX(subs_id, "Requested PPP call but NW did not return PPP.");
    return DS3G_FAILURE;
  }
  ...
  {
    DS_3GPP_MSG0_HIGH("Unable to update APN table, cannot set up the bearer");
    return DS3G_FAILURE;
  }

  DS_3GPP_MSG0_HIGH("Return SUCCESS");
  return DS3G_SUCCESS; 
...
/*--------------------------------------------------------------------------*/
  ...
  {
    DS_3GPP_ERROR_LOG_0_EX(subs_id, "Invalid inpout in dsUmtsPdnCntx_activateInd");
    return FALSE;
  }
  ...
  /*lint +e731 */
  {
    ...
    -----------------------------------------------------------------------*/
     DS_3GPP_MSG0_HIGH("PDP type mismatch");
  ...
  ds_3gpp_pco_epco_hdlr_get_config_from_umts
    (umts_call_info_p,
     &ds_proto_config_options,
     &(pdn_cntx_p->ds_pdn_context_dyn_p->epco_enabled));
  /*-------------------------------------------------------------------------
  ...
  {
    DS_3GPP_MSG0_LOW("Invalid flow_manager_ptr ptr");
  }
  ...
  {
    DS_3GPP_MSG0_LOW("Initilizing BCM_info as UE only");
    flow_manager_ptr->ds_flow_manager_dyn_p->bcm_info = DS_3GPP_BCM_UE_ONLY;
  }
  ...
  if( (umts_call_info_p->pdp_addr.pdp_type_org & 0x0F) == 0x01    )
  {
        ...
        ( umts_call_info_p->pdp_addr.pdp_type_num !=  DS_3GPP_PDP_TYPE_IPV4V6_NUM) )
    {
      ...
      if(ds_3gpp_pdp_addr.pdp_addr.pdp_addr_ipv4 == 0)
      {
        ...
        ----------------------------------------------------------------------*/
        DS_3GPP_MSG0_HIGH(" NULL IP address provided by NW!");      
        return DS3G_FAILURE;
      }  
    ...
    else if( umts_call_info_p->pdp_addr.pdp_type_num ==  DS_3GPP_PDP_TYPE_IPV6_NUM)   
    {
      ...
      if( ds_3gpp_pdp_addr.pdp_addr.pdp_addr_ipv6.in6_u.u6_addr64[DS_3GPP_IPV6_IID_OFFSET] == 0)
      {
        /*---------------------------------------------------------------------
          No DHCP/static IP support, so reject call_connected
        ----------------------------------------------------------------------*/
        DS_3GPP_MSG0_HIGH(" NULL IID provided by NW!");      
        return DS3G_FAILURE;
      } 
  ...
  {
    DS_3GPP_MSG0_HIGH("Setting up UMTS PDP PPP call");
...
)
{
  ...
  {
    DS_3GPP_MSG0_LOW("NULL arguments to report TDS throughput quality");
    return ret_val;
  }
  ...
  DS_3GPP_MSG3_HIGH("TDS LL reports throughput quality %d UL allowed 0x%x%x kbps",
                    ul_throughput_quality->ul_throughput_quality,
                    QWORD_HIGH(ul_throughput_quality->allowed_ul_rate),
                    QWORD_LOW(ul_throughput_quality->allowed_ul_rate));
  ret_val = TRUE;
#else
  DS_3GPP_ERROR_LOG_0_EX(cm_subs_id, "TDS mode not supported");
  ret_val = FALSE;
#endif
  return ret_val;
...
PARAMETERS
    ...
    *pdp_profile_parms        - Profile parameters
     ...
     subs_id                  - Subscription Id
    *ds_proto_config_option_p - Ptr to protocol config 
...
)
{
  ...
  /*-------------------------------------------------------------------------
    ...
    valid.
  -------------------------------------------------------------------------*/
  if (profile_p == NULL || ds_proto_config_option_p == NULL)
    return DS3G_FAILURE;

  if(ppp_info_p == NULL)
  {
    DS_3GPP_MSG0_HIGH("Fill PPP params from Profile");
    return dsumtsps_pdp_ip_fill_ppp_params_from_profile
             (orig_params_p, profile_p, subs_id,ds_proto_config_option_p);
  }
  /*-------------------------------------------------------------------------
  ...
  {
    DS_3GPP_MSG1_HIGH("Inst:%d Auth type %d from Info",
    ...
    (void) ds_pdn_cntxt_fill_auth_params_from_info 
             (ds_proto_config_option_p, profile_p, ppp_info_p, subs_id);
  }
  else
  {
    /*-------------------------------------------------------------------------
    ...
    (void) ds_pdn_cntxt_fill_auth_params_from_profile
             (ds_proto_config_option_p, profile_p, subs_id);
  }
  ...
  ---------------------------------------------------------------------------*/
    ...
    if ( rel_version < (uint8)DS_3GPP_REL8 )
    {
      DS_3GPP_MSG0_LOW("IPV4 DNS container not filled, return FALSE"); 
      result = FALSE;
    }
    else
    {
  ...
  if ( (profile_p->context.pdp_type != DS_UMTS_PDP_IPV6 ) && (!static_ip_specified) )
  {
    ...
    if ( rel_version < (uint8)DS_3GPP_REL8 )
    {
      DS_3GPP_ERROR_LOG_0_EX(subs_id, "IP Addr alloc field is not populated"); 
    }
    else
    {
...
)
{
  ...
  if(ul_throughput_quality == NULL)
  {
    DS_3GPP_MSG0_LOW("NULL Arguments to report GPRS UL throughput quality");
    return ret_val;
  }
  if (!ds3gsubsmgr_is_subs_id_valid(cm_subs_id))
  {
  ...
  DS_3GPP_MSG3_HIGH("GERAN LL reports throughput quality %d UL allowed 0x%x%x kbps",
                    ul_throughput_quality->ul_throughput_quality,
                    QWORD_HIGH(ul_throughput_quality->allowed_ul_rate),
                    QWORD_LOW(ul_throughput_quality->allowed_ul_rate));
  ret_val = TRUE;
...
)
{
  ...
  if(ul_throughput_quality == NULL)
  {
    DS_3GPP_MSG0_LOW("Invalid argument to report WCDMA UL t'put quality");
    return ret_val;
  }
  ...
  else
  {
    ...
    DS_3GPP_MSG3_HIGH("No WCDMA EUL stats. Report throughput Quality %d UL allowed"
                      " tput 0x%lx%lx kbps",
                      ul_throughput_quality->ul_throughput_quality,
                      QWORD_HIGH(ul_throughput_quality->allowed_ul_rate),
                      QWORD_LOW(ul_throughput_quality->allowed_ul_rate));
...
)
{
  ...
  if ( NULL == ds_proto_config_option_p )
  {
    return DS3G_FAILURE;
  }
  ...
  DS_3GPP_MSG1_HIGH(" Auth type %d",profile_p->auth.auth_type);
  ...
  if(!result) {
    DS_3GPP_ERROR_LOG_0_EX(subs_id, "Auth Info generation failed ");
  }
  ...
  */
  ...
  (profile_p->context.pdp_type != DS_UMTS_PDP_IPV4V6)
    ...
    {
     DS_3GPP_ERROR_LOG_1_EX(subs_id, "Iface has invalid IP type d",profile_p->context.pdp_type);
     return DS3G_FAILURE;
    }
  ...
  {
    DS_3GPP_MSG2_HIGH("IPCP DNS options P:%lx S:%lx",
              profile_p->dns_addr.primary_dns_addr.pdp_addr.pdp_addr_ipv4,
              profile_p->dns_addr.secondary_dns_addr.pdp_addr.pdp_addr_ipv4);
    ...
    if ( rel_version < (uint8)DS_3GPP_REL8 )
    {
      DS_3GPP_MSG0_LOW("IPV4 DNS container not filled"); 
      result = FALSE;
    }
    else
    {
          ...
          (profile_p->context.pdp_type == DS_UMTS_PDP_IPV4V6))
  {
    ...
    if (result == FALSE)
    {
      /*---------------------------------------------------------------------
        ...
        PDP ctxt would also fail (using the same params)
      ---------------------------------------------------------------------*/
      DS_3GPP_MSG0_LOW("IPV6 DNS Genaration Failed");
    }
  }
#endif /* FEATURE_DATA_PS_IPV6 */
  ...
  if ((profile_p->context.pdp_type != DS_UMTS_PDP_IPV6 ) && (!static_ip_specified) )
  {
    ...
    if ( rel_version < (uint8)DS_3GPP_REL8 )
    {
      DS_3GPP_MSG0_LOW("IP Addr alloc field is not populated"); 
    }
    else
    {
      ds_pdn_cntxt_fill_address_alloc_param(ds_proto_config_option_p,
                                              profile_p);
    }
...
)
{
  ...
  ds_3gpp_pdn_throt_ntw_o_ridden_ip_type_e  override_ip = DS_3GPP_PDN_THROT_PDP_UNDEFINED;
  void                         *fill_op_pco_ptr= NULL;
  ds_umts_pdp_profile_type     *profile_p = NULL; 
  sys_plmn_id_s_type            current_plmn;
  ds_3gpp_protocol_cfg_options_s   ds_proto_config_option;
  uint16                           loop = 0;
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_3GPP_MSG2_HIGH("Originating Type PDP-IP %d for CM subs id %d",
                     pdp_type,subs_id);
  ...
  else if(pdp_type == DS_UMTS_PDP_IPV4V6)
  {
      ...
      override_ip = ds_3gpp_network_override_hdlr_get_overridden_ip
                    (profile_p->context.apn,DS_3GPP_THROT_RAT_UMTS, 
                      &current_plmn, subs_id);

      DS_3GPP_MSG1_LOW("Network_override_IP is %d", override_ip);

      switch(override_ip)
      {
         case DS_3GPP_PDN_THROT_PDP_IPV4:
         ...
         case DS_3GPP_PDN_THROT_PDP_IPV6:
         ...
         default:
           ...
           /* -----------------------------------------------------------------
              When IP fallback is configured make sure IPV6 is first requested
              over the air when PDP type is IPv4v6
           ------------------------------------------------------------------*/ 
           if (ds_3gpp_ip_fallback_is_ip_fallback_active(subs_id)) 
           {
             orig_params_p->pdp_addr.pdp_type_num  = 0x57;
             DS_3GPP_MSG0_HIGH("IP_Fallback: Request IPv6 OTA");
           }
           break;
      }/* Switch */
  ...
  #else
    DS_3GPP_MSG0_LOW("Dual IP is not supported at NAS level, failing the call");
    return DS3G_FAILURE;
      ...
      ((network_params_info_type *)(orig_info_p->info_ptr))->valid_fields )
  {
    ...
    DS_3GPP_MSG0_LOW("PPP Auth/IPCP info passed: Using it ");
  }
  ...
  {
    DS_3GPP_ERROR_LOG_0_EX(subs_id, "pdp_ip_fill_ppp_params returned failure");
    goto memfree;
  }
  ...
  if( ds_pdn_cntxt_gen_pcscf_address_request_packet(\
        &ds_proto_config_option, profile_p,subs_id) == DS3G_FAILURE)
  {
    goto memfree;
  }
  ...
  if(profile_p->im_cn_flag == TRUE)
  {
    ...
    else
    {
      if(ds_pdn_cntxt_gen_im_cn_subsystem_request_packet(
          &ds_proto_config_option, subs_id) == DS3G_FAILURE)
      {
        DS_3GPP_MSG0_HIGH("IM CN flag generate failed.");
        goto memfree;
      }
      DS_3GPP_MSG0_LOW("IM CN flag generate success");
    }
  }
#endif
  ...
  -------------------------------------------------------------------------*/
  ds_pdn_cntxt_fill_bcm_indication(&ds_proto_config_option, subs_id);


  if(profile_p->request_msisdn_flag)
  {
    /*------------------------------------------------------------------   -------
      Fill the MSISDN Container
    --------------------------------------------------------------------     -----*/
    ds_pdn_cntxt_fill_msisdn_support_container(&ds_proto_config_option, subs_id);
  }
  ...
  if(fill_op_pco_ptr != NULL)
  {
    ...
    DS_3GPP_MSG0_HIGH("FILL OP PCO contents for UMTS from modem app");

    (void)ds_pdn_cntxt_fill_oper_pco_info_from_app(
       &ds_proto_config_option,fill_op_pco_ptr,subs_id);
   
    pdn_cntx_p->ds_pdn_context_dyn_p->modem_pco_req = TRUE;
  ...
  ds_3gpp_pdn_cntx_fill_ps_data_off_ue_support_pco_container
      (subs_id, &ds_proto_config_option);
  ...
  /*-------------------------------------------------------------------------
    Convert the DS PCO to NAS format. We always used PCO when bringing up a
    new PDN
  --------------------------------------------------------------------------*/
  orig_params_p->config_options.valid = 
    ds_3gpp_pco_epco_hdlr_convert_ds_pco_to_nas
      (&ds_proto_config_option,&(orig_params_p->config_options));
  return DS3G_SUCCESS;
...
memfree:
  ...
  {
    modem_mem_free( ds_proto_config_option.protocol_info_p[loop],
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(call_info != NULL)
  {
    ...
    phys_link = &(bearer_cntx_p->ds_bearer_context_dyn_p->phys_link);
  ...
  {
    DS_3GPP_MSG1_HIGH("PDP profile :%d not defined, failing call orig",
                       profile_number);
    return DS3G_FAILURE;
  }
  ...
  DS_3GPP_MSG3_HIGH("ORIG_CALL_HDLR: Pri call activation, prf num [%d] of type %d",
         (byte)gw_ps_orig_params_p->profile_number, pdp_type, 0);
  ...
  DS_3GPP_MSG1_HIGH_EX(subs_id, "APN encoded Name:length=%d", apn_encoded_length);
  ...
  switch( profile_p->context.pdp_type )
  {
    ...
    case DS_UMTS_PDP_PPP:      
      ...
      break;
    default:
      DATA_MSG1_LOW("Invalid PDP type from profile type: %d",
                       profile_p->context.pdp_type );
      interm_result = DS3G_FAILURE ;
  }
...
{   
  ...
  DS_3GPP_MSG2_HIGH ("ANS_HDLR: Call act for MT req, profile num: %d PDP type %d",
             (byte)answer_params->ans_params.gw_ps_ans.assoc_pri_profile_num,
             mm_orig_params.orig_params_u.gw_ps_orig_params_ptr->pdp_addr.pdp_type_num);
...
{ 
  ...
  -------------------------------------------------------------------------*/  
  (void)ds_3gpp_cfg_get_subsid( DS_3GPP_AUTO_ANSWER_ENABLE, 
...
)
{
  ...
  if(pdn_cntx_p->ds_pdn_context_dyn_p->state == DS_PDN_CONTEXT_STATE_GOING_DOWN)
  {
    DS_3GPP_ERROR_LOG_0_EX(subs_id, "Got call connected while PDN going down - Rejecting");
  ...
  {
    DS_3GPP_ERROR_LOG_0_EX(subs_id, "Received bearer Activ Ind for primary PDP");
    return DS3G_FAILURE;
  }
  ...
  if(result == DS3G_FAILURE)
  {
    DS_3GPP_ERROR_LOG_0_EX(subs_id, "Error in accepting the passed flow params");
    /*-------------------------------------------------------------------
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_3GPP_ERROR_LOG_1("Dynamic memory already allocated at index:d",
                        loop_index);
  ...
  if (umts_pdn_context_dyn_p == NULL)
  {
    ...
    if (umts_pdn_context_dyn_p != NULL)
    {
      ...
      DS_3GPP_MSG2_MED("Umts PDN context dyn mem allocated at index:%d, result %d",
                        umts_pdn_context_dyn_p->index, (uint8)ret_val );
      ret_val = TRUE;
    }
  }
...
/*----------------------------------------------------------------------------- 
  ...
  DS_3GPP_MSG2_MED("Freeing dyn allocated PDN pointer at index %d result %d",
                    loop_index, (uint8) ret_val);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  else if (umts_pdn_cntxt_dyn_p->cookie == UMTS_PDN_CNTXT_VALID_COOKIE)
  {
    ret_val = TRUE;
  }
  DS_3GPP_MSG2_MED("Umts Pdn context 0x%x is_valid %d",
                   umts_pdn_cntxt_p,(uint8)ret_val);

  return ret_val;
}/* dsUmtsPdnCntx_validate_pdn_context */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_3GPP_MSG0_LOW("FAIL modem_mem_alloc for dsUmtsPdnCntx_configurePdpPpp");
    return -1;
  }
  ...
  ---------------------------------------------------------------------*/
  ppp_config_info->subs_id = ppp_get_subs_id( subs_id );
...
/*-------------------------------------------------------------------------*/
  ...
  if(!ds_3gpp_pdn_cntxt_get_subs_id_from_pdn(pdn_cntx_p, &subs_id) ||
     (ip_support == NULL))
  {
    DS_3GPP_MSG2_LOW(" Pass in NULL Pdn Cntxt pointer %d ,ip_support ptr"
  ...
  DS_3GPP_MSG4_HIGH("invalid cause code for PDP req %d,redial req %d N/w PDP type %d IP Support %d",
                               pdp_type_ctx_req,
                               is_redial_allowed,
                               pdp_type_num,
                               ip_support); 
  ...
  switch(pdp_type_ctx_req)
  {
    ...
    case DS_UMTS_PDP_IPV4V6:
      {
         switch (pdp_type_num)
         {
           ...
           {
             DS_3GPP_MSG0_HIGH("UE requested IPv4v6, N/w assigned IPV4V6");
           }
           break;
    ...
    DS_3GPP_MSG1_HIGH("DBG: result %d",result);
