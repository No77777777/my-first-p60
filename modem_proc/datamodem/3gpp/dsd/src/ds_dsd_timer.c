...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp/dsd/src/ds_dsd_timer.c#2 $
...
/*---------------------------------------------------------------------------
  ...
  If additional DSD NVs are added, ensure conf file is updated in ds3gcommonutils
---------------------------------------------------------------------------*/
...
#define DS_NR5GOOSHYST_TIMER_1_PATH "/nv/item_files/data/dsd/data_nr5g_oos_hyst_timer"
#define DS_WLANACQHYST_TIMER_1_PATH "/nv/item_files/data/dsd/data_wlan_acq_hyst_timer"
#define DS_NULLBEARERHYST_TIMER_1_PATH "/nv/item_files/data/dsd/data_null_bearer_hyst_timer"
...
---------------------------------------------------------------------------*/
#define DS_1X_ACQ_HYST_VAL         20  /* in sec */
...
#define DS_NR5G_OOS_HYST_VAL       15  /* in sec */
...
#define DS_RAT_EXP_HYST_VAL        22  /* in sec */ /* TODO: check value */
#define DS_NULL_BEARER_HYST_VAL    3   /* in sec */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_ERROR_LOG_0(MSG_LEGACY_ERROR, "DSD TIMER id passed is NULL");
    return NULL;
  }
  ...
  switch(dsd_timer_id)
  {
    ...
    case DS_RatExpHystTimerID_T13:
      dsd_timer_info = &(dsd_timer_list->rat_expansion_hyst_timer);
      break;

    case DS_NR5GOOSHystTimerID_T14:
      dsd_timer_info = &(dsd_timer_list->nr5g_oos_hyst_timer);
      break;

    case DS_NullBearerHystTimerID_T15:
      dsd_timer_info = &(dsd_timer_list->null_bearer_hyst_timer);
      break;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch(dsd_timer_id)
  {
    ...
    /* NR5G OOS hysteresis timer */
    case DS_NR5GOOSHystTimerID_T14:
      strlcpy(item_file_path,
	      DS_NR5GOOSHYST_TIMER_1_PATH,
	      sizeof(item_file_path));
      is_timer_valid = TRUE;
      break;
    ...
    case DS_NullBearerHystTimerID_T15:
      strlcpy(item_file_path,
              DS_NULLBEARERHYST_TIMER_1_PATH,
              sizeof(item_file_path));
      is_timer_valid = TRUE;
      break;
  ...
  DATA_MSG3_EX(MSG_LEGACY_HIGH,  subs_id, "ds_dsd_timer_get_nv_value: timer_id=%d,"
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /*----------------------------------------------------------------------
  DO NOT remove the below F3 as its used by test teams.
  ----------------------------------------------------------------------*/
  DATA_MSG3_EX(MSG_LEGACY_HIGH,  
               data_block_ptr->subs_id, 
               "DS DSD TIMER CB TIMER:%d TimerTick %d subs_id:%d",
               data_block_ptr->timer_id, 
               timetick_get_ms(), 
               data_block_ptr->subs_id);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_ERROR_LOG_3_EX(MSG_LEGACY_ERROR,
                      subs_id,
                      "DSD proc ptr: d,dsd timer info ptr: d "
                      "for subs_id:d",dsd_proc_ptr,
                      dsd_timer_info, subs_id);
    return FALSE;
  }

  /* Special handlings are done here */
  duration = (uint32)dsd_timer_info->value*1000;
  switch(dsd_timer_id)
  {
    case DS_HDRAcqHystTimerID_T3:
      if( ds_dsd_ext_get_dds_switch_type() == DDS_CAUSE_TEMPORARY)
      {
        duration = 0;
      }
      break;
  ...
  if(ret_val == TRUE)
  {
    ...
    DATA_MSG4_EX(MSG_LEGACY_HIGH, 
                 subs_id,
                 "Timer start. Timer ID: %d Duration: %d TimerTick %d ms subs_id:%d", 
                 dsd_timer_id, 
                 duration, 
                 timetick_get_ms(), 
                 subs_id);
  ...
  DATA_MSG4_EX(MSG_LEGACY_MED, 
               subs_id,
               "start Timer ID %d is running: %d , duration: %d for subs_id:%d ", 
               dsd_timer_id,
               ret_val,
               duration, 
               subs_id);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  dsd_timer_info = ds_dsd_timer_get_timer_info_ptr(dsd_timer_id, subs_id);
    /* Get DSD proc for subs id */
  dsd_proc_ptr = ds_dsd_proc_get_dsd_proc_instance(subs_id);
  if(dsd_timer_info == NULL || dsd_proc_ptr == NULL)
  {
    return FALSE;
  }

  duration = (uint32)dsd_timer_info->value*1000;
  if (dsd_timer_id == DS_LTEOOSHystTimerID_T6)
  {
    duration += (dsd_proc_ptr->timer_list.epc_data_cntx_timer.value *
                dsd_proc_ptr->epc_present_flag)*1000;
  }
  ...
  DATA_MSG4_EX(MSG_LEGACY_MED, 
               subs_id,
               "stop Timer ID %d is stoppped: %d ,timer value: %d for subs_id:%d", 
               dsd_timer_id,
               ret_val,
...
#ifdef FEATURE_LAPP
    case DS_DSD_RADIO_5G:
      timer_id = DS_NR5GOOSHystTimerID_T14;
      break;
#endif /* FEATURE_LAPP */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if( (cmd_ptr == NULL) || (cmd_ptr->cmd_payload_ptr == NULL) )
  {
    DS_ERROR_LOG_0(MSG_LEGACY_ERROR, "NULL dsd_timer_id_container.");
    return;
  }
  ...
  switch((unsigned long)dsd_timer_id)
  {
    ...
    default:
      DS_ERROR_LOG_1_EX(MSG_LEGACY_ERROR, subs_id,"Invalid parameter : 0xx", dsd_timer_id);
      break;
  }
       ...
       timer_indx++ )
  {
      ...
      )
    {
      DS_ERROR_LOG_2_EX(MSG_LEGACY_ERROR, subs_id, 
                "timer_id:d for subs_id:d not initialized",
                timer_indx, subs_id);
    }
  }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  dsd_timer_list->rat_expansion_hyst_timer.value = DS_RAT_EXP_HYST_VAL;
  dsd_timer_list->nr5g_oos_hyst_timer.value = DS_NR5G_OOS_HYST_VAL;
  dsd_timer_list->null_bearer_hyst_timer.value = DS_NULL_BEARER_HYST_VAL;
       ...
       timer_indx++ )
  {
    ...
    if((timer_indx == DS_EPCDataCntxTimerID_T8) ||
       (timer_indx == DS_HDRHandDownTimerID_T9) ||
       (timer_indx == DS_HDRHandUpAvoidTimerID_T10) ||
       (timer_indx == DS_HDRNwDetachHystTimerID_T11) ||
       (timer_indx == DS_LTENwDetachHystTimerID_T12) ||
       (timer_indx == DS_RatExpHystTimerID_T13) ||
       (timer_indx == DS_NR5GOOSHystTimerID_T14))
    {
      continue;
    }
  ...
  if( ds_dsd_timer_get_edct_nv_value(subs_id, &timer_val) )
  {
    DATA_MSG2_EX(MSG_LEGACY_HIGH, 
                 subs_id,
                 "Setting timer_id %d to %d",
                 DS_EPCDataCntxTimerID_T8, 
                 timer_val);

    dsd_timer_list->epc_data_cntx_timer.value = timer_val;
  }

  DATA_MSG9_EX(MSG_LEGACY_MED, 
               subs_id,
               "DSD Timer Values "
               "1x Acq Hyst Timer = %d "
               "1x OOS Hyst Timer = %d "
  ...
  DATA_MSG2(MSG_LEGACY_MED,
            "DSD Timer Values "
            "NR5G OOS Hyst Timer = %d "
            "Null Bearer Hyst Timer = %d ",
            dsd_timer_list->nr5g_oos_hyst_timer.value,
            dsd_timer_list->null_bearer_hyst_timer.value);
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  DATA_MSG2_EX(MSG_LEGACY_HIGH, 
               subs_id, 
               "ds_dsd_timer_epc_notify, epc present: %d, subs id %d",
               epc_present,
               subs_id);
  ...
  {
    DS_ERROR_LOG_4_EX(MSG_LEGACY_ERROR,subs_id,
                      "dsd_proc_ptr ptr: d,lte_oos_timer_info ptr: d"
     ...
     (edct_timer_val > 0))
  {
    ...
    else
    {
      ...
      {
        DATA_MSG0_EX(MSG_LEGACY_HIGH, subs_id, "Stopping LTE OOS HYST timer for EDCT");
      }
    }
  }
