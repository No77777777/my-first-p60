...
$Header: //components/rel/data.mpss/3.5..2.1/3gpp/lteps/inc/ds_eps_pdn_context.h#5 $
...
/*--------------------------------------------------------------------------- 
    ML1 can only have up to 7 active scells (index 1 to 7. 0 is the pscell).
    These are the macro definitions for the range.
  ---------------------------------------------------------------------------*/
#define DS_EPS_SCELL_INDEX_MIN 1
#define DS_EPS_SCELL_INDEX_MAX 7
...
extern ds_eps_downlink_throughput_estimation_s 
                          ds_eps_dl_tput_est_tbl[DS_SUBSCRIPTION_MAX];
...
SIDE EFFECTS
  None.
===========================================================================*/
boolean ds_eps_pdn_cntxt_is_attach_pdn_up
(
  sys_modem_as_id_e_type    subs_id
);
...
===========================================================================*/
ds_eps_bringup_attach_pdn_result_e ds_eps_pdn_cntxt_bring_up_attach_pdn
(
  sys_modem_as_id_e_type    subs_id
);
...
/*===========================================================================
FUNCTION      DS_EPS_PDN_CNTXT_GET_LTE_ATTACH_PARAMS_EX

DESCRIPTION
  Updates the LTE attach extended parameters if available.
  LTE attach parameters are available when
  UE is attached on LTE because of the following conditions
  Fresh LTE attach occurs (Not IRAT from 3g) i.e
  PDN is UP on the attach profile
 
  Returns Failure if
  1. Input parameters are invalid.
  2. Attach PDN context is not UP
    a. Can be because the attach PDN is torn down (
    b. IRAT from 3g results in attach PDN never being brought up
...
SIDE EFFECTS
  None.
===========================================================================*/
int ds_eps_pdn_cntxt_get_lte_attach_params_ex
(
  void                    *arg_val_ptr,
  sys_modem_as_id_e_type   subs_id,
  int16                   *ps_errno
...
/*===========================================================================
FUNCTION   DS_EPS_PDN_CNTXT_FILL_LTE_ATTACH_PARAMS_IP_ADDR
 
DESCRIPTION 
  Updates the LTE attach extended parameters with V4/V6 address 
...
SIDE EFFECTS
  None.
===========================================================================*/
void ds_eps_pdn_cntxt_fill_lte_attach_params_ip_addr
(
   ps_sys_ioctl_3gpp_lte_attach_params_ex_type    *attach_params_ptr,
   ds_pdn_context_s                               *pdn_cntx_p,
   ds_umts_pdp_type_enum_type                      pdp_type
);

/*===========================================================================
FUNCTION DS_EPS_PDN_CNTX_SEND_LTE_ATTACH_PARAM_EX_IND

DESCRIPTION
  This function is used to send an indication to PS indicating LTE attach
...
SIDE EFFECTS
  None.
===========================================================================*/
void ds_eps_pdn_cntx_send_lte_attach_param_ex_ind
(
  sys_modem_as_id_e_type subs_id,
  boolean                is_valid
);
...
SIDE EFFECTS
  None.
===========================================================================*/
boolean ds_eps_pdn_cntx_attach_complete_ind_hdlr
(
...
/*===========================================================================
FUNCTION DS_EPS_PDN_CNTXT_GET_NUM_OF_ACTIVE_SCELLS

DESCRIPTION
  This function returns the number of active scells on a given subscription
...
SIDE EFFECTS
  None.
===========================================================================*/ 
uint8 ds_eps_pdn_cntxt_get_num_of_active_scells
(
  sys_modem_as_id_e_type subs_id
);

/*===========================================================================
FUNCTION DS_EPS_PDN_CNTXT_GET_NUM_OF_UL_ACTIVE_SCELLS

DESCRIPTION
  This function returns the number of active UL capable scells on a given
...
RETURN VALUE
  Number of active UL capable scells.  

SIDE EFFECTS
  None.
===========================================================================*/ 
uint8 ds_eps_pdn_cntxt_get_num_of_ul_active_scells
(
  sys_modem_as_id_e_type subs_id
);

/*===========================================================================
FUNCTION DS_EPS_PDN_CNTXT_CLEAR_ACTIVE_SCELLS

DESCRIPTION
  This function clears the lte active scell mask for carrier aggregation
...
SIDE EFFECTS
  None.
===========================================================================*/
void ds_eps_pdn_cntxt_clear_active_scells
(
  ds3gsubsmgr_subs_id_e_type    ds3g_subs_id
);

/*===========================================================================
FUNCTION DS_EPS_PDN_CNTXT_HANDLE_LPM_MODE

DESCRIPTION
  This function resets cached sequence number when LPM mode is triggered 
  such that new PDN connectivity request wouldn't be sent when current PDN 
  is brought down.
...
SIDE EFFECTS
  None.
===========================================================================*/ 
void ds_eps_pdn_cntxt_handle_lpm_mode(void);
...
/*===========================================================================
FUNCTION  DS_EPS_PDN_CNTXT_RESET_RESOLVED_APN

DESCRIPTION
  This function resets the resolved apn stored in ads3geventmgr_conf_tech_3gpp.
...
===========================================================================*/
void ds_eps_pdn_cntxt_reset_resolved_apn
(
  sys_modem_as_id_e_type   subs_id
);
...
/*===========================================================================
FUNCTION DS_EPS_PDN_CNTXT_SEND_DETACH_REQ

DESCRIPTION
  This function ensures CM detaches on LTE and CM sends DS CALL_END request

PARAMETERS

DEPENDENCIES
  This function does not check for any last standing PDN
...
SIDE EFFECTS
  None.
============================================================================*/
void ds_eps_pdn_cntxt_send_detach_req
(
  sys_modem_as_id_e_type   subs_id
);

/*===========================================================================
FUNCTION DS_EPS_PDN_CNTX_TAU_COMPLETE_IND

DESCRIPTION
  This function is used to handle the message from NAS. This message is sent
  when NAS gets the TAU complete message.
...
SIDE EFFECTS
  None.
===========================================================================*/
boolean ds_eps_pdn_cntx_tau_complete_ind_hdlr
(
  msgr_umid_type              msgrtype,
  sys_modem_as_id_e_type      subs_id,
  const msgr_hdr_struct_type *dsmsg_ptr
...
/*===========================================================================
FUNCTION DS_EPS_APN_PARAM_CHANGE_TEARDOWN_CALL

DESCRIPTION
  This function tears down the call in apn para change scenario and 
  gracefully handle it if call is already down
...
SIDE EFFECTS
  None.
===========================================================================*/
void ds_eps_apn_param_change_teardown_call
(
   ds_bearer_context_s *bearer_context_p
);
