...
Notice that changes are listed in reverse chronological order.
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/3gpp/lteps/src/ds_eps_pdn_context.c#16 $ $DateTime: 2021/10/11 00:30:03 $ $Author: pwbldsvc $
...
#define DS_EPS_T3402_DEFAULT_VALUE 720000
...
/*===========================================================================
FUNCTION DS_EPS_PDN_CNTXT_CPHY_CA_EVENT_EXT_HDLR

DESCRIPTION
  This function is used to handle the LTE_CPHY_CA_EVENT_EXT_IND message from LTE
  ML1. MH will send this info to ps_sys module.
...
SIDE EFFECTS
  None.
===========================================================================*/
static boolean ds_eps_pdn_cntxt_cphy_ca_event_ext_hdlr
(
  msgr_umid_type             msgr_type,
  sys_modem_as_id_e_type     subs_id,
  const msgr_hdr_struct_type *dsmsg_ptr
...
----------------------------------------------------------------------------*/
static uint32 ds_eps_t3402_value[DS3GSUBSMGR_SUBS_ID_MAX] = {DS_EPS_T3402_DEFAULT_VALUE};
...
/*---------------------------------------------------------------------------
  Caches the number of active scells as a bit mask. Each bit tells if an
  scell is active at that index.
---------------------------------------------------------------------------*/
static uint32 ds_eps_lte_active_scell_idx_mask[DS3GSUBSMGR_SUBS_ID_MAX] = {0};

/*---------------------------------------------------------------------------
  Caches the number of active scells that is UL capable as a bit mask.
  Each bit tells if an scell is UL capable at that index.
---------------------------------------------------------------------------*/
static uint32 ds_eps_lte_ul_active_scell_idx_mask[DS3GSUBSMGR_SUBS_ID_MAX] = {0};
...
)
{
  ...
  {
    DS_LTE_MSG1_ERROR_EX(subs_id, "ds_eps_get_t3402_timer_val: input error subs_id d",subs_id);
    return 0;
  }
  return ds_eps_t3402_value[subs_id];
}
...
===========================================================================*/
void ds_eps_pdn_cntxt_set_t3402_timer_val
(
  sys_modem_as_id_e_type subs_id,
  uint32 new_t3402_val
)
{
  if (!ds3gsubsmgr_is_subs_id_valid(subs_id))
  {
    DS_LTE_MSG1_ERROR_EX(subs_id, "ds_eps_set_t3402_timer_val: input error subs_id d",subs_id);
    return;
  }
  else
  {
    DS_LTE_MSG2_HIGH_EX(subs_id,"ds_eps_set_t3402_timer_val: Updating subs_id %d T3402 to %d",
                     subs_id,new_t3402_val);
    ds_eps_t3402_value[subs_id] = new_t3402_val;
  }
}
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  DS_LTE_MSG2_MED_EX(subs_id,
                  "Fallback enabled: %d, Attach FB ip type : %d",
                  ds_eps_lte_attach_fb_ip_type[subs_id].fallback_enabled,
...
{
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  DS_3GPP_MSG2_MED("Setting Attach FB ip type activation status :%d, "
                   "IP FB type: $d", activation_status, attach_fb_ip_type);
...
/*-------------------------------------------------------------------------*/
  do
  {
    ...
    {
      attach_prof = ds_3gpp_apn_switch_hdlr_get_user_profile_id(subs_id);
      if (attach_prof != APN_SWITCH_INVALID_PROFILE_ID)
      {
        DS_LTE_MSG1_HIGH_EX(subs_id, 
                            "Profile Id used for attach is %d", 
                            attach_prof);
        return attach_prof;
      }
    }
    ...
    {
      DS_LTE_MSG2_ERROR_EX(subs_id, "Unable to get the attach profile from APM, "
                        "return_code d, attach_apn_avail d",
                        dsd_result,apm_payload.attach_apn_avail);
      if (dsd_result == DS_DSD_APM_ALL_ATTACH_APN_BLOCKED)
      {
        //DS_LTE_MSG0_ERROR_EX(subs_id, "All LTE attach APNs are blocked");
      ...
      {
        //DS_LTE_MSG0_ERROR_EX(subs_id, "APM blocked LTE attach");
  ...
  DS_LTE_MSG2_HIGH_EX(subs_id,
                     "Profile Id used for attach is %d, Local error: %d", 
                      attach_prof, local_err);
  return attach_prof;
}
/*===========================================================================
...
)
{
  ...
  DS_LTE_MSG1_HIGH_EX(subs_id,"In ds_eps_pdn_ip_fill_ppp_params_from_profile: "
  ...
  *auth_info_being_sent = FALSE;
  ...
  {
     DS_LTE_MSG1_LOW("Iface has invalid IP type %d",profile_p->context.pdp_type);
     return DS3G_FAILURE;
  }
  ...
  if((profile_p->context.pdp_type == DS_UMTS_PDP_IPV4) ||
     (profile_p->context.pdp_type == DS_UMTS_PDP_IPV4V6) )
  {
    DS_LTE_MSG2_HIGH_EX(subs_id,"IPCP DNS options P:%lx S:%lx",
...
)
{
  ...
  DS_LTE_MSG0_HIGH_EX(subs_id,"In ds_eps_pdn_ip_fill_ppp_params_from_info:");
  ...
  if(ppp_info_p == NULL)
  {
    return ds_eps_pdn_ip_fill_ppp_params_from_profile
            (orig_params_p, profile_p, auth_info_being_sent, 
             subs_id,ds_proto_config_option_p);
  }
  /*-------------------------------------------------------------------------
   Fill in Protocol specific options : for IP, this would contain the PPP
   pkts (lcp, ipcp ) The purpose of the protocol configuration options
  ...
  *auth_info_being_sent = FALSE;
  ...
  {
    DS_LTE_MSG1_HIGH_EX(subs_id,"Auth type %d from Info",ppp_info_p->auth_info.auth_type);
  ...
  /*-------------------------------------------------------------------------
    Request Primary and Secondary DNS addresses from the network.
  -------------------------------------------------------------------------*/
  DS_LTE_MSG0_LOW("IPCP DNS options specified");
  ...
  ---------------------------------------------------------------------------*/
    ...
    else
    {
      ...
      DS_LTE_MSG2_HIGH_EX(subs_id,"IPCP DNS options from profile P:%lx S:%lx",
                profile_p->dns_addr.primary_dns_addr.pdp_addr.pdp_addr_ipv4,
                       profile_p->dns_addr.secondary_dns_addr.pdp_addr.pdp_addr_ipv4);
      result = ds_pdn_cntxt_fill_ipcp_orig_params
             (
...
)
{
  ...
  do 
  {
    ...
    {
      DS_LTE_MSG0_LOW("EPS Orig params is NULL");
      break;
    }
    ...
    DS_LTE_MSG2_MED_EX(subs_id,"Nw Overridden IP is %d, Profile PDP Type: %d",
                    ntw_overridden_ip, profile_p->context.pdp_type);
    ...
    -----------------------------------------------------------------------*/
    if (ntw_overridden_ip == DS_3GPP_PDN_THROT_PDP_IPV6)
    {
      if (((profile_p->context.pdp_type == DS_UMTS_PDP_IPV6) ||
           (profile_p->context.pdp_type == DS_UMTS_PDP_IPV4V6)) &&
          (ds3g_is_ipv6_enabled()))
      {
        DS_LTE_MSG0_LOW("Updating the outgoing PDN request to IPV6");
    ...
    else if (ntw_overridden_ip == DS_3GPP_PDN_THROT_PDP_IPV4)
    {
      if((profile_p->context.pdp_type == DS_UMTS_PDP_IPV4) ||
           (profile_p->context.pdp_type == DS_UMTS_PDP_IPV4V6))
      {
        DS_LTE_MSG0_LOW("Updating the outgoing PDN request to IPV4");
...
)
{
  ...
  ds_3gpp_pdn_throt_ntw_o_ridden_ip_type_e  ntw_overridden_ip = DS_3GPP_PDN_THROT_PDP_UNDEFINED;
  sys_modem_as_id_e_type           subs_id=SYS_MODEM_AS_ID_NONE;
  void                            *fill_op_pco_ptr = NULL;
  uint16                           collision_pco_id = FALSE;
 ...
 /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (eps_orig_params_p == NULL)
  {
    DS_3GPP_ERROR_FATAL("Input ptr in NULL");
    return FALSE;
  }
  ...
  if ((ntw_overridden_ip = ds_3gpp_network_override_hdlr_get_overridden_ip(
      (byte *)profile_p->context.apn,DS_3GPP_THROT_RAT_LTE, &current_plmn, subs_id)) 
  ...
  else
  {
    ...
    else if (profile_p->context.pdp_type == DS_UMTS_PDP_IPV4V6)
    {
      ...
      {
        eps_orig_params_p->pdn_type = NAS_ESM_IPV4;
        DS_LTE_MSG0_HIGH_EX(subs_id,"Updating the IP type to IPV4 only since IPV6 "
                         "NV is disabled");
        (void)ds_pdn_cntxt_update_apn_table(pdn_cntx_p->ds_pdn_context_dyn_p
                        ->pdp_profile.context.apn,DS_IPV4_ONLY, pdn_cntx_p);
    ...
    {
      DS_LTE_MSG1_LOW("PDP type not supported. PDP type: %d",profile_p->context.pdp_type);
      return FALSE;
    }
  }
  ...
  /*-------------------------------------------------------------------------
    Fill in the PCO
  -------------------------------------------------------------------------*/
  memset(&(ds_proto_config_option),
         0, sizeof(ds_3gpp_protocol_cfg_options_s));
  /*-------------------------------------------------------------------------
    valid flag will be set by the pco subfunctions if they get called
  -------------------------------------------------------------------------*/
  ds_proto_config_option.valid = FALSE;
  ds_proto_config_option.config_protocol = 0;
  ds_proto_config_option.num_protocol_ids = 0;
     ...
     (pdn_cntx_p->ds_pdn_context_dyn_p->info.valid_fields)))
  {
    ...
    DS_LTE_MSG0_HIGH_EX(subs_id,"Redial Scenario, Using Cached Info from PDN ");
  }
  ...
  if(fill_op_pco_ptr != NULL)
  {
    ...
    DS_3GPP_ERROR_LOG_0_EX(subs_id, "FILL OP PCO contents for LTE from modem app");
    ...
    collision_pco_id = ds_pdn_cntxt_check_op_pco_collision(profile_p,
                                                           fill_op_pco_ptr,
                                                           subs_id);
  }
  ...
  /*-------------------------------------------------------------------------
    ...
    for the same PCO ID.
    We can only have one PCO ID. This would mean configuration is incorrect
    ------------------------------------------------------------------------*/
  if(ds_pdn_cntxt_fill_oper_pco_info_from_profile(
       &ds_proto_config_option, profile_p,collision_pco_id, subs_id) 
       == DS3G_FAILURE)
  {
    /*-------------------------------------------------------------------
      If an error occurs in encoding pkt, do not include the pkt but
      ...
      PDP ctxt would also fail (using the same params)
    ------------------------------------------------------------------*/
    DS_LTE_MSG0_HIGH_EX(subs_id,"Operator reserved Pco packet generation failed");
...
//
        ...
        subs_id) == DS3G_FAILURE )
  {
    /*-------------------------------------------------------------------
      ...
      PDP ctxt would also fail (using the same params)
    ------------------------------------------------------------------*/
    DS_LTE_MSG0_HIGH_EX(subs_id,"PCSCF Address request packet generation failed");
  }
  ...
  if(profile_p->im_cn_flag == TRUE)
  {
    ...
    {
      DS_LTE_MSG0_ERROR_EX(subs_id, "Call with IM CN Flag already up");
      goto memfree;
    }
    else
    {
      ...
      {
        DS_LTE_MSG0_HIGH_EX(subs_id,"IM CN flag generate failed.");
      }
  ...
  {
    ds_pdn_cntxt_fill_msisdn_support_container(
                               &ds_proto_config_option,subs_id);
  }
  ...
  ds_pdn_cntxt_fill_ipv4v6_link_mtu_container
    (&ds_proto_config_option, profile_p, subs_id);
  ...
  DS_LTE_MSG1_HIGH_EX(subs_id, "APN Name:length=%d", apn_encoded_length);
  ...
  /*-------------------------------------------------------------------------
    ...
    new PDN
  --------------------------------------------------------------------------*/
  eps_orig_params_p->protocol_config_options.valid = 
    ds_3gpp_pco_epco_hdlr_convert_ds_pco_to_nas
      (&ds_proto_config_option,&(eps_orig_params_p->protocol_config_options));
...
)
{
  ...
  if(ds_3gpp_ip_fallback_is_ip_fallback_active(subs_id))
  {
    if(!ds_3gpp_ip_fallback_update_pdn_cntxt_info(rejected_pdn_cntx_p, subs_id))
    {      
      return;
    }
  ...
  if(mode_info_p == NULL)
  {
    DS_LTE_MSG0_ERROR_EX(subs_id, "Pdn Rej Ind: In Params Null. APN table could be wrong");
  }
  else
  {
    if(mode_info_p->info.lte_call.esm_cause.valid)
    {
      ...
      (void)ds_eps_bearer_cntxt_map_cause_codes(
        ...
        &ps_down_reason, (const void*)mode_info_p, subs_id
      );
    }
    else if (mode_info_p->info.lte_call.emm_failure_cause.cause_type !=
            LTE_NAS_IRAT_NONE)
    {
      (void)ds_eps_bearer_cntxt_map_cause_codes(
    ...
    {
      (void)ds_eps_bearer_cntxt_map_cause_codes(
      CALL_END_ESM_CAUSE_TO_PS_IFACE_CAUSE_CODES, &ps_down_reason,
      (const void*)mode_info_p, subs_id);
    }

    backoff_timer = mode_info_p->info.lte_call.backoff_timer;
    reattempt_indicator = mode_info_p->info.lte_call.reattempt_indicator;
   
   /*-------------------------------------------------------------------- 
     Fetch the parsed info
     -------------------------------------------------------------------*/
    ds_pdn_throt_spec_parse_backoff_info
      (backoff_timer, reattempt_indicator, subs_id, ps_down_reason,
       &parsed_info, SYS_SYS_MODE_LTE);

   /*-------------------------------------------------------------------- 
     Fill in the throttling rule information
     -------------------------------------------------------------------*/
    throt_rule_info.rejected_pdn_cntx_p = rejected_pdn_cntx_p;
    throt_rule_info.mode_info_p = mode_info_p;
    throt_rule_info.sys_mode = SYS_SYS_MODE_LTE;
    throt_rule_info.down_reason = ps_down_reason;
    ...
    /*-------------------------------------------------------------------------
      ...
      2. UE is rejected with a fallback cause code that is configured in the
         XML other than CC #50 (For CC 50, we want apply spec throttling)
    -----------------------------------------------------------------------*/        
    if (ds_3gpp_ip_fallback_is_ip_fallback_active(subs_id) && 
        (ps_down_reason != PS_NET_DOWN_REASON_IP_V4_ONLY_ALLOWED) &&
              ...
              throt_rule_info.down_reason, subs_id))
    {
      ds_3gpp_network_override_hdlr_set_overridden_ip(
            ...
            ds_3gpp_pdn_throt_sm_sys_mode_to_rat_mask(throt_rule_info.sys_mode),
            reattempt_indicator,
            TRUE,
            FALSE,
            subs_id);
...

{
  ...
  {
    DS_LTE_MSG2_ERROR_EX(subs_id, "Default bearer does not exist: lbi=d bearer_id=d",
  ...
  {
    DS_LTE_MSG0_HIGH_EX(subs_id,"Got Activate Bearer Indication while PDN going down - Rejecting");
  ...
  if(def_bearer_cntx_p !=(ds_bearer_context_s *)pdn_cntx_p->ds_pdn_context_dyn_p
     ->def_bearer_context_ptr)
  {
    DS_LTE_MSG2_ERROR_EX(subs_id, "Invalid Linked bearer ID passed lbi:d, bearer_id: d",
                      lte_info_ptr->lbi, lte_info_ptr->eps_bearer_id);
    DS_EPS_SET_ESM_CAUSE(lte_info_ptr, NAS_ESM_UNKNOWN_EPS_BEARER_CONTEXT);
    return DS3G_FAILURE;
  }
  ...
  {
    DS_LTE_MSG2_ERROR_EX(subs_id, "Dedicated bearer already exists:lbi=d bearer_id=d",
                      lte_info_ptr->lbi, lte_info_ptr->eps_bearer_id);
  ...
  -------------------------------------------------------------------------*/
  pdn_cntx_awaiting_bearer_alloc_p = ds_3gpp_get_pdn_cntxt_awaiting_bearer_allocation();
  ...
  if (ds_pdn_cntxt_store_bearer_context(pdn_cntx_p,(void *)bearer_cntx_p) != DS3G_SUCCESS)
    {
      DS_LTE_MSG0_HIGH_EX(subs_id,"Could not store the allocated bearer");
    ...
    if(result == DS3G_FAILURE)
    {
      ...
      ret_val = ps_phys_link_up_cmd(&bearer_cntx_p->ds_bearer_context_dyn_p->phys_link,
                           &ds_errno, NULL);

      if (ret_val == 0)
      {
        DS_LTE_MSG0_ERROR_EX(subs_id, " Bringing up phys link when already up Inst:");
      }
      ...
      -------------------------------------------------------------------------*/
      DS3GPP_ASSERT(cm_act_res_p != NULL);
...
)
{
  ...
  boolean                            tear_down_if_unused = FALSE;
  boolean                            start_unused_pdn_timer = FALSE;
  ...
  if ( mode_info_p == NULL )
  {
    DS_3GPP_ERROR_FATAL("mode_info_p=NULL in pdnCntxBearerActivatedInd");
    return DS3G_FAILURE;
  }
  ...
  else if (lte_info_ptr->lbi == ESM_INVALID_BEARER_ID)
  {
    ...
    -------------------------------------------------------------------------*/
    DS_LTE_MSG0_HIGH_EX(subs_id,"Bearer context non allocated while default bearer setup");
    return DS3G_FAILURE;
  }
  ...
  {
    DS_LTE_MSG0_ERROR_EX(subs_id, " PDN address is Invalid, valid flag not set");
    result = DS3G_FAILURE;
  ...
  ----------------------------------------------------------------------*/
  if (DS_3GPP_PDN_SUPPORTS_EMERGENCY_SERVICES(pdn_cntx_p)) // clang_sa_ignore [core.NullDereference]
  {
    ...
    else
    {
      ip_support = DS_SINGLE_BEARER;
                                                                  ...
                                                                  DS3G_SUCCESS)
  {
    DS_LTE_MSG1_ERROR_EX(subs_id, "Could not get IP Support info. PDN: 0xx",
                      pdn_cntx_p);
    result = DS3G_FAILURE;
    lte_info_ptr->esm_cause.valid = TRUE;
    lte_info_ptr->esm_cause.esm_cause = NAS_ESM_INVALID_MANDATORY_INFORMATION;
  ...
  DS_LTE_MSG2_HIGH_EX(subs_id,"Profile pdp_type: %d,initializing ip_support to:%d",
  ...
  DS_LTE_MSG3_HIGH_EX(subs_id,"IP Support: %d, ESM Cause Valid: %d, ESM Cause: %d",
                    ip_support, lte_info_ptr->esm_cause.valid, 
     ...
     (lte_info_ptr->esm_cause.valid != TRUE))
  {
    ...
    switch (ip_support)
    {
      ...
      default:
        DS_LTE_MSG1_LOW("Invalid ip_support: %d for invalid ESM cause",
                           ip_support);
        break;
    }
    ...
    if (PS_IFACE_IS_VALID(other_ps_iface_p))
    {
      ...
      ds_3gpp_pdn_cntx_validate_assert(pdn_cntx_p); 
    }
    else
    {
    ...
    DS_LTE_MSG1_LOW("Esm cause not set for IPV4V6 call,hence update IP support"
  ...
  {
    DS_LTE_MSG1_LOW("Processing esm_cause: %d",
                     lte_info_ptr->esm_cause.esm_cause);
    /*-------------------------------------------------------------------------------
    ...
    (void)ds_eps_bearer_cntxt_map_cause_codes(CALL_END_ESM_CAUSE_TO_PS_IFACE_CAUSE_CODES,
      &ps_down_reason,(const void*)mode_info_ptr_i,subs_id);
    ...
    {
      DS_LTE_MSG1_ERROR_EX(subs_id, "ds_eps_pdn_cntxt_bearer_activate_ind: Invalid ESM cause d",
                        lte_info_ptr->esm_cause.esm_cause);
      result = DS3G_FAILURE;
      lte_info_ptr->esm_cause.valid = TRUE;
    ...
    else switch(pdn_cntx_p->ds_pdn_context_dyn_p->pdp_profile.context.pdp_type)
    {
      case DS_UMTS_PDP_IPV4:
        ...
        {
          DS_LTE_MSG0_HIGH_EX(subs_id,"Received ESM cause IPV6 only while profile req is V4");
          ip_support = DS_IPV6_ONLY;
          result = DS3G_FAILURE;
          lte_info_ptr->esm_cause.valid = TRUE;
          lte_info_ptr->esm_cause.esm_cause = NAS_ESM_ACTIVATION_REJECTED;
          ...
          ds_3gpp_network_override_hdlr_set_overridden_ip(
              ...
              lte_info_ptr->reattempt_indicator,
              FALSE,
              FALSE,
              subs_id);
        }
        ...
        {
          DS_LTE_MSG0_HIGH_EX(subs_id,"Handling Single addr bearer only while profile req is V4");
          ...
          {
            DS_LTE_MSG0_ERROR_EX(subs_id, "Received V6 addr only while profile req is V4");
          ...
          {
            DS_LTE_MSG1_HIGH_EX(subs_id,"Incorrect PDN address type: %d",
                             lte_info_ptr->pdn_addr.pdn_type_val);
            result = DS3G_FAILURE;
            lte_info_ptr->esm_cause.valid = TRUE;
      ...
      case DS_UMTS_PDP_IPV6:
        ...
        {
          DS_LTE_MSG0_HIGH_EX(subs_id,"Received ESM cause IPV4 only while profile req is V6");
          ip_support = DS_IPV4_ONLY;
          result = DS3G_FAILURE;
          lte_info_ptr->esm_cause.valid = TRUE;
          lte_info_ptr->esm_cause.esm_cause = NAS_ESM_ACTIVATION_REJECTED;
          ...
          ds_3gpp_network_override_hdlr_set_overridden_ip(
           ...
           NAS_ESM_SINGLE_ADR_BEARERS_ONLY_ALLOWED)
        {
          DS_LTE_MSG0_HIGH_EX(subs_id,"Handling Single addr bearer only while profile req is V6");
          if(lte_info_ptr->pdn_addr.pdn_type_val == NAS_ESM_IPV4)
          {
            DS_LTE_MSG0_ERROR_EX(subs_id, "Received V4 addr only while profile req is V6");
            ip_support = DS_IPV4_SINGLE_BEARER;
            result = DS3G_FAILURE;
            lte_info_ptr->esm_cause.valid = TRUE;
            lte_info_ptr->esm_cause.esm_cause = NAS_ESM_ACTIVATION_REJECTED;
          ...
          {
            DS_LTE_MSG1_HIGH_EX(subs_id,"Incorrect PDN address type: %d",lte_info_ptr->pdn_addr.pdn_type_val);
            result = DS3G_FAILURE;
            lte_info_ptr->esm_cause.valid = TRUE;
            lte_info_ptr->esm_cause.esm_cause = NAS_ESM_UNKNOWN_PDN_TYPE;
            break;
      ...
      case DS_UMTS_PDP_IPV4V6:
        if(lte_info_ptr->esm_cause.esm_cause == \
           NAS_ESM_PDN_TYPE_IPV4_ONLY_ALLOWED)
        {
          DS_LTE_MSG0_HIGH_EX(subs_id,"Received ESM cause IPV4 only while profile req is V4V6");
          ip_support = DS_IPV4_ONLY;
          other_ps_iface_p = DSPDNCNTXT_GET_V6_PS_IFACE_PTR(pdn_cntx_p);
          /*----------------------------------------------------------------------
         ...
         if(ds_3gpp_throt_sm_is_cc_sp_throttling_enabled(subs_id) != TRUE)
         {
            throt_ident_info.pdp_type = DS_UMTS_PDP_IPV6;
            throt_ident_info.sm_type = DS_3GPP_PDN_THROT_TYPE_SPEC;
          ...
          ds_3gpp_network_override_hdlr_set_overridden_ip(
           ...
           NAS_ESM_PDN_TYPE_IPV6_ONLY_ALLOWED)
        {
          DS_LTE_MSG0_HIGH_EX(subs_id,"Received ESM cause IPV6 only while profile req is V4V6");
          ip_support = DS_IPV6_ONLY;
          other_ps_iface_p = DSPDNCNTXT_GET_V4_PS_IFACE_PTR(pdn_cntx_p);
          /*----------------------------------------------------------------------
         ...
         if(ds_3gpp_throt_sm_is_cc_sp_throttling_enabled(subs_id) != TRUE)
         {
           throt_ident_info.pdp_type = DS_UMTS_PDP_IPV4;
           throt_ident_info.sm_type = DS_3GPP_PDN_THROT_TYPE_SPEC;
           ds_3gpp_pdn_throt_perform_reg_throttling (&throt_ident_info);
          }
          ...
          ds_3gpp_network_override_hdlr_set_overridden_ip(
           ...
           NAS_ESM_SINGLE_ADR_BEARERS_ONLY_ALLOWED)
        {
          DS_LTE_MSG0_HIGH_EX(subs_id,"Received ESM cause Single addr bearer only while profile req is V4V6");
          if(lte_info_ptr->pdn_addr.pdn_type_val == NAS_ESM_IPV4)
          {
            ip_support = DS_IPV4_SINGLE_BEARER;
            DS_3GPP_APM_SET_INVALID_V6_IPADDR(ind_payload, PS_NET_DOWN_REASON_SINGLE_ADDR_BEARER_ONLY);
        ...
        {
          DS_LTE_MSG0_HIGH_EX(subs_id,"other_ps_iface is not valid, not dissociating");
        }
        break;
      default:
        DS_LTE_MSG0_HIGH_EX(subs_id,"Unknown PDP type.");
          result = DS3G_FAILURE;
          lte_info_ptr->esm_cause.valid = TRUE;
          lte_info_ptr->esm_cause.esm_cause = NAS_ESM_UNKNOWN_PDN_TYPE;
    }
  ...
  if(result == DS3G_FAILURE)
  {
    DS_LTE_MSG0_HIGH_EX(subs_id,"Cannot proceed with bearer activation");
    return result;
  }

  /*--------------------------------------------------------------------

    If Network granted IP Type does not match the IP Type requested by
    the application, UE can reject the Bearer Activation from the Network.

    However, there are certain exceptions cases in which UE does not
    reject the bearer activation.
    1. If UE is not attached, it will accept any IP support that the
    network returns for a particular APN
    2. If this is an emergency call and the Network provided IPv4 only
       or IPv6 only, DS will start a hysteresis timer to give 
       application a chance to latch on to the Bearer granted by the 
       network. If application latches on to that Bearer, all is 
       good. However if the hysteresis timer expires and the bearer is
       still not being used by any client, we will tear down the PDN
       connection. We have this special logic for emergency calls, 
       so that the call can get connected faster, without additional 
       signalling.
  --------------------------------------------------------------------*/
  if (DS_3GPP_PDN_SUPPORTS_EMERGENCY_SERVICES(pdn_cntx_p))
  {
    tear_down_if_unused = TRUE;

    if (ip_support == DS_IPV4_ONLY ||
        ip_support == DS_IPV6_ONLY)
    {
      start_unused_pdn_timer = TRUE;
    }
  }
  else
  {
  ...
  if(tear_down_if_unused)
  {
    ...
    if (ds_3gpp_pdn_cntxt_accept_nw_provided_ip_type (pdn_cntx_p, ip_support)
         == FALSE)
    {
      DS_LTE_MSG1_ERROR_EX(subs_id, "Mismatch between NW and UE Ip types. "
                        "start_unused_pdn_timer: d",start_unused_pdn_timer );

      if (start_unused_pdn_timer)
      {
        rex_set_timer(&(pdn_cntx_p->ds_pdn_context_dyn_p->unused_pdn_timer), 
                      DS_3GPP_UNUSED_PDN_TIMER_VAL);
      }
      else
      {
        lte_info_ptr->esm_cause.valid = TRUE;
  ...
  {
    DS_LTE_MSG0_ERROR_EX(subs_id, "apn name validation failed and decoding.");
    result = DS3G_FAILURE;
    return result;
  }

  DATA_LTE_MSG_SPRINTF_2_EX(MSG_LEGACY_HIGH,
                            subs_id,
                            "APN Name in Prof is :%s , "
                            "Resolved APN name: %s", 
                            pdn_cntx_p->ds_pdn_context_dyn_p->pdp_profile.context.apn, 
                            decoded_apn);
  ...
  if(ds_3gpp_pdn_limit_is_enabled(subs_id))
  {
                                            ...
                                            decoded_apn))
    {
      ...
      -------------------------------------------------------------------------*/
      if(strlen((char *)decoded_apn) > 0 &&
         strlen((char*)pdn_cntx_p->ds_pdn_context_dyn_p->pdp_profile.context.apn) == 0)
      {
        ...
        {
          DS_3GPP_ERROR_LOG_1_EX(subs_id, "Set PS_SYS_CONF_NULL_RESOLVED_APN failed.Err_no:d"
                             ,ps_errno);
        }
        ...
        {
          DS_3GPP_ERROR_LOG_0_EX(subs_id, "Unable to provide Resolved APN info to internal"
                             " clients");
        }
      }
    ...
    {
      DS_LTE_MSG0_ERROR_EX(subs_id, "Error updating resolved name in APN table");
    }
  ...
  if(strlen((char*)pdn_cntx_p->ds_pdn_context_dyn_p->pdp_profile.context.apn) == 0)
  {
    ...
    DATA_LTE_MSG_SPRINTF_1_EX(MSG_LEGACY_HIGH,
                              subs_id,
                              "APN Name in Prof is NULL: , Resolved APN name: %s", 
                              decoded_apn);
  ...
  -------------------------------------------------------------------------*/
  DS_LTE_MSG1_HIGH_EX(subs_id,"Processing granted pdn addr, type: %d",
                   lte_info_ptr->pdn_addr.pdn_type_val);
  ...
  if(lte_info_ptr->pdn_addr.pdn_type_val == NAS_ESM_IPV4)
  {
    ...
    if(ip_support != DS_IPV4_ONLY &&
       ip_support != DS_IPV4_SINGLE_BEARER)
    {
      DS_LTE_MSG2_ERROR_EX(subs_id, "IPV4 ret when req was diff. Req: d, Ret: d. Rej call",
                        pdn_cntx_p->ds_pdn_context_dyn_p->pdp_profile.context.pdp_type,
                        ip_support);
      return DS3G_FAILURE;
    }
    ...
    if(ds_3gpp_pdp_addr.pdp_addr.pdp_addr_ipv4 == 0)
    {
      /*---------------------------------------------------------------------
        ...
        DHCP to be used.
      ----------------------------------------------------------------------*/
      DS_LTE_MSG0_HIGH_EX(subs_id,"NULL V4 IP address provided by NW!");
      if (profile_ptr->context.ipv4_addr_alloc != DS_UMTS_PDP_IPV4_ADDR_ALLOC_DHCPV4)
      {
        DS_LTE_MSG0_ERROR_EX(subs_id, "NULL V4 IP address but DHCPV4 is not supported!");
  ...
  else if(lte_info_ptr->pdn_addr.pdn_type_val == NAS_ESM_IPV6)
  {
    ...
    if(ip_support != DS_IPV6_ONLY &&
       ip_support != DS_IPV6_SINGLE_BEARER)
    {
      DS_LTE_MSG2_ERROR_EX(subs_id, "IPV6 ret when req was diff. Req: d, Ret: d. Rej call",
                pdn_cntx_p->ds_pdn_context_dyn_p->pdp_profile.context.pdp_type,
                        ip_support);
      return DS3G_FAILURE;
    }
       ...
       u6_addr64[DS_3GPP_IPV6_IID_OFFSET] == 0)
    {
      ...
      ----------------------------------------------------------------------*/
      DS_LTE_MSG0_ERROR_EX(subs_id, " NULL V6 IID address provided by NW!");
  ...
  else if(lte_info_ptr->pdn_addr.pdn_type_val == NAS_ESM_IPV4V6)
  {
    ...
    if(ip_support != DS_IPV4V6)
    {
      DS_LTE_MSG1_ERROR_EX(subs_id, "Wrong profile config: IPV4V6 ret when profile req was: d",
                         pdn_cntx_p->ds_pdn_context_dyn_p->
      ...
      {
        DS_LTE_MSG2_ERROR_EX(subs_id, "IPV4V6 ret when req was diff. Req: d, Ret: d. Rej call",
                          pdn_cntx_p->ds_pdn_context_dyn_p->
  ...
  {
    DS_LTE_MSG0_ERROR_EX(subs_id, " PDN address type is Invalid");
    lte_info_ptr->esm_cause.valid = TRUE;
    lte_info_ptr->esm_cause.esm_cause = NAS_ESM_UNKNOWN_PDN_TYPE;
    return DS3G_FAILURE;
  }
  ...
  {
    DS_LTE_MSG0_ERROR_EX(subs_id, "Unable to update APN table, cannot set up the bearer");
    lte_info_ptr->esm_cause.valid = TRUE;
  ...
  DS_LTE_MSG1_HIGH_EX(subs_id,"Storing pdp group id: %d.",lte_info_ptr->pdp_group_id);
  ...
  if (lte_info_ptr->sdf_qos.valid == TRUE)
  {
    ...
    DS_LTE_MSG0_HIGH_EX(subs_id,"Storing sdf qos info for default bearer.");
    bflow_manager_p=ds_bearer_flow_manager_find_flow_manager(bearer_cntx_p);

    if(bflow_manager_p == NULL)
    {
      DS_3GPP_ERROR_FATAL("Bearer associated with a bearer flow manager");
      lte_info_ptr->esm_cause.valid = TRUE;
      lte_info_ptr->esm_cause.esm_cause =NAS_ESM_INSUFFICIENT_RESOURCES;
      return DS3G_FAILURE;
    }
                                                     ...
                                                     &icause_code) != DS3G_SUCCESS)
    {
      if(ds_eps_flow_manager_convert_cause_code(mode_info_ptr_i,&icause_code)
                                                               ==FALSE)
      {
        DS_LTE_MSG0_ERROR_EX(subs_id, "Cause code conversion failed");
      }
      return DS3G_FAILURE;
    }
  ...
  /*--------------------------------------------------------------------------
    If APN AMBR or extended APN AMBR is passed in default bearer activation,
    store it in the flow manager.
  ---------------------------------------------------------------------------*/
  if ( lte_info_ptr->apn_ambr.valid == TRUE
#ifdef FEATURE_LAPP
      || (lte_info_ptr->extended_apn_ambr.valid == TRUE)
#endif 
      )
  {
    DS_LTE_MSG0_HIGH_EX(subs_id,"Storing apn ambr for default bearer.");
    ...
    {
      DS_LTE_MSG0_ERROR_EX(subs_id, "eps_bearer_context_p is NULL");
      lte_info_ptr->esm_cause.valid = TRUE;
      lte_info_ptr->esm_cause.esm_cause =NAS_ESM_INSUFFICIENT_RESOURCES;
      return DS3G_FAILURE;
    }
    ...
    if (lte_info_ptr->apn_ambr.valid == TRUE)
    {
      memscpy((void *)&(eps_flow_manager_p->eps_flow_manager_dyn_p->eps_pdn_ambr),
               sizeof(apn_ambr_T),
              (void *)&(lte_info_ptr->apn_ambr),
              sizeof(apn_ambr_T));
    }
#ifdef FEATURE_LAPP    
    if (lte_info_ptr->extended_apn_ambr.valid == TRUE) 
    {
      memscpy((void *)&(eps_flow_manager_p->eps_flow_manager_dyn_p->eps_pdn_ext_ambr),
              sizeof(ext_apn_ambr_T),
              (void *)&(lte_info_ptr->extended_apn_ambr),
              sizeof(ext_apn_ambr_T));
    }
#endif /* FEATURE_LAPP */
  ...
  if(result != DS3G_FAILURE)
  {
   ...
   -------------------------------------------------------------------------*/
    if(ds_3gpp_throt_sm_is_cc_sp_throttling_enabled(subs_id) == TRUE)
    {
      ...
      ds_3gpp_pdn_clear_cc_sp_throt_state_for_apn_on_current_plmn(decoded_apn, 
                                                                  subs_id);
    }
  }
  return result;
...
)
{
  ...
  if(pdn_inst == -1)
  {
    DS_LTE_MSG1_HIGH_EX(subs_id,"Cannot generate SDF ID. PDN context: %x not found in table",
                     pdn_cntx_p);
    return DS3G_FAILURE;
  }
  /*-------------------------------------------------------------------------
  ...
  if (DS_3GPP_PDN_SUPPORTS_EMERGENCY_SERVICES(pdn_cntx_p)&&
      (pdn_cntx_p->ds_pdn_context_dyn_p->handover_attach_flag))
  {
    (mm_orig_params_p->orig_params_u.lte_ps_orig_params_ptr)->request_type =
                                                   ESM_EMERGENCY_HANDOVER;
  }
  else if (DS_3GPP_PDN_SUPPORTS_EMERGENCY_SERVICES(pdn_cntx_p))
  {
    (mm_orig_params_p->orig_params_u.lte_ps_orig_params_ptr)->request_type =
                                                   ESM_EMERGENCY;
  }
  ...
  DS_LTE_MSG1_HIGH_EX(subs_id,"Setting request type to %d",
                   (mm_orig_params_p->orig_params_u.lte_ps_orig_params_ptr)->request_type);

  if(ds_3gpp_iwlan_to_lte_ho_is_ho_enabled_for_pdn
     (pdn_cntx_p->ds_pdn_context_dyn_p->pdp_profile_num))
  {
    DS_LTE_MSG1_LOW("PDN is iWLAN handover PDN : %d",
                     pdn_cntx_p->ds_pdn_context_dyn_p->pdp_profile_num);
    (mm_orig_params_p->orig_params_u.lte_ps_orig_params_ptr)->request_type = ESM_HANDOVER;
  }
...
)
{
  ...
  /*--------------------------------------------------------------------------
    Returns current state for attach SM.
  --------------------------------------------------------------------------*/
  DS_LTE_MSG2_LOW("Attach SM. Current State: %d on Subs Id (CM) %d",
...
)
{
  ...
  if(argval_ptr == NULL)
  {
    DS_3GPP_MSG0_LOW("Invalid args to get cost of mdm power");
    *ps_errno = DS_EFAULT;
    return ret_val;
  }
...
)
{
  ...
  DS_LTE_MSG3_HIGH_EX(subs_id,
                    "Attach SM. Old State: %d, New State: %d on sub id %d",
  ...
  event_info.event_payload_p = (void *)attach_sm_ptr;
  ds3geventmgr_set_event_info(DS3GEVENTMGR_LTE_ATTACH_SM_INFO_EV,
                              (void*)&event_info,
                              &filter_info);
...
)
{
  ...
  {
    DS_3GPP_ERROR_LOG_0_EX(cm_subs_id, "Invalid arguments to report LTE throughput quality");
    return ret_val;
  }
  ...
  DS_3GPP_MSG2_HIGH("LTE ML1 reports Allowed t'put %u Bps, "
                    "T'put quality %d",
                    allowed_rate, 
...
void ds_eps_pdn_cntxt_init( void )
{
  ...
  for(ds3g_subs_id = DS3GSUBSMGR_SUBS_ID_MIN; ds3g_subs_id < DS3GSUBSMGR_SUBS_ID_MAX;
      ds3g_subs_id++)
  {
    ds_eps_lte_active_scell_idx_mask[ds3g_subs_id] = 0;
    ds_eps_lte_ul_active_scell_idx_mask[ds3g_subs_id] = 0;
	ds_eps_t3402_value[ds3g_subs_id] = DS_EPS_T3402_DEFAULT_VALUE;
  }
  ...
  --------------------------------------------------------------------------*/
  ds3gmsgr_reg_msgr_cb(NAS_EMM_ATTACH_COMPLETE_IND,
               (ds3g_msgr_client_func_ptr)ds_eps_pdn_cntx_attach_complete_ind_hdlr);

  /*--------------------------------------------------------------------------
    Register for the L2 for TAU complete ind.
  --------------------------------------------------------------------------*/
  ds3gmsgr_reg_msgr_cb(NAS_EMM_TAU_COMPLETE_IND,
               (ds3g_msgr_client_func_ptr)ds_eps_pdn_cntx_tau_complete_ind_hdlr);

  /*--------------------------------------------------------------------------
    Now register with DS Message Router to receive the NAS message for PLMN
    change notification
  --------------------------------------------------------------------------*/
  ds3gmsgr_reg_msgr_cb(NAS_EMM_IND_PLMN_CHANGE,
                      (ds3g_msgr_client_func_ptr)ds_eps_pdn_cntx_plmn_id_chg_hdlr);

  /*--------------------------------------------------------------------------
    Register with DS Message Router to receive the NAS message for T3402
    change notification
  --------------------------------------------------------------------------*/
  ds3gmsgr_reg_msgr_cb(NAS_EMM_T3402_CHANGED_IND,
               (ds3g_msgr_client_func_ptr)ds_eps_pdn_cntx_t3402_changed_ind_hdlr);

  /*--------------------------------------------------------------------------
    Register with DS Message Router to receive the NAS message for T3420
    change notification
  --------------------------------------------------------------------------*/
  ds3gmsgr_reg_msgr_cb(NAS_EMM_DEACT_NON_EMC_BEARER_IND,
               (ds3g_msgr_client_func_ptr)ds_eps_pdn_cntxt_deact_non_emc_bearer_ind_hdlr);

  /*--------------------------------------------------------------------------
    Register with DS Message Router to receive the NAS message for resetting
    Attach APN
  --------------------------------------------------------------------------*/
  ds3gmsgr_reg_msgr_cb(NAS_EMM_RESET_APN_SWITCH_IND,
               (ds3g_msgr_client_func_ptr)ds_eps_pdn_cntxt_reset_attach_apn_ind_hdlr);

  /*--------------------------------------------------------------------------
    Register with DS Message Router to receive the LTE ML1 message for DL TPUT
    estimation indication notification
  --------------------------------------------------------------------------*/
  ds3gmsgr_reg_msgr_cb(LTE_CPHY_DL_TPUT_ESTM_REPORT_IND,
               (ds3g_msgr_client_func_ptr)ds_eps_lte_cphy_dl_throughput_estm_ind_hdlr);

  /*--------------------------------------------------------------------------
    Register with DS Message Router to receive the LTE ML1 message for modem
    power statistics
  --------------------------------------------------------------------------*/
  ds3gmsgr_reg_msgr_cb(LTE_CPHY_TX_POWER_REPORT_INFO_CNF,
               (ds3g_msgr_client_func_ptr)ds_eps_stats_modem_power_info_cnf_hdlr);
  ...
  --------------------------------------------------------------------------*/
  ds3gmsgr_reg_msgr_cb(LTE_CPHY_DL_TPUT_ESTM_REPORT_RSP,
               (ds3g_msgr_client_func_ptr)ds_eps_lte_cphy_dl_throughput_report_rsp_hdlr);

  ds3gmsgr_reg_msgr_cb(LTE_CPHY_CONFIGURED_TPUT_INFO_IND,
               (ds3g_msgr_client_func_ptr)ds_eps_lte_cphy_ind_configured_throughput_hdlr);

  ds3gmsgr_reg_msgr_cb(LTE_CPHY_ACHIEVABLE_TPUT_INFO_CNF,
               (ds3g_msgr_client_func_ptr)ds_eps_lte_cphy_cnf_achievable_throughput_hdlr);

  ds3gmsgr_reg_msgr_cb(LTE_CPHY_ACHIEVABLE_TPUT_STATUS_IND,
               (ds3g_msgr_client_func_ptr)ds_eps_lte_cphy_achievable_throughput_status_hdlr);

  ds3gmsgr_reg_msgr_cb(DS_3GPP_UPLINK_TPUT_SCALING_INFO_IND,
     (ds3g_msgr_client_func_ptr)ds_eps_bearer_handle_uplink_scaling_info);

  ds3gmsgr_reg_msgr_cb(LTE_CPHY_RF_UNAVAILABLE_IND,
     (ds3g_msgr_client_func_ptr)ds_3gpp_flow_control_rf_unavail_hdlr);
  /*--------------------------------------------------------------------------------
    Register with DS Message Router to receive handover complete indication from RRC
  ---------------------------------------------------------------------------------*/
  ds3gmsgr_reg_msgr_cb(LTE_RRC_HANDOVER_COMPLETED_IND,
                       (ds3g_msgr_client_func_ptr)ds_eps_pdn_cntxt_ex_ho_complete_ind_hdlr);
#ifdef FEATURE_DATA_FR44336
  ds3gmsgr_reg_msgr_cb(LTE_RRC_SERVICE_IND,
                       (ds3g_msgr_client_func_ptr)ds_eps_pdn_cntxt_ex_ho_complete_ind_hdlr);
#endif/*FEATURE_DATA_44336*/
  ds3gmsgr_reg_msgr_cb(LTE_CPHY_RF_AVAILABLE_IND,
     (ds3g_msgr_client_func_ptr)ds_3gpp_flow_control_rf_avail_hdlr);
  ds3gmsgr_reg_msgr_cb(LTE_RRC_PAGE_SHARE_STATUS_IND,
   (ds3g_msgr_client_func_ptr)ds_3gpp_flow_control_page_share_status_hdlr);
#ifdef FEATURE_DATA_RAVE_SUPPORT
  ds3gmsgr_reg_msgr_cb(LTE_MAC_UL_TPUT_INFO_START_CNF,
               (ds3g_msgr_client_func_ptr)ds_eps_lte_mac_ulcnf_tput_info_start_cnf);

  ds3gmsgr_reg_msgr_cb(LTE_MAC_UL_TPUT_INFO_IND,
               (ds3g_msgr_client_func_ptr)ds_eps_lte_mac_ul_tput_info);
#endif
...
#error code not present
  ...
  --------------------------------------------------------------------------*/
  ds3gmsgr_reg_msgr_cb(LTE_CPHY_CA_EVENT_IND,
                     (ds3g_msgr_client_func_ptr)ds_eps_pdn_cntxt_cphy_ca_event_hdlr);

  ds3gmsgr_reg_msgr_cb(LTE_CPHY_CA_EVENT_EXT_IND,
                     (ds3g_msgr_client_func_ptr)ds_eps_pdn_cntxt_cphy_ca_event_ext_hdlr);
...
#ifdef FEATURE_DATA_RAVE_SUPPORT
  ds_eps_ul_tput_estimation_init();
#endif
...
/*----------------------------------------------------------------------------*/
  ...
  if(new_mode == SYS_SYS_MODE_LTE)
  {
        ...
        ds_eps_attach_sm_get_state(subs_id) == DS_EPS_PROCESSED_CNTXT_TXFR_IND)
    {
      ...
      /*---------------------------------------------------------------------------- 
        If the Default PDN is set to TRUE, at this point it means LTE attach params
        will be valid, Send a PS SYS LTE GET PARAMS IND EX 
        ----------------------------------------------------------------------------*/
      if(ds_3gpp_pdn_cntxt_get_def_pdn(subs_id) != NULL)
      {
        ds_eps_pdn_cntx_send_lte_attach_param_ex_ind(subs_id,TRUE);
      }
      ...
      if(ds_3gpp_nv_manager_get_always_connect_def_pdn_flag(subs_id) == TRUE)
      {
        ...
        ----------------------------------------------------------------------*/
        DS_LTE_MSG1_HIGH_EX(subs_id,
                         "Full Service in LTE - Bringing up def PDN conn "
        ...
        {
          DS_LTE_MSG0_HIGH_EX(subs_id,"Attach PDN is UP, not bringing up again");

          /*
          Tag the default PDN context for attach retry.after IRAT to LTE is 
          successful. This caters to a specific scenario where the attach APN 
          is not cleaned up from the APN table because the IPv6 iface is not
          down yet. Thus we will attempt to bring up the tagged default PDNs 
          when the IPv6 Iface is completely down.
          */
          ds_3gpp_pdn_cntxt_tag_def_pdn_for_connection_retry(subs_id);
        }
      }
      /* ----------------------------------------------------------------------
         Enable the inactivity and polling timers for all pdns that are up
         and valid for this sub
     ------------------------------------------------------------------------*/
      ds_3gpp_pdn_context_set_inactivity_polling_timers_all_pdns(subs_id, TRUE);
    }
  }
  ...
  if(curr_srv_status == SYS_SRV_STATUS_SRV)
  {
    ...
    {
      DS_LTE_MSG1_MED_EX(subs_id,"Context transfer info match the target rat %d"
                       ,cntxt_transfer_target_rat);
          ...
          ))
      {
        ...
        {
          DS_LTE_MSG1_HIGH_EX(subs_id,"Resetting attach SM to NULL State"
                            " on Subs Id (CM) %d",subs_id);
          ds_eps_pdn_cntx_reset_attach_sm_params(subs_id);
        }
        /*------------------------------------------------------------------- 
          Clear Throttling if throttle behaviour is set to PLMN BLOCKING
         --------------------------------------------------------------------*/
        if(ds_3gpp_data_retry_is_enabled(subs_id))
        {
           ds_3gpp_network_override_hdlr_reset(subs_id, FALSE, FALSE);
        }

        /* ------------------------------------------------------------------- 
           Disable the Inactivity and Polling Timer
        --------------------------------------------------------------------*/
        ds_3gpp_pdn_context_set_inactivity_polling_timers_all_pdns(subs_id, FALSE);
      }
...
)
{
       ...
       (new_pref_sys != SYS_SYS_MODE_LTE) )
  {
    ...
    for(pdn_inst = 0; pdn_inst < DS_3GPP_MAX_PDN_CONTEXT; pdn_inst++)
    {
      ...
      {
        DS_LTE_MSG0_ERROR_EX(subs_id, "Bearer context passed is Invalid");
        continue;
      }
...
)
{
 ...
 /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  /*------------------------------------------------------------------------
   ...
   roaming_status = ds_3gpp_roaming_get_status(subs_id);
   
   new_pdp_type =   ds_3gpp_pdn_context_determine_pdp_type(
                     subs_id,
                     &pdp_profile_ptr->context,
                     roaming_status,
                     FALSE,
   ...
   /*----------------------------------------------------------------------
    ...
    Old IP type                 ----- Changed IP type  -- PDP disconnect
     ...
     ----------------------------------------------------------------------*/
   DS_LTE_MSG3_HIGH_EX(subs_id,
                     "Has APN changed: %d, PDN redial:Old PDP type:%d, New PDP type:%d",
                     has_apn_changed,
                     old_pdp_type,
                     new_pdp_type);
   switch(old_pdp_type)
    ...
    -------------------------------------------------------------------------*/
     case DS_UMTS_PDP_IPV4V6:
       if(new_pdp_type == DS_UMTS_PDP_IPV4)
       {
         ...
         {
           DS_LTE_MSG1_ERROR_EX(subs_id, "Iface ptr:0xx is NULL or "
       ...
       else if (new_pdp_type == DS_UMTS_PDP_IPV6)
       {
         ...
         if (PS_IFACE_IS_VALID(v4_iface_p))
         {
           ...
           ds_3gpp_pdn_cntx_validate_assert(pdn_context_p); 
         }
     ...
     if(has_apn_changed)
     {
       override_ip = ds_3gpp_network_override_hdlr_get_overridden_ip((byte *)new_apn,
                                                                     DS_3GPP_THROT_RAT_LTE,
                                                                     &current_plmn, subs_id);
     }
     else
     {
       override_ip = ds_3gpp_network_override_hdlr_get_overridden_ip((byte *)old_apn,
                                                                     DS_3GPP_THROT_RAT_LTE,
                                                                     &current_plmn, subs_id);
     }
     ...
     /*------------------------------------------------------------------------
       ...
       ------------------------------------------------------------------------*/
     DS_LTE_MSG2_HIGH_EX(subs_id,
                      "Network override IP in place:Overridden IP:%d, "
                      "new PDP IP:%d",override_ip,new_pdp_type);

     if(override_ip != DS_3GPP_PDN_THROT_PDP_UNDEFINED)
     {
       switch(override_ip)
       {
     ...
     {
       DS_3GPP_ERROR_LOG_0_EX(subs_id, "APN disable flag is set");
       bearer_context_p->ds_bearer_context_dyn_p->down_reason =
             PS_NET_DOWN_REASON_APN_PARAM_CHANGE;
       set_bearer_going_down = TRUE;
       goto exit_func;
     ...
     {
       DS_LTE_MSG1_ERROR_EX(subs_id, "APN bearer is incompatible:0xx",
     ...
     if (has_apn_changed)
     {
       switch(new_pdp_type)
       {
         case DS_UMTS_PDP_IPV4:
               ...
               [DS_3GPP_PDN_CONTEXT_IPV4_IFACE] != NULL)
           {
             ...
             {
               DS_LTE_MSG0_ERROR_EX(subs_id, "Changed APN for V4 IPtype is throttled");
               bearer_context_p->ds_bearer_context_dyn_p->down_reason =
                 PS_NET_DOWN_REASON_APN_PARAM_CHANGE;
               set_bearer_going_down = TRUE;
               goto exit_func;
         ...
         case DS_UMTS_PDP_IPV6:
               ...
               [DS_3GPP_PDN_CONTEXT_IPV6_IFACE] != NULL)
           {
             ...
             {
               DS_LTE_MSG0_ERROR_EX(subs_id, "Changed APN for V6 IP type is throttled");
               bearer_context_p->ds_bearer_context_dyn_p->down_reason =
                 PS_NET_DOWN_REASON_APN_PARAM_CHANGE;
               set_bearer_going_down = TRUE;
               goto exit_func;
         ...
         case DS_UMTS_PDP_IPV4V6:
               ...
               [DS_3GPP_PDN_CONTEXT_IPV6_IFACE] != NULL))
           {
             ...
             {
               DS_LTE_MSG0_ERROR_EX(subs_id, "Changed APN for either V4V6 IP type is throttled");
               bearer_context_p->ds_bearer_context_dyn_p->down_reason =
                 PS_NET_DOWN_REASON_APN_PARAM_CHANGE;
               set_bearer_going_down = TRUE;
               goto exit_func;
       ...
       for (pdn_index = 0;pdn_index < DS_3GPP_MAX_PDN_CONTEXT;pdn_index++)
       {
         ...
         if(!ds_3gpp_pdn_cntx_validate_pdn_context_internal(temp_pdn_context_ptr))
         {
           continue;
         }
         /*----------------------------------------------------------
         ...
         {
           /*--------------------------------------------------------------------
             APN match has occured, Check if PDN is up or coming up
             ---------------------------------------------------------------------*/
           DS_3GPP_ERROR_LOG_3_EX(subs_id, "PDN_Index:d has the new changed apn and "
                              "is in %d state in subs id %d, No redial",
                              temp_pdn_context_ptr->ds_pdn_context_dyn_p->index, 
                              temp_pdn_context_ptr->ds_pdn_context_dyn_p->state, 
                              subs_id);
             bearer_context_p->ds_bearer_context_dyn_p->down_reason =
                 PS_NET_DOWN_REASON_APN_PARAM_CHANGE;
             set_bearer_going_down = TRUE;
       ...
       */
       ...
       if (pdn_context_p->ds_pdn_context_dyn_p->keep_alive_pdn == TRUE)
       {
         ...
         {
           DS_LTE_MSG0_HIGH_EX(subs_id,"Keep Alive PDN does not support NULL APN, tearing "
                             "it down");
           bearer_context_p->ds_bearer_context_dyn_p->down_reason =
                 PS_NET_DOWN_REASON_APN_PARAM_CHANGE;
           set_bearer_going_down = TRUE;
         ...
         {
           DS_LTE_MSG0_ERROR_EX(subs_id, "Preferred Packet Mode is not LTE for KAMGR:"
                              " Tearing it down");
           bearer_context_p->ds_bearer_context_dyn_p->down_reason =
                 PS_NET_DOWN_REASON_APN_PARAM_CHANGE;
           set_bearer_going_down = TRUE;
       ...
       if(!ds_3gpp_nv_manager_get_null_apn_support_for_non_attach_pdn(subs_id) &&
        ds_pdn_cntx_reject_null_apn(pdn_context_p, new_apn))
       {
         DS_LTE_MSG0_ERROR_EX(subs_id, "Changed APN name is in NULL and non attach apn"
                              " Tearing it down");
         bearer_context_p->ds_bearer_context_dyn_p->down_reason = 
               PS_NET_DOWN_REASON_NULL_APN_DISALLOWED;
         set_bearer_going_down = TRUE;
         goto exit_func;
       }
       else if(TRUE == ds3g_profile_validator_is_apn_name_forbidden(pdn_context_p, new_apn, subs_id))
       {
         DS_LTE_MSG0_ERROR_EX(subs_id, "Changed APN name is in forbidden list"
                              " Tearing it down");
         bearer_context_p->ds_bearer_context_dyn_p->down_reason = 
               PS_NET_DOWN_REASON_APN_PARAM_CHANGE;
         set_bearer_going_down = TRUE;
       ...
       {
         DS_3GPP_ERROR_LOG_0_EX(subs_id, "APN tbl entry deletion failure");
       }
     }

     /*------------------------------------------------------------------------- 
       Reset handover flag here, HO flag can never be set to TRUE when we hit
       this condition
       -------------------------------------------------------------------------*/
     ...
     exit_func:
       ...
       {
         DS_LTE_MSG0_HIGH_EX(subs_id,"Setting bearer and PDN to going down state");
  ...
  if(pdn_context_p->ds_pdn_context_dyn_p->apn_param_chg_teardown)
  {
    if(reset_gateway_iid == TRUE)
    {
     v6_iface_p = DSPDNCNTXT_GET_V6_PS_IFACE_PTR(pdn_context_p);
      ...
      {
       DS_3GPP_MSG0_MED_EX(subs_id,"Reseting the Gateway IID of primary IPv6 addr");
...
)
{
  boolean result = FALSE;
  sys_modem_as_id_e_type           subs_id  = SYS_MODEM_AS_ID_NONE;
  ds_umts_pdp_type_enum_type       pdp_type_to_use = DS_UMTS_PDP_IPV4;
 /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
 ...
 /*-------------------------------------------------------------------------
  ...
  pdp_type_to_use =  ds_3gpp_pdn_context_determine_pdp_type(
                        subs_id,
                       &pdn_context_p->ds_pdn_context_dyn_p->pdp_profile.context,
                        ds_3gpp_roaming_get_status(subs_id),
                        FALSE,
                        ds3g_get_current_network_mode_ex(subs_id));

  pdn_context_p->ds_pdn_context_dyn_p->pdp_profile.context.pdp_type = pdp_type_to_use;
...
)
{
  ...
  {
    DS_LTE_MSG1_HIGH_EX(subs_id,"ATTACH SM state EPS_EMERGENCY_ATTACHED"
  ...
  {
    DS_LTE_MSG1_HIGH_EX(subs_id,"ATTACH SM state EPS_ATTACHED on Subs Id (CM) %d",subs_id);
    ds_eps_attach_sm_set_state(DS_EPS_ATTACHED,subs_id);
    /*Reset attach pending null bearer reason bit*/
  ...
  /*---------------------------------------------------------------------
    Notify DSD of LTE Attach Completed event
  ----------------------------------------------------------------------*/
  ...
  DS_LTE_MSG1_MED_EX(subs_id,"Posted Attach Complete to APM, error code: %d",apm_error_code);

  /*-------------------------------------------------------------------- 
    If attach list has been changed by AP, handle here. This is the case
    where AP sends set_lte_attach_pdn_list when the current attach is
    still in progress
    --------------------------------------------------------------------*/
  ds_3gpp_apn_switch_mgr_detach_if_attach_list_modified_by_ap(subs_id);
  ...
  ps_dpm_err_ind_cback_reg( ds_3gpp_get_bearer_err_threshhold_cnt(subs_id),
...
)
{
  ...
  {
    DS_LTE_MSG0_ERROR_EX(subs_id, "attach info p is NULL");
    return FALSE;
  }
  ...
  {
    any_active_bearers = ds_3gpp_bearer_cntxt_is_any_bearer_not_down(subs_id);
  }

  if (any_active_bearers || any_hanging_context)
  {

    DS_LTE_MSG2_HIGH_EX(subs_id,"Found active bearer:%d  hanging bearer:%d, caching PDN_CONN_REQ",
                     any_hanging_context,
                     any_active_bearers);
    /*--------------------------------------------------------------------------
    ...
    if(ds_3gpp_pdn_cntx_validate_pdn_context(pdn_cntx_p))
    {
      ...
      {
        DS_LTE_MSG0_HIGH_EX(subs_id,"Default bearer/Default PDN Pending REDIAL");
      ...
      else
      {
        ...
        DS_LTE_MSG1_HIGH_EX(subs_id,"Default pdn bearer isn't in PEND_REDIAL: 0x%x",pdn_cntx_p);
      }
    }
    else
    {
      DS_LTE_MSG0_HIGH_EX(subs_id,"Not a Default PDN context;Not Redialing");
    }
    ...
    DS_LTE_MSG0_HIGH_EX(subs_id,"atleast one pending call,delaying GET_PDN_CONN_IND"
                     " processing");
    ...
    DS_LTE_MSG3_HIGH_EX(subs_id,"Setting target rat while attach is cached, attach_profile: %d,"
...
)
{
  ...
  *pdn_cntx_p = ds_3gpp_pdn_cntxt_get_def_pdn(subs_id);
  ...
  if(*pdn_cntx_p != NULL)
  {
    DS_LTE_MSG1_HIGH_EX(subs_id,"ds_eps_pdn_cntxt_get_attach_pdn. There is already "
                                           ...
                                           subs_id) == TRUE)
  {
    ...
    DS_LTE_MSG2_HIGH_EX(subs_id,"IP type in APN Table: %d, IP Type in Attach Profile: %d",
    ...
    if(ds_3gpp_pdn_cntx_validate_pdn_context(*pdn_cntx_p))
    {
      (*pdn_cntx_p)->ds_pdn_context_dyn_p->is_default = TRUE; // clang_sa_ignore [core.NullDereference]
    }
    else
    {
      DS_LTE_MSG0_ERROR_EX(subs_id, "Cant get PDN eventhough APN is present in APN table");
    }
...
)
{
  ...
  if (ps_iface_p != NULL)
  {
    ...
    {
      DS_LTE_MSG1_ERROR_EX(subs_id, "Could not set addr family to d", addr_family);
    }
    else
    {
    ...
    DS_LTE_MSG1_LOW("Set 3GPP profile num %d to iface",
                     pdn_cntx_p->ds_pdn_context_dyn_p->pdp_profile_num);
    ps_iface_set_3gpp_profile_id(ps_iface_p, 
                         pdn_cntx_p->ds_pdn_context_dyn_p->pdp_profile_num);
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    info_ptr = ds_3gpp_pre_attach_hdlr_get_user_data(subs_id);
  }
  else if (ds_3gpp_apn_switch_hdlr_enabled(subs_id))
  {
    info_ptr = ds_3gpp_apn_switch_hdlr_get_user_data(subs_id);
  }  
  ...
  ds_3gpp_pdp_profile_process_status
    = ds_pdn_cntxt_process_pdp_profile_info(
                             pdn_cntx_p,
                             pdn_cntx_p->ds_pdn_context_dyn_p->pdp_profile_num,
                             roaming_status,
                             &ip_support,
  ...
  {
    DS_LTE_MSG0_ERROR_EX(subs_id, "pdp_profile can't be processed");
    return_value = DS3G_FAILURE;
    goto exit_function;
  }
  ...
  if( pdn_cntx_p->ds_pdn_context_dyn_p->is_default )
  {
    ...
    ind_payload.attach_pdp_type
      = (ds_profile_3gpp_pdp_type_enum_type)
          pdn_cntx_p->ds_pdn_context_dyn_p->pdp_profile.context.pdp_type;
    ind_payload.profile_id = profile_no;
  ...
  if(ds_eps_get_lte_attach_dual_ip_fb_status(subs_id) == TRUE)
  {
    ...
    {
      DS_LTE_MSG1_HIGH_EX(subs_id,"Previous attach attempt failed with cause 0x%x",
  ...
  switch(pdn_cntx_p->ds_pdn_context_dyn_p->pdp_profile.context.pdp_type)
  {
    case DS_UMTS_PDP_IPV4:
      ...
      {
        DS_LTE_MSG0_ERROR_EX(subs_id, "Cant allocate V4 DS iface");
        return_value = DS3G_FAILURE;
        goto exit_function;
      }
            ...
            (pdn_cntx_p, IFACE_IPV4_ADDR_FAMILY, subs_id))
      {
        ret_val = ps_iface_active_bring_up_cmd
                    (DSPDNCNTXT_GET_V4_PS_IFACE_PTR(pdn_cntx_p), &ps_error, info_ptr);
      }
      else
      {
        ret_val = 0;
    ...
    case DS_UMTS_PDP_IPV6:
      if(ds3g_is_ipv6_enabled() == TRUE)
      {
        ...
        {
          DS_LTE_MSG0_ERROR_EX(subs_id, "Cant allocate V6 DS iface");
          return_value = DS3G_FAILURE;
          goto exit_function;
        }
        ...
        {
          ret_val = ps_iface_active_bring_up_cmd
                      (DSPDNCNTXT_GET_V6_PS_IFACE_PTR(pdn_cntx_p), &ps_error, info_ptr);
        }
        else
        {
          ret_val = 0;
      ...
      {
        DS_LTE_MSG0_ERROR_EX(subs_id, "Cannot initiate Attach with V6 prof."
                          " V6 NV not enabled");
        ret_val = 0;
        ps_error = DS_EOPNOTSUPP;
      }
    ...
    case DS_UMTS_PDP_IPV4V6:
      ...
      if(ds_iface_v4_p == NULL)
      {
        DS_LTE_MSG0_ERROR_EX(subs_id, "Could not allocate V4 iface");
        return_value = DS3G_FAILURE;
        goto exit_function;
      }
      ...
      else
      {
        if(ds3g_is_ipv6_enabled() == TRUE)
        {
          ...
          if(ds_iface_v6_p == NULL)
          {
            DS_3GPP_ERROR_FATAL("Could not set addr family to V6");
          }
          else
          {
            ds_iface_v6_p->client_data_ptr = pdn_cntx_p;
        ...
        {
          DS_LTE_MSG0_ERROR_EX(subs_id, "Attach with V4V6 prof V6 NV not enabled");
          ret_val = -1;
    ...
    default:
      DS_LTE_MSG0_HIGH_EX(subs_id,"Unknown PDP type. Cannot Initiate Attach process");
      /*----------------------------------------------------------------------
  ...
  {
    DS_LTE_MSG2_ERROR_EX(subs_id, "Iface Bring Up error. ret_val: d, err code:d",
                      ret_val, ps_error);
    return_value = DS3G_FAILURE;
    goto exit_function;
  }
  ...
  {
    DS_LTE_MSG2_ERROR_EX(subs_id, "Error updating APN table. Pdn: 0xx, IP support:d",
...
exit_function:
  if((ds_3gpp_pdn_cntx_validate_pdn_context(pdn_cntx_p)
      && return_value == DS3G_FAILURE))
  {
    DS_LTE_MSG1_ERROR_EX(subs_id, "Iface bring up failed. Remove the PDN entry from the "
                      "APN table and free the PDN 0xx",
                      pdn_cntx_p);
    ...
    {
      DS_3GPP_ERROR_LOG_0_EX(subs_id, "Invalid PDN Passed to remove apn table");
    }
    ...
    if((index = ds_3gpp_pdn_limit_tbl_find_entry
         ((byte*)pdn_cntx_p->ds_pdn_context_dyn_p->pdp_profile.context.apn, // clang_sa_ignore [core.NullDereference]
           subs_id)) != DS_3GPP_PDN_LIMIT_TBL_INVALID_ENTRY) // clang_sa_ignore [core.NullDereference]
    {
...
)
{
  ...
  ds_umts_pdp_profile_type               *prof_context_p = NULL;
  ...
  ds_3gpp_pdn_process_new_plmn(subs_id, current_plmn_id,
                               SYS_SYS_MODE_LTE);
  ind_payload.subs_id = subs_id;
  ...
  -----------------------------------------------------------------------*/
  prof_context_p = (ds_umts_pdp_profile_type*)modem_mem_alloc(
                  sizeof(ds_umts_pdp_profile_type),
                  MODEM_MEM_CLIENT_DATA);
  if (prof_context_p == NULL)
  {
    return DS_EPS_BRINGUP_ATTACH_PDN_FAIL;
  }
  memset(prof_context_p, 0, sizeof(ds_umts_pdp_profile_type));
  ...
  if (FALSE == dsPdnContext_is_any_other_normal_pdn_up(NULL, subs_id))
  {
    ...
    ----------------------------------------------------------------------*/
    attach_profile_id = ds_eps_get_attach_prof(subs_id);
    DS_LTE_MSG0_HIGH_EX(subs_id,"Attach initiation. Set Target_Rat to LTE");

    if(FALSE == ds3gmgr_send_trat_ind(SYS_SYS_MODE_LTE,
                                      attach_profile_id, prof_context_p,
                                      subs_id))
    {
      ds_3gpp_pdn_cntxt_block_plmn(subs_id);
      ds_eps_pdn_cntx_reset_attach_sm_params(subs_id);
      modem_mem_free(prof_context_p, MODEM_MEM_CLIENT_DATA);
      return DS_EPS_BRINGUP_ATTACH_PDN_FAIL;
    }
  }
  else
  {
    ...
    if (DS_UMTS_PDP_SUCCESS != ds_umts_get_pdp_profile_all_data_per_subs(
												...
												prof_context_p))
    {
      DS_LTE_MSG1_ERROR_EX(subs_id, "Invalid attach_profile_id: d", attach_profile_id);
      ds_3gpp_pdn_cntxt_block_plmn(subs_id);
      ds_eps_pdn_cntx_reset_attach_sm_params(subs_id);
      modem_mem_free(prof_context_p, MODEM_MEM_CLIENT_DATA);
      return DS_EPS_BRINGUP_ATTACH_PDN_FAIL;
    ...
    DS_LTE_MSG1_HIGH_EX(subs_id,"Not fresh attach, bringing up attach PDN after IRAT"
  ...
  if(ds_3gpp_cfg_get_subsid(DS_3GPP_CFG_PROFILE_LIST,
                    ...
                    (sizeof(uint16) * DS_3GPP_CFG_PROFILES_MAX),
                     subs_id) == -1)
  {
    DS_LTE_MSG0_ERROR_EX(subs_id, "Cannot read EFS Config item DS_3GPP_CFG_PROFILE_LIST");
  }
  else
  {
    ...
    {
      DS_LTE_MSG0_ERROR_EX(subs_id, "Invalid Profile List, not checking apn disable flag");
    }
    else
    {
      for(index = 1; index <= num_profiles; index++)
      {
         ...
         if(ds_umts_get_pdp_profile_apn_disable_flag_per_subs(profile,
                                                ...
                                                &apn_disable_flag) !=
                                                           DS_UMTS_PDP_SUCCESS)
         {
           DS_LTE_MSG0_ERROR_EX(subs_id, "Cannot read APN disable flag in"
                             "Attach profile");
           continue;
         }
         ...
         if(apn_disable_flag == TRUE)
         {
           ...
           ------------------------------------------------------------------*/
           DS_LTE_MSG1_HIGH_EX(subs_id,"Disabling LTE srv during init - APN (%d)"
                            "is disabled", profile);
  ...
  {
    DS_LTE_MSG1_HIGH_EX(subs_id,"Error while getting Attach PDN on Subs Id (CM) %d",
                        subs_id);
    return_value = DS_EPS_BRINGUP_ATTACH_PDN_FAIL;
    goto exit_function;
  }
     ...
     (pdn_cntx_p->ds_pdn_context_dyn_p->cookie == PDN_CNTXT_VALID_COOKIE) )
  {
    ...
    memset(&ind_payload, 0, sizeof(ds_dsd_apm_ind_pay_load));
    ind_payload.subs_id = subs_id;
    ind_payload.attach_pdp_type = (ds_profile_3gpp_pdp_type_enum_type)
             pdn_cntx_p->ds_pdn_context_dyn_p->pdp_profile.context.pdp_type;
    ind_payload.profile_id = attach_profile_id;

    if(ds_3gpp_pdn_cntxt_get_subs_id_from_pdn(pdn_cntx_p,&pdn_cntxt_subs_id))
    {
      if (pdn_cntxt_subs_id != subs_id)
      {
        DS_LTE_MSG2_ERROR_EX(subs_id, "Error using Attach sub id d and pdn cntxt "
                          "sub id d for Attach",subs_id,pdn_cntxt_subs_id);
        return_value = DS_EPS_BRINGUP_ATTACH_PDN_FAIL;
        goto exit_function;
      }
    }
    ...
    {
      DS_LTE_MSG1_HIGH_EX(subs_id,"Default PDN not in DOWN / redial state %d- "
                       "Ignore Bring up",
                       pdn_cntx_p->ds_pdn_context_dyn_p->state);
    ...
    /*-----------------------------------------------------------------------
      If the PDN Context is in pending redial state, we ignore the LTE
      Attach Profile and continue to use the profile which was used to
      bring up the UMTS call
    ------------------------------------------------------------------------*/
    if( pdn_cntx_p->ds_pdn_context_dyn_p->state ==
            DS_PDN_CONTEXT_STATE_PEND_REDIAL )
    {
      ...
      if (ds_eps_pdn_cntxt_is_silent_redial_allowed(pdn_cntx_p))
      {
        ...
        DS_LTE_MSG0_HIGH_EX(subs_id,
                         "Redialing PDN conn as part of Attach process, "
        ...
        /*---------------------------------------------------------------------
          ...
          Setting is_gw_originated here since the code path is common for all
          IRAT scenarios.
        ---------------------------------------------------------------------*/
    ...
    DS_LTE_MSG0_HIGH_EX(subs_id,"Default already PDN exists, No bring up again");

    ds_subs_id = ds3gsubsmgr_subs_id_cm_to_ds(subs_id);
    
    if( (iface_p = DSPDNCNTXT_GET_V4_PS_IFACE_PTR(pdn_cntx_p)) != NULL )
    {
      PS_IFACE_SET_SUBS_ID(iface_p, ds_subs_id);
    }
    
    if( (iface_p = DSPDNCNTXT_GET_V6_PS_IFACE_PTR(pdn_cntx_p)) != NULL )
    {
      PS_IFACE_SET_SUBS_ID(iface_p, ds_subs_id);
    }
   ...
   (void)ds_dsd_apm_ind(DS_DSD_APM_ATTACH_PDP_TYPE_IND, &ind_payload);
   /*----------------------------------------------------------------------------------------------- 
     If this point is reached it means there is already another pdn context which is being brought
     up, Ensure that the profile number of the pdn context that is beign brought up is the
     same as the attach profile id, This will ensure APM profile id mismatch
     -----------------------------------------------------------------------------------------------*/
    pdn_cntx_p->ds_pdn_context_dyn_p->pdp_profile_num = attach_profile_id;
    pdn_cntx_p->ds_pdn_context_dyn_p->pdp_profile.context.pdp_context_number = attach_profile_id;
  } /* pdn_cntx_p != NULL */
  else
  {
    if(FALSE == ds_eps_pdn_cntxt_allocate_pdn_and_bringup_ifaces(
                                                   attach_profile_id,
                                                   NULL,
                                                   prof_context_p,
                                                   subs_id))
  ...
  if((attach_init == TRUE) && (return_value != DS_EPS_BRINGUP_ATTACH_PDN_FAIL))
  {
    /*-------------------------------------------------------------------------
    ...
    DS_LTE_MSG0_HIGH_EX(subs_id,"Sent PS_SYS_EVENT_3GPP_LTE_ATTACH_INIT ");
...
exit_function:
  ...
  {
    DS_LTE_MSG1_HIGH_EX(subs_id,"Resetting attach SM to NULL State for Subs Id (CM) %d",
                     subs_id);
    ds_eps_pdn_cntx_reset_attach_sm_params(subs_id);
  }
  modem_mem_free(prof_context_p, MODEM_MEM_CLIENT_DATA);
...
)
{
  ...
  if((attach_params_ptr == NULL) || 
     (!ds_3gpp_pdn_cntx_validate_pdn_context(pdn_cntx_p)))
  {
    DS_LTE_MSG0_ERROR("Input parameters are invalid");
    return;
  }

  ps_v4_iface_p = DSPDNCNTXT_GET_V4_PS_IFACE_PTR(pdn_cntx_p);
  ps_v6_iface_p = DSPDNCNTXT_GET_V6_PS_IFACE_PTR(pdn_cntx_p);

  /*------------------------------------------------------------------------ 
    Populate V4 and V6 address
   ------------------------------------------------------------------------*/
  if((ps_v4_iface_p != NULL) && (pdp_type != DS_UMTS_PDP_IPV6))
  {
    attach_params_ptr->attach_params.ipv4_addr = 
      ps_iface_get_v4_addr(ps_v4_iface_p);

    ret_ip_addr.type = IPV4_ADDR;
    ps_iface_get_peer_addr (ps_v4_iface_p,&ret_ip_addr);

    attach_params_ptr->attach_params.ipv4_gateway_addr =
      ret_ip_addr.addr.v4.ps_s_addr;

    attach_params_ptr->attach_params.ipv4_subnet_mask =
      ps_v4_iface_p->net_info.net_ip.v4.net_mask.ps_s_addr;
  }

  if((ps_v6_iface_p != NULL) && (pdp_type != DS_UMTS_PDP_IPV4))
  {

    attach_params_ptr->attach_params.ipv6_addr_iid =
       ps_iface_get_v6_iid(ps_v6_iface_p);

    attach_params_ptr->attach_params.ipv6_addr_prefix =
      ps_iface_get_v6_prefix(ps_v6_iface_p);

    ret_ip_addr.type = IPV6_ADDR;
    ps_iface_get_peer_addr(ps_v6_iface_p,&ret_ip_addr);

    attach_params_ptr->attach_params.ipv6_gateway_prefix =
       ret_ip_addr.addr.v6.ps_s6_addr64[0];

    attach_params_ptr->attach_params.ipv6_gateway_iid =
       ret_ip_addr.addr.v6.ps_s6_addr64[1];
  }

  return;
}/*ds_eps_pdn_cntxt_fill_lte_attach_params_ip_addr*/
...
)
{
  ...
  if ((arg_val_ptr == NULL) || (ps_errno == NULL))
  {
    ...
    DS_LTE_MSG0_ERROR_EX(subs_id, "Null Pointer passed");
    return -1;
  }
  ...
  {
    ds3gpp_leave_global_crit_section();
    DS_LTE_MSG0_ERROR_EX(subs_id, "LTE Attach not complete,PDN not in up state");
    *ps_errno = DS_EINVAL;
    return -1;
  }
  ...
  else if(ds_pdn_cntxt_get_resolved_apn_name(pdn_cntx_p,
                       &(apn_ptr)) == FALSE)
  {
     /*---------------------------------------------------------------------
       This is the case when both APN/Resolved APN are NULL.
     -----------------------------------------------------------------------*/
     DS_LTE_MSG0_HIGH_EX(subs_id,"APN/Resolved APN unavailable in the APN table");
     memset(apn_info->apn_string,0,DS_UMTS_MAX_APN_STRING_LEN+1);
     DS_LTE_MSG0_HIGH_EX(subs_id, 
                         "APN/Resolved APN unavailable in the APN table");
  }
  else
  {
 ...
 /*-----------------------------------------------------------------------
  ...
  switch(pdp_support_type)
  {
    ...
    case  DS_UMTS_PDP_IPV4V6:
      ...
      break;
    default:
       DS_LTE_MSG0_ERROR_EX(subs_id, "IP type is not valid");
      *ps_errno = DS_EINVAL;
      return -1;
  }
...
DESCRIPTION
  ...
  LTE attach params are not valiud in these cases
  1. Attach PDN context is not UP
...
)
{
  ...
  ds_pdn_context_s                               *pdn_cntx_p = NULL;
  ps_sys_ioctl_3gpp_lte_attach_params_ex_type    *attach_params_p = NULL;
  byte                                           *apn_ptr = NULL;
  ds_umts_pdp_type_enum_type                      pdp_support_type =
                                                      DS_UMTS_PDP_MAX;
  ds_eps_attach_sm_s                             *attach_sm_ptr = NULL;
  ...
  do
  {
    ...
    attach_params_p = (ps_sys_ioctl_3gpp_lte_attach_params_ex_type *)arg_val_ptr;

    memset(attach_params_p,
           0,
           sizeof(ps_sys_ioctl_3gpp_lte_attach_params_ex_type));


    attach_params_p->attach_params.is_valid = FALSE;

    /*-------------------------------------------------------------------------- 
      If attach sm is NULL, return FAILURE
      --------------------------------------------------------------------------*/
    attach_sm_ptr = ds_eps_get_attach_sm_per_sub_id(subs_id);

    if(attach_sm_ptr == NULL)
    {
      DS_LTE_MSG1_ERROR_EX(subs_id, "Invalid Attach SM ptr for Subs Id (CM) d",subs_id);
     *ps_errno = DS_EINVAL;
      break;
    }
    ...
    /*-------------------------------------------------------------------------- 
      Get the default PDN, IF the PDN is not VAlid it means one of the below
      1. UE did an IRAT to LTE from 3G
      2. Default PDN was brought down (because of NV's turned on)
      --------------------------------------------------------------------------*/
    if (!ds_3gpp_pdn_cntx_validate_pdn_context(pdn_cntx_p))
    {
	   *ps_errno = DS_EINVAL;      
      DS_LTE_MSG0_LOW("PDN cntxt passed is not valid");
      break;
    }

    if(!ds3gpp_get_is_attach_complete_flag(subs_id))
    {
	   *ps_errno = DS_EINVAL;
      DS_LTE_MSG0_LOW("LTE Attach not complete");
      break;
    }
    /*------------------------------------------------------------------------ 
      get the PDN state.
      Even if PDN state is COMING UP its ok to send out the indication
      If we reach the stage and PDN is in coming up state, it means
      ATTACH COMPLETE IND has been received but RAB_REG_DL_CNF is pending.
     
      If RAB_REG_DL_CNF is never received LTE INVALID IND will anyways be sent
      ------------------------------------------------------------------------*/
    if ((pdn_cntx_p->ds_pdn_context_dyn_p->state != DS_PDN_CONTEXT_STATE_UP) &&
        (pdn_cntx_p->ds_pdn_context_dyn_p->state != DS_PDN_CONTEXT_STATE_COMING_UP))
    {
      DS_LTE_MSG1_ERROR_EX(subs_id, "PDN not in up state:d",
                        pdn_cntx_p->ds_pdn_context_dyn_p->state);
      break;
    }
    ...
    ret_val = 0;
   *ps_errno = 0;
    attach_params_p->attach_params.is_valid = TRUE;
   ...
   -------------------------------------------------------------------*/
    ...
    if(strlen((char *)pdn_cntx_p->ds_pdn_context_dyn_p->pdp_profile.context.apn) != 0)
    {
      (void)strlcpy((char *)attach_params_p->attach_params.apn_string,
                    (char *)pdn_cntx_p->ds_pdn_context_dyn_p->pdp_profile.context.apn,
                    DS_UMTS_MAX_APN_STRING_LEN+1);
    }
    else if(ds_pdn_cntxt_get_resolved_apn_name(pdn_cntx_p,
                       &(apn_ptr)) == FALSE)
    {
     ...
     -----------------------------------------------------------------------*/
      DS_LTE_MSG0_HIGH_EX(subs_id,"APN/Resolved APN unavailable in the APN table");
      memset(attach_params_p->attach_params.apn_string,0,DS_UMTS_MAX_APN_STRING_LEN+1);
    }
    else
    {
      (void)strlcpy((char *)attach_params_p->attach_params.apn_string,(char *)apn_ptr,
            DS_UMTS_MAX_APN_STRING_LEN+1);
    }
  ...
  /*---------------------------------------------------------------------------- 
    If attach param is not valid return here, nothing else needs to be processed
    -----------------------------------------------------------------------------*/
  if((ret_val == -1) || (!attach_params_p->attach_params.is_valid))
  {
    DS_LTE_MSG0_HIGH_EX(subs_id,"LTE attach param is not valid, return");
    return ret_val;
  }

  /*--------------------------------------------------------------------------- 
    Now populate the PDP type
    ---------------------------------------------------------------------------*/

  switch(pdp_support_type)
  {
    case  DS_UMTS_PDP_IPV4:
      attach_params_p->attach_params.ip_type = PS_SYS_IP_TYPE_IPV4;
      break;
    case  DS_UMTS_PDP_IPV6:
      attach_params_p->attach_params.ip_type = PS_SYS_IP_TYPE_IPV6;
      break;
    case  DS_UMTS_PDP_IPV4V6:
      attach_params_p->attach_params.ip_type = PS_SYS_IP_TYPE_IPV4IPV6;
      break;
    default:
       DS_LTE_MSG0_ERROR_EX(subs_id, "IP type is not valid");
      *ps_errno = DS_EINVAL;
  ...
  attach_params_p->attach_params.apn_string_len = strlen((char *)attach_params_p->
                                                          attach_params.apn_string);

  attach_params_p->ota_attach_performed = attach_sm_ptr->ota_attach_performed;

  /*--------------------------------------------------------------------------------- 
    Populate the IP addresses
    ---------------------------------------------------------------------------------*/
  ds_eps_pdn_cntxt_fill_lte_attach_params_ip_addr(attach_params_p,
                                                  pdn_cntx_p,
                                                  pdp_support_type);
  return ret_val;
}/*ds_eps_pdn_cntxt_get_lte_attach_params_ex*/
...
)
{
  ...
  {
    DS_3GPP_MSG3_LOW("Attach SM ptr:0x%x,attach_info_p:0x%x,subs_id",
                        attach_sm_ptr,attach_info_ptr,subs_id);
    return DS3G_FAILURE;
  }

  /* Updated PLMN has to be processed to avoid back to back PLMN block/unblock requests */
  ...
  /*---------------------------------------------------------------------------------
   If enter attach pending state, store seq num and paging response for reprocessing
   and return right away. Otherwise, keep processing the IND.
  ----------------------------------------------------------------------------------*/
  if (ds_eps_pdn_cntxt_ex_check_attach_pending(subs_id))
  {
    attach_sm_ptr->cached_seq_num = attach_info_ptr->seq_num;
    attach_sm_ptr->cached_paging_response = attach_info_ptr->responding_to_page;
    DS_LTE_MSG0_HIGH_EX(subs_id, "ds_eps_pdn_cntxt_get_pdn_conn_ind_hdlr "
                                 "enter attach pending");
    return DS3G_FAILURE;
  }
  ...
  ---------------------------------------------------------------------------*/
  if (ds_3gpp_dam_get_pdn_blocked_status(subs_id))
  {
  ...
  {
    DS_LTE_MSG2_HIGH_EX(subs_id,"Dup Attach request rx'ed (or) prev attach clean up in"
  ...
  /*--------------------------------------------------------------------
    Clear the cached PLMN ID here from CM because.  This is to prevent
    the race condition where UE moves from RAT 1 full service to RAT 2
    without getting full service, then back to RAT 1 full service.  In
    the second leg of this transition, it's possible we will not detect
    the PLMN change from RAT 2 to RAT 1 because this state only gets
    updated when CM's ss event shows full service for that RAT.
  --------------------------------------------------------------------*/
  ...
  DS_LTE_MSG0_HIGH_EX(subs_id,"Setting attach SM to PROCESS_GET_PDN_CONN_IND State");
  ...
  if(bringup_attach_pdn_status == DS_EPS_BRINGUP_ATTACH_PDN_FAIL)
  {
    ...
    ----------------------------------------------------------------------*/
    DS_LTE_MSG0_HIGH_EX(subs_id,"GET_PDN_CONN_IND_HDLR - Failed to bring up attach PDN, "
    ...
    {
      DS_LTE_MSG1_HIGH_EX(subs_id,"Resetting DS3GPP internal mode after Attach Failure to %d",
                        network_mode);
...
/* - - - - - - - - - - - - - -  - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if((msgr_type != NAS_EMM_T3402_CHANGED_IND)
     || (dsmsg_ptr == NULL))
  {
    DS_LTE_MSG1_LOW("ds_3gpp_t3402_changed_ind_hdlr: Invalid message %d",
                      msgr_type);
    return FALSE;
  }
  ...
  DS_LTE_MSG2_HIGH_EX(subs_id,"ds_3gpp_t3402_changed_ind_hdlr: Received T3402 from NAS: %d "
                    " on sub id %d",msg_ptr->t3402_value,subs_id);
  ds_eps_pdn_cntxt_set_t3402_timer_val(subs_id,msg_ptr->t3402_value);
...
)
{
  ...
  do
  {
    ...
    {
      //DS_LTE_MSG0_ERROR_EX(subs_id, "lte_info_ptr is NULL");
      local_err = DS_3GPPI_LOCAL_ERR_VAL_0;
      ret_val = FALSE;
      break;
    }
    ...
    {
      //DS_LTE_MSG0_ERROR_EX(subs_id, " APN, valid flag not set");
      local_err = DS_3GPPI_LOCAL_ERR_VAL_1;
      lte_info_ptr->esm_cause.valid = TRUE;
      lte_info_ptr->esm_cause.esm_cause = NAS_ESM_INVALID_MANDATORY_INFORMATION;
      ret_val =  FALSE;
    ...
    {
      DS_LTE_MSG1_ERROR_EX(subs_id, "APN, Insufficinet space to store incoming APN,Length",
                                              ...
                                              decoded_apn) == FALSE)
    {
      ...
      {
       // DS_LTE_MSG0_ERROR_EX(subs_id, "Decoding of apn name failed and disable_apn_matching"
       //                   " is not set");
        local_err = DS_3GPPI_LOCAL_ERR_VAL_2;
        lte_info_ptr->esm_cause.valid = TRUE;
        lte_info_ptr->esm_cause.esm_cause = NAS_ESM_INVALID_MANDATORY_INFORMATION;
  ...
  {
    DS_LTE_MSG1_ERROR_EX(subs_id, "ds_eps_pdn_cntx_validate_apn_name_and_decode, local error: d",
                      local_err);
  }
  ...
  if(msgrtype == NAS_EMM_IND_PLMN_CHANGE && dsmsg_ptr!= NULL)
  {
              ...
              sizeof(sys_plmn_id_s_type)) != 0)
    {
              ...
              sizeof(sys_plmn_id_s_type)) != 0)
      {
        ...
        DS_LTE_MSG0_MED_EX(subs_id,"Updated the current PLMN. Send current PLMN to APM module");
      }
...
)
{
  ...
  sys_modem_as_id_e_type          subs_id = SYS_MODEM_AS_ID_NONE;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  subs_id = (sys_modem_as_id_e_type)(int)client_data_ptr;
  if (!ds3gsubsmgr_is_subs_id_valid(subs_id))
  {
    return;
  }
...
)
{
  ...
  DS_LTE_MSG2_HIGH_EX(subs_id,"attach_complete_ind_hdlr: umid %d,subs_id %d", msgrtype,subs_id);
  ...
  if(msgrtype == NAS_EMM_ATTACH_COMPLETE_IND)
  {
   ...
   if(ds_3gpp_pdn_cntx_validate_pdn_context(pdn_cntx_p) == TRUE)
   {
    ...
    if(event_ret_val != 0)
    {
      DS_LTE_MSG0_ERROR_EX(subs_id, "Fail to post PS_SYS_EVENT_3GPP_LTE_ATTACH_COMPLETE");
    }
#endif /*FEATURE_EPC_HANDOFF*/
    }
    attach_sm_ptr = ds_eps_get_attach_sm_per_sub_id(subs_id);
    ...
    attach_sm_ptr->null_algo_used = payload_ptr->null_algo_used;
    ...
    ----------------------------------------------------------------------*/
    DS_LTE_MSG0_HIGH_EX(subs_id,"Attach complete. Reset Target_Rat");
    ds3gmgr_send_trat_ind(SYS_SYS_MODE_NONE,0,NULL,subs_id);

    ret_val = TRUE;

    /*----------------------------------------------------------------------- 
      Check IP type and if IP type is IPV4, then send a
      PS SYS LTE ATTACH PARAMS EX indication
      For IPV6 or V4V6, there is no need to send this indication, This will
      be taken care when V6 Addr indicatiion is received.
      -----------------------------------------------------------------------*/
    if (attach_comp_info.v6_iface_ptr == NULL)
    {
      ds_eps_pdn_cntx_send_lte_attach_param_ex_ind(subs_id,TRUE);
    }
    /*----------------------------------------------------------------------- 
     If we have reached the else case, it is V6 or V4V6 ip type,
     See if IPV6's iface is in UP state, we are safe to post an indication
     here
      -----------------------------------------------------------------------*/
    else
    {
       iface_state = ps_iface_state(v6_iface);
       if((iface_state == IFACE_UP) ||
          (iface_state == IFACE_ROUTEABLE))
       {
         DS_LTE_MSG1_HIGH_EX(subs_id,"V6 iface is %d Posting LTE attach param ex ind",
                          iface_state);
         ds_eps_pdn_cntx_send_lte_attach_param_ex_ind(subs_id,TRUE);
       }
    }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_LTE_MSG0_HIGH_EX(subs_id,"in ds_eps_pdn_cntxt_handle_cntxt_txfr_ind"
                     " GET_PDN_CONN_IND for LTE Attach is cached");
    return;
  }

  /* Set the transfer_in_progress flag*/
  memset(&apm_ind_payload, 0, sizeof(ds_dsd_apm_ind_pay_load));
  DS_3GPP_APM_SET_IS_TRANSFER_IN_PROGRESS_FLAG(apm_ind_payload, TRUE, subs_id);
  ds_dsd_apm_ind(DS_DSD_APM_TRANSFER_IND, &apm_ind_payload);

  if (DS_EPS_ATTACH_STATE_NULL == ds_eps_attach_sm_get_state(subs_id))
  {
    DS_LTE_MSG0_HIGH_EX(subs_id,"Posting EPS CNTXT TRANSFER ev to DSD");
...
} /* ds_eps_pdn_cntxt_handle_cntxt_txfr_ind */
...
)
{
  ...
  /*----------------------------------------------------------------------------
   ...
   if(ds_eps_attach_sm_get_state(subs_id) == DS_EPS_PROCESSED_CNTXT_TXFR_IND)
   {
     ...
     ds_3gpp_network_override_hdlr_reset_if_plmn_changed(subs_id,
                                                         current_plmn,
                                                         DS_3GPP_THROT_RAT_LTE,
                                                         FALSE);

     ds_3gpp_pdn_reset_max_simul_active_pdp_context_on_plmn_change(DS_3GPP_THROT_RAT_LTE,
                                                                  subs_id,
                                                                  current_plmn);
     /*----------------------------------------------------------------------------
     ...
     if(ds_3gpp_nv_manager_get_always_connect_def_pdn_flag(subs_id) == TRUE)
     {
       /*----------------------------------------------------------------------
         ...
         Ignore the return value of the function call.
       ----------------------------------------------------------------------*/
       DS_LTE_MSG0_HIGH_EX(subs_id,"Resume Ind - Bringing up def PDN conn if needed");
   ...
   {
     DS_LTE_MSG0_HIGH_EX(subs_id,"Resume Ind - attach SM isn't in PROCESSED_CNTXT_TRANSFER state - ignore");
   }
...
DESCRIPTION
  ...
  parameters EX.
  is_valid  -- Set to TRUE when LTE attach parameters are valid
  is_valid  -- Set to FALSE when LTE attach parameters become invalid
...
)
{
  ...
  int                                          api_ret_val     = -1;
 /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  attach_params_p = (ps_sys_ioctl_3gpp_lte_attach_params_ex_type *)
                     modem_mem_alloc(sizeof(ps_sys_ioctl_3gpp_lte_attach_params_ex_type),
                                     MODEM_MEM_CLIENT_DATA);

  if (attach_params_p == NULL)
  {
    DS_LTE_MSG0_ERROR_EX(subs_id, "Failure to send PS SYS ind, HEap allocation failure");
    return;
  }

  /*---------------------------------------------------------------------------- 
    When attach params are being reset(is_valid set to FALSE),
    It is sufficient to fill is_valid to FALSE,No other parameters need to be populated
   
    When attach params are being set(is_valid set to TRUE), there is a need to
    populate attach params 
    ---------------------------------------------------------------------------*/
  if (is_valid == FALSE)
  {
    attach_params_p->attach_params.is_valid = FALSE;
  ...
  {
    api_ret_val = ds_eps_pdn_cntxt_get_lte_attach_params_ex((void *)attach_params_p,
                                                             subs_id,
                                                             &err_no);

    if (api_ret_val == -1)
    {
      DS_LTE_MSG0_ERROR_EX(subs_id, "Failure to populate LTE attach params correctly");
    }


  }

  DS_LTE_MSG0_LOW("Posting LTE attach PARAMS EX event");

  event_ret_val = ps_sys_event_ind_ex(
                                       PS_SYS_TECH_3GPP,
                                       PS_SYS_EVENT_3GPP_LTE_ATTACH_PARAMS_EX,
                                       (ps_sys_subscription_enum_type)
                                         ds3gsubsmgr_subs_id_cm_to_ds(subs_id),
                                       (void*)attach_params_p,
                                       &ps_errno
                                     );
  ...
  modem_mem_free(attach_params_p,MODEM_MEM_CLIENT_DATA);
  return;
}/*ds_eps_pdn_cntx_send_lte_attach_param_ind*/
...
)
{
  ...
  /*-------------------------------------------------------------------------
  When emergency attach is initiated, DSD has been informed about it and the 
  LTE SM is in IN_SVC state. If the emergency attach fails, DSD needs to get 
  the DETACH event so that the LTE SM can be throttled and if the UE moves to 
  another RAT, it can be declared as the preferred RAT. In case of emergency 
  attach, the attach SM remains in NULL state. Thus, there is no NULL check 
  below so that the DETACH event goes out in emergency attach scenarios as well
  --------------------------------------------------------------------------*/
    ds3gdsdif_notify_dsd_lte_attach_event(
       subs_id,
    ...
    /*---------------------------------------------------------------------
      Notify APM module for LTE Detach Indication
    ---------------------------------------------------------------------*/
    apm_pay_load.subs_id = subs_id;
    apm_pay_load.ind_field_mask = 0;

    apm_error_code = ds_dsd_apm_ind(DS_DSD_APM_DETACH_IND,&apm_pay_load);
    DS_LTE_MSG1_MED_EX(subs_id,"Posted Detach Indication to DSD and APM, "
                    "Err code: %d", apm_error_code);

  /*------------------------------------------------------------------ 
    Send out an indication only when the attach sm transition happens
    from ATTACHED to NULL
    This is to prevent duplicate notifications when ATTACH is rejected
    -----------------------------------------------------------------*/
  if (ds_eps_attach_sm_get_state(subs_id) == DS_EPS_ATTACHED)
  {
    ds_eps_pdn_cntx_send_lte_attach_param_ex_ind(subs_id,FALSE);
  }
  ...
  ds_eps_pdn_cntxt_clear_active_scells(ds3gsubsmgr_subs_id_cm_to_ds3g(subs_id));
#ifdef FEATURE_DATA_4_5G
  ds_eps_pdn_cntxt_ex_clear_ca_info(ds3gsubsmgr_subs_id_cm_to_ds3g(subs_id));
#endif
  ...
  attach_sm_ptr->is_gw_originated = FALSE;
  attach_sm_ptr->responding_to_page = FALSE;
  attach_sm_ptr->null_algo_used = FALSE;
  memset((void*)&attach_sm_ptr->prev_attach_status,
         0,
         sizeof(ds_eps_prev_attach_status_type));
  ...
  /*-------------------------------------------------------------------- 
    We check here if an action confirmation response is pending for AP
    initiated action to set the lte_attach_pdn_list and provide response
   
    If attach SM is in NULL , send response
   
    Attach SM can be NULL in below 2 scenarios
    1. NV 74074 is 1 (Attach Pdn Disconnect Only) and this is a single PDN case,
    so after the only PDN goes down, Attach SM will be NULL, so we send response.
   
    2. NV 74074 is 0 (Detach), after the Last remaining PDN goes down,
    attach SM will be set to NULL here , so we send response
    -----------------------------------------------------------------*/
  ...
  ds_3gpp_pdn_cntxt_set_emergency_auth_fail(FALSE, subs_id);
  ...
  ds_eps_udt_mgr_reset_udt_info(subs_id);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_LTE_MSG2_HIGH_EX(subs_id,"PDN_CONN_REQ is pending, cached_seq_num:%d "
...
)
{
  ...
  if ( status != DS_UMTS_PDP_SUCCESS )
  {
    DS_LTE_MSG0_ERROR_EX(subs_id, "Unable to get attach profile context");
    return ret_val;
  }
                                           ...
                                           subs_id) == TRUE)
  {
    ...
    else
    {
      DATA_3GPP_MSG_SPRINTF_1(MSG_LEGACY_ERROR,
                              "Attach APN: %s is present in APN table but no "
                              "v4 or v6 pdn ptr is present",
                              attach_profile_context.apn);
      ret_val = FALSE;
    }
  }
...
/*-------------------------------------------------------------------------------*/
  ...
  {
    DS_LTE_MSG0_HIGH_EX(subs_id,"Can't find default socket profile, "
                      "No APN to attach");
    local_err = DS_3GPPI_LOCAL_ERR_VAL_1;
    goto func_exit;
  }

  DS_LTE_MSG1_HIGH_EX(subs_id,"Attach profile: %d", attach_prof);

func_exit:
  if (local_err != DS_3GPPI_LOCAL_ERR_VAL_NONE)
  {
    /*---------------------------------------------------------- 
      If we error out, always create and return profile 1. 
      ----------------------------------------------------------*/

    if(DS_UMTS_PDP_SUCCESS != ds_3gpp_profile_create_profile
                   (dsumts_subs_mgr_get_subs_id(subs_id),
                                                   TRUE,
                              DS_UMTS_DEF_SOCKET_PROFILE))
    {
      DS_LTE_MSG0_ERROR_EX(subs_id, "Unable to create default attach profile");
      DS3GPP_ASSERT(0);
      attach_prof = DS_EPS_INVALID_PROF;
    }
  }
...
/*--------------------------------------------------------------------------*/
  ...
  if(ip_support == NULL)
  {
    DS_LTE_MSG0_ERROR_EX(subs_id, "NULL ptr being passing for ip support");
    return;
  }
  ...
  DS_LTE_MSG2_HIGH_EX(subs_id,"Received invalid cause code for v4v6, is redial allowed %d, "
...
/*-------------------------------------------------------------------------*/
  ...
  if(!ds_3gpp_pdn_cntxt_get_subs_id_from_pdn(pdn_cntxt_p, &subs_id)||
     (ind_payload == NULL))
  {
    DS_LTE_MSG2_ERROR_EX(subs_id, " Pass in NULL Pdn Cntxt pointer 0xx ,ind_payload ptr"
                      " 0xx ",pdn_cntxt_p,ind_payload);
    return ;
  }
  ...
  /*-----------------------------------------------------------------------
    ...
    invoked
  -----------------------------------------------------------------------*/
  ds_3gpp_throt_fill_cb_data (&cb_data, NULL, NULL, (void*)pdn_cntxt_p);
  ...
  (
     &throt_ident_info, 
     pdn_cntxt_p->ds_pdn_context_dyn_p->pdp_profile.context.apn,
     subs_id,
     FALSE,
     rat_type,
     DS_3GPP_PDN_THROT_PROC_GLOBAL,
  ...
  switch(ip_support) 
  {
    case DS_IPV4_ONLY:
    {
      ...
      throt_ident_info.pdp_type = DS_UMTS_PDP_IPV6;
      throt_ident_info.cause_code = PS_NET_DOWN_REASON_IP_V4_ONLY_ALLOWED;
    ...
    case DS_IPV6_ONLY:
    {
      ...
      throt_ident_info.pdp_type = DS_UMTS_PDP_IPV4;
      throt_ident_info.cause_code = PS_NET_DOWN_REASON_IP_V6_ONLY_ALLOWED;
    ...
    default:
      DS_LTE_MSG0_ERROR_EX(subs_id, "Invalid combination , error Network sends IPV4V6 "
                        "with cause code after IPV4V6 is requested ");
      break;
  }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if (!retVal)
  {
    DS_LTE_MSG1_ERROR_EX(subs_id, "ds_bearer_cntxt_prepare_for_teardown returned: d",retVal);
    return;
  }
...
}/* ds_eps_apn_param_change_teardown_call */
 ...
 /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
   ...
   if (bearer_context_p->ds_bearer_context_dyn_p->is_default)
   {
     ...
     if ((pdn_pdp_type == DS_UMTS_PDP_IPV4) ||
         ((pdn_pdp_type == DS_UMTS_PDP_IPV4V6) && (ds3gpp_v6_iface_p == NULL)))
     {
       ds_eps_apn_param_change_teardown_call(bearer_context_p);

       if (retVal)
       {
         ds_3gpp_bearer_cntxt_set_bearer_pdn_pend_redial_state(bearer_context_p,
                                                               pdn_context_p);
       }      
     }
     /*----------------------------------------------------------------------------
      When APN Param change is triggered, we should wait for Net down event to be
      received for IPv6 before continuing with PDN Disconnect. This would allow 
      for graceful V6 prefix cleanup and avoid race condition.
     ------------------------------------------------------------------------------*/
     else 
     {
       if (!retVal)
...
)
{
 ...
 /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
        ...
        pdn_index++)
  {
    ...
    if(!ds_pdn_cntxt_validate_pdn_subscription_internal(pdn_cntxt_p, subs_id))
    {
      continue;
    }
    ...
    {
      DS_LTE_MSG1_HIGH_EX(subs_id,"APN Parameter change already in Progress; "
    ...
    {
      DS_LTE_MSG0_HIGH_EX(subs_id,"Emergency PDN, Ignore apn_param_chg");
      continue;
    }
    ...
    {
      DS_LTE_MSG2_HIGH_EX(subs_id,"PDN context with profile number:%d found, subs id: %d",
	  	               prof_num, subs_id);
...
)
{
  ...
  DS_LTE_MSG1_HIGH_EX(subs_id,"Reset Target Rat after Attach Failure, Network mode: %d",
                   network_mode);
...
)
{
  ...
  if((msgr_type != NAS_EMM_DEACT_NON_EMC_BEARER_IND) ||
     (dsmsg_ptr == NULL))
  {
    DS_LTE_MSG2_ERROR_EX(subs_id, "ds_eps_pdn_cntxt_deact_non_emc_bearer_ind_hdlr: Invalid "
  ...
  {
    DS_LTE_MSG0_HIGH_EX(subs_id,"Received T3420 from NAS");
    ds_3gpp_pdn_cntxt_set_emergency_auth_fail(TRUE, subs_id);
    ds_3gpp_pdn_cntxt_set_pending_teardown_flag();
    ds_3gpp_pdn_cntxt_check_and_teardown_non_emergency_pdns();
  }
  else
  {
    DS_LTE_MSG0_HIGH_EX(subs_id,"Ignore T3420 indication, no emergency call available");
  }
  return;
...
)
{
  ...
  ds3gsubsmgr_subs_id_e_type      ds3g_subs_id = DS3GSUBSMGR_SUBS_ID_INVALID;
  uint8                           scell_idx = 0;
  uint32                          scell_idx_mask = 0;
  uint32                          prev_scell_active_idx_mask = 0;
  uint32                          prev_scell_ul_active_idx_mask = 0;
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  {
    DS_LTE_MSG2_ERROR_EX(subs_id, "ds_eps_cphy_ca_event_hdlr: Invalid "
                      "message d or dsmsg_ptr d",
                      msgr_type, dsmsg_ptr);
    return FALSE;
  }
  ...
  /*-------------------------------------------------------------------
    ...
    MH checks to see if this scell was already activated/deactivated or
    not by checking against the cached bitmask. MH will only update
    DSD if this is a unique event for the specified scell (i.e. if scell
    #1 receives multiple activates or deactives, only take into account
    the first one).
  -------------------------------------------------------------------*/
  scell_idx = ds_lte_cphy_ca_event_ptr->scell_info.scell_idx;

  if (scell_idx < DS_EPS_SCELL_INDEX_MIN || scell_idx > DS_EPS_SCELL_INDEX_MAX)
  {
    DS_LTE_MSG1_ERROR_EX(subs_id, "Invalid scell index passed: d",scell_idx);
    return FALSE;
  }

  scell_idx_mask = (0x1 << scell_idx);
  ...
  {
    DS_LTE_MSG0_ERROR_EX(subs_id, "NULL LTE CA display threshold pointer");
    return FALSE;
  }
  ...
  switch ( ds_lte_cphy_ca_event_ptr->scell_info.scell_state )
  {
    ...
    default:
      DS_LTE_MSG1_MED_EX(subs_id,"Unhandled scell state received: %d",
  ...
  do
  {
    ...
    {
      DS_LTE_MSG2_MED_EX(subs_id,"UE category:%d < Threshold UE category. "
    ...
    /*---------------------------------------------------------------------
      ...
      NV aggregated DL threshold bandwidth threshold is in units of 1MHz,
      LTE_CPHY_CA_EVENT_IND provides total DL threshold bandwidth in 100kHZ
      Thus, we must divide LTE_CPHY_CA_EVENT_IND total configured bw by 10
      in order to convert the units to MHz.
    ---------------------------------------------------------------------*/
    aggr_dl_bw = ds_lte_cphy_ca_event_ptr->total_configured_bw/10;
    if( aggr_dl_bw < ca_disp_thresh_p->aggr_dl_bw )
    {
      DS_LTE_MSG2_MED_EX(subs_id,"Aggr DL BW:%d < Threshold Aggr DL BW. "
                        "Notifying CA unavailable to DSD",
  ...
  if( ca_available )
  {
    prev_scell_active_idx_mask = ds_eps_lte_active_scell_idx_mask[ds3g_subs_id];
    ds_eps_lte_active_scell_idx_mask[ds3g_subs_id] |= scell_idx_mask;

    if (ds_lte_cphy_ca_event_ptr->scell_info.ul_enabled == TRUE)
    {
      prev_scell_ul_active_idx_mask =
        ds_eps_lte_ul_active_scell_idx_mask[ds3g_subs_id];
      ds_eps_lte_ul_active_scell_idx_mask[ds3g_subs_id] |= scell_idx_mask;
    }

    /* Notify DSD of LTE CA acquired only in first instance of scell active */
    if (((prev_scell_active_idx_mask == 0) &&
         (ds_eps_lte_active_scell_idx_mask[ds3g_subs_id] > 0)) ||
        ((prev_scell_ul_active_idx_mask == 0) &&
         (ds_eps_lte_ul_active_scell_idx_mask[ds3g_subs_id] > 0)))
    {
      lte_ca_dl_available = TRUE;

      if (ds_eps_lte_ul_active_scell_idx_mask[ds3g_subs_id] > 0)
      {
        lte_ca_ul_available = TRUE;
      }
  ...
  {
    prev_scell_active_idx_mask =
      ds_eps_lte_active_scell_idx_mask[ds3g_subs_id];
    ...
    /*Force mask to 0 if num scell configured is 0*/
    if (ds_lte_cphy_ca_event_ptr->num_scell_configured == 0)
    {
      ds_eps_lte_active_scell_idx_mask[ds3g_subs_id] = 0;
      ds_eps_lte_ul_active_scell_idx_mask[ds3g_subs_id] = 0;
    }
        ...
        (ds_eps_lte_active_scell_idx_mask[ds3g_subs_id] == 0))
    {
      ...
      lte_ca_ul_available = FALSE;

      notify_dsd = TRUE;
    }
    /* Notify DSD of scell UL lost if all UL scells are deactivated */
    else
    {
      if ((prev_scell_ul_active_idx_mask > 0) &&
          (ds_eps_lte_ul_active_scell_idx_mask[ds3g_subs_id] == 0))
      {
        lte_ca_dl_available = TRUE;
        lte_ca_ul_available = FALSE;
...
#ifdef FEATURE_DATA_4_5G
  ret_val = ds_eps_pdn_cntxt_ex_process_num_configured_scells(ds_lte_cphy_ca_event_ptr,
                                                                    ds3g_subs_id);

  DS_LTE_MSG5_HIGH("Processing LTE_CPHY_CA_EVENT_IND on subs id (CM) %d. "
                   "Scell state: %d. Active scell mask: 0x%x. "
                   "Active UL scell mask 0x%x. Num configured scells: %d.",
                   subs_id,
                   ds_lte_cphy_ca_event_ptr->scell_info.scell_state,
                   ds_eps_lte_active_scell_idx_mask[ds3g_subs_id],
                   ds_eps_lte_ul_active_scell_idx_mask[ds3g_subs_id],
                   ds_eps_pdn_cntxt_ex_get_num_configured_scells(ds3g_subs_id));
#else
  DS_LTE_MSG4_HIGH("Processing LTE_CPHY_CA_EVENT_IND on subs id (CM) %d. "
                   "Scell state: %d. Active scell mask: 0x%x. "
                   "Active UL scell mask 0x%x. ",
                   subs_id,
                   ds_lte_cphy_ca_event_ptr->scell_info.scell_state,
                   ds_eps_lte_active_scell_idx_mask[ds3g_subs_id],
                   ds_eps_lte_ul_active_scell_idx_mask[ds3g_subs_id]);
  ...
  if (notify_dsd == TRUE || ret_val == TRUE)
  {
    DS_LTE_MSG3_HIGH_EX(subs_id,"Notify DSD of LTE CA availability on subs id %d (CM). "
                     "DL: %d UL: %d",
                     subs_id,
                     lte_ca_dl_available,
                     lte_ca_ul_available);
...
{
  lte_cphy_ca_event_ext_ind_s    *ds_lte_cphy_ca_event_ext_ptr = NULL;
  uint8                           scell_idx = 0;
  ps_sys_lte_cell_info_type       lte_cell_info;
  uint8                           i = 0;
  int16                           dss_errno = 0;
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -  - - - - - - - */

  if((msgr_type != LTE_CPHY_CA_EVENT_EXT_IND) || (dsmsg_ptr == NULL))
  {
    DS_LTE_MSG2_ERROR_EX(subs_id, "ds_eps_cphy_ca_event_ext_hdlr: Invalid "
                      "message d or dsmsg_ptr d",
                      msgr_type, dsmsg_ptr);
    return FALSE;
  }

  ds_lte_cphy_ca_event_ext_ptr = (lte_cphy_ca_event_ext_ind_s*)dsmsg_ptr;
  ...
  lte_cell_info.num_scell = ds_lte_cphy_ca_event_ext_ptr->num_scells;
  for ( i = 0; i < lte_cell_info.num_scell; i++)
  {
    scell_idx = ds_lte_cphy_ca_event_ext_ptr->scell_info[i].scell_idx;
    if (scell_idx < DS_EPS_SCELL_INDEX_MIN || scell_idx > DS_EPS_SCELL_INDEX_MAX)
    {
      DS_LTE_MSG1_ERROR_EX(subs_id, "Invalid scell index passed: d",scell_idx);
      return FALSE;
    ...
    /* Copy the scell info */
    lte_cell_info.scell_info[i].scell_state = (ps_sys_lte_scell_state_enum_type)
      ds_lte_cphy_ca_event_ext_ptr->scell_info[i].scell_state;
    lte_cell_info.scell_info[i].ul_enabled  = 
      ds_lte_cphy_ca_event_ext_ptr->scell_info[i].ul_enabled;
    lte_cell_info.scell_info[i].cell_info.dl_bandwidth = 
      (ps_sys_lte_cell_bandwidth_enum_type) 
       ds_lte_cphy_ca_event_ext_ptr->scell_info[i].dl_bandwidth;
    lte_cell_info.scell_info[i].cell_info.freq = 
      ds_lte_cphy_ca_event_ext_ptr->scell_info[i].freq;
    lte_cell_info.scell_info[i].cell_info.pci =
      ds_lte_cphy_ca_event_ext_ptr->scell_info[i].pci;
    lte_cell_info.scell_info[i].cell_info.serving_cell_id = 
      ds_lte_cphy_ca_event_ext_ptr->scell_info[i].scell_idx;
    lte_cell_info.scell_info[i].cell_info.band_info = 
      ds_lte_cphy_ca_event_ext_ptr->scell_info[i].sys_band;
  }

  /* Copy the pcell info */
  lte_cell_info.pcell_info.dl_bandwidth = (ps_sys_lte_cell_bandwidth_enum_type)
    ds_lte_cphy_ca_event_ext_ptr->pcell_info.dl_bandwidth;
  lte_cell_info.pcell_info.freq = 
    ds_lte_cphy_ca_event_ext_ptr->pcell_info.freq;
  lte_cell_info.pcell_info.pci = 
    ds_lte_cphy_ca_event_ext_ptr->pcell_info.pci;
  lte_cell_info.pcell_info.serving_cell_id = 
    ds_lte_cphy_ca_event_ext_ptr->pcell_info.scell_idx;
  lte_cell_info.pcell_info.band_info = 
    ds_lte_cphy_ca_event_ext_ptr->pcell_info.sys_band;

  ps_sys_conf_set_ex(PS_SYS_TECH_3GPP,
                     PS_SYS_CONF_3GPP_LTE_CELL_INFO,
                     (ps_sys_subscription_enum_type)
                     ds3gsubsmgr_subs_id_cm_to_ds(subs_id),
                     &lte_cell_info, &dss_errno);

  return TRUE;
} /* ds_eps_pdn_cntxt_cphy_ca_event_ext_hdlr */
...
{
  ds3gsubsmgr_subs_id_e_type ds3g_subs_id = DS3GSUBSMGR_SUBS_ID_INVALID;
  uint8                      num_of_active_scells = 0;
  uint32                     active_scell_mask = 0;
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  active_scell_mask = ds_eps_lte_active_scell_idx_mask[ds3g_subs_id];

  /* Get number of set bits, which corresponds to the number of active scells */
  while(active_scell_mask)
  {
    active_scell_mask &= (active_scell_mask-1) ;
    num_of_active_scells++;
  }

  return num_of_active_scells;

} /* ds_eps_pdn_cntxt_get_num_of_active_scells */
...
{
  ds3gsubsmgr_subs_id_e_type ds3g_subs_id = DS3GSUBSMGR_SUBS_ID_INVALID;
  uint8                      num_of_ul_active_scells = 0;
  uint32                     ul_active_scell_mask = 0;
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  ul_active_scell_mask = ds_eps_lte_ul_active_scell_idx_mask[ds3g_subs_id];

  /* Get number of set bits, which corresponds to the number of UL scells */
  while(ul_active_scell_mask)
  {
    ul_active_scell_mask &= (ul_active_scell_mask-1) ;
    num_of_ul_active_scells++;
  }

  return num_of_ul_active_scells;

} /* ds_eps_pdn_cntxt_get_num_of_ul_active_scells */
...
)
{
  ...
  ds_eps_lte_active_scell_idx_mask[ds3g_subs_id] = 0;
  ds_eps_lte_ul_active_scell_idx_mask[ds3g_subs_id] = 0;
}/* ds_eps_pdn_cntxt_clear_active_scells */
...
SIDE EFFECTS
  None.
===========================================================================*/
void ds_eps_pdn_cntxt_handle_lpm_mode()
{
  uint8 subs_index;
  ds_eps_attach_sm_s *attach_sm_ptr = NULL;
  ...
  {
    attach_sm_ptr = ds_eps_get_attach_sm_per_sub_id(subs_index);

    if (NULL != attach_sm_ptr)
    {
      /* Reset cached seq num in LPM mode */
      attach_sm_ptr->cached_seq_num = 0;
    }
  }
  /*----------------------------------------------------------------------
  Exit LTE Attach pending state, clean null bearer reason bit if it's set
  ----------------------------------------------------------------------*/
  ds_eps_pdn_cntxt_ex_reset_attach_pending(subs_index);
} /* ds_eps_pdn_cntxt_handle_lpm_mode */
...
)
{
 ...
 /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_LTE_MSG0_ERROR_EX(subs_id, "Input ptr is NULL");
    return ret_val;
  }
...
)
{
 ...
 /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  ds3gpp_enter_global_crit_section();
  /*---------------------------------------------------------------------------------
    Reset the resolved apn (when null apn tear down)
   ---------------------------------------------------------------------------------*/
  if(ds3geventmgr_set_event_info(DS3GEVENTMGR_NULL_RESOLVED_APN_EV,
                                 &resolved_apn_info, &filter_info)== FALSE)
  {
    DS_LTE_MSG0_ERROR_EX(subs_id,"Unable to provide Resolved APN info to internal"
                       " clients");
  }
  ...
  return ;
}/*ds_eps_pdn_cntxt_reset_resolved_apn*/
...
)
{
  ...
  do
  {
    ...
    if ((ntw_overridden_ip = ds_3gpp_network_override_hdlr_get_overridden_ip(
	  (byte *)pdn_cntxt_ptr->ds_pdn_context_dyn_p->pdp_profile.context.apn,
	   DS_3GPP_THROT_RAT_LTE, &current_plmn, subs_id)) 
	   != DS_3GPP_PDN_THROT_PDP_UNDEFINED)
    {
	  pdp_type = pdn_cntxt_ptr->ds_pdn_context_dyn_p->pdp_profile.context.pdp_type;
	  if(((ntw_overridden_ip == DS_3GPP_PDN_THROT_PDP_IPV4)&&
		 (pdp_type != DS_UMTS_PDP_IPV4))||
		 ((ntw_overridden_ip == DS_3GPP_PDN_THROT_PDP_IPV6)&&
    ...
    {
      DS_LTE_MSG0_ERROR_EX(subs_id, "Memory allocation failure, fail silent redial");
      break;
    }
    ...
    {
      DS_LTE_MSG1_ERROR_EX(subs_id, "Invalid Profile: d in PDN Context",
                         pdn_cntxt_ptr->ds_pdn_context_dyn_p->pdp_profile_num);
      break;
    }
    ...
    /*-------------------------------------------------------------------------
      Silent Redial will be allowed only when the pdp_type of the current call
      matches that of the pdp_type_to_use after 
  ...
  if (NULL != (temp_pdn_cntxt.ds_pdn_context_dyn_p))
  {
    DS_LTE_MSG4_HIGH_EX(subs_id,"PDP type to use: %d, v4_iface in _use: %d v6_iface_in_use: %d"
		     " Redial_allowed: %d",
...
)
{
  ...
  do
  {
    ...
    ds_3gpp_pdn_cntx_get_current_plmn_ex(&plmn_blocking_info.plmn,subs_id);
    ...
    fn_call_ret_val = ds3g_msh_disable_lte_per_plmn_ex(plmn_blocking_info);
  ...
  DS_LTE_MSG1_HIGH_EX(subs_id,"ds_eps_pdn_cntxt_send_detach_req:%d",dbg_val);
}/*ds_eps_pdn_cntxt_send_detach_req*/
...
{
  emm_tau_complete_ind_type                *payload_ptr = NULL;
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

  if(msgrtype == NAS_EMM_TAU_COMPLETE_IND)
  {
    if (dsmsg_ptr == NULL)
    {
      DS_LTE_MSG0_LOW("Invalid ds msgr ptr");
    ...
    payload_ptr = (emm_tau_complete_ind_type*)dsmsg_ptr;

    if((ds_3gpp_throt_sm_is_cc_sp_throttling_enabled(subs_id) == TRUE) &&
       (ds_3gpp_pdn_cntxt_get_any_active_pdn(subs_id) != NULL) &&
       (payload_ptr->tau_type != NAS_3GPP_PERIODIC_TYPE))
    {
      DS_LTE_MSG0_HIGH_EX(subs_id,"Non periodic TAU. Clear throttling.");
      ds_3gpp_cc_sp_reset_throttling(subs_id);                  
    }
    return TRUE;
  }
  return FALSE;
} /* ds_eps_pdn_cntx_tau_complete_ind_hdlr */
