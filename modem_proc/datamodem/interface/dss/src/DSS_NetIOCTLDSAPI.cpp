...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/dss/src/DSS_NetIOCTLDSAPI.cpp#3 $
  ...
  2015-05-21 pg  Added IOCTL support for PDN detach.
...
#define DSS_IOCTL_ASSERT(expression) \
         dss_ioctl_assert_wrapper(__LINE__, expression)
...
===========================================================================*/
static void dss_ioctl_assert_wrapper
(
...
static sint15 dss_iface_ioctl_get_path_mtu
(
   void *argval_ptr,
   DSSNetMonitored* pNetMonitored,
   sint15  *dss_errno
)
{
   ...
   dss_iface_ioctl_mtu_type* pMtu = reinterpret_cast<dss_iface_ioctl_mtu_type*>(argval_ptr);
   IDS_ERR_RET_ERRNO(pNetMonitored->GetPathMtu(pMtu));
   return DSS_SUCCESS;
}
...
)
{
   ...
   case DSS_IFACE_IOCTL_DPD_PING_INFO_EV:
      break;
   case DSS_IFACE_IOCTL_RAN_ASST_MEDIA_ADAPTATION_EV:
   case DSS_IFACE_IOCTL_PACKET_DISCARD_EV:
      break;
...
{
  LOG_MSG_INFO3_1("dss_iface_ioctl_reg_event_cb(): event: %d",
...
static sint15 dss_iface_ioctl_get_msisdn_info
(
 void* argval_ptr,
 DSSNetMonitored* pNetMonitored,
 sint15* dss_errno
)
{
  ...
  IDS_ERR_RET_ERRNO(pNetMonitored->GetMSISDN(reinterpret_cast<dss_iface_msisdn_info_type*>(argval_ptr)));
  return DSS_SUCCESS;
}


static sint15 dss_iface_ioctl_get_op_res_pco_list
(
 void* argval_ptr,
 DSSNetMonitored* pNetMonitored,
 sint15* dss_errno
 )
{
  ...
  IDS_ERR_RET_ERRNO(pNetMonitored->GetOpResPcoList(reinterpret_cast<dss_iface_op_reserved_pco_list_type*>(argval_ptr)));
  return DSS_SUCCESS;
}
...
static sint15 dss_iface_ioctl_start_dpd_ping
(
  void* argval_ptr,
  DSSNetMonitored* pNetMonitored,
  sint15* dss_errno
)
{
  if (NULL == argval_ptr)
   ...
   IDS_ERR_RET_ERRNO(pNetMonitored->StartDpdPing(reinterpret_cast<dss_iface_ioctl_dpd_ping_config_type *>(argval_ptr)));
   return DSS_SUCCESS;
}
...
)
{
   ...
   ds::ErrorType nRes2 = pNetQoSManagerPriv->RequestNetInitiatedReq(&requestedQoSSpec,
                                                                    &pNetQoSInitiatedReq,
                                                                    net_initiated_qos_status_request->fltr_match_dir);
   ...
   IDS_ERR_RET_ERRNO(DSSGlobals::Instance()->GetFreeQoSNetInitiatedReqHandle(&(net_initiated_qos_status_request->req_handle)));

   LOG_MSG_INFO1_5("dss_iface_ioctl_qos_net_initiated_request(): "
                   "iface_id:%u, argval_ptr:0x%p, errno: 0x%x, NetActive:0x%x Received qos net initiated request handle 0x%x",
                   iface_id, argval_ptr, dss_errno, pNetActive, net_initiated_qos_status_request->req_handle);
   ...
   ds::ErrorType nRes3 = pNetActive->AddQoSNetInitiatedReq(
      ...
      net_initiated_qos_status_request->req_handle,    // DSS identifer of the request
   ...
   if (AEE_SUCCESS != nRes3){
      DSSGlobals::Instance()->ReleaseQoSNetInitiatedReqHandle(net_initiated_qos_status_request->req_handle); 
      *dss_errno = DSSConversion::IDS2DSErrorCode(nRes3);
      return DSS_ERROR;
   }

   LOG_MSG_FUNCTION_EXIT_2("dss_iface_ioctl_qos_net_initiated_request(): iface_id:%u, "
                           "qos net initiated request handle 0x%x",
                           iface_id,net_initiated_qos_status_request->req_handle);
...
/*
Function is called when DSS ioctl DSS_IFACE_IOCTL_RAN_ASST_MEDIA_ADAPT_REG is 
processed.
To do:
1. sanity check (null ptr, etc)
2. Convert DS filter spec to IDS/ds_net spec -> using dss_conversion and store the spec
3. assign handle 
4. store to DSS_NetActive list
5. find matching filter from network to get a phys link ptr
    5.1 if a phys link ptr is found, call into MH to retrieve the bit rate info
    5.2. if bit rate info is returned, generate RAN asst event
*/

static sint15 dss_iface_ioctl_ran_asst_media_adapt_reg
(
   void*                                argval_ptr,
   DSSNetActive*                        pNetActive,
   sint15*                              dss_errno, 
...
)
{
  ...
  IRanAsstMediaAdaptReqType               *pNetRanAsstReq   = NULL;
  ...
  dss_iface_ioctl_ran_asst_media_adapt_reg_type* ranAsstRegRequest =
      (dss_iface_ioctl_ran_asst_media_adapt_reg_type*)argval_ptr;
  /*-----------------------------------------------------------------------------------
    * Null check
   ------------------------------------------------------------------------------------*/
  if ( NULL == dss_errno || NULL == ranAsstRegRequest||
       NULL == ranAsstRegRequest->cback_fn) {
     LOG_MSG_ERROR_2 ("dss_iface_ioctl_ran_asst_media_adapt_reg(): "
                      "dss error no 0x%p app ran asst reg ptr 0x%p", 
                      dss_errno, ranAsstRegRequest);
     if(NULL != dss_errno)
     {
       *dss_errno = DS_EFAULT;
     }
  ...
  memset (&bitRateInfo, 0, sizeof(bitRateInfo));
     
  /*-----------------------------------------------------------------------------------
    * Get  RanAsstMediaAdapt Manger class object
   ------------------------------------------------------------------------------------*/
  IDS_ERR_RET(pNetActive->GetNetRanAsstManager(&pRanAsstManager));
  DSSGenScope scopeRanAsstMgr(pRanAsstManager,DSSGenScope::IDSIQI_TYPE);

  /*-----------------------------------------------------------------------------------
    * Enter into critical section, it will be automatically released once we left this function
   ------------------------------------------------------------------------------------*/
  pRanAsstManager->GetCritsect(&pCritSect);
  DSSCritScope cs(*pCritSect);
  
  /*-----------------------------------------------------------------------------------
    * Get the NetworkFactory from DSSGlobals for Filter Spec creation 
   ------------------------------------------------------------------------------------*/
...
/*-----------------------------------------------------------------------------------
  * Pass over the filters and convert them to the new API filters in order to set the filters on the corresponding IDSNetwork object.
 ------------------------------------------------------------------------------------*/
  IDS_ERR_RET_ERRNO(DSSConversion::DS2IDSIPFilter(&(ranAsstRegRequest->ip_spec), pIIPFilterPriv));

  /*-----------------------------------------------------------------------------------
    * Create Net RAMA request object (It will be set into DSS RAMA request)
   ------------------------------------------------------------------------------------*/
  IDS_ERR_RET_ERRNO(pRanAsstManager->CreateMediaAdaptRanAsstReq(pIIPFilterPriv, &pNetRanAsstReq));
  DSSGenScope scopeRanAsstReq(pNetRanAsstReq,DSSGenScope::IDSIQI_TYPE);

  /*-----------------------------------------------------------------------------------
    * Add into DSS NetActive ( It will create internal DSS RAMA request object and store DS Net RAMA request) 
   ------------------------------------------------------------------------------------*/
  result = pNetActive->AddRanAsstMediaAdaptReq(
                                                pNetRanAsstReq,                
                                                ranAsstRegRequest->cback_fn,    
                                                ranAsstRegRequest->user_data,
                                                iface_id);
  ...
  /*-----------------------------------------------------------------------------------
    * Update Ran Asst Media Adapt handle into Application ioctl arg ptr
   ------------------------------------------------------------------------------------*/
  pNetRanAsstReq->GetRanAsstMediaAdaptHandle( &handle );

  ranAsstRegRequest->ran_asst_ma_handle = static_cast <sint15>(handle);
  /*-----------------------------------------------------------------------------------
      *Get Real Time Bit rate (Check if Bit rate is already present).  Notify to Applicaition if bit rate is present
  ------------------------------------------------------------------------------------*/ 
  if ( AEE_SUCCESS ==  pRanAsstManager->GetRealTimeBitRate(pNetRanAsstReq, &bitRateInfo))
  {
    memset(&event_info, 0x0, sizeof(event_info));

    event_info.ran_asst_media_adapt_info.ran_asst_handle 
      = ranAsstRegRequest->ran_asst_ma_handle;
    event_info.ran_asst_media_adapt_info.bit_rate_info.bit_rate 
      = bitRateInfo.bit_rate_info.bit_rate;
    event_info.ran_asst_media_adapt_info.bit_rate_info.direction 
      = bitRateInfo.bit_rate_info.direction;
    event_info.ran_asst_media_adapt_info.is_rohc_enabled        
      = bitRateInfo.is_rohc_enabled;

    ranAsstRegRequest->cback_fn( DSS_IFACE_IOCTL_RAN_ASST_MEDIA_ADAPTATION_EV,
                                    event_info,
                                    ranAsstRegRequest->user_data,
                                    ranAsstRegRequest->app_id,
                                    ranAsstRegRequest->iface_id);
  }

  LOG_MSG_FUNCTION_EXIT_2("dss_iface_ioctl_ran_asst_media_adapt_reg(): iface_id:%u, "
                          "ran_asst_reg_request->ran_asst_ma_handle 0x%x ",
                          iface_id,ranAsstRegRequest->ran_asst_ma_handle);
...
/*
this funtion deregisters the RAN assisted media adaptation filters installed 
using dss_iface_ioctl_ran_asst_media_adapt_reg.
1. use handle provided to delete the filter req from mpDSSRanAsstReqList
*/
static sint15 dss_iface_ioctl_ran_asst_media_adapt_dereg
(
   void *             argval_ptr,
   DSSNetActive*      pNetActive,
   sint15 *           dss_errno,
...
)
{
  ...
  dss_iface_ioctl_ran_asst_media_adapt_dereg_type* ranAsstDeregInfo =
      (dss_iface_ioctl_ran_asst_media_adapt_dereg_type*)argval_ptr;   

   /*-----------------------------------------------------------------------------------
     * NULL Check
    ------------------------------------------------------------------------------------*/
   if(argval_ptr == NULL || NULL == dss_errno){
     return DSS_ERROR;
   }
   ...
   /*-----------------------------------------------------------------------------------
    ...
    ------------------------------------------------------------------------------------*/
   pRanAsstManager->GetCritsect(&pCritSect);
   //DS_NULL_CHECK_RETURN(pCritSect); todo for shannon
   DSSCritScope cs(*pCritSect);

   /*-----------------------------------------------------------------------------------
     * Get DS NET Rama req object corresponding to handle
    ------------------------------------------------------------------------------------*/
   result = pNetActive->RemoveRanAsstMediaAdaptReq(ranAsstDeregInfo->ran_asst_ma_handle);
   ...
   LOG_MSG_FUNCTION_EXIT_3("dss_iface_ioctl_ran_asst_media_adapt_dereg(): iface_id:%u, "
                           "qos net initiated request handle 0x%x, ret %d",
                           iface_id,
                           (ranAsstDeregInfo != NULL ) ? ranAsstDeregInfo->ran_asst_ma_handle : 0,
                           result);
   return result;
}/* dss_iface_ioctl_ran_asst_media_adapt_dereg */


/*
this function triggers by DSS_IFACE_IOCTL_RAN_ASST_MEDIA_ADAPT_UPDATE.
1. query MH if modem supports update configuration
2. use the client requested ran asst ma handle to find a match in 
   mpDSSRanAsstReqList
2. if no match found, return DS_EBADAPP
3. if match, use the handle to fetch filter spec, map it to phys link, then 
call phys link ioctl
*/
static sint15 dss_iface_ioctl_ran_asst_media_adapt_update
(
   void *             argval_ptr,
   DSSNetActive*      pNetActive,
   sint15 *           dss_errno,
...
)
{
  ...
  dss_iface_ioctl_ran_asst_media_adapt_update_type* ranAsstUpdateInfo =
      (dss_iface_ioctl_ran_asst_media_adapt_update_type*)argval_ptr;
  ...
  if(argval_ptr == NULL || NULL == dss_errno){
   ...
   /*-----------------------------------------------------------------------------------
     * Get DS NET Rama req object corresponding to handle
    ------------------------------------------------------------------------------------*/
   result = pNetActive->GetNetRanAsstMediaAdaptReq(&pNetRanAsstReq,                
                                                   ranAsstUpdateInfo->ran_asst_ma_handle);
   if (AEE_SUCCESS != result || NULL == pNetRanAsstReq){
      *dss_errno = DSSConversion::IDS2DSErrorCode(result);
      return DSS_ERROR;
   }
   /*-----------------------------------------------------------------------------------
     * Invoke RAMA Manager API to update the bit rate
    ------------------------------------------------------------------------------------*/
    result = pRanAsstManager->UpdateBitRateInfo(pNetRanAsstReq, &ranAsstUpdateInfo->bit_rate_info);
    
   return result;
   
}/* dss_iface_ioctl_ran_asst_media_adapt_update */
...
)
{
   ...
   IDS_ERR_RET_ERRNO(pIDSNetQoSSecondaryPriv->GetGrantedFlowSpecPriv(&rxQoSFlow, &txQoSFlow, &bearer_id));
...
)
{
   ...
   IDS_ERR_RET_ERRNO(pIDSNetQoSDefaultPriv->GetGrantedFlowSpecPriv(&rxQoSFlow, &txQoSFlow, &bearer_id));
   ...
   pPrimaryQoSGetFlowType->bearer_id = *bearer_id;
...
)
{
   ...
   LOG_MSG_FUNCTION_EXIT_1("dss_iface_ioctl_enable_firewall(): "
                           "Ioctl: Enable Firewall, Allow pkts: %d Success", pEnableFirewall->is_pkts_allowed);
   return DSS_SUCCESS;
}
...
)
{
   ...
   LOG_MSG_FUNCTION_EXIT_1("dss_iface_ioctl_delete_firewall_rule(): "
                           "Ioctl: Delete firewall rule, handle %d Success", delete_firewall_rule->handle);
   return DSS_SUCCESS;
}
...
)
{
   ...
   LOG_MSG_FUNCTION_EXIT_1("dss_iface_ioctl_get_firewall_rule(): "
                           "Ioctl: Get firewall rule, handle %d Success", get_firewall_rule->handle);
   return DSS_SUCCESS;
}
...
static sint15 dss_iface_ioctl_wlan_init_dpd_check
(
  DSSNetMonitored* pNetMonitored,
  sint15* dss_errno
)
{
  if ((NULL == dss_errno) || (NULL == pNetMonitored))
  {
    return DSS_ERROR;
  }
  IDS_ERR_RET_ERRNO(pNetMonitored->InitDPDcheck());
  return DSS_SUCCESS;
}
...
)
{
   ...
   *dss_errno = DSS_SUCCESS;
   ...
   switch (ioctl_name) {
     ...
     case DSS_IFACE_IOCTL_WLAN_START_DPD_PING:
     {
        if(NULL == argval_ptr) {
           *dss_errno = DS_EFAULT;
           return DSS_ERROR;
   ...
   switch (ioctl_name) {
      ...
      case DSS_IFACE_IOCTL_GET_PATH_MTU:
         return dss_iface_ioctl_get_path_mtu(argval_ptr, netMonitoredScope.Fetch(), dss_errno);
      ...
      case DSS_IFACE_IOCTL_GET_MSISDN_INFO:
        if (NULL == netActiveScope.Fetch()) {
          dss_error_code = DSS_ERROR_IOCTL_FAILED_NO_ACTIVE_NETWORK;
          *dss_errno     = DS_ENETNONET;
          goto bail;
        }
        return dss_iface_ioctl_get_msisdn_info(argval_ptr, netMonitoredScope.Fetch(), dss_errno);

      case DSS_IFACE_IOCTL_GET_OP_RES_PCO_LIST:
        if (NULL == netActiveScope.Fetch()) {
          dss_error_code = DSS_ERROR_IOCTL_FAILED_NO_ACTIVE_NETWORK;
          *dss_errno     = DS_ENETNONET;
          goto bail;
        }
        return dss_iface_ioctl_get_op_res_pco_list(argval_ptr, netMonitoredScope.Fetch(), dss_errno);
     ...
     case DSS_IFACE_IOCTL_SET_SILENT_REDIAL:
      ...
      case DSS_IFACE_IOCTL_WLAN_START_DPD_PING:
        if (NULL == netActiveScope.Fetch()) {
         ...
         return dss_iface_ioctl_start_dpd_ping(argval_ptr, netMonitoredScope.Fetch(), dss_errno);
      ...
      case DSS_IFACE_IOCTL_RAN_ASST_MEDIA_ADAPT_REG:
        return dss_iface_ioctl_ran_asst_media_adapt_reg(argval_ptr, netActiveScope.Fetch(), dss_errno, iface_id);
        
      case DSS_IFACE_IOCTL_RAN_ASST_MEDIA_ADAPT_DEREG:
        return dss_iface_ioctl_ran_asst_media_adapt_dereg(argval_ptr, netActiveScope.Fetch(), dss_errno, iface_id);
        
      case DSS_IFACE_IOCTL_RAN_ASST_MEDIA_ADAPT_UPDATE:
        return dss_iface_ioctl_ran_asst_media_adapt_update(argval_ptr, netActiveScope.Fetch(), dss_errno, iface_id);

      case DSS_IFACE_IOCTL_WLAN_INITIATE_DPD_CHECK:
        if (NULL == netActiveScope.Fetch()) 
        {
          dss_error_code = DSS_ERROR_IOCTL_FAILED_NO_ACTIVE_NETWORK;
          *dss_errno     = DS_ENETNONET;
        ...
        return dss_iface_ioctl_wlan_init_dpd_check(netMonitoredScope.Fetch(), dss_errno);
