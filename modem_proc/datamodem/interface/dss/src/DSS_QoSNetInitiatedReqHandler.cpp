...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/dss/src/DSS_QoSNetInitiatedReqHandler.cpp#2 $
...
void DSSQoSNetInitiatedReqHandler::EventOccurred()
{
   ...
   if (mpHandlerDataClone->mpEventData->bReg) {
      ...
      while (pIQoSNetInitiatedReq->IsPendingMatchingFlow()) {
         ...
         mpParent->GetHandle(
           &eventInfo.qos_net_initiated_available_info.req_handle);
         ...
         if (AEE_SUCCESS != res) {
            dss_error_code = DSS_ERROR_GET_FREEQOSFLOWID_FAILED;
            goto bail;
         }

         qos_handle = iface_id | flowID;
         eventInfo.qos_net_initiated_available_info.qos_inst_handle = qos_handle;

         // Create a new DSS object representing the new Network Initiated QoS
         res = mpParent->CreateDSSQoSNetInitiated(eventInfo.qos_net_initiated_available_info.qos_inst_handle,
                                                  &eventInfo.qos_net_initiated_available_info.fltr_match_dir);
         if (AEE_SUCCESS != res) {
            if (AEE_EALREADY == res) {
               dss_error_code = DSS_ERROR_CREATE_DSS_QOSNETINITIATED_FAILED;
               DSSGlobals::Instance()->ReleaseQoSFlowID((uint8)(flowID));
               goto bail;
           }
           else {
              dss_error_code = DSS_ERROR_CREATE_DSS_QOSNETINITIATED_FAILED;
              DSSGlobals::Instance()->ReleaseQoSNetInitiatedReqHandle(eventInfo.qos_net_initiated_available_info.req_handle);
              DSSGlobals::Instance()->ReleaseQoSFlowID((uint8)(flowID));
              goto bail;
            }
         }

         LOG_MSG_INFO1_4("DSSQoSNetInitiatedReqHandler::EventOccurred(): "
                         "Dispatching DSS_IFACE_IOCTL_QOS_NET_INITIATED_AVAILABLE_EV (event %d), "
                         "req handle 0x%x, qos handle 0x%x, dir %d",
                         DSS_IFACE_IOCTL_QOS_NET_INITIATED_AVAILABLE_EV,
                         eventInfo.qos_net_initiated_available_info.req_handle,
                         eventInfo.qos_net_initiated_available_info.qos_inst_handle,
                         eventInfo.qos_net_initiated_available_info.fltr_match_dir);
      ...
      bail:
      LOG_MSG_ERROR_2("DSSQoSNetInitiatedReqHandler::EventOccurred(): "
                      "failed: Req Object: 0x%p, error code %d",
                      mpParent, dss_error_code);
      return;
   }
}
