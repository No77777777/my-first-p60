...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/dss/src/DSS_Conversion.cpp#3 $
...
#define DSS_CONVERSION_ASSERT(expression) \
         dss_conversion_assert_wrapper(__LINE__, expression)
...
===========================================================================*/
static void dss_conversion_assert_wrapper
(
...
AEEResult DSSConversion::DS2IDSNetPolicy(const dss_net_policy_info_type *from, IPolicyPriv *to)
{
   ...
   */
   ...
   IDS_ERR_RET(to->SetBringUpApnTypeFlag(from->bring_up_using_apn_type));
   IDS_ERR_RET(to->SetBringUpApnNameFlag(from->bring_up_using_apn_name));
   ...
   switch (DSOptName) {
      ...
      case DSS_TCP_MAX_BACKOFF_TIME:
          *OptName = ds::Sock::OptName::QDS_TCP_MAX_BACKOFF_TIME;
          break;
      case DSS_IP_RECVTTL:
          *OptName = ds::Sock::OptName::QDS_IP_RECVTTL;
          break;
      case DSS_IP_RECVECN:
          *OptName = ds::Sock::OptName::QDS_IP_RECVECN;
          break;
      case DSS_UDP_ENCAPS:
          *OptName = ds::Sock::OptName::QDS_UDP_ENCAPS;
          break;
      case DSS_PORT_PREF_HANDLE:
          *OptName = ds::Sock::OptName::QDS_SO_PORT_PREF_HANDLE;
          break;
      case DSS_PORT_IWLAN_CAPABLE:
          *OptName = ds::Sock::OptName::QDS_SO_PORT_IWLAN_CAPABLE;
          break;
      case DSS_TCP_ACCESS_PRIORITY:
           *OptName = ds::Sock::OptName::QDS_TCP_ACCESS_PRIORITY;
          break;
      case DSS_IP_RECVSUBID:
          *OptName = ds::Sock::OptName::QDS_IP_RECVSUBID;
          break;
...
AEEResult DSSConversion::IDS2DSMSISDN(ds::Net::MSISDNType* pMSISDN,
                                        dss_iface_msisdn_info_type* pGetMSISDN)
{
  if (NULL != pMSISDN)
  {
    pGetMSISDN->msisdn_length = pMSISDN->msisdn_length;
    (void)memscpy (&(pGetMSISDN->msisdn), 255, &(pMSISDN->msisdn), 255);
  }
  else
  {
    pGetMSISDN->msisdn_length = 0;
  }
  return AEE_SUCCESS;
}

AEEResult DSSConversion::IDS2DSOpResPcoList(ds::Net::NetworkOpReservedPcoListType* popReservedPcoList,
                                        dss_iface_op_reserved_pco_list_type* pGetOpResPcoList)
{
  for (int cnt_pcos = 0; cnt_pcos < PS_IFACE_MAX_NUM_PCO; cnt_pcos++)
  {
    pGetOpResPcoList->pco_info[cnt_pcos].mcc = popReservedPcoList->pco_info[cnt_pcos].mcc;
    pGetOpResPcoList->pco_info[cnt_pcos].mnc = popReservedPcoList->pco_info[cnt_pcos].mnc;
    pGetOpResPcoList->pco_info[cnt_pcos].mnc_includes_pcs_digit = popReservedPcoList->pco_info[cnt_pcos].mnc_includes_pcs_digit;
    pGetOpResPcoList->pco_info[cnt_pcos].app_specific_info_len = popReservedPcoList->pco_info[cnt_pcos].app_specific_info_len;
    pGetOpResPcoList->pco_info[cnt_pcos].container_id = popReservedPcoList->pco_info[cnt_pcos].container_id;
    (void)memscpy (&(pGetOpResPcoList->pco_info[cnt_pcos].app_specific_info), 255,
      &(popReservedPcoList->pco_info[cnt_pcos].app_specific_info), 255);
    pGetOpResPcoList->pco_info[cnt_pcos].pco_id = 
      (ps_iface_op_reserved_pco_id_enum_type)popReservedPcoList->pco_info[cnt_pcos].pco_id;
  }
...
AEEResult DSSConversion::DS2IDSDpdPingConfig(dss_iface_ioctl_dpd_ping_config_type* pDpdConfig,
                                           ds::Net::dpdPingConfigType* pIDSDpdConfig)
{
  pIDSDpdConfig->num_pings = pDpdConfig->num_pings;
  pIDSDpdConfig->frequency = pDpdConfig->frequency;
  pIDSDpdConfig->set_ka_guard = pDpdConfig->set_ka_guard;
  pIDSDpdConfig->max_wait_time = pDpdConfig->max_wait_time;
  pIDSDpdConfig->user_data_ptr = pDpdConfig->user_data_ptr;
  return AEE_SUCCESS;
}
                                       ...
                                       QoSModifyMaskType *modifyMask)
{
   ...
   dss::Error::dss_error_enum_type  dss_error_code = DSS_ERROR_MIN;
      ...
      if (!bIgnoreFilters)
      {
         ...
         if (0 != specType->txFilterLen) {
            if (NULL == specType->txFilter) {
               dss_error_code = DSS_ERROR_TX_FILTER_ALLOC_FAILED;
               goto bail;
            }
         }
   ...
   if( ((qos_spec_field_mask_type)QOS_MODIFY_MASK_RX_FLTR_MODIFY & reqType->qos.field_mask) &&
      ...
      if (0 != specType->rxFilterLen) {
         if (NULL == specType->rxFilter) {
            dss_error_code = DSS_ERROR_RX_FILTER_ALLOC_FAILED;
            goto bail;
         }
      }
...
bail:
   ...
   LOG_MSG_ERROR_2 ("DSSConversion::DS2IDSQoSSpec(): "
                    "res %d, error code %d", nRet, dss_error_code);
...
bail:
   ...
   LOG_MSG_ERROR_2 ("DSSConversion::DS2IDSQoSNetInitiatedSpec(): "
                    "res %d, error code %d", nRet, dss_error_code);
...
AEEResult DSSConversion::DS2IDSIPFilter(ip_filter_type* ipFilterType, IIPFilterPriv* NetIPFilter)
{
      ...
      switch( next_hdr_prot ){
         ...
         case PS_IPPROTO_ICMP6:
            ...
            if ((ipfltr_icmp_hdr_field_mask_type)IPFLTR_MASK_ICMP_MSG_ID & ipFilterType->next_prot_hdr.icmp.field_mask) {
               (void) pIDSNetIPFilterPriv->SetICMPId(ipFilterType->next_prot_hdr.icmp.id);
            }
   ...
   if (IP_V4 == ipFilterType->ip_vsn) {
      ...
      if (0 == ipFilterType->ip_hdr.v4.field_mask)
      {
         ...
         abort_qos_request = true;
         dss_error_code = DSS_ERROR_IPV4_FIELD_MASK_IS_EMPTY;
         goto bail;
      }
      ...
      if (ipFilterType->ip_hdr.v4.field_mask & ~(ipfltr_ip4_hdr_field_mask_type)IPFLTR_MASK_IP4_ALL)
      {
         ...
         abort_qos_request = true;
         dss_error_code = DSS_ERROR_IPV4_FIELD_MASK_IS_INVALID;
         goto bail;
      }
   ...
   if (IP_V6 == ipFilterType->ip_vsn) {
      ...
      if (0 == ipFilterType->ip_hdr.v6.field_mask)
      {
         ...
         abort_qos_request = true;
         dss_error_code = DSS_ERROR_IPV6_FIELD_MASK_IS_EMPTY;
         goto bail;
      }
      ...
      if (ipFilterType->ip_hdr.v6.field_mask & ~(ipfltr_ip6_hdr_field_mask_type)IPFLTR_MASK_IP6_ALL)
      {
         ...
         abort_qos_request = true;
         dss_error_code = DSS_ERROR_IPV6_FIELD_MASK_IS_INVALID;
         goto bail;
      }
...
bail :
   ...
   LOG_MSG_FUNCTION_EXIT_4("DSSConversion::DS2IDSIPFilter(): "
                           "abort_qos_request 0x%x, error code %d, "
                           "ipv4 field mask (%u), ipv6 field mask (%u)",
                           abort_qos_request,
                           dss_error_code,
