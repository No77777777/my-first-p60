...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/dss/src/DSS_Socket.cpp#2 $
...
#define DSS_SOCK_ASSERT(expression) \
         dss_sock_assert_wrapper(__LINE__, expression)
...
===========================================================================*/
static void dss_sock_assert_wrapper
(
   ...
   for (uint32 eventId = DS_WRITE_EVENT; eventId <= DS_ACCEPT_EVENT; eventId <<= 1) {
      if (0 != (eventId & eventMask)) {
         ...
         if (NULL == *ppiSignalCtl) {
            ...
            if (AEE_SUCCESS != res) {
               dss_error_code = DSS_ERROR_EVENT_REGISTRATION_FAILED;
               mSigOnOffMask &= ~eventId;
               goto bail;
            }
         }
...
bail:
  LOG_MSG_ERROR_2("DSSSocket::RegEvent(): "
...
bail:
   LOG_MSG_ERROR_2("DSSSocket::SetSdbAckCb "
                   "failed: result %d, error code %d", res, dss_error_code);
...
void DSSSocket::SdbAckSignalFcn()
{
   ...
   if (AEE_SUCCESS != res){
      dss_error_code = DSS_ERROR_GET_DOSACKINFO_FAILED;
      goto bail;
   }

   res = DSSConversion::IDS2DSSDBAckStatus(newAPIStatus, &oldAPIStatus);
   if (AEE_SUCCESS != res){
      dss_error_code = DSS_ERROR_CONVERT_ACKSTATUS_FAILED;
      goto bail;
   }
...
bail:
   LOG_MSG_ERROR_2("DSSSocket::SdbAckSignalFcn(): "
                   "failed: result %d, error code %d", res, dss_error_code);
   DSSCommon::ReleaseIf((IQI**)&pIDSExtSokect);
}
