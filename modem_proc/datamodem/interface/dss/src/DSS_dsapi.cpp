...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/dss/src/DSS_dsapi.cpp#3 $
...
#define DSS_DSAPI_ASSERT(expression) \
         dss_dsapi_assert_wrapper(__LINE__, expression)
...
===========================================================================*/
static void dss_dsapi_assert_wrapper
(
...
{
   LOG_MSG_FUNCTION_ENTRY_2("dss_open_netlib(): "
                            "net_callback_fcn 0x%p, socket_callback_fcn 0x%p",
                            net_callback_fcn, socket_callback_fcn);
...
)
{
   ...
   IDSNetPolicyScope.Fetch(), &pIDSNetwork, &netdownreason);
   if (AEE_SUCCESS != res) {
...
)
{
   ...
   LOG_MSG_FUNCTION_ENTRY_3("dss_socket(): "
                            "NetHandle: %d, family:%d, type:%d",
                            dss_nethandle, family, type);
...
bail:
   DSSCommon::ReleaseIf((IQI**)&pSockFactory);
   DSSCommon::ReleaseIf((IQI**)&pIDSSocket);
   LOG_MSG_FUNCTION_EXIT_4( "dss_socket2(): "
                            "socket 0x%x, family:%d, type:%d, protocol:%d", sRet , family, type, protocol);
   return sRet;
}
...
)
{
   ...
   LOG_MSG_FUNCTION_ENTRY_1("dss_bind(): socket:%d", sockfd);
...
)
{
   ...
   LOG_MSG_FUNCTION_ENTRY_1("dss_connect(): socket:%d", sockfd);
...
)
{
   ...
   LOG_MSG_FUNCTION_ENTRY_1("dss_listen(): socket:%d", sockfd);
...
)
{
   ...
   LOG_MSG_FUNCTION_ENTRY_1("dss_accept(): socket:%d", sockfd);
...
)
{
   ...
   LOG_MSG_FUNCTION_ENTRY_2("dss_async_select(): "
                            "socket:%d, bitmask:%d", sockfd, interest_mask);
...
)
{
   ...
   LOG_MSG_FUNCTION_ENTRY_2("dss_async_deselect(): "
                            "socket:%d, clear_bitmask:%d",
                            sockfd, clr_interest_mask);
...
)
{
   ...
   LOG_MSG_FUNCTION_ENTRY_2("dss_getnextevent(): "
                            "net_handle:%d, socket:%d",
                            dss_nethandle,
...
)
{
   ...
   LOG_MSG_INFO3_2("dss_read(): socket:%d, errno 0x%x", sockfd,dss_errno);
...
)
{
   ...
   LOG_MSG_INFO3_1("dss_readv(): socket:%d", sockfd);
...
)
{
   ...
   LOG_MSG_INFO3_2("dss_recvfrom(): "
                   "socket:%d, flags:%u", sockfd, flags);
...
bail:
   DSSCommon::ReleaseIf((IQI**)&pIDSSocket);
   LOG_MSG_FUNCTION_EXIT_5( "dss_recvfrom(): "
                            "socket %d, flags:%u, ret 0x%x, error %d, res %d", 
                            sockfd, flags, sRet,dss_error_code, res);
   return sRet;
}
...
)
{
   ...
   LOG_MSG_INFO3_1("dss_recvmsg(): socket:%d", sockfd);
   ...
   if (0 != nOutAncLen)
   {
      ...
      memset(pOutAncData, 0, sizeof(ds::Sock::IAncDataPriv *)*nOutAncLen);
   }
   ...
   *dss_errno = DSSConversion::IDS2DSErrorCode(res);
   ...
   res = pIDSSocket->GetOpt(/*in*/ ::ds::Sock::OptLevel::QDS_LEVEL_IP,
                            /*in*/ ::ds::Sock::OptName::QDS_IP_RECVTTL,
                            /*rout*/ &isRecvTTLSet);
   if (AEE_SUCCESS != res)
   {
      dss_error_code = DSS_ERROR_GET_OPT_OPERATION_FAILED;
   ...
   res = pIDSSocket->GetOpt(/*in*/ ::ds::Sock::OptLevel::QDS_LEVEL_IP,
                            /*in*/ ::ds::Sock::OptName::QDS_IP_RECVECN,
                            /*rout*/ &isRecvECNSet);
   if (AEE_SUCCESS != res)
   {
      dss_error_code = DSS_ERROR_GET_OPT_OPERATION_FAILED;
   ...
   res = pIDSSocket->GetOpt(/*in*/ ::ds::Sock::OptLevel::QDS_LEVEL_IP,
                            /*in*/ ::ds::Sock::OptName::QDS_IP_RECVSUBID,
                            /*rout*/ &isRecvSUBIDSet);
   if (AEE_SUCCESS != res)
   {
      dss_error_code = DSS_ERROR_GET_OPT_OPERATION_FAILED;
   ...
   for (i=0 , cmsg_ptr = DSS_CMSG_FIRSTHDR(msg) ; i < nOutAncLenReq; i++)
   {
      ...
      unsigned char RecvTTLHandle  = 0;
      unsigned char RecvECNHandle  = 0;
      unsigned int  RecvSubID  = 0;
      dss_in_ttlinfo_type  recv_ttlinfo;
      dss_in_ecninfo_type  recv_ecninfo;
      dss_in_recv_subs_id_type  recv_subs_id_info;

      memset(&err_info, 0, sizeof(dssocki_err_msg_in));
      memset(&recv_ttlinfo, 0, sizeof(dss_in_ttlinfo_type));
      memset(&recv_ecninfo, 0, sizeof(dss_in_ecninfo_type));
      memset(&recv_subs_id_info, 0, sizeof(dss_in_recv_subs_id_type));
      ...
      switch (AncID)
      {
         ...
         case ::ds::Sock::AncData::RECV_TTL_INFO:
         {
            ...
            {
              res = ((ds::Sock::IRecvTTLInfoPriv*)pOutAncData[i])->GetRecvTTLHandle(&RecvTTLHandle);
              if (AEE_SUCCESS != res) 
              {
                LOG_MSG_ERROR_1("dss_recvmsg(): "
              ...
              recv_ttlinfo.ttl = RecvTTLHandle;
         ...
         case ::ds::Sock::AncData::RECV_ECN_INFO:
         {
            ...
            {
              res = ((ds::Sock::IRecvECNInfoPriv*)pOutAncData[i])->GetRecvECNHandle(&RecvECNHandle);
              if (AEE_SUCCESS != res) 
              {
                LOG_MSG_ERROR_1("dss_recvmsg(): "
                             "GetRecvTTLHandle failed:%d", res);
              ...
              recv_ecninfo.ecn = RecvECNHandle;
         ...
         case ::ds::Sock::AncData::RECV_SUBID_INFO:
         {
            ...
            {
              res = ((ds::Sock::IRecvSUBIDInfoPriv*)pOutAncData[i])->GetRecvSubID(&RecvSubID);
              if (AEE_SUCCESS != res) 
              {
                LOG_MSG_ERROR_1("dss_recvmsg(): "
              ...
              recv_subs_id_info.subs_id = RecvSubID;
             ...
             // Write the ancillary data to the old msg structure
              tempRes =
                ds::Utils::Conversion::PutCMsg(msg,
                                            cmsg_ptr,
                                            (int16)PS_IPPROTO_IP,
                                            (int16)DSS_IP_RECVSUBID,
                                            sizeof(recv_subs_id_info),
                                            (void*)&recv_subs_id_info,
                                            &controllen );
...
)
{
   ...
   LOG_MSG_INFO3_2("dss_write(): socket:%d, buffer:0x%p",
                    sockfd, buffer);
...
bail:
   DSSCommon::ReleaseIf((IQI**)&pIDSSocket);
   LOG_MSG_FUNCTION_EXIT_4( "dss_write(): "
                            "socket %d,  buffer:0x%p, ret 0x%x, error %d", 
                            sockfd, buffer, sRet, dss_error_code);
   return sRet;
}
...
)
{
   ...
   LOG_MSG_INFO3_1("dss_writev(): socket:%d", sockfd);
...
)
{
   ...
   LOG_MSG_INFO3_2("dss_sendto(): "
                   "socket:%d, buffer:%d", sockfd, buffer);
...
bail:
   DSSCommon::ReleaseIf((IQI**)&pIDSSocket);
   LOG_MSG_FUNCTION_EXIT_5( "dss_sendto(): "
                            "socket %d, buffer:0x%p, ret = %d, error %d, res", 
                            sockfd, buffer, sRet,dss_error_code, res);
   return sRet;
}
...
)
{
   ...
   LOG_MSG_INFO3_3("dss_sendmsg(): "
                   "socket:%d, flags:%d, msg:0x%p",
                    sockfd, flags, msg);
...
bail:
   ...
   DSSCommon::ReleaseIf((IQI**)&pIDSsocketPriv);
   PS_SYSTEM_HEAP_MEM_FREE (bufs);
   LOG_MSG_FUNCTION_EXIT_6( "dss_sendmsg(): "
                            "socket %d, flags:%d, msg:0x%p, ret = %d, error %d, res %d",
                            sockfd, flags, msg, sRet, dss_error_code, res);
   return sRet;
}
...
)
{
   ...
   int nNumOfSocks = 0;
   sint15 sRet = DSS_SUCCESS;
   sint15 prev_dss_errno = DSS_SUCCESS;
   AEEResult res = AEE_SUCCESS;
   dss_error_enum_type dss_error_code = DSS_ERROR_MIN;
...
bail:
   ...
   DSSCommon::ReleaseIf((IQI**)&pIDSSocket);
   LOG_MSG_QTRACE_HFT_MED(PS_TAG_DSS, PS_SYS_DEFAULT_SUBS,
                          "dss_setsockopt(): socket %d, level %d, option_name %d ret = %d, error %d, res %d",
                          sockfd, level, optname, sRet, dss_error_code, res);
   return sRet;
}
...
)
{
  ...
  remoteSockManagerPtr = IManager::GetInstance();

  remoteSockManagerPtr->InitSocketsPref(port_pref);

  LOG_MSG_INFO1_4("dss_init_port_preference_info() Port Pref Initialized with "
                  " prop %d count %d base port %d max port %d",
                   port_pref->port_property ,
                   port_pref->pref_count ,
                   port_pref->base_port_range,
                   port_pref->max_port_range);

}/*dss_init_port_preference_info*/
...
)
{
  ...
  int32                    result = -1;
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if( (port_pref.base_port_range == 0 && port_pref.max_port_range == 0) ||
      (port_pref.base_port_range < port_pref.max_port_range))
  {
    remoteSockManagerPtr = IManager::GetInstance();
    result = remoteSockManagerPtr->RequestSocketsPref(&port_pref, cb_fn, userdata);
  }
  
  LOG_MSG_INFO1_5("dss_request_port_pref() Port Pref request with "
                " prop %d count %d base port %d max port %d res %d",
                 port_pref.port_property ,
                 port_pref.pref_count ,
                 port_pref.base_port_range,
                 port_pref.max_port_range,
...
)
{
  ...
  result = remoteSockManagerPtr->ReleaseSocketsPref(dss_request_handle,
             cb_fn, userdata);

  LOG_MSG_INFO1_2("dss_release_port_pref_handle() Release handle %d result %d",
                 dss_request_handle, result);

  return result;
}/*dss_release_port_pref_handle*/
