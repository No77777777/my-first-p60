...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/dss/src/DSS_MCastRegisterHandler_2_0.cpp#2 $
...
void DSSMCastRegisterHandler2_0::EventOccurred()
{
  dss_error_enum_type dss_error_code = DSS_ERROR_MIN;
   ...
   if (mpHandlerDataClone->mpEventData->bReg)
   {
      ...
      if (AEE_SUCCESS != res) {
         dss_error_code = DSS_ERROR_GET_MCASTSESSION_FAILED;
         goto bail;
      }

      res = piNetMCast->GetRegStateAndInfoCodeBCMCS2_0(&stateInfo);
      if (AEE_SUCCESS != res) {
         dss_error_code = DSS_ERROR_GET_REGSTATEANDINFOCODE_FAILED;
         goto bail;
      }
      ...
      bail:
      LOG_MSG_ERROR_2("DSSMCastRegisterHandler2_0::EventOccurred(): "
                      "MCastEvent info code was received: %d",
                      stateInfo.infoCode);
   }
}
