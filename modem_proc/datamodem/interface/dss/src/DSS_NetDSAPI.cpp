...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/dss/src/DSS_NetDSAPI.cpp#2 $
...
)
{
  ...
  LOG_MSG_FUNCTION_ENTRY_6("dss_reg_ip_filter_ex(): "
                           "socket %d, iface type %d, iface info id %u, "
                           "filter_type %d, filters 0x%p, num filters %d",
                           sockfd, iface.kind, iface.info.id, filter_type,
                           filters, num_filters);

  LOG_MSG_FUNCTION_ENTRY_2("dss_reg_ip_filter_ex(): "
                           "iface group 0x%x, dss_errno 0x%p",
                           iface.info.name, dss_errno);
...
)
{
  ...
  if (AEE_SUCCESS != res) {
   ...
   LOG_MSG_FUNCTION_EXIT_5 ("dss_dereg_ip_filter_by_handle(): "
                            "ret %d, res %d, dss_errno %d,"
                            "failed to remove filter %d",
                            ret, res, *dss_errno, filter_handle, failedToRmF);
   return ret;
}
...
)
{
   if(policy_info_ptr != NULL)
   {
      ...
      policy_info_ptr->bring_up_using_apn_name = FALSE;
      policy_info_ptr->bring_up_using_apn_type = FALSE;
...
)
{
   ...
   LOG_MSG_FUNCTION_ENTRY_1("dss_pppopen(): app_id %d", app_id);
 ...
 )
{
   ...
   LOG_MSG_FUNCTION_ENTRY_2("dss_last_netdownreason(): app_id %d, dss_errno 0x%p",
                            appid, dss_errno);
...
bail:
   ...
   LOG_MSG_FUNCTION_EXIT_4( "dss_get_iface_id(): "
                            "appId %d, ifaceId 0x%x, error %d, res %d",
                            appId, ifaceId, dss_error_code, res);
   return ifaceId;
}
...
{
   LOG_MSG_FUNCTION_ENTRY_1("dss_get_iface_id_by_qos_handle(): qos_handle %d",
                            handle);
...
{
   LOG_MSG_FUNCTION_ENTRY_1("dss_get_app_profile_id(): app_type %d", app_type);
...
{
   LOG_MSG_FUNCTION_ENTRY_1("dss_get_scope_id_by_iface_id(): iface_id 0x%x",
                            iface_id);
...
)
{
   ...
   LOG_MSG_FUNCTION_ENTRY_6("dss_reg_ip_filter(): socket %d, iface_id 0x%x, "
                            "filter_type %d dss_errno 0x%x, filters 0x%p, num filters %d",
...
)
{
  ...
  LOG_MSG_FUNCTION_ENTRY_2("dss_dereg_ip_filter(): socket %d, iface_id 0x%x",
                           sockfd, iface_id);
...
)
{
   ...
   LOG_MSG_FUNCTION_ENTRY_6("dss_reg_ip_filter_by_group(): socket %d iface_group 0x%x num_filters %d "
                            "filters 0x%p, filter type %d, dss errno 0x%p",
                            sockfd, iface_group, num_filters, filters, filter_type, dss_errno);
...
)
{
  ...
  LOG_MSG_FUNCTION_ENTRY_1("dss_dereg_ip_filter_by_group(): socket %d", 
                            sockfd);
