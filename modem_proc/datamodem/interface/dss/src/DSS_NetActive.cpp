...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/dss/src/DSS_NetActive.cpp#2 $
...
#define DSS_NETACTIVE_ASSERT(expression) \
         dss_netactive_assert_wrapper(__LINE__, expression)
...
===========================================================================*/
static void dss_netactive_assert_wrapper
(
...
DSSNetActive::DSSNetActive(sint15 netHandle):
   ...
   mpDSNetFirewallManager(NULL), mNetCb(NULL), mpNetUserData(NULL),
   ...
   mLastIfaceStateSentToApp(IFACE_STATE_INVALID), mpDSRanAsstManager(NULL),
   mpDSSRanAsstReqList(NULL), mpCritSect(NULL)
{
                             ...
                             void* pNetUserData)
{
   ...
   if (NULL == pSignalFactory || AEE_SUCCESS != res) {
      dss_error_code = DSS_ERROR_GET_SIGNALFACTORY_FAILED;
      goto bail;
   }
   ...
   res = pSignalFactory->CreateSignal(
      ...
      &mpIfaceUpSignalCtl);
   if (AEE_SUCCESS != res) {
      dss_error_code = DSS_ERROR_CREATE_IFACEUPSIGNAL_FAILED;
       goto bail;
   }
   ...
   if (AEE_SUCCESS != res) {
       dss_error_code = DSS_ERROR_CREATE_IFACEDOWNSIGNAL_FAILED;
       goto bail;
   }
   ...
   if(AEE_SUCCESS != res){
      dss_error_code = DSS_ERROR_QUERY_INTERFACE_INETWORKPRIV_FAILED;
      goto bail;
   }
   ...
   if(AEE_SUCCESS != res){
      dss_error_code = DSS_ERROR_QUERY_INTERFACE_INETWORKEXT_FAILED;
      goto bail;
   }
   ...
   if(AEE_SUCCESS != res){
      dss_error_code = DSS_ERROR_QUERY_INTERFACE_INETWORKEXT2_FAILED;
      goto bail;
   }
...
bail:
   ...
   LOG_MSG_FUNCTION_EXIT_2("NetActive::Init(): "
...
AEEResult DSSNetActive::RegAutoEvents(ds::Net::INetworkPriv* pIDSNetworkPriv)
{
   ...
   // There are 2 events that every network is automatically registered to: iface down & iface up
...
void DSSNetActive::IfaceUpOrDownSignalFcn(SignalHandlerBase* pSignalHandlerBaseParam)
{
  ...
  if (true == invokeCback && NULL != netCb) {
    (void) netCb (
      ...
      dss_errno,
      pUserData);
    LOG_MSG_INFO1_4("DSSNetActive::IfaceUpOrDownSignalFcn(): "
                    "DSS returned from App callback, netHandle: %d, ifaceId: %u, state=%d, NetApp 0x%x",
                    netHandle, ifaceId, state, pSignalHandlerBaseParam);
  }
}
...
void DSSNetActive::Destructor() throw()
{
   ...
   DSSCommon::ReleaseIf((IQI**)&mpDSRanAsstManager);
...
AEEResult DSSNetActive::GetNetRanAsstManager(IRanAsstMediaAdaptManager** ppRanAsstManager)
{

  //Validate Argument
  if (NULL == ppRanAsstManager)
  {
     return QDS_EINVAL;
  }
  //Check if RanAsstManager is already allocated (due to previous request) , Return same manager object
  if(NULL != mpDSRanAsstManager)
  {
     *ppRanAsstManager = mpDSRanAsstManager;
     (void)mpDSRanAsstManager->AddRef();
     return AEE_SUCCESS;
  }


  IDS_ERR_RET(mpIDSNetworkExt->CreateNetRanAsstManager(ppRanAsstManager));

  //Update into DSS_NetActive  member so it can be used later
  SetNetRanAsstManager(*ppRanAsstManager);
 
   return AEE_SUCCESS;
}/*GetNetRanAsstManager*/

AEEResult DSSNetActive::SetNetRanAsstManager
(
  IRanAsstMediaAdaptManager* pNetRanAsstManager
)
{
   mpDSRanAsstManager = pNetRanAsstManager;
   (void)mpDSRanAsstManager->AddRef();

   return AEE_SUCCESS;
}/*DSSNetActive::SetNetRanAsstManager*/

AEEResult DSSNetActive::AddRanAsstMediaAdaptReq
(
  ds::Net::IRanAsstMediaAdaptReqType*  pRanAsstManager,
  dss_iface_ioctl_event_cb             cback_fn,
  void*                                user_data,
  dss_iface_id_type                    ifaceId
)
{
  dss_iface_ioctl_ev_cb_type      evCbInfo;
  DSSNetMonitored                *pPrimaryNetMonitored = NULL;
  DSSNetMonitoredScope            primaryNetMonitoredScope;
  DSSNetApp*                      pNetApp = NULL;
  AEEResult                       res;
  /*-----------------------------------------------------------------------------------
    * Allocate DSS RAMA request 
   ------------------------------------------------------------------------------------*/
  DSSRanAsstMediaAdaptReq*  pDSSRAMAReq = 
                              PS_MEM_NEW(DSSRanAsstMediaAdaptReq(pRanAsstManager, ifaceId));
  if (NULL == pDSSRAMAReq) {
     LOG_MSG_ERROR_0("DSSNetActive::AddRanAsstMediaAdaptReq(): "
                     "Couldn't allocate AddRanAsstMediaAdaptReq");
     return AEE_ENOMEMORY;
  }

  /*-----------------------------------------------------------------------------------
    * Fetch NetActive and Net Monitor object and store into DSS RAMA req object
   ------------------------------------------------------------------------------------*/
  res = DSSGlobals::Instance()->GetNetApp(mNetHandle, &pNetApp);
  
  if (AEE_SUCCESS != res || NULL == pNetApp) {
    LOG_MSG_ERROR_1("NetApp is already removed by close_netlib call,"
                    " mNetHandle %d", mNetHandle);
    goto bail;
  }
  (void)pNetApp->GetPrimaryNetMonitored(&pPrimaryNetMonitored);
  (void)primaryNetMonitoredScope.Init(pPrimaryNetMonitored);
  ...
  /*-----------------------------------------------------------------------------------
    * Store DSS net monitor
   ------------------------------------------------------------------------------------*/
  pDSSRAMAReq->SetDSSNetMonitored(pPrimaryNetMonitored);
  /*-----------------------------------------------------------------------------------
    * Store NetActive object into DSS RAMA req object
   ------------------------------------------------------------------------------------*/
  pDSSRAMAReq->SetDSSNetActive(this);
    
  /*-----------------------------------------------------------------------------------
    * Register event DSS_IFACE_IOCTL_RAN_ASST_MEDIA_ADAPTATION_EV 
   ------------------------------------------------------------------------------------*/
  memset(&evCbInfo, 0x0, sizeof(evCbInfo));

  evCbInfo.event         = DSS_IFACE_IOCTL_RAN_ASST_MEDIA_ADAPTATION_EV;
  evCbInfo.app_id        = mNetHandle;
  evCbInfo.event_cb      = cback_fn;
  evCbInfo.user_data_ptr = user_data;
  
  res = pDSSRAMAReq->RegEventCB(&evCbInfo, ifaceId);
  if(AEE_SUCCESS != res) {
     goto bail;
  }

  /*-----------------------------------------------------------------------------------
    * Store newly created object into list
   ------------------------------------------------------------------------------------*/
  pDSSRAMAReq->InsertToList(mpDSSRanAsstReqList);
  mpDSSRanAsstReqList = pDSSRAMAReq;

  return AEE_SUCCESS;
  
  bail:
    PS_MEM_RELEASE(pDSSRAMAReq);
    return res;

}

AEEResult DSSNetActive::RemoveRanAsstMediaAdaptReq
 (
   sint15                                handle
 )
{
  AEEResult                     res = AEE_EBADPARM;
  DSSRanAsstMediaAdaptReq*      pTempDSSRanAsstReq     = NULL;
  DSSRanAsstMediaAdaptReq*      pTempPrevDSSRanAsstReq = NULL;
  DSSRanAsstMediaAdaptReq*      pTempNextDSSRanAsstReq = NULL;
  IRanAsstMediaAdaptReqType*    pTempNetRanAsstReq;
  uint32                        tempHandle;
  boolean                       nodeFound = FALSE;
  
  if (NULL == mpDSSRanAsstReqList) {
     LOG_MSG_ERROR_0("DSSNetActive::RemoveRanAsstMediaAdaptReq(): "
                     " No registration found");
     return AEE_EMEMPTR;
  }

  pTempDSSRanAsstReq = mpDSSRanAsstReqList;
    
  while (NULL != pTempDSSRanAsstReq)
  {
    pTempDSSRanAsstReq->GetIDSNetIRanAsstMediaAdaptReq(&pTempNetRanAsstReq);
    DSSGenScope scopeNetRanAsst(pTempNetRanAsstReq,DSSGenScope::IDSIQI_TYPE);

    if ( NULL != pTempNetRanAsstReq )
    {
      pTempNetRanAsstReq->GetRanAsstMediaAdaptHandle(&tempHandle);

      if ( ((sint15)tempHandle) == handle )
      {
        res = AEE_SUCCESS;
        nodeFound = TRUE;
        break;
    ...
    pTempPrevDSSRanAsstReq = pTempDSSRanAsstReq;
    pTempDSSRanAsstReq->GetNext(&pTempDSSRanAsstReq);
  }

  if ( TRUE == nodeFound && NULL != pTempDSSRanAsstReq)
  {
    //Remove Current Head pointer and Update List with next ptr
    if ( NULL == pTempPrevDSSRanAsstReq )
    {
      pTempDSSRanAsstReq->GetNext(&mpDSSRanAsstReqList);
      PS_MEM_RELEASE(pTempDSSRanAsstReq);
       
    } //Remove middle node
    else
    {
      pTempDSSRanAsstReq->GetNext(&pTempNextDSSRanAsstReq);
      pTempPrevDSSRanAsstReq->InsertToList(pTempNextDSSRanAsstReq);
      PS_MEM_RELEASE(pTempDSSRanAsstReq);
    
    }
  }
  return res;
}/*DSSNetActive::RemoveRanAsstMediaAdaptReq*/

AEEResult DSSNetActive::GetNetRanAsstMediaAdaptReq(
 ds::Net::IRanAsstMediaAdaptReqType**  pRanAsstReq,
 sint15                                handle
 )
{
  ...
  AEEResult                     res = AEE_EBADPARM;
  if (NULL == pRanAsstReq) {
     LOG_MSG_ERROR_0("DSSNetActive::GetNetRanAsstMediaAdaptReq(): "
                     " Invalid  RanAsstMediaAdaptReqType");
     return AEE_EMEMPTR;
  }
  ...
  while (NULL != pTempDSSRanAsstReq)
  {
    ...
    if ( NULL != pTempNetRanAsstReq )
    {
      ...
      {
        res = AEE_SUCCESS;
        *pRanAsstReq = pTempNetRanAsstReq;
        break;
      }
    }
    pTempDSSRanAsstReq->GetNext(&pTempDSSRanAsstReq);
...
bail:
  LOG_MSG_ERROR_2("DSSNetActive::AddQoSNetInitiatedReq(): "
                  "Req Object: 0x%p, error code %d",
                  pDSSQoSNetInitiatedReq, dss_error_code);
                                        ...
                                        dss_iface_ioctl_fltr_match_dir_type* fltr_match_dir_ptr)
{
  ...
  *ppDSSQoSNetInitiated = NULL;
  ...
  res = pNetQoSManagerPriv->CreateQoSNetInitiated(pQoSNetInitiatedReq,
                                                  &pQoSNetInitiated,
                                                  (QoSFltrMatchDirType*) fltr_match_dir_ptr);
  if (AEE_SUCCESS != res) {
  ...
  // and real time check.
  pDSSQoSNetInitiated = GetDSSQoSNetInitiated(pQoSNetInitiated, pDSSQoSNetInitiatedReq);
  if (NULL != pDSSQoSNetInitiated)
  {
    // Update matched direction
    pDSSQoSNetInitiated->SetFltrMatchDir(*fltr_match_dir_ptr);
    LOG_MSG_INFO1_2("DSSNetActive::AddQoSNetInitiated(): Update dir %d, "
                    "QoSNetInitiated obj 0x%x already exist, not notifying application again",
                    pQoSNetInitiated, *fltr_match_dir_ptr);
    res = AEE_EALREADY;
    goto bail;
  }
  ...
  *ppDSSQoSNetInitiated = PS_MEM_NEW(DSSQoSNetInitiated(pQoSNetInitiated, qos_handle, ifaceId, (uint32)pDSSQoSNetInitiatedReq, *fltr_match_dir_ptr));
  ...
  // put the new DSSQoSNetInitiated at the beginning of the DSSQoSNetQosInitiatedReq list.   
  {
     ...
     // Add the ref count as we will return ppDSSQoSNetInitiated object to the caller and its caller
     // responsibility to release the ref count 
     (void)(*ppDSSQoSNetInitiated)->AddRef();
...
AEEResult DSSNetActive::AddDSSFirewallRule(IFirewallRule* pIFirewallRule, uint32 *pHandle)
{
   ...
   DSS_NETACTIVE_ASSERT (NULL != pIFirewallRule);
   DSS_NETACTIVE_ASSERT (NULL != pHandle);
...
AEEResult DSSNetActive::GetDSSFirewallRule(uint32 firewallHandle, IFirewallRule** ppIFirewallRule)
{
   ...
   DSS_NETACTIVE_ASSERT (NULL != ppIFirewallRule);
   *ppIFirewallRule = 0;
  ...
  while(NULL != pDSSQoSNetInitiated)
  {
    ...
    if (tmpQoSHandle == qos_handle)
    {
      *ppDSSQoSNetInitiated = pDSSQoSNetInitiated;
      ...
      // Add the ref count as we will return pDSSQoSNetInitiated object to the caller and its caller
      // responsibility to release the ref count later.
      (void) pDSSQoSNetInitiated->AddRef();
...
void DSSNetActive::FreeLists()
{
   ...
   while (NULL != mpDSSRanAsstReqList)
   {
     ret = mpDSSRanAsstReqList->GetNext(&pTempDSSRanAsstReq);
     if (AEE_SUCCESS != ret)
     {
       return;
     }
     PS_MEM_RELEASE(mpDSSRanAsstReqList);
     mpDSSRanAsstReqList = pTempDSSRanAsstReq;
   }
...
AEEResult DSSNetActive::PPPOpen()
{
   ...
   if (AEE_SUCCESS != res) {
      ...
      if ((NetworkState::QDS_OPEN_IN_PROGRESS != netState) &&
         ...
         res = pTmpIDSNetworkPriv->BringUpInterface(&netdownreason);
...
DSSQoSNetInitiated* DSSNetActive::GetDSSQoSNetInitiated(IQoSNetInitiated* pQoSNetInitiatedToCheck, 
                                                        DSSQoSNetInitiatedReq* pDSSQoSNetInitiatedReq)
{
  // using the critical section to guard access to mpDSSQoSNetInitiatedList
  DSSCritScope lockDSSQoSNetInitiatedList(*mpCritSect);
  ...
  while (NULL != pDSSQoSNetInitiated)
  {
    ...
    {
      dss_error_code = DSS_ERROR_GET_QOSNETINITIATED_FAILED;
      goto bail;
    }
    DSSGenScope scopeQoSNetInitiated(pQoSNetInitiated,DSSGenScope::IDSIQI_TYPE);
    ...
    {
      return pDSSQoSNetInitiated;
    }

    res = pDSSQoSNetInitiated->GetNext(&pDSSQoSNetInitiated);
    if (AEE_SUCCESS != res)
    {
      dss_error_code = DSS_ERROR_DSSQOSNETINITIATED_GETNEXT_FAILED;
      goto bail;
    }
  }

  bail:
  LOG_MSG_ERROR_2("DSSNetActive::QoSNetInitiatedExists(): "
