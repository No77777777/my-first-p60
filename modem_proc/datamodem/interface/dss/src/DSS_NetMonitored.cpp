...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/dss/src/DSS_NetMonitored.cpp#3 $
...
DSSNetMonitored::DSSNetMonitored(bool bPolicyBitIsOn):
   ...
   mpApnParamChangedHandler(NULL),mpDpdPingStatHandler(NULL),
   mpPhysLinkPacketDiscardHandler(NULL),
   ...
   mpDSNetPhysLink(0), mpDSNetTechUMTS(0)
{
   ...
   if (AEE_SUCCESS != DSSGlobals::Instance()->GetCritSect(&mpCritSect)) {
      //DS_UTILS_ERROR_FATAL("Couldn't fetch CritSect object from DSSGlobals");
   }
}
...
bail:
   ...
   LOG_MSG_FUNCTION_EXIT_2("DSSNetMonitored::Init(): "
...
void DSSNetMonitored::Destructor() throw()
{
   ...
   PS_MEM_RELEASE(mpDpdPingStatHandler);
   PS_MEM_RELEASE(mpPhysLinkPacketDiscardHandler);
...
//===================================================================
//  FUNCTION:   DSSNetMonitored::GetPathMtu
//
//  DESCRIPTION:
//  Serves DSS_IFACE_IOCTL_GET_PATH_MTU
//===================================================================
AEEResult DSSNetMonitored::GetPathMtu(dss_iface_ioctl_mtu_type* pMtu)
{
   if (NULL == mpIDSNetwork) {
      return QDS_EINVAL;
   }

   IDS_ERR_RET(mpIDSNetwork->GetNetPathMTU((int*)pMtu));
...
AEEResult DSSNetMonitored::SetSilentRedial(dss_iface_ioctl_set_silent_redial_type* pSilentRedial)
{
  ...
  LOG_MSG_FUNCTION_EXIT_2("DSSNetMonitored::DoSilentRedial(): "
                          "NetHandle :0x%x , result :0x%x ", mpIDSNetwork, res);
...
AEEResult DSSNetMonitored::StartDpdPing(dss_iface_ioctl_dpd_ping_config_type* pDpdPingConfig)
{
  AEEResult res = AEE_SUCCESS;
  dpdPingConfigType dpdPingConfig;
 
  if (NULL == mpIDSNetwork) {
    return QDS_EINVAL;
  }

  IDS_ERR_RET(DSSConversion::DS2IDSDpdPingConfig(pDpdPingConfig, &dpdPingConfig));
  IDS_ERR_RET(mpIDSNetwork->StartDpdPing(dpdPingConfig));
  LOG_MSG_FUNCTION_EXIT_2("DSSNetMonitored::StartDpdPing(): "
                          "NetHandle :0x%x , result :0x%x ", mpIDSNetwork, res);
...
AEEResult DSSNetMonitored::GetMSISDN(dss_iface_msisdn_info_type* pGetMSISDN)
{
  AEEResult res = AEE_SUCCESS;
  if (NULL == mpIDSNetwork) {
    return QDS_EINVAL;
  ...
  IDS_ERR_RET(mpIDSNetwork->GetMSISDN(&msisdn));
  IDS_ERR_RET(DSSConversion::IDS2DSMSISDN(&msisdn, pGetMSISDN));
  return res;
}

AEEResult DSSNetMonitored::GetOpResPcoList(dss_iface_op_reserved_pco_list_type* pGetOpResPcoList)
{
  AEEResult res = AEE_SUCCESS;
  if (NULL == mpIDSNetwork) {
    return QDS_EINVAL;
  ...
  NetworkOpReservedPcoListType opReservedPcoList;

  IDS_ERR_RET(mpIDSNetwork->GetOpResPcoList(&opReservedPcoList));
  IDS_ERR_RET(DSSConversion::IDS2DSOpResPcoList(&opReservedPcoList, pGetOpResPcoList));
  return res;
}
                                      ...
                                      dss_iface_id_type ifaceId)
{
   ...
   LOG_MSG_INFO3_2("DSSNetMonitored::RegEventCB(): "
                                           ...
                                           bool bInit)
{
   ...
   switch (event) {
      ...
      case DSS_IFACE_IOCTL_DPD_PING_INFO_EV:
        return FetchHandler(&mpDpdPingStatHandler, ppEventHandler, bInit,DSS_IFACE_IOCTL_DPD_PING_INFO_EV);
      case DSS_IFACE_IOCTL_PACKET_DISCARD_EV:
        return FetchHandler(&mpPhysLinkPacketDiscardHandler, ppEventHandler, bInit,DSS_IFACE_IOCTL_PACKET_DISCARD_EV);
...
AEEResult DSSNetMonitored::ReleaseEventHandler(dss_iface_ioctl_event_enum_type event)
{
   ...
   switch (event) {
      ...
      case DSS_IFACE_IOCTL_APN_PARAM_CHANGE_EV:
        ...
        break;
      case DSS_IFACE_IOCTL_DPD_PING_INFO_EV:
        DSSCommon::ReleaseIf((IQI**)(&mpDpdPingStatHandler));
        break;
      case DSS_IFACE_IOCTL_PACKET_DISCARD_EV:
        DSSCommon::ReleaseIf((IQI**)(&mpPhysLinkPacketDiscardHandler));
        break;
      default:
...
#if 0
AEEResult DSSNetMonitored::GetDpdPingStat
(
 dss_iface_dpd_ping_stat_info_type * pDpdPingStat
)
{
  DpdPingStatInfoType  dpdPingStat;

  if (NULL == mpIDSNetwork || 0 == pApnParams) {
    return QDS_EINVAL;
  }

  memset(&dpdPingStat, 0, sizeof(dpdPingStat));
...
AEEResult DSSNetMonitored::InitDPDcheck()
{
  AEEResult res = AEE_SUCCESS;
  ...
  IDS_ERR_RET(mpIDSNetwork->InitDPDcheck());
  return res;
}

AEEResult DSSNetMonitored::IsDefaultPhysLink
(
...
)
{
  ...
  if (NULL == isDefaultPhysLink || NULL == mpIDSNetwork) {
    return QDS_EINVAL;
  }

  IDS_ERR_RET(mpIDSNetwork->IsDefaultPhysLink(physLinkHandle, isDefaultPhysLink));
  return res;
}
