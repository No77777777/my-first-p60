...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/dss/src/DSS_NetApp.cpp#2 $
...
#define DSS_NETAPP_ASSERT(expression) \
         dss_netapp_assert_wrapper(__LINE__, expression)
...
===========================================================================*/
static void dss_netapp_assert_wrapper
(
...
AEEResult DSSNetApp::CreateNetActive()
{
   ...
   if (NULL == mpNetActive) {
      dss_error_code = DSS_ERROR_CREATE_NETACTIVE_FAILED;
      res = AEE_ENOMEMORY;
      goto bail;
   }
   ...
   if (AEE_SUCCESS != localRes) {
      res = localRes;
      dss_error_code = DSS_ERROR_FETCH_TO_NETACTIVE_FAILED;
      goto bail;
   }
   ...
   if ((AEE_SUCCESS != res) && (AEE_EWOULDBLOCK != res)) {
      mLastNetDownReason = (dss_net_down_reason_type)res;
      dss_error_code = DSS_ERROR_NETACTIVESCOPE_INIT_FAILED;
      goto bail;
   }
   ...
   DSS_NETAPP_ASSERT(NULL != mpPrimaryNetMonitored);
   ...
   if (AEE_SUCCESS != localRes) {
      res = localRes;
      dss_error_code = DSS_ERROR_REGAUTOEVENTS_FAILED;
      goto bail;
   }
...
bail: // Only on error
  LOG_MSG_ERROR_3("DSSNetApp::CreateNetActive(): "
                  "Failed to fetch NetActive from NetApp object. res=%d pNetApp=0x%x",
                  res, this, dss_error_code);
   ...
   DSSNetMonitored** ppNetMonitored)
{
   ...
   if (AEE_SUCCESS != res) {
      dss_error_code = DSS_ERROR_CREATE_DSNETPOLICY_FAILED;
      goto bail;
   }
   ...
   if (AEE_SUCCESS != res) {
      dss_error_code = DSS_ERROR_CREATE_IDSNETWORKFACTORY_FAILED;
      goto bail;
   }
   ...
   res = pIDSNetworkFactory->CreateNetwork(
      ...
      &netdownreason);
   if (AEE_SUCCESS != res) {
      dss_error_code = DSS_ERROR_CREATE_INETWORK_FAILED;
      goto bail;
   }
...
bail:
   LOG_MSG_ERROR_3("DSSNetApp::GetNetMonitoredByIfaceId(): "
                   "result %d iface id %u in policy, error code %d",
                   res, ifaceId, dss_error_code);
...
AEEResult DSSNetApp::UpdateNetDownReason()
{
   ...
   if(AEE_SUCCESS != res) {
      dss_error_code = DSS_ERROR_IDSNETACTIVESCOPE_INIT_FAILED;
      goto bail;
   }

   res = IDSNetActiveScope.Fetch()->GetLastNetDownReason(&netdownreason);
   if(AEE_SUCCESS != res) {
      dss_error_code = DSS_ERROR_GET_LASTNETDOWNREASON_FAILED;
      goto bail;
   }
...
bail:
   LOG_MSG_ERROR_2("DSSNetApp::UpdateNetDownReason(): "
                   "IDS2DSNetworkDownReason failed result %d, error code %d",
                   res, dss_error_code);
...
AEEResult DSSNetApp::PPPOpen()
{
   ...
   if (NULL != mpNetActive) {
      ...
      if ((AEE_SUCCESS) != res && (AEE_EWOULDBLOCK != res)) {
         dss_error_code = DSS_ERROR_NETACTIVE_BRINGUP_FAILED;
         goto bail;
      }
   }
   ...
   if (AEE_SUCCESS != localRes) {
      res = localRes;
      dss_error_code = DSS_ERROR_GET_IFACE_ID_FAILED;
      goto bail;
   }
   ...
   if (AEE_SUCCESS != localRes) {
      res = localRes;
      dss_error_code = DSS_ERROR_FORCELOOKUPINTERFACEBYIFACEID_FAILED;
      goto bail;
   }
}
...
bail:
   ...
   LOG_MSG_ERROR_5("DSSNetApp::PPPOpen(): "
                   "obj 0x%x, res %d, mpNetActive 0x%x, mpPrimaryNetMonitored 0x%x",
                   this, res, mpNetActive, mpPrimaryNetMonitored, dss_error_code);
