...
/*===========================================================================
  ...
  $PVCSPath: $
  $Header: //components/rel/data.mpss/3.5..2.1/interface/dss/src/dss_ping_comm_mgr.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
/* This variable is updated from NV item 74012*/
/* Default this NV set to 1 to support multi ping */
/* modem pings use icmp id 0 if NV is set to 0 (TMO)*/
boolean is_multi_ping_supported;
...
#define DSS_PING_ASSERT(expression) \
         dss_ping_assert_wrapper(__LINE__, expression)
...
===========================================================================*/
static void dss_ping_assert_wrapper
(
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  icmp_echo_rsp_seq_num = ps_ntohs(icmp_echo_rsp_seq_num);
    ...
    if (!(ping_sess_ptr->ping_ack_info_bit_field & ((uint64)0x1 << bit_pos)))
    {
      LOG_MSG_INFO1_0("dssi_ping_comm_mgr_read_ping_response(): "
                      "Multiple ping response for same ping request ");
      continue;
    }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(is_multi_ping_supported)
  {
    icmp_msg_id = ping_sess_ptr->icmp_index;
  }
  ...
  if ( TRUE == ret )
  {
    ...
    if(icmp_msg_id)
    {
      filter.next_prot_hdr.icmp.id = dss_htons(icmp_msg_id);
    }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  for(i = 0; i < DSS_PING_MAX_PING_SESSIONS; ++i)
  {
    if( dssi_ping_comm_mgr_ping_sess_pool[i].is_free )
    {
      ...
      if(is_multi_ping_supported)
      {
        new_ping_session->icmp_echo_msg_id = (int16)new_ping_session->icmp_index;
      }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if( ping_sess_ptr->sockfd == 0 )
  {
    ...
    if( ret == DSS_SUCCESS || (ret == DSS_ERROR && dss_errno == DS_EWOULDBLOCK) )
    {
    }
    else
    {
      ...
      reason = DSS_PING_ERROR;
      LOG_MSG_FUNCTION_EXIT_3("dssi_ping_comm_mgr_send_icmp_req(): "
                        "ret %d, reason %d, error %d", ret, reason, dss_errno);
      (void)dss_ping_comm_mgr_abort_ping((dss_ping_handle)ping_sess_ptr, &reason, dss_errno);
    }
    return;
  }
...
{
  uint8  i;
  const char * dss_multi_ping_config_path = 
         {PS_NV_CONF_ITEM_INFO(PS_NV_CONF_DSS_MULTI_PING)};
  int32 retval = 0;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  retval = ps_read_efs_nv(dss_multi_ping_config_path,
                          &is_multi_ping_supported, 
                          sizeof(is_multi_ping_supported));
  if(0 != retval)
  {
    is_multi_ping_supported = TRUE;
	if(ps_write_efs_nv(dss_multi_ping_config_path,
                       &is_multi_ping_supported,
                       sizeof(is_multi_ping_supported)))
    {
      LOG_MSG_ERROR_0("failed to multi ping config to NV item");
    }
