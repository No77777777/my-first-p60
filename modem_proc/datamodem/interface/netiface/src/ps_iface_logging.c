...
/*===========================================================================
 ...
 $Header: //components/rel/data.mpss/3.5..2.1/interface/netiface/src/ps_iface_logging.c#2 $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR_INT_1("ps_iface_dpl_support_network_logging(): "
  ...
  DPL_RESET_IFACE_LOG_DROP_COUNT(ps_iface_ptr);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (DPL_IID_IFNAME_MIN > ifname || DPL_IID_IFNAME_MAX <= ifname)
  {
    LOG_MSG_ERROR_INT_1("ps_iface_dpl_support_flow_logging(): "
                    "Invalid ifname (%d) passed", ifname);
    DS_UTILS_ASSERT(0);
    return -1;
  }
  ...
  DS_UTILS_ASSERT(PS_FLOW_IS_VALID(flow_ptr));
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (DPL_IID_IFNAME_MIN > ifname || DPL_IID_IFNAME_MAX <= ifname)
  {
    LOG_MSG_ERROR_INT_1("ps_iface_dpl_support_link_logging(): "
                    "Invalid ifname (%d) passed", ifname);
    DS_UTILS_ASSERT(0);
    return -1;
  }
  ...
  if( !PS_PHYS_LINK_IS_VALID(phys_link_ptr) )
  {
    LOG_MSG_ERROR_1("ps_iface_dpl_support_link_logging(): invalid phys_link %p",
                    phys_link_ptr);
    PS_BRANCH_LEAVE_CRIT_SECTION(&global_ps_crit_section);
  ...
  for (i = 0; i < PS_IFACE_GET_NUM_PHYS_LINKS(ps_iface_ptr); i++)
  {
    ...
    phys_link_ptr->dpl_link_cb.recv_dpl_id.link_instance   =
      phys_link_ptr->phys_private.instance;
    phys_link_ptr->dpl_link_cb.dropped_count = 0;
