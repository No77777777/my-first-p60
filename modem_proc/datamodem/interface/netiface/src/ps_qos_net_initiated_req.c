...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/netiface/src/ps_qos_net_initiated_req.c#2 $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  -------------------------------------------------------------------------*/
   ...
   {
      LOG_MSG_ERROR_INT_0("ps_iface_qos_net_initiated_req_init(): "
...
(
  ...
  const ps_iface_filter_spec_type    * qos_spec_ptr,
  int16                              * ps_errno
)
{
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
       ...
       fltr_index++)
  {
    ...
    ps_iface_log_qos_spec_filter_params(&qos_spec_ptr->rx_fltr_template.list_ptr[fltr_index], fltr_index);
  }
       ...
       fltr_index++)
  {
    ...
    ps_iface_log_qos_spec_filter_params(&qos_spec_ptr->tx_fltr_template.list_ptr[fltr_index], fltr_index);
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_UTILS_ERROR_FATAL("ps_qos_net_initiated_delete_qos_spec(): QOS info pointer of qos net initiated req NULL");
    return;
  }
...
(
  ...
  const ps_iface_filter_spec_type      * qos_net_initiated_spec,
  int16                                * ps_errno
)
{
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  LOG_MSG_INFO1_0("ps_qos_net_initiated_store_qos_spec()");
  DS_UTILS_ASSERT(NULL != ps_errno);

  if (NULL == qos_net_initiated_spec)
  {
    DS_UTILS_ERROR_FATAL("ps_qos_net_initiated_store_qos_spec(): QOS_SPEC is NULL");
    *ps_errno = DS_EFAULT;
    return -1;
  }
  ...
  {
    DS_UTILS_ERROR_FATAL("ps_qos_net_initiated_store_qos_spec(): qos_net_initiated_req or qos_info_ptr is NULL");
    *ps_errno = DS_EFAULT;
    return -1;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (NULL == iface_ptr ||
      ...
      NULL == ps_errno)
  {
    DS_UTILS_ERROR_FATAL("ps_iface_create_qos_net_initiated_request: Invalid args");
    return -1;    
  }
  ...
  if (ps_flowi_validate_net_initiated_qos_spec(&qos_net_initiated_request_ptr->net_initiated_qos_spec,
                                               ...
                                               ps_errno) != 0)
  {
    LOG_MSG_ERROR_INT_0("ps_iface_create_qos_net_initiated_request(): "
  ...
  /* Set filter matching direction */
  qos_net_initiated_req->fltr_match_dir = qos_net_initiated_request_ptr->fltr_match_dir;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (NULL == iface_ptr || 
      ...
      NULL == ps_errno)
  {
    DS_UTILS_ERROR_FATAL("Invalid args to ps_iface_release_qos_net_initiated");
    return -1;    
  }
  ...
  while (flow_ptr != NULL)
  {
    ...
    if ( PS_FLOW_IS_NW_INITIATED(flow_ptr) &&
         PS_QOS_FILTR_DIR_MAX > (matched_dir = ps_flow_match_filter(flow_ptr,
                                                    ...
                                                    qos_net_initiated_real_time_check_ptr->fltr_match_dir
                                                    )))
    {
       LOG_MSG_INFO1_2("ps_iface_qos_net_initiated_real_time_check : "
                       "Flow 0x%p has a match dir %d for NIQ request. Notifying application",
                       flow_ptr, matched_dir);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    matched_dir = ps_qos_net_initiated_check_one_request(ps_qos_net_initiated_req,
                                                         qos_spec_ptr);
    if (PS_QOS_FILTR_DIR_MAX > matched_dir)
    {
      /*--------------------------------------------------------------------- 
        Notify application on Network Initiated QoS matching application
        specified filters 
        -------------------------------------------------------------------*/
       LOG_MSG_INFO1_2("ps_qos_net_initiated_check_requests(): "
                       "NIQ Request 0x%p has a match dir %d for flow. Notifying application",
                       ps_qos_net_initiated_req, matched_dir);
       event_info.iface_net_initiated_qos_available_info.handle =
         (int32)ps_qos_net_initiated_req;
       event_info.iface_net_initiated_qos_available_info.fltr_match_dir = matched_dir;
...
RETURN VALUE
  < PS_QOS_FILTR_DIR_MAX : on success
  PS_QOS_FILTR_DIR_MAX   : on failure
...
SIDE EFFECTS
  None
===========================================================================*/
ps_iface_qos_fltr_match_dir_enum_type ps_qos_net_initiated_check_one_request
(
  ps_qos_net_initiated_req_type    * ps_qos_net_initiated_req,
  qos_spec_type                    * qos_spec_ptr
)
{
  ...
  ps_iface_qos_fltr_match_dir_enum_type  fltr_req_dir =
                                         ps_qos_net_initiated_req->fltr_match_dir;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /*-------------------------------------------------------------------------
    ...
    Compare the filter only if the condition on the table is satisfied.

    Requested DIR | TX only | RX only | TX & RX
    ----------------------------------------------
          UL      |   Yes   |   No    |   Yes
          DL      |   No    |   Yes   |   Yes
       UL or DL   |   Yes   |   Yes   |   Yes
       UL and DL  |   No    |   No    |   Yes
  -------------------------------------------------------------------------*/
  ...
  if (PS_QOS_FILTR_DIR_DL != fltr_req_dir)
  {
    ...
    {
      if ( -1 ==  ps_utils_match_one_fltr_in_fltrspec(&filter_buf_ptr->filter,
  ...
  if (PS_QOS_FILTR_DIR_UL != fltr_req_dir &&
      !(PS_QOS_FILTR_DIR_BIDIR == fltr_req_dir && FALSE == is_ul_matched) )
  {
    filter_buf_ptr = list_peek_front(
  ...
  LOG_MSG_INFO1_1("ps_qos_net_initiated_check_one_request(): "
                  "SUCCESS dir %d", ret_dir);
...
(
  ...
  ps_iface_qos_fltr_match_dir_enum_type  direction
)
{
  ...
  int                    i;
  int32                  retval = -1;
  boolean                is_rx_matched = FALSE;
  boolean                is_tx_matched = FALSE;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(PS_QOS_FILTR_DIR_UL != direction)
  {
    do
    {
      if(0 != num_rx_filters )
      ...
      -------------------------------------------------------------------------*/
        ...
        if ( -1 == ps_utils_match_one_fltr_in_fltrspec(rx_ip_filter_ptr,
                                                       &rx_filter_spec) )
        {
  ...
  if(PS_QOS_FILTR_DIR_DL != direction ||
     (PS_QOS_FILTR_DIR_BIDIR == direction && TRUE == is_rx_matched) ||
     (PS_QOS_FILTR_DIR_UL_OR_DL == direction && FALSE == is_rx_matched) )
  {
    LOG_MSG_INFO1_1("ps_qos_net_initiated_check_flow() "
                    "Zero RX filters, ps_flow 0x%p", flow_ptr);
    do
    {
      ...
      -------------------------------------------------------------------------*/
        ...
        if ( -1 == ps_utils_match_one_fltr_in_fltrspec(tx_ip_filter_ptr,
                                                       &tx_filter_spec) )
        {
          /* Did not find any filter in the qos_spec of the flow that matches
             this filter that the application specified for network initiated QOS
  ...
  /* We went over all filters specified by the application and for each one we
     found a matching filter in the new flow's QoS Specification */
  if(PS_QOS_FILTR_DIR_BIDIR == direction)
  {
    if (TRUE == is_rx_matched && TRUE == is_tx_matched)
    {
      retval = 0;
    }
  }
...
)
{
  ...
  switch(fltr_ptr1->ip_vsn)
  {
    ...
    default:
      ...
      LOG_MSG_ERROR_INT_1("Unsupported IP Version %d", fltr_ptr1->ip_vsn);
      DS_UTILS_ASSERT(0);
      return FALSE;
  }
  ...
  switch(next_hdr_prot)
  {
    ...
    default:
      ...
      LOG_MSG_ERROR_INT_1("Unsupported protocol %d", next_hdr_prot);
      DS_UTILS_ASSERT(0);
      return FALSE;
  }
  ...
  if(NULL != flow_ptr)
  {
    ...
    while (NULL != tmp_tx_fltr_buf)
    {
      if(tmp_tx_fltr_buf->fi_handle == fltr_handle)
      {
         ...
         if ( -1 ==  ps_qos_check_one_filter(&filter_buf, filtr_info_ptr) )
        {
          PS_LEAVE_CRIT_SECTION(&global_ps_crit_section);
          return -1;       
        }	
