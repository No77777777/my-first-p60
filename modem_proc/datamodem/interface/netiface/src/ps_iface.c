...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/netiface/src/ps_iface.c#4 $
...
11/13/14    cx     IPv6 address cache optmization
...
static ps_iface_ipfltr_handle_type  ps_iface_low_latency_timer_hdl;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR_INT_0("ps_iface_init(): "
  ...
  /*-------------------------------------------------------------------------
    Register command handlers for low latency command
  -------------------------------------------------------------------------*/
  (void) ps_set_cmd_handler( PS_IFACE_LOW_LATENCY_EVENT_CMD,
                             ps_iface_low_latency_event_handler );
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_QTRACE_HFT_MED(PS_TAG_NETIFACE, PS_SYS_DEFAULT_SUBS,
                 "ps_iface_create_internal(): Creating iface(0x%x) 0x%x:%d",
  ...
  (void)q_init(&(this_iface_ptr->iface_private.event_handle_q));
  ...
  /*-------------------------------------------------------------------------
    Set reassembly processing flag.
  -------------------------------------------------------------------------*/
  PS_IFACE_SET_REASSEMBLY_FLAG(this_iface_ptr, TRUE);
  ...
  {
    LOG_MSG_ERROR_INT_0("ps_iface_create_internal(): "
  ...
  PS_SYSTEM_HEAP_MEM_ALLOC(this_iface_ptr->event_info_cache.dpd_ping_stat_info,
                           sizeof(ps_iface_dpd_ping_stat_info_type),
                           ps_iface_dpd_ping_stat_info_type*);

  if(this_iface_ptr->event_info_cache.dpd_ping_stat_info == NULL)
  {
    LOG_MSG_ERROR_0("ps_iface_create_internal(): "
                    "No mem for dpd_ping_stat_info");
    return -1;
  }
  memset(this_iface_ptr->event_info_cache.dpd_ping_stat_info,
         0, sizeof(ps_iface_dpd_ping_stat_info_type));

  this_iface_ptr->handoff_class = PS_IFACE_HANDOFF_CLASS_NONE;
  
  this_iface_ptr->powersave_restrictive = FALSE;
  /*-------------------------------------------------------------------------
    Default disbale QoS filter offload to HW
  -------------------------------------------------------------------------*/

  this_iface_ptr->qos_fltr_offloaded_to_hw = FALSE;

  /*-------------------------------------------------------------------------
    Set the data path flag to default (Bridging Mode).
  -------------------------------------------------------------------------*/  
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (ps_errno == NULL)
  {
    DS_UTILS_ERROR_FATAL("ps_iface_active_associate(): NULL parameter is passed");
    return -1;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (ps_errno == NULL)
  {
    DS_UTILS_ERROR_FATAL("ps_iface_passive_associate(): NULL parameter is passed");
    return -1;
  }
  ...
  -------------------------------------------------------------------------*/
  PS_IFACE_SET_ASSOC_IFACE(logical_iface_ptr, new_assoc_iface_ptr);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (ps_errno == NULL)
  {
    DS_UTILS_ERROR_FATAL("ps_iface_active_disassociate(): NULL parameter is passed");
    return -1;
  }
  ...
  -------------------------------------------------------------------------*/
  PS_IFACE_SET_ASSOC_IFACE(logical_iface_ptr, NULL);
  logical_iface_ptr->iface_private.inherit_ip_info     = FALSE;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (ps_errno == NULL)
  {
    DS_UTILS_ERROR_FATAL("ps_iface_passive_disassociate(): NULL parameter is passed");
    return(-1);
  }
...
void ps_iface_phys_link_flow_ctl_ioctl
(
  ps_phys_link_type           * phys_link_ptr,
  ds3g_flow_e_type              flow_type,
  uint64                        flow_mask
)
{
  ps_phys_link_ioctl_flow_ctl_type  ioctl_param;
  sint15                            ps_errno;
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  
  LOG_MSG_INFO2_4("ps_iface_phys_link_flow_ctl_ioctl(): "
                  "phys_link_ptr 0x%x flow_type %d  mask 0x%x cnt %d",
                  phys_link_ptr,
                  flow_type,
                  flow_mask,
                  phys_link_ptr->phys_private.iface_flow_ctl_mask_cnt);
  
  if (   DS_FLOW_DISABLE == flow_type )
  {
    if ( 0 < phys_link_ptr->phys_private.iface_flow_ctl_mask_cnt)
    {
      phys_link_ptr->phys_private.iface_flow_ctl_mask_cnt++;
      return;
    }
  ...
  ioctl_param.flow_mask = flow_mask;
  
  if ( DS_FLOW_ENABLE == flow_type )
  {
    ioctl_param.flow_ctl_req = PS_PHYS_LINK_IOCTL_FLOW_ENABLE;
  }
  else
  {
    ioctl_param.flow_ctl_req = PS_PHYS_LINK_IOCTL_FLOW_DISABLE;
  }

  ps_phys_link_ioctl( phys_link_ptr, PS_PHYS_LINK_IOCTL_FLOW_CTL, &ioctl_param, &ps_errno);
  
}/* ps_iface_phys_link_flow_ctl_ioctl */


void ps_ifacei_phy_link_flow_ctl
(
  ps_iface_type                *this_iface_ptr,
  ds3g_flow_e_type              flow_type,
  uint64                        flow_mask
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /*-------------------------------------------------------------------------
    Notify Mode handler About Iface flow enable/disable. This function will be invoked for both 
    physical and logical ifaces  hence mainitaining ref count in phylink to suppress additional enable 
    and disable
  -------------------------------------------------------------------------*/
  ...
  /*-------------------------------------------------------------------------
    Invalid Base iface(May be Base Iface down in middle of processing). Ignore
  -------------------------------------------------------------------------*/
  if (!PS_IFACE_IS_VALID(this_iface_ptr))
  {
    return;
  ...
  /*-------------------------------------------------------------------------
    Invoke IOCTL for Primary/Default phylink
  -------------------------------------------------------------------------*/
  phys_link_ptr = PS_IFACEI_GET_PHYS_LINK(this_iface_ptr);
  
  if (PS_PHYS_LINK_IS_VALID(phys_link_ptr))
  {
    ps_iface_phys_link_flow_ctl_ioctl(phys_link_ptr, flow_type, flow_mask);
  }

  /*-------------------------------------------------------------------------
    Loop through all secondary flows and invoke physlink ioctls
  -------------------------------------------------------------------------*/
  flow_ptr = list_peek_front(&(this_iface_ptr->iface_private.flow.sec_flow_list));
  ...
  while (flow_ptr != NULL)
  {
    ...
    flow_ptr = list_peek_next(&this_iface_ptr->iface_private.flow.sec_flow_list,
                          &flow_ptr->link);
  }

}/* ps_ifacei_phy_link_flow_ctl */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
     ...
     event_info.flow_mask != ALL_FLOWS_ENABLED)
  {
    ...
    ps_ifacei_invoke_event_cbacks(this_iface_ptr,
                                  ...
                                  event_info);
#ifdef FEATURE_DATA_FLOW_MGMT
    ps_ifacei_phy_link_flow_ctl(this_iface_ptr, flow_type, DS_FLOW_PHYS_LINK_IFACE_MASK);
#endif/* FEATURE_DATA_FLOW_MGMT*/ 
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR_INT_0("ps_iface_set_v4_addr(): "
...
/*===========================================================================
FUNCTION PS_IFACE_SET_WLAN_V4_ADDR()

DESCRIPTION
  This function sets ipv4 address for wlan iface and also generates ps_sys ip
  address change.
...
{
  ps_sys_wlan_ip_addr_type   wlan_ip_addr;
  uint32                     result = -1;  
  int16                      ps_errno;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  do
  {
    if (0 != ps_iface_set_v4_addr( this_iface_ptr, ip_addr_ptr ))
    {
      break;
    }
      
    memset(&wlan_ip_addr, 0, sizeof(ps_sys_wlan_ip_addr_type));

    /* Retrieve the previous wlan ip address information since either v4 or
       v6 address is being changed
    */
    result = ps_sys_conf_get_ex(PS_SYS_TECH_WLAN,
                                PS_SYS_CONF_GET_WLAN_IP_ADDR,
    ...
    memscpy(&(wlan_ip_addr.ipv4_addr), 
           sizeof(ps_ip_addr_type),
           ip_addr_ptr,
           sizeof(ps_ip_addr_type));
           
    /* Save the IP address in the configuration cache and generate sys wlan
       IP address change event */
    result = ps_sys_conf_set_ex(PS_SYS_TECH_WLAN,
                                PS_SYS_CONF_GET_WLAN_IP_ADDR,
                                (ps_sys_subscription_enum_type)subs_id,
                                &wlan_ip_addr,
                                &ps_errno);
  ...
  LOG_MSG_ERROR_INT_0("ps_iface_set_wlan_v4_addr: Failed to set ipv4 address");
  
  return -1;
} /* ps_iface_set_wlan_v4_addr() */

/*===========================================================================
FUNCTION PS_IFACE_SET_WLAN_IPV6_ADDR()

DESCRIPTION
  Applies a prefix and iid to a particular interface. This generates 
  ps sys ipv6 address change event if prefix/iid has changed.
...
SIDE EFFECTS
  None
===========================================================================*/
int ps_iface_set_wlan_ipv6_addr
(
  ps_iface_type                 *this_iface_ptr,
  uint64                          gateway_iid,
  uint64                          prefix,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  do
  {
    ret_val = ps_iface_apply_v6_prefix( this_iface_ptr,
                                        gateway_iid,
                                        prefix,
                                        valid_lifetime,
                                        pref_lifetime, 
    ...
    if (FALSE == ps_iface_set_v6_iid( this_iface_ptr, iid) )
    {
      break;
    }
    ...
    */
    ...
    wlan_ip_addr.ipv6_addr.addr.v6.ps_s6_addr64[0] = prefix;
    wlan_ip_addr.ipv6_addr.addr.v6.ps_s6_addr64[1] = iid;
    wlan_ip_addr.ipv6_prefix_len = prefix_length;
  ...
  LOG_MSG_ERROR_INT_0("ps_iface_set_wlan_ipv6_addr: Failed to set ipv6 address");
  
  return -1;
} /* ps_iface_set_wlan_ipv6_addr() */

/*===========================================================================
...
)
{
                    ...
                    ps_dpm_get_dpm_rm_info_by_iface( this_iface_ptr )) )
  {
    ...
    if ( RMNET_ENABLE_DATA_AGG_MBIM == dpm_rm_info_ptr->dl_data_agg_protocol)
    {
  ...
  {      
    ...
    set_data_path.data_path    = PS_IFACE_IOCTL_DATA_PATH_SOFTWARE;
    set_data_path.client_type = PS_IFACE_IOCTL_CLIENT_DFS;
  ...
  else
  {
    enable = FALSE;
    set_data_path.client_type = PS_IFACE_IOCTL_CLIENT_DFS;
    set_data_path.data_path    = PS_IFACE_IOCTL_DATA_PATH_HARDWARE;
  ...
  base_um_iface_ptr = ps_iface_bridge_iface(this_iface_ptr);
    ...
    if(-1 ==  ps_iface_ioctl( assoc_iface_ptr,
                            ...
                            ps_errno))
  {
    LOG_MSG_ERROR_INT_1("Data path switch to %d failed",set_data_path.data_path);
  }
  ...
  LOG_MSG_ERROR_INT_2( "ps_iface_set_powersave_filtering_mode: result.errno=%d,"
                   " ps_errno=%d", err_no, 
...
)
{
  ...
  {
    DS_UTILS_ERROR_FATAL("NULL paraemter in ps_iface_get_ipv6_addr()");
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch(ip_addr_ptr->type)
  {
    ...
    case IPV6_ADDR:
      ...
      {
        v6_addr_ptr = PS_IFACE_GET_V6_PRIMARY_ADDR_PTR(base_iface_ptr);
        if( (v6_addr_ptr == NULL) ||
            ((v6_addr_ptr->prefix == 0) &&(v6_addr_ptr->iid == 0)))
        {
...
/*===========================================================================
FUNCTION PS_IFACE_GET_CACHED_IPV4_ADDR()

DESCRIPTION
  This function will return iface_private.ipv4_addr member of the base iface.
...
DEPENDENCIES
  This function performs no input validation

SIDE EFFECTS
  None
===========================================================================*/
uint32 ps_iface_get_cached_ipv4_addr
(
  ps_iface_type *this_iface_ptr
)
{
  uint32 ret_val = 0; /* return value*/
  ps_iface_type       * base_iface_ptr;  /* iface with ip addr stored locally*/
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  ret_val = base_iface_ptr->iface_private.ipv4_addr;
 
  PS_LEAVE_CRIT_SECTION(&global_ps_crit_section);

  return ret_val;
} /* ps_iface_get_cached_ipv4_addr() */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    PS_BRANCH_LEAVE_CRIT_SECTION(&global_ps_crit_section);
    LOG_MSG_ERROR_INT_0("ps_iface_set_addr_family(): "
...
#ifdef FEATURE_DATA_PS_IPV6
    /*----------------------------------------------------------------------
      Clear the public address
    -----------------------------------------------------------------------*/
    v6_addr_ptr = PS_IFACE_GET_V6_PRIMARY_ADDR_PTR(this_iface_ptr);
    if(v6_addr_ptr != NULL)
    {
      PS_TIMER_FREE_HANDLE(v6_addr_ptr->pref_lifetimer_handle);
      PS_TIMER_FREE_HANDLE(v6_addr_ptr->valid_lifetimer_handle);
      PS_MEM_FREE(v6_addr_ptr);
      PS_IFACE_SET_V6_PRIMARY_ADDR_PTR(this_iface_ptr, NULL);
    }
    /*----------------------------------------------------------------------
      Clear the secondeary addresses in cache and destroy the cache
    -----------------------------------------------------------------------*/
    v6_addr_ptr = PS_IFACEI_SEC_V6_CACHE_GET_FIRST_NODE(this_iface_ptr);
    while(v6_addr_ptr != NULL)
    {
      PS_TIMER_FREE_HANDLE(v6_addr_ptr->pref_lifetimer_handle);
      PS_TIMER_FREE_HANDLE(v6_addr_ptr->valid_lifetimer_handle);
      PS_TIMER_FREE_HANDLE(v6_addr_ptr->unused_addr_timer_handle);
      PS_IFACEI_SEC_V6_CACHE_DELETE_NODE(this_iface_ptr,v6_addr_ptr);
      PS_MEM_FREE(v6_addr_ptr);
      v6_addr_ptr = PS_IFACEI_SEC_V6_CACHE_GET_FIRST_NODE(this_iface_ptr);
    }
    ps_ifacei_sec_v6_cache_destroy(this_iface_ptr);
#endif /* FEATURE_DATA_PS_IPV6 */
    break;
  ...
  case IFACE_IPV6_ADDR_FAMILY:
    /*------------------------------------------------------------------------
      Allocate the public address. All privacy addresses are allocated
    ...
    if(v6_addr_ptr == NULL)
    {
      ...
      if(NULL == v6_addr_ptr)
      {
        PS_BRANCH_LEAVE_CRIT_SECTION(&global_ps_crit_section);
        LOG_MSG_ERROR_INT_0("ps_iface_set_addr_family(): "
                        "No IPv6 address items available!");
        DS_UTILS_ASSERT(0);
        return -1;
      }
      PS_IFACE_SET_V6_PRIMARY_ADDR_PTR(this_iface_ptr, v6_addr_ptr);
    }
    else
    {  
    ...
    /*------------------------------------------------------------------------
      Initial the secondary v6 cache
    ------------------------------------------------------------------------*/
    v6_addr_ptr = PS_IFACEI_SEC_V6_CACHE_GET_FIRST_NODE(this_iface_ptr);
    while(v6_addr_ptr != NULL)
    {
    ...
    ps_ifacei_sec_v6_cache_init(this_iface_ptr);
  ...
  default:
    LOG_MSG_ERROR_INT_1("ps_iface_set_addr_family(): "
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  case IPV6_ADDR:
    ...
    -----------------------------------------------------------------------*/
#ifdef FEATURE_DATA_PS_IPV6
    v6_addr_ptr = PS_IFACE_GET_V6_PRIMARY_ADDR_PTR((ps_iface_type *)this_iface_ptr);
    if((v6_addr_ptr != NULL) && v6_addr_ptr->gateway_iid)
    {
      ip_addr_ptr->addr.v6.ps_s6_addr32[0] = ps_htonl(0xFE800000UL);
      ip_addr_ptr->addr.v6.ps_s6_addr32[1] = 0;
      ip_addr_ptr->addr.v6.ps_s6_addr64[1] = v6_addr_ptr->gateway_iid;
    }
    else
#endif /* FEATURE_DATA_PS_IPV6 */
    {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /*-------------------------------------------------------------------------
    ...
    iface chain.
  -------------------------------------------------------------------------*/
  DS_UTILS_ASSERT(ps_iface_get_addr_family(base_iface_ptr) == IPV4_ADDR);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (ps_errno == NULL)
  {
    DS_UTILS_ERROR_FATAL("ps_iface_bring_up_cmd_ex(): NULL ps_errno");
    return -1;
  }
  ...
  {
    DS_UTILS_ERROR_FATAL("ps_iface_bring_up_cmd_ex(): Invalid App priority");
    *ps_errno = DS_EINVAL;
    return -1;
  }
  ...
  LOG_MSG_INFO1_5("Bringing up iface 0x%x:%d, old priority 0x%x, "
                  "req app priority %d subs_id %d",
                  this_iface_ptr->name,
                  this_iface_ptr->instance,
  ...
  if ( ret_val == -1 && *ps_errno == DS_EOPNOTSUPP )
  {
    ...
    PS_BRANCH_LEAVE_CRIT_SECTION(&global_ps_crit_section);
    LOG_MSG_ERROR_INT_0("ps_iface_bring_up_cmd_ex(): "
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (ps_errno == NULL)
  {
    DS_UTILS_ERROR_FATAL("ps_iface_active_bring_up_cmd_ex(): NULL ps_errno");
    return -1;
  }
  ...
  case IFACE_LINGERING:
    ...
    else
    {
      *ps_errno = DS_EINVAL;
      ret_val = -1;
      DS_UTILS_ERROR_FATAL("ps_iface_active_bring_up_cmd_ex(): Invalid linger timer handle ");
    }
    break;
  ...
  case IFACE_DOWN:
        ...
        ( this_iface_ptr->bring_up_cmd_f_ptr_ex != NULL ) )
    {
      ...
      else
      {
        ...
        dpli_log_iface_desc_and_status( this_iface_ptr,NULL);
        *ps_errno = DS_EWOULDBLOCK;
      }
    }
    else
    {
      LOG_MSG_ERROR_INT_0("ps_iface_active_bring_up_cmd_ex(): "
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (ps_errno == NULL)
  {
    DS_UTILS_ERROR_FATAL("ps_iface_tear_down_cmd_ex(): NULL ps_errno");
    return -1;
  }
  ...
  LOG_MSG_INFO1_6("Tearing down iface 0x%x:%d, state %d, ref_cnt %d, "
                  "new priority 0x%x sub_id %d",
                  this_iface_ptr->name,
                  this_iface_ptr->instance,
                  PS_IFACEI_GET_STATE(this_iface_ptr),
                  PS_IFACE_GET_REF_CNT(this_iface_ptr),
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (ps_errno == NULL)
  {
    DS_UTILS_ERROR_FATAL("ps_iface_active_tear_down_cmd_ex(): NULL ps_errno");
    return -1;
  }
  ...
  LOG_MSG_INFO1_6("Active tear down iface 0x%x:%d, state %d, "
                  "priority- curr 0x%x, app %d sub_id %d",
                  this_iface_ptr->name,
                  this_iface_ptr->instance,
                  PS_IFACEI_GET_STATE(this_iface_ptr),
                  PS_IFACEI_GET_PRIORITY_MASK(this_iface_ptr),
    ...
    switch(this_iface_ptr->iface_private.state)
    {
      ...
      if( (this_iface_ptr->tear_down_cmd_f_ptr    == NULL) &&
        ...
        {
          LOG_MSG_ERROR_INT_0("ps_iface_active_tear_down_cmd_ex(): "
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (ps_errno == NULL)
  {
    DS_UTILS_ERROR_FATAL("ps_iface_arbitration_tear_down_cmd(): NULL ps_errno");
    return -1;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_UTILS_ERROR_FATAL("ps_iface_go_null_cmd_ex(): NULL parameter is passed");
    return -1;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (ps_errno == NULL)
  {
    DS_UTILS_ERROR_FATAL("ps_iface_go_null_cmd(): NULL parameter is passed");
    return -1;
  }
  ...
  switch(this_iface_ptr->iface_private.state)
  {
    ...
    case IFACE_LINGERING:
      ...
      if( (this_iface_ptr->tear_down_cmd_f_ptr    == NULL) &&
          (this_iface_ptr->tear_down_cmd_f_ptr_ex == NULL) )
      {
        LOG_MSG_ERROR_INT_0("ps_iface_go_null_cmd(): "
                        "No tear down command");
        ret_val = -1;
        *ps_errno = DS_EOPNOTSUPP;
        break;
      ...
      if ( this_iface_ptr->tear_down_cmd_f_ptr_ex != NULL )
      {
        (void) this_iface_ptr->tear_down_cmd_f_ptr_ex( 
                                               this_iface_ptr,
                                               PS_POLICY_MGR_LEGACY_PRIORITY,
                                               PS_NET_DOWN_REASON_NOT_SPECIFIED,
                                               client_data_ptr);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (ps_errno == NULL)
  {
    DS_UTILS_ERROR_FATAL("ps_ifacei_reset_cmd(): NULL parameter is passed");
    return -1;
  }
  ...
  LOG_MSG_INFO2_4("ps_ifacei_reset_cmd(): "
                  "if 0x%x:%d state %d subs_id %d",
                  this_iface_ptr->name,
                  this_iface_ptr->instance,
                  this_iface_ptr->iface_private.state,
                  PS_IFACE_GET_SUBS_ID(this_iface_ptr));
  ...
  switch(this_iface_ptr->iface_private.state)
  {
    ...
    case IFACE_LINGERING:
         ...
         ps_timer_is_running(this_iface_ptr->iface_private.iface_linger_timer))
      {
        ...
        if(timer_ret_val != PS_TIMER_SUCCESS)
        {
          LOG_MSG_ERROR_1("ps_ifacei_reset_cmd(): "
                          "Error canceling timer 0x%x",
                          this_iface_ptr->iface_private.iface_linger_timer);
          DS_UTILS_ASSERT(0);
        }
      ...
      if( (this_iface_ptr->tear_down_cmd_f_ptr    == NULL) &&
          (this_iface_ptr->tear_down_cmd_f_ptr_ex == NULL) )
      {
        LOG_MSG_ERROR_INT_0("ps_ifacei_reset_cmd(): "
                        "No tear down command");
        ret_val = -1;
        *ps_errno = DS_EOPNOTSUPP;
        break;
      ...
      {
        ret_val = ps_iface_active_tear_down_cmd_ex(this_iface_ptr,
                                                  ps_iface_get_app_priority(this_iface_ptr),
                                                   ps_errno,
                                                   client_data_ptr);
      }
      else
      {
        ...
        iface_out_of_use_info.proc_id = this_iface_ptr->iface_private.proc_id;
        iface_out_of_use_info.state   = PS_IFACEI_GET_STATE(this_iface_ptr);
        iface_out_of_use_info.netdown_reason = netdown_reason;
        iface_out_of_use_info.bearer_tech.rat_value =  DS_SYS_RAT_UNKNOWN;
        ...
        if (this_iface_ptr->out_use_cmd_f_ptr != NULL)
        {
          ...
          if (ret_val == 0)
          {
            ...
            /*----------------------------------------------------------------
             Post IFACE_RESET_EV to logical iface to bring down EPC iface
            ----------------------------------------------------------------*/
            ps_iface_generic_ind( this_iface_ptr,
                                  IFACE_RESET_EV,
                                  &iface_out_of_use_info);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (ps_errno == NULL)
  {
    DS_UTILS_ERROR_FATAL("ps_iface_phys_link_up_cmd(): NULL parameter is passed");
    return -1;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (ps_errno == NULL)
  {
    DS_UTILS_ERROR_FATAL("ps_iface_phys_link_down_cmd(): NULL parameter is passed");
    return -1;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  for (i = 0; i < count; i++)
  {
          ...
          sip_serv_addr_ptr_array[i].type)
    {
      ps_iface_free_sip_addr_list(&base_iface_ptr->sip_serv_addr_list);
      PS_BRANCH_LEAVE_CRIT_SECTION(&global_ps_crit_section);
      LOG_MSG_ERROR_INT_0("ps_iface_set_sip_serv_addr(): "
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_ERROR_INT_1("ps_iface_get_sip_serv_addr_ex(): "
                  "Error code %d", err_no);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (ps_errno == NULL)
  {
    LOG_MSG_ERROR_INT_0("ps_iface_set_domain_name_search_list(): "
                    "ps_errno is null");
    return -1;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (!PS_IFACE_IS_VALID( this_iface_ptr))
  {
    LOG_MSG_ERROR_1("ps_iface_disable_iface_linger(): "
                    "Invalid iface 0x%p", this_iface_ptr);
    DS_UTILS_ASSERT(0);
    return -1;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (timer_handle == PS_TIMER_INVALID_HANDLE)
  {
    LOG_MSG_ERROR_INT_0("ps_iface_enable_multicast(): "
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (ps_timer_is_running(this_iface_ptr->iface_private.querier_v1_timer))
  {
    ...
    if (timer_ret_val != PS_TIMER_SUCCESS)
    {
      LOG_MSG_ERROR_INT_0("ps_iface_disable_multicast(): "
...
)
{
  ...
  if ((tx_meta_info_ptr == NULL) ||
       ...
       (iface_ptr == NULL))
  {
    LOG_MSG_ERROR_INT_0("ps_iface_fill_next_hop_addr(): "
  ...
  switch(ip_pkt_info_ptr->ip_vsn)
  {
    ...
    case IP_V6:
      ...
      default:
      LOG_MSG_ERROR_INT_1("ps_iface_fill_next_hop_addr(): "
...
)
{
  ...
  DS_UTILS_ASSERT(global_iface_ptr_array_index <= MAX_SYSTEM_IFACES);
...
/*===========================================================================
FUNCTION PS_IFACE_GET_MSISDN

DESCRIPTION
  This function gets the MSISDN of the iface.

PARAMETERS
  iface_ptr : Target iface ptr

RETURN VALUE
  MSISDN : MSISDN of the iface
  0   : if iface is invalid
...
SIDE EFFECTS
  None
===========================================================================*/
ps_iface_msisdn_info_type * ps_iface_get_msisdn
(
  ps_iface_type      *iface_ptr
)
{
  ps_iface_msisdn_info_type    * msisdn_info_ptr = NULL;
  ps_iface_type                * base_iface_ptr = NULL;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ( PS_IFACE_IS_VALID(iface_ptr) )
  {
    ...
    msisdn_info_ptr = base_iface_ptr->msisdn_ptr;
  }

  return msisdn_info_ptr;
} /* ps_iface_get_msisdn() */

/*===========================================================================
FUNCTION PS_IFACE_SET_MSISDN

DESCRIPTION
  This function sets the MSISDN of the iface.

PARAMETERS
  iface_ptr : Target iface ptr
  msisdn     : MSISDN to set
...
SIDE EFFECTS
  None
===========================================================================*/
void ps_iface_set_msisdn
(
  ps_iface_type                  * this_iface_ptr,
  ps_iface_msisdn_info_type      * msisdn_ptr
)
{
  ps_iface_event_info_u_type           event_info;
  ps_iface_type                      * base_iface_ptr = NULL;
  ps_iface_type                      * companion_iface_ptr = NULL;
  ps_sys_msisdn_info_per_apn_type      msisdn_info_per_apn;
  int16                                ps_errno;
  ps_sys_ioctl_apn_iface_info_type     apn_iface_info;
  boolean                              generate_ev = FALSE;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  do
  {
    ...
    /*-------------------------------------------------------------------------
     Get the companion iface for given iface and enfore same settings
    -------------------------------------------------------------------------*/
    ps_iface_ioctl (this_iface_ptr,
                    PS_IFACE_IOCTL_GET_COMPANION_IFACE, 
                    &companion_iface_ptr, 
                    &ps_errno);
                    
    /*------------------------------------------------------------------------
      Retrieve MSISDN info from base iface.
    ------------------------------------------------------------------------*/
    base_iface_ptr = PS_IFACE_GET_BASE_IFACE(this_iface_ptr);
    if(NULL == base_iface_ptr->msisdn_ptr)
    {
      base_iface_ptr->msisdn_ptr = 
        ps_system_heap_mem_alloc(sizeof(ps_iface_msisdn_info_type));
      ...
      memset (base_iface_ptr->msisdn_ptr, 0x0, sizeof(ps_iface_msisdn_info_type));
      memscpy(base_iface_ptr->msisdn_ptr, sizeof(ps_iface_msisdn_info_type),
              msisdn_ptr, sizeof(ps_iface_msisdn_info_type));
      generate_ev = TRUE;      
    }
    else if(0 != memcmp(base_iface_ptr->msisdn_ptr, msisdn_ptr,
                        sizeof(ps_iface_msisdn_info_type)))
    {
      memscpy(base_iface_ptr->msisdn_ptr, sizeof(ps_iface_msisdn_info_type),
              msisdn_ptr, sizeof(ps_iface_msisdn_info_type));
      generate_ev = TRUE;
    ...
    if (TRUE == generate_ev)
    {
      event_info.extended_ip_config_change_mask = PS_IFACE_EXT_IP_CFG_MASK_MSISDN_PCO;
      ...
      if(!(PS_IFACE_IS_VALID(companion_iface_ptr) && 
         (NULL != companion_iface_ptr->msisdn_ptr) &&
         0 == memcmp(companion_iface_ptr->msisdn_ptr, msisdn_ptr,
                     sizeof(ps_iface_msisdn_info_type))))
      {                        
        /*--------------------------------------------------------------------
           Retrieve APN information from iface and generate sys msisdn 
           change event for any client interested in recieving msisdn change
           for apn
        --------------------------------------------------------------------*/       
        memset (&msisdn_info_per_apn, 0x0, sizeof(ps_sys_msisdn_info_per_apn_type));
      
        memset (&apn_iface_info, 0, 
                 sizeof(ps_sys_ioctl_apn_iface_info_type));
        apn_iface_info.iface_ptr = base_iface_ptr;

        /* Retrieve apn name from iface ptr*/          
        (void) ps_sys_ioctl_ex(PS_SYS_TECH_ALL, 
                               PS_SYS_IOCTL_GET_APN_NAME_FROM_IFACE, 
                               base_iface_ptr->iface_private.subs_id, 
                               &apn_iface_info, 
                               &ps_errno);

        memscpy( &(msisdn_info_per_apn.apn_name),
                 strlen(apn_iface_info.apn_name) + 1,
                 apn_iface_info.apn_name,
                 strlen(apn_iface_info.apn_name) + 1);                                    
                                      
        memscpy(&(msisdn_info_per_apn.msisdn_info), sizeof(ps_sys_msisdn_info_type),
                base_iface_ptr->msisdn_ptr, sizeof(ps_sys_msisdn_info_type));

       (void) ps_sys_event_ind_ex(PS_SYS_TECH_ALL,
                                  PS_SYS_EVENT_APN_MSISDN_INFO_CHANGE,
                                  base_iface_ptr->iface_private.subs_id, 
                                  &msisdn_info_per_apn,
                                  &ps_errno);
      }                                
    ...
    LOG_MSG_INFO1_3("ps_iface_set_msisdn(): Set MSISDN to %d on iface 0x%x:%d, ",
                    msisdn_ptr->msisdn[0],
                    this_iface_ptr->name,
                    this_iface_ptr->instance);
  } while (0);
...
/*===========================================================================
FUNCTION PS_IFACEI_SET_LOW_LATENCY_TIMER_HANDLE

DESCRIPTION
  This function sets the low latency timer handle 

PARAMETERS
  timer_hdl  : low latency timer handle
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_INFO1_1("ps_ifacei_set_low_latency_timer_handle: result=%d",ret_val);
  return ret_val;
...
PARAMETERS
  traffic_state           :  Indicateds low latency traffic status
  pkt_interval_time_in_ms :  The interval, in ms, between two low latency 
                             packets arrival
...
SIDE EFFECTS
  None
===========================================================================*/
void ps_ifacei_send_low_latency_traffic_status_ev
(
...
)
{
  ...
  if( 0 != ret_val )
  {
    LOG_MSG_ERROR_INT_1( "ps_ifacei_send_low_latency_traffic_status_ev:"
                     "Event indication failed with error %d"
                     "Discarding the event",ps_errno );
  }
} /* ps_ifacei_send_low_latency_traffic_status_ev */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  ps_send_cmd( PS_IFACE_LOW_LATENCY_EVENT_CMD,
               (void *)TRUE );
  
} /* ps_ifacei_low_latency_timer_expiry_cb */

/*===========================================================================
FUNCTION PS_IFACE_LOW_LATENCY_TRAFFIC_STATUS_CB()

DESCRIPTION
  When the packets match the low latency filters installed, this callback is
  called by IPA for indicating the start/continuation of ping traffic

  This function posts a low latency event command to PS task.
  starts/restarts/stops the low latency traffic status timer and sends
...
PARAMETERS
  filter_hdl: Handle of the matched low latency filter 
...
SIDE EFFECTS
  None
===========================================================================*/
void ps_iface_low_latency_traffic_status_cb( uint32 filter_hdl )
{
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ps_send_cmd( PS_IFACE_LOW_LATENCY_EVENT_CMD, (void *)FALSE );
}/* ps_iface_low_latency_traffic_status_cb */

/*===========================================================================
FUNCTION PS_IFACE_LOW_LATENCY_EVENT_HANLDER()

DESCRIPTION

  This function starts/restarts the low latency traffic status timer and sends
  out indication for every ping request/response. It also calculates time 
  difference between two ping request and sends the information in indication.
...
)
{
  ...
  ps_iface_ipfltr_handle_type timer_hdl;
  boolean                     is_timer_expiry;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  is_timer_expiry = (boolean)(uint32)user_data_ptr;
  
  if( FALSE == is_timer_expiry )
  {
    /*-------------------------------------------------------------------------
  ...
  {
    LOG_MSG_ERROR_INT_1("ps_iface_low_latency_event_handler: err_no=%d", err_no);
  }
} /* ps_iface_low_latency_event_handler */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  do
  {
    ...
    {
      LOG_MSG_ERROR_INT_1("ps_iface_go_null_cmd_ex_handler() : "
...
)
{
  ...
  if( iface_index  >= MAX_SYSTEM_IFACES)
  {
    LOG_MSG_ERROR_INT_1("ps_ifacei_get_iface_ptr(): "
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (!PS_IFACE_IS_VALID(ps_iface_ptr))
  {
    DS_UTILS_ERROR_FATAL("ps_iface_log_ip_config(): Invalid Iface passed");
  }

  LOG_MSG_INFO1_4("Iface: 0x%x:%d, Ip Config "
                  "container Validity: %d (Invalid=0,Valid=1) on sub %d",
                  ps_iface_ptr->name,
                  ps_iface_ptr->instance,
                  iface_ip_config_ptr->is_ip_cfg_valid,
                  PS_IFACE_GET_SUBS_ID(ps_iface_ptr));
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (!PS_IFACE_IS_VALID(ps_iface_ptr))
  {
    DS_UTILS_ERROR_FATAL("ps_iface_get_ip_config_from_iface(): Invalid Iface passed");
    return -1;
  }
  ...
  {
    LOG_MSG_ERROR_INT_0("source interface IP Address is invalid");
    PS_LEAVE_CRIT_SECTION(&global_ps_crit_section);
    return -1;
  }
  ...
  {
    v6_addr = PS_IFACE_GET_V6_PRIMARY_ADDR_PTR(ps_iface_ptr);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (!PS_IFACE_IS_VALID(ps_iface_ptr))
  {
    DS_UTILS_ERROR_FATAL("ps_iface_set_ip_config_to_iface(): Invalid Iface passed");
    return ret_val;
  }
  ...
  {
    LOG_MSG_ERROR_INT_0("ps_iface_set_ip_config_to_iface(): "
  ...
  do
  {
    ...
    {    
      if(-1 == ps_iface_set_sip_serv_addr(
                   ...
                   &ps_errno))
      {
        LOG_MSG_ERROR_INT_1("ps_iface_set_ip_config_to_iface(): "
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  -------------------------------------------------------------------------*/
  if (base_iface_ptr->operator_reserved_pco_list_ptr == NULL)
  {
    PS_BRANCH_LEAVE_CRIT_SECTION(&global_ps_crit_section);
    LOG_MSG_ERROR_1("ps_iface_get_operator_reserved_pco(): "
                    "Invalid operator_reserved_pco_ptr 0x%p",
                    base_iface_ptr->operator_reserved_pco_list_ptr);
    return -1;
  }
  else
  {
    memscpy(operator_reserved_pco_info_ptr,
           ...
           &(base_iface_ptr->operator_reserved_pco_list_ptr->pco_info[0]),
           sizeof(ps_iface_op_reserved_pco_type));
  }
...
/*===========================================================================
FUNCTION PS_IFACE_SET_OPERATOR_RESERVED_PCO_LIST
...
SIDE EFFECTS
  None
===========================================================================*/
int ps_iface_set_operator_reserved_pco_list
(
 ps_iface_type                        *this_iface_ptr,
 ps_iface_op_reserved_pco_list_type   *operator_reserved_pco_ex_info_ptr
)
{
  ps_iface_type                       *base_iface_ptr;
  ps_iface_type                       *companion_iface_ptr = NULL;  
  ps_iface_op_reserved_pco_list_type  *new_operator_reserved_pco_ex_info_ptr;
  ps_iface_event_info_u_type           event_info;
  ps_sys_pco_info_per_apn_type        *pco_info = NULL;
  int16                                ps_errno;
  boolean                              generate_sys_event = FALSE;
  ps_sys_ioctl_apn_iface_info_type     apn_iface_info;
  int                                  num_pcos = 0;
  int                                  cnt_pcos = 0;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  memset (&event_info, 0x0, sizeof(ps_iface_event_info_u_type));
  if (operator_reserved_pco_ex_info_ptr == NULL)
  {
    return -1;
  }
  ...
  /*------------------------------------------------------------------------
    1. Check if base iface's pco ex ptr is NULL, if its NULL then generate
       event. Store the pco ex ptr to iface.
    2. if base iface pco ex ptr is not NULL
       2.1 Compare the iface pco ex ptr with new pco ex ptr, if its same
           then do not generate the event and do not update iface's pco ex 
           info ptr
       2.2 if both pco are different then compare first pco information 
            of pco ex ptr and if they match  generate the old iface pco event
            with mask PS_IFACE_EXT_IP_CFG_MASK_OP_RES_PCO. Also generate new
            iface pco list eventwith mask PS_IFACE_EXT_IP_CFG_MASK_OP_RES_PCO_LIST
            Generate ps sys pco change event            
       2.3 Replace the iface pco information with new one
       2.4 Compare the pco information with that of companion iface. If its
           same then do not generate the ps sys pco change event
  -------------------------------------------------------------------------*/  
  ...
  if (base_iface_ptr->operator_reserved_pco_list_ptr == NULL)
  {
    /* If PCO Id 0xFF00 is there then generate extended ip config event */
    if (0 != operator_reserved_pco_ex_info_ptr->pco_info[0].pco_id)
    {
      event_info.extended_ip_config_change_mask |= 
    ...
    event_info.extended_ip_config_change_mask |= 
     PS_IFACE_EXT_IP_CFG_MASK_OP_RES_PCO_LIST;
     
    new_operator_reserved_pco_ex_info_ptr = 
      ps_system_heap_mem_alloc(sizeof(ps_iface_op_reserved_pco_list_type));

    if (new_operator_reserved_pco_ex_info_ptr == NULL)
    {
      PS_BRANCH_LEAVE_CRIT_SECTION(&global_ps_crit_section);
      return -1;
    }

    memscpy(new_operator_reserved_pco_ex_info_ptr,
           sizeof(*new_operator_reserved_pco_ex_info_ptr),
           operator_reserved_pco_ex_info_ptr,
           sizeof(*operator_reserved_pco_ex_info_ptr));
           
    base_iface_ptr->operator_reserved_pco_list_ptr = 
      new_operator_reserved_pco_ex_info_ptr;
      
    generate_sys_event = TRUE;
  }
  /*------------------------------------------------------------------------
    Step 2.  
  -------------------------------------------------------------------------*/    
  else
  {
    /*--------------------------------------------------------------------
      Step 2.1
    ---------------------------------------------------------------------*/   
    if (0 == memcmp(base_iface_ptr->operator_reserved_pco_list_ptr,
                operator_reserved_pco_ex_info_ptr,
                sizeof(*operator_reserved_pco_ex_info_ptr)))
    {
      PS_BRANCH_LEAVE_CRIT_SECTION(&global_ps_crit_section);
    ...
    else
    {
      ...
      if ( PS_IFACEI_OP_RES_PCO_IS_CHANGED(
        (&(base_iface_ptr->operator_reserved_pco_list_ptr->pco_info[0])), 
        (&(operator_reserved_pco_ex_info_ptr->pco_info[0]))))
      {
        event_info.extended_ip_config_change_mask |= 
         PS_IFACE_EXT_IP_CFG_MASK_OP_RES_PCO;       
      } 
      ...
      /*------------------------------------------------------------------
        Step 2.3
      -------------------------------------------------------------------*/  
      PS_SYSTEM_HEAP_MEM_FREE(base_iface_ptr->operator_reserved_pco_list_ptr);      
  ...
  -------------------------------------------------------------------------*/      
  if (PS_IFACE_IS_VALID(companion_iface_ptr) && 
      (NULL != companion_iface_ptr->operator_reserved_pco_list_ptr) &&
      (0 == memcmp(companion_iface_ptr->operator_reserved_pco_list_ptr,
                   operator_reserved_pco_ex_info_ptr,
                   sizeof(*operator_reserved_pco_ex_info_ptr))))
  {
    generate_sys_event = FALSE;
  }   

  if (TRUE == generate_sys_event)
  {
    pco_info = 
    ps_system_heap_mem_alloc(sizeof(ps_sys_pco_info_per_apn_type));
    ...
    memset (pco_info, 0x0, sizeof(ps_sys_pco_info_per_apn_type));
    ...
    memscpy( &(pco_info->apn_name),
             strlen(apn_iface_info.apn_name) + 1,
             apn_iface_info.apn_name,
             strlen(apn_iface_info.apn_name) + 1);

    for (cnt_pcos = 0; cnt_pcos < PS_IFACE_MAX_NUM_PCO; cnt_pcos++)
    {
      if(0 == new_operator_reserved_pco_ex_info_ptr->pco_info[cnt_pcos].pco_id)
      {
        continue;
      }

      memscpy( &(pco_info->apn_pco_info.pco_info[num_pcos]),
               sizeof(pco_info->apn_pco_info.pco_info[num_pcos]),
               &(new_operator_reserved_pco_ex_info_ptr->pco_info[cnt_pcos]),
               sizeof(new_operator_reserved_pco_ex_info_ptr->pco_info[cnt_pcos]));
      num_pcos++;
    }

    pco_info->apn_pco_info.num_pcos = num_pcos;
  ...
  if (pco_info != NULL)
  {
    PS_SYSTEM_HEAP_MEM_FREE(pco_info);
  } 
  
  return 0;

} /* ps_iface_set_operator_reserved_pco_list() */

/*===========================================================================
FUNCTION PS_IFACE_GET_OPERATOR_RESERVED_PCO_LIST
...
PARAMETERS
  this_iface_ptr: ptr to interface control block on which to operate on.
  ps_iface_op_reserved_pco_list_type: pointer to the operator reserved PCO info
...
SIDE EFFECTS
  None
===========================================================================*/
int ps_iface_get_operator_reserved_pco_list
(
  ps_iface_type                        *this_iface_ptr,
  ps_iface_op_reserved_pco_list_type   *operator_reserved_pco_ex_info_ptr
)
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  memset(operator_reserved_pco_ex_info_ptr,0,
         sizeof(ps_iface_op_reserved_pco_list_type));
  ...
  if (base_iface_ptr->operator_reserved_pco_list_ptr == NULL)
  {
    PS_BRANCH_LEAVE_CRIT_SECTION(&global_ps_crit_section);
    LOG_MSG_ERROR_1("ps_iface_get_operator_reserved_pco(): "
                    "Invalid operator_reserved_pco_list_ptr 0x%p",
                    base_iface_ptr->operator_reserved_pco_list_ptr);
    return -1;
  }
  else
  {
    memscpy(operator_reserved_pco_ex_info_ptr,
            sizeof(ps_iface_op_reserved_pco_list_type),
           base_iface_ptr->operator_reserved_pco_list_ptr,
           sizeof(ps_iface_op_reserved_pco_list_type));
  }
...
} /* ps_iface_get_operator_reserved_pco_list() */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_ERROR_INT_1("ps_iface_set_modem_reserved_pco(): "
                  "Error code %d",err_no);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_ERROR_INT_1("ps_iface_get_modem_reserved_pco(): "
                  "Error code %d",err_no);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_ERROR_INT_1("ps_iface_set_wlan_mac_addr(): "
                  "Error code %d", err_no);
...
/*===========================================================================
FUNCTION ps_ifacei_set_assoc_iface()

DESCRIPTION
  This macro sets the associated iface for a logical iface.
  It also sets the back_iface_ptr in the associated iface (if not NULL)
  to point back at this_iface. This is done to make it possible to travel
  backward through the association pointers simplifying the RX process.
...
RETURN VALUE
  None.
===========================================================================*/
void ps_ifacei_set_assoc_iface
(
  ps_iface_type* this_iface,
  ps_iface_type* assoc_iface
)
{
  ps_iface_type* old_assoc_iface;

  old_assoc_iface = this_iface->iface_private.assoc_iface_ptr;
  /* removing previous back pointer (if still relevant) */
  if(old_assoc_iface != NULL)
  {
    if(old_assoc_iface->iface_private.back_iface_ptr == this_iface)
    {
      old_assoc_iface->iface_private.back_iface_ptr = NULL;
    }
  }
  ...
  if(assoc_iface != NULL)
  {
    assoc_iface->iface_private.back_iface_ptr = this_iface;

    /*---------------------------------------------------------------------
      Inherit the REASSEMBLY flag from assoc_iface if inherit IP info
      if TRUE for this_iface.
      The flag always inherits from associate iface to this iface.
      Performing inherit twice will reset the REASSEMBLY flag on this iface.
      Thus make sure not to re inherit again if already done.
    ---------------------------------------------------------------------*/
    if (TRUE == this_iface->iface_private.inherit_ip_info &&
        FALSE == PS_IFACE_GET_REASSEMBLY_FLAG(this_iface))
    {
      PS_IFACE_SET_REASSEMBLY_FLAG(this_iface, 
                                   PS_IFACE_GET_REASSEMBLY_FLAG(assoc_iface));

      PS_IFACE_SET_REASSEMBLY_FLAG(assoc_iface, FALSE); 
    }
  }

   LOG_MSG_DATA_PATH_INFO_3("ps_ifacei_set_assoc_iface(): this_iface"
                            "%x:%d, do_reassembly = %d",
                            this_iface->name,
                            this_iface->instance,
                            this_iface->iface_private.do_reassembly);

  if(assoc_iface != NULL)
  {
       LOG_MSG_DATA_PATH_INFO_3("ps_ifacei_set_assoc_iface(): assoc_iface"
                                "%x:%d, do_reassembly = %d",
                                assoc_iface->name,
                                assoc_iface->instance,
                                assoc_iface->iface_private.do_reassembly);
  }

} /* ps_ifacei_set_assoc_iface() */
...
)
{
  ...
  do
  {
    switch(iface_ioctl_client_type)
    {
      case PS_IFACE_IOCTL_CLIENT_DSS:
        *switch_data_path_client = PS_IFACE_DSS_HW_PATH;
        break;

      case PS_IFACE_IOCTL_CLIENT_WDS:
        *switch_data_path_client = PS_IFACE_WDS_HW_PATH;
        break;

      case PS_IFACE_IOCTL_CLIENT_DFS:
        *switch_data_path_client = PS_IFACE_DFS_HW_PATH;
        break;
  ...
  LOG_MSG_INFO2_2("ps_ifacei_get_data_path_client(): "
                  "iface ioctl client %d data path client %d",
                  iface_ioctl_client_type, *switch_data_path_client );

  return result;
}/*ps_ifacei_get_data_path_client*/

/**
  @DPM will invoke this API to update iface bridged info.
 
  @param 
    ps_iface_ptr[in]            input iface
    iface_client[in]             iface client
    data_path[in]               data path info
  ...
  @dependencies 
    None
*/
void ps_iface_update_bridge_mask
(
  ps_iface_type                        * ps_iface_ptr,
  ps_iface_data_path_clients_enum_type   iface_client,
  ps_iface_ioctl_data_path_enum_type     data_path  
)
{
  ...
  /*-------------------------------------------------------------------------
   Get the companion iface for given iface and enfore same settings
  -------------------------------------------------------------------------*/
  ps_iface_ioctl (ps_iface_ptr,
                  PS_IFACE_IOCTL_GET_COMPANION_IFACE, 
                  &companion_iface_ptr, 
                  &ps_errno);
  
  LOG_MSG_INFO2_4("ps_iface_update_bridge_mask(): "
                  "iface client %d data path %d Iface 0x%x other Iface 0x%x",
                  iface_client, data_path, ps_iface_ptr, companion_iface_ptr);
  /*-------------------------------------------------------------------------
   * Reset/disable client bit in given physical and in logical iface 
   * delete filter from HW
   * Enforce same setting into companion iface if available
  -------------------------------------------------------------------------*/
  if ( PS_IFACE_IOCTL_DATA_PATH_SOFTWARE == data_path )
  {
    PS_IFACE_DISBALE_BRIDGING(ps_iface_ptr, iface_client)
    
    logical_iface_ptr = ps_iface_ptr->iface_private.back_iface_ptr;
    
    while ( PS_IFACE_IS_VALID(logical_iface_ptr) )
    {
      PS_IFACE_DISBALE_BRIDGING(logical_iface_ptr, iface_client)
            
      ps_iface_ipfltr_hw_delete_all_iface_filters(logical_iface_ptr, 
                                                  IP_FLTR_CLIENT_SOCKETS);
     logical_iface_ptr = logical_iface_ptr->iface_private.back_iface_ptr;
    ...
    if (PS_IFACE_IS_VALID(companion_iface_ptr))
    {
      PS_IFACE_DISBALE_BRIDGING(companion_iface_ptr, iface_client)
        
      logical_iface_ptr = companion_iface_ptr->iface_private.back_iface_ptr;
  ...
  /*-------------------------------------------------------------------------
   * Set/Enable client bit in given physical and in logical iface chain 
   * Add filter back to HW
   * Enforce same setting into companion iface if available
  -------------------------------------------------------------------------*/  
  else if ( PS_IFACE_IOCTL_DATA_PATH_HARDWARE == data_path )
  {
    PS_IFACE_ENABLE_BRIDGING(ps_iface_ptr, iface_client)

    logical_iface_ptr = ps_iface_ptr->iface_private.back_iface_ptr;
    
    while (PS_IFACE_IS_VALID(logical_iface_ptr))
    {
      PS_IFACE_ENABLE_BRIDGING(logical_iface_ptr, iface_client)

      ps_iface_ipfltr_hw_add_all_iface_filters(logical_iface_ptr,
                                               IP_FLTR_CLIENT_SOCKETS);
    ...
    if (PS_IFACE_IS_VALID(companion_iface_ptr))
    {
      PS_IFACE_ENABLE_BRIDGING(companion_iface_ptr, iface_client)
...
}/*ps_iface_update_bridge_mask()*/

/*===========================================================================
FUNCTION PS_IFACE_GET_PROFILES_IN_USE


DESCRIPTION
   The function will return current profiles used by Ifaces.
   
PARAMETERS
  profile_type   : target iface
  prof_3gpp      : 3ggp profiles array
  num_prof_3gpp  : number of 3gpp profiles
  prof_3gpp2     : 3ggp2 profiles array
  num_prof_3gpp2 : number of 3gpp profiles
  
RETURN VALUE
  NONE
===========================================================================*/

void ps_iface_get_profiles_in_use(int profile_type, uint32 *prof_3gpp, uint8 *num_prof_3gpp, uint32 *prof_3gpp2, uint8 *num_prof_3gpp2)
{
  #define DS_PROFILE_TECH_3GPP 0
  #define DS_PROFILE_TECH_3GPP2 1
  #define DS_PROFILE_TECH_3GPP_3GPP2 2
  ...
  for(; loop >= 0; loop--)
  {
	...
	{
	  if(PS_IFACE_IS_UP(iface_ptr))
	  {
	    if( (profile_type == DS_PROFILE_TECH_3GPP || profile_type == DS_PROFILE_TECH_3GPP_3GPP2) &&
			(iface_ptr->name == WLAN_IFACE || iface_ptr->name == UMTS_IFACE || iface_ptr->name == IWLAN_S2B_IFACE || 
			iface_ptr->name == IWLAN_3GPP_IFACE || iface_ptr->name == EMBMS_IFACE) )
	    {
	      *prof_3gpp = iface_ptr->iface_private.profile_id_3gpp;
		  LOG_MSG_ERROR_2("ps_iface_get_profiles_in_use(): "
                  "profile 3gpp num %d iface_ptr 0x%x", *prof_3gpp, iface_ptr);
		  prof_3gpp++;
		  (*num_prof_3gpp)++;
	    }
	    if((profile_type == DS_PROFILE_TECH_3GPP2 || profile_type == DS_PROFILE_TECH_3GPP_3GPP2) && 
			(iface_ptr->name == CDMA_SN_IFACE || iface_ptr->name == IWLAN_3GPP2_IFACE) )
	    {
	      *prof_3gpp2 = iface_ptr->iface_private.profile_id_3gpp2;
		  LOG_MSG_ERROR_2("ps_iface_get_profiles_in_use(): "
                  "profile 3gpp2 num %d iface_ptr 0x%x", *prof_3gpp2, iface_ptr);
		  prof_3gpp2++;
		  (*num_prof_3gpp2)++;
	    }
	  }
  ...
  PS_LEAVE_CRIT_SECTION(&global_ps_crit_section);
}/* ps_iface_get_profiles_in_use */

/*===========================================================================
FUNCTION PS_IFACE_CHECK_PROFILE_IN_USE


DESCRIPTION
   The function will return whether profile is in use or not.
   
PARAMETERS
  profile_num   : profile number
  prof_list     : profiles list to check
  len           : number of profiles
  
RETURN VALUE
  TRUE/FALSE
===========================================================================*/

boolean ps_iface_check_profile_in_use(uint32 profile_num, uint32 *prof_list, uint8 len)
{
  int i=0;

  for(i=0; i<len; i++)
  {
    if(profile_num == *prof_list)
    {
      LOG_MSG_ERROR_INT_1("ps_iface_check_profile_in_use(): "
                         "profile num %d in use", profile_num);
      return TRUE;
    }
    prof_list++;
  }
  return FALSE;
} /* ps_iface_check_profile_in_use */

/*===========================================================================
FUNCTION PS_IFACE_SET_3GPP_PROFILE_ID


DESCRIPTION
   The function set the profile id to Iface.
...
RETURN VALUE
  NONE
===========================================================================*/
void ps_iface_set_3gpp_profile_id( ps_iface_type *iface_ptr, uint32 profile_id )
{
   LOG_MSG_ERROR_2("ps_iface_set_3gpp_profile_id(): "
                  "profile 3gpp num %d  iface_ptr 0x%x", profile_id, iface_ptr);
   PS_IFACEI_SET_3GPP_PROFILE_ID(iface_ptr, profile_id);
} /* ps_iface_set_3gpp_profile_id */


uint8 ps_iface_get_bearer_id_ex_from_physlink
(
  ps_iface_type                    * ps_iface_ptr,
  ps_phys_link_type                * phys_link_ptr
)
{
  uint8                             bearer_id_ex = 0xFF;
  ps_phys_link_type               * default_link_ptr = NULL;
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
       ...
       PS_PHYS_LINK_IS_VALID(phys_link_ptr))
  {
    ...
    default_link_ptr = PS_IFACE_GET_PHYS_LINK(ps_iface_ptr);
    
    if ( phys_link_ptr == default_link_ptr)
    {
      bearer_id_ex =  0;
    }
    else
    {
      bearer_id_ex =  phys_link_ptr->bearer_id_ex;
    }
  }
  return bearer_id_ex;
}/*ps_iface_get_bearer_id_ex_from_physlink*/