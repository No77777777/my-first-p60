...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/netiface/src/ps_flow_ioctl.c#2 $
...
)
{
  ...
  int                    ret_val;
  int                    filter_index;
  ps_phys_link_type    * phys_ptr = NULL;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (ps_errno == NULL)
  {
    DS_UTILS_ERROR_FATAL("ps_flow_ioctl(): NULL parameter is passed");
    return -1;
  }
  ...
  switch (ioctl_name)
  {
    ...
    case PS_FLOW_IOCTL_QOS_GET_GRANTED_FLOW_SPEC2:
      ...
      phys_ptr = ps_flowi_get_phys_link(flow_ptr);
      if(NULL != phys_ptr)
      {
        get_flow_ptr->bearer_id = phys_ptr->bearer_id;
        LOG_MSG_INFO2_1("ps_flow_ioctl(): Received bearer id %d",get_flow_ptr->bearer_id);
      }
    ...
    case PS_FLOW_IOCTL_QOS_GET_GRANTED_FILTER_SPEC:
           ...
           filter_index++)
      {
        ps_iface_log_qos_spec_filter_params (
          &get_filter_ptr->rx_filter_spec.list_ptr[filter_index], filter_index);
      }
      ...
      {
        ps_iface_log_qos_spec_filter_params (
          &get_filter_ptr->tx_filter_spec.list_ptr[filter_index], filter_index);
      }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (ps_errno == NULL)
  {
    DS_UTILS_ERROR_FATAL("ps_flow_default_logical_flow_ioctl_hdlr(): NULL errno");
    return -1;
  }
  ...
  {
    LOG_MSG_ERROR_INT_0("ps_flow_default_logical_flow_ioctl_hdlr(): "
