...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/netiface/src/ps_route.c#3 $ $DateTime: 2021/02/05 03:34:20 $ $Author: pwbldsvc $
...
static int16 ps_route_get_prof_num_by_apn_info
(
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  ACL_TRACE_DUMP_6(ACL_LEVEL_MED,
                      "Policy_Flag %d, Iface 0x%x:%x, "
                      "IP_family %d, proc_id %d subs_id %d",
  ...
  if((DS_PROFILE_REG_RESULT_SUCCESS == profile_status) && (NULL != profile_hndl))
  {
   ...
   (void)ds_profile_get_param(profile_hndl, 
...
)
{
  ...
  -------------------------------------------------------------------------*/
  DS_UTILS_ASSERT( destination.type == IPV4_ADDR );
  DS_UTILS_ASSERT( netmask.type == IPV4_ADDR );
  DS_UTILS_ASSERT( gateway.type == IPV4_ADDR );
...
)
{
  switch (deny_reason)
  {
    ...
    case ACL_DENY_REASON_IMS_SETTINGS_DISABLED:
      return PS_NET_DOWN_REASON_IMS_SETTINGS_DISABLED;
    case ACL_DENY_REASON_IFACE_AND_POL_FAMILY_MISMATCH:
      return PS_NET_DOWN_REASON_IFACE_AND_POL_FAMILY_MISMATCH; 
    case ACL_DENY_REASON_IP_V6_ONLY_ALLOWED:
      return PS_NET_DOWN_REASON_IP_V6_ONLY_ALLOWED;
    case ACL_DENY_REASON_IP_V4_ONLY_ALLOWED:
      return PS_NET_DOWN_REASON_IP_V4_ONLY_ALLOWED;
    case ACL_DENY_REASON_POL_NO_MATCH:
    default:
      return PS_NET_DOWN_REASON_NOT_SPECIFIED;
  }
...
)
{
  ...
  DS_UTILS_ASSERT( NULL != params_ptr );
  DS_UTILS_ASSERT( NULL != deny_reason );
      ...
      (NULL == result_ptr) )
  {
    *deny_reason = ACL_DENY_REASON_POL_NO_MATCH;
    DS_UTILS_ERROR_FATAL("ps_route_local_acl_policy_search_list():Invalid args");
    return -1;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ( -1 == ps_route_local_acl_policy_search_list( ROUTE_ACL_DEFAULT_OUTBOUND,
                                                    ...
                                                    deny_reason) )
  {
    LOG_MSG_ERROR_INT_1("ps_route_local_acl_policy_search(): "
  ...
  if( acl_ptr )
  {
    LOG_MSG_INFO2_4("ps_route_local_acl_policy_search(): "
                    "ACL Srch (phys): Found ACL match on iface 0x%x:%d with priority %d subs_ID %d",
                    ACL_IF_PTR(acl_ptr)->name,
                    ACL_IF_PTR(acl_ptr)->instance,
                    rt_result.priority,
                    PS_IFACE_GET_SUBS_ID(ACL_IF_PTR(acl_ptr)));
    /*----------------------------------------------------------------------
      Preserve proxy iface attributes if specified
    -----------------------------------------------------------------------*/
    *result_ptr = params_ptr->acl_pi_ptr->rt_result;
  ...
  if ( (*deny_reason == ACL_DENY_REASON_V4_DISALLOWED_SUBS_LIMITED ||
      *deny_reason == ACL_DENY_REASON_IP_VERSION_MISMATCH)  && 
       !(params_ptr->acl_pi_ptr->exclude_clat_routing) &&
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_UTILS_ASSERT( NULL != params_ptr);
  DS_UTILS_ASSERT( NULL != net_down_reason);
  ...
  if (params_ptr->acl_pi_ptr->iface.kind == DSS_IFACE_ID)
  {
    ...
    {
      LOG_MSG_ERROR_INT_1("ps_route_network_selection_by_policy(): Invalid iface ID: %d",
                    params_ptr->acl_pi_ptr->iface.info.id);
      *net_down_reason = PS_NET_DOWN_REASON_VALIDATION_FAILURE;
      PS_LEAVE_CRIT_SECTION(&global_ps_crit_section);
      return -1;
    }
    ...
    {
      LOG_MSG_ERROR_INT_0("ps_route_network_selection_by_policy(): "
    ...
    for (index = 0; index < (uint32) ACL_PROC_MAX; index++)
    {
      ...
      if (_acl_priority_cb_tbl[ index ])
      {
        ...
        if (-1 == _acl_priority_cb_tbl[ index ]( params_ptr,
                                                 ...
                                                 &deny_reason ) )
        {
          LOG_MSG_ERROR_INT_1("ps_route_network_selection_by_policy(): "
  ...
  {
    LOG_MSG_INFO2_4("ps_route_network_selection_by_policy(): "
                    "RT: Found ACL match on iface 0x%x:%d with priority %d subs_id %d",
                    ACL_IF_PTR(acl_match_ptr)->name,
                    ACL_IF_PTR(acl_match_ptr)->instance,
                    params_ptr->acl_pi_ptr->rt_result.priority,
                    PS_IFACE_GET_SUBS_ID(ACL_IF_PTR(acl_match_ptr)));
...
)
{
  ...
  if(ip_pkt_ptr)
  {
    DS_UTILS_ASSERT( ip_pkt_ptr->ip_vsn == IP_V4 || ip_pkt_ptr->ip_vsn == IP_V6);
  }
  *if_ptr = NULL;
  ...
  if(ip_pkt_ptr == NULL)
  {
    ...
    {
      DS_UTILS_ERROR_FATAL("route_get(): Null IP pkt info, addr routing not allowed");
      return -1;
    }
  }
  ...
  /*-------------------------------------------------------------------
    Find Profile number if acl policy bring_up_using_apn_name and/or type 
    are TRUE
  --------------------------------------------------------------------*/
  if(acl_pi_ptr->bring_up_using_apn_name || acl_pi_ptr->bring_up_using_apn_type)
  {
    if( 0 != ps_route_get_prof_num_by_apn_info(acl_pi_ptr))
    {
      LOG_MSG_ERROR_0("ps_route_look_up_iface_by_policy(): "
                      "ps_route_get_prof_num_by_apn_info ERROR!");
    }
  }
...
)
{
  ...
  switch( priority )
  {
    ...
    case ROUTE_ACL_TOP_PRIORITY:
      LOG_MSG_ERROR_INT_0("route_acl_add(): "
                      "RT: Unsupported op" );
      break;
...
)
{
  ...
  *if_ptr = NULL;
  ...
  {
    DS_UTILS_ASSERT(0);
    LOG_MSG_ERROR_INT_1("route_acl_execute(): "
...
)
{
  ...
  if(acl_pi_ptr->subs_id < PS_SYS_DEFAULT_SUBS ||
     acl_pi_ptr->subs_id >= PS_SYS_SUBS_MAX)
  {
    LOG_MSG_ERROR_INT_1("route_scope_by_policy(): "
                    "Invalid subs  %d", acl_pi_ptr->subs_id);
    DS_UTILS_ASSERT(0);
    return (route_scope);
  }
  ...
  for (index = ROUTE_ACL_OUTBOUND_MIN; index < ROUTE_ACL_OUTBOUND_MAX; index++)
  {
    ...
    if (acl_list_ptr == NULL)
    {
      LOG_MSG_ERROR_INT_1("Couldn't find acl list at idx %d", index);
      DS_UTILS_ASSERT(0);
      return route_scope;
    }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch(ip_addr_ptr->type)
  {
    ...
    default:
      LOG_MSG_ERROR_INT_1("ps_route_lo_update(): "
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(acl_pi_ptr->subs_id < PS_SYS_DEFAULT_SUBS ||
     acl_pi_ptr->subs_id >= PS_SYS_SUBS_MAX)
  {
    LOG_MSG_ERROR_INT_1("ps_route_look_up_iface_by_policy(): "
                    "Invalid subs  %d", acl_pi_ptr->subs_id);
    DS_UTILS_ASSERT(0);
    return NULL;
  }
  ...
  if( num_iface_in_use_cfg_match > 0 && ds3g_get_enable_arbitration() )
  {
  ...
  if ( result == -1 && num_iface_in_use > 0 && ds3g_get_enable_arbitration() )
  {
    LOG_MSG_INFO1_1("ps_route_look_up_iface_by_policy(): "
                    "lookup: in_use > 0, ARBITRATE!, in_use [%d]",
                     num_iface_in_use );
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (ps_errno == NULL)
  {
    DS_UTILS_ERROR_FATAL("ps_route_bring_up_iface_by_policy_ex(): NULL ps_errno");
    return -1;
  }
  ...
  {
    *ps_errno = DS_EINVAL;
    DS_UTILS_ERROR_FATAL("ps_route_bring_up_iface_by_policy_ex(): NULL args");
    return -1;
  }
  ...
  {
    *ps_errno = DS_EINVAL;
    DS_UTILS_ERROR_FATAL("ps_route_bring_up_iface_by_policy_ex(): bring_up flag is not set");
    return -1;
  }

  if (net_down_reason == NULL)
  {
    DS_UTILS_ERROR_FATAL("ps_route_bring_up_iface_by_policy_ex(): NULL net_down_reason");
    return -1;
  }
  ...
  if(acl_pi_ptr->subs_id < PS_SYS_DEFAULT_SUBS ||
     acl_pi_ptr->subs_id >= PS_SYS_SUBS_MAX)
  {
    DS_UTILS_ERROR_FATAL("ps_route_bring_up_iface_by_policy_ex(): Invalid subs");
    return -1;
  }
  ...
  if(acl_pi_ptr->bring_up_using_apn_name || acl_pi_ptr->bring_up_using_apn_type)
  {
    ...
    {
      LOG_MSG_ERROR_0("ps_route_look_up_iface_by_policy(): "
                      "ps_route_get_prof_num_by_apn_info");
    }
  }
  ...
  *if_ptr = NULL;
  ...
  else if (num_iface_in_use_cfg_match > 0 && ds3g_get_enable_arbitration() )
  {
  ...
  else if (result == -1 && num_iface_in_use > 0 && ds3g_get_enable_arbitration())
  {
...
)
{
  ...
  if( proc_id >= temp )
  {
    DS_UTILS_ERROR_FATAL("ps_route_register_acl_policy_routing_func(): invalid args");
    return NULL;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  *if_ptr        = NULL;
  ...
  if(acl_pi_ptr->subs_id < PS_SYS_DEFAULT_SUBS ||
     acl_pi_ptr->subs_id >= PS_SYS_SUBS_MAX)
  {
    LOG_MSG_ERROR_INT_1("ps_route_data_path_lookup(): "
                    "Invalid subs  %d", acl_pi_ptr->subs_id);
    return -1;
  }
...
/*===========================================================================
FUNCTION PS_ROUTE_GET_PROF_NUM_BY_APN_INFO()

DESCRIPTION
  This function performs mapping between provided APN name and type from policy
  to a profile number
...
{
  int16                               ps_errno_3gpp = 0;
  int16                               ps_errno_3gpp2 = 0;
  ps_sys_ioctl_mapping_apn_info_type  apn_info_mapping;
  int16                               result_3gpp   = 0;
  int16                               result_3gpp2   = 0;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

/*-------------------------------------------------------------------------
  Look up profile number by APN type or APN name or both
-------------------------------------------------------------------------*/
  memset(&apn_info_mapping, 0 , sizeof(apn_info_mapping));

  apn_info_mapping.apn_name_is_valid = acl_pi_ptr->bring_up_using_apn_name;
  apn_info_mapping.apn_name_length = acl_pi_ptr->apn.length;
  memscpy(apn_info_mapping.apn_name,
          apn_info_mapping.apn_name_length,
          acl_pi_ptr->apn.name,
          acl_pi_ptr->apn.length);
  
  apn_info_mapping.apn_type_is_valid = acl_pi_ptr->bring_up_using_apn_type;
  if(acl_pi_ptr->bring_up_using_apn_type)
  {
    switch(acl_pi_ptr->apn_type)
    {
    case PS_IFACE_APN_TYPE_INTERNET:
      apn_info_mapping.apn_type = PS_SYS_APN_TYPE_INTERNET;
      break;
    case PS_IFACE_APN_TYPE_IMS:
      apn_info_mapping.apn_type = PS_SYS_APN_TYPE_IMS;
      break;
    case PS_IFACE_APN_TYPE_MMS:
      apn_info_mapping.apn_type = PS_SYS_APN_TYPE_MMS;
      break;
    case PS_IFACE_APN_TYPE_DUN:
      apn_info_mapping.apn_type = PS_SYS_APN_TYPE_DUN;
      break;
    case PS_IFACE_APN_TYPE_SUPL:
      apn_info_mapping.apn_type = PS_SYS_APN_TYPE_SUPL;
      break;
    case PS_IFACE_APN_TYPE_HIPRI:
      apn_info_mapping.apn_type = PS_SYS_APN_TYPE_HIPRI;
      break;
    case PS_IFACE_APN_TYPE_FOTA:
      apn_info_mapping.apn_type = PS_SYS_APN_TYPE_FOTA;
      break;
    case PS_IFACE_APN_TYPE_CBS:
      apn_info_mapping.apn_type = PS_SYS_APN_TYPE_CBS;
      break;
    case PS_IFACE_APN_TYPE_IA:
      apn_info_mapping.apn_type = PS_SYS_APN_TYPE_IA;
      break;
    case PS_IFACE_APN_TYPE_EMERGENCY:
      apn_info_mapping.apn_type = PS_SYS_APN_TYPE_EMERGENCY;
      break;
    default:
      LOG_MSG_ERROR_1("INVALID apn type %d", acl_pi_ptr->apn_type);
      break;
    }
  }

  apn_info_mapping.profile_num = ((uint16*)((uint32*)&acl_pi_ptr->pdp_info));
  result_3gpp = ps_sys_ioctl_ex(PS_SYS_TECH_3GPP,
                                PS_SYS_IOCTL_3GPP_GET_PROFILE_ID_FROM_APN_INFO,
                                acl_pi_ptr->subs_id,
                               &apn_info_mapping,
                               &ps_errno_3gpp);

  apn_info_mapping.profile_num = ((uint16*)&acl_pi_ptr->data_session_profile_id);
  result_3gpp2 = ps_sys_ioctl_ex(PS_SYS_TECH_3GPP2,
                                 PS_SYS_IOCTL_3GPP2_GET_PROFILE_ID_FROM_APN_INFO,
                                 acl_pi_ptr->subs_id,
                                &apn_info_mapping,
                                &ps_errno_3gpp2);

  if(result_3gpp && result_3gpp2)
  {
    LOG_MSG_ERROR_4("ps_route_get_prof_num_by_apn_info()"
                    "result_3gpp %d ps_errno_3gpp %d result_3gpp2 %d ps_errno_3gpp2 %d",
                    result_3gpp,
                    ps_errno_3gpp,
                    result_3gpp2,
                    ps_errno_3gpp2);
  ...
  LOG_MSG_INFO1_2("ps_route_get_prof_num_by_apn_info() 3gpp profile ID %d 3gpp2 profile ID %d",
                   (uint32)acl_pi_ptr->pdp_info,
                   acl_pi_ptr->data_session_profile_id);
  return 0;
}
