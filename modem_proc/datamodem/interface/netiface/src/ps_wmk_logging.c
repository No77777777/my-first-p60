...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/netiface/src/ps_wmk_logging.c#2 $
  $Author: pwbldsvc $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
   ...
   {
      LOG_MSG_ERROR_INT_0("ps_wmk_logging_event_report: Null physlink ptr");
      return -1;
   }
...
)
{
 ...
 /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
    ...
    if(um_phys_link_ptr == NULL)
    {
       LOG_MSG_ERROR_INT_0("ps_wmk_logging_event_report_ex: Null physlink ptr");
       return -1;
    }

    if(iface_ptr == NULL)
    {
       LOG_MSG_ERROR_INT_0("ps_wmk_logging_event_report_ex: Null iface ptr");
       return -1;
    }
    ...
    if(iface_ptr->name == UMTS_IFACE)
    {
      ...
      ps_wmk_logging_events_info_ex.addr_family = 
        ...
        LOG_MSG_ERROR_INT_2("ps_wmk_logging_event_report_ex() profile id = %d,"
                        "addr family = %d",
                         ps_wmk_logging_events_info_ex.profile_id,
                         ps_wmk_logging_events_info_ex.addr_family);
    ...
    {
      LOG_MSG_ERROR_INT_0("ps_wmk_logging_event_report_ex() : "
...
)
{
 ...
 /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(um_phys_link_ptr == NULL)
  {
     LOG_MSG_ERROR_INT_0("ps_wmk_logging_modem_embedded_calls_info: Null physlink ptr");
     return -1;
  }
  ...
  {
    LOG_MSG_ERROR_INT_0("ps_wmk_logging_modem_embedded_calls_info() "
  ...
  if(iface_ptr == NULL)
  {
     LOG_MSG_ERROR_INT_0("ps_wmk_logging_modem_embedded_calls_info() Null iface ptr");
     return -1;
  }
  ...
  LOG_MSG_ERROR_INT_2("ps_wmk_logging_modem_embedded_calls_info() rx_wm_curr_cnt = %d,tx_wm_curr_cnt = %d",
                   rx_wm_curr_cnt,
                   tx_wm_curr_cnt);
...
)
{
  ...
  LOG_MSG_ERROR_INT_1("ps_wmk_logging_first_reorig_ip_pkt_hdr(): "
                  "Error code %d", err_no);
...
)
{
  ...
  LOG_MSG_ERROR_INT_1("ps_wmk_logging_reorig_pkt(): "
                  "Error code %d", err_no);
