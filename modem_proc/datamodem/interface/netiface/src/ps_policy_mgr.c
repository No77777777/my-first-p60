...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/netiface/src/ps_policy_mgr.c#2 $
  $Author: pwbldsvc $ $DateTime: 2019/10/22 02:53:17 $
when        who    what, where, why
--------    ---    ----------------------------------------------------------
15/03/01    cx     Added ps_policy_mgr_delete_record API for QTF usage
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR_INT_1("ps_policy_mgr_init(): "
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  while (this_policy_ptr != NULL)
  {
    ...
    tmp_app_id_l = (uint32)(tmp_app_id & 0xFFFFFFFF);
    tmp_app_id_u = (uint32)(tmp_app_id >> 32);
    LOG_MSG_INFO1_5("ps_policy_mgr_get_priority(): "
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ((priority > PS_POLICY_MGR_PRIORITY_MAX) ||
      (priority <= PS_POLICY_MGR_PRIORITY_INVALID))
  {
    LOG_MSG_ERROR_INT_1("ps_policy_mgr_add_record(): "
  ...
  if (policy_list_size >= PS_POLICY_MGR_MAX_POLICY_ENTRIES )
  {
    ...
    LOG_MSG_ERROR_INT_0("ps_policy_mgr_add_record(): "
                    "DB is full");
    return -1;
  }
  ...
  tmp_app_id_l = (uint32)(tmp_app_id & 0xFFFFFFFF);
  tmp_app_id_u = (uint32)(tmp_app_id >> 32);
  LOG_MSG_INFO2_5("ps_policy_mgr_add_record(): "
                  "add policy: app_id [0x%x%x], profile [0x%x], prio [%d] --- pos [%d]",
                  tmp_app_id_u, tmp_app_id_l, profile_id, priority, tmp_rec_pos );
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR_INT_0("ps_policy_mgr_get_profile(): "
