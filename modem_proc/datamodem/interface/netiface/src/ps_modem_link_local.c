...
#define PS_MODEM_LINKLOCAL_MAX_PHYS_LINK_PER_IFACE                    1

/*---------------------------------------------------------------------------
  Maximum grant
---------------------------------------------------------------------------*/
#define PS_MODEM_LINKLOCAL_PHYS_LINK_NUM_ALLOWED_BYTES 0x50000
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /*-------------------------------------------------------------------------
    Create physlink
  -------------------------------------------------------------------------*/
  ps_phys_link_create(&ps_modem_link_local_v6_phys_link, PS_MODEM_LINKLOCAL_MAX_PHYS_LINK_PER_IFACE);
  ...
  if (-1 == result)
  {
    DS_UTILS_ERROR_FATAL("ps_modem_link_local_powerup_init(): Unable to create modem link local iface");
    return;
  }
...
/*===========================================================================
FUNCTION  ps_modem_link_local_phys_link_ioctl_hdlr
...
===========================================================================*/
int ps_modem_link_local_phys_link_ioctl_hdlr
(
...
)
{
  ...
  LOG_MSG_INFO2_2("ps_modem_link_local_phys_link_ioctl_hdlr(): "
                  "phys link ptr %p, ioctl_name %d",
                  phys_link_ptr,
                  ioctl_name);
  ...
  switch (ioctl_name)
  {
    ...
    {
      allowed_bytes_info_ptr = (ps_phys_link_ioctl_allowed_num_bytes_type*)argval_ptr;
      allowed_bytes_info_ptr->num_of_bytes = PS_MODEM_LINKLOCAL_PHYS_LINK_NUM_ALLOWED_BYTES;
    }
    break;

    case PS_PHYS_LINK_IOCTL_FLOW_CTL_ACK:
    {
      memset(&flow_ctl_params, 0x0, sizeof(flow_ctl_params));

      flow_ctl_params.ack_required = TRUE;
      flow_ctl_params.allowed_num_of_bytes = PS_MODEM_LINKLOCAL_PHYS_LINK_NUM_ALLOWED_BYTES;
      flow_ctl_params.flow_mask = DS_FLOW_PDCP_MASK;
      flow_ctl_params.is_flow_enable = TRUE;

      ps_phys_link_flow_ctl_cmd(phys_link_ptr, flow_ctl_params);
    }
    break;
#endif  /*FEATURE_DATA_FLOW_MGMT */
  ...
  return ret_val;
} /* ps_modem_link_local_phys_link_ioctl_hdlr() */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  v6_addr_ptr = PS_IFACE_GET_V6_PRIMARY_ADDR_PTR(&ps_modem_link_local_iface6);
  ...
  /*-------------------------------------------------------------------------
    Set Bearer ID
  -------------------------------------------------------------------------*/
  PS_PHYS_LINK_SET_BEARER_ID (&ps_modem_link_local_v6_phys_link , 0x0);
  /*-------------------------------------------------------------------------
     Set Phys link ioctl
   -------------------------------------------------------------------------*/
  ps_modem_link_local_v6_phys_link.ioctl_f_ptr = ps_modem_link_local_phys_link_ioctl_hdlr;
