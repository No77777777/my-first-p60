...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/netiface/src/ps_flow.c#2 $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (ps_errno == NULL)
  {
    DS_UTILS_ERROR_FATAL("ps_flow_activate_cmd(): NULL parameter passed");
    return -1;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (ps_errno == NULL)
  {
    DS_UTILS_ERROR_FATAL("ps_flow_configure_cmd(): NULL parameter passed");
    return -1;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (ps_errno == NULL)
  {
    DS_UTILS_ERROR_FATAL("ps_flow_resume_cmd(): NULL parameter passed");
    return -1;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (ps_errno == NULL)
  {
    DS_UTILS_ERROR_FATAL("ps_flow_suspend_cmd(): NULL parameter passed");
    return -1;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (ps_errno == NULL)
  {
    DS_UTILS_ERROR_FATAL("ps_flow_go_null_cmd(): NULL parameter passed");
    return -1;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_QTRACE_HFT_MED(PS_TAG_NETIFACE, PS_SYS_DEFAULT_SUBS,
                 "ps_flow_go_null_ind(): FLOW GO NULL IND 0x%x, state 0x%x, info code %d",
                 flow_ptr, PS_FLOWI_GET_STATE(flow_ptr), info_code);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(flow_ptr == NULL)
  {
    LOG_MSG_ERROR_INT_0("ps_flow_qos_modify_accepted_ind(): "
                    "NULL flow_ptr");
    return;
  }
  ...
  if (!PS_FLOWI_GET_CAPABILITY(flow_ptr, PS_FLOW_CAPABILITY_DEFAULT))
  {
    ...
    DS_UTILS_ASSERT(qos_info_ptr != NULL && qos_modify_info_ptr != NULL);
    ...
    if (PS_FLOWI_IS_MODIFY_RX_FLTR_SPEC_CREATED(flow_ptr) ||
        PS_FLOWI_IS_MODIFY_RX_FLTR_SPEC_MODIFIED(flow_ptr))
    {
      DS_UTILS_ASSERT(PS_FLOWI_GET_RX_FLTR_HANDLE(flow_ptr) == 0);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch (event)
  {
    case FLOW_PRIMARY_MODIFY_RESULT_EV:
      ...
      {
        PS_BRANCH_LEAVE_CRIT_SECTION(&global_ps_crit_section);
        DS_UTILS_ERROR_FATAL("ps_flow_generic_ind(): NULL parameter is passed");
        return;
      }
    ...
    default:
      PS_BRANCH_LEAVE_CRIT_SECTION(&global_ps_crit_section);
      LOG_MSG_ERROR_INT_1("ps_flow_generic_ind(): "
...
void ps_flowi_phy_link_flow_ctl
(
  ps_flow_type                * flow_ptr,
...
)
{
  ...
  LOG_MSG_INFO2_4("ps_flowi_phy_link_flow_ctl(): "
                  "phys_link_ptr 0x%x flow_type %d  mask 0x%x cnt %d",
                  phys_link_ptr,
                  flow_type,
                  flow_mask,
                  phys_link_ptr->phys_private.flow_flow_ctl_mask_cnt);
  
  if (  DS_FLOW_DISABLE == flow_type )
  {
    if ( 0 < phys_link_ptr->phys_private.flow_flow_ctl_mask_cnt)
    {
      phys_link_ptr->phys_private.flow_flow_ctl_mask_cnt++;
      return;
    }
...
#ifdef FEATURE_DATA_FLOW_MGMT
    ps_flowi_phy_link_flow_ctl(flow_ptr, flow_type, DS_FLOW_PHYS_LINK_FLOW_MASK);
#endif/* FEATURE_DATA_FLOW_MGMT*/ 
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (ps_errno == NULL)
  {
    LOG_MSG_ERROR_INT_0("ps_flow_set_assoc_flow(): "
                    "NULL errno");
    return -1;
  }
  ...
  {
    LOG_MSG_ERROR_INT_1("ps_flow_set_assoc_flow(): "
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_UTILS_ERROR_FATAL("ps_flow_get_rx_fltr_by_handle(): Invalid parameters are passed");
    return FALSE;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (fltr == NULL || prcd == NULL || next_handle == NULL)
  {
    DS_UTILS_ERROR_FATAL("ps_flow_get_rx_fltr_by_handle_ex(): Invalid parameters are passed");
    return FALSE;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (ps_errno == NULL)
  {
    DS_UTILS_ERROR_FATAL("ps_flow_get_rx_fltr_spec(): NULL parameter is passed");
    return FALSE;
  }

  if(ip_fltr_spec == NULL)
  {
    DS_UTILS_ERROR_FATAL("ps_flow_get_rx_fltr_spec(): Null value passed to get_all_rx_fltrs()");
    *ps_errno = DS_EINVAL;
    return FALSE;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(ip_fltr_spec == NULL)
  {
    LOG_MSG_ERROR_INT_0("ps_flow_set_rx_filter_spec(): "
...
/*===========================================================================
FUNCTION PS_FLOW_MATCH_FILTER()

DESCRIPTION
  Check if the flow has the specified filter spec

PARAMETERS
  flow_ptr                   : pointer to the flow
  ps_iface_filter_spec_type : pointer to the request
  fltr_req_dir: requested filter matching direction
...
SIDE EFFECTS
  None
===========================================================================*/
ps_iface_qos_fltr_match_dir_enum_type ps_flow_match_filter
(
  ps_flow_type                * flow_ptr,
  ps_iface_filter_spec_type   * fltr_spec_ptr,
...
)
{
  ...
  int                    i;
  uint8                  local_errno = 0;
  boolean                is_dl_matched = FALSE;
  boolean                is_ul_matched = FALSE;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(PS_QOS_FILTR_DIR_UL != fltr_req_dir)
  {
    do
    {
      /*-------------------------------------------------------------------------
  ...
  -------------------------------------------------------------------------*/
    for (i = 0; i < fltr_spec_ptr->rx_fltr_template.num_filters; i++)
    {
      rx_ip_filter_ptr = &(fltr_spec_ptr->rx_fltr_template.list_ptr[i]);
          if ( -1 == ps_utils_match_one_fltr_in_fltrspec(rx_ip_filter_ptr, &rx_filter_spec) )
          {
            /* Did not find any filter in the spec of the flow that matches
               this filter that the application specified notification => 
               The flow does not match application's specification */
            local_errno = 3;
            break;
          }
        }
      ...
      {
        if (0 < fltr_spec_ptr->rx_fltr_template.num_filters) 
        {
          local_errno = 4;
          break;
        }
  ...
  if(PS_QOS_FILTR_DIR_DL != fltr_req_dir &&
     !(PS_QOS_FILTR_DIR_BIDIR == fltr_req_dir && FALSE == is_dl_matched) )
  {
    do
    {
      local_errno = 0;
  ...
  -------------------------------------------------------------------------*/
    for (i = 0; i < fltr_spec_ptr->tx_fltr_template.num_filters; i++)
    {
    tx_ip_filter_ptr = &(fltr_spec_ptr->tx_fltr_template.list_ptr[i]);
          if ( -1 == ps_utils_match_one_fltr_in_fltrspec(tx_ip_filter_ptr,&tx_filter_spec) )
    {
      /* Did not find any filter in the spec of the flow that matches
         this filter that the application specified 
  ...
  {
    if (0 < fltr_spec_ptr->tx_fltr_template.num_filters) 
    {
          local_errno = 8;
          break;
    }
  ...
  LOG_MSG_INFO1_6("ps_flow_match_filter(): "
                  "Flow filters rx:%d, tx:%d, App filters rx:%d, tx:%d ps_flow 0x%p local errno %d",
                  num_rx_filters,
                  num_tx_filters,
                  fltr_spec_ptr->rx_fltr_template.num_filters,
                  fltr_spec_ptr->tx_fltr_template.num_filters,
                  flow_ptr,
                  local_errno);

  ret_dir = ps_utils_get_fltr_match_result(fltr_req_dir, is_ul_matched, is_dl_matched);
  LOG_MSG_INFO1_3("rx_ip_filter_ptr 0x%p tx_ip_filter_ptr  0x%p matched dir%d",
                   rx_ip_filter_ptr,
                   tx_ip_filter_ptr,
                   ret_dir);

  return ret_dir;
} /* ps_flow_match_filter() */
