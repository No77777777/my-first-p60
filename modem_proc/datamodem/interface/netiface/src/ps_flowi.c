...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/netiface/src/ps_flowi.c#3 $
  $Author: pwbldsvc $ $DateTime: 2022/11/03 23:46:02 $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  -------------------------------------------------------------------------*/
  *is_fltr_spec_valid &=
    ps_iface_ipfltr_validate_fltr_param_ex(client_id,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  *flow_ptr_ptr = (ps_flow_type *) ps_mem_get_buf(PS_MEM_PS_FLOW_TYPE);
  ...
  -------------------------------------------------------------------------*/
  (void) q_init(&((*flow_ptr_ptr)->flow_private.event_handle_q));

  (*flow_ptr_ptr)->flow_private.state        = FLOW_INIT;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR_INT_0("ps_flowi_bind_phys_link(): "
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  LOG_MSG_QTRACE_HFT_MED(PS_TAG_NETIFACE, PS_SYS_DEFAULT_SUBS,
                 "ps_flowi_unbind_phys_link(): Unbinding flow, 0x%x, from phys link, 0x%x",
                  flow_ptr, PS_FLOWI_GET_PHYS_LINK(flow_ptr));

  if (PS_FLOWI_GET_PHYS_LINK(flow_ptr) == NULL)
  {
    LOG_MSG_QTRACE_HFT_MED(PS_TAG_NETIFACE, PS_SYS_DEFAULT_SUBS,
                 "ps_flowi_unbind_phys_link(): Flow, 0x%x, is not bound to a phys link", flow_ptr);
    return 0;
  }
...
ps_phys_link_type * ps_flowi_get_phys_link(ps_flow_type * flow_ptr)
{
  ...
  {
    LOG_MSG_ERROR_INT_0("ps_flowi_get_phys_link(): "
                    "Invalid flow_ptr");
    return NULL;
  }
...
{
  ps_flowi_event_handle_type  * flow_ev_handle_ptr = NULL;
  ps_flowi_event_handle_type  * next_flow_ev_handle_ptr = NULL;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  flow_ev_handle_ptr = q_check(&flow_ptr->flow_private.event_handle_q);
  while (flow_ev_handle_ptr != NULL)
  {
    next_flow_ev_handle_ptr =
        q_next(&flow_ptr->flow_private.event_handle_q,
               &(flow_ev_handle_ptr->link));

    q_delete(&flow_ptr->flow_private.event_handle_q,
               &(flow_ev_handle_ptr->link));
    memset(flow_ev_handle_ptr, 0, sizeof(ps_flowi_event_handle_type));

    PS_SYSTEM_HEAP_MEM_FREE(flow_ev_handle_ptr);
    flow_ev_handle_ptr = next_flow_ev_handle_ptr;
  }

  /*-----------------------------------------------------------------------
    Destroy the event_handle_q
  -----------------------------------------------------------------------*/
  q_destroy(&flow_ptr->flow_private.event_handle_q);
...
(
  const ps_iface_filter_spec_type    *   net_initiated_qos_spec,
  ps_iface_addr_family_type              iface_addr_family,
  int16                              *   ps_errno
)
{
  ...
  DS_UTILS_ASSERT(iface_addr_family != IFACE_INVALID_ADDR_FAMILY);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (qos_req_param->qos_spec->field_mask & QOS_MASK_TX_FLOW)
  {
    ...
    if (qos_req_param->qos_spec->tx.fltr_template.num_filters > 0)
    {
...
{
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  DS_UTILS_ASSERT(ps_errno != NULL);
  if (qos_spec == NULL)
  {
    DS_UTILS_ERROR_FATAL("ps_flowi_store_qos_spec(): QOS_SPEC is NULL");
    *ps_errno = DS_EFAULT;
    return -1;
  }
  ...
  {
    DS_UTILS_ERROR_FATAL("ps_flowi_store_qos_spec(): flow_ptr or qos_info_ptr is NULL");
    *ps_errno = DS_EFAULT;
    return -1;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_UTILS_ERROR_FATAL("ps_flowi_store_qos_modify_spec(): QOS_MODIFY_SPEC is NULL");
    *ps_errno = DS_EFAULT;
    return -1;
  }
  ...
  -------------------------------------------------------------------------*/
  DS_UTILS_ASSERT(!PS_FLOWI_IS_QOS_MODIFY_IN_PROGRESS(flow_ptr));
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_UTILS_ERROR_FATAL("ps_flowi_delete_qos_spec(): QOS info pointer of flow pointer is NULL");
    return -1;
  }
  /*-------------------------------------------------------------------------
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_UTILS_ERROR_FATAL("ps_flowi_get_rx_fltr_by_handle_ex(): Invalid parameters are passed");
    return FALSE;
  }
