...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/netiface/src/ps_iface_logical_flow.c#2 $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (ps_errno == NULL)
  {
    DS_UTILS_ERROR_FATAL("ps_iface_create_logical_flows(): NULL errno");
    return -1;
  }

  if (ioctl_argval_ptr == NULL || out_flow_ptr_ptr == NULL)
  {
    DS_UTILS_ERROR_FATAL("ps_iface_create_logical_flows(): NULL params");
    *ps_errno = DS_EFAULT;
    return -1;
  }
  ...
  switch (ioctl_name)
  {
    case PS_IFACE_IOCTL_QOS_REQUEST:
    {
      ...
      {
        DS_UTILS_ERROR_FATAL("ps_iface_create_logical_flows(): NULL QoS spec");
        *ps_errno = DS_EFAULT;
        return -1;
      }
    ...
    case PS_IFACE_IOCTL_QOS_REQUEST_EX:
    {
      ...
      {
        LOG_MSG_ERROR_INT_2("ps_iface_create_logical_flows(): "
  ...
  for (cnt = 0; cnt < num_qos_flows; cnt++)
  {
    ...
    if (ret_val != 0)
    {
      LOG_MSG_ERROR_INT_1 ("ps_iface_create_logical_flows(): "
    ...
    if (default_flow_meta_info_ptr != NULL)
    {
      ...
      {
        LOG_MSG_ERROR_INT_0("ps_iface_create_logical_flows(): "
        ...
        (void) ps_iface_delete_flow(iface_ptr,
                                    out_flow_ptr_ptr[cnt],
                                    &tmp_errno);
	PS_TX_META_INFO_FREE(&logical_flow_meta_info_ptr);
  ...
  if (cnt != num_qos_flows)
  {
    for (--cnt; cnt >= 0; cnt--)
    {
      ps_flow_go_null_ind(out_flow_ptr_ptr[cnt], PS_EIC_NOT_SPECIFIED );
      (void) ps_iface_delete_flow(iface_ptr, out_flow_ptr_ptr[cnt], &tmp_errno);
      PS_TX_META_INFO_FREE(&(out_flow_ptr_ptr[cnt]->flow_private.logical_flow.mi_ptr));
    }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (ps_errno == NULL)
  {
    DS_UTILS_ERROR_FATAL("ps_iface_modify_logical_flows(): NULL errno");
    return -1;
  }

  if (ioctl_argval_ptr == NULL || modify_flow_ptr == NULL)
  {
    DS_UTILS_ERROR_FATAL("ps_iface_modify_logical_flows(): NULL params");
    *ps_errno = DS_EFAULT;
    return -1;
  }
