...
Copyright (c) 2005-2010,2015 Qualcomm Technologies Incorporated.
All Rights Reserved.
...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/netiface/src/ps_stat_flow.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_UTILS_ERROR_FATAL("ps_stat_get_flow_is_default(): Invalid stat value passed to ps_stat_get_flow_i_all");
  ...
  if (flow_ptr == NULL)
  {
    DS_UTILS_ERROR_FATAL("ps_stat_get_flow_is_default:Null value of flow_ptr passed");
    return E_NOT_ALLOWED ;
  }
  ...
  {
    LOG_MSG_ERROR_INT_0("ps_stat_get_flow_is_default(): "
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_UTILS_ERROR_FATAL("ps_stat_get_flow_instance_all(): Invalid stat value passed to ps_stat_get_flow_i_all");
    return E_NOT_ALLOWED ;
  }
  ...
  if (flow_ptr == NULL)
  {
    DS_UTILS_ERROR_FATAL("ps_stat_get_flow_instance_all:Null value of flow_ptr passed");
    return E_NOT_ALLOWED ;
  }
  ...
  if (return_value == NULL)
  {
    LOG_MSG_ERROR_INT_0("ps_stat_get_flow_instance_all(): "
                    "return_value should not be NULL ");
    return E_NOT_ALLOWED ;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR_INT_1("ps_stat_get_flow_control_block(): "
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (PS_STAT_INST_DESC_ALL == handle)
  {
    for (idx = 0; idx < MAX_SYSTEM_IFACES; ++idx)
    {
      ...
      if (NULL == log_pkt_ptr)
      {
        LOG_MSG_ERROR_INT_0("ps_stat_get_flow_stat_log_pkt(): "
  ...
  else
  {
    ...
    if (E_SUCCESS != ret)
    {
      LOG_MSG_ERROR_INT_1("ps_stat_get_flow_stat_log_pkt(): "
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR_INT_1("ps_stat_reset_flow(): "
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR_INT_0("ps_stat_get_flow(): "
