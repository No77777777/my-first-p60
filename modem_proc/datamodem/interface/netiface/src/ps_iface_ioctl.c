...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/netiface/src/ps_iface_ioctl.c#3 $ $DateTime: 2021/08/02 22:53:02 $ $Author: pwbldsvc $
...
)
{
  ...
  ps_iface_op_reserved_pco_list_type   * operator_reserved_pco_list_info_ptr = NULL;
  ps_dpm_switch_data_path_info_type      ps_dpm_data_path_info;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (ps_errno == NULL)
  {
    DS_UTILS_ERROR_FATAL("ps_iface_ioctl(): NULL args");
    return -1;
  }
  ...
  *ps_errno = DSS_SUCCESS;
  ...
  LOG_MSG_QTRACE_HFT_MED(PS_TAG_NETIFACE, PS_SYS_DEFAULT_SUBS, 
                         "ps_iface_ioctl(): IOCTL %d, iface 0x%x:%d",
                         ioctl_name, ps_iface_ptr->name, ps_iface_ptr->instance);
  ...
  switch(ioctl_name)
  {
    ...
    case PS_IFACE_IOCTL_GET_IPV4_SECO_DNS_ADDR:
      ...
      {
        LOG_MSG_ERROR_INT_0("ps_iface_ioctl(): "
                        "v4 iface required");
        *ps_errno = DS_EINVAL;
        goto bail;
      }
  ...
  switch (ioctl_name)
  {
    case PS_IFACE_IOCTL_GET_DATA_PATH_BRIDGE_MODE:
    {
      ...
      LOG_MSG_INFO1_1("ps_iface_ioctl(): Data path software %d",
                       is_data_path_software);
      if (1 == is_data_path_software)
      {
        data_path_info_ptr->data_path = PS_IFACE_IOCTL_DATA_PATH_SOFTWARE;
      }
      else if (0 == is_data_path_software)
      {
        data_path_info_ptr->data_path = PS_IFACE_IOCTL_DATA_PATH_HARDWARE;
      }
      else
      {
        *ps_errno = DS_EINVAL;
    ...
    case PS_IFACE_IOCTL_SET_DATA_PATH_BRIDGE_MODE:
    {
      ...
      if (PS_IFACE_IOCTL_DATA_PATH_HARDWARE == data_path_info_ptr->data_path)
      {     
        ps_dpm_data_path_info.data_path   = PS_DPM_DATA_PATH_HARDWARE;
      }
      else if (PS_IFACE_IOCTL_DATA_PATH_SOFTWARE == data_path_info_ptr->data_path)
      {
        ps_dpm_data_path_info.data_path =   PS_DPM_DATA_PATH_SOFTWARE;
      }      
      else
      {
        *ps_errno = DS_EFAULT;
      ...
      if (-1 == ps_ifacei_get_data_path_client(
                 data_path_info_ptr->client_type, 
                 &(ps_dpm_data_path_info.iface_client)))
      {
        *ps_errno = DS_EFAULT;
        goto bail;
      }
      
      ps_dpm_data_path_info.iface_ptr = PS_IFACE_GET_BASE_IFACE(ps_iface_ptr);
        
      ps_dpm_switch_data_path (&ps_dpm_data_path_info);
    ...
    /*-----------------------------------------------------------------------
      Get Data Path MTU of iface
    -----------------------------------------------------------------------*/
    case PS_IFACE_IOCTL_GET_PATH_MTU:
      if (NULL == argval_ptr)
      {
        *ps_errno = DS_EFAULT;
        goto bail;
      ...
      mtu = (uint32 *)argval_ptr;
      *mtu = ps_ip_calc_path_mtu( ps_iface_get_addr_family(ps_iface_ptr), 
                                  ps_iface_ptr );
      LOG_MSG_INFO2_1("ps_iface_ioctl(): "
                      "pMTU is %d", *mtu);
      break;
    ...
    /*-----------------------------------------------------------------------
      Get op res pco list of iface
    -----------------------------------------------------------------------*/
    case PS_IFACE_IOCTL_GET_MSISDN_INFO:
      if (NULL == argval_ptr || ps_iface_get_msisdn(ps_iface_ptr) == NULL)
      {
        *ps_errno = DS_EFAULT;
        goto bail;
      }

      msisdn_info_ptr = (ps_iface_msisdn_info_type *)argval_ptr;
      memscpy(msisdn_info_ptr, sizeof(ps_iface_msisdn_info_type), 
              ps_iface_get_msisdn(ps_iface_ptr), 
    ...
    /*-----------------------------------------------------------------------
      Get op res pco list of iface
    -----------------------------------------------------------------------*/
    case PS_IFACE_IOCTL_GET_OP_RES_PCO_LIST:
      if (NULL == argval_ptr)
      {
        *ps_errno = DS_EFAULT;
        goto bail;
      }
      operator_reserved_pco_list_info_ptr = 
        (ps_iface_op_reserved_pco_list_type *)argval_ptr;
      if( 0 != ps_iface_get_operator_reserved_pco_list
                (ps_iface_ptr,operator_reserved_pco_list_info_ptr) )
      {
        *ps_errno = DS_EINVAL;
        goto bail;
      }
    ...
    case PS_IFACE_IOCTL_GET_SUBNET_MASK:
      ...
      {
        *ps_errno = DS_EFAULT;
        LOG_MSG_ERROR_INT_0("NULL agval_ptr");
        goto bail;
      }
      ...
      if (!PS_IFACE_IS_VALID(ps_iface_ptr))
      {
        ...
        *ps_errno = DS_EFAULT;
        LOG_MSG_ERROR_INT_0("Invalid iface");
        goto bail;
      }
      ...
      case IFACE_COMING_UP:
          ...
          LOG_MSG_ERROR_INT_0("Subnet mask is invalid due to IFACE state");
        break;
    ...
    case PS_IFACE_IOCTL_REFRESH_DHCP_CONFIG_INFO:
      ...
      if (TRUE == ps_iface_addr_family_is_v4 (ps_iface_ptr))
      {
        ...
        ret_ip_addr.type = IPV4_ADDR;
        ps_iface_get_addr (ps_iface_ptr, &ret_ip_addr);
        DS_UTILS_ASSERT(IPV4_ADDR == ret_ip_addr.type);
    ...
    case PS_IFACE_IOCTL_GET_FLOW_LIST_FROM_FLTR:
      if (NULL == argval_ptr)
      {
         *ps_errno = DS_EFAULT;
         goto bail;
      ...
      if(-1 == ps_iface_get_flow_list_from_fltr(ps_iface_ptr,
                                   argval_ptr,
                                   ps_errno))
      {
         return -1;
   ...
   case PS_IFACE_IOCTL_GET_COMPANION_IFACE:
      if (NULL == argval_ptr)
      {
         *ps_errno = DS_EFAULT;
         goto bail;
      ...
      if (NULL == comp_iface_ptr &&
          UMTS_IFACE == PS_IFACE_GET_NAME(base_iface_ptr))
      {
            
        /*-------------------------------------------------------------------------
         Get the companion iface for 3GPP
        -------------------------------------------------------------------------*/
        ps_iface_ioctl (base_iface_ptr,
                        PS_IFACE_IOCTL_GET_3GPP_COMPANION_IFACE, 
                        &comp_iface_ptr, 
                        ps_errno);        
      }
      
      (*(ps_iface_type **)argval_ptr) = comp_iface_ptr;
    ...
    break;
/*
    case PS_IFACE_IOCTL_WLAN_START_DPD_PING:
      if (NULL == argval_ptr)
      {
         *ps_errno = DS_EFAULT;
         goto bail;
...
bail:
  LOG_MSG_ERROR_INT_1 ("ps_iface_ioctl err %d", *ps_errno);
  return -1;
