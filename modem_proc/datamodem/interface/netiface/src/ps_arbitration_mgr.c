...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/netiface/src/ps_arbitration_mgr.c#2 $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_UTILS_ERROR_FATAL("ps_arbitration_mgr_net_arbitrate(): NULL param");
    return NULL;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (iface_in_use_ptr_arr == NULL || acl_pi_ptr == NULL)
  {
    DS_UTILS_ERROR_FATAL("ps_arbitration_mgr_net_arbitrate_lookup(): NULL param");
    return NULL;
  }
  ...
  LOG_MSG_INFO2_3("ps_arbitration_mgr_net_arbitrate_lookup(): "
                  "lookup: arb for app_id [0x%x], profile [0x%x], arb list sz [%d]",
                  acl_pi_ptr->app_identifier,
                  acl_pi_ptr->data_session_profile_id,
                  num_iface_in_use);
