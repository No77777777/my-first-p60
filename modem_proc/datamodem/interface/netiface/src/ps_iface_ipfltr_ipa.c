...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/netiface/src/ps_iface_ipfltr_ipa.c#3 $
  $DateTime: 2021/10/29 02:22:08 $ $Author: pwbldsvc $

when        who    what, where, why
--------    ---    ----------------------------------------------------------
10/03/18    rj     CLAT hardware capability fix and enable modem CLAT by default for Talos
...
#define IPA_ICMP_ECHO_REQ_TYPE_VAL     8
#define IPA_ICMP_ECHO_REQ_CODE_VAL     0
#define IPA_ICMP_ECHO_RESP_TYPE_VAL    0
#define IPA_ICMP_ECHO_RESP_CODE_VAL    0

#define IPA_ICMPV6_ECHO_REQ_TYPE_VAL   128
#define IPA_ICMPV6_ECHO_REQ_CODE_VAL   0
#define IPA_ICMPV6_ECHO_RESP_TYPE_VAL  129
#define IPA_ICMPV6_ECHO_RESP_CODE_VAL  0

// This is the number of low latency filters to be installed on UL or DL
#define PS_IFACE_IPFLTR_IPA_MAX_LOW_LATENCY_FLTRS 4
...
#define PS_IFACE_IPFLTRI_IPA_ADD_ICMPV6_UL_RULE ps_iface_ifptri_ipa_add_icmpv6_ul_rule
ipa_ipfltr_eq_result_e ps_iface_ifptri_ipa_add_icmpv6_ul_rule
(
  ipa_ipfltr_rule_handle_t    ipa_rule_handle, 
  uint8                       ipa_icmp_offset,
  uint32                      icmp_type,
  uint32                      icmp_type_range
)
{
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  return
      ipa_ipfltr_add_ihl_offset_range_eq_16(ipa_rule_handle,
                                            ipa_icmp_offset,
                                            icmp_type,
                                            icmp_type_range);                                                                    
} /* ps_iface_ifptri_ipa_add_icmpv6_ul_rule */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch (next_hdr_prot)
  {
    ...
    case PS_IPPROTO_ICMP6:
    {
      ...
      {
        /*---------------------------------------------------------------
          Identify uplink icmpv6 filter for RS/NS/NA and install 
          range filter for type field to reduce number of filters in 
          IPA
        ---------------------------------------------------------------*/
        if (fltr_ptr->ip_vsn == IP_V6 &&
            fltr_ptr->next_prot_hdr.icmp.type >= 133 &&
            fltr_ptr->next_prot_hdr.icmp.type <= 136 &&
            fltr_ptr->next_prot_hdr.icmp.type_range > 0)
        {
          ipa_ret_val =
            PS_IFACE_IPFLTRI_IPA_ADD_ICMPV6_UL_RULE
            (
              ipa_rule_handle,
              IPA_OFFSET_ICMP_TYPE,
              ps_ntohs(fltr_ptr->next_prot_hdr.icmp.type),
              ps_ntohs(fltr_ptr->next_prot_hdr.icmp.type +
              fltr_ptr->next_prot_hdr.icmp.type_range)
            );           
        }
        else
        {        
    ...
    default:
    {
      ...
      DS_UTILS_ERROR_FATAL("ps_iface_ipfltri_ipa_add_next_hdr_prot_param(): Unsupported next hdr protocol");
      break;
    }
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  do
  {
    ...
    if (NULL == ipa_rule_handle)
    {
      LOG_MSG_ERROR_INT_0("ps_iface_ipfltri_ipa_add(): "
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  do
  {
    ...
    {
      LOG_MSG_ERROR_INT_1("ps_iface_ipfltri_ipa_add(): "
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR_INT_1("ps_iface_ipfltri_ipa_enable_tcp_ack_prioritization(): "
...
/*===========================================================================
FUNCTION PS_IFACE_IPFLTRI_IPA_ADD_LINK_LOCAL_FLTR()

DESCRIPTION
  This function adds V6 link local rule to IPA
...
===========================================================================*/
static void ps_iface_ipfltri_ipa_add_link_local_fltr
(
  void
)
{
  ipa_ipfltr_rule_handle_t         ipa_rule_handle[1];
  ipa_ipfltr_eq_result_e           ipa_ret_val;
  ipa_err_code_e                   ipa_err_code = IPA_SUCCESS;
  ps_iface_error_enum_type         err_type = PS_IFACE_SUCCESS;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  do
  {
    ...
    ---------------------------------------------------------------------*/  
    ipa_rule_handle[0] =
      ipa_ipfltr_alloc_rule_handle(IPA_IPFLTR_RULE_UL_STATIC_LOCAL,
                                   IPA_IPFLTR_IPV6,
                                   PS_IFACE_IPFLTR_RESULT_UL_STATIC_LOCAL,
                                   0);
    if (NULL == ipa_rule_handle[0])
    ...
    {
      err_type = PS_IFACE_ERROR_HW_FILTER_ERR;
      (void) ipa_ipfltr_remove_rules(1, ipa_rule_handle);
      break;
    }

  } while(0);

  LOG_MSG_INFO2_2("ps_iface_ipfltri_ipa_add_link_local_fltr(): "
                  "status %d ipa_err_code %d", err_type, ipa_err_code);  
  return;
} /* ps_iface_ipfltri_ipa_add_link_local_fltr() */


/*===========================================================================
FUNCTION PS_IFACE_IPFLTRI_IPA_ADD_LOW_LATENCY_FILTERS()

DESCRIPTION
  This function installs low latency filters on IPA for uplink
...
===========================================================================*/
static boolean ps_iface_ipfltri_ipa_add_low_latency_filters
(
  ipa_ipfltr_rule_type_e ipa_rule_type
)
{
  ipa_ipfltr_rule_handle_t  ipa_rule_handle[PS_IFACE_IPFLTR_IPA_MAX_LOW_LATENCY_FLTRS];
  uint32                    num_rules = 0; 
  uint32                    usr_data = 0;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  /*-------------------------------------------------------------------------
    1. Allocate IPA rule handles for four filters for specified type
       (  ICMP ECHO Request/Response + ICMPv6 ECHO Request/Response )
    2. Add corresponding rules
    3. Commit the rule handle array
  -------------------------------------------------------------------------*/
  do
  {
    if( ( ipa_rule_type != IPA_IPFLTR_RULE_UL_STATIC_LOW_LAT ) &&
        ( ipa_rule_type != IPA_IPFLTR_RULE_DL_LOW_LATENCY ) )
    {
      err_type = PS_IFACE_ERROR_INVALID_ARG;
      break;
    }
    
    memset(&filter, 0, sizeof(ip_filter_type));
    memset(ipa_rule_handle, 0, sizeof(ipa_ipfltr_rule_handle_t) *
                               PS_IFACE_IPFLTR_IPA_MAX_LOW_LATENCY_FLTRS );
    
    /* ------------------------------------------------------------------------
        Install filters to catch ICMP ECHO requests on uplink. 
        ICMP Echo request. Type = 0x8, Code = 0
        ---------------------------------------------------------------------*/
    filter.ip_vsn = IP_V4;
    ...
    filter.next_prot_hdr.icmp.type = IPA_ICMP_ECHO_REQ_TYPE_VAL;
    filter.next_prot_hdr.icmp.code = IPA_ICMP_ECHO_REQ_CODE_VAL;

    ipa_rule_handle[num_rules] =  ps_iface_ipfltri_ipa_add( 
                                                 &filter,
                                                 ipa_rule_type,
                                                 IPA_IPFLTR_RULE_RESULT_INVALID,
                                                 &usr_data );
    if ( ipa_rule_handle[num_rules] == NULL )
    {
      LOG_MSG_ERROR_INT_0("DBG: failed");
      err_type = PS_IFACE_ERROR_HW_FILTER_ERR;
      break; 
    }
    LOG_MSG_ERROR_INT_0("DBG: passed");
    num_rules++;

    /* ------------------------------------------------------------------------
        Install filters to catch ICMP ECHO response on uplink. 
        ICMP Echo response. Type = 0x0, Code = 0
        ---------------------------------------------------------------------*/
  /*-------------------------------------------------------------------------
    Fill only required fileds and carry over common fields from ICMP request
    -------------------------------------------------------------------------*/
    filter.next_prot_hdr.icmp.type = IPA_ICMP_ECHO_RESP_TYPE_VAL;
    filter.next_prot_hdr.icmp.code = IPA_ICMP_ECHO_RESP_CODE_VAL;
    ...
    /* ------------------------------------------------------------------------
        Install filters to catch ICMPV6 ECHO requests on uplink. 
        ICMP Echo request. Type = 0x80, Code = 0
        ---------------------------------------------------------------------*/
    filter.ip_vsn = IP_V6;
    ...
    filter.next_prot_hdr.icmp.type = IPA_ICMPV6_ECHO_REQ_TYPE_VAL;
    filter.next_prot_hdr.icmp.code = IPA_ICMPV6_ECHO_REQ_CODE_VAL;
    ...
    /* ------------------------------------------------------------------------
    Install filters to catch ICMPV6 ECHO response on uplink. 
    ICMP Echo response. Type = 0x81, Code = 0
    -------------------------------------------------------------------------*/
   /*-------------------------------------------------------------------------
    Fill only required fileds and carry over common fields from ICMP request
    -------------------------------------------------------------------------*/
    filter.next_prot_hdr.icmp.type = IPA_ICMPV6_ECHO_RESP_TYPE_VAL;
    filter.next_prot_hdr.icmp.code = IPA_ICMPV6_ECHO_RESP_CODE_VAL;
    ...
    /*-------------------------------------------------------------------------
     Passing filter client as a dummy value(sockets) so that it handles filters
     commit failure as desired.
    -------------------------------------------------------------------------*/
    if (0 != ps_iface_ipfltr_hw_commit_rules( ipa_rule_handle, 
                                              num_rules,
  ...
  LOG_MSG_INFO1_2("ps_iface_ipfltri_ipa_add_ul_low_latency_fltrs(): "
                  "result %d, total filters succeeded %d", 
                  err_type,
                  num_rules );

  return ( PS_IFACE_SUCCESS == err_type )?TRUE:FALSE;
} /* ps_iface_ipfltri_ipa_add_low_latency_fltrs() */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (NULL == ipa_rule_handle)
  {
    LOG_MSG_ERROR_INT_0("ps_iface_ipfltri_ipa_add_dl_clat_icmpv6_fltr(): "
                    "Couldn't alloc IPA rule handle");
    return NULL;
  }
  ...
  if (IPA_IPFLTR_SUCCESS != ipa_ret_val)
  {
    LOG_MSG_ERROR_INT_1("ps_iface_ipfltri_ipa_add_dl_clat_icmpv6_fltr(): "
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /*---------------------------------------------------------------------------
  Install STATIC low latency filters on uplink
  ---------------------------------------------------------------------------*/
  if ( FALSE == ps_iface_ipfltri_ipa_add_low_latency_filters
                (
...
#if 0
  /*---------------------------------------------------------------------------
  Install low latency filters on downlink
  ---------------------------------------------------------------------------*/
  if ( FALSE == ps_iface_ipfltri_ipa_add_low_latency_filters
                (
  ...
  {
    LOG_MSG_ERROR_INT_0("ps_iface_ipfltr_hw_init : Low latency indication cb failed");
    DS_UTILS_ASSERT(0);
    return;
  }

#ifdef FEATURE_DATA_PS_464XLAT  
  if(ps_iface_ipfltr_hw_get_clat_cap())
  {
  ...
  do
  {
        ...
        (client_id == IP_FLTR_CLIENT_CLAT))
    {
      ...
      if (PS_IFACE_IS_VALID(iface_ptr))
      {
        ...
        {
          base_iface_ptr = PS_DPM_GET_BASE_IFACE(iface_ptr);
        }
        else
        #endif /* FEATURE_DATA_PS_464XLAT */
        {
      ...
      else if (filter_buf_ptr->iface_group == RM_GROUP)
      {
        ...
        if (filter_buf_ptr->fi_result >= PS_IFACE_IPFLTR_RESULT_UL_UPLINK_MONITOR &&
            filter_buf_ptr->fi_result <= PS_IFACE_IPFLTR_RESULT_UL_UPLINK_MONITOR_MAX )
        {
          ipa_rule_type = IPA_IPFLTR_RULE_UL_STATIC_WWAN;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  LOG_MSG_INFO2_6("ps_iface_ipfltri_ipa_add_v6_frag_fltr(): "
                  "iface 0x%x:%d Prefix 0x%x%08x iid 0x%x%08x",
                  iface_ptr->name, 
                  iface_ptr->instance,                  
                  (uint32)(v6_addr_ptr->ps_s6_addr64[0] >> 32),
                  (uint32)(v6_addr_ptr->ps_s6_addr64[0]),
                  (uint32)(v6_addr_ptr->ps_s6_addr64[1] >> 32),
                  (uint32)(v6_addr_ptr->ps_s6_addr64[1]));
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (!PS_IFACE_IS_VALID(iface_ptr))    
  {
    LOG_MSG_ERROR_INT_0("ps_iface_ipfltr_hw_delete_all_iface_filters() : "
                    "Invalid iface");
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_INFO1_3("ps_iface_ipfltr_hw_add_clat_v6_dst_addr_fltr(): "
                  "Installing IPV6 address based fltr to IPA"
                  " iface 0x%x:%d on sub %d",
                   iface_ptr->name, 
                   iface_ptr->instance,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_INFO2_3("ps_iface_ipfltr_hw_add_ul_dest_prefix_comp_fltr(): "
                  "SIO Stream ID %d prefix 0x%x, prefix length %d", 
                  stream_id, 
                  prefix_info.prefix.ps_s6_addr64[0],
                  prefix_info.prefix_len);
  ...
  /*-------------------------------------------------------------------------
    ...
    Note that IPA expects prefix in host order
  -------------------------------------------------------------------------*/
  return ipa_ipfltr_add_ul_dest_ipv6_prefix_rule((sio_stream_id_type) stream_id,
                                                  prefix_info.prefix_len,
                                                  prefix_ipa,
                                                  ipa_fltr_result);
