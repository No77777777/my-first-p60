...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/netiface/src/ps_phys_link_io.c#2 $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_UTILS_ERROR_FATAL("ps_phys_link_tx_cmd(): Null packet to transmit");
    PS_TX_META_INFO_FREE(&meta_info_ptr);
    return -1;
  }
  ...
  switch(this_phys_link_ptr->hdr_comp_protocol)
  {
    case PS_PHYS_LINK_HDR_COMP_PROTOCOL_ROHC:
      if(rohc_compress(this_phys_link_ptr->hdr_comp_handle,
                       ...
                       &err) == -1)
      {
        LOG_MSG_ERROR_INT_1("ps_phys_link_tx_cmd(): "
    ...
    case PS_PHYS_LINK_HDR_COMP_PROTOCOL_IPHC:
      ...
      if(iphc_compress(this_phys_link_ptr->hdr_comp_handle,
                       ...
                       &iphc_pkt) == -1)
      {
        LOG_MSG_ERROR_INT_0("ps_phys_link_tx_cmd(): "
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(*pkt_ref_ptr != NULL)
  {
    ...
    -----------------------------------------------------------------------*/
    (void)ps_dl_phys_opt_rx_pkt(this_phys_link_ptr, pkt_ref_ptr, meta_info_ptr, NULL );
  }
