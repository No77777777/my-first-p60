...
$Header: //components/rel/data.mpss/3.5..2.1/interface/netiface/src/ps_hashtable.c#2 $
...
{                                                             \
  (entry_p)->next_ptr = NULL;                                     \
  (entry_p)->prev = NULL;                                     \
                                                              \
  if (tail_p)                                                 \
  {                                                           \
    (tail_p)->next_ptr = (entry_p);                               \
  }                                                           \
                                                              \
  (entry_p)->prev = (tail_p);                                 \
  (tail_p) = (entry_p);                                       \
...
{                                                             \
  if ((entry_p)->next_ptr)                                        \
  {                                                           \
    ((entry_p)->next_ptr)->prev =                                 \
     (entry_p)->prev;                                         \
  }                                                           \
  if ((entry_p)->prev)                                        \
  {                                                           \
    ((entry_p)->prev)->next_ptr =                                 \
     (entry_p)->next_ptr;                                         \
  }                                                           \
} 
...
)
{
  ...
  {
    DS_UTILS_ERROR_FATAL("ps_ht_powerup_init(): Instance mem init error");
    return;
  }
} /* ps_ht_powerup_init() */
...
)
{
 ...
 /*-------------------------------------------------------------------------
  ...
  while (lookup_node)
  {
    ...
    lookup_node = lookup_node->next_ptr;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/   
  ...
  {
    LOG_MSG_ERROR_INT_0("ps_hashtable_add(): Invalid handle");
    return FALSE;
  }

  if (NULL == value)
  {
    LOG_MSG_ERROR_INT_0("ps_hashtable_add(): NULL value pointer");
    return FALSE;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/   
  ...
  if (!HT_CB_IS_VALID(ht_cb_ptr))
  {
    LOG_MSG_ERROR_INT_0("ps_hashtable_lookup(): Invalid handle");
    return FALSE;
  }

  if (NULL == value)
  {
    LOG_MSG_ERROR_INT_0("ps_hashtable_lookup(): NULL value pointer");
    return FALSE;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/   
  ...
  if (!HT_CB_IS_VALID(ht_cb_ptr))
  {
    LOG_MSG_ERROR_INT_0("ps_hashtable_reset_state(): Invalid handle");
    return FALSE;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/   
  ...
  if (!HT_CB_IS_VALID(ht_cb_ptr))
  {
    LOG_MSG_ERROR_INT_0("ps_hashtable_get_next_key_value_pair(): Invalid handle");
    return PS_HT_FAILURE;
  }

  if (NULL == value && NULL == key)
  {
    LOG_MSG_ERROR_INT_0("ps_hashtable_get_next_key_value_pair(): NULL key & value pointer");
    return PS_HT_FAILURE;
  }

  if (NULL != ht_cb_ptr->last_node && 
      NULL != ht_cb_ptr->last_node->next_ptr)
  {
    /*-------------------------------------------------------------------------
      If there are mode nodes in the buckets, update last_node to new node.
    -------------------------------------------------------------------------*/
    ht_cb_ptr->last_node = ht_cb_ptr->last_node->next_ptr;
  }
  else
  {
    /*-------------------------------------------------------------------------
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/   
  ...
  if (!HT_CB_IS_VALID(ht_cb_ptr))
  {
    LOG_MSG_ERROR_INT_0("ps_hashtable_remove(): Invalid handle");
    return FALSE;
  }
...
)
{
 ...
 /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (!HT_CB_IS_VALID(ht_cb_ptr))
  {
    LOG_MSG_ERROR_INT_0("ps_hashtable_remove_all(): Invalid handle");
    return FALSE;
  }
  /*-------------------------------------------------------------------------
    Allocate memory to hold the control block
...
)
{
 ...
 /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (!HT_CB_IS_VALID(ht_cb_ptr))
  {
    LOG_MSG_ERROR_INT_0("ps_hashtable_destroy(): Invalid handle");
    return FALSE;
  }
