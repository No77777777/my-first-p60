...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/netiface/src/ps_aclrules.c#2 $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    if (!(this_if_ptr && ((ps_iface_type*)this_if_ptr)->name == CLAT_IFACE &&
        deny_reason == ACL_DENY_REASON_IP_VERSION_MISMATCH))
    {
      LOG_MSG_QTRACE_HFT_MED(PS_TAG_NETIFACE, PS_SYS_DEFAULT_SUBS,
                    "ACL 0x%x:%d DENIED, Reason %d",
                    ((ps_iface_type*)(this_if_ptr)?((ps_iface_type*)this_if_ptr)->name:0),
                    ((ps_iface_type*)(this_if_ptr)?((ps_iface_type*)this_if_ptr)->instance:0),
                    deny_reason);
    }
  }
  return;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (ACL_DEFAULT_CLASS != acl_rule_val) 
  {
    LOG_MSG_QTRACE_HFT_MED(PS_TAG_NETIFACE, PS_SYS_DEFAULT_SUBS,
                           "PASSING Iface 0x%x:%d with priority %d", ((ps_iface_type*)this_if_ptr)->name,
                           ((ps_iface_type*)this_if_ptr)->instance, acl_rule_val);
  }
  return;
} /* acl_trace_end() */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  ACL_TRACE_DUMP(ACL_LEVEL_LOW,
	 "IP_POL_FAMILY_IS: IP Pol Family %d, IP family %d, (No Match)",
	  ((policy_info_ptr)?(policy_info_ptr->ip_family):IP_ADDR_INVALID), ipfam, 0);
  return FALSE;
} /* ip_pol_family_is() */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  ACL_TRACE_DUMP(ACL_LEVEL_LOW,
                 "DIFF_IFACE_ID_REQD: Pol iface ID 0x%x, iface ID  0x%x(No Match)",
                 policy_info_ptr->iface.info.id & 0xFF000000, 
                 (PS_IFACE_GET_ID((ps_iface_type*) this_if_ptr) & 0xFF000000), 
                 0);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  ACL_TRACE_DUMP(ACL_LEVEL_LOW,
		 ...
		 "Pol iface name 0x%x iface name 0x%x (No Match)",
		 ((policy_info_ptr)?(policy_info_ptr->iface.info.name):0),
                 ((this_if_ptr)?((ps_iface_type*) this_if_ptr->name):0), 0);
  return FALSE;
