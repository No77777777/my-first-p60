...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/netiface/src/ps_stat_phys_link.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_UTILS_ERROR_FATAL("ps_stat_get_phys_link_is_primary(): Invalid stat value passed to ps_stat_get_phys_link_i_all");
  ...
  {
    LOG_MSG_ERROR_INT_0("ps_stat_get_phys_link_is_primary: "
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_UTILS_ERROR_FATAL("ps_stat_get_phys_link_instance_all(): Invalid stat value passed to ps_stat_get_phys_link_i_all");
    return E_NOT_ALLOWED ;
  }
  ...
  if ( PS_PHYS_LINK_IS_VALID(phys_link_ptr) )
  {
    LOG_MSG_ERROR_INT_0("ps_stat_get_phys_link_instance_all: "
                    "Invalid value of phys_link_ptr passed");
    return E_NOT_ALLOWED ;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR_INT_1("ps_stat_get_phys_link_control_block(): "
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
    ...
    for (idx_iface = 0; idx_iface < MAX_SYSTEM_IFACES; ++idx_iface)
    {
           ...
           ++idx_phys_link)
      {
        ...
        if (log_pkt_avail_size < desc_size)
        {
          ...
          if (NULL == log_pkt_ptr)
          {
            LOG_MSG_ERROR_INT_0("ps_stat_get_phys_link_desc_log_pkt(): "
  ...
  else
  {
    ...
    if (E_SUCCESS != ret)
    {
      LOG_MSG_ERROR_INT_1("ps_stat_get_phys_link_desc_log_pkt(): "
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (PS_STAT_INST_DESC_ALL == handle)
  {
    for (idx_iface = 0; idx_iface < MAX_SYSTEM_IFACES; ++idx_iface)
    {
           ...
           ++idx_phys_link)
      {
        ...
        /*---------------------------------------------------------------------
          Phys link is being taken from iface. So it shouldn't be null/invalid
        ---------------------------------------------------------------------*/
        if (NULL == phys_link_ptr)
        {
          LOG_MSG_ERROR_INT_0("ps_stat_get_phys_link_stat_log_pkt(): "
                          "Null phys link ptr");
          return;
        }
  ...
  else
  {
    ...
    if (E_SUCCESS != ret)
    {
      LOG_MSG_ERROR_INT_1("ps_stat_get_phys_link_stat_log_pkt(): "
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (ret != E_SUCCESS)
  {
    LOG_MSG_ERROR_INT_1("ps_stat_reset_phys_link(): "
...
)
{
  ps_phys_link_type         * phys_link_ptr = NULL;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  phys_link_ptr = (ps_phys_link_type *)instance_ptr;
  ...
  if ( PS_PHYS_LINK_IS_VALID(phys_link_ptr) )
  {
    LOG_MSG_ERROR_INT_0("ps_stat_get_phys_link(): "
                    " Instance pointer should NOT be invalid "
                    "for PHYS_LINK stats");
    return E_INVALID_ARG ;
  }
