...
Copyright (c) 2005-2012,2015 Qualcomm Technologies Incorporated.
All Rights Reserved.
...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/netiface/src/ps_stat_iface.c#3 $ $DateTime: 2021/08/02 22:53:02 $ $Author: pwbldsvc $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (NULL == log_pkt_ptr)
  {
    LOG_MSG_ERROR_INT_0("ps_stat_fill_iface_name(): "
                    "Null log packet");
    return 0;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR_INT_1("ps_stat_get_iface_instance_all(): "
                    "Invalid stat %d", stat);
    DS_UTILS_ASSERT(0);
  ...
  if (iface_ptr == NULL)
  {
    DS_UTILS_ERROR_FATAL("ps_stat_get_iface_instance_all(): NULL iface");
    return E_NOT_ALLOWED ;
  }
  ...
  {
    LOG_MSG_ERROR_INT_0("ps_stat_get_iface_instance_all(): "
                    "NULL return_value");
    return E_NOT_ALLOWED ;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
...
#ifdef FEATURE_DATA_PS_IPV6
  /*-----------------------------------------------------------------------
    Add the length of public address
  -----------------------------------------------------------------------*/
  v6_addr_ptr = PS_IFACE_GET_V6_PRIMARY_ADDR_PTR(iface_ptr);
  if(v6_addr_ptr != NULL)
    {
  ...
  -----------------------------------------------------------------------*/
  for(v6_addr_ptr  = PS_IFACEI_SEC_V6_CACHE_GET_FIRST_NODE(iface_ptr);
      v6_addr_ptr != NULL;
      v6_addr_ptr  = PS_IFACEI_SEC_V6_CACHE_GET_NEXT_NODE(iface_ptr,
                                                          v6_addr_ptr))
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (NULL == log_pkt_ptr)
  {
    DS_UTILS_ERROR_FATAL("ps_stat_fill_iface_inst_desc_default_cback(): NULL log pkt");
    return;
  }
  
  memset(&iface_mcast_v6_addr, 0, sizeof(struct ps_in6_addr ));
  ...
  if(v6_addr_ptr != NULL)
  {
    ...
    iface_v6_desc_ptr->addr_prefix = v6_addr_ptr->prefix;
    iface_v6_desc_ptr->addr_iid = v6_addr_ptr->iid;
    iface_v6_desc_ptr->addr_type = v6_addr_ptr->addr_type;
    iface_v6_desc_ptr->addr_state = v6_addr_ptr->addr_state;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (PS_STAT_INST_DESC_ALL == handle)
  {
         ...
         ++idx)
    {
      ...
      if (log_pkt_avail_size < desc_size)
      {
        ...
        if (NULL == log_pkt_ptr)
        {
          LOG_MSG_ERROR_INT_0("ps_stat_get_iface_desc_log_pkt(): "
  ...
  else
  {
    ...
    {
      LOG_MSG_ERROR_INT_1("ps_stat_get_iface_desc_log_pkt(): "
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (PS_STAT_INST_DESC_ALL == handle)
  {
    for (idx = 0; idx < MAX_SYSTEM_IFACES; ++idx)
    {
      ...
      if (NULL == log_pkt_ptr)
      {
        LOG_MSG_ERROR_INT_0("ps_stat_get_iface_stat_log_pkt(): "
  ...
  else
  {
    ...
    if ( (MAX_SYSTEM_IFACES <= handle) || ( 0 > handle) )
    {
      LOG_MSG_ERROR_INT_1("ps_stat_get_iface_stat_log_pkt(): "
                      "Invalid handle %d", handle);
      return;
    }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ( (MAX_SYSTEM_IFACES <= handle) || ( 0 > handle) )
  {
    LOG_MSG_ERROR_INT_1("ps_stat_reset_iface(): "
                    "Invalid handle %d", handle);
    return;
  }
