...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/netiface/src/ps_iface_flow.c#2 $
...
/*----------------------------------------------------------------------------
   MACROS used in logging multiple information using buffer mechanism 
   and finally printing as string. This approach can be used in logging function
   where we may need to log number of information for debugging purpose.
  ----------------------------------------------------------------------------*/
#define PS_FLOW_MAX_LOG_SIZE 480             /* Three times the threshold */
#define PS_FLOW_MAX_LOG_THRESHOLD_SIZE 160   /* Rough estimate that one QXDM F3 is of length 160-200. */

/* Macro for allocating buffer */
#define PS_FLOW_ALLOCATE_BUFFER_FOR_LOG(buffer_ptr)  { \
    PS_SYSTEM_HEAP_MEM_ALLOC(buffer_ptr, PS_FLOW_MAX_LOG_SIZE, uint8*); \
    if(NULL == buffer_ptr) \
    { \
      return; \
    } \
    memset(buffer_ptr, 0x0, PS_FLOW_MAX_LOG_SIZE); \
  }


/* MACRO to store log in buffer if filled buffer length is less than threshold. 
   If buffer reach threshold or if it is last log in the function, we will print it  */

#define PS_FLOW_LOG(buffer_ptr, length_filled, is_last) { \
    if ((is_last && (length_filled > 0)) ||(length_filled >= PS_FLOW_MAX_LOG_THRESHOLD_SIZE )) \
    { \
      LOG_MSG_STR_INFO1_02("%s(): %s", __FUNCTION__, buffer_ptr); \
      length_filled = 0; \
      memset(buffer_ptr, 0x0, PS_FLOW_MAX_LOG_SIZE); \
    } \
  }

#define PS_FLOW_STORE_OR_LOG(buffer_ptr, length_filled, summary, ...) {\
    length_filled += snprintf((char *)(buffer_ptr + length_filled), \
                       (PS_FLOW_MAX_LOG_SIZE - length_filled), \
                       summary, \
                       __VA_ARGS__); \
    PS_FLOW_LOG(buffer_ptr, length_filled, 0) \
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR_INT_0("ps_iface_flow_init(): "
...
)
{
  ...
  PS_FLOW_ALLOCATE_BUFFER_FOR_LOG(buffer_ptr);

  PS_FLOW_STORE_OR_LOG(buffer_ptr, length_filled,
                       "filter_index %d obj %p ip_vsn %d ",
                       filter_index, fltr_buf_ptr, fltr_buf_ptr->ip_vsn);
  ...
  if (IP_V4 == fltr_buf_ptr->ip_vsn )
  {
    ...
    {
      PS_FLOW_STORE_OR_LOG(buffer_ptr, length_filled, 
                           "src ip addr %d.%d.%d.%d subnet mask %d.%d.%d.%d ",
                           (int)(fltr_buf_ptr->ip_hdr.v4.src.addr.ps_s_addr) & 0xFF,
                           (int)(fltr_buf_ptr->ip_hdr.v4.src.addr.ps_s_addr >> 8) & 0xFF,
                           (int)(fltr_buf_ptr->ip_hdr.v4.src.addr.ps_s_addr >> 16) & 0xFF,
                           (int)(fltr_buf_ptr->ip_hdr.v4.src.addr.ps_s_addr >> 24) & 0xFF,
                           (int)(fltr_buf_ptr->ip_hdr.v4.src.subnet_mask.ps_s_addr ) & 0xFF,
                           (int)(fltr_buf_ptr->ip_hdr.v4.src.subnet_mask.ps_s_addr >> 8) & 0xFF,
                           (int)(fltr_buf_ptr->ip_hdr.v4.src.subnet_mask.ps_s_addr >> 16) & 0xFF,
                           (int)(fltr_buf_ptr->ip_hdr.v4.src.subnet_mask.ps_s_addr >> 24) & 0xFF);
    }

    if (IPFLTR_MASK_IP4_DST_ADDR & fltr_buf_ptr->ip_hdr.v4.field_mask)
    {
      PS_FLOW_STORE_OR_LOG(buffer_ptr, length_filled,
                           "dst ip addr %d.%d.%d.%d subnet mask %d.%d.%d.%d ",
                           (int)(fltr_buf_ptr->ip_hdr.v4.dst.addr.ps_s_addr ) & 0xFF,
                           (int)(fltr_buf_ptr->ip_hdr.v4.dst.addr.ps_s_addr >> 8) & 0xFF,
                           (int)(fltr_buf_ptr->ip_hdr.v4.dst.addr.ps_s_addr >> 16) & 0xFF,
                           (int)(fltr_buf_ptr->ip_hdr.v4.dst.addr.ps_s_addr >> 24) & 0xFF,
                           (int)(fltr_buf_ptr->ip_hdr.v4.dst.subnet_mask.ps_s_addr ) & 0xFF,
                           (int)(fltr_buf_ptr->ip_hdr.v4.dst.subnet_mask.ps_s_addr >> 8) & 0xFF,
                           (int)(fltr_buf_ptr->ip_hdr.v4.dst.subnet_mask.ps_s_addr >> 16) & 0xFF,
                           (int)(fltr_buf_ptr->ip_hdr.v4.dst.subnet_mask.ps_s_addr >> 24) & 0xFF);
    }
    ...
    if (IPFLTR_MASK_IP4_NEXT_HDR_PROT & fltr_buf_ptr->ip_hdr.v4.field_mask)
    {
      ...
      if ( PS_IPPROTO_TCP_UDP == fltr_buf_ptr->ip_hdr.v4.next_hdr_prot)
      {
        ...
        {
          PS_FLOW_STORE_OR_LOG(buffer_ptr, length_filled,
                               "tcp+udp src start port: %d range %d ",
        ...
        {
          PS_FLOW_STORE_OR_LOG(buffer_ptr, length_filled, 
                               "tcp+udp dest start port: %d range %d ",
      ...
      if ( PS_IPPROTO_ESP == fltr_buf_ptr->ip_hdr.v4.next_hdr_prot)
      {
        if (IPFLTR_MASK_ESP_SPI & fltr_buf_ptr->next_prot_hdr.esp.field_mask)
        {
          PS_FLOW_STORE_OR_LOG(buffer_ptr, length_filled,
                               "esp spi: 0x%x ",
                               (unsigned int)fltr_buf_ptr->next_prot_hdr.esp.spi );
        }
      }
      if ( PS_IPPROTO_AH == fltr_buf_ptr->ip_hdr.v4.next_hdr_prot)
      {
        if (IPFLTR_MASK_AH_SPI & fltr_buf_ptr->next_prot_hdr.ah.field_mask)
        {
          PS_FLOW_STORE_OR_LOG(buffer_ptr, length_filled,
                               "ah spi: 0x%x",
                               (unsigned int)fltr_buf_ptr->next_prot_hdr.ah.spi );
        }
      }
    }
  }
  else if( IP_V6 == fltr_buf_ptr->ip_vsn ) 
  {
    ...
    {
      PS_FLOW_STORE_OR_LOG(buffer_ptr, length_filled,
                           "src ipv6 addr: %04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x, prefix len %d ",
                           ps_ntohs(fltr_buf_ptr->ip_hdr.v6.src.addr.ps_s6_addr16[0]),
                           ps_ntohs(fltr_buf_ptr->ip_hdr.v6.src.addr.ps_s6_addr16[1]),
                           ps_ntohs(fltr_buf_ptr->ip_hdr.v6.src.addr.ps_s6_addr16[2]),
                           ps_ntohs(fltr_buf_ptr->ip_hdr.v6.src.addr.ps_s6_addr16[3]),
                           ps_ntohs(fltr_buf_ptr->ip_hdr.v6.src.addr.ps_s6_addr16[4]),
                           ps_ntohs(fltr_buf_ptr->ip_hdr.v6.src.addr.ps_s6_addr16[5]),
                           ps_ntohs(fltr_buf_ptr->ip_hdr.v6.src.addr.ps_s6_addr16[6]),
                           ps_ntohs(fltr_buf_ptr->ip_hdr.v6.src.addr.ps_s6_addr16[7]),
    ...
    {
      PS_FLOW_STORE_OR_LOG(buffer_ptr, length_filled,
                           "dest ipv6 addr: %04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x, prefix len %d ",
                           ps_ntohs(fltr_buf_ptr->ip_hdr.v6.dst.addr.ps_s6_addr16[0]),
                           ps_ntohs(fltr_buf_ptr->ip_hdr.v6.dst.addr.ps_s6_addr16[1]),
                           ps_ntohs(fltr_buf_ptr->ip_hdr.v6.dst.addr.ps_s6_addr16[2]),
                           ps_ntohs(fltr_buf_ptr->ip_hdr.v6.dst.addr.ps_s6_addr16[3]),
                           ps_ntohs(fltr_buf_ptr->ip_hdr.v6.dst.addr.ps_s6_addr16[4]),
                           ps_ntohs(fltr_buf_ptr->ip_hdr.v6.dst.addr.ps_s6_addr16[5]),
                           ps_ntohs(fltr_buf_ptr->ip_hdr.v6.dst.addr.ps_s6_addr16[6]),
                           ps_ntohs(fltr_buf_ptr->ip_hdr.v6.dst.addr.ps_s6_addr16[7]),
    ...
    if (IPFLTR_MASK_IP6_FLOW_LABEL & fltr_buf_ptr->ip_hdr.v6.field_mask)
    {
      PS_FLOW_STORE_OR_LOG(buffer_ptr, length_filled,
                           "flow label: %u ",
                           (unsigned int)fltr_buf_ptr->ip_hdr.v6.flow_label );
    }
  ...
  /* To log last leftover */
  PS_FLOW_LOG(buffer_ptr, length_filled, 1);

  if(NULL != buffer_ptr)
  {
    PS_SYSTEM_HEAP_MEM_FREE(buffer_ptr);
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_INFO1_4( "ps_iface_log_qos_spec_params(): "
                   "obj 0x%p qos spec field mask: 0x%x RX Filters %d TX Filters %d",
  ...
  if (qos_spec->rx.fltr_template.num_filters)
  {
    ...
    {
      ps_iface_log_qos_spec_filter_params(&qos_spec->rx.fltr_template.list_ptr[i], i);
    }
  }
  ...
  if (qos_spec->tx.fltr_template.num_filters)
  {
    ...
    {
      ps_iface_log_qos_spec_filter_params(&qos_spec->tx.fltr_template.list_ptr[i], i);
    }
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (ps_errno == NULL)
  {
    DS_UTILS_ERROR_FATAL("ps_iface_create_flow(): NULL parameter is passed");
    return -1;
  }
  ...
  {
    DS_UTILS_ERROR_FATAL("ps_iface_create_flow(): NULL parameters are passed");
    *ps_errno = DS_EFAULT;
    return -1;
  }
  ...
  *out_flow_ptr_ptr = NULL;
  ...
  if (ps_flowi_validate_qos_spec(qos_req_param,
                                 ...
                                 ps_errno) != 0)
  {
    LOG_MSG_ERROR_INT_0("ps_iface_create_flow(): "
  ...
  if ((qos_req_param->qos_spec->field_mask & QOS_MASK_TX_FLOW)
      ...
      && qos_req_param->qos_spec->tx.fltr_template.num_filters > 0
  ...
  flow_ptr->dpl_flow_cb.dropped_count = 0;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (ps_errno == NULL)
  {
    DS_UTILS_ERROR_FATAL("ps_iface_modify_flow(): NULL parameter is passed");
    return -1;
  }

  if (qos_modify_param == NULL || qos_modify_param->qos_spec == NULL)
  {
    DS_UTILS_ERROR_FATAL("ps_iface_modify_flow(): NULL parameters are passed");
    *ps_errno = DS_EFAULT;
    return -1;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (ps_errno == NULL)
  {
    DS_UTILS_ERROR_FATAL("ps_iface_delete_flow(): NULL parameter is passed");
    return -1;
  }
  ...
  if (PS_FLOWI_GET_STATE(flow_ptr) != FLOW_NULL &&
      PS_FLOWI_GET_STATE(flow_ptr) != FLOW_INIT )
  {
    PS_BRANCH_LEAVE_CRIT_SECTION(&global_ps_crit_section);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (ps_errno == NULL)
  {
    DS_UTILS_ERROR_FATAL("ps_iface_modify_flow_accepted(): NULL parameter is passed");
    return -1;
  }
  ...
  if (!PS_FLOWI_GET_CAPABILITY(flow_ptr, PS_FLOW_CAPABILITY_DEFAULT))
  {
    ...
    if (PS_FLOWI_IS_MODIFY_TX_FLTR_SPEC_CREATED(flow_ptr) ||
        PS_FLOWI_IS_MODIFY_TX_FLTR_SPEC_MODIFIED(flow_ptr))
    {
      DS_UTILS_ASSERT(PS_FLOWI_GET_TX_FLTR_HANDLE(flow_ptr) == 0);
      PS_FLOWI_SET_TX_FLTR_HANDLE
      (
        flow_ptr,
        PS_FLOWI_GET_MODIFY_TX_FLTR_HANDLE(flow_ptr)
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (ps_errno == NULL)
  {
    DS_UTILS_ERROR_FATAL("ps_iface_modify_flow_rejected(): NULL parameter is passed");
    return -1;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if (!PS_IFACE_IS_VALID(iface_ptr) || sec_flow_ptr == NULL ||
      next_handle == NULL)
  {
    DS_UTILS_ERROR_FATAL("ps_iface_get_sec_flow_by_handle(): Invalid parameters are passed");
    return FALSE;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (ps_errno == NULL)
  {
    DS_UTILS_ERROR_FATAL("ps_iface_filtering_control(): NULL parameter is passed");
    return -1;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_UTILS_ERROR_FATAL("ps_iface_get_tx_fltr_cnt(): Invalid parameters are passed");
    return -1;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (!PS_IFACE_IS_VALID(ps_iface_ptr))
  {
    DS_UTILS_ERROR_FATAL("ps_iface_flow_get_tx_fltr_cnt_ex(): Invalid parameters are passed");
    return -1;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (!PS_IFACE_IS_VALID(ps_iface_ptr) || fltr == NULL || prcd == NULL ||
      next_handle == NULL)
  {
    DS_UTILS_ERROR_FATAL("ps_ifacei_get_tx_fltr_by_handle_ex(): Invalid parameters are passed");
    return FALSE;
  }
  ...
  {
    PS_BRANCH_LEAVE_CRIT_SECTION(&global_ps_crit_section);
    DS_UTILS_ERROR_FATAL("ps_ifacei_get_tx_fltr_by_handle_ex(): Invalid handle is passed");
    return FALSE;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (!PS_IFACE_IS_VALID(ps_iface_ptr))
  {
    DS_UTILS_ERROR_FATAL("ps_iface_get_tx_fltr_handle(): Invalid parameters are passed");
    return NULL;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (!PS_IFACE_IS_VALID(ps_iface_ptr) || fltr == NULL || prcd == NULL ||
      next_handle == NULL)
  {
    DS_UTILS_ERROR_FATAL("ps_iface_get_tx_fltr_by_handle(): Invalid parameters are passed");
    return FALSE;
  }
  ...
  {
    PS_BRANCH_LEAVE_CRIT_SECTION(&global_ps_crit_section);
    DS_UTILS_ERROR_FATAL("ps_iface_get_tx_fltr_by_handle(): Invalid handle is passed");
    return FALSE;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (!PS_IFACE_IS_VALID(ps_iface_ptr) || fltr == NULL || prcd == NULL ||
      next_handle == NULL)
  {
    DS_UTILS_ERROR_FATAL("ps_iface_flow_get_tx_fltr_by_handle_ex(): Invalid parameters are passed");
    return FALSE;
  }
  ...
  {
    PS_BRANCH_LEAVE_CRIT_SECTION(&global_ps_crit_section);
    DS_UTILS_ERROR_FATAL("ps_iface_flow_get_tx_fltr_by_handle_ex(): Invalid handle is passed");
    return FALSE;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (ps_errno == NULL)
  {
    DS_UTILS_ERROR_FATAL("ps_iface_flow_get_tx_fltr_spec(): NULL errno");
    return FALSE;
  }

  if (NULL == ip_fltr_spec || NULL == ip_fltr_spec->list_ptr)
  {
    LOG_MSG_ERROR_INT_0("ps_iface_flow_get_tx_fltr_spec(): "
                    "NULL ip_filter_spec");
    *ps_errno = DS_EINVAL;
    return FALSE;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_UTILS_ERROR_FATAL("ps_iface_flow_set_tx_filter_spec(): Null value passed to set_tx_filter_spec()");
    return FALSE;
  }
...
/*===========================================================================
FUNCTION PS_IFACE_GET_FLOW_LIST_FROM_FLTR()

DESCRIPTION
  Handles PS_IFACE_IOCTL_GET_FLOW_LIST_FROM_FLTR. Returns a list of matching
  flow list from the provided iface

PARAMETERS
  iface_ptr         : iface that has the filter spec to be match against.
                      This MUST be a logical iface, else an error is
                      returned
  real_time_fltr_check_ptr : pointer to structure containing the
                             filter spec for the logical flows to be
...
===========================================================================*/
int ps_iface_get_flow_list_from_fltr
(
  ps_iface_type                                         * iface_ptr,
  ps_iface_ioctl_fltr_check_type                        * fltr_check_ptr,
  int16                                                 * ps_errno
)
{
  ps_flow_type                         * flow_ptr              = NULL;
  ...
  uint8                                  local_errno = PS_IFACE_SUCCESS;
  ps_iface_qos_fltr_match_dir_enum_type  matched_dir = PS_QOS_FILTR_DIR_MAX;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  do
  {
    if (NULL == iface_ptr || 
        ...
        NULL == ps_errno)
    {
      local_errno = PS_IFACE_ERROR_INVALID_ARG;
      retval = -1;
      break;
    }
    ...
    if (!PS_IFACE_IS_VALID(iface_ptr))
    {
      local_errno = PS_IFACE_ERROR_IFACE_INVALID;
      retval = -1;
      break;
    }
    ...
    while (flow_ptr != NULL)
    {
      if ( PS_FLOW_IS_NW_INITIATED(flow_ptr) &&
           PS_QOS_FILTR_DIR_MAX > (matched_dir = ps_flow_match_filter(flow_ptr,
                                                      &fltr_check_ptr->fltr_spec,
                                                      fltr_check_ptr->fltr_match_dir)))
      {
        retval = 0;
        temp_ps_errno = 0;
        ...
        if (NULL == matching_flow_ptr)
        {
           ...
           matching_flow_ptr = (ps_iface_ioctl_matching_flow_type*)
             list_pop_front(&fltr_check_ptr->matching_flow_list);
         ...
         matching_flow_ptr->fltr_match_dir = matched_dir;
         list_push_back(
           &fltr_check_ptr->matching_flow_list,
    ...
    LOG_MSG_INFO1_3("ps_iface_get_flow_list_from_fltr: errno %d iface_ptr 0x%p ps_errno %d", local_errno, iface_ptr, 
    ((NULL == ps_errno)? 0 : *ps_errno));
    return retval;
  
} /* ps_iface_get_flow_list_from_fltr() */
