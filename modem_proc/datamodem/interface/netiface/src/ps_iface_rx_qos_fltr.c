...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/netiface/src/ps_iface_rx_qos_fltr.c#2 $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (ps_errno == NULL)
  {
    DS_UTILS_ERROR_FATAL("ps_ifacei_rx_qos_fltr_add(): NULL parameter is passed");
    return PS_IFACE_IPFLTR_INVALID_HANDLE;
  }

  if (qos_info_ptr == NULL)
  {
    DS_UTILS_ERROR_FATAL("ps_ifacei_rx_qos_fltr_add(): qos_info_ptr is NULL");
    *ps_errno = DS_EFAULT;
    return PS_IFACE_IPFLTR_INVALID_HANDLE;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (ps_errno == NULL)
  {
    DS_UTILS_ERROR_FATAL("ps_ifacei_rx_qos_fltr_modify(): NULL parameter is passed");
    return PS_IFACE_IPFLTR_INVALID_HANDLE;
  }

  if (qos_info_ptr == NULL)
  {
    DS_UTILS_ERROR_FATAL("ps_ifacei_rx_qos_fltr_modify(): qos_info_ptr is NULL");
    *ps_errno = DS_EFAULT;
    return PS_IFACE_IPFLTR_INVALID_HANDLE;
  }
