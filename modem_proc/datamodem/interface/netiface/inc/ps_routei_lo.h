...
$Header: //components/rel/data.mpss/3.5..2.1/interface/netiface/inc/ps_routei_lo.h#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
)
{
  ...
  for(index = 0; global_v6_lo_route_array[index].iface_ptr != NULL; index++)
  {
    ...
    /*--------------------------------------------------------------------
        Check the public address on the iface
    --------------------------------------------------------------------*/
    v6_addr_ptr = PS_IFACE_GET_V6_PRIMARY_ADDR_PTR
      (global_v6_lo_route_array[index].iface_ptr);
    if(v6_addr_ptr != NULL)
    {
       if(v6_addr_ptr->prefix == ip_addr->ps_s6_addr64[0] &&
          v6_addr_ptr->iid == ip_addr->ps_s6_addr64[1] && 
          ps_iface_state(global_v6_lo_route_array[index].iface_ptr) != 
          IFACE_ROUTEABLE)
       {
    ...
    /*--------------------------------------------------------------------
        Check the secondary addresses on the iface
    --------------------------------------------------------------------*/
    for(v6_addr_ptr  = PS_IFACEI_SEC_V6_CACHE_GET_FIRST_NODE
          (global_v6_lo_route_array[index].iface_ptr);
        v6_addr_ptr != NULL;
        v6_addr_ptr  = PS_IFACEI_SEC_V6_CACHE_GET_NEXT_NODE
          (global_v6_lo_route_array[index].iface_ptr,v6_addr_ptr))
    {
      /*--------------------------------------------------------------------
