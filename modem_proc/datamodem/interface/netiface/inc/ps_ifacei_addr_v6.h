...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/netiface/inc/ps_ifacei_addr_v6.h#2 $
  $DateTime: 2019/10/22 02:53:17 $

  when      who    what, where, why
--------    ---    ----------------------------------------------------------
10/31/15    ssh    Probe threshold default to 50%
...
typedef enum
{
   PS_IFACEI_V6_ADDRESS_LOG_EVENT_INVALID = 0,
   PS_IFACEI_V6_ADDRESS_LOG_EVENT_ALLOCATED = 1,
   PS_IFACEI_V6_ADDRESS_LOG_EVENT_UPDATED = 2,
   PS_IFACEI_V6_ADDRESS_LOG_EVENT_DEPRECATED = 3,         
   PS_IFACEI_V6_ADDRESS_LOG_EVENT_EXPIRED = 4,            
   PS_IFACEI_V6_ADDRESS_LOG_EVENT_DELETED = 5,               
   PS_IFACEI_V6_ADDRESS_LOG_EVENT_BOUND = 6,
   PS_IFACEI_V6_ADDRESS_LOG_EVENT_UNBOUND = 7,
   PS_IFACEI_V6_ADDRESS_LOG_EVENT_MAX
} ps_ifacei_v6_addr_log_event_enum;
...
#define IPV6_EXT_ADDR_DEFAULT_THRESHOLD_PERCENT             50
...
/*----------------------------------------------------------------------------
  Check if it is at pre-threshold stage or post-threshold stage. public address 
  is moved out of cache in the new design of v6 addr cache, So we need to add 
  1 the cnt of sec v6 cache so that the behavior will be consistent with
  legacy behavior
---------------------------------------------------------------------------*/
#define PS_IFACEI_V6_EXT_ADDRS_THRESHOLD_TRIGGERED(ps_iface_ptr) \
  (PS_IFACEI_SEC_V6_CACHE_GET_CNT(ps_iface_ptr) + 1 >                             \
...
/*--------------------------------------------------------------------------
  Check if the v6 address cache is initialed
---------------------------------------------------------------------------*/
#define PS_IFACEI_SEC_V6_CACHE_IS_INIT(iface_ptr)                          \
  ((iface_ptr)->iface_private.sec_v6_addr_cache_ptr != NULL)  

/*--------------------------------------------------------------------------
  Get the first node of secondary v6 address cache
---------------------------------------------------------------------------*/
#define PS_IFACEI_SEC_V6_CACHE_GET_FIRST_NODE(iface_ptr)                   \
  ((ps_ifacei_v6_addr_type *)                                              \
    (((iface_ptr)->iface_private.sec_v6_addr_cache_ptr != NULL) ?          \
      q_check(&((iface_ptr)->iface_private.sec_v6_addr_cache_ptr->         \
        sec_v6_addr_q)): NULL))
  
/*--------------------------------------------------------------------------
  Get the next node of passed in node in secondary v6 address cache
---------------------------------------------------------------------------*/
#define PS_IFACEI_SEC_V6_CACHE_GET_NEXT_NODE(iface_ptr,addr_ptr)           \
  ((ps_ifacei_v6_addr_type *)q_next(&((iface_ptr)->iface_private.          \
    sec_v6_addr_cache_ptr->sec_v6_addr_q),&((addr_ptr)->link)))

/*--------------------------------------------------------------------------
  Add one node to the secondary v6 address cache
---------------------------------------------------------------------------*/
#define PS_IFACEI_SEC_V6_CACHE_ADD_NODE(iface_ptr,addr_ptr)                \
  (q_put(&((iface_ptr)->iface_private.sec_v6_addr_cache_ptr->              \
      sec_v6_addr_q),q_link((void *)(addr_ptr),&((addr_ptr)->link))))

/*--------------------------------------------------------------------------
  Delete one node from secondary v6 address cache
---------------------------------------------------------------------------*/
#define PS_IFACEI_SEC_V6_CACHE_DELETE_NODE(iface_ptr,addr_ptr)             \
  q_delete_ext(                                                            \
    &((iface_ptr)->iface_private.sec_v6_addr_cache_ptr->sec_v6_addr_q),    \
      &(addr_ptr)->link)

/*----------------------------------------------------------------------------
  Get the count of the secondary v6 address cache
---------------------------------------------------------------------------*/
#define PS_IFACEI_SEC_V6_CACHE_GET_CNT(iface_ptr)                          \
  (PS_IFACEI_SEC_V6_CACHE_IS_INIT(iface_ptr) ?                             \
    q_cnt(&((iface_ptr)->iface_private.sec_v6_addr_cache_ptr->             \
      sec_v6_addr_q)): 0)
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(v6_addr_ptr != NULL)
  {
    if( (v6_addr_ptr->iid == *priv_iid) ||
        (v6_addr_ptr->gateway_iid == *priv_iid))
    {
      return FALSE;
    }
  }

  for(v6_addr_ptr  = PS_IFACEI_SEC_V6_CACHE_GET_FIRST_NODE(base_iface_ptr);
      v6_addr_ptr != NULL;
      v6_addr_ptr  = PS_IFACEI_SEC_V6_CACHE_GET_NEXT_NODE(base_iface_ptr,
                                                          v6_addr_ptr))
  {
    if( (v6_addr_ptr->iid == *priv_iid) ||
        (v6_addr_ptr->gateway_iid == *priv_iid))
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /*-----------------------------------------------------------------------
    The public address is moved out of cache in new desgin, so we need add 1
    to the count of secondary v6 address cache so that the behavior will be
    consistent with legacy behavior
  ------------------------------------------------------------------------*/
  queue_cnt = PS_IFACEI_SEC_V6_CACHE_GET_CNT(base_iface_ptr);
  if(queue_cnt + 1 < MAX_IPV6_ADDRS)
  {
    LOG_MSG_INFO1_1("The current queue count is %d", queue_cnt);
    return TRUE;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  v6_addr_ptr = PS_IFACE_GET_V6_PRIMARY_ADDR_PTR(base_iface_ptr);
  if( (v6_addr_ptr != NULL) &&
      (v6_addr_ptr->addr_state == addr_state) &&
      (v6_addr_ptr->addr_type == addr_type) )
  {
    *v6_addr_ptr_ptr = v6_addr_ptr;
    return TRUE;
  }
  ...
  for( v6_addr_ptr  = PS_IFACEI_SEC_V6_CACHE_GET_FIRST_NODE(base_iface_ptr); 
       ...
       v6_addr_ptr  = PS_IFACEI_SEC_V6_CACHE_GET_NEXT_NODE(base_iface_ptr,
                                                           v6_addr_ptr))
  {
    if( (v6_addr_ptr->addr_state == addr_state) &&
        (v6_addr_ptr->addr_type == addr_type) )
    {
      *v6_addr_ptr_ptr = v6_addr_ptr;
      return TRUE;
...
/*===========================================================================
FUNCTION PS_IFACE_FIND_IPV6_ADDR2()

DESCRIPTION
  This function locates an IPv6 address on any UP interface in 
  global iface array.
...
SIDE EFFECTS
  None
===========================================================================*/
boolean ps_iface_find_ipv6_addr2
(
  struct ps_in6_addr       *ip_addr_ptr,
  ps_ifacei_v6_addr_type  **v6_addr_ptr_ptr,
  ps_iface_type           **ps_iface_ptr_ptr,
...
/*===========================================================================
FUNCTION PS_IFACEI_V6_CACHE_INIT()

DESCRIPTION
  Initialize a v6 cache
...
SIDE EFFECTS
  None
===========================================================================*/
void ps_ifacei_sec_v6_cache_init
(
  ps_iface_type* iface_ptr
);

/*===========================================================================
FUNCTION PS_IFACEI_V6_CACHE_DESTROY()
...
RETURN VALUE
  TURE -- The cache is destoried.
  FALSE  -- In failure case.
...
SIDE EFFECTS
  None
===========================================================================*/
void ps_ifacei_sec_v6_cache_destroy
(
  ps_iface_type* iface_ptr
);

/*===========================================================================
FUNCTION PS_IFACEI_V6_ADDR_LOG_EVENT_MSG()

DESCRIPTION
  Logs DIAG MESG for IPv6 address for event passed in to this function.
 
PARAMETERS
  iface_ptr   Pointer to the interface.
  v6_addr_ptr Pointer to v6 address
  event       event to log
...
SIDE EFFECTS
  None
===========================================================================*/
void ps_ifacei_v6_addr_log_event_msg
(
  ps_iface_type                    * iface_ptr,
  ps_ifacei_v6_addr_type           * v6_addr_ptr,
  ps_ifacei_v6_addr_log_event_enum   event
);
