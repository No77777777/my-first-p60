...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/pssock/src/PS_Sock_Platform_Socket.cpp#3 $
    ...
    namespace Platform
    {
      ...
      namespace Opt
      {
        ...
        const InfoType sockOptInfoArr[ ] =
        {
          ...
          /* IP_RECVTTL */
          { true, IP_RECVTTL_DEF_VAL },
          
          /* IP_RECVECN */
          { true, IP_RECVECN_DEF_VAL },
          ...
          /* TCP_ACCESS_PRIORITY */
          { true , TCP_ACCESS_PRIORITY_DEF_VAL },

          /* IP_RECVSUBID */
          { true, IP_RECVSUBID_DEF_VAL }
        };
      } /* namespace Opt */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  *dsErrnoPtr = SocketIOFactory::GetReaderAndWriter( _protocol,
  ...
  if (AEE_SUCCESS != *dsErrnoPtr ||
      ...
      0 == inetRcvQWriterPtr)
  {
    ps_sock_error_code = DSS_ERROR_GET_READER_WRITER_OPERATION_FAILED;
    goto bail;
  }
  ...
  *dsErrnoPtr = SocketIOFactory::GetReaderAndWriter( _protocol,
  ...
  if (0 == platformSockPtr)
  {
    *dsErrnoPtr = QDS_EMFILE;
    ps_sock_error_code = DSS_ERROR_NO_MEMORY;
    goto bail;
  }
  ...
  if (-1 == bufIndex)
  {
    ...
    *dsErrnoPtr = QDS_EINVAL;
    ps_sock_error_code = DSS_ERROR_BUFFER_MEMORY_ALLOCATION_FAILED;
    goto bail;
  }
  ...
  if (PS_HANDLE_MGR_INVALID_HANDLE == platformSockPtr->platformSockHandle)
  {
    DS_UTILS_ASSERT(0);
    *dsErrnoPtr = QDS_EINVAL;
    ps_sock_error_code = DSS_ERROR_INVALID_HANDLE;
    goto bail;
  }
  ...
  {
    *dsErrnoPtr = QDS_EMFILE;
    ps_sock_error_code = DSS_ERROR_CREATE_PCB_FAILED;
    goto bail;
  }
  ...
  /*-------------------------------------------------------------------------
    Initialize Remote socket and ephemeral cache handle to MAX
  -------------------------------------------------------------------------*/  
  platformSockPtr->SetRemoteSockHandle(UINT32_MAX);
  platformSockPtr->SetEphemeralCacheHandle(UINT32_MAX);
...
bail:
  ...
  LOG_MSG_ERROR_2("Socket::CreateInstance() "
                  "dsErr %d error code %d", 
                  *dsErrnoPtr, ps_sock_error_code);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_QTRACE_HFT_MED(PS_TAG_PSSOCK, PS_SYS_DEFAULT_SUBS,
                            "Socket::Destructor(): Sock %d", platformSockHandle);
...
)
{
  ...
  EphemeralSocketFlag      remoteSockBindFlag = EPHEMERAL_FLAG_NONE;
  int32                    udpEpncapsVal;
  int32                    udpEpncapsLen;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    remoteSockInfo.SetFamily(family);
    remoteSockInfo.SetProtocol(protocol);

    if (AEE_SUCCESS ==  GetOpt(ds::Sock::OptLevel::QDS_LEVEL_UDP, 
                               ...
                               &udpEpncapsLen))
    {
      if (0 != udpEpncapsVal)
      {
        remoteSockBindFlag = EPHEMERAL_FLAG_UDP_ENCAPS;
      }            
    }
    if( UINT32_MAX == GetEphemeralCacheHandle())
    {
      SetEphemeralCacheHandle( platformSockFactoryPtr->GetDefaultEphemeralCacheHandle());
    }

    remoteSockManagerPtr->RetrieveAndRequestSocket( GetEphemeralCacheHandle(), 
                                                    &remoteSockInfo, 
                                                    remoteSockBindFlag);
    v6SockAddr.port = remoteSockInfo.GetPort();
    SetRemoteSockHandle(remoteSockInfo.GetHandle());
    
    if (0 == v6SockAddr.port)
    {
      LOG_MSG_INFO2_2("Socket::Bind(): "
                      "Couldn't get ephem port, sock %d. Requested for "
                      "ports from AP pool Handle %d", 
                      platformSockHandle,
                      GetEphemeralCacheHandle());
      
      DS_UTILS_RELEASEIF(platformSockFactoryPtr);   
      SetRemoteSockHandle(UINT32_MAX);
      return QDS_EINVAL;
    }

    if( (EPHEMERAL_FLAG_UDP_ENCAPS == remoteSockBindFlag) &&
        (UINT32_MAX != remoteSockInfo.GetHandle()))
    {
      remoteSockManagerPtr->SendUDPEncapsSockOpt(remoteSockInfo.GetHandle(), TRUE);
    }
  }
  else if (0 == listenHeadPtr)
  {
    remoteSockInfo.SetFamily(family);
    remoteSockInfo.SetProtocol(protocol);
    remoteSockInfo.SetPort(v6SockAddr.port);
   
    if( UINT32_MAX != GetEphemeralCacheHandle())
    {
      if ( FALSE == remoteSockManagerPtr->RetrieveAndRequestSocket( GetEphemeralCacheHandle(), 
                                                      &remoteSockInfo, 
                                                      remoteSockBindFlag))
      {
        LOG_MSG_INVALID_INPUT_2("Socket::Bind(): Unable to retrieve remote socket"
                                "Port %d in use, cache handle %d",
                                 dss_ntohs( v6SockAddr.port), 
                                 GetEphemeralCacheHandle());
        SetRemoteSockHandle(UINT32_MAX);
    ...
    /*-----------------------------------------------------------------------
      Note: Application can bind on port which is part of remote socket cache. In such case
      we return ADDRESS IN USE error to second socket bind and will release remote socket handle.
      Application has to re-bind on some other port
      
      Alternate Approach:  We can avoid binding to particular  port if socket is in cache and appropriate
      cache handle is not set but in that case we will Penalize application which is unware about caching
    -----------------------------------------------------------------------*/
                                                 ...
                                                 false))
    {
      ...
      if( UINT32_MAX != GetRemoteSockHandle())
      {
        remoteSockManagerPtr->ReleaseRemoteSocket(&remoteSockInfo, GetEphemeralCacheHandle());
        SetRemoteSockHandle(UINT32_MAX);
      }
      DS_UTILS_RELEASEIF(platformSockFactoryPtr);
...
void Socket::SetRemoteSockHandle
(
  uint32  handle
)
{
...
void Socket::SetEphemeralCacheHandle
(
  uint32  handle
)
{
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ephemeralCacheHandle = handle;
} /* Socket::SetEphemeralCacheHandle */    

uint32 Socket::GetEphemeralCacheHandle
(
  void
)
{
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  return ephemeralCacheHandle;
} /* Socket::GetEphemeralCacheHandle*/ 
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    dsErrno = QDS_EFAULT;
    ps_sock_error_code = DSS_ERROR_QUEUE_IS_EMPTY;
    goto bail;
  }
  ...
  if (AEE_SUCCESS != dsErrno)
  {
    DS_UTILS_ASSERT( 0);
    dsErrno = QDS_EINVAL;
    ps_sock_error_code = DSS_ERROR_GET_ADDR_FAILED;
    goto bail;
  }
  ...
  *newPlatformSockPtrPtr = acceptPlatformSockPtr;
  ...
  LOG_MSG_FUNCTION_EXIT_3("Socket::Accept(): "
                          "Success, sock %d, Accepted sock %d, partial count %d",
                          platformSockHandle, acceptPlatformSockPtr->platformSockHandle, listenList.Count());
  return AEE_SUCCESS;
...
bail:
  /*-------------------------------------------------------------------------
    Free accepted sock
  -------------------------------------------------------------------------*/
  LOG_MSG_ERROR_3("Socket::Accept(): "
                  "sock %d err 0x%x error code %d",
                  platformSockHandle, dsErrno, ps_sock_error_code);
  if (0 != acceptPlatformSockPtr)
  {
    /*-----------------------------------------------------------------------
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch (optName)
  {
    ...
    case ds::Sock::OptName::QDS_SO_PORT_PREF_HANDLE:
    case ds::Sock::OptName::QDS_SO_PORT_IWLAN_CAPABLE:
    {
      postCmdToPS = false;
      break;
    }
    ...
    case ds::Sock::OptName::QDS_IP_RECVTTL:
    case ds::Sock::OptName::QDS_IP_RECVECN:
    case ds::Sock::OptName::QDS_TCP_ACCESS_PRIORITY:
    case ds::Sock::OptName::QDS_IP_RECVSUBID:
    {      
      postCmdToPS = true;
      break;
    }
    ...
    case ds::Sock::OptName::QDS_SO_FIONREAD:
    {
      ...
      ps_sock_error_code = DSS_ERROR_SET_OPT_NOT_SUPPORTED;
      goto bail;
    }
    ...
    {
      dsErrno = QDS_EOPNOTSUPP;
      ps_sock_error_code = DSS_ERROR_UNKNOWN_OPERATION;
      goto bail;
    }
  } /* switch (optName) */
  ...
  switch (optName)
  {
    case ds::Sock::OptName::QDS_SO_SNDBUF:
    {
      ...
      {
        dsErrno = QDS_EINVAL;
        ps_sock_error_code = DSS_ERROR_SNDQWRITER_IS_NULL;
        goto bail;
      }
    ...
    case ds::Sock::OptName::QDS_SO_RCVBUF:
    {
      ...
      {
        dsErrno = QDS_EINVAL;
        ps_sock_error_code = DSS_ERROR_RCVQWRITER_IS_NULL;
        goto bail;
      }
    ...
    case ds::Sock::OptName::QDS_SO_PORT_PREF_HANDLE:
    {
      ...
      {
        SetOptCache( optName);
        SetEphemeralCacheHandle(optVal);
      }
      break;
    }
    case ds::Sock::OptName::QDS_UDP_ENCAPS:
    {
      if (0 != optVal)
      {
        ...
        if (UINT32_MAX != remoteSockHandle)
        {
    ...
    case ds::Sock::OptName::QDS_SO_SKIP_ROUTE_SCOPE_CHECK:
    {
      ...
      if (Protocol::QDS_TCP == protocol)
      {
        ...
        {
          dsErrno = QDS_EINVAL;
          ps_sock_error_code = DSS_ERROR_SOCKET_NOT_IN_LISTEN_STATE;
          goto bail;
        }
      }
      ...
      {
        dsErrno = QDS_EINVAL;
        ps_sock_error_code = DSS_ERROR_INVALID_SOCKET_OPTIONS;
        goto bail;
      }
      else
      {
      ...
      if (AEE_SUCCESS != dsErrno)
      {
        ps_sock_error_code = DSS_ERROR_SET_ROUTESCOPE_FAILED;
        goto bail;
      }
  ...
  -------------------------------------------------------------------------*/
bail:
  LOG_MSG_ERROR_5("Socket::SetOpt: "
                  "sock %d name %d state %d err 0x%x error code %d", 
                  platformSockHandle, optName, state, dsErrno, ps_sock_error_code);
  if (0 != setOptPSCmdBufPtr)
  {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch (optName)
  {
    ...
    case ds::Sock::OptName::QDS_SO_PORT_PREF_HANDLE:
    {
      *optValPtr = GetEphemeralCacheHandle();
      break;
    }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (State::LISTEN == state)
  {
    for (listenList.First(); !listenList.IsDone(); listenList.Next())
    {
      ...
      DS_UTILS_ASSERT( 0 != listenPlatformSockPtr);
  ...
  -------------------------------------------------------------------------*/
bail:
  LOG_MSG_ERROR_3("Socket::SetOptPriv: "
                  
"name %d err 0x%x error code %d", 
                  optName, dsErrno, ps_sock_error_code);
  if (0 != setOptPSCmdBufPtr)
  {
    ps_socket_cmd_free( &setOptPSCmdBufPtr);
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_FUNCTION_ENTRY_5("Socket::GetOptPriv(): "
                           "Sock %d level %d name %d, val 0x%x, len 0x%x",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
LOG_MSG_FUNCTION_ENTRY_4("Socket::SetSOLingerReset(): "
                         "sock 0x%x soLingerPtr 0x%x state %d linger is enabled %d",
                         platformSockHandle, soLingerPtr, state, 
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    dsErrno = QDS_EFAULT;
    ps_sock_error_code = DSS_ERROR_ROUTE_META_INFO_NULL;
    goto bail;
  }
  ...
  {
    dsErrno = QDS_EINVAL;
    ps_sock_error_code = DSS_ERROR_PCB_ALREADY_FREED;
    goto bail;
  }
  ...
  -------------------------------------------------------------------------*/
bail:
  LOG_MSG_ERROR_3("Socket::SetRtMetaInfo: "
                  "sock %d err 0x%x error code %d", platformSockHandle, 
                  dsErrno, ps_sock_error_code);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_INFO4_1( "Socket::SetRouteScope(): Skip Route Scope "
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (0 != v6RemoteSockAddr.port && 0 != remoteAddrPtr)
  {
    ...
    if (AEE_SUCCESS != dsErrno)
    {
      ps_sock_error_code = DSS_ERROR_GET_SOCKADDR_FAILED;
      goto bail;
    }
    ...
    if (false == PS_IN6_ARE_ADDR_EQUAL( v6RemoteSockAddr.addr, &ps_in6addr_any))
    {
      ...
      {
        dsErrno = QDS_EISCONN;
        ps_sock_error_code = DSS_ERROR_SOCKET_ALREADY_CONNECTED;
        goto bail;
      }
    }
  }
  ...
  if (0 != writeEventBitMask)
  {
    ...
    ps_sock_error_code = DSS_ERROR_EVENT_BIT_MASK_FAILED;
    goto bail;
  }
  ...
  if (0 == v6LocalSockAddr.port)
  {
    ...
    if (AEE_SUCCESS != dsErrno)
    {
      ps_sock_error_code = DSS_ERROR_BIND_OPERATION_FAILED;
  ...
  *numWrittenPtr = retVal;
  ...
  -------------------------------------------------------------------------*/
bail:
  LOG_MSG_ERROR_3("Socket::SendMsg(): "
                  "sock %d err 0x%x error code %d",
                  platformSockHandle, dsErrno, ps_sock_error_code);
  if (0 != writePSCmdBufPtr)
  {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  *numWrittenPtr = retVal;
  ...
  -------------------------------------------------------------------------*/
bail:
  LOG_MSG_ERROR_3("Socket::SendDSMChain(): "
                  "Sock %d err 0x%x, error code %d",
                  platformSockHandle, dsErrno, ps_sock_error_code);
  if (0 != writePSCmdBufPtr)
  {
...
)
{
  ...
  /*-------------------------------------------------------------------------
    1. Platform layer should not release unbound socket to remote socket layer (port is 0)
    2. Platform layer should not release socket which doesnt has valid handle and cache handle
  -------------------------------------------------------------------------*/
  if( ( 0 == GetPort() ) ||
      ( UINT32_MAX == GetEphemeralCacheHandle() &&
        UINT32_MAX == GetRemoteSockHandle()) 
    )
  {
    return;
  }
  ...
  -------------------------------------------------------------------------*/
  remoteSockManagerPtr = PS::Sock::RemoteSocket::Manager::GetInstance();
  sockInfo.SetHandle(GetRemoteSockHandle());
  sockInfo.SetFamily( GetFamily());
  sockInfo.SetPort(GetPort());
  sockInfo.SetProtocol( GetProtocol());
  remoteSockManagerPtr->ReleaseRemoteSocket( &sockInfo, GetEphemeralCacheHandle());  

  /*-----------------------------------------------------------------------
    Reset Remote socket handle and cache handle to MAX
  -----------------------------------------------------------------------*/  
  SetRemoteSockHandle(UINT32_MAX);
  SetEphemeralCacheHandle(UINT32_MAX);
  
} /* Socket::CloseRemoteSocket() */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (0 == pcbHandle || TRUE == silentCloseFlag)
  {
    ...
    /* -------------------------------------------------------------------------
       SilentClose Flag will be true for non TCP sockets and for TCP sockets 
       whose linger time is 0 . In this case release the remote socket immediately.
       For other Open TCP case we will release remote socket only after TCP close 
...
)
{
  ...
  bool allowingReUseOfAddrOfSystemSock = false;
  bool allowingReUseOfAddrOfSock = false;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if (0 == v6SockAddrPtr)
  {
    DS_UTILS_ERROR_FATAL("Socket::IsSockAddrInUse(): NULL addr");
    return true;
  }
  ...
  {
    DS_UTILS_ERROR_FATAL("Socket::IsSockAddrInUse(): 0 port");
    return true;
  }
 ...
 /*-------------------------------------------------------------------------
        ...
        PS_IN6_ARE_ADDR_EQUAL( v6SockAddrPtr->addr, &ps_in6addr_any)))
  {
    ...
    {
      allowingReUseOfAddrOfSock = true;
      isSockAddrInUse = false;
    }
    else if ((true == isSystemSocket) && (false == isEphemeralPort))
    {
      allowingReUseOfAddrOfSystemSock = true;
      isSockAddrInUse = false;
    }
    else
    {
  ...
  DS_INTERFACE_LOG_MSG_INFO1_6(PS_LOGGING_CAT_SOCKET, "Socket::IsSockAddrInUse(): "
                          "Returning %d, sock %d bool Reuse of addr of sock %d"
                          " Reuse of addr of system sock %d"
                          " Proto %d  isEphemeralPort flag %d",
                          isSockAddrInUse, (int)platformSockHandle, 
                          allowingReUseOfAddrOfSock, allowingReUseOfAddrOfSystemSock,
                          _protocol, isEphemeralPort);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (0 != listenHeadPtr)
  {
    ...
    /* No need to release remote socket here as State is
       set to closed and remote socket will be released 
       in below call to Close() */
    (void) Close();
  }
  else if (State::LISTEN != state)
  {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_UTILS_ERROR_FATAL("Socket::Process(): Unexpected call");
  return TRUE;
...
)
{
  ...
  if (IsOptCacheSet(ds::Sock::OptName::QDS_IP_RECVTTL))
  {
    sockPtr->SetOpt( ds::Sock::OptLevel::QDS_LEVEL_IP,
                     ds::Sock::OptName::QDS_IP_RECVTTL,
                     TRUE,
                     sizeof(int));
  }

  if (IsOptCacheSet(ds::Sock::OptName::QDS_IP_RECVECN))
  {
    sockPtr->SetOpt( ds::Sock::OptLevel::QDS_LEVEL_IP,
                     ds::Sock::OptName::QDS_IP_RECVECN,
                     TRUE,
                     sizeof(int));
  }
  ...
  if (IsOptCacheSet(ds::Sock::OptName::QDS_TCP_ACCESS_PRIORITY))
  {
    sockPtr->SetOpt( ds::Sock::OptLevel::QDS_LEVEL_TCP,
                     ds::Sock::OptName::QDS_TCP_ACCESS_PRIORITY,
                     TRUE,
                     sizeof(int));
  }
  
  if (IsOptCacheSet(ds::Sock::OptName::QDS_IP_RECVSUBID))
  {
    sockPtr->SetOpt( ds::Sock::OptLevel::QDS_LEVEL_IP,
                     ds::Sock::OptName::QDS_IP_RECVSUBID,
                     TRUE,
                     sizeof(int));
  }
...
/*-------------------------------------------------------------------------
  ...
  -------------------------------------------------------------------------*/
bail:
  LOG_MSG_ERROR_2("Socket::Clone(): "
                 "sock %d, error code %d", platformSockHandle, ps_sock_error_code);
  DS_UTILS_RELEASEIF(platformSockFactoryPtr);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_UTILS_ASSERT( 0 != v6RemoteAddrPtr);
  ...
  LOG_MSG_FUNCTION_EXIT_4("Socket::GetDSErrnoForConnect(): "
                          "Err 0x%x, sock %d state %d state is unknown %d", 
                          dsErrno, platformSockHandle, state, stateIsUnknown);
  return dsErrno;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  else
  {
    LOG_MSG_INFO1_2("Socket::SetOrClearEvent(): "
                    "Unable to post ev %d, sock %d", event, platformSockHandle);
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_QTRACE_HFT_MED(PS_TAG_PSSOCK, PS_SYS_DEFAULT_SUBS,
                 "Socket::SetEventBitMask(): Sock %d ev %d bit mask 0x%x -> ev mask 0x%x",
...
)
{
  ...
  RecvTTLInfo *            recvTTLInfoPtr;
  RecvECNInfo *            recvECNInfoPtr;  
  RecvSUBIDInfo *          recvSUBIDInfoPtr;  
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (cnt < numAncData &&
      IsOptCacheSet( ds::Sock::OptName::QDS_IP_RECVTTL))
  {
    recvTTLInfoPtr = new RecvTTLInfo( readInfoPtr->ttl);
    ancillaryDataPtr[cnt++] = recvTTLInfoPtr;
  }

  if (cnt < numAncData &&
      IsOptCacheSet( ds::Sock::OptName::QDS_IP_RECVECN))
  {
    recvECNInfoPtr = new RecvECNInfo( readInfoPtr->ecn);
    ancillaryDataPtr[cnt++] = recvECNInfoPtr;
  }

  if (cnt < numAncData &&
      IsOptCacheSet( ds::Sock::OptName::QDS_IP_RECVSUBID))
  {
    recvSUBIDInfoPtr = new RecvSUBIDInfo( readInfoPtr->subs_id);
    ancillaryDataPtr[cnt++] = recvSUBIDInfoPtr;
  }
