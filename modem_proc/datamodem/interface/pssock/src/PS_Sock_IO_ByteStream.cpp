...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/pssock/src/PS_Sock_IO_ByteStream.cpp#2 $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (WRITE_SHUTDOWN == state || CLOSED == state)
  {
    *psErrnoPtr = DS_EINVAL;
    ps_sock_error_code = DSS_ERROR_INVALID_WRITE_STATE;
    goto bail;
  }
  ...
  if (-1 == byteCount)
  {
    ps_sock_error_code = DSS_ERROR_COPY_BUF_TO_DSM_FAILED;
    goto bail;
  }
  else
  {
...
bail:
  critSectPtr->Leave();
  LOG_MSG_ERROR_3("ByteStream::Write(): "
                  "state %d err %d, error code %d",
                  state, *psErrnoPtr, ps_sock_error_code);
  return -1;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  else
  {
    ...
    -----------------------------------------------------------------------*/
    DS_UTILS_ASSERT( bufLen == dsm_length_packet( *dsmItemPtrPtr));
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_FUNCTION_ENTRY_3("ByteStream::SetWrCritSection(): "
                           "Stream 0x%x CS 0x%x psErrno 0x%x", 
                           this, newCritSectPtr, psErrnoPtr);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (READ_SHUTDOWN == state || CLOSED == state)
  {
    *psErrnoPtr = DS_EINVAL;
    byteCount   = -1;
    ps_sock_error_code = DSS_ERROR_INVALID_READ_STATE;
    goto bail;
  }
  ...
  if (0 == currSize)
  {
    if (WRITE_SHUTDOWN == state)
    {
      *psErrnoPtr = DS_EEOF;
      byteCount   = 0;
      ps_sock_error_code = DSS_ERROR_QUEUE_IS_WR_SHUTDOWN;
    }
    else
    {
...
bail:
  critSectPtr->Leave();
  LOG_MSG_FUNCTION_EXIT_4("ByteStream::Read(): "
                          "state %d, byte count %d err %d, error code %d", 
                          state, byteCount, *psErrnoPtr, ps_sock_error_code);
  return byteCount;
...
bail:
  critSectPtr->Leave();
  LOG_MSG_FUNCTION_EXIT_4("ByteStream::Read(): "
                          "state %d, tmp size %d err %d, error code %d", 
                          state, tmpSize, *psErrnoPtr, ps_sock_error_code);
  return tmpSize;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  while (numPeeked < numBytesToPeek)
  {
    ...
    if (0 == itemPtr)
    {
      ...
      ps_sock_error_code = DSS_ERROR_OUT_OF_DSM_ITEMS;
      goto bail;
    }
...
bail:
  ...
  LOG_MSG_FUNCTION_EXIT_3("ByteSteam::Peek(): "
                          "state %d err %d error code %d", 
                          state, *psErrnoPtr, ps_sock_error_code);
  return -1;
...
#ifdef FEATURE_DATA_PS_DEBUG
  DS_UTILS_ASSERT( numBytesToFree == numBytesPulledUp);
#endif /* FEATURE_DATA_PS_DEBUG */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_FUNCTION_ENTRY_3("ByteStream::SetRdCritSection(): "
                           "Stream 0x%x CS 0x%x, psErrno 0x%x", 
                           this, newCritSectPtr, psErrnoPtr);
