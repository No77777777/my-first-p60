...
int CallbackMgrModem::per_each_num_node_invoke_cb(void* data, va_list args)
{
  ...
  PROFILE_DB_LOG_HIGH_3("per_each_num_node_invoke_cb(): "
                        "posting Invoke CB command with callback_id 0x%x, "
                        "event %d, profiles count %d",
                        list_node->callback_id, 
                        cmd->event, 
                        cmd->profiles_count);
...
int CallbackMgrModem::per_each_techmask_node_invoke_cb(void* data, va_list args)
{
  ...
  PROFILE_DB_LOG_HIGH_3("per_each_techmask_node_invoke_cb(): posting Invoke CB "
                        "command with callback_id 0x%x, "
                        "event %d, profiles count %d",
                        list_node->callback_id, cmd->event, 
                        cmd->profiles_count);
...
{  
  ...
  {
    if (ds_util_list_get_hndl(&theInstance->valid_clients_ids[iter])
         != DS_SUCCESS)
    {
    res = DB_RESULT_ERR_OUT_OF_MEMORY;
    goto bail;
    ...
    if (ds_util_list_get_hndl(&theInstance->mh_valid_clients_ids[iter]) 
        != DS_SUCCESS)
    {
    res = DB_RESULT_ERR_OUT_OF_MEMORY;
    goto bail;
    ...
    if (ds_util_list_get_hndl(&theInstance->clients_per_number[iter]) 
        != DS_SUCCESS)
    {
    res =  DB_RESULT_ERR_OUT_OF_MEMORY;
    goto bail;
    ...
    if (ds_util_list_get_hndl(&theInstance->clients_per_techmask[iter]) 
        != DS_SUCCESS)
    {
    res = DB_RESULT_ERR_OUT_OF_MEMORY;
    goto bail;
    ...
    if (ds_util_list_get_hndl(&theInstance->mh_clients_per_number[iter]) 
        != DS_SUCCESS)
    {
    res =  DB_RESULT_ERR_OUT_OF_MEMORY;
    goto bail;
    ...
    if (ds_util_list_get_hndl(&theInstance->mh_clients_per_techmask[iter]) 
        != DS_SUCCESS)
    {
    res = DB_RESULT_ERR_OUT_OF_MEMORY;
    goto bail;
  ...
  bool treat_tech_mask_as_OR /* = false */)
{
  ...
  {
    callback_info = RegisterForTechMask(tech_mask, 
                                        cback, 
                                        curr_callback_id, 
                                        user_data, 
                                        subs, 
  ...
  {
    callback_info = RegisterForNum(num, 
                                   tech_mask, 
                                   cback, 
                                   curr_callback_id, 
                                   user_data, 
  ...
  if (!callback_info)
  {
    PROFILE_DB_LOG_ERROR_2("Register(): could not register "
                           "for num %d, tech_mask %d", num, tech_mask);
    return 0;
  }
  ...
  ds_util_list_hndl_type client_ids = 
    (mode_handler) ? mh_valid_clients_ids[0] : valid_clients_ids[subs];

  if (ds_util_list_add(client_ids, 
                       &cb_node, 
                       sizeof(cb_node)) != DS_SUCCESS)
  {
                                                       ...
                                                       ds_profile_db_subs_type subs)
{
  ...
  ds_util_list_hndl_type client_ids = 
    (callback_id & MH_CALLBACK_ID_FLAG) ? mh_valid_clients_ids[0] : valid_clients_ids[subs];

  callback_id_node = ds_util_list_find_node(client_ids, 
                                            &callback_id, 
                                            sizeof(callback_id));
  if (!callback_id_node)
  {
    PROFILE_DB_LOG_ERROR_1("Deregister(): callback_id 0x%x is "
                           "not in the list of valid callbacks", callback_id);
  ...
  if (ds_util_list_remove(client_ids, 
                          &callback_id, 
                          sizeof(callback_id)) != DS_SUCCESS)
  {
    return DB_RESULT_ERR_INVAL;
  }
...
{
  ds_cmd_type *ds_cmd = NULL;

   /* This NotifyInternal will be used for SUBS READY Event */
  if (DB_PROFILES_READY_ON_SUBS_EVENT != event)
  {
  ...
  ds_cmd = ds_allocate_cmd_buf(sizeof(ds_profile_db_callback_cmd_type));
  if (!ds_cmd)
  {
  ...
  cmd->operation_type = DS_PROFILE_DB_SUBS_READY_NOTIFY_CMD;
  cmd->notify_subs_ready_cmd.event = event;
  cmd->notify_subs_ready_cmd.callback_id = callback_id;
  cmd->notify_subs_ready_cmd.cback = cback;
  cmd->notify_subs_ready_cmd.subs = subs;
  cmd->notify_subs_ready_cmd.user_data = user_data;

  PROFILE_DB_LOG_HIGH_5("NotifyInternal(): posting %d cmd for event %d with callback_id %d cback 0x%x on sub: %d "
                         "profiles: 0x%x",
                        cmd->operation_type,
                        cmd->notify_subs_ready_cmd.event,
                        callback_id,
                        cback,
                        cmd->notify_subs_ready_cmd.subs);
  ...
  ds_profile_db_subs_type subs)
{
  ...
  PROFILE_DB_LOG_HIGH_5("NotifyInternal(): posting %d cmd for event %d with profile_count %d on sub: %d "
                         "profiles: 0x%x",
                        cmd->operation_type,
                        cmd->notify_cmd.event,
                        cmd->notify_cmd.profiles_count,
                        cmd->notify_cmd.subs,
                        cmd->notify_cmd.profiles);

  if (event != DB_REFRESH_PROFILE_EVENT && event != DB_PROFILES_READY_ON_SUBS_EVENT)
  {
  ...
  bool treat_tech_mask_as_OR /*= false */)
{
  ...
  for (unsigned int i = 0; i < profiles_count_for_power_up; i++)
  {
    ...
    if (IsMaskSupported(tech_mask, 
                        treat_tech_mask_as_OR, 
                        profiles_for_power_up[i]->GetSupportedTechMask(), NULL))
    {
      profiles_count++;
    }
  ...
  if (!IS_IN_DS_TASK())
{
  ...
  if (in_callback)
  {
    PROFILE_DB_LOG_HIGH("WithinCallback(): Calling DB api from within Callback, "
...
{    
  ...
  switch (cmd_info->operation_type)
  {
    ...
    case(DS_PROFILE_DB_SUBS_READY_NOTIFY_CMD):
      return NotifySubsReadyHandler(&(cmd_info->notify_cmd)); /* mutex is released inside */    
    default:
...
void CallbackMgrModem::NotifyMH(void* cmd)
{
  ...
  {
    PROFILE_DB_LOG_ERROR("NotifyMH(): could not allocate memory "
                         "for gathered_callbacks list");
    theInstance->mutex->Release();
    return;
  }

  NotifyNumClients((ds_profile_db_notify_cmd_type *)cmd, 
                   theInstance->mh_clients_per_number[0], 
                   gathered_callbacks);
  NotifyTechMaskClients((ds_profile_db_notify_cmd_type *)cmd, 
                        theInstance->mh_clients_per_techmask[0], 
                        gathered_callbacks);
...
void CallbackMgrModem::NotifyHandler(void* cmd)
{
  ...
  else if(notify_cmd->event == DB_PROFILES_READY_ON_SUBS_EVENT)
  {
    return NotifySubsReadyToAllClientsHandler(cmd);
  }
  if (ds_util_list_get_hndl(&gathered_callbacks) != DS_SUCCESS)
  {
  ...
  NotifyNumClients((ds_profile_db_notify_cmd_type *)cmd, 
                   theInstance->clients_per_number[subs], 
                   gathered_callbacks);
  NotifyTechMaskClients((ds_profile_db_notify_cmd_type *)cmd, 
                        theInstance->clients_per_techmask[subs], 
                        gathered_callbacks);
...
void CallbackMgrModem::NotifyRefreshHandler(void *cmd)
{
      ...
      DB_PROFILE_ACTIVE_SUBSCRIPTION_MAX <= subs)
  {
    ds_util_list_rel_hndl(gathered_callbacks);
    theInstance->mutex->Release();
    PROFILE_DB_LOG_ERROR_1("ValidateSubscriptionId(): invalid subs_id %d", 
                           subs);
    return;
  }
...
void CallbackMgrModem::NotifySubsReadyHandler(void *cmd)
{
  ds_profile_db_subs_type subs;
  ds_profile_db_notify_subs_ready_cmd_type *notify_cmd = (ds_profile_db_notify_subs_ready_cmd_type *)cmd;

  subs = (ds_profile_db_subs_type)(notify_cmd->subs);
  if (DB_ACTIVE_SUBSCRIPTION_NO_CHANGE > subs ||
      DB_PROFILE_ACTIVE_SUBSCRIPTION_MAX <= subs)
  {
    PROFILE_DB_LOG_ERROR_1("NotifySubsReadyHandler(): invalid subs_id %d", 
                           subs);
    return;
  }
  PROFILE_DB_LOG_MED_5("NotifySubsReadyHandler():cback 0x%x callback id %d event %d user_data 0x%x subs %d ",
                       notify_cmd->cback, 
                       notify_cmd->callback_id,
                       notify_cmd->event,
                       notify_cmd->user_data,
                       subs);

  theInstance->InvokeCBInternal(notify_cmd->cback, 
                   notify_cmd->callback_id,
                   notify_cmd->event, 
                   NULL,
                   0,
...
void CallbackMgrModem::NotifySubsReadyToAllClientsHandler(void *cmd)
{
  ds_util_list_hndl_type gathered_callbacks;
  ds_profile_db_subs_type subs;
  ds_profile_db_notify_cmd_type *notify_cmd = (ds_profile_db_notify_cmd_type *)cmd;
  ...
  {
    theInstance->mutex->Release();
    PROFILE_DB_LOG_MED("NotifySubsReadyToAllClientsHandler(): ");
    return;
  }
  subs = (ds_profile_db_subs_type)(notify_cmd->subs);
  if (DB_ACTIVE_SUBSCRIPTION_NO_CHANGE > subs ||
      DB_PROFILE_ACTIVE_SUBSCRIPTION_MAX <= subs)
  {
    ds_util_list_rel_hndl(gathered_callbacks);
    theInstance->mutex->Release();
    PROFILE_DB_LOG_ERROR_1("NotifySubsReadyToAllClientsHandler(): invalid subs_id %d", 
                           subs);
    return;
  }
  ...
  Profile*** profiles_copy)
{
  ...
  for (unsigned int i = 0; i < profiles_count; i++)
  {
    ...
    PROFILE_DB_LOG_HIGH_2("CallbackMgrModem::CopyProfiles profiles Copy (): "
                          "profile num %d, profile subs %d",
                          (*profiles_copy)[i]->GetNum(),
