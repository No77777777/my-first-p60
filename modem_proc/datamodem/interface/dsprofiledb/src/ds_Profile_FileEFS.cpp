...
static int FlagTypeToEFSflag(File::FlagType flag)
{
  switch (flag)
  {
    ...
    case File::F_WRITE:
      return MCFG_FS_O_WRONLY | MCFG_FS_O_TRUNC; 
      /* file is always truncated to 0 when written, TODO revisit in Phase 2 */
    case File::F_WRITE_CREATE:
      return MCFG_FS_O_WRONLY | MCFG_FS_O_CREAT | MCFG_FS_O_TRUNC; 
      /* file is always truncated to 0 when written, TODO revisit in Phase 2 */
    case File::F_RW:
                                               ...
                                               char *file_name)
{
  typedef enum
  {
    DS_PROFILE_DB_ERR_EFS_NONE    = 0,
    DS_PROFILE_DB_ERR_EFS_STRCPY  = 1,
    DS_PROFILE_DB_ERR_EFS_SPRINTF = 2
  }ds_profile_db_local_error_type;

  ds_profile_db_local_error_type local_err =
    DS_PROFILE_DB_ERR_EFS_NONE;

  ds_profile_db_result_type res = DB_RESULT_SUCCESS;

  do 
  {
    res = Profile::ValidateProfileNum(profile_num);
    ...
    {
      local_err = DS_PROFILE_DB_ERR_EFS_STRCPY;
    ...
    if (!ret) {
      local_err = DS_PROFILE_DB_ERR_EFS_SPRINTF;
      res = DB_RESULT_FAIL;
      break;
    }
...
static ds_profile_db_result_type CheckAndCreateDirectory()
{
  ...
  efs_res = mcfg_fs_stat(PROFILE_DIR, 
                         &dont_care, 
                         FileEFS::file_type, 
                         mcfg_subs_id);
  ...
  {
    if (-1 == mcfg_mkdir(PROFILE_DIR, 
                         MCFG_FS_ALLPERMS, 
                         FileEFS::file_type))
    {
      PROFILE_DB_LOG_ERROR("CheckandCreateDirectory(): "
...
FileEFS::FileEFS(ds_profile_db_profile_num_type num, 
                                        ...
                                        /*= DB_PROFILE_ACTIVE_SUBSCRIPTION_1*/)
{
  ...
  do 
  {
    ...
    {
      fs_handle = mcfg_fopen(file_name, 
                             efs_flag, 
                             MCFG_FS_DEFFILEMODE, 
                             file_type, 
                             mcfg_subs_id);
  ...
  PROFILE_DB_LOG_HIGH_4("Open(): exiting with EFS error %d,"
                        "res %d, local_err %d, fs_handle %d",
                        mcfg_fs_errno( file_type ),
                        res, local_err, fs_handle);
  return res;
}
...
ds_profile_db_result_type FileEFS::Close(void)
{
  ...
  {
    PROFILE_DB_LOG_ERROR_1("Close(): could not close file, error %d",
                           mcfg_fs_errno(file_type));
    res = DB_RESULT_ERR_FILE_ACCESS;
  }
  fs_handle = -1;
                                         ...
                                         bool is_3gpp2_default_profile)
{
  ...
  return Create(profile, is_reset_event, 
                reset_profile, is_create, 
...
ds_profile_db_result_type FileEFS::Read(Profile **profile) const
{
  ...
  */
  ...
  gen_params.profile_client = efs_header.profile_client;
  ...
  {
    PS_SYSTEM_HEAP_MEM_ALLOC(tech_param,
                             Profile::GetCStructSize(tech),
                             void *);
    Profile::GetDefaultCParams(tech, tech_param);
    ...
    tmpProf->SetProfileclient(gen_params.profile_client,FALSE);
    PS_SYSTEM_HEAP_MEM_FREE(tech_param);
  }
  ...
  *profile = tmpProf;
  if ((DB_RESULT_SUCCESS != res))
  {
    PROFILE_DB_LOG_ERROR_2("Read(): could not create profile num %d "
                           "from tech part %d or from general params read from EFS",
                           gen_params.tech_mask, gen_params.profile_number);
  }
  return res;
}
                                          ...
                                          ds_profile_db_subs_type subs)
{
  ...
  if (MCFG_FS_STATUS_OK != mcfg_fs_delete(file_name, 
                                          file_type, 
                                          mcfg_subs_id))
  {
    PROFILE_DB_LOG_ERROR_1("Delete(): could not delete file, error %d", 
                           mcfg_fs_errno( file_type ));
    return DB_RESULT_ERR_FILE_ACCESS;
  }
                                          ...
                                          bool is_3gpp2_default_profile)
{
  ...
  */
  ...
  if (reset_profile || is_create)
  {
    ...
    efs_header.profile_client = profile->GetProfileclient();
    efs_header.size = 0;
    ...
    num_bytes = mcfg_fwrite(fs_handle, 
                            &efs_header, 
                            sizeof(efs_header), 
                            file_type);
  ...
  {
    READ_FROM_INDEX(heapMemForFile, 
  ...
  if (size_to_write != mcfg_fwrite(fs_handle, 
                                   heapMemForFile, 
                                   size_to_write, 
                                   file_type))
  {
                                           ...
                                           ds_profile_db_subs_type subs /*= DB_PROFILE_ACTIVE_SUBSCRIPTION_1*/)
{
  ...
  {
      PROFILE_DB_LOG_HIGH_STR_1("ReadAll(): Could not open "
                                "profiles directory: %s , creating it", PROFILE_DIR);
  ...
  mcfg_subs_id = ds_profile_db_convert_ds_to_mcfg_subs(subs);
  while ((dirent = mcfg_fs_readdir_subid(dirp, 
                                         file_type, 
                                         mcfg_subs_id)) != 0)
  {
    ...
    if (S_ISREG(fmode) || S_ISITM(fmode))
    {
      ...
      profiles[new_count]->SetSubscriptionId(subs);
      new_count++;
...
bail:
  ...
  {
    PROFILE_DB_LOG_ERROR_2("ReadAll(): error occurred %d, mcfg_fs_errno( file_type ) %d", 
                            res, mcfg_fs_errno( file_type ));
  }
