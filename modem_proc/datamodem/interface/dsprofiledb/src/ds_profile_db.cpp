...
{
  return DBMgr::Instance()->DeleteProfileAttach(num, tech_mask, subs);
}
...
{
  return DBMgr::Instance()->ProfilesReadyOnSubs(subs);
}
...
{
  return DBMgr::Instance()->GetProfileclient(profile_num, subs, profile_client);
}
...
)
{
  ...
  if (!delete_profile_list_head_ptr)
  {
    ...
    if (delete_profile_list_head_ptr)
    {
      ...
      delete_profile_list_head_ptr->next_ptr = NULL;
  ...
  else
  {
    if (profile_delete_count == 0)
    {
      ...
      temp_next_profile = temp_delete_profile->next_ptr;
      memscpy(temp_delete_profile, sizeof(ds_profile_db_deleted_profile_entry),
              deleted_profile, sizeof(ds_profile_db_deleted_profile_entry));
      temp_delete_profile->next_ptr = temp_next_profile;
      temp_delete_profile = temp_delete_profile->next_ptr;
      deleted_profiles++;
    ...
    {
      temp_delete_profile = delete_profile_list_head_ptr;
      while(temp_delete_profile->next_ptr != NULL && profile_delete_count)
      {
        temp_delete_profile = temp_delete_profile->next_ptr;
      }

      PS_SYSTEM_HEAP_MEM_ALLOC(temp_delete_profile->next_ptr, 
                               sizeof(ds_profile_db_deleted_profile_entry),
                               ds_profile_db_deleted_profile_entry*);
      if (temp_delete_profile->next_ptr)
      {
        memscpy(temp_delete_profile->next_ptr, sizeof(ds_profile_db_deleted_profile_entry),
                deleted_profile, sizeof(ds_profile_db_deleted_profile_entry));
        temp_delete_profile->next_ptr->next_ptr = NULL;
        profile_delete_count--;
        if (delete_profile_list_head_ptr->next_ptr == NULL)
        {
          delete_profile_list_head_ptr->next_ptr = temp_delete_profile;
        }
        if (profile_delete_count == 0)
        {
          temp_delete_profile = delete_profile_list_head_ptr;
...
mcfg_fs_sub_id_e_type ds_profile_db_convert_ds_to_mcfg_subs(ds_profile_db_subs_type subs)
{
  ...
  /* mcfg_subs_id is similar to SYS MODEM SUBS ID */
...
{
  ds_sys_subscription_enum_type ds_sys_subs_id = DS_SYS_DEFAULT_SUBS;
  switch (subs)
  {
    case DB_PROFILE_ACTIVE_SUBSCRIPTION_1:
      ds_sys_subs_id = DS_SYS_PRIMARY_SUBS;
      break;
    case DB_PROFILE_ACTIVE_SUBSCRIPTION_2:
      ds_sys_subs_id = DS_SYS_SECONDARY_SUBS;
      break;
    case DB_PROFILE_ACTIVE_SUBSCRIPTION_3: 
      ds_sys_subs_id = DS_SYS_TERTIARY_SUBS;
      break;
    default:
      break;
  }
  return ds_sys_subs_id;
}
