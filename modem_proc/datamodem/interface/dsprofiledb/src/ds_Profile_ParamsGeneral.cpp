...
ds_profile_db_result_type ParamsGeneral::Init(void)
{
  ...
  DB_FILL_PARAM_MAP(GENERAL_PROFILE_PARAM_ATTACH_PROFILE_FLAG,
	                  is_attach);

  DB_FILL_PARAM_MAP(GENERAL_PROFILE_PARAM_CLAT_ENABLE_FLAG,
                    clat_enabled);
  
  DB_FILL_PARAM_MAP (GENERAL_PROFILE_PARAM_APN_TYPE, 
                     apn_type_mask);
...
void ParamsGeneral::GetDefaultCParams(ds_profile_db_gen_params_type* params)
{
  ...
  gen_params->is_attach = false;
  gen_params->clat_enabled = false;
  gen_params->apn_type_mask = 0;
}
...
ds_profile_db_result_type ParamsGeneral::SetProfileclient( uint8 profile_client, bool track )
{
  ds_profile_db_result_type res;

  if (track)
  {
    if (DB_RESULT_SUCCESS != (res = TrackParamChange(GENERAL_PROFILE_PARAM_PROFILE_CLIENT)))
    {
      return res;
    }
  }

  params.profile_client = profile_client;
...
ds_profile_db_result_type ParamsGeneral::SetApnTypeMask( uint64 apn_type_mask, bool track)
{
  ds_profile_db_result_type res;

  if (track)
  {
    if (DB_RESULT_SUCCESS != (res = TrackParamChange(GENERAL_PROFILE_PARAM_APN_TYPE)))
    {
      return res;
    }
  }

  params.apn_type_mask = apn_type_mask;
...
uint8 ParamsGeneral::GetProfileclient(void)
{
  return params.profile_client;
}
...
uint64 ParamsGeneral::GetApnType(void)
{
  return params.apn_type_mask;
}

ds_profile_db_result_type ParamsGeneral::SetAttachProfileFlag( boolean is_attach, bool track)
{
  ds_profile_db_result_type res;

  if (track)
  {
    if (DB_RESULT_SUCCESS != (res = TrackParamChange(GENERAL_PROFILE_PARAM_ATTACH_PROFILE_FLAG)))
    {
      return res;
    }
  }

  params.is_attach = is_attach;

  return DB_RESULT_SUCCESS;
}

boolean ParamsGeneral::GetAttachProfileFlag(void)
{
  return params.is_attach;
}
