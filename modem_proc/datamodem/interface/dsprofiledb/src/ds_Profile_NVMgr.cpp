...
)
{
  ...
  ds_profile_db_efs_read_state_e_type  efs_state
    = DB_FILE_READ_STATE_TYPE_NONE;

  memset(&file_path[0], 0 , sizeof(file_path));
  memset(&ds_profile_db_epc_support, 0 , 
         sizeof(ds_profile_db_epc_support));

  temp_data = strlcpy(file_path,  
                      PROFILE_DIR, 
                      sizeof(file_path));
  ...
  switch (file_type) 
  {
    case DB_EFS_FILE_TYPE_EPC_SUPPORT:
      ...
      if (strlcpy(&file_path[temp_data], 
                  EPC_FILE, 
...
bail:
   PROFILE_DB_LOG_HIGH_1("NVMgr ReadEFSFile State = %d", efs_state);
   return;
}

ds_profile_db_result_type NVMgr::ds_profile_db_nv_init_efs_conf_file
(
  void
)
{
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  PROFILE_DB_LOG_MED_1("Read From NV returning status=%d", status);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  PROFILE_DB_LOG_MED_1("BuildPath From NV returning status=%d", status);
