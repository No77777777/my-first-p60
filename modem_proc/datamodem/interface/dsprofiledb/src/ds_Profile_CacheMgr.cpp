  ...
  ds_profile_db_common_tech_params_type *common)
{
  ...
  cache_block->cache.profile_client = profile_client;
  cache_block->cache.is_persistent = is_persistent; 
  cache_block->cache.is_attach = FALSE;
  if (cache_block->cache.tech_mask & DB_TECH_TO_TECHMASK(DB_TECH_COMMON) &&
          common)
      {
...
{
  #define CACHE_3GPP2_TECH_MASK 2
  Profile *tmpProf;
  ds_profile_db_result_type res = DB_RESULT_SUCCESS;
  cache_entry_block *cache_block = NULL;
      ...
      if (tech_mask == cache_block->cache.tech_mask)
      {
        if (!num_omh_profiles &&
           (cache_block->cache.num >= omh_min_num 
            && tech_mask == CACHE_3GPP2_TECH_MASK
        ...
        if (DB_RESULT_SUCCESS != res)
        {
          ...
          {
            cache_block = 
              (cache_entry_block *)q_next(&cache[subs], 
                                          &(cache_block->link));
            continue;
          } 
          else
        ...
        if (INVALID_PROFILE_PARAM != ident)
        {
          if (tmpProf)
          {
            if (!tmpProf->CompareParam(ident, param_info))
            {
              delete tmpProf;
              tmpProf = NULL;
              cache_block = (cache_entry_block *)q_next(&cache[subs], 
                                                        &(cache_block->link));
              continue;
            }
          }
        ...
        {
          PROFILE_DB_LOG_ERROR("GetProfilesFromInternalCache "
                               "could not get temp Profile");
        }
      }
    }
    cache_block = (cache_entry_block *)q_next(&cache[subs], 
...
ds_profile_db_result_type CacheMgr::GetProfileclient(ds_profile_db_profile_num_type num,
                                                   ds_profile_db_subs_type subs,
                                                   uint8 *profile_client)
{
  TechMgr *techMgr = NULL;
...
/*---------------------------------------------------------------------------------------*/
  ...
  PROFILE_DB_LOG_ERROR("GetProfileclient!!!");
  while (cache_block)
  {
    if (num == cache_block->cache.num)
    {
      ...
      if ((techMgr = DBMgr::Instance()->GetTechManager(tech)))
      {
        if (first_tech)
        {
          ...
          if (profile_client)
          {
            *profile_client = profile_general_params.profile_client;
             if(cache_block->cache.profile_client)
             {
               *profile_client = cache_block->cache.profile_client;
             }
          } 
          else
          {
...
ds_profile_db_result_type CacheMgr::GetAllAttachProfiles
(
  ds_profile_db_subs_type        subs,
  ds_profile_db_tech_type        tech_type,
  ds_profile_db_profile_num_type *nums,
  uint32                         *num_attach_profiles
) 
{
  cache_entry_block  *cache_block        = NULL;
  uint32             num_attach_prf      = 0;
/*-------------------------------------------------------------------*/
  ...
  while (cache_block)
  {
    if (subs == cache_block->cache.subs &&
        tech_type == ds_profile_db_tech_mask_to_tech(
           cache_block->cache.tech_mask)
        && cache_block->cache.is_attach)
    {
         nums[num_attach_prf++] = 
           cache_block->cache.num;
    }
    cache_block = (cache_entry_block*)q_next(&cache[subs], &(cache_block->link));
  }
  *num_attach_profiles = num_attach_prf;
  return DB_RESULT_SUCCESS;
}

ds_profile_db_result_type CacheMgr::GetAttachFlag
(
  ds_profile_db_profile_num_type    num,
  ds_profile_db_subs_type           subs,
...
/*---------------------------------------------------------------------------------------*/
  ...
  if (is_attach)
  {
    ...
    {
      if (num == cache_block->cache.num &&
          tech_type == cache_block->cache.tech_mask
          )
      {
        *is_attach = cache_block->cache.is_attach;
         break;
      }
      cache_block = (cache_entry_block*)q_next(&cache[subs], &(cache_block->link));
    }
...
ds_profile_db_result_type CacheMgr::SetAttachFlag
(
  ds_profile_db_profile_num_type    num,
  ds_profile_db_subs_type           subs,
  ds_profile_db_supported_tech_mask tech_type,
...
/*---------------------------------------------------------------------------------------*/
  ...
  if (is_attach)
  {
    while (cache_block)
    {
      ...
      {
        cache_block->cache.is_attach = is_attach;
        break;
      }
      cache_block = (cache_entry_block*)q_next(&cache[subs], &(cache_block->link));
    }
