...
static void SpecialSetAuthUserNameParam(
  const ds_profile_db_info_type* info,
  void*                          params)
{
  ds_profile_db_common_tech_params_type* profile;
	...
	PROFILE_DB_LOG_ERROR("SpecialSetAuthUserNameParam");
	/*if(profile)
  	{
	for(int i=0;profile->auth_info.username[i]!= '\0' && i<3;i++)
	{
	  PROFILE_DB_LOG_ERROR_1("User Name param %c ",profile->auth_info.username[i]);
	}
	}*/

}

static void SpecialSetAuthPasswdParam(
  const ds_profile_db_info_type* info,
  void*                          params)
{
  ds_profile_db_common_tech_params_type* profile;
  ...
  PROFILE_DB_LOG_ERROR("SpecialSetAuthPasswdParam");
  
}

static void SpecialSetAuthtypeParam(
  const ds_profile_db_info_type* info,
  void*                          params)
{
  ds_profile_db_common_tech_params_type* profile;
...
{  
  switch (ident)
  {
    ...
    case(COMMON_TECH_PROFILE_PARAM_AUTH_USERNAME):
      return SpecialSetAuthUserNameParam(info, params);
    case(COMMON_TECH_PROFILE_PARAM_AUTH_PASSWORD):
      return SpecialSetAuthPasswdParam(info, params);
    case(COMMON_TECH_PROFILE_PARAM_AUTH_TYPE):
      return SpecialSetAuthtypeParam(info, params);
    default:
      return;
  }
}
  ...
  const void*                    params)
{
  ...
  PROFILE_DB_LOG_ERROR("SpecialGetAuthPass");
  ...
  const void*                    params)
{
  ...
  PROFILE_DB_LOG_ERROR("SpecialGetAuthName");

  return DB_RESULT_SUCCESS;
}

static ds_profile_db_result_type SpecialGetAuthType(
  ds_profile_db_info_type*       info,
  const void*                    params)
{
  ds_profile_db_common_tech_params_type* profile;
  profile = (ds_profile_db_common_tech_params_type*)params;
  PROFILE_DB_LOG_ERROR("SpecialGetAuthType");
  return DB_RESULT_SUCCESS;
}
...
{  
  ...
  {     
    ...
    case(COMMON_TECH_PROFILE_PARAM_AUTH_TYPE):
      return SpecialGetAuthType(info, params); 
    case(COMMON_TECH_PROFILE_PARAM_DNS_ADDR_V4_PRIMARY):
