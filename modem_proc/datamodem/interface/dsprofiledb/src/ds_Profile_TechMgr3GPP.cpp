  ...
  if (during_init)
{
  ...
  {
    PROFILE_DB_LOG_HIGH_3("3GPP: Can ReserveOutside of Allocation func num %d, "
                          "during_init %d, is_epc %d", 
                          num, during_init, is_epc);

    if (num >= DB_3GPP_CREATE_RANGE_MIN && 
        num <= DB_3GPP_CREATE_RANGE_MAX)
    {
      if (is_epc)
      {
        return false;
    ...
    else if (num >= TechMgr::ALLOC_RANGE_MIN && 
                                   ...
                                   boolean enable_deleting_default_profile) const
{
  ...
  for (int i = DB_EMBEDDED_PROFILE_FAMILY; i < DB_MAX_PROFILE_FAMILY; i++)
  {
    ...
    {
      PROFILE_DB_LOG_ERROR_1("CanDeleteProfile(): could not get default profile num, "
                             "error %d, returning false", res);
      return false;
    }

    if (num == default_num)
    {
      PROFILE_DB_LOG_ERROR_1("CanDeleteProfile(): can't delete default "
                             "profile for family %d", i);
      return false;
    }
      
    if (num == DB_3GPP_DEFAULT_PROFILE)
    {
      PROFILE_DB_LOG_ERROR_1("CanDeleteProfile(): can't delete default "
                             "profile for family %d", i);
      return false;
         ...
         j < DB_PROFILE_ACTIVE_SUBSCRIPTION_MAX; j++)
    {
      ...
      {
        PROFILE_DB_LOG_ERROR_1("CanDeleteProfile(): could not get "
                               "default profile num, error %d, returning "
                               "false", res);
        return false;
      }
      ...
      if (num == default_num)
      {
        PROFILE_DB_LOG_ERROR_2("CanDeleteProfile(): can't delete default "
                               "profile for family %d, subs %d", i, j);
        return false;
      }
    }
  }
...
{
  return ds_3gpp_profile_get(num, subs, (ds_profile_db_3gpp_tech_params_type *)tech_params, gen_params);
}
...
{
  return ds_3gpp_profile_get_default_profile_num_per_subs(num, family, subs);
}
...
ds_profile_db_result_type TechMgr3GPP::SetDefaultProfileNum(
  ...
  ds_profile_db_subs_type subs) const
{
  return ds_3gpp_profile_set_default_profile_num_per_subs(num, family, subs);
}
...
ds_profile_db_result_type TechMgr3GPP::GetIsEpcParamSet(
  ...
  ds_profile_db_subs_type subs) const
{
  return ds_3gpp_profile_is_epc_tech_param_set(num, subs, ident, is_tech_param_set);
}
