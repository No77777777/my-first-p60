...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/rmifacectls/inc/ds_rmnet_xport.h#2 $
...
/*-------------------------------------------------------------------------- 
 The Data Port Buffer Info Parameters during enumeration
----------------------------------------------------------------------------*/
typedef PACKED struct PACKED_POST
{
  ...
  uint32 dl_fifo_size;
  uint32 dl_buf_size;
} rmnet_phys_xport_buffer_info_type; 
...
typedef struct
{
  ...
  uint32                           flow_mask;
  uint32                           pipe_pkt_cnt;    // UL pipe pkt count
  uint64                           pipe_pkt_cnt_ts; // pkt count timestamp
  boolean                          pipe_inactive;   // is pipe inactive
  rmnet_phys_xport_buffer_info_type phys_xport_buffer_info; //Data Aggregation parameters
...
typedef struct rmnet_xport_type_s
{
  ...
  uint32                           offload_fc_mask; // Offload FC mask
  ...
  timer_type                       ep_cnf_timer; //debug code
  struct rmnet_xport_type_s      * next_ptr;
} rmnet_xport_type;
...
#define RMNET_XPORT_UL_DATA_AGG_MAX_NUM(xport) \
  ((xport) == NULL ? 1 : (xport)->data_format_cache.ul_data_agg_max_num)
