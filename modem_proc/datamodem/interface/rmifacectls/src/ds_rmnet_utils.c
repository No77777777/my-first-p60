...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/rmifacectls/src/ds_rmnet_utils.c#2 $
...
#define DS_RMNET_UTILS_ASSERT(expression) \
         ds_rmnet_utils_assert_wrapper(__LINE__, expression)
...
FUNCTION DS_RMNET_UTILS_ASSERT_WRAPPER()

DESCRIPTION
  Wrapper function for DS_RMNET_UTILS ASSERT
...
===========================================================================*/
static void ds_rmnet_utils_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("DS_RMNET_UTILS_ERR_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
}/* ds_rmnet_utils_assert_wrapper */
...
/*===========================================================================
  ...
  PARAMETERS
    ...
    is_filter_q_empty : TRUE(only when packet filter queue is empty) or FALSE

  RETURN VALUE
    boolean    : TRUE for success, FALSE otherwise
                 (if there are no items in the packet filter cache,
                  return TRUE with packet filter state as disabled)
...
/*-------------------------------------------------------------------------*/
  ...
  /*-------------------------------------------------------------------------
    No entry found in cache so return True with packet filter 
    state as disabled by default
  -------------------------------------------------------------------------*/ 
  if (NULL == packet_filter_item)
  {
    *is_filter_q_empty = TRUE;
    return TRUE;
  }
