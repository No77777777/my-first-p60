...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/rmifacectls/src/ds_qmap.c#2 $
...
#define DS_QMAP_ASSERT(expression) \
         ds_qmap_assert_wrapper(__LINE__, expression)
...
typedef PACKED struct PACKED_POST
{
  ...
  uint16                       reserved2;
  uint8                        bearer_id;
} qmap_cmd_ul_data_sent_conf_type;  
#endif/* FEATURE_DATA_FLOW_MGMT*/ 
/*---------------------------------------------------------------------------
...
static void ds_qmap_process_qmap_cmd
(
  sio_stream_id_type          stream_id,
  uint8                       cmd_name,   
  uint8                       cmd_type,
...
FUNCTION DS_QMAP_ASSERT_WRAPPER()

DESCRIPTION
  Wrapper function for DS_QMAP ASSERT
...
===========================================================================*/
static void ds_qmap_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("DS_QMAP_ERR_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
}/* ds_qmap_assert_wrapper */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (!cmd_info_ptr)
  {
    ...
    PS_SYSTEM_HEAP_MEM_ALLOC_ASSERT(qmap_cmd_info, sizeof(ds_qmap_cmd_info_type),
    ...
    PS_SYSTEM_HEAP_MEM_ALLOC_ASSERT(qmap_cmd_info->cmd_ptr, cmd_size, void*);
...
)
{
 ...
 /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  cmd_ptr = (ds_qmap_cmd_buf_type *)
                    ds_qmap_get_cmd_buf(QMI_CMD_QMAP_CMD_CB);
  if (cmd_ptr != NULL)
  {
    cmd_ptr->cmd_id = QMI_CMD_QMAP_CMD_CB;
...
/*-------------------------------------------------------------------------*/
  ...
  if (cmd_ptr == NULL)
  {
    DS_UTILS_ERROR_FATAL("NULL QMAP cmd_ptr!");
    return;
  }
  ...
  {
    case QMI_CMD_QMAP_CMD_CB:
      if ( DS_QMAP_CMD_TYPE_ACK == cmd_buf_ptr->qmap.qmap_resp.cmd_type )
      {
      ...
      else if ( DS_QMAP_CMD_TYPE_REQUEST == cmd_buf_ptr->qmap.qmap_resp.cmd_type )
      {
        ds_qmap_process_qmap_cmd (cmd_buf_ptr->qmap.qmap_resp.stream_id,
                                  cmd_buf_ptr->qmap.qmap_resp.cmd_name,
                                  cmd_buf_ptr->qmap.qmap_resp.cmd_type,
                                  cmd_buf_ptr->qmap.qmap_resp.tx_id,
                                  cmd_buf_ptr->qmap.qmap_resp.dsm_ptr);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  PS_SYSTEM_HEAP_MEM_ALLOC_ASSERT(cmd_buf_ptr, sizeof(ds_qmap_cmd_buf_type),
                           void*);
  return cmd_buf_ptr;
...
/*===========================================================================
  FUNCTION DS_QMAP_PROCESS_QMAP_CMD()
...
)
{
  ...
  void                              * value;
  uint16                              pkt_size;
  qmap_cmd_ul_data_sent_conf_type     ul_data_cmd;
  uint16                              seq_num= 0;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  memset(&ul_data_cmd, 0x0, sizeof(ul_data_cmd));
  ...
  do
  {
    ...
    if (DS_QMAP_CMD_UL_DATA_SENT_CNF != rmnet_shim_ipa_to_ds_qmap_cmd ((uint32)cmd_name))
    {
      break;
    }
    /*-------------------------------------------------------------------------
   ...
   seq_num = ((ul_data_cmd.seq_num & 0x00FF) << 8)|((ul_data_cmd.seq_num & 0xFF00) >> 8);
    
   qmi_dfc_ul_data_sent_ack_rcvd(ul_data_cmd.conf_version, ul_data_cmd.conf_type, 
    seq_num, ul_data_cmd.bearer_id);
    
  }while(0);

  LOG_MSG_INFO1_9("ds_qmap_process_qmap_cmd Recvd QMAP cmd [%d] version [%d] type [%d] "
                  "seq num [%d] bearer id [%d] stream id [0x%x] tx_id [0x%x] pkt size %d expected size %d",
                     cmd_name,
                     ul_data_cmd.conf_version,
                     ul_data_cmd.conf_type,
                     seq_num,
                     ul_data_cmd.bearer_id,
                     stream_id,
                     tx_id,
                     pkt_size,
                     sizeof(ul_data_cmd));
