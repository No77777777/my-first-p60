...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/rmifacectls/src/ds_rmnet_v6_sm.c#2 $
...
#define DS_RMNET_V6_SM_ASSERT(expression) \
         ds_rmnet_v6_sm_assert_wrapper(__LINE__, expression)
...
char *rmnet_v6_smi_event_names[RMNET_V6_SM_MAX_EV] =
{
...
FUNCTION DS_RMNET_V6_SM_ASSERT_WRAPPER()

DESCRIPTION
  Wrapper function for DS_RMNET_V6_SM ASSERT
...
===========================================================================*/
static void ds_rmnet_v6_sm_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("DS_RMNET_V6_SM_ERR_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
}/* ds_rmnet_v6_sm_assert_wrapper */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_RMNET_V6_SM_ASSERT(instance < RMNET_INSTANCE_MAX);
  info = rmnet_smi_info[instance];
  DS_RMNET_V6_SM_ASSERT(info != NULL);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_RMNET_V6_SM_ASSERT (0);
    LOG_MSG_ERROR_INT_1 ("Invalid RMNET_V6_SM event [d], ignored", event);
    return;
  }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  DS_RMNET_V6_SM_ASSERT (cmd_ptr && (cmd == DCC_RMNET_SM_EV_CMD) && (cmd_ptr->sm_id == RMNET_V6_SM));
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_RMNET_V6_SM_ASSERT (cmd_ptr && (cmd == DCC_RMNET_SM_CMD) && (cmd_ptr->sm_id == RMNET_V6_SM));
...
{
  ps_iface_event_enum_type rmnet_v6_addr_ev = IFACE_PREFIX_UPDATE_EV;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

  DS_RMNET_V6_SM_ASSERT(info->sub_sm_type == RMNET_V6_SM);
  ...
  switch( info->v6_sm.state )
  {
    case RMNET_V6_SM_NULL_STATE:
    {
      ...
      {
        if (NULL == (info->v6_sm.um_prefix_update_buf_ptr = ps_iface_alloc_event_handle(info->um_iface_ptr,
                                                                                        rmnet_v6_smi_um_iface_ev_hdlr_cb,
                                                                                        (void *)info)))
        {
          DS_UTILS_ERROR_FATAL("Event buf allocation failed");
          return;
        }

        if (0 != ps_iface_subscribe_event_list(info->v6_sm.um_prefix_update_buf_ptr,
                                               &rmnet_v6_addr_ev,
                                               sizeof(rmnet_v6_addr_ev)/sizeof(ps_iface_event_enum_type)))
      {
        rmnet_meta_sm_post_event(RMNET_V6_SM_INFO_TO_INSTANCE(info), 
                                 RMNET_META_SM_ADDR_CFG_FAILURE_EV);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch( info->v6_sm.state )
  {
    ...
    default:
    {
      DS_UTILS_ERROR_FATAL("Bad RmNet V6 SM state!");
      break;
    }
  } /* switch( info->v6_sm.state ) */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if( RMNET_V6_SM != info->sub_sm_type )
  {
    LOG_MSG_ERROR_0("UM_PREFIX_ADDED_EV being processed when " 
                    "sub_meta_sm is not RMNET_V6_SM, ignoring");
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_RMNET_V6_SM_ASSERT (user_data_ptr != NULL);

  info     = (rmnet_smi_info_type *) user_data_ptr;
  nd_event = (ps_icmp6_nd_event_enum_type) event;

  LOG_MSG_INFO2_2( "Recvd ICMP6 ND ev %d on sub %d", nd_event, info->subs_id );
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_RMNET_V6_SM_ASSERT (userdata != NULL);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch( new_state )
  {
    ...
    case RMNET_V6_SM_NULL_STATE:
    {
      ...
      if(info->um_iface_ptr)
      {
        ps_iface_free_event_handle(info->v6_sm.um_prefix_update_buf_ptr);
        info->v6_sm.um_prefix_update_buf_ptr = NULL;
      }
      /*-------------------------------------------------------------------------
        Reset the sub state machine type.
      -------------------------------------------------------------------------*/
    ...
    case RMNET_V6_SM_DHCP6_WAITING_STATE:
    {
      ...
      v6_addr_ptr = PS_IFACE_GET_V6_PRIMARY_ADDR_PTR(info->rmnet_iface_ptr);
      v6_addr_ptr->prefix = ip_addr.addr.v6.ps_s6_addr64[0];
