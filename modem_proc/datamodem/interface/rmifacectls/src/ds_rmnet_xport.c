...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/rmifacectls/src/ds_rmnet_xport.c#2 $
...
#define RMNET_PHYS_XPORT_TRAFFIC_WIN 3000  // 3ms

#define DS_RMNET_XPORT_ASSERT(expression) \
         ds_rmnet_xport_assert_wrapper(__LINE__, expression)
...
FUNCTION DS_RMNET_XPORT_ASSERT_WRAPPER()

DESCRIPTION
  Wrapper function for DS_RMNET_XPORT ASSERT
...
===========================================================================*/
static void ds_rmnet_xport_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("DS_RMNET_XPORT_ERR_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
}/* ds_rmnet_xport_assert_wrapper */
...
)
{
  ...
  int                       i;
  rmnet_err_enum_type       err_code;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

  if (ep_id == 0 || phys_xport_buffer_info == NULL)
  {
  ...
  phys_xport->phys_xport_buffer_info   = *phys_xport_buffer_info;
  ...
  if (is_hw_port)
  {
    ...
    -----------------------------------------------------------------------*/
    err_code = rmnet_shim_set_per_mapping(phys_xport);
    if (err_code)
    {
      if(err_code != RMNET_ERR_MODEM_SSR)
      {
        DS_UTILS_ERROR_FATAL("Failed to set physical to peripheral mapping");
      }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  while (phys_xport->xport_list != NULL)
  {
    ...
    phys_xport->xport_list = xport_info->next_ptr;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  while (xport_info != NULL)
  {
    ...
    xport_info = xport_info->next_ptr;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  memset(&local_phys_xport_buffer_info, 0 , sizeof(rmnet_phys_xport_buffer_info_type));
  ...
  cmd_ptr->phys_xport_buffer_info = local_phys_xport_buffer_info;
    ...
    case RMNET_XPORT_CMD_PORT_OPEN:
      result = rmnet_phys_xport_open(cmd_ptr->port_id, cmd_ptr->ep_id,
                cmd_ptr->consumer_pipe_num, cmd_ptr->producer_pipe_num,
                                     &cmd_ptr->phys_xport_buffer_info); 
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (data_format != NULL)
  {
    ...
    if (data_format_mask & RMNET_DATA_FORMAT_MASK_UL_DATA_AGG_NUM)
    {
      data_format_to_set.ul_data_agg_max_num = data_format->ul_data_agg_max_num;
    }
    if (data_format_mask & RMNET_DATA_FORMAT_MASK_UL_DATA_AGG_SIZE)
    {
      data_format_to_set.ul_data_agg_max_size = data_format->ul_data_agg_max_size;
    }
  }
  ...
  {
    // No aggregation by default
    if (data_format_to_set.ul_data_agg_max_num == 0)
    {
      data_format_to_set.ul_data_agg_max_num = 1;
    }

    if (data_format_to_set.ul_data_agg_max_size == 0)
    {
      data_format_to_set.ul_data_agg_max_size = 0xFFFFFFFF;
    }
  }
  /*-------------------------------------------------------------------------
...
/*===========================================================================
  FUNCTION RMNET_PHYS_XPORT_HAS_UL_DIRECT_TRAFFIC()

  DESCRIPTION
    Check if a physical end point has direct (non-offloaded) UL data traffic

  PARAMETERS
    phys_xport : ptr to rmnet phys xport

  RETURN VALUE
    TRUE:  The channel has UL direct data traffic
    FALSE: no UL direct traffic 
  ...
  SIDE EFFECTS
    None
===========================================================================*/
boolean rmnet_phys_xport_has_ul_direct_traffic
(
  rmnet_phys_xport_type  * phys_xport
)
{
  uint32   pipe_pkt_cnt = 0;
  uint64   ts;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  // Get HW stats. Assume has traffic if failed.
  if (!rmnet_shim_get_pipe_stats(
             phys_xport->consumer_pipe_num,
             &pipe_pkt_cnt))
  {
    return TRUE;
  }

  // If packet count has no change -- no traffic.
  if (pipe_pkt_cnt == phys_xport->pipe_pkt_cnt)
  {
    if (!phys_xport->pipe_inactive)
    {
      ts = time_get_ms_native();
      if (phys_xport->pipe_pkt_cnt_ts == 0)
      {
        phys_xport->pipe_pkt_cnt_ts = ts;
      }
      else if (ts - phys_xport->pipe_pkt_cnt_ts > RMNET_PHYS_XPORT_TRAFFIC_WIN)
      {
        phys_xport->pipe_inactive = TRUE;
        LOG_MSG_INFO2_0("Pipe inactivity detected!");
      }
    }
  }
  else
  {
    // If packet count changed -- has traffic
    phys_xport->pipe_pkt_cnt = pipe_pkt_cnt;
    phys_xport->pipe_pkt_cnt_ts = 0;
    if (phys_xport->pipe_inactive)
    {
      phys_xport->pipe_inactive = FALSE;
      LOG_MSG_INFO2_0("Pipe activity detected!");
    }
  }

  return !(phys_xport->pipe_inactive);

} /* rmnet_phys_xport_has_ul_direct_traffic() */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  -------------------------------------------------------------------------*/
  xport_info->next_ptr = phys_xport->xport_list;
  ...
  if (phys_xport != NULL)
  {
    ...
    while (xport_this != NULL)
    {
      if (xport_this == xport_info)
      {
        if (xport_prev == NULL)
        {
          phys_xport->xport_list = xport_this->next_ptr;
        }
        else
        {
          xport_prev->next_ptr = xport_this->next_ptr;
        }
        break;
      }

      xport_prev = xport_this;
      xport_this = xport_this->next_ptr;
    }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  while (logical_xport_ptr != NULL)
  {
    ...
    logical_xport_ptr = logical_xport_ptr->next_ptr;
  }  
...
/*===========================================================================
  FUNCTION RMNET_LOGICAL_XPORT_SET_OFFLOAD_FC()
  ...
  SIDE EFFECTS
    None
===========================================================================*/
void rmnet_logical_xport_set_offload_fc
(
  rmnet_xport_type  * xport_info,
  uint32              fc_mask,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  old_mask = xport_info->offload_fc_mask;

  if (flow_disable)
  {
    // Flow disable
    xport_info->offload_fc_mask |= fc_mask;
    if (old_mask == 0 && xport_info->offload_fc_mask != 0)
    {
      rmnet_shim_set_offload_fc(xport_info, TRUE);
    }
  }
  else
  {
    ...
    if (old_mask != 0 && xport_info->offload_fc_mask == 0)
    {
      rmnet_shim_set_offload_fc(xport_info, FALSE);
    }
  }

} /* rmnet_logical_xport_set_offload_fc() */
...
{
  qsh_mdump_collect_high(&rmnet_phys_xport, RMNET_PHYS_XPORT_MAX * sizeof(rmnet_phys_xport_type *));
}
#endif /* FEATURE_QSH_MDUMP */
