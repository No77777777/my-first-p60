...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/rmifacectls/src/ds_rmnet_v4_sm.c#2 $
...
#define DS_RMNET_V4_SM_ASSERT(expression) \
         ds_rmnet_v4_sm_assert_wrapper(__LINE__, expression)
...
---------------------------------------------------------------------------*/
char *rmnet_v4_smi_state_names[RMNET_V4_SM_MAX_STATE] =
{
  "Null state",
...
FUNCTION DS_RMNET_V4_SM_ASSERT_WRAPPER()

DESCRIPTION
  Wrapper function for DS_RMNET_V4_SM ASSERT
...
===========================================================================*/
static void ds_rmnet_v4_sm_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("DS_RMNET_V4_SM_ERR_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
}/* ds_rmnet_v4_sm_assert_wrapper */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_RMNET_V4_SM_ASSERT(instance < RMNET_INSTANCE_MAX);
  info = rmnet_smi_info[instance];
  DS_RMNET_V4_SM_ASSERT(info != NULL);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_RMNET_V4_SM_ASSERT (0);
    LOG_MSG_ERROR_INT_1 ("Invalid RMNET_V4_SM event [d], ignored", event);
    return;
  }
#if 0
#ifdef T_ARM
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  DS_RMNET_V4_SM_ASSERT (cmd_ptr && (cmd == DCC_RMNET_SM_EV_CMD) && (cmd_ptr->sm_id == RMNET_V4_SM));
  ...
  if(is_err == TRUE)
  {
    LOG_MSG_ERROR_INT_2("error (d) for event (d)",err,event);
    DS_RMNET_V4_SM_ASSERT(0);
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_RMNET_V4_SM_ASSERT (cmd_ptr && (cmd == DCC_RMNET_SM_CMD) && (cmd_ptr->sm_id == RMNET_V4_SM));
...
)
{
  rmnet_smi_info_type *info;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  DS_RMNET_V4_SM_ASSERT((NULL != te_ip_addr) && (NULL != rm_ip_addr) && 
...
{
  ps_iface_event_enum_type rmnet_addr_cfg_ev = IFACE_ADDR_CHANGED_EV;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

  DS_RMNET_V4_SM_ASSERT(info->sub_sm_type == RMNET_V4_SM);
  ...
  switch( info->v4_sm.state )
  {
    case RMNET_V4_SM_NULL_STATE:
    {
      ...
      {
        if (NULL == (info->v4_sm.um_addr_changed_buf_ptr = ps_iface_alloc_event_handle(info->um_iface_ptr,
                                                                                        rmnet_v4_smi_um_iface_ev_hdlr_cb,
                                                                                        (void *)info))) 
        {
          DS_UTILS_ERROR_FATAL("IFACE Address alloc handle failed!");
          return;
        }
        if (0 !=  ps_iface_subscribe_event_list( info->v4_sm.um_addr_changed_buf_ptr,
                                                 &rmnet_addr_cfg_ev,
                                                 sizeof(rmnet_addr_cfg_ev)/sizeof(ps_iface_event_enum_type)))
        {
          LOG_MSG_ERROR_INT_0("IFACE Address change event reg failed!");
          rmnet_meta_sm_post_event( RMNET_V4_SM_INFO_TO_INSTANCE(info), 
                                    RMNET_META_SM_ADDR_CFG_FAILURE_EV );
          return;
        }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(info->sub_sm_type == RMNET_INVALID_SM)
  {
  ...
  switch( info->v4_sm.state )
  {
    ...
    default:  /* all other states */
    {
      LOG_MSG_ERROR_INT_0("Got stop address cfg event in unexpected state");
      break;
    }
  } /* switch( info->v4_sm.state ) */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch( info->v4_sm.state )
  {
    ...
    case RMNET_V4_SM_DHCP_IN_PROGRESS_STATE:
    case RMNET_V4_SM_ADDR_CFG_COMPLETE_STATE:
      LOG_MSG_ERROR_INT_0("Received DHCP started ev in invalid state.");
      break;
    ...
    default:  /* all other states */
    {
      LOG_MSG_ERROR_INT_0("Got DHCP started event in unexpected state");
      break;
    }
  } /* switch( info->v4_sm.state ) */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch( info->v4_sm.state )
  {
    ...
    default:  /* all other states */
    {
      LOG_MSG_ERROR_INT_0("Got DHCP complete event in unexpected state");
      break;
    }
  } /* switch( info->v4_sm.state ) */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_RMNET_V4_SM_ASSERT (user_data_ptr != NULL);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_RMNET_V4_SM_ASSERT (userdata != NULL);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_RMNET_V4_SM_ASSERT(user_data != NULL);
  info = (rmnet_smi_info_type *) user_data;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  DS_RMNET_V4_SM_ASSERT (info && msg_type);

  if(info->v4_sm.inited != TRUE)
  {
    DS_UTILS_ERROR_FATAL("RMNET_V4_SM not inited");
    return;
  }
  ...
  switch( new_state )
  {
    ...
    case RMNET_V4_SM_NULL_STATE:
    {
      ...
      if (info->um_iface_ptr)
      {
        ...
        -------------------------------------------------------------------*/
        ps_iface_free_event_handle(info->v4_sm.um_addr_changed_buf_ptr);
        info->v4_sm.um_addr_changed_buf_ptr = NULL;
      }
    ...
    default:
    {
      LOG_MSG_ERROR_INT_1("Invalid state [d]", new_state);
      DS_RMNET_V4_SM_ASSERT(0);
    }
  } /* switch(new state) */
