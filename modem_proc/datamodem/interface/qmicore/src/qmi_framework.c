...
/*===========================================================================
   ...
   $Header: //components/rel/data.mpss/3.5..2.1/interface/qmicore/src/qmi_framework.c#4 $ 
   $DateTime: 2025/04/09 05:00:54 $ $Author: pwbldsvc $
...
#define QMI_FRAMEWORK_ASSERT(expression) \
         qmi_framework_assert_wrapper(__LINE__, expression)
...
FUNCTION QMI_FRAMEWORK_ASSERT_WRAPPER()

DESCRIPTION
  Wrapper function for QMI_FRAMEWORK ASSERT
...
===========================================================================*/
static void qmi_framework_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("QMI_FRAMEWORK_ERR_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
}/* qmi_framework_assert_wrapper */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (msg_buf == NULL)
  {
    LOG_MSG_ERROR_INT_0 ("Alloc alloc_msg_buf: Can't get memory allocation!");
    ASSERT(msg_buf);
  }
  return msg_buf;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if(msg_buf == NULL)
  {
    LOG_MSG_ERROR_INT_0("NULL pointer passed to qmi_frameworki_dealloc_msg_buf");
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  QMI_FRAMEWORK_ASSERT (msg_hdr && msg_ptr);
  ...
  {
    LOG_MSG_ERROR_INT_1 ("No msg_buffer!  Discarding service [d] message",
                     msg_hdr->common_hdr.service);
    dsm_free_packet(&msg_ptr);
    return FALSE;
  }

  LOG_MSG_INFO3_5 ("Sending svc %d msg to QMUX: "
  ...
  if( rex_self() != rex_qmi_modem_tcb )
  {
    ...
    if (cmd_ptr == NULL)
    {
      LOG_MSG_ERROR_INT_1 ("No command buffer!  Discarding service [d] message",
                       msg_hdr->common_hdr.service);
      dsm_free_packet(&msg_ptr);
      qmi_frameworki_dealloc_msg_buf(&qmi_framework_msg_buf);
      QMI_FRAMEWORK_ASSERT(0);
      return FALSE;
    }
    cmd_ptr->cmd.qmi.id = qmi_cmd; 
    cmd_ptr->cmd.qmi.data.fw.user_data_ptr = (void *)qmi_framework_msg_buf;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  QMI_FRAMEWORK_ASSERT(msg_ptr && msg_buf);
  ...
  if (msg_ptr)
  {
    ...
    {
      LOG_MSG_QTRACE_HFT_MED(PS_TAG_QMI_FW, PS_SYS_DEFAULT_SUBS,
                       "Send QMI service: %d tx_id: %d via QMUX - SIO to client: %d",
                       msg_buf->msg_hdr.common_hdr.service,
                       msg_buf->msg_hdr.common_hdr.transaction_id,
...
/*-----------------------------------------------------------------*/
  if(NULL == msg_hdr || msg_hdr->common_hdr.qmi_instance >= QMI_INSTANCE_MAX || msg_hdr->common_hdr.service >= QMUX_SERVICE_MAX)
  {
    QMI_FRAMEWORK_ASSERT (msg_hdr && msg_hdr->common_hdr.qmi_instance < QMI_INSTANCE_MAX
                   && msg_hdr->common_hdr.service < QMUX_SERVICE_MAX );
    return FALSE;
  }
  ...
  if (svc_info == NULL)
  {
    LOG_MSG_ERROR_INT_1("Unknown service [d]", msg_hdr->common_hdr.service);
    return FALSE;
  }
...
/*-------------------------------------------------------------------------*/
  ...
  {
    LOG_MSG_ERROR_INT_0("Invalid service or common_msg_hdr!");
    return;
  }
  ...
  if (svc_info == NULL)
  {
    LOG_MSG_ERROR_INT_1("Unknown service [d]", common_msg_hdr->service);
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  QMI_FRAMEWORK_ASSERT (sdu_in && qmi_instance < QMI_INSTANCE_MAX && service < QMUX_SERVICE_MAX);
  ...
  {
    LOG_MSG_ERROR_INT_2 ("Got QMI service [d] message for unassigned client [d]! discarding",
                     service, client_id);
  ...
  if (svc_info == NULL)
  {
    LOG_MSG_ERROR_INT_1("Unknown service [d]", service);
    dsm_free_packet (&sdu_in);
    return FALSE;
  }
  ...
  {
    LOG_MSG_ERROR_INT_2 ("Got QMI service [d] message for invaid instance: [d]",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  QMI_FRAMEWORK_ASSERT(user_data_ptr != NULL);
  ...
  QMI_FRAMEWORK_ASSERT(msg_hdr && msg_ptr && msg_hdr->common_hdr.service < QMUX_SERVICE_MAX); 
  ...
  else if((qmi_cmd_id_e_type)qmi_cmd == QMI_CMD_FRAMEWORK_SEND_IND)
  {
    ...
    if (temp == -1)
    {
      LOG_MSG_ERROR_INT_0 ("Couldn't extract the ind_cmd_id %d from msg_ptr");
      goto func_end;
    }
    ...
    {
      LOG_MSG_ERROR_INT_0 ("Couldn't append the ind_cmd_id to msg_ptr");
      goto func_end;
    }
  }
  else
  {
    LOG_MSG_ERROR_INT_1 ("Invalid QMI svc cmd to process cmd [d]", qmi_cmd);
    goto func_end;
  }
  ...
  {
    LOG_MSG_ERROR_INT_1 ("Msg header validation failed - unable to send Response for cmd [d]",
                     msg_hdr->msg_ctl_flag);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  QMI_FRAMEWORK_ASSERT(common_hdr != NULL);
  
  if (common_hdr->service >= QMUX_SERVICE_MAX)
  {
    LOG_MSG_ERROR_INT_1 ("Invalid service %d", common_hdr->service);
  ...
  {
    LOG_MSG_ERROR_INT_1 ("Service [d] not registered with QMI Framework!",
                     common_hdr->service);
    return QMI_ERR_INVALID_SERVICE_TYPE;
  }
  ...
  {
    LOG_MSG_ERROR_INT_0 ("Clid allocation failed due to invalid svc cb");
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (svc_info == NULL)
  {
    LOG_MSG_ERROR_INT_1("Unknown service [d]", common_hdr->service);
    return QMI_ERR_INVALID_SERVICE_TYPE;
  }
  ...
  {
    LOG_MSG_ERROR_INT_0 ("Clid deallocation failed due to invalid svc cb");
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  QMI_FRAMEWORK_ASSERT(instance < QMI_INSTANCE_MAX);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  QMI_FRAMEWORK_ASSERT (service < QMUX_SERVICE_MAX && cfg);
  ...
  -------------------------------------------------------------------------*/
  svc_info->svc_reg_ts = time_get_ms_native();
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  QMI_FRAMEWORK_ASSERT (service < QMUX_SERVICE_MAX);
  ...
  {
    LOG_MSG_ERROR_INT_1 ("Service [d] not registered with QMI Framework!", service);
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  QMI_FRAMEWORK_ASSERT (msg_ptr && msg_hdr);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  QMI_FRAMEWORK_ASSERT (msg_hdr && ind);

  if (msg_hdr->common_hdr.service >= QMUX_SERVICE_MAX)
  {
    LOG_MSG_ERROR_INT_1(" Invalid service [d]",
                    msg_hdr->common_hdr.service);
    return FALSE;
  }
  /*-------------------------------------------------------------------------
  ...
  if ( qmi_framework_svc_state[msg_hdr->common_hdr.service] == NULL || 
       ...
       msg_hdr->common_hdr.client_id >= QMI_FRAMEWORK_SVC_MAX_CLIENTS ) 
  {
    LOG_MSG_ERROR_INT_2(" Invalid service [d] or cl id [d] ",
                    msg_hdr->common_hdr.service, 
                    msg_hdr->common_hdr.client_id);
  }
  else
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (msg_hdr == NULL)
  {
    ...
    QMI_FRAMEWORK_ASSERT(msg_hdr != NULL);
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  QMI_FRAMEWORK_ASSERT(common_msg_hdr != NULL);
...
void qmi_framework_get_svc_info_per_clid
(
  uint8      svc_type,
  uint8      clid,
  qmi_svc_info_type * qmi_svc_info
)
{
  qmux_service_e_type            service;
  qmi_framework_svc_info_type  * svc_info;
/*-------------------------------------------------------------------------*/
  service = (qmux_service_e_type)svc_type;
  svc_info = qmi_framework_svc_state[service];

  if (svc_info != NULL &&
      svc_info->cfg.cbs.get_svc_info != NULL)
  {
    svc_info->cfg.cbs.get_svc_info(clid, qmi_svc_info);
  }
  
  return;
} /* qmi_framework_get_svc_info_per_clid() */
...
{
  qsh_mdump_collect_high(&qmi_framework_svc_state, 
                   QMUX_SERVICE_MAX * sizeof(qmi_framework_svc_info_type *));
}
#endif /* FEATURE_QSH_MDUMP */