...
/*===========================================================================
   ...
   $Header: //components/rel/data.mpss/3.5..2.1/interface/qmicore/src/qmi_sap_xport_qmux.c#3 $ 
...
#define QMI_SAP_XPORT_QMUX_ASSERT(expression) \
         qmi_sap_xport_qmux_assert_wrapper(__LINE__, expression)
...
FUNCTION QMI_SAP_XPORT_QMUX_ASSERT_WRAPPER()

DESCRIPTION
  Wrapper function for QMI_SAP_XPORT_QMUX ASSERT
...
===========================================================================*/
static void qmi_sap_xport_qmux_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("QMI_SAP_XPORT_QMUX_ERR_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
}/* qmi_sap_xport_qmux_assert_wrapper */
...
)
{
  ...
  *clid = xh->next_clid;
  while (qmi_qsapi_is_valid_clid_cb(sp, *clid))
  {
   ...
   Added a clientCount variable to fix the infinite loop if all the bits of the xh->active_clients
    are set
   */
	clientCount++;
	if(clientCount == QMI_SVC_CLID_BROADCAST)
	{
	  LOG_MSG_ERROR_0("Max client limit reached returning Unused client");
	 *clid = QMI_SVC_CLID_UNUSED;
	 return ;
	}
    /*-----------------------------------------------------------------------
...
)
{
  ...
  if(xh)
  {
    ...
    xh->qmux_instance = (qmi_instance_e_type)(unsigned int)xport_data;
    xh->next_clid = 1;
  }
  return xh;
}/* qsap_xport_open */
 ...
 )
{
  ...
  xh->cfg.base_version.major = (version & 0xFFFF);
 ...
 )
{
  ...
  if(dsm_pushdown_packed(&msg, buf, len, DSM_DS_SMALL_ITEM_POOL) != len)
  {
    QMI_SAP_XPORT_QMUX_ASSERT(0);
    dsm_free_packet(&msg);
    return QMI_SAP_INTERNAL_ERR;
