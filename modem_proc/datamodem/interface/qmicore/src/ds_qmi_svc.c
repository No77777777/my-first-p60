...
Copyright (c) 2005-2016,2018 Qualcomm Technologies Incorporated.
All Rights Reserved.
...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/qmicore/src/ds_qmi_svc.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
#define DS_QMI_SVC_ASSERT(expression) \
         ds_qmi_svc_assert_wrapper(__LINE__, expression)
...
typedef enum
{
  QMI_SVC_LOCAL_ERR_NONE = 0,
  QMI_SVC_LOCAL_ERR_NO_CMD_BUF,
  QMI_SVC_LOCAL_ERR_TOO_MANY_CMDS,
  QMI_SVC_LOCAL_ERR_INVALID_CMD_ID,
  QMI_SVC_LOCAL_ERR_CMD_BUF_NULL,
  QMI_SVC_LOCAL_ERR_CMD_BUF_TX_PTR_NULL,
  QMI_SVC_LOCAL_ERR_GLOBAL_TX_FREE,
  QMI_SVC_LOCAL_ERR_IGNORE_TX_PTR,
  QMI_SVC_LOCAL_ERR_UNHANDLED_CMD,
  QMI_SVC_LOCAL_ERR_INVALID_CMD_SVC_TYPE,
  QMI_SVC_LOCAL_ERR_INVALID_CMD_TYPE,
  QMI_SVC_LOCAL_ERR_INVALID_INSTANCE,
  QMI_SVC_LOCAL_ERR_INVALID_SVC,
  QMI_SVC_LOCAL_ERR_CLIENT_RELEASED,
  QMI_SVC_LOCAL_ERR_NULL_TXN_BUFFER,
  QMI_SVC_LOCAL_ERR_PREPREND_HDR_FAIL,
  QMI_SVC_LOCAL_ERR_SVC_SEND_FAIL,
  QMI_SVC_LOCAL_ERR_NO_SPEC_SLOT_AVAIL,
  QMI_SVC_LOCAL_ERR_INVALID_QMI_INST,
  QMI_SVC_LOCAL_ERR_NULL_IND_LIST,
  QMI_SVC_LOCAL_ERR_NULL_SVC_HNDL,
  QMI_SVC_LOCAL_ERR_NORMAL_STATE_SPEC,
  QMI_SVC_LOCAL_ERR_UNINITIALIZED_INST,
  QMI_SVC_LOCAL_ERR_SET_POWER_STATE_FAIL,
  QMI_SVC_LOCAL_ERR_NOTIFY_POWER_STATE_FAIL,
  QMI_SVC_LOCAL_ERR_NO_POWER_STATE_CHANGE,
  QMI_SVC_LOCAL_ERR_NORMAL_PWRSAVE_STATE
}qmi_svci_local_err_type;
...
FUNCTION DS_QMI_SVC_ASSERT_WRAPPER()

DESCRIPTION
  Wrapper function for DS_QMI_SVC ASSERT
...
===========================================================================*/
static void ds_qmi_svc_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("DS_QMI_SVC_ERR_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
}/* ds_qmi_svc_assert_wrapper */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_QMI_SVC_ASSERT (response != NULL);
...
)
{
  ...
  q_type *               q_ptr;
  qmi_svci_local_err_type local_err;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

  DS_QMI_SVC_ASSERT (x_p != NULL);
  local_err = QMI_SVC_LOCAL_ERR_NONE;

  do
  {
    if (x_p->n_cmds >= MAX_MSGS_IN_BUNDLE)
    {
      local_err = QMI_SVC_LOCAL_ERR_TOO_MANY_CMDS;
      break;
    }
    ...
    if (x_p->cl_sp)  // cl_sp != NULL means this transaction is client specific
    {
      ...
      if (NULL == cmd_buf_p)
      {
        local_err = QMI_SVC_LOCAL_ERR_NO_CMD_BUF;
        break;
      }
    }
  ...
  if (QMI_SVC_LOCAL_ERR_NONE == local_err)
  {
    return cmd_buf_p;
  }
  else
  {
    LOG_MSG_INFO1_1 ("QMI SVC New Buf: Local Error %d", local_err);
    return NULL;
  }
} /* qmi_svc_new_cmd_buf() */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_QMI_SVC_ASSERT (cmd_buf_handle != NULL);
  ...
  *cmd_buf_handle = NULL; // free the callers reference immediately
  ...
  do 
  {
    if (freeing_cmd_buf_p)
    {
      ...
      {
        local_err = QMI_SVC_LOCAL_ERR_GLOBAL_TX_FREE;
      }
    }
    else
    {
  ...
  LOG_MSG_INFO3_2 ("Freeing QMI command buffer %x,"
                   "local_err %d", freeing_cmd_buf_p, local_err);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  else /* if no client sp, must be a broadcast indication */
  {
    ...
    if(svc_sp->bcast_transaction_buf_free == FALSE)
    {
      ...
      ---------------------------------------------------------------------*/
      DS_UTILS_ERROR_FATAL("Broadcast indication in use!");
      return NULL;
    }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  do
  {
    ...
    if (svc_sp == NULL)
    {
      local_err = QMI_SVC_LOCAL_ERR_IGNORE_TX_PTR;
      break;
    }
  ...
  if (QMI_SVC_LOCAL_ERR_NONE != local_err)
  {
    LOG_MSG_INFO2_1 ("Free Transaction exit with local_err %d ", 
                     local_err);
    return;
  }
  ...
  if (free_x)
  {
    ...
    if (cl_sp)
    {
      DS_QMI_SVC_ASSERT (*x_p != &svc_sp->bcast_transaction);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_QMI_SVC_ASSERT (svc_sp != NULL && cl_sp != NULL);
  svc_handle = (qmux_svc_info_type *)svc_sp->qmux_svc_handle;

  DS_QMI_SVC_ASSERT (svc_handle->cfg.cbs.reset_client != NULL);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_QMI_SVC_ASSERT (sp && qmux_svc_handle);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_QMI_SVC_ASSERT (sp && client_sp);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_QMI_SVC_ASSERT (cl_sp_in != NULL);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_QMI_SVC_ASSERT (svc_sp && cmd_buf_p && sdu_in);
  svc_handle = (qmux_svc_info_type *)svc_sp->qmux_svc_handle;
  DS_QMI_SVC_ASSERT (svc_handle != NULL);
  ...
  do
  {
    ...
    {
      local_err = QMI_SVC_LOCAL_ERR_INVALID_CMD_SVC_TYPE;
    ...
    switch (cmd_buf_p->x_p->ctl & QMI_FLAG_MASK_MSGTYPE)
    {
      ...
      case QMI_FLAG_MSGTYPE_IND:
        ...
        break;
      default:
        local_err = QMI_SVC_LOCAL_ERR_INVALID_CMD_TYPE;
        response_ptr = NULL;
        goto send_result;
    }
    ...
    {
      local_err = QMI_SVC_LOCAL_ERR_UNHANDLED_CMD;
      response_ptr = NULL;
    }
...
send_result:
  LOG_MSG_INFO3_3 ( "Got QMI Service cmd type %d with local_err %d, "
                    "response 0x%p",cmd_buf_p->cmd_type, 
                     local_err, response_ptr);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_QMI_SVC_ASSERT(svc_sp && sdu_in);
  ...
  while (sdu_in)
  {
    ...
    else if (sdu_len < remaining_bytes)
    {
      ...
      ---------------------------------------------------------------------*/
      DS_QMI_SVC_ASSERT (next_sdu == NULL);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_QMI_SVC_ASSERT (cmd_buf_p && (*cmd_buf_p) && (*cmd_buf_p)->x_p && msg);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_QMI_SVC_ASSERT( QMI_INSTANCE_MAX > qmi_instance && QMUX_SERVICE_MAX > svc_id);
  ...
  do
  {
    ...
    {
      local_err = QMI_SVC_LOCAL_ERR_INVALID_INSTANCE;
      break;
    }
    ...
    {
      local_err = QMI_SVC_LOCAL_ERR_INVALID_SVC;
      break;
    }
  ...
  if (QMI_SVC_LOCAL_ERR_NONE != local_err)
  {
    LOG_MSG_ERROR_1("QMI Filter message exit with local error %d",
                      local_err);
    return FALSE;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_QMI_SVC_ASSERT (svc_sp && cmd_buf_p && msg_ptr);
  ...
  if( FALSE == qmi_svc_queue_response( &cmd_buf_p, &msg_ptr ) )
  {
    ...
    DS_QMI_SVC_ASSERT (msg_ptr == NULL); // msg_ptr is freed in queue_response
    return FALSE;
  }

  DS_QMI_SVC_ASSERT (cmd_buf_p == NULL && msg_ptr == NULL);
  ...
  for( i = 0; i < x_p->n_cmds; i++ )
  {
    DS_QMI_SVC_ASSERT( x_p->resp_list[i] != QMI_SVC_RESPONSE_PENDING );
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_QMI_SVC_ASSERT (sp && cmd_buf_p_in && msg_ptr);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  do 
  {
    ...
    {
      local_err = QMI_SVC_LOCAL_ERR_CLIENT_RELEASED;
      dsm_free_packet(&ind);
      break;
    }
    ...
    if (x_p == NULL)
    {
      local_err = QMI_SVC_LOCAL_ERR_NULL_TXN_BUFFER;
      dsm_free_packet(&ind);
      break;
    }
    ...
    {
      local_err = QMI_SVC_LOCAL_ERR_PREPREND_HDR_FAIL;
      /*-----------------------------------------------------------------------
        Note - no need to free msg_ptr, taken care of in 
               qmi_svc_prepend_msg_hdr()
      -----------------------------------------------------------------------*/
    ...
    {
      local_err = QMI_SVC_LOCAL_ERR_SVC_SEND_FAIL;
      break;
    } 
    else
    {
      LOG_MSG_INFO2_3 ("Sent the indication (for cmd=%d)! and cl_sp seq no %d",
                   cmd_type, cl_sp?cl_sp->seq_no:0,0);
    }
  ...
  if (QMI_SVC_LOCAL_ERR_NONE != local_err)
  {
    LOG_MSG_INFO1_2 ("Indication for Client %d exit with local_err %d ",
                      clid, local_err);
    return FALSE;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_QMI_SVC_ASSERT(svci && sdu_out);
...
/*-------------------------------------------------------------------------*/
  ...
  DS_QMI_SVC_ASSERT(common_msg_hdr && ((qmi_cmd == QMI_CMD_GET_CLIENT_ID_RESP) ||
         (qmi_cmd == QMI_CMD_RELEASE_CLIENT_ID_RESP)));  
  
  cmd_ptr = qmi_get_cmd_data_buf();
  if (cmd_ptr == NULL)
  {
    LOG_MSG_ERROR_INT_2 ("No cmd buffer! service [d] client ID:[d] response message ",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  DS_QMI_SVC_ASSERT(common_msg_hdr != NULL);
...
)
{
  ...
  if(NULL == svc_cfg )
  {
    LOG_MSG_ERROR_INT_0("Invalid service handle");
...
{ 
  ...
  if (svc_cfg->cbs.dealloc_clid)
  {
    ...
    {
      LOG_MSG_INFO2_0("qmi_svc_dealloc_clid() failed after call "
                      "to service dealloc_clid callback!");
    }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  *out_spec_ptr = NULL;
  ...
  {
    LOG_MSG_ERROR_INT_1("Invalid qmi inst [d]", qmi_inst);
    return FALSE;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  *out_spec_ptr = NULL;
  ...
  if (qmi_inst >= QMI_INSTANCE_MAX)
  {
    local_err = QMI_SVC_LOCAL_ERR_INVALID_QMI_INST;
    goto bail;
  }
  ...
  local_err = QMI_SVC_LOCAL_ERR_NO_SPEC_SLOT_AVAIL;
bail:
  LOG_MSG_INFO2_2("qmi_svc_pwrsave_find_empty_spec qmi_inst %d, "
                  "exit with local_err %d", qmi_inst, local_err);
  return FALSE;
...
)
{
  ...
  uint8                       j;
  qmi_svci_local_err_type     local_err = QMI_SVC_LOCAL_ERR_NONE;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

  DS_QMI_SVC_ASSERT( service_handle && ind_list);
  ...
  do
  {
    ...
    {
      local_err = QMI_SVC_LOCAL_ERR_NULL_SVC_HNDL;
      break;
    }
    ...
    if (ind_list == NULL)
    {
      local_err = QMI_SVC_LOCAL_ERR_NULL_IND_LIST;
      break;
    }

    svc_s = (qmux_svc_info_type *)service_handle;
    qmi_instance = qmi_instance_by_qmux_handle(svc_s);
    DS_QMI_SVC_ASSERT(qmi_instance < QMI_INSTANCE_MAX);
    ...
    {
      local_err = QMI_SVC_LOCAL_ERR_NORMAL_STATE_SPEC;
      break;
    }
  ...
  LOG_MSG_INFO3_1("Exit qmi_svc_pwrsave_store_spec with local_err %d", local_err);
  if(QMI_SVC_LOCAL_ERR_NONE != local_err)
  {
    return FALSE;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_QMI_SVC_ASSERT( old_state_handle != NULL);
...
bail:
  LOG_MSG_INFO3_1("qmi_set_pwrsave_state_by_instance with local_err %d", local_err);
  if(QMI_SVC_LOCAL_ERR_NONE != local_err)
  {
    return FALSE;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_QMI_SVC_ASSERT( service_handle && old_state_handle_ptr);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_QMI_SVC_ASSERT( service_handle && state_handle_ptr);
  ...
  if (qmi_instance >= QMI_INSTANCE_MAX)
  {
    LOG_MSG_ERROR_INT_1("Instance specified not valid: d", qmi_instance);
    return FALSE;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  do
  {
    ...
    {
      local_err = QMI_SVC_LOCAL_ERR_UNINITIALIZED_INST;
      break;
    }
    ...
    if (!qmi_set_pwrsave_state_by_instance(instance,
                                           ...
                                           &old_state_handle))
    {
      local_err = QMI_SVC_LOCAL_ERR_SET_POWER_STATE_FAIL;
    }
    else
    {
      ...
      if (old_state_handle != state_handle)
      {
        if (!qmi_ctl_pwrsave_mode_change_notify(instance,
                                                ...
                                                old_state_handle))
        {
          local_err = QMI_SVC_LOCAL_ERR_NOTIFY_POWER_STATE_FAIL;
        }
      } 
      else
      {
        local_err = QMI_SVC_LOCAL_ERR_NO_POWER_STATE_CHANGE;
      }
    }
  }
  while (0); 
  ...
  LOG_MSG_INFO2_1("power save evt hdlr exit with local_err %d",
                  local_err);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if( IS_PWRSAVE_STATE_VALID( new_state ) )
  {
    ...
    {
      LOG_MSG_ERROR_INT_1( "Could not get cmd buffer from QMI task for cmd id [d]",
...
)
{
  ...
  if (NULL == spc_code)
  {
    LOG_MSG_ERROR_INT_0( "Unable to function on NULL SPC code" );
    return retval;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (service_handle == NULL)
  {
    DS_UTILS_ERROR_FATAL("QMI_SVC service_handle pointer is NULL!");
    return FALSE;
  }
  ...
  if (state_handle == QMI_SVC_PWRSAVE_STATE_NORMAL)
  {
    local_err = QMI_SVC_LOCAL_ERR_NORMAL_PWRSAVE_STATE;
    goto bail;
  }
...
bail:
  if (QMI_SVC_LOCAL_ERR_NONE != local_err)
  {
    LOG_MSG_INFO1_1("PwrSave store exiting with local_err %d", local_err);
    return FALSE;
  }
  return TRUE;
...
/*-------------------------------------------------------------------------*/
  ...
  {
    DS_UTILS_ERROR_FATAL("QMI_SVC service_handle pointer NULL!");
    return FALSE;
  }
...
/*--------------------------------------------------------------------------*/
   ...
   {
     // We reutrn QMI_ERR_NONE_V01 if msg_ptr is NULL. 
   ...
   do
   {
     ...
     retval = qmi_idl_message_decode(
                      ...
                      p_req_size         // size of request message
                      );
   ...
   {
     LOG_MSG_ERROR_3("Decode failed: msg_id (%d) retval (%d) errval (%d)",
            message_id, retval, errval);
   }
...
/*--------------------------------------------------------------------------*/
  ...
  do
  {
    ...
    /*----------------------------------------------------------------------
      Accessor function for getting the maximum message length
      of a response message.
    -----------------------------------------------------------------------*/
    retval = qmi_idl_get_max_message_len(svc, type, message_id, &rsp_buf_len);
    ...
    *p_response = NULL;
    if (size_buf != dsm_pushdown_packed(p_response,
                       &rsp_buf[0], size_buf, DSM_DS_SMALL_ITEM_POOL) )
    {
  ...
  if (errval != QMI_ERR_NONE_V01)
  {
    LOG_MSG_ERROR_3("Encode failed: msg_id (%d) retval (%d) errval (%d)",
               message_id, retval, errval);
    return FALSE;
  }
