...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/qmicore/src/qmi_cmd_process.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
===========================================================================*/
#define QMI_CMD_PROCESS_ASSERT(expression) \
         qmi_cmd_process_assert_wrapper(__LINE__, expression)

/*===========================================================================

FUNCTION QMI_CMD_PROCESS_ASSERT_WRAPPER()

DESCRIPTION
  Wrapper function for QMI_CMD_PROCESS ASSERT
...
===========================================================================*/
static void qmi_cmd_process_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("QMI_CMD_PROCESS_ERR_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
}/* qmi_cmd_process_assert_wrapper */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  QMI_CMD_PROCESS_ASSERT(svci && sdu_in);
...
{
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  QMI_CMD_PROCESS_ASSERT(svci && qmux_pdu);
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  QMI_CMD_PROCESS_ASSERT (cmd_ptr && cmd == QMI_CMD && cmd_ptr->cmd.qmi.id < QMI_CMD_MAX);
