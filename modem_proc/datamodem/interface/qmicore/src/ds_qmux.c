...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/qmicore/src/ds_qmux.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
#define DS_QMUX_ASSERT(expression) \
         ds_qmux_assert_wrapper(__LINE__, expression)
...
FUNCTION DS_QMUX_ASSERT_WRAPPER()

DESCRIPTION
  Wrapper function for DS_QMUX ASSERT
...
===========================================================================*/
static void ds_qmux_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("DS_QMUX_ERR_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
}/* ds_qmux_assert_wrapper */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR_INT_0("Failed to initialize QMI EFS NV conf file");
  }
  ...
  if(NULL == dcc_cmd_ptr)
  {
    DS_UTILS_ERROR_FATAL("QMI can't get modem memory heap allocation");
    return FALSE;
  }
  ...
  {
    LOG_MSG_ERROR_INT_0("Couldn't initialize QMI_PWRMGMT_SHIM module!");
  }
#endif /* FEATURE_DATA_QMI_APPS_POWER_COLLAPSE */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR_INT_1("Service ID [d] out of range!", service);
    return NULL;
  }
  ...
  {
    LOG_MSG_ERROR_INT_1 ("QMI instance out of range (d)!", qmi_instance);
    return NULL;
  }
  ...
  if (cfg == NULL)
  {
    LOG_MSG_ERROR_INT_0("QMI service cfg is NULL!");
    return NULL;
  }

  if( !qmux_state[qmi_instance].initialized )
  {
    LOG_MSG_ERROR_INT_1 ("QMI link [d] not initialized!", qmi_instance);
    return NULL;
  }
  ...
  {
    LOG_MSG_ERROR_INT_2 ("Service [d] already registered with QMUX#[d]!",
  ...
  if ( ((cfg->cbs.recv == NULL) && 
       ...
       ((cfg->cbs.recv != NULL) && 
        ((cfg->cmd_hdlr_array != NULL) || (cfg->cmd_num_entries != 0))) )*/
  {
    LOG_MSG_ERROR_INT_0 ("Invalid QMI service config");
    return NULL;
  }
  ...
  -------------------------------------------------------------------------*/
  svc_s->svc_reg_ts = time_get_ms_native();
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_QMUX_ASSERT (service_handle != NULL);

  /* check that service_handle is not NULL in case ASSERT is removed */
  if (service_handle == NULL)
  {
    LOG_MSG_ERROR_INT_0 ("Deregistering NULL service handle!");
    return;
  }
  ...
  DS_QMUX_ASSERT (svc_s->registered);
...
/*-------------------------------------------------------------------------*/
  ...
  {
    LOG_MSG_ERROR_INT_1("Unknown service [d]", service_id);
    DS_QMUX_ASSERT(0);
    return;
  }
  ...
  if (cmd_ptr == NULL)
  {
    LOG_MSG_ERROR_INT_1 ("No cmd buffer! service [d] response message ",
                     service_id);
    DS_QMUX_ASSERT(0);
    return;
  }
  ...
  -------------------------------------------------------------------------*/
  LOG_MSG_QTRACE_HFT_MED(PS_TAG_QMICORE, PS_SYS_DEFAULT_SUBS,
                 "Posting cmd QMI_CMD_CTL_SVC_AVAILABLE to QMI Modem task"
                 "for service %d qmi_instance %d",
                  service_id, qmi_instance);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_QMUX_ASSERT( qmux_sdu_pptr != NULL);
  ...
  {
    DS_UTILS_ERROR_FATAL("Error calculating FCS value");
    return FALSE;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_QMUX_ASSERT( qmux_sdu_pptr && (*qmux_sdu_pptr) && pending_sdu_pptr);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_QMUX_ASSERT (qmi_instance < QMI_INSTANCE_MAX);

  /* check that qmi_instance is within range in case ASSERT is removed */
  if (qmi_instance >= QMI_INSTANCE_MAX)
  {
    LOG_MSG_ERROR_INT_1("QMI instance out of range (d)!  Discarding",
                    qmi_instance);
  ...
  qmux_state[qmi_instance].tx_pkt_cnt++;
  QMI_LEAVE_CRIT_SECTION(&qmi_port_cfg_crit_section);
} /* qmux_sio_send */
...
)
{
   ...
   DS_QMUX_ASSERT(service_handle && qmux_sdu);

  /* check that service_handle is not NULL in case ASSERT is removed */
  if (service_handle == NULL)
  {
    LOG_MSG_ERROR_INT_0 ("QMUX send with NULL service handle!");
    dsm_free_packet (&qmux_sdu);
    return;
  }
  ...
  if (qmux_sdu == NULL)
  {
    LOG_MSG_ERROR_INT_0("QMUX SDU NULL!");
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_QMUX_ASSERT(instance < QMI_INSTANCE_MAX);

  /* check that instance is within range in case ASSERT is removed */
  if (instance >= QMI_INSTANCE_MAX)
  {
    LOG_MSG_ERROR_INT_1("QMI instance out of range (d)!  Discarding", instance);
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(cmd_ptr == NULL)
  {
    LOG_MSG_ERROR_INT_0("Out of QMUX cmd buf");
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_QMUX_ASSERT (qmux_handle != NULL);

  /* check that qmux_handle is not NULL in case ASSERT is removed */
  if (qmux_handle == NULL)
  {
    LOG_MSG_ERROR_INT_0("QMUX handle NULL!");
    return NULL;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR_INT_1 ("QMI instance [d] out of range", qmi_inst);
    return SIO_PORT_NULL;
  }

  if (FALSE == qmux_state[qmi_inst].initialized)
  {
    LOG_MSG_ERROR_INT_1 ("QMUX instance [d] not initialized", qmi_inst);
    return SIO_PORT_NULL;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_QMUX_ASSERT( (qmi_inst >= QMI_INSTANCE_MIN) && (qmi_inst < QMI_INSTANCE_MAX) );
  qmux_s = &qmux_state[qmi_inst];

  if ( (qmux_s->io.sio_handle == 0 ) ||
       (qmux_s->io.sio_handle == SIO_NO_STREAM_ID) )
  {
    LOG_MSG_ERROR_INT_1("QMI inst [d] port not initialized", qmi_inst);
    return FALSE;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_QMUX_ASSERT (qmux_s && qmux_pdu);
  ...
  if (qmux_s == NULL)
  {
    LOG_MSG_ERROR_INT_0("QMUX state NULL!");
    return FALSE;
  }
  ...
  if (qmux_pdu == NULL)
  {
    LOG_MSG_ERROR_INT_0("QMUX PDU NULL!");
    return FALSE;
  }
  ...
  {
    LOG_MSG_ERROR_INT_2 ("Invalid control frame len (d) expected (d)!",
  ...
  {
    LOG_MSG_ERROR_INT_0 ("QMI service message not handled");
    return FALSE;
  }
  ...
  {
    LOG_MSG_ERROR_INT_1 ("Invalid QMUX type (d)!", qmux_hdr.svc_type);
    return FALSE;
  }
  ...
  if((qmux_hdr.svc_type == QMI_SIM_INTERNAL) || (qmux_hdr.svc_type == QMI_STX))
  {
   ...
   LOG_MSG_INFO2( "Received QMUX PDU (%d bytes) on QMI link %d and svc type %d",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  else
  {
	...
	break;
      ...
      default:
      {
	LOG_MSG_ERROR_INT_1 ("Invalid control frame type (d)!", iftype);
	break;
      }
    }
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_QMUX_ASSERT (qmux_s->io.sio_handle != SIO_NO_STREAM_ID);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR_INT_1("Could not get cmd buffer from QMI modem task for cmd id [d]",
                    cmd );
  }
...
{
  DS_QMUX_ASSERT ( (qmux_s != NULL) && (ioctl_param != NULL) );
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_QMUX_ASSERT( (inst >= QMI_INSTANCE_MIN) && (inst < QMI_INSTANCE_MAX) );
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR_INT_0("Can't allocate QMI cmd buf");
    return;
  }
...
)
{
  switch (cmd_id)
  {
    ...
    default:
      LOG_MSG_ERROR_INT_1("Unknown QMI IF command [d]", (int32)cmd_id);
  }
  return NULL;
...
(
  ...
  rmnet_phys_xport_buffer_info_type *phys_xport_buffer_info
)
{
...
/*-------------------------------------------------------------------------*/
  ...
  if (phys_xport_buffer_info)
  {
    request->phys_xport_buffer_info  = 
      *phys_xport_buffer_info;
  }
  request->next_ptr               = NULL;
  ...
  while (elem_ptr->next_ptr != NULL)
  {
    elem_ptr = elem_ptr->next_ptr;
  }

  elem_ptr->next_ptr = request;
  return TRUE;
...
/*-------------------------------------------------------------------------*/
  ...
  {
    elem_ptr = qmi_port_cfg_queue;
    qmi_port_cfg_queue = elem_ptr->next_ptr;
      ...
      if (elem_ptr->status == QMI_PORT_TO_OPEN)
      {
        qmux_add_data_port(elem_ptr->sio_port, elem_ptr->ep_id,
               elem_ptr->consumer_pipe_num, elem_ptr->producer_pipe_num,
                           &elem_ptr->phys_xport_buffer_info);
      }
      else
      {
...
/*-------------------------------------------------------------------------*/
  ...
  {
    LOG_MSG_ERROR_INT_0("Invalid SIO port");
    return FALSE;
  }

  if (qmuxi_queue_port_request(
         TRUE,  QMI_PORT_TO_OPEN, sio_port, default_ep_id, 0, 0, NULL) == TRUE)
  {
  ...
  for (i = QMI_INSTANCE_MIN; i < QMI_INSTANCE_MAX; i++)
  {
    ...
    if (qmux_state[i].initialized && !qmux_state[i].io.ports_active)
    {
      ...
      if (result)
      {
        qmux_state[i].port_open_ts = time_get_ms_native();
...
/*-------------------------------------------------------------------------*/
  ...
  if (qmuxi_queue_port_request(
         TRUE,  QMI_PORT_TO_CLOSE, sio_port, 0, 0, 0, NULL) == TRUE)
  {
...
{
  boolean  result = FALSE;
  rmnet_phys_xport_buffer_info_type local_phys_xport_buffer_info;
/*-------------------------------------------------------------------------*/

  if (ep_id == 0)
  {
    LOG_MSG_ERROR_INT_0("Invalid ep_id");
    return FALSE;
  }
  if (!phys_xport_buffer_info)
  {
    LOG_MSG_ERROR_INT_0("Invalid Data agg pointer");
    return FALSE;
  }

  local_phys_xport_buffer_info = *phys_xport_buffer_info;

  if (qmuxi_queue_port_request(
            FALSE,  QMI_PORT_TO_OPEN, sio_port,
            ep_id, consumer_pipe_num, producer_pipe_num,
            &local_phys_xport_buffer_info) == TRUE)
  {
...
/*-------------------------------------------------------------------------*/
  ...
  if (qmuxi_queue_port_request(
           FALSE,  QMI_PORT_TO_CLOSE, 0, ep_id, 0, 0, NULL) == TRUE)
  {
...
boolean qmuxi_is_service_registered
(
  qmi_instance_e_type     qmi_inst,
  uint8                   svc_type
)
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  svci = qmux_state[qmi_inst].service[svc_type];
  ...
  return TRUE;
} /* qmuxi_is_service_registered() */


void qmuxi_get_svc_info_per_clid
(
  qmi_instance_e_type     qmi_inst,
  uint8                   svc_type,
  uint8                   clid,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  else
  {
    svci = qmux_state[qmi_inst].service[svc_type];
    if (svci != NULL &&
        svci->cfg.cbs.get_svc_info != NULL)
    {
      svci->cfg.cbs.get_svc_info(clid, svc_info);
    }
  }
  return;
} /* qmuxi_get_subscription_per_clid() */
...
{
  qsh_mdump_collect_high(&qmux_state, QMI_INSTANCE_MAX * sizeof(qmux_state_type));
}
#endif /* FEATURE_QSH_MDUMP */
