...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/qmicore/src/ds_qmux_logging.c#2 $
...
#define QMUX_HDR_SVC_TYPE_OFFSET 4
#define QMUX_HDR_CLID_OFFSET 5

/*===========================================================================
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /*-----------------------------------------------------------------------
    Peek in to the qmux header to retrieve service type
   -----------------------------------------------------------------------*/
  (void) dsm_extract(item_ptr, QMUX_HDR_SVC_TYPE_OFFSET, &svc_type, 1);
  memset(&svc_info, 0, sizeof(svc_info));
  svc_info.subscription_id = subscription_id;

  if (svc_type < QMUX_SERVICE_MAX)
  {
    /*-----------------------------------------------------------------------
      Peek in to the qmux header to retrieve client id
     -----------------------------------------------------------------------*/
    (void) dsm_extract(item_ptr, QMUX_HDR_CLID_OFFSET, &clid, 1);
    if (qmuxi_is_service_registered(qmi_instance, svc_type))
    {
      qmuxi_get_svc_info_per_clid(qmi_instance, svc_type, clid, &svc_info);
    } 
    else 
    {
      qmi_framework_get_svc_info_per_clid(svc_type, clid, &svc_info);
    } 
  }

   subscription_id = svc_info.subscription_id;
  /*-------------------------------------------------------------------------
  ...
  -------------------------------------------------------------------------*/
  offset = 0;
  log_ptr = (uint8*)log_alloc_ext (log_code, len + sizeof(log_hdr_type), 1,&subscription_id);
  if(log_ptr == NULL) 
  {
    /*-----------------------------------------------------------------------
