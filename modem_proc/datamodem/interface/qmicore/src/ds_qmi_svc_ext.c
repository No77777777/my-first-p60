...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/qmicore/src/ds_qmi_svc_ext.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
#define DS_QMI_SVC_EXT_ASSERT(expression) \
         ds_qmi_svc_ext_assert_wrapper(__LINE__, expression)
...
FUNCTION DS_QMI_SVC_EXT_ASSERT_WRAPPER()

DESCRIPTION
  Wrapper function for DS_QMI_SVC_EXT ASSERT
...
===========================================================================*/
static void ds_qmi_svc_ext_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("DS_QMI_SVC_EXT_ERR_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
}/* ds_qmi_svc_ext_assert_wrapper */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  DS_QMI_SVC_EXT_ASSERT (rx_rate && tx_rate);
...
{
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  DS_QMI_SVC_EXT_ASSERT (rx_slots && tx_slots);
