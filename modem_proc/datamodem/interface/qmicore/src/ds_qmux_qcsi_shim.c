...
/*===========================================================================
   ...
   $Header: //components/rel/data.mpss/3.5..2.1/interface/qmicore/src/ds_qmux_qcsi_shim.c#3 $ 
   $DateTime: 2021/02/12 10:11:56 $ $Author: pwbldsvc $
...
===========================================================================*/
#define DS_QMUX_QCSI_SHIM_ASSERT(expression) \
         ds_qmux_qcsi_shim_assert_wrapper(__LINE__, expression)
...
FUNCTION DS_QMUX_QCSI_SHIM_ASSERT_WRAPPER()

DESCRIPTION
  Wrapper function for DS_QMUX_QCSI_SHIM ASSERT
...
===========================================================================*/
static void ds_qmux_qcsi_shim_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("DS_QMUX_QCSI_SHIM_ERR_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
}/* ds_qmux_qcsi_shim_assert_wrapper */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  return ( (svc_info->service_id == 
                          (qmux_service_e_type)(unsigned int)compare_val) ? 1 : 0 );
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  return ( (clnt_info->qmux_client_id == 
                          (uint8)(unsigned int)compare_val) ? 1 : 0 );
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  DS_QMUX_QCSI_SHIM_ASSERT (cmd_ptr && (cmd == QMI_QMUX_QCSI_SHIM_CMD) && (cmd_ptr->cmd_type < QMUX_QCSI_SHIM_CMD_MAX));
...
)
{
  ...
  DS_QMUX_QCSI_SHIM_ASSERT (service < QMUX_SERVICE_MAX);
  ...
  do
  {
    ...
    /*-------------------------------------------------------------------------
    Set the Max number of indications from 30(default) to 100 which are allowed 
    to be in flight for PBM service as the Client is not able to process the PBM
    indications with the rate at which PBM is sending the indications resulting 
    in dropping the indications. Increasing the queue size will allow to hold 
    more number of PBM indications at QCSI/QCCI level. 
    -------------------------------------------------------------------------*/
    if(( service == QMUX_SERVICE_PBM )||(service == QMUX_SERVICE_UIM))
    {
      QMI_CSI_OPTIONS_SET_MAX_OUTSTANDING_INDS(options, 100);
    }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  DS_QMUX_QCSI_SHIM_ASSERT((sig >= QMI_QMUX_QCSI_SHIM_SVC_SIGNAL_1) && 
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_QMUX_QCSI_SHIM_ASSERT(connection_handle && service_handle);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_QMUX_QCSI_SHIM_ASSERT(dsm_item && common_hdr.service < QMUX_SERVICE_MAX);
