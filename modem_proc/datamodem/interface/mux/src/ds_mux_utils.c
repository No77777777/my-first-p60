...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/mux/src/ds_mux_utils.c#3 $ 
  $DateTime: 2020/01/10 06:07:30 $ 
  $Author: pwbldsvc $
...
ds_mux_result_enum_type ds_mux_util_parse_start
(
  dsm_item_type                 **item_ptr,
  ds_mux_io_rx_info              *rx_info
);

ds_mux_result_enum_type ds_mux_util_parse_address
(
  dsm_item_type                 **item_ptr,
  ds_mux_io_rx_info              *rx_info
);

ds_mux_result_enum_type ds_mux_util_parse_ctrl
(
  dsm_item_type                 **item_ptr,
  ds_mux_io_rx_info              *rx_info
);

ds_mux_result_enum_type ds_mux_util_parse_length
(
  dsm_item_type                 **item_ptr,
  ds_mux_io_rx_info              *rx_info
);

ds_mux_result_enum_type ds_mux_util_parse_extended_length
(
  dsm_item_type                 **item_ptr,
  ds_mux_io_rx_info              *rx_info
);

ds_mux_result_enum_type ds_mux_util_parse_info
(
  dsm_item_type                 **item_ptr,
  ds_mux_io_rx_info              *rx_info
);

ds_mux_result_enum_type ds_mux_util_parse_fcs
(
  dsm_item_type                 **item_ptr,
  ds_mux_io_rx_info              *rx_info
);

ds_mux_result_enum_type ds_mux_util_parse_stop
(
  dsm_item_type                 **item_ptr,
  ds_mux_io_rx_info              *rx_info
);
...
)
{
  ...
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  DS_MUX_MEM_ALLOC(frame_ptr, sizeof(ds_mux_io_frame_struct_type), ds_mux_io_frame_struct_type* );
...
)
{
  ...
  {
    DS_MUX_LOG_MSG_ERROR_1("ds_mux_util_parse_start(): Unable to retrieve flag byte  %d", curr_char);
...
)
{
  ...
  if ( 1 == dsm_pullup_long ( item_ptr, &curr_char, 1) )
  {
    ...
    else
    {
      ...
      {
         LOG_MSG_ERROR_0("ds_mux_util_parse_address(): EA bit is not set in address ");
         result   = DS_MUX_FAILURE;
      }
      else
      {
...
)
{
  ...
  if ( 1 == dsm_pullup_long ( item_ptr, &curr_char, 1) )
  {
    ...
    {
      DS_MUX_LOG_MSG_ERROR_0("ds_mux_util_parse_ctrl(): Invalid frame type ");
      result = DS_MUX_FAILURE;
    }
    else
    {
  ...
  {
    DS_MUX_LOG_MSG_ERROR_0("ds_mux_util_parse_ctrl(): Unable to retrieve ctrl byte  ");
    result = DS_MUX_FAILURE;
  }
  return result;
}/* ds_mux_util_parse_ctrl */
...
)
{
  ...
  /*-----------------------------------------------------------------------
     1. Pull out one character from DSM item
     2. Calculate length from one byte
     3. Calculate FCS on length bytes
     4. Length can be in 1 or 2 byte based on EA bit
        4.1 if EA bit is 1 then length is in 1 byte
            Move into Next step to parse info
        4.2 if EA bit is 0 then length is in 2 byte
            Move into next step to parse next byte
     Note: DSM PULLUP API will free the DSM item if used become 0
   -----------------------------------------------------------------------*/ 
  if ( 1 == dsm_pullup_long ( item_ptr, &curr_char, 1) )
  {
    ...
    {
      /*Step 4.1*/
      rx_info->curr_frame_state = DS_MUX_IO_FRAME_EXT_LEN_PARSED;
  ...
  {
    DS_MUX_LOG_MSG_ERROR_0("ds_mux_util_parse_length(): Invalid length ");
    result = DS_MUX_FAILURE;
  }
  return result;
}/* ds_mux_util_parse_length */
...
)
{
  ...
  /*-----------------------------------------------------------------------
     1. Pull out one character from DSM item
     2. Calculate length from two bytes
     3. Move into Next step
     4. Calculate FCS on length bytes
     Note: DSM PULLUP API will free the DSM item if used become 0
   -----------------------------------------------------------------------*/ 
      if (1 == dsm_pullup_long ( item_ptr, &ext_len_char, 1))
      {
        /*Step 2*/
         rx_info->curr_frame_ptr->frame.length |= (((uint16)ext_len_char) << 7);
         /*Step 3*/
         rx_info->curr_frame_state = DS_MUX_IO_FRAME_EXT_LEN_PARSED;
         ...
         /*Step 4 Calculate fcs on second length byte*/
        rx_info->curr_frame_ptr->frame.fcs = ds_mux_utils_calc_fcs_per_char ( 
  ...
  {
    DS_MUX_LOG_MSG_ERROR_1("ds_mux_util_parse_extended_length(): failed to parse length %d",
      rx_info->curr_frame_ptr->frame.length);
  }

  return result;
} /* ds_mux_util_parse_extended_length */
  ...
  do
  {
    ...
    if ( NULL != rx_info->curr_frame_ptr->frame.information_ptr )
    {
      ...
      else
      {
        ...
        result  = DS_MUX_FAILURE; 
        DS_MUX_LOG_MSG_ERROR_2("ds_mux_util_parse_info(): Invalid curr pkt len %d  pkt info len %d", 
...
)
{
  ...
  if ( 1 == dsm_pullup_long ( item_ptr, &curr_char, 1) )
  {
    ...
    {
      DS_MUX_LOG_MSG_ERROR_2("ds_mux_util_parse_fcs(): mismatched fcs "
        "%x from calculation %x from packet",
        rx_info->curr_frame_ptr->frame.fcs, curr_char);
      //Check Sum failed
      result = DS_MUX_FAILURE;
    }
...
)
{
  ...
  if ( 1 == dsm_pullup_long ( item_ptr, &curr_char, 1) )
  {
    ...
    {
      result = DS_MUX_FAILURE;
      DS_MUX_LOG_MSG_ERROR_1("ds_mux_util_parse_stop(): invalid stop code %d ",
        curr_char);
    }
  }
  else
...
)
{
  ...
  while( (TRUE == processing_frames) && ( NULL != item_ptr) )
  {
    ...
    /*-----------------------------------------------------------------------
        ...
        | Flag  |  Address | Control  Byte| Length Indicator  |  Information |  FCS   | Flag  |
        ________________________________________________________________________
    ...
    switch ( rx_info->curr_frame_state )
    {
      ...
      case DS_MUX_IO_FRAME_INIT:
        result = ds_mux_util_parse_start( &item_ptr, rx_info);
        break;

      case DS_MUX_IO_FRAME_START_PARSED:
        result = ds_mux_util_parse_address( &item_ptr, rx_info);
        break;

      case DS_MUX_IO_FRAME_ADDR_PARSED:
        result = ds_mux_util_parse_ctrl( &item_ptr, rx_info);
        break;

      case DS_MUX_IO_FRAME_CTRL_PARSED:
        result = ds_mux_util_parse_length( &item_ptr, rx_info);
        break;

      case DS_MUX_IO_FRAME_LEN_PARSED:
        result = ds_mux_util_parse_extended_length( &item_ptr, rx_info);
        break;

      case DS_MUX_IO_FRAME_EXT_LEN_PARSED:
        result = ds_mux_util_parse_info( &item_ptr, rx_info);
        break;

      case DS_MUX_IO_FRAME_INFO_PARSED:
        result = ds_mux_util_parse_fcs( &item_ptr, rx_info);
        break;

      case DS_MUX_IO_FRAME_FCS_PARSED:
        result = ds_mux_util_parse_stop( &item_ptr, rx_info);
        break;
    ...
    {
      processing_frames   = FALSE;
      DS_MUX_LOG_MSG_ERROR_1("ds_mux_util_decode_input(): failed to parse: current state %d ",
        rx_info->curr_frame_state);
    }
...
)
{
  ...
  return result;
}/* ds_mux_util_process_wake_up_request */
...
)
{
  ...
  if ( ( NULL == item_ptr ) || ( NULL == rx_info ) )
  {
    DS_MUX_LOG_MSG_ERROR_2("ds_mux_util_process_wake_up_response(): Invalid params dsm item 0x%x  rx info 0x%x", 
        item_ptr, rx_info);
    return DS_MUX_INVALID_PARAM;
  }
  /*-----------------------------------------------------------------------
     Review received payload, It should contain F9s to complete wake-up hand-shake
   -----------------------------------------------------------------------*/
  pkt_len = dsm_length_packet( item_ptr );
  ...
  if ( 0 != pkt_len )
  {
    ...
    /*-----------------------------------------------------------------------
     Scneario : When both MUX trying to wake up and both are in WAKE-Up request mode 
     UART drop F9 response in one direction. In such situation one mux move into OPEN state
     and another is waiting for wake up response.Waiting MUX may drop  one packet 
     as  packet first F9 will be treated as wake up sequnce for waiting mux
   -----------------------------------------------------------------------*/

    
  /*-----------------------------------------------------------------------
     Free recevied DSM item
   -----------------------------------------------------------------------*/
  dsm_free_packet( &item_ptr );
  ...
  return result;
}/* ds_mux_util_process_wake_up_response */
...
)
{
  ...
  /*-----------------------------------------------------------------------
     ...
     come out from powersave otherwise wait for valid frame.
   -----------------------------------------------------------------------*/
  ...
  return result;
}/* ds_mux_util_process_first_frame_after_wake_up */
...
)
{
  ...
  if ( TRUE == is_command )
  {
     //Incase of Initiator; Initiator will set bit 1
...
)
{
  ...
  /*-----------------------------------------------------------------------
    ...
    sent by the responding station have the C/R bit set to 0.
  ...
  {
    output_frame->command_response = TRUE;
  }
  else
  {
