...
DESCRIPTION
  ...
  DS MUX Task services source file. All definitions, structures,
...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/mux/src/ds_mux_svc.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  ds_mux_enabled_sig_mask |= ( (rex_sigs_type) 1 << (rex_sigs_type) sig );
...
FUNCTION DS_MUX_WAIT()

DESCRIPTION
  This function performs the DS_MUX Task waiting function. This function will
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  while ( (set_sigs & requested_mask) == 0 )
  {
    set_sigs = rex_wait( requested_mask |
                         (  1 << (rex_sigs_type)DS_MUX_DOG_HB_REPORT_SIGNAL ) );

    /*-----------------------------------------------------------------------
      If dog heart beat received, ack the beat. Do ping<->pong.
    -----------------------------------------------------------------------*/
    if( set_sigs & ( 1 << (rex_sigs_type)DS_MUX_DOG_HB_REPORT_SIGNAL ) )
    {
      (void)rex_clr_sigs( rex_ds_mux_tcb,
                    (  1 << (rex_sigs_type)DS_MUX_DOG_HB_REPORT_SIGNAL ) );
  ...
  (void)rex_clr_sigs( rex_ds_mux_tcb, ( set_sigs & requested_mask ) );
