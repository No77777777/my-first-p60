...
GENERAL DESCRIPTION
  ...
  tasks/application to interface to the MUX Ports over SIO.
...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/mux/src/ds_mux_logical.c#2 $ 
...
/*-----------------------------------------------------------------------
  Note: This variable can be set and reset only for SSR handling. Otherwise it set TRUE only 
  once ( e.g In +CMUX mode this variable ) as device assigned only once to service and remain 
  till reboot
-----------------------------------------------------------------------*/
static boolean                     disallow_port_reassignment = FALSE;
...
)
{
  ...
  ds_mux_logical_sio_iface_info_type *ds_mux_logical_conn_ptr = &ds_mux_logical_conn_info;
  ...
  if ( FALSE == is_initialized )
  {
    ...
    memset( ds_mux_logical_conn_ptr, 0x0, sizeof(ds_mux_logical_sio_iface_info_type));
  
    for ( index  = 0; index < DS_MUX_MAX_LOGICAL_CONNECTION; index ++)
    {
  
      ds_mux_logical_conn_ptr->logical_cons[index].dlci_id        = 0xFF;
  
      ds_mux_logical_conn_ptr->logical_cons[index].port_id        = SIO_PORT_NULL;
          
      ds_mux_logical_conn_ptr->logical_cons[index].rdm_oprt_retry = 0;
      
      ds_mux_logical_conn_ptr->logical_cons[index].timer_handle   = DS_MUX_TIMER_INVALID_HANDLE;
  
      ds_mux_logical_conn_ptr->logical_cons[index].port_status = 
        DS_MUX_LOGICAL_PORT_MIN;
  
      ds_mux_logical_conn_ptr->logical_cons[index].rdm_device  = 
        DS_MUX_BASE_LOGICAL_RDM_DEVICE + index;
      
      ds_mux_logical_conn_ptr->logical_cons[index].service     = 
        DS_MUX_LOGICAL_SERVICE + index;

     ds_mux_logical_conn_ptr->logical_cons[index].ioctl_data.in_bound_flow_disabled = TRUE;
...
uint8   ds_mux_get_idx_from_rdm_srv
(
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/  
  ...
  for ( index  = 0; index < DS_MUX_MAX_LOGICAL_CONNECTION; index ++)
  {
     if ( service == ds_mux_logical_conn_info.logical_cons[index].service )
     {
       break;
     }
  }
  ...
  DS_MUX_LOG_MSG_INFO2_2( "ds_mux_get_idx_from_rdm_dev index %d service %d ",
    index, service);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /*-------------------------------------------------------------------------
    ...
    1. Store close  call back function. Close call back will be invoked once DLCI receive MSC acknowledgement
  ...
  ds_mux_logical_conn_info.logical_cons[index].port_status =  DS_MUX_LOGICAL_PORT_CLOSING; 
  ...
  /*-----------------------------------------------------------------------
    Free Tx Wmk items and dsm items here ( as application may invoke delete wmk API ) After API call.
  -----------------------------------------------------------------------*/
  wmk = ds_mux_logical_conn_info.logical_cons[index].tx_queue;
  while (( NULL != wmk) && ( NULL != (item_ptr = dsm_dequeue( wmk ))))
  {
    (void)dsm_free_packet(&item_ptr);
  }

  ds_mux_logical_conn_info.logical_cons[index].tx_queue = NULL;
  /*-----------------------------------------------------------------------
    Rx Wmk should be drained by application as data has delivered to them hence not 
    flushing any rx data only resetting rx wmk ptr
  -----------------------------------------------------------------------*/
  ds_mux_logical_conn_info.logical_cons[index].rx_queue = NULL;
...
{
  uint8                      index = 0xFF;
  void                     (*close_func_ptr)(void) = NULL;
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

  DS_MUX_LOG_MSG_INFO2_1( "ds_mux_logical_notify_logical_port_close dlci ID %d ", dlci_id);
  
  /*-----------------------------------------------------------------------
    1 Received Acknowledgement about port close
    2.Set port status as closed
    3.Notify Application If Application registered for Close call back function. 
  -----------------------------------------------------------------------*/ 
  DS_MUX_ENTER_CRIT_SECTION(&global_ds_mux_crit_section);

  if ( 0xFF != ( index = ds_mux_get_idx_from_dlci_id( dlci_id )) )
  {
    if ( NULL != ds_mux_logical_conn_info.logical_cons[index].close_cb_ptr &&
         DS_MUX_LOGICAL_PORT_CLOSING == ds_mux_logical_conn_info.logical_cons[index].port_status )
    {
      close_func_ptr = ds_mux_logical_conn_info.logical_cons[index].close_cb_ptr;
    }
    
    ds_mux_logical_conn_info.logical_cons[index].port_status =  DS_MUX_LOGICAL_PORT_CLOSED;
    ds_mux_logical_conn_info.logical_cons[index].close_cb_ptr = NULL;
    /*-----------------------------------------------------------------------
      RESET ioctl call back function .
    -----------------------------------------------------------------------*/
    memset(&ds_mux_logical_conn_info.logical_cons[index].ioctl_data.dtr_evt_info, 0x0,
               sizeof(sio_ioctl_enable_dtr_event_ext_type));
    memset(&ds_mux_logical_conn_info.logical_cons[index].ioctl_data.dtr_backward, 0x0,
               sizeof(sio_vv_func_ptr_type));
    memset(&ds_mux_logical_conn_info.logical_cons[index].ioctl_data.cd_evt_info, 0x0,
               sizeof(sio_ioctl_enable_cb_event_ext_type));
    memset(&ds_mux_logical_conn_info.logical_cons[index].ioctl_data.ri_evt_info, 0x0,
               sizeof(sio_ioctl_enable_ri_event_ext_type));

  }

  DS_MUX_LEAVE_CRIT_SECTION(&global_ds_mux_crit_section);
  
  if(NULL != close_func_ptr)
  {
    close_func_ptr();
  }
}/* ds_mux_logical_notify_logical_port_close*/
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if( DS_MUX_LOGICAL_PORT_CLOSED == ds_mux_logical_conn_info.logical_cons[index].port_status)
  {
    return;
  }
  DS_MUX_ENTER_CRIT_SECTION(&global_ds_mux_crit_section);
...
)
{
  ...
  if (( 0xFF != index) &&
     ...
     (  TRUE  == ds_mux_logical_conn_info.logical_cons[index].ioctl_data.remote_sig_state.rtc) &&
...
)
{
  ...
  for ( index = 0; index < DS_MUX_MAX_LOGICAL_CONNECTION; index++ )
  {
    ...
    /* Update SIO Port ID */
    ds_mux_logical_conn_info.logical_cons[index].port_id = device_iface.device_id;
        
    /* Step 4: Register with RDM */
    rdm_device_info.device = DS_MUX_BASE_LOGICAL_RDM_DEVICE + index;
    ...
    rdm_register_device(&rdm_device_info);
     ...
     DS_MUX_LOG_MSG_INFO2_4( "ds_mux_logicali_driver device_id %d dlci_id %d sio port %d d port id %d", 
        device_id, 
        ds_mux_logical_conn_info.logical_cons[index].dlci_id,
        sio_port_id[index],
        ds_mux_logical_conn_info.logical_cons[index].port_id );
...
)
{
  ...
  DS_MUX_LOG_MSG_INFO2_3( "ds_mux_logical_rdm_assign_port_cb status %d service %d device %d",
    status, service, device);

  /*-----------------------------------------------------------------------
    Get Index from device or service
    -----------------------------------------------------------------------*/
  if ( RDM_NULL_DEV != device)
  {
  ...
  {
    if(0xFF == (inst = ds_mux_get_idx_from_rdm_srv(service)) )
    {
      return;
    }
  }
  ...
  if ( RDM_DEVMAP_BUSY_S == status )
  {
    if( ds_mux_logical_conn_info.logical_cons[inst].rdm_oprt_retry  < DS_MUX_MAX_RDM_OPR_RETRY)
    {
      ...
      if( PS_TIMER_FAILURE == ds_mux_timer_start( ds_mux_logical_conn_info.logical_cons[inst].timer_handle,
...
static void ds_mux_logical_rdm_null_device_timer_cb
(
  void *  data
)
{
  ...
  DS_MUX_LOG_MSG_INFO2_2( "ds_mux_logical_rdm_null_device_timer_cb instance %d result %d", inst, result);
  
}/* ds_mux_logical_rdm_null_device_timer_cb */


void ds_mux_logical_assign_port_to_service
(
...
)
{
  uint32          inst             = 0;
  boolean         res              = FALSE;
  rdm_device_enum_type     assign_device    = RDM_NULL_DEV;
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  
/*-----------------------------------------------------------------------
  1.  Reset the retry counter
  2.  Allocate timer for retry
  3.  Assign Port : Set compatibility and assign the device to servce so that clients can get
       RDM open call back function
  4. Close Port : Assign NULL device to service
  -----------------------------------------------------------------------*/ 
  DS_MUX_ENTER_CRIT_SECTION(&global_ds_mux_crit_section);

  if(FALSE == disallow_port_reassignment)
  {
    disallow_port_reassignment = TRUE;
    ...
    for (inst = 0; inst < DS_MUX_MAX_LOGICAL_CONNECTION; inst++) 
    {
      ...
      if ( DS_MUX_LOGICAL_PORT_ASSIGN == port_status )
      {
      rdm_set_compatibility( ds_mux_logical_conn_info.logical_cons[inst].service,
                             ds_mux_logical_conn_info.logical_cons[inst].rdm_device);
  
        assign_device = ds_mux_logical_conn_info.logical_cons[inst].rdm_device;

       ds_mux_logical_conn_info.logical_cons[inst].timer_handle  =  
          ds_mux_timer_alloc( ds_mux_logical_rdm_oprt_timer_cb,  (void *)inst );
      }
      else
      {
        ds_mux_logical_conn_info.logical_cons[inst].timer_handle  = 
            ds_mux_timer_alloc( ds_mux_logical_rdm_null_device_timer_cb,  (void *)inst );
      }

      ds_mux_logical_conn_info.logical_cons[inst].port_status = port_status;
  
      res = rdm_assign_port( ds_mux_logical_conn_info.logical_cons[inst].service,
                             assign_device,
                             ds_mux_logical_rdm_assign_port_cb );
      
      DS_MUX_LOG_MSG_INFO1_5( "ds_mux_logical_assign_port_to_service result %d port_status %d timer handle %d "
                              " inst %d assign_device %d",res, port_status, 
                              ds_mux_logical_conn_info.logical_cons[inst].timer_handle, inst, assign_device);
      
    }
  }
  DS_MUX_LEAVE_CRIT_SECTION(&global_ds_mux_crit_section);
  
}/* ds_mux_logical_assign_port_to_service */
...
)
{
  ...
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ds_mux_logical_assign_port_to_service( DS_MUX_LOGICAL_PORT_ASSIGN );
...
void ds_mux_de_assert_all_ctl_signals
(
  uint8                   index
)
{
...
}/*ds_mux_de_assert_all_ctl_signals*/
...
)
{
  ...
  if (0xFF != ( index = ds_mux_get_idx_from_dlci_id( dlci_id )))
  {
    ds_mux_de_assert_all_ctl_signals(index);
...
)
{
  ...
  DS_MUX_LOG_MSG_INFO2_1( "ds_mux_logical_notify_connected_dlci dlci ID %d ", dlci_id);
  ...
  {
    if ( TRUE == ds_mux_logical_conn_info.logical_cons[index].ioctl_data.cd_asserted ||
         TRUE == ds_mux_logical_conn_info.logical_cons[index].ioctl_data.ri_asserted ||
...
}/* ds_mux_logical_notify_connected_dlci*/
...
)
{
  ...
  for (inst = 0; inst < DS_MUX_MAX_LOGICAL_CONNECTION; inst++) 
  {
    ...
    if ( NULL != ds_mux_logical_conn_info.logical_cons[inst].tx_queue &&
        ( FALSE == dsm_is_wm_empty( ds_mux_logical_conn_info.logical_cons[inst].tx_queue )))
    {
      return FALSE;
    }
  ...
  return TRUE;
}/* ds_mux_logical_is_power_save_allowed*/
...
)
{
  ...
  /*-----------------------------------------------------------------------
    2.  Assign NULL Port (device) to services so that Application can get RDM close call back
    -----------------------------------------------------------------------*/ 
  ds_mux_logical_reset_port_reassignment_flag();
  ds_mux_logical_assign_port_to_service( DS_MUX_LOGICAL_PORT_ASSIGN_NONE );

  /*-----------------------------------------------------------------------
  Application will invoke sio_close which will close the port and clear other application related 
  info
   -----------------------------------------------------------------------*/ 
...
{
  disallow_port_reassignment = FALSE;
}/* ds_mux_logical_set_assgined_flag */
