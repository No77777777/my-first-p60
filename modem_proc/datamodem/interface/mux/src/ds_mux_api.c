...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/mux/src/ds_mux_api.c#2 $ 
...
)
{
  ...
  if (SIO_PORT_NULL == sio_port_id )
  {
    return DS_MUX_INVALID_PARAM;
  }
  

  result = ds_muxi_active_init(TRUE, sio_port_id, event_info);

  DS_MUX_LOG_MSG_INFO2_2("ds_mux_active_init(): "
                         "port %d result %d", sio_port_id, result );
...
)
{
  ...
  if ( DS_MUX_SUBSET_UIH <= subset &&
       subset <= DS_MUX_SUBSET_I)
  {
...
)
{
  ...
  if ( DS_MUX_PHY_PORT_SPEED_INVALID < port_speed &&
       port_speed <= DS_MUX_PHY_PORT_SPEED_7)
  {
...
)
{
         ...
         wake_up_timer <= DS_MUX_CMUX_MAX_TX_T3 )
  {
    ...
    dlci_param->max_wake_up_time_T3  = wake_up_timer;
...
)
{
  ...
  ret_val = ds_mux_logical_driver_init( register_driver_cb_fn );
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_MUX_LOG_MSG_INFO2_0("ds_mux_logical_driver_init(): ");
