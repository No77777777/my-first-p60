...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/dsprofile/src/ds_profilei.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
{
  /* Call tech specific handler */
  if (plm_tbl.vtbl.validate_profile_num == NULL) 
...
)
{
  ...
  do 
  {
    ...
    if( tmp_hndl == NULL )
    {
      return_status = DS_PROFILE_REG_RESULT_ERR_OUT_OF_MEMORY;
      break;
    }
    ...
    if ((return_status = 
         ( plm_tbl.vtbl.profile_read(tech, num,
                                     &tmp_hndl->prf_blob,
                                     subs_id))) 
  ...
  if (DS_PROFILE_REG_RESULT_SUCCESS != return_status) 
  {
    if (tmp_hndl) 
    {
      if (tmp_hndl->prf_blob != NULL &&
          plm_tbl.vtbl.dealloc != NULL)
      {
  ...
  LOG_MSG_QTRACE_HFT_MED(PS_TAG_DSPROFILE, ps_utils_subs_id_dsprofile_to_ps(subs_id),
                         "_alloc_hndl exiting: Trn %d, tech %d,"
                         "num %d, subs %d, result %d ",
...
{  
  ...
  if ((return_status = dsi_profile_validate_profile_num(dsi_hndl->tech, 
                                                        dsi_hndl->num, 
                                                        subs_id)) !=
      DS_PROFILE_REG_RESULT_SUCCESS )
  {
...
/*lint -restore Restore lint error 655*/
  ...
  if ( return_status !=  DS_PROFILE_REG_RESULT_SUCCESS )
  {
    DS_PROFILE_LOGE( "_set_param: EXIT with ERR %d",
                     return_status);
    return return_status;
  }
...
)
{
  ...
  if ( (return_status = plm_tbl.vtbl.profile_write(dsi_hndl->tech,
       ...
       !=  DS_PROFILE_REG_RESULT_SUCCESS )
  {
    DS_PROFILE_LOGE( "_end_transaction: FAIL write %d"
                     , return_status );
    return return_status;
  }
...
/*===========================================================================
FUNCTION DSI_PROFILE_DELETE_ATTACH
...
SIDE EFFECTS 
  none
===========================================================================*/
ds_profile_status_etype dsi_profile_delete_attach( 
  ds_profile_tech_etype tech,
  ds_profile_num_type   num,
  ds_profile_subs_etype subs_id
)
{
  if ( plm_tbl.vtbl.del == NULL )
    return DS_PROFILE_REG_RESULT_ERR_INVAL_PROFILE_TYPE;
  /* Call tech specific handler */
  return plm_tbl.vtbl.del_attach(tech, num, subs_id);
}
...
)
{
  ...
  {
    DS_PROFILE_LOGD( "_set_default_profile_num_per_subs: "
                     "EXIT with ERR subs %d", subs_id );
...
)
{
  ...
  {
    DS_PROFILE_LOGD( "_set_default_profile_num_per_subs: "
                     "EXIT with ERR with subs id %d", subs_id );
    return DS_PROFILE_REG_RESULT_ERR_INVAL_SUBS_ID;
  }
  /*--------------------------------------------------------------------
    Dual sim feature is not turn on so calling the 
...
)
{
  ...
  {
    (void)ds_util_list_rel_hndl(hndl);
    return DS_PROFILE_REG_RESULT_ERR_OUT_OF_MEMORY;
  }
...
)
{
  ...
  {
    DS_PROFILE_LOGE("_itr_next: list end ", 0 );
...
)
{
  ...
  {
    DS_PROFILE_LOGE("_itr_first: exit with failure ", 0 );
    return DS_PROFILE_REG_RESULT_FAIL;
  }
...
{  
  ...
  /* Validate tech type */
  if ( !TECH_IS_VALID( dsi_hndl->tech ) 
       && dsi_hndl->tech != DS_PROFILE_TECH_EPC ) 
  {
    return DS_PROFILE_REG_RESULT_ERR_INVAL_PROFILE_TYPE;
  }
  ...
  if ( return_status !=  DS_PROFILE_REG_RESULT_SUCCESS )
  {
    DS_PROFILE_LOGE( "_get_param_in_use: EXIT with ERR %d", return_status);
    return return_status;
  }
...
{
  /*check whether passed TECH is initialized or not*/
  if ((DS_PROFILE_TECH_MIN <= tech && tech < DS_PROFILE_TECH_MAX) && 
  	   ((valid_tech_mask & (0x01 << tech)) != 0))
  {
