  ...
  ds_profile_identifier_type ident)
{
  switch (ident)
  {
    ...
    case (DS_PROFILE_GENERAL_PROFILE_PARAM_APN_TYPE):
      return GENERAL_PROFILE_PARAM_APN_TYPE;
    default:
      return ident;
  }
}
...
{
  #define DS_PROFILE_AP_CLIENT 0x01
  ...
  {    
    ...
    if (config_ptr->config_mask & DS_PROFILE_CONFIG_MASK_ATTACH_PROFILE) 
    {
      params.is_attach = config_ptr->is_attach;
    }

    if (config_ptr->config_mask & DS_PROFILE_CONFIG_MASK_AP_CLIENT) 
    {
      params.profile_client = DS_PROFILE_AP_CLIENT;
    }
...
{
  ds_profile_db_subs_type db_subs = DB_ACTIVE_SUBSCRIPTION_NO_CHANGE;
  ds_profile_status_etype res = DS_PROFILE_REG_RESULT_SUCCESS;
...
===========================================================================*/
ds_profile_status_etype ds_profile_db_handler_delete_attach(
  ds_profile_tech_etype tech,
  ds_profile_num_type   num,
  ds_profile_subs_etype subs
)
{
  ...
  return db_status_to_ds_profile_status(ds_profile_db_delete_attach(num, 
                                                             legacy_tech_to_techmask(tech), 
                                                             db_subs));
}
...
)
{
  ...
  return db_status_to_ds_profile_status(ds_profile_db_reset_profile_param(num, 
                                                                          legacy_tech_to_techmask(tech), 
...
/*===========================================================================
FUNCTION ds_profile_db_handler_get_profile_client
...
===========================================================================*/
ds_profile_status_etype ds_profile_db_handler_get_profile_client(
  ds_profile_num_type profile_num,
...
)
{
  ...
  return db_status_to_ds_profile_status(
	ds_profile_db_get_profile_client(profile_num, db_subs, 
								  profile_client));
}
