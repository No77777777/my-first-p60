...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/dpm/src/ps_dpm_utils.c#2 $
...
static const ps_dpmi_cfg_item_info_t ps_dpmi_cfg_item_info[] = {
    ...
    { 
        PS_DPM_CFG_DISABLE_HW_PATH, 
        "Disable hardware path",
        "modem/data/interface/disable_dl_data_hw_path",
        sizeof(boolean)
    }  /* NV-item number: 73884 */    
};
...
{
    int result = -1;
    DS_UTILS_ASSERT(info_p != NULL);
    DS_UTILS_ASSERT(path !=NULL);
...
{ 
    ...
    if (!val) 
    {
        LOG_MSG_ERROR_INT_0("val ptr is NULL");
        goto err;
    }
    ...
    if (!info_p)
    {
        LOG_MSG_ERROR_INT_1("Failed to get item info, item %d", (int)item);
    }
...
/*-------------------------------------------------------------------------*/
  ...
  {
    LOG_MSG_ERROR_INT_0("Failed to allocate memory");
    return -1;
  }
  ...
  for (i = 0; i < PS_DPM_CFG_MAX; i ++)
  {
                                       ...
                                       PS_DPMI_CFG_MAX_PATH_SIZE))
    {
      ...
      {
        LOG_MSG_ERROR_INT_1("Fail to copy all files path in buffer file_paths_buf_len %d",
                         file_paths_buf_len);
        return -1;
      }
    ...
    {
      LOG_MSG_ERROR_INT_1("ps_dpmi_cfg_get_item_path failed for item %d skipping",
                      i);
    }
  }
...
)
{
  ...
  if( MCFG_FS_STATUS_OK != rsp )
  {
    ...
    if( ENOENT != rsp )
    {
      LOG_MSG_ERROR_INT_1("efs_lstat error %d", rsp);
      return rsp;
    }
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR_INT_0("conf_file_path is NULL");
    return -1;
  }
  ...
  if( 0 != result )
  {
    ...
    LOG_MSG_ERROR_INT_0("Create /nv dir in EFS");
    result = mcfg_mkdir( "/nv", MCFG_FS_ALLPERMS, file_type);
    if ( -1 == result )
    {
      LOG_MSG_ERROR_INT_1("Create EFS Dir Failed: "
  ...
  if ( 0 > config_fd )
  {
    LOG_MSG_ERROR_INT_1("Error creating config file, "
                    "error %d", mcfg_fs_errno( file_type ));
    return mcfg_fs_errno( file_type );
  }
  ...
  if ( 0 != result )
  {
    LOG_MSG_ERROR_INT_1("Error closing config file, "
                    "error %d", mcfg_fs_errno( file_type ));
    return mcfg_fs_errno( file_type );
  }
  return 0;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR_INT_0("Error storing efs item file paths in buffer");
  ...
  if ( 0 > config_fd )
  {
    if ( ENOENT == mcfg_fs_errno( file_type ) ) /* Conf file does not exist */
    {
      ...
      if ( 0 != result )
      {
        LOG_MSG_ERROR_INT_1("EFS: Error creating config file, "
                        "error %d", mcfg_fs_errno( file_type ));
        PS_SYSTEM_HEAP_MEM_FREE(file_paths_buf_ptr);
        return mcfg_fs_errno( file_type );
      }
      ...
      if ( 0 > config_fd )
      {
        LOG_MSG_ERROR_INT_1("EFS: Error opening config file, "
                        "error %d", mcfg_fs_errno( file_type ));
        PS_SYSTEM_HEAP_MEM_FREE(file_paths_buf_ptr);
        return mcfg_fs_errno( file_type );
      }
  ...
  if(conf_file_valid)
  {
    ...
    {
      LOG_MSG_ERROR_INT_2("Conf file invalid size %d, expected size %d", 
  ...
  if(conf_file_valid)
  {
    ...
    {
      LOG_MSG_ERROR_INT_0("memcmp failed, write into conf file again");
      conf_file_valid = FALSE;
    }
  }
  ...
  if(FALSE == conf_file_valid)
  {
    ...
    if(mcfg_fwrite(config_fd, file_paths_buf_ptr, 
       ...
       != (fs_size_t)strlen(file_paths_buf_ptr))
    {
      LOG_MSG_ERROR_INT_0("Failed to write efs file paths into conf file");
      ret_val = -1;
    }
