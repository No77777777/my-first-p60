...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/dpm/src/ps_dpm_mbim_hdlr.c#2 $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_UTILS_ASSERT(user_data != NULL);
  ...
  {
    LOG_MSG_ERROR_INT_0("Unable to retrieve NTH heder");
  ...
  while ( (ndp_offset != 0) && (ndp_offset < nth_hdr.mbim_block_len) )
  {
    ...
    {
      LOG_MSG_ERROR_INT_0("ps_dpm_mbim_rx_sig_hdlr : Unable to retrieve "
    ...
    if (ndp_buf == NULL)
    {
      LOG_MSG_ERROR_INT_0("ps_dpm_mbim_rx_sig_hdlr : NDP buf not allocated");
      dsm_free_packet(&ntb_dsm_item);
      return FALSE;
    }
    ...
    {
      LOG_MSG_ERROR_INT_0("ps_dpm_mbim_rx_sig_hdlr : Unable to retrieve full "
	                  "NDP buffer");
      PS_SYSTEM_HEAP_MEM_FREE (ndp_buf);
      dsm_free_packet(&ntb_dsm_item);
      return FALSE;
...
{
  LOG_MSG_ERROR_INT_0("ps_dpm_mbim_rx_sig_hdlr : MBIM not supported");
  return TRUE;
} /* ps_dpm_mbim_rx_sig_hdlr */
