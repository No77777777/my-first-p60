...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/dpm/src/ps_dpm_mbim_ul_opt.c#3 $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_UTILS_ERROR_FATAL("ps_dpm_mbim_ul_opt_init(): Couldn't alloc meta info");
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  do
  {
    ...
    if (PS_DPM_OPT_V4_VERSION == version)
    {
      ...
      {
        LOG_MSG_ERROR_INT_0("ps_dpm_mbim_ul_opt_fragment_packet(): Dropping pkt"
      ...
      {          
        ...
        {
          LOG_MSG_ERROR_INT_2("ps_dpm_mbim_ul_opt_fragment_packet(): Dropping pkt"
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  do
  {
    ...
    if (version == PS_DPM_OPT_V4_VERSION)                   /* IPv4 packet */
    {
      ...
      {
        LOG_MSG_ERROR_INT_1("ps_dpm_mbim_ul_opt_ip_rx_sig_hdlr(): "
    ...
    else
    {
      ...
      else
      {
        ...
        {
          LOG_MSG_ERROR_INT_0("ps_dpm_mbim_ul_opt_ip_rx_sig_hdlr(): "
    ...
    {
      LOG_MSG_INFO1_0("ps_dpm_mbim_ul_opt_ip_rx_sig_hdlr(): "
  ...
  switch (path_type)
  {
    ...
    case PS_DPM_UL_OPT_QOS_PATH:
    {
          ...
          PS_TX_META_GET_RT_META_INFO_PTR(qos_tx_meta_info_ptr) == NULL)
      {
        ...
        DPL_LOG_NETWORK_RX_DROPPED_PACKET(rm_iface_ptr, rx_pkt, DPL_IID_NETPROT_IP)
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {        
    ...
    if (version == PS_DPM_OPT_V4_VERSION) 
    {
      ...
      if (NULL == v4_dpm_um_info_ptr)
      {
        LOG_MSG_ERROR_INT_1("ps_dpm_mbim_ul_opt_clat_ip_rx_sig_hdlr(): "
                        "v4 call not UP for sig %d , dropping packet", rx_sig);
        path_type = PS_DPM_UL_OPT_NO_PATH;                        
        break;
      }
    ...
    if(logical_iface_ptr == NULL)
    {
       LOG_MSG_ERROR_2("ps_dpm_mbim_ul_opt_clat_ip_rx_sig_hdlr(): "
                        "logical_iface_ptr NULL for iface 0x%x:%d ,dropping packet",
    ...
    {    
      ...
      {    
        ...
        if (-1 == frag_result)
        {
          LOG_MSG_ERROR_INT_0("ps_dpm_mbim_ul_opt_clat_ip_rx_sig_hdlr(): "
                          "Protocols fragmentation failed , dropping packet");
          path_type = PS_DPM_UL_OPT_NO_PATH;
          break;
        }   
      }
             ...
             (NULL != pkts_array[num_pkts]))
      {
                                     ...
                                     (um_base_iface_ptr->clat_pdn_cntxt))->client_pdn_handle))
        {
          path_type = PS_DPM_UL_OPT_NO_PATH;
          LOG_MSG_ERROR_INT_0("ps_dpm_mbim_ul_opt_clat_ip_rx_sig_hdlr(): "
                          "CLAT Translation failed , dropping packet");
