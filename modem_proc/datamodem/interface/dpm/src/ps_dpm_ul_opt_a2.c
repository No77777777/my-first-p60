...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/dpm/src/ps_dpm_ul_opt_a2.c#2 $
  ...
  do
  {
    ...
    if (PS_DPM_OPT_V4_VERSION == version)
    {
      ...
      {          
        ...
        {
          LOG_MSG_ERROR_INT_2("ps_dpm_ul_opt_fragment_packet(): Dropping pkt"
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ( NULL != rmnet_iface_ptr && 
      (*(uint64*)(&src_v6_prefix_addr32[0])) == 
        PS_IFACE_GET_V6_PRIMARY_ADDR_PTR(rmnet_iface_ptr)->prefix
     )
  {
    return TRUE;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  prefix = ps_ntohll(*(uint64*)((&src_v6_prefix_addr32[0])));
  LOG_MSG_ERROR_2("ps_dpm_ul_opt_ipv6_prefix_cmp_unaligned() "
                  "Src V6 prefix 0x%x%08x doesnt match Um prefix, drop pkt!",
                  (uint32)(prefix >> 32),
                  (uint32)(prefix));
  return FALSE;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_UTILS_ERROR_FATAL("ps_dpm_ul_opt_init(): Couldn't alloc meta info");
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  do
  {
    ...
    if (version == PS_DPM_OPT_V4_VERSION) 
    {
      ...
      if (NULL == rm_iface_ptr)
      {
        LOG_MSG_ERROR_1("ps_dpm_ul_opt_ip_no_qos_rx_sig_hdlr(): "
                        "rm iface 0x%p is NULL, dropping packet", rm_iface_ptr);
        path_type = PS_DPM_UL_OPT_NO_PATH;
        break;
      }   
      ...
      {
        LOG_MSG_ERROR_2("ps_dpm_ul_opt_ip_no_qos_rx_sig_hdlr():Call in down" 
                        "or going down state um iface 0x%p rm iface 0x%p," 
                        "dropping packet", um_iface_ptr, rm_iface_ptr);
        path_type = PS_DPM_UL_OPT_NO_PATH;
        break;      
      }       
    ...
    {
      LOG_MSG_ERROR_1("ps_dpm_ul_opt_ip_no_qos_rx_sig_hdlr(): "
                      "um base iface 0x%p is NULL, dropping packet", um_base_iface_ptr);
      path_type = PS_DPM_UL_OPT_NO_PATH;
      break;
    }     
    ...
    else
    {
      ...
      {
        LOG_MSG_ERROR_INT_0("ps_dpm_ul_opt_ip_no_qos_rx_sig_hdlr(): "
  ...
  while (NULL != pkts_array[num_pkts] && 
         num_pkts < PS_DPM_UL_OPT_MAX_FRAG_PACKETS)
  {
    PS_ENTER_CRIT_SECTION(&global_ps_crit_section);
    ...
    /*-----------------------------------------------------------------------
      Log packet on the Um iface. HW do logging if ADPL is enabled
    -----------------------------------------------------------------------*/
...
send_data :
    ...
    switch (path_type)
    {
      ...
      case PS_DPM_UL_OPT_QOS_PATH:
      {
        ...
        {
          LOG_MSG_ERROR_1("ps_dpm_ul_opt_ip_no_qos_rx_sig_hdlr() "
                          "Cannot allocate meta info tx 0x%p ," 
                          "dropping packet", qos_tx_meta_info_ptr);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  do
  {
    ...
    /*-----------------------------------------------------------------------
      ...
      process the packet appropriately
    -----------------------------------------------------------------------*/
    (void) dsm_extract(rx_pkt, PS_DPM_OPT_QOS_HEADER_OFFSET, &ip_hdr_first_byte, 1);
    ...
    if (version == PS_DPM_OPT_V4_VERSION) 
    {
      if (NULL == v4_dpm_um_info_ptr)
      {
        LOG_MSG_ERROR_INT_1("ps_dpm_ul_opt_ip_qos_rx_sig_hdlr(): "
                        "v4 call not UP for sig %d, dropping packet", rx_sig);
        path_type   = PS_DPM_UL_OPT_NO_PATH;
        break;
      }
    ...
    if (version == PS_DPM_OPT_V4_VERSION)
    {
      ...
      if( (ip_hdr_len + PS_DPM_OPT_TCP_HDR_LEN_EXTRACT) > 
                      (PS_OPT_PATH_IP_QOS_HEADER_LEN + 1) )
      {
        path_type = PS_DPM_UL_OPT_LEGACY_PATH;
        break;
      }
      ...
      if (TRUE 
          == ps_dpm_ul_opt_ipv4_is_legacy(pkt_buf, rm_iface_ptr, ip_hdr_len))
      {
        LOG_MSG_INFO3_0("Sending pkt up the legacy path");
        path_type = PS_DPM_UL_OPT_LEGACY_PATH;
        break;
      }
    }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  do
  {
    ...
    if (version == PS_DPM_OPT_V4_VERSION)
    {
      if (NULL == v4_dpm_um_info_ptr)
      {
        LOG_MSG_ERROR_INT_1("ps_dpm_ul_opt_eth_qos_rx_sig_hdlr(): "
                        "v4 call not UP for sig %d, dropping packet", rx_sig);
        path_type = PS_DPM_UL_OPT_NO_PATH;
        break;
      }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {        
    ...
    /*---------------------------------------------------------------------
      Perform processing for normal v6 pkt
    ---------------------------------------------------------------------*/	 
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  do
  {
    ...
    {    
      ...
      if (NULL == v4_dpm_um_info_ptr)
      {
        LOG_MSG_ERROR_INT_1("ps_dpm_ul_opt_clat_ip_qos_rx_sig_hdlr(): "
                        "v4 call not UP for sig %d, dropping packet", rx_sig);
        path_type = PS_DPM_UL_OPT_NO_PATH;                        
        break;
      }
    ...
    if (version == PS_DPM_OPT_V4_VERSION) 
    {
      ...
      /*-----------------------------------------------------------------------
        Log the packet on the Rm iface before it is fragmented/translated. HW do logging if
        ADPL is enabled
      -----------------------------------------------------------------------*/
      if (FALSE == PS_DPM_HW_IS_DPL_ENABLED())
      {
    ...
    if (FALSE == is_pkt_trnsltd) 
    {
     ...
     /*-----------------------------------------------------------------------
        If pkt_size >  MTU, then fragment the packet. In case of error
        protocols will free dsm item.
      -----------------------------------------------------------------------*/
      if (total_len > PS_IFACE_GET_MTU(um_iface_ptr))
      {
        pkts_array[0] = NULL;
        frag_result = ps_clat_ip6_fragment_optimized
                     (
                       PS_IFACE_GET_MTU(um_iface_ptr),
                       &rx_pkt,
                       pkts_array,
  ...
  {    
    ...
    else if (version == PS_DPM_OPT_V6_VERSION)
    {
      ...
      if (NULL == um_iface_ptr)
      {
        LOG_MSG_ERROR_1("ps_dpm_ul_opt_clat_eth_no_qos_rx_sig_hdlr(): "
                        "Call in down or going down state um iface 0x%p "
                        "rm iface 0x%p, dropping packet ", um_iface_ptr);                        
        path_type = PS_DPM_UL_OPT_NO_PATH;
        break;      
      }      
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {      
    ...
    else if (version == PS_DPM_OPT_V6_VERSION)
    {
      ...
      if (NULL == v6_dpm_um_info_ptr)
      {
        LOG_MSG_ERROR_INT_1("ps_dpm_ul_opt_clat_eth_qos_rx_sig_hdlr(): "
                        "v6 call not UP for sig %d, dropping packet", rx_sig);
        path_type = PS_DPM_UL_OPT_NO_PATH;                        
        break;
      }
