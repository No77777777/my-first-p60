...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/dpm/src/ps_dpmi.h#2 $
...
#define PS_DPM_MAX_PDN_SESSIONS  20 //MSR TODO rename
...
typedef struct
{
  struct 
  {
    struct
    {
      uint64 hw_filter_matched;
      uint64 pkt_too_small; 
      uint64 frag_pkt;
      uint64 link_local; 
      uint64 non_tcp_udp;  
      uint64 opt_filter_matched;  
      uint64 rm_iface_null; 
      uint64 no_bytes; /*Some pkts are too small to be processed*/
    } pkts;
    struct
    {
      uint64 hw_filter_matched;
  ...
  struct 
  {
    struct
    {
      uint64 invalid_iface;
      uint64 invalid_ip_version; 
      uint64 dpm_rm_null;
      uint64 mbim_filter; 
      uint64 powersave_filter; 
      uint64 ether_pkt_err;  
      uint64 pkt_info_err;            
      uint64 clat_pdn_null;
      uint64 clat_translate_failure;
      uint64 no_bytes; /*Some pkts are too small to be processed*/
    } pkts;
    struct
    {
  ...
  uint64 total_dl_legacy_path_pkts; 
  uint64 total_dl_legacy_path_bytes; 
  
  uint64 total_dl_opt_path_pkts;
  uint64 total_dl_opt_path_bytes;  
} ps_dpm_dl_stats_info_type;

/**
  @brief Data type that holds DPM info per bearer
*/
struct ps_dpm_bearer_info
{
  ...
  ps_dpm_dl_stats_info_type           stats;
  struct  ps_dpm_bearer_info    *this_ptr;
};

typedef struct ps_dpm_bearer_info ps_dpm_bearer_info_type;
...
typedef struct
{
  struct 
  {
    struct 
    {
      uint64 dpm_um_null;
      uint64 iface_down;
      uint64 prefix_mismatch;
      uint64 flow_disallowed;
      uint64 frag;
      uint64 pkt_conv_failed;
      uint64 null_frag;
      uint64 meta_info_null;
      uint64 version_incorrect;
      uint64 clat_cntxt_null;
    } pkts;
  ...
  uint64 total_dequed_pkts;
  uint64 total_dequed_bytes;
  
  uint64 total_dropped_pkts;
  uint64 total_dropped_bytes;  
} ps_dpm_ul_stats_info_type;
...
*/
typedef struct ps_dpm_um_info_s
{
...
struct ps_dpm_rm_info_s
{
  ...
  void * v4_iface_event_buf_ptr;
  void * v6_iface_event_buf_ptr;
  ...
  ps_dpm_ul_stats_info_type   stats;
};
...
/**
  @brief Data type associated with loopback message event in DPM
*/
typedef struct
{
  uint32                               callback_data;
} ps_dpm_loopback_sync_msg_type;

/**
  @brief DPM command type
*/
typedef enum ps_dpm_cmd_e
{
  ...
  PS_DPM_PREFIX_UPDATED_EV_CMD       =  14,
#ifdef FEATURE_DATA_PS_464XLAT
  PS_DPM_CLAT_PDN_CONTEXT_EV_CMD        =  17,

  PS_DPM_CLAT_GLOBAL_CONTEXT_EV_CMD     =  18,
  PS_DPM_CLAT_PDN_REREG_EV_CMD          =  19,
  PS_DPM_CLAT_PDN_DEREG_EV_CMD          =  20,
#endif /* FEATURE_DATA_PS_464XLAT */
  PS_DPM_OFFLOAD_CHECKSUM_CMD         =  21,
  PS_DPM_UPDATE_TLB_BEARER_UL_LEN_CMD  = 22,
#ifdef FEATURE_LAPP_UFS
  #error code not present
#endif /* FEATURE_LAPP_UFS */
  PS_DPM_LOOPBACK_REG_CFG_CMD,
  PS_DPM_LOOPBACK_SYNC_MSG_CMD,
  PS_DPM_DPL_LOG_TIMER_CMD,
  PS_DPM_MAX_CMD
...
typedef struct
{
  ...
  union
  {
    ...
    ps_dpm_update_tlb_bearer_ul_len_info_type  um_tlb_bearer_um_len_info;
    ...
    ps_dpm_clat_dereg_pdn_cntxt_info_type   clat_pdn_cntxt_dereg_ev_info;
#endif /* FEATURE_DATA_PS_464XLAT */ 
#ifdef FEATURE_LAPP_UFS
    #error code not present
#endif /* FEATURE_LAPP_UFS */
    ps_dpm_loopback_cfg_type                loopback_reg_cfg;
    ps_dpm_loopback_sync_msg_type           loopback_sync_msg;
  } dpm_cmd_info;
...
/**
  @brief Enum describing different source of HW path switching
*/
typedef enum
{
  PS_DPMI_NV_HW_PATH            = 0x0,
  PS_DPMI_SYS_IOCTL_HW_PATH     = 0x1,
  PS_DPMI_HW_PATH_MAX           = 0xFF,
  
} ps_dpmi_switch_hw_path_enum_type;

/**
  @brief  Default HW bridging is allowed hence Set all bits to 1
*/
#define PS_DPMI_BRIDGING_ALLOWED ( 1 << PS_DPMI_NV_HW_PATH |   \
                                   1 << PS_DPMI_SYS_IOCTL_HW_PATH) 
...
/*===========================================================================
FUNCTION PS_DPM_BEARER_IS_VALID()

DESCRIPTION
  This macro returns a boolean indicating whether the bearer pointer passed in
  is valid.

PARAMETERS
  bearer_ptr: pointer to the interface in question.

RETURN VALUE
  TRUE: if bearer_ptr is valid
  FALSE: otherwise.
===========================================================================*/
#define PS_DPM_BEARER_IS_VALID(bearer_ptr)                        \
  (((bearer_ptr) != NULL) &&                                 \
   ((bearer_ptr)->this_ptr == (bearer_ptr)))
...
{ 
 if(PS_DPM_BEARER_IS_VALID(bearer_ptr))
 {
  if(err_info.err_ind_cb_fn_ptr != NULL)
  {
...
/**
  @brief
   Increments the dl stats
*/
#define PS_DPM_DL_INC_STATS(PTR,FIELD,INC)    \
  if (PTR) PS_DPM_INC_STATS(PTR->FIELD,INC)

/**
  @brief
   Increments the DPM stats
*/
#define PS_DPM_INC_STATS(FIELD,INC)    \
  (FIELD) += (INC);
/**
...
/**
  @brief  This API returns dpm rm info for given rm rx watermark

  @param[in] wmk_ptr  pointer to dpm rm  wm

  @return  DPM rm entry corresponding to rx rm wm

  @code
  @endcode
*/
ps_dpm_rm_info_type * ps_dpm_get_dpm_rm_info_by_sio_rx_wmk
(
...
/*
MACRO PS_DPMI_DISBALE_BRIDGING()

@brief 
   The marco will reset bit corresponding to NV/SYS IOCTL.
...
@return
  none
*/
#define PS_DPMI_DISBALE_BRIDGING(data_path_client)                           \
  {                                                                          \
    ps_dpm_hw_bridging_allowed &= ~(1 << data_path_client);                  \
  }

/*
MACRO PS_DPMI_ENABLE_BRIDGING()

@brief 
   The marco will set bit corresponding to NV/SYS IOCTL.
...
@return
  none
*/
#define PS_DPMI_ENABLE_BRIDGING(data_path_client)                            \
  {                                                                          \
    ps_dpm_hw_bridging_allowed |= (1 << data_path_client);                   \
  }
