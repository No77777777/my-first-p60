...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/dpm/src/ps_dpm_clat.c#2 $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR_INT_0("ps_dpm_clat_ev_cback_reg(): NULL cbak registered");
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_ERROR_INT_1("ps_dpm_clat_reg_global_context() : DPM Err %d", dpm_error_info);
  return -1;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  do
  {
    ...
    if (NULL == cntxt_info_ptr->iface_ptr)
    {
      dpm_error_info = PS_DPM_ERROR_ARG_INVALID;
      break;
    }
  
    um_base_iface_ptr = PS_DPM_GET_BASE_IFACE(cntxt_info_ptr->iface_ptr);
    ...
    {
      dpm_error_info = PS_DPM_ERROR_CLAT_CNTXT_NOT_NULL;
      break;    
    }
  ...
  LOG_MSG_ERROR_INT_1("ps_dpm_clat_reg_pdn_context() : DPM Err %d", dpm_error_info);
  return -1;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_ERROR_INT_1("ps_dpm_clat_rereg_pdn_context() : DPM Err %d", dpm_error_info);
  return -1;
} /* ps_dpm_clat_rereg_pdn_context */

int ps_dpm_clat_dereg_pdn_context
(
  ps_dpm_clat_dereg_pdn_cntxt_info_type        * dereg_info_ptr
)
{
  ps_dpm_cmd_info_type  * dpm_cmd_info_ptr;
  ps_dpm_error_enum_type  dpm_error_info = PS_DPM_SUCCESS; 
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  do
  {
    ...
    dpm_cmd_info_ptr->dpm_cmd = PS_DPM_CLAT_PDN_DEREG_EV_CMD;

    memscpy(&(dpm_cmd_info_ptr->dpm_cmd_info.clat_pdn_cntxt_dereg_ev_info),
            sizeof(ps_dpm_clat_dereg_pdn_cntxt_info_type),
            dereg_info_ptr,
            sizeof(ps_dpm_clat_dereg_pdn_cntxt_info_type)); 
  ...
  LOG_MSG_ERROR_INT_1("ps_dpm_clat_dereg_pdn_context() : DPM Err %d", dpm_error_info);
  return -1;
} /* ps_dpm_clat_dereg_pdn_context */
...
)
{
  ...
  int32                                   result = -1;
  int32                                   hw_result = -1;
  ps_dpm_clat_hw_handle_type              hw_pdn_handle = MAX_UINT32;
  ps_dpm_error_enum_type                  dpm_error_info = PS_DPM_SUCCESS;
  ...
  ps_dpm_error_enum_type                  hw_dpm_err_no = PS_DPM_SUCCESS;                                            
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if (NULL == clat_reg_info_ptr)
  {
    DS_UTILS_ASSERT(0);
    return result;
  }
  ...
  do
  {
    ...
    clat_pdn_cntxt->is_stream_id_associated = FALSE;
    ...
    hw_result = ps_dpm_hw_clat_reg_pdn_context(clat_reg_info_ptr, &hw_pdn_handle, &hw_dpm_err_no);
    if (0 != hw_result)
    {
      if (PS_DPM_ERROR_HARDWARE_OP_NOT_SUPPORTED == hw_dpm_err_no)
      {
        ...
        clat_pdn_cntxt->hw_v6_addr_handle = 
          ps_iface_ipfltr_add_clat_v6_dst_addr_fltr(clat_reg_info_ptr->iface_ptr, 
                                              &(clat_reg_info_ptr->v6_dev_addr),
                                              clat_reg_info_ptr->v6_dev_addr_prefix_len); 
  ...
  {
    LOG_MSG_ERROR_INT_0("ps_dpm_clati_reg_pdn_cntxt_cmd_hdlr() : "
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  do
  {
    ...
    if (0 != hw_result)
    {
      LOG_MSG_ERROR_INT_0("ps_dpm_clati_rereg_pdn_cntxt_cmd_hdlr() : "
      ...
      clat_pdn_cntxt->hw_v6_addr_handle = 
        ps_iface_ipfltr_add_clat_v6_dst_addr_fltr(clat_rereg_info_ptr->iface_ptr, 
...
)
{
  ...
  ps_dpm_clat_pdn_cntxt_info_type * clat_pdn_cntxt = NULL;
  ps_dpm_clat_ev_enum_type          clat_event = 
                                     PS_DPM_CLAT_EV_PDN_CONTEXT_DEREGISTERED;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if (NULL == clat_dereg_info_ptr)
  {
    DS_UTILS_ASSERT(0);
    return;
  }

  LOG_MSG_INFO2_2("ps_dpm_clati_dereg_pdn_cntxt_cmd_hdlr(): iface_ptr 0x%x:%d", 
                  PS_IFACE_IS_VALID(clat_dereg_info_ptr->iface_ptr)? 
                    clat_dereg_info_ptr->iface_ptr->name : 0,
                  PS_IFACE_IS_VALID(clat_dereg_info_ptr->iface_ptr)? 
                    clat_dereg_info_ptr->iface_ptr->instance : 0);
  ...
  do
  {
    if (NULL == clat_dereg_info_ptr->iface_ptr)
    {
      dpm_error_info = PS_DPM_ERROR_ARG_INVALID;
      break;
    }
  
    um_base_iface_ptr = PS_DPM_GET_BASE_IFACE(clat_dereg_info_ptr->iface_ptr);
    if (!PS_IFACE_IS_VALID(um_base_iface_ptr))
    {
      dpm_error_info = PS_DPM_ERROR_IFACE_INVALID;
      break;
    ...
    -----------------------------------------------------------------------*/  
    if (MAX_UINT32 != clat_pdn_cntxt->hw_pdn_handle)
    {
    ...
    if (NULL != clat_pdn_cntxt->hw_v6_addr_handle)
    {
      ps_iface_ipfltr_delete_v6_dst_addr_fltr(clat_pdn_cntxt->hw_v6_addr_handle);  
      clat_pdn_cntxt->hw_v6_addr_handle = NULL;
    }
  ...
  if (PS_DPM_SUCCESS != dpm_error_info)
  {    
    clat_event = PS_DPM_CLAT_EV_PDN_CONTEXT_DEREGISTRATION_FAILED;
  }
  ...
  {
    ps_dpm_clat_ev_cback_ptr(clat_event,
                             clat_dereg_info_ptr->client_pdn_handle);
  }
  else
  {
    LOG_MSG_ERROR_INT_0("ps_dpm_clati_dereg_pdn_cntxt_cmd_hdlr() : "
                    "No callback registered by clat client");
  }  
  ...
  LOG_MSG_INFO1_1("ps_dpm_clati_dereg_pdn_cntxt_cmd_hdlr() : DPM Err %d", 
                   dpm_error_info);    

} /* ps_dpm_clati_dereg_pdn_cntxt_cmd_hdlr */
