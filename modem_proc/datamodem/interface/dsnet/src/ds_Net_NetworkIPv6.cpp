...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/dsnet/src/ds_Net_NetworkIPv6.cpp#2 $
...
{
  ds::ErrorType  result;
  bool           initHandleFailed = false;
  bool           createSignalBusFailed = false;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (AEE_SUCCESS != result)
  {
    LOG_MSG_ERROR_4 ("NetworkIPv6::Init() :"
                     "result 0x%x, obj 0x%p, handle init failed %d, create "
                     "signal bus failed %d",
                     result, this, initHandleFailed, createSignalBusFailed);
...
)
{
  ...
  bool                        generatePrivateAddrFailed = false;
  bool                        allocNetworkIpv6AddrFailed = false;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if (0 == ppIPv6PrivAddr)
  {
    return QDS_EFAULT;
  ...
  do
  {
    ...
    {
      generatePrivateAddrFailed = true;
      break;
    }
    ...
    if (0 == pNetIPv6Addr)
    {
      allocNetworkIpv6AddrFailed = true;
      result = AEE_ENOMEMORY;
      break;
    }
  ...
  if (AEE_SUCCESS != result)
  {
    LOG_MSG_ERROR_2 ("NetworkIPv6::GeneratePrivAddr(): "
                     "NetworkIPv6Address alloc failed %d, alloc "
                     "NetworkIPv6Address failed %d",
                     generatePrivateAddrFailed, allocNetworkIpv6AddrFailed);
