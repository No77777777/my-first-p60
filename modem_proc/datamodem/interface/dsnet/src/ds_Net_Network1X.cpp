...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/dsnet/src/ds_Net_Network1X.cpp#2 $
...
#define DS_NET_NETWROK1X_ASSERT(expression) \
         dsnet_network1x_assert_wrapper(__LINE__, expression)
...
===========================================================================*/
static void dsnet_network1x_assert_wrapper
(
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_NET_NETWROK1X_ASSERT (NULL != pParent);
...
{
  ds::ErrorType  result;
  bool           signalBusCreationFailed = false;
  bool           signalBusInitFailed = false;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  do
  {
  ...
  if (AEE_SUCCESS != result)
  {
    LOG_MSG_FUNCTION_EXIT_4 ("Network1X::Init(): "
                             "Result 0x%x, signal bus creation failed %d, "
                             "signal bus init failed %d, obj 0x%p",
                             result, signalBusCreationFailed,
                             signalBusInitFailed, this);
...
{
  ds::ErrorType  result;
  bool           getStrongRefFromParentFailed = false;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if (0 == ppo)
  {
    return QDS_EFAULT;
  ...
  switch (iid)
  {
    case AEEIID_INetwork:
    {
      ...
      {
        getStrongRefFromParentFailed = true;
  ...
  LOG_MSG_FUNCTION_EXIT_6 ("Network1X::QueryInterface(): "
                           "Result 0x%x, ppo 0x%p, obj 0x%p, iid 0x%x, get "
                           "strong ref from parent failed %d, parent 0x%p",
                           result, ppo, this, iid, 
                           getStrongRefFromParentFailed, mpParent);
  mpICritSect->Leave();
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  do
  {
    ...
    LOG_MSG_INFO1_5 ("Network1X::Process(): "
                     "Handle 0x%x, event %d, event handle 0x%x, event %d, "
                     "obj 0x%p",
                     ifaceHandle,
