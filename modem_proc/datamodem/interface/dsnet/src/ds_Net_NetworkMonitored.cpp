...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/dsnet/src/ds_Net_NetworkMonitored.cpp#2 $
...
#define DS_NET_MONITORED_ASSERT(expression) \
         dsnet_monitored_assert_wrapper(__LINE__, expression)
...
===========================================================================*/
static void dsnet_monitored_assert_wrapper
(
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  do
  {
    ...
    DS_INTERFACE_LOG_MSG_INFO1_5 (PS_LOGGING_CAT_DSNET,
                                  "NetworkMonitored::ProcessIfaceStateEvent(): "
                     "Handle 0x%x, event %d, event handle 0x%x, event%d, "
                     "obj 0x%p",
                                  (unsigned int)ifaceHandle,
                                  (unsigned int)eventInfoPtr->psEventName,
                                  (int)eventInfoPtr->handle,
                                  (int)eventInfoPtr->psEventName,
                     this);
...
// can we replace the log_msg_errors with ASSERT(0) since they cause ASSERT(0) on failure anyway?
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (AEE_SUCCESS != result)
  {
    ...
    DS_UTILS_RELEASEIF (mpSigBusDpdPingInfo);
  }
  return result;
...
)
{
  ...
  *ppISigBus = NULL;
  ...
  LOG_MSG_INFO3_4 ("NetworkMonitored::GetSignalBus(): "
