...
$Header: //components/rel/data.mpss/3.5..2.1/interface/dsnet/src/ds_Net_PSQoSSpecWrapper.cpp#2 $
...
#define DS_NET_PSQOS_ASSERT(expression) \
         dsnet_psqos_assert_wrapper(__LINE__, expression)
...
===========================================================================*/
static void dsnet_psqos_assert_wrapper
(
 ...
 : refCnt (1)
{
  ...
  if (AEE_SUCCESS != DS_Utils_CreateInstance (NULL, 
                                              ...
                                              (void **) &mpICritSect))
  {
    DS_UTILS_ERROR_FATAL("PSQoSSpecWrapper::PSQoSSpecWrapper(): Cannot create crit sect");
  }
 ...
 )
{
  ...
  LOG_MSG_INFO2_4 ("PSQoSSpecWrapper::GetParams(): "
                   "obj 0x%p, param 0x%x, dst 0x%p, dst len %d",
                   this, paramName, pDst, dstLen);
 ...
 )
{
  ...
  LOG_MSG_INFO2_5 ("PSQoSSpecWrapper::SetParams(): "
                   "obj 0x%p, param 0x%x, dst 0x%p, dst len %d, field maks 0x%p",
                   this, paramName, pDst, dstLen, pFieldMask);
...
ds::ErrorType CDECL PSQoSSpecWrapper::SetICMPId
( 
 uint16 ICMPId
)
{
  ...
  IPFilterIPNextProtocolType  nextHdrProt;
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  LOG_MSG_INFO2_1 ("PSQoSSpecWrapper::SetICMPId(): "
                   "SetICMPId %c", ICMPId);
  ...
  if ((uint8) PS_IPPROTO_ICMP == nextHdrProt)
  {
    mPSIPFilterSpec.next_prot_hdr.icmp.id = ICMPId;
    mPSIPFilterSpec.next_prot_hdr.icmp.field_mask |= 
      (uint32) IPFLTR_MASK_ICMP_MSG_ID;
  }
  else
  {
    result = QDS_EINVAL;
...
bail:
  LOG_MSG_ERROR_1 ("PSQoSSpecWrapper::SetICMPId(): "
                   "SetICMPId failed, err %d", result);
  return result;
