...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/dsnet/src/ds_Net_Conversion.cpp#3 $
...
#define DS_NET_CONVERSION_ASSERT(expression) \
         dsnet_conversion_assert_wrapper(__LINE__, expression)


/*===========================================================================

FUNCTION dsnet_conversion_assert_wrapper()

DESCRIPTION
  Wrapper function for DS_NET_CONVERSION_ASSERT
...
===========================================================================*/
static void dsnet_conversion_assert_wrapper
(
...
}/* dsnet_conversion_assert_wrapper */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_FUNCTION_ENTRY_5("DS2PSQoSModifySpec(): "
                           "ModifySpec ptr %p, rxFlows %p, rxFlowsLen %d, "
                           "txFlows %p, txFlowxLen %d", pPSQoSModifySpec, 
                           ppRxFlows, rxFlowsLen, ppTxFlows, txFlowsLen);
...
} /* DS2PSQoSNetInitiatedRequestSpec() */

ds::ErrorType  DS2PSRAMARequestSpec
(
  IIPFilterPriv*                           mFilterSpec, 
  NetPlatform::PSRAMAFilterSpecType*       pPSRamaFilterSpec
)
{
  //int                                      index  = 0;
  /*-------------------------------------------------------------------------
  Validation.
  -------------------------------------------------------------------------*/
  if (NULL == mFilterSpec || NULL == pPSRamaFilterSpec)
  {
    return QDS_EFAULT;
  }

  pPSRamaFilterSpec->num_filters =     1;

  PS_SYSTEM_HEAP_MEM_ALLOC( pPSRamaFilterSpec->list_ptr ,
                              sizeof (PSFilterSpecType) ,
                               ip_filter_type* );
    
  if (NULL == pPSRamaFilterSpec->list_ptr)
  {
    return AEE_ENOMEMORY;
  }
 
 (void) DS2PSIPFilterSpec(mFilterSpec, &pPSRamaFilterSpec->list_ptr[0]);
  return AEE_SUCCESS;
}/* DS2PSRAMARequestSpec */
  ...
  *pDSEventGroup = EVENT_GROUP_INVALID;
  ...
  switch (psEventName)
  {
    ...
    case PHYS_LINK_RAN_ASST_MEDIA_ADAPT_INFO_EV:
      *pDSEventName = PhysLinkEvent::QDS_EV_RAN_ASST_MEDIA_ADAPT_EV;
      break;

    case PHYS_LINK_PACKET_DISCARD_INFO_EV:
      *pDSEventName = PhysLinkEvent::QDS_EV_PACKET_DISCARD_EV;
      break;
    ...
    case IFACE_DPD_PING_INFO_EV:
        *pDSEventName = NetworkExtEvent::QDS_EV_DPD_PING_STAT_EV;
        break;
    default:
      *pDSEventName = -1;
      break;
