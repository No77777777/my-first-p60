...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/dssock/src/ds_Sock_Socket.cpp#5 $
...
#include "ds_sys.h"
#include "ds_Sock_IRecvTTLInfoPriv.h"
...
namespace ds
{
  namespace Sock
  {
    namespace OptName
    {
      const OptNameType DS_SOCK_OPT_MIN_VAL = 0;
      const OptNameType DS_SOCK_OPT_MAX_VAL = 31;
    }

    namespace OptNamePriv
    {
      const OptNameTypePriv DS_SOCK_OPT_MIN_VAL = 0;
      const OptNameTypePriv DS_SOCK_OPT_MAX_VAL = 4;
    }
    ...
    const OptInfoType sockOptInfoArr[ ] =
    {
      ...
      { OptLevel::QDS_LEVEL_SOCKET, SO_SKIP_ROUTE_SCOPE_DEF_VAL, 0, OPT_MAX_VAL },
       ...
       /* DSS_IP_RECVTTL */
      { OptLevel::QDS_LEVEL_IP, IP_RECVTTL_DEF_VAL, 0, OPT_MAX_VAL },

       /* DSS_IP_RECVECN */
      { OptLevel::QDS_LEVEL_IP, IP_RECVECN_DEF_VAL, 0, OPT_MAX_VAL },

       /* DSS_UDP_ENCAPS */
      { OptLevel::QDS_LEVEL_UDP, UDP_ENCAPS_DEF_VAL, 0, OPT_MAX_VAL},

       /* DSS_PORT_PREF_HANDLE */
      { OptLevel::QDS_LEVEL_SOCKET, static_cast <int32> (SO_PREF_HANDLE_DEF_VAL), 0, OPT_MAX_VAL},

      /* DSS_PORT_IWLAN_CAPABLE */
      { OptLevel::QDS_LEVEL_SOCKET, 0, 0, OPT_MAX_VAL},

      /* DSS_TCP_ACCESS_PRIORITY */
      { OptLevel::QDS_LEVEL_TCP, TCP_ACCESS_PRIORITY_DEF_VAL, 0, TCP_ACCESS_PRIOR_MAX_VAL},

       /* DSS_IP_RECVSUBID */
      { OptLevel::QDS_LEVEL_IP, IP_RECVSUBID_DEF_VAL, 0, OPT_MAX_VAL },
    };
...
#define DS_SOCK_SOCKET_ASSERT(expression) \
         ds_sock_socket_assert_wrapper(__LINE__, expression)
...
===========================================================================*/
static void ds_sock_socket_assert_wrapper
(
...
)
{
  ...
  bool                   isSetNetPolicyAllowed = false;
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  -------------------------------------------------------------------------*/
  isSetNetPolicyAllowed = IsSetNetPolicySupported();
  
  if (false == isSetNetPolicyAllowed)
  {
...
bail:
  LOG_MSG_FUNCTION_EXIT_3("Socket::SetNetPolicy(): "
                          " result sock 0x%x Set policy allowed %d dsErrno %x",
                          this, isSetNetPolicyAllowed, dsErrno);
  critSectPtr->Leave();
  return dsErrno;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (AEE_SUCCESS != dsErrno)
  {
    ds_sock_error_code = DSS_ERROR_REG_NETWORK_STATE_CHANGE_FAILED;
    goto bail;
  }
  ...
  {
    ds_sock_error_code = DSS_ERROR_GET_IFACE_ID_FAILED;
    goto bail;
  }
  ...
  if (AEE_SUCCESS != dsErrno)
  {
    ds_sock_error_code = DSS_ERROR_GET_ROUTESCOPE_FAILED;
    goto bail;
  }
  ...
  if (AEE_SUCCESS != dsErrno)
  {
    ds_sock_error_code = DSS_ERROR_SET_ROUTESCOPE_FAILED;
    goto bail;
  }
...
bail:
  ...
  critSectPtr->Leave();
  LOG_MSG_FUNCTION_EXIT_5("Socket::SetNetwork(): "
                          "sock 0x%x ifaceId %d from netCtl obj 0x%x"
                          " err 0x%x error_code %d",
                          this, ifaceId, networkCtlPtr, 
                          dsErrno, ds_sock_error_code);
  return dsErrno;
} /* Socket::SetNetwork() */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    dsErrno = QDS_EINVAL;
    ds_sock_error_code = DSS_ERROR_PLATFORM_SOCKET_IS_NULL;
    goto bail;
  }
  ...
  switch (paramFamily)
  {
    case ds::AddrFamily::QDS_AF_INET6:
    {
      ...
      {
        dsErrno = QDS_EINVAL;
        ds_sock_error_code = DSS_ERROR_BIND_V6_ADDR_TO_V4_SOCK_FAILED;
        goto bail;
      }
          ...
          PS_IN6_IS_ADDR_MULTICAST( tempLocalAddr.v6.addr))
      {
        ...
        {
          dsErrno = QDS_EINVAL;
          ds_sock_error_code = DSS_ERROR_MULTICAST_NOT_SUPPORTED;
          goto bail;
        }
      }
      ...
      {
        dsErrno = QDS_EOPNOTSUPP;
        ds_sock_error_code = DSS_ERROR_INVALID_V4_MAPPED_V6;
        goto bail;
      }
      else if (false == PS_IN6_IS_ADDR_UNSPECIFIED( tempLocalAddr.v6.addr))
      {
        ...
        {
          dsErrno = QDS_EOPNOTSUPP;
          ds_sock_error_code = DSS_ERROR_GET_SOCK_OPT_FAILED;
          goto bail;
        }
        ...
        if (DSS_ERROR ==
              NetPlatform::IPv6PrivAddrIncRefCntEx( &(tempLocalAddr.v6.addr),
                                                    ...
                                                    &dsErrno))
        {
          ds_sock_error_code = DSS_ERROR_INC_REF_CNT_FAILED;
          goto bail;
        }
      }
    ...
    default:
    {
      ...
      ds_sock_error_code = DSS_ERROR_UNSUPPORTED_FAMILY;
      goto bail;
    }
...
bail:
  critSectPtr->Leave();
  LOG_MSG_FUNCTION_EXIT_4("Socket::Bind(): "
                          "sock 0x%x paramFamily %d, err 0x%x, error code %d",
                          this, paramFamily, dsErrno, ds_sock_error_code);
  return dsErrno;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (AEE_SUCCESS != dsErrno)
  {
    ds_sock_error_code = DSS_ERROR_GET_SOCKADDR_FAILED;
    goto bail;
  }
  ...
  {
    ds_sock_error_code = DSS_ERROR_CONNECT_NOT_SUPPORTED;
    goto bail;
  }
  ...
  if (false == PS_IN6_ARE_ADDR_EQUAL( newV6RemoteAddr.addr, &ps_in6addr_any))
  {
    ...
    {
      dsErrno = QDS_EINVAL;
      ds_sock_error_code = DSS_ERROR_V6_REMOTE_PORT_NULL;
      goto bail;
    }
    ...
    if (AEE_SUCCESS != GetIndexFromEvent(SocketEvent::QDS_EV_WRITE, &writeEventIndex)) {
      dsErrno = QDS_EINVAL;
      ds_sock_error_code = DSS_ERROR_INCORRECT_EVENT_TYPE;
      goto bail;
    }
        ...
        if (AEE_EWOULDBLOCK != dsErrno ||
              (AEE_EWOULDBLOCK == dsErrno &&
               (0 != ( (uint32)SocketEvent::WRITE_BIT_MASK_NULL_ROUTING_INFO &
                        eventBitMask[writeEventIndex]))))
        {
          ds_sock_error_code = DSS_ERROR_ROUTEPACKET_FAILED;
          goto bail;
        }
      }
    }
  ...
  if (AEE_SUCCESS != dsErrno)
  {
    if (AEE_EWOULDBLOCK != dsErrno)
    {
      ds_sock_error_code = DSS_ERROR_CONNECT_FAILED;
      goto bail;
    }
  }
  else
...
bail:
  critSectPtr->Leave();
  LOG_MSG_FUNCTION_EXIT_3("Socket::Connect(): "
                          "sock 0x%x err 0x%x error code %d",
                          this, dsErrno, ds_sock_error_code);
  return dsErrno;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_QTRACE_HFT_MED(PS_TAG_DSSOCK, PS_SYS_DEFAULT_SUBS,
                         "Socket::SetOpt(): Sock 0x%x level %d name %d val %d",
                         this, optLevel, optName, optVal);

  /*-------------------------------------------------------------------------
    Validate that optName is in the supported range
  -------------------------------------------------------------------------*/
  if (OptName::DS_SOCK_OPT_MIN_VAL > optName || OptName::DS_SOCK_OPT_MAX_VAL <= optName)
  {
    return QDS_EINVAL;
  }
  ...
  if (AEE_SUCCESS != dsErrno)
  {
    ds_sock_error_code = DSS_ERROR_SET_OPT_FAILED;
    goto bail;
  }
...
bail:
  critSectPtr->Leave();
  LOG_MSG_FUNCTION_EXIT_3("Socket::SetOpt(): "
                          "sock 0x%x err 0x%x error code %d",
                          this, dsErrno, ds_sock_error_code);
  return dsErrno;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (OptName::QDS_SO_TX_IFACE == optName)
  {
    ...
    LOG_MSG_FUNCTION_EXIT_5("Socket::GetOpt(): "
                          "sock 0x%x optName %d optVal %d err 0x%x error code %d",
                          this, optName, *optValPtr, dsErrno, ds_sock_error_code);
    optLen = sizeof( int32);
  }
  else
  {
    ...
    if (AEE_SUCCESS != dsErrno)
    {
      ds_sock_error_code = DSS_ERROR_GET_OPT_OPERATION_FAILED;
      goto bail;
    }
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_FUNCTION_ENTRY_5("Socket::SetOptPriv(): "
                           "Sock 0x%x level %d name %d val %d, family %d",
                           this, optLevel, optName, optVal, family);

  /*-------------------------------------------------------------------------
    Validate that optName is in the supported range
  -------------------------------------------------------------------------*/
  if (OptNamePriv::DS_SOCK_OPT_MIN_VAL > optName || OptNamePriv::DS_SOCK_OPT_MAX_VAL <= optName)
  {
    return QDS_EINVAL;
  }
...
bail:
  critSectPtr->Leave();
  LOG_MSG_FUNCTION_EXIT_3("Socket::SetOptPriv(): "
                          "sock 0x%x err 0x%x error code %d",
                          this, dsErrno, ds_sock_error_code);
  return dsErrno;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_FUNCTION_ENTRY_4("Socket::GetOptPriv(): "
  ...
  if (AEE_SUCCESS != dsErrno)
  {
    ds_sock_error_code = DSS_ERROR_GET_OPT_FAILED;
    goto bail;
  }
...
bail:
  critSectPtr->Leave();
  LOG_MSG_FUNCTION_EXIT_3("Socket::GetOptPriv(): "
                          "sock 0x%x, err 0x%x error code %d",
                          this, dsErrno, ds_sock_error_code);
  return dsErrno;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_FUNCTION_ENTRY_2("Socket::Shutdown(): "
                           "sock 0x%x dir %d",this, shutdownDir);
  ...
  if (AEE_SUCCESS != dsErrno)
  {
    ds_sock_error_code = DSS_ERROR_SHUTDOWN_FAILED;
    goto bail;
  }
...
bail:
  critSectPtr->Leave();
  LOG_MSG_FUNCTION_EXIT_3("Socket::Shutdown(): "
                          "sock 0x%x err 0x%x error code %d",
                          this, dsErrno, ds_sock_error_code);
  return dsErrno;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (0 != eventBitMask[writeEventIndex])
  {
    ...
    if (AEE_SUCCESS != dsErrno)
    {
      ds_sock_error_code = DSS_ERROR_GET_PEERNAME_OPERATION_FAILED;
      goto bail;
    }
    ...
    if (0 != eventBitMask[writeEventIndex])
    {
      ...
      ds_sock_error_code = DSS_ERROR_EVENT_BIT_MASK_FAILED;
      goto bail;
    }
  }
  ...
  if (AEE_SUCCESS != dsErrno)
  {
    ds_sock_error_code = DSS_ERROR_SENDMSG_FAILED;
    goto bail;
  }
...
bail:
  critSectPtr->Leave();
  LOG_MSG_FUNCTION_EXIT_3("Socket::Write(): "
                          "sock 0x%x, err 0x%x, error code %d",
                          this, dsErrno, ds_sock_error_code);
  return dsErrno;
...
bail:
  critSectPtr->Leave();
  LOG_MSG_FUNCTION_EXIT_3("Socket::WriteV(): "
                  "sock 0x%x err 0x%x, error code %d",
                  this, dsErrno, ds_sock_error_code);
  return dsErrno;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
    ...
    if (0 != eventBitMask[writeEventIndex])
    {
      ...
      if (isDoS == true)
      {
                  ...
                  SocketEvent::WRITE_BIT_MASK_FLOW_DISABLED) != 0)
        {
          if(!IsFlowDoSEnabled())
          {
            ds_sock_error_code = DSS_ERROR_FLOW_DISABLED_FOR_DOS;
            goto bail;
          }
        }
      }
  ...
  if (localSendPktMetaInfo)
  {
    ...
    if (AEE_SUCCESS != dsErrno)
    {
      ds_sock_error_code = DSS_ERROR_GENERATE_PKT_INFO_FAILED;
      goto bail;
    }
  }
...
bail:
  ...
  LOG_MSG_FUNCTION_EXIT_4("Socket::SendMsg(): "
                          "sock 0x%x ev bit mask 0x%x err 0x%x error code %d",
                          this, eventBitMask[writeEventIndex], dsErrno,
                          ds_sock_error_code);
  critSectPtr->Leave();
  return dsErrno;
...
)
{
  ...
  *msg_controlLenReq = nOutAncLenReq * sizeof(struct dss_cmsghdr);
  ...
  if ( AEE_SUCCESS != res )
  {
    ds_sock_error_code = DSS_ERROR_GET_RECVIF_OPT_FAILED;
    goto bail;
  }
  ...
  if ( AEE_SUCCESS != res )
  {
    ds_sock_error_code = DSS_ERROR_GET_RECVERR_OPT_FAILED;
    goto bail;
  }

   // Check if RECVTTL was set to the socket
   res = GetOpt( OptLevel::QDS_LEVEL_IP,
                 OptName::QDS_IP_RECVTTL,
                 &isRecvTTLSet );
   
   if ( AEE_SUCCESS != res )
   {
     ds_sock_error_code = DSS_ERROR_GET_RECTTL_OPT_FAILED;
     goto bail;
   }

   // Check if RECVECN was set to the socket
   res = GetOpt( OptLevel::QDS_LEVEL_IP,
                 OptName::QDS_IP_RECVECN,
                 &isRecvECNSet );
   
   if ( AEE_SUCCESS != res )
   {
     ds_sock_error_code = DSS_ERROR_GET_RECVECN_OPT_FAILED;
     goto bail;
   }
  
   // Check if RECVSUBID was set to the socket
   res = GetOpt( OptLevel::QDS_LEVEL_IP,
                 OptName::QDS_IP_RECVSUBID,
                 &isRecvSUBIDSet );
   
   if ( AEE_SUCCESS != res )
   {
     ds_sock_error_code = DSS_ERROR_GET_RECVSUBID_OPT_FAILED;
     goto bail;
   }
  ...
  for (i=0 , cmsg_ptr = DSS_CMSG_FIRSTHDR(&msg); i < nOutAncLenReq; i++)
  {
    ...
    unsigned char       RecvTTLInfo = 0;
    unsigned char       RecvECNInfo = 0;
    RecvSubidInfoType   RecvSubidInfo = {0};
    if (NULL == cmsg_ptr)
    {
      res = DS_EFAULT;
      ds_sock_error_code = DSS_ERROR_INVALID_CMSG_NXTHDR;
      goto bail;
    }
    ...
    if (NULL != pOutAncData)
    {
      ...
      if ( AEE_SUCCESS != res)
      {
        ds_sock_error_code = DSS_ERROR_GET_ANCID_FAILED;
        goto bail;
      }
    }
    ...
    switch (AncID)
    {
      case AncData::ICMP_ERROR_INFO:
      {
        if ((RecvFlags::QDS_MSG_ERRQUEUE & flags) && (TRUE == isRecvErrSet))
        {
          ...
          if ( AEE_SUCCESS != res )
          {
            ds_sock_error_code = DSS_ERROR_GET_ERR_FAILED;
            goto bail;
          }
          ...
          if ( AEE_SUCCESS != res )
          {
            ds_sock_error_code = DSS_ERROR_GET_ADDR_FAILED;
            goto bail;
          }
          ...
          {
            ds_sock_error_code = DSS_ERROR_CMSG_BUF_TOO_SHORT;
            res = QDS_ETRUNCATED;
            goto bail;
          }
      ...
      case ::ds::Sock::AncData::RECV_TTL_INFO:
      {
        if (TRUE == isRecvTTLSet)
        {
          res = ((ds::Sock::IRecvTTLInfoPriv*)
                   pOutAncData[i])->GetRecvTTLHandle(&RecvTTLInfo);
          if (AEE_SUCCESS != res)
          {
            LOG_MSG_ERROR_1("Socket::RecvMsg(): "
                            "GetRecvTTLHandle failed:%d", res);
          }
          /*-------------------------------------------------------------------------
                Sanity check for not exceeding msg_control buf.
                It was already requested by setting nOutAncLen
      ...
      case ::ds::Sock::AncData::RECV_ECN_INFO:
      {
        if (TRUE == isRecvECNSet)
        {
          res = ((ds::Sock::IRecvECNInfoPriv*)
                   pOutAncData[i])->GetRecvECNHandle(&RecvECNInfo);
          if (AEE_SUCCESS != res)
          {
            LOG_MSG_ERROR_1("Socket::RecvMsg(): "
                            "GetRecvECNHandle failed:%d", res);
          }
          /*-------------------------------------------------------------------------
                Sanity check for not exceeding msg_control buf.
                It was already requested by setting nOutAncLen
      ...
      case ::ds::Sock::AncData::RECV_SUBID_INFO:
      {
        if (TRUE == isRecvSUBIDSet)
        {
          res = ((ds::Sock::IRecvSUBIDInfoPriv*)
                   pOutAncData[i])->GetRecvSubID(&RecvSubidInfo.subs_id);
          if (AEE_SUCCESS != res)
          {
            LOG_MSG_ERROR_1("Socket::RecvMsg(): "
                            "GetRecvSubID failed:%d", res);
          }
          /*-------------------------------------------------------------------------
                Sanity check for not exceeding msg_control buf.
                It was already requested by setting nOutAncLen
                to the correct len before calling the private recvmsg.
              -------------------------------------------------------------------------*/
          if ( (controllen + DSS_CMSG_SPACE(sizeof(RecvSubidInfo)) ) > msg.msg_controllen )
          {
                ds_sock_error_code = DSS_ERROR_CMSG_BUF_TOO_SHORT;
                goto bail;
          }
          ...
          tempRes = ds::Utils::Conversion::PutCMsg( &msg, cmsg_ptr,
                                                    (int16)OptLevel::QDS_LEVEL_IP,
                                                    (int16)OptName::QDS_IP_RECVSUBID,
                                                     sizeof(RecvSubidInfo),
                                                    (void*)&RecvSubidInfo,
                                                    &controllen);
          if ( AEE_SUCCESS != tempRes )
          {
...
bail:
  ...
  if(AEE_SUCCESS != res)
  {
    LOG_MSG_FUNCTION_EXIT_3("Socket::RecvMsg(): "
                            "sock 0x%x, res %d, error code %d",
                            this, res, ds_sock_error_code);
  }
  return res;
}
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (0 != eventBitMask[readEventIndex])
  {
    if (0 != ( FLOW_CONTROL_EVENT_BIT_MASK & eventBitMask[readEventIndex]))
    {
      ...
      ds_sock_error_code = DSS_ERROR_READ_IS_BLOCKED;
      goto bail;
    }
    else
    {
      DS_SOCK_SOCKET_ASSERT( 0);
      dsErrno = AEE_EWOULDBLOCK;
      ds_sock_error_code = DSS_ERROR_EVENT_BIT_MASK_FAILED;
      goto bail;
    }
  ...
  {
    dsErrno = QDS_EFAULT;
    ds_sock_error_code = DSS_ERROR_NULL_ARGVAL;
    goto bail;
  }
...
bail:
  ...
  if ( numReadPtr != 0 )
  {
    LOG_MSG_FUNCTION_EXIT_4("Socket::RecvMsg(): "
                          "sock 0x%x read %d err 0x%x, error code %d",
                          this, *numReadPtr, dsErrno,
                          ds_sock_error_code);
  }
  return dsErrno;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (TRUE == GetStrongRef())
  {
    ...
    switch (eventInfoPtr->eventGroup)
    {
      case Event::EVENT_GROUP_NETWORK:
      {
        ...
        switch (eventInfoPtr->eventMask)
        {
          ...
          {
            /*------------------------------------------------------------------
              1. When Prefix update or iface addr family changes always 
                 send TCP RST
              2. If IP address change event is received without HO, 
                 send TCP RST
              3. If IP address change event(v6 sockets) is received because of HO
                 a. If the IP address to which socket is bound is same 
                    than the IP address of IFACE(SRAT IP address) send
                    TCP RST
                 b. If the IP address to which socket is bound is different 
                    than the IP address of IFACE(SRAT IP address) ignore 
                    this event (e.g IMS binding to HLOS address for hybrid IMS)
            ------------------------------------------------------------------*/
            routingCachePtr = PS_RT_META_GET_ROUTING_CACHE( rtMetaInfoPtr);
            routingCache    = reinterpret_cast <int32> ( routingCachePtr);
            ...
            if (eventInfoPtr->handle == routingCache ||
                SocketIPSec::IsHandleInIfaceList( eventInfoPtr->handle,
                                                  rtMetaInfoPtr))
            {
              if ((ds::AddrFamily::QDS_AF_INET6 == platformSockPtr->GetFamily()) &&
			  	    (IFACE_ADDR_CHANGED_EV == eventInfoPtr->eventMask))
              {
                if (AEE_SUCCESS != GetIndexFromEvent(SocketEvent::QDS_EV_WRITE,
                                                     &writeEventIndex))
                {
                  //Fallback to legacy behavior if we cant get write evnt idx
                  ProcessNetworkConfigChangedEvent( QDS_EIPADDRCHANGED);				  
                  ds_sock_error_code = DSS_ERROR_GET_INDEX_FROM_EVENT_FAILED;
                  break;
            }
                  dsErrno = platformSockPtr->GetSockName( &localSockAddr);
                  if (AEE_SUCCESS != dsErrno)
                  {
                    ds_sock_error_code = DSS_ERROR_GET_SOCK_NAME_FAILED;
            break;
          }
                  ...
                  if (AEE_SUCCESS != dsErrno)
          {
                  ...
                  if (PS_IN6_ARE_ADDR_EQUAL(tmpV6LocalSockAddr.addr,
                                             &eventInfoPtr->eventInfo.ip_addr.addr.v6))
            {
                    ProcessNetworkConfigChangedEvent( QDS_EIPADDRCHANGED);
                  }
				  else 
				  {
				      LOG_MSG_INFO1_2("SID DBG"
                     "Processing ev %d , sock 0x%x Dont send RST",
                     eventInfoPtr->eventMask, this);
				  }
                }
                else
                {
          ...
          case FLOW_ACTIVATED_EV:
          {
            ...
            else if (eventInfoPtr->handle == origFltrResult)
            {
              ...
              {
                ds_sock_error_code = DSS_ERROR_USE_ORIG_FLTR_RESULT_FAILED;
                break;
              }
            }
          ...
          case FLOW_SUSPENDED_EV:
          {
            ...
            if (eventInfoPtr->handle == psFlowHandle)
            {
              ...
              {
                ds_sock_error_code = DSS_ERROR_RT_META_INFO_UPDATE_FAILED;
                break;
              }
            }
          ...
          case PHYS_LINK_DOWN_EV:
          {
            ...
            if (eventInfoPtr->handle == physLinkHandle)
            {
              ...
              if (psFlowHandle != defaultPSFlowHandle)
              {
                ...
                {
                  ds_sock_error_code = DSS_ERROR_NOT_FWDING_DATA_TO_DEFAULT_FLOW;
                }
              }
              else
              {
    ...
    if(ds_sock_error_code != DSS_ERROR_MIN)
    {
      LOG_MSG_FUNCTION_EXIT_2("Socket::Process(): "
                              "sock 0x%x error code %d", this, ds_sock_error_code);
    }
  } /* if (TRUE == GetStrongRef()) */
...
throw()
{
  ds::ErrorType  dsErrno;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  LOG_MSG_INFO3_3("Socket::QueryInterface(): "
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  -------------------------------------------------------------------------*/
  DS_SOCK_SOCKET_ASSERT(0 != _networkCtlPtr);
  ...
  DS_SOCK_SOCKET_ASSERT( 0 != platformSockFactoryPtr);
  ...
  -------------------------------------------------------------------------*/
bail:
  LOG_MSG_FUNCTION_EXIT_3("Socket::Init(): "
                          "sock 0x%x err 0x%x error code %d",
                          this, dsErrno, ds_sock_error_code);
  return dsErrno;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_QTRACE_HFT_MED(PS_TAG_DSSOCK, PS_SYS_DEFAULT_SUBS,
                         "Socket::Destructor(): Sock 0x%x", this);
  ...
  DS_SOCK_SOCKET_ASSERT( 0 != sockFactoryPtr);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
             ...
             eventBitMask[writeEventIndex])) && isDoS == false)
  {
    ...
    {
      dsErrno = AEE_EWOULDBLOCK;
      ds_sock_error_code = DSS_ERROR_PHYS_LINK_HANDLE_NULL;
      ResetRoutingCache(true);
      goto bail;
    }
    ...
    {
      ds_sock_error_code = DSS_ERROR_PHYS_LINK_BRING_UP_CMD_FAILED;
      goto bail;
    }
  ...
  -------------------------------------------------------------------------*/
bail:
  LOG_MSG_FUNCTION_EXIT_4("Socket::HandleNonZeroWriteEventBitMask(): "
                          "sock 0x%x ev mask 0x%x, returning 0x%x, error code %d",
                          this, eventBitMask[writeEventIndex], dsErrno,
                          ds_sock_error_code);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (0 == newRtMetaInfoPtr)
  {
    ...
    -----------------------------------------------------------------------*/
    dsErrno = AEE_EWOULDBLOCK;
    ds_sock_error_code = DSS_ERROR_ROUTE_META_INFO_NULL;
    goto bail;
  }
  ...
  if (AEE_SUCCESS != dsErrno)
  {
    ds_sock_error_code = DSS_ERROR_UPDATE_PKT_INFO_FAILED;
    goto bail;
  }
  ...
  if (AEE_SUCCESS != dsErrno)
  {
    ds_sock_error_code = DSS_ERROR_SET_RT_META_INFO_FAILED;
    goto bail;
  }
...
bail:
  ...
  LOG_MSG_FUNCTION_EXIT_3("Socket::RoutePacket(): "
                          "sock 0x%x err 0x%x, error code %d",
                          this, dsErrno, ds_sock_error_code);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
      ...
      PS_IN6_IS_ADDR_LINKLOCAL( v6RemoteAddrPtr->addr))
  {
    if (0 == v6RemoteAddrPtr->scopeId)
    {
      ...
      ds_sock_error_code = DSS_ERROR_SCOPE_ID_NOT_SPECIFIED;
      goto bail;
    }
    ...
    {
      dsErrno = QDS_EINVAL;
      ds_sock_error_code = DSS_ERROR_INVALID_IFACE_HANDLE;
      goto bail;
    }
  }
  ...
  if (AEE_SUCCESS != dsErrno)
  {
    ds_sock_error_code = DSS_ERROR_FILL_PKT_INFO_FAILED;
    goto bail;
  }
  ...
  -------------------------------------------------------------------------*/
bail:
  LOG_MSG_FUNCTION_EXIT_4("Socket::GeneratePktInfo(): "
                          "sock 0x%x scope id %d returning 0x%x error code %d",
                          this, v6RemoteAddrPtr->scopeId, dsErrno,
                          ds_sock_error_code);
  return dsErrno;
  ...
  if (true == PS_IN6_IS_ADDR_UNSPECIFIED( tmpV6LocalSockAddr.addr))
  {
        ...
        false == PS_IN6_IS_ADDR_V4MAPPED( ifaceIPAddr))
    {
          ...
          false == PS_IN6_IS_ADDR_LINKLOCAL( v6RemoteAddrPtr->addr))
      {
        ...
        ds_sock_error_code = DSS_ERROR_IP_ADDR_IN_INVALID_STATE;
        goto bail;
      }
...
bail:
  LOG_MSG_FUNCTION_EXIT_3("Socket::UpdateSrcAddrInPktInfo(): "
                          "sock 0x%x returning 0x%x error code",
                          this, dsErrno, ds_sock_error_code);
  return dsErrno;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (AEE_SUCCESS != dsErrno)
  {
    ...
    ds_sock_error_code = DSS_ERROR_CREATE_SIGNALFACTORY_FAILED;
    goto bail;
  }
  ...
  {
    dsErrno = AEE_ENOMEMORY;
    ds_sock_error_code = DSS_ERROR_SIGNAL_FAILED;
    goto bail;
  }
  ...
  if (AEE_SUCCESS != dsErrno)
  {
    ds_sock_error_code = DSS_ERROR_EVENT_REGISTRATION_FAILED;
    goto bail;
  }
...
bail:
  ...
  DS_UTILS_RELEASEIF(signalFactoryPtr);
  LOG_MSG_FUNCTION_EXIT_3("Socket::RegNetworkStateChange(): "
                          "sock 0x%x err 0x%x error code %d"
                          , this, dsErrno, ds_sock_error_code);
  return dsErrno;
...
bail:
  ...
  LOG_MSG_FUNCTION_EXIT_3("Socket::EventCallBackIfaceUp: "
                        "sock 0x%x returns 0x%x error code %d"
                        , sockPtr, dsErrno, ds_sock_error_code);
  return;
...
bail:
  sockPtr->critSectPtr->Leave();
  LOG_MSG_FUNCTION_EXIT_3("Socket::EventCallBackIfaceDown: "
                        "sock 0x%x returns -x%x error code %d"
                        , sockPtr, dsErrno, ds_sock_error_code);
  return;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (0 != platformSockPtr)
  {
    ...
    if (AEE_SUCCESS != NetPlatform::DeleteIfaceHandleFromRouteScope( &routeScope,
                                                                 routingCache))
    {
      ds_sock_error_code = DSS_ERROR_DELETE_FROM_ROUTESCOPE_FAILED;
      goto bail;
    }
...
bail:
  critSectPtr->Leave();
  LOG_MSG_FUNCTION_EXIT_2("Socket::DeleteRoutinCacheFromRoutScope: "
                          "sock 0x%x error code %d",
                          this, ds_sock_error_code);
  return;
...
{
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  DS_SOCK_SOCKET_ASSERT(0 != eventIndexPtr);
