...
$Header: //components/rel/data.mpss/3.5..2.1/interface/dssock/src/ds_Sock_NetworkCtl.cpp#2 $
...
#define DS_SOCK_NETCTL_ASSERT(expression) \
         ds_sock_netctl_assert_wrapper(__LINE__, expression)
...
===========================================================================*/
static void ds_sock_netctl_assert_wrapper
(
...
bail:
  LOG_MSG_FUNCTION_EXIT_3( "NetworkCtl::SetNetPolicy(): "
                           "obj 0x%x, newPolicyPtr 0x%x, result 0x%x",
                           this, newPolicyPtr, result);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_INFO3_3("NetworkCtl::UpdateRouteScope(): "
...
)
{
  ...
  if (AEE_SUCCESS != dsErrno)
  {
    ds_sock_error_code = DSS_ERROR_GET_POLICY_FAILED;
    goto bail;
  }
...
bail:
  ...
  LOG_MSG_FUNCTION_EXIT_2("NetworkCtl::UpdateNetwork(): "
                          "dsErrno 0x%x, error code %d",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    policyPtrNotNull = true;
    result = QDS_EOPNOTSUPP;
  }

  LOG_MSG_FUNCTION_EXIT_4( "NetworkCtl::SetNetwork(): "
                           "obj 0x%x, Net obj 0x%x policyPtrNotNull %d result 0x%x",
                            this, dsNetObjPtr, policyPtrNotNull,
                            result);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (AEE_SUCCESS != dsErrno)
  {
    ...
    ds_sock_error_code = DSS_ERROR_CREATE_INSTANCE_FAILED;
    goto bail;
  }
  ...
  if (AEE_SUCCESS != dsErrno)
  {
    ds_sock_error_code = DSS_ERROR_IFACE_UP_EVENT_REGISTRATION_FAILED;
    goto bail;
  }
  ...
  if (AEE_SUCCESS != dsErrno)
  {
    ds_sock_error_code = DSS_ERROR_IFACE_DOWN_EVENT_REGISTRATION_FAILED;
    goto bail;
  }
...
bail:
  ...
  DS_UTILS_RELEASEIF(ifaceDownSignalCtlPtr);
  DS_UTILS_RELEASEIF(signalFactoryPtr);
  LOG_MSG_FUNCTION_EXIT_3("NetworkCtl::RegNetowrkStateChangedEvent(): "
                          " NetworkCtl obj 0x%x dsErrno 0x%x, error code %d",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (AEE_SUCCESS != dsErrno)
  {
    ds_sock_error_code = DSS_ERROR_NETWORK_STATE_CHANGE_FAILED;
    goto bail;
  }
  ...
  -------------------------------------------------------------------------*/
bail:
  LOG_MSG_FUNCTION_EXIT_2("NetworkCtl::RegSignal2Event(): "
                          "returns dsErrno 0x%x, error code %d",
                          dsErrno, ds_sock_error_code);
  return dsErrno;
