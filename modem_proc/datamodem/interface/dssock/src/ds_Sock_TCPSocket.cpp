...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/dssock/src/ds_Sock_TCPSocket.cpp#2 $
...
#define DS_SOCK_TCP_ASSERT(expression) \
         ds_sock_tcp_assert_wrapper(__LINE__, expression)
...
===========================================================================*/
static void ds_sock_tcp_assert_wrapper
(
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    ds_sock_error_code = DSS_ERROR_LISTEN_OPERATION_FAILED;
    goto bail;
  }
...
bail:
  critSectPtr->Leave();
  LOG_MSG_FUNCTION_EXIT_3("TCPSocket::Listen(): "
                          "returns sock 0x%x err 0x%x, error code %d",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (AEE_SUCCESS != dsErrno)
  {
    ds_sock_error_code = DSS_ERROR_ACCEPT_OPERATION_FAILED;
    goto bail;
  }
  ...
  if (AEE_SUCCESS != dsErrno)
  {
    ds_sock_error_code = DSS_ERROR_CLONE_SOCKET_FAILED;
    goto bail;
  }
  ...
  DS_SOCK_TCP_ASSERT( 0 != sockFactoryPtr);
...
bail:
  ...
  critSectPtr->Leave();
  LOG_MSG_FUNCTION_EXIT_3("TCPSocket::Accept(): "
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  -------------------------------------------------------------------------*/
bail:
  LOG_MSG_ERROR_4("TCPSocket::WriteDSMChain(): "
                  "sock 0x%x dsErrno 0x%x Write ev bit mask 0x%x error code %d",
                  this, dsErrno, eventBitMask[writeEventIndex], ds_sock_error_code);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (AEE_EWOULDBLOCK == dsErrno)
  {
    ...
    {
      dsErrno = QDS_ENETDOWN;
      ds_sock_error_code = DSS_ERROR_NULL_ROUTING_CACHE;
    }
  }
  ...
  -------------------------------------------------------------------------*/
bail:
  LOG_MSG_ERROR_4("TCPSocket::ReadDSMChain(): "
                  "sock 0x%x dsErrno 0x%x Read ev bit mask 0x%x error code %d", 
                  this, dsErrno, eventBitMask[readEventIndex], ds_sock_error_code);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_FUNCTION_EXIT_2("TCPSocket::IsConnectSupported(): "
                          "sock 0x%x status %d", this, status);
  return status;
