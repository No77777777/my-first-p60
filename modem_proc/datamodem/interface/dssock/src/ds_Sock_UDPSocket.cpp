...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/dssock/src/ds_Sock_UDPSocket.cpp#2 $
...
#define DS_SOCK_UDP_ASSERT(expression) \
         ds_sock_udp_assert_wrapper(__LINE__, expression)
...
===========================================================================*/
static void ds_sock_udp_assert_wrapper
(
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (AEE_SUCCESS != dsErrno)
  {
    ds_sock_error_code = DSS_ERROR_SENDTO_DSM_CHAIN_FAILED;
    goto bail;
  }
...
bail:
  ...
  critSectPtr->Leave();
  LOG_MSG_ERROR_4("DUPSocket::SendToDSMChain: "
                  "sock 0x%x event bit mask 0x%xerr 0x%x error code %d",
                  this, eventBitMask[writeEventIndex], dsErrno, ds_sock_error_code);
  return dsErrno;
...
bail:
  critSectPtr->Leave();
  LOG_MSG_ERROR_5("UDPSocket::RecvFromDSMChain(): "
                  " sock 0x%x flags %d event mask 0x%x err 0x%x error code %d",
                  this, flags, eventBitMask[readEventIndex], dsErrno,
                  ds_sock_error_code);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    dsErrno = QDS_EINVAL;
    ds_sock_error_code = DSS_ERROR_NULL_MCAST_HANDLE;
    goto bail;
  }
#endif // FEATUTE_DATA_PS_MCAST
  ...
  if (ds::Net::INVALID_IFACE_ID == ifaceId)
  {
    ...
    if (0 == networkCtlPtr)
    {
      ...
      ds_sock_error_code = DSS_ERROR_NULL_NETWORK_CTL_PTR;
      goto bail;
    }
...
bail:
  critSectPtr->Leave();
  LOG_MSG_ERROR_4("UDPSocket::AddIPMembership(): "
                  "sock 0x%x netCtl obj 0x%x err 0x%x error code %d",
                  this, networkCtlPtr, dsErrno, ds_sock_error_code);
  return dsErrno;
...
bail:
  critSectPtr->Leave();
  LOG_MSG_ERROR_5("UDPSocket::DropIPMembership(): "
                  "Sock 0x%x  ifaceid %d from netCtl obj 0x%x, err 0x%x error code %d",
                  this, ifaceId, networkCtlPtr, dsErrno, ds_sock_error_code);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_SOCK_UDP_ASSERT( 0 != dosAckEventInfoPtr);
