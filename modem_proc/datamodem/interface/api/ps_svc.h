...
$Header: //components/rel/data.mpss/3.5..2.1/interface/api/ps_svc.h#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $

when        who    what, where, why
--------    ---    ----------------------------------------------------------
07/15/15    ds     Handler timer expiration in PS task
11/17/14    skc    Removing usage of PS_RX_UW_FMC_SIGNAL
...
/*--------------------------------------------------------------------------
  This is the max signal enum value allowed for certain signals that are
  registered with external clients
--------------------------------------------------------------------------*/
#define PS_SIG_REG_EXT_CLNT_MAX_VAL 31
...
typedef enum
{
  PS_CMD_Q_SIGNAL         = 0,    /* PS command Q signal           */
  PS_DPM_DL_SIGNAL_0,
  PS_DPM_DL_SIGNAL_1,
  PS_DPM_DL_SIGNAL_2,
  PS_DPM_DL_SIGNAL_3,
  ...
  PS_DPM_DL_SIGNAL_6,
  PS_DPM_DL_SIGNAL_7,
  PS_DPM_UL_MBIM_SIG,             /* DPM UL MBIM signal            */
  PS_REV_IP_TRANSPORT_RX_DATA_Q_SIGNAL, /* Reverse IP Transport --> PS data sig */
  PS_SOCKET_RX_Q_SIGNAL,          /* Socket API has data to send   */

  PS_PPP_UM_SN_RX_SIGNAL,         /* Um PPP Rx data signal         */
  PS_RM_RX_SIGNAL,                /* Rm PPP RX data signal         */
  PS_RM_2_RX_SIGNAL,              /* rx signal for second RM device */ 

  PS_LO_TX_DATA_Q_SIGNAL,         /* loopback data q signal        */
  PS_DPM_UL_LEGACY_SIG,           /* DPM UL legacy signal          */

  PS_NV_CMD_SIGNAL,               /* NV cmd event signal           */
  PS_UIM_CMD_SIGNAL,
  PS_PPP_UM_AN_RX_SIGNAL,         /* HDR AN AUTH PPP RX            */

  PS_3GPP_RMSM_AT_UM_RX_Q_SIGNAL, /* RMSM AT UM data TX signal     */
  PS_3GPP_LO_B_UM_RX_Q_SIGNAL,    /* 3GPP Loopback mode B V4 Rx Sig   */
  PS_3GPP_LO_B_UM_RX_Q_SIGNAL_1,  /* 3GPP Loopback mode B V6 Rx Sig   */

  PS_DPM_DPL_SIGNAL,              /**< DPL packets available Signal  */
  PS_DPM_DPL_DROPPED_PKT_SIGNAL,  /**< Dropped packets available Signal  */
#ifdef FEATURE_LAPP_UFS
  #error code not present
#endif /* FEATURE_LAPP_UFS */

  /*--------------------------------------------------------------------------
    Note: Below 3 signals needs to be within enum value < 32 since they will
    be used for registering with external clients who require them to be < 32
  ---------------------------------------------------------------------------*/
  PS_DOG_HB_REPORT_SIGNAL,        /* Dog heart beat report signal  */
  PS_DPL_DIAG_SIGNAL,             /* Incoming TCP data signal      */
#ifdef TEST_FRAMEWORK
  #error code not present
#endif /* TEST_FRAMEWORK */

  PS_SIG_MASK_CHANGE_SIGNAL,      /* Indicates sig mask has changed*/

  PS_3GPP_UM_RX_Q_SIGNAL_0,       /* 3GPP Bearer 0 signal          */
  PS_3GPP_UM_RX_Q_SIGNAL_1,       /* 3GPP Bearer 1 signal          */
  PS_3GPP_UM_RX_Q_SIGNAL_2,       /* 3GPP Bearer 2 signal          */
  PS_3GPP_UM_RX_Q_SIGNAL_3,       /* 3GPP Bearer 3 signal          */
  PS_3GPP_UM_RX_Q_SIGNAL_4,       /* 3GPP Bearer 4 signal          */
  PS_3GPP_UM_RX_Q_SIGNAL_5,       /* 3GPP Bearer 5 signal          */
  PS_3GPP_UM_RX_Q_SIGNAL_6,       /* 3GPP Bearer 6 signal          */
  PS_3GPP_UM_RX_Q_SIGNAL_7,       /* 3GPP Bearer 7 signal          */

  PS_QTF_SYNC_SIGNAL,             /* PS QTF synchronization signal */
  PS_3GPP_EMBMS_RX_Q_SIGNAL,      /**< 3GPP eMBMS DL Data Signal   */
  PS_3GPP_LB_SIGNAL_0,            /**< 3GPP loopback signal        */


#ifdef FEATURE_DATA_PS_SLIP
  PS_SLIP_RX_DATA_Q_SIGNAL,       /* SLIP Rx data q signal         */
...
typedef enum
{
 ...
 /*-------------------------------------------------------------------------
  ...
  ,PS_DHCP6_CLIENT_T1_TIMER_CMD      = 96  /* Process T1 timer expiration   */

  ,PS_DHCP6_CLIENT_T2_TIMER_CMD      = 97  /* Process T2 timer expiration   */

  ,PS_DHCP6_CLIENT_LEASE_TIMER_CMD   = 98  /* Process Lease timer expiration   */

  ,PS_DHCP6_CLIENT_TIMER_EXPIRED_CMD = 99  /* Process DHCPv6 core timer expiration   */

  ,PS_RSVP_SOCKET_EVENT_CMD          = 100  /* RSVP socket event call back */

  /* Commands for IGMP/MLD */
  ,PS_MCAST_SEND_REPORT_CMD          = 101 /* Send IGMP/MLD JOIN Message     */

  ,PS_MCAST_SEND_LEAVE_CMD           = 102 /* Send IGMP/MLD LEAVE Message    */

  ,PS_DSS_PING_SEND_PING_CMD         = 103 /* Send ICMP ECHO_REQUEST */

  ,PS_DSS_PING_NET_CB_CMD            = 104 /* Ping network callback */

  ,PS_DSS_PING_SOCK_CB_CMD           = 105 /* Ping socket callback */

  ,PS_DSS_PING_RSP_TIMEOUT_CB_CMD    = 106 /* Ping response timeout cb */

  ,PS_DSM_MEM_EVENT_CTRL_CMD         = 107 /* Post WRITE_EV on sockets      */

  /*-------------------------------------------------------------------------
     Commands for DSS Net Manager
  -------------------------------------------------------------------------*/
  ,PS_DSS_NET_MGR_NET_UP_CMD         = 108 /* Net mgr net up */
  ,PS_DSS_NET_MGR_NET_DOWN_CMD       = 109 /* net mgr net down cmd          */

  /*-------------------------------------------------------------------------
    Domain Name System resolver command.
  -------------------------------------------------------------------------*/
  ,PS_DNS_RESOLVE_CMD                = 110 /* To start the DNS resolver     */
  ,PS_DNS_IO_MGR_SOCK_EVENT_CMD      = 111 /* IO manager sock event cmd     */
  ,PS_DNS_RESOLVER_TIMEOUT_CMD       = 112 /* Resolver timeout event        */
  ,PS_DNS_DELETE_SESSION_CMD         = 113 /* Delete session command        */

  /* Commands for MCAST */
  ,PS_MCAST_JOIN_CMD                 = 114 /* Send MCAST JOIN Message */

  ,PS_MCAST_LEAVE_CMD                = 115 /* Send MCAST LEAVE Message */

  /*-------------------------------------------------------------------------
    MIPv6 Registration State Machine commands
  -------------------------------------------------------------------------*/
  ,PS_MIP6_RSM_REG_CMD               = 116  /* Trigger registration        */
  ,PS_MIP6_RSM_DEREG_CMD             = 117  /* Trigger de-registration     */
  ,PS_MIP6_RSM_REG_SUCCESS_CMD       = 118  /* Reg success                 */
  ,PS_MIP6_RSM_REG_FAIL_CMD          = 119  /* Reg failure                 */
  ,PS_MIP6_RSM_CLOSE_CMD             = 120  /* Tear-down the RSM           */

  /*-------------------------------------------------------------------------
    Instance statistics logging commands
   ------------------------------------------------------------------------*/
  ,PS_STAT_INST_GET_DESC_CMD         = 121  /* Get instance description    */
  ,PS_STAT_INST_GET_STAT_CMD         = 122  /* Get instance stats          */
  ,PS_STAT_INST_RESET_STAT_CMD       = 123  /* Reset instance statistics   */

  /*-------------------------------------------------------------------------
    SIM event handling commands
   ------------------------------------------------------------------------*/
  ,PS_SIM_EVENT_HANDLER_CMD          = 124  /* Handle SIM_INIT_COMPLETED ev*/

  /*-------------------------------------------------------------------------
    EAP/PPP-MMGSDI registration events handling
  -------------------------------------------------------------------------*/
  ,PS_EAP_CLNT_REG_CMD               = 125  /* EAP-MMGSDI client reg       */
  ,PS_EAP_SESSION_EVT_CB_CMD         = 126  /* EAP-MMGSDI session reg      */
  ,PS_PPP_CLNT_REG_CMD               = 127  /* PPP-MMGSDI client reg       */
  ,PS_PPP_EAP_TASK_CMD               = 128  /* PPP/EAP MMGSDI task switch  */

  /*-------------------------------------------------------------------------
    DSSOCK/DSNET commands to process iface, flow, phys link, DOS_ACK events.
  -------------------------------------------------------------------------*/
  ,PS_DSSOCK_PROCESS_GENERIC_EVENT_CMD  = 129
  ,PS_DSSOCK_PROCESS_DOS_ACK_EVENT_CMD  = 130
  ,PS_DSNET_PROCESS_GENERIC_EVENT_CMD   = 131

 /*-------------------------------------------------------------------------
    DHCP MULTIPLE IP MGR event handling commands
   ------------------------------------------------------------------------*/
  ,PS_DHCP_SERVER_MGR_START_CMD         = 132 /* Multiple IP DHCP mgr start */
  ,PS_DHCP_SERVER_MGR_STOP_CMD          = 133 /* Multiple IP DHCP mgr stop */
  ,PS_DNS_PROXY_SOCK_EVENT_CMD          = 134  /* DNS Proxy Socket ev cmd   */
  ,PS_DNS_PROXY_ENABLE_CMD              = 135  /* DNS Proxy Enable command  */
  ,PS_DNS_PROXY_DISABLE_CMD             = 136  /* DNS Proxy Disable command */
  ,PS_DNS_PROXY_IFACE_EVENT_CMD         = 137  /* DNS Proxy Iface ev command*/

  /*-------------------------------------------------------------------------
    PS UTILS AKA commands
  -------------------------------------------------------------------------*/
  ,PS_UTILS_AKA_CMD                     = 138  /* PS UTILS AKA CMD handler */
   
  /*-------------------------------------------------------------------------
    PPP NCP commands (contd..)
  -------------------------------------------------------------------------*/
  ,PS_PPP_NCP_RESYNC_CMD                = 139 /* resync an NCP instance    */ 
  ,PS_PPP_NCP_ABORT_CMD                 = 140 /* abort an NCP instance     */ 
  ,PS_NAT_DELETE_DMZ_CMD                = 141 /* Delete DMZ command        */

  /*-------------------------------------------------------------------------
    PPP DEVICE STOP command
  -------------------------------------------------------------------------*/
  ,PS_PPP_DEVICE_STOP_CMD               = 142 /* PPP Device STOP command   */
  ,PS_START_BCAST_GARP                  = 143 /* Start broadcasting GARP   */

  /*-------------------------------------------------------------------------
    Commands to flow control ps_iface, ps_flow, and ps_phys_link
  -------------------------------------------------------------------------*/
  ,PS_IFACE_FLOW_CTRL_CMD               = 144
  ,PS_FLOW_FLOW_CTRL_CMD                = 145
  ,PS_PHYS_LINK_FLOW_CTRL_CMD           = 146

  /*-------------------------------------------------------------------------
    PS SYS commands to process sys events.
  -------------------------------------------------------------------------*/
  ,PS_SYS_EVENT_CMD_HANDLER             = 147
  
  /*-------------------------------------------------------------------------
    PPP Trigger Link Status Determination (LSD) command
  -------------------------------------------------------------------------*/
  ,PS_PPP_TRIGGER_LSD_CMD               = 148 /* PPP Trigger LSD command   */

  /*-------------------------------------------------------------------------
    Stateless DHCPv6 Server MGR event handling commands
  -------------------------------------------------------------------------*/
  ,PS_DHCP6_SL_SERVER_MGR_START_CMD     = 149 /* Stateless DHCP6 mgr start */
  ,PS_DHCP6_SL_SERVER_MGR_STOP_CMD      = 150 /* Stateless DHCP6 mgr stop */
  ,PS_DHCP6_SL_SERVER_SOCK_EVENT_CMD    = 151 /* Send notification of
                                                 socket event */
  /*-------------------------------------------------------------------------
    More Domain Name System resolver commands.
  -------------------------------------------------------------------------*/
  ,PS_DNS_RESOLVER_IFACE_DOWN_CMD         = 152 /* Iface Down event received */
  ,PS_DNS_RESOLVER_IFACE_ADDR_CHANGED_CMD = 153 /* Iface Address Changed event
                                                   received */
  ,PS_DNS_RESOLVER_DELETE_INSTANCE_CMD    = 154 /* Delete the resolver */

  /*-------------------------------------------------------------------------
    Send immediate MIP registration command
   ------------------------------------------------------------------------*/
  ,PS_MIP_SEND_IMMED_REG_CMD            = 155

  /*-------------------------------------------------------------------------
    Would be posted once DHCP invokes socket creation and bind APIs
  -------------------------------------------------------------------------*/
  ,PS_DHCP_SERVER_STARTED               = 156

  /*-------------------------------------------------------------------------
    Domain Name System flush cache commands.
  -------------------------------------------------------------------------*/
  ,PS_DNS_FLUSH_CACHE_CMD               = 157 /* Flush cache command       */
  ,PS_DNS_FLUSH_CACHE_ENTRY_CMD         = 158 /* Flush cache-entry command */
  ,PS_AUDIO_OFFLOAD_CLIENT_SIO_EVENT_CMD = 1659/* SIO notification */
  ,PS_AUDIO_OFFLOAD_CLIENT_SOCKET_EVENT_CMD = 160
  ,PS_AUDIO_OFFLOAD_CLIENT_DSM_EVENT_CMD = 161
 /*-------------------------------------------------------------------------
    MCFG NV refresh command.
  -------------------------------------------------------------------------*/
  ,PS_UTILS_NV_REFRESH_EVENT_CMD            = 162  /*MCFG NV refresh */
 /*-------------------------------------------------------------------------
    IPDC logging command
  -------------------------------------------------------------------------*/
  ,PS_COMP_LOGGING_HELPER_CMD               = 163  /* Comp Logging Helper command */

  /*-------------------------------------------------------------------------
    FTP cmds
  -------------------------------------------------------------------------*/
  ,PS_FTP_SOCK_CMD =   164                 /* FTP socket cmds */
  ,PS_FTP_TIMERS_CMD =  165                 /* FTP Timers */

  /*-------------------------------------------------------------------------
    Certificate utilities
  -------------------------------------------------------------------------*/
  ,PS_CERT_UTIL_CMD = 166   /*Certificate utility cmds*/

  /*-------------------------------------------------------------------------
    IFACE GO NULL CMD
  -------------------------------------------------------------------------*/
  ,PS_IFACE_GO_NULL_CTRL_CMD = 167 
  ,PS_IFACE_LOW_LATENCY_EVENT_CMD =  168    /* Low latency event command */ 
  ,PS_TIMER_CALLBACK_EXT_CMD     = 169 /* PS Timer expired call cback
                                             in PS context.                */
  /*-------------------------------------------------------------------------
    Remote Socket CMD
  -------------------------------------------------------------------------*/
  ,PS_REMOTE_SOCKET_CMD     = 170 /* Remote Socket commands                */
...
===========================================================================*/
#define PS_WAIT    ps_wait
extern rex_sigs_type* ps_wait
( 
