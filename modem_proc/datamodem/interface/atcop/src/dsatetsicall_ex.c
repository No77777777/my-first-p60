...
/*===========================================================================
  ...
  Notice that changes are listed in reverse chronological order.
   ...
   $Header: //components/rel/data.mpss/3.5..2.1/interface/atcop/src/dsatetsicall_ex.c#4 $ $DateTime: 2020/01/09 11:25:22 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
01/09/20   ks      Fixed issue with CGREG URC incorrect after service resume request
01/09/20   ks      Fixed issue where wrong call Id is passed for AT+CHLD=0 after AT+CHLD=2
...
#define PREV_RAC_MME    ph_ss_ms_val->dsat_net_reg_state.dsat_creg_reported.lte_rac_or_mme_code
  ...
  if(((DSAT_SS_CELL_INFO_MASK | DSAT_SS_SYS_MODE_MASK ) & changed_fields) &&
    ...
    if ( call_event->event_info.dsat_mode_info.info.gw_cs_call.call_ss_success )
    {
      switch ( call_event->event_info.sups_type )
      {
        ...
        case CM_CALL_SUPS_TYPE_HOLD_CALL:
          ...
          {
            if((call_da_val->was_hold_call_retrieved || call_da_val->was_waiting_call_connected) &&
               call_da_val->was_active_call_hold)
            {
...
)
{
  ...
  {
    call_da_val->etsicall_wait_call_present = FALSE;
    if(etsicall_cmd_pending[subs_id] == ((uint8)CMD_CHLD2 | (uint8)CMD_CONF))
    {
      call_da_val->was_waiting_call_connected = TRUE;
    }
  }
