...
INITIALIZATION AND SEQUENCING REQUIREMENTS
  None
   ...
   Copyright (c) 2003 - 2020 by Qualcomm Technologies Incorporated.
   All Rights Reserved.
...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/atcop/src/dsatsms_ex.c#4 $ $DateTime: 2020/08/25 05:57:04 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
08/25/20   ks      Resolving MT SMS always getting routed to SIM memory by 
                   providing EFS based fix.
08/25/20   sp      ATCOP changes to support new WMS callback
...
)
{
  ...
  else if( event == WMS_MSG_EVENT_SMSC_ADDR_CHANGE )
  {
    if(!(IS_VALID_SUBS_ID(event_ptr->status_info.as_id)))
    {
      DS_AT_MSG0_HIGH("Invalid subs id");
      return;
    }
...
)
{
  ...
  switch ( wms_event )
  {
    ...
    case WMS_MSG_EVENT_WRITE_STS_REPORT:
      ...
      break;
    case WMS_MSG_EVENT_SMSC_ADDR_CHANGE:
      atcop_event = SMS_MSG_EVENT_SMSC_ADDR_CHANGE;
      break;
    default:
      atcop_event = SMS_EVENT_NONE;
      break;
