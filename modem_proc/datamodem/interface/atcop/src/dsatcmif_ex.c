...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/atcop/src/dsatcmif_ex.c#8 $ $DateTime: 2021/02/12 10:11:56 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
08/26/20   ks      Fixing formatting issues in AT commands.
08/24/20   ks      Fixed CSSI issue where it wasn't displaying correctly
09/18/19   ks      Fixed issue where wrong call Id is passed for AT+CHLD=0 after AT+CHLD=2
11/05/18   ya      Added Rac info parameter to CGREG command.
...
)
{
  ...
  switch (call_event->event)
  {
    ...
    case CM_CALL_EVENT_SRVCC_COMPLETE_IND:
      ...
      break;
    case CM_CALL_EVENT_PROGRESS_EARLY_MEDIA:
	if ( (call_event->event_info.call_type == CM_CALL_TYPE_VOICE) ||
             (call_event->event_info.call_type == CM_CALL_TYPE_EMERGENCY))
        {
           call_da_val->call_info_before_connect.call_id = call_event->event_info.call_id;
           call_da_val->call_info_before_connect.info_type = call_event->event_info.dsat_mode_info.info_type;
        }
       break;
    default:
      break;
...
if ( dsatcmif_is_voice_call_type(event_ptr->call_type)
      ...
      ||( event == CM_CALL_EVENT_PROGRESS_EARLY_MEDIA)
     )
  {
      ...
      if( NULL != event_ptr )  /*lint !e774 */
      {
        ...
        switch (event_ptr->mode_info.info_type)
        {
          ...
          case CM_CALL_MODE_INFO_GW_CS:
            ...
            call_event->event_info.ss_info = event_ptr->mode_info.info.gw_cs_call.ss_info;
    ...
    case DSAT_CALL_DA_VALS:
     {
       ...
       da->was_waiting_call_connected = FALSE;
