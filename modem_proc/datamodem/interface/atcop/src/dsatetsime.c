...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/atcop/src/dsatetsime.c#5 $ $DateTime: 2020/08/24 10:46:49 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
07/10/19   sp      Fixed issue related to AT+CPAS
  ...
  if ((NULL != call_event_ptr) && (0 != call_event_ptr->event_info.dsat_cm_call_info_list.number_of_active_calls))
  {
    ...
    for (i=0; (i < call_event_ptr->event_info.dsat_cm_call_info_list.number_of_active_calls) && !done; i++)
    {
      ...
      case CM_CALL_STATE_CONV:
        ...
        done = ((call_event_ptr->event_info.dsat_cm_call_info_list.number_of_voice_calls >= 1) && (call_event_ptr->event_info.dsat_cm_call_info_list.info[i].call_type==CM_CALL_TYPE_VOICE))?TRUE:FALSE;
        break;    
...
)
{
  ...
  if (mmgsdi_result == MMGSDI_SUCCESS) 
  {
    ...
    dsatme_pending_mmgsdi_command[apps_id].is_pending = TRUE;
    dsatme_pending_mmgsdi_command[apps_id].cb_cmd     = CLCK_MMGSDI_PIN1_VERIFY;
...
)
{
  ...
  {
    dsatme_pending_mmgsdi_command[dsat_get_current_gw_apps_id(FALSE)].is_pending = TRUE;
    dsatme_pending_mmgsdi_command[dsat_get_current_gw_apps_id(FALSE)].cb_cmd     = req_submitter;
    return DSAT_ASYNC_CMD;
  }
  else
  {
...
)
{
  ...
  if (mmgsdi_status == MMGSDI_SUCCESS) 
  {
    ...
    dsatme_pending_mmgsdi_command[dsat_get_current_gw_apps_id(FALSE)].is_pending = TRUE;
    dsatme_pending_mmgsdi_command[dsat_get_current_gw_apps_id(FALSE)].cb_cmd     = CLCK_MMGSDI_PIN1_STATUS;
    return DSAT_ASYNC_CMD;
  }
  else
  {
...
#define GENERATE_QCSIMSTAT_RESPONSE(slot_id) \
  ...
  case MMGSDI_SESSION_CHANGED_EVT:
  {
    ...
    {
      if (TRUE == dsatme_pending_mmgsdi_command[apps_id].is_pending)
      {
        dsatme_pending_mmgsdi_command[apps_id].is_pending = FALSE;
        dsatme_pending_mmgsdi_command[apps_id].cb_cmd     = CB_NONE;
...
#define PREP_ICCID(val)\
  ...
  dsatme_pending_mmgsdi_command[dsat_get_current_gw_apps_id(FALSE)].is_pending = FALSE;
...
)
{
  ...
  if (tok_ptr->op == NA)
  {
    ...
    {\
      ...
      for ( j = 0; j < MAX_CMD_LIST; j++)\
      {\
        if( ( dsat_fwd_at_cmds[j].client_id != DSAT_INVALID_CLIENT_ID ) && \
          ( dsat_fwd_at_cmds[j].cmd_name[0] != '\0' ) ) \
        { \
          if ( 0 ==  dsatutil_strcmp_ig_sp_case( (const byte *)dsat_fwd_at_cmds[j].cmd_name, (const byte *)table.name) ) \
          { \
