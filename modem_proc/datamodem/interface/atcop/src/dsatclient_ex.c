...
DESCRIPTION
  ...
  Copyright (c) 2005 - 2015,2017 by Qualcomm Technologies Incorporated.
  All Rights Reserved.
...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/atcop/src/dsatclient_ex.c#2 $
    ...
    for (index1 = 0; index1 < fwd_at_cmd_ptr->num_valid_cmds; index1++)
    {
      ...
      for (index2 = 0; index2 < MAX_CMD_LIST; index2++)
      {
                    ...
                    (const byte *)dsat_fwd_at_cmds[index2].cmd_name)))
        {
          ...
          DS_ATCOP_ERROR_LOG_1("removed command at index =d",index2);
          break;
        }
      }
    }
...
)
{
  ...
  {
    DS_ATCOP_ERROR_LOG_1("Num_valid_cmds(=d) is greater than allowed length",
      num_valid_cmds);
    return FALSE;
  }
  ...
  for(cmd_loop = 0; cmd_loop < num_valid_cmds; cmd_loop++)
  {
    ...
    if(valid_cmd == FALSE)
    {
     DS_ATCOP_ERROR_LOG_0("Un-supported command received");
      return FALSE;
    }
