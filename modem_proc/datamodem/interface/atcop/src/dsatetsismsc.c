...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/atcop/src/dsatetsismsc.c#4 $ $DateTime: 2021/02/12 10:11:56 $ $Author: pwbldsvc $
...
{
  #define DSAT_CPMS_MEM3_F_NAME \
                        (char*)"/ds/atcop/atcop_cpms_mem3_index.txt"
  ...
  if ( tok_ptr->op == (NA|EQ|AR) || (tok_ptr->op == (NA|QU)) )
  {
    ...
    if ( tok_ptr->op == (NA|EQ|AR) )
    {
      ...
      if(mem3_type != (dsat_num_item_type)dsatutil_get_val(
           DSATETSI_EXT_ACT_CPMS_ETSI_IDX,subs_id,CPMS_MEM3,NUM_TYPE))
      {
        /* Whenever mem3 changes, write to the new value to the EFS.*/
        mem3_type = (dsat_num_item_type)dsatutil_get_val(
          DSATETSI_EXT_ACT_CPMS_ETSI_IDX,subs_id,CPMS_MEM3,NUM_TYPE);
        
        dsatutil_memscpy(file_path,DSAT_MAX_PATH_LEN,DSAT_CPMS_MEM3_F_NAME,strlen(DSAT_CPMS_MEM3_F_NAME) + 1);
