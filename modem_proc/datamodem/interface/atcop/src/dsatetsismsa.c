...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/atcop/src/dsatetsismsa.c#4 $ $DateTime: 2020/08/25 01:08:22 $ $Author: pwbldsvc $
...
01/10/20   ks      Added support for SMSC adress change event handler.
...
LOCAL dsat_result_enum_type smsc_addr_change_event_handler
(
  dsat_mode_enum_type mode, /* AT command mode */
  sms_event_info_s_type  *event_info   /* Event info ptr */
);
...
const msg_handler_tab_type dsatetsisms_msg_handler_table_entries[] = 
{
  ...
  { SMS_MSG_EVENT_SMSC_ADDR_CHANGE,   smsc_addr_change_event_handler            },
};
...
{
  wms_gw_template_s_type *smsc_templ_ptr; 
  boolean                is_csca = TRUE;
  byte                   sca_ptr[SCA_MAX_LEN] = {0};
  byte                   no_quotes[SCA_MAX_LEN] = {0};
  uint8                  type_of_addr = 0;
  ...
  dsat_string_item_type  *str_item;
  uint8                  local_error = 0;
  dsat_string_item_type  dsat_csca_tosca[CSCA_TOSCA_STR_MAX_LEN+1];
  dsat_mixed_param_val_type smsc_value[1]= {
  	{dsat_csca_tosca}
  };
  smsc_templ_ptr = &event_info->u.msg_status.u.gw_template;
  if ( (smsc_templ_ptr != NULL) && (smsc_templ_ptr->mask & WMS_GW_TEMPLATE_MASK_SC_ADDR) )
  {
    temp_ptr = sca_ptr;
    temp_ptr = dsatetsismsi_addr_to_str (
              smsc_templ_ptr->sc_addr,
              temp_ptr,
              &type_of_addr, 
              is_csca,
              (uint8)SCA_MAX_LEN
    ...
    *temp_ptr = '\0';
    ...
    str_item = (uint8 *)dsatutil_get_val(DSATETSI_EXT_CSCA_IDX,event_info->subs_id,0,MIX_STR_TYPE);
    (void) strlcpy( (char *)str_item,(const char *)no_quotes,CSCA_SCA_STR_MAX_LEN+1);

    str_item = (uint8 *)dsatutil_get_val(DSATETSI_EXT_CSCA_IDX,event_info->subs_id,1,MIX_STR_TYPE);
    (void) dsatutil_itoa((uint32) type_of_addr, smsc_value[0].string_item, 10);

    (void) strlcpy( (char *)str_item,(const char *)smsc_value[0].string_item,CSCA_TOSCA_STR_MAX_LEN+1);
    return DSAT_ASYNC_EVENT;
  } 
  else
  {
  ...
  bail:
    DS_AT_MSG1_ERROR("SMSC address event handler with local error %d",local_error);
    return DSAT_ASYNC_EVENT;
}
