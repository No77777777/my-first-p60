...
/*===========================================================================
  ...
  $PVCSPath:  L:/src/asw/MSM5200/data/vcs/dsatetsicall.c_v   1.7   12 Jul 2002 10:05:08   randrew  $
   $Header: //components/rel/data.mpss/3.5..2.1/interface/atcop/src/dsatetsicall.c#7 $ $DateTime: 2021/02/12 10:11:56 $ $Author: pwbldsvc $
...
10/30/18   ya      CREG URC Act value validation.
    ...
    if ( (uint8)etsicall_cmd_pending[subs_id] & (uint8)CMD_CONF )
    {
      if ( call_event->event_info.call_ss_success)
      {
        ...
        if ( clr_pending )
        {
          ...
          call_da_val->was_waiting_call_connected = FALSE;
          result = DSAT_OK;
        }
      } /* End successful */
      else
  ...
  if(convert_to_cscs)
  {
                                                                   ...
                                                                   num_of_chars) )
    {
      if(dsatutil_convert_chset( (const char *) dst_data,
                                 ALPHA_GSM,
                                 (const char *) dst_data,
                                 (dsati_chset_type)(dsat_num_item_type)dsatutil_get_val
                                           (DSATETSI_EXT_CSCS_IDX,0,0,NUM_TYPE),
                                 dst_size+1,
                                 TRUE) == NULL)
      {
