...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/atcop/src/dsatcmif.c#3 $ $DateTime: 2020/08/26 01:18:24 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
08/26/20   ks      Fixing formatting issues in AT commands.
11/05/18   ya      Fixed incorrect Cell-ID info update when moved from L->W.
...
LOCAL dsat_result_enum_type cmif_process_cm_attach_data
(
  const ds_at_cm_ss_event_type * event_ptr,        /* DS event pointer */
  dsat_stack_id_e_type           stack_id,
  sys_modem_as_id_e_type         subs_id
  ...
  switch (event_ptr->event)
  {
    case CM_SS_EVENT_SRV_CHANGED:
    {
      for ( stack_id = DSAT_STACK_MAIN; stack_id < event_ptr->number_of_stacks; stack_id++ )
      {
        ...
        {
           DS_AT_MSG1_HIGH("stack_id =%d", stack_id);
           switch(subs_mode)
           {
#ifdef FEATURE_SGLTE
              case DSAT_SUB_SGLTE:
              ...
              case DSAT_SUB_NO_SRV:
                 if ( ( event_ptr->event_info[stack_id].changed_fields & 
                      ...
                      ( DSAT_CM_REQ_ID == event_ptr->event_info[stack_id].cm_req_id ) )
                 {
                    result_tmp = cmif_process_cm_attach_data(event_ptr, stack_id,
                                                          subs_id);
#ifdef FEATURE_SGLTE
...
FUNCTION  CMIF_PROCESS_CM_ATTACH_DATA
