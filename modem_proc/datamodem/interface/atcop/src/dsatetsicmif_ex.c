...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/atcop/src/dsatetsicmif_ex.c#3 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
05/29/20   ks      atcop_cops_auto_mode.txt is implemented as subs based.
...
uint8 cops_no_mode_change[SYS_MODEM_AS_ID_MAX-1] = {0,0,0};
...
)
{
  ...
  case DSAT_COPS_MODE_AUTO:
    if(cops_no_mode_change[subs_id] == TRUE)
    {
    ...
    DS_AT_MSG2_HIGH("COPS setting Auto network registration %d act_mode_pref %d cops_no_mode_change",
          act_mode_pref,cops_no_mode_change[subs_id]);
         ...
         if( ph_ss_ms_val->dsatetsicall_network_list.cmd_idx == CMD_IDX_QCCOPS )
         {
           ...
           {
            DS_ATCOP_ERROR_LOG_0("cm_user_pref_update_req failed");
             return DSAT_ERROR;
           }
         }
  ...
  case  DSAT_COPS_MODE_MANAUTO:
    ...
    */    
    ...
    DS_AT_MSG2_HIGH("COPS setting manual network registration %d act_mode_pref %d cops_no_mode_change",act_mode_pref,cops_no_mode_change[subs_id]);
