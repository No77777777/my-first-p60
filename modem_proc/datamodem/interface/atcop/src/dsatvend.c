...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/atcop/src/dsatvend.c#8 $ $DateTime: 2021/02/12 10:11:56 $ $Author: pwbldsvc $
  
when       who     what, where, why
--------   ---     ----------------------------------------------------------
08/26/20   ks      Fixing formatting issues in AT commands.
08/24/20   ks      Added changes to AT$QCSQ issue.
4/17/20    sp      Added changes to resolve compilation OF IMS for Agatti
...
)
{
  ...
  if ( tok_ptr->op == (NA) )
  {
     ...
     * This command is applicable for GW RATs only.
     * There is no RSCP concept for LTE.
     * RSCP is not a valid parameter for GSM. RSSI is the only valid 
       parameter in GSM.
     */
    ...
    /* FEATURE_WCDMA_DATA_FOR_UI_BAR_DISPLAY is only applicable for WCDMA RAT*/
    if(curr_mode == DSAT_MODE_WCDMA)
    {
    ...
    {
      /* Report RSCP value as it is. Internally concluded to display invalid value*/
      res_buff_ptr->used = (word)snprintf(
                               (char*)res_buff_ptr->data_ptr,
                                res_buff_ptr->size,
                                "$QCSQ: %d,",
    ...
    {
      /* RSSI is the only applicable parameter for all other RATs except WCDMA.*/
      res_buff_ptr->used = (word)snprintf(
                             (char*)res_buff_ptr->data_ptr,
                              res_buff_ptr->size,
                              "%s%d",
...
)
{
  ...
  if ( (NA|EQ|AR) == tok_ptr->op )
  {
    ...
    switch( in_pref_val )
    {
        ...
        user_pref.mode_pref = CM_MODE_PREF_AUTOMATIC;
        break;
#endif /* FEATURE_DSAT_EXTENDED_CMD */
      case DSAT_PREFMODE_SYSMODE_CDMA:
        user_pref.mode_pref = CM_MODE_PREF_CDMA_ONLY;
        break;
      case DSAT_PREFMODE_SYSMODE_HDR:
        user_pref.mode_pref = CM_MODE_PREF_HDR_ONLY;
        break;
      case DSAT_PREFMODE_SYSMODE_HYBRID_CDMA_HDR:
        user_pref.mode_pref = CM_MODE_PREF_CDMA_HDR_ONLY;
    ...
    if ( FALSE == cm_is_valid_mode_pref(user_pref.mode_pref))
    {
...
)
{
  ...
  if (tok_ptr->op == NA)
  {
     ...
     nv_status = dsatutil_get_nv_item_per_subs( NV_MEID_I , &ds_nv_item,
...
)
{
  ...
  if ( tok_ptr->op == (NA|EQ|AR) )
  {
    if ( dsatparm_exec_param_cmd(mode, parse_table, tok_ptr, res_buff_ptr) != DSAT_ERROR )
    {
      ...
      set_drx_info.drx_coefficient = (sys_drx_cn_coefficient_s1_e_type)(dsat_num_item_type)
...
)
{
  ...
  if(tok_ptr->op == (NA|EQ|AR))
  {
    ...
    if(DSAT_QCCALLATTR_CALL_TYPE_VOICE ==
       (dsat_qccallattr_call_type_e_type)(dsat_num_item_type)dsatutil_get_val(DSAT_VENDOR_QCCALLATTR_IDX,0,0,NUM_TYPE) &&
       DSAT_QCCALLATTR_CALL_ATTR_TXRX != 
       (dsat_qccallattr_call_type_e_type)(dsat_num_item_type)dsatutil_get_val(DSAT_VENDOR_QCCALLATTR_IDX,0,1,NUM_TYPE))
    {
      DSATUTIL_SET_VAL(DSAT_VENDOR_QCCALLATTR_IDX,0,1,0,DSAT_QCCALLATTR_CALL_ATTR_TXRX,NUM_TYPE)
      result = DSAT_ERROR;
    }
...
{
  dsat_client_register_ims_srv(f_ptr,ims_data);
  return TRUE;
}/*~ FUNCTION dsat_client_register_ims_srv_ex */
