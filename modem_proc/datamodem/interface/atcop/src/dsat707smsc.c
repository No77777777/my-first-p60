...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/atcop/src/dsat707smsc.c#3 $ $DateTime: 2020/08/25 05:57:04 $ $Author: pwbldsvc $
...
--------   ---     ----------------------------------------------------------
05/29/20   ks      Resolving MT SMS always getting routed to SIM memory by 
...
{
#define SM_MEM 2
#define DSAT_QCPMS_MEM3_F_NAME \
                        (char*)"/ds/atcop/atcop_qcpms_mem3_index.txt"
    ...
    if ( tok_ptr->op == (NA|EQ|AR) )
    {
      ...
      */
      ...
      if(mem3_type != (int)dsatutil_get_val(
                        DSAT707_SMS_QCPMS_IDX,0,CPMS_MEM3,NUM_TYPE))
      {
        /* Whenever mem3 changes, write the new value to the EFS.*/
        mem3_type = (dsat_num_item_type)dsatutil_get_val(
                    DSAT707_SMS_QCPMS_IDX,0,CPMS_MEM3,NUM_TYPE);
        
        dsatutil_memscpy(file_path,DSAT_MAX_PATH_LEN,DSAT_QCPMS_MEM3_F_NAME,strlen(DSAT_QCPMS_MEM3_F_NAME) + 1);
        memset ((void*)buf, 0x0, sizeof(buf));
        buf[0] ='0' + mem3_type;

        dsatutil_efs_exist_file_write(file_path,
                                  buf,
                                  MCFG_FS_SUBID_NONE);
