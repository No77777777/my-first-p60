...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/qmidata/src/ds_qmi_wds_lte.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
)
{
  ...
  attach_pdn_list.attach_action = PS_SYS_3GPP_ATTACH_PDN_LIST_ACTION_NONE;
  if(req_msg.action_valid)
  {
    switch (req_msg.action)
    {
      case WDS_ATTACH_PDN_LIST_ACTION_NONE_V01:
        attach_pdn_list.attach_action = PS_SYS_3GPP_ATTACH_PDN_LIST_ACTION_NONE;
        break;
      case WDS_ATTACH_PDN_LIST_ACTION_DETACH_OR_PDN_DISCONNECT_V01:
        attach_pdn_list.attach_action = 
  ...
  attach_pdn_list.user_data_ptr = (void*)cl_sp;
...
{
  dsm_item_type *                                response;
  wds_get_lte_attach_pdn_list_resp_msg_v01     * resp_msg = NULL;
  qmi_error_type_v01                             errval;
  int16                                          ps_errno = 0;
  int                                            cnt_pdn;
  int                                            i;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  PS_SYSTEM_HEAP_MEM_ALLOC(
        resp_msg,
        sizeof(wds_get_lte_attach_pdn_list_resp_msg_v01),
        wds_get_lte_attach_pdn_list_resp_msg_v01*);
  ...
  memset(resp_msg, 0, sizeof(wds_get_lte_attach_pdn_list_resp_msg_v01));
  ...
  if ( ps_result  < 0 )
  {
    LOG_MSG_ERROR_2("ps_sys_ioctl %d operation returned : (%d)",
                    PS_SYS_IOCTL_3GPP_GET_LTE_ATTACH_PDN_LIST_EX, ps_errno);
    switch (ps_errno)
    {                                   
      case DS_NOMEMORY:
        errval = QMI_ERR_NO_MEMORY_V01;
  ...
  /*-------------------------------------------------------------------------
    Current attach PDN list
  -------------------------------------------------------------------------*/
  cnt_pdn = attach_pdn_list->active_attach_pdn_list.attach_pdn_profile_list_num;
  if (cnt_pdn > 0 && cnt_pdn <= QMI_WDS_ATTACH_PDN_MAX_V01)
  {
    resp_msg->attach_pdn_list_valid = TRUE;
    resp_msg->attach_pdn_list_len = cnt_pdn;
    for (i = 0; i < cnt_pdn; i++)
    {
      resp_msg->attach_pdn_list[i] =
            attach_pdn_list->active_attach_pdn_list.attach_pdn_profile_list[i];
    }
  }
  else if (cnt_pdn > QMI_WDS_ATTACH_PDN_MAX_V01)
  {
    errval = QMI_ERR_INTERNAL_V01;
    goto send_result;
  }
  ...
  /*-------------------------------------------------------------------------
    Pending attach PDN list
  -------------------------------------------------------------------------*/
  cnt_pdn = attach_pdn_list->pending_attach_pdn_list.attach_pdn_profile_list_num;
  if (cnt_pdn > 0 && cnt_pdn <= QMI_WDS_ATTACH_PDN_MAX_V01)
  {
    resp_msg->pending_attach_pdn_list_valid = TRUE;
    resp_msg->pending_attach_pdn_list_len = cnt_pdn;
    for (i = 0; i < cnt_pdn; i++)
    {
      resp_msg->pending_attach_pdn_list[i] =
            attach_pdn_list->pending_attach_pdn_list.attach_pdn_profile_list[i];
    }
  }
  else if (cnt_pdn > QMI_WDS_ATTACH_PDN_MAX_V01)
  {
...
send_result:
  ...
  if (attach_pdn_list != NULL)
  {
    PS_SYSTEM_HEAP_MEM_FREE(attach_pdn_list);
  }
  if (resp_msg != NULL)
  {
