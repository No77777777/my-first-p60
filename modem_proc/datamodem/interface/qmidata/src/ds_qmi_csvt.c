...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/qmidata/src/ds_qmi_csvt.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
#define DS_QMI_CSVT_ASSERT(expression) \
         ds_qmi_csvt_assert_wrapper(__LINE__, expression)
...
static void qmi_csvti_get_svc_info_cb
(  
  uint8  clid,
...
FUNCTION DS_QMI_CSVT_ASSERT_WRAPPER()

DESCRIPTION
  Wrapper function for DS_QMI_CSVT ASSERT
...
===========================================================================*/
static void ds_qmi_csvt_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("DS_QMI_CSVT_ERR_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
}/* ds_qmi_csvt_assert_wrapper */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  qmi_csvti_cfg.fw_cfg.cbs.get_svc_info       = qmi_csvti_get_svc_info_cb;
...
/*===========================================================================
FUNCTION qmi_csvti_get_svc_info_cb()
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (clid > 0 && clid <= CSVTI_MAX_CLIDS)
  {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  DS_QMI_CSVT_ASSERT(event_ptr && user_info_ptr);
  ...
  switch(event_ptr->call_event)
  {
    case CM_CALL_EVENT_CONNECT:
      ...
      else
      {
       //TODO: Might need to return from here or leave port tlv and send other tlv's
        LOG_MSG_ERROR_INT_0("UCSD didn't send port info in connect evt!!");
      }
      break;
    ...
    case CM_CALL_EVENT_INCOM:
      ...
      {
        LOG_MSG_ERROR_INT_0("Incoming no len sent by ucsd is too large");
      ...
      {
        LOG_MSG_ERROR_INT_0("Incoming UUS data len sent by ucsd is too large");
        qmi_csvti_free_cmd_buf(cmd_ptr);
        return;
      }
    ...
    case CM_CALL_EVENT_PROGRESS_INFO_IND:
      ...
      {
        //TODO: Might need to return from here or leave port tlv and send other tlv's
        LOG_MSG_ERROR_INT_0("UCSD didn't send port info in progress evt!!");
      }
    break;
    ...
    default:
      LOG_MSG_ERROR_INT_1("Unregistered csvt event [d] recvd, ignoring",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(NULL == cl_sp)
  {
    LOG_MSG_ERROR_INT_0("NULL client sp");
    return;
  }
  ...
  {
    LOG_MSG_ERROR_INT_0("Invalid subscription binding");
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_QMI_CSVT_ASSERT(sp && cmd_buf_p && cl_sp && sdu_in);
...
)
{
  ...
  {
    LOG_MSG_ERROR_INT_0("CSVT: csvt registration with ucsd failed");
    errval = QMI_ERR_INTERNAL;
    goto send_result;
  }
  else
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR_INT_0("Ignoring Originate call request from stray control point");
    errval = QMI_ERR_INVALID_OPERATION;
    goto send_result;
  }
  ...
  {
    LOG_MSG_ERROR_INT_0("Ignoring originate call as there is a pending request");
    errval = QMI_ERR_INVALID_OPERATION;
    goto send_result;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR_INT_2("Ignoring Confirm call request from stray control point.subs= d, active_cs_subs= d",
                  subs_id,qmi_csvti_global.active_cs_subs);
    errval = QMI_ERR_INVALID_OPERATION;
    goto send_result;
  }
  ...
  if(TRUE == qmi_csvti_global.pending.active)
  {
    LOG_MSG_ERROR_INT_0("Ignoring confirm call as there is a pending request");
    errval = QMI_ERR_INVALID_OPERATION;
    goto send_result;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(qmi_csvti_global.is_qmss_enabled)
  {
    if (TRUE == ans_param.reject)
    {
      ...
      {  
        LOG_MSG_INFO1_1("ANSWER CSVT,reject value: %d",v_in3.reject_value);
...
#error code not present
#else
  LOG_MSG_ERROR_INT_0("SCUDIF support not available");
  errval = QMI_ERR_INVALID_OPERATION;
  goto send_result;
...
)
{
  ...
  while (*sdu_in)
  {
    ...
    switch (type)
    {
      ...
      default:
        LOG_MSG_ERROR_INT_1( "Unrecognized TLV type (d)",type);
        break;
    }
...
)
{
  ...
  if((subs_id >= CSVTI_BIND_SUBS_MAX) || (subs_id < CSVTI_BIND_SUBS_PRIMARY))
  {
    LOG_MSG_ERROR_INT_1("Invalid subscription id stored for client d",csvt_client_sp->common.clid);
    errval = QMI_ERR_INTERNAL;
    goto send_result;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch(call_event)
  {
    ...
    case CSVTI_CALL_INCOMING_EVT:
      ...
      {
        LOG_MSG_ERROR_INT_0("Invalid incoming number or incoming number length");
        dsm_free_packet(&ind);
        return;
      }
      if( FALSE == qmi_svc_put_param_tlv2(&ind, 
...
/*-------------------------------------------------------------------------*/
  ...
  DS_QMI_CSVT_ASSERT(cmd_ptr != NULL);
...
/*-------------------------------------------------------------------------*/
  ...
  {
    LOG_MSG_ERROR_INT_1("Invalid wcdma subs d.Ignoring confirm info",active_cs_subs);
    return;
  }
  cl_sp = (qmi_csvti_client_state_type *)qmi_csvti_global.controlling_client_sp[active_cs_subs - 1];
...
/*-------------------------------------------------------------------------*/
  ...
  if(CSVTI_BIND_SUBS_MAX <= active_cs_subs)
  {
    LOG_MSG_ERROR_INT_1("Invalid wcdma subs d.Ignoring connect info",active_cs_subs);
    return;
  }
  cl_sp = (qmi_csvti_client_state_type *)qmi_csvti_global.controlling_client_sp[active_cs_subs - 1];
...
/*-------------------------------------------------------------------------*/
  ...
  if(CSVTI_BIND_SUBS_MAX <= active_cs_subs)
  {
    LOG_MSG_ERROR_INT_1("Invalid wcdma subs d.Ignoring end info",active_cs_subs);
    return;
  }
  cl_sp = (qmi_csvti_client_state_type *)qmi_csvti_global.controlling_client_sp[active_cs_subs - 1];
...
/*-------------------------------------------------------------------------*/
  ...
  if(CSVTI_BIND_SUBS_MAX <= active_cs_subs)
  {
    LOG_MSG_ERROR_INT_1("Invalid wcdma subs d.Ignoring incoming info",active_cs_subs);
    return;
  }
  cl_sp = (qmi_csvti_client_state_type *)qmi_csvti_global.controlling_client_sp[active_cs_subs - 1];
...
/*-------------------------------------------------------------------------*/
  ...
  if(CSVTI_BIND_SUBS_MAX <= active_cs_subs)
  {
    LOG_MSG_ERROR_INT_1("Invalid wcdma subs d.Ignoring modify info",active_cs_subs);
    return;
  }
  cl_sp = (qmi_csvti_client_state_type *)qmi_csvti_global.controlling_client_sp[active_cs_subs - 1];
...
/*-------------------------------------------------------------------------*/
  ...
  switch(cmd_buf_ptr->cmd_id)
  {
    ...
    default:
      LOG_MSG_ERROR_INT_1("Unexpected csvt cmd(d) recd in QMI CSVT cmd handler",
                      cmd_buf_ptr->cmd_id);
    break;
  }/* switch(cmd_buf_ptr->cmd_id) */
...
)
{
  qmi_csvti_cmd_buf_type *cmd_ptr;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  DS_QMI_CSVT_ASSERT(common_msg_hdr != NULL);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  DS_QMI_CSVT_ASSERT(msg_hdr != NULL);
...
/*-------------------------------------------------------------------------*/
  ...
  DS_QMI_CSVT_ASSERT(common_msg_hdr && (common_msg_hdr->service == QMUX_SERVICE_CSVT));
...
)
{
  ...
  {
    LOG_MSG_ERROR_3("clid or inst id is invalid clid= %d , "
                    "inst id= %d line num = %d",
...
/*-------------------------------------------------------------------------*/
  ...
  DS_QMI_CSVT_ASSERT(msg_hdr && sdu_in);
  ...
  if (cl_sp == NULL)
  {
    LOG_MSG_ERROR_INT_0 ("Process QMI CSVT svc command handlr callback function: No valid client state!");
    DS_QMI_CSVT_ASSERT(cl_sp != NULL);
  }
  else
  {
