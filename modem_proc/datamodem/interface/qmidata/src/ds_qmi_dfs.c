...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/qmidata/src/ds_qmi_dfs.c#2 $
...
#define DS_QMI_DFS_ASSERT(expression) \
         ds_qmi_dfs_assert_wrapper(__LINE__, expression)
...
typedef struct
{
  ...
  union
  {
    ...
    ps_sys_remote_socket_release_list_type socket_release;
...
static qmi_csi_cb_error  qmi_dfsi_get_capability(
  qmi_dfsi_client_info_type        *clnt_info,
  qmi_req_handle           req_handle,
  int                      msg_id,
  void                    *req_c_struct,
...
static uint8 qmi_dfsi_get_powersave_fltr_cnt();
...
) = 
{
  ...
  NULL,
  NULL,
  qmi_dfsi_get_capability,                   /* Request handler for message ID 0x36 */ 
};
...
FUNCTION DS_QMI_DFS_ASSERT_WRAPPER()

DESCRIPTION
  Wrapper function for DS_QMI_DFS ASSERT
...
===========================================================================*/
static void ds_qmi_dfs_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("DS_QMI_DFS_ERR_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
}/* ds_qmi_dfs_assert_wrapper */
...
)
{
  ...
  ps_iface_event_enum_type  rev_ip_enum = IFACE_REVERSE_IP_TRANSPORT_IPFLTR_CHANGED_EV;
  ps_iface_event_enum_type  powersave_enum = IFACE_POWERSAVE_FILTERING_MODE_CHANGE_EV;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR_INT_1("PS_SYS event reg failed for: d",sys_event);
    DS_QMI_DFS_ASSERT(0);
    return;
  }

  /* Register powersave filter mode change event */
  qmi_dfs_state.dfs_ps_iface_ev_buf_ptr = ps_iface_alloc_event_handle(NULL,
                                                                      qmi_dfsi_iface_ev_cb,
                                                                      NULL);
  if (qmi_dfs_state.dfs_ps_iface_ev_buf_ptr != NULL)
  {
    if (0 != ps_iface_subscribe_event_list(qmi_dfs_state.dfs_ps_iface_ev_buf_ptr,
                                           &rev_ip_enum,
                                           sizeof(rev_ip_enum)/sizeof(ps_iface_event_enum_type)))
    {
      DS_QMI_DFS_ASSERT(0);
      return;
    }
  ...
  if (0 != ps_iface_subscribe_event_list(qmi_dfs_state.dfs_ps_iface_ev_buf_ptr,
                                         &powersave_enum,
                                         sizeof(powersave_enum)/sizeof(ps_iface_event_enum_type)))
  {
    DS_UTILS_ERROR_FATAL("IFACE_POWERSAVE_FILTERING_MODE_CHANGE_EV reg failed");
    return;
  } 
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  DS_QMI_DFS_ASSERT((sig == DCC_DFS_RDR_SIGNAL) && svc_ptr);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_QMI_DFS_ASSERT(connection_handle && service_handle);
...
)
{
  qmi_dfsi_state_info_type    *dfs_svc_ptr;
  qmi_dfsi_client_info_type    *clnt_info_ptr;
  ps_sys_remote_socket_service_avail_type   remote_socket_service_avail;
  int16                                     ps_errno = 0;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR_1("qmi_dfsi_svc_disconnect_cb: dfs client 0x%p not found",
                  connection_handle);
    return;
  }

  if( TRUE == clnt_info_ptr->remote_socket_handling)
  {
    /*--------------------------------------------------------------------
        Notify Remote Socket Manger about service un-available
      ---------------------------------------------------------------------*/ 
    remote_socket_service_avail.remote_socket_service_avail = FALSE;
    remote_socket_service_avail.remote_service_version      = PS_SYS_REMOTE_SOCKET_HANDLING_V0;

    ps_sys_conf_set_ex(PS_SYS_TECH_ALL,
                       PS_SYS_CONF_REMOTE_SOCKET_SERVICE_AVAIL,
                       PS_SYS_PRIMARY_SUBS, 
                       (void *)&remote_socket_service_avail,
                       &ps_errno);
    LOG_MSG_INFO1_1("qmi_dfsi_svc_disconnect_cb remote serv un-available status %d", ps_errno);
  }

  if(TRUE != q_delete_ext( &dfs_svc_ptr->client_q,
                             &clnt_info_ptr->link ) )
  {
    LOG_MSG_ERROR_1("qmi_dfsi_svc_disconnect_cb: dfs client 0x%p could not be deleted from queue",
                    clnt_info_ptr);
    return;
  }
...
#endif
/*-----------------------------------------------------------------------------*/
  DS_QMI_DFS_ASSERT(req_c_struct && (req_c_struct_len > 0) && clnt_info && service_handle);
...
/*===========================================================================
  FUNCTION QMI_DFS_GET_CAPABILITY()

  DESCRIPTION
    Get the additional capability of QMI_DFS.
...
{
  dfs_get_capability_resp_msg_v01   resp_msg;
/*-----------------------------------------------------------------------------*/
  DS_QMI_DFS_ASSERT(req_c_struct && (req_c_struct_len > 0) && clnt_info && service_handle);
  memset(&resp_msg, 0, sizeof(dfs_get_capability_resp_msg_v01));

  resp_msg.remote_socket_capability_valid = TRUE;
  resp_msg.remote_socket_capability = TRUE;
  
  resp_msg.remote_socket_capability_version_valid = TRUE;
  resp_msg.remote_socket_capability_version = DFS_REMOTE_SOCKET_HANDLING_V1_V01;
...
//====================================================================
  ...
  if (req_msg->filter_id_valid) 
  {
    ...
    {
      LOG_MSG_ERROR_INT_2( "Error: Client subs doesn't match current data call subs"
...
)
{
  ...
  if(req_msg->filter_rules_len > 0 && 
     req_msg->filter_rules_len  <= (max_allowed - qmi_dfsi_get_powersave_fltr_cnt()))
  {
    ...
    if(retval != 0)
    {
      LOG_MSG_ERROR_INT_1("ps_iface_get_powersave_filtering_mode failed: d)",retval);
      resp_msg.resp.error  = QMI_ERR_INTERNAL_V01;
      resp_msg.resp.result = QMI_RESULT_FAILURE_V01;
      goto bail;
    }
...
)
{
  ...
  if(retval != 0)
  {
    LOG_MSG_ERROR_INT_1("ps_iface_set_powersave_filtering_mode failed (d)",retval);
    resp_msg.resp.error  = QMI_ERR_INTERNAL_V01;
    resp_msg.resp.result = QMI_RESULT_FAILURE_V01;
  }
  else
  {
    resp_msg.resp.error = QMI_ERR_NONE_V01;
    resp_msg.resp.result = QMI_RESULT_SUCCESS_V01;;
        //Do flow config logging
    rmnet_meta_sm_generate_fc_config_log_pkt
    (
      clnt_info->ep_id, 
      clnt_info->mux_id,
      (clnt_info->bound_ip_type==DFS_IP_FAMILY_IPV4_V01)?IPV4_ADDR:IPV6_ADDR
    );    
  }	
bail:
  qmi_csi_send_resp(req_handle, msg_id, &resp_msg, 
...
)
{
  ...
  if(retval != 0)
  {
    LOG_MSG_ERROR_INT_1("ps_iface_get_powersave_filtering_mode (d)",retval);
    resp_msg.resp.error  = QMI_ERR_INTERNAL_V01;
    resp_msg.resp.result = QMI_RESULT_FAILURE_V01;
  }
  else
...
)
{
  ...
  if (ps_iface_ioctl(rev_ip_iface_ptr,
                     ...
                     &ps_errno) != 0)
  {
    LOG_MSG_ERROR_INT_1("Couldn't get IPfltrs, IOCTL gave errno: d", ps_errno);
    resp_msg->resp.error = QMI_ERR_INTERNAL_V01;
    resp_msg->resp.result = QMI_RESULT_FAILURE_V01;
    goto bail;
  }
...
)
{
  ...
  ps_sys_remote_socket_allocated_list_type    * allocated_sockets;
  uint8    itr = 0;
  int16    ps_result = 0;
  int16    ps_errno = 0;
  qmi_csi_error rc;
  ...
  PS_SYSTEM_HEAP_MEM_ALLOC(allocated_sockets,
                           sizeof(ps_sys_remote_socket_allocated_list_type),
                           ps_sys_remote_socket_allocated_list_type*);
  if(allocated_sockets == NULL)
  {
    return QMI_CSI_CB_NO_MEM;
  }
  memset(allocated_sockets, 0, sizeof(ps_sys_remote_socket_allocated_list_type));
   
  LOG_MSG_INFO1_3(" Remote Socket num sockets = %d  Request handle present %d  handle %d", 
    req_msg->socket_list_len, req_msg->request_identifier_valid, req_msg->request_identifier);
  
  allocated_sockets->num_sockets = req_msg->socket_list_len;

  if( TRUE == req_msg->request_identifier_valid)
  {
    allocated_sockets->request_handle = req_msg->request_identifier;
  }
  ...
  LOG_MSG_INFO1_1(" Remote Socket rc = %d", rc);
...
)
{
  ...
  memset(&remote_socket_service_avail,0,sizeof(ps_sys_remote_socket_service_avail_type));
  ...
  LOG_MSG_INFO1_2("Remote Socket  info valid %d available %d ", 
    req_msg->remote_socket_handling_valid, req_msg->remote_socket_handling);
  ...
  if(req_msg->remote_socket_handling_valid)
  {
    ...
    if(req_msg->remote_socket_handling > 0)
    {
      ...
      remote_socket_service_avail.remote_socket_service_avail = TRUE;
      
      if(req_msg->remote_socket_handling_version_valid )
      {
        remote_socket_service_avail.remote_service_version = 
            req_msg->remote_socket_handling_version;
      }
      else
      {
        remote_socket_service_avail.remote_service_version = 
            PS_SYS_REMOTE_SOCKET_HANDLING_V0;
      }
    }
    else
    {
      ...
      remote_socket_service_avail.remote_socket_service_avail = FALSE;
      remote_socket_service_avail.remote_service_version      = 0;
    }
    ...
    if (0 > ps_sys_conf_set_ex(PS_SYS_TECH_ALL,
                       ...
                       &ps_errno))
    {
      LOG_MSG_ERROR_INT_1("ps_sys_conf_set error: d", ps_errno);
      resp_msg.resp.error = QMI_ERR_INTERNAL_V01;
      goto bail;    
    }
  }
...
)
{
  ...
  /*--------------------------------------------------------------*/
  DS_QMI_DFS_ASSERT( cmd_ptr != NULL);
  ...
  while( clnt_info != NULL )
  {
    if( clnt_info->report_low_latency_traffic_status)
    {
      ...
      if ( rc != QMI_CSI_NO_ERR )
      {
        LOG_MSG_ERROR_INT_1("qmi_dfsi_generate_filter_mode_indication error d",
                          rc);
        goto bail;
      }
    }
...
)
{
  ...
  uint32                                itr=0;
  /*----------------------------------------------------------*/
  DS_QMI_DFS_ASSERT(cmd_ptr && cmd_ptr->data.rev_ip_fltr_change.iface_ptr);
  ...
  while(client_info_ptr != NULL)
  {
    if (client_info_ptr->report_rev_ip_transport_filters == TRUE)
    {
      ...
      if(cmd_ptr->data.rev_ip_fltr_change.iface_ptr == rev_ip_iface_ptr)
      {
        ...
        if ( rc != QMI_CSI_NO_ERR )
        {
          LOG_MSG_ERROR_INT_1("qmi_dfsi_generate_rev_ip_fltr_changed_ind error: d", rc);
          break;		
        }
      }
    }
...
)
{
  ...
  ps_sys_remote_socket_list_type        * socket_list    = &socket_request->sockets_list;
  /*----------------------------------------------------------*/
  DS_QMI_DFS_ASSERT(cmd_ptr != NULL);
  ...
  ind->request_socket_list_len = socket_list->num_sockets;

  for (itr=0; itr< socket_list->num_sockets; itr++)
  {
    ind->request_socket_list[itr].ip_family = 
      (dfs_ip_family_enum_v01)socket_list->socket_list[itr].ip_family;
    ind->request_socket_list[itr].xport_prot = 
      (dfs_xport_protocol_enum_v01)socket_list->socket_list[itr].xport_proto;
    ind->request_socket_list[itr].port_no =
      ps_ntohs(socket_list->socket_list[itr].port_no);
  }

  ind->request_identifier_valid = TRUE;
  ind->request_identifier       = socket_request->request_handle;

  ind->port_pref_valid           = TRUE;
  ind->port_pref.port_property   = socket_request->port_property;
  ind->port_pref.base_port_value = socket_request->base_port_range;
  ind->port_pref.max_port_value  = socket_request->max_port_range;
  ...
  while(client_info_ptr != NULL)
  {
    if (client_info_ptr->remote_socket_handling == TRUE)
    {
      ...
      if ( rc != QMI_CSI_NO_ERR )
      {
        LOG_MSG_ERROR_INT_1("qmi_dfsi_generate_remote_socket_request_indication error: d", rc);
        break;		
      }
    }
    client_info_ptr = q_next(&qmi_dfs_state.client_q,
...
)
{
  ...
  qmi_csi_error                         rc;
  qmi_dfsi_client_info_type*            client_info_ptr = NULL;
  ps_sys_remote_socket_release_list_type     * socket_release = (ps_sys_remote_socket_release_list_type*)&cmd_ptr->data.socket_release;
  /*----------------------------------------------------------*/
  DS_QMI_DFS_ASSERT(cmd_ptr != NULL);
  ...
  while(client_info_ptr != NULL)
  {
    if (client_info_ptr->remote_socket_handling == TRUE)
    {
      ...
      if ( rc != QMI_CSI_NO_ERR )
      {
        LOG_MSG_ERROR_INT_1("qmi_dfsi_generate_remote_socket_release_indication error: d", rc);
        break;		
      }
    }
    client_info_ptr = q_next(&qmi_dfs_state.client_q,
...
)
{
  ...
  while(client_info_ptr != NULL)
  {
    if (client_info_ptr->remote_socket_handling == TRUE)
    {
      ...
      if ( rc != QMI_CSI_NO_ERR )
      {
        LOG_MSG_ERROR_INT_1("qmi_dfsi_generate_remote_socket_set_option_indication error: d", rc);
        break;		
      }
    }
    client_info_ptr = q_next(&qmi_dfs_state.client_q,
...
)
{
  ...
  rmnet_instance_e_type                 rmnet_instance = RMNET_INSTANCE_MAX;
  /*----------------------------------------------------------*/
  DS_QMI_DFS_ASSERT(cmd_ptr && cmd_ptr->data.pws_state_data.iface_ptr);
  ...
  while(client_info_ptr != NULL)
  {
    ...
    if(rmnet_instance >= RMNET_INSTANCE_MAX)
    {
      LOG_MSG_ERROR_INT_0("Invalid instance - Ignored indication");
      client_info_ptr = q_next(&qmi_dfs_state.client_q,
                               &(client_info_ptr->link));
      continue;
    }
	   ...
	   (cmd_ptr->data.pws_state_data.iface_ptr == rm_iface_ptr))
    {
      ...
      if ( rc != QMI_CSI_NO_ERR )
      {
        LOG_MSG_ERROR_INT_1("qmi_dfsi_generate_filter_mode_indication error: d", rc);
        break;		
      }
    }
    client_info_ptr = q_next(&qmi_dfs_state.client_q,
...
)
{
  ...
  DS_QMI_DFS_ASSERT( this_iface_ptr != NULL);
  ...
  switch( event )
  {
    ...
    case IFACE_REVERSE_IP_TRANSPORT_IPFLTR_CHANGED_EV:
      ...
      if(cmd_ptr != NULL)
      {
        ...
        {
          LOG_MSG_ERROR_INT_0("Unable to copy IPFLTR list");
    ...
    default:
      LOG_MSG_ERROR_INT_1("Unhandled IFACE ev: d", event);
      return;
  }
...
)
{
  ...
  ps_sys_low_latency_traffic_info_type   * low_latency_traffic_info_ptr = NULL;
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  DS_QMI_DFS_ASSERT( event_info_ptr != NULL);
 
  low_latency_traffic_info = 
    (ps_sys_low_latency_traffic_info_type *)event_info_ptr;

  /*---------------------------------------------------------------------------
    We will ignore low latency traffic continue event as clients won't be 
    interested in it
    -------------------------------------------------------------------------*/
...
)
{
  ...
  switch (event_name)
  {
    case PS_SYS_EVENT_REMOTE_SOCKET_REQUEST:
    {
      ...
      ps_sys_remote_socket_list_type    *temp_socket_list = &temp_ptr->sockets_list;
      uint16 i;
      /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
      
      LOG_MSG_INFO1_4(" Remote Socket property : %d Base : %d Max %d count %d ",temp_ptr->port_property,
        temp_ptr->base_port_range,temp_ptr->max_port_range, temp_ptr->sockets_list.num_sockets);
      
      for (i=0; i< temp_socket_list->num_sockets; i++)
      {
        LOG_MSG_INFO3_3("Remote Socket IP family %d Protocol %d Port %d",temp_socket_list->socket_list[i].ip_family,
          temp_socket_list->socket_list[i].xport_proto, temp_socket_list->socket_list[i].port_no);
      }
    ...
    {
      ps_sys_remote_socket_release_list_type *temp_ptr = (ps_sys_remote_socket_release_list_type*)event_info_ptr;
      /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
      
      LOG_MSG_INFO1_1(" Remote Socket num_sockets: %d",temp_ptr->num_sockets);
      ...
      if (cmd_ptr != NULL)
      {
        ...
        memscpy(&cmd_ptr->data.socket_release, sizeof(ps_sys_remote_socket_release_list_type),
                event_info_ptr, sizeof(ps_sys_remote_socket_release_list_type));
      }
      break;
    }
    case PS_SYS_EVENT_REMOTE_SOCKET_SET_OPTION:
    {
      ...
      LOG_MSG_INFO1_2(" Remote Socket Socket handle 0x%x is_udp_encaps %d ", temp_ptr->socket_handle, temp_ptr->is_udp_encaps);
     ...
     default:
       LOG_MSG_ERROR_INT_1("Remote Socket Unhandled ps_sys ev: d", event_name);
       return;
  }
  if (cmd_ptr == NULL)
  {
...
)
{
  ...
  switch (cmd_id)
  {
    ...
    default:
      LOG_MSG_ERROR_INT_1("Unknown QMI DFS command: d", (int32)cmd_id);
      break;
  }
  return NULL;
...
{
  DS_QMI_DFS_ASSERT(filter_rules && fltr_spec);
...
{
  DS_QMI_DFS_ASSERT(xport_info && fltr_spec);

  LOG_MSG_INFO3_1("qmi_dfsi_fill_fltr_xport_info: protocol = %d",
...
)
{
  ...
  DS_QMI_DFS_ASSERT(filter_spec && error_mask);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  DS_QMI_DFS_ASSERT(clnt_info != NULL);
...
/*===========================================================================
  FUNCTION QMI_DFSI_GET_POWERSAVE_FLTR_CNT()

  DESCRIPTION
    This function counts number of powersave filters currently present on all interfaces.
  ...
  SIDE EFFECTS
    None
===========================================================================*/
uint8 qmi_dfsi_get_powersave_fltr_cnt()
{
  ps_iface_type*                        rm_iface_ptr = NULL;
  uint8                                 fltr_cnt = 0;
  q_type                            *   ipfltr_q_ptr;
  rmnet_instance_e_type                 instance = 0;
  
  /*-------------------------------------------------------------------------------------------*/
  /*loop through all rmnet instances and get the active interfaces to calculate total powersave filters */
  for (instance = 0; instance < RMNET_INSTANCE_MAX; instance++)
  {
    /*skip the rmnet interface if it is not in call*/
    if( !rmnet_meta_sm_in_call(instance) )
    {
      continue;
    }
    rm_iface_ptr = rmnet_meta_sm_iface(instance);
    if ( PS_IFACE_IS_VALID(rm_iface_ptr) )
    {
      ipfltr_q_ptr = &(rm_iface_ptr->iface_private.ipfltr_info[IP_FLTR_CLIENT_POWERSAVE]);
      fltr_cnt += q_cnt(ipfltr_q_ptr);
      LOG_MSG_INFO1_4("qmi_dfsi_get_powersave_fltr_cnt: iface 0x%x:%d qc_cnt %d fltr_cnt %d",
                    rm_iface_ptr->name, rm_iface_ptr->instance,q_cnt(ipfltr_q_ptr),fltr_cnt);
    }
  }
  return fltr_cnt;
}
