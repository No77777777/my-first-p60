...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/qmidata/src/ds_qmi_wds_profile.c#4 $ $DateTime: 2023/02/03 04:40:34 $ $Author: pwbldsvc $
...
#define CHECK_RETVAL()  if (FALSE == retval) { dsm_free_packet(&response); \
                                               return NULL; }
#define DS_QMI_WDS_PROFILE_ASSERT(expression) \
         ds_qmi_wds_profile_assert_wrapper(__LINE__, expression)
...
static const qmi_profile_3gpp_param_tl_list_type qmi_profile_3gpp_param_tl_list[] = 
{
  ...
  { DS_PROFILE_3GPP_PROFILE_OP_RESERVED_PCO_ID_LIST,
    sizeof(ds_profile_3gpp_op_reserved_pco_id_list) },  

  /*0x48*/
  { DS_PROFILE_3GPP_PROFILE_MSISDN_REQ_FLAG,
    sizeof(ds_profile_3gpp_request_msisdn_flag_type) },

  /*0x49 RESERVED PROFILE PARAM ID */

  /*0x4A*/
  { DS_PROFILE_3GPP_PROFILE_PARAM_PDN_LABEL, 
    0 }
...
typedef enum
{
  ...
  QMI_DELETE_ALL_PROFILES_SUCCESS         = 0,
  QMI_DELETE_ALL_PROFILES_FAILURE         = 1,
  QMI_DELETE_ALL_PROFILES_PARTIAL_FAILURE = 2
  
}qmi_profile_delete_all_profiles_status_enum_type;
...
static const qmi_profile_general_param_tl_list_type qmi_profile_general_param_tl_list[] = 
{
  ...
  /*0XDD*/
  { DS_PROFILE_GENERAL_PROFILE_PARAM_APN_TYPE, 
   sizeof(uint64) },
...
FUNCTION DS_QMI_WDS_PROFILE_ASSERT_WRAPPER()

DESCRIPTION
  Wrapper function for DS_QMI_WDS_PROFILE ASSERT
...
===========================================================================*/
static void ds_qmi_wds_profile_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("DS_QMI_WDS_PROFILE_ERR_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
}/* ds_qmi_wds_profile_assert_wrapper */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_QMI_WDS_PROFILE_ASSERT(sdu_in != NULL);
  ...
  *error_val_ptr = QMI_ERR_NONE_V01;
  ...
  while (*sdu_in)
  {
    ...
    //For attach profile we need only 3gpp parameters.
    if (TRUE == is_attach)
    {
      if (type >= DS_PROFILE_3GPP_PROFILE_PARAM_MAX && 
          type != DS_PROFILE_GENERAL_PROFILE_PARAM_IPV6_PREFIX_DELEGATION_FLAG &&
          type != DS_PROFILE_GENERAL_PROFILE_PARAM_CLAT_ENABLE_FLAG)
      {
...
send_result:
  ...
  *profile_err_status = profile_status;
...
)
{
  ...
  ds_profile_hndl_type              profile_hndl;
  ds_profile_identifier_type        profile_identifier = 0;
  ds_profile_info_type              param_info;
  byte                              value[WDS_MAX_PROFILE_PARAM_VAL_LEN];
  uint16                            param_len = 0;
  boolean                           cancel_txn = FALSE;
  uint32                            *ipv4_value;
  ds_profile_3gpp_tft_params_type   tft_profile_param;
  qmi_profile_tft_params_type       tft_qmi_param;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /* if profiles are not ready on the bound sub_id, return device_not_ready */
  if(!qmi_wds_is_profile_ready(subs_id))
  {
  ...
  profile_status = ds_profile_get_persistence_from_profile_num_per_sub(profile_index, 
                                                                       &is_persistent, 
                                                                       subs_id);
  if (profile_status != DS_PROFILE_REG_RESULT_SUCCESS)
  {
    LOG_MSG_ERROR_1 ("Get Persistence failed. Error %d", profile_status);
    errval = QMI_ERR_EXTENDED_INTERNAL_V01;
    goto send_result;
  }
  ...
  retval = qmi_svc_put_param_tlv(&response,
                                 WDSI_PRM_TYPE_PROFILE_PERSISTENT,
                                 sizeof(is_persistent),
                                 (void*)&is_persistent);
  if (FALSE == retval)
  {
    HANDLE_FAILURE(&response);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  profile_config.config_mask |= DS_PROFILE_CONFIG_MASK_AP_CLIENT;
...
/*===========================================================================
  FUNCTION QMI_WDS_PROFILE_DELETE_ALL()
 
  DESCRIPTION
  Deletes a specified profile for the specified technology.
 
  PARAMETERS
    delete_all_indication   : wds delete all profiles result indication
    profile_type            : profile type (3GPP/3GPP2)
    persistent              : profile persistency.
...
void qmi_wds_profile_delete_all
(
  wds_delete_all_profiles_result_ind_msg_v01  *delete_all_indication,
  uint8 		                       profile_type,
  uint8 		                       persistent,
...
)
{
  ...
  #define DEFAULT_3GPP_PROFILE 1
  #define DEFAULT_3GPP2_PROFILE 0
  #define DS_PROFILE_TECH_3GPP_3GPP2 2
  #define DELETE_ALL_PROFILES_MAX QMI_WDS_DELETE_ALL_PROFILES_LIST_MAX_V01
  #define DELETE_PROFILE_CLIENT_ANY 0x80

  uint8                                  len = 0;  
  ds_profile_itr_type                    itr;
  ds_profile_status_etype                profile_status = DS_PROFILE_REG_RESULT_SUCCESS;
  ds_profile_status_etype                profile_delete_status = DS_PROFILE_REG_RESULT_SUCCESS;
  ds_profile_status_etype                profile_list_status = DS_PROFILE_REG_RESULT_SUCCESS;
  ds_profile_list_type                   search_list_info;
  ds_profile_list_info_type              list_info;
  ds_profile_info_type                   profile_info;
  byte                                   value[150];
  uint8                                  pdp_type = 0;
  boolean                                is_persistent = FALSE;
  uint8                                  profile_type_mask = DS_PROFILE_TECH_3GPP;
  ds_profile_hndl_type                   profile_handle = NULL;
  ds_profile_info_type                   param_info;
  ds_profile_identifier_type             profile_ident;
  qmi_profile_delete_all_profiles_status_enum_type result = QMI_DELETE_ALL_PROFILES_SUCCESS;
  uint32 profiles_iface_3gpp[10] = {0}; /* 8 Pdns + 2 reserved */
  uint32 profiles_iface_3gpp2[10] = {0};
  uint8 num_profiles_3ggp = 0;
  uint8 num_profiles_3ggp2 = 0;
  uint8 client_type = 0xFF; 
  int16 ps_errno = DS_ENOERR;
  int16 ps_result;
  ps_sys_3gpp_get_lte_attach_pdn_list_ex_type	* attach_pdn_list = NULL;
  int i = 0;

  delete_all_indication->status = result;
  delete_all_indication->deleted_profiles_list_len = 0;   

  ps_iface_get_profiles_in_use(profile_type, &profiles_iface_3gpp[0], &num_profiles_3ggp, &profiles_iface_3gpp2[0], &num_profiles_3ggp2);
  PS_SYSTEM_HEAP_MEM_ALLOC(
        attach_pdn_list,
        sizeof(ps_sys_3gpp_get_lte_attach_pdn_list_ex_type),
        ps_sys_3gpp_get_lte_attach_pdn_list_ex_type*);

  if (attach_pdn_list != NULL)
  {
    memset(attach_pdn_list, 0, sizeof(ps_sys_3gpp_get_lte_attach_pdn_list_ex_type));
    /* Get LTE pdn list */
    ps_result = ps_sys_ioctl_ex ( PS_SYS_TECH_3GPP,
                                PS_SYS_IOCTL_3GPP_GET_LTE_ATTACH_PDN_LIST_EX,
  ...
  do
  {
    profile_list_status = DS_PROFILE_REG_RESULT_SUCCESS;
    
    if(profile_type == DS_PROFILE_TECH_3GPP_3GPP2 && profile_type_mask == DS_PROFILE_TECH_3GPP)
    {
      profile_type_mask = profile_type;
	  profile_type = DS_PROFILE_TECH_3GPP;
    }
	else if(profile_type == DS_PROFILE_TECH_3GPP && profile_type_mask == DS_PROFILE_TECH_3GPP_3GPP2)
	{
	  profile_type_mask = DS_PROFILE_TECH_3GPP;
	  profile_type = DS_PROFILE_TECH_3GPP2;
	}
    memset(&itr,0,sizeof(itr));
    memset(&search_list_info, 0, sizeof(search_list_info));
    search_list_info.dfn = DS_PROFILE_LIST_ALL_PROFILES;
    ...
    profile_status = ds_profile_get_list_itr_per_sub(profile_type,
													&search_list_info, 
													&itr,
													subs_id);
	
    if((profile_status != DS_PROFILE_REG_RESULT_SUCCESS) && (profile_status != DS_PROFILE_REG_RESULT_LIST_END))
    {   
       LOG_MSG_ERROR_1 ("Getting profile list iterator failed. Error %d",
		     				 profile_status);	
	   break;
    ...
    while(profile_list_status != DS_PROFILE_REG_RESULT_LIST_END)
    {
	  memset(&list_info, 0, sizeof (ds_profile_list_info_type));
	  memset(value, 0, sizeof(value));
	  ...
	  /* skip default profiles deletion */
	  if(((profile_type == DS_PROFILE_TECH_3GPP) && (list_info.num == DEFAULT_3GPP_PROFILE)) || 
	  	    ((profile_type == DS_PROFILE_TECH_3GPP2) && (list_info.num == DEFAULT_3GPP2_PROFILE)))
	  {
	    profile_list_status = ds_profile_itr_next(itr);
	    continue;
	  }

	  if(persistent < QMI_WDS_PROFILE_PERSISTENT_V01)
	  {
	    /* get persistency of profile */
	    profile_status = ds_profile_get_persistence_from_profile_num_per_sub(list_info.num, &is_persistent,subs_id);
		
	    if((profile_status != DS_PROFILE_REG_RESULT_SUCCESS) || 
			   (is_persistent != persistent))
	    {
	       LOG_MSG_ERROR_1 ("profile persistency error %d",
		           			 profile_status);
		   profile_list_status = ds_profile_itr_next(itr);
	       continue; /* continue deleting next profile */
	    }
	  }

	  if((profile_type == DS_PROFILE_TECH_3GPP) && 
	  	   ps_iface_check_profile_in_use(list_info.num, &profiles_iface_3gpp[0], num_profiles_3ggp))
	  {
	    profile_list_status = ds_profile_itr_next(itr);
	    continue; /* continue deleting next profile */
	  }
	  else if((profile_type == DS_PROFILE_TECH_3GPP) && attach_pdn_list &&
	  	  (attach_pdn_list->active_attach_pdn_list.attach_pdn_profile_list_num || 
	  	   attach_pdn_list->pending_attach_pdn_list.attach_pdn_profile_list_num))
	  {
	    for(i =0; i < attach_pdn_list->active_attach_pdn_list.attach_pdn_profile_list_num; i++)
	    {
	      if(attach_pdn_list->active_attach_pdn_list.attach_pdn_profile_list[i] == list_info.num)
	      {
                /* found profile in attach list */
                break;
	      }
	    }
	    if(i<attach_pdn_list->active_attach_pdn_list.attach_pdn_profile_list_num)
	    {
              profile_list_status = ds_profile_itr_next(itr);
	      continue; /* continue deleting next profile */
	    }		
        for(i =0; i < attach_pdn_list->pending_attach_pdn_list.attach_pdn_profile_list_num; i++)
        {
          if(attach_pdn_list->pending_attach_pdn_list.attach_pdn_profile_list[i] == list_info.num)
          {
            /* found profile in attach list */
	        break;
          }
        }
	    if(i<attach_pdn_list->pending_attach_pdn_list.attach_pdn_profile_list_num)
	    {
          profile_list_status = ds_profile_itr_next(itr);
          continue; /* continue deleting next profile */
        }
	  }
	  else if((profile_type == DS_PROFILE_TECH_3GPP2) && 
	  	       ps_iface_check_profile_in_use(list_info.num, &profiles_iface_3gpp2[0], num_profiles_3ggp2))
	  {
	    profile_list_status = ds_profile_itr_next(itr);
	    continue; /* continue deleting next profile */
	  }

      /* Get APN name and PDP type */ 
      profile_status = ds_profile_begin_transaction_per_sub(DS_PROFILE_TRN_READ,
                                                            profile_type,
                                                            list_info.num,
                                                            subs_id,
                                                            &profile_handle );
	  if( DS_PROFILE_REG_RESULT_SUCCESS != profile_status ||
          profile_handle == NULL )
      {
        LOG_MSG_ERROR_2("Begin transaction failed %d %x", 
                         profile_status,profile_handle);
        profile_list_status = ds_profile_itr_next(itr);
		(void)ds_profile_end_transaction(profile_handle,
                                          DS_PROFILE_ACTION_COMMIT);
		continue;
      }
	  ...
	  param_info.buf = (void*)&pdp_type;
	  param_info.len = sizeof(uint8);

	  if(profile_type == DS_PROFILE_TECH_3GPP)
	  {
		profile_ident = DS_PROFILE_3GPP_PROFILE_PARAM_PDP_CONTEXT_PDP_TYPE;
	  }
	  else
	  {  
		profile_ident = DS_PROFILE_3GPP2_PROFILE_PARAM_PDN_TYPE;
	  }

	  profile_status = ds_profile_get_param(profile_handle,
										    profile_ident,
										    &param_info);

	  if(profile_type == DS_PROFILE_TECH_3GPP)
	  {
	    profile_ident = DS_PROFILE_3GPP_PROFILE_PARAM_PDP_CONTEXT_APN;
	  }
	  else
	  {
	    profile_ident = DS_PROFILE_3GPP2_PROFILE_PARAM_APN_STRING;
	  }

	  memset(&param_info, 0, sizeof(ds_profile_info_type));
	  memset(value,0,sizeof(value));	  
	  param_info.buf = (void*)value;
	  param_info.len = sizeof(value)+1;	  
	  ...
	  (void)ds_profile_end_transaction(profile_handle,
           ...
           (void)ds_profile_get_profile_client_from_profile_num_per_sub(list_info.num, 
	                                                               &client_type, 
	  	                                                           subs_id);
	  

          if((profile_client >= DELETE_PROFILE_CLIENT_ANY) || (profile_client == client_type))
          {
            /* delete profile from DB */
	    profile_status = ds_profile_delete_per_sub(profile_type,
		  		  							     list_info.num,
											     subs_id);
          }
	  else
	  ...
	  /* Update the delete all profiles list */
	  if(len < DELETE_ALL_PROFILES_MAX && (DS_PROFILE_REG_RESULT_SUCCESS == profile_status))
	  {
	    delete_all_indication->deleted_profiles_list[len].profile_ident_type.profile_index = list_info.num;
	    delete_all_indication->deleted_profiles_list[len].profile_ident_type.profile_type = profile_type;
	    if((DS_PROFILE_TECH_3GPP2 == profile_type) && pdp_type )
            {
               /* 3gpp2 pdn types are not same 3gpp */
               pdp_type++;
            }
	    delete_all_indication->deleted_profiles_list[len].pdp_type = pdp_type;
	    memscpy((char *)delete_all_indication->deleted_profiles_list[len].apn_name,sizeof(value),
			            (char *)value,sizeof(value));
	    len++;
          }
	  else
	  {
	    profile_delete_status = profile_status;
	  }

	  profile_list_status = ds_profile_itr_next(itr);
    
	  if ( (profile_list_status != DS_PROFILE_REG_RESULT_SUCCESS) &&
		   (profile_list_status != DS_PROFILE_REG_RESULT_LIST_END) )
	  {
	     LOG_MSG_ERROR_1 ("Getting Next iterator failed. Error %d",
	             	       profile_list_status);
	     break;
  ...
  while(profile_type_mask == DS_PROFILE_TECH_3GPP_3GPP2); /* delete both 3gpp and 3gpp2 profiles */

  if(len)
  {
    delete_all_indication->deleted_profiles_list_len = len;
    delete_all_indication->deleted_profiles_list_valid = TRUE;
  }

  if (profile_delete_status != DS_PROFILE_REG_RESULT_SUCCESS)
  {
    if(len)
    {
      result = QMI_DELETE_ALL_PROFILES_PARTIAL_FAILURE;
    }
    else
    {
      result = QMI_DELETE_ALL_PROFILES_FAILURE;
    }
  }
...
/*===========================================================================
  FUNCTION qmi_wds_create_and_set_attach_profile_info()
 
  DESCRIPTION
    Create and set attach profile info.
  ...
  SIDE EFFECTS
    None
===========================================================================*/
dsm_item_type * qmi_wds_create_and_set_attach_profile_info
(
  dsm_item_type **    sdu_in,
  uint32              subs_id
)
{
  dsm_item_type *                            response;
  wds_set_attach_profile_info_resp_msg_v01   resp_msg;
  wds_set_attach_profile_info_req_msg_v01*   req_msg = NULL;
  qmi_error_type_v01                         errval;
  ds_profile_status_etype                    profile_status;
  uint16                                     profile_num;
  ds_profile_config_type                     profile_config = {0};
  ...
  ps_sys_set_attach_profile_info_type        attach_info = {{0}};
  int16                                      ps_sys_status = 0;
  int16                                      ps_errno = 0;
  uint32                                     profile_err = 0;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  PS_SYSTEM_HEAP_MEM_ALLOC(req_msg,
                           sizeof(wds_set_attach_profile_info_req_msg_v01),
                           wds_set_attach_profile_info_req_msg_v01*);
  if(req_msg == NULL)
  {
    errval = QMI_ERR_NO_MEMORY_V01;
    goto send_error;
  ...
  {
    memset(req_msg,0,sizeof(wds_set_attach_profile_info_req_msg_v01));
  }
  ...
  errval = QMI_WDSI_DECODE_REQUEST_MSG(QMI_WDS_SET_ATTACH_PROFILE_INFO_REQ_V01,
                                       sdu_in,
                                       req_msg,
                                       sizeof(wds_set_attach_profile_info_req_msg_v01));
  if(errval != QMI_ERR_NONE_V01)
  ...
  profile_config.config_mask |= DS_PROFILE_CONFIG_MASK_SUBS_ID;
  profile_config.subs_id = subs_id;
  profile_config.config_mask |= DS_PROFILE_CONFIG_MASK_ATTACH_PROFILE;
  profile_config.is_attach = TRUE;
  
  profile_status = ds_profile_create_ex( tech, 
                                        &profile_config,
                                        &profile_num );
  ...
  /*-------------------------------------------------------------------------
    ...
    by modify API in success as well as fail case.
  -------------------------------------------------------------------------*/
  LOG_MSG_INFO1_1("calling modify set %d",tech);
  response = qmi_wds_profile_modify_settings(&sdu_dup,
                                             tech,
                                             (uint8)profile_num,
                                             subs_id,
  ...
  if (response == NULL || errval != QMI_ERR_NONE_V01)
  {
    profile_status = (ds_profile_status_etype)profile_err;
    (void) ds_profile_delete_per_sub(tech,
                                     profile_num,
                                     subs_id);
    goto send_error;
  }
  
  /*Free the response at the end 
   and create a new one with profile type and number*/
  resp_msg.profile_index_valid = TRUE;
  resp_msg.profile_index = (uint8)profile_num;

  //Set attach INfo
  attach_info.attach_rules.profile_id = profile_num;

  if (req_msg->append_flag_valid)
  {
    attach_info.attach_profile_action_type.append_flag = 
      req_msg->append_flag;
  }
  else
  {
    attach_info.attach_profile_action_type.append_flag = FALSE;
  }

  if (req_msg->attach_config_action_valid)
  {
    attach_info.attach_profile_action_type.attach_config_action = 
      (ps_sys_attach_profile_action_enum_type)
        req_msg->attach_config_action;
  }
  else
  {
    attach_info.attach_profile_action_type.attach_config_action = 
      PS_SYS_ATTACH_PROFILE_ACTION_DEFAULT;
  }

  if (req_msg->attach_apn_ip_addr_rules_valid)
  {
    attach_info.attach_rules.attach_apn_rules.ip_addr_rules = 
      (ps_sys_attach_address_rule_enum_type)
        req_msg->attach_apn_ip_addr_rules;
	attach_info.attach_rules.attach_apn_rules.ip_addr_rules_valid = TRUE;
  }
  

  if (req_msg->attach_apn_dns_addr_rules_valid)
  {
    attach_info.attach_rules.attach_apn_rules.dns_addr_rules = 
      (ps_sys_attach_address_rule_enum_type)
        req_msg->attach_apn_dns_addr_rules;
	attach_info.attach_rules.attach_apn_rules.dns_addr_rules_valid = TRUE;
  }
  

  if (req_msg->attach_apn_pcscf_addr_rules_valid)
  {
    attach_info.attach_rules.attach_apn_rules.pcscf_addr_rules = 
      (ps_sys_attach_address_rule_enum_type)
        req_msg->attach_apn_pcscf_addr_rules;
	attach_info.attach_rules.attach_apn_rules.pcscf_addr_rules_valid = TRUE;
  }
  

  if (req_msg->attach_apn_pcscf_or_dns_addr_rules_valid)
  {
    attach_info.attach_rules.attach_apn_rules.pcscf_or_dns_addr_rules = 
      (ps_sys_attach_address_rule_enum_type)
        req_msg->attach_apn_pcscf_or_dns_addr_rules;
	attach_info.attach_rules.attach_apn_rules.pcscf_or_dns_addr_rules_valid = TRUE;
  }
  

  if (req_msg->on_demand_apn_ip_addr_rules_valid)
  {
    attach_info.attach_rules.on_demand_apn_rules.ip_addr_rules = 
      (ps_sys_attach_address_rule_enum_type)
        req_msg->attach_apn_ip_addr_rules;
	attach_info.attach_rules.on_demand_apn_rules.ip_addr_rules_valid = TRUE;
  }
  

  if (req_msg->on_demand_apn_dns_addr_rules_valid)
  {
    attach_info.attach_rules.on_demand_apn_rules.dns_addr_rules = 
      (ps_sys_attach_address_rule_enum_type)
        req_msg->on_demand_apn_dns_addr_rules;
	attach_info.attach_rules.on_demand_apn_rules.dns_addr_rules_valid = TRUE;
  }
  

  if (req_msg->on_demand_apn_pcscf_addr_rules_valid)
  {
    attach_info.attach_rules.on_demand_apn_rules.pcscf_addr_rules = 
      (ps_sys_attach_address_rule_enum_type)
        req_msg->on_demand_apn_pcscf_addr_rules;
	attach_info.attach_rules.on_demand_apn_rules.pcscf_addr_rules_valid = TRUE;
  }
  

  if (req_msg->on_demand_apn_pcscf_or_dns_addr_rules_valid)
  {
    attach_info.attach_rules.on_demand_apn_rules.pcscf_or_dns_addr_rules = 
      (ps_sys_attach_address_rule_enum_type)
        req_msg->on_demand_apn_pcscf_or_dns_addr_rules;
	attach_info.attach_rules.on_demand_apn_rules.pcscf_or_dns_addr_rules_valid = TRUE;
  }
  
 
  ps_sys_status = ps_sys_ioctl_ex(PS_SYS_TECH_3GPP,
  ...
  if (ps_sys_status < 0)
  {
    ...
    (void) ds_profile_delete_attach_per_sub(tech,
                                           profile_num,
                                           subs_id);

    resp_msg.profile_index_valid = FALSE;
  }
...
send_error:
  ...
  QMI_WDSI_ENCODE_RESPONSE_MSG(QMI_WDS_SET_ATTACH_PROFILE_INFO_RESP_V01,
                               &resp_msg,
                               sizeof(resp_msg),
                               errval,
                               &response);
  return response;
}/* qmi_wds_create_and_set_attach_profile_info() */
