...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/qmidata/src/ds_qmi_ctl.c#2 $
...
#define DS_QMI_CTL_ASSERT(expression) \
         ds_qmi_ctl_assert_wrapper(__LINE__, expression)

/*===========================================================================
...
{
  uint8                        svc_id;
  struct svc_avail_list_node   *next_ptr;
...
FUNCTION DS_QMI_CTL_ASSERT_WRAPPER()

DESCRIPTION
  Wrapper function for DS_QMI_CTL ASSERT
...
===========================================================================*/
static void ds_qmi_ctl_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("DS_QMI_CTL_ERR_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
}/* ds_qmi_ctl_assert_wrapper */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR_INT_1("Recd invalid inst_id 'd' in qmi_ctl_init",
                    (int32)qmi_inst);
    DS_QMI_CTL_ASSERT(0);
    return;
  }
...
/*-------------------------------------------------------------------------*/
  ...
  DS_QMI_CTL_ASSERT(cmd_ptr != NULL);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_UTILS_ERROR_FATAL("Recd NULL ptr in qmi_ctl_get_instance_id");
    return FALSE;
  }

  if (QMI_INSTANCE_MAX <= qmi_instance)
  {
    LOG_MSG_ERROR_INT_1("Recd invalid inst_id 'd' in qmi_ctl_get_instance_id",
                    (int32)qmi_instance);
    DS_QMI_CTL_ASSERT(0);
    return FALSE;
  }
...
/*-------------------------------------------------------------------------*/
  ...
  {
    LOG_MSG_ERROR_INT_1 ("Invalid QCTL cmd: d ", QMI_CTLI_CMD_VAL_GET_CLIENT_ID);
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_QMI_CTL_ASSERT( QMUX_SERVICE_MAX > svc_id );
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  -------------------------------------------------------------------------*/
  DS_QMI_CTL_ASSERT(sp && QMUX_SERVICE_MAX > svc_id);
  ...
  if((ctl_sp != NULL) && (ctl_sp->report_status.report_svc_avail))
  {
    ...
    else
    {
      ...
      if( FALSE == qmi_ctli_send( ctl_sp,
                                  ...
                                  ind ) )
      {
        LOG_MSG_ERROR_INT_0 ("Unable to generate Event Report IND");
        return FALSE;
      }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (NULL == sp)
  {
    DS_UTILS_ERROR_FATAL("Can't perform close on NULL svc ptr");
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (NULL == sp)
  {
    ...
    DS_UTILS_ERROR_FATAL("Recd NULL svc ptr, dropping message");
    goto send_result;
  }
  else if (NULL == sdu_in)
  {
    DS_UTILS_ERROR_FATAL("Recd NULL SDU ptr, dropping message");
    goto send_result;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  else if (FALSE == qmi_ctli_is_valid_client_id(sp,clid))
  {
    ...
    DS_UTILS_ERROR_FATAL("Recd pkt from invalid client, dropping message");
    return;
  }
  else if (NULL == sdu_in)
  {
    DS_UTILS_ERROR_FATAL("Recd NULL SDU ptr, bailing out");
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (NULL == sp)
  {
    ...
    DS_UTILS_ERROR_FATAL("Unable to send pkt on NULL svc ptr");
    return FALSE;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(cmd_ptr == NULL)
  {
    LOG_MSG_ERROR_INT_0 ("Unable to allocate QMI CTL cmd buf");
    errval = QMI_ERR_NO_MEMORY;
    goto send_result;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (NULL == sp)
  {
    LOG_MSG_ERROR_INT_0("Recd NULL svc ptr, dropping message");
    if ((NULL != sdu_in) && (NULL != *sdu_in))
    {
      dsm_free_packet(sdu_in);
    }
  ...
  {
    LOG_MSG_ERROR_INT_0("Recd pkt from invalid client, dropping message");
    if ((NULL != sdu_in) && (NULL != *sdu_in))
    {
      dsm_free_packet(sdu_in);
    }
  ...
  {
    LOG_MSG_ERROR_INT_0("Recd NULL SDU ptr, bailing out");
    return NULL;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR_INT_0("Data format already set by WDA");
    errval = QMI_ERR_INVALID_OPERATION;
    goto send_result;
  }
...
)
{
  ...
  if (NULL == sp)
  {
    DS_UTILS_ERROR_FATAL("Can't perform sync on NULL svc ptr");
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if( FALSE == qmi_ctl_global.inited )
  {
    DS_UTILS_ERROR_FATAL("Port init attempted before QMI init completed");
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if( NULL == ctl_sp ) 
  {
    DS_UTILS_ERROR_FATAL("Null ctl_sp in ctl_evt_rpt_ind");
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  for (index = 0 ; index < v_in_required->n_pwrsave_state;index++)
  {
    ...
    if( !spec_stored )
    {
      LOG_MSG_ERROR_INT_0 ("Error storing power save spec!");
      errval = QMI_ERR_INTERNAL;
      goto send_result;
    }
  }
  ...
  {
    LOG_MSG_ERROR_INT_0 ("Error: Invalid QMI INSTANCE!");
    errval = QMI_ERR_INTERNAL;
    goto send_result;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (QMI_INSTANCE_MAX <= qmi_inst)
  {
    LOG_MSG_ERROR_INT_1("Recd invalid inst_id 'd' in qmi_ctl_pwrsave_mode_change_notify",
                    (int32)qmi_inst);
    DS_QMI_CTL_ASSERT(0);
    return FALSE;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_UTILS_ERROR_FATAL("Mem alloc from system heap failed.");
  ...
  {
    LOG_MSG_ERROR_INT_0("Unable to issue a Online command ");
    errval = QMI_ERR_INTERNAL;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if( !response_reporting_spec )
  {
    LOG_MSG_ERROR_INT_0 ("Error: Invalid QMI Service Available List!");
    errval = QMI_ERR_INTERNAL;
    goto send_result;
  }
...
(
 ...
 /*-------------------------------------------------------------------------------*/
   ...
   if( !response_reporting_spec )
   {
     LOG_MSG_ERROR_INT_0 ("Error: Invalid QMI INSTANCE Found!");
     errval = QMI_ERR_INTERNAL;
     goto send_result;
   }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_UTILS_ERROR_FATAL("QMI_SVC list NULL!");
    return FALSE;
  }
  if (num_svcs == 0)
  {
    LOG_MSG_ERROR_INT_0 ("Incorrect Service Available list req!");
    return FALSE;
  }
  ...
  for (i=1; i<num_svcs; i++)
  {
    ...
    {
      while (current->next_ptr !=NULL && current->next_ptr->svc_id <= temp->svc_id)
      {
      ...
      temp->next_ptr = current->next_ptr;
...
/*-------------------------------------------------------------------------*/
  ...
  {
    DS_UTILS_ERROR_FATAL("QMI_SVC state pointer,svc list, num_svcs NULL!");
    return FALSE;
  }
  ...
  if(ctl_sp->svc_avail_list == NULL)
  {
    LOG_MSG_ERROR_INT_0 ("QMI Service Available List not initialized!");
    return FALSE;
  }
...
---------------------------------------------------------------------------*/
  ...
  while (svc_avail_list != NULL)
  {
    ...
    svc_avail_list = svc_avail_list->next_ptr;
  }
  *num_svcs = svc_len;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_QMI_CTL_ASSERT( QMUX_SERVICE_MAX > service_id );
  
  /* check that service_id is valid in case ASSERT is removed */
  if (service_id >= QMUX_SERVICE_MAX )
  {
    LOG_MSG_ERROR_INT_1("QMI service ID not valid (d)!  Discarding", service_id);
    return FALSE;
  }
  ...
  {
    LOG_MSG_ERROR_INT_0("QMI SVC Avail List is not initialized!");
    return FALSE;
  }
