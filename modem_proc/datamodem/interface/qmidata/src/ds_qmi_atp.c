...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/qmidata/src/ds_qmi_atp.c#2 $
  $DateTime: 2019/10/22 02:53:17 $

when        who    what, where, why
--------    ---    ----------------------------------------------------------
04/06/17    ss     Return appropriate error code when message is not suppoted
04/01/16    skc    Fixed KW issues.
...
{
  uint32 report_id;
  struct qmi_atpi_filter_report_status_s * next_ptr;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (DS_ATP_PROXY_ERR_NONE != atp_errno)
  {
    ...
    qmi_csi_send_resp(req_handle, msg_id, &resp_msg, 
                      sizeof(atp_policy_info_resp_msg_v01));
    PS_SYSTEM_HEAP_MEM_FREE(atp_policy);
    return QMI_CSI_CB_NO_ERR;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  new_filter->next_ptr =  clnt_info->filter_list;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  -------------------------------------------------------------------------*/
    ...
    if (filter_found)
    {
      ...
      if (fltr_prev == NULL)
      {
        cl_sp->filter_list = fltr->next_ptr;
      }
      else
      {
        fltr_prev->next_ptr = fltr->next_ptr;
      }
