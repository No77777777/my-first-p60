...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/qmidata/src/ds_qmi_if.c#5 $
...
#define DS_QMI_IF_ASSERT(expression) \
         ds_qmi_if_assert_wrapper(__LINE__, expression)
...
FUNCTION DS_QMI_IF_ASSERT_WRAPPER()

DESCRIPTION
  Wrapper function for DS_QMI_IF ASSERT
...
===========================================================================*/
static void ds_qmi_if_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("DS_QMI_IF_ERR_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
}/* ds_qmi_if_assert_wrapper */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR_INT_0 ("WWAN interface layer initialization failed!");
  ...
  {
    LOG_MSG_ERROR_INT_0("cm_ph_cmd_get_ph_info() req failed ");
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    /* CM client callback activation failed */
    DS_QMI_IF_ASSERT(0);
    return FALSE;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR_INT_0("CM client callback deactivation failed");
    return FALSE;
  }
...
)
{
  ...
  {
    LOG_MSG_ERROR_INT_0("Got invalid ds_qmi_wcdma_tbl");
    return;
  }
#else
  ...
  *tx_rate = 0;
  *rx_rate = 0;
  LOG_MSG_ERROR_INT_0("Invalid Channel data rate request");
#endif /* FEATURE_WCDMA */
...
)
{
 /* Range check */
  ...
  {
    LOG_MSG_ERROR_INT_1("qmi_if_is_subs_id_valid: Invalid subs_id: d.",subs_id);
    return FALSE;
  }
 return TRUE;
...
{
  return (qmi_if_mode_e_type)dsatcmdp_get_current_mode_per_subs((sys_modem_as_id_e_type)asub_id);
}
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  asub_id = (qmi_if_asubs_id_type)dsat_get_current_1x_subs_id();
  if(!qmi_if_is_subs_id_valid((qmi_if_asubs_id_type)asub_id))
  { 
    /*Lets take Default as primary*/
    LOG_MSG_INFO1_0("qmi_if_get_current_cdma_sub_id: return Default sub_id:0");
    asub_id = QMI_IF_AS_ID_1;
  }
  return asub_id;
}
...
)
{
  ...
  qmi_if_asubs_id_type asub_id;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  DS_QMI_IF_ASSERT (rx_rate && tx_rate);
  ...
  if(iface_ptr != NULL)
  {
    ...
    if(retval == 0)
    {
      if(((channel_rate_info.max_tx_bearer_data_rate >=0) &&
         ...
         ((channel_rate_info.data_rate_ex.max_tx_bearer_data_rate >=0) &&
         (channel_rate_info.data_rate_ex.max_rx_bearer_data_rate >=0)))
      {
        *tx_rate = (uint32) channel_rate_info.max_tx_bearer_data_rate;
        *rx_rate = (uint32) channel_rate_info.max_rx_bearer_data_rate;
        *tx_rate_ex = (uint64) channel_rate_info.data_rate_ex.max_tx_bearer_data_rate;
        *rx_rate_ex = (uint64) channel_rate_info.data_rate_ex.max_rx_bearer_data_rate;      
        *rate_type = channel_rate_info.data_rate_ex.rate_type;        
        return;
      }
    } 
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(retval == 0)
  {
    if(((channel_rate_info.current_tx_bearer_data_rate >=0) &&
       ...
       ((channel_rate_info.data_rate_ex.current_tx_bearer_data_rate >=0) &&
       (channel_rate_info.data_rate_ex.current_rx_bearer_data_rate >=0)))
    {
      *tx_rate = (uint32) channel_rate_info.current_tx_bearer_data_rate;
      *rx_rate = (uint32) channel_rate_info.current_rx_bearer_data_rate;
      *tx_rate_ex = (uint64) channel_rate_info.data_rate_ex.current_tx_bearer_data_rate;
      *rx_rate_ex = (uint64) channel_rate_info.data_rate_ex.current_rx_bearer_data_rate;      
      *rate_type = channel_rate_info.data_rate_ex.rate_type;
      return;
    }
  }
  ...
  *tx_rate = 0xFFFFFFFF;
  *rx_rate = 0xFFFFFFFF;
  *tx_rate_ex = 0xFFFFFFFF;
  *rx_rate_ex = 0xFFFFFFFF;      
} /* qmi_if_get_current_channel_rate() */
...
{
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  DS_QMI_IF_ASSERT (power_status && battery_lvl);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR_INT_1("CM SS event received for invalid subs_id: d. Discard",asubs_id);
    return;
  }
  ...
  switch (ss_event)
  {
    ...
    case CM_SS_EVENT_INFO:
    {
      ...
      {
        LOG_MSG_INFO2_4 ("Got new serving system info event (%x), changed_fields = "
                       "(0x%x) (0x%x), asubs_id %d",ss_event, 
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch (ph_event)
  {
    case CM_PH_EVENT_INFO_AVAIL:
      {
        ...
        qmi_if_info.ph_info.prl_info[asubs_id].prl_only  = ph_info.prl_info[asubs_id].prl_only;
    ...
    if (memcmp(&qmi_if_info.ph_info.rf_init_status, &ph_info.rf_init_status, sizeof(ph_info.rf_init_status)) ||
        (qmi_if_info.ph_info.is_gps_init_completed != ph_info.is_gps_init_completed))
        {
          qmi_if_info.ph_info.rf_init_status.error_code = ph_info.rf_init_status.error_code;
          qmi_if_info.ph_info.rf_init_status.status = ph_info.rf_init_status.status;
          qmi_if_info.ph_info.is_gps_init_completed = ph_info.is_gps_init_completed;
          qmi_dms_generate_device_state_info_ind(&qmi_if_info.ph_info.rf_init_status, qmi_if_info.ph_info.is_gps_init_completed);
        }
    ...
    case CM_PH_EVENT_INFO:
      qmi_if_info.ph_info.oprt_mode = ph_info.oprt_mode;
      qmi_if_info.ph_info.prl_info[asubs_id].is_prl_load_complete = 
                                   ph_info.prl_info[asubs_id].is_prl_load_complete;
      ...
      if ( (qmi_if_info.prl_info_valid[asubs_id]!= TRUE) && 
           ( ph_info.prl_info[asubs_id].is_prl_load_complete == TRUE ) )
      {
        qmi_if_info.ph_info.prl_info[asubs_id].prl_id   = ph_info.prl_info[asubs_id].prl_id;
        qmi_if_info.ph_info.prl_info[asubs_id].prl_only = ph_info.prl_info[asubs_id].prl_only;
        qmi_if_info.ph_info.prl_info[asubs_id].prl_source = ph_info.prl_info[asubs_id].prl_source;
        qmi_if_info.prl_info_valid[asubs_id] = TRUE;
    ...
    case CM_PH_EVENT_PRL_INIT:
      qmi_if_info.ph_info.prl_info[asubs_id].is_prl_load_complete = ph_info.prl_info[asubs_id].is_prl_load_complete;
      qmi_if_info.ph_info.prl_info[asubs_id].prl_id   = ph_info.prl_info[asubs_id].prl_id;
      qmi_if_info.ph_info.prl_info[asubs_id].prl_only = ph_info.prl_info[asubs_id].prl_only;
      qmi_if_info.ph_info.prl_info[asubs_id].prl_source = ph_info.prl_info[asubs_id].prl_source;
      qmi_if_info.prl_info_valid[asubs_id] = TRUE;  
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if (!ss_info_ptr) 
  {
    LOG_MSG_ERROR_INT_0("ss_info_ptr is NULL");
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if (!ph_info_ptr) 
  {
    LOG_MSG_ERROR_INT_0("ph_info_ptr is NULL");
    return;
  }
  
  asubs_id = (qmi_if_asubs_id_type)ph_info_ptr->asubs_id;
  if(!qmi_if_is_subs_id_valid(asubs_id))
  {
    return;
  }
  ...
  LOG_MSG_INFO2_2 ("qmi_if_cm_ph_event() ph_event %d on sub_id %d", ph_event,asubs_id);
  switch(ph_event)
  {
    case CM_PH_EVENT_INFO_AVAIL:
    {
      ...
      cmd_ptr->data.cm_ph.info.prl_info[asubs_id].prl_only = ph_info_ptr->prl_pref_only;
      cmd_ptr->data.cm_ph.asubs_id = asubs_id;
      cmd_ptr->data.cm_ph.info.oprt_mode = ph_info_ptr->oprt_mode;
      cmd_ptr->data.cm_ph.info.rf_init_status.error_code = ph_info_ptr->rf_init_status.error_code;
      cmd_ptr->data.cm_ph.info.rf_init_status.status = ph_info_ptr->rf_init_status.status;  
      cmd_ptr->data.cm_ph.info.is_gps_init_completed = ph_info_ptr->is_gps_init_completed;
    ...
    case CM_PH_EVENT_INFO:
    {
      ...
      cmd_ptr->data.cm_ph.event = ph_event;
      cmd_ptr->data.cm_ph.asubs_id = asubs_id;
      cmd_ptr->data.cm_ph.info.prl_info[asubs_id].is_prl_load_complete = ph_info_ptr->is_prl_load_complete;
      if ( ph_info_ptr->is_prl_load_complete == TRUE )
      {
        cmd_ptr->data.cm_ph.info.prl_info[asubs_id].prl_id = ph_info_ptr->prl_id;
        cmd_ptr->data.cm_ph.info.prl_info[asubs_id].prl_only = ph_info_ptr->prl_pref_only;
        cmd_ptr->data.cm_ph.info.prl_info[asubs_id].prl_source = ph_info_ptr->prl_source;
      }
      cmd_ptr->data.cm_ph.info.rf_init_status.error_code = ph_info_ptr->rf_init_status.error_code;
      cmd_ptr->data.cm_ph.info.rf_init_status.status = ph_info_ptr->rf_init_status.status;  
      cmd_ptr->data.cm_ph.info.is_gps_init_completed = ph_info_ptr->is_gps_init_completed;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if (!call_info_ptr) 
  {
    LOG_MSG_ERROR_INT_0("call_info_ptr is NULL");
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch(cnf_type)
  {
    case MMGSDI_CLIENT_ID_AND_EVT_REG_CNF:
      ...
      {
        /* MMGSDI session registration failed */
        DS_QMI_IF_ASSERT(0);
        return;
      }
...
SIDE EFFECTS
  None
===========================================================================*/
void qmi_if_mmgsdi_reg(void)
{
  static boolean mmgsdi_registered = FALSE;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  /*Check if DMS service is Initalized. If yes then register with MMGSDI else return.
    This is to avoid missing events in case of getting MMSGSDI events before DMS is initlized*/
  if( !(qmi_dms_is_inited() && (mmgsdi_registered == FALSE)))
  {
    return;
  }
  mmgsdi_registered = TRUE;
  ...
  {
    LOG_MSG_ERROR_INT_0("MMGSDI client id & event reg failed");
    DS_QMI_IF_ASSERT(0);
  }
  return;
}
...
)
{
  ...
  {
      LOG_MSG_ERROR_INT_1("qmi_if_mmgsdi_session_reg: Session Open GW Primary"
  ...
  if ( MMGSDI_SUCCESS != status )
  {
      LOG_MSG_ERROR_INT_1("qmi_if_mmgsdi_session_reg: Session Open 1X Primary"
                      "Registration Failed: d ", status);
  }
  ...
  if ( MMGSDI_SUCCESS != status )
  {
     LOG_MSG_ERROR_INT_1("qmi_if_mmgsdi_session_reg: Session Open GW Secondary"
                     "Registration Failed: d ", status);
  }
#endif
  ...
  if ( MMGSDI_SUCCESS != status )
  {
     LOG_MSG_ERROR_INT_1("qmi_if_mmgsdi_session_reg: Session Open GW Tertiary"
                     "Registration Failed: d ", status);
    return;
  }
#endif
...
)
{
  ...
  if (NULL == event) 
  {
    LOG_MSG_ERROR_INT_0("qmi_if_mmgsdi_session_evt_cb: event pointer is NULL ");
    return;
  }
  ...
  {
    LOG_MSG_ERROR_INT_1("qmi_if_mmgsdi_session_evt_cb:invalid session ID = d",
                    event->session_id);
    return;
  }
...
)
{
  ...
  if( NULL == cnf_ptr )
  {
    LOG_MSG_ERROR_INT_0("qmi_if_mmgsdi_session_open_cb: MMGSDI session open "
                    "failure as cnf_ptr is NULL");
    return;
  }
  if(cnf_ptr->session_open_cnf.response_header.client_data >= QMI_IF_MMGSD_MAX_SESSION)
  {
    LOG_MSG_ERROR_INT_1("qmi_if_mmgsdi_session_open_cb: MMGSDI session open "
                    "failure as client data invalid [d]",
...
static void qmi_if_determine_tds_rates(uint32* tx_rate, uint32* rx_rate)
{
  ...
  {
    LOG_MSG_ERROR_INT_1("EFS read on TDS RRC HSDPA Cat failed errno=d",
  ...
  if ((efs_ret != MCFG_FS_STATUS_OK))
  {
    LOG_MSG_ERROR_INT_1("EFS read on TDS RRC HSUPA Cat failed errno=d",
                    mcfg_fs_errno( file_type ));
  }
...
/*-------------------------------------------------------------------------*/
  ...
  switch(cmd_buf_ptr->cmd_id)
  {
    case QMI_CMD_CM_CLIENT_ACTIVATE:
      ...
      {
        LOG_MSG_ERROR_INT_0("cm_client_act() failed, couldn't initialize QMI_IF module!");
      }
      break;
    case QMI_CMD_PROCESS_OTKSL_FLAG_UPDATE:
      qmi_if_process_otksl_flag_update(
    ...
    case QMI_CMD_CDMA_LOCK_MODE:
      qmi_if_process_cm_ph_event( cmd_buf_ptr->data.cm_ph.event,
                                  ...
                                  cmd_buf_ptr->data.cm_ph.asubs_id);
      break;
