...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/qmidata/src/ds_qmi_fw_common.c#2 $
...
#define DS_QMI_FW_COMMON_ASSERT(expression) \
         ds_qmi_fw_common_assert_wrapper(__LINE__, expression)
...
FUNCTION DS_QMI_FW_COMMON_ASSERT_WRAPPER()

DESCRIPTION
  Wrapper function for DS_QMI_FW_COMMON ASSERT
...
===========================================================================*/
static void ds_qmi_fw_common_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("DS_QMI_FW_COMMON_ERR_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
}/* ds_qmi_fw_common_assert_wrapper */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (x_p == NULL)
  {
    LOG_MSG_ERROR_INT_1("No transaction container! Discarding service [d] message",
  ...
  while (sdu_in)
  {
    ...
    if (temp == -1)
    {
      LOG_MSG_ERROR_INT_0("Short QMI svc header! Discarding transaction");
      dsm_free_packet( &sdu_in );
    ...
    {
      LOG_MSG_ERROR_INT_1("Short QMI svc payload! Discarding service [d] message",
                      msg_hdr->common_hdr.service);
      dsm_free_packet( &sdu_in );
      ds_qmi_fw_free_transaction( &x_p );
      return;
    ...
    else if (sdu_len < remaining_bytes)
    {
      ...
      ---------------------------------------------------------------------*/
      DS_QMI_FW_COMMON_ASSERT (next_sdu == NULL);
    ...
    -----------------------------------------------------------------------*/
    DS_QMI_FW_COMMON_ASSERT (x_p != NULL);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR_INT_1("Unable to send Response for cmd: d", cmd_buf_p->cmd_type);
    dsm_free_packet(&msg_ptr);
    ds_qmi_fw_free_cmd_buf(&cmd_buf_p);
    return FALSE;
  }
  ...
  {
    LOG_MSG_ERROR_INT_0("Unable to push pending response to send queue");
    dsm_free_packet(&msg_ptr);
    ds_qmi_fw_free_cmd_buf(&cmd_buf_p);
    return FALSE;
  }

  DS_QMI_FW_COMMON_ASSERT (cmd_buf_p == NULL && msg_ptr == NULL);
  ...
  for( i = 0; i < x_p->n_cmds; i++ )
  {
    DS_QMI_FW_COMMON_ASSERT( x_p->resp_list[i] != QMI_SVC_RESPONSE_PENDING );
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR_INT_0( "Unable to send QMI indication to QMI Framework" );
    dsm_free_packet(&ind);
    return FALSE;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (x_p->cl_sp)  // cl_sp != NULL means this transaction is client specific
  {
    ...
    if (NULL == cmd_buf_p)
    {
      LOG_MSG_ERROR_INT_0("No command buffers available!");
      return NULL;
    }
  }
  else
  {
      LOG_MSG_ERROR_INT_0("Invalid state pointer !");
      return NULL;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
      LOG_MSG_ERROR_INT_0("Invalid client state pointer!");
      return NULL;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if( (*cmd_buf_p) != x_p->cmd_list[(*cmd_buf_p)->cmd_no] )
  {
    ...
    -----------------------------------------------------------------------*/
    /* Command is linked to transaction it doesn't belong to! discarding */
    DS_QMI_FW_COMMON_ASSERT(0);
    dsm_free_packet( msg );
    ds_qmi_fw_free_cmd_buf( cmd_buf_p );
    return FALSE;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_QMI_FW_COMMON_ASSERT (svc_sp && cl_sp);
  /*-----------------------------------------------------------------------
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_QMI_FW_COMMON_ASSERT (cl_sp != NULL);
  /*-------------------------------------------------------------------------
