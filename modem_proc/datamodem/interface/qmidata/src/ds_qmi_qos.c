...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/qmidata/src/ds_qmi_qos.c#4 $ $DateTime: 2023/03/20 00:18:48 $ $Author: pwbldsvc $
...
#define QOSI_MAX_QOS_ID (0xFFFFFFFF)
...
#define DS_QMI_QOS_ASSERT(expression) \
         ds_qmi_qos_assert_wrapper(__LINE__, expression)
...
typedef enum
{
  ...
  QOSI_CMD_VAL_GLOBAL_QOS_FLOW_EX_IND        = 0x0034,
  QOSI_CMD_VAL_GET_QOS_INFO_EX               = 0x0035,
  QOSI_CMD_VAL_GET_QOS_IDS                   = 0x0036,
...
typedef struct qmi_qosi_client_state_s
{
  ...
  struct qmi_qosi_client_state_s * next_ptr;    // Link to next client
  ...
  struct
  {
    ...
    boolean report_global_flow_ex_segmented;
    boolean suppress_flow_ctl_report;
  } report_status;
  qmi_qosi_subs_enum_type        subscription_id; //Subs id of the client
  /* Unique for the client
     Usage in qmi_qos_global_flow_ex_ind */
  uint32                         tx_id;
...
typedef struct qmi_qosi_state_s
{
  ...
  -------------------------------------------------------------------------*/
  void *  flow_evt_buf_ptr;
  void *  iface_evt_buf_ptr;

  uint8   num_qos_reqs;
  boolean network_qos_aware_status;
  boolean flow_ev_reg;
  uint16  uplink_fc_seq_num; // Default flow Seq No
  uint8   last_fc_ev; // Last non-suppressed flow event
...
static dsm_item_type*  qmi_qosi_get_qos_ids( void*,
                                             void*,
                                             void*,
                                             dsm_item_type**);

static dsm_item_type*  qmi_qosi_get_qos_info_ex( void*,
                                                 void*,
                                                 void*,
                                                 dsm_item_type**);
...
static void qmi_qosi_powersave_cb
(
  uint32  ep_id,
  uint32  mask
);
...
static void qosi_set_offload_fc
(
  qmi_qosi_state_type *   qos_sp,
  qmi_flow_info_type  **  qmi_flow_info_ptr_ptr,
  uint16                  num_qos_ids,
...
static void  qmi_qosi_get_svc_info_cb
(  
  uint8  clid,
  qmi_svc_info_type * svc_info  
);

static int qosi_compare_powersave_ep_id
( 
  void *item_ptr,
  void *compare_val 
);

static void qosi_generate_indication_for_qos_ids
(
  qmi_qosi_client_state_type   *cl_sp,
  uint32                        qos_id
);

static void qosi_generate_indication_for_qos_ids_hdlr
(
  qmi_qosi_client_state_type   *cl_sp,
  uint32                        tx_id,
  uint32                        qos_id
...
static void qmi_qosi_fill_common_info_per_segment
(
  qos_global_qos_flow_ind_msg_v01    * ind_msg,
  qos_global_qos_flow_ex_ind_msg_v01 * ex_ind_msg,  
  boolean                              is_first_segment,
  uint32                             * common_msg_size,
  uint32                               tx_id
);

static void qmi_qosi_fill_qos_filter
(
  qos_global_qos_flow_ex_ind_msg_v01  *ex_ind_msg,
  qos_global_qos_flow_ind_msg_v01     *ind_msg,
  uint8                                start_idx,
...
static void qmi_qosi_segment_global_qos_flow_ind
(
  qos_global_qos_flow_ind_msg_v01 * ind_msg,
  qmi_qosi_state_type             * qos_sp,
  qmi_qosi_client_state_type      * cl_sp,
  uint32                            tx_id,
  boolean                           is_last_flow
);

/*===========================================================================

FUNCTION DS_QMI_QOS_ASSERT_WRAPPER()

DESCRIPTION
  Wrapper function for DS_QMI_QOS ASSERT
...
===========================================================================*/
static void ds_qmi_qos_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("DS_QMI_QOS_ERR_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
}/* ds_qmi_qos_assert_wrapper */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  uint32                          idl_major_ver = QOSI_BASE_VER_MAJOR;
  uint32                          idl_minor_ver = QOSI_BASE_VER_MINOR;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /* Retrieving IDL major and minor version for QMI QOS */
#ifndef TEST_FRAMEWORK
  qmi_idl_get_idl_version(qos_get_service_object_v01(),&idl_major_ver);
  qmi_idl_get_idl_minor_version(qos_get_service_object_v01(),&idl_minor_ver);
#endif /* TEST_FRAMEWORK */  
  qmi_qosi_cfg.fw_cfg.base_version.major     = idl_major_ver;
  qmi_qosi_cfg.fw_cfg.base_version.minor     = idl_minor_ver;
  ...
  qmi_qosi_cfg.fw_cfg.cbs.get_svc_info       = qmi_qosi_get_svc_info_cb;
...
/*===========================================================================
  FUNCTION QMI_QOS_GET_HANDLE_FROM_PHY_LINK_PTR()

  DESCRIPTION
    Given a flow ptr it returns the corresponding qos handle

  PARAMETERS
    ps_iface_ptr    :  Iface ptr
    phys_link_ptr   :  Phys link pointer to be matched
    qos_handle      : QOS handle corresponding to flow handle
  ...
  SIDE EFFECTS
    None
===========================================================================*/
boolean  qmi_qos_get_handle_from_phys_link_ptr
(
  ps_iface_type          *ps_iface_ptr,
  ps_phys_link_type      *phys_link_ptr,
  uint32                 *qos_handle
)
{
  ...
  /*---------------------------------------------------------------------
    Check if the phys link corresponds to any Netowork initiated QMI QoS flows. Also check iface
    type (In case of DUAL IP both v4 and v6 iface will have same physlink)
  ---------------------------------------------------------------------*/
  for ( index  = 0; index < QOSI_MAX_NETWORK_INIT_FLOW_NUM; index++ )
  {
    if ( (PS_FLOW_GET_PHYS_LINK(nw_init_qosi_flow_info[index].flow_ptr) == phys_link_ptr) &&
         (PS_FLOW_GET_IFACE(nw_init_qosi_flow_info[index].flow_ptr) == ps_iface_ptr))
    {
      *qos_handle = nw_init_qosi_flow_info[index].qos_handle;
      return TRUE;
    }
  }
  /*---------------------------------------------------------------------
    Check if the phys link corresponds to any UE initiated QMI QoS flows
  ---------------------------------------------------------------------*/
  for (index =0; index < QOSI_MAX_QOS_REQUESTS; index++)
  {
    if(PS_FLOW_GET_PHYS_LINK(qosi_flow_info[index].flow_ptr) == phys_link_ptr)
    {
      *qos_handle = qosi_flow_info[index].qos_handle;
      return TRUE;
    }
  }

  return FALSE;
}/* qmi_qos_get_handle_from_flow_ptr */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if( cmd_ptr == NULL)
  {
    LOG_MSG_ERROR_INT_0("Unable to allocate QMI QOS cmd buf");
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_QMI_QOS_ASSERT(common_msg_hdr != NULL);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_QMI_QOS_ASSERT(msg_hdr != NULL);
...
/*===========================================================================
FUNCTION qmi_qosi_get_svc_info_cb()

DESCRIPTION
  Function returns subscription id associated with client id.
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  cl_sp = qmi_qosi_get_cl_sp_by_clid(NULL, clid);    
  if (cl_sp) 
  {
...
{  
  ps_iface_event_enum_type      qos_iface_event_list[] = { IFACE_707_NETWORK_SUPPORTED_QOS_PROFILES_CHANGED_EV };
  ...
  if(qmi_qosi_global.inited == FALSE)
  {
    ...
    /* IFACE, QOS AWARE, UNAWARE events */

    qmi_qosi_global.iface_qos_ev_buf_ptr =
      ps_iface_alloc_event_handle(NULL,
                                  qosi_iface_ev_cb,
                                     NULL );
    if (qmi_qosi_global.iface_qos_ev_buf_ptr == NULL)
    {
      /* Allocation failed for iface events */
      DS_QMI_QOS_ASSERT(0);
    }

    qmi_qosi_global.phys_link_ev_buf_ptr =
      ps_phys_link_alloc_event_handle(NULL,
                                      qosi_phys_link_ev_cb,
                                      NULL);
    if (qmi_qosi_global.phys_link_ev_buf_ptr == NULL)
    {
      /* Allocation failed for phys_link events */
      DS_QMI_QOS_ASSERT(0);
    }
    /* Register for AWARE/UNAWARE IFACE events */
  
    if (0 != ps_iface_subscribe_event_list(qmi_qosi_global.iface_qos_ev_buf_ptr,
                                           qos_iface_event_list,
                                           sizeof(qos_iface_event_list)/sizeof(ps_iface_event_enum_type)))
    {
      /* Subscribe event list failed! */
      DS_QMI_QOS_ASSERT(0);
      return;
    }
    ...
    qmi_wds_set_call_status_handler(qosi_call_status_cb, QMI_WDS_CALL_STATUS_TYPE_QOS);

    /*-------------------------------------------------------------------------
       Register to listem powersave mode change from WDA
    -------------------------------------------------------------------------*/
    memset(&qmi_qosi_global.powersave_clnt_q, 0x0, sizeof(q_type));
    (void) q_init(&qmi_qosi_global.powersave_clnt_q);
    qmi_wda_reg_powersave_mode(QMI_WDA_POWERSAVE_CONFIG_FLOW_CTL,
                               qmi_qosi_powersave_cb);
...
/*-------------------------------------------------------------------------*/
  ...
  {
    LOG_MSG_ERROR_INT_1("Invalid qos inst: d", qos_inst);
    DS_QMI_QOS_ASSERT(0);
    return FALSE;
  }
...
/*-------------------------------------------------------------------------*/
  ...
  if (qmi_qosi_qos_inst_bind(qos_inst, ep_id, 0))
  {
    ...
    if (cl_sp != NULL)
    {
      LOG_MSG_ERROR_INT_1("clid [d] exists", common_msg_hdr->client_id);
    ...
    if (cl_sp)
    {
      ...
      cl_sp->next_ptr = qos_sp->client_list;
      ...
      if(cl_sp != NULL)
      {
        LOG_MSG_INFO2_3("Allocated QOS clid %d QMI inst %d cl_sp %p", 
...
/*-------------------------------------------------------------------------*/
  ...
  if (cl_sp == NULL)
  {
    LOG_MSG_ERROR_INT_1("Can't find client: d", common_msg_hdr->client_id);
  ...
  while (cl_sp != NULL)
  {
    if (cl_sp->common.clid == common_msg_hdr->client_id)
    {
      ...
      if (cl_sp_prev == NULL)
      {
        qos_sp_v4->client_list = cl_sp->next_ptr;
      }
      else
      {
        cl_sp_prev->next_ptr = cl_sp->next_ptr;
      }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    dsm_free_packet(&sdu);
    LOG_MSG_ERROR_INT_1("Can't find client: d",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    curr_qos_sp_v4->client_list = cl_sp->next_ptr;
    curr_qos_sp_v6->client_list = cl_sp->next_ptr;
  ...
  else
  {
    ...
    {
      if (cl_sp_prev->next_ptr == cl_sp)
      {
        break;
      }
      cl_sp_prev = cl_sp_prev->next_ptr;
    }
    if (cl_sp_prev == NULL)
    {
  ...
  cl_sp->next_ptr = bind_qos_sp_v4->client_list;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (cl_sp == NULL)
  {
    ...
    while ( (loop_cl_sp != NULL) && (ret == TRUE) )
    {
      ...
      loop_cl_sp = loop_cl_sp->next_ptr;
    } /* while */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR_INT_1("Failed to alloc qos sp: d", qos_inst);
    return FALSE;
  }
  ...
  {
    DS_QMI_QOS_ASSERT(qos_sp_v4->binding.ep_id == 0);
    // Bind qos instance
    ...
    LOG_MSG_INFO1_3("Binding QOS inst %d to ep_id 0x%x mux_id %d",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_QMI_QOS_ASSERT(sp && cmd_buf_p && cl_sp && sdu_in);
...
/*===========================================================================
  FUNCTION qmi_qosi_log_fc_event()

  DESCRIPTION
    Logs the Flow control event with qxdm

  PARAMETERS
    qos_sp            : service provided state pointer (user data)
    qmi_flow_info_ptr : Array of flow ptr
    num_qos_ids       : No of qos flow
  ...
  SIDE EFFECTS
    None
===========================================================================*/
static void qmi_qosi_log_fc_event
(
  qmi_qosi_state_type *         qos_sp,
...
)
{
  uint32                 qos_handle;
  uint16                 cnt_flow;
  dpl_log_fc_ev_type  log_ev_info; 
  ps_iface_type        * um_iface_ptr = NULL;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  
  for(cnt_flow = 0; cnt_flow < num_qos_ids; cnt_flow++)
  {
    memset(&log_ev_info, 0, sizeof(log_ev_info));
    if(qmi_flow_info_ptr[cnt_flow] != NULL)
    {
      qos_handle = qmi_flow_info_ptr[cnt_flow]->qos_handle;
    }
    else
    {
      qos_handle = 0; //default flow
    }
    if (QOSI_REPORT_STATUS_FLOW_ENABLED == flow_change_ev)
    {
      log_ev_info.fc_state = 0;
    }
    else
    {
      log_ev_info.fc_state = 1;
    }
    log_ev_info.ep_id = qos_sp->binding.ep_id;
    log_ev_info.mux_id = qos_sp->binding.mux_id;
    log_ev_info.rmnet_inst = qos_sp->binding.rmnet_inst;
    log_ev_info.subs_id = qos_sp->binding.subs_id;
    if (qmi_flow_info_ptr[cnt_flow] && 
        NULL != qmi_flow_info_ptr[cnt_flow]->flow_ptr)
    {
      log_ev_info.flow_ptr = (uint32)qmi_flow_info_ptr[cnt_flow]->flow_ptr;    
      log_ev_info.phys_link_ptr = 
        (uint32)PS_FLOW_GET_PHYS_LINK(qmi_flow_info_ptr[cnt_flow]->flow_ptr);      
    }

    um_iface_ptr = PS_IFACE_GET_BASE_IFACE(qos_sp->ps_iface_ptr);
    if (NULL != um_iface_ptr)
    {
      log_ev_info.addr_family = ps_iface_get_addr_family(um_iface_ptr);
      if (IPV4_ADDR == log_ev_info.addr_family)
      {
        log_ev_info.v4_iface_name_um = um_iface_ptr->name;
        log_ev_info.v4_iface_inst_um = um_iface_ptr->instance;
      } 
      else  
      {
        log_ev_info.v6_iface_name_um = um_iface_ptr->name;
        log_ev_info.v6_iface_inst_um = um_iface_ptr->instance;   
      }
      //log_ev_info.eps_id = 
      //  ps_dpm_get_default_eps_id(qos_sp->binding.mux_id, ps_iface_get_addr_family(um_iface_ptr));
      
      if (UMTS_IFACE == um_iface_ptr->name)
      {
        log_ev_info.profile_id = um_iface_ptr->iface_private.profile_id_3gpp;
      }
      else
      {
        log_ev_info.profile_id = um_iface_ptr->iface_private.profile_id_3gpp2;    
      }
      log_ev_info.fc_iface_mask = um_iface_ptr->iface_private.tx_flow_mask;
    }
    
    if (qmi_flow_info_ptr[cnt_flow] && 
        NULL != qmi_flow_info_ptr[cnt_flow]->flow_ptr)
    {
      log_ev_info.fc_flow_mask = 
        PS_FLOWI_GET_TX_MASK(qmi_flow_info_ptr[cnt_flow]->flow_ptr);
      if (((ps_phys_link_type *)log_ev_info.phys_link_ptr))
      {
        log_ev_info.fc_phys_link_mask = 
          ((ps_phys_link_type *)log_ev_info.phys_link_ptr)->phys_private.tx_flow_mask;         
      }  
    } 
    log_ev_info.allowed_num_bytes = 0xFFFFFFFF; // for traditional flow control, allowed num bytes is meaningless
    dpl_log_fc_event_pkt(&log_ev_info);
  }
} /* qmi_qosi_log_fc_event() */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
        ...
        while (*sdu_in)
        {
          ...
          switch (type)
          {
            ...
            case QMI_PRM_TYPE_TX_QOS_FLOW_REQ:
            {
              ...
              DS_QMI_QOS_ASSERT(flow_count <= QOSI_MAX_QOS_FLOWS_PER_REQUEST);
  ...
  {
    LOG_MSG_ERROR_INT_0("QMI QoS SP is NULL!");
    ps_iface_ptr = NULL;
  }
  ...
  if(return_val == 0) //IOCTL succeeded
  {
    ...
    for(j = 0; j < qos_req_ex.num_qos_specs; j++)
    {
      ...
      {
        LOG_MSG_ERROR_INT_0("Invalid handle returned by handle mgr");
      ...
      qosi_flow_info[i].uplink_fc_seq_num = 0;
      qosi_flow_info[i].last_fc_ev = QOSI_REPORT_STATUS_FLOW_ENABLED;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ( (got_v_opt_req_prim == TRUE) && (v_opt_req_primary == TRUE) )
  {
    ...
    DS_QMI_QOS_ASSERT (def_flow_ptr != NULL);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ( (got_v_opt_req_prim) && (v_opt_req_primary == TRUE) )
  {
    ...
    DS_QMI_QOS_ASSERT (flow_ptr !=NULL);
  }
  else
  {
  ...
  /*-------------------------------------------------------------------------
    Check if the given qos_handle is valid
  -------------------------------------------------------------------------*/
    for(i=0; i < QOSI_MAX_QOS_REQUESTS; i++)
    {
...
)
{
  ...
  boolean              retval;
  boolean              got_v_in_reqd;
  qmi_qosi_state_type  *  qos_sp = (qmi_qosi_state_type *) sp;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    if((qosi_flow_info[i].qos_handle == v_in_reqd.qos_handle) &&
       (PS_FLOW_GET_IFACE(qosi_flow_info[i].flow_ptr) == qos_sp->ps_iface_ptr))
    {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    {
      LOG_MSG_ERROR_INT_1("ps_sys_conf_get_ex: operation failed:(d)",ps_errno);
      switch (ps_errno)
      {                                   
        case DS_NOMEMORY:
          errval = QMI_ERR_NO_MEMORY;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_QMI_QOS_ASSERT(qos_sp && cl_sp);
...
#define FILL_OPER_FAIL_TLV(BIT, ERROR) \
  ...
  if (v_out_oper_failure.num_failures > 0)
  {
    ...
    DS_QMI_QOS_ASSERT (v_out_oper_failure.num_failures <= QMI_QOSI_REQ_OPER_MAX);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  switch(info_code)
  {
    ...
    default:
      LOG_MSG_ERROR_INT_1("Unrecognized extended info code (d)", info_code);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  for(i = 0; i < num_qos_ids; i++)
  {
    ...
    LOG_MSG_INFO2_3("sending flow event %d for handle 0x%x subs_id %d",
                      flow_change_ev,
                      v_out1.qos_handle,
    ...
    switch(flow_change_ev)
    {
      case QOSI_REPORT_STATUS_FLOW_ACTIVATED:
      {
        ...
        {
          LOG_MSG_ERROR_INT_0("Flow ptr NULL, unable to send QoS event report indication");
          return;
        }
        ...
        bearer_id = PS_IFACE_GET_BEARER_ID_EX_FROM_PHYSLINK(
            PS_FLOW_GET_IFACE(qmi_flow_info_ptr[i]->flow_ptr),
            phys_link_ptr);
      ...
      case QOSI_REPORT_STATUS_FLOW_SUSPENDED:
      {
        ...
        if(qosi_append_filter_spec(&ind,
                                   ...
                                   QMI_PRM_TYPE_RX_QOS_FILTER_REQ) != QMI_ERR_NONE)
        {
          LOG_MSG_ERROR_INT_0("Error returned from qosi_append_filter_spec");
          dsm_free_packet(&ind);
          return;
        }
        total_bytes_pushed += num_bytes_pushed;
      ...
      default:
        LOG_MSG_ERROR_INT_1("Unhandled flow change ev [d] received in event_rpt_ind",
                        flow_change_ev);
        return;
    }
...
/*===========================================================================
  FUNCTION QOSI_IS_LAST_FLOW_FOR_SEGMENTS()

  DESCRIPTION
    This function check if there are more flows.

  PARAMETERS
    qos_flow_info_ptr  : QoS Flow Pointer
    num_qos_ids        : Number of QoS IDs
    flow_change_ev     : flow change event
    start_index        : Qos Index to start

  RETURN VALUE
    TRUE  if it is last flow
    FALSE if there are more flows
  ...
  SIDE EFFECTS
    None
===========================================================================*/
static boolean qosi_is_last_flow_for_segments
(
  qmi_flow_info_type **         qmi_flow_info_ptr,
  uint8                         num_qos_ids,
  uint8                         flow_change_ev,
...
{
  uint8 qos_index = 0;
  boolean is_last_flow = TRUE;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

  for (qos_index = start_index; qos_index < num_qos_ids; qos_index++)
  {
    if (qmi_flow_info_ptr[qos_index] != NULL)
    {
  ...
  return is_last_flow;
}/* qosi_is_last_flow_for_segments */

/*===========================================================================
  FUNCTION QMI_QOSI_GLOBAL_QOS_FLOW_IND()
  ...
  SIDE EFFECTS
    None
===========================================================================*/
static void qmi_qosi_global_qos_flow_ind
(
  qmi_qosi_state_type          *qos_sp,
  qmi_qosi_client_state_type   *cl_sp,
  qmi_flow_info_type **         qmi_flow_info_ptr,
...
)
{
  ...
  qmi_qosi_powersave_info_type  *powersave_info_ptr;
  boolean                        is_last_flow = TRUE;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  /*-------------------------------------------------------------------------
    Do not send flow enabled/disabled indication if powersave mode is enabled
  -------------------------------------------------------------------------*/
  if(NULL == qos_sp)
  {
    return;
  ...
  powersave_info_ptr = q_linear_search(&qmi_qosi_global.powersave_clnt_q,
                                       qosi_compare_powersave_ep_id,
                                       &qos_sp->binding.ep_id);

  if( (NULL != powersave_info_ptr) &&
      (0 != (powersave_info_ptr->mask & QMI_WDA_POWERSAVE_CONFIG_FLOW_CTL)) &&
      (flow_change_ev == QOSI_REPORT_STATUS_FLOW_ENABLED ||
       flow_change_ev == QOSI_REPORT_STATUS_FLOW_DISABLED) )
  {
    LOG_MSG_INFO2_2("qmi_qosi_global_qos_flow_ind(): indication is suppressed "
                    "due to powersave mode, mask %x, event %d",
                     powersave_info_ptr->mask,
                     flow_change_ev);
    return;
  }
  ...
  for(i = 0; i < num_qos_ids; i++)
  {
    ...
    if(qmi_flow_info_ptr[i] != NULL)
    {
      ...
      if(cl_sp->report_status.report_global_flow_ex_segmented &&
         (flow_change_ev == QOSI_REPORT_STATUS_FLOW_ENABLED ||
          flow_change_ev == QOSI_REPORT_STATUS_FLOW_DISABLED))
      {
        /* Not supporting new indication for FLOW Enabled and Disable event */
        continue;
      }
    }
    else
    {
      if(cl_sp->report_status.report_global_flow_ex_segmented)
      {
        /* Not supporting new indication for default flow */
        continue;
      }
    ...
    switch(ind_msg->qos_flow_state.state_change)
    {
      case QOS_FLOW_ACTIVATED_V01:
      {
        ...
        ind_msg->bearer_id = PS_IFACE_GET_BEARER_ID_EX_FROM_PHYSLINK(
            PS_FLOW_GET_IFACE(qmi_flow_info_ptr[i]->flow_ptr),
            phys_link_ptr);
      ...
      case QOS_FLOW_SUSPENDED_V01:
      {
        ...
        if(qosi_append_filter_spec_ex(qos_sp->ps_iface_ptr,
                                      ...
                                      ind_msg->rx_qos_filters) != QMI_ERR_NONE_V01)
        {
          LOG_MSG_ERROR_INT_0("Error returned from qosi_append_filter_spec_ex");
          PS_SYSTEM_HEAP_MEM_FREE(ind_msg);
          return;
        }
      }
    ...
    if(cl_sp->report_status.report_global_flow_ex)
    {
    ...
    else if(cl_sp->report_status.report_global_flow_ex_segmented)
    {
      is_last_flow = qosi_is_last_flow_for_segments(qmi_flow_info_ptr, num_qos_ids, flow_change_ev, (i + 1));
      qmi_qosi_segment_global_qos_flow_ind(ind_msg, qos_sp, cl_sp, tx_id, is_last_flow);
    }
  }
  PS_SYSTEM_HEAP_MEM_FREE(ind_msg);
} /* qmi_qosi_global_qos_flow_ind() */

/*===========================================================================
  FUNCTION QMI_QOSI_FILL_COMMON_INFO_PER_SEGMENT()

  DESCRIPTION
    This function fills qos flow info for etended indication message.

  PARAMETERS
    ind_msg          : input qos flow pointer
    ex_ind_msg       : output qos flow pointer
    is_first_segment : Flag for first segment
    common_msg_size  : common field size
  ...
  DEPENDENCIES
    Depends on qos_global_qos_flow_ind_msg_v01.
...
{
  /* Type size 1, Length size 2*/
  #define QMI_QOS_TL_SIZE 3
  /* Type size 1, Length size 2 Number size 1*/
  #define QMI_QOS_TL_LIST_SIZE 4
  uint32                         msg_size = 0;
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

  if(NULL == ind_msg || NULL == ex_ind_msg || NULL == common_msg_size)
  {
    return;
  }

  /* Fill TLVs */
  msg_size += sizeof(ex_ind_msg->qos_flow_state) + QMI_QOS_TL_SIZE;
  ex_ind_msg->qos_flow_state = ind_msg->qos_flow_state;  

  if (is_first_segment)
  {
    if (ind_msg->tx_qos_flow_granted_valid)
    {
      msg_size += sizeof(ex_ind_msg->tx_qos_flow_granted) + QMI_QOS_TL_SIZE;
      //TODO:Send segment if it is exceeding segment max size
      ex_ind_msg->tx_qos_flow_granted_valid = TRUE;
      ex_ind_msg->tx_qos_flow_granted = ind_msg->tx_qos_flow_granted;
   }

    if (ind_msg->rx_qos_flow_granted_valid)
    {
      msg_size += sizeof(ex_ind_msg->rx_qos_flow_granted) + QMI_QOS_TL_SIZE;
      //TODO:Send segment if it is exceeding segment max size
      ex_ind_msg->rx_qos_flow_granted_valid = TRUE;
      ex_ind_msg->rx_qos_flow_granted = ind_msg->rx_qos_flow_granted;
    }

    if (ind_msg->flow_type_valid)
    {
      msg_size += sizeof(ex_ind_msg->flow_type) + QMI_QOS_TL_SIZE;
      //TODO:Send segment if it is exceeding segment max size
      ex_ind_msg->flow_type_valid = TRUE;
      ex_ind_msg->flow_type = ind_msg->flow_type;
    }

    if (ind_msg->bearer_id_valid)
    {
      msg_size += sizeof(ex_ind_msg->bearer_id) + QMI_QOS_TL_SIZE;
      //TODO:Send segment if it is exceeding segment max size
      ex_ind_msg->bearer_id_valid = TRUE;
      ex_ind_msg->bearer_id = ind_msg->bearer_id;
    }
  }

  msg_size += sizeof(ex_ind_msg->tx_id) + QMI_QOS_TL_SIZE;
  ex_ind_msg->tx_id = tx_id;

  msg_size += sizeof(ex_ind_msg->is_last_segment) + QMI_QOS_TL_SIZE;

  /* Including size of tx and rx qos filters  headers */
  msg_size += 2 * QMI_QOS_TL_LIST_SIZE;

  *common_msg_size = msg_size;

  return; 
}/* qmi_qosi_fill_common_info_per_segment */

/*===========================================================================
  FUNCTION QMI_QOSI_FILL_QOS_FILTER()

  DESCRIPTION
    This function fills TX or RX QOS filters.

  PARAMETERS
    ind_msg          : input qos flow pointer
    ex_ind_msg       : output qos flow pointer
    start_idx        : Offset to filters
    num_filters      : number of filters
    is_tx            : is TX qos filter
...
)
{
  ...
  if(NULL == ex_ind_msg || NULL == ind_msg)
  {
    return;
  }

  /* Include TX QoS Filter */
  if(is_tx)
  {
    ex_ind_msg->tx_qos_filters_valid = TRUE;
    ex_ind_msg->tx_qos_filters_len = num_filters;
    for(i = 0; i < num_filters; i++)
    {		  	
      ex_ind_msg->tx_qos_filters[i] = ind_msg->tx_qos_filters[start_idx + i];
    }
  }
  /* Include RX QoS Filter */
  else
  {
    ex_ind_msg->rx_qos_filters_valid = TRUE;
    ex_ind_msg->rx_qos_filters_len = num_filters;
    for(i = 0; i < num_filters; i++)
    {		  	
      ex_ind_msg->rx_qos_filters[i] = ind_msg->rx_qos_filters[start_idx + i];
    }
  }
}/* qmi_qosi_fill_qos_filter */

/*===========================================================================
  FUNCTION QMI_QOSI_CALCULATE_FILTERS_IN_SEGMENT()

  DESCRIPTION
    This function calculates number of filters to be included in one segment.

  PARAMETERS
    num_tx_filters   : Number of TX filters to be included
    num_rx_filters   : Number of RX filters to be included
    more_filters     : Boolean flag for more filters
    num_filters_in_segment  : number of filters to be inlcuded in a segment
    rem_tx_qos_filters      : Remaining number of TX qos filters
    rem_rx_qos_filters      : Remaining number of RX qos filters
  ...
  SIDE EFFECTS
    None
===========================================================================*/
static void qmi_qosi_calculate_filters_in_segment
(
  uint8      *num_tx_filters,
  uint8      *num_rx_filters,
  boolean    *more_filters,
  uint32      num_filters_in_segment,
  uint8      *rem_tx_qos_filters,
  uint8      *rem_rx_qos_filters
)
{
  uint8                          rem_rx_qos_fltrs = 0;
  uint8                          rem_tx_qos_fltrs = 0;
  uint8                          half_qos_fltrs = 0;
  uint8                          num_tx_fltrs = 0;
  uint8                          num_rx_fltrs = 0;
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

  if(NULL == num_tx_filters ||
     NULL == num_rx_filters ||
     NULL == more_filters ||
     NULL == rem_tx_qos_filters ||
     NULL == rem_rx_qos_filters)
  {
    return;
  }
  ...
  num_tx_fltrs = 0;
  num_rx_fltrs = 0;
  half_qos_fltrs = num_filters_in_segment/2;
  rem_tx_qos_fltrs = *rem_tx_qos_filters;
  rem_rx_qos_fltrs = *rem_rx_qos_filters;

  /* 1. Check if all qos filters or remaining filters can be included in first segment
     2. Include TX and RX filters almost equally and not exceeding num_filters_in_segment
     3. If TX or RX completed, try to include RX or TX respectively in that segment
     3. Adjust TX or RX filters if it is exceeding QMI_QOS_MAX_QOS_FILTERS_EX_V01
  */
 
  /* All QOS Filters in one segment */
  if((rem_rx_qos_fltrs + rem_tx_qos_fltrs) <= num_filters_in_segment)
  {
    num_tx_fltrs = rem_tx_qos_fltrs;
    num_rx_fltrs = rem_rx_qos_fltrs;
    *more_filters = FALSE;
  }
  else
  {
    /* TX */
    if(rem_tx_qos_fltrs <= half_qos_fltrs)
    {
      num_tx_fltrs = rem_tx_qos_fltrs;
    }
    else
    {
      num_tx_fltrs = half_qos_fltrs;
    }

    /* RX */
    if(rem_rx_qos_fltrs <= (num_filters_in_segment - half_qos_fltrs))
    {
      num_rx_fltrs = rem_rx_qos_fltrs;
    }
    else
    {
      num_rx_fltrs = num_filters_in_segment - half_qos_fltrs;
    }

    /* Remaining filters */
    rem_tx_qos_fltrs -= num_tx_fltrs;
    rem_rx_qos_fltrs -= num_rx_fltrs;

    /* Check if more filters can be added */
    if((rem_tx_qos_fltrs + rem_rx_qos_fltrs) > 0 &&
       (num_tx_fltrs + num_rx_fltrs) != num_filters_in_segment)
    {
      /* If we are reaching here, it means either all TX or all RX filters are included */

      /* TX filters completed; Include more RX filters */
      if(num_tx_fltrs < half_qos_fltrs)
      {
        /* Adding more RX filters in this segment */
        /* Dont change below order */
        rem_rx_qos_fltrs -= (num_filters_in_segment - (num_tx_fltrs + num_rx_fltrs));
        num_rx_fltrs = num_filters_in_segment - num_tx_fltrs; 		 
      }
      /* RX filters completed; Include more TX filters */
      else if(num_rx_fltrs < (num_filters_in_segment - half_qos_fltrs))
      {
        /* Adding more TX filters in this segment */
        /* Don't change below order */
        rem_tx_qos_fltrs -= (num_filters_in_segment - (num_tx_fltrs + num_rx_fltrs));
        num_tx_fltrs = num_filters_in_segment - num_rx_fltrs;
      }
    }

    /* As filter array size in indication is QMI_QOS_MAX_QOS_FILTERS_EX_V01,
       adjust if exceeding */
    /* Adjustment TX */
    if(num_tx_fltrs > QMI_QOS_MAX_QOS_FILTERS_EX_V01)
    {
      /* Don't change below order */
      rem_tx_qos_fltrs += num_tx_fltrs - QMI_QOS_MAX_QOS_FILTERS_EX_V01;
      num_tx_fltrs = QMI_QOS_MAX_QOS_FILTERS_EX_V01;
    }
    /* Adjustment RX */
    if(num_rx_fltrs > QMI_QOS_MAX_QOS_FILTERS_EX_V01)
    {
      /* Don't change below order */
      rem_rx_qos_fltrs += num_rx_fltrs - QMI_QOS_MAX_QOS_FILTERS_EX_V01;
      num_rx_fltrs = QMI_QOS_MAX_QOS_FILTERS_EX_V01;
    }

    /* More segments required or not */
    if((rem_tx_qos_fltrs + rem_rx_qos_fltrs) > 0)
    {
      *more_filters = TRUE;
    }
  ...
  *num_tx_filters = num_tx_fltrs;
  *num_rx_filters = num_rx_fltrs;
  *rem_tx_qos_filters = rem_tx_qos_fltrs;
  *rem_rx_qos_filters = rem_rx_qos_fltrs;
  return;
}/* qmi_qosi_calculate_filters_in_segment */

/*===========================================================================
  FUNCTION QMI_QOSI_SEGMENT_GLOBAL_QOS_FLOW_IND()

  DESCRIPTION
    This function segments large global qos flow and send segmented indicaiton.

  PARAMETERS
    ind_msg      : input qos flow pointer
    qos_sp       : output qos flow pointer
    cl_sp        : Flag for first segment
    tx_id        : transaction id
...
{
  //Size of other fields: ~300 bytes
  //Size of a filter: 184
  //12-13 Filters can be included in one segment
  //>(12-13) filters can be included from second segment
#define QMI_QOS_MAX_MESSAGE_SIZE 2500
#define QMI_QOS_SIZE_PER_QOS_FILTER (sizeof(qos_filter_rule_resp_ind_type_v01))

  dsm_item_type                 *ind = NULL;
  uint8                          rem_rx_qos_filters = 0;
  uint8                          rem_tx_qos_filters = 0;
  uint32                         common_msg_size = 0;
  uint32                         num_filters_in_segment = 0;
  uint8                          num_tx_filters = 0;
  uint8                          num_rx_filters = 0;
  boolean                        more_filters = FALSE;
  uint8	                         start_fltr_idx_tx = 0;
  uint8	                         start_fltr_idx_rx = 0;
  qos_global_qos_flow_ex_ind_msg_v01 *ex_ind_msg = NULL;
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

  if(NULL == cl_sp || NULL == qos_sp || NULL == ind_msg)
  {
    return;
  }

  /* Segmentation Rule
     1. Fill TLVs other than qos filter TLV in first segment
     2. Calculate number of filters to be set in first segment
     3. Calculate remaining qos filters
     3. Fill all filters in first segment if it can be accomodated
     4. If there are more filters in segment, filters are sent in multiple segments.
     5. TX Filters and RX filters  are filled in a segment in optimized way so that 
        maximum filters are included.
  */

  /*-------------------------------------------------------------------------
    Prepare indication
  -------------------------------------------------------------------------*/
  PS_SYSTEM_HEAP_MEM_ALLOC_ASSERT(ex_ind_msg, sizeof(qos_global_qos_flow_ex_ind_msg_v01),
                           qos_global_qos_flow_ex_ind_msg_v01*);
  ...
  memset(ex_ind_msg, 0, sizeof(qos_global_qos_flow_ex_ind_msg_v01));

  /* Fill TLVs for first segment */
  qmi_qosi_fill_common_info_per_segment(ind_msg, ex_ind_msg, TRUE,
                                 &common_msg_size, tx_id);
  
  rem_tx_qos_filters = ind_msg->tx_qos_filters_len;
  rem_rx_qos_filters = ind_msg->rx_qos_filters_len;

  /* Number of filters in first segment */
  num_filters_in_segment = (QMI_QOS_MAX_MESSAGE_SIZE - common_msg_size)/QMI_QOS_SIZE_PER_QOS_FILTER;

  if(num_filters_in_segment > (2 * QMI_QOS_MAX_QOS_FILTERS_EX_V01))
  {
    num_filters_in_segment = 2 * QMI_QOS_MAX_QOS_FILTERS_V01;
  }
  ...
  do
  {
    ...
    LOG_MSG_INFO1_5("qmi_qosi_segment_global_qos_flow_ind() common_msg_size %d rem tx %d rx %d, "
                    "num_filters_in_segment %d qos filter size %d",
                    common_msg_size, rem_tx_qos_filters, rem_rx_qos_filters, 
                    num_filters_in_segment, QMI_QOS_SIZE_PER_QOS_FILTER );

    qmi_qosi_calculate_filters_in_segment(&num_tx_filters, &num_rx_filters,
                                          &more_filters, num_filters_in_segment,
                                          &rem_tx_qos_filters, &rem_rx_qos_filters);

    if(num_tx_filters)
    {
      /* Include TX QOS Filter */
      qmi_qosi_fill_qos_filter(ex_ind_msg, ind_msg,
	                       start_fltr_idx_tx, num_tx_filters, TRUE );
      start_fltr_idx_tx += num_tx_filters;
    }
    ...
    if(num_rx_filters)
    {
      /* Include TX QOS Filter */
      qmi_qosi_fill_qos_filter(ex_ind_msg, ind_msg,
	                           start_fltr_idx_rx, num_rx_filters, FALSE );
      start_fltr_idx_rx += num_rx_filters;
    }

    if(more_filters)
    {
      ex_ind_msg->is_last_segment = FALSE;
    }
    else
    {
      ex_ind_msg->is_last_segment = is_last_flow;
    }

    if(!qmi_svc_idl_message_encode(qos_svc_obj,
                                     QMI_IDL_INDICATION,
                                     QMI_QOS_GLOBAL_QOS_FLOW_EX_IND_V01,
                                     (void*)ex_ind_msg,
                                     sizeof(qos_global_qos_flow_ex_ind_msg_v01),
                                     &ind))
    {
      LOG_MSG_ERROR_INT_0("encoding failed for global flow ind ");
    }
    ...
    {
      (void)qmi_qosi_send_ind( qos_sp, cl_sp, QOSI_CMD_VAL_GLOBAL_QOS_FLOW_EX_IND, ind );
    }
    ...
    if(more_filters)
    {
      memset(ex_ind_msg, 0, sizeof(qos_global_qos_flow_ex_ind_msg_v01));
      common_msg_size = 0;
      /* Fill TLVs for second segment onwards */
      qmi_qosi_fill_common_info_per_segment(ind_msg, ex_ind_msg, FALSE, &common_msg_size, tx_id);
      /* Number of filters for next segment */
      num_filters_in_segment = (QMI_QOS_MAX_MESSAGE_SIZE - common_msg_size)/QMI_QOS_SIZE_PER_QOS_FILTER;
    }
  }while(more_filters);

  PS_SYSTEM_HEAP_MEM_FREE(ex_ind_msg);
}/* qmi_qosi_segment_global_qos_flow_ind */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch (ev_rep_info->ind_cause)
  {
    ...
    default:
      LOG_MSG_ERROR_INT_1("Unexpected ev cause [d] received in event_rpt_ind",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  *num_bytes_pushed = 0;
  ...
  DS_QMI_QOS_ASSERT(f_tlv_cnt <= 20);
  if(FALSE == qmi_svc_put_param_tlv(response,
...
{
  DS_QMI_QOS_ASSERT(filter_rules && fltr_spec);
...
{
  DS_QMI_QOS_ASSERT(xport_info && fltr_spec);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_QMI_QOS_ASSERT (qos_sp->ps_iface_ptr != NULL);
  *total_bytes_pushed = 0;
  num_bytes = 0;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_QMI_QOS_ASSERT (iface_ptr && flow_ptr);
...
)
{
  qmi_qosi_state_type *  qos_sp;
  ps_iface_type       *  ps_iface_ptr;
  ps_iface_event_enum_type  flow_event_list[] = { FLOW_ACTIVATED_EV,
                                                  FLOW_SUSPENDED_EV,
                                                  FLOW_NULL_EV,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (qos_inst >= QOS_INSTANCE_MAX)
  {
    LOG_MSG_ERROR_INT_1("QMI instance out of range (d)!", qos_inst);
    return;
  }
  ...
  if(ps_iface_ptr == NULL)
  {
    /* Rmnet call not up, while trying to reg for def flow ev */
    DS_QMI_QOS_ASSERT(0);
    return;
  }
  ...
  qos_sp->flow_evt_buf_ptr = ps_flow_alloc_event_handle(NULL,
                                                    qosi_flow_ev_cb,
                                                    qos_sp);
  if (qos_sp->flow_evt_buf_ptr == NULL)
  {
    /* Alloc Handle failed for flow */
    DS_QMI_QOS_ASSERT(0);
    return;
  }
  
  if (0 != ps_flow_subscribe_event_list(qos_sp->flow_evt_buf_ptr,
                                        flow_event_list,
                                        sizeof(flow_event_list)/sizeof(ps_iface_event_enum_type)))
    {
    /* Couldn't Subscribe event list for flow */
    DS_QMI_QOS_ASSERT(0);
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (qos_sp == NULL)
  {
    LOG_MSG_ERROR_INT_1("QOS state [d] is not initialized", qos_inst);
    return;
  }
  ...
  {
    if( (nw_init_qosi_flow_info[j].qos_handle != 0) && (nw_init_qosi_flow_info[j].qos_sp == qos_sp) )
    {
      qmi_flow_info_ptr_arr[0] = &nw_init_qosi_flow_info[j];
      qosi_generate_event_report_ind(qos_sp,
                                     qmi_flow_info_ptr_arr,
                                     1,
                                     QOSI_REPORT_STATUS_FLOW_DELETED,
  ...
  /* dereg flow events */
  ps_flow_free_event_handle(qos_sp->flow_evt_buf_ptr);
  qos_sp->flow_evt_buf_ptr = NULL;
...
{
  ps_iface_event_enum_type phys_link_ev_list[] = {PHYS_LINK_FLOW_ENABLED_EV,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (0 != ps_phys_link_subscribe_event_list(qmi_qosi_global.phys_link_ev_buf_ptr,
                                             phys_link_ev_list,
                                             sizeof(phys_link_ev_list)/sizeof(ps_iface_event_enum_type)))
  {
    LOG_MSG_ERROR_INT_0("Reg Phys link events failed");
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR_INT_0("Phys link events not registered");
    return;
  }
  ...
  ps_phys_link_free_event_handle(qmi_qosi_global.phys_link_ev_buf_ptr);
  qmi_qosi_global.phys_link_ev_buf_ptr = NULL;
...
)
{
  ...
  ps_iface_event_enum_type qos_iface_event_list[] = { IFACE_FLOW_ENABLED_EV,
                                                      IFACE_FLOW_DISABLED_EV }; 
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

  DS_QMI_QOS_ASSERT(sp != NULL);
  qos_sp = (qmi_qosi_state_type*)sp;
  DS_QMI_QOS_ASSERT(qos_sp->ps_iface_ptr != NULL);

  if (!qos_sp->iface_evt_buf_ptr)
  {
    qos_sp->iface_evt_buf_ptr = ps_iface_alloc_event_handle(qos_sp->ps_iface_ptr,
                                                            qosi_iface_ev_cb,
                                                            (void *)qos_sp);
    if (qos_sp->iface_evt_buf_ptr == NULL)
    {
      /* Alloc Handle failed for IFACE */
      DS_QMI_QOS_ASSERT(0);
      return;
    }
  }

  if (0 != ps_iface_subscribe_event_list(qos_sp->iface_evt_buf_ptr,
                                         qos_iface_event_list,
                                         sizeof(qos_iface_event_list)/sizeof(ps_iface_event_enum_type)))
  {
    LOG_MSG_ERROR_INT_1("IFACE event (d) reg failed",event );
    DS_QMI_QOS_ASSERT(0);
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /* dereg iface events */
  ps_iface_free_event_handle(qos_sp->iface_evt_buf_ptr);
  qos_sp->iface_evt_buf_ptr = NULL;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_QMI_QOS_ASSERT(cmd_ptr != NULL);

  /* check that cmd_ptr is not NULL in case ASSERT is removed */
  if (cmd_ptr == NULL)
  {
    LOG_MSG_ERROR_INT_0("cmd_ptr is NULL!");
    return;
  }
  ...
  switch(cmd_buf_ptr->cmd_id)
  {
    ...
    case QMI_CMD_QOS_NW_STATUS_CHANGE_EVT:
      ...
      break;
    case QMI_CMD_QOS_GENERATE_QOS_FLOW_IND:
      qosi_generate_indication_for_qos_ids_hdlr(cmd_buf_ptr->data.flow_info_ind.cl_sp,
                                                   cmd_buf_ptr->data.flow_info_ind.tx_id,
                                                   cmd_buf_ptr->data.flow_info_ind.qos_id);
      break;
    default:
      LOG_MSG_ERROR_INT_1("Invalid QMI cmd with type [d] recd",
                      cmd_buf_ptr->cmd_id);
      DS_QMI_QOS_ASSERT(0);
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(call_up == TRUE)
  {
    ...
    // When call is up, by default flow is enabled
    if (is_v6)
    {
      qos_sp_v6->last_fc_ev = QOSI_REPORT_STATUS_FLOW_ENABLED;
    }
    else
    {
      qos_sp_v4->last_fc_ev = QOSI_REPORT_STATUS_FLOW_ENABLED;
    }
...
/*===========================================================================
  FUNCTION QOSI_UPDATE_FLOW_INFO_PTR_ARR

  DESCRIPTION
    Update flow info ptr array if needed. Suppressed flow info ptr is
    removed from the array.

  PARAMETERS
    qos_sp : QMI QoS instance

  RETURN VALUE
    The new flow info ptr array
  ...
  SIDE EFFECTS
    None
===========================================================================*/
static void qosi_update_flow_info_ptr_arr
(
  qmi_qosi_state_type *   qos_sp,
  qmi_flow_info_type  **  in_flow_info_ptr_arr,
  uint16                  in_num_flow,
  uint8                   flow_change_ev,
  qmi_flow_info_type   ** out_flow_info_ptr_arr,
  uint16                * out_num_flow 
)
{
  uint16                 temp_num_flow = 0;
  int                    i;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

  /*-------------------------------------------------------------------------
    Check if we need to suppress flow control -- If tethered traffic
    is offloaded to HW we don't want to wake up AP. Make sure the last
    status is FLOW_ENABLE before suppressing the indication.
  -------------------------------------------------------------------------*/
  if (flow_change_ev == QOSI_REPORT_STATUS_FLOW_ENABLED ||
      flow_change_ev == QOSI_REPORT_STATUS_FLOW_DISABLED)
  {
    if (rmnet_has_ul_direct_traffic(qos_sp->binding.rmnet_inst))
    {
      // If AP is active, don't suppress any indication
      for (i = 0; i < in_num_flow && i < QOSI_MAX_QOS_REQUESTS; i++)
      {
        out_flow_info_ptr_arr[i] = in_flow_info_ptr_arr[i];
      }
      *out_num_flow = in_num_flow;
    }
    else
    {
      // Only send flow ind if last ind is flow disable
      for (i = 0; i < in_num_flow && i < QOSI_MAX_QOS_REQUESTS; i++)
      {
        if ( (in_flow_info_ptr_arr[i] == NULL &&
              qos_sp->last_fc_ev == QOSI_REPORT_STATUS_FLOW_ENABLED) ||
             (in_flow_info_ptr_arr[i] != NULL &&
              in_flow_info_ptr_arr[i]->last_fc_ev == QOSI_REPORT_STATUS_FLOW_ENABLED) )
        {
          LOG_MSG_INFO2_1("Flow ev %d suppressed", flow_change_ev);
        }
        else
        {
          // Copy the flow info to the new array
          out_flow_info_ptr_arr[temp_num_flow++] = in_flow_info_ptr_arr[i];
        }
      }

      // Use the new array
      *out_num_flow = temp_num_flow;
    }

    // Update last flow status for non-suppressed flow info
    for (i = 0; i < (*out_num_flow) && i < QOSI_MAX_QOS_REQUESTS; i++)
    {
      if (out_flow_info_ptr_arr[i] == NULL)
      {
        qos_sp->last_fc_ev = flow_change_ev;
      }
      else
      {
        out_flow_info_ptr_arr[i]->last_fc_ev = flow_change_ev;
      }
    }
...
(
  qmi_qosi_state_type *   qos_sp,
  qmi_flow_info_type  **  in_qmi_flow_info_ptr_ptr,
  uint16                  in_num_qos_ids,
  uint8                   flow_change_ev,
  boolean                 new_flow
)
{
  ...
  uint16                      * uplink_fc_seq_num;
  qmi_flow_info_type          * qmi_flow_info_ptr_ptr[QOSI_MAX_QOS_REQUESTS] = {NULL};
  uint16                        num_qos_ids = 0;
  boolean                       suppress_flow_ctl_report = FALSE;
  boolean                       event_sent = FALSE;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

  /*-------------------------------------------------------------------------
    Do flow control on the offloaded tethered traffic
  -------------------------------------------------------------------------*/
  if ( (flow_change_ev == QOSI_REPORT_STATUS_FLOW_ENABLED ||
        flow_change_ev == QOSI_REPORT_STATUS_FLOW_DISABLED ) &&
        rmnet_meta_sm_qos_or_te_fc_enabled(qos_sp->binding.rmnet_inst) )
  {
    qosi_set_offload_fc(qos_sp,
                        in_qmi_flow_info_ptr_ptr,
                        in_num_qos_ids,
                        flow_change_ev);
  }

  /*-------------------------------------------------------------------------
    Update the flow info ptr array if needed, to suppress QMI indications
    for the tethered offloaded traffic
  -------------------------------------------------------------------------*/
  qosi_update_flow_info_ptr_arr(
                  qos_sp,
                  in_qmi_flow_info_ptr_ptr,
  ...
  if (num_qos_ids > QOSI_MAX_QOS_REQUESTS)
  {
    DS_QMI_QOS_ASSERT(0);
    return;
  }
  ...
  while(cl_sp != NULL)
  {
    ...
    if( flow_change_ev == QOSI_REPORT_STATUS_FLOW_ENABLED || 
        flow_change_ev == QOSI_REPORT_STATUS_FLOW_DISABLED)
    {
      if( cl_sp->report_status.suppress_flow_ctl_report == TRUE )
      {
        suppress_flow_ctl_report = TRUE;
      }
    }
    if ((qos_sp == qmi_qosi_get_qos_sp_by_cl_sp(cl_sp)) &&
        (cl_sp->report_status.report_global_flow_ex == TRUE ||
         cl_sp->report_status.report_global_flow_ex_segmented == TRUE) &&
        (suppress_flow_ctl_report == FALSE) &&
         qmi_qosi_verify_client_subs(cl_sp, qos_sp->binding.subs_id, TRUE))
      {
        if (cl_sp->report_status.report_global_flow_ex_segmented == TRUE)
        {
          cl_sp->tx_id++;
        }
        qmi_qosi_global_qos_flow_ind(qos_sp,
                                  cl_sp,
                                  qmi_flow_info_ptr_ptr,
                                  num_qos_ids,
                                  flow_change_ev,
      ...
      flow_change_ev == QOSI_REPORT_STATUS_FLOW_DISABLED )
  {
    if (event_sent)
    {
      //LOG THE EVENT FOR each qmi_flo_info_ptr
      qmi_qosi_log_fc_event(qos_sp, 
                            qmi_flow_info_ptr_ptr, 
                            num_qos_ids, 
                            flow_change_ev);
    }
...
}/*qosi_generate_event_report_ind*/
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_QMI_QOS_ASSERT (user_data_ptr != NULL);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(def_flow_ptr == flow_ptr)
  {
    switch(event)
    {
      ...
      default:
      {
        LOG_MSG_ERROR_INT_1("Unregistered flow event [d] recvd, ignoring", event);
        break;
      }
    }
    return;
  ...
  if(i == QOSI_MAX_QOS_REQUESTS)
  {
    ...
    else 
    {
      ...
      if (j >= QOSI_MAX_NETWORK_INIT_FLOW_NUM)
      {
        LOG_MSG_ERROR_INT_0("QOS nw inited handle slots exhausted");
        return;
      }
      else
      {
        ...
        {
          LOG_MSG_INFO1_4("Mapping flow_ptr 0x%x to handle 0x%x qos_sp 0x%p j = %d", 
                           flow_ptr, nw_init_qosi_flow_info[j].qos_handle,qos_sp,j);

          nw_init_qosi_flow_info[j].flow_ptr   = flow_ptr;
          nw_init_qosi_flow_info[j].uplink_fc_seq_num = 0;
          nw_init_qosi_flow_info[j].last_fc_ev = QOSI_REPORT_STATUS_FLOW_ENABLED;
          nw_init_qosi_flow_info[j].qos_sp = qos_sp;
        }
      }
    }
    ...
    if (event_info.flow_event_info.state == FLOW_NULL
        ...
        || event_info.flow_event_info.state == FLOW_INIT )
    {
      new_flow = TRUE;
    }
    else
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(def_phys_link_ptr == phys_link_ptr)
  {
    switch(event)
    {
      ...
      default:
      {
        LOG_MSG_ERROR_INT_1("Unregistered default flow event: d recvd, ignoring",
                        event);
        break;
      }
    }
  ...
  switch( event )
  {
    ...
    case PHYS_LINK_FLOW_DISABLED_EV:
    {
      ...
      {
        /* Check if the flow is associated with the phys link on which event occurred
           and also if it is associated with the QOS iface. Send event on dedicated bearer 
           also irrespective of default flow state*/
        if ((nw_init_qosi_flow_info[i].qos_handle != 0) &&
            (PS_FLOW_GET_PHYS_LINK(nw_init_qosi_flow_info[i].flow_ptr) == phys_link_ptr) &&
            (PS_FLOW_GET_IFACE(nw_init_qosi_flow_info[i].flow_ptr) == qos_sp->ps_iface_ptr) )
        {
  ...
  switch( event )
  {
    case IFACE_707_NETWORK_SUPPORTED_QOS_PROFILES_CHANGED_EV:
      ...
      if ( (ev_rep_info->event_info.nw_supp_qos_prof.tech_name != CDMA_SN_IFACE) &&
           ...
           (ev_rep_info->event_info.nw_supp_qos_prof.tech_name != CLAT_IFACE) )
      {
        LOG_MSG_ERROR_INT_1("Don't report nw supported qos profile for iface_name= d",
                        iface_ptr->name);
        DS_QMI_QOS_ASSERT(0);
        break;
      }
    ...
    default:
      LOG_MSG_ERROR_INT_1("Unregistered event: d recvd, ignoring", event);
      break;
  } /* switch( event ) */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
         ...
         && ps_flow_handle != NULL)
  {
    ...
    if (!result)
    {
      LOG_MSG_ERROR_INT_0("Returning incomplete list of flows");
    ...
    {
      /* For IFACE_FLOW_ENABLED_EV, check if flow is enabled on the NW
         initiated flow and corresponding physlink also to send ind.
         For IFACE_FLOW_DISABLED_EV, No checks needed, just sent ind*/
      for (itr = 0; itr < QOSI_MAX_NETWORK_INIT_FLOW_NUM; itr++)
      {
        if (nw_init_qosi_flow_info[itr].flow_ptr == ps_flow_ptr)
        {
          if(IFACE_FLOW_DISABLED_EV == event) 
          {
            send_info = TRUE;
            nw_init_qosi_flow_info[itr].last_rep = QMI_FLOW_DISABLED;
            qos_handle = nw_init_qosi_flow_info[itr].qos_handle;
            break;
          }
          else if(PS_IFACE_COMMON_IS_FLOW_ENABLED(iface_ptr, ps_flow_ptr))
          {
            send_info = TRUE;
            nw_init_qosi_flow_info[itr].last_rep = QMI_FLOW_ENABLED;
            qos_handle = nw_init_qosi_flow_info[itr].qos_handle;
            break;
          }
        }/*if(flow_ptr)*/
    ...
    else /* UE initiated flow */
    {
      ...
      {
        if(qosi_flow_info[itr].flow_ptr == ps_flow_ptr)
        {
          switch (event)
          {
            case IFACE_FLOW_ENABLED_EV:
              if(qosi_flow_info[itr].last_rep == QMI_FLOW_DISABLED)
              {
                ...
                if (PS_IFACE_COMMON_IS_FLOW_ENABLED(iface_ptr, check_ps_flow_ptr))
                {
                  ...
                  qos_handle = qosi_flow_info[itr].qos_handle;
                }
              }
              break;
    ...
    if (send_info)
    {
      ...
      qmi_flow_info_ptr_arr[num_qos_ids]->qos_handle = qos_handle;
      num_qos_ids++;
    }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_QMI_QOS_ASSERT (cl_sp_in != NULL);
  ...
  DS_QMI_QOS_ASSERT((inst < QOS_INSTANCE_MAX) && qos_sp);
...
/*-------------------------------------------------------------------------
  ...
  if(0 < qos_id_cnt)
  {
    ...
    while(loop_cl_sp != NULL)
    {
      ...
      {
        if(TRUE == loop_cl_sp->report_status.report_global_flow)
        {
        ...
        else if (TRUE == loop_cl_sp->report_status.report_global_flow_ex ||
                 TRUE == loop_cl_sp->report_status.report_global_flow_ex_segmented)
        {
          if (loop_cl_sp->report_status.report_global_flow_ex_segmented == TRUE)
          {
            loop_cl_sp->tx_id++;
          }
          /* send delete flow event report ind */
          (void) qmi_qosi_global_qos_flow_ind(qos_sp,
                                           loop_cl_sp,
                                           qmi_flow_info_ptr_arr,
                                           qos_id_cnt,
                                           QOSI_REPORT_STATUS_FLOW_DELETED,
  ...
  cl_sp->report_status.report_global_flow_ex_segmented = 0;
  cl_sp->tx_id = 0;
      ...
      if (nw_init_idx >= QOSI_MAX_NETWORK_INIT_FLOW_NUM)
      {
        ...
        else
        {
          ...
          else
          {
            ...
            LOG_MSG_INFO2_4("Mapping flow_ptr 0x%x to handle 0x%x qos_sp 0x%p nw_init_idx = %d", 
                            ps_flow_ptr, nw_init_qosi_flow_info[nw_init_idx].qos_handle,qos_sp,nw_init_idx);

            nw_init_qosi_flow_info[nw_init_idx].flow_ptr   = ps_flow_ptr;
            nw_init_qosi_flow_info[nw_init_idx].uplink_fc_seq_num = 0;
            nw_init_qosi_flow_info[nw_init_idx].last_fc_ev = QOSI_REPORT_STATUS_FLOW_ENABLED;
            nw_init_qosi_flow_info[nw_init_idx].qos_sp = qos_sp;
          }
        }
      }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_QMI_QOS_ASSERT(inst < QOS_INSTANCE_MAX);

  /* check that qmi_inst is within range in case ASSERT is removed */
  if (inst >= QOS_INSTANCE_MAX)
  {
    LOG_MSG_ERROR_INT_1("QOS instance out of range (d)!", inst);
    return;
  }
  ...
  if (qos_sp == NULL)
  {
    LOG_MSG_ERROR_INT_1("qos_sp for inst: d is NULL", inst);
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_QMI_QOS_ASSERT(sp && cmd_buf_p && cl_sp_in && sdu_in);
...
void qmi_qos_swap_instances(uint32  ep_id, uint8   mux_id)
{
  ...
  {
      LOG_MSG_ERROR_INT_1("QOS sp not initialized (d)!", bind_inst);
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
      LOG_MSG_ERROR_INT_1("Invalid subs id: d",subscription_id);
      errval = QMI_ERR_INVALID_ARG;
      goto send_result;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /* Allow registration of only one global qos flows indication */
  if(TRUE == req_msg.report_global_qos_flows_valid &&
     TRUE == req_msg.report_global_qos_flows_ex_valid)

  {
    if(req_msg.report_global_qos_flows &&
       req_msg.report_global_qos_flows_ex)
    {
  ...
  if (req_msg.report_global_qos_flows_ex_valid == TRUE)
  {
    qos_client_sp->report_status.report_global_flow_ex_segmented = req_msg.report_global_qos_flows_ex ? TRUE : FALSE;
    missing_tlv = FALSE;
  }

  /* Suppress flow control message  */
  if (req_msg.suppress_report_flow_control_valid == TRUE)
  {
    qos_client_sp->report_status.suppress_flow_ctl_report = 
        req_msg.suppress_report_flow_control ? TRUE : FALSE;
    missing_tlv = FALSE;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(( QMI_AF_ANY == qos_client_sp->bound_ip_type ) || 
     ( ip_type != qos_client_sp->bound_ip_type ))
  {
    LOG_MSG_ERROR_INT_2("IP type: d in request, client bound to IP type: d",
                    ip_type, qos_client_sp->bound_ip_type);
...
/*===========================================================================
  FUNCTION QMI_QOS_GET_QOS_INFO_EX()

  DESCRIPTION
    Get the granted QoS flow spec and filters for a particular QoS identifier.
    If indication size is more than threshold, it will be segmented.
  ...
  SIDE EFFECTS
    None
===========================================================================*/
static dsm_item_type *  qmi_qosi_get_qos_info_ex
(
  void *            sp,
  void *            cmd_buf_p,
  void *            cl_sp,
...
{
  qmi_qosi_state_type               *qos_sp = NULL;
  qmi_qosi_client_state_type        *qos_client_sp = NULL;
  dsm_item_type                     *response = NULL;
  uint8                              qos_index = 0;
  qmi_error_type_v01                 errval = QMI_ERR_NONE_V01;
  qos_get_qos_info_ex_req_msg_v01    req_msg;
  qos_get_qos_info_ex_resp_msg_v01  *resp_msg = NULL;
  uint8                              found_dedicated_flow = 0;
  uint32                             qos_id = QOSI_MAX_QOS_ID;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  PS_SYSTEM_HEAP_MEM_ALLOC_ASSERT(resp_msg, sizeof(qos_get_qos_info_ex_resp_msg_v01),
                                  qos_get_qos_info_ex_resp_msg_v01*);
  if (resp_msg == NULL)
  {
    return NULL;
  }

  memset(&req_msg, 0, sizeof(qos_get_qos_info_ex_req_msg_v01));
  memset(resp_msg, 0, sizeof(qos_get_qos_info_ex_resp_msg_v01));

  errval = QMI_QOSI_DECODE_REQUEST_MSG(QMI_QOS_GET_QOS_INFO_EX_REQ_V01,
  	                                   sdu_in,
  	                                   &req_msg,
  	                                   sizeof(req_msg));
  ...
  if (FALSE == PS_IFACE_IS_VALID(qos_sp->ps_iface_ptr))
  {
    errval = QMI_ERR_OUT_OF_CALL_V01;
    goto send_result;
  }
  ...
  if (TRUE == req_msg.qos_id_valid)
  {
    qos_id = req_msg.qos_id;
    if (0  == qos_id) //Default flow
    {
  ...
  /* Network Initiated QoS Flow IDs */
  for (qos_index = 0; qos_index < QOSI_MAX_NETWORK_INIT_FLOW_NUM; qos_index++)
  {
    if(PS_FLOW_GET_IFACE(nw_init_qosi_flow_info[qos_index].flow_ptr) == qos_sp->ps_iface_ptr)
    {
      if (qos_id == QOSI_MAX_QOS_ID && 0 != nw_init_qosi_flow_info[qos_index].qos_handle ) 
      {
        /* At least one flow found, call generate indication */
        found_dedicated_flow = TRUE;
        break;
      }
      else if (nw_init_qosi_flow_info[qos_index].qos_handle == qos_id)
      {
        /* Found requested flow */
        found_dedicated_flow = TRUE;
        break;
      }
    }
  ...
  if (found_dedicated_flow)
  {
    /* TX ID */
    qos_client_sp->tx_id++;
    resp_msg->tx_id_valid = TRUE;
    resp_msg->tx_id = qos_client_sp->tx_id;
    if(qos_client_sp->report_status.report_global_flow_ex_segmented)
    {
      /* Generate Indication
         Post cmd in dcc task to generate global_qos_flow_ex_ind
         If qos_id TLV is not set in request, client is requesting for all qos; Pass QOSI_MAX_QOS_ID
      */
      qosi_generate_indication_for_qos_ids(qos_client_sp, ((req_msg.qos_id_valid)?req_msg.qos_id:QOSI_MAX_QOS_ID));
    }
  }
  else
  {
    if (qos_id != QOSI_MAX_QOS_ID)
    {
      errval = QMI_ERR_INVALID_QOS_ID_V01;
    }
    else
...
send_result:
  QMI_QOSI_ENCODE_RESPONSE_MSG(QMI_QOS_GET_QOS_INFO_EX_RESP_V01,
                               resp_msg,
                               sizeof(qos_get_qos_info_ex_resp_msg_v01),
                               errval,
                               &response);
  PS_SYSTEM_HEAP_MEM_FREE(resp_msg);
  return response;
...
/*===========================================================================
  FUNCTION QMI_QOS_GET_QOS_IDS()

  DESCRIPTION
    Get the list of qos ids for a PDN.
  ...
  SIDE EFFECTS
    None
===========================================================================*/
static dsm_item_type *  qmi_qosi_get_qos_ids
(
  void *            sp,
  void *            cmd_buf_p,
  void *            cl_sp,
...
)
{
  ...
  uint8                          qos_index = 0;
  qmi_error_type_v01             errval;
  qos_get_qos_ids_req_msg_v01    req_msg;
  qos_get_qos_ids_resp_msg_v01  *resp_msg = NULL;
  uint8                          num_qos_ids = 0;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  PS_SYSTEM_HEAP_MEM_ALLOC_ASSERT(resp_msg, sizeof(qos_get_qos_ids_resp_msg_v01),
                                  qos_get_qos_ids_resp_msg_v01*);
  if (resp_msg == NULL)
  {
    return NULL;
  }

  memset(&req_msg, 0, sizeof(qos_get_qos_ids_req_msg_v01));
  memset(resp_msg, 0, sizeof(qos_get_qos_ids_resp_msg_v01));

  errval = QMI_QOSI_DECODE_REQUEST_MSG(QMI_QOS_GET_QOS_IDS_REQ_V01,
  	                                   sdu_in,
  	                                   &req_msg,
  	                                   sizeof(req_msg));
  ...
  for (qos_index = 0; qos_index < QOSI_MAX_NETWORK_INIT_FLOW_NUM; qos_index++)
  {
    if (0 != nw_init_qosi_flow_info[qos_index].qos_handle && 
        (PS_FLOW_GET_IFACE(nw_init_qosi_flow_info[qos_index].flow_ptr) == qos_sp->ps_iface_ptr))
    {
      resp_msg->qos_ids_list[num_qos_ids++] = nw_init_qosi_flow_info[qos_index].qos_handle;
    }
  }

  resp_msg->qos_ids_list_valid = TRUE;
  resp_msg->qos_ids_list_len = num_qos_ids;

send_result:
  QMI_QOSI_ENCODE_RESPONSE_MSG(QMI_QOS_GET_QOS_IDS_RESP_V01,
                               resp_msg,
                               sizeof(qos_get_qos_ids_resp_msg_v01),
                               errval,
                               &response);
  PS_SYSTEM_HEAP_MEM_FREE(resp_msg);
  return response;
...
/*===========================================================================
  FUNCTION QOSI_GENERATE_INDICATION_FOR_QOS_IDS()

  DESCRIPTION
    This function post command to dcc task to generate indication for specific qos id
    or for all qos flows.
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  cmd_ptr = (qmi_qosi_cmd_buf_type *)
              qmi_qosi_get_cmd_buf(QMI_CMD_QOS_GENERATE_QOS_FLOW_IND);
  ...
  cmd_ptr->data.flow_info_ind.cl_sp = cl_sp;
  cmd_ptr->data.flow_info_ind.tx_id = cl_sp->tx_id;
  cmd_ptr->data.flow_info_ind.qos_id = qos_id;
  cmd_ptr->cmd_id = QMI_CMD_QOS_GENERATE_QOS_FLOW_IND;

  dcc_send_cmd_ex (DCC_QMI_CMD, cmd_ptr);
} /* qosi_generate_indication_for_qos_ids() */

/*===========================================================================
  FUNCTION QOSI_GENERATE_INDICATION_FOR_QOS_IDS_HDLR()

  DESCRIPTION
    This function generates global qos flow indication for query request
    (qmi_qos_get_qos_info_ex).

  PARAMETERS
    cl_sp          : client state pointer
    qos_ids_len    : number of qos ids
    qos_ids        : array of qos ids
...
)
{
  ...
  if (NULL == qos_sp || 
     FALSE == cl_sp->report_status.report_global_flow_ex_segmented)
  {
    /* Client is no more or client is not interested for the indicaiton */
    return;
  }
  ...
  for (qos_index = 0; qos_index < QOSI_MAX_NETWORK_INIT_FLOW_NUM; qos_index++)
  {
    if (PS_FLOW_GET_IFACE(nw_init_qosi_flow_info[qos_index].flow_ptr) == qos_sp->ps_iface_ptr)
    {
      if (qos_id == QOSI_MAX_QOS_ID && 0 != nw_init_qosi_flow_info[qos_index].qos_handle)
      {
        qmi_flow_info_ptr_arr[num_qos_ids++] = &nw_init_qosi_flow_info[qos_index];
      }
      else if (nw_init_qosi_flow_info[qos_index].qos_handle == qos_id ) 
      {
        qmi_flow_info_ptr_arr[0] = &nw_init_qosi_flow_info[qos_index];
        num_qos_ids = 1;
        break;
      }
    }
  ...
  LOG_MSG_INFO1_5("qosi_generate_indication_for_qos_ids_hdlr() calling qmi_qosi_global_qos_flow_ind()"
                  " with qos_sp 0x%x cl_sp 0x%x qmi_flow_info_ptr_arr 0x%x qos count %d tx_id %d ",
                  qos_sp, cl_sp, qmi_flow_info_ptr_arr,
                  num_qos_ids, tx_id);
  
  qmi_qosi_global_qos_flow_ind(qos_sp, cl_sp, qmi_flow_info_ptr_arr,
                              num_qos_ids, QOSI_REPORT_STATUS_FLOW_ACTIVATED, FALSE, tx_id);
}/* qosi_generate_indication_for_qos_ids_hdlr */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_QMI_QOS_ASSERT(event_info_ptr != NULL);
...
/*===========================================================================
  FUNCTION QOSI_SET_OFFLOAD_FC()

  DESCRIPTION
    Perform flow control on the offloaded tethered traffic
 
  PARAMETERS
    qos_sp:                QMI QoS instance pointer
    qmi_flow_info_ptr_ptr: Flow ptr array
    num_qos_ids:           Number of QoS IDs in the flow ptr array
    flow_change_ev:        Flow event
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /*-------------------------------------------------------------------------
     We only support this on the default flow
  -------------------------------------------------------------------------*/
  for (i = 0; i < num_qos_ids; i++)
  {
    if (qmi_flow_info_ptr_ptr[i] == NULL)
    {
      rmnet_set_offload_fc(
            qos_sp->binding.rmnet_inst,
            flow_change_ev == QOSI_REPORT_STATUS_FLOW_DISABLED ? TRUE : FALSE,
            FALSE);
      break;
    }
  }
...
{
  qmi_qosi_powersave_info_type *powersave_info = (qmi_qosi_powersave_info_type*)item_ptr;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  return ( (powersave_info->ep_id == *(uint32*)compare_val) ? 1 : 0 );
} /* qosi_compare_powersave_ep_id */

/*===========================================================================
  FUNCTION QMI_QOSI_POWERSAVE_CB()

  DESCRIPTION
    This the callback that we have registered with WDAJ for powersave mode
    change event
 
  PARAMETERS
    ep_id:  End point ID of the client
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(NULL == powersave_info_ptr)
  {
    ...
    /*-------------------------------------------------------------------------
      Add a new entry to the list if the end point ID is not found
    ------------------------------------------------------------------------*/
    PS_SYSTEM_HEAP_MEM_ALLOC(powersave_info_ptr,
                             sizeof(qmi_qosi_powersave_info_type),
                             qmi_qosi_powersave_info_type*);

    if (NULL == powersave_info_ptr)
    {
      LOG_MSG_ERROR_0("qmi_qosi_powersave_cb: Failed to allocate memory");
      return;
    }
    memset( powersave_info_ptr, 0x0, sizeof(qmi_qosi_powersave_info_type));
    powersave_info_ptr->ep_id = ep_id;
    powersave_info_ptr->mask  = mask;
    (void) q_link ( powersave_info_ptr, &powersave_info_ptr->link );
    q_put( &(qmi_qosi_global.powersave_clnt_q), &(powersave_info_ptr->link) );
  }
  else
  {
    /*-------------------------------------------------------------------------
      Delete entry from the list when powersave mode is disabled or WDA client
      is disconnected
    ------------------------------------------------------------------------*/
    if (0 == mask)
    {
      q_delete( &(qmi_qosi_global.powersave_clnt_q), &(powersave_info_ptr->link) );
      PS_SYSTEM_HEAP_MEM_FREE(powersave_info_ptr);
    }
    else
    {
      powersave_info_ptr->mask = mask;
...
{
  qsh_mdump_collect_high(&qmi_qos_state, QOSI_MAX_STATE * QOS_INSTANCE_MAX * sizeof(qmi_qosi_state_type *));
  qsh_mdump_collect_high(&qosi_flow_info, QOSI_MAX_QOS_REQUESTS * sizeof(qmi_flow_info_type));
  qsh_mdump_collect_high(&nw_init_qosi_flow_info, QOSI_MAX_NETWORK_INIT_FLOW_NUM * sizeof(qmi_flow_info_type));
  qsh_mdump_collect_high(&qmi_qosi_global, sizeof(qmi_qosi_global));
}
#endif /* FEATURE_QSH_MDUMP */
