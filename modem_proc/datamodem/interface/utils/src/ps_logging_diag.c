...
/*===========================================================================
 ...
 $Header: //components/rel/data.mpss/3.5..2.1/interface/utils/src/ps_logging_diag.c#2 $
  ...
  ps_iface_dpl_lookup_info_type ps_iface_dpl_lookup_table[DPL_IID_IFNAME_MAX] =
{
  ...
  /* 108 */   ,{NULL,         ""                          }
  /* 109 */   ,{NULL,         ""                          }
  /* 110 */   ,{NULL,         "Rm Network 17"             }
  /* 111 */   ,{NULL,         "Rm Network 18"             }
  /* 112 */   ,{NULL,         "Rm Network 19"             }
  /* 113 */   ,{NULL,         "Rm Network 20"             }
  /* 114 */   ,{NULL,         "Reserved 1"                }
  /* 115 */   ,{NULL,         "Reserved 2"                }
  /* 116 */   ,{NULL,         "Reserved 3"                }
  /* 117 */   ,{NULL,         "Reserved 4"                }
  ...
  /* 120 */   ,{NULL,         "Reserved 7"                }
  /* 121 */   ,{NULL,         "Reserved 8"                }
  /* 122 */   ,{NULL,         "V2X Iface 1"               }
  /* 123 */   ,{NULL,         "V2X Iface 2"               }
  /* 124 */   ,{NULL,         "V2X Iface 3"               }
  /* 125 */   ,{NULL,         "V2X Iface 4"               }
  /* 126 */   ,{NULL,         "3GPP Context 16"           }
  /* 127 */   ,{NULL,         "3GPP Context 17"           }
  /* 128 */   ,{NULL,         "IWLAN S2B Iface 17"        }
  /* 129 */   ,{NULL,         "IWLAN S2B Iface 18"        }
};
...
===========================================================================*/
static PACKED void *ps_logging_diag_cmd_hdlr
(
  PACKED void  * req_pkt,
  uint16         pkt_len
);
...
{
  {
   PS_LOGGING_DIAG_DATA_PATH,
   PS_LOGGING_DIAG_MAX,
   ps_logging_diag_cmd_hdlr
  }
};

static ps_logging_diag_internal_lookup_info ps_logging_diag_lookup_table[PS_LOGGING_DIAG_MAX] =
{
  {PS_LOGGING_DIAG_DATA_PATH, NULL},
  {PS_LOGGING_DIAG_DS_MUX, NULL},
  {PS_LOGGING_DIAG_EXT_DEBUG_LOGGING, NULL},
  {PS_LOGGING_DIAG_MAX, NULL}/* This should be the last entry*/
};

/*===========================================================================
...
void ps_diag_cmd_init(void)
{
  ...
  DIAGPKT_DISPATCH_TABLE_REGISTER((diagpkt_subsys_id_type)DIAG_SUBSYS_PS,
                                   ps_logging_diag_tbl);
...
)
{
  ...
  uint16                      subsys_cmd = 0;
  uint8                       total_arg = 0;
  ps_logging_diag_input_type  input_args;
  uint8                       idx = 0;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  LOG_MSG_INFO1_2("ps_logging_diag_cmd_hdlr(): "
                  "QXDM called ps_logging_diag_cmd_hdlr pkt len %d diag hdr len %d", pkt_len, DIAG_HDR_LEN);
  ...
  subsys_cmd = diagpkt_subsys_get_cmd_code((void*)req_pkt);
  ...
  /*-------------------------------------------------------------------------
    Extract Arguments. PS diag handler expect args in  byte byte format.
    e.g send_data 75 42 03 00 01 01 01
     ...
     Arg 3 - 01
  -------------------------------------------------------------------------*/
  if ( pkt_len > DIAG_HDR_LEN )
  {
    total_arg = pkt_len - DIAG_HDR_LEN;
  }

  LOG_MSG_INFO1_1("ps_logging_diag_cmd_hdlr(): "
                  "QXDM called ps_logging_diag_cmd_hdlr total_arg %d ", total_arg);
  
  memset(&input_args, 0x0, sizeof(input_args));
  
  for ( idx = 0; idx < PS_LOGGING_CMD_MAX_SUPPORTED_ARG && total_arg != 0; idx++, total_arg--)
  {
    input_args.arg_list[idx] = get8(request_ptr++);
  }
                  
  /*-------------------------------------------------------------------------
    Find valid client registration 
  -------------------------------------------------------------------------*/
  if ( subsys_cmd < PS_LOGGING_DIAG_MAX)
  {
    if ( NULL != ps_logging_diag_lookup_table[ subsys_cmd ].call_back)
    {
        ps_logging_diag_lookup_table[ subsys_cmd ].call_back( subsys_cmd, input_args );
    }
  }
  /*-------------------------------------------------------------------------
    Fill response with diagnostic header
  -------------------------------------------------------------------------*/
  return_ptr = diagpkt_subsys_alloc(DIAG_SUBSYS_PS,
                                    subsys_cmd,
                                    DIAG_HDR_LEN );
...
} /* ps_logging_diag_loopback_ul_path() */
...
)
{
  ...
  if ( (client_id >= (uint32)PS_LOGGING_DIAG_MAX) ||
        (NULL == notify_cb))
  {
    return; 
  }
  
  for ( index = 0; index < PS_LOGGING_DIAG_MAX; index++ )
  {
    if( client_id == ps_logging_diag_lookup_table[ index ].client_id )
    {
      ps_logging_diag_lookup_table[ client_id ].call_back = notify_cb;
      break;
    }
  }
}/* ps_logging_diag_register_cmd_cb */
