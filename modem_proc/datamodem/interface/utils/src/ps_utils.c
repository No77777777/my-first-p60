...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/utils/src/ps_utils.c#3 $ $DateTime: 2020/01/09 01:55:36 $ $Author: pwbldsvc $

when        who    what, where, why
--------    ---    ---------------------------------------------------------- 
04/01/16    yl     IP data path re-architecture
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  DS_UTILS_ASSERT( rex_self() == rex_ps_tcb );
  ...
  PS_SET_SIG(sig_mask, PS_NV_CMD_SIGNAL);
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  LOG_MSG_INFO3_2("ps_utils_convert_subs_id(): "
...
/*=========================================================================== 
  FUNCTION  PS_UTILS_CONVERT_SUBS_SYS_TO_PS

  DESCRIPTION
    This function converts SYS SUB to PS subs id.
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  switch (cm_subs_id)
  {
    case SYS_MODEM_AS_ID_1:
      ps_subs_id = PS_SYS_PRIMARY_SUBS;
      break;
      
    case SYS_MODEM_AS_ID_2:
      ps_subs_id = PS_SYS_SECONDARY_SUBS;
      break;
      
    case SYS_MODEM_AS_ID_3:  
      ps_subs_id = PS_SYS_TERTIARY_SUBS;
      break;                  
      
    default:
      break;
  }
  LOG_MSG_INFO3_2("ps_utils_convert_subs_sys_to_ps(): "
    "PS subs id %d CM subs ID %d",ps_subs_id,cm_subs_id);
...
/*===========================================================================
FUNCTION      PS_UTILS_SUBS_ID_DSPROFILE_TO_PS

DESCRIPTION   
   This function converts the input DS Profile subscription id to PS subscription id.

DEPENDENCIES  
   None.

RETURN VALUE 
   PS SYS Subscirption id

SIDE EFFECTS  
   None.
===========================================================================*/
ps_sys_subscription_enum_type ps_utils_subs_id_dsprofile_to_ps
(
  ds_profile_subs_etype  profile_subs_id
)
{
  ps_sys_subscription_enum_type ps_subs_id = PS_SYS_DEFAULT_SUBS;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  return ps_subs_id;
}/* ps_utils_subs_id_dsprofile_to_ps() */
...
/*===========================================================================
  ...
  DESCRIPTION
    This function retrieves the specified item from NV. caller should call api with valid subs id
...
(
  ...
  sys_modem_as_id_e_type         subs_id         /* Subscription ID's*/
)
{
  rex_sigs_type                      sig_mask[PS_MAX_SIG_ARR_LEN];
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  ps_nv_cmd_ext_buf.context = subs_id;
...
/*===========================================================================
  ...
  DESCRIPTION
    Write an item to NV memory.  Wait until write is completed. caller should call api with valid subs id
...
/*=========================================================================== 
  ...
  SIDE EFFECTS
    None
===========================================================================*/
ps_sys_subscription_enum_type ps_utils_convert_subs_mcfg_to_ps
(
...
/*SYS_MODEM_AS_ID_3*/
  ...
  return subs_id;
}/*ps_utils_convert_subs_mcfg_to_ps*/
...
)
{
  ...
  do
  {
    ...
    ps_refresh_info.refresh_subs =
            ps_utils_convert_subs_mcfg_to_ps(mcfg_event_info->sub_index);
...
PARAMETERS
  sig_bit_field - pointer to begining of signal bit field
  length - length of signal bit fields
  buffer - assumed to be long enough to handle all the set bits.
...
SIDE EFFECTS
  None.
===========================================================================*/
uint32 ps_find_set_bits(rex_sigs_type* sig_bit_field, uint32 length, uint32* buffer)
{
  uint32 pos;
  uint32 set_bits;
  uint32 i;
...
/*===========================================================================
FUNCTION PS_UTILS_MATCH_ONE_FLTR_IN_FLTRSPEC()

DESCRIPTION
  This function receives one filter specification 
...
PARAMETERS
  ...
  filter_spec_of_flow  : pointer to one filter to be matched to
...
SIDE EFFECTS
  None
===========================================================================*/
int32 ps_utils_match_one_fltr_in_fltrspec
(  
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  -------------------------------------------------------------------------*/
   ...
   {
      if ( TRUE == ps_utils_match_ip_fltr(filter_spec_of_flow->list_ptr + index, 
                                 app_specified_filter))
      {
         /* Match found */
         LOG_MSG_INFO1_2("ps_utils_match_one_fltr_in_fltrspec(): "
                         "Match: app req filter obj 0x%p matched NW filter obj 0x%p",
                         app_specified_filter,
  ...
  /* We went over a list of NIQ filters but
   ...
   return -1;       
} /* ps_utils_match_one_fltr_in_fltrspec() */


/*===========================================================================
FUNCTION PS_UTILS_MATCH_IP_FLTR

DESCRIPTION
  This function matches a specified filter with another specified filter.

PARAMETERS
  fltr_ptr1 - Filter 1 against the filter2 is matched. This filter spec may have a range of UDP/TCP ports
  fltr_ptr2 - Filter 2 which needs to be matched. This filter spec cannot have a range of UDP/TCP ports
...
===========================================================================*/
boolean ps_utils_match_ip_fltr
(
...
)
{
  ...
  uint8 tmp_prot;
  uint8 local_error = PS_UTILS_SUCCESS;
    ...
    case IP_V4:
    {
        ...
        while(tmp_mask)
        {
          if(tmp_mask & IPFLTR_MASK_IP4_SRC_ADDR)
          {
            ...
            {
              local_error = PS_UTILS_V4_IP_SRC_ADDR_MISMATCH;
              goto send_result;
            }
          ...
          if(tmp_mask & IPFLTR_MASK_IP4_DST_ADDR)
          {
            ...
            {
              local_error = PS_UTILS_V4_IP_DST_ADDR_MISMATCH;
              goto send_result;
            }
          ...
          if(tmp_mask & IPFLTR_MASK_IP4_NEXT_HDR_PROT)
          {
            if (PS_IPPROTO_TCP_UDP == fltr_ptr1->ip_hdr.v4.next_hdr_prot)
            {
              ...
              {
                local_error = PS_UTILS_V4_HDR_PORT_INVALID;
                goto send_result;
              }
            }
            else if (fltr_ptr1->ip_hdr.v4.next_hdr_prot != fltr_ptr2->ip_hdr.v4.next_hdr_prot)
            {
              local_error = PS_UTILS_V4_HDR_PORT_MISMATCH;
              goto send_result;
            }
          ...
          if(tmp_mask & IPFLTR_MASK_IP4_TOS)
          {
            ...
            {
              local_error = PS_UTILS_V4_TOS_MISMATCH;
              goto send_result;
            }
          ...
          if (0 != tmp_mask)
          {
            local_error = PS_UTILS_V4_PARAM_UNKNOWN;
          }
        }
    }
    break;
    ...
    case IP_V6:
    {
      ...
      while(tmp_mask)
      {
        if (tmp_mask & IPFLTR_MASK_IP6_SRC_ADDR)
        {
          ...
          {
            local_error = PS_UTILS_V6_SRC_PREFIX_MISMATCH;
            goto send_result;
        ...
        if (tmp_mask & IPFLTR_MASK_IP6_DST_ADDR)
        {
          ...
          {
            local_error = PS_UTILS_V6_DST_PREFIX_MISMATCH;
            goto send_result;
          }
        ...
        if (tmp_mask & IPFLTR_MASK_IP6_NEXT_HDR_PROT)
        {
          if (PS_IPPROTO_TCP_UDP == fltr_ptr1->ip_hdr.v6.next_hdr_prot)
          {
            ...
            {
              local_error = PS_UTILS_V6_HDR_PROT_INVALID;
              goto send_result; 
            }
          }
          ...
          {
            local_error = PS_UTILS_V6_HDR_PROT_MISMATCH;
            goto send_result;
          }
        ...
        if (tmp_mask & IPFLTR_MASK_IP6_TRAFFIC_CLASS)
        {
          ...
          {
            local_error = PS_UTILS_V6_TRF_MISMISMATCH;
            goto send_result;
        ...
        if (tmp_mask & IPFLTR_MASK_IP6_FLOW_LABEL)
        {
          ...
          {
            local_error = PS_UTILS_V6_FLOW_MISMATCH;
            goto send_result;
          }
        ...
        if (0 != tmp_mask)
        {
          local_error = PS_UTILS_V6_PARAM_UNKNOWN;
        }
      }
    }
    break;
  ...
  switch(next_hdr_prot)
  {
    case PS_IPPROTO_TCP:
    {
      ...
      if((tmp_mask & IPFLTR_MASK_TCP_SRC_PORT) &&
         FALSE == ps_utils_match_port_range (
      ...
      {
         local_error = PS_UTILS_TCP_SRC_PORT_MISMATCH;
         goto send_result;
      }
      ...
      {
         local_error = PS_UTILS_TCP_DST_PORT_MISMATCH;
         goto send_result;
      }
    }
    break;
    ...
    case PS_IPPROTO_UDP:
    {
      ...
      {
        local_error = PS_UTILS_UDP_SRC_PORT_MISMATCH;
        goto send_result;
      }
      ...
      {
        local_error = PS_UTILS_UDP_DST_PORT_MISMATCH;
        goto send_result;
      }
    }
    break;
    ...
    case PS_IPPROTO_ICMP6:
    {
      ...
      {
        local_error = PS_UTILS_ICMP_MSG_MISMATCH;
        goto send_result;
      ...
      {
        local_error = PS_UTILS_ICMP_MSG_CODE_MISMATCH;
        goto send_result;
    ...
    case PS_IPPROTO_ESP:
    {
      ...
      {
         local_error = PS_UTILS_ESP_SPI_MISMATCH;
          goto send_result;
      }
    }
    break;
    ...
    case PS_IPPROTO_AH:
    {
      ...
      {
        local_error = PS_UTILS_AH_SPI_MISMATCH;
         goto send_result; 
      }
    }
    break;
    ...
    case PS_IPPROTO_TCP_UDP:
    {
      ...
      switch (tmp_prot)
      {
        ...
        case PS_IPPROTO_UDP:
          ...
          {
            local_error = PS_UTILS_TCP_UDP_SRC_PORT_RANGE_MISMATCH;
            goto send_result;
          }
          ...
          {
              local_error = PS_UTILS_TCP_UDP_DST_PORT_RANGE_MISMATCH;
              goto send_result;
          }
          break;
...
send_result:
  LOG_MSG_INFO1_1("ps_utils_match_ip_fltr(): Filters Match local error %d ", local_error);
  if(local_error == PS_UTILS_SUCCESS)
  {
  /*-------------------------------------------------------------------------
      If we reach here with local error still 0, all the specified parameters matched
    ---------------------------------------------------------------------*/
    result = TRUE;
  }
  else 
  ...
  return result;
} /*ps_utils_match_ip_fltr*/

/*===========================================================================
FUNCTION ps_utils_match_port_range 
...
PARAMETERS
  port_1  - port number for first set of filter
  range_1 - port range for first set of filter
  port_2  - port number for second set of filter
  range_2 - port range for second set of filter
...
===========================================================================*/
boolean ps_utils_match_port_range 
(
  uint16 port_1,
  uint16 range_1,
  uint16 port_2,
...
)
{
  ...
  if (port_1 + range_1 < port_2 + range_2)
  {
    port_match = FALSE;
    goto send_result;
  }
  ...
  send_result:
    LOG_MSG_INFO1_5("ps_utils_match_port_range() "
                  "port_1: %d, range_1: %d port_2: %d range_2: %d match result %d",
                  (int)port_1, (int)range_1, (int)port_2, (int)range_2, port_match);
    return port_match;
}/* ps_utils_match_port_range  */

/*===========================================================================
FUNCTION PS_UTILS_GET_FLTR_MATCH_RESULT()

DESCRIPTION
  This function returns the matched direction based on requested direction
  and UL/DL filter matches

PARAMETERS
  fltr_req_dir      : requested direction
  is_ul_matched     : TRUE if filter ma
  qos_spec_ptr      : pointer to the qos spec of the flow

RETURN VALUE
  Direction if matched
  Otherwise PS_QOS_FILTR_DIR_MAX
...
SIDE EFFECTS
  None
===========================================================================*/
ps_iface_qos_fltr_match_dir_enum_type ps_utils_get_fltr_match_result
(
  ps_iface_qos_fltr_match_dir_enum_type   fltr_req_dir,
  boolean                                 is_ul_matched,
  boolean                                 is_dl_matched
)
{
  ps_iface_qos_fltr_match_dir_enum_type   ret_dir = PS_QOS_FILTR_DIR_MAX;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

  /*-------------------------------------------------------------------------
    Check if the filter matches with given direction
  -------------------------------------------------------------------------*/
  switch (fltr_req_dir)
  {
    case PS_QOS_FILTR_DIR_UL:
      if (TRUE == is_ul_matched)
      {
        ret_dir = PS_QOS_FILTR_DIR_UL;
      }
      break;
    case PS_QOS_FILTR_DIR_DL:
      if (TRUE == is_dl_matched)
      {
        ret_dir = PS_QOS_FILTR_DIR_DL;
      }
      break;
    case PS_QOS_FILTR_DIR_BIDIR:
      if (TRUE == is_ul_matched && TRUE == is_dl_matched)
      {
        ret_dir = PS_QOS_FILTR_DIR_BIDIR;
      }
      break;
    case PS_QOS_FILTR_DIR_UL_OR_DL:
      if (TRUE == is_ul_matched && TRUE == is_dl_matched)
      {
        ret_dir = PS_QOS_FILTR_DIR_BIDIR;
      }
      else if (TRUE == is_ul_matched)
      {
        ret_dir = PS_QOS_FILTR_DIR_UL;
      }
      else if (TRUE == is_dl_matched)
      {
        ret_dir = PS_QOS_FILTR_DIR_DL;
      }
      break;
  ...
  LOG_MSG_INFO1_4("ps_utils_get_fltr_match_result(): fltr_req_dir %d, "
                  "is_ul_matched %d, is_dl_matched %d, matched dir%d",
                   fltr_req_dir,
                   is_ul_matched,
                   is_dl_matched,
                   ret_dir);
  ...
  return ret_dir;
} /* ps_utils_get_fltr_match_result */
