...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/utils/src/ps_mem.c#2 $
  $DateTime: 2019/10/22 02:53:17 $Author: $
...
static char *ps_mem_pool_name_arr[] =
{
  ...
  "PS_MEM_RAN_ASST_MEDIA_ADAPT_MANAGER_TYPE",
  "PS_MEM_RAN_ASST_MEDIA_ADAPT_REQUEST_TYPE",
  "MAX_POOLS"
};
...
/*===========================================================================
FUNCTION     PS_MEM_GET_REF_CNT()

DESCRIPTION  This function returns ref count for buffer.

RETURN VALUE Ref cnt

DEPENDENCIES None
===========================================================================*/
int16 ps_mem_get_ref_cnt
(
  void   * buf
)
{
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ( buf == NULL )
  {
    LOG_MSG_INFO2_0("ps_mem_dup(): NULL ptr");
    return -1;
  }
  ...
  do
  {
    ...
    if (!ps_mem_is_valid(buf, (ps_mem_pool_enum_type)poolid))
    {
      LOG_MSG_ERROR_1("ps_mem_dup(): Invalid buf 0x%p", buf);
      DS_UTILS_ASSERT(0);
      break;
    }

    ret_val = buf_hdr->ref_cnt;
  } while(0);
...
)
{
  ...
  {
    DS_UTILS_ERROR_FATAL("ps_mem_get_stat_instance_all(): Invalid stat value passed to ps_mem_get_stat_i_all");
  ...
  {
    DS_UTILS_ERROR_FATAL("ps_mem_get_stat_instance_all:Null value of pool_info_ptr passed");
    return E_NOT_ALLOWED ;
  }
...
/*===========================================================================
FUNCTION PS_MEM_GET_NUM_USED

DESCRIPTION
  This function is to get the number of used items in particular pool.
...
{
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if (poolid < 0 || poolid >= PS_MEM_MAX_POOLS)
  {
    LOG_MSG_ERROR_1("ps_mem_get_num_used(): "
                    "Invalid pool id %d", poolid);
    return -1;
  }

  if (NULL == num_used)
  {
    LOG_MSG_ERROR_0("ps_mem_get_num_used(): "
                    "NULL args");
    return -1;
  }

  *num_used = (int32)ps_mem_pool_info[poolid].num_used;
  return 0;
} /* ps_mem_get_num_used() */
...
{
  qsh_mdump_collect_high(&ps_mem_pool_info, sizeof(ps_mem_pool_info_s_type));
}
#endif /* FEATURE_QSH_MDUMP */
