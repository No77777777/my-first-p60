...
Copyright (c) 2007-2013,2017 Qualcomm Technologies Incorporated. 
All Rights Reserved.
...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/dssvc/src/ps_dnsi_msg_mgr.c#3 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
08/03/17   skc     Replaced strcasecmp() by strncasecmp().
...
typedef enum
{
  ...
  TYPE_SRV        = 33,        /*   Service record                  [RFC 2782] */
  TYPE_NAPTR      = 35        /*   Naming Authority Pointer     [RFC 2915] */
...
int16  ps_dnsi_extract_string_field
(
  dsm_item_type    * dns_response_ptr,
  int16              offset,
  uint8            * dst_buf_ptr,
...
)
{
  ...
  if( NULL == dns_response_ptr || NULL == dst_buf_ptr)
  {
    LOG_MSG_ERROR_0("ps_dnsi_extract_string_field(): NULL parameters");
    return DSS_ERROR;
  }
  /*-----------------------------------------------------------------------
  ...
  if( sizeof(uint8) != dsm_extract( dns_response_ptr,
                                    ...
                                    sizeof(uint8) ) )
  {
    LOG_MSG_ERROR_1("Cant extract  length from DSM chain 0x%p",
                    dns_response_ptr);
    return DSS_ERROR;
  }
  LOG_MSG_INFO1_2("DEBUG(Temp) ps_dnsi_extract_string_field len %d offset %d", len, offset);

  /*-----------------------------------------------------------------------
    Length is 0 hence skip extract 
  -----------------------------------------------------------------------*/
  if( 0 == len )
  {
    return len + 1;
  ...
  /*-----------------------------------------------------------------------
     Length is more than destination address should not be parsed.
  -----------------------------------------------------------------------*/
  if( len > dst_buf_len )
  {
    return DSS_ERROR;
  ...
  /*---------------------------------------------------------------------
    Move offset past len byte and extract string field value into destination buffer.
  ---------------------------------------------------------------------*/
  if( len != dsm_extract( dns_response_ptr,
                          offset,
                          dst_buf_ptr,
                          len ) )
  {
    LOG_MSG_ERROR_2("Cant extract  string value from DSM chain 0x%p into buffer 0x%p",
                    dns_response_ptr, dst_buf_ptr);
    return DSS_ERROR;
  }
  /*---------------------------------------------------------------------
    Total Length includes 1 length byte + Field Length value .
  ---------------------------------------------------------------------*/
  return (int16)(len + 1);
}/* ps_dnsi_extract_string_field */

/*===========================================================================
FUNCTION PS_DNSI_MSG_MGRI_EXTRACT_NAPTR_FIELDS
...
SIDE EFFECTS
  None.
===========================================================================*/
static int16 ps_dnsi_msg_mgri_extract_naptr_fields
(
  dsm_item_type    * dns_response_ptr,
  int16              offset,
  char             * dst_buf_ptr
...
{
  ps_dnsi_generic_naptr_rdata_type    *naptr_record_ptr = NULL;
  uint16                               temp_data        = 0;
  int16                                len              =  0;
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if( NULL == dns_response_ptr || NULL == dst_buf_ptr)
  {
    LOG_MSG_ERROR_0("ps_dnsi_msg_mgri_extract_naptr_fields(): NULL parameters");
    return DSS_ERROR;
  }

  naptr_record_ptr =  ( ps_dnsi_generic_naptr_rdata_type *)dst_buf_ptr;

  /*-------------------------------------------------------------------------
    RFC 2915 Order : A 16-bit unsigned integer specifying the order in which the NAPTR
  ...
  if( sizeof(uint16) != dsm_extract(  dns_response_ptr,
                                      ...
                                      sizeof(uint16) ) )
  {
    LOG_MSG_ERROR_0("NAPTR order extract failed");
    return DSS_ERROR;
  }
  offset += sizeof(uint16);
  naptr_record_ptr->order = dss_ntohs( temp_data );
  
  LOG_MSG_INFO1_3("DEBUG(Temp) NAPTR order %d hostorder %d  offset %d",
                  temp_data, naptr_record_ptr->order, offset);
  /*-------------------------------------------------------------------------
    RFC 2915 Preference : A 16-bit unsigned integer that specifies the order in which NAPTR
      records with equal "order" values SHOULD be processed, low numbers being processed 
      before high numbers..
  -------------------------------------------------------------------------*/
  if( sizeof(uint16) != dsm_extract(  dns_response_ptr,
                                      offset,
                                      ...
                                      sizeof(uint16) ) )
  {
    LOG_MSG_ERROR_0("NAPTR preference extract failed");
    return DSS_ERROR;
  }
  offset += sizeof(uint16);
  naptr_record_ptr->preference = dss_ntohs( temp_data );
  
  LOG_MSG_INFO1_3("DEBUG(Temp) NAPTR preference %d hostpref %d  offset %d",
                  temp_data, naptr_record_ptr->order, offset);
  /*-------------------------------------------------------------------------
    RFC 2915 Flags : A <character-string> containing flags to control aspects of the rewriting 
    and interpretation of the fields in the record.  Flags are single characters from the set [A-Z0-9]
  -------------------------------------------------------------------------*/
   len = ps_dnsi_extract_string_field( dns_response_ptr, offset, 
           &naptr_record_ptr->flag[0], PS_DNSI_MAX_FLAG_NAME_LEN );
  ...
  offset += len;
  /*-------------------------------------------------------------------------
    RFC 2915 Service : A <character-string> Specifies the service(s) available down this rewrite path.
    It may also specify the particular protocol that is used to talk with a service.
  -------------------------------------------------------------------------*/
   len = ps_dnsi_extract_string_field( dns_response_ptr, offset, 
           &naptr_record_ptr->service[0], PS_DNSI_MAX_DOMAIN_NAME_LEN );
  ...
  offset += len;
  /*-------------------------------------------------------------------------
    RFC 2915 Regexp : A <character-string> A STRING containing a substitution expression that is 
    applied to the original string held by the client in order to construct the next domain name to lookup.
  -------------------------------------------------------------------------*/
   len = ps_dnsi_extract_string_field( dns_response_ptr, offset, 
           &naptr_record_ptr->regxp[0], PS_DNSI_MAX_DOMAIN_NAME_LEN );
  ...
  offset += len;
  /*-------------------------------------------------------------------------
    RFC 2915 Replacement : A <domain-name> which specifies the new value in the
      case where the regular expression is a simple replacement operation.
  -------------------------------------------------------------------------*/
  offset = ps_dnsi_msg_mgri_extract_domain_name( dns_response_ptr, offset, 
           (char *)&naptr_record_ptr->replacement[0] );
  ...
  return DSS_SUCCESS;
}/* ps_dnsi_msg_mgri_extract_naptr_fields */

/*===========================================================================
FUNCTION PS_DNSI_MSG_MGRI_EXTRACT_SRV_FIELDS
...
SIDE EFFECTS
  None.
===========================================================================*/
static int16 ps_dnsi_msg_mgri_extract_srv_fields
(
  dsm_item_type    * dns_response_ptr,
  int16              offset,
  char             * dst_buf_ptr
...
{
  ps_dnsi_generic_srv_rdata_type      *srv_record_ptr  = NULL;
  uint16                               temp_data        = 0;
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if( NULL == dns_response_ptr || NULL == dst_buf_ptr)
  {
    LOG_MSG_ERROR_0("ps_dnsi_msg_mgri_extract_srv_fields(): NULL parameters");
    return DSS_ERROR;
  }

  srv_record_ptr =  ( ps_dnsi_generic_srv_rdata_type *)dst_buf_ptr;

  /*-------------------------------------------------------------------------
    RFC 2782 Priority : The priority of this target host.  A client MUST attempt to
  ...
  if( sizeof(uint16) != dsm_extract(  dns_response_ptr,
                                      ...
                                      sizeof(uint16) ) )
  {
    LOG_MSG_ERROR_0("SRV Prioirty extract failed");
    return DSS_ERROR;
  }
  offset += sizeof(uint16);
  srv_record_ptr->priority= dss_ntohs( temp_data );
  
  LOG_MSG_INFO1_3("DEBUG(Temp) SRV priority %d hostorder %d  offset %d",
                  temp_data, srv_record_ptr->priority, offset);
  
  /*-------------------------------------------------------------------------
    RFC 2782 Weight : A server selection mechanism.  The weight field specifies a
  ...
  offset += sizeof(uint16);
  srv_record_ptr->weight= dss_ntohs( temp_data );
  
  LOG_MSG_INFO1_3("DEBUG(Temp) SRV weight %d hostorder %d  offset %d",
                  temp_data, srv_record_ptr->weight, offset);

  /*-------------------------------------------------------------------------
    RFC 2782 Port : The port on this target host of this service.  The range is 0-
  ...
  offset += sizeof(uint16);
  srv_record_ptr->port= temp_data ;
  
  LOG_MSG_INFO1_3("DEBUG(Temp) SRV port %d hostorder %d  offset %d",
                  temp_data, srv_record_ptr->port, offset);
  
  /*-------------------------------------------------------------------------
    RFC 2915 Target : A <domain-name>The domain name of the target host. 
    There MUST be one or more address records for this name, the name MUST NOT be an alias 
    (in the sense of RFC 1034 or RFC 2181).
    A Target of "." means that the service is decidedly not  available at this domain.
  -------------------------------------------------------------------------*/
  offset = ps_dnsi_msg_mgri_extract_domain_name( dns_response_ptr, offset, 
           (char *)&srv_record_ptr->target[0] );
  ...
  return DSS_SUCCESS;
}/* ps_dnsi_msg_mgri_extract_srv_fields */

/*===========================================================================
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/  
  ...
  *offset_ptr += sizeof(uint16);
  ...
  /*-------------------------------------------------------------------------
    ...
    For TYPE_NAPTR, rdata_len should not exceed generic NAPTR response (theoretical)
    For TYPE_SRV, rdata_len should not exceed generic SRV response (theoretical)
    DSS_DNS_MAX_DOMAIN_NAME_LEN
  -------------------------------------------------------------------------*/
  ...
  if( 0 == rdata_len ||
      !( ((uint16)TYPE_A == rr_type && sizeof(uint32) == rdata_len) ||
         ...
         ((uint16)TYPE_NAPTR == rr_type && sizeof(ps_dnsi_generic_naptr_rdata_type) >= rdata_len)||
         ((uint16)TYPE_SRV == rr_type && sizeof(ps_dnsi_generic_srv_rdata_type) >= rdata_len)) )
  {
  ...
  else if( (uint16)TYPE_NAPTR == rr_type )
  {
    rr_node->rdata_len = sizeof(ps_dnsi_generic_naptr_rdata_type);
    rr_node->rdata = ps_dnsi_mem_alloc(sizeof(ps_dnsi_generic_naptr_rdata_type));
    if( NULL != rr_node->rdata )
    {
      memset( rr_node->rdata, 0x0, sizeof(ps_dnsi_generic_naptr_rdata_type));
    }
  }
  else if( (uint16)TYPE_SRV == rr_type )
  {
     rr_node->rdata_len = sizeof(ps_dnsi_generic_srv_rdata_type);
    rr_node->rdata = ps_dnsi_mem_alloc(sizeof(ps_dnsi_generic_srv_rdata_type));
    if( NULL != rr_node->rdata )
    {
      memset( rr_node->rdata, 0x0, sizeof(ps_dnsi_generic_srv_rdata_type));
    }
  }
  else
  {
  ...
  LOG_MSG_INFO2_2("ps_dnsi_msg_mgri_get_generic_rr(): rr_node %d  rr_rdata %p",rr_node, rr_node->rdata);
  ...
  switch( rr_type )
  {
    ...
    case TYPE_NAPTR:
    {
      rr_node->rr_type = PS_DNSI_QUERY_TYPE_NAPTR;

      if (DSS_ERROR == ps_dnsi_msg_mgri_extract_naptr_fields( dns_response_ptr,
                                                  *offset_ptr,
                                                  (char *)rr_node->rdata ))
      {
        LOG_MSG_ERROR_0("Error extracting NAPTR RR");
        error_flag = TRUE;
      }
      *offset_ptr += rdata_len;
      break;
    ...
    case TYPE_SRV:
    {
       rr_node->rr_type = PS_DNSI_QUERY_TYPE_SRV;
    
       if (DSS_ERROR == ps_dnsi_msg_mgri_extract_srv_fields( dns_response_ptr,
                                                   *offset_ptr,
                                                   (char *)rr_node->rdata ))
       {
         LOG_MSG_ERROR_0("Error extracting SRV RR");
         error_flag = TRUE;
       }
       *offset_ptr += rdata_len;
       break;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  *ps_errno = DSS_SUCCESS;
  ...
  /*-------------------------------------------------------------------------
    ...
    Note: No Special handling for CNAME here. CNAME records will be treated as Normal resouce record
    and API control block will take next action (Either do recursive query or return to application). 
  -------------------------------------------------------------------------*/
