...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/dssvc/src/ps_dnsi_resolver.c#2 $$DateTime: 2019/10/22 02:53:17 $$Author: pwbldsvc $
...
#define DNS_RESOLVER_ASSERT(expression) \
         dns_resolver_assert_wrapper(__LINE__, expression)

/*===========================================================================

FUNCTION dns_host_assert_wrapper()
...
===========================================================================*/
static void dns_resolver_assert_wrapper
(
...
)
{
  ...
  dss_dns_linear_backoff_retry_info_type
                                   query_retry_params;
  dss_dns_exp_retry_info_type      query_exp_retry_params;
  ...
  while (TRUE == iterate)
  {
    ...
    switch (res_cb_ptr->state)
    {
      case  PS_DNSI_RESOLVER_STATE_START:
        ...
        -------------------------------------------------------------------*/
        DNS_RESOLVER_ASSERT (PS_DNSI_RESOLVER_EVENT_NONE == res_cb_ptr->event);
	...
	/* DS will run exponential timer if client sets exponential timer.
		Exponentioal factor specified in seconds and timer runs in milliseconds format. */

	retval = 
             dss_dns_get_config_params (
                             res_cb_ptr->session_handle,
                             DSS_DNS_CONFIG_PARAMS_EXP_RETRY_INFO,
                             &query_exp_retry_params,
                             sizeof (dss_dns_exp_retry_info_type),
                             &ps_errno);

       
       if(retval == DSS_SUCCESS && query_exp_retry_params.is_exp_timer_valid)
       {
        /*
        Calculate the timeout value based on the query_retry_params
        */
	 for(exp_itr = 0; (exp_itr < res_cb_ptr->io_retries); exp_itr++)
         {
           query_timeout = query_timeout*query_exp_retry_params.exp_time_factor;		   
         }
      ...
      else
      {
       ...
       LOG_MSG_INFO2_2 ("Starting timer for IO mgr event for %d ms  retries %d ", 
										query_timeout,res_cb_ptr->io_retries); 
      ...
      case PS_DNSI_RESOLVER_STATE_FAIL:
          ...
          case  PS_DNSI_RESOLVER_EVENT_ERROR_MSG_MGR_PARSE_RESP:
            ...
            retval = 
              dss_dns_get_config_params (
                             ...
                             sizeof (dss_dns_exp_retry_info_type),
                             &ps_errno);
           if(query_exp_retry_params.is_exp_timer_valid)
           {
              if (DSS_ERROR == retval)
              {
                LOG_MSG_ERROR_0 ("Get session info exp RETRY_MECHANISM_INFO failed");
                res_cb_ptr->state = PS_DNSI_RESOLVER_STATE_FAIL;
                res_cb_ptr->event = PS_DNSI_RESOLVER_EVENT_ERROR_SESSION_MGR;
                iterate = TRUE;
                break;
              }
	      max_retries = query_exp_retry_params.max_retries;
           }
           else
           {
            retval = 
	    ...
	    max_retries = query_retry_params.max_retries;
	  }
            /*
            Check that number of retries does't exceed the session setting 
            query_retry_params.
            */
            if (max_retries > res_cb_ptr->io_retries++)
            {
               int32 retval32;
              /*-------------------------------------------------------------
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_INFO2_2 ("ps_dnsi_resolveri_cmd_handler ps_cmd %d res_data 0x%x", ps_cmd, res_data);
  ...
  if (NULL == res_cb_ptr || res_cb_ptr != ps_dnsi_resolveri_is_handle_valid(res_cb_ptr->handle))
  {
  ...
  if ( api_cb_ptr->hints_info.ai_flags & (int)DSS_DNS_AI_FLAGS_LOCAL_QUERY ) {
     res_cb_ptr->state = PS_DNSI_RESOLVER_STATE_FAIL;
