...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/interface/dssvc/src/dss_dns_session_mgr.c#2 $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /*------------------------------------------------------------------------
    Delete results (Internal generic records) then queue
  -------------------------------------------------------------------------*/
  while ( NULL != (result_cb_ptr = q_get( &session_mgr_ptr->passive_results_q ) ) )
  {
    ...
    ps_dnsi_mem_free((void **)&result_cb_ptr);
  }
  q_destroy(&session_mgr_ptr->passive_results_q);
...
SIDE EFFECTS
  None.
===========================================================================*/
ps_dnsi_session_mgr_cb_type*  dss_dns_get_session_mgr_from_handle
(
  dss_dns_session_mgr_handle_type     session_handle
)
{
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  PS_LEAVE_CRIT_SECTION(&global_ps_crit_section);
  return session_mgr_ptr;
} /* dss_dns_get_session_mgr_from_handle() */
/*===========================================================================
...
/*===========================================================================
FUNCTION  DSS_DNS_CREATE_SESSION_INTERNAL()
...
PARAMETERS
  ...
  app_cback_f_ptr_ext   - Application registered callback function pointer.
...
SIDE EFFECTS
  None.
===========================================================================*/
dss_dns_session_mgr_handle_type   dss_dns_create_session_internal
(
  dss_dns_cback_f_type            app_cback_f_ptr,
  dss_dns_cback_f_type_ex        app_cback_f_ptr_ext,
  void                          * user_data_ptr,
  int16                         * dss_errno
)
{
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  session_mgr_ptr->cback_f_ptr_ext      = app_cback_f_ptr_ext;
  ...
  /* Initialize the passive result queues and related parameters */
  memset(&(session_mgr_ptr->passive_results_q), 0,
         sizeof(session_mgr_ptr->passive_results_q));
  (void) q_init(&(session_mgr_ptr->passive_results_q));
...
)
{
  ...
  retval = dss_dns_create_session_internal(app_cback_f_ptr, NULL, user_data_ptr, dss_errno);
...
)
{
  ...
  retval = dss_dns_create_session_internal(NULL, app_cback_f_ptr_ext, user_data_ptr, dss_errno);
 
  return retval;
}/* dss_dns_create_session_ex */
  ...
  switch (param_name)
  {
    ...
    case DSS_DNS_CONFIG_PARAMS_NET_POLICY:
      ...
      else
      {
        ...
        {
           LOG_MSG_INFO1_3 ("Net policy was set obj 0x%p, iface kind is %d, Iface id is %d",
                            &(session_mgr_ptr->net_policy),
                            session_mgr_ptr->net_policy.iface.kind,
                            session_mgr_ptr->net_policy.iface.info.id);
        }
        else
        {
           LOG_MSG_INFO1_3 ("Net policy was set obj 0x%p, iface kind is %d, Iface name is %d",
                            &(session_mgr_ptr->net_policy),
                            session_mgr_ptr->net_policy.iface.kind,
    ...
    case DSS_DNS_CONFIG_PARAMS_EXP_RETRY_INFO:
      PS_ENTER_CRIT_SECTION(&global_ps_crit_section);
      if (sizeof(dss_dns_exp_retry_info_type) != param_len ||
	  	    DSS_DNS_DEFAULT_MAX_QUERY_RETRIES <
          ((dss_dns_exp_retry_info_type *)param_val_ptr)->max_retries || 
          (!(((dss_dns_exp_retry_info_type *)param_val_ptr)->exp_time_factor)))
      {
        LOG_MSG_ERROR_3 ("Invalid param length for %d or max_retries is larger the MAX supported %d > %d", 
			                  param_name,((dss_dns_exp_retry_info_type*)param_val_ptr)->max_retries,DSS_DNS_DEFAULT_MAX_QUERY_RETRIES);
        *dss_errno = DS_EFAULT;
      }
      else
      {
         memscpy (&session_mgr_ptr->exp_timer_params_info,
           param_len, param_val_ptr, param_len);
         retval = DSS_SUCCESS;        
      }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch (param_name)
  {
    ...
    case DSS_DNS_CONFIG_PARAMS_EXP_RETRY_INFO:
      ...
      {
        memscpy (param_val_ptr, param_len,
          &(session_mgr_ptr->exp_timer_params_info),
          param_len);
        retval = DSS_SUCCESS;
      }
      PS_LEAVE_CRIT_SECTION(&global_ps_crit_section);
...
/*===========================================================================
FUNCTION DSS_DNS_SESSION_MGR_IS_LEGACY_SESSION
...
SIDE EFFECTS
  None.
===========================================================================*/
boolean dss_dns_mgr_is_legacy_session
(
...
)
{
  ...
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if ( NULL != session_ptr && 
       NULL != session_ptr->cback_f_ptr )
  {
    return TRUE;
  }

  return FALSE;
}/* dss_dns_mgr_is_legacy_session_type */
