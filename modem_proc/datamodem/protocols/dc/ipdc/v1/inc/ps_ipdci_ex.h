...
--------   ---     ----------------------------------------------------------
06/13/16   cx      Added support for super large dsm
...
#define IPDC_DACC_STATIC_HDR_SIZE_IN_BYTES     1
...
typedef struct 
{
  ...
  /* DACC compression gain % for this flow
	 This gain is used to track whether the flow is incompressible or not.
	 The orignal compression gain calculation is not accurate because of 
	 Introduction of short header. The new compression gain calculation only
	 consider the compression gain comes from DACC. If the DACC gain is lower
	 thank the threshold, we do the header only compression, otherwise full
	 packet compression. The threshold is set to 3% 
...
typedef struct 
{
  ...
  /* The index for the most recent flow */
  uint8                     mru_flow_index;

  /* The number of active flows */
  uint8                     num_active_flow;
...
typedef struct 
{
  ...
  /* the calculated compressed packet size */
  uint16                              cmp_pkt_len_app;
