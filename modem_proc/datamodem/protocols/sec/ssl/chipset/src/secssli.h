...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/protocols/sec/ssl/chipset/src/secssli.h#2 $
  $Date: 2025/11/09 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
06/22/18   shrvn   Added support for SSL_VERIFY_NONE
02/14/18   ggrewal FR 46987: Google Server Certificate Management for modem SSL
09/21/16   ks      DS Independent TLS
09/09/16   nm      TLS-PSK AES256/3des CS support
06/20/16   nm      TLS-PSK support
04/04/16   nm      support to SetSessoionCert with Name or RAM Cert_ID 
12/11/15   nm      TLSv1.2 support
04/15/15   nm      AES256 support
...
#define SECSSL_CIPHER_LIST_SIZE   13
...
{                                                                             \
  ...
  { SECSSL_KEY_PSK,           SECSSL_CRYPT_AES128, SECSSL_HASH_SHA, 0x8c00 }, \
  { SECSSL_KEY_PSK,           SECSSL_CRYPT_3DES,   SECSSL_HASH_SHA, 0x8b00 }, \
  { SECSSL_KEY_PSK,           SECSSL_CRYPT_AES256, SECSSL_HASH_SHA, 0x8d00 }, \
}
...
typedef struct
{
  ...
  secx509_cert_type*                    root_ca_cert_ptr[SECSSL_SES_MAX_ROOT_CERTS];
  ...
  uint8    client_public_key[SECSSL_PUBLIC_KEY_MAX_SIZE] __attribute__((aligned));
  ...
  uint8    server_psk_hint[SECSSL_PUBLIC_KEY_MAX_SIZE];
  uint16   server_psk_hint_size;
  ...
  secssl_ses_psk_data_type         psk_data;
  /* set to TRUE, TLS has to use non-DS EVENTs */
  boolean   is_tlsevent; 
  /* If Session Cert is set with Cert name*/
  boolean ses_ca_cert_name_set;
  boolean   enable_dynamic_certs;
  /* SSL Verify Mode */
...
/*==========================================================================
FUNCTION secssli_dynamic_cert_lookup_response_proc_sig
 
DESCRIPTION 
  This function processes the dynamic certificates returned from AP
...
SIDE EFFECTS
  None
===========================================================================*/
extern void secssli_dynamic_cert_lookup_response_proc_sig( void );
...
FUNCTION secssli_set_ses_enable_dynamic_certs

DESCRIPTION
  To enable/disable fetching of certificates from AP.
...
PARAMETERS
  session_ptr    - Pointer to session parameter belongs to
  enable_dynamic_certs   - enable or disable.
...
secerrno_enum_type secssli_set_ses_enable_dynamic_certs
(
  secssl_session_type*                        session_ptr,
  boolean     								  enable_dynamic_certs
);
...
FUNCTION SECSSLI_SET_CA_CERT_NAME

DESCRIPTION
  Duirng SSL handshake, the valid x509 certificates name
  in rootCA folder and used as CA certificate for session .
(For mav only use with QMI PDS)
...
PARAMETERS
  session_ptr                             - Pointer to session parameter belongs to
  secssl_ses_ca_cert_name_data_type     - Pointer to session specified CA certs name
...
===========================================================================*/
secerrno_enum_type secssli_set_ses_ca_cert_name
(
  secssl_session_type*                      session_ptr,
  secssl_ses_ca_cert_name_data_type*        ca_cert_folder_ptr
);

/*===========================================================================

FUNCTION SECSSLI_SET_CA_CERT_ID

DESCRIPTION
  Duirng SSL handshake, the valid x509 certificates id 
  in ram Certs array rootCA  with Cer_id will be searched and used as CA certificate for session .
(For mav only use with QMI PDS)
...
PARAMETERS
  session_ptr                             - Pointer to session parameter belongs to
  uint32     -                               - Id of session specified RAM CA cert
...
===========================================================================*/
secerrno_enum_type secssli_set_ses_ca_cert_id
(
  secssl_session_type*                      session_ptr,
  uint32        ca_cert_id
);
...
FUNCTION SECSSLI_SET_SES_PSK_DATA

DESCRIPTION
to Set the PreShareKey for the session with TLS_PSK CS 
...
PARAMETERS
  session_ptr                             - Pointer to session parameter belongs to
  secssl_ses_psk_data_type       - Pointer to session PSK data
...
===========================================================================*/
secerrno_enum_type secssli_set_ses_psk_data
(
  secssl_session_type*                        session_ptr,
  secssl_ses_psk_data_type*                   session_psk_data
);
...
FUNCTION SECSSLI_SES_TLS_EVENT

DESCRIPTION
SSL CONNECTION to use TLS_EVENTS OR DS_EVENTS
...
PARAMETERS
  session_ptr                             - Pointer to session parameter belongs to
  boolean    -                            - session_use_tlsevent boolean value
...
===========================================================================*/
secerrno_enum_type secssli_set_ses_tls_event 
(
  secssl_session_type*                        session_ptr,
  boolean                                     session_non_dss_pdata
);
...
FUNCTION SECSSLI_SET_GET_PSK_DATA

DESCRIPTION
to Get the PreShareKey hint data if received from server in case of TLS_PSK CS 
...
RETURN VALUE
  secssl_ses_psk_data_type       - Pointer to session PSK data (HINT data only)
  Security Services error number to indicate execution outcome.
  (see secerrno.h for details)

===========================================================================*/
secerrno_enum_type secssli_get_ses_psk_data
(
  secssl_session_type*                        session_ptr,
  secssl_ses_psk_data_type*                   session_psk_data
);
