...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/protocols/linklayer/src/ps_ppp_auth.c#2 $
...
05/08/17    ds     Code changes to pass response_ptr as double pointer to 
                   function chapi_calc_chap_md5_ruim_response() so that it retains
                   its updated value in caller function chapi_send_resp().
01/16/17    ds     Fixed KW issues.
01/05/17    ssh    LLVM Static Analysis fixes
04/01/16    skc    Use of dsm_item_set_priority() to set priority
04/06/15    sd     Fallback support for CHAP challenge response calculation.
11/05/14    vl     Stack size optimization changes
...
} ppp_uim_auth[PPP_MAX_SUBS_ID];

#ifdef FEATURE_RUIM_FUNC_INTERFACE

#ifdef FEATURE_MMGSDI_SESSION_LIB
/*---------------------------------------------------------------------------
TYPEDEF PPP_MMGSDI_1X_SESSION_E_TYPE

DESCRIPTION
  This is an enum type to define the session type info
---------------------------------------------------------------------------*/
typedef enum
{
  PPP_MMGSDI_1X_PROV_PRI_SESSION = 0,
  PPP_MMGSDI_1X_PROV_SEC_SESSION,
  PPP_MMGSDI_1X_PROV_MAX_SESSION
}ppp_mmgsdi_1x_session_e_type;
...
ppp_mmgsdi_auth_config_type ppp_mmgsdi_auth_config[PPP_MAX_SUBS_ID];
#endif /* FEATURE_RUIM_FUNC_INTERFACE */
...
static struct ppp_cb_auth_s auth_device_constants[PPP_MAX_SUBS_ID];
...

{
  ...
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ppp_uim_auth[PPP_SUBS_ID_1].report_status = UIM_FAIL;
  ppp_uim_auth[PPP_SUBS_ID_2].report_status = UIM_FAIL;
  if( NULL == cnf_ptr )
  {
    /* "mmgsdi_client_id_reg_resp_rpt: MMGSDI CLIENT ID REG "
       ...
       == client_reg_ptr->response_header.response_type ) )
  {
         ...
         == client_reg_ptr->response_header.mmgsdi_status ) )
    {
      ...
      ppp_mmgsdi_auth_config[PPP_SUBS_ID_1].reg_info.client_id = 
        client_reg_ptr->response_header.client_id;
      ppp_uim_auth[PPP_SUBS_ID_1].report_status = UIM_PASS;
      LOG_MSG_INFO1_2("[SUBS ID = %d] ppp_mmgsdi_client_id_reg_resp_rpt: "
                      "Client id - %d", 
                      PPP_SUBS_ID_1,
                      client_reg_ptr->response_header.client_id);


      ppp_mmgsdi_auth_config[PPP_SUBS_ID_2].reg_info.client_id = 
        client_reg_ptr->response_header.client_id;
      ppp_uim_auth[PPP_SUBS_ID_2].report_status = UIM_PASS;
      LOG_MSG_INFO1_2("[SUBS ID = %d] ppp_mmgsdi_client_id_reg_resp_rpt: "
                      "Client id - %d", 
                      PPP_SUBS_ID_2,
                      client_reg_ptr->response_header.client_id);
...
)
{
  ...
  for(index = 0; index < PPP_MAX_SUBS_ID; index++)
  {
    if( ppp_mmgsdi_auth_config[index].reg_info.session_info.session_id
    	    == event->session_id )
    {
      break;
    }
  ...
  if (PPP_MAX_SUBS_ID == index)
  {
    /*"ppp_mmgsdi_session_evt_cb: Invalid session ID"*/
    DATA_PROTO_ERROR_LOG_1(PS_BAD_ARG_OR_NULL_PTR,index);
    return;
  }
  ...
  switch (event->evt)
  {
    case MMGSDI_SUBSCRIPTION_READY_EVT:
      LOG_MSG_INFO2_2("[SUBS ID = %d] ppp_mmgsdi_session_evt_cb: Processing " 
              "MMGSDI_SUBSCRIPTION_READY_EVT for client_id %d", 
               index,
               event->client_id);
      PS_ENTER_CRIT_SECTION (&ppp_reg_crit_section);
      memscpy (&(ppp_mmgsdi_auth_config[index].reg_info.session_info.app_info_type), 
               sizeof (mmgsdi_app_info_type),
               &(event->data.subscription_ready.app_info),
               sizeof (mmgsdi_app_info_type));
      ppp_mmgsdi_auth_config[index].reg_info.session_info.session_active = TRUE;
      PS_LEAVE_CRIT_SECTION (&ppp_reg_crit_section);
      break;

    default:
      LOG_MSG_INFO2_3("[SUBS ID = %d] ppp_mmgsdi_session_evt_cb: Processing " 
              "event %d for client_id %d", 
              index, event->evt, event->client_id);
  }
...
)
{
         ...
         == ssn_open_ptr->response_header.response_type ) )
  {
         ...
         == ssn_open_ptr->response_header.mmgsdi_status ) )
    {
      ...
      /*-------------------------------------------------------------------------
        Map mmgsdi ID to PPP subs id
      -------------------------------------------------------------------------*/
      subs_id = (ppp_subs_id_enum_type)ssn_open_ptr->response_header.client_data;
      if (PPP_MAX_SUBS_ID <= subs_id)
      {
        /*ppp_mmgsdi_session_open_cb: invalid SUBS ID */
        DATA_PROTO_ERROR_LOG_1(PS_BAD_ARG_OR_NULL_PTR,subs_id);
        return;
      }
     
      ppp_mmgsdi_auth_config[subs_id].reg_info.session_info.session_id 
        = ssn_open_ptr->session_id;
      LOG_MSG_INFO2_2("[SUBS ID = %d] ppp_mmgsdi_session_open_cb: " 
              "Session id = %d", 
               subs_id,
               ssn_open_ptr->session_id);
...
)
{
  ...
  -----------------------------------------------------------------------*/
  status = mmgsdi_session_open (ppp_mmgsdi_auth_config[PPP_SUBS_ID_1].reg_info.client_id,
                                MMGSDI_1X_PROV_PRI_SESSION, 
                                NULL, 
                                mmgsdi_ssn_evt_cb,
                                FALSE, 
                                mmgsdi_ssn_open_cb, 
                  (mmgsdi_client_data_type)PPP_SUBS_ID_1);

  if ( status != MMGSDI_SUCCESS )
  {
    LOG_MSG_ERROR_1("ppp_client_reg_cmd_hdlr: Primary Session Open 1x "
                    "Registration Failed %d ", status);
  }

  status = mmgsdi_session_open (ppp_mmgsdi_auth_config[PPP_SUBS_ID_2].reg_info.client_id,
                                MMGSDI_1X_PROV_SEC_SESSION, 
                                NULL, 
                                mmgsdi_ssn_evt_cb,
                                FALSE, 
                                mmgsdi_ssn_open_cb, 
                  (mmgsdi_client_data_type)PPP_SUBS_ID_2);

  if ( status != MMGSDI_SUCCESS )
  {
    LOG_MSG_ERROR_1("ppp_client_reg_cmd_hdlr: Secondary Session Open 1x "
                    "Registration Failed %d ", status);
  }
...
/*===========================================================================
FUNCTION PPP_CMMGSDI_GET_CURRENT_SESSION_TYPE()

DESCRIPTION
  This function retrieves the session ID mapped to the corresponding 
  current CDMA subs ID.
...
SIDE EFFECTS
  None
===========================================================================*/
ppp_mmgsdi_1x_session_e_type ppp_mmgsdi_get_current_session_type
(
  ppp_subs_id_enum_type subs_id
)
{
  sys_modem_as_id_e_type       curr_sys     = SYS_MODEM_AS_ID_NONE;
  ppp_mmgsdi_1x_session_e_type session_type = PPP_MMGSDI_1X_PROV_MAX_SESSION;
  
  curr_sys = ppp_get_sys_subs_id( subs_id );

  switch(curr_sys)
  {
    case SYS_MODEM_AS_ID_1:
      session_type = PPP_MMGSDI_1X_PROV_PRI_SESSION;
      break;

    case SYS_MODEM_AS_ID_2:
    	 session_type = PPP_MMGSDI_1X_PROV_SEC_SESSION;
    	 break;
  ...
  return session_type;
}/* ppp_mmgsdi_get_current_session_type */
...
)
{
  ...
  for(subs_id = PPP_MIN_SUBS_ID; subs_id < PPP_MAX_SUBS_ID; subs_id++)
  {
    ppp_mmgsdi_auth_config[subs_id].rand_type = 
                       (mmgsdi_cave_rand_type_type) UIM_GLOBAL_RAND_CHAL;

    ppp_mmgsdi_auth_config[subs_id].dig_len = (mmgsdi_cave_dig_len_type) 0;

    for (index = 0; index < MMGSDI_CAVE_DIGITS_SIZE ; index++)
    {
       ppp_mmgsdi_auth_config[subs_id].digits[index] = 0;
    }
 
    ppp_mmgsdi_auth_config[subs_id].process_control = 
                 (mmgsdi_cave_process_control_type)RUIM_DEFLT_PROC_CNTRL;

    LOG_MSG_INFO2_1("PPP process control type = %d",
                    ppp_mmgsdi_auth_config[subs_id].process_control);

    nv_status = ps_get_nv_item_ext( NV_ESN_I,
                                    &ps_cave_chap_esn,
                                    subs_id );

    if ( nv_status == NV_DONE_S )
    {
      ppp_mmgsdi_auth_config[subs_id].esn[0] = ps_cave_chap_esn.esn.esn;
      ppp_mmgsdi_auth_config[subs_id].esn[1] = 0;
    }
    else
    {
      LOG_MSG_DATA_PROTO_INFO_0("ppp_auth_init: Could not read NV item to initialize"
                                " ESN");
    }
  }
#endif /* FEATURE_UIM_CAVE_AN_AUTH */
  ...
  {
    LOG_MSG_DATA_PROTO_INFO_0("Could not obtain valid client id");
  } 
#endif /* FEATURE_MMGSDI_SESSION_LIB */
...

/*===========================================================================
FUNCTION PPP_AUTH_SET_CONSTANT()
...
{
  struct ppp_cb_auth_s *const_ptr;
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  const_ptr = &auth_device_constants[subs_id];


  switch(constant)
  {
    case AUTH_TIMEOUT: 
      const_ptr->timeout = (uint16)value;
    break;

    case AUTH_RETRY: 
      const_ptr->retry_limit = (uint8)value;
    break;

    case AUTH_REQUIRE_ENCRYPTED_PASSWORD: 
      const_ptr->require_encrypted_passwd = (boolean)value;
    break;

    default:
      LOG_MSG_DATA_PROTO_INFO_1("ppp_auth_set_constant: Unknown constant %d",
                                 constant);
    break;

  };

} /* ppp_auth_set_constant() */

/*===========================================================================
FUNCTION PPP_AUTH_GET_CONSTANT()

DESCRIPTION
  This function is called to get the configurable protocol constants for
  AUTH.
...
)
{
  ...
  {
    case AUTH_TIMEOUT: 
      return const_ptr->timeout;
    break;

    case AUTH_RETRY: 
      return const_ptr->retry_limit;
    break;

    case AUTH_REQUIRE_ENCRYPTED_PASSWORD: 
      return const_ptr->require_encrypted_passwd;
    break;

    default:
      LOG_MSG_DATA_PROTO_INFO_1("ppp_auth_get_constant: Unknown constant %d",
                                 constant);
    break;
  };
 
  return -1;
} /* ppp_auth_get_constant() */
...
)
{
  ...
  /*-------------------------------------------------------------------------
    Initialize the protocol dependent constants that are configurable
  -------------------------------------------------------------------------*/
  LOG_MSG_INFO1_2("PPP AUTH CONSTANT:%d VALUE:%d",constant,value);
  switch(constant)
  {
    case AUTH_TIMEOUT:
      PS_ENTER_CRIT_SECTION( pppi_get_crit_sect_lock(device) );
      if( 0 < value )
      {
        ...
        LOG_MSG_DATA_PROTO_INFO_1("PPP AUTH TIMEOUT %d", value);
      }
      PS_LEAVE_CRIT_SECTION( pppi_get_crit_sect_lock(device) );
      break;
    ...
    case AUTH_RETRY: 
      ...
      if( 0 < value )
      {
        ...
        LOG_MSG_DATA_PROTO_INFO_1("PPP AUTH RETRY LIMIT %d", value);
      }
      PS_LEAVE_CRIT_SECTION( pppi_get_crit_sect_lock(device) );
      break;
    ...
    case AUTH_REQUIRE_ENCRYPTED_PASSWORD: 
      ...
      if( 0 < value )
      {
        ppp_cb_ptr->auth.require_encrypted_passwd =(boolean)value;
        LOG_MSG_DATA_PROTO_INFO_1("PPP AUTH REQUIRE ENCRYPTED PASSWORD %d", value);
      }
      PS_LEAVE_CRIT_SECTION( pppi_get_crit_sect_lock(device) );
      break;

    default:
      LOG_MSG_DATA_PROTO_INFO_1("ppp_auth_init_device_constant: Unknown constant %d",
                      constant);
      break;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  -------------------------------------------------------------------------*/
    LOG_MSG_DATA_PROTO_INFO_0("ppp_auth_complete: Deleting an existing eap_handle");
    eap_delete_instance(ppp_cb_ptr->auth.ps_eap_handle);
    ppp_cb_ptr->auth.ps_eap_handle = EAP_INVALID_HANDLE;

    LOG_MSG_DATA_PROTO_INFO_1("ppp_auth_complete:Reinitialising a new eap_handle for "
...
)
{
  ...
  -------------------------------------------------------------------------*/
   LOG_MSG_INFO2_2("dev %d %d timeout occured", device, ppp_cb_ptr->auth.mode);
   switch(ppp_cb_ptr->auth.mode)
   {
   case PPP_DO_PAP:
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  ppp_cb_array = fsm_get_ppp_cb_array();
  ppp_cb_ptr = ppp_cb_array + (uint8)device;
  LOG_MSG_ERROR_2( "ppp_cb_ptr %x, subs id %d, chap_proc", ppp_cb_ptr, ppp_cb_ptr->subs_id );
  ...
  switch(auth_hdr.code)
  {
    case PPP_CHAP_CHALLENGE:
    {
      ...
      ---------------------------------------------------------------------*/
      LOG_MSG_DATA_PROTO_INFO_1("dev %d recieved CHAP challenge", device);
      ...
      LOG_MSG_DATA_PROTO_INFO_2("dev %d CHAP chal_len = %d", device, challenge_len);
    ...
    case PPP_CHAP_SUCCESS:
    {
      ...
      {
        LOG_MSG_DATA_PROTO_INFO_1( "dev %d got CHAP Success", device);
        ppp_auth_complete(ppp_cb_ptr, PPP_CHAP_PROTOCOL);
      }
      else
      {
    ...
    case PPP_CHAP_FAILURE:
    {
      ...
      if(auth_hdr.identifier == ppp_cb_ptr->auth.active_id)
      {
        ...
        LOG_MSG_DATA_PROTO_INFO_1( "dev %d got CHAP Failure.", device);
        ppp_cb_ptr->auth.flags |= PPP_AP_FAILED;
        ppp_auth_complete(ppp_cb_ptr, PPP_CHAP_PROTOCOL);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  case PPP_PAP_NAK:
    ...
    if(auth_hdr.identifier == ppp_cb_ptr->auth.active_id)
    {
      ...
      LOG_MSG_DATA_PROTO_INFO_1("dev %d got PAP Nak.", device);
      ppp_cb_ptr->auth.flags |= PPP_AP_FAILED;
  ...
  case PPP_PAP_ACK:
    if(auth_hdr.identifier == ppp_cb_ptr->auth.active_id)
    {
      ...
      ---------------------------------------------------------------------*/
      LOG_MSG_DATA_PROTO_INFO_1("dev %d got PAP Ack.", device);
      ppp_auth_complete(ppp_cb_ptr, PPP_PAP_PROTOCOL);
    }
    else
    {
...
)
{
  ...
  {
    /*"chapi_calc_chap_cave_ruim_response():"
                    " response_ptr or response_ptr->data_ptr is NULL"*/
    DATA_PROTO_ERROR_LOG_1(PS_BAD_ARG_OR_NULL_PTR,response_ptr);
    return FALSE;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  dsm_item_set_priority(auth_success, DSM_QUEUE_PRIORITY_HIGH);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  dsm_item_set_priority(auth_req, DSM_QUEUE_PRIORITY_HIGH);  
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  subs_id = (ppp_subs_id_enum_type) 
                    compute_ip_cnf_ptr->response_header.client_data;
  if (PPP_MAX_SUBS_ID <= subs_id)
  {
    /*"chapi_uim_3gpd_calc_chap_resp_rpt: invalid SUBS ID "*/
    DATA_PROTO_ERROR_LOG_1(PS_BAD_ARG_OR_NULL_PTR,subs_id);
    return;
  }

  ppp_uim_auth[subs_id].report_status = UIM_FAIL;
  ppp_uim_auth[subs_id].sw_info.present = FALSE;
  ppp_uim_auth[subs_id].sw_info.sw1 = 0;
  ppp_uim_auth[subs_id].sw_info.sw2 = 0;
  /*-------------------------------------------------------------------------
    Copy the uim report status into a variable local to the file
  -------------------------------------------------------------------------*/
#ifdef FEATURE_MMGSDI_SESSION_LIB
  if (compute_ip_cnf_ptr->response_header.client_id 
        != ppp_mmgsdi_auth_config[subs_id].reg_info.client_id)
  {
    LOG_MSG_ERROR_3("[SUBS_ID = %d] chapi_uim_3gpd_calc_chap_resp_rpt: UIM 3GPD CHAP"
         ...
         compute_ip_cnf_ptr->response_header.response_type ) )
  {
    ...
    {
      ppp_uim_auth[subs_id].report_status = UIM_PASS;
    /*-----------------------------------------------------------------------
      The uim calc chap command was successful.  This copies the data from
      the uim report
    -----------------------------------------------------------------------*/
      memscpy(ppp_uim_auth[subs_id].challenge_response,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if( NULL == cnf_ptr )
  {
    /*"chapi_uim_calc_chap_resp_rpt: UIM-CHAP command failure"
                  " cnf_ptr NULL"*/
    DATA_PROTO_ERROR_LOG_0(PS_BAD_ARG_OR_NULL_PTR);
    PS_SET_SIGNAL(PS_UIM_CMD_SIGNAL);
    return;
  ...
  if (PPP_MAX_SUBS_ID <= subs_id)
  {
    /*"chapi_uim_calc_chap_resp_rpt: invalid SUBS ID "*/
    DATA_PROTO_ERROR_LOG_1(PS_BAD_ARG_OR_NULL_PTR,subs_id);
    return;
  }
  ...
  if (compute_ip_cnf_ptr->response_header.client_id 
        != ppp_mmgsdi_auth_config[subs_id].reg_info.client_id)
  {
    LOG_MSG_ERROR_3("[SUBS_ID = %d] chapi_uim_calc_chap_resp_rpt: UIM-CHAP command failure"
...
-----------------------------------------------------------------------*/
  ...
  {
    LOG_MSG_ERROR_3("[SUBS_ID = %d] chapi_uim_calc_chap_resp_rpt: Improper command confirmation " 
                    " cnf %d response_type %d",
                    subs_id, cnf, compute_ip_cnf_ptr->response_header.response_type);
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if( NULL == cnf_ptr )
  {
    /*"chapi_uim_an_cave_resp_rpt: RUN CAVE command failure"
                   " cnf_ptr NULL"*/
    DATA_PROTO_ERROR_LOG_0(PS_BAD_ARG_OR_NULL_PTR);
    PS_SET_SIGNAL(PS_UIM_CMD_SIGNAL);
    return;
  ...
  subs_id = (ppp_subs_id_enum_type)
                        run_cave_cnf_ptr->response_header.client_data;
  if (PPP_MAX_SUBS_ID <= subs_id)
  {
    /*"chapi_uim_an_cave_resp_rpt: invalid SUBS ID"*/
    DATA_PROTO_ERROR_LOG_1(PS_BAD_ARG_OR_NULL_PTR,subs_id);
    return;
  }
  ...
  if (run_cave_cnf_ptr->response_header.client_id 
        != ppp_mmgsdi_auth_config[subs_id].reg_info.client_id)
  {
    LOG_MSG_ERROR_3("[SUBS_ID = %d] chapi_uim_an_cave_resp_rpt: RUN CAVE command failure"
              " Client ids do not match! response client id %d, client id %d", 
              subs_id, 
         ...
         run_cave_cnf_ptr->response_header.response_type ) )
  {
       ...
       ( MMGSDI_SUCCESS == run_cave_cnf_ptr->response_header.mmgsdi_status ) )
    {
      ...
      ppp_uim_auth[subs_id].run_cave_response =
              run_cave_cnf_ptr->run_cave_response_data;
      /* Shift the 18-bit AUTHR to the MSB */
      ppp_uim_auth[subs_id].run_cave_response <<= 14;

      b_packd(ppp_uim_auth[subs_id].run_cave_response,
              ppp_uim_auth[subs_id].challenge_response,
              0,
    ...
    else
    {
      LOG_MSG_ERROR_3("[SUBS_ID = %d] chapi_uim_an_cave_resp_rpt: RUN CAVE command failure"
                      " status %d and mmgsdi_status %d are ",
                      subs_id, status, run_cave_cnf_ptr->response_header.mmgsdi_status);
    }
  }
  else
  {
    LOG_MSG_ERROR_3("[SUBS_ID = %d] chapi_uim_an_cave_resp_rpt: Improper command confirmation"
             " cnf %d response_type %d", 
             subs_id, cnf, run_cave_cnf_ptr->response_header.response_type);
  }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if( !ruim_response_isvalid( ppp_cb_ptr, response_ptr ) )
  {
    return NULL;
  }
...
)
{
  ...
  dsm_item_type  *local_response_ptr = NULL;
  local_response_ptr = *response_ptr;
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if( !ruim_response_isvalid( ppp_cb_ptr, local_response_ptr ) )
  {
    return NULL;
  }
  ...
  ------------------------------------------------------------------------*/
  session_type = ppp_mmgsdi_get_current_session_type(ppp_cb_ptr->subs_id);
  if (PPP_MMGSDI_1X_PROV_MAX_SESSION == session_type)
  {
    /*"chapi_calc_chap_md5_ruim_response : Invalid session."*/
    DATA_PROTO_ERROR_LOG_0(PS_BAD_ARG_OR_NULL_PTR);
    return NULL;
  }  

#ifdef FEATURE_MMGSDI_SESSION_LIB
  if (FALSE == ppp_mmgsdi_auth_config[ppp_cb_ptr->subs_id].reg_info.session_info.session_active)
  {
  ...
  {	
    ...
    ------------------------------------------------------------------------*/
    insert_ptr    = local_response_ptr->data_ptr;
    *insert_ptr++ = PPP_CHAP_RESPONSE;
    *insert_ptr++ = msg_id;
    insert_ptr    = put16(insert_ptr,(uint16)*response_len);
    ...
    *insert_ptr++ = PPP_CHAP_MD5_SIZE;
    ...
    retval = mmgsdi_session_compute_ip_auth ( 
                     ppp_mmgsdi_auth_config[ppp_cb_ptr->subs_id].reg_info.session_info.session_id, 
                     cmpt_ip_data,
                     response_cb_ptr,
                     (mmgsdi_client_data_type) ppp_cb_ptr->subs_id
             );
    ...
    LOG_MSG_DATA_PROTO_INFO_0("Blocking PS Task on UIM cmd signal");

    memset(sig_mask, 0, sizeof(sig_mask));
    PS_SET_SIG(sig_mask, PS_UIM_CMD_SIGNAL);
    (void)PS_WAIT(sig_mask); /* Wait for completion*/

    LOG_MSG_DATA_PROTO_INFO_0("Unblocking PS TASK on UIM cmd signal");
    if(ppp_uim_auth[ppp_cb_ptr->subs_id].report_status == UIM_PASS)
    {
    /*-------------------------------------------------------------------------
      The code tries to access dsm item without dsm API:
      1. data_ptr: point to the beginning the the dsm buffer
      ...
      3. insert_ptr: point to the current avaliable buffer 
         ...
         the beginning of  buffer + used size - the current pointer 
    -------------------------------------------------------------------------*/
      LOG_MSG_DATA_PROTO_INFO_0("UIM CHAP challenge response calculation success");
      memscpy(insert_ptr,
              local_response_ptr->data_ptr + local_response_ptr->used - insert_ptr,
              &ppp_uim_auth[ppp_cb_ptr->subs_id].challenge_response,
              UIM_COMPUTE_IP_AUTH_RESPONSE_LENGTH);
      break;
    }
    else
    {
      ...
      if( ppp_cb_ptr->auth.info.nai_entry_index > 0 &&
          (ppp_uim_auth[ppp_cb_ptr->subs_id].sw_info.present == TRUE &&
           ppp_uim_auth[ppp_cb_ptr->subs_id].sw_info.sw1 == PPP_AUTH_CHAP_FAILURE_SW1 &&
           ppp_uim_auth[ppp_cb_ptr->subs_id].sw_info.sw2 == PPP_AUTH_CHAP_FAILURE_SW2) )
      { 
        // Fallback to index 0.       
        ...
        else
        {
          ...
          nv_status = ps_get_nv_item_ext( NV_DS_SIP_NAI_INFO_I, 
                                          &nv_item,
                                          ppp_cb_ptr->subs_id);  
#else
         ...
         -------------------------------------------------------------*/
          ...
          dsm_free_packet(&local_response_ptr);
          ...
          *response_len =  PPP_AUTH_CODE_SIZE
          ...
          local_response_ptr = dsm_offset_new_buffer(DSM_DS_POOL_SIZE(
                                                *response_len + PPP_HDR_LEN),
                                               PPP_HDR_LEN);
          if(local_response_ptr == NULL || local_response_ptr->data_ptr == NULL)
          {
            /*"dev <ppp_cb_ptr->fsm[LCP].device> CHAP resp mem alloc failed"*/
            DATA_PROTO_ERROR_LOG_2(PS_BAD_ARG_OR_NULL_PTR,
                               local_response_ptr,ppp_cb_ptr->fsm[LCP].device);
            *response_ptr = NULL;
            return NULL;
          }

          local_response_ptr->used     = (uint16)*response_len;
          dsm_item_set_priority(local_response_ptr, DSM_QUEUE_PRIORITY_HIGH);
          DSM_TOUCH_ITEM(local_response_ptr);		  
          continue;
        }
      }
      else
  ...
  /*update response_ptr with new updated location of new DSM buffer. */
  *response_ptr = local_response_ptr;
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_ERROR_2( "ppp_cb_ptr %x, subs id %d, chapi_calc_an_hrpd_md5_ruim_response", ppp_cb_ptr, ppp_cb_ptr->subs_id );
  ...
  if (PPP_MMGSDI_1X_PROV_MAX_SESSION == session_type)
  {
    /*"chapi_calc_an_hrpd_md5_ruim_response : Invalid session."*/
    DATA_PROTO_ERROR_LOG_0(PS_BAD_ARG_OR_NULL_PTR);
    return NULL;
  }
  ...
  if (FALSE == ppp_mmgsdi_auth_config[ppp_cb_ptr->subs_id].reg_info.session_info.session_active)
  {
    LOG_MSG_ERROR_3( "ppp_cb_ptr %x, subs id %d"
                     " [SUBS_ID = %d] chapi_calc_an_hrpd_md5_ruim_response:"
                     " PPP Client not registered",
                       ppp_cb_ptr,
                       ppp_cb_ptr->subs_id,
                       ppp_cb_ptr->subs_id );
  ...
  *insert_ptr++ = PPP_CHAP_MD5_SIZE;
  ...
  if(MMGSDI_INCORRECT_PARAMS == retval) 
  {
    /*("chapi_calc_an_hrpd_md5_ruim_response: Incorrect Paramter values
        supplied to mmgsdi_compute_ip_auth algo")*/
    PS_PPP_AUTH_ASSERT(0);
    return NULL;
  }
  else if (MMGSDI_CMD_QUEUE_FULL == retval)
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (PPP_MMGSDI_1X_PROV_MAX_SESSION == session_type)
  {
    /*"chapi_calc_chap_cave_ruim_response : Invalid session."*/
    DATA_PROTO_ERROR_LOG_0(PS_BAD_ARG_OR_NULL_PTR);
    return NULL;
  }
  ...
  *insert_ptr++ = PPP_CHAP_MD5_SIZE;
  ...
  retval = mmgsdi_session_run_cave_algo ( 
                     ppp_mmgsdi_auth_config[ppp_cb_ptr->subs_id].reg_info.session_info.session_id,
                     ppp_mmgsdi_auth_config[ppp_cb_ptr->subs_id].rand_type,
                     rand_chal,
                     ppp_mmgsdi_auth_config[ppp_cb_ptr->subs_id].dig_len,
                     ppp_mmgsdi_auth_config[ppp_cb_ptr->subs_id].digits,
                     ppp_mmgsdi_auth_config[ppp_cb_ptr->subs_id].process_control,
                     ppp_mmgsdi_auth_config[ppp_cb_ptr->subs_id].esn,
                     response_cb_ptr,
                     (mmgsdi_client_data_type) ppp_cb_ptr->subs_id);

  if(MMGSDI_INCORRECT_PARAMS == retval) 
  {
    /*("chapi_calc_chap_cave_ruim_response: Incorrect Paramter 
        values supplied to mmgsdi_run_cave_algo")*/
    PS_PPP_AUTH_ASSERT(0);
    return NULL;
  }
  else if (MMGSDI_CMD_QUEUE_FULL == retval)
  ...
  if(ppp_uim_auth[ppp_cb_ptr->subs_id].report_status == UIM_PASS)
  {
    ...
    *insert_ptr++ = ppp_uim_auth[ppp_cb_ptr->subs_id].challenge_response[0];
    *insert_ptr++ = ppp_uim_auth[ppp_cb_ptr->subs_id].challenge_response[1];
    *insert_ptr++ = (ppp_uim_auth[ppp_cb_ptr->subs_id].challenge_response[2] & 0xC0);
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  LOG_MSG_ERROR_2( "ppp_cb_ptr %x, subs id %d, chapi_send_resp", ppp_cb_ptr, ppp_cb_ptr->subs_id );
  ...
  dsm_item_set_priority(response_ptr, DSM_QUEUE_PRIORITY_HIGH);
  ...
  )
  {
    ...
    if (NULL == insert_ptr)
    {
      ...
      {      
        ...
        {
          /*"chapi_send_resp: Allocation of ps_cave_nai failed"*/
...
#ifdef FEATURE_DATA_IS707
    nv_status =  ps_get_nv_item_ext(NV_HDR_AN_AUTH_NAI_I, ps_cave_nai_ptr,
                                             ppp_cb_ptr->subs_id);
#else
    nv_status =  ps_get_nv_item(NV_HDR_AN_AUTH_NAI_I, ps_cave_nai_ptr);
#endif
        if ( NV_DONE_S == nv_status )
        {
          ...
          {
            /*"chapi_send_resp: Invalid NV item (CAVE user ID)"*/
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  dsm_item_set_priority(chal_ptr, DSM_QUEUE_PRIORITY_HIGH);
...
)
{
  ...
  eap_client_config_subs_e_type eap_subs_id = EAP_SUBS_MAX;
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if( PPP_MAX_DEV <= device)
  {
    /*( "Invalid argument passed: device")*/
    PS_PPP_AUTH_ASSERT(0);
    return;
  }
  ppp_cb_array = fsm_get_ppp_cb_array();
  ...
  if(ppp_cb_ptr->auth.ps_eap_handle != EAP_INVALID_HANDLE)
  {
    /*"EAP Handle already present"*/
    DATA_PROTO_ERROR_LOG_1(PS_BAD_ARG_OR_NULL_PTR,ppp_cb_ptr->auth.ps_eap_handle);
    return;
  }
  ...
  {
    /*("Invalid aka_algo_seqnum_arr_num_elements")*/
    PS_PPP_AUTH_ASSERT(0);
    return;
  }
  ...
  /*-------------------------------------------------------------------------
    Update PPP to allow authentication on secondary 1x and primary 1x session
    Get current ds707 subs and set the corresponding session type in EAP
  -------------------------------------------------------------------------*/

  switch ( ppp_cb_ptr->subs_id  )
  {
    case PPP_SUBS_ID_1:
      eap_subs_id = EAP_SUBS_PRIMARY;
      break;
    case PPP_SUBS_ID_2:
      eap_subs_id = EAP_SUBS_SECONDARY;
      break;
    default:
      LOG_MSG_ERROR_1("Incorrect ds707 subs_id, defaulting to primary to give it a chance %d",
                      ppp_cb_ptr->subs_id);
      eap_subs_id = EAP_SUBS_PRIMARY;
      break;
  }

  ppp_eap_meta_info.eap_session_is_set = TRUE;
  ppp_eap_meta_info.eap_session_type = eap_client_config_get_session_type_mh( 
    eap_subs_id, ppp_eap_meta_info.sim_aka_algo_type );
 
  /*-------------------------------------------------------------------------
    Set EAP meta info to eap handle
  -------------------------------------------------------------------------*/
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    /*Invalid cmd */
    DATA_PROTO_ERROR_LOG_1(PS_BAD_ARG_OR_NULL_PTR,cmd);
    return;
  }
...
/*===========================================================================
FUNCTION ps_ppp_auth_qsh_mdump_collect
  
DESCRIPTION
  This function collects the mdump for ppp_mmgsdi_auth_config variable
...
===========================================================================*/
QSH_MDUMP_FN_ATTR void ps_ppp_auth_qsh_mdump_collect()
{
  qsh_mdump_collect_high(&ppp_mmgsdi_auth_config, PPP_MAX_SUBS_ID * sizeof(ppp_mmgsdi_auth_config_type));
