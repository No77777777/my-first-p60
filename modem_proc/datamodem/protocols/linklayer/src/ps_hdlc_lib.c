...
Copyright (c)2006-2017 by Qualcomm Technologies Incorporated.
All Rights Reserved.
...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/protocols/linklayer/src/ps_hdlc_lib.c#2 $
...
#define PS_HDLC_LIB_ASSERT(expression)  \
          ps_hdlc_lib_assert_wrapper(__LINE__, expression)
...
/*===========================================================================
FUNCTION ps_hdlc_lib_assert_wrapper

DESCRIPTION
  Wrapper function for PS_HDLC_LIB ASSERT
...
=============================================================================*/
static void ps_hdlc_lib_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("PS_HDLC_LIB_ERR_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
  
}/* ps_hdlc_lib_assert_wrapper */
...
#define PRINT_HDLC_MODE( instance )                                         \
  ...
  {                                                                         \
    LOG_MSG_DATA_PROTO_INFO("Instance 0x%x using hardware mode", instance, 0, 0);           \
  }                                                                         \
  else if( USING_SOFTWARE( instance ) )                                     \
  {                                                                         \
    LOG_MSG_DATA_PROTO_INFO("Instance 0x%x using software mode", instance, 0, 0);           \
  }                                                                         \
  else                                                                      \
  {                                                                         \
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  while(*framed != NULL)
  {
    ...
    -----------------------------------------------------------------------*/
    LOG_MSG_DATA_PROTO_INFO_2("hdlci_sw_unframe() processing item %p containing %d bytes",
                    *framed, (*framed)->used);
    ...
    {
    case FINDING_7E:
      LOG_MSG_DATA_PROTO_INFO_1("hdlci_sw_unframe() in FINDING_7E(%d) state",
                      instance->state);
      /*=====================================================================
      ...
      if(read_ptr == end_ptr)
      {
        LOG_MSG_DATA_PROTO_INFO_1("hdlci_sw_unframe(): at end of item (in FINDING_7E state)",
                        instance->state);
      ...
      /*---------------------------------------------------------------------
        ...
        buffer, and continue.
      ---------------------------------------------------------------------*/
      LOG_MSG_DATA_PROTO_INFO_0("hdlci_sw_unframe() changing to FINDING_SOP state");
      instance->state = FINDING_SOP;
      read_ptr++;
      if(read_ptr == end_ptr)
      {
        LOG_MSG_DATA_PROTO_INFO_1("hdlci_sw_unframe(): at end of item %p "
    ...
    case FINDING_SOP:
      LOG_MSG_DATA_PROTO_INFO_1("hdlci_sw_unframe() in FINDING_SOP(%d) state",
                      instance->state);
      /*=====================================================================
      ...
      if(read_ptr == end_ptr)
      {
        LOG_MSG_DATA_PROTO_INFO_1("hdlci_sw_unframe(): at end of item %p (in FINDING_SOP state)",
                        *framed);
        (void)dsm_free_buffer(*framed);
        *framed = rest_of_chain;
        continue;
      ...
      ---------------------------------------------------------------------*/
      LOG_MSG_DATA_PROTO_INFO_0("hdlci_sw_unframe() changed to UNFRAMING state");
      instance->state = UNFRAMING;
      instance->frame_crc = HDLC_FCS_START;
      /*fall through */

    case UNFRAMING:
      LOG_MSG_DATA_PROTO_INFO_1("hdlci_sw_unframe() in UNFRAMING(%d) state",
                      instance->state);
      ...
      while(read_ptr < end_ptr)
      {
        ...
        {
          LOG_MSG_DATA_PROTO_INFO_2("hdlci_sw_unframe() found packet boundary in %p @ position %d",
          ...
          {
            LOG_MSG_DATA_PROTO_INFO_1("hdlci_sw_unframe() data after packet boundary in %p",
                            *framed);
                              ...
                              (uint16)(end_ptr - read_ptr) /* count */) == 0)
            {
              ...
              hdlci_finalize_unframed_pkt(instance,
                                          *framed,
                                          write_ptr -((*framed)->data_ptr));
              LOG_MSG_DATA_PROTO_INFO_0("hdlci_sw_unframe() changing to FINDING_7E state");
          ...
          *unframed = instance->unframed_pkt_ptr;
          ...
          {
            LOG_MSG_DATA_PROTO_INFO_2("HDLC SW Deframer instance (%p), unframed packet (%p)",
                            instance, *unframed);
            /*-----------------------------------------------------------------
              Clear last byte was esc in error cases,
              to allow next packet to be processed properly.
        ...
        {
          LOG_MSG_DATA_PROTO_INFO_2("hdlci_sw_unframe() found escape character in %p @ %d",
                          *framed, (read_ptr - (*framed)->data_ptr));
          instance->last_byte_was_esc = HDLC_ESC_COMPL;
          read_ptr++;
          /*-----------------------------------------------------------------
      ...
      /*---------------------------------------------------------------------
        ...
        - make sure that there is no left over data - whenever there is all
          ...
          handling above.
      ---------------------------------------------------------------------*/
      LOG_MSG_DATA_PROTO_INFO_1("hdlci_sw_unframe() unframed item %p", *framed);
      ...
      if(*framed != NULL)
      {
        ...
        {
          PS_HDLC_LIB_ASSERT(instance->unframed_pkt_tail_ptr != NULL);
          instance->unframed_pkt_tail_ptr->pkt_ptr = *framed;
          instance->unframed_pkt_tail_ptr = *framed;
        }
      }
      PS_HDLC_LIB_ASSERT(left_over_data == NULL);
      *framed = rest_of_chain;
      break;
...
)
{
  ...
  *framed_byte_ptr++ = HDLC_FLAG;
  ...
  while(*unframed != NULL)
  {
    for( byte_index = 0; byte_index < (*unframed)->used; byte_index++ )
    {
      ...
      {
        LOG_MSG_DATA_PROTO_INFO_0("Appending to packet");
        ...
        -------------------------------------------------------------------*/
        PS_HDLC_LIB_ASSERT(outgoing_tail_ptr->size > 7);
  ...
  *framed = outgoing_head_ptr;
  ...
  LOG_MSG_DATA_PROTO_INFO_2("HDLC SW Framer instance (%p), framed packet (%p)",
           instance, *framed);
...
)
{
  ...
  LOG_MSG_DATA_PROTO_INFO_1("HDLC lib allocated HDLC instance %p", instance);
...
)
{
  ...
  *ps_errno = HDLC_OK;
  LOG_MSG_DATA_PROTO_INFO_2("hdlc_ioctl(0x%x) called for instance %p",
                  ioctl_type, instance);
  ...
  {
    LOG_MSG_DATA_PROTO_INFO_1("Instance 0x%x in invalid mode", instance);
  ...
  switch( ioctl_type )
  {
    ...
    case HDLC_IOCTL_SET_ACCM:
      ...
      LOG_MSG_DATA_PROTO_INFO_2("HDlC instance 0x%x, accm set to 0x%x",
    ...
    case HDLC_IOCTL_SET_HDLC_MODE:
      LOG_MSG_DATA_PROTO_INFO_1("HW acceleration not supported, current mode unchanged for "
    ...
    case HDLC_IOCTL_GET_UNFRAMER_STATE:
      ...
      LOG_MSG_DATA_PROTO_INFO_2("Instance 0x%x, unframer state is 0x%x",
        ...
        case HDLC_SW:
          switch( ioctl_data->unframe_state )
          {
            ...
            case FINDING_SOP:
              ...
              LOG_MSG_DATA_PROTO_INFO_2("Instance 0x%x, software unframer state set to 0x%x",
