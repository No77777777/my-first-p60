...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/protocols/linklayer/src/ps_ppp_ncp.c#3 $
  $Author: pwbldsvc $ 
  $DateTime: 2020/01/10 09:58:47 $
...
--------    ---    ---------------------------------------------------------- 
01/10/20    sg     Code changes to handle wraparound scenario of 
                   session_handle once it reaches 0xffff so that session
                   handle doesn't becomes 0.
...
===========================================================================*/
#define PS_PPP_NCP_ASSERT(expression)  \
               ps_ppp_ncp_assert_wrapper(__LINE__, expression)
...
===========================================================================*/ 
 ...
 FUNCTION ps_ppp_ncp_assert_wrapper
 
 DESCRIPTION
   Wrapper function for PS_PPP_NCP_ASSERT
 ...
 ===========================================================================*/
 static void ps_ppp_ncp_assert_wrapper
 (
 ...
 )
 {
   if ( !expression )
   {
     ERR_FATAL("PS_PPP_NCP_ERR_FATAL at line:%d ",  
                                    line_num,0,0);  
   }
 }/* ps_ppp_ncp_assert_wrapper */ 
/*===========================================================================
...
)
{
  ...
  {
    /*("PPP NCP: Instance mem init error")*/
    PS_PPP_NCP_ASSERT(0);
    return;
  }
...
)
{
  ...
  if( ppp_cb_ptr->phase == pppTERMINATE )
  {
    ...
    /*-------------------------------------------------------------------------
      Inherit SUBS ID from PPP CB.
    -------------------------------------------------------------------------*/
    ncp_cb_ptr->subs_id = ppp_cb_ptr->subs_id;

	
    if (PPP_MAX_SUBS_ID <= ncp_cb_ptr->subs_id)
    {
      DATA_PROTO_ERROR_LOG_1(PS_BAD_ARG_OR_NULL_PTR,ncp_cb_ptr->subs_id);
      return;
    }

    if (0 != vsncp_3gpp2_init( ncp_cb_ptr ))
    {
      LOG_MSG_DATA_PROTO_INFO_0("vsncp_3gpp2_init() returned error");
      return;
    }
  ...
  if( ppp_cb_ptr->phase == pppDEAD )
  {
    ...
    {
      /*"ncpi_start: Failed to allocate dev_config"*/  
    ...
    if( pppi_apply_cached_opts( ncp_cb_ptr->device, dev_config_ptr ) != 0 )
    {
      /*-------------------------------------------------------------------------
        ...
        The purpose of vsncp_3gpp2_init here is only to be able to send DOWN event
        to mode handler.Set the subs_id to PPP_SUBS_ID_1 incase of invalid sub_id 
        and continue on posting the down event to MH. 
      -------------------------------------------------------------------------*/
      if (PPP_MAX_SUBS_ID <= ncp_cb_ptr->subs_id)
      {
        DATA_PROTO_ERROR_LOG_1(PS_BAD_ARG_OR_NULL_PTR,ncp_cb_ptr->subs_id);
        ncp_cb_ptr->subs_id = PPP_SUBS_ID_1;
      }
      if (0 != vsncp_3gpp2_init( ncp_cb_ptr ))
      {
        PS_SYSTEM_HEAP_MEM_FREE(dev_config_ptr);
        /*("vsncp_3gpp2_init() returned error")*/
        PS_PPP_NCP_ASSERT(0);
        return;
      }

      LOG_MSG_DATA_PROTO_INFO_0("Device not configured! Posting VSNCP down!");
      ...
      -------------------------------------------------------------------------*/
      LOG_MSG_DATA_PROTO_INFO_2( "Aborting NCP 0x%x, handle %p",
    ...
    /*-------------------------------------------------------------------------
      Inherit SUBS ID from device config cache
    -------------------------------------------------------------------------*/
    ppp_cb_ptr->subs_id = dev_config_ptr->subs_id;
    ncp_cb_ptr->subs_id = ppp_cb_ptr->subs_id;
    if (PPP_MAX_SUBS_ID <= ncp_cb_ptr->subs_id)
    {  
      DATA_PROTO_ERROR_LOG_1(PS_BAD_ARG_OR_NULL_PTR,ncp_cb_ptr->subs_id);
...
)
{
  ...
  {
    /*"Invalid NCP control block in ncpi_stop"*/
    DATA_PROTO_ERROR_LOG_0(PS_BAD_ARG_OR_NULL_PTR);
    return;
  }

  LOG_MSG_DATA_PROTO_INFO_1("ncpi_stop - handle %p", handle);
...
)
{
  ...
  if( NCPI_CB_IS_INITED( ncp_cb_ptr ) ==  FALSE )
  {
    /*"Invalid NCP control block in ncpi_resync"*/
    DATA_PROTO_ERROR_LOG_0(PS_BAD_ARG_OR_NULL_PTR);
    return;
  }

  LOG_MSG_DATA_PROTO_INFO_1("ncpi_resync - handle %p", handle);
  /*-------------------------------------------------------------------------
  ...
  switch( ncp_cb_ptr->ncp )
  {
    ...
    default:
      /* unexpected! */
      LOG_MSG_DATA_PROTO_INFO_1( "Unsupported NCP 0x%x", ncp_cb_ptr->ncp);
      return;
  } /* switch( ncp ) */
...
)
{
  ...
  if( NCPI_CB_IS_VALID( ncp_cb_ptr ) ==  FALSE )
  {
    /*"Invalid NCP control block in ncpi_abort"*/
    DATA_PROTO_ERROR_LOG_0(PS_BAD_ARG_OR_NULL_PTR);
    return;
  }
  ...
  LOG_MSG_DATA_PROTO_INFO_1("ncpi_abort - handle %p", handle);
...
)
{
  ...
  LOG_MSG_DATA_PROTO_INFO_1("PPP device %d entering network phase", device);
  ppp_cb_array = fsm_get_ppp_cb_array();
        ...
        ( ncp_cb_ptr->device == device ) )
    {
      switch( ncp_cb_ptr->ncp )
      {
        case PPP_NCP_VSNCP_3GPP2:
          ...
          -----------------------------------------------------------------*/
          LOG_MSG_DATA_PROTO_INFO_2( "Starting 3GPP2-VSNCP - dev %d, handle %p",
                           device, ncp_cb_ptr );
          vsncp_3gpp2_start( ncp_cb_ptr );
          break;
        ...
        case PPP_NCP_IPV6CP:
        default:
          LOG_MSG_DATA_PROTO_INFO_1("Unsupported protocol 0x%x", ncp_cb_ptr->ncp);
          break;
      } /* ncp */
    } 
  } /* for all NCP CBs */
  ...
  {
    LOG_MSG_DATA_PROTO_INFO_1( "Starting IPCP on dev %d", device );
    ipcp_start( ppp_cb_ptr );
  }
#ifdef FEATURE_DATA_PS_IPV6
  if( ppp_cb_ptr->fsm[IPV6CP].mode == PPP_CP_MODE_ACTIVE )
  {
    LOG_MSG_DATA_PROTO_INFO_1( "Starting IPv6CP on dev %d", device );
...
)
{
  ...
  if( device >= PPP_MAX_DEV   || 
      ...
      ncp_config_ptr == NULL )
  {
    /*("Invalid params to ppp_ncp_alloc")*/
    PS_PPP_NCP_ASSERT(0);
    return NULL;
  }
...
)
{
  ...
  LOG_MSG_DATA_PROTO_INFO_1("ppp_ncp_start - handle %p", handle);
  /*-------------------------------------------------------------------------
  ...
  for(index = 0; index < PS_PPP_NCP_CB_NUM; index++ )
  {
    ...
    if( ( NCPI_CB_IS_VALID( ncp_cb_ptr ) == TRUE )  && 
        ( ncp_cb_ptr == (ncpi_cb_type *)handle ) ) 
    {
      LOG_MSG_DATA_PROTO_INFO_1("Matching NCP handle found: %p", ncp_cb_ptr);
      /*---------------------------------------------------------------------
...
)
{
  ...
  LOG_MSG_DATA_PROTO_INFO_1("ppp_ncp_stop - handle %p", handle);
  /*-------------------------------------------------------------------------
    Check if a matching control block exists
  -------------------------------------------------------------------------*/
  for(index = 0; index < PS_PPP_NCP_CB_NUM; index++ )
...
)
{
  ...
  LOG_MSG_DATA_PROTO_INFO_1("ppp_ncp_resync - handle %p", handle);
  /*-------------------------------------------------------------------------
    Check if a matching control block exists
  -------------------------------------------------------------------------*/
  for(index = 0; index < PS_PPP_NCP_CB_NUM; index++ )
...
)
{
  ...
  {
    /* Invalid handle passed to ppp_ncp_abort %p", handle */
    DATA_PROTO_ERROR_LOG_1(PS_BAD_ARG_OR_NULL_PTR,handle);
    return -1;
  }

  LOG_MSG_DATA_PROTO_INFO_1("ppp_ncp_abort - handle %p", handle);
  /*-------------------------------------------------------------------------
        ...
        ( ncp_cb_ptr == (ncpi_cb_type *)handle ) ) 
    {
          ...
          vsncp_cb_ptr->fsm.state == fsmOPENED)
      {
        switch(param_name)
        {
          ...
          default:
            LOG_MSG_DATA_PROTO_INFO_1("Unsupported param %d", param_name);
            return -1;
            break;       
        }
      }
...
)
{
  ...
  ppp_cb_array = fsm_get_ppp_cb_array();
  ppp_cb_ptr = &ppp_cb_array[device];
  LOG_MSG_DATA_PROTO_INFO_1( "ncpi_all_resync device %d ", device );
...
)
{
  ...
  LOG_MSG_DATA_PROTO_INFO_1( "ncpi_all_bring_down device : %d", device );
