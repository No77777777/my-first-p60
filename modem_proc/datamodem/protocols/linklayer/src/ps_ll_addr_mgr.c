...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/protocols/linklayer/src/ps_ll_addr_mgr.c#2 $
  $Author: pwbldsvc $
...
#define PS_LL_ADDR_MGR_ASSERT(expression)  \
          ps_ll_addr_mgr_assert_wrapper(__LINE__, expression)
...
#define LL_ADDR_MGR_FSM_STATE_TRANSITION(handle,state_next)                  \
        do                                                                   \
        {                                                                    \
          LOG_MSG_DATA_PROTO_INFO_2("Link local address manager FSM  State %d ==>  State %d",  \
...
FUNCTION ps_ll_addr_mgr_assert_wrapperr

DESCRIPTION
  Wrapper function for PS_LL_ADDR_MGR ASSERT
...
=============================================================================*/
static void ps_ll_addr_mgr_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("PS_LL_ADDR_MGR_ERR_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
  
}/* ps_ll_addr_mgr_assert_wrapper */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if( NULL == iface_ptr)
  {
    /* ll_addr_mgr_start: iface_ptr is not valid */
    PS_LL_ADDR_MGR_ASSERT(0);
    return NULL;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(instance >= LL_ADDR_MGR_CB_INSTANCE_MAX)
  {
    /* ll_addr_mgr_stop called with invalid handle */
    PS_LL_ADDR_MGR_ASSERT(0);
    return -1;
  }
  ...
  {
    LOG_MSG_DATA_PROTO_INFO_0("ll_addr_mgr stop is pending\n");
    LL_ADDR_MGR_BRANCH_LEAVE_CRIT_SECTION();
    return -1;
  }
  ...
  if (TRUE != ll_addr_mgr_sm_cb_ptr->start_lock)
  {
    LOG_MSG_DATA_PROTO_INFO_0("ll_addr_mgr is not started\n");
    LL_ADDR_MGR_BRANCH_LEAVE_CRIT_SECTION();
    return -1;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(instance >= LL_ADDR_MGR_CB_INSTANCE_MAX)
  {
    /* ll_addr_mgr_claim called with invalid handle */
    PS_LL_ADDR_MGR_ASSERT(0);
    return -1;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_DATA_PROTO_INFO_0("Link local address manager started");
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(instance >= LL_ADDR_MGR_CB_INSTANCE_MAX)
  {
    /* lli_addr_mgr_stop called with invalid handle */
    PS_LL_ADDR_MGR_ASSERT(0);
    return;
  }
  ...
  LL_ADDR_MGR_LEAVE_CRIT_SECTION();
  LOG_MSG_DATA_PROTO_INFO_1("Link local address manager stopped in state %d",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_DATA_PROTO_INFO_2("Posting event %d to Link local address manager in state %d.",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    /* NULL ll_addr_mgr_sm_cb_ptr passed for processing PS_LL_ADDR_MGR_TIMER_EV event */
    PS_LL_ADDR_MGR_ASSERT(0);
    return;
  }

  LOG_MSG_DATA_PROTO_INFO_1("Link local address manager processing PS_LL_ADDR_MGR_TIMER_EV \
  ...
  switch (ll_addr_mgr_sm_cb_ptr->state)
  {
    ...
    case LL_ADDR_MGR_SM_CLAIMED_S:
      DATA_PROTO_ERROR_LOG_1(PS_BAD_ARG_OR_NULL_PTR, ll_addr_mgr_sm_cb_ptr->state);
      break;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  LOG_MSG_DATA_PROTO_INFO_1("Link local address manager processing PS_LL_ADDR_MGR_CONFLICT_EV \
          in state %d", ll_addr_mgr_sm_cb_ptr->state);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(instance >= LL_ADDR_MGR_CB_INSTANCE_MAX)
  {
    /* ll_addr_mgr_sm_process_claim_ev called with invalid handle */
    PS_LL_ADDR_MGR_ASSERT(0);
    return;
  }
  ...
  LOG_MSG_DATA_PROTO_INFO_1("Link local address manager processing LL_ADDR_MGR_CLAIM_EV \
          in state %d", ll_addr_mgr_sm_cb_ptr->state);
  ...
  LOG_MSG_DATA_PROTO_INFO_1( "Opening ip configuration file: %s", filename);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if( NULL == ll_addr_mgr_sm_cb_ptr)
  {
    /* NULL ll_addr_mgr_sm_cb_ptr passed for processing PS_LL_ADDR_MGR_PROBE_FAIL_EV event */
    PS_LL_ADDR_MGR_ASSERT(0);
    return;
  }

  LOG_MSG_DATA_PROTO_INFO_1("Link local address manager processing PS_LL_ADDR_MGR_PROBE_FAIL_EV \
          in state %d", ll_addr_mgr_sm_cb_ptr->state);
  ...
  switch (ll_addr_mgr_sm_cb_ptr->state)
  {
    case LL_ADDR_MGR_SM_PROBING_S:
      ...
      if( fd < 0 )
      {
        ...
        ----------------------------------------------------------------------*/
        LOG_MSG_DATA_PROTO_INFO_1( "Error opening file: %s", filename);
    ...
    case LL_ADDR_MGR_SM_CLAIMED_S:
    case LL_ADDR_MGR_SM_DEFENDING_S:
      LOG_MSG_DATA_PROTO_INFO_1("Ignore the LL_ADDR_MGR_PROBE_FAIL_EV in state %d",
              ll_addr_mgr_sm_cb_ptr->state);
      break;
    case LL_ADDR_MGR_SM_INIT_S:
      DATA_PROTO_ERROR_LOG_1(PS_BAD_ARG_OR_NULL_PTR, ll_addr_mgr_sm_cb_ptr->state);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(instance >= LL_ADDR_MGR_CB_INSTANCE_MAX)
  {
    /* ll_addr_mgr_arp_probe_cback called with invalid handle */
    PS_LL_ADDR_MGR_ASSERT(0);
    return;
  }
  ...
  if(ARP_PROBE_FAILURE == status)
  {
    ...
    if ( -1 == return_code )
    {
      DATA_PROTO_ERROR_LOG_1(PS_BAD_ARG_OR_NULL_PTR, LL_ADDR_MGR_PROBE_FAIL_EV);
      return;
    }
  }
  else if(ARP_PROBE_SUCCESS == status)
  {
    ...
    if ( -1 == return_code )
    {
      DATA_PROTO_ERROR_LOG_1(PS_BAD_ARG_OR_NULL_PTR, LL_ADDR_MGR_CONFLICT_EV);
      return;
    }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(instance >= LL_ADDR_MGR_CB_INSTANCE_MAX)
  {
    /* ll_addr_mgr_conflict_cback called with invalid handle */
    PS_LL_ADDR_MGR_ASSERT(0);
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(instance >= LL_ADDR_MGR_CB_INSTANCE_MAX)
  {
    /* ll_addr_mgr_timer_cback called with invalid handle */
    PS_LL_ADDR_MGR_ASSERT(0);
    return;
  }
