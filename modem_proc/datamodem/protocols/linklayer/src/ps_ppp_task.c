...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/protocols/linklayer/src/ps_ppp_task.c#2 $ 
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  for(subs_id = PPP_MIN_SUBS_ID; subs_id < PPP_MAX_SUBS_ID; subs_id++)
  {
    /*-----------------------------------------------------------------------
      Configure CCP
    -----------------------------------------------------------------------*/
    if( NV_DONE_S == ps_get_nv_item_ext( NV_PPP_CCP_REJECT_I,
                                         &ppp_nv_item,
                                         subs_id ) )
    {
    ...
    -----------------------------------------------------------------------*/
    if( NV_DONE_S == ps_get_nv_item_ext( NV_AUTH_REQUIRE_PASSWORD_ENCRYPTION_I,
                                         &ppp_nv_item,
                                         subs_id ) )
    {
    ...
    -------------------------------------------------------------------------*/  
    jcdma_mode = FALSE;
    if (0 != ps_read_efs_nv_ext(item_file_path, &jcdma_mode, sizeof(jcdma_mode), (ps_sys_subscription_enum_type) subs_id))
    {
    ...
    for(device = PPP_MIN_DEV; device < PPP_MAX_DEV; device++)
    {
      ...
      if( NV_DONE_S != ps_get_nv_item_ext( NV_PPP_CONFIG_DATA_I, 
                                           &ppp_nv_item,
                                           subs_id ) )
      {
        /*---------------------------------------------------------------------
      ...
      -----------------------------------------------------------------------*/
      lcp_set_device_constants(device, &ppp_nv_item.ppp_config_data, subs_id);
      ipcp_set_device_constants(device, &ppp_nv_item.ppp_config_data, subs_id);
  #ifdef FEATURE_DATA_PS_IPV6
      ipv6cp_set_device_constants(device, &ppp_nv_item.ppp_config_data, subs_id);
  #endif /* FEATURE_DATA_PS_IPV6 */
                                   ...
                                   subs_id))
    {
      ...
      -----------------------------------------------------------------------*/
      ppp_auth_set_constant(AUTH_TIMEOUT, 
                            ppp_nv_item.ppp_config_data.auth_timeout, 
                            subs_id);
      ppp_auth_set_constant(AUTH_RETRY, 
                            ppp_nv_item.ppp_config_data.auth_retry, 
                            subs_id);
      ppp_auth_set_constant(AUTH_REQUIRE_ENCRYPTED_PASSWORD, 
                            require_encrypted_passwd, 
                            subs_id);
      ...
      if(0 > ppp_set_device_constant(device, 
                                     ...
                                     subs_id))
      {
        /*"ppp_task_init: Failed to set LCP constant values"
          "  for device <device> constant <LCP_CCP_CONFIG>"*/
        DATA_PROTO_ERROR_LOG_2(PS_BAD_ARG_OR_NULL_PTR,device,LCP_CCP_CONFIG); 
        PS_PPP_TASK_ASSERT(0);
        return;
      }
    } /* for(all PPP devices) */
    ...
    -----------------------------------------------------------------------*/
    if( NV_DONE_S == ps_get_nv_item_ext( NV_PPP_VSNCP_CONFIG_DATA_I, 
                                         &ppp_nv_item,
                                         subs_id ) )
    {
      if(0 > ppp_set_device_constant
          ( PPP_UM_SN_DEV, /* ignored for VSNCP */
            ...
            subs_id ) )
      {
        /*("ppp_task_init: failed to set default");*/
        PS_PPP_TASK_ASSERT(0);
        return;
      }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_DATA_PROTO_INFO_0("PPP processing START cmd");
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_DATA_PROTO_INFO_0("PPP processing RESYNC cmd");
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_DATA_PROTO_INFO_0("PPP processing ABORT cmd");
  /*-------------------------------------------------------------------------
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_DATA_PROTO_INFO_0("PPP processing STOP cmd");
  /*-------------------------------------------------------------------------
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_DATA_PROTO_INFO_0("PPP processing SET_MODE cmd");
  /*-------------------------------------------------------------------------
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_DATA_PROTO_INFO_0("PPP processing INJECT_PKT cmd");
  /*-------------------------------------------------------------------------
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    /*("Invalid params in ppp_task_process_vendor_send");*/
    PS_PPP_TASK_ASSERT(0);
    return;
  }

  LOG_MSG_DATA_PROTO_INFO_0("PPP processing VENDOR_SEND cmd");
  /*-------------------------------------------------------------------------
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_DATA_PROTO_INFO_0("PPP processing NCP_START cmd");
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_DATA_PROTO_INFO_0("PPP processing NCP_STOP cmd");
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {  
    /*"Invalid args in NCP resync"*/
    DATA_PROTO_ERROR_LOG_2(PS_BAD_ARG_OR_NULL_PTR,cmd,data_ptr);
    return;
  }

  LOG_MSG_DATA_PROTO_INFO_0( "PPP processing NCP_RESYNC cmd");
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {  
    /*"Invalid args in NCP abort, cmd , data_ptr "*/
    DATA_PROTO_ERROR_LOG_2(PS_BAD_ARG_OR_NULL_PTR,cmd,data_ptr);
    return;
  }

  LOG_MSG_DATA_PROTO_INFO_0( "PPP processing NCP_ABORT cmd");
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_DATA_PROTO_INFO_0("PPP processing DEVICE_STOP cmd");
  /*-------------------------------------------------------------------------
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_DATA_PROTO_INFO_0("PPP processing TRIGGER_LSD cmd");
