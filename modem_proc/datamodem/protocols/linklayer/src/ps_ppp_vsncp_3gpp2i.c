...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/protocols/linklayer/src/ps_ppp_vsncp_3gpp2i.c#2 $
  $Author: pwbldsvc $
  $DateTime: 2019/10/22 02:53:17 $

when        who    what, where, why
--------    ---    ---------------------------------------------------------- 
11/01/17    ak     CR 2125724: FR 42790: F3/Debug Messages Reduction
01/05/17    ssh    LLVM Static Analysis fixes
05/12/16    ssh    Fixed KW error in memory allocation
05/06/15    cb     Fixed LLVM Static Analysis warnings
02/24/15    cx     Fixed KW warnings
...
#define PS_PPP_VSNCP_3GPP2I_ASSERT(expression)  \
                ps_ppp_vsncp_3gpp2i_assert_wrapper(__LINE__, expression)
...
/*---------------------------------------------------------------------------
  Size of buffer to compose option when send vsncp packet
---------------------------------------------------------------------------*/
#define VSNCP_3GPP2I_SEND_OPTION_BUFFER_SIZE  256

/*---------------------------------------------------------------------------
  Size of buffer to hold option when receive vsncp packet
---------------------------------------------------------------------------*/
#define VSNCP_3GPP2I_RECEIVE_OPTION_BUFFER_SIZE  8
...
---------------------------------------------------------------------------*/
#define COPY_BYTES( buffer, buffer_size, src_ptr, size, offset )            \
{                                                                           \
  ...
  {                                                                           \
    ...
    offset += size;                                                           \
      ...
      LOG_MSG_ERROR_3("Offset %d + size %d exceeds buffer boundary %d",       \
...
---------------------------------------------------------------------------*/
static vsncp_3gpp2i_nv_info_type vsncp_3gpp2_nv_info[PPP_MAX_SUBS_ID];
...
===========================================================================*/
 ...
 FUNCTION ps_ppp_vsncp_3gpp2i_assert_wrapper
 
 DESCRIPTION
   Wrapper function for PS_PPP_VSNCP_3GPP2I_ASSERT
 ...
 ===========================================================================*/
 static void ps_ppp_vsncp_3gpp2i_assert_wrapper
 (
 ...
 )
 {
   if ( !expression )
   {
     ERR_FATAL("PS_PPP_VSNCP_3GPP2I_ERR_FATAL at line:%d ",  
                                    line_num,0,0);  
   }
 }/* ps_ppp_vsncp_3gpp2i_assert_wrapper */
...
)
{
  ...
  {
    /*("Invalid handle for vsncp_3gpp2 pco mem free");*/
    PS_PPP_VSNCP_3GPP2I_ASSERT(0);
    return;
  }
  ...
  {
    temp_v4_ptr = p_cscf_v4_ptr->next_ptr;
  ...
  {
    temp_v6_ptr = p_cscf_v6_ptr->next_ptr;
  ...
  {
    temp_v6_ptr = dns_v6_ptr->next_ptr;
...
{
  LOG_MSG_DATA_PROTO_INFO_0("vsncp_3gpp2i_free()");
  /* NOOP */
  return;
} /* vsncp_3gpp2i_free() */
...
)
{
  ...
  LOG_MSG_DATA_PROTO_INFO_1("Cleaning up NCP handle %p", ncp_cb_ptr);
  if( ncp_cb_ptr != NULL )
  {
    vsncp_cb_ptr = &(ncp_cb_ptr->cb_u.vsncp_3gpp2);
...
)
{
  ...
  uint8                                 *mcc_ptr = 0;
  uint8                                 *mnc_ptr = 0;
  ps_iface_op_reserved_pco_list_type    *operator_reserved_pco_list_ptr = NULL ;
  uint16                                 pco_index = 0;

  if(NULL == iface_ptr ||
     NULL == op_pco_content_ptr)
  {
    /*"vsncp_set_iface_op_pco_info: invalid input"*/
    DATA_PROTO_ERROR_LOG_2(PS_BAD_ARG_OR_NULL_PTR,iface_ptr,op_pco_content_ptr);
    return -1;
  }
  
  op_pco_info.container_id = op_pco_content_ptr->container_id;
  op_pco_info.pco_id = op_pco_content_ptr->container_id;
  ...
  LOG_MSG_DATA_PROTO_INFO_1("VSNCP: iface set op PCO app info, length:%d",
                  op_pco_content_ptr->app_specific_info_len);
  ...
  {
    /*"VSNCP: invalid op PCO mnc length"*/
    DATA_PROTO_ERROR_LOG_1(PS_BAD_ARG_OR_NULL_PTR,op_pco_content_ptr->mnc_len);
    return -1;
  }
  LOG_MSG_DATA_PROTO_INFO_2("VSNCP: iface set op PCO mnc:%d,%d",
  ...
  {
    /*"VSNCP: invalid op PCO MCC length"*/
    DATA_PROTO_ERROR_LOG_1(PS_BAD_ARG_OR_NULL_PTR,op_pco_content_ptr->mcc_len);
    return -1;
  }
  LOG_MSG_DATA_PROTO_INFO_1("VSNCP: iface set op PCO mcc:%d", op_pco_info.mcc);

  operator_reserved_pco_list_ptr = 
         (ps_iface_op_reserved_pco_list_type *)
         modem_mem_alloc(sizeof(ps_iface_op_reserved_pco_list_type),
                         MODEM_MEM_CLIENT_DATA);

  if( operator_reserved_pco_list_ptr == NULL )
  {
    /*"Failed to allocate memory for pco list"*/
    DATA_PROTO_ERROR_LOG_0(PS_MEM_ALLOC_ERR);
    return -1;
  }
  memset(operator_reserved_pco_list_ptr,
         0,sizeof(ps_iface_op_reserved_pco_list_type));

  ps_iface_get_operator_reserved_pco_list(iface_ptr,
                                          operator_reserved_pco_list_ptr);

  /*Break the loop if pco id matches or pco id is zero */
  while(pco_index < PS_IFACE_MAX_NUM_PCO) 
  {
     if ((op_pco_info.pco_id == 
            operator_reserved_pco_list_ptr->pco_info[pco_index].pco_id ) ||
         (0 == operator_reserved_pco_list_ptr->pco_info[pco_index].pco_id ))
     {
        break;
     }
     pco_index++;
  ...
  if (pco_index != PS_IFACE_MAX_NUM_PCO) 
  {

     memscpy(&(operator_reserved_pco_list_ptr->pco_info[pco_index]),
             sizeof(ps_iface_op_reserved_pco_type),
             &op_pco_info,
             sizeof(ps_iface_op_reserved_pco_type));
  }

  if(-1 == ps_iface_set_operator_reserved_pco_list(
             iface_ptr,
             operator_reserved_pco_list_ptr))
  {
    LOG_MSG_ERROR_0( "ps_iface_set_operator_reserved_pco failed");
    modem_mem_free(operator_reserved_pco_list_ptr, MODEM_MEM_CLIENT_DATA);
    return -1;
  }
  modem_mem_free(operator_reserved_pco_list_ptr, MODEM_MEM_CLIENT_DATA);
  return 0;
...
)
{
  ...
  {
    /*"Invalid fsm_ptr in vsncp_3gpp2i_opening"*/
      ...
      ) ) )
  {
    ...
    LOG_MSG_DATA_PROTO_INFO_2("pdn-id %d addr type 0x%x unable to get address! Quitting...",
...
#endif
  {
    ...
    ---------------------------------------------------------------------*/
    LOG_MSG_DATA_PROTO_INFO_0("VSNCP: Got IPv4 addr");
    ...
    if( PS_IFACE_IS_VALID(fsm_ptr->IPV4_IFACE_PTR) )
    {
      ...
      {
        LOG_MSG_DATA_PROTO_INFO_0("VSNCP: Got gateway v4 addr");
      ...
      if( fsm_ptr->local.work & (uint16)VSNCP_3GPP2I_N_PCO )
      {
        ...
        {
          LOG_MSG_DATA_PROTO_INFO_0("VSNCP: Got primary DNS v4 addr");
          IPV4_ADDR_MSG( pco_vals_ptr->dns_v4_primary.ps_s_addr );
          LOG_MSG_DATA_PROTO_INFO_0("VSNCP: Got secondary DNS v4 addr");
        ...
        if( lwantp->pco.p_cscf_is_requested )
        {
          ...
          {
            listcount++;
            v4_addr_ptr = v4_addr_ptr->next_ptr;
          }
  
          if( listcount > 0 )
          {
            LOG_MSG_DATA_PROTO_INFO_1("Got %d P-CSCF addresses from HSGW", listcount);
            /*---------------------------------------------------------------
            ...
            {
              /*"Mem error: Unable to set v4 P-CSCF addrs"*/
              DATA_PROTO_ERROR_LOG_0(PS_MEM_ALLOC_ERR);
            }
            else
            {
              ...
              for( ( i = listcount - 1, \
                   ...
                   ( i--, v4_addr_ptr = v4_addr_ptr->next_ptr ) )
              {
                ip_addr_array[i].type = IPV4_ADDR;
                ip_addr_array[i].addr.v4.ps_s_addr = v4_addr_ptr->addr;
                LOG_MSG_DATA_PROTO_INFO_0("VSNCP: Got P-CSCF v4 addr");
        ...
        if ( pco_vals_ptr->mtu != 0 ) 
        {
          ...
          LOG_MSG_DATA_PROTO_INFO_1("VSNCP: set mtu to iface in IPv4: %d", pco_mtu);
        }                              
#endif /* FEATURE_DS_MTU_PCO_VSNCP */
    ...
    {
      LOG_MSG_DATA_PROTO_INFO_0("VSNCP: v4 iface ptr NULL, could not set v4 addr");
    }
...
#endif
  {
    ...
    ---------------------------------------------------------------------*/
    LOG_MSG_DATA_PROTO_INFO_1( "VSNCP: Got interface ID = 0x%x", lworkp->iid);
    if( PS_IFACE_IS_VALID(fsm_ptr->IPV6_IFACE_PTR) )
    {
      ...
      if( fsm_ptr->local.work & (uint16)VSNCP_3GPP2I_N_PCO )
      {
        ...
        if( dns_ptr != NULL )
        {
          ...
          -----------------------------------------------------------------*/
          while ( dns_ptr->next_ptr != NULL )
          {
            last_ptr = dns_ptr;
            dns_ptr = dns_ptr->next_ptr;
          }
        ...
        {
          LOG_MSG_DATA_PROTO_INFO_0("VSNCP: Got primary DNS v6 addr");
          IPV6_ADDR_MSG( prim_dns.addr.v6.ps_s6_addr64 );
          LOG_MSG_DATA_PROTO_INFO_0("VSNCP: Got secondary DNS v6 addr");
        ...
        if( lwantp->pco.p_cscf_is_requested )
        {
          ...
          {
            listcount++;
            v6_addr_ptr = v6_addr_ptr->next_ptr;
          }
          ...
          if( listcount > 0 )
          {
            ...
            if( ip_addr_array == NULL )
            {
              /*"Mem error: Unable to set v6 P-CSCF addrs"*/
              DATA_PROTO_ERROR_LOG_0(PS_MEM_ALLOC_ERR);
            }
            else
            {
              ...
              for( ( i = listcount - 1, \
                   ...
                   ( i--, v6_addr_ptr = v6_addr_ptr->next_ptr ) )
              {
                ip_addr_array[i].type = IPV6_ADDR;
                ip_addr_array[i].addr.v6 = v6_addr_ptr->addr;
                LOG_MSG_DATA_PROTO_INFO_0("VSNCP: Got P-CSCF v6 addr");
	    ...
	    if ( pco_vals_ptr->mtu != 0 ) 
	    {
	      PS_IFACE_SET_MTU(fsm_ptr->IPV6_IFACE_PTR, pco_vals_ptr->mtu);
			       pco_mtu = pco_vals_ptr->mtu;
	      LOG_MSG_DATA_PROTO_INFO_1("VSNCP: set mtu to iface in IPv6: %d", pco_mtu);
	    }
#endif /* FEATURE_DS_MTU_PCO_VSNCP */
    ...
    {
      LOG_MSG_DATA_PROTO_INFO_0("VSNCP: v6 iface ptr NULL");
    }
...
#endif /* FEATURE_DS_MTU_PCO_VSNCP */
          LOG_MSG_DATA_PROTO_INFO_1("VSNCP: v4 PDN type mtu: %d",
                          PS_IFACE_GET_MTU(fsm_ptr->IPV4_IFACE_PTR));
        }
        else
        {
          LOG_MSG_DATA_PROTO_INFO_0("VSNCP: v4 iface ptr NULL");
        }
      }
      break;
...
#endif /* FEATURE_DS_MTU_PCO_VSNCP */
          LOG_MSG_DATA_PROTO_INFO_1("VSNCP: v6 PDN type  mtu: %d",
...
#endif/* FEATURE_DS_MTU_PCO_VSNCP */		  
          LOG_MSG_DATA_PROTO_INFO_1("VSNCP: v4_v6 PDN type  v4 MTU: %d",
                          PS_IFACE_GET_MTU(fsm_ptr->IPV4_IFACE_PTR));
        }
        else
        {
...
#endif/* FEATURE_DS_MTU_PCO_VSNCP */
          LOG_MSG_DATA_PROTO_INFO_1("VSNCP: v4_v6 PDN type  v6 MTU: %d",
                          PS_IFACE_GET_MTU(fsm_ptr->IPV6_IFACE_PTR));
        }
        else
        {
...
{
  LOG_MSG_DATA_PROTO_INFO_0("vsncp_3gpp2i_closing()");
  /* NOOP */
  return;
...
)
{
  ...
  LOG_MSG_DATA_PROTO_INFO_0("vsncp_3gpp2i_stopping()");
  ...
  if( ( fsm_ptr == NULL ) ||
      ...
      ( fsm_ptr->pdv == NULL ) )
  {
    /*"Invalid fsm_ptr in ehrpd_vsncp_stopping"*/
    DATA_PROTO_ERROR_LOG_1(PS_BAD_ARG_OR_NULL_PTR,fsm_ptr);
    return;
  }
                                          ...
                                          )
{
  ...
  {
    DATA_PROTO_ERROR_LOG_2(PS_BAD_ARG_OR_NULL_PTR,op_pco_ptr->mcc_len,
                           op_pco_ptr->mnc_len);
    return FALSE;
  }
...
{
  uint8 buf[VSNCP_3GPP2I_SEND_OPTION_BUFFER_SIZE];
  uint16 offset = 0;
  ...
  if ( vp == NULL )
  {
    /*("Invalid vp pointer in vsncp_3gpp2i_option");*/
    PS_PPP_VSNCP_3GPP2I_ASSERT(0);
  }
  ...
  switch( opt_num )
  {
    case VSNCP_3GPP2I_PDN_ID:
      ...
      COPY_BYTES( buf, VSNCP_3GPP2I_SEND_OPTION_BUFFER_SIZE, 
                  &opthdr, OPTION_HDR_LEN, offset );
      COPY_BYTES( buf, VSNCP_3GPP2I_SEND_OPTION_BUFFER_SIZE,
                  &value_p->pdn_id, sizeof( uint8), offset );
      break;
    ...
    case VSNCP_3GPP2I_APN:
      ...
      {
        COPY_BYTES( buf, VSNCP_3GPP2I_SEND_OPTION_BUFFER_SIZE,
                    &value_p->apn[0], value_p->apn_len, offset );
      }
      break;
    ...
    case VSNCP_3GPP2I_PDN_TYPE:
      ...
      COPY_BYTES( buf, VSNCP_3GPP2I_SEND_OPTION_BUFFER_SIZE, 
                  &opthdr, OPTION_HDR_LEN, offset );
      COPY_BYTES( buf, VSNCP_3GPP2I_SEND_OPTION_BUFFER_SIZE,
                  &value_p->ip_ver_capability, sizeof( uint8), offset );
      break;
    ...
    case VSNCP_3GPP2I_PDN_ADDR:
      ...
      if( value_p->attach_type == VSNCP_3GPP2_ATTACH_TYPE_INITIAL )
      {
        ...
        COPY_BYTES( buf, VSNCP_3GPP2I_SEND_OPTION_BUFFER_SIZE, 
                    &opthdr, OPTION_HDR_LEN, offset );
        COPY_BYTES( buf, VSNCP_3GPP2I_SEND_OPTION_BUFFER_SIZE,
                    &pdn_type_empty, sizeof( uint8), offset );
        break;
      }
      ...
      {
        /*("Unexpected: no IP address found.");*/
        PS_PPP_VSNCP_3GPP2I_ASSERT(0);
      }
      ...
      switch( value_p->req_addr_type )
      {
        case VSNCP_3GPP2_PDN_TYPE_IPV4:
          ...
          COPY_BYTES( buf, VSNCP_3GPP2I_SEND_OPTION_BUFFER_SIZE, 
                      &opthdr, OPTION_HDR_LEN, offset );
          COPY_BYTES( buf, VSNCP_3GPP2I_SEND_OPTION_BUFFER_SIZE,
                      &value_p->req_addr_type, sizeof( uint8), offset );
          COPY_BYTES( buf,
                      VSNCP_3GPP2I_SEND_OPTION_BUFFER_SIZE,
        ...
        case VSNCP_3GPP2_PDN_TYPE_IPV6:
          ...
          COPY_BYTES( buf, VSNCP_3GPP2I_SEND_OPTION_BUFFER_SIZE,
                      &value_p->req_addr_type, sizeof( uint8), offset );
          COPY_BYTES( buf, VSNCP_3GPP2I_SEND_OPTION_BUFFER_SIZE,
                      &value_p->iid, sizeof( uint64 ), offset );
          break;
        ...
        case VSNCP_3GPP2_PDN_TYPE_IPV4_IPV6:
          ...
          if( value_p->iid == 0 )
          {
            ...
            COPY_BYTES( buf, VSNCP_3GPP2I_SEND_OPTION_BUFFER_SIZE,
                        &req_addr_type, sizeof( uint8), offset );
            COPY_BYTES( buf, 
                        VSNCP_3GPP2I_SEND_OPTION_BUFFER_SIZE,
                        &value_p->ip4.ps_s_addr, 
                        sizeof( struct ps_in_addr ), 
    ...
    case VSNCP_3GPP2I_PCO:
      ...
      COPY_BYTES( buf, VSNCP_3GPP2I_SEND_OPTION_BUFFER_SIZE,
                  &pco_cfg_proto, sizeof( uint8 ), offset );
      lworkp = (vsncp_3gpp2_local_opts_val_type *)(fsm_ptr->local.work_pdv);
      ...
      if( lworkp->pco.pdn_level_auth.protocol == PPP_AUTH_PAP )
      {
        /*---------------------------------------------------------------
          ...
          +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
        ----------------------------------------------------------------*/
        LOG_MSG_DATA_PROTO_INFO_0("PDN Level Authentication requested using PAP");
        ...
        COPY_BYTES( buf, VSNCP_3GPP2I_SEND_OPTION_BUFFER_SIZE,
                    &identifier, sizeof( uint16 ), offset );
        COPY_BYTES( buf, VSNCP_3GPP2I_SEND_OPTION_BUFFER_SIZE,
                    &auth_pkt_size, sizeof( uint8 ), offset );
        ...
        COPY_BYTES( buf, VSNCP_3GPP2I_SEND_OPTION_BUFFER_SIZE,
                    &auth_password_len, sizeof( uint8 ), offset );
      ...
      if( lworkp->pco.pdn_level_auth.protocol == PPP_AUTH_CHAP )
      {
        /*---------------------------------------------------------------
          ...
          +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+               
        ----------------------------------------------------------------*/
        LOG_MSG_DATA_PROTO_INFO_0("PDN Level Authentication requested using CHAP");
        ...
        COPY_BYTES( buf, VSNCP_3GPP2I_SEND_OPTION_BUFFER_SIZE,
                    &auth_chap_value_size, sizeof( uint8 ), offset );
      ...
      )
      {
        ...
        {
          LOG_MSG_DATA_PROTO_INFO_1("Adding IPv4 DNS request to PCO via IPCP. "
        ...
        addr.ps_s_addr = 0;
        COPY_BYTES( buf, VSNCP_3GPP2I_SEND_OPTION_BUFFER_SIZE,
                    &ipcp_opthdr, OPTION_HDR_LEN, offset );
        COPY_BYTES( buf, VSNCP_3GPP2I_SEND_OPTION_BUFFER_SIZE,
                    &addr.ps_s_addr, sizeof( struct ps_in_addr ), offset );
        ...
        {
          LOG_MSG_DATA_PROTO_INFO_1("Adding IPv4 DNS request to PCO via Container. "
                          "request type : %d ", lworkp->pco.dns_v4_addr_req);
          /*---------------------------------------------------------------
      ...
      {
        LOG_MSG_DATA_PROTO_INFO_1("Adding IPv6 DNS request to PCO dns mask : %d ",
      ...
      {
        LOG_MSG_DATA_PROTO_INFO_1("op pco with id: %x is needed",
        ...
        {
          LOG_MSG_DATA_PROTO_INFO_3("op pco id:%x, len:%x, mccmnc:%x%x%x",
                          identifier, length, mccmnc);
          LOG_MSG_DATA_PROTO_INFO_3("op pco mccmnc:%x%x%x",
          ...
          COPY_BYTES( buf, VSNCP_3GPP2I_SEND_OPTION_BUFFER_SIZE,
                      &length, sizeof( uint8 ), offset );
          COPY_BYTES( buf, VSNCP_3GPP2I_SEND_OPTION_BUFFER_SIZE,
                      mccmnc, VSNCP_3GPP2I_OP_PCO_MCCMNC_LEN, offset );
        }
      }
      /*---------------------------------------------------------------------
      ...
      if( lworkp->pco.mtu_is_requested )
      {
        switch( lworkp->req_addr_type )
        {
         ...
         case VSNCP_3GPP2_PDN_TYPE_IPV6:
          ...
          default:
            LOG_MSG_DATA_PROTO_INFO_1("Unexpected req_addr_type 0x%x",
      ...
      COPY_BYTES( buf, VSNCP_3GPP2I_SEND_OPTION_BUFFER_SIZE,
                  &opthdr, OPTION_HDR_LEN, opthdr_offset );
      break;
    ...
    case VSNCP_3GPP2I_ATTACH_TYPE:
      ...
      COPY_BYTES( buf, VSNCP_3GPP2I_SEND_OPTION_BUFFER_SIZE,
                  &opthdr, OPTION_HDR_LEN, offset );
      COPY_BYTES( buf, VSNCP_3GPP2I_SEND_OPTION_BUFFER_SIZE,
                  &value_p->attach_type, sizeof( uint8), offset );
      break;
    ...
    case VSNCP_3GPP2I_ADDR_ALLOC_CAUSE:
      ...
      COPY_BYTES( buf, VSNCP_3GPP2I_SEND_OPTION_BUFFER_SIZE,
                  &value_p->addr_alloc_cause, sizeof( uint8), offset );
      break;

    default:
      /*( "Got invalid 3GPP2-VSNCP option");*/
      PS_PPP_VSNCP_3GPP2I_ASSERT(0);
      break;
  } /* switch(option) */;
                                   ...
                                   )
{
  ...
  LOG_MSG_DATA_PROTO_INFO_3("vsncp decoded mcc: %x%x%x",
  ...
  LOG_MSG_DATA_PROTO_INFO_1("vsncp decoded mnc_len: %d", op_pco_ptr->mnc_len);
  LOG_MSG_DATA_PROTO_INFO_3("vsncp decoded mcc: %x%x%x",
...
)
{
  ...
  {
    DATA_PROTO_ERROR_LOG_1(PS_BAD_ARG_OR_NULL_PTR,pkt_ptr_ptr);
    return -1;
  }
        ...
        !( ( remote_p->will | remote_p->want ) & ( 1 << n_type ) ) ) )
  {
    ...
    -----------------------------------------------------------------------*/
    LOG_MSG_DATA_PROTO_INFO_1("Invalid option 0x%x in vsncp_3gpp2i_check", ohp->type);
    if( used > 0 )
    {
  ...
  switch( ohp->type )
  {
    ...
    case VSNCP_3GPP2I_PDN_ADDR:
      ...
      if( request == FALSE )
      {
        ...
        switch( lwork_ptr->req_addr_type )
        {
          ...
          default:
            /* unexpected */
            LOG_MSG_DATA_PROTO_INFO_0("Unexpected addr type!");
            return -1;
    ...
    case VSNCP_3GPP2I_PCO:
      ...
      if( request == FALSE )
      {
        ...
        while (signed_len >= VSNCP_3GPP2I_PCO_CONTAINER_HDR_LEN )
        {
          ...
          {
            LOG_MSG_DATA_PROTO_INFO_1("op pco with ID: %x received", container_id);
            /*---------------------------------------------------------------
          ...
          else
          {
            switch( container_id )
            {
              ...
              case PPP_CHAP_PROTOCOL:
                ...
                {
                  LOG_MSG_DATA_PROTO_INFO_0("Tossing PAP/CHAP auth container, not requested!");
                  TOSS_BYTES( pkt_ptr_ptr, signed_container_len, used );
                  break;
                }
              ...
              {
                LOG_MSG_DATA_PROTO_INFO_0("Tossing PAP/CHAP auth container, incorrect auth!");
                TOSS_BYTES( pkt_ptr_ptr, signed_container_len, used );
                break;
              }
              ...
              {
                LOG_MSG_DATA_PROTO_INFO_0("Error grabbing PAP/CHAP cnf hdr");
                return -1;
              }
              ...
              if( container_id == PPP_PAP_PROTOCOL )
              {
                ...
                {
                  case PPP_PAP_ACK:
                    LOG_MSG_DATA_PROTO_INFO_0("PDN Level Auth Success using PAP");
                    break;

                  case PPP_PAP_NAK:
                    LOG_MSG_DATA_PROTO_INFO_0("PDN Level Auth Failure using PAP");
                    break;

                  default:
                    LOG_MSG_DATA_PROTO_INFO_1("PDN Level Auth using PAP,"
              ...
              else /* container_id == PPP_CHAP_PROTOCOL */
              {
                switch( cnf_hdr.code )
                {
                  case PPP_CHAP_SUCCESS:
                    LOG_MSG_DATA_PROTO_INFO_0("PDN Level Auth Success using CHAP");
                    break;
                
                  case PPP_CHAP_FAILURE:
                    LOG_MSG_DATA_PROTO_INFO_0("PDN Level Auth Failure using CHAP");
                    break;

                  default:
                    LOG_MSG_DATA_PROTO_INFO_1("PDN Level Auth using CHAP,"
                                  "Received incorrect code=%d",
                                    cnf_hdr.code);
                    break;
                }
              ...
              while( signed_container_len > 0 )
              {
                ...
                switch( opt_hdr.type )
                {
                  case IPCP_PRIMARY_DNS:
                    ...
                    LOG_MSG_DATA_PROTO_INFO_0("Received DNS v4 primary address in PCO IPCP");
                    PULLUP_BYTES( pkt_ptr_ptr,
                                  &(pco_vals_ptr->dns_v4_primary.ps_s_addr),
                                  sizeof( struct ps_in_addr ),
                                  used );
                  ...
                  case IPCP_SECONDARY_DNS:
                    ...
                    LOG_MSG_DATA_PROTO_INFO_0("Received DNS v4 secondary address in PCO IPCP");
                    PULLUP_BYTES( pkt_ptr_ptr,
                                  &(pco_vals_ptr->dns_v4_secondary.ps_s_addr),
                                  sizeof( struct ps_in_addr ),
                                  used );
                  ...
                  default:
                    LOG_MSG_DATA_PROTO_INFO_1("Unrecognized ipcp option 0x%x in IPCP C-Ack",
                                    opt_hdr.type);
                    TOSS_BYTES( pkt_ptr_ptr,
          ...
          case VSNCP_3GPP2I_PCO_DNS_V4_ADDR_ID:
              ...
              {
                LOG_MSG_DATA_PROTO_INFO_1("Invalid DNS PCO length %d",
                                signed_container_len);
                return -1;
              }
              ...
              if ( 0 == dns_v4_addr )
              {
                LOG_MSG_DATA_PROTO_INFO_0("DNS v4 Response contains 0.0.0.0 address. "
              ...
              {
                LOG_MSG_DATA_PROTO_INFO_0("Received DNS v4 primary address in PCO");
              ...
              {
                LOG_MSG_DATA_PROTO_INFO_0("Received DNS v4 secondary address in PCO");
                pco_vals_ptr->dns_v4_secondary.ps_s_addr = dns_v4_addr;
                is_dns_v4_secondary_addr_obtained = TRUE;
              }
              else
              {
                LOG_MSG_DATA_PROTO_INFO_0("Already received primary and secondary DNS v4 "
             ...
             case VSNCP_3GPP2I_PCO_MTU_ID:              
                ...
                {
                  LOG_MSG_DATA_PROTO_INFO_1("Invalid MTU PCO length %d, MTU ignored!",
                                   signed_container_len);
                  TOSS_BYTES( pkt_ptr_ptr, signed_container_len, used);
                  break;              
                }
                ...
                PULLUP_BYTES( pkt_ptr_ptr,
                              ...
                              used );
                mtu= ps_ntohs(mtu);
                LOG_MSG_DATA_PROTO_INFO_1("VSNCP received mtu value: %d", mtu);
                ...
                {
                  LOG_MSG_DATA_PROTO_INFO_0("VSNCP received mtu values 0!");
                }
                break;
#endif /* FEATURE_DS_MTU_PCO_VSNCP */
            ...
            case VSNCP_3GPP2I_PCO_PCSCF_ADDR_ID:
              ...
              {
                LOG_MSG_DATA_PROTO_INFO_1("Invalid P-CSCF PCO length %d",
                                signed_container_len);
                return -1;
              }
              ...
              {
                LOG_MSG_DATA_PROTO_INFO_0("Tossing P-CSCF PCO!");
                TOSS_BYTES( pkt_ptr_ptr, signed_container_len, used );
                break;
              }
              ...
              if( v6_addr_ptr == NULL )
              {
                /*"Out of heap mem, tossing v6 P-CSCF!"*/
                DATA_PROTO_ERROR_LOG_0(PS_MEM_ALLOC_ERR);
                TOSS_BYTES( pkt_ptr_ptr, signed_container_len, used );
                break;
              }
              ...
              if( PS_IN6_IS_ADDR_V4MAPPED( &v6_addr_ptr->addr ) )
              {
                ...
                if( lwant_ptr->req_addr_type == VSNCP_3GPP2_PDN_TYPE_IPV6 )
                {
                  LOG_MSG_DATA_PROTO_INFO_0("v4 P-CSCF for v6 PDN! Ignoring!");
                ...
                if( v4_addr_ptr == NULL )
                {
                  /*"Out of heap mem, Ignoring v4 P-CSCF!"*/
                  DATA_PROTO_ERROR_LOG_0(PS_MEM_ALLOC_ERR);
                  vsncp_3gpp2i_mem_free( v6_addr_ptr );
                  break;
                }
                ...
                v4_addr_ptr->next_ptr = pco_vals_ptr->p_cscf_v4_head_ptr;
              ...
              else
              {
                ...
                {
                  LOG_MSG_DATA_PROTO_INFO_0("v6 P-CSCF for v4 PDN! Ignoring!");
                  vsncp_3gpp2i_mem_free( v6_addr_ptr );
                  break;
                }
                ...
                -----------------------------------------------------------*/
                v6_addr_ptr->next_ptr = pco_vals_ptr->p_cscf_v6_head_ptr;
            ...
            case VSNCP_3GPP2I_PCO_V4_PCSCF_ADDR_ID:
              {
                if( signed_container_len != SIZE_OF_V4_ADDR )
                {
                  LOG_MSG_DATA_PROTO_INFO_1("Invalid V4 P-CSCF PCO length %d",
                                  signed_container_len);
                  return -1;
                }
                ...
                if( lwant_ptr->pco.p_cscf_is_requested == FALSE )
                {
                  LOG_MSG_DATA_PROTO_INFO_0("Tossing V4 P-CSCF PCO!");
                  TOSS_BYTES( pkt_ptr_ptr, signed_container_len, used );
                  break;
                }
                /*-----------------------------------------------------------
            ...
            case VSNCP_3GPP2I_PCO_DNS_V6_ADDR_ID:
              ...
              {
                LOG_MSG_DATA_PROTO_INFO_0("Tossing v6 DNS PCO!");
                TOSS_BYTES( pkt_ptr_ptr, signed_container_len, used );
                break;
              }
              ...
              if( v6_addr_ptr == NULL )
              {
                /*"Out of heap mem, tossing v6 DNS!"*/
                DATA_PROTO_ERROR_LOG_0(PS_MEM_ALLOC_ERR);
                TOSS_BYTES( pkt_ptr_ptr, signed_container_len, used );
                break;
              }
              ...
              if( PS_IN6_IS_ADDR_V4MAPPED( &v6_addr_ptr->addr ) )
              {
                LOG_MSG_DATA_PROTO_INFO_0("Rx v4 address in v6 DNS PCO! Ignoring");
                vsncp_3gpp2i_mem_free( v6_addr_ptr );
                break;
              }
              ...
              -------------------------------------------------------------*/
              v6_addr_ptr->next_ptr = pco_vals_ptr->dns_v6_head_ptr;
            ...
            case VSNCP_3GPP2I_PCO_SELECTED_BCM_ID:
              ...
              {
                LOG_MSG_DATA_PROTO_INFO_1("Invalid selected bcm PCO length %d",
                                signed_container_len);
                return -1;
              }
              ...
              {
                LOG_MSG_DATA_PROTO_INFO_0("Tossing BCM PCO!");
                TOSS_BYTES( pkt_ptr_ptr, signed_container_len, used );
                break;
              }
            ...
            case VSNCP_3GPP2I_PCO_IP_ADDR_ALLOC_VIA_NAS_ID:
              ...
              {
                LOG_MSG_DATA_PROTO_INFO_1("Invalid ip4_via_nas PCO len %d",
                                signed_container_len);
                return -1;
              }
              else
              {
                LOG_MSG_DATA_PROTO_INFO_1("Ignoring IPv4 via NAS PCO 0x%x",
                                container_id);
                TOSS_BYTES( pkt_ptr_ptr, signed_container_len, used );
              }
              break;
            ...
            default:
              LOG_MSG_DATA_PROTO_INFO_1("Unrecognized container id 0x%x", container_id);
              TOSS_BYTES( pkt_ptr_ptr, signed_container_len, used );
              break;
          } /* container id */
          }
    ...
    default:
      LOG_MSG_DATA_PROTO_INFO_1("3gpp2-VSNCP saw unknown option 0x%x", ohp->type);
      break;
  } /* switch(option) */
  ...
  if( used > 0 )
  {
    ...
    -----------------------------------------------------------------------*/
    LOG_MSG_DATA_PROTO_INFO_1("Tossing extra bytes in option 0x%x", ohp->type);
    if( dsm_pullup( pkt_ptr_ptr, NULL, (uint16)used ) != (uint16)used )
    {
      LOG_MSG_ERROR_0("Unable to toss bytes!");
      return -1;
...
)
{
  ...
  int retval;
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  LOG_MSG_DATA_PROTO_INFO_1("vsncp_3gpp2i_reject() - option 0x%x", ohp->type);
  ...
  switch( ohp->type )
  {
    case VSNCP_3GPP2I_ERROR_CODE:
      ...
      {
        LOG_MSG_DATA_PROTO_INFO_1("Invalid error code %d ", error_code);
        fsm_ptr->fail_reason = PPP_FAIL_REASON_VSNCP_FAILURE;        
      }
      else
      {
        LOG_MSG_DATA_PROTO_INFO_1("Rxed Conf-Rej - error code = 0x%x", error_code);
      ...
      if( ppp_ncp_stop( (ppp_ncp_handle_type)fsm_ptr->pdv ) != 0 )
      {
        LOG_MSG_DATA_PROTO_INFO_0("Unable to terminate 3gpp2 VSNCP instance");
      }
...
)
{
  ...
  {
    LOG_MSG_DATA_PROTO_INFO_1("Invalid PDN-ID 0x%x", option_vals_ptr->pdn_id);
    return -1;
  }
  ...
  {
    LOG_MSG_DATA_PROTO_INFO_1("Invalid apn length 0x%x", option_vals_ptr->apn_len);
    return -1;
  }
  ...
  {
    LOG_MSG_DATA_PROTO_INFO_1("Invalid ip_ver_capability 0x%x",
  ...
  {
    LOG_MSG_DATA_PROTO_INFO_1("Invalid req_addr_type 0x%x",
  ...
  {
    LOG_MSG_DATA_PROTO_INFO_1("Invalid attach_type 0x%x", option_vals_ptr->attach_type);
    return -1;
  }
          ...
          VSNCP_3GPP2_PDN_TYPE_IPV4_IPV6 ) )
  {
    ...
    {
      LOG_MSG_DATA_PROTO_INFO_0("ip4_via_nas set to FALSE!");
      return -1;
    }
    ...
    {
      LOG_MSG_DATA_PROTO_INFO_0("Non-zero default rtr addr in init attach!");
      return -1;
    }
  }
       ...
       PPP_AUTH_CHAP == option_vals_ptr->pco.pdn_level_auth.protocol )
  {
    ...
    {
      LOG_MSG_DATA_PROTO_INFO_0( "Invalid PDN level auth password" );
      return -1;
    }
  }
#endif /* FEATURE_DATA_EHRPD_PDN_LEVEL_AUTH */
...
)
{
  ...
  switch(constant)
  {
    case VSNCP_3GPP2_REQ_TRY:
      vsncp_3gpp2_nv_info[subs_id].req_try = (uint16)value;
      break;

    case VSNCP_3GPP2_TERM_TRY:
      vsncp_3gpp2_nv_info[subs_id].term_try = (uint16)value;
      break;

    case VSNCP_3GPP2_VSNCP_ACK_TIMEOUT:
      vsncp_3gpp2_nv_info[subs_id].ack_timeout = (uint32)value;
      break;

    case VSNCP_3GPP2_TERM_TIMEOUT:
      vsncp_3gpp2_nv_info[subs_id].term_timeout = (uint32)value;
      break;

    default:
      LOG_MSG_DATA_PROTO_INFO_1("Unknown constant %d", constant);
      break;
  }
...
)
{
  ...
  {
    case VSNCP_3GPP2_REQ_TRY:
      *value = vsncp_3gpp2_nv_info[subs_id].req_try ;
      break;

    case VSNCP_3GPP2_TERM_TRY:
      *value = vsncp_3gpp2_nv_info[subs_id].term_try;
      break;

    case VSNCP_3GPP2_VSNCP_ACK_TIMEOUT:
      *value = vsncp_3gpp2_nv_info[subs_id].ack_timeout;
      break;

    case VSNCP_3GPP2_TERM_TIMEOUT:
      *value = vsncp_3gpp2_nv_info[subs_id].term_timeout;
      break;
...
void vsncp_3gpp2_powerup_init( void )
{
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
    ...
    -------------------------------------------------------------------------*/
    memset( &vsncp_3gpp2_nv_info[PPP_SUBS_ID_1], 0, sizeof( vsncp_3gpp2i_nv_info_type ) );
    memset( &vsncp_3gpp2_nv_info[PPP_SUBS_ID_2], 0, sizeof( vsncp_3gpp2i_nv_info_type ) );
...
)
{
  ...
  {
    /*("Invalid handle for vsncp_3gpp2 init");*/
    PS_PPP_VSNCP_3GPP2I_ASSERT(0);
    return -1;
  }
  ...
  -------------------------------------------------------------------------*/
  vsncp_cb_ptr = &(ncp_cb_ptr->cb_u.vsncp_3gpp2);
  LOG_MSG_DATA_PROTO_INFO_1("vsncp_3gpp2_init(), pdn_id 0x%x",
                  vsncp_cb_ptr->config.option_vals.pdn_id);
  ...
  {
    /*("Unable to start a timer for 3gpp2 VSNCP");*/
    PS_PPP_VSNCP_3GPP2I_ASSERT(0);
    return -1;
  }
  ...
  memscpy( &nv_constants,
           ...
           &vsncp_3gpp2_nv_info[ncp_cb_ptr->subs_id],
           sizeof(vsncp_3gpp2i_nv_info_type) );

  LOG_MSG_INFO1_4("PPP VSNCP INIT try_configure %d"
                  " try_terminate %d"
                  " ack_timeout %d"
                  " term_timeout %d",
                  nv_constants.req_try,
                  nv_constants.term_try,
                  nv_constants.ack_timeout,
                  nv_constants.term_timeout);
...
)
{
  ...
  LOG_MSG_DATA_PROTO_INFO_1("vsncp_3gpp2_start - handle %p", ncp_cb_ptr);
  if( NCPI_CB_IS_INITED( ncp_cb_ptr ) == FALSE )
  {
    /*("Invalid handle for vsncp_3gpp2 init");*/
    PS_PPP_VSNCP_3GPP2I_ASSERT(0);
...
)
{
  ...
  LOG_MSG_DATA_PROTO_INFO_0("vsncp_3gpp2_prot_rej()");
  /*-------------------------------------------------------------------------
  ...
  for( index = 0; index < PS_PPP_NCP_CB_NUM; index++ )
  {
    ...
    {
      LOG_MSG_DATA_PROTO_INFO_1("Matching NCP handle found: %p", instance_ptr);
  ...
  {
    LOG_MSG_DATA_PROTO_INFO_1("dev %d, single active NCP, proto_rej on LCP!", device);
    ppp_cb_array = fsm_get_ppp_cb_array();
...
)
{
  ...
  struct config_hdr cnf_hdr;
  struct option_hdr opt_hdr;
  uint8 buf[VSNCP_3GPP2I_RECEIVE_OPTION_BUFFER_SIZE];
  uint8 error_code;
  uint8 offset = 0;
  vsncp_3gpp2i_type * vsncp_cb_ptr;
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

  LOG_MSG_DATA_PROTO_INFO_0("vsncp_3gpp2_receive()");
  ...
  for( index = 0; index < PS_PPP_NCP_CB_NUM; index++ )
  {
    ...
    {
      LOG_MSG_DATA_PROTO_INFO_1("VSNCP: Matching NCP handle found: %p", instance_ptr);
  ...
  if( cnf_hdr.code == CONFIG_REQ )
  {
    ...
    COPY_BYTES( buf, VSNCP_3GPP2I_RECEIVE_OPTION_BUFFER_SIZE, 
                &opt_hdr, OPTION_HDR_LEN, offset );
    COPY_BYTES( buf, VSNCP_3GPP2I_RECEIVE_OPTION_BUFFER_SIZE, 
                &error_code, sizeof( uint8), offset );
...
)
{
  ...
  LOG_MSG_DATA_PROTO_INFO_0("vsncp_3gpp2_request()");
  ...
  if( fsm_send( fsm_ptr,
                ...
                reply_pkt_ptr ) != 0 )
  {
    LOG_MSG_DATA_PROTO_INFO_0("Unable to send C-Ack message");
    dsm_free_packet( &pkt_ptr );
    return -1;
  }
...
)
{
  ...
  signed_len = config_hdr_ptr->length;
  LOG_MSG_DATA_PROTO_INFO_1("vsncp_3gpp2_term_req, length: %d", signed_len );
  while( ( signed_len > 0 )  &&  ( ntohopt( &option, pkt_ptr_ptr ) != -1 ) )
  {
    ...
    LOG_MSG_DATA_PROTO_INFO_2("vsncp_3gpp2_term_req, opt len: %d, opt type:%d",
    ...
    if( VSNCP_3GPP2I_RECONNECT_IND==option.type )
    {
        ...
        {
          LOG_MSG_DATA_PROTO_INFO_0("3gpp2 vsncp(): wrong option length for reconnect ind option");
          dsm_free_packet( pkt_ptr_ptr );
          return -1;
        }
        ...
        LOG_MSG_DATA_PROTO_INFO_1("recved vsncp term req with reconnect indication: %d",
                        reconnect_ind_value );
    }
    else
    {
      ...
      LOG_MSG_DATA_PROTO_INFO_1("recved vsncp term req with option: %d, discard",
                      option.type );
    }
  }
  dsm_free_packet( pkt_ptr_ptr );
...
)
{
  ...
  LOG_MSG_DATA_PROTO_INFO_0("vsncp_3gpp2_ack()");
  ...
  while( ( signed_len > 0 )  &&  ( ntohopt( &option, &pkt_ptr ) != -1 ) )
  {
    ...
    if( signed_len < 0 )
    {
      LOG_MSG_DATA_PROTO_INFO_0("3gpp2 vsncp(): option length > packet length");
      dsm_free_packet( &pkt_ptr );
      return -1;
    }
    ...
    if( option_result < 0 )
    {
      LOG_MSG_DATA_PROTO_INFO_0("3gpp2 vsncp option check failed, dropping C-Ack!");
      /*---------------------------------------------------------------------
...
)
{
  ...
  LOG_MSG_DATA_PROTO_INFO_0("vsncp_3gpp2_reject()");
  ...
  while( ( signed_length > 0 )  &&  ( ntohopt( &option, &pkt_ptr ) != -1 ) )
  {
    ...
    if( signed_length < 0 )
    {
      LOG_MSG_DATA_PROTO_INFO_0("Malformed option in Config-Rej, dropping pkt");
      dsm_free_packet( &pkt_ptr );
      return -1;
    }
    ...
    if( option_index == -1 )
    {
      LOG_MSG_DATA_PROTO_INFO_3( "vsncp_3gpp2_reject(): dev %d, fsm %d, "
    ...
    {
      LOG_MSG_DATA_PROTO_INFO_0("Unsupported option in Config-Rej, dropping pkt");
      dsm_free_packet( &pkt_ptr );
      return -1;
    }
    ...
    {
      LOG_MSG_DATA_PROTO_INFO_0("Invalid option in Config-Rej, dropping pkt");
      dsm_free_packet( &pkt_ptr );
      return -1;
    }
    ...
    {
      LOG_MSG_DATA_PROTO_INFO_0("Throwing away Config-Rej packet");
      dsm_free_packet( &pkt_ptr );
      return -1;
    }
