...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/protocols/linklayer/src/ps_eap_peer.c#3 $ $DateTime: 2020/01/09 10:46:34 $ $Author: pwbldsvc $


when        who    what, where, why
--------    ---    ----------------------------------------------------------
09/29/19    ak     Remove default addition of EAP-AKA for AKA'
...
)
{
  ...
  {
    EAP_INPUT_ERROR_LOG_1(peer_lyr_handle);
    return;
  }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  if( cback == NULL ||
      (EAP_MIN_EXT_METHOD > method) || 
      (EAP_MAX_METHOD <= method))
  {
    EAP_INPUT_ERROR_LOG_2(cback, method);
    return;
  }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if( -1 == peer_lyr_handle  || NULL == peer_lyr_info)
  {
    /*-----------------------------------------------------------------------
      Could not find a free instance
    -----------------------------------------------------------------------*/
    EAP_INPUT_ERROR_LOG_2(peer_lyr_handle, peer_lyr_info);
    return -1;
  }
  ...
  {
    EAP_MEM_ERROR_LOG(sizeof(eap_peer_lyr_cb_type));
    return -1;
  }
  ...
  /*-------------------------------------------------------------------------
    ...
    @9/29/2019:
    Since Client has set only EAP_AKA_PRIME_METHOD,
    eap_auth_prot_mask shall not be modified.Hence commenting below code.
  -------------------------------------------------------------------------*/
/* 
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  EAP_MSG_DEBUG_1("EAP peer delete instance - EAP peer handle: %d",
                  peer_lyr_handle  );
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if( FALSE == eap_handle_is_valid(eap_handle) ||
      EAP_MAX_CODE <= code )
  {
    EAP_INPUT_ERROR_LOG_2(eap_handle, code);
    dsm_free_packet(sdu);
    return;
  }
  ...
  switch(eap_peer_lyr_cb[peer_lyr_handle]->state)
  {
    ...
    default:
      /*---------------------------------------------------------------------
        SANITY check
      ---------------------------------------------------------------------*/
      LOG_MSG_INFO1_1("Discarded outgoing EAP response silently in state %d",
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  EAP_MSG_DEBUG_0("eap_peer_transaction_closing(): called" );
  /*-------------------------------------------------------------------------
...
)
{
  ...
  if( FALSE == EAP_PEER_HANDLE_IS_VALID( peer_lyr_handle ) ||
      NULL == identity )
  {
    EAP_INPUT_ERROR_LOG_2(peer_lyr_handle,identity);
    return;
  }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  /*-------------------------------------------------------------------------
    Check for handle validity and retrieve the Peer layer handle
  -------------------------------------------------------------------------*/
  if( FALSE == EAP_PEER_HANDLE_IS_VALID( peer_lyr_handle ) || NULL == buf )
  {
    EAP_INPUT_ERROR_LOG_2(peer_lyr_handle, buf);
    return 0;
  }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  *sdu = NULL;
  ...
  LOG_MSG_INFO2_1("eap_peeri_request_input(): type %d pkt rcvd",peer_frm_hdr.type);
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if(EAP_PEER_MIN_S >= state || EAP_PEER_MAX_S <= state ||
     EAP_PEER_MIN_EV >= event || EAP_PEER_MAX_EV <= event )
  {
    EAP_INPUT_ERROR_LOG(state, event, 0);
    return;
  }

  EAP_MSG_DEBUG_2("eap_peeri_process_event(): state %d, event %d",
                  state, event);

  if( NULL != eap_peer_state_ev_fnc[state][event] )
  {
    EAP_MSG_DEBUG_1("Posting event %d to EAP peer", event);
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  /*-------------------------------------------------------------------------
    Check validity of state and handle
  -------------------------------------------------------------------------*/  
  if( FALSE == EAP_PEER_HANDLE_IS_VALID( peer_lyr_handle ) ||
      (EAP_PEER_MIN_S > state) || (EAP_PEER_MAX_S < state) )
  {
    EAP_INPUT_ERROR_LOG_2(peer_lyr_handle,state);
    return;
  }

  EAP_MSG_DEBUG_2("Changing state from %d --> %d",
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  EAP_MSG_DEBUG_2("eap_peeri_a_n_auth_ind: Changing state from %d --> %d",
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  EAP_MSG_DEBUG_2("eap_peeri_a_n_result: Changing state from %d --> %d",
                  eap_peer_lyr_cb[peer_lyr_handle]->state, EAP_PEER_CLOSING_S);
  /*-------------------------------------------------------------------------
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if( EAP_MAX_METHOD == requested_method )
  {
    EAP_INPUT_ERROR_LOG_1(requested_method);
    return; 
  }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  {
    /*-----------------------------------------------------------------------
      Different authentication method already in progress or
      Invalid EAP auth request from Authenticator
    -----------------------------------------------------------------------*/
    EAP_INPUT_ERROR_LOG_2(eap_peer_lyr_cb[peer_lyr_handle]->pending_pkt_type, 
                          eap_peer_lyr_cb[peer_lyr_handle]->current_method_type);
...
)
{
 /* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  LOG_MSG_INFO2_1("Received auth indication after EAP Success/Failure %d packet",
                   eap_peer_lyr_cb[peer_lyr_handle]->auth_info.result );
  
  if( EAP_RESULT_FAILURE == 
      eap_peer_lyr_cb[peer_lyr_handle]->auth_info.result )
  {
    eap_eap_auth_non_method_complete_ind( 
      eap_peer_lyr_cb[peer_lyr_handle]->eap_handle,
      EAP_RESULT_FAILURE,
      NULL,
       0 );
  ...
  else
  {
    eap_eap_auth_non_method_complete_ind( 
      eap_peer_lyr_cb[peer_lyr_handle]->eap_handle,
      eap_peer_lyr_cb[peer_lyr_handle]->auth_info.result,
      eap_peer_lyr_cb[peer_lyr_handle]->auth_info.pre_master_key,
      eap_peer_lyr_cb[peer_lyr_handle]->auth_info.pre_master_key_len );
  }
