...
#define PS_LAN_LLC_ASSERT(expression)  \
          ps_lan_llc_assert_wrapper(__LINE__, expression)
...
FUNCTION ps_lan_llc_assert_wrapperr

DESCRIPTION
  Wrapper function for PS_LAN_LLC ASSERT
...
=============================================================================*/
static void ps_lan_llc_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("PS_LAN_LLC_ERR_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
  
}/* ps_lan_llc_assert_wrapper */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ( TRUE == init_flag)
  {
    /* PS LAN LLC Module already initialized */
    PS_LAN_LLC_ASSERT(0);
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    /* Invalid Configuration for LLE start up */
    PS_LAN_LLC_ASSERT(0);
    return -1;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    /* Invalid Configuration for LLE reconfig */
    PS_LAN_LLC_ASSERT(0);
    return -1;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch(ethertype)
  {
    ...
    default:
     /* Registering unsupported ethertype by LLC */
     PS_LAN_LLC_ASSERT(0);
     return -1;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    /* Invalid PS_IFACE pointer in LLE start info */
    PS_LAN_LLC_ASSERT(0);
    return -1;
  }
  ...
  {
    /* Invalid Tx Func Ptr in LLE Start info */
    PS_LAN_LLC_ASSERT(0);
    return -1;
  }
