...
Copyright 1992-2017 William Allen Simpson
...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/protocols/linklayer/src/ps_pppi.c#2 $
...
05/13/16    ssh    meta info leak in pppi_receive()
05/08/15    skc    Use of dsm_item_set_priority() and dsm_item_get_priority()
                   to set and get priority respectively
12/10/14    vl     Added a function ppp_event_deinit() to deallocate all the 
                   event info structure on behalf of clients for TEST_FRAMEWORK
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_DATA_PROTO_INFO_2("a2_hdlc_config_sio_mdm called with enable/disable: %d with accm: 0x%x",
                  enable, hdlc_accm);
  if(enable)
  {
...
)
{
  ...
  LOG_MSG_DATA_PROTO_INFO_1("dev init, setting HDLC ACCM = 0x%x", ioctl_data.accm);
  ...
  {
    LOG_MSG_DATA_PROTO_INFO_2("Unable to set HDLC ACCM for dev %d, ps_errno %d",
...
)
{
      ...
      PPP_MAX_DEV <= configbuf_ptr->device )
  {
    ...
    /*( "Invalid arguments: config_ptr, device")*/
    PS_PPPI_ASSERT(0);
    return;
  }
  ...
  if(session_handle == 0)
  {
    LOG_MSG_DATA_PROTO_INFO_1("starti() for free dev %d", configbuf_ptr->device);
  }
  else
  {
    /*-----------------------------------------------------------------------
    ...
    if( PPP_CP_MODE_INVALID  != config_ptr->ipcp_info.mode )
    {
      ...
      {
        LOG_MSG_DATA_PROTO_INFO_0("FSM in fsmSTOPPED state, stopping IPCP");
    ...
    if( PPP_CP_MODE_INVALID  != config_ptr->ipv6cp_info.mode )
    {
      ...
      {
        LOG_MSG_DATA_PROTO_INFO_0("FSM in fsmSTOPPED state, stopping IPv6CP");
...
)
{
  ...
  if(session_handle == 0)
  {
    LOG_MSG_DATA_PROTO_INFO_1("resynci() for free dev %d", configbuf_ptr->device);
  }
  else
  {
    /*-----------------------------------------------------------------------
        ...
        fsmINITIAL != ppp_cb_ptr->fsm[IPCP].state )
    {
      ...
      if( TRUE == start_ncp )
      {
        if( NULL != config_ptr )
        {
          ...
          {
            LOG_MSG_DATA_PROTO_INFO_0("pppi_resync: Disabling DNS option in IPCP");
            /*---------------------------------------------------------------
      ...
      for( index = 0; index < PS_PPP_NCP_CB_NUM; index++ )
      {
           ...
           ( ncp_handle_ptr->device == device ))
        {
          switch (ncp_handle_ptr->ncp)
          {
            ...
            default:
              /* unexpected! */
              LOG_MSG_DATA_PROTO_INFO_1("Unsupported NCP 0x%x", ncp_handle_ptr->ncp);
             break;
           }/* switch (ncp) */      
...
)
{
  ...
  {
    LOG_MSG_DATA_PROTO_INFO_1 ("Invalid stop_info.protocol = %d", stop_info.protocol);
    return;
  }
  ...
  {
    LOG_MSG_DATA_PROTO_INFO_1("stopi() for free dev %d", stop_info.device);
    stop = FALSE;
  }
  else
  {
...
)
{
  ...
  {
    LOG_MSG_DATA_PROTO_INFO_1("Ignoring, pppi_abort(%d) on free device!",
                    abort_info.device);
    return;
  }
  ...
  {
    LOG_MSG_DATA_PROTO_INFO_1("Error in resetting HDLC instance 0x%x",
...
)
{
  ...
  if (dsm_item_get_priority(*incoming_pkt_ptr) == DSM_QUEUE_PRIORITY_HIGH)
  {
    is_tcp_ack = TRUE;
  }
  ...
  {
    PPP_INC_INSTANCE_STATS(device, pkts_dropped_tx, 1);
    LOG_MSG_DATA_PROTO_INFO_1("PPP %d not configured discading pkt", device);
    dsm_free_packet(incoming_pkt_ptr);
    PS_TX_META_INFO_FREE(&meta_info_ptr);
    return -1;
  }
    ...
    if( vsncp_iface_ptr != NULL )
    {
      for( index = 0; index < PS_PPP_NCP_CB_NUM; index++ )
      {
             ...
             ) )
        {
          ...
          {
            LOG_MSG_DATA_PROTO_INFO_2( "dev %d, pdn %d not open for VSNP traffic",
                             device, pdn_id);
            PPP_INC_INSTANCE_STATS(device,pkts_dropped_tx, 1);
            dsm_free_packet(incoming_pkt_ptr);
            PS_TX_META_INFO_FREE(&meta_info_ptr);
  ...
  if( PPP_IP_PROTOCOL == protocol )
  {
    ...
    {
      LOG_MSG_DATA_PROTO_INFO_1( "dev %d Not Open for IP Traffic", device);
      PPP_INC_INSTANCE_STATS(device,pkts_dropped_tx, 1);
      dsm_free_packet(incoming_pkt_ptr);
      PS_TX_META_INFO_FREE(&meta_info_ptr);
      return -1;
    ...
    if(ppp_cb_ptr->fsm[IPCP].remote.work & IPCP_N_COMPRESS)
    {
          ...
          NULL != ((ipcp_pdv_type*)ppp_cb_ptr->fsm[IPCP].pdv)->slhcp)\
      {
                           ...
                           &slhc_pkt_type ) == SLHC_SUCCESS )
        {
          ...
          default:
              LOG_MSG_DATA_PROTO_INFO_1("pppi_outgoing: Invalid slhc packet type: %d.",
                              slhc_pkt_type);
              break;
          }
        }
        else
        {
            ...
            ---------------------------------------------------------------*/
            PPP_INC_INSTANCE_STATS(device,pkts_dropped_tx, 1);
            LOG_MSG_DATA_PROTO_INFO_0( "Bad IP Packet");
            dsm_free_packet(incoming_pkt_ptr);
            PS_TX_META_INFO_FREE(&meta_info_ptr);
            PS_BRANCH_LEAVE_CRIT_SECTION( pppi_get_crit_sect_lock(device) );
            return -1;
  ...
  if( PPP_IPV6_PROTOCOL == protocol )
  {
    ...
    {
      LOG_MSG_DATA_PROTO_INFO_1( "dev %d Not Open for IPv6 Traffic", device);
      PPP_INC_INSTANCE_STATS(device,pkts_dropped_tx, 1);
      dsm_free_packet(incoming_pkt_ptr);
      PS_TX_META_INFO_FREE(&meta_info_ptr);
      return -1;
  ...
  *cp++ = (protocol & 0x00ff);
  ...
  else
  {
    ...
    *incoming_pkt_ptr = NULL;
    ...
    if (is_tcp_ack)
    {
      dsm_item_set_priority(tx_item_ptr, DSM_QUEUE_PRIORITY_HIGH);
    }
...
)
{
  ...
  if(ppp_cb_ptr->hdlc_mode != HDLC_OFF)
  {
    ...
    if(first_byte != HDLC_ALL_ADDR)
    {
         ...
         !(negotiated & LCP_N_ACFC))
      {
        ...
        LOG_MSG_DATA_PROTO_INFO_0( "Missing ALL Address");
        pppi_skipped(device,  item_ref_ptr);
        return;
      }
    } /* if(first byte != FF) */
    ...
    else if(second_byte != HDLC_UI)
    {
          ...
          !(negotiated & LCP_N_PFC)))
      {
        ...
        LOG_MSG_DATA_PROTO_INFO_0( "Missing UI");
        pppi_skipped(device,  item_ref_ptr);
        return;
      }
    }
  ...
  {
    LOG_MSG_DATA_PROTO_INFO_0( "Truncated packet");
    pppi_skipped(device,  item_ref_ptr);
    return;
  }
  ...
  if( protocol & 0x01)
  {
      ...
      !(negotiated & LCP_N_PFC))
    {
      ...
      LOG_MSG_DATA_PROTO_INFO_0( "Missing Upper Protocol Byte");
      pppi_skipped(device,  item_ref_ptr);
      return;
    }
  }
  ...
  else
  {
    ...
    {
      PPP_INC_INSTANCE_STATS(device,bad_proto,1);
      LOG_MSG_DATA_PROTO_INFO_0( "Missing Lower Protocol Byte");
      pppi_skipped(device,  item_ref_ptr);
      return;
    }
  }
  ...
  switch( protocol )
  {
    ...
    case PPP_VSNCP_PROTOCOL:
      ...
      switch( oui )
      {
        ...
        default:
          LOG_MSG_DATA_PROTO_INFO_1("Unsupported OUI 0x%x rxed in VSNCP pkt", oui);
          PPP_INC_INSTANCE_STATS(device, not_enabled_proto, 1);
  ...
  case PPP_EAP_PROTOCOL:
    ...
    case PPP_IP_PROTOCOL:
      ...
      else
      {
        ...
        {
          PPP_INC_INSTANCE_STATS(device,not_enabled_proto, 1);
          LOG_MSG_DATA_PROTO_INFO_0( "Not Open For IP Traffic");
          pppi_skipped(device,  item_ref_ptr);
          break;
        }
    ...
    case PPP_IPV6_PROTOCOL:
      ...
      else
      {
        ...
        {
          PPP_INC_INSTANCE_STATS(device,not_enabled_proto, 1);
          LOG_MSG_DATA_PROTO_INFO_0( "Not Open For IPv6 Traffic");
          pppi_skipped(device, item_ref_ptr);
          break;
        }
    ...
    case PPP_VSNP_PROTOCOL:
      ...
      for( index = 0; index < PS_PPP_NCP_CB_NUM; index++ )
      {
              ...
              == pdn_id ) )
        {
          ...
          {
            PPP_INC_INSTANCE_STATS(device, not_enabled_proto, 1);
            LOG_MSG_DATA_PROTO_INFO_1("pdn id 0x%x not open for VSNP traffic!", pdn_id);
            pppi_skipped(device,  item_ref_ptr);
            return;
          }
          ...
          {
            LOG_MSG_DATA_PROTO_INFO_1("Invalid IP packet found in pkt = 0x%x",
                            item_ref_ptr);
            if( meta_info_ex_ptr != NULL )
            {
              PS_RX_META_INFO_FREE(&meta_info_ptr);
      ...
      ---------------------------------------------------------------------*/
      LOG_MSG_DATA_PROTO_INFO_1("Dropping data for invalid pdn id 0x%x", pdn_id);
      pppi_skipped(device,  item_ref_ptr);
      break;
#endif /* FEATURE_DATA_PS_EHRPD */
    ...
    case PPP_VJCH_PROTOCOL:
      ...
      else
      {
        ...
        {
          PPP_INC_INSTANCE_STATS(device,not_enabled_proto, 1);
          LOG_MSG_DATA_PROTO_INFO_0( "Not Open for Compressed TCP/IP");
          pppi_skipped(device,  item_ref_ptr);
          break;
        }
        ...
        {
          PPP_INC_INSTANCE_STATS(device,not_enabled_proto, 1);
          LOG_MSG_DATA_PROTO_INFO_0( "Compressed TCP/IP Not Enabled");
          pppi_skipped(device,  item_ref_ptr);
          break;
        }
        ...
        {
          LOG_MSG_DATA_PROTO_INFO_0( "Compressed TCP/IP Packet Err");
          pppi_skipped(device,  item_ref_ptr);
          PS_LEAVE_CRIT_SECTION( pppi_get_crit_sect_lock(device) );
          break;
        }
    ...
    case PPP_VJUCH_PROTOCOL:
      ...
      else
      {
        ...
        {
          PPP_INC_INSTANCE_STATS(device,not_enabled_proto, 1);
          LOG_MSG_DATA_PROTO_INFO_0( "Not Open for Uncompressed TCP/IP");
          pppi_skipped(device,  item_ref_ptr);
          break;
        }
        ...
        {
          PPP_INC_INSTANCE_STATS(device,not_enabled_proto, 1);
          LOG_MSG_DATA_PROTO_INFO_0( "Uncompressed TCP/IP Not Enabled");
          pppi_skipped(device, item_ref_ptr);
          break;
        }
        ...
        {
          LOG_MSG_DATA_PROTO_INFO_0( "Uncompressed TCP/IP Packet Err");
          dsm_free_packet(item_ref_ptr);
          PS_RX_META_INFO_FREE(&meta_info_ptr);
          PS_LEAVE_CRIT_SECTION( pppi_get_crit_sect_lock(device) );
          break;
...
)
{
  ...
  LOG_MSG_DATA_PROTO_INFO_0("pppi_default_rx()!");
  return NULL;
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  LOG_MSG_DATA_PROTO_INFO_0("pppi_default_tx() freeing pkt!");
...
)
{
  ...
  ppp_cb_array = fsm_get_ppp_cb_array();
  ppp_cb_ptr = &(ppp_cb_array[device]);
  LOG_MSG_DATA_PROTO_INFO_1( "Unknown PPP protocol: %x", protocol);
    ...
    switch(protocol)
    {
      ...
      case PPP_PROTOCOL_VSNCP:
       ...
       ------------------------------------------------------------------*/
        ...
        {
          LOG_MSG_DATA_PROTO_INFO_1("Calling cback for NCP instance 0x%x",
                          ncp_handle_ptr);
          event_ptr->cback[protocol]( event_ptr->device,
                                      protocol,
                                      event,
...
)
{
  ...
  LOG_MSG_DATA_PROTO_INFO_3("pppi_report_event_to_diag: dev (%d), prot (%d), state (%d)",
...
)
{
  ...
  ppp_cb_array = fsm_get_ppp_cb_array();
  LOG_MSG_DATA_PROTO_INFO_2("PPP discarding received packet in NONE mode(dev %d, mode %d)",
...
)
{
  ...
  ppp_cb_array = fsm_get_ppp_cb_array();
  LOG_MSG_DATA_PROTO_INFO_2("PPP discarding received packet in RELAY mode(dev %d, mode %d)",
                  device, ppp_cb_array[device].mode);
  PS_PPPI_ASSERT( 0 );
  dsm_free_packet( packet );
  return;
      ...
      switch( ps_errno )
      {
        case HDLC_MORE_PKTS_AVAIL:
          /*----------------------------------------------------------------
            ...
            again.
          ----------------------------------------------------------------*/
          LOG_MSG_DATA_PROTO_INFO_0("HDLC Unframer: More packets available");
          if( unframed != NULL )
          {
    ...
    )
  {
    ...
    switch( snoop_res )
    {
      case SNOOP_IGNORE_PKT:
        LOG_MSG_DATA_PROTO_INFO_1(" %d bytes snooped - not interested in pkt",
                        (param_ptr->snoop_pkt_ptr)->used);
        dsm_free_packet( &( param_ptr->snoop_pkt_ptr ) );
        return FINDING_EOP;

      case SNOOP_DO_UNFRAMING:
        LOG_MSG_DATA_PROTO_INFO_1(" %d bytes snooped - might have protocol",
                        (param_ptr->snoop_pkt_ptr)->used);
        return UNFRAMING;

      case SNOOP_NEXT_TWO_BYTES:
        LOG_MSG_DATA_PROTO_INFO_1(" %d bytes snooped - need to unframe more",
                        (param_ptr->snoop_pkt_ptr)->used);
        return SNOOPING;
...
)
{
  ...
  while( *in_pkt_ptr != NULL )
  {
    ...
    LOG_MSG_DATA_PROTO_INFO_2("Dev %d Pseudonet in %d state", device, param_ptr->state);
    switch( param_ptr->state )
    {
      ...
      case UNFRAMING:
        ...
        if(param_ptr->framed_pkt_ptr != NULL)
        {
          LOG_MSG_DATA_PROTO_INFO_2("Dev %d Pseudonet in %s state - Appending stored "
    ...
    if( *in_pkt_ptr != NULL)
    {
      ...
      if( read_ptr == end_ptr ) /* exhausted current dsm item */
      {
        ...
        {
          LOG_MSG_DATA_PROTO_INFO_2("Dev %d Pseudonet in %s state - APPENDING",
                          device, param_ptr->state);
...
)
{
  ...
  {
    LOG_MSG_DATA_PROTO_INFO_1("PPP device %d not initialized, failing...", device);
    return -1;
  }

  /*-----------------------------------------------------------------------
    Apply subs ids from cache config
  -----------------------------------------------------------------------*/  
  if( PPP_MAX_SUBS_ID <= cached_config_ptr->subs_id )
  {
    DATA_PROTO_ERROR_LOG_2(PS_BAD_ARG_OR_NULL_PTR,
                           device,cached_config_ptr->subs_id);
    return -1;
  }

  config_ptr->subs_id = cached_config_ptr->subs_id;
...
)
{
  ...
  if(ppp_cb_ptr->session_handle == 0)
  {
    LOG_MSG_DATA_PROTO_INFO_1("pppi_device_stop() for free dev %d", device);
    return;
  }
  else
  {
...
)
{
      ...
      ppp_cb_ptr->lsd.config.echo_req_timeout > 0 )
  {
    do
    {
      ...
      ---------------------------------------------------------------------*/
      LOG_MSG_DATA_PROTO_INFO_1("Dev %d, sending LCP Echo-Req", device );
                    ...
                    echo_req_pkt ) != 0 )
      {
        ...
        -------------------------------------------------------------------*/
        LOG_MSG_DATA_PROTO_INFO_1("Dev %d, Unable to send LCP Echo-Req!", device);
        break;
      }
...
/*===========================================================================
FUNCTION ps_pppi_qsh_mdump_collect
  
DESCRIPTION
  This function collects the mdump for ppp_info variable
...
===========================================================================*/
QSH_MDUMP_FN_ATTR 
void ps_pppi_qsh_mdump_collect()
{
  qsh_mdump_collect_high(&ppp_info, PPP_MAX_DEV * sizeof(ppp_info));
