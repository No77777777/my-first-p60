...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/protocols/linklayer/src/ps_ppp_vend.c#2 $
...
#define PS_PPP_VEND_ASSERT(expression)  \
               ps_ppp_vend_assert_wrapper(__LINE__, expression)
...
=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=*/
 ...
 FUNCTION ps_ppp_vend_assert_wrapper
 
 DESCRIPTION
   Wrapper function for PS_PPP_VEND_ASSERT
 ...
 ===========================================================================*/
 static void ps_ppp_vend_assert_wrapper
 (
 ...
 )
 {
   if ( !expression )
   {
     ERR_FATAL("PS_PPP_VEND_ERR_FATAL at line:%d ",  
                                    line_num,0,0);  
   }
 }/* ps_ppp_vend_assert_wrapper */
...
)
{
  ...
  {
    DATA_PROTO_ERROR_LOG_2(PS_BAD_ARG_OR_NULL_PTR,device,oui);
    return -1;
  }
...
)
{
  ...
  {
    DATA_PROTO_ERROR_LOG_2(PS_BAD_ARG_OR_NULL_PTR,device,pkt_ptr);
    return -1;
  }
  ...
  if(pkt_info == NULL)
  {
    /*"ppp_vendor_send(): Cannot obtain ppp_vendor_pkt_info_s_type"
                   " memory buffer"    */
...
)
{
  ...
  PS_PPP_VEND_ASSERT( hdr->code == VENDOR_SPECIFIC );
...
)
{
  ...
  PS_PPP_VEND_ASSERT(PPP_IS_CURR_TASK_OWNER());
