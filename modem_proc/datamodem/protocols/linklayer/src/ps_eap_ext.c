...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/protocols/linklayer/src/ps_eap_ext.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
)
{
  ...
  EAP_MSG_DEBUG_1("EAP EXT Init %d", eap_ext_already_init);
...
)
{
  ...
  EAP_MSG_DEBUG_1("Creating an EAP Extended Instance, EAP handle %d",
                  eap_handle);
...
)
{
  ...
  EAP_MSG_DEBUG_1("Deleting an EAP Extended Instance, EAP handle %d",
                  eap_handle);
  ...
  if ( EAP_EXT_MAX_INST <= index || NULL == eap_ext_cb[index] ) 
  {
    EAP_INPUT_ERROR_LOG_1(index);
    return;
  }
...
)
{
  ...
  EAP_MSG_DEBUG_1("Input cb called for EAP Ext, EAP handle %d", eap_handle);
  ...
  if ((0 == vendor_id[0]) && (0 == vendor_id[1]) && (0 == vendor_id[2]))
  {
    ...
    -----------------------------------------------------------------------*/
    LOG_MSG_ERROR_1("EAP Ext Input: Invalid packet received vendor type %d",
                    vendor_type );

    if (EAP_EXT_NAK_TYPE == vendor_type)
    {
      /*---------------------------------------------------------------------
        Expanded NAK packet received. Indicating failure to backend Vendor
      ---------------------------------------------------------------------*/
      dsm_free_packet(msg_ptr);
...
)
{
  ...
  EAP_MSG_DEBUG_1("Close cb called for EAP Ext, EAP handle %d", eap_handle);
  ...
  /*-----------------------------------------------------------------------
    Check if the vendor information is set, anyhow ignore and close all
    vendor handlers
  -------------------------------------------------------------------------*/
  if ( !eap_ext_cb[index]->vendor_info_set )
  {
...
)
{
  ...
  EAP_MSG_DEBUG_1("Result cb called for EAP Ext, EAP handle %d", eap_handle);
  ...
  /*-----------------------------------------------------------------------
    Check if the vendor information is set, if not found ignore
  -------------------------------------------------------------------------*/
  if (!eap_ext_cb[index]->vendor_info_set)
  {
    EAP_INPUT_ERROR_LOG_1(index);
...
)
{
  ...
  EAP_MSG_DEBUG_1("Xmit packet called for EAP Ext, EAP handle %d", eap_handle);
  ...
  {
    /*---------------------------------------------------------------------
      Vendor ID not registered with this instance
    -----------------------------------------------------------------------*/
    EAP_INPUT_ERROR_LOG_2(vendor_index, 
...
{
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  EAP_ERROR_DEBUG_1("Auth complete ind called for EAP Ext, EAP handle %d",
                    handle);
...
)
{
  ...
  EAP_ERROR_DEBUG_1("Clean CB for instance %d", index);

  /*-----------------------------------------------------------------------
    Sanity check
  -------------------------------------------------------------------------*/
  if ( EAP_EXT_MAX_INST <= index || eap_ext_cb[index] == NULL ) 
  {
    EAP_INPUT_ERROR_LOG_1(index);
    return;
  }
...
)
{
  ...
  EAP_MSG_DEBUG_1("Find instance for EAP handle %d", eap_handle);
  ...
  for( index = 0; index < EAP_EXT_MAX_INST; index++ )
  {
    ...
    {
      EAP_MSG_DEBUG_2("Found instance %d for EAP handle %d",
                      index, eap_handle);
      break;
    }
  }
...
)
{
  ...
  /*-----------------------------------------------------------------------
    Sanity check
  -----------------------------------------------------------------------*/
  EAP_MSG_DEBUG_0("EAP Ext: Setting Vendor Information");
  ...
  if ( NULL == meta_info ) 
  {
    EAP_INPUT_ERROR_LOG_2(index, eap_ext_cb[index]->eap_handle);
    return FALSE;
  }
