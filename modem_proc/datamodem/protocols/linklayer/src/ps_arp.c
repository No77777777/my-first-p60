...
$Header: //components/rel/data.mpss/3.5..2.1/protocols/linklayer/src/ps_arp.c#2 $
...
#define PS_ARP_ASSERT(expression)  \
          ps_arp_assert_wrapper(__LINE__, expression)
...
FUNCTION ps_arp_assert_wrapper

DESCRIPTION
  Wrapper function for PS_ARP ASSERT
...
=============================================================================*/
static void ps_arp_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("PS_ARP_ERR_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
  
}/* ps_arp_assert_wrapper */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ( TRUE == init_flag)
  {
   /* PS ARP Module already initialized */
    PS_ARP_ASSERT(0);
    return;
  }
  ...
  LOG_MSG_DATA_PROTO_INFO_0("ARP Module Initialized");
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    /* arp_start(): Invalid LLE */
    PS_ARP_ASSERT(0);
    return -1;
  }

  PS_ARP_ASSERT(NULL != llc_tx_f_ptr);
  PS_ARP_ASSERT(NULL != arp_config_ptr);
  ...
  {
    PS_ARP_ASSERT(NULL != ptr_arpi_cb->proxy_query_f_ptr);
  ...
  if ( TRUE == arp_config_ptr->enable_bcast_gratuitous_arp)
  {
    ...
    {
      /* Failed to allocate PS timer for Gratuitous ARP */
      PS_ARP_ASSERT(0);
      return -1;
    }
  ...
  LOG_MSG_DATA_PROTO_INFO_1("ARP instance %d started!", lle_instance);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    /* Error flushing ARP cache associated with LLE instane */
    PS_ARP_ASSERT(0);
    return -1;
  }
       ...
       ( ptr_arpi_cb->garp_bcast_interval > 0 ) )
  {
    ...
    {
      /* Error stopping bcast gratuitous ARP cache associated with LLE instane */
      PS_ARP_ASSERT(0);
      return -1;
    }
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ( NULL != ptr_arpi_entry)
  {
    ...
    LOG_MSG_DATA_PROTO_INFO_1("Found ARP entry in cache for IP addr 0x%x",
    ...
    if ( !(ptr_arpi_entry->status & ARP_STATUS_RES_DONE))
    {
      ...
      LOG_MSG_DATA_PROTO_INFO_1("ARP Resolution pending for IP 0x%x also "
      ...
      {
        /* arp_ipv4_tx_pkt(): ptr_arpi_entry->pending_pkt or ptr_arpi_entry->pending_pkt->app_ptr is null */
        PS_ARP_ASSERT(0);
        return -1;
      }
  ...
  {
    LOG_MSG_DATA_PROTO_INFO_1("No ARP entry found in cache for IP addr: 0x%x",
                   ps_ntohl(dst_ip_addr));
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch (frm_type)
  {
    ...
    {
      LOG_MSG_DATA_PROTO_INFO_0("Received Unknown ARP frame, dropping it");
    ...
    case ARP_REQUEST_FRM:
    {
      ...
      if ( NULL != ptr_arp_entry )
      {
        ...
        {
          LOG_MSG_DATA_PROTO_INFO_1("Updating ARP entry(IP: 0x%x) with incomming ARP Request",
        ...
        else if ( ptr_arp_entry->status & ARP_STATUS_PROBING)
        {
                      ...
                      LAN_IEEE_MAC_ADDR_LEN)) )
          {
            ...
            ---------------------------------------------------------------*/
            PS_ARP_ASSERT(NULL != ptr_arp_entry->arp_cback);
      ...
      else
      {
        ...
        if ( NULL != ptr_arp_entry)
        {
          ...
          if ( ptr_arpi_cb->flags & ARP_CACHE_REFRESH_ENABLE)
          {
            ...
            ---------------------------------------------------------------*/ 
            LOG_MSG_DATA_PROTO_INFO_1("Start monitoring entry of IP 0x%x",
      ...
      if ( TRUE == should_respond )
      {
        ...
        -------------------------------------------------------------------*/
        LOG_MSG_DATA_PROTO_INFO_0("Transmitting ARP response");
        ...
        {
          /* Fail to transmit ARP response packet */
          PS_ARP_ASSERT(0);
    ...
    case ARP_RESPONSE_FRM:
    {
      ...
      {
        LOG_MSG_DATA_PROTO_INFO_0("ARP Response not directed to us");
      ...
      if ( NULL != ptr_arp_entry )
      {
        ...
        if ( ptr_arp_entry->status & ARP_STATUS_RES_PEND)
        {
          ...
          -----------------------------------------------------------------*/ 
          LOG_MSG_DATA_PROTO_INFO_1("Transmitting IP Packet(IP: 0x%x) after successful ARP",
                          ptr_arp_entry->ip_addr);
          ...
          {
            ps_tx_meta_info_type** tmp_ps_mi_ptr;
            /* Fail to transmit pkt after successful ARP resolution */
            PS_ARP_ASSERT(0);
      
            PS_ARP_ASSERT(NULL != pend_pkt->app_ptr);
        ...
        else if ( ptr_arp_entry->status & ARP_STATUS_CHECKING)
        {
          ...
          -----------------------------------------------------------------*/ 
          LOG_MSG_DATA_PROTO_INFO_0("Receive response to unicast probe");
      ...
      {
        LOG_MSG_DATA_PROTO_INFO_0("ARP Response directly to us with no request pending");
        ARP_INC_INSTANCE_STATS(lle_instance, no_req_pending_rx, 1); 
        LOG_MSG_DATA_PROTO_INFO_2("ARP Response info: sender IP(0x%x), target IP(0x%x)",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (loop == LAN_LLE_MAX)
  {
    /* Cannot find ARP instance attached to PS IFACE for update */
    PS_ARP_ASSERT(0);
    return;
  }
  ...
  PS_ARP_ASSERT ( (ptr_arpi_cb->arp_instance > LAN_LLE_MIN) &&
  ...
  if ( FALSE == set_or_clear)
  {
    ...
    if (NULL == ptr_arpi_entry)
    {
      ...
      ---------------------------------------------------------------------*/
      LOG_MSG_DATA_PROTO_INFO_1("ARP entry of IP 0x%x to clear doesn't actually exist!",
                      ip_addr);
      return;
    }
    else
  ...
  else
  {
    ...
    if ( NULL != ptr_arpi_entry)
    {
      ...
      if ( ptr_arpi_entry->status & ARP_STATUS_RES_DONE)
      {
        LOG_MSG_DATA_PROTO_INFO_1("Update ARP entry of IP 0x%x", ip_addr);
    ...
    else
    {
      ...
      if ( NULL == ptr_arpi_entry)
      {
        /*-------------------------------------------------------------------
          We're dead here
        -------------------------------------------------------------------*/ 
        /* Cannot insert new entry for ARP_IPV4_UPDATE */
        PS_ARP_ASSERT(0);
      }
      else
      {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (loop == LAN_LLE_MAX)
  {
    /* Cannot find ARP instance attached to PS IFACE! */
    PS_ARP_ASSERT(0);
    return -1;
  }
  ...
  switch (probe_params_ptr->probe_type)
  {
    case ARP_BROADCAST_PROBE:
    {
      ...
      ---------------------------------------------------------------------*/
      /* Error perfoming broadcast ARP probing */
      PS_ARP_ASSERT(0);
      return -1;      
    }
    ...
    case ARP_UNICAST_PROBE:
    {
      ...
      if(NULL == ptr_arpi_entry)
      {
        /* Unicast ARP probe for an entry which is not in cache */
        PS_ARP_ASSERT(0);
        return -1;
      }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if( ((NULL == ptr_arpi_cb->conflict_cback) &&
      ...
      ((NULL != ptr_arpi_cb->conflict_cback) &&
       (NULL != arp_conflict_cb_ptr)) )
  {
    /* Invalid registration of conflict cback */
    PS_ARP_ASSERT(0);
    return -1;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
                                          ...
                                          req_duration)) )
  {
    ...
    -----------------------------------------------------------------------*/
    /* Impossible to start Gratuitous ARP Request timer*/
    PS_ARP_ASSERT(0);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  PS_ARP_ASSERT( cmd == PS_START_BCAST_GARP );
