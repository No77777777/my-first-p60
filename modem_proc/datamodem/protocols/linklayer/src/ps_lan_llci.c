...
$Header: //components/rel/data.mpss/3.5..2.1/protocols/linklayer/src/ps_lan_llci.c#2 $
$Author: pwbldsvc $ $DateTime: 2019/10/22 02:53:17 $

when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
11/01/2017 my      CR 2125723 : FR 42790: F3 reduction
03/20/17   ds      Fix SA errors.
01/05/17    ssh    LLVM Static Analysis fixes
03/25/15   cb      Fix LLVM SA warnings
12/01/14   skc     ps_disable_sig() removed
...
#define PS_LAN_LLCI_ASSERT(expression)  \
          ps_lan_llci_assert_wrapper(__LINE__, expression)
...
FUNCTION ps_lan_llci_assert_wrapperr

DESCRIPTION
  Wrapper function for PS_LAN_LLCI ASSERT
...
=============================================================================*/
static void ps_lan_llci_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("PS_LAN_LLCI_ERR_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
  
}/* ps_lan_llci_assert_wrapper */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
    ...
    {
      DATA_PROTO_ERROR_LOG_1(PS_BAD_ARG_OR_NULL_PTR, lle_instance);
      return;
    }
    ...
    {
       /* The Interface hardware address is NULL */
       PS_LAN_LLCI_ASSERT(0);
       return;
    }
    ...
    LOG_MSG_DATA_PROTO_INFO_1("PS LLC instance %d started !", lle_instance );
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_DATA_PROTO_INFO_1("PS LLC instance %d, Stopped!", lle_instance);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ( TRUE != ptr_lle_cb->start_lock)
  {
    ...
    {
      LOG_MSG_DATA_PROTO_INFO_1("Receive RX signal when LLC instance %d stopped",
                      lle_instance);
  ...
  else
  {
    if ( TRUE != ptr_lle_cb->started)
    {
      LOG_MSG_DATA_PROTO_INFO_1("Receive RX signal when LLC instance %d startup is pending",
                      lle_instance);
      /*-----------------------------------------------------------------------
  ...
  -------------------------------------------------------------------------*/
  PS_LAN_LLCI_ASSERT(rx_sig == ptr_start_info->lle_rx_sig);
           ...
           ) != NULL)
  {
    ...
    if ( rx_pkt == NULL )
    {
      LOG_MSG_DATA_PROTO_INFO_0("Zero payload ethernet frame."
    ...
    switch (frm_type)
    {
      case LLC_UNKNOWN_FRM:
      {
        ...
        -------------------------------------------------------------------*/
        LOG_MSG_DATA_PROTO_INFO_0("Receive unknown LLC frame, Frame will be dropped!");
      ...
      case LLC_CTRL_FRM:
      {
        /*-------------------------------------------------------------------
          LLC Control frames
        -------------------------------------------------------------------*/
        LOG_MSG_DATA_PROTO_INFO_0("Processing LLC Control frame");
      ...
      case LLC_IPV4_FRM:
      {
        ...
        {
          LOG_MSG_DATA_PROTO_INFO_0("Inputting IPv4 frame to PS IFACE");
        ...
        {
          LOG_MSG_DATA_PROTO_INFO_0("Dropping mismatched IPv4 packet for interface type");
          dsm_free_packet(&rx_pkt);
          LLC_INC_INSTANCE_STATS(lle_instance, frms_drop_rx, 1);
        }
        break;
...
#ifdef FEATURE_DATA_PS_IPV6
      {
        ...
        if(ps_iface_addr_family_is_v6(ptr_start_info->iface_ptr))
        {
          LOG_MSG_DATA_PROTO_INFO_0("Inputting IPv6 frame to PS IFACE");
          (void) ps_iface_input(ptr_start_info->iface_ptr,
                                &rx_pkt,
                                meta_info_ex_ptr);
          meta_info_ex_ptr = NULL; /* ps_iface_input will free this */
        ...
        {
          LOG_MSG_DATA_PROTO_INFO_0("Dropping mismatch IPv6 packet for interface type");
          dsm_free_packet(&rx_pkt);
          LLC_INC_INSTANCE_STATS(lle_instance, frms_drop_rx, 1);
        }
        break;
...
#else
      {
        LOG_MSG_DATA_PROTO_INFO_0("IPv6 not supported. Dropping packet");
        dsm_free_packet(&rx_pkt);
        LLC_INC_INSTANCE_STATS(lle_instance, frms_drop_rx, 1);
        break;
      }
      ...
      case LLC_ARP_FRM:
      {
        ...
        /*-------------------------------------------------------------------
          Input to ARP Module
        -------------------------------------------------------------------*/
        LOG_MSG_DATA_PROTO_INFO_0("LLC Module inputting ARP fram to ARP module");
        ...
        {
          LOG_MSG_DATA_PROTO_INFO_0("Dropping ARP frame on non IPv4 interface");
          dsm_free_packet(&rx_pkt);
          LLC_INC_INSTANCE_STATS(lle_instance, frms_drop_rx, 1);
        }
        break;
      ...
      case LLC_802_1X_FRM:
      {
        ...
        {
          LOG_MSG_DATA_PROTO_INFO_0( "LLC Module inputting 802 1X frame to 802.1X module");
      ...
      case LLC_802_1X_FRM_PREAUTH:
      {
        ...
        {
          LOG_MSG_DATA_PROTO_INFO_0( "LLC inputting 802 1X Preauth frame to LAN 1X module");
      ...
      case LLC_WAPI_FRM:
      {
        ...
        {
          LOG_MSG_DATA_PROTO_INFO_0( "LLC Module inputting WAPI frame to WAPI module");
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ( TRUE != ptr_lle_cb->started)
  {
    LOG_MSG_DATA_PROTO_INFO_1(" LLC instance %d is already stopped\n", lle_instance);
    PS_TX_META_INFO_FREE(&meta_info_ptr);
    dsm_free_packet(pkt_chain_ptr);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  -------------------------------------------------------------------------*/
  PS_LAN_LLCI_ASSERT( NULL != pkt_chain_ptr );
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ( rx_pkt_ptr == NULL )
  {
    LOG_MSG_DATA_PROTO_INFO("Zero payload ethernet frame."
  ...
  switch (frm_type)
  {
    case LLC_UNKNOWN_FRM:
    {
      /*-------------------------------------------------------------------
        Just drop frame for now
      -------------------------------------------------------------------*/
      LOG_MSG_DATA_PROTO_INFO("Receive unknown LLC frame, Frame will be dropped!", 0,0, 0);
    ...
    case LLC_CTRL_FRM:
    {
      /*-------------------------------------------------------------------
        LLC Control frames
      -------------------------------------------------------------------*/
      LOG_MSG_DATA_PROTO_INFO("Processing LLC Control frame", 0,0, 0);
    ...
    case LLC_IPV4_FRM:
    {
      ...
      if(!ps_iface_addr_family_is_v6(ptr_start_info->iface_ptr))
      {
        LOG_MSG_DATA_PROTO_INFO("Inputting IPv4 frame to PS IFACE", 0, 0, 0);
        (void) ps_iface_input(ptr_start_info->iface_ptr,
                              &rx_pkt_ptr,
                              meta_info_ex_ptr);
        meta_info_ex_ptr = NULL; /* ps_iface_input will free this */
      ...
      {
        LOG_MSG_DATA_PROTO_INFO("Dropping mismatched IPv4 packet for interface type",0,0,0);
        dsm_free_packet(&rx_pkt_ptr);
        LLC_INC_INSTANCE_STATS(lle_instance, frms_drop_rx, 1);
      }
      break;
  ...
  #ifdef FEATURE_DATA_PS_IPV6
    {
      ...
      if(ps_iface_addr_family_is_v6(ptr_start_info->iface_ptr))
      {
        LOG_MSG_DATA_PROTO_INFO("Inputting IPv6 frame to PS IFACE", 0, 0, 0);
        (void) ps_iface_input(ptr_start_info->iface_ptr,
                              &rx_pkt_ptr,
                              meta_info_ex_ptr);
        meta_info_ex_ptr = NULL; /* ps_iface_input will free this */
      ...
      {
        LOG_MSG_DATA_PROTO_INFO("Dropping mismatch IPv6 packet for interface type",0,0,0);
        dsm_free_packet(&rx_pkt_ptr);
        LLC_INC_INSTANCE_STATS(lle_instance, frms_drop_rx, 1);
      }
      break;
  ...
  #else
    {
      LOG_MSG_DATA_PROTO_INFO("IPv6 not supported. Dropping packet", 0, 0, 0);
      dsm_free_packet(&rx_pkt_ptr);
      LLC_INC_INSTANCE_STATS(lle_instance, frms_drop_rx, 1);
      break;
    }
    ...
    case LLC_ARP_FRM:
    {
      ...
      /*-------------------------------------------------------------------
        Input to ARP Module
      -------------------------------------------------------------------*/
      LOG_MSG_DATA_PROTO_INFO("LLC Module inputting ARP fram to ARP module", 0, 0, 0);
      arp_input(lle_instance,
                &rx_pkt_ptr,
                src_hw_addr,
                dst_hw_addr);
      ...
      {
        LOG_MSG_DATA_PROTO_INFO("Dropping ARP frame on non IPv4 interface",0,0,0);
        dsm_free_packet(&rx_pkt_ptr);
        LLC_INC_INSTANCE_STATS(lle_instance, frms_drop_rx, 1);
      }
      break;
    ...
    case LLC_802_1X_FRM:
    {
      ...
      if ( NULL != ptr_lle_cb->lan_1x_input_f_ptr )
      {
        LOG_MSG_DATA_PROTO_INFO( "LLC Module inputting 802 1X frame to 802.1X module",
                 0, 0, 0);
        ptr_lle_cb->lan_1x_input_f_ptr( lle_instance,
                                        &rx_pkt_ptr,
                                        src_hw_addr,
    ...
    case LLC_802_1X_FRM_PREAUTH:
    {
      ...
      if ( NULL != ptr_lle_cb->lan_1x_preauth_input_f_ptr )
      {
        LOG_MSG_DATA_PROTO_INFO( "LLC inputting 802 1X Preauth frame to LAN 1X module",
                 0, 0, 0);
        ptr_lle_cb->lan_1x_preauth_input_f_ptr( lle_instance,
                                                &rx_pkt_ptr,
                                                src_hw_addr,
    ...
    case LLC_WAPI_FRM:
    {
      ...
      if ( NULL != ptr_lle_cb->lan_wapi_input_f_ptr )
      {
        LOG_MSG_DATA_PROTO_INFO( "LLC Module inputting WAPI frame to WAPI module",
                 0, 0, 0);
        ptr_lle_cb->lan_wapi_input_f_ptr( lle_instance,
                                          &rx_pkt_ptr,
                                          src_hw_addr,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  PS_LAN_LLCI_ASSERT(NULL != src_hw_addr);
  PS_LAN_LLCI_ASSERT(NULL != dst_hw_addr);
  ...
  switch (ptr_start_info->lle_mode)
  {
    ...
    case LAN_LLC_MODE_1:
    {
      ...
      {
        LOG_MSG_DATA_PROTO_INFO_0("Receive probably an  internal LLC Ctrl frame");
        return LLC_CTRL_FRM;
      }
  ...
  -------------------------------------------------------------------------*/
  LOG_MSG_DATA_PROTO_INFO_1("Received %d Frame within an LLC Packet", ps_ntohs(ethertype));
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ( LAN_LLC_MODE_1 == lle_mode)
  {
    ...
    LOG_MSG_DATA_PROTO_INFO_0("Received an LLC control frame");
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  PS_LAN_LLCI_ASSERT ((NULL != meta_info_ptr) && (NULL!= *meta_info_ptr) &&
          (NULL != PS_TX_META_GET_RT_META_INFO_PTR(*meta_info_ptr)));
