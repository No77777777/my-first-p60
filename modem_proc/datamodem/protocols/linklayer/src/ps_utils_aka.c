...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/protocols/linklayer/src/ps_utils_aka.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
/*---------------------------------------------------------------------------
  LOG MMGSDI Errors
---------------------------------------------------------------------------*/
void ps_utils_aka_log_mmgsdi_error
(
   unsigned int line_num,
   mmgsdi_return_enum_type ret_val
)
{
  LOG_MSG_ERROR_2("Ps util AKA line %d mmgsdi failed ret val %d",
                  line_num, ret_val);
}
#define PS_UTIL_LOG_MMGSDI_ERR(r_val)  \
  ps_utils_aka_log_mmgsdi_error(__LINE__, r_val)

/*---------------------------------------------------------------------------
  LOG MCFG Errors
---------------------------------------------------------------------------*/
void ps_utils_aka_log_mcfg_error
(
   unsigned int line_num,
   int result
)
{
  LOG_MSG_ERROR_2("Ps util AKA line %d mcfg failed result %d",
                  line_num, result);
}
#define PS_UTIL_LOG_MCFG_ERR(result)  \
  ps_utils_aka_log_mcfg_error(__LINE__, result)
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    EAP_INPUT_ERROR_LOG_0();
    return -1;
  }
  
  /*-------------------------------------------------------------------------
    Create common directories if needed for isn file
  -------------------------------------------------------------------------*/
  EAP_MSG_DEBUG_0("ps_utils_aka_create_efs_file(): "
  ...
  if( 0 != result )
  {
    ...
    if ( -1 == result )
    {
      PS_UTIL_LOG_MCFG_ERR(mcfg_fs_errno(MCFG_FS_TYPE_EFS));
...
)
{
  ...
  int result;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  EAP_MSG_DEBUG_0("ps_utils_aka_store_isn(): "
                "EFS store sequence number");

  if (NULL == seqnum_arr || 
      PS_UTILS_AKA_SEQ_NUM_ARRAY_ELEMENTS != seqnum_arr_num_elements )
  {
    EAP_INPUT_ERROR_LOG_1(seqnum_arr_num_elements);
    return;
  }
...
)
{
  ...
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  EAP_MSG_DEBUG_0("ps_utils_aka_run_algo_in_sw() called");
  ...
  {
    EAP_INPUT_ERROR_LOG_1(aka_handle);
    return;
  }
  ...
  switch ( status )
  {
    ...
    case AKAALGO_SUCCESS:
    {
      ...
      else
      {
        ...
        if (FALSE == ret_val)
        {
          EAP_ERROR_DEBUG_0("ps_utils_aka_run_algo_in_sw(): "
                        "Creating HMAC_MD5 digest failed");
          ps_utils_aka_send_failure_to_client(aka_handle);
          return;
        }
      }
...
)
{
  ...
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  EAP_MSG_DEBUG_0("ps_utils_run_aka_algo() called");
  ...
  if ((rand_len > PS_UTILS_AKA_MAX_RAND_LEN)
      ...
      ((PS_UTILS_AKA_V1 != aka_ver) && (PS_UTILS_AKA_V2 != aka_ver)) )
  {
    EAP_INPUT_ERROR_LOG(rand_len,autn_len, aka_ver);
    return aka_handle;
  }
  ...
  if (TRUE == aka_in_sw)
  {
    ...
    EAP_MSG_DEBUG_0("ps_utils_run_aka_algo(): "
                  "Reading initial sequence number from EFS");  
    ...
    {
      EAP_ERROR_DEBUG_0("ps_utils_run_aka_algo(): "
...
)
{
  ...
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  EAP_MSG_DEBUG_0("ps_utils_aka_create_hmac() called");

  if ((NULL == data) || (NULL == key) || (NULL == output_data))
  {
    EAP_INPUT_ERROR_LOG(data, key, output_data);
    return FALSE;
  }
...
)
{
  ...
  {
    EAP_INPUT_ERROR_LOG_2(create_digest_ptr, 
                          hmac_md5_out_ptr);
    return FALSE;
  }
  ...
  {
    EAP_INPUT_ERROR_LOG(create_digest_ptr->res, 
  ...
  {
    EAP_INPUT_ERROR_LOG(create_digest_ptr->res_len, 
...
{
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  EAP_MSG_DEBUG_0("ps_utils_aka_send_failure_to_client() called");
...
)
{
  ...
  ps_utils_aka_sub_cmd_enum_type sub_cmd;
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  EAP_MSG_DEBUG_0("ps_utils_aka_cmd_hdlr() called");

  if (PS_UTILS_AKA_CMD != cmd_name)
  {
    EAP_INPUT_ERROR_LOG_1(cmd_name);
    return;
  }
  ...
  switch(sub_cmd)
  {
    ...
    case PS_UTILS_AKA_MMGSDI_CARD_INS_EVT:
      ...
      {
        PS_UTIL_LOG_MMGSDI_ERR(ret_val);
        return;
      }
      break;
    ...
    case PS_UTILS_AKA_MMGSDI_SESSION_REG:
      ...
      {
        EAP_MSG_DEBUG_0("ps_utils_aka_cmd_hdlr(): "
                      "Trying to re-open session for ISIM");
    ...
    default:
      EAP_INPUT_ERROR_LOG_1(ps_utils_aka_sub_cmd);
      break;
  }
...
)
{
  ...
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  EAP_MSG_DEBUG_0("ps_utils_aka_run_isim_algo_cb_task() called");
  ...
  if(MMGSDI_AUTS_FAIL == auth_info_ptr->response_header.mmgsdi_status)
  {
    if ( auth_info_ptr->auts_len <= PS_UTILS_AKA_MAX_AUTS )
    {
      ...
      {
        EAP_MEM_ERROR_LOG(aka_data_len);
        ps_utils_aka_send_failure_to_client (aka_handle);
        return;
      }
  ...
  if(MMGSDI_SUCCESS == auth_info_ptr->response_header.mmgsdi_status)
  {
    ...
    {
      EAP_INPUT_ERROR_LOG(auth_info_ptr->res_len , 
    ...
    else
    {
      ...
      if (FALSE == status)
      {
        EAP_ERROR_DEBUG_0("ps_utils_aka_run_isim_algo_cb_task(): "
                      "Creating HMAC_MD5 digest failed");
        ps_utils_aka_send_failure_to_client (aka_handle);
        return;
      }
    ...
    if( (aka_data_len - offset) >= ik_len )
    {
       ...
       /*-------------------------------------------------------------------
         if offset is used again below for aka_data_ptr then proper value is:
           offset += ik_len;
         Removed dead assignment since offset is not used below
       ---------------------------------------------------------------------*/   
    }
...
)
{
  ...
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  EAP_MSG_DEBUG_0("ps_utils_aka_run_isim_algo_cb() called");

  /*-----------------------------------------------------------------------
   Sanity Check
  -------------------------------------------------------------------------*/
  if ( NULL == cnf_ptr || MMGSDI_ISIM_AUTH_CNF != cnf ) 
  {
    EAP_INPUT_ERROR_LOG(cnf_ptr, cnf, status);
    return;
  }

  /*-----------------------------------------------------------------------
   log if MMGSDI AKA ISIM failed
  -------------------------------------------------------------------------*/
  if( MMGSDI_SUCCESS != status )
  {
    PS_UTIL_LOG_MCFG_ERR(status);
  }
  ...
  {
    EAP_INPUT_ERROR_LOG_2(cnf, status);
    return;
  }
  ...
  {
    EAP_INPUT_ERROR_LOG( isim_auth_cnf->res.data_len, 
...
)
{
  ...
  ps_utils_aka_create_digest_info   create_digest;
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  EAP_MSG_DEBUG_0("ps_utils_aka_run_usim_algo_cb_task() called");
  ...
  {
    PS_UTIL_LOG_MMGSDI_ERR(mmgsdi_status);
    ps_utils_aka_send_failure_to_client (aka_handle);
    return;
  }
  ...
  {
    EAP_INPUT_ERROR_LOG_2(auth_data_ptr,
                          usim_info_ptr->auth_response.data_len);
    ps_utils_aka_send_failure_to_client (aka_handle);
    return;
  }
  ...
  if(PS_UTILS_AKA_AUTH_3G_UMTS_SUCCESS == *(auth_data_ptr))
  {
    ...
    else
    {
      ...
      if (FALSE == status)
      {
        EAP_ERROR_DEBUG_0("ps_utils_aka_run_usim_algo_cb_task(): "
                      "Creating HMAC_MD5 digest failed");
        PS_SYSTEM_HEAP_MEM_FREE (auth_data_ptr);
        ps_utils_aka_send_failure_to_client (aka_handle);
        return;
...
)
{
  ...
  mmgsdi_response_header_type  *response_header = NULL;
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  EAP_MSG_DEBUG_0("ps_utils_aka_run_usim_algo_cb() called");

  /*-----------------------------------------------------------------------
   Sanity Check
  -------------------------------------------------------------------------*/
  if ( NULL == cnf_ptr || MMGSDI_USIM_AUTH_CNF != cnf) 
  {
    EAP_INPUT_ERROR_LOG_2(cnf_ptr,cnf);    
    return;
  }
      ...
      && (MMGSDI_SUCCESS == response_header->mmgsdi_status)))
  {
    ...
    {
      EAP_MEM_ERROR_LOG(usim_auth_cnf->auth_response.data_len);
      return;
    }
...
)
{
  ...
  uint8  offset = 0;
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  EAP_MSG_DEBUG_0("ps_utils_aka_run_algo_on_card() called");
  ...
  if ((NULL == aka_rand) || (NULL == autn)
      ...
      || (autn_len > PS_UTILS_AKA_MAX_AUTN_LEN))
  {
    EAP_INPUT_ERROR_LOG(aka_rand,autn,rand_len);
    return FALSE;
  }
  ...
  LOG_MSG_INFO1("ps_utils_aka_run_algo_on_card(): "
                "Running MMGSDI AKA Algo isim_present %d, active %d", 
                isim_present, 
                aka_cb.mmgsdi_info.session_info.session_active, 0);
...
)
{
  ...
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  EAP_MSG_DEBUG_0("ps_utils_aka_apps_cb() called");
...
)
{
  ...
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  EAP_MSG_DEBUG_0("ps_utils_aka_mmgsdi_session_open_cb() called");
  ...
  {
    EAP_INPUT_ERROR_LOG_2(ssn_open_ptr->response_header.response_type,
                          cnf);
    return;
  }
...
{
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  EAP_MSG_DEBUG_0("ps_utils_aka_mmgsdi_evt_cb() called");
  ...
  switch ( event->evt )
  {
    case MMGSDI_CARD_INSERTED_EVT:
      ...
      {
        LOG_MSG_ERROR_1("Ignoring card insert event for slot %d",
    ...
    default:
      EAP_INPUT_ERROR_LOG_1(event->evt);   
      break;
  }
...
{
  EAP_MSG_DEBUG_2("ps_utils_aka_mmgsdi_session_refresh_comp_cb(): "
...
)
{
  ...
  switch (event->evt)
  {
    ...
    case MMGSDI_REFRESH_EVT:
      ...
      else if ( MMGSDI_REFRESH_STAGE_WAIT_FOR_OK_TO_INIT == event->data.refresh.stage )
      {
        mmgsdi_session_ok_to_refresh( aka_cb.mmgsdi_info.session_info.session_id,
                                      TRUE,
                                      ps_utils_aka_mmgsdi_session_refresh_comp_cb,
                                      ( mmgsdi_client_data_type ) 0 );
      }
...
)
{
  ...
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  EAP_MSG_DEBUG_0("ps_utils_aka_mmgsdi_session_reg_isim() called");
...
)
{
  mmgsdi_return_enum_type         status;
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  EAP_MSG_DEBUG_0("ps_utils_aka_mmgsdi_session_reg_usim() called");
...
)
{
  ...
  EAP_MSG_DEBUG_0("ps_utils_aka_id_reg_cb(): "
                  "MMGSDI ID & Event Registration Callback");
...
)
{
  mmgsdi_return_enum_type      retval;
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  EAP_MSG_DEBUG_0("ps_utils_aka_reg_mmgsdi_client_id() called");
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    EAP_INPUT_ERROR_LOG(from,to,data_read);
    return -1;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    EAP_INPUT_ERROR_LOG_2(efs_db_ptr,aka_isn_info_ptr);
  ...
  *err_num = (sint15) E_FAILURE;
          ...
          != (ret_val = ps_efs_tokenizer(efs_db_ptr, &from, &to )))
  {
    ...
    else if(PS_EFS_TOKEN_PARSE_SUCCESS == ret_val)
    {
      ...
      {
        EAP_ERROR_DEBUG_0("ps_utils_aka_read_record_from_efs(): "
                      "Error in reading. Abort");
        *err_num = (sint15) E_FAILURE;
        return -1;
      }
      else
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
                                        ...
                                        aka_isn_info_ptr ) == -1)
  {
    ...
    {
      EAP_MSG_DEBUG_1("ps_utils_aka_read_isn_from_efs(): "
    ...
    {
      EAP_MSG_DEBUG_0("ps_utils_aka_read_isn_from_efs(): "
                    "EOF reached reading record from EFS");
