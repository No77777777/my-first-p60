...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/protocols/linklayer/src/ps_ppp.c#2 $
...
02/07/17    fn     Fixes for ehrpd on sub 2 
06/23/15    ds     Post VSNCP up immediately if it is up already at reg
12/01/14    skc    PPP_DISABLE_SIG() removed
...
static ppp_dev_const_s_type ppp_def_dev_const[PPP_MAX_SUBS_ID][PPP_MAX_DEV];
ppp_ccp_config_e_type ccp_config[PPP_MAX_SUBS_ID]; 
...
/*===========================================================================
FUNCTION PPP_GET_SUBS_ID()

DESCRIPTION
  This function translates system specific subs ID to PPP enum.

PARAMETERS
  subs_id: system subs ID value

RETURN VALUE
  PPP subs id enum value.
...
SIDE EFFECTS
  None
===========================================================================*/
ppp_subs_id_enum_type ppp_get_subs_id
(
...
)
{
  switch(subs_id)
  {
     case SYS_MODEM_AS_ID_1:
       return PPP_SUBS_ID_1;
       break;
     case SYS_MODEM_AS_ID_2:
       return PPP_SUBS_ID_2;
       break;
     default:
       return PPP_INVALID_SUBS_ID;
       break;
  }
}

/*===========================================================================
FUNCTION PPP_GET_SYS_SUBS_ID()

DESCRIPTION
  This function translates PPP subs ID to System specific SUBS ID.

PARAMETERS
  subs_id: PPP subs ID value
...
SIDE EFFECTS
  None
===========================================================================*/
sys_modem_as_id_e_type ppp_get_sys_subs_id
(
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /*-------------------------------------------------------------------------
    Set default SUBS ID.
  -------------------------------------------------------------------------*/
  config_ptr->subs_id = PPP_SUBS_ID_1;
...
)
{
  ...
  {
    /*Invalid arguments passed: device, config_ptr*/
    PS_PPP_ASSERT(0);
    return -1;
  }
...
#ifdef FEATURE_DATA_PS_EHRPD
  /*-------------------------------------------------------------------------
    Do the above for VSNCP as well if registering for UP event for the first
    time
  -------------------------------------------------------------------------*/
  if( ( PPP_PROTOCOL_VSNCP == protocol ) &&
      !( (uint16)PPP_UP_EV_MASK & (uint16)event_info_ptr->event_mask[protocol] ) && 
      ( (uint16)PPP_UP_EV_MASK & (uint16)event_mask ) )
  {
    /*-----------------------------------------------------------------------
      Make sure VSNCP instance is inited and UP
    -----------------------------------------------------------------------*/
    ncp_handle_ptr = (ncpi_cb_type*)event_info_ptr->ncp_handle_ptr;
    fsm_ptr = &ncp_handle_ptr->cb_u.vsncp_3gpp2.fsm;
    if( NCPI_CB_IS_INITED( ncp_handle_ptr ) &&
        VSNCP_3GPP2I_FSM_IS_VALID( fsm_ptr ) &&
        fsmOPENED == fsm_ptr->state )
    {
      /*---------------------------------------------------------------------
        Post event only for following conditions
        - this is a v4-only PDN and v4 is UP
        - this is a v6-only PDN and v6 is UP
        - this is a v4v6 PDN and both v4 and v6 are UP
      ---------------------------------------------------------------------*/
      ...
      if( ( ( lwantp->req_addr_type == VSNCP_3GPP2_PDN_TYPE_IPV4 ) && 
          ...
          ( ( lwantp->req_addr_type == VSNCP_3GPP2_PDN_TYPE_IPV4_IPV6 ) && 
            ...
            ( lworkp->iid != 0 ) ) )
      {
        LOG_MSG_INFO1_2("event_register: posting VSNCP UP, instance %p, req_addr_type %d", 
                        ncp_handle_ptr, lwantp->req_addr_type);
        event_info_ptr->cback[protocol]( event_info_ptr->device,
                                         protocol,
                                         PPP_UP_EV,
                                         event_info_ptr->user_data[protocol],
...
)
{
  ...
  if( device >= PPP_MAX_DEV || 
      config_ptr == NULL    || 
      config_ptr->subs_id >= PPP_MAX_SUBS_ID ||
      ( ( config_ptr->bridge_dev < PPP_MAX_DEV ) &&  
        ( config_ptr->bridge_dev == device ) ) )
  {
    /*("Invalid params to ppp_device_configure")*/
    PS_PPP_ASSERT(0);
    return -1;
  }
  ...
  if( NULL != ppp_device_config[device])
  {
    ...
    DATA_PROTO_ERROR_LOG_1(PS_BAD_ARG_OR_NULL_PTR,ppp_device_config[device]);
    return -1;
  }
  ...
  if( config_ptr->lcp_mode == PPP_CP_MODE_DISABLED ||
      ...
      config_ptr->hdlc_mode >= HDLC_INVALID_MODE )
  {
    /*"Invalid protocol mode passed"*/
    DATA_PROTO_ERROR_LOG_2(PS_BAD_ARG_OR_NULL_PTR,
                           config_ptr->lcp_mode,config_ptr->hdlc_mode);
    return -1;
  }
  ...
  {
    /* Invalid auth mode <config_ptr->auth_role> passed  */
    DATA_PROTO_ERROR_LOG_1(PS_BAD_ARG_OR_NULL_PTR,config_ptr->auth_role);
    return -1;  
  }
  ...
  {
    /* No auth protocol specified! */
    DATA_PROTO_ERROR_LOG_2(PS_BAD_ARG_OR_NULL_PTR,
                           config_ptr->auth_role,config_ptr->auth_proto_mask);
    return -1;
  }
  ...
  {
    /* Invalid Auth configuration param(s)! */
    DATA_PROTO_ERROR_LOG_3(PS_BAD_ARG_OR_NULL_PTR,
                        config_ptr->auth_info.user_id_len,
                        config_ptr->auth_info.passwd_len,
                        config_ptr->auth_info.challenge_len);
      ...
      ( config_ptr->rx_sig_f_ptr == NULL ) )
  {
    ...
    DATA_PROTO_ERROR_LOG_2(PS_BAD_ARG_OR_NULL_PTR,
                           config_ptr->tx_f_ptr,config_ptr->rx_sig_f_ptr);
    return -1;
  }
  ...
  if (ds707_is_pkt_classifier_enabled()) 
  {
    ...
    LOG_MSG_DATA_PROTO_INFO_0("Pakcet Classifier: Allow NO_AUTH");
  }
#endif /* FEATURE_DATA_PKT_CLASSIFIER */
  ...
  if(NULL == ppp_device_config[device])
  {
    /* Memory allocation of ppp_device_config failed */
    DATA_PROTO_ERROR_LOG_0(PS_MEM_ALLOC_ERR);
    return -1; 
  }
...
)
{
  ...
  LOG_MSG_DATA_PROTO_INFO_1("ppp_device_unconfigure(dev %d)", device);
  if(device >= PPP_MAX_DEV)
  {
    PS_PPP_ASSERT(0);
    return -1;
  ...
  else
  {
    ...
    LOG_MSG_DATA_PROTO_INFO_2("Freed ppp_device_config %p for device %d",
...
)
{
  ...
  if( device >= PPP_MAX_DEV )
  {
    /*("Invalid params to ppp_device_reset")*/
    PS_PPP_ASSERT(0);
    return -1;
  }
...
)
{
  ...
  if( device >= PPP_MAX_DEV )
  {
    /*("Invalid device passed to ppp_is_device_configured")*/
    PS_PPP_ASSERT(0);
    return FALSE;
  }
...
)
{
  ...
  LOG_MSG_DATA_PROTO_INFO_1("ppp_device_stop(dev %d)", device);
  if(device >= PPP_MAX_DEV)
  {
    PS_PPP_ASSERT(0);
    return -1;
...
)
{
  ...
  LOG_MSG_DATA_PROTO_INFO_1("ppp_start(dev %d)", device);
  if(device >= PPP_MAX_DEV || NULL == config_ptr)
  {
    PS_PPP_ASSERT(0);
    return -1;
  ...
  -----------------------------------------------------------------------*/
  subs_id = config_ptr->subs_id;
  ppp_cb_ptr->subs_id = subs_id;
  LOG_MSG_DATA_PROTO_INFO_3( "dev %d, ppp_cb_ptr %x, subs id %d", 
                              device, 
                              ppp_cb_ptr, 
                              ppp_cb_ptr->subs_id );

  LOG_MSG_INFO1_1("PPP SUBS ID %d", ppp_cb_ptr->subs_id);

  /*-----------------------------------------------------------------------
    Initialize protocol dependent constants for SUBS ID
  -----------------------------------------------------------------------*/
  ppp_cb_array->fsm[LCP].pdc    = lcp_get_device_constants(device, subs_id);
  ppp_cb_array->fsm[IPCP].pdc   = ipcp_get_device_constants(device, subs_id);
  ppp_cb_array->fsm[IPV6CP].pdc = ipv6cp_get_device_constants(device, subs_id);

  /*-----------------------------------------------------------------------
    Initialize auth constants for SUBS ID
  -----------------------------------------------------------------------*/
  ppp_auth_init_device_constant(device, 
                                AUTH_TIMEOUT, 
                                ppp_auth_get_constant(AUTH_TIMEOUT,
                                                      subs_id));

  ppp_auth_init_device_constant(device, 
                                AUTH_RETRY, 
                                ppp_auth_get_constant(AUTH_RETRY,
                                                      subs_id));

  ppp_auth_init_device_constant(device, 
                                AUTH_REQUIRE_ENCRYPTED_PASSWORD, 
                                ppp_auth_get_constant(AUTH_REQUIRE_ENCRYPTED_PASSWORD,
                                                      subs_id));
  ppp_get_device_constant(device,
                          LCP_CCP_CONFIG,
                          &value,
                          subs_id);

  ppp_cb_array[device].ccp_config = (ppp_ccp_config_e_type)(value); 
...
)
{
  ...
  {
    /*ppp_resync on free device<device> */
      ...
      pppi_config_check(config_ptr) < 0))
  {
    ...
    DATA_PROTO_ERROR_LOG_2(PS_BAD_ARG_OR_NULL_PTR,
                           config_ptr,config_ptr->lcp_info.mode); 
    return -1;
  } /* if(config_ptr != NULL) */
...
)
{
  ...
  LOG_MSG_DATA_PROTO_INFO_1("ppp_abort(dev %d)", device);
  if(device >= PPP_MAX_DEV)
  {
    PS_PPP_ASSERT(0);
    return -1;
...
)
{
  ...
  LOG_MSG_INFO2_4("ppp_set_mode(%d) session_handle(%d)"
                  " device(%d) ppp_cb_ptr->mode(%d)", 
                    new_mode,ppp_cb_ptr->session_handle,
                    device,ppp_cb_ptr->mode);

  if(ppp_cb_ptr->session_handle == 0)
  {
    LOG_MSG_DATA_PROTO_INFO_2("ppp_set_mode(%d) on free device(%d)!", 
                               new_mode, device);
  }
  else
  {
    LOG_MSG_DATA_PROTO_INFO_3("dev %d mode from %d to %d",
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  {
    /*("Invalid args: device, failover_value")*/
    PS_PPP_ASSERT(0);
    return -1;
  }
...
)
{
  ...
  {
    /*Invalid params passed to ppp_update_password(dev <device>)*/
    DATA_PROTO_ERROR_LOG_2(PS_BAD_ARG_OR_NULL_PTR,password,password_len);
    return -1;
  }
...
)
{
  ...
  {
    /*Invalid params passed to ppp_update_user_id(dev <device>)*/
    DATA_PROTO_ERROR_LOG_3(PS_BAD_ARG_OR_NULL_PTR,user_id,user_id_len,device);
    return -1;
  }
...
)
{
  ...
  ppp_cb_array = fsm_get_ppp_cb_array();
  PS_PPP_ASSERT( ppp_cb_array[device].hdlc_handle != NULL );
...
)   
{
  ...
  {
    /*( "ppp_set_device_constant: Invalid args: device,"
                     " constant")*/
    PS_PPP_ASSERT(0);
    return -1;
  }
  ...
  switch(constant)
  {
    ...
    case LCP_REQUEST_TRY: 
       ppp_lcp_init_device_constant(device, constant, value, subs_id);
       break;

    case LCP_CCP_CONFIG: 
        ccp_config[subs_id] = value;
        break;

    case IPCP_ACK_TIMEOUT: 
    case IPCP_REQUEST_TRY: 
      ppp_ipcp_init_device_constant(device, constant, value, subs_id);
      break;
    ...
    case IPV6CP_REQUEST_TRY: 
      ppp_ipv6cp_init_device_constant(device, constant, value, subs_id);
      break;
#endif /* FEATURE_DATA_PS_IPV6 */
    case DEFAULT_LCP_ACK_TIMEOUT: 
       ppp_def_dev_const[subs_id][device].lcp_ack_timeout = (uint16)value;
       break;
    case DEFAULT_LCP_REQUEST_TRY: 
       ppp_def_dev_const[subs_id][device].lcp_try_configure = (uint8)value;
       break;
    case DEFAULT_IPCP_ACK_TIMEOUT: 
       ppp_def_dev_const[subs_id][device].ipcp_ack_timeout = (uint16)value;
       break;
    case DEFAULT_IPCP_REQUEST_TRY: 
       ppp_def_dev_const[subs_id][device].ipcp_try_configure = (uint8)value;
       break;
    case DEFAULT_AUTH_TIMEOUT:
       ppp_def_dev_const[subs_id][device].auth_timeout = (uint16)value;
       break;
    case DEFAULT_AUTH_RETRY: 
       ppp_def_dev_const[subs_id][device].auth_retry_limit = (uint8)value;
       break;
#ifdef FEATURE_DATA_PS_IPV6
    case DEFAULT_IPV6CP_ACK_TIMEOUT: 
       ppp_def_dev_const[subs_id][device].ipv6cp_ack_timeout = (uint16)value;
       break;
    case DEFAULT_IPV6CP_REQUEST_TRY: 
       ppp_def_dev_const[subs_id][device].ipv6cp_try_configure = (uint8)value;
       break;
#endif /* FEATURE_DATA_PS_IPV6 */
#ifdef FEATURE_DATA_PS_EHRPD
    ...
    case VSNCP_3GPP2_TERM_TIMEOUT:
       /*--------------------------------------------------------------------
        ...
        -------------------------------------------------------------------*/
       vsncp_3gpp2_set_constant( constant, value, subs_id );
       break;
#endif /* FEATURE_DATA_PS_EHRPD */
    default:
      LOG_MSG_DATA_PROTO_INFO_1("ppp_set_device_constant: Unknown constant %d",
                      constant);
      return -1;
  }
  return 0;
...
)   
{
  ...
  if( device >= PPP_MAX_DEV || constant >= PPP_MAX_INIT_CONSTS ||
      value  == NULL )
  {
    /*( "ppp_get_device_constant: Invalid args: device,"
               " constant, value")*/
    PS_PPP_ASSERT(0);
    return -1;
  }
  ...
  {
    case LCP_CCP_CONFIG: 
       *value = ccp_config[subs_id];
       LOG_MSG_INFO1_1("PPP CCP CONFIG %d", *value);
       break;
    case DEFAULT_LCP_ACK_TIMEOUT: 
       *value = ppp_def_dev_const[subs_id][device].lcp_ack_timeout;
       break;
    case DEFAULT_LCP_REQUEST_TRY: 
       *value = ppp_def_dev_const[subs_id][device].lcp_try_configure;
       break;
    case DEFAULT_IPCP_ACK_TIMEOUT: 
       *value = ppp_def_dev_const[subs_id][device].ipcp_ack_timeout;
       break;
    case DEFAULT_IPCP_REQUEST_TRY: 
       *value = ppp_def_dev_const[subs_id][device].ipcp_try_configure;
       break;
    case DEFAULT_AUTH_TIMEOUT:
       *value = ppp_def_dev_const[subs_id][device].auth_timeout;
       break;
    case DEFAULT_AUTH_RETRY: 
       *value = ppp_def_dev_const[subs_id][device].auth_retry_limit;
       break;
                          
#ifdef FEATURE_DATA_PS_IPV6
    case DEFAULT_IPV6CP_ACK_TIMEOUT: 
       *value = ppp_def_dev_const[subs_id][device].ipv6cp_ack_timeout;
       break;
    case DEFAULT_IPV6CP_REQUEST_TRY: 
       *value = ppp_def_dev_const[subs_id][device].ipv6cp_try_configure;
       break;
#endif /* FEATURE_DATA_PS_IPV6 */

    default:
      LOG_MSG_DATA_PROTO_INFO_1("ppp_get_device_constant: Unsupported constant %d",
                      constant);
      return -1;
  }
  return 0;
...
)
{
  ...
  if( config_ptr->tx_f_ptr == NULL )
  {
    /*"NULL TX function passed"*/
    DATA_PROTO_ERROR_LOG_1(PS_BAD_ARG_OR_NULL_PTR,config_ptr->tx_f_ptr);
    return -1;
  }
...
#endif /* FEATURE_DATA_PS_IPV6 */
  {
    /*Invalid protocol mode or PS iface passed*/
    DATA_PROTO_ERROR_LOG_3(PS_BAD_ARG_OR_NULL_PTR,
                        config_ptr->lcp_info.mode,
                        config_ptr->hdlc_mode,
                        config_ptr->ipcp_info.rx_iface_ptr);
    return -1;
  }
...
#endif /* FEATURE_DATA_PS_IPV6 */
  {
    /*No NCP is in Active or Passive mode*/
    DATA_PROTO_ERROR_LOG_2(PS_BAD_ARG_OR_NULL_PTR,
  ...
  {
    /*Invalid LCP configuration param(s)*/
    DATA_PROTO_ERROR_LOG_3(PS_BAD_ARG_OR_NULL_PTR,
                      config_ptr->lcp_info.want_mask[FSM_LOCAL]  & ~lcp_accept,
                      config_ptr->lcp_info.will_mask[FSM_LOCAL]  & ~lcp_accept,
                      config_ptr->lcp_info.want_mask[FSM_REMOTE] & ~lcp_accept);
    DATA_PROTO_ERROR_LOG_1(PS_BAD_ARG_OR_NULL_PTR,
                      config_ptr->lcp_info.will_mask[FSM_REMOTE] & ~lcp_accept);
    return -1;
  }
      ...
      (config_ptr->lcp_info.will_mask[FSM_REMOTE] & LCP_N_MAGIC)  == 0 )
  {
    /*Invalid LCP configuration param(s)!*/
    DATA_PROTO_ERROR_LOG_2(PS_BAD_ARG_OR_NULL_PTR,
                      config_ptr->lcp_info.want_mask[FSM_LOCAL] & LCP_N_MAGIC,
                      config_ptr->lcp_info.will_mask[FSM_REMOTE] & LCP_N_MAGIC);
    return -1;
  }
  ...
  {
    /*Invalid IPCP configuration param(s)*/
    DATA_PROTO_ERROR_LOG_3(PS_BAD_ARG_OR_NULL_PTR,
                        config_ptr->ipcp_info.mode,
                        config_ptr->ipcp_info.rx_iface_ptr,
                        config_ptr->ipcp_info.want_mask[FSM_LOCAL]  & ~ipcp_accept);
    DATA_PROTO_ERROR_LOG_3(PS_BAD_ARG_OR_NULL_PTR,
                        config_ptr->ipcp_info.will_mask[FSM_LOCAL]  & ~ipcp_accept,
                        config_ptr->ipcp_info.want_mask[FSM_REMOTE] & ~ipcp_accept,
                        config_ptr->ipcp_info.will_mask[FSM_REMOTE] & ~ipcp_accept);
    return -1;
  }
  ...
  if( config_ptr->ipv6cp_info.mode != PPP_CP_MODE_INVALID &&
      (config_ptr->ipv6cp_info.rx_iface_ptr == NULL ||
       ...
       (config_ptr->ipv6cp_info.will_mask[FSM_REMOTE] & ~ipv6cp_accept)
                                                                    != 0) )
  {
    /*Invalid IPv6CP configuration param(s)!*/
    DATA_PROTO_ERROR_LOG_3(PS_BAD_ARG_OR_NULL_PTR,
                        config_ptr->ipv6cp_info.mode,
                        config_ptr->ipv6cp_info.rx_iface_ptr,
                        config_ptr->ipv6cp_info.want_mask[FSM_LOCAL]  & ~ipv6cp_accept);
        ...
        (uint32)PPP_AUTH_EAP) )
  {
    ...
    {
      LOG_MSG_DATA_PROTO_INFO_0("No need to validate the EAP parameters from the card since"
                      " USIM is present");
      return 0;
    }
    ...
    {
      /*Invalid EAP configuration parameter*/
      DATA_PROTO_ERROR_LOG_1(PS_BAD_ARG_OR_NULL_PTR,
                        config_ptr->auth_info.aka_algo_seqnum_arr_num_elements);
      return -1;
    }
  }
  return 0;
...
)
{
  ...
  if( ( device >= PPP_MAX_DEV ) || 
      ( lsd_config_ptr == NULL ) )
  {
    /*( "Invalid args dev, config passed to trigger lsd")*/
    PS_PPP_ASSERT(0);
    return -1;
  }
      ...
      ( lsd_config_ptr->result_cback == NULL ) )
  {
    /*"Invalid configuration!"*/
    DATA_PROTO_ERROR_LOG_3(PS_BAD_ARG_OR_NULL_PTR,
                        lsd_config_ptr->echo_req_attempts,
                        lsd_config_ptr->echo_req_timeout,
                        lsd_config_ptr->result_cback);
    return -1;
  } 
  ...
  {
    /*"LCP not open: Cannot trigger LSD"*/
    DATA_PROTO_ERROR_LOG_1(PS_BAD_ARG_OR_NULL_PTR,ppp_cb_ptr->fsm[LCP].state);
    PS_BRANCH_LEAVE_CRIT_SECTION( pppi_get_crit_sect_lock(device) );
    return -1;
  }
  PS_LEAVE_CRIT_SECTION( pppi_get_crit_sect_lock(device) );
  ...
  {
    /*("No memory to service PPP LSD command!")*/
    PS_PPP_ASSERT(0);
    return -1;
  }
  else
...
/*===========================================================================
FUNCTION ps_ppp_qsh_mdump_collect
  
DESCRIPTION
  This function collects the mdump for ppp_def_dev_const variable
...
===========================================================================*/
QSH_MDUMP_FN_ATTR 
void ps_ppp_qsh_mdump_collect()
{
  qsh_mdump_collect_high(&ppp_def_dev_const, PPP_MAX_SUBS_ID * PPP_MAX_DEV * sizeof(ppp_dev_const_s_type));
}
