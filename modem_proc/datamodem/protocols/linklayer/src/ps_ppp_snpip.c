...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/protocols/linklayer/src/ps_ppp_snpip.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $


when        who    what, where, why
--------    ---    ---------------------------------------------------------- 
11/01/17    ak     CR 2125724: FR 42790: F3/Debug Messages Reduction
10/30/15    kc     Initialized event array before unsubscribe_event() 
12/19/14    kc     Added new changes for FR21288
12/05/14    kc     Added changes for FR21288
19/11/14    vl     Added de-initialization function for QTF memory leak
...
{
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ps_iface_event_enum_type iface_ipfltr_event_reg_array[1]; 
      ...
      switch(snpipi_cb_arr[dev].state)
      {
        ...
        case SNOOP_IP_NONCOMP_STATE:
         ...
         case SNOOP_IP_NONE_STATE:
          ...
          {
             iface_ipfltr_event_reg_array[0] = IFACE_IPFLTR_UPDATED_EV; 
             if(-1 == ps_iface_unsubscribe_event_list(snpipi_cb_arr[dev].ev_buf_handle,
                                                      iface_ipfltr_event_reg_array,
                                                      1))
             {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_DATA_PROTO_INFO_1("Recvd PPP_DEV_UP_EV, state %d", snpipi_cb_arr[dev].state);
  ...
  switch(snpipi_cb_arr[dev].state)
  {
    case SNOOP_IP_NULL_STATE:
      ...
      /* Allocate event handle for PPP device */
      if(NULL == snpipi_cb_arr[dev].ev_buf_handle)
      {
        if(NULL == (snpipi_cb_arr[dev].ev_buf_handle = 
          (void*)ps_iface_alloc_event_handle(snpipi_cb_arr[dev].if_ptr,
                                             snoop_ipi_iface_ev_handler_cb,
                                             (void *)dev)))
        {
          /*("Snoop IP event buf alloc failed");*/
          PS_PPP_SNPIP_ASSERT(0);
          return;
        }
      }
    ...
    case SNOOP_IP_ALL_STATE:
      ...
      ---------------------------------------------------------------------*/
      LOG_MSG_DATA_PROTO_INFO_1("Ignoring PPP_DEV_UP_EV, state %d",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_DATA_PROTO_INFO_1("Recvd PPP_DEV_DOWN_EV, state %d", snpipi_cb_arr[dev].state);
  ...
  switch(snpipi_cb_arr[dev].state)
  {
    ...
    case SNOOP_IP_NULL_STATE:
      /*---------------------------------------------------------------------
        Ignoring this event in all other states
      ---------------------------------------------------------------------*/
      LOG_MSG_DATA_PROTO_INFO_0("Ignoring PPP_DEV_DOWN_EV");
      break;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_DATA_PROTO_INFO_1("Recvd IP_FLTR_ENABLED_EV, state %d",
                  snpipi_cb_arr[dev].state);
  ...
  switch(snpipi_cb_arr[dev].state)
  {
    ...
    case SNOOP_IP_ALL_STATE:
      /*---------------------------------------------------------------------
        Ignoring this event in all other states
      ---------------------------------------------------------------------*/
      LOG_MSG_DATA_PROTO_INFO_1("Ignoring IP_FLTR_ENABLED_EV, state %d",
                      snpipi_cb_arr[dev].state );
      break;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_DATA_PROTO_INFO_1("Recvd IP_FLTR_DISABLED_EV,state %d",
                  snpipi_cb_arr[dev].state);
  ...
  switch(snpipi_cb_arr[dev].state)
  {
    ...
    case SNOOP_IP_NONE_STATE:
      /*---------------------------------------------------------------------
        Ignoring this event in all other states
      ---------------------------------------------------------------------*/
      LOG_MSG_DATA_PROTO_INFO_1("Ignoring IP_FLTR_DISABLED_EV, state %d",
                      snpipi_cb_arr[dev].state );
      break;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_DATA_PROTO_INFO_1("Recvd IP_COMP_ON_EV, state %d", snpipi_cb_arr[dev].state);
  ...
  switch(snpipi_cb_arr[dev].state)
  {
    ...
    case SNOOP_IP_ALL_STATE:
      /*---------------------------------------------------------------------
        Ignoring this event in all other states
      ---------------------------------------------------------------------*/
      LOG_MSG_DATA_PROTO_INFO_1("Ignoring IP_COMP_ON_EV, state %d",
                      snpipi_cb_arr[dev].state );
      break;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_DATA_PROTO_INFO_1("Recvd IP_COMP_OFF_EV, state %d", snpipi_cb_arr[dev].state);
  ...
  switch(snpipi_cb_arr[dev].state)
  {
    ...
    case SNOOP_IP_NONCOMP_STATE:
      /*---------------------------------------------------------------------
        Ignoring this event in all other states
      ---------------------------------------------------------------------*/
      LOG_MSG_DATA_PROTO_INFO_1("Ignoring IP_COMP_OFF_EV, state %d",
                      snpipi_cb_arr[dev].state );
      break;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(dev >= PPP_MAX_DEV)
  {
    /*("snoop_ipi_iface_ev_handler_cb: Invalid device");*/
    PS_PPP_SNPIP_ASSERT(0);
    return;
  }

  if (this_iface_ptr != snpipi_cb_arr[dev].if_ptr)
  {
    LOG_MSG_ERROR_2("snoop_ipi_iface_ev_handler_cb: iface mismatch 0x%x != 0x%x", 
                     this_iface_ptr,
                     snpipi_cb_arr[dev].if_ptr);  
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (inited == TRUE)
  {
    LOG_MSG_DATA_PROTO_INFO_0("Snoop IP SM already inited");
    return;
  }
