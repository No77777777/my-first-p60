...
$Header: //components/rel/data.mpss/3.5..2.1/protocols/linklayer/src/ps_arpi.c#2 $
...
#define PS_ARPI_ASSERT(expression)  \
          ps_arpi_assert_wrapper(__LINE__, expression)
...
FUNCTION ps_arpi_assert_wrapper

DESCRIPTION
  Wrapper function for PS_ARPI ASSERT
...
=============================================================================*/
static void ps_arpi_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("PS_ARPI_ERR_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
  
}/* ps_arpi_assert_wrapper */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  PS_ARPI_ASSERT( NULL != arp_pkt);

  PS_ARPI_ASSERT( NULL != arp_body);
  ...
  if ( FALSE == good_frm)
  {
    LOG_MSG_DATA_PROTO_INFO_0("Unknown ARP header received within ARP frame");
  ...
  if ( TRUE == good_frm )
  {
    ...
    case ARP_OP_REQUEST:
      {
        LOG_MSG_DATA_PROTO_INFO_0("Receive an ARP Request frame");
  ...
  /*-------------------------------------------------------------------------
    Frame is bad
  -------------------------------------------------------------------------*/
  LOG_MSG_DATA_PROTO_INFO_0("Receive an ARP frame with bad body content");
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  PS_ARPI_ASSERT(NULL != ptr_arp_entry);

  PS_ARPI_ASSERT(NULL != dst_hw_addr);
  ...
  {
    /* ARP control block pointer is NULL or its iface pointer is NULL */
    PS_ARPI_ASSERT(0);
    return -1;
  }
                                          ...
                                          req_duration)) )
  {
    ...
    -----------------------------------------------------------------------*/
    /* Impossible to start ARP Request timer */
    PS_ARPI_ASSERT(0);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_DATA_PROTO_INFO_1("PS Timer %d not running", timer_handle);
  }
} /* arpi_stop_timer() */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    /* arpi_entry_timeout(): Invalid LLE */
    PS_ARPI_ASSERT(0);
    return ;
  }
  ...
  if ( 0 < ptr_arp_entry->nb_retries)
  {
    ...
    else
    {
      ...
      LOG_MSG_DATA_PROTO_INFO_1("ARP request retransmitted for entry of IP (0x%x)",
      ...
      {
        PS_ARPI_ASSERT( ARP_STATUS_PROBING & ptr_arp_entry->status );
  ...
  if ( ptr_arp_entry->status & ARP_STATUS_RES_PEND)
  {
    ...
    LOG_MSG_DATA_PROTO_INFO_1("Failed to receive ARP Response after maximum retries"
    ...
    PS_ARPI_ASSERT(NULL != ptr_arp_entry->pending_pkt);
    PS_ARPI_ASSERT(NULL != ptr_arp_entry->pending_pkt->app_ptr);
  ...
  if ( ptr_arp_entry->status & ARP_STATUS_PROBING)
  {
    /*-----------------------------------------------------------------------
      Invoke Call back
    -----------------------------------------------------------------------*/
    LOG_MSG_DATA_PROTO_INFO_1("ARP Probe failed for IP(0x%x)! Notifying upper layer",
                    ps_ntohl(ptr_arp_entry->ip_addr));
  ...
  if ( ptr_arp_entry->status & ARP_STATUS_CHECKING)
  {
    ...
    -----------------------------------------------------------------------*/
    LOG_MSG_DATA_PROTO_INFO_1("Peer node of IP(0x%x) failed to repond to Unicast ARP request"
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_DATA_PROTO_INFO_0("ARP CMGR properly initialized");
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    /* Attempt to deactivate a free ARP entry */
    PS_ARPI_ASSERT(0);
    return;
  }

  LOG_MSG_DATA_PROTO_INFO_1("Deallocating ARP entry (0x%x)", ptr_arp_entry);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  for (loop =0; loop < ARP_CMGR_MAX_NUM_ENTRIES; loop +=1)
  {
    ...
    if (TRUE == ptr_entry_env->is_free)
    {
      LOG_MSG_DATA_PROTO_INFO_0("Allocating new ARP entry");
      ...
      {
        /* Failed to allocate PS timer for ARP entry */
        PS_ARPI_ASSERT(0);
        return NULL;
      }
  ...
  if (TRUE == force)
  {
    ...
    if ( NULL != ptr_arp_entry)
    {
      ...
      /*---------------------------------------------------------------------
        ...
        There should be a pending packet...
      ---------------------------------------------------------------------*/
      LOG_MSG_DATA_PROTO_INFO_1("Freeing waiting packet (next hop IP 0x%x)",
                      ps_ntohl(ptr_arp_entry->ip_addr));
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ( TRUE == restart_timer)
  {
    ...
    {
      /* Failed to start ARP Cache Manager timer */
      PS_ARPI_ASSERT(0);
      return;
    }
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  for ( loop = 0; loop < ARP_CMGR_MAX_NUM_ENTRIES; loop +=1)
  {
    ...
    if ( FALSE == ptr_entry_env->is_free)
    {
      ...
      {
        LOG_MSG_DATA_PROTO_INFO_1("Found in cache entry of IP (0x%x)", ip_addr);
        return ptr_arp_entry;
      }
    }
  } // end loop for
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ( FALSE == cmgr_timer_allocated)
  {
    ...
    {
      /* Failed to allocate ARP Cache Manager timer */
      PS_ARPI_ASSERT(0);
      return;
    }
  ...
  {
    LOG_MSG_DATA_PROTO_INFO_0("Starting Cache Manager timer for cache refresh");
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  PS_ARPI_ASSERT(NULL != src_hw_addr);
  PS_ARPI_ASSERT(NULL != target_hw_addr);
...
)
{
  ...
  PS_ARPI_ASSERT(data == arpi_cmgr_refresh_duration);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    PS_ARPI_ASSERT(0 < ptr_arp_entry->max_random_incr);
