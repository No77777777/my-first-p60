...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/protocols/linklayer/src/ps_ppp_fsm.c#3 $
  $Author: pwbldsvc $ $DateTime: 2020/08/07 04:41:29 $

when        who    what, where, why
--------    ---    ----------------------------------------------------------
08/07/20    sg     Code changes to correct the F3 message to log the 
                   correct ID. 
...
)
{
  ...
  {
    LOG_MSG_DATA_PROTO_INFO_0("Unable to add 3gpp2-oui");
    return -1;
  }
#endif /* FEATURE_DATA_PS_EHRPD */
  ...
  ---------------------------------------------------------------------*/
  DSM_TOUCH_ITEM(bp);
  dsm_item_set_priority(bp, DSM_QUEUE_PRIORITY_HIGH);
  ...
  {
    LOG_MSG_DATA_PROTO_INFO_0("VSNCP TX: Calling activity cback");
  ...
  if( NULL == fsm_ptr->pdc )
  {
    /* Invalid pdc in fsm_send */
    DATA_PROTO_ERROR_LOG_1(PS_BAD_ARG_OR_NULL_PTR,fsm_ptr->pdc);
    return -1;
  }
...
)
{
  ...
  LOG_MSG_DATA_PROTO_INFO_0( "fsm_makeoptions:" );
  ...
  for ( o_type = 0; o_type < fsm_ptr->pdc->n_option_limit; o_type++ )
  {
    ...
    {
      LOG_MSG_DATA_PROTO_INFO_1( "Adding option: %d", fsm_ptr->pdc->n_option[o_type] );
...
)
{
  ...
  LOG_MSG_DATA_PROTO_INFO_0( "fsm_makereq:" );
...
)
{
  ...
  LOG_MSG_DATA_PROTO_INFO_0 ("Send term_req");
  if ( fsm_ptr->retry_request == 0 )
  {
    return -1;
  }
...
)
{
  ...
  LOG_MSG_DATA_PROTO_INFO_2 ("fsm_termack (dev %d, proto 0x%x)",
...
)
{
  ...
  {
    LOG_MSG_DATA_PROTO_INFO_1( "Remote requests option: %d", option.type );
    ...
    {
      /*-----------------------------------------------------------------------
        LLVM Static Analyzer false positive:
        "Assigned value is garbage or undefined [core.uninitialized.Assign]"
      -----------------------------------------------------------------------*/
      fsm_ptr->remote.work |= (1 << option_index); // clang_sa_ignore
...
)
{
  ...
  while (signed_length > 0  &&  ntohopt(&option, &bp) != -1)
  {
    ...
    if (-1 == (option_index = fsm_n_option(fsm_ptr->pdc, option.type)))
    {
      LOG_MSG_DATA_PROTO_INFO_3( "fsm_nak(): dev %d, fsm %d, unrecognized option %d",
               fsm_ptr->device, fsm_ptr->pdc->fsmi, option.type);
      continue;
    }
...
)
{
  ...
  while (signed_length > 0  &&  ntohopt(&option, &bp) != -1)
  {
    ...
    if (-1 == (option_index = fsm_n_option(fsm_ptr->pdc, option.type)))
    {
      LOG_MSG_DATA_PROTO_INFO_3( "fsm_reject(): dev %d, fsm %d, unrecognized option %d",
               fsm_ptr->device, fsm_ptr->pdc->fsmi, option.type);
      continue;
    }
...
)
{
  ...
  if( VSNCP_3GPP2I_FSM_IS_VALID( fsm_ptr ) )
  {
    ...
    {
      LOG_MSG_DATA_PROTO_INFO_0("Unable to remove OUI!");
      dsm_free_packet( &bp);
      return;
    }
  ...
  MSG_4(MSG_SSID_DS,
        ...
        hdr.code);
#else
  LOG_MSG_DATA_PROTO_INFO_3("fsm_proc(dev %d state %d) for msg %d",
          fsm_ptr->device,
          fsm_ptr->state,
          hdr.code);
#endif
  ...
  {
  case VENDOR_SPECIFIC:
    LOG_MSG_DATA_PROTO_INFO_2("PPP vendor-specific packet received, dev %d, proto 0x%x",
                    fsm_ptr->device, fsm_ptr->pdc->protocol);
    switch( fsm_ptr->state )
    {
      case fsmOPENED:
  ...
  case PROT_REJ:
    ...
    if( NULL != tmp_fsm_ptr )
    {
      ...
      LOG_MSG_DATA_PROTO_INFO_3("got proto_rej on dev %d in state %d for protocol 0x%x",
                tmp_fsm_ptr->device,
                tmp_fsm_ptr->state,
                rejected);
      ...
      if( PPP_PROTOCOL_LCP != protocol )
      {
        ...
        {
          LOG_MSG_DATA_PROTO_INFO_1( "Received Protocol reject for an unsupported protocol %d ",
                           protocol );
          dsm_free_packet(&bp);
          break;
        }
  ...
  case ECHO_REPLY:
    ...
    if( PPP_LCP_PROTOCOL == fsm_ptr->pdc->protocol )
    {
      ...
      {
        LOG_MSG_DATA_PROTO_INFO_1("Dev %d, Link Status Determination succesful!",
                        fsm_ptr->device);
...
)
{
  ...
  case fsmCLOSING:
    ...
    else
    {
      ...
      LOG_MSG_DATA_PROTO_INFO_1("if%d CLOSING timeout; PPP Done", fsm_ptr->device);
      (*fsm_ptr->pdc->stopping)(fsm_ptr);
      fsm_ptr->state = fsmCLOSED;
      return FALSE;
    }
  ...
  case fsmSTOPPING:
    ...
    {
      LOG_MSG_DATA_PROTO_INFO_1( "if%d STOPPING timeout: PPP Done", fsm_ptr->device);
      (*fsm_ptr->pdc->stopping)(fsm_ptr);
      fsm_ptr->state = fsmSTOPPED;
      return FALSE;
    }
