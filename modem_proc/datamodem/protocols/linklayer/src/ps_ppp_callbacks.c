...
Copyright (c) 1998-2017 QUALCOMM Technologies Incorporated. 
All Rights Reserved.
...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/protocols/linklayer/src/ps_ppp_callbacks.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $

when        who    what, where, why
--------    ---    ---------------------------------------------------------- 
11/01/17    ak     CR 2125724: FR 42790: F3/Debug Messages Reduction 
05/08/15    cb     Moved MSG1.0 macros to MSG2.0  
...
===========================================================================*/
#define PS_PPP_CALLBACKS_ASSERT(expression)  \
            ps_ppp_callbacks_assert_wrapper(__LINE__, expression)
...
= = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =*/
 ...
 FUNCTION ps_ppp_callbacks_assert_wrapper
 
 DESCRIPTION
   Wrapper function for PS_PPP_CALLBACKS_ASSERT
 ...
 ===========================================================================*/
 static void ps_ppp_callbacks_assert_wrapper
 (
 ...
 )
 {
   if ( !expression )
   {
     ERR_FATAL("PS_PPP_CALLBACKS_ERR_FATAL at line:%d ",  
                                    line_num,0,0);  
   }
 }/* ps_ppp_callbacks_assert_wrapper */

/*===========================================================================
FUNCTION ITEM_ISVALID()

DESCRIPTION
  This function check validity of item
...
SIDE EFFECTS
===========================================================================*/
static dsm_item_type ** item_isvalid
(
  dsm_item_type **item_head_ptr, /* Ptr to PPP packet                      */
  uint8 validate_data_ptr
)
{
  ...
  if( validate_data_ptr && NULL == (*item_head_ptr)->data_ptr)
  {
    /*"item data_ptr NULL"*/
    DATA_PROTO_ERROR_LOG_2(PS_BAD_ARG_OR_NULL_PTR,validate_data_ptr,
                           (*item_head_ptr)->data_ptr);
...
)
{
  ...
  uint32 aligned_buffer[(PPP_OPT_MAX_LEN / sizeof(uint32) + 1) + sizeof(uint32)];
  byte *opt_val = (byte *)aligned_buffer; /* array to copy option value into */
...
)
{
  ...
  if(*item_head_ptr != NULL)
  {
    ...
    /**/
    LOG_MSG_DATA_PROTO_INFO_3("C-Req CB device %d, id %d, proto %x",
...
)
{
  ...
  {
    dsm_free_packet(item_head_ptr);
    /*("Saw LCP C-Ack when NOT in Pseudo Net/Full Net mode!Dumping packet")*/
    PS_PPP_CALLBACKS_ASSERT(0);
    return SNOOP_CB_IGNORE_PKT;
  }
  ...
  LOG_MSG_DATA_PROTO_INFO_2("LCP C-Ack CB device %d, id: %d", device, id);
  ...
  if(id == curr_ppp_id[device])
  {
    if(device == PPP_RM_DEV || device == PPP_RM_2_DEV)
    {
      ...
      {
        LOG_MSG_DATA_PROTO_INFO_0("Received two Rm LCP C-Acks!");
      }
#endif
      lcp_c_ack_status =
    ...
    else /* this came in on Um */
    {
      ...
      {
        LOG_MSG_DATA_PROTO_INFO_0("Received two Um LCP C-Acks!");
      }
#endif
      lcp_c_ack_status =
        (ack_status_enum_type)( (uint8)lcp_c_ack_status 
    ...
    if(lcp_c_ack_status == BOTH_ACKS_RCVD)
    {
      ...
      ---------------------------------------------------------------------*/
      LOG_MSG_DATA_PROTO_INFO_0("LCP now established!");
...
)
{
  ...
  LOG_MSG_DATA_PROTO_INFO_2("Rx Init IPCP CReq ifac %d id %d", device, pkt_id);
...
)
{
  ...
  {
    dsm_free_packet(item_head_ptr);
    /*( "Seek extract failed - dumping pkt")*/
    PS_PPP_CALLBACKS_ASSERT(0);
    return SNOOP_CB_IGNORE_PKT;
  }

  LOG_MSG_DATA_PROTO_INFO_2("IPCP C_Ack, device %d, id =%d", device, pkt_id);
  ...
  if(pkt_id == curr_ppp_id[out_device])
  {
    ...
    if(device == PPP_RM_DEV || device == PPP_RM_2_DEV)
    {
      ...
      {
        LOG_MSG_DATA_PROTO_INFO_0("Received two Rm IPCP C-Acks!");
      }
#endif
      ipcp_c_ack_status =
    ...
    else /* this came in on Um */
    {
      ...
      {
        LOG_MSG_DATA_PROTO_INFO_0("Received two Um IPCP C-Acks!");
      }
#endif
      ipcp_c_ack_status =
        (ack_status_enum_type)( (uint8)ipcp_c_ack_status 
    ...
    if(ipcp_c_ack_status == BOTH_ACKS_RCVD)
    {
      ...
      ---------------------------------------------------------------------*/
      LOG_MSG_DATA_PROTO_INFO_0("PPP now established!");
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  PS_PPP_CALLBACKS_ASSERT(protocol == PPP_CCP_PROTOCOL || 
                           protocol == PPP_ILCCP_PROTOCOL);
  PS_PPP_CALLBACKS_ASSERT(device == PPP_RM_DEV || device == PPP_UM_SN_DEV ||
                           device == PPP_RM_2_DEV);

  item_head_ptr = item_isvalid(item_head_ptr, TRUE);
  ...
  {
    /*"No memory to send CCP prot-rej!  silently discard"*/
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  /**/
  /**/
  PS_PPP_CALLBACKS_ASSERT(item_head_ptr != NULL && *item_head_ptr != NULL);
  PS_PPP_CALLBACKS_ASSERT(device < PPP_MAX_DEV);
  PS_PPP_CALLBACKS_ASSERT(protocol == PPP_LCP_PROTOCOL || 
                           protocol == PPP_IPCP_PROTOCOL);
  /**/
 /*-------------------------------------------------------------------------
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  PS_PPP_CALLBACKS_ASSERT( device == PPP_RM_DEV || device == PPP_RM_2_DEV);
  PS_PPP_CALLBACKS_ASSERT( protocol == PPP_LCP_PROTOCOL || 
                           protocol == PPP_IPCP_PROTOCOL );
 
  item_head_ptr = item_isvalid(item_head_ptr, FALSE);
  ...
  PS_PPP_CALLBACKS_ASSERT(ppp_cb_array != NULL);
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  -------------------------------------------------------------------------*/
  PS_PPP_CALLBACKS_ASSERT(ppp_cb_array[PPP_UM_SN_DEV].bridge_dev == PPP_RM_DEV ||
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  {
    LOG_MSG_ERROR_0("bridge device of UM is invalid!");
    return FALSE;
  }
  ...
  {
    LOG_MSG_INFO2_4("TE2 IP address changed! "
                    "New TE2 address 0x%x:%x Old TE2 address 0x%x:%x",
  ...
  {
    LOG_MSG_INFO2_2("TE2 %d, MT2 %d"
                    " Mobile VJ comp on when was off!",
            (te2_ipcp_work & IPCP_N_COMPRESS),
  ...
  {
    LOG_MSG_INFO2_2("Original IWF %d, new IWF %d"
                    " IWF VJ comp off when was on!",
          ...
          (new_iwf_ipcp_work & IPCP_N_COMPRESS))
  {
    ...
    {
      LOG_MSG_INFO2_2("New IWF compr %x, Old IWF compr %d"
    ...
    {
      LOG_MSG_INFO2_2("New IWF slots %d, Old IWF slots %d"
    ...
    {
      LOG_MSG_INFO2_2("New IWF slot comp %d, Old IWF slot comp %d"
          ...
          (mt2_ipcp_work & IPCP_N_COMPRESS))
  {
    ...
    {
      LOG_MSG_INFO2_2("New TE2 slots %d, Old TE2 slots %d"
    ...
    {
      LOG_MSG_INFO2_2("TE2 slot comp now ON (%d), was OFF (%d)!"
                      " TE2 slot comp now ON when was OFF!",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    /*"Rm device is invalid!"*/
    DATA_PROTO_ERROR_LOG_1(PS_BAD_ARG_OR_NULL_PTR,rm_device);
    return FALSE;
  }
  /*-------------------------------------------------------------------------
...
)
{
  ...
  case PPP_OOC_META_STATE:
    ...
    if(PPP_UP_META_STATE == old_ppp_meta_state)
    {
      ...
      {
        LOG_MSG_ERROR_0("The RM device is NULL ! ");  
      }
  ...
  case PPP_INIT_META_STATE:
    ...
    -----------------------------------------------------------------------*/
    PS_PPP_CALLBACKS_ASSERT(rm_device == PPP_RM_DEV || rm_device == PPP_RM_2_DEV);
