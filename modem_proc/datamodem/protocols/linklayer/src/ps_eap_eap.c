...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/protocols/linklayer/src/ps_eap_eap.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  for( eap_lyr_cb_cnt=0; eap_lyr_cb_cnt<EAP_MAX_INSTANCES; eap_lyr_cb_cnt++ )
  {
    if( NULL == eap_eap_lyr_cb[eap_lyr_cb_cnt] )
    {
      ...
      {
        EAP_MEM_ERROR_LOG(sizeof(eap_eap_lyr_cb_type));
  ...
  if( EAP_INVALID_HANDLE == eap_lyr_handle )
  {
    ...
    -----------------------------------------------------------------------*/
    EAP_INPUT_ERROR_LOG_1(eap_lyr_handle);
    return EAP_INVALID_HANDLE;
  }
...
===========================================================================*/
void eap_eap_send_to_tx
(
  eap_eap_handle_type      eap_lyr_handle,
  dsm_item_type**          tx_sdu
)
{
  uint8*  tx_sdu_buffer = NULL;
  uint16  tx_sdu_buffer_len = 0;
  uint32  bytes_read = 0;
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */

  if( FALSE == EAP_EAP_HANDLE_IS_VALID( eap_lyr_handle ) || NULL == tx_sdu)
  {
    EAP_INPUT_ERROR_LOG_2(eap_lyr_handle, tx_sdu);
    dsm_free_packet( tx_sdu );
    return;
  }

  if(  NULL !=  eap_eap_lyr_cb[eap_lyr_handle]->trp_info.trp_tx_f_ptr )
  {
  ...
  else if (NULL !=  eap_eap_lyr_cb[eap_lyr_handle]->trp_info.trp_tx_buf_f_ptr)
  {
    tx_sdu_buffer_len = (uint16)dsm_length_packet(*tx_sdu);

    tx_sdu_buffer = (uint8*) ps_system_heap_mem_alloc(tx_sdu_buffer_len);

    if( NULL == tx_sdu_buffer )
    {
      EAP_MEM_ERROR_LOG(tx_sdu_buffer_len);
      dsm_free_packet( tx_sdu );
      return;
    }
                           
    bytes_read = dsm_pullup( tx_sdu, 
                             (void *)tx_sdu_buffer, 
                             tx_sdu_buffer_len );

    dsm_free_packet( tx_sdu );

    if( bytes_read != (uint32)tx_sdu_buffer_len )
    {
      LOG_MSG_ERROR_2("eap_eap_tx: EAP tx pkt dsm pullup failed %d %d",
		                   bytes_read, tx_sdu_buffer_len);
    }

    eap_eap_lyr_cb[eap_lyr_handle]->trp_info.trp_tx_buf_f_ptr(
       eap_eap_lyr_cb[eap_lyr_handle]->eap_handle, 
       eap_eap_lyr_cb[eap_lyr_handle]->trp_info.trp_tx_user_data,
       tx_sdu_buffer, 
       tx_sdu_buffer_len );

    PS_SYSTEM_HEAP_MEM_FREE(tx_sdu_buffer);
  }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if( TRUE == eap_eap_lyr_cb[eap_lyr_handle]->raw_mode )
  {
    /*-----------------------------------------------------------------------
       ...
       raw mode
    -----------------------------------------------------------------------*/
    EAP_MSG_DEBUG_0("eap_eap_input(): processing pkt in RAW mode");
  ...
  {
    LOG_MSG_ERROR_1("eap_eap_input(): Malformed packet received %d", bytes_read );
    dsm_free_packet(sdu);
    return;
  }
  ...
  else if( payload_length < rx_bytes )
  {
    ...
    EAP_MSG_DEBUG_0("eap_eap_input(): Removing data link padding from"
                    " EAP packet" );
    ...
    {
      EAP_MEM_ERROR_LOG(bytes_removed);
      dsm_free_packet( sdu );
      return;
    }
  }
  ...
  else if (EAP_AUTHENTICATOR_ROLE == eap_get_role(eap_lyr_handle))
  {
    ...
    {    
      EAP_MSG_DEBUG_2("eap_eap_input():Server Role Unexpected EAP CODE %d rcvd,"
                      " id %d. Freeing pkt",
                      eap_frm_hdr.code, eap_frm_hdr.identifier);
      dsm_free_packet( sdu );
      return;
    ...
    case EAP_REQUEST_CODE:
          ...
          (sint15)eap_frm_hdr.identifier )
      {
        ...
        if( NULL != eap_eap_lyr_cb[eap_lyr_handle]->last_tx_rsp_pkt )
        {
          ...
          {
            EAP_MEM_ERROR_LOG(dup_bytes);
            dsm_free_packet( sdu );
            return;
          }

          eap_eap_send_to_tx(eap_lyr_handle, &tx_pkt );
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  {
    EAP_INPUT_ERROR_LOG_1(eap_handle);
    dsm_free_packet( sdu );
    return;
  }
  ...
  if( FALSE == EAP_EAP_HANDLE_IS_VALID( eap_lyr_handle ) )
  {
    EAP_INPUT_ERROR_LOG_2(eap_handle,eap_lyr_handle);
    dsm_free_packet( sdu );
    return;
  }
  ...
  if( TRUE == eap_eap_lyr_cb[eap_lyr_handle]->raw_mode )
  {
    ...
    -----------------------------------------------------------------------*/
    eap_eap_send_to_tx(eap_lyr_handle, sdu );
    return;
  }
  ...
  {
    EAP_INPUT_ERROR_LOG_2(eap_eap_lyr_cb[eap_lyr_handle]->eap_role, 
                          eap_lyr_handle);
    dsm_free_packet( sdu );
    return;
  }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  EAP_MSG_DEBUG_2("eap_eap_rexmit_req_pkt(): req_pkt_length = %d, dup_bytes = %d",
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  EAP_MSG_DEBUG_0("eap_eap_auth_complete_ind(): called");
  ...
  /*-------------------------------------------------------------------------
    ...
    Or client may want to wait until supp_ind_f_ptr for result. 
  -------------------------------------------------------------------------*/
  if( NULL != eap_eap_lyr_cb[eap_lyr_handle]->trp_info.result_ind_f_ptr )
  {
    eap_eap_lyr_cb[eap_lyr_handle]->trp_info.
  ...
  {
    EAP_MSG_DEBUG_0("eap_eap_auth_complete_ind(): null result_ind_f_ptr, waiting for supp_ind_f_ptr" );
  }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  EAP_MSG_DEBUG_0("eap_eap_supp_auth_complete_ind(): called" );
  ...
  /*-------------------------------------------------------------------------
    Make sure client is registered for supplementary result
  -------------------------------------------------------------------------*/
  eap_lyr_handle = eap_get_eap_lyr_handle(eap_handle);

  if( FALSE == EAP_EAP_HANDLE_IS_VALID( eap_lyr_handle ) ||
      FALSE == eap_eap_supp_result_f_ind_ptr_is_valid(eap_handle) )
  {
    EAP_INPUT_ERROR_LOG_1(eap_lyr_handle);
    return;
  }
...
FUNCTION EAP_EAP_AUTH_NON_METHOD_COMPLETE_IND

DESCRIPTION
  This function is called by the peer layer to notify EAP layer that the
  authentication result indication AS WELL as an EAP Success/Failure have
  been received and method is not invoked
...
===========================================================================*/
void eap_eap_auth_non_method_complete_ind
(
  eap_handle_type          eap_handle,
  eap_result_enum_type     result,
  uint8                   *pre_master_key,
...
{
  eap_eap_handle_type              eap_lyr_handle;
  eap_sim_aka_auth_cred_type      *supp_auth_info_ptr = NULL;
  eap_sim_aka_meta_info_type      *meta_info_ptr = NULL;
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */

  EAP_MSG_DEBUG_0("eap_eap_auth_non_method_complete_ind(): called");
  ...
  /*-------------------------------------------------------------------------
    Inform the client (e.g. 802.1x) of the authentication result.
      if result_ind_f_ptr is null, notify using supp_result_ind_f_ptr with
      dummy values
  -------------------------------------------------------------------------*/
  if( NULL != eap_eap_lyr_cb[eap_lyr_handle]->trp_info.result_ind_f_ptr )
  {
    eap_eap_lyr_cb[eap_lyr_handle]->trp_info.result_ind_f_ptr(eap_handle,
                        eap_eap_lyr_cb[eap_lyr_handle]->trp_info.result_ind_user_data,
                        result,
                        pre_master_key,
                        pre_master_key_len);
  ...
  else
  {
    ...
    supp_auth_info_ptr = (eap_sim_aka_auth_cred_type*)
      ps_system_heap_mem_alloc(sizeof(eap_sim_aka_auth_cred_type));	
    
    if( NULL == supp_auth_info_ptr )
    {
      EAP_MEM_ERROR_LOG(sizeof(eap_sim_aka_auth_cred_type));
      return;
    }

    memset(supp_auth_info_ptr,0,sizeof(eap_sim_aka_auth_cred_type));			

    eap_eap_lyr_cb[eap_lyr_handle]->trp_info.supp_result_ind_f_ptr(
         eap_handle,
         eap_eap_lyr_cb[eap_lyr_handle]->trp_info.supp_result_ind_user_data,
         result,
         meta_info_ptr->supp_result_auth_info_mask,
         sizeof(eap_sim_aka_auth_cred_type),
         supp_auth_info_ptr );
    
    PS_SYSTEM_HEAP_MEM_FREE(supp_auth_info_ptr);
...
===========================================================================*/
void eap_eap_inject_result
(
  eap_eap_handle_type   eap_lyr_handle,
  eap_result_enum_type  result
)
{
  eap_code_enum_type code = EAP_FAILURE_CODE;
  ...
  -------------------------------------------------------------------------*/
  if (EAP_PEER_ROLE  != eap_get_role(eap_lyr_handle) ||
      TRUE == eap_eap_lyr_cb[eap_lyr_handle]->raw_mode )
  {
    EAP_INPUT_ERROR_LOG_2(eap_get_role(eap_lyr_handle), 
                          eap_eap_lyr_cb[eap_lyr_handle]->raw_mode);
    return;
  }
  ...
  /*-------------------------------------------------------------------------
    Sanity checks
  -------------------------------------------------------------------------*/
  if( EAP_RESULT_SUCCESS == result )
  {
    code = EAP_SUCCESS_CODE;
  }

  /*-------------------------------------------------------------------------
    Inject to Peer layer as if we processed the EAP hdr packet
  -------------------------------------------------------------------------*/
  eap_peer_input( eap_eap_lyr_cb[eap_lyr_handle]->eap_handle,
                  code,
                  NULL);
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  EAP_MSG_DEBUG_1("eap_get_reauth_id(): Obtain the reauth id : %d", result);
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  EAP_MSG_DEBUG_1("eap_get_pseudonym_id(): Obtain the pseudonym id : %d", result);
  return result;
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  EAP_MSG_DEBUG_1("In eap_client_start_ind eap_lyr_handle %d", eap_lyr_handle);
