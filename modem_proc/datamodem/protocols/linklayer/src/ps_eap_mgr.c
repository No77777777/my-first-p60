...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/protocols/linklayer/src/ps_eap_mgr.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
{
  EAP_MSG_DEBUG_2("eap_method_input_cback_reg method:%d, role:%d",
                  method, role);
  /*-------------------------------------------------------------------------
...
)
{
  ...
  {
    EAP_MSG_DEBUG_0( "Only EAP Peer Role supported; invalid cback reg" );
  }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if( FALSE == eap_svc_inited )
  {
    ...
    -----------------------------------------------------------------------*/
    EAP_INPUT_ERROR_LOG_1(eap_svc_inited);
    return -1;
  }
  ...
  /*-------------------------------------------------------------------------
    Clients can configure for result or supp_result but trp_tx_f_ptr must be
    configured
  -------------------------------------------------------------------------*/
  if ( ( NULL == eap_info->result_ind_f_ptr &&
         NULL == eap_info->supp_result_ind_f_ptr )|| 
       ( NULL == eap_info->trp_tx_f_ptr && 
         NULL == eap_info->trp_tx_buf_f_ptr ) )
  {
  ...
  eap_eap_info.eap_trp_info.trp_tx_buf_f_ptr  = eap_info->trp_tx_buf_f_ptr;
  ...
  EAP_MSG_DEBUG_1("EAP role = %d", eap_eap_info.eap_role );
...
)
{
  ...
  dsm_item_type*  input_pkt = NULL;
  uint16          write_bytes = 0;
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */

  LOG_MSG_INFO1_3("eap_input_buffer(handle %d, 0x%x , len = %d): called , ", 
                  eap_handle, eap_pkt, eap_pkt_len );
  ...
  /*-------------------------------------------------------------------------
    Convert buffer to dsm item and send to eap layer
  -------------------------------------------------------------------------*/
  write_bytes = dsm_pushdown_packed(&input_pkt, eap_pkt, eap_pkt_len, DSM_DS_SMALL_ITEM_POOL);

  EAP_MSG_DEBUG_3("eap_input_buffer input_pkt = 0x%x , dsm_len = %d: write_bytes %d , ", 
                  input_pkt, dsm_length_packet(input_pkt), write_bytes );

  if( write_bytes != eap_pkt_len )
  {
    EAP_INPUT_ERROR_LOG_1(eap_handle);
  }

  eap_eap_input( eap_cb[eap_handle].eap_lyr_handle, &input_pkt );
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  {
    EAP_INPUT_ERROR_LOG_2(eap_handle, role);
    dsm_free_packet(sdu);
    return;
  }
...
)
{
  ...
  EAP_MSG_DEBUG_1("Setting authenticator_rsp_required to: %d",
...
FUNCTION EAP_GET_USER_IDENTITY
...
===========================================================================*/
eap_identity_type eap_get_user_identity
(
  eap_handle_type eap_handle
)
{
...
)
{
  ...
  /*-------------------------------------------------------------------------
    For some clients configure session type base on subs id set in meta_info_ptr
  -------------------------------------------------------------------------*/
  eap_client_config_session_type(eap_handle, meta_info_ptr);
...
)
{
  ...
  /*-------------------------------------------------------------------------
    This API is restricted to specify clients as per design
  -------------------------------------------------------------------------*/
  if ( FALSE == eap_client_config_is_set_result_allowed(eap_handle) )
  {
    EAP_INPUT_ERROR_LOG_1(eap_handle);
    return;
  }
  ...
  /*-------------------------------------------------------------------------
    Inject result to eap layer
  -------------------------------------------------------------------------*/
  LOG_MSG_INFO1_2("eap_set_result(): called, eap_handle %d result %d", eap_handle, result );
  eap_eap_inject_result(eap_cb[eap_handle].eap_lyr_handle, result);
