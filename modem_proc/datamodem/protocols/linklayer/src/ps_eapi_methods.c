...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/protocols/linklayer/src/ps_eapi_methods.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  -------------------------------------------------------------------------*/
  EAP_MSG_DEBUG_0("EAP Notification message received by Peer" );
  dsm_free_packet( sdu );
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  {
    EAP_INPUT_ERROR_LOG_2(eap_handle, auth_mgr);
    dsm_free_packet(sdu);
    return;
  }
  ...
  {
    EAP_MEM_ERROR_LOG( challenge_len);
    dsm_free_packet(sdu);
    return;
  }
  ...
  if ( data == NULL )
  {
    EAP_MEM_ERROR_LOG(EAP_MD5_PKT_MAX_LEN);
    dsm_free_packet(sdu);
    return;
  }
