...
$Header: //components/rel/data.mpss/3.5..2.1/protocols/linklayer/src/ps_eap_server.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  EAP_MSG_DEBUG_0("eap_server_init() called ");
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  /*-----------------------------------------------------------------------
    Check if a free instance is found
  -----------------------------------------------------------------------*/
  if ( EAP_MAX_INSTANCES == server_lyr_cb_cnt || 
      -1 == server_lyr_handle )
  {
    EAP_INPUT_ERROR_LOG_1(server_lyr_handle);
    return -1;
  }
  ...
  {
    EAP_MEM_ERROR_LOG(sizeof(eap_server_lyr_cb_type));
    return -1;
  }
  ...
  /*-------------------------------------------------------------------------
    ...
    requested, other method types are currently not supported in raw mode
  -------------------------------------------------------------------------*/
  if (TRUE == eap_server_lyr_cb[server_lyr_handle]->raw_mode)
  {
    if (EAP_SERVER_MIN_TYPE 
                != eap_server_lyr_cb[server_lyr_handle]->current_method_type)
    {
      EAP_INPUT_ERROR_LOG_2(server_lyr_handle,
                            eap_server_lyr_cb[server_lyr_handle]->current_method_type);
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  EAP_MSG_DEBUG_1("eap_server_delete_instance() called , server_lyr_handle : %d ",
                  server_lyr_handle);
  ...
  {
    EAP_ERROR_DEBUG_1("Invalid server_lyr_handle : %d ",
                      server_lyr_handle);
  }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  switch(eap_server_lyr_cb[server_lyr_handle]->state)
  {
    ...
    case EAP_SERVER_INIT_S:
      /*---------------------------------------------------------------------
        Packet can't be sent in INIT state
        intentiaonal fall through
      ---------------------------------------------------------------------*/
    default:
      /*---------------------------------------------------------------------
        SANITY check
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  if( (cback == NULL) || 
      (EAP_MIN_EXT_METHOD > method) || (EAP_MAX_METHOD <= method))
  {
    EAP_INPUT_ERROR_LOG_1(method);
    return;
  }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  /*-------------------------------------------------------------------------
    Check for handle validity and retrieve the Server layer handle & code
  -------------------------------------------------------------------------*/
  server_lyr_handle = eap_get_server_lyr_handle(eap_handle);
  ...
  if (FALSE == EAP_SERVER_HANDLE_IS_VALID(server_lyr_handle) ||
      EAP_MAX_CODE <= code)
  {
    EAP_INPUT_ERROR_LOG_2(server_lyr_handle,code);
    dsm_free_packet(sdu);
    return;
  }
  ...
  if (TRUE == eap_server_lyr_cb[server_lyr_handle]->raw_mode)
  {
    ...
    -----------------------------------------------------------------------*/
    EAP_INPUT_ERROR_LOG_1(curr_method);
    dsm_free_packet(sdu);    
    return;
  ...
  *sdu = NULL;
  ...
  switch (server_frm_hdr.type)
  {
    case EAP_SERVER_IDENTITY_TYPE:
      ...
      {
        /*-------------------------------------------------------------------
         IDENTITY type pkt rcvd in unsupported state 
        -------------------------------------------------------------------*/
        EAP_INPUT_ERROR_LOG_2(server_lyr_handle,
                              eap_server_lyr_cb[server_lyr_handle]->state);
        dsm_free_packet(&(eap_server_lyr_cb[server_lyr_handle]->pending_pkt));
    ...
    case EAP_SERVER_EXT_TYPE:
      ...
      {
        /*-------------------------------------------------------------------
         EXT type pkt rcvd in unsupported state 
        -------------------------------------------------------------------*/
        EAP_INPUT_ERROR_LOG_2(server_lyr_handle,
                               eap_server_lyr_cb[server_lyr_handle]->state);
        dsm_free_packet(&(eap_server_lyr_cb[server_lyr_handle]->pending_pkt));
    ...
    case EAP_SERVER_NAK_TYPE:
      ...
      {
        /*-------------------------------------------------------------------
         NAK type pkt rcvd in unsupported state 
        -------------------------------------------------------------------*/
        EAP_INPUT_ERROR_LOG_2(server_lyr_handle,
                              eap_server_lyr_cb[server_lyr_handle]->state);
        dsm_free_packet(&(eap_server_lyr_cb[server_lyr_handle]->pending_pkt));
    ...
    default:
      EAP_INPUT_ERROR_LOG(server_lyr_handle,
                      eap_server_lyr_cb[server_lyr_handle]->state,
                      server_frm_hdr.type);
      dsm_free_packet(&(eap_server_lyr_cb[server_lyr_handle]->pending_pkt));
      break;
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if (FALSE == EAP_SERVER_HANDLE_IS_VALID(server_lyr_handle))
  {
    EAP_INPUT_ERROR_LOG_2(server_lyr_handle,eap_handle);
    return;
  }
