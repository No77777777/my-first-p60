...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/protocols/linklayer/src/ps_ppp_ipcp.c#2 $
...
static struct fsm_constant_s ipcp_device_constants[PPP_MAX_SUBS_ID][PPP_MAX_DEV];
...
)
{
  ...
  -------------------------------------------------------------------------*/
  memscpy(&ipcp_device_constants[PPP_SUBS_ID_1][device],
          sizeof(ipcp_device_constants[PPP_SUBS_ID_1][device]),
          &ipcp_default_constants,
          sizeof(ipcp_default_constants));


  memscpy(&ipcp_device_constants[PPP_SUBS_ID_2][device],
          sizeof(ipcp_device_constants[PPP_SUBS_ID_2][device]),
          &ipcp_default_constants,
          sizeof(ipcp_default_constants));
  ...
  -------------------------------------------------------------------------*/
  fsm_ptr->pdc             = &ipcp_device_constants[PPP_SUBS_ID_1][device];
...
)
{
  ...
  pconst = &ipcp_device_constants[subs_id][device];
...
/*===========================================================================
FUNCTION IPCP_GET_DEVICE_CONSTANTS()
...
RETURN VALUE
  Configurable protocol constants for IPCP.
...
SIDE EFFECTS
  None
===========================================================================*/
struct fsm_constant_s *ipcp_get_device_constants
(
  ppp_dev_enum_type           device, 
  ppp_subs_id_enum_type       subs_id
)
{
  ...
  LOG_MSG_INFO1_6("PPP IPCP DEVICE CONSTANT SUBS ID %d"
                  " try_configure %d"
                  " try_nak %d"
                  " try_terminate %d"
                  " term_timeout %d"
                  " ack_timeout %d", 
                    subs_id,
                    ipcp_device_constants[subs_id][device].try_configure,
                    ipcp_device_constants[subs_id][device].try_nak,
                    ipcp_device_constants[subs_id][device].try_terminate,
                    ipcp_device_constants[subs_id][device].term_timeout,
                    ipcp_device_constants[subs_id][device].ack_timeout);

  return &ipcp_device_constants[subs_id][device];
} /* ipcp_get_device_constants() */
...
)
{
  ...
  switch(constant)
  {
    ...
    default:
      LOG_MSG_DATA_PROTO_INFO_1("ppp_ipcp_init_device_constant: Unknown constant %d",
                      constant);
      break;
  }
...
)
{
  ...
  LOG_MSG_DATA_PROTO_INFO_1("ipcp_init(device %d)", device);
...
)
{
  ...
  {
    LOG_MSG_DATA_PROTO_INFO_1("IPCP: Got IPv4 Address = %x", lworkp->address.ps_s_addr);
...
#define IPCP_ADD_OPTION(pkt_ref_ptr, opt_hdr_ptr, param_ptr, param_size) \
  ...
  LOG_MSG_DATA_PROTO_INFO_1( "ipcp_option: optnum = %d", opt_num );
  ...
  {
  case IPCP_1ADDRESS:
    LOG_MSG_DATA_PROTO_INFO_1( "ipaddr = 0x%x", value_p->address.ps_s_addr );
  ...
  case IPCP_PRIMARY_DNS:
    LOG_MSG_DATA_PROTO_INFO_1( "pridns = 0x%x", value_p->primary_dns.ps_s_addr );
    opthdr.length = sizeof(ppp_opt_hdr_type) + sizeof(struct ps_in_addr);
  ...
  case IPCP_SECONDARY_DNS:
    LOG_MSG_DATA_PROTO_INFO_1( "secdns = 0x%x", value_p->secondary_dns.ps_s_addr );
    opthdr.length = sizeof(ppp_opt_hdr_type) + sizeof(struct ps_in_addr);
  ...
  case IPCP_COMPRESS:
    LOG_MSG_DATA_PROTO_INFO_1( "compression = 0x%x", value_p->compression );
  ...
  default:
    LOG_MSG_DATA_PROTO_INFO_1( "Got IPCP option %d", opt_num);
    break;
  } /* switch(option) */;
...
)
{
  ...
  LOG_MSG_DATA_PROTO_INFO_1( "ipcp_check: opttype = %d", ohp->type );
  switch( ohp->type)
  {
    /*-----------------------------------------------------------------------
      IP address option
    -----------------------------------------------------------------------*/
  case IPCP_1ADDRESS:
    LOG_MSG_DATA_PROTO_INFO_1( "IPCP_1ADDRESS = 0x%x", swork_ptr->address.ps_s_addr );
    ...
    -----------------------------------------------------------------------*/
  case IPCP_PRIMARY_DNS:
    LOG_MSG_DATA_PROTO_INFO_1( "IPCP_PRIMARY_DNS = 0x%x",
    ...
    -----------------------------------------------------------------------*/
  case IPCP_SECONDARY_DNS:
    LOG_MSG_DATA_PROTO_INFO_1( "IPCP_SECONDARY_DNS = 0x%x",
  ...
  case IPCP_COMPRESS:
    LOG_MSG_DATA_PROTO_INFO_1( "IPCP_COMPRESS = 0x%x", swork_ptr->compression );
...
)
{
  ...
  {
    LOG_MSG_DATA_PROTO_INFO_3( "ipcp_reject(): dev %d, fsm %d, unrecognized option %d",
  ...
  else
  {
    ...
    if (ohp->type == IPCP_COMPRESS)
    {
      ...
      case PPP_VJCH_PROTOCOL:
        ...
        break;
      default:
        LOG_MSG_DATA_PROTO_INFO_1("Received rejection for compression protocol 0x%x",
                        ps_ntohs(proto) );
        break;
      }
...
)
{
  ...
  {
    LOG_MSG_DATA_PROTO_INFO_0("PRIMARY NBNS ADDRESS PRESENT");
...
)
{
  ...
  {
    LOG_MSG_DATA_PROTO_INFO_0 ("Could not negotiate required options during resync");
  }
  ...
  if( ppp_cb_ptr->resync_dns_opts == FALSE )
  {
    ...
    LOG_MSG_DATA_PROTO_INFO_1("ipcp_opening, dev %d : Disabling DNS option in IPCP",
                    fsm_ptr->device);
    /*-----------------------------------------------------------------------
...
)
{
  ...
  -------------------------------------------------------------------------*/
  if ( NULL == fsm_ptr->rx_iface_ptr )
  {
    /*("Invalid rx_iface_ptr")*/
    PS_PPP_IPCP_ASSERT(0);
    return;
  }
  phys_link_ptr = ps_iface_get_phys_link(fsm_ptr->rx_iface_ptr);
  ...
  if( -1 != mru )
  {
    ...
    LOG_MSG_DATA_PROTO_INFO_1("IPCP MTU set = %d ", PS_IFACE_GET_MTU(fsm_ptr->rx_iface_ptr));
  }
} /* ipcpi_iface_config() */
