...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/protocols/linklayer/src/ps_ppp_lcp.c#2 $
...
static struct fsm_constant_s lcp_device_constants[PPP_MAX_SUBS_ID][PPP_MAX_DEV];
...
)
{
  ...
  -------------------------------------------------------------------------*/
  memscpy(&lcp_device_constants[PPP_SUBS_ID_1][device],
          sizeof(lcp_device_constants[PPP_SUBS_ID_1][device]),
          &lcp_default_constants,
          sizeof(lcp_default_constants));

  memscpy(&lcp_device_constants[PPP_SUBS_ID_2][device],
          sizeof(lcp_device_constants[PPP_SUBS_ID_2][device]),
          &lcp_default_constants,
          sizeof(lcp_default_constants));
  ...
  -------------------------------------------------------------------------*/
  fsm_ptr->pdc             = &lcp_device_constants[PPP_SUBS_ID_1][device];
...
)
{
  ...
  pconst = &lcp_device_constants[subs_id][device];
...
/*===========================================================================
FUNCTION LCP_GET_DEVICE_CONSTANTS()
...
RETURN VALUE
  Configurable protocol constants for LCP.
...
SIDE EFFECTS
  None
===========================================================================*/
struct fsm_constant_s *lcp_get_device_constants
(
...
)
{
  ...
  LOG_MSG_INFO1_6("PPP LCP DEVICE CONSTANT SUBS ID %d"
                  " try_configure %d"
                  " try_nak %d"
                  " try_terminate %d"
                  " term_timeout %d"
                  " ack_timeout %d", 
                    subs_id,
                    lcp_device_constants[subs_id][device].try_configure,
                    lcp_device_constants[subs_id][device].try_nak,
                    lcp_device_constants[subs_id][device].try_terminate,
                    lcp_device_constants[subs_id][device].term_timeout,
                    lcp_device_constants[subs_id][device].ack_timeout);

  return &lcp_device_constants[subs_id][device];
} /* lcp_get_device_constants() */

...
)
{
  ...
  switch(constant)
  {
    ...
    default:
      LOG_MSG_DATA_PROTO_INFO_1("ppp_lcp_init_device_constant: Unknown constant %d",
                      constant);
      break;
  }
...
)
{
  ...
  switch(opt_num)
  {
    ...
    default:
      LOG_MSG_DATA_PROTO_INFO_1("Got unknown option %d - ignoring", opt_num);
...
)
{
  ...
  if( fsm_ptr->remote.work & LCP_N_ACCM )
  {
    ...
    LOG_MSG_DATA_PROTO_INFO_2("lcp_setup_device (%d), setting HDLC ACCM = 0x%x",
...
)
{
  ...
  LOG_MSG_DATA_PROTO_INFO_1("lcp_option: optnum = %d", opt_num);
...
)
{
  ...
  LOG_MSG_DATA_PROTO_INFO_1("lcp_check: opttype = %d", ohp->type);
  ...
  case LCP_AP: /* Authentication protocol */
    ...
    -----------------------------------------------------------------------*/
    LOG_MSG_DATA_PROTO_INFO_0("PPP auth check (only for resync)");
    LOG_MSG_DATA_PROTO_INFO_3("use_same_auth %d, work->auth 0x%x, cur->auth 0x%x",
    ...
    case PPP_PAP_PROTOCOL:
      ...
      if(request)
      {
        LOG_MSG_DATA_PROTO_INFO_0("Received Config. Req. with PAP option");
      }
      else
      {
        LOG_MSG_DATA_PROTO_INFO_0("Received Config. NAK with PAP option");
      }
      ...
      {
        LOG_MSG_DATA_PROTO_INFO_0("C-NAKing PAP, proposing CHAP");
      ...
      {
        LOG_MSG_DATA_PROTO_INFO_0("C-NAKing PAP, proposing EAP");
    ...
    case PPP_CHAP_PROTOCOL:
      ...
      if(request)
      {
        LOG_MSG_DATA_PROTO_INFO_0("Received Config. Req. with CHAP option");
      }
      else
      {
        LOG_MSG_DATA_PROTO_INFO_0("Received Config. NAK with CHAP option");
      }
      ...
      if( ( (uint16)s_want_ptr->auth_proto_mask ) & ( (uint16)PPP_AUTH_CHAP ) )
      {
        ...
        LOG_MSG_DATA_PROTO_INFO_2("dev %d got C-req with chap digest: %d",
                        fsm_ptr->device, rx_chap_digest);
      }
      else if( ( (uint16)s_want_ptr->auth_proto_mask ) & 
               ( (uint16)PPP_AUTH_PAP ) )
      {
        LOG_MSG_DATA_PROTO_INFO_0("C-NAKing CHAP, proposing PAP");
      ...
      {
        LOG_MSG_DATA_PROTO_INFO_0("C-NAKing CHAP, proposing EAP");
        s_work_ptr->authentication = PPP_EAP_PROTOCOL;
        option_result              = CONFIG_NAK;
      }
#endif /*FEATURE_DATA_PS_PPP_EAP*/
    ...
    case PPP_EAP_PROTOCOL:
      if(request)
      {
        LOG_MSG_DATA_PROTO_INFO_0("Received Config Req with EAP option");
      }
      else
      {
        LOG_MSG_DATA_PROTO_INFO_0("Received Config NAK with EAP option");
      }
    ...
    default:
      ...
      ---------------------------------------------------------------------*/
      LOG_MSG_DATA_PROTO_INFO_1("Rxd Unknown Auth proto:0x%x ",
                      s_work_ptr->authentication);
      if( ( (uint16)s_want_ptr->auth_proto_mask ) & ( (uint16)PPP_AUTH_CHAP ) )
      {
        LOG_MSG_DATA_PROTO_INFO_0("C-NAKing Unknown auth proto, proposing CHAP");
        s_work_ptr->authentication = PPP_CHAP_PROTOCOL;
        s_work_ptr->chap_digest    = PPP_CHAP_MD5;
        option_result              = CONFIG_NAK;
      }
               ...
               ( (uint16)PPP_AUTH_PAP ) )
      {
        LOG_MSG_DATA_PROTO_INFO_0("C-NAKing Unknown auth proto, proposing PAP");
        s_work_ptr->authentication = PPP_PAP_PROTOCOL;
        option_result              = CONFIG_NAK;
      }
      else
...
)
{
  ...
  LOG_MSG_DATA_PROTO_INFO_2("lcp_starting, setting HDLC ACCM = 0x%x for dev %d",
...
#endif /*FEATURE_DATA_PS_PPP_EAP*/
  LOG_MSG_INFO2_3("lcp_starting, dev (%d) auth_proto_mask 0x%x subs %d",
                  fsm_ptr->device, rwant_ptr->auth_proto_mask, ppp_cb_ptr->subs_id);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if( fsm_ptr->remote.work & LCP_N_ACCM )
  {
    ...
    LOG_MSG_DATA_PROTO_INFO_1("LCP negotiation finished, setting HDLC ACCM = 0x%x",
                    ioctl_data.accm);
#ifdef FEATURE_DATA_PS_HDLC_A2_AUTONOMOUS 
    if(ppp_cb_ptr->hdlc_mode == HDLC_A2_AUTONOMOUS)
    {
