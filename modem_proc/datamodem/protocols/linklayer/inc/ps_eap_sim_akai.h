...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/protocols/linklayer/inc/ps_eap_sim_akai.h#3 $ 
...
03/28/17    vls    Added support for checkcode computation
...
typedef struct
{
  ...
  /* Determines if we succeed authentication phase */
  eap_result_enum_type           auth_phase;
  ...
  /* Computed checkcode to be sent over challenge response/reauthentication 
     response packet */
  ...
  /* Received checkcode in an incoming pkt and its len - stored for validation*/
  uint8                          rx_checkcode[EAP_AKA_PRIME_MAX_CHECKCODE];
  uint16                         rx_checkcode_len;

  /* Boolean to check if server has sent the checkcode( Since this attribute is optional) */
  boolean                        rx_checkcode_present;
  ...
  /* Per client config whether to verify the received checkcode and/or to 
    send the checkcode */
  /* This will be set if both the global NV and the config per client is set */
  boolean                       verify_rx_checkcode;
  boolean                       send_checkcode;
...
{
  boolean verify_rx_checkcode;
  boolean tx_checkcode;
}eap_sim_aka_checkcode_enable_type;
...
/*===========================================================================
FUNCTION  eap_sim_akai_frm_validate_checkcode

DESCRIPTION
  Called to validate the checkcode for a request.
...
===========================================================================*/
boolean 
eap_sim_akai_frm_validate_checkcode
(
  eap_sim_aka_ctx*  sim_aka_inst
);
...
/*===========================================================================
FUNCTION  eap_check_if_verify_checkcode_enabled

DESCRIPTION
  Called to check if checkcode verification for a particular client is enabled
...
===========================================================================*/
boolean eap_check_if_verify_checkcode_nv_enabled
(
   void
);

/*===========================================================================
FUNCTION  eap_check_if_send_checkcode_nv_enabled

DESCRIPTION
  Called to check if checkcode can be computed and sent for a particular client
...
===========================================================================*/
boolean eap_check_if_send_checkcode_nv_enabled
(
   void
);
