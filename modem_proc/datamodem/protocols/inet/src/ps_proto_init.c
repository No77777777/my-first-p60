...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/protocols/inet/src/ps_proto_init.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $

when        who    what, where, why
--------    ---    ----------------------------------------------------------
13/12/17    my     CR 2157129 : FR 42790: F3 reduction
04/10/15    fn     Added initialization function for FTP and cert utility
03/23/15    kc     Added ps_clat_powerdown() into ps_proto_deinit()
11/11/14    cx     Added support for checksum offload
11/21/14    ds     Added de-initialization function for DHCP and IP frag for
                   QTF memory leaks.
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ( cmd_id != PS_SIM_EVENT_HANDLER_CMD ||
       NULL == userdata_ptr)
  {
    DATA_PROTO_ERROR_LOG_1(PS_BAD_ARG_OR_NULL_PTR, cmd_id);
    ASSERT(0);
    return;
  }
