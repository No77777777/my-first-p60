...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/protocols/inet/src/dsstcp.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $


when        who    what, where, why
--------    ---    ----------------------------------------------------------
05/23/18    gbk    Initialise ps_iface to NULL to avoid garbage value
...
#define DSSTCP_ASSERT(expression)  \
          dsstcp_assert_wrapper(__LINE__, expression)
...
FUNCTION dsstcp_assert_wrapper

DESCRIPTION
  Wrapper function for DSSTCP ASSERT
...
=============================================================================*/
static void dsstcp_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("DSSTCP_ERR_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
  
}/* dsstcp_assert_wrapper */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch (opt_name)
  {
    ...
    case ds_Sock_OptName_QDS_TCP_MAX_BACKOFF_TIME:
      ...
      break;
    case ds_Sock_OptName_QDS_TCP_ACCESS_PRIORITY:
      tcb_ptr->access_priority = (uint32)opt_val;
      if (TCP_ACCESS_PRIORITY_DISABLE == tcb_ptr->access_priority)
      {
        if ((tcb_ptr->ipcb.meta_info_ptr != NULL) &&
           (PS_TX_META_GET_RT_META_INFO_PTR(tcb_ptr->ipcb.meta_info_ptr) != NULL))
           {
             ps_iface_ptr = (ps_iface_type *) PS_TX_META_GET_ROUTING_CACHE( tcb_ptr->ipcb.meta_info_ptr);
             if (NULL != ps_iface_ptr)
             {
               ret_val = ps_iface_ioctl(ps_iface_ptr, 
                         PS_IFACE_IOCTL_SET_TCP_LEVEL_TRM_REQUEST, 
                         &tcb_ptr->access_priority,
                         &ps_errno);
             }
           }
      }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  while (NULL != tcb_ptr)
  {
    DSSTCP_ASSERT(0 != tcb_ptr->sockfd);
