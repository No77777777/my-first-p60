...
$Id: //components/rel/data.mpss/3.5..2.1/protocols/inet/src/dhcp_mgr.c#2 $ 
...
#define DHCP_MGR_ASSERT(expression)  \
          dhcp_mgr_assert_wrapper(__LINE__, expression)
/*===========================================================================

FUNCTION DHCP_MGR_ASSERT_WRAPPER()

DESCRIPTION
  Wrapper function for DHCP_MGR ASSERT
...
=============================================================================*/
static void dhcp_mgr_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("DHCP_MGR_ERR_FATAL at line:%d ",  
              line_num,
              0,
              0);  
  }
  
}/* dhcp_mgr_assert_wrapper */
...
)
{
  ...
  {
    /* "FAIL: COOKIE check failed <handle->cookie_val> <handle->cookie_ptr> <handle>" */
    DHCP_MGR_ASSERT(0);
    return FALSE;
  }
  return TRUE;
...
)
{
  ...
  DHCP_MGR_ASSERT(NULL != server);
  DHCP_MGR_ASSERT(NULL != am);
  ...
  if ( NULL == handle )
  {
    /* "dhcp_start unable to get memory for handle" */
    DHCP_MGR_ASSERT(0);
    return NULL;
  }
...
)
{
  ...
  DHCP_MGR_ASSERT( NULL != user_data_ptr );
  DHCP_MGR_ASSERT( PS_DHCP_STOP_CMD == cmd );
...
)
{
  ...
  /* we are likely to have hit a data abort above if this is foobar */
  DHCP_MGR_ASSERT( NULL != user_data_ptr );
  DHCP_MGR_ASSERT( PS_DHCP_START_CMD == cmd );
  DHCP_MGR_ASSERT( NULL != start->done_cb );
...
)
{
  ...
  {
    /* "dhcp_start got invalid ps_iface_handle <client> <am>" */
    DHCP_MGR_ASSERT(0);
    return FALSE;
  }
...
)
{
  ...
  DHCP_MGR_ASSERT( NULL != handle_ptr );
  ...
  {
    /* "dhcp_stop called with invalid handle <*handle_ptr>" */
    DHCP_MGR_ASSERT(0);
    return FALSE;
  }
...
)
{
  ...
  DHCP_MGR_ASSERT( NULL != info );
  DHCP_MGR_ASSERT( NULL != client_ip );
  DHCP_MGR_ASSERT( NULL != server_id );
  DHCP_MGR_ASSERT( NULL != netmask );
