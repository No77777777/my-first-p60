...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/protocols/inet/src/ps_ip_fraghdl.c#2 $
...
#define PS_IP_FRAGHDL_ASSERT(expression)  \
          ps_ip_fraghdl_assert_wrapper(__LINE__, expression)
...
FUNCTION ps_ip_fraghdl_assert_wrapper

DESCRIPTION
  Wrapper function for PS_IP_FRAGHDL ASSERT
...
=============================================================================*/
static void ps_ip_fraghdl_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("PS_IP_FRAGHDL_ERR_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
  
}/* ps_ip_fraghdl_assert_wrapper */
...
)
{
  ...
  if( NULL != fraghdl_ptr->device_ptr )
  {
    ...
    if ( IP_FRAGHDL_BRIDGE_TYPE_IFACE_BRIDGE == fraghdl_ptr->device_type )
    {
      ...
      LOG_MSG_DATA_PATH_INFO_2("Iface Bridging - Sending all the queued "
               "fragments onto Iface 0x%x:%d", bridge_iface_ptr->name,
    ...
    {
      LOG_MSG_DATA_PATH_INFO_0("PPP Bridging - Sending all the queued "
               "fragments onto PPP Dev");
      /*------------------------------------------------------------------
  ...
  else
  {
       ...
       )
    {
      ...
      if (NULL != iface_ptr)
      {
        LOG_MSG_DATA_PATH_INFO_2("Meta_info's iface 0x%x:%d",
    ...
    else
    {
      ...
      -------------------------------------------------------------------*/
      LOG_MSG_DATA_PATH_INFO_0("Out-Of-Order fragment, drop it!");
      DPL_LOG_NETWORK_RX_DROPPED_PACKET(iface_ptr, item_ptr, DPL_IID_NETPROT_IP);
      dsm_free_packet(&item_ptr);
    }
  }
...
)
{
  ...
  return (ps_rx_meta_info_type *)fraghdl_ptr->meta_info_ptr;
...
{
  qsh_mdump_collect_high(&rcvd_fragments_num, sizeof(uint32));
  qsh_mdump_collect_high(&ip_fraghdl_array, NUM_FRAGHDL * sizeof(ip_fraghdl_s_type));
