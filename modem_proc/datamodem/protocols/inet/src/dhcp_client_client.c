...
$Id: //components/rel/data.mpss/3.5..2.1/protocols/inet/src/dhcp_client_client.c#2 $
...
#define DHCP_CLIENT_CLIENT_ASSERT(expression)  \
          dhcp_client_client_assert_wrapper(__LINE__, expression)
/*===========================================================================

FUNCTION DHCP_CLIENT_CLIENT_ASSERT_WRAPPER()

DESCRIPTION
  Wrapper function for DHCP_CLIENT_CLIENT ASSERT
...
=============================================================================*/
static void dhcp_client_client_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("DHCP_CLIENT_CLIENT_ERR_FATAL at line:%d ",  
              line_num,
              0,
              0);  
  }
  
}/* dhcp_client_client_assert_wrapper */
...
)
{
  ...
  {
    /*  "got null dest or src" */
    DHCP_CLIENT_CLIENT_ASSERT(0);
    return FALSE;
  }
...
)
{
  ...
  {
    /*  "Didn't get sock event cmd PS_DHCP_CLIENT_SOCK_EVENT_CMD" */
    DHCP_CLIENT_CLIENT_ASSERT(0);
    return;
  }
  ...
  {
    /* "Cookie pointer failed in sock callback" */
    DHCP_CLIENT_CLIENT_ASSERT(0);
    return;
  }
  if ( DHCP_CLIENT_CLIENT_COOKIE != info->cookie )
  {
    /* "Cookie mismatch in sock callback" */
	DHCP_CLIENT_CLIENT_ASSERT(0);
    return;
  }
  if ( info->sock != sockfd )
  {
    /* "Got callback for unknown socket" */
    DHCP_CLIENT_CLIENT_ASSERT(0);
    return;
  }
...
)
{
  ...
  if ( NULL == event )
  {
    /* "dhcpi_client_client_sock_cb unable to get parameter memory" */
    DHCP_CLIENT_CLIENT_ASSERT(0);
    return;
  }
...
)
{
  ...
  LOG_MSG_DATA_PROTO_INFO_0( "INFO: --- DHCP CLIENT CLIENT INIT ---" );

  /* Sanity check parameters */
  if ( NULL == init )
  {
    /* "Got NULL pointer for init" */
	DHCP_CLIENT_CLIENT_ASSERT(0);
    return NULL;
  }
  if ( NULL == init->d_malloc ||
       NULL == init->d_free )
  {
    /* "Got NULL for malloc and/or d_free" */
    DHCP_CLIENT_CLIENT_ASSERT(0);
    return NULL;
  }
  ...
  if ( NULL == handle )
  {
    /* "Insufficent memory to init DHCP client layer" */
    DHCP_CLIENT_CLIENT_ASSERT(0);
    return NULL;
  }
...
)
{
  ...
  LOG_MSG_DATA_PROTO_INFO_0( "INFO: --- DHCP CLIENT CLIENT START ---" );
  ...
  if ( DSS_ERROR != dss_ret )
  {
    ...
    if ( 0 !=
         dss_reg_ip_filter ( info->sock,
                           ...
                           &dss_errno ) )
    {
      /* "FAIL:dhcp_client_start unable to add filter" */
      DHCP_CLIENT_CLIENT_ASSERT(0);
      ret = FALSE;
    }
  }
...
)
{
  ...
  if ( NULL == handle )
  {
    /* "Got NULL handler in client stop" */
    DHCP_CLIENT_CLIENT_ASSERT(0);
    return;
  }
  info = *handle;
  ...
  if ( DSS_ERROR == dss_ret && DS_EWOULDBLOCK == dss_errno )
  {
    ...
    if ( DSS_SUCCESS != dss_ret )
    {
      /* "Failed async select CLOSE on <info->sock> <dss_errno>" */
      info->init.d_free(info);
      DHCP_CLIENT_CLIENT_ASSERT(0);
    }
  }
  ...
  {
    /* "Error closing socket <info->sock> <dss_errno>" */
    info->state = DHCP_CLIENT_CLIENT_STATE_CLOSED;
    info->init.d_free(info);
    DHCP_CLIENT_CLIENT_ASSERT(0);
  }
  ...
  LOG_MSG_DATA_PROTO_INFO_0( "INFO: --- DHCP CLIENT CLIENT STOP ---" );
  *handle = NULL;
} /* dhcp_clienti_client_stop */
...
)
{
  ...
  if ( NULL == msg )
  {
    /* "Got NULL message pointer pointer" */
    DHCP_CLIENT_CLIENT_ASSERT(0);
    return FALSE;
  }
