...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/protocols/inet/src/ps_iputil.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
PARAMETER
  pkt_info_ptr     pointer points to the packet info
  ph_ptr           pointer points to the pseduo header
  m                pointer points to dsm item
  len              the length of data 
  offset           the offset in dsm item to calculate the cksum
...
{
  if(PS_PKT_INFO_GET_CKSUM_OFFLOAD_FLAG(pkt_info_ptr))
  {
    LOG_MSG_DATA_PATH_INFO_0("Checksum offload mode");
    return cksum_update( NULL,
                         ph_ptr,
                         PS_PKT_INFO_GET_CKSUM(pkt_info_ptr));
  }
  else
  {
    LOG_MSG_DATA_PATH_INFO_0("Checksum software mode");
    return cksum(ph_ptr,
                 m,
                 len,
                 offset);
...
)
{
  ...
  LOG_MSG_DATA_PATH_INFO_1("Passed in checksum: 0x%x",old_cksum);
  /*------------------------------------------------------------
    Calculate checksum for old psuedo header
  -------------------------------------------------------------*/
  if(old_ph != NULL)
  {
    for(i=0 ; i< 8; i++)
    ...
    LOG_MSG_DATA_PATH_INFO_1("Old ph checksum: 0x%x",old_ph_cksum);
    /*----------------------------------------------------------
    Remove checksum for old psuedo header
    -----------------------------------------------------------*/
    old_cksum = cksum_sub(old_cksum, (uint16)old_ph_cksum);
  }

  LOG_MSG_DATA_PATH_INFO_1("Wihtout old ph checksum: 0x%x",
                           old_cksum);
    
  /*------------------------------------------------------------
    Calculate checksum for new psuedo header
  -------------------------------------------------------------*/
  if(new_ph != NULL)
  {
    for(i=0 ; i< 8; i++)
    ...
    new_ph_cksum = ~eac(new_ph_cksum) & 0xffff;

    LOG_MSG_DATA_PATH_INFO_1("New ph checksum: 0x%x",new_ph_cksum);
  
    old_cksum = cksum_add(old_cksum,new_ph_cksum);
  }

  LOG_MSG_DATA_PATH_INFO_1("Final checksum: 0x%x",old_cksum);
...
void cksum_offload_init(void)
{
  const char * item_file_path = 
     {PS_NV_CONF_ITEM_INFO(PS_NV_CONF_CKSUM_OFFLOAD_ENABLE)};
  boolean cksum_is_offload = FALSE;

  if (0 != ps_read_efs_nv(item_file_path, 
                          &cksum_is_offload,
                          sizeof(cksum_is_offload)))
  { 
    LOG_MSG_ERROR_0("Unable to read cksum offload flag, using default (TRUE)");
    cksum_is_offload = TRUE;          
  }

  if(TRUE == cksum_is_offload)
  {
    LOG_MSG_INFO1_0("Checksum offloaded!");
    (void)ps_dpm_offload_checksum();
  }
}
