...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/protocols/inet/src/ps_ip4_frag.c#2 $
  $DateTime: 2019/10/22 02:53:17 $
  $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
04/16/19    rj     Free the DSM item for the malformed fragmented packet
...
)
{
  ...
  while( fragment_offset < payload_length )
  {
    ...
    -----------------------------------------------------------------------*/ 
    PS_DSM_SET_TIMESTAMP(fragment_ptr,PS_GET_TIMESTAMP_3_4_MS());  
...
)
{
  ...
  {
    LOG_MSG_ERROR_1( "ip4_fragment_optimized():"
...
)
{
  ...
  {
    LOG_MSG_ERROR_1("Failed to parse IP packet, free the source DSM item: 0x%x", pkt_ptr);
    /* Free the packet */ 
    dsm_free_packet(&pkt_ptr);
    return NULL;
  }
  ...
  if(ip_fraghdl_put( handle, pkt_ptr, pkt_info ) != 0)
  {
    ...
    DPL_LOG_NETWORK_RX_DROPPED_PACKET(iface_ptr, pkt_ptr, DPL_IID_NETPROT_IP);
    /* Free the packet */
    dsm_free_packet( &pkt_ptr );
    return NULL;
  }
      ...
      0 != datagram_len )
  {
    ...
    while( frag_offset < datagram_len )
    {
      ...
      if( NULL == curr_ptr )
      {
        ...
        DPL_LOG_NETWORK_RX_DROPPED_PACKET(iface_ptr, reas_ptr, DPL_IID_NETPROT_IP);
      ...
      if( FALSE == ps_ip4_hdr_parse( curr_ptr, &payload_offset, &currhdr ) )
      {
        ...
        IPV4_INC_STATS(reasm_fails,1) ;
        DPL_LOG_NETWORK_RX_DROPPED_PACKET(iface_ptr, curr_ptr, DPL_IID_NETPROT_IP);
