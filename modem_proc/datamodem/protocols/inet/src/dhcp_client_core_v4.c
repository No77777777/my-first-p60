...
$Id: //components/rel/data.mpss/3.5..2.1/protocols/inet/src/dhcp_client_core_v4.c#2 $
...
#define DHCP_CLIENT_CORE_v4_ASSERT(expression)  \
          dhcp_client_core_v4_assert_wrapper(__LINE__, expression)
/*===========================================================================

FUNCTION DHCP_CLIENT_CORE_v4_ASSERT_WRAPPER()

DESCRIPTION
  Wrapper function for DHCP_CLIENT_CORE_v4 ASSERT
...
=============================================================================*/
static void dhcp_client_core_v4_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("DHCP_CLIENT_CORE_v4_ERR_FATAL at line:%d ",  
              line_num,
              0,
              0);  
  }
  
}/* dhcp_client_core_v4_assert_wrapper */
...
typedef struct dhcp_client_meta_option {
  struct dhcp_client_meta_option * next_ptr;
...
)
{
  ...
  {
    sip_addr = info->core.sip_addr->next_ptr;
  ...
  {
    sip_dn = info->core.sip_domain_name->next_ptr;
...
)
{
  ...
  if ( NULL == src )
  {
    /* "get_option_data_bytes: src NULL" */
    DHCP_CLIENT_CORE_v4_ASSERT(0);
    return -1;
  }
  ...
  if ( NULL == dst_buf )
  {
    /* "get_option_data_bytes: dst_buf NULL" */
    DHCP_CLIENT_CORE_v4_ASSERT(0);
    return -1;
  }
  ...
  {
    /* "offset less than minimum offset for option data" */
    DHCP_CLIENT_CORE_v4_ASSERT(0);
    return -1;
  }
...
)
{
  ...
  LOG_MSG_DATA_PROTO_INFO_0("Sending DHCPREQUEST message");
  info->diag_event_type = DHCP_CLIENT_EVENT_REQUEST;

  /* Initialize the core */
  DHCP_CLIENT_CORE_v4_ASSERT( NULL == info->core.opt );
  ...
  {
    last_opt = last_opt->next_ptr = info->init.d_malloc(sizeof(dhcp_client_meta_option));
    DHCP_CLIENT_CORE_v4_ASSERT(NULL != last_opt);
  ...
  {
    last_opt = info->core.opt;
    info->core.opt = last_opt->next_ptr;
...
)
{
  ...
  LOG_MSG_DATA_PROTO_INFO_0("Sending DHCPRELEASE message");
...
)
{
  ...
  LOG_MSG_DATA_PROTO_INFO_0("Sending DHCPDISCOVER message");
...
)
{
  ...
  LOG_MSG_DATA_PROTO_INFO_0("Sending DHCPDECLINE message");
  ...
  last_opt = last_opt->next_ptr = info->init.d_malloc(sizeof(dhcp_client_meta_option) + 1);
  DHCP_CLIENT_CORE_v4_ASSERT(NULL != last_opt);
  last_opt->option = DHCP_CLIENT_OPTION_MSG_TYPE;
  last_opt->len = 1; /* Per RFC 2132, 9.3 */
  last_opt->option_txt = (uint8 *) &last_opt[1];
...
)
{
  ...
  LOG_MSG_DATA_PROTO_INFO_0("Sending DHCPINFORM message");
...
)
{
  ...
  if ( NULL == opt )
  {
    /* "Insufficent memory to alloc option link" */
...
)
{
  ...
  while ( NULL != *optptr )
  {
    ...
    {
      optret = *optptr;
      *optptr = optret->next_ptr;
      optret->next_ptr = NULL;
      break;
    }
    optptr = &(*optptr)->next_ptr;
  }
...
)
{
  ...
  LOG_MSG_DATA_PROTO_INFO_0("INFO: OVERLOAD option");
  if ( 1 != optlen )
  {
    /* "FAIL: Corrupt option len" */
    ret = DHCP_CLIENT_OPTION_RET_FAIL;
  }
  if ( 0 != info->overload )
  {
    /* "FAIL: multiple overload options?" */
    ret = DHCP_CLIENT_OPTION_RET_FAIL;
  }
  if( ! info->extract( &info->overload, info->msg, info->offset, 1 ) )
  {
    /* "FAIL: sort read in overload option"*/
    ret = DHCP_CLIENT_OPTION_RET_FAIL;
  }
  if ( DHCP_CLIENT_OPTION_OVERLOAD_FILE > info->overload ||
       DHCP_CLIENT_OPTION_OVERLOAD_FILE_SNAME < info->overload )
  {
    /* "FAIL: overload value out of range %d" */
    ret = DHCP_CLIENT_OPTION_RET_FAIL;
  }

  LOG_MSG_INFO1_3( "INFO: OVERLOAD option: Did overload FAIL? %d " 
                     "option length = %d ; Overload value = %d", 
                     ret, optlen, info->overload);
...
)
{
  ...
  boolean              parse_end = TRUE;
  boolean              parse_pad = TRUE;
  ...
  while ( DHCP_CLIENT_OPTION_RET_SUCCESS == ret )
  {
    ...
    {
      field_size--;
      parse_end = (DHCP_CLIENT_OPTION_END == option);
      parse_pad = (DHCP_CLIENT_OPTION_PAD == option);
      LOG_MSG_INFO1_2("INFO: Parse END? %d ; Parse PAD? %d",
                      parse_end,
                      parse_pad);
    ...
    else
    {
      ...
      if ( NULL == opt )
      {
        ...
        if ( DHCP_CLIENT_OPTION_RET_FAIL == ret )
        {
          LOG_MSG_ERROR_0("FAIL: dhcpi_client_option_alloc_link() failed");
          break;
        }
...
)
{
  ...
  {
    /*"FAIL: Invalid op field 0x%x", info->core.op */
    ret = DHCP_CLIENT_OPTION_RET_FAIL;
  }
  if ( info->core.flags & DHCP_CLIENT_BOOTP_FLAG_MBZ_MASK )
  {
    /* "FAIL: Non-zero MBZ in flags field 0x%x" */
    ret = DHCP_CLIENT_OPTION_RET_FAIL;
  }
  if ( DHCP_MAGIC_NUM != info->core.magic )
  {
    /* "FAIL: bad magic number 0x%lx" */
    ret = DHCP_CLIENT_OPTION_RET_FAIL;
  }

  LOG_MSG_INFO1_4( "INFO: Parsing bootp 'option': Did Parsing FAIL? %d "
                   "op field = %d ; flags field = %d ; magic number = %d", 
                   ret, info->core.op, info->core.flags, info->core.magic);
...
)
{
  ...
  {
    /* "option not DHCP_CLIENT_OPTION_SUBNET_MASK" */
    DHCP_CLIENT_CORE_v4_ASSERT(0);
    return FALSE;
  }
...
)
{
  ...
  {
    /* "option not DHCP_CLIENT_OPTION_ROUTER" */
    DHCP_CLIENT_CORE_v4_ASSERT(0);
    return FALSE;
  }
...
)
{
  ...
  {
    /* "option not DHCP_CLIENT_OPTION_DNS" */
    DHCP_CLIENT_CORE_v4_ASSERT(0);
    return FALSE;
  }
  DHCP_CLIENT_CORE_v4_ASSERT(NULL == info->core.dns);
...
)
{
  ...
  {
    /* "option not DHCP_CLIENT_OPTION_DOMAIN_NAME" */
    DHCP_CLIENT_CORE_v4_ASSERT(0);
    return FALSE;
  }
  ...
  {
    /* "Failed to malloc for domain name of len %d",
                     info->core.opt->len */
    DATA_PROTO_ERROR_LOG_0(PS_MEM_ALLOC_ERR);
    return FALSE;
  }
...
)
{
  ...
  {
    /* "option not DHCP_CLIENT_OPTION_MTU" */
    DHCP_CLIENT_CORE_v4_ASSERT(0);
    return FALSE;
  }
...
)
{
  ...
  {
    /* "option not DHCP_CLIENT_OPTION_IP_LEASE" */
    DHCP_CLIENT_CORE_v4_ASSERT(0);
    return FALSE;
  }
...
)
{
  ...
  {
    /* "option not DHCP_CLIENT_OPTION_MSG_TYPE" */
    DHCP_CLIENT_CORE_v4_ASSERT(0);
    return FALSE;
  }
...
)
{
  ...
  {
    /* "option not DHCP_CLIENT_OPTION_SERVER_ID" */
    DHCP_CLIENT_CORE_v4_ASSERT(0);
    return FALSE;
  }
...
)
{
  ...
  {
    /* "option not DHCP_CLIENT_OPTION_RENEW_TIME" */
    DHCP_CLIENT_CORE_v4_ASSERT(0);
    return FALSE;
  }
...
)
{
  ...
  {
    /* "option not DHCP_CLIENT_OPTION_REBIND_TIME" */
    DHCP_CLIENT_CORE_v4_ASSERT(0);
    return FALSE;
  }
...
)
{
  ...
  {
    /* "option not DHCP_CLIENT_OPTION_MESSAGE" */
    DHCP_CLIENT_CORE_v4_ASSERT(0);
    return FALSE;
  }
...
)
{
  ...
  {
    /* "option not DHCP_CLIENT_OPTION_SIP" */
    DHCP_CLIENT_CORE_v4_ASSERT(0);
    return FALSE;
  }

  /* sip lists should be NULLed out from last message received. */
  DHCP_CLIENT_CORE_v4_ASSERT(NULL == info->core.sip_addr);
  DHCP_CLIENT_CORE_v4_ASSERT(NULL == info->core.sip_domain_name);
    ...
    case 0: /* content encoding: Domain names */
      ...
      else
      {
        ...
        while( offset != opt->len + DHCP_CLIENT_MAX_OPTION_HEADER_LEN )
        {
          ...
          tmp_dn->next_ptr = info->core.sip_domain_name;
          ...
          else
          {
             ...
             * falls here. */
            DHCP_CLIENT_CORE_v4_ASSERT( expandret > 0 );
            offset = (unsigned int) expandret;

            DHCP_CLIENT_CORE_v4_ASSERT( offset <= opt->len + DHCP_CLIENT_MAX_OPTION_HEADER_LEN );
          }
        } /* while (offset != opt->len + DHCP_CLIENT_MAX_OPTION_HEADER_LEN) */
      }
      break;
    case 1: /* content encoding: IP addresses */
      ...
      else
      {
        ...
        for ( i = 0; i < num_addrs; i++ )
        {
          ...
          /* list is appended from front */
          tmp_addr->next_ptr = info->core.sip_addr;
...
)
{
  ...
  {
    /* "option not DHCP_CLIENT_OPTION_CLIENT_ID" */
    DHCP_CLIENT_CORE_v4_ASSERT(0);
    return FALSE;
  }
  ...
  {
    /* "NULL client_id type and value found" */
    DATA_PROTO_ERROR_LOG_0(PS_BAD_ARG_OR_NULL_PTR);
    return FALSE;
  }
  ...
  DHCP_CLIENT_CORE_v4_ASSERT(NULL != info->core.client_id);
...
{
  DHCP_CLIENT_CORE_v4_ASSERT (NULL != info);
...
)
{
  ...
  DHCP_CLIENT_CORE_v4_ASSERT(info != NULL); /* added the '!= NULL' part to take care of compilation error */
...
)
{
  ...
  LOG_MSG_DATA_PROTO_INFO_0( "INFO: --- DHCP CLIENT CORE INIT ---");
  ...
  if (( NULL == init->core_params.hw_addr &&
      ...
      ( NULL != init->core_params.hw_addr &&
        0 == init->core_params.hw_addr_len ))
  {
    /* "hw_addr does not match hw_addr_len" */
    DHCP_CLIENT_CORE_v4_ASSERT(0);
    return NULL;
  }
  ...
  if ( NULL == handle )
  {
    /* "Insufficent memory to init DHCP core" */
    DHCP_CLIENT_CORE_v4_ASSERT(0);
    return NULL;
  }
...
)
{
  ...
  LOG_MSG_DATA_PROTO_INFO_0( "INFO: --- DHCP CLIENT CORE START ---");
...
)
{
  ...
  LOG_MSG_DATA_PROTO_INFO_0( "INFO: --- DHCP CLIENT CORE STOP ---");
  *handle = NULL;
} /* dhcp_clienti_core_stop */
