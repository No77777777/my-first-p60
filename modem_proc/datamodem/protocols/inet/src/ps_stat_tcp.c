...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/protocols/inet/src/ps_stat_tcp.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
{
  uint8                               * log_pkt_ptr            = NULL;
  ps_stat_global_tcp_log_pkt_type       global_tcp_log_pkt ;
  ps_stat_tcp_enum_type                 stat_index = 
                                          PS_STAT_TCP_CRNT_OPEN_CONNS ;
  uint64                                total_len = 0;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  memset(&global_tcp_log_pkt, 0x0, sizeof(ps_stat_global_tcp_log_pkt_type));
  ...
  (void) ps_stat_get_tcp(PS_STAT_TCP_ALL,
                         NULL,
                         &(global_tcp_log_pkt.global_tcp_stats),
                         sizeof(ps_stat_tcp_g_s_type));

  while( ps_stat_tcp_g_table[stat_index].len != 0 )
  {
    if( sizeof(uint64) == ps_stat_tcp_g_table[stat_index].len )
    {
      hput64( ((byte *)log_pkt_ptr + total_len ),
              *(uint64*)((uint8 *)&(global_tcp_log_pkt.global_tcp_stats) + 
               ps_stat_tcp_g_table[stat_index].offset) );
    }
    else
    {
      hput32( ((byte *)log_pkt_ptr + total_len ),
              *(uint32*)((uint8 *)&(global_tcp_log_pkt.global_tcp_stats) + 
               ps_stat_tcp_g_table[stat_index].offset) );
    }

    total_len += ps_stat_tcp_g_table[stat_index].len;
    stat_index++;
  }

  ps_stat_logging_commit_log_pkt ( log_pkt_ptr, total_len );
