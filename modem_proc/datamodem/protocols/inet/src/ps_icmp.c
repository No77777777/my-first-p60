...
/*===========================================================================
 ...
 $Header: //components/rel/data.mpss/3.5..2.1/protocols/inet/src/ps_icmp.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if(dsm_pullup(&item_ptr, NULL, offset) != offset )
  {
    ...
    LOG_MSG_INFO2_0( "Truncated packet received, dropping packet" );
  ...
  {
    LOG_MSG_INFO3_0( "ICMP Broadcast Rx'd, dropping packet");
    DPL_LOG_NETWORK_RX_DROPPED_PACKET(iface_ptr, item_ptr, DPL_IID_NETPROT_IP);
    dsm_free_packet( &item_ptr );
    return;
  }
  ...
  if( cksum_value != 0)
  {
    ...
    ICMPV4_INC_STATS(msgs_dropped_rx, 1) ;
    LOG_MSG_INFO1_0("Invalid ICMP Checksum, dropping packet");
    DPL_LOG_NETWORK_RX_DROPPED_PACKET(iface_ptr, item_ptr, DPL_IID_NETPROT_IP);
    dsm_free_packet( &item_ptr);
    return;
  }
  ...
  {
    ICMPV4_INC_STATS(msgs_dropped_rx, 1) ;
    LOG_MSG_INFO1_0("icmp_hdr_parse conversion err, dropping packet");
    DPL_LOG_NETWORK_RX_DROPPED_PACKET(iface_ptr, item_ptr, DPL_IID_NETPROT_IP);
    dsm_free_packet( &item_ptr);
    return;
  }
  ...
  default:
    ...
    if (dssicmp_ps_input( &icmp_hdr, item_ptr, ip_pkt_info_ptr ) == DSS_ERROR)
    {
      ...
      ICMPV4_INC_STATS(msgs_dropped_rx, 1);
      LOG_MSG_INFO1_1( "Unsupported ICMP message %d, dropping packet", type);
      DPL_LOG_NETWORK_RX_DROPPED_PACKET(iface_ptr, item_ptr, DPL_IID_NETPROT_IP);
      dsm_free_packet( &item_ptr);
    }
    return;
  ...
  {
    ICMPV4_INC_STATS(msgs_dropped_tx, 1);
    LOG_MSG_INFO1_0("icmp_hdr_create() conversion err, dropping packet");
    DPL_LOG_NETWORK_RX_DROPPED_PACKET(iface_ptr, item_ptr, DPL_IID_NETPROT_IP);
    dsm_free_packet( &item_ptr);
    return;
  }
