...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/protocols/inet/src/ps_ip.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
)
{
  ...
  ps_iface_type           *iface_ptr = NULL;          /* iface pointer */
#if 0
#ifdef FEATURE_SEC_IPSEC
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if( NULL == ip_pkt_info_ptr )
  {
    DATA_PROTO_ERROR_LOG_0(PS_BAD_ARG_OR_NULL_PTR);
    return;
  }
  iface_ptr = ip_pkt_info_ptr->if_ptr;
  /*-------------------------------------------------------------------------
  ...
  if (ip_pkt_info_ptr->ip_vsn == IP_V4)
  {
    ...
    if (ip_pkt_info_ptr->ip_hdr.v4.length > dsm_length_packet(bp))
    {
      ...
      DPL_LOG_NETWORK_RX_DROPPED_PACKET(iface_ptr, bp, DPL_IID_NETPROT_IP);
  ...
  if(ipsec_handle != NULL)
  {
    ...
    if (  if_name == IPSEC_IFACE ) {
      LOG_MSG_ERROR_0("Filtering failure on the packet which has come on IPsec Iface: Drop the Packet");
      PS_RX_META_INFO_FREE(&meta_info_ptr);
      dsm_free_packet(&bp);
      return;
    }
    ...
    default:
      ...
      else
      {
        ...
        *-----------------------------------------------------------------*/
        ...
        LOG_MSG_ERROR_0("IPv6 packet, ignored");
        DPL_LOG_NETWORK_RX_DROPPED_PACKET(iface_ptr, bp, DPL_IID_NETPROT_IP);
        dsm_free_packet(&bp);
      }
#endif /* FEATURE_DATA_PS_IPV6 */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch(optname)
  {
    ...
    case ds_Sock_OptName_QDS_IP_RECVTTL:
    case ds_Sock_OptName_QDS_IP_RECVECN:
#ifdef FEATURE_DATA_PS_IPV6
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  {
    DATA_PROTO_ERROR_LOG_3(PS_BAD_ARG_OR_NULL_PTR, meta_info_ptr, 
  ...
  {
    LOG_MSG_DATA_PATH_INFO_1( "ps_ip_udpate_ipsec_info_in_metainfo(): "
