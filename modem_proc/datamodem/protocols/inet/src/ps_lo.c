...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/protocols/inet/src/ps_lo.c#2 $
  $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
13/12/17    my     CR 2157129 : FR 42790: F3 reduction
10/22/15   kc     Added logging for dropped packets 
11/04/14   kc      Added lo_powerup_deinit() and FR21288 ev reg optimization
...
#define PS_LO_ASSERT(expression)  \
          ps_lo_assert_wrapper(__LINE__, expression)
...
/*---------------------------------------------------------------------------
  Parameters used for updating the loopback routing tables.
---------------------------------------------------------------------------*/
static void *ps_lo_iface_event_handle;
...
FUNCTION ps_lo_assert_wrapper

DESCRIPTION
  Wrapper function for PS_LO ASSERT
...
=============================================================================*/
static void ps_lo_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("PS_LO_ERR_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
  
}/* ps_lo_assert_wrapper */
...
{
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ps_iface_event_enum_type ps_lo_iface_event_reg_array[3];

  /*-------------------------------------------------------------------------
    Register for iface events used to update loopback routing tables.
  -------------------------------------------------------------------------*/
  /*-----------------------------------------------------------------------------
    Alloc and register for IFACE_UP_EV, IFACE_DOWN_EV and IFACE_ADDR_CHANGED_EV
  -----------------------------------------------------------------------------*/
  ps_lo_iface_event_handle = 
    (void*)ps_iface_alloc_event_handle(NULL, ps_lo_iface_event_cback, NULL);

  if (NULL == ps_lo_iface_event_handle)
  {
  ...
  ps_lo_iface_event_reg_array[0] = IFACE_UP_EV;
  ps_lo_iface_event_reg_array[1] = IFACE_DOWN_EV;
  ps_lo_iface_event_reg_array[2] = IFACE_ADDR_CHANGED_EV;

  if (-1 == ps_iface_subscribe_event_list(
             ps_lo_iface_event_handle,
             ps_lo_iface_event_reg_array,
             3))
  {
...
/*===========================================================================
FUNCTION LO_POWERUP_DEINIT()

DESCRIPTION
  This function is called at powerdown to destroy
...
)
{
  if(NULL != ps_lo_iface_event_handle)
  {
    ps_iface_free_event_handle(ps_lo_iface_event_handle);
    ps_lo_iface_event_handle = NULL;
  }
} /* lo_powerup_deinit() */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  -------------------------------------------------------------------------*/
  v6_addr_ptr = PS_IFACE_GET_V6_PRIMARY_ADDR_PTR(&lo_iface6);
  v6_addr_ptr->prefix = 0;
  v6_addr_ptr->iid = ps_in6addr_loopback.ps_s6_addr64[1];
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_INFO1_1("Invalid iface state %d. Dropping packet", iface_state);
    DPL_LOG_NETWORK_TX_DROPPED_PACKET(this_iface_ptr, *pkt_chain_ptr, DPL_IID_NETPROT_IP);
    dsm_free_packet(pkt_chain_ptr);
    return -1;
  }
