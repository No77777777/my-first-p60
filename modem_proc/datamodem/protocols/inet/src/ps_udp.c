...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/protocols/inet/src/ps_udp.c#3 $ $DateTime: 2020/01/21 03:29:52 $ $Author: pwbldsvc $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ( udp_hdr.chksum != 0)
  {
    ...
    /*-----------------------------------------------------------------------
      ...
      (DevRevPDU) then queue and tell UI a UP client UDP packet is
       available.
    -----------------------------------------------------------------------*/
    calc_checksum = cksum_ex(ip_pkt_info_ptr,
  ...
  /*------------------------------------------------------------------------- 
    Fill Ancilary informaition (ECN & TTL/Hop Limit) for Socket layer.
  -------------------------------------------------------------------------*/  
  switch (ip_pkt_info_ptr->ip_vsn)
  {
    case IP_V4:
      read_info.ttl = ip_pkt_info_ptr->ip_hdr.v4.ttl;
      read_info.ecn = ip_pkt_info_ptr->ip_hdr.v4.tos & 0x03;
      break;
    case IP_V6:
      read_info.ttl = ip_pkt_info_ptr->ip_hdr.v6.hdr_body.base_hdr.hop_limit;
      read_info.ecn = ip_pkt_info_ptr->ip_hdr.v6.hdr_body.base_hdr.trf_cls & 0x03;
      break;
    default:
      LOG_MSG_ERROR_1("Invalid IP version (%d) while filling ancilary info",
  ...
  read_info.subs_id = PS_IFACE_GET_SUBS_ID((ps_iface_type *)(ip_pkt_info_ptr->if_ptr));
