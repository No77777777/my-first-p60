...
$Id: //components/rel/data.mpss/3.5..2.1/protocols/inet/src/dhcp_client_timer.c#2 $
...
#define DHCP_CLIENT_TIMER_ASSERT(expression)  \
          dhcp_client_timer_assert_wrapper(__LINE__, expression)
/*===========================================================================

FUNCTION DHCP_CLIENT_TIMER_ASSERT_WRAPPER()

DESCRIPTION
  Wrapper function for DHCP_CLIENT_TIMER ASSERT
...
=============================================================================*/
static void dhcp_client_timer_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("DHCP_CLIENT_TIMER_ERR_FATAL at line:%d ",  
              line_num,
              0,
              0);  
  }
  
}/* dhcp_client_timer_assert_wrapper */
...
)
{
  ...
  LOG_MSG_DATA_PROTO_INFO_0( "INFO: --- DHCP CLIENT TIMER INIT ---" );
  ...
  if ( NULL == handle )
  {
    /* "Insufficent memory to init DHCP timer" */
    DHCP_CLIENT_TIMER_ASSERT(0);
    return NULL;
  }
...
)
{
  ...
  LOG_MSG_DATA_PROTO_INFO_0( "INFO: --- DHCP CLIENT TIMER START ---" );
  /* Sanity check parameters */
  if (NULL == info)
  {
    /* "info NULL?" */
...
)
{
  ...
  LOG_MSG_DATA_PROTO_INFO_0( "INFO: --- DHCP CLIENT TIMER FREE ---" );
  *handle = NULL;
} /* dhcp_clienti_timer_free */
