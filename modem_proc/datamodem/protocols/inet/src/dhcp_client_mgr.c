...
$Id: //components/rel/data.mpss/3.5..2.1/protocols/inet/src/dhcp_client_mgr.c#2 $
...
#define DHCP_CLIENT_MGR_ASSERT(expression)  \
          dhcp_client_mgr_assert_wrapper(__LINE__, expression)
/*===========================================================================

FUNCTION DHCP_CLIENT_MGR_ASSERT_WRAPPER()

DESCRIPTION
  Wrapper function for DHCP_CLIENT_MGR ASSERT
...
=============================================================================*/
static void dhcp_client_mgr_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("DHCP_CLIENT_MGR_ERR_FATAL at line:%d ",  
              line_num,
              0,
              0);  
  }
  
}/* dhcp_client_mgr_assert_wrapper */
...
)
{
  if ( NULL == handle )
  {
    /* "FAIL: verify handle got a null handle" */
    DHCP_CLIENT_MGR_ASSERT(0);
    return FALSE;
  }
...
)
{
  ...
  LOG_MSG_DATA_PROTO_INFO_1( "dhcpi_client_stop: called with %p", handle );
...
)
{
  ...
  {
    /* "params->hw_addr does not match hw_addr_len" */
    DHCP_CLIENT_MGR_ASSERT(0);
    return;
  }
  ...
  configuration_init.userdata  = NULL;
...
)
{
  ...
  if ( NULL == status_cb )
  {
    /* "dhcp_start with NULL status_cb" */
    DHCP_CLIENT_MGR_ASSERT(0);
    return NULL;
  }
  ...
  if (( 0 < params->hw_addr_len && NULL == params->hw_addr ) ||
      ( 0 == params->hw_addr_len && NULL != params->hw_addr ))
  {
    /* "params->hw_addr NULL for non-zero hw_addr_len" */
    DHCP_CLIENT_MGR_ASSERT(0);
    return NULL;
  }
  ...
  if ( NULL == start )
  {
    /* "dhcp_start unable to get parameter memory" */
    DHCP_CLIENT_MGR_ASSERT(0);
    return NULL;
  }
  ...
  if ( NULL == info )
  {
    /* "dhcp_start unable to get handle memory"  */
    dhcpi_client_mem_free(start); 
    DHCP_CLIENT_MGR_ASSERT(0);
    return NULL;
  }
...
)
{
  ...
  {
    /* "dhcp_client_renew called with NULL handle" */
    DHCP_CLIENT_MGR_ASSERT(0);
    return;
  }
  ...
  if ( NULL == renew )
  {
    /* "dhcp_client_renew unable to get parameter memory" */
    DHCP_CLIENT_MGR_ASSERT(0);
    return;
  }
...
)
{
  ...
  if ( NULL == dhcp_client_handle )
  {
    /* "dhcp_client_release called with NULL handle" */
    DHCP_CLIENT_MGR_ASSERT(0);
    return;
  }
  ...
  if ( NULL == release )
  {
    /* "dhcp_client_release unable to get parameter memory" */
    DHCP_CLIENT_MGR_ASSERT(0);
    return;
  }
...
)
{
  ...
  if ( NULL == dhcp_client_handle )
  {
    /* "dhcp_client_inform called with NULL handle" */
    DHCP_CLIENT_MGR_ASSERT(0);
    return;
  }
  ...
  if ( NULL == inform )
  {
    /* "dhcp_client_inform unable to get parameter memory" */
    DHCP_CLIENT_MGR_ASSERT(0);
    return;
  }
...
)
{
  ...
  DHCP_CLIENT_MGR_ASSERT( NULL != dhcp_client_handle );

  if ( NULL == *dhcp_client_handle )
  {
    /* "dhcp_stop called with NULL handle" */
    DATA_PROTO_ERROR_LOG_0(PS_BAD_ARG_OR_NULL_PTR);
    return;
  }
  ...
  if ( NULL == stop )
  {
    /* "dhcp_stop unable to get parameter memory" */
    DHCP_CLIENT_MGR_ASSERT(0);
    return;
  }
...
)
{
  ...
  if ( NULL == dhcp_client_handle )
  {
    /* "dhcp_client_is_dhcp_in_progress called with NULL handle" */
    DHCP_CLIENT_MGR_ASSERT(0);
    return FALSE;
  }
