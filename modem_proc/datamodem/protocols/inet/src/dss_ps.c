...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/protocols/inet/src/dss_ps.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $

when        who    what, where, why
--------    ---    ----------------------------------------------------------
04/01/16    vl     Moved IPv6 related NV item to ps_ip6_sm.c file.
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch (ps_cmd_ptr->kind)
  {
    ...
    case PS_SOCKET_CMD_CONNECT:
      if (sp->ps_connect != NULL)
      {
        sp->ps_connect( ps_cmd_ptr->pcb_handle,
                     ps_cmd_ptr->sockfd,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
 ...
 /*-------------------------------------------------------------------------
  ...
  nv_status = ps_get_nv_item_ext( NV_TCP_GRACEFUL_DORMANT_CLOSE_I,
                                  ps_nv_item_ptr,
                                  ds707_curr_subs_id() );
#else
  ...
  -------------------------------------------------------------------------*/
#ifdef FEATURE_DATA_IS707
  nv_status = ps_get_nv_item_ext(NV_TCP_KEEPALIVE_IDLE_TIME_I, ps_nv_item_ptr,
                                 ds707_curr_subs_id());
#else
