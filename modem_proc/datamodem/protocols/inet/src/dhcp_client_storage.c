...
$Id: //components/rel/data.mpss/3.5..2.1/protocols/inet/src/dhcp_client_storage.c#2 $
...
#define DHCP_CLIENT_STORAGE_ASSERT(expression)  \
          dhcp_client_storage_assert_wrapper(__LINE__, expression)
/*===========================================================================

FUNCTION DHCP_CLIENT_STORAGE_ASSERT_WRAPPER()

DESCRIPTION
  Wrapper function for DHCP_CLIENT_STORAGE ASSERT
...
=============================================================================*/
static void dhcp_client_storage_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("DHCP_CLIENT_STORAGE_ERR_FATAL at line:%d ",  
              line_num,
              0,
              0);  
  }
  
}/* dhcp_client_storage_assert_wrapper */
...
)
{
  ...
  DHCP_CLIENT_STORAGE_ASSERT( TRUE == info->efs_valid );
                                   ...
                                   MCFG_FS_SUBID_NONE)) )
  {
    ...
    if ( MCFG_FS_STATUS_OK != mcfg_fs_delete(lease_filename,
                                             ...
                                             MCFG_FS_SUBID_NONE) )
    {
      /* "Write_recover, unable to delete file!" */
      DHCP_CLIENT_STORAGE_ASSERT(0);
      /* What else can we do? */
    }
  }
  else
...
)
{
  ...
  LOG_MSG_DATA_PROTO_INFO_0( "INFO: --- DHCP CLIENT STORAGE INIT ---" );
  ...
  if ( NULL == handle )
  {
    /* "Insufficent memory to init DHCP storage layer" */
    DHCP_CLIENT_STORAGE_ASSERT(0);
    return NULL;
  }
...
)
{
  ...
  LOG_MSG_DATA_PROTO_INFO_0( "INFO: --- DHCP CLIENT STORAGE START ---" );
...
)
{
  ...
  LOG_MSG_DATA_PROTO_INFO_0( "INFO: --- DHCP CLIENT STORAGE STOP ---" );
  *handle = NULL;
} /* dhcp_clienti_storage_stop */
