...
$Id: //components/rel/data.mpss/3.5..2.1/protocols/inet/src/dhcp_core_v4.c#2 $

when        who    what, where, why
--------    ---    ----------------------------------------------------------
12/13/17    shrvn  F3 Reduction: Wrapper function for ASSERTS
...
#define DHCP_CORE_v4_ASSERT(expression)  \
          dhcp_core_v4_assert_wrapper(__LINE__, expression)
/*===========================================================================

FUNCTION DHCP_CORE_v4_ASSERT_WRAPPER()

DESCRIPTION
  Wrapper function for DHCP_CORE_v4 ASSERT
...
=============================================================================*/
static void dhcp_core_v4_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("DHCP_CORE_v4_ERR_FATAL at line:%d ",  
              line_num,
              0,
              0);  
  }
  
}/* dhcp_core_v4_assert_wrapper */
...
typedef struct dhcp_meta_option {
  struct dhcp_meta_option * next_ptr;
...
)
{
  ...
  else
  {
    ...
    opt->len = len;
    opt->next_ptr = info->gather_options;
...
)
{
  ...
  while ( DHCP_OPTION_RET_SUCCESS == ret )
  {
    ...
    {
      field_size--;
      parse_end = (DHCP_OPTION_END == option);
      parse_pad = (DHCP_OPTION_PAD == option);
...
)
{
  ...
  DHCP_CORE_v4_ASSERT(NULL != handle);
  DHCP_CORE_v4_ASSERT(NULL != buffer);
  DHCP_CORE_v4_ASSERT(NULL != *buffer);
  DHCP_CORE_v4_ASSERT(NULL != data);
...
)
{
       ...
       DHCP_BOOTP_SIZE + info->output_option_size + 1 )
  {
    ...
    {
      /* "INFO: unable to malloc memory for option field" */
      DATA_PROTO_ERROR_LOG_0(PS_MEM_ALLOC_ERR);
    }
  }
...
)
{
  ...
  while ( DHCP_OPTION_RET_SUCCESS == ret && NULL != info->gather_options)
  {
    ...
    info->gather_options = info->gather_options->next_ptr;
    info->current_option->next_ptr = NULL;
    ...
    else
    {
      ...
      while ( NULL != info->current_option )
      {
        ...
        tmp = info->current_option->next_ptr;
        /* add current option to output option list */
        info->current_option->next_ptr = info->output_options;
...
)
{
  ...
  if ( DHCP_OPTION_OVERLOAD_FILE > info->overload ||
       DHCP_OPTION_OVERLOAD_FILE_SNAME < info->overload )
  {
    /* "FAIL: overload value out of range %d", info->overload */
    ret = DHCP_OPTION_RET_FAIL;
  }
...
)
{
  ...
  case DHCP_DHCPREQUEST:
         ...
         info->config.lease_get( info->config.lease_handle, &info->core ) )
    {
      if ( DHCP_DHCPNACK == info->core.dhcp_msg_reply )
      {
        ...
        {
          opt = info->gather_options;
          info->gather_options = opt->next_ptr;
          info->init.d_free( opt );
        }
   ...
   */
    DHCP_CORE_v4_ASSERT(NULL != info->config.addr_info_server_id );
    ...
    {
      LOG_MSG_INFO1_1("DHCP INFORM received with ciaddr 0x%x ; DHCP sending ACK",
                      info->core.ciaddr );
...
)
{
  ...
  DHCP_CORE_v4_ASSERT(NULL != info);
  DHCP_CORE_v4_ASSERT(NULL != info->config.addr_info_netmask );
  ...
  info->current_option->next_ptr =
    info->init.d_malloc(sizeof(dhcp_meta_option) + 4);
  if ( NULL != info->current_option->next_ptr )
  {
    netmask = info->config.addr_info_netmask(info->config.addr_info_handle);
    info->current_option->next_ptr->next_ptr = NULL;
    info->current_option->next_ptr->option = DHCP_OPTION_SUBNET_MASK;
    info->current_option->next_ptr->option_txt =
      (uint8*)&info->current_option->next_ptr[1];
    memscpy( info->current_option->next_ptr->option_txt,4, &netmask, 4);
    info->current_option->next_ptr->len = 4; /* Per RFC2132 */
    ret = DHCP_OPTION_RET_SUCCESS;
  }
  else
  {
    /* "Failed to alloc memory for subnetmask option" */
    DATA_PROTO_ERROR_LOG_0(PS_MEM_ALLOC_ERR);
  }
  return ret;
} /* dhcpi_option_subnet_mask_router_handler */
...
)
{
  ...
  if(NULL == info)
  {
    /* "dhcp_option_parameter_req_handler(): Info pointer is NULL" */
    DHCP_CORE_v4_ASSERT(0);
  ...
  {
    /* "dhcp_option_parameter_req_handler(): current_option pointer is NULL" */
    DHCP_CORE_v4_ASSERT(0);
    return DHCP_OPTION_RET_FAIL;
  }
  ...
  {
    /* "dhcp_option_parameter_req_handler(): current_option text is NULL" */
    DHCP_CORE_v4_ASSERT(0);
    info->init.d_free(info->current_option);
    info->current_option = NULL;
    return DHCP_OPTION_RET_FAIL;
...
)
{
  ...
  {
    LOG_MSG_ERROR( "Invalid MAX SIZE option length %d", 
                    info->current_option->len, 0, 0);
  }
  info->init.d_free(info->current_option);
  info->current_option = NULL;
  return DHCP_OPTION_RET_SUCCESS;
...
)
{
  ...
  {
    /* "Allocation of dhcp_core_v4_temp failed" */
    DATA_PROTO_ERROR_LOG_0(PS_MEM_ALLOC_ERR);
    return DHCP_OPTION_RET_FAIL;
  }
...
)
{
  ...
  DHCP_CORE_v4_ASSERT(NULL != init);
  DHCP_CORE_v4_ASSERT(NULL != init->d_malloc);
  DHCP_CORE_v4_ASSERT(NULL != init->d_free);
...
)
{
  ...
  /* sanity check info */
  DHCP_CORE_v4_ASSERT(NULL != core);
  DHCP_CORE_v4_ASSERT(DHCP_CORE_COOKIE == core->cookie);
  DHCP_CORE_v4_ASSERT(NULL != config);
...
)
{
  ...
  {
    /* "dhcp_core_stop(): handle or *handle ptr is NULL" */
    DHCP_CORE_v4_ASSERT(0);
    return ;
  }
...
)
{
  ...
  DHCP_CORE_v4_ASSERT(NULL != info);
  DHCP_CORE_v4_ASSERT(info->cookie_ptr == info);
  DHCP_CORE_v4_ASSERT(DHCP_CORE_COOKIE == info->cookie);
  DHCP_CORE_v4_ASSERT(NULL != msg);
  DHCP_CORE_v4_ASSERT(NULL != extract);
  /* Zero the message info to clear any previous info */
  memset( &info->core, 0, sizeof( dhcp_core_msg_info ) );
  info->output_option_size = 0;
  info->output_option_max_size = DHCP_MAX_MAX_REPLY_LEN;
  DHCP_CORE_v4_ASSERT( NULL == info->gather_options );
  DHCP_CORE_v4_ASSERT( NULL == info->current_option );
  DHCP_CORE_v4_ASSERT( NULL == info->output_options );
  ...
  {
    opt = info->output_options->next_ptr;
    info->init.d_free(info->output_options);
    info->output_options = opt;
  }
  while( NULL != info->gather_options )
  {
    opt = info->gather_options->next_ptr;
