...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/protocols/inet/src/dssicmp.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
#define DSSICMP_ASSERT(expression)  \
          dssicmp_assert_wrapper(__LINE__, expression)
...
FUNCTION dssicmp_assert_wrapper

DESCRIPTION
  Wrapper function for DSSICMP ASSERT
...
=============================================================================*/
static void dssicmp_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("DSSICMP_ERR_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
  
}/* dssicmp_assert_wrapper */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (NULL == icmp_cb_ptr)
  {
    /* "dssicmpi_free_icmp_cb(): NULL icmp_cb_ptr" */
    DSSICMP_ASSERT(0);
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DSSICMP_ASSERT(SOCKAVAIL != icmp_cb_ptr->sockfd);
