...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/protocols/inet/src/dsstcpi.c#4 $ $DateTime: 2020/03/02 22:28:53 $ $Author: pwbldsvc $


when        who    what, where, why
--------    ---    ----------------------------------------------------------
03/03/20    sg     Code changes to reset the total number of backoffs and 
                   total backoff time once TCP connection is established.
03/02/20    sg     Updated the condition to send as many as 
                   TCP_MAX_KEEPALIVE_PROBES only.
03/29/18    rj     CR 2207344 geniss timer handling
04/17/18    gbk    FR 48727 IMS signaling TCP retry with Access priority
...
#define DSSTCPI_ASSERT(expression)  \
          dsstcpi_assert_wrapper(__LINE__, expression)
...
FUNCTION dsstcpi_assert_wrapper

DESCRIPTION
  Wrapper function for DSSTCPI ASSERT
...
=============================================================================*/
static void dsstcpi_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("DSSTCPI_ERR_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
  
}/* dsstcpi_assert_wrapper */
...
)
{
  ...
  tcb_ptr->access_priority = TCP_ACCESS_PRIORITY_DISABLE;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /*-------------------------------------------------------------------------
    Allocate the geniss timer
    If allocation fails, free any other allocated timers and return failure.
  -------------------------------------------------------------------------*/
  tcb_ptr->ticks_passed = 0;
  tcb_ptr->flag = 1;
  
  timer_handle = ps_timer_alloc(tcp_geniss_timer_expiry_cb, tcb_ptr);
  if (timer_handle == PS_TIMER_INVALID_HANDLE)
  {
    goto bail;
  }
  ...
  {
    tcb_ptr->geniss_timer = timer_handle;
  } 
  
  //Start the geniss timer
  //ps_timeri_start(tcb_ptr->geniss_timer, TCP_GENISS_USEC_TIMER);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  -------------------------------------------------------------------------*/
  DSSTCPI_ASSERT( tcb_ptr->state == TCP_CLOSED);
  ...
  if (PS_TIMER_SUCCESS != ps_timer_free( tcb_ptr->geniss_timer) )
  {
    LOG_MSG_ERROR_1("Error freeing timer 0x%x", tcb_ptr->geniss_timer);
    ASSERT(0);
  }
