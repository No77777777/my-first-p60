...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/protocols/inet/src/ps_ip_addr.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $

when        who    what, where, why
--------    ---    ----------------------------------------------------------
06/12/15    skc    Fixed incorrect preferred lifetime for private address
...
boolean dhcp6_client_rapid_commit;
...
)
{
  ...
  ps_ifacei_v6_addr_type  *temp_addr_ptr = NULL;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if(ip_addr_ptr == NULL)
  {
  ...
  if(ps_iface_find_ipv6_addr(ip_addr_ptr, &v6_addr_ptr, &ps_iface_ptr) ||
     ps_iface_find_ipv6_addr2(ip_addr_ptr, &v6_addr_ptr, &ps_iface_ptr, IPV6_ADDR_TYPE_ALL))
  {
     ...
     /*---------------------------------------------------------------------
      ...
      if (v6_addr_ptr->ref_cnt == 1)
      {
        ...
        v6_addr_ptr->hw_fltr_handle =
          ps_iface_ipfltr_add_v6_dst_addr_fltr(ps_iface_ptr, &v6_addr);
      ...
      {
        temp_addr_ptr = PS_IFACE_GET_V6_PRIMARY_ADDR_PTR(ps_iface_ptr);
        pref_lifetime = ps_timer_remaining(temp_addr_ptr->pref_lifetimer_handle);
      ...
      /*---------------------------------------------------------------------
        Log v6 addr event message
      ---------------------------------------------------------------------*/
      ps_ifacei_v6_addr_log_event_msg(ps_iface_ptr, 
                                      v6_addr_ptr, 
                                      PS_IFACEI_V6_ADDRESS_LOG_EVENT_BOUND);
     ...
     ps_iface_find_ipv6_addr2(ip_addr_ptr, &v6_addr_ptr, &ps_iface_ptr, IPV6_ADDR_TYPE_ALL))
  {
        ...
        (v6_addr_ptr->addr_type == IPV6_ADDR_TYPE_PRIV_SHARED)) )
    {
      if(v6_addr_ptr->ref_cnt > 0)
      {
        ...
        ps_ifacei_v6_addr_log_event_msg(ps_iface_ptr, 
                                        v6_addr_ptr, 
                                        PS_IFACEI_V6_ADDRESS_LOG_EVENT_UNBOUND);
      }
      else
      {
      ...
      if(v6_addr_ptr->ref_cnt == 0)
      {
        ...
        if (PS_IFACE_IPFLTR_INVALID_HANDLE != v6_addr_ptr->hw_fltr_handle)
        {
          ps_iface_ipfltr_delete_v6_dst_addr_fltr(v6_addr_ptr->hw_fltr_handle);
