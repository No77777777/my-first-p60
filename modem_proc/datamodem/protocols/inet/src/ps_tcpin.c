...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/protocols/inet/src/ps_tcpin.c#5 $ $DateTime: 2021/09/28 00:26:21 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
09/28/21   sg      Code changes to add validation check for timer before 
                   cancelling it.
03/02/20   sg      Code changes to reset the keealive probes counter to 0
                   once ACK is recevied indicating active TCP session.
01/14/20   rj      Changes to allocate the ISN secret buffer on the stack 
                   instead of the heap.
01/24/19    rj     Increase the secret key size to 128 bits used for ISN
                   generation.
03/12/18   rj      TCP Sequence Number Randomization fix microsecond generated
01/02/18   rj      FR 33864 (TCP Sequence Number Randomization) changes
11/20/17   gb      TCP List Ptr for socket based on Iface Ptr mapping for 
                   server socket
02/20/15   ss      Reset Backoff counter properly when TS Permitted option is
...
11/26/14   ss      Start persist timer only if required
...
#define TCP_GENISS_USEC_TIMER 4
...
/*===========================================================================
FUNCTION TCP_GENISS_TIMER_EXPIRY_CB

DESCRIPTION
  This function is registered with tcp geniss timer allocation.
  1.never the timer expires microsecond timer for generating iss
  will be reset to zero.
  2. If called explicitly, calculates the ticks passed and stores in 
  the tcb variable ticks_passed.

DEPENDENCIES
  TCP Instance should be valid.

RETURN VALUE
  None

SIDE EFFECTS
  Tcb flags and ticks_passed parameter may change.
===========================================================================*/

void tcp_geniss_timer_expiry_cb
(
  void *user_data
)
{ 
  register struct tcb *tcb_info;
  /*------------------------------------------------------------------------*/

  /* Validate args. */
  ASSERT(user_data);
  /* Fetch TCB instance. */
  tcb_info = (struct tcb *)user_data;
  if(tcb_info == NULL)
  {
    LOG_MSG_ERROR_0("tcp_geniss_timer_expiry_cb:NULL user_data passed for TCP gen-iss");
	return;
  }
  switch(tcb_info->flag)
  {
  	case 0:
			/* Reset the microsecond timer for tcp gen-iss*/ 
			ps_timeri_start(tcb_info->geniss_timer, TCP_GENISS_USEC_TIMER); 
			break;
			
	case 1:
			tcb_info->ticks_passed = (uint32)ps_timeri_remaining(tcb_info->geniss_timer);
			if (tcb_info->ticks_passed >= TCP_GENISS_USEC_TIMER)
				tcb_info->ticks_passed = (tcb_info->ticks_passed)%(TCP_GENISS_USEC_TIMER);
                        tcb_info->ticks_passed = TCP_GENISS_USEC_TIMER - tcb_info->ticks_passed;
			break;
	default:break;		
  }
...
FUNCTION TCP_GEN_RND_ISS)

DESCRIPTION
  This function will generate a randomized initial sequence number 
  using microsecond timer and md5 hash function
...
{ 
  MD5_CTX context;
  uint32 hash_return;
  unsigned char md5_iss_return_value[16], v6_s_d_port_secret[4]={0};
  unsigned char secretkey[16]={0};
  
  tcb->ticks_passed = 0;
  tcb->flag = 1;
  tcp_geniss_timer_expiry_cb(tcb);

  //MD5 hashing
  MD5Init(&context);
  MD5Update(&context, tcb->conn.local.address.ps_s6_addr, 16);
  MD5Update(&context, tcb->conn.remote.address.ps_s6_addr, 16);
  
  v6_s_d_port_secret[0] = (tcb->conn.local.port) & 0x00FF;
  v6_s_d_port_secret[1] = (tcb->conn.local.port>>8) & 0x00FF;
  v6_s_d_port_secret[2] = (tcb->conn.remote.port) & 0x00FF;
  v6_s_d_port_secret[3] = (tcb->conn.remote.port>>8) & 0x0FF;
  MD5Update(&context, v6_s_d_port_secret, 4);

  secutil_get_random(secretkey, 16);
  MD5Update(&context, secretkey, 16);

  MD5Final(md5_iss_return_value, &context);
   
  hash_return = (uint32)((md5_iss_return_value[3]<<24)|(md5_iss_return_value[2]<<16)|(md5_iss_return_value[1]<<8)|(md5_iss_return_value[0]));
  tcb->iss = (uint32)(tcb->ticks_passed + hash_return);
  return tcb->iss;
};
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  tcb->iss = tcp_gen_rnd_iss(tcb);  
  LOG_MSG_INFO1_1("tcp_send_syn: initial generated seqno with FR %u",tcb->iss);
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if( ip_cb != NULL )
  {
    ip_cb->src_ip_addr = pseudo_hdr->dest;
    LOG_MSG_INFO2_0( "TCP RST: Using below source address");
    IPV6_ADDR_MSG(ip_cb->src_ip_addr.ps_s6_addr64);
  }
...
)
{
  ...
  uint32               iface_index = PS_IFACE_INVALID_IFACE_INDEX ;
  boolean tcbptr_found = FALSE;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  while (NULL != listen_tcb_ptr)
  {
    ...
    if (TCP_CLOSED != listen_tcb_ptr->state)
    {
                               ...
                               &listen_tcb_ptr->conn.local.address)))
      {
          ...
          {
            iface_index =  PS_IFACE_GET_IFACE_INDEX(if_ptr);
            LOG_MSG_INFO1_1("dsstcp_lookup_serv_af() iface_index 0x%x,",iface_index);
            if (iface_index!=PS_IFACE_INVALID_IFACE_INDEX)
            {
              if (ps_route_scope_is_iface_idx_set( &listen_tcb_ptr->ipcb.route_scope, iface_index ))
              {
...
{
  return dsstcp_lookup_serv_af(conn, ds_AddrFamily_QDS_AF_UNSPEC,NULL);
}
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  cksum_value = cksum_ex(ip_pkt_info_ptr,
                         &ip_pkt_info_ptr->pseudo_hdr,
                         tcp_seg_ptr,
                         length,
                         offset);
  if(cksum_value != 0)
  ...
  /*-------------------------------------------------------------------------
    In sockets calls, the tcb from dsstcp.c is used.
  -------------------------------------------------------------------------*/
  ...
  else
  {
    ...
    else if(current_connections <= (DSS_MAX_TCBS - TCP_LOW_THRESHOLD))
    {
      ...
      {
        tcb = dsstcp_lookup_serv_af( &conn, ds_AddrFamily_QDS_AF_INET, (ps_iface_type*)ip_pkt_info_ptr->if_ptr);
      }
      else if( ip_pkt_info_ptr->ip_vsn == IP_V6 )
      {
        tcb = dsstcp_lookup_serv_af( &conn, ds_AddrFamily_QDS_AF_INET6,(ps_iface_type*)ip_pkt_info_ptr->if_ptr );
      }
      else
      {
