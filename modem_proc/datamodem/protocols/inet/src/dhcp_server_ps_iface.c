...
$Id: //components/rel/data.mpss/3.5..2.1/protocols/inet/src/dhcp_server_ps_iface.c#3 $

when        who    what, where, why
--------    ---    ----------------------------------------------------------
08/24/20    rj     Skip the packet processing having NULL UDP payload
...
#define DHCP_SERVER_PS_IFACE_ASSERT(expression)  \
          dhcp_server_ps_iface_assert_wrapper(__LINE__, expression)
/*===========================================================================

FUNCTION DHCP_SERVER_PS_IFACE_ASSERT_WRAPPER()

DESCRIPTION
  Wrapper function for DHCP_SERVER_PS_IFACE ASSERT
...
=============================================================================*/
static void dhcp_server_ps_iface_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("DHCP_SERVER_PS_IFACE_ERR_FATAL at line:%d ",  
              line_num,
              0,
              0);  
  }
  
}/* dhcp_server_ps_iface_assert_wrapper */
...
{
  DHCP_SERVER_PS_IFACE_ASSERT(NULL != dest);
  DHCP_SERVER_PS_IFACE_ASSERT(NULL != src);
...
)
{
  ...
  /* Verify sanity */
  DHCP_SERVER_PS_IFACE_ASSERT(NULL != handle);
  DHCP_SERVER_PS_IFACE_ASSERT(NULL != info);
  ...
  if ( NULL == event )
  {
    /* "dhcpi_server_sock_cb unable to get parameter memory" */
    DHCP_SERVER_PS_IFACE_ASSERT(0);
    return;
  }
...
)
{
  ...
  if ( NULL == event )
  {
    /* "Got NULL cmd in sock event cmd" */
    DHCP_SERVER_PS_IFACE_ASSERT(0);
    return;
  }
  ...
  if ( NULL == info )
  {
    /* "Got NULL info in sock callback" */
    DHCP_SERVER_PS_IFACE_ASSERT(0);
    return;
  }
  ...
  {
    while (0 <= (read_bytes = dss_recvfrom_dsm_chain( info->sock,
                                        &dss_packet,
                                        0,
                                        ...
                                        &dss_errno )))
    {
      if(read_bytes <= 0 && DS_EWOULDBLOCK != dss_errno)
      {
        LOG_MSG_ERROR_0("dhcpi_server_sock_event_cmd(): Zero bytes read from"                        " the queue. Skip this packet processing");
  ...
  {
    /* "Got ACCEPT event on client socket" */
    DHCP_SERVER_PS_IFACE_ASSERT(0);
  }
  ...
  if ( DS_WRITE_EVENT & event_mask )
  {
    while ( NULL != (dss_packet = dsm_dequeue(&info->write_wm)) )
    {
      ...
      if ( sizeof(uint32) != dsm_pullup(&dss_packet,
                                        ...
                                        sizeof(uint32)) )
      {
        /* "Unable to pullup addr from packet" */
        DHCP_SERVER_PS_IFACE_ASSERT(0);
        dsm_free_packet( &dss_packet );
        continue;
      }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    /* "dhcp_server_ipfltr_init(): Failed to reg fltr" */
    DHCP_SERVER_PS_IFACE_ASSERT(0);
  }
...
)
{
  ...
  LOG_MSG_DATA_PROTO_INFO_0( "INFO: --- DHCP SERVER INIT ---" );
  /* sanity check info */
  DHCP_SERVER_PS_IFACE_ASSERT(NULL != init);
  DHCP_SERVER_PS_IFACE_ASSERT(NULL != init->d_malloc);
  DHCP_SERVER_PS_IFACE_ASSERT(NULL != init->d_free);
  /* Get some memory for our info */
  server = init->d_malloc(sizeof(dhcp_server_info));
  if ( NULL == server )
  {
    /* "FAIL: Insufficent memory to init DHCP server" */
    DHCP_SERVER_PS_IFACE_ASSERT(0);
    return NULL;
  }
...
)
{
  ...
  LOG_MSG_DATA_PROTO_INFO_0( "INFO: --- DHCP SERVER START ---" );

  /* Sanity check info */
  DHCP_SERVER_PS_IFACE_ASSERT(NULL != handle);
  DHCP_SERVER_PS_IFACE_ASSERT(NULL != config);
  DHCP_SERVER_PS_IFACE_ASSERT(DHCP_SERVER_COOKIE == server->cookie);
  DHCP_SERVER_PS_IFACE_ASSERT(NULL != config->addr_info_server_id);
  ...
  if ( DSS_ERROR != dss_ret )
  {
    ...
    if ( DSS_SUCCESS != dss_ret )
    {
      /* "Unable to set socket to system" */
      DHCP_SERVER_PS_IFACE_ASSERT(0);
      ret = FALSE;
    }
    ...
    if ( DSS_SUCCESS != dss_ret )
    {
      /* "Unable to bind to DHCP client port" */
      DHCP_SERVER_PS_IFACE_ASSERT(0);
      ret = FALSE;
    }
  ...
  {
    /* "DHCP Server failed to set server IP" */
    DHCP_SERVER_PS_IFACE_ASSERT(0);
    ret = FALSE;
  }
  if ( TRUE == ret )
...
)
{
  ...
  {
    /* "dhcp_server_stop(): handle or *handle ptr is NULL" */
    DHCP_SERVER_PS_IFACE_ASSERT(0);
    return;
  }
  ...
  {
    /* "Error closing socket <server->sock> <dss_errno>" */
    server->cookie = 0;
    server->cookie_ptr = NULL;
    server->sock = 0;
    server->init.d_free(server);
  ...
  LOG_MSG_DATA_PROTO_INFO_0( "INFO: --- DHCP SERVER STOP ---" );
  *handle = NULL;
} /* dhcp_server_stop */
/*===========================================================================
...
{
  (void)handle;
  DHCP_SERVER_PS_IFACE_ASSERT(NULL != msg);
...
)
{
  ...
  (void)hwlen;
  DHCP_SERVER_PS_IFACE_ASSERT(NULL != server);
  DHCP_SERVER_PS_IFACE_ASSERT(DHCP_SERVER_COOKIE == server->cookie);
  DHCP_SERVER_PS_IFACE_ASSERT(server == server->cookie_ptr);
  DHCP_SERVER_PS_IFACE_ASSERT(NULL != msg);
  DHCP_SERVER_PS_IFACE_ASSERT(NULL != hwaddr);
...
/*===========================================================================
FUNCTION dhcp_server_ps_iface_qsh_mdump_collect
  
DESCRIPTION
  This function collects the mdump for dhcp_static_fltr_handle variable
...
===========================================================================*/
QSH_MDUMP_FN_ATTR 
void dhcp_server_ps_iface_qsh_mdump_collect()
{
  qsh_mdump_collect_high(&dhcp_static_fltr_handle, sizeof(uint32));
