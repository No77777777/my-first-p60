...
$Id: //components/rel/data.mpss/3.5..2.1/protocols/inet/src/dhcp_am_ps_iface.c#2 $

when        who    what, where, why
--------    ---    ----------------------------------------------------------
12/13/17    shrvn  F3 Reduction
07/07/17    gk     DHCP Class based subnet calculation
...
#define DHCP_AM_PS_IFACE_ASSERT(expression)  \
          dhcp_am_ps_iface_assert_wrapper(__LINE__, expression)
/*===========================================================================

FUNCTION DHCP_AM_PS_IFACE_ASSERT_WRAPPER()

DESCRIPTION
  Wrapper function for DHCP_AM_PS_IFACE ASSERT
...
=============================================================================*/
static void dhcp_am_ps_iface_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("DHCP_AM_PS_IFACE_ERR_FATAL at line:%d ",  
              line_num,
              0,
              0);  
  }
  
}/* dhcp_am_ps_iface_assert_wrapper */
...
/*  Returned status codes for requested operation.                         */
  typedef enum {
    DHCP_NETMASK_CLASSLESS=1,          /* Classless, internal implementation */
    DHCP_NETMASK_CLASSTYPE=2,          /* Based on different Classtype A/B/C/D */
    DHCP_NETMASK_STATIC=3,        /* based on input mask */
    DHCP_NETMASK_MAX
  } dhcp_netmask_enum_type;

typedef struct dhcp_netmask_calc
{
	dhcp_netmask_enum_type netmask_calc_type;
	uint32 netmask_value;
}dhcp_netmask_calc;

static dhcp_netmask_calc dhcp_netmask_calc_static_var = {DHCP_NETMASK_CLASSLESS,0};

/*=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
...
/* Added for QTF testing. time_get_secs() API doesn't work in case of QTF */
  ...
  time_t time_in_secs = 0;
  time_in_secs = time(NULL);
  return (uint32)time_in_secs;
...
)
{
  ...
  uint32 host;
  uint32 network;
  const char      * dhcpmask_nv_path = 
    {PS_NV_CONF_ITEM_INFO(PS_NV_CONF_DHCP_SUBNET_MASKING_CALC)};
  dhcp_netmask_calc dhcp_netmask_calc_param = { 0 };
  DHCP_AM_PS_IFACE_ASSERT(NULL != am);
  DHCP_AM_PS_IFACE_ASSERT(DHCP_AM_COOKIE == am->cookie);
  
#define DHCP_AM_MIN_MASK ((uint32)(0xfffffffc))

/*-------------------------------------------------------------------------
    Check if NV is configured for DHCP Dynamic class selection and Netmask value
  -------------------------------------------------------------------------*/
  if (0 == ps_read_efs_nv( dhcpmask_nv_path, 
                           &dhcp_netmask_calc_param, 
                           sizeof(dhcp_netmask_calc_param) ) )
  {
    LOG_MSG_INFO1_0("Read DHCP NV config info from NV" );
    memscpy( (void*)&dhcp_netmask_calc_static_var, 
             sizeof(dhcp_netmask_calc),
             &dhcp_netmask_calc_param, 
             sizeof(dhcp_netmask_calc));
  }  
  else
  {
    /*-------------------------------------------------------------------------
      Install defaults if NV not configured
    -------------------------------------------------------------------------*/
    LOG_MSG_INFO1_0("DHCP NV not configured, using defaults" );
	  dhcp_netmask_calc_param.netmask_calc_type  = DHCP_NETMASK_CLASSLESS;
    memscpy( (void*)&dhcp_netmask_calc_static_var, 
             sizeof(dhcp_netmask_calc),
             &dhcp_netmask_calc_param, 
             sizeof(dhcp_netmask_calc));
...
/* "dhcpi_am_ps_single_ip_calcs: Requested mask type %d" */
   LOG_MSG_INFO1_2("dhcpi_am_ps_single_ip_calcs: Requested mask type %d and mask id 0x%lx",
                   dhcp_netmask_calc_static_var.netmask_calc_type,
						  dhcp_netmask_calc_static_var.netmask_value);


  /* Do the calculation in host byte oder */
  ip = dss_ntohl(am->lease.client_ip);

  if (DHCP_NETMASK_CLASSLESS == dhcp_netmask_calc_static_var.netmask_calc_type  )
  {
    /*------------------------------------------------------------------------ 
  ...
  else if (DHCP_NETMASK_CLASSTYPE  == dhcp_netmask_calc_static_var.netmask_calc_type  )
  {
   /* class C address */
...
#if defined(PS_DHCP_FIXED_NETMASK_255_255_255_255)
  /*
 ...
 else if (DHCP_NETMASK_STATIC == dhcp_netmask_calc_static_var.netmask_calc_type  )
 {
	 mask  = dhcp_netmask_calc_static_var.netmask_value;
	 am->netmask  = dss_htonl(dhcp_netmask_calc_static_var.netmask_value);

 }
 else
 {
	LOG_MSG_INFO1_0("dhcpi_am_ps_single_ip_calcs() INVALID SUBNET MASK CONFIGURATION");

 }

 LOG_MSG_INFO1_1("dhcpi_am_ps_single_ip_calcs: Requested mask type and mask id 0x%lx",
						am->netmask);
...
)
{
  DHCP_AM_PS_IFACE_ASSERT( NULL != am );
  DHCP_AM_PS_IFACE_ASSERT( DHCP_AM_COOKIE == am->cookie );
  DHCP_AM_PS_IFACE_ASSERT( am->cookie_ptr == am );
...
{
  DHCP_AM_PS_IFACE_ASSERT(NULL != am);
  DHCP_AM_PS_IFACE_ASSERT(NULL != core);
...
)
{
  ...
  /* sanity check info */
  DHCP_AM_PS_IFACE_ASSERT(NULL != init);
  DHCP_AM_PS_IFACE_ASSERT(NULL != init->d_malloc);
  DHCP_AM_PS_IFACE_ASSERT(NULL != init->d_free);
  /* Get some memory for our handle */
  am = init->d_malloc(sizeof(dhcp_am_ps_iface_info));
  if ( NULL == am )
  {
    /* "FAIL: Insufficent memory to init DHCP am" */
    DHCP_AM_PS_IFACE_ASSERT(0);
    return NULL;
  }
...
)
{
  ...
  DHCP_AM_PS_IFACE_ASSERT(NULL != am);
  DHCP_AM_PS_IFACE_ASSERT(am->cookie == DHCP_AM_COOKIE);
  DHCP_AM_PS_IFACE_ASSERT(NULL != config);
...
)
{
  ...
  DHCP_AM_PS_IFACE_ASSERT(NULL != handle);
  am = *handle;
  DHCP_AM_PS_IFACE_ASSERT(NULL != am);
  DHCP_AM_PS_IFACE_ASSERT(DHCP_AM_COOKIE == am->cookie);
  if (am != am->cookie_ptr)
...
)
{
  ...
  DHCP_AM_PS_IFACE_ASSERT(NULL != am);
  DHCP_AM_PS_IFACE_ASSERT(DHCP_AM_COOKIE == am->cookie);
  DHCP_AM_PS_IFACE_ASSERT(am == am->cookie_ptr);
  DHCP_AM_PS_IFACE_ASSERT(NULL != core);
 ...
 */
  ...
  if ( FALSE == am->lease.client_bad )
  {
           ...
           0 == memcmp(am->lease.client_hw, core->chaddr, core->hlen)))
    {
      ...
      default:
        /* "Invalid msg_type in get_lease <core->dhcp_msg_request>" */
        DHCP_AM_PS_IFACE_ASSERT(0);
        break;
      }
    }
...
)
{
  ...
  DHCP_AM_PS_IFACE_ASSERT(DHCP_AM_COOKIE == am->cookie);
  DHCP_AM_PS_IFACE_ASSERT(am == am->cookie_ptr);
  DHCP_AM_PS_IFACE_ASSERT(NULL != user_data);
  DHCP_AM_PS_IFACE_ASSERT(NULL != ip_addr);
...
)
{
  ...
  DHCP_AM_PS_IFACE_ASSERT(DHCP_AM_COOKIE == am->cookie);
  DHCP_AM_PS_IFACE_ASSERT(am == am->cookie_ptr);
  DHCP_AM_PS_IFACE_ASSERT(NULL != sip_addr_list);
  DHCP_AM_PS_IFACE_ASSERT(NULL != count);
...
)
{
  ...
  DHCP_AM_PS_IFACE_ASSERT(DHCP_AM_COOKIE == am->cookie);
  DHCP_AM_PS_IFACE_ASSERT(am == am->cookie_ptr);
  DHCP_AM_PS_IFACE_ASSERT(NULL != sip_domain_name_list);
  DHCP_AM_PS_IFACE_ASSERT(NULL != count);
