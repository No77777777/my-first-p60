...
/*===========================================================================
 ...
 $Id: //components/rel/data.mpss/3.5..2.1/protocols/inet/src/dhcp_am_mp.c#2 $
...
#define DHCP_AM_MP_ASSERT(expression)  \
          dhcp_am_mp_assert_wrapper(__LINE__, expression)
/*===========================================================================

FUNCTION DHCP_AM_MP_ASSERT_WRAPPER()

DESCRIPTION
  Wrapper function for DHCP_AM_MP ASSERT
...
=============================================================================*/
static void dhcp_am_mp_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("DHCP_AM_MP_ERR_FATAL at line:%d ",  
              line_num,
              0,
              0);  
  }
  
}/* dhcp_am_mp_assert_wrapper */
...
{
  /* list pointers */
  struct dhcp_am_backend_info * next_ptr;
...
typedef struct dhcp_am_mp_info
{
  ...
  struct dhcp_am_mp_info  * next_ptr, ** prev;
...
)
{
  ...
  DHCP_AM_MP_ASSERT(NULL != backend);
  DHCP_AM_MP_ASSERT(DHCP_AM_MP_BACKEND_COOKIE == backend->cookie);
  DHCP_AM_MP_ASSERT(backend == backend->cookie_ptr);
  am = backend->head;
  while ( NULL != am )
  {
...
)
{
  ...
  /* sanity check info */
  DHCP_AM_MP_ASSERT(NULL != init);
  DHCP_AM_MP_ASSERT(NULL != init->d_malloc);
  DHCP_AM_MP_ASSERT(NULL != init->d_free);
  ...
  if ( NULL == backend )
  {
    ...
    {
      backend->ref = 1;
      backend->next_ptr = dhcp_am_mp_head;
      if ( NULL != backend->next_ptr )
      {
        backend->next_ptr->prev = &(backend->next_ptr);
      }
   ...
   */
  am->next_ptr = backend->head;
  if ( NULL != am->next_ptr )
  {
    am->next_ptr->prev = &(am->next_ptr);
  }
...
)
{
  ...
  DHCP_AM_MP_ASSERT(NULL != am);
  DHCP_AM_MP_ASSERT(DHCP_AM_COOKIE == am->cookie);
  DHCP_AM_MP_ASSERT(NULL != am->backend);
  ...
  am->cookie_ptr = am;
  DHCP_AM_MP_ASSERT( am->backend->cookie_ptr == am->backend );
  return ret;
} /* dhcp_am_mp_start */

/*===========================================================================
...
)
{
   ...
   * with backend.
   */
  DHCP_AM_MP_ASSERT( NULL != am->prev );
  *(am->prev) = am->next_ptr;
  if(am->next_ptr)
  {
    am->next_ptr->prev = am->prev;
  }
  ...
  if ( 0 == --backend->ref )
  {
    DHCP_AM_MP_ASSERT( NULL != backend->prev );
    *(backend->prev) = backend->next_ptr;
    if (NULL != backend->next_ptr)
    {
      backend->next_ptr->prev = backend->prev;
    }
...
)
{
  ...
  DHCP_AM_MP_ASSERT(NULL != am);
  DHCP_AM_MP_ASSERT(DHCP_AM_COOKIE == am->cookie);
  DHCP_AM_MP_ASSERT(am == am->cookie_ptr);
  DHCP_AM_MP_ASSERT(NULL != am->backend);
...
)
{
  ...
  DHCP_AM_MP_ASSERT(am == am->cookie_ptr);
  DHCP_AM_MP_ASSERT(NULL != am->backend);
  DHCP_AM_MP_ASSERT(NULL != cookie);
  DHCP_AM_MP_ASSERT(NULL != ip_addr);
...
)
{
  ...
  DHCP_AM_MP_ASSERT(am == am->cookie_ptr);
  DHCP_AM_MP_ASSERT(NULL != am->backend);
  DHCP_AM_MP_ASSERT(NULL != sip_addr_list);
  DHCP_AM_MP_ASSERT(NULL != count);
...
)
{
  ...
  DHCP_AM_MP_ASSERT(am == am->cookie_ptr);
  DHCP_AM_MP_ASSERT(NULL != am->backend);
  DHCP_AM_MP_ASSERT(NULL != sip_domain_name_list);
  DHCP_AM_MP_ASSERT(NULL != count);
...
/*===========================================================================
FUNCTION dhcp_am_mp_qsh_mdump_collect
  
DESCRIPTION
  This function collects the mdump for dhcp_am_mp_head variable
...
===========================================================================*/ 
QSH_MDUMP_FN_ATTR 
void dhcp_am_mp_qsh_mdump_collect()
{
  qsh_mdump_collect_high(&dhcp_am_mp_head, sizeof(dhcp_am_backend_info));
}
