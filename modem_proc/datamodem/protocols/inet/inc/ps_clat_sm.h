...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/protocols/inet/inc/ps_clat_sm.h#3 $
  $Author: pwbldsvc $ $DateTime: 2021/10/29 00:54:48 $

--------    ---    ----------------------------------------------------------
10/29/21    rj     Configure CLAT DNS64 query via NV
01/09/19    rj     Added support for QSH MDUMP collection
10/30/17    shrvn  Included function declaration and macros for printing error messages
11/30/15    kc     Add changes to event handling in XLAT
11/09/15    kc     Post net_down_reason to iface down event callback
03/10/15    ssh    Deregister with DPM during cleanup
03/03/15    cx     Added net down reason when clat is down
02/04/15    ssh    Introduced failover v6 iface in passthrough mode bringup
12/11/14    kc     Added changes for CLAT QTF test 
11/04/14    kc     Added changes in ps_clat_sm_cb_s for FR21288
...
typedef enum
{
  ...
  CLAT_SM_DPM_CONTEXT_DEREGISTERED_EV  = 12
...
struct ps_clat_sm_cb_s 
{
  ...
  struct
  {
    ...
    void          * iface_ev_handle;             /* IFACE EV buf */
  } assoc_iface;
  ...
  void                    * bring_up_client_data_ptr; /* Client data ptr */
  ps_clat_sm_type        * this_sm_ptr;          /* self pointer           */
};

/*---------------------------------------------------------------------------
TYPEDEF PS_CLAT_EVENT_INFO_TYPE

DESCRIPTION
  Structure to hold CLAT instance and event info
---------------------------------------------------------------------------*/
typedef struct ps_clat_event_info_type_s
{
  ps_clat_sm_type *instance;
  ps_iface_net_down_reason_type net_down_reason;
} ps_clat_event_info_type;
...
/*---------------------------------------------------------------------------
TYPEDEF CLAT_DNS64_QUERY_CONFIG_TYPE

DESCRIPTION
  This structure stores the DNS64 query config for PLAT discovery
---------------------------------------------------------------------------*/
typedef PACKED struct PACKED_POST
{
  uint32                      timeout;      /* DNS64 query - timeout in ms */
  uint8                       retries;      /* DNS64 query - no of retries */
} clat_dns64_query_config_type;
...
===========================================================================*/
#define PS_CLAT_SM_LOG_BUF_SIZE    128

#define CLAT_INST_INITIALIZED(inst_ptr)                        \
  ((inst_ptr != NULL) && (inst_ptr->this_sm_ptr == inst_ptr))
  
#define PS_CLAT_SM_MSG_3(fmt, arg1, arg2, arg3)                \
{                                                              \
  uint8  buf[PS_CLAT_SM_LOG_BUF_SIZE] = {0};                   \
  snprintf((char *)&buf[0], PS_CLAT_SM_LOG_BUF_SIZE,           \
       #fmt,(int16)arg1,(int16)arg2,(int16)arg3);              \
  ps_clat_sm_log_msg(__FILENAME__, __LINE__, buf);             \
}

#define PS_CLAT_SM_MSG_2(fmt, arg1, arg2)                      \
{                                                              \
  uint8  buf[PS_CLAT_SM_LOG_BUF_SIZE] = {0};                   \
  snprintf((char *)&buf[0], PS_CLAT_SM_LOG_BUF_SIZE,           \
       #fmt,(int16)arg1,(int16)arg2);                          \
  ps_clat_sm_log_msg(__FILENAME__, __LINE__, buf);             \
}

#define PS_CLAT_SM_MSG_1(fmt, arg1)                            \
{                                                              \
  uint8  buf[PS_CLAT_SM_LOG_BUF_SIZE] = {0};                   \
  snprintf((char *)&buf[0], PS_CLAT_SM_LOG_BUF_SIZE,           \
       #fmt,(int16)arg1);                                      \
...
FUNCTION PS_CLAT_SM_LOG_MSG()
				
DESCRIPTION
Function for Error Msg and Line Number.
...
#ifdef FEATURE_QSH_MDUMP 
/*===========================================================================
FUNCTION ps_clat_sm_qsh_mdump_collect
  
DESCRIPTION
  This function collects the mdump for clat_config_info variable
...
===========================================================================*/
QSH_MDUMP_FN_ATTR 
void ps_clat_sm_qsh_mdump_collect
(
  void
);
#endif/* FEATURE_QSH_MDUMP */
