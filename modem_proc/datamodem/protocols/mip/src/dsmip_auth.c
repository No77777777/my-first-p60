...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/protocols/mip/src/dsmip_auth.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $

when        who    what, where, why
--------    ---    ----------------------------------------------------------
04/19/17    fn     Fix auth for multiple subs
04/29/16    ds     F3 reduction.
04/01/16    vl     Optimization of F3 messages
...
#define DSMIP_AUTH_ASSERT(expression)  \
          dsmip_auth_assert_wrapper(__LINE__, expression)
...
FUNCTION dsmip_metasm_assert_wrapper

DESCRIPTION
  Wrapper function for DSMIP_AUTH ASSERT
...
===========================================================================*/
static void dsmip_auth_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("DSMIP_AUTH_ERR_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
}/* dsmip_auth_assert_wrapper */
...
FUNCTION MIP_AUTH_LOOKUP_SESSION_TYPE

DESCRIPTION
  This function searches for the session type to use for ds707 for mip 
 authentication
...
RETURN VALUE
  mmgsdi_session_type_enum_type session type
...
===========================================================================*/
mmgsdi_session_type_enum_type mip_auth_lookup_session_type
(
  void
)
{
  mmgsdi_session_type_enum_type session_type = MMGSDI_1X_PROV_PRI_SESSION;
  sys_modem_as_id_e_type        sys_subs_id;

  sys_subs_id = ds707_curr_subs_id();

  if (FALSE == ds3g_mmgsdi_get_cdma_session_type_from_sub_id(sys_subs_id, &session_type))
  {
    session_type = MMGSDI_1X_PROV_PRI_SESSION;
  }

  DATA_MIP_MSG2(MSG_LEGACY_MED, "mip_auth_lookup_session_type ds707 sys_subs_id %d session_type %d", 
                sys_subs_id, session_type);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  else
  {
    ...
    session_type = mip_auth_lookup_session_type();

    DATA_MIP_MSG1(MSG_LEGACY_MED, "Sending Compute Ip Auth - MN-HA, session_type %d", session_type);

    mmgsdi_status = ds3g_mmgsdi_session_compute_ip_auth (session_type, cmpt_ip_data);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  else
  {
    ...
    DATA_MIP_MSG1(MSG_LEGACY_MED, "Sending Compute Ip Auth - RRQ sys session type %d", session_type);
    ...
    DATA_MIP_MSG1(MSG_LEGACY_MED, "Sending Compute Ip Auth - MN-AAA sys session_type %d", session_type);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  -------------------------------------------------------------------------*/
  DSMIP_AUTH_ASSERT (mn_ha_ss != NULL);
  DSMIP_AUTH_ASSERT (mn_ha_ss_len != NULL);
  DSMIP_AUTH_ASSERT (mn_aaa_ss != NULL);
  DSMIP_AUTH_ASSERT (mn_aaa_ss_len != NULL);
  ...
  if (!(*mn_aaa_ss) || !(*mn_ha_ss))
  {
    ...
    -----------------------------------------------------------------------*/
    nv_status = ps_get_nv_item_ext(NV_DS_MIP_ENABLE_PROF_I, &mip_nv_item,
                                   ds707_curr_subs_id());
    ...
    nv_status = ps_get_nv_item_ext(NV_DS_MIP_SS_USER_PROF_I, &mip_nv_item,
                                   ds707_curr_subs_id());
    if( nv_status != NV_DONE_S)
    {
      /*---------------------------------------------------------------------
