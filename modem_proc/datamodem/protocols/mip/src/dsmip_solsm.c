...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/protocols/mip/src/dsmip_solsm.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
#define DSMIP_SOLSM_ASSERT(expression)  \
          dsmip_solsm_assert_wrapper(__LINE__, expression)
...
FUNCTION dsmip_solsm_assert_wrapper

DESCRIPTION
  Wrapper function for DSMIP_SOLSM ASSERT
...
===========================================================================*/
static void dsmip_solsm_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("DSMIP_SOLSM_ERR_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
}/* dsmip_solsm_assert_wrapper */
...
)
{
  ...
  if(PS_TIMER_INVALID_HANDLE == ssmi_info.timer)
  {
    DSMIP_SOLSM_ASSERT(0);
    return -1;
  }
...
)
{
  ...
  DSMIP_SOLSM_ASSERT(ssmi_inited == TRUE);
  ...
  case SSM_IMMED_RETRY_EV:
    ...
    case SSMI_SEND_SOL_STATE:
      DSMIP_SOLSM_ASSERT(ssmi_info.retry_cnt != -1);
      ssmi_info.retry_cnt -= 1;
      timer_result = ps_timer_cancel(ssmi_info.timer);
      DSMIP_SOLSM_ASSERT(timer_result != PS_TIMER_FAILURE); 
      ssmi_transition_state(SSMI_SEND_SOL_STATE);
      break;
