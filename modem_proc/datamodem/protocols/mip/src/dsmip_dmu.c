...
$Header: //components/rel/data.mpss/3.5..2.1/protocols/mip/src/dsmip_dmu.c#2 $                       $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
 
when        who    what, where, why
-------     ---    -----------------------------------------------------------
04/01/16    cx     Replaced EFS APIs with MCFG APIs
...
/*- -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  nv_status = ps_get_nv_item_ext (NV_DS_MIP_DMU_PKOID_I, ps_nv_item_ptr,
                                  ds707_curr_subs_id());
...
/*-------------------------------------------------------------------------*/
  ...
  else /* read the active profile id from nv if not already read */
  {
    ...
    {
      nv_status = ps_get_nv_item_ext(NV_DS_MIP_ACTIVE_PROF_I, ps_nv_item_ptr,
                                     ds707_curr_subs_id());
      if( nv_status == NV_DONE_S )
      {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  nv_status = ps_get_nv_item_ext(NV_DS_MIP_DMU_MN_AUTH_I,ps_nv_item_ptr,
                                 ds707_curr_subs_id());
  if( nv_status != NV_DONE_S )
  {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  nv_status = ps_put_nv_item_ext (NV_DS_MIP_SS_USER_PROF_I, ps_nv_item_ptr,
                                  ds707_curr_subs_id());
  if (nv_status != NV_DONE_S)
  {
  ...
  nv_status = ps_put_nv_item_ext (NV_HDR_AN_AUTH_PASSWD_LONG_I, ps_nv_item_ptr,
                                  ds707_curr_subs_id());
  if (nv_status != NV_DONE_S)
  {
    /* should really pre-commit keys and not do DMU if pre-committing fails */
  ...
  nv_status = ps_put_nv_item_ext (NV_PPP_PASSWORD_I, ps_nv_item_ptr,
                                  ds707_curr_subs_id());
  if (nv_status != NV_DONE_S)
  {
    /* should really pre-commit keys and not do DMU if pre-committing fails */
