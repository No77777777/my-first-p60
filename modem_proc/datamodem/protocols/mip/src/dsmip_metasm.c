...
Copyright (c) 2000-2016 Qualcomm Technologies Incorporated.
All Rights Reserved.
...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/protocols/mip/src/dsmip_metasm.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
#define DSMIP_METASM_ASSERT(expression)  \
          dsmip_metasm_assert_wrapper(__LINE__, expression)
...
DESCRIPTION
  Wrapper function for DSMIP_METASM ASSERT
...
===========================================================================*/
static void dsmip_metasm_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("DSMIP_METASM_ERR_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
}/* dsmip_metasm_assert_wrapper */
      ...
      switch(msmi_info.state)
      {
        ...
        case MSMI_OPENING_STATE:
          ...
          {         
            ...
            DATA_MIP_MSG3(MSG_LEGACY_HIGH, 
                          "Changing net policy iface id 0x%x and iface kind %d"
                          " Changing net policy for sockets with profile id %d",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch(new_state)
  {
    ...
    case MSMI_CLOSED_STATE:
      ...
      dsmip_fill_mip_vals(ps_get_nv_item_ext, ps_nv_item_ptr, 
                          ds707_curr_subs_id());
      ...
      /*-----------------------------------------------------------------------
        Close all the opened MIP sockets
      -----------------------------------------------------------------------*/
      mipio_set_initialized_flag(FALSE);
      if (FALSE == mipio_close_all_sockets())
      {
        DATA_MIP_MSG0(MSG_LEGACY_ERROR, "MIP sockets close operation failed");	   
        return;
      }
...
_MIP_SM_REG: 
   DATA_MIP_MSG0(MSG_LEGACY_ERROR,
        "M.IP SolSM init failed! or M.IP RegSM init failed! or mip_meta_sm_init() not supported ");
  return -1;                                      
