...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/protocols/mip/src/dsmip_regsm.c#3 $ $DateTime: 2021/02/12 03:50:40 $ $Author: pwbldsvc $

when        who    what, where, why
--------    ---    ----------------------------------------------------------
02/12/21    rk     Compiler Version Upgrade to 8.5.01 fixes
06/21/16    ds     Fixed KW errors.
04/29/16    ds     F3 reduction.
06/15/15    skc    Fixed immdeiate retry event logic for DeRRQ.
12/05/14    sd     Fixed immediate retry event logic for RRQ and DeRRQ.
...
10/08/17    my     F3/Debug messages reduction
...
/*---------------------------------------------------------------------------
  Max IO failure count to keep a check on the DeRRQ retry attempts.
  One immediate retry and an extra attempt for IO failure are allowed.
  Extra attempt will be utilised in case when immediate retry fails as 
  traffic channel was not yet brought up.
  This fix was done as we were not incrementing the retry count for IO failure
  which was leading to never ending attempts as the derrq timer was canceeled
  out by the write event from sockets and failure count never reached
  till the maximum.
---------------------------------------------------------------------------*/
#define MAX_IO_FAILURE_ALLOWED      2
...
#define DSMIP_REGSM_ASSERT(expression)  \
          dsmip_regsm_assert_wrapper(__LINE__, expression)
...
FUNCTION dsmip_regsm_assert_wrapper

DESCRIPTION
  Wrapper function for DSMIP_REGSM ASSERT
...
===========================================================================*/
static void dsmip_regsm_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("DSMIP_REGSM_ERR_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
...
#ifdef ASSERT
  DSMIP_REGSM_ASSERT(rsmi_inited == TRUE);
#endif /* ASSERT */
...
/*#ifndef T_ARM
  ...
  -------------------------------------------------------------------------*/
/*#ifndef T_ARM
  if (!(session->rsm_info.state > RSMI_MIN_STATE || 
        session->rsm_info.state < RSMI_MAX_STATE))
  {
  ...
  case RSMI_DEREG_MIP_EV:
    ...
    case RSMI_RE_RRQ_STATE:
      ...
      DSMIP_REGSM_ASSERT(timer_result != PS_TIMER_FAILURE);
  ...
  case RSM_IMMED_RETRY_EV:
    ...
    case RSMI_DE_RRQ_STATE:
      ...
      else if(io_failure <= MAX_IO_FAILURE_ALLOWED)
      {
        session->rsm_info.rrq_cnt -= 1;
      }
      else
...
DESCRIPTION
  Increments the IO failure count.
...
SIDE EFFECTS
  None
===========================================================================*/
void mip_reg_sm_increment_io_failure
(
  void
)
{
...
/*===========================================================================
FUNCTION MIP_REG_SM_LOCAL_CLEANUP

DESCRIPTION
  Local clean up for MIP dereg
...
SIDE EFFECTS
  None
===========================================================================*/
void mip_reg_sm_local_cleanup
(
  void
)
{
  DATA_MIP_MSG0(MSG_LEGACY_MED, "MIP local cleanup");

  mipio_stop_udp_snooping(0);  
  rsmi_transition_state(INDEX_2_MIP_SES_PTR(0), RSMI_DEREGISTERED_STATE);
}
