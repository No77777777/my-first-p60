...
$Header: //components/rel/data.mpss/3.5..2.1/protocols/mip/src/dsmip_outmsg.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
#define DSMIP_OUTMSG_ASSERT(expression)  \
          dsmip_outmsg_assert_wrapper(__LINE__, expression)
...
FUNCTION dsmip_outmsg_assert_wrapper

DESCRIPTION
  Wrapper function for DSMIP_OUTMSG ASSERT
...
===========================================================================*/
static void dsmip_outmsg_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("DSMIP_OUTMSG_ERR_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
  
}/* dsmip_outmsg_assert_wrapper */
...
/*lint -restore Restore lint error 572*/
  ...
  if (result == TRUE)
  {
    ...
    DSMIP_OUTMSG_ASSERT (result != PS_TIMER_FAILURE);
