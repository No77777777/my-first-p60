...
Copyright (c) 2000-2017 Qualcomm Technologies Incorporated.
All Rights Reserved.
...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/protocols/mip/src/dsmip.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $

when        who    what, where, why
--------    ---    ----------------------------------------------------------
11/30/16    hb     Fixed KW errors
04/29/16    ds     F3 reduction.
04/01/16    sd     Added support to open MIP sockets when MIP session is 
                   configured.
...
31/07/17    my      F3/Debug messages reduction
===========================================================================*/
...
#define DSMIP_ASSERT(expression)  \
          dsmip_assert_wrapper(__LINE__, expression)
...
FUNCTION dsmip_assert_wrapper

DESCRIPTION
  Wrapper function for DSMIP ASSERT
...
=============================================================================*/
static void dsmip_assert_wrapper
(
...
)
{
  if (!expression)
  {
    ERR_FATAL("DSMIP_ERR_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
  
}/* dsmip_regsm_assert_wrapper */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    nv_status = ds3gcfgmgr_read_legacy_nv_ex( item_code, data_ptr,
                                              ds707_curr_subs_id());
  }
  else if (rex_self() == mip_get_ps_tcb_ptr())
  {
    nv_status = ps_get_nv_item_ext( item_code, data_ptr,
                                    ds707_curr_subs_id() );
  }
  else
  {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    goto _MEM_OR_SESSION_ERR;   /* FATAL_ERR reduction */
  }
  ...
  DATA_MIP_MSG5(MSG_LEGACY_HIGH, "MIP profile id: %d, MIP max RRQ retry: %d"
                 " MIP max DERRQ retry: %d, MIP retry int: %d, "
                 "MIP prereg ret int: %d", mip_index, 
                 session->rsm_info.max_rrqs,
                 session->rsm_info.max_derrqs,
                 ps_nv_item_ptr->ds_mip_retry_int,
                 ps_nv_item_ptr->ds_mip_pre_re_rrq_time);
  /*-------------------------------------------------------------------------
    Check NV to see if active profile is enabled or disabled
  -------------------------------------------------------------------------*/
  nv_status = mip_get_nv_item(NV_DS_MIP_ENABLE_PROF_I, ps_nv_item_ptr);
  if((nv_status != NV_DONE_S) && session->profile_index < NV_DS_MIP_MAX_NUM_PROF)
  {
    /*-----------------------------------------------------------------------
  ...
  /*-------------------------------------------------------------------------
    ...
    - NV settings
  -------------------------------------------------------------------------*/
  if ((session->profile_index < NV_DS_MIP_MAX_NUM_PROF) && 
      ps_nv_item_ptr->ds_mip_enable_prof[session->profile_index])
  {
    /*-----------------------------------------------------------------------
    ...
    {
      DATA_MIP_MSG0(MSG_LEGACY_LOW, "Using shadow NV item for Home Add");
    ...
    {
      DATA_MIP_MSG0(MSG_LEGACY_LOW, "Using shadow NV item for Pri HA");
    ...
    {
      DATA_MIP_MSG0(MSG_LEGACY_LOW, "Using shadow NV item for Sec HA");
    ...
    {
      DATA_MIP_MSG0(MSG_LEGACY_LOW, "Using shadow NV item for NAI");
    ...
    {
      DATA_MIP_MSG0(MSG_LEGACY_LOW, "Using shadow NV item for AAA-SPI");
    ...
    {
      DATA_MIP_MSG0(MSG_LEGACY_LOW, "Using shadow NV item for HA-SPI");
    ...
    {
      DATA_MIP_MSG0(MSG_LEGACY_LOW, "Using shadow NV item for Rev Tun");
...
void dsmip_read_mip_nv_items(void)
{
  ...
  nv_stat_enum_type       nv_status;
  /*NV_DS_MIP_RETRIES_I*/
  nv_status = ds3gcfgmgr_read_legacy_nv_ex( NV_DS_MIP_RETRIES_I , &ds_nv_item,
                                            ds707_curr_subs_id());
  if(nv_status == NV_DONE_S)
  {
  ...
  nv_status = ds3gcfgmgr_read_legacy_nv_ex(NV_DS_MIP_PRE_RE_RRQ_TIME_I, &ds_nv_item,
                                           ds707_curr_subs_id());
  if(nv_status == NV_DONE_S)
  {
  ...
  nv_status = ds3gcfgmgr_read_legacy_nv_ex(NV_DS_MIP_RETRY_INT_I, &ds_nv_item,
                                           ds707_curr_subs_id());
  if(nv_status == NV_DONE_S)
  {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DATA_MIP_MSG0(MSG_LEGACY_MED,"In ds_qcmip_update");
  
  if(TRUE == read_flag)
  {
    status = ds3gcfgmgr_read_legacy_nv_ex( NV_DS_QCMIP_I, &ds707_nv_item,
                                           ds707_curr_subs_id());
    if( status == NV_DONE_S)
    {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  else
  {
    ...
    {
      dsmip_fill_mip_vals(ds3gcfgmgr_read_legacy_nv_ex, &ds707_nv_item,
                          ds707_curr_subs_id());
    }
    else if (rex_self() == rcinit_lookup_rextask("ps")) 
    {
      dsmip_fill_mip_vals(ps_get_nv_item_ext, &ds707_nv_item, 
                          ds707_curr_subs_id());
    }
#else
    if (rex_self() == &ds_tcb) 
    ...
    ---------------------------------------------------------------------*/
    (void)ds3gcfgmgr_read_legacy_nv_ex(NV_DS_MIP_ENABLE_PROF_I,&ds707_nv_item,
                                       ds707_curr_subs_id());
    ...
    status = ds3gcfgmgr_write_legacy_nv_ex(NV_DS_MIP_ACTIVE_PROF_I, &ds707_nv_item,
		                           ds707_curr_subs_id());
    ...
    if(!mip_dmu_in_progress())
    {
      ...
      {
        DATA_MIP_MSG0(MSG_LEGACY_MED,"Changed profile, re-encrypt new keys");
...
{
  DATA_MIP_MSG0(MSG_LEGACY_MED,"Erase shadow NV");
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
 ...
 -------------------------------------------------------------------------*/
  ...
  nv_status = ds3gcfgmgr_read_legacy_nv_ex(NV_DS_MIP_GEN_USER_PROF_I,
                                           &ds707_nv_item,
                                           ds707_curr_subs_id());
 ...
 -------------------------------------------------------------------------*/
  switch(nv_status)
  {
    ...
    case NV_NOTACTIVE_S:
      DATA_MIP_MSG0(MSG_LEGACY_MED,"User profile never written");
   ...
   ---------------------------------------------------------------------*/ 
      ...
      DATA_MIP_MSG0(MSG_LEGACY_MED,"Create default MIP user profile");
     ...
     /*---------------------------------------------------------------------
       Write profile to nv
     ---------------------------------------------------------------------*/
      nv_status = ds3gcfgmgr_write_legacy_nv_ex( NV_DS_MIP_GEN_USER_PROF_I, 
     ...
     ---------------------------------------------------------------------*/
      ...
      (void) ds3gcfgmgr_write_legacy_nv_ex (NV_DS_MIP_ENABLE_PROF_I, 
                                            &ds707_nv_item, 
                                            ds707_curr_subs_id());
      break;
  ...
  nv_status = ds3gcfgmgr_read_legacy_nv_ex(NV_DS_MIP_SS_USER_PROF_I,
                                           &ds707_nv_item,
                                           ds707_curr_subs_id());
  switch(nv_status)
  {
    ...
    case NV_NOTACTIVE_S:
      DATA_MIP_MSG0(MSG_LEGACY_MED,"Shared secret profile never written");
      ...
      (void) dsmip_get_1x_profile_val_num(DS_QCMIPP_VAL,
      	                                   (void *)&temp_val,MIP_NON_ARRAY);
      ds707_nv_item.ds_mip_ss_user_prof.index = temp_val;
      nv_status = ds3gcfgmgr_write_legacy_nv_ex( NV_DS_MIP_SS_USER_PROF_I,
                                   &ds707_nv_item, ds707_curr_subs_id() );
      if( nv_status != NV_DONE_S )
      {
        DATA_MIP_MSG1(MSG_LEGACY_ERROR,
...
(
  nv_stat_enum_type (* nv_get_fn)(nv_items_enum_type, nv_item_type *, sys_modem_as_id_e_type),
  nv_item_type *         nv_item,
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  nv_get_status = (*nv_get_fn)(NV_DS_MIP_GEN_USER_PROF_I, nv_item, subs_id);
  if(nv_get_status != NV_DONE_S)
  {
    DATA_MIP_MSG0(MSG_LEGACY_ERROR,
                    "Trouble getting MIP user profile from NV!");
    DATA_MIP_MSG0(MSG_LEGACY_MED,"Set AT values to defaults!");
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch(type)
  {
    case DS_QCMIP_VAL:
      ...
      status = ds3gcfgmgr_write_legacy_nv_ex( NV_DS_QCMIP_I, &ds707_nv_item,
                                              ds707_curr_subs_id());
      if( status != NV_DONE_S )
      {
        DATA_MIP_MSG1(MSG_LEGACY_HIGH,
...
---------------------------------------------------------------------------*/
    ...
    default:
      DATA_MIP_MSG1(MSG_LEGACY_HIGH, "No match for type %d",type);
  }
  
  return ret_val;
  _INVALID_NUM:	
       DATA_MIP_MSG0(MSG_LEGACY_ERROR,"Invalid no_of_elements");
  return ret_val; 	
}
...
)
{
  ...
  DATA_MIP_MSG1(MSG_LEGACY_HIGH,"Setting %d",type);
  if( (str == NULL) || (length == 0) )
  {
    DATA_MIP_MSG0(MSG_LEGACY_ERROR,
                  "String pointer is NULL or string length is 0.");
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  nv_status = ds3gcfgmgr_read_legacy_nv_ex( NV_DS_MIP_RM_NAI_I, &nv_item,
                                            ds707_curr_subs_id());
  if( nv_status != NV_DONE_S )
  {
    goto _NAI_ERROR;
...
_NAI_ERROR: 
    DATA_MIP_MSG0(MSG_LEGACY_ERROR, "Laptop NAI cannot be retrieved or mip_get_rm_nai() is not supported.");
  return (-1);
} /* mip_get_rm_nai() */
