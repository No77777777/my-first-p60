...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/protocols/mip/src/dsmip_parse.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $
...
#define DSMIP_PARSE_ASSERT(expression)  \
          dsmip_parse_assert_wrapper(__LINE__, expression)
...
FUNCTION dsmip_parse_assert_wrapper

DESCRIPTION
  Wrapper function for DSMIP_PARSE ASSERT
...
===========================================================================*/
static void dsmip_parse_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("DSMIP_PARSE_ERR_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
  
}/* dsmip_parse_assert_wrapper */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /*-------------------------------------------------------------------------
    ...
    magic # = icmp x 2 + 1 udp / session 
            = 2 + MIP_MAX_SESSIONS
  -------------------------------------------------------------------------*/
  DSMIP_PARSE_ASSERT (index < (MIP_MAX_SESSIONS + 2));
  if (data == NULL)
  {
    DSMIP_PARSE_ASSERT(0);
    return;
