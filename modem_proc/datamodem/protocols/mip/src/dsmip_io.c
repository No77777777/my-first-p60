...
Copyright (c) 2000-2015 Qualcomm Technologies Incorporated.
All Rights Reserved.
...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/protocols/mip/src/dsmip_io.c#2 $ $DateTime: 2019/10/22 02:53:17 $ $Author: pwbldsvc $

when        who    what, where, why
--------    ---    ----------------------------------------------------------
04/01/15    sd     Added support to open MIP sockets when MIP session is 
                   configured.
...
#define DSMIP_IO_ASSERT(expression)  \
          dsmip_io_assert_wrapper(__LINE__, expression)
...
FUNCTION dsmip_io_assert_wrapper

DESCRIPTION
  Wrapper function for DSMIP_IO ASSERT
...
===========================================================================*/
static void dsmip_io_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("DSMIP_IO_ERR_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
}/* dsmip_io_assert_wrapper */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DATA_MIP_MSG0(MSG_LEGACY_HIGH, "Closing MIPIO sockets"); 
  /*-------------------------------------------------------------------------
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  -------------------------------------------------------------------------*/
  DSMIP_IO_ASSERT (prot == PS_IPPROTO_ICMP || prot == PS_IPPROTO_UDP);
  DSMIP_IO_ASSERT (identifier < MIPIO_MAX_SOCKS);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if( !mipioi_open_all_sockets() )
  {
    ...
    -----------------------------------------------------------------------*/
    DATA_MIP_MSG0(MSG_LEGACY_LOW, "Couldn't open mobile IP sockets!");
    mipioi_close_all_sockets();
    return FALSE;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (l_errno == DS_EWOULDBLOCK)
  {
    ...
    result = dss_async_select( mipioi_info.sd[identifier], 
                               DS_READ_EVENT, 
                               &l_errno );
    DSMIP_IO_ASSERT (result == DSS_SUCCESS);
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(result == DSS_ERROR)
  {
    if(l_errno == DS_EWOULDBLOCK)
    {
      ...
      mip_reg_sm_increment_io_failure();
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  status = dss_async_deselect( mipioi_info.sd[MIPIO_ICMP_TX_HANDLE], 
                               DS_WRITE_EVENT, 
                               &l_errno );
  DSMIP_IO_ASSERT( status == DSS_SUCCESS );
...
FUNCTION MIPIO_SET_INITIALIZED_FLAG

DESCRIPTION
  Sets mipioi_initialized flag which keeps the current status of MIP IO library
...
===========================================================================*/
void mipio_set_initialized_flag
(
  boolean flag
)
{
  mipioi_initialized = flag;
  return;
} /* mipio_set_initialized_flag() */
