...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/protocols/hc/src/ps_comp_logging_helper.c#2 $
  $Author: pwbldsvc $
  $DateTime: 2019/10/22 02:53:17 $

when        who    what, where, why
--------    ---    ----------------------------------------------------------
10/20/17    vls    Added support for multiple UDC instances
...
typedef struct comp_logging_s_type
{
  ps_comp_logging_generic_cback_type ps_comp_logging_generic_cback;
  void* user_data_ptr;
}comp_logging_cmd_data;
/*===========================================================================
...
)
{
  ...
  ps_iface_type              *ps_iface_ptr = NULL;
  comp_logging_cmd_data      *comp_logging_cmd_data_ptr = NULL;
...
/*-------------------------------------------------------------------------*/
  ...
  comp_logging_cmd_data_ptr = (comp_logging_cmd_data*)user_data_ptr;
  
  /* Check if we need to invoke generic callback */
  if((comp_logging_cmd_data_ptr) &&
     (comp_logging_cmd_data_ptr->ps_comp_logging_generic_cback != NULL))
  {
    //LOG_MSG_INFO2_1( "Invoking generic cback 0x%x", 
    //                 ps_comp_logging_generic_cback );
    comp_logging_cmd_data_ptr->ps_comp_logging_generic_cback(comp_logging_cmd_data_ptr->user_data_ptr);
    comp_logging_cmd_data_ptr->ps_comp_logging_generic_cback = NULL;
  }

  if(comp_logging_cmd_data_ptr)
  {
    PS_SYSTEM_HEAP_MEM_FREE(comp_logging_cmd_data_ptr);
    comp_logging_cmd_data_ptr = NULL;
  }
...
/*-------------------------------------------------------------------------*/
  ...
  comp_logging_cmd_data_ptr = (comp_logging_cmd_data*)ps_system_heap_mem_alloc(sizeof(comp_logging_cmd_data));

  if(NULL == comp_logging_cmd_data_ptr)
  {
    LOG_MSG_ERROR_0("ps_comp_logging_invoke_generic_cback(): Out of memory");
    return;    
  }
  
  comp_logging_cmd_data_ptr->ps_comp_logging_generic_cback = cback;
  comp_logging_cmd_data_ptr->user_data_ptr = user_data_ptr;

  /* Set cmd to switch context to PS task*/
  ps_send_cmd(PS_COMP_LOGGING_HELPER_CMD, comp_logging_cmd_data_ptr);
}
