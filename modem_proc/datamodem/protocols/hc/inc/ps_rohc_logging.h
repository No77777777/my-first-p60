...
/*===========================================================================
  ...
  $Header: //components/rel/data.mpss/3.5..2.1/protocols/hc/inc/ps_rohc_logging.h#2 $
  $Author: pwbldsvc $
  $DateTime: 2019/10/22 02:53:17 $

when        who    what, where, why
--------    ---    ----------------------------------------------------------
04/05/18    ds     added  support for payload size in ROHC TCP Compressor
                   log packet.
...
typedef struct {
    uint8   version;       /* version (QXDM enforced)               */
    uint32  rohc_instance;
    uint32  received_timestamp;
    uint16  cid; 
    uint8   profile;
    uint8   mode;
    uint8   compressor_state; 
    uint32  error; 
    uint8   packet_type;
    uint32  compressed_header_size;
    uint32  original_header_size;
    uint32  payload_size;
    uint16  msn;
    uint32  tcp_seq_num;
    uint32  seq_scale_factor;
    uint32  seq_num_scaled;
    uint32  tcp_ack_num;
    uint32  ack_stride;
    uint32  ack_num_scaled;
    uint8   tcp_rsf_flags;
    uint8   tcp_ack_flag;
    uint8   tcp_psh_flag;
} rohc_tcp_comp_t;
...
/*---------------------------------------------------------------------------
  Log packet for ROHC TCP Compressor
  Log Code:  LOG_ROHC_TCP_COMPRESSOR_C
---------------------------------------------------------------------------*/
typedef struct {
  log_hdr_type  hdr;
  rohc_tcp_comp_t  comp_tcp;
} rohc_tcp_comp_log_packet_s_type;

/*---------------------------------------------------------------------------
  Log packet for ROHC TCP Decompressor
  Log Code:  LOG_ROHC_TCP_DECOMPRESSOR_C
---------------------------------------------------------------------------*/
typedef struct {
  log_hdr_type    hdr;
  rohc_tcp_decomp_t  decomp_tcp;
} rohc_tcp_decomp_log_packet_s_type;

/*---------------------------------------------------------------------------
  Log packet for log codes:
    LOG_ROHC_TCP_FEEDBACK_COMPRESSOR_C
    LOG_ROHC_TCP_FEEDBACK_DECOMPRESSOR_C
---------------------------------------------------------------------------*/
...
uint8                     msn_not_valid;
uint8                     msn_option;
uint8                     context_memory;
}rohc_tcp_feedback_t;

typedef struct {
 log_hdr_type    hdr;
 rohc_tcp_feedback_t   tcp_fb;
} rohc_tcp_feedback_log_packet_s_type;
#pragma pack(pop)
