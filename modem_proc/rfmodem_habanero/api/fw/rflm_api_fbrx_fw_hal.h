...
$Header: //components/rel/rfmodem_habanero.mpss/1.11/api/fw/rflm_api_fbrx_fw_hal.h#1 $
...
12/29/15    ka/hm  Dynamic ADC settings code cleanup 
07/08/15    JJ     FBRx ADC re-config for current WTR
04/16/15    kg     Support for re-programming ADC settings to compensate Vcm voltage on 4905 07/24/14    aa     Added support to store lmem sample captures for log packet 0x184A
...
/*----------------------------------------------------------------------------*/
extern void rfcommon_fbrx_msm_store_lmem_sample_captures_v2( uint32 *data_ptr,
                                                             uint16 data_len, 
                                                             uint32 offset);

/*----------------------------------------------------------------------------*/
void rfcommon_fbrx_msm_config_dynamic_adc_settings(rflm_handle_rx_t fbrx_lm_handle, uint32 device_id, uint8 fbrx_chain, rflm_tech_id_t rflm_tech );

/*----------------------------------------------------------------------------*/
extern uint32 rfcommon_fbrx_msm_get_fb_nb_rate_hz( uint32 fbrx_lm_handle );
