...
$Header: //components/rel/rfmodem_habanero.mpss/1.11/api/ccs/rflm_ccs_mmu_intf.h#1 $
$Date: 2021/03/09 $
when         who     what, where, why
--------   ---     -----------------------------------------------------------
01/24/20   moh     RF MDUMP Support.
...
#define RFLM_CCS_MMU_BACKEND_SCHEDULE_ENABLE     /* enable or disable early enq handling */

#define RFLM_CCS_MMU_EARLY_ENQ_BUFFER_DEPTH      16
#define RFLM_CCS_MMU_EARLY_ENQ_DEPTH_MASK        0xF
#define RFLM_CCS_MMU_EARLY_ENQ_BUF_INDEX_MAX     RFLM_CCS_MMU_EARLY_ENQ_BUFFER_DEPTH
#define RFLM_CCS_MMU_EARLY_ENQ_THRESHOLD_US      1000
#define RFLM_CCS_MMU_EARLY_ENQ_THRESHOLD         (RFLM_CCS_MMU_EARLY_ENQ_THRESHOLD_US*19.2)
#define RFLM_CCS_MMU_EARLY_ENQ_CHK_THRESHOLD_US  (RFLM_CCS_MMU_EARLY_ENQ_THRESHOLD_US + 10)
#define RFLM_CCS_MMU_EARLY_ENQ_CHK_THRESHOLD     (RFLM_CCS_MMU_EARLY_ENQ_CHK_THRESHOLD_US*19.2)
//#define RFLM_CCS_MMU_EARLY_ENQ_CHK_THRESHOLD_PAST -32640 /* -1.7ms, the same as timing scrutiny*/
#define RFLM_CCS_MMU_EARLY_ENQ_CHK_THRESHOLD_PAST -384 /* -20us*/

#define RFLM_CCS_MMU_EARLY_ENQ_VSTMR_INTERVAL_US 400
#define RFLM_CCS_MMU_EARLY_ENQ_VSTMR_INTERVAL    (RFLM_CCS_MMU_EARLY_ENQ_VSTMR_INTERVAL_US*19.2)
#define RFLM_CCS_MMU_EARLY_ENQ_BUFFER_PAYLOAD_SIZE_UINT32 (RFLM_CCS_MMU_MAX_ALLOC_SIZE_IN_BYTES/sizeof(uint32))
#define RFLM_CCS_MMU_EARLY_ENQ_CONFIG_SIZE_UINT32 7
...
#define RFLM_CCS_MMU_STATIC_SCRIPTS_1H_NUM     3
#define RFLM_CCS_MMU_STATIC_SCRIPTS_2H_NUM     3
#define RFLM_CCS_MMU_STATIC_SCRIPTS_3H_NUM     2
#define RFLM_CCS_MMU_STATIC_SCRIPTS_7H_50_NUM  2
...
typedef enum 
{
  RFLM_CCS_MMU_EARLY_ENQ_EVENT,
  RFLM_CCS_MMU_EARLY_ENQ_TASK,
  RFLM_CCS_MMU_EARLY_ENQ_SEQ
}rflm_ccs_mmu_early_enq_type_e;

/* Backend thread definitions*/
typedef enum
{
  RFLM_CCS_MMU_SHUTDOWN_SIG,
  RFLM_CCS_MMU_EARLY_ENQ_CHK_SIG,
  RFLM_CCS_MMU_VSTMR_INIT_SIG,
  RFLM_CCS_MMU_VSTMR_WAKEUP_SIG,
  RFLM_CCS_MMU_MAX_SIGNALS
}rflm_ccs_mmu_sig_e;
...
/* Early enqueue buffer definitions */
typedef struct ALIGN(4)rflm_ccs_mmu_early_enq_s
{
  uint32 enq_config[RFLM_CCS_MMU_EARLY_ENQ_CONFIG_SIZE_UINT32];
  uint32 trig_time;
  ccs_rf_task_t task;                                                 // task                                     
  uint32 payload[RFLM_CCS_MMU_EARLY_ENQ_BUFFER_PAYLOAD_SIZE_UINT32];  // task payload
  void * arg_0;
  void * arg_1;
  rflm_ccs_mmu_early_enq_type_e enq_type;
  rflm_tech_id_t tech_id;
  uint8 tq_idx;
  uint32 task_idx; // used for task and issue_seq to store the actul taskq index after enqueued
}rflm_ccs_mmu_early_enq_t;
...
#define RFLM_MMU_THREAD_MAX_NAME_LEN        16

//! @brief RFLM firmware thread info structure.
typedef struct {
  char              name[RFLM_MMU_THREAD_MAX_NAME_LEN];
  int               priority;
  qurt_thread_t     thread;
  void 	            *stackAddr;
  int               stackSize;
  qurt_anysignal_t  signals;
} rflm_mmu_thread_info;
...
extern void rflm_ccs_mmu_vstmr_init(boolean bypass_vstmr_int_reg);
...
void rflm_ccs_mmu_set_signal(rflm_ccs_mmu_sig_e);
...
void rflm_ccs_mmu_qsh_mdump(void);
#endif
