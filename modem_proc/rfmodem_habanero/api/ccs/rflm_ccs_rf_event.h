...
$Header: //components/rel/rfmodem_habanero.mpss/1.11/api/ccs/rflm_ccs_rf_event.h#1 $ 
$Date: 2021/03/09 $   
when       who     what, where, why 
--------   ---     ------------------------------------------------------------- 
11/19/13   ra     Export data structure and prototype in RFLM CCS API component
11/16/13   ra     Remove obsolete reference to a header
...
/*! @brief Init event timing checking config */
#define RFLM_CCS_EVENT_TIMING_CHK_INIT(evt_chk_struct) memset(&evt_chk_struct, 0, sizeof(rflm_ccs_rf_event_time_chk_t))
...
void rflm_ccs_program_sbi_rffe (
  /*! Source CCS RF task pointer, with common fields of task set */
  ccs_rf_task_t *rflm_ccs_rf_inp_task_p,
...
extern void rflm_ccs_event_sw_port_pimutex_lock(void);
extern void rflm_ccs_event_sw_port_pimutex_unlock(void);

void rflm_ccs_event_done_callback (
  /*!< RF handle */
