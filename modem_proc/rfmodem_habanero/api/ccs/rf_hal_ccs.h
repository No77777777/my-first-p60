...
/*==============================================================================
  ...
  $Header: //components/rel/rfmodem_habanero.mpss/1.11/api/ccs/rf_hal_ccs.h#1 $

when       who     what, where, why
--------   ---     ------------------------------------------------------------- 
06/17/16   apm     added ftm for task queue simulation from efs file
08/25/15   sbo     SW port streamline changes.
06/11/15   ra      Clean up rflm ccs event structure
05/20/15   sbo     Support for dump of user requested registers during error handling
06/03/15   sbo     Increase RFLM_CCS_THREAD priority to 26
...
#define RFLM_CCS_THREAD_PRI            26 /*! IST priority, same as the DPM */
...
/*----------------------------------------------------------------------------*/
void rf_hal_bus_set_default_grfc_bit
(
  uint32 grfc_num
);

/*----------------------------------------------------------------------------*/
void rf_hal_bus_clear_default_grfc_bit
(
  uint32 grfc_num
);
...
/*----------------------------------------------------------------------------*/
rf_hal_bus_result_type rf_bus_ccs_command_interface_execute_ext
(
   rf_bus_ccs_cmd_intf_event_type* cmd,
   boolean async_mode
);

#define rf_bus_ccs_command_interface_execute(cmd,...) \
	rf_bus_ccs_command_interface_execute_ext(cmd,(FALSE, ## __VA_ARGS__))
...
/*----------------------------------------------------------------------------*/
void rf_hal_ccs_sleep
(
   void
);

/*----------------------------------------------------------------------------*/
boolean rf_hal_add_rf_register_dump_table
(
   uint32 table_entry_count,
   rf_hal_rf_register_dump_table_entry_type *table_ptr
);
...
/*----------------------------------------------------------------------------*/
 extern boolean rf_hal_rffe_ext_read_bytes_offline(uint8 channel, uint8 slave_id, boolean rd_delay, boolean half_rate, uint8 num_bytes, uint16 addr, uint32 *data);


/*----------------------------------------------------------------------------*/

 extern boolean rf_hal_rffe_read_byte_offline(uint8 channel, uint8 slave_id, boolean rd_delay, boolean half_rate, uint8 addr, uint32* data);

/*----------------------------------------------------------------------------*/

int rflm_ccs_post_mortem_read_debug_file(void);

int rflm_ccs_efs_online_read_debug_file(void);

int is_efs_online_read(void);
rf_hal_bus_resource_script_type* rflm_ccs_get_register_data(rflm_tech_id_t tech,rf_hal_band_type band,rf_hal_mode_type mode,rf_hal_bus_resource_type bus);

char *rflm_ccs_strtok(char *s1, const char *s2, char **s3);


/*----------------------------------------------------------------------------*/
uint32 rflm_ccs_hex_to_int
(
  char *str
);

/*----------------------------------------------------------------------------*/
int rflm_ccs_char_to_int
(
  char *str
);

/*----------------------------------------------------------------------------*/
char *rflm_ccs_stristr
(
  char *  string_to_be_searched , 
  const char *  substring_to_search_for 
);

/*----------------------------------------------------------------------------*/
char *rflm_ccs_strdup 
(
  char *s
);
