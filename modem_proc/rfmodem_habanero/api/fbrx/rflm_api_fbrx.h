...
$Header: //components/rel/rfmodem_habanero.mpss/1.11/api/fbrx/rflm_api_fbrx.h#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
08/31/16   sk      Fix 16/64-QAM modulation backoff for TxAGC and FBRX
08/11/16   sk      New Static NV for ULCA FBRx switch points
07/11/16   aa      FBRx logging fixes to update the lin params and abort data correctly
06/15/16   aa      Added predet_idx_online param in exp gain data
06/13/16   pk      Adjusted LDO ON\OFF timings to fix bad lse in FBRX\VSWR
06/09/16   aa      Fix lin data logging for fbrx 1849
05/24/16   aa      Added triple buffer safety code for fbrx results
03/01/15   aa      Revert the FBRx error theshold change back to 0x1F4
03/01/15   aa      Update the Tx/Rx rms and dc values in dBFS10 units
02/29/16   aa      Changes to udpate the fbrx proc type
...
01/18/16   am      Add each carrier's bw to fbrx dm buff
11/23/15   jj      Pass FW fbrx handle id
10/06/15    aa     Added more FBRx ftm debug commands
09/28/15    aa     Changes for FBRX 0x184A V2 Logging support 
09/21/15    aa     Added suport for FBRx debug ftm command
08/28/15    JJ     FBRx scheduling change for Interband ULCA
08/25/15    aa     FBRX SWPT compensation (dummy on TA)
08/18/15    kg     stage_3 en support
08/17/15    ka     Dynamic ADC settings code cleanup
08/17/15    yb     fix compilation problem
08/07/15    aa     Update v6 logging params
...
06/02/15   zhh    FBRX ILPC optimize
06/02/15    gh/zhh Added support for handling stale data and handling LDO_OFF CB
06/02/15    gh/zhh Added support for flush log buffer for fbrx layer
06/02/15    gh/zhh Add support for additional ILPC immediate correction
07/02/15    bn     TH FBRx feature interface changes
03/31/15    yb     Add ABORT event status for VSWR captures(only interface update)
06/01/15    sub    Add new parameters for local updates, max power nv 
01/29/15    pv     Support for increase the FBRx sample size based on NV control.
01/29/14    tod     NV 11 ILPC fixes
01/06/15    yb     Added status type for FBRx measurements
01/05/15    ka     Restrict operation to high power for WTR4905
12/12/14    aa     Added enum for FBRx gain state
...
11/21/14   oei     Add env_scale to get_exp_gain_in_t and ilpc_est_data_t
...
#define RFLM_FBRX_NUM_GAIN_STATES RFLM_FBRX_GAIN_STATE_MAX     
#define RFLM_FBRX_NUM_MODES 4           
#define RFLM_FBRX_NUM_CAL_CHANNELS 16   
#define RFLM_FBRX_NUM_FILTER_TAPS RFLM_FBRX_RX_FILTER_TAPS_NUM
...
#define RFLM_FBRX_TXAGC_HIST_SIZE 4
#define RFLM_FBRX_TXAGC_HIST_SIZE_HIGH_PWR 2
#define RFLM_FBRX_TXAGC_HIST_SIZE_LOW_PWR RFLM_FBRX_TXAGC_HIST_SIZE
#define RFLM_FBRX_TXAGC_HIST_SIZE_ALGO2_RAMP 6
...
#define RFLM_CDMA_FBRX_DISABLE_TIME_OFFSET_USTMR   1730
...
#define RFLM_FBRX_POWER_RAMP_DETECT_THRESH  -80

#define RFLM_FBRX_ACCUM_ERR_THRESH 0x1F4

#define RFLM_FBRX_NUM_CARRIER_MAX (RFLM_FBRX_NUM_CARRIER_INVALID - 1)
...
typedef enum
{
  RFLM_FBRX_NUM_CARRIER_0 = 0,      ///< indicate 0 carrier 
  RFLM_FBRX_NUM_CARRIER_1 = 1,      ///< indicate 1 carrier 
  RFLM_FBRX_NUM_CARRIER_2 = 2,      ///< indicate 2 carriers
  RFLM_FBRX_NUM_CARRIER_3 = 3,      ///< indicate 3 carriers
  RFLM_FBRX_NUM_CARRIER_INVALID, ///< indicate max num of carriers 
} rflm_fbrx_num_carriers_type;

/*----------------------------------------------------------------------------*/
/*! @brief Enums for FBRx run requested Type */
typedef enum
{
  RFLM_FBRX_SINGLE_CARRIER = 0,
  RFLM_FBRX_LTE_INTRA_ULCA = 1,     ///< indicate LTE intra_band ULCA 
  RFLM_FBRX_LTE_INTER_ULCA_OR_DSDA = 2,  ///< indicate LTE inter_band ULCA or DSDA
  RFLM_FBRX_CARRIER_MODE_INVALID = 3,   ///< indicate invalid Carrier mode
} rflm_fbrx_carrier_mode_type;
/*----------------------------------------------------------------------------*/
...
/*----------------------------------------------------------------------------*/
/*! @brief Gain States supported by FBRx module */
typedef enum
{
  RFLM_FBRX_GAIN_STATE_0,       ///< FBRx Gain State 0
  RFLM_FBRX_GAIN_STATE_1,       ///< FBRx Gain State 1
  RFLM_FBRX_GAIN_STATE_2,       ///< FBRx Gain State 2
  RFLM_FBRX_GAIN_STATE_3,       ///< FBRx Gain State 3
  RFLM_FBRX_GAIN_STATE_MAX,     ///< FBRx Max number of gain states supported
} rflm_fbrx_gain_state_e;

/*----------------------------------------------------------------------------*/
/*! @brief FBRx Modes supported by FBRx module */
typedef enum
{
  RFLM_FBRX_MODE_0,       ///< FBRx Mode 0
  RFLM_FBRX_MODE_1,       ///< FBRx Mode 1
  RFLM_FBRX_MODE_2,       ///< FBRx Mode 2
  RFLM_FBRX_MODE_3,       ///< FBRx Mode 3
  RFLM_FBRX_MODE_MAX,     ///< Max number of fbrx modes supported
} rflm_fbrx_mode_e;
...
typedef enum
{
  ...
  RFLM_FBRX_CCS_GAIN_STATE_0_EVENT, ///< indicates FBRx Gain State 0 Update Event
  RFLM_FBRX_CCS_GAIN_STATE_1_EVENT, ///< indicates FBRx Gain State 1 Update Event
  RFLM_FBRX_CCS_GAIN_STATE_2_EVENT, ///< indicates FBRx Gain State 2 Update Event
  RFLM_FBRX_CCS_GAIN_STATE_3_EVENT, ///< indicates FBRx Gain State 3 Update Event
...
typedef enum
{
  ...
  RFLM_FBRX_MEAS_ABORT,               ///< indicates FBRx measurement has  been aborted
...
/*----------------------------------------------------------------------------*/
/*! @brief Enum used to indicate current WTR for FBRx static settings */
typedef enum
{
  RFCOMMON_FBRX_ADC_MODE1,            ///< WTR3925 
  RFCOMMON_FBRX_ADC_MODE2,            ///< WTR4905
  RFCOMMON_FBRX_ADC_MODE_MAX,
} rfcommon_fbrx_adc_mode_type;

/*! @brief FBRx Run type for each TxAGC iteration for current Slot per SF */
typedef enum
{
  RFLM_FBRX_DEBUG_RUN_INDEX_0_ONLY = 0,   
  RFLM_FBRX_DEBUG_RUN_INDEX_1_ONLY = 1,   
  RFLM_FBRX_DEBUG_RUN_INDEX_2_ONLY = 2,   
  RFLM_FBRX_DEBUG_RUN_INDEX_BOTH_0_1 = 3, 
  RFLM_FBRX_DEBUG_RUN_INDEX_MAX_SUPPORTED = 4, 
} rflm_fbrx_debug_run_type;

/*----------------------------------------------------------------------------*/
/*! 
  @brief
    Bit mask of NV item 27472: Bit3~Bit0
...
typedef enum
{
  RFLM_WCDMA_FBRX_ENABLE         = (0x1 << 0),  //RFCOMMON_FBRX_WCDMA_ENABLE
  RFLM_WCDMA_FBRX_LIN_CORRECTION = (0x1 << 1),  //RFCOMMON_FBRX_WCDMA_LIN_CORRECTION
  RFLM_WCDMA_FBRX_SW_BASED       = (0x1 << 2),  //RFCOMMON_FBRX_WCDMA_SW_BASED
  RFLM_WCDMA_FBRX_IMM_UPDATE     = (0x1 << 3)  //RFCOMMON_FBRX_WCDMA_IMM_UPDATE
}rflm_wcdma_bit_mask_type;

/*! @brief ULCA FBRx switch point NV item idx */
typedef enum
{
  RFLM_FBRX_SP_NV_item_0 = 0,   
  RFLM_FBRX_SP_NV_item_1 = 1,   
  RFLM_FBRX_SP_NV_item_2 = 2,   
  RFLM_FBRX_SP_NV_item_3 = 3, 
  RFLM_FBRX_SP_NV_item_4 = 4, 
  RFLM_FBRX_SP_NV_item_5 = 5,   
  RFLM_FBRX_SP_NV_item_6 = 6,   
  RFLM_FBRX_SP_NV_item_7 = 7,   
  RFLM_FBRX_SP_NV_item_8 = 8, 
  RFLM_FBRX_SP_NV_item_9 = 9,
} rflm_fbrx_sp_nv_item_type;
...
typedef struct
{
  boolean   crash_en;               ///< crash enable flag
  uint16    lse_thresh;             ///< Bad sample threshold LSE = 3% 
  uint16    gain_err_thresh;        ///< FBRx Gain error threshold below which Lin updates are applied 
  boolean   invert_bsc;             ///< invert flag to enable capturing good samples
} rflm_fbrx_bsc_thresh_ctrl_data_t;

typedef struct
{
  boolean   crash_en;               ///< crash enable flag
  uint16    high_gain_est_thresh;   ///< high fbrx gain estimate threshold
  uint16    low_gain_est_thresh;    ///< low fbrx gain estimate threshold
} rflm_fbrx_gain_est_thresh_ctrl_data_t;


typedef enum
{
  RFCOMMON_FBRX_GAIN_STATE = 0,
  RFCOMMON_FBRX_PA_STATE = 1,
}rflm_fbrx_gain_pa_sel_ctrl_type;

typedef enum
{
  RFCOMMON_FBRX_IS_EQUAL = 0,
  RFCOMMON_FBRX_IS_GREATER = 1,
  RFCOMMON_FBRX_IS_LESS = 2,             
}rflm_fbrx_gain_pa_meas_ctrl_type;

typedef struct
{
  boolean                              crash_en;            ///< 
  rflm_fbrx_gain_pa_sel_ctrl_type      gain_pa_state_t;     ///< 
  rflm_fbrx_gain_pa_meas_ctrl_type     gain_pa_meas_ctrl_t; ///< 
  uint8                                gain_pa_state_val;   ///< gain or pa state value
} rflm_fbrx_gain_pa_state_thresh_ctrl_data_t;

typedef struct
{
  boolean  is_override;
  int32    value;
} rflm_fbrx_tx_rx_delay_ctrl_data_t;

typedef struct
{
  boolean is_override;
  uint32 value;
} rflm_fbrx_proc_type_ctrl_data_t;

typedef struct
{
  boolean   crash_en;                     ///< crash enable flag
  uint16    counter_val;                  ///< crash counter value
  int16     high_txagc_thresh;      ///< high txagc threshold
  int16     low_txagc_thresh;       ///< low txagc threshold
  rflm_fbrx_meas_req_type_e meas_type;    ///< Type of the FBRx Measurement request
  rflm_fbrx_debug_run_type   run_type;    ///< Run index
} rflm_fbrx_common_debug_data_t;

/*!  @brief fbrx DM data for fbrx threshold control settings */
typedef struct
{
  rflm_fbrx_bsc_thresh_ctrl_data_t              bsc_data;
  rflm_fbrx_gain_est_thresh_ctrl_data_t         gain_est_data;
  rflm_fbrx_gain_pa_state_thresh_ctrl_data_t    gain_pa_state_data;
  rflm_fbrx_common_debug_data_t                 fbrx_debug_common_data;
  boolean                                       fbrx_debug_crash_condition_success;
  boolean                                       fbrx_debug_run_is_valid[RFLM_FBRX_DEBUG_RUN_INDEX_MAX_SUPPORTED];
} rflm_fbrx_debug_ctrl_data_t;
...
typedef struct
{
  rflm_fbrx_gain_state_e  fbrx_gain_state;         ///< fbrx gain state (0,1,2 or 3) 
  ...
  int8   modulator_adjustment_db10; ///< Adjustment to baseband power applied by the modulator for carrier 0
  int8   modulator_adjustment_cc1_db10; ///< Adjustment to baseband power applied by the modulator for carrier 1
  ...
  uint32  fbrx_meas_capture_id;                 ///< Capture id of the FBRx Measurement request
  rflm_txagc_xpt_mode_t  xpt_mode; ///< APT,EPT,ET
  uint32                 env_scale; ///< Env_Scale for the fbrx run (used by Auto Pin)

  uint16 predet_idx_online;   ///< Predet idx used in online mode
...
typedef struct
{
  ...
  rflm_fbrx_mode_e fbrx_mode;           ///< flag indicating the fbrx mode (0,1,2 or 3)
  ...
  int32 log_results_context_id;           ///< rflm log packet context id for logging fbrx uk results 
  int32 log_captures_context_id;          ///< rflm log packet context id for logging fbrx sample captures  
  int32 log_results_basic_context_id;     ///< rflm log packet context id for logging basic fbrx uk results 
  ...
  uint8 fbrx_gain_chan_size;            ///< FBRx gain chan size in cal
  ...
  rflm_ustmr_t prev_last_ldo_time;      ///< Time at which the previous FBRx LDO was last turned on 
  ...
  uint16 nref;                          ///< Number of reference samples used, needed for overflow-underflow  
  uint8 fbrx_ilpc_activated;            ///< FBRx ILPC actived history - keeps track of current
                                        ///< (LSB) and 7 prev states.
  boolean increase_sample_size_nv_status;   ///< Tx Sample size increased status
  boolean apt_only_mode;                ///< Indicates if the phone uses only APT
  int16 mtpl_db10;                      ///< Desired max power limit in dB10
  uint32 fbrx_adc_config3_reg;          ///< FBRx ADC adca_config3_reg value
  uint8 container_val;                 ///< NV Container mode
  uint32 tx_handle;                     ///< Tx handle info
  boolean autopin_run_flag;             ///< flag indicating that autopin will run on this FBRX
  boolean skip_clamp_thres;
  uint32 num_tx_in_samples;             ///< Num of aggregated Tx samples read from FTR-IB for ca0/ca1 
  uint32 num_rx_in_samples;             ///< Num of Rx samples to be read from RxFE output (num_rx_samples+num_rx_taps-1)
  rfcommon_fbrx_tx_cfg_type bw_per_carrier;  ///< each carrier's bw. for LTE intraband ONLY, SCC bw is 20+20, but we also need scc's carrier bw. 
  uint8 fbrx_chain;
  uint16 predet_idx_online[RFLM_FBRX_MODE_MAX][RFLM_FBRX_GAIN_STATE_MAX];   ///< predet index to be used in online mode              

  int32  tx_rms_dbfs;       ///< Tx Rms: RFSW to convert to dBFS 
  int32  tx_dc_dbfs;        ///< Tx dc : RFSW to convert to dBFS
  int32  rx_rms_dbfs;       ///< Rx Rms: RFSW to convert to dBFS
  int32  rx_dc_dbfs;        ///< Rx dc : RFSW to convert to dBFS

  boolean wcdma_adaptive_pa_comp_update;

  uint8 fbrx_results_buf_idx;

} rflm_fbrx_ctl_data_t;

/*----------------------------------------------------------------------------*/
/*! @brief FBRx Log contex id data */
typedef struct
{
  int32 log_results_cid;           ///< rflm log packet context id for logging fbrx uk results 
  int32 log_captures_cid;          ///< rflm log packet context id for logging fbrx sample captures  
  int32 log_results_basic_cid;     ///< rflm log packet context id for logging basic fbrx uk results 
} rflm_fbrx_log_context_id_data_t;
...
typedef struct
{
  ...
  ///< table containing the updated filtered error 
  int16 filt_error_db10[RFLM_TXAGC_PA_STATE_MAX][RFLM_TXAGC_TX_LIN_APT_SIZE]; 
  ///< table containing the updated tx Lin smps biased power 
  int16 tx_lin_data[RFLM_TXAGC_PA_STATE_MAX][RFLM_TXAGC_TX_LIN_APT_SIZE]; 
  ...
  int16 curr_pwr;
  int16 updated_pwr;
  int32 max_abs_lse[RFLM_FBRX_MAX_RUNS_PER_SF];
  uint16 lse_thresh[RFLM_FBRX_MAX_RUNS_PER_SF];
  boolean l_all_rgi_updated_flag;
  rflm_txagc_xpt_mode_t  xpt_mode[RFLM_FBRX_MAX_RUNS_PER_SF];
  uint8  last_rb_blocks[RFLM_FBRX_MAX_RUNS_PER_SF]; ///< number of rb blocks 
  boolean lin_err_data_to_be_updated[RFLM_FBRX_MAX_RUNS_PER_SF];
  uint16 return_capture_id;
...
typedef struct
{
  ...
  int8 fbrx_updates[RFLM_TXAGC_PA_STATE_MAX][(RFLM_TXAGC_TX_LIN_APT_SIZE+7)/8]; 
  ...
  uint8 last_pa_state;
  int16 tx_agc_hist;
  uint8 tpc_run_length;
  int16  prev_tpc_run_length;
  int8  prev_tx_agc_delta_sign;
  uint8 zero_delta_run_length;
  boolean power_ramp_active;
} rflm_fbrx_run_dec_data_t;


/*----------------------------------------------------------------------------*/
/*!  @brief fbrx swpt control data */
typedef struct
{
  ///< error injected dbm10
  boolean triggered; // used for FBRX SWPT
  uint32  trigger_time; 
  int8    ramp_direction;
  uint8   expected_gain_state;
  int16   expected_txagc;
}  rflm_fbrx_run_dec_swpt_data_t;
...
typedef struct
{
  ...
  rflm_fbrx_tx_rx_delay_ctrl_data_t   user_tx_rx_delay;
  rflm_fbrx_proc_type_ctrl_data_t user_fbrx_proc_type;
...
typedef struct {
  ...
  void (*enqueue_ldo_off_cb)(rflm_handle_tx_t, rflm_ustmr_t);///< CB for scheduling LDO OFF
...
typedef struct
{
  ...
  uint32 incident_cgain_est;
  uint32 reflected_cgain_est;
  uint32 last_action_time; /* hold action time when last fbrx cl event is committed */
...
RFLM FBRX Event Template Type :  SW to FBRX Main Interface
------------------------------------------------------------------------------*/
/*! @brief	Structure to hold ULCA switch points */
typedef struct
{
    boolean valid_swp;  ///< flag to check if there are valid ulca swp. 
    int16 fbrx_swp_ulca[RFLM_FBRX_NUM_GAIN_STATES];			   ///< ULCA swp>
} rflm_fbrx_ulca_swp_data_t;
...
typedef struct
{
  ...
  rflm_fbrx_debug_ctrl_data_t    fbrx_debug_ctrl;  ///< fbrx debug control data for debug 
  ...
  rflm_fbrx_ulca_swp_data_t      fbrx_ulca_swp_data; ///< FBRx ULCA Switchpoints
  union {
    struct {
  ...
  rflm_fbrx_update_results_in_t  results_log[RFLM_FBRX_MAX_BUFFER_SIZE];   ///< store the uK processing results
  ...
  rflm_fbrx_tech_post_process_vswr_measurement_fp_type post_process_vswr_meas_fp;/* Call back function for post processing the
                                                                                    VSWR measurements*/
  rflm_fbrx_dynamic_param_t fbrx_dyn_params; /// FBRX dynamic params which includes TXC rollover period information 
...
typedef struct
{
  ...
  int8                           modulator_adjustment_cc1_db10;
  ...
  boolean                        is_fbrx_gain_state_overriden;
  uint8                          overriden_fbrx_gain_state;
  boolean                        skip_clamping_threshold;
...
/*==============================================================================
  For rflm_fbrx_swpt_get_run_decision
==============================================================================*/
/*! @brief  Input parameters for rflm_fbrx_swpt_get_run_decision API */
typedef struct
{
  int16 tx_agc;
} rflm_fbrx_swpt_get_run_decision_in_t;

/*! @brief  Output parameters for rflm_fbrx_swpt_get_run_decision API */
typedef struct
{
  boolean run_fbrx_swpt; 
  uint8 fbrx_gain_state;
} rflm_fbrx_swpt_get_run_decision_out_t;
...
/*!
  @brief
  Decides if FBRx SWPT capture should be run for a given TxAGC iteration.
...
===========================================================================*/
extern rflm_err_t rflm_fbrx_swpt_get_run_decision( rflm_handle_tx_t handle,
                                              rflm_fbrx_swpt_get_run_decision_in_t  *in,
                                              rflm_fbrx_swpt_get_run_decision_out_t *out );
...
===========================================================================*/
extern boolean rflm_fbrx_configure_chain( rflm_handle_rx_t handle, uint8 fbrx_chain );
...
extern void rflm_fbrx_init_thresh_control_data( rflm_fbrx_dm_template_t *fbrx_data_ptr );
/*----------------------------------------------------------------------------*/
/*!
  @brief
...
/*----------------------------------------------------------------------------*/
extern rflm_fbrx_gps_state_type rflm_fbrx_get_gps_state( rflm_handle_tx_t tx_handle );
...
extern rflm_err_t rflm_fbrx_update_dynamic_adc_settings(rflm_handle_tx_t handle_id);

extern rflm_err_t rflm_fbrx_get_dynamic_adc_settings( rflm_handle_tx_t handle_id, uint32 *adc_mask );

extern rflm_err_t rflm_fbrx_get_stage3_enable_setting( rflm_handle_tx_t handle_id ,boolean *stage_3_en);
...
extern void rflm_fbrx_log_sample_capture_buffer_alloc_cb( void );
extern boolean rflm_fbrx_get_log_capture_buffer_available_status(void );
extern int32 rflm_fbrx_get_log_context_id(int32 fbrx_log_id);

/* FBRx debug related APIs */
void rflm_fbrx_debug_bsc_results( rflm_fbrx_dm_template_t *fbrx_data_ptr, uint8 run_idx, uint8 results_buf_idx, uint8 read_buf_idx );
void rflm_fbrx_debug_gain_est_results( rflm_fbrx_dm_template_t *fbrx_data_ptr, uint8 run_idx, uint8 results_buf_idx, uint8 read_buf_idx);
void rflm_fbrx_debug_gain_pa_state_results( rflm_fbrx_dm_template_t *fbrx_data_ptr, uint8 run_idx, uint8 read_buf_idx);
