...
$Header: //components/rel/rfmodem_habanero.mpss/1.11/hal/wcdma/inc/rfwcdma_msm.h#1 $
...
07/20/15   nv      Update tx rot resolution bit width
04/13/15   sd      Add msm interface to reprogram cal codes for DACs
10/17/14   sd      Enable NBEE RX calibration
...
11/18/14   rmb     Add dynamic functions in MSM to compile for Tabasco.
11/10/14   aro     Interface to config offline clock for XPT IQ capture
11/10/14   vs      Added API to return the static DVGA offsets from the RXLM SS.
...
#define RFWCDMA_MSM_TX_ROT_ROSOLUTION          22

/* qeuried with rfwcdma_msm_get_txc_input_freq()*/
#if 0
...
typedef enum
{
  ...
  /* During regular tune for SCell for PRx and DRx as SCell is not tuned when dynamic callback is made */
  RFWCDMA_MSM_STATIC_PLUS_DYNAMIC_NOTCH_TUNE,
...
/*Common Notch  Config*/
  rfwcdma_msm_wb_notch_filter_cfg wb_notch_01[RFWCDMA_NUM_WB_NOTCH_GROUP];
  rfwcdma_msm_wb_notch_filter_cfg wb_notch_23[RFWCDMA_NUM_WB_NOTCH_GROUP];
...
/*--------------------------------------------------------------------------*/
boolean rfwcdma_msm_set_cw(rfm_device_enum_type device, boolean on_off, int32 freq_offset,lm_handle_type txlm_handle);
...
/*--------------------------------------------------------------------------*/
boolean rfwcdma_msm_cmn_notch_filter_reallocate(rfcom_device_enum_type device, uint32 rxlm_buf_idx, boolean is_rat);

/*--------------------------------------------------------------------------*/
boolean rfwcdma_msm_cmn_notch_filter_deallocate(rfcom_device_enum_type device, uint32 rxlm_buf_idx, boolean is_rat);
/*--------------------------------------------------------------------------*/

void rfwcdma_msm_dac_reprogram_tx_msbcal_code(rfcom_device_enum_type device,  rfcom_wcdma_band_type band,  boolean etdac_setup);

/*----------------------------------------------------------------------------*/

uint32 rfwcdma_msm_get_txc_input_freq(lm_handle_type txlm_handle);
