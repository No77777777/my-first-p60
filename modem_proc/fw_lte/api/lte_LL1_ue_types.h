...
$Header: //components/rel/fw_lte.mpss/17.0.c4/api/lte_LL1_ue_types.h#1 $
...
typedef enum
{
  ...
  LTE_LL1_UE_MOD_SCC_ACTIVATE       = 0x4000    ///< SCC is getting activated by NW.
} lte_LL1_ue_mod_e;
...
#define LTE_LL1_VALID_FREEZE_UPDATE_MASK (  (uint32)(LTE_LL1_UE_FREEZE_UPDATE_MOD_AGC) | (uint32)(LTE_LL1_UE_FREEZE_TDD_RXON_TOGGLE)  )
...
/*! @brief Enum for preserving SCC HARQ in FW */
typedef enum
{
  LTE_LL1_HARQ_NO_CHANGE = 0,		/* no change in EMEM or HARQ states */
  LTE_LL1_HARQ_ACQUIRE = 0x01,		/* request EMEM (when acquiring demback); reset HARQ (when acquiring demback) */
  LTE_LL1_HARQ_RELEASE = 0x02,		/* release EMEM (when releasing demback); */

} lte_LL1_harq_state_s;

typedef enum {
  LTE_LL1_INVALID_MODE,
  LTE_LL1_NO_USLEEP,
  LTE_LL1_LIGHT_USLEEP,
  LTE_LL1_DEEP_USLEEP,
  LTE_LL1_FORCE_DEEP_USLEEP,
  LTE_LL1_LIGHT_USLEEP_CDRX
} lte_LL1_usleep_mode_e;
