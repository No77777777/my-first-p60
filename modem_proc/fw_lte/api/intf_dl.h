...
$Header: //components/rel/fw_lte.mpss/17.0.c4/api/intf_dl.h#2 $
...
#ifdef LTE_LL1_FEATURE_THOR_HORXD
//check if we need this for iq sample capture
#define LTE_LL1_IQ_SAMP_CAPTURE_RX2_MEMIDX_OFFSET 2*LTE_LL1_IQ_SAMP_CAPTURE_MAX_NUM_SAMPLES //offset of RX1 iq sample in the shared mem 
#define LTE_LL1_IQ_SAMP_CAPTURE_RX3_MEMIDX_OFFSET 3*LTE_LL1_IQ_SAMP_CAPTURE_MAX_NUM_SAMPLES //offset of RX1 iq sample in the shared mem 
#define LTE_LL1_IQ_CAPTURE_RX2_BIT 2 ///< RX0 bit is bit 0
#define LTE_LL1_IQ_CAPTURE_RX3_BIT 3 ///< RX1 bit is bit1 
#endif
...
typedef enum
{
  ...
  LTE_LL1_MBMS_RNTI,
  LTE_LL1_RNTI_COUNT,
  /* RLM_RNTI is a dummy RNTI used by ML1 to trigger RLM, it does NOT increase RNTI_COUNT;
     this bit position must be set by ML1 in rnti_enable_mask in lte_LL1_dl_pdcch_params_struct;
     there is no RNTI value to go with it, it is just a bit position to trigger FW RLM */
  LTE_LL1_RLM_RNTI = LTE_LL1_RNTI_COUNT
} lte_LL1_rnti_type_enum;

typedef enum
{
  LTE_LL1_DCH_RNTI_TYPE = 9,
  LTE_LL1_SCH_RNTI_TYPE,
} lte_LL1_lted_rnti_type_enum;
...
typedef struct {
  ...
  uint16   resource_config; ///< 0�31(36.211 table 6.10.5.2-1 & table 6.10.5.2-2). 
...
/*! @brief HARQ Throtlling enable/disable request input structure
*/
typedef struct {
  lte_LL1_req_header_struct req_hdr;

  boolean harq_throttling_enable;    ///< 0: disable, 1: enable
} lte_LL1_harq_throttling_enable_disable_req_struct;
typedef struct {
  msgr_hdr_struct_type msg_hdr;
  lte_LL1_harq_throttling_enable_disable_req_struct msg_payload;
} lte_LL1_harq_throttling_enable_disable_req_msg_struct;

/*! @brief TDEC related clock rate information from ML1
*/

typedef struct 
{
  lte_LL1_req_header_struct req_hdr;
  uint32 tdec_clock_rate; /* Clock Rate of Turbo Decober in units of kHZ */
  uint32 ccs_clk_rate;    /* Clock Rate of CCS in units of kHZ */
  uint32 descr_deintlv;   /* Clock Rate of Descrambler/Deinterleaver in units of KHz */
  uint32 dma_bw;  /* mega bytes per msecs */
} lte_LL1_dl_clock_rate_info_req_struct;

typedef struct {
  msgr_hdr_struct_type msg_hdr;
  lte_LL1_dl_clock_rate_info_req_struct msg_payload;
} lte_LL1_dl_clock_rate_info_req_msg_struct;
// --------------------------------------------------------------------------
//
...
/*! @ brief parameter structure HARQ Throtlling enable/disable confirmation
 */
typedef struct { 
  lte_LL1_cnf_header_struct cnf_hdr;
  lte_LL1_dl_cnf_enum err_code;
} lte_LL1_harq_throttling_enable_disable_req_cnf_struct;
typedef struct {
  msgr_hdr_struct_type                      msg_hdr;      ///< message router header
  lte_LL1_harq_throttling_enable_disable_req_cnf_struct   msg_payload; 
} lte_LL1_harq_throttling_enable_disable_cnf_msg_struct;
...
typedef struct {    
  ...
  // s-vector for PUSCH allocation type 1, see 36.213 section 8.1.2 "Uplink resource allocation type 1"
  uint8 s_index[LTE_LL1_UL_PUSCH_NUM_CLUSTERS*2];
#endif
  ...
  //whenever pusch is droppeed by FW either due to QTA or DSDA
  //we indicate to the ML
  boolean  is_ul_dropped;

  // True if uSleep (micro-sleep) kicked in this SF
  boolean  uSleep_kicked_in;

  /* usleep duration in units of ODFM symbols */
  uint8 usleep_duration; 

  //True if it's fake pdcch SF
  boolean is_fake_PDCCH_sf;
...
typedef struct {    
  ...
  int32 rssi[LTE_LL1_MAX_NUM_UE_RX_ANT];                     ///< RSSI per Rx antenna in dBm. Q-format: 32S24 
...
#ifdef FEATURE_XMOD
  uint8 alt_cqi_table;    ///< 0: allSubframes; 1: csi-SubframeSet1; 2: csi-SubframeSet2; 0xFF: not configured
#endif
  boolean preserve_pmi_reports; ///< if set will not clear the PMI report buffer for implicit PMI reporting
  boolean restore_prev_rank; /*don't RESET rank - use the previous rank. Use for L+L interruption*/
...
typedef struct 
{
  ...
  boolean use_sw_timing;     ///< Flag to indicate use SW timing for serving cell pbch
...
typedef struct
{
  ...
  int8  rsp_delay;              ///< diff b/w SFN of decoded RV & that of Cnf
                                ///< both according to serv cell time
  int16  decode_sfn;            ///< serv SFN corresp. to the serv RV, over which
                                ///< an IC cell is decoded
  ...
  boolean is_sfn_sync;          ///< TRUE if IC's 40-ms boundary is aligned w/ serv
                                ///< valid ONLY if crc_pass is TRUE
...
typedef struct {
  ...
  uint32    subframe_offset;   /* neighbor cell offset from serving frame bndry, ranging from 0 to 9; when setting up ic cell, FW needs to add offset to neighbor cell and mod by 10 */
...
#if 1//def HABANERO_MERGE
  /* Check This Variable */
  //AT uses it, HE does not has it
  boolean enable_crs_detection;
#endif
} lte_LL1_dl_ic_cell_info_struct;

#if 1//def HABANERO_MERGE
/*! @brief QICE enable mode enum. Different modes are mapped to
           different serving vs nbr RSRP delta SNR ranges.
*/
typedef enum {
  LTE_LL1_DL_QICE_DISABLE,        /* Disable QICE */
  LTE_LL1_DL_LQICE_ENABLE,        /* Enable Legacy QICE */
  LTE_LL1_DL_QICE_RESERVED,       /* Reserved item */
  LTE_LL1_DL_PLQICE_ENABLE,       /* Enable Partial Loading QICE */
  LTE_LL1_DL_QICE_ENABLE_MODE_COUNT,
} lte_LL1_dl_lqice_enable_mode_e;
...
#endif
/* allowed meas bandwidth for RSRP-IC;
   RRC IE has ENUMERATED {mbw6, mbw15, mbw25, mbw50, mbw75, mbw100}; 
   ML1 decide 6 or 8 RB based on it; because NB meas only use 6 or 8, no more than that */
typedef enum {
  LTE_LL1_ALLOWED_MEAS_BW_6_RB,
  LTE_LL1_ALLOWED_MEAS_BW_8_RB
} lte_LL1_allowed_meas_bw_e;
...
typedef struct {
  ...
  */
  //Added from HE, AT
  lte_LL1_dl_lqice_enable_mode_e lqice_enable_mode;

  /* Check This Variable */
  //AT uses this Var But HE does not
...
#ifndef SIM_PLATFORM
  /* Check This Variable */
  //Neither AT, not HE uses it
  /* 6 or 8 RB, ML1 decide based on RRC IE AllowedMeasBandwidth */
  //
  lte_LL1_allowed_meas_bw_e allowed_meas_bw;
#endif
  /* max 3 IC cell info */
...
/*! @brief  Colliding cell ind to enable min-mode
*/
typedef struct {
  lte_LL1_carrier_type_enum   carrier_type;
  boolean                     is_predom_colliding_cell_detected;
} lte_LL1_dl_predominant_colliding_interf_update_carrier_info;

typedef struct {
  lte_LL1_req_header_struct                 req_hdr;          ///< request header
  uint8                                     num_of_carrier;
  lte_LL1_dl_predominant_colliding_interf_update_carrier_info    predom_colliding_interf_carrier_info[LTE_LL1_DL_NUM_CARRIERS];
} lte_LL1_dl_predominant_colliding_interf_update_req_struct;

LTE_LL1_INTF_MSG( dl_predominant_colliding_interf_update_req );
