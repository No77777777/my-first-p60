...
$Header: //components/rel/fw_lte.mpss/17.0.c4/api/lte_LL1_log_csf.h#2 $
...
---------------------------------------------------------------------------*/
#define LTE_LL1_LOG_CSF_WHITENED_MATRICES_LOG_VERSION 22
#define LTE_LL1_MAX_NUM_WHITENING_SAMPLES 416
 ...
 *  from ML. 
---------------------------------------------------------------------------*/
#define LTE_LL1_LOG_CSF_PUSCH_VERSION 142
...
typedef struct
{
  ...
  uint32 start_system_frame_number         : 10;  ///< Range 0..1023
  uint32 carrier_index                    : 4; ///< 0-PCC, 1-SCC
  uint32 scell_idx                        : 5; ///< scell index
  uint32 reserved0                        : 1;

  uint32 pusch_reporting_mode             : 3;   ///< Range: 0..7
                                                          ///< 0: MODE_APERIODIC_RM12
                                                          ///< 1: MODE_APERIODIC_RM20
                                                          ///< 2: MODE_APERIODIC_RM22
                                                          ///< 3: MODE_APERIODIC_RM30
                                                          ///< 4: MODE_APERIODIC_RM31
                                                          ///< 5: MODE_APERIODIC_RM32
                                                          ///< 6: MODE_APERIODIC_RM10
                                                          ///< 7: MODE_APERIODIC_RM11
  ...
  uint32 wideband_cqi_cw1                 : 4;   ///< Range: 0...15
                                                          ///< This field is valid only when pusch_reporting_mode = 0 or 2 or 4 or 7
                                                          ///< i.e, in reporting modes 1-2, 2-2, 3-1 & 1-1
  ...
  uint32 alt_cqi_tab                           : 1;  ///< Range: 0..1 0=> legacy table. 1=> ALT table
  ...
  uint32 num_csirs_ports                       : 4;  ///< Range: 1...8
  uint32 cri                               : 3;   ///< CRI index
  uint32 reserved4                         : 1;

  uint32 ul_frame_num                      : 10;  ///< UL frame number
  uint32 ul_subframe_num                   : 4;   ///< UL subframe number
  uint32 payload_len                       : 7;   ///< UL payload length
  uint32 reserved5                         : 11;
...
/*==========================================================================*/
/*! @brief
 *  CSF adaptation algorithm logging (Log code ID: 0xB147)
---------------------------------------------------------------------------*/
/* Note: DONT change constants in array below. They are required for correct apex parsing. 
         any optimization needs new apex version */
#define LTE_LL1_LOG_CSF_ADAPTATION_VERSION 131
#define LTE_LL1_LOG_CSF_ADAPTATION_NUM_RECORDS_FIXED (1)
#define LTE_LL1_CSF_MAX_SUPPORTED_RANK 2

typedef struct
{
  uint32                                  update                      : 1;      ///< was CQI table updated (0/1)
  uint32                                  NomSEIndex                  : 8;      ///< Nominal SE
  uint32                                  mcs                         : 5;      ///< MCS for this codeword
  uint32                                  num_cbs_decoded             : 6;      ///< num codeblocks
  uint32                                  num_cbs_crc_pass            : 6;      ///< num codeblocks passed CRC
  uint32                                  num_tx                      : 3;      ///< retransmission index (0-3)
  uint32                                  reserved2                   : 3;
  int32                                   log_SpefPerLayer_S32Q12;              ///< per layer spectral efficiency
  int32                                   CQI;                                  ///< TX-CQI
  uint8                                   MCS2CQI[30];                          ///< MCS to CQI table of dimensions [LTE_LL1_CSF_MAX_SE_BIN]
  uint16                                  log_cqiState                : 8;      ///< CQI state
  uint16                                  reserved3                   : 8;      ///< padding for next field to start at 4byte boundary
  int32                                   delta_S32Q12;                         ///< CQI table change magnitude
  int16                                   BLERAll_S16Q14[30];                   ///< BLER of non-first TX of dimensions [LTE_LL1_CSF_MAX_SE_BIN]
  int16                                   Activity_S16Q14[4][30];               ///< Activity of dimensions [LTE_LL1_CSF_MAX_TRANSMISSIONS][LTE_LL1_CSF_MAX_SE_BIN];
  int16                                   BLER_S16Q14[4][30];                   ///< BLER of 1st Tx of dimensions [LTE_LL1_CSF_MAX_TRANSMISSIONS][LTE_LL1_CSF_MAX_SE_BIN];
  int16                                   CQIActivity_S16Q14[16];               ///< cqi activity of dimensions [16]
  int16                                   Kappa_S16Q14[30];                     ///< Kappa of dimensions [16]
  int32                                   CQIActualRate_S32Q12[16];             ///< cqi actual rate of dimensions [16]
  int32                                   newSumSE_S32Q12[4];                   ///< per rank newSumSE
} lte_LL1_log_csf_cqi_adaptation_record_s;

typedef struct
{
  uint32                                  RankBiasUpdate              : 2;      ///< was rank bias updated (0/1)
  uint32                                  log_rankState_cw0           : 8;      ///< rank state for codeword0
  uint32                                  log_rankState_cw1           : 8;      ///< rank state for codeword1
  uint32                                  reserved1                   : 14;
  int32                                   RankRate_S32Q12[4];                   ///< per-rank rate of dimensions [LTE_LL1_CSF_NUM_RANK];
  int32                                   RankCSFSE_S32Q12[4];                  ///< per-rank sum-SE of dimensions [LTE_LL1_CSF_NUM_RANK];
  int16                                   RankActivity_S16Q14[4];               ///< per rank activity of dimensions [LTE_LL1_CSF_NUM_RANK];
  int32                                   rankSpefBiasDelta_S32Q12[4];          ///< per rank bias delta of dimensions [LTE_LL1_CSF_NUM_RANK];
  //int32                                   rankSpefBiasDeltaNonNeighbor_S32Q12[3];   ///< per rank bias neighbour of dimensions 3;
  int32                                   rankSpefBiasDeltaNonNeighbor_S32Q12[3];   ///< per rank bias neighbour of dimensions 3;
  int32                                   my_rank_spef_bias_S32Q12[4];          ///< per rank bias of dimensions [LTE_LL1_CSF_NUM_RANK];
  //int32                                   my_rank_spef_bias_non_neighbor_S32Q12[3]; ///< per rank bias neighbour of dimensions 3;
  int32                                   my_rank_spef_bias_non_neighbor_S32Q12[3]; ///< per rank bias neighbour of dimensions 3;
  int32                                   init_rankSpefBiasDelta_S32Q12[4];     ///< per rank bias delta of dimensions [LTE_LL1_CSF_NUM_RANK];
  int32                                   init_my_rank_spef_bias_S32Q12[4];     ///< per rank bias of dimensions [LTE_LL1_CSF_NUM_RANK];
} lte_LL1_log_csf_rank_adaptation_record_s;

typedef struct
{
  int32                                   alphaAct_S16Q14           : 16;
  int32                                   InitBLER_S16Q14           : 16;
  int16                                   ActivBler_S16Q14[2];
  int16                                   ActivRank_S16Q14[2];
  uint32                                  SumRateAlpha_U16Q15       : 16;
  uint32                                  alphaBler_U16Q15          : 16;
  int32                                   deltaUp_S32Q12;
  int32                                   deltaDown_S32Q12;
  int32                                   deltaRankBias_S32Q12;
  int32                                   CQIAdaptAggres_S32Q12;
  int32                                   myMaxCorrection_S32Q12;
  uint32                                  globalUpdate              : 8;
  uint32                                  LimitedCQIAdapt           : 8;
  uint32                                  MaxCorrectPerCQI          : 8;
  uint32                                  reserved_0                : 8;
} lte_LL1_log_csf_tune_params_record_s;

typedef struct
{
  uint32                                      sfn                     : 16;     ///< frame number (0-1023)
  uint32                                      subfn                   : 8;      ///< subframe number (0-9)
  uint32                                      Rank                    : 4;      ///< rank (0-3)
  uint32                                      num_cw                  : 4;      ///< number of codewords (1-2)
  int32                                       init_cqitable_S32Q12[4][15];      ///< CQI table of dimensions [LTE_LL1_CSF_NUM_SUPPORTED_RANK][LTE_LL1_CSF_NUM_SUPPORTED_CQI];
  int32                                       cqitable_S32Q12_cw0[4][15];       ///< CQI table of dimensions [LTE_LL1_CSF_NUM_SUPPORTED_RANK][LTE_LL1_CSF_NUM_SUPPORTED_CQI] for codeword0;
  int32                                       cqitable_S32Q12_cw1[4][15];       ///< CQI table of dimensions [LTE_LL1_CSF_NUM_SUPPORTED_RANK][LTE_LL1_CSF_NUM_SUPPORTED_CQI] for codeword1;
  lte_LL1_log_csf_cqi_adaptation_record_s     cqi_record[2];                    ///< CQI adaptation records per codeword [LTE_LL1_CSF_NUM_CODEWORDS_MAX];
  lte_LL1_log_csf_rank_adaptation_record_s    rank_record;                      ///< Rank adaptation record
  lte_LL1_log_csf_tune_params_record_s        tune_params;                      ///< systems tune params
} lte_LL1_log_csf_adaptation_record_s;

/*! @brief lte_LL1_log_csf_adaptation_record_s
    Array size is number_of_records
*/
typedef struct
{
  // WORD0
  uint32                                          version                                                      : 8;      ///< Log packet version. Range: 0...255.
  uint32                                          carrier_index                                                : 3;      ///< 0-PCC, 1-SCC_0, 2-SCC_1, 3-SCC_2, 4-SCC_3
  uint32                                          number_of_records                                            : 5;      ///< Range: 1-10
  uint32                                          reserved0                                                    : 16;
  lte_LL1_log_csf_adaptation_record_s             record[ LTE_LL1_LOG_CSF_ADAPTATION_NUM_RECORDS_FIXED ];
} lte_LL1_log_csf_adaptation_ind_struct;
