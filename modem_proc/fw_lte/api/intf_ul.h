...
$Header: //components/rel/fw_lte.mpss/17.0.c4/api/intf_ul.h#1 $
...
#define  LTE_LL1_UL_CARRIER_PCC    0
#define  LTE_LL1_UL_CARRIER_SCC_0  1
#define  LTE_LL1_UL_NUM_CARRIERS   2
//#define  LTE_LL1_UL_NUM_SLOTS      3
#define  LTE_LL1_UL_NUM_SLOTS      4
#define  LTE_LL1_UL_INVALID_PWR    0x7fffffff

#define  LTE_LL1_UL_SINGLE_TAG_TA_ADJ LTE_LL1_UL_NUM_CARRIERS
#define  LTE_LL1_UL_PUSCH_NUM_CLUSTERS   2
...
typedef enum
{   
   LTE_LL1_SRS_CELL_SPECIFIC = 0,  ///<. cell SRS, last symbol will be dropped
   LTE_LL1_SRS_UE_SPECIFIC   = 1,  ///<. UE-SRS, last symbol will be used to send SRS
   LTE_LL1_SRS_DROP_LAST_SYM = 2   ///<. in CA single TAG, we need to drop last symbol if SRS is sent on the other carrier
...
/*! @brief type of UL resource allocation */
typedef enum
{
  PUSCH_RES_ALLOC_TYPE_0,
  PUSCH_RES_ALLOC_TYPE_1,
}lte_LL1_pusch_resource_alloc_type_t;
#endif
...
#ifdef TA_ALIGN_THOR_API_20
  uint8  num_rb_cluster[LTE_LL1_UL_PUSCH_NUM_CLUSTERS];    ///< number of RB's allocated for multi-cluster PUSCH transmission 
#endif
...
typedef struct
{
...
#ifdef TA_ALIGN_THOR_API_20
   lte_LL1_pusch_resource_alloc_type_t        res_alloc_type;      ///< RB resource allocation type
   uint8  starting_rb_k0_cluster[LTE_LL1_UL_PUSCH_NUM_CLUSTERS];   ///< starting resource block from UL grant for multi-cluster PUSCH
   uint8  num_rb_cluster[LTE_LL1_UL_PUSCH_NUM_CLUSTERS];           ///< number of RB's allocated for multi-cluster PUSCH transmission 
#endif
   ...
   boolean  is_psdch; // is_psdch == True it means we are scheduling LTE-D UL
                      // is_psdch ==  False it means we are scheduling WWAN UL
...
typedef struct 
{
   ...
   lte_l1_tx_antenna_selection_e          ant_switch_type;
...
typedef struct 
{
   ...
   uint8                        ul_carrier_index; ///< UL Carrier Index;
   lte_LL1_carrier_type_enum    dl_carrier_type;  ///< corresponding DL carrier type
   uint8                        tag_id;           ///< TAG ID for this UL carrier
...
typedef struct
{
   ...
   lte_LL1_ul_chan_config_enum   chan_type;       ///<  indicates type of channel to be configured. 
                                                  ///<  PUSCH and PUCCH are configured together
   uint8                         ul_carrier_idx;  ///<  Configured UL carrier.
...
typedef struct
{
   ...
   lte_LL1_ul_chan_config_enum    chan_type;       ///<  indicates type of channel to be configured.
                                                   ///<  PUSCH and PUCCH are configured together
   uint8                          ul_carrier_idx;  ///< De-configured UL carrier.
...
/*! @brief phychan schedule req for UL CA
*/
typedef struct 
{
   lte_LL1_ul_chan_type_enum          chan_type;         ///< PRACH / PUSCH / PUCCH / SRS only / UpPTS
   uint8                              carrier_index;     ///< The UL carrier index
   lte_LL1_srs_dyn_params_struct      srs_dyn_params_fdd_tdd0;///< SRS for either fdd or tdd0 
   lte_LL1_srs_dyn_params_struct      srs_dyn_params_tdd1;  ///< SRS for tdd1 
   lte_LL1_ul_dyn_params_union        ul_dyn_params_un;  ///<.
   /* These apply only for the duration of the subframe */ 
   lte_LL1_ul_cxm_params_s            cxm_dyn_params;
   boolean                            capture_dtr_out;   ///< True if we want to capture UP-3 for this subframe
                                                         ///< for this UL carrier.
   int32                              P_max_carrier[LTE_LL1_UL_NUM_SLOTS];///< Max power allowed per carrier

} lte_LL1_ul_carrier_config_t;

typedef struct 
{
   lte_LL1_req_header_struct          req_hdr;          ///< request header
   uint16                             num_carrier;      ///< number of carriers
   lte_LL1_ul_carrier_config_t        carrier_cfg[LTE_LL1_UL_NUM_CARRIERS];
   int32                              P_max_total; ///< P_max total over all carriers
   int32                              dummy_for_64bit_allignment;
} lte_LL1_ul_phychan_schedule_ca_req_struct;

LTE_LL1_INTF_MSG( ul_phychan_schedule_ca_req );
...
// --------------------------------------------------------------------------
//
// Per Carrier TX LM config 
//
// --------------------------------------------------------------------------

/*! @brief  Message structure for TX LM config request per UL CA
...
typedef struct {                      
  ...
  boolean ul_rf_tune;      ///< True if we need to do RF Tune for this UL carrier.
  boolean txfe_config;   ///<TRUE if we need to configure DTR/load mkernel for this carrier
  uint8 ul_carrier_idx;
  lte_LL1_carrier_type_enum    dl_carrier_type;
} lte_LL1_ul_tx_lm_carr_config_struct;

// --------------------------------------------------------------------------
//
// TX LM config for UL CA
//
// --------------------------------------------------------------------------

/*! @brief  Message structure for TX LM config request for UL CA
...
typedef struct {                      
  ...
  uint8 num_carrier;/*Number of UL carriers*/
  lte_LL1_ul_tx_lm_carr_config_struct txlm_carr_cfg[LTE_LL1_UL_NUM_CARRIERS];
  boolean                        ul_rf_tune;
  boolean                        single_chain_ulca_enabled;  ///< For below two flags, FW will overwrite unconditionally.
  boolean                        inter_band_ulca_enabled;    
} lte_LL1_ul_tx_lm_config_ca_req_struct;
LTE_LL1_INTF_MSG( ul_tx_lm_config_ca_req );
...
typedef struct {                      
  ...
  uint8 ul_carrier_idx;///< Carrier Index to be used
  //Ping pong EIB addr for the carrier
  uint32 eib_base_addr[2]; ///< EIB base addr
...
// --------------------------------------------------------------------------
//
// TX LM config cnf for UL CA
//
// --------------------------------------------------------------------------

/*! @brief  Message structure for TX LM config confirm for UL CA
...
typedef struct {                      
  ...
  lte_LL1_cnf_header_struct cnf_hdr;
  uint8 ul_carrier_mask;///< Carriers Present.. Corresponds to the TXLM_CA_REQ sent
  //Ping pong EIB addr for the 2 carriers
  uint32 eib_base_addr[LTE_LL1_UL_NUM_CARRIERS][2]; ///< EIB base addr

} lte_LL1_ul_tx_lm_config_ca_cnf_struct;

LTE_LL1_INTF_MSG( ul_tx_lm_config_ca_cnf );
