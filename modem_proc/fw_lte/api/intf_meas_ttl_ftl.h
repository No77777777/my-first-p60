...
$Header: //components/rel/fw_lte.mpss/17.0.c4/api/intf_meas_ttl_ftl.h#3 $
...
/* Temperary workaround until system vector is updated */
#define SERVING_CELL_MEAS_API_WORKAROUND
...
typedef struct {
  ...
  uint8   threshold_offset_rx[LTE_LL1_MAX_NUM_UE_RX_ANT];///<  Linear factor by which threshold (per Rx) is offset. SNE taps within window 
...
/*! @brief  Request message for WB cell measurements

    @detail This message is sent every time a WB cell measurement 
...
typedef struct
{
  lte_earfcn_t  earfcn;   ///< Cell EARFCN for logging / debugging. Range: 0 ~ 65535

  uint16        nCell;    /* 1 to 4 (1 + LTE_MAX_NUM_CRS_IC_CELLS) */
  /* these cell_id must be within the 1+3 cells configured in crsic_lqice_req; if not, FW assert;
     serv cell may or may not be included */
  uint16        cell_id[ 1 + LTE_MAX_NUM_CRS_IC_CELLS ];

  lte_LL1_carrier_type_enum carrier_type; ///< Enum for dl carriers in  CA

} lte_LL1_meas_wb_cell_info_struct;
...
typedef struct {
  ...
  lte_LL1_meas_wb_cell_info_struct  carrier[LTE_LL1_DL_NUM_CARRIERS];

}lte_LL1_meas_scell_req_struct;


#if defined(SIM_PLATFORM) && defined(SERVING_CELL_MEAS_API_WORKAROUND)
...
}lte_LL1_meas_scell_req_struct_old;
...
typedef struct {
  ...
  uint32  cell_timing[LTE_LL1_MAX_NUM_UE_RX_ANT];             ///< Cell timing per Rx antenna as a modulo 10 ms offset in a radio frame.
                                      ///< Range: 0 ~ 307199

  uint16  cell_timing_sfn[LTE_LL1_MAX_NUM_UE_RX_ANT];         ///< SFN per Rx antenna corresponding to the cell_timing value.
                                      ///< Range: 0 ~ 1023

  int32   scell_timing_offset_rx[LTE_LL1_MAX_NUM_UE_RX_ANT];  ///< @todo !!!!!!!!!!!!!!!!!! REMOVE ONCE ML REMOVES DEPENDENCY ON THIS VARIABLE !!!!!!!!!!!!!!!!!!!


  int32   inst_rsrp_rx[LTE_LL1_MAX_NUM_UE_RX_ANT ];  ///<  Instantaneous RSRP value per Rx antenna. Combined across Tx antennas. In dBm.
                            ///<  <EM> Q-factor: Currently 32S23. </EM>
                            
  int32   inst_rsrp;        ///<  Instantaneous RSRP value combined across Tx-Rx pairs. In dBm.
                            ///<  <EM> Q-factor: Currently 32S23. </EM>

  int32   inst_rsrq_rx[LTE_LL1_MAX_NUM_UE_RX_ANT ];  ///<  Instantaneous RSRQ value per Rx antenna. Combined across Tx antennas. In dBm.
                            ///<  <EM> Q-factor: 32S24 </EM>
  int32   inst_rsrq_p_rx[LTE_LL1_MAX_NUM_UE_RX_ANT ];///<  Instantaneous RSRQ value per Rx antenna. Combined across Tx antennas. In dBm.                          
  ...
  int32   inst_rsrq_p;      ///<  Instantaneous proper RSRQ value combined across Tx-Rx pairs. In dBm. 
                            ///<  <EM> Q-factor: 32S24 </EM>                                    
  int32   inst_rssi_rx[LTE_LL1_MAX_NUM_UE_RX_ANT ];  ///<  RSSI value per Rx antenna. In dBm. Used for debugging.
                            ///<  <EM> Q-factor: 32S24 </EM>
  int32   inst_rssi_p_rx[LTE_LL1_MAX_NUM_UE_RX_ANT ];///<  proper RSSI value per Rx antenna. In dBm. Used for debugging.
                            ///<  <EM> Q-factor: 32S24 </EM>                            
  ...
  int32   inst_rssi_p;        ///<  RSSI value combined across Rx antennas. In dBm. Used for debugging.
                            ///<  <EM> Q-factor: 32U24 </EM>
  int32   dvga_rx_log[LTE_LL1_MAX_NUM_UE_RX_ANT ];     ///< DVGA gain per Rx antenna in log domain. Q-factor: 32S24

  int32   lna_gain_rx_log[LTE_LL1_MAX_NUM_UE_RX_ANT ]; ///< LNA gain per Rx antenna in log domain. Q-factor: 32S24

  /*! Additioanl fields for IOT */
  uint16  cqi_wb;             ///< Wideband CQI
  int16   res_freq_error;     ///< Residual frequency error

  uint32  filt_snr_rx[LTE_LL1_MAX_NUM_UE_RX_ANT ];     ///< Filtered SNR per Rx antenna. Linear.
                              ///< <EM> Q-factor: 32U16 </EM>
  
  int32	  filt_rssi_rx[LTE_LL1_MAX_NUM_UE_RX_ANT ];    ///< Filtered RSSI per Rx antenna. In dBm.
                              ///< <EM> Q-factor: 32S24 </EM>

  uint32  geometry_est[LTE_LL1_MAX_NUM_UE_RX_ANT ];    ///< Geometry estimation per Rx in linear scale.
                              ///< <EM> Q-factor: 32U16 </EM>
  ...
  int32  cinr_per_rx[LTE_LL1_MAX_NUM_UE_RX_ANT]; ///<CINR in dynamic antenna mapping algorithm @ ML1
  ...
  int16   start_offset;     ///< @todo Starting offset of thresholding window to be obtained from TTL. Set to CIR domain CP length for now.
  uint32  exp_se_rx[LTE_LL1_MAX_NUM_UE_RX_ANT];     ///< Exponent of SE (per Rx) used for fixed-point conversion
  uint16  threshold_rx[LTE_LL1_MAX_NUM_UE_RX_ANT];  ///< Measurement threshold (per Rx)

  uint32 num_valid_rx; ///< Gives number of Rx on which valid measurements are present.

  lte_LL1_ue_rx_ant_enum physical_rx_idx[LTE_LL1_MAX_NUM_UE_RX_ANT]; ///< This maps the logical Rx to actual physical Rx
                                                       ///< Ex. if mapping is [Rx3, Rx1, Rx0, Rx2], then 0th index will have
                                                       ///< meas results for Rx3 and this feild will tell that 0th index is for Rx3
                                                       ///< So basically it mapps th logical Rx to actual physical index

} lte_LL1_meas_wb_cell_result_struct;
...
typedef struct {
  ...
  lte_earfcn_t  earfcn;               ///< Cell EARFCN for logging / debugging.
                                      ///< Range: 0 ~ 65535
  uint16 nCell;
  uint16 hw_cell_idx[ 1 + LTE_MAX_NUM_CRS_IC_CELLS ]; // map to VPE HW cell idx (0..3)
  /* the data in cell[] may not be continuous, it maps to VPE HW cell idx;
     e.g. if ML1 request WB meas for 2 cells, FW will return hw_cell_idx[0,1] = 1,3;
     then the result is in cell[1] & cell[3]; cell[0] & cell[2] are not updated junk */
  lte_LL1_meas_wb_cell_result_struct cell[ 1 + LTE_MAX_NUM_CRS_IC_CELLS ];

} lte_LL1_meas_wb_result_struct;

/*! @brief  Confirmation message for WB cell measurements

    @detail The measurement results of the WB cell are reported after 
    completing processing for the ML serving cell request msg 
    lte_LL1_wb_meas_req_msg_struct. The values are reported per Rx antenna.
*/
typedef struct {    
  ...
  lte_LL1_meas_wb_result_struct  carrier[LTE_LL1_DL_NUM_CARRIERS];
...
typedef struct
{
  ...
  boolean ncell_cer_bad;  ///< FW will consider stronger rx (based on RSRP) and set this flag based on that rx.
                          ///< This flag will be set if the NCELL CER is bad based on RSRP threshold algo.
...
typedef enum
{
  LTE_LL1_MEAS_PATTERN_NONE = 0,
  LTE_LL1_MEAS_SERV_MEAS_PATN ,
  LTE_LL1_MEAS_NCELL_MEAS_PATN,
  LTE_LL1_MEAS_PATTREN_TYPE_COUNT
} lte_LL1_meas_pattern_type_enum;

typedef struct {
  uint8   update_mask; ///< each bit corresponding to one pattern,
                       ///< refer to lte_LL1_meas_pattern_type_enum.
                       ///< "1" means corresponding pattern is valid.
  ...
  lte_LL1_meas_subframe_pattern_struct   pcell_meas_pattern_param;
  lte_LL1_meas_subframe_pattern_struct   ncell_meas_pattern;
} lte_LL1_meas_pattern_update_struct;

typedef struct {

  lte_LL1_req_header_struct req_hdr;                      ///<  Common request parameters

  lte_LL1_meas_pattern_update_struct  pattern_cfg;

}lte_LL1_meas_pattern_update_req_struct;

LTE_LL1_INTF_MSG( meas_pattern_update_req );
...
typedef struct {
  ...
  uint8   update_mask;               ///< each bit corresponding to one pattern,
                                     ///< refer to lte_LL1_meas_pattern_type_enum.
                                     ///< "1" means corresponding pattern is updated.

  uint16  sfn;                       ///< System frame number for the subframe when pattern was updated in fw.
                                     ///< Range: 0 ~ 1023

  uint16  subfn;                     ///< Subframe number for the subframe when pattern was updated in fw.
                                     ///< Range: 0 ~ 1023
}lte_LL1_meas_pattern_update_cnf_struct;

LTE_LL1_INTF_MSG( meas_pattern_update_cnf );
