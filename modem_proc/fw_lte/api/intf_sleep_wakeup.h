...
$Header: //components/rel/fw_lte.mpss/17.0.c4/api/intf_sleep_wakeup.h#2 $
...
typedef struct
{
  ...
  boolean                 is_qdls;          ///< if Deep Light Sleep, boolean for ML1 to indicate Quick DLS
  boolean                 tcm_unload;       /// TRUE if TCM needs to be unloaded.
  lte_LL1_sys_hw_res_req_payload_t hw_res_req_params; ///< SCC invalidation, if needed in Msim, to be indicated in SLEEP request
} lte_LL1_sys_sleep_req_struct;

LTE_LL1_INTF_MSG( sys_sleep_req );

/*! @brief
    Idle mode 1 rx related information passed to ML1
*/
typedef struct
{
   /* bit 0 for LTE_LL1_UE_RX_ANT_0 and bit 1 for LTE_LL1_UE_RX_ANT_1 */
   uint8  rxagc_valid_bitmask;  
} lte_LL1_idle_one_rx_sleep_info_struct;
...
typedef struct { 
  ...
  lte_LL1_idle_one_rx_sleep_info_struct	idle_1rx_sleep_params;
