...
$Header: //components/rel/fw_lte.mpss/17.0.c4/api/lte_LL1_log_demback.h#1 $
...
---------------------------------------------------------------------------*/
#define LTE_LL1_LOG_DEMBACK_PHICH_DECODING_RESULTS_VERSION 101
...
typedef struct
{
  ...
  uint32 carrier_idx                      : 3;  ///< Carrier idx
  uint32 reserved0                        : 7; 
...
typedef struct
{
  ...
  uint32 alt_tbs_enabled                  : 1; /// bit, is alt_tbs configured
  uint32 reserved2                        : 9;
...
#define LTE_LL1_LOG_DEMBACK_PDSCH_DECODING_RESULTS_VERSION 32
#define LTE_LL1_LOG_DEMBACK_NUMBER_OF_STREAMS 2
#define LTE_LL1_LOG_DEMBACK_NUMBER_OF_CBLK    16
#define LTE_LL1_LOG_PDSCH_DEMBACK_NUMBER_OF_RECORDS 20
...
typedef struct ALIGN(4)
{
  ...
  uint32 llr_overrun                     : 1;  // set if overrun the LLR memory
 
  /* Word 1 */
  uint32 prog_max_hi                     :4;
  uint32 actual_hi                       :4;
  uint32 timeAvailable                   :16;
  uint32 unused2                         :8;
...
typedef struct ALIGN(4)
{
  uint32 transport_block_crc              : 1;  ///< 0: Fail
                                                ///< 1: Pass
  uint32 ndi                              : 2;  ///< NDI bit
  uint32 code_block_size_plus             : 13; ///< Unit: number of bits. Range: 0...8191
  uint32 num_code_block_plus              : 5;  ///< 1  to 16
  uint32 max_tdec_iter                    : 3;  ///< 8,7,6, max_iteration_num.. can remove
  ...
  uint32 last_pass_cb_idx                 : 5;

  /* Word 2 */
  uint32 usec_to_hi_q8                    :16;  // Conversion factor based off TDEC clock rate
  uint32 minTimeCB                        :8;   // Minimum time per CB in usecs
  uint32 tdecRateDiv16                    :8;  // Tdec clock Rate divided by 16  
...
typedef struct ALIGN(4)
{
  ...
  uint32 carrier_index                    : 4; ///< 0-PCC, 1-SCC 
  uint32 number_of_records                : 5;  ///< Range: 0..20
  lte_LL1_log_demback_pdsch_decoding_results_records_s lte_LL1_log_demback_pdsch_decoding_results_records[LTE_LL1_LOG_PDSCH_DEMBACK_NUMBER_OF_RECORDS];
