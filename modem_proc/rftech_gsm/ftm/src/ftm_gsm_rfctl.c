...
/*===========================================================================
  ...
  $Header: //components/rel/rftech_gsm.mpss/3.11/ftm/src/ftm_gsm_rfctl.c#3 $
  $DateTime: 2021/04/07 02:17:54 $ 
  $Author: mplcsds1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
04/05/21   aad     [CR2882992] Removing unused API FTM_GSM_SET_PDM
02/02/20   mpa     To support MiniDump Feature
09/17/18   mpa     RFVFS QSEQ crash Fix
02/22/18   mpa     [CR2192310] KW errors fix
02/01/18   par     Set PRx gain and burst command in QSEQ when DRx is not supported
02/01/18   par     Set band and channel for DRx in QSEQ test
12/12/16   kpa     Apply gain range and offset for DRx path
04/06/16   mz      Fix FreqAdjust for tx device
04/01/16   ld      Always get the latest and continuous IQ data for GSM CtoN estimation
01/06/15   zbz     Fix DRx IQ capture in sensitivity estimation
12/21/15   ld      Set arfcn in GL1 Rx burst metrics packets(0x506C/0x5A6C)
12/05/15   hk      Disable temperature/vbatt compensation during IIP2 calibration
11/18/15   zbz     DRx IQ capture changes
11/02/15   par     Fix QSEQ RFVFS failures
10/16/15   cpm     Add RFM dev remap in FTM exec
10/08/15   dpk     Fixing response packet for GSM FTM IQ capture
09/24/15   ck      Fix and Enable ftm gsm fbrx selftest measurement
09/09/15   tsr     Change the error fatal in TX stop to error messages
09/08/15   tsr     Stop Temp comp and vbatt comp during GSM RX Calibration 
                   using correct TX device 
08/07/15   ck      Fix KW error
...
07/29/15   tsr     Fix Idle Frame processing in FTM 
07/29/15   ec      Replace hardcoded Sub id with correct run-time value
07/17/15   tsr     Add Support for FTM Device manager
...
06/26/15   tsr     Implemented subscription based handling of FTM<->RF Tasks MSGR messages 
06/26/15   sg      Fix crash in FTM GSM deact sequence
05/29/15   cpm     raised ftm_gsm_get_frame_sweep_cal_running() to its meaning
06/10/15   tsr     Added FTM and Non-Signalling Support on a chain if TX device is not supported
05/29/15   ld      Update arfcn when getting RxGainOffset according to expected Rx power
04/06/15   tsr     Bug fix for obtaining DRX RSSI using FTM_GSM_GET_MEAS_PWR_DRX 
...
04/15/15   sg      Turn the Tx slot ON by default during FTM GSM init Tx
04/15/15   sc      Rely on chain vars for split Rx/Tx info throughout call-flow
04/15/15   sc      Remove PRx device mapping API and use device passed in
...
02/25/15   ec      Reset SAWless linearity override to Low Lin when FTM channel changes
01/23/14   ec      Use new debug NV mechanism for KV cal log enabling
...
// Variable that is used as Response packet for ANALYZE SAMPLES Command
static ftm_iq_capture_analyze_samples_rsp_pkt_type ftm_gsm_iq_capture_analyze_samples_rsp_pkt;
...
void ftm_gsm_rf_task_set_qpoet_mode_bias( rfm_device_enum_type rfm_dev,
                                     apt_mode_type mode,
                                     uint16 bias,
                                     boolean override );
...
ARFCN_T ftm_gsm_get_arfcn( ftm_gsm_chain_vars_s * chain_vars_ptr );
...
void ftm_gsm_populate_variables (ftm_gsm_rfctl_vars_s * vars)
{
  ...
  */
  ...
  vars->selftest_ready_to_issue_TxCmd = FALSE;
  vars->selftest_tx_mcs = 0;
  vars->selftest_tx_pwr_dbm = 2500;
#endif
...
*/
ftm_gsm_rfctl_vars_s * ftm_gsm_create_and_init_vars( rfm_device_enum_type rfm_device,
                                                     rfm_device_enum_type dispatch_device )
{
  if( rfm_device >= RFM_MAX_WAN_DEVICES )
  {
    ERR_FATAL("ftm_gsm_create_and_init_vars() invalid remapped_device %d", rfm_device, 0, 0);
    return NULL;
  }

  if( dispatch_device >= RFM_MAX_WAN_DEVICES )
  {
    ERR_FATAL("ftm_gsm_create_and_init_vars() invalid dispatch_device %d", dispatch_device, 0, 0);
    return NULL;
  }

  if( ftm_gsm_rfctl_vars[rfm_device] == NULL )
  {
    MSG_1( MSG_SSID_FTM, MSG_LEGACY_HIGH, "Initializing GSM FTM vars for device %d", rfm_device);

    ftm_gsm_rfctl_vars[rfm_device] = modem_mem_alloc( sizeof(ftm_gsm_rfctl_vars_s), MODEM_MEM_CLIENT_RFA );

    if( ftm_gsm_rfctl_vars[rfm_device] != NULL )
    {
      ftm_gsm_rfctl_vars[rfm_device]->rfm_dev = rfm_device;

      ftm_gsm_rfctl_vars[rfm_device]->dispatch_device = dispatch_device;

      ftm_gsm_populate_variables(ftm_gsm_rfctl_vars[rfm_device]);

      rfgsm_mdsp_cal_init_vars( &ftm_gsm_rfctl_vars[rfm_device]->mdsp_cal_vars );

      if (ftm_gsm_device_is_tx_supported(rfm_device))
      {
         RF_NULL_CHECK_RTN(rfgsm_core_dm_alloc_tx_cal_data_manager_mem( rfm_device ),"rflm_gsm_tx_cal_data_ptr is NULL!", FALSE);
      }
    }
    else
    {
      ERR_FATAL( "Mem alloc for FTM GSM RFCTL for device %d failed", rfm_device, 0, 0);
      return NULL;
    }
  }
  else
  {
    MSG_1( MSG_SSID_FTM, MSG_LEGACY_HIGH, "GSM FTM RFCTL variables already inited for device %d", rfm_device );
  }

  /* Everything went OK */
  return ftm_gsm_rfctl_vars[rfm_device];
}
...
*/
ftm_gsm_rfctl_vars_s * ftm_gsm_get_vars_ptr( rfm_device_enum_type remapped_device )
{
  ...
  if ( remapped_device >= RFM_MAX_WAN_DEVICES )
  {
    ERR_FATAL("ftm_gsm_get_vars_ptr() invalid remapped_device %d", remapped_device, 0, 0);
    return NULL;
  }

  return ftm_gsm_rfctl_vars[remapped_device];
}
...
boolean ftm_gsm_get_predist_override(rfm_device_enum_type rfm_dev)
{
  ...
  /* Return if we are in online mode */
  if ( ftm_mode != FTM_MODE )
  {
    return FALSE;
  }
  ...
  /* Due to Rx/Tx device split, need to get current ftm state for PRx device and not the passed in Tx device because ftm state is only stored for PRx */
  if(ftm_get_current_state(ftm_gsm_get_prx_device(gvars->dispatch_device, rfm_dev)) == FTM_STATE_GSM) /*Check if in FTM cal mode*/
  {
...
===========================================================================*/
void ftm_gsm_set_tx( rfm_device_enum_type tx_device,
                     boolean on_off)
{
  if (ftm_gsm_device_is_tx_supported(tx_device)) 
  {
    ...
    if ( gvars->ftm_gsm_state.ftm_tx_state == on_off)
    {
      if((gvars->stop_tx_triggered != TRUE) || (on_off == OFF))
      {
        MSG_2(MSG_SSID_FTM, MSG_LEGACY_ERROR,
              "ftm_gsm_set_tx() ftm_gsm_set_tx state is already : %d, pending stop_tx_triggered=%d",
              on_off, gvars->stop_tx_triggered);
        return;
      }
    
      MSG_2(MSG_SSID_FTM, MSG_LEGACY_MED,
            "ftm_gsm_set_tx() ftm_gsm_set_tx state is already : %d, looses pending stop_tx_triggered= %d",
            on_off, gvars->stop_tx_triggered);
    }
  ...
  else
  {
    MSG_1(MSG_SSID_FTM, MSG_LEGACY_MED,
          "ftm_gsm_set_tx state is already : %d, as TX is not supported",
          on_off);
  } /* !ftm_gsm_device_is_tx_supported */
                            ...
                            ftm_lna_range_type range )
{
  ...
  if( gvars == NULL )
  {
    ERR_FATAL("ftm_gsm_set_lna_range() ftm vars NULL for device %d", rfm_dev, 0, 0);
  }
...
void ftm_gsm_set_arfcn( ftm_gsm_chain_vars_s * chain_vars_ptr,
                        uint16 arfcn )
{
  ftm_gsm_rfctl_vars_s * tx_gvars = NULL;
  boolean tx_is_supported = FALSE;
  rfm_device_enum_type rx_device = chain_vars_ptr->prx_dev;
  ftm_gsm_rfctl_vars_s * rx_gvars = ftm_gsm_get_vars_ptr(rx_device);

  rfgsm_core_handle_type *rfgsm_core_handle_ptr = rfgsm_core_handle_get(rx_device);
  ...
  if (rfgsm_nv_check_debug_nv(rx_device, RFGSM_DEBUG_NV_SAWLESS_FORCE_MODE) == FALSE)
  {
  ...
  {
    if (rfgsm_nv_check_debug_nv(rx_device, RFGSM_DEBUG_NV_SAWLESS_MODE_FORCE_HIGH) == TRUE)
    {
  ...
  if ( arfcn != chain_vars_ptr->cmn_vars_ptr->ftm_channel)
  {
    /* make sure the channel is valid */
    if ( ftm_gsm_is_valid_arfcn((ftm_mode_id_type)rx_gvars->ftm_gsm_state.ftm_rf_mode, arfcn) )
    {
      chain_vars_ptr->cmn_vars_ptr->ftm_channel = arfcn;
    }
  }
  ...
  */
  if (rx_gvars->rxBurstOn)
  {
    /* Set the reset frame count flag */
    rx_gvars->ftm_gsm_rfctl_reset_transit_rx_frame_cnt = TRUE;
  }
...
void ftm_gsm_init_tx( rfm_device_enum_type rfm_dev )
{
  ...
  if( gvars == NULL )
  {
    ERR_FATAL("ftm_gsm_init_tx() gvars NULL for Tx device %d", rfm_dev, 0, 0);
    return;
  }
  ...
  {
     ERR_FATAL("ftm_gsm_init_tx() chain_vars_ptr NULL for dispatch device %d and chain %d", gvars->dispatch_device, gsm_curr_chain, 0 );
     return;
  }
  /* Assign ACCESS STRATUM from chain vars sub ID */
  as_id = gl1_hw_rf_map_rf_sub_id_to_as_id( chain_vars_ptr->sub_id );
  ...
  /* Set default slot 0 config as TRUE. This is needed for Tx power to be
     measured correctly when linear APIs are called back-to-back for two bands */
  gvars->ftm_slot_assign[0].on_off = TRUE;
                                    ...
                                    uint16* paindex )
{
  ...
  if( gvars == NULL )
  {
    ERR_FATAL("ftm_gsm_prepare_continuous_tx() gvars NULL for Tx device %d", rfm_dev, 0, 0);
    return;
  }
  ...
  if( chain_vars_ptr == NULL ) 
  {
     ERR_FATAL("ftm_gsm_prepare_continuous_tx() chain_vars_ptr NULL for dispatch device %d and chain %d", gvars->dispatch_device, gsm_curr_chain, 0 );
     return;
  }
  ...
  /*TUNE THE CHANNEL*/
   ARFCN_T  arfcn;
   arfcn = ftm_gsm_get_arfcn( chain_vars_ptr );
...
===========================================================================*/
void ftm_gsm_do_tx_burst_buffer_data(rfm_device_enum_type tx_device )
{
  ftm_gsm_rfctl_vars_s * tx_vars = ftm_gsm_get_vars_ptr(tx_device);
  ...
  tx_vars->tx_cont_on = OFF;

  ftm_gsm_tx_burst( tx_device );
...
void ftm_gsm_do_tx_cont_rnd( rfm_device_enum_type rfm_dev )
{
  ...
  if( gvars == NULL )
  {
    ERR_FATAL("ftm_gsm_do_tx_cont_rnd() gvars NULL for Tx device %d", rfm_dev, 0, 0);
    return;
  }
  ...
  if( chain_vars_ptr == NULL ) 
  {
     ERR_FATAL("ftm_gsm_do_tx_cont_rnd() chain_vars_ptr NULL for dispatch device %d and chain %d", gvars->dispatch_device, gsm_curr_chain, 0 );
     return;
  }
...
void ftm_gsm_do_tx_cont_tone( rfm_device_enum_type rfm_dev )
{
  ...
  if( gvars == NULL )
  {
    ERR_FATAL("ftm_gsm_do_tx_cont_tone() gvars NULL for Tx device %d", rfm_dev, 0, 0);
    return;
  }
  ...
  if( chain_vars_ptr == NULL ) 
  {
     ERR_FATAL("ftm_gsm_do_tx_cont_tone() chain_vars_ptr NULL for dispatch device %d and chain %d", gvars->dispatch_device, gsm_curr_chain, 0 );
     return;
  }
...
void ftm_gsm_do_tx_stop_cont(rfm_device_enum_type rfm_dev )
{
  ...
  {
    FTM_MSG_1( FTM_ERROR,
               "ftm_gsm_do_tx_stop_cont() gvars NULL for Tx device %d", 
               rfm_dev);
    return;
  }
  ...
  {
     FTM_MSG_2( FTM_ERROR,
                "ftm_gsm_do_tx_stop_cont() chain_vars_ptr NULL for dispatch device %d and chain %d", 
                gvars->dispatch_device, gsm_curr_chain);
     return;
  }
...
{
  ARFCN_T ftm_infor;  
  ftm_infor.band = chain_vars_ptr->cmn_vars_ptr->ftm_gl1_hw_gsm_band;
  ftm_infor.num  = chain_vars_ptr->cmn_vars_ptr->ftm_channel;
...
{
  uint8 rfm_dev, assigned_dev;
  uint16 device_count = 0;
  uint8 slot_index = 0;  
  uint8 band;
  ftm_gsm_rfctl_vars_s * rx_gvars;
  ftm_gsm_rfctl_vars_s * tx_gvars;
  ftm_gsm_chains_enum_type gsm_curr_chain = FTM_GSM_INVALID_CHAIN;
  ftm_gsm_chain_vars_s* chain_vars_ptr = NULL;  
  ...
  { 
    /* With the FTM Device Manager, device_id from tool should have already been remapped to PRx device
     * in gsm extended dispatch.
     */
    rfm_dev = config_header_ptr->device_id[device_count];
    assigned_dev = ftm_device_manager_get_assigned_device(rfm_dev);

    /* Assigned dev being invalid can happen say in cal when Tools already sends the right remapped device
    In which cases go ahead and use the existing device parameter itself */
    if (assigned_dev == RFM_INVALID_DEVICE)
    {
      MSG_1(MSG_SSID_FTM, MSG_LEGACY_ERROR,
            "ftm_gsm_frame_sweep_execute()::FDM device not assigned:"
            ":bypassing legacy device rfm_dev=%d",
            rfm_dev);
      assigned_dev = rfm_dev;
    }
    else
    {
      MSG_2(MSG_SSID_FTM, MSG_LEGACY_HIGH,"Frame sweep Mapping done::In dev=%d, Mapped device=%d", 
	    rfm_dev, assigned_dev);
    }
	
    gsm_curr_chain = ftm_gsm_get_chain_from_dispatch_device(assigned_dev);
    chain_vars_ptr = ftm_gsm_get_chain_vars_ptr( gsm_curr_chain );

    if (chain_vars_ptr == NULL || chain_vars_ptr->prx_dev == RFM_INVALID_DEVICE)
    {
      MSG_1(MSG_SSID_FTM, MSG_LEGACY_ERROR,
            "ftm_gsm_frame_sweep_execute():"
            ":failed to find a valid PRX device for rfm_dev=%d :"
            ":ignoring", assigned_dev);
      continue;
    }
    
    rx_gvars = ftm_gsm_get_vars_ptr(assigned_dev);
    tx_gvars = ftm_gsm_get_vars_ptr(chain_vars_ptr->associated_tx_dev);
    ...
    MSG_2(MSG_SSID_FTM, MSG_LEGACY_HIGH,"ftm_gsm_frame_sweep enabling on PRx device %d and Tx device %d", assigned_dev, chain_vars_ptr->associated_tx_dev);

    cmn_vars_ptr->frame_sweep_header_ptr = gsm_frame_sweep_config.buffer + sizeof(ftm_gsm_config_header_type);

    band = ((ftm_gsm_frame_header_type *)cmn_vars_ptr->frame_sweep_header_ptr)->band;
	
    /* set band from 1st frame payload*/	
    /* rfm_dev is now the same as dispatch device so ok to pass it to ftm_gsm_set_mode */
    ftm_gsm_set_mode( assigned_dev, (ftm_mode_id_type)band);

    if((config_header_ptr->num_device > 1) && (assigned_dev == RFM_DEVICE_2))
    {
    ...
    cmn_vars_ptr->ftm_gsm_idle_proc_flag = FALSE;	

    /*during frame_sweep, disable persistent flag*/
    tx_gvars->tx_frame_matrix_persistent = FALSE; 

    /* disable all slots which may be ON*/
    for (slot_index = 0; slot_index < FTM_MAX_GSM_SLOT_PER_FRAME; slot_index++)
    {
      tx_gvars->ftm_slot_assign[slot_index].on_off = FALSE;
      tx_gvars->ftm_slot_assign[slot_index].powdBm = 0xFFFF;
    }
  }
  ...
  cmn_vars_ptr->gsm_frame_sweep_enabled = TRUE;
...
void ftm_gsm_frame_sweep_handler(ftm_gsm_chain_vars_s * chain_vars_ptr)
{
  ...
  rfm_device_enum_type rxd_device;
  rfm_device_enum_type tx_device;
  rfgsm_core_handle_type *rx_core_handle_ptr;
  rfgsm_core_handle_type *rxd_core_handle_ptr;
  rfgsm_core_handle_type *tx_core_handle_ptr;
  rfgsm_enh_rx_params_type enh_rx_params;
  ftm_gsm_config_header_type *ftm_gsm_config_header;
  uint8 assigned_dev = RFM_INVALID_DEVICE;
  ...
  rx_device = chain_vars_ptr->prx_dev;
  rxd_device = chain_vars_ptr->associated_drx_dev;
  tx_device = chain_vars_ptr->associated_tx_dev;

  rx_gvars = ftm_gsm_get_vars_ptr(rx_device);
  tx_gvars = ftm_gsm_get_vars_ptr(tx_device); 
  cmn_vars_ptr = chain_vars_ptr->cmn_vars_ptr;

  if (rx_gvars == NULL)
  {
    MSG_1(MSG_SSID_FTM, MSG_LEGACY_ERROR,"NULL FTM handle for rx_device %d!", rx_device);
    return;
  }

  if (tx_gvars == NULL)
  {
    MSG_1(MSG_SSID_FTM, MSG_LEGACY_ERROR,"NULL FTM handle for tx_device %d!", tx_device);
    return;
  }

  if (cmn_vars_ptr == NULL)
  {
    MSG_1(MSG_SSID_FTM, MSG_LEGACY_ERROR,"NULL cmn_vars_ptr for dispatch device %d!", rx_device);
    return;
  }

  if (cmn_vars_ptr->gsm_frame_sweep_enabled == FALSE)
  {
    return;
  }

  rx_core_handle_ptr = rfgsm_core_handle_get(rx_device);
  rxd_core_handle_ptr = rfgsm_core_handle_get(rxd_device);  
  tx_core_handle_ptr = rfgsm_core_handle_get(tx_device);

  if (rx_core_handle_ptr == NULL)
  {
    MSG_1(MSG_SSID_FTM, MSG_LEGACY_ERROR,"NULL core handle for rx_device= %d !", rx_device);
    return;
  }
  if(rxd_core_handle_ptr == NULL)
  {
  	MSG_1(MSG_SSID_FTM, MSG_LEGACY_ERROR,"NULL core handle for rxd_device= %d !", rxd_device);
    return;
  }	
  if (tx_core_handle_ptr == NULL)
  {
    MSG_1(MSG_SSID_FTM, MSG_LEGACY_ERROR,"NULL core handle for tx_device= %d !", tx_device);
    return;
  }
  if (cmn_vars_ptr->frame_sweep_header_ptr == NULL)
  {
    MSG_1(MSG_SSID_FTM, MSG_LEGACY_ERROR,"NULL frame_sweep_header_ptr for dispatch device= %d !", rx_device);
    return;
  }
  
  header_ptr = (ftm_gsm_frame_header_type *)cmn_vars_ptr->frame_sweep_header_ptr;
  payload_ptr = (ftm_gsm_slot_payload_type *)(cmn_vars_ptr->frame_sweep_header_ptr + sizeof(ftm_gsm_frame_header_type));
  
  /* check if band change sequence is in progress*/
  if(cmn_vars_ptr->gsm_frame_sweep_paused_count > 0)
  {
    /* decrement band_change_delay_count*/
    cmn_vars_ptr->gsm_frame_sweep_paused_count--;

    /* is stop rx pending */
    if((int8)rx_gvars->stop_rx < 0)
    {
      rx_gvars->stop_rx++;
      if(rx_gvars->stop_rx == 0)
      {
        rx_gvars->rxBurstOn = FALSE;
        MSG_1(MSG_SSID_FTM, MSG_LEGACY_HIGH,"frame_sweep: RX triggered late burst off", 0);
      }
    }

    if (cmn_vars_ptr->gsm_frame_sweep_paused_count == 1)
    {
      ftm_gsm_frame_sweep_set_band( rx_device, (ftm_mode_id_type)header_ptr->band);
     
      MSG_3(MSG_SSID_FTM, MSG_LEGACY_HIGH,"frame_sweep: set band dev %d, ftm_cont_rx=%d ftm_cont_tx=%d",
                          rx_device, rx_gvars->ftm_cont_rx, tx_gvars->ftm_cont_tx);
	  return;
	}
	else if (cmn_vars_ptr->gsm_frame_sweep_paused_count == 0)
	{
      /*during frame_sweep, disable persistent flag*/
      tx_gvars->tx_frame_matrix_persistent = FALSE; 

      /*Enable Tx*/
      ftm_gsm_set_tx(tx_device, TRUE);
      
      /*Enable Rx*/
      ftm_rx_burst.slot_num = 0;
      ftm_rx_burst.ftm_rx_funct = FTM_RX_FUNCTION_SINGLE_BURST;
      ftm_gsm_rx_burst_command(rx_device, &ftm_rx_burst, num_frames_avg);

      MSG_2(MSG_SSID_FTM, MSG_LEGACY_HIGH,"frame_sweep: enabled Rx on dev %d and Tx on dev %d", rx_device, tx_device);
    }
    else
    {
      MSG_3(MSG_SSID_FTM, MSG_LEGACY_HIGH,"frame_sweep: paused count %d, ftm_cont_rx=%d ftm_cont_tx=%d",
                          cmn_vars_ptr->gsm_frame_sweep_paused_count, rx_gvars->ftm_cont_rx, tx_gvars->ftm_cont_tx);
      return;
    } 
  } // if(gvars->gsm_frame_sweep_paused_count > 0)

  /*if it is first frame tick, enable Rx and Tx now*/
  if (cmn_vars_ptr->frame_sweep_fn_count == 0)
  {
    /*Enable Rx*/
    ftm_rx_burst.slot_num = 0;
    ftm_rx_burst.ftm_rx_funct = FTM_RX_FUNCTION_SINGLE_BURST;
    ...
    tx_gvars->tx_brst.data_source = FTM_TX_DATA_SOURCE_PSDRND;
  }

  /*continue with frame sweep*/  
  if( (cmn_vars_ptr->frame_sweep_fn_count < ftm_gsm_config_header->num_frame) && 
      (cmn_vars_ptr->frame_sweep_fn_count < MAX_NUM_OF_FRAMES))
  {
    /*If band is changed, stop rx/tx, trigger band change sequence and return*/
    if (rx_gvars->ftm_gsm_state.ftm_rf_mode != header_ptr->band)
    {
      /*Set stop Rx flags towards the end of a block */
      int8 stop_remaining_bursts = (4 - tx_gvars->ftm_FNmod4) % 4 + 1;

      if(stop_remaining_bursts > FTM_GSM_MIN_RX_FRAME_CNT)
      {
        rx_gvars->stop_rx = (uint8) (FTM_GSM_MIN_RX_FRAME_CNT - stop_remaining_bursts);
      ...
      /*set stop Tx*/
      ftm_gsm_set_tx(tx_device, OFF);
 
      MSG_4(MSG_SSID_FTM, MSG_LEGACY_MED,"frame_sweep: stop Rx dev_id=%d Tx dev_id=%d, stop_rx=%d, ftm_FNmod4=%d",
            rx_device, tx_device, rx_gvars->stop_rx, tx_gvars->ftm_FNmod4);

      /* set paused_count to enable band change sequence from next frame*/
      cmn_vars_ptr->gsm_frame_sweep_paused_count = FRAME_SWEEP_BAND_CHANGE_DELAY;
    ...
    if (cmn_vars_ptr->ftm_channel != header_ptr->chan)
    {
      /* make sure the channel is valid */
      if (ftm_gsm_is_valid_arfcn((ftm_mode_id_type)rx_gvars->ftm_gsm_state.ftm_rf_mode, header_ptr->chan))
      {
        cmn_vars_ptr->ftm_channel = header_ptr->chan;
      }
    }
    ...
    for(slot_count = 0; slot_count < header_ptr->num_slot_payload; slot_count++)
    {
       ...
       * assigned_dev is invalid at calibration or for a 1 to 1 mapping
       * rx_device shall not be invalid, or we would crash anyway.
       */

      assigned_dev = ftm_device_manager_get_assigned_device(payload_ptr->device_id);
    
      if ((payload_ptr->device_id == rx_device) || (assigned_dev == rx_device))
      {
        ...
        case TX_PCL_MEAS_OP:
          ...
          RF_NULL_CHECK_RTN_V( tx_core_handle_ptr->rfgsm_tx_nv_tbl, "rfgsm_tx_nv_tbl is NULL!. Check RF Tx cal data" );
          RF_NULL_CHECK_RTN_V( tx_core_handle_ptr->rfgsm_tx_nv_tbl->tx_static_nv_data_ptr, "rfgsm_tx_nv_tbl->tx_static_nv_data_ptr is NULL!. Check RF Tx cal data" );
          tx_frame_matrix.power_dB = tx_core_handle_ptr->rfgsm_tx_nv_tbl->tx_static_nv_data_ptr->power_levels[payload_ptr->op_data.tx_pcl_meas_data.pcl];
          tx_frame_matrix.mcs      = payload_ptr->op_data.tx_pcl_meas_data.mcs;
          ftm_gsm_set_frame_matrix(tx_device, &tx_frame_matrix);
          break;
        ...
        case TX_RGI_MEAS_OP:
          ...
          RF_NULL_CHECK_RTN_V( tx_core_handle_ptr->rfgsm_tx_nv_tbl, "rfgsm_tx_nv_tbl is NULL!. Check RF Tx cal data" );
          RF_NULL_CHECK_RTN_V( tx_core_handle_ptr->rfgsm_tx_nv_tbl->tx_static_nv_data_ptr, "rfgsm_tx_nv_tbl->tx_static_nv_data_ptr is NULL!. Check RF Tx cal data" );
          tx_slot_config.smps_bias= tx_core_handle_ptr->rfgsm_tx_nv_tbl->tx_static_nv_data_ptr->smps_pdm_tbl.cal_smps_pdm_tbl[tx_slot_config.pa_range];
          ...
          ftm_gsm_set_tx_slot_config(tx_device, &tx_slot_config);
          break;
      
        case RX_RSSI_MEAS_OP:
          /* update slot num and LNA state information*/
          rx_gvars->rx_slot_num = payload_ptr->slot_num;

          /* get gain value for this LNA range, it will be used for logging in ftm_gsm_do_rx_burst_processing API*/
          /* PRX */
          rx_gvars->ftm_gsm_state.ftm_lna_range = (enum_type)payload_ptr->op_data.rx_rssi_meas_data.lna_state;
          rx_gvars->ftm_gain_range.valx16 = rfgsm_core_get_lna_gain( rx_device, cmn_vars_ptr->ftm_channel, rx_gvars->ftm_gsm_state.ftm_lna_range);
  
          /* DRX */
          rx_gvars->ftm_gsm_state.ftm_lna_range_drx = (enum_type)payload_ptr->op_data.rx_rssi_meas_data.lna_state;
          rx_gvars->ftm_gain_range_drx.valx16 = rfgsm_core_get_lna_gain( rxd_device, cmn_vars_ptr->ftm_channel, rx_gvars->ftm_gsm_state.ftm_lna_range_drx);
        ...
        case RX_DBM_MEAS_OP:
          ...
          rx_gvars->rx_slot_num = payload_ptr->slot_num;
          rx_core_handle_ptr->rf_rx_arfcn=chain_vars_ptr->cmn_vars_ptr->ftm_channel;
          rxd_core_handle_ptr->rf_rx_arfcn=chain_vars_ptr->cmn_vars_ptr->ftm_channel;
          rxd_core_handle_ptr->band=rx_core_handle_ptr->band;
          ...
          /* PRX */
          rfgsm_core_set_radio_gain( rx_device, 
                                     payload_ptr->op_data.rx_dbm_meas_data.expected_rx_pwr, 
                                     &rx_gvars->ftm_gain_range,
                                     enh_rx_params ); //gain range returned here is indexed 1-5
          rx_gvars->ftm_gsm_state.ftm_lna_range = rx_gvars->ftm_gain_range.range-1; //ftm lna & gl1 lna range interpretation is from 0-4
          /* DRX */
          rfgsm_core_set_radio_gain( rxd_device, 
                                     payload_ptr->op_data.rx_dbm_meas_data.expected_rx_pwr, 
                                     &rx_gvars->ftm_gain_range_drx,
                                     enh_rx_params ); //gain range returned here is indexed 1-5
          rx_gvars->ftm_gsm_state.ftm_lna_range_drx = rx_gvars->ftm_gain_range_drx.range-1; //ftm lna & gl1 lna range interpretation is from 0-4
          break;
        ...
        default:
              MSG_3(MSG_SSID_FTM, MSG_LEGACY_ERROR,
                    "frame_sweep::prx_dev=%d, device_id %d, op_id %d is invalid",
                    rx_device, payload_ptr->device_id, payload_ptr->op_id);
          break;
        }
      }
      else
      {
      	MSG_3(MSG_SSID_FTM, MSG_LEGACY_ERROR,
              "ftm_gsm_frame_sweep_handler::device_id=%d, op_id=%d ignored for prx_dev=%d",
              payload_ptr->device_id, payload_ptr->op_id, rx_device);
      }
    ...
    cmn_vars_ptr->frame_sweep_header_ptr = (uint8 *)payload_ptr;

    /* increase the frame count*/
    cmn_vars_ptr->frame_sweep_fn_count++;
  }
  else
  {
    /* disable frame sweep state machine*/
    cmn_vars_ptr->gsm_frame_sweep_enabled = FALSE;
    cmn_vars_ptr->frame_sweep_fn_count = 0;
    ...
    cmn_vars_ptr->ftm_gsm_idle_proc_flag = TRUE;

    MSG_2(MSG_SSID_FTM, MSG_LEGACY_HIGH,"frame_sweep: disabled on rx_device %d and tx_device %d", rx_device, tx_device);	 
  }
}
...
void ftm_gsm_do_rx_cmd_rx_burst_metrics( rfm_device_enum_type rfm_dev )
{
  ...
  if( gvars == NULL )
  {
    ERR_FATAL("ftm_gsm_do_rx_cmd_rx_burst_metrics() gvars NULL for Rx device %d", rfm_dev, 0, 0);
    return;
  }
  ...
  if( chain_vars_ptr == NULL ) 
  {
     ERR_FATAL("ftm_gsm_do_rx_cmd_rx_burst_metrics() chain_vars_ptr NULL for dispatch device %d and chain %d", gvars->dispatch_device, gsm_curr_chain, 0 );
     return;
  }
  ...
  */
  if (gvars->beginning_rx >= FTM_GSM_MIN_RX_FRAME_CNT)
  {
    if (gvars->multislot_rx_on )
    {
      for (i = 0; i <GERAN_API_MAX_GL1_DEFS_MAX_DL_MSGS; i++)
      {
        ...
        gvars->gsm_rx_multislot_metrics[i].arfcn = ftm_gsm_get_arfcn( chain_vars_ptr );
      ...
      } //^ if( (gvars->tags_rd_index <  FTM_MAX_GSM_BURST_TAGS) && (gvars->num_rx_slots < GERAN_API_MAX_GL1_DEFS_MAX_DL_MSGS) )
    ...
    else
    {
      ...
      if (gvars->tags_rd_index <FTM_MAX_GSM_BURST_TAGS)
      {
        ...
        gvars->gsm_rx_burst_metrics.arfcn = ftm_gsm_get_arfcn( chain_vars_ptr );
      ...
      ftm_gsm_rx_burst_update_rssi(rfm_dev); 
    } //^ else of if (gvars->multislot_rx_on )
  ...
  if (gvars->stop_rx >= FTM_GSM_MIN_RX_FRAME_CNT)
  {
    ...
    burst_info.arfcn               = ftm_gsm_get_arfcn( chain_vars_ptr );
    ...
    multislot_burst_info.arfcn               = ftm_gsm_get_arfcn( chain_vars_ptr );
  ...
  else
  {
   /* Wait for three additional frames to collect RSSI which were scheduled before stop rx was set to zero*/
    ...
    return;
  } //^ else of if (gvars->stop_rx >= FTM_GSM_MIN_RX_FRAME_CNT)
...
void ftm_gsm_do_rx_burst_processing( rfm_device_enum_type rfm_dev )
{
  ...
  if( gvars == NULL )
  {
    ERR_FATAL("ftm_gsm_do_rx_burst_processing() gvars NULL for Rx device %d", rfm_dev, 0, 0);
    return;
  }
  ...
  if( chain_vars_ptr == NULL ) 
  {
     ERR_FATAL("ftm_gsm_do_rx_burst_processing() chain_vars_ptr NULL for dispatch device %d and chain %d", gvars->dispatch_device, gsm_curr_chain, 0 );
     return;
  }
  ...
  if (gvars->stop_rx >= FTM_GSM_MIN_RX_FRAME_CNT)
  {
    if (gvars->tags_wr_index < FTM_MAX_GSM_BURST_TAGS )
    {
      ...
      gvars->gsm_frame_sweep_tags[gvars->tags_wr_index].frame_id        = chain_vars_ptr->cmn_vars_ptr->frame_sweep_fn_count - 1;
      gvars->gsm_frame_sweep_tags[gvars->tags_wr_index].slot_num        = gvars->rx_slot_num;
      gvars->gsm_frame_sweep_tags[gvars->tags_wr_index].chan            = chain_vars_ptr->cmn_vars_ptr->ftm_channel;
...
void ftm_gsm_do_rx_cmd_rx_cont( rfm_device_enum_type rfm_dev  )
{
  ...
  if( gvars == NULL )
  {
    ERR_FATAL("ftm_gsm_do_rx_cmd_rx_cont() gvars NULL for Rx device %d", rfm_dev, 0, 0);
    return;
  }
  ...
  if( chain_vars_ptr == NULL ) 
  {
     ERR_FATAL("ftm_gsm_do_rx_cmd_rx_cont() chain_vars_ptr NULL for dispatch device %d and chain %d", gvars->dispatch_device, gsm_curr_chain, 0 );
     return;
  }
  ...
  */
  if (gvars->beginning_rx == 0)
  {
    ...
    if (gvars->tags_wr_index < FTM_MAX_GSM_BURST_TAGS)
    {
      ...
      geran_test_mode_api_start_acquisition( ftm_gsm_get_arfcn( chain_vars_ptr ),
                                             rx_signal_strength,
                                             0,
                                             0,
                                             FALSE,
...
void ftm_gsm_write_data_block( rfm_device_enum_type rfm_dev )
{
  ...
  if( gvars == NULL )
  {
    ERR_FATAL("ftm_gsm_write_data_block() gvars NULL for Tx device %d", rfm_dev, 0, 0);
    return;
  }
  ...
  if( chain_vars_ptr == NULL ) 
  {
     ERR_FATAL("ftm_gsm_write_data_block() chain_vars_ptr NULL for dispatch device %d and chain %d", gvars->dispatch_device, gsm_curr_chain, 0 );
     return;
  }
...
===========================================================================*/
void ftm_get_burst_info( ftm_gsm_chain_vars_s * chain_vars_ptr,
                         gl1_hw_tx_pkt_burst_type *burst_infor)
{
  ftm_gsm_rfctl_vars_s * tx_gvars = ftm_gsm_get_vars_ptr(chain_vars_ptr->associated_tx_dev);

  burst_infor->arfcn.band = chain_vars_ptr->cmn_vars_ptr->ftm_gl1_hw_gsm_band;
  burst_infor->arfcn.num = chain_vars_ptr->cmn_vars_ptr->ftm_channel;
  burst_infor->tsc = tx_gvars->tx_brst.tsc_index;
  burst_infor->bsic = 0;
  burst_infor->timing_advance = 0;
  burst_infor->encode_flag = (tx_gvars->ftm_FNmod4 == 0 ? TRUE : FALSE);
  burst_infor->loopback_flag = FALSE;
#ifdef FEATURE_GERAN_RF_SELF_TEST
  burst_infor->selftest_ready_to_issue_txcmd = tx_gvars->selftest_ready_to_issue_TxCmd;
  if(burst_infor->selftest_ready_to_issue_txcmd) 
  {
    MSG(MSG_SSID_FTM, MSG_LEGACY_ERROR, "ck-debug: ready_to_issue is sent");
    tx_gvars->selftest_ready_to_issue_TxCmd = FALSE;
  }
#endif  
...
void ftm_gsm_tx_burst( rfm_device_enum_type rfm_dev )
{
  ...
  if( gvars == NULL )
  {
    ERR_FATAL("ftm_gsm_tx_burst() gvars NULL for Tx device %d", rfm_dev, 0, 0);
    return;
  }
  ...
  {
    ERR_FATAL("ftm_gsm_tx_burst() rfgsm_core_handle_ptr NULL for Tx device %d", rfm_dev, 0, 0);
    return;
  }
  ...
  if( chain_vars_ptr == NULL ) 
  {
     ERR_FATAL("ftm_gsm_tx_burst() chain_vars_ptr NULL for dispatch device %d and chain %d", gvars->dispatch_device, gsm_curr_chain, 0 );
     return;
  }
  ...
  ftm_get_burst_info( chain_vars_ptr, &ftm_burst_info);
...
#ifdef FEATURE_GERAN_RF_SELF_TEST
  //store pwr and mcs for gsm fbrx measurement 
  if(bm->on_off)
  {
    gvars->selftest_tx_pwr_dbm = bm->power_dB;
    gvars->selftest_tx_mcs = bm->mcs;
  }
#endif
  RFGSM_SLOT_LOCK(&rfgsm_slot_mutex);
...
FUNCTION  ftm_gsm_selftest_enable_ref_slot

DESCRIPTION
  This function turns on 2nd~4th slots for GSM fbrx selftest power measurements
  as reference power level
...
void ftm_gsm_selftest_enable_ref_slot(rfm_device_enum_type rfm_dev)
{
  ftm_gsm_rfctl_vars_s * gvars = ftm_gsm_get_vars_ptr(rfm_dev);
  ...
  if(rfgsm_core_handle_ptr == NULL)
  {    
    MSG(MSG_SSID_FTM, MSG_LEGACY_ERROR,"rfgsm_core_handle_ptr is NULL");  
    return;
  }
  
  RFGSM_SLOT_LOCK(&rfgsm_slot_mutex);  

  for(slot_num=1; slot_num<4; slot_num++)
  {    
    gvars->ftm_slot_assign[slot_num].on_off = TRUE;
    gvars->ftm_slot_assign[slot_num].mcs = (gl1_hw_channel_type)(gvars->selftest_tx_mcs +GL1_HW_PDTCHU_MCS1);
    gvars->ftm_slot_assign[slot_num].powdBm = gvars->selftest_tx_pwr_dbm;
  }
  ftm_gsm_set_slot_override_flag( rfm_dev, FALSE );
  	
  MSG_1(MSG_SSID_FTM, MSG_LEGACY_HIGH,"Turn on ref slots at power %d for GSM fbrx meas", gvars->ftm_slot_assign[1].powdBm );  

  RFGSM_SLOT_UNLOCK(&rfgsm_slot_mutex);  

  rfgsm_core_handle_ptr->selftest_flat_bb_gain= TRUE;
  ftm_clk_rex_wait(35);
  gvars->selftest_ready_to_issue_TxCmd= TRUE;
...
FUNCTION  ftm_gsm_selftest_disable_ref_slot

DESCRIPTION
  This function turns off 2nd~4th slots for GSM fbrx selftest power measurements
  as reference power level
...
void ftm_gsm_selftest_disable_ref_slot(rfm_device_enum_type rfm_dev)
{
  ...
  for(slot_num=1; slot_num<4; slot_num++)
  {    
    gvars->ftm_slot_assign[slot_num].on_off = FALSE;
  }
  ftm_gsm_set_slot_override_flag( rfm_dev, FALSE );
  	
  MSG(MSG_SSID_FTM, MSG_LEGACY_HIGH,"Turn off ref slots");  
                                 ...
                                 ftm_gsm_rxcal_parms_prx_drx_type * parm_ptr)
{
  ...
  if( gvars == NULL ) 
  {
     ERR_FATAL("ftm_gsm_do_rx_calibration() gvars NULL for remapped Rx device %d", rfm_dev, 0, 0 );
     return;
  }
  ...
  if( chain_vars_ptr == NULL ) 
  {
     ERR_FATAL("ftm_gsm_do_rx_calibration() chain_vars_ptr NULL for dispatch device %d and chain %d", gvars->dispatch_device, gsm_curr_chain, 0 );
     return;
  }
  ...
  /*Check if GSM TX is supported on the associated TX device for the given ftm chain*/
  ...
  if ( parm_ptr->toggle_rx_on && !gvars->rxBurstOn )
  {
    ...
    /* Turn off the 10sec timers for Vbatt and temp comp to 
       avoid interruption during RF cal */
    if (tx_is_supported) /*if TX is supported on the ftm chain*/
    {
      /* Stop Vbatt Tx pwr compensation. */
      rfgsm_core_vbatt_comp_stop(chain_vars_ptr->associated_tx_dev);

      /* Stop PA Temp pwr compensation. */
      rfgsm_core_temp_comp_stop(chain_vars_ptr->associated_tx_dev);
    }
  ...
  for ( i=0; i<8 ; i++ )
  {
    ...
    if ( parm_ptr->gain_range_bitmask & (0x1<<i) )
    {
      ...
      local_frame_count= sub_vars_ptr->ftm_frame_count ;

      /* wait num_avg frames +2 to get the RSSI averaged. */
      while ( sub_vars_ptr->ftm_frame_count <= (local_frame_count + parm_ptr->num_avgs + FTM_GSM_MIN_RX_FRAME_CNT ))
      {
        ftm_clk_rex_wait(1); /* this is to unblock other waiting threads. */    
      }
  ...
  if ( parm_ptr->toggle_rx_on && gvars->rxBurstOn )
  {
    ...
    if (tx_is_supported) /*if TX is supported on the ftm chain*/
    {
      /* Start Vbatt Tx pwr compensation. */
      rfgsm_core_vbatt_comp_start(chain_vars_ptr->associated_tx_dev);

      /* Start PA Temp pwr compensation. */
      if(chain_vars_ptr->sub_id == FTM_SUB_ID_1)
      {
        rfgsm_core_temp_comp_start(chain_vars_ptr->associated_tx_dev,RFLM_GSM_SUBSCRIPTION_2);
      }
      else
      {
        rfgsm_core_temp_comp_start(chain_vars_ptr->associated_tx_dev,RFLM_GSM_DEFAULT_SUBSCRIPTION);
      }
    }
...
boolean ftm_gsm_cal_kv_ave( rfm_device_enum_type rfm_dev, rfgsmlib_kv_code_type *kv_value )
{
  ...
  if( gvars == NULL )
  {
    ERR_FATAL("ftm_gsm_cal_kv_ave() gvars NULL for Tx device %d", rfm_dev, 0, 0);
    return FALSE;
  }
  ...
  if( chain_vars_ptr == NULL ) 
  {
     ERR_FATAL("ftm_gsm_cal_kv_ave() chain_vars_ptr NULL for dispatch device %d and chain %d", gvars->dispatch_device, gsm_curr_chain, 0 );
     return FALSE;
  }
...
FUNCTION ftm_gsm_set_qpoet_mode_bias

DESCRIPTION
   This function will force the qpoets settings as required. 
...
void ftm_gsm_set_qpoet_mode_bias( rfm_device_enum_type rfm_dev,
                             const ftm_gsm_qpoet_mode_bias_type *qpoet_params, ftm_rsp_pkt_type *const rsp_pkt_ptr)
{

  ftm_gsm_set_qpoet_mode_bias_resp_type* set_qpoet_resp = NULL;

  if(rsp_pkt_ptr != NULL)
  {
    set_qpoet_resp = (ftm_gsm_set_qpoet_mode_bias_resp_type*)((byte *)rsp_pkt_ptr->pkt_payload + sizeof(ftm_composite_cmd_header_type));
  }

  ftm_gsm_rf_task_set_qpoet_mode_bias( rfm_dev,qpoet_params->mode, qpoet_params->bias, qpoet_params->override);

  if(set_qpoet_resp != NULL)
  {
    set_qpoet_resp->status = TRUE; 
  }
                           ...
                           ftm_gsm_generic_2x16bit_u *chan)
{
  ...
  ARFCN_T                 chan_infor;
  rfgsm_nv_band_tx_data_type *rfgsm_tx_nv_tbl_ptr;
  ...
  chan_infor = ftm_gsm_get_arfcn( chain_vars_ptr );

  switch ( cmn_vars_ptr->ftm_gl1_hw_gsm_band)
  {
  ...
  default:
    MSG_1( MSG_SSID_FTM, MSG_LEGACY_ERROR, "unsupport band %d", cmn_vars_ptr->ftm_gl1_hw_gsm_band );
       ...
       (band != RFCOM_NUM_GSM_BANDS) ) 
  {
    ...
    ftm_gsm_set_arfcn( chain_vars_ptr, chan->generic_a_16bit_u );
    ...
    ftm_gsm_set_arfcn( chain_vars_ptr, chan->generic_b_16bit_u );
    ...
    rfgsm_tx_nv_tbl_ptr = (rfgsm_nv_band_tx_data_type *)rfgsm_nv_get_tbl_ptr( rfm_dev, band, RFGSM_NV_TX );

  	if (rfgsm_tx_nv_tbl_ptr == NULL)
  	{
  	  MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR,"ftm_gsm_do_tx_kv_cal NULL NV ptr!", 0);
  	  return;
  	}
	
    kv_cal_data_ptr = &(rfgsm_tx_nv_tbl_ptr->tx_static_nv_data_ptr->kv_cal);
    ...
    /* Restore FTM channel */
    ftm_gsm_set_arfcn( chain_vars_ptr, chan_infor.num );
                                         ...
                                         int16* parameters )
{
  ...
  /* Get the corresponding DRx device*/
  rfm_device_enum_type rxd_device = ftm_gsm_get_associated_rx_device( rfm_dev );
  ...
  /*Update Rx Channel*/
  rfgsm_core_handle_ptr->rf_rx_arfcn = chain_vars_ptr->cmn_vars_ptr->ftm_channel;

  /* Set range and gain offset for PRx*/
  rfgsm_core_set_radio_gain( rfm_dev, power_dbm16, &gvars->ftm_gain_range, enh_rx_params ); //gain range returned here is indexed 1-5
  
  if (rxd_device < RFM_MAX_WAN_DEVICES)
  {
    rfgsm_core_handle_ptr_rxd = rfgsm_core_handle_get(rxd_device);
    if (rfgsm_core_handle_ptr_rxd != NULL)
    {
      /* Update Rx Band/Channel for DRx */
      rfgsm_core_handle_ptr_rxd->rf_rx_arfcn = chain_vars_ptr->cmn_vars_ptr->ftm_channel;
      rfgsm_core_handle_ptr_rxd->band = rfgsm_core_handle_ptr->band;
    
      /* Set range and gain offset for DRx */
      rfgsm_core_set_radio_gain( rxd_device, power_dbm16, &gvars->ftm_gain_range_drx, enh_rx_params ); //gain range returned here is indexed 1-5
    }
    else /* Ideally we would end up here if DRx is not supported on a specific RFC design */
    {
      MSG_1(MSG_SSID_RF, MSG_LEGACY_HIGH,"rfgsm_core_handle_ptr of DRx is NULL! No DRx case: dev_id=%d", rxd_device);
    }
  }
  else
  {
    MSG_1(MSG_SSID_RF, MSG_LEGACY_HIGH,"Invalid DRx device passed %d", rxd_device);
  }
...
===========================================================================*/
int16 ftm_gsm_get_rssi_dbx16( rfm_device_enum_type rx_device)
{
  dBx16_T dBm_x16_value = 0;
  rf_gain_range_T gain;
  uint32 wait_counter = 0;
  uint32 rssi;
  boolean  device_is_prx = FALSE;
  rfm_device_enum_type prx_device;

  ftm_gsm_rfctl_vars_s * rx_vars_ptr = NULL;
  ftm_gsm_rfctl_vars_s * prx_vars_ptr = NULL;

  rf_gain_T * stored_gain_ptr = NULL;
  ...
  if( rx_vars_ptr == NULL )
  {
    ERR_FATAL("ftm_gsm_get_rssi_dbx16() rx_vars_ptr is NULL for rx device %d", rx_device, 0, 0);
    return((int16)dBm_x16_value);
  }

  device_is_prx = ftm_gsm_is_device_prx( rx_vars_ptr->dispatch_device,
                                         rx_device);

  /*Legacy only stores both PRX and DRX RSSI against Primary device */
  /*@Todo.. Implement PRX and DRX separately*/
  if (device_is_prx)  /*device is  prx*/
  {
    prx_device = rx_device;
    prx_vars_ptr = rx_vars_ptr;
  }
  else /*device is not prx*/
  {
    /*obtain PRX device from rfc*/
    prx_device = ftm_gsm_get_associated_rx_device( rx_device );

    prx_vars_ptr = ftm_gsm_get_vars_ptr( prx_device );
    if (prx_vars_ptr == NULL)
    {
      ERR_FATAL("ftm_gsm_get_rssi_dbx16() rx_vars_ptr is NULL for remapped prx device %d", prx_device , 0, 0);
      return((int16)dBm_x16_value);
    }
  }

  if( ( prx_vars_ptr->ftm_rx_funct == ftm_gsm_do_rx_cmd_rx_burst_metrics) && 
        prx_vars_ptr->ftm_gsm_rssi_no_frames_avg )
  {
    while(!ftm_gsm_get_ave_rssi_update_flag( prx_device ) && 
  ...
  else
  {
    if (device_is_prx) 
    {
      rssi = prx_vars_ptr->ftm_gsm_state.ftm_rssi;
	  stored_gain_ptr = &prx_vars_ptr->ftm_gain_range;

    }
    else
    {
      rssi = prx_vars_ptr->ftm_gsm_state.ftm_rssi_drx;
	  stored_gain_ptr = &prx_vars_ptr->ftm_gain_range_drx;
    }
    ...
    MSG_6(MSG_SSID_FTM,
          MSG_LEGACY_ERROR,
          "ftm_gsm_get_rssi_dbx16()|rx dev:%d| RSSI Value: raw %d, db16 %d, dbm %d, stored gain %d, stored gain_stage %d",
          rx_device,rssi,dBm_x16_value,dBm_x16_value/16, stored_gain_ptr->valx16, stored_gain_ptr->range);

    ftm_gsm_set_ave_rssi_update_flag( prx_device, FALSE) ;
  }
  return((int16)dBm_x16_value);
...
/*===========================================================================
FUNCTION ftm_gsm_compute_freq_err

DESCRIPTION
   This API is used for compututation of frequency error compensation value per rfm_dev. Used in ftm_gsm_set_freq_error
...
boolean ftm_gsm_compute_freq_err_rx(int32 freq_error_composite_hz,
	                                      ...
	                                      rfgsm_core_handle_type *rfgsm_core_handle_ptr)
{
  ...
  RF_NULL_CHECK_RTN(gvars, "gvars is NULL!", FALSE);
  ...
  if (rfgsm_core_get_gl1_freq_error_flag(rfgsm_core_handle_ptr) == FALSE)
  {
    ...
    {   
      ...
      {
        gvars->gl1_freq_error = rfgsm_core_get_freq_error_rx( rfgsm_core_handle_ptr ) - freq_error_composite_hz;
      }
      else
      {
        gvars->gl1_freq_error = rfgsm_core_get_freq_error_rx( rfgsm_core_handle_ptr );
      }
      // set the gl1_freq_error_saved flag to True
      gvars->gl1_freq_error_saved = TRUE;
    }

    if(freq_error_composite_hz != 0)
    {
      gvars->new_freq_error = gvars->gl1_freq_error + freq_error_composite_hz;  //pass freq_error from API on top of frequency error passed by GL1. add if freq_error observed -ve on the call box 
    }
    else
    {
    ...
    gvars->prev_freq_error = freq_error_composite_hz; // update previous frequency error
...
/*===========================================================================
FUNCTION ftm_gsm_compute_freq_err_tx

DESCRIPTION
   This API is used for compututation of frequency error compensation value per rfm_dev. Used in ftm_gsm_set_freq_error_tx
...
boolean ftm_gsm_compute_freq_err_tx(int32 freq_error_composite_hz,
	                                      uint8 enable_xo_freq_adjust,
	                                      ftm_gsm_rfctl_vars_s *gvars,
	                                      rfgsm_core_handle_type *rfgsm_core_handle_ptr)
{
  ...
  if (rfgsm_core_get_gl1_freq_error_flag(rfgsm_core_handle_ptr) == FALSE)
  {
    ...
    if ( gvars->gl1_freq_error_saved == FALSE)
    {
      if( enable_xo_freq_adjust )
      {
        gvars->gl1_freq_error = rfgsm_core_get_freq_error_tx( rfgsm_core_handle_ptr ) - freq_error_composite_hz;
      }
      else
      {
        gvars->gl1_freq_error = rfgsm_core_get_freq_error_tx( rfgsm_core_handle_ptr );
      }
      // set the gl1_freq_error_saved flag to True
      gvars->gl1_freq_error_saved = TRUE;
    }
...
===========================================================================*/
int32 ftm_gsm_set_freq_err( rfm_device_enum_type prx_dev,
                            int32 user_freq_adjust_ppb, 
                            uint32 center_frequency_hz, 
                            uint8 enable_xo_freq_adjust )
{
  int32 freq_error_composite_hz = 0;
  ...
  tx_device = ftm_gsm_get_associated_tx_device( prx_dev );
  ...
  if(enable_xo_freq_adjust)
  {
    ...
    freq_error_composite_hz = (-1)*(user_freq_adjust_ppb  + freq_error_xo_ppb);

    MSG_1(MSG_SSID_FTM, MSG_LEGACY_HIGH,"ftm_gsm_set_freq_error: tx_freq_error_composite_hz : %d",freq_error_composite_hz);    
    }
  else
  {
    freq_error_composite_hz = (-1)*(user_freq_adjust_ppb);
    MSG_1(MSG_SSID_FTM, MSG_LEGACY_HIGH,"ftm_wcdma_set_freq_adjust: tx_freq_error_composite_hz : %d",freq_error_composite_hz);    
  }


   
  /* Part 1: apply RX freq error  only if freq override is enabled*/
  gvars = ftm_gsm_get_vars_ptr( prx_dev);
  rfgsm_core_handle_ptr = rfgsm_core_handle_get(prx_dev);
	
  rfgsm_core_data.rf_task = RF_TASK_FTM;
  rfgsm_core_data.rfm_dev = prx_dev;
  rfgsm_core_data.sub_id = 0; //Default until support available if required
  rfgsm_core_data.as_id = SYS_MODEM_AS_ID_1; //Default until GL1 export get AS ID func
  rfgsm_core_data.triple_buffer_id = RFLM_GSM_BUFFER_ID_NOT_REQUIRED;
  rfgsm_core_data.rfgsm_core_handle_ptr = rfgsm_core_handle_ptr;
  if(ftm_gsm_compute_freq_err_rx(freq_error_composite_hz,enable_xo_freq_adjust,gvars,rfgsm_core_handle_ptr))
  {
    status = TRUE;
    rfgsm_core_update_rx_freq_error(&rfgsm_core_data,gvars->new_freq_error);
    }
    else
    {
    status = FALSE;
    MSG(MSG_SSID_FTM, MSG_LEGACY_ERROR,"Fail to set rx freq error");    
  }


  /* Part 2: apply TX freq error  only if freq override is enabled*/
  gvars = ftm_gsm_get_vars_ptr( tx_device);
  rfgsm_core_handle_ptr = rfgsm_core_handle_get(tx_device);
	
  rfgsm_core_data.rf_task = RF_TASK_FTM;
  rfgsm_core_data.rfm_dev = tx_device;
  rfgsm_core_data.sub_id = 0; //Default until support available if required
  rfgsm_core_data.as_id = SYS_MODEM_AS_ID_1; //Default until GL1 export get AS ID func
  rfgsm_core_data.triple_buffer_id = RFLM_GSM_BUFFER_ID_NOT_REQUIRED;
  rfgsm_core_data.rfgsm_core_handle_ptr = rfgsm_core_handle_ptr;

  if(ftm_gsm_compute_freq_err_tx(freq_error_composite_hz,enable_xo_freq_adjust,gvars,rfgsm_core_handle_ptr))
  {
    status = status && TRUE;
    rfgsm_core_update_tx_freq_error(&rfgsm_core_data,gvars->new_freq_error);
    }
    else
    {
    status = status && FALSE;
    MSG(MSG_SSID_FTM, MSG_LEGACY_ERROR,"Fail to set tx freq error");    
  }


  /* return tx freq error */
  if(status)
  {
...
{	
  ...
  /* Allocate buffer for DRx IQ capture */
  if (rfgsm_core_handle_ptr->iq_capture_drx != NULL)
  {
    MSG_1(MSG_SSID_FTM, MSG_LEGACY_ERROR, "GSM IQ capture buffer for dev %d already inited", rfgsm_core_get_drx_dev(rfm_dev));
    return;
  }

  rfgsm_core_handle_ptr->iq_capture_drx = 
    (rfgsm_core_iq_capture_type*)modem_mem_alloc(sizeof(rfgsm_core_iq_capture_type), MODEM_MEM_CLIENT_RFA);

  if (rfgsm_core_handle_ptr->iq_capture_drx == NULL)
  {
    MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, "Could not allocate GSM IQ buffer for dev %d", rfgsm_core_get_drx_dev(rfm_dev));
    return;
  }

  rfgsm_core_handle_ptr->iq_capture_drx->capture_type = FTM_IQ_CAPTURE_STOP;
  rfgsm_core_handle_ptr->iq_capture_drx->total_iq_samples_copied = 0;
  memset(rfgsm_core_handle_ptr->iq_capture_drx->rfgsm_iq_buffer, 0, RFGSM_MAX_IQ_BUFFER_LEN*sizeof(int16));

  MSG_1( MSG_SSID_FTM, MSG_LEGACY_HIGH, "GSM IQ capture buffer for device %d initialized", rfgsm_core_get_drx_dev(rfm_dev));
}
                                    ...
                                    ftm_iqcapture_type iq_cap_type )
{
  ...
  msgr_umid_type cmd_umid, rsp_umid;
  errno_enum_type result;
  uint32 bytes_recvd;
  uint8 sub_id = ftm_gsm_get_sub_id_from_device(rfm_dev);
  sys_modem_as_id_e_type as_id = ftm_gsm_ctl_map_sub_id_to_as_id(sub_id);

  switch (sub_id) 
  {
    case FTM_SUB_ID_0:
     cmd_umid = RFA_RF_GSM_FTM_CAPTURE_IQ_SUB1_CMD ;
     rsp_umid = RFA_RF_GSM_FTM_CAPTURE_IQ_SUB1_RSP ;
     break;

    case FTM_SUB_ID_1:
     cmd_umid = RFA_RF_GSM_FTM_CAPTURE_IQ_SUB2_CMD ;
     rsp_umid = RFA_RF_GSM_FTM_CAPTURE_IQ_SUB2_RSP ;
     break;

    default:
     cmd_umid = RFA_RF_GSM_FTM_CAPTURE_IQ_SUB1_CMD ;
     rsp_umid = RFA_RF_GSM_FTM_CAPTURE_IQ_SUB1_RSP ;
     ERR_FATAL( "ftm_gsm_start_stop_iq_capture()invalid Sub id:%d",
                sub_id, 0, 0 );
     break;
  }
  ...
  msgr_init_hdr( &rfa_rf_gsm_ftm_capture_iq_cmd.hdr, MSGR_RFA_RF_GSM_FTM, cmd_umid );
  ...
  (void)ftm_gsm_rf_msgr_send_debug( rfm_dev, cmd_umid, result);  
  ...
  ftm_gsm_rf_msgr_receive_debug( rsp_umid,
                                 result,
...
ftm_iq_capture_error_code_type ftm_gsm_iq_capture_acquire_samples(rfm_device_enum_type rfm_dev, ftm_iq_capture_acquire_samples_type *data)
{
  ...
  int16* ftm_gsm_iq_capture_ptr = NULL;
  int16* ftm_gsm_iq_capture_ptr_drx = NULL;
  ftm_gsm_iq_capture_info_type* ftm_gsm_iq_capture_info_ptr = NULL;
  ftm_gsm_iq_capture_info_type* ftm_gsm_iq_capture_info_ptr_drx = NULL;
  ftm_gsm_rfctl_vars_s * gvars = ftm_gsm_get_vars_ptr(rfm_dev);

  ftm_gsm_iq_capture_ptr = rfgsm_core_get_iq_buffer_ptr(rfm_dev);
  ftm_gsm_iq_capture_info_ptr = ftm_gsm_iq_capture_info_create_and_get(rfm_dev);
  
  int32 sample_copied = 0;
  int32 copy_size = 0;
  int16* start_ptr = NULL;
  ...
  sample_copied = (rfgsm_core_handle_get(rfm_dev))->iq_capture->total_iq_samples_copied;
  // I/Q pairs, so the number of samples should x2
  if (sample_copied >= 2 * FTM_GSM_IQ_MAX_SAMPLES)
  {
    copy_size = 2 * FTM_GSM_IQ_MAX_SAMPLES;
    memscpy(ftm_gsm_iq_capture_info_ptr->gsm_iq_buf_ptr,
	  FTM_IQ_SAMPLE_NUM_OF_BYTE * copy_size,
	  ftm_gsm_iq_capture_ptr + sample_copied - copy_size,
	  FTM_IQ_SAMPLE_NUM_OF_BYTE * copy_size);
  }
  else
  {
    // copy the first part from the bottom of circular buffer to gsm iq capture buffer
    copy_size = 2 * FTM_GSM_IQ_MAX_SAMPLES - sample_copied;
    start_ptr = ftm_gsm_iq_capture_ptr + RFGSM_MAX_IQ_BUFFER_LEN - copy_size;
    ...
    // copy the second part from the top of circular buffer to gsm iq capture buffer
    memscpy(ftm_gsm_iq_capture_info_ptr->gsm_iq_buf_ptr + copy_size,
	  FTM_IQ_SAMPLE_NUM_OF_BYTE * sample_copied,
  ...
  ftm_gsm_iq_capture_ptr_drx = rfgsm_core_get_iq_buffer_ptr_drx(rfm_dev);
  ftm_gsm_iq_capture_info_ptr_drx = ftm_gsm_iq_capture_info_create_and_get(rfgsm_core_get_drx_dev(rfm_dev));

  //Nullcheck ftm_gsm_iq_capture_info_ptr 
  if (ftm_gsm_iq_capture_info_ptr_drx == NULL) {
    MSG( MSG_SSID_FTM, MSG_LEGACY_ERROR, "Unable to allocate memory for ftm_gsm_iq_capture_info_ptr" ); 
    return FTM_IQ_GENERAL_FAILURE;
  }
  // Allocate memory of Maximum sample size 2 * 2* 8192 bytes.
  if(ftm_gsm_iq_capture_info_ptr_drx->gsm_iq_buf_ptr == NULL) 
  {
    ftm_gsm_iq_capture_info_ptr_drx->gsm_iq_buf_ptr=(int16*)ftm_malloc(2*FTM_IQ_SAMPLE_NUM_OF_BYTE*FTM_GSM_IQ_MAX_SAMPLES);
    // If memory is not allocated still, then declare it as failure.
    if(ftm_gsm_iq_capture_info_ptr_drx->gsm_iq_buf_ptr == NULL)
    {
     MSG( MSG_SSID_FTM, MSG_LEGACY_ERROR, "Unable to allocate memory for IQ buffer" ); 
    ...
    memset(ftm_gsm_iq_capture_info_ptr_drx->gsm_iq_buf_ptr,0,2*FTM_IQ_SAMPLE_NUM_OF_BYTE*FTM_GSM_IQ_MAX_SAMPLES);
  }
 
   // Setting the sample size
  ftm_gsm_iq_capture_info_ptr_drx->number_of_samples_req=data->sample_size;
  // Unset the data available at the begining of  the function. Set it only if we are able to acquire samples successfully.
  ftm_gsm_iq_capture_info_ptr_drx->data_available = FALSE;

  // First check if IQ capture is NULL
  if (ftm_gsm_iq_capture_ptr_drx == NULL)
  {
    MSG( MSG_SSID_FTM, MSG_LEGACY_ERROR, "rfgsm_iq_buffer is NULL" );	
    return FTM_IQ_GENERAL_FAILURE;
  }	
  ...
  sample_copied = (rfgsm_core_handle_get(rfm_dev))->iq_capture_drx->total_iq_samples_copied;
  // I/Q pairs, so the number of samples should x2
  if (sample_copied >= 2 * FTM_GSM_IQ_MAX_SAMPLES)
  {
    copy_size = 2 * FTM_GSM_IQ_MAX_SAMPLES;
    memscpy(ftm_gsm_iq_capture_info_ptr_drx->gsm_iq_buf_ptr,
	  FTM_IQ_SAMPLE_NUM_OF_BYTE * copy_size,
	  ftm_gsm_iq_capture_ptr_drx + sample_copied - copy_size,
	  FTM_IQ_SAMPLE_NUM_OF_BYTE * copy_size);
  }
  else
  {
    // copy the first part from the bottom of circular buffer to gsm iq capture buffer
    copy_size = 2 * FTM_GSM_IQ_MAX_SAMPLES - sample_copied;
    start_ptr = ftm_gsm_iq_capture_ptr_drx + RFGSM_MAX_IQ_BUFFER_LEN - copy_size;
    ...
    // copy the second part from the top of circular buffer to gsm iq capture buffer
    memscpy(ftm_gsm_iq_capture_info_ptr_drx->gsm_iq_buf_ptr + copy_size,
	  FTM_IQ_SAMPLE_NUM_OF_BYTE * sample_copied,
	  ftm_gsm_iq_capture_ptr_drx,
	  FTM_IQ_SAMPLE_NUM_OF_BYTE * sample_copied);
  }
  
  // Now set the data available flag.
  ftm_gsm_iq_capture_info_ptr_drx->data_available = TRUE;
...
SIDE EFFECTS
 None
=============================================================================*/
void ftm_gsm_iq_capture_get_samples(rfm_device_enum_type rfm_dev, ftm_iq_capture_get_samples_type *data, diagpkt_subsys_cmd_code_type ftm_chain,ftm_rsp_pkt_type *rsp_pkt)
{
  ...
  ftm_gsm_iq_capture_get_samples_rsp_pkt.subsys_cmd_code = ftm_chain;
...
/*!
  @brief
  Function to analyze IQ CAPTURE SAMPLES
  
  @param	  
  rfm_dev: rfm device
  
  @param	  
  cmd_ptr_pkt_for_analyze_samples: 
    pointer to the command packet of triggering IQ capture analysis
    
  @param	  
  rsp_pkt_ptr: pointer to the response packet
  
  @details
  This API is used to analyze the IQ data captured 
  This API is designed to be extendable, so that the new types of analysis can be added 
  without requiring an entirely new command.
  The user can specify the combination of analysis in the request packet and the response 
  packet structure can vary in structure, according to the requested analysis type.
  

=============================================================================*/
void ftm_gsm_iq_capture_analyze_samples
(
  rfm_device_enum_type prx_device,
  rfm_device_enum_type rfm_dev,
  ftm_iq_capture_analyze_samples_type *cmd_ptr_pkt_for_analyze_samples,
  diagpkt_subsys_cmd_code_type ftm_chain,
  ftm_rsp_pkt_type *rsp_pkt_ptr)
{
  ftm_iq_capture_error_code_type status = FTM_IQ_GENERAL_FAILURE;
  int32 analysis_result[FTM_IQ_CAPTURE_ANALYZE_SAMPLES_MAX_PAYLOAD_SIZE];
  ftm_iq_capture_get_config_type iq_capture_config = {FTM_IQ_SUCCESS, 0,0,0,0}; 
  ftm_gsm_iq_capture_info_type *iq_capture_info_ptr = NULL;
  
  ftm_iq_capture_acquire_samples_type cmd_acquire_samples = {FTM_IQ_SUCCESS, FTM_GSM_IQ_MAX_SAMPLES, 0};
  	
  ftm_common_sens_est_result_type sens_est_result = {0,0,0};
  
  uint32 analysis_id;
  boolean enable_c2n;
  boolean enable_estimated_sensitivity;

  uint16 *iq_buf_ptr = NULL;
  uint16 payload_size = 0;
  uint16 payload_cnt = 0;
  uint16 dl_pwr_lvl;

  ftm_gsm_rfctl_vars_s* gvars = ftm_gsm_get_vars_ptr(rfm_dev);

  if(cmd_ptr_pkt_for_analyze_samples==NULL)
  {
	return;
  }
  
  /* Initialize array before it is being used */
  memset(&analysis_result, 0, FTM_IQ_CAPTURE_ANALYZE_SAMPLES_MAX_PAYLOAD_SIZE*sizeof(int32));


  analysis_id = cmd_ptr_pkt_for_analyze_samples->analysis_id;
   
  dl_pwr_lvl = (int16)(cmd_ptr_pkt_for_analyze_samples->dl_pwr & 0xffff);

  /*  To get the Sampling Frequency we need to call this api    */
  ftm_gsm_iq_capture_get_config(&iq_capture_config);

  /* Acquire IQ samples  */
  cmd_acquire_samples.sample_size = iq_capture_config.max_sample_size;
  status = ftm_gsm_iq_capture_acquire_samples(prx_device, &cmd_acquire_samples);

  if(cmd_ptr_pkt_for_analyze_samples->sample_size > cmd_acquire_samples.sample_size)
  {
    status = FTM_IQ_GENERAL_FAILURE;
  }
  
  if (status == FTM_IQ_SUCCESS)
  {
     /* Get the pointer to IQ samples for post-processing*/
    iq_capture_info_ptr = ftm_gsm_iq_capture_info_create_and_get(rfm_dev);
    if( iq_capture_info_ptr && iq_capture_info_ptr->data_available)
    {
      iq_buf_ptr = (uint16 *)iq_capture_info_ptr->gsm_iq_buf_ptr;
	  enable_c2n = (analysis_id & FTM_IQ_CAPTURE_MEASURE_CTON)? TRUE: FALSE;
	  enable_estimated_sensitivity = (analysis_id & FTM_IQ_CAPTURE_MEASURE_ESTIMATED_SENSITIVITY)? TRUE: FALSE;
	  
      if (enable_c2n || enable_estimated_sensitivity)
      {
        ftm_common_analyze_iq_samples(&sens_est_result, iq_buf_ptr, 
                                          cmd_ptr_pkt_for_analyze_samples->sample_size,
                                          iq_capture_config.sampling_freq, FTM_RF_TECH_GSM,dl_pwr_lvl);
        
      
        MSG_3(MSG_SSID_FTM, MSG_LEGACY_HIGH, "iq_capture_analyze - "
                    "C/N = %d  Sensitivity =  %d Peak Frequncey = %d",
                  sens_est_result.cnr_db100,
                  sens_est_result.sens_db100,
                  sens_est_result.peak_freq);
      
      }
  	  status = FTM_IQ_SUCCESS;
    }
    else
    {
      status = FTM_IQ_IQ_SAMPLES_UNAVAILABLE;
    }
  }
  ...
  {
	 // Fill in the constant fields of response packet. Sample buffer will be filled after sanity check.
	 ftm_gsm_iq_capture_analyze_samples_rsp_pkt.cmd_code = 75;
	 ftm_gsm_iq_capture_analyze_samples_rsp_pkt.subsys_id = FTM;
	 /*if ( (rfm_dev == RFM_DEVICE_2) || (rfm_dev == RFM_DEVICE_3) )
	 {
		ftm_gsm_iq_capture_analyze_samples_rsp_pkt.subsys_cmd_code = FTM_GSM_C2_C;
	 }
	 else
	 {
		ftm_gsm_iq_capture_analyze_samples_rsp_pkt.subsys_cmd_code = FTM_GSM_C0_C;
	 }*/
	 ftm_gsm_iq_capture_analyze_samples_rsp_pkt.subsys_cmd_code	= ftm_chain;
	 ftm_gsm_iq_capture_analyze_samples_rsp_pkt.ftm_rf_cmd = FTM_IQ_CAPTURE_ANALYZE_SAMPLES;  
	 ftm_gsm_iq_capture_analyze_samples_rsp_pkt.ftm_error_code = (uint16)status;
	 ftm_gsm_iq_capture_analyze_samples_rsp_pkt.select_rx_chain = cmd_ptr_pkt_for_analyze_samples->select_rx_chain;
	 ftm_gsm_iq_capture_analyze_samples_rsp_pkt.analysis_id = cmd_ptr_pkt_for_analyze_samples->analysis_id;
	 ftm_gsm_iq_capture_analyze_samples_rsp_pkt.num_averages = cmd_ptr_pkt_for_analyze_samples->num_averages;
	 ftm_gsm_iq_capture_analyze_samples_rsp_pkt.sample_size = cmd_ptr_pkt_for_analyze_samples->sample_size;
	 ftm_gsm_iq_capture_analyze_samples_rsp_pkt.reserved = (uint32)0;

	 if (status == FTM_IQ_SUCCESS)
	 {
	   if (enable_estimated_sensitivity)
	   {
		 analysis_result[payload_cnt++] = sens_est_result.sens_db100;
		 payload_size += sizeof(uint32);
		 analysis_result[payload_cnt++] = sens_est_result.peak_freq;
		 payload_size += sizeof(uint32);
	   }
	   if (enable_c2n)
	   {
		 analysis_result[payload_cnt++] = sens_est_result.cnr_db100;
		 payload_size += sizeof(uint32);
	   }
	   memscpy(ftm_gsm_iq_capture_analyze_samples_rsp_pkt.analysis_result, payload_size,
	   		analysis_result, payload_size);

	 }
	 ftm_gsm_iq_capture_analyze_samples_rsp_pkt.payload_size = payload_size;	
	 
	 // Fill in the constant fields of response packet.Size of the packet will be based on the requested sample size.
	 rsp_pkt_ptr->cmd = FTM_RSP_DO_NEW_PKT;   
	 rsp_pkt_ptr->pkt_payload = &(ftm_gsm_iq_capture_analyze_samples_rsp_pkt);
	 ...
	 if((status == FTM_IQ_SUCCESS) && (payload_size <= FTM_IQ_CAPTURE_ANALYZE_SAMPLES_MAX_PAYLOAD_SIZE) )
	 {
		rsp_pkt_ptr->pkt_len += payload_size; // Header size + payload_size
	 }
                                   ...
                                   rfgsm_modulation_type mod_type )
{
  ...
  {
    case FTM_SUB_ID_0:
     cmd_umid = RFA_RF_GSM_FTM_SET_LINEAR_PA_RANGE_SUB1_CMD;
     rsp_umid = RFA_RF_GSM_FTM_SET_LINEAR_PA_RANGE_SUB1_RSP;
     break;

    case FTM_SUB_ID_1:
     cmd_umid = RFA_RF_GSM_FTM_SET_LINEAR_PA_RANGE_SUB2_CMD;
     rsp_umid = RFA_RF_GSM_FTM_SET_LINEAR_PA_RANGE_SUB2_RSP;
     break;

    default:
     cmd_umid = RFA_RF_GSM_FTM_SET_LINEAR_PA_RANGE_SUB1_CMD;
     rsp_umid = RFA_RF_GSM_FTM_SET_LINEAR_PA_RANGE_SUB1_RSP;
     ERR_FATAL( "ftm_gsm_rf_task_set_pa_range()invalid Sub id:%d",
                sub_id, 0, 0 );
     break;
  }
  ...
  /* Initialise msgr header */
  msgr_init_hdr( &rfa_rf_gsm_ftm_set_pa_range_cmd.hdr, MSGR_RFA_RF_GSM_FTM, cmd_umid );
                                      ...
                                      int8 index )
{
  ...
  {
    case FTM_SUB_ID_0:
     cmd_umid = RFA_RF_GSM_FTM_SET_TX_POW_DBM_SUB1_CMD;
     rsp_umid = RFA_RF_GSM_FTM_SET_TX_POW_DBM_SUB1_RSP;
     break;

    case FTM_SUB_ID_1:
     cmd_umid = RFA_RF_GSM_FTM_SET_TX_POW_DBM_SUB2_CMD;
     rsp_umid = RFA_RF_GSM_FTM_SET_TX_POW_DBM_SUB2_RSP;
     break;

    default:
     cmd_umid = RFA_RF_GSM_FTM_SET_TX_POW_DBM_SUB1_CMD;
     rsp_umid = RFA_RF_GSM_FTM_SET_TX_POW_DBM_SUB1_RSP;
     ERR_FATAL( "ftm_gsm_rf_task_set_tx_pow_dbm()invalid Sub id:%d",
                sub_id, 0, 0 );
     break;
  }
  ...
  /* Initialise msgr header */
  msgr_init_hdr( &rfa_rf_gsm_ftm_set_pow_dbm_cmd.hdr, MSGR_RFA_RF_GSM_FTM, cmd_umid );
  ...
  /* Blocking wait for set tx power RSP message from RF Task */
  result = msgr_receive(&ftm_rfgsm_msgr_client,
                                     ...
                                     uint16 input_delay_value )
{
  ...
  {
    case FTM_SUB_ID_0:
     cmd_umid = RFA_RF_GSM_FTM_SET_PATH_DELAY_SUB1_CMD;
     rsp_umid = RFA_RF_GSM_FTM_SET_PATH_DELAY_SUB1_RSP;
     break;

    case FTM_SUB_ID_1:
     cmd_umid = RFA_RF_GSM_FTM_SET_PATH_DELAY_SUB2_CMD;
     rsp_umid = RFA_RF_GSM_FTM_SET_PATH_DELAY_SUB2_RSP;
     break;

    default:
     cmd_umid = RFA_RF_GSM_FTM_SET_PATH_DELAY_SUB1_CMD;
     rsp_umid = RFA_RF_GSM_FTM_SET_PATH_DELAY_SUB1_RSP;
     ERR_FATAL( "ftm_gsm_rf_task_set_path_delay()invalid Sub id:%d",
                sub_id, 0, 0 );
     break;
  }
  ...
  /* Initialise msgr header */
  msgr_init_hdr( &rfa_rf_gsm_ftm_set_path_delay_cmd.hdr, MSGR_RFA_RF_GSM_FTM, cmd_umid );
                                     ...
                                     uint16 smps_bias )
{
  ...
  {
    case FTM_SUB_ID_0:
     cmd_umid = RFA_RF_GSM_FTM_SET_LINEAR_RGI_SUB1_CMD;
     rsp_umid = RFA_RF_GSM_FTM_SET_LINEAR_RGI_SUB1_RSP;
     break;

    case FTM_SUB_ID_1:
     cmd_umid = RFA_RF_GSM_FTM_SET_LINEAR_RGI_SUB2_CMD;
     rsp_umid = RFA_RF_GSM_FTM_SET_LINEAR_RGI_SUB2_RSP;
     break;

    default:
     cmd_umid = RFA_RF_GSM_FTM_SET_LINEAR_RGI_SUB1_CMD;
     rsp_umid = RFA_RF_GSM_FTM_SET_LINEAR_RGI_SUB1_RSP;
     ERR_FATAL( "ftm_gsm_rf_task_set_linear_rgi()invalid Sub id:%d",
                sub_id, 0, 0 );
     break;
  }
  ...
  /* Initialise msgr header */
  msgr_init_hdr( &rfa_rf_gsm_ftm_set_linear_rgi_cmd.hdr, MSGR_RFA_RF_GSM_FTM, cmd_umid );
...
/*===========================================================================
FUNCTION ftm_gsm_rf_task_set_qpoet_mode_bias

DESCRIPTION 
  Signal RF Task via MSGR to set qpoet mode(bypass/forced apt) and bias
...
void ftm_gsm_rf_task_set_qpoet_mode_bias( rfm_device_enum_type rfm_dev,
                                     ...
                                     boolean override )
{
  rfa_rf_gsm_ftm_set_qpoet_mode_bias_cmd_s  rfa_rf_gsm_ftm_set_qpoet_cmd;
  rfa_rf_gsm_ftm_set_qpoet_mode_bias_rsp_s  rfa_rf_gsm_ftm_set_qpoet_rsp;
  msgr_umid_type cmd_umid, rsp_umid;
  errno_enum_type result;
  uint32 bytes_recvd;
  uint8 sub_id = ftm_gsm_get_sub_id_from_device(rfm_dev);
  ...
  {
    case FTM_SUB_ID_0:
     cmd_umid = RFA_RF_GSM_FTM_SET_QPOET_MODE_BIAS_SUB1_CMD;
     rsp_umid = RFA_RF_GSM_FTM_SET_QPOET_MODE_BIAS_SUB1_RSP;
     break;

    case FTM_SUB_ID_1:
     cmd_umid = RFA_RF_GSM_FTM_SET_QPOET_MODE_BIAS_SUB2_CMD;
     rsp_umid = RFA_RF_GSM_FTM_SET_QPOET_MODE_BIAS_SUB2_RSP;
     break;
  ...
  /* Initialise msgr header */
  msgr_init_hdr( &rfa_rf_gsm_ftm_set_qpoet_cmd.hdr, MSGR_RFA_RF_GSM_FTM, cmd_umid );

  /* Populate payload parameters */
  rfa_rf_gsm_ftm_set_qpoet_cmd.rfm_dev                 = rfm_dev;
  rfa_rf_gsm_ftm_set_qpoet_cmd.qpoet_params.mode       = mode;
  rfa_rf_gsm_ftm_set_qpoet_cmd.qpoet_params.bias       = bias;
  rfa_rf_gsm_ftm_set_qpoet_cmd.qpoet_params.override   = override;


  /* Send msgr message to RF Task */
  result = msgr_send(&rfa_rf_gsm_ftm_set_qpoet_cmd.hdr, sizeof(rfa_rf_gsm_ftm_set_qpoet_mode_bias_cmd_s));

  /* Sanity check and print debug for msgr_send */
  ftm_gsm_rf_msgr_send_debug( rfm_dev, cmd_umid, result);  

  /* Blocking wait for set Linear RGI RSP message from RF Task */
  result = msgr_receive(&ftm_rfgsm_msgr_client,
                                (uint8 *)&rfa_rf_gsm_ftm_set_qpoet_rsp,
                                (uint32)sizeof(rfa_rf_gsm_ftm_set_qpoet_mode_bias_rsp_s),
                                &bytes_recvd);
  ...
  ftm_gsm_rf_msgr_receive_debug( rsp_umid,
                                 result,
                                 rfa_rf_gsm_ftm_set_qpoet_rsp.hdr.id,
                                 rfa_rf_gsm_ftm_set_qpoet_rsp.cmd_result );
                                             ...
                                             boolean flag )
{
  ...
  {
    case FTM_SUB_ID_0:
     cmd_umid = RFA_RF_GSM_FTM_SET_SLOT_OVERRIDE_FLAG_SUB1_CMD;
     rsp_umid = RFA_RF_GSM_FTM_SET_SLOT_OVERRIDE_FLAG_SUB1_RSP;
     break;

    case FTM_SUB_ID_1:
     cmd_umid = RFA_RF_GSM_FTM_SET_SLOT_OVERRIDE_FLAG_SUB2_CMD;
     rsp_umid = RFA_RF_GSM_FTM_SET_SLOT_OVERRIDE_FLAG_SUB2_RSP;
     break;

    default:
     cmd_umid = RFA_RF_GSM_FTM_SET_SLOT_OVERRIDE_FLAG_SUB1_CMD;
     rsp_umid = RFA_RF_GSM_FTM_SET_SLOT_OVERRIDE_FLAG_SUB1_RSP;
     ERR_FATAL( "ftm_gsm_rf_task_set_slot_override_flag()invalid Sub id:%d",
                sub_id, 0, 0 );
     break;
  }
  ...
  /* Initialise msgr header */
  msgr_init_hdr( &rfa_rf_gsm_ftm_set_slot_override_flag_cmd.hdr, MSGR_RFA_RF_GSM_FTM, cmd_umid );
...
===========================================================================*/
rfgsm_ftm_dispatch_outcome_t ftm_gsm_rf_msgr_send_debug( rfm_device_enum_type rfm_dev,
                                                         msgr_umid_type umid, 
                                                         errno_enum_type result )
{
  rfgsm_ftm_dispatch_outcome_t cmd_rsp = RFGSM_FTM_DISPATCH_SUCCESSFUL;
  ...
  else
  {
    ...
    return cmd_rsp = RFGSM_FTM_DISPATCH_UNSUCCESSFUL;
  }
}
                                        ...
                                        uint8* tuner_id_ptr)
{
  ...
  {
    case FTM_SUB_ID_0:
     cmd_umid = RFA_RF_GSM_FTM_TUNER_TUNE_CODE_OVERRIDE_SUB1_CMD;
     rsp_umid = RFA_RF_GSM_FTM_TUNER_TUNE_CODE_OVERRIDE_SUB1_RSP;
     break;

    case FTM_SUB_ID_1:
     cmd_umid = RFA_RF_GSM_FTM_TUNER_TUNE_CODE_OVERRIDE_SUB2_CMD;
     rsp_umid = RFA_RF_GSM_FTM_TUNER_TUNE_CODE_OVERRIDE_SUB2_RSP;
     break;

    default:
     cmd_umid = RFA_RF_GSM_FTM_TUNER_TUNE_CODE_OVERRIDE_SUB1_CMD;
     rsp_umid = RFA_RF_GSM_FTM_TUNER_TUNE_CODE_OVERRIDE_SUB1_RSP;
     ERR_FATAL( "ftm_gsm_rf_task_tuner_override()invalid Sub id:%d",
                sub_id, 0, 0 );
     break;
  }
  ...
  /* Initialise msgr header */
  msgr_init_hdr( &rfa_rf_gsm_ftm_tuner_override_cmd.hdr, MSGR_RFA_RF_GSM_FTM, cmd_umid );
...
boolean ftm_gsm_get_frame_sweep_cal_running(rfm_device_enum_type rfm_dev)
{
  ...
  return (ftm_gsm_frame_sweep_running && (gvars->ftm_gsm_cal_flag  || rfm_get_calibration_state()));
}
...
boolean ftm_gsm_set_ip2_cal_params_override ( rfm_device_enum_type rfm_dev, ftm_pkt_type* cmd_ptr, ftm_rsp_pkt_type* rsp_pkt_ptr  )
{
  ...
  uint32 start_time = geran_test_mode_api_get_qsym_count(as_id);
  uint32 stop_time = 0;
  ...
  if (!ip2_cal_config->use_default_params)
  {
    ...
    /*get the cmd and rsp umids based on ftm sub id*/
    switch (sub_id) 
    {
      case FTM_SUB_ID_0:
       cmd_umid = RFA_RF_GSM_FTM_SET_IP2_CAL_PARAMS_SUB1_CMD ;
       rsp_umid = RFA_RF_GSM_FTM_SET_IP2_CAL_PARAMS_SUB1_RSP ;
       break;

      case FTM_SUB_ID_1:
       cmd_umid = RFA_RF_GSM_FTM_SET_IP2_CAL_PARAMS_SUB2_CMD ;
       rsp_umid = RFA_RF_GSM_FTM_SET_IP2_CAL_PARAMS_SUB2_RSP ;
       break;

      default:
       cmd_umid = RFA_RF_GSM_FTM_SET_IP2_CAL_PARAMS_SUB1_CMD ;
       rsp_umid = RFA_RF_GSM_FTM_SET_IP2_CAL_PARAMS_SUB1_RSP ;
       ERR_FATAL( "ftm_gsm_set_ip2_cal_params_override()invalid Sub id:%d",
                  sub_id, 0, 0 );
       break;
    }
    ...
    /* Initialise msgr header */
    msgr_init_hdr( &rfa_rf_gsm_set_ip2_cal_params_cmd.hdr, MSGR_RFA_RF_GSM_FTM, cmd_umid );
 ...
 stop_time = geran_test_mode_api_get_qsym_count(as_id);
...
void ftm_gsm_do_ip2_cal(rfm_device_enum_type rfm_dev)
{
	...
	if( gvars == NULL )
	{
	  ERR_FATAL("ftm_gsm_do_ip2_cal() gvars NULL for Tx device %d", rfm_dev, 0, 0);
	  return;
	}
	...
	if( chain_vars_ptr == NULL ) 
	{
	   ERR_FATAL("ftm_gsm_do_ip2_cal() chain_vars_ptr NULL for dispatch device %d and chain %d", gvars->dispatch_device, gsm_curr_chain, 0 );
	   return;
	}
    ...
    else
    {
      ...
      if( (vars_ptr->ip2_cal_data.start_frame_ctr % num_frames_single_iter) == 1 )
      {
       ...
       geran_test_mode_api_do_ip2cal( vars_ptr->ip2_cal_data.num_cal_steps,vars_ptr->ip2_cal_data.step_duration_qs,as_id );
...
boolean ftm_gsm_set_ip2_cal_params_default(rfm_device_enum_type rfm_dev, uint8 rfcom_band)
{
  ...
  msgr_umid_type cmd_umid, rsp_umid;
  ftm_gsm_rfctl_vars_s * gvars = NULL;
  uint8 sub_id = FTM_SUB_ID_MAX;
  ftm_rfstate_enum_type ftm_rf_state = ftm_get_current_state(rfm_dev);
  ...
  /*get the current ftm sub id*/
  sub_id = ftm_gsm_get_sub_id_from_device(rfm_dev);

  /*get the cmd and rsp umids based on ftm sub id*/
  switch (sub_id) 
  {
    case FTM_SUB_ID_0:
     cmd_umid = RFA_RF_GSM_FTM_GET_IP2_CAL_PARAMS_SUB1_CMD ;
     rsp_umid = RFA_RF_GSM_FTM_GET_IP2_CAL_PARAMS_SUB1_RSP ;
     break;

    case FTM_SUB_ID_1:
     cmd_umid = RFA_RF_GSM_FTM_GET_IP2_CAL_PARAMS_SUB2_CMD ;
     rsp_umid = RFA_RF_GSM_FTM_GET_IP2_CAL_PARAMS_SUB2_RSP ;
     break;

    default:
     cmd_umid = RFA_RF_GSM_FTM_GET_IP2_CAL_PARAMS_SUB1_CMD ;
     rsp_umid = RFA_RF_GSM_FTM_GET_IP2_CAL_PARAMS_SUB1_RSP ;
     ERR_FATAL( "ftm_gsm_set_ip2_cal_params_default()invalid Sub id:%d",
                sub_id, 0, 0 );
     break;
  }
  ...
  /* Initialise msgr header */
  msgr_init_hdr( &rfa_rf_gsm_get_ip2_cal_params_cmd.hdr, MSGR_RFA_RF_GSM_FTM, cmd_umid );
  ...
  rfa_rf_gsm_get_ip2_cal_params_cmd.sub_id               = sub_id;
                                    ...
                                    boolean cal_end)
{
  ...
  {
    case FTM_SUB_ID_0:
     cmd_umid = RFA_RF_GSM_FTM_GET_IP2_CAL_RESULTS_SUB1_CMD ;
     rsp_umid = RFA_RF_GSM_FTM_GET_IP2_CAL_RESULTS_SUB1_RSP ;
     break;

    case FTM_SUB_ID_1:
     cmd_umid = RFA_RF_GSM_FTM_GET_IP2_CAL_RESULTS_SUB2_CMD ;
     rsp_umid = RFA_RF_GSM_FTM_GET_IP2_CAL_RESULTS_SUB2_RSP ;
     break;

    default:
     cmd_umid = RFA_RF_GSM_FTM_GET_IP2_CAL_RESULTS_SUB1_CMD ;
     rsp_umid = RFA_RF_GSM_FTM_GET_IP2_CAL_RESULTS_SUB1_RSP ;
     ERR_FATAL( "ftm_gsm_get_ip2_cal_results()invalid Sub id:%d",
                sub_id, 0, 0 );
     break;
  }
  ...
  /* Initialise msgr header */
  msgr_init_hdr( &rfa_rf_gsm_get_ip2_cal_results_cmd.hdr, MSGR_RFA_RF_GSM_FTM, cmd_umid );

  /*Populate data to send*/
  rfa_rf_gsm_get_ip2_cal_results_cmd.rfm_dev              = rfm_dev;  
  rfa_rf_gsm_get_ip2_cal_results_cmd.sub_id               = sub_id;
                                      ...
                                      ftm_gsm_sawless_lin_type* lin_params )
{
  ...
  {
    case FTM_SUB_ID_0:
     cmd_umid = RFA_RF_GSM_FTM_SET_SAWLESS_LIN_MODE_SUB1_CMD ;
     rsp_umid = RFA_RF_GSM_FTM_SET_SAWLESS_LIN_MODE_SUB1_RSP ;
     break;

    case FTM_SUB_ID_1:
     cmd_umid = RFA_RF_GSM_FTM_SET_SAWLESS_LIN_MODE_SUB2_CMD ;
     rsp_umid = RFA_RF_GSM_FTM_SET_SAWLESS_LIN_MODE_SUB2_RSP ;
     break;

    default:
     cmd_umid = RFA_RF_GSM_FTM_SET_SAWLESS_LIN_MODE_SUB1_CMD ;
     rsp_umid = RFA_RF_GSM_FTM_SET_SAWLESS_LIN_MODE_SUB1_RSP ;
     ERR_FATAL( "ftm_gsm_sawless_lin_override()invalid Sub id:%d",
                sub_id, 0, 0 );
     break;
  }
  ...
  /* Initialise msgr header */
  msgr_init_hdr( &rfa_rf_gsm_set_sawless_lin_mode_cmd.hdr, MSGR_RFA_RF_GSM_FTM, cmd_umid );
                                      ...
                                      ftm_gsm_sawless_lin_type* lin_params )
{
  ...
  {
    case FTM_SUB_ID_0:
     cmd_umid = RFA_RF_GSM_FTM_GET_SAWLESS_LIN_MODE_SUB1_CMD ;
     rsp_umid = RFA_RF_GSM_FTM_GET_SAWLESS_LIN_MODE_SUB1_RSP ;
     break;

    case FTM_SUB_ID_1:
     cmd_umid = RFA_RF_GSM_FTM_GET_SAWLESS_LIN_MODE_SUB2_CMD ;
     rsp_umid = RFA_RF_GSM_FTM_GET_SAWLESS_LIN_MODE_SUB2_RSP ;
     break;

    default:
     cmd_umid = RFA_RF_GSM_FTM_GET_SAWLESS_LIN_MODE_SUB1_CMD ;
     rsp_umid = RFA_RF_GSM_FTM_GET_SAWLESS_LIN_MODE_SUB1_RSP ;
     ERR_FATAL( "ftm_gsm_get_sawless_lin_mode()invalid Sub id:%d",
                sub_id, 0, 0 );
     break;
  }
  /* Get handle to the FTM gsm mutex */
  ...
  /* Initialise msgr header */
  msgr_init_hdr( &rfa_rf_gsm_get_sawless_lin_mode_cmd.hdr, MSGR_RFA_RF_GSM_FTM, cmd_umid );
...
/*----------------------------------------------------------------------------*/
/*!
   @brief - ftm_gsm_get_rfctrl_vars_address

   @details - Returns the address of ftm_gsm_rfctl_vars

   @param - None
   
   @retval 
   returns the address of ftm_gsm_rfctl_vars
*/
QSH_MDUMP_FN_ATTR ftm_gsm_rfctl_vars_s ** ftm_gsm_get_rfctrl_vars_address ( void )
{
  return &ftm_gsm_rfctl_vars[0];
}

QSH_MDUMP_FN_ATTR ftm_gsm_rfctl_vars_s * ftm_gsm_get_rfctrl_vars( rfm_device_enum_type rfm_dev )
{
