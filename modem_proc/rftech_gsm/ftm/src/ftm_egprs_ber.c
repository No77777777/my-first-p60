...
/*===========================================================================
  ...
  $Header: //components/rel/rftech_gsm.mpss/3.11/ftm/src/ftm_egprs_ber.c#1 $ 
...
07/07/15   tsr     Removed check for remapped device as device could be preferred or user updated 
...
static void ftm_egprs_ber_send_msg_to_gpl1( sys_modem_as_id_e_type as_id,
                                            void* msg);
...
static void ftm_egprs_ber_send_MAC_L1_FREQ_CONFIG( sys_modem_as_id_e_type as_id,
                                                   ftm_gsm_chains_enum_type gsm_curr_chain);
static void ftm_egprs_ber_send_MAC_L1_DL_TBF_CONFIG( sys_modem_as_id_e_type as_id, 
                                                     ftm_gsm_chains_enum_type gsm_curr_chain);
static void ftm_egprs_ber_send_MAC_L1_RELEASE( sys_modem_as_id_e_type as_id, 
                                               ftm_gsm_chains_enum_type gsm_curr_chain);
static void ftm_egprs_ber_send_MAC_L1_DYNAMIC_TBF_CONFIG( sys_modem_as_id_e_type as_id,
                                                          ftm_gsm_chains_enum_type gsm_curr_chain);
static void ftm_egprs_ber_send_MAC_L1_TS_RECONFIG( sys_modem_as_id_e_type as_id,
                                                   ftm_gsm_chains_enum_type gsm_curr_chain);

/* Functions to send primitives as RR would */

static void ftm_egprs_ber_send_MPH_START_GPRS_IDLE_MODE_REQ( sys_modem_as_id_e_type as_id);
...
void ftm_egprs_ber_handle_msgs_from_gl1( void *msg)
{
  ...
  {
    FTM_MSG_1( FTM_ERROR,"ftm_egprs_ber_handle_msgs_from_gl1()"
              "ftm_gsm_ns_active_chain is invalid! %d",
              gsm_curr_chain);
    return;
  }
  ...
  {
     FTM_MSG_1( FTM_ERROR,"ftm_egprs_ber_handle_msgs_from_gl1()"
                "cmn_vars_ptr not created for FTM GSM CHAIN: %d",
                gsm_curr_chain );
     return;
  }
    ...
    case MPH_SELECT_SPECIFIC_BCCH_CNF:
      {
        ...
        FTM_EGPRS_BER_SET_DEV_STATE( chain_vars_ptr->cmn_vars_ptr,  FTM_EGPRS_BER_STATE_NULL);
    ...
    case MPH_START_IDLE_MODE_CNF:
      {
        ...
        if ( FTM_EGPRS_BER_GET_DEV_STATE( chain_vars_ptr->cmn_vars_ptr) == FTM_EGPRS_BER_STATE_GPRS_IDLE_PENDING)
        {

          MSG_1(MSG_SSID_FTM, MSG_LEGACY_HIGH,"Received GPRS Start IDLE CNF",0);

          FTM_EGPRS_BER_SET_DEV_STATE( chain_vars_ptr->cmn_vars_ptr,  FTM_EGPRS_BER_STATE_GPRS_IDLE);

          ftm_gsm_ber_set_dev_state( (uint8)gsm_curr_chain, FTM_GSM_BER_STATE_GPRS_IDLE);
...
void ftm_egprs_ber_handle_msgs_from_gpl1( void)
{
  ...
  ftm_gsm_rfctl_vars_s *rx_vars_ptr;
  ...
  if ( gsm_curr_chain >= FTM_GSM_MAX_CHAINS )
  {
    FTM_MSG_1( FTM_ERROR,"ftm_egprs_ber_handle_msgs_from_gpl1()"
              "ftm_gsm_ns_active_chain is invalid! %d",
              gsm_curr_chain);
    return;
  }
  ...
  if( chain_vars_ptr->cmn_vars_ptr == NULL )
  {
    ERR_FATAL( "ftm_egprs_ber_handle_msgs_from_gpl1() cmn_vars_ptr is NULL for active chain %d", 
                gsm_curr_chain, 0,0 );
    return;
  }
      ...
      case L1_MAC_FREQ_ASS_CNF:
        {
          ...
          MSG_2(MSG_SSID_FTM, MSG_LEGACY_HIGH, 
                ...
                FTM_EGPRS_BER_GET_DEV_STATE( chain_vars_ptr->cmn_vars_ptr));

          if ( message->msg.tbf_freq_ass_cnf.tbf_freq_cnf   == DL_TBF_FREQ_CONF  &&
               FTM_EGPRS_BER_GET_DEV_STATE( chain_vars_ptr->cmn_vars_ptr) == FTM_EGPRS_BER_STATE_DL_TBF_ESTABLISHING   )
          {
            ...
            if ( message->msg.tbf_freq_ass_cnf.band_supported == TRUE)
            {
              ...
              ftm_egprs_ber_send_MAC_L1_DL_TBF_CONFIG( as_id, gsm_curr_chain );
            }
            else
            {
              ...
              ftm_egprs_ber_log_tbf_ind( FTM_LOG_EGPRS_BER_TBF_DL, FTM_LOG_EGPRS_TBF_EVENT_ERROR);
                    ...
                    FTM_EGPRS_BER_GET_DEV_STATE( chain_vars_ptr->cmn_vars_ptr) == FTM_EGPRS_BER_STATE_UL_TBF_ESTABLISHING )
          {
            ...
            if ( message->msg.tbf_freq_ass_cnf.band_supported == TRUE)
            {
              ...
              ftm_egprs_ber_send_MAC_L1_DYNAMIC_TBF_CONFIG( as_id, gsm_curr_chain );
            }
            else
            {

              FTM_EGPRS_BER_SET_DEV_STATE( chain_vars_ptr->cmn_vars_ptr,  FTM_EGPRS_BER_STATE_DL_TBF_ESTABLISHED);
              ftm_egprs_ber_log_tbf_ind( FTM_LOG_EGPRS_BER_TBF_DL, FTM_LOG_EGPRS_TBF_EVENT_ERROR);
            }
          }
          else if ( message->msg.tbf_freq_ass_cnf.tbf_freq_cnf   == UL_DL_TBF_FREQ_CONF                      &&
                    FTM_EGPRS_BER_GET_DEV_STATE( chain_vars_ptr->cmn_vars_ptr)                    == FTM_EGPRS_BER_STATE_UL_TBF_ESTABLISHED )
          {
            ...
            if ( message->msg.tbf_freq_ass_cnf.band_supported == TRUE)
            {
              ...
              ftm_egprs_ber_send_MAC_L1_TS_RECONFIG( as_id, gsm_curr_chain );
            }
            else
            {
      ...
      case MAC_PH_CONNECT_IND:
        {
               ...
               FTM_EGPRS_BER_GET_DEV_STATE(chain_vars_ptr->cmn_vars_ptr) == FTM_EGPRS_BER_STATE_DL_TBF_ESTABLISHING   )
          {
            ...
            ftm_gsm_ber_set_dev_state( (uint8)gsm_curr_chain, FTM_GSM_BER_STATE_GPRS_PDTCH);
            ...
            FTM_EGPRS_BER_SET_DEV_STATE( chain_vars_ptr->cmn_vars_ptr,  FTM_EGPRS_BER_STATE_UL_TBF_ESTABLISHING);

            /* Send the Freq config for UL TBF */

            ftm_egprs_ber_send_MAC_L1_FREQ_CONFIG( as_id, gsm_curr_chain );
                    ...
                    FTM_EGPRS_BER_GET_DEV_STATE( chain_vars_ptr->cmn_vars_ptr)             == FTM_EGPRS_BER_STATE_UL_TBF_ESTABLISHING )
          {
            ...
            FTM_EGPRS_BER_SET_DEV_STATE( chain_vars_ptr->cmn_vars_ptr,  FTM_EGPRS_BER_STATE_UL_TBF_ESTABLISHED);
	    ...
	    as_id = as_id;
          ...
          MSG_2(MSG_SSID_FTM, MSG_LEGACY_HIGH,
                ...
                message->msg.connect_ind.tbf_type_ind, FTM_EGPRS_BER_GET_DEV_STATE( chain_vars_ptr->cmn_vars_ptr)); 
          break;
        }
      ...
      case L1_MAC_TBF_REL_CONFIRM:
        {
          ...
          if ( message->msg.tbf_released.tbf_released == DL_TBF_RELEASE_CONF &&
               FTM_EGPRS_BER_GET_DEV_STATE( chain_vars_ptr->cmn_vars_ptr)              == FTM_EGPRS_BER_STATE_DL_TBF_RELEASING)
          {
            ...
            ftm_gsm_ber_set_dev_state( (uint8)gsm_curr_chain, FTM_GSM_BER_STATE_GPRS_IDLE );
                    ...
                    FTM_EGPRS_BER_GET_DEV_STATE( chain_vars_ptr->cmn_vars_ptr)              == FTM_EGPRS_BER_STATE_UL_DL_TBF_RELEASING)
          {
          ...
          MSG_2(MSG_SSID_FTM, MSG_LEGACY_HIGH,"Received MAC_TBF_REL_CONFIRM: type=%d state=%d", 
                message->msg.tbf_released.tbf_released,FTM_EGPRS_BER_GET_DEV_STATE( chain_vars_ptr->cmn_vars_ptr));
          break;
        }
                                                   ...
                                                   ftm_gsm_chains_enum_type gsm_curr_chain)
{
  ...
  if (chain_vars_ptr->cmn_vars_ptr == NULL)
  {
    ERR_FATAL( "ftm_egprs_ber_send_MAC_L1_FREQ_CONFIG() cmn_vars_ptr NULL for FTM GSM Chain %d", 
               gsm_curr_chain, 0, 0 );
    return;
  }
  cmn_vars_ptr = chain_vars_ptr->cmn_vars_ptr;

  if ( FTM_EGPRS_BER_GET_DEV_STATE( cmn_vars_ptr) == FTM_EGPRS_BER_STATE_DL_TBF_ESTABLISHING)
  {

    config_data.mac_l1_freq_config.tbf_freq_cnf               = DL_FREQ_CONFIG;
    config_data.mac_l1_freq_config.frequency_information.tsc  = cmn_vars_ptr->current_egprs_dl_tbf_info.dl_tsc;
  }
  else if ( FTM_EGPRS_BER_GET_DEV_STATE( cmn_vars_ptr) == FTM_EGPRS_BER_STATE_UL_TBF_ESTABLISHING)
  {

    config_data.mac_l1_freq_config.tbf_freq_cnf               = UL_FREQ_CONFIG;
    config_data.mac_l1_freq_config.frequency_information.tsc  = cmn_vars_ptr->current_egprs_ul_tbf_info.ul_tsc;
  }
  else if ( FTM_EGPRS_BER_GET_DEV_STATE( cmn_vars_ptr) == FTM_EGPRS_BER_STATE_UL_TBF_ESTABLISHED)
  {
  ...
  config_data.mac_l1_freq_config.frequency_information.frequency_list.channel[0].num  = cmn_vars_ptr->current_egprs_dl_tbf_info.dl_arfcn.channel;      
  config_data.mac_l1_freq_config.frequency_information.frequency_list.channel[0].band = (sys_band_T)cmn_vars_ptr->current_egprs_dl_tbf_info.dl_arfcn.band;
  ...
  ftm_egprs_ber_send_msg_to_gpl1( as_id, ( void *) &config_data);
                                                   ...
                                                   ftm_gsm_chains_enum_type gsm_curr_chain )
{
  ...
  if (chain_vars_ptr->cmn_vars_ptr == NULL)
  {
    ERR_FATAL( "ftm_egprs_ber_send_MAC_L1_TS_RECONFIG() cmn_vars_ptr NULL for FTM GSM Chain %d", 
               gsm_curr_chain, 0, 0 );
    return;
  }
  cmn_vars_ptr = chain_vars_ptr->cmn_vars_ptr;
  ...
  {
    if ( cmn_vars_ptr->current_egprs_dl_tbf_info.dl_slot_config[i].slot_active == TRUE)
    {

      slot = cmn_vars_ptr->current_egprs_dl_tbf_info.dl_slot_config[i].slot_number;
      ...
      if ( slot > MAX_TN)
      {
        ...
        cmn_vars_ptr->current_egprs_dl_tbf_info.dl_slot_config[i].slot_active = FALSE;
      }
      else
      {
  ...
  config_data.mac_l1_ts_reconfig.dl_tfi                     = cmn_vars_ptr->current_egprs_dl_tbf_info.dl_tfi;
  ...
  for ( i = 0; i < FTM_EGPRS_BER_MAX_DL_UL_SLOTS_V2; i++)
  {
    ...
    if ( cmn_vars_ptr->current_egprs_ul_tbf_info.ul_slot_config[i].slot_active == TRUE)
    {

      slot = cmn_vars_ptr->current_egprs_ul_tbf_info.ul_slot_config[i].slot_number;
      ...
      {
        /* turn off slot if invalid */
        cmn_vars_ptr->current_egprs_ul_tbf_info.ul_slot_config[i].slot_active = FALSE;
      }
      else
      {
        ...
        if ( gamma_ptr != NULL)
        {
          ...
          gamma_ptr->gamma   = cmn_vars_ptr->current_egprs_ul_tbf_info.ul_slot_config[i].ul_gamma;
        }
  ...
  for ( i = 0; i < FTM_EGPRS_BER_MAX_DL_UL_SLOTS_V2; i++)
  {
    ...
    if ( cmn_vars_ptr->current_egprs_ul_tbf_info.ul_slot_config[i].slot_active == TRUE)
    {
      ...
      config_data.mac_l1_ts_reconfig.usf[ slot].usf_value = cmn_vars_ptr->current_egprs_ul_tbf_info.usf;
    }
  }
                                                     ...
                                                     ftm_gsm_chains_enum_type gsm_curr_chain)
{
  ...
  if (chain_vars_ptr->cmn_vars_ptr == NULL)
  {
    ERR_FATAL( "ftm_egprs_ber_send_MAC_L1_DL_TBF_CONFIG() cmn_vars_ptr NULL for FTM GSM Chain %d", 
               gsm_curr_chain, 0, 0 );
    return;
  }
  cmn_vars_ptr = chain_vars_ptr->cmn_vars_ptr;
  ...
  config_data.mac_l1_dl_tbf_config.dl_tfi                           = cmn_vars_ptr->current_egprs_dl_tbf_info.dl_tfi;
  ...
  cmn_vars_ptr->current_egprs_l1_dl_tbf_info = config_data.mac_l1_dl_tbf_config;
                                               ...
                                               ftm_gsm_chains_enum_type gsm_curr_chain )
{
  ...
  if ( FTM_EGPRS_BER_GET_DEV_STATE( cmn_vars_ptr) <= FTM_EGPRS_BER_STATE_GPRS_IDLE)
  {
  ...
  if ( FTM_EGPRS_BER_GET_DEV_STATE( cmn_vars_ptr) == FTM_EGPRS_BER_STATE_DL_TBF_ESTABLISHED)
  {

    config_data.mac_l1_release.release_tbf = DL_TBF_NORMAL;

    FTM_EGPRS_BER_SET_DEV_STATE( cmn_vars_ptr,  FTM_EGPRS_BER_STATE_DL_TBF_RELEASING);
  }
  else if ( FTM_EGPRS_BER_GET_DEV_STATE( cmn_vars_ptr) == FTM_EGPRS_BER_STATE_UL_TBF_ESTABLISHED)
  {
    ...
    FTM_EGPRS_BER_SET_DEV_STATE( cmn_vars_ptr,  FTM_EGPRS_BER_STATE_UL_DL_TBF_RELEASING);
  }

  MSG_2(MSG_SSID_FTM, MSG_LEGACY_HIGH,"Sending MAC_L1_RELEASE type=%d state=%d", 
        config_data.mac_l1_release.release_tbf, FTM_EGPRS_BER_GET_DEV_STATE( cmn_vars_ptr));
                                                          ...
                                                          ftm_gsm_chains_enum_type gsm_curr_chain )
{
  ...
  if (chain_vars_ptr->cmn_vars_ptr == NULL)
  {
    ERR_FATAL( "ftm_egprs_ber_send_MAC_L1_DYNAMIC_TBF_CONFIG() cmn_vars_ptr NULL for FTM GSM Chain %d", 
               gsm_curr_chain, 0, 0 );
    return;
  }
  cmn_vars_ptr = chain_vars_ptr->cmn_vars_ptr;
  ...
  for ( i = 0; i < FTM_EGPRS_BER_MAX_DL_UL_SLOTS_V2; i++)
  {
    if ( cmn_vars_ptr->current_egprs_ul_tbf_info.ul_slot_config[i].slot_active == TRUE)
    {
      if ( cmn_vars_ptr->current_egprs_ul_tbf_info.ul_slot_config[i].slot_number > MAX_TN)
      {
        /* turn off slot if invalid */
        cmn_vars_ptr->current_egprs_ul_tbf_info.ul_slot_config[i].slot_active = FALSE;
      }
  ...
  for ( i = 0; i < FTM_EGPRS_BER_MAX_DL_UL_SLOTS_V2; i++)
  {
    if ( cmn_vars_ptr->current_egprs_ul_tbf_info.ul_slot_config[i].slot_active == TRUE)
    {
      ...
      config_data.mac_l1_dynamic_tbf_config.dynamic_tbf.usf[ slot].usf_value = cmn_vars_ptr->current_egprs_ul_tbf_info.usf;
    }
  }
  ...
  cmn_vars_ptr->current_egprs_l1_dynamic_tbf_info = config_data.mac_l1_dynamic_tbf_config.dynamic_tbf;
...
PARAMETERS 
  ftm_egprs_ber_pkt_type*  - payload pointer from ftm gsm ber dispatch command
  as_id              - GL1 as_id
  curr_chain   ftm gsm active chain
...
===========================================================================*/
ftm_rsp_pkt_type ftm_egprs_ber_dispatch(  ftm_egprs_ber_pkt_type* pkt, 
                                          sys_modem_as_id_e_type as_id,
                                          uint8 curr_chain)
{
  ...
  ftm_gsm_chain_vars_s* chain_vars_ptr = ftm_gsm_get_chain_vars_ptr( curr_chain );
  ...
  if( chain_vars_ptr->cmn_vars_ptr == NULL )
  {
    ERR_FATAL( "ftm_gsm_ber_handle_rr_msgs() cmn_vars_ptr is NULL for FTM GSM Chain %d", 
               chain_vars_ptr->gsm_chain, 0, 0 );
    return( rsp );
  }
  /*update the variable pointer locally*/
  cmn_vars_ptr = chain_vars_ptr->cmn_vars_ptr;
  ...
  **************************************************************************/
  ...
  case FTM_EGPRS_BER_START_GPRS_IDLE_MODE_REQ:
    {
      ...
      if ( ftm_gsm_ber_get_dev_state(curr_chain) != FTM_GSM_BER_STATE_ACQ_BCCH)
      {
      ...
      ftm_egprs_ber_send_MPH_START_GPRS_IDLE_MODE_REQ( as_id );

      FTM_EGPRS_BER_SET_DEV_STATE( cmn_vars_ptr, FTM_EGPRS_BER_STATE_GPRS_IDLE_PENDING);
  ...
  case FTM_EGPRS_BER_ESTABLISH_SRB_LOOPBACK:
    {
      ...
      if ( FTM_EGPRS_BER_GET_DEV_STATE( cmn_vars_ptr) == FTM_EGPRS_BER_STATE_GPRS_IDLE)
      {
        ...
        FTM_EGPRS_BER_SET_DEV_STATE( cmn_vars_ptr,  FTM_EGPRS_BER_STATE_DL_TBF_ESTABLISHING);
        ftm_egprs_ber_send_MAC_L1_FREQ_CONFIG( as_id, curr_chain );

        cmd_status = FTM_EGPRS_BER_SYNC_STATUS_SUCCESS;
      }
      else if ( FTM_EGPRS_BER_GET_DEV_STATE( cmn_vars_ptr) == FTM_EGPRS_BER_STATE_DL_TBF_ESTABLISHED)
      {

        MSG_1(MSG_SSID_FTM, MSG_LEGACY_HIGH,"Establishing UL TBF, and closing loop", 0);

        FTM_EGPRS_BER_SET_DEV_STATE( cmn_vars_ptr,  FTM_EGPRS_BER_STATE_UL_TBF_ESTABLISHING);
        ftm_egprs_ber_send_MAC_L1_FREQ_CONFIG( as_id, curr_chain );
  ...
  case FTM_EGPRS_BER_RELEASE_ALL_TBF:
    {
      ...
      if ( FTM_EGPRS_BER_GET_DEV_STATE( cmn_vars_ptr) != FTM_EGPRS_BER_STATE_DL_TBF_ESTABLISHED &&
           FTM_EGPRS_BER_GET_DEV_STATE( cmn_vars_ptr) != FTM_EGPRS_BER_STATE_UL_TBF_ESTABLISHED)
      {
      ...
      ftm_egprs_ber_send_MAC_L1_RELEASE( as_id,
                                         curr_chain );
  ...
  case FTM_EGPRS_BER_CONFIGURE_DL_TBF_V2:
    {
      ...
      cmn_vars_ptr->current_egprs_dl_tbf_info.dl_tfi         = dl_tbf_pkt->dl_tfi;
      cmn_vars_ptr->current_egprs_dl_tbf_info.dl_tsc         = dl_tbf_pkt->dl_tsc;
      cmn_vars_ptr->current_egprs_dl_tbf_info.dl_arfcn       = dl_tbf_pkt->dl_arfcn;
      ...
      {
        cmn_vars_ptr->current_egprs_dl_tbf_info.dl_slot_config[ slot_config_index].slot_active = FALSE;
      }

      for ( slot_config_index = 0; slot_config_index < num_slots; slot_config_index++)
      {

        cmn_vars_ptr->current_egprs_dl_tbf_info.dl_slot_config[ slot_config_index] = dl_tbf_pkt->dl_slot_config[ slot_config_index];
      }
  ...
  case FTM_EGPRS_BER_CONFIGURE_UL_TBF_V2:
    {
      ...
      cmn_vars_ptr->current_egprs_ul_tbf_info.usf      = ul_tbf_pkt->usf;
      cmn_vars_ptr->current_egprs_ul_tbf_info.ul_tsc   = ul_tbf_pkt->ul_tsc;
      ...
      for ( slot_config_index = 0; slot_config_index < FTM_EGPRS_BER_MAX_DL_UL_SLOTS_V2; slot_config_index++)
      {
        cmn_vars_ptr->current_egprs_ul_tbf_info.ul_slot_config[ slot_config_index].slot_active = FALSE;
      }

      for ( slot_config_index = 0; slot_config_index < num_slots; slot_config_index++)
      {

        cmn_vars_ptr->current_egprs_ul_tbf_info.ul_slot_config[ slot_config_index] = ul_tbf_pkt->ul_slot_config[ slot_config_index];
      }

      MSG_2(MSG_SSID_FTM, MSG_LEGACY_LOW,"  UL TBF Config: slot_0: %d, slot_1: %d", 
            cmn_vars_ptr->current_egprs_ul_tbf_info.ul_slot_config[0].slot_number,
            cmn_vars_ptr->current_egprs_ul_tbf_info.ul_slot_config[1].slot_number);
      MSG_2(MSG_SSID_FTM, MSG_LEGACY_LOW,"  UL TBF Config: slot_0_gamma: %d, slot_1_gamma: %d",  
            cmn_vars_ptr->current_egprs_ul_tbf_info.ul_slot_config[0].ul_gamma, 
            cmn_vars_ptr->current_egprs_ul_tbf_info.ul_slot_config[1].ul_gamma);
  ...
  if ( cmd_status == FTM_EGPRS_BER_SYNC_STATUS_GEN_FAILURE )
  {
    ...
    MSG_3(MSG_SSID_FTM, MSG_LEGACY_HIGH,
          ...
          pkt->ftm_cmd_header.cmd_id, 
          ftm_gsm_ber_get_dev_state( curr_chain), 
          FTM_EGPRS_BER_GET_DEV_STATE(cmn_vars_ptr));
  }
...
DESCRIPTION    This function returns current state of FTM EGPRS BER operations.
 
PARAMETERS 
  gsm_curr_chain  -FTM GSM current chain 
 
RETURN VALUE 
  ftm_egprs_ber_state_type - current state of FTM EGPRS BER operations

DEPENDENCIES
  none

===========================================================================*/
ftm_egprs_ber_state_type ftm_egprs_ber_get_dev_state(uint8 gsm_curr_chain) 
{
  ftm_gsm_chain_vars_s * chain_vars_ptr = ftm_gsm_get_chain_vars_ptr( (ftm_gsm_chains_enum_type)gsm_curr_chain );

  if( chain_vars_ptr->cmn_vars_ptr == NULL )
  {
    ERR_FATAL( "ftm_egprs_ber_get_dev_state() cmn_vars_ptr is NULL for FTM GSM Chain %d", 
               gsm_curr_chain, 0, 0 );
    return FTM_EGPRS_BER_STATE_NULL;
  }

  return FTM_EGPRS_BER_GET_DEV_STATE(chain_vars_ptr->cmn_vars_ptr);
}
