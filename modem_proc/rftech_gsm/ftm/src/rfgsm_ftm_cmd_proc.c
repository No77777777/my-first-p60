...
$Header: //components/rel/rftech_gsm.mpss/3.11/ftm/src/rfgsm_ftm_cmd_proc.c#1 $ 
...
07/28/15   ec      Use passed in umid for rsp to set SAWless lin mode
06/19/15   tsr     Implemented subscription based handling of FTM<->RF Tasks MSGR messages  
06/12/15   tsr     Added MSG Router Framework for dispatching FTM Task->RF Tasks messages based
                   on FTM Subscription 
01/06/15   piy     Adding support for RFM_DEVICE_4 and RFM_DEVICE_5 in the 
                   rfgsm_ftm_cmd_task_dispatcher API 
05/04/15   zbz     DRx IQ capture
04/16/15   sc      Add split Rx/Tx device restrictions on dispatch APIs
04/15/15   sc      Remove device restrictions on dispatch APIs
...
/*! @todo The remainder of these commands are placeholders for possible future use */
    ...
    RFA_RF_GSM_FTM_SET_QPOET_MODE_BIAS_CMD,
...
/*! @var rfgsm_ftm_umid_cmd_sub1_list                                         */
/*! @brief This array holds the request (CMD) command list used to register  */
/*!        the whole list with MSGR for subscription 1                       */

static msgr_umid_type rfgsm_ftm_umid_cmd_sub1_list[] = 
{
  RFA_RF_GSM_FTM_SET_PATH_DELAY_SUB1_CMD,
  RFA_RF_GSM_FTM_SET_SLOT_OVERRIDE_FLAG_SUB1_CMD,
  RFA_RF_GSM_FTM_SET_LINEAR_PA_RANGE_SUB1_CMD,
  RFA_RF_GSM_FTM_SET_LINEAR_RGI_SUB1_CMD,
  RFA_RF_GSM_FTM_SET_QPOET_MODE_BIAS_SUB1_CMD,
  RFA_RF_GSM_FTM_CAPTURE_IQ_SUB1_CMD,
  RFA_RF_GSM_FTM_SET_TX_POW_DBM_SUB1_CMD,
  RFA_RF_GSM_FTM_TUNE_TX_SUB1_CMD,
  RFA_RF_GSM_FTM_TUNER_TUNE_CODE_OVERRIDE_SUB1_CMD,
  RFA_RF_GSM_FTM_SET_IP2_CAL_PARAMS_SUB1_CMD,
  RFA_RF_GSM_FTM_GET_IP2_CAL_PARAMS_SUB1_CMD,
  RFA_RF_GSM_FTM_GET_IP2_CAL_RESULTS_SUB1_CMD,
  RFA_RF_GSM_FTM_SET_SAWLESS_LIN_MODE_SUB1_CMD,
  RFA_RF_GSM_FTM_GET_SAWLESS_LIN_MODE_SUB1_CMD,
};
/*=========================================================================== */

/*! @var rfgsm_ftm_umid_cmd_sub2_list                                         */
/*! @brief This array holds the request (CMD) command list used to register  */
/*!        the whole list with MSGR for subscription 2                       */
static msgr_umid_type rfgsm_ftm_umid_cmd_sub2_list[] = 
{
  RFA_RF_GSM_FTM_SET_PATH_DELAY_SUB2_CMD,
  RFA_RF_GSM_FTM_SET_SLOT_OVERRIDE_FLAG_SUB2_CMD,
  RFA_RF_GSM_FTM_SET_LINEAR_PA_RANGE_SUB2_CMD,
  RFA_RF_GSM_FTM_SET_LINEAR_RGI_SUB2_CMD,
  RFA_RF_GSM_FTM_SET_QPOET_MODE_BIAS_SUB2_CMD,
  RFA_RF_GSM_FTM_CAPTURE_IQ_SUB2_CMD,
  RFA_RF_GSM_FTM_SET_TX_POW_DBM_SUB2_CMD,
  RFA_RF_GSM_FTM_TUNE_TX_SUB2_CMD,
  RFA_RF_GSM_FTM_TUNER_TUNE_CODE_OVERRIDE_SUB2_CMD,
  RFA_RF_GSM_FTM_SET_IP2_CAL_PARAMS_SUB2_CMD,
  RFA_RF_GSM_FTM_GET_IP2_CAL_PARAMS_SUB2_CMD,
  RFA_RF_GSM_FTM_GET_IP2_CAL_RESULTS_SUB2_CMD,
  RFA_RF_GSM_FTM_SET_SAWLESS_LIN_MODE_SUB2_CMD,
  RFA_RF_GSM_FTM_GET_SAWLESS_LIN_MODE_SUB2_CMD,
};
...
/*! @brief Definitions of the RF GSM FTM message count for the mailboxes.
*/
#define RF_GSM_FTM_CMD_CNT(x) \
...
===========================================================================*/
extern boolean ftm_gsm_print_umid_debug;
                                                     ...
                                                     msgr_umid_type rsp_umid )
{
  ...
  rsp_msg.hdr.id = 0;
  msgr_init_hdr( &(rsp_msg.hdr), MSGR_RFA_RF_GSM_FTM, rsp_umid);
  ...
  cmd_rsp =  ftm_gsm_rf_msgr_send_debug( rfgsm_ftm_msg_ptr->rfm_dev, rsp_umid, ret_status);
                                                        ...
                                                        msgr_umid_type rsp_umid )
{
  ...
  /* Sanity check and print debug for msgr_send */
  cmd_rsp =  ftm_gsm_rf_msgr_send_debug( rsp_msg.rfm_dev, rsp_umid, msgr_send_status );
...
{
  /* Initialise response and result variables */
  rfa_rf_gsm_ftm_set_qpoet_mode_bias_rsp_s rsp_msg;
  rfgsm_ftm_dispatch_outcome_t cmd_rsp = RFGSM_FTM_DISPATCH_UNSUCCESSFUL;
  errno_enum_type msgr_send_status = E_SUCCESS;
  ...
  if( rfgsm_ftm_msg_ptr == NULL )
  {
    MSG( MSG_SSID_FTM, MSG_LEGACY_ERROR, "Pointer argument is NULL in rfgsm_ftm_set_qpoet_mode_bias" );
    cmd_rsp = RFGSM_FTM_DISPATCH_NULL_PTR;
    return cmd_rsp;
  }
  ...
  rsp_msg.hdr                           = rfgsm_ftm_msg_ptr->hdr;
  rsp_msg.rfm_dev                       = rfgsm_ftm_msg_ptr->rfm_dev;
  rsp_msg.qpoet_params                  = rfgsm_ftm_msg_ptr->qpoet_params;
  rsp_msg.cmd_result                    = RFA_RF_GSM_FTM_EXECUTION_ERROR;

  rfgsm_core_handle_ptr->ftm_apt_mode_override = rfgsm_ftm_msg_ptr->qpoet_params.mode;
  rfgsm_core_handle_ptr->ftm_bias_override = rfgsm_ftm_msg_ptr->qpoet_params.bias;
  rfgsm_core_handle_ptr->ftm_qpoet_override = rfgsm_ftm_msg_ptr->qpoet_params.override;
	
  MSG_3(MSG_SSID_RF, MSG_LEGACY_HIGH,"rfgsm_ftm_set_qpoet_mode_bias, mode:%d, bias :%d, override:%d ", 
  rfgsm_ftm_msg_ptr->qpoet_params.mode, rfgsm_ftm_msg_ptr->qpoet_params.bias, rfgsm_ftm_msg_ptr->qpoet_params.override);
  ...
  rsp_msg.cmd_result = RFA_RF_GSM_FTM_SUCCESS;
  msgr_init_hdr( &(rsp_msg.hdr), MSGR_RFA_RF_GSM_FTM, rsp_umid );
  msgr_send_status = msgr_send( &(rsp_msg.hdr), sizeof(rfa_rf_gsm_ftm_set_qpoet_mode_bias_rsp_s) );
  /* Sanity check and print debug for msgr_send */
  cmd_rsp =  ftm_gsm_rf_msgr_send_debug( rsp_msg.rfm_dev, rsp_umid, msgr_send_status );
                                                         ...
                                                         msgr_umid_type rsp_umid )
{
  ...
  errno_enum_type msgr_send_status = E_SUCCESS;
  rfgsm_core_handle_type *rfgsm_core_handle_ptr = NULL;
  ftm_gsm_rfctl_vars_s * ftm_gsm_rfctl_vars_ptr = NULL;
  ...
  ftm_gsm_rfctl_vars_ptr = ftm_gsm_get_vars_ptr(rfgsm_ftm_msg_ptr->rfm_dev);
  rfgsm_core_handle_ptr = rfgsm_core_handle_get(rfgsm_ftm_msg_ptr->rfm_dev);
  if (rfgsm_core_handle_ptr == NULL)
  {
  ...
  rsp_msg.iq_capture_drx                = rfgsm_core_handle_ptr->iq_capture_drx;
  ...
  {
    MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, "IQ capture buffer for device %d is NULL", rfgsm_core_get_drx_dev(rfgsm_ftm_msg_ptr->rfm_dev) );
    cmd_rsp = RFGSM_FTM_DISPATCH_NULL_PTR;
    return cmd_rsp;
  }

  rfgsm_core_handle_ptr->iq_capture_drx->capture_type = rfgsm_ftm_msg_ptr->iq_capture_type;
  ...
  if(rfgsm_ftm_msg_ptr->iq_capture_type != FTM_IQ_CAPTURE_STOP)
  {
    ...
    memset( rfgsm_core_handle_ptr->iq_capture_drx->rfgsm_iq_buffer, 
            0, 
            RFGSM_MAX_IQ_BUFFER_LEN*sizeof(int16)); 
  }
...
rfgsm_ftm_dispatch_outcome_t rfgsm_ftm_internal_cmd_task1_dispatcher(rfgsm_ftm_msg_u* msg_ptr)
{
  ...
  if (msg_ptr != NULL)
  {
    ...
    {
      case RFA_RF_GSM_FTM_SET_LINEAR_PA_RANGE_SUB1_CMD:
        MSG_1( MSG_SSID_RF, MSG_LEGACY_LOW, 
               "rfgsm_ftm_internal_cmd_task1_dispatcher() Message received ID:"
               "RFA_RF_GSM_FTM_SET_LINEAR_PA_RANGE_SUB1_CMD, UMID=0x%x",
               msgr_hdr_ptr->id );
        cmd_rsp = rfgsm_ftm_set_pa_range( (rfa_rf_gsm_ftm_set_pa_range_type_cmd_s*) msg_ptr, 
                                          RFA_RF_GSM_FTM_SET_LINEAR_PA_RANGE_SUB1_RSP );
        break;

      case RFA_RF_GSM_FTM_SET_PATH_DELAY_SUB1_CMD:
        MSG_1( MSG_SSID_RF, MSG_LEGACY_LOW,
               "rfgsm_ftm_internal_cmd_task1_dispatcher() Message received ID:"
               "RFA_RF_GSM_FTM_SET_PATH_DELAY_SUB1_CMD, UMID=0x%x",
                msgr_hdr_ptr->id );
        cmd_rsp = rfgsm_ftm_set_path_delay( (rfa_rf_gsm_ftm_set_path_delay_type_cmd_s*) msg_ptr,
                                            RFA_RF_GSM_FTM_SET_PATH_DELAY_SUB1_RSP );
        break;

      case RFA_RF_GSM_FTM_SET_LINEAR_RGI_SUB1_CMD:
        MSG_1( MSG_SSID_RF, MSG_LEGACY_LOW,
               "rfgsm_ftm_internal_cmd_task1_dispatcher() Message received ID:"
               "RFA_RF_GSM_FTM_SET_LINEAR_RGI_SUB1_CMD, UMID=0x%x",
                msgr_hdr_ptr->id );
        cmd_rsp = rfgsm_ftm_set_linear_rgi( (rfa_rf_gsm_ftm_set_linear_rgi_type_cmd_s*) msg_ptr,
                                            RFA_RF_GSM_FTM_SET_LINEAR_RGI_SUB1_RSP );
        break;

      case RFA_RF_GSM_FTM_SET_QPOET_MODE_BIAS_SUB1_CMD:
        MSG_1( MSG_SSID_RF, MSG_LEGACY_LOW,
               "rfgsm_ftm_internal_cmd_task1_dispatcher() Message received ID:"
               "RFA_RF_GSM_FTM_SET_QPOET_MODE_BIAS_SUB1_CMD, UMID=0x%x",
                msgr_hdr_ptr->id );
        cmd_rsp = rfgsm_ftm_set_qpoet_mode_bias( (rfa_rf_gsm_ftm_set_qpoet_mode_bias_cmd_s*) msg_ptr,
                                            RFA_RF_GSM_FTM_SET_QPOET_MODE_BIAS_SUB1_RSP );
        break;

      case RFA_RF_GSM_FTM_SET_TX_POW_DBM_SUB1_CMD:
        MSG( MSG_SSID_RF, MSG_LEGACY_LOW, 
             "rfgsm_ftm_internal_cmd_task1_dispatcher() Message received ID:"
             "RFA_RF_GSM_FTM_SET_TX_POW_DBM_SUB1_CMD" );
      ...
      case RFA_RF_GSM_FTM_SET_SLOT_OVERRIDE_FLAG_SUB1_CMD:
        MSG( MSG_SSID_RF, MSG_LEGACY_LOW,
              "rfgsm_ftm_internal_cmd_task1_dispatcher() Message received ID:"
              "RFA_RF_GSM_FTM_SET_SLOT_OVERRIDE_FLAG_SUB1_CMD" );
      ...
      case RFA_RF_GSM_FTM_CAPTURE_IQ_SUB1_CMD:
        MSG( MSG_SSID_RF, MSG_LEGACY_LOW,
             "rfgsm_ftm_internal_cmd_task1_dispatcher() Message received ID:"
             "RFA_RF_GSM_FTM_CAPTURE_IQ_SUB1_CMD" );
      ...
      case RFA_RF_GSM_FTM_TUNE_TX_SUB1_CMD:
        MSG( MSG_SSID_RF, MSG_LEGACY_LOW,
             "rfgsm_ftm_internal_cmd_task1_dispatcher() Message received ID:"
             "RFA_RF_GSM_FTM_TUNE_TX_SUB1_CMD" );
       ...
       case RFA_RF_GSM_FTM_TUNER_TUNE_CODE_OVERRIDE_SUB1_CMD:
        MSG( MSG_SSID_RF, MSG_LEGACY_LOW,
             "rfgsm_ftm_internal_cmd_task1_dispatcher() Message received ID:"
             "RFA_RF_GSM_FTM_TUNER_TUNE_CODE_OVERRIDE_SUB1_CMD" );
       ...
       case RFA_RF_GSM_FTM_SET_IP2_CAL_PARAMS_SUB1_CMD:
         MSG( MSG_SSID_RF, MSG_LEGACY_LOW,
              "rfgsm_ftm_internal_cmd_task1_dispatcher() Message received ID:"
              "RFA_RF_GSM_FTM_SET_IP2_CAL_PARAMS_SUB1_CMD" );
       ...
       case RFA_RF_GSM_FTM_GET_IP2_CAL_PARAMS_SUB1_CMD:
         MSG( MSG_SSID_RF, MSG_LEGACY_LOW,
              "rfgsm_ftm_internal_cmd_task1_dispatcher() Message received ID:"
              "RFA_RF_GSM_FTM_GET_IP2_CAL_PARAMS_SUB1_CMD" );
       ...
       case RFA_RF_GSM_FTM_GET_IP2_CAL_RESULTS_SUB1_CMD:
         MSG( MSG_SSID_RF, MSG_LEGACY_LOW,
              "rfgsm_ftm_internal_cmd_task1_dispatcher() Message received ID:"
              "RFA_RF_GSM_FTM_GET_IP2_CAL_RESULTS_SUB1_CMD" );
    ...
    case RFA_RF_GSM_FTM_SET_SAWLESS_LIN_MODE_SUB1_CMD:
       MSG( MSG_SSID_RF, MSG_LEGACY_LOW,
            "rfgsm_ftm_internal_cmd_task1_dispatcher() Message received ID:"
            "RFA_RF_GSM_FTM_SET_SAWLESS_LIN_MODE_SUB1_CMD" );
    ...
    case RFA_RF_GSM_FTM_GET_SAWLESS_LIN_MODE_SUB1_CMD:
       MSG( MSG_SSID_RF, MSG_LEGACY_LOW,
            "rfgsm_ftm_internal_cmd_task1_dispatcher() Message received ID: "
            "RFA_RF_GSM_FTM_GET_SAWLESS_LIN_MODE_SUB1_CMD" );
      ...
      default:
        MSG( MSG_SSID_FTM, MSG_LEGACY_ERROR,
             "rfgsm_ftm_internal_cmd_task1_dispatcher()"
             "Undefined RF GSM FTM CMD" );
...
/*!
  @brief
  This function is the rf gsm ftm dispatcher for the RFGSM FTM CMDs 
  the RF task2 receives. The dispatcher calls the function that sends a command 
  to the respective RF function. Upon completion a response is sent to the
  FTM task.
...
rfgsm_ftm_dispatch_outcome_t rfgsm_ftm_internal_cmd_task2_dispatcher(rfgsm_ftm_msg_u* msg_ptr)
{
  ...
  if (msg_ptr != NULL)
  {
    switch (msgr_hdr_ptr->id)
    {
      case RFA_RF_GSM_FTM_SET_LINEAR_PA_RANGE_SUB2_CMD:
        MSG_1( MSG_SSID_RF, MSG_LEGACY_LOW, 
               "rfgsm_ftm_internal_cmd_task2_dispatcher() Message received ID:"
               "RFA_RF_GSM_FTM_SET_LINEAR_PA_RANGE_SUB2_CMD, UMID=0x%x",
               msgr_hdr_ptr->id );
        cmd_rsp = rfgsm_ftm_set_pa_range( (rfa_rf_gsm_ftm_set_pa_range_type_cmd_s*) msg_ptr,
                                          RFA_RF_GSM_FTM_SET_LINEAR_PA_RANGE_SUB2_RSP );
        break;

      case RFA_RF_GSM_FTM_SET_PATH_DELAY_SUB2_CMD:
        MSG_1( MSG_SSID_RF, MSG_LEGACY_LOW,
               "rfgsm_ftm_internal_cmd_task2_dispatcher() Message received ID:"
               "RFA_RF_GSM_FTM_SET_PATH_DELAY_SUB2_CMD, UMID=0x%x",
                msgr_hdr_ptr->id );
        cmd_rsp = rfgsm_ftm_set_path_delay( (rfa_rf_gsm_ftm_set_path_delay_type_cmd_s*) msg_ptr,
                                            RFA_RF_GSM_FTM_SET_PATH_DELAY_SUB2_RSP );
        break;

      case RFA_RF_GSM_FTM_SET_LINEAR_RGI_SUB2_CMD:
        MSG_1( MSG_SSID_RF, MSG_LEGACY_LOW,
               "rfgsm_ftm_internal_cmd_task2_dispatcher() Message received ID:"
               "RFA_RF_GSM_FTM_SET_LINEAR_RGI_SUB2_CMD, UMID=0x%x",
                msgr_hdr_ptr->id );
        cmd_rsp = rfgsm_ftm_set_linear_rgi( (rfa_rf_gsm_ftm_set_linear_rgi_type_cmd_s*) msg_ptr,
                                            RFA_RF_GSM_FTM_SET_LINEAR_RGI_SUB2_RSP );
        break;

      case RFA_RF_GSM_FTM_SET_QPOET_MODE_BIAS_SUB2_CMD:
        MSG_1( MSG_SSID_RF, MSG_LEGACY_LOW,
               "rfgsm_ftm_internal_cmd_task1_dispatcher() Message received ID:"
               "RFA_RF_GSM_FTM_SET_QPOET_MODE_BIAS_SUB2_CMD, UMID=0x%x",
                msgr_hdr_ptr->id );
        cmd_rsp = rfgsm_ftm_set_qpoet_mode_bias( (rfa_rf_gsm_ftm_set_qpoet_mode_bias_cmd_s*) msg_ptr,
      ...
      case RFA_RF_GSM_FTM_SET_TX_POW_DBM_SUB2_CMD:
        MSG( MSG_SSID_RF, MSG_LEGACY_LOW, 
             "rfgsm_ftm_internal_cmd_task2_dispatcher() Message received ID:"
             "RFA_RF_GSM_FTM_SET_TX_POW_DBM_SUB2_CMD" );
        cmd_rsp = rfgsm_ftm_set_tx_pow_dbm( (rfa_rf_gsm_ftm_set_tx_pow_dbm_type_cmd_s*) msg_ptr,
      ...
      case RFA_RF_GSM_FTM_SET_SLOT_OVERRIDE_FLAG_SUB2_CMD:
        MSG( MSG_SSID_RF, MSG_LEGACY_LOW,
              "rfgsm_ftm_internal_cmd_task2_dispatcher() Message received ID:"
              "RFA_RF_GSM_FTM_SET_SLOT_OVERRIDE_FLAG_SUB2_CMD" );
        cmd_rsp = rfgsm_ftm_set_slot_override_flag( (rfa_rf_gsm_ftm_set_slot_override_flag_type_cmd_s*) msg_ptr,
      ...
      case RFA_RF_GSM_FTM_CAPTURE_IQ_SUB2_CMD:
        MSG( MSG_SSID_RF, MSG_LEGACY_LOW,
             "rfgsm_ftm_internal_cmd_task2_dispatcher() Message received ID:"
             "RFA_RF_GSM_FTM_CAPTURE_IQ_SUB2_CMD" );
        cmd_rsp = rfgsm_ftm_iq_capture_setup( (rfa_rf_gsm_ftm_capture_iq_type_cmd_s*) msg_ptr,
      ...
      case RFA_RF_GSM_FTM_TUNE_TX_SUB2_CMD:
        MSG( MSG_SSID_RF, MSG_LEGACY_LOW,
             "rfgsm_ftm_internal_cmd_task2_dispatcher() Message received ID:"
             "RFA_RF_GSM_FTM_TUNE_TX_SUB2_CMD" );
        cmd_rsp = rfgsm_ftm_set_mode_cmd( (rfa_rf_gsm_ftm_set_mode_type_cmd_s*) msg_ptr,
       ...
       case RFA_RF_GSM_FTM_TUNER_TUNE_CODE_OVERRIDE_SUB2_CMD:
        MSG( MSG_SSID_RF, MSG_LEGACY_LOW,
             "rfgsm_ftm_internal_cmd_task1_dispatcher() Message received ID:"
             "RFA_RF_GSM_FTM_TUNER_TUNE_CODE_OVERRIDE_SUB1_CMD" );
        cmd_rsp = rfgsm_ftm_tuner_override( (rfa_rf_gsm_ftm_tuner_override_type_cmd_s*) msg_ptr,
       ...
       case RFA_RF_GSM_FTM_SET_IP2_CAL_PARAMS_SUB2_CMD:
         MSG( MSG_SSID_RF, MSG_LEGACY_LOW,
              "rfgsm_ftm_internal_cmd_task2_dispatcher() Message received ID:"
              "RFA_RF_GSM_FTM_SET_IP2_CAL_PARAMS_SUB2_CMD" );
         cmd_rsp = rfgsm_ftm_set_ip2_cal_params( (rfa_rf_gsm_ftm_ip2_cal_type_cmd_s*) msg_ptr,
       ...
       case RFA_RF_GSM_FTM_GET_IP2_CAL_PARAMS_SUB2_CMD:
         MSG( MSG_SSID_RF, MSG_LEGACY_LOW,
              "rfgsm_ftm_internal_cmd_task2_dispatcher() Message received ID:"
              "RFA_RF_GSM_FTM_GET_IP2_CAL_PARAMS_SUB2_CMD" );
         cmd_rsp = rfgsm_ftm_get_ip2_cal_params ((rfa_rf_gsm_ftm_get_ip2_cal_info_cmd_s*) msg_ptr,
       ...
       case RFA_RF_GSM_FTM_GET_IP2_CAL_RESULTS_SUB2_CMD:
         MSG( MSG_SSID_RF, MSG_LEGACY_LOW,
              "rfgsm_ftm_internal_cmd_task2_dispatcher() Message received ID:"
              "RFA_RF_GSM_FTM_GET_IP2_CAL_RESULTS_SUB2_CMD" );
         cmd_rsp = rfgsm_ftm_get_ip2_cal_results ((rfa_rf_gsm_ftm_get_ip2_cal_results_cmd_s*) msg_ptr,
    ...
    case RFA_RF_GSM_FTM_SET_SAWLESS_LIN_MODE_SUB2_CMD:
       MSG( MSG_SSID_RF, MSG_LEGACY_LOW,
            "rfgsm_ftm_internal_cmd_task2_dispatcher() Message received ID:"
            "RFA_RF_GSM_FTM_SET_SAWLESS_LIN_MODE_SUB2_CMD" );
       cmd_rsp = rfgsm_ftm_set_sawless_lin_mode( (rfa_rf_gsm_ftm_set_sawless_lin_mode_cmd_s*) msg_ptr,
    ...
    case RFA_RF_GSM_FTM_GET_SAWLESS_LIN_MODE_SUB2_CMD:
       MSG( MSG_SSID_RF, MSG_LEGACY_LOW,
            "rfgsm_ftm_internal_cmd_task2_dispatcher() Message received ID: "
            "RFA_RF_GSM_FTM_GET_SAWLESS_LIN_MODE_SUB2_CMD" );
        cmd_rsp = rfgsm_ftm_get_sawless_lin_mode((rfa_rf_gsm_ftm_get_sawless_lin_mode_cmd_s*) msg_ptr,
      ...
      default:
        MSG( MSG_SSID_FTM, MSG_LEGACY_ERROR,
             "rfgsm_ftm_internal_cmd_task2_dispatcher()"
             "Undefined RF GSM FTM CMD" );
        cmd_rsp = RFGSM_FTM_DISPATCH_UNSUCCESSFUL;
        break;
    }
...
void rfgsm_ftm_cmd_dispatcher( rf_cmd_type* req_ptr )
{
  ...
  if(req_ptr != NULL)
  {
    ...
    cmd_rsp = rfgsm_ftm_internal_cmd_task1_dispatcher(&rfgsm_msgr_req);

    if (cmd_rsp != RFGSM_FTM_DISPATCH_SUCCESSFUL)
    {
      MSG_2( MSG_SSID_FTM, MSG_LEGACY_ERROR,
             "rfgsm_ftm_cmd_dispatcher():GSM FTM Command processor could not "
             "process the command 0x%x. Exited with error code %d",
             req_ptr->msgr_hdr.id,cmd_rsp );
...
void rfgsm_ftm_cmd_task_2_dispatcher( rf_cmd_type* req_ptr )
{
  ...
  if( req_ptr != NULL )
  {
    ...
    cmd_rsp =  rfgsm_ftm_internal_cmd_task2_dispatcher(&rfgsm_msgr_req);

    if (cmd_rsp != RFGSM_FTM_DISPATCH_SUCCESSFUL)
    {
      MSG_2( MSG_SSID_FTM, MSG_LEGACY_ERROR,
             "rfgsm_ftm_cmd_task_2_dispatcher():GSM FTM Command processor could not "
             "process the command 0x%x. Exited with error code %d",
             req_ptr->msgr_hdr.id,cmd_rsp );
      cmd_rsp = RFGSM_FTM_DISPATCH_UNSUCCESSFUL;
    }
...
boolean rfgsm_ftm_msgr_register( msgr_client_t *client_id, msgr_id_t id )
{
  ...
  ret_status = msgr_register_block(MSGR_RFA_RF_GSM_FTM,
                                   client_id,
                                   id,
                                   rfgsm_ftm_umid_cmd_sub1_list,
                                   RF_GSM_FTM_CMD_CNT(rfgsm_ftm_umid_cmd_sub1_list));
...
/*===========================================================================
  ...
  FUNCTION:  rfgsm_ftm_msgr_register_sub3
...
boolean rfgsm_ftm_msgr_register_sub2( msgr_client_t *client_id, msgr_id_t id )
{
  ...
  ret_status = msgr_register_block(MSGR_RFA_RF_GSM_FTM,
                                   client_id,
                                   id,
                                   rfgsm_ftm_umid_cmd_sub2_list,
                                   RF_GSM_FTM_CMD_CNT(rfgsm_ftm_umid_cmd_sub2_list) );
  if (ret_status != E_SUCCESS)
  {
    MSG( MSG_SSID_FTM, MSG_LEGACY_ERROR, "rfgsm_ftm_msgr_register_sub2() Unable to register GSM FTM message router register block." );
    rc = FALSE;
  }
  return(rc);
}
                                                            ...
                                                            msgr_umid_type rsp_umid)
{
  ...
  rsp_msg.rfm_dev=RFM_INVALID_DEVICE;
  if( rfgsm_ftm_msg_ptr == NULL )
  {
    MSG( MSG_SSID_FTM, MSG_LEGACY_ERROR, "Pointer argument is NULL in rfgsm_ftm_tuner_override" );
    cmd_rsp = RFGSM_FTM_DISPATCH_NULL_PTR;
  ...
  else if( rfgsm_core_handle_ptr->rfc_gsm_core_ptr == NULL )
  {
    MSG( MSG_SSID_RF, MSG_LEGACY_ERROR, "rfc_gsm_core_ptr is NULL!" );
    cmd_rsp = RFGSM_FTM_DISPATCH_NULL_PTR;
  }
  
  if( cmd_rsp != RFGSM_FTM_DISPATCH_SUCCESSFUL )
  {
    MSG( MSG_SSID_RF, MSG_LEGACY_ERROR, "rfgsm_ftm_set_ip2_cal_params cmd_rsp error not executing IP2 info!" );
  }
  else
  {
    ...
    if (! (rfgsm_ftm_msg_ptr->ip2_cal_params.use_default_params))
    {
      ...
      if(!rfgsm_core_dm_set_ip2_cal_info( rfgsm_ftm_msg_ptr->sub_id, 
                                          ...
                                          rfgsm_core_handle_ptr->ip2_cal_alpha, //needs to be revisited due to split Rx/Tx
