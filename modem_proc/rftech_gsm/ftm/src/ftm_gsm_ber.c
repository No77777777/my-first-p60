...
INITIALIZATION AND SEQUENCING REQUIREMENTS
  ...
  Copyright (c) 2008 - 2017 by Qualcomm Technologies INCORPORATED.
  All Rights Reserved.
*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*/
...
/*===========================================================================
  ...
  $Header: //components/rel/rftech_gsm.mpss/3.11/ftm/src/ftm_gsm_ber.c#1 $ 

when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
11/15/17   vb      [CR2006576] Added FTM_GSM_NS_CHAIN_SELECT feature
07/23/15   tsr     Move Non-Signalling device allocation in ftm_dispatch to 
                   update ftm device manager
07/16/15   tsr     Add GSM Support for FTM Device manager
07/13/15   tsr     Fix Klockwork Error from previous check-in 
07/13/15   tsr     Fixed compiler error 
07/13/15   tsr     Move GSM Non-Signalling state variables under common chain variables 
07/07/15   tsr     Added support to use Preferred RX and TX devices for Non_Signalling
07/07/15   tsr     Update GL1 structure for devices and DRX band support based on DRX device support  
...
06/03/15   tsr     Update GRF<->GL1 interface to use FTM devices in Non-Signalling
...
04/09/15   tsr     Remove deprecated NAS apis from GSM Non-Signalling call flow. 
                   The apis are not required for NS 
...
extern ftm_gsm_chains_enum_type ftm_gsm_ns_active_chain;
...
extern uint64 rfcommon_nv_get_rx_device_bc_config(rfm_device_enum_type device);
...
boolean ftm_gsm_ber_create_and_init_vars( ftm_gsm_cmn_vars_s* cmn_vars_ptr);
...
gs_status_T ftm_gsm_ber_send_FTM_SELECT_SPECIFIC_BCCH_REQ( sys_modem_as_id_e_type as_id,
                                                           uint16 chanNum, 
                                                           sys_band_T band );

gs_status_T ftm_gsm_ber_send_FTM_START_GSM_MODE_REQ( sys_modem_as_id_e_type as_id,
                                                     ftm_gsm_chains_enum_type gsm_curr_chain);

gs_status_T ftm_gsm_ber_send_FTM_CHANNEL_ASSIGN_REQ( sys_modem_as_id_e_type as_id,
                                                     ftm_gsm_cmn_vars_s* cmn_vars_ptr);

gs_status_T ftm_gsm_ber_send_FTM_CHANNEL_RELEASE_REQ( sys_modem_as_id_e_type as_id,
                                                      ftm_gsm_cmn_vars_s* cmn_vars_ptr);

gs_status_T ftm_gsm_ber_send_FTM_START_IDLE_REQ( sys_modem_as_id_e_type as_id );

gs_status_T ftm_gsm_ber_send_FTM_STOP_GSM_MODE_REQ( sys_modem_as_id_e_type as_id );
...
ftm_gsm_ber_state_type ftm_gsm_ber_get_state( void )
{
  ...
  for (i=0; i<FTM_GSM_MAX_CHAINS; i++) 
  {
    chain_vars_ptr = ftm_gsm_get_chain_vars_ptr( i );

    if( chain_vars_ptr->cmn_vars_ptr == NULL )
    {
      MSG_1( MSG_SSID_FTM, MSG_LEGACY_ERROR,"ftm_gsm_ber_get_dev_state() cmn_vars_ptr is NULL for FTM GSM Chain %d", 
...
/* If any of the devices are initialized but not stopped, return that state */
  return FTM_GSM_BER_DEV_GET_STATE(chain_vars_ptr->cmn_vars_ptr);
}
...
*/
void ftm_gsm_ber_set_dev_state( uint8 gsm_curr_chain, 
                                ftm_gsm_ber_state_type new_state)
{

  ftm_gsm_chain_vars_s* chain_vars_ptr = ftm_gsm_get_chain_vars_ptr( (ftm_gsm_chains_enum_type)gsm_curr_chain );

  if( chain_vars_ptr->cmn_vars_ptr == NULL )
  {
    ERR_FATAL( "ftm_gsm_ber_set_dev_state() cmn_vars_ptr is NULL for FTM GSM Chain %d", 
               chain_vars_ptr->gsm_chain, 0, 0 );
    return;
  }

  FTM_GSM_BER_DEV_SET_STATE( chain_vars_ptr->cmn_vars_ptr, new_state);
...
*/
ftm_gsm_ber_state_type ftm_gsm_ber_get_dev_state( uint8 gsm_curr_chain)
{
...
boolean ftm_gsm_ber_create_and_init_vars( ftm_gsm_cmn_vars_s* cmn_vars_ptr)
{
  ...
  {
    MSG( MSG_SSID_FTM, MSG_LEGACY_HIGH, "ftm_gsm_ber_create_and_init_vars() NS variables initialised");

    //Initialise status here
    cmn_vars_ptr->ftm_gsm_ber_status = ber_status_init;
    return TRUE;
  }
  else
  {
    ERR_FATAL( "ftm_gsm_ber_create_and_init_vars() NULL pointer passed to FTM BER INIT", 0,0,0 );
    return FALSE;
  }
...
===========================================================================*/
boolean ftm_gsm_ber_cleanup( rfm_device_enum_type remapped_rx_device )
{
  ...
  rx_vars_ptr = ftm_gsm_get_vars_ptr( remapped_rx_device );

  if( rx_vars_ptr == NULL )
  {
    ERR_FATAL("ftm_gsm_ber_cleanup() rx_vars_ptr is NULL for remapped_device %d", remapped_rx_device, 0, 0);
  }

  /*Update the GSM chain variables by retrieving RX/TX/DRX devices from RFC*/
  chain_vars_ptr = ftm_gsm_device_get_and_update_chain_vars( rx_vars_ptr->dispatch_device);

  if( chain_vars_ptr == NULL )
  {
    ERR_FATAL( "ftm_gsm_ber_cleanup() chain_vars_ptr NULL for dispatch_device %d and remapped_device %d", 
               rx_vars_ptr->dispatch_device, remapped_rx_device, 0 );
  }

  chain_vars_ptr->cmn_vars_ptr->gsm_current_state = FTM_STATE_PARK;

  as_id = ftm_gsm_ctl_map_sub_id_to_as_id( chain_vars_ptr->sub_id );

  if ( FTM_GSM_BER_DEV_GET_STATE(chain_vars_ptr->cmn_vars_ptr) == FTM_GSM_BER_STATE_STOPPED)
  {
    return TRUE;
  }

  if ( chain_vars_ptr->cmn_vars_ptr->ftm_gsm_ber_status.loopback_type != FTM_GSM_LOOPBACK_TYPE_NULL)
  {
  ...
  chain_vars_ptr->cmn_vars_ptr->ftm_gsm_ber_status.loopback_type       = FTM_GSM_LOOPBACK_TYPE_NULL;   
  chain_vars_ptr->cmn_vars_ptr->ftm_gsm_ber_status.saved_loopback_type = FTM_GSM_LOOPBACK_TYPE_NULL;   

  status = ftm_gsm_ber_send_FTM_STOP_GSM_MODE_REQ(as_id);
  FTM_GSM_BER_DEV_SET_STATE( chain_vars_ptr->cmn_vars_ptr,  FTM_GSM_BER_STATE_STOPPED);
...
void ftm_gsm_ber_send_rx_data_to_ftm( const ftm_gsm_ber_rx_data_ind_type *rx_data_ind)
{
  ...
  ftm_gsm_chains_enum_type gsm_curr_chain = ftm_gsm_ns_active_chain;
  ftm_gsm_chain_vars_s *chain_vars_ptr = NULL;
  ...
  if ( gsm_curr_chain >= FTM_GSM_MAX_CHAINS )
  {
    MSG_1( MSG_SSID_FTM, MSG_LEGACY_ERROR,
           "ftm_gsm_ber_send_rx_data_to_ftm() ftm_gsm_ns_active_chain invalid! %d",
           gsm_curr_chain);
    return;
  }
  ...
  if( chain_vars_ptr->cmn_vars_ptr == NULL )
  {
    ERR_FATAL( "ftm_gsm_ber_send_rx_data_to_ftm() cmn_vars_ptr NULL for active chain %d", 
               gsm_curr_chain, 0, 0 );
    return;
  }

  /* This is a temporary work-around until we implement a way to get the device from GL1 */
  if ( FTM_GSM_BER_DEV_GET_STATE(chain_vars_ptr->cmn_vars_ptr) != FTM_GSM_BER_STATE_TCH
#ifdef FEATURE_GSM_EGPRS
       && FTM_GSM_BER_DEV_GET_STATE(chain_vars_ptr->cmn_vars_ptr) != FTM_GSM_BER_STATE_GPRS_PDTCH
#endif
     )
  {
...
void ftm_gsm_ber_handle_rx_data_indications( void)
{
  ...
  if ( gsm_curr_chain >= FTM_GSM_MAX_CHAINS )                                                    
  {                                                                                                        
    MSG_1( MSG_SSID_FTM, MSG_LEGACY_ERROR,"ftm_gsm_ber_handle_rx_data_indications() "
                                          "ftm_gsm_ns_active_chain invalid! %d",
           gsm_curr_chain);
    return;                                                                                                
  }
  ...
  if( chain_vars_ptr->cmn_vars_ptr == NULL )
  {
    ERR_FATAL( " ftm_gsm_ber_handle_rx_data_indications() cmn_vars_ptr NULL for active chain %d", 
               gsm_curr_chain, 0, 0 );
    return;
  }
  ...
  while ( (message = q_get( &ftm_gsm_ber_rx_data_q)) != NULL)
  {
    ...
    if ( cmn_vars_ptr->ftm_gsm_ber_status.loopback_type == FTM_GSM_LOOPBACK_TYPE_C)
    {
      ...
      ftm_se_ber_run( &message->msg.rx_data[0], &config);
              ...
              ( cmn_vars_ptr->ftm_gsm_ber_status.loopback_type == FTM_GSM_LOOPBACK_TYPE_A ||
                cmn_vars_ptr->ftm_gsm_ber_status.loopback_type == FTM_GSM_LOOPBACK_TYPE_B))
    {
...
#ifdef FEATURE_GSM_EGPRS
    else if ( ftm_gsm_ber_get_dev_state(gsm_curr_chain) == FTM_GSM_BER_STATE_GPRS_PDTCH)
    {
...
void ftm_gsm_ber_handle_rr_msgs( void *msg)
{
  ...
  {
    MSG_1( MSG_SSID_FTM, MSG_LEGACY_ERROR,
           "ftm_gsm_ber_handle_rr_msgs() ftm_gsm_ns_active_chain invalid! %d",
           gsm_curr_chain);
    return;
  }

  /*obtain gsm chain variables from curr chain*/
  chain_vars_ptr = ftm_gsm_get_chain_vars_ptr( gsm_curr_chain );

  if( chain_vars_ptr == NULL || chain_vars_ptr->cmn_vars_ptr == NULL )
  {
    ERR_FATAL( "ftm_gsm_ber_handle_rr_msgs() chain_vars_ptr/cmn_vars_ptr "
               "NULL for active chain and prx device %d", 
               gsm_curr_chain, 0, 0 );
    return;
  }
  ...
  **************************************************************************/
  ...
  case FTM_START_GSM_MODE_CNF:
    ...
    if ( FTM_GSM_BER_DEV_GET_STATE(cmn_vars_ptr) == FTM_GSM_BER_STATE_STOPPED)
    {

      FTM_GSM_BER_DEV_SET_STATE( cmn_vars_ptr, FTM_GSM_BER_STATE_NULL);
    }
  ...
  case FTM_STOP_GSM_MODE_CNF:
    ...
    FTM_GSM_BER_DEV_SET_STATE( cmn_vars_ptr, FTM_GSM_BER_STATE_STOPPED);

    /* Now set the NS active chain invalid */
    ftm_gsm_ns_active_chain = FTM_GSM_INVALID_CHAIN;

    MSG_1(MSG_SSID_FTM, MSG_LEGACY_HIGH, "FTM GSM active chain is now invalid",0);
  ...
  case FTM_START_IDLE_MODE_CNF:
    ...
    if ( FTM_GSM_BER_DEV_GET_STATE(cmn_vars_ptr) == FTM_GSM_BER_STATE_ACQ_BCCH)
    {

      FTM_GSM_BER_DEV_SET_STATE( cmn_vars_ptr,  FTM_GSM_BER_STATE_IDLE);
    }
  ...
  case FTM_SELECT_SPECIFIC_BCCH_CNF:
    ...
    if ( message->msg.ftm_pseudo_rr_msg.mph_select_specific_bcch_cnf.BCCH_found == TRUE)
    {
      ...
      MSG_3(MSG_SSID_FTM, MSG_LEGACY_HIGH,
            ...
            chain_vars_ptr->prx_dev,
            cmn_vars_ptr->ftm_gsm_ber_status.arfcn.num,
            cmn_vars_ptr->ftm_gsm_ber_status.arfcn.band);


      ftm_gsm_ber_compute_BSIC_from_SCH_block( &cmn_vars_ptr->ftm_gsm_ber_status.network_parms.bsic,
    ...
    if ( FTM_GSM_BER_DEV_GET_STATE(cmn_vars_ptr) == FTM_GSM_BER_STATE_NULL ||
         FTM_GSM_BER_DEV_GET_STATE(cmn_vars_ptr) == FTM_GSM_BER_STATE_ACQ_BCCH ||
         FTM_GSM_BER_DEV_GET_STATE(cmn_vars_ptr) == FTM_GSM_BER_STATE_IDLE
#ifdef FEATURE_GSM_EGPRS
         || FTM_GSM_BER_DEV_GET_STATE(cmn_vars_ptr) == FTM_GSM_BER_STATE_GPRS_IDLE
#endif                 
       )
    {
      if ( message->msg.ftm_pseudo_rr_msg.mph_select_specific_bcch_cnf.BCCH_found == TRUE)
      {
        ...
        FTM_GSM_BER_DEV_SET_STATE( cmn_vars_ptr,  FTM_GSM_BER_STATE_ACQ_BCCH);
      }
      else
      {
        FTM_GSM_BER_DEV_SET_STATE( cmn_vars_ptr,  FTM_GSM_BER_STATE_NULL );
  ...
  case FTM_CHANNEL_ASSIGN_CNF:
    ...
    if ( FTM_GSM_BER_DEV_GET_STATE(cmn_vars_ptr) == FTM_GSM_BER_STATE_IDLE)
    {
      FTM_GSM_BER_DEV_SET_STATE( cmn_vars_ptr,  FTM_GSM_BER_STATE_TCH);
    }

    /* restore the loopback type across handover */

    if ( cmn_vars_ptr->ftm_gsm_ber_status.saved_loopback_type != FTM_GSM_LOOPBACK_TYPE_NULL)
    {

      geran_test_mode_api_do_tch_loopback( (gl1_defs_loopback_type) cmn_vars_ptr->ftm_gsm_ber_status.saved_loopback_type, as_id );    
      cmn_vars_ptr->ftm_gsm_ber_status.loopback_type             = cmn_vars_ptr->ftm_gsm_ber_status.saved_loopback_type; 
      cmn_vars_ptr->ftm_gsm_ber_status.saved_loopback_type       = FTM_GSM_LOOPBACK_TYPE_NULL;                   
    }
  ...
  case FTM_CHANNEL_RELEASE_CNF:
    ...
    if ( FTM_GSM_BER_DEV_GET_STATE(cmn_vars_ptr) == FTM_GSM_BER_STATE_TCH)
    {
      FTM_GSM_BER_DEV_SET_STATE( cmn_vars_ptr,  FTM_GSM_BER_STATE_ACQ_BCCH);
    }
...
void ftm_gsm_ber_handle_l2_msgs( void *msg)
{
  ...
  {
    MSG_1( MSG_SSID_FTM, MSG_LEGACY_ERROR,
           "ftm_gsm_ber_handle_l2_msgs() ftm_gsm_ns_active_chain invalid! %d",
           gsm_curr_chain);
    return;
  }

  /*Obtain GSM chain variable ptr from active chain in NS*/
  chain_vars_ptr = ftm_gsm_get_chain_vars_ptr( gsm_curr_chain );

  if( chain_vars_ptr == NULL )
  {
    ERR_FATAL( "ftm_gsm_ber_handle_l2_msgs() chain_vars_ptr NULL for active chain and prx device %d", 
               gsm_curr_chain, 0, 0 );
    return;
  }
                                                     ...
                                                     ftm_gsm_chains_enum_type gsm_curr_chain)
{
  ...
  {
    MSG_1( MSG_SSID_FTM, MSG_LEGACY_ERROR,
           "ftm_gsm_ber_send_FTM_START_GSM_MODE_REQ() ftm_gsm_ns_active_chain invalid! %d",
           gsm_curr_chain);
    return status;
  }
  ...
  MSG_2( MSG_SSID_FTM, MSG_LEGACY_HIGH,
         "ftm_gsm_ber_send_FTM_START_GSM_MODE_REQ() rx device:%d, tx device:%d",
         chain_vars_ptr->prx_dev, chain_vars_ptr->associated_tx_dev);
  ...
  ftm_gsm_ns_active_chain = gsm_curr_chain;

  MSG_2( MSG_SSID_FTM, MSG_LEGACY_HIGH,
         "FTM GSM active chain is now chain %d, using as_id %d", 
         gsm_curr_chain,as_id);
...
gs_status_T ftm_gsm_ber_send_FTM_STOP_GSM_MODE_REQ( sys_modem_as_id_e_type as_id)
{
  ...
  status = ftm_send_FTM_STOP_GSM_MODE_REQ( as_id);
                                                     ...
                                                     ftm_gsm_cmn_vars_s* cmn_vars_ptr )
{
  ...
  copy_ber_status.tsc = cmn_vars_ptr->ftm_gsm_ber_status.tsc;
  copy_ber_status.tch_arfcn = cmn_vars_ptr->ftm_gsm_ber_status.tch_arfcn;
  copy_ber_status.tch_slot = (geran_test_mode_api_ftm_gsm_ber_tch_slot_type) cmn_vars_ptr->ftm_gsm_ber_status.tch_slot;
  copy_ber_status.tch_mode = (geran_test_mode_api_ftm_gsm_ber_tch_mode_type) cmn_vars_ptr->ftm_gsm_ber_status.tch_mode;
  copy_ber_status.tch_ordered_power_level = cmn_vars_ptr->ftm_gsm_ber_status.tch_ordered_power_level;
#ifdef FEATURE_DUAL_SIM 
                                                      ...
                                                      ftm_gsm_cmn_vars_s* cmn_vars_ptr)
{
  ...
  copy_ber_status.arfcn = cmn_vars_ptr->ftm_gsm_ber_status.arfcn;
  copy_ber_status.network_parms.bsic = cmn_vars_ptr->ftm_gsm_ber_status.network_parms.bsic;
#ifdef FEATURE_DUAL_SIM 
                                       ...
                                       ftm_gsm_ber_pkt_type *cmd_ptr)
{
  ...
  gs_status_T                                   status = GS_FAILURE;
  uint16                                        cmd_fail = FTM_GSM_BER_SYNC_STATUS_SUCCESS;
  boolean                                       Rxd_support = TRUE;
  sys_modem_as_id_e_type             as_id; 
  ...
  /*Obtain the associated TX and RX(DRX) devices from RFC*/
  ...
  {
    FTM_MSG_1( FTM_ERROR, 
               "ftm_gsm_ber_dispatch() chain_vars_ptr or cmn_vars_ptr is NULL "
               "for dispatch device:%d", 
               rx_device);
    return rsp_pkt;
  }

  if ( ftm_gsm_ns_active_chain != chain_vars_ptr->gsm_chain         && 
       cmd_ptr->ftm_cmd_header.cmd_id != FTM_GSM_BER_START_GSM_MODE_REQ )
  {
    FTM_MSG_3( FTM_ERROR, 
              "New chain (%d) and PRX device %d active for FTM GSM BER "
              "without doing START_GSM_MODE_REQ!, command id : %d", 
              chain_vars_ptr->gsm_chain,
              chain_vars_ptr->prx_dev,
  ...
  /*Assign cmn var ptr to local variable*/
  cmn_vars_ptr = chain_vars_ptr->cmn_vars_ptr;

  /*Obtain geran as id from ftm sub id*/
  as_id = ftm_gsm_ctl_map_sub_id_to_as_id( chain_vars_ptr->sub_id );

  if ( ftm_gsm_device_is_tx_supported(chain_vars_ptr->associated_tx_dev)== FALSE ) 
  {
    FTM_MSG_2( FTM_ERROR, 
               "ftm_gsm_ber_dispatch() Non-Sig can't be performed:"
               "TX device %d not supported for ftm chain %d",
               chain_vars_ptr->associated_tx_dev, chain_vars_ptr->gsm_chain );
    return rsp_pkt;
  }

  /*Update GL1 device structure with RX and TX device*/
  ...
  { 
    /*Update GL1 with correct RXD device and DRX band support*/
  ...
  {
    /* Update the GL1 device structue to invalid 
      device as DRX device not supported */
    devices.drx = RFM_INVALID_DEVICE;
  }
  ...
  for (rfgsm_com_band = 0; rfgsm_com_band < RFGSM_MAX_UNIQUE_RFCOM_BANDS; rfgsm_com_band++) 
  {
    ...
    if (rxd_dev_band_mask & bit_mask) 
    {
       ...
       FTM_MSG_2( FTM_MED,
                  "ftm_gsm_ber_dispatch() GSM RX Diversity "
                  "is supported for drx device:%d on band %d", 
                   chain_vars_ptr->associated_drx_dev, rfgsm_com_band);
    }
    else
    {
      ...
      FTM_MSG_2( FTM_MED, 
                 "ftm_gsm_ber_dispatch() GSM RX Diversity "
                  "is not supported for drx device:%d on band %d", 
                  chain_vars_ptr->associated_drx_dev, rfgsm_com_band);
    }
  }
  devices.ftm_rxchain = GSM_NO_ACTION;
  /*Geran api to update ftm devices for NS*/
  geran_test_mode_api_set_ftm_devices(devices,as_id);

#if defined(FEATURE_GSM_EGPRS) && defined(FEATURE_FTM_EGPRS_BER) 
  if ( cmd_ptr->ftm_cmd_header.cmd_id >= FTM_EGPRS_BER_CMD_BASE)
  {
    rsp_pkt = ftm_egprs_ber_dispatch( (ftm_egprs_ber_pkt_type *) cmd_ptr, 
                                      as_id,
                                      chain_vars_ptr->gsm_chain ); /*lint !e740*/ 
  }
  else
#endif
  {
    ...
    */
      ...
      ftm_rfmode_enter( rx_device, FTM_STATE_GSM_NS );
      ...
      cmn_vars_ptr->ftm_gsm_ber_status.loopback_type       = FTM_GSM_LOOPBACK_TYPE_NULL; 
      cmn_vars_ptr->ftm_gsm_ber_status.saved_loopback_type = FTM_GSM_LOOPBACK_TYPE_NULL; 

      /* Now, activate GSM L1 using RR primitive */

      status = ftm_gsm_ber_send_FTM_START_GSM_MODE_REQ(as_id, chain_vars_ptr->gsm_chain);
    ...
    case FTM_GSM_BER_SELECT_SPECIFIC_BCCH_REQ:
      ...
      cmn_vars_ptr->ftm_gsm_ber_status.arfcn.num  = select_specific_bcch_req->channel;
      cmn_vars_ptr->ftm_gsm_ber_status.arfcn.band = ( sys_band_T) select_specific_bcch_req->band;

      status = ftm_gsm_ber_send_FTM_SELECT_SPECIFIC_BCCH_REQ( as_id,
                                                              cmn_vars_ptr->ftm_gsm_ber_status.arfcn.num,
                                                              cmn_vars_ptr->ftm_gsm_ber_status.arfcn.band );
    ...
    case FTM_GSM_BER_START_IDLE_MODE_REQ:
      ...
      status = ftm_gsm_ber_send_FTM_START_IDLE_REQ( as_id );
    ...
    case FTM_GSM_BER_CONFIG_LOOPBACK_TYPE_REQ:
      ...
      if ( loopback_type <  FTM_GSM_NUM_LOOPBACK_TYPES )
      {
        ...
        if ( loopback_type != cmn_vars_ptr->ftm_gsm_ber_status.loopback_type)
        {
          ...
          cmn_vars_ptr->ftm_gsm_ber_status.loopback_type = loopback_type;
        }
    ...
    case FTM_GSM_BER_CHANNEL_ASSIGN_REQ:
      ...
      if ( FTM_GSM_BER_DEV_GET_STATE(cmn_vars_ptr) != FTM_GSM_BER_STATE_IDLE &&
           FTM_GSM_BER_DEV_GET_STATE(cmn_vars_ptr) != FTM_GSM_BER_STATE_TCH)
      {
      ...
      cmn_vars_ptr->ftm_gsm_ber_status.tch_mode                = (ftm_gsm_ber_tch_mode_type) channel_assign_req->tch_mode;
      cmn_vars_ptr->ftm_gsm_ber_status.tch_slot                = (ftm_gsm_ber_tch_slot_type) channel_assign_req->slot;
      cmn_vars_ptr->ftm_gsm_ber_status.tsc                     = 0;                  // default TSC
      cmn_vars_ptr->ftm_gsm_ber_status.tch_arfcn               = cmn_vars_ptr->ftm_gsm_ber_status.arfcn;   // use bcch arfcn
      cmn_vars_ptr->ftm_gsm_ber_status.tch_ordered_power_level = 15;                 // default PCL
      ...
      */
      if ( FTM_GSM_BER_DEV_GET_STATE(cmn_vars_ptr) == FTM_GSM_BER_STATE_TCH && 
           cmn_vars_ptr->ftm_gsm_ber_status.loopback_type != FTM_GSM_LOOPBACK_TYPE_NULL)
      {
        cmn_vars_ptr->ftm_gsm_ber_status.saved_loopback_type = cmn_vars_ptr->ftm_gsm_ber_status.loopback_type; 
        geran_test_mode_api_do_tch_loopback( (gl1_defs_loopback_type) FTM_GSM_LOOPBACK_TYPE_NULL, as_id );    
        cmn_vars_ptr->ftm_gsm_ber_status.loopback_type       = FTM_GSM_LOOPBACK_TYPE_NULL;                   
      }
      status = ftm_gsm_ber_send_FTM_CHANNEL_ASSIGN_REQ( as_id,
                                                        cmn_vars_ptr );
    ...
    case FTM_GSM_BER_CHANNEL_ASSIGN_V2_REQ:
      {
        ...
        cmn_vars_ptr->ftm_gsm_ber_status.tsc                     = channel_assign_v2_req->tsc;
        cmn_vars_ptr->ftm_gsm_ber_status.tch_arfcn.num           = channel_assign_v2_req->channel;
        cmn_vars_ptr->ftm_gsm_ber_status.tch_arfcn.band          = (sys_band_T)channel_assign_v2_req->band;
        cmn_vars_ptr->ftm_gsm_ber_status.tch_mode                = (ftm_gsm_ber_tch_mode_type) channel_assign_v2_req->tch_mode;
        cmn_vars_ptr->ftm_gsm_ber_status.tch_slot                = (ftm_gsm_ber_tch_slot_type) channel_assign_v2_req->slot;
        cmn_vars_ptr->ftm_gsm_ber_status.tch_ordered_power_level = channel_assign_v2_req->power_level;
    ...
    case FTM_GSM_BER_CHANNEL_RELEASE_REQ:
      ...
      if ( cmn_vars_ptr->ftm_gsm_ber_status.loopback_type != FTM_GSM_LOOPBACK_TYPE_NULL)
      {
        geran_test_mode_api_do_tch_loopback( (gl1_defs_loopback_type) FTM_GSM_LOOPBACK_TYPE_NULL, as_id );
      }


      status = ftm_gsm_ber_send_FTM_CHANNEL_RELEASE_REQ( as_id,
                                                         cmn_vars_ptr );
    ...
    case FTM_GSM_BER_STOP_GSM_MODE_REQ:
      ...
      if ( FTM_GSM_BER_DEV_GET_STATE(cmn_vars_ptr) == FTM_GSM_BER_STATE_STOPPED)
      {
         FTM_MSG_1( FTM_MED,"ftm_gsm_ber_dispatch() "
                    "GSM_BER_STATE is already STOPPED %d", 
                    FTM_GSM_BER_STATE_STOPPED );
        status =  GS_SUCCESS;
        break;
      }
      else if ( cmn_vars_ptr->ftm_gsm_ber_status.loopback_type != FTM_GSM_LOOPBACK_TYPE_NULL)
      {
        /* clear the loopback on stop GSM */
        geran_test_mode_api_do_tch_loopback( (gl1_defs_loopback_type) FTM_GSM_LOOPBACK_TYPE_NULL, as_id );
      }
      ...
      status = ftm_gsm_ber_send_FTM_STOP_GSM_MODE_REQ( as_id);
      FTM_MSG_1( FTM_MED,"ftm_gsm_ber_dispatch() "
                 "ftm_gsm_ber_send_FTM_STOP_GSM_MODE_REQ status %d",
                 status);
      FTM_GSM_BER_DEV_SET_STATE( cmn_vars_ptr,  FTM_GSM_BER_STATE_STOPPED);
   ...
   ****************************************************************************/
    ...
    case FTM_LOG_GSM_BER_RX_LEVEL_REQ:
      if ( FTM_GSM_BER_DEV_GET_STATE(cmn_vars_ptr)   != FTM_GSM_BER_STATE_TCH  &&
           ftm_egprs_ber_get_dev_state(chain_vars_ptr->gsm_chain) != FTM_EGPRS_BER_STATE_UL_TBF_ESTABLISHED)
      {
        status = GS_FAILURE;
    ...
    break;
      ...
      /*************************************************************************
      *            FTM_GSM_NS_CHAIN_SELECT                           *
      *                                                                        *
      **************************************************************************/
       /* FTM receives Rx chain param from tools 
        - Rx chain param 0, PRx only : GL1 will force only PRx chain here
        - Rx chain param 1, DRx only : GL1 will force only DRx chain here
        - Rx chain param 2, PRx + DRx: GL1 will force PRx and DRx both
        - Rx chain param 3, no action : This is the default option as in all 
          existing trees. We don't send any command to GL1 in this case and GL1 
          ensures existing non-override functionality remains.*/

    case FTM_GSM_NS_CHAIN_SELECT:
    {
      ftm_gsm_ber_ns_chain_header_type *ftm_ns_chain_request = (ftm_gsm_ber_ns_chain_header_type*)cmd_ptr;
      status   = GS_SUCCESS;

      switch(ftm_ns_chain_request->ftm_ns_rx_chain_param)	  
      {
        case GSM_ONLY_PRX_ENABLED:
        {
          devices.ftm_rxchain = ftm_ns_chain_request->ftm_ns_rx_chain_param;
          geran_test_mode_api_set_chain_select(devices, as_id);
          break;
        }
        ...
        case GSM_BOTH_PRX_DRX_ENABLED:
        {
          for (rfgsm_com_band = 0; rfgsm_com_band < RFGSM_MAX_UNIQUE_RFCOM_BANDS; rfgsm_com_band++) 
          {
            /*Query RX Diversity support for all bands*/
            Rxd_support|=devices.drx_band_support[rfgsm_com_band];
          }
          ...
          {
            status = GS_FAILURE;
            cmd_fail = FTM_GSM_BER_TRM_NO_CLIENT;
          }
          break;
        }
        case GSM_NO_ACTION:
        ...
        default:
        MSG_1(MSG_SSID_FTM, MSG_LEGACY_HIGH,"Invalid FTM_GSM_NS_CHAIN_SELECT param %d", 
               ftm_ns_chain_request->ftm_ns_rx_chain_param);
        status = GS_FAILURE;
        break;
     }

     MSG_3(MSG_SSID_FTM, MSG_LEGACY_HIGH,"FTM_GSM_NS_CHAIN_SELECT, DRx support: %d, Status: %d, Chain_select : %d ", 
                cmd_fail,
                status,
                ftm_ns_chain_request->ftm_ns_rx_chain_param);
    ...
    if ( status != GS_SUCCESS)
    {
      MSG_2(MSG_SSID_FTM, MSG_LEGACY_HIGH,"FTM GSM BER, Command Failure: %d, State: %d", 
            cmd_ptr->ftm_cmd_header.cmd_id,
           FTM_GSM_BER_DEV_GET_STATE(cmn_vars_ptr));
    }
    else
    {
      cmd_fail = FALSE;
...
FUNCTION ftm_gsm_ber_is_start_mode_req

DESCRIPTION
  This function checks if ftm gsm ber command received is
  FTM_GSM_BER_START_GSM_MODE_REQ
 
PARAMETERS
cmd_ptr  -> ftm cmd pointer
 
DEPENDENCIES
  None.

RETURN VALUE
  TRUE or FALSE- Returns True if cmd id received is
                 FTM_GSM_BER_START_GSM_MODE_REQ else False

SIDE EFFECTS
  None.

===========================================================================*/
boolean ftm_gsm_ber_is_start_mode_req(ftm_gsm_ber_pkt_type *cmd_ptr)
{
   if (cmd_ptr->ftm_cmd_header.cmd_id == FTM_GSM_BER_START_GSM_MODE_REQ) 
   {
      return TRUE;
   }
   else
