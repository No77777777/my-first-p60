...
$Header: //components/rel/rftech_gsm.mpss/3.11/rf/nv/src/rfgsm_nv_cmn.c#1 $
$DateTime: 2021/03/09 05:10:17 $ 
$Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
02/37/17   bp      Rx freq comp interpolation changes
07/01/16   hkm     Revert the previous change
06/30/16   hkm     Add an interim HACK in code to pass GCF 21.1 Monitor measurement test cases.
05/24/16   ase     Removing additional F3s for non-supported device configuration
04/18/16   kpa     Add debug NV for excessive F3 logging reduction
03/15/16   dsm     AsDiv with max power back off for SAR consideration
03/11/16   par     Fix bank mask usage
02/29/16   dsm     Added Checks to support Reload GSM NVs during RF cal
11/10/15   tws     Add FBRX VSWR support
10/09/15   zc      Asdiv elna NV loading fix
09/24/15   zc      Support different gains along different ASDIV sw positions
08/18/15   ec      Integrate SAWless into split Rx\Tx
07/27/15   sc      Deprecate alternate path
07/27/15   sw      Update the lowest_supported_band api to device based
07/15/15   cpm     FR27546: MTPL reported for the top/bottom antennas
07/14/15   sw      Check for NULL pointer
...
01/23/14   ec      Add index and bitmask definitions for debug NV as well as new API definition
                   for accessing debug NV
01/19/15   sw      Introduce a new GSM NV (debug and profiling)
...
#include "rfm_antenna_types.h" /* For RFM_ANTENNA_X definitions */
...
/*!
  @brief
  The static maximum number of Rx data structures that can be allocated.
 
  @details
  Currently support 6 Rx chains (RF_GSM_RX_NV_PATH_NUM) and 4 bands
  (RF_GSM_NV_BAND_NUM)
*/
#define RF_GSM_RX_NV_PATH_NUM RF_GSM_NV_PATH_NUM
#define RF_GSM_RX_NV_MAX_STRUCTURES (RF_GSM_RX_NV_PATH_NUM * RF_GSM_NV_BAND_NUM)

/*!
  @brief
  The static maximum number of Tx data structures that can be allocated.
 
  @details
  Currently support 2 Tx chains (RF_GSM_TX_NV_PATH_NUM) and 4 bands
  (RF_GSM_NV_BAND_NUM)
*/
#define RF_GSM_TX_NV_PATH_NUM 2
#define RF_GSM_TX_NV_MAX_STRUCTURES (RF_GSM_TX_NV_PATH_NUM * RF_GSM_NV_BAND_NUM)

#define RFGSM_NV_NUM_PA_RANGES 8


/* Tables to store fequency compensated Rx swt pts*/
 rfgsm_nv_rx_switch_type gsm_850_rx_switch_dBm[RF_GSM_NV_PATH_NUM][ASDIV_POSITION_MAX];
 rfgsm_nv_rx_switch_type gsm_900_rx_switch_dBm[RF_GSM_NV_PATH_NUM][ASDIV_POSITION_MAX];
 rfgsm_nv_rx_switch_type gsm_1800_rx_switch_dBm[RF_GSM_NV_PATH_NUM][ASDIV_POSITION_MAX];
 rfgsm_nv_rx_switch_type gsm_1900_rx_switch_dBm[RF_GSM_NV_PATH_NUM][ASDIV_POSITION_MAX];

/* Tables to store Rx enh swt pts*/
 rfgsm_nv_rx_switch_type gsm_850_enh_rx_switch_dBm[RF_GSM_NV_PATH_NUM][ASDIV_POSITION_MAX];
 rfgsm_nv_rx_switch_type gsm_900_enh_rx_switch_dBm[RF_GSM_NV_PATH_NUM][ASDIV_POSITION_MAX];
 rfgsm_nv_rx_switch_type gsm_1800_enh_rx_switch_dBm[RF_GSM_NV_PATH_NUM][ASDIV_POSITION_MAX];
 rfgsm_nv_rx_switch_type gsm_1900_enh_rx_switch_dBm[RF_GSM_NV_PATH_NUM][ASDIV_POSITION_MAX];

/* Tables to store frequency compensated Rx swt pts for High Linearity mode on SAWless targets */
 rfgsm_nv_rx_switch_type gsm_850_hl_rx_switch_dBm[RF_GSM_NV_PATH_NUM][ASDIV_POSITION_MAX];
 rfgsm_nv_rx_switch_type gsm_900_hl_rx_switch_dBm[RF_GSM_NV_PATH_NUM][ASDIV_POSITION_MAX];
 rfgsm_nv_rx_switch_type gsm_1800_hl_rx_switch_dBm[RF_GSM_NV_PATH_NUM][ASDIV_POSITION_MAX];
 rfgsm_nv_rx_switch_type gsm_1900_hl_rx_switch_dBm[RF_GSM_NV_PATH_NUM][ASDIV_POSITION_MAX];


 rfgsm_nv_rx_switch_storage rfgsm_nv_swpt_type_table[RF_GSM_MAX_RX_SWPT] = 
 {
   {
      gsm_850_rx_switch_dBm,
      gsm_900_rx_switch_dBm,
      gsm_1800_rx_switch_dBm,
      gsm_1900_rx_switch_dBm,
   },
   {
      gsm_850_enh_rx_switch_dBm,
      gsm_900_enh_rx_switch_dBm,
      gsm_1800_enh_rx_switch_dBm,
      gsm_1900_enh_rx_switch_dBm
   },
   {
      gsm_850_hl_rx_switch_dBm,
      gsm_900_hl_rx_switch_dBm,
      gsm_1800_hl_rx_switch_dBm,
      gsm_1900_hl_rx_switch_dBm
   }
 };
...
typedef struct
{
 /* indicates whether memory for Rx or TX related Static NVs data allocated*/
 boolean static_nv_alloc_valid;

 /*indicates whether memory for all the Rx or Tx NV data (i.e. static & calibrated) is allocated or not*/
 boolean nv_data_alloc_valid;

}
nv_alloc_type;

/*This structure contains Rx NV memory allocation related information for NV container and NV band*/
static nv_alloc_type rx_nv_alloc_info[RF_GSM_NV_PATH_MAX][RFGSM_NV_MAX_SUPPORTED];

/*This structure contains Tx NV memory allocation related information for NV container and NV band*/
static nv_alloc_type tx_nv_alloc_info[RF_GSM_NV_PATH_MAX][RFGSM_NV_MAX_SUPPORTED];
...
static const rfgsm_debug_nv_mask_info_type rfgsm_debug_nv_masks[RFGSM_DEBUG_NV_MAX_SUPPORTED] = 
{
  ...
  { RFGSM_SPARE_NV_OVERRIDE_IDX, NV_BIT6_MASK },        /* RFGSM_DEBUG_NV_FBRX_VSWR_ENABLE  */
  { RFGSM_SPARE_NV_OVERRIDE_IDX, NV_BIT7_MASK },        /* RFGSM_DEBUG_NV_FBRX_VSWR_ENABLE_FBRX_EVERY_BURST  */
  { RFGSM_SPARE_NV_OVERRIDE_IDX, NV_BIT8_MASK },        /* RFGSM_DEBUG_NV_FBRX_VSWR_DISABLE_COUPLER_SWITCH  */
  ...
  { RFGSM_RESERVED_TIME_PROF_NV_IDX, NV_BIT1_MASK },    /* RFGSM_DEBUG_NV_WAKEUP_TIME_PROFILE_ENABLE */
  { RFGSM_RESERVED_TIME_PROF_NV_IDX, NV_BIT2_MASK },    /* RFGSM_DEBUG_NV_SLEEP_TIME_PROFILE_ENABLE */
  { RFGSM_RESERVED_TIME_PROF_NV_IDX, NV_BIT3_MASK },    /* RFGSM_DEBUG_NV_RX_BURST_PROFILE_ENABLE */
  { RFGSM_RESERVED_TIME_PROF_NV_IDX, NV_BIT4_MASK },    /* RFGSM_DEBUG_NV_TX_BURST_PROFILE_ENABLE */
};
...
typedef struct 
{
 ...
 /*!
  @brief
  Pointer to the GSM Rx configuration structure for this index entry.
  */
  rfgsm_nv_band_rx_data_type *rfgsm_nv_band_rx_data_buffer_ptr; 
 
 /*!
  @brief
  Pointer to the GSM Tx configuration structure for this index entry.
  */
  rfgsm_nv_band_tx_data_type *rfgsm_nv_band_tx_data_buffer_ptr; 
...
static struct
{
  ...
  */
  gsm_rx_static_nv_type* rx_static_nv_data_buffers[RF_GSM_RX_NV_MAX_STRUCTURES];
  gsm_tx_static_nv_type* tx_static_nv_data_buffers[RF_GSM_TX_NV_MAX_STRUCTURES];
...
/*!
  ...
  rfgsm_nv_data[chain].band_index[band].rf_gsm_nv_rx_data_buffer_ptr->rx_static_nv_data_ptr
  rfgsm_nv_data[chain].band_index[band].rf_gsm_nv_tx_data_buffer_ptr->tx_static_nv_data_ptr
...
static struct
{
  /*!
    @brief
    The current number of Rx structures that have been allocated.

    @details
    This item starts at 0 during initialization and is incremented
    every time an Rx buffer is requested.  If allocated becomes equal
    to RF_GSM_RX_NV_MAX_STRUCTURES, NV memory is exhausted.
  */
  int allocated_rx_buffers;

  /*!
    @brief
    The current number of Tx structures that have been allocated.

    @details
    This item starts at 0 during initialization and is incremented
    every time a Tx buffer is requested.  If allocated becomes equal
    to RF_GSM_TX_NV_MAX_STRUCTURES, NV memory is exhausted.
  */
  int allocated_tx_buffers;

  /*!
    @brief
    An array of Rx configuration structures that may be returned
    by the allocator.

    @details
    The buffer indexed by the allocated structure member will
    be the next buffer returned by the allocator, as long as
    allocated is less than RF_GSM_RX_NV_MAX_STRUCTURES.
  */
  rfgsm_nv_band_rx_data_type *rx_buffers[RF_GSM_RX_NV_MAX_STRUCTURES];

  /*!
    @brief
    An array of Tx configuration structures that may be returned
    by the allocator.
  
    @details
    The buffer indexed by the allocated structure member will
    be the next buffer returned by the allocator, as long as
    allocated is less than RF_GSM_TX_NV_MAX_STRUCTURES.
  */
  rfgsm_nv_band_tx_data_type *tx_buffers[RF_GSM_TX_NV_MAX_STRUCTURES];
...
/*! This RF NV Item Mapping for base Band GSM850, these NVs IDs are used to get 
  ...
  NOTE: as a result of Rx/Tx device split feature, C4 and C5 NV items have been added.
  In legacy, it was known that C1 was the diversity path for C0, and C3 the diversity for C2,
  therefore no separate C1 and C3 Tx NV items were created, instead just pointing to the 
  existing C0 and C2 Tx NV items since they wouldn't be populated anyways.
  Now with the new feature this assumption no longer holds true. However, we can still
  relatively safely assume that the primary RF path would always be followed by the diversity
  RF path in consecutive order and that this would map to an even-odd ordering of NV path, 
  therefore the Tx NV items for the odd NV paths (C1, C3, C5) could still point to (C0, C2, C4) 
  respectively. Of course, if any of these assumptions are broken in the future, then new Tx NV 
  items for each of the NV paths would need to be created and pointed to.
  */
...
/*RF_GSM_NV_PATH_0 */
{
  ...
  /* SAR BACKOFF for both default and switched antenna - variant sar nv item */
   RFNV_GSM_GSM850_SAR_BACKOFF_VAR,
  ...
  /*LNA enh switch points*/
   ...
   RFNV_GSM_C0_GSM850_ASDIV_ELNA_GAIN_DATA
},
/*RF_GSM_NV_PATH_1
...
/*RF_GSM_NV_PATH_2 */
{
  ...
  /*Antenna Tuner CL data*/
   ...
   RFNV_GSM_C2_GSM850_ASDIV_ELNA_GAIN_DATA
...
{
  /*Rx cal*/
   RFNV_GSM_C4_GSM850_RX_CAL_DATA_I,
 	 
  /*Tx cal*/
   {RFNV_GSM_C4_GSM850_TX_CAL_DATA_I, RFNV_GSM_C4_GSM850_TX_CAL_DATA_I},
 
  /*LNA switch points*/
   RFNV_GSM_C4_GSM850_LNA_SWPT_I,
 
  /*TX_RX_ANT_TUNER*/
   RFNV_GSM_C4_GSM850_TX_RX_ANT_TUNER_I,
 
  /* tx_timing_related NVs */
   RFNV_GSM_C4_GSM850_TX_TIMING_I,
 
  /*PA extended switch points*/
   RFNV_GSM_C4_GSM850_EXTENDED_PA_SWPT_I,
 
  /*KV CAL*/
   RFNV_GSM_C4_GSM850_KV_CAL_I,
 
  /*TX_GTR_THRESH*/
   RFNV_GSM_C4_GSM850_TX_GTR_THRESH_I,
 
  /*Polar path delay*/
   RFNV_GSM_C4_GSM850_POLAR_PATH_DELAY_I,
 
  /*VBATT*/
   RFNV_GSM_C4_GSM850_VBATT_I, 
 
  /*ENV GAIN*/
   RFNV_GSM_C4_GSM850_ENV_GAIN_I,
 
  /*AMAM TEMP COMP*/ 
   RFNV_GSM_C4_GSM850_AMAM_TEMP_COMP_I,
 
  /* TEMP COMP DATA*/
   RFNV_GSM_C4_GSM850_TEMP_COMP_I,
 
  /* ENH TEMP COMP DATA*/
   RFNV_GSM_C4_GSM850_ENH_TEMP_COMP_I,
 
  /* LINEAR_TX_GAIN_PARAM*/
   RFNV_GSM_C4_GSM850_LINEAR_TX_GAIN_PARAM_I,
 
  /*MULTISLOT_MAX_TX_PWR*/
   RFNV_GSM_C4_GSM850_MULTISLOT_MAX_TX_PWR_I,
 
  /*POWER LEVELS*/
   RFNV_GSM_C4_GSM850_POWER_LEVELS_I,
 
  /*SMPS_PDM_TBL*/
   RFNV_GSM_C4_GSM850_EXTENDED_SMPS_PDM_TBL_I,
 
  /*SAR BACKOFF*/
   RFNV_GSM_C4_GSM850_SAR_BACKOFF_I, 
 
  /* SAR BACKOFF for both default and switched antenna - variant sar nv item */
   RFNV_GSM_GSM850_SAR_BACKOFF_VAR,
   
  /* PA ramp profile*/
   RFNV_GSM_C4_GSM850_POLAR_RAMP_PROFILE_I,
 
  /* PA range map*/
   RFNV_GSM_C4_GSM850_PA_RANGE_MAP_I,
 
  /*Enhanced APT data */
   RFNV_GSM_C4_GSM850_ENH_APT_I,
 
  /*PA BIAS ICQ data*/
   RFNV_GSM_C4_GSM850_PA_ICQ_BIAS,
 
  /* Therm Read During Calibration */
   RFNV_GSM_C4_GSM850_CAL_TEMP_I,
 
 #ifdef FEATURE_RF_HAS_QTUNER
   /*Antenna Tuner CL data*/
   RFNV_GSM_C4_850_ANT_TUNER_CL_I,
 #endif 
 
   RFNV_GSM_C4_GSM850_COEX_TX_OFFSET_INFO_TBL,
 
   RFNV_GSM_C4_GSM850_COEX_RX_OFFSET_INFO_TBL,  
 
   /*LNA enh switch points*/
   RFNV_GSM_C4_GSM850_LNA_ENH_SWPT_I,
 
   /* Rx switchpoints and cal for HL mode on SAWless targets */
   RFNV_GSM_C4_GSM850_HL_RX_LNA_SWPT_I,
 
   RFNV_GSM_C4_GSM850_HL_RX_CAL_I,

   RFNV_GSM_C4_GSM850_ASDIV_ELNA_GAIN_DATA
  
},
/*RF_GSM_NV_PATH_5*/
{
   /*Rx cal*/
   RFNV_GSM_C5_GSM850_RX_CAL_DATA_I,
     
   /*Tx cal*/
   {RFNV_GSM_C4_GSM850_TX_CAL_DATA_I, RFNV_GSM_C4_GSM850_TX_CAL_DATA_I},
   
   /*LNA switch points*/
   RFNV_GSM_C5_GSM850_LNA_SWPT_I,
   
   /*TX_RX_ANT_TUNER*/
   RFNV_GSM_C5_GSM850_TX_RX_ANT_TUNER_I,
   ...
   /* Rx switchpoints and cal for HL mode on SAWless targets */
   RFNV_GSM_C5_GSM850_HL_RX_LNA_SWPT_I,
   
   RFNV_GSM_C5_GSM850_HL_RX_CAL_I,
...
/*----------------------------------------------------------------------------*/
/*! RF NV Item Mapping for NVs which are common across the bands.
...
rfgsm_nv_cmn_items_map[RF_GSM_NV_PATH_NUM] =
{
  ...
  /*RF_GSM_NV_PATH_4 */
  {
    ...
    RFNV_GSM_C4_PA_ICQ_BIAS_ENABLE,
...
*/
uint64 rfgsm_nv_get_rf_bc_config( rfgsm_nv_path_type nv_path )
{
  uint64 config = 0;
  ...
  {
    config = rfcommon_nv_tbl_ptr->rf_bc_config[nv_path];
  }
  else
  {
    MSG_ERROR("invalid nv_path in rfgsm_nv_get_rf_bc_config(): %d",
              nv_path, 0, 0);
  }
...
/*!
  ...
  @param rx_tx
  Rx or Tx for which the lowest GSM band supported is required

  @return
  GSM band for passed RFM device
*/
rfcom_gsm_band_type rfgsm_nv_get_lowest_supported_band( rfm_device_enum_type rfm_dev, rfgsm_nv_rxtx_enum_type rx_tx )
{
  uint8 band_mask;
  ...
  {
    MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR,"rfgsm_nv_get_lowest_supported_band: rfgsm_core_handle_ptr is NULL!", 0);
    return RFCOM_BAND_GSM850;
  }

  /* Check if device supports any GSM bands */
  if ( rx_tx == RFGSM_NV_RX )
  {
    band_mask = rfgsm_core_handle_ptr->device_capability.rx_supported_band_mask;
  }
  else if ( rx_tx == RFGSM_NV_TX ) 
  {
    band_mask = rfgsm_core_handle_ptr->device_capability.tx_supported_band_mask;
  }
  else /* Invalid type */
  {
    MSG_2(MSG_SSID_RF, MSG_LEGACY_ERROR,"rfgsm_nv_get_lowest_supported_band: invalid rx_tx type %d for rfm device =%d", rx_tx, rfm_dev);
    return RFCOM_BAND_GSM850;
  }

  if ( band_mask ) 
  {
    if( band_mask & (1<<RFCOM_BAND_GSM850) )
    {
      first_nv_band_supported = RFCOM_BAND_GSM850;
    }
    else if( band_mask & (1<<RFCOM_BAND_GSM900) )
    {
      first_nv_band_supported = RFCOM_BAND_GSM900;
    }
    else if( band_mask & (1<<RFCOM_BAND_GSM1800) )
    {
      first_nv_band_supported = RFCOM_BAND_GSM1800;
    }
    else if ( band_mask & (1<<RFCOM_BAND_GSM1900) )
    {
      first_nv_band_supported = RFCOM_BAND_GSM1900;
    }
    else
    {
      MSG_2(MSG_SSID_RF, MSG_LEGACY_ERROR,"rfgsm_nv_get_lowest_supported_band: band_mask %d device %d does not support any GSM bands", band_mask, rfm_dev);
      return RFCOM_BAND_GSM850;
    }
  }
  else
  {
    MSG_3(MSG_SSID_RF, MSG_LEGACY_ERROR,"rfgsm_nv_get_lowest_supported_band: band_mask %d not set for rx_tx type %d rfm device =%d", band_mask, rx_tx, rfm_dev);
    return RFCOM_BAND_GSM850;
  }
...
RETURN VALUE
  Returns pointer to void to be cast to Rx or Tx GSM RF NV table.

SIDE EFFECTS
  None

===========================================================================*/
void *rfgsm_nv_get_tbl_ptr
( 
...
) 
{
  ...
  {
    nv_data_ptr = ((rx_tx == RFGSM_NV_RX) ? ((void *)rfgsm_nv_data[nv_path].rfgsm_nv_band_node[nv_band].rfgsm_nv_band_rx_data_buffer_ptr) : ((void *)rfgsm_nv_data[nv_path].rfgsm_nv_band_node[nv_band].rfgsm_nv_band_tx_data_buffer_ptr));
...
FUNCTION rfgsm_nv_get_rx_switch_ptr                                 

DESCRIPTION
  Generic function for each freq comp switch type
...
rfgsm_nv_rx_switch_type *rfgsm_nv_get_rx_switch_ptr
(
  rfm_device_enum_type rfm_dev,
  rfcom_gsm_band_type band,
  uint8 asdiv_pos,
  rfgsm_nv_rx_switchpoint_type swpt_type
)
{
  ...
  if(swpt_type < RF_GSM_MAX_RX_SWPT)
  {
    rx_switch_ptr = &rfgsm_nv_swpt_type_table[swpt_type].gsm_900_rx_switch[nv_path][asdiv_pos];
  }
  else
  {
    ERR_FATAL("Invalid Switch Point Type = %d", swpt_type, 0, 0);
  }

  switch (nv_band)
  {
    case RFGSM_NV_850:
      rx_switch_ptr = &rfgsm_nv_swpt_type_table[swpt_type].gsm_850_rx_switch[nv_path][asdiv_pos];;
      break;
    ...
    case RFGSM_NV_1800:
      rx_switch_ptr = &rfgsm_nv_swpt_type_table[swpt_type].gsm_1800_rx_switch[nv_path][asdiv_pos];;
      break;
    
    case RFGSM_NV_1900:
      rx_switch_ptr = &rfgsm_nv_swpt_type_table[swpt_type].gsm_1900_rx_switch[nv_path][asdiv_pos];;
      break;
...
uint8 rfgsm_nv_get_closest_arfcn_index( rfgsm_nv_band_rx_data_type *rfgsm_rx_nv_tbl, uint16 arfcn)
{
  /* --------------------------------------------------------------------- */
  ...
  */
  ...
  arfcn_index = arfcn_index - 1;
    ...
    if (arfcn_index > rfgsm_rx_nv_tbl->rx_cal_nv_data.rx_cal_chan_size)
  {
      MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, " arfcn index is out of range: %d",arfcn_index);
	  arfcn_index = rfgsm_rx_nv_tbl->rx_cal_nv_data.rx_cal_chan_size - 1; 
    }
  }
  else
  {
    MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, "NV pointer is NULL %X",rfgsm_rx_nv_tbl);   
  }
  return(arfcn_index);
...
(
  rfm_device_enum_type rfm_dev,
  rfgsm_nv_band_rx_data_type *rfgsm_rx_nv_tbl,
  rfcom_gsm_band_type band,
  uint16 arfcn,
  uint8 arfcn_index,
  rfnv_gsm_lna_state_type lna_state
)
{
  int16 rxgain_low = 0;
  int16 rxgain_high = 0;
  ...
  if (NULL == rfgsm_rx_nv_tbl)
  {
    MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, "rfgsm_rx_nv_tbl is NULL. Set RxGain to %d", rxgain);
    return (rxgain);
  }  
  ...
  if ( rfgsm_core_get_band_sawless_status(rfm_dev, rfgsm_core_handle_ptr->band) == TRUE)
  {
    ...
    if ( rfgsm_core_is_arfcn_in_high_lin(rfgsm_core_handle_ptr, 
  ...
  /* Extract lower and upper channels as well as Rx calibration gain from NV */
  if (rfgsm_rx_nv_tbl->rx_cal_nv_data.rx_cal_chan_list[arfcn_index] < arfcn)
  {
    /* The channel to be interpolated is larger than the closest arfcn index. Interpolation
       will happen on the right side of the arfcn_closest_cal_index bin.  Therefore, check to make sure
       that arfcn_closest_cal_index is not at the max bin already.
    */
		
    if (arfcn_index < (rfgsm_rx_nv_tbl->rx_cal_nv_data.rx_cal_chan_size - 1))
  {
      arfcn_low   = rfgsm_rx_nv_tbl->rx_cal_nv_data.rx_cal_chan_list[arfcn_index];
      rxgain_low  = rfgsm_rx_nv_tbl->rx_cal_nv_data.rx_freq_comp_data[lna_state].freq_comp_data[arfcn_index];
      arfcn_high  = rfgsm_rx_nv_tbl->rx_cal_nv_data.rx_cal_chan_list[arfcn_index + 1];
      rxgain_high = rfgsm_rx_nv_tbl->rx_cal_nv_data.rx_freq_comp_data[lna_state].freq_comp_data[arfcn_index + 1];
  }
  else
  {
      arfcn_index = rfgsm_rx_nv_tbl->rx_cal_nv_data.rx_cal_chan_size - 1;
      arfcn_low   = rfgsm_rx_nv_tbl->rx_cal_nv_data.rx_cal_chan_list[arfcn_index];
      rxgain_low  = rfgsm_rx_nv_tbl->rx_cal_nv_data.rx_freq_comp_data[lna_state].freq_comp_data[arfcn_index];
      arfcn_high  = arfcn_low;
      rxgain_high = rxgain_low;
    } 
  }
  else if (rfgsm_rx_nv_tbl->rx_cal_nv_data.rx_cal_chan_list[arfcn_index] > arfcn)
  {
    /* The channel to be interpolated is less than the closest arfcn index.	Interpolation
       will happen on the left side of the arfcn_closest_cal_index bin.	Therefore, check to make sure
       that arfcn_closest_cal_index is not at the min bin already.
    */
    if (0 != arfcn_index)
    {
      arfcn_low	  = rfgsm_rx_nv_tbl->rx_cal_nv_data.rx_cal_chan_list[arfcn_index - 1];
      rxgain_low  = rfgsm_rx_nv_tbl->rx_cal_nv_data.rx_freq_comp_data[lna_state].freq_comp_data[arfcn_index - 1];
      arfcn_high  = rfgsm_rx_nv_tbl->rx_cal_nv_data.rx_cal_chan_list[arfcn_index];
      rxgain_high = rfgsm_rx_nv_tbl->rx_cal_nv_data.rx_freq_comp_data[lna_state].freq_comp_data[arfcn_index];
    }
    else
    {
      arfcn_low	   = rfgsm_rx_nv_tbl->rx_cal_nv_data.rx_cal_chan_list[arfcn_index];
  ...
  /* Interpolate the Rx gain based on the DL ARFCN */
  if (arfcn_low != arfcn_high)
  {
    /* The DL ARFCN falls between two calibration channels. Interpolate between these two
       calibrated channels to get the DL ARFCN frequency compensation.*/
    rxgain = rxgain_low + ((rxgain_high - rxgain_low) * (arfcn - arfcn_low)) / (arfcn_high - arfcn_low);
  }
  else
  {
    /* The DL ARFCN is the same as the calibrated ARFCN. Not interpolation is needed */
    rxgain = rxgain_low;
  }
...
void rfgsm_nv_precompute_rx_switch_points_band
(
  rfm_device_enum_type rfm_dev, 
  rfcom_gsm_band_type band,
  rfgsm_nv_rx_switchpoint_type swpt_type    
)
{
  ...
  rfgsm_nv_band_rx_data_type *rf_gsm_nv_data_buffer_ptr = NULL;
  rfgsm_nv_rx_switch_type *rx_switch_ptr = NULL;
  ...
  uint8 asdiv_pos = ASDIV_POSITION_0;
  rfnv_gsm_rx_elna_asdiv_type * rfgsm_nv_rx_elna_asdiv_info = (rfnv_gsm_rx_elna_asdiv_type *)rfgsm_nv_get_elna_asdiv_info(rfm_dev, band, RFGSM_NV_RX);

  /* get NV data Buffer ptr for given NV path and band*/
  rf_gsm_nv_data_buffer_ptr = (rfgsm_nv_band_rx_data_type *)rfgsm_nv_get_tbl_ptr(rfm_dev, band, RFGSM_NV_RX);
  ...
  if(swpt_type == RF_GSM_HL_RX_SWPT)
  {
  ...
  else if(swpt_type == RF_GSM_FREQ_COMP_RX_SWPT)
  {
  ...
  else if(swpt_type == RF_GSM_ENH_RX_SWPT)
  {
    switchpoints = rf_gsm_nv_data_buffer_ptr->rx_static_nv_data_ptr->lna_enh_switch_point;;
    rx_cal_data = rf_gsm_nv_data_buffer_ptr->rx_cal_nv_data;
  }
  ...
  for(asdiv_pos = ASDIV_POSITION_0; asdiv_pos < ASDIV_POSITION_MAX; asdiv_pos++)
  {
  
    rx_switch_ptr = rfgsm_nv_get_rx_switch_ptr(rfm_dev, band, asdiv_pos, swpt_type);
    ...
    for (i=0; i< RFCOM_GSM_NUM_RX_CAL_CHAN; i++)
    {
      ...
      if(rfgsm_nv_rx_elna_asdiv_info != NULL)
      {
        gr1_max_dBm += rfgsm_nv_rx_elna_asdiv_info->rx_elna_asdiv_switch_point_offsets[asdiv_pos].switch_point_offset[RFNV_GSM_LNA_STATE_1].rise_offset;
        gr2_min_dBm += rfgsm_nv_rx_elna_asdiv_info->rx_elna_asdiv_switch_point_offsets[asdiv_pos].switch_point_offset[RFNV_GSM_LNA_STATE_2].fall_offset;
      }
      ...
      if(rfgsm_nv_rx_elna_asdiv_info != NULL)
      {
        gr2_max_dBm += rfgsm_nv_rx_elna_asdiv_info->rx_elna_asdiv_switch_point_offsets[asdiv_pos].switch_point_offset[RFNV_GSM_LNA_STATE_2].rise_offset;
        gr3_min_dBm += rfgsm_nv_rx_elna_asdiv_info->rx_elna_asdiv_switch_point_offsets[asdiv_pos].switch_point_offset[RFNV_GSM_LNA_STATE_3].fall_offset;
      }
      ...
      if(rfgsm_nv_rx_elna_asdiv_info != NULL)
      {
        gr3_max_dBm += rfgsm_nv_rx_elna_asdiv_info->rx_elna_asdiv_switch_point_offsets[asdiv_pos].switch_point_offset[RFNV_GSM_LNA_STATE_3].rise_offset;
        gr4_min_dBm += rfgsm_nv_rx_elna_asdiv_info->rx_elna_asdiv_switch_point_offsets[asdiv_pos].switch_point_offset[RFNV_GSM_LNA_STATE_4].fall_offset;
      }
      ...
      if(rfgsm_nv_rx_elna_asdiv_info != NULL)
      {
        gr4_max_dBm += rfgsm_nv_rx_elna_asdiv_info->rx_elna_asdiv_switch_point_offsets[asdiv_pos].switch_point_offset[RFNV_GSM_LNA_STATE_4].rise_offset;
        gr5_min_dBm += rfgsm_nv_rx_elna_asdiv_info->rx_elna_asdiv_switch_point_offsets[asdiv_pos].switch_point_offset[RFNV_GSM_LNA_STATE_5].fall_offset;
      }
...
void rfgsm_nv_precompute_rx_switch_points(rfm_device_enum_type rfm_dev)
{
  ...
  */
  rfgsm_nv_precompute_rx_switch_points_band(rfm_dev, RFCOM_BAND_GSM850, RF_GSM_FREQ_COMP_RX_SWPT);
  rfgsm_nv_precompute_rx_switch_points_band(rfm_dev, RFCOM_BAND_GSM850, RF_GSM_HL_RX_SWPT);

  /* Compute rx switch points for GSM900.
  */
  rfgsm_nv_precompute_rx_switch_points_band(rfm_dev, RFCOM_BAND_GSM900, RF_GSM_FREQ_COMP_RX_SWPT);
  rfgsm_nv_precompute_rx_switch_points_band(rfm_dev, RFCOM_BAND_GSM900, RF_GSM_HL_RX_SWPT);

  /* Compute rx switch points for GSM1800.
  */
  rfgsm_nv_precompute_rx_switch_points_band(rfm_dev, RFCOM_BAND_GSM1800, RF_GSM_FREQ_COMP_RX_SWPT);
  rfgsm_nv_precompute_rx_switch_points_band(rfm_dev, RFCOM_BAND_GSM1800, RF_GSM_HL_RX_SWPT);

  /* Compute rx switch points for GSM1900.
  */
    rfgsm_nv_precompute_rx_switch_points_band(rfm_dev, RFCOM_BAND_GSM1900, RF_GSM_FREQ_COMP_RX_SWPT);
    rfgsm_nv_precompute_rx_switch_points_band(rfm_dev, RFCOM_BAND_GSM1900, RF_GSM_HL_RX_SWPT);
...
void rfgsm_nv_precompute_enh_rx_switch_points(rfm_device_enum_type rfm_dev)
{
  ...
  */
  rfgsm_nv_precompute_rx_switch_points_band(rfm_dev, RFCOM_BAND_GSM850, RF_GSM_ENH_RX_SWPT);

  /* Compute enhanced rx switch points for GSM900.
  */
  rfgsm_nv_precompute_rx_switch_points_band(rfm_dev, RFCOM_BAND_GSM900, RF_GSM_ENH_RX_SWPT);

  /* Compute enhanced rx switch points for GSM1800.
  */
  rfgsm_nv_precompute_rx_switch_points_band(rfm_dev, RFCOM_BAND_GSM1800, RF_GSM_ENH_RX_SWPT);

  /* Compute enhanced rx switch points for GSM1900.
  */
  rfgsm_nv_precompute_rx_switch_points_band(rfm_dev, RFCOM_BAND_GSM1900, RF_GSM_ENH_RX_SWPT);
...
void rfgsm_nv_precompute_tx_gain_ranges (rfm_device_enum_type rfm_dev, rfcom_gsm_band_type rfcom_band)
{
  ...
  rfgsm_nv_band_tx_data_type *rfgsm_tx_nv_tbl = NULL;
  ...
  * 0xFFFF which will be smaller than any pa range.*/
  ...
  rfgsm_tx_nv_tbl = (rfgsm_nv_band_tx_data_type *)rfgsm_nv_get_tbl_ptr(rfm_dev, rfcom_band, RFGSM_NV_TX);

  if (rfgsm_tx_nv_tbl == NULL)
  {
	MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR,"rfgsm_tx_nv_tbl ptr is NULL! for device %d", rfm_dev);
	return;
  }

  pa_switch_point_data = rfgsm_tx_nv_tbl->tx_static_nv_data_ptr->pa_switch_point;
...
void rfgsm_nv_init_amam_ampm_table(rfm_device_enum_type rfm_dev, rfcom_gsm_band_type band)
{
  ...
  */
  ...
  rfgsm_tx_nv_tbl = (rfgsm_nv_band_tx_data_type *)rfgsm_nv_get_tbl_ptr(rfm_dev, band, RFGSM_NV_TX);

  if (rfgsm_tx_nv_tbl == NULL)
  {
    MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR,"rfgsm_tx_nv_tbl ptr is NULL!", 0);
    return;
  }
  ...
  */
  ...
  amam_master_table_ptr = &(rfgsm_tx_nv_tbl->tx_cal_nv_data.amam_master_table); 
	
  /* Ptr to AMPM master table NV*/
  ampm_master_table_ptr = &(rfgsm_tx_nv_tbl->tx_cal_nv_data.ampm_master_table);
...
{
  if ( rfgsm_nv_common_config_storage.rx_buffer_count < RF_GSM_RX_NV_MAX_STRUCTURES )
  {
    ...
    if ( rfgsm_nv_common_config_storage.rx_static_nv_data_buffers[rfgsm_nv_common_config_storage.rx_buffer_count]
         == NULL )
    {
      MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, "rfgsm_nv_alloc_common_config: Could not allocate Rx NV memory on heap", 0);	  
      return NULL;
    }
    else
    {
...
{
  if ( rfgsm_nv_common_config_storage.tx_buffer_count < RF_GSM_TX_NV_MAX_STRUCTURES )
  {
    ...
    if ( rfgsm_nv_common_config_storage.tx_static_nv_data_buffers[rfgsm_nv_common_config_storage.tx_buffer_count]
         == NULL )
    {
      MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, "rfgsm_nv_alloc_common_config: Could not allocate Tx NV memory on heap", 0);	  
      return NULL;
    }
    else
    {
...
/*!
  @brief
  Allocate a new NV data structure for storing the GSM Rx configuration
...
static
rfgsm_nv_band_rx_data_type*
rfgsm_rx_nv_alloc_config
(
  gsm_rx_static_nv_type* rx_static_nv_ptr
)
{
  rfgsm_nv_band_rx_data_type* rfgsm_nv_band_rx_data_buffer;
  gsm_rx_static_nv_type** rx_config_ptr;

  if ( rf_gsm_nv_config_storage.allocated_rx_buffers < RF_GSM_RX_NV_MAX_STRUCTURES )
  {
    /* Allocate the NV data buffer*/
    rfgsm_nv_band_rx_data_buffer = (rfgsm_nv_band_rx_data_type*)modem_mem_alloc( sizeof(rfgsm_nv_band_rx_data_type), MODEM_MEM_CLIENT_RFA );

    if ( rfgsm_nv_band_rx_data_buffer == NULL )
    {
      MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, "rfgsm_rx_nv_alloc_config: Could not allocate Rx NV memory on heap", 0);
      return NULL;
    }
    else
    {
      memset((void *)rfgsm_nv_band_rx_data_buffer, 0, sizeof(rfgsm_nv_band_rx_data_type));
	  
      /* if main buffer allocation succeeded, hook static NV ptrs to it*/
      rx_config_ptr = (gsm_rx_static_nv_type**)
        &(rfgsm_nv_band_rx_data_buffer->rx_static_nv_data_ptr);
      *rx_config_ptr = rx_static_nv_ptr;

      rf_gsm_nv_config_storage.rx_buffers[rf_gsm_nv_config_storage.allocated_rx_buffers] = rfgsm_nv_band_rx_data_buffer;
      return rf_gsm_nv_config_storage.rx_buffers[rf_gsm_nv_config_storage.allocated_rx_buffers++];
    }
  }
  else
  {
...
/*!
  @brief
  Allocate a new NV data structure for storing the GSM Tx configuration
  items for a band and path.
...
static
rfgsm_nv_band_tx_data_type*
rfgsm_tx_nv_alloc_config
(
  gsm_tx_static_nv_type* tx_static_nv_ptr
)
{
  rfgsm_nv_band_tx_data_type* rfgsm_nv_band_tx_data_buffer;
  gsm_tx_static_nv_type** tx_config_ptr;

  if ( rf_gsm_nv_config_storage.allocated_tx_buffers < RF_GSM_TX_NV_MAX_STRUCTURES )
  {
    /* Allocate the NV data buffer*/
    rfgsm_nv_band_tx_data_buffer = (rfgsm_nv_band_tx_data_type*)modem_mem_alloc( sizeof(rfgsm_nv_band_tx_data_type), MODEM_MEM_CLIENT_RFA );

    if ( rfgsm_nv_band_tx_data_buffer == NULL )
    {
      MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, "rfgsm_tx_nv_alloc_config: Could not allocate Tx NV memory on heap", 0);
      return NULL;
    }
    else
    {
      memset((void *)rfgsm_nv_band_tx_data_buffer, 0, sizeof(rfgsm_nv_band_tx_data_type));
	  
      tx_config_ptr = (gsm_tx_static_nv_type**)
        &(rfgsm_nv_band_tx_data_buffer->tx_static_nv_data_ptr);
      *tx_config_ptr = tx_static_nv_ptr;
	  
      rf_gsm_nv_config_storage.tx_buffers[rf_gsm_nv_config_storage.allocated_tx_buffers] = rfgsm_nv_band_tx_data_buffer;
      return rf_gsm_nv_config_storage.tx_buffers[rf_gsm_nv_config_storage.allocated_tx_buffers++];
    }
  }
  else
  {
...
)
{
  ...
  rfgsm_nv_band_rx_data_type*   rf_gsm_rx_config_ptr;
  rfgsm_nv_band_tx_data_type*   rf_gsm_tx_config_ptr;
  rfc_cfg_info_type cfg_struct;

  /* Allocate memory for Rx related Static NVs data for a given GSM band and path */
  /* Rx/Tx device split: only allocate if rfm_dev is Rx capable since it can be Tx only device */
  if(rfc_gsm_get_cfg_info( rfm_dev, (rfcom_gsm_band_type)band, RFC_CONFIG_RX, &cfg_struct, 0))
  {
    ...
    if ( rfgsm_rx_static_nv_ptr == NULL )
    {
      ...
      rx_nv_alloc_info[path][band].static_nv_alloc_valid = FALSE;	
      return NULL;
    }
    else
    {
      rx_nv_alloc_info[path][band].static_nv_alloc_valid = TRUE;

      rfgsm_nv_elna_asdiv_init_default_values(RFGSM_NV_RX, (void*)(&rfgsm_rx_static_nv_ptr->asdiv_elna_gain_info));
      rfgsm_nv_elna_asdiv_init_default_values(RFGSM_NV_TX, (void*)(&rfgsm_rx_static_nv_ptr->asdiv_elna_gain_info));
    }

    /* Allocate memory to hold all the NV data for a given GSM band and path 
      memory for Rx static NV data is already allocated above, just pass that ptr so that it can be linked to main NV memory structure*/
    rf_gsm_rx_config_ptr =  rfgsm_rx_nv_alloc_config( rfgsm_rx_static_nv_ptr );

    /* Check if memory allocation was successful, otherwise return a failure*/  
    if ( rf_gsm_rx_config_ptr == NULL )
    {
      MSG_2(MSG_SSID_RF, MSG_LEGACY_ERROR, "Failed to allocate GSM Rx NV structure for NV path %d and band %d", path, band);

      rx_nv_alloc_info[path][band].nv_data_alloc_valid = FALSE;
	  return NULL;
    }
    else
    {
      rf_gsm_rx_config_ptr->band = band;
      rf_gsm_rx_config_ptr->path = path;

      if(rfgsm_nv_data[path].rfgsm_nv_band_node[band].rfgsm_nv_band_rx_data_buffer_ptr == NULL)
      {
        rfgsm_nv_data[path].rfgsm_nv_band_node[band].rfgsm_nv_band_rx_data_buffer_ptr= rf_gsm_rx_config_ptr;
        rfgsm_nv_data[path].rfgsm_nv_band_node[band].data_valid = FALSE;		
      }
         
      rx_nv_alloc_info[path][band].nv_data_alloc_valid = TRUE;
    }
  }
  ...
  if(rfc_gsm_get_cfg_info( rfm_dev, (rfcom_gsm_band_type)band, RFC_CONFIG_TX, &cfg_struct, 0))
  {
    ...
    if ( rfgsm_tx_static_nv_ptr == NULL )
    {
      ...
      tx_nv_alloc_info[path][band].static_nv_alloc_valid = FALSE;	
      return NULL;
    }
    else
    {
      tx_nv_alloc_info[path][band].static_nv_alloc_valid = TRUE;
    }
	
    /* Allocate memory to hold all the NV data for a given GSM band and path 
      memory for Tx static NV data is already allocated above, just pass that ptr so that it can be linked to main NV memory structure*/
    rf_gsm_tx_config_ptr =  rfgsm_tx_nv_alloc_config( rfgsm_tx_static_nv_ptr );
  
    /* Check if memory allocation was successful, otherwise return a failure*/
    if ( rf_gsm_tx_config_ptr == NULL )
    {
      MSG_2(MSG_SSID_RF, MSG_LEGACY_ERROR, "Failed to allocate GSM Tx NV structure for NV path %d and band %d", path, band);
  
      tx_nv_alloc_info[path][band].nv_data_alloc_valid = FALSE;
   	  return NULL;
    }
    else
    {
      rf_gsm_tx_config_ptr->band = band;
      rf_gsm_tx_config_ptr->path = path;
    
      if(rfgsm_nv_data[path].rfgsm_nv_band_node[band].rfgsm_nv_band_tx_data_buffer_ptr == NULL)
      {
        rfgsm_nv_data[path].rfgsm_nv_band_node[band].rfgsm_nv_band_tx_data_buffer_ptr= rf_gsm_tx_config_ptr;
        rfgsm_nv_data[path].rfgsm_nv_band_node[band].data_valid = FALSE;	
      } 	
    
      tx_nv_alloc_info[path][band].nv_data_alloc_valid = TRUE;
    }
  }
...
/*!
  @brief
   Initialize sar lut with default values, i.e. max which means no sar backoff
*/

void rfgsm_nv_init_default_sar_values( gsm_sar_lut_type* local_sar_lut )
{
  ...
  for (slot_count=0; slot_count < RFNV_GSM_SAR_NUM_UL_SLOT_MAX; slot_count++ )
  {
    ...
    {
      local_sar_lut->sar_lut[slot_count][sar_state].sar_gmsk = RFNV_GSM_SAR_BACK_OFF_LIMIT_DEFAULT;
      local_sar_lut->sar_lut[slot_count][sar_state].sar_8psk = RFNV_GSM_SAR_BACK_OFF_LIMIT_DEFAULT;
    }
  }
}
...
)
{
  ...
  else
  {
    ...
    rfgsm_nv_init_default_sar_values( static_nv_struct );
    return TRUE;
...
/*!
  @brief
  Generic function to load the SAR_BACKOFF related data for any band from variant type sar nv items.
  The variant sar nv item is identified per GSM band including default and switched antenna sar lut for different chains

  @details
  Checks that the variant type NV item for this device's and band's calibration channel
  list is loaded, and if so, initalizes the SAR_BACKOFF data for that band.
 
  @param use_switched_antenna_sar_lut
  Indicates that the switched antenna table is loaded and should therefore be used
 
  @param default_ant_nv_struct
  Legacy sar lut (for default antenna position)
 
  @param switched_ant_nv_struct
  Sar lut for switched antenna position
 
  @param sar_lut_var_data_item
  Variant NV item code for NV that contains the SAR_BACKOFF value.
 
  @param nv_path
  NV path - can be one of 0, 2 or 4
 
  @param band
  Band for which the sar backoff data should be loaded
...
boolean
rfgsm_nv_load_sar_lut_var
(
  boolean* use_switched_antenna_sar_lut,
  gsm_sar_lut_type* default_ant_nv_struct,
  gsm_sar_lut_type* switched_ant_nv_struct,
  rfnv_item_id_enum_type sar_lut_var_data_item,
  rfgsm_nv_path_type nv_path,
  rfgsm_nv_supported_type band,
  rex_tcb_type* caller_tcb_ptr,
  rex_sigs_type task_nv_wait_sig,
...
)
{
  ...
  /* NV read status */
  nv_stat_enum_type  sar_var_nv_read_status = NV_DONE_S;
  ...
  /* Helper pointer to hold moving ptr location during data copy*/
  uint8     *temp_data_ptr = NULL;

  /* actual size of memory read from the NV*/
  rfnv_item_size_type gsm_sar_var_nv_read_size = sizeof(rfnv_item_type);
  ...
  /* contains no of type_ids found in sar var NV*/
  ...
  boolean sar_switched_ant_done = FALSE;
  
  rfnv_data_variant_marker_type sar_var_marker;

  rfnv_data_sar_backoff_type nv_sar_data;	//represent the nv DataType RFNV_DATA_SAR_BACKOFF_TYPE defined in NvDefinition.xml

  uint8 sar_chain_id; 

  sar_chain_id = (uint8)nv_path;

  /* Allocate memory for raw NV data */
  nv_data_ptr = (rfnv_item_type *)modem_mem_alloc( nv_item_size,
  ...
  if (nv_data_ptr == NULL)
  {
    ...
    MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, "rfgsm_nv_load_sar_lut_var - failed to allocate memory for %d bytes.", nv_item_size);
    api_status = FALSE;
    return FALSE;
  }
  
  /* Adding an offset to RF NV id. sar_lut_var_data_item = sar_lut_var_data_item + band*/
  sar_lut_var_data_item = sar_lut_var_data_item + band;
  
  //load all contents of the nv item based on item_code, i.e. band based variant sar nv item
  sar_var_nv_read_status = rfcommon_rfnv_get_item(sar_lut_var_data_item,
                                                  nv_data_ptr,
                                                  &gsm_sar_var_nv_read_size,
                                                  caller_tcb_ptr,
                                                  task_nv_wait_sig,
                                                  task_wait_func_ptr );				

  if ( sar_var_nv_read_status == NV_DONE_S )
  {    
    MSG_1(MSG_SSID_RF, MSG_LEGACY_MED, "rfgsm_nv_load_sar_lut_var: SAR NV read size = %d", gsm_sar_var_nv_read_size);
	
  } /* if (sar_var_nv_read_status == NV_DONE_S ) */
  else
  {
    /* sar vairant nv item loading failure, so static sar nv is used, and initialized by rfgsm_nv_load_sar_lut()*/
    MSG(MSG_SSID_RF, MSG_LEGACY_ERROR, "rfgsm_nv_load_sar_lut_var: No variant sar nv item available. Initialzed the sar data for switched antenna to default");	

    /* If variant sar nv items are not avaiable, then default sar LUT is used (set by rfgsm_nv_load_sar_lut); initialize sar_lut_2 with default values */
    rfgsm_nv_init_default_sar_values( switched_ant_nv_struct );			

    api_status = FALSE;

    /* Indicate that only default antenna sar_backoff LUT will be used */
    *use_switched_antenna_sar_lut = FALSE;
  ...
  /* If the variant sar NV data has been successfully retrieved proceed to populating the local sar lut data structure for both default and switched antenna */
  if (api_status == TRUE) 
  {
    ...
    memscpy((void *)&sar_var_marker, sizeof(sar_var_marker), (void*)temp_data_ptr, nv_element_size );
    ...
    num_type_id = sar_var_marker.num_elements;

    /* Pick up right data sar data for default and switched antenna*/
    for( type_id_idx = 0; (type_id_idx < num_type_id) && (api_status == TRUE); type_id_idx++ )
    {
      ...
      if((gsm_sar_var_nv_read_size - num_bytes_parsed) < nv_element_size )
      {
        MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, "rfgsm_nv_load_sar_lut_var - exiting num_bytes_parsed = %d", num_bytes_parsed );
        api_status = FALSE;
        break;
      }
      ...
      {
        MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, "rfgsm_nv_load_sar_lut_var - memory exhausted", 0 );
        api_status = FALSE;
        break;
      }
      ...
      temp_num_bytes_left = gsm_sar_var_nv_read_size - num_bytes_parsed;

      MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, "rfgsm_nv_load_sar_lut_var - TypeId = %d", type_id );

      if(type_id != RFNV_DATA_TYPE_ID_SAR_BACKOFF)
      {
        MSG_2(MSG_SSID_RF, MSG_LEGACY_ERROR, "rfgsm_nv_load_sar_lut_var - element of type ID %d not supported "
      ...
      if( temp_num_bytes_left <= 0 )
      {
        MSG_2(MSG_SSID_RF, MSG_LEGACY_ERROR, 
              "rfgsm_nv_load_sar_lut_var - No elements left to extract "
      ...
      /* Extract the sar element */
      memscpy( (void*) &nv_sar_data, sizeof(nv_sar_data), temp_data_ptr, sizeof(nv_sar_data) );

      /* chain 0 and 1 is identified in nv sar dat as 0; chain 2 and 3 is identified in nv sar dat as 2 */
      if(sar_chain_id % 2 != 0)
      {
         sar_chain_id -= 1;
      }

      MSG_2(MSG_SSID_RF, MSG_LEGACY_ERROR, "rfgsm_nv_load_sar_lut_var - nv data chain_identifier = %d antenna_index %d", nv_sar_data.chain_identifier, nv_sar_data.antenna_index );

      /* Sar nv data must be for the nv_path 0 or 2*/
      if (nv_sar_data.chain_identifier == sar_chain_id) 
      {	   	
        if (nv_sar_data.antenna_index == RFM_ANTENNA_0)
        {
	  memscpy(default_ant_nv_struct, sizeof(*default_ant_nv_struct), &(nv_sar_data.sar_backoff_lut[0][0]), sizeof(gsm_sar_lut_type));
	}
        else if (nv_sar_data.antenna_index == RFM_ANTENNA_1)
        {		   
          memscpy(switched_ant_nv_struct, sizeof(*switched_ant_nv_struct), &(nv_sar_data.sar_backoff_lut[0][0]), sizeof(gsm_sar_lut_type));
          sar_switched_ant_done = TRUE;
          *use_switched_antenna_sar_lut = TRUE; //indicate switched antenna sar_backoff LUT will be used 
        }
        /*expand here if more antennas are added*/
      }

      /* Update running total of bytes parsed with num bytes from last element extracted */
      num_bytes_parsed += sizeof(nv_sar_data);	   
    } /* end of for loop*/
  }

  if( sar_switched_ant_done != TRUE || api_status == FALSE )
  {
    /* if not found sar LUT for switched antenna or nv item is corrupted, initialize local sar LUT for switched antenna with default values */
    rfgsm_nv_init_default_sar_values( switched_ant_nv_struct ); 		
    MSG(MSG_SSID_RF, MSG_LEGACY_ERROR, "rfgsm_nv_load_sar_lut_var: No variant sar nv item available. Initialzed switched_nv to default");	
    MSG_2(MSG_SSID_RF, MSG_LEGACY_ERROR, "rfgsm_nv_load_sar_lut_var: sar_switched_ant_done=%d api_status=%d", sar_switched_ant_done, api_status);	
    *use_switched_antenna_sar_lut = FALSE;	//indicate only default antenna sar_backoff LUT will be used	
  }
...
rfgsm_nv_path_type rfgsm_nv_get_nv_container(rfm_device_enum_type rfm_dev, rfcom_gsm_band_type band, rfgsm_nv_rxtx_enum_type rx_tx)
{
  ...
  if (rx_tx == RFGSM_NV_RX)
  {
    rfc_dev_cfg_supported = rfc_gsm_get_cfg_info( rfm_dev, band, RFC_CONFIG_RX, &cfg_struct, RFGSM_ALT_PATH_NOT_SUPPORTED);
  }
  else if (rx_tx == RFGSM_NV_TX)
  {
    rfc_dev_cfg_supported = rfc_gsm_get_cfg_info( rfm_dev, band, RFC_CONFIG_TX, &cfg_struct, RFGSM_ALT_PATH_NOT_SUPPORTED);
  }
  else
  {
  ...
  /* Removing additional F3s, we get prints from RFC AG files if device configuration is not supported */
...
#define RF_GSM_STATIC_VARIANT_NV_HEADER_SIZE sizeof(rfnv_data_variant_marker_type)

/*----------------------------------------------------------------------------*/
/*!
  @brief
  Intializes default values for the asdiv items if they haven't been initialized yet

  @details
  Initializes all asdiv elna offsets to 0. Called when NV data not present

  @param rfgsm_nv_rxtx_enum_type rx_tx
  Rx or Tx elna asdiv NV item type

  @param rx_elna_asdiv_item
  NV Item id for GSM ELNA Offset Item

  @return
  Void
*/

void rfgsm_nv_elna_asdiv_init_default_values
(
  rfgsm_nv_rxtx_enum_type rx_tx, 
  void *gsm_elna_offset_item
)
{
  if(gsm_elna_offset_item != NULL)
  {
    if(rx_tx == RFGSM_NV_RX)
    {
      rfnv_gsm_rx_elna_asdiv_type* rx_gsm_elna_offset_item = gsm_elna_offset_item;

      /*Initialize all Rx gain offsets to 0*/
      memset(rx_gsm_elna_offset_item->rx_elna_asdiv_gain_offset, 0, ASDIV_POSITION_MAX*sizeof(rfnv_asdiv_rx_gain_offset_type));
      memset(rx_gsm_elna_offset_item->rx_elna_asdiv_switch_point_offsets, 0, ASDIV_POSITION_MAX*sizeof(rfnv_asdiv_rx_switch_points_type));
    }
    if(rx_tx == RFGSM_NV_TX)
    {
      rfnv_gsm_tx_elna_asdiv_type* tx_gsm_elna_offset_item = gsm_elna_offset_item;

      /*Initialize all Rx gain offsets to 0*/
      memset(tx_gsm_elna_offset_item->tx_elna_asdiv_gain_offset, 0, ASDIV_POSITION_MAX*sizeof(rfnv_asdiv_tx_gain_offset_type));
    }
  }
  else
  {
    MSG_2(MSG_SSID_RF, MSG_LEGACY_ERROR, "gsm_elna_offset_item hasn't been allocated!", 0, 0);
  }
  return;
}
...
/*!
  @brief  
  HELPER API for extracting Tx Gain offsets for asdiv with eLNA component
...
*/
boolean
rfgsm_nv_extract_elna_asdiv_tx_gain_offsets
(
  const void * const nv_data_ptr,
  const uint32 num_bytes_available,
  uint32 * const extracted_element_size,
  rfnv_gsm_tx_elna_asdiv_type *gsm_tx_asdiv_elna_data
)
{
    /* Overall API status */
  boolean api_status = TRUE;
  /* The item to extract from NV */
  rfnv_asdiv_tx_gain_offset_type tx_gain_offsets;
  /* Contains the number of bytes for the element in NV */
  uint32 num_bytes_needed;
  /* For indexing */
  uint8 rgi_idx;
  ...
  *extracted_element_size = 0;
  ...
  /* The number of bytes needed to extract element */
  num_bytes_needed = sizeof(tx_gain_offsets);

  RF_MSG( RF_HIGH, "rfgsm_nv_extract_elna_asdiv_tx_gain_offsets" );

  /* Ensure there is enough memory left before actually extracting the element */
  if( num_bytes_available < num_bytes_needed )
  {
    MSG_2(MSG_SSID_RF, MSG_LEGACY_ERROR, "rfgsm_nv_extract_elna_asdiv_tx_gain_offsets - Not enough elements left to extract",0,0 );
    api_status = FALSE;
  }
  ...
  {
    /* Extract the element */
    memscpy( (void*) &tx_gain_offsets, sizeof(tx_gain_offsets), nv_data_ptr, num_bytes_needed );

    /* update this variable so that caller API can know how many bytes are copied in this call */
    *extracted_element_size = num_bytes_needed;
    
    if (tx_gain_offsets.switch_position == ASDIV_POSITION_0)
    {
      gsm_tx_asdiv_elna_data->tx_elna_asdiv_gain_offset[ASDIV_POSITION_0] = tx_gain_offsets;
    }
    else if (tx_gain_offsets.switch_position == ASDIV_POSITION_1)
    {
      gsm_tx_asdiv_elna_data->tx_elna_asdiv_gain_offset[ASDIV_POSITION_1] = tx_gain_offsets;
    }
    else
    {
      MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, "rfgsm_nv_extract_elna_asdiv_tx_gain_offsets invalid pos %d",tx_gain_offsets.switch_position);
      api_status = FALSE;
    }
  }
...
/*!
  @brief  
  HELPER API for extracting Rx Switch Point offsets for asdiv with eLNA component
...
*/
boolean
rfgsm_nv_extract_elna_asdiv_rx_swpt_offsets
(
  const void * const nv_data_ptr,
  const uint32 num_bytes_available,
  uint32 * const extracted_element_size,
  rfnv_gsm_rx_elna_asdiv_type *gsm_rx_asdiv_elna_data
)
{
  /* Overall API status */
  boolean api_status = TRUE;
  /* The item to extract from NV */
  rfnv_asdiv_rx_switch_points_type rx_swpt_offsets;
  /* Contains the number of bytes for the element in NV */
  uint32 num_bytes_needed;
  /* For indexing */
  uint8 rgi_idx;
  ...
  *extracted_element_size = 0;
  ...
  /* The number of bytes needed to extract element */
  num_bytes_needed = sizeof(rx_swpt_offsets);

  RF_MSG( RF_HIGH, "rfgsm_nv_extract_elna_asdiv_rx_swpt_offsets" );

  /* Ensure there is enough memory left before actually extracting the element */
  if( num_bytes_available < num_bytes_needed )
  {
    MSG_2(MSG_SSID_RF, MSG_LEGACY_ERROR, "rfgsm_nv_extract_elna_asdiv_rx_swpt_offsets - Not enough elements left to extract",0,0 );
    api_status = FALSE;
  }
  ...
  {
    /* Extract the element */
    memscpy( (void*) &rx_swpt_offsets, sizeof(rx_swpt_offsets), nv_data_ptr, num_bytes_needed );

    /* update this variable so that caller API can know how many bytes are copied in this call */
    *extracted_element_size = num_bytes_needed;
    
    if (rx_swpt_offsets.switch_position == ASDIV_POSITION_0)
    {
      gsm_rx_asdiv_elna_data->rx_elna_asdiv_switch_point_offsets[ASDIV_POSITION_0] = rx_swpt_offsets;
    }
    else if (rx_swpt_offsets.switch_position == ASDIV_POSITION_1)
    {
      gsm_rx_asdiv_elna_data->rx_elna_asdiv_switch_point_offsets[ASDIV_POSITION_1] = rx_swpt_offsets;
    }
    else
    {
      MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, "rx_elna_asdiv_switch_point_offsets invalid pos %d",rx_swpt_offsets.switch_position);
	    api_status = FALSE;
    }
  }
...
/*!
  @brief  
  HELPER API for extracting Rx Gain Offsets for ASDIV with elna component
...
*/
boolean
rfgsm_nv_extract_elna_asdiv_rx_gain_offsets
(
  const void * const nv_data_ptr,
  const uint32 num_bytes_available,
  uint32 * const extracted_element_size,
...
)
{
  ...
  boolean api_status = TRUE;
  /* The item to extract from NV */
  rfnv_asdiv_rx_gain_offset_type rx_gain_offsets;
  /* Contains the number of bytes for the element in NV */
  uint32 num_bytes_needed;
  /* For indexing */
  uint8 rgi_idx;
  ...
  *extracted_element_size = 0;
  ...
  /* The number of bytes needed to extract element */
  num_bytes_needed = sizeof(rx_gain_offsets);

  RF_MSG( RF_HIGH, "rfgsm_nv_extract_elna_asdiv_rx_gain_offsets" );

  /* Ensure there is enough memory left before actually extracting the element */
  if( num_bytes_available < num_bytes_needed )
  {
    MSG_2(MSG_SSID_RF, MSG_LEGACY_ERROR, "rfgsm_nv_extract_elna_asdiv_rx_gain_offsets - Not enough elements left to extract",0,0 );
    api_status = FALSE;
  }
  ...
  {
    /* Extract the element */
    memscpy( (void*) &rx_gain_offsets, sizeof(rx_gain_offsets), nv_data_ptr, num_bytes_needed );

    /* update this variable so that caller API can know how many bytes are copied in this call */
    *extracted_element_size = num_bytes_needed;
    
    if (rx_gain_offsets.switch_position == ASDIV_POSITION_0)
    {
      gsm_rx_asdiv_elna_data->rx_elna_asdiv_gain_offset[ASDIV_POSITION_0] = rx_gain_offsets;
    }
    else if (rx_gain_offsets.switch_position == ASDIV_POSITION_1)
    {
      gsm_rx_asdiv_elna_data->rx_elna_asdiv_gain_offset[ASDIV_POSITION_1] = rx_gain_offsets;
    }
    else
    {
      MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, "rfgsm_nv_extract_elna_asdiv_rx_gain_offsets invalid pos %d",rx_gain_offsets.switch_position);
      api_status = FALSE;
    }
  }
...
/*!
  @brief
  Loads the ELNA asdiv offset items
...
boolean rfgsm_nv_load_asdiv_elna_offset_items
(
  rfnv_gsm_rx_elna_asdiv_type *gsm_elna_rx_offset_item,
  rfnv_gsm_tx_elna_asdiv_type *gsm_elna_tx_offset_item,
  rfnv_item_id_enum_type elna_asdiv_item,
  rfgsm_nv_supported_type band,
  rex_tcb_type* caller_tcb_ptr,
  rex_sigs_type task_nv_wait_sig,
  void (* task_wait_func_ptr)( rex_sigs_type )
)
{
  ...
  /* actual size of memory read from the NV*/
  rfnv_item_size_type   gsm_elna_asdiv_nv_read_size = sizeof(rfnv_item_type);

  rfnv_data_variant_marker_type variant_marker;

  /*Rx/Tx asdiv ELNA items*/
  rfnv_gsm_rx_elna_asdiv_type * rx_elna_asdiv_items = gsm_elna_rx_offset_item;
  rfnv_gsm_tx_elna_asdiv_type * tx_elna_asdiv_items = gsm_elna_tx_offset_item;
  ...
  if (nv_data_ptr == NULL)
  {
    ...
    MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, "rfgsm_nv_load_asdiv_elna_offset_items - failed to allocate memory for %d bytes.", nv_item_size);
    api_status = FALSE;
    return FALSE;
  }

  /*Adding a offset to RF NV id. tx_cal_data_item = tx_cal_data_item + band*/
  elna_asdiv_item = elna_asdiv_item + band;
  
  read_status = rfcommon_rfnv_get_item( elna_asdiv_item,
                                                  nv_data_ptr,
                                                  &gsm_elna_asdiv_nv_read_size,
                                                  caller_tcb_ptr,
                                                  task_nv_wait_sig,
                                                  task_wait_func_ptr );
  ...
  {    
    MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, "rfgsm_nv_load_asdiv_elna_offset_items: NV read size = %d", gsm_elna_asdiv_nv_read_size);
  } 
  else
  {
    /* initialize the structure with default values accordlingly*/
    MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, "rfgsm_nv_load_tx_cal_items: Initialzed the TX cal data to default", 0);	
    rfgsm_nv_elna_asdiv_init_default_values(RFGSM_NV_RX, rx_elna_asdiv_items);
    rfgsm_nv_elna_asdiv_init_default_values(RFGSM_NV_TX, tx_elna_asdiv_items);
    api_status = FALSE;
  ...
  if (api_status == TRUE) 
  {
    ...
    /* copy data for fixed part and variant header of the GSM Tx cal NV, need to adjust size*/
    nv_element_size = RF_GSM_STATIC_VARIANT_NV_HEADER_SIZE;

    if((rx_elna_asdiv_items != NULL) || (tx_elna_asdiv_items != NULL))
    {
      /*Just get the variant header*/
      memscpy(&variant_marker, nv_element_size, (void*)temp_data_ptr, nv_element_size );

      /* Update running total of bytes already parsed */
      num_bytes_parsed += nv_element_size;

      num_type_id = variant_marker.num_elements;
      ...
      for(type_id_idx = 0; type_id_idx < num_type_id; type_id_idx++)
      {
        ...
        /* Ensure there is enough memory left before actually extracting the element */
        if((gsm_elna_asdiv_nv_read_size - num_bytes_parsed) < nv_element_size )
        {
         MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, "rfgsm_nv_load_asdiv_elna_offset_items - exiting num_bytes_parsed = %d", num_bytes_parsed );
         api_status = FALSE;
         break;
        }
        ...
        /* Update running total of bytes parsed with num bytes from the variant header ID */
        num_bytes_parsed += nv_element_size;

        /* Overflow Check*/
        temp_num_bytes_left = gsm_elna_asdiv_nv_read_size - num_bytes_parsed;
        if( temp_num_bytes_left <= 0 )
        {
         MSG_2(MSG_SSID_RF, MSG_LEGACY_ERROR, 
               "rfgsm_nv_load_tx_cal_items - No elements left to extract "
        ...
        switch( type_id )
        {
          ...
          {
            if(rx_elna_asdiv_items)
            {
              rfgsm_nv_extract_elna_asdiv_rx_gain_offsets(temp_data_ptr, temp_num_bytes_left, &nv_element_size, rx_elna_asdiv_items);
            }
            else
            { 
              nv_element_size = sizeof(rfnv_asdiv_rx_gain_offset_type);
              RF_MSG( RF_ERROR, "rfgsm_nv_extract_elna_asdiv_rx_gain_offsets is NULL" );
              api_status = FALSE;
            }
            break;
          }
          ...
          case RFNV_DATA_TYPE_ASDIV_RX_SWITCH_POINTS:
          {
            if(rx_elna_asdiv_items)
            {
              rfgsm_nv_extract_elna_asdiv_rx_swpt_offsets(temp_data_ptr, temp_num_bytes_left, &nv_element_size, rx_elna_asdiv_items );
            }
            else
            {
              nv_element_size = sizeof(rfnv_asdiv_rx_gain_offset_type);
          ...
          {
            if(tx_elna_asdiv_items)
            {
              rfgsm_nv_extract_elna_asdiv_tx_gain_offsets(temp_data_ptr, temp_num_bytes_left, &nv_element_size, tx_elna_asdiv_items );
            }
            else
            {
    ...
    {
      MSG_2(MSG_SSID_RF, MSG_LEGACY_ERROR, "Pointer to static NV gsm_elna_offset_item not initialized!", 0, 0);
    }
...
FUNCTION rfgsm_nv_get_elna_asdiv_info                                 

DESCRIPTION
  Returns pointer to GSM RF Asdiv Elna Offset Table/For either Rx or Tx. 
  
  API user needs to typecast value to either rfnv_gsm_tx_elna_asdiv_type or rfnv_gsm_rx_elna_asdiv_type
  depending on param rx_tx passed in

DEPENDENCIES
  None

RETURN VALUE
  Returns pointer to RF GSM Asdiv Elna Offset Table
...
void * rfgsm_nv_get_elna_asdiv_info
( 
  rfm_device_enum_type rfm_dev,
  rfcom_gsm_band_type band,
  rfgsm_nv_rxtx_enum_type rx_tx
...
{
  void *rf_gsm_nv_data_buffer_ptr = NULL;
  void * rf_gsm_nv_asdiv_elna_offset_table_ptr = NULL;
  
  /* get NV data Buffer ptr for given NV path and band*/
  rf_gsm_nv_data_buffer_ptr = rfgsm_nv_get_tbl_ptr(rfm_dev, band, rx_tx);
  ...
  if(rx_tx == RFGSM_NV_RX)
  {
    rfgsm_nv_band_rx_data_type * rx_nv_data_ptr = rf_gsm_nv_data_buffer_ptr;
    if(rx_nv_data_ptr->rx_static_nv_data_ptr == NULL)
    {
      MSG_2(MSG_SSID_RF, MSG_LEGACY_ERROR,"rx_static_nv_data_ptr is NULL! for device %d, band %d", rfm_dev, band);
      return NULL;
    }
    rf_gsm_nv_asdiv_elna_offset_table_ptr = (void*)(&rx_nv_data_ptr->rx_static_nv_data_ptr->asdiv_elna_gain_info);
  }
  else if(rx_tx == RFGSM_NV_TX)
  {
    rfgsm_nv_band_tx_data_type * tx_nv_data_ptr = rf_gsm_nv_data_buffer_ptr;
    if(tx_nv_data_ptr->tx_static_nv_data_ptr == NULL)
    {
      MSG_2(MSG_SSID_RF, MSG_LEGACY_ERROR,"tx_static_nv_data_ptr is NULL! for device %d, band %d", rfm_dev, band);
      return NULL;
    }
    rf_gsm_nv_asdiv_elna_offset_table_ptr = (void*)(&tx_nv_data_ptr->tx_static_nv_data_ptr->asdiv_elna_gain_info);
  }

  return rf_gsm_nv_asdiv_elna_offset_table_ptr;
...
)
{
  ...
  rfgsm_nv_band_rx_data_type* rf_gsm_nv_band_rx_data_buffer_ptr;
  rfgsm_nv_band_tx_data_type* rf_gsm_nv_band_tx_data_buffer_ptr;

  rfnv_gsm_rx_elna_asdiv_type * rf_gsm_rx_elna_asdiv_ptr = NULL;
  rfnv_gsm_tx_elna_asdiv_type * rf_gsm_tx_elna_asdiv_ptr = NULL;
  ...
  if(rfc_gsm_get_cfg_info( rfm_dev, (rfcom_gsm_band_type)band, RFC_CONFIG_RX, &cfg_struct, 0))
  {
    rf_gsm_nv_band_rx_data_buffer_ptr = index_entry->rfgsm_nv_band_rx_data_buffer_ptr;
    rx_static_nv_data_ptr = rf_gsm_nv_band_rx_data_buffer_ptr->rx_static_nv_data_ptr;

	if (rx_static_nv_data_ptr == NULL)
    {      
	  MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, "rfgsm_nv_load_band_static_items: Invalid NULL pointer for the Rx static config data", 0);   
	  return FALSE;
    }
  ...
  if(rfc_gsm_get_cfg_info( rfm_dev, (rfcom_gsm_band_type)band, RFC_CONFIG_TX, &cfg_struct, 0))
  {
    rf_gsm_nv_band_tx_data_buffer_ptr = index_entry->rfgsm_nv_band_tx_data_buffer_ptr;    
	tx_static_nv_data_ptr = rf_gsm_nv_band_tx_data_buffer_ptr->tx_static_nv_data_ptr;
	
    if (tx_static_nv_data_ptr == NULL)
    {      
	  MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, "rfgsm_nv_load_band_static_items: Invalid NULL pointer for the Tx static config data", 0);   
	  return FALSE;
    }
   ...
   /* Load SAR BACKOFF for default antenna related data */
  ...
  /* Load variant SAR BACKOFF for both default and switched antenna related sar backoff data */ 
	
  if ( retval == TRUE )
  {
	index_entry->last_nv_item_read = rfgsm_nv_items_map_ptr->sar_lut_var;
	tx_static_nv_data_ptr->use_switched_antenna_sar_lut = FALSE;
	retval = rfgsm_nv_load_sar_lut_var(&tx_static_nv_data_ptr->use_switched_antenna_sar_lut, 
		                                   &tx_static_nv_data_ptr->sar_lut_data, 
                                                   &tx_static_nv_data_ptr->sar_lut_data_switched_ant,
                                                   rfgsm_nv_items_map_ptr->sar_lut_var,
                                                   nv_path,
                                                   band,
                                                   caller_tcb_ptr,
                                                   task_nv_wait_sig,
  ...
  if(retval == TRUE)
  {
    ...
    index_entry->last_nv_item_read = rfgsm_nv_items_map_ptr->elna_asdiv_gain_data;

    if((rf_gsm_nv_band_tx_data_buffer_ptr != NULL) && (rf_gsm_nv_band_rx_data_buffer_ptr != NULL))
    {
      rf_gsm_rx_elna_asdiv_ptr = &rf_gsm_nv_band_rx_data_buffer_ptr->rx_static_nv_data_ptr->asdiv_elna_gain_info;
      rf_gsm_tx_elna_asdiv_ptr = &rf_gsm_nv_band_tx_data_buffer_ptr->tx_static_nv_data_ptr->asdiv_elna_gain_info;
      MSG_1(MSG_SSID_RF, MSG_LEGACY_HIGH, "Valid Rx/Tx Static Data Ptrs", 0);
    }
    else if((rf_gsm_nv_band_tx_data_buffer_ptr == NULL) && (rf_gsm_nv_band_rx_data_buffer_ptr != NULL))
    {
      rf_gsm_rx_elna_asdiv_ptr = &rf_gsm_nv_band_rx_data_buffer_ptr->rx_static_nv_data_ptr->asdiv_elna_gain_info;
      MSG_1(MSG_SSID_RF, MSG_LEGACY_HIGH, "Valid Rx Static Data Ptrs", 0);
    }
    else if((rf_gsm_nv_band_tx_data_buffer_ptr != NULL) && (rf_gsm_nv_band_rx_data_buffer_ptr == NULL))
    {
      rf_gsm_tx_elna_asdiv_ptr = &rf_gsm_nv_band_tx_data_buffer_ptr->tx_static_nv_data_ptr->asdiv_elna_gain_info;
      MSG_1(MSG_SSID_RF, MSG_LEGACY_HIGH, "Valid Tx Static Data Ptrs", 0);
    }

    
    retval = rfgsm_nv_load_asdiv_elna_offset_items(rf_gsm_rx_elna_asdiv_ptr,
                                               rf_gsm_tx_elna_asdiv_ptr,
                                               rfgsm_nv_items_map_ptr->elna_asdiv_gain_data,
                                               band,
                                               caller_tcb_ptr,
                                               task_nv_wait_sig,
                                               task_wait_func_ptr);
...
{
  rfgsm_nv_band_rx_data_type* rf_gsm_nv_rx_data_buffer_ptr;
  rfgsm_nv_band_tx_data_type* rf_gsm_nv_tx_data_buffer_ptr;
  ...
  if(rfc_gsm_get_cfg_info( rfm_dev, (rfcom_gsm_band_type)band, RFC_CONFIG_RX, &cfg_struct, 0))
  {
    rf_gsm_nv_rx_data_buffer_ptr = index_entry->rfgsm_nv_band_rx_data_buffer_ptr;

	if ( rf_gsm_nv_rx_data_buffer_ptr == NULL )
    {
      MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, "rfgsm_nv_load_band_cal_items: Invalid NULL pointer for the Rx cal config data", 0);	
      return FALSE;
    }
    ...
    if ( retval == TRUE )
    {
      ...
      retval = rfgsm_nv_load_rx_cal_items( &rf_gsm_nv_rx_data_buffer_ptr->rx_cal_nv_data,
    ...
    if ( retval == TRUE )
    {
      ...
      retval = rfgsm_nv_load_rx_cal_items( &rf_gsm_nv_rx_data_buffer_ptr->hl_rx_cal_nv_data,
  ...
  if(rfc_gsm_get_cfg_info( rfm_dev, (rfcom_gsm_band_type)band, RFC_CONFIG_TX, &cfg_struct, 0))
  {  
    rf_gsm_nv_tx_data_buffer_ptr = index_entry->rfgsm_nv_band_tx_data_buffer_ptr;
	
    if ( rf_gsm_nv_tx_data_buffer_ptr == NULL)
    {
      MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, "rfgsm_nv_load_band_cal_items: Invalid NULL pointer for the Tx cal config data", 0);	
      return FALSE;
    }
    ...
    if ( retval == TRUE )
    {
        ...
        retval = rfgsm_nv_load_tx_cal_items( &rf_gsm_nv_tx_data_buffer_ptr->tx_cal_nv_data,
    ...
    if ( retval == TRUE )
    {
      ...
      retval = rfgsm_nv_load_cal_temp( &rf_gsm_nv_tx_data_buffer_ptr->tx_cal_nv_data.cal_temp,
...
)
{
  ...
  uint8 i = 0;
  boolean dup_nv_path = FALSE;
  static boolean nv_cont_init_check=FALSE;
  ...
  //Initialize only unchecked entries of the common NV container for a logical device
  
   if(nv_cont_init_check==FALSE)
   {
     for (i = 0; i < RFM_MAX_WAN_DEVICES; i++)
     {
        rfgsm_cmn_nv_container[i] = RF_GSM_NV_PATH_NUM;
     }
     nv_cont_init_check = TRUE;   
   }
  ...
  for (band=0;band<RFCOM_NUM_GSM_BANDS;band++)
  {
    ...
    if (rfgsm_nv_path < RF_GSM_NV_PATH_NUM)
    {
      ...
      *flag_ptr = (uint8)TRUE;
      ...
      if(rfc_common_get_device_capability(rfm_dev) == RFM_DEVICE_RX_SUPPORTED)
      {
        band_mask = rfcommon_nv_get_rx_device_bc_config ( rfm_dev );
      ...
      else if(rfc_common_get_device_capability(rfm_dev) == RFM_DEVICE_TX_SUPPORTED)
      {
        bc_cfg_check = TRUE;
        break;
      }
  ...
  {
    //With split Rx/Tx device two different rfm_dev can point to same rfgsm_nv_path so need to prevent allocating and loading common NVs again for the same path
    for (i = 0; i < RFM_MAX_WAN_DEVICES; i++)
    {
      if (rfgsm_cmn_nv_container[i] == rfgsm_nv_path)
      {
        rfgsm_nv_cmn_tbl_ptr = rfgsm_nv_get_cmn_tbl_ptr(i);
        //Added an extra check to see whether there is any valid data in the duplicate nv path before setting it to TRUE		
  ...
  for (band = 0; band < RFCOM_NUM_GSM_BANDS; band++)
  {
    ...
    for (i = 0; i < 2; i++)  //  0 for RX and 1 for Tx in each band
    {
      ...
      {
        if(rfc_common_get_device_capability(rfm_dev) == RFM_DEVICE_TX_SUPPORTED)
        { 
          band_mask = predef_band_mask[band];
        }
      }
...
void rfgsm_nv_cmn_get_sar_value
(
 rfm_device_enum_type rfm_dev,
 uint8 num_tx_slots,
 rfm_common_sar_state_type sar_state,
 rfnv_gsm_sar_unit_type *sar_limit,
 rfm_antenna_enum_type ant_pos
 )
{
   uint8 sar_indx=0;
#ifdef FEATURE_RF_ASDIV
   rfcommon_asdiv_position_enum_type asdiv_pos=ASDIV_POSITION_0;
#endif
   rfgsm_core_handle_type *rfgsm_core_handle_ptr = NULL;
   ...
   if ( tx_static_nv_data_ptr == NULL ) 
   {
      MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR,"tx_static_nv_data_ptr is NULL!", 0);
      return;
   }
#ifdef FEATURE_RF_ASDIV
   ...
   */
   if(sar_limit != NULL)
   {
        ...
        )
     {
       ...
       MSG_3(MSG_SSID_RF, MSG_LEGACY_MED, "sar for default antenna, sar_state %d num_tx_slots %d: GMSK - %d", sar_state, num_tx_slots, sar_limit->sar_gmsk);    
     }   
#ifdef FEATURE_RF_ASDIV
     /* Check current antenna position if switched antenna backoff NV is available */
     else if(tx_static_nv_data_ptr->use_switched_antenna_sar_lut == TRUE && asdiv_pos == ASDIV_POSITION_1)
     {
       sar_indx = (uint8)sar_state;
       sar_limit->sar_gmsk = tx_static_nv_data_ptr->sar_lut_data_switched_ant.sar_lut[num_tx_slots-1][sar_indx-1].sar_gmsk;
       sar_limit->sar_8psk = tx_static_nv_data_ptr->sar_lut_data_switched_ant.sar_lut[num_tx_slots-1][sar_indx-1].sar_8psk;
       MSG_3(MSG_SSID_RF, MSG_LEGACY_MED, "sar for switched antenna, sar_state %d num_tx_slots %d: GMSK - %d", sar_state, num_tx_slots, sar_limit->sar_gmsk);   
     }
#endif
     else
     {
...
void rfgsm_nv_deinit(rfm_device_enum_type rfm_dev)
{
  ...
  for (band = RFGSM_NV_850; band < RFGSM_NV_MAX_SUPPORTED; band++)
  {
    ...
    for (i = 0;i < 2;i++)
    {
      ...
      //Added a check to free up only those buffers which the rfm device supports either rx or tx
      if (rfgsm_nv_data[nv_path].rfgsm_nv_band_node[band].rfgsm_nv_band_rx_data_buffer_ptr != NULL && rfc_common_get_device_capability(rfm_dev) == RFM_DEVICE_RX_SUPPORTED)
      {
        if (rfgsm_nv_data[nv_path].rfgsm_nv_band_node[band].rfgsm_nv_band_rx_data_buffer_ptr->rx_static_nv_data_ptr != NULL)
        {
          modem_mem_free( rfgsm_nv_data[nv_path].rfgsm_nv_band_node[band].rfgsm_nv_band_rx_data_buffer_ptr->rx_static_nv_data_ptr, MODEM_MEM_CLIENT_RFA );
          rfgsm_nv_data[nv_path].rfgsm_nv_band_node[band].rfgsm_nv_band_rx_data_buffer_ptr->rx_static_nv_data_ptr = NULL;
          rfgsm_nv_common_config_storage.rx_buffer_count--;
        }

        modem_mem_free( rfgsm_nv_data[nv_path].rfgsm_nv_band_node[band].rfgsm_nv_band_rx_data_buffer_ptr, MODEM_MEM_CLIENT_RFA );
        rfgsm_nv_data[nv_path].rfgsm_nv_band_node[band].rfgsm_nv_band_rx_data_buffer_ptr = NULL;
      
        rf_gsm_nv_config_storage.allocated_rx_buffers--;
      }
	  
      if (rfgsm_nv_data[nv_path].rfgsm_nv_band_node[band].rfgsm_nv_band_tx_data_buffer_ptr != NULL && rfc_common_get_device_capability(rfm_dev) == RFM_DEVICE_TX_SUPPORTED)
      {
        if (rfgsm_nv_data[nv_path].rfgsm_nv_band_node[band].rfgsm_nv_band_tx_data_buffer_ptr->tx_static_nv_data_ptr != NULL)
        {
          modem_mem_free( rfgsm_nv_data[nv_path].rfgsm_nv_band_node[band].rfgsm_nv_band_tx_data_buffer_ptr->tx_static_nv_data_ptr, MODEM_MEM_CLIENT_RFA );
          rfgsm_nv_data[nv_path].rfgsm_nv_band_node[band].rfgsm_nv_band_tx_data_buffer_ptr->tx_static_nv_data_ptr = NULL;
          rfgsm_nv_common_config_storage.tx_buffer_count--;
        }

        modem_mem_free( rfgsm_nv_data[nv_path].rfgsm_nv_band_node[band].rfgsm_nv_band_tx_data_buffer_ptr, MODEM_MEM_CLIENT_RFA );
        rfgsm_nv_data[nv_path].rfgsm_nv_band_node[band].rfgsm_nv_band_tx_data_buffer_ptr = NULL;
      
        rf_gsm_nv_config_storage.allocated_tx_buffers--;
      }
    }
  }
}
...
void rfgsm_nv_populate_coex_band_map_tbl( void )
{
  ...
  memset( &rx_nv_alloc_info, 0, sizeof(nv_alloc_type) );
  memset( &tx_nv_alloc_info, 0, sizeof(nv_alloc_type) );
...
void rfgsm_nv_data_structure_init(void)
{
   ...
   for(path=0; path <RF_GSM_NV_PATH_MAX; path++)
   {
      for(band=0; band < RFGSM_NV_MAX_SUPPORTED; band++)
      {
		...
		rfgsm_nv_data[path].rfgsm_nv_band_node[band].rfgsm_nv_band_rx_data_buffer_ptr = NULL;
		rfgsm_nv_data[path].rfgsm_nv_band_node[band].rfgsm_nv_band_tx_data_buffer_ptr = NULL;
