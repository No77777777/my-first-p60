...
$Header: //components/rel/rftech_gsm.mpss/3.11/rf/core/src/rfgsm_core_temp_comp.c#2 $
$DateTime: 2022/02/24 02:06:35 $
$Author: mplcsds1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
12/01/21   aad     To fix therm read issue: disabled temp_comp_suspended flag during tx wakeup
09/03/20   mpa     Dummy Checkin
08/31/20   mpa     To handle corner therm case to handle therm trigger after tx sleep
12/23/19   mpa     Enable therm read in HA
12/09/19   mpa     Disable therm read in HA
05/25/16   ds      temparature compensation optimization enhancements
05/05/16   ds      Added code for temparature compensation optimization.
...
09/08/15   sw      New F3 to indicate use of mid-bin in case of therm read failure
...
01/23/14   ec      Use new debug NV mechanism to initialise temp comp logging flag
...
/* ===========================PUBLIC FUNCTIONS ==============================*/
/*! @fn rfgsm_core_prepare_therm_read                                        */
/*! @brief is the device Rx capable                                          */
/*! @details This function returns if the device is Rx capable               */
/*! @param sub_id - subscription executing therm read                        */
/*! @param tx_device - device to build script for                            */
/*! @param shared_mem_ptr - GFW shared memory to store for event creation    */
/*! @retval boolean result - was script creation successful                  */
                                       ...
                                       void * shared_mem_ptr )
{
  ...
  rf_path_enum_type rf_path = RF_PATH_0; //path is unused by device driver

  if( rfgsm_debug_flags.disable_temp_comp == TRUE )
  {
    /* Therm read is being bypassed */
    RF_MSG( MSG_LEGACY_MED, "rfgsm_core_prepare_therm_read() therm read bypassed" );
    return TRUE;
  }

  /* Get core handles and validate */
  rfgsm_core_handle_ptr = rfgsm_core_handle_get( tx_device );
  if( rfgsm_core_handle_ptr == NULL )
  {
    ERR_FATAL( "rfgsm_core_prepare_therm_read() core handle NULL for sub %d dev %d", sub_id, tx_device, 0 );
    return FALSE;
  }
  ...
  {
    ERR_FATAL( "rfgsm_core_prepare_therm_read() shared_mem_ptr NULL for sub %d dev %d", sub_id, tx_device, 0 );
    return FALSE;
  }

  /* Store core handle ptr therm shared mem with shared mem from L1 */
  rfgsm_core_handle_ptr->therm_read.therm_read_shared_mem_ptr = shared_mem_ptr;

  /* Set the GFW therm read trigger UMID based on current context */
  switch( sub_id )
  {
    case RFLM_GSM_SUBSCRIPTION_1:
      rfgsm_core_handle_ptr->therm_read.fw_therm_read_umid = rfgsm_sub_to_therm_umid_map[RF_TASK_1].therm_read_umid;/*GERAN_FW_RFM_THERM_READ_CMD */
      rfgsm_core_handle_ptr->therm_read.fw_therm_rsp_umid = rfgsm_sub_to_therm_umid_map[RF_TASK_1].therm_resp_umid;/*RFLM_GSM_THERM_READ_RSP*/
      break;

    case RFLM_GSM_SUBSCRIPTION_2:
      rfgsm_core_handle_ptr->therm_read.fw_therm_read_umid = rfgsm_sub_to_therm_umid_map[RF_TASK_2].therm_read_umid;/*GERAN_FW2_RFM_THERM_READ_CMD*/
      rfgsm_core_handle_ptr->therm_read.fw_therm_rsp_umid  = rfgsm_sub_to_therm_umid_map[RF_TASK_2].therm_resp_umid;/*RFLM_GSM_THERM_READ_SUB2_RSP*/
      break;

    default:
      ERR_FATAL("rfgsm_core_prepare_therm_read() invalid sub_id %d", sub_id, 0, 0);
      break;
  }

  /* Clear the therm read script buffer */
  if( !rf_buffer_clear( (rf_buffer_intf*)rfgsm_core_handle_ptr->scripts.therm_read_script ) )
  {
    RF_MSG( MSG_LEGACY_ERROR, "RF GSM card therm read script buffer was unsuccessfully cleared" );
  }

  /* Create the therm read script */
  (void)rfdevice_gsm_cmd_dispatch( tx_device,
                                   RF_PATH_0, //unused
                                   RFDEVICE_GSM_BUILD_THERM_READ_SCRIPT,
                                   (rf_buffer_intf*)rfgsm_core_handle_ptr->scripts.therm_read_script );

  return TRUE;
} /* rfgsm_core_prepare_therm_read */
...
boolean rfgsm_core_temp_comp_data_init(rfm_device_enum_type rfm_dev)
{
 ...
 /* Structure to hold temp comp information (default is set to mid-bins) */
  ...
  if((rfgsm_debug_flags.disable_temp_comp == FALSE) && 
     rfgsm_nv_check_debug_nv(rfm_dev, RFGSM_DEBUG_NV_DISABLE_THERM_READ))
  {
    rfgsm_debug_flags.disable_temp_comp = TRUE;
    MSG(MSG_SSID_RF, MSG_LEGACY_MED,"Therm read is disabled via Debug NV!");
  }
...
void rfgsm_core_therm_read_handler(rf_apps_cmd_type* cmd_ptr) 
{
  ...
  if(RF_TASK_1 == rfgsm_rf_task)
  {
    ...
    {
      RFGC_MSG_2( MSG_LEGACY_HIGH,"Not building therm event due to mismatch between UMID [0x%x] and SUB_ID [%d] ",
                                                    rfgsm_core_handle_ptr->therm_read.fw_therm_read_umid, sub_id);
      return;
    }
  }
...
{ 
  ...
  {
    MSG(MSG_SSID_RF, MSG_LEGACY_HIGH,"Invalid therm read value, use middle bin for temperature compensation");
...
boolean rfgsm_core_process_therm_read_rsp(rfgsm_core_data_type *core_data, uint8 tag, boolean therm_read_ok)
{
  ...
  if (therm_read_ok)
  {
	...
	if( rfgsm_mdsp_get_therm_data(core_data, &read_buf[0], tag, &num_rffe_retrieved))
    {
	  ...
	  if( rflm_abs_satw(prev_raw_therm - core_data->rfgsm_core_handle_ptr->therm_read.raw_therm) > RFGSM_CORE_DELTA_OF_THERM_READ )
	  {
...
void rfgsm_core_temp_comp_update_req(rfm_device_enum_type rfm_dev, int16 temp_comp_scaled_value, uint8 temp_bin_16,uint8 sub_id)
{
  ...
  rfa_rf_gsm_update_temp_comp_req.cnf_required = FALSE;
...
void rfgsm_core_temp_comp_tbl_adjust(rfm_device_enum_type rfm_dev)
{
  ...
  for (band = 0; band < RFCOM_BAND_GSM1800_B; band ++)
  {
    ...
    RF_NULL_CHECK_RTN_V(rfgsm_tx_nv_tbl," rfgsm_tx_nv_tbl is NULL! ");

    therm_read = rfgsm_tx_nv_tbl->tx_cal_nv_data.cal_temp.therm_read;

    for (pcl_index = 0; pcl_index < RFNV_NUMBER_OF_PCLS; pcl_index++)
    {
      for (temp_index = 0; temp_index < RFNV_NUMBER_OF_PCLS; temp_index++)
      {
        pcl_vector[temp_index] = rfgsm_tx_nv_tbl->tx_static_nv_data_ptr->enh_temp_comp_data.tempcomp_table_gmsk[temp_index][pcl_index];
      }
      ...
      for (temp_index = 0; temp_index < RFNV_GSM_NUM_TEMP_BIN; temp_index++)
      {
        rfgsm_tx_nv_tbl->tx_static_nv_data_ptr->enh_temp_comp_data.tempcomp_table_8psk[temp_index][pcl_index] -= offset_adjust;
        rfgsm_tx_nv_tbl->tx_static_nv_data_ptr->enh_temp_comp_data.tempcomp_table_gmsk[temp_index][pcl_index] -= offset_adjust;
      }
    }
  }
}
