...
$Header: //components/rel/rftech_gsm.mpss/3.11/rf/core/src/rfgsm_core_vbatt_comp.c#1 $
$DateTime: 2021/03/09 05:10:17 $
$Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
01/23/18   par     Trigger Vbatt Comp if it changes by 50 mv
07/28/15   hk      vbatt backoff per pa range. 
04/14/15   ec      Do not attempt to read ADC if ADC init has failed
01/20/14   ec      Use new debug NV flags for VBatt Comp logging
...
/* To trigger Tx gain config only when Vbatt is varied by a threshold margin */
#define RFGSM_VBATT_CHANGE_THRESHOLD 50
...
void rfgsm_core_vbatt_comp_init( rfm_device_enum_type rfm_dev )
{
  ...
  for( uint8 i=0; i < RFGSM_MAX_TX_GAIN_RANGE; i++ )
  {
    rfgsm_core_handle_ptr->vbatt_comp_info.vbatt_comp_parange_dBx100[i] = 0;
  }
...
void rfgsm_core_vbatt_read_handler(rf_apps_cmd_type* cmd_ptr) 
{
  ...
  if (rfgsm_core_handle_ptr->vbatt_comp_info.vbatt_adc_client_init_status == FALSE)
  {
    MSG( MSG_SSID_RF, MSG_LEGACY_HIGH, "ADC init not done, ADC read will fail. Not updating Vbatt reading." );
  }
  else
  {
  ...
  {
    if (((rfgsm_core_handle_ptr->vbatt_comp_info.vbatt_value_mv - curr_vbatt_read) >=  RFGSM_VBATT_CHANGE_THRESHOLD) ||
         ((curr_vbatt_read - rfgsm_core_handle_ptr->vbatt_comp_info.vbatt_value_mv) >= RFGSM_VBATT_CHANGE_THRESHOLD))
    {
      flag = TRUE;
    }
...
{ 
  ...
  if (vbatt_comp_info_ptr->vbatt_value_mv != last_valid_vbatt_val)
  {
    ...
    if ( temp_vbatt_val >= vbatt_level_mid )
    {
      ...
      // Calculate the per pa state comp only up to the number of PA states supported in NV
      for( uint8 i=0; i<RFCOM_GSM_NUM_VBATT_PA_STATE;i++ )
      {
        vbatt_comp_info_ptr->vbatt_comp_parange_dBx100[i] = (int16)(vbatt_nv_data.vbatt_comp_value_hi.vbatt_comp_parange_dBx100[i] *
                                                                          (temp_vbatt_val - vbatt_level_mid) /
                                                                          (vbatt_level_high - vbatt_level_mid));
      }
    }
    else
    {
      ...
      for( uint8 i=0; i<RFCOM_GSM_NUM_VBATT_PA_STATE;i++ )
      {
        vbatt_comp_info_ptr->vbatt_comp_parange_dBx100[i] = (int16)(vbatt_nv_data.vbatt_comp_value_lo.vbatt_comp_parange_dBx100[i] *
                                                                  (temp_vbatt_val - vbatt_level_mid) /
                                                                  (vbatt_level_low - vbatt_level_mid));
      }
    }
    ...
    if ( enable_logging == TRUE )
    {
      ...
      RFGC_MSG_5( MSG_LEGACY_HIGH, 
                  "Compensated vbatt is %d, per pa_state [0]=%d, [1]=%d, [2]=%d, [3]=%d", 
                  vbatt_comp_info_ptr->vbatt_comp_dBx100,
                  vbatt_comp_info_ptr->vbatt_comp_parange_dBx100[0],
                  vbatt_comp_info_ptr->vbatt_comp_parange_dBx100[1],
                  vbatt_comp_info_ptr->vbatt_comp_parange_dBx100[2],
                  vbatt_comp_info_ptr->vbatt_comp_parange_dBx100[3] );
    }
