...
$Header: //components/rel/rftech_gsm.mpss/3.11/rf/core/src/rfgsm_core_tx_power.c#1 $
$DateTime: 2021/03/09 05:10:17 $
$Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
24/12/18   bp      SAR antenna switch fix 
01/23/18   par     Update LDO mode setting algorithm
12/13/17   mpa     Changes to support RT-SAR
03/08/16   dsm     SAR code antenna switch then disable
02/03/16   dsm     Report to RFM layer to disable asdiv switching on specific sar code
02/03/16   dsm     Support different gains along different ASDIV sw positions
12/07/15   hk      Correcting the datatypes for pcl_pwr correction
09/24/15   zc      Support different gains along different ASDIV sw positions
08/20/15   cpm     use static power_levels table to retrieve pcl_idx bin from power request
08/11/15   cpm     improve processing time in the  MTPL - SAR limiting calculation
07/30/15   ck      add FTM GSM cmd to override QPOET mode and bias voltage
07/29/15   rc      Add power logging support
07/28/15   hk      Enable vbatt_comp per pa_range.
07/15/15   cpm     FR27546: MTPL reported for the top/bottom antennas
06/26/15   ck      Unidirection env_gain to fix tw power discontinuity 
06/19/15   sc      Update and enable optmised Rx burst and feature guard
05/29/15   cpm     make use of true ftm_gsm_get_frame_sweep_cal_running() 
                   to nullify temptr/volt compensation out of TX AGC
06/03/15   cpm     handling of max_pcl_index calculation when no ftm_slot_override[]
06/01/15   cpm     implemented rfgsm_core_tx_get_closest_pcl_index_vs_power
...
03/25/15   hoh     Use max pcl if multislot APT feature is not enabled
01/23/14   ec      Use new debug NV mechanism for logging
...
#define RFGSM_ASDIV_POSITION_MAX  RFM_ANTENNA_1
#define RFGSM_ASDIV_POSITION_0    RFM_ANTENNA_0
//typedef rfcommon_asdiv_position_enum_type rfm_antenna_enum_type;
#else
#define RFGSM_ASDIV_POSITION_MAX  MIN(((rfm_antenna_enum_type)ASDIV_POSITION_MAX), RFM_ANTENNA_MAX)
#define RFGSM_ASDIV_POSITION_0    ((rfm_antenna_enum_type)ASDIV_POSITION_0)
#endif

#define RFGSM_CORE_TX_LOWBAND_SAR_LIMIT_MIN  500  /* 5 dBm -- dB100*/
#define RFGSM_CORE_TX_HIGHBAND_SAR_LIMIT_MIN 0  /* 0 dBm */

#define RFGSM_CORE_TX_LOWBAND_VBATT_LIMIT_MIN  500  /* 5 dBm -- dB100*/
#define RFGSM_CORE_TX_HIGHBAND_VBATT_LIMIT_MIN 0  /* 0 dBm */

#define RFGSM_CORE_MAX_NON_PREDIST_BB_BOOST -100

#ifndef FEATURE_RF_USE_GSM_BIDIRECTIONAL_TX_SCALING
#define RFGSM_CORE_GMSK_ENV_BACKOFF_LIMIT 0
#endif

#define RFGSM_CORE_MAX_ENV_BACKOFF_dBx100 2490
/*@ todo: HEADROOM setting should be exposed to customer as an NV item or by any other means. For now, hardcoding to default 1.2V */
#define RFGSM_VBATT_HEADROOM_VAL 1200
#define RFGSM_VBATT_HEADROOM_MAX 1200
...
inline int32 rfgsm_core_tx_get_power_limits(rfm_device_enum_type rfm_dev,
                                            int32 pwr_in_dbm,
                               ...
                               uint8 num_assigned_uplink_slots)
{
  ...
  /* Assign Tx Data*/
  tx_data.triple_buffer_id = core_data->triple_buffer_id;
  tx_data.pa_ramp_ping_pong_id = core_data->rfgsm_core_handle_ptr->pa_ramp_buffer_id;
  tx_data.amxm_ramp_ping_pong_id = core_data->rfgsm_core_handle_ptr->amam_ampm_buffer_id;
  tx_data.num_tx_slots = num_tx_slots;
  tx_data.gain_data = core_data->rfgsm_core_handle_ptr->rfc_gsmctl_linear_pa_tx_gain_buff;

  /* Set Tx data in RFLM DM */
  if( !rfgsm_core_dm_set_tx_data( core_data->sub_id, &tx_data ) )
  {
    ERR_FATAL( "rfgsm_core_set_tx_power() set Tx data failed for sub %d", 
               core_data->sub_id, 0, 0 );
    return;
  }
}
...
void rfgsm_core_set_simult_vbatt_limit(rfgsm_core_handle_type *rfgsm_core_handle_ptr, int16 vbatt_limit_gmsk[5], int16 vbatt_limit_8psk[5])
{
  uint32 index = 0;
  ...
  for (index = 0; index < RFGSM_MAX_TX_SLOTS_PER_FRAME; index++)
  {
    rfgsm_core_handle_ptr->simult_vbatt_limit_gmsk[index] = vbatt_limit_gmsk[index];
    rfgsm_core_handle_ptr->simult_vbatt_limit_8psk[index] = vbatt_limit_8psk[index];
  }
...
/*!
  @details
   This function gets current Tx pwr vbatt limit for a given rfm device, modulation and no of uplink slot.
   This limit is the value as requested by MCS for current state of all tx chains.

  @param rfm_dev : RFM device for which tx pwr limit is required
               mod_type : 0 for GMSK, 1 for 8PSK
               uplink slots : No of Uplink slots in a frame
               simult_tx_pwr_vbatt_limit : Variable to return tx_pwr_vbatt_limit               
*/

int16 rfgsm_core_get_simult_tx_pwr_vbatt_limit(rfm_device_enum_type rfm_dev, rfgsm_modulation_type mod_type, uint8 uplink_slots)
{

  rfgsm_core_handle_type *rfgsm_core_handle_ptr;
  int16 simult_tx_pwr_vbatt_limit = 0;
  rfgsm_core_handle_ptr = rfgsm_core_handle_get(rfm_dev);
	
  RF_NULL_CHECK_RTN(rfgsm_core_handle_ptr, "rfgsm_core_handle_ptr is NULL!", simult_tx_pwr_vbatt_limit);

  if (mod_type == RF_MOD_GMSK)
  {
    simult_tx_pwr_vbatt_limit = rfgsm_core_handle_ptr->simult_vbatt_limit_gmsk[uplink_slots -1];
  }
  else if (mod_type == RF_MOD_8PSK)
  {
    simult_tx_pwr_vbatt_limit = rfgsm_core_handle_ptr->simult_vbatt_limit_8psk[uplink_slots -1];
  }
  else
  {
    simult_tx_pwr_vbatt_limit = 0;
    RFGC_MSG( MSG_LEGACY_ERROR, "rfgsm_core_get_simult_tx_pwr_limit, bad modulation!" );
  }
  
  return simult_tx_pwr_vbatt_limit;
}
...
void rfgsm_core_compute_pcl_vs_power (rfm_device_enum_type rfm_dev, rfcom_gsm_band_type band, uint16 arfcn)
{
  ...
  if(use_interpolation == TRUE)
  {
    ...
    {
      if(rfgsm_core_handle_ptr->rfgsm_enable_pcl_logging)
      {
        RFGC_MSG( MSG_LEGACY_HIGH, "rfgsm_core_compute_pcl_vs_power, tx_pwr_err valid!" );
      }
  ...
  else
  {
    ...
    if ( tx_cal_nv_data_ptr->gsm_tx_pwr_err_per_pcl.tx_pwr_err_data[freq_index].data_valid == TRUE )
    {
      if(rfgsm_core_handle_ptr->rfgsm_enable_pcl_logging)
      {
        RFGC_MSG( MSG_LEGACY_HIGH,"rfgsm_core_compute_pcl_vs_power tx_pwr_err tx_pwr_err valid!" );
      }
                               ...
                               rfgsm_modulation_type mod_type, uint8 *rgi_ptr)
{
     ...
     if (pmeas_tbl[i] >= rfgsm_pcl_pwr_dbm)
     {
        ...
        else
        {
	     ...
	     rgi_index = rgi_index_hi;
             ...
             if(rfgsm_core_handle_ptr->rfgsm_enable_pcl_logging)
             {
               RFGC_MSG(MSG_LEGACY_HIGH, "UNI_DIRECTIONAL_ENV_GAIN is enabled");
             }
   	   }
           else
#endif
  ...
  */
  ...
  if (mod_type == RF_MOD_GMSK)
  {
/* for uni-directional env_gain feature, boosted env_gain is not allowed for GMSK*/ 
#ifndef FEATURE_RF_USE_GSM_BIDIRECTIONAL_TX_SCALING
    if(delta_dbx100 < RFGSM_CORE_GMSK_ENV_BACKOFF_LIMIT)
    {
      delta_dbx100 = RFGSM_CORE_GMSK_ENV_BACKOFF_LIMIT;
    }
#endif
  ...
  /* power logging : log tx agc data */
  #ifdef FEATURE_RF_POWER_REPORTING
  RFLM_DIAG_POWER_REPORT(rfgsm_core_handle_ptr->txlm_buf_index,
                         rflm_time_read_xo_time(),
                         ...
                         rfgsm_pcl_pwr_dbm/10,      //convert to dbm
  ...
  rflm_dm_get_handle_info(rfgsm_core_handle_ptr->txlm_buf_index, &handle_meta);
  RFGC_MSG_1( MSG_LEGACY_HIGH, "rfgsm_core_set_tx_agc log agc data, handle id %d", rfgsm_core_handle_ptr->txlm_buf_index );
  RFGC_MSG_1( MSG_LEGACY_HIGH, "rfgsm_core_set_tx_agc log agc data, device num %d", handle_meta.device_id );
  #endif
...
int32 rfgsm_core_tx_pwr_in_dbm_get(rfm_device_enum_type rfm_dev, rfgsm_modulation_type mod_type, uint16 pcl, uint8 uplink_slots, uint8 slot_count)
{
  ...
  if(ftm_gsm_tx_device_is_ftm_state(rfm_dev) == TRUE) /*Check if tx device is FTM_STATE_GSM and not FTM_STATE_GSM_NS */
  {
...
/*!
  @brief
  apply the SAR limiting factor and return the power (in dBm100) after applying the SAR limit.

  @details
  the SAR limiting factors are applied ...
  - with a valid SAR state not RF_SAR_STATE_DEFAULT and less or equal to RF_SAR_STATE_MAX
  - with multi-slots TX configuration in range of 1 to 4
  - and not in calibration mode
  
  the power (in dBm100) is limited by the SAR lower limit of multi-slots SAR and/or
  valid SAR switched of the chosen antenna within those conditions.

  the SAR LUT are all retrieved from the GSM static RFNV.

  @param
  'rfm_dev'             - logical TX device
  'pwr_after_sar_limit' - power in dBm100 for which SAR limiting need to be applied.
  'mod_type'            - 0 = GMSK or 1 = 8PSK
  'uplink_slots'        - the number of active tx slots [0 = no TX - RFNV_GSM_SAR_NUM_UL_SLOT_MAX].
  'ant_pos'             - 0 = bottom antenna (PRX) - 1 = top antenna (DRX)

  @retval 
  power_in_dbm':  the tx power in dBm100 after applying the SAR limiting.

*/
int32 rfgsm_core_tx_pwr_apply_sar_limiting
(
 rfm_device_enum_type rfm_dev,
...
)
{
  ...
  int16 sar_limit_other_ant = 0;
  rfgsm_mc_sar_forced_antenna_enum_type sar_forced_antenna = RFGSM_MC_SAR_ANTENNA_NONE;
#endif
  ...
  rfm_antenna_enum_type curr_ant_pos = rfcommon_asdiv_get_current_position(rfm_dev);
  
  rfgsm_core_handle_ptr = rfgsm_core_handle_get(rfm_dev);

  RF_NULL_CHECK_RTN(rfgsm_core_handle_ptr, "rfgsm_core_tx_pwr_apply_sar_limiting():"
                    "rfgsm_core_handle_ptr is NULL. not limiting",
                    pwr_after_sar_limit);

  RF_NULL_CHECK_RTN(rfgsm_core_handle_ptr->rfgsm_tx_nv_tbl,
                    "rfgsm_core_tx_pwr_apply_sar_limiting():"
                    ":rfgsm_tx_nv_tbl is NULL. not limiting",
                    pwr_after_sar_limit);
   ...
   * if sar_index is greater than or equal to RF_SAR_STATE_MAX
   */
  if ((sar_state >= RF_SAR_STATE_MAX) ||
      (0 == uplink_slots) ||
      (uplink_slots > RFNV_GSM_SAR_NUM_UL_SLOT_MAX) ||
      rfm_get_calibration_state())
  {
    rfgsm_mc_set_sar_forced_ant(sar_forced_antenna);
    return pwr_after_sar_limit;
  }
  else if((sar_state == RF_SAR_STATE_DEFAULT) && (simult_sar_limit)) /*Apply RT-SAR limit if needed*/
  {
#ifdef FEATURE_RF_ASDIV
    rfgsm_mc_set_sar_forced_ant(sar_forced_antenna);
#endif
    sar_limit = simult_sar_limit;
    pwr_after_sar_limit = MIN(pwr_after_sar_limit, sar_limit);

/*    if(rfgsm_core_handle_ptr->rfgsm_enable_pcl_logging)
    {
      MSG_7(MSG_SSID_RF, MSG_LEGACY_MED,
          "num_tx_slots=%d, sar_state=%d, ant_pos=%d, "
          "mode=%d, sar_limit=%d, simult_sar_limit=%d, for pwr=%d",
          uplink_slots, sar_state, ant_pos, mod_type, sar_limit, simult_sar_limit, pwr_after_sar_limit);    
    } */
  ...
  RF_NULL_CHECK_RTN(tx_static_nv_data_ptr,
                    "rfgsm_core_tx_pwr_apply_sar_limiting():"
                    ":tx_static_nv_data_ptr is NULL. not limiting",
                    pwr_after_sar_limit);
  
  /* If the sar_state is within the range, cap the output power to the SAR limit NV LUT */
  sar_idx = (uint8)sar_state - 1;
  slots_idx = uplink_slots - 1;
  ...
  switch (mod_type)
  {
    case RF_MOD_GMSK:
        ...
        if(tx_static_nv_data_ptr->use_switched_antenna_sar_lut == TRUE && ant_pos == ASDIV_POSITION_1)
        {          
          sar_limit = tx_static_nv_data_ptr->sar_lut_data_switched_ant.sar_lut[slots_idx][sar_idx].sar_gmsk; 
          sar_limit_other_ant = tx_static_nv_data_ptr->sar_lut_data.sar_lut[slots_idx][sar_idx].sar_gmsk;          
        }
        else
#endif
        {
          sar_limit = tx_static_nv_data_ptr->sar_lut_data.sar_lut[slots_idx][sar_idx].sar_gmsk;
#ifdef FEATURE_RF_ASDIV
          sar_limit_other_ant = tx_static_nv_data_ptr->sar_lut_data_switched_ant.sar_lut[slots_idx][sar_idx].sar_gmsk;
#endif
        }
          
        /* Get the Simultaneous Tx pwr limit according to the modulation and number of UL slots*/
        simult_sar_limit = rfgsm_core_handle_ptr->simult_sar_limit_gmsk[slots_idx];
        break;
    ...
    case RF_MOD_8PSK:
        ...
        if(tx_static_nv_data_ptr->use_switched_antenna_sar_lut == TRUE && ant_pos == ASDIV_POSITION_1)
        {
          sar_limit = tx_static_nv_data_ptr->sar_lut_data_switched_ant.sar_lut[slots_idx][sar_idx].sar_8psk;
          sar_limit_other_ant = tx_static_nv_data_ptr->sar_lut_data.sar_lut[slots_idx][sar_idx].sar_8psk;
        }
        else
#endif
        {
          sar_limit = tx_static_nv_data_ptr->sar_lut_data.sar_lut[slots_idx][sar_idx].sar_8psk;
#ifdef FEATURE_RF_ASDIV
          sar_limit_other_ant = tx_static_nv_data_ptr->sar_lut_data_switched_ant.sar_lut[slots_idx][sar_idx].sar_8psk;
#endif
        }
          
        /* Get the Simultaneous Tx pwr limit according to the modulation and number of UL slots*/
        simult_sar_limit = rfgsm_core_handle_ptr->simult_sar_limit_8psk[slots_idx];
        break;
  ...
  if((sar_limit == (int16)RFGSM_SAR_FORCED_ANT_VALUE) && (sar_limit_other_ant != (int16)RFGSM_SAR_FORCED_ANT_VALUE))
  {
    if(curr_ant_pos == ASDIV_POSITION_0)
    {    
      sar_forced_antenna = RFGSM_MC_SAR_ANTENNA_1;
    }
    else if(curr_ant_pos == ASDIV_POSITION_1)
    {    
      sar_forced_antenna = RFGSM_MC_SAR_ANTENNA_0;
    }
    sar_limit = sar_limit_other_ant; 
  }
  else if( (sar_limit != (int16)RFGSM_SAR_FORCED_ANT_VALUE) && (sar_limit_other_ant == (int16)RFGSM_SAR_FORCED_ANT_VALUE))
  {
  ...
  else if( (sar_limit_other_ant == (int16)RFGSM_SAR_FORCED_ANT_VALUE) && (sar_limit == (int16)RFGSM_SAR_FORCED_ANT_VALUE))
  {
    sar_forced_antenna = RFGSM_MC_SAR_ANTENNA_0;
  }
  else
  {  
    sar_forced_antenna = RFGSM_MC_SAR_ANTENNA_NONE;
  }
  
  rfgsm_mc_set_sar_forced_ant(sar_forced_antenna);
  MSG_4(MSG_SSID_RF, MSG_LEGACY_HIGH, "Setting sar_forced_ant = %d, curr_ant_pos = %d, sar_limit = %d, sar_limit_other_ant = %d", 
        sar_forced_antenna, 
        curr_ant_pos, 
        sar_limit,
        sar_limit_other_ant);
  
  if(((uint16)sar_limit) > RFNV_GSM_SAR_BACK_OFF_LIMIT_DEFAULT)
  {
    sar_limit = RFNV_GSM_SAR_BACK_OFF_LIMIT_DEFAULT;
  }
#endif
...
/*!
   @brief
  apply the VBATT limiting factor and return the pwr_in_dbm after applying the VBATT limit.

   @details
  apply the VBATT limiting factor and return the pwr_in_dbm after applying the SAR limit.
 
   @param 
  'mod_type' - GMSK or 8PSK
  'pwr_in_dbm'- pwr in dBm for which VBATT limiting need to be applied.
  'uplink_slots'  - the number of active tx slots.
 
   @retval 
  power_in_dbm':  the tx power in dBm after applying the VBATT limiting.

*/
int32 rfgsm_core_tx_pwr_apply_vbatt_limiting(rfm_device_enum_type rfm_dev, int32 pwr_in_dbm, rfgsm_modulation_type mod_type, uint8 uplink_slots)
{
  int16 simult_vbatt_limit = 0;
  int32 pwr_after_vbatt_limit;
  rfgsm_core_handle_type *rfgsm_core_handle_ptr;
  rfgsm_core_handle_ptr = rfgsm_core_handle_get(rfm_dev);

  RF_NULL_CHECK_RTN(rfgsm_core_handle_ptr, "rfgsm_core_handle_ptr is NULL!", pwr_in_dbm);

  pwr_after_vbatt_limit = pwr_in_dbm;

  /* Get the Simultaneous Tx vbatt pwr limit according to the modulation and number of UL slots*/
  simult_vbatt_limit = rfgsm_core_get_simult_tx_pwr_vbatt_limit(rfm_dev, mod_type, uplink_slots);

  /* Don't apply VBATT limiting in cal mode - ZGL: TBD by system spec*/
  if( ftm_gsm_is_in_calibration_mode()== FALSE)
  {
     /* Don't allow VBATT limit goes lower than the standard */
     if (rfgsm_core_handle_ptr->tx_band ==RFCOM_BAND_GSM850 || rfgsm_core_handle_ptr->tx_band == RFCOM_BAND_GSM900)
     {
       if (simult_vbatt_limit < RFGSM_CORE_TX_LOWBAND_VBATT_LIMIT_MIN)
       {
     	simult_vbatt_limit = RFGSM_CORE_TX_LOWBAND_VBATT_LIMIT_MIN;
       }
     }
     else
     {
       if (simult_vbatt_limit < RFGSM_CORE_TX_HIGHBAND_VBATT_LIMIT_MIN)
       {
     	simult_vbatt_limit = RFGSM_CORE_TX_HIGHBAND_VBATT_LIMIT_MIN;
       }
     }
     
     /* Cap it to the simult VBATT limit */
     if (pwr_after_vbatt_limit > simult_vbatt_limit)
     {
       pwr_after_vbatt_limit = simult_vbatt_limit;
     }
  }

  return pwr_after_vbatt_limit;
}
                                     ...
                                     rfgsm_modulation_type mod_type)
{
  ...
  {
      RFGC_MSG_5( MSG_LEGACY_HIGH, "DBG: PCL pwr offset = %d, Temp comp power offset = %d, pwr_in_dbm = %d, equivalent pcl_index = %d, scaled_temp_K = %d",
            pcl_pwr_offset, temp_offset, pwr_in_dbm, pcl_index, scaled_temp_K );
      RFGC_MSG_3( MSG_LEGACY_HIGH, "DBG: p1 = %d, p2 = %d, lin_intrep_fac = %d", p1, p2, lin_interp_fac );
  }
...
int16 rfgsm_core_get_enh_temp_comp_pwr_offset(rfm_device_enum_type rfm_dev, int16 scaled_value, uint16 pcl, int16 pwr_in_dbm, rfgsm_modulation_type mod_type)
{
  ...
  if( rfgsm_core_handle_ptr->therm_read.enable_logging == TRUE )
  {
    RFGC_MSG_6( MSG_LEGACY_HIGH, "DBG: pcl_index = %d, Temp_bin = %d, p1_offset = %d, p2_offset = %d, scaled_value = %d, Enhanced Temp Comp power offset = %d",
                                   ...
                                   int16* backoff_params ) 
{
 ...
 if (rfgsm_nv_cmn_tbl_ptr->enh_tempcomp_enable_nv == TRUE)
  ...
  if ( rfgsm_core_handle_ptr->apt_mode == BYPASS_MODE || 
   ...
   {
    // Check if the PA range is < the number supported in NV
    if (pa_range < RFCOM_GSM_NUM_VBATT_PA_STATE)
    {
      vbatt_comp += rfgsm_core_handle_ptr->vbatt_comp_info.vbatt_comp_parange_dBx100[pa_range]; 
    }
    else
    {
      // If not, use the value from the highest supported index
      RF_MSG_3( MSG_LEGACY_HIGH, "pa_range %d not supported by NV, using value from pa_range %d (%d)",
                pa_range, 
                RFCOM_GSM_NUM_VBATT_PA_STATE-1, 
                rfgsm_core_handle_ptr->vbatt_comp_info.vbatt_comp_parange_dBx100[pa_range] );

      vbatt_comp += rfgsm_core_handle_ptr->vbatt_comp_info.vbatt_comp_parange_dBx100[RFCOM_GSM_NUM_VBATT_PA_STATE-1]; 
    }
  ...
  {
    RFGC_MSG_2( MSG_LEGACY_HIGH, "GSM VBatt Comp: vbatt comp: %d, Total vbatt and temp comp:  %d ", vbatt_comp, tx_pwr_offset );
  }
                                     ...
                                     uint8 num_assigned_uplink_slots )
{
  ...
  rfm_antenna_enum_type ant_pos = RFGSM_ASDIV_POSITION_0;
  rfnv_gsm_tx_elna_asdiv_type * rfgsm_nv_tx_elna_asdiv_info = NULL;
  uint16 tx_power_offset_asdiv_db100 = 0;
  ...
  rfgsm_nv_tx_elna_asdiv_info = (rfnv_gsm_tx_elna_asdiv_type*)rfgsm_nv_get_elna_asdiv_info(rfm_dev, rfgsm_core_handle_ptr->tx_band, RFGSM_NV_TX);

  if(rfgsm_nv_tx_elna_asdiv_info != NULL)
  {
#ifdef FEATURE_RF_ASDIV
    ant_pos = rfcommon_asdiv_get_current_position(rfm_dev);
    if( ant_pos >= ASDIV_POSITION_MAX )
    {
      MSG_3( MSG_SSID_RF, MSG_LEGACY_ERROR, 
             "Invalid antenna position %d, defaulting ant pos to %d and Tx pwr offset to %d", 
             ant_pos, RFGSM_ASDIV_POSITION_0, tx_power_offset_asdiv_db100 );
      ant_pos = RFGSM_ASDIV_POSITION_0;
      tx_power_offset_asdiv_db100 = 0;
    }
    else
#endif
    {
      tx_power_offset_asdiv_db100 = rfgsm_nv_tx_elna_asdiv_info->tx_elna_asdiv_gain_offset[ant_pos].switch_point_offset;
    }
  }
  else
  {
    MSG_3( MSG_SSID_RF, MSG_LEGACY_ERROR, 
           "Invalid Tx Gain offset ptr! band %d, dev %d Tx pwr offset %", 
           rfgsm_core_handle_ptr->tx_band, rfm_dev, tx_power_offset_asdiv_db100 );
  }
  ...
  if(ftm_gsm_tx_device_is_ftm_state(rfm_dev) == TRUE) /*Check if tx device is in FTM_STATE_GSM and not FTM_STATE_GSM_NS */
  {
    ...
    for (slot_count = 0; slot_count < RFGSM_MAX_TX_SLOTS_PER_FRAME; slot_count++)
    {
      ...
      if ( ftm_temp->pa_range != 0xFFFF )
      {
        ...
        if(rfgsm_core_handle_ptr->ftm_qpoet_override)
        {
          mode[slot_count] = rfgsm_core_handle_ptr->ftm_apt_mode_override;
          bias[slot_count] = rfgsm_core_handle_ptr->ftm_bias_override;
          RFGC_MSG_2( MSG_LEGACY_HIGH, "overwrite bias %d and mode %d during pa_range override", bias[slot_count], mode[slot_count] );

        }

        rfc_gsm_tx_gain_config( rfm_dev, 
                                rfgsm_core_handle_ptr->tx_band,
                                ftm_temp->modulation, 
                                slot_count, 
                                ftm_temp->tx_pow_dbm, 
                                rfgsm_core_handle_ptr->parange_override_val,
                                bias_flag, 
                                bias[slot_count], 
                                mode[slot_count], 
                                override_start_time,
                                rfgsm_core_handle_ptr->rfgsm_tx_nv_tbl,
                                rfgsm_core_handle_ptr->current_rgi,
                                &rfgsm_core_handle_ptr->tx_log,
  ...
  for (slot_count=0; slot_count < num_tx_slots; slot_count++)
  {
    ...
    if (ftm_gsm_tx_device_is_ftm_state(rfm_dev) == FALSE) /* in SIG/FTM_STATE_GSM_NS states */
    {
    ...
    MSG_3(MSG_SSID_RF, MSG_LEGACY_HIGH, "Power in dBm= %d, ASD power offset = %d, curr_pos =%d", pwr_in_dbm, tx_power_offset_asdiv_db100, ant_pos);
    pwr_in_dbm += tx_power_offset_asdiv_db100;
    ...
    if ((pwr_in_dbm_adjusted <= multislot_max_pwr_dbm_adjusted) ||
        (ftm_gsm_tx_device_is_ftm_state(rfm_dev) == TRUE))/*Check if tx device is FTM_STATE_GSM and not FTM_STATE_GSM_NS */
    {
...
#if defined(FEATURE_RF_ASDIV) || defined(FEATURE_RF_HAS_TPTUNER_SWITCH)
    /* NOTE: 
     * - for the purpose of trade off between performance and precision of measures
     *  the power limits are checked only on the slot of requested maximum power.
     *  A best measure would be to take the minimum of MTPLs calculated on all slots.
     */
    if ((slot_index == slot_count) &&
        (rfgsm_core_handle_ptr->mtpl_update_req))
    {
      for(ant_pos = RFGSM_ASDIV_POSITION_0; ant_pos < RFGSM_ASDIV_POSITION_MAX; ant_pos++)
      {
        rfgsm_core_handle_ptr->mtpl[ant_pos] =
          rfgsm_core_tx_get_power_limits(rfm_dev,
                                         multislot_max_pwr_dbm,
                                         mod_type,
                                         uplink_slots,
                                         ant_pos,
                                         pwr_profile[slot_count].backoff);
      }
    ...
    //note: ant_pos is defaulted to RFGSM_ASDIV_POSITION_0 without ASDIV features
    pwr_in_dbm = rfgsm_core_tx_get_power_limits(rfm_dev,
                                                pwr_in_dbm,
                                                mod_type,
                                                uplink_slots,
                                                ant_pos,
    ...
    if ( 
         ((ftm_gsm_tx_device_is_ftm_state(rfm_dev) == TRUE) && 
    ...
    if (! predist_flag ) /* pre-distortion OFF */
    {
      ...
      if((slot_count == num_tx_slots-1) && rfgsm_core_handle_ptr->selftest_flat_bb_gain) 
      {
        edge_env_gain = 0; 
        RFGC_MSG_1(MSG_LEGACY_HIGH,"flat env gain at slot count %d for gsm fbrx power meas", slot_count);
      }
#endif      
    }
    ...
    if (rfgsm_core_handle_ptr->parange_override_val == 0xFF)
    {
      ...
      /* Enable QFE4100 APT-LDO Mode */
      if (RFGSM_VBATT_HEADROOM_VAL <= RFGSM_VBATT_HEADROOM_MAX)
      {				 
         if ((mode[slot_count] == FORCED_APT_MODE) || (mode[slot_count] == REGULAR_APT_MODE) || (mode[slot_count] == LDO_MODE))
         {		 
            if (rfgsm_core_handle_ptr->vbatt_comp_info.vbatt_value_mv < (bias[slot_count] + (RFGSM_VBATT_HEADROOM_VAL)))
            {
              mode[slot_count] = LDO_MODE;
            }
            else
            {
              mode[slot_count] = FORCED_APT_MODE;
...
/*!
   ...
   functional group which applies in sequence "flat" backoffs, power limits
   to the requested target power on a GSM Tx slot.

   @details
   
   the wanted power (in dBm100) is offset with backoffs 
   RFGSM_COEX_VBATT_BACKOFF,  RFGSM_COEX_DESENSE_BACKOFF,
   and capped with a lower limits of the LUT:
   sar_lut_data.sar_lut,  simult_vbatt_limit.

   @param rfm_dev        -- tx logical device

   @param pwr_in_dbm     -- target power (dBm100) to be corrected

   @param mod_type       -- rfgsm_modulation_type

   @param uplink_slots   -- wanted numbers of contiguous Tx slots

   @param ant_pos        -- antenna position transmit will happen

   @param backoff_params -- sets of TX parameters passed on by GL1

   @retval
   
   corrected Tx power in dBm100

*/


inline int32 rfgsm_core_tx_get_power_limits
(
 rfm_device_enum_type rfm_dev,
 int32 pwr_in_dbm,
 rfgsm_modulation_type mod_type,
...
{
  int32 pwr_sar_in_dbm;
  int32 pwr_vbatt_in_dbm;
  
  /* Subtract the coex_backoff (if any) from the calculated power before anything else happens */
  pwr_in_dbm -= rfgsm_core_get_coex_backoff(pwr_in_dbm, backoff_params);

  /* apply SAR limiting */
  pwr_sar_in_dbm = rfgsm_core_tx_pwr_apply_sar_limiting(rfm_dev, pwr_in_dbm, mod_type, uplink_slots, ant_pos);

  /* apply VBATT limiting */
  pwr_vbatt_in_dbm = rfgsm_core_tx_pwr_apply_vbatt_limiting(rfm_dev, pwr_in_dbm, mod_type, uplink_slots);
  
  pwr_in_dbm = MIN(pwr_vbatt_in_dbm, pwr_sar_in_dbm);
