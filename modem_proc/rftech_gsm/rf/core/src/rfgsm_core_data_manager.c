...
$Header: //components/rel/rftech_gsm.mpss/3.11/rf/core/src/rfgsm_core_data_manager.c#1 $
...
01/23/18   par     Adding APT scripsts in Tx stop to move to STANDBY 
05/05/16   hk      Store and dump CCS build script errors
02/29/16   dsm     Fix bug that prevents events being built if first script 
                   pointer is NULL
12/03/15   tws     Move fwd/rev coupler events to TX frontend config.
11/10/15   tws     Add FBRX VSWR support
10/07/15   tws     Add API to set TX_STOP CCS event priority to be used in 
                   HMSC case
09/25/15   tws     Add FBRX VSWR support
...
06/11/15   ra      Clean up rflm ccs event structure
05/27/15   sc      Calculate Rx burst data for optimised Rx burst API
04/22/15   zbz     Add DRx scripts when building Rx wake up event
04/22/15   sc      Remove unused functionality
04/15/15   piy     Added a check for UMID for both ASD Response and Device Status Read
04/09/15   piy     Fixed bug in Device status read feature for Mon Burst
04/07/15   sc      Add interface index update for Rx/Tx wakeup
                                                    ...
                                                    
{
  ...
  if(((rflm_ccs_rf_event_description_buffer_header_type*)event_mem_ptr)->event_status != RFLM_CCS_RF_EVENT_IDLE && 
     ((rflm_ccs_rf_event_description_buffer_header_type*)event_mem_ptr)->event_status != RFLM_CCS_RF_EVENT_COMPLETE)
  {
    /* @Todo Introduce an error count here */
    RF_MSG_1(MSG_LEGACY_ERROR,
             "THERM READ SBI Event status not in IDLE or COMPLETE, status = %d", 
             ((rflm_ccs_rf_event_description_buffer_header_type*)event_mem_ptr)->event_status);
                                                 ...
                                                 void* event_mem_ptr )
{
  ...
  */
  ((rflm_ccs_rf_event_description_buffer_header_type*)event_mem_ptr)->response_required = TRUE;
 
  /* the UMID of ASD RSP*/
  ((rflm_ccs_rf_event_description_buffer_header_type*)event_mem_ptr)->scratch_mem[0]=(uint32)core_data->rfgsm_core_handle_ptr->therm_read.fw_therm_rsp_umid;
 
  /* Store device ID and event_handle_tag for retrieval on therm read response */ 
  /* @todo Replace device with sub-id */
  ((rflm_ccs_rf_event_description_buffer_header_type*)event_mem_ptr)->scratch_mem[1]=(uint32)(((uint32)core_data->rfm_dev<<16)|
  ...
  switch(script_type)
  {
    case RFLM_GSM_TX_STOP_SCRIPT:
	  if((core_data->rfgsm_core_handle_ptr->device_config_to_read.params[RF_TX_BURST].enable == TRUE) && (task_q == RFLM_GSM_CCS_TASK_Q_A))
	  {
        ...
        ((rflm_ccs_rf_event_description_buffer_header_type*)event_mem_ptr)->response_required = TRUE;
        ((rflm_ccs_rf_event_description_buffer_header_type*)event_mem_ptr)->scratch_mem[0] = (uint32)core_data->rfgsm_core_handle_ptr->device_config_to_read.device_status_rsp_umid;
        ((rflm_ccs_rf_event_description_buffer_header_type*)event_mem_ptr)->scratch_mem[1] = (uint32)(((uint32)core_data->rfm_dev<<28) |
	  ...
	  if((core_data->rfgsm_core_handle_ptr->device_config_to_read.params[RF_TX_BURST].enable_grfc_read == TRUE) && (task_q == RFLM_GSM_CCS_TASK_Q_B))
	  {
		...
		((rflm_ccs_rf_event_description_buffer_header_type*)event_mem_ptr)->response_required = FALSE;
	    ((rflm_ccs_rf_event_description_buffer_header_type*)event_mem_ptr)->scratch_mem[1] = core_data->rfgsm_core_handle_ptr->device_config_to_read.params[RF_TX_BURST].enable_grfc_read;	    
      }
      break;
    ...
    case RFLM_GSM_RX_PCH_STOP_SCRIPT:
	  ...
	  {
		((rflm_ccs_rf_event_description_buffer_header_type*)event_mem_ptr)->response_required = FALSE;
        ((rflm_ccs_rf_event_description_buffer_header_type*)event_mem_ptr)->scratch_mem[1] = (uint32)(core_data->rfgsm_core_handle_ptr->device_config_to_read.params[RF_RX_BURST].enable_grfc_read);     	
	  }
                                                 ...
                                                 void* event_mem_ptr )
{
  ...
  /* the UMID of ASD RSP*/
  ((rflm_ccs_rf_event_description_buffer_header_type*)event_mem_ptr)->scratch_mem[0]=(uint32)core_data->rfgsm_core_handle_ptr->asd_fw_rsp_umid;
  ...
  ((rflm_ccs_rf_event_description_buffer_header_type*)event_mem_ptr)->scratch_mem[1]=(uint32)(&core_data->rfgsm_core_handle_ptr->asd_switch_info);
...
static rflm_gsm_intf_data* rfgsm_core_dm_get_rflm_intf_data( uint32 intf_idx )
{
  ...
  if( intf_idx >= (RFGSM_CORE_DM_MAX_INTF_DATA_ENTRIES + RFLM_GSM_DM_MAX_UNIQUE_INDEXES) )
  {
...
/*===========================================================================*/
/*! @fn rfgsm_core_dm_populate_rx_wakeup_intf_data
 *  @brief This function populates the Rx wakeup interface data
 *  @details This function populates the command-specific interface data in
 *           the internally-managed RFLM GSM intf data index location
 *  @retval - boolean success status
*/
                                                    ...
                                                    void *shared_mem_ptr )
{
  ...
  /* Populate the FW shared mem index value */
  if( !rfgsm_mdsp_populate_rx_wakeup_index( shared_mem_ptr, idx ) )
  {
    MSG(MSG_SSID_RF, MSG_LEGACY_ERROR,"RF GSM core DM Rx Wakeup FW intf data index update failed");
    return FALSE;
  }
  ...
  if( !rfgsm_core_dm_populate_rflm_intf_data( core_data,
                                              ...
                                              idx ) )
  {
    MSG(MSG_SSID_RF, MSG_LEGACY_ERROR,"RF GSM core DM Rx Wakeup FW intf data update failed");
    return FALSE;
  }
...
/*===========================================================================*/
/*! @fn rfgsm_core_dm_populate_tx_wakeup_intf_data
 *  @brief This function populates the Tx wakeup interface data
 *  @details This function populates the command-specific interface data in
 *           the internally-managed RFLM GSM intf data index location
 *  @retval - boolean success status
*/
                                                    ...
                                                    void *shared_mem_ptr )
{
  ...
  /* Populate the FW shared mem index value */
  if( !rfgsm_mdsp_populate_tx_wakeup_index( shared_mem_ptr, idx ) )
  {
    MSG(MSG_SSID_RF, MSG_LEGACY_ERROR,"RF GSM core DM Tx Wakeup FW intf data index update failed");
    return FALSE;
  }
  ...
  if( !rfgsm_core_dm_populate_rflm_intf_data( core_data,
                                              ...
                                              idx ) )
  {
    MSG(MSG_SSID_RF, MSG_LEGACY_ERROR,"RF GSM core DM Tx Wakeup FW intf data update failed");
    return FALSE;
  }
...
{
  /* Get the current intf data index */
  static uint32 idx[RFLM_GSM_MAX_SUBSCRIPTIONS] = 
  {
    RFLM_GSM_DM_THERM_READ_INTF_IDX_SUB_1,
    RFLM_GSM_DM_THERM_READ_INTF_IDX_SUB_2
  };

  /* Validate the therm read subscription ID */
  if( core_data->sub_id >= RFLM_GSM_MAX_SUBSCRIPTIONS )
  {
    MSG_1( MSG_SSID_RF, MSG_LEGACY_ERROR,
           "RF GSM core DM Therm read intf data invalid sub ID %d", 
           core_data->sub_id );
    return FALSE;
  }

  /* Populate the FW shared mem index value */
  if( !rfgsm_mdsp_populate_therm_read_index( shared_mem_ptr, idx[core_data->sub_id] ) )
  {
                                                   ...
                                                   uint32 rf_sequence_number)
{
  ...
  if( !rfgsm_mdsp_populate_rx_start_index( shared_mem_ptr, idx, rf_sequence_number ) )
  {
                                                   ...
                                                   uint32 rf_sequence_number)
{
  ...
  if( !rfgsm_mdsp_populate_tx_start_index( shared_mem_ptr, idx, rf_sequence_number ) )
  {
                                               ...
                                               rfgsm_core_dm_task_q task_q )
{
  ...
  case RFLM_GSM_RX_WAKEUP_SCRIPT:
    ...
    {
	  event_addr = (void*)&table_ptr->rfgsm_rx_wakeup_event.task_q_a;
    }
    else if( task_q == RFLM_GSM_CCS_TASK_Q_B )
    {
	  event_addr = (void*)&table_ptr->rfgsm_rx_wakeup_event.task_q_b;
    }
    break;
  ...
  case RFLM_GSM_TX_WAKEUP_SCRIPT:
    if( task_q == RFLM_GSM_CCS_TASK_Q_A )
    {
      event_addr = (void*)&table_ptr->rfgsm_tx_wakeup_event.task_q_a;
    }
    else if( task_q == RFLM_GSM_CCS_TASK_Q_B )
    {
      event_addr = (void*)&table_ptr->rfgsm_tx_wakeup_event.task_q_b;
    }
    break;
...
/*---------------------------------------------------------------------------*/
/*! @fn rfgsm_core_dm_set_rx_burst_data
 *  @brief 
 *  @param sub_id - subscription
 *  @param rx_data - pointer to gain, LIF and rsb data
 *  @return boolean - function success status
 */
                                         ...
                                         rfgsm_core_dm_rx_burst_data * rx_data )
{
  ...
  {
    MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, 
          "RF GSM core DM set Rx burst data table_ptr NULL for sub %d",
          sub_id);
    return FALSE;
  }

  /* Validate the Rx burst type */
  if ( rx_data->rx_burst_type >= RFGSM_MAX_RX_BURST_TYPES || 
       rx_data->rx_burst_type <= RFGSM_RX_DEFAULT_RX_BURST )
  {
    MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, "RF GSM core DM Rx burst type %d invalid", rx_data->rx_burst_type);
    return FALSE;
  }

  /* Validate the Rx data */
  if( !rfgsm_core_dm_validate_rx_params( rx_data->buffer_id, 
                                         rflm_gsm_rx_start_script_map[rx_data->rx_burst_type], 
                                         rx_data->burst_num ) )
  {
    MSG(MSG_SSID_RF, MSG_LEGACY_ERROR, "RF GSM core DM Rx param error");
    return FALSE;
  ...
  if( rx_data_ptr == NULL )
  {
    MSG_3(MSG_SSID_RF, MSG_LEGACY_ERROR, 
         ...
         rx_data->buffer_id, rflm_gsm_rx_start_script_map[rx_data->rx_burst_type], rx_data->burst_num);
    return FALSE;
  }

  /* Set gain and rsb coefficient data */
  if( rx_data->is_prx ) 
  {
    rx_data_ptr->rf_gain_prx   = rx_data->gain;
    rx_data_ptr->rsb_coefA_prx = rx_data->rsb_coefA;
    rx_data_ptr->rsb_coefB_prx = rx_data->rsb_coefB;
    rx_data_ptr->rflm_enh_rx_on_flag     = rx_data->enh_rx_on_flag;
    rx_data_ptr->rflm_enh_rx_offset_freq = rx_data->enh_rx_freq_offset;
  }
  else
  {
    rx_data_ptr->rf_gain_drx   = rx_data->gain;
    rx_data_ptr->rsb_coefA_drx = rx_data->rsb_coefA;
    rx_data_ptr->rsb_coefB_drx = rx_data->rsb_coefB;
  }
...
/*---------------------------------------------------------------------------*/
/*! @fn rfgsm_core_dm_set_tx_data
 *  @brief 
 *  @param sub_id - subscription
 *  @param tx_data - triple_buffer_id
 *                 - pa_ramp_ping_pong_id
 *                 - amxm_ramp_ping_pong_id
 *                 - num_tx_slots
 *                 - gain_data
 *  @return boolean - function success status
 */
                                   ...
                                   rfgsm_core_dm_tx_data * tx_data )
{
  ...
  uint32 index;
  rflm_gsm_data_type * table_ptr = NULL;
  rflm_gsm_tx_data * tx_burst_data = NULL;
  uint8 pa_ramp_ping_pong_id = tx_data->pa_ramp_ping_pong_id;
  uint8 amxm_ping_pong_id = tx_data->amxm_ramp_ping_pong_id;
  rflm_gsm_pa_ramp_type * pa_ramps;

  /* Check the burst buffer ID is valid */
  if( (tx_data->triple_buffer_id >= RFLM_GSM_TRIPLE_BUFFER) && 
      (tx_data->triple_buffer_id != RFLM_GSM_BUFFER_ID_NOT_REQUIRED) )
  {
    MSG_1( MSG_SSID_RF, MSG_LEGACY_ERROR, 
           "RF GSM core DM set Tx data triple_buffer_id %d invalid", 
           tx_data->triple_buffer_id );
    return FALSE;
  }

  /* Verify the PA and AMXM ramp ping-pong indexes */
  if( pa_ramp_ping_pong_id >= RFGSM_CORE_MAX_DOUBLE_BUF || 
      amxm_ping_pong_id >= RFGSM_CORE_MAX_DOUBLE_BUF )
  {
    MSG_2( MSG_SSID_RF, MSG_LEGACY_ERROR, 
           "RF GSM core DM set Tx data ping pong buffer PA %d or AMXM %d invalid", 
           pa_ramp_ping_pong_id, amxm_ping_pong_id );
    return FALSE;
  }

  /* Ensure number of uplink slots in the frame is 4 or less */
  if( tx_data->num_tx_slots > RFLM_GSM_MAX_TX_UL_SLOTS )
  {
    MSG_1( MSG_SSID_RF, MSG_LEGACY_ERROR, 
           "RF GSM core DM set Tx data num slots %d invalid", 
           tx_data->num_tx_slots );
    return FALSE;
  }
  ...
  {
    MSG_1( MSG_SSID_RF, MSG_LEGACY_ERROR, 
           "RF GSM core DM set Tx data table_ptr NULL for sub %d",
           sub_id );
    return FALSE;
  }

  /* Assign Tx burst DM memory location to place data */
  tx_burst_data = &table_ptr->rfgsm_burst_event[tx_data->triple_buffer_id].tx;

  /* Assign the double buffered AMAM/AMPM tables' pointers */
  tx_burst_data->amam_table = table_ptr->rfgsm_txfe_data.amam_ampm_tables[amxm_ping_pong_id].amam_table;
  tx_burst_data->ampm_table = table_ptr->rfgsm_txfe_data.amam_ampm_tables[amxm_ping_pong_id].ampm_table;

  /* Validate */
  if ( tx_burst_data->amam_table == NULL || tx_burst_data->ampm_table == NULL )
  {
    MSG(MSG_SSID_RF, MSG_LEGACY_ERROR, "RF GSM core DM set Tx amam/ampm tables amam or ampm table NULL");
    return FALSE;
  }

  /* Initialise RFLM program AMXM flag to FALSE */
  tx_burst_data->program_amam_ampm_tables = FALSE;

  /* Set the slot-specific Tx data */
  for( index = 0; index < tx_data->num_tx_slots; index++ )
  {
    /* Get the PA ramp tables */
    pa_ramps = &table_ptr->rfgsm_txfe_data.pa_frame_ramps[pa_ramp_ping_pong_id].pa_slot_ramps[index];

    /* Assign the double buffered PA ramp tables' pointers */
    tx_burst_data->pa_ramp_up_profile[index]   = pa_ramps->pa_ramp_up_table;
    tx_burst_data->pa_ramp_down_profile[index] = pa_ramps->pa_ramp_down_table;

    /* Assign the envelope gain */
    tx_burst_data->env_gain[index] = tx_data->gain_data[index].txCfg2GainSetupEnvGain;

    /* Assign the predistortion enabled flag */
    tx_burst_data->predist_enabled[index] = tx_data->gain_data[index].txCfg2GainSetupPredist;

    /* If any of the slots have predistortion enabled set AMXM flag */
    /* to inform RFLM to prepare predistortion TxFE settings */
    tx_burst_data->program_amam_ampm_tables |= tx_data->gain_data[index].txCfg2GainSetupPredist;

    /* Set the Tx power of the slot */
    tx_burst_data->tx_pwr_level_dbm10[index] = RFGSM_DM_DBM100_TO_DBM10(tx_data->gain_data[index].txPower);
  }
                                                    ...
                                                    uint8 burst_num )
{
  ...
  uint8 index = 0;
  void* event_mem_ptr = NULL;
  boolean build_event = FALSE;

  /* If script buffer pointer is NULL, nothing to process - return from this function */
  if ( NULL == ccs_scripts )
  {
    MSG(MSG_SSID_RF, MSG_LEGACY_ERROR, "CCS scripts pointer is NULL");
    return FALSE;
  }

  // Iterate over the script pointers to check for anything to be programmed
  for (index = 0; index < num_scripts; index++) 
  {
    if( ccs_scripts[index] != NULL )
    {
      build_event = TRUE;
      break;
    }
  ...
  // All of the script pointers were NULL (ie nothing to program), so don't build the event. 
  // This is not an error, so return TRUE
  if (build_event == FALSE)
  {
    return TRUE;
  }
  ...
  for (index = 0; index < num_scripts ; index++) 
  {
    ...
    {
      if( !rfgsm_core_dm_push_script( *event_handle_ptr , RF_HAL_BUS_RESOURCE_RFFE, ccs_scripts[index], script_type, task_q, core_data->gl1_lower_rx_ccs_prio ) )
      {
        MSG(MSG_SSID_RF, MSG_LEGACY_ERROR, "RF GSM core DM dynamic event wrapper push failed");
        status &= FALSE;
      }

      if( !rfgsm_core_dm_push_script( *event_handle_ptr , RF_HAL_BUS_RESOURCE_GRFC, ccs_scripts[index], script_type, task_q, core_data->gl1_lower_rx_ccs_prio ) )
      {
        MSG(MSG_SSID_RF, MSG_LEGACY_ERROR, "RF GSM core DM dynamic event wrapper push failed");
        status &= FALSE;
      }
...
/*===========================================================================*/
/*! @fn rfgsm_core_dm_dynamic_rx_wakeup_event_wrapper                        */
/*! @brief This function acts as a wrapper for the dynamic event method to be*/
/*!        backwards compatible with the old event building method           */
boolean rfgsm_core_dm_dynamic_rx_wakeup_event_wrapper( rfgsm_core_data_type *core_data )
{
  rf_buffer_intf* rx_wakeup_ccs_scripts_taskq_a[] = {(rf_buffer_intf *)core_data->rfgsm_core_handle_ptr->scripts.enter_mode_script,
  ...
  if( !rfgsm_core_dm_dynamic_event_wrapper( core_data,
                                            RFLM_GSM_RX_WAKEUP_SCRIPT,
                                            RFLM_GSM_CCS_TASK_Q_A,
                                            (rf_buffer_intf**)rx_wakeup_ccs_scripts_taskq_a,
                                            sizeof(rx_wakeup_ccs_scripts_taskq_a)/sizeof((rf_buffer_intf**)rx_wakeup_ccs_scripts_taskq_a),
                                            NULL,
                                            0 ) )
  {
...
/*===========================================================================*/
/*! @fn rfgsm_core_dm_dynamic_tx_wakeup_event_wrapper                        */
/*! @brief This function acts as a wrapper for the dynamic event method to be*/
/*!        backwards compatible with the old event building method           */
boolean rfgsm_core_dm_dynamic_tx_wakeup_event_wrapper( rfgsm_core_data_type *core_data )
{
  rf_buffer_intf* tx_wakeup_ccs_scripts_taskq_a[] = {(rf_buffer_intf *)core_data->rfgsm_core_handle_ptr->scripts.enter_mode_script,
                                                       NULL};
  ...
  if( !rfgsm_core_dm_dynamic_event_wrapper( core_data,
                                            RFLM_GSM_TX_WAKEUP_SCRIPT,
                                            RFLM_GSM_CCS_TASK_Q_A,
                                            (rf_buffer_intf**)tx_wakeup_ccs_scripts_taskq_a,
                                            sizeof(tx_wakeup_ccs_scripts_taskq_a)/sizeof((rf_buffer_intf**)tx_wakeup_ccs_scripts_taskq_a),
                                            NULL,
                                            0 ) )
  {
...
/*===========================================================================*/
/*! @fn rfgsm_core_dm_set_rx_rf_sequence_num                                 */
/*! @brief This function updates the RFLM DM rf sequence number passed from  */
/*!        GL1                                                               */
/*! @param core_data - Core data populated based in GL1 data in cmd proc     */
/*! @param rx_burst_type - Rx type passed from GL1 determines memory region  */
/*! @param rx_burst_num - Rx burst number from GL1                           */
/*! @param rf_sequence_number - sequence number passed from GL1 per Rx       */
/*! @return boolean - API success/failure                                    */
                                              ...
                                              uint32 rf_sequence_number )
{
  ...
  {
    MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, 
          "Set Rx RF sequence num table_ptr NULL for sub %d",
          core_data->sub_id);
    return FALSE;
  }
  ...
  {
    MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, 
          "Set Rx RF sequence num Rx burst type %d invalid", 
          rx_burst_type);
    return FALSE;
  }
  ...
  {
    MSG_3(MSG_SSID_RF, MSG_LEGACY_ERROR, 
          "Set Rx RF sequence num rx_data_ptr NULL for burst_buffer_id %d rx start script %d rx_burst_num %d",
          core_data->triple_buffer_id, rflm_gsm_rx_start_script_map[rx_burst_type], rx_burst_num);
    return FALSE;
  }

  /* Set RF sequence number */
  rx_data_ptr->rf_sequence_number = rf_sequence_number;
...
/*===========================================================================*/
/*! @fn rfgsm_core_dm_set_tx_rf_sequence_num                                 */
/*! @brief This function updates the RFLM DM rf sequence number passed from  */
/*!        GL1                                                               */
/*! @param core_data - Core data populated based in GL1 data in cmd proc     */
/*! @param rf_sequence_number - sequence number passed from GL1 per Tx frame */
/*! @return boolean - API success/failure                                    */
                                              ...
                                              uint32 rf_sequence_number )
{
  ...
  {
    MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, 
          "Set Tx RF sequence num table_ptr NULL for sub %d",
          core_data->sub_id);
    return FALSE;
  }

  /* Set RF sequence number */
  table_ptr->rfgsm_burst_event[core_data->triple_buffer_id].tx.rf_sequence_number = rf_sequence_number;
                                                ...
                                                uint8 num_tx_slots )
{
  uint32 index = 0;
  rfgsm_mdsp_ccs_handles_s* ccs_handles = rfgsm_mdsp_ccs_handles[core_data->rfm_dev];
  static uint8 coupler_index[3] = {0,0,0};
  static boolean coupler_indicies_configured = FALSE;
  ...
  rf_buffer_intf* tx_frontend_config_ccs_scripts_taskq_b[] = {NULL, // Placeholder for coupler FWD script
                                                              (rf_buffer_intf *)core_data->rfgsm_core_handle_ptr->scripts.tx_start_script[TX_ATUNER_SCRIPT],
                                                              NULL,  // Placeholder for coupler REV script
                                                              NULL};
  ...
  rf_buffer_intf* tx_stop_ccs_scripts_taskq_b[] = {(rf_buffer_intf *)core_data->rfgsm_core_handle_ptr->scripts.tx_stop_script[TX_ASM_SCRIPT],
                                                   ...
                                                   (rf_buffer_intf *)core_data->rfgsm_core_handle_ptr->scripts.tx_stop_script[TX_APT_SCRIPT],
                                                   (rf_buffer_intf *)core_data->rfgsm_core_handle_ptr->scripts.tx_stop_script[TX_GRFC_SCRIPT],
                                                   NULL, // Placeholder for coupler OPEN script
                                                   NULL};
  ...
  // First time this function is run, find the where coupler scripts are to be inserted
  if( coupler_indicies_configured == FALSE )
  {
    while ( tx_frontend_config_ccs_scripts_taskq_b[coupler_index[0]] != NULL ) { coupler_index[0]++; }
    coupler_index[1] = coupler_index[0] + 1;
    while ( tx_frontend_config_ccs_scripts_taskq_b[coupler_index[1]] != NULL ) { coupler_index[1]++; }
    while ( tx_stop_ccs_scripts_taskq_b[coupler_index[2]] != NULL ) { coupler_index[2]++; }

    coupler_indicies_configured = TRUE;
  }

  // If VSWR measurement has been requested add the coupler scripts to the ccs events at the placeholder NULL
  if( ( core_data->rfgsm_core_handle_ptr->coupler_switch_disabled == FALSE &&
        ( rfgsm_core_dm_vswr_get_meas_requested(core_data->sub_id) || 
          core_data->rfgsm_core_handle_ptr->enable_fbrx_every_burst ) ) )
  {
    tx_frontend_config_ccs_scripts_taskq_b[coupler_index[0]] = (rf_buffer_intf *)core_data->rfgsm_core_handle_ptr->scripts.tx_start_script[TX_COUPLER_FWD_SCRIPT];
    tx_frontend_config_ccs_scripts_taskq_b[coupler_index[1]] = (rf_buffer_intf *)core_data->rfgsm_core_handle_ptr->scripts.tx_start_script[TX_COUPLER_REV_SCRIPT]; 
    tx_stop_ccs_scripts_taskq_b[coupler_index[2]]   = (rf_buffer_intf *)core_data->rfgsm_core_handle_ptr->scripts.tx_stop_script[TX_COUPLER_OPEN_SCRIPT];
  }
...
boolean rfgsm_core_dm_dynamic_therm_event_wrapper( rfgsm_core_data_type *core_data )
{
  ...
  if( !rfgsm_core_dm_push_script(event_handle, RF_HAL_BUS_RESOURCE_RFFE, script, RFLM_GSM_THERM_READ_SCRIPT, RFLM_GSM_CCS_TASK_Q_A, FALSE) )
  {
  ...
  if( !rfgsm_core_dm_push_script(event_handle, RF_HAL_BUS_RESOURCE_GRFC, script, RFLM_GSM_THERM_READ_SCRIPT, RFLM_GSM_CCS_TASK_Q_A, FALSE) )
  {
...
/*===========================================================================*/
/*!   @fn rfgsm_core_get_intf_data_ptr_address                               */
/*!  @brief  - Returns the address of  rflm_gsm_intf_data_ptr                */
/*!  @return rflm_gsm_intf ** - address of rflm_gsm_intf_data_ptr            */
...
/*===========================================================================*/
/*! @fn rfgsm_core_dm_set_vswr_meas_state_sleep                              */
/*! @brief This function sets the VSWR meas state to idle.                  */
                                           ...
                                           rflm_gsm_vswr_meas_state_type state )
{
  ...
  {
    MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, 
          "RF GSM core DM set VSWR meas state table_ptr NULL for sub %d",
          sub_id);
    return FALSE;
  }

  table_ptr->rfgsm_fbrx_data.vswr_meas_state = state;

  RF_MSG_1( RF_HIGH, "GSM VSWR meas: state set to %d", state );
...
/*===========================================================================*/
/*! @fn rfgsm_core_dm_set_vswr_meas_period                                   */
/*! @brief This function sets the VSR measurement period in DM               */
                                          ...
                                          uint32 fbrx_buf_index )
{
  ...
  {
    MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, 
          "RF GSM core DM set fbrx_buf_index table_ptr NULL for sub %d",
          sub_id);
    return FALSE;
  }

  table_ptr->rfgsm_fbrx_data.fbrx_buf_index = fbrx_buf_index;
                                            ...
                                            uint16 fbrx_ldo_settle_time_qs )
{
  ...
  {
    MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, 
          "RF GSM core DM arm VSWR meas table_ptr NULL for sub %d",
          sub_id);
    return FALSE;
  }

  table_ptr->rfgsm_fbrx_data.vswr_meas_duration_qs = vswr_meas_duration_qs;
  table_ptr->rfgsm_fbrx_data.fbrx_ldo_settle_time_qs = fbrx_ldo_settle_time_qs;
...
/*===========================================================================*/
/*! @fn rfgsm_core_dm_vswr_use_extended_alpha                                */
/*! @brief This function sets TRUE if GFW should be using extended alpha for 
  VSWR measurements */
boolean rfgsm_core_dm_vswr_get_meas_requested( uint8 sub_id )
{
  /* Initialise variables */
  rflm_gsm_data_type *table_ptr = rflm_gsm_dm_get_table_ptr( sub_id );

  RF_NULL_CHECK_RTN(table_ptr, "RF GSM core DM arm VSWR meas table_ptr is NULL!", FALSE);

  // return TRUE if we are in the MEAS_REQUESTED state
  return( table_ptr->rfgsm_fbrx_data.vswr_meas_state == RFLM_GSM_VSWR_MEAS_REQUESTED );
}
