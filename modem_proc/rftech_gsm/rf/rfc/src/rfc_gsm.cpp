/*!
   ...
   @todo reduce input parameters to RFC functions
         saving from not calling core handles outweighs additional variables but
         this is hard to read and hard to maintain. Either structure inputs to
         be created or split API into smaller functions.
*/
...
$Author: pwbldsvc $
$Header: //components/rel/rftech_gsm.mpss/3.11/rf/rfc/src/rfc_gsm.cpp#1 $

when       who     what, where, why
-------------------------------------------------------------------------------
04/03/20   mpa     To call asm sleep in tx sleep as well
...
03/09/18   kpa     [CR2190910]Support of configuring the multiple tuner in same path
03/01/18   mpa     To support RFFE QAT3522 ATUNER
01/23/18   par     Enable QFE4100 APT-LDO Mode: GSM Tech changes
10/05/16   kpa     Reduced the coupler rev offset from 325 to 260
05/02/16   hk      PGSM support after TXRX split.
04/22/16   par     Update GSM Tx frequency in KHz to return to ATUNER manager
04/22/16   par     Update GSM Rx frequency in KHz to return to ATUNER manager
03/17/16   tws     Disable tuner when device changes 
03/03/16   hkm     force crash when asm script building fails
02/29/16   dsm     Changes for putting GSM front end components to sleep after X2G IRAT
02/29/16   dsm     Put eLNA to sleep at end of Rx burst and fix bug in rfc sleep api
11/26/15   ndb     Move the Dummy tuner manager creation outside of ASDIV
...
08/30/15   hj      Add tuner disable to sleep_device_config() for IRAT
08/10/15   hoh     Fix issue of GSM not calling Atuner disable during sleep
...
07/08/15   sg      Change the logic in the RFC devices sleep/wakeup handling 
                   to sleep all the device instances that have been woken up
07/07/15   hoh     Pass device info into antenna tuner mgr for sleep config
07/01/15   hoh     Changes to support eLNA programming for GSM
...
05/15/15   hoh     Remove hardcoding and pass arfcn to tuner
...
01/30/15   sw      Disable tuner devices for all bands during GSM Sleep
...
01/07/14   zbz     Fix bug which prevented multislot bias being programmed 
12/08/14   zbz     Fix KW
...
01/08/15   cpm     [RUMI] reenabled init_rfdevice_for_all_bands()
...
/*----------------------------------------------------------------------------*/
/*  VSWR Coupler script offset                                                */
/*----------------------------------------------------------------------------*/
/*! @brief Set coupler in REV in the middle of the training sequence */
#define RFGSM_COUPLER_REV_OFFSET_QS  (260) 

/*! @brief Set the coupler in forward position 150qs before the start of the burst */
#define RFGSM_COUPLER_FWD_OFFSET_QS   (-150) 

/*! @brief Set the coupler in open position at the end of the burst */
#define RFGSM_COUPLER_OPEN_OFFSET_QS  (0)
...
// Get the address of rfc_gsm_ptr
QSH_MDUMP_FN_ATTR rfc_gsm ** rfc_gsm::get_instance_address (void)
{
   return &rfc_gsm_ptr;
}
...
rfc_gsm::rfc_gsm()
{
  ...
  for (log_dev = 0; log_dev < RFM_MAX_WAN_DEVICES; log_dev++)
  {
    ...
    rx_ant_tuner_curr_dev_list[log_dev] = NULL;
    rx_ant_tuner_curr_band[log_dev].gsm_band = RFCOM_NUM_GSM_BANDS;
    tx_ant_tuner_curr_dev_list[log_dev] = NULL;
    tx_ant_tuner_curr_band[log_dev].gsm_band = RFCOM_NUM_GSM_BANDS;
  }
...
void rfc_gsm::config_mode( rfm_device_enum_type rfm_dev, rf_buffer_intf* enter_mode_buffer, rfdevice_rx_mode_type rfdevice_op_mode, uint8 alt_path)
{
  ...
  rfcom_band.gsm_band = RFCOM_NUM_GSM_BANDS;
  rfcommon_atuner_config_req_type atuner_config_p = 
        {rfm_dev, RFM_EGSM_MODE, RFCMN_ATUNER_CARRIER_ID_0, ((rfdevice_op_mode == RFDEVICE_PRX_OPER_MODE) ? RFCMN_ATUNER_PRX_DEVICE : RFCMN_ATUNER_DRX_DEVICE), rfcom_band, 0}; 
  ...
  for (band_local=0; band_local<(int)RFCOM_NUM_GSM_BANDS; band_local++)
  {
    ...
    cfg.band = band_local;
    rfcom_band.gsm_band = (rfcom_gsm_band_type)band_local;
    atuner_config_p.band = rfcom_band;
    ...
    if (dev_list != NULL)
    {
      ...
      {
        ret_val = rfcommon_atuner_init((rfcommon_atuner_intf *)dev_list[index], &atuner_config_p, &script_token, RFDEVICE_CREATE_SCRIPT, enter_mode_buffer, 0);
        if (ret_val && (script_token != ATUNER_INVALID_TOKEN_VAL))
        {
          rfcommon_atuner_acknowledge_script((rfcommon_atuner_intf *)dev_list[index], script_token);
        }
    ...
    if (dev_list != NULL)
    {
      ...
      {
        wakeup_device_config(dev_list[index], enter_mode_buffer, FALSE, rfdevice_op_mode);
        index++;
      }
    }
	
    /*Wakeup all Ant Switch Devices*/
    ant_switch_device_wakeup(rfm_dev, (rfcom_gsm_band_type)cfg.band, enter_mode_buffer, RFC_CONFIG_RX);
  }

  //avoid executing below code for Tx twice by checking for PRx
  if (rfdevice_op_mode == RFDEVICE_PRX_OPER_MODE)
  {
    ...
    for (band_local=0; band_local<(int)RFCOM_NUM_GSM_BANDS; band_local++)
    {
      ...
      rfcom_band.gsm_band = (rfcom_gsm_band_type)band_local;
      atuner_config_p.band = rfcom_band;
      atuner_config_p.rx_tx = RFCMN_ATUNER_TX_DEVICE;
...
void rfc_gsm::rx_burst_config(rfm_device_enum_type rfm_dev,
                              ...
                              uint16 arfcn,
                               ...
                               rfgsm_mode_type gsm_mode,
                               boolean rfgsm_ant_switch_in_progress )
{
  uint8 i = 0;
  ...
  rf_buffer_clear_script((rf_buffer_intf *)scripts->rx_start_script[RX_GRFC_SCRIPT],
                         RF_BUFFER_ALL);
  rf_buffer_clear_script((rf_buffer_intf *)scripts->rx_stop_script[RX_GRFC_SCRIPT],
                         RF_BUFFER_ALL);

  /*Clear Rx asm scripts befor populating*/
  rf_buffer_clear_script((rf_buffer_intf *)scripts->rx_start_script[RX_ASM_SCRIPT],
                         RF_BUFFER_ALL);
  rf_buffer_clear_script((rf_buffer_intf *)scripts->rx_stop_script[RX_ASM_SCRIPT],
                         RF_BUFFER_ALL);

  /* Clear Tuner scripts before populating */
  rf_buffer_clear_script((rf_buffer_intf *)scripts->rx_start_script[RX_ATUNER_SCRIPT],
                         RF_BUFFER_ALL);
  rf_buffer_clear_script((rf_buffer_intf *)scripts->rx_stop_script[RX_ATUNER_SCRIPT],
                         RF_BUFFER_ALL);

#ifdef FEATURE_RF_ASDIV 
  /* Clear XSW scripts before populating */
  if(!rf_buffer_clear_script( (rf_buffer_intf *)scripts->rx_start_script[RX_XSW_SCRIPT], RF_BUFFER_ALL)||
   !rf_buffer_clear_script( (rf_buffer_intf *)scripts->rx_stop_script[RX_XSW_SCRIPT], RF_BUFFER_ALL))
  {
  ...
  rfc_common_tdd_update_grfc_script( sig_ptr,
                                     (void *)scripts->rx_start_script[RX_GRFC_SCRIPT],
                                     (void *)scripts->rx_stop_script[RX_GRFC_SCRIPT],
                                     TRUE /* gnss_blanking_enabled */ );
  ...
  for (i = 0; i < RFDEVICE_TYPE_MAX_NUM; i++)
  {
    ...
    if (dev_list != NULL)
    {
      if((rfdevice_type_enum_type)i != RFDEVICE_TUNER_MANAGER)
      {
        uint8 index = 0;
        while (dev_list[index] != NULL && index < RFC_MAX_DEVICE_INSTANCES_PER_BAND)
        {
          status = rx_burst_device_config(rfm_dev, dev_list[index], rfgsm_band, sig_ptr, arfcn, probe_burst, set_antenna_position, lna_range, scripts, rfdevice_op_mode,gsm_mode, rfgsm_ant_switch_in_progress );
          if (status == FALSE)
          {
            MSG_3( MSG_SSID_RF, MSG_LEGACY_ERROR,
                   "Failed to configure Rx rf-devices! rfm_dev=%d,band=%d,dev_type=%d",
      ...
      {
        status = rx_burst_tuner_device_config(rfm_dev, dev_list, rfgsm_band, sig_ptr, arfcn, probe_burst, set_antenna_position, scripts, rfdevice_op_mode,gsm_mode );
        if (status == FALSE)
        {
          MSG_3( MSG_SSID_RF, MSG_LEGACY_ERROR,
                 "Failed to configure Rx rf-devices! rfm_dev=%d,band=%d,dev_type=%d",
                                         ...
                                         boolean rfgsm_ant_switch_in_progress )
{
  ...
  rfcommon_atuner_config_req_type atuner_config_p = 
        {rfm_dev, RFM_EGSM_MODE, RFCMN_ATUNER_CARRIER_ID_0, 
          ((rfdevice_op_mode == RFDEVICE_PRX_OPER_MODE) ? RFCMN_ATUNER_PRX_DEVICE : RFCMN_ATUNER_DRX_DEVICE),
          rfcom_band, arfcn}; 
  ...
  case RFDEVICE_ASM:
    ...
    asm_cfg_s.rfgsm_ant_switch_in_progress = rfgsm_ant_switch_in_progress;

    rf_common_enter_critical_section( &(((rfdevice_asm *)dev_obj)->rfdevice_critical_sect));

    if(!rfdevice_asm_enable(&asm_cfg_s,
                        dev_obj,
                        (rf_buffer_intf *)scripts->rx_start_script[RX_ASM_SCRIPT],
                        RFDEVICE_CREATE_SCRIPT,
                        timing_info.start_delta))
    {
      ERR_FATAL("fail to build asm enable script for rx start",0,0,0);
    }

    asm_cfg_s.trigger_type = RFDEVICE_ASM_TRIGGER_DISABLE_RX;
    if(!rfdevice_asm_disable(&asm_cfg_s,
                         dev_obj,
                         (rf_buffer_intf *)scripts->rx_stop_script[RX_ASM_SCRIPT],
                         RFDEVICE_CREATE_SCRIPT,
                         timing_info.stop_delta))
    {
      ERR_FATAL("fail to build asm disable script for rx stop",0,0,0);
    }
  ...
  case RFDEVICE_LNA:
    /* Use ASM timing control for eLNA, no need for separate timing per RF System team */
    rfc_common_tdd_get_grfc_info(sig_ptr, RFC_TIMING_ASM_CTL, &timing_info);

    /* Lowest LNA gain range is 1; subtract 1 to map to eLNA gain range */
    if(lna_range > 0)
    {
      lna_range--;
    }

    /* Build eLNA programming script based on current system gain */
    rfdevice_gsm_lna_set_gain(rfm_dev, 
                  band, 
                  (rf_buffer_intf *)scripts->rx_start_script[RX_ASM_SCRIPT], 
                  RFDEVICE_CREATE_SCRIPT, 
                  timing_info.start_delta, 
    ...
    /* Program eLNA to sleep state during Rx Stop. Sleep setting to be populated by customer */
    rfdevice_gsm_lna_sleep(rfm_dev, 
               band, 
               (rf_buffer_intf *)scripts->rx_stop_script[RX_ASM_SCRIPT], 
               RFDEVICE_CREATE_SCRIPT, 
               timing_info.stop_delta, 
    ...
    /* Program eLNA to bypass mode (mapped to high WTR LNA gain) during Rx stop */
    /* rfdevice_gsm_lna_set_gain(rfm_dev, 
              band, 
              (rf_buffer_intf *)scripts->rx_stop_script[RX_ASM_SCRIPT], 
              RFDEVICE_CREATE_SCRIPT, 
  ...
  //case RFDEVICE_TUNER_MANAGER: refer rx_burst_tuner_device_config()
...
/*!
   ...
   device type, calls a specific API to configure the device for the specified rx band.
*/
boolean rfc_gsm::rx_burst_tuner_device_config( rfm_device_enum_type rfm_dev,
                                         ...
                                         rfgsm_mode_type gsm_mode )
{
  ...
  rf_buffer_intf *fe_buffer;
  rfcom_band.gsm_band = band;
  uint8 index = 0,index_old=0,index_new=0;;
  boolean tuner_match=FALSE;
  rfcommon_atuner_config_req_type atuner_config_p = 
        {rfm_dev, RFM_EGSM_MODE, RFCMN_ATUNER_CARRIER_ID_0, 
          ((rfdevice_op_mode == RFDEVICE_PRX_OPER_MODE) ? RFCMN_ATUNER_PRX_DEVICE : RFCMN_ATUNER_DRX_DEVICE),
          rfcom_band, arfcn}; 
  ...
  if (NULL == dev_list)
  {
    MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR,"rf-front-end device NOT Created! dev_list = NULL", 0);
    return FALSE;
  }

    fe_buffer = (rf_buffer_intf *)scripts->rx_start_script[RX_ATUNER_SCRIPT];
#ifdef FEATURE_RF_ASDIV
    if( (probe_burst == RFGSM_RX_PROBE_BURST )
        || (set_antenna_position != RF_INVALID_VALUE_8BIT))
    {
    ...
    // Set the frequency for the antenna tuner
    atuner_config_p.freq = rfgsm_core_convert_arfcn_to_rx_freq(rfcom_band.gsm_band,arfcn)/1000;

    /*For IRAT/QTA mode:Call the Tuneaway*/
    if( gsm_mode == RFGSM_IRAT_MODE )
    {
      while (dev_list[index] != NULL && index < RFC_MAX_DEVICE_INSTANCES_PER_BAND)
      {
        // Tune the antenna away to the requested band/frequency
        ret_val = rfcommon_atuner_tune_away( (rfcommon_atuner_intf *)dev_list[index], 
                                           &atuner_config_p, 
                                           &script_token, 
                                           RFDEVICE_CREATE_SCRIPT, 
                                           fe_buffer, 
    ...
    {
      if(((rfcommon_atuner_intf *)rx_ant_tuner_curr_dev_list[rfm_dev])&&((rfcommon_atuner_intf *)rx_ant_tuner_curr_dev_list[rfm_dev] != (rfcommon_atuner_intf *)dev_list))
      {
        while ((rfcommon_atuner_intf *)rx_ant_tuner_curr_dev_list[rfm_dev][index_old] != NULL && index_old < RFC_MAX_DEVICE_INSTANCES_PER_BAND)
        {
          // The tuner manager needs to know the old band and PRX/DRX mode
          rfcommon_atuner_config_req_type atuner_disable_config =
                {rfm_dev, RFM_EGSM_MODE, RFCMN_ATUNER_CARRIER_ID_0, 
                 atuner_config_p.rx_tx, rx_ant_tuner_curr_band[rfm_dev], 0 };
 
          // Disable the old tuner device as we've switched to a different one
          ret_val = rfcommon_atuner_disable( (rfcommon_atuner_intf *) rx_ant_tuner_curr_dev_list[rfm_dev][index_old], 
                                           &atuner_disable_config, 
                                           &script_token,
                                           RFDEVICE_CREATE_SCRIPT, 
                                           fe_buffer, 
                                           timing_info.start_delta);
          ...
          if( ret_val && (script_token != ATUNER_INVALID_TOKEN_VAL) )
          {
              rfcommon_atuner_acknowledge_script( (rfcommon_atuner_intf *)rx_ant_tuner_curr_dev_list[rfm_dev][index_old], 
                                                script_token );
          }
          MSG_1(MSG_SSID_RF, MSG_LEGACY_HIGH, "rfc_gsm::disabled old RX tuner device 0x%x", rx_ant_tuner_curr_dev_list[rfm_dev][index_old]);
          index_old++;
        }
      }
      index_new = 0;
      while (dev_list[index_new] != NULL && index_new < RFC_MAX_DEVICE_INSTANCES_PER_BAND)
      {
        // Set the antenna tuner for GSM RX
        ret_val = rfcommon_atuner_set_rx_mode( (rfcommon_atuner_intf *)dev_list[index_new], 
                                             &atuner_config_p, 
                                             &script_token, 
                                             RFDEVICE_CREATE_SCRIPT, 
                                             fe_buffer, 
        ...
        if( ret_val && (script_token != ATUNER_INVALID_TOKEN_VAL) )
        {
          rfcommon_atuner_acknowledge_script( (rfcommon_atuner_intf *)dev_list[index_new], script_token );
        }
        index_new++;
      }
    } // ^ Normal mode  

    // Store the current device and band to check next burst
    rx_ant_tuner_curr_band[rfm_dev] =rfcom_band;
    rx_ant_tuner_curr_dev_list[rfm_dev]=dev_list;
    status = ret_val;
                             ...
                             rf_buffer_intf* tx_band_script)
{
  ...
  if( rfm_dev >= RFM_MAX_WAN_DEVICES )
  {
    MSG_1(MSG_SSID_RF, MSG_LEGACY_FATAL, "tx_band_config() invalid dev %d", rfm_dev);
    return;
  }
...
*/
boolean rfc_gsm::tx_band_device_config( rfm_device_enum_type rfm_dev, 
                                        rfdevice_class* dev_obj, 
                                        rfcom_gsm_band_type band,
                                        rf_buffer_intf* band_buffer)
{
  ...
  case RFDEVICE_PAPM:
    ...
    rfdevice_papm_set_mode_bias_w_time( dev_obj,
		                                rfm_dev,
		                                RFM_EGSM_MODE,
		                                rfcom_band,
                               ...
                               rfgsm_modulation_type mod_type)
{
  ...
  rf_buffer_clear_script((rf_buffer_intf *)scripts->tx_start_script[TX_GRFC_SCRIPT], RF_BUFFER_ALL);
  rf_buffer_clear_script((rf_buffer_intf *)scripts->tx_stop_script[TX_GRFC_SCRIPT],  RF_BUFFER_ALL);

  /*Clear the ASM device scripts before populating*/
  rf_buffer_clear_script((rf_buffer_intf *)scripts->tx_start_script[TX_ASM_SCRIPT], RF_BUFFER_ALL);
  rf_buffer_clear_script((rf_buffer_intf *)scripts->tx_stop_script[TX_ASM_SCRIPT],  RF_BUFFER_ALL);

  /*Clear the PA device scripts before populating*/
  rf_buffer_clear_script((rf_buffer_intf *)scripts->tx_start_script[TX_PA_SCRIPT], RF_BUFFER_ALL);
  rf_buffer_clear_script((rf_buffer_intf *)scripts->tx_stop_script[TX_PA_SCRIPT],  RF_BUFFER_ALL);

  /*Clear the ATUNER device scripts before populating*/
  rf_buffer_clear_script((rf_buffer_intf *)scripts->tx_start_script[TX_ATUNER_SCRIPT], RF_BUFFER_ALL);

  /*Clear the COUPLER device scripts before populating*/
  rf_buffer_clear_script((rf_buffer_intf *)scripts->tx_start_script[TX_COUPLER_FWD_SCRIPT], RF_BUFFER_ALL);
  rf_buffer_clear_script((rf_buffer_intf *)scripts->tx_start_script[TX_COUPLER_REV_SCRIPT], RF_BUFFER_ALL);
  rf_buffer_clear_script((rf_buffer_intf *)scripts->tx_stop_script[TX_COUPLER_OPEN_SCRIPT], RF_BUFFER_ALL);
  ...
  if (tx_supported == TRUE)
  {
    ...
    rfc_common_tdd_update_grfc_script(rfc_gsm_tx_sig_list_ptrs[rfm_dev][rfgsm_band],
                                      (void *)scripts->tx_start_script[TX_GRFC_SCRIPT],
                                      (void *)scripts->tx_stop_script[TX_GRFC_SCRIPT], 
                                      rfcommon_get_gnss_blanking_is_enabled( RFCOM_GSM_MODE, rfgsm_band ) );
    ...
    for (i = 0; i < RFDEVICE_TYPE_MAX_NUM; i++)
    {
      ...
      if (dev_list != NULL)
      {
        if((rfdevice_type_enum_type)i != RFDEVICE_TUNER_MANAGER)
        {
          ...
          while (dev_list[index] != NULL && index < RFC_MAX_DEVICE_INSTANCES_PER_BAND)
          {
            status = tx_burst_device_config(rfm_dev, dev_list[index], rfgsm_band, arfcn, txlm_buf_index, rfgsm_tx_nv_tbl, scripts, mod_type);
        ...
        {
          status = tx_burst_tuner_device_config(rfm_dev, dev_list, rfgsm_band, arfcn, txlm_buf_index, rfgsm_tx_nv_tbl, scripts);
          if (status == FALSE)
          {
             MSG_3(MSG_SSID_RF, MSG_LEGACY_ERROR,"Failed to configure Tx rf-devices! rfm_dev=%d,band=%d,dev_type=%d",
                   cfg.logical_device, cfg.band, (rfdevice_type_enum_type)i);
...
void rfc_gsm::append_txd_grfc(uint8 command, rf_buffer_intf *buffer, void *cb_data)
{
  rfc_sig_cfg_type *sig_ptr = NULL;
...
void rfc_gsm::append_asdiv_rffe(uint8 command, rf_buffer_intf *buffer, void *cb_data)
{
  ...
  {
    RF_MSG( RF_ERROR, "fc_gsm - append_asdiv_rffe: CB Data is null " );
    return;
  }
    ...
    while (index < device_info_ptr->num_devices)
    {
      ...
      case RFDEVICE_XSW:
        ...
        if( device_obj != NULL )
        {
		  ...
		  status = rfdevice_xsw_set_cross_switch_config(&xsw_config,(void*)device_obj,buffer,(rf_device_execution_type)immediate,xsw_script_data->start_delta);   
                  xsw_config.trigger=RFDEVICE_XSW_TRIGGER_ALL;		  
		  status = rfdevice_xsw_trigger(&xsw_config,(void*)device_obj,buffer,(rf_device_execution_type)immediate,xsw_script_data->start_delta);	
        }
        else
        {
          status = FALSE ;
...
void rfc_gsm::get_timing_info( rfm_device_enum_type rfm_dev, 
                               rfcom_gsm_band_type rfgsm_band, 
                               rfdevice_type_enum_type dev_type, 
                               rfc_signal_type sig_info, 
                               int16 * start_delta)
{
                                        ...
                                        rfgsm_modulation_type mod_type)
{
  ...
  rfcom_band.gsm_band = band;
  rfcommon_atuner_config_req_type atuner_config_p = 
        {rfm_dev, RFM_EGSM_MODE, RFCMN_ATUNER_CARRIER_ID_0, RFCMN_ATUNER_TX_DEVICE, rfcom_band, arfcn}; 

  /* Get device type from device object after validation */
  if (NULL == dev_obj)
  {
    MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR,"rf-front-end device NOT Created! dev_obj = NULL", 0);
    return FALSE;
  ...
  case RFDEVICE_ASM:
    ...
    asm_cfg_s.rfgsm_ant_switch_in_progress = FALSE;

    rf_common_enter_critical_section( &(((rfdevice_asm *)dev_obj)->rfdevice_critical_sect));

    if(!rfdevice_asm_enable(&asm_cfg_s,
                        dev_obj,
                        (rf_buffer_intf *)scripts->tx_start_script[TX_ASM_SCRIPT],
                        RFDEVICE_CREATE_SCRIPT,
                        timing_info.start_delta))
    {
      ERR_FATAL("fail to build asm enable script for tx start",0,0,0);
    }
    ...
    if(!rfdevice_asm_disable(&asm_cfg_s,
                         dev_obj,
                         (rf_buffer_intf *)scripts->tx_stop_script[TX_ASM_SCRIPT],
                         RFDEVICE_CREATE_SCRIPT,
                         timing_info.stop_delta))
    {
      ERR_FATAL("fail to build asm disable script for tx stop",0,0,0);
    }
  ...
  case RFDEVICE_PA:
    ...
    pa_params_s.rfm_device = rfm_dev;
    pa_params_s.is_mod_type_gmsk = (mod_type == RF_MOD_GMSK)?TRUE:FALSE;

    rfdevice_set_pa_on_off( &pa_params_s, TRUE, (void *)dev_obj,
                            (rf_buffer_intf *)scripts->tx_start_script[TX_PA_SCRIPT],
                            RFDEVICE_CREATE_SCRIPT,
                            timing_info.start_delta);

    rfdevice_set_pa_on_off( &pa_params_s, FALSE, (void *)dev_obj,
                            (rf_buffer_intf *)scripts->tx_stop_script[TX_PA_SCRIPT],
                            RFDEVICE_CREATE_SCRIPT,
                            timing_info.stop_delta);
  ...
  case RFDEVICE_COUPLER:
    // Only build the scripts if the coupler switch is enabled
    rfdevice_coupler_tech_cfg_type coupler_cfg;

    coupler_cfg.mode = RFM_EGSM_MODE;
    coupler_cfg.band.gsm_band = band;
    coupler_cfg.gain = RFDEVICE_COUPLER_GAIN_0;
    coupler_cfg.port = 0;

    coupler_cfg.direction = RFDEVICE_COUPLER_DIRECTION_FWD;
    status &= rfdevice_coupler_config( dev_obj,
                                       coupler_cfg,
                                       (rf_buffer_intf *)scripts->tx_start_script[TX_COUPLER_FWD_SCRIPT],
                                       RFDEVICE_CREATE_SCRIPT,
                                       RFGSM_COUPLER_FWD_OFFSET_QS );

    coupler_cfg.direction = RFDEVICE_COUPLER_DIRECTION_REV;
    status &= rfdevice_coupler_config( dev_obj,
                                       coupler_cfg,
                                       (rf_buffer_intf *)scripts->tx_start_script[TX_COUPLER_REV_SCRIPT],
                                       RFDEVICE_CREATE_SCRIPT,
                                       RFGSM_COUPLER_REV_OFFSET_QS );

    coupler_cfg.direction = RFDEVICE_COUPLER_DIRECTION_OPEN;
    status &= rfdevice_coupler_config( dev_obj,
                                       coupler_cfg,
                                       (rf_buffer_intf *)scripts->tx_stop_script[TX_COUPLER_OPEN_SCRIPT],
                                       RFDEVICE_CREATE_SCRIPT,
                                       RFGSM_COUPLER_OPEN_OFFSET_QS );

    status = TRUE;
    break;

  /*case RFDEVICE_TUNER_MANAGER: refer tx_burst_tuner_devbice_config()*/
  case RFDEVICE_TRANSCEIVER:
  case RFDEVICE_TRANSMITTER:
  case RFDEVICE_RECEIVER:
  case RFDEVICE_TUNER:
...
/*!
   ...
   device type, calls a specific API to configure the device for the specified rx band.
*/
boolean rfc_gsm::tx_burst_tuner_device_config( rfm_device_enum_type rfm_dev,
                                        rfdevice_class **dev_list,
                                        rfcom_gsm_band_type band,
                                        uint16 arfcn,
                                        uint32 txlm_buf_index,
                                        ...
                                        rfgsm_mdsp_scripts_ptrs * scripts)
{
  ...
  cl_info.cl_nv_ctrl_info  = &rfgsm_tx_nv_tbl->tx_static_nv_data_ptr->ant_tuner_cl_data;

  // Get the tuner timing, ie how long before burst the tuner is configured
  rfc_common_tdd_get_grfc_info(rfc_gsm_tx_sig_list_ptrs[rfm_dev][band],
                                 RFC_TIMING_TUNER_CTL, 
                                 &timing_info);

  // Set the frequency for the antenna tuner
  atuner_config_p.freq = rfgsm_core_convert_arfcn_to_tx_freq(rfcom_band.gsm_band,arfcn)/1000;

  if(((rfcommon_atuner_intf *)rx_ant_tuner_curr_dev_list[rfm_dev])&&(rfcommon_atuner_intf *)rx_ant_tuner_curr_dev_list[rfm_dev] != (rfcommon_atuner_intf *)dev_list)
  {
    while ((rfcommon_atuner_intf *)rx_ant_tuner_curr_dev_list[rfm_dev][index_old] != NULL && index_old < RFC_MAX_DEVICE_INSTANCES_PER_BAND)
    {
      // Config for disabling the old tuner, use the old band
      rfcommon_atuner_config_req_type atuner_disable_config =
              { rfm_dev, RFM_EGSM_MODE, RFCMN_ATUNER_CARRIER_ID_0, 
                  RFCMN_ATUNER_TX_DEVICE, tx_ant_tuner_curr_band[rfm_dev], 0 };

      // Disable the old tuner device as we've switched to a different one
      status = rfcommon_atuner_disable( (rfcommon_atuner_intf *) tx_ant_tuner_curr_dev_list[rfm_dev][index_old], 
                                        &atuner_disable_config, 
                                        &script_token,
                                        RFDEVICE_CREATE_SCRIPT, 
                                        (rf_buffer_intf *)scripts->tx_start_script[TX_ATUNER_SCRIPT], 
                                        timing_info.start_delta);

      // Acknowledge the script 
      if( status && (script_token != ATUNER_INVALID_TOKEN_VAL) )
      {
        rfcommon_atuner_acknowledge_script( (rfcommon_atuner_intf *) tx_ant_tuner_curr_dev_list[rfm_dev][index_old], 
                                            script_token );
      }

      MSG_1(MSG_SSID_RF, MSG_LEGACY_HIGH, "rfc_gsm::disabled old TX tuner device 0x%x", tx_ant_tuner_curr_dev_list[rfm_dev][index_old]);
      index_old++;
    }
  }
  ...
  while (dev_list[index_new] != NULL && index_new < RFC_MAX_DEVICE_INSTANCES_PER_BAND)
  {
    ...
    // Set the antenna tuner for TX 
    status = rfcommon_atuner_set_tx_mode( (rfcommon_atuner_intf *)dev_list[index_new], 
                                          &atuner_config_p, 
                                          &cl_info, 
                                          &script_token, 
                                          RFDEVICE_CREATE_SCRIPT, 
  ...
  // Store the current device and band to check next burst
  tx_ant_tuner_curr_dev_list[rfm_dev] = dev_list;
  tx_ant_tuner_curr_band[rfm_dev] = rfcom_band;
...
*/
void rfc_gsm::tx_gain_override( rfm_device_enum_type rfm_dev, 
                                rfcom_gsm_band_type band,
                                rfgsm_modulation_type mod_type, 
                                uint8 pa_range,
                                boolean bias_flag,
                                ...
                                rfgsm_mdsp_scripts_ptrs * scripts )
{
  ...
  rf_buffer_intf * tx_trans_buffer = NULL;
  ...
  tx_trans_buffer = (rf_buffer_intf *)scripts->tx_trans_script[TX_TRANS_FE_SCRIPT][0];
  ...
  if( rfm_dev >= RFM_MAX_WAN_DEVICES )
  {
    MSG_1(MSG_SSID_RF, MSG_LEGACY_FATAL, "tx_gain_override() invalid dev %d", rfm_dev);
    return;
  }

  if (rfgsm_tx_nv_tbl != NULL)
  {
    pa_range_map = rfgsm_tx_nv_tbl->tx_static_nv_data_ptr->pa_range_map;
  ...
  for (i = 0; i < RFDEVICE_TYPE_MAX_NUM; i++)
  {
    ...
    if (dev_list != NULL)
    {
      while (dev_list[index] != NULL && index < RFC_MAX_DEVICE_INSTANCES_PER_BAND)
      {
        status = tx_gain_device_config( rfm_dev, 
                                        dev_list[index], 
                                        ...
                                        tx_trans_buffer, 
                                        0,
                                        (rf_buffer_intf *)scripts->tx_start_script[TX_APT_SCRIPT],
                                        bias_flag, 
                                        bias, 
                                        apt_mode, 
                                        override_start_time, 
...
/*!
  ...
  logic information to the specified buffer.
*/
void rfc_gsm::tx_gain_config( rfm_device_enum_type rfm_dev, 
                              rfcom_gsm_band_type band,
                              rfgsm_modulation_type mod_type, 
                              uint8 slot_num, 
                              int32 pwr_in_dbm,
                              ...
                              rfgsm_mdsp_scripts_ptrs * scripts )
{
  ...
  tx_trans_buffer = (rf_buffer_intf *)scripts->tx_trans_script[TX_TRANS_FE_SCRIPT][slot_num];

  if( rfm_dev >= RFM_MAX_WAN_DEVICES )
  {
    MSG_1(MSG_SSID_RF, MSG_LEGACY_FATAL, "tx_gain_config() invalid dev %d", rfm_dev);
    return;
  }
  ...
  if((rfgsm_tx_nv_tbl != NULL) && (rfc_gsm_gtr_threshold_grfc_info[rfm_dev].grfc_id != RF_HAL_GRFC_MAX_NUM))
  {
    gsm_tx_gtr_thresh_type gtr_thresh = rfgsm_tx_nv_tbl->tx_static_nv_data_ptr->tx_gtr_thresh;
  ...
  if (slot_num)
  {
    apt_script_buf = (rf_buffer_intf *)scripts->apt_script[slot_num];
  }
  else
  {
    apt_script_buf = (rf_buffer_intf *)scripts->tx_start_script[TX_APT_SCRIPT];
  }
...
*/
boolean rfc_gsm::tx_gain_device_config( rfm_device_enum_type rfm_dev, 
                                        rfdevice_class * dev_obj, 
                                        rfcom_gsm_band_type band,
                                        uint8 pa_range, 
                                        rfgsm_modulation_type mod_type,
                                        rf_buffer_intf * tx_trans_buffer,
                                        uint8 slot_num, 
                                        rf_buffer_intf * apt_buffer,
                                        boolean bias_flag, 
                                        uint16 bias, 
                                        uint8 apt_mode, 
                                        boolean override_start_time,
                                        ...
                                        rfgsm_core_tx_log_type * tx_log )
{
  ...
  if( rfgsm_tx_nv_tbl == NULL )
  {
    MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR,"rfgsm_tx_nv_tbl is NULL!", 0);
    return FALSE;
  }
  ...
  case RFDEVICE_PA:
    ...
    {
      if( (pa_range < MAX_NUM_OF_GSM_PA_GAIN_STATE) && (mod_type < MAX_NUM_OF_GSM_MOD_TYPES) && (current_rgi < MAX_NUM_OF_GSM_RGI_INDEX) )
      {
        pa_icq = (uint16)((rfgsm_tx_nv_tbl->tx_static_nv_data_ptr->pa_icq_bias_data.pa_icq_bias[pa_range][mod_type][current_rgi]) & 0xFFFF);
      }
      else
      {
        pa_icq = 0;
        MSG_3(MSG_SSID_RF, MSG_LEGACY_ERROR, "out-of-range index to get pa_icq_pdm: pa_range = %d; mod = %d; rgi = %d",pa_range, mod_type, current_rgi);
      }
    }

    tx_log->tx_slot_log[slot_num].icq_bias = pa_icq;
  ...
  case RFDEVICE_PAPM:
    ...
    else if ( apt_mode == LDO_MODE)
    {
      mode = RFDEVICE_LDO_MODE;
    }
    else
    {
                           ...
                           uint16 rf_tx_arfcn )
{
  ...
  for (i = 0; i < RFDEVICE_TYPE_MAX_NUM; i++)
  {
    ...
    if (dev_list != NULL)
    {
      ...
      while (dev_list[index] != NULL && index < RFC_MAX_DEVICE_INSTANCES_PER_BAND)
      {
        status = tx_disable_device_config(dev_list[index], (rfcom_gsm_band_type)cfg.band, rf_tx_arfcn, rfm_dev);
  ...
  ant_switch_device_sleep(rfm_dev, (rfcom_gsm_band_type)cfg.band, NULL, RFC_CONFIG_TX);
...
boolean rfc_gsm::tx_disable_device_config(rfdevice_class *dev_obj, rfcom_gsm_band_type rfgsm_band, uint16 arfcn, rfm_device_enum_type rfm_dev)
{
  ...
  rfcommon_atuner_config_req_type atuner_config_p = 
        {rfm_dev, RFM_EGSM_MODE, RFCMN_ATUNER_CARRIER_ID_0, RFCMN_ATUNER_TX_DEVICE, band, arfcn}; 
  ...
  case RFDEVICE_TUNER_MANAGER:
    status = rfcommon_atuner_tx_disable((rfcommon_atuner_intf *)dev_obj, &atuner_config_p, &script_token, RFDEVICE_EXECUTE_IMMEDIATE, NULL, 0);
...
*/
int32 rfc_gsm::tx_enable( rfm_device_enum_type rfm_dev,rfcom_gsm_band_type band)
{
  ...
  for (i = 0; i < RFDEVICE_TYPE_MAX_NUM; i++)
  {
    ...
    if (dev_list != NULL)
    {
      ...
      while (dev_list[index] != NULL && index < RFC_MAX_DEVICE_INSTANCES_PER_BAND)
      {
        status = tx_enable_device_config(dev_list[index], rfm_dev,band);
  ...
  ant_switch_device_wakeup(rfm_dev, (rfcom_gsm_band_type)cfg.band, NULL, RFC_CONFIG_TX);
...
boolean rfc_gsm::tx_enable_device_config(rfdevice_class *dev_obj, rfm_device_enum_type rfm_dev,rfcom_gsm_band_type rfgsm_band)
{
  boolean status = FALSE;
  rfdevice_type_enum_type dev_type = RFDEVICE_TYPE_INVALID;
  rfcom_band_type_u rfcom_band;
  ...
  rfcom_band.gsm_band = rfgsm_band;
...
*/
int32 rfc_gsm::sleep( rfm_device_enum_type rfm_dev, rfcom_gsm_band_type band, rfgsm_mode_type rfgsm_mode, rf_buffer_intf *sleep_buffer)
{
  rfc_cfg_params_type cfg = { RF_PATH_0, rfm_dev, RFC_CONFIG_RX, RFCOM_NUM_GSM_BANDS, RFC_REQ_INIT};
  rfc_sig_cfg_type *sig_ptr = NULL;
  uint8 i = 0;
  ...
  rfdevice_type_enum_type rx_device_sleep_all_bands_list[]={RFDEVICE_ASM, RFDEVICE_TUNER_MANAGER};
  ...
  if (dev_list != NULL)
  {
    ...
    {
      /* call sleep TX API*/
      sleep_device_config_tx(dev_list[index], rfm_dev);
      index++;
    }
  }
  ...
  for (band_local=0; band_local<(int)RFCOM_NUM_GSM_BANDS; band_local++)
  {
    ...
    for (i = 0; i < (sizeof(rx_device_sleep_all_bands_list)/sizeof(rfdevice_type_enum_type)); i++)
    {
      dev_list = get_rf_device_list(rfm_dev, cfg.rx_tx, (rfcom_gsm_band_type)cfg.band, rx_device_sleep_all_bands_list[i]);

      if (dev_list != NULL)
      {
        uint8 index = 0, instance=0;
        do {
           if (dev_list[index] != NULL) 
        {
               instance = dev_list[index]->rfdevice_instance_num_get();
             if((instance < RFC_MAX_DEVICE_INSTANCES) && (!gsm_rf_devices_sleep_list[rx_device_sleep_all_bands_list[i]][instance]))
          {
               status = sleep_device_config(dev_list[index], rfm_dev, rfgsm_mode, sleep_buffer);
                 if (status == FALSE)
                 {
            MSG_3(MSG_SSID_RF, MSG_LEGACY_ERROR,"Failed to configure rf-devices! rfm_dev=%d,band=%d,dev_type=%d",
                cfg.logical_device, cfg.band, (rfdevice_type_enum_type)i);
          }
               gsm_rf_devices_sleep_list[rx_device_sleep_all_bands_list[i]][instance] = status;
               }
        }
           index++;
        }while(index < RFC_MAX_DEVICE_INSTANCES_PER_BAND);
      }
    }
	
ant_switch_device_sleep(rfm_dev, (rfcom_gsm_band_type)cfg.band, NULL, RFC_CONFIG_RX);
  }
  return 1;
}
...
*/
boolean rfc_gsm::sleep_device_config_tx(rfdevice_class *dev_obj, rfm_device_enum_type rfm_dev)
{
  boolean status = FALSE;
  rfdevice_type_enum_type dev_type = RFDEVICE_TYPE_INVALID;
  rfdevice_asm_cfg_type asm_cfg_s;
  ...
  rfcommon_atuner_config_req_type atuner_config_p =
    {rfm_dev, RFM_EGSM_MODE, RFCMN_ATUNER_CARRIER_ID_0, RFCMN_ATUNER_TX_DEVICE, rfcom_band, 0};
  ...
  case RFDEVICE_TUNER_MANAGER:    
    status = rfcommon_atuner_disable((rfcommon_atuner_intf *)dev_obj, &atuner_config_p, &script_token, RFDEVICE_EXECUTE_IMMEDIATE, NULL, 0);

    // Clear the stored tuner device so we don't try to disable upon wakeup
    tx_ant_tuner_curr_dev_list[rfm_dev] = NULL;
    tx_ant_tuner_curr_band[rfm_dev].gsm_band = RFCOM_NUM_GSM_BANDS;
    /* No need to ACK tuner when the tuner script construction is in immediate mode*/
...
/*!
   ...
   device type, calls a specific API to configure the device to sleep mode.
*/
boolean rfc_gsm::sleep_device_config(rfdevice_class *dev_obj, rfm_device_enum_type rfm_dev, rfgsm_mode_type rfgsm_mode, rf_buffer_intf *sleep_buffer)
{
  boolean status = FALSE;
  rfdevice_type_enum_type dev_type = RFDEVICE_TYPE_INVALID;
  uint32 script_token;
  ...
  rfcom_band.gsm_band = RFCOM_BAND_GSM900;
  rfcommon_atuner_config_req_type atuner_config_p =
    {rfm_dev, RFM_EGSM_MODE, RFCMN_ATUNER_CARRIER_ID_0, RFCMN_ATUNER_PRX_DEVICE, rfcom_band, 0};
  ...
  {
    atuner_config_p.rx_tx = RFCMN_ATUNER_PRX_DEVICE;
  }
  else
  {
    atuner_config_p.rx_tx = RFCMN_ATUNER_DRX_DEVICE;
  }
  ...
  case RFDEVICE_ASM:
    ...
    if (rfgsm_mode == RFGSM_IRAT_MODE)
    {
      rfdevice_asm_sleep(&asm_cfg_s, dev_obj, sleep_buffer, RFDEVICE_CREATE_SCRIPT, RFCOM_INVALID_TIMING_OFFSET);
    }
    else
    {
    rfdevice_asm_sleep(&asm_cfg_s, dev_obj, NULL, RFDEVICE_EXECUTE_IMMEDIATE, RFCOM_INVALID_TIMING_OFFSET);
  ...
  case RFDEVICE_TUNER_MANAGER:
    /* Do the tuner disable only for default gsm mode or in NULL2X IRAT MODE */
    if ((rfgsm_mode == RFGSM_DEFAULT_MODE)||(rfgsm_mode == RFGSM_IRAT_NULL2X_MODE))
    {
      /* No need to ACK tuner when the tuner script construction is in immediate mode */
      status = rfcommon_atuner_disable( (rfcommon_atuner_intf *)dev_obj, 
                                        &atuner_config_p, 
                                        &script_token, 
      ...
      // Clear the stored tuner device so we don't try to disable upon wakeup
      rx_ant_tuner_curr_dev_list[rfm_dev] = NULL;
      rx_ant_tuner_curr_band[rfm_dev].gsm_band = RFCOM_NUM_GSM_BANDS;
    }
    else if (rfgsm_mode== RFGSM_IRAT_MODE)
    {
      status = rfcommon_atuner_tune_back((rfcommon_atuner_intf *)dev_obj, 
                                        &atuner_config_p, 
                                        &script_token, 
                                        RFDEVICE_CREATE_SCRIPT, 
                                        sleep_buffer,
      ...
      if ((TRUE==status) && (script_token != ATUNER_INVALID_TOKEN_VAL))
      {
        rfcommon_atuner_acknowledge_script((rfcommon_atuner_intf *)dev_obj, script_token);
      }
    }
    else
    {
...
*/
int32 rfc_gsm::wakeup( rfm_device_enum_type rfm_dev, 
                       uint8 alt_path,
                       ...
                       rfdevice_rx_mode_type rx_mode )
{
  ...
  uint32 instance = 0;
  uint8 band_local = 0;
  boolean gsm_rf_devices_wakeup_list[RFDEVICE_TYPE_MAX_NUM][RFC_MAX_DEVICE_INSTANCES_PER_BAND][RFC_MAX_DEVICE_INSTANCES_PER_BAND];

  //Add the devices which we want to be put to wakeup. 
  //This device list should be same as that during sleep. 
  rfdevice_type_enum_type rx_device_wakeup_list[2]={RFDEVICE_ASM, RFDEVICE_TUNER_MANAGER};
  ...
  memset(&gsm_rf_devices_wakeup_list[0][0][0], 0, sizeof(gsm_rf_devices_wakeup_list));
  ...
  for (band_local=0; band_local<(int)RFCOM_NUM_GSM_BANDS; band_local++)
  {
    ...
    /*loop through all device types*/
    for (i = 0; i < (sizeof(rx_device_wakeup_list)/sizeof(rfdevice_type_enum_type)); i++)
    {
      dev_list = get_rf_device_list(rfm_dev, cfg.rx_tx, (rfcom_gsm_band_type)cfg.band, rx_device_wakeup_list[i]);

      if (dev_list != NULL)
      {
        uint8 index = 0, instance = 0;      
        do {
              if (dev_list[index] != NULL && index < RFC_MAX_DEVICE_INSTANCES_PER_BAND)
              {
                instance = dev_list[index]->rfdevice_instance_num_get();
    
                if((instance < RFC_MAX_DEVICE_INSTANCES_PER_BAND) && (!gsm_rf_devices_wakeup_list[rx_device_wakeup_list[i]][index][instance]))
        {
          status = wakeup_device_config(dev_list[index], (rf_buffer_intf *)scripts->enter_mode_script, pwr_off_in_sleep, rx_mode);
          ...
          if (status == FALSE)
          {
            MSG_3(MSG_SSID_RF, MSG_LEGACY_ERROR,"Failed to configure rf-devices! rfm_dev=%d,band=%d,dev_type=%d",
                  ...
                  gsm_rf_devices_wakeup_list[rx_device_wakeup_list[i]][index][instance] = status;
        }
      }
           } while (index++);
       }
...
*/
boolean rfc_gsm::wakeup_device_config(rfdevice_class *dev_obj, rf_buffer_intf *wakeup_buffer, boolean pwr_off_in_sleep, rfdevice_rx_mode_type rfdevice_op_mode)
{
  boolean status = FALSE;
  uint32 script_token;
  rfdevice_asm_cfg_type asm_cfg_s;
  ...
  rfcom_band.gsm_band = RFCOM_NUM_GSM_BANDS;
  rfcommon_atuner_config_req_type atuner_config_p = 
        {RFM_MAX_WAN_DEVICES, RFM_EGSM_MODE, RFCMN_ATUNER_CARRIER_ID_0, ((rfdevice_op_mode == RFDEVICE_PRX_OPER_MODE) ? RFCMN_ATUNER_PRX_DEVICE : RFCMN_ATUNER_DRX_DEVICE), rfcom_band, 0}; 
  ...
  case RFDEVICE_TUNER_MANAGER:
    ...
    {
      status = rfcommon_atuner_init((rfcommon_atuner_intf *)dev_obj, &atuner_config_p, &script_token, RFDEVICE_CREATE_SCRIPT, wakeup_buffer, 0);
    ...
    {
      status = rfcommon_atuner_init((rfcommon_atuner_intf *)dev_obj, &atuner_config_p, &script_token, RFDEVICE_EXECUTE_IMMEDIATE, NULL, 0);
...
void rfc_gsm::manage_vregs (rf_path_enum_type path, rfcom_gsm_band_type band,
                            rfm_path_state state, rfm_mode_enum_type mode, rfm_device_enum_type device)
{
  rfc_vreg_param vreg_param(path, mode);
  vreg_param.set_band(band);
  vreg_param.set_device(device);
...
void rfc_gsm::update_timing_based_on_nv(rfm_device_enum_type rfm_dev, rfcom_gsm_band_type band, rfc_sig_cfg_type *rf_signals)
{
  ...
  if (rfgsm_tx_nv_tbl == NULL)
  {
    MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR,"update_timing_based_on_nv NULL Tx NV ptr!", 0);
    return;
  }

  tx_static_nv_data_ptr = rfgsm_tx_nv_tbl->tx_static_nv_data_ptr;
  ...
  if ( rfcommon_get_gnss_blanking_is_enabled( RFCOM_GSM_MODE, band ) )
  {
...
/*!
  ...
  @param band: GSM band for which timing data to be updated.
*/
void rfc_gsm::update_timing_based_on_card( rfcom_gsm_band_type band, rfc_sig_cfg_type * rf_signals,
                                           rfgsm_nv_band_tx_data_type * rfgsm_tx_nv_tbl,
...
/*!
  ...
  @param None.
*/
int32 rfc_gsm::tx_timing_adjust( rfm_device_enum_type rfm_dev, 
                                 rfcom_gsm_band_type band,
                                 rfgsm_nv_band_tx_data_type * rfgsm_tx_nv_tbl,
                                 int16 rf_pa_start_time,
                                 int16 rf_pa_stop_time )
{
      ...
      rfc_data->sig_cfg_data_get(&cfg, &sig_ptr )==TRUE)
  {
    ...
    update_timing_based_on_card( band, rfc_gsm_tx_sig_list_ptrs[rfm_dev][band], rfgsm_tx_nv_tbl, rf_pa_start_time, rf_pa_stop_time );
  }
...
void rfc_gsm::init_rf_devices(rfc_cfg_params_type *cfg, rfc_device_info_type *device_info_ptr)
{
  ...
  while (i < device_info_ptr->num_devices)
  {
    ...
    case RFDEVICE_TRANSCEIVER:
      if (cmn_dev_obj != NULL)
      {
        ...
        if (tmp_status == TRUE)
        {
          ...
          if(sys_band == SYS_BAND_GSM_EGSM_900)
          {
            rfc_dynamic_params.sys_band = SYS_BAND_GSM_PGSM_900;
            (void) rfc_cmn->generate_rfcard_dynamic_properties( &rfc_dynamic_params );   
          }
        }
        else
    ...
    case RFDEVICE_TUNER:
    {
      ...
      {
        gsm_rf_devices[cfg->logical_device][cfg->rx_tx][cfg->band][RFDEVICE_TUNER_MANAGER][local_instance] = tuner_cmn_dev_obj;
        tuner_dev_instance++;
      }
    ...
    case RFDEVICE_COUPLER:
      ...
      // Set up the coupler band config map
      ((rfdevice_coupler *)cmn_dev_obj)->set_band_config_map(RFM_EGSM_MODE, rfcom_band, 
                                                              device_info_ptr->rf_asic_info[i].data);
      break;
    ...
    case RFDEVICE_XSW:
      ...
      {
        /* Update the XSW port map by invoking the API below */
          rfdevice_xsw_set_band_map_swap(
          &xsw_cfg,
          cmn_dev_obj,
          device_info_ptr->rf_asic_info[i].data,
          RFC_ASIC_INFO_DATA_SIZE );
    ...
    case RFDEVICE_LNA:      
      ...
      if ((cmn_dev_obj != NULL) && (cfg->rx_tx == RFC_CONFIG_RX))
      {     
        /* Populate LNA config params */
        rfdevice_lna_config_type lna_cfg;
        lna_cfg.rfm_device = cfg->logical_device;
        lna_cfg.band = rfcom_band;
        lna_cfg.mode = RFM_EGSM_MODE;
        lna_cfg.trigger_type = RFDEVICE_LNA_TRIGGER_NONE;
        lna_cfg.lna_gain = RFDEVICE_LNA_GAIN_INVALID;
        lna_cfg.system_gain = 0;

        /* Set the Rx band map for this ELNA device for selected band */ 
        if (!rfdevice_lna_set_band_map((void*)cmn_dev_obj, &lna_cfg, device_info_ptr->rf_asic_info[i].data, sizeof(device_info_ptr->rf_asic_info[i].data)))
        {
          RF_MSG_3(RF_FATAL, "rfdevice_lna_set_band_map failed! dev_type %d log_dev %d band %d", dev_type, cfg->logical_device, rfcom_band.gsm_band);
        }
      }
  ...
  */
  ...
  if (tuner_dev_instance == 0 )
  {
    ...
    {
      gsm_rf_devices[cfg->logical_device][cfg->rx_tx][cfg->band][RFDEVICE_TUNER_MANAGER][tuner_dev_instance] = tuner_mgr_obj;
...
void rfc_gsm::init_rfdevices_for_all_bands(rfm_device_enum_type rfm_dev)
{
   ...
   */
  MSG(MSG_SSID_RF, MSG_LEGACY_HIGH, "Processing asdiv_config info table for GSM...");
  ...
  /* Restore the logical device to the parameter passed in before proceeding */
       ...
       if(rfc_data->devices_cfg_data_get(&cfg, &device_info_ptr)== TRUE)
       {
           ...
           )
         {
              RF_MSG_3( RF_ERROR, " rfc_set_fbrx_mapping() failed for rfm device %d. "
                                  ...
                                  rfm_device,RFCOM_GSM_MODE,rfgsm_band);
         } 
       }
    }
  }
...
/*-------------------------------------------------------------------------------------------------------------------*/
boolean rfc_gsm::ant_switch_device_wakeup( rfm_device_enum_type rfm_device,
                                                rfcom_gsm_band_type rfgsm_band,
                                                rf_buffer_intf* scripts,
                                                rfc_rxtx_enum_type rx_tx)
{
  /*TODO: Move the xsw devices to the gsm_front_end_devices array, by adding in the rfc_gsm::init_rf_devices function...*/
  rfdevice_class **ant_switch_dev_list = get_rf_device_list(rfm_device, rx_tx, (rfcom_gsm_band_type)rfgsm_band, RFDEVICE_XSW);
  rfdevice_xsw_config_type xsw_cfg_s;
  rf_device_execution_type execution_type = RFDEVICE_EXECUTE_IMMEDIATE;
  rfcom_band_type_u band;
  band.gsm_band = rfgsm_band;
  boolean status = FALSE;

  if(ant_switch_dev_list != NULL)
  {
    uint8 index = 0;
    while ((ant_switch_dev_list[index] != NULL) && (index < RFC_MAX_DEVICE_INSTANCES_PER_BAND))
    {
      if(ant_switch_dev_list[index]->rfdevice_type_get() == RFDEVICE_XSW)
      {
        /*Fill In Params (mode, band and XSmode)*/
        xsw_cfg_s.band                     = band; 
        xsw_cfg_s.mode                     = RFCOM_GSM_MODE;
        xsw_cfg_s.xsmode                   = RFDEVICE_XSW_PRX_DRX_PATH_DEFAULT; 
        xsw_cfg_s.trigger                   = RFDEVICE_XSW_TRIGGER_WAKEUP; 

        status &= rfdevice_xsw_wakeup( &xsw_cfg_s, 
                                      (void*)ant_switch_dev_list[index],
                                      scripts,
                                      execution_type, 
    ...
    if(status == FALSE)
    {
      ...
      "Failed to configure portA/B Rx rf-devices for wakeup! rfm_dev=%d, dev_type=%d",
      rfm_device , RFDEVICE_XSW );
    }
  }
...
boolean rfc_gsm::ant_switch_device_sleep(rfm_device_enum_type rfm_device,
                                         rfcom_gsm_band_type rfgsm_band,
                                         rf_buffer_intf * scripts,
                                         rfc_rxtx_enum_type rx_tx)
{
  ...
  if(ant_switch_dev_list != NULL)
  {
    ...
    while ((ant_switch_dev_list[index] != NULL) && (index < RFC_MAX_DEVICE_INSTANCES_PER_BAND))
    {
      if(ant_switch_dev_list[index]->rfdevice_type_get() == RFDEVICE_XSW)
      {
        ...
        xsw_cfg_s.mode                     = RFCOM_GSM_MODE;
        xsw_cfg_s.xsmode                   = RFDEVICE_XSW_PRX_DRX_PATH_DEFAULT;
        xsw_cfg_s.trigger              = RFDEVICE_XSW_TRIGGER_SLEEP; 


        status_device = rfdevice_xsw_sleep( &xsw_cfg_s, 
                                            (void*)ant_switch_dev_list[index],
                                            scripts,
                                            execution_type, 
                                            RFCOM_INVALID_TIMING_OFFSET );
        if (status_device == FALSE)
        {
          ...
          {
            MSG_2( MSG_SSID_RF, MSG_LEGACY_ERROR,
                   "Failed to configure Ant XSW for Rx sleep! rfm_dev=%u, index=%u",
                   rfm_device, index );
          }
		  else if (rx_tx == RFC_CONFIG_TX)
          {
            MSG_2( MSG_SSID_RF, MSG_LEGACY_ERROR,
                   "Failed to configure Ant XSW for Tx sleep! rfm_dev=%u,  index=%u",
                   rfm_device, index );
		  }
		  else
		  {
		    MSG_1( MSG_SSID_RF, MSG_LEGACY_ERROR,"Invalid Rx/Tx in Ant XSW sleep %u",rx_tx);
		  }
        }		
      }
      else
      {
		  MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR,"Ant XSW NOT Configured, index=%u", index);
		  status_device = FALSE;
      }
...
boolean rfc_gsm::setup_fb_device(  
   rfm_device_enum_type device, 
   rfcom_gsm_band_type band, 
   uint16 tx_chan, boolean config, uint8 fbrx_gain_state
)
{
  rfgsm_core_handle_type *rfgsm_core_handle_ptr = NULL;
  rfc_cfg_params_type cfg;
  ...
  rfc_device_info_type *device_info_ptr = NULL;
  rfc_gsm_data *rfc_data = NULL;
  rf_buffer_intf* fbrx_script = NULL;
  uint8 fbrx_setup_t_us = 0;
  boolean status = TRUE;
  rfgsm_core_handle_ptr = rfgsm_core_handle_get(device);

  
  cfg.port_state= RFC_PORT_STATE_DEFAULT; 
  cfg.alternate_path = 0;
  cfg.band = (int)band;
  cfg.rx_tx = RFC_CONFIG_TX;
  ...
  if (device_info_ptr == NULL)
  {
    MSG_6(MSG_SSID_RF, MSG_LEGACY_ERROR,"RFC GSM device_info_ptr is Null, rx_tx = %d, alter_path = %d, band = %d, device = %d, req = %d",
          cfg.rx_tx,
          cfg.alternate_path,
          cfg.band,
          cfg.logical_device,
  ...
  trx_device = rfc_common_get_cmn_device_object(0);
 ...
 // TODO:  Waiting on WTR4905 fbrx setup api to be ready and we can enable the code below. 20150807.
 MSG(MSG_SSID_RF, MSG_LEGACY_ERROR,"fbrx is not setup. Waiting on support from WTR4905 fbrx apis");
    
  if(config)
  {
    if( !rfdevice_gsm_fbrx_self_test_on( device,band,fbrx_gain_state))
    {
      RF_MSG(RF_ERROR, "fbrx setup is not done ");   
    }
    else
    {
      RF_MSG_1(RF_HIGH, "fbrx setup is done with fbrx_gain_state = %d",fbrx_gain_state );   
    }
  }
  else 
  {
    if( rfdevice_gsm_fbrx_self_test_off( device))
    {
      RF_MSG(RF_ERROR, "fbrx off is not done ");   
    }
    else
    {
      RF_MSG(RF_HIGH, "fbrx off is done ");   
    }
  }
  return status;
}
