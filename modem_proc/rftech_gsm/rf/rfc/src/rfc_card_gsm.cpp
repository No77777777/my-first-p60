...
$Header: //components/rel/rftech_gsm.mpss/3.11/rf/rfc/src/rfc_card_gsm.cpp#1 $
...
07/27/15   sw      ASM should not build XSW if GRF has been asked to do so
06/30/15   ndb     Added Tuner IRAT support in RxTx split feature
07/01/15   hoh     Changes to support eLNA for GSM 
06/01/15   sg      Changes to remove un-necessary tuner transactions 
                   during probe burst
...
*/
void rfc_gsm_mode_config( rfm_device_enum_type rfm_dev, rf_buffer_intf* enter_mode_buffer, rfdevice_rx_mode_type rfdevice_op_mode, uint8 alt_path)
{
  rfc_gsm *rfc_obj = rfc_gsm::get_instance();
  ...
  rfc_obj->config_mode(rfm_dev, enter_mode_buffer, rfdevice_op_mode, alt_path);
                             ...
                             rf_buffer_intf  *tx_band_script)
{
  ...
  if ( rfc_obj == NULL )
  {
    MSG_1(MSG_SSID_RF, MSG_LEGACY_FATAL,"rfc_gsm_tx_band_config() GSM RFC Obj is NOT created. ",0);
    return;
  }
...
*/
void rfc_gsm_tx_gain_config( rfm_device_enum_type rfm_dev,
                             rfcom_gsm_band_type band,
                             rfgsm_modulation_type mod_type,
                             uint8 slot_num,
                             int32 pwr_in_dbm, 
                             ...
                             rfgsm_mdsp_scripts_ptrs * scripts )
{
  ...
  rfc_obj->tx_gain_config( rfm_dev, 
                           band, 
                           mod_type, 
                           slot_num, 
                           pwr_in_dbm, 
...
*/
void rfc_gsm_tx_gain_override( rfm_device_enum_type rfm_dev, 
                               rfcom_gsm_band_type band,
                               rfgsm_modulation_type mod_type, 
                               uint8 pa_range, 
                               boolean bias_flag, 
                               ...
                               rfgsm_mdsp_scripts_ptrs * scripts )
{
  ...
  rfc_obj->tx_gain_override( rfm_dev, 
                             band, 
                             mod_type, 
                             pa_range, 
                             bias_flag, 
...
/*!
   ...
   @Returns None
*/
void rfc_gsm_manage_vregs ( rf_path_enum_type path, rfcom_gsm_band_type band, rfm_path_state state, uint8 sub_id, rfm_device_enum_type device )
{
  ...
  rfc_obj->manage_vregs(path, band, state, mode[sub_id], device);
...
/*!
   ...
   @Returns None
*/
void rfc_gsm_sleep( rfm_device_enum_type rfm_dev, rfcom_gsm_band_type band,rfgsm_mode_type rfgsm_mode, rf_buffer_intf *sleep_buffer)
{
  rfc_gsm *rfc_obj = rfc_gsm::get_instance();
  if ( rfc_obj == NULL )
  {
  ...
  rfc_obj->sleep( rfm_dev, band, rfgsm_mode, sleep_buffer );
...
/*!
   ...
   @Returns None
*/
void rfc_gsm_wakeup( rfm_device_enum_type rfm_dev,
                     uint8 alt_path,
                     ...
                     rfdevice_rx_mode_type rfdevice_op_mode )
{
  ...
  rfc_obj->wakeup(rfm_dev, alt_path, pwr_off_in_sleep, band_before_sleep, scripts, rfdevice_op_mode);
                         ...
                         uint16 rf_tx_arfcn )
{
  ...
  rfc_obj->tx_disable(rfm_dev, tx_band, rf_tx_arfcn);
...
/*!
   ...
   @Returns None
*/
void rfc_gsm_tx_enable(rfm_device_enum_type rfm_dev,rfcom_gsm_band_type band)
{
  rfc_gsm *rfc_obj = rfc_gsm::get_instance();
  if ( rfc_obj == NULL )
  {
  ...
  rfc_obj->tx_enable(rfm_dev,band);
...
/*!
  ...
  @param None.
*/
void rfc_gsm_tx_timing_adjust( rfm_device_enum_type rfm_dev, 
                               rfcom_gsm_band_type band,
                               rfgsm_nv_band_tx_data_type * rfgsm_tx_nv_tbl,
                               ...
                               int16 rf_pa_stop_time )
{
  ...
  rfc_obj->tx_timing_adjust(rfm_dev, band, rfgsm_tx_nv_tbl, rf_pa_start_time, rf_pa_stop_time);
                                      ...
                                      uint8 * tx_supported_band_mask )
{
 ...
 // Get the TX and RX support for each band
  for (rfgsm_band = (uint8)RFCOM_BAND_GSM850; rfgsm_band < (uint8)RFCOM_NUM_GSM_BANDS; rfgsm_band++)
  {
    ...
    {
      *tx_supported_band_mask |= (1 << rfgsm_band);
    }
    ...
    if( rfc_data->devices_cfg_data_get(&cfg, &device_info_ptr ) == TRUE )
    {
      *rx_supported_band_mask |= (1 << rfgsm_band);
    }  
  } 
  return TRUE; 
}
...
boolean rfc_gsm_setup_fb_device(
   ...
   uint8 fbrx_gain_state)
{
  ...
  return rfc_obj->setup_fb_device(device,band,tx_chan,config,fbrx_gain_state);
}
/* ----------------------------------------------------------------------- */
/*!
  @brief
...
*/
uint32 rfc_gsm_get_fb_chain(rfm_device_enum_type tx_device, rfcom_gsm_band_type tx_band)
{
  ...
  if ( rfc_obj == NULL )
  {
    MSG_1(MSG_SSID_RF, MSG_LEGACY_FATAL,"rfc_gsm_ptr is NOT created. ",0);
    return RFM_INVALID_DEVICE ;
  }

  fb_modem_chain = rfc_obj->get_fb_chain( tx_device, tx_band);
  */
...
/*!
  @brief
  This function returns the address of rfc_gsm_ptr.
  ...
  @Returns
  Address of rfc_gsm_ptr

*/
QSH_MDUMP_FN_ATTR void * rfc_gsm_get_address(void)
{
  rfc_gsm *rfc_obj = rfc_gsm::get_instance();

  return (void*) (rfc_obj->get_instance_address());
}

/* ----------------------------------------------------------------------- */
/*!
  @brief
  This function returns the rfc_gsm_ptr.
...
*/
QSH_MDUMP_FN_ATTR void * rfc_gsm_ptr_get_address(void)
{
  rfc_gsm *rfc_obj = rfc_gsm::get_instance();
...
/*!
  @brief
  This function returns the size of rfc_gsm_ptr.
...
*/
QSH_MDUMP_FN_ATTR uint32 rfc_gsm_ptr_get_size(void)
{
  return (uint32) sizeof(rfc_gsm);
}
