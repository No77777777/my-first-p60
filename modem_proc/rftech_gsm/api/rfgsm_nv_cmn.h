...
$Header: //components/rel/rftech_gsm.mpss/3.11/api/rfgsm_nv_cmn.h#1 $ 
...
04/18/16   kpa     Add debug NV check for Excessive F3 logging reduction
...
04/09/15   sg      Add FEATURE_RF_HAS_QTUNER in GSM NV structure mapping
...
01/23/15   ec      Add enum and prototype for new debug NV mechanism
01/19/15   sw      Introduce a new GSM NV (for debug and profiling)
...
#define FEATURE_RFGSM_ELNA_ASDIV
...
#define RFNV_GSM_VBATT_BACK_OFF_LIMIT_DEFAULT 12700  /* 127 dBm in dB100*/
...
/*! 
  ...
  RFGSM_DEBUG_NV_WAKEUP_TIME_PROFILE_ENABLE
  - Used to enable expanded time profiling in Wakeup
    
  RFGSM_DEBUG_NV_SLEEP_TIME_PROFILE_ENABLE
    - Used to enable expanded time profiling in Sleep 
    
  RFGSM_DEBUG_NV_RX_BURST_PROFILE_ENABLE
    - Used to enable expanded time profiling in Rx burst

  RFGSM_DEBUG_NV_TX_BURST_PROFILE_ENABLE
    - Used to enable expanded time profiling in Tx burst
...
typedef enum
{
  ...
  RFGSM_DEBUG_NV_FBRX_VSWR_ENABLE,                  // Enable the FBRX VSWR measurements
  RFGSM_DEBUG_NV_FBRX_VSWR_ENABLE_FBRX_EVERY_BURST, // Enable the FBRX path every TX burst
  RFGSM_DEBUG_NV_FBRX_VSWR_DISABLE_COUPLER_SWITCH,  // Disable the reverse coupler switch
...
typedef enum
{
  ...
  RFGSM_NV_CONT_DRX,
  RFGSM_NV_CONT_MAX,
}rfgsm_nv_container_device_type;
...
/*WHY IS THIS A DIFFERENT STRUCTURE??????*/
typedef struct
{
...
/*----------------------------------------------------------------------------*/
/*Stored in an array of 2 (parrallel or crossed asdiv config) , per band, per path*/
typedef struct
{
  /*Variant Marker*/
  ...
  /*Gain Offsets*/
  rfnv_asdiv_rx_gain_offset_type rx_elna_asdiv_gain_offset[ASDIV_POSITION_MAX];

  /*Switch Point Offsets*/
  rfnv_asdiv_rx_switch_points_type rx_elna_asdiv_switch_point_offsets[ASDIV_POSITION_MAX];
  
}rfnv_gsm_rx_elna_asdiv_type;
...
typedef struct
{
  ...
  /*Tx Gain Offsets*/
  rfnv_asdiv_tx_gain_offset_type tx_elna_asdiv_gain_offset[ASDIV_POSITION_MAX];
}rfnv_gsm_tx_elna_asdiv_type;
...
typedef struct
{
  ...
  /*SAR BACKOFF variant NV ID RFNV_GSM_GSM850_SAR_BACKOFF_VAR*/
  rfnv_item_id_enum_type sar_lut_var;
  ...
  rfnv_item_id_enum_type elna_asdiv_gain_data;
...
typedef struct
{
  ...
  */
  ...
  rfnv_gsm_rx_elna_asdiv_type asdiv_elna_gain_info;
...
/* SAR Variant NV item (chain and antenna position specific) */
typedef PACK (struct)
{
  uint8 chain_identifier;  /*0,2,4,...*/
  uint8 antenna_index;     /*0,1,2,... 0 is for default antenna, 1 is for switched antenna*/
  gsm_sar_backoff_data_type sar_backoff_lut[RFNV_GSM_SAR_NUM_UL_SLOT_MAX][RF_SAR_STATE_MAX-1];
}rfnv_data_sar_backoff_type;		
...
typedef enum
{
  ...
  RF_GSM_NV_PATH_3,
  /*NV Path for RF Transceiver Path 3 */  

  RF_GSM_NV_PATH_4,
  /*NV Path for RF Transceiver Path 4 */  

  RF_GSM_NV_PATH_5,
  /*NV Path for RF Transceiver Path 5 */  
...
typedef struct 
{
 ...
 /*!
  ...
  enh_apt_data_type enh_apt_data;
	...
	/*!
		@brief
	    SAR-BACKOFF for default antenna
	   */
  gsm_sar_lut_type sar_lut_data;

	/*!
		@brief
	    SAR-BACKOFF for switched antenna
	   */
  gsm_sar_lut_type sar_lut_data_switched_ant;

        /*!
                @brief
            SAR-BACKOFF for switched antenna flag
           */
 ...
 /*!
  ...
  rfnv_gsm_tx_elna_asdiv_type asdiv_elna_gain_info;
...
/*New typedefs to generalize the rx switchpoint interface*/
typedef enum
{
 RF_GSM_FREQ_COMP_RX_SWPT = 0,
 RF_GSM_ENH_RX_SWPT,
 RF_GSM_HL_RX_SWPT,
 RF_GSM_MAX_RX_SWPT
}rfgsm_nv_rx_switchpoint_type;


/*Table to store all of the different switch points. Make the code more easily expandable than it currently is*/
typedef struct
{
  rfgsm_nv_rx_switch_type  (*gsm_850_rx_switch)[ASDIV_POSITION_MAX];
  rfgsm_nv_rx_switch_type  (*gsm_900_rx_switch)[ASDIV_POSITION_MAX];
  rfgsm_nv_rx_switch_type  (*gsm_1800_rx_switch)[ASDIV_POSITION_MAX];
  rfgsm_nv_rx_switch_type  (*gsm_1900_rx_switch)[ASDIV_POSITION_MAX];
}rfgsm_nv_rx_switch_storage;
...
/*!
  ...
  @param gsm_rx_elna_offset
  Pointer to a switch position
...
FUNCTION RFGSM_NV_GET_RX_FREQ_COMP_SWITCH_PTR_ASDIV                                 

DESCRIPTION
  Returns pointer to GSM RX frequency compensation switch pointer, for a given asdiv switch position

DEPENDENCIES
  None

RETURN VALUE
  Returns pointer to GSM RX frequency compensation switch pointer, for a given asdiv position

SIDE EFFECTS
  None

===========================================================================*/
rfgsm_nv_rx_switch_type * rfgsm_nv_get_rx_freq_comp_switch_ptr_asdiv_elna
(
  rfm_device_enum_type rfm_dev,
  rfcom_gsm_band_type band,
  rfcommon_asdiv_position_enum_type pos
);
...
RETURN VALUE
  Returns pointer to void to be cast to GSM RF Rx or Tx NV table.
...
/*!
  ...
  post static NV loaded accessor for SAR values

  @details
  
  @param rfm_device_enum_type      --  RFM tx device for which this NV is required
  @param uint8 num_tx_slots        --  number of uplink slots to be transmitted [1-5]
  @param rfm_common_sar_state_type sar_state -- 
  @param rfnv_gsm_sar_unit_type *sar_limit   --
  @param rfm_antenna_enum_type ant_pos -    -- antenna position (bottom Prxtx = 0, top Drx =1)

  @retval
  update nv_gsm_sar_unit_type *sar_limit -- is updated with SAR limits
