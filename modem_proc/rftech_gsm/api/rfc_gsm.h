...
$Header: //components/rel/rftech_gsm.mpss/3.11/api/rfc_gsm.h#1 $

when       who     what, where, why
------------------------------------------------------------------------------- 
02/02/20   mpa     To support MiniDump Feature
11/07/19   mpa     Changes to set QET5102 papm to standby in wakeup
05/09/18   mpa     [CR2066053] GSM/EDGE Modulation Type Issue on PA driver
03/16/18   kpa     [CR2190910]Support of configuring the multiple tuner in same path
03/01/18   mpa     RFFE QAT3522 support 
...
public:
...
#ifdef FEATURE_QSH_MDUMP
	QSH_MDUMP_FN_ATTR static rfc_gsm ** get_instance_address(void);
#endif /* FEATURE_QSH_MDUMP */


  virtual void init_once(rfm_device_enum_type rfm_dev);
  virtual void config_mode(rfm_device_enum_type rfm_dev, rf_buffer_intf* enter_mode_buffer, rfdevice_rx_mode_type rfdevice_op_mode, uint8 alt_path);
  ...
  virtual void tx_gain_config( rfm_device_enum_type rfm_dev, 
  ...
  virtual void tx_gain_override( rfm_device_enum_type rfm_dev, 
                                 rfcom_gsm_band_type band,
                                 rfgsm_modulation_type mod_type, 
                                 uint8 pa_range,
                                 boolean bias_flag,
  ...
  virtual int32 tx_enable(rfm_device_enum_type rfm_dev,rfcom_gsm_band_type band);

  virtual int32 tx_disable( rfm_device_enum_type rfm_dev,
                            rfcom_gsm_band_type tx_band,
                            uint16 rf_tx_arfcn );

  virtual int32 tx_timing_adjust( rfm_device_enum_type rfm_dev, 
                                  rfcom_gsm_band_type band,
                                  rfgsm_nv_band_tx_data_type * rfgsm_tx_nv_tbl,
                                  int16 rf_pa_start_time,
                                  int16 rf_pa_stop_time );

  virtual int32 sleep( rfm_device_enum_type rfm_dev, rfcom_gsm_band_type band, rfgsm_mode_type rfgsm_mode, rf_buffer_intf *sleep_buffer );
  virtual int32 wakeup( rfm_device_enum_type rfm_dev, 
                        uint8 alt_path,
  ...
  boolean ant_switch_device_wakeup( rfm_device_enum_type rfm_device,
                                    rfcom_gsm_band_type rfgsm_band,
                                    rf_buffer_intf* scripts,
                                    rfc_rxtx_enum_type rx_tx);
  boolean ant_switch_device_sleep(rfm_device_enum_type rfm_device,
                                    rfcom_gsm_band_type rfgsm_band,
                                    rf_buffer_intf * scripts,
                                    rfc_rxtx_enum_type rx_tx);
  ...
  virtual void manage_vregs(rf_path_enum_type path, rfcom_gsm_band_type band, rfm_path_state state, rfm_mode_enum_type mode, rfm_device_enum_type device);
  ...
  virtual boolean setup_fb_device(rfm_device_enum_type device,rfcom_gsm_band_type band, uint16 tx_chan, boolean config, uint8 fbrx_gain_state);
  
  #ifdef FEATURE_RF_ASDIV
    virtual void append_txd_grfc(uint8 command, rf_buffer_intf *start_buffer, void *cb_data);
...
private:
  ...
  void update_timing_based_on_card( rfcom_gsm_band_type band, rfc_sig_cfg_type * rf_signals,
                                    rfgsm_nv_band_tx_data_type * rfgsm_tx_nv_tbl,
                                    int16 rf_pa_start_time,
                                    int16 rf_pa_stop_time );
  ...
  boolean tx_burst_device_config(rfm_device_enum_type rfm_dev,
                                 ...
                                 rfgsm_mdsp_scripts_ptrs * scripts,
                                 rfgsm_modulation_type mod_type);
  /* tuner devices config APIs */
  boolean tx_burst_tuner_device_config( rfm_device_enum_type rfm_dev,
  ...
  /* tuner devices config APIs */
  boolean rx_burst_tuner_device_config( rfm_device_enum_type rfm_dev,
                                        rfdevice_class **dev_list,
                                        rfcom_gsm_band_type band,
                                        rfc_sig_cfg_type *sig_ptr,
                                        uint16 arfcn,
  ...
  boolean tx_gain_device_config( rfm_device_enum_type rfm_dev, 
                                 rfdevice_class * dev_obj, 
                                 rfcom_gsm_band_type band,
                                 uint8 pa_range, 
                                 rfgsm_modulation_type mod_type,
  ...
  boolean tx_enable_device_config(rfdevice_class *dev_obj, rfm_device_enum_type rfm_dev,rfcom_gsm_band_type rfgsm_band);
  boolean sleep_device_config_tx(rfdevice_class *dev_obj, rfm_device_enum_type rfm_dev);
  boolean sleep_device_config(rfdevice_class *dev_obj, rfm_device_enum_type rfm_dev, rfgsm_mode_type rfgsm_mode, rf_buffer_intf *sleep_buffer);
 #ifndef TEST_FRAMEWORK
  boolean wakeup_device_config(rfdevice_class *dev_obj, rf_buffer_intf *wakeup_buffer, boolean pwr_off_in_sleep, rfdevice_rx_mode_type rfdevice_op_mode);
  #endif
  ...
  rfdevice_class ** rx_ant_tuner_curr_dev_list[RFM_MAX_WAN_DEVICES];
  rfdevice_class ** tx_ant_tuner_curr_dev_list[RFM_MAX_WAN_DEVICES];
  rfcom_band_type_u rx_ant_tuner_curr_band[RFM_MAX_WAN_DEVICES];
  rfcom_band_type_u tx_ant_tuner_curr_band[RFM_MAX_WAN_DEVICES];
};
