...
$Header: //components/rel/rftech_gsm.mpss/3.11/api/rfgsm_core.h#1 $
...
08/18/15   ec      Integrate SAWless code into split Rx\Tx
08/05/15   sc      Deprecate alternate path
07/07/15   sc      Further Rx burst optimisations (~20% @ 144MHz)
07/01/15   sc      Fix LIF and enable optimised Rx burst processing
...
02/18/15   sw      Avoid name-mangling of declarations
01/30/15   sw      GL1->RF handshake mechanism to handle ASD response error case
01/16/15   sw      Use tuner start delta to program XSW
...
/* Data type for storing worst case RX timings for IRAT                  */
/* During CM_ENTER, GL1 has not been granted a device => GRF must not    */
/* make any assumptions about device and simply return worst case values */
typedef struct
{
  int    pll_settling_time_qs;
  uint16 rx_burst_sbis_before_pll;
  uint32 rx_burst_processing_time_from_pll_qs;
} rfc_gsm_irat_rx_timing_info_type;

typedef struct
{
  rfc_gsm_irat_rx_timing_info_type rx_timing;
  rfc_gsm_irat_rx_timing_info_type rx_mon_timing;
  int32                            ip2_cal_alpha;
  uint16                           rf_warmup_time;
} rfgsm_core_irat_rx_timings_type;



/* Alternate path is currently deprecated                                    */
/* This definition is used as a default 0 value to input to all APIs         */
/* requiring alternate path.                                                 */
#define RFGSM_ALT_PATH_NOT_SUPPORTED 0
...
/*!
  @details
   This function applies VBATT limit passed from MCS
...
boolean rfgsm_core_is_arfcn_in_high_lin( rfgsm_core_handle_type* core_handle_ptr, 
                                         uint16 arfcn, 
...
void rfgsm_core_configure_sawless(rfgsm_core_handle_type *core_handle_ptr, 
                                  boolean *is_high_lin);
...
void rfgsm_core_update_rx_freq_error(rfgsm_core_data_type *core_data, int32 freq_err);
	...
	/*----------------------------------------------------------------------------*/
	/*
	  rfgsm_core_rx_handle_get_address
	  ...
	  @retval
	  Adreess of rfgsm_core_rx_handles
	*/
