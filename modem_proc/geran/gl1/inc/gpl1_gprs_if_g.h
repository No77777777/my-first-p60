...
$Header: //components/rel/geran.mpss/7.4.0/gl1/inc/gpl1_gprs_if_g.h#2 $
...
typedef struct
{
  ...
  uint16  message_set;
  uint16  message_id;
}current_rr_mac_l1_msg_id_t;


typedef struct
{

current_rr_mac_l1_msg_id_t current_rr_mac_l1_msg_id;

/* Required to preserve the message contents until it can be handled */
 ...
 * scheduling system.
 */
mac_l1_edtm_cs_rel_tbf_alloc_t  queued_mac_l1_edtm_cs_rel_tbf_alloc;
...
typedef struct
{
  ...
  sys_algo_agc_T              packet_transfer_div_agc;
#endif
  uint8                       mac_mode;
  uint8                       l1_test_mode_tn;
  gas_id_t                    transfer_gas_id;
#ifdef FEATURE_GSM_GPRS_DL_REL_HDR_DEC_ONLY
  /* debug purpose to know how many dl blocks with tfi match may have been early header decoded */
  uint16                      t3192_tfi_match_count;
#endif /* FEATURE_GSM_GPRS_DL_REL_HDR_DEC_ONLY */
#ifdef FEATURE_GSM_DTM
  boolean                     slam_dtm_assign;        // use for calculating lowest tn during DTM to transfer state transition
#endif /* FEATURE_GSM_DTM */
/* NW has sent DL TBF or SINGLE BLOCK with polling at TBF starting time.*/
  boolean                     polling_at_tbf_start_time;
} transfer_data_T;
