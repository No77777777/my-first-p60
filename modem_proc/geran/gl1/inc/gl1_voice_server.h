...
/*===========================================================================
  ...
  $Header: //components/rel/geran.mpss/7.4.0/gl1/inc/gl1_voice_server.h#2 $
...
extern uint32 l1_vs_stop_client(gas_id_t gas_id);                                            

#ifdef FEATURE_GSM_AUDIO_GAP_OPTIMIZATION
extern uint32 l1_vs_change_in_voc(gl1_msg_chan_type msg_chan_type, 
...
extern uint32 l1_vs_set_ul_amr_mode(gl1_msg_amr_mode_type codec_mode, gas_id_t gas_id);         
...
void l1_vs_send_gsm_packet_to_client ( uint8 *data, boolean bfi, uint8 sid, 
                                       boolean taf, boolean ufi, gas_id_t gas_id);
...
extern uint32 l1_vs_set_dtx_mode(l1_vs_dtx_mode_t dtx_mode, gas_id_t gas_id);

/* Request GSM Voice Adapter to grant the vocoder to GSM during irat HO */
extern uint32 l1_vs_request_vocoder_ownership(gas_id_t gas_id); 

/* Returns TRUE if the client is ready to exchange voice frames */
extern boolean l1_vs_is_client_ready(gas_id_t gas_id);
  
/* Returns the voice server state */
extern l1_voice_server_state_t l1_vs_get_voice_server_state(gas_id_t gas_id);

/* Returns the Vocoder in use (AMR, AMRWB, FR, EFR or HR) */
extern uint32 l1_vs_get_vocoder_id(gas_id_t gas_id);

/* Returns true if dtx mode is enabled */
extern boolean l1_vs_get_dtx_mode(gas_id_t gas_id);
