...
$Header: //components/rel/geran.mpss/7.4.0/gl1/inc/l1_sc.h#6 $
...
31/08/17   ng        CR2101566 Get correct freq index while applying MCVS boost 1 frame in advance before intiating G2L search in ded mode
...
13/09/16   ng        CR1066410 Use LTE defined API for MACRO
07/12/17   km        CR2101400 Remove redundant FastSearchEndFN struct variable
...
04/11/16   akm       CR1086006 Resolving compiler warnings
...
04/03/15   sb        CR749012: GL1 changes for LTE removal
...
24/01/15   js        CR749012: GL1 changes for LTE removal
...
#define LTE_PRUNE_LOW_RSRP_LEVEL -130
#define LTE_PRUNE_LOW_RSRQ_LEVEL -30
#else
...
#define  L1_SC_DURATION_FCB_IDLE  (4+SYS_PARAM_MAX_FRAMES_FOR_SCH_DECODING_QTA)
...
#define L1_SC_DURATION_NEW_BCCH 6
#define L1_SC_BCCH_OFFSET       2500  /* CR1050459: 2500 qbits  */
 ...
 * 3 WCDMA + 4 LTE FDD + 4 LTE TDD upto 8 LTE can be measured.
 * For TDS, CMCC requires 9 TDS freq
 */

#define LIMIT_WCDMA_FREQS            (6)
#define LIMIT_LTE_FREQS             (11)
#define TOTAL_WCDMA_FREQS            (3)
#define TOTAL_LTE_FDD_FREQS          (4)
#define TOTAL_LTE_TDD_FREQS          (4)
...
/* 4 LTE FDD + 4 LTE TDD + (0 WCDMA | 9) TDS */
#define MAX_WCDMA_FREQS    (TOTAL_LTE_FREQS+ MAX_UTRAN_TDD_MPH_FREQ_UPDATE_LIST)
...
/* 4 LTE FDD + 4 LTE TDD + 3 WCDMA */
#define MAX_WCDMA_FREQS    (TOTAL_LTE_FREQS+TOTAL_WCDMA_FREQS)
...
#define TICK_TO_MS_DED   (480)               /*1 sacch period --480msec */
#define TICK_TO_MS_TRANS (60)                /*surround tick of 13 frames in transfer state--60msec */
#define TICK_TO_MS_IDLE  (235)               /*Idle tick is 51frames=235msec*/
#define TICKS_PER_TEN_SECONDS_IDLE      42   /* 10 sec */

/*Flag indicates active scenario for g2x mcpm request*/
extern uint8 g2w_active_mcpm_scenario[];
extern uint8 g2l_active_mcpm_scenario[];
extern uint8 g2t_active_mcpm_scenario[];

#define TOTAL_NGHBR_RAT_TYPE 3
#define TOTAL_GL1_MODES 3
#define GL1_MODE_XFER 1
#define GL1_MODE_IDLE 2
#define G2X_MAX_GAP_ABORT_COUNT 5
#define G2X_GAP_SUCCESS_COUNT 3
...
typedef enum
{
  GTOW_SRCH_TYPE_NULL,
  GTOW_SRCH_TYPE_ID,
  GTOW_SRCH_TYPE_RECONFIRM,
  GTOW_SRCH_TYPE_ID_IN_TBF,
  GTOW_SRCH_TYPE_DEDICATED
} e_wcdma_srch_type;
...
typedef enum
{
  ...
  GTOW_SRCH_STATE_CLEANUP,
  GTOW_SRCH_STATE_CLEANUP_WAIT,
  GTOW_SRCH_STATE_ABORTING,
  GTOW_SRCH_STATE_RAN_OUT_OF_FRAMES,
  GTOL_SRCH_STATE_ABORT_CNF_WAIT
} e_wcdma_srch_state;
...
/* 
** for fast search we record the measurement status to allow ghost frequencies to be blacklisted
*/

#define FAST_SEARCH_ENABLE_LIMIT     (4)

typedef enum
{
   G2X_FS_INACTIVE       =0x00,
   G2X_FS_NOT_MEASURED   =0x01, 
   G2X_FS_PREV_MEASURED  =0x02,
   G2X_FS_NEWLY_MEASURED =0x04,
   G2X_FS_NO_RESULT      =0x08,
   G2X_FS_BLACKLISTED    =0x10

} g2x_freq_status_e;

typedef enum
{
   FAST_SEARCH_INACTIVE  =0x00,
   FAST_SEARCH_START     =0x01,
   FAST_SEARCH_ACTIVE    =0x02, 
   FAST_SEARCH_PAUSED    =0x04,
   FAST_SEARCH_ARMED     =0x08,
   FAST_SEARCH_REARMED   =0x10

} g2x_fast_search_state_e;
...
#endif
    uint16                             priority;               /* absolute priority of an IRAT frequency 0..7 */
    rr_l1_irat_rat_e                   rat;                    /* WCDMA / LTE / TDD */
#if defined (FEATURE_LTE) 
    rr_l1_eutran_type_e                lte_type;                /* added for LTE TDD */
#endif
    uint16                             best_lte_tdd_cell_idx;   /* for LTE TDD vcell_info */
    uint16                             freq_status;             /* FR25799 fast search optimisation */

    uint8                              freq_age;                /* G2L meas only support */
...
typedef struct
{
  e_GtowState  state, prev_state;

  /* These state machines are used for scheduling both WCDMA
   * LTE cells.
   */
  e_wcdma_srch_state  srch_state;
  e_wcdma_srch_type   srch_type;
...
/** use LTE values LTE_CPHY_IMEAS_MAX_MEAS_CELLS=9 in TA at present*/
#define G2L_CM_LIMIT_MEAS_CELLS (LTE_CPHY_IMEAS_MAX_MEAS_CELLS)
...
/*
** G2L if a measurement occurred but was not detected,
...
typedef struct
{
  uint8   meas_cnt;
  uint8   det_cnt;
  uint8   consecutive_fail_det_cnt;
...
*/
  ...
  uint32  bForcedReq2Index;  /* bit mask for each freq_index forced req     */
  uint32  bForcedMeas2Index; /* bit mask for each freq_index forced meas    */
   ...
   */
  /* FR21768 additions */
  uint32  bFastSearchFreqList;
  uint32  bFastSearchFreqMeas;
  uint32  FastSearchExpiryFN;    /* 30s timeout to clear blacklisted frequencies in Idle and dedicated*/

  uint16  fastSearchState;
  uint16  fastSearchIndex;
  uint16  fastSearchNumMeasured;  
  uint16  fastSearchLoopCount;
...
typedef struct
{
   ...
   /*Flag to indicate if cell is not reconfirmed for 30 seconds */
   boolean    cell_stale_sch;
   ...
   uint8      bad_gdet_pwr_mon_cntr; /* bad gdet counter incremented when gdet score goes below synq fr threshold */
   boolean    gdet_based_sch_reconfirm; /* field to mark need of SCH reconfirmation as per synq algo */
#endif
#endif
...
typedef struct
{
  ...
  rr_l1_irat_rat_e        active_irat;
   ...
   /*Stores the GSTMR FN in which last dedicated channel transition occured  */
   uint32                 last_dedi_sc_start_FN; 

   /*Flag to indicate if NCELL reporting can be started and acq can be scheduled in dedicated mode. SET based on sys threshold of 150 frames after every transition */
   boolean                start_ncell_rep_n_acq_dedi;

   cell_T*                last_top3_fcch_attempted_cell;
 ...
 * complete.
 */
extern l1_ncell_afc_info_t l1_scell_afc_info[];
...
typedef struct
{
  ...
  boolean  wcdma_irat_mcpm_active;
  boolean  wcdma_update_list_irat_active;
  boolean  elevator_flag; 
  boolean  l1_skip_g2x_pkt_txfr;  /* skip any G2X if gprs timeslot is reconfigured */
  boolean  l1_skip_g2x_ded_mode;  /*Skip any G2X if TS change is happening*/
 /*
 ** TRUE: IRAT is active. 
 ** FALSE: GBTA is active
 */
  boolean                         gbta_not_allowed;

  /* Used for IRAT reservation with TRM and DRX manager */
  uint32   id_search_fn;
  boolean  l1_irat_reserve_at_sent;
  uint8    drx_ref;

#ifdef  FEATURE_GSM_TO_LTE
  boolean  lte_irat_active;
  boolean  lte_update_list_irat_active;
  boolean  lte_irat_mcpm_active;
  uint16   lte_cm_irat_active;
  g2l_mdsp_cmd_events_e           l1_sc_trigger_g2l_gap;   
  ...
  /*G2X Abort Flag, if search is aborted by XL1 */
  boolean                         g2x_aborted;
  uint8                           g2x_abort_count;
  uint8                           g2x_success_count;
  /* Indicates fast search is ongoing only on 1 EARFCN */
  boolean                         g2l_one_freq_meas_fast_srch;

  boolean rfTuneback_required;
 ...
 boolean                          tds_irat_mcpm_active;
  uint32                                reconf_req_fn;
/* bit field must be larger enough for MAX_WCDMA_FREQS */
  uint32                          toggle_id_reconf;
  byte                                  tds_state;
  ...
  boolean                         issue_g2x_deinit;

  uint32                          scheduled_wcdma_reconf_fn;
...
/*Variable which stores gstmr FN on which TS adjustment has happened*/
extern uint32 ded_gstmr_cha_fn[];

#if defined ( FEATURE_DUAL_SIM ) || defined ( FEATURE_TRIPLE_SIM )
extern gpl1_g2x_ta_data_t  gpl1_g2x_ta_data[];
...
#define NO_FRAMES_TO_CHECK_IF_GL1_IS_STUCK 35
