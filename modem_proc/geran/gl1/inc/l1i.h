...
$Header: //components/rel/geran.mpss/7.4.0/gl1/inc/l1i.h#4 $
...
07/03/17   mk       CR1097205: adding a comment block explaining when "_divrx_disabled_in_sacch_period" is set and reset 
...
27/07/16   mn       CR1046704: Using MSGR variant 
...
26/04/16   hd       CR998694 Declare l1_update_and_send_srch_time_ind
...
01/04/15   aga      CR822202 TRM interface for QTA
...
11/21/14   dp       CR759046 Fix debug code so it doesn't crash in unintended scenarios
...
/*LTE and TDS Irat searched in idle can take 15 frames, +2, if they have just begun, +3 to make sure j.i.c*/
#define GL1_MAX_FRAMES_FOR_IDLE_ABORT   20
...
typedef struct
{
   ...
   word   rx_gain_set;
   dBx16_T  snr;
   dBx16_T divrx_power_instant;
   int32  divrx_power;    /* accumulator of dBx16_T powers */
   int32  divrx_power_sub;
   int32  divrx_power_block;
   int32  divrx_power_val;
   boolean  is_divrx_disabled_in_sacch_period; /* Default:FALSE, set TRUE if diveristy is disabled within a SACCH reporting period, reset at end of every SACCH period*/
...
enum {
     ...
     , L1_TRANSFER_COMMAND_IDLE_ABORTED   = 62  /*@CR 757734*/
     , L1_TRANSFER_COMMAND_RESERVE_AT     = 63
#endif /* FEATURE_DUAL_SIM | FEATURE_TRIPLE_SIM */
     };
...
enum {
   ...
   ,L1_MULTI_SIM_PSCAN_MCPM_WAIT
   ,L1_MULTI_SIM_PSCAN_MCPM_WAIT_ABORT
   ...
   ,L1_MULTI_SIM_ACQ_BCCH_LIST_MCPM_WAIT
   ,L1_MULTI_SIM_ACQ_BCCH_LIST_MCPM_WAIT_ABORT
   ...
   ,L1_MULTI_SIM_ACQUIRING_MCPM_WAIT
   ,L1_MULTI_SIM_ACQUIRING_MCPM_WAIT_ABORT
   ...
   ,L1_MULTI_SIM_FIND_SCH_MCPM_WAIT
   ,L1_MULTI_SIM_FIND_SCH_MCPM_WAIT_ABORT
   ...
   /*Sub State where AsDiv decision for PScan Phase is made*/
   ,L1_ASDIV_ACQ_SWITCH
...
typedef struct {
   ...
   dBx16_T          rx_power_instant[ RR_MAX_CANDIDATE_FREQ_LIST ];
...
/* l1_bplmn_running_state when ACTIVE implies that GERAN is active in BPLMN procedure */
typedef enum
{
    BPLMN_INACTIVE,
    BPLMN_ACTIVE
} l1_bplmn_running_state_T;
...
typedef struct {
  ...
  uint16                      decstat;
   ...
   uint32                     do_pwr_meas_FN;
   boolean                    non_updated_lai_in_nmo1;
}idle_data_T;

/* Buffer to implement running average of serving cell measurements */
#define L1_IDLE_SERV_MEAS_BUF_LEN 20  /* conservative */
...
typedef struct
{
  ...
  uint32 ta_dur;
  boolean ta_prepared;
  boolean ta_gap_started;
  uint32  ta_gap_start_fn;;
} l1_idle_ta_t;
...
typedef struct
{
  boolean              background_HPLMN_search_suspended;
  uint8                idle_rx_in_progress;
  uint8                idle_monitors_in_progress;

  dBx16_T              idle_serv_meas_buf[L1_IDLE_SERV_MEAS_BUF_LEN];
#ifdef FEATURE_GSM_RX_DIVERSITY_IDLE
  dBx16_T              idle_serv_divrx_meas_buf[L1_IDLE_SERV_MEAS_BUF_LEN];
#endif /*FEATURE_GSM_RX_DIVERSITY_IDLE*/
  cbch_state_T         cbch_state;
  gl1_msg_rx_rpt       idle_rpt;
  gl1_msg_decode_rpt   idle_decode_rpt;
  gl1_msg_metrics_rpt  idle_metrics_rpts[2][4];
  boolean              idle_update_afc_tt;
  boolean              cbch_abort_in_progress;
  boolean              receive_cbch_sdcch_4;
  boolean              receive_cbch_sdcch_8;
  uint8                CBCH_TN;
  uint8                CCCH_TN;
  uint8                cbch_rx_in_progress;
  sys_algo_agc_T       cbch_agc;

  uint32               l1_bad_pch_block_cnt;
  byte                 last_idle_sub_state;
  uint32               next_req_cbch_frame;
  uint8                cbch_drx_ref;
  uint8                tag;

/* Variables to support GPRS non-DRX timer. */
  uint32               l1_idle_last_non_drx_timer_FN;
  boolean              l1_idle_non_drx_timer_flag;
  boolean              l1_idle_mm_non_drx_mode_flag;
  idle_data_T         *idle_data_ptr_1;
  idle_data_T         *idle_data_ptr_2;

#ifdef FEATURE_GTOW_BACKGROUND_PLMN_SEARCH
  boolean              background_wcdma_plmn_scan_suspend_ncell;
uint32  ind_timer_value;
uint32  ind_timer_FN;
#endif

  uint16               not_idle_cnt;
  uint32               next_req_frame;
  uint8                drx_ref;
  boolean              perform_post_idle_init;

  boolean              l1_is_asynch_int_possible;

/* Define a timer for controlling SAIC disable in idle */
  rex_timer_type       l1_idle_saic_disable_timer;

  boolean              is_saic_disable_possible;
#if defined( FEATURE_G2X_IDLE_TUNEAWAY) || defined(FEATURE_G2X_REORG_TUNEAWAY)
  l1_idle_ta_t         l1_idle_ta;
#endif /*  FEATURE_G2X_IDLE_TUNEAWAY || FEATURE_G2X_REORG_TUNEAWAY */
}l1_local_idle_data_t;
...
{
    GL1_GFCM_NULL,
    GL1_GFCM_TIMER_ACTIVE,
    GL1_GFCM_TIMER_STOPPED,
    GL1_CFCM_TIMER_EXPIRED
} L1_cfcm_state_type;

typedef struct
{
  L1_cfcm_state_type  gl1_mms_cfcm_state;
  uint32              gl1_mms_gfn_timer;
}L1_cfcm_data_struct;

extern L1_cfcm_data_struct gl1_cfcm_data;
#endif
...
typedef struct {
   ...
   boolean                    time_slot_change_pending;
      /*Flag indicates that X2G IRAT HO is triggered*/
   boolean                    x2g_inter_rat_ho_triggered;
   boolean                    acq_for_blind_ho;
   l1_isrtsk_blk_params_T     next_params;
   l1_isrtsk_blk_params_T     current_params;
   uint32                     transfer_to_idle_fn;
} ISRTIM_CMD_BLK;
...
extern uint32 gl1_hw_sleep_wup_conf_calculate_next_pch_gap(gas_id_t gas_id);
extern void   gl1_hw_wmgr_drx_require_tick(uint32 reprog_fn, gas_id_t gas_id);
extern uint32 gl1_idle_get_gap_to_next_activity(uint32 frame_offset, gas_id_t gas_id );
...
typedef union
{
  ...
  GfwHostInvalidTxRflmIndexRsp		gfwHostInvalidTxRflmIndexRsp;
#endif
...
void  L1_send_TDS_RSSI_MEAS_RSP ( gl1_hw_x2g_rssi_meas_ctl_t *ptr, sys_modem_as_id_e_type as_id );

void L1_send_TDS_G2T_ABORT_REQ (gas_id_t gas_id);

void L1_send_TDS_ACQ_RSP( gsm_irat_acquisition_rsp_t *ptr,sys_modem_as_id_e_type as_id );
...
typedef union
{
  ...
  lte_cphy_irat_meas_build_rf_script_cnf_s lte_cphy_irat_meas_build_rf_script_cnf;
...
#endif
#ifdef FEATURE_GSM_TO_WCDMA_MSGR
  wl1_irat_x2w_init_cnf_struct_type       wl1_irat_g2w_init_cnf;
  wl1_irat_x2w_deinit_cnf_struct_type     wl1_irat_g2w_deinit_cnf;
  wl1_irat_x2w_abort_cnf_struct_type      wl1_irat_g2w_abort_cnf;
  wl1_irat_x2w_pre_srch_cnf_struct_type   wl1_irat_g2w_pre_srch_cnf;
  wl1_irat_x2w_srch_cnf_struct_type       wl1_irat_g2w_srch_done;
#endif /* FEATURE_GSM_TO_WCDMA_MSGR */

  geran_gl1_qta_msg_type          geran_gl1_qta_startup_req;
  geran_gl1_qta_msg_type          geran_gl1_qta_cleanup_req;
...
#endif
  cxm_coex_tech_state_bcast_ind_s        cxm_coex_tech_state_bcast_info;
} u_geran_cxm_msgr;
...
{
  q_link_type       link;
  mmoc_vfr_map_s_type  msg;

} geran_gl1_vfrmap_msgr_t;
...
===========================================================================*/
extern uint32 l1_idle_saic_get_timer_remaining(gas_id_t gas_id);
...
===========================================================================*/
extern void l1_ded_set_l1_diversity_status(boolean status, gas_id_t gas_id);

extern background_HPLMN_activity_type L1_get_current_background_HPLMN_search_activity( gas_id_t gas_id );
...
enum {
   ...
   L1_PAR_ACQ_SPEEDUP           = 0x0800,
#endif /* FEATURE_GSM_PARALLEL_SCH_DEDICATED */
   L1_FIND_BCCH_SPEEDUP         = 0x1000,
   L1_DED_SPEEDUP_LOOPBACK      = 0x2000,
...
typedef enum{
  ...
  L1_FNAME_L1_SC_IRAT_TDS             =    4,
  L1_FNAME_L1_SC_RCV                  =    8,
  L1_FNAME_L1_SC_PWR                  =    0x10,
  L1_FNAME_GPL1_GPRS_DYNAMIC          =    0x20,
 }l1_fname_T;
