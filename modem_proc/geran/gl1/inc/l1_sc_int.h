...
$Header: //components/rel/geran.mpss/7.4.0/gl1/inc/l1_sc_int.h#1 $
...
14/04/15   cgc       CR794901 deprecated MPH_SURROUND_WCDMA_DEDICATED_MEAS_IND
...
===========================================================================*/
#define GL1_INVALID_POWER_LEVEL        (0)
#define GL1_TDS_RUNNING_AVG_COUNT       (4)

#ifdef FEATURE_GSM_DED_SCELL_MONITOR
extern boolean  ba_list_empty ;
...
typedef enum
{
  PRX_MEAS_BUFFER,
  DIVRX_MEAS_BUFFER,
} gl1_power_meas_buffer_type;
/* Type for implementing running averages of measurements */
/* measurement on diversity chain is calculated in the
   same way as primary chain. If diversity chain measurement
   is invalid or 0 then use primary chain measurement. This
   will keep other params like buf_len, avg_len, num_meas
   oldest_ptr, newest_ptr and just_stuffed same for both
   primary and diversity chain*/
typedef struct
{
   dBx16_T  *meas_buf;
   uint8    buf_len;
...
typedef struct
{
   ...
   dBx16_T            divrx_power_instant; /* instantaneous power */
...
/* can be removed if FEATURE_GSM_G2X_TIMELINE_REDUCTION is mainlined and NV always on */
...
/*Function to check if Idle or Dedicated Searches are Active*/
