...
$Header: //components/rel/geran.mpss/7.4.0/gl2/src/l2_main.c#2 $ $DateTime: 2020/10/21 03:56:32 $ $Author: pwbldsvc $
...
typedef union
{
  ...
  ph_cs_abort_ind_T       ph_cs_abort_ind;
...
===========================================================================*/
static void l2_initialise(const boolean from_power_on, const gas_id_t gas_id)
{
...
static l2_store_T * l2_process_mdl_release_req(l2_msg_type *msg_ptr)
{
  ...
  if (msg_ptr != NULL)
  {
    gas_id_t gas_id = msg_ptr->mdl_release_req.gas_id;
    ...
    if (l2_data_ptr != NULL)
    {
      ...
      l2_initialise(FALSE, gas_id);
      MSG_GERAN_ERROR_0_G("MDL_RELEASE_REQ resetting L2");
...
FUNCTION  l2_handle_ph_cs_abort_ind

DESCRIPTION
  Processes the PH_CS_ABORT_IND primitive from GERAN L1
...
===========================================================================*/
static l2_store_T *l2_handle_ph_cs_abort_ind(l2_msg_type *new_message_ptr)
{
  l2_store_T *l2_data_ptr;
  gas_id_t    gas_id;

  /* determine the correct data space using the GAS ID */
  gas_id      = new_message_ptr->ph_cs_abort_ind.gas_id;
  l2_data_ptr = l2_get_store(gas_id);
  if (l2_data_ptr != NULL)
  {
    MSG_GERAN_ERROR_2_G("PH_CS_ABORT_IND rcvd with cause %d, chan %d",
                        (int)(new_message_ptr->ph_cs_abort_ind.l1_abort_cause),
                        (int)(new_message_ptr->ph_cs_abort_ind.l2_channel_type));

    /* default response to this Layer 1 primitive is to inform GRR that there */
    /* has been some error on the link and it should perform abnormal release */
    l2_data_ptr->current_l2_channel_type = new_message_ptr->ph_cs_abort_ind.l2_channel_type;
    l2_data_ptr->current_SAPI            = SAPI0;
...
static l2_store_T * l2_handle_L1_message(l2_msg_type *new_message_ptr)
{
  ...
  switch(new_message_ptr->message_header.message_id)
  {
    ...
    case PH_CS_ABORT_IND:
      l2_data_ptr = l2_handle_ph_cs_abort_ind(new_message_ptr);
      break;
...
static l2_store_T * l2_handle_timer_message(l2_msg_type *new_message_ptr)
{
  ...
  if (new_message_ptr->message_header.message_id == MID_TIMER_EXPIRY)
  {
    ...
    {
      L2_timer_state_T T200_state = sapi_info_ptr->T200_state;

      MSG_GERAN_MED_2_G("T200 expired for SAPI%d with T200 state as %d",(int)sapi,(int)T200_state);

      /* Clear the timer entry. */
      l2_clear_T200_timer(sapi_info_ptr);

      if (T200_state == RUNNING)
      {
        ...
        {
          MSG_GERAN_ERROR_0_G("Channel type changed");
        }
    ...
    {
      MSG_GERAN_HIGH_2_G("Optional send timeout for SAPI%d, seq num=%d",(int)sapi, sequence_number);
      ...
      if (sequence_number == sapi_info_ptr->T_ack_seq_number)
      {
        ...
        if (l2_data_ptr->dcch_ready == TRUE)
        {
          ...
          {
            MSG_GERAN_ERROR_0_G("Optional send timeout: DCCH buffer empty!");
          }
        }
      ...
      {
        MSG_GERAN_ERROR_1_G("Optional send timeout: sequence mismatch T_ack_seq_number=%d", 
          sapi_info_ptr->T_ack_seq_number);
      }
    }
    else
    {
      MSG_GERAN_ERROR_1_G("Unknown timer %d", new_message_ptr->mid_timer_expiry.timer_id);
    }
  }
  else
  {
