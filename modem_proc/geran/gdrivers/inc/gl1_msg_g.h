...
$Header: //components/rel/geran.mpss/7.4.0/gdrivers/inc/gl1_msg_g.h#2 $
...
27/05/16    snjv    CR1000909 ACI400 switchpoint detection stat from GFW to GRF via GL1
...
05/01/16   nk       CR901011 Backout CR814147
...
22/04/15   ak       CR813711 Debug code for SCE power monitors
...
typedef enum
{
  ...
  ,RACH_STATE_CONFLICT_CHECK
#endif
} rach_state_type;

/* States for acquisition results */
typedef enum
{
  NO_CELL_FOUND = 16,
  FCCH_FOUND,
  SCH_CRC_FAILED,
  CONVENTIONAL_SCH_DECODED,
  PARALLEL_SCH_DECODED
} gl1_cell_acq_decode_status_t;
...
#ifdef FEATURE_GSM_COEX_RACH_DELAY
typedef void (*gl1_msg_tx_rach_callback)(uint16 content, gas_id_t gas_id, boolean yield);
#else
 ...
 */
extern void gl1_msg_get_afc_info( int32 *afc_val_p, gas_id_t gas_id );
...
FUNCTION gl1_msg_get_cch_tt_acc
...
SIDE EFFECTS
  None
===========================================================================*/
extern int32 gl1_msg_get_dtm_expected_power_divrx(gas_id_t gas_id);
...
extern gl1_cell_acq_decode_status_t gl1_hw_was_gsm_cell_detected(
                                      ARFCN_T         arfcn,
                                      uint8           bsic_expected,
...
extern gl1_cell_acq_decode_status_t gl1_hw_was_parallel_sch_detected(
                                     ARFCN_T         arfcn,
                                     uint8           bsic_expected,
                                     acq_decode_rpt *acq_decode_data_p,
                                     acq_decode_rpt *acq_decode_rpt_p,
...
extern uint32 gl1_msg_get_start_acq_FN( gas_id_t gas_id );

boolean gl1_msg_getting_sch( gas_id_t gas_id );

#ifdef FEATURE_GSM_SCE_PWR_MEAS_DBG

/* debug store for pwr meas cmds / state changes */
typedef struct gl1_msg_pwr_meas_dbg_store_S gl1_msg_pwr_meas_dbg_store_T; /* forward declaration */
typedef enum
{
   GL1_MSG_PWR_MEAS_DBG_CMD_DO_PWR_MEAS                       = 1,
   GL1_MSG_PWR_MEAS_DBG_CMD_DONE_PWR_MEAS                     = 2,
   GL1_MSG_PWR_MEAS_DBG_CMD_STOP_PWR_MEAS                     = 3,
   GL1_MSG_PWR_MEAS_DBG_CMD_ABORT_PWR_MEAS                    = 4,
   GL1_MSG_PWR_MEAS_DBG_CMD_TERMINATE_PWR_MEAS                = 5,
   GL1_MSG_PWR_MEAS_DBG_CMD_PENDING_STOP_PWR_MEAS             = 6,
   GL1_MSG_PWR_MEAS_DBG_CMD_UBS_STOP_PWR_MEAS                 = 7,
   GL1_MSG_PWR_MEAS_DBG_CMD_DO_PWR_MEAS_FAIL_SM_FULL          = 8,
   GL1_MSG_PWR_MEAS_DBG_CMD_DO_PWR_MEAS_FAIL_INVALID_MAX_MEAS = 9,

   GL1_MSG_PWR_MEAS_DBG_CMD_GBTA_Q_PWR_MON_MISSED_PAGE        = 21,
   GL1_MSG_PWR_MEAS_DBG_CMD_GBTA_Q_PWR_MON_ON_PCH             = 22,
   GL1_MSG_PWR_MEAS_DBG_CMD_GBTA_DO_PWR_MEAS                  = 23,

   GL1_MSG_PWR_MEAS_DBG_CMD_DO_INT_MEAS                       = 31,
   GL1_MSG_PWR_MEAS_DBG_CMD_DONE_INT_MEAS                     = 32,
   GL1_MSG_PWR_MEAS_DBG_CMD_STOP_INT_MEAS                     = 33,
   GL1_MSG_PWR_MEAS_DBG_CMD_ABORT_INT_MEAS                    = 34,
   GL1_MSG_PWR_MEAS_DBG_CMD_TERMINATE_INT_MEAS                = 35,
   GL1_MSG_PWR_MEAS_DBG_CMD_PENDING_STOP_INT_MEAS             = 36
} gl1_msg_pwr_meas_dbg_cmd_T;
...
#define GL1_MSG_PWR_MEAS_DBG_NEW_ENTRY(cmd, gas_id)  ((void) gl1_msg_pwr_meas_dbg_new_entry(cmd, gas_id))

#else /* FEATURE_GSM_SCE_PWR_MEAS_DBG */

#define GL1_MSG_PWR_MEAS_DBG_NEW_ENTRY(cmd, gas_id)
