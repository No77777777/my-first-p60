...
$Header: //components/rel/geran.mpss/7.4.0/gdrivers/inc/gtmrs_g.h#2 $
...
11/06/15   js        CR848503 GL1 ISR profiling data during multi sim FW RF initialization
...
30/09/14   mc        CR656792:24-bit USTMR on VSTMR targets
...
extern uint32 gstmr_qs_in_future_frame_to_xo(uint16 qs_offset, gas_id_t gas_id, int fn_offset);
#endif
...
extern void GSTMR_SET_HWFN( gas_id_t gas_id );

extern boolean gstmr_is_int_pending( gas_id_t gas_id );

extern uint32 calc_sync_gstmr_tick( uint32 *ustmr, gas_id_t gas_id );
extern uint32 gstmr_get_ota_ustmr_start_time( gas_id_t gas_id );

extern uint32 gstmr_get_ota_ustmr_start_time_sw_fn(uint8 offset,  gas_id_t gas_id );

extern uint32 gstmr_get_current_fn_ustmr_start_time(gas_id_t gas_id) ;
...
typedef struct
{
  uint32    number_of_dropped_tx;
  uint32    number_of_success_pow_mon;
  uint32    number_of_success_pow_mon_old;
} geran_rf_timeline_recovery_type;
#endif 
...
#define IS_FRAME_CONFLICT( fn_a_start, fn_a_end, fn_b_start, fn_b_end ) \
   ( !( IS_FRAME_NUM_LATER( (fn_b_start), (fn_a_end) ) || \
        IS_FRAME_NUM_LATER( (fn_a_start), (fn_b_end) ) ) )
...
typedef enum
{
    ...
    START_SEND_BLK_CMD, 
    END_SEND_BLK_CMD,
    START_MDSP_ASYNC_CMD_SEND_TO_FW,
    END_MDSP_ASYNC_CMD_RESP_FW
} tag_id;
...
#ifdef FEATURE_GERAN_RF_TIMELINE_ERROR_RECOVERY
  geran_rf_timeline_recovery_type geran_rf_timeline_recovery;
...
#define USTMR_PER_FRAME               ( 88616 ) /* 19.2MHz * 4.61538 ms = 88615.296 */
