...
$Header: //components/rel/geran.mpss/7.4.0/gdrivers/inc/gl1_hw_clk_ctl_g.h#4 $ $DateTime: 2021/08/12 06:17:45 $ $Author: pwbldsvc $
...
typedef enum
{
  ...
  GL1_MCPM_GERAN_INIT     = 2,
  GL1_MCPM_GERAN_IDLE     = 3,
  GL1_MCPM_GERAN_TX_ON    = 4,
  GL1_MCPM_GERAN_TX_OFF   = 5,
  GL1_MCPM_GERAN_RAND     = 6,
  GL1_MCPM_GERAN_VOICE    = 7,
  GL1_MCPM_GERAN_DATA     = 8,
  GL1_MCPM_GERAN_DTM      = 9,
  GL1_MCPM_GERAN_SLEEP    = 10,
  GL1_MCPM_GERAN_FTM      = 11,
  GL1_MCPM_GERAN_STARTING = 12
} gl1_mcpm_state_type;
...
typedef struct
{
 uint32 ScenarioId;
 uint32 ActionTime;
} gl1_mcpm_scenario_info;

typedef struct
{

 gl1_mcpm_scenario_info scenario;
} gl1_mcvs_param_type;


typedef struct
{
 mcpm_nbr_meas_type nbr_type;
 boolean irat_nbr;
} gl1_mcpm_nbr_param_type;


typedef struct
{
 mcpm_request_type mcpm_request;
 uint16 update_info;
 union
 {
 gl1_mcpm_state_type gl1_state;
 mcpm_nbr_meas_type nbr_type;
 gl1_mcvs_param_type mcvs_param;
 boolean aeq_enable;
 gas_id_t gas_id;
 } mcpm_param;
 uint32 mcvs_q6_speed;
 /* TRUE if this DPC task has not finished in DPC context */
 boolean ongoing;
 gas_id_t gas_id;
}gl1_mcpm_update_gsm_dpc_param_type;


#define MCPM_DPC_INDEX_MAX    ( ( NUM_GERAN_DATA_SPACES * 4 ) + 1 )

typedef struct
{
   boolean irat_active;
   boolean bplmn_active;
}gl1_g2x_mcpm_status_type;
...
extern uint32 gl1_get_mcvs_speed(gas_id_t gas_id);
