/*===========================================================================
     ...
     IMPLEMENTATION OF GPRS NPL1 DOWNLINK PCCCH FUNCTIONS

GENERAL DESCRIPTION
   This file implements GPRS NPL1 downlink PCCCH functions.
...
$Header: //components/rel/geran.mpss/7.4.0/gdrivers/src/gl1_msg_pccch.c#1 $
...
/*-----------------------------------------------------------------------------
* Macros to handle functionality local to pccch
------------------------------------------------------------------------------*/
...
DESCRIPTION
   This function initializes data structures that are used by pccch.
...
void gl1_msgi_pccch_init( void )
{
  ...
  chn_cfg_params.chan_type   = GL1_HW_PCCCH;
...
)
{
  ...
  /* There is only one message for PCCCH */
...
)
{
  ...
  * For the received message,store the frequency and signal info.
  ...
  chn_rx_params.signal_info[ MSG_NUM ].rx_signal_strength
    = sys_algo_agc_gprs_expected_pwr(agc_ptr->pwr_dBm_x16, GL1_HW_PCCCH,GERAN_ACCESS_STRATUM_ID_1);
  chn_rx_params.agc_ptr = agc_ptr;
#ifdef FEATURE_GSM_RX_DIVERSITY_DATA
  chn_rx_params.signal_info[ MSG_NUM ].rx_signal_strength_divrx
    = sys_algo_agc_gprs_expected_pwr(agc_ptr_divrx->pwr_dBm_x16, GL1_HW_PCCCH,GERAN_ACCESS_STRATUM_ID_1);
...
/*=============================================================================
FUNCTION gl1_msg_abort_dl_pccch

DESCRIPTION
   Immediately bborts the processing of the PCCCH. Some of the drivers
...
void gl1_msg_abort_pccch( void )
{
    ...
    gl1_msg_pdch_abort_dl_pbcch_pccch_pacch_pdtch( GL1_HW_PCCCH,GERAN_ACCESS_STRATUM_ID_1);
}
