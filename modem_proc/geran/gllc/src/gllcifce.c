...
***
*** $Header: //components/rel/geran.mpss/7.4.0/gllc/src/gllcifce.c#2 $
***
...
uint16 gllc_ll_get_max_N201_U( uint8 sapi )
{
  if ( sapi >= 16 )
  {
    MSG_GERAN_ERROR_1( "GLC LL: Get Max N201-U: Invalid SAPI %d", sapi);
    return (0);
  }
  else if ( gllc_lle_lookup[sapi] == GLLC_LL_SAPI_RESERVED )
  {
    MSG_GERAN_ERROR_1( "GLC LL: Get Max N201-U: Reserved SAPI %d", sapi);
    return (0);
  }
  else
