...
***
*** $Header: //components/rel/geran.mpss/7.4.0/gllc/src/gllcstate.c#3 $
***
...
)
{
  ...
  MSG_GERAN_HIGH_1_G("GLC ME: Rcvd LLGMM-ASSIGN-REQ, Debug [%d]", 
    geran_ciphering_logging_enabled(gas_id));
...
)
{
  ...
  case GLLC_LLME_TLLI_ASSIGNED:
    ...
    if ( trigger_req_mes -> tlli == gllc_llme[gas_id].tlli )
    {
      ...
      {
        MSG_GERAN_HIGH_0_G( "GLC ME: Rcvd LLGMM-TRIG-REQ Page Resp");
        grr_cause = GRR_CAUSE_GMM_PAGE_RESPONSE;
      }
      else if ( trigger_req_mes -> cause == GMMLLC_CELL_UPDATE )
      {
        MSG_GERAN_HIGH_0_G( "GLC ME: Rcvd LLGMM-TRIG-REQ Cell Update");
        grr_cause = GRR_CAUSE_GMM_CELL_UPDATE;
      }
      else if ( trigger_req_mes -> cause == GMMLLC_CELL_NOTIFY )
      {
        MSG_GERAN_HIGH_0_G( "GLC ME: Rcvd LLGMM-TRIG-REQ Cell Notify");
...
)
{
  ...
  MSG_GERAN_HIGH_0_G( "GLC ME: Rcvd LLGMM-SUSPEND-REQ");
...
)
{
  ...
  {
    MSG_GERAN_HIGH_1_G( "GLC ME: Rcvd STRT-RDY-TMR-REQ %dms",
...
)
{
  ...
  {
    MSG_GERAN_HIGH_0_G( "GLC ME: Rcvd STOP-RDY-TMR-REQ");
...
void gllc_llme_process_grr_resume_cnf_mes ( gas_id_t gas_id )
{
  ...
  if ( gllc_llme[gas_id].is_resume_cnf_pending )
  {
    ...
    MSG_GERAN_HIGH_0_G( "GLC ME: Rcvd GRR-RESUME-CNF " );
    ...
    MSG_GERAN_HIGH_2_G("GLC ME: gcf_flag %d, qos_flag %d ",
    ...
    for ( lle = 0; lle <= (uint32)GLLC_LL_LAST_SAPI; lle++ )
    {
      ...
      if (((gllc_lle[gas_id][lle].state == GLLC_LLE_ADM) &&
          ...
          ((gllc_lle[gas_id][lle].state == GLLC_LLE_ADM) &&
           ...
           (!(mcfg_gcf_nv_get_status()) && b_qos_enabled_flag))
         )
      {
  ...
  else
  {
    ...
    MSG_GERAN_HIGH_0_G( "GLC ME: GRR-RESUME-CNF is unexpected" );
...
)
{
  ...
  case GLLC_LLE_ADM:
    ...
    MSG_GERAN_HIGH_1_G( "GLC LL %d: Rcvd LL-ACT-IND ADM",
      gllc_sapi_addr_lookup[lle]);
...
)
{
  ...
  case GLLC_LLE_ADM:
    ...
    MSG_GERAN_HIGH_1_G( "GLC LL %d: Rcvd LL-DEACT-IND ADM",
      gllc_sapi_addr_lookup[lle]);
...
)
{
  ...
  case GLLC_LLE_ABM:
    ...
    MSG_GERAN_HIGH_2_G( "GLC LL %d: xid reset in progress %d.",
...
)
{
  ...
  if ( gllc_lle_validate_u_format_cr_xid_req(gas_id, lle, u_frame) )
  {
    ...
    {
      MSG_GERAN_HIGH_2_G( "GLC LL %d: Rcvd XID Cmd L3 St %d",
        gllc_sapi_addr_lookup[lle], gllc_lle[gas_id][lle].state );
    }
    else
    {
...
)
{
  ...
  MSG_GERAN_LOW_0_G("GLC LL: Info DL Suspend");
