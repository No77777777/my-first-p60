...
***
*** $Header: //components/rel/geran.mpss/7.4.0/gmac/src/gmacutil.c#4 $
***
...
void gprs_l1_mac_acc_func(mac_l1_sig_t *common_msg_ptr,
                          gas_id_t      gas_id)
{
  gs_queue_id_T l1_gs_queue = geran_get_gl1_queue_id(gas_id);
  ...
  switch (common_msg_ptr->mac_l1_freq_config.message_header.message_id)
  {
    ...
    case MAC_L1_RECOVERY_REQ:
      PUT_IMH_LEN(sizeof(mac_l1_recovery_req_t) - sizeof(IMH_T),
        &common_msg_ptr->mac_l1_recovery_req.message_header);
      break;
#endif /* FEATURE_GERAN_PTM_ERR_RECOVERY */
...
)
{
  ...
  if(arac_retran == FALSE)
  {
    ...
    **************************************************************************************/
    ...
    status = ghdi_ds_read_hw_classmark( &hw_classmark, geran_map_gas_id_to_nas_id(gas_id));
#else
                                              ...
                                              gas_id_t gas_id)
{
  ...
  global_data_ptr->grr_sig.mac_grr_dtm_channel_assign_rej.mac_status = mac_status;

  MSG_GERAN_MED_3_G("MAC Sent MAC_GRR_DTM_CHANNEL_ASSIGN_REJ reason %d dtm_ass_status %d mac_status %d",
                    global_data_ptr->grr_sig.mac_grr_dtm_channel_assign_rej.RR_cause,dtm_ass_status,mac_status);
...
/*===========================================================================
===
===  FUNCTION      gmac_get_no_serv_pend
===
===  DESCRIPTION
===
===  This function is used by other states to fetch current PS_ACCESS state,
===  if GRR triggered NO_PS_ACCESS in IDLE or TRANSFER mode.
===
===  DEPENDENCIES
===
===  RETURN VALUE
===  If GRR initiated NO_PS_ACCESS, return true, otherwise false
===
===  SIDE EFFECTS
===
===========================================================================*/
static boolean gmac_get_no_serv_pend (gas_id_t gas_id)
{
  gmac_global_data_t   *global_data_ptr = gmac_get_global_data_ptr(gas_id);
  ...
  boolean              no_serv_pend = global_data_ptr->c_flags.no_service_penidng;
...
/*===========================================================================
===
===  FUNCTION      gmac_util_send_mac_l1_recovery_req
===
===  DESCRIPTION
===
===  These changes are for adding support for new error recovery mechanism
===  between GMAC and GL1 when there is error scenrio, out of sync or GL1
===  stuck conditions happen. GL1 will indicate GMAC with L1_MAC_RECOVERY_IND 
==   to start recovery and GMAC responds with MAC_L1_RECOVERY_REQ
===
===  DEPENDENCIES
===
===  RETURN VALUE
...
===
===========================================================================*/
void gmac_util_send_mac_l1_recovery_req (gas_id_t gas_id)
{
  mac_l1_sig_t   send_mac_l1_sig;

  MSG_GERAN_MED_0_G("MAC Sent MAC_L1_RECOVERY_REQ");
  send_mac_l1_sig.mac_l1_recovery_req.message_header.message_id  = (uint8)MAC_L1_RECOVERY_REQ;
  send_mac_l1_sig.mac_l1_recovery_req.message_header.message_set = MS_MAC_L1;

  gprs_l1_mac_acc_func(&send_mac_l1_sig, gas_id);
} /* gmac_util_send_mac_l1_recovery_req */

/*===========================================================================
===
===  FUNCTION      gmac_util_handle_error_recovery
===
===  DESCRIPTION
===
===  These changes are for adding support for new error recovery mechanism
===  between GMAC and GL1 when there is error scenrio, out of sync or GL1
===  stuck conditions happen. 
===    case 1: GL1 will indicate GMAC with L1_MAC_RECOVERY_IND 
===    to start recovery and GMAC responds with MAC_L1_RECOVERY_REQ
===    case 2: GMAC detects some mismatch, it reset the state variables,
===    prepare for recovery and send MAC_L1_RECOVERY_REQ to trigger recovery at GL1
===
===  DEPENDENCIES
===
===  RETURN VALUE
...
===
===========================================================================*/
void gmac_util_handle_error_recovery (gas_id_t gas_id)
{
  gmac_global_data_t *global_data_ptr = gmac_get_global_data_ptr(gas_id);
  gmac_timers_t      *mac_timers_ptr  = gmac_get_timers_ptr(gas_id);

  /* We don't inform GRR yet of GMAC mode change */
  if (TRUE == gmac_set_mac_null_substate(NULL_RECOVERY, gas_id))
  {
    boolean b_no_serv_pend = gmac_get_no_serv_pend(gas_id);
  
    gmac_transfer_set_transfer_state(TRANSFER_INACTIVE, gas_id);
  
    /* Clear GRLC UL and DL States */
    gmac_util_send_dl_release_to_rlc(gas_id);
    gmac_util_block_rlc(NO_DEL, GMAC_TASK, __LINE__, gas_id);
    ...
    /* Restore NO_PS_ACCESS status after init_common_flags */
    global_data_ptr->c_flags.no_service_penidng = b_no_serv_pend;
  
    /* Handle any pending actions and timers */
    if (global_data_ptr->gmac_local_pccf_params.pccf_pending == TRUE)
    {
      gmac_util_complete_pccf_process(FAILED_TO_SEND_PCCF, MAC_INITIATED, GMAC_TASK, __LINE__, gas_id);
    }
    ...
    gmac_util_send_mac_l1_recovery_req(gas_id);
  }
  else
  {
    MSG_GERAN_ERROR_0_G("GMAC couldn't Enter NULL_RECOVERY");
  }
} /* gmac_util_handle_error_recovery */
#endif /* FEATURE_GERAN_PTM_ERR_RECOVERY */
