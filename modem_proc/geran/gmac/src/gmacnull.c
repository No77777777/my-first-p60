...
***
*** $Header: //components/rel/geran.mpss/7.4.0/gmac/src/gmacnull.c#2 $
***
...
/*===========================================================================
===
===  FUNCTION    gmac_set_mac_null_substate()
===
===  DESCRIPTION
===  Set MAC NULL substate from other MAC states
===
===  DEPENDENCIES
===
===
===  RETURN VALUE
===  TRUE if substate change success, false if already same substate was set
===
===  SIDE EFFECTS
===
===========================================================================*/
boolean gmac_set_mac_null_substate(gprs_mac_null_substates_t substate, gas_id_t gas_id)
{
  ...
  {
    case NULL_RESET:
      if (null_data_ptr->mac_null_substate != NULL_RESET)
      {
        MSG_GERAN_MED_0_G("GMAC State Enter NULL_RESET");
...
#ifdef FEATURE_GERAN_PTM_ERR_RECOVERY
    case NULL_RECOVERY:
      if (null_data_ptr->mac_null_substate != NULL_RECOVERY)
      {
        if (null_data_ptr->mac_null_substate != NULL_RESET)
        {
          MSG_GERAN_MED_0_G("GMAC State Enter NULL_RECOVERY");
          null_data_ptr->mac_null_substate = NULL_RECOVERY;
          state_change = TRUE;
        }
        else
        {
          MSG_GERAN_HIGH_0_G("GMAC ignored RECOVERY in NULL_RESET state");
        }
      }
      break;
#endif /* FEATURE_GERAN_PTM_ERR_RECOVERY */
    ...
    default:
    /* Ignore, implement new states when required in future */
      break;
  }
  return state_change;
}
                                    ...
                                    gas_id_t gas_id)
{
    ...
    gmac_send_mac_grr_dtm_channel_assign_rej(dtm_ass_rej,MAC_IDLE,gas_id);
    gmac_util_send_grr_mac_status(MAC_IDLE,gas_id);
}
...
void null_mode_handler(uint8 task_id, uint8 signal_id, gas_id_t gas_id)
{
  ...
  /* In NULL state GMAC does not expect below messages from GL1:
   * MAC_PH_DATA_IND
   * L1_MAC_USF_DETECTED
   * 
   * To trap cases where GL1 is out-of-sync with GMAC, below ERR_FATAL has been added.
   * Asserting early here to help GL1 to debug their issues. (CR#1027907)
  if ((task_id == MS_MAC_L1) &&
      ((signal_id == MAC_PH_DATA_IND) || (signal_id == L1_MAC_USF_DETECTED)))
  {
    if (geran_get_nv_recovery_restart_enabled(gas_id))
    {
      MSG_GERAN_ERROR_1_G("Unexpected message from GL1: %d", signal_id);
      MSG_GERAN_ERROR_0_G("GMAC initiated Recovery Restart");
      geran_initiate_recovery_restart(GERAN_CLIENT_GMAC, gas_id);
    }
    else
    {
      ERR_GERAN_FATAL_1_G("Unexpected message from GL1: %d", signal_id);
    }
  } */
  ...
  switch(null_data_ptr->mac_null_substate)
  {
      ...
      else if (task_id == MS_MAC_MAC)
      {
        switch (signal_id)
        {
          ...
          case MAC_TIMER_T3172:
            ...
            {
              MSG_GERAN_HIGH_0_G("MAC Ignored MAC_TIMER_T3172, NO_SERVICE processing pending");
            }
            break;
          ...
          case L1_MAC_TBF_REL_CONFIRM:
            ...
            else
            {
              ...
              else
              {
                ...
                {
                  MSG_GERAN_ERROR_1_G("MAC error recovery TBF REL CNF %d", global_data_ptr->msg_ptr->l1_mac_sig.msg.tbf_released.tbf_released);
            ...
            if (null_data_ptr->gmac_null_releasing_l1 == FALSE)
            {
              ...
              else
              {
                ...
                else
                {
                  ...
                  if (TRUE == geran_nv_ptm_err_recovery())
                  {
                    MSG_GERAN_ERROR_1_G("MAC recovery info Unexpected connection type %d ", global_data_ptr->msg_ptr->l1_mac_sig.msg.connect_ind.tbf_type_ind);
                    gmac_util_handle_error_recovery(gas_id);
                  }
                  else
#endif /* FEATURE_GERAN_PTM_ERR_RECOVERY */
        ...
        switch (signal_id)
        {
          ...
          case RM_UL_RELEASE_IND:
            ...
            if (global_data_ptr->msg_ptr->rlc_mac_sig.msg.ul_release_ind.cause == REL_UL_ABNORMAL_WITH_ACCESS_RETRY)
            {
              gmac_util_block_rlc(NO_DEL, GMAC_NULL, __LINE__, gas_id);
              null_data_ptr->gmac_null_tbf_rel_cause = GMAC_NULL_TBF_REL_NORMAL;
              gmac_null_send_release_to_l1(GMAC_NULL_TBF_REL_NORMAL, UL_AND_DL_TBF_ABNORMAL, gas_id);
              mac_log_ul_tbf_release(MAC_LOG_UL_RELEASE_IND, gas_id);
            }
            else
            {
              MSG_GERAN_ERROR_0_G("MAC Rcvd RM_UL_RELEASE_IND with wrong release cause");
            }
            break;
    ...
    ************************************/
    ...
    break;
      ...
      if (task_id == MS_MAC_L1)
      {
        if (signal_id == L1_MAC_RECOVERY_CNF)
        {
          MSG_GERAN_MED_0_G("MAC ignored GMAC L1_MAC_RECOVERY_CNF in NULL_RESET");
        }
      }
#endif /* FEATURE_GERAN_PTM_ERR_RECOVERY */
      break;
    ...
    ***************************************/
    case NULL_RECOVERY:
      ...
      if (task_id == MS_MAC_L1)
      {
        switch (signal_id)
        {
          case L1_MAC_RECOVERY_CNF:
            gmac_util_send_grr_mac_status(MAC_IDLE, gas_id);

            MSG_GERAN_MED_0_G("GMAC State null NULL_INACTIVE [L1_MAC_RECOVERY_CNF]");
            null_data_ptr->mac_null_substate = NULL_INACTIVE;
            (void)mac_log_state(gas_id);

            /* RECOVERY is done. Trigger PS_ACCESS for RLC to request TBF if anything is there in Queue
             * If GRR triggered NO_PS_ACCESS is pending or T3172 is running, GMAC shouldn't start PS_ACCESS
             */
            if (global_data_ptr->c_flags.no_service_penidng == FALSE &&
          ...
          default:
            /* unknown signal from L1 */
            MSG_GERAN_HIGH_1_G("MAC unknown signal %d from L1 [NULL_RECOVERY]",signal_id);
            break;
        }
      } /* end of MAC_L1 signals */
      else if (task_id == MS_MAC_RLC)
      {
        /* unknown internal signal from RLC */
        MSG_GERAN_HIGH_1_G("MAC unknown signal %d from RLC [NULL_RECOVERY]",signal_id);
      } /* end of RLC_MAC signals */
      else if(task_id == MS_MAC_RR)
      {
        switch (signal_id)
        {
          case GRR_MAC_SEND_MEAS_RPT:
            MSG_GERAN_MED_0_G("GMAC Rcvd GRR_MAC_SEND_MEAS_RPT in NULL_RECOVERY");
            gmac_util_send_pmr_cnf_to_grr(MAC_GRR_FAILURE, gas_id);
            break;

          case GRR_MAC_SEND_PCCF:
            MSG_GERAN_HIGH_0_G("GMAC Rcvd GRR_MAC_SEND_PCCF in NULL_RECOVERY");
            gmac_util_complete_pccf_process(FAILED_TO_SEND_PCCF, MAC_INITIATED, GMAC_NULL, __LINE__, gas_id);
            break;

          case GRR_MAC_RACH_REQ:
            MSG_GERAN_MED_0_G("MAC Rcvd GRR_MAC_RACH_REQ in NULL_RECOVERY");
            gmac_util_send_grr_mac_rach_rej(gas_id);
            break;
          ...
          case GRR_MAC_PS_ACCESS_REQ:
            MSG_GERAN_ERROR_0_G("MAC Rcvd GRR_MAC_RACH_REQ in NULL_RECOVERY");
            global_data_ptr->c_flags.no_service_penidng = FALSE;
            break;

#ifdef FEATURE_GSM_DTM
          case GRR_MAC_DTM_CHANNEL_ASSIGN_REQ:
            gmac_send_mac_grr_dtm_channel_assign_rej(DTM_ASS_REJ_PROTOCOL_ERR, MAC_IDLE, gas_id);
            break;
#endif /* FEATURE_GSM_DTM */
          ...
          default:
            /* unknown signal from GRR */
            MSG_GERAN_HIGH_1_G("MAC unknown signal %d from GRR [NULL_RECOVERY]",signal_id);
            break;
        }
      } /* end of GRR_MAC signals */
      else if(task_id == MS_MAC_MAC)
      {
        switch (signal_id)
        {
          ...
          case MAC_TIMER_T3200_8:
            MSG_GERAN_MED_1_G("MAC timer t3200 expired sig_id %d", signal_id);
          ...
          default:
            MSG_GERAN_HIGH_1_G("MAC unknown signal %d [NULL_RECOVERY]",signal_id);
            break;
        }
      }/* end of GMAC_MAC signals */
      break;
#endif /* FEATURE_GERAN_PTM_ERR_RECOVERY */

    default:
      /* unknown NULL substate */
      MSG_GERAN_HIGH_0_G("MAC unknown NULL Substate");
      break;
  }
}
...
/*===========================================================================
===
===  FUNCTION      gmac_null_get_tbfs_active()
===
===  DESCRIPTION
===   This function can be called externally to gmac null state in
===   order to determine which TBFs are active.
===
===  DEPENDENCIES
===
===  RETURN VALUE
===    active_tbf_t enum values
===
===  SIDE EFFECTS
===  NONE
===
===========================================================================*/
active_tbf_t gmac_null_get_tbfs_active(gas_id_t gas_id)
{
  active_tbf_t     active_tbf     = NO_TBF;
  gmac_null_data_t *null_data_ptr = gmac_get_null_data_ptr(gas_id);
...
#ifdef FEATURE_QSH_MDUMP
QSH_MDUMP_FN_ATTR void gmac_qsh_mdump_collect_null_data(boolean actv_sub, gas_id_t gas_id)
{
  gmac_null_data_t *null_data_ptr = gmac_get_null_data_ptr(gas_id);

  if (actv_sub == TRUE)
  {
    qsh_mdump_collect_high(null_data_ptr, sizeof(gmac_null_data_t));
  }
  else
  {
    qsh_mdump_collect_high(&(null_data_ptr->mac_null_substate), sizeof(gprs_mac_null_substates_t));
  }
}
#endif /* FEATURE_QSH_MDUMP */
