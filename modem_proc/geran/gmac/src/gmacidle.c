...
***
*** $Header: //components/rel/geran.mpss/7.4.0/gmac/src/gmacidle.c#3 $
***
         ...
         else if(task_id == MS_MAC_MAC)
         {
           ...
           switch (signal_id)
           {
               case MAC_TIMER_T3168:
                    ...
                    MSG_GERAN_MED_0_G("MAC info timer t3168 expired during 2p cont res");
           ...
           switch (signal_id)
           {
               ...
               case MAC_TIMER_T3180:
                     ...
                     **********************************************************************/
                     ...
                     {
                       MSG_GERAN_MED_0_G("MAC Rcvd MAC_TIMER_T3166 during during 1p cont res");
                     }
   ...
   switch (task_id)
   {
     case MS_MAC_L1 :
           switch (sig_id)
           {
             ...
             case L1_MAC_SINGLE_BLOCK_SENT:
               MSG_GERAN_MED_3_G( "MAC Rcvd L1_MAC_SINGLE_BLOCK_SENT, ln %d, m_state %d, m_act %d", line_no, mac_state, current_action );
             break;

             case L1_MAC_RRBP_FAILED_IND:
               MSG_GERAN_MED_3_G( "MAC Rcvd L1_MAC_RRBP_FAILED_IND, ln %d, m_state %d, m_act %d", line_no, mac_state, current_action );
             break;

#ifdef FEATURE_GERAN_PTM_ERR_RECOVERY
             case L1_MAC_RECOVERY_IND:
                  MSG_GERAN_MED_3_G( "MAC Rcvd L1_MAC_RECOVERY_IND, ln %d, m_state %d, m_act %d",line_no,mac_state ,current_action );
             break;

             case L1_MAC_RECOVERY_CNF:
                  MSG_GERAN_MED_3_G( "MAC Rcvd L1_MAC_RECOVERY_CNF, ln %d, m_state %d, m_act %d",line_no,mac_state ,current_action );
             break;
...
/*===========================================================================
===
===  FUNCTION      gmac_idle_get_tbfs_active
===
===  DESCRIPTION
===   This function can be called externally to gmac idle state in
...
===
===========================================================================*/
active_tbf_t gmac_idle_get_tbfs_active(gas_id_t gas_id)
{
...
#ifdef FEATURE_QSH_MDUMP
QSH_MDUMP_FN_ATTR void gmac_qsh_mdump_collect_idle_data(boolean actv_sub, gas_id_t gas_id)
{
  gmac_idle_data_t *idle_data_ptr = gmac_get_idle_data_ptr(gas_id);

  if (actv_sub == TRUE)
  {
    qsh_mdump_collect_high(idle_data_ptr, sizeof(gmac_idle_data_t));
  }
  else
  {
    qsh_mdump_collect_high(&(idle_data_ptr->mac_idle_substate), sizeof(gprs_mac_idle_substates_t));
    qsh_mdump_collect_high(&(idle_data_ptr->release_cause), sizeof(release_cause_t));
    qsh_mdump_collect_high(&(idle_data_ptr->contention_result), sizeof(cont_res_success_t));
    qsh_mdump_collect_high(&(idle_data_ptr->tbf_rel), sizeof(release_tbf_t));
    qsh_mdump_collect_high(&(idle_data_ptr->idle_realloc_attempts_count), sizeof(uint8));
  }
}
#endif /* FEATURE_QSH_MDUMP */
