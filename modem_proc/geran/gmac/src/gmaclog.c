...
$Header: //components/rel/geran.mpss/7.4.0/gmac/src/gmaclog.c#1 $
...
boolean mac_log_state(gas_id_t gas_id)
{
  ...
  else
  {
    ...
    log_ptr->mac_transfer_fsm_state = (byte) gmac_transfer_get_transfer_state(gas_id);
                                   ...
                                   gas_id_t    gas_id)
{
  ...
  else
  {
    /* Log an event if we have a payload */
      ...
      payload[1] = 0;                         /* Protocol discriminator */
      payload[2] = channel_type;              /* Channel type */

      GERAN_EVENT_REPORT_PAYLOAD( (((channel_type & 0x80) != 0)
...
{
  GERAN_EVENT_REPORT(EVENT_GPRS_MAC_CAMPED_ON_CELL);
...
void mac_log_grr_events( uint8 signal_id, gas_id_t gas_id )
{
  ...
  switch ( signal_id )
  {
    case GRR_MAC_SEND_PCCF:
      ...
      {
        GERAN_EVENT_REPORT_PAYLOAD(EVENT_GPRS_CELL_CHANGE_FAILURE, 1, &global_data_ptr->msg_ptr->grr_mac_sig.msg.gm_send_pccf.pccf.cause);
      }
      else
      {
...
void mac_log_gprs_pua_event( uint8 ts_alloc, uint8 cs, gas_id_t gas_id )
{
  ...
  pua_event[0] = ts_alloc;
  pua_event[1] = cs;

  GERAN_EVENT_REPORT_PAYLOAD( EVENT_GPRS_PACKET_UPLINK_ASSIGNMENT, 2, pua_event );
}
...
void mac_log_gprs_pda_event( uint8 ts_alloc, uint8 tfi, gas_id_t gas_id )
{
  ...
  pda_event[0] = ts_alloc;
  pda_event[1] = tfi;

  GERAN_EVENT_REPORT_PAYLOAD( EVENT_GPRS_PACKET_DOWNLINK_ASSIGNMENT, 2, pda_event );
}
...
{
  GERAN_EVENT_REPORT_PAYLOAD( EVENT_GPRS_TBF_RELEASE, 1, &release_type );
}
...
void mac_log_timer_event(const uint32 timer_id, gas_id_t gas_id)
{
  ...
  {
    GERAN_EVENT_REPORT_PAYLOAD( EVENT_GPRS_TIMER_EXPIRY, 1, &ev_rpt_timer_exp );
  }
}
...
{
  GERAN_EVENT_REPORT_PAYLOAD(EVENT_GPRS_PACKET_RESOURCE_REQUEST, 1, &access_req);
}
                       ...
                       gas_id_t gas_id)
{
  ...
  GERAN_EVENT_REPORT_PAYLOAD( EVENT_PACKET_TIMESLOT_RECONFIGURE, 4, ptr_event );
}
