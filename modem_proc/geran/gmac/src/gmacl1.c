...
***
*** $Header: //components/rel/geran.mpss/7.4.0/gmac/src/gmacl1.c#2 $
***
...
===  DESCRIPTION
===
===  This API is used by other modules to fetch current GMAC mode
===  and TBF status.
===
===  DEPENDENCIES
===
...
void gmac_get_tbf_status (uint8* main_state, uint8* tbf_active, gas_id_t gas_id)
{
  ...
  if(main_state == NULL || tbf_active == NULL)
  {
    return;
  }

  *main_state = global_data_ptr->mac_mode;

  switch(global_data_ptr->mac_mode)
  {
    case NULL_MODE:
      *tbf_active = gmac_null_get_tbfs_active(gas_id);
    break;

    case IDLE_MODE:
      *tbf_active = gmac_idle_get_tbfs_active(gas_id);
    break;

    case TRANSFER_MODE:
      *tbf_active = gmac_tran_get_tbfs_active(gas_id);
    break;
  }
} /* gmac_get_tbf_status */
#endif /* FEATURE_GERAN_PTM_ERR_RECOVERY */
...
void mac_put_cmd_l1_mac(l1_mac_sig_t *sig_ptr, gas_id_t gas_id)
{
  ...
  switch (sig_ptr->sig_id)
  {
    ...
    case L1_MAC_RECOVERY_IND:
      cfa_log_packet_ex
      (
        GS_QUEUE_MAC, MS_MAC_L1, (uint8)L1_MAC_RECOVERY_IND,
        0,
        NULL
      );
      break;
    ...
    case L1_MAC_RECOVERY_CNF:
      cfa_log_packet_ex
      (
        GS_QUEUE_MAC, MS_MAC_L1, (uint8)L1_MAC_RECOVERY_CNF,
        0,
        NULL
      );
      break;
