...
***
*** $Header: //components/rel/geran.mpss/7.4.0/gmac/src/gmactran_ul.c#2 $
***
  ...
  /* The value of MAC_MODE may be changed between DYNAMIC and EXTENDED DYNAMIC
    ...
    */
...
#endif
    {
      ...
      {
        /* Error in CONTROL_ACK - ignore PDA and inform network ref 11.1.4.2*/
        MSG_GERAN_HIGH_0_G("MAC info PDA Ctrl Ack 0");
        gmac_transfer_send_tbf_status(MOBILE_TBF_STATUS_PROTOCOL_ERROR_CAUSE, PACKET_DOWNLINK_ASSIGNMENT, gas_id);
        return (pda_accept_flag);
      }
    }
    else
...
boolean gmac_tdynamic_handle_ptr(gas_id_t gas_id)
{
  ...
  /*to guard against multiple PTR's rcvd on the same FN but a different TS*/
  if(transfer_data_ptr->fn_of_last_PTR == global_data_ptr->msg_ptr->l1_mac_sig.msg.data_ind.frame_no)
  {
    MSG_GERAN_MED_0_G("MAC duplicate PTR rcvd on same FN but different TS's filtered out");
    return (ptr_accept_flag);
  }

  transfer_data_ptr->fn_of_last_PTR = global_data_ptr->msg_ptr->l1_mac_sig.msg.data_ind.frame_no;
    ...
    if (gmac_util_global_tfi(ptr_ptr->global_tfi,gas_id))
    {
      ...
      */
...
#endif
      {
        ...
        {
          /* Error in CONTROL_ACK - ignore PTR and inform network ref 11.1.4.2*/
          MSG_GERAN_HIGH_0_G("MAC info PTA Ctrl Ack 0");
          gmac_transfer_send_tbf_status(MOBILE_TBF_STATUS_PROTOCOL_ERROR_CAUSE, PACKET_TIMESLOT_RECONFIGURE,gas_id);
          return (ptr_accept_flag);
        }
      }
      else
...
void gmac_tdynamic_handle_puan(boolean dl_present, gas_id_t gas_id)
{
  ...
  if (gmac_util_global_tfi(puan_tfi,gas_id) == TRUE)
  {
    ...
    {
      MSG_GERAN_ERROR_0_G("MAC info PUAN NON_DISTRIBUTION_ERROR");
      gmac_transfer_send_tbf_status(MOBILE_TBF_STATUS_NON_DIST_PART_ERR, PACKET_UPLINK_ACK_NACK, gas_id);
      return;
    }
...
void gmac_tdynamic_data_ind(gas_id_t gas_id)
{
  ...
  switch (global_data_ptr->csn_dec_hdr_ptr->message_type)
  {
    ...
    case PACKET_CS_RELEASE:
      ...
      {
        MSG_GERAN_MED_1_G( "GMAC State tran = %d",tran_state_change);
      }
