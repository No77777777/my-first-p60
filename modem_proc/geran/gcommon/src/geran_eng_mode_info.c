...
$Header: //components/rel/geran.mpss/7.4.0/gcommon/src/geran_eng_mode_info.c#2 $
...
static boolean geran_eng_tx_pwr_diff(int16* a_ptr, int16* b_ptr , uint8* highPwr, const uint8 num_slots);
                               ...
                               gas_id_t gas_id)
{
  ...
  switch(geran_eng_mode_info_type)
  {
    ...
    case ENG_MODE_SCELL_INFO:
      ...
      if (eng_mode_data_ptr->geran_eng_mode_info.scell_info.cell_id != GERAN_ENG_MODE_INVALID_VALUE_OF_PARAMETER)
      {
        eng_mode_data_ptr->geran_eng_mode_info.scell_info_persist = eng_mode_data_ptr->geran_eng_mode_info.scell_info;
        eng_mode_data_ptr->geran_eng_mode_info.scell_info_persist_valid_flag = TRUE;
      }
      break;

    case ENG_MODE_SCELL_NCELL_INFO:
    {
      if(valid_flag)
      {
        eng_mode_scell_ncell_info_t sncell_arfcn = *(eng_mode_scell_ncell_info_t*)param_copy;

        /* Update SCELL info parameters if changed */
        if (eng_mode_data_ptr->geran_eng_mode_info.all_freq_info.scell_arfcn.num != (*(eng_mode_scell_ncell_info_t*)param_copy).scell_arfcn.num ||
            eng_mode_data_ptr->geran_eng_mode_info.all_freq_info.scell_arfcn.band != (*(eng_mode_scell_ncell_info_t*)param_copy).scell_arfcn.band)
        {
          eng_mode_data_ptr->geran_eng_mode_info.all_freq_info.all_freq_info_valid_flag = TRUE;
          eng_mode_data_ptr->geran_eng_mode_info.all_freq_info.scell_arfcn.num = sncell_arfcn.scell_arfcn.num;
          eng_mode_data_ptr->geran_eng_mode_info.all_freq_info.scell_arfcn.band = sncell_arfcn.scell_arfcn.band;
          data_changed = TRUE;
        }
        
        /* Update NCELL info parameters */
        if(memcmp(&eng_mode_data_ptr->geran_eng_mode_info.all_freq_info.ncell_freq_list,
                   &sncell_arfcn.ncell_freq_list,
                   sizeof(eng_mode_ncell_freq_list_T)) != 0)
        {
          eng_mode_data_ptr->geran_eng_mode_info.all_freq_info.all_freq_info_valid_flag = TRUE;
          memscpy(&(eng_mode_data_ptr->geran_eng_mode_info.all_freq_info.ncell_freq_list),
                 sizeof(eng_mode_ncell_freq_list_T),
                 (eng_mode_ncell_freq_list_T*)&sncell_arfcn.ncell_freq_list,
                 sizeof(eng_mode_ncell_freq_list_T));
    
          data_changed = TRUE;
        }

        if (eng_mode_data_ptr->geran_eng_mode_info.all_freq_info.hopping_status.hopping_status == TRUE)
        {
          /* Since the cell has changed, reset eng mode hopping_status & hopping_freq_list info here */
          memset(&eng_mode_data_ptr->geran_eng_mode_info.all_freq_info.hopping_status,
           (uint8)GERAN_ENG_MODE_INVALID_VALUE_OF_PARAMETER,
           sizeof(eng_mode_hopping_status_t));
          memset(&(eng_mode_data_ptr->geran_eng_mode_info.all_freq_info.hopping_freq_list),
        ...
        if (eng_mode_data_ptr->geran_eng_mode_info.all_freq_info.dedicated_or_pdch_freq.num != (uint16)GERAN_ENG_MODE_INVALID_VALUE_OF_PARAMETER)
        {
          memset(&eng_mode_data_ptr->geran_eng_mode_info.all_freq_info.dedicated_or_pdch_freq,
           GERAN_ENG_MODE_INVALID_VALUE_OF_PARAMETER,
           sizeof(ENG_MODE_ARFCN_T));
          data_changed = TRUE;
        }
      }
      else
      {
        /* SCELL and NCELL data became invalid, reset all_freq_info */
        if (eng_mode_data_ptr->geran_eng_mode_info.all_freq_info.all_freq_info_valid_flag == TRUE)
        {
          data_changed = TRUE;
        }

        memset(&eng_mode_data_ptr->geran_eng_mode_info.all_freq_info,
         GERAN_ENG_MODE_INVALID_VALUE_OF_PARAMETER,
         sizeof(eng_mode_all_frequency_list_T));
    ...
    case ENG_MODE_HOPPING_FREQUENCY_LIST:
    {
      ...
      if(valid_flag)
      {
        ...
        if(memcmp(&eng_mode_data_ptr->geran_eng_mode_info.hopping_freq_list, param_copy, sizeof(eng_mode_frequency_list_T)) != 0  ||
           memcmp(&eng_mode_data_ptr->geran_eng_mode_info.all_freq_info.hopping_freq_list, param_copy, sizeof(eng_mode_frequency_list_T)) != 0)
        {
          ...
          memscpy(&(eng_mode_data_ptr->geran_eng_mode_info.all_freq_info.hopping_freq_list),
                   sizeof(eng_mode_frequency_list_T),
                   (eng_mode_frequency_list_T*)param_copy,
                   sizeof(eng_mode_frequency_list_T));

         eng_mode_data_ptr->geran_eng_mode_info.all_freq_info.hopping_status.hopping_status = TRUE;
    ...
    case ENG_MODE_HOPPING_STATUS:
    {
      ...
      if (valid_flag != eng_mode_data_ptr->geran_eng_mode_info.hopping_status_valid_flag)
      {
        ...
        if (valid_flag)
        {
          ...
          eng_mode_data_ptr->geran_eng_mode_info.all_freq_info.hopping_status.hopping_status = eng_mode_data_ptr->geran_eng_mode_info.hopping_status.hopping_status;
    ...
    case ENG_MODE_DED_OR_PDCH_ARFCN:
    {
      if (valid_flag)
      {
        ENG_MODE_ARFCN_T* arfcn = (ENG_MODE_ARFCN_T*)param_copy;

        if(eng_mode_data_ptr->geran_eng_mode_info.all_freq_info.dedicated_or_pdch_freq.num != arfcn->num ||
           eng_mode_data_ptr->geran_eng_mode_info.all_freq_info.dedicated_or_pdch_freq.band != arfcn->band)
        {
          eng_mode_data_ptr->geran_eng_mode_info.all_freq_info.dedicated_or_pdch_freq = (*(ENG_MODE_ARFCN_T*)param_copy);
          eng_mode_data_ptr->geran_eng_mode_info.all_freq_info.all_freq_info_valid_flag = TRUE;

          /* Since the hopping status has changed, reset eng mode hopping_status & hopping_freq_list info here */
          memset(&eng_mode_data_ptr->geran_eng_mode_info.all_freq_info.hopping_status,
           (uint8)GERAN_ENG_MODE_INVALID_VALUE_OF_PARAMETER,
           sizeof(eng_mode_hopping_status_t));
          memset(&(eng_mode_data_ptr->geran_eng_mode_info.all_freq_info.hopping_freq_list),
    ...
    case ENG_MODE_SCELL_INFO_PERSISTENT:
    {
      if (valid_flag == TRUE)
      {
        eng_mode_data_ptr->geran_eng_mode_info.scell_info_persist = *(eng_mode_scell_info_t*)param_copy;
        eng_mode_data_ptr->geran_eng_mode_info.scell_info_persist_valid_flag = TRUE;

        MSG_GERAN_MED_3_G("Eng mode CI:%d Scell Info LAC %d, PLMN %6x",
                          eng_mode_data_ptr->geran_eng_mode_info.scell_info_persist.cell_id,
                          eng_mode_data_ptr->geran_eng_mode_info.scell_info_persist.lac,
                          ( (eng_mode_data_ptr->geran_eng_mode_info.scell_info_persist.plmn.plmn_id[0] << 16) + \
                          (  eng_mode_data_ptr->geran_eng_mode_info.scell_info_persist.plmn.plmn_id[1] << 8)  + \
                             eng_mode_data_ptr->geran_eng_mode_info.scell_info_persist.plmn.plmn_id[2]));
      }
      break;
    }
    ...
    case ENG_MODE_TX_PWR_DED:
      if(valid_flag)
      {
        uint8  TxLevHi;
        uint8  num_slots;

        /* number of slots being diff'd (and then memscpy'd) depends on the info type being supplied */
        num_slots = (ENG_MODE_TX_PWR_DED == geran_eng_mode_info_type) ? ENG_MODE_TX_PWR_NUM_TIMESLOTS_DED : ENG_MODE_TX_PWR_NUM_TIMESLOTS;

        if(geran_eng_tx_pwr_diff(eng_mode_data_ptr->geran_eng_mode_info.tx_pwr_in_use,(int16*)param_copy, &TxLevHi, num_slots) == TRUE)
        {
          memscpy(eng_mode_data_ptr->geran_eng_mode_info.tx_pwr_in_use,
                  sizeof(eng_mode_data_ptr->geran_eng_mode_info.tx_pwr_in_use),
                  (int16 *)param_copy,
                  num_slots * sizeof(int16));
          data_changed = TRUE;
        }
        /*
    ...
    case ENG_MODE_CODEC_HO_INFO:
    {
      ...
      if (SYS_SPEECH_CODEC_INFO == codec_info_ptr->cmd)
      {
        eng_mode_data_ptr->geran_eng_mode_info.ho_codec = *(sys_codec_ho_info *)param_copy;
       }
       else
       { /* Other commands are related to HO, just set the HO Type  */
         eng_mode_data_ptr->geran_eng_mode_info.ho_codec.cmd = codec_info_ptr->cmd;
         eng_mode_data_ptr->geran_eng_mode_info.ho_codec.ho_type = codec_info_ptr->ho_type ;
       }
                              ...
                              sys_modem_as_id_e_type asubs_id)
{
  ...
  switch(geran_eng_mode_info_type)
  {
    ...
    case ENG_MODE_ALL_FREQ_INFO:
      *((eng_mode_all_frequency_list_T*) param_copy) = eng_mode_data_ptr->geran_eng_mode_info.all_freq_info;
    break;
    ...
    */
    ...
    case ENG_MODE_SCELL_INFO_PERSISTENT:
      *((eng_mode_scell_info_t*) param_copy) = eng_mode_data_ptr->geran_eng_mode_info.scell_info_persist;
    break;
                                            ...
                                            gas_id_t gas_id)
{
  ...
  switch (geran_eng_mode_info_type)
  {
    ...
    /* Send data related to ENG_MODE_ALL_FREQ_INFO */
    case ENG_MODE_SCELL_NCELL_INFO:
    case ENG_MODE_HOPPING_FREQUENCY_LIST:
    case ENG_MODE_DED_OR_PDCH_ARFCN:
    {
      all_freq_info_cb_fnc_ptr all_freq_info_fnc_ptr = NULL;

      for (i = 0; i < MAX_NUM_CALLBACK_REGISTRATIONS; i++)
      {
        if (eng_mode_data_ptr->eng_mode_cb_funcs[ENG_MODE_ALL_FREQ_INFO][i].func_ptr != NULL)
        {
          all_freq_info_fnc_ptr = (all_freq_info_cb_fnc_ptr)(eng_mode_data_ptr->eng_mode_cb_funcs[ENG_MODE_ALL_FREQ_INFO][i].func_ptr);
          all_freq_info_fnc_ptr(eng_mode_data_ptr->geran_eng_mode_info.all_freq_info);

          if (geran_eng_mode_logging_enabled)
          {
            MSG_GERAN_MED_1_G("Eng Mode trigger all_freq_info call back 0x%x",all_freq_info_fnc_ptr);
          }
        }
      }
    }
...
static void geran_eng_initialise_store(sys_modem_as_id_e_type as_id)
{
  ...
  /*ENG_MODE_SCELL_INFO_PERSISTENT*/
  /*Initialise only for the first time, for all other case keep the info*/
  if (FALSE == eng_mode_data_ptr->store_initialised)
  {
    memset(&eng_mode_data_ptr->geran_eng_mode_info.scell_info_persist,
           (uint8)GERAN_ENG_MODE_INVALID_VALUE_OF_PARAMETER,
           sizeof(eng_mode_data_ptr->geran_eng_mode_info.scell_info_persist));
  }
...
FUNCTION geran_eng_mode_eng_band_to_sys_band

DESCRIPTION

  Convert eng mode band to sys band
...
===========================================================================*/
sys_band_T geran_eng_mode_eng_band_to_sys_band(eng_mode_band_t band)
{
   switch(band)
   {
     case ENG_MODE_SCELL_BAND_DCS:
        return SYS_BAND_DCS_1800;
        break;
     case ENG_MODE_SCELL_BAND_PCS:
        return SYS_BAND_PCS_1900;
        break;
     case ENG_MODE_SCELL_BAND_GSM_850:
        return SYS_BAND_CELL_850;
        break;
     case ENG_MODE_SCELL_BAND_PGSM:
        return SYS_BAND_PGSM_900;
        break;
     case ENG_MODE_SCELL_BAND_EGSM:
        return SYS_BAND_EGSM_900;
        break;
     default:
        MSG_ERROR("Unsupported RF band: %d", band,0,0);
