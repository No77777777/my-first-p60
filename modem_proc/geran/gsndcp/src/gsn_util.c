...
***
*** $Header: //components/rel/geran.mpss/7.4.0/gsndcp/src/gsn_util.c#1 $
***
...
*/
static uint16 gsn_get_ack_nsapis_on_sapi( gsn_sapi_e_t * );

static uint16 gsn_get_unack_nsapis_on_sapi( gsn_sapi_e_t * );

static void gsn_get_nsapis_on_sapi( gas_id_t, gsn_sapi_e_t *, uint8 * );

static void gsn_get_app_nsapis( gsn_nsapi_t *, uint16  );

static boolean gsn_decode_peer_xid( gas_id_t gas_id,
                             dsm_item_type **,
                             gsn_dec_xid_t *,
...
static void gsn_del_pcomp_e_mapped_to_alg( gas_id_t, gsn_sapi_e_t *, uint8);

static uint8 gsn_next_unassigned_pcomp_e( const gsn_sapi_e_t * );

static uint8 gsn_next_unassigned_pcomp_val( const gsn_sapi_e_t * );

static int8 gsn_create_pcomp_val( uint8 *, uint8, gsn_sapi_e_t *);

static boolean gsn_check_pcomp_e( gas_id_t, const pcomp_e_t *, gsn_sapi_e_t * );

static void gsn_reset_pcomp_ent( gas_id_t gas_id, pcomp_e_t *pcomp_e_ptr );

static pcomp_e_t *gsn_create_pcomp_e( const gsn_xid_t2_t *, gsn_sapi_e_t * );

#endif /* FEATURE_GSM_GPRS_SNDCP_PCOMP */

#ifdef FEATURE_GSM_GPRS_SNDCP_DCOMP

static void gsn_del_dcomp_e_mapped_to_alg( gas_id_t gas_id, gsn_sapi_e_t *, uint8);

static uint8 gsn_next_unassigned_dcomp_e( const gsn_sapi_e_t * );

static uint8 gsn_next_unassigned_dcomp_val( const gsn_sapi_e_t * );

static int8 gsn_create_dcomp_val( uint8 *, uint8, gsn_sapi_e_t * );

static boolean gsn_check_dcomp_e( gas_id_t, const dcomp_e_t *, gsn_sapi_e_t * );

static void gsn_reset_dcomp_ent( gas_id_t, dcomp_e_t * );

static dcomp_e_t *gsn_create_dcomp_e( const gsn_xid_t1_t *, gsn_sapi_e_t * );
...
static uint8 gsn_next_unassigned_pcomp_e( const gsn_sapi_e_t *sapi_ptr )
{
  uint8 i;
...
static uint8 gsn_next_unassigned_dcomp_e( const gsn_sapi_e_t *sapi_ptr )
{
  uint8 i;
  uint8 ent_num = NO_COMP_ENT;
...
===
===========================================================================*/
static uint8 gsn_next_unassigned_pcomp_val( const gsn_sapi_e_t *sapi_ptr )
{
  uint8 i;
...
===
===========================================================================*/
static boolean gsn_check_pcomp_e
(
  gas_id_t            gas_id,
...
)
{
  ...
  if( pcomp_e_ptr != NULL  )
  {
    ...
    {
      MSG_GERAN_HIGH_1_G("GSN "" Invalid PCI algorithm %d", pcomp_e_ptr ->t2_alg);
      comp_e_consistent = FALSE;
    }
  }
  else
...
===
===========================================================================*/
static boolean gsn_check_dcomp_e
(
  gas_id_t            gas_id,
...
)
{
  ...
  if( dcomp_e_ptr != NULL  )
  {
    ...
    {
      MSG_GERAN_HIGH_1_G("GSN "" Invalid DATA algorithm %d", dcomp_e_ptr->t1_alg);
      comp_e_consistent = FALSE;
    }
  }
  else
...
===
===========================================================================*/
static uint8 gsn_next_unassigned_dcomp_val( const gsn_sapi_e_t *sapi_ptr )
{
  uint8 i;
...
===
===========================================================================*/
static void gsn_reset_dcomp_ent( gas_id_t gas_id, dcomp_e_t *dcomp_e_ptr )
{
  if( dcomp_e_ptr->state == ASSIGNED )
  {
    gsn_V42_initialise
...
static void gsn_reset_pcomp_ent( gas_id_t gas_id, pcomp_e_t *pcomp_e_ptr )
{
  if( pcomp_e_ptr->state == ASSIGNED )
  {
    ...
    if(pcomp_e_ptr->t2_alg == T2_ALG_RFC1144)
    {
      ...
      if(result < 0)
      {
        MSG_GERAN_ERROR_1_G("GSN ""slhc reinit failed: %d",result);
      }
    }
#ifdef FEATURE_GSM_SNDCP_IPHC
    else if(pcomp_e_ptr->t2_alg == T2_ALG_RFC2507)
    {
      ...
      if(result < 0)
      {
        MSG_GERAN_ERROR_1_G("GSN ""iphc reinit failed: %d",result);
    ...
    {
      MSG_GERAN_ERROR_1_G("GSN ""Invalid t2 alg %d",pcomp_e_ptr->t2_alg);
    }
  }
}
  ...
  */
  ...
  if( xid_i->length )
  {
    if( xid_neg_len != (xid_i->length + XID_TYPE_HDR_LEN) )
      MSG_GERAN_ERROR_0_G("GSN ""XID encode. Error encoding T1 XID params");
  }
  else
  {
  ...
  if( *xid_neg != NULL )
  {
    ...
    {
      MSG_GERAN_ERROR_0_G("GSN ""XID RSP append XID block failed");
    }
  }
  else
  {
...
===
===========================================================================*/
static dcomp_e_t *gsn_create_dcomp_e( const gsn_xid_t1_t *t1, gsn_sapi_e_t *sapi_ptr )
{
  uint8       ent_num;
  t1_alg_q_t  *q_item;
...
===
===========================================================================*/
static int8 gsn_create_dcomp_val
(
...
===
===========================================================================*/
static void gsn_del_dcomp_e_mapped_to_alg( gas_id_t gas_id, gsn_sapi_e_t *sapi_ptr, uint8 t1_alg )
{
  ...
  if( sapi_ptr->is_nsapi_ptr_q_valid )
  {
      ...
      */
      ...
      while( q_item != NULL )
      {
        ...
        while( nsapi_q_item != NULL )
        {
          ...
          {
            MSG_GERAN_HIGH_1_G("GSN ""XID %d: Inval T1 algorithm",
        ...
        MSG_GERAN_MED_2_G("GSN ""XID %d: Rmv DCOMP entity %d", q_item->dcomp_e_ptr->ent_num,
                                                   0);
    ...
    else
    {
      ...
      MSG_GERAN_HIGH_1_G("GSN ""XID %d: Resp inval T1 alg queue",
                  gsn_extern_sapi[sapi_ptr->sapi]);
    }
  }
  else
  {
    MSG_GERAN_ERROR_1_G("GSN "" XID %d: NSAPI ptr queue invalid",
                    gsn_extern_sapi[sapi_ptr->sapi]);
  }
}
#endif /* FEATURE_GSM_GPRS_SNDCP_DCOMP */
...
)
{
  ...
  */
  ...
  {
    if( xid_neg_len != (xid_i->length + XID_TYPE_HDR_LEN) )
      MSG_GERAN_ERROR_0_G("GSN ""XID encode. Error encoding T2 XID params");
  }
  else
  {
    if( xid_neg_len != (APP_NSAPIS_LEN + XID_FIELD_HDR_P0_LEN ) )
...
===
===========================================================================*/
static pcomp_e_t *gsn_create_pcomp_e( const gsn_xid_t2_t *t2, gsn_sapi_e_t *sapi_ptr )
{
  uint8       ent_num;
  t2_alg_q_t  *q_item;
...
===
===========================================================================*/
static int8 gsn_create_pcomp_val
(
...
===
===========================================================================*/
static void gsn_del_pcomp_e_mapped_to_alg( gas_id_t gas_id, gsn_sapi_e_t *sapi_ptr, uint8 t2_alg )
{
  ...
  if( sapi_ptr->is_nsapi_ptr_q_valid )
  {
      ...
      while( q_item != NULL )
      {
        ...
        while( nsapi_q_item != NULL )
        {
                   ...
                   ( t2_alg == T2_ALG_RFC3095 ) )
          {
            ...
            MSG_GERAN_ERROR_1_G("GSN ""Invalid algorithm type %d",
        ...
        if( !q_cnt( &sapi_ptr->t2_alg_q[t2_alg] ) )
        {
          ...
          {
            MSG_GERAN_ERROR_1_G("GSN ""Invalid algorithm type %d",t2_alg);
          }
        }
        ...
        {
          MSG_GERAN_ERROR_1_G("GSN ""Invalid t2 algorithm type %d",t2_alg);
        }
    ...
    {
      /* At least one item should be in the queue */
      MSG_GERAN_ERROR_1_G("GSN ""XID %d: Resp inval T2 alg queue",
                  gsn_extern_sapi[sapi_ptr->sapi]);
    }
  }
  else
...
===
===========================================================================*/
static uint16 gsn_get_ack_nsapis_on_sapi( gsn_sapi_e_t *sapi_ptr )
{
...
===
===========================================================================*/
static uint16 gsn_get_unack_nsapis_on_sapi( gsn_sapi_e_t *sapi_ptr )
{
  gsn_nsapi_q_t     *item_ptr;
  uint16            nsapis = 0x0;
...
===
===========================================================================*/
static void gsn_get_nsapis_on_sapi
(
  gas_id_t      gas_id,
  gsn_sapi_e_t *sapi_ptr,  /* SAPI in use */
...
#define XID_BLK_SIZE_CHECK(curr_bit_pos, bytes_needed, xid_len)                                  \
  ...
  {                                                                                              \
    MSG_GERAN_LOW_3_G("GSN ""XID block insufficient data (msg_pos=%d, bytes_needed=%d, len=%d)", \
      curr_bit_pos, bytes_needed, xid_len);                                                      \
    return FALSE;                                                                                \
  }

static boolean gsn_decode_peer_xid
(
  gas_id_t            gas_id,
...
)
{
  ...
  if( xid_blck_len <= XID_BLOCK_LEN_MAX )
  {
    ...
    {
      MSG_GERAN_ERROR_0_G("GSN ""XID strip to buff failed");
      return ( FALSE );
    }
  }
  else
  {
    ...
    MSG_GERAN_ERROR_1_G("GSN ""XID block len %d > XID_BLOCK_LEN_MAX",
                    xid_blck_len);
  ...
  while(xid_blck_len > (msg_pos / XID_BYTE_SIZE + XID_BLK_MIN_SIZE))
  {
    ...
    MSG_GERAN_HIGH_1_G("GSN "" xid blk:    type       = %d",dxid_tmp.type);
    MSG_GERAN_HIGH_1_G("GSN "" xid length: length     = %d",dxid_tmp.length);
    ...
    if( (msg_pos / XID_BYTE_SIZE + dxid_tmp.length) > xid_blck_len )
    {
      ...
      MSG_GERAN_ERROR_2_G("GSN ""XID type len %d > XID block len %d",
    ...
    case XID_TYPE_0:
      ...
      */
      ...
      {
        MSG_GERAN_LOW_1_G("GSN ""XID T0 invalid len %d",dxid_tmp.length);
      ...
      else if( !dxid_tmp.length )
      {
        ...
        MSG_GERAN_LOW_0_G("GSN ""XID T0 len is 0");
        break;
      }
    ...
    case XID_TYPE_1:
      {
        ...
        */
          ...
          while( (t1_end_pos > msg_pos ) && (num_of_ent < ENT_NUM_MAX))
          {
            ...
            */
                ...
                (ent_end_pos > t1_end_pos ) )
            {
              ...
              */
              MSG_GERAN_HIGH_2_G("GSN ""Inval T1 Param len %d, T1 blck %d",
            ...
            if( ent_xid->hdr.p && (ent_xid->hdr.alg_type > T1_ALG_MAX) )
            {
              ...
              MSG_GERAN_HIGH_1_G("GSN ""T1 unsupported alg %d",
                ent_xid->hdr.alg_type);
            ...
            ** and the P bit. See TS04.65 table 7(2nd table).*/
            ...
            if( ( ent_xid->hdr.p  &&
                ...
                ( ent_xid->hdr.length > 6)) )
            {
              MSG_GERAN_HIGH_1_G("GSN ""Inval T1 field len %d",
               ent_xid->hdr.length);
        ...
        else
        {
          ...
          MSG_GERAN_MED_0_G("GSN "" XID T1 body not included");
    ...
    case XID_TYPE_2:
      {
        ...
        */
        if( dxid_tmp.length )
        {
          ...
          while(( t2_end_pos > msg_pos ) && ( num_of_ent < ENT_NUM_MAX ))
          {
            ...
            */
            ...
            MSG_GERAN_HIGH_1_G("GSN "" xid hdr: p-bit      = %d",ent_xid->hdr.p);
            MSG_GERAN_HIGH_1_G("GSN "" xid hdr: x1-bit     = %d",ent_xid->hdr.x1);
            MSG_GERAN_HIGH_1_G("GSN "" xid hdr: entity num = %d",ent_xid->hdr.ent_num);
            ...
            if( ent_xid->hdr.p )
            {
              ...
              */
               ...
               MSG_GERAN_HIGH_1_G("GSN "" xid hdr: x2-bit   = %d",ent_xid->hdr.x2);
               MSG_GERAN_HIGH_1_G("GSN "" xid hdr: alg type = %d",ent_xid->hdr.alg_type);
            }
#ifdef FEATURE_GSM_GPRS_SNDCP_PCOMP
            ...
            MSG_GERAN_HIGH_1_G("GSN "" xid hdr: length       = %d",ent_xid->hdr.length);
            ...
            if( ent_end_pos > t2_end_pos )
            {
              ...
              */
              MSG_GERAN_HIGH_2_G("GSN ""Inval T2 Param len %d, T2 blck %d",
            ...
            if(alg_type == T2_ALG_RFC3095)
            {
              ...
              dec_blk->t2.ent[ent_in_blck].status = XID_INVALID;
              MSG_GERAN_HIGH_0_G("GSN ""ROHC disabled");
              continue;
            }
#endif
            switch(alg_type)
            {
              case T2_ALG_RFC1144:
              {
                ...
                if(( ent_xid->hdr.p    &&
                   ...
                   ( ent_xid->hdr.length > 3)))
                {
                  MSG_GERAN_HIGH_1_G("GSN ""Inval T2 len %d", ent_xid->hdr.length);
                ...
                if( ent_xid->hdr.p && ent_xid->hdr.length )
                {
                  ...
                  {
                    MSG_GERAN_HIGH_2_G("GSN ""Inval PCOMP Value (RFC1144) %d, %d",
                      ent_xid->alg.xid_rfc1144.pcomp1,
                      ent_xid->alg.xid_rfc1144.pcomp2);
              ...
              case T2_ALG_RFC2507:
              {
                ...
                if( ent_xid->hdr.p && (ent_xid->hdr.length >= 3))
                {
                  ...
                  {
                    MSG_GERAN_HIGH_3_G("GSN ""Inval PCOMP Val (RFC2507) 1-%d, 2-%d, 3- %d",
                      ent_xid->alg.xid_rfc2507.pcomp1,
                      ent_xid->alg.xid_rfc2507.pcomp2,
                      ent_xid->alg.xid_rfc2507.pcomp3 );

                    MSG_GERAN_HIGH_2_G("GSN ""Inval PCOMP Val (RFC2507) 4-%d, 5-%d",
                      ent_xid->alg.xid_rfc2507.pcomp4,
                      ent_xid->alg.xid_rfc2507.pcomp5);
              ...
              case T2_ALG_RFC3095:
              {
                ...
                if( ent_xid->hdr.p && ent_xid->hdr.length )
                {
                  ...
                  MSG_GERAN_HIGH_3_G("GSN "" xid : pcomp1 %d, pcomp2 %d",
                                          ent_xid->alg.xid_rfc3095.pcomp1,
                                          ent_xid->alg.xid_rfc3095.pcomp2,
                                          0);
                  ...
                  {
                    MSG_GERAN_HIGH_2_G("GSN ""Inval PCOMP Value (RFC3095) %d, %d",
                      ent_xid->alg.xid_rfc3095.pcomp1,
                      ent_xid->alg.xid_rfc3095.pcomp2);
                    ...
                    ( ent_xid->hdr.length >= 2)) )
                {
                  ...
                  MSG_GERAN_HIGH_1_G("GSN "" xid : nsapis  = %d",ent_xid->alg.xid_rfc3095.nsapis);
                }
                    ...
                    ( ent_xid->hdr.length >= 4)) )
                {
                  ...
                  MSG_GERAN_HIGH_1_G("GSN "" xid : max_cid  = %d",ent_xid->alg.xid_rfc3095.max_cid);
                }
                    ...
                    ( ent_xid->hdr.length >= 6)) )
                {
                  ...
                  MSG_GERAN_HIGH_1_G("GSN "" xid : max_hdr  = %d",ent_xid->alg.xid_rfc3095.max_hdr);
                }
                    ...
                    ( ent_xid->hdr.length >= 8)) )
                {
                  ...
                  MSG_GERAN_HIGH_1_G("GSN "" xid : profile1  = %d",ent_xid->alg.xid_rfc3095.profile1);
                }
                    ...
                    ( ent_xid->hdr.length >= 10)) )
                {
                  ...
                  MSG_GERAN_HIGH_1_G("GSN "" xid : profile2  = %d",ent_xid->alg.xid_rfc3095.profile2);
                }
                    ...
                    ( ent_xid->hdr.length >= 12)) )
                {
                  ...
                  MSG_GERAN_HIGH_1_G("GSN "" xid : profile3  = %d",ent_xid->alg.xid_rfc3095.profile3);
                }
                    ...
                    ( ent_xid->hdr.length >= 14)) )
                {
                  ...
                  MSG_GERAN_HIGH_1_G("GSN "" xid : profile4  = %d",ent_xid->alg.xid_rfc3095.profile4);
                }
                    ...
                    ( ent_xid->hdr.length >= 16)) )
                {
                  ...
                  MSG_GERAN_HIGH_0_G("Unexpected profiles");
                }
              ...
              default:
              {
                ...
                MSG_GERAN_HIGH_1_G("GSN ""Unsupported T2 alg %d",
                  ent_xid->hdr.alg_type);
        ...
        else
        {
          ...
          MSG_GERAN_MED_0_G("GSN "" XID T2 body not included");
    ...
    default:
      ...
      else
      {
        ...
        MSG_GERAN_HIGH_1_G("GSN ""Invalid XID param type : %d ",dxid_tmp.type);
...
boolean gsn_decode_data_pdu_hdr( gas_id_t gas_id, uint8 *src, gsn_data_pdu_hdr_t *sn_data)
{
  ...
  if ( sn_data->t )
  {
    ...
    valid_hdr = FALSE;
    MSG_GERAN_HIGH_0_G("GSN ""DL SN-UDATA rcvd while SN-DATA expected");
  }
  ...
  if( GSN_VALID_NSAPI( sn_data->nsapi ) )
  {
        ...
        (actv_nsapis[ gas_id ][sn_data->nsapi]->sapi_ptr == NULL) )
    {
      ...
      MSG_GERAN_HIGH_1_G("GSN ""DL LL-DATA received at an invalid NSAPI%d",
  ...
  else
  {
    ...
    MSG_GERAN_HIGH_1_G("GSN ""DL SN-DATA rcvd at an invalid NSAPI %d", sn_data->nsapi);
  }
  ...
  if(( sn_data->f ) && ( valid_hdr ))
  {
    ...
    */
    ...
    if( actv_nsapis[ gas_id ][sn_data->nsapi]->dle.reas_state == DL_RCV_1ST_SEQ )
    {
      ...
      {
        valid_hdr = FALSE;
        MSG_GERAN_HIGH_1_G("GSN ""DL LL-DATA rcvd inval PCOMP %d ", sn_data->pcomp);
      }
      ...
      {
        valid_hdr = FALSE;
        MSG_GERAN_HIGH_1_G("GSN " "DL LL-DATA rcvd with unallocated PCOMP %d", sn_data->pcomp);
      }
      ...
      {
        valid_hdr = FALSE;
        MSG_GERAN_HIGH_1_G("GSN ""DL LL-DATA rcvd inval DCOMP %d",
                      sn_data->dcomp);
      }
      ...
      {
        valid_hdr = FALSE;
        MSG_GERAN_HIGH_1_G("GSN " "DL LL-UDATA rcvd with unallocated DCOMP %d", sn_data->dcomp);
      }
...
boolean gsn_decode_unitdata_pdu_hdr( gas_id_t gas_id, uint8 *src, gsn_unitdata_pdu_hdr_t *sn_udata)
{
  ...
  **  Get SN-UNITDATA PDU header fields
  ...
  if( !(sn_udata->t) )
  {
    ...
    valid_hdr = FALSE;
    MSG_GERAN_HIGH_0_G("GSN ""DL SN-DATA rcvd while SN-UDATA expected");
  }
  ...
  if( GSN_VALID_NSAPI( sn_udata->nsapi ) )
  {
        ...
        (actv_nsapis[ gas_id ][sn_udata->nsapi]->sapi_ptr == NULL) )
    {
      ...
      MSG_GERAN_HIGH_1_G("GSN ""DL SN-UDATA rcvd at an inactive NSAPI %d",
  ...
  else
  {
    ...
    MSG_GERAN_HIGH_1_G("GSN ""DL SN-UDATA rcvd at an invalid NSAPI %d", sn_udata->nsapi);
  }
  ...
  if( sn_udata->f )
  {
    ...
    */
    ...
    if( actv_nsapis[ gas_id ][sn_udata->nsapi]->dle.reas_state == DL_RCV_1ST_SEQ )
    {
      ...
      {
        valid_hdr = FALSE;
        MSG_GERAN_HIGH_1_G("GSN ""DL LL-UDATA rcvd inval PCOMP %d ",sn_udata->pcomp);
      }
      ...
      {
        valid_hdr = FALSE;
        MSG_GERAN_HIGH_1_G("GSN " "DL LL-UDATA rcvd with unallocated PCOMP %d", sn_udata->pcomp);
      }
      ...
      {
        valid_hdr = FALSE;
        MSG_GERAN_HIGH_1_G("GSN ""DL LL-UDATA rcvd inval DCOMP %d ",sn_udata->dcomp);
      }
      ...
      {
        valid_hdr = FALSE;
        MSG_GERAN_HIGH_1_G("GSN " "DL LL-UDATA rcvd with unallocated DCOMP %d", sn_udata->dcomp);
      }
            ...
            if(!sapi_ptr->pcomp_e_pool[ent_num] )
            {
              ...
              */
              ...
              else
              {
                ...
                */
                MSG_GERAN_LOW_2_G("GSN "" XID %d: Resp inval PCOMP Entity num %d",
                  gsn_extern_sapi[sapi_ptr->sapi],ent_num);
            ...
            */
            if( pcomp_e_ptr->t2_alg == T2_ALG_RFC1144 )
            {
                   ...
                   (dec_ent->xid.alg.xid_rfc1144.nsapis) )
              {
                ...
                */
                MSG_GERAN_LOW_2_G("GSN "" XID %d: Resp Inval App NSAPIs %d ",
                    gsn_extern_sapi[sapi_ptr->sapi],
                      dec_ent->xid.alg.xid_rfc1144.nsapis);
              ...
              if( dec_ent->xid.hdr.length >= (APP_NSAPIS_LEN + RFC1144_S0_1_LEN) )
              {
                if( dec_ent->xid.alg.xid_rfc1144.s0_1 > pcomp_e_ptr->alg.xid_rfc1144.s0_1 )
                {
                  ...
                  MSG_GERAN_LOW_2_G("GSN "" XID %d: Resp Inval S0_1 %d ",
              ...
              */
              if( pcomp_e_ptr->state == SELECTED )
              {
                ...
                else
                {
                  ...
                  ** entity.
                  */
                  MSG_GERAN_ERROR_1_G("GSN "" XID %d: PCOMP requested on different alg ",
                    gsn_extern_sapi[sapi_ptr->sapi]);
                  return( FALSE );
                }
              ...
              */
              for( i = 0; i < (uint8) GSN_NSAPI_MAX; i++ )
              {
                    ...
                    (0x1 << GSN_EXTERN_NSAPI(nsapi)) )
                {
                  if( actv_nsapis[ gas_id ][nsapi] != NULL )
                    ...
                    MSG_GERAN_ERROR_1_G("GSN ""XID: Invalid NSAPI %d",nsapi);
                }
              }
              ...
              {
                MSG_GERAN_ERROR_2_G("GSN "" XID: PCI comp entity %d params not consistant",
              ...
              {
                MSG_GERAN_MED_3_G("GSN ""LL %d, NS %d: Use RFC 1144 ent %d ",
                  gsn_extern_sapi[sapi_ptr->sapi],
            ...
            else if(pcomp_e_ptr->t2_alg == T2_ALG_RFC2507)
            {
                                              ...
                                              RFC2507_NTCP_SP_LEN) )
              {
                      ...
                      pcomp_e_ptr->alg.xid_rfc2507.max_per)
                {
                  ...
                  MSG_GERAN_LOW_3_G("GSN "" XID %d: Resp Inval F_MAX_PERIOD %d ",
                                  gsn_extern_sapi[sapi_ptr->sapi],
                      ...
                      pcomp_e_ptr->alg.xid_rfc2507.max_time)
                {
                  ...
                  MSG_GERAN_LOW_3_G("GSN "" XID %d: Resp Inval F_MAX_TIME %d ",
                                  gsn_extern_sapi[sapi_ptr->sapi],
                      ...
                      pcomp_e_ptr->alg.xid_rfc2507.max_hdr)
                {
                  ...
                  MSG_GERAN_LOW_3_G("GSN "" XID %d: Resp Inval F_MAX_HEADER %d ",
                                  gsn_extern_sapi[sapi_ptr->sapi],
                      ...
                      pcomp_e_ptr->alg.xid_rfc2507.tcp_sp)
                {
                  ...
                  MSG_GERAN_LOW_3_G("GSN "" XID %d: Resp Inval TCP_SPACE %d ",
                                  gsn_extern_sapi[sapi_ptr->sapi],
                ...
                {
                  /* Invalid value of TCP_SPACE. Sense of negotiation is down */
                  MSG_GERAN_LOW_3_G("GSN "" XID %d: Resp Inval NON_TCP_SPACE %d ",
                                  gsn_extern_sapi[sapi_ptr->sapi],
              ...
              */
              ...
              {
                MSG_GERAN_MED_3_G("GSN ""LL %d, NS %d: Use RFC 2507 ent %d ",
                             gsn_extern_sapi[sapi_ptr->sapi],
                             pcomp_e_ptr->alg.xid_rfc2507.nsapis,
                             pcomp_e_ptr->ent_num);
              }
            ...
            else if(pcomp_e_ptr->t2_alg == T2_ALG_RFC3095)
            {
                                              ...
                                              (RFC3095_PROF_LEN*3)))
              {
                      ...
                      pcomp_e_ptr->alg.xid_rfc3095.profile1)
                {
                  ...
                  MSG_GERAN_LOW_3_G("GSN "" XID %d: Resp Inval PROFILE1 %d ",
                                  gsn_extern_sapi[sapi_ptr->sapi],
                      ...
                      pcomp_e_ptr->alg.xid_rfc3095.profile2)
                {
                  ...
                  MSG_GERAN_LOW_3_G("GSN "" XID %d: Resp Inval PROFILE2 %d ",
                                  gsn_extern_sapi[sapi_ptr->sapi],
                      ...
                      pcomp_e_ptr->alg.xid_rfc3095.profile3)
                {
                  ...
                  MSG_GERAN_LOW_3_G("GSN "" XID %d: Resp Inval PROFILE3 %d ",
                                  gsn_extern_sapi[sapi_ptr->sapi],
              ...
              */
              ...
              {
                MSG_GERAN_MED_3_G("GSN ""LL %d, NS %d: Use RFC 3095 ent %d ",
                             gsn_extern_sapi[sapi_ptr->sapi],
                             pcomp_e_ptr->alg.xid_rfc3095.nsapis,
                             pcomp_e_ptr->ent_num);
              }
            ...
            {
              MSG_GERAN_ERROR_0_G("GSN ""Invalid T2 alg on selected comp entity ");
            }
          }
#endif /*  FEATURE_GSM_GPRS_SNDCP_PCOMP */
...
/* Find any SELECTED entities that have been implicitly accepted by
        ...
        if( q_item->pcomp_e_ptr->state == SELECTED )
        {
              ...
              (sapi_ptr->pcomp_pool[q_item->pcomp_e_ptr->alg.xid_rfc1144.pcomp2] != NULL) )
          {
            ...
            else
            {
              ...
              */
              MSG_GERAN_ERROR_1_G("GSN "" XID %d: PCOMP values are't SELECTED ",
                gsn_extern_sapi[sapi_ptr->sapi]);
              return( FALSE );
            }
          ...
          {
            MSG_GERAN_ERROR_0_G("GSN ""Invalid algorithm type");
          }
  ...
  */
  if( xid_req )
  {
    ...
    {
      MSG_GERAN_ERROR_1_G("GSN ""LL %d: Append XID block failed",
        gsn_extern_sapi[sapi_ptr->sapi]);
    }
  }
  ...
  if( dec_xid.t1.status !=  XID_ABSENT )
  {
    ...
    {
      MSG_GERAN_LOW_2_G("GSN "" XID %d: inval DCOMP XID ent %d",
          ...
          */
...
#else
          {
            ...
            if(!sapi_ptr->dcomp_e_pool[ent_num] )
            {
              ...
              */
              ...
              else
              {
                /* Invalid Comp Entity number. Compression entity does not exist.
                */
                MSG_GERAN_LOW_2_G("GSN "" XID %d: Resp inval DCOMP Entity num %d",
                  gsn_extern_sapi[sapi_ptr->sapi],ent_num);
                return( FALSE );
              }
            }
            ...
            */
            if( dcomp_e_ptr->t1_alg == T1_ALG_V42BIS )
            {
                    ...
                    (APP_NSAPIS_LEN + V42BIS_P0_LEN) )
              {
                if( dec_ent->xid.alg.xid_v42bis.p0 > dcomp_e_ptr->alg.xid_v42bis.p0 )
                {
                  ...
                  MSG_GERAN_LOW_2_G("GSN "" XID %d: Resp Inval P0 %d ",
                  ...
                  (APP_NSAPIS_LEN + V42BIS_P0_LEN + V42BIS_P1_LEN) )
              {
                        ...
                        dcomp_e_ptr->alg.xid_v42bis.p1) )
                {
                  ...
                  MSG_GERAN_LOW_2_G("GSN "" XID %d: Resp Inval P1 %d ",
                  ...
                  (V42BIS_LEN_P1_MAX - V42BIS_DCOMP_LEN) )
              {
                        ...
                        dcomp_e_ptr->alg.xid_v42bis.p2) )
                {
                  ...
                  MSG_GERAN_LOW_2_G("GSN "" XID %d: Resp Inval P2 %d ",
              ...
              */
              if( dcomp_e_ptr->state == SELECTED )
              {
                ...
                else
                {
                  ...
                  ** entity.
                  */
                  MSG_GERAN_ERROR_1_G("GSN "" XID %d: DCOMP1 requested on different alg ",
                    gsn_extern_sapi[sapi_ptr->sapi]);
                  return( FALSE );
                }
              ...
              */
              ...
              {
                MSG_GERAN_ERROR_2_G("GSN "" XID: Data comp entity %d params not consistant",
              ...
              {
                MSG_GERAN_MED_3_G("GSN ""LL %d, NS %d: Use V42bis ent %d ",
                  gsn_extern_sapi[sapi_ptr->sapi],
            ...
            {
              MSG_GERAN_ERROR_0_G("GSN ""Invalid T1 alg on selected comp entity ");
            }
          }
#endif /*  FEATURE_GSM_GPRS_SNDCP_DCOMP */
        ...
        if( q_item->dcomp_e_ptr->state == SELECTED )
        {
          ...
          if( (sapi_ptr->dcomp_pool[q_item->dcomp_e_ptr->alg.xid_v42bis.dcomp1] != NULL) )
          {
            ...
            else
            {
              ...
              */
              MSG_GERAN_ERROR_1_G("GSN "" XID %d: DCOMP values is not SELECTED ",
                gsn_extern_sapi[sapi_ptr->sapi]);
              return( FALSE );
            }
  ...
  */
  ...
  if( dec_xid.t0.status != XID_ABSENT )
  {
          ...
          (dec_xid.t0.ver_num != GSN_VER_NUM_1  ) )))
    {
      ...
      MSG_GERAN_LOW_0_G("GSN "" XID: Resp Inval len or value of XID T0");
      return( FALSE );
    }
  }
...
#else
          {
                  ...
                  */
                  if( dec_ent->xid.hdr.p == 0 )
                  {
                    ...
                    if( sapi_ptr->pcomp_e_pool[ent_num]->t2_alg == T2_ALG_RFC1144 )
                    {
                      ...
                      {
                        MSG_GERAN_ERROR_3_G("GSN "" Inval PCOMP1 %d, PCOMP2 %d",
                    ...
                    else if( sapi_ptr->pcomp_e_pool[ent_num]->t2_alg == T2_ALG_RFC2507 )
                    {
                      ...
                      {
                        MSG_GERAN_ERROR_3_G("GSN "" Inval PCOMP1 %d, PCOMP2 %d PCOMP3 %d",
                        ...
                        MSG_GERAN_ERROR_3_G("GSN "" Inval PCOMP4 %d, PCOMP5 %d",
                    ...
                    {
                      MSG_GERAN_ERROR_3_G("GSN "" Invalid t2 alg %d",
            ...
            */
            if( !dec_ent->xid.hdr.p  )
            {
              if( sapi_ptr->pcomp_e_pool[ent_num] == NULL )
              {
                ...
                MSG_GERAN_MED_0_G("GSN ""XID req: P=0 for unassigned entity");
            ...
            if( dec_ent->xid.hdr.p && dec_ent->xid.hdr.length )
            {
              ...
              if(dec_ent->xid.hdr.alg_type == T2_ALG_RFC1144)
              {
                    ...
                    [dec_ent->xid.alg.xid_rfc1144.pcomp2]->state == SELECTED) ) )
                {
                  ...
                  MSG_GERAN_ERROR_1_G("GSN ""LL %d: PCOMP val SELECTED after collision res",
                        gsn_extern_sapi[sapi_ptr->sapi]);
                }
            ...
            */
                     ...
                     (sapi_ptr->pcomp_e_pool[ent_num]->state   == SELECTED) )
            {
              ...
              MSG_GERAN_ERROR_1_G("GSN ""LL %d: Ent SELECTED after collision res",
                      gsn_extern_sapi[sapi_ptr->sapi]);
            }
            ...
            if(dec_ent->xid.hdr.alg_type == T2_ALG_RFC1144)
            {
                ...
                if( ack_nsapis && unack_nsapis )
                {
                  ...
                  */
                  ...
                  else
                  {
                    ...
                    MSG_GERAN_ERROR_1_G("GSN ""LL %d: No Actv NSAPIs on an Actv SAPI",
                      gsn_extern_sapi[sapi_ptr->sapi]);
                  }
                }
              ...
              if( sapi_ptr->pcomp_e_pool[ent_num]  == NULL )
              {
                ...
                if( gsn_create_pcomp_e( &xid_i.type_field.xid_t2, sapi_ptr ) )
                {
                  ...
                  */
                  ...
                  {
                    MSG_GERAN_HIGH_1_G("GSN ""PCI comp ent %d couldn't be initialised",ent_num);
                    rmv_ent = TRUE;
                  }
                }
                else
                {
                  MSG_GERAN_HIGH_1_G("GSN ""PCI comp ent %d could not be created ",ent_num);
                  rmv_ent = TRUE;
                }
              }
                  ...
                  [dec_ent->xid.alg.xid_rfc1144.pcomp2]  == NULL)  )
              {
                ...
                if( dec_ent->xid.alg.xid_rfc1144.pcomp1 )
                {
                  ...
                  {
                    MSG_GERAN_HIGH_2_G("GSN ""XID %d: Inval PCOMP val %d",
                      ...
                      [dec_ent->xid.alg.xid_rfc1144.pcomp2]  == NULL)  )
              {
                 ...
                 */
                MSG_GERAN_ERROR_0_G("GSN "" Invalid state of PCOMP values");
                rmv_ent = TRUE;
              }
              ...
              if( sapi_ptr->pcomp_e_pool[ent_num]  == NULL )
              {
                ...
                if( gsn_create_pcomp_e( &xid_i.type_field.xid_t2, sapi_ptr ) )
                {
                  ...
                  */
                  ...
                  {
                    MSG_GERAN_HIGH_1_G("GSN ""iphc comp ent %d couldn't be initialised",ent_num);
                    rmv_ent = TRUE;
                  }
              ...
              if( sapi_ptr->pcomp_e_pool[ent_num]  == NULL )
              {
                ...
                if( gsn_create_pcomp_e( &xid_i.type_field.xid_t2, sapi_ptr ) )
                {
                  ...
                  */
                  ...
                  {
                    MSG_GERAN_HIGH_1_G("GSN ""rohc comp ent %d couldn't be initialised",ent_num);
                    rmv_ent = TRUE;
                  }
                }
                else
            ...
            {
              MSG_GERAN_ERROR_1_G("GSN ""Invalid t2 alg %d",
                    sapi_ptr->pcomp_e_pool[ent_num]->t2_alg);
            }
          }
#endif /*  FEATURE_GSM_GPRS_SNDCP_PCOMP */
                  ...
                  if( dec_ent->xid.hdr.p == 0 )
                  {
                    ...
                    if( sapi_ptr->dcomp_e_pool[ent_num]->t1_alg == T1_ALG_V42BIS )
                    {
                            ...
                            [sapi_ptr->dcomp_e_pool[ent_num]->alg.xid_v42bis.dcomp1] != NULL )
                      {
                        ...
                        MSG_GERAN_ERROR_1_G("GSN "" Invalid DCOMP value %d", sapi_ptr->dcomp_e_pool[ent_num]->
            ...
            */
            ...
            if( dec_ent->xid.hdr.p && dec_ent->xid.hdr.length  )
            {
              ...
              */
              ...
              {
                /* After collision reslolution there shouldn't be any SELECTED DCOMP values */
                MSG_GERAN_ERROR_1_G("GSN ""LL %d: DCOMP val SELECTED after collision res",
                        gsn_extern_sapi[sapi_ptr->sapi]);
              }
            ...
            */
              ...
              */
                  ...
                  ( dec_ent->xid.hdr.length >= 5)) )
              {
                ...
                */
                ...
                {
                  MSG_GERAN_HIGH_2_G("GSN ""LL %d: Invalid XID header len %d",
            ...
            if( sapi_ptr->dcomp_e_pool[ent_num]  == NULL )
            {
              ...
              if( gsn_create_dcomp_e( &xid_i.type_field.xid_t1, sapi_ptr ) )
              {
                ...
                */
                ...
                {
                  MSG_GERAN_HIGH_1_G("GSN ""Data comp ent %d couldn't be initialised",ent_num);
                  rmv_ent = TRUE;
                }
              }
              else
              {
                MSG_GERAN_HIGH_1_G("GSN ""Data comp ent %d could not be created ",ent_num);
                rmv_ent = TRUE;
              }
            }
                ...
                [dec_ent->xid.alg.xid_v42bis.dcomp1]  == NULL )
            {
              ...
              if( dec_ent->xid.alg.xid_v42bis.dcomp1 )
              {
                ...
                {
                  MSG_GERAN_HIGH_2_G("GSN ""XID %d: Inval DCOMP val %d",
...
dsm_item_type* gsn_encode_req_xid( gas_id_t gas_id, const gsn_nsapi_e_t *nsapi_ptr )
{
  ...
  if( nsapi_ptr->pci_comp == TRUE )
  {
    ...
    {
      MSG_GERAN_LOW_1_G( "GSN NS %d: No need for PCI comp XID req",
    ...
    else
    {
      ...
      */
      ...
      {
        MSG_GERAN_HIGH_0_G("GSN ""No PCI comp entities left");
        pxid_req_ptr = NULL;
      }
      else
      {
        ...
        */
        if( nsapi_ptr->sn_xid_req.t2_alg == T2_ALG_RFC1144 )
        {
          ...
          */
          if( q_cnt(&nsapi_ptr->sapi_ptr->t2_alg_q[T2_ALG_RFC1144]) )
          {
            ...
            {
              MSG_GERAN_HIGH_0_G("GSN ""Invalid PCI comp entity in T2 queue");
        ...
        {
          MSG_GERAN_HIGH_1_G("GSN ""LL %d: Inval t2 algorithm",
        ...
        if( ent_num != NO_COMP_ENT )
        {
          ...
          */
          ...
          if( pcomp_e_ptr != NULL )
          {
            ...
            {
              MSG_GERAN_HIGH_0_G("Invalid pcomp alg");
            }
            ...
            if( t2_len != (pxid_src.length + XID_TYPE_HDR_LEN) )
              MSG_GERAN_HIGH_0_G("GSN ""XID Error encoding T2 XID params");
        ...
        {
          MSG_GERAN_HIGH_0_G("GSN ""PCI comp entity has not been created");
          pxid_req_ptr = NULL;
        }
      }
    }
  ...
  if( nsapi_ptr->data_comp == TRUE )
  {
    ...
    */
    ...
    {
      MSG_GERAN_LOW_1_G( "GSN NS %d: No need for Data comp XID req",
        GSN_EXTERN_NSAPI( nsapi_ptr->nsapi ));
    }
    else
    {
      ...
      */
      ...
      if( op_mode_match && t1_q_item )
      {
        if( t1_q_item->dcomp_e_ptr->state != UNASSIGNED )
        {
          ...
          */
          ...
          {
            MSG_GERAN_LOW_0_G("GSN ""XID encode. Error encoding T1 XID params");
          }
        ...
        {
          MSG_GERAN_ERROR_1_G("GSN ""LL %d: Inval V42 bis state",
                      gsn_extern_sapi[nsapi_ptr->sapi_ptr->sapi]);
        }
      }
      else
      {
        ...
        */
        ...
        if( ent_num == NO_COMP_ENT )
        {
          MSG_GERAN_HIGH_0_G("GSN ""No Data comp entities left");
          dxid_req_ptr = NULL;
        }
        else
        {
          ...
          */
          if( nsapi_ptr->sn_xid_req.t1_alg == T1_ALG_V42BIS )
          {
            ...
            */
            if( q_cnt(&nsapi_ptr->sapi_ptr->t1_alg_q[T1_ALG_V42BIS]) )
            {
              ...
              {
                MSG_GERAN_HIGH_0_G("GSN ""Invalid Data comp entity in T1 queue");
          ...
          {
            MSG_GERAN_HIGH_1_G("GSN ""LL %d: Inval T1 algorithm",
                      gsn_extern_sapi[nsapi_ptr->sapi_ptr->sapi]);
          ...
          if( ent_num != NO_COMP_ENT )
          {
            ...
            */
            ...
            if( dcomp_e_ptr != NULL )
            {
              ...
              /* check length of T2 XID */
              if( t1_len != (dxid_src.length + XID_TYPE_HDR_LEN) )
                MSG_GERAN_HIGH_0_G("GSN ""XID Error encoding T1 XID params");
              ...
              */
              if( !q_cnt(&nsapi_ptr->sapi_ptr->t1_alg_q[xid_t1.hdr.alg_type]) )
              {
                if( xid_t1.hdr.alg_type == T1_ALG_V42BIS )
                {
                  ...
                  MSG_GERAN_HIGH_0_G("GSN ""XID Invalid T1 algorithm");
              }
            }
          }
          else
          {
            MSG_GERAN_HIGH_0_G("GSN ""Data comp entity has not been created");
            dxid_req_ptr = NULL;
          }
        }
      }
  ...
  */
  ...
  if( !pxid_req_ptr && !dxid_req_ptr )
  {
    ...
    MSG_GERAN_MED_1_G("GSN ""LL %d: No PCOMP or DCOMP xid req",
                      gsn_extern_sapi[nsapi_ptr->sapi_ptr->sapi]);
  } /*lint -restore*/
  else
  {
    ...
    */
    if( pxid_req_ptr && dxid_req_ptr )
    {
      ...
      MSG_GERAN_MED_1_G("GSN ""LL %d: PCOMP & DCOMP xid req",
                        gsn_extern_sapi[nsapi_ptr->sapi_ptr->sapi]);
    }
    else if( pxid_req_ptr )
    {
      ...
      MSG_GERAN_MED_1_G("GSN ""LL %d: Only PCOMP xid req",
                        gsn_extern_sapi[nsapi_ptr->sapi_ptr->sapi]);
    }
    else if( dxid_req_ptr )
    {
      ...
      MSG_GERAN_MED_1_G("GSN ""LL %d: Only DCOMP xid req",
                        gsn_extern_sapi[nsapi_ptr->sapi_ptr->sapi]);
    }
    else
      xid_req_ptr = NULL;
    ...
    if( xid_req_ptr )
    {
      if( nsapi_ptr->sapi_ptr->xid_req_dup_ptr == NULL )
      {
        ...
        MSG_GERAN_HIGH_1_G("GSN ""LL %d: Inval xid_req_dup_ptr",
                        gsn_extern_sapi[nsapi_ptr->sapi_ptr->sapi]);
    }
#endif /* FEATURE_GSM_GPRS_SNDCP_PCOMP || FEATURE_GSM_GPRS_SNDCP_DCOMP */
  }
...
void gsn_del_dcomp_e( gas_id_t gas_id, uint8 ent_num, gsn_sapi_e_t *sapi_ptr )
{
  ...
  if( sapi_ptr->dcomp_e_pool[ent_num] == NULL )
  {
    MSG_GERAN_ERROR_1_G("GSN ""Invalid Data compression entity %d", ent_num);
    return;
  }
  ...
  if( alg == T1_ALG_V42BIS )
  {
    ...
    {
      /* At least one item should be in the queue */
      MSG_GERAN_ERROR_1_G("GSN ""XID %d: Resp inval T1 alg queue",
                  gsn_extern_sapi[sapi_ptr->sapi]);
    }
  }
  else
    MSG_GERAN_ERROR_1_G("GSN ""XID %d: Inval T1 alg",
                  gsn_extern_sapi[sapi_ptr->sapi]);
...
void gsn_del_pcomp_e( gas_id_t gas_id, uint8 ent_num, gsn_sapi_e_t *sapi_ptr )
{
  ...
  if( sapi_ptr->pcomp_e_pool[ ent_num ] == NULL )
  {
    MSG_GERAN_ERROR_1_G("GSN "" Invalid PCI compression entity %d", ent_num);
    return;
  }
  ...
  if( alg <= T2_ALG_MAX )
  {
      ...
      while( q_item != NULL )
      {
        ...
        if( q_item->pcomp_e_ptr->ent_num == ent_num )
        {
          ...
          for( i = 0; i < (uint8) GSN_NSAPI_MAX; i++ )
          {
            ...
            {
              MSG_GERAN_ERROR_2_G("GSN ""XID %d: Inval Algorithm type",
          ...
          {
            MSG_GERAN_ERROR_0_G("GSN ""Inval Algorithm type");
          }
   ...
   MSG_GERAN_ERROR_1_G("GSN "" Inval T2 alg",
                gsn_extern_sapi[sapi_ptr->sapi]);
}
#endif /* FEATURE_GSM_GPRS_SNDCP_PCOMP */
...
void gsn_print_xid_params ( gas_id_t gas_id, dsm_item_type  *xid_pdu, boolean ul )
{
  ...
  if( xid_pdu )
  {
    ...
    {
      MSG_GERAN_MED_0_G("GSN ""DEBUG: XID strip to buff failed");
    }
    else
    {
      ptr = &src[0];
      ...
      if( ul == TRUE )
      {
        MSG_GERAN_MED_1_G("GSN ""UL XID len %d", len);
        MSG_GERAN_MED_3_G("GSN ""UL XID %08x %08x %08x", a, b, c );
      }
      else
      {
        MSG_GERAN_MED_1_G("GSN ""DL XID len %d", len);
        MSG_GERAN_MED_3_G("GSN ""DL XID %08x %08x %08x", a, b, c );
      }
      ...
      if( len > 12 )
      {
        ...
        if( ul == TRUE )
        {
          MSG_GERAN_MED_3_G("GSN ""UL XID cont %08x %08x %08x", a, b, c );
        }
        else
        {
          MSG_GERAN_MED_3_G("GSN ""DL XID cont %08x %08x %08x", a, b, c );
        }
      }
    }
  }
  ...
  {
    MSG_GERAN_MED_0_G("GSN ""DEBUG: Empty XID PDU ");
  }
}
