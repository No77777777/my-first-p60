...
***
*** $Header: //components/rel/geran.mpss/7.4.0/gsndcp/src/gsn_hdlr.c#1 $
***
...
#include "gsndcpi.h"
#include "gprsmod.h"
#include "mcfg_nv_api.h"      /* read Anite GCF flag */
...
static void gsn_send_ll_rel_req( gas_id_t, gsn_sapi_e_t *, boolean, boolean);
...
gsndcp_debug_data_t gsndcp_debug_data[ NUM_GERAN_DATA_SPACES ];
#endif
...
/*-------------------------------------------------------------------------------*/
  case ( IDLE ):
    ...
    MSG_GERAN_HIGH_1_G("GSN ""LL %d: Sent LL_ACT_IND",gsn_extern_sapi[sapi]);
...
/*-------------------------------------------------------------------------------*/
  case ( IDLE ):
    ...
    /* Post message to LLC and log for call flow analysis. */
    MSG_GERAN_HIGH_1_G("GSN ""LL %d: Sent LL_DEACT_IND",gsn_extern_sapi[sapi]);
    ...
    */
    ...
    if( nsapi_e_ptr->ule.npdu_buff_valid == TRUE )
    {
      ...
      while( npdu_item_ptr )
      {
        ...
        {
          MSG_GERAN_HIGH_0_G("GSN ""No seg ref buff");
        }
    ...
    {
      MSG_GERAN_HIGH_0_G("GSN ""No npdu buff");
    }
      ...
      for(alg = 0; alg <= T2_ALG_MAX; alg++ )
      {
        ...
        while( t2_item_ptr )
        {
              ...
              (t2_item_ptr->pcomp_e_ptr->t2_alg == T2_ALG_RFC1144) )
          {
            ...
            MSG_GERAN_HIGH_2_G("GSN ""XID %d: Rmv PCOMP entity %d",
      ...
      for( alg = 0; alg <= T1_ALG_MAX; alg++ )
      {
        ...
        while( t1_item_ptr )
        {
              ...
              (t1_item_ptr->dcomp_e_ptr->t1_alg == T1_ALG_V42BIS) )
          {
            ...
            MSG_GERAN_HIGH_2_G("GSN ""XID %d: Rmv DCOMP entity %d",
...
static void gsn_activate_nsapi ( gas_id_t gas_id, gsnsm_prim_type prim_type, const void *smsn_mes_ptr )
{
  ...
  if( actv_nsapis[ gas_id ][nsapi] == NULL )
  {
    ...
    if( prim_type == GSNSM_ACTIVATE_IND )
    {
      ...
      MSG_GERAN_HIGH_2_G("GSN ""GAS_ID: %d for NSAPI: %d ",
       gas_id, nsapi );
      ...
      MSG_GERAN_HIGH_3_G("GSN ""Rel Class %d, PFI %d",
    ...
    else
    {
      ...
      MSG_GERAN_HIGH_2_G("GSN ""GAS_ID: %d for NSAPI: %d ", gas_id, nsapi );
#endif /* FEATURE_DUAL_SIM */
                                           ...
                                           const void *snsm_msg_ptr )
{
  if(prim_type == GSNSM_ACTIVATE_IND)
  {
    ...
    {
     MSG_GERAN_HIGH_0_G("No data comp");
     nsapi_ptr->sn_xid_req.t1_alg  = T1_ALG_NONE;
    }
    else
    {
     MSG_GERAN_HIGH_2_G("Invalid T1 alg: %d for NSAPI: %d",
                                                 nsapi_ptr->sn_xid_req.t1_alg,
                                                 nsapi_ptr->nsapi );
     nsapi_ptr->sn_xid_req.t1_alg  = T1_ALG_NONE;
    }
    ...
    {
      MSG_GERAN_HIGH_0_G("No hdr comp");
      nsapi_ptr->sn_xid_req.t2_alg    = T2_ALG_NONE;
    }
    else
    {
     MSG_GERAN_HIGH_3_G("Invalid pcomp alg (%d) and pdp_type (%d) params ",
                 hdr_comp_alg,
                 pdp_type,
                 0);
     nsapi_ptr->sn_xid_req.t2_alg     = T2_ALG_NONE;
  ...
  else
  {
    ...
    {
      MSG_GERAN_HIGH_2_G("Invalid T2 alg: %d for NSAPI: %d",nsapi_ptr->sn_xid_req.t1_alg,
                                                  nsapi_ptr->nsapi);
      nsapi_ptr->sn_xid_req.t1_alg  = T1_ALG_NONE;
    }
    ...
    {
      MSG_GERAN_HIGH_2_G("Invalid pcomp alg(%d) and pdp_type (%d) params",
                 hdr_comp_alg,
                 pdp_type);
      nsapi_ptr->sn_xid_req.t2_alg     = T2_ALG_NONE;
    }
                           ...
                           const void      *snsm_msg_ptr )
{
  ...
  */
  ...
  if ( prim_type == GSNSM_MODIFY_IND )
  {
    if ( ( ( (gsnsm_modify_ind_type *)snsm_msg_ptr)->qos.reliability_class == 0x1 ) ||
         ( ( (gsnsm_modify_ind_type *)snsm_msg_ptr)->qos.reliability_class == 0x2 )
       )
    {
      ...
      MSG_GERAN_ERROR_0_G("GSN ""LINK EST Timer defined");
    }
  }
}
...
static void gsn_deactv_nsapi( gas_id_t gas_id, gsn_nsapi_e_t *nsapi_ptr )
{
  if( nsapi_ptr != NULL )
  {
    ...
    rex_undef_timer( &nsapi_ptr->sgsn_initiated_link_est_fail_timer );

    MSG_GERAN_ERROR_0_G("GSN ""LINK EST Timer undefined");
...
===
===========================================================================*/
static boolean gsn_check_ll_rel_needed(gsn_nsapi_t nsapi, gsn_sapi_t sapi, gas_id_t gas_id)
{
  ...
  if (sapi_ptr->is_nsapi_ptr_q_valid)
  {
    ...
    if (q_cnt(&(sapi_ptr->nsapi_ptr_q)) > 1)
    {
      ...
      boolean sapi_uses_ack_op = gsn_ack_nsapis_on_sapi(gas_id, nsapi, sapi);
...
static boolean gsn_release_sapi(gas_id_t gas_id, gsn_nsapi_t nsapi, gsn_sapi_t sapi, boolean local)
{
  ...
  {
    sapi_ptr = actv_sapis[gas_id][temp_sapi];
    ...
    if ((temp_sapi != sapi) && (sapi_ptr != NULL))
    {
         ...
         q_linear_search(&(sapi_ptr->nsapi_ptr_q), (q_compare_func_type)gsn_find_nsapi, &nsapi) != NULL)
      {
        ...
        if (gsn_check_ll_rel_needed(nsapi, temp_sapi, gas_id))
        {
          gsn_send_ll_rel_req(gas_id, sapi_ptr, TRUE, FALSE);
        }

        /* Remove the NSAPI link from this SAPI */
        gsn_remove_nsapi_from_sapi(gas_id, nsapi, temp_sapi);
      }
    }
  }
  ...
  case ( ESTABLISHED):
    {
      ...
      /* Post message to LLC and log for call flow analysis. */
      MSG_GERAN_HIGH_1_G("GSN ""LL %d: Sent LL_ESTABLISH_RES",gsn_extern_sapi[sapi]);
...
===
===========================================================================*/
static void gsn_send_ll_rel_req(gas_id_t gas_id, gsn_sapi_e_t *sapi_ptr, boolean local, boolean cnf_needed)
{
  gsn_sapi_t            sapi = sapi_ptr->sapi;
  gprs_ll_ul_sig_mes_t  *ul_mes;
  ...
  case ( ESTABLISHED ):
    {
      ...
      /* Post message to LLC and log for call flow analysis. */
      MSG_GERAN_HIGH_3_G("GSN ""LL %d: Sent LL_RELEASE_REQ (local=%d, cnf_needed=%d)", gsn_extern_sapi[sapi], local, cnf_needed);
  ...
  case ( ESTABLISHED):
    {
      ...
      /* Post message to LLC and log for call flow analysis. */
      MSG_GERAN_HIGH_1_G("GSN ""LL %d: Sent LL_XID_RES",gsn_extern_sapi[sapi]);
  ...
  case ( ESTABLISHED):
    {
      ...
      /* Post message to LLC and log for call flow analysis. */
      MSG_GERAN_HIGH_1_G("GSN ""LL %d: Sent LL_RESET_RES",gsn_extern_sapi[sapi]);
        ...
        if( item_ptr )
        {
          while( item_ptr != NULL )
          {
                ...
                (item_ptr->nsapi_ptr->modif_est_resp_pend)    )
            {
              ...
              */
              rex_undef_timer( &item_ptr->nsapi_ptr->sgsn_initiated_link_est_fail_timer );
      ...
      case GPRS_LL_NO_PEER_RESPONSE:
        ...
        if( sapi_ptr->link_estab_originator == TRUE)
        {
          ...
          */
          ...
          gsn_send_ll_est_req( gas_id, sapi_ptr, NULL );
#endif /* !(FEATURE_GSM_GPRS_SNDCP_PCOMP ||FEATURE_GSM_GPRS_SNDCP_DCOMP) */
        }
        break;
      ...
      if (sapi_ptr->is_nsapi_ptr_q_valid)
      {
        ...
        */
        while((qcnt > 0) && (item_ptr != NULL))
        {
          ...
          MSG_GERAN_HIGH_2_G("Temp_DBG:: qcnt=%d, NSAPI=%d", qcnt, item_ptr->nsapi_ptr->nsapi);
    ...
    if( new_sapi != old_sapi )
    {
      ...
      */
                ...
                (actv_sapis[gas_id][new_sapi]->state == IDLE) )
      {
        ...
        ** link est failure timer.
        */
         rex_undef_timer( &nsapi_e_ptr->sgsn_initiated_link_est_fail_timer );
...
static void gsn_send_snsm_actv_res( gas_id_t gas_id, gsn_nsapi_t nsapi )
{
  ...
  MSG_GERAN_HIGH_1_G("GSN ""ME: Sent SNSM_ACTIVATE_RSP NSAPI%d",GSN_EXTERN_NSAPI(nsapi));
...
static void gsn_send_snsm_deactv_res( gas_id_t gas_id, gsn_nsapi_t nsapi )
{
  ...
  MSG_GERAN_HIGH_1_G("GSN ""ME: Sent SNSM_DEACTIV_RESP NSAPI%d",GSN_EXTERN_NSAPI(nsapi));
...
static void gsn_send_snsm_mod_res( gas_id_t gas_id, gsn_nsapi_t nsapi )
{
  ...
  MSG_GERAN_HIGH_1_G("GSN ""ME: Sent SNSM_MODIFY_RESP NSAPI%d",GSN_EXTERN_NSAPI(nsapi));
...
static void gsn_send_snsm_seq_res( gas_id_t gas_id, gsn_nsapi_t nsapi, gsn_nsapi_mode_t nsapi_mode )
{
  ...
  MSG_GERAN_HIGH_1_G("GSN ""ME: Sent SNSM_SEQUENCE_RESP NSAPI%d",GSN_EXTERN_NSAPI(nsapi));
  ...
  while( len )
  {
    ...
    */
    ...
    case GSN_SAPI_3:
      MSG_GERAN_HIGH_3_G
      (
        "GSN LL 3: Sent LL_DATA_REQ %d:%d:%d",
        sn_hdr.nsapi,
    ...
    case GSN_SAPI_5:
      MSG_GERAN_HIGH_3_G
      (
        "GSN LL 5: Sent LL_DATA_REQ %d:%d:%d",
        sn_hdr.nsapi,
        ll_mes->mes.data_req_mes.sndcp_ref.npdu_num,
        ll_mes->mes.data_req_mes.sndcp_ref.seg_num
      );
    ...
    case GSN_SAPI_9:
      MSG_GERAN_HIGH_3_G
      (
        "GSN LL 9: Sent LL_DATA_REQ %d:%d:%d",
        sn_hdr.nsapi,
        ll_mes->mes.data_req_mes.sndcp_ref.npdu_num,
        ll_mes->mes.data_req_mes.sndcp_ref.seg_num
      );
    ...
    case GSN_SAPI_11:
      MSG_GERAN_HIGH_3_G
      (
        "GSN LL 11: Sent LL_DATA_REQ %d:%d:%d",
        sn_hdr.nsapi,
        ll_mes->mes.data_req_mes.sndcp_ref.npdu_num,
        ll_mes->mes.data_req_mes.sndcp_ref.seg_num
      );
...
/*===========================================================================
===
===  FUNCTION      gsndcp_get_debug_data_ptr()
===
===  DESCRIPTION
===
...
gsndcp_debug_data_t* gsndcp_get_debug_data_ptr(gas_id_t gas_id)
{
  gsndcp_debug_data_t *debug_data_ptr = &gsndcp_debug_data[0];

  /* validate the GAS ID */
  if (gas_id <= GERAN_ACCESS_STRATUM_ID_MAX)
  {
    debug_data_ptr = &gsndcp_debug_data[GERAN_MAP_GAS_ID_TO_DATA_SPACE_INDEX(gas_id)];
  }
  else
  {
  ...
  case( ESTABLISHED ):
    {
      ...
      /* Post message to LLC and log for call flow analysis. */
      MSG_GERAN_HIGH_1_G("GSN ""LL %d: Sent LL_ESTABLISH_REQ",gsn_extern_sapi[sapi]);
...
/*-------------------------------------------------------------------------------*/
  ...
  gsndcp_qsh_metric_capture(gas_id);
#endif
}
...
)
{
  ...
  if( actv_sapis[gas_id][sapi] != NULL )
  {
    ...
    {
      MSG_GERAN_ERROR_3_G("GSN ""Invalid GAS_ID: Cur = %d, Rcvd = %d",
    ...
    case LL_RESET_IND:
      ...
      MSG_GERAN_HIGH_1_G("GSN ""LL %d: Rcvd LL_RESET_IND",gsn_extern_sapi[sapi]);
    ...
    case LL_ESTABLISH_IND:
      ...
      MSG_GERAN_HIGH_1_G("GSN ""LL %d: Rcvd LL_EST_IND",gsn_extern_sapi[sapi]);
    ...
    case LL_ESTABLISH_CNF:
      ...
      MSG_GERAN_HIGH_1_G("GSN ""LL %d: Rcvd LL_EST_CNF",gsn_extern_sapi[sapi]);
    ...
    case LL_RELEASE_IND:
      ...
      MSG_GERAN_HIGH_1_G("GSN ""LL %d: Rcvd LL_REL_IND",gsn_extern_sapi[sapi]);
    ...
    case LL_RELEASE_CNF:
      ...
      MSG_GERAN_HIGH_1_G("GSN ""LL %d: Rcvd LL_REL_CNF",gsn_extern_sapi[sapi]);
    ...
    case LL_XID_IND:
      ...
      MSG_GERAN_HIGH_1_G("GSN ""LL %d: Rcvd LL_XID_IND",gsn_extern_sapi[sapi]);
    ...
    case LL_XID_CNF:
      ...
      MSG_GERAN_HIGH_1_G("GSN ""LL %d: Rcvd LL_XID_CNF",gsn_extern_sapi[sapi]);
    ...
    case LL_STATUS_IND:
      ...
      MSG_GERAN_HIGH_1_G("GSN ""LL %d: Rcvd LL_STATUS_IND",gsn_extern_sapi[sapi]);
...
void gsn_sm_prim_hdlr( gas_id_t gas_id, const gsmsn_mes_type * mes_ptr)
{
  ...
  case GSNSM_ACTIVATE_IND:
    ...
    MSG_GERAN_HIGH_2_G("GSN ""ME: Rcvd SNSM_ACTIV_IND for NSAPI%d, SAPI%d",
  ...
  case GSNSM_DEACTIVATE_IND:
    ...
    {
      MSG_GERAN_HIGH_1_G("GSN ""ME: Rcvd SNSM_DEACTIV_IND NSAPI%d",
        mes_ptr->mes.snsm_deactiv_ind.nsapi[0]);
    }
    else
    {
      MSG_GERAN_HIGH_0_G("GSN ""ME: Rcvd SNSM_DEACTIV_IND with num of NSAPIs=0");
    }
  ...
  case GSNSM_MODIFY_IND:
    ...
    MSG_GERAN_HIGH_2_G("GSN ""ME: Rcvd SNSM_MODIFY_IND for NSAPI%d, SAPI%d",
  ...
  case GSNSM_SEQUENCE_IND:
    ...
    MSG_GERAN_HIGH_1_G("GSN ""ME: Rcvd SNSM_SEQUENCE_IND for NSAPI%d",
...
gas_id_t gsn_int_prim_get_gas_id ( gsn_internal_msg_t *mes_ptr )
{
  ...
  case (TIMER_CMD):
    ...
    break;
#ifdef FEATURE_QSH_EVENT_METRIC
   case (QSH_METRIC_DATA_REQ):

    gas_id = mes_ptr->gsn_cmd.qsh_metric_data_req.gas_id;
...
void gsn_int_cmd_hdlr( gas_id_t gas_id, const gsn_internal_msg_t *mes_ptr)
{
  ...
  case( PDP_REG_REQ ):
    ...
    else
    {
           ...
           actv_nsapis[ gas_id ][gnsapi] != NULL                            )
      {
        ...
        {
          MSG_GERAN_HIGH_3_G("GSN ""Info Reg func suspend_ptr=%p,resume_fnc_ptr=%p,pdp_dl_ptr=%p",
        ...
        else
        {
                  ...
                  )
          {
            ...
            MSG_GERAN_HIGH_1_G("GSN " "NS %d: PDP Reg failed",
  ...
  case (QSH_METRIC_DATA_REQ):
  {
    ...
    qsh_client_cb_params_s *cb_params_ptr =
      (qsh_client_cb_params_s *)&mes_ptr->gsn_cmd.qsh_metric_data_req.qsh_cb_params;
    ...
    if(metric_config_params_ptr->action == QSH_METRIC_ACTION_START)
    {
      ...
      debug_data_ptr->gsndcp_qsh_metrics_capture_activate = TRUE;
    }
    else
    {
      debug_data_ptr->gsndcp_qsh_metrics_capture_activate = FALSE;
    }
    
    /* Indicate callback action done (once the action has been processed)
    */
    gsndcp_ind_cb_action_done(cb_params_ptr);
    break;
  }
...
void gsn_process_ll_data_cnf( gas_id_t gas_id, gsn_sapi_e_t *sapi_ptr, ll_data_cnf_t *ll_mes)
{
  ...
  {
  case GSN_SAPI_3:
    MSG_GERAN_HIGH_3_G("GSN ""LL 3: Rcvd LL_DATA_CNF %d:%d:%d",
                  GSN_EXTERN_NSAPI(nsapi), npdu_num, seg_num );
    break;

  case GSN_SAPI_5:
    MSG_GERAN_HIGH_3_G("GSN ""LL 5: Rcvd LL_DATA_CNF %d:%d:%d",
                  GSN_EXTERN_NSAPI(nsapi), npdu_num, seg_num );
    break;

  case GSN_SAPI_9:
    MSG_GERAN_HIGH_3_G("GSN ""LL 9: Rcvd LL_DATA_CNF %d:%d:%d",
                  GSN_EXTERN_NSAPI(nsapi), npdu_num, seg_num );
    break;

  case GSN_SAPI_11:
    MSG_GERAN_HIGH_3_G("GSN ""LL 11: Rcvd LL_DATA_CNF %d:%d:%d",
                GSN_EXTERN_NSAPI(nsapi), npdu_num, seg_num );
    break;
...
void gsn_process_ll_udata_cnf( gas_id_t gas_id, gsn_sapi_e_t *sapi_ptr, ll_unitdata_cnf_t *ll_mes)
{
  ...
  {
    case GSN_SAPI_3:
      MSG_GERAN_HIGH_3_G("GSN ""LL 3: Rcvd LL_UDATA_CNF %d:%d:%d",
                    GSN_EXTERN_NSAPI(nsapi), npdu_num, seg_num );
    break;

    case GSN_SAPI_5:
      MSG_GERAN_HIGH_3_G("GSN ""LL 5: Rcvd LL_UDATA_CNF %d:%d:%d",
                    GSN_EXTERN_NSAPI(nsapi), npdu_num, seg_num );
      break;

    case GSN_SAPI_9:
      MSG_GERAN_HIGH_3_G("GSN ""LL 9: Rcvd LL_UDATA_CNF %d:%d:%d",
                    GSN_EXTERN_NSAPI(nsapi), npdu_num, seg_num );
      break;

    case GSN_SAPI_11:
      MSG_GERAN_HIGH_3_G("GSN ""LL 11: Rcvd LL_UDATA_CNF %d:%d:%d",
                    GSN_EXTERN_NSAPI(nsapi), npdu_num, seg_num );
      break;
    ...
    */
     ...
     (actv_nsapis[ gas_id ][GSN_INTERN_NSAPI(temp_nsapi)] != NULL))
    {
      if( hdr_pkd[0] & (0x1 << F_BIT_POS) )
      {
        ...
        MSG_GERAN_HIGH_3_G("GSN " "LL %d: Rcvd LL_DATA_IND %d:%d:F",
      ...
      else
      {
        ...
        {
          MSG_GERAN_HIGH_3_G("GSN " "LL %d: Rcvd LL_DATA_IND %d:%d:M",
          gsn_extern_sapi[sapi], temp_nsapi,
          actv_nsapis[ gas_id ][GSN_INTERN_NSAPI(temp_nsapi)]->dle.first_seg_info.ack_npdu_num );
        }
        else
        {
          MSG_GERAN_HIGH_3_G("GSN " "LL %d: Rcvd LL_DATA_IND %d:%d:L",
          gsn_extern_sapi[sapi], temp_nsapi,
          actv_nsapis[ gas_id ][GSN_INTERN_NSAPI(temp_nsapi)]->dle.first_seg_info.ack_npdu_num );
        }
      }
            ...
            switch( actv_nsapis[ gas_id ][sn_hdr.nsapi]->dle.state )
            {
              case DL_NORMAL:
                  ...
                  )
                {
                  ...
                  MSG_GERAN_HIGH_2_G("GSN ""LL %d: Rcvd duplicated N-PDU %d",
                  gsn_extern_sapi[sapi],
                  actv_nsapis[ gas_id ][sn_hdr.nsapi]->dle.first_seg_info.ack_npdu_num);
                ...
                else
                {
                  ...
                  /* Send N-PDU to PDP */
                  MSG_GERAN_HIGH_2_G("GSN " "NS %d: Sent to PDP SN_DATA_IND NPDU %d ",
                                GSN_EXTERN_NSAPI(sn_hdr.nsapi),
                                actv_nsapis[ gas_id ][sn_hdr.nsapi]->dle.ack_rcv_npdu);
    ...
    if( gsn_decode_unitdata_pdu_hdr( gas_id, hdr_pkd, &sn_hdr) )
    {
      ...
      */
      if( actv_nsapis[ gas_id ][sn_hdr.nsapi] != NULL )
      {
        ...
        {
        case GSN_SAPI_3:
          MSG_GERAN_HIGH_3_G("GSN "" LL 3: Rcvd LL_UDATA_IND %d:%d:%d",
                        GSN_EXTERN_NSAPI(sn_hdr.nsapi), sn_hdr.num, sn_hdr.seg );
          break;

        case GSN_SAPI_5:
          MSG_GERAN_HIGH_3_G("GSN ""LL 5: Rcvd LL_UDATA_IND %d:%d:%d",
                        GSN_EXTERN_NSAPI(sn_hdr.nsapi), sn_hdr.num, sn_hdr.seg );
          break;

        case GSN_SAPI_9:
          MSG_GERAN_HIGH_3_G("GSN "" LL 9: Rcvd LL_UDATA_IND %d:%d:%d",
                        GSN_EXTERN_NSAPI(sn_hdr.nsapi), sn_hdr.num, sn_hdr.seg );
          break;

        case GSN_SAPI_11:
          MSG_GERAN_HIGH_3_G("GSN "" LL 11: Rcvd LL_UDATA_IND %d:%d:%d",
                        GSN_EXTERN_NSAPI(sn_hdr.nsapi), sn_hdr.num, sn_hdr.seg );
          break;
          ...
          */
          if( npdu_ref_item_ptr != NULL )
          {
            ...
            while( seg_item_ptr != NULL )
            {
              ...
              {
                /* Send N-PDU to PDP */
                MSG_GERAN_HIGH_3_G("GSN " "NS %d: Sent to PDP SN_UNITDATA_IND NPDU %d",
...
/*-------------------------------------------------------------------------------*/
  ...
  case ( ESTABLISHED):
    ...
    /* Post message to LLC and log for call flow analysis. */
    MSG_GERAN_HIGH_1_G("GSN ""LL %d: Sent LL_XID_REQ",gsn_extern_sapi[sapi]);
...
void gsn_send_snsm_stat_req( gas_id_t gas_id, gsn_sapi_t sapi, uint32 tlli, uint8 cause)
{
  ...
  MSG_GERAN_HIGH_1_G("GSN ""ME: Sent SNSM_STATUS_REQ SAPI %d",gsn_extern_sapi[sapi]);
#ifdef FEATURE_QSH_EVENT_NOTIFY_HANDLER
  #error code not present
#endif /* FEATURE_QSH_EVENT_NOTIFY_HANDLER */
  sm_put_cmd( snsm_dl_mes );
