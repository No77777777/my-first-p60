...
***
*** $Header: //components/rel/geran.mpss/7.4.0/gsndcp/inc/gsni.h#2 $
***
...
typedef enum
{
  ...
  QSH_METRIC_DATA_REQ  /* Perform QSH Metric Configure */ 
#endif
...
typedef struct
{
  qsh_client_cb_params_s  qsh_cb_params;  /*Parmaters QSH provides via gsndcp_qsh_data_capture_cb()*/
  gas_id_t                gas_id;
} gsn_qsh_metric_data_req_cmd_t;

typedef struct
{
  gsndcp_trace_msg_t              gsndcp_trace_msg_buf[100];
  byte                            gsndcp_trace_msg_buf_index;

  uint8                          *saved_cb_metric_dump_ptr;             /* Points to the current QSH metic location alloacted for SNDCP*/
  ...
  boolean                         gsndcp_qsh_metrics_capture_activate;  /* set by the metric action type rcvd from QSH*/
} gsndcp_debug_data_t;
...
#if defined (FEATURE_QSH_EVENT_METRIC) || defined (FEATURE_QSH_DUMP)
    gsn_qsh_metric_data_req_cmd_t qsh_metric_data_req;
#endif /* FEATURE_QSH_EVENT_METRIC || FEATURE_QSH_DUMP  */
...
extern qsh_client_cb_params_s  gsndcp_qsh_cb_param[NUM_GERAN_DATA_SPACES];
#endif
...
__attribute__((section(".uncompressible.text")))
gsndcp_debug_data_t* gsndcp_get_debug_data_ptr
(
  gas_id_t                    gas_id
);
...
__attribute__((section(".uncompressible.text")))
void gsndcp_ind_cb_action_done
(
...
__attribute__((section(".uncompressible.text")))
void gsndcp_qsh_metric_capture
(
  gas_id_t                    gas_id
);
