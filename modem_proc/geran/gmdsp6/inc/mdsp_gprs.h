...
$Header: //components/rel/geran.mpss/7.4.0/gmdsp6/inc/mdsp_gprs.h#2 $
...
#define MDSP_GPRS_TX_KNOCK_RX        0x0080
...
typedef enum
{
   MDSP_SEND_PWR_CMD_SCE_DBG=1,
   MDSP_GET_PWR_RESULTS_SCE_DBG,
   L1_SEND_PWR_CMD_SCE_DBG,
   L1_GET_PWR_RESULTS_SCE_DBG
} l1_sce_dbg_mdsp_gprs_cmd_T;

typedef struct
{
  uint32                         FN;
  l1_sce_dbg_mdsp_gprs_cmd_T     cmd;
  uint16                         data_len;
  uint16                         readymask;
  uint16                         seq_num_exp;
  uint16                         seq_num_fw;
  ...
  uint16                         offset_s;            /* offset for first arfcn (start)   */
  uint16                         offset_e;            /* offset for last arfcn (end)      */
  uint32                         arfcn_s;             /* first arfcn whose pwr needs to be measured   */
  uint32                         arfcn_e;             /* last arfcn whose pwr needs to be measured    */
  boolean                        mdsp_results_avail;
  ...
  uint32                         hist_wr;
  uint32                         hist_rd;
  uint32                         num_meas_hist [3     /* HIST_QUEUE_SIZE */ ];
  uint8                          num_attempts  [3     /* HIST_QUEUE_SIZE */ ];       /* if we have polled mdsp yet */

  uint32                         num_iterations;
  boolean                        break_loop;
  boolean                        second_attempt[3     /* HIST_QUEUE_SIZE */ ];
  uint32                         expected_results;
  uint32                         retrieved_results;
  boolean                        results_avail;
} l1_sce_dbg_mdsp_gprs_T;

extern l1_sce_dbg_mdsp_gprs_T *l1_sce_dbg_mdsp_gprs_new_entry(l1_sce_dbg_mdsp_gprs_cmd_T cmd, gas_id_t gas_id);
