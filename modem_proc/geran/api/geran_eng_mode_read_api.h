...
***
***  $Header: //components/rel/geran.mpss/7.4.0/api/geran_eng_mode_read_api.h#2 $
***
*** DESCRIPTION
***
...
{                                                        /*geran_modem_stats_mask bit position*/
  ...
  ENG_MODE_TX_PWR_DED,                                   /* No Bit Pos*/
  ENG_MODE_SCELL_INFO_PERSISTENT,                        /* No Bit Pos*/
                                                         /* Add new entries here if they are not required for geran_modem_stats_mask */
  ENG_MODE_ALL_INFORMATION,                              /* Obsolete */
  ENG_MODE_SCELL_NCELL_INFO,                             /* SCELL ARFCN information and NCELL ARFCNs */
  ENG_MODE_DED_OR_PDCH_ARFCN,                            /* Dedicated/PDCH ARFCN */
  ENG_MODE_ALL_FREQ_INFO,                                /* SCELL, NCELL & Hopping frequency list */
...
/************** ENG_MODE_TX_PWR_DED ************************************/
/***********************************************************************/
#define ENG_MODE_TX_PWR_NUM_TIMESLOTS_DED 4

/* type: int16 tx_pwr[ENG_MODE_TX_PWR_NUM_TIMESLOTS_DED] */
/* The array is populated in CS mode only and represents one timeslot
   over 1 block. It not used for Xfer.
   The array indexes do NOT correspond to timeslot numbers.
   Valid range of TX power - 0..31. See Etsi 45.005 sect 4.1.1 */
/***********************************************************************/
...
/***********************************************************************/
/* Should be in sync with sys_band_T  */
typedef enum
{
...
{
   byte                 no_of_items; /*Default to 0 items when frequency list is invalid*/
   ENG_MODE_ARFCN_T     channel[32];
} eng_mode_ncell_freq_list_T;
...
typedef struct
{
  ENG_MODE_ARFCN_T           scell_arfcn;
  eng_mode_ncell_freq_list_T ncell_freq_list;
}eng_mode_scell_ncell_info_t;

typedef struct
{
  boolean                    all_freq_info_valid_flag;
  ENG_MODE_ARFCN_T           scell_arfcn;
  eng_mode_ncell_freq_list_T ncell_freq_list;
  eng_mode_hopping_status_t  hopping_status;
  eng_mode_frequency_list_T  hopping_freq_list;
  ENG_MODE_ARFCN_T           dedicated_or_pdch_freq;
}eng_mode_all_frequency_list_T;
...
/***********************************************************************/
/* same type as ENG_MODE_SCELL_INFO*/
/* Scell info persists even when ENG_MODE_SCELL_INFO is reset*/
...
typedef struct
{
  ...
  /*ENG_MODE_SCELL_INFO_PERSISTENT*/
  eng_mode_scell_info_t      scell_info_persist;
  boolean                    scell_info_persist_valid_flag;

  /*ENG_MODE_ALL_FREQ_INFO*/
  eng_mode_all_frequency_list_T  all_freq_info;
...
/* ENG_MODE_ALL_FREQ_INFO */
typedef void (*all_freq_info_cb_fnc_ptr)(eng_mode_all_frequency_list_T all_freq_info);
