...
$Header: //components/rel/geran.mpss/7.4.0/api/gl1_hw.h#3 $ $DateTime: 2019/12/30 04:21:22 $ $Author: pwbldsvc $
...
03/07/15    jj      CR865562 Remove featurisation introduced for CR839611
...
#include "sys_type.h"  /* For sys_band_T */
...
#ifdef FEATURE_VSWR
extern boolean gl1_vswr_meas_requested[];
extern boolean  gl_hw_use_extended_tx_alpha[];
#endif /*FEATURE_VSWR*/
extern boolean gl1_deep_sleep_pending[];
...
/*If any new scenario needs to be added, it should be appended at the end, not in middle
  This is a strict requirement from MCPM team*/
typedef enum
{
  L2G_QTA_BIMC = 1,
  L1_IDLE_G2L,
  ...
  L1_DED_LOOPBACK,
  L1_DED_G2L_BIMC,
  GSM_SCENARIO_LAST = L1_DED_G2L_BIMC
} gl1_mcpm_clk_scenario;
...
typedef struct
{
  rfm_meas_common_param_type *common_rf_structure_ptr;

  uint32                     rxlm_gsm_buf_idx;
  rfm_device_enum_type       target_rf_device;
  sys_band_T                 target_band;
  boolean                    is_a_qta_gap;
  /* This is used for T2G idle in DR mode only for RF bus contention issue */
  boolean                    use_long_rx_alpha;

/* The following variable is no longer used by GL1 and will be deprecated when LTE have released CR884541*/
...
typedef struct
{
   ...
   uint32  rssi;
   uint16  sch_snr;
   boolean parallel_sch;
   boolean schedule_error;
} gl1_hw_sch_decode_data;

/* Used to identofy what type of SCH decode is required,
 * simple check for SCH enabled means keep first entry as
 * NO_SCH decode and second as CONVENTIONAL_SCH
 */
typedef enum {
  GL1_NO_SCH_DECODE           = 4,
  GL1_CONVENTIONAL_SCH_DECODE,
  X2G_CONVENTIONAL_SCH_DECODE,
  T2G_CONVENTIONAL_SCH_DECODE,
  GL1_PARALLEL_SCH_DECODE,
  X2G_PARALLEL_SCH_DECODE,
  T2G_PARALLEL_SCH_DECODE
} gl1_hw_sch_decode_t;

/* Flags for controlling RxDiversity in burst level */
#define ENABLE_RXDIVERSITY    (1 << 0)
#define ENABLE_DIVERSITY_RX   (1 << 1)
#define ENABLE_PRIMARY_RX     (1 << 2)
#define ENABLE_IMRD           (1 << 3)
#define ENABLE_MSRD           (1 << 4)
#define RXDIV_FORCED_ON       (1 << 5)
typedef struct
{
  uint16 enableRxd   :1; //Enable Rx Diversity
  uint16 enableDrx   :1; //Use Drx
  uint16 enablePrx   :1; //Use Prx
  uint16 enableIMRD  :1; //Enable Rx Diversity for Idle (IMRD)
  uint16 enableMSRD  :1; //Enable Rx Diversity for Data (MSRD)
  uint16 RxdForcedOn :1; //Keep Rx Diversity enabled always in FTM mode (NV 71588: 63 - enabled, 31(default Rxd setting) - disabled ) 
  uint16 threshold   :3; //Threshold for diff b/w Prx and Drx power
  uint16 unused      :7;
...
#define GL1_HW_CM_X2G_STARTUP_CMD_DURATION_QS(as_id)        gl1_hw_cm_x2g_startup_cmd_duration_qs(as_id)
#define GL1_HW_CM_X2G_STOP_CMD_DURATION_QS(as_id)           gl1_hw_cm_x2g_stop_cmd_duration_qs(as_id )
#define GL1_HW_CM_X2G_CLEANUP_CMD_DURATION_QS(as_id)        gl1_hw_cm_x2g_cleanup_cmd_duration_qs(as_id )

#define GL1_HW_CM_X2G_PWR_MEAS_CMD_OVERHEAD_QS(as_id)       gl1_hw_cm_x2g_power_meas_cmd_overhead_qs(as_id) // tune time
#define GL1_HW_CM_X2G_PWR_MEAS_CMD_LONG_OVERHEAD_QS(as_id)  gl1_hw_cm_x2g_power_meas_cmd_long_overhead_qs(as_id)

#define GL1_HW_CM_X2G_ACQ_CMD_OVERHEAD_QS(as_id)            gl1_hw_cm_x2g_power_meas_cmd_overhead_qs(as_id )
#define GL1_HW_CM_X2G_ACQ_CMD_LONG_OVERHEAD_QS(as_id)       gl1_hw_cm_x2g_power_meas_cmd_long_overhead_qs(as_id)

#define GL1_HW_CM_X2G_ASYNC_RX_CMD_OVERHEAD_QS(as_id)       gl1_hw_cm_x2g_power_meas_cmd_overhead_qs(as_id )
#define GL1_HW_CM_X2G_ASYNC_RX_CMD_LONG_OVERHEAD_QS(as_id)  gl1_hw_cm_x2g_power_meas_cmd_long_overhead_qs(as_id)
...
===========================================================================*/
void geran_l1_x2g_irat_enter
(
   const gl1_hw_cm_rf_info_t* irat_meas_rf_params_ptr,
   sys_modem_as_id_e_type     as_id
);
...
===========================================================================*/
extern void gl1_hw_cm_check_delay_after_qta_startup(gas_id_t gas_id);
#endif /* QTA_EVENT_DATA_LOGGING */


#if defined (FEATURE_WCDMA)
extern void gl1_hw_get_next_qta_gap_params(uint32 *start_sclks, uint32 *end_sclks , sys_modem_as_id_e_type as_id);
#endif
/*===========================================================================
...
DESCRIPTION
  Returns the X2G power measurement command overhead in quarter symbols.
...
FUNCTION gl1_hw_cm_x2g_power_meas_cmd_long_overhead_qs
...
DESCRIPTION
  Returns the X2G startup command duration in quarter symbols.
...
DESCRIPTION
  Returns the X2G cleanup command duration in quarter symbols.
...
DESCRIPTION
  Returns the X2G W PLL settling time in us
