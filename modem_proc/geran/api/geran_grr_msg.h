...
/* $Header: //components/rel/geran.mpss/7.4.0/api/geran_grr_msg.h#2 $ */
...
enum
{
...
#endif /* FEATURE_LTE */
  MSGR_DEFINE_UMID(GERAN, GRR, REQ, MTC_DRX,                  0x15, geran_grr_mtc_drx_req_t),  
  ...
  MSGR_DEFINE_UMID(GERAN, GRR, REQ, DEPRIORITIZE_GCELL,       0x1B, geran_grr_deprioritize_gcell_req_t),

  /*! @brief The last message in the enum */
  GERAN_GRR_REQ_LAST = GERAN_GRR_DEPRIORITIZE_GCELL_REQ
...
/*!@brief                        
  This structure contains the message type for GERAN_GRR_MTC_DRX_REQ, sent by
  QMI-NAS to GRR to get current DRX level
*/
typedef struct
{
  /*! @brief Message header. */
...
typedef struct
{
  ...
  sys_plmn_id_s_type     plmn_id;            /* PLMN id of the cell */
  uint16                 cell_id;            /* Cell identifier of the cell */
  uint16                 location_area_code; /* Location Area Code for the cell */
  uint32                 deprioritized_time; /* time (in milli seconds) cell to be deprioritize */
} geran_grr_deprioritize_gcell_req_t;
...
typedef union
{
  ...
  geran_grr_deprioritize_gcell_req_t  deprioritize_gcell_req;
} geran_grr_msg_req_u;
...
#endif /* FEATURE_LTE */
  MSGR_DEFINE_UMID(GERAN, GRR, RSP, MTC_DRX,                  0x13, geran_grr_mtc_drx_rsp_t),
...
/*!@brief                        
  This structure contains the message type for GERAN_GRR_MTC_DRX_RSP, sent by
  GRR to QMI-NAS in response to GERAN_GRR_MTC_DRX_REQ
*/
typedef struct
{
  /*! @brief Message header */
  ...
  /*! @brief Gives execution status of GERAN_GRR_MTC_DRX_REQ. */
  sys_get_drx_status_e_type status;

  /*! @brief Gives current DRX level */
  uint8                     drx_level;

} geran_grr_mtc_drx_rsp_t;
...
typedef union
{
  ...
  geran_grr_mtc_drx_rsp_t                  mtc_drx_rsp;
} geran_grr_msg_rsp_u;
/*
...
{
  uint8   no_of_entries; 
  uint16  bcch_arfcn[MAX_NUM_GSM_NEIGH];
} neighbour_cell_list_t;

typedef struct 
{
  msgr_hdr_struct_type            hdr;
  geran_grr_gcell_identifier_t    uid_camped_cell;
  neighbour_cell_list_t           gsm_neighbor_cell_unique_arfcn_list; 
  ...
  uint8                           si13_position;
  uint8                           MSCR;
  uint8                           cell_reselect_hysterisis;
...
#endif /* FEATURE_LTE */
  geran_grr_qmi_cell_info_ind_t            qmi_cell_info_ind;
} geran_grr_msg_ind_u;
