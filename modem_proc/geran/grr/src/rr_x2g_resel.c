...
*/
/* $Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_x2g_resel.c#3 $ */
/* $DateTime: 2020/01/27 07:04:09 $$Author: pwbldsvc $ */
            ...
            case RR_X2G_RESEL_IMSG:
            {
              switch (message->message_header.message_id)
              {
                case RR_X2G_RESEL_IMSG_L1_ACTIVATED_IND:
                {
                  ...
                  bsic_info.bsic = rr_x2g_resel_data_ptr->target_bsic;
                  bsic_info.mismatch_action = RR_CELL_ACQ_BSIC_MISMATCH_ACTION_STOP;
                  bsic_info.send_to_l1 = TRUE;
              ...
              if (plmn_suitable)
              {
                ...
                else
                {
                  ...
                  rr_log_reselect_success(
                    rr_gprs_get_pcell_info(gas_id)->gsm.BCCH_ARFCN,
                    rr_gprs_get_pcell_info(gas_id)->gsm.BSIC,
                    gas_id
                  );
...
QSH_MDUMP_FN_ATTR
void grr_mdump_rr_x2g_resel(const gas_id_t gas_id)
{
  uint8 as_index = GERAN_MAP_GAS_ID_TO_DATA_SPACE_INDEX(gas_id);
  qsh_mdump_collect_high(&rr_x2g_resel_data_ptrs[as_index], sizeof(rr_x2g_resel_data_t*));
