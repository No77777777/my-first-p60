...
============================================================================*/
/* $Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_select_bcch_if.h#1 $ */
/* $DateTime: 2019/06/06 23:27:31 $ */

/** 
  @file rr_select_bcch_if.h
  @brief This header file specifies the RR IMSG/OMSG signalling interface for the RR-SELECT-BCCH module
    
  Also see the associated module header file, rr_select_bcch.h. This contains other external functions which
  are required for the module to function correctly, such as the initialisation function main state machine
  message input function.
    
  External functions are provided to generate the RR-SELECT-BCCH IMSGs that this module understands, and the
  RR-SELECT-BCCH OMSGs to be processed by the parent module.
 */
...
/** 
  ...
  RR_SELECT_BCCH_EARLY_CAMP_SI3_ALWAYS : Eary-camp when SI3 received, even if the last SI received.
...
typedef enum
{
  ...
  RR_SELECT_BCCH_EARLY_CAMP_SI3_ALWAYS,
...
typedef struct
{
  boolean dont_deact_pend_store_when_finished;
} rr_select_bcch_csi_params_t;
...
typedef struct
{
  ...
  uint16                                   si_already_received_mask;
...
/** 
  ...
  @brief Union of all the RR-SELECT-BCCH IMSGs which can be sent to rr_select_bcch_control 
 */ 
typedef union
{
  rr_message_header_t                         header;
...
/** 
  ...
  @brief Union of all the RR-SELECT-BCCH OMSGs which can be output by rr_select_bcch_control 
 */ 
typedef union
{
  rr_message_header_t                      header;
...
/** 
  ...
  This message instructs the RR-SELECT-BCCH module to perform a reselection power-scan to verify the presence
  of a cell, select it, and acquire the indicated SI messages.
   
  @param arfcn
  @param cell_suitable_cb_fn  Call-back function to be called when SI3 is received. If this call-back returns
                              FALSE, then the cell is deemed not suitable and a fail indication will be
                              output. Set to NULL if not required.
  @see rr_cell_suitable_cb_fn_t 
  @param notify_on_si_mask    When the indicated SIs have been received, a notification indication is sent out
                              (but this does not stop the procedure). Set to MASK_SI_NONE if not required.
  @param wait_for_si_mask     When the indicated SIs have been received, the procedure completes. This can be
                              set to MASK_SI_NONE, but if early camping based on SI3 is required, then this is
...
/** 
  ...
  This message instructs the RR-SELECT-BCCH module to perform a reselection power-scan over a list of supplied 
  ARFCNs, calling the supplied cell suitability call-back function to determine which cell to pick. After a 
  suitable cell is found, it is selected and the indicated SIs aquired. 
  ...
  @param notify_on_si_mask    When the indicated SIs have been received, a notification indication is sent out
                              (but this does not stop the procedure). Set to MASK_SI_NONE if not required.
  @param wait_for_si_mask     When the indicated SIs have been received, the procedure completes. Note: This 
                              can be set to MASK_SI_NONE, but it is always internally modified to add SI3.
...
/** 
  ...
  This message instructs the RR-SELECT-BCCH module to select the specified cell, and acquire the indicated SI 
  messages. Note: It is assumed that L1 is not tuned to the cell, and a MPH_SELECT_SPECIFIC_BCCH_REQ will be 
  sent to L1 to do this. If L1 is already tuned to the cell, then use RR_SELECT_BCCH_IMSG_ACQUIRE_SI_REQ 
  instead. 
  ...
  @param cell_suitable_cb_fn  Call-back function to be called when SI3 is received. If this call-back returns 
                              ...
                              output. Set to NULL if not required.
  @see rr_cell_suitable_cb_fn_t 
  @param early_camp_required  The early camping mode required.
  @param notify_on_si_mask    When the indicated SIs have been received, a notification indication is sent out
                              (but this does not stop the procedure). Set to NULL if not required.
  @param wait_for_si_mask     When the indicated SIs have been received, the procedure completes. This can be 
                              set to MASK_SI_NONE, but if early camping based on SI3 is required, then this is
                              internally modified to add SI3.
 */
...
/** 
  ...
  selected cell. Note: It is assumed that L1 is already tuned to the cell (i.e. L1 has just left dedicated 
  state, or a MPH_SELECT_SPECIFIC_BCCH_REQ has previously been sent) and SIs are being reaed. If this is not 
  the case, then use RR_SELECT_BCCH_IMSG_SELECT_REQ instead. 
  
  @param arfcn                ARFCN of the selected cell
  @param rxlev_average        Latest RXLEV (average) of the selected cell
  ...
  @param wait_for_si_mask     When the indicated SIs have been received, the procedure completes. This can be 
                              set to MASK_SI_NONE, but if early camping based on SI3 is required, then this is
                              internally modified to add SI3
  @param init_params_ptr      Initial parameters (optional).Set to NULL if not required. 
  @param csi_params_ptr       CSI parameters (optional). Set to NULL if not required. 
...
extern void rr_select_bcch_send_imsg_acquire_si_req(
  ...
  rr_select_bcch_csi_params_t *csi_parmas_ptr,
  uint32 max_allowed_time_ms,
  const gas_id_t gas_id
);
 ...
 */
extern void rr_select_bcch_send_imsg_abort_req(const gas_id_t gas_id);


/** 
  @brief Called to inform RR-SELECT-BCCH module that the indicated SI message has been received (and the 
         contents decoded and stored)
  
  @param si_received 
 */
extern void rr_select_bcch_send_imsg_bsi_received_ind(rr_si_type_t si_received, const gas_id_t gas_id);


/** 
  @brief Call to send RR_SELECT_BCCH_OMSG_SELECT_CNF 
   
  The message indicates that the requested cell has been successfully selected, and specified SIs have been 
  acquired 
...
/** 
  ...
  This message indicates that the requested cell could not be selected, or that the specified SIs could not be
  acquired 
...
/** 
  ...
  This message indicates that the SIs specified in notify_si_mask in the original request have been acquired 
   
  @param arfcn 
  @param bsic
 */
extern void rr_select_bcch_send_omsg_si_notify_ind(ARFCN_T arfcn, BSIC_T bsic, const gas_id_t gas_id);

/*!
 * \brief Function which can be used to initialise the Initial parameters structure to default values.
 * 
 * \param init_params_ptr (out) - Initial parameters structure
 */
extern void rr_select_bcch_init_params_init(rr_select_bcch_init_params_t *init_params_ptr);

/*!
 * \brief Function which can be used to initialise the CSI parameters structure to default values.
 * 
 * \param csi_params_ptr (out) - CSI parameters structure
 */
extern void rr_select_bcch_csi_params_init(rr_select_bcch_csi_params_t *csi_params_ptr);
