...
*/
/* $Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_sys_info_si.c#4 $ */
...
)
{
  ...
  /* Update the required SI bitmask based on current BA list validity */
  ...
  send_mph_read_sys_info_req(
    rr_si_get_remaining_si_bitmask(scell_info),
  ...
  // Inform Multi SIM Controller about start of Optional SI reading on this Cell
  rr_ms_sys_info_status_update(RR_SYS_INFO_ACQ_OPTIONAL_SI, gas_id);
...
#if defined(FEATURE_DUAL_SIM) || defined(FEATURE_TRIPLE_SIM)
  // Inform Multi SIM Controller about stop of SI reading on this Cell
...
)
{
  ...
  */
  ...
  {
    uint16 not_required_mask = SI_TYPE_TO_MASK(last_si);  
    cell_info->gsm.update_message_received |= not_required_mask;
    cell_info->gsm.update_message_received |= cell_info->bcch_sys_info.si_messages_not_broadcast;	
  }
...
)
{
  ...
  Handler is inactive
  */
  rr_event = rr_gprs_bsi_acquisition_control(EV_SI_SYS_INFO_MESSAGE_RECEIVED, NULL, gas_id);
  /*
...
)
{
  ...
  if ( scell_info->bcch_sys_info.si13_data_valid_flag )
  {
    ...
    if ( messages_changed != MASK_SI_NONE )
    {
      ...
      {
        MSG_GERAN_HIGH_0_G("SI13 changed flush the messages n/w wants UE to update");
        /* Flush SI messages which need to be updated */
        rr_csi_flush_multiple_si_data(scell_info->gsm.BCCH_ARFCN,                    // ARFCN
                                      messages_changed,                              // SI_MASK
                                      &scell_info->gsm.location_area_identification, // LAI
                                      &scell_info->gsm.cell_identity,                // CI
  ...
  */
  ...
  else
  {
    ...
    for (i = 0; i < MAX_INSTANCES; i++)
    {
      ...
      if (!si2quater_instance || !si2quater_instance->valid_data)
      {
        continue;
      }
...
)
{
  ...
  rr_sys_info_si_data_ptr = rr_sys_info_si_get_data_ptr(gas_id);
  RR_NULL_CHECK_FATAL(rr_sys_info_si_data_ptr);
  MSG_GERAN_MED_0_G("inside bsi_acquisition_control ");
  /*****************************************************************/
  /* Perform state-independent common processing on incoming event */
  /*****************************************************************/

  if (grr_get_l1_in_test_mode(gas_id) && (event_in != SIEV_STOP_AND_RESET_SOFTWARE))
  {
    ...
    switch ( rr_sys_info_si_data_ptr->state )
    {
      ...
      case BSIST_ACQ_PAGING_REORG:
      {
        ...
        if ( event_in == SIEV_SYS_INFO_MESSAGE_RECEIVED )
        {
          ...
          if (scell_info != NULL)
          {
            if (SI_TYPE_SI_3 == scell_info->bcch_sys_info.last_si_message_received)
            {
#if defined(FEATURE_DUAL_SIM) || defined(FEATURE_TRIPLE_SIM)            
              geran_api_set_tuneaway_blocked(GERAN_API_CLIENT_PAGE, FALSE, geran_map_gas_id_to_nas_id(gas_id));
#endif /*FEATURE_DUAL_SIM || FEATURE_TRIPLE_SIM*/
      ...
      case BSIST_ACQ_REQUESTED:
      {
        ...
        {
          gprs_scell_info_t * cell_info = rr_gprs_get_scell_info(gas_id);
          uint16 remaining_si = rr_si_get_remaining_si_bitmask( cell_info );
          ...
          if ( remaining_si & MASK_SI_13 )
          {
            ...
            if (rr_sys_info_timeout_due_to_si13_not_read(gas_id))
            {
              ...
              (void)check_if_bsi_acquisition_complete(gas_id);
              new_event = SIEV_SYS_INFO_ACQUISITION_COMPLETE; 
            }
            else
            {
...
)
{
...
#if defined (FEATURE_WCDMA) || defined (FEATURE_GSM_TDS)
  {
    ...
    if (si2ter_inst_ptr != NULL)
    {
      ...
      for (i = 0; i < MAX_SI2TER_INSTANCES; i++)
      {
        ...
        if (!si2ter_ptr || !si2ter_ptr->valid_data)
        {
          continue;
        }
...
#ifdef FEATURE_LTE
  if (rr_create_lte_neighbour_list(&cell->gsm.lte_neighbour_list, si2q_inst_ptr ))
    {
      cell->lte_neighbour_list = cell->gsm.lte_neighbour_list;

      if (rr_nv_increase_lte_cell_priority_enabled(RR_GAS_ID_TO_AS_ID) && rr_is_geran_priority_higher_than_lte(cell))
      {
         MSG_GERAN_HIGH_0("Increasing LTE cell priority over GERAN");
         rr_increase_lte_priority_over_geran(cell, gas_id);
      }

#ifdef FEATURE_FAST_G2L_PSEUDO_SI2Q
      if (rr_lte_neighbour_list_has_valid_earfcn(gas_id))
      {
  ...
  if (rr_timer_is_running(RR_PRIORITIZE_LTE_ABORT_TIMER, gas_id))
  {
    ...
    {
      MSG_GERAN_MED_0_G("[LTE Prio] Prioritizing LTE frequencies");
  ...
  {
  /* If pseudo SI2q data was used earlier, revaluate it again */
    rr_set_rr_using_pseudo_si2q_data(FALSE, gas_id);
  ...
  {
    MSG_GERAN_HIGH_0_G("Restoring serving cell priority for collision fallback");
    rr_override_scell_si2q_params( cell, gas_id);
  }
...
)
{
  ...
  if ((cell != NULL) && (si2q_inst_ptr != NULL))
  {
    ...
    for (i = 0; i < MAX_INSTANCES; i++)
    {
      gprs_bsic_description_instance_t *instance;
...
#ifdef FEATURE_LTE
      instance = si2q_inst_ptr->instance[i]->si2q_mi.si2q_params.gprs_bsic_description;
 ...
 * \return uint16
 */
uint16 rr_si_get_remaining_si_bitmask(
  gprs_scell_info_t *cell_info
)
{
...
} /* rr_si_get_remaining_si_bitmask() */
 ...
 * \param si_bitmask (in)
 */
void rr_si_print_bitmask(uint16 si_bitmask)
{
//#ifdef PERLUTF
#if 1
