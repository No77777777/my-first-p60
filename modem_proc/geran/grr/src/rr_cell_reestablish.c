...
*/
/* $Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_cell_reestablish.c#5 $ */
/* $DateTime: 2020/10/21 01:49:42 $$Author: pwbldsvc $ */
      ...
      case RR_REEST_NULL:
      {
        switch (event_in)
        {
          ...
          case EV_CHOOSE_CELL_FOR_CAMP:
          {
            ...
            /* Inform Multi-SIM Controller about Cell Reestablishment Service Recovery */
            rr_ms_service_recovery_started(RR_SERVICE_RECOVERY_CELL_REESTABLISHING, gas_id);
                      ...
                      case RR_SELECT_BCCH_OMSG_SELECT_CNF:
                      {
                        ...
                        else
                        {
                          ...
                          else
                          {
                            ...
                            /*invalidate power scan to ensure acq DB scan is performed during cell selection*/
              ...
              case MS_MM_RR:
              {
                switch (message_id)
                {
                  ...
                  case RR_ABORT_REQ:
                  {
                    ...
                    // Abort the ongoing search for a cell suitable for re-establishment
                  ...
                  case RR_SELECT_BCCH_OMSG:
                  {
                    switch (message_ptr->rr.select_bcch.omsg.header.imh.message_id)
                    {
                      ...
                      case RR_SELECT_BCCH_OMSG_SELECT_FAIL_IND:
                      {
                        ...
                        {
                          event_out = EV_G2L_BLIND_REDIR_REQUIRED;
                          // If there is a RR_EST_REQ saved, check the est req cause and proceed further.
                          if (rr_reest_discard_rr_est_req(gas_id))
                          {
                            /* If Est req cause is for LAU, then abort EST REQ and process with redirection else cell selection */
                            if (rr_reest_data_ptr->rr_est_req.establishment_cause == (uint8) MM_RR_LOCATION_UPDATE)
                            {
                              rr_send_abort_ind(OTHER_REASON, MM_RR_OTHER_FAILURE, gas_id);
                            }
                            else
...
#endif
        ERR_GERAN_FATAL_1_G("Unknown state (%d)",(int)rr_reest_data_ptr->state);
        break;
      }
    }
...
#ifdef FEATURE_QSH_MDUMP
QSH_MDUMP_FN_ATTR
void grr_mdump_rr_reest(const gas_id_t gas_id)
{
  rr_reest_api_data_t *rr_reest_api_data_ptr = rr_reest_api_get_data_ptr(gas_id);
  uint8 as_index = GERAN_MAP_GAS_ID_TO_DATA_SPACE_INDEX(gas_id);

  qsh_mdump_collect_high(&rr_reest_data_ptrs[as_index], sizeof(rr_reest_data_t*));
  
  if (rr_reest_data_ptrs[as_index] != NULL)
  {
    qsh_mdump_collect_high(rr_reest_data_ptrs[as_index], sizeof(rr_reest_data_t));
  }

  qsh_mdump_collect_high(rr_reest_api_data_ptr, sizeof(rr_reest_api_data_t));
}
