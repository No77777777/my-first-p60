...
$Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_control.h#2 $ $DateTime: 2020/01/27 04:29:28 $ $Author: pwbldsvc $
...
typedef enum
{
  ...
  RR_X2G_REDIRECTION_ABORT_BY_RRC,
...
extern boolean rr_plmn_search_in_progress(const gas_id_t gas_id);
...
/**
  @brief Determine if G2W reselection/redirection is currently allowed by NAS.

  G2L reselection/redirection may be disallowed if the WCDMA network rejected UE registration
  via reject cause 15 on H3G/TIM network.

  @return TRUE if G2W reselection/redirection is allowed, FALSE otherwise.
*/
extern boolean rr_g2w_resel_redir_allowed(const gas_id_t gas_id);
...
#ifdef FEATURE_WCDMA
/**
  @brief Determine if G2L redirection is currently allowed by NAS.

  G2L redirection may be disallowed if the LTE network rejected UE registration
  via reject cause 15.

  @return TRUE if G2L redirection is allowed, FALSE otherwise.
*/
extern boolean rr_g2l_redir_allowed(const gas_id_t gas_id);
#endif /* FEATURE_WCDMA */
...
*/
extern boolean rr_is_csfb_call_in_progress(const gas_id_t gas_id);

/**
  @brief Set the flag which confims whether a redirection happened after CSFB call 

*/

extern void rr_set_redir_after_csfb_call(const gas_id_t gas_id);

/**
  @brief ResSet the flag which confims whether a redirection happened after CSFB call 

*/

extern void rr_reset_redir_after_csfb_call(const gas_id_t gas_id);

/**
  @brief Indicates whether a redirection happened after CSFB call 

  @return TRUE if a redir after csfb is happening FALSE if not.
*/

extern boolean rr_is_redir_after_csfb_call(const gas_id_t gas_id);
...
/**
  @brief Determines if GRR can process rr_procedure_ind

  @return TRUE/FALSE
*/
extern boolean rr_can_process_rr_procedure_ind(rr_cmd_bdy_type *msg_ptr, const gas_id_t gas_id);
#endif /* FEATURE_DUAL_SIM || FEATURE_TRIPLE_SIM */

extern boolean rr_check_for_si13_reception(uint8 message_type, uint8 tc, gprs_scell_info_t *cell_info_ptr , const gas_id_t gas_id);

extern boolean rr_check_for_optional_sI2bis_SI2ter_reception(uint8 message_type, uint8 tc, gprs_scell_info_t *cell_info_ptr , const gas_id_t gas_id);

extern boolean rr_sys_info_timeout_due_to_si13_not_read(const gas_id_t gas_id);

extern word rr_sys_info_timeout_due_to_SI2bis_SI2ter_SI13_not_read(const gas_id_t gas_id);

extern void rr_act_on_for_SI_nt_rcvd(word  rr_si_nt_rcvd, const gas_id_t gas_id);

extern void  rr_switch_to_CS_only_mode(const gas_id_t gas_id);

extern void rr_update_SI2bis_not_rcvd(const gas_id_t gas_id);

extern void rr_update_SI2ter_not_rcvd(const gas_id_t gas_id);

extern void rr_increment_tc_read_count_for_optional_SI2bis_SI2ter_SI13(uint8 message_type, const gas_id_t gas_id);

extern void rr_decrement_read_count_for_optional_SI2bis_SI2ter_SI13(const gas_id_t gas_id);

extern void rr_reset_tc_read_count_for_optional_SI2bis_SI2ter_SI13(uint8 message_type,const gas_id_t gas_id);

extern uint8 rr_get_tc_0_4_read_count_fr_si13(const gas_id_t gas_id);

extern uint8 rr_get_tc_5_read_count_fr_SI2bis(const gas_id_t gas_id);

extern uint8 rr_get_tc_4_5_read_count_fr_SI2ter(const gas_id_t gas_id);
...
/**
  ...
  @return None
*/
extern void rr_set_csfb_call_status_ind(const gas_id_t gas_id);
/**
  @brief This function reset csfb call status ind parameter in control data structure.
  @param
  @return None
*/
extern void rr_reset_csfb_call_status_ind(const gas_id_t gas_id);
#endif /* FEATURE_LTE */

#ifdef FEATURE_QSH_MDUMP
QSH_MDUMP_FN_ATTR
extern void grr_mdump_rr_control(const gas_id_t gas_id);
QSH_MDUMP_FN_ATTR
extern void grr_mdump_rr_state(const gas_id_t gas_id);
