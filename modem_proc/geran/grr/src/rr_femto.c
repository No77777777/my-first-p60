...
============================================================================*/
/* $Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_femto.c#3 $ */
 ...
 */
static boolean rr_femto_disc_select_next_cell(const gas_id_t gas_id)
{
  boolean cell_found;
...
/**
  ...
  @param msg_ptr
 */
void rr_femto_control(rr_cmd_bdy_type *msg_ptr, const gas_id_t gas_id)
{
  RR_NULL_CHECK_FATAL(msg_ptr);
              ...
              switch (msg_ptr->rr.decode_bcch.imsg.header.imh.message_id)
              {
                case RR_DECODE_BCCH_OMSG_FINISHED_IND:
                {
                  ...
                  if (num_cells > 0)
                  {
                    ...
                    (void) rr_femto_disc_select_next_cell(gas_id);
            ...
            case RR_SELECT_BCCH_OMSG:
            {
              switch (msg_ptr->rr.select_bcch.imsg.header.imh.message_id)
              {
                case RR_SELECT_BCCH_OMSG_SELECT_CNF:
                {
                  ...
                  if (rr_femto_disc_select_next_cell(gas_id))
                  {
