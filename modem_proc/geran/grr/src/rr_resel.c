...
============================================================================*/
/* $Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_resel.c#3 $ */
...
*/
rr_resel_algo_e rr_resel_algo(
  const gas_id_t gas_id
)
{
  rr_resel_data_t *rr_resel_data_ptr = rr_resel_get_data_ptr(gas_id);
  boolean irat_check                 = FALSE;
  uint8	i;
  RR_NULL_CHECK_FATAL(rr_resel_data_ptr);

  /* Spec 45.008 Section 6.6.6 Agorithm for inter-RAT cell re-selection based on priority information
    A mobile station supporting E-UTRAN shall support priority based inter-RAT cell re-selection 
    towards all the supported RATs.
    A mobile station not supporting E-UTRAN and supporting UTRAN and supporting priority based 
    reselection from UTRAN to GERAN (see 3GPP TS 25.306) shall support priority based inter-RAT
    cell re-selection towards UTRAN. */

#ifdef FEATURE_LTE
  irat_check = rr_is_lte_supported(gas_id);
#endif /* FEATURE_LTE */

#ifdef FEATURE_WCDMA
  irat_check = irat_check || rrc_rr_absolute_priority_support(RR_GAS_ID_TO_AS_ID);
#endif /* FEATURE_WCDMA */

#ifdef FEATURE_GSM_TDS
    irat_check = irat_check || tdsrrc_rr_absolute_priority_support(RR_GAS_ID_TO_AS_ID);
#endif /* FEATURE_GSM_TDS */
  ...
  if (irat_check &&
      ...
      rr_resel_data_ptr->inhibit_pri_resel == FALSE)
  {
...
)
{
  ...
  if (!rr_gapi_is_supported_band((sys_band_T)GET_ARFCN_BAND(arfcn), gas_id))
  {
    MSG_GERAN_MED_1_G("BA entry=%d not included (unsupported band)",(int) arfcn.num);
    return;
  }
 ...
 )
{
  ...
  num_frequencies_to_find   = (uint8) MIN(rr_resel_data_ptr->rps_data.pscan_cell_list.no_of_entries, MAX_NO_OF_RPS_CELLS_TO_TRY_PER_CYCLE);
  /* old serving cell where SNR happened, if any */
  channel_number            = rr_resel_data_ptr->rr_gprs_old_scell.reselection_data.BCCH_ARFCN;
  snr_reselection_triggered = rr_resel_data_ptr->rr_gprs_old_scell.snr_reselection_triggered;
  no_of_entries             = rr_pscan_results_db_ptr->pscan_results.num_frequencies;

  /* Put the last serving cell at the bottom of the list if that cell was left 30 seconds ago and it was due to SNR based reselection */
  /* Old scell to put at the bottom of the list of POWER scan database, if RR_SERV_CELL_BAD_SNR_TIMER (30secs) timer is running */
  if ((channel_number.num != INVALID_ARFCN) && (rr_timer_is_running(RR_SERV_CELL_BAD_SNR_TIMER, gas_id) == TRUE))
  {
    /* Check if old_scell where SNR happend is in the POWER_SCAN_CNF list*/
    for (pscanIndex = 0; (pscanIndex < no_of_entries) && (pscanIndex < RR_MAX_PSCAN_FREQ_SCAN_ARRAY_SIZE); pscanIndex++ )
    {
      if (ARFCNS_EQUAL((rr_resel_data_ptr->rr_gprs_old_scell.reselection_data.BCCH_ARFCN),(rr_pscan_results_db_ptr->pscan_results.meas[pscanIndex].arfcn)))
      {
        /* if the old scell is already at the bottom of pscan result db, Don't need to do anything */
        if (pscanIndex == (no_of_entries-1))
        {
          /* done with loop */
          break;
        }

        /* old scell moved to bottom of the pscan result db */
        rr_pscan_rearrange_freq_in_list(
...
{
  boolean send_bcch_decode_list_req;
  ...
  // Allocate temporary storage for the frequency list
  bcch_freq_list_ptr = GPRS_MEM_CALLOC(sizeof(rr_l1_idle_freq_list_T));

  RR_NULL_CHECK_RETURN_PARAM(bcch_freq_list_ptr, FALSE);

  // Start with the assumption that the list will be empty
  send_bcch_decode_list_req = FALSE;
  ...
  */
  for (best_six_index=0;best_six_index<best_six_ncells_ptr->no_of_entries;best_six_index++)
  {
    if (best_six_ncells_ptr->ncell_entry[best_six_index].ncell_data->ncell_data.bcch.gsm.valid_data_flag == FALSE )
    {
      ...
      bcch_freq_list_ptr->arfcn[bcch_freq_list_ptr->num_frequencies++] =
  ...
  */
  if (send_bcch_decode_list_req)
  {
    rr_candidate_db_set_valid(candidate_cell_db);

    rr_decode_bcch_send_start_ind(
      RR_SEARCH_RESELECTION,   // bcch_decode_type
      FALSE,                   // si3_only
      bcch_freq_list_ptr,      // bcch_freq_list_ptr
      gas_id                   // gas_id
    );
  }
...
)
{
  ...
  */
  ...
  /* Stop the Serving cell bad SNR timer, if running*/
  (void)rr_gprs_stop_serving_cell_bad_snr_timer(gas_id);
...
#ifdef FEATURE_LOCK_ARFCN
  if (rr_is_ms_locked_to_channel(gas_id))
  {
      MSG_GERAN_HIGH_0_G("MS is locked always trigger resel fail so that cell selection get triggered.");
     rr_resel_data_ptr->rr_gprs_target_cell= NULL;
  }
  else
#endif /*FEATURE_LOCK_ARFCN*/
  {
...
)
{
  ...
  /**************************************************************/
  {
    ...
    pcell_info->gsm.complete_BA_list.BA_IND       |= INVALID_BA_IND; /* add the flag */
    pcell_info->gsm.SI2_BA_list.BA_IND            |= INVALID_BA_IND; /* add the flag */
    pcell_info->gsm.SI2bis_BA_list.BA_IND         |= INVALID_BA_IND; /* add the flag */
    pcell_info->gsm.SI2ter_BA_list.BA_IND         |= INVALID_BA_IND; /* add the flag */
  }
...
#else
/**
  @brief Returns the reselection state machine's current state.
*/
uint32  rr_resel_current_state(
  const gas_id_t gas_id
)
{
  rr_resel_data_t *rr_resel_data_ptr = rr_resel_get_data_ptr(gas_id);

  return rr_resel_data_ptr->state;
}
...
)
{
  ...
  PLMN_id_T               *plmn_id_ptr       = NULL;
  rr_cell_acq_bsic_info_t *bsic_info_ptr     = NULL;
  ...
  if (bcch_ncell_info_ptr != NULL)
  {
    ...
    bsic_info.bsic = bcch_ncell_info_ptr->gsm.BSIC;
    bsic_info.mismatch_action = RR_CELL_ACQ_BSIC_MISMATCH_ACTION_REQUIRE_SI3;
    bsic_info.send_to_l1 = FALSE;

    // Update the pointer from NULL to pass into API below
    bsic_info_ptr = &bsic_info;
  };
...
)
{
  ...
  {
    MSG_GERAN_HIGH_2_G(" rr_resel_greatest_gsm_s_value(TRUE, gas_id) = %d, THRESH_gsm_low == %d", rr_resel_greatest_gsm_s_value(TRUE, gas_id), (thresh_gsm_low * 2) );
...
)
{
  ...
  {
    MSG_GERAN_LOW_0_G("Stopped T3174");
  }
  ...
  rr_resel_data_ptr->rr_gprs_old_scell.snr_reselection_triggered = FALSE;
...
)
{
  ...
  pcell_ptr->gsm.complete_BA_list.BA_IND        = INVALID_BA_IND;
  pcell_ptr->gsm.SI2_BA_list.BA_IND             = INVALID_BA_IND;
  pcell_ptr->gsm.SI2bis_BA_list.BA_IND          = INVALID_BA_IND;
  pcell_ptr->gsm.SI2ter_BA_list.BA_IND          = INVALID_BA_IND;
}
...
)
{
  ...
  else if (rr_resel_data_ptr->state == RR_RESEL_STATE_PCCO && rr_cell_acq_pending_mac_suspension(gas_id))
  {
    MSG_GERAN_MED_0_G("PCCO based  reselection is pending");
    return TRUE;
  }  
  else if (rr_resel_data_ptr->state == RR_RESEL_STATE_RPS)
  {
    MSG_GERAN_MED_0_G("RPS reselection is pending");
    return TRUE;
  }  
  else
  {
  ...
  while (event != EV_NO_EVENT)
  {
                ...
                switch (message->rr.header.rr_message_set)
                {
                  case RR_RESEL_IMSG:
                  {
                    switch (message->rr.resel.imsg.header.imh.message_id)
                    {
                      ...
                      case RR_RESEL_IMSG_IRAT_ALLOWED_UPDATED_IND:
                      {
                        /* If G2L reselection is now disallowed then any
                        running G2L timers must be stopped, and LTE neighbour
                        cell measurements cleared. Do the same for wcdma as well. */
#ifdef FEATURE_LTE
...
#endif /* FEATURE_WCDMA */                        
                        break;
                      } /* RR_RESEL_IMSG_IRAT_ALLOWED_UPDATED_IND */
                  ...
                  case MAC_GRR_DATA_IND:
                  {
                    ...
                    switch (mac_grr_data_ind->data_ptr.header_ptr->message_type)
                    {
                      case PACKET_CELL_CHANGE_ORDER:
                      {
...
#ifdef FEATURE_QSH_EVENT_NOTIFY_TO_QSH 
                        rr_qsh_event_notify(RR_QSH_EVENT_PCCO, gas_id);
                        ...
                        {
                           MSG_GERAN_HIGH_0_G("MS locked No PCCO possible");
                           rr_gprs_send_pccf(ANONYMOUS_ACCESS, gas_id);
                        }
#endif /*FEATURE_LOCK_ARFCN*/
                      }
                      break;
...
#endif /* FEATURE_WCDMA || FEATURE_GSM_TDS || FEATURE_LTE */
#ifdef FEATURE_LOCK_ARFCN
                && !(rr_is_ms_locked_to_channel(gas_id))
#endif /*FEATURE_LOCK_ARFCN*/
                )
            {
              rr_resel_algo_e resel_algo = rr_resel_algo(gas_id);
              ...
              switch (resel_algo)
              {
                ...
                case RR_RESEL_ALGO_PRIORITY_BASED:
                {
                  if (rr_irat_check_for_priority_based_reselection(
                        ...
                        rr_timer_is_running(RR_PRIORITIZE_LTE_ABORT_TIMER, gas_id),
                        gas_id
                      ))
                  {
                    rr_resel_data_ptr->resel_type = RESEL_G2W_NORMAL;
...
#ifdef FEATURE_QSH_EVENT_NOTIFY_TO_QSH 
              if (new_event == EV_CELL_RESELECTION)
              {
                rr_qsh_event_notify(RR_QSH_EVENT_RESEL, gas_id);
              }
#endif // FEATURE_QSH_EVENT_NOTIFY_TO_QSH  
            }
            else
          ...
          case EV_INPUT_MESSAGE:
          {
            switch (message_set)
            {
              case MS_RR_RR:
              {
                switch (message->rr.header.rr_message_set)
                {
                  // Pass thru internal messages required by RR-CELL-ACQ
                    ...
                    switch (message->rr.cell_acq.omsg.header.imh.message_id)
                    {
                      case RR_CELL_ACQ_OMSG_FINISHED_IND:
                      {
                        ...
                        switch (message->rr.cell_acq.omsg.finished_ind.cell_acq_status)
                        {
                          ...
                          {
                            /* Log camping attempt failure event */
                ...
                switch (message_id)
                {
                  case MID_TIMER_EXPIRY:
                  {
                    ...
                    switch (timer_expiry->timer_id)
                    {
                      ...
                      {
#ifdef FEATURE_QSH_EVENT_NOTIFY_TO_QSH 
                        if(timer_expiry->timer_id == RR_GSM_SYS_INFO_TIMER)
                        {
                      ...
                      case CELL_RESELECTION_TIMER:
                      {
                        ...
                        if (MASK_SI_NONE != rr_sys_info_timeout_due_to_SI2bis_SI2ter_SI13_not_read(gas_id))
                        {
                          rr_cell_acq_control(EV_INPUT_MESSAGE, message, gas_id);
                        }
                        else
                  ...
                  case RR_CELL_ACQ_OMSG:
                  {
                    switch (message->rr.cell_acq.omsg.header.imh.message_id)
                    {
                      ...
                      case RR_CELL_ACQ_OMSG_RESELECTED_IND:
                      {
                        ...
                        pncd_enough_to_camp = rr_gprs_is_pncd_enough_to_camp(gas_id);

                        rr_cell_acq_send_imsg_nacc_acq_status_ind(
                          pncd_enough_to_camp,
                          rr_gprs_get_pcell_info(gas_id)->gsm.update_message_received,
                          gas_id
                        );

                        if (!pncd_enough_to_camp)
                        {
                      ...
                      case RR_RESEL_IMSG_ABORT_REQ:
                      {
                        ...
                        switch (message->rr.resel.imsg.abort_req.reason)
                        {
                          ...
                          case RR_RESEL_ABORT_REASON_MM_REQ:
                          {
                            if (rr_timer_cancel(T3174, gas_id))
                            {
                              MSG_GERAN_HIGH_0_G("T3174 stopped");
                            }
                      ...
                      case PACKET_CELL_CHANGE_ORDER:
                      {
                        ...
                        */
                        ...
                        if (rr_gmminfo_get_gmm_status(gas_id) == GMM_READY_STATE)
                        {
                          ...
                          {
                            /* This check ensures PCCO received with PNCD received immediately is ignored; 
                            and the first one is not completely processed ( i.e. PS access completely disabled) */

                            if(rr_resel_data_ptr->nacc_pcco_received == FALSE)
                            {
                              rr_resel_data_ptr->state = RR_RESEL_STATE_PCCO;
                            }
                          }
          ...
          case RR_RESEL_RPS_IDLE:
          {
            switch ( event )
            {
              ...
              case EV_PS_ACCESS_BLOCKED:
              {
                ...
                if (rr_is_ms_locked_to_channel(gas_id))
                {
                   MSG_GERAN_HIGH_0_G("MS locked ,Trigger force cell selection");
                   rr_resel_data_ptr->rps_data.pscan_cell_list.no_of_entries =0;
                }
#endif /*FEATURE_LOCK_ARFCN*/
                if(rr_resel_data_ptr->rps_data.pscan_cell_list.no_of_entries != 0)
                      ...
                      case RR_CELL_ACQ_OMSG:
                      {
                        switch (message->rr.cell_acq.omsg.header.imh.message_id)
                        {
                          case RR_CELL_ACQ_OMSG_FINISHED_IND:
                          {
                            ...
                            rr_cell_acq_status_e rr_cell_acq_status = message->rr.cell_acq.omsg.finished_ind.cell_acq_status;
                            MSG_GERAN_HIGH_1_G("RR_CELL_ACQ_OMSG_FINISHED_IND status:0x%x rxd",rr_cell_acq_status);
                            ...
                            switch(rr_cell_acq_status)
                            {
                              ...
                              default:
                              {
                                ...
                                /* Log camping attempt failure event */
                                if(RR_CELL_ACQ_STATUS_FAILURE_BCCH_NOT_FOUND == rr_cell_acq_status)
                                {
                                  rr_log_camping_attempt_failure(RR_LOG_CAMPING_FAILURE_BCCH_NOT_FOUND, gas_id);
                                }
                                else if(RR_CELL_ACQ_STATUS_FAILURE_BSIC_MISMATCH == rr_cell_acq_status)
                                {
                                  /* TODO: Matching RR_LOG_CAMPING_FAILURE enum to be defined? */
                                  rr_log_camping_attempt_failure(RR_LOG_CAMPING_FAILURE_BCCH_NOT_FOUND, gas_id);
                                }
                  ...
                  case RR_CELL_ACQ_OMSG:
                  {
                    switch (message->rr.cell_acq.omsg.header.imh.message_id)
                    {
                      ...
                      case RR_CELL_ACQ_OMSG_RESELECTED_IND:
                      {
                        ...
                        {
                          boolean pncd_enough_to_camp = rr_gprs_is_pncd_enough_to_camp(gas_id);

                          /* send nacc status ind here then cell acq control sends back finished ind*/
                          rr_cell_acq_send_imsg_nacc_acq_status_ind(
                            pncd_enough_to_camp,
                            rr_gprs_get_pcell_info(gas_id)->gsm.update_message_received,
                            gas_id
...
#endif
                            {
                              ...
                              if (rr_resel_data_ptr->old_scell_arfcn_valid)
                              {
...
#ifdef FEATURE_LTE
                                if (RR_RAT_LTE == rr_resel_data_ptr->g2x_target.rat)
                                {
                                  rr_log_reselect_start(RR_RESELECTION_TRIGGER_GSM_TO_LTE_FAILED, gas_id);
                                }
#endif /* FEATURE_LTE */
#ifdef FEATURE_GSM_TDS
                                if (RR_RAT_UTRAN_TDD == rr_resel_data_ptr->g2x_target.rat)
                                {
                                  rr_log_reselect_start(RR_RESELECTION_TRIGGER_GSM_TO_TDD_FAILED, gas_id);
                                }
#endif /* FEATURE_GSM_TDS */
#ifdef FEATURE_WCDMA
                                if(RR_RAT_WCDMA == rr_resel_data_ptr->g2x_target.rat)
                                {
                                ...
                                if (!rr_timer_is_running(CELL_RESELECTION_TIMER, gas_id))
                                {
                                  if (rr_timer_start(CELL_RESELECTION_TIMER, CELL_RESELECTION_TIMEOUT, gas_id))
                                  {
                                    MSG_GERAN_MED_0_G("Restarted the CELL_RESELECTION_TIMER after G2X fail");
                                  }
                                }
            ...
            switch (message->message_header.message_set)
            {
                          ...
                          default:
                          {
                            ...
                            if (rr_resel_data_ptr->rps_data.pscan_cell_list.no_of_entries != 0)
                            {
                              if (!rr_timer_is_running(CELL_RESELECTION_TIMER, gas_id))
                              {
                                 if (rr_timer_start(CELL_RESELECTION_TIMER, CELL_RESELECTION_TIMEOUT, gas_id))
                                 {
                                   MSG_GERAN_MED_0_G("Restarted the CELL_RESELECTION_TIMER after resync fail for RPS");
                                 }
                              }
                              rr_perform_rps(&rr_resel_data_ptr->rps_data.pscan_cell_list, gas_id);
                              rr_resel_data_ptr->state = RR_RESEL_STATE_RPS;
              ...
              case MS_MM_RR:
              {
                switch (message->message_header.message_id)
                {
                  ...
                  {
                     MSG_GERAN_HIGH_0_G("Upper layer aborted the procedure");
                     rr_cell_acq_send_imsg_abort_req(gas_id);
                     break;
                  }
                }
    ...
    if (rr_resel_data_ptr->state != rr_resel_data_ptr->old_state)
    {
      ...
      MSG_SPRINTF_2(MSG_SSID_DFLT,
                    ...
                    rr_resel_state_name(rr_resel_data_ptr->state));
#else
      MSG_GERAN_HIGH_2_G("rr_resel_control(%d->%d)", 
...
#else
      MSG_GERAN_HIGH_2_G("rr_resel_rps_control(%d->%d)",
                         rr_resel_data_ptr->rps_data.old_rps_sub_state,
                         rr_resel_data_ptr->rps_data.rps_sub_state);
#endif /* FEATURE_GERAN_REDUCED_SPRINTF */
...
boolean rr_inhibit_utran_reselection(const gas_id_t gas_id)
{
  ...
  if (rr_resel_algo(gas_id) == RR_RESEL_ALGO_RANK_BASED)
  {
    ...
    {
      if (rr_SI2q_MI_all_instances_received(rr_get_instances_ptr(SI2Q_INSTANCE,gas_id)))
      {
...
void rr_gprs_set_old_scell_snr_resel_status(
  ARFCN_T ARFCN,
  boolean snr_reselection_triggered,
  const gas_id_t gas_id
)
{
  ...
  /* old scell where SNR happened */
  rr_resel_data_ptr->rr_gprs_old_scell.reselection_data.BCCH_ARFCN = ARFCN ;

  /* Potential ncell found, now Mark SNR_TRIGGERED of the old_scell */
  rr_resel_data_ptr->rr_gprs_old_scell.snr_reselection_triggered = snr_reselection_triggered;

  if (ARFCN.num != INVALID_ARFCN)
  {
    /* Start the 30 secs timer*/
    (void)rr_gprs_start_serving_cell_bad_snr_timer(gas_id);
    MSG_GERAN_HIGH_2_G("old scell ARFCN: %d where SNR_resel_triggered = %d",
      (int)ARFCN.num,
      (int)(snr_reselection_triggered)
    );
  }
}
 ...
 *
*/
boolean rr_set_inhibit_pri_resel(const boolean inhibit, const gas_id_t gas_id)
{
  rr_resel_data_t *rr_resel_data_ptr = rr_resel_get_data_ptr(gas_id);
  boolean inhibit_change;

  RR_NULL_CHECK_FATAL(rr_resel_data_ptr); 
  inhibit_change = (rr_resel_data_ptr->inhibit_pri_resel ^ inhibit);

  rr_resel_data_ptr->inhibit_pri_resel = inhibit;

  return inhibit_change;
}
 ...
 *
*/
boolean rr_get_inhibit_pri_resel(const gas_id_t gas_id)
{
  rr_resel_data_t *rr_resel_data_ptr = rr_resel_get_data_ptr(gas_id);

  RR_NULL_CHECK_FATAL(rr_resel_data_ptr); 
  return rr_resel_data_ptr->inhibit_pri_resel;
}

#ifdef FEATURE_QSH_MDUMP
QSH_MDUMP_FN_ATTR
void grr_mdump_rr_resel(const gas_id_t gas_id)
{
  rr_resel_data_t *rr_resel_data_ptr = rr_resel_get_data_ptr(gas_id);
  qsh_mdump_collect_high(rr_resel_data_ptr, sizeof(rr_resel_data_t));
}
