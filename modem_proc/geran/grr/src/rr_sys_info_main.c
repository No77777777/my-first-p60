...
*/
/* $Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_sys_info_main.c#2 $ */
...
/*!
 * \brief Returns MSCR for the serving cell.
 * 
 * \param gas_id (in)
 * 
 * \return uint8 - MSCR
 */
mscr_t rr_si_get_mscr(const gas_id_t gas_id)
{
  if (rr_gprs_get_scell_info(gas_id)->gsm.control_channel_description.MSCR == 1)
  {
    return MSCR_REV_99;
  }
  else
...
#ifdef FEATURE_GSM_DTM
    if ((rr_in_dedicated(gas_id)) || (rr_is_control_state(RR_CONNECTION_RELEASE, gas_id)))
    {
