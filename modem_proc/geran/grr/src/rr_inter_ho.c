...
$Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_inter_ho.c#2 $ $DateTime: 2020/01/27 04:29:28 $ $Author: pwbldsvc $
...
static rr_event_T deactivate_during_x2g_handover(const gas_id_t gas_id)
{
  ...
  return(rr_mode_control(EV_DEACTIVATE_GSM_MODE_FOR_X2G_HO_FAIL, NULL, gas_id));
}
...
#ifdef FEATURE_LTE
                MSG_GERAN_HIGH_3_G("T2G HO:G2L blind redir NV: after_voice_ss_or_periodic_lau=%d, after_CSFB=%d, csfb_call_status=%d",
                                   redir, redir_after_csfb, tdsrr_interrat_handover_req->csfb_call_status);
                if (redir_after_csfb && tdsrr_interrat_handover_req->csfb_call_status)
                {
                  rr_connection_release_set_g2l_blind_redir_after_csfb_flag(gas_id);
                }
                else if(redir)
        ...
        else if (handover_event == EV_HANDOVER_COMMAND_CHECKED)
        {
          switch (handover_details_ptr->handover_type)
          {
...
#ifdef FEATURE_QSH_EVENT_NOTIFY_TO_QSH 
              rr_qsh_event_notify(RR_QSH_EVENT_G2G_HO_STARTED, gas_id);
#endif //FEATURE_QSH_EVENT_NOTIFY_TO_QSH
            ...
            case RR_L1_LTE_GSM:
            {
              ...
              {
                rr_ms_set_connection_priority(SYS_PROC_TYPE_CS_HANDOVER, FALSE, gas_id);
              }
              else
              {
                rr_ms_set_connection_priority(SYS_PROC_TYPE_SRVCC, FALSE, gas_id);
              }
              ...
              new_handover_event = rr_mode_control(EV_ACTIVATE_GSM_MODE_FOR_X2G_HO, NULL, gas_id);
              ...
              else
              {
                ...
                switch (handover_details_ptr->handover_type)
                {
                  case RR_L1_GSM_GSM:
                  {
                    ...
                    {
                      rr_perform_handover_failure(
                        RR_LOWER_LAYER_FAILURE,    // RR_cause
                        TRUE,                             // reconnect
                        gas_id                            // gas_id
                      );
                    }
            ...
            if (message_id == (byte) DL_RESUME_CNF)
            {
              ...
              switch (handover_details_ptr->handover_type)
              {
                ...
                case RR_L1_LTE_GSM:
                {
                  ...
                  rr_log_sync_ind_sent_during_srvcc(gas_id);
      ...
      case AWAITING_HANDOVER_COMPLETION:
        if (handover_event == EV_HANDOVER_COMPLETED)
        {
          ...
          */
          ...
          {
            /* Ultra Flash CSFB from GERAN prospective should be treated as CSFB if csfb call status is set to TRUE*/
            /* else this should be a regular SRVCC case */         
            if (rr_is_csfb_call_in_progress(gas_id))
            {
              if(rr_g2l_blind_redir_after_voice_ss_or_periodic_lau_enabled(gas_id))
              {
                MSG_GERAN_HIGH_0_G("UltraFlash CSFB in progress set the flag for FRLTE for VF");
                rr_connection_release_set_g2l_blind_redir_flag(gas_id);           
              }
              else if (rr_g2l_blind_redir_after_csfb_enabled(gas_id))
              {
                MSG_GERAN_HIGH_0_G("UltraFlash CSFB in progress set the flag for FRLTE for TMO");              
                rr_connection_release_set_g2l_blind_redir_after_csfb_flag(gas_id);              
              }                        
            }
            else if(rr_g2l_blind_redir_after_srvcc_enabled(gas_id))
            {
              rr_connection_release_set_g2l_blind_redir_flag(gas_id);
...
#ifdef FEATURE_QSH_MDUMP
QSH_MDUMP_FN_ATTR
void grr_mdump_rr_inter_ho(const gas_id_t gas_id)
{
  rr_inter_ho_data_t *rr_inter_ho_ptr = rr_inter_ho_get_data_ptr(gas_id);
  qsh_mdump_collect_high(rr_inter_ho_ptr, sizeof(rr_inter_ho_data_t));
}
