...
$Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_conn_establish.c#2 $ $DateTime: 2020/01/27 04:29:28 $ $Author: pwbldsvc $
...
{
  rr_conn_est_info_T *data_ptr = rr_conn_est_get_data_ptr(gas_id);

  rr_ce_set_access_reason(access_type, gas_id);

  data_ptr->egprs_radio_priority = (byte) radio_priority;
  data_ptr->pkt_est_cause = pkt_est_cause;
  ...
  switch(pkt_est_cause)
  {
    ...
    case RR_PKT_EST_CAUSE_PAGE_RESP:
      ...
      rr_qsh_event_notify( RR_QSH_EVENT_PS_DATA, gas_id);
#endif
      break;
...
#ifdef FEATURE_QSH_EVENT_NOTIFY_TO_QSH
      rr_qsh_event_notify( RR_QSH_EVENT_PS_SIGNALLING, gas_id);
#endif
      break;
...
static byte rr_construct_paging_response(byte *l3_msg_ptr, const gas_id_t gas_id)
{
  ...
  /**************************************************/
   if ((l3_msg_ptr == NULL) ||
       (rr_encode_classmark2and3(RR_CLASSMARK_2_ONLY, &classmark_2, NULL, gas_id) == FALSE))
   {
     return(0);
   }
...
#endif /* !FEATURE_LTE */
  {
    // Re-encode the Classmark 1 and/or Classmark 2 IEs to ensure the relevant cell information is used

    if (rr_conn_est_info_ptr->cm1_start_pos > 0)
    {
      MSG_GERAN_HIGH_1_G("cm1_start_pos=%d",rr_conn_est_info_ptr->cm1_start_pos);

      // Re-encode the Classmark 1 IE
      rr_conn_est_info_ptr->service_request_message[rr_conn_est_info_ptr->cm1_start_pos] =
        rr_encode_classmark1(gas_id);
    }
    else
    {
      MSG_GERAN_HIGH_0_G("cm1_start_pos=0");
    }

    if (rr_conn_est_info_ptr->cm2_start_pos > 0)
    {
      uint8 *cm2_ptr = &(rr_conn_est_info_ptr->service_request_message[rr_conn_est_info_ptr->cm2_start_pos]);

      MSG_GERAN_HIGH_1_G("cm2_start_pos=%d",rr_conn_est_info_ptr->cm2_start_pos);

      // Re-encode the Classmark 2 IE
      rr_encode_classmark2and3(
        RR_CLASSMARK_2_ONLY,
    ...
    {
      MSG_GERAN_HIGH_0_G("cm2_start_pos=0");
    }
...
)
{
  ...
  /* Notify Multi SIM Module with Connection Establishment Failure */
#if defined(FEATURE_DUAL_SIM) || defined(FEATURE_TRIPLE_SIM)
  if (FALSE == rr_ce_for_gprs_access(gas_id) )
  {
    rr_ms_cs_conn_failed(gas_id);
  }
#endif /* FEATURE_DUAL_SIM || FEATURE_TRIPLE_SIM */
...
* Handles RR_EV_START_RPS_AFTER_DENIAL 
*
* @param con_est_event : The conn establishment event
*  
*  If DSF occured on scell due to TRM denial,indicated by  RR_EV_START_RPS_AFTER_DENIAL, 
*  do not forbid the cell for the 5 second period.
*  In all other cases(EV_DSF_RESELECTION), forbid the scell.
*
* * @return void
*/
...
{
  if (con_est_event != RR_EV_START_RPS_AFTER_DENIAL)
  {   
...
static l2_channel_type_T rr_ce_get_rach_channel_type(const gas_id_t gas_id)
{
  ...
  if (rr_ce_for_gprs_access(gas_id))
  {
    if ((rr_conn_est_info_ptr->channel_request_counter == 1) &&
        (rr_conn_est_info_ptr->pkt_est_cause != RR_PKT_EST_CAUSE_PAGE_RESP))
    {
...
static void rr_ce_initiate_random_access(const gas_id_t gas_id)
{
  ...
  else /* CS ACCESS REASON */
  {
    ...
    case MM_RR_SHORT_MESSAGE:
      geran_set_traffic_reason(GERAN_TRAFFIC_MO_SMS, gas_id);
    ...
    case MM_RR_LOCATION_UPDATE:
      geran_set_traffic_reason(GERAN_TRAFFIC_CS_REGISTRATION, gas_id);
...
#ifdef FEATURE_GSM_CXM_L3_STATE
/* Report RACH state to CxM */
  rr_cxm_state_send_l3_state_ind(CXM_TECH_L3_RACH, gas_id);
#endif /* FEATURE_GSM_CXM_L3_STATE */

  if (IS_GPRS_ACCESS_REASON(access_reason))
  {
    rr_conn_est_info_ptr->access_status = RR_CE_ACCESS_FOR_TBF;
  }
  else
  {
    rr_conn_est_info_ptr->access_status = RR_CE_ACCESS_FOR_CS;
  }
...
)
{
  ...
  rr_conn_est_info_ptr->cm1_start_pos = message_ptr->cm1_start_pos;
  rr_conn_est_info_ptr->cm2_start_pos = message_ptr->cm2_start_pos;
...
)
{
  ...
  if (access_reason == MM_RR_EMERGENCY_CALL)
  {
    ...
    else if (access_result == NORMAL_CALLS_ALLOWED) 
    {
      new_event = EV_ACCESS_CLASS_NOT_ALLOWED_EMERGECNY_CALL;
      MSG_GERAN_HIGH_0_G("Normal calls only allowed, Emergency calls not allowed(access class)");
    }
    else
    {
 ...
 *
 * @param access_status       indicates if rr is in IDLE mode or not
 *
...
)
{
  ...
  if (dedicated_mode_or_tbf & 0x01)
  {
    ...
    if (RR_CE_ACCESS_FOR_CS == access_status)
    {
    ...
    else
    {
      ...
      new_event = grr_respond_to_gprs_immediate_assignment(
        ...
        dedicated_mode_or_tbf,                           // dedicated_mode_or_tbf_ie
        &rr_conn_est_info_ptr->waiting_for_2nd_msg,      // waiting_for_2nd_msg
        rr_conn_est_info_ptr->channel_request_counter,   // no_of_rach_attempts
        rr_conn_est_info_ptr->request_references,        // request_references_array
...
#ifdef FEATURE_QSH_EVENT_METRIC
  if (EV_ASSIGNMENT_REJECTION == new_event)
  {
    rr_qsh_metric_log_ia_rej_rcvd(gas_id);
  }
#endif // FEATURE_QSH_EVENT_METRIC   
...
)
{
  ...
  switch (message_type)
  {
    ...
    {
#ifdef FEATURE_QSH_EVENT_NOTIFY_TO_QSH 
          rr_qsh_event_notify(RR_QSH_EVENT_IA_RECEIVED, gas_id);
#endif //FEATURE_QSH_EVENT_NOTIFY_TO_QSH
  ...
  if ( (access_status != RR_CE_ACCESS_INACTIVE) &&
 ...
 */
static void rr_show_con_est_state_change(
  rr_con_est_state_T new_state, rr_con_est_state_T old_state, const gas_id_t gas_id)
{
...
#else
  MSG_GERAN_MED_2_G("ConEstState(%d to %d)",
                     old_state,
                     new_state);
...
)
{
  ...
  while (con_est_event != EV_NO_EVENT)
  {
    ...
    else if (con_est_event == EV_GPRS_RESET_RR_CE_STATE)
    {
      ...
      if (rr_timer_cancel(TMULTIFRM, gas_id))
      {
        ...
        rr_conn_est_info_ptr->waiting_for_2nd_msg = FALSE;
        rr_gprs_two_part_assignment_failure(rr_conn_est_info_ptr->access_status, gas_id);
    ...
    switch(rr_conn_est_info_ptr->con_est_state)
    {
      ...
      case RR_CP_INACTIVE :
      {
              ...
              case RR_PS_OMSG:
              {
                switch (message_ptr->rr.header.imh.message_id)
                {
                  case RR_PS_OMSG_NO_PS_ACCESS_CNF:
                  {
                    ...
                    // (see EV_DSF_RESELECTION, RR_EV_START_RPS_AFTER_DENIAL, RR_EV_START_RESELECTION_POWER_SCAN and
            ...
            else
            {
              ...
              else
              {
                ...
                new_con_est_event = rr_con_est_process_dl_unit_data_ind(
                  ...
                  RR_CE_ACCESS_INACTIVE,            // access_status
        ...
        else if (con_est_event == EV_GPRS_INIT_ACCESS_PROCEDURE)
        {
          ...
          /* If non-DRX was started just for handling 2 part DL IA, stop non-DRX now 
             as we are starting UL RACH and L1 will be reading full CCCH anyways. */
          if (!rr_gprs_non_drx_mode_requested(gas_id) && 
              rr_timer_is_running(TMULTIFRM, gas_id))
          {
            MSG_GERAN_MED_0_G("Resetting non-DRX mode for 2-part DL IA");
        ...
        else if (con_est_event == EV_ACCESS_CLASS_NOT_ALLOWED_EMERGECNY_CALL)
        {
          /***********************************************************/
          /* The user can't access the network, because the class    */
          /* they belong to is not authorised                        */
          ...
          new_rr_event = EV_ACCESS_CLASS_NOT_ALLOWED_EMERGECNY_CALL;
        } /* EV_ACCESS_CLASS_NOT_ALLOWED */
        /*********************************************************************/
      ...
      case RR_CP_SENDING_RANDOM_ACCESS:
      {
        if ((con_est_event == EV_INPUT_MESSAGE) && (message_ptr != NULL))
        {
          ...
          if (message_set == MS_RR_L2)
          {
            ...
            if (message_id == (byte) DL_UNIT_DATA_IND)
            {
              ...
              new_con_est_event = rr_con_est_process_dl_unit_data_ind(
                &message_ptr->dl_unit_data_ind,        // message_ptr
                rr_conn_est_info_ptr->access_status,   // access_status
                FALSE,                                 // ignore_ia_reject
                gas_id                                 // gas_id
              );
            } /* DL_UNIT_DATA_IND */
        ...
        else if((con_est_event == EV_DSF_RESELECTION) || (con_est_event == RR_EV_START_RPS_AFTER_DENIAL))
        {
          // It is possible to get 'stuck' in a loop retrying connection establishment if the signal level
          // is poor and/or the network is not responding. If GMM is not suspended, PDU build-up can occur.
          // This is only done for CS establishment attempts, excluding X2G PCCO. This is because one/all
      ...
      case RR_CP_MAX_RETRIES_ATTEMPTED :
      {
        if ((con_est_event == EV_INPUT_MESSAGE) && (message_ptr != NULL))
        {
          ...
          {
           /*
           In case of DTF simulation. Generate a T3146/T3126 expiry event. Avoid processing IA. 
           This will make sure, we dont send IA req to L1
           */
            if (message_id == (byte) DL_UNIT_DATA_IND)
            {             
#ifdef FEATURE_QSH_EVENT_NOTIFY_HANDLER
 ...
 // The DL_UNIT_DATA_IND could contain IA or IA Reject
        ...
        else if ((con_est_event == EV_DSF_RESELECTION) || (con_est_event == RR_EV_START_RPS_AFTER_DENIAL))
        {
          ...
          /* Forbid the current cell  if DSF occured for
          reasons other than TRM denial. */
          rr_ce_handle_scell_dsf_event(con_est_event, gas_id);
...
#ifdef FEATURE_QSH_EVENT_NOTIFY_TO_QSH 
          rr_qsh_event_notify(RR_QSH_EVENT_RACH_FAILURE, gas_id);
#endif //FEATURE_QSH_EVENT_NOTIFY_TO_QSH        
        } /* EV_T3126_EXPIRY */
...
#ifdef FEATURE_GSM_CXM_L3_STATE
    /* Report CONNECTING state to CxM as IA is being processed */
    if((EV_IMMEDIATE_ASSIGNMENT == new_con_est_event) ||
       (EV_GPRS_IMMEDIATE_ASSIGNMENT == new_con_est_event))
    {
      rr_cxm_state_send_l3_state_ind(CXM_TECH_L3_CONNECTING, gas_id);  
    }
#endif /* FEATURE_GSM_CXM_L3_STATE */
  ...
  if (rr_conn_est_info_ptr->con_est_state != rr_conn_est_info_ptr->old_con_est_state)
  {
    if (rr_conn_est_info_ptr->con_est_state == RR_CP_INACTIVE)
    {
      ...
      required. */
      rr_conn_est_info_ptr->pcco_in_progress = FALSE;
      rr_conn_est_info_ptr->access_status = RR_CE_ACCESS_INACTIVE;
    }
...
/**
 *
 * @return TRUE if failed RACH count is more than RR_MS_MAX_FAILED_RACH_ATTEMPTS
 */
boolean rr_ce_max_failed_rach_sent(const gas_id_t gas_id)
{
  // Obtain a pointer to the module data
  rr_conn_est_info_T *rr_conn_est_info_ptr = rr_conn_est_get_data_ptr(gas_id);
  RR_NULL_CHECK_FATAL(rr_conn_est_info_ptr);
  return(RR_MS_MAX_FAILED_RACH_ATTEMPTS <= rr_conn_est_info_ptr->failed_rach_count);
} /* rr_ce_failed_rach_count */
...
#ifdef FEATURE_QSH_MDUMP
QSH_MDUMP_FN_ATTR
void grr_mdump_rr_conn_est(const gas_id_t gas_id)
{
  rr_conn_est_info_T *rr_conn_est_info_ptr = rr_conn_est_get_data_ptr(gas_id);
  qsh_mdump_collect_high(rr_conn_est_info_ptr, sizeof(rr_conn_est_info_T));
}
