...
*/
/* $Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_pscan.h#1 $ */
 ...
 * \return word 
 */
extern word rr_pscan_process_power_scan_results(const gas_id_t gas_id);
...
/*!
 * \brief  executes any default actions for the locking feature when a power scan completes.
 * 
 * \param gas_id
 * 
 */
extern void rr_default_power_scan_cb(word num_arfcns,  sys_modem_as_id_e_type as_id);
...
/*!
 * \brief Puts the suppiled ARFCN at the bottom of the power scan database list .
 * 
 * \param pscan_db_ptr - pointer to power scan database
 * \param arfcn
 * \param gas_id
 * 
 */
extern void rr_pscan_rearrange_freq_in_list(
  rr_pscan_results_db_T *pscan_db_ptr,
  ARFCN_T  ARFCN,
  const gas_id_t gas_id
);

extern void rr_pscan_set_last_acq_db_params(
  boolean requested_plmn_only,
  PLMN_id_T *plmn_id,
  const gas_id_t gas_id 
);
 ...
 */
extern boolean rr_pscan_get_last_acq_db_freqs_valid(const gas_id_t gas_id);
 ...
 * \return boolean - TRUE if successful; FALSE otherwise
 */
extern boolean rr_pscan_get_last_acq_db_for_requested_plmn_only(const gas_id_t gas_id);
 ...
 *
 */
extern PLMN_id_T rr_pscan_get_last_acq_db_plmn_id(const gas_id_t gas_id);

extern boolean rr_pscan_results_are_acq_db(const gas_id_t gas_id);

extern ARFCN_T rr_pscan_get_strongest(const gas_id_t gas_id);

extern boolean rr_pscan_is_filtered_pscan(const gas_id_t gas_id);
