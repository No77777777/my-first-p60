...
$Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_timer.c#3 $ $DateTime: 2020/07/14 23:56:54 $ $Author: pwbldsvc $
...
static timer_type     rr_timer_dummy;
...
static rr_timer_ptr_t rr_timer_create_entry(rr_timer_handle_t timer_handle)
{
  ...
  if (new_ptr != NULL)
  {
    ...
    /* dynamically allocate a new timer entry */
    timer_ptr = (timer_ptr_type)GPRS_MEM_MALLOC(sizeof(timer_type));

    if (timer_ptr != NULL)
    {
      /* initialise the memory before passing it to the timer services module */
      memset(timer_ptr, 0, sizeof(timer_type));
      timer_def(timer_ptr, NULL, geran_tcb_read(GERAN_TASK_ID_GRR), 0,
                rr_timer_expiry_callback, (timer_cb_data_type)timer_handle);
      new_ptr->rr_timer_hndl = timer_handle;
      new_ptr->timer_ptr = timer_ptr;
    ...
    else
    {
      ...
      new_ptr = NULL;
      MSG_GERAN_ERROR_0("Unable to create a timer entry");
    }
  }
  else
  {
 ...
 *                to be assigned to the timer within it (after freeing)
 *   -----------
 *
 *   Returns:     Void
 *   --------
 *
 ****************************************************************************/
static void rr_timer_deallocate_entry(rr_timer_ptr_t rr_timer_ptr, timer_ptr_type next_timer_ptr)
{
  if (rr_timer_ptr != NULL)
  {
    /* if the timer was dynamically allocated, free it now */
    if ((rr_timer_ptr->timer_ptr != NULL) &&
        (rr_timer_ptr->timer_ptr != RR_EXPIRED_TIMER))
    {
      /* Stops an active timer */
      (void)timer_clr(rr_timer_ptr->timer_ptr, T_MSEC);
      /* Removes timer from active list and deallocates all the timer memory for the given timer handle. */
      (void)timer_undef(rr_timer_ptr->timer_ptr);
      GPRS_MEM_FREE(rr_timer_ptr->timer_ptr);
    }

    /* and then set the timer pointer as instructed */
    rr_timer_ptr->timer_ptr = next_timer_ptr;
  }
 ...
 *
 ****************************************************************************/
static rr_duration_t rr_timer_query_ms(rr_timer_ptr_t rr_timer_ptr)
{
  rr_duration_t time_left = 0;

  if (rr_timer_ptr != NULL)
  {
    if ((rr_timer_ptr->timer_ptr != RR_EXPIRED_TIMER) &&
        (rr_timer_ptr->expiry_posted == RR_TIMER_EXPIRY_NOT_POSTED))
    {
      time_left = timer_get(rr_timer_ptr->timer_ptr, T_MSEC);
    }
  }
 ...
 *   --------------
 *
 *   Description:    This function is passed to timer service as the handler for all
 *                   RR timer expiries
 *
 ...
 *
 ****************************************************************************/
static boolean rr_timer_run(rr_timer_id_T timer_id, rr_duration_t millisecs, rr_timer_cid_t context_id)
{
  rr_timer_handle_t timer_handle;
  rr_timer_ptr_t    rr_timer_ptr;
  boolean           bStarted;
  ...
  if (rr_timer_ptr != NULL)
  {
    ...
    {
      /* clear the existing timer, ready for reuse */
      (void)timer_clr(rr_timer_ptr->timer_ptr, T_MSEC);
    }
    else
    {
      ...
      if (rr_timer_ptr->timer_ptr != RR_EXPIRED_TIMER)
      {
        ...
        /* will be unique to avoid collisions between timer expiries  */
  ...
  if (rr_timer_ptr != NULL)
  {
    timer_set(rr_timer_ptr->timer_ptr, millisecs, 0, T_MSEC);
    bStarted = TRUE;
  }
  else
  {
 ...
 *
 ****************************************************************************/
boolean rr_timer_start(rr_timer_id_T timer_id, rr_duration_t millisecs, const gas_id_t gas_id)
{
...
static boolean rr_timer_clear(rr_timer_id_T timer_id, rr_timer_cid_t context_id)
{
  ...
  /* NOTE: if an expiry message had just been posted by timer service and */
...
)
{
  ...
  if (rr_timer_ptr != NULL)
  {
    ...
    if (time_remaining > RR_TIMER_MIN_DURATION_FOR_RESTART)
    {
      ...
      timer_set(rr_timer_ptr->timer_ptr, time_remaining + millisecs, 0, T_MSEC);
                          ...
                          const gas_id_t     gas_id)
{
  ...
  if (rr_timer_ptr != NULL)
  {
    ...
    {
      /* clear the existing timer, and then restart it with the shorter duration */
      (void)timer_clr(rr_timer_ptr->timer_ptr, T_MSEC);
      timer_set(rr_timer_ptr->timer_ptr, max_duration, 0, T_MSEC);
      bTruncated = TRUE;
    }
    else
    {
 ...
 *
 ****************************************************************************/
static rr_timer_ptr_t rr_timer_create_expired_entry(rr_timer_id_T timer_id, rr_timer_cid_t context_id, rr_timer_handle_t *handle_ptr)
{
  rr_timer_handle_t expired_handle;
  rr_timer_ptr_t    expired_ptr;

  /* check whether the pool has any handle for this timer already */
  expired_ptr = rr_timer_find_entry(timer_id, context_id, &expired_handle);
  ...
  else
  {
    ...
    expired_handle = rr_timer_generate_handle(timer_id, context_id);
    expired_ptr  = rr_timer_find_handle(RR_TIMER_INVALID_HANDLE);
  }

  if (expired_ptr != NULL)
  {
    expired_ptr->timer_ptr = RR_EXPIRED_TIMER;
 ...
 *
 ****************************************************************************/
boolean rr_timer_force_timer_expiry(
  rr_timer_id_T timer_id,
  const gas_id_t gas_id
)
{
  ...
  expired_ptr = rr_timer_create_expired_entry(timer_id, (rr_timer_cid_t)gas_id, &expired_handle);
  if (NULL != expired_ptr)
  {
...
)
{
  ...
  /* use internal utility function to create an expired timer entry */
  expired_ptr = rr_timer_create_expired_entry(timer_id, (rr_timer_cid_t)gas_id, NULL);
 ...
 *
 ****************************************************************************/
rr_duration_t rr_timer_get_time_remaining(
  rr_timer_id_T timer_id,
  const gas_id_t gas_id
)
{
  rr_duration_t time_left;
  rr_timer_ptr_t     rr_timer_ptr;
  rr_timer_cid_t     context_id;
...
/**
 * Start deprioritized cell timer. 
 */
void rr_start_cell_deprioritized_timer(const gas_id_t gas_id)
{
  if (!rr_timer_is_running(RR_CELL_DEPRIORITIZED_TIMER, gas_id))
  {
    MSG_GERAN_HIGH_1_G("Deprioritized cell timer started (timeout = %dms)", RR_CELL_DEPRIORITIZED_TIMEOUT);
    (void)rr_timer_start(RR_CELL_DEPRIORITIZED_TIMER,RR_CELL_DEPRIORITIZED_TIMEOUT, gas_id);
  }
  else
  {
    MSG_GERAN_HIGH_0_G("Deprioritized cell timer is already running ");
  }
  ...
  rr_timer_id_e timer_id)
{
  switch (timer_id)
  {
...
#ifdef FEATURE_FAKE_BTS_CELL_DEPRIORITIZE
    case RR_CELL_DEPRIORITIZED_TIMER:
      return "RR_CELL_DEPRIORITIZED_TIMER";
#endif/*FEATURE_GCELL_DEPRIORITIZING*/
    ...
    case RR_TECH_RECENT_ACTIVITY_TIMER:
      return "RR_TECH_RECENT_ACTIVITY_TIMER";
   ...
   case RR_MM_ACTIVATION_RSP_TIMER:
    ...
    case RR_SERV_CELL_BAD_SNR_TIMER:
      return "RR_SERV_CELL_BAD_SNR_TIMER";
...
#ifdef FEATURE_LTE
    case RR_PRIORITIZE_LTE_ABORT_TIMER:
      return "RR_PRIORITIZE_LTE_ABORT_TIMER";

    case RR_PRIORITIZE_LTE_DEFER_TIMER:
      return "RR_PRIORITIZE_LTE_DEFER_TIMER";
#endif /* FEATURE_LTE */
