...
/* $Header: //components/rel/geran.mpss/7.4.0/grr/src/geran_grr_api.c#2 $ */
...
/**
  @brief ARFCN on which user wants to lock 
  @paramAS_ID

  @return TRUE if successful; FALSE otherwise
*/

boolean geran_api_lock_channel(ARFCN_T *channel_ptr, sys_modem_as_id_e_type as_id)
{
#ifdef FEATURE_LOCK_ARFCN
  boolean found = TRUE;
  ...
  if (channel_ptr != NULL)
  {
    MSG_GERAN_HIGH_3_G("Camp on ARFCN.num = %d , ARFCN.band = %d for gas_id=%d",
                      channel_ptr->num,
                      channel_ptr->band,
                      gas_id);

    channel_number.num  = channel_ptr->num;
    channel_number.band = channel_ptr->band;
    /*First find that this channel should be in candidate cell db*/
    if (rr_candidate_db_is_candidate_cell(channel_number, gas_id ) && (!rr_is_camped(gas_id)))
    {
      rr_send_imsg_sel_locking_ind(channel_number,gas_id);
    }
    else
    {
      MSG_GERAN_HIGH_0_G("Wrong ARFCN selected or incorrect state for trying locking, try again....");
      found = FALSE;
    }
  ...
  {
    MSG_GERAN_HIGH_0_G("Unlock triggered , reinitiate the selection procedure");
    rr_send_imsg_sel_unlocking_ind(gas_id);
  }
  else
  {
    MSG_GERAN_HIGH_0_G("No locking triggered, continue legacy."); 
    rr_send_imsg_sel_nolocking_required(gas_id);
  }
...
/*===========================================================================
FUNCTION geran_api_register_scan_complete_callback

DESCRIPTION
  This function enables upper layers to register a callback function.
  Once the function is registered, it will be called whenever there is a power scan results ready
...
boolean geran_api_register_scan_complete_callback(void *callback_func_ptr, sys_modem_as_id_e_type as_id )
{
  if (callback_func_ptr != NULL)
  {
#ifdef FEATURE_LOCK_ARFCN
    gas_id_t gas_id = geran_map_nas_id_to_gas_id(as_id);
    return rr_register_scan_complete_callback((pscan_completion_cb_fn)callback_func_ptr, gas_id);
#endif /*FEATURE_LOCK_ARFCN*/
  }
 return FALSE;
}
 ...
 */
boolean geran_grr_is_opposite_sub_in_data_call(const sys_modem_as_id_e_type as_id)
{
#ifdef FEATURE_DUAL_SIM
  gas_id_t gas_id;
...
/*!
 * \brief Returns the current 3gpp release version of the Geran code
 *
 * \param none
 * \return uint8 - 3gpp release version
 */
uint8 geran_grr_get_3gpp_release_version(void)
{
  return GSM_RELEASE_VERSION;
}
