...
============================================================================*/
/* $Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_resel_utils.c#1 $ */
...
)
{
  ...
  {
    MSG_GERAN_MED_1_G("BSIC of  ncell %d changed, invalidate sys info",
      CONVERT_FROM_ARFCN_T(resel_candidate->ncell_data->ncell_data.bcch.gsm.BCCH_ARFCN)
    );
    resel_candidate->ncell_data->ncell_data.bcch.gsm.valid_data_flag = FALSE;
        ...
        */
        ...
        if ( best_six_ncells_ptr->ncell_entry[best_six_ncells_ptr->no_of_entries].ncell_data != NULL )
        {
          if ( best_six_ncells_ptr->ncell_entry[best_six_ncells_ptr->no_of_entries].ncell_data->bcch_or_pmo == BCCH_FORMAT )
          {
            ...
            {
              MSG_GERAN_MED_1_G("BSIC of  ncell %d changed, invalidate sys info",
                CONVERT_FROM_ARFCN_T(best_six_ncells_ptr->ncell_entry[best_six_ncells_ptr->no_of_entries].ncell_data->ncell_data.bcch.gsm.BCCH_ARFCN)
              );
              best_six_ncells_ptr->ncell_entry[best_six_ncells_ptr->no_of_entries].ncell_data->ncell_data.bcch.gsm.valid_data_flag = FALSE;
            }
          }
    ...
    if (ARFCNS_EQUAL(arfcn, meas_chan_info_ptr->BCCH_ARFCN))
    {
      if (meas_chan_info_ptr->valid_info == TRUE)
      {
        ...
        else
        {
          ...
          {
            MSG_GERAN_MED_1_G("BSIC of  ncell %d changed, invalidate sys info",
              CONVERT_FROM_ARFCN_T(best_six_ncells_ptr->ncell_entry[index].ncell_data->ncell_data.bcch.gsm.BCCH_ARFCN)
            );
            best_six_ncells_ptr->ncell_entry[index].ncell_data->ncell_data.bcch.gsm.valid_data_flag = FALSE;
          }
        }
      }
      else
...
)
{
  ...
  cell_type = rr_get_cell_priority(
    ...
    if(!rr_fcell_is_cell_available(ptr_surround_database->gsm.location_area_identification,
                                   ptr_surround_database->gsm.cell_id,
                                   gas_id))
    {
      MSG_GERAN_HIGH_1_G("Cell (%d) is on EFS forbidden cell list",ptr_surround_database->gsm.BCCH_ARFCN.num);
      return suitable_cell;
    }
  ...
  if (forbidden_lai)
  {
    rr_mm_send_resel_fail_ind(&ptr_surround_database->gsm.location_area_identification, gas_id);
  }
...
/****************************************************************************
 *   Function name:  rr_gprs_start_serving_cell_bad_snr_timer
 *   --------------
 *
 *   Description: This function starts the RR_SERV_CELL_BAD_SNR_TIMER timer  
 *
 *   Parameters: gas_id
 *   -----------
 *
 ...
 *
 ****************************************************************************/
void rr_gprs_start_serving_cell_bad_snr_timer(
  const gas_id_t gas_id
)
{
  if (rr_timer_start(RR_SERV_CELL_BAD_SNR_TIMER,GRR_TIMEOUT_30_SECONDS, gas_id))
  {
    MSG_GERAN_HIGH_0_G("Started serving cell Bad SNR timer (30secs)");
  }
  else
  {
    MSG_GERAN_ERROR_0_G("Failed to start serving cell Bad SNR timer (30secs)");
  }

  return;

} /* rr_gprs_start_serving_cell_bad_snr_timer */

/****************************************************************************
 *   Function name:  rr_gprs_stop_serving_cell_bad_snr_timer
 *   --------------
 *
 *   Description: This function stops the RR_SERV_CELL_BAD_SNR_TIMER timer  
 *   -----------  
 *
 *   Parameters: gas_id
 *   -----------
 ...
 *
 ****************************************************************************/
void rr_gprs_stop_serving_cell_bad_snr_timer(
  const gas_id_t gas_id
)
{
  /* only generate stopping message if timer is not already stopped */
  if (rr_timer_cancel(RR_SERV_CELL_BAD_SNR_TIMER, gas_id))
  {
    MSG_GERAN_MED_0_G("Stopped serving cell BAD SNR timer");
  }
  
} /* rr_stop_serving_snr_trigg_timer */
      ...
      if (current_meas_channel_ptr->valid_info == TRUE)
      {
        ...
        if (!SAME_BSIC(BSIC, ncell_ptr->bsic))
        {
          *bsic_changed_ptr = TRUE;
          ...
          {
            MSG_GERAN_MED_1_G("BSIC of  ncell %d changed, invalidate sys info",
              CONVERT_FROM_ARFCN_T(ncell_ptr->ncell_data.bcch.gsm.BCCH_ARFCN)
            );
            ncell_ptr->ncell_data.bcch.gsm.valid_data_flag = FALSE;
          }
        }
...
)
{
  ...
  for (index = 0; index < best_six_ncells_ptr->no_of_entries; index++)
  {
    ...
    gsm_ncell_list_member_t *resel_cand_ptr = best_six_ncells_ptr->ncell_entry[index].ncell_data;
    bcch_ncell_info_t *ncell_data_ptr = &resel_cand_ptr->ncell_data.bcch;

    CONVERT_TO_ARFCN_T(arfcn_as_type, ncell_data_ptr->gsm.BCCH_ARFCN);
    candidate_cell_ptr = rr_candidate_db_get_entry(candidate_db_ptr, arfcn_as_type);

    if ( candidate_cell_ptr != NULL && ncell_data_ptr->gsm.valid_data_flag )
    {
      MSG_GERAN_MED_2_G("Candidate cell %d valid BCCH with RXLEV %d",
                        CONVERT_FROM_ARFCN_T(ncell_data_ptr->gsm.BCCH_ARFCN),
                        candidate_cell_ptr->meas.RXLEV_average);

      if (candidate_cell_ptr->bsic_valid)
      {
        resel_cand_ptr->bsic = candidate_cell_ptr->bsic;
        resel_cand_ptr->bsic_valid = TRUE;
        ncell_data_ptr->gsm.BSIC = candidate_cell_ptr->bsic;
      }

      surrounding_channel_info->BCCH_ARFCN = ncell_data_ptr->gsm.BCCH_ARFCN;
      ...
      spoof_sch_block = (((uint8)((ncell_data_ptr->gsm.BSIC.PLMN_colour_code << 3) | ncell_data_ptr->gsm.BSIC.BS_colour_code) << 2) & 0xfc);
      surrounding_channel_info->SCH_block[0] = spoof_sch_block;
      surrounding_channel_info->valid_info = TRUE;
      best_six_info.no_of_entries++;

      ncell_data_ptr->gprs.two_quater_position = candidate_cell_ptr->si_info.two_quater_position;
    }
    else
    {
      MSG_GERAN_MED_1_G("Candidate Cell %d not found", CONVERT_FROM_ARFCN_T(ncell_data_ptr->gsm.BCCH_ARFCN));
    }
  }
  ...
  else
  {
    ...
    if (element_to_delete < RR_GPRS_RESELECTION_STRUCTURE_SIZE-1)
    {
      if (reselection_list->no_of_entries > 0)
      {
        ...
        memsmove(&reselection_list->instant_surrounding_values[element_to_delete],
                 (sizeof(reselection_list->instant_surrounding_values)-(element_to_delete*sizeof(reselection_list->instant_surrounding_values[0]))),
                 &reselection_list->instant_surrounding_values[element_to_copy_from],
                 sizeof(gprs_instant_reselection_values_T) * (reselection_list->no_of_entries - element_to_delete));
      }
    }
  }
}
