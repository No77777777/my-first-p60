...
*/
/* $Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_acq_db.c#2 $ */
/* $DateTime: 2021/06/28 07:53:28 $$Author: pwbldsvc $ */
...
static sys_band_mask_type convert_sys_band_to_sys_band_mask(sys_band_T band)
{
...
)
{
  ...
  while (entry != NULL)
  {
    if (sys_plmn_match(entry->plmn_id, plmn_id))
    {
      count++;
    }
  ...
  for (i = RR_ACQ_DB_CHUNK_COUNT; i > 0; i--)
  {
    ...
    for (j = NV_RR_ACQ_DB_MAX_ENTRIES_PER_CHUNK; j > 0; j--)
    {
          ...
          entry->plmn_id[2] != NV_RR_ACQ_DB_INVALID_PLMN_ID_BYTE)
      {
        ...
        {
          (void)rr_acq_db_add_arfcn(arfcn, plmn_id, FALSE, gas_id);
        }
      }
    }
  }
...
)
{
  ...
  else
  {
    ...
    /* Update MMCP-SD with the PLMN-Band combination if
    it is a new entry or the PLMN id is changed for an existing ARFCN in the ACQ DB.
    !INACTVE check is to avoid sending redundant updates to MMCP when ACQ DB is filled with NV data on power up.
    SD task might not be active at that time either. */
    if(!rr_is_inactive(gas_id))
    {
      if((!entry->valid) ||
         (RR_ACQ_DB_PLMN_CHANGE == db_result))
      {
        sd_update_manual_search_bst_per_subs(plmn_id, SYS_SYS_MODE_GSM, rr_convert_band_info(arfcn.band), RR_GAS_ID_TO_AS_ID);        
      }
    }
...
/* If the F3s have to be logged, to_log variable will be set to TRUE. Only in case of rr_start_ccch51_idle_mode() F3s are print in log packet*/
 ...
 * See header file for description.
 */
void rr_acq_db_populate_log_packet(rr_log_arfcn_list_t *arfcn_list, int *count)
{ 
  rr_acq_db_entry_t *entry;
  rr_acq_db_data_t  *rr_acq_db_data_ptr;
  sys_mcc_type               mcc;
  ...
  while (entry != NULL)
  {
    ...
    arfcn_list[*count].band  = entry->arfcn.band;
    arfcn_list[*count].arfcn = entry->arfcn.num;
    arfcn_list[*count].mcc   = mcc;         
    arfcn_list[*count].mnc   = mnc;
    (*count)++;
...
{
  rr_pscan_results_db_T   * temp_pscan_db_ptr;
  ...
  temp_pscan_db_ptr = GPRS_MEM_MALLOC(sizeof(rr_pscan_results_db_T));

  RR_NULL_CHECK_FATAL(temp_pscan_db_ptr);

  rr_acq_db_populate_pscan_db(
    &temp_pscan_db_ptr->pscan_results,   // pscan_request
    band_pref,                           // band_pref
    plmn_id,                             // plmn_id
    FALSE,                               // requested_plmn_only
    RR_PSCAN_REQ_NONE,                   // req_type
    gas_id                               // gas_id
  );

  num_of_arfcn = temp_pscan_db_ptr->pscan_results.num_frequencies;

  GPRS_MEM_FREE(temp_pscan_db_ptr);
  ...
  MSG_GERAN_HIGH_4_G("LTE GERAN freq update rxd freq_info_present %d, num frequencies %d, LTE PLMN Id %6x AND HST Flag %d",
   ...
   if (freq_info_ptr->freq_info_present)
   {
     ...
     for (other_gas_id = GERAN_ACCESS_STRATUM_ID_1; other_gas_id <= GERAN_ACCESS_STRATUM_ID_MAX; other_gas_id++)
     {
       ...
       {
         rr_send_imsg(RR_IMSG_FREQ_LIST_UPDATE_REQUIRED,gas_id);
       }
     }
#endif /* FEATURE_GERAN_HST_OPT */
   }
