 ...
 */
/* $Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_qsh_metric.c#1 $ */
...
)
{
  ...
  if (rr_metric_def[metric_id].data_ptr != NULL)
  {
    ...
    {
      MSG_GERAN_ERROR_0_G("GRR QSH: metric is not configured (no buffer)");
    }
  }
  else
  {
    MSG_GERAN_ERROR_0_G("GRR QSH: metric has no associated stored data");
  }
...
)
{
  ...
  /* Do not log metric when GRR is inactive as we would not have useful data */
...
void rr_qsh_metric_log_msg(uint8 msg_set, uint32 msg_id, const gas_id_t gas_id)
{
  ...
  if (qsh_buffer_ptr != NULL)
  {
    ...
    if(rr_nv_qsh_debug_enabled(RR_GAS_ID_TO_AS_ID))
    {
      MSG_GERAN_HIGH_2_G("GRR QSH: Logged MSG: %d,%d", metric_ptr->msg_set, metric_ptr->msg_id);
    }
...
void rr_qsh_metric_log_imsg(uint8 msg_set, uint8 msg_id, const gas_id_t gas_id)
{
  ...
  if (qsh_buffer_ptr != NULL)
  {
    ...
    if(rr_nv_qsh_debug_enabled(RR_GAS_ID_TO_AS_ID))
    {
      MSG_GERAN_HIGH_2_G("GRR QSH: Logged IMSG: %d,%d", metric_ptr->msg_set, metric_ptr->msg_id);    
    }
...
void rr_qsh_metric_log_ota_msg(uint8 channel_type, uint8 *msg_ptr, uint8 msg_len, const gas_id_t gas_id)
{
  ...
  if (qsh_buffer_ptr != NULL)
  {
    ...
    {
      MSG_GERAN_HIGH_5_G(
        "GRR QSH: Logged OTA MSG: channel_type=0x%x, Msg=0x %x %x %x %x ...",
        channel_type, 
        metric_ptr->msg_buf[0], 
        metric_ptr->msg_buf[1],
        metric_ptr->msg_buf[2],
        metric_ptr->msg_buf[3]
      );
    }
...
void rr_qsh_metric_log_event(uint16 event, uint8 sm, uint8 state, const gas_id_t gas_id)
{
  ...
  if (qsh_buffer_ptr != NULL)
  {
    ...
    {
      MSG_GERAN_HIGH_3_G(
        "GRR QSH: Logged Event: event=%d, sm=%d, state=%d",
        metric_ptr->event, 
        metric_ptr->sm,
        metric_ptr->state
      );
...
void rr_qsh_metric_log_cell_info(const gas_id_t gas_id)
{
  ...
  if (qsh_buffer_ptr != NULL)
  {
    ...
    {
      MSG_GERAN_HIGH_3_G(
        "GRR QSH: Logged Cell Info: plmn_id= 0x %x %x %x",
        metric_ptr->plmn_id[2], 
        metric_ptr->plmn_id[1],
        metric_ptr->plmn_id[0]
      );
      MSG_GERAN_HIGH_4_G(
        "GRR QSH: Logged Cell Info: arfcn=%d,lac=%d,rac=%d,cell_id=%d",
        metric_ptr->arfcn, 
        metric_ptr->lac,
        metric_ptr->rac,
        metric_ptr->cell_identity
      );
    }
...
void rr_qsh_metric_log_conn_end_info(sys_call_end_event_e_type type, uint8 cause, const gas_id_t gas_id)
{
  ...
  if (qsh_buffer_ptr != NULL)
  {
    ...
    {
      MSG_GERAN_HIGH_2_G(
        "GRR QSH: Logged Conn End Info: type=%d, cause=%d",
        metric_ptr->type, 
        metric_ptr->cause
      );
    }
...
/*!
 * \brief Logs GRR_QSH_METRIC_RACH (num_of_t3126_t3146_expiry member).
 *
 * \param gas_id
 */
__attribute__((section(".uncompressible.text")))
void rr_qsh_metric_log_rach_timer_expiry(const gas_id_t gas_id)
{
  if (get_qsh_metric_buffer(GRR_QSH_METRIC_RACH, gas_id) != NULL)
  {
    uint8 sub_index = GERAN_MAP_GAS_ID_TO_DATA_SPACE_INDEX(gas_id);

    rr_metric_data.stored_data.rach[sub_index].num_of_t3126_t3146_expiry++;

    if(rr_nv_qsh_debug_enabled(RR_GAS_ID_TO_AS_ID))
    {
      MSG_GERAN_HIGH_1_G(
        "GRR QSH: num_of_t3126_t3146_expiry=%d",
        rr_metric_data.stored_data.rach[sub_index].num_of_t3126_t3146_expiry
      );
    }
  }
...
void rr_qsh_metric_log_rlf(const gas_id_t gas_id)
{
  if (get_qsh_metric_buffer(GRR_QSH_METRIC_SERVICE, gas_id) != NULL)
  {
    ...
    rr_metric_data.stored_data.service[sub_index].num_of_rlf++;

    if(rr_nv_qsh_debug_enabled(RR_GAS_ID_TO_AS_ID))
    {
      MSG_GERAN_HIGH_1_G(
        "GRR QSH: num_of_rlf=%d",
        rr_metric_data.stored_data.service[sub_index].num_of_rlf
      );
    }
  }
