...
============================================================================*/
/* $Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_l1_idle_mode.c#3 $ */
/* $DateTime: 2020/01/27 04:29:28 $ */
...
/**
  @brief returns TRUE if state of RR_L1_IDLE is RR_L1_STATE_START_EARLY_CAMP 
*/
boolean rr_l1_is_early_camp (const gas_id_t gas_id)
{
  rr_l1_idle_data_t *rr_l1_idle_data_ptr = rr_l1_idle_get_data_ptr(gas_id);

  RR_NULL_CHECK_FATAL(rr_l1_idle_data_ptr);

  return (rr_l1_idle_data_ptr->state == RR_L1_STATE_START_EARLY_CAMP) ? TRUE : FALSE;
}
...
#ifdef FEATURE_QSH_MDUMP
QSH_MDUMP_FN_ATTR
void grr_mdump_rr_l1_idle(const gas_id_t gas_id)
{
  rr_l1_idle_data_t *rr_l1_idle_data_ptr = rr_l1_idle_get_data_ptr(gas_id);
  qsh_mdump_collect_high(rr_l1_idle_data_ptr, sizeof(rr_l1_idle_data_t));
}
