...
$Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_defs.h#3 $   $DateTime: 2020/07/14 23:56:54 $ $Author: pwbldsvc $
...
#define RR_ASSERT_FATAL(xx_exp)                                                \
  ...
  {                                                                            \
    ...
    ERR_GERAN_FATAL_0("RR Assertion " #xx_exp " failed");                      \
  }
...
#define RR_ASSERT_FATAL_INVALID_GAS_ID(xx)                                   \
  ...
  {                                                                          \
    QSH_MDUMP_SET(QSH_MDUMP_CRASH_CAT_UNKNOWN, QSH_CLT_GRR, QSH_CLT_GRR)     \
    ERR_GERAN_FATAL_1(#xx" = 0x%x",(int)xx);                                 \
  }
...
/* OTA BA_IND can be either 0 or 1. */
#define INVALID_BA_IND 0x02
...
typedef struct
{
  ...
  uint8 utran_csg_dedicated_freq_count;
  rr_utran_arfcn_t  utran_csg_dedicated_freq[CSN_UTRAN_CSG_DESCRIPTION_MAX_ENTRIES];
...
typedef struct
{
  ...
  uint8 eutran_csg_dedicated_freq_count;
  lte_earfcn_t  earfcn_csg_dedicated_freq[CSN_EUTRAN_CSG_DESCRIPTION_MAX_ENTRIES];
...
/*Max size is 11, CMCC can configure up to 11 TDS UARFCN's*/
#define MAX_UTRAN_FREQ_UPDATE_LIST (11)
...
typedef union
{
  ...
  wcdma_ded_measurements_T utran_fdd;
...
{
  SI_TYPE_ALL_RCVD = 0,
  SI_TYPE_SI_2_BIS_NOT_SENT,
  SI_TYPE_SI_2_TER_NOT_SENT,
  SI_TYPE_SI13_NOT_SENT,
  SI_TYPE_MAX 
  
} rr_si_type_not_sent_t;
...
typedef enum
{
  RR_CE_ACCESS_INACTIVE,
  RR_CE_ACCESS_FOR_CS,
  RR_CE_ACCESS_FOR_TBF
} rr_ce_access_status_t;
...
{
  boolean        is_downlink;
  boolean        wait_for_2nd_part;
  rr_gprs_ia_t  *ia_ptr;
} rr_gprs_ia_params_t;

typedef void (*rr_gprs_ia_cb_fn_t)(rr_gprs_ia_params_t, boolean, const gas_id_t);
...
#define RR_INVALID_REMAINING_TIME  0xFFFFFFFF

/* Max number of cells that can be deprioritized */
#define RR_MAX_NUM_OF_DEPRIORITIZED_CELLS 6
...
typedef struct
{
  ...
  rr_csi_cgi_t cgi;         /* cgi of the cell */
  boolean cgi_valid;
...
typedef rr_barred_cell_info_T rr_deprioritized_cell_info_T;
 ...
 *
 ****************************************************************************/
rr_ded_irat_measurements_t *rr_get_wcdma_ded_meas_ptr(const gas_id_t gas_id);
...
/****************************************************************************
 *   Function name:  rr_get_DSC_counter_ptr
 *   --------------------------
 *
 *   Description: Returns a pointer to DSC_counter.
 *
 *   Parameters:
 *   -----------
 *
 ...
 *
 ****************************************************************************/
int *rr_get_DSC_counter_ptr(const gas_id_t gas_id);
...
QSH_MDUMP_FN_ATTR
extern void grr_mdump_rr_plmn_list_req(const gas_id_t gas_id);
