...
============================================================================*/
/* $Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_ded_config.c#2 $ */
...
#ifdef FEATURE_GSM_CXM_L3_STATE
    /* Report CONNECTING state to CxM if L2 link establishment is ongoing */
    if(SAPI0_LINK_IN_PROGRESS == link_status)
    {
      rr_cxm_state_send_l3_state_ind(CXM_TECH_L3_CONNECTING, gas_id);
    }
    /* Report CONNECTED_CS state to CxM if L2 link is established */
    else if(NO_LINK != link_status)
    {
      rr_cxm_state_send_l3_state_ind(CXM_TECH_L3_CONNECTED_CS, gas_id);
    }
#endif /* FEATURE_GSM_CXM_L3_STATE */
  }
  else
