...
$Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_decode_ie.c#1 $ $DateTime: 2019/06/06 23:27:31 $ $Author: mplcsds1 $
  ...
  *decoding_ok = TRUE;
  ...
  else
  {
    *decoding_ok = FALSE;
    ...
    MSG_GERAN_ERROR_2_G("Format not supported(%d,%d)",(int) support,(int) IE[0]);
  ...
  else
  {
    ...
    for (index = 1; index < 9; index++)
    {
      ...
      if (increment != 0)
      {
        if ((ARFCN + increment) > 124)
        {
          MSG_GERAN_ERROR_3_G("RR Decode freq chan descrip ARFCN out of range, ARFCN:0x%x, index:%d, IE:0x%x",
                    ARFCN,
                    index,
                    IE[index]);
          *decoding_ok = FALSE;
...
)
{
  ...
  ASSERT(IE_ptr != NULL);
  ASSERT(access_reference_ptr != NULL);
  ASSERT(frame_number_ptr != NULL);
...
void rr_decode_starting_time(byte *IE_ptr, word *starting_time_ptr, boolean *decode_ok_ptr)
{
  ...
  ASSERT(IE_ptr != NULL);
  ASSERT(starting_time_ptr != NULL);
  ASSERT(decode_ok_ptr != NULL);
...
{
  ASSERT (SI2quater != NULL);
  /* ASSERT (SI2quater_INDEX < MAX_INSTANCES); */

  if (SI2quater_INDEX < MAX_INSTANCES)
  {
    if (SI2quater->instance[SI2quater_INDEX] &&
        SI2quater->instance[SI2quater_INDEX]->valid_data == TRUE)
    {
      return TRUE;
    }
  }
...
)
{
  ...
  if (index < MAX_INSTANCES)
  {
    ...
    {
      MSG_GERAN_ERROR_1("Memory is not allocated for index %d", index);
    }
  }
  else
  {
...
void rr_reset_SI2q_MI_changed(instances_T *instances, byte array_length)
{
  int i;
...
static boolean rr_MI_changed_3g_wait_instances (instances_T *MeasInfo, byte array_length)
{
  ...
  for (i = 0; i < array_length; ++i)
  {
    SI2q_MI_instance_T *instance = MeasInfo->instance[i];
  
    if (instance && instance->common_params.WCDMA_3G_NC_Desc_PI)
    {
...
boolean rr_SI2q_MI_changed (instances_T *instances)
{
  ...
  {
    SI2q_MI_instance_T *instance = instances->instance[i];
...
)
{
  ...
  ASSERT (encoded_fdd_info != NULL);
  ASSERT (decoding_ok != NULL);
  ASSERT ((si2q_params != NULL) || (mi_params != NULL));
  ASSERT (encoded_tdd_info != NULL);
...
)
{
  ...
  if (gs_bit_stream_extract_field(bit_stream_ptr,1))
  {
    ...
    ASSERT(gprs_bsic_description != NULL);
...
)
{
  ...
  ASSERT (encoded_fdd_cells != NULL);
  ASSERT (decoding_ok != NULL);
  ASSERT (encoded_tdd_cells != NULL);
  ASSERT (common_params != NULL);
...
{
  ASSERT (tempEncoded_FDD_Info != NULL);
...
boolean rr_received_all_si2ter_instances(si2ter_instances_T *instances, byte array_length, byte COUNT)
{
  ...
  if (array_length == COUNT+1) /* For one instance SI2ter count = 0, hence the +1 */
  {
    received = TRUE;
...
boolean rr_si2ter_changed (si2ter_instances_T *instances)
{
  ...
  {
    if (instances->instance[i] && 
        instances->instance[i]->valid_data)
    {
      /* bitwise OR of the received bit of all instances */
...
)
{
  ...
  /* Asserts */
  ASSERT (IE != NULL);
  ASSERT (si2ter_rest_instances != NULL);
  ASSERT (si2ter_received != NULL);
  ASSERT (decoding_ok != NULL);
  ...
  *si2ter_received = FALSE;
  ...
  {
    MSG_GERAN_MED_0_G("SI2ter rest not present");
  ...
  {
    MSG_GERAN_HIGH_2_G("SI2ter_INDEX > SI2ter_COUNT in SI 2ter(%d > %d)",(int) this_si2ter_instance.SI2ter_INDEX,
  ...
  if (si2ter_rest_instances->instance[this_si2ter_instance.SI2ter_INDEX] &&
      si2ter_rest_instances->instance[this_si2ter_instance.SI2ter_INDEX]->valid_data)
  {
    /* Check if there is no change in both the 3G_BA_IND and MP_CHANGE_MARK */
    if ((rr_si2ter_3G_BA_changed(si2ter_rest_instances, this_si2ter_instance.SI2ter_3G_CHANGE_MARK) == FALSE) &&
        (rr_si2ter_MP_changed(si2ter_rest_instances, this_si2ter_instance.SI2ter_MP_CHANGE_MARK) == FALSE))
    {
      MSG_GERAN_HIGH_2_G("Rx SI2ter(inst=%d, max=%d): No 3G BA or MP change", this_si2ter_instance.SI2ter_INDEX, this_si2ter_instance.SI2ter_COUNT);
  ...
  */
  ...
  MSG_GERAN_HIGH_2_G("Decoding SI2ter instance %d, max=%d",(int) this_si2ter_instance.SI2ter_INDEX, (int) this_si2ter_instance.SI2ter_COUNT );

  /* If it is a newly received instance then allocate memory for it  */
  if (si2ter_rest_instances->instance[this_si2ter_instance.SI2ter_INDEX] == NULL)
  {
    rr_allocate_SI2ter_instance(si2ter_rest_instances, this_si2ter_instance.SI2ter_INDEX, gas_id);
  }
  ...
  if (rr_si2ter_3G_BA_changed(si2ter_rest_instances, this_si2ter_instance.SI2ter_3G_CHANGE_MARK))
  {
    ...
    rr_reset_SI2ter_instances(si2ter_rest_instances, gas_id);

    /* Allocate memory for current instance */
    rr_allocate_SI2ter_instance(si2ter_rest_instances, this_si2ter_instance.SI2ter_INDEX, gas_id);

    if (sys_info_db_ptr->cell_identity_valid_flag)
    {
      MSG_GERAN_HIGH_0_G("SI2ter 3G_BA_IND changed flush SI2ter");
      rr_csi_flush_si_data(sys_info_db_ptr->BCCH_ARFCN,                    // ARFCN
                           SYSTEM_INFORMATION_TYPE_2_TER,                  // MSG_TYPE 
                           &sys_info_db_ptr->location_area_identification, // LAI
                           &sys_info_db_ptr->cell_identity,                // CI
                           CSI_FLUSH_SI_CHANGE,                            // FLUSH_CAUSE 
                           gas_id);
    }
  }
  ...
  if (rr_si2ter_MP_changed(si2ter_rest_instances, this_si2ter_instance.SI2ter_MP_CHANGE_MARK))
  {
    ...
    if (sys_info_db_ptr->cell_identity_valid_flag)
    {
      MSG_GERAN_HIGH_0_G("SI2ter MP_CHANGE_MARK changed flush SI2ter");
      rr_csi_flush_si_data(sys_info_db_ptr->BCCH_ARFCN,                     // ARFCN
                           SYSTEM_INFORMATION_TYPE_2_TER,                   // MSG_TYPE 
                           &sys_info_db_ptr->location_area_identification,  // LAI
                           &sys_info_db_ptr->cell_identity,                 // CI
  ...
  *(si2ter_rest_instances->instance[this_si2ter_instance.SI2ter_INDEX]) = this_si2ter_instance;
  ...
  MSG_GERAN_HIGH_2_G("Decoded SI2ter inst=%d, max=%d", this_si2ter_instance.SI2ter_INDEX, this_si2ter_instance.SI2ter_COUNT);
}
...
/**************************************************************************************
Function
rr_si2quater_csn_psc_group_ie_decode

Description
This function is a static function to extract the UTRAN PSC GROUP IE received 
in the UTRAN CSG Top Level Container.
If the contents are valid, they are transferred to the relevant structure.

Parameters
[IN]
byte                  *bitstream:
Pointer to the bit stream containing the UTRAN CSG IE
received from the network.

[OUT]
instances_T           *dst:
Pointer to destination structure where the information is decoded.

Returns
None

**********************************************************************************************/
static void rr_si2quater_csn_psc_group_ie_decode(gs_bit_stream_T * bitstream, csn_psc_group_ie_t* csg_psc_split_ptr  )
{
  ...
  if (bitstream == NULL)
  {
    MSG_ERROR("bitstream == NULL", 0, 0, 0);
    return;
  }
  /* Explicit PSC coding */
  loop_count = 0;
  while ( gs_bit_stream_extract_field(bitstream, 1) && (loop_count < RR_CSG_GUARD_COUNT ))
  {
    /* Extract an individual PSC  */
    uint16 psc = (uint16 )gs_bit_stream_extract_field(bitstream, 9);
  ...
  while (gs_bit_stream_extract_field(bitstream, 1) && ( loop_count < RR_CSG_GUARD_COUNT ) )
  {
    uint8 length = (byte) gs_bit_stream_extract_field(bitstream, 3) + 1;
    uint8 value = (byte) gs_bit_stream_extract_field(bitstream, length);
    uint8 sense = (byte)gs_bit_stream_extract_field(bitstream, 1);
    ...
    {
      MSG_GERAN_ERROR_1("PSC Pattern Count Exceeded MAX: %d", psc_group.psc_pattern_count);    
    }
    loop_count++;	
  }

  if (csg_psc_split_ptr != NULL)
  {
    *csg_psc_split_ptr = psc_group;
  }
  else
  {
    MSG_GERAN_ERROR_0("rr_si2quater_csn_psc_group_ie_decode: NULL Output Pointer ");
  }
}
...
Description
This function is a top level function to extract the UTRAN CSG information Structure.
If the contents are valid, they are transferred to the relevant structure.
...
)
{
  ...
  while ( gs_bit_stream_extract_field(bitstream, 1) && (loop_count < RR_CSG_GUARD_COUNT) )
  {
    ...
    uint8 innner_loop_count;
	
    memset(&csg_psc_split, 0, sizeof(csn_psc_group_ie_t));
    rr_si2quater_csn_psc_group_ie_decode(bitstream, &csg_psc_split);
    MSG_GERAN_HIGH_0("UTRAN CSG Decoded");
    innner_loop_count = 0;
    while ( gs_bit_stream_extract_field(bitstream, 1) && (innner_loop_count < RR_CSG_GUARD_COUNT) )
    {
    ...
    if ( dst->entry_count < CSN_UTRAN_CSG_DESCRIPTION_MAX_ENTRIES)
    {
      dst->entries[dst->entry_count].csg_psc_split = csg_psc_split;
      dst->entries[dst->entry_count].utran_frequency_indices_bitmap = utran_frequency_indices_bitmap;
      ++dst->entry_count;
    }
    else
    {
  ...
  while ( gs_bit_stream_extract_field(bitstream, 1) && (loop_count < RR_CSG_GUARD_COUNT) )
  {
    if ( dst->utran_csg_dedicated_freq_count < CSN_UTRAN_CSG_DESCRIPTION_MAX_ENTRIES )
    {
      if (gs_bit_stream_extract_field(bitstream, 1))
      {
        dst->utran_csg_dedicated_freq[dst->utran_csg_dedicated_freq_count].rat_type  = RR_UTRAN_TDD;
      }
      else
      {
        dst->utran_csg_dedicated_freq[dst->utran_csg_dedicated_freq_count].rat_type  = RR_UTRAN_FDD;
      }

      dst->utran_csg_dedicated_freq[dst->utran_csg_dedicated_freq_count].uarfcn =  (word) gs_bit_stream_extract_field(bitstream, 14);
      dst->utran_csg_dedicated_freq_count++;
    }
    else
    {
      MSG_GERAN_ERROR_0("Overflow decoding UTRAN CSG Description");
...
/**************************************************************************************
Function
rr_si2quater_csn_pcid_group_ie_decode

Description
This function is a static function to extract the E-UTRAN PC-ID GROUP IE received 
in the E-UTRAN CSG Top Level Container.
If the contents are valid, they are transferred to the relevant structure.

Parameters
[IN]
byte                  *bitstream:
Pointer to the bit stream containing the E-UTRAN CSG IE
received from the network.
...
**********************************************************************************************/
static void rr_si2quater_csn_pcid_group_ie_decode(gs_bit_stream_T * bitstream, csn_pcid_group_ie_t* csg_pcid_split_ptr  )
{
  ...
  {
    /* Extract an individual PCID  */
    uint16 pcid =  (uint16) gs_bit_stream_extract_field(bitstream, 9);
  ...
  if( gs_bit_stream_extract_field(bitstream, 1))
  {
    ...
    for (n = 1; n < 7; ++n)
    {
      ...
      if (IS_BIT_NUM_SET(pcid_bitmap_group, (n - 1)))
      {
        ...
        {
          RR_PCID_GROUP_IE_SET(pcid_group.bitmap, i);
        }
      }
    }
  }
  ...
  while ( gs_bit_stream_extract_field(bitstream, 1) && (loop_count < RR_CSG_GUARD_COUNT) )
  {
    uint8 length = (uint8) gs_bit_stream_extract_field(bitstream, 3) + 1;
    uint8 value =  (uint8) gs_bit_stream_extract_field(bitstream, length);
    uint8 sense =  (uint8) gs_bit_stream_extract_field(bitstream, 1);
  ...
  if (csg_pcid_split_ptr != NULL)
  {
    *csg_pcid_split_ptr = pcid_group;
  }
  else
  {
    MSG_GERAN_ERROR_0("rr_si2quater_csn_pcid_group_ie_decode: NULL Output Pointer ");
  }
}
...
Description
This function is a top level function to extract the E-UTRAN CSG information Structure.
If the contents are valid, they are transferred to the relevant structure.
...
)
{
  ...
  while ( gs_bit_stream_extract_field(bitstream, 1) && (loop_count < RR_CSG_GUARD_COUNT) )
  {
    ...
    uint32			eutran_frequency_indices_bitmap = 0;
	uint8           inner_loop_count;
    memset(&csg_pci_split, 0, sizeof(csn_pcid_group_ie_t));

    rr_si2quater_csn_pcid_group_ie_decode(bitstream, &csg_pci_split);
    MSG_GERAN_HIGH_0(" EUTRAN CSG Decoded");
    inner_loop_count = 0;
    while (gs_bit_stream_extract_field(bitstream, 1) && (inner_loop_count < RR_CSG_GUARD_COUNT) )
    {
      uint32 eutran_frequency_index = gs_bit_stream_extract_field(bitstream, 3);

      if (eutran_frequency_index < 32)
      {
    ...
    if(dst->entry_count < CSN_EUTRAN_CSG_DESCRIPTION_MAX_ENTRIES)
    {
      dst->entries[dst->entry_count].csg_pci_split = csg_pci_split;
      dst->entries[dst->entry_count].eutran_frequency_indices_bitmap = (uint8) eutran_frequency_indices_bitmap;
      ++dst->entry_count;
    }
    else
    {
      MSG_GERAN_ERROR_0("Overflow decoding EUTRAN CSG Description");
    }  
    loop_count++;
  }
  ...
  while ( gs_bit_stream_extract_field(bitstream, 1) && (loop_count < RR_CSG_GUARD_COUNT))
  {
    if ( dst->eutran_csg_dedicated_freq_count < CSN_EUTRAN_CSG_DESCRIPTION_MAX_ENTRIES )
    {	
      dst->earfcn_csg_dedicated_freq[dst->eutran_csg_dedicated_freq_count] =  gs_bit_stream_extract_field(bitstream, 16);
      dst->eutran_csg_dedicated_freq_count++;
    }
    else
    {
      MSG_GERAN_ERROR_0("Overflow decoding EUTRAN CSG Description");
...
{
  MSG_GERAN_HIGH_0("rr_si2quater_ro_utran_csg_desc_free_children: No Child Allocations Exists");
}
...
{
  MSG_GERAN_HIGH_0("rr_si2quater_ro_eutran_csg_desc_free_children: No Child Allocations Exists");
}
...
)
{
  ...
  ASSERT (IE != NULL);
  ASSERT (SI2quater != NULL);
  ASSERT (si2quater_received != NULL);
  ASSERT (decoding_ok != NULL);
  ...
  *si2quater_received = FALSE;
  ...
  {
    /* No Change; This instance has been received and processed already; So Ingore */
    MSG_GERAN_HIGH_2_G("Repeat SI2quater instance %d",(int) temp_common_params.INDEX, 0 );
    MSG_GERAN_MED_2_G("Received SI2q(Index=%d, Count=%d)",(int) temp_common_params.INDEX, (int) temp_common_params.COUNT );
    return;
  }

  MSG_GERAN_HIGH_2_G("Received SI2q(Index=%d, Count=%d)",(int) temp_common_params.INDEX, (int) temp_common_params.COUNT );
  ...
  {
    MSG_GERAN_HIGH_2_G("INDEX > COUNT in SI 2quater(%d > %d)",(int) temp_common_params.INDEX, (int) temp_common_params.COUNT );
    *decoding_ok = FALSE;
    return;
  }
  ...
  if (*decoding_ok == FALSE)
  {
    MSG_GERAN_HIGH_2_G("rr_extract_SI2q_MI_3G_NC_Desc failed for SI2q(%d, %d)",(int) temp_common_params.INDEX, (int) temp_common_params.COUNT );
    return;
  }
  ...
  if (!bit_stream_ptr->overflow)
  {
    ...
    if (gs_bit_stream_extract_HL(bit_stream_ptr))
    {
      ...
      if (gs_bit_stream_extract_field(bit_stream_ptr, 1))
      {
        ...
        if (bit_stream_ptr->overflow)
        {
          MSG_GERAN_ERROR_0_G("3G Additional Measurement Parameters indicated but message overflow detected!");
      ...
      if (gs_bit_stream_extract_field(bit_stream_ptr, 1))
      {
        ...
        if (bit_stream_ptr->overflow)
        {
          MSG_GERAN_ERROR_0_G("3G Additional Measurement Parameters 2 indicated but message overflow detected!");
      ...
      if (!bit_stream_ptr->overflow)
      {
        ...
        if (!bit_stream_ptr->overflow)
        {
          ...
          if (gs_bit_stream_extract_HL(bit_stream_ptr))
          {
            MSG_GERAN_MED_0_G("SI2quater Rel-7 Additions present");
            if (gs_bit_stream_extract_field(bit_stream_ptr, 1))
            {
          ...
          if (!bit_stream_ptr->overflow)
          {
            ...
            if (gs_bit_stream_extract_HL(bit_stream_ptr))
            {
              MSG_GERAN_MED_0_G("SI2quater Rel-8 Additions present");
              ...
              if (bit_stream_ptr->overflow)
              {
                ...
                MSG_GERAN_ERROR_0_G("Error in network encoding of priority and eutran params, non-fatal");
              }
              ...
              if (bit_stream_ptr->overflow)
              {
                ...
                MSG_GERAN_ERROR_0_G("Error in network encoding of utran csg, non-fatal");
              }
              ...
              if (bit_stream_ptr->overflow)
              {
                ...
                MSG_GERAN_ERROR_0_G("Error in network encoding of eutran csg, non-fatal");
              }
            }
          }
          ...
          if (!bit_stream_ptr->overflow)
          {
            ...
            if (gs_bit_stream_extract_HL(bit_stream_ptr))
            {
              MSG_GERAN_MED_0_G("SI2quater Rel-9 Additions present");
              ...
              if (!bit_stream_ptr->overflow)
              {
                ...
                if (gs_bit_stream_extract_HL(bit_stream_ptr))
                {
                  MSG_GERAN_MED_0_G("SI2quater Rel-10 Additions present");
                  ...
                  MSG_GERAN_MED_2_G("int_pwr_red_present %d int_pwr_red %d",
                           temp_si2q_params.int_pwr_red_present,temp_si2q_params.int_pwr_red);
  ...
  if (bit_stream_ptr->overflow)
  {
    ...
    #error code not present
#else
    MSG_GERAN_ERROR_0_G("Error in network encoding of SI2quater, non-fatal");
#endif
  }
  /************ Done extracting information from the IE into local storage ***************/
  ...
  */
  ...
  /* If it is a newly received instance then allocate memory for it  */
  if (SI2quater->instance[temp_common_params.INDEX] == NULL)
  {
    rr_allocate_SI2q_MI_instance(SI2quater, temp_common_params.INDEX, SI2Q_INSTANCE, gas_id);
  }
       ...
       rr_MP_changed(SI2quater, &temp_common_params) )
  {
    ...
    MSG_GERAN_HIGH_2_G("3G_BA_IND      old: <%d>, new: <%d> in SI2quater", \
      (int) common_params->BA_IND_3G, (int) temp_common_params.BA_IND_3G);
    MSG_GERAN_HIGH_2_G("MP_CHANGE_MARK old: <%d>, new: <%d> in SI2quater", \
    ...
    rr_initialize_SI2q_MI_instances(SI2quater, SI2Q_INSTANCE, gas_id);

    /* Allocate memory for current instance and reassign pointers */
    rr_allocate_SI2q_MI_instance(SI2quater, temp_common_params.INDEX, SI2Q_INSTANCE, gas_id);
    common_params = &SI2quater->instance[temp_common_params.INDEX]->common_params;
    si2q_params   = &SI2quater->instance[temp_common_params.INDEX]->si2q_mi.si2q_params;
    ...
    {
      MSG_GERAN_HIGH_0_G("SI2Q MP changed flush SI2quater");
      rr_csi_flush_si_data(cell_info_ptr->BCCH_ARFCN,                       // ARFCN
                           SYSTEM_INFORMATION_TYPE_2_QUATER,                // MSG_TYPE
                           &(cell_info_ptr->location_area_identification),  // LAI
                           &(cell_info_ptr->cell_identity),                 // CI
                           CSI_FLUSH_SI_CHANGE,                            // FLUSH_CAUSE
                           gas_id);
    }
  }
  ...
  *common_params = temp_common_params;
  ...
  {
#ifdef FEATURE_NONBLIND_HANDOVER_DEBUG
    MSG_GERAN_HIGH_0_G("Received all instances of SI2quater");
#endif
...
)
{
  ...
  /* Assert */
  ASSERT (IE != NULL);
  ASSERT (MeasInfo != NULL);
  ASSERT (send_wcdma_neighbor_list_to_L1 != NULL);
  ASSERT (mi_received != NULL);
  ASSERT (decoding_ok != NULL);
  ...
  *mi_received = FALSE;
  ...
  {
    /* No Change; This instance has been received and processed already; So Ingore */
    MSG_GERAN_HIGH_2_G("Repeat MI instance %d",(int) temp_common_params.INDEX, 0 );
    /* log the index and count */
    MSG_GERAN_MED_2_G("Received MI(Index=%d, Count=%d)",(int) temp_common_params.INDEX, (int) temp_common_params.COUNT );
  ...
  if (temp_common_params.INDEX > temp_common_params.COUNT)
  {
    MSG_GERAN_HIGH_2_G("INDEX > COUNT in MI (%d > %d)",(int) temp_common_params.INDEX, (int) temp_common_params.COUNT );
    *decoding_ok = FALSE;
    return;
  }
  ...
  if (*decoding_ok == FALSE)
  {
    MSG_GERAN_HIGH_2_G("rr_extract_SI2q_MI_3G_NC_Desc failed for MI(%d,%d)",(int) temp_common_params.INDEX, (int) temp_common_params.COUNT );
    return;
  }
  ...
  if (bit_stream_ptr->overflow)
  {
    ...
    #error code not present
#else
    MSG_GERAN_ERROR_0_G("Error in network encoding of Measurement Information, ignoring");
#endif
    *decoding_ok = FALSE;
    return;
  }
  ...
  if (gs_bit_stream_extract_HL(bit_stream_ptr))
  {
    MSG_GERAN_MED_0_G("Release 5 Additions present");
    if (gs_bit_stream_extract_field(bit_stream_ptr, 1))
    {
      MSG_GERAN_MED_0_G("3G Additional Measurement Parameters Description 2 present");
      rr_extract_3g_addl_meas_params_desc_2(bit_stream_ptr, &temp_mi_params);
    }
    else
    {
      MSG_GERAN_MED_0_G("3G Additional Measurement Parameters Description 2 not present");
    }
    ...
    */
    if (gs_bit_stream_extract_HL(bit_stream_ptr))
    {
      MSG_GERAN_MED_0_G("Additions in Rel-7 present");
      ...
      */
      if (gs_bit_stream_extract_HL(bit_stream_ptr))
      {
        MSG_GERAN_MED_0_G("Additions in Rel-8 present");
        ...
        */
        if (gs_bit_stream_extract_field(bit_stream_ptr, 1))
        {
          ...
          if (bit_stream_ptr->overflow)
          {
            MSG_GERAN_ERROR_0_G("Truncation detected during E-UTRAN Paramters decode, discarding IE");
          }
        }
        ...
        */
        if (gs_bit_stream_extract_HL(bit_stream_ptr))
        {
          MSG_GERAN_MED_0_G("Additions in Rel-9 present");
        }
        else
        {
          MSG_GERAN_MED_0_G("Additions in Rel-9 not present");
        }
      }
      else
      {
        MSG_GERAN_MED_0_G("Additions in Rel-7 not present");
  ...
  {
    MSG_GERAN_MED_0_G("Release 5 Additions not present");
    if (bit_stream_ptr->overflow)
    {
  ...
  */
  ...
  MSG_GERAN_HIGH_2_G("Decoding Measurement Information instance %d, max=%d",(int) temp_common_params.INDEX, (int) temp_common_params.COUNT );

  /* If it is a newly received instance then allocate memory for it  */
  if (MeasInfo->instance[temp_common_params.INDEX] == NULL)
  {
    rr_allocate_SI2q_MI_instance(MeasInfo, temp_common_params.INDEX, MI_INSTANCE, gas_id);
  }
  ...
  {
#ifdef FEATURE_NONBLIND_HANDOVER_DEBUG
    MSG_GERAN_HIGH_2_G("3G_BA_IND in MI:      old <%d>, new <%d> ", (int)common_params->BA_IND_3G,
      (int) temp_common_params.BA_IND_3G);
    MSG_GERAN_HIGH_2_G("MP_CHANGE_MARK in MI: old <%d>, new <%d> ", (int)common_params->MP_CHANGE_MARK,
      (int) temp_common_params.MP_CHANGE_MARK);
#endif
    /* Re-read all instances of MeasInfo and rebuild the 3G Neighbor list */
    /* This will set valid_data =FALSE and received/changed to FALSE */
    rr_initialize_SI2q_MI_instances(MeasInfo, MI_INSTANCE, gas_id);
  ...
  */
  if (temp_mi_params.MI_3G_WAIT_PI)
  {
    ...
    {
#ifdef FEATURE_NONBLIND_HANDOVER_DEBUG
      MSG_GERAN_HIGH_2_G("3G_WAIT(%d)<MI_COUNT(%d)",(int)temp_mi_params.MI_3G_WAIT,(int)temp_common_params.COUNT);
#endif
  ...
  if (rr_received_all_instances(MeasInfo,MeasInfo->num_instances,temp_common_params.COUNT))
  {
    *mi_received = TRUE;
#ifdef FEATURE_NONBLIND_HANDOVER_DEBUG
    MSG_GERAN_HIGH_0_G("Received all instances of Measurement Information");
#endif
    ...
    if (rr_SI2q_MI_changed(MeasInfo))
    {
      *send_wcdma_neighbor_list_to_L1 = TRUE;
#ifdef FEATURE_NONBLIND_HANDOVER_DEBUG
    MSG_GERAN_HIGH_0_G("Sending WCDMA NCELL List to L1");
#endif
      /* reset changed flags; */
  ...
  else if (target_type == FDD_TARGET)
  {
            ...
            (num_of_freq < MAX_UTRAN_FDD_FREQUNECIES) )
    {
      ...
      if (gs_bit_stream_extract_field(bit_stream_ptr, 1) == 1)
      {
        ...
        {
          /* ASSERT (encoded_fdd_cells.num_cells < MAX_UTRAN_CELLS_PER_FREQUENCY+1); */
...
#ifdef FEATURE_GSM_TDS_G2T_REDIRECTION
    {
              ...
              (num_of_freq < MAX_UTRAN_TDS_FREQUNECIES) )
      {
        ...
        if (gs_bit_stream_extract_field(bit_stream_ptr, 1) == 1)
        {
          ...
          {
            /* ASSERT (encoded_tdd_cells.num_cells < MAX_UTRAN_CELLS_PER_FREQUENCY+1); */
