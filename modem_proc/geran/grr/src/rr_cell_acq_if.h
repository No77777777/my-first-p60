...
============================================================================*/
/* $Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_cell_acq_if.h#1 $ */
...
typedef enum
{
  RR_CELL_ACQ_BSIC_MISMATCH_ACTION_NONE,
  RR_CELL_ACQ_BSIC_MISMATCH_ACTION_STOP,
  RR_CELL_ACQ_BSIC_MISMATCH_ACTION_REQUIRE_SI3,
  RR_CELL_ACQ_BSIC_MISMATCH_ACTION_MAX
} rr_cell_acq_bsic_mismatch_action_e;

typedef struct
{
  BSIC_T                              bsic;
  rr_cell_acq_bsic_mismatch_action_e  mismatch_action;
  boolean                             send_to_l1;
} rr_cell_acq_bsic_info_t;
...
typedef enum
{
  ...
  /* Indicates to the RR cell acquisition state machine whaether all system
  information required for NACC camping has been received.  On a BCCH cell
  this would normally be SI1, SI3 and SI13. */
...
} rr_cell_acq_imsg_nacc_acq_status_ind_t;
...
typedef union
{
  ...
  rr_cell_acq_imsg_nacc_acq_status_ind_t  nacc_acq_status_ind;
} rr_cell_acq_imsg_u;
...
/**
  @brief Send an RR_CELL_ACQ_IMSG_NACC_ACQ_STATUS_IND to the RR cell acquisition state machine. 
*/
extern void rr_cell_acq_send_imsg_nacc_acq_status_ind(
  boolean         pncd_enough_to_camp,
  uint16          si_messages_received,
  const gas_id_t  gas_id
);
