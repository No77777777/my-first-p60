...
$Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_g2w.c#1 $
...
#define RR_G2X_TIMER_EXPIRY_WAVEOFF_TIME   100
...
)
{
  ...
  else if (no_of_neighbor_wcdma_cells != 0)
  {
    no_of_neighbor_wcdma_cells = 0;
    MSG_GERAN_HIGH_0_G("G2W reselection blocked due to RAT restriction");
  }
  ...
  else if (no_of_neighbor_utran_tdd_cells != 0)
  {
    no_of_neighbor_utran_tdd_cells = 0;
    MSG_GERAN_HIGH_0_G("G2T reselection blocked due to RAT restriction");
  }
  ...
  for (count = 0; count < no_of_neighbor_utran_tdd_cells; ++count)
  {
    ...
    if (utran_tdd_info_ptr->rat == RR_RAT_UTRAN_TDD)
    {
      ...
      if (tdsrrc_check_cell_barred_sub_id(
            ...
            (uint8)utran_tdd_info_ptr->data.utran_tdd.cell_parameter_id,
            RR_GAS_ID_TO_AS_ID
          ) != TDSRRC_NOT_BARRED)
#else
...
/**
    @detail
    This function checks if LP check is required due to Slow IRAT mode.

    If (HP timer running or expired)
        Do HP check
    Else If (LP timer running or expired)
        Do LP check
    Else // No timer running
        If (Regular IRAT mode)
            Based on priority, do either HP or LP check
        Else // Slow IRAT mode
            Do LP check 
  
    @param  entry: reselection candidate cell
           
  
    @return TRUE if lower priority reselection path to be taken and FALSE otherwise.
**/
static boolean rr_is_LP_check_required(rr_irat_priority_list_entry_t *entry, const gas_id_t gas_id)
{
  boolean check_LP = FALSE;
  
  if (rr_timer_is_running(entry->measurement->thresh_high_timer, gas_id) ||
      rr_timer_has_expired(entry->measurement->thresh_high_timer, gas_id))
  {
    check_LP = FALSE;
  }
  else if (rr_timer_is_running(entry->measurement->thresh_low_timer, gas_id) || 
           rr_timer_has_expired(entry->measurement->thresh_low_timer, gas_id) ||    
           rr_timer_is_running(entry->measurement->h_prio_timer, gas_id) ||
           rr_timer_has_expired(entry->measurement->h_prio_timer, gas_id))
  {
    check_LP = TRUE;
  }
  else /* neither timer is running */
  {
    if (rr_ms_in_slow_irat_mode(gas_id))
    {
      check_LP = TRUE;
    }
    else
...
)
{
  ...
  for (i = 0; i < priority_list.length; ++i)
  {
    ...
    boolean                        entry_suitable = rr_irat_priority_list_entry_suitable(entry,gas_id);
    boolean                        check_LP = FALSE;
    ...
    /* First, decide if LP check is required due to Slow IRAT mode */
    check_LP = rr_is_LP_check_required(entry, gas_id);

    if (entry->priority > geran_priority)
    {
      if (check_LP == FALSE)
      {
        /* Entry is a high priority cell. */
        if (entry->s_value > entry->thresh_high && entry_suitable)
        {
          if (rr_timer_is_running(entry->measurement->thresh_high_timer, gas_id))
          { 
             /* No action necessary. */
          ...
          {
            /*Check for the cell which is stronger than current one */
            for (k = 0; k < i; k++)
            {
              rr_irat_priority_list_entry_t *strongest_entry = &priority_list.entries[k];
              /* Entry is a high priority cell. */
              if (strongest_entry->s_value > strongest_entry->thresh_high && rr_irat_priority_list_entry_suitable(strongest_entry, gas_id))
              {
                 if ((rr_timer_is_running(strongest_entry->measurement->thresh_high_timer, gas_id)) &&
                     ((rr_timer_get_time_remaining(strongest_entry->measurement->thresh_high_timer, gas_id) < RR_G2X_TIMER_EXPIRY_WAVEOFF_TIME)))
                 {
                   entry = strongest_entry;
                   rr_timer_cancel(strongest_entry->measurement->thresh_high_timer, gas_id);
                   MSG_GERAN_MED_1_G("Strongest cell at index %d ",k);              
                   break;
                 }
              }         
            }
    ...
    {
      /* Entry belongs to a lower priority layer or sub is in slow IRAT mode - cancel thresh_high_timer if
      ...
      if (priority_resel_to_lower_layers_permitted)
      {
        if (entry->s_value > entry->thresh_low &&  entry_suitable)
        {
...
void rr_g2w_update_lte_arfcn_deprioritized_list(geran_grr_lte_depri_freq_req_type *lte_depri_freq_ptr,
                                                const gas_id_t gas_id)
{
  // Obtain a pointer to the module data
  rr_g2w_data_t *rr_g2w_data_ptr = rr_g2w_get_data_ptr(gas_id);
...
void rr_g2w_update_lte_dedicated_pri_info_with_depri_list(
...
void rr_g2w_depri_update_lte_about_requested_plmn(PLMN_id_T *requested_plmn_id,
  const gas_id_t gas_id)
{
  rr_g2w_data_t                      *rr_g2w_data_ptr;
