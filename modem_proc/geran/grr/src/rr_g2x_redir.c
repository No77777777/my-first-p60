...
============================================================================*/
/* $Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_g2x_redir.c#2 $ */
 ...
 */
rr_rat_e rr_g2x_redir_map_redir_type_to_target_rat( rr_cell_selection_target_e type  )
{
  rr_rat_e rat_type;
  ...
  switch(type)
  {
    case GSM_TARGET:
      rat_type = RR_RAT_GSM;
      MSG_GERAN_HIGH_0("Unexpected use of GSM target for G2X redir");
    break;
...
)
{
  ...
  if (!rr_is_any_cell_selection_mode(gas_id))
  {
    ...
    {
	  redir_req.nas_params.req_plmn = scell_info_ptr->gsm.location_area_identification.plmn_id;
    }
    else 
    {
  ...
  redir_req.redir_after_csfb = rr_is_redir_after_csfb_call(gas_id);
...
#endif /* FEATURE_SGLTE */
/**
  @brief checking g2x redirection state.
         TRUE: if redir state is in redirecting state, otherwise FALSE
*/
boolean rr_g2x_is_in_redirecting_state(const gas_id_t gas_id)
{
  rr_g2x_redir_data_t *rr_g2x_redir_data_ptr;
  ...
  return (rr_g2x_redir_data_ptr->state == RR_G2X_REDIR_STATE_REDIRECTING);
}
