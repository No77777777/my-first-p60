...
$Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_tds_send.c#2 $
...
#ifndef PERLUTF
    tds_cmd->cmd.interrat_plmn_srch_req.high_priority = high_priority;
    tds_cmd->cmd.interrat_plmn_srch_req.is_foreground_search = foreground_search;
#else
    #error code not present
#endif /*PERLUTF*/

    MSG_GERAN_HIGH_4_G("Sent TDSRRC_INTERRAT_PLMN_SRCH_REQ num_plmns:%d, nw_sel_mode:%d, timer:%d, Priority %d",num_plmns,network_select_mode,timer_val,high_priority);
...
/**
 * Send the TDSRRC_GRR_INTERRAT_PROC_ABORT_IND primitive to TDS-RRC
 *
 * @status: Abort Cnf cause
 */

void rr_send_tds_interrat_proc_abort_ind(
  tdsrrc_rr_status_e_type status,
  const gas_id_t gas_id
)
{
  ...
  if (rr_is_rat_segment_loaded(RR_SEG_LOAD_TDSCDMA) == FALSE)
  {
    MSG_GERAN_ERROR_1_G("RR Can't send RRC message %d, RRC not loaded",TDSRRC_GRR_INTERRAT_PROC_ABORT_IND);
    return;
  }
  ...
  tds_cmd->cmd_hdr.cmd_id = TDSRRC_GRR_INTERRAT_PROC_ABORT_IND;
  MSG_GERAN_MED_0_G("Sent TDSRRC_GRR_INTERRAT_PROC_ABORT_IND");
  rr_send_message_to_tds_rrc(tds_cmd, gas_id);
}
