...
$Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_sys_info_cache.c#1 $ 
...
#define NUM_SI_TYPES_OPTIONAL 1
...
static uint8 si_retrieval_order_optional[NUM_SI_TYPES_OPTIONAL] = {SYSTEM_INFORMATION_TYPE_2_QUATER
                                                                  };

static uint8 si_list[MAX_CSI_PER_CGI];
...
static const char * rr_print_si_name(const uint8 msg_type);
/*!
...
static csi_entry_t *rr_csi_find_multi_instance_entry(csi_entry_t *target_ptr)
{
  ...
  {
    (void)rr_csi_get_si_index(target_ptr,&req_si_index);
    ...
    for (i = 0; i < MAX_NUMBER_CACHED_SI; i++, entry_ptr++)
    {
      ...
      {
        (void)rr_csi_get_si_index(entry_ptr,&cached_si_index);
...
===========================================================================*/
csi_entry_t *rr_csi_find_entry(csi_entry_t *target_ptr)
{
  csi_entry_t *entry_ptr;
  int          i;
...
static const char * rr_print_si_name(const uint8 msg_type)
{
  ...
  switch (msg_type)
  {
    ...
    default:
    {
      MSG_GERAN_HIGH_1("Unexpected SI type: %d", msg_type);
    }
  }
  return s;
                                            ...
                                            const gas_id_t      gas_id)
{
  ...
  /*RR Wrapper function to get the MCC/MNC from LAI */
  rr_get_mcc_mnc(
    lai_ptr->plmn_id,
    &mcc,
    &mnc
  ...
  MSG_SPRINTF_2(MSG_SSID_DFLT,
                MSG_LEGACY_HIGH,
                "rr_csi_update_entry for ARFCN=%d, MSG:%s",arfcn.num,rr_print_si_name(msg_type));
#else
  MSG_GERAN_HIGH_5_G("rr_csi_update_entry for ARFCN=%d, PLMN=%d-%d, LAC=%d, CI=%d",
    arfcn.num,
    (int) mcc, 
    (int) mnc, 
    (int) lai_ptr->location_area_code,
    *ci_ptr
  );
  ...
  if (IS_MULTI_INSTANCE_SI(msg_type))
  {
    ...
    /* get the index for MULTI_INSTANCE_SI*/
    (void)rr_csi_get_si_index(&new_entry,&instance);
    MSG_GERAN_HIGH_1_G("rr_csi_update_entry instance:%d", instance);
  }
  else
  {
  ...
  {    
    ...
    MSG_SPRINTF_3(MSG_SSID_DFLT,
                  MSG_LEGACY_HIGH,
                  "rr_csi_update_entry update existing entry: 0x%x, ARFCN:%d, MSG:%s",                        
                  entry_ptr,
                  entry_ptr->si_arfcn.num,
                  rr_print_si_name(entry_ptr->msg_type));
#else
    MSG_GERAN_HIGH_3_G("rr_csi_update_entry update existing entry: 0x%x, ARFCN:%d, MSG:%d",
                        entry_ptr,
                        entry_ptr->si_arfcn.num,
                        entry_ptr->msg_type);
#endif /* FEATURE_GERAN_REDUCED_SPRINTF */
  ...
  else
  {
    ...
    if (entry_ptr != NULL)
    {
      ...
      {        
        ...
        *entry_ptr = new_entry;
        ...
        MSG_SPRINTF_3(MSG_SSID_DFLT,
                      MSG_LEGACY_HIGH,
                      "rr_csi_update_entry add to free entry: 0x%x, ARFCN:%d, MSG:%s",                        
                      entry_ptr,
                      entry_ptr->si_arfcn.num,
                      rr_print_si_name(entry_ptr->msg_type));
#else
        MSG_GERAN_HIGH_3_G("rr_csi_update_entry add to free entry: 0x%x, ARFCN:%d, MSG:%d",
                            entry_ptr,
                            entry_ptr->si_arfcn.num,
                            entry_ptr->msg_type);
#endif /* FEATURE_GERAN_REDUCED_SPRINTF */        
      ...
      else
      {
        ...
        MSG_SPRINTF_1(MSG_SSID_DFLT,
                      MSG_LEGACY_HIGH,
                      "Unable to cache MSG:%s, modem heap unavailable",                        
                      rr_print_si_name(msg_type));
#else
        MSG_GERAN_HIGH_1_G("Unable to cache MSG:%d, modem heap unavailable",
                           msg_type);
#endif /* FEATURE_GERAN_REDUCED_SPRINTF */
      }
    }
    else
    {
      ...
      MSG_SPRINTF_1(MSG_SSID_DFLT,
                    MSG_LEGACY_HIGH,
                    "Unable to cache MSG:%s, database full",                        
                    rr_print_si_name(msg_type));
#else
      MSG_GERAN_HIGH_1_G("Unable to cache MSG:%d, database full",
                         msg_type);
#endif /* FEATURE_GERAN_REDUCED_SPRINTF */
    }
  }
...
FUNCTION  rr_csi_init_si_list

DESCRIPTION
  Initializes the variables of rr_csi_init_si_list
...
===========================================================================*/
static void rr_csi_init_si_list(rr_csi_si_list_t *si_list_array)
{
  RR_NULL_CHECK_RETURN_VOID(si_list_array);

  /* Initializes si_list_array */
  si_list_array->sis = si_list;
  si_list_array->si2q_logged = FALSE;
  si_list_array->si2ter_logged = FALSE;
  si_list_array->num_entries = 0;
  si_list_array->max_entries = MAX_CSI_PER_CGI;
...
FUNCTION  rr_csi_add_si_to_list

DESCRIPTION
  Adds supplied SI MSG_TYPE to an array.
...
===========================================================================*/
static void rr_csi_add_si_to_list(
  uint8 msg_type,
  rr_csi_si_list_t *si_list_array_ptr,
  const gas_id_t gas_id
)
{
  if (si_list_array_ptr != NULL)
  {
    if (si_list_array_ptr->num_entries < si_list_array_ptr->max_entries)
    {
      switch (msg_type)
      {
        case SYSTEM_INFORMATION_TYPE_1:
        ...
        case SYSTEM_INFORMATION_TYPE_6:
        case SYSTEM_INFORMATION_TYPE_13:
          si_list_array_ptr->sis[si_list_array_ptr->num_entries] = msg_type;
          si_list_array_ptr->num_entries++;          
          break;
  ...
  } /* si_list_array_ptr != NULL */
}
/*===========================================================================

FUNCTION  rr_csi_populate_requested_si_to_flush

DESCRIPTION
  Populates an ARRAY with SI message types, from supplied internal SI mask 
...
===========================================================================*/
static void rr_csi_populate_requested_si_to_flush(
  uint16 requested_si_mask,
  uint8 *requested_si_to_flush,
  const gas_id_t gas_id
)
{
  uint8 i = 0;
  if (requested_si_mask != MASK_SI_NONE)
  {
    if (requested_si_mask & MASK_SI_1)
    {
      requested_si_to_flush[i] = SYSTEM_INFORMATION_TYPE_1;
      i++;
    }
    if (requested_si_mask & MASK_SI_2)
    {
      requested_si_to_flush[i] = SYSTEM_INFORMATION_TYPE_2;
      i++;
    }
    if (requested_si_mask & MASK_SI_2_BIS)
    {
      requested_si_to_flush[i] = SYSTEM_INFORMATION_TYPE_2_BIS;
      i++;
    }
    if (requested_si_mask & MASK_SI_2_TER)
    {
      requested_si_to_flush[i] = SYSTEM_INFORMATION_TYPE_2_TER;
      i++;
    }
    if (requested_si_mask & MASK_SI_2_QUATER)
    {
      requested_si_to_flush[i] = SYSTEM_INFORMATION_TYPE_2_QUATER;
      i++;
    }
    if (requested_si_mask & MASK_SI_3)
    {
      requested_si_to_flush[i] = SYSTEM_INFORMATION_TYPE_3;
      i++;
    }
    if (requested_si_mask & MASK_SI_4)
    {
      requested_si_to_flush[i] = SYSTEM_INFORMATION_TYPE_4;
      i++;
    }
    if (requested_si_mask & MASK_SI_13)
    {
      requested_si_to_flush[i] = SYSTEM_INFORMATION_TYPE_13;
      i++;
    }
  }
  /* Fill the remained array with 0xFF */
  for(; i < MAX_CSI_PER_CGI ;i++)
  {
    requested_si_to_flush[i] = 0xFF;
    i++;
  }
}
...
FUNCTION  rr_csi_populate_flushed_si
...
===========================================================================*/
static void rr_csi_populate_flushed_si(
  csi_entry_t *entry_ptr,
  rr_csi_si_list_t *si_list_array_ptr,
  const gas_id_t gas_id)
{
  RR_NULL_CHECK_RETURN_VOID(entry_ptr);
  RR_NULL_CHECK_RETURN_VOID(si_list_array_ptr);

  if (IS_MULTI_INSTANCE_SI(entry_ptr->msg_type))
  {
    if ((!si_list_array_ptr->si2q_logged)&& (entry_ptr->msg_type == SYSTEM_INFORMATION_TYPE_2_QUATER))
    {
      /* Add SI MSG to array */
      (void)rr_csi_add_si_to_list(entry_ptr->msg_type,si_list_array_ptr,gas_id);
      si_list_array_ptr->si2q_logged = TRUE;
    }
    if ((!si_list_array_ptr->si2ter_logged) && (entry_ptr->msg_type == SYSTEM_INFORMATION_TYPE_2_TER))
    {
      /* Add SI MSG to array */
      (void)rr_csi_add_si_to_list(entry_ptr->msg_type,si_list_array_ptr,gas_id);
      si_list_array_ptr->si2ter_logged = TRUE;
    }
  }
  else
  {
...
DESCRIPTION
  Maps an internal SI mask to to SI message type
...
===========================================================================*/
static uint8 rr_csi_map_bitmask_to_msg_type(uint16 si_bitmask)
{
  uint8 msg_type = INVALID_SYSTEM_INFO_MESSAGE;
  if (si_bitmask & MASK_SI_1)
  {
    msg_type = SYSTEM_INFORMATION_TYPE_1;
  }
  else if (si_bitmask & MASK_SI_2)
  {
    msg_type = SYSTEM_INFORMATION_TYPE_2;
  }
  else if (si_bitmask & MASK_SI_2_BIS)
  {
    msg_type = SYSTEM_INFORMATION_TYPE_2_BIS;
  }
  else if ((si_bitmask & MASK_SI_2_TER) )
  {  
    msg_type = SYSTEM_INFORMATION_TYPE_2_TER;
  }
  else if (si_bitmask & MASK_SI_2_QUATER)
  { 
    msg_type = SYSTEM_INFORMATION_TYPE_2_QUATER;
  }
  else if (si_bitmask & MASK_SI_3)
  {
    msg_type = SYSTEM_INFORMATION_TYPE_3;
  }
  else if (si_bitmask & MASK_SI_4)
  {
    msg_type = SYSTEM_INFORMATION_TYPE_4;
  }
  else if (si_bitmask & MASK_SI_13)
  {
    msg_type = SYSTEM_INFORMATION_TYPE_13;
  }
  else
  {
    return msg_type;
...
FUNCTION  rr_csi_log_flush_event_and_packet

DESCRIPTION
  Logs an EVENT_GSM_SI_CACHE_FLUSH and LOG_GSM_RR_CSI_CACHE_FLUSH_DATA_C
...
void rr_csi_log_flush_event_and_packet(
  csi_entry_t *entry_ptr,
...
)
{
  ...
  /* Make sure EVENT is logged only if SI is flushed */
  if (si_list_array->num_entries > 0)
  {   
    for (; si_list_array->num_entries < MAX_CSI_PER_CGI ;si_list_array->num_entries++)
    {
      si_list_array->sis[si_list_array->num_entries] = 0xFF;     
    }
    /* Logs an EVENT_GSM_SI_CACHE_FLUSH */
    (void)rr_log_csi_flush_data(entry_ptr,csi_flush_cause,gas_id);
    /* log packet LOG_GSM_RR_CSI_CACHE_FLUSH_DATA_C */
    rr_log_csi_cache_flush_data(entry_ptr,csi_flush_cause,requested_si_to_flush,si_list_array->sis,gas_id);
  }
  else
  {
    MSG_GERAN_MED_0_G("Not logging an EVENT or LOG_PACKET as nothing flushed");    
  }          
}
...
{
  csi_entry_t      *entry_ptr;
  csi_entry_t      *local_entry_ptr = NULL; 
  uint16           i;
  rr_csi_cgi_t     requested_cgi;
  rr_csi_si_list_t si_list_array;
  uint8            *requested_si_to_flush;
  uint16           requested_si_to_flush_mask =MASK_SI_ALL;
  boolean          first_entry = FALSE;

  sys_mcc_type mcc = 0;
  sys_mnc_type mnc = 0;

  /* use local buffer for a list of SIs */
  requested_si_to_flush = si_list; 
  ...
  {
    /* Populate SI array with SIs been requested to flush*/
    (void)rr_csi_populate_requested_si_to_flush(requested_si_to_flush_mask,requested_si_to_flush,gas_id);      
    ...
    MSG_GERAN_HIGH_5_G("RR flush SI of PLMN=%d-%d, LAC=%d, CI=%d for CAUSE=%d",     
      (int) mcc, 
      (int) mnc, 
      (int) lai_ptr->location_area_code,
      *ci_ptr,
    ...
    entry_ptr = sys_info_cache;
    local_entry_ptr = entry_ptr;
   
    /* Initializes si_list_array */
    rr_csi_init_si_list(&si_list_array);
    ...
    for (i = 0; i < MAX_NUMBER_CACHED_SI; i++, entry_ptr++)
    {
      if (IS_VALID_OTA_ENTRY(entry_ptr))
      {
        if (rr_csi_is_cgi_equal(entry_ptr->cgi,requested_cgi))
        {
          if (!first_entry)
          {
            /* Take the copy of entry_ptr which being used to log the EVENT */
            local_entry_ptr = entry_ptr;  
            first_entry = TRUE;      
          }
          /* Populate SI_LIST with SIs being flushed */
          (void)rr_csi_populate_flushed_si(entry_ptr,&si_list_array,gas_id);
          (void)rr_csi_discard_entry_data(entry_ptr);
        }
      }
    }
    if (local_entry_ptr != NULL)
    {
      /* log flush event and log packet */
      rr_csi_log_flush_event_and_packet(local_entry_ptr,csi_flush_cause,requested_si_to_flush,&si_list_array,gas_id);
    }
  }
...
FUNCTION  rr_csi_flush_si_data

DESCRIPTION
  Discards requested SI for a specified cell
...
)
{
  if ( (lai_ptr == NULL) ||
       (ci_ptr == NULL) )
  {
    MSG_GERAN_ERROR_2_G("NULL ptr supplied can't Flush SI's, LAI ptr:0x%x, CELL_ID ptr:0x%x", lai_ptr, ci_ptr);
  }
  else
  {
    if (msg_type != INVALID_SYSTEM_INFO_MESSAGE)
    {
      uint16  requested_si_to_flush_mask = MASK_SI_NONE;

      /* Get the SI MASK for SI MSG */
      requested_si_to_flush_mask = rr_csi_map_msg_type_to_mask(msg_type);
      /* Flush SI messages which need to be updated */
      rr_csi_flush_multiple_si_data(arfcn,                       // ARFCN
                                    requested_si_to_flush_mask,  // SI_MASK
                                    lai_ptr,                     // LAI
                                    ci_ptr,                      // CI
                                    csi_flush_cause,             // FLUSH_CAUSE 
                                    gas_id);

    }
    else 
    {
      MSG_GERAN_HIGH_1_G("Invalid MSG_TYPE: %d",msg_type);
    }
  }
}
...
DESCRIPTION
  Discards requested multiple SIs for a specified cell
...
===========================================================================*/
void rr_csi_flush_multiple_si_data(
  const ARFCN_T arfcn,
...
)
{
  ...
  uint16           requested_si_to_flush_mask = flush_si_mask;
  byte             local_msg_type;
  uint16           i;
  uint8            *requested_si_to_flush;
  boolean          first_entry = FALSE;
  ...
  MSG_GERAN_HIGH_5_G("RR flush SI for PLMN=%d-%d, LAC=%d, CI=%d for CAUSE=%d", 
    (int) mcc, 
    (int) mnc, 
    (int) lai_ptr->location_area_code,
    *ci_ptr,
  ...
  /* Populate SI array with SIs requested to flush*/
  (void)rr_csi_populate_requested_si_to_flush(requested_si_to_flush_mask,requested_si_to_flush,gas_id);
  
  csi_entry.si_arfcn =  arfcn;
  csi_entry.cgi.lai  = *lai_ptr;
  csi_entry.cgi.ci   = *ci_ptr;
  ...
  RR_SI_PRINT_BITMASK("SIs requested to flush", requested_si_to_flush_mask);

  /* Only retrieve this SI message if it's not excluded */
  while ( requested_si_to_flush_mask != MASK_SI_NONE)
  {
    /* Get the MSG type need to be flushed */
    local_msg_type = rr_csi_map_bitmask_to_msg_type(requested_si_to_flush_mask);
        
    if (local_msg_type != INVALID_SYSTEM_INFO_MESSAGE)
    {
      csi_entry.msg_type = local_msg_type;
      /* Get the entry pointer of the requested SI*/
      entry_ptr = rr_csi_find_entry(&csi_entry);

      /* loop through the cache and delete the requested SI and early break*/
      if (entry_ptr != NULL)
      { 
        for (i = 0; i < MAX_NUMBER_CACHED_SI; i++,entry_ptr++)
        {
          /* This is already done in rr_csi_find_entry() but checking again*/
          if((entry_ptr->msg_type == local_msg_type) &&
              IS_VALID_OTA_ENTRY(entry_ptr) &&
              ARFCNS_EQUAL(arfcn,entry_ptr->si_arfcn) &&
              rr_csi_is_cgi_equal(entry_ptr->cgi,requested_cgi))
          {
            if (!first_entry)
            {
              /* Take the copy of entry_ptr which being used to log the EVENT */
      ...
      /* Clear the mask for the msg which already deleted*/
      si_mask = rr_csi_map_msg_type_to_mask(local_msg_type);
      requested_si_to_flush_mask &= ~(si_mask);
    }
    else
    { 
      requested_si_to_flush_mask = MASK_SI_NONE;
    }
  }
  ...
  {
    MSG_GERAN_MED_0_G("Requested SIs not flushed");    
  }          
}
...
void rr_csi_init_cache(void)
{
  ...
  /* Initializes si_list_array, used for logging flush event and log packet*/
  rr_csi_init_si_list(&si_list_array);
  return;
}
...
RETURN VALUE
  SI mask indicating the SI messages retrieved from the cache

===========================================================================*/
uint16 rr_csi_retrieve_all_SIs(
  ARFCN_T arfcn,
  LAI_T *lai_ptr,
  uint16 *cell_id_ptr,
...
)
{
  ...
  uint16 all_si_mask = MASK_SI_NONE;
  ...
  if (rr_nv_sys_info_cache_is_enabled(RR_GAS_ID_TO_AS_ID) == FALSE)
  {
    MSG_GERAN_HIGH_0_G("No SI retrieve as SI caching feature is disabled");
    return 0;
  }
  ...
  rr_get_mcc_mnc(
   ...
   MSG_GERAN_HIGH_5_G("RR retrieve all SIs for arfcn=%d PLMN=%d-%d, LAC=%d, CI=%d",
    arfcn.num,
    (int) mcc, 
    (int) mnc, 
    (int) lai_ptr->location_area_code,
  ...
  else if (SIs_to_retrieve == CSI_RETRIEVE_OPTIONAL_SIS)
  {
    si_retrieval_order_ptr = si_retrieval_order_optional;
    num_si_types_to_retrieve = NUM_SI_TYPES_OPTIONAL;
  }
  else
  {
  ...
  {
    byte msg_type = si_retrieval_order_ptr[i];
    uint16 si_mask = rr_csi_map_msg_type_to_mask(msg_type);

    // Only retrieve this SI message if it's not excluded
    if ((si_mask & (~exclude_si_mask)) != MASK_SI_NONE)
    {
#ifdef PERLUTF
  ...
  /*if retrieving optional SI's only the WCDMA update should be handled autonomously */
  if (is_multiple_instance_si_retrieved && (SIs_to_retrieve != CSI_RETRIEVE_OPTIONAL_SIS) )
  {
    rr_send_imsg(RR_IMSG_CSI_MULTIPLE_INSTANCES_SIS_RETRIEVED_IND, gas_id);
  }

  if (all_si_mask != MASK_SI_NONE)
  {
    MSG_GERAN_HIGH_1_G("SIs retrived from cache are %d",all_si_mask);
  }
...
boolean rr_csi_retrieve_SI3_4(ARFCN_T arfcn, LAI_T *lai_ptr, uint16 *cell_id_ptr, const gas_id_t gas_id)
{
  ...
  if (rr_nv_sys_info_cache_is_enabled(RR_GAS_ID_TO_AS_ID) == FALSE)
  {
    MSG_GERAN_HIGH_0_G("No SI3/SI4 retrieve as SI caching feature is disabled");
    return FALSE;
  }
  ...
  MSG_GERAN_HIGH_4_G("RR retrieve SI3/4 for PLMN=%d-%d, LAC=%d, CI=%d", 
    (int) mcc, 
    (int) mnc, 
    (int) lai_ptr->location_area_code,
    *cell_id_ptr
  ...
  if (entry_ptr != NULL)
  {
    ...
    MSG_SPRINTF_3(MSG_SSID_DFLT,
                  MSG_LEGACY_HIGH,
                  "rr_csi_retrieve_SI3_4 Send entry entry_ptr:0x%x, ARFCN:%d, MSG:%s",                        
                  entry_ptr,
                  entry_ptr->si_arfcn.num,
                  rr_print_si_name(entry_ptr->msg_type));
#else
    MSG_GERAN_HIGH_3_G("rr_csi_retrieve_SI3_4 Send entry entry_ptr:0x%x ARFCN:%d, MSG:%d",
                       entry_ptr,
                       entry_ptr->si_arfcn.num,
                       entry_ptr->msg_type);
#endif /* FEATURE_GERAN_REDUCED_SPRINTF */ 
  ...
  for (i = 0; i < NELEMS(pend_store_ptr->store); i++)
  {
    ...
    if (IS_VALID_OTA_ENTRY(entry_ptr) == FALSE)
    {
      ...
      {        
        ...
        MSG_SPRINTF_3(MSG_SSID_DFLT,
                      MSG_LEGACY_HIGH,
                      "rr_csi_add_entry_to_pend_store add to free entry: 0x%x, ARFCN:%d, MSG:%s",
                      entry_ptr,
                      entry_ptr->si_arfcn.num,
                      rr_print_si_name(entry_ptr->msg_type));
#else
        MSG_GERAN_HIGH_3_G("rr_csi_add_entry_to_pend_store add to free entry: 0x%x, ARFCN:%d, MSG:%d",
                           entry_ptr,
                           entry_ptr->si_arfcn.num,
                           entry_ptr->msg_type);
#endif /* FEATURE_GERAN_REDUCED_SPRINTF */
                                     ...
                                     const gas_id_t      gas_id)
{
  ...
  MSG_GERAN_HIGH_5_G("RR copy pend store to cache for PLMN=%d-%d, ARFCN=%d, LAC=%d, CI=%d",
    (int) mcc, 
    (int) mnc,
    arfcn.num,
    (int) lai_ptr->location_area_code,
...
void rr_csi_retrieve_arfcns_for_plmn(LAI_T *lai_ptr, rr_csi_arfcn_list_t *list_ptr, const gas_id_t gas_id)
{
  if ((lai_ptr != NULL) && (list_ptr != NULL))
  {
    ARFCN_T *output_ptr;

    /* initialise output (max_entries should be set by the caller) */
    list_ptr->num_entries = 0;
    output_ptr = list_ptr->arfcns;

    /* validate the output structure before using it */
    if ((output_ptr != NULL) && (list_ptr->max_entries > 0))
    {
      csi_entry_t *entry_ptr;
      int          i;

      /* spin through the cache */
      entry_ptr = sys_info_cache;
      for (i = 0; i < MAX_NUMBER_CACHED_SI; i++, entry_ptr++)
      {
        /* match any SI3 entry with the same PLMN ID */
        if ((entry_ptr->msg_type                    == SYSTEM_INFORMATION_TYPE_3)&&
            (entry_ptr->cgi.lai.plmn_id.identity[0] == lai_ptr->plmn_id.identity[0]) &&
            (entry_ptr->cgi.lai.plmn_id.identity[1] == lai_ptr->plmn_id.identity[1]) &&
            (entry_ptr->cgi.lai.plmn_id.identity[2] == lai_ptr->plmn_id.identity[2]))
        {
          /* if there is enough storage space, copy the ARFCN for the cache entry */
          if (list_ptr->num_entries < list_ptr->max_entries)
          {
            *output_ptr = entry_ptr->si_arfcn;
            MSG_GERAN_HIGH_1_G("rr_csi_retrieve_arfcns_for_plmn arfcn=%d ",output_ptr->num);
            output_ptr++;
            list_ptr->num_entries++;
          }
          else
          {
            /* storage is full, return with partial result */
            return;
          }
        }
      }
