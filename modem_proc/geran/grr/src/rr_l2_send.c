...
$Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_l2_send.c#1 $ $DateTime: 2019/06/06 23:27:31 $ $Author: mplcsds1 $
...
/*!
 * \brief Copies a L3 message from one buffer to another.
 * 
 * \param dst_ptr 
 * \param dst_len 
 * \param src_ptr 
 * \param src_len 
 */
static void copy_l3_msg(uint8 *dst_ptr, uint8 dst_len, uint8 *src_ptr, uint8 src_len)
{
...
)
{
  ...
  copy_l3_msg(
    req_ptr->layer3_message,          // dst_ptr
    sizeof(req_ptr->layer3_message),  // dst_len
    msg_ptr,                          // src_ptr
    msg_len                           // src_len
  );
...
)
{
  ...
  if (SAPI == SAPI0)
  {
    ...
    copy_l3_msg(
      dl_establish_req.layer3_message,          // dst_ptr
      sizeof(dl_establish_req.layer3_message),  // dst_len
      layer3_message,                           // src_ptr
      dl_establish_req.l3_message_length        // src_len
    );
  }
  else
  {
...
)
{
  ...
  copy_l3_msg(
    dl_reconnect_req.layer3_message,          // dst_ptr
    sizeof(dl_reconnect_req.layer3_message),  // dst_len
    layer3_message,                           // src_ptr
    message_length                            // src_len
  );
...
)
{
  ...
  copy_l3_msg(
    dl_resume_req.layer3_message,          // dst_ptr
    sizeof(dl_resume_req.layer3_message),  // dst_len
    layer3_message,                        // src_ptr
    message_length                         // src_len
  );
...
)
{
  ...
  copy_l3_msg(
    dl_unit_data_req.layer3_message,          // dst_ptr
    sizeof(dl_unit_data_req.layer3_message),  // dst_len
    layer3_message,                           // src_ptr
    message_length                            // src_len
  );
