...
*** TITLE
***
***  $Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_gprs_debug.c#5 $
***
*** DESCRIPTION
***
...
static rr_cmd_bdy_type message_algn __attribute__((aligned(4)));
...
const char * rr_gprs_debug_msgr_umid_name(msgr_umid_type umid)
{
  switch (umid)
  {
    ...
    case LTE_RRC_G_ABORT_REDIR_REQ:              return "LTE_RRC_G_ABORT_REDIR_REQ";
    ...
    case LTE_RRC_G_GET_PLMN_PRTL_RESULTS_REQ:    return "LTE_RRC_G_GET_PLMN_PRTL_RESULTS_REQ";
...
#endif /* FEATURE_SGLTE */
    /* This message could come in non sg-lte mode too */
...
#ifdef FEATURE_GSM_CXM_L3_STATE
    case MCS_CXM_COEX_TECH_L3_STATE_IND:         return "MCS_CXM_COEX_TECH_L3_STATE_IND";
#endif /* FEATURE_GSM_CXM_L3_STATE */
    case MCS_CXM_COEX_TECH_STATE_BCAST_IND:      return "MCS_CXM_COEX_TECH_STATE_BCAST_IND";

    /* DRX level query */
    case GERAN_GRR_MTC_DRX_REQ:                  return "GERAN_GRR_MTC_DRX_REQ";
    case GERAN_GRR_MTC_DRX_RSP:                  return "GERAN_GRR_MTC_DRX_RSP";
...
#ifdef FEATURE_PRIORITIZE_LTE_FREQ
    case LTE_RRC_ACQ_DB_EARFCN_REQ:              return "LTE_RRC_ACQ_DB_EARFCN_REQ";
    case LTE_RRC_ACQ_DB_EARFCN_RSP:              return "LTE_RRC_ACQ_DB_EARFCN_RSP";
    case QMI_NAS_PRIORITIZE_LTE_IND:             return "QMI_NAS_PRIORITIZE_LTE_IND";
#endif /* FEATURE_PRIORITIZE_LTE_FREQ */

#ifdef FEATURE_FAKE_BTS_CELL_DEPRIORITIZE
    case GERAN_GRR_DEPRIORITIZE_GCELL_REQ:       return "GERAN_GRR_DEPRIORITIZE_GCELL_REQ";
      ...
      case MS_RRC_RR:
      {
        switch (message_id)
        {
          ...
          case RRC_GRR_INTERRAT_PROC_ABORT_IND:
            s = "RRC_GRR_INTERRAT_PROC_ABORT_IND";
            break;
      ...
      case MS_MM_RR:
      {
        switch (message_id)
        {
          ...
          case RR_GMM_RETX_PDU_TUNEAWAY_FAIL_IND:
            s = "RR_GMM_RETX_PDU_TUNEAWAY_FAIL_IND";
          break;
        ...
        switch (message_id)
        {
         ...
         case MPH_STOP_GSM_MODE_CNF:
          ...
          case MPH_VOICE_SAMPLE_RATE_CHANGE_IND:		  	
            s = "MPH_VOICE_SAMPLE_RATE_CHANGE_IND";
            break;
          ...
          case MPH_PREEMPTION_IND:
            s = "MPH_PREEMPTION_IND";
            break;

          case MPH_ACQUISITION_STATE_IND:
            s = "MPH_ACQUISITION_STATE_IND";
            break;
#endif /* FEATURE_DUAL_SIM || FEATURE_TRIPLE_SIM */

          case MPH_IDLE_COLL_FALLBACK_IND:
            s = "MPH_IDLE_COLL_FALLBACK_IND";
            break;
          ...
          switch (msg_ptr->rr.header.rr_message_set)
          {
            ...
            case RR_CELL_ACQ_IMSG:
            {
              switch (msg_ptr->rr.cell_acq.imsg.header.imh.message_id)
              {
                ...
                {
                  s = "RR_CELL_ACQ_IMSG_NACC_ACQ_STATUS_IND";
                  break;
                }
      ...
      case MS_TDSRRC_RR:
      {
        switch (message_id)
        {
          ...
          case TDSRRC_GRR_INTERRAT_PROC_ABORT_IND:
            s = "TDSRRC_GRR_INTERRAT_PROC_ABORT_IND";
            break;
          default:
...
)
{
...
#ifdef FEATURE_QSH_EVENT_METRIC
  /* Do not print QSH timer expiry indications to avoid excessive logging */
  if (message_ptr != NULL && message_set == MS_RR_RR &&
      message_ptr->rr.header.rr_message_set == RR_IMSG)     
  {
    if ((message_id == RR_IMSG_QSH_METRIC_TIMER_EXPIRY_IND) ||
        (message_id == RR_IMSG_QSH_METRIC_CONFIG_IND))
    {
      return;
    }
  }
...
#ifdef FEATURE_GERAN_REDUCED_SPRINTF
  boolean log_ota_message = FALSE;

  if (message_ptr != NULL && message_set == MS_RR_L2 &&
      ((message_id == (byte) DL_UNIT_DATA_IND) || (message_id == (byte) DL_DATA_IND)))
  {
    /* Print the OTA message string instead of RR_L2 message id */
    log_ota_message = TRUE;
  }
...
#else
    MSG_GERAN_HIGH_2_G("Timer %d expired in state %d",
                       message_ptr->mid_timer_expiry.timer_id,
                       state);
#endif /* FEATURE_GERAN_REDUCED_SPRINTF */
  }
  ...
  {
    MSG_GERAN_ERROR_0_G("Message pointer is NULL");
  }
...
)
{
  ...
  switch(msg_level)
  {
...
#endif /* FEATURE_GERAN_REDUCED_SPRINTF */
      if (!msg_status(MSG_SSID_DFLT, MSG_MASK_5))
      {
    ...
    case MSG_LEGACY_MED:
    {
      ...
      if (!msg_status(MSG_SSID_DFLT, MSG_MASK_6))
      {
        MSG_3(
          MSG_SSID_DFLT,
          MSG_LEGACY_MED,
    ...
    case MSG_LEGACY_HIGH:
    {
      ...
      if (!msg_status(MSG_SSID_DFLT, MSG_MASK_7))
      {
        MSG_3(
          MSG_SSID_DFLT,
          MSG_LEGACY_HIGH,
...
char * rr_event_name(const rr_event_T event)
{
  ...
  switch (event)
  {
    ...
    case RR_EV_TRIGGER_CELL_SELECTION_ACQ_DB_ONLY:
      s = "RR_EV_TRIGGER_CELL_SELECTION_ACQ_DB_ONLY";
      break;
    ...
    case EV_TRIGGER_CELL_SELECTION_ACQ_DB_ONLY:
      s = "EV_TRIGGER_CELL_SELECTION_ACQ_DB_ONLY";
      break;
    ...
    case RR_EV_START_RPS_AFTER_DENIAL:
      s = "RR_EV_START_RPS_AFTER_DENIAL";
      break;
    ...
    */
    ...
    case EV_ACCESS_CLASS_NOT_ALLOWED_EMERGECNY_CALL:
      s = "EV_ACCESS_CLASS_NOT_ALLOWED_EMERGECNY_CALL";
      break;
...
/*!
 * \brief Prints the sys band mask to diag.
 *
 * \param band_mask (in)
 */
void rr_debug_print_sys_band_mask(char *descr_text, sys_band_mask_type band_mask)
{
#ifndef FEATURE_GERAN_REDUCED_SPRINTF
  if (descr_text != NULL)
  {
  ...
  if (band_mask == SYS_BAND_MASK_EMPTY)
  {
    MSG_GERAN_HIGH_0("  (none)");
  }
  if (band_mask & SYS_BAND_MASK_GSM_PGSM_900)
  {
    MSG_GERAN_HIGH_0("  PGSM 900");
  }
  if (band_mask & SYS_BAND_MASK_GSM_EGSM_900)
  {
    MSG_GERAN_HIGH_0("  EGSM 900");
  }
  if (band_mask & SYS_BAND_MASK_GSM_DCS_1800)
  {
    MSG_GERAN_HIGH_0("  DCS 1800");
  }
  if (band_mask & SYS_BAND_MASK_GSM_PCS_1900)
  {
    MSG_GERAN_HIGH_0("  PCS 1900");
  }
  if (band_mask & SYS_BAND_MASK_GSM_850)
  {
    MSG_GERAN_HIGH_0("  GSM 850");
  }
  else
  {
    MSG_GERAN_HIGH_1("  0x%x",(int)band_mask);
  }
...
/*!
 * \brief Prints the sys band to diag.
 *
 * \param band_mask (in)
 */
void rr_debug_print_sys_band(char *descr_text, sys_band_T band)
{
#ifndef FEATURE_GERAN_REDUCED_SPRINTF
  if (descr_text != NULL)
  {
  ...
  if (band == SYS_BAND_NONE)
  {
    MSG_GERAN_HIGH_0("  (none)");
  }
  else if (band == SYS_BAND_EGSM_900)
  {
    MSG_GERAN_HIGH_0("  EGSM 900");
  }
  else if (band == SYS_BAND_PGSM_900)
  {
    MSG_GERAN_HIGH_0("  PGSM 900");
  }
  else if (band == SYS_BAND_DCS_1800)
  {
    MSG_GERAN_HIGH_0("  DCS 1800");
  }
  else if (band == SYS_BAND_PCS_1900)
  {
    MSG_GERAN_HIGH_0("  PCS 1900");
  }
  else if (band == SYS_BAND_CELL_850)
  {
    MSG_GERAN_HIGH_0("  GSM 850");
  }
  else
  {
    MSG_GERAN_HIGH_1("  0x%x",(int)band);
  }
  
  return;
}

void rr_debug_print_candidate_cell(const rr_candidate_cell_T *candidate_cell_ptr)
{
#ifdef PERLUTF
  #error code not present
#else // PERLUTF
  RR_NULL_CHECK_RETURN_VOID(candidate_cell_ptr);

  MSG_GERAN_HIGH_3("candidate cell %d type=%d processed=%d",
                   (int)candidate_cell_ptr->meas.arfcn.num,
                   (int)candidate_cell_ptr->type,
                   (int)candidate_cell_ptr->processed);
#endif // PERLUTF

  return;
}

void rr_debug_print_candidate_cell_db(const rr_candidate_cell_db_T *candidate_cell_db_ptr)
{
#ifdef PERLUTF
  #error code not present
#endif // PERLUTF
...
#ifdef FEATURE_QSH_MDUMP
QSH_MDUMP_FN_ATTR
void grr_mdump_rr_gprs_debug(const gas_id_t gas_id)
{
  rr_gprs_debug_data_t *rr_gprs_debug_data_ptr = rr_gprs_debug_get_data_ptr(gas_id);
  qsh_mdump_collect_high(rr_gprs_debug_data_ptr, sizeof(rr_gprs_debug_data_t));
}
