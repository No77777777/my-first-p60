...
*/
/* $Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_sys_info_pacch.c#1 $ */
...
)
{
      ...
      (scell_info_ptr->gsm.two_ti != RR_SI2TER_NOT_EXPECTED))
  {
    ...
    if (si2ter_inst_ptr->num_instances)
    {
      ...
      {
        if (si2ter_inst_ptr->instance[i] && 
            si2ter_inst_ptr->instance[i]->valid_data)
        {
      ...
      else
      {
        ...
        else
        {
          ...
          {
            if (si2ter_inst_ptr->instance[msg_index] && 
                si2ter_inst_ptr->instance[msg_index]->valid_data)
            {
      ...
      (scell_info_ptr->gsm.two_quater_position != SI2Q_POSITION_NONE))
  {
    ...
    if (si2q_inst_ptr->num_instances)
    {
      ...
      {
        if (si2q_inst_ptr->instance[i] && si2q_inst_ptr->instance[i]->valid_data)
        {
      ...
      else
      {
        ...
        else
        {
          ...
          {
            SI2q_MI_instance_T *instance = si2q_inst_ptr->instance[msg_index];
        
            if (instance && instance->valid_data)
            {
              bitmap |= (uint16) (1U << instance->common_params.INDEX);
            }
          }
...
)
{
  ...
  *******************************************************************/
  if (MASK_SI_NONE == rr_si_get_remaining_si_bitmask(rr_gprs_get_scell_info(gas_id)))
  {
...
)
{
  if ((rr_macinfo_get_mac_status(gas_id)            == MAC_TRANSFER) && /* GPRS transfer in progress */
      (FALSE                            == mcfg_gcf_nv_get_status()) && /* GCF flag is turned OFF */
      (scell_ptr                                    != NULL))
  {
    /* reset the defaults for a single sys info status message */
    rr_gprs_set_sysinfo_status_params(
...
)
{
  ...
  /* After all required SI's are received, only in case of EDTM activity, 
     the final SI_STATUS indication has to be sent . */
  if (MASK_SI_NONE != rr_si_get_remaining_si_bitmask(rr_gprs_get_scell_info(gas_id))
#if defined(FEATURE_GSM_DTM) && defined(FEATURE_GSM_EDTM)
      || rr_gprs_use_sysinfo_status_enhanced_priority(gas_id)
#endif /* FEATURE_GSM_DTM && FEATURE_GSM_EDTM */    
  )
  {
  ...
  {
    /* No EDTM activity and all required SI's are received. */
    bStartSecondary = FALSE;
  }
