...
============================================================================*/
/* $Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_msgr.c#1 $ */
...
static void rr_msgr_show_output_message(msgr_hdr_struct_type *msg_ptr, const gas_id_t gas_id)
{
...
#ifdef FEATURE_GSM_CXM_L3_STATE
  if (MCS_CXM_COEX_TECH_L3_STATE_IND == msg_ptr->id)
  {
    rr_show_output_message("MCS CXM", NULL_PTR, MS_MSGR_RR, msg_ptr->id, gas_id);
  }
  else
#endif /* FEATURE_GSM_CXM_L3_STATE */
  if ((GERAN_GRR_MTC_DRX_RSP == msg_ptr->id)
#ifdef FEATURE_FAKE_BTS_CELL_INFO
      ||(GERAN_GRR_QMI_CELL_INFO_IND == msg_ptr->id)||(GERAN_GRR_QMI_CELL_INFO_RSP == msg_ptr->id)
  ...
  else
  {
    ...
    {
      /* output debug if the GAS ID is not capable of inter-op with LTE */
      MSG_GERAN_ERROR_0_G("Using MSGR for non-LTE subscription");
    }
  }
...
/* Although the variant is set during msg_init_hdr, for safety we are setting it again */
  msgr_set_hdr_variant(msg_ptr, as_id);
