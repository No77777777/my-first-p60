...
==============================================================================*/
/* $Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_resel_nacc.c#1 $ */
...
)
{
  ...
  if (si_db_ptr != NULL)
  {
     ...
     */
    if (si_db_ptr->SI2_BA_list.BA_IND & INVALID_BA_IND)
    {
      bsi_mask |= MASK_SI_2;
    }
    
    if ((si_db_ptr->SI2bis_BA_list.BA_IND & INVALID_BA_IND) &&
        (si_db_ptr->decode_SI2bis != RR_SI_NOT_REQUIRED))
    {
      bsi_mask |= MASK_SI_2_BIS;
    }
    
    if ((si_db_ptr->SI2ter_BA_list.BA_IND & INVALID_BA_IND) &&
...
#else
  MSG_GERAN_HIGH_1_G("rr_gprs_nacc_notify_event(event_in => %d)", event_in);
#endif /* FEATURE_GERAN_REDUCED_SPRINTF */
...
#else
    MSG_GERAN_HIGH_2_G("nacc_state(%d->%d)",
                       rr_resel_nacc_data_ptr->old_nacc_state,
                       rr_resel_nacc_data_ptr->nacc_state);
#endif /* FEATURE_GERAN_REDUCED_SPRINTF */
