...
*** TITLE
***
*** $Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_gprs_conn_establish.c#3 $
***
*** DESCRIPTION
***
...
void rr_gprs_two_part_assignment_failure(rr_ce_access_status_t access_status, const gas_id_t gas_id)
{
  rr_tbf_est_static_data_t *rr_tbf_est_data_ptr;
  ...
  {
    if (RR_CE_ACCESS_INACTIVE == access_status)
    {
...
static void rr_gprs_two_part_assignment_complete(rr_ce_access_status_t access_status, const gas_id_t gas_id)
{
  rr_tbf_est_static_data_t *rr_tbf_est_data_ptr;
    ...
    *waiting_for_2nd_msg = FALSE;
    ...
    if (tbf_info_ptr->tma == 0)
    {
      ...
      *single_IA_msg = FALSE;
      ...
      rr_qsh_event_notify(RR_QSH_EVENT_IA_PART2_RCVD, gas_id);
#endif //FEATURE_QSH_EVENT_NOTIFY_HANDLER

      rr_gprs_two_part_assignment_complete(access_status, gas_id);
    }
    else
    {
      ...
      rr_gprs_two_part_assignment_failure(access_status, gas_id);
...
#ifdef FEATURE_QSH_EVENT_NOTIFY_TO_QSH 
      rr_qsh_event_notify(RR_QSH_EVENT_IA_PART1_RCVD, gas_id);
#endif //FEATURE_QSH_EVENT_NOTIFY_HANDLER
 ...
 */
static void send_gprs_dl_ia_to_mac(
...
/**
 * @brief This is the call-back functions supplied in the RR_IMSG_GPRS_IA_IND. This is called when GRR is in 
 *        Packet Idle and the IA can be sent to GMAC, or to indicate that the IA should be ignored.
 *        Note: If the IA parameters indicate that this is the 1st part of a 2-part IA, then GL1 will be put
 *        in non-DRX mode.
 * 
 * @param params (in)    - IA parameters
 * @param ignore_ia (in) - TRUE if the IA should be ignored (i.e. RR state or ongoing procedure means that 
 *                         sending a GPRS DL IA is inappropriate)
 * @param gas_id (in)
 */
static void rr_imsg_gprs_ia_ind_cb_fn(rr_gprs_ia_params_t params, boolean ignore_ia, const gas_id_t gas_id)
{
  if (!ignore_ia) 
  {
    if (params.wait_for_2nd_part && params.is_downlink)
    {
       ...
       */
      MSG_GERAN_MED_0_G("Start non-DRX mode for 2-part IA");
      rr_send_mph_start_mm_non_drx_ind(FALSE, gas_id);
    }
    else
    {
      send_gprs_dl_ia_to_mac(
        &params.ia_ptr->packet_ch_desc,
        &params.ia_ptr->ia_rest_octets,
        &params.ia_ptr->frequency_list,
        params.ia_ptr->timing_advance,
        gas_id
      );
    }
  }
  ...
  {
    MSG_GERAN_HIGH_0_G("IA ignored");
  }

  if (params.ia_ptr != NULL)
  {
    GPRS_MEM_FREE(params.ia_ptr);
  }
...
/*!
 * \brief Processes the Immediate Assignment OTA message (when it is a GPRS msg) and decodes the information 
 * in the message. If the message is destined for the mobile and decoded correctly then it defines a PDCH 
 * assignment which is then forwarded to GMAC. 
 * 
 * \param current_IE (in)
 * \param dedicated_mode_or_tbf_ie (in)
 * \param waiting_for_2nd_msg (in/out)
 * \param no_of_rach_attempts (in)
 * \param request_references_array (in) 
 * \param access_status (in)
 * \param gas_id (out)
 * 
 * \return rr_event_T - {EV_NO_EVENT, EV_GPRS_IMMEDIATE_ASSIGNMENT}
 */
...
)
{
  ...
  {
    /* ----------- UL IA received --------------- */
    /* "waiting for 2nd msg" check has been added due to the CR 979065, In 3GPP 44.018 spec Table 10.5.2.25b.2 
       says 2nd is set to zero for downlink message, this message assigns an uplink TBF or is the second message 
       of two in a two-message assignment of an uplink or downlink TBF.if we receive 2nd bit as zero, even though
       we receive 1st part for downlink, we consider UL IA received based 2nd bit(0). in case of 2nd part of UL
       Assignment already taken care by access_status. */
    if ((access_status != RR_CE_ACCESS_FOR_TBF) && (*waiting_for_2nd_msg != TRUE))
    {
      MSG_GERAN_HIGH_0_G("UL IA received but no UL requested");
      return EV_NO_EVENT;
    }

    /* If accessing with the EGPRS Packet Channel Request then the Immediate Assignment must be matched using
       the Extended RA.  When set to '0111 1111' the Request Reference field indicates that the Extended RA
    ...
    if (rr_ce_egprs_access_valid(gas_id))
    {
      ...
      if ( access_reference == EGPRS_EXTENDED_RA_INDICATOR )
      {
        ...
        switch (temp_ia_rest_octets.contents_type)
        {
          ...
          default: ;  /*extended_ra_found already set to FALSE at declaration*/
        }
      }
 ...
 /*****************************/
  ...
  if (decoding_ok)
  {
    if (rr_tbf_est_data_ptr->data_ptr->ia_rest_octets.contents_type == packet_downlink_ass) /* Downlink */
    {
      if (check_tlli(
      ...
      else
      {
        ...
        {
          /**
           * How the DL IA is sent to GMAC depends on the access status. This IA could be received during 
           * RACHing, or at some other time when GL1 is reading PCH (e.g. when in Packet Idle or during BPLMN 
           * search). This IA could also be the 1st part of a 2-part IA and in this case, it is not ready to 
           * be sent to GMAC. 
           * If RACHing, the IA can be sent straight to GMAC (once complete). 
           * If not RACHing, then the IA is encapsulated in a RR_IMSG_GPRS_IA_IND and sent into RR. This is so 
           * that an assessment can be made of whether the current RR state is appropriate to sending an IA to 
           * GMAC, and any ongoing procedure (e.g. BPLMN search) can be aborted. 
           *  
           * Note on 2 part IA handling when not RACHing: 
           * Even in the case of a 2-part IA, a RR_IMSG_GPRS_IA_IND is sent when the 1st part is received. The 
           * IA details will be missing from the message, but this will cause any ongoing procedure (e.g. 
           * BPLMN search) to be aborted, and then the given call-back function will be called. This will then 
           * put GL1 in non-DRX mode in order to receive the 2nd part. 
           * When the 2nd part is received, another RR_IMSG_GPRS_IA_IND is sent, which will include the IA 
           * details. The given call-back function will be called, which will send the IA to GMAC. 
           */
          if (RR_CE_ACCESS_INACTIVE == access_status)
          {
            /* -------- Not RACHing - IA will be sent to GMAC via RR_IMSG_GPRS_IA_IND -------- */

            rr_gprs_ia_params_t params;

            rr_gprs_ia_t *ia_ptr = NULL;

            params.is_downlink = TRUE;
            params.wait_for_2nd_part = *waiting_for_2nd_msg;
            params.ia_ptr = NULL;

            /**
             * If not waiting for a 2nd part, then the IA is complete and heap memory is allocated to hold a
             * copy of the IA. This is passed by pointer in the RR_IMSG_GPRS_IA_IND. When the call-back 
             * function is called, either the IA will be sent to GMAC or it will be ignored. Either way, this 
             * heap memory will then be freed. 
             */
            if (!(*waiting_for_2nd_msg))
            {
              ia_ptr = GPRS_MEM_MALLOC(sizeof(rr_gprs_ia_t));

              if (ia_ptr != NULL)
              {
                ia_ptr->timing_advance = rr_tbf_est_data_ptr->data_ptr->timing_advance;
                ia_ptr->ia_rest_octets = rr_tbf_est_data_ptr->data_ptr->ia_rest_octets;
                ia_ptr->packet_ch_desc = packet_channel_desc;
                ia_ptr->frequency_list = frequency_list;

                params.ia_ptr = ia_ptr;
              }
            }

            /**
             * Inform RR-CONTROL that a GPRS IA has been received. Any ongoing procedure will be aborted, if 
             * appropriate, and then the given call-back function called, with an indication of whether the IA 
             * can be used, or should be discarded. 
             */
            rr_send_imsg_gprs_ia_ind(params, rr_imsg_gprs_ia_ind_cb_fn, gas_id);
          }
          else
          {
            /* -------- RACHing - IA will be sent to GMAC directly -------- */
    ...
    else if ( rr_tbf_est_data_ptr->data_ptr->ia_rest_octets.contents_type == egprs_packet_uplink_ass)
    {
      if (FALSE == *waiting_for_2nd_msg)
      {
        new_event = EV_GPRS_IMMEDIATE_ASSIGNMENT;
    ...
    else if (rr_tbf_est_data_ptr->data_ptr->ia_rest_octets.contents_type == packet_uplink_ass)        /* Uplink */
    {
      if (FALSE == *waiting_for_2nd_msg)
      {
        new_event = EV_GPRS_IMMEDIATE_ASSIGNMENT;
...
#ifdef FEATURE_QSH_EVENT_METRIC
  if (EV_GPRS_IMMEDIATE_ASSIGNMENT == new_event)
  {
    rr_qsh_metric_log_ia_rcvd(gas_id);
  }
#endif // FEATURE_QSH_EVENT_METRIC   
...
static rr_event_T abort_tbf_est(const gas_id_t gas_id)
{
  ...
  /* Reset RACH reference to avoid possible interference with next TBF establishment attempt */
  rr_reset_rach_references(gas_id);
...
)
{
  ...
  if ( ( event_out == EV_GRR51_GO_TO_PACKET_IDLE ) ||
       ( event_out == EV_FORCE_CELL_RESELECTION ) ||
       ( event_out == RR_EV_START_RESELECTION_POWER_SCAN ) ||
       ( event_out == RR_EV_START_RPS_AFTER_DENIAL ) || 
