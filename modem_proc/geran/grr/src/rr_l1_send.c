...
============================================================================*/
/* $Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_l1_send.c#4 $ */
/* $DateTime: 2020/04/29 05:42:52 $ */
...
#ifdef FEATURE_LTE
/**
  @brief Indicate if LTE measurements are allowed
*/
static boolean rr_resel_lte_measurements_allowed(const gas_id_t gas_id)
{
  boolean meas_allowed = FALSE;
  boolean lte_enabled = rr_is_lte_enabled(gas_id);
  boolean resel_allowed = rr_g2l_resel_allowed(gas_id);
  boolean nv_enabled = rr_g2l_measurements_enabled_in_nv(gas_id);

  if (lte_enabled && resel_allowed && nv_enabled)
  {
    meas_allowed = TRUE;
  }

  MSG_GERAN_HIGH_4_G("lte meas allowed:%d lte_enabled:%d g2l_resel_allowed:%d g2l_nv:%d",
                      meas_allowed, lte_enabled, resel_allowed, nv_enabled);

  return meas_allowed;
}

static boolean rr_is_all_lte_cell_barred(const gas_id_t gas_id)
{ 
  uint32 i;
  gprs_scell_info_t * scell = rr_gprs_get_scell_info(gas_id);
  
#ifndef PERLUTF
  if (FALSE == rr_nv_revert_to_rank_base_if_plmn_match(rr_gprs_get_scell_plmn_id(gas_id), gas_id))
  {
    return FALSE;
  }
#endif /*PERLUTF*/  

  for (i = 0; i < scell->lte_neighbour_list.count && i < MAX_RR_EUTRAN_NEIGHBOUR_LIST_SIZE ; ++i)
  {
  ...
  if (scell->lte_neighbour_list.count != 0)
  {
    MSG_GERAN_HIGH_0_G("All LTE cell configured are barred revert to rank based if UTRAN priority missing");
    return TRUE;
  }
  return FALSE;               
}
/**
 *@brief Makes decision to inhibit priority based reselection  
 *       based on whether LTE measurements are allowed and if
 *       UTRAN priority params are received.
*/
static void rr_handle_pri_resel_inhibit(boolean utran_measured, const gas_id_t gas_id)
{
  boolean inhibit_changed;

#ifdef FEATURE_FAST_G2L_PSEUDO_SI2Q
  if (rr_is_pseudo_si2q_data_used(gas_id) == FALSE)
#endif /*FEATURE_FAST_G2L_PSEUDO_SI2Q*/
  {
    if (((rr_resel_lte_measurements_allowed(gas_id) == FALSE)|| (rr_is_all_lte_cell_barred(gas_id) == TRUE)) &&
        (rr_utran_priority_parameters_ie_received(rr_get_instances_ptr(SI2Q_INSTANCE,gas_id),gas_id) == FALSE) &&
        (utran_measured == TRUE))
    {
      /*Priority reselection is enabled because serving cell priorities are present but if LTE measurements
        are not allowed and UTRAN priorities are missing (note this is a bad NW config) no IRAT can take place.
        In this case revert back to rank based reselection to allow UTRAN to be measured */
      inhibit_changed = rr_set_inhibit_pri_resel(TRUE, gas_id);
    }
    else
    {
      inhibit_changed = rr_set_inhibit_pri_resel(FALSE, gas_id);
    }
...
static void rr_l1_send_message_to_l1(IMH_T *message_ptr, const gas_id_t gas_id)
{
  ...
  gs_status = gs_send_message(geran_get_gl1_queue_id(gas_id), (void *)message_ptr, TRUE);
 ...
 * \param arfcn (in)
 */
rr_l1_band_bitmap_T rr_l1_get_band_bitmap_for_arfcn(ARFCN_T arfcn)
{
  switch(arfcn.band)
  {
    case SYS_BAND_PGSM_900:
      return((rr_l1_band_bitmap_T)RR_L1_BAND_BITMAP_PGSM);

    case SYS_BAND_EGSM_900:
      return((rr_l1_band_bitmap_T)RR_L1_BAND_BITMAP_EGSM);

    case SYS_BAND_DCS_1800:
      return((rr_l1_band_bitmap_T)RR_L1_BAND_BITMAP_DCS_1800);

    case SYS_BAND_PCS_1900:
      return((rr_l1_band_bitmap_T)RR_L1_BAND_BITMAP_PCS_1900);

    case SYS_BAND_CELL_850:
      return((rr_l1_band_bitmap_T)RR_L1_BAND_BITMAP_GSM_850);

    default:
      MSG_GERAN_ERROR_1("Unknown band %d", arfcn.band);
      break;
  }

  return((rr_l1_band_bitmap_T)RR_L1_BAND_BITMAP_NONE);
}
...
void rr_send_mph_decode_bcch_list_req(
  rr_l1_idle_freq_list_T *freq_list_ptr,
...
)
{
  ...
  rr_l1_band_bitmap_T requested_bands = RR_L1_BAND_BITMAP_NONE;
  ...
  // Copy frequency list and determine bands used

  num_freqs = MIN(freq_list_ptr->num_frequencies, ARR_SIZE(freq_list_ptr->arfcn));

  for (i = 0; i < num_freqs; i++)
  {
    mph_decode_bcch_list_req.frequency_list.arfcn[i] = freq_list_ptr->arfcn[i];

    requested_bands |= rr_l1_get_band_bitmap_for_arfcn(freq_list_ptr->arfcn[i]);
  }

  mph_decode_bcch_list_req.frequency_list.num_frequencies = num_freqs;
  ...
  mph_decode_bcch_list_req.requested_bands = requested_bands;
...
)
{
  ...
  {
    /* Multi-SIM controller tracks the responsiveness of the L1 interface */
...
#if defined(FEATURE_DUAL_SIM) || defined(FEATURE_TRIPLE_SIM)
      /* apply veto for blocking tune away during re-org mode*/
      geran_api_set_tuneaway_blocked(GERAN_API_CLIENT_PAGE, TRUE, geran_map_gas_id_to_nas_id(gas_id));
#endif /*FEATURE_DUAL_SIM || FEATURE_TRIPLE_SIM*/
...
#if defined(FEATURE_DUAL_SIM) || defined(FEATURE_TRIPLE_SIM)
      /* clear  veto for un-blocking tune away during in ext mode*/
      geran_api_set_tuneaway_blocked(GERAN_API_CLIENT_PAGE, FALSE, geran_map_gas_id_to_nas_id(gas_id));
#endif /*FEATURE_DUAL_SIM || FEATURE_TRIPLE_SIM*/
...
#if defined(FEATURE_DUAL_SIM) || defined(FEATURE_TRIPLE_SIM)
      /* clear  veto for un-blocking tune away during in normal mode*/
      geran_api_set_tuneaway_blocked(GERAN_API_CLIENT_PAGE, FALSE, geran_map_gas_id_to_nas_id(gas_id));
#endif /*FEATURE_DUAL_SIM || FEATURE_TRIPLE_SIM*/
      break;
    }
 ...
 */
void rr_send_mph_select_specific_channel(ARFCN_T arfcn, BSIC_T bsic, const gas_id_t gas_id)
{
  ...
  /* In X2G CGI scenario, we are not attempting to camp. So do not log the event. */
  if(!rr_is_control_state(RR_X2G_CGI, gas_id))
#endif /* FEATURE_LTE_TO_GSM_CGI */
  {
    rr_log_camping_attempt_start(arfcn, bsic, gas_id);
  }

  mph_select_specific_bcch_req.specific_channel_no = arfcn;
...
)
{
...

  {
    ...
    eng_mode_scell_ncell_info_t sncell_arfcn = {0};

    serving_cell_ptr = rr_gprs_get_scell_info(gas_id);
    RR_NULL_CHECK_FATAL(serving_cell_ptr);

    sncell_arfcn.scell_arfcn.num = serving_cell_ptr->gsm.BCCH_ARFCN.num;
    sncell_arfcn.scell_arfcn.band = (eng_mode_sys_band_T)serving_cell_ptr->gsm.BCCH_ARFCN.band;

    sncell_arfcn.ncell_freq_list.no_of_items = serving_cell_ptr->gsm.complete_BA_list.no_of_entries;
    memscpy(sncell_arfcn.ncell_freq_list.channel,
      32*(sizeof(ENG_MODE_ARFCN_T)),
      serving_cell_ptr->gsm.complete_BA_list.BCCH_ARFCN,
      32*(sizeof(ENG_MODE_ARFCN_T)));

    geran_eng_mode_data_write(
      ENG_MODE_SCELL_NCELL_INFO,
      (void*)&sncell_arfcn,
      TRUE,
      gas_id
    );
...
)
{
  ...
  mph_stop_gsm_mode_req.x2g_ho_fail = x2g_ho_fail;

#if defined(FEATURE_DUAL_SIM) || defined(FEATURE_TRIPLE_SIM)
  /* advise the multi-SIM controller */
  rr_ms_l1_shutdown_msg_expected(MPH_STOP_GSM_MODE_CNF, gas_id);
#endif /* FEATURE_DUAL_SIM || FEATURE_TRIPLE_SIM */
...
)
{
  ...
  mph_start_gsm_mode_req.x2g_ho_in_progress = x2g_ho_in_progress;

  MSG_GERAN_MED_4_G("Starting L1 (gsm_only: %d, x2g_resel: %d, x2g_cco: %d, x2g_ho: %d)",    
	...
	(int)mph_start_gsm_mode_req.x2g_ho_in_progress);
...
*/
void rr_send_mph_set_priority_ind(rr_priority_info_t* rr_priority_info_p, const gas_id_t gas_id)
{
  mph_set_priority_ind_T mph_set_priority_ind;
  RR_NULL_CHECK_FATAL(rr_priority_info_p);

  RR_PREPARE_L1_MESSAGE(mph_set_priority_ind, MPH_SET_PRIORITY_IND, gas_id);

  /* load the message with the parameters */
  mph_set_priority_ind.procedure_type = rr_priority_info_p->procedure_type;
  mph_set_priority_ind.new_priority   = rr_priority_info_p->new_priority;
  mph_set_priority_ind.new_procedure  = rr_priority_info_p->new_procedure;
  mph_set_priority_ind.sys_proc_type  = rr_priority_info_p->sys_proc_type;

  MSG_GERAN_MED_4_G("MPH_SET_PRIORITY_IND Procedure=%d Priority=%d NewProc=%1d Sys Proc=%d",
                    (int)rr_priority_info_p->procedure_type, (int)rr_priority_info_p->new_priority,
                    (int)rr_priority_info_p->new_procedure, (int)rr_priority_info_p->sys_proc_type);
...
)
{
      ...
      ))
  {
    ...
    utran_frequency_index = rr_utran_frequency_list_search(&utran_frequency_list,RR_UTRAN_FREQUENCY_LIST_ENTRY_TYPE_FDD,cell.uarfcn.uarfcn);

    MSG_GERAN_HIGH_3_G("Adding supported FDD UARFCN %d, SC %d of Frequency List Index %d to L1 3G nlist",
                       cell.uarfcn.uarfcn, cell.cell_code, utran_frequency_index);
...
)
{
  ...
  if (scell != NULL)
  {
    ...
    uint8  max_lte_neighbours = MAX_RR_L1_EUTRAN_FREQUENCIES;

#ifdef FEATURE_PRIORITIZE_LTE_FREQ
    if (rr_timer_is_running(RR_PRIORITIZE_LTE_ABORT_TIMER, gas_id))
    {
      wcdma_cell_update_list_req.prioritize_lte_meas = TRUE;
      max_lte_neighbours = rr_nv_get_prioritize_lte_meas_info(RR_GAS_ID_TO_AS_ID)->max_lte_frequencies;
      nw_supp_rats = RR_NW_SUPP_UTRAN_NONE;
    }
...

    {
      ...
      boolean utran_measured = use_tdd || use_fdd;

      MSG_GERAN_HIGH_2_G("serving_cell_priority_parameters_ie_received: %d, utran_measured: %d",
                         serving_cell_priority_parameters_ie_received,
                         utran_measured);
    
      if((rr_get_fallback_flag(gas_id) == TRUE) && (serving_cell_priority_parameters_ie_received == TRUE) && (rr_get_efs_nv_coll_fallback_enabled(RR_GAS_ID_TO_AS_ID)))
      {
        use_priority_reselection_algo = TRUE;
      }
      else if (serving_cell_priority_parameters_ie_received)
      {
        rr_handle_pri_resel_inhibit(utran_measured, gas_id);

        switch (rr_resel_algo(gas_id))
          {
      ...
      if ( use_fdd )
      {
        ...
        if (rr_g2w_measurements_enabled_in_nv(gas_id) && 
            rr_g2w_resel_redir_allowed(gas_id))
        {
        ...
        {
          if(!rr_g2w_resel_redir_allowed(gas_id))
          {
            MSG_GERAN_HIGH_0_G("G2W MEASUREMENTS BLOCKED BY NAS");
          }
          else
          {
      ...
      {
        gprs_scell_info_t* scell = rr_gprs_get_scell_info(gas_id);
        uint8              thresh_prio_srch = scell->serving_cell_priority_params.thresh_priority_search;

        if(thresh_prio_srch >= 10 && thresh_prio_srch <= 15)
        {
          if((rr_get_fallback_flag(gas_id) == TRUE) && (rr_get_efs_nv_coll_fallback_enabled(RR_GAS_ID_TO_AS_ID) == TRUE))
          {
            thresh_prio_srch = 4;
          }
          MSG_GERAN_HIGH_1_G("THRESH_priority_search changed to = %d", thresh_prio_srch);
        }

        wcdma_cell_update_list_req.THRESH_priority_search = thresh_prio_srch;
...
#ifdef FEATURE_LTE
        if (rr_resel_lte_measurements_allowed(gas_id))
        {
          for (i = 0;
               i < scell->lte_neighbour_list.count &&
               i < MAX_RR_EUTRAN_NEIGHBOUR_LIST_SIZE &&
               wcdma_cell_update_list_req.eutran_frequency_count < max_lte_neighbours;
               ++i)
          {
            rr_lte_neighbour_list_entry_t * src;
  ...
  rr_log_irat_si2q_resel_params(&wcdma_cell_update_list_req,NULL,gas_id);
  rr_log_irat_si2q_meas_params(&wcdma_cell_update_list_req,NULL,gas_id);
    ...
    if (rr_dedicated_mode_is_lte_enabled(gas_id))
    {
           ...
           ++i)
      {
        ...
        else
        {
          ...
          if (lte_rrc_earfcn_is_supported(as_id, src->frequency.earfcn, rr_band_pref_lte(gas_id)))
          {
            uint32 pcid;
...
/* Before sending detect Duplicates and Remove them from the list */
  ...
  rr_log_irat_si2q_resel_params(NULL,&mph_ded_wcdma_cell_update_list_req,gas_id);
  rr_log_irat_si2q_meas_params(NULL,&mph_ded_wcdma_cell_update_list_req,gas_id);
...
#if defined(FEATURE_DUAL_SIM) || defined(FEATURE_TRIPLE_SIM)
  /* advise the multi-SIM controller */
  rr_ms_l1_shutdown_msg_expected(MPH_RESET_CNF, gas_id);
#endif /* FEATURE_DUAL_SIM || FEATURE_TRIPLE_SIM */
...
)
{
  ...
  /* START_GPRS_IDLE logging starts here */
  {
    ...
    /* If last status was GSM_IDLE, the event would have been logged on sending MPH_START_IDLE_REQ */
    if(RR_L1_IDLE_STATUS_GSM_IDLE != rr_l1_send_get_l1_idle_status(gas_id))
    {
      rr_log_camping_attempt_success(gas_id);
    }
...

  {
    ...
    /* Write ENG_MODE_SCELL_NCELL_INFO */
    sncell_arfcn.scell_arfcn.num = serving_cell_ptr->gsm.BCCH_ARFCN.num;
    sncell_arfcn.scell_arfcn.band = (eng_mode_sys_band_T)serving_cell_ptr->gsm.BCCH_ARFCN.band;
...
#if defined(FEATURE_DUAL_SIM) || defined(FEATURE_TRIPLE_SIM)
  {
    ...
    imsi_mod_1000 = (word)(idle_mode_params->ccch_idle_mode_params.layer1_extra_gprs_sys_info.imsi_mod1000.msb);
    imsi_mod_1000 *= 256;
    imsi_mod_1000 += (word)(idle_mode_params->ccch_idle_mode_params.layer1_extra_gprs_sys_info.imsi_mod1000.lsb);
...
/* SURROUND_UPDATE logging ends */
  ...
  if (GRR_L1_DECODE_SI3_ONLY == gprs_surround_data->bcch_params.si_requested)
  {
    ...
    if (bcch_ncell_info != NULL &&
        ...
        bcch_ncell_info->gsm.valid_data_flag)
    {
...
void rr_send_mph_start_mm_non_drx_ind(
  boolean non_updated_lai_in_nmo1,
  const gas_id_t gas_id
)
{
  ...
  mph_start_mm_non_drx_ind.non_updated_lai_in_nmo1 = non_updated_lai_in_nmo1;
...
)
{
...
#endif /* FEATURE_WCDMA || FEATURE_GSM_TDS */
  boolean                          include_tdd = FALSE; /* TDD cells are sent to GL1 for measurements */
  boolean                          include_fdd = FALSE; /* FDD cells are sent to GL1 for measurements */
...
#ifdef FEATURE_LTE
  /* Default value(0x0F) is used for QSEARCH_C_EUTRAN if priority based reselection is not used*/
...
#endif /*defined (FEATURE_WCDMA) || defined (FEATURE_GSM_TDS)*/

  if (rr_serving_cell_priority_parameters_ie_received(rr_gprs_get_scell_info(gas_id)))
  {
    ...
    if (use_priority_reselection_algo)
    {
      boolean utran_measured = include_tdd || include_fdd;
  ...
  if (use_priority_reselection_algo)
  {
    ...
    msg.THRESH_priority_search = thresh_prio_srch; 
    ...
    if (rr_resel_lte_measurements_allowed(gas_id))
    {
      for (i = 0;
           i < scell->lte_neighbour_list.count &&
           i < MAX_RR_EUTRAN_NEIGHBOUR_LIST_SIZE &&
           msg.eutran_frequency_count < max_lte_neighbours;
           ++i)
      {
        rr_lte_neighbour_list_entry_t * src;
...
sys_band_mask_type rr_l1_band_bitmap_to_sys_band_mask(rr_l1_band_bitmap_T rr_l1_band_bitmap)
{
  sys_band_mask_type sys_band_mask = SYS_BAND_MASK_EMPTY;

  if ((rr_l1_band_bitmap & (rr_l1_band_bitmap_bands_T)RR_L1_BAND_BITMAP_PGSM) != 0)
  {
    sys_band_mask |= SYS_BAND_MASK_GSM_PGSM_900;
  }
  if ((rr_l1_band_bitmap & (rr_l1_band_bitmap_bands_T)RR_L1_BAND_BITMAP_EGSM) != 0)
  {
    sys_band_mask |= SYS_BAND_MASK_GSM_EGSM_900;
  }
  if ((rr_l1_band_bitmap & (rr_l1_band_bitmap_bands_T)RR_L1_BAND_BITMAP_DCS_1800) != 0)
  {
    sys_band_mask |= SYS_BAND_MASK_GSM_DCS_1800;
  }
  if ((rr_l1_band_bitmap & (rr_l1_band_bitmap_bands_T)RR_L1_BAND_BITMAP_PCS_1900) != 0)
  {
    sys_band_mask |= SYS_BAND_MASK_GSM_PCS_1900;
  }
  if ((rr_l1_band_bitmap & (rr_l1_band_bitmap_bands_T)RR_L1_BAND_BITMAP_GSM_850) != 0)
  {
    sys_band_mask |= SYS_BAND_MASK_GSM_850;
  }
