...
*/
/* $Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_mm_send.h#1 $ */
...
#ifdef FEATURE_SGLTE
extern void rr_sglte_srvcc_send_no_service_ind(const gas_id_t gas_id);
#endif /* FEATURE_SGLTE */
...
extern void rr_mm_send_rr_preemption_ind(
  const gas_id_t gas_id
);

extern void rr_gmm_retx_pdu_tuneaway_fail_ind(
  const gas_id_t gas_id
);
...
/*!
 * \brief Sends the forbidden LAI to NAS
 */
extern void rr_mm_send_resel_fail_ind(LAI_T *lai_ptr, const gas_id_t gas_id);
