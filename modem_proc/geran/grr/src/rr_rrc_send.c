...
$Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_rrc_send.c#3 $
...
)
{
  ...
  rrc_put_rr_cmd_sub_id(rrc_cmd_ptr, geran_map_gas_id_to_nas_id(gas_id));
#else
  rrc_put_rr_cmd(rrc_cmd_ptr);
#endif /* FEATURE_DUAL_SIM && FEATURE_DUAL_WCDMA */
}
...
/**
 * Send the RRC_GRR_INTERRAT_PROC_ABORT_IND primitive to RRC
 *
 */
void rr_send_interrat_proc_abort_ind(
  rrc_rr_status_e_type status,
  const gas_id_t gas_id
)
{
  ...
  if (rr_is_rat_segment_loaded(RR_SEG_LOAD_WCDMA) == FALSE)
  {
    MSG_GERAN_ERROR_1_G("RR Can't send RRC message %d, RRC not loaded",RRC_GRR_INTERRAT_PROC_ABORT_IND);
    return;
  }
  ...
  rrc_cmd->cmd_hdr.cmd_id = RRC_GRR_INTERRAT_PROC_ABORT_IND;
  rr_send_message_to_rrc(rrc_cmd, gas_id);
}
...
)
{
  ...
  rrc_cmd->cmd.interrat_handover_req.network_select_mode = plmn_request_details->network_select_mode;
...
)
{
...
#ifndef PERLUTF
  rrc_cmd->cmd.interrat_plmn_srch_req.high_priority = high_priority;
#else
  #error code not present
#endif /*PERUTF*/
  ...
  rrc_cmd->cmd.interrat_plmn_srch_req.gtow_arfcn_list = rr_plmn_list_req_info_ptr->arfcn_list;
  rrc_cmd->cmd.interrat_plmn_srch_req.search_priority = rr_plmn_list_req_info_ptr->srch_priority;
  rrc_cmd->cmd.interrat_plmn_srch_req.preferred_plmn_list = rr_plmn_list_req_info_ptr->preferred_plmn_list;

  MSG_GERAN_HIGH_4_G("Sent RRC_INTERRAT_PLMN_SRCH_REQ num_plmns:%d, nw_sel_mode:%d, timer:%d, Priority %d",num_plmns,network_select_mode,timer_val,high_priority);
...
void rr_send_cipher_sync_ind(const gas_id_t gas_id)
{
  ...
  {
    MSG_GERAN_HIGH_0_G("No need for RR_CIPHER_SYNC_IND, WCDMA not supported");
  }
  else
  {
    rrc_rr_cmd_type *rrc_cmd = NULL;
