...
$Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_plmn_list.c#4 $
...
typedef struct
{
  ...
  /*!< TRUE if preferred PLMN found else false.*/
...
#endif /* FEATURE_FEMTO_CSG */
  /* FR 42544: Support for frequency/channel level MPLMN scanning */
  if (rr_plmn_list_req_info->srch_priority != SYS_SRCH_PRIORITY_NORMAL)
  {
    MSG_GERAN_MED_0("Not filtering PLMN list (PERIODIC_FREQ_SEARCH)");
    filter = FALSE;
  }
  return filter;
}
 ...
 *
 * Input:    GERAN Access Stratum ID
 * Output:   MPH_DEVICE_MODE_IND to Layer 1
 */
...
static void rr_plc_search_complete(const gas_id_t gas_id)
{
  ...
  *filtered_available_plmn_list_ptr = *available_plmn_list_ptr;
  ...
  /* inform Priority Inversion Manager that the search is complete */
  rr_ms_plmn_list_search_complete(gas_id);
          ...
          else
          {
            ...
            else
            {
              ...
              pscan_params = rr_find_cell_new_search(
                ...
                rr_plc_band_pref(gas_id),     // band_pref
                FALSE,                         // search_by_band_allowed
                candidate_db_ptr,             // candidate_db_ptr
                gas_id                        // gas_id
              );

              rr_pscan_send_start_ind(
                RR_SEARCH_BACKGROUND,     // search_mode
                  ...
                  case RR_PSCAN_OMSG:
                  {
                    switch (message_ptr->rr.pscan.omsg.header.imh.message_id)
                    {
                      case RR_PSCAN_OMSG_FINISHED_IND:
                      {
                        ...
                        (void) rr_pscan_process_power_scan_results(gas_id);
                        ...
                        if (num_freqs != 0)
                        {
                          ...
                          filter_band_mask = bcch_decodes_params.band_mask;
                ...
                switch(message_id)
                {
                  case TDSRR_INTERRAT_PLMN_SRCH_REQ:
                  {
                    ...
                    rr_plc_setup(
                      ...
                      message_ptr->tdsrr_plmn_srch_req.high_priority, //priority to use
                      gas_id                                          // gas_id
                    );
                ...
                switch(message_id)
                {
                  case RR_INTERRAT_PLMN_SRCH_REQ:
                  {
                    ...
                    rr_plc_setup(
                      ...
                      message_ptr->rr_interrat_plmn_srch_req.high_priority, //priority to use
                      gas_id                                                // gas_id
                    );
                ...
                switch (message_ptr->rr_msgr_msg.message.hdr.id)
                {
                  ...
                  case GERAN_GRR_LTE_PLMN_SRCH_REQ:
                  {
                    ...
                    rr_plc_setup(
                      ...
                      message_ptr->
                        rr_msgr_msg.message.geran_grr_msg_req.plmn_srch_req.high_priority,// priority to use
                      gas_id                                                              // gas_id
                    );
              ...
              case MS_RR_L1:
              {
                switch (message_id)
                {
                  case MPH_SUSPEND_GSM_MODE_IND:
                  {
                    ...
                    if (rr_timer_is_running(RR_BCCH_DECODE_COMPLETE_TIMER, gas_id))
                    {
                      /* timer expiry and timer expiry callback */
                      if (rr_timer_force_timer_expiry(RR_BCCH_DECODE_COMPLETE_TIMER, gas_id))
                      {
                        MSG_GERAN_MED_0_G("X2G PLMN List Search:RR_BCCH_DECODE_COMPLETE_TIMER forcefully expired");
                        break;
                      }
                    }
...
)
{
...
#ifndef PERLUTF  
  plmn_srch_req.high_priority = high_priority;
#else
  #error code not present
#endif /*PERLUTF*/
  ...
  /* FR 42544: Support for frequency/channel level MPLMN scanning */
  plmn_srch_req.frequency_list       = rr_plmn_list_req_info_ptr->arfcn_list;
  plmn_srch_req.search_priority      = rr_plmn_list_req_info_ptr->srch_priority;
  plmn_srch_req.preferred_plmn_list  = rr_plmn_list_req_info_ptr->preferred_plmn_list;

#ifndef PERLUTF
  plmn_srch_req.foreground_search  = foreground_search;
#endif 
  ...
  MSG_GERAN_MED_2_G("G2L PLMN List Search: GL1 deactivated, sending LTE_RRC_G_PLMN_SRCH_REQ (%d ms), Priority =%d", plmn_srch_req.timer_val, high_priority);
...
)
{
  ...
  switch (rat)
  {
    ...
    case RR_RAT_LTE:
    {
      ...
      /* FR 42544: Support for frequency/channel level MPLMN scanning */
      // Obtain the partial results from L
      rr_plc_data_ptr->x_rat_partial_results_ptr = GPRS_MEM_CALLOC(sizeof(sys_detailed_plmn_list_s_type));

      if (rr_plc_data_ptr->x_rat_partial_results_ptr != NULL)
      {
        lte_rrc_get_inter_rat_bplmn_prtl_result(
          geran_map_gas_id_to_nas_id(gas_id),          // as_id
          rr_plc_data_ptr->x_rat_partial_results_ptr   // available_plmn_list
        );
      }
            ...
            switch (rr_plc_data_ptr->rat_search_info_current->rat)
            {
              ...
              case RR_RAT_UTRAN_TDD:
              {
                ...
                rr_send_tds_interrat_plmn_search_req(
                  ...
                  FALSE,            // camped suitably
                  TRUE,             // FG or BG search
                  TRUE,             // High priority
                  gas_id
                );
              ...
              case RR_RAT_LTE:
              {
                ...
                rr_plc_send_lte_rrc_g_plmn_srch_req(RR_MAX_PLMN_LIST_SEARCH_TIME, &detected_frequency_list, TRUE, TRUE, gas_id);
                  ...
                  case RRC_INTERRAT_PLMN_SRCH_CNF:
                  {
                    ...
                    if(plmn_srch_cnf->srch_done)
                    {
                      ...
                      else
                      {
                       ...
                       /* FR 42544: Support for frequency/channel level MPLMN scanning */
                      if (rr_plmn_list_req_info_ptr->srch_priority != SYS_SRCH_PRIORITY_NORMAL)
                      {
                        rr_plc_data_ptr->rat_search_info_current->preferred_plmn_found = plmn_srch_cnf->preferred_plmn_found;
                        MSG_GERAN_HIGH_1_G("FR_42544_DEBUG: Preferred PLMN found is = %d", plmn_srch_cnf->preferred_plmn_found?TRUE:FALSE);
                      }
              ...
              case MS_MSGR_RR:
              {
                switch (message_ptr->rr_msgr_msg.message.hdr.id)
                {
                  ...
                  case LTE_RRC_G_PLMN_SRCH_RSP:
                  {
                    ...
                    MSG_GERAN_MED_3_G("G2L PLMN List Search: received LTE_RRC_G_PLMN_SRCH_CNF, partial_srch=%d, srch_done=%d, hplmn_found=%d",
                             plmn_srch_rsp->partial_srch,
                    ...
                    if(plmn_srch_rsp->srch_done)
                    {
                      ...
                      if (rr_plmn_list_req_info_ptr->srch_priority != SYS_SRCH_PRIORITY_NORMAL)
                      {
                        rr_plc_data_ptr->rat_search_info_current->preferred_plmn_found = plmn_srch_rsp->preferred_plmn_found;
                        MSG_GERAN_HIGH_1_G("FR_42544_DEBUG: Preferred PLMN found = %d", plmn_srch_rsp->preferred_plmn_found?TRUE:FALSE);
                      }
                        ...
                        switch (rr_plc_data_ptr->rat_search_info_current->rat)
                        {
                          ...
                          case RR_RAT_UTRAN_TDD:
                          {
                            ...
                            rr_send_tds_interrat_plmn_search_req(
                              ...
                              camped_suitably,                                             // Camped suitably
                              FALSE,                                                       // FG or BG search 
                              rr_ms_plmn_search_update_priority_rqrd(TRUE, FALSE, gas_id), //High priority
                              gas_id
                            );
                            break;
                          }
                          ...
                          case RR_RAT_WCDMA:
                          {
                            ...
                            rr_send_interrat_plmn_search_req(
                              ...
                              message_ptr->mph.suspend_gsm_mode_cnf.timer_value,
                              camped_suitably,
                              rr_ms_plmn_search_update_priority_rqrd(TRUE, FALSE, gas_id),
                              &detected_frequency_list,
                              gas_id
                            );
#else
                ...
                switch (message_id)
                {
                  case RRC_INTERRAT_PLMN_SRCH_CNF:
                  {
                    ...
                    if (plmn_srch_cnf->srch_done)
                    {
                      ...
                      {
                        rr_plc_data_ptr->rat_search_info_current->preferred_plmn_found= plmn_srch_cnf->preferred_plmn_found;
                        MSG_GERAN_HIGH_1_G("FR_42544_DEBUG: Preferred PLMN found = %d", plmn_srch_cnf->preferred_plmn_found?TRUE:FALSE);
                      }
                    }
...
static boolean rr_passed_service_search_irat_plmn_check(const gas_id_t gas_id)
{
  ...
  if (plmn_check_pass)
  {
    ...
    if (plmn_check_pass)
    {
      ...
      else if((rr_get_fallback_flag(gas_id)) &&
              (rr_get_efs_nv_coll_fallback_enabled(RR_GAS_ID_TO_AS_ID)) && 
              (rr_get_fallback_pref_mode(gas_id) == TRM_FALLBACK_GSM_PREF)
             )
      {        
        MSG_GERAN_HIGH_0_G("RR IRAT PLMN service search abandoned Fall back active in GSM preferred mode");
        plmn_check_pass = FALSE;
  ...
  while (other_gas_id <= GERAN_ACCESS_STRATUM_ID_MAX)
  {
    if (other_gas_id != gas_id)
    {
          ...
          rr_plmn_list_req_info_ptr->network_selection_mode == SYS_NETWORK_SELECTION_MODE_MANUAL) 
      {
        ...
        if (rr_is_forbidden_LAI(&other_scell_info_ptr->gsm.location_area_identification, gas_id))
        {
          MSG_GERAN_HIGH_0_G("Not adding other cell PLMN,forbidden LAI");
        }
        else
        {
...
/**
 * Function: rr_plc_search_starting
 *           Performs further actions necessary when a PLMN list search begins/resumes           
 *
 * @Params:  search_type:   FOREGROUND_SEARCH, BACKGROUND_SEARCH or XTOG_SEARCH
 *           high_priority: TRUE/FALSE
 *
 */
static void rr_plc_search_starting(const rr_plc_search_type_e search_type, boolean high_priority, const gas_id_t gas_id)
{
#if defined(FEATURE_DUAL_SIM) || defined(FEATURE_TRIPLE_SIM)
  rr_PLMN_list_req_info_T *req_ptr = rr_get_plmn_list_req_info_ptr(gas_id);
  rr_ms_plmn_list_search_starting(req_ptr, search_type, high_priority, gas_id);
#else
  NOTUSED(search_type);
  NOTUSED(gas_id);
  NOTUSED(high_priority)
#endif /* FEATURE_DUAL_SIM || FEATURE_TRIPLE_SIM */
 ...
 *
 * @Params:  search_type:   FOREGROUND_SEARCH, BACKGROUND_SEARCH or XTOG_SEARCH
 *           timer_val:     used only for XTOG search
  ...
  const gas_id_t gas_id)
{
  ...
  MSG_GERAN_HIGH_3_G("   -- search_type: %d, timer_val: %d, Priority: %d", search_type, timer_val, high_priority);
  ...
  rr_plc_search_starting(search_type, high_priority, gas_id);
...
)
{
  ...
  if ((ps_service_enable) && (rr_mac_ul_tbf_est_is_allowed(gas_id)))
  {
    ...
    rr_plc_data_ptr->ps_service_enable = TRUE;
  }
  else
  {
    MSG_GERAN_HIGH_0_G("ps_service is blocked during background PLMN search");
    rr_plc_data_ptr->ps_service_enable = FALSE;
...
)
{
  ...
  rr_plc_data_t *rr_plc_data_ptr;
  byte       index = 0;
  boolean    is_lte_enable = FALSE;
  boolean    is_wcdma_enable = FALSE;
        ...
        switch(event)
        {
          case EV_PLMN_SEARCH_START:
          {
            ...
            if (rr_plc_data_ptr->rat_search_info_current != NULL && service_search_proceed)
            {
              ...
              /* FR 42544: Support for frequency/channel level MPLMN scanning */
              // Priority is not normal (low/high) and timer value was not mentioned for incremental search.
              if ((rr_plmn_list_req_info_ptr->srch_priority != SYS_SRCH_PRIORITY_NORMAL) && (max_search_timeout_ms == 0))
              {
                /* Get the Active rat info from RAT PRIO LIST and set it resepective flags */
                for (index = 0; index < rr_plc_data_ptr->rat_search_count; index++)
                {
                  if (rr_plc_data_ptr->rat_search_info[index].rat == RR_RAT_LTE)
                  {
                    is_lte_enable = TRUE;
                  }
                  if (rr_plc_data_ptr->rat_search_info[index].rat == RR_RAT_WCDMA)
                  {
                    is_wcdma_enable = TRUE;
                  }
                }
                /**************************************************************************************/
                /* All Information is RAT/BAND/FREQ Available                                         */
                /* Mode      RAT-Specified    BAND      FREQ                  AS timer                */
                /* LW          LW              LW        LW                      20sec                */
                /*                                                                                    */
                /* Either W/L only Present in RAT and  Freq List                                      */             
                /*  L          L               L         L                       20sec                */
                /*  W          W               W         W                       20sec                */
                /*                                                                                    */
                /* Any of the RAT in list request is having NULL frequency list - start 150 sec timer */
                /* LW         L/W             L/W       L(Freq)/W(No)           150sec                */
                /* LW         L/W             L/W       L(No)/W(Freq)           150sec                */
                /**************************************************************************************/
                /* if all the RAT in the list request has valid frequencies */ 
                MSG_GERAN_HIGH_2_G("FR_42544_DEBUG: W_FREQ_CNT = %d, L_FREQ_CNT = %d",
                  rr_plmn_list_req_info_ptr->arfcn_list.wcdma_count,rr_plmn_list_req_info_ptr->arfcn_list.lte_count);
                if ((is_lte_enable && rr_plmn_list_req_info_ptr->arfcn_list.lte_count > 0) && (is_wcdma_enable && rr_plmn_list_req_info_ptr->arfcn_list.wcdma_count > 0))
                {
                  max_search_timeout_ms = RR_PLMN_LIST_SEARCH_GUARD_TIMER_DURATION;
                }
                else if (((TRUE == is_lte_enable   && rr_plmn_list_req_info_ptr->arfcn_list.lte_count > 0)   && (FALSE == is_wcdma_enable))||
                         ((TRUE == is_wcdma_enable && rr_plmn_list_req_info_ptr->arfcn_list.wcdma_count > 0) && (FALSE == is_lte_enable)))
                {
                  max_search_timeout_ms = RR_PLMN_LIST_SEARCH_GUARD_TIMER_DURATION;
                }
                MSG_GERAN_HIGH_1_G(
                    "Max search timer %d secs (periodic_freq_search)",
              ...
              if ((rr_plc_data_ptr->search_type == RR_PLC_BACKGROUND_SEARCH) && !rr_gsm_only(gas_id))
              {
#ifdef FEATURE_SGLTE
        ...
        switch (event)
        {
          case EV_PLMN_SEARCH_COMPLETE:
          {
            ...
            else
            {
              ...
              else if (rr_plc_data_ptr->rat_search_info_current->preferred_plmn_found)
              {
                 rr_plc_search_complete(gas_id);
                out_event = EV_PLMN_SEARCH_COMPLETE;
                rr_plc_data_ptr->state = RR_PLC_STATE_NULL;
...
boolean rr_plc_wtog_decoding_bcch(const gas_id_t gas_id)
{
  ...
  if (rr_plc_data_ptr != NULL)
  {
    ...
    return(rr_plmn_search_in_progress(gas_id) &&
           (rr_plc_data_ptr->state == RR_PLC_STATE_X2G) &&
           (rr_plc_data_ptr->x2g_state == RR_PLC_X2G_STATE_BCCH_LIST_DECODE) );
  }
...
FUNCTION     rr_plc_bplmn_pscan_active

DESCRIPTION  Queries whether the PLMN list state machine is currently
             running a GSM power scan activity in background mode

PARAMS       GAS ID

DEPENDENCIES None

RETURN VALUE TRUE if a background power scan is active
...
boolean rr_plc_bplmn_pscan_active(const gas_id_t gas_id)
{
  ...
  if (rr_plc_data_ptr != NULL)
  {
    if ((RR_PLC_BACKGROUND_SEARCH == rr_plc_data_ptr->search_type) &&
        (RR_PLC_GSM_STATE_PSCAN   == rr_plc_data_ptr->gsm_state)   &&
        (RR_PLC_STATE_GSM         == rr_plc_data_ptr->state))
    {
      return(TRUE);
    }
  }
...
FUNCTION     rr_set_plc_service_search_type

DESCRIPTION  Set the PLC service search type
...
===========================================================================*/
void rr_plc_set_service_search_type(boolean foreground_search, const gas_id_t gas_id)
{
  rr_plc_data_t *rr_plc_data_ptr = rr_plc_get_data_ptr(gas_id);

  RR_NULL_CHECK_FATAL(rr_plc_data_ptr);
 
  rr_plc_data_ptr->search_type = foreground_search ? RR_PLC_FOREGROUND_SEARCH : RR_PLC_BACKGROUND_SEARCH;
  MSG_GERAN_HIGH_1_G("Set PLMN search type as %d", rr_plc_data_ptr->search_type); 
  return;
}

#ifdef FEATURE_QSH_MDUMP
QSH_MDUMP_FN_ATTR
void grr_mdump_rr_plc(const gas_id_t gas_id)
{
  rr_plc_data_t *rr_plc_data_ptr = rr_plc_get_data_ptr(gas_id);
  qsh_mdump_collect_high(rr_plc_data_ptr, sizeof(rr_plc_data_t));
}
