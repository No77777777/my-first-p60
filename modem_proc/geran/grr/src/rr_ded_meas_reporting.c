...
$Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_ded_meas_reporting.c#1 $
...
/**
  @brief Move the ARFCN 0 to the last position in the complete_BA_sacch list in the BA list database.

  @param Pointed to the complete_BA_sacch list in the BA list database.
  @return void
*/
void rr_reorder_BA_list(BCCH_frequency_list_T *freq_list_ptr)
{
  unsigned int i;
  ARFCN_T arfcn_zero;
  ...
  for (i = 0; (i < freq_list_ptr->no_of_entries) && (freq_list_ptr->no_of_entries <= BA_MAX_LEN); i++)
  {
    if (freq_list_ptr->BCCH_ARFCN[i].num == 0)
    {
      if ((i+1) != freq_list_ptr->no_of_entries) /* i.e. i is not the last index. */
      {
        arfcn_zero = freq_list_ptr->BCCH_ARFCN[i];
        freq_list_ptr->BCCH_ARFCN[i] = freq_list_ptr->BCCH_ARFCN[i+1];
        freq_list_ptr->BCCH_ARFCN[i+1] = arfcn_zero;
      }
    }
  }
}
...
static void rr_dedicated_mode_irat_measurement_list_add_wcdma_measurements(
  rr_dedicated_mode_irat_measurement_list_t * irat_measurement_list,
  rr_ded_irat_measurements_t                * wcdma_measurements,
  rr_utran_meas_params_t                    * utran_meas_params,
...
)
{
  ...
  for (i = 0;
       ...
       i < ARR_SIZE(wcdma_measurements->entries);
       ++i)
  {
    boolean cell_index_valid = FALSE;
    uint8   cell_index = 0;
    ...
    cell_index = rr_find_wcdma_frequency_index(
      ...
      &wcdma_measurements->entries[i].data.utran_fdd.cell,
      &cell_index_valid,
      &rssi_index_valid,
      &rssi_index
    );
    ...
    if (cell_index_valid == FALSE)
    {
      MSG_GERAN_ERROR_2_G("Reporting index not valid for WCDMA cell (UARFCN: %d, SC: %d)",
                          wcdma_measurements->entries[i].data.utran_fdd.cell.uarfcn,
                          wcdma_measurements->entries[i].data.utran_fdd.cell.scrambling_code);
      continue;
    }
    ...
    if (cell_index < MAX_CELL_COUNT_FOR_REPORT_TYPE(report_type))
    {
      ...
      {
        reported_value = rr_get_reported_RSCP(wcdma_measurements->entries[i].data.utran_fdd.RSCP);
        non_reported_value = rr_get_reported_Ec_No(wcdma_measurements->entries[i].data.utran_fdd.Ec_Nox2);
      }
      else
      {
        reported_value = rr_get_reported_Ec_No(wcdma_measurements->entries[i].data.utran_fdd.Ec_Nox2);
        non_reported_value = rr_get_reported_RSCP(wcdma_measurements->entries[i].data.utran_fdd.RSCP);
      }
          ...
          non_reported_value >= fdd_reporting_threshold_2)
      {
        if (irat_measurement_list->count < ARR_SIZE(irat_measurement_list->entries))
        {
          ...
          entry->data.wcdma.uarfcn           = wcdma_measurements->entries[i].data.utran_fdd.cell.uarfcn;
          entry->data.wcdma.scrambling_code  = wcdma_measurements->entries[i].data.utran_fdd.cell.scrambling_code;
          entry->data.wcdma.reported_rssi    = rr_get_reported_RSSI(wcdma_measurements->entries[i].data.utran_fdd.RSSI);
...
#ifdef FEATURE_WCDMA
  rr_ded_irat_measurements_t               * wcdma_measurements = NULL;
#endif /* FEATURE_WCDMA */
#ifdef FEATURE_GSM_TDS
...
#if defined(FEATURE_WCDMA) || defined(FEATURE_GSM_TDS)
  rr_log_utran_meas_params(&dedicated_mode_params->utran_params.meas_params, gas_id);
#endif /* FEATURE_WCDMA || FEATURE_GSM_TDS */
#ifdef FEATURE_LTE
  rr_log_eutran_meas_params(&dedicated_mode_params->eutran_params.meas_params, gas_id);
#endif /* FEATURE_LTE */
...
)
{
  ...
  surrounding_measurement_T                  gsm_measurements;
#ifdef FEATURE_WCDMA
  rr_ded_irat_measurements_t               * wcdma_measurements = rr_get_wcdma_ded_meas_ptr(gas_id);
#endif /* FEATURE_WCDMA */
#ifdef FEATURE_GSM_TDS
...
)
{
  ...
  rr_ded_irat_measurements_t    *stored_wcdma_ded_meas_ptr = rr_get_wcdma_ded_meas_ptr(gas_id);
  ...
  num_entries = stored_wcdma_ded_meas_ptr->no_of_entries;
  ...
  if (wcdma_ded_nlist_ptr->valid_data == TRUE)
  {
    ...
    for (j = 0; j < num_cells; ++j)
    {
      ...
      {
        wcdma_ded_meas_ptr = &(stored_wcdma_ded_meas_ptr->entries[k].data.utran_fdd);
        /* cells with valid PN offset */
        if (
            (wcdma_ded_nlist_ptr->wcdma_cell_list.cell_list[j].uarfcn.uarfcn == wcdma_ded_meas_ptr->cell.uarfcn) &&
            (wcdma_ded_nlist_ptr->wcdma_cell_list.cell_list[j].cell_code == wcdma_ded_meas_ptr->cell.scrambling_code) &&
            (wcdma_ded_meas_ptr->PN_Offset != 0xFFFF) // use PN_OFFSET_UNKNOWN when defined in RRC
           )
        {
          byte logi = log_3g_meas_rpt->NUM_CELLS;
          byte num_uarfcns = rr_get_num_uarfcns_sent_to_l1_in_dedicated(gas_id);
          log_3g_meas_rpt->rr_3g_report[logi].UARFCN = wcdma_ded_meas_ptr->cell.uarfcn;
          log_3g_meas_rpt->rr_3g_report[logi].SC_CODE = wcdma_ded_meas_ptr->cell.scrambling_code;
          log_3g_meas_rpt->rr_3g_report[logi].DIVERSITY = wcdma_ded_meas_ptr->cell.diversity;
          log_3g_meas_rpt->rr_3g_report[logi].EXPIRES = (word)(104*4.615*(num_uarfcns - wcdma_ded_meas_ptr->PN_position_validity_count));
          log_3g_meas_rpt->rr_3g_report[logi].PN_POS = wcdma_ded_meas_ptr->PN_Offset;

          /* This function loops the complete list to find the highest index that
             matches the given uarfcn */
          report_index = rr_find_wcdma_frequency_index(
            wcdma_ncell_list_data_ptr->wcdma_neighbor_cell_list,
            &wcdma_ded_meas_ptr->cell,
            &found_index,
            &has_freq_only,
            &freq_only_index
          );
          ...
          if ((wcdma_ded_nlist_ptr->mi_params.FDD_REP_QUANT == FDD_REP_QUANT_RSCP) && (has_freq_only == FALSE))
          {
            ...
            log_3g_meas_rpt->rr_3g_report[logi].ENERGY = (int8)wcdma_ded_meas_ptr->RSCP;
          }
          else if ((wcdma_ded_nlist_ptr->mi_params.FDD_REP_QUANT == FDD_REP_QUANT_Ec_No)&& (has_freq_only == FALSE))
          {
            ...
            log_3g_meas_rpt->rr_3g_report[logi].ENERGY = (int8)(wcdma_ded_meas_ptr->Ec_Nox2/2);
          }
          ...
          if ((wcdma_ded_nlist_ptr->mi_params.FDD_REP_QUANT == FDD_REP_QUANT_RSCP) && (has_freq_only == TRUE))
          {
            ...
            log_3g_meas_rpt->rr_3g_report[logi].RSSI = (int8)wcdma_ded_meas_ptr->RSSI;
            log_3g_meas_rpt->rr_3g_report[logi].ENERGY = (int8)wcdma_ded_meas_ptr->RSCP;
          }
  ...
  else if (wcdma_idle_nlist_ptr->valid_data)
  {
    ...
    for (j = 0; j < num_cells; ++j)
    {
      ...
      for (k = 0; ((k < num_entries) && (log_3g_meas_rpt->NUM_CELLS < MAX_WCDMA_STORED_DED_MEASUREMENTS)); ++k)
      {
        ...
        /* matched measurement */
        if (
            (wcdma_idle_nlist_ptr->wcdma_cell_list.cell_list[j].uarfcn.uarfcn == wcdma_ded_meas_ptr->cell.uarfcn) &&
            (wcdma_idle_nlist_ptr->wcdma_cell_list.cell_list[j].cell_code == wcdma_ded_meas_ptr->cell.scrambling_code) &&
...
static boolean rr_irat_measurement_match_fdd_uarfcn(
...
/*************************************************************************
Function
rr_irat_store_dedicated_wcdma_measurements


Description
Process rr_irat_store_dedicated_wcdma_measurements message from L1 containing
measurement on WCDMA cells

Parameters
[IN]
rr_l1_wcdma_measurements_t *new_utran_fdd_measurements: The input message

Rules
1) Only known cells are reported by L1

***************************************************************************/
static void rr_irat_store_dedicated_wcdma_measurements(
 rr_l1_wcdma_measurements_t * new_utran_fdd_measurements,
 const gas_id_t               gas_id
)
{
  byte                           i, k;
  uint16                         num_best_cells;
  ...
  ASSERT (new_utran_fdd_measurements != NULL);
  RR_NULL_CHECK_FATAL(stored_wcdma_ded_meas_ptr);
  RR_NULL_CHECK_FATAL(rr_ded_meas_reporting_data_ptr);

  MSG_GERAN_HIGH_2_G("Received %d UTRAN FDD measurements for UARFCN %d",
           new_utran_fdd_measurements->no_of_entries,
           new_utran_fdd_measurements->uarfcn);

  if (new_utran_fdd_measurements->no_of_entries > MAX_WCDMA_MEASUREMENTS_PER_FREQUENCY)
  {
    MSG_GERAN_ERROR_2_G("RR number of FDD ded meas out of range:%d, max:%d",new_utran_fdd_measurements->no_of_entries,MAX_WCDMA_MEASUREMENTS_PER_FREQUENCY);
    return;
  }

  for (i = 0; i < new_utran_fdd_measurements->no_of_entries; ++i)
  {
    MSG_GERAN_HIGH_3_G("PSC: %d, RSCP: %d, Ec/No: %d",
                        new_utran_fdd_measurements->entries[i].scrambling_code,
                        new_utran_fdd_measurements->entries[i].rscp,
                        new_utran_fdd_measurements->entries[i].ecno);
                      }

  rr_ded_meas_reporting_data_ptr->mph_surround_wcdma_dedicated_meas_ind_received = TRUE;
  num_best_cells = new_utran_fdd_measurements->no_of_entries;

    /* Delete all cells in specified ARFCN, no cells found on this frequency */
  if (num_best_cells == 0)
  {
    rr_ded_irat_measurements_delete_matching_entries(
      stored_wcdma_ded_meas_ptr,
      rr_irat_measurement_match_fdd_uarfcn,
      &new_utran_fdd_measurements->uarfcn
    );
  }

  if (stored_wcdma_ded_meas_ptr != NULL)
  {
    ...
    for (i = stored_wcdma_ded_meas_ptr->no_of_entries; i > 0; --i)
    {
      if (stored_wcdma_ded_meas_ptr->entries[i-1].data.utran_fdd.cell.uarfcn == new_utran_fdd_measurements->uarfcn)
      {
        uint32 j;

        for (j = 0; j < new_utran_fdd_measurements->no_of_entries && j < MAX_WCDMA_MEASUREMENTS_PER_FREQUENCY; ++j)
        {
          if (stored_wcdma_ded_meas_ptr->entries[i-1].data.utran_fdd.cell.scrambling_code ==
              new_utran_fdd_measurements->entries[j].scrambling_code)
          {
            break;
          }
        }

        if (j == new_utran_fdd_measurements->no_of_entries)
        {
          /* The ncell is not present in the new measurements from L1, so
          should be deleted from the store. */
          rr_ded_irat_measurements_delete_index(stored_wcdma_ded_meas_ptr, i-1);
        }
      }
    }
  }
  ...
  {
    rcvd_meas.UARFCN          = new_utran_fdd_measurements->uarfcn;
    rcvd_meas.Scrambling_Code = new_utran_fdd_measurements->entries[k].scrambling_code;
    rcvd_meas.RSSI            = new_utran_fdd_measurements->entries[k].rssi;
    rcvd_meas.Ec_Nox2         = new_utran_fdd_measurements->entries[k].ecno;
    rcvd_meas.RSCP            = new_utran_fdd_measurements->entries[k].rscp;
    rcvd_meas.PN_Offset       = new_utran_fdd_measurements->entries[k].pn_offset;
    ...
    for (i = 0; i < stored_wcdma_ded_meas_ptr->no_of_entries; ++i)
    {
      wcdma_cell_T *cell = &stored_wcdma_ded_meas_ptr->entries[i].data.utran_fdd.cell;
    ...
    stored_meas = &stored_wcdma_ded_meas_ptr->entries[i].data.utran_fdd;

    /* if not present create a new entry */
    if (i == stored_wcdma_ded_meas_ptr->no_of_entries)
    {
      stored_wcdma_ded_meas_ptr->no_of_entries++;
      stored_wcdma_ded_meas_ptr->entries[i].rat = RR_RAT_WCDMA;
...
)
{
  ...
  switch (surround_irat_meas_ind->rat)
  {
    ...
    {
      rr_irat_store_dedicated_wcdma_measurements(
        &surround_irat_meas_ind->measurements.wcdma,
        gas_id
      );
      break;
...
#if defined(FEATURE_GSM_TDS) || defined(FEATURE_WCDMA)
/**
...
*/
void rr_ded_irat_measurements_delete_index(
...
/**
  ...
  @param match_value The value that is being searched for - if a measurement
         ...
         structure.
*/
void rr_ded_irat_measurements_delete_matching_entries(
  rr_ded_irat_measurements_t       * measurements,
...
#endif /* FEATURE_GSM_TDS || FEATURE_WCDMA*/
...
)
{
  ...
  for (i = 0; i < MAX_INSTANCES; i++)
  {
    ...
    if (instance->common_params.BA_IND == rr_ba_lists_ptr->complete_BA_sacch.BA_IND)
    {
      if (instance->si2q_mi.mi_params.BSIC_Desc_PI == TRUE)
      {
        byte j;
        BSIC_Desc_T *bsic_desc = &instance->si2q_mi.mi_params.BSIC_Desc;
        BSIC_T tmp_bsic;
...
)
{
  ...
  for (i = 0; i < si2quater_instances->num_instances && i < ARR_SIZE(si2quater_instances->instance); ++i)
  {
    ...
    if (instance && instance->valid_data)
    {
      if (instance->si2q_mi.si2q_params.MP_Desc_PI)
      {
        si2quater_meas_params_instance = &instance->si2q_mi.si2q_params;
      }

      if (instance->si2q_mi.si2q_params.gprs_meas_params.valid)
      {
        si2quater_gprs_meas_params_instance = &instance->si2q_mi.si2q_params;
      }
    }
  }
  ...
  */
  ...
  rr_log_gsm_meas_params(&dst->meas_params, gas_id);
}
...
)
{
  ...
  for (i = 0; i < mi_instances->num_instances && i < ARR_SIZE(mi_instances->instance); ++i)
  {
    ...
    if (instance && instance->valid_data)
    {
      mi_common_params_instance = &instance->si2q_mi.mi_params;

      if (instance->si2q_mi.mi_params.MP_Desc_PI)
      {
        mi_meas_params_instance = &instance->si2q_mi.mi_params;
      }
    }
  }
...
)
{
  ...
  for (i = 0; i < si2quater_instances->num_instances && i < ARR_SIZE(si2quater_instances->instance); ++i)
  {
    ...
    if (instance && instance->valid_data)
    {
      if (instance->si2q_mi.si2q_params.WCDMA_3G_MP_Desc_PI)
      {
        si2quater_3g_meas_params_instance = &instance->si2q_mi.si2q_params;
      }

      if (instance->si2q_mi.si2q_params.gprs_3g_meas_params.valid)
      {
        si2quater_gprs_3g_meas_params_instance = &instance->si2q_mi.si2q_params;
      }

      if (instance->si2q_mi.si2q_params.FDD_additional_MPs_2_PI)
      {
        si2quater_3g_addl_meas_params_2_instance = &instance->si2q_mi.si2q_params;
      }
    }
  }
  ...
  */
  ...
  rr_log_utran_meas_params(&dst->meas_params, gas_id);
}
...
)
{
  ...
  for (i = 0; i < mi_instances->num_instances && i < ARR_SIZE(mi_instances->instance); ++i)
  {
    ...
    if (instance && instance->valid_data)
    {
      if (instance->si2q_mi.mi_params.MI_3G_MP_Desc_PI)
      {
        mi_3g_meas_params_instance = &instance->si2q_mi.mi_params;
      }
  
      if (instance->si2q_mi.mi_params.fdd_reporting_threshold_2_valid)
      {
        mi_3g_addl_meas_params_2_instance = &instance->si2q_mi.mi_params;
      }
    }
  }
...
)
{
  ...
  rr_log_eutran_meas_params(&dst->meas_params, gas_id);
}
...
)
{
  ...
  rr_dedicated_mode_process_gsm_params(scell, mi_instances, gas_id);

#if defined(FEATURE_WCDMA) || defined(FEATURE_GSM_TDS)
  rr_dedicated_mode_process_utran_params(scell, mi_instances, gas_id);
#endif /* FEATURE_WCDMA || FEATURE_GSM_TDS */

#ifdef FEATURE_LTE
  rr_dedicated_mode_process_eutran_params(scell, mi_instances, gas_id);
#endif /* FEATURE_LTE */
}
