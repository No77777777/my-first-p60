...
$Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_control.c#8 $ $DateTime: 2020/07/14 23:56:54 $ $Author: pwbldsvc $
...
#define RR_T304_ASSISTED_MIN_DURATION   (2000)
#define RR_T304_UNASSISTED_MIN_DURATION (5000)
...
typedef struct
{
  ...
  boolean                    waiting_for_rr_interrat_wl1_stop_ind;
#endif /* FEATURE_WCDMA || FEATURE_LTE || FEATURE_GSM_TDS */
  ...
  struct
  {
    ...
    to FALSE if registration on LTE is rejected with reject cause 15 or 14 
    and is indicated in RR_IRAT_ALLOWED_STATUS_IND. */
    boolean g2l_resel_allowed;
      
    /* TRUE if reselection to LTE is allowed, FALSE otherwise.  This may be set
    to FALSE if registration on LTE is rejected with reject cause 15 
    and is indicated in RR_IRAT_ALLOWED_STATUS_IND. */
    boolean g2l_redir_allowed;
#endif /* FEATURE_LTE */

#ifdef FEATURE_WCDMA
    /* TRUE if reselection/redirection to WCDMA is allowed, FALSE otherwise.  This may be set
    to FALSE if registration on WCDMA is rejected with reject cause 15 and is
    indicated in RR_IRAT_ALLOWED_STATUS_IND. */
    boolean g2w_resel_redir_allowed;
#endif /* FEATURE_WCDMA */
  } irat_allowed_by_nas;
#endif /* FEATURE_LTE || FEATURE_WCDMA */
  ...
  uint8   tc_0_4_read_count_fr_si13;
  uint8   tc_5_read_count_fr_si2bis;
  uint8   tc_4_5_read_count_fr_si2ter;
} rr_control_data_t;
...
static rr_rat_e rr_map_x2g_resel_type_to_rat(rr_x2g_resel_type_e x2g_resel_type);
 ...
 *
 * @param  for_new_search - if TRUE, BPLMN DSC is reset to scell max
 *
 * @return VOID
 *
 */
...
)
{
  ...
  if (for_new_search == TRUE)
  {
    ...
    rr_control_data_ptr->bplmn_dsc_max = rr_gprs_get_max_dsc_value(gas_id);
...
)
{
  ...
  if (rr_control_data_ptr->pending_mode_change_data.valid)
  {
    ...
    boolean gsm_only =
        rr_rat_pri_list_info_gsm_only(&rr_control_data_ptr->pending_mode_change_data.rat_pri_list_info);
...
#ifdef FEATURE_PRIORITIZE_LTE_FREQ
    if ((rr_is_lte_enabled(gas_id) != old_lte_enabled) && (old_lte_enabled == TRUE))
    {
      MSG_GERAN_HIGH_0_G("[LTE Prio] LTE got disabled");
      rr_general_stop_lte_prioritization(gas_id);
    }
#endif /* FEATURE_PRIORITIZE_LTE_FREQ */  
#ifdef FEATURE_WCDMA
...
/**
 * Compares the current rat priority list with newly receive rat priority list.
 */
boolean rr_change_in_supported_IRAT_info(sys_rat_pri_list_info_s_type *rat_pri_list_info, const gas_id_t gas_id)
{
  boolean rat_info_changed = FALSE;

#ifdef FEATURE_WCDMA
  rat_info_changed |= (rr_is_wcdma_enabled(gas_id) != rr_rat_pri_list_info_is_sys_mode_present(rat_pri_list_info, SYS_SYS_MODE_WCDMA));
#endif /* FEATURE_WCDMA */
#ifdef FEATURE_LTE
  rat_info_changed |= (rr_is_lte_enabled(gas_id) != rr_rat_pri_list_info_is_sys_mode_present(rat_pri_list_info, SYS_SYS_MODE_LTE));
#endif /*FEATURE_LTE*/
#ifdef FEATURE_GSM_TDS
  rat_info_changed |= (rr_is_utran_tdd_enabled(gas_id) != rr_rat_pri_list_info_is_sys_mode_present(rat_pri_list_info, SYS_SYS_MODE_TDS));
#endif /*FEATURE_GSM_TDS*/

  return rat_info_changed;
} /* rr_compare_rat_pri_list_info */
...
)
{
  ...
  rr_control_data_ptr->waiting_for_rr_interrat_wl1_stop_ind = FALSE;
#endif /* FEATURE_WCDMA || FEATURE_LTE  || FEATURE_GSM_TDS */
  rr_control_data_ptr->is_csfb_call_in_progress = FALSE;
  rr_control_data_ptr->redir_after_csfb = FALSE;
...
#ifdef FEATURE_LTE
  rr_control_data_ptr->irat_allowed_by_nas.g2l_resel_allowed = TRUE;
  rr_control_data_ptr->irat_allowed_by_nas.g2l_redir_allowed = TRUE;
#endif /* FEATURE_LTE */

#ifdef FEATURE_WCDMA
  rr_control_data_ptr->irat_allowed_by_nas.g2w_resel_redir_allowed = TRUE;
#endif /* FEATURE_WCDMA */
...
)
{
  ...
  rr_select_bcch_recovery_init(gas_id);
...
{
#ifdef FEATURE_WCDMA
  return(rr_g2x_rat_supported(RR_RAT_WCDMA, gas_id));
#else
  return(FALSE);
#endif /* FEATURE_WCDMA */
}
...
{
#ifdef FEATURE_GSM_TDS
  return(rr_g2x_rat_supported(RR_RAT_UTRAN_TDD, gas_id));
#else
  return(FALSE);
#endif /* FEATURE_GSM_TDS */
}
...
)
{
  ...
  rr_reset_redir_after_csfb_call(gas_id);
  ...
  switch ( deact_reason )
  {
    case RR_DEACT_REASON_DEACT_REQ:
    {
      ...
      /* reset resource cnt */
      rr_ms_reset_no_resource_cnt(gas_id);

#if defined (FEATURE_DUAL_SIM) && defined(FEATURE_GSM_DECODE_ALL_FREQS)
      /* reset scan more freq */
      rr_ms_reset_scan_more_freq_allowed(gas_id);
#endif /* FEATURE_DUAL_SIM && FEATURE_GSM_DECODE_ALL_FREQS */
    ...
    case RR_DEACT_REASON_STOP_CAMPING_THEN_RESTART:
    {
      ...
      (void) rr_gprs_bsi_acquisition_control(EV_RESET_SOFTWARE, NULL, gas_id);
  ...
  rr_reset_locking_channel_info(gas_id);
#endif /**/
...

  {
    ...
    eng_mode_scell_ncell_info_t sncell_info;
    ...
    geran_eng_mode_data_write(
      ENG_MODE_SCELL_NCELL_INFO,   // geran_eng_mode_info_type
      (void*)&sncell_info,         // param_copy
      FALSE,                       // valid_flag
      gas_id                       // gas_id
    );
  }
...
boolean rr_plmn_search_in_progress(const gas_id_t gas_id)
{
  // Obtain a pointer to the module data
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr(gas_id);
  RR_NULL_CHECK_FATAL(rr_control_data_ptr);
...
void rr_reject_nas_msg_during_intrn_shutdown(rr_cmd_bdy_type* msg_ptr, const gas_id_t gas_id)
{
  ...
  switch (msg_ptr->message_header.message_id)
  {
    ...
    case RR_EST_REQ:
      ...
      rr_send_abort_ind(OTHER_REASON, MM_RR_SGLTE_SRVCC_CS_HANDOVER, gas_id);
      break;
...
static void rr_validate_rat_priority_for_plmn_search(sys_rat_pri_list_info_s_type *rat_pri_list_ptr, const gas_id_t gas_id)
{
  ...
  {
    MSG_GERAN_ERROR_0_G("Invalid RAT priority list pointer");
  }
  else
  {
...
)
{
...
#if defined(FEATURE_WCDMA) || defined(FEATURE_GSM_TDS) || defined(FEATURE_LTE)
  /* validate the RATs present in the priority list provided in this search request */
  ...
  RR_ZERO_STRUCTURE(rr_plmn_list_req_info_ptr->arfcn_list);
  RR_ZERO_STRUCTURE(rr_plmn_list_req_info_ptr->preferred_plmn_list);
  rr_plmn_list_req_info_ptr->arfcn_list = plmn_list_req->arfcn_list;
  rr_plmn_list_req_info_ptr->srch_priority = plmn_list_req->srch_priority;
  rr_plmn_list_req_info_ptr->preferred_plmn_list = plmn_list_req->preferred_plmn_list;
   ...
   */
  rr_plc_setup(foreground ? RR_PLC_FOREGROUND_SEARCH : RR_PLC_BACKGROUND_SEARCH, 0, FALSE, gas_id);
...
)
{
   ...
   * setup rr_plmn_list_control()
   */
  rr_plc_setup(foreground ? RR_PLC_SGLTE_X2G_FOREGROUND_SEARCH : RR_PLC_SGLTE_X2G_BACKGROUND_SEARCH, 0, FALSE, gas_id);
...
)
{
   ...
   * setup rr_plmn_list_control()
   */
  rr_plc_setup(RR_PLC_TTOG_SEARCH, msg_ptr->timer_val, msg_ptr->high_priority, gas_id);
}
#endif /*FEATURE_GSM_TDS*/
...
)
{
   ...
   * setup rr_plmn_list_control()
   */
  rr_plc_setup(RR_PLC_WTOG_SEARCH, msg_ptr->timer_val, msg_ptr->high_priority, gas_id);
}
    ...
    for (other_gas_id = GERAN_ACCESS_STRATUM_ID_1; other_gas_id <= GERAN_ACCESS_STRATUM_ID_MAX; other_gas_id++)
    {
      if (other_gas_id != gas_id)
      {
        ...
        if (rr_mode_gsm_active_or_starting(other_gas_id))
        {
          ...
          rr_send_mph_set_priority_ind(&rr_priority_info, other_gas_id);
        }
      }
    }
  }
...
)
{
   ...
   * setup rr_plmn_list_control()
   */
  rr_plc_setup(RR_PLC_LTOG_SEARCH, msg_ptr->timer_val, msg_ptr->high_priority, gas_id);
}
...
)
{
  ...
  else if (rr_control_data_ptr->rr_state == RR_X2G_CCO_FAILED &&
           rr_x2g_cco_in_progress(gas_id))
  {
    pass_to_rr_control = TRUE;
  }
...
/**
 * Function: This function handles MPH_BLOCK_QUALITY_IND received
 *           during the PLMN List search state

 * @Params:  Incoming inter-task message (MPH_BLOCK_QUALITY_IND)
 *
 * @Outputs: New event for main FSM
 */
static rr_event_T rr_plc_block_quality_report(rr_cmd_bdy_type *msg_ptr, const gas_id_t gas_id)
{
  if (msg_ptr != NULL)
  {
#if defined(FEATURE_DUAL_SIM) || defined(FEATURE_TRIPLE_SIM)
    ...
    {
      (void)rr_ms_plmn_search_update_priority_rqrd(FALSE, FALSE, gas_id);
      if (rr_plc_bplmn_pscan_active(gas_id))
      {
        /* for DSDS background power scan, update the Search Health Metric */
        /* which is analogous to the Downlink Signalling Counter mechanism */
        return(rr_ms_bplmn_dsc_check(msg_ptr->mph.block_quality_ind.block_quality, gas_id));
      }
      else
      {
        MSG_GERAN_HIGH_1_G("GL1 reports block quality %d during PLMN search",
                           (int)(msg_ptr->mph.block_quality_ind.block_quality));      
      }
    }
#endif /* FEATURE_DUAL_SIM || FEATURE_TRIPLE_SIM */
  }
    ...
    case MS_RR_L1:
      switch (msg_id)
      {
        case MPH_BLOCK_QUALITY_IND:
          new_rr_event = rr_plc_block_quality_report(new_message_ptr, gas_id);
          break;
    ...
    case MS_MM_RR:
      switch (msg_id)
      {
        ...
        case RR_CHANGE_MODE_IND:
        {
          ...
          if (!rr_plc_search_in_foreground(gas_id))
          {
            MSG_GERAN_HIGH_0_G("storing Change mode parameters to rat prio list for BPLMN search");
        ...
        case RR_IMSG:
        {
          switch (new_message_ptr->rr.header.imh.message_id)
          {
            ...
            case RR_IMSG_MSC_CHANGE_IND:
            {
              // TODO - check this
              /*
              If RR is camped, a RR_SERVICE_IND can be sent to MM if required
              But search type is other than BG search, it is assumed that G was inactive and
              IRAT PLMN_SEARCH_REQ is received
              */
              rr_msc_update(
                (!rr_plc_search_in_foreground(gas_id)),    // can_send_service_ind
                gas_id   // gas_id
              );
              break;
            }
            ...
            case RR_IMSG_GPRS_IA_IND:
            {
              ...
              else
              {
                ...
                // Call the supplied call-back function indicating that the IA should be ignored
                if (imsg->cb_fn != NULL)
                {
                  (*imsg->cb_fn)(
                    imsg->params,   // params
            ...
            case RR_IMSG_PAGE_MODE_IND:
            {
              ...
              if (imsg->new_page_mode == PAGING_REORGANIZATION)
              {
                MSG_GERAN_HIGH_0_G("Forcefully setting page mode to Normal during BPLMN search.");
                rr_ce_update_l1_page_mode(NORMAL_PAGING, gas_id);
              }
              break;
            }
...
)
{
  ...
  else if(rr_control_data_ptr->waiting_for_rr_interrat_wl1_stop_ind)
  {
    MSG_GERAN_HIGH_0_G("No G2X measurements allowed, waiting for RR_INTERRAT_WL1_STOP_IND");
    return FALSE;
  }
#ifdef FEATURE_GSM_TDS
...
boolean rr_g2l_resel_allowed(const gas_id_t gas_id)
{
  ...
  return rr_control_data_ptr->irat_allowed_by_nas.g2l_resel_allowed;
}

/**
  @brief Check whether G2L redirection is allowed by NAS or not.

  @return TRUE if G2L redirection is allowed, FALSE otherwise.
*/
boolean rr_g2l_redir_allowed(const gas_id_t gas_id)
{
  // Obtain a pointer to the module data
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr(gas_id);
  RR_NULL_CHECK_FATAL(rr_control_data_ptr);

  return rr_control_data_ptr->irat_allowed_by_nas.g2l_redir_allowed;
}
#endif /* FEATURE_LTE */

#ifdef FEATURE_WCDMA
/**
  @brief Check whether G2W reselection/redirection is allowed by NAS or not.

  @return TRUE if G2W reselection/redirection is allowed, FALSE otherwise.
*/
boolean rr_g2w_resel_redir_allowed(const gas_id_t gas_id)
{
  // Obtain a pointer to the module data
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr(gas_id);
  RR_NULL_CHECK_FATAL(rr_control_data_ptr);

  return rr_control_data_ptr->irat_allowed_by_nas.g2w_resel_redir_allowed;
}
#endif /* FEATURE_WCDMA */
...
/*!
 * \brief function to allow redir when fall back mode is enabled.
 *
 * \param rat_type (either WCDMA/LTE)
 * \param gas_id 
 */
static boolean rr_allow_redir_during_fallback_mode(rr_cell_selection_target_e rat_type, const gas_id_t gas_id)
{ 
  boolean redir_allowed = TRUE;

  MSG_GERAN_HIGH_4_G("Redirection to RAT = %d, fallback pref mode = %d, fallback flag = %d, coll fallback nv = %d",
    rat_type,
    rr_get_fallback_pref_mode(gas_id),
  ...
  {
    if ((rat_type == FDD_TARGET) && (rr_get_fallback_pref_mode(gas_id) != TRM_FALLBACK_UMTS_PREF)) 
    {
      redir_allowed = FALSE;
    }
    else if ((rat_type == LTE_TARGET) && (rr_get_fallback_pref_mode(gas_id) != TRM_FALLBACK_LTE_PREF))
    {
      redir_allowed = FALSE;
    }
  }

  return redir_allowed;
}
...
/**
  @brief Handle GERAN_GRR_MTC_DRX_REQ and send a response back.

  @return None.
*/
static void rr_handle_mtc_drx_req(geran_grr_mtc_drx_req_t *req_ptr, const gas_id_t gas_id)
{
  geran_grr_mtc_drx_rsp_t   rsp;
  sys_get_drx_status_e_type status = SYS_DRX_STATUS_NOT_IDLE;
  uint8                     drx_level = 0xFF;
  errno_enum_type           result;
  ...
  msgr_init_hdr_variant(
    &rsp.hdr, 
    MSGR_GERAN_GRR, 
    GERAN_GRR_MTC_DRX_RSP,
    RR_GAS_ID_TO_AS_ID);

  /* When we are INACTIVE */
  if (rr_is_control_state(RR_INACTIVE, gas_id) || rr_is_control_state(RR_GOING_INACTIVE, gas_id))
  {
    status = SYS_DRX_STATUS_NO_SERVICE;
  }
  /* When in IDLE state */
  else if (grr_in_camped_state(gas_id) && (rr_macinfo_get_mac_status(gas_id) == MAC_IDLE))
  {
    status = SYS_DRX_STATUS_SUCCESS;
  }
  /* During PLMN search */
  else if (rr_is_control_state(RR_PLMN_LIST_SEARCH, gas_id))
  {
    /* We will be OOS during foreground search. */
    if (rr_plc_search_in_foreground(gas_id))
    {
      status = SYS_DRX_STATUS_NO_SERVICE;
    }
  ...
  /* Rest are considered as NOT in IDLE */
  else
  {
    status = SYS_DRX_STATUS_NOT_IDLE;
  }

  if (status == SYS_DRX_STATUS_SUCCESS)
  {
    gprs_scell_info_t *scell_ptr = rr_gprs_get_scell_info(gas_id);
    RR_NULL_CHECK_FATAL(scell_ptr);
    
    /* Sanity check */
    if (scell_ptr->gsm.control_channel_description_valid)
    {
      drx_level = scell_ptr->gsm.control_channel_description.BS_PA_MFRMS;

      if (drx_level < BS_PA_MFRMS_MIN)
      {
        MSG_GERAN_ERROR_2_G("DRX level adjusted: old=%d new=%d", drx_level, BS_PA_MFRMS_MIN);
        drx_level = BS_PA_MFRMS_MIN;
      }
      else if (drx_level > BS_PA_MFRMS_MAX)
      {
        MSG_GERAN_ERROR_2_G("DRX level adjusted: old=%d new=%d", drx_level, BS_PA_MFRMS_MAX);
        drx_level = BS_PA_MFRMS_MAX;
      }
    }
    else
    {
      MSG_GERAN_ERROR_0_G("CCD not valid");
      status = SYS_DRX_STATUS_NOT_IDLE;
    }
  }

  rsp.status = status;
  rsp.drx_level = drx_level;
  
  MSG_GERAN_HIGH_2_G("Sending GERAN_GRR_MTC_DRX_RSP:: status=%d, drx_level=%d", status, drx_level);
...
/**
  @brief Get the arfcn for given arfcn number and band id combination.

  @return None.
*/

static ARFCN_T rr_convert_to_arfcn_type(
  uint16 arfcn,
...
)
{
  ...
  if (is_band_valid)
  {
    switch (band_as_sys_sband)
    {
      ...
      {
        if (rr_gapi_is_supported_band( (sys_band_T) SYS_SBAND_GSM_PGSM_900, gas_id))
        {
  ...
  while (rr_event != EV_NO_EVENT)
  {
    ...
    if (rr_control_data_ptr->sglte_g2x_plmn_list_search_in_progress == TRUE)
    {
      ...
      else if ((rr_event == EV_INPUT_MESSAGE) && (message_set == MS_TIMER) )
      {
         ...
         MSG_GERAN_HIGH_0_G("RR_PLMN_LIST_SEARCH_GUARD_TIMER expired,
            RR_QSH_EVENT_MPLMN_TIMEOUT notified");
         rr_qsh_event_notify(RR_QSH_EVENT_MPLMN_TIMEOUT, gas_id);
#endif //FEATURE_QSH_EVENT_NOTIFY_TO_QSH
    ...
    if (rr_event == EV_INPUT_MESSAGE)
    {
      if (message_set == MS_MM_RR)
      {
        switch ((mm_rr_message_id_enum_T) message_id)
        {
          ...
          case RR_IRAT_ALLOWED_STATUS_IND:
          {
...
#ifdef FEATURE_LTE
            boolean g2l_resel_allowed = new_message_ptr->mm.rr_irat_allowed_status_ind.lte_reselection_allowed;
            boolean g2l_redir_allowed = new_message_ptr->mm.rr_irat_allowed_status_ind.lte_redir_allowed;
#endif /* FEATURE_LTE */
#ifdef FEATURE_WCDMA
            boolean g2w_resel_redir_allowed = new_message_ptr->mm.rr_irat_allowed_status_ind.wcdma_resel_redir_allowed;
#endif /* FEATURE_WCDMA */

#ifdef FEATURE_LTE
            if ((rr_control_data_ptr->irat_allowed_by_nas.g2l_resel_allowed != g2l_resel_allowed) ||
                (rr_control_data_ptr->irat_allowed_by_nas.g2l_redir_allowed != g2l_redir_allowed))
            {
              isChanged = TRUE;
            }
#endif /* FEATURE_LTE */
...
#ifdef FEATURE_WCDMA
            if (rr_control_data_ptr->irat_allowed_by_nas.g2w_resel_redir_allowed != g2w_resel_redir_allowed)
            {
              isChanged = TRUE;
            }
#endif /* FEATURE_WCDMA */
            ...
            {
#ifdef FEATURE_LTE            
              rr_control_data_ptr->irat_allowed_by_nas.g2l_resel_allowed = g2l_resel_allowed;
              rr_control_data_ptr->irat_allowed_by_nas.g2l_redir_allowed = g2l_redir_allowed;
              MSG_GERAN_HIGH_2_G(
                "Allowed LTE IRAT by NAS: LTE resel: %d, LTE redir: %d", 
                g2l_resel_allowed, 
                g2l_redir_allowed); 
#endif /* FEATURE_LTE */             
#ifdef FEATURE_WCDMA
              rr_control_data_ptr->irat_allowed_by_nas.g2w_resel_redir_allowed = g2w_resel_redir_allowed;              
              MSG_GERAN_HIGH_1_G(
                "Allowed WCDMA IRAT by NAS: WCDMA resel/redir: %d", 
                g2w_resel_redir_allowed);
#endif /* FEATURE_WCDMA */

              /* Send an indication to ourselves that the irat allowed status
              has been updated.  This will trigger an update of GRR if
              necessary. */
              rr_send_imsg(RR_IMSG_IRAT_ALLOWED_UPDATED_IND, gas_id);
            }

            rr_event = EV_NO_EVENT;
            break;
          } /* RR_IRAT_ALLOWED_STATUS_IND */
#endif /* FEATURE_LTE || FEATURE_WCDMA */
...
#endif // FEATURE_QSH_EVENT_METRIC
              rr_set_redir_after_csfb_call(gas_id);
            }
          ...
          {
            if (rr_is_procedure_for_ps(new_message_ptr->mm.rr_procedure_ind.proc_type, gas_id))
            {
              rr_ms_set_connection_priority(new_message_ptr->mm.rr_procedure_ind.proc_type, TRUE, gas_id);
            }
            else 
            { /* RR Procedure Indication from NAS is for CS */
              if (rr_control_data_ptr->rr_state == RR_DATA_TRANSFER)
              {              
                rr_ms_set_connection_priority(new_message_ptr->mm.rr_procedure_ind.proc_type, FALSE, gas_id);
              }
              else
              {
                MSG_GERAN_HIGH_1_G("RR_PROCEDURE_IND in state %d, expect CS procedure in RR_EST_REQ", (uint32)(rr_control_data_ptr->rr_state));
              }
            }
            rr_event = EV_NO_EVENT;
            break;
      ...
      else if (message_set == MS_TDSRRC_RR)
      {
        ...
        else if (message_id == (byte) TDSRRC_DEDICATED_PRIORITIES_RSP)
        {
          ...
          else
          {
            ...
            {
              MSG_GERAN_HIGH_0_G("GERAN sub with no LTE Support: request for Dedicated priorities N/A");
            }
          }
      ...
      else if (message_set == MS_RRC_RR)
      {
        ...
        else if (message_id == (byte) RR_INTERRAT_WL1_STOP_IND)
        {
          MSG_GERAN_MED_1_G("RR rxd RR_INTERRAT_WL1_STOP_IND cause:%d",new_message_ptr->rr_wl1_stop_ind.cause);



          if (new_message_ptr->rr_wl1_stop_ind.cause == RR_WL1_STOP_IND_W2G_CCO)
          {
            rr_control_data_ptr->waiting_for_rr_interrat_wl1_stop_ind = FALSE;
            rr_send_imsg(RR_IMSG_X2G_CCO_COMPLETE_IND, gas_id);
            rr_event = EV_NO_EVENT;
      ...
      else if ((message_set == MS_TIMER) && (message_id == MID_TIMER_EXPIRY))
      {
...
#ifdef FEATURE_PRIORITIZE_LTE_FREQ
        else if (timer_id == RR_PRIORITIZE_LTE_DEFER_TIMER)
        {
          rr_general_check_and_prioritize_lte_frequencies(gas_id);
        }
        else if (timer_id == RR_PRIORITIZE_LTE_ABORT_TIMER)
        {
          if (get_rr_state(gas_id) == RR_GRR)
          {
            MSG_GERAN_HIGH_0_G("RR_PRIORITIZE_LTE_ABORT_TIMER expired, falling back to legacy mode");
          ...
          else
          {
            MSG_SPRINTF_2(
              MSG_SSID_DFLT,
              MSG_LEGACY_MED,
              "gs%d:RR_PRIORITIZE_LTE_ABORT_TIMER expiry ignored in state %s",
              (gas_id+1),
        ...
        else if (timer_id == RR_SERV_CELL_BAD_SNR_TIMER)
        {
          MSG_GERAN_MED_0_G("RR_SERV_CELL_BAD_SNR_TIMER expired");
        }
        /****************************************************************/
        ...
        /* Handle deprioritized cell timer expiry */
        else if (timer_id == RR_CELL_DEPRIORITIZED_TIMER)
        {
          rr_process_deprioritized_cell_timer_expiry(gas_id);
        }
      ...
      else if (message_set == MS_MSGR_RR)
      {
        switch (new_message_ptr->rr_msgr_msg.message.hdr.id)
        {
          ...
          case GERAN_GRR_FAKE_GCELL_ACTION_REQ:
          {
            if (rr_get_fake_bts_cell_barring_enabled(gas_id) == TRUE)
            {
              if (new_message_ptr->rr_msgr_msg.message.geran_grr_msg_req.fake_gcell_action_req.barred_time != 0)
              {
                ...
                fake_cell.arfcn = rr_convert_to_arfcn_type(new_message_ptr->rr_msgr_msg.message.geran_grr_msg_req.fake_gcell_action_req.arfcn, band_ind, gas_id);
                ...
                if ((scell_info_ptr->gsm.BCCH_ARFCN.num == fake_cell.arfcn.num) && (scell_info_ptr->gsm.BCCH_ARFCN.band == fake_cell.arfcn.band) &&
                    ...
                    (rr_same_lai(&scell_info_ptr->gsm.location_area_identification, &fake_cell.cgi.lai)) && (scell_info_ptr->gsm.cell_identity == fake_cell.cgi.ci) &&
            ...
            {
              MSG_GERAN_HIGH_0_G("FAKE BTS barring is disabled");
              rr_event = EV_NO_EVENT;
            }
            break;
          }
...
#ifdef FEATURE_FAKE_BTS_CELL_DEPRIORITIZE
          case GERAN_GRR_DEPRIORITIZE_GCELL_REQ:
          {
            if (rr_get_fake_bts_cell_deprioritization_enabled(gas_id) == TRUE)
            {
              rr_deprioritized_cell_info_T depriotize_cell;
              geran_grr_band_ind_e band_ind;

              gprs_scell_info_t *scell_info_ptr = rr_gprs_get_scell_info(gas_id);
              band_ind = ((new_message_ptr->rr_msgr_msg.message.geran_grr_msg_req.deprioritize_gcell_req.arfcn >> 15)& 0x01);
              new_message_ptr->rr_msgr_msg.message.geran_grr_msg_req.deprioritize_gcell_req.arfcn &= 0x7FFF;
              depriotize_cell.arfcn = geran_grr_convert_to_arfcn_type(new_message_ptr->rr_msgr_msg.message.geran_grr_msg_req.deprioritize_gcell_req.arfcn, band_ind);
              depriotize_cell.bsic.PLMN_colour_code = new_message_ptr->rr_msgr_msg.message.geran_grr_msg_req.deprioritize_gcell_req.bsic>>3 & 0x07;
              depriotize_cell.bsic.BS_colour_code = new_message_ptr->rr_msgr_msg.message.geran_grr_msg_req.deprioritize_gcell_req.bsic & 0x07;
              depriotize_cell.remaining_time = new_message_ptr->rr_msgr_msg.message.geran_grr_msg_req.deprioritize_gcell_req.deprioritized_time;
              depriotize_cell.cgi.lai.plmn_id = new_message_ptr->rr_msgr_msg.message.geran_grr_msg_req.deprioritize_gcell_req.plmn_id;
              depriotize_cell.cgi.lai.location_area_code = new_message_ptr->rr_msgr_msg.message.geran_grr_msg_req.deprioritize_gcell_req.location_area_code;
              depriotize_cell.cgi.ci = new_message_ptr->rr_msgr_msg.message.geran_grr_msg_req.deprioritize_gcell_req.cell_id;
              depriotize_cell.cgi_valid = TRUE;

              rr_add_to_deprioritized_cell_list(&depriotize_cell, gas_id);
            }
            else
            {
              MSG_GERAN_HIGH_0_G("Cell Deprioritization is not enabled");
            }
          ...
          case GERAN_GRR_LTE_ABORT_CGI_REQ:
          {
            if (rr_is_sglte_mode_active(gas_id) && (rr_control_data_ptr->rr_state != RR_X2G_CGI) )
            {
              /* When SGLTE mode is active, Abort CGI Req from LTE is handled
                in a stateless manner, for all cases except an ongoing CGI Acq. */
              rr_sglte_send_lte_cgi_abort_rsp(gas_id);
              rr_event = EV_NO_EVENT;
            }
            break;
          } /* GERAN_GRR_LTE_PS_CAPABILITIES_REQ */
          ...
          case LTE_RRC_ACQ_DB_EARFCN_RSP:
          {
            rr_general_store_lte_acq_db(&new_message_ptr->rr_msgr_msg.message.lte_rrc_acq_db_earfcn_rsp, gas_id);
            rr_event = EV_NO_EVENT;
            break;
          }
          ...
          case GERAN_GRR_MTC_DRX_REQ:
          {
            geran_grr_mtc_drx_req_t *mtc_drx_req =
              &new_message_ptr->rr_msgr_msg.message.geran_grr_msg_req.mtc_drx_req;
            
            rr_handle_mtc_drx_req(mtc_drx_req, gas_id);
            rr_event = EV_NO_EVENT;
            
            break;
          } /* GERAN_GRR_MTC_DRX_REQ */

#ifdef FEATURE_PRIORITIZE_LTE_FREQ
          case QMI_NAS_PRIORITIZE_LTE_IND:
          {
            rr_general_check_and_prioritize_lte_frequencies(gas_id);
            rr_event = EV_NO_EVENT;
            break;
...
#else
                MSG_GERAN_HIGH_1_G("RR_IMSG_RR_EVENT_IND contains %d", rr_event);
...
#else
                MSG_GERAN_HIGH_2_G("RR_IMSG_SAVED_MSG_IND contains (%d,%d))",
                                   new_message_ptr->message_header.message_set,
                                   new_message_ptr->message_header.message_id);
#endif /* FEATURE_GERAN_REDUCED_SPRINTF */
                break;
    ...
    switch (rr_control_data_ptr->rr_state)
    {
          ...
          switch (message_set)
          {
            case MS_MM_RR:
            {
              switch ((mm_rr_message_id_enum_T) message_id)
              {
                ...
                case RR_STOP_GSM_MODE_REQ:
                {
                  ...
                  (void)discard_all_rr_temporary_data(gas_id);
                  break;
                }
                ...
                case RR_IRAT_ALLOWED_STATUS_IND:
                {
                  MSG_GERAN_MED_0_G("Ignoring RR_IRAT_ALLOWED_STATUS_IND in RR_INACTIVE");
                  break;
                } /* RR_IRAT_ALLOWED_STATUS_IND */
              ...
              switch (new_message_ptr->rr.header.rr_message_set)
              {
                case RR_IMSG:
                {
                  switch (new_message_ptr->rr.header.imh.message_id)
                  {
                    ...
                    {
                      /* X2G CCO failed during RACH. RR_STOP_MODE_REQ received when the other RAT tried to activate and GRR is deactivated.
                         Other RAT sends CCO_COMPLETED_IND/STOP_IND. No action needed.
                       */
                      break;
                    }
#ifdef FEATURE_GERAN_HST_OPT
                    case RR_IMSG_FREQ_LIST_UPDATE_REQUIRED:
                    {
                      /* HST Information can be ignored here, No Action Need */
                      break;
                    }
              ...
              switch ((tdsrr_rrc_cmd_e_type) message_id)
              {
                ...
                case TDSRR_INTERRAT_CC_ORDER_REQ:
                {
...
#endif /* FEATURE_SGLTE */
                  {
                    ...
                    if (rr_timer_start(T3174, req->T309_value * 1000, gas_id))
                    {
                      rr_ms_set_procedure_duration(req->T309_value * 1000, gas_id);
                    }
                ...
                /* In corner case scenarios, it is possible for GRR to receive an CCO_ABORT_REQ,
                     just after sending CCO_CNF(FAILURE) to TRRC.*/
            ...
            case MS_RRC_RR:
            {
              switch ((rrc_rr_cmd_e_type) message_id)
              {
                ...
                /* In corner case scenarios, it is possible for GRR to receive an CCO_ABORT_REQ,
                     just after sending CCO_CNF(FAILURE) to RRC.
                */
                case RR_INTERRAT_CC_ORDER_ABORT_REQ:
                {
                  rr_send_interrat_cc_order_abort_cnf(RRC_RR_SUCCESS, gas_id);
            ...
            case MS_MSGR_RR:
            {
              switch (new_message_ptr->rr_msgr_msg.message.hdr.id)
              {
                ...
                case GERAN_GRR_LTE_RESEL_REQ:
                {
                  ...
                  rr_store_plmn_id(&req->requested_PLMN, gas_id);
#if defined(FEATURE_DUAL_SIM) || defined(FEATURE_TRIPLE_SIM)
                  rr_ms_acquisition_started(gas_id);
#endif /* FEATURE_DUAL_SIM || FEATURE_TRIPLE_SIM */
                  arfcn.num = req->arfcn;
                ...
                case GERAN_GRR_LTE_CCO_REQ:
                {
                  ...
                  if ((req->no_of_si_messages > 0) && (req->t304 < RR_T304_ASSISTED_MIN_DURATION))
       
                  {
                    MSG_GERAN_HIGH_2_G(
                      "T304 %dms too low for tunnelled Sis cell increasing to %dms",
                      req->t304,
                      RR_T304_ASSISTED_MIN_DURATION
                    );
                    t304_time = RR_T304_ASSISTED_MIN_DURATION;
                  }
                  else if ((req->no_of_si_messages == 0) && (req->t304 < RR_T304_UNASSISTED_MIN_DURATION))
                  {
                    MSG_GERAN_HIGH_2_G(
                      "T304 %dms too low increasing to %dms",
                      req->t304,
                      RR_T304_UNASSISTED_MIN_DURATION
                    );
                    t304_time = RR_T304_UNASSISTED_MIN_DURATION;                    
                  }
                  else
                  {
                    t304_time = req->t304;
                  ...
                  if (rr_timer_start(T3174, t304_time, gas_id))
                  {                              
                    rr_ms_set_procedure_duration(t304_time, gas_id);
                  }
      ...
      case RR_W2G_INTERRAT_HANDOVER_PROGRESS:
      {
        ...
        if ((rr_event == EV_INPUT_MESSAGE) && (new_message_ptr != NULL))
        {
          ...
          else if (message_set == MS_RR_L1)
          {
            ...
            else if (message_id == (byte) MPH_VOICE_SAMPLE_RATE_CHANGE_IND)
            {
              rr_eng_mode_update_voice_sample_rate(
                new_message_ptr->mph.voice_sample_rate_change_ind.sample_rate,
                gas_id
              );
            }
            else
        ...
        else if (rr_event == RR_EV_W2G_GO_TO_INACTIVE)
        {
          ...
          {
             rr_sglte_srvcc_send_no_service_ind(gas_id);
          }
          rr_sglte_reinit_processing_l2g_ho_related_parameter(gas_id);
#endif /*FEATURE_SGLTE*/
        ...
        else if (rr_event == RR_EV_W2G_HO_ABORTED)
        {
          ...
          // Send RR_ABORT_REQ, so that it will be processed in DATA_TRANSFER State.
...
#endif // FEATURE_DUAL_SIM || FEATURE_TRIPLE_SIM
          new_message_ptr->rr_abort_req = rr_rr_abort_req;
          new_rr_event = EV_INPUT_MESSAGE;
...
#endif
#if defined(FEATURE_LTE) || defined(FEATURE_GSM_TDS) || defined (FEATURE_WCDMA)
              case MPH_SURROUND_IRAT_MEAS_IND:
#endif /* FEATURE_LTE || FEATURE_GSM_TDS  || FEATURE_WCDMA */
              case MPH_POWER_SCAN_CNF:
              case MPH_DECODE_BCCH_LIST_FAILURE_IND:
              case MPH_DECODE_BCCH_LIST_CNF:
          ...
          switch (message_set)
          {
            ...
            case MS_MM_RR:
            {
              switch ((mm_rr_message_id_enum_T) message_id)
              {
                ...
                case RR_CHANGE_MODE_IND:
                {
                  rr_PLMN_request_details_T *plmn_req_details = rr_get_PLMN_request_details_ptr(gas_id);

                  /* Mode change indication not expected to be received when PLMN selection or W2G redirection is ongoing. */
                  if (plmn_req_details->search_reason == RR_AUTONOMOUS_CELL_SELECTION)
                  {
                    rr_pending_mode_change_data_store(
                      &new_message_ptr->mm.rr_change_mode_ind,
                      gas_id
...
#ifdef FEATURE_LOCK_ARFCN
                case RR_WAIT_FOR_ARFCN_TO_LOCK_TIMER:
                {
                  MSG_GERAN_MED_0_G("RR_WAIT_FOR_ARFCN_TO_LOCK_TIMER expired , no locking required");
                  new_rr_event = rr_selection_control(EV_NO_LOCKING_REQUIRED,NULL, gas_id);
                  break;
                }
...
#ifdef FEATURE_LOCK_ARFCN
                    case RR_IMSG_SELECTION_START_LOCKING_IND:
                    {
                      rr_set_locking_channel(new_message_ptr->rr.rr.imsg.sel_start_locking_ind.locking_ARFCN, gas_id);
                      new_rr_event = rr_selection_control(EV_START_LOCKING_TO_CHANNEL,NULL, gas_id);

                      break;
                    }
                    case RR_IMSG_SELECTION_NO_LOCKING_REQUIRED:
                    {
                      new_rr_event = rr_selection_control(EV_NO_LOCKING_REQUIRED,NULL, gas_id);
                      break;
                    }
...
#ifdef FEATURE_LOCK_ARFCN
          /*Reinit the locking channle info*/
         rr_reset_locking_channel_info(gas_id);
#endif /*FEATURE_LOCK_ARFCN*/
...
#ifdef FEATURE_SGLTE
                 /* Manual PLMN Search in SGLTE is handled via legacy way, remember that Device Mode is changing */
                 /* GL1 will always send Mode change Confirm in SGLTE  */
                 if (rr_is_sglte_mode_active(gas_id))
                 {
                   rr_set_pending_mode_change(TRUE, gas_id);
                 }
...
#endif /* FEATURE_SGLTE */
                  {
                    new_rr_event = rr_trigger_gsm_as_deactivation(RR_DEACT_REASON_X2G_BPLMN_SEARCH_COMPLETE, gas_id);
                    rr_control_data_ptr->rr_state = RR_GOING_INACTIVE;
                  }
                  break;
      ...
      case RR_GRR:
      {
          ...
          switch (message_set)
          {
              ...
              switch (new_message_ptr->rr.header.rr_message_set)
              {
                case RR_IMSG:
                {
                  switch (message_id)
                  {
                    ...
                    case RR_IMSG_IRAT_ALLOWED_UPDATED_IND:
                    {
                      rr_grr_send_imsg(RR_GRR_IMSG_IRAT_ALLOWED_UPDATED_IND, gas_id);
                      break;
                    ...
                    case RR_IMSG_FREQ_LIST_UPDATE_REQUIRED:
                    {
                      rr_grr_send_imsg(RR_GRR_IMSG_FREQ_LIST_UPDATE_IND, gas_id);
                      break;
                    }
#endif /* FEATURE_GERAN_HST_OPT */
                    case RR_IMSG_PS_ACCESS_REQ:
                    ...
                    case RR_GRR_OMSG_X2G_CCO_STATUS_IND:
                    {
                      ...
                      switch (new_message_ptr->rr.grr.omsg.x2g_cco_status_ind.status)
                      {
                        case RR_CCO_STATUS_SUCCESS:
                        {
                          switch (rr_control_data_ptr->x2g_resel_type)
                          {
                            ...
                            case RR_X2G_RESEL_TYPE_W2G_CELL_CHANGE_ORDER:
                            {
                              ...
                              rr_control_data_ptr->waiting_for_rr_interrat_wl1_stop_ind = TRUE;
                              break;
                            } /* RR_X2G_RESEL_TYPE_W2G_CELL_CHANGE_ORDER */
#endif /* FEATURE_WCDMA */
                        ...
                        case RR_CCO_STATUS_ABORTED_HIGH_PRIORITY_REQUEST:
                        {
                          ...
                          /* Send a signal to the originating RRC indicating that the
                               CCO procedure was aborted due to a high priority request
                               from NAS. LTE would send completed_ind, and W/T would send stop_ind.
                               GRR should wait for them to finish the procedure. */
                          // TODO DSDA
            ...
            case MS_MM_RR:
            {
              switch (message_id)
              {
                ...
                case RR_STOP_GSM_MODE_REQ:
                {
                  ...
                  if (rr_save_msg(new_message_ptr, gas_id))
                  {
                    if(!rr_control_data_ptr->waiting_for_rr_interrat_wl1_stop_ind)
                    {
                      new_rr_event = grr_control(EV_INPUT_MESSAGE, new_message_ptr, gas_id);
                    }
                  }
            ...
            case MS_MSGR_RR:
            {
              switch (new_message_ptr->rr_msgr_msg.message.hdr.id)
              {
                ...
                case GERAN_GRR_LTE_CCO_REQ:
                {
                  if (rr_is_sglte_mode_active(gas_id))
                  {
                    if (rr_sglte_is_full_service_available(gas_id) && rr_cell_supports_gprs(gas_id))
                    {
                      ...
                      if (req->t304 < RR_T304_ASSISTED_MIN_DURATION)
                      {
                        MSG_GERAN_ERROR_2_G("T304 %dms too low, increasing to %dms",
                                            req->t304, RR_T304_ASSISTED_MIN_DURATION);
                        (void)rr_timer_start(T3174, RR_T304_ASSISTED_MIN_DURATION, gas_id);
                      }
                      else
                      {
                ...
                {
                  if (rr_is_sglte_mode_active(gas_id))
                  { // If SGLTE Mode is Active, allow CGI Request in Camped (RR_GRR) State.
                    geran_grr_lte_get_cgi_req_t * req = &new_message_ptr->rr_msgr_msg.message.geran_grr_msg_req.lte_get_cgi_req;

                    if ( (req->num_arfcns == 0) || (rr_sglte_is_serv_cell_in_lte_cgi_req(new_message_ptr, gas_id)== FALSE))
                    {
                      rr_sglte_send_empty_lte_cgi_rsp(gas_id);
                    }
                    else
                    {
                      rr_sglte_send_serv_cell_lte_cgi_rsp(new_message_ptr, gas_id);
                    }
                  }
                  else
                  {
                    MSG_GERAN_ERROR_1_G("Invalid LTE Message %d in RR_GRR when device Mode NOT SGLTE", new_message_ptr->rr_msgr_msg.message.hdr.id);
                  }
                  break;
                } /* GERAN_GRR_LTE_GET_CGI_REQ */
                ...
                case TDSRR_INTERRAT_CC_ORDER_REQ:
                {
                  if (rr_is_sglte_mode_active(gas_id))
                  {
                    if (rr_sglte_is_full_service_available(gas_id) && rr_cell_supports_gprs(gas_id))
                    {
                      ...
                      if ((req->T309_value *1000) < RR_T304_ASSISTED_MIN_DURATION)
                      {
                        MSG_GERAN_ERROR_2_G("T309 %dms too low, increasing to %dms",
                                            req->T309_value * 1000, RR_T304_ASSISTED_MIN_DURATION);
                        (void) rr_timer_start(T3174, RR_T304_ASSISTED_MIN_DURATION, gas_id);
                      }
                      else
                      {
        ...
        /**********************************************************/
        /* RR_EV_TRIGGER_CELL_SELECTION_ACQ_DB_ONLY               */
        /* A cell selection needs to be triggered                 */
        /**********************************************************/
        else if (rr_event == RR_EV_TRIGGER_CELL_SELECTION_ACQ_DB_ONLY)
        {
          rr_control_data_ptr->rr_state = RR_CELL_SELECTION;
          new_rr_event = rr_selection_control(EV_TRIGGER_CELL_SELECTION_ACQ_DB_ONLY, NULL, gas_id);
        }
        ...
        else if(rr_event == EV_ACCESS_CLASS_NOT_ALLOWED_EMERGECNY_CALL)
        {
          if(rr_nv_camp_for_emergency_call_enabled(gas_id))
          {
            rr_send_no_service_ind(gas_id);
      ...
      case RR_CONNECTION_PENDING:
      {
...
#if defined (FEATURE_LTE) || defined (FEATURE_GSM_TDS) || defined (FEATURE_WCDMA)
            else if (message_id == (byte) MPH_SURROUND_IRAT_MEAS_IND)
            {
              /*Possible that when specific freq measurement timer expired at L1 side it sends */
              /*measurement hence GRR need to handle */
...
#ifdef FEATURE_QSH_EVENT_METRIC
                rr_qsh_metric_log_rach_timer_expiry(gas_id);
#endif // FEATURE_QSH_EVENT_METRIC
                break;
              }
                 ...
                 (rr_event == RR_EV_START_RESELECTION_POWER_SCAN))
        {
             ...
             )
          {
            if (rr_is_lte_enabled(gas_id))
            {
              ...
              /* CSI has been sent to redirection SM so it is no longer required. */
            ...
            {
              MSG_GERAN_ERROR_0_G("G2L blind redir required but LTE is not supported");

              /* Abandon any Lock Exchange that may have been set for this Redirection */
            ...
            MSG_GERAN_HIGH_0_G("Resetting blind redirection flags)");
            rr_connection_release_reset_g2l_blind_redir_flag(gas_id);
        ...
        else if ((rr_event == EV_DSF_RESELECTION) || (rr_event == RR_EV_START_RPS_AFTER_DENIAL))
        {
        ...
        else if (rr_event == EV_RR_EST_CNF)
        {
          ...
          boolean g2l_blind_redir_after_csfb_required =
            rr_connection_release_get_g2l_blind_redir_after_csfb_flag(gas_id);
          boolean g2l_blind_redir_enable = 
            rr_connection_release_get_g2l_blind_redir_flag(gas_id);
#endif /* FEATURE_LTE */
          ...
          if (rr_ce_access_reason(gas_id) == MM_RR_MO_FULL_SPEECH ||
              ...
              rr_ce_access_reason(gas_id) == MM_RR_SUPPLEMENTARY_SERVICE ||
              rr_ce_access_reason(gas_id) == MM_RR_CALL_REEST ||
              rr_ce_access_reason(gas_id) == MM_RR_LOCATION_UPDATE )
          {
             if (rr_g2l_blind_redir_after_csfb_enabled(gas_id))
             {
                MSG_GERAN_MED_0_G("G2L blind redir after CSFB enabled");
             }
             else
             { 
                MSG_GERAN_MED_0_G("G2L blind redir after CSFB disabled");
             }
          }

          if ((TRUE == g2l_blind_redir_enable) && (rr_g2l_blind_redir_after_srvcc_enabled(gas_id)))
          {
            rr_connection_release_set_g2l_blind_redir_flag(gas_id);
            MSG_GERAN_HIGH_0_G("Call Re-Establishment was successful after srvcc.G2L blind redirection will be performed after call release");
          }
#endif /* FEATURE_LTE */
        ...
        else if (rr_event == EV_GO_TO_IDLE)
        {
          ...
          /*Here check if GRR is returning to idle after RACH failure OTA */
          if (rr_ce_max_failed_rach_sent(gas_id))
          {
            MSG_GERAN_HIGH_0_G("Idle mode after TRM denial for RACH");
            new_rr_event = RR_EV_CAMP_ON_CELL_FOR_RACH_FAIL_OTA;
          }
          else
          {
        ...
        else if ( rr_event == EV_CONNECTION_ABORTION)
        {
          ...
          /* Create an abort req message and move to release state */
          rr_rr_abort_req.message_header.message_set = MS_MM_RR;
          rr_rr_abort_req.message_header.message_id  = (byte) RR_ABORT_REQ;
#if defined(FEATURE_DUAL_SIM) || defined(FEATURE_TRIPLE_SIM)
          rr_rr_abort_req.as_id = RR_GAS_ID_TO_AS_ID;
      ...
      case RR_CELL_REESTABLISH:
      {
...
#ifdef FEATURE_QSH_EVENT_NOTIFY_TO_QSH 
                if (new_message_ptr->mid_timer_expiry.timer_id == RR_GSM_SYS_INFO_TIMER)
                {
                  rr_qsh_event_notify(RR_QSH_EVENT_GSM_SYS_INFO_TIMER_EXPIRY, gas_id);
                }
#endif //FEATURE_QSH_EVENT_NOTIFY_HANDLER              
        ...
        */
                 ...
                 (rr_event == EV_CELL_CHOSEN_EARLY_CAMPED))
        {
          ...
          {
            /* Send SI mask to acquire remaining SIs. 
             * This is applicable only if iterative SI acquisition is enabled. */
...
#ifdef FEATURE_LOCK_ARFCN
                    else if (rr_is_ms_locked_to_channel(gas_id))
                    {
                       MSG_GERAN_MED_0_G("NO inter possible in lock mode");
                       new_rr_event = EV_NO_EVENT;
                    }
#endif /*FEATURE_LOCK_ARFCN*/
            ...
            case MS_MSGR_RR:
            {
              switch (new_message_ptr->rr_msgr_msg.message.hdr.id)
              {
               ...
               case GERAN_GRR_LTE_GET_CGI_REQ:
                {
                  if (rr_is_sglte_mode_active(gas_id))
                  { // If SGLTE Mode is Active, allow CGI Request in DATA_TRANSFER State.
                    geran_grr_lte_get_cgi_req_t * req = &new_message_ptr->rr_msgr_msg.message.geran_grr_msg_req.lte_get_cgi_req;
                  ...
                  {
                    MSG_GERAN_ERROR_1_G("Invalid LTE Message %d in RR_DATA_TRANSFER when device Mode NOT SGLTE", new_message_ptr->rr_msgr_msg.message.hdr.id);
                  }
                  break;
                } /* GERAN_GRR_LTE_GET_CGI_REQ */
      ...
      case RR_CONNECTION_RELEASE:
      {
                  ...
                  case RR_CE_OMSG_CS_RELEASE_IND:
                  {
                    ...
                    switch (imsg_ptr->status)
                    {
                      case RR_CE_REL_STATUS_CAMP_ON_CELL:
                      {
                        ...
                        {   
                          if (rr_csi_retrieve_all_SIs(scell_info_ptr->gsm.BCCH_ARFCN,
                                                    &(scell_info_ptr->gsm.location_area_identification),
                                                    &(scell_info_ptr->gsm.cell_identity),
                                                      ...
                                                      gas_id) )
                          {
                            MSG_GERAN_HIGH_0_G("RR retrieved optional SI's not essential for camping");
                          }
                        }
...
#if defined(FEATURE_DUAL_SIM) || defined(FEATURE_TRIPLE_SIM)
                        // Inform Multi SIM Controller about start of SI reading on this Cell
                        rr_ms_sys_info_status_update(RR_SYS_INFO_ACQ_STARTED, gas_id);
#endif /* FEATURE_DUAL_SIM || FEATURE_TRIPLE_SIM */
                        break;
                      }
        ...
        else if (rr_event == EV_CHOOSE_CELL_BY_CELL_SELECTION_INDICATOR)
        {
          ...
          if (rr_cell_select_indicator_ptr->valid)
          {
            ...
            {
              //Dont do G to W redirection after channel release
              if (rr_is_wcdma_enabled(gas_id) && rr_allow_redir_during_fallback_mode(FDD_TARGET,gas_id))
              {
                uint32 i;
            ...
            {
              if (rr_allow_redir_during_fallback_mode(LTE_TARGET,gas_id))
              {
                uint16 i;
                     ...
                     ++i)
                {
                  ...
                  if (lte_rrc_earfcn_is_supported(as_id, frequency.data.lte, rr_band_pref_lte(gas_id)))
                  {
                    bands_ok = TRUE;
                  }
                }
              ...
              else
              {
                ...
                new_rr_event = EV_CHOOSE_CELL;
                MSG_GERAN_MED_0_G("LTE redirection indicated but coll_fallback enabled, perform cell selection");
              }
            }
#endif /* FEATURE_LTE */
            else
        ...
        else if (rr_event == EV_G2L_BLIND_REDIR_REQUIRED)
        {
          ...
          if (rr_is_lte_enabled(gas_id) && rr_allow_redir_during_fallback_mode(LTE_TARGET,gas_id))
          {
            rr_chan_rel_cell_selection_indicator_t *csi = rr_get_cell_select_indicator_ptr(gas_id);
        ...
        if ((rr_event == EV_INPUT_MESSAGE) && (new_message_ptr != NULL))
        {
            ...
            switch (new_message_ptr->rr.header.rr_message_set)
            {
              ...
              case RR_IMSG:
              {
                switch (new_message_ptr->message_header.message_id)
                {
                  ...
                  {
                    rr_imsg_page_mode_ind_t *imsg = &new_message_ptr->rr.rr.imsg.page_mode_ind;
                    rr_ce_update_l1_page_mode(imsg->new_page_mode, gas_id);
                    break;
                  }
          ...
          else if (message_set == MS_MM_RR)
          {
            ...
            else if (message_id == (byte) RR_PLMN_LIST_REQ)
            {
              ...
              */
              ...
              {
                (void)rr_save_msg(new_message_ptr, gas_id);
              MSG_GERAN_HIGH_0_G("Saved RR_PLMN_LIST_REQ in EARLY_CAMP_WAIT_FOR_SI");
              }
            }
            else if (message_id == (byte) RR_PLMN_SELECT_REQ)
            {
               ...
               */
              if (TRUE == rr_change_in_supported_IRAT_info(&new_message_ptr->mm.rr_plmn_select_req.rat_pri_list_info, gas_id))
              {
                MSG_GERAN_HIGH_0_G("RAT Prior list got changed, send wcdma list req with empty list");
          ...
          else if ((message_set == MS_TIMER) && (message_id == (byte) MID_TIMER_EXPIRY))
          {
            switch (new_message_ptr->mid_timer_expiry.timer_id)
            {
              case RR_GSM_SYS_INFO_TIMER:
              {                
                word rr_si_nt_rcvd = rr_sys_info_timeout_due_to_SI2bis_SI2ter_SI13_not_read(gas_id);
                if (MASK_SI_NONE != rr_si_nt_rcvd)
                {
                  rr_act_on_for_SI_nt_rcvd(rr_si_nt_rcvd, gas_id);
              ...
              {
                if (rr_sys_info_timeout_due_to_si13_not_read(gas_id))
                {
                  rr_switch_to_CS_only_mode(gas_id);
                  new_rr_event = EV_GO_TO_IDLE;
                }
                else
                {
...
#ifdef FEATURE_QSH_EVENT_NOTIFY_TO_QSH 
                rr_qsh_event_notify(RR_QSH_EVENT_GPRS_SYS_INFO_TIMER_EXPIRY, gas_id);
#endif //FEATURE_QSH_EVENT_NOTIFY_TO_QSH 
                    ...
                    switch (new_message_ptr->message_header.message_id)
                    {
                      case RR_X2G_RESEL_OMSG_RESEL_CNF:
                      {
                        ...
                        else
                        {
                          ...
                          {
                            rr_log_reselect_failure(RR_LOG_X2G_RESEL_FAILURE_UNDEFINED, gas_id);
                          }
                ...
                switch (new_message_ptr->rr.header.rr_message_set)
                {
                  ...
                  case RR_X2G_RESEL_OMSG:
                  {
                    switch (new_message_ptr->message_header.message_id)
                    {
                      ...
                      {
                        MSG_GERAN_MED_1_G("x2g_resel_type=%d",rr_control_data_ptr->x2g_resel_type);
                        switch (rr_control_data_ptr->x2g_resel_abort_cause)
                        {
                          case RR_X2G_RESEL_ABORT_CAUSE_IRAT:
                          {
                            /* X2G Reselection / CCO procedure was aborted after receiving an abort request
                                 from the initiating RAT.  L1 is shut down by rr_x2g_resel_control() so all that
                                 happens now is rr_control() state is reset to INACTIVE, any X2G state variables
                                 are reset and RR sends an abort response to the initiating RAT. */
                            switch (rr_control_data_ptr->x2g_resel_type)
                            {
#ifdef FEATURE_WCDMA
                              case RR_X2G_RESEL_TYPE_W2G_RESELECTION:
                          ...
                          case RR_X2G_RESEL_ABORT_CAUSE_NAS:
                          {
                            if (rr_x2g_cco_in_progress(gas_id))
                            {
                              switch (rr_control_data_ptr->x2g_resel_type)
                              {
                                ...
                                case RR_X2G_RESEL_TYPE_W2G_CELL_CHANGE_ORDER:
                                {
                                  rr_x2g_send_x2g_cco_rsp_aborted_by_nas(RR_RAT_WCDMA,gas_id);
                                  rr_control_data_ptr->waiting_for_rr_interrat_wl1_stop_ind = TRUE;
                                  break;
                                } /* RR_X2G_RESEL_TYPE_W2G_CELL_CHANGE_ORDER */
#endif /* FEATURE_WCDMA */
                                ...
                                case RR_X2G_RESEL_TYPE_T2G_CELL_CHANGE_ORDER:
                                {
                                  rr_x2g_send_x2g_cco_rsp_aborted_by_nas(RR_RAT_UTRAN_TDD,gas_id);
                                ...
                                case RR_X2G_RESEL_TYPE_L2G_CELL_CHANGE_ORDER:
                                {
                                  rr_x2g_send_x2g_cco_rsp_aborted_by_nas(RR_RAT_LTE,gas_id);
                                  rr_control_data_ptr->waiting_for_lte_rrc_cco_nacc_completed_ind = TRUE;
                                  break;
                                } /* RR_X2G_RESEL_TYPE_L2G_CELL_CHANGE_ORDER */
#endif /* FEATURE_LTE */
                          ...
                          case RR_X2G_RESEL_ABORT_CAUSE_CCO_TIMER_EXPIRY:
                          {
                            ...
                            switch (rr_control_data_ptr->x2g_resel_type)
                            {
                              ...
                              case RR_X2G_RESEL_TYPE_L2G_CELL_CHANGE_ORDER:
                              {
                                if (rr_get_x2g_cco_act_rsp_rcvd(gas_id))
                                {
                                  rr_x2g_send_x2g_cco_rsp_failure_after_act_ind(RR_RAT_LTE,
                                    RR_CCO_STATUS_UNDEFINED,
                                    gas_id);
                                }
                                else
                                {
                                  rr_x2g_send_x2g_cco_rsp_failure_before_act_ind(RR_RAT_LTE,
                                    RR_X2G_RESEL_STATUS_UNDEFINED,
                                    gas_id);
                                }
...
#endif /* FEATURE_GSM_TDS */
                              default:
                                MSG_GERAN_ERROR_1_G("Unexpected X2G abort cause: %d",
                                  rr_control_data_ptr->x2g_resel_abort_cause);
                      ...
                      case RR_G2X_REDIR_OMSG_REDIR_CNF:
                      {
                        ...
                        switch (new_message_ptr->rr.g2x_redir.omsg.redir_cnf.status)
                        {
                          case RR_G2X_REDIR_STATUS_SUCCESS:
                          {
                            ...
                            rr_control_data_ptr->rr_state = RR_GOING_INACTIVE;
#ifdef FEATURE_QSH_EVENT_NOTIFY_TO_QSH 
                            rr_qsh_event_notify(RR_QSH_EVENT_G2X_REDIR, gas_id);
#endif //FEATURE_QSH_EVENT_NOTIFY_TO_QSH
                            break;
                ...
                switch (new_message_ptr->message_header.message_id)
                {
                  ...
                  case RR_EST_REQ:
                  {
                    switch (new_message_ptr->rr_est_req.establishment_cause)
                    {
                      ...
                      {
                        if (TRUE == rr_g2x_is_in_redirecting_state(gas_id))
                        {
                          rr_send_abort_ind(RR_ABORT_GTOW_REDIRECTION_IN_PROGRESS, MM_RR_OTHER_FAILURE, gas_id);
                        }
                        else
            ...
            switch (new_message_ptr->message_header.message_set)
            {
              case MS_RR_RR:
              {
                switch (new_message_ptr->rr.header.rr_message_set)
                {
                  ...
                  default:
                  {
                    rr_femto_control(new_message_ptr, gas_id);
                  }
                }
                break;
              } /* MS_RR_RR */
                  ...
                  */
                  case RR_STOP_GSM_MODE_REQ:
                  {
                    switch (rr_control_data_ptr->x2g_resel_type)
                    {
                      ...
                      case RR_X2G_RESEL_TYPE_L2G_CELL_CHANGE_ORDER:
                      {
                        /* L2G CCO failure procedure completion is indicated by RR_LTOG_CCO_FAILURE stop reason.
                             Any other stop reason is treated as an failure abort by RR. */
                        switch (new_message_ptr->rr_stop_gsm_mode_req.rr_mm_gsm_stop_reason)
                        {
                          ...
                          default:
                          {
                            ...
                            rr_x2g_send_x2g_cco_failure_procedure_ind(RR_RAT_LTE,gas_id);
                  ...
                  case RR_ABORT_REQ:
                  {
                    switch (rr_control_data_ptr->x2g_resel_type)
                    {
                      ...
                      case RR_X2G_RESEL_TYPE_W2G_CELL_CHANGE_ORDER:
                      {
                        ...
                        rr_send_abort_cnf(MM_AS_SUCCESS, gas_id);
                        rr_send_interrat_proc_abort_ind(RRC_RR_SUCCESS,gas_id);
                        rr_control_data_ptr->rr_state = RR_X2G_CCO_FAILED_ABORTED;
                        break;
                      } /* RR_X2G_RESEL_TYPE_W2G_CELL_CHANGE_ORDER */
#endif /* FEATURE_WCDMA */
                      ...
                      case RR_X2G_RESEL_TYPE_T2G_CELL_CHANGE_ORDER:
                      {
                        ...
                        rr_send_abort_cnf(MM_AS_SUCCESS, gas_id);
                        rr_send_tds_interrat_proc_abort_ind(TDSRRC_RR_SUCCESS,gas_id);
                        rr_control_data_ptr->rr_state = RR_X2G_CCO_FAILED_ABORTED;
                        break;
                      } /* RR_X2G_RESEL_TYPE_T2G_CELL_CHANGE_ORDER */
#endif /* FEATURE_GSM_TDS */
                  ...
                  default:
                  {
                    switch(rr_control_data_ptr->x2g_resel_type)
                    {
                      ...
                      case RR_X2G_RESEL_TYPE_L2G_CELL_CHANGE_ORDER:
                        rr_save_msg(new_message_ptr, gas_id);
                        rr_x2g_send_x2g_cco_failure_procedure_ind(
                          rr_map_x2g_resel_type_to_rat(rr_control_data_ptr->x2g_resel_type),
                          gas_id);
                        rr_control_data_ptr->rr_state = RR_X2G_CCO_FAILED_ABORTED;
                        break;
                      ...
                      default:
                      {
                        /* If anything else is received from NAS at this point, it will not be handled. */
                   ...
                   case RR_SELECT_BCCH_OMSG:
                   {
                     switch (new_message_ptr->rr.select_bcch.omsg.header.imh.message_id)
                     {
                       case RR_SELECT_BCCH_OMSG_SELECT_CNF:
                       {
                         ...
                         MSG_GERAN_MED_1_G("cell_identity_valid_flag = %d",pcell_info_ptr->gsm.cell_identity_valid_flag);
                         ...
                         if (pcell_info_ptr->gsm.cell_identity_valid_flag)
                         {
                           if (rr_csi_retrieve_all_SIs(pcell_info_ptr->gsm.BCCH_ARFCN,
                                                      ...
                                                      &(pcell_info_ptr->gsm.cell_identity),
                                                       ...
                                                       gas_id))
                           {
                             MSG_GERAN_HIGH_0_G("RR retrieve SIs from cache after RR_SELECT_BCCH_OMSG_SELECT_CNF");
                           }
                         }
...
#ifdef FEATURE_GSM_CXM_L3_STATE
        /* Report INACTIVE state to CXM when RR is not reactivated again */
        if(EV_NO_EVENT == new_rr_event)
        {
          rr_cxm_state_send_l3_state_ind(CXM_TECH_L3_INACTIVE, gas_id);
        }
#endif /* FEATURE_GSM_CXM_L3_STATE */
        if(EV_NO_EVENT == new_rr_event)
        {
...
)
{
  ...
  switch (rr_control_data_ptr->rr_state)
  {
    ...
    case RR_CONNECTION_RELEASE:
    {
      /* To avoid any race conditions where RR starts to release a CS
      connection as NAS requests blind redirection, RR should process the
      blind redirection request in RR_CONNECTION_RELEASE as if it were
      received in RR_DATA_TRANSFER state. */
      return TRUE;
    } /* RR_CONNECTION_RELEASE */
...
#if defined(FEATURE_DUAL_SIM) || defined(FEATURE_TRIPLE_SIM)
boolean rr_can_process_rr_procedure_ind(rr_cmd_bdy_type *msg_ptr, const gas_id_t gas_id)
{
  boolean process_message = TRUE;

  if (rr_is_procedure_for_ps(msg_ptr->mm.rr_procedure_ind.proc_type, gas_id))
  {
    process_message = grr_can_process_rr_procedure_ind(gas_id);
  }
  else
  {
    /* Wait for CS Connection to be Established before applying new CS priority */
    if (rr_in_connection_pending(gas_id))
    {
...
void rr_set_redir_after_csfb_call(const gas_id_t gas_id)
{
  // Obtain a pointer to the module data
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr(gas_id);
  RR_NULL_CHECK_FATAL(rr_control_data_ptr);
  rr_control_data_ptr->redir_after_csfb = TRUE;
  MSG_GERAN_HIGH_1_G("redir_after_csfb = %d",rr_control_data_ptr->redir_after_csfb);  
}

void rr_reset_redir_after_csfb_call(const gas_id_t gas_id)
{
  // Obtain a pointer to the module data
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr(gas_id);
  RR_NULL_CHECK_FATAL(rr_control_data_ptr);
...
boolean rr_is_redir_after_csfb_call(const gas_id_t gas_id)
{
  // Obtain a pointer to the module data
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr(gas_id);
  RR_NULL_CHECK_FATAL(rr_control_data_ptr);
  MSG_GERAN_HIGH_1_G("redir_after_csfb sent to LTE is  = %d",rr_control_data_ptr->redir_after_csfb); 
  return rr_control_data_ptr->redir_after_csfb;
}
...
MSG_GERAN_HIGH_5_G(
  ...
  if (rr_x2g_redir_with_si_in_progress(gas_id))
  {
    ...
    {
     if (rr_get_nv_fast_si_acq_during_csfb_enabled(as_id) && (FALSE == mcfg_gcf_nv_get_status()))
     {
...
#ifdef FEATURE_GSM_TDS
  if ((rr_t2g_resel_in_progress(gas_id) || rr_t2g_cco_in_progress(gas_id) || rr_t2g_redirection_in_progress(gas_id) ) && !nv_dtm_enabled && (FALSE == mcfg_gcf_nv_get_status()))
  {
  ...
  if ( (rr_l2g_resel_in_progress(gas_id) || rr_l2g_cco_in_progress(gas_id)|| rr_l2g_redirection_in_progress(gas_id)) &&
       ((!rr_get_nv_dtm_enabled(as_id)) && (FALSE == mcfg_gcf_nv_get_status()) && (rr_is_fast_si_acq_during_x2g_allowed(gas_id))) )
  {
...
{
  if (rr_x2g_redirection_in_progress_on_redirected_cell(gas_id))
  {
    MSG_GERAN_HIGH_0_G("X2G redirection on redirected cell list, ngeative C1 is disabled");
    return FALSE;
  }
...
void rr_sglte_x2g_cco_nas_split_cb_fn(gas_id_t gas_id)
{
  ...
  switch (rr_control_data_ptr->x2g_resel_type)
  {
    ...
    case RR_X2G_RESEL_TYPE_T2G_CELL_CHANGE_ORDER:
    {
      rr_x2g_send_x2g_cco_rsp_success(rr_map_x2g_resel_type_to_rat(rr_control_data_ptr->x2g_resel_type), gas_id);
      break;
    }
#endif /*FEATURE_LTE || FEATURE_GSM_TDS*/
...
rr_event_T rr_handle_fake_gcell_action_req(rr_cmd_bdy_type* new_message_ptr, const gas_id_t gas_id)
{
  ...
  switch (rr_control_data_ptr->rr_state)
  {
    ...
    case RR_GRR:
    {
      rr_event = grr_control(EV_INPUT_MESSAGE, new_message_ptr, gas_id);
      break;
    }
    default:  
      rr_event = EV_NO_EVENT;
...
/**
  @brief This function briefly resets or increment the count depending on the SI13 status on TC=0 or 4
  @param
  @return None
*/

boolean rr_check_for_si13_reception(uint8 message_type, uint8 tc, gprs_scell_info_t *cell_info_ptr , const gas_id_t gas_id )
{

  if (FALSE == rr_nv_cs_camp_when_cell_supports_ps_but_no_si13_rcvd(gas_id))
  {
    MSG_GERAN_HIGH_0_G("EFS disabled camping on cell which supports PS but not broadcasting SI13");  
    return TRUE;
  }
  
  if (cell_info_ptr == NULL)
  {
    MSG_GERAN_ERROR_0_G("Cell info ptr is NULL assert");  
    return TRUE;
  }
  /*If SI13 has been received  or the message is SI13 itself then reset the count*/
  if ((message_type == SYSTEM_INFORMATION_TYPE_13) ||
      (ARE_SET( MASK_SI_13, cell_info_ptr->gsm.update_message_received))
     )
  {
    rr_reset_tc_read_count_for_optional_SI2bis_SI2ter_SI13(SYSTEM_INFORMATION_TYPE_13, gas_id);
    return TRUE;
  ...
  /*Step 1.Detrmine first if SI3 has been read or not and then check for GPRS  is supported or not*/	
  if ((ARE_SET(MASK_SI_3, cell_info_ptr->gsm.update_message_received)) && (cell_info_ptr->gsm.gprs_indicator)) 
  {
    /*Step 2. Check is SI13 is broadcasted on extended BCCH*/
    if (MASK_SI_13 == cell_info_ptr->bcch_sys_info.si_messages_on_bcch_ext)
    {
      /*Step 3 Now check if messagye type is not SI13 if yes then incremnet the count and is on TC = 0*/
      if ((tc == 0) && (message_type != SYSTEM_INFORMATION_TYPE_13))
      {
        MSG_GERAN_HIGH_0_G("SI13 not received for BCCH ext on TC=0 "); 
        rr_increment_tc_read_count_for_optional_SI2bis_SI2ter_SI13(SYSTEM_INFORMATION_TYPE_13, gas_id);
      }
    }
    /*SI13 is on BCCH norm*/
    else
    {
      /*Step 3 Now check if messagye tyep is not SI13 if yes then incremnet the count and is on TC = 4*/
      if ((tc == 4) && (message_type != SYSTEM_INFORMATION_TYPE_13))
      {
        MSG_GERAN_HIGH_0_G("SI13 not received for BCCH norm on TC=4 "); 
        rr_increment_tc_read_count_for_optional_SI2bis_SI2ter_SI13(SYSTEM_INFORMATION_TYPE_13, gas_id);
      }
    }
  }
...
/**
  @brief This function briefly resets or increment the count depending 
            on the SI2bis/SI2ter status on TC=4 or TC= 5
  @param
  @return None
*/

boolean rr_check_for_optional_sI2bis_SI2ter_reception(uint8 message_type, uint8 tc, gprs_scell_info_t *cell_info_ptr , const gas_id_t gas_id )
{ 
  if (FALSE == rr_nv_camp_when_SI2bis_SI2ter_is_indicated_but_not_sent(gas_id))
  {
    MSG_GERAN_HIGH_0_G("EFS disabled camping on cell without SI2ter or SI2bis is not allowed if indicated");  
    return TRUE;
  }
  ...
  /*If message itself is SI2bis or SI2ter reset the count*/
  if ((message_type == SYSTEM_INFORMATION_TYPE_2_BIS) || (message_type == SYSTEM_INFORMATION_TYPE_2_TER)) 
  {
    rr_reset_tc_read_count_for_optional_SI2bis_SI2ter_SI13(message_type, gas_id);
  }

  /*First check if both SI2 and SI3 has been received, this will ensure that optional SIs are needed or not*/  
  if (FALSE == ARE_SET(MASK_SI_2|MASK_SI_3 , cell_info_ptr->gsm.update_message_received))
  {
    return TRUE;
  }
  /*If both SI2bis or SI2ter has been received then return*/
  if (ARE_SET(MASK_SI_2_BIS| MASK_SI_2_TER , cell_info_ptr->gsm.update_message_received))
  {
    return TRUE;
  }

  MSG_GERAN_HIGH_2_G("SI2 bis is rqrd = %d, SI 2ter is rqrd = %d",cell_info_ptr->gsm.decode_SI2bis, cell_info_ptr->gsm.decode_SI2ter );
  
  /*When both  SI2bis or SI2ter  are required */
  if ((cell_info_ptr->gsm.decode_SI2bis == RR_SI_REQUIRED) && (cell_info_ptr->gsm.decode_SI2ter == RR_SI_REQUIRED))
  {  
    /*Step 1. If SI2bis has not been received and there is a message on TC = 5, then it has to be SI2bis*/
    if ((tc == 5) && (message_type != SYSTEM_INFORMATION_TYPE_2_BIS) && (FALSE == ARE_SET(MASK_SI_2_BIS, cell_info_ptr->gsm.update_message_received)))
    {
      MSG_GERAN_HIGH_0_G("SI2bis not received on TC=5"); 
      rr_increment_tc_read_count_for_optional_SI2bis_SI2ter_SI13(SYSTEM_INFORMATION_TYPE_2_BIS, gas_id);
    }
    /*Step 2. If SI2ter has not been received and there is a message on TC = 4, then it has to be SI2ter*/
    else if ((tc == 4) && (message_type != SYSTEM_INFORMATION_TYPE_2_TER) && (FALSE == ARE_SET(MASK_SI_2_TER, cell_info_ptr->gsm.update_message_received))) 
    {
      MSG_GERAN_HIGH_0_G("SI2ter not received on TC=4");
      rr_increment_tc_read_count_for_optional_SI2bis_SI2ter_SI13(SYSTEM_INFORMATION_TYPE_2_TER, gas_id);
    }

  }
  /*When either of them is required then SI2bis or SI2ter should come on TC =5*/
  else 
  {
    if ((tc == 5) && (message_type != SYSTEM_INFORMATION_TYPE_2_BIS) && (message_type != SYSTEM_INFORMATION_TYPE_2_TER)) 
    {
      if (cell_info_ptr->gsm.two_ti == RR_SI2TER_EXPECTED)
      {
        MSG_GERAN_HIGH_0_G("SI2ter not received on TC=5");
        rr_increment_tc_read_count_for_optional_SI2bis_SI2ter_SI13(SYSTEM_INFORMATION_TYPE_2_TER, gas_id);
      }
      else 
      {
...
/**
  @brief This function briefly checks if timeout has happned and GRR was still waiting for SI13
  @param
  @return None
*/

boolean rr_sys_info_timeout_due_to_si13_not_read(const gas_id_t gas_id )
{
  ...
  MSG_GERAN_HIGH_2_G("SI13  received = %d, Other SIrcvd= %d",
                     ARE_SET(MASK_SI_13, cell_info->gsm.update_message_received),
                     ARE_ALL_BITS_SET(cell_info->gsm.update_message_received, MASK_FULL_SI_EXCEPT_SI13_SI2_QUATER));
  /*Step 1. Check if SI13 is not yet received and all other SI except SI13 and SI2quater are received*/
  if (!(ARE_SET(MASK_SI_13, cell_info->gsm.update_message_received)) && 
      (ARE_ALL_BITS_SET(cell_info->gsm.update_message_received, MASK_FULL_SI_EXCEPT_SI13_SI2_QUATER)) 
     )
  {
     /*Step 2*/
     if (rr_get_tc_0_4_read_count_fr_si13(gas_id) >= MAX_SI_TC_READ_COUNT)
     {
       return TRUE;
     }
  }
...
/**
  @brief This function briefly checks if timeout has happned and GRR was still 
             waiting for SI2bis or SI2ter or SI13
  @param
  @return None
*/

word rr_sys_info_timeout_due_to_SI2bis_SI2ter_SI13_not_read(const gas_id_t gas_id)
{
  gprs_scell_info_t *cell_info;
  word  rr_si_not_rcvd = 0;
  if (rr_is_camped(gas_id))
  {
    cell_info = rr_gprs_get_scell_info(gas_id); 
  }
  ...
  /*Check if SI12bis is not yet received */
  if (!(ARE_SET(MASK_SI_2_BIS, cell_info->gsm.update_message_received))) 

  {
    /*Step 2*/
    if (rr_get_tc_5_read_count_fr_SI2bis(gas_id) >= MAX_SI_TC_READ_COUNT)
    {
      rr_si_not_rcvd |= MASK_SI_2_BIS ;
    }
  }
  
  /*Check if SI12ter is not yet received */
  if (!(ARE_SET(MASK_SI_2_TER, cell_info->gsm.update_message_received))) 

  {
    /*Step 2*/
    if (rr_get_tc_4_5_read_count_fr_SI2ter(gas_id) >= MAX_SI_TC_READ_COUNT)
    {
      rr_si_not_rcvd |= MASK_SI_2_TER ;
    }
  }  

  /*Check if SI13 is not yet received */
  if (!(ARE_SET(MASK_SI_13, cell_info->gsm.update_message_received)))
  {
     /*Step 2*/
     if (rr_get_tc_0_4_read_count_fr_si13(gas_id) >= MAX_SI_TC_READ_COUNT)
     {
       rr_si_not_rcvd |= MASK_SI_13;
     }
  }

  MSG_GERAN_HIGH_1_G("SI not recvd after time out = 0x%x ", rr_si_not_rcvd);
  return rr_si_not_rcvd;
}

/**
  @brief This function according to SI which are not received calls the releavnt function to set the message is not broacasted
  @param
  @return None
*/

void rr_act_on_for_SI_nt_rcvd(word rr_si_nt_rcvd, const gas_id_t gas_id)
{
  if (ARE_SET(MASK_SI_2_BIS, rr_si_nt_rcvd))
  {
    rr_update_SI2bis_not_rcvd(gas_id);
  }

  
  if (ARE_SET(MASK_SI_2_TER, rr_si_nt_rcvd))
  {
    rr_update_SI2ter_not_rcvd(gas_id);
  }
  
  if (ARE_SET(MASK_SI_13, rr_si_nt_rcvd))
  {
    rr_switch_to_CS_only_mode(gas_id);
  }
...
/**
  @brief This function briefly switches to GSM only mode
  @param
  @return None
*/
void rr_switch_to_CS_only_mode(const gas_id_t gas_id)
{
  gprs_scell_info_t *cell_info;
  if (rr_is_camped(gas_id))
  {
  ...
  MSG_GERAN_HIGH_0_G("Switch to GSM only mode");
  ...
  cell_info->bcch_sys_info.si13_expected_flag = FALSE;
  cell_info->bcch_sys_info.si13_data_valid_flag = FALSE;
  cell_info->gsm.gprs_indicator = FALSE;
  cell_info->bcch_sys_info.ra_colour = 0;
  cell_info->bcch_sys_info.si13_position = BCCH_NORMAL;
}

/**
  @brief This function briefly updated the bit mask for SI2bis not received
  @param
  @return None
*/
void rr_update_SI2bis_not_rcvd(const gas_id_t gas_id)
{
  gprs_scell_info_t *cell_info;
  if (rr_is_camped(gas_id))
  {
  ...
  MSG_GERAN_HIGH_0_G("SI2 bis is not rcvnd ignore it");
  bsi_update_messages_being_broadcast(
    cell_info,
    MASK_SI_NONE,
    MASK_SI_NONE,
  ...
  cell_info->gsm.decode_SI2bis = RR_SI_NOT_REQUIRED;
}

/**
  @brief This function briefly updated the bit mask for SI2ter not received
  @param
  @return None
*/
void rr_update_SI2ter_not_rcvd(const gas_id_t gas_id)
{
  gprs_scell_info_t *cell_info;
  if (rr_is_camped(gas_id))
  {
  ...
  MSG_GERAN_HIGH_0_G("SI2 ter is not recvd ignore it");
  ...
  cell_info->gsm.two_ti = RR_SI2TER_NOT_EXPECTED;
  cell_info->gsm.decode_SI2ter = RR_SI_NOT_REQUIRED;
}


/**
  @brief This function increments the count by one if on  relevant TC SI2bis or SI2ter or SI13has not been read.
  @param
  @return None
*/

void rr_increment_tc_read_count_for_optional_SI2bis_SI2ter_SI13(uint8 message_type, const gas_id_t gas_id)
{
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr(gas_id);
  
  if (message_type == SYSTEM_INFORMATION_TYPE_2_BIS)
  {
    rr_control_data_ptr->tc_5_read_count_fr_si2bis++;
    MSG_GERAN_HIGH_1_G("SI2bis not received count incremented , curr value is = %d",rr_control_data_ptr->tc_5_read_count_fr_si2bis);
  }
  else if (message_type == SYSTEM_INFORMATION_TYPE_2_TER)
  {
    rr_control_data_ptr->tc_4_5_read_count_fr_si2ter++;
    MSG_GERAN_HIGH_1_G("SI2ter not received count incremented , curr value is = %d",rr_control_data_ptr->tc_4_5_read_count_fr_si2ter);
  }
  else if (message_type == SYSTEM_INFORMATION_TYPE_13)
  {
    rr_control_data_ptr->tc_0_4_read_count_fr_si13++;
    MSG_GERAN_HIGH_1_G("SI13 not received count incremented , curr value is = %d",rr_control_data_ptr->tc_0_4_read_count_fr_si13);
  }
}

/**
  @brief This function decrements the count by one if on  relevant TC 
            SI2bis or SI2ter has not been read.
  @param
  @return None
*/

void rr_decrement_read_count_for_optional_SI2bis_SI2ter_SI13(const gas_id_t gas_id)
{
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr(gas_id);
  
  if (rr_control_data_ptr->tc_5_read_count_fr_si2bis > 0)
  {
    rr_control_data_ptr->tc_5_read_count_fr_si2bis--;
  }

  if (rr_control_data_ptr->tc_4_5_read_count_fr_si2ter > 0)
  {
    rr_control_data_ptr->tc_4_5_read_count_fr_si2ter--;
  }

  if (rr_control_data_ptr->tc_0_4_read_count_fr_si13 > 0)
  {
    rr_control_data_ptr->tc_0_4_read_count_fr_si13--;
  }
  MSG_GERAN_HIGH_3_G("SI2bis SI2ter SI13 TC read but  not rcvd count is = %d, %d,  %d",
                      rr_control_data_ptr->tc_5_read_count_fr_si2bis,
                      rr_control_data_ptr->tc_4_5_read_count_fr_si2ter,
                      rr_control_data_ptr->tc_0_4_read_count_fr_si13);
}
...
/**
  @brief This function reset  the count for that SIs.
  @param
  @return None
*/

void rr_reset_tc_read_count_for_optional_SI2bis_SI2ter_SI13(uint8 message_type, const gas_id_t gas_id)
{
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr(gas_id);
  
  if (message_type == SYSTEM_INFORMATION_TYPE_2_BIS)
  {
    rr_control_data_ptr->tc_5_read_count_fr_si2bis = 0;
    MSG_GERAN_HIGH_0_G("Resetting  tc_5_read_count_fr_si2bis");
  }
  else if (message_type == SYSTEM_INFORMATION_TYPE_2_TER)
  {
    rr_control_data_ptr->tc_4_5_read_count_fr_si2ter =0;
    MSG_GERAN_HIGH_0_G("Resetting  tc_4_5_read_count_fr_si2ter");
  }  
  else if (message_type == SYSTEM_INFORMATION_TYPE_13)
  {
    rr_control_data_ptr->tc_0_4_read_count_fr_si13 =0;
    MSG_GERAN_HIGH_0_G("Resetting  tc_0_4_read_count_fr_si13");
  }
  else
  {
    rr_control_data_ptr->tc_5_read_count_fr_si2bis = 0;
...
/**
  @brief This function returns the count 
  @param
  @return None
*/

uint8 rr_get_tc_0_4_read_count_fr_si13(const gas_id_t gas_id)
{
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr(gas_id);
  MSG_GERAN_HIGH_1_G("SI13 not received tried  = %d times",rr_control_data_ptr->tc_0_4_read_count_fr_si13);
  return rr_control_data_ptr->tc_0_4_read_count_fr_si13; 
}

/**
  @brief This function returns the count for TC read for SI2bis
  @param
  @return None
*/

uint8 rr_get_tc_5_read_count_fr_SI2bis(const gas_id_t gas_id)
{
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr(gas_id);
  MSG_GERAN_HIGH_1_G("SI2bis not received tried  = %d times",rr_control_data_ptr->tc_5_read_count_fr_si2bis);
  return rr_control_data_ptr->tc_5_read_count_fr_si2bis; 
}

/**
  @brief This function returns the count for TC read for SI2ter
  @param
  @return None
*/

uint8 rr_get_tc_4_5_read_count_fr_SI2ter(const gas_id_t gas_id)
{
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr(gas_id);
  MSG_GERAN_HIGH_1_G("SI2ter not received tried  = %d times",rr_control_data_ptr->tc_4_5_read_count_fr_si2ter);
  return rr_control_data_ptr->tc_4_5_read_count_fr_si2ter; 
}

#ifdef FEATURE_QSH_MDUMP
QSH_MDUMP_FN_ATTR
void grr_mdump_rr_control(const gas_id_t gas_id)
{
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr(gas_id);
  qsh_mdump_collect_high(rr_control_data_ptr, sizeof(rr_control_data_t));
}

QSH_MDUMP_FN_ATTR
void grr_mdump_rr_state(const gas_id_t gas_id)
{
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr(gas_id);
  qsh_mdump_collect_high(&rr_control_data_ptr->rr_state, sizeof(rr_state_T));
  qsh_mdump_collect_high(&rr_control_data_ptr->old_rr_state, sizeof(rr_state_T));
}
...
#ifdef FEATURE_LTE
/**
  @brief This function set csfb call status ind parameter in control data structure.
  @param gas_id
  @return None
*/
void rr_set_csfb_call_status_ind(const gas_id_t gas_id)
{
  // Obtain a pointer to the module data
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr(gas_id);
  RR_NULL_CHECK_FATAL(rr_control_data_ptr);

  rr_control_data_ptr->is_csfb_call_in_progress = TRUE;
}

/**
  @brief This function reset csfb call status ind parameter in control data structure.
  @param
  @return None
*/
void rr_reset_csfb_call_status_ind(const gas_id_t gas_id)
{
  // Obtain a pointer to the module data
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr(gas_id);
  RR_NULL_CHECK_FATAL(rr_control_data_ptr);
