...
$Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_cipher.c#4 $ $DateTime: 2020/10/21 01:49:42 $ $Author: pwbldsvc $
...
#define ALGORITHM_A5_NONE -1
ssgccs_2g_cell_enc_info_msg_handler cell_2g_enc_info_msg_handler = NULL;
 ...
 * \return NONE
 */
void register_ssgccs_2g_enc_info_msg_handler(ssgccs_2g_cell_enc_info_msg_handler msg_handler)
{
  MSG_GERAN_HIGH_0("<ssgccs> 2G encryption info msg handler is registered");
  cell_2g_enc_info_msg_handler = msg_handler;
} /* register_ssgccs_2g_enc_info_msg_handler() */
#endif /* PERLUTF */
/*===========================================================================
...
)
{
  ...
  (void)ghdi_ds_read_hw_classmark(&hw_classmark, RR_GAS_ID_TO_AS_ID);
#else
  (void)ghdi_read_hw_classmark(&hw_classmark);
#endif /* FEATURE_DUAL_SIM || FEATURE_TRIPLE_SIM */
  MSG_GERAN_HIGH_1_G("[nv533 check]hw_classmark.A5_4_supported :%d",hw_classmark.A5_4_supported);
  ...
  if (current_IE != NULL)
  {
    ...
    if (current_IE->start_IE != NULL)
    {
      ...
      if ((*(current_IE->start_IE) & 0x01) == 0x01)
      {
        ...
        /* Current Encryption algo to SSGCCS module */
        cell_enc_info.cell_2g_enc = algorithm_identifier;
      ...
      else
      {
        MSG_GERAN_HIGH_0_G("Cipher mode command with SC=0 (no ciphering)");
        {
          ...
          /* Current Encryption algo to SSGCCS module */
          cell_enc_info.cell_2g_enc = ALGORITHM_A5_NONE;

          geran_eng_mode_data_write(ENG_MODE_GSM_CIPHER_INFO,(void*)&gsm_cipher_info, TRUE, gas_id);
        }
      }

      /* Update Enc info to SSGCCS Module */
      if (cell_2g_enc_info_msg_handler != NULL)
      {
        cell_enc_info.as_id = geran_map_gas_id_to_nas_id(gas_id);
        cell_2g_enc_info_msg_handler(&cell_enc_info);
      }
