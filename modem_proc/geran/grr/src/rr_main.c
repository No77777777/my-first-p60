...
$Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_main.c#4 $ $DateTime: 2020/10/12 06:57:59 $ $Author: pwbldsvc $
...
static timer_type  rr_sleep_timer;
...
static void rr_main_task_init(void)
{
  ...
  timer_def(&rr_sleep_timer, NULL, rex_self(), RR_SLEEP_SIG, NULL, 0);
#endif
...
{
  rr_pending_queue_T *queue_ptr;

  if (gas_id <= GERAN_ACCESS_STRATUM_ID_MAX)
  {
    queue_ptr = &(rr_pending_queue[GERAN_MAP_GAS_ID_TO_DATA_SPACE_INDEX(gas_id)]);
  }
  else
  {
    queue_ptr = NULL;
    MSG_GERAN_ERROR_0("Invalid GAS ID for pending RR queue");
  }
...
)
{
  ...
  switch (msg_ptr->message_header.message_set)
  {
...
#if defined (FEATURE_DUAL_SIM) || defined (FEATURE_TRIPLE_SIM)
            process_message = rr_can_process_rr_procedure_ind(msg_ptr, gas_id);		  	
#else
      ...
      switch (msg_ptr->rr.header.rr_message_set)
      {
        ...
        case RR_GRR_IMSG:
        {
          switch (msg_ptr->rr.header.imh.message_id)
          {
            ...
            {
              process_message = grr_can_process_rr_restore_scell_info(gas_id);
              valid_message = process_message;
        ...
        case RR_CELL_ACQ_IMSG:
        {
          switch (msg_ptr->rr.resel.imsg.header.imh.message_id)
          {
            ...
            {
              process_message = rr_cell_acq_can_process_surr_si_received_ind(gas_id);
              break;
            }
    ...
    case MS_MAC_RR:
    {
      switch (msg_ptr->message_header.message_id)
      {
        ...
        {
          process_message = rr_dtm_can_process_ul_tbf_req(gas_id);
          break;
        } 
        default: ;
      }
    ...
    case MS_MSGR_RR:
    {
      /*Added as part of MSGR feature*/
      MSGR_CLEAR_VARIANT(msg_ptr->rr_msgr_msg.message.hdr.id);
  ...
  if (!process_message)
  {
    ...
    MSG_GERAN_HIGH_3_G("Processing of (%d,%d) blocked in %d",
                       msg_ptr->message_header.message_set,
                       msg_ptr->message_header.message_id,
                       (RR_GRR == rr_state)? get_grr_state(gas_id): rr_state);
#endif /* FEATURE_GERAN_REDUCED_SPRINTF */
  }
  
  if(msg_ptr->message_header.message_set == MS_MSGR_RR)
  {
    if((process_message == FALSE) || ((process_message == TRUE) && (checking_for_pend_msg == TRUE)))
    {
      MSGR_SET_VARIANT(msg_ptr->rr_msgr_msg.message.hdr.id, RR_GAS_ID_TO_AS_ID);
    }
  }
...
#endif // PERLUTF
{
  ...
  while ((cmd_ptr = rr_q_get()) != NULL)
  {
    ...
    {
      process_message = should_message_be_processed(&cmd_ptr->msg, FALSE, gas_id);
    }
        ...
        case MS_MSGR_RR:
        {
          switch (cmd_ptr->msg.rr_msgr_msg.message.hdr.id)
          {
            ...
            case GERAN_GRR_LTE_GET_CGI_REQ:
            {
              ...
              {
                MSG_GERAN_ERROR_0("In SGLTE Mode Msg GERAN_GRR_LTE_GET_CGI_REQ, Reject Invoked when NO Abort Received");
              }
              break;
            }
...
)
{
  ...
  if (cmd_ptr != NULL)
  {
    ...
    do
    {
      ...
      if (should_message_be_processed(&cmd_ptr->msg, TRUE, gas_id) == RR_PROCESS_MSG_NOW)
      {
        // This is the first queue item which is not MS_RR_RR
...
static void rr_main_msgr_init(void)
{
  ...
  for (gas_id = GERAN_ACCESS_STRATUM_ID_1; gas_id <= GERAN_ACCESS_STRATUM_ID_MAX; gas_id++)
  {
    ...
    msgr_error = msgr_register_range_variant(
      MSGR_GERAN_GRR,
      &msgr_client,
      msgr_id,
...
#ifdef FEATURE_LTE
  msgr_error=msgr_register_variant(
    MSGR_GERAN_GRR,
    &msgr_client,
    msgr_id,
  ...
  */
  ...
  if (new_message->message_header.message_set == MS_RR_RR)
  {
    ...
    switch (new_message->rr.header.rr_message_set)
    {
      ...
      case RR_CELL_ACQ_IMSG:
      {
        rr_cell_acq_control(EV_INPUT_MESSAGE, new_message, gas_id);
        break;
      }

      case RR_DECODE_BCCH_IMSG:
      {
        rr_decode_bcch_list_control(EV_INPUT_MESSAGE, new_message, gas_id);
        break;
      }
      ...
      case RR_IMSG:
      {
        switch (new_message->rr.rr.imsg.header.imh.message_id)
        {
          ...
          case RR_IMSG_QSH_EVENT_CONFIG_IND:
          {
            rr_qsh_process_event_config_ind(&new_message->rr.rr.imsg.qsh_event_config_ind);
            break;
          }
      ...
      case RR_GRR_IMSG:
      {
        grr_control(EV_INPUT_MESSAGE, new_message, gas_id);
        break;
      }
  ...
  else if (new_message->message_header.message_set == MS_RR_L1)
  {
    ...
    switch (new_message->message_header.message_id)
    {
      ...
      {
        rr_handle_persistent_page_collison_ind( &new_message->mph.idle_coll_fallback_ind, gas_id);
        break;
      }
  ...
  else if (new_message->message_header.message_set == MS_MSGR_RR)
  {
    switch (new_message->rr_msgr_msg.message.hdr.id)
    {
      ...
      {
        rr_ms_store_tech_state(&(new_message->rr_msgr_msg.message.tech_state_bcast_ind));
        break;
      }
#endif /* FEATURE_DUAL_SIM || FEATURE_TRIPLE_SIM */
...
void rr_timed_wait(dword millisecs)
{
  ...
  {
    /*-------------------------------------------------------
       ** A call to timer_set and rex_wait will perform a
       ** timed wait
       **-----------------------------------------------------*/
    (void)timer_set(&rr_sleep_timer, (rr_duration_t)millisecs, 0, T_MSEC);

    rex_wait((rex_sigs_type)RR_SLEEP_SIG);
  }
#endif
}
...
#ifdef FEATURE_DUAL_SIM
    (void)ghdi_ds_read_hw_classmark(&hw_classmark, SYS_MODEM_AS_ID_1);
    (void)ghdi_ds_read_hw_classmark(&hw_classmark, SYS_MODEM_AS_ID_2);
#ifdef FEATURE_TRIPLE_SIM
    (void)ghdi_ds_read_hw_classmark(&hw_classmark, SYS_MODEM_AS_ID_3);
#endif
#else
