...
$Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_general.c#7 $ $DateTime: 2020/09/21 04:02:22 $ $Author: pwbldsvc $
...
typedef enum
{ 
  COLL_FALLBACK_UNKNOWN,
  COLL_FALLBACK_INVALID,
  COLL_FALLBACK_VALID
} coll_fallback_status_t;

typedef struct
{
  rr_resel_priority_e rat_priority[RR_RAT_MAX];
  coll_fallback_status_t coll_fallback_valid;
  trm_idle_coll_fallback_enum_t fallback_pref;
  sys_sys_mode_e_type least_priority_rat;
  rr_resel_priority_e pmin;
  rr_resel_priority_e pmax;
  uint8 offset;
  boolean persistent_page_collisions;
}rr_coll_fallback_data_t;
...
typedef struct
{
  ...
  /* Depriritized cell list. */
  rr_deprioritized_cell_info_T    rr_deprioritized_cell_list[RR_MAX_NUM_OF_DEPRIORITIZED_CELLS];
  ...
  #endif /* FEATURE_LTE */
  rr_coll_fallback_data_t idle_fallback_data;
#ifdef FEATURE_PRIORITIZE_LTE_FREQ
  rr_general_lte_acq_db_earfcn_s  lte_acq_db;
  uint8                           prioritize_lte_defer_count;
#endif /* FEATURE_PRIORITIZE_LTE_FREQ */
} rr_general_data_t;
...
/*+452757 ASR */
/**
  @brief Checks whether all SI2q/MI instances have been received.

  @return TRUE if all SI2q/MI instances have been received, FALSE
          otherwise.  Also returns FALSE if no SI2q/MI instances have been
          received, which means either that none are broadcast or none have
          been received.
*/
boolean rr_SI2q_MI_all_instances_received(
  instances_T *instances
)
{
  RR_NULL_CHECK_RETURN_PARAM(instances, FALSE);

  if (instances->num_instances > 0)
  {
    if (instances->instance[0] && instances->instance[0]->valid_data)
    {
      if (instances->instance[0]->common_params.COUNT == (instances->num_instances - 1))
      {
        return TRUE;
      }
      else
...
/**
 * Initiatise the DSC counter (See 05.08) based on
 * information in system information database
 */
static void rr_initialise_DSC_counter(
  system_information_database_T *si_db_ptr,
  const gas_id_t gas_id
)
{
  byte BS_PA_MFRMS = 0;
  int *DSC_counter_ptr = rr_get_DSC_counter_ptr(gas_id);

  /***********************************************/
  /* A new channel has been selected. We have to */
  /* initialise the DSC counter                  */
  /***********************************************/
  if (si_db_ptr && si_db_ptr->valid_data_flag)
  {
    BS_PA_MFRMS = si_db_ptr->control_channel_description.BS_PA_MFRMS;
  }
  
  if (BS_PA_MFRMS == 0)
  {
    BS_PA_MFRMS = BS_PA_MFRMS_MIN;
    MSG_GERAN_HIGH_0_G("Invalid BS_PA_MFRMS: default to BS_PA_MFRMS=2");
  }

  *DSC_counter_ptr = 90/BS_PA_MFRMS;
}
...
rr_l1_dtx_indicator_T rr_get_DTX_indicator(
  const gas_id_t gas_id
)
{
  rr_l1_dtx_indicator_T  DTX_indicator = RR_L1_DTX_OFF;
  byte                   DTX_value     = 0;
  gprs_scell_info_t *scell_info_ptr = rr_gprs_get_scell_info(gas_id);
  ...
  else
  {
    ...
    {
      rr_l1_dtx_indicator_T half_rate_map[] = {
                                               RR_L1_DTX_OFF,
                                               RR_L1_DTX_OFF,
                                               RR_L1_DTX_OFF,
                                               RR_L1_DTX_ON,
      ...
      rr_l1_dtx_indicator_T full_rate_map[] = {
                                               RR_L1_DTX_ON,
                                               RR_L1_DTX_ON,
                                               RR_L1_DTX_OFF,
                                               RR_L1_DTX_ON,
  ...
  if ( (rr_general_data_ptr->rr_dtx_override_status == DTX_NAS_OVERRIDE_ACTIVE) ||
       (rr_general_data_ptr->rr_dtx_override_status == DTX_RR_OVERRIDE_ACTIVE) )
  {
    DTX_indicator = RR_L1_DTX_OVERRIDE;
  }
...
/**
 * determines if the given procedure is for PS activity,
 * if NOT the function returns FALSE
 *
 * @param sys_proc_type_e_type proc_type   
 *
 * @return TRUE if the procedure is for PS
 */

boolean rr_is_procedure_for_ps( sys_proc_type_e_type proc_type, const gas_id_t gas_id )
{
  switch (proc_type)
  {
    ...
    case SYS_PROC_TYPE_NONE: /* Default None is PS */
      return TRUE;
    break;
    default:
    break;
...
{
  byte power_class;
  sd_ss_band_pref_e_type band_cap;
  boolean valid_band_support = FALSE;
  ...
  //  - which are not supported by the current subscription
#ifdef FEATURE_DUAL_SIM
  band_cap = sd_misc_get_subs_supp_band_pref(geran_map_gas_id_to_nas_id(gas_id));
#else
  band_cap = sd_misc_get_supp_band_pref();
#endif /* FEATURE_DUAL_SIM */

  if (SD_BAND_CONTAIN(band_cap, SD_SS_BAND_PREF_GSM_RGSM_900))
  {
    valid_band_support = TRUE;
  }
  else
  if (SD_BAND_CONTAIN(band_cap, SD_SS_BAND_PREF_GSM_EGSM_900))
  {
    valid_band_support = TRUE;
  }
  else
  if (SD_BAND_CONTAIN(band_cap, SD_SS_BAND_PREF_GSM_PGSM_900))
  {
    valid_band_support = TRUE;
  }

  if (SD_BAND_CONTAIN(band_cap, SD_SS_BAND_PREF_GSM_DCS_1800))
  {
    valid_band_support = TRUE;
  }

  if (SD_BAND_CONTAIN(band_cap, SD_SS_BAND_PREF_GSM_PCS_1900))
  {
    valid_band_support = TRUE;
  }

  if (SD_BAND_CONTAIN(band_cap, SD_SS_BAND_PREF_GSM_850))
  {
    valid_band_support = TRUE;
  }

  if (FALSE == valid_band_support)
  {
    MSG_GERAN_HIGH_0_G("No SD band support send power class as FF ");
    power_class = 0xff;
  }
  else
  {
...
)
{
  ...
  rr_reset_tc_read_count_for_optional_SI2bis_SI2ter_SI13(SYSTEM_INFORMATION_TYPE_2_TER, gas_id);
  rr_reset_tc_read_count_for_optional_SI2bis_SI2ter_SI13(SYSTEM_INFORMATION_TYPE_2_BIS, gas_id);
  rr_reset_tc_read_count_for_optional_SI2bis_SI2ter_SI13(SYSTEM_INFORMATION_TYPE_13, gas_id);
...
#ifdef FEATURE_FAKE_BTS_CELL_BARRING 
        if (rr_get_fake_bts_cell_barring_enabled(gas_id) == TRUE)
        {
...
#ifdef FEATURE_FAKE_BTS_CELL_DEPRIORITIZE
/**
 * Initialise deprioritized cell list.
 */
static void rr_clear_deprioritized_cell_data(uint8 index, const gas_id_t gas_id)
{
  rr_general_data_t *rr_general_data_ptr = rr_general_get_data_ptr(gas_id);

  MSG_GERAN_HIGH_1_G("Clear Index [%d] Cell data from Deprioritized Cell List", index);
  rr_general_data_ptr->rr_deprioritized_cell_list[index].arfcn.num = INVALID_ARFCN;
  rr_general_data_ptr->rr_deprioritized_cell_list[index].arfcn.band = INVALID_BAND;
  rr_general_data_ptr->rr_deprioritized_cell_list[index].bsic.BS_colour_code = INVALID_BSIC_CC;
  rr_general_data_ptr->rr_deprioritized_cell_list[index].bsic.PLMN_colour_code = INVALID_PLMN_CC;
  rr_general_data_ptr->rr_deprioritized_cell_list[index].cgi_valid = FALSE;
  rr_general_data_ptr->rr_deprioritized_cell_list[index].remaining_time = RR_INVALID_REMAINING_TIME;
}

/**
 * Search deprioritized cell list for a cell.
 */
void rr_init_deprioritized_cell_list(const gas_id_t gas_id)
{
  uint8 index;
  ...
  MSG_GERAN_HIGH_0_G("Initializing deprioritized cell list");
  for(index =0; index < RR_MAX_NUM_OF_DEPRIORITIZED_CELLS; index++)
  {
     rr_clear_deprioritized_cell_data( index, gas_id);
  }
} 

/**
 * Find the deprioritized cell with shortest remaining time. This cell will be removed
 * from the list in case list is full and a new cell needs adding.
 */
static uint8 rr_find_oldest_deprioritized_cell(const gas_id_t gas_id)
{
  uint8 index;
  uint8 index_of_shortest_time = RR_MAX_NUM_OF_DEPRIORITIZED_CELLS;
  uint32 shortest_remaining_time = RR_INVALID_REMAINING_TIME;
  ...
  for(index =0; index < RR_MAX_NUM_OF_DEPRIORITIZED_CELLS; index++)
  {
    if(rr_general_data_ptr->rr_deprioritized_cell_list[index].remaining_time <= shortest_remaining_time)
    {
      shortest_remaining_time = rr_general_data_ptr->rr_deprioritized_cell_list[index].remaining_time;
      index_of_shortest_time = index;
    }
  }

  MSG_GERAN_HIGH_4_G("Oldest deprioritized cell : Index [%d]  arfcn: %d, ncc: %d, bcc: %d",
     index_of_shortest_time,
     rr_general_data_ptr->rr_deprioritized_cell_list[index_of_shortest_time].arfcn.num,
     rr_general_data_ptr->rr_deprioritized_cell_list[index_of_shortest_time].bsic.PLMN_colour_code,
     rr_general_data_ptr->rr_deprioritized_cell_list[index_of_shortest_time].bsic.BS_colour_code);
 ...
 * Search deprioritized cell list for a cell.
 */
boolean rr_is_cell_in_deprioritized_cell_list(
  ARFCN_T arfcn,
  BSIC_T bsic,
  rr_csi_cgi_t *cgi,
  uint8 *cell_index,
...
)
{
  ...
  for (index = 0; index < RR_MAX_NUM_OF_DEPRIORITIZED_CELLS; index++)
  {
    if (ARFCNS_EQUAL(rr_general_data_ptr->rr_deprioritized_cell_list[index].arfcn, arfcn) &&
        rr_same_BSIC(&rr_general_data_ptr->rr_deprioritized_cell_list[index].bsic,  &bsic) &&
        ((cgi == NULL) ||
         (rr_same_lai(&rr_general_data_ptr->rr_deprioritized_cell_list[index].cgi.lai, &cgi->lai) &&
          rr_general_data_ptr->rr_deprioritized_cell_list[index].cgi.ci == cgi->ci)) &&
        (rr_general_data_ptr->rr_deprioritized_cell_list[index].remaining_time != RR_INVALID_REMAINING_TIME))
    {
      MSG_GERAN_HIGH_4_G("Cell exist in deprioritized cell list, Index[%d]: arfcn: %d, ncc: %d, bcc: %d",
        index,
        rr_general_data_ptr->rr_deprioritized_cell_list[index].arfcn.num,
        rr_general_data_ptr->rr_deprioritized_cell_list[index].bsic.PLMN_colour_code,
        rr_general_data_ptr->rr_deprioritized_cell_list[index].bsic.BS_colour_code);
...
/**
 * Add cell to deprioritized cell list
 */
void rr_add_to_deprioritized_cell_list(rr_deprioritized_cell_info_T *deprioritized_cell_ptr, const gas_id_t gas_id)
{
  uint8 index;
  uint8 cell_index = RR_MAX_NUM_OF_DEPRIORITIZED_CELLS;
  rr_general_data_t *rr_general_data_ptr = rr_general_get_data_ptr(gas_id);

  /* Restart deprioritized cell timer if there are deprioritized cells in the list */
  if (!rr_is_cell_in_deprioritized_cell_list(
         deprioritized_cell_ptr->arfcn,
         deprioritized_cell_ptr->bsic,
         &deprioritized_cell_ptr->cgi,
         &cell_index,
         gas_id
       )
     )
  {
    if (deprioritized_cell_ptr->remaining_time == 0)
    {
      MSG_GERAN_HIGH_0_G("Invalid deprioritized time 0 for new cell");
      return;
    }

    /* Find an unused location in deprioritized cell list */
    for (index = 0; index < RR_MAX_NUM_OF_DEPRIORITIZED_CELLS; index++)
    {
      if (rr_general_data_ptr->rr_deprioritized_cell_list[index].remaining_time == RR_INVALID_REMAINING_TIME)
      {
        cell_index = index;
        break;
      }
    ...
    /* List is full, cell is not added. Remove the oldest deprioritized cell from the list, 
     * i.e. cell with shortest remaining time and add new cell in it's place */
    if (cell_index == RR_MAX_NUM_OF_DEPRIORITIZED_CELLS)
    {
      MSG_GERAN_HIGH_0_G("Deprioritized Cell List is Full, hence over writing oldest entry in the list");
      cell_index = rr_find_oldest_deprioritized_cell(gas_id);
    }

    if (cell_index < RR_MAX_NUM_OF_DEPRIORITIZED_CELLS)
    {
      MSG_GERAN_HIGH_2_G("Adding requested Cell at index [%d] (timeout = %dms)", cell_index, deprioritized_cell_ptr->remaining_time);
      rr_general_data_ptr->rr_deprioritized_cell_list[cell_index].arfcn = deprioritized_cell_ptr->arfcn;
      rr_general_data_ptr->rr_deprioritized_cell_list[cell_index].bsic = deprioritized_cell_ptr->bsic;
      rr_general_data_ptr->rr_deprioritized_cell_list[cell_index].cgi = deprioritized_cell_ptr->cgi; 
      rr_general_data_ptr->rr_deprioritized_cell_list[cell_index].cgi_valid = deprioritized_cell_ptr->cgi_valid;
      rr_general_data_ptr->rr_deprioritized_cell_list[cell_index].remaining_time = deprioritized_cell_ptr->remaining_time;

      MSG_GERAN_HIGH_5_G("Cell added to deprioritized cell list, arfcn: %d, ncc: %d,  bcc: %d, lac %d, ci %d",
        deprioritized_cell_ptr->arfcn.num,
        deprioritized_cell_ptr->bsic.PLMN_colour_code,
        deprioritized_cell_ptr->bsic.BS_colour_code,
        deprioritized_cell_ptr->cgi.lai.location_area_code,
        deprioritized_cell_ptr->cgi.ci);
      
      rr_start_cell_deprioritized_timer(gas_id);
    }
    else
    {
      MSG_GERAN_ERROR_0_G("Failed to add the cell to deprioritized cell list");
    }
  }
  else
  {
    /* Cell already exists in the list. Update the new remaining time */
    MSG_GERAN_HIGH_3_G("Updating requested Cell at index [%d] (remaining_time = %dms, new timeout = %dms)", 
      cell_index,
      rr_general_data_ptr->rr_deprioritized_cell_list[cell_index].remaining_time,
      deprioritized_cell_ptr->remaining_time);
    if (deprioritized_cell_ptr->remaining_time == 0)
    {
      rr_clear_deprioritized_cell_data(cell_index, gas_id);
    }
    else
    {
      rr_general_data_ptr->rr_deprioritized_cell_list[cell_index].remaining_time = deprioritized_cell_ptr->remaining_time;
...
/**
 * Update remaining time of each cell in deprioritized cell list. If remaining time
 * is zero for any cell, then remove the cell from the list
 */
static void rr_update_deprioritized_cell_remaining_time(const gas_id_t gas_id)
{
  uint8 index;
  rr_general_data_t *rr_general_data_ptr = rr_general_get_data_ptr(gas_id);
  ...
  for(index =0; index < RR_MAX_NUM_OF_DEPRIORITIZED_CELLS; index++)
  {
    ...
    {
      /* Deduct timeout value from remaining time */
      if(rr_general_data_ptr->rr_deprioritized_cell_list[index].remaining_time > RR_CELL_DEPRIORITIZED_TIMEOUT)
      {
        rr_general_data_ptr->rr_deprioritized_cell_list[index].remaining_time -= RR_CELL_DEPRIORITIZED_TIMEOUT;
        MSG_GERAN_HIGH_2_G("Deprioritzed time stil for cell [%d] : %d", 
            index, 
            rr_general_data_ptr->rr_deprioritized_cell_list[index].remaining_time);
      }
      /* Otherwise, remove the cell from list, i.e. invalidate remaining time, arfcn, and bsic */
      else
      {
        MSG_GERAN_HIGH_2_G("Deprioritized time(%d) for cell [%d] is less than 2s hence clearing the cell Data",
			rr_general_data_ptr->rr_deprioritized_cell_list[index].remaining_time,
			index);
...
/**
 * Find out number of deprioritized cells in the list.
 */
static uint8 rr_get_deprioritized_cell_count(const gas_id_t gas_id)
{
  uint8 index;
  uint8 deprioritized_cell_count = 0;
  rr_general_data_t *rr_general_data_ptr = rr_general_get_data_ptr(gas_id);
  ...
  for(index =0; index < RR_MAX_NUM_OF_DEPRIORITIZED_CELLS; index++)
  {
    if(rr_general_data_ptr->rr_deprioritized_cell_list[index].remaining_time != RR_INVALID_REMAINING_TIME)
    {
      deprioritized_cell_count++;
    }
  }

  MSG_GERAN_HIGH_1_G("Deprioritized cell count: %d",deprioritized_cell_count);

  return deprioritized_cell_count;
}

/**
 * RR_CELL_DEPRIORITIZED_TIMER expired. Update deprioritized cell remaining time. Remove any
 * deprioritized cells whose remaining time is zero. Start RR_CELL_DEPRIORITIZED_TIMER if
 * there are deprioritized cells in the list.
 */
void rr_process_deprioritized_cell_timer_expiry(const gas_id_t gas_id)
{
  MSG_GERAN_HIGH_0_G("RR_CELL_DEPRIORITIZED_TIMEOUT received");

  rr_update_deprioritized_cell_remaining_time(gas_id);

  /* Restart deprioritized cell timer if there are deprioritized cells in the list */
  if (rr_get_deprioritized_cell_count(gas_id) != 0)
  {
    rr_start_cell_deprioritized_timer(gas_id);
  }
}
...
)
{
  ...
  for (cell_index = 0; (cell_index < no_of_entries) && (cell_index < BA_MAX_LEN); ++cell_index)
  {
    ...
    if (!remove_serving_cell || (serving_cell_arfcn.num != BA_ARFCN.num))
    {
      ...
      if (!rr_gapi_is_supported_band((sys_band_T)GET_ARFCN_BAND(BA_ARFCN), gas_id))
      {
...
void rr_notify_deep_sleep_to_other_rats(boolean in_deep_sleep, const gas_id_t gas_id)
{
  ...
  {
#if defined(FEATURE_DUAL_SIM) || defined(FEATURE_TRIPLE_SIM)
    rrc_set_ue_in_deep_sleep_sub_id(in_deep_sleep, geran_map_gas_id_to_nas_id(gas_id));
#else
  ...
  if (rr_g2x_rat_supported(RR_RAT_UTRAN_TDD, gas_id))
  {
    ...
    tdsrrc_set_ue_in_deep_sleep_sub_id(in_deep_sleep, geran_map_gas_id_to_nas_id(gas_id));
#else
...
)
{
  ...
  rr_ded_irat_measurements_t    *wcdma_ded_meas_ptr = rr_get_wcdma_ded_meas_ptr(gas_id);
  ...
  /* reinitialize variables for new serving cell */
  rr_initialise_DSC_counter(sys_info_source, gas_id);
  ...
  rr_log_acq_db_arfcn_list_add(gas_id);
  ...
  else
  {
    ...
    else
    {
      ...
      else
      {
        ...
        {
          MSG_GERAN_HIGH_2_G("fallback flag = %d, coll fallback nv = %d",
                   rr_get_fallback_flag(gas_id),
                   rr_get_efs_nv_coll_fallback_enabled(RR_GAS_ID_TO_AS_ID)
                   );

          if ((rr_get_fallback_flag( gas_id) == TRUE) && rr_get_efs_nv_coll_fallback_enabled(RR_GAS_ID_TO_AS_ID))
          {
            rr_override_scell_si2q_params( scell_info_ptr, gas_id);
            rr_genereate_pmin_pmax_priority_for_earfcn_priority_list(gas_id);  
          }
...
#if defined(FEATURE_FAST_G2L_PSEUDO_SI2Q) || defined(FEATURE_PRIORITIZE_LTE_FREQ)
        else if (rr_is_pseudo_si2q_data_used(gas_id) == TRUE || rr_timer_is_running(RR_PRIORITIZE_LTE_ABORT_TIMER, gas_id))
        {
          MSG_GERAN_HIGH_2_G("fallback flag = %d, coll fallback nv = %d",
                    rr_get_fallback_flag(gas_id),
                    rr_get_efs_nv_coll_fallback_enabled(RR_GAS_ID_TO_AS_ID)
          ...
          #error code not present
#else
          MSG_GERAN_MED_0_G("Sending Pseudo SI2Q/Prioritized LTE data to L1");
          rr_send_mph_wcdma_cell_update_list_req(scell_info_ptr, gas_id);
...
)
{
  ...
  for (i = 0; i < MAX_INSTANCES; ++i)
  {
    ...
    if (!instance || !instance->valid_data)
    {
      continue;
    }
...
Parameters
[IN]
rr_ded_irat_measurements_t  *surr_wcdma_ded_meas_ptr
...
{
  byte num_meas_entries = surr_wcdma_ded_meas_ptr->no_of_entries;
  ...
  else
  {
    ...
    for (i = 0; i < num_meas_entries; ++i)
    {
      ...
      wcdma_cell_T *meas_cell = &surr_wcdma_ded_meas_ptr->entries[i].data.utran_fdd.cell;
      ...
      {
        memset((void *)&surr_wcdma_ded_meas_ptr->entries[i].data.utran_fdd, 0, sizeof(wcdma_ded_measurements_T));
        surr_wcdma_ded_meas_ptr->no_of_entries--;
...
)
{
  ...
  if (wcdma_ded_neighbor_list_ptr->mi_params.MI_3G_MP_Desc_PI == FALSE)
  {
    ...
    {
      /*+452757- ASR */
      if (rr_SI2q_MI_all_instances_received(rr_get_instances_ptr(SI2Q_INSTANCE,gas_id)))//All SI2qs  received
      {
      /*-452757- ASR */
        wcdma_ded_neighbor_list_ptr->mi_params.QSearch_C = QSEARCH_I_ALWAYS;
...
Parameters
[IN]
rr_ded_irat_measurements_t  *wcdma_ded_meas_ptr
...
)
{
  ...
  memset((void *)wcdma_ded_meas_ptr, 0, sizeof(rr_ded_irat_measurements_t));

  num_elements = (int)(sizeof(wcdma_ded_meas_ptr->entries) / sizeof(wcdma_ded_meas_ptr->entries[0]));

  /* Initialize the PN_offset to 0xFFFF */
  for (i = 0; i < num_elements; i++)
  {
    wcdma_ded_meas_ptr->entries[i].data.utran_fdd.PN_Offset = 0xFFFF;
  }
}
...
)
{
  ...
  for (i = 0; i < MAX_INSTANCES; ++i)
  {
...
#endif /*FEATURE_GSM_TDS*/
    byte curr_wcdma_cell_list_index;
    ...
    num_repeated_FDD_info = instance->common_params.num_repeated_FDD_info;
#ifdef FEATURE_GSM_TDS
    num_repeated_TDD_info = instance->common_params.num_repeated_TDD_info;
#endif /*FEATURE_GSM_TDS*/
    ...
    curr_wcdma_cell_list_index = instance->common_params.Index_Start_3G;
...
)
{
  ...
  rr_l1_dtx_indicator_T       DTX_indicator;
  rr_ba_lists_t               *rr_ba_lists_ptr = rr_get_ba_lists_ptr(gas_id);
...
{
  rr_timer_id_T ignored_timers[7];
  int           num_entries;
...
#ifdef FEATURE_FAKE_BTS_CELL_DEPRIORITIZE
  ignored_timers[num_entries] = RR_CELL_DEPRIORITIZED_TIMER;
  num_entries++;
...
)
{
  ...
  /* Re-order complete_BA_sacch list after adding ARFCNs from SI5/SI5bis
       This is to make sure that if ARFCN 0 is present in this sub-list, it will be moved to the end of the list */
  rr_reorder_BA_list(&rr_ba_lists_ptr->complete_BA_sacch);
  ...
  {
    BCCH_frequency_list_T temp_SI5ter_list = rr_ba_lists_ptr->SI5ter_BA_list;
    MSG_GERAN_MED_0_G("Adding new SI5ter to BA list");

    /*  If ARFCN 0 is present in this sub-list, then move it to the end and then add the list to complete_BA_sacch */
    rr_reorder_BA_list(&temp_SI5ter_list);

    /* SI5ter BA list should always be concatentated on to the end of the
    SI5/5bis list WITHOUT any further sorting. */
    rr_add_ba_list_without_sorting(&rr_ba_lists_ptr->complete_BA_sacch, &temp_SI5ter_list);
  }
  else
  {
...
/**************************************************************************************
Function
rr_allocate_SI2ter_instance

Description
Allocate memory for one SI2ter instance and initialize to the default values.
...
**********************************************************************************************/
void rr_allocate_SI2ter_instance(si2ter_instances_T *inst_ptr, byte index, const gas_id_t gas_id)
{
  si2ter_rest_T *instance;

  RR_NULL_CHECK_FATAL(inst_ptr);
  RR_NULL_CHECK_FATAL(index < MAX_SI2TER_INSTANCES);

  if (inst_ptr->instance[index] == NULL)
  {
    inst_ptr->instance[index] = GPRS_MEM_MALLOC(sizeof(si2ter_rest_T));
    RR_NULL_CHECK_FATAL(inst_ptr->instance[index]);
  }
  else
  {
    MSG_GERAN_ERROR_1_G("Memory was not freed for instance %d of SI2ter", index);
  }

  instance = inst_ptr->instance[index];
  memset((void *) instance, 0, sizeof(si2ter_rest_T));

  /* initialize some parameters to their default values which is different from 0 */
  instance->Qsearch_I       = DEFAULT_QSEARCH_I;
  instance->FDD_Qmin        = DEFAULT_FDD_QMIN;
  instance->FDD_Qoffset     = DEFAULT_FDD_QOFFSET;
  instance->FDD_Qmin_offset = DEFAULT_FDD_QMIN_OFFSET;
  instance->FDD_RSCPmin     = DEFAULT_FDD_RSCPMIN;
  instance->TDD_Qoffset     = DEFAULT_TDD_QOFFSET;
}

void rr_reset_SI2ter_instances(si2ter_instances_T *inst_ptr, const gas_id_t gas_id)
{
  byte i;

  RR_NULL_CHECK_FATAL(inst_ptr != NULL);

  MSG_GERAN_HIGH_0_G("Clearing SI2ter instances!");
...
void rr_initialize_SI2ter_instances(const gas_id_t gas_id)
{
  ...
  rr_reset_SI2ter_instances(si2ter_inst_ptr, gas_id);
}

/**************************************************************************************
Function
rr_allocate_SI2q_MI_instance

Description
Allocate memory for one SI2q/MI instance and initialize to the default values.

Parameters
[IN] *instances: Pointer to global storage for the instances.
[IN] index     : Index to the instance for which memory is going to be allocated
[IN]si2q_or_mi : SI2q or MI type

Returns
None

**********************************************************************************************/
void rr_allocate_SI2q_MI_instance(
  instances_T            *instances,
  byte                    index,
...
)
{
  ...
  RR_NULL_CHECK_FATAL(index < MAX_INSTANCES);

  if (instances->instance[index] == NULL)
  {
    instances->instance[index] = GPRS_MEM_MALLOC(sizeof(SI2q_MI_instance_T));
    RR_NULL_CHECK_FATAL(instances->instance[index]);
  }
  else
  {
    MSG_GERAN_ERROR_1_G("Memory was not freed for instance %d of SI2q/MI", index);
  }

  instance = instances->instance[index];
  memset((void *) instance, 0, sizeof(SI2q_MI_instance_T));

  /* initialize some parameters to their default values which is different from 0 */
  if (si2q_or_mi == SI2Q_INSTANCE)
  {
    instance->si2q_mi.si2q_params.FDD_Qmin  = DEFAULT_FDD_QMIN;
    instance->si2q_mi.si2q_params.QSearch_I = DEFAULT_QSEARCH_I;
    instance->si2q_mi.si2q_params.REPORT_TYPE = REPORT_TYPE_N0RMAL;
    instance->si2q_mi.si2q_params.FDD_Qoffset = DEFAULT_FDD_QOFFSET;
    instance->si2q_mi.si2q_params.TDD_Qoffset = DEFAULT_TDD_QOFFSET;
    instance->si2q_mi.si2q_params.FDD_Qmin_offset = DEFAULT_FDD_QMIN_OFFSET;
    instance->si2q_mi.si2q_params.FDD_RSCPmin = DEFAULT_FDD_RSCPMIN;
  }
  else if (si2q_or_mi == MI_INSTANCE)
  {
    MSG_GERAN_HIGH_1_G("Memory allocated for instance %d of MI", index);

    instance->si2q_mi.mi_params.REPORT_TYPE = REPORT_TYPE_N0RMAL;
  }
}
...
)
{
  ...
  RR_NULL_CHECK_FATAL(instances != NULL);

  if (si2q_or_mi == SI2Q_INSTANCE)
  {
    MSG_GERAN_HIGH_0_G("Clearing SI2quater instances!");
  }
  else if (si2q_or_mi == MI_INSTANCE)
  {
    MSG_GERAN_HIGH_0_G("Clearing Measurement Information instances!");
  }

  for (i = 0; i < MAX_INSTANCES; ++i)
  {
    if (instances->instance[i] != NULL)
    {
      if (si2q_or_mi == SI2Q_INSTANCE)
      {
      ...
      GPRS_MEM_FREE(instances->instance[i]);
...
)
{
  ...
  rr_initialize_SI2q_MI_instances(si2q_inst_ptr, SI2Q_INSTANCE, gas_id);
...
)
{
  ...
  rr_initialize_SI2q_MI_instances(mi_inst_ptr, MI_INSTANCE, gas_id);
...
)
{
  if (dedic_ptr != NULL)
  {
    ...
    {
      if ((sync_reason == MM_RR_RES_ASS)&&
          (rr_g2l_blind_redir_after_voice_ss_or_periodic_lau_enabled(gas_id)))
      {
         rr_connection_release_set_g2l_blind_redir_flag(gas_id);
      }
    }

    if ((sync_reason == MM_RR_RES_ASS)&& (MM_RR_LOCATION_UPDATE == rr_ce_access_reason(gas_id)))
    {
      MSG_GERAN_HIGH_0_G("Connection established for LAU, now modified for speech");
...
boolean rr_is_manual_plmn_selection_mode(
  const gas_id_t gas_id
)
{
  rr_PLMN_request_details_T *rr_PLMN_request_details_ptr = rr_get_PLMN_request_details_ptr(gas_id);
...
/*==========================================================================
  ...
  FUNCTION    rr_eng_mode_update_voice_sample_rate

  DESCRIPTION

  Updates the Eng Mode Data for Voice Sample Rate when it changes.

  PARAMETERS
    uint32 - voice_sample_rate
  ...
  SIDE EFFECTS
    None.
===========================================================================*/
void rr_eng_mode_update_voice_sample_rate(uint32 sample_rate, const gas_id_t gas_id)
{

#ifdef FEATURE_ENHANCED_AMR
  if (sample_rate != RR_DEFAULT_SAMPLE_RATE && sample_rate != RR_ENHANCED_SAMPLE_RATE)
  {
    MSG_GERAN_ERROR_1_G("rr_eng_mode_update_voice_sample_rate arg bad value=%d", sample_rate);
  }
  else
  {
    geran_eng_mode_codec_ho_sample_rate_t param_copy;

    MSG_GERAN_HIGH_1_G("rr_eng_mode_update_voice_sample_rate:: sample_rate=%d", sample_rate);

    param_copy.speech_enc_samp_freq = sample_rate;
...
)
{
  ...
  if (rr_SI2q_MI_all_instances_received(si2quater_instances))
  {
    return TRUE;
  }
  ...
  for (i = 0; i < MAX_INSTANCES; ++i)
  {
    ...
    if (si2quater_instances->instance[i] &&
        si2quater_instances->instance[i]->valid_data &&
        ...
        si2quater_instances->instance[i]->si2q_mi.si2q_params.priority_and_eutran_params_desc->eutran_params_desc->eutran_start)
    {
      for (j = i; j < MAX_INSTANCES; ++j)
      {
        ...
        if (si2quater_instances->instance[j] &&
            si2quater_instances->instance[j]->valid_data)
        {
...
)
{
  ...
  for (i = 0; i < MAX_INSTANCES; ++i)
  {
    ...
    if (instance &&
        instance->valid_data &&
        instance->si2q_mi.si2q_params.priority_and_eutran_params_desc != NULL &&
        instance->si2q_mi.si2q_params.priority_and_eutran_params_desc->eutran_params_desc != NULL)
    {
      uint32 j;

      eutran_params_desc = instance->si2q_mi.si2q_params.priority_and_eutran_params_desc->eutran_params_desc;
...
)
{
  ...
  {
    boolean dedicated_priority_present = FALSE;  
    boolean dedicated_priority_found = FALSE;  	  
    ...
    if (individual_priorities != NULL && *individual_priorities != NULL)
    {
      ...
      MSG_GERAN_HIGH_0_G("individual_priorities are valid");

      dedicated_priority_present = TRUE;
      dedicated_priority_found   = TRUE;  

      MSG_GERAN_HIGH_0_G("individual_priorities are valid for W/T");
      ...
      for (i = 0; i < dedicated_priority_list->num_utra_fdd_freq && i < LTE_IRAT_MAX_UTRAN_FREQ; ++i)
      {
        ...
        {
          if(rr_get_fallback_flag (gas_id) && rr_get_efs_nv_coll_fallback_enabled(RR_GAS_ID_TO_AS_ID))
          {
            dedicated_priority_list->utra_fdd_freq_list[i].priority = rr_get_modified_rat_priority(RR_RAT_WCDMA, gas_id);
          }
      ...
      if ((*individual_priorities)->default_utran_priority_valid)
      {
        if(rr_get_fallback_flag(gas_id) && rr_get_efs_nv_coll_fallback_enabled(RR_GAS_ID_TO_AS_ID))
        {
          (*individual_priorities)->default_utran_priority = rr_get_modified_rat_priority(RR_RAT_WCDMA, gas_id);
        }
        return (*individual_priorities)->default_utran_priority;
      }

      if (rr_nv_use_bcch_priority_when_dedicated_priorty_is_missing(gas_id))
      {
        dedicated_priority_found   = FALSE;  
        MSG_GERAN_HIGH_1_G("individual_priorities could not be found for UARFCN =%d",uarfcn);
      }
    }
	
    if (FALSE == (dedicated_priority_present && dedicated_priority_found))
    {
      rr_utran_frequency_list_t          utran_frequency_list;
      gprs_scell_info_t                * scell = rr_gprs_get_scell_info(gas_id);
      uint32                             utran_frequency_index;
      ...
      if (utran_frequency_index < RR_UTRAN_FREQUENCY_LIST_SIZE)
      {
        ...
        if (utran_priority_params != NULL)
        {
          if (utran_priority_params->priority_valid)
          {
            if(rr_get_fallback_flag(gas_id) && rr_get_efs_nv_coll_fallback_enabled(RR_GAS_ID_TO_AS_ID))
            {
              utran_priority_params->priority = rr_get_modified_rat_priority(RR_RAT_WCDMA, gas_id);
            }
          ...
          else
          {
            ...
            if (scell->utran_priority_params.default_params_valid)
            {
              if(rr_get_fallback_flag(gas_id) && rr_get_efs_nv_coll_fallback_enabled(RR_GAS_ID_TO_AS_ID))
              {
                scell->utran_priority_params.default_params.priority = rr_get_modified_rat_priority(RR_RAT_WCDMA, gas_id);
              }
...
)
{
  if (rr_is_camped(gas_id))
  {
    ...
    if (individual_priorities != NULL && *individual_priorities != NULL)
    {
	  ...
	  dedicated_priority_present = TRUE;
      dedicated_priority_found   = TRUE;
      MSG_GERAN_HIGH_0_G("individual_priorities are valid for LTE");
      ...
      for (i = 0; i < dedicated_priority_list->num_eutra_freq && i < LTE_IRAT_MAX_EUTRAN_FREQ; ++i)
      {
        if (dedicated_priority_list->eutra_freq_list[i].arfcn == earfcn)
        {
          if(rr_get_fallback_flag(gas_id) && rr_get_efs_nv_coll_fallback_enabled(RR_GAS_ID_TO_AS_ID))
          {
            dedicated_priority_list->eutra_freq_list[i].priority =
            rr_get_modified_earfcn_priority(dedicated_priority_list->eutra_freq_list[i].priority, gas_id);
          }
      ...
      if ((*individual_priorities)->default_eutran_priority_valid)
      {
        if(rr_get_fallback_flag(gas_id) && rr_get_efs_nv_coll_fallback_enabled(RR_GAS_ID_TO_AS_ID))
        {
           (*individual_priorities)->default_eutran_priority =
            rr_get_modified_earfcn_priority((*individual_priorities)->default_eutran_priority, gas_id);
        }
      ...
      {
        dedicated_priority_found   = FALSE;  
        MSG_GERAN_HIGH_1_G("individual_priorities could not be found for EARFCN = %d", earfcn);
      }
    }
    ...
    if (FALSE == (dedicated_priority_present && dedicated_priority_found))
    {
           ...
           ++i)
      {
        if (scell->lte_neighbour_list.entries[i].frequency.earfcn == earfcn)
        {
          if(rr_get_fallback_flag(gas_id) && rr_get_efs_nv_coll_fallback_enabled(RR_GAS_ID_TO_AS_ID))
          {
            scell->lte_neighbour_list.entries[i].priority = 
            rr_get_modified_earfcn_priority(scell->lte_neighbour_list.entries[i].priority, gas_id);
          }
...
uint8 rr_extract_priority_from_prority_list(
  rr_l1_irat_frequency_priority_list_t priority_list,
  uint32 arfcn,
  rr_l1_irat_rat_e rat)
{
  int i, priority = 0xFF;
  for (i = 0; (i < priority_list.count) && (i < MAX_RR_L1_PRIORITY_ENTRIES); i++)
  {
    if ((priority_list.entries[i].rat == rat) && (priority_list.entries[i].arfcn == arfcn))
    {
      priority = priority_list.entries[i].priority;
      break;
    }
  }
...
void rr_get_priority_params_for_uarfcn (
  uint16 uarfcn,
  rr_utran_rat_type_t rat_type,
  uint8 *priority,
  uint8 *thresh_high,
...
)
{
  ...
  /* Create the UTRAN Frequency List dynamically, then search it for the UARFCN that we are interested in.
     This gives the UTRAN Frequency Index, which is then searched for in the scell->utran_priority_params structure
     (which is populated whenever SI2quater / PMO is received that modifies the system information). */
  rr_utran_frequency_list_create(&utran_frequency_list, gas_id);
  ...
  *qrxlevmin = 0xFF;
  ...
  if (utran_frequency_index < RR_UTRAN_FREQUENCY_LIST_SIZE)
  {
    /* Search the global UTRAN Priority Parameters structure for an entry for the UTRAN Frequency Index
    ...
    if (utran_priority_params != NULL)
    {
      if(utran_priority_params->priority_valid)
      {
        *priority = utran_priority_params->priority;
        *thresh_high = utran_priority_params->thresh_high;
        *thresh_low = utran_priority_params->thresh_low;
        *qrxlevmin = utran_priority_params->qrxlevmin;
      }
      else if(scell->utran_priority_params.default_params_valid)
      {
        *priority = scell->utran_priority_params.default_params.priority;
        *thresh_high = scell->utran_priority_params.default_params.thresh;
        *thresh_low = scell->utran_priority_params.default_params.thresh;
        *qrxlevmin = scell->utran_priority_params.default_params.qrxlevmin;
      }
    }
  }
}
...
)
{
     ...
     ) == TRUE )
  {
    ...
    else if (tds_rrc_rat_is_supported(plmn_request_details->requested_PLMN_id))
    {
      tdd_enabled = TRUE;
    }
  }
...
boolean rr_is_utran_tdd_enabled(const gas_id_t gas_id)
{
  boolean tdd_enabled;
...
boolean rr_is_lte_enabled(const gas_id_t gas_id)
{
...
sys_band_T rr_map_sys_band_class_to_sys_band(sys_sband_e_type sys_sband)
{
  switch (sys_sband)
  {
...
sys_sband_e_type rr_map_sys_band_to_sys_band_class(sys_band_T sys_band)
{
  switch (sys_band)
  {
...
)
{
  ...
  lte_rrc_dedicated_pri_set_owner(RR_GAS_ID_TO_AS_ID, LTE_RRC_DEDICATED_PRI_OWNER_GERAN);
...
)
{
  ...
  if (rr_g2x_rat_supported(RR_RAT_LTE, gas_id))
  {
    ...
    if (lte_rrc_dedicated_pri_get_owner(as_id) == LTE_RRC_DEDICATED_PRI_OWNER_GERAN)
    {
      lte_rrc_dedicated_pri_set_owner(as_id, LTE_RRC_DEDICATED_PRI_OWNER_NONE);
    }
  }
  else
  {
    MSG_GERAN_HIGH_0_G("GERAN sub with no LTE Support: request to Clearing Individual Priorities N/A ");
  }
...
)
{
  ...
  if (rr_g2x_rat_supported(RR_RAT_LTE, gas_id))
  {
    ...
    {
      lte_rrc_dedicated_pri_owner_e dedicated_pri_owner = lte_rrc_dedicated_pri_get_owner(as_id);
      ...
      {
        lte_rrc_dedicated_pri_set_owner(as_id, LTE_RRC_DEDICATED_PRI_OWNER_GERAN);
  ...
  {
    MSG_GERAN_HIGH_0_G("GERAN Sub with no LTE Support: request to Dedicated priorities N/A ");
  }
...
)
{
  ...
  {
    MSG_GERAN_HIGH_0_G("GERAN Sub with no LTE Support: Requesting dedicated priorities N/A ");
  }
  ...
  if (rr_is_camped(gas_id))
  {
    ...
    if (individual_priorities != NULL && *individual_priorities != NULL)
    {
      ...
      if ((*individual_priorities)->default_geran_priority_valid)
      {
        if(rr_get_fallback_flag(gas_id) && rr_get_efs_nv_coll_fallback_enabled(RR_GAS_ID_TO_AS_ID))
        {
          (*individual_priorities)->default_geran_priority = rr_get_modified_rat_priority(RR_RAT_GSM, gas_id);
        }
      ...
      else
      {
        ...
        for (i = 0; i < (*individual_priorities)->dedicated_priority_list.num_geran_freq && i < LTE_IRAT_MAX_GERAN_FREQ; ++i)
        {
              ...
              (*individual_priorities)->dedicated_priority_list.geran_freq_list[i].band == rr_map_sys_band_to_sys_band_class(arfcn.band))
          {
            if(rr_get_fallback_flag(gas_id) && rr_get_efs_nv_coll_fallback_enabled(RR_GAS_ID_TO_AS_ID))
            {
              (*individual_priorities)->dedicated_priority_list.geran_freq_list[i].priority = rr_get_modified_rat_priority(RR_RAT_GSM, gas_id);;
            }
    ...
    if(rr_get_fallback_flag(gas_id) && rr_get_efs_nv_coll_fallback_enabled(RR_GAS_ID_TO_AS_ID))
    {
      scell->serving_cell_priority_params.geran_priority = rr_get_modified_rat_priority(RR_RAT_GSM, gas_id);
    }
...
void rr_sglte_dedicated_priority_info_store(sglte_l2g_resel_req_ded_pri_info_t * sglte_dedicated_pri_info_ptr, const gas_id_t gas_id)
{
  ...
  else
  {
    ...
    owner = lte_rrc_dedicated_pri_get_owner(as_id);
...
FUNCTION  rr_lte_neighbour_list_has_valid_earfcn

DESCRIPTION  This fucntion checks if LTE frequencies present in neighbour list are valid.
It returns TRUE if at least one valid frequency present in the list.

DEPENDENCIES None

RETURN VALUE boolean (TRUE/FALSE) Depdning upon whether valid earfcn present in the list.

SIDE EFFECTS None

===========================================================================*/

boolean rr_lte_neighbour_list_has_valid_earfcn(const gas_id_t gas_id)
{
  gprs_scell_info_t *scell_ptr = rr_gprs_get_scell_info(gas_id);
  uint8 i;

  for (i = 0; i < scell_ptr->lte_neighbour_list.count; i++)
  {
    if (lte_rrc_earfcn_is_supported(RR_GAS_ID_TO_AS_ID, scell_ptr->lte_neighbour_list.entries[i].frequency.earfcn, rr_band_pref_lte(gas_id)))
    {
      MSG_GERAN_HIGH_0_G("[Pseudo SI2q] LTE neighbout list has valid frequency");
      return TRUE;
    }
  }

  MSG_GERAN_HIGH_1_G("[Pseudo SI2q] LTE neighbout list has no valid frequency (total count = %d)", scell_ptr->lte_neighbour_list.count);
  return FALSE;
}
...
{
  earfcn_nas_type nas_earfcn = {0};
  rr_general_earfcn_list earfcn_list = {0};
  ...
  {
    MSG_GERAN_ERROR_0_G("[Pseudo SI2q] No Valid Serving Cell to Save the Data");
    return FALSE;
  }
  ...
  {
    MSG_GERAN_HIGH_0_G("[Pseudo SI2q] GRR Fast G2L Reselection Feature NOT enabled or EFS Data NOT VALID");
    return FALSE;
  }

  if ( rr_is_pseudo_si2q_data_used(gas_id) && (serving_cell_ptr->lte_neighbour_list.count > 0) )
  {
    MSG_GERAN_HIGH_0_G("[Pseudo SI2q] Invoked when PSEUDO SI2Q data already set in DB and Valid LTE cell List Exists");
    return TRUE;
  }
  ...
  {
     MSG_GERAN_HIGH_0_G("[Pseudo SI2q] NAS get_lte_earfcn_list_from_nas() returns FALSE");
     nas_earfcn.count = 0;
  }
  else if (nas_earfcn.count == 0)
  {
     MSG_GERAN_ERROR_0_G("[Pseudo SI2q] NAS get_lte_earfcn_list_from_nas() returns zero num of freq");
  }
  else if (nas_earfcn.count > NAS_MAX_EARFCN_COUNT)
  {
     MSG_GERAN_ERROR_1_G("[Pseudo SI2q] NAS get_lte_earfcn_list_from_nas() returns invalid num of freq. %d", nas_earfcn.count );
     nas_earfcn.count = NAS_MAX_EARFCN_COUNT;
  }

  for (i = 0; i < nas_earfcn.count; i++)
  {
    /* Check for the frequencies which do not fall on Supported Bands */
    if (lte_rrc_earfcn_is_supported(as_id, nas_earfcn.earfcn_nas[i], rr_band_pref_lte(gas_id)))
    {
      earfcn_list.earfcns[earfcn_list.count++] = nas_earfcn.earfcn_nas[i];
    }
    else
    {
      MSG_GERAN_ERROR_1_G("[Pseudo SI2q] EARFCN %d not supported", nas_earfcn.earfcn_nas[i]);
    }
  }

#ifdef FEATURE_PRIORITIZE_LTE_FREQ
   rr_general_add_lte_acq_db_earfcns(&earfcn_list, GRR_PSEUDO_SI2Q_MAX_EARFCN, serving_cell_ptr->gsm.location_area_identification.plmn_id, gas_id);
#endif /* FEATURE_PRIORITIZE_LTE_FREQ */

  if (earfcn_list.count == 0)
  {
     MSG_GERAN_ERROR_0_G("[Pseudo SI2q] No valid freqs present");
     return FALSE;
  }

  if (serving_cell_ptr->gsm.camped_on_flag == FALSE)
  {
    MSG_GERAN_ERROR_0_G("[Pseudo SI2q] called when GRR NOT CAMPED");
  }
  ...
  if (pseudo_si2q_data_p != NULL)
  {
    ...
    if ((serving_cell_ptr->lte_neighbour_list.count == 0) || (rr_is_pseudo_si2q_data_used(gas_id) == FALSE ))
    {
      ...
      /* Now the LTE Part */
      for (i =0; (i < earfcn_list.count) && (i < GRR_PSEUDO_SI2Q_MAX_EARFCN); i++)
      {

        MSG_GERAN_HIGH_1_G("[Pseudo SI2q] Adding LTE EARFCN %d", earfcn_list.earfcns[i]);

        dst = &serving_cell_ptr->lte_neighbour_list.entries[i];
        dst->frequency.earfcn = earfcn_list.earfcns[i];
      ...
      serving_cell_ptr->lte_neighbour_list.count = earfcn_list.count;
  ...
  MSG_GERAN_ERROR_0_G("[Pseudo SI2q] Returns FALSE from End of Function");
    ...
    else if (rr_is_camped(gas_id))
    {
          ...
          (cell_info->gsm.two_quater_position == SI2Q_POSITION_NONE))
      {
        ...
        update_reqd = TRUE;
               ...
               (cell_info->gsm.lte_neighbour_list.count == 0 || !rr_lte_neighbour_list_has_valid_earfcn(gas_id)))
      {
...
/*!
 * \brief Returns the MCC and MNC contained in a PLMN ID.
 *
 * \param plmn(in)
 *
 * \param mcc(in)
 ...
 * \return void
 */
void rr_get_mcc_mnc(sys_plmn_id_s_type  plmn,
                    sys_mcc_type       *mcc,
                    sys_mnc_type       *mnc)
{
...
/*!
 * \brief Returns TRUE or FALSE based on the presence of the cell LAI in EFS F-Cell list
 *
 * \param LAI
 *
 * \param gas_id
 ...
 */
boolean rr_fcell_is_lai_in_efs_f_cells_list(LAI_T lai, const gas_id_t gas_id)
{
  rr_efs_geran_forbidden_cells_list_t *fcells_list_ptr = rr_nv_get_f_cells_list_ptr(); /* pointer to GERAN only EFS F-Cells */
  rr_efs_geran_forbidden_cells_t *fcells_ptr = fcells_list_ptr->fcells_ptr;
  uint8 num_of_fcells = fcells_list_ptr->num_of_fcells;
  uint8 i;
  sys_mcc_type mcc;
  sys_mnc_type mnc;
  ...
  sys_plmn_get_mcc_mnc(lai.plmn_id,&plmn_is_undefined,&mnc_includes_pcs_digit,&mcc,&mnc);

  if ((!rr_is_any_cell_selection_mode(gas_id)) && (fcells_ptr != NULL))
  {
    for (i=0; i < num_of_fcells; i++)
    {
      if ((rr_same_lai(&lai, &(fcells_ptr[i].lai))))
      {
        MSG_GERAN_HIGH_0("Cell LAI present in EFS F-cell list and cell id is not yet received.");
        return TRUE;
      }
    }
  }
...
boolean rr_fcell_is_cell_in_efs_f_cells_list(LAI_T lai, uint16 cell_id)
{
  rr_efs_geran_forbidden_cells_list_t *fcells_list_ptr = rr_nv_get_f_cells_list_ptr(); /* pointer to GERAN only EFS F-Cells */
  rr_efs_geran_forbidden_cells_t *fcells_ptr = fcells_list_ptr->fcells_ptr;
  uint8 num_of_fcells = fcells_list_ptr->num_of_fcells;
  ...
  if (fcells_ptr != NULL)
  {
    MSG_GERAN_MED_0("fcells_ptr is not NULL");
    for (i=0; i < num_of_fcells; i++)
    {

      if ((rr_same_lai(&lai, &(fcells_ptr[i].lai))) && (cell_id == fcells_ptr[i].ci))
      {
        MSG_GERAN_HIGH_0("Cell present in EFS F-cell list");
        return TRUE;
      }
    }
  }
...
boolean rr_fcell_is_cell_available(LAI_T lai, uint16 cell_id, const gas_id_t gas_id)
{
  if ((!rr_is_any_cell_selection_mode(gas_id)) && 
      (rr_fcell_is_cell_in_efs_f_cells_list(lai,cell_id)))
  {
    return FALSE;
  }
...
/*!
 * \brief Returns TRUE if Requested PLMN match with FCell PLMN
 *        
 *
 * \param plmn_id (in)
 *
 * \return TRUE - Matched with Requested PLMN with FCELL PLMN, FALSE - Could not match
 */

boolean rr_is_plmn_in_efs_f_cells_list(PLMN_id_T plmn_id)
{
  uint8 i;
  rr_efs_geran_forbidden_cells_list_t *fcells_list_ptr = rr_nv_get_f_cells_list_ptr(); /* pointer to GERAN only EFS F-Cells */
  rr_efs_geran_forbidden_cells_t *fcells_ptr = fcells_list_ptr->fcells_ptr;
  ...
  if (fcells_ptr != NULL)
  {
    for (i=0; i < num_of_fcells; i++)
    {
      if (sys_plmn_match(plmn_id,fcells_ptr[i].lai.plmn_id))
      {
        MSG_GERAN_HIGH_0("Cell present in EFS F-cell list");
        return TRUE;
      }
    }
    MSG_GERAN_MED_1("fcells list is not having desired PLMN, number of PLMN are %d",num_of_fcells );
  }
  return FALSE;
}
...
boolean rr_high_speed_train_opt_allowed(const gas_id_t gas_id)
{
  ...
  if (TRUE == rr_is_hst_cell_freq_list_present(gas_id))
  {
    ...
    if ( TRUE == rr_nv_hst_opt_is_plmn_match(irat_acq_db_ptr->plmn_id, gas_id) && /*HST opt only used for customer specified white list*/
         TRUE == rr_ms_sub_capabilities_exclude_sys_mode(SYS_SYS_MODE_MASK_LTE, gas_id)  /*HST opt is only used on non LTE sub*/   
       )
    {
...
/*!
 * \brief Update Received freq inform of BA List 
 *        and send to L1      
 *
 * \param gas_id (in)
 *
 * \return void
 */
void rr_update_hst_freqs_to_l1(const gas_id_t gas_id
)
{
  gprs_scell_info_t *si_db_ptr = rr_gprs_get_scell_info(gas_id);
  BCCH_frequency_list_T irat_acq_db_freq_list;  
  ...
  if (rr_high_speed_train_opt_allowed(gas_id))
  {
    ...
    irat_acq_db_freq_list.no_of_entries = 0;
    irat_acq_db_freq_list.BA_IND = si_db_ptr->gsm.complete_BA_list.BA_IND;
    ...
    for ( i=0; i < irat_acq_db_ptr->freq_list.num_arfcns; i++ )
    {
      ...
      if ( rr_ds_is_supported_band((rr_internal_band_T)arfcn.band, RR_GAS_ID_TO_AS_ID) )
      {
        MSG_GERAN_HIGH_1_G("Adding IRAT acq DB ARFCN %d to the idle BA list During RR camped",arfcn.num);
        irat_acq_db_freq_list.BCCH_ARFCN[irat_acq_db_freq_list.no_of_entries] = arfcn;
        irat_acq_db_freq_list.no_of_entries++;
      }
    }

    rr_add_ba_list_without_sorting(&si_db_ptr->gsm.complete_BA_list,&irat_acq_db_freq_list);
    /* Generate a BA List from Freq's */
    rr_gprs_gsm_ba_list_updated(TRUE, gas_id);
  }
}
...
FUNCTION     rr_override_scell_si2q_params

DESCRIPTION  Copies Serving Cell and UTRAN Priority Params from NV Data Base to Scell.
...
===========================================================================*/
void rr_override_scell_si2q_params(gprs_scell_info_t *serving_cell_ptr, const gas_id_t gas_id)
{
  boolean irat_check;
  rr_efs_pseudo_si2q_def_data_t *pseudo_si2q_data_p = NULL;

  pseudo_si2q_data_p = rr_get_efs_nv_g2X_override_scell_si2q_data(RR_GAS_ID_TO_AS_ID);
 
  if ((serving_cell_ptr == NULL) || (pseudo_si2q_data_p == NULL))
  {
    MSG_GERAN_ERROR_0_G("rr_override_scell_si2q_params: No Valid Serving Cell OR Default NV Data Store to set");
    return;
  }

   /*
     1. When EUTRnetwork doesnt configure priority based reselection, UE shall use the self-generated SI-2qtr
     for priority based reselection between GSM and WCDMA.

     2. Incase of EUTRAN, presence of EUTRAN neigh implies Priority based reselection
   */ 

  irat_check = rr_is_lte_supported(gas_id) || rrc_rr_absolute_priority_support(RR_GAS_ID_TO_AS_ID);

  if ((!(irat_check && rr_serving_cell_priority_parameters_ie_received(rr_gprs_get_scell_info(gas_id)) && rr_get_inhibit_pri_resel(gas_id) == FALSE)))
  {
    serving_cell_ptr->serving_cell_priority_params_valid = TRUE;
    ...
    serving_cell_ptr->serving_cell_priority_params.t_reselection = pseudo_si2q_data_p->rr_geran_def_data.t_reselection;
    serving_cell_ptr->serving_cell_priority_params.h_prio = pseudo_si2q_data_p->rr_geran_def_data.h_prio;
    serving_cell_ptr->eutran_params.meas_params.qsearch_c = 0x0f;
    ...
    MSG_GERAN_HIGH_0_G("rr_override_scell_si2q_params, Scell & UTRAN Priority Parameters SET from Default EFS Data ");
  } 

  /* Some times NW broadcast Serving cell prioirty and but no UTRAN priority are broadcasted, Default set utran priority params. */
  if ((!(irat_check && (FALSE == rr_serving_cell_priority_parameters_ie_received(rr_gprs_get_scell_info(gas_id))) && rr_get_inhibit_pri_resel(gas_id) == FALSE)) &&
         (FALSE == serving_cell_ptr->utran_priority_params.default_params_valid))
  {
    /* This indicates that there are NO Default UTRAN Priority params or specific UTRAN Priority Params */
    serving_cell_ptr->utran_priority_params.default_params_valid = TRUE;
    serving_cell_ptr->utran_priority_params.default_params.priority = pseudo_si2q_data_p->rr_utran_def_data.priority;
    serving_cell_ptr->utran_priority_params.default_params.thresh = pseudo_si2q_data_p->rr_utran_def_data.thresh_high;
    serving_cell_ptr->utran_priority_params.default_params.qrxlevmin = pseudo_si2q_data_p->rr_utran_def_data.qrxlevmin;
    MSG_GERAN_HIGH_0_G("rr_override_scell_si2q_params, Serving cell priority params present, but UTRAN Priority Parameters SET from Default EFS Data ");
  }
...
FUNCTION     rr_handle_persistent_page_collison_ind

DESCRIPTION  This function handles the persisten page collision indication message from MCS.
...
===========================================================================*/
void rr_handle_persistent_page_collison_ind( 
...
)
{
  ...
  if ((mph_idle_coll_fallback_ind->fallback_pref == TRM_FALLBACK_GSM_PREF) ||
      (mph_idle_coll_fallback_ind->fallback_pref == TRM_FALLBACK_LTE_PREF) ||
      (mph_idle_coll_fallback_ind->fallback_pref == TRM_FALLBACK_UMTS_PREF)
     )
  {
    fallback_flag = TRUE;
  }
  ...
  if ((rr_general_data_ptr->idle_fallback_data.persistent_page_collisions != fallback_flag))
  {
    if (fallback_flag == FALSE)
    {
      rr_reset_fallback_data(gas_id);
     
      if(get_rr_state(gas_id) == RR_GRR)
      {    
        MSG_GERAN_HIGH_0_G("RR_GRR_IMSG_REVERT_SCELL_INFO_IND send");
        rr_grr_send_imsg(RR_GRR_IMSG_REVERT_SCELL_INFO_IND, gas_id);
      }
      else
      {
        MSG_GERAN_HIGH_1_G("RR_GRR_IMSG_REVERT_SCELL_INFO_IND not send. RR state = %d", get_rr_state(gas_id));
      }
    }
    else
    { 
      rr_set_fallback_data( mph_idle_coll_fallback_ind, gas_id );         
    }
  }  
  else if ((rr_general_data_ptr->idle_fallback_data.fallback_pref != mph_idle_coll_fallback_ind->fallback_pref) ||
           (rr_general_data_ptr->idle_fallback_data.least_priority_rat != mph_idle_coll_fallback_ind->least_priority_rat))
  {
    rr_set_modified_rat_priority(mph_idle_coll_fallback_ind->least_priority_rat, mph_idle_coll_fallback_ind->fallback_pref, gas_id);
  }
}

/*===========================================================================

FUNCTION     rr_get_fallback_flag

DESCRIPTION  This function returns the status of persisten page collision.
...
)
{
  ...
  if ((rr_general_data_ptr->idle_fallback_data.coll_fallback_valid == COLL_FALLBACK_UNKNOWN) 
       && (rr_is_camped(gas_id)))
  {
    rr_mcc_fallback_allowed(gas_id);
  }  
  
  return ((rr_general_data_ptr->idle_fallback_data.persistent_page_collisions) && (rr_general_data_ptr->idle_fallback_data.coll_fallback_valid == COLL_FALLBACK_VALID));
}

/*===========================================================================

FUNCTION     rr_set_fallback_data

DESCRIPTION  This function sets the persisten page collision data to TRUE.
...
)
{
  ...
  rr_general_data_ptr->idle_fallback_data.persistent_page_collisions = TRUE;
  rr_set_modified_rat_priority(mph_idle_coll_fallback_ind->least_priority_rat, 
    mph_idle_coll_fallback_ind->fallback_pref, gas_id);
  
    MSG_GERAN_HIGH_0_G("persistent page collisions set to TRUE");
}

/*===========================================================================

FUNCTION     rr_reset_fallback_data

DESCRIPTION  This function resets the persisten page collision data.
...
)
{
  ...
  rr_general_data_ptr->idle_fallback_data.persistent_page_collisions = FALSE;
  rr_general_data_ptr->idle_fallback_data.coll_fallback_valid        = COLL_FALLBACK_UNKNOWN;
  rr_general_data_ptr->idle_fallback_data.fallback_pref              = TRM_FALLBACK_LTE_PREF;
  rr_general_data_ptr->idle_fallback_data.rat_priority[RR_RAT_GSM]   = RR_RESEL_PRIORITY_5;
  rr_general_data_ptr->idle_fallback_data.rat_priority[RR_RAT_WCDMA] = RR_RESEL_PRIORITY_5;
  rr_general_data_ptr->idle_fallback_data.rat_priority[RR_RAT_LTE]   = RR_RESEL_PRIORITY_5;
  rr_general_data_ptr->idle_fallback_data.least_priority_rat         = SYS_SYS_MODE_NONE;

  MSG_GERAN_HIGH_0_G("persistent page collisions set to FALSE; Data reset");
}

/*===========================================================================

FUNCTION     rr_mcc_fallback_allowed

DESCRIPTION  This function checks whether persistent page collision feature can be 
             supported on the MCC of the serving cell.
...
)
{
  ...
  sys_plmn_get_mcc_mnc(
      serving_cell_ptr->gsm.location_area_identification.plmn_id,
      &plmn_is_undefined,
  ...
  if (rr_general_data_ptr->idle_fallback_data.persistent_page_collisions == TRUE)
  {
    rr_general_data_ptr->idle_fallback_data.coll_fallback_valid = COLL_FALLBACK_VALID;
  }
}

/*===========================================================================

FUNCTION     rr_reset_coll_fallback_valid

DESCRIPTION  This function sets the coll_fallback_valid flag to COLL_FALLBACK_UNKNOWN.
...
===========================================================================*/
void rr_reset_coll_fallback_valid( 
  const gas_id_t gas_id 
)
{
  rr_general_data_t *rr_general_data_ptr = rr_general_get_data_ptr(gas_id);
  ...
  MSG_GERAN_HIGH_0_G("coll_fallback_valid set to COLL_FALLBACK_UNKNOWN");
  rr_general_data_ptr->idle_fallback_data.coll_fallback_valid = COLL_FALLBACK_UNKNOWN;
}

/*===========================================================================

FUNCTION     rr_set_modified_rat_priority

DESCRIPTION  This function evaluates the new RAT priorities of G, W & L,
             based on the least_priority_rat & fallback_pref.
...
===========================================================================*/
void rr_set_modified_rat_priority( 
  sys_sys_mode_e_type least_priority_rat, 
  trm_idle_coll_fallback_enum_t fallback_pref,
  const gas_id_t gas_id
)
{
  ...
  rr_general_data_ptr->idle_fallback_data.least_priority_rat = least_priority_rat;
  rr_general_data_ptr->idle_fallback_data.fallback_pref = fallback_pref;

  /* In G preferred fallback mode; Set GERAN_PRIORITY = 7; Based on least_rat_priority set L,W priority*/

  if (rr_general_data_ptr->idle_fallback_data.fallback_pref== TRM_FALLBACK_GSM_PREF)
  {
    rr_general_data_ptr->idle_fallback_data.rat_priority[RR_RAT_GSM] = RR_RESEL_PRIORITY_7;

    if (rr_general_data_ptr->idle_fallback_data.least_priority_rat == SYS_SYS_MODE_WCDMA)
    {
      rr_general_data_ptr->idle_fallback_data.rat_priority[RR_RAT_WCDMA] = RR_RESEL_PRIORITY_0;
      rr_general_data_ptr->idle_fallback_data.rat_priority[RR_RAT_LTE] = RR_RESEL_PRIORITY_6;
      rr_general_data_ptr->idle_fallback_data.offset = 1;
    }
    else if (rr_general_data_ptr->idle_fallback_data.least_priority_rat == SYS_SYS_MODE_LTE)
    {
      rr_general_data_ptr->idle_fallback_data.rat_priority[RR_RAT_WCDMA] = RR_RESEL_PRIORITY_6;
      rr_general_data_ptr->idle_fallback_data.rat_priority[RR_RAT_LTE] = RR_RESEL_PRIORITY_5;
      rr_general_data_ptr->idle_fallback_data.offset = 0;
    }
  }

  /* In LTE preferred fallback mode, Set GERAN_PRIORITY = 1; Set UTRAN_PRIORITY = 0;*/
  if (rr_general_data_ptr->idle_fallback_data.fallback_pref== TRM_FALLBACK_LTE_PREF)
  {
    rr_general_data_ptr->idle_fallback_data.rat_priority[RR_RAT_GSM]   = RR_RESEL_PRIORITY_1;
    rr_general_data_ptr->idle_fallback_data.rat_priority[RR_RAT_WCDMA] = RR_RESEL_PRIORITY_0;
    rr_general_data_ptr->idle_fallback_data.rat_priority[RR_RAT_LTE]   = RR_RESEL_PRIORITY_7;
    rr_general_data_ptr->idle_fallback_data.offset = 2;
  }

  /* In WCDMA Preferred Fallback Mode, Set WCDMA PRIORITY = 7, GSM is next Priority(6) and LTE is least prioirty(0-5). */
  if (rr_general_data_ptr->idle_fallback_data.fallback_pref == TRM_FALLBACK_UMTS_PREF)
  {
    rr_general_data_ptr->idle_fallback_data.rat_priority[RR_RAT_WCDMA] = RR_RESEL_PRIORITY_7;
    rr_general_data_ptr->idle_fallback_data.rat_priority[RR_RAT_GSM] = RR_RESEL_PRIORITY_6;
    rr_general_data_ptr->idle_fallback_data.rat_priority[RR_RAT_LTE] = RR_RESEL_PRIORITY_5;
    rr_general_data_ptr->idle_fallback_data.offset = 0;
  }

  MSG_GERAN_HIGH_5_G("fallback_pref = %d least_priority_rat = %d G priority = %d W priority = %d L priority = %d",
    rr_general_data_ptr->idle_fallback_data.fallback_pref,
    rr_general_data_ptr->idle_fallback_data.least_priority_rat,
    rr_general_data_ptr->idle_fallback_data.rat_priority[RR_RAT_GSM],
    rr_general_data_ptr->idle_fallback_data.rat_priority[RR_RAT_WCDMA],
    rr_general_data_ptr->idle_fallback_data.rat_priority[RR_RAT_LTE]);
}

/*===========================================================================

FUNCTION     rr_get_modified_rat_priority

DESCRIPTION  This function returns the priority of given RAT.

DEPENDENCIES None

RETURN       rr_resel_priority_e

SIDE EFFECTS None

===========================================================================*/
rr_resel_priority_e rr_get_modified_rat_priority( 
  rr_rat_e rat_type,
  const gas_id_t gas_id
)
{
  ...
  MSG_GERAN_HIGH_2_G("rat_type = %d rat_priority = %d", rat_type, rr_general_data_ptr->idle_fallback_data.rat_priority[rat_type]); 

  return(rr_general_data_ptr->idle_fallback_data.rat_priority[rat_type]);
}

/*===========================================================================

FUNCTION     rr_get_round_val

DESCRIPTION  This function returns the roundedoff value of a double variable.
...
===========================================================================*/
 uint8 rr_get_round_val(double x)
{
  uint8 new_val;
  uint8 rem,qtnt;
  uint8 r_value;
...
FUNCTION     rr_get_modified_earfcn_priority

DESCRIPTION  This function evaluates the modified priority of a given earfcn.
...
===========================================================================*/
rr_resel_priority_e rr_get_modified_earfcn_priority(
  uint8 prio_orig, 
  const gas_id_t gas_id
)
{
  uint8 pmin, pmax, offset;
  rr_resel_priority_e prio_new;
  rr_general_data_t *rr_general_data_ptr = rr_general_get_data_ptr(gas_id);
  RR_NULL_CHECK_FATAL(rr_general_data_ptr);
  
  pmin = rr_general_data_ptr->idle_fallback_data.pmin;
  pmax = rr_general_data_ptr->idle_fallback_data.pmax;
  offset = rr_general_data_ptr->idle_fallback_data.offset;

  if (pmin == pmax)
  {
    prio_new = rr_general_data_ptr->idle_fallback_data.rat_priority[RR_RAT_LTE];
  }
  else
  {
    prio_new = (rr_resel_priority_e) rr_get_round_val((((prio_orig - pmin)*5 )/(pmax - pmin)) + offset);
  } 

  MSG_GERAN_HIGH_4_G("pmin = %d pmax = %d offset = %d prio_new = %d", pmin, pmax, offset, prio_new);
  
  /* check whether prio_new is within range of [pmin, pmax] */
  if (rr_general_data_ptr->idle_fallback_data.fallback_pref== TRM_FALLBACK_GSM_PREF)
  {
    if (rr_general_data_ptr->idle_fallback_data.least_priority_rat == SYS_SYS_MODE_WCDMA)
    {
      if ((prio_new >= RR_RESEL_PRIORITY_1) && (prio_new <= RR_RESEL_PRIORITY_6))
      {
        return (prio_new);
      }
    }
    ...
    {
      if ((prio_new >= RR_RESEL_PRIORITY_0) && (prio_new <= RR_RESEL_PRIORITY_5))
      {
        return (prio_new);
      }
    }
  }
  else if (rr_general_data_ptr->idle_fallback_data.fallback_pref== TRM_FALLBACK_LTE_PREF)
  {
    if((prio_new >= RR_RESEL_PRIORITY_2) && (prio_new <= RR_RESEL_PRIORITY_7))
    {
      return (prio_new);
    }
  }
  else if (rr_general_data_ptr->idle_fallback_data.fallback_pref == TRM_FALLBACK_UMTS_PREF)
  {
    if ((prio_new >= RR_RESEL_PRIORITY_0) && (prio_new <= RR_RESEL_PRIORITY_5))
    {
      return (prio_new);
  ...
  return (rr_general_data_ptr->idle_fallback_data.rat_priority[RR_RAT_LTE]);
}

/*===========================================================================

FUNCTION     rr_genereate_pmin_pmax_priority_for_earfcn_priority_list

DESCRIPTION  This function generates the min and max range priorities from the earfcn list.
...
===========================================================================*/
void rr_genereate_pmin_pmax_priority_for_earfcn_priority_list(
  const gas_id_t gas_id
)
{
  rr_resel_priority_e pmin, pmax;
  uint32 i;
  rr_general_data_t *rr_general_data_ptr;
  gprs_scell_info_t *scell = rr_gprs_get_scell_info(gas_id);

  rr_general_data_ptr = rr_general_get_data_ptr(gas_id);
  RR_NULL_CHECK_FATAL(rr_general_data_ptr);

  pmin = RR_RESEL_PRIORITY_7; 
  pmax = RR_RESEL_PRIORITY_0; 
  ...
  for (i = 0;
       ...
       ++i)
  {  
    if (pmin > scell->lte_neighbour_list.entries[i].priority)
    {
      pmin = scell->lte_neighbour_list.entries[i].priority;
    }

    if (pmax < scell->lte_neighbour_list.entries[i].priority)
    {
      pmax = scell->lte_neighbour_list.entries[i].priority;
    }
  }
  rr_general_data_ptr->idle_fallback_data.pmin = pmin;
  rr_general_data_ptr->idle_fallback_data.pmax = pmax;

  MSG_GERAN_HIGH_2_G("pmin = %d pmax = %d", pmin, pmax);  
}

/*===========================================================================

FUNCTION     rr_get_fallback_pref_mode

DESCRIPTION  This function returns the fallback preferred mode.

DEPENDENCIES None

RETURN       trm_idle_coll_fallback_enum_t

SIDE EFFECTS None

===========================================================================*/

trm_idle_coll_fallback_enum_t rr_get_fallback_pref_mode(
  const gas_id_t gas_id
)
{
  rr_general_data_t *rr_general_data_ptr = rr_general_get_data_ptr(gas_id);

  return (rr_general_data_ptr->idle_fallback_data.fallback_pref);
}

boolean rr_is_geran_priority_higher_than_lte(gprs_scell_info_t *cell)
{
  uint32 i;

  for(i = 0; i < cell->lte_neighbour_list.count; i++)
  {
    if (cell->serving_cell_priority_params.geran_priority >= cell->lte_neighbour_list.entries[i].priority)
    {
      MSG_GERAN_HIGH_0("GERAN priority is higher than LTE priority, returns TRUE ");
      return TRUE;
    }
  }
  return FALSE;
...
void rr_increase_lte_priority_over_geran(gprs_scell_info_t *cell, const gas_id_t gas_id)
{
  uint32 i;
  rr_efs_increase_lte_cell_priority_data_t *rr_efs_increase_lte_cell_priority_data = rr_nv_get_increase_lte_cell_priority_data(RR_GAS_ID_TO_AS_ID);

  cell->serving_cell_priority_params.geran_priority = rr_efs_increase_lte_cell_priority_data->geran_default_priority;
  for(i = 0; i < cell->lte_neighbour_list.count; i++)
  {
    cell->lte_neighbour_list.entries[i].priority = rr_efs_increase_lte_cell_priority_data->lte_default_priority;
  }
}

#ifdef FEATURE_PRIORITIZE_LTE_FREQ
/**
  @brief this funtion checks if the provided earfcn is present in earfcn nas list
*/
static boolean rr_general_check_for_duplicate_earfcn(rr_general_earfcn_list *earfcn_list, uint32 earfcn)
{
  int index;

  for (index = 0 ; index < earfcn_list->count; index++)
  {
    if (earfcn_list->earfcns[index] == earfcn)
    {
      return TRUE;
    }
  }
...
/**
  @brief this funtion sends LTE_RRC_ACQ_DB_EARFCN_REQ to LRRC
*/
void rr_general_send_lte_rrc_acq_db_earfcn_req(const gas_id_t gas_id)
{
  lte_rrc_acq_db_earfcn_req_s acq_db_earfcn_req;
  errno_enum_type             result;

  memset(&acq_db_earfcn_req, 0, sizeof(lte_rrc_acq_db_earfcn_req_s));

  msgr_init_hdr_variant(
    &acq_db_earfcn_req.msg_hdr,
    MSGR_GERAN_GRR,
    LTE_RRC_ACQ_DB_EARFCN_REQ,
    RR_GAS_ID_TO_AS_ID
  );

  result = rr_msgr_send(&acq_db_earfcn_req.msg_hdr, sizeof(acq_db_earfcn_req), gas_id);
  if (result != E_SUCCESS)
  {
    MSG_GERAN_ERROR_1_G("rr_msgr_send failed: %d", result);
  }
...
void rr_general_store_lte_acq_db(lte_rrc_acq_db_earfcn_rsp_s *lte_acq_db_earfcn_rsp, const gas_id_t gas_id)
{
  rr_general_data_t *rr_general_data_ptr = rr_general_get_data_ptr(gas_id);
  uint8             index;

  if (lte_acq_db_earfcn_rsp->num_earfcn_count > 0)
  {
    rr_general_data_ptr->lte_acq_db.earfcn_count = lte_acq_db_earfcn_rsp->num_earfcn_count;
    for (index = 0; index < lte_acq_db_earfcn_rsp->num_earfcn_count && index < LTE_RRC_CSP_MAX_ACQ_DB_EXT_ENTRIES; index++)
    {
      rr_general_data_ptr->lte_acq_db.earfcn_list[index] = lte_acq_db_earfcn_rsp->earfcn_list[index];
    }
  }
  else
  {
    MSG_GERAN_HIGH_0("There is no earfcn in the list received from LTE" );
  }
}

void rr_general_add_lte_acq_db_earfcns(rr_general_earfcn_list *earfcn_list, uint8 max_lte_frequencies, sys_plmn_id_s_type camped_plmn, const gas_id_t gas_id)
{
  rr_general_data_t *rr_general_data_ptr = rr_general_get_data_ptr(gas_id);
  uint8              i;
  uint8              plmn_index;
  rrc_plmn_identity_type lte_plmn = {0};
  sys_plmn_id_s_type earfcn_plmn;

  for (i = 0; (i < rr_general_data_ptr->lte_acq_db.earfcn_count) && (earfcn_list->count < max_lte_frequencies); i++)
  {
    if (rr_general_check_for_duplicate_earfcn(earfcn_list, rr_general_data_ptr->lte_acq_db.earfcn_list[i].earfcn))
    {
      continue;
    }

    for (plmn_index = 0; plmn_index < rr_general_data_ptr->lte_acq_db.earfcn_list[i].plmn_list.num_plmns; plmn_index++)
    {
      if (!rr_is_any_cell_selection_mode(gas_id))
      {
        lte_plmn.mcc[0] = rr_general_data_ptr->lte_acq_db.earfcn_list[i].plmn_list.plmn[plmn_index].mcc[0];
        lte_plmn.mcc[1] = rr_general_data_ptr->lte_acq_db.earfcn_list[i].plmn_list.plmn[plmn_index].mcc[1];
        lte_plmn.mcc[2] = rr_general_data_ptr->lte_acq_db.earfcn_list[i].plmn_list.plmn[plmn_index].mcc[2];
        lte_plmn.num_mnc_digits = rr_general_data_ptr->lte_acq_db.earfcn_list[i].plmn_list.plmn[plmn_index].num_mnc_digits;
        lte_plmn.mnc[0] = rr_general_data_ptr->lte_acq_db.earfcn_list[i].plmn_list.plmn[plmn_index].mnc[0];
        lte_plmn.mnc[1] = rr_general_data_ptr->lte_acq_db.earfcn_list[i].plmn_list.plmn[plmn_index].mnc[1];
        lte_plmn.mnc[2] = rr_general_data_ptr->lte_acq_db.earfcn_list[i].plmn_list.plmn[plmn_index].mnc[2];

        earfcn_plmn = mmumts_convert_rrc_plmn_id_to_nas_plmn_id(lte_plmn);
        if (!(sys_plmn_match(camped_plmn, earfcn_plmn)) && 
            !((rr_eplmn_list_equivalent_plmn(earfcn_plmn, gas_id)) && (rr_eplmn_list_equivalent_plmn(camped_plmn, gas_id))) &&
            !((rr_ehplmn_list_equivalent_plmn(earfcn_plmn, gas_id)) && (rr_ehplmn_list_equivalent_plmn(camped_plmn, gas_id))))
        {
          if (plmn_index == (rr_general_data_ptr->lte_acq_db.earfcn_list[i].plmn_list.num_plmns-1))
          {
            MSG_GERAN_MED_1_G("PLMN mismatch for LTE ACQ-DB EARFCN %d", rr_general_data_ptr->lte_acq_db.earfcn_list[i].earfcn);
          }
          continue;
        }
      }

      MSG_GERAN_MED_1_G("Adding LTE ACQ-DB EARFCN %d", rr_general_data_ptr->lte_acq_db.earfcn_list[i].earfcn);
      earfcn_list->earfcns[earfcn_list->count] = rr_general_data_ptr->lte_acq_db.earfcn_list[i].earfcn;
      earfcn_list->count++;
      break;
    }
  }
...
void rr_general_add_ncell_earfcns(rr_general_earfcn_list *earfcn_list, uint8 max_lte_frequencies, const gas_id_t gas_id)
{
  gprs_scell_info_t *scell_info_ptr = rr_gprs_get_scell_info(gas_id);
  uint8             index;

  for (index = 0; (index < scell_info_ptr->lte_neighbour_list.count) && (earfcn_list->count < max_lte_frequencies); index++)
  {
    if(rr_general_check_for_duplicate_earfcn(earfcn_list, scell_info_ptr->lte_neighbour_list.entries[index].frequency.earfcn))
    {
      continue;
    }

    MSG_GERAN_MED_1_G("Adding nCell EARFCN %d", scell_info_ptr->lte_neighbour_list.entries[index].frequency.earfcn);
    earfcn_list->earfcns[earfcn_list->count] = scell_info_ptr->lte_neighbour_list.entries[index].frequency.earfcn;
    earfcn_list->count++;
  }
}

void rr_general_prioritize_lte_frequencies(const gas_id_t gas_id)
{
  gprs_scell_info_t *scell_info_ptr = rr_gprs_get_scell_info(gas_id);
  rr_efs_prioritize_lte_meas_info_t* prioritize_lte_meas_info = rr_nv_get_prioritize_lte_meas_info(RR_GAS_ID_TO_AS_ID);
  rr_general_earfcn_list earfcn_list = {0};
  uint8             index;

  /* prioritize lte frequencies from received list of earfcn from LTE*/
  rr_general_add_lte_acq_db_earfcns(&earfcn_list, MAX_RR_EUTRAN_NEIGHBOUR_LIST_SIZE, scell_info_ptr->gsm.location_area_identification.plmn_id, gas_id);

  /* if earfcns in the list are less than the MAX_EARFCNs then append earfcn from lte ncell of scell*/
  rr_general_add_ncell_earfcns(&earfcn_list, MAX_RR_EUTRAN_NEIGHBOUR_LIST_SIZE, gas_id);

  scell_info_ptr->lte_neighbour_list.count = 0;

  // check and validate LTE frequencies
  if (earfcn_list.count > 0)
  {
    rr_lte_neighbour_list_entry_t * dst;
    for (index = 0; index < earfcn_list.count; index++)
    {
      dst = &scell_info_ptr->lte_neighbour_list.entries[index];
      dst->frequency.earfcn = earfcn_list.earfcns[index];
      dst->frequency.measurement_bandwidth = RR_PRIO_LTE_DEF_MEAS_BW;
      dst->priority = RR_PRIO_LTE_DEF_LTE_PRIO;
      dst->thresh_low = RR_PRIO_LTE_DEF_THRESH_LOW;
      dst->qrxlevmin = prioritize_lte_meas_info->qrxlevmin;
      dst->thresh_high = prioritize_lte_meas_info->thresh_high;
    }
    scell_info_ptr->lte_neighbour_list.count = earfcn_list.count;
    rr_restore_default_serv_utran_pseudo_si2q_params(scell_info_ptr, gas_id);
    scell_info_ptr->serving_cell_priority_params.geran_priority = RR_PRIO_LTE_DEF_GERAN_PRIO;
    scell_info_ptr->eutran_params.meas_params.qsearch_c = 0x07;
    if (rr_timer_is_running(RR_PRIORITIZE_LTE_ABORT_TIMER, gas_id) == FALSE)
    {
      MSG_GERAN_HIGH_1_G("Starting  RR_PRIORITIZE_LTE_ABORT_TIMER (timeout=%ds)", prioritize_lte_meas_info->prioritize_meas_timeout);
      (void)rr_timer_start(RR_PRIORITIZE_LTE_ABORT_TIMER, prioritize_lte_meas_info->prioritize_meas_timeout * 1000, gas_id);
    }
  }
}

void rr_general_stop_lte_prioritization(const gas_id_t gas_id)
{
  rr_general_data_t *rr_general_data = rr_general_get_data_ptr(gas_id);

  MSG_GERAN_HIGH_0_G("LTE prioritization stopped");
  if (rr_timer_is_running(RR_PRIORITIZE_LTE_DEFER_TIMER, gas_id))
  {
    rr_timer_cancel(RR_PRIORITIZE_LTE_DEFER_TIMER, gas_id);
  }
  if (rr_timer_is_running(RR_PRIORITIZE_LTE_ABORT_TIMER, gas_id))
  {
    rr_timer_force_timer_expiry(RR_PRIORITIZE_LTE_ABORT_TIMER, gas_id);
  }
  rr_general_data->prioritize_lte_defer_count = 0;
}

boolean rr_general_prioritize_lte_meas_enabled(const gas_id_t gas_id)
{
  sys_modem_as_id_e_type as_id = RR_GAS_ID_TO_AS_ID;

  if (!rr_nv_prioritize_lte_meas_enabled(as_id))
  {
    MSG_GERAN_HIGH_0_G("LTE frequencies will not be prioritized - EFS is disabled");
    return FALSE;
  }
  else if (!rr_is_lte_enabled(gas_id))
  {
    MSG_GERAN_HIGH_0_G("LTE frequencies will not be prioritized - lte is not enabled");
    return FALSE;
  }
  else if ((geran_get_multi_sim_mode() != SYS_MODEM_DEVICE_MODE_SINGLE_SIM) && (geran_get_current_dds_sub_id() != as_id))
  {
    MSG_GERAN_HIGH_2_G("LTE frequencies will not be prioritized - camped on nDDS SUB (multi_sim_modoe=%d, dds_sub_id=%d)",
      geran_get_multi_sim_mode(), geran_get_current_dds_sub_id());
    return FALSE;
  }
...
void rr_general_check_and_prioritize_lte_frequencies(const gas_id_t gas_id)
{
  rr_general_data_t *rr_general_data = rr_general_get_data_ptr(gas_id);
  rr_state_T        rr_state = get_rr_state(gas_id);
  byte              access_reason = rr_ce_access_reason(gas_id);
  rr_efs_prioritize_lte_meas_info_t* prioritize_lte_meas_info = rr_nv_get_prioritize_lte_meas_info(RR_GAS_ID_TO_AS_ID);

  if (!rr_general_prioritize_lte_meas_enabled(gas_id))
  {
    rr_general_data->prioritize_lte_defer_count = 0;
  }
  else if (rr_state == RR_DATA_TRANSFER && 
           (access_reason == MM_RR_MO_FULL_SPEECH || access_reason == MM_RR_MO_HALF_SPEECH ||
            access_reason == MM_RR_CALL_REEST || access_reason == MM_RR_EMERGENCY_CALL ||
            access_reason == MM_RR_ANSWER_TO_PAGING ))
  {
    MSG_GERAN_HIGH_1_G("LTE frequencies will not be prioritized - UE is in dedicated mode (access_reason=%d)", access_reason);
    rr_general_data->prioritize_lte_defer_count = 0;
  }
  else if (rr_timer_is_running(RR_PRIORITIZE_LTE_DEFER_TIMER, gas_id))
  {
    MSG_GERAN_HIGH_0_G("LTE prioritization already in queue - defer count reset to zero");
    rr_general_data->prioritize_lte_defer_count = 0;
  }
  else if (rr_timer_is_running(RR_PRIORITIZE_LTE_ABORT_TIMER, gas_id))
  {
    MSG_GERAN_HIGH_0_G("LTE prioritization already in effect - timer restarted");
    (void)rr_timer_restart(RR_PRIORITIZE_LTE_ABORT_TIMER, prioritize_lte_meas_info->prioritize_meas_timeout * 1000, gas_id);
    if ((rr_state == RR_GRR) && grr_is_idle_camped(gas_id))
    {
      rr_send_mph_wcdma_cell_update_list_req(rr_gprs_get_scell_info(gas_id), gas_id);
    }
  }
  else if ((rr_state == RR_GRR) && grr_is_idle_camped(gas_id))
  {
    rr_general_data->prioritize_lte_defer_count = 0;
    rr_general_prioritize_lte_frequencies(gas_id);
    rr_g2w_lte_measurements_delete(gas_id);
    rr_send_mph_wcdma_cell_update_list_req(rr_gprs_get_scell_info(gas_id), gas_id);
  }
  else
  {
    if (rr_general_data->prioritize_lte_defer_count >= prioritize_lte_meas_info->max_defer_count)
    {
      MSG_GERAN_HIGH_1("prioritize_lte_defer_count(%d) has reached max", rr_general_data->prioritize_lte_defer_count);
      rr_general_data->prioritize_lte_defer_count = 0;
    }
    else
    {
      rr_general_data->prioritize_lte_defer_count += 1;
      MSG_GERAN_HIGH_2("RR_PRIORITIZE_LTE_DEFER_TIMER timer started (defer timeout=%dms, defer count=%d)",
        prioritize_lte_meas_info->defer_timeout,
        rr_general_data->prioritize_lte_defer_count);
      (void)rr_timer_start(RR_PRIORITIZE_LTE_DEFER_TIMER, prioritize_lte_meas_info->defer_timeout,gas_id);
    }
  }
...
FUNCTION  rr_bcch_decode_all_freq_required

DESCRIPTION
  This function is called during bcch decode time, find all freq can be decoded or not.
...
===========================================================================*/
boolean rr_bcch_decode_all_freq_required(rr_pscan_results_db_T *pscan_db_ptr, const gas_id_t gas_id)
{
  word pscan_result_num_frequencies = rr_pscan_get_num_frequencies(pscan_db_ptr);
  
  if ( RR_PLMN_LIST_SEARCH == get_rr_state(gas_id) )
  {
    return FALSE;
  }
  
  if ( pscan_result_num_frequencies > RR_NUM_FREQ_MONITOR ) 
  {
    return TRUE;
  }
