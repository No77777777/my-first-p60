...
*/
/* $Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_x2g_cgi.c#1 $ */
              ...
              switch (message->message_header.message_id)
              {
                case RR_X2G_CGI_IMSG_CGI_REQ:
                {
                  ...
                  if (time_available > 0)
                  {
                    ...
                    rr_ms_set_procedure_duration(time_available, gas_id);
            ...
            case MPH_SELECT_SPECIFIC_BCCH_CNF:
            {
              if (message->mph.select_specific_bcch_cnf.BCCH_found)
              {
                    ...
                    rr_x2g_cgi_data_ptr->bsic.BS_colour_code == BS_colour_code)
                {
                  ...
                  rr_select_bcch_csi_params_init(&csi_params);
                  csi_params.dont_deact_pend_store_when_finished = TRUE;
