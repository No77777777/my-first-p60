...
*/
/* $Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_select_bcch.c#3 $ */
...
typedef struct
{
  ...
  uint16                            max_si_mask_for_l1;
  boolean                           init_params_valid;
  rr_select_bcch_init_params_t      init_params;
  boolean                           csi_params_valid;
  rr_select_bcch_csi_params_t       csi_params;
  uint16                            csi_mask;
...
)
{
   ...
   */
   ...
   pcell_info_ptr->gsm.complete_BA_list.BA_IND        = INVALID_BA_IND;
   pcell_info_ptr->gsm.SI2_BA_list.BA_IND             = INVALID_BA_IND;
   pcell_info_ptr->gsm.SI2bis_BA_list.BA_IND          = INVALID_BA_IND;
   pcell_info_ptr->gsm.SI2ter_BA_list.BA_IND          = INVALID_BA_IND;
...
)
{
  ...
  {
    data_ptr->early_camp_required = RR_SELECT_BCCH_EARLY_CAMP_SI3_ALWAYS;
  }
  else
  {
...
)
{
  ...
  // Limit the SI read requests to L1 to the "wait-for" mask
  data_ptr->max_si_mask_for_l1 = msg_ptr->wait_for_si_mask;
...
)
{
  ...
  if (msg_ptr->csi_params_valid)
  {
    MSG_GERAN_HIGH_1_G(
      "CSI params supplied: dont_deact_pend=%d",
      (int)msg_ptr->csi_params.dont_deact_pend_store_when_finished
    );

    data_ptr->csi_params = msg_ptr->csi_params;
    data_ptr->csi_params_valid = TRUE;
  }
  ...
  if ((msg_ptr->early_camp_required == RR_SELECT_BCCH_EARLY_CAMP_SI3) &&
      (wait_for_si_mask == MASK_SI_3))
  {
    data_ptr->early_camp_required = RR_SELECT_BCCH_EARLY_CAMP_SI3_ALWAYS;
  }
  ...
  // If Early Camping is going to be done immediately, then don't start the SI acquisition guard timer
  if ((msg_ptr->early_camp_required != RR_SELECT_BCCH_EARLY_CAMP_USE_PARAMS) &&
      (msg_ptr->max_allowed_time_ms > 0))
  {
    rr_select_bcch_start_sys_info_timer(gas_id);
  }
...
static void rr_select_bcch_finished(rr_select_bcch_fail_reason_e reason, const gas_id_t gas_id)
{
  ...
  // Inform Multi SIM Controller about stop of SI Acq procedure.
  rr_ms_sys_info_status_update(RR_SYS_INFO_ACQ_STOPPED, gas_id);
#endif /* FEATURE_DUAL_SIM || FEATURE_TRIPLE_SIM */
  }

  if (data_ptr->csi_params_valid &&
      (FALSE == data_ptr->csi_params.dont_deact_pend_store_when_finished))
  {
    rr_csi_deactivate_pend_store(gas_id);
  }

  //setting the status to RR_L1_EARLY_CAMP_NOT_REQUESTED. so that it does not hang on.
  data_ptr->early_camp_status = RR_L1_EARLY_CAMP_NOT_REQUESTED;
...
/*!
 * \brief Releases any stored SI messages in the Sys Info Cache
 *
 * \param gas_id
 *
 * \return uint16
...
static uint16 rr_select_bcch_csi_retrieve_all(const gas_id_t gas_id)
{
  ...
  data_ptr->csi_mask = rr_csi_retrieve_all_SIs(
    ...
    &data_ptr->init_params.lai,   // lai_ptr
    &data_ptr->init_params.ci,    // cell_id_ptr
    CSI_RETRIEVE_ALL_SIS,         // SIs_to_retrieve
    data_ptr->si_received_mask,   // exclude_si_mask
    gas_id                        // gas_id
  );
...
/*!
 * \brief Send MPH_READ_SYS_INFO_REQ to L1 with a new bitmask, if Iterative SI Acq is enabled
 *
 * \param si_received_mask
 * \param gas_id
 */
void rr_select_bcch_send_l1_read_sys_info_req(
  uint16 si_received_mask,
  uint16 max_si_mask,
  const gas_id_t gas_id
)
{
  ...
  {
    uint16 si_not_broadcast_mask = rr_si_messages_not_broadcast(rr_gprs_get_pcell_info(gas_id));
    ...
    remaining_si_mask &= (~si_not_broadcast_mask);

    // Now reduce this to the maximum SI mask allowed, if specified
    if (max_si_mask != MASK_SI_NONE)
    {
      remaining_si_mask &= max_si_mask;
    }

    MSG_GERAN_HIGH_3_G(
      "SI info: Received 0x%x; Not broadcast 0x%x; Required 0x%x",
      (int)(si_received_mask & (~si_not_broadcast_mask)),
      (int)si_not_broadcast_mask,
      (int)remaining_si_mask
    );
...
/*!
 * \brief Start Early Camping (via RR-L1-IDLE-MODE module)
 *
 * \param gas_id
 */
static void rr_select_bcch_start_early_camp(const gas_id_t gas_id)
{
  ...
  /**
   * If the SI acquisition guard timer is running, then this is suspended while GL1 transitions to IDLE. Once 
   * MPH_START_IDLE_MODE_CNF is received from GL1, the timer is resumed. 
   * So note the time remaining on the timer and store it, so it can be restarted later. 
   */
  if (rr_timer_is_running(RR_GSM_SYS_INFO_TIMER, gas_id))
  {
    data_ptr->time_remaining_ms = (uint32)rr_timer_get_time_remaining(RR_GSM_SYS_INFO_TIMER, gas_id);

    rr_timer_cancel(RR_GSM_SYS_INFO_TIMER, gas_id);
  }
...
/*!
 * \brief Start Early Camping (via RR-L1-IDLE-MODE module) using supplied Initial Parameters
 *
 * \param gas_id
 */
static void rr_select_bcch_start_early_camp_using_init_params(const gas_id_t gas_id)
{
  ...
  /** 
   * The SI acquisition guard timer won't have been started if Early Camping is done immediately. This timer 
   * will be started when MPH_START_IDLE_MODE_CNF is received from GL1, using the remaining acquisition time. 
   * As the timer hasn't been started yet, the remaining acquisition time is the maximum time-out value 
   * originally set. 
   */
  data_ptr->time_remaining_ms = data_ptr->max_allowed_time_ms;
...
/*!
 * \brief Reset function called if recovery restart occurs.
 * 
 * \param gas_id 
 */
void rr_select_bcch_recovery_init(const gas_id_t gas_id)
{
  rr_select_bcch_control_data_t *control_data_ptr;
            ...
            case RR_PSCAN_OMSG:
            {
              switch (msg_ptr->rr.pscan.omsg.header.imh.message_id)
              {
                case RR_PSCAN_OMSG_FINISHED_IND:
                {
                  ...
                  num_freqs_found = rr_pscan_process_power_scan_results(gas_id);
      ...
      switch (msg_ptr->message_header.message_set)
      {
            ...
            case RR_SELECT_BCCH_IMSG:
            {
              switch (msg_ptr->rr.select_bcch.imsg.header.imh.message_id)
              {
                case RR_SELECT_BCCH_IMSG_BSI_RECEIVED_IND:
                {
                  ...
                  {
                    rr_select_bcch_fail_reason_e fail_reason = RR_SELECT_BCCH_FAIL_REASON_NONE;

                    // A sys info message has been received (from a BCCH read or the sys info cache)

                    if ((imsg_ptr->si_received == SI_TYPE_SI_3) ||
                        (imsg_ptr->si_received == SI_TYPE_SI_4))
                    {
                      // If a function was supplied to check cell suitability, call it now
                      if (control_data_ptr->data_ptr->cell_suitable_cb_fn != NULL)
                      {
                        boolean cell_suitable = data_ptr->cell_suitable_cb_fn(
                        ...
                        if (!cell_suitable)
                        {
                          // Procedure will finish as cell is not suitable
                          fail_reason = RR_SELECT_BCCH_FAIL_REASON_CELL_NOT_SUITABLE;
                        }
                      }
                    }

                    if (fail_reason == RR_SELECT_BCCH_FAIL_REASON_NONE)
                    {
                      // Cell is suitable (or assumed to be)
                      ...
                      /**
                       * If SI3 has been received and Early Camping is required but has not been done
                        ...
                        if (data_ptr->csi_mask == MASK_SI_NONE)
                        {
                          ...
                          if (RR_L1_EARLY_CAMP_CONFIRMED == data_ptr->early_camp_status)
                          {
                            ...
                            rr_select_bcch_send_l1_read_sys_info_req(
                              ...
                              data_ptr->max_si_mask_for_l1,   // max_si_mask
                              gas_id                          // gas_id
                            );
                          }
                        }
                    ...
                    else
                    {
                      ...
                      rr_select_bcch_finished(fail_reason, gas_id);
                      control_data_ptr->state = RR_SELECT_BCCH_STATE_NULL;
                    }
                  }
                  break;
                ...
                case RR_SELECT_BCCH_IMSG_ABORT_REQ:
                {
                  ...
                  //check RRL1 idle state is early camp, just to make sure everythig is in sync
                  if ( (RR_L1_EARLY_CAMP_REQUESTED == data_ptr->early_camp_status ) && ( rr_l1_is_early_camp( gas_id) == TRUE ) )
                  {
            ...
            case RR_L1_IDLE_OMSG:
            {
              switch (msg_ptr->rr.header.imh.message_id)
              {
                case RR_L1_IDLE_OMSG_START_EARLY_CAMP_CNF:
                {
                  ...
                  if (data_ptr->time_remaining_ms > 0)
                  {
                    MSG_GERAN_MED_1_G("Restarting RR_GSM_SYS_INFO_TIMER (%dms)",(int)data_ptr->time_remaining_ms);

                    rr_timer_start(RR_GSM_SYS_INFO_TIMER, (rr_duration_t)data_ptr->time_remaining_ms, gas_id);
                  }
                  ...
                  if (RR_L1_EARLY_CAMP_REQUESTED == data_ptr->early_camp_status)
                  {
                    ...
                    else
                    {
                      ...
                      data_ptr->csi_mask = rr_select_bcch_csi_retrieve_all(gas_id);
        ...
        case MS_TIMER:
        {
          ...
          switch (timer_id)
          {
            ...
            {
              if ((timer_id == CELL_RESELECTION_TIMER) &&
                  (MASK_SI_NONE != rr_sys_info_timeout_due_to_SI2bis_SI2ter_SI13_not_read(gas_id))) 
              {
                rr_act_on_for_SI_nt_rcvd(rr_sys_info_timeout_due_to_SI2bis_SI2ter_SI13_not_read(gas_id), gas_id);
                // Procedure finished successfully
                rr_select_bcch_finished(RR_SELECT_BCCH_FAIL_REASON_NONE, gas_id);
                control_data_ptr->state = RR_SELECT_BCCH_STATE_NULL;    
                break;
...
#ifdef FEATURE_QSH_MDUMP
QSH_MDUMP_FN_ATTR
void grr_mdump_rr_select_bcch(const gas_id_t gas_id)
{
  rr_select_bcch_control_data_t *control_data_ptr = rr_select_bcch_get_control_data_ptr(gas_id);
  qsh_mdump_collect_high(control_data_ptr, sizeof(rr_select_bcch_control_data_t));     
}
