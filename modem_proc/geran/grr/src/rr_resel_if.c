...
============================================================================*/
/* $Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_resel_if.c#1 $ */
...
)
{
  switch (imsg)
  {
    ...
    case RR_RESEL_IMSG_IRAT_ALLOWED_UPDATED_IND:
      return "RR_RESEL_IMSG_IRAT_ALLOWED_UPDATED_IND";
...
)
{
  ...
  {
    if (cause != RR_RESEL_CAUSE_RPS )
    {
      MSG_GERAN_HIGH_1_G("MS is locked to channel, change the current cause = %d to RR_RESEL_CAUSE_RPS",cause);  
      cause = RR_RESEL_CAUSE_RPS;
    }
  }
#endif /*FEATURE_LOCK_ARFCN*/
