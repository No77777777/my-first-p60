...
============================================================================*/
/* $Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_mode.c#4 $ */
    ...
    switch (rr_mode_data_ptr->state)
    {
      case RR_MODE_INACTIVE:
      {
        switch (event)
        {
          ...
          {
            rr_send_mph_start_gsm_mode_req(FALSE, FALSE, FALSE, gas_id);
          ...
          {
            rr_send_mph_start_gsm_mode_req(TRUE, FALSE, FALSE, gas_id);
            rr_mode_data_ptr->stop_reason = RR_L1_GSM_STOP_REASON_UNDEFINED;
            rr_mode_data_ptr->state = RR_MODE_GOING_ACTIVE;
            break;
          }
          ...
          {
            rr_send_mph_start_gsm_mode_req(FALSE, TRUE, FALSE, gas_id);
            rr_mode_data_ptr->stop_reason = RR_L1_GSM_STOP_REASON_UNDEFINED;
            rr_mode_data_ptr->state = RR_MODE_GOING_ACTIVE;
            break;
          }

          case EV_ACTIVATE_GSM_MODE_FOR_X2G_HO: 		 
          { 
            rr_send_mph_start_gsm_mode_req(FALSE, FALSE, TRUE, gas_id);
            rr_mode_data_ptr->stop_reason = RR_L1_GSM_STOP_REASON_UNDEFINED;			  
            rr_mode_data_ptr->state = RR_MODE_GOING_ACTIVE;			 
            break;		  
          }
      ...
      case RR_MODE_GOING_ACTIVE:
      {
        switch (event)
        {
              ...
              case MS_RR_L1:
              {
                switch ((rr_l1_message_id_enum_T) message_id)
                {
                  case MPH_START_GSM_MODE_CNF:
                    ...
                    geran_stack_status(TRM_PROTOCOL_STACK_ACTIVE, gas_id);
                    ...
                    {
                      rr_general_send_lte_rrc_acq_db_earfcn_req(gas_id);
                    }
          ...
          case EV_ACTIVATE_GSM_MODE:
          case EV_DEACTIVATE_GSM_MODE:
          case EV_DEACTIVATE_GSM_MODE_FOR_X2G_HO_FAIL:
          {
              ...
              case MS_RR_L1:
              {
                switch (message_id)
                {
                  case MPH_STOP_GSM_MODE_CNF:
                  {
                    ...
                    geran_stack_status(TRM_PROTOCOL_STACK_INACTIVE, gas_id);
...
#ifdef FEATURE_QSH_MDUMP
QSH_MDUMP_FN_ATTR
void grr_mdump_rr_mode(const gas_id_t gas_id)
{
  rr_mode_data_t *rr_mode_data_ptr = rr_mode_get_data_ptr(gas_id);
  qsh_mdump_collect_high(rr_mode_data_ptr, sizeof(rr_mode_data_t));
}
