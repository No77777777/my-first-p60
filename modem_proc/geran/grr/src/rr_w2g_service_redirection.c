...
$Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_w2g_service_redirection.c#2 $
...
#define log_unexpected_state(st, gas_id) \
...
#else
#define log_unexpected_state(st, gas_id) \
  MSG_GERAN_ERROR_1_G("Unexpected state %d", st);
...
typedef enum
{
  RR_X2G_CALL_EST_CAUSE_UNDEFINED,
  RR_X2G_CALL_EST_CAUSE_MO_CALL,
  RR_X2G_CALL_EST_CAUSE_MT_CALL
} rr_x2g_call_est_cause_e;
...
typedef struct
{
  ...
  rr_x2g_call_est_cause_e        rr_x2g_call_est_cause;
...
)
{
  ...
  rr_x2g_redir_data_ptr->rr_x2g_call_est_cause = RR_X2G_CALL_EST_CAUSE_UNDEFINED;
...
#endif /* FEATURE_WCDMA && FEATURE_LTE */
#ifdef FEATURE_LTE
  rr_reset_csfb_call_status_ind(gas_id);
#endif /* FEATURE_LTE */
}
...
)
{
  ...
  MSG_GERAN_MED_1_G("process_tds_rr_interrat_redirect_req(), type: %d", req->cell_srch_reason);
  MSG_GERAN_MED_3_G(" network_select_mode: %d, is_emergency_call: %d,  call_est_cause %d",
  ...
  if (TDSRR_CAUSE_MO_CS_CALL == req->call_est_cause)
  {
    rr_x2g_redir_data_ptr->rr_x2g_call_est_cause = RR_X2G_CALL_EST_CAUSE_MO_CALL;
    rr_ms_redirected_for_cs_call(gas_id);
  }
  else if (TDSRR_CAUSE_MT_CS_CALL == req->call_est_cause)
  {
    rr_x2g_redir_data_ptr->rr_x2g_call_est_cause = RR_X2G_CALL_EST_CAUSE_MT_CALL;
  }
  else
  {
    rr_x2g_redir_data_ptr->rr_x2g_call_est_cause = RR_X2G_CALL_EST_CAUSE_UNDEFINED;
...
)
{
  ...
  MSG_GERAN_MED_1_G("process_rr_interrat_redirect_req(), type %d", req->cell_srch_reason);
  MSG_GERAN_MED_3_G("network_select_mode: %d, is_emergency_call: %d, call_est_cause = %d",
    req->network_select_mode,
    req->is_emergency_call,
    req->call_est_cause);
  ...
  rr_x2g_redir_data_ptr->is_emergency_call = req->is_emergency_call;
  if (RRC_RR_CAUSE_MO_CS_CALL == req->call_est_cause )
  {
    rr_x2g_redir_data_ptr->rr_x2g_call_est_cause = RR_X2G_CALL_EST_CAUSE_MO_CALL;
    rr_ms_redirected_for_cs_call(gas_id);  
  }
  else if (RRC_RR_CAUSE_MT_CS_CALL == req->call_est_cause)
  {
    rr_x2g_redir_data_ptr->rr_x2g_call_est_cause = RR_X2G_CALL_EST_CAUSE_MT_CALL;
...
#ifdef FEATURE_LTE
  redir = rr_g2l_blind_redir_after_voice_ss_or_periodic_lau_enabled(gas_id);
  redir_after_csfb = rr_g2l_blind_redir_after_csfb_enabled(gas_id);

  MSG_GERAN_HIGH_3_G("G2L blind redir NV: after_voice_ss_or_periodic_lau=%d, after_CSFB=%d, csfb_call_status_ind=%d",
    redir, redir_after_csfb, req->csfb_call_status_ind);
  if(req->csfb_call_status_ind)
  {
    if (redir_after_csfb)
    {
    ...
    rr_set_csfb_call_status_ind(gas_id);
  }
#endif /* FEATURE_LTE */
...
)
{
  ...
  if (lte_redir_req->redir_info.num_of_freq > 0)
  {
    ...
    (void)rr_log_x2g_redir_gsm_cell_list(rr_w2g_redir_cell_list_ptr, gas_id);
#ifdef FEATURE_GERAN_ADD_LAST_SCELL_CSFB_OPT
    /*Add the last camped GERAN scell into the redirection list, perform these checks first:
       1. Check if the PLMN matches with whitelist
       
       2. Check the last camped LAI matches the LAI that LTE is in. (If LAI is not available from
           LTE do NOT add the last camped cell.)
           
       3. Ensure the last camped GERAN scell isn't already in the list*/
    /*Add the last camped cell after all of the redirection list cells have been processed
          and there is still room in the pscan list*/
          
    if ((rr_nv_add_last_serv_cell_csfb_is_plmn_match(rr_PLMN_request_details_ptr->requested_PLMN_id, gas_id)) &&
        (rr_is_csfb_call_in_progress(gas_id))
       )
    {
      if (rr_w2g_redir_cell_list_ptr->no_of_entries < ARR_SIZE(rr_w2g_redir_cell_list_ptr->cell_list))
      {
        eng_mode_scell_info_t scell_info;
        sys_modem_as_id_e_type as_id = RR_GAS_ID_TO_AS_ID;
 
        /*get the last camped GERAN scell from the engineering mode store*/
        geran_multisim_eng_mode_data_read(ENG_MODE_SCELL_INFO_PERSISTENT,
                                          (void*) (&scell_info), 
                                          as_id);

        if (scell_info.cell_id != GERAN_ENG_MODE_INVALID_VALUE_OF_PARAMETER)
        {
          inter_task_lai_T lte_lai;
 
          if (mm_get_updated_lai(&lte_lai, as_id))
          {
            sys_mcc_type target_mcc = 0;
            sys_mnc_type target_mnc = 0;
            sys_mcc_type last_gscell_mcc = 0;
            sys_mnc_type last_gscell_mnc = 0;
            sys_plmn_id_s_type scell_info_plmn;
            ARFCN_T      scell_info_arfcn;
            boolean plmn_ok = FALSE;
            boolean lac_ok = FALSE;
 
            scell_info_plmn.identity[0] = scell_info.plmn.plmn_id[0];
            scell_info_plmn.identity[1] = scell_info.plmn.plmn_id[1];
            scell_info_plmn.identity[2] = scell_info.plmn.plmn_id[2];
            scell_info_arfcn.num = scell_info.arfcn;
            scell_info_arfcn.band = geran_eng_mode_eng_band_to_sys_band(scell_info.band);

            /*check PLMN's match*/
            /* Check if the target cell's PLMN matches our requested PLMN. */
            plmn_ok = sys_plmn_match(
                         scell_info_plmn,
                         lte_lai.PLMN_id
                         );
            ...
            /* Check if both the target cell's PLMN and our requested PLMN are in
              ...
              plmn_ok = rr_eplmn_list_equivalent_plmn(scell_info_plmn, gas_id) &&
                        rr_eplmn_list_equivalent_plmn(lte_lai.PLMN_id, gas_id);
            }
            ...
            MSG_GERAN_HIGH_5_G( "last scell PLMN:%d-%d, LTE PLMN:%d-%d match:%d", 
                                last_gscell_mcc,
                                last_gscell_mnc,
                                target_mcc,
                                target_mnc,
            ...
            if (plmn_ok)
            {
              uint16 lac = (uint16) ((lte_lai.location_area_code[0] * 256) + (lte_lai.location_area_code[1]));

              if (scell_info.lac == lac)
              {
                lac_ok = TRUE;
              }
            }

            MSG_GERAN_HIGH_4_G( "last scell LAC:0x%x, LTE LAC:0x%x%x match:%d", scell_info.lac,
                                                                                lte_lai.location_area_code[0],
                                                                                lte_lai.location_area_code[1],
                                                                                lac_ok);

            if (plmn_ok && lac_ok && (FALSE == rr_is_gsm_cell_in_depriotized_list(scell_info_arfcn,gas_id)))
            {
              boolean unique_arfcn = TRUE;

              /* check that the last G serving cell is not already in the pscan list*/
              for (j = 0; j < rr_w2g_redir_cell_list_ptr->no_of_entries && j < RR_MAX_RES_PSCAN_CELL_LIST_SIZE;++j)
              {
                if (ARFCNS_EQUAL(scell_info_arfcn, rr_w2g_redir_cell_list_ptr->cell_list[j]))
                {
                  unique_arfcn = FALSE;
              ...
              if (unique_arfcn)
              {
                MSG_GERAN_HIGH_1_G( "Add last camped GERAN ARFCN %d to redirection list", scell_info_arfcn.num );
                                     rr_w2g_redir_cell_list_ptr->cell_list[rr_w2g_redir_cell_list_ptr->no_of_entries] = scell_info_arfcn;
                                     ++rr_w2g_redir_cell_list_ptr->no_of_entries;
              }
              else
              {
                MSG_GERAN_HIGH_1_G( "Last camped GERAN ARFCN %d already in redirection list", scell_info_arfcn.num );
              }
              (void)rr_log_x2g_redir_gsm_cell_list(rr_w2g_redir_cell_list_ptr, gas_id);
            }
            else
            {
              MSG_GERAN_HIGH_1_G( "Last camped GERAN ARFCN %d is in either depriotized list or plmn/lac not ok", scell_info_arfcn.num );
            }
          }
          else
          {
            MSG_GERAN_HIGH_0_G( "LAI not avaiable from LTE, last camped cell not added" );
          }
        }
        else
        {
          MSG_GERAN_HIGH_0_G( "Last camped GERAN cell is not valid");
        }
      }
      else
      {
        MSG_GERAN_HIGH_1_G("Can't add last scell redirection list full, no entries:%d", rr_w2g_redir_cell_list_ptr->no_of_entries);
      }
    }
#endif /*FEATURE_GERAN_ADD_LAST_SCELL_CSFB_OPT*/

    /* make sure redir list has more than one freq's */
    if (rr_w2g_redir_cell_list_ptr->no_of_entries > 0)
    {
      rr_w2g_redir_cell_list_ptr->valid_data = TRUE;
    }
  }
...
)
{
  ...
  if (rr_is_forbidden_LAI(&pcell_info_ptr->gsm.location_area_identification, gas_id))
  {
    rr_mm_send_resel_fail_ind(&pcell_info_ptr->gsm.location_area_identification, gas_id);
  }
  ...
  {
    MSG_GERAN_HIGH_0_G("Cell un-suitable , PLMN mismatch or forbidden");
    return RR_EV_W2G_NO_SUITABLE_CELL_FOUND;
  }
    ...
    switch (rr_x2g_redir_data_ptr->state)
    {
        ...
        else if (event == EV_NO_BCCH_FOUND)
        {
          ...
          if (rr_x2g_redir_data_ptr->type == RR_X2G_REDIR_TYPE_REDIRECTION_AFTER_CONNECTION_RELEASE)
          {
            if (rr_w2g_redir_cell_list_ptr->valid_data)
            {
              ...
              rr_w2g_redir_cell_list_ptr->valid_data = FALSE;
              for (index = 0;index < rr_x2g_redir_data_ptr->cell_info_list_count && index < RR_X2G_REDIR_CELL_INFO_LIST_MAX_SIZE; ++index)
              {
                rr_x2g_redir_cell_info_free(rr_x2g_redir_data_ptr->cell_info_list[index]);
              }
              rr_x2g_redir_data_ptr->cell_info_list_count = 0;
              new_event = EV_TRIGGER_CELL_SELECTION;
            }
      ...
      case STATE_PERFORMING_CELL_SELECTION_ABORTED_BY_RRC:
      {
        if (event == EV_CELL_SELECTION_ABORTED)
        {
          ...
          /* advise the multi-SIM controller of this failure */
          rr_ms_x2g_procedure_failure(RR_X2G_REDIRECTION_ABORT_BY_RRC, gas_id);
          new_event = rr_mode_control(EV_DEACTIVATE_GSM_MODE, NULL, gas_id);
          rr_x2g_redir_data_ptr->state = STATE_GOING_INACTIVE_DUE_TO_RRC;
        }
#ifdef FEATURE_PCS_850_FREQ_SCAN_OPTIMIZATION
      ...
      default:
      {
        log_unexpected_state(rr_x2g_redir_data_ptr->state, gas_id)
      }
    }
...
/*!
 * \brief Returns TRUR or FALSE depending on whether  redirection from X RAT  is ongoing on redirected cell list.
 *
 * \param gas_id
 *
 * \return TRUE/FALSE
 */
boolean rr_x2g_redirection_in_progress_on_redirected_cell(
  const gas_id_t gas_id
)
{
  rr_reselection_pscan_cell_list_T *rr_w2g_redir_cell_list_ptr;
  ...
  if (rr_x2g_redir_data_ptr != NULL)
  {
    if ((rr_x2g_redir_data_ptr->state != STATE_NULL) &&
        (rr_x2g_redir_data_ptr->type  == RR_X2G_REDIR_TYPE_REDIRECTION_AFTER_CONNECTION_RELEASE) &&
        (rr_w2g_redir_cell_list_ptr->valid_data == TRUE)
       )
    {
...
#ifdef FEATURE_QSH_MDUMP
QSH_MDUMP_FN_ATTR
void grr_mdump_rr_x2g_redir(const gas_id_t gas_id)
{
  rr_x2g_redir_data_t *rr_x2g_redir_data_ptr = rr_x2g_redir_get_data_ptr(gas_id);
  qsh_mdump_collect_high(rr_x2g_redir_data_ptr, sizeof(rr_x2g_redir_data_t));
}
#endif /* FEATURE_QSH_MDUMP */

#ifdef FEATURE_LTE_TO_GSM_REDIR_WITH_SI_SUPPORT
boolean rr_x2g_redirection_cell_has_mulitple_bsics(rr_candidate_cell_T *candidate, const gas_id_t gas_id)
{
  byte   i;
  byte   count = 0;
  rr_x2g_redir_data_t *rr_x2g_redir_data_ptr = rr_x2g_redir_get_data_ptr(gas_id);

  if (TRUE == rr_x2g_redir_with_si_in_progress(gas_id))
  {
    for (i = 0; (i < rr_x2g_redir_data_ptr->cell_info_list_count) && (i < RR_X2G_REDIR_CELL_INFO_LIST_MAX_SIZE); ++i)
    {
      if (ARFCNS_EQUAL(rr_x2g_redir_data_ptr->cell_info_list[i]->arfcn, candidate->meas.arfcn))
      {
        count++;
        if (count > 1)
        {
          MSG_GERAN_HIGH_1_G("L2G Redirection: Multiple Bsic's are received for Cell = %d", candidate->meas.arfcn.num);
          return TRUE; 
        }
      }
    }
