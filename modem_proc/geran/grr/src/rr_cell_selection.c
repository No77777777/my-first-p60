...
$Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_cell_selection.c#5 $
...
typedef enum
{
  ...
  RR_SELECTION_ABORT
#ifdef FEATURE_LOCK_ARFCN
  ,RR_SELECTION_WAIT_FOR_LOCKING_EVENT
#endif /*FEATURE_LOCK_ARFCN*/
} cell_select_state_T;
...
typedef struct
{
 pscan_completion_cb_fn func_ptr;
} pscan_complete_cb_t;
...
typedef struct
{
  ...
  boolean  ue_locked;
  pscan_complete_cb_t pscan_complete_cb;
  uint16 time_left;

} rr_channel_lock_info_t;

#endif /*FEATURE_LOCK_ARFCN*/

typedef enum
{
  RR_NO_VALID_CELLS_FOUND,
  RR_NO_SUITABLE_CELL_FOUND,
  RR_SUITABLE_CELL_FOUND,
  RR_CELL_SEARCH_RESULT_MAX
} rr_cell_search_result_e_t;

typedef enum
{
  RR_SEL_FAIL_REASON_NONE,
  RR_SEL_FAIL_REASON_SI_TIMEOUT,
  RR_SEL_FAIL_REASON_CELL_BARRED,
  RR_SEL_FAIL_REASON_BCCH_NOT_FOUND,
  RR_SEL_FAIL_REASON_ABORTED,
  RR_SEL_FAIL_REASON_CELL_NOT_SUITABLE,
  RR_SEL_FAIL_REASON_MAX
} rr_cell_select_fail_reason_e_t;
...
#ifdef FEATURE_LOCK_ARFCN
static rr_channel_lock_info_t rr_channel_lock_info[NUM_GERAN_DATA_SPACES];
#endif /*FEATURE_LOCK_ARFCN*/
...
QSH_MDUMP_FN_ATTR
#endif
static rr_cell_selection_data_t *rr_cell_selection_get_data_ptr(const gas_id_t gas_id)
{
...
#ifdef FEATURE_LOCK_ARFCN
/**
 * Get the channle lock info pointer
 */
static rr_channel_lock_info_t *rr_channel_lock_info_get_data_ptr(const gas_id_t gas_id)
{
  uint8 as_index = GERAN_MAP_GAS_ID_TO_DATA_SPACE_INDEX(gas_id);

  return &rr_channel_lock_info[as_index];
}
#endif /*FEATURE_LOCK_ARFCN*/
...
static char * rr_cell_selection_state_name(cell_select_state_T state)
{
  ...
  switch (state)
  {
    ...
    case RR_SELECTION_WAIT_FOR_SYS_INFO_RR_SELECT_BCCH:
      s = "WAIT_FOR_SYS_INFO_RR_SELECT_BCCH";
      break;
    ...
    case RR_SELECTION_WAIT_FOR_LOCKING_EVENT:
      s = "WAIT_FOR_LOCKING_EVENT";
      break;
#endif /*FEATURE_LOCK_ARFCN*/
...
)
{
  if ((rr_PLMN_request_details_ptr != NULL) &&
      ...
      (rr_PLMN_request_details_ptr->scan_scope == SYS_SCAN_SCOPE_ACQ_DB))
  {
    return TRUE;
  }
...
)
{
  ...
  {
    /* do nothing, WCDMA-RRC does not need to be informed */
    return;
  }
...
)
{
  ...
  tdsrrc_update_band_preference_sub_id(band_pref_mask, geran_map_gas_id_to_nas_id(gas_id));
#else
 ...
 */
boolean rr_cell_selection_get_ue_oos_status(const gas_id_t gas_id)
{
  // Obtain a pointer to the module data
  return rr_cell_selection_get_data_ptr(gas_id)->rr_cell_selection_details.ue_oos;
}

/**
 * Sets the ue_oos flag for use in RR_PLMN_SELECT_CNF.
 */
void rr_cell_selection_set_ue_oos_status(boolean status, const gas_id_t gas_id)
{
  // Obtain a pointer to the module data
  rr_cell_selection_get_data_ptr(gas_id)->rr_cell_selection_details.ue_oos = status;
}

/**
 * Clears PLMN camping list
 */
static void rr_clear_plmn_list(const gas_id_t gas_id)
{
  // Obtain a pointer to the module data
  rr_cell_selection_data_t *data_ptr = rr_cell_selection_get_data_ptr(gas_id);

  data_ptr->rr_plmn_camping_list.high_priority_array.num_frequencies = 0;
  data_ptr->rr_plmn_camping_list.low_priority_array.num_frequencies = 0;
  data_ptr->rr_plmn_camping_list.tried_high_priority_cells = 0;
  data_ptr->rr_plmn_camping_list.tried_low_priority_cells = 0;
  data_ptr->rr_plmn_camping_list.valid = FALSE;
} /* rr_clear_plmn_list */
...
)
{
  ...
  rr_cell_selection_details_T *data_ptr = &rr_cell_selection_data_ptr->rr_cell_selection_details;

  data_ptr->bcch_list_cnf_received = FALSE;
  data_ptr->early_camping_done = FALSE;
  data_ptr->mm_abort = FALSE;
  data_ptr->cell_selection_abort_timeout = FALSE;
  data_ptr->abort_after_power_scan_cnf = FALSE;
#ifdef FEATURE_INTERRAT_REDIRECTION_WTOG
  data_ptr->abort_reason = EV_NO_EVENT;
#ifdef FEATURE_PCS_850_FREQ_SCAN_OPTIMIZATION
  data_ptr->suitable_cell_found = FALSE;
#endif /* FEATURE_PCS_850_FREQ_SCAN_OPTIMIZATION */
#endif /* FEATURE_INTERRAT_REDIRECTION_WTOG */
  data_ptr->ue_oos = TRUE;
  data_ptr->retry_required = FALSE;
  data_ptr->cell_selection_for_plmn_select_req = FALSE;
  data_ptr->denied_bands = SYS_BAND_MASK_EMPTY;
  data_ptr->acq_db_search = FALSE;
  data_ptr->acq_db_pref = RR_ACQ_DB_PREF_ALLOWED;
 ...
 */
static void rr_cleanup_cell_selection(const gas_id_t gas_id)
{
 ...
 */
static void rr_restart_cell_selection(rr_candidate_cell_db_T *candidate_db_ptr, const gas_id_t gas_id)
{
  boolean retry_in_progress;
  ...
  retry_in_progress = data_ptr->rr_cell_selection_details.retry_in_progress;
  ...
  data_ptr->rr_cell_selection_details.retry_in_progress = retry_in_progress;
 ...
 */
void rr_store_plmn_id(PLMN_id_T *PLMN_id, const gas_id_t gas_id)
{
 ...
 */
void rr_set_equivalent_plmn(PLMN_id_T *PLMN_id, const gas_id_t gas_id)
{
  rr_get_PLMN_request_details_ptr(gas_id)->equivalent_PLMN_id = (* PLMN_id);
 ...
 */
boolean rr_equivalent_plmn_is_undefined(const gas_id_t gas_id)
{
  return (sys_plmn_id_is_undefined(rr_get_PLMN_request_details_ptr(gas_id)->equivalent_PLMN_id));

} /* end rr_equivalent_plmn_is_undefined() */

// Returns the current service domain
sys_srv_domain_e_type rr_sel_get_service_domain(const gas_id_t gas_id)
{
  return rr_get_PLMN_request_details_ptr(gas_id)->service_domain;
}

byte rr_cell_selection_get_decode_bcch_retry_status(const gas_id_t gas_id)
{
  // Obtain a pointer to the module data
  rr_cell_selection_data_t *rr_cell_selection_data_ptr = rr_cell_selection_get_data_ptr(gas_id);
...
void rr_cell_selection_clear_decode_bcch_retry_status(const gas_id_t gas_id)
{
  rr_cell_selection_get_data_ptr(gas_id)->rr_cell_selection_details.retry_in_progress = FALSE;
}
...
/*!
 * \brief Stores the ACQ-DB pref.
 * 
 * \param acq_db_pref (in)
 * \param gas_id (in)
 */
static void set_acq_db_pref(rr_acq_db_pref_e_t acq_db_pref, const gas_id_t gas_id)
{
  rr_cell_selection_get_data_ptr(gas_id)->rr_cell_selection_details.acq_db_pref = acq_db_pref;
}

/*!
 * \brief Indicates if the current search is ACQ-DB only.
 * 
 * \param gas_id (in)
 * 
 * \return boolean - 
 */
static boolean search_is_acq_db_only(const gas_id_t gas_id)
{
  if (RR_ACQ_DB_PREF_ONLY == rr_cell_selection_get_data_ptr(gas_id)->rr_cell_selection_details.acq_db_pref)
  {
    return TRUE;
  }
...
)
{
  ...
  boolean search_by_band_allowed = TRUE;
#else
  boolean search_by_band_allowed = FALSE;
  ...
  if (rr_w2g_redir_cell_list_ptr->valid_data)
  {
    ...
    (void) rr_find_cell_new_search(
      RR_ACQ_DB_PREF_NOT_ALLOWED,    // acq_db_pref
      rr_band_pref(gas_id),          // band_pref
      search_by_band_allowed,        // search_by_band_allowed
      candidate_db_ptr,              // candidate_db_ptr
      gas_id                         // gas_id
    );
  ...
  {
    rr_find_cell_pscan_params_t pscan_params;

    pscan_params = rr_find_cell_new_search(
      RR_ACQ_DB_PREF_ALLOWED,        // acq_db_pref
      rr_band_pref(gas_id),          // band_pref
      search_by_band_allowed,        // search_by_band_allowed
      candidate_db_ptr,              // candidate_db_ptr
      gas_id                         // gas_id
    ...
    if (pscan_params.acq_db_search)
    {
      ...
      {
        (void) rr_find_cell_no_freq_found(gas_id);
 ...
 */
static rr_event_T rr_selection_start_cell_selection(rr_acq_db_pref_e_t acq_db_pref, const gas_id_t gas_id)
{
  ...
  if (rr_pscan_data_capture_enabled(gas_id))
  {
    ...
    acq_db_pref = RR_ACQ_DB_PREF_NOT_ALLOWED;
  }

  set_acq_db_pref(acq_db_pref, gas_id);
  ...
  {
    return rr_perform_w2g_redirection_power_scan(candidate_db_ptr, acq_db_pref, gas_id);
  }
  else
#endif /* FEATURE_INTERRAT_REDIRECTION_WTOG */
  {
    ...
    rr_find_cell_action_e action = RR_FIND_CELL_ACTION_PSCAN;

    sys_band_mask_type band_pref = rr_search_band_pref(gas_id);

    /* Invalidate PSCAN data only if new requested PLMN is different than last PLMN  and
       Last pscan was ACQ-DB (contains only requested PLMN) */
    if (rr_pscan_get_last_acq_db_freqs_valid(gas_id))
    {
      if (rr_pscan_get_last_acq_db_for_requested_plmn_only(gas_id) &&
          (!sys_plmn_match(
             rr_pscan_get_last_acq_db_plmn_id(gas_id),
             rr_PLMN_request_details_ptr->requested_PLMN_id))
         )
      {
        PLMN_id_T last_acq_db_plmn = rr_pscan_get_last_acq_db_plmn_id(gas_id);
        MSG_GERAN_MED_1_G("Last pscan ACQ_DB valid, PSCAN results belong to PLMN Id:=%6x",
         ((last_acq_db_plmn.identity[0] << 16)+ \
          (last_acq_db_plmn.identity[1] << 8) + \
           last_acq_db_plmn.identity[2] )
        );
    ...
    {
      acq_db_pref = RR_ACQ_DB_PREF_ONLY;
    }
    /* If last PSCAN results had some ARFCNs filtered out, start with ACQ DB scan. If not found service,
    we would do FULL scan next and so we would not miss to scan any ARFCN. */
    else if ((FALSE == rr_pscan_is_filtered_pscan(gas_id) &&
              rr_pscan_results_are_reusable(band_pref, gas_id) && 
             !rr_pscan_results_are_acq_db(gas_id))
            )
    {
      /*This means that last PSCAN was done on complete band and hence this should be not be 
      repeated if GRR is using the existing PSCAN */    
      acq_db_pref = RR_ACQ_DB_PREF_NOT_ALLOWED;
    }

    pscan_params = rr_find_cell_new_search(
      acq_db_pref,          // acq_db_pref
      band_pref,            // band_pref
      TRUE,                 // search_by_band_allowed
      candidate_db_ptr,     // candidate_db_ptr
      gas_id                // gas_id
    );
    ...
    if (pscan_params.acq_db_search)
    {
      ...
      if (rr_is_any_cell_selection_mode(gas_id) ||
          (rr_sel_is_scan_scope_acq_db(rr_PLMN_request_details_ptr) &&
          (!rr_is_manual_plmn_selection_mode(gas_id))))
      {
      ...
      if (output_event == EV_NO_BCCH_FOUND)
      {
        action = rr_find_cell_no_freq_found(gas_id);

        if (RR_FIND_CELL_ACTION_NONE == action)
        {
      ...
      {
        action = RR_FIND_CELL_ACTION_NONE;
      }
    }

    if (RR_FIND_CELL_ACTION_PSCAN == action)
    {
      rr_find_cell_pscan_params_t pscan_params = rr_find_cell_get_pscan_params(gas_id);

      output_event = rr_perform_power_scan(
        RR_SEARCH_NORMAL,                  // search_mode
        pscan_params.band_mask,            // band_pref
        pscan_params.filter_acq_db_freqs,  // filter_acq_db_freqs
        gas_id                             // gas_id
      );
    }
  }
...
)
{
  ...
  /* Clear retry flags of BCCH decode as they are no longer applicable */
...
)
{
  ...
  if (cell_found==FALSE)
  {
    ...
    /*************************************************************/
    return rr_selection_start_cell_selection(RR_ACQ_DB_PREF_ALLOWED, gas_id);
  }
  else
  {
...
static rr_event_T rr_selection_process_low_priority_cells(
  ...
  boolean select_cell_if_suitable,
  const gas_id_t gas_id
)
{
  ...
  if (rr_nv_get_csfb_snr_enabled(RR_GAS_ID_TO_AS_ID))
  {
  ...
  while (!done)
  {
    ...
    {
      /* Find the strongest processed high priority cell in the database */
      cell_found = rr_candidate_db_find_high_priority_processed_suitable_cell(candidate_db_ptr,&candidate_cell_entry_ptr, gas_id);
      if (cell_found)
      {
        MSG_GERAN_HIGH_0_G("High priority suitable cell found while starting low priortity search, disable SNR");
        snr_based_selection = FALSE;
      }
      else
      { 
    ...
    else if (candidate_cell_entry_ptr != NULL) /* low priority cell was found */
    {
      ...
      if (candidate_cell_entry_ptr->type == RR_CANDIDATE_VALID_BCCH)
      {
        ...
        if (snr_based_selection)
        {
          ...
          rr_candidate_db_find_best_of_fcch_snr_for_low_prior_cells(candidate_db_ptr, &best_cell_found, &best_cell_index, gas_id);
                                   ...
                                   gas_id))
  {
    ...
    if (candidate_cell_entry_ptr->type == RR_CANDIDATE_VALID_BCCH)
    {
      ...
      if (cell_suitable)
      {
        ...
        {
          output_event = RR_EV_SPECIFIC_CHANNEL_SEARCH;
        }
        break;
      }
    }
...
static rr_event_T check_rgs_retry_status(const gas_id_t gas_id)
{
  // Default is no particular action required
  rr_event_T rgs_event = EV_NO_EVENT;

  // Obtain a pointer to the module data
  rr_cell_selection_data_t *data_ptr = rr_cell_selection_get_data_ptr(gas_id);

  if (data_ptr->rr_cell_selection_details.retry_in_progress)
  {
    MSG_GERAN_HIGH_0_G("RGS multipass retry failed");

    data_ptr->rr_cell_selection_details.retry_in_progress = FALSE;

    // Exit indicating that the RGS retry has failed
    rgs_event = EV_MULTIPASS_SEARCH_FAILED;
  }
  else
  {
    // 'retry_required' as given in MPH_DECODE_BCCH_LIST_CNF
    if (data_ptr->rr_cell_selection_details.retry_required)
    {
      MSG_GERAN_HIGH_0_G("RGS multipass retry");

      data_ptr->rr_cell_selection_details.retry_in_progress = TRUE;

      // Exit indicating that RGS retry is now required
      rgs_event = EV_PERFORM_MULTIPASS_SEARCH;
    }
  }

  if (rgs_event != EV_NO_EVENT)
  {
    rr_invalidate_power_scan_results(gas_id);
  }
...
static rr_cell_search_result_e_t search_candidates_for_lower_priority_cells(
  rr_candidate_cell_db_T *candidate_db_ptr,
  rr_available_plmn_db_T *available_plmn_db_ptr,
  boolean select_cell_if_suitable,
  const gas_id_t gas_id
)
{
  ...
  if (any_valid_cell_found)
  {
    ...
    /************************************************************************/
    rr_event = rr_selection_process_low_priority_cells(
      candidate_db_ptr,
      available_plmn_db_ptr,
      select_cell_if_suitable,
      gas_id
    );

    if (rr_event == EV_NO_BCCH_FOUND)
    {
      rr_event = rr_selection_check_for_forbidden_cells(
        candidate_db_ptr,
        select_cell_if_suitable,
        gas_id
      );
    ...
    if (rr_event == EV_NO_BCCH_FOUND)
    {
      return RR_NO_SUITABLE_CELL_FOUND;
    }
    else
    {
      return RR_SUITABLE_CELL_FOUND;
    }
  }

  return RR_NO_VALID_CELLS_FOUND;
}

static rr_event_T perform_next_action(
  rr_find_cell_action_e next_action,
  boolean acq_db_search_done,
  rr_candidate_cell_db_T *candidate_db_ptr,
  rr_available_plmn_db_T *available_plmn_db_ptr,
  const gas_id_t gas_id
)
{
  ...
  boolean next_action_done = FALSE;

  RR_NULL_CHECK_RETURN_PARAM(candidate_db_ptr, EV_NO_BCCH_FOUND);
  RR_NULL_CHECK_RETURN_PARAM(available_plmn_db_ptr, EV_NO_BCCH_FOUND);

  while (FALSE == next_action_done)
  {
    switch (next_action)
    {
      case RR_FIND_CELL_ACTION_PSCAN:
      {
        rr_find_cell_pscan_params_t pscan_params = rr_find_cell_get_pscan_params(gas_id);

        boolean proceed_with_pscan = TRUE;

        rr_event = EV_NO_BCCH_FOUND;

        /**
         * If all previous candidates have been tried, then check if any BCCH decodes was successful, and if
         * so, check for cell suitability based on low priority / forbidden cells.
         */
        if (pscan_params.all_candidate_db_cells_tried)
        {
          rr_event_T rgs_event = check_rgs_retry_status(gas_id);

          if (EV_NO_EVENT != rgs_event)
          {
            rr_find_cell_search_complete(gas_id);

            proceed_with_pscan = FALSE;
            rr_event = rgs_event;
          }
          else
          {
            rr_cell_search_result_e_t search_result;

            /**
             * This will return
             *  RR_NO_VALID_CELLS_FOUND - no valid BCCHs found in candidate DB
             *  RR_SUITABLE_CELL_FOUND - suitable cell found
             *  RR_NO_SUITABLE_CELL_FOUND - no suitable cell found
             */
            search_result = search_candidates_for_lower_priority_cells(
              candidate_db_ptr,
              available_plmn_db_ptr,
#if defined (FEATURE_INTERRAT_REDIRECTION_WTOG) && defined (FEATURE_PCS_850_FREQ_SCAN_OPTIMIZATION)
              (FALSE == rr_w2g_service_search_in_progress(gas_id)),
#else
              TRUE,
#endif
              gas_id
            ...
            if (RR_NO_SUITABLE_CELL_FOUND == search_result)
            {
              /**
               * One or more cells were successfully decoded.
               * If none are suitable, then the next action depends on whether these were ACQ-DB cells, or
               * cells from a full power-scan.
               * If ACQ-DB cells, continue with the full power-scan.
               * If a full power-scan has been done, then exit and fail the search.
               */
              if (acq_db_search_done)
              {
                MSG_GERAN_MED_0_G("No lower priority cells found - continue search");
              }
              else
              {
                MSG_GERAN_MED_0_G("No lower priority cells found - search failed");
                proceed_with_pscan = FALSE;
              }
            }
            else if (RR_SUITABLE_CELL_FOUND == search_result)
            {
              MSG_GERAN_MED_0_G("Suitable cell found");
              proceed_with_pscan = FALSE;
              rr_event = RR_EV_SPECIFIC_CHANNEL_SEARCH;
            }
          }
        }

        if (proceed_with_pscan)
        {
          /**
           * This will return
           *  RR_EV_PERFORM_POWER_SCAN - power-scan request sent to L1
           *  EV_NO_BCCH_FOUND - no frequencies found for power-scan request
           */
          rr_event = rr_perform_power_scan(
            RR_SEARCH_NORMAL,
            pscan_params.band_mask,
            pscan_params.filter_acq_db_freqs,
            gas_id
          );
        }

        if (EV_NO_BCCH_FOUND == rr_event)
        {
          next_action = rr_find_cell_no_suitable_cell_found(SYS_BAND_MASK_EMPTY, gas_id);
        }
        else
        {
      ...
      {
        rr_find_cell_bcch_decodes_params_t bcch_decode_params = rr_find_cell_get_bcch_decodes_params(gas_id);

        /* reset the flag to indicate that a MPH_DECODE_BCCH_LIST_CNF has not been received */
        data_ptr->rr_cell_selection_details.bcch_list_cnf_received = FALSE;

        /**
         * If all candidates have been tried, then check if any BCCH decodes was successful, and if so,
         * check for cell suitability based on low priority / forbidden cells.
         */
        if (bcch_decode_params.all_candidate_db_cells_tried)
        {
          rr_cell_search_result_e_t search_result;
          ...
          {
            case RR_NO_VALID_CELLS_FOUND:
            {
              /**
               * No cells were successfully decoded.
               * The next action is to try more cells if there are frequencies remaining from the power-scan
               * which haven't been tried.
               */
              rr_event = rr_perform_bcch_list_decode_from_candidate_db(candidate_db_ptr, gas_id);
              break;
            }

            case RR_NO_SUITABLE_CELL_FOUND:
            {
              /**
               * One or more cells were successfully decoded.
               * If none are suitable, then the search has failed and cell selection is finished.
               */
              rr_event = EV_NO_BCCH_FOUND;
              break;
            }

            case RR_SUITABLE_CELL_FOUND:
            {
              /**
               * A suitable cell was chosen.
               */
              rr_event = RR_EV_SPECIFIC_CHANNEL_SEARCH;
              break;
            }
        ...
        if (RR_EV_DECODING_BCCH == rr_event)
        {
      ...
      case RR_FIND_CELL_ACTION_NONE:
      {
        ...
        else
        {
          ...
          if (RR_SUITABLE_CELL_FOUND == search_result)
          {
            rr_event = RR_EV_SPECIFIC_CHANNEL_SEARCH;
          }
          else
      ...
      default:
      {
        MSG_GERAN_ERROR_1_G("next_action=%d", (int)next_action);
        next_action_done = TRUE;
      }
    } // switch (next_action)
  ...
  {
    if (RR_EV_SPECIFIC_CHANNEL_SEARCH == rr_event)
    {
      rr_event = RR_EV_ABORT_PROCEDURE;
    }
  }
#endif

  if ((EV_NO_BCCH_FOUND == rr_event) ||
      (RR_EV_ABORT_PROCEDURE == rr_event))
  {
    rr_find_cell_search_complete(gas_id);
  }

  /**
   * The return event will be one of
   *  RR_EV_PERFORM_POWER_SCAN - Power-scan request sent to GL1
   *  RR_EV_DECODING_BCCH - BCCH list decode request sent to GL1
   *  RR_EV_SPECIFIC_CHANNEL_SEARCH - suitable cell chosen
   *  EV_NO_BCCH_FOUND - search failed (exit)
   *  EV_MULTIPASS_SEARCH_FAILED - search failed (exit)
   *  EV_PERFORM_MULTIPASS_SEARCH - search failed (try a 2nd attempt)
   *  RR_EV_ABORT_PROCEDURE - X2G redirection search completed
   */

  return rr_event;
}

static uint16 rr_selection_prioritise_pscan_results(boolean acq_db_search, const gas_id_t gas_id)
{
  rr_pscan_results_db_T *pscan_results_db_ptr = rr_get_pscan_results_db_ptr(gas_id);

  if (acq_db_search)
  {
    // Move cells from the requested PLMN to the front
    rr_acq_db_prioritise_pscan_results(
      &pscan_results_db_ptr->pscan_results,
      rr_get_PLMN_request_details_ptr(gas_id)->requested_PLMN_id,
      gas_id
    );
  }
  ...
  if (rr_x2g_redir_with_si_in_progress(gas_id))
  {
    rr_x2g_redir_prioritise_pscan_results(&pscan_results_db_ptr->pscan_results, gas_id);
  }
#endif /* FEATURE_LTE_TO_GSM_REDIR_WITH_SI_SUPPORT */

#ifdef FEATURE_SGLTE
  if (rr_is_sglte_mode_active(gas_id) && rr_sel_get_service_domain(gas_id) == SYS_SRV_DOMAIN_CS_ONLY)
  {
    rr_sglte_band_coex_prioritise_pscan_results(&pscan_results_db_ptr->pscan_results);
  }
#endif /* FEATURE_SGLTE */

  return pscan_results_db_ptr->pscan_results.num_frequencies;
}
 ...
 *
 * @return
 *     EV_POWER_SCAN_RESULTS_EMPTY - Power scan results indicate no cell are available
 *     EV_NO_BCCH_FOUND - No candidates for BCCH decodes
...
)
{
...
#ifdef FEATURE_LOCK_ARFCN
  rr_channel_lock_info_t *rr_channel_lock_info_ptr  = rr_channel_lock_info_get_data_ptr(gas_id);
#endif /*FEATURE_LOCK_ARFCN*/

  // Obtain a pointer to the module data
  rr_cell_selection_data_t *data_ptr = rr_cell_selection_get_data_ptr(gas_id);

  num_frequencies_found = rr_pscan_process_power_scan_results(gas_id);

  if (num_frequencies_found > 0)
  {
    MSG_GERAN_MED_1_G("%d freqs in pscan results", (int)num_frequencies_found);

#ifdef FEATURE_LOCK_ARFCN
    if ((rr_channel_lock_info_ptr->pscan_complete_cb.func_ptr != NULL) &&
        (data_ptr->cell_select_state == RR_SELECTION_WAIT_FOR_PSCAN_RESULTS))
    {
      rr_pscan_results_db_T * rr_pscan_results_db_ptr = rr_get_pscan_results_db_ptr(gas_id);
      rr_channel_lock_info_ptr->pscan_complete_cb.func_ptr(rr_pscan_results_db_ptr->pscan_results.num_frequencies, RR_GAS_ID_TO_AS_ID);
      MSG_GERAN_HIGH_0_G("ARFCN locking enabled wait for trigger ");
      return EV_WAIT_FOR_ARFCN;
    }
#endif /*FEATURE_LOCK_ARFCN*/

    /* reset the flag to indicate that a MPH_DECODE_BCCH_LIST_CNF has not been received */
    data_ptr->rr_cell_selection_details.bcch_list_cnf_received = FALSE;
    data_ptr->rr_cell_selection_details.denied_bands = SYS_BAND_MASK_EMPTY;

    /* Tell L1 to do a bcch list decode */
    return_event = rr_perform_bcch_list_decode_from_candidate_db(candidate_db_ptr, gas_id);

    if (EV_NO_BCCH_FOUND == return_event)
    {
      rr_find_cell_action_e next_action;

      MSG_GERAN_MED_0_G("No candidates to decode");

      next_action = rr_find_cell_no_suitable_cell_found(SYS_BAND_MASK_EMPTY, gas_id);

      return_event = perform_next_action(
        next_action,
        acq_db_search,
        candidate_db_ptr,
        available_plmn_db_ptr,
...
#ifdef FEATURE_LTE_TO_GSM_REDIR_WITH_SI_SUPPORT
    else if (rr_x2g_redir_with_si_in_progress(gas_id))
    {
      rr_x2g_redir_with_si_send_si3_si4_if_available(
        candidate_db_ptr,
        gas_id
  ...
  else
  {
    ...
    MSG_GERAN_MED_0_G("No freqs in pscan results");

    next_action = rr_find_cell_no_freq_found(gas_id);
   ...
   *  RR_EV_DECODING_BCCH - BCCH list decode request sent to GL1
   *  RR_EV_SPECIFIC_CHANNEL_SEARCH - suitable cell chosen
   *  RR_NO_BCCH_FOUND - search failed (exit)
   *  RR_EV_ABORT_PROCEDURE - W2G redirection search failed
   */
...
static sys_band_mask_type get_band_mask(rr_internal_band_T band)
{
...
)
{
  ...
  rr_PLMN_request_details_ptr->scan_scope = rr_plmn_select_req->scan_scope;
  ...
  MSG_GERAN_HIGH_4_G("PLMNSelReq,any=%d,auto=%d,csOnly=%d,is_ehplmn_to_be_camped=%d",
  ...
  if (rr_is_camped(gas_id))
  {
    ...
    if (rr_default_irat_params_update_required(gas_id))
    {
      ...
      {
        MSG_GERAN_HIGH_0_G("[Pseudo SI2q] Pseudo SI2q configured");
      } 
    }
#endif
#if defined (FEATURE_WCDMA) || defined (FEATURE_GSM_TDS) || defined (FEATURE_LTE)
    ...
    else if (rat_info_changed == TRUE)
    {
      MSG_GERAN_HIGH_0_G("RAT prior list changed, update l1 with empty wcdma list.");
      rr_send_mph_wcdma_cell_update_list_req(FALSE, gas_id);
    }
  }
...
)
{
  ...
  {
    acq_db_pref = RR_ACQ_DB_PREF_ALLOWED;
  }
  ...
  if (rr_plmn_db_is_valid(available_plmn_db_ptr, rr_band_pref(gas_id), gas_id))
  {
    ...
    else /* the PLMN camping list is empty */
    {
      ...
      /* power scan results                                             */
      /******************************************************************/
      rr_candidate_db_set_invalid(candidate_db_ptr);
    }
  }
  else
  {
  ...
  return rr_selection_start_cell_selection(acq_db_pref, gas_id);
...
)
{
  ...
  data_ptr->rr_cell_selection_details.bcch_list_cnf_received = TRUE;
  rr_set_decode_cnf_received(TRUE, gas_id);

  rr_candidate_set_denied_bands(candidate_db_ptr, denied_bands);

  if (rr_candidate_db_decode_bcch_complete(gas_id))
  {
    rr_find_cell_action_e next_action = rr_find_cell_no_suitable_cell_found(denied_bands, gas_id);
    ...
    /**
     * At this point, the return event will be one of
     *  RR_EV_PERFORM_POWER_SCAN - Power-scan request sent to GL1
     *  RR_EV_DECODING_BCCH - BCCH list decode request sent to GL1
     *  RR_EV_SPECIFIC_CHANNEL_SEARCH - suitable cell chosen
     *  RR_NO_BCCH_FOUND - search failed (exit)
     *  RR_EV_ABORT_PROCEDURE - X2G redirection search failed
     *  EV_PERFORM_MULTIPASS_SEARCH - retry required
     *  EV_MULTIPASS_SEARCH_FAILED - retry failed
     */
  }
...
)
{
  ...
  while (!done)
  {
    if (snr_based_selection)
    {
        cell_found = rr_candidate_db_fcch_snr_high_priority_find_next(
          candidate_db_ptr, &candidate_cell_entry_ptr, gas_id);
    }
    else
    {
        cell_found = rr_candidate_db_find_next(
          candidate_db_ptr, &candidate_cell_entry_ptr, RR_CANDIDATE_HIGH_PRIORITY_CELLS, gas_id);
    }

    if (!cell_found)
    {
      /* did not find any normal unprocessed cells in the database */
      if (data_ptr->rr_cell_selection_details.bcch_list_cnf_received)
      {
        ...
        return
          rr_selection_bcch_decode_completed(
            candidate_db_ptr,
            available_plmn_db_ptr,
            data_ptr->rr_cell_selection_details.denied_bands,
            data_ptr->rr_cell_selection_details.acq_db_search,
            gas_id
          );
      }
    ...
    else
    {
      ...
      if (candidate_cell_entry_ptr != NULL) /* normal priority cell was found */
      {
        ...
        {
          if ((snr_based_selection) && 
              (FALSE == candidate_cell_entry_ptr->fcch_snr_rcvd) && 
              (candidate_cell_entry_ptr->type != RR_CANDIDATE_UNKNOWN)
#ifdef FEATURE_LTE_TO_GSM_REDIR_WITH_SI_SUPPORT
              && (rr_x2g_redir_with_si_in_progress(gas_id))
#endif
            )
          {
            MSG_GERAN_HIGH_0_G("SNR based X2G, with tunneled SIs,wait for strogest cell FCCH_SCH ...");
            /* Wait for strogest cell F/S to start snr algorithm */
            return EV_NO_EVENT;
          }
          else
          {
        ...
        if (candidate_cell_entry_ptr->type == RR_CANDIDATE_VALID_BCCH || process_only_snr)
        {
          ...
          if (cell_suitable)
          {
            ...
            if (rr_w2g_service_search_in_progress(gas_id))
            {
              data_ptr->rr_cell_selection_details.suitable_cell_found = TRUE;
...
#ifdef FEATURE_LTE_TO_GSM_REDIR_WITH_SI_SUPPORT
              if (rr_x2g_redirection_cell_has_mulitple_bsics(candidate_cell_entry_ptr, gas_id))
              {
                return
                  rr_selection_perform_specific_channel_search(
                    candidate_db_ptr,
 ...
 */
static void rr_selection_complete_cell_camping(const gas_id_t gas_id)
{
  int C1;
  ...
  // The search for a cell has completed
  rr_find_cell_search_complete(gas_id);
  ...
  /**
   * Send an ABORT_IND to MM if there is an LAI change during cell selection and if there is a saved
   * RR_EST_REQ
   */
 ...
 */
static rr_event_T rr_selection_process_channel_data(boolean acquisition_success, const gas_id_t gas_id)
{
...
static rr_event_T rr_selection_recover_from_acquisition_failure(
  rr_candidate_cell_db_T *candidate_db_ptr,
  rr_available_plmn_db_T *available_plmn_db_ptr,
  rr_cell_select_fail_reason_e_t failure_reason,
  const gas_id_t gas_id
)
{
  gprs_scell_info_t          *pcell_info_ptr = rr_gprs_get_pcell_info(gas_id);
      ...
      (rr_cell_selection_data_ptr->rr_plmn_camping_list.valid            == TRUE))
  {
    ...
    else
    {
      if (rr_sel_is_scan_scope_acq_db(rr_PLMN_request_details_ptr))
      {
        MSG_GERAN_HIGH_1_G("Attempt to acquire using availble PLMM DB %d failed, doing RR_MM_ACQ_DB_SCAN, abort search", arfcn_failed_to_camp_on.num);
  ...
  else if ((RR_SEL_FAIL_REASON_CELL_BARRED == failure_reason) ||
           (RR_SEL_FAIL_REASON_BCCH_NOT_FOUND == failure_reason))
  {
    if ((RR_W2G_SERVICE_REDIRECTION == rr_PLMN_request_details_ptr->search_reason) ||
        (RR_W2G_SERVICE_REDIRECTION_EMERGENCY_CALL == rr_PLMN_request_details_ptr->search_reason))
    {
      rr_find_cell_search_complete(gas_id);
      out_event = EV_NO_BCCH_FOUND;
    }
    ...
    {
      out_event = rr_perform_bcch_list_decode_from_candidate_db(candidate_db_ptr, gas_id);

      if (EV_NO_BCCH_FOUND == out_event)
      {
        rr_find_cell_action_e next_action;

        next_action = rr_find_cell_no_suitable_cell_found(SYS_BAND_MASK_EMPTY, gas_id);

        rr_find_cell_acquisition_failure(gas_id);

        if (RR_FIND_CELL_ACTION_NONE != next_action)
        {
          out_event = perform_next_action(
            next_action,             // next_action
            TRUE,                    // acq_db_search
            candidate_db_ptr,        // candidate_db_ptr
            available_plmn_db_ptr,   // available_plmn_db_ptr
            gas_id                   // gas_id
          );
        }
      }
  ...
  {
    if (search_is_acq_db_only(gas_id))
    {
      MSG_GERAN_HIGH_1_G("Attempt to acquire %d failed, abort search (ACQ-DB only)", arfcn_failed_to_camp_on.num);
      out_event = EV_NO_BCCH_FOUND;
    }
    else
    {
  ...
  if (out_event == EV_NO_EVENT)
  {
    ...
    switch (rr_PLMN_request_details_ptr->search_reason)
    {
      ...
      default:
      {
        ...
        pscan_params = rr_find_cell_new_search(
          RR_ACQ_DB_PREF_NOT_ALLOWED,    // acq_db_pref
          rr_search_band_pref(gas_id),   // band_pref
          TRUE,                          // search_by_band_allowed
          candidate_db_ptr,              // candidate_db_ptr
          gas_id                         // gas_id
        );

        out_event = rr_perform_power_scan(
          RR_SEARCH_NORMAL,         // search_mode
          pscan_params.band_mask,   // band_group
          FALSE,                    // filter_acq_db_freqs
          gas_id                    // gas_id
        );
        break;
      }
 ...
 */
static boolean rr_cell_suitable_camp_cb_fn(
...
)
{
  ...
  if (!cell_suitable)
  {
    MSG_GERAN_HIGH_3_G("Cell %d not suitable (candidate cell check failed any_cell:%d auto_plmn:%d)",
             (int)pcell_info_ptr->gsm.BCCH_ARFCN.num, rr_is_any_cell_selection_mode(gas_id), rr_is_automatic_plmn_selection_mode(gas_id));
  }

  return cell_suitable;

} /* rr_cell_suitable_camp_cb_fn */

/*!
 * \brief Print the ACQ-DB pref to diag.
 * 
 * \param acq_db_pref (in)
 */
static void print_acq_db_pref(rr_acq_db_pref_e_t acq_db_pref)
{
  switch (acq_db_pref)
  {
    case RR_ACQ_DB_PREF_NOT_ALLOWED:
    {
      MSG_GERAN_HIGH_0("acq_db_pref=NOT_ALLOWED");
      break;
    }

    case RR_ACQ_DB_PREF_ALLOWED:
    {
      MSG_GERAN_HIGH_0("acq_db_pref=ALLOWED");
      break;
    }

    case RR_ACQ_DB_PREF_ONLY:
    {
      MSG_GERAN_HIGH_0("acq_db_pref=ONLY");
      break;
    }

    default:
    {
      MSG_GERAN_ERROR_1("acq_db_pref=? (%d)", (int)acq_db_pref);
      break;
    }
  }
...
)
{
  ...
  /* Proceed with early camp only if bcch is found and pending cell is not in
     barred cell list */
  if (mph_select_specific_bcch_cnf->BCCH_found)
  {
    rr_csi_cgi_t cgi;
       ...
       )
    {
      ...
      else
      {
        ...
        if (rr_is_fast_si_acq_enabled(NULL, gas_id))
        {
          ...
          /* Set up BA_INDs with invalid value (0x02).  This is a value that can not
          ...
          pcell->gsm.complete_BA_list.BA_IND        = INVALID_BA_IND;
          pcell->gsm.SI2_BA_list.BA_IND             = INVALID_BA_IND;
          pcell->gsm.SI2bis_BA_list.BA_IND          = INVALID_BA_IND;
          pcell->gsm.SI2ter_BA_list.BA_IND          = INVALID_BA_IND;
    ...
    {
      /* Cell in barred list */
      MSG_GERAN_HIGH_1_G("Cell %d in barred list", pcell_info_ptr->gsm.BCCH_ARFCN.num);

      /* log the failed camping attempt */
      rr_log_camping_attempt_failure(RR_LOG_CAMPING_FAILURE_CELL_BARRED, gas_id);

      return rr_selection_recover_from_acquisition_failure(
        candidate_db_ptr,
        available_plmn_db_ptr,
        RR_SEL_FAIL_REASON_CELL_BARRED,
        gas_id
  ...
  {
    /* BCCH was not found */
    MSG_GERAN_HIGH_1_G("Cell %d not found", pcell_info_ptr->gsm.BCCH_ARFCN.num);
...
)
{
  ...
  if (rr_PLMN_request_details_ptr->scan_scope == SYS_SCAN_SCOPE_ACQ_DB)
  {
    MSG_GERAN_HIGH_0_G("avail PLMN DB not enabled for acq DB only scan");
  }
  else
  {
  ...
  /**********************************************************************************************/
  /* reset the offset in the candidate database back to zero in preparation for the next cell   */
  /* selection or PLMN list construction operation.This is done after sending no service ind    */
  /* because GRR needs  to know how many cells  were required to be decoded to find the failure */
  /* reason when cell selection times out                                                       */
  /**********************************************************************************************/
  rr_candidate_db_set_valid(candidate_db_ptr);
  rr_candidate_db_clear_offset(candidate_db_ptr);
...
)
{
  ...
  rr_ms_service_recovery_started(RR_SERVICE_RECOVERY_AUTO_CELL_SELECTION, gas_id);
#endif /* FEATURE_DUAL_SIM || FEATURE_TRIPLE_SIM */
...
)
{
  ...
  return rr_selection_start_cell_selection(RR_ACQ_DB_PREF_ALLOWED, gas_id);
} /* rr_perform_w2g_service_redirection_cell_selection */
 ...
 */
static rr_event_T rr_selection_complete_abort(const gas_id_t gas_id)
{
  rr_candidate_cell_db_T *candidate_db_ptr = rr_get_candidate_cell_db_ptr(gas_id);
  rr_available_plmn_db_T *available_plmn_db_ptr = rr_get_available_plmn_db_ptr(gas_id);
  ...
  else
  {
...
#ifdef FEATURE_GSM_DECODE_ALL_FREQS
    /* During decoding of BCCH freq's, Selection timer has been aborted,
       Copy back Any valid cell stored in Pending DB to Candidate DB */
    ...
    /************************************************************************************/
    /* log the end of the cell selection (if the event is still outstanding and has not */
    /* been sent already)must be aborted because of a timeout */
    /************************************************************************************/
    if (!rr_log_cell_selection_log_complete(gas_id))
    {
 ...
 */
static void rr_selection_aborted(rr_event_T abort_reason, const gas_id_t gas_id)
{
  // Obtain a pointer to the module data
  rr_cell_selection_data_t *rr_cell_selection_data_ptr = rr_cell_selection_get_data_ptr(gas_id);
...
{
  // Obtain a pointer to the module data
  return (rr_cell_selection_get_data_ptr(gas_id)->cell_select_state == RR_SELECTION_WAIT_FOR_PSCAN_RESULTS);
}

#ifdef FEATURE_LOCK_ARFCN
/*===========================================================================

FUNCTION rr_pause_selection_specific_data

DESCRIPTION
  Pause the channel lock info related data.
...
static void rr_pause_selection_specific_data(cell_select_state_T old_cell_select_state,const gas_id_t gas_id)
{
  rr_channel_lock_info_t *rr_channel_lock_info_ptr = rr_channel_lock_info_get_data_ptr(gas_id);
  RR_NULL_CHECK_FATAL(rr_channel_lock_info_ptr);

  rr_channel_lock_info_ptr->old_cell_select_state = old_cell_select_state;
  rr_channel_lock_info_ptr->time_left = (uint16)rr_timer_get_time_remaining(RR_CELL_SELECTION_ABORT_TIMER, gas_id);
  (void)rr_timer_cancel(RR_CELL_SELECTION_ABORT_TIMER, gas_id);
  (void)rr_timer_start(RR_WAIT_FOR_ARFCN_TO_LOCK_TIMER,RR_WAIT_FOR_ARFCN_TO_LOCK_TIME_OUT, gas_id);
  return;
}

/*===========================================================================

FUNCTION rr_resume_selection_specific_data

DESCRIPTION
  Resume the cell selction activity.

PARAMS
  locked       - channle is getting locked
  gas_id_t    - gas_id
...
static void rr_resume_selection_specific_data(boolean locked, const gas_id_t gas_id)
{
  rr_channel_lock_info_t *rr_channel_lock_info_ptr = rr_channel_lock_info_get_data_ptr(gas_id);
  RR_NULL_CHECK_FATAL(rr_channel_lock_info_ptr);

  (void)rr_timer_cancel(RR_WAIT_FOR_ARFCN_TO_LOCK_TIMER, gas_id);
  (void)rr_timer_start(RR_CELL_SELECTION_ABORT_TIMER,rr_channel_lock_info_ptr->time_left, gas_id);

  MSG_GERAN_HIGH_2_G("cell selection timer resumed for time= %d, lock_state =%d",
                      rr_channel_lock_info_ptr->time_left,
                      locked);
  rr_channel_lock_info_ptr->ue_locked = locked;
  return;
}

/*===========================================================================

FUNCTION rr_reset_locking_channel_info

DESCRIPTION
  Reset the parametre related to locking channel.
...
void rr_reset_locking_channel_info(const gas_id_t gas_id)
{
  rr_channel_lock_info_t *rr_channel_lock_info_ptr = rr_channel_lock_info_get_data_ptr(gas_id);
  RR_NULL_CHECK_FATAL(rr_channel_lock_info_ptr);
  rr_channel_lock_info_ptr->locking_ARFCN.num     = INVALID_ARFCN;
  rr_channel_lock_info_ptr->locking_ARFCN.band    = SYS_BAND_NONE;
  rr_channel_lock_info_ptr->old_cell_select_state = RR_SELECTION_IDLE;
  rr_channel_lock_info_ptr->time_left             = 0;
  rr_channel_lock_info_ptr->ue_locked             = FALSE;
  return;
}

/*===========================================================================

FUNCTION rr_is_ms_locked_to_channel

DESCRIPTION
  Checks that whether GERAN is locked to a channle or not.
...
RETURN VALUE
  TRUE: If GERAN is locked
  FALSE : If GERAN is not locked
===========================================================================*/

boolean rr_is_ms_locked_to_channel(const gas_id_t gas_id)
{
  rr_channel_lock_info_t *rr_channel_lock_info_ptr = rr_channel_lock_info_get_data_ptr(gas_id);
  RR_NULL_CHECK_FATAL(rr_channel_lock_info_ptr);
  MSG_GERAN_HIGH_1_G("Is MS  locked = %d",rr_channel_lock_info_ptr->ue_locked);
  return rr_channel_lock_info_ptr->ue_locked;
}

/*===========================================================================

FUNCTION rr_modify_candidate_cell_db_to_locked_channel

DESCRIPTION
Modifies the candidate cell DB to have only the ARFCN which client has aksed to lock on.

PARAMS
  ARFCN_T - Channel on which GERAN needs to be locked
...
rr_event_T rr_modify_candidate_cell_db_to_locked_channel(ARFCN_T channel_to_set, const gas_id_t gas_id)
{
   static rr_l1_idle_freq_list_T frequency_list;  /* static_dual_spaced_ignore - memory saving */
   rr_event_T cell_select_event;
   rr_available_plmn_db_T *available_plmn_db_ptr;
   rr_cell_selection_data_t *rr_cell_selection_data_ptr;
   rr_candidate_cell_db_T *candidate_db_ptr;
   ...
   rr_cell_selection_data_ptr = rr_cell_selection_get_data_ptr(gas_id);
   available_plmn_db_ptr = rr_get_available_plmn_db_ptr(gas_id);
   acq_db_search = rr_cell_selection_data_ptr->rr_cell_selection_details.acq_db_search;
   
   num_frequencies = candidate_db_ptr->num_frequencies;

   RR_ZERO_STRUCTURE(frequency_list);

   if (ARFCN_VALID_CHANNEL(channel_to_set))
   {
     //Get the hash value of the first ARFCN in candidate cell DB, //
     //and the hash value of ARFCN to be locked in cadidate cell db */

      uint16 hashed_value        = rr_candidate_db_hash_arfcn(candidate_db_ptr->candidate_cells[0].meas.arfcn);
      uint16 hashed_value_to_set = rr_candidate_db_hash_arfcn(channel_to_set);

      MSG_GERAN_HIGH_1_G("Valid ARFCN  = %d provided for locking",channel_to_set.num);

      /* find the index where this cell is in candidate cell database */
      for (index = 0; index < num_frequencies; ++index)
      {
        if (ARFCNS_EQUAL(channel_to_set,candidate_db_ptr->candidate_cells[index].meas.arfcn))
       {

         memsmove(&(candidate_db_ptr->candidate_cells[0]),sizeof(candidate_db_ptr->candidate_cells[index]),
                   &(candidate_db_ptr->candidate_cells[index]),sizeof(candidate_db_ptr->candidate_cells[index])
                  );


         candidate_db_ptr->frequency_index[hashed_value_to_set] = 0;

         if (hashed_value != hashed_value_to_set)
         {
           candidate_db_ptr->frequency_index[hashed_value] = RR_NON_CANDIDATE_CELL;
         }

          MSG_GERAN_HIGH_1_G("Cell is on index = %d, moved to 0th ",index);
          candidate_db_ptr->num_frequencies = 1;
          frequency_list.arfcn[0] = candidate_db_ptr->candidate_cells[index].meas.arfcn;
   ...
   {
     MSG_GERAN_HIGH_0_G("No  ARFCN for locking  continue without locking");
   }
       /* reset the flag to indicate that a MPH_DECODE_BCCH_LIST_CNF has not been received */
   rr_cell_selection_data_ptr->rr_cell_selection_details.bcch_list_cnf_received = FALSE;
   rr_cell_selection_data_ptr->rr_cell_selection_details.denied_bands = SYS_BAND_MASK_EMPTY;
   /* Tell L1 to do a bcch list decode */
   cell_select_event = rr_perform_bcch_list_decode_from_candidate_db(candidate_db_ptr, gas_id);

   if (EV_NO_BCCH_FOUND == cell_select_event)
   {
     rr_find_cell_action_e next_action;
     MSG_GERAN_MED_0_G("No candidates to decode");

     next_action = rr_find_cell_no_suitable_cell_found(SYS_BAND_MASK_EMPTY, gas_id);
     cell_select_event = perform_next_action(
        next_action,
        acq_db_search,
        candidate_db_ptr,
        available_plmn_db_ptr,
...
return cell_select_event;
}

/*===========================================================================

FUNCTION rr_set_locking_channel

DESCRIPTION
Stores the channle on which GERAN is currently locked on.

PARAMS
  ARFCN_T - Channel on which GERAN  to be locked
...
void rr_set_locking_channel(ARFCN_T channel_number, const gas_id_t gas_id)
{
  rr_channel_lock_info_t *rr_channel_lock_info_ptr = rr_channel_lock_info_get_data_ptr(gas_id);
  RR_NULL_CHECK_FATAL(rr_channel_lock_info_ptr);
  rr_channel_lock_info_ptr->locking_ARFCN = channel_number;
}

/*===========================================================================

FUNCTION rr_register_scan_complete_callback

DESCRIPTION
Sotres the call back function in channel info structure

PARAMS
  pscan_completion_cb_fn - callback_func_ptr which needs to be called when GERAN has the power scan results
...
RETURN VALUE
TRUE: If it has registered.

===========================================================================*/

boolean rr_register_scan_complete_callback(pscan_completion_cb_fn callback_func_ptr, const gas_id_t gas_id)
{
  rr_channel_lock_info_t *rr_channel_lock_info_ptr = rr_channel_lock_info_get_data_ptr(gas_id);
  rr_channel_lock_info_ptr->pscan_complete_cb.func_ptr = callback_func_ptr;
  MSG_GERAN_HIGH_0_G("PSCAN complete call back reg.");
  return TRUE;
  ...
  while (cell_select_event != EV_NO_EVENT)
  {
    ...
    switch (rr_cell_selection_data_ptr->cell_select_state)
    {
      case RR_SELECTION_IDLE:
      {
        ...
        else if ( cell_select_event == RR_EV_PROCESS_PLMN_SELECT_REQ )
        {
          ...
          new_cell_select_event = rr_perform_plmn_select_req_selection(
            ...
            gas_id
                 ...
                 (cell_select_event == EV_TRIGGER_CELL_SELECTION_ACQ_DB_ONLY))
        {
          rr_acq_db_pref_e_t acq_db_pref;
          ...
          if (EV_TRIGGER_CELL_SELECTION_ACQ_DB_ONLY == cell_select_event)
          {
            acq_db_pref = RR_ACQ_DB_PREF_ONLY;
          }
          else
        ...
        {
          rr_find_cell_pscan_results_params_t pscan_results_params = rr_find_cell_pscan_complete(gas_id);

#if defined(FEATURE_DUAL_SIM) || defined(FEATURE_TRIPLE_SIM)
          rr_ms_acquisition_started(gas_id);
#endif /* FEATURE_DUAL_SIM || FEATURE_TRIPLE_SIM */

          // Perform any cell / band priorisation
          rr_selection_prioritise_pscan_results(
            pscan_results_params.acq_db_search,    // acq_db_sesrch
            gas_id                                 // gas_id
          );

          rr_cell_selection_data_ptr->rr_cell_selection_details.acq_db_search =
            pscan_results_params.acq_db_search;
      ...
      case RR_SELECTION_WAIT_FOR_PSCAN_RESULTS:
      {
        ...
        if ((cell_select_event == EV_INPUT_MESSAGE) && (message_ptr != NULL))
        {
          if ((message_set == MS_RR_L1) && (message_id == (int) MPH_POWER_SCAN_CNF))
          {
            ...
            else
            {
              ...
              // Perform any cell / band prioritisation
              rr_selection_prioritise_pscan_results(
                pscan_results_params.acq_db_search,   // acq_db_sesrch
                gas_id                                // gas_id
              );
...
#if defined(FEATURE_DUAL_SIM) || defined(FEATURE_TRIPLE_SIM)
                /* advise the multi-SIM controller that an abort is required */
                rr_ms_power_scan_abort_required(RR_EV_MM_REQ_RECEIVED_ABORT, gas_id);
#endif /* FEATURE_DUAL_SIM || FEATURE_TRIPLE_SIM */
              }
              else
              {
        ...
        {
          ; // Power-scan request has been sent to GL1
        }
        else if ( cell_select_event == RR_EV_DECODING_BCCH )
        {
          // BCCH decodes request has been sent to GL1
          rr_cell_selection_data_ptr->cell_select_state = RR_SELECTION_DECODING_BCCH;
        }
        else if ( (cell_select_event == EV_NO_BCCH_FOUND) ||
                  (cell_select_event == EV_MULTIPASS_SEARCH_FAILED) )
...
#if defined(FEATURE_DUAL_SIM) || defined(FEATURE_TRIPLE_SIM)
          /* advise the multi-SIM controller that an abort is required */
          rr_ms_power_scan_abort_required(cell_select_event, gas_id);
#endif /* FEATURE_DUAL_SIM || FEATURE_TRIPLE_SIM */
...
#ifdef FEATURE_LOCK_ARFCN
        else if (cell_select_event == EV_WAIT_FOR_ARFCN)
        {
          rr_cell_selection_data_ptr->cell_select_state = RR_SELECTION_WAIT_FOR_LOCKING_EVENT;
          rr_pause_selection_specific_data(RR_SELECTION_WAIT_FOR_PSCAN_RESULTS,gas_id);
      ...
      case RR_SELECTION_DECODING_BCCH:
      {
        ...
        if ((cell_select_event == EV_INPUT_MESSAGE) && (message_ptr != NULL))
        {
          if (message_set == MS_RR_L1)
          {
            ...
            else if ((rr_l1_message_id_enum_T) message_id == MPH_DECODE_BCCH_LIST_CNF)
            {
              ...
              mph_decode_bcch_list_cnf_T *decode_bcch_list_cnf_ptr = &message_ptr->mph_decode_bcch_list_cnf;

              denied_bands = rr_l1_band_bitmap_to_sys_band_mask(decode_bcch_list_cnf_ptr->denied_bands);

              // Store the denied bands (if any) for use later
              rr_cell_selection_data_ptr->rr_cell_selection_details.denied_bands = denied_bands;
...
#endif /* FEATURE_DUAL_SIM || FEATURE_TRIPLE_SIM */
              /* Preserve the retry decision across multiple band BCCH decodes w.r.t. search-by-band FR */
              if (rr_cell_selection_data_ptr->rr_cell_selection_details.retry_required == FALSE)
              {
                rr_cell_selection_data_ptr->rr_cell_selection_details.retry_required =
                  decode_bcch_list_cnf_ptr->retry_required;
              }
        ...
        else if ( cell_select_event == RR_EV_DECODE_BCCH_COMPLETE_TIMEOUT )
        {
          ...
          new_cell_select_event = rr_selection_bcch_decode_completed(
            candidate_db_ptr,
            available_plmn_db_ptr,
            rr_cell_selection_data_ptr->rr_cell_selection_details.denied_bands,
            rr_cell_selection_data_ptr->rr_cell_selection_details.acq_db_search,
            gas_id
          );
        }
        ...
        else if (cell_select_event == EV_PERFORM_MULTIPASS_SEARCH)
        {
          ...
          new_cell_select_event = rr_selection_start_cell_selection(RR_ACQ_DB_PREF_ALLOWED, gas_id);
          rr_cell_selection_data_ptr->cell_select_state = RR_SELECTION_IDLE;
        }
        ...
        else if ( cell_select_event == RR_EV_CHANNEL_FOUND )
        {
          if (rr_sel_use_rr_select_bcch(gas_id))
          {
            ...
            rr_select_bcch_send_imsg_acquire_si_req(
              ...
              rr_cell_suitable_camp_cb_fn,                          // cell_suitable_cb_fn
      ...
      case RR_SELECTION_WAIT_FOR_SYS_INFO:
      {
        ...
        if ((cell_select_event == EV_INPUT_MESSAGE) && (message_ptr != NULL))
        {
              ...
              case RR_L1_IDLE_OMSG:
              {
                switch (message_ptr->rr.header.imh.message_id)
                {
                  case RR_L1_IDLE_OMSG_START_EARLY_CAMP_CNF:
                  {
                    ...
                    /*After early camping GRR should honor block quality and hence initialise the DSC*/
          ...
          else if (message_set == MS_RR_L1)
          {
            switch ((rr_l1_message_id_enum_T) message_id)
            {
              ...
              {
                mph_block_quality_ind_T *block_qual_ind = &message_ptr->mph_block_quality_ind;
                new_cell_select_event = rr_gprs_check_dsc_in_early_camp(
                   block_qual_ind->block_quality,
                   block_qual_ind->l2_channel_type,
                   gas_id
                );
                break;
              }
        ...
        /***************************************************************/
        /* RR_EV_DSF_DURING_EARLY_CAMP                                                      */
        /* XCCH failure after early camping and  waiting for GSM SI                       */
        /***************************************************************/
        else if ( cell_select_event == RR_EV_DSF_DURING_EARLY_CAMP )
        {
          /*******************************************************************/
          /* xCCH failure while  waiting for SI,after early camp is done */
          /*******************************************************************/
          if (rr_timer_is_running(RR_GSM_SYS_INFO_TIMER, gas_id))
          {
            /* timer expiry and timer expiry callback */
            if (rr_timer_force_timer_expiry(RR_GSM_SYS_INFO_TIMER, gas_id))
            {
              MSG_GERAN_HIGH_0_G("XCCH failure observed after early camp:RR_GSM_SYS_INFO_TIMER forcefully expired");
            }
          }
        }
        /***************************************************************/
        /* RR_EV_DSF_DUE_TO_DENIAL_DURING_EARLY_CAMP                                      */
        /* Resource not available after early camping and  waiting for GSM SI                                  */
        /***************************************************************/
        else if ( cell_select_event == RR_EV_DSF_DUE_TO_DENIAL_DURING_EARLY_CAMP )
        {
          /*******************************************************************/
          /* No resource  while  waiting for SI,after early camp is done */
          /*******************************************************************/
          if (rr_timer_is_running(RR_CELL_SELECTION_ABORT_TIMER,gas_id))
          {
            /* timer expiry and timer expiry callback */
            if (rr_timer_force_timer_expiry(RR_CELL_SELECTION_ABORT_TIMER, gas_id))
            {
              MSG_GERAN_HIGH_0_G("Resource not available after early camp:RR_CELL_SELECTION_ABORT_TIMER forcefully expired");
            }
          }
        }
                  ...
                  cell_select_event == EV_NO_BCCH_FOUND )
        {
          if (search_is_acq_db_only(gas_id))
          {
            MSG_GERAN_HIGH_0_G("SI acquisition failed, abort search (ACQ-DB only)");
      ...
      case RR_SELECTION_WAIT_FOR_SYS_INFO_RR_SELECT_BCCH:
      {
        switch (cell_select_event)
        {
                    ...
                    switch (message_ptr->rr.select_bcch.omsg.header.imh.message_id)
                    {
                      ...
                      case RR_SELECT_BCCH_OMSG_SELECT_FAIL_IND:
                      {
                        ...
                        switch (omsg_ptr->reason)
                        {
						  ...
						  case RR_SELECT_BCCH_FAIL_REASON_CELL_NOT_SUITABLE:
                          {
                            rr_log_camping_attempt_failure(RR_LOG_CAMPING_FAILURE_CELL_NOT_SUITABLE, gas_id);
          ...
          case RR_EV_DSF_DURING_EARLY_CAMP:
          {
            ...
            if (rr_timer_is_running(RR_GSM_SYS_INFO_TIMER, gas_id))
            {
              ...
              if (rr_timer_force_timer_expiry(RR_GSM_SYS_INFO_TIMER, gas_id))
              {
                 MSG_GERAN_MED_0_G("XCCH failure observed after early camp:RR_GSM_SYS_INFO_TIMER forcefully expired");
              }
            }
            break;
          }

          case RR_EV_DSF_DUE_TO_DENIAL_DURING_EARLY_CAMP:
          {
            /* Make sure timer is running */
            if (rr_timer_is_running(RR_CELL_SELECTION_ABORT_TIMER, gas_id))
            {
              ...
              if (rr_timer_force_timer_expiry(RR_CELL_SELECTION_ABORT_TIMER, gas_id))
              {
                 MSG_GERAN_MED_0_G("No resource available after early camp:RR_CELL_SELECTION_ABORT_TIMER forcefully expired");
              }
            }
            break;
          }
      ...
      case RR_SELECTION_WAIT_FOR_LOCKING_EVENT:
      {
        switch (cell_select_event)
        {
          case EV_START_LOCKING_TO_CHANNEL:
          {
            rr_channel_lock_info_t *rr_channel_lock_info_ptr = rr_channel_lock_info_get_data_ptr(gas_id);

            new_cell_select_event = rr_modify_candidate_cell_db_to_locked_channel(rr_channel_lock_info_ptr->locking_ARFCN, gas_id);
            rr_resume_selection_specific_data(TRUE, gas_id);
            /*move to correct state to handle the new event*/
            rr_cell_selection_data_ptr->cell_select_state = rr_channel_lock_info_ptr->old_cell_select_state;
            break;
          }
          case EV_NO_LOCKING_REQUIRED:
          {
            rr_channel_lock_info_t *rr_channel_lock_info_ptr = rr_channel_lock_info_get_data_ptr(gas_id);
            new_cell_select_event = rr_modify_candidate_cell_db_to_locked_channel(rr_channel_lock_info_ptr->locking_ARFCN, gas_id);
            rr_resume_selection_specific_data(FALSE,gas_id);
            /*move to correct state to handle the new event*/
            rr_cell_selection_data_ptr->cell_select_state = rr_channel_lock_info_ptr->old_cell_select_state;
            break;
          }
...
/*!
 * \brief Returns TRUE if the service request indicated RR_MM_FULL_RAT_ACQ scan.
 *
 * \param rr_PLMN_request_details_ptr
 *
 * \return boolean - TRUE if the RR_PLMN_SELECT_REQ indicated RR_MM_FULL_RAT_ACQ scan, FALSE otherwise
 */
boolean rr_sel_is_search_cause_service_req_mm_full_rat_acq_db_scan(
  rr_PLMN_request_details_T *rr_PLMN_request_details_ptr
)
{
  if ((rr_PLMN_request_details_ptr != NULL) &&
      (rr_PLMN_request_details_ptr->search_reason == RR_PLMN_SELECTION) &&
      (rr_PLMN_request_details_ptr->service_req_cause == RR_MM_FULL_RAT_ACQ))
  {
    return TRUE;
  }
...
#ifdef FEATURE_QSH_MDUMP
QSH_MDUMP_FN_ATTR
void grr_mdump_rr_cell_selection(const gas_id_t gas_id)
{
  rr_cell_selection_data_t *rr_cell_selection_data_ptr = rr_cell_selection_get_data_ptr(gas_id);
  qsh_mdump_collect_high(rr_cell_selection_data_ptr, sizeof(rr_cell_selection_data_t));
}
