...
============================================================================*/
/* $Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_resel_calcs.c#2 $ */
...
#define RR_DEPRIORITEZED_CELL_C2_PENALTY 5
      ...
      while ( local_gprs_reselection_list_ptr->no_of_entries && rr_gprs_is_snr_counter_zero(gas_id) )
      {
        ...
        /* Make sure that the cell is suitable before bothering to test it further */
          ...
          */
          ...
          /*Check to see if the cells are in the same RA, this can only be done if both the cells support gprs and UE is GPRS attach*/
          if ( (grr_is_ps_allowed_by_service_domain(gas_id)) &&
               (ptr_bcch_surround_db->gprs.ra_colour != scell_info->bcch_sys_info.ra_colour) &&
...
)
{
  ...
  */
  ...
  if ( !ncell_better && different_la_ra_cell_list.no_of_entries)
  {
    ...
    /* Add resel hysteresis to discourage cell reselection outside LA/RA */
    hysteresis_adjust += scell_info->gsm.cell_selection_parameters.CELL_RESELECT_HYSTERESIS;

    MSG_GERAN_MED_1_G("++Slow SNR outside LA/RA Test++ new ha=%d", hysteresis_adjust);
      ...
      rr_gprs_is_dsc_below_max_value(gas_id))
  {
    ...
    */
    ...
    if (rr_gprs_is_nC2_near_sC2_and_nC2inst_greater_than_sC2inst(gas_id))
    {
      ...
      ARFCN = scell_info->gsm.BCCH_ARFCN;
      
      rr_gprs_init_bad_snr_counter(gas_id);
      bad_scell_reselection = TRUE;

      /* SET the snr_reselection_triggered flag and start RR_SERV_CELL_BAD_SNR_TIMER */
      rr_gprs_set_old_scell_snr_resel_status(ARFCN,TRUE,gas_id);
...
/***************************************************/
  ...
  rr_check_cell_deprioritized_and_reduce_c2(
...
#ifdef FEATURE_FAKE_BTS_CELL_DEPRIORITIZE
     rr_check_cell_deprioritized_and_reduce_c2(
       ptr_bcch_surround_db->gsm.BCCH_ARFCN,
       ptr_bcch_surround_db->gsm.BSIC,
       ptr_bcch_surround_db->gsm.location_area_identification,
       ptr_bcch_surround_db->gsm.cell_id,
       &C2,
       gas_id
     );
#endif/*FEATURE_GCELL_DEPRIORITIZING*/
...
)
{
  ...
  /*Check to see if the cells are in the same RA, this can only be done if both the cells support gprs and UE is GPRS attach */
  if ( (grr_is_ps_allowed_by_service_domain(gas_id)) &&
       (ncell_info->gprs.ra_colour != scell_info->bcch_sys_info.ra_colour) &&
       scell_info->gsm.gprs_indicator &&
...
)
{
  ...
  *dest_gprs_reselection_values = *src_gprs_reselection_values;
  ...
  if ((ptr_surround_bcch = rr_gprs_get_surround_ncell_sys_info(src_gprs_reselection_values->BCCH_ARFCN, gas_id)) != NULL)
  {
    ...
    if ( (grr_is_ps_allowed_by_service_domain(gas_id)) &&
         (scell_info->bcch_sys_info.ra_colour != ptr_surround_bcch->gprs.ra_colour) &&
         scell_info->gsm.gprs_indicator &&
       ...
       const gas_id_t gas_id)
{
  ...
  {   
    MSG_GERAN_HIGH_3_G("scell_threshold_db=%d scell_threshold_rxlev=%d scell rxlev=%d (greater than threshold)",
  ...
  MSG_GERAN_HIGH_4_G("scell_threshold_db=%d scell_threshold_rxlev=%d scell rxlev=%d (valid=%d)",
                      scell_threshold_db, scell_threshold_rxlev,
                      (int)local_gprs_reselection_list_ptr->serving_values.RXLEV_average,
...
)
{
  ...
  /*Required only for GPRS attach UE. i.e. Service domain is CS+PS */
  if ( (grr_is_ps_allowed_by_service_domain(gas_id)) &&
       (ptr_ncell_info->gprs.ra_colour != scell_info->bcch_sys_info.ra_colour) &&
       scell_info->gsm.gprs_indicator &&
     ...
     )
  {
    ...
    /******************************************************************/
    CELL_RESELECT_HYSTERESIS_DB = 0;
    MSG_GERAN_MED_0_G("Same LA/RA, no CRH used");
  }
  else
  {
    /*********************************************************************/
    ...
    MSG_GERAN_MED_1_G("Using a CRH value of %ddB", CELL_RESELECT_HYSTERESIS_DB);
  ...
  MSG_GERAN_MED_3_G("sur C2:%d srv C2:%d CRH:%d", surroundingC2, servingC2, CELL_RESELECT_HYSTERESIS_DB);
...
#ifdef FEATURE_FAKE_BTS_CELL_DEPRIORITIZE
/*********************************************************************************
 *   Function name:  rr_check_scell_deprioritized_and_reduce_c2
 *   --------------------------
 *
 *   Description: This function is used to check whether SCell is in deprioritized
                  cell list or not. If cell is cell is present in deprioritized 
                  cell list then reduces the C2 value by penalty
 *
 *   Parameters: C2, gas_id
 *   -----------
 *
 *   Returns:  none
 *   --------
 *
 *********************************************************************************/
void rr_check_cell_deprioritized_and_reduce_c2(
  ARFCN_T arfcn,
  BSIC_T bsic,
  LAI_T lai,
  word ci,
...
)
{
  if (rr_get_fake_bts_cell_deprioritization_enabled(gas_id) == TRUE)
  {
    ...
    if (rr_is_cell_in_deprioritized_cell_list(arfcn, bsic, &cgi, NULL, gas_id))
    {
      /*reduce the C2 by penalty amount */
      *C2 -= RR_DEPRIORITEZED_CELL_C2_PENALTY;
      MSG_GERAN_HIGH_2_G("C2 with penalty amount deduction for ARFCN(%d) = %d", arfcn.num, *C2);
    }
  }
}
#endif/*FEATURE_GCELL_DEPRIORITIZING*/
