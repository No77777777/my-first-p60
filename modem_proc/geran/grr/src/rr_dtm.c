...
============================================================================*/
/* $Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_dtm.c#2 $ */
...
static boolean process_dtm_information(const gas_id_t gas_id)
{
  ...
  if (decoding_ok)
  {
    ...
    cell_identity_valid_flag = cell_info->gsm.cell_identity_valid_flag;

    if (!cell_identity_valid_flag || (cell_info->gsm.cell_identity != (word) dtm_information_details.cell_identity) )
    {
      log_cell_info = TRUE;
    }
    ...
    if (log_cell_info)
    {
      ...
      scell_info = geran_eng_mode_format_scell_info(
        cell_info,    // scell_info_ptr
        ENG_MODE_NO_CAUSE,   // cause
        gas_id               // gas_id
      );
      
      MSG_GERAN_HIGH_3_G("DTM Information Update Serving Cell Params to CM  LAC = %d, RAC = %d, Cell Identity =%d",
        cell_info->gsm.location_area_identification.location_area_code, cell_info->bcch_sys_info.routing_area_code, cell_info->gsm.cell_identity);
	  
      scell_info.rac = routing_area_id.routing_area_code;
        ...
        switch ( event_in )
        {
          ...
          case EV_INPUT_MESSAGE:
          {
                ...
                (message_id == (byte) MPH_DTM_CHANNEL_RELEASE_IND))
            {
              ...
              if (dtm_channel_release_ind->error_code == DTM_ASS_FAILED)
              {
                // Clear any saved UL request
                save_mac_grr_ul_tbf_req(NULL, gas_id);
...
/**
 * Determines whether a uplink tbf request can be processed in current dtm_assignmet_state
 *
 * @param gas_id
 *
 * @return TRUE/FALSE
...
boolean rr_dtm_can_process_ul_tbf_req(const gas_id_t gas_id)
{
  ...
  if (rr_dtm_data_ptr != NULL)
  {
    if (rr_dtm_data_ptr->data_ptr != NULL)
    {
      if (rr_dtm_data_ptr->data_ptr->substate.assign_command_control == DTM_ASSIGN_CMD_RESUMING_LINK_AFTER_REJECT)
      {
        MSG_GERAN_HIGH_0_G("assign_command_control is DTM_ASSIGN_CMD_RESUMING_LINK_AFTER_REJECT");
        process = FALSE;
      }
    }
  }
        ...
        switch ( event_in )
        {
          case EV_INPUT_MESSAGE:
          {
            if ( (message_set == MS_RR_L1) &&
                 (message_id == (byte) MPH_DTM_CHANNEL_RELEASE_IND) )
            {
              mph_dtm_channel_release_ind_t* dtm_channel_release_ind = (mph_dtm_channel_release_ind_t *) message_ptr;
              ...
              if (dtm_channel_release_ind->error_code == DTM_ASS_FAILED)
              {
                ...
                send_dtm_assign_failure_ota( DTM_ASS_FAILED, FALSE, TRUE, gas_id);
  
                rr_dtm_data_ptr->data_ptr->assign_command_control_data.ps_access_blocked = FALSE; 
                rr_dtm_data_ptr->data_ptr->substate.assign_command_control = DTM_ASSIGN_CMD_RESUMING_LINK_AFTER_REJECT;
              }
            }
            else
            if ( (message_set == MS_RR_L1) &&
                 ...
                 (message_id == (byte) MAC_GRR_DTM_CHANNEL_ASSIGN_REJ) )
            {
              ...
              if (dtm_channel_assign_rej->mac_status == MAC_TRANSFER)
              {
                rr_dtm_data_ptr->data_ptr->assign_command_control_data.ps_access_blocked = FALSE;
              }
              else
        ...
        switch ( event_in )
        {
          case EV_INPUT_MESSAGE:
          {
                 ...
                 (message_id == (byte) DL_RESUME_CNF) )
            {
              ...
              {
                rr_band_avoid_ded_freq_upd_send_imsg(
                  &rr_dtm_data_ptr->dtm_assignment_details.cs_assign.channel_assignment.channel_description.frequency_list,
                  gas_id
                );
              }
      ...
      case DTM_CELL_DOES_NOT_SUPPORT_DTM:
      {
        switch ( event_in )
        {
          ...
          case EV_DTM_MSC_CHANGE_IND:
          {
            /*
            Update NAS as required
            There is no need to send a RR_SERVICE_IND_FROM_OTA_MSG here, as if the DTM support of the cell
            is updated, then an update will be sent then.
            Note: This also covers the case where a X2G HO has just occurred and SI6 hasn't been received yet,
            meaning the whole RR_SERIVCE_IND_FROM_OTA_MSG content isn't known (e.g. PLMN ID).
            */
            rr_msc_update(
        ...
        switch ( event_in )
        {
          ...
          case EV_DTM_PACKET_ASSIGNMENT_RECEIVED:
          {
            ...
            if (process_result != EV_DTM_ASSIGNMENT_ERROR)
            {
              ...
              // with GMM. T3142 is also stopped.
...
#ifdef FEATURE_QSH_MDUMP
QSH_MDUMP_FN_ATTR
void grr_mdump_rr_dtm(const gas_id_t gas_id)
{
  rr_dtm_static_data_t *rr_dtm_data_ptr = rr_dtm_get_data_ptr(gas_id);
  qsh_mdump_collect_high(rr_dtm_data_ptr, sizeof(rr_dtm_static_data_t));
}
