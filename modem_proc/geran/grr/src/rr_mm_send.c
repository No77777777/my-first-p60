...
$Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_mm_send.c#2 $ $DateTime: 2024/10/03 02:59:43 $ $Author: pwbldsvc $
 ...
 */
static rr_mm_send_data_t *rr_mm_send_get_data_ptr(const gas_id_t gas_id)
{
...
*/
static void rr_mm_service_available(sys_srv_capability_e_type service_capability, const gas_id_t gas_id)
{
...
/**
  @brief Updates the local copy of the service status & capability
*/
static void rr_mm_service_not_available(const gas_id_t gas_id)
{
  // Obtain a pointer to the module data
  rr_mm_send_data_t *rr_mm_send_data_ptr = rr_mm_send_get_data_ptr(gas_id);
...
/*!
 * \brief Sets the last cell information
 * 
 * \param arfcn (in)
 * \param bsic (in)
 * \param lai (in)
 * \param gas_id (in)
 */
static void rr_mm_send_set_last_cell_info(
  ARFCN_T arfcn,
  BSIC_T bsic,
  LAI_T lai,
...
)
{
  ...
  rr_mm_send_data_ptr->rr_previous_BCCH_ARFCN  = arfcn;
  rr_mm_send_data_ptr->rr_previous_BSIC = bsic;
  rr_mm_send_data_ptr->rr_previous_LAI = lai;

  rr_nv_set_last_camped_cell(arfcn, bsic, lai, gas_id);
 ...
 */
void rr_mm_send_reset_last_cell_info(const gas_id_t gas_id)
{
  // Obtain a pointer to the module data
  rr_mm_send_data_t *rr_mm_send_data_ptr = rr_mm_send_get_data_ptr(gas_id);
 ...
 */
void rr_set_previous_LAI(PLMN_id_T plmn_id, word lac, const gas_id_t gas_id)
{
  // Obtain a pointer to the module data
  rr_mm_send_data_t *rr_mm_send_data_ptr = rr_mm_send_get_data_ptr(gas_id);
...
*/
mm_as_service_state_T rr_mm_get_service_state(const gas_id_t gas_id)
{
  // Obtain a pointer to the module data
  return rr_mm_send_get_data_ptr(gas_id)->service_state;
}
 ...
 */
sys_srv_capability_e_type rr_mm_get_service_capability(const gas_id_t gas_id)
{
  // Obtain a pointer to the module data
  return rr_mm_send_get_data_ptr(gas_id)->service_capability;
}
 ...
 */
boolean rr_mm_diff_gprs_sys_info(mm_gprs_sys_info_T *gprs_sys_info_ptr, const gas_id_t gas_id)
{
  // Obtain a pointer to the module data
  rr_mm_send_data_t *rr_mm_send_data_ptr = rr_mm_send_get_data_ptr(gas_id);
 ...
 */
static void rr_send_message_to_nas(IMH_T *imh_ptr, const gas_id_t gas_id)
{
...
sys_band_class_e_type rr_gapi_get_band_indicator_for_nas(boolean in_service, const gas_id_t gas_id)
{
...
sys_band_class_e_type rr_ds_get_band_indicator_for_nas(boolean in_service, sys_modem_as_id_e_type as_id)
{
 ...
 * @param GAS ID
 */
void rr_send_abort_ind(byte RR_abort_cause, mm_rr_failure_cause_e_type MM_fail_cause, const gas_id_t gas_id)
{
 ...
 */
void rr_send_abort_cnf(mm_as_status_T status, const gas_id_t gas_id)
{
 ...
 */
void rr_send_deact_cnf(const gas_id_t gas_id)
{
 ...
 */
static void rr_send_ds_data_ind(byte *msg_ptr, byte msg_len, const gas_id_t gas_id)
{
 ...
 */
void rr_send_data_ind(byte *layer3_message, byte message_length, const gas_id_t gas_id)
{
#if defined(FEATURE_DUAL_SIM) || defined(FEATURE_TRIPLE_SIM)
 ...
 */
void rr_send_paging_ind(rr_page_domain_e domain, rr_identity_type_e identity_type, const gas_id_t gas_id)
{
 ...
 */
void rr_send_est_cnf(const gas_id_t gas_id)
{
...
)
{
       ...
       (sys_info_db_ptr != NULL) )
  {
    ...
    rr_mm_send_set_last_cell_info(
      sys_info_db_ptr->BCCH_ARFCN,   // arfcn
      sys_info_db_ptr->BSIC,         // bsic
      *lai_ptr,                      // lai
      gas_id                         // gas_id
    );
 ...
 */
void rr_send_plmn_select_cnf_no_service(mm_as_status_T error_code, const gas_id_t gas_id)
{
  rr_send_plmn_select_cnf(
 ...
 */
void rr_send_plmn_select_cnf_success(const gas_id_t gas_id)
{
 ...
 */
void rr_send_rel_ind(byte RR_cause, const gas_id_t gas_id)
{
...
void rr_sglte_srvcc_send_no_service_ind(const gas_id_t gas_id)
{
  rr_service_ind_T   rr_service_ind;
  RR_PREPARE_MM_MESSAGE(rr_service_ind, RR_SERVICE_IND);
  ...
  rr_service_ind.band_indicator = rr_get_band_indicator_for_nas(FALSE);
  ...
  return;
} /* rr_sglte_srvcc_send_no_service_ind */
...
void rr_send_est_ind(const gas_id_t gas_id)
{
...
boolean rr_abort_pending_rr_est_req(byte RR_abort_cause, const gas_id_t gas_id)
{
 ...
 */
void rr_abort_pending_rr_est_req_if_lai_changed(LAI_T *current_lai_ptr, const gas_id_t gas_id)
{
  // Obtain a pointer to the module data
  rr_mm_send_data_t *rr_mm_send_data_ptr = rr_mm_send_get_data_ptr(gas_id);
 ...
 */
void rr_send_no_service_ind(const gas_id_t gas_id)
{
  gprs_scell_info_t *scell_info_ptr = rr_gprs_get_scell_info(gas_id);
...
void rr_send_no_service_for_plmn_list(const gas_id_t gas_id)
{
  ...
  rr_mm_send_get_data_ptr(gas_id)->gprs_sys_info_present = FALSE;
...
)
{
  ...
  #error code not present
#else
  rr_sync_ind.ciphering_enabled = FALSE;
 ...
 */
void rr_send_stop_gsm_mode_cnf(mm_as_status_T status, const gas_id_t gas_id)
{
 ...
 *
 */
void rr_send_activation_ind(mm_as_status_T status, const gas_id_t gas_id)
{
...
)
{
  ...
  rr_cell_change_ind.as_id        = geran_map_gas_id_to_nas_id(gas_id);
 ...
 */
/*===========================================================================*/
void rr_send_cm_status_ind(const gas_id_t gas_id)
{
  cm_rpt_type *cmd_ptr;
 ...
 */
/*===========================================================================*/
void rr_send_cb_info(boolean cbch_present, const gas_id_t gas_id)
{
  cm_rpt_type *cmd_ptr;
  ...
  if (cmd_ptr != NULL)
  {
    ...
    /* sys_cb_info_s_type is a common interface, external to CM, */
    /* so subscription ID also needs to be in the signal content */
    cmd_ptr->asubs_id = geran_map_gas_id_to_nas_id(gas_id);
    cmd_ptr->cmd.cell_bc_info.subscription_id = cmd_ptr->asubs_id;
 ...
 *
 */
void rr_send_grr_gmm_gprs_suspension_ind(const gas_id_t gas_id)
{
...
void rr_send_rr_reselection_ind(const gas_id_t gas_id)
{
 ...
 *
 */
void rr_send_rr_gmm_gprs_service_cnf(const gas_id_t gas_id)
{
...
void rr_send_gtow_cco_complete_ind(const gas_id_t gas_id)
{
  ...
  return;
} /* end rr_send_gtow_cco_complete_ind() */

#endif /* FEATURE_INTERRAT_PCCO_GTOW */

#ifdef FEATURE_GPRS_PS_HANDOVER

void rr_send_ps_handover_ind(const gas_id_t gas_id)
{
...
*/
static void rr_send_cm_change_test_mode(cmtc_ue_mode_type_T mode_type, const gas_id_t gas_id)
{
...
void rr_ms_send_rr_ps_abort_cnf(const gas_id_t gas_id)
{
  ...
  RR_PREPARE_MM_MESSAGE(rr_ps_abort_cnf, RR_PS_ABORT_CNF);
 ...
 */
void rr_ms_send_rr_plmn_list_cnf(const rr_plmn_list_req_T *req_ptr, const mm_as_status_T status)
{
  rr_plmn_list_cnf_T rr_plmn_list_cnf;
 ...
 */
void rr_mm_send_multimode_subs_chgd_cnf(void)
{
  rr_multimode_subs_chgd_cnf_T rr_multimode_subs_chgd_cnf;

  RR_PREPARE_MM_MESSAGE(rr_multimode_subs_chgd_cnf, RR_MULTIMODE_SUBS_CHGD_CNF);
...
/*!
 * \brief Sends RR_GMM_RETX_PDU_TUNEAWAY_FAIL_IND to MM.
 */
void rr_gmm_retx_pdu_tuneaway_fail_ind(const gas_id_t gas_id)
{
  rr_gmm_retx_pdu_tuneaway_fail_ind_T rr_gmm_retx_pdu_tuneaway_fail_ind;

  RR_PREPARE_MM_MESSAGE(rr_gmm_retx_pdu_tuneaway_fail_ind, RR_GMM_RETX_PDU_TUNEAWAY_FAIL_IND);

  rr_gmm_retx_pdu_tuneaway_fail_ind.as_id = geran_map_gas_id_to_nas_id(gas_id);

  /* Fill the channel Mode and Channel Type for CS Connection */
  rr_send_message_to_nas((IMH_T *)&rr_gmm_retx_pdu_tuneaway_fail_ind, gas_id);

  return;
}

/*!
 * \brief Sends RR_PREEMPTION_IND to MM.
 */
void rr_mm_send_rr_preemption_ind(const gas_id_t gas_id)
{
  rr_preemption_ind_T rr_preemption_ind;
  rr_dedicated_data_T   *dedic_ptr;
  dedic_ptr = rr_get_dedicated_data_ptr(gas_id);

  RR_PREPARE_MM_MESSAGE(rr_preemption_ind, RR_PREEMPTION_IND);

  rr_preemption_ind.as_id = geran_map_gas_id_to_nas_id(gas_id);

  /* Fill the channel Mode and Channel Type for CS Connection */
  rr_preemption_ind.channel_mode = dedic_ptr->curr_channel_spec.channel_mode_1;
  rr_preemption_ind.channel_type = dedic_ptr->curr_channel_spec.channel_info_1_after.channel_type;
  rr_send_message_to_nas((IMH_T *)&rr_preemption_ind, gas_id);
 ...
 * \param gas_id (in)
 */
void rr_mm_send_rr_ps_status_change_ind(boolean ps_is_active, const gas_id_t gas_id)
{
...
/**
  ...
  Note: The primary purpose of this function is to send forbidden LAI to NAS
*/
void rr_mm_send_resel_fail_ind(LAI_T *lai_ptr, const gas_id_t gas_id)
{
  rr_plmn_resel_fail_ind_T rr_plmn_resel_fail_ind;

  RR_PREPARE_MM_MESSAGE(rr_plmn_resel_fail_ind, RR_PLMN_RESEL_FAIL_IND);

  MSG_GERAN_MED_1_G("Reselection FAILURE: LAI = 0x%04x is forbidden", lai_ptr->location_area_code);
  rr_plmn_resel_fail_ind.lai   = rr_convert_rr_lai_to_inter_task_lai(lai_ptr);
  rr_plmn_resel_fail_ind.as_id = geran_map_gas_id_to_nas_id(gas_id);

  rr_send_message_to_nas((IMH_T *)&rr_plmn_resel_fail_ind, gas_id);
} /* rr_send_resel_fail_ind  */
...
void rr_populate_gcell_info( geran_grr_qmi_cell_info_ind_t *grr_fake_cell_data_ptr, const gas_id_t gas_id )
{
  ...
  if ((scell_ptr->gsm.valid_data_flag == TRUE) && (scell_ptr->gsm.camped_on_flag == TRUE))
  {
    ...
    grr_fake_cell_data_ptr->cell_reselect_hysterisis = scell_ptr->gsm.cell_selection_parameters.CELL_RESELECT_HYSTERESIS;
    grr_fake_cell_data_ptr->ms_txpwr_max_cch = scell_ptr->gsm.cell_selection_parameters.MS_TXPWR_MAX_CCH;
    grr_fake_cell_data_ptr->temporary_offset = scell_ptr->gsm.cell_selection_parameters.TEMPORARY_OFFSET;
    grr_fake_cell_data_ptr->penalty_time = scell_ptr->gsm.cell_selection_parameters.PENALTY_TIME;
    grr_fake_cell_data_ptr->gsm_neighbor_cell_unique_arfcn_list.no_of_entries = scell_ptr->gsm.complete_BA_list.no_of_entries;

      for (index = 0; (index < grr_fake_cell_data_ptr->gsm_neighbor_cell_unique_arfcn_list.no_of_entries) && (index < MAX_NUM_GSM_NEIGH) ; index++)
      {
        grr_fake_cell_data_ptr->gsm_neighbor_cell_unique_arfcn_list.bcch_arfcn[index] = scell_ptr->gsm.complete_BA_list.BCCH_ARFCN[index].num;
      }
  ...
  else
  {
    ...
    grr_fake_cell_data_ptr->mandatory_si_status = FALSE;
    grr_fake_cell_data_ptr->cell_reselect_hysterisis = 0;
    grr_fake_cell_data_ptr->ms_txpwr_max_cch = 0;
    grr_fake_cell_data_ptr->temporary_offset = 0;
    grr_fake_cell_data_ptr->penalty_time = 0;
    grr_fake_cell_data_ptr->gsm_neighbor_cell_unique_arfcn_list.no_of_entries = 0;

      for (index = 0; index < BA_MAX_LEN; index++)
      {
        grr_fake_cell_data_ptr->gsm_neighbor_cell_unique_arfcn_list.bcch_arfcn[index] = 0;
      }
