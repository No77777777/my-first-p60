...
*/
/* $Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_data.c#2 $ */
...
static rr_ded_irat_measurements_t  wcdma_ded_meas[NUM_GERAN_DATA_SPACES];
...
/**
 * Downlink Signaling Counter (DSC) maintains a count of decoded paging blocks.
 * Used in Idle to detect a downlink signaling failure (move to general)
 */
static int  DSC_counter[NUM_GERAN_DATA_SPACES];
...
)
{
  ...
  if (si2ter_instances_ptrs[as_index] == NULL)
  {
    MSG_GERAN_FATAL_0_G("rr_get_SI2ter_instances_ptr() alloc failed");
  }
 ...
 *
 * \return surr_irat_meas_ind_T*
 */
rr_ded_irat_measurements_t *rr_get_wcdma_ded_meas_ptr(
  const gas_id_t gas_id
)
{
  uint8 as_index = GERAN_MAP_GAS_ID_TO_DATA_SPACE_INDEX(gas_id);
...
static void rr_data_free_SI2ter_instances_data(const gas_id_t gas_id)
{
  ...
  if (si2ter_instances_ptrs[as_index] != NULL)
  {
    ...
    {
      if (si2ter_instances_ptrs[as_index]->instance[i] != NULL)
      {
        GPRS_MEM_FREE(si2ter_instances_ptrs[as_index]->instance[i]);
        si2ter_instances_ptrs[as_index]->instance[i] = NULL;
      }
    }
...
static void rr_data_free_SI2quater_instances_data(const gas_id_t gas_id)
{
  ...
  if (si2q_instances_ptrs[as_index] != NULL)
  {
    ...
    {
      if (si2q_instances_ptrs[as_index]->instance[i] != NULL)
      {
        /* Free the memory allocated dynamically for structures assoiciated with si2q_params */
        rr_si2q_params_free_children(&si2q_instances_ptrs[as_index]->instance[i]->si2q_mi.si2q_params);
        GPRS_MEM_FREE(si2q_instances_ptrs[as_index]->instance[i]);
        si2q_instances_ptrs[as_index]->instance[i] = NULL;
      }
    }
...
static void rr_data_free_mi_instances_data(const gas_id_t gas_id)
{
  ...
  if (mi_instances_ptrs[as_index] != NULL)
  {
    ...
    for (i = 0; i < MAX_INSTANCES; ++i)
    {
      if (mi_instances_ptrs[as_index]->instance[i] != NULL)
      {
        MSG_GERAN_HIGH_1_G("Freeing memory for instance %d of MI", i);
        GPRS_MEM_FREE(mi_instances_ptrs[as_index]->instance[i]);
        mi_instances_ptrs[as_index]->instance[i] = NULL;
      }
    }
...
/*!
 * \brief Returns a poitner to DSC_counter
 *
 * \return int*
 */
int *rr_get_DSC_counter_ptr(
  const gas_id_t gas_id
)
{
  uint8 as_index = GERAN_MAP_GAS_ID_TO_DATA_SPACE_INDEX(gas_id);

  return (&DSC_counter[as_index]);
}
...
#ifdef FEATURE_QSH_MDUMP
QSH_MDUMP_FN_ATTR
void grr_mdump_rr_plmn_list_req(const gas_id_t gas_id)
{
  rr_PLMN_list_req_info_T *rr_PLMN_list_req_info_ptr = rr_get_plmn_list_req_info_ptr(gas_id);
  qsh_mdump_collect_high(rr_PLMN_list_req_info_ptr, sizeof(rr_PLMN_list_req_info_T));
}
