...
$Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_general.h#5 $ $DateTime: 2020/09/18 02:47:25 $ $Author: pwbldsvc $
...
#define RR_BPLMN_TOT_FREQ_MONITOR_GSM_900          200
#define RR_BPLMN_MAX_FREQ_MONITOR_GSM_900          200

#define RR_BPLMN_TOT_FREQ_MONITOR_DCS_1800         200
#define RR_BPLMN_MAX_FREQ_MONITOR_DCS_1800         200

#define RR_BPLMN_TOT_FREQ_MONITOR_GSM_850          200
#define RR_BPLMN_MAX_FREQ_MONITOR_GSM_850          200

#define RR_BPLMN_TOT_FREQ_MONITOR_PCS_1900         200
#define RR_BPLMN_MAX_FREQ_MONITOR_PCS_1900         200
...
#define RR_DEFAULT_SAMPLE_RATE  8000
#define RR_ENHANCED_SAMPLE_RATE 16000
...
{
  uint8  count;
  uint32 earfcns[MAX_RR_EUTRAN_NEIGHBOUR_LIST_SIZE]; 
}rr_general_earfcn_list;

#ifdef FEATURE_PRIORITIZE_LTE_FREQ

/* In case LTE_RRC_CSP_MAX_ACQ_DB_EXT_ENTRIES is not defined in LTE APIs */
#ifndef LTE_RRC_CSP_MAX_ACQ_DB_EXT_ENTRIES
#define LTE_RRC_CSP_MAX_ACQ_DB_EXT_ENTRIES 10
#endif

typedef struct
{
  uint8                 earfcn_count;
  lte_rrc_acq_db_list_s earfcn_list[LTE_RRC_CSP_MAX_ACQ_DB_EXT_ENTRIES];
}rr_general_lte_acq_db_earfcn_s;
#endif /* FEATURE_PRIORITIZE_LTE_FREQ */
...
void rr_add_to_barred_cell_list(rr_barred_cell_info_T *barred_cell_ptr, const gas_id_t gas_id);
...
boolean rr_is_cell_in_deprioritized_cell_list(ARFCN_T arfcn, BSIC_T bsic, rr_csi_cgi_t *cgi, uint8 *cell_index, const gas_id_t gas_id);
...
rr_l1_dtx_indicator_T rr_get_DTX_indicator(const gas_id_t gas_id);
...
/*+452757 ASR*/
extern boolean rr_SI2q_MI_all_instances_received(
...
/*!
 * \brief checks if the given procedure type is for PS procedure or not
 * 
 * \param procedure type
 *
 * \return boolean - TRUE if the procedure type is for PS, FALSE otherwise
 */
extern boolean rr_is_procedure_for_ps(sys_proc_type_e_type proc_type, const gas_id_t gas_id );
...
void rr_initialize_wcdma_ded_measurements(rr_ded_irat_measurements_t *wcdma_ded_meas_ptr);
...
extern boolean rr_is_manual_plmn_selection_mode(const gas_id_t gas_id);
...
/*==========================================================================
  ...
  DESCRIPTION
  Updates the Eng Mode Data for Voice Sample Rate when the change in notified from L1.
  ...
  SIDE EFFECTS
    None.
===========================================================================*/
extern void rr_eng_mode_update_voice_sample_rate(uint32 sample_rate, const gas_id_t gas_id);
...
/**
  @brief Extracts the priority for the ARFCN/RAT combination from the priority list passed.

  @return uint8 priority.
*/
extern uint8 rr_extract_priority_from_prority_list(
  rr_l1_irat_frequency_priority_list_t priority_list,
  uint32 arfcn, 
  rr_l1_irat_rat_e rat);

/**
  @brief Determines the priority parameters for the UARFCN-RAT type passed.

  @return void.
*/
extern void rr_get_priority_params_for_uarfcn (
  uint16 uarfcn,
 ...
 * \return void
 */
extern void rr_get_mcc_mnc(sys_plmn_id_s_type  plmn,
                           sys_mcc_type        *mcc,
                           sys_mnc_type        *mnc);

#ifdef FEATURE_FAST_G2L_PSEUDO_SI2Q
extern boolean rr_lte_neighbour_list_has_valid_earfcn(const gas_id_t gas_id);
...
extern boolean rr_fcell_is_lai_in_efs_f_cells_list(LAI_T lai, const gas_id_t gas_id);

/*!
 * \brief Returns TRUE or FALSE based on the presence of the cell in EFS F-Cell list
 * 
 * \return TRUE/FALSE
 */

extern boolean rr_fcell_is_cell_in_efs_f_cells_list(LAI_T lai,uint16 cell_id);

extern boolean rr_fcell_is_cell_available(LAI_T lai, uint16 cell_id, const gas_id_t gas_id);

/*!
 * \brief Returns TRUE or FALSE based on the Requested PLMN match with F-Cell PLMN
 * 
 * \return TRUE/FALSE
 */

extern boolean rr_is_plmn_in_efs_f_cells_list(PLMN_id_T plmn_id);
 ...
 * \return void
 */
extern void rr_update_hst_freqs_to_l1(const gas_id_t gas_id);
...
DESCRIPTION
  This function is called to find all freq can be decoded or not.
...
extern boolean rr_bcch_decode_all_freq_required(rr_pscan_results_db_T *pscan_db_ptr, const gas_id_t gas_id);
#endif
#ifdef PERLUTF
...
/**
  ...
  return None
*/
extern void rr_utran_frequency_list_create(rr_utran_frequency_list_t * dst,const gas_id_t gas_id);
/**
...
*/
extern uint32 rr_utran_frequency_list_search(rr_utran_frequency_list_t * utran_frequency_list,rr_utran_frequency_list_entry_type_e type,uint16 uarfcn);

/**
  @brief This function stores Serving Cell and UTRAN Priority Params from NV Data Base.
  return None
*/
extern void rr_override_scell_si2q_params(
  gprs_scell_info_t *serving_cell_ptr, 
  const gas_id_t gas_id
);

/**
  @brief This function returns the persisten page collision flag.
  return boolean
*/
extern boolean rr_get_fallback_flag(
  const gas_id_t gas_id
);

/**
  @brief This function sets the persisten page collision flag to TRUE.
  return None
*/
extern void rr_set_fallback_data( 
  mph_idle_coll_fallback_ind_T *mph_idle_coll_fallback_ind,
  const gas_id_t gas_id
);

/**
  @brief This function handles the persisten page collision indication message from GL1.
  return None
*/
extern void rr_handle_persistent_page_collison_ind( 
  mph_idle_coll_fallback_ind_T *mph_idle_coll_fallback_ind, 
  const gas_id_t gas_id
);

/**
  @brief This function resets the persisten page collision flag to FALSE.
  return None
*/
extern void rr_reset_fallback_data(
  const gas_id_t gas_id
);

/**
  @brief This function checks whether the MCC camped on is valid for persisten page collision.
  return None
*/
extern void rr_mcc_fallback_allowed( 
  const gas_id_t gas_id
);

/**
  @brief This function sets the rr_reset_mcc_china_valid to COLL_FALLBACK_UNKNOWN.
  return None
*/
extern void rr_reset_coll_fallback_valid(
  const gas_id_t gas_id
);
...
/**
  ...
  return None
*/
extern void rr_set_modified_rat_priority( 
...
/**
  ...
  return None
*/
extern rr_resel_priority_e rr_get_modified_rat_priority( 
  rr_rat_e rat_type, 
  const gas_id_t gas_id
);

/**
  @brief This function returns modified priority of a given earfcn.
  return rr_resel_priority_e
*/
extern rr_resel_priority_e rr_get_modified_earfcn_priority(uint8 prio_orig, const gas_id_t gas_id);

/**
  @brief This function generates the min and max priority range from the earfcn list.
  return None
*/
extern void rr_genereate_pmin_pmax_priority_for_earfcn_priority_list(
  const gas_id_t gas_id
);

/**
  @brief This function returns the roundedoff value of a double variable.
  return uint8
*/
extern uint8 rr_get_round_val(
  double x
);

/**
  @brief This function returns the fallback preferred mode.
  return trm_idle_coll_fallback_enum_t
*/
extern trm_idle_coll_fallback_enum_t rr_get_fallback_pref_mode(
  const gas_id_t gas_id
);


extern boolean rr_is_geran_priority_higher_than_lte(gprs_scell_info_t *cell);
extern void rr_increase_lte_priority_over_geran(gprs_scell_info_t *cell, const gas_id_t gas_id);
#ifdef FEATURE_PRIORITIZE_LTE_FREQ
extern void rr_general_send_lte_rrc_acq_db_earfcn_req(const gas_id_t gas_id);
extern void rr_general_store_lte_acq_db(lte_rrc_acq_db_earfcn_rsp_s *lte_acq_db_earfcn_rsp, const gas_id_t gas_id);
extern void rr_general_add_lte_acq_db_earfcns(rr_general_earfcn_list *earfcn_list, uint8 max_lte_frequencies, sys_plmn_id_s_type camped_plmn, const gas_id_t gas_id);
extern void rr_general_add_ncell_earfcns(rr_general_earfcn_list *earfcn_list, uint8 max_lte_frequencies, const gas_id_t gas_id);
extern void rr_general_prioritize_lte_frequencies(const gas_id_t gas_id);
extern void rr_general_stop_lte_prioritization(const gas_id_t gas_id);
extern boolean rr_general_prioritize_lte_meas_enabled(const gas_id_t gas_id);
extern void rr_general_check_and_prioritize_lte_frequencies(const gas_id_t gas_id);
#endif /* FEATURE_PRIORITIZE_LTE_FREQ */
