...
============================================================================*/
/* $Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_gprs_debug.h#2 $ */
...
#define LOG_UNEXPECTED_TIMER_EXPIRY(xx_timer, xx_state, xx_state_name_fn) \
  MSG_SPRINTF_2(MSG_SSID_DFLT, \
                MSG_LEGACY_ERROR, \
                "Unexpected timer %d expiry in state %s", \
                xx_timer, \
                xx_state_name_fn(xx_state))
#else
#define LOG_UNEXPECTED_TIMER_EXPIRY(xx_timer, xx_state, xx_state_name_fn) \
  MSG_GERAN_ERROR_2("Unexpected timer %d expiry in state %d", \
                    xx_timer, \
                    xx_state)
...
#else /* FEATURE_GERAN_REDUCED_SPRINTF */

#ifdef FEATURE_DUAL_SIM
#define RR_LOG_STATE_TRANSITION(xx_state_machine, xx_old_state, xx_new_state, xx_state_name_fn, xx_gas_id) \
  MSG_3(MSG_SSID_DFLT, \
        MSG_LEGACY_HIGH, \
        "gs%d:RRState: " xx_state_machine "(%d->%d)", \
        (int)(xx_gas_id+1), \
        xx_old_state, \
        xx_new_state); \
  NOTUSED(xx_state_name_fn)
#else
#define RR_LOG_STATE_TRANSITION(xx_state_machine, xx_old_state, xx_new_state, xx_state_name_fn, xx_gas_id) \
   MSG_2(MSG_SSID_DFLT, \
         MSG_LEGACY_HIGH, \
...
extern void rr_debug_print_sys_band_mask(char *descr_text, sys_band_mask_type band_mask);

extern void rr_debug_print_sys_band(char *descr_text, sys_band_T band);

extern void rr_debug_print_candidate_cell(const rr_candidate_cell_T *candidate_cell_ptr);

extern void rr_debug_print_candidate_cell_db(const rr_candidate_cell_db_T *candidate_cell_db_ptr);

#ifdef FEATURE_QSH_MDUMP
QSH_MDUMP_FN_ATTR
extern void grr_mdump_rr_gprs_debug(const gas_id_t gas_id);
