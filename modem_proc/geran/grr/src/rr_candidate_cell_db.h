...
*/
/* $Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_candidate_cell_db.h#3 $ */
...
#define RR_NON_CANDIDATE_CELL (RR_MAX_CANDIDATE_FREQ_LIST_BPLMN_SRCH+1)
...
typedef enum
{
  ...
  RR_CANDIDATE_DENIED_BAND,
  RR_CANDIDATE_MAX_COUNT
} rr_candidate_freq_type_e_t;
...
typedef struct
{
  ...
  boolean                          processed;
  boolean                          bcch_fail_due_to_rsrc_denial;
} rr_candidate_cell_T;

typedef struct
{
  rr_candidate_cell_T candidate_cells[RR_MAX_CANDIDATE_FREQ_LIST_BPLMN_SRCH];
...
typedef rr_candidate_cell_db_T rr_candidate_cell_pend_db_T;
...
extern rr_candidate_cell_db_T *rr_get_candidate_cell_db_ptr(const gas_id_t gas_id);

extern rr_available_plmn_db_T *rr_get_available_plmn_db_ptr(const gas_id_t gas_id);

extern void rr_candidate_cell_db_init(const gas_id_t gas_id);

extern void rr_candidate_db_clear(rr_candidate_cell_db_T *candidate_db_ptr);

extern void rr_candidate_db_set_valid(rr_candidate_cell_db_T *candidate_db_ptr);

extern void rr_candidate_db_set_invalid(rr_candidate_cell_db_T *candidate_db_ptr);

extern void rr_candidate_db_clear_offset(rr_candidate_cell_db_T *candidate_db_ptr);

extern uint16 rr_candidate_db_get_power_scan_offset(const rr_candidate_cell_db_T *candidate_db_ptr);

extern uint16 rr_candidate_db_get_num_frequencies(const rr_candidate_cell_db_T *candidate_db_ptr);

extern uint16 rr_candidate_db_get_num_unprocessed_cell(const rr_candidate_cell_db_T *candidate_db_ptr);
...
extern boolean rr_candidate_db_is_candidate_cell(ARFCN_T BCCH_ARFCN, const gas_id_t gas_id);
...
extern void rr_candidate_db_mark_invalid_entries_as_processed(rr_candidate_cell_db_T *candidate_db_ptr);
...
extern boolean rr_candidate_db_fcch_snr_high_priority_find_next(
  rr_candidate_cell_db_T           *candidate_db_ptr,
  rr_candidate_cell_T             **entry_ptr_ptr,
  const gas_id_t                    gas_id
);

extern boolean rr_candidate_db_find_high_priority_processed_suitable_cell(
  rr_candidate_cell_db_T           *candidate_db_ptr,
  rr_candidate_cell_T             **entry_ptr_ptr,
  const gas_id_t                    gas_id
);
...
extern void rr_candidate_db_mark_valid_entries_as_unprocessed(rr_candidate_cell_db_T *candidate_db_ptr);

extern void rr_set_decode_cnf_received(boolean cnf_received, const gas_id_t gas_id);

extern boolean rr_candidate_db_decode_bcch_complete(const gas_id_t gas_id);
...
extern void rr_candidate_db_find_best_of_fcch_snr_for_low_prior_cells(
  rr_candidate_cell_db_T *candidate_db_ptr,
  boolean                *best_cell_found,
  uint16                 *best_cell_index,
  const gas_id_t          gas_id
...
extern void rr_candidate_set_denied_bands(
  rr_candidate_cell_db_T *candidate_db_ptr,
  sys_band_mask_type denied_bands
);

extern void rr_candidate_reset_denied_bands(rr_candidate_cell_db_T *candidate_db_ptr);

extern rr_event_T rr_perform_bcch_list_decode_from_candidate_db(
  rr_candidate_cell_db_T *candidate_db_ptr,
  const gas_id_t gas_id
);
...
extern boolean rr_candidate_db_send_decode_bcch_start_ind(
...
extern uint16 rr_candidate_db_get_num_unknown_cell(const rr_candidate_cell_db_T *candidate_db_ptr);
...
extern void rr_ms_candidate_db_search_timeout(const gas_id_t gas_id);
...
extern boolean  rr_candidate_db_failure_due_to_no_resource(
  const gas_id_t gas_id
);

extern uint16 rr_candidate_db_hash_arfcn(ARFCN_T arfcn);
#ifdef FEATURE_GSM_DECODE_ALL_FREQS
extern rr_candidate_cell_pend_db_T *rr_get_candidate_cell_pend_db_ptr(const gas_id_t gas_id);
extern void rr_copy_pend_db_to_candidate_db(const gas_id_t gas_id);

extern void rr_candidate_db_copy_data(
...
QSH_MDUMP_FN_ATTR
extern void grr_mdump_rr_decode_bcch(const gas_id_t gas_id);
