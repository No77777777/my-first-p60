...
$Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_log_types.h#1 $
...
typedef enum
{
  ...
  RR_RESELECTION_TRIGGER_GSM_TO_LTE_FAILED = 0x1d,
  /*!< Inter-RAT G2L reselection has failed, reselection back to GSM is
  triggered. */

  RR_RESELECTION_TRIGGER_GSM_TO_TDD_FAILED = 0x1e,
  /*!< Inter-RAT G2T reselection has failed, reselection back to GSM is
  triggered. */
...
typedef enum
{
   ...
   */
  RR_LOG_RES_ABORT_UL_ACCESS_REJECT_ON_TARGET   = 0x8,

  /**
   * During cell update or routing update, the network can force MS to standby mode
   */
  RR_LOG_RES_ABORT_FORCED_TO_STANDBY_ON_TARGET  = 0x9,

  RR_LOG_RES_FAILURE_NO_PBCCH_FOUND             = 0xA,

  RR_LOG_X2G_RESEL_FAILURE_UNDEFINED            = 0XB
  ...
  RR_LOG_G2G_RESEL_STATUS_SUCCESS = 0xFD,
...
typedef enum
{
  RR_LOG_IDLE_CELL_UPDATE_LIST_REQ = 0x0,
  RR_LOG_DED_CELL_UPDATE_LIST_REQ
} rr_log_cell_update_list_req_type_T;
...
/* LOG_GSM_RR_SI2QUATER_RESELECTION_PARAMETERS_C */
#define RR_LOG_MAX_3G_FREQS  12
#define RR_LOG_MAX_3G_CELLS  50
#define RR_LOG_MAX_LTE_FREQS 10

#define RR_LOG_MAX_CSI_PER_CGI 10
...
typedef PACK(struct)
{
  ...
  uint8  thresh_utran_high;
  uint8  thresh_utran_low;
  uint8  utran_qrxlevmin;
} rr_log_3g_freq_list;
...
typedef PACK(struct)
{
  ...
  PACK(struct)
  {
    ...
    uint8 default_thresh_utran;
    uint8 default_qrxlevmin;
    uint8 num_3g_frequencies;
    rr_log_3g_freq_list utran_freq_list[RR_LOG_MAX_3G_FREQS];
    uint8 num_3g_cells;
    rr_log_3g_cell_list utran_cell_list[RR_LOG_MAX_3G_CELLS];
  } utran_params;
  PACK(struct)
  {
    uint8 eutran_start;
    uint8 eutran_stop;
    uint8 eutran_ccn_active;
    uint8 num_lte_frequencies;
    rr_log_lte_freq_list lte_freq_list[RR_LOG_MAX_LTE_FREQS];
  } eutran_params;
} rr_log_si2q_resel_params_t;

/* LOG_GSM_RR_SI2QUATER_MEASUREMENT_PARAMETERS_C */
typedef PACK(struct)
{
  log_hdr_type hdr;
  uint8        version;
  ...
  PACK(struct)
  {
    ...
    uint8 eutran_fdd_measurement_report_offset;
    uint8 eutran_tdd_reporting_threshold;
    uint8 eutran_tdd_reporting_threshold_2;
    uint8 eutran_tdd_reporting_offset;
    uint8 eutran_tdd_measurement_report_offset;
    uint8 eutran_reporting_qranularity;
  } eutran_params;
} rr_log_si2q_meas_params_t;

/*Packet added to log GSM/ UTRAN/ EUTRAN measurement parameters*/
...
typedef PACK(struct)
{
  ...
  {  
    ...
    PACK(struct)
    {
      ...
      uint8         fdd_reporting_threshold; 
      uint8         fdd_reporting_threshold_2;
      uint8         fdd_measurement_report_offset; 
      uint8         fdd_reporting_offset;
      uint8         tdd_reporting_threshold; 
      uint8         tdd_reporting_threshold_2;
      uint8         tdd_measurement_report_offset; 
      uint8         tdd_reporting_offset;
      uint8         reporting_granularity;
    } eutran_meas_params;

  }rr_meas_params_t;
  
} rr_log_meas_params_t;

#define RR_LOG_MAX_ACQ_DB_ENTRIES 971
...
typedef PACK(struct)
{
  ...
  uint16       Cell_Id;
  uint8        Flush_cause;
  uint8        SI_requested_to_flush[RR_LOG_MAX_CSI_PER_CGI];
  uint8        SI_flushed[RR_LOG_MAX_CSI_PER_CGI];
...
#define RR_LOG_MAX_X2G_REDIR_GSM_CELL_DATA  32
typedef PACK(struct)
{
  log_hdr_type hdr;
  uint8        version;
  uint16       ARFCN[RR_LOG_MAX_X2G_REDIR_GSM_CELL_DATA];
  byte         num_entries;
} rr_log_x2g_redir_gsm_cell_list_info_t;
...
typedef PACK(struct)
{
  ...
  rr_log_ncell_list_t  complete_BA_list;
  uint8                CELL_RESELECT_HYSTERESIS;
  uint8                MS_TXPWR_MAX_CCH;
