...
*/
/* $Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_sys_info_pcell.c#1 $ */
...
)
{
  ...
  if ( ncell && ( ncell->bcch_or_pmo == BCCH_FORMAT ) )
  {
    ...
    if ( ncell_info->gsm.control_channel_description_valid )
    {
      ...
      pcell_info_ptr->gsm.control_channel_description = ncell_info->gsm.control_channel_description;
      pcell_info_ptr->gsm.location_area_identification = ncell_info->gsm.location_area_identification;
      pcell_info_ptr->bcch_sys_info.si_messages_not_broadcast = ncell_info->si_not_broadcast;
      pcell_info_ptr->bcch_sys_info.si_messages_on_bcch_ext = ncell_info->si_on_bcch_ext;
    }
    else
    {
    ...
    if (ncell_info->gsm.cell_id_valid)
    {
      ...
      pcell_info_ptr->gsm.cell_identity = (word)ncell_info->gsm.cell_id;
    }
