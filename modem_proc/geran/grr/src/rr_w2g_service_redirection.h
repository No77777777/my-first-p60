...
*/
/* $Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_w2g_service_redirection.h#2 $ */
...
extern boolean rr_x2g_redirection_in_progress_on_redirected_cell(
  const gas_id_t gas_id
);
...
/*!
 * \brief function is used to find the number of bsics for the cell. 
 *        if more than one bsic has configured for the cell, GRR sets INVALID BSIC In specific BCCH request. 
 * \param candidate cell is current camping cell.
 * \param gas_id 
 */

extern boolean rr_x2g_redirection_cell_has_mulitple_bsics(
...
QSH_MDUMP_FN_ATTR
extern void grr_mdump_rr_x2g_redir(const gas_id_t gas_id);
