...
$Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_g2w_handover.c#2 $ $DateTime: 2020/01/27 04:29:28 $ $Author: pwbldsvc $
...
static uint16 rr_get_PN_Offset(wcdma_cell_T *input_cell, rr_ded_irat_measurements_t *wcdma_ded_meas_ptr)
{
  byte i;
  byte num_entries;
  ...
  num_entries = wcdma_ded_meas_ptr->no_of_entries;

  for (i = 0; i < num_entries; ++i)
  {
    wcdma_cell_T *cell = &wcdma_ded_meas_ptr->entries[i].data.utran_fdd.cell;
    if (
    ...
    {
      return wcdma_ded_meas_ptr->entries[i].data.utran_fdd.PN_Offset;
    }
  }
...
#ifdef FEATURE_QSH_EVENT_NOTIFY_TO_QSH 
            rr_qsh_event_notify(RR_QSH_EVENT_G2W_HO_STARTED, gas_id);
#endif //FEATURE_QSH_EVENT_NOTIFY_TO_QSH
          }
          else
          {
...
#ifdef FEATURE_QSH_EVENT_NOTIFY_TO_QSH 
          rr_qsh_event_notify(RR_QSH_EVENT_G2W_HO_FAILURE, gas_id);
#endif //FEATURE_QSH_EVENT_NOTIFY_TO_QSH
...
#ifdef FEATURE_QSH_MDUMP
QSH_MDUMP_FN_ATTR
void grr_mdump_rr_g2w_handover(const gas_id_t gas_id)
{
  rr_g2w_handover_static_data_t *rr_g2w_handover_data_ptr = rr_g2w_handover_get_data_ptr(gas_id);
  qsh_mdump_collect_high(&rr_g2w_handover_data_ptr->g2w_ho_state, sizeof(rr_g2w_ho_state_T));
  qsh_mdump_collect_high(&rr_g2w_handover_data_ptr->old_g2w_ho_state, sizeof(rr_g2w_ho_state_T));
  qsh_mdump_collect_high(&rr_g2w_handover_data_ptr->rrc_failure_cause, sizeof(rr_rrc_failure_cause_e_type));
}
