...
$Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_sys_info.c#3 $ $DateTime: 2020/11/12 06:40:25 $ $Author: pwbldsvc $
...
/* define the amount of time it takes to receive n full sys info TC cycles:
 * this equates to ((n * (8 * 51)) * 4.615) milliseconds
 */
#define ONE_FULL_TC_CYCLE    1883
#define THREE_FULL_TC_CYCLES 5649
#define FOUR_FULL_TC_CYCLES  7532
...
/**
 * Flush the SIs which are not broadcast by network
 *
 * @param channel_number       ARFCN number  
 *
 * @param si2ter_broadcast     if TRUE then don't flush SI2ter
 *
 * @param si2quater_broadcast  if TRUE then don't flush SIquater
 *
 * @param si13_broadcast       if TRUE then don't flush SI13
 *
 * @param lai_ptr              pointer to location area code
 *
 * @param ci_ptr               pointer to cell_ID ptr 
 */
static void rr_sys_info_flush_not_broadcast_sis(
  ARFCN_T channel_number,
  boolean si2ter_broadcast, 
  boolean si2quater_broadcast,
  boolean si13_broadcast,
  LAI_T   *lai_ptr,
  uint16  *ci_ptr,
  const gas_id_t gas_id
)
{
  uint16 messages_not_broadcast = MASK_SI_NONE;  
  
  RR_NULL_CHECK_RETURN_VOID(lai_ptr);
  RR_NULL_CHECK_RETURN_VOID(ci_ptr);

  /* Flush SI2quater */
  if (!si2quater_broadcast) 
  {
    messages_not_broadcast |= MASK_SI_2_QUATER;
  }
  /* Flush SI2ter */  
  if (!si2ter_broadcast)
  {
    messages_not_broadcast |= MASK_SI_2_TER;
  }
  /* Flush SI13 */ 
  if(!si13_broadcast) 
  {
    messages_not_broadcast |= MASK_SI_13;
  }
  
  if (messages_not_broadcast != MASK_SI_NONE)
  {
    MSG_GERAN_MED_0_G("SIs not broadcast by n/w will be flushed");   
    rr_csi_flush_multiple_si_data(channel_number,              // ARFCN
                                  messages_not_broadcast,      // SI_MASK
                                  lai_ptr,                     // LAI
                                  ci_ptr,                      // CI
                                  CSI_FLUSH_SI_NOT_BROADCAST,  // FLUSH_CAUSE
                                  gas_id);
  }
  return;  
}
  ...
  else /* mobile station does not only support PGSM band */
  {
    if (EXT_IND_valid)
    {
      if (EXT_IND == 0)
      {
        ...
        si_db_ptr->decode_SI2bis = RR_SI_NOT_REQUIRED;
        MSG_GERAN_HIGH_0_G("SI2bis is not broadcasted");
      }
      else
      {
        /* SI2 bis is expected and should be decoded */
        si_db_ptr->decode_SI2bis = RR_SI_REQUIRED;
        MSG_GERAN_HIGH_0_G("SI2bis is broadcasted");
      }
    }
...
)
{
  ...
  if (si_db_ptr->valid_data_flag)
  {
    ...
    if (si_db_ptr->complete_BA_list.BA_IND != BCCH_frequency_list_ptr->BA_IND)
    {
      ...
      /* Flushing of changed SIs from the cache and adding them to required SI mask for acquisition
       * is taken care in IDLE mode during SI13 BCCH_CHANGE_MARK check. 
       * Flush the SIs here so that BA_IND change detected during AGCH monitoring will be taken care. */
      if (si_db_ptr->cell_identity_valid_flag)
      {
        MSG_GERAN_HIGH_0_G("BA_IND changed. Flush SI2, 2bis, 2ter");
        rr_csi_flush_multiple_si_data(si_db_ptr->BCCH_ARFCN,                     // ARFCN
                                      MASK_SI_2 | MASK_SI_2_BIS | MASK_SI_2_TER, // SI_MASK 
                                      &si_db_ptr->location_area_identification,  // LAI
                                      &si_db_ptr->cell_identity,                 // CI
                                      CSI_FLUSH_SI_CHANGE,                       // FLUSH_CAUSE 
                                      gas_id);
      }
  
      /* Setting BA_IND to invalid value will take care of adding these SIs to the required mask after starting IDLE mode. */
      si_db_ptr->SI2_BA_list.BA_IND    = INVALID_BA_IND;
      si_db_ptr->SI2bis_BA_list.BA_IND = INVALID_BA_IND;
      si_db_ptr->SI2ter_BA_list.BA_IND = INVALID_BA_IND;
  ...
  if (rr_si2ter_changed(si2ter_inst_ptr))
  {
        ...
        scell_info_ptr->gsm.wcdma_neighbor_list_si2ter.valid_data == TRUE)
    {
      ...
      if ( (si2ter_rest_rcvd == TRUE) && (scell_info_ptr->gsm.two_quater_position == SI2Q_POSITION_NONE) &&
            (scell_info_ptr->gsm.lte_neighbour_list.count == 0 || !rr_lte_neighbour_list_has_valid_earfcn(gas_id)) )
      {
  ...
  if (set_si2q_rcvd == TRUE)
  {
    ...
    if (rr_SI2q_MI_changed(si2q_inst_ptr))
    {
      ...
      if (rr_is_camped(gas_id) && !rr_gsm_only(gas_id))
      {
        ...
        if (sys_info_db_ptr->lte_neighbour_list.count == 0 || !rr_lte_neighbour_list_has_valid_earfcn(gas_id))
        {
    ...
    rr_reset_SI2q_MI_changed (si2q_inst_ptr, MAX_INSTANCES);
  }
  else
  {
    if (rr_SI2q_MI_changed(si2q_inst_ptr))
...
)
{
  ...
  /* First, a quick check on whether all MI instances have been
  ...
  if (rr_SI2q_MI_all_instances_received(mi_instances))
  {
    return TRUE;
  }
  ...
  for (i = 0; i < MAX_INSTANCES; ++i)
  {
    ...
    if (mi_instances->instance[i] &&
        mi_instances->instance[i]->valid_data &&
        mi_instances->instance[i]->si2q_mi.mi_params.eutran_params.eutran_start)
    {
      for (j = i; j < MAX_INSTANCES; ++j)
      {
        ...
        if (mi_instances->instance[j] &&
            mi_instances->instance[j]->valid_data)
        {
  ...
  if (decoding_ok == TRUE)
  {
    ...
    */
    ...
    if ((set_MI_received == TRUE) && (send_wcdma_neighbor_list_to_L1 == TRUE))
    {
      ...
      rr_dedicated_mode_process_params(scell_info_ptr, mi_inst_ptr, gas_id);
      ...
      rr_reset_SI2q_MI_changed (mi_inst_ptr, MAX_INSTANCES);
...
)
{
      ...
      if (current_IE->start_IE)
      {
        ...
        if (error_code == CSN_DECODE_NO_ERROR)
        {
          ...
          if (si3ro.si2quater_indicator_flag == valid)
          {
            ...
            {
              si2q_pos = BCCH_EXTENDED;
            }
            else
            {
              si2q_pos = BCCH_NORMAL;
            }
          }
          else
          {
  ...
  if (decoding_ok == TRUE)
  {
    ...
    if ((!si2q_pi) || (!two_ti) || (!gprs_indicator_flag))
    {
      (void)rr_sys_info_flush_not_broadcast_sis(channel_number,
                                                two_ti,
                                                si2q_pi,
                                                gprs_indicator_flag,
                                                &location_area_identification,
    ...
    /* system_information_database store              */
    /**************************************************/
    if (rr_is_this_the_scell(channel_number, gas_id) && (! rr_plmn_search_in_progress(gas_id)))
    {
      gprs_scell_info_t *scell_info_ptr = rr_gprs_get_scell_info(gas_id);
      boolean notify_MM = FALSE;
      boolean notify_L1 = FALSE;
    ...
    if (rr_gprs_is_this_a_surround_ncell(channel_number, gas_id))
    {
      ...
      if (ncell_info_ptr)
      {
        ...
        ncell_info_ptr->si_not_broadcast = MASK_SI_NONE;
        ncell_info_ptr->si_on_bcch_ext = MASK_SI_NONE;

        if (FALSE == two_ti)
        {
          ncell_info_ptr->si_not_broadcast |= MASK_SI_2_TER;
          ncell_info_ptr->si_not_broadcast |= MASK_SI_2_TER_REST;
        }

        if (si2q_pi)
        {
          if (BCCH_EXTENDED == si2q_pos)
          {
            ncell_info_ptr->si_on_bcch_ext |= MASK_SI_2_QUATER;
          }
        }
        else
        {
          ncell_info_ptr->si_not_broadcast |= MASK_SI_2_QUATER;
        }
        ...
        if (gprs_indicator_flag)
        {
          ...
          if (BCCH_EXTENDED == gprs_indicator.si13_position)
          {
            ncell_info_ptr->si_on_bcch_ext |= MASK_SI_13;
          }
        }
        else
        {
          ncell_info_ptr->gprs_supported = FALSE;
          ncell_info_ptr->si_not_broadcast |= MASK_SI_13;
        }
...
)
{
  ...
  if (decoding_ok == TRUE)
  {
    ...
    if (rr_candidate_db_is_candidate_cell(channel_number, gas_id))
    {
      ...
      candidate_cell_ptr = rr_candidate_db_store_si_info(channel_number, &si_info, gas_id);
      *lai_not_in_fcell_list = (si_info.cell_id_valid || !rr_fcell_is_lai_in_efs_f_cells_list(si_info.lai, gas_id));

      /* The extra check here is to avoid considering the cell as acceptable in case SI4 is received before SI3,
         and there are valid entries configured in the EFS F-cell database.

         If LAI of the cell matches with an entry in the EFS F-cell database and if the cell_id_flag is not received
         in SI4, then wait till SI3 before making a decision about the acceptability of the cell. */
      if ((candidate_cell_ptr != NULL) && *lai_not_in_fcell_list)
      {
        rr_plmn_db_update(candidate_cell_ptr, channel_number, gas_id);
      }
...
#endif /*FEATURE_GSM_RR_CELL_CALLBACK*/
      /* Do not update SCELL_INFO to CM, During Handover Procedure */
...
)
{
  ...
  boolean lai_not_in_fcell_list = FALSE;
  ...
  if (message_ok == TRUE)
  {
    ...
    {
      /*Check whether SI13 was expected and received or not*/
      (void)rr_check_for_si13_reception(message_type, message_ptr->tc,scell_info_ptr,gas_id);
      (void)rr_check_for_optional_sI2bis_SI2ter_reception(message_type, message_ptr->tc,scell_info_ptr,gas_id);
      /*******************************************************************/
    ...
    {      
      ...
      /*Check whether SI13 was expected and received or not*/
      (void)rr_check_for_si13_reception(message_type, message_ptr->tc,pcell_info_ptr,gas_id);
      (void)rr_check_for_optional_sI2bis_SI2ter_reception(message_type, message_ptr->tc, pcell_info_ptr, gas_id);
      /*******************************************************************/
	...
	if ((message_ptr->tc == 2) || (message_ptr->tc == 6)|| (message_ptr->tc == 3)|| (message_ptr->tc == 7)) 
    {       
      if ((!(message_type == SYSTEM_INFORMATION_TYPE_3)) && (!(message_type == SYSTEM_INFORMATION_TYPE_4)))       
      { 
        MSG_GERAN_HIGH_3_G("ARFCN =%d:msg=%d,TC =%d;Incorrest SI received on expected TC",(int)channel_number.num,message_type,message_ptr->tc); 
        rr_candidate_db_set_type(rr_get_candidate_cell_db_ptr(gas_id), channel_number,RR_CANDIDATE_BCCH_WITH_DECODE_FAILURES); 
        return; 
      } 
    }
    ...
    switch (message_type)
    {
      ...
      {
        rr_handle_system_info_4(channel_number, &lai_not_in_fcell_list, message_ptr, gas_id);
    ...
    if (!dedicated_si_message)
    {
      ...
      if (rr_candidate_db_is_candidate_cell(channel_number, gas_id))
      {
        ...
        if ((message_type == SYSTEM_INFORMATION_TYPE_3)||((message_type == SYSTEM_INFORMATION_TYPE_4) && lai_not_in_fcell_list))
        {
       ...
       )
    {
      ...
      if (dedicated_si_message)
      {
           ...
           )
        {
         ...
         (void)rr_acq_db_add_arfcn(scell_info_ptr->gsm.BCCH_ARFCN, 
...
)
{
  ...
  else
  {
    ...
    if (&cell_info_ptr->gsm == &pcell_info_ptr->gsm)
    {
      ...
      else if (rr_timer_is_running(RR_GSM_SYS_INFO_TIMER, gas_id))
      {
        /**
         * If the RR_GSM_SYS_INFO_TIMER is running, then extend it to give enough time for SI2ter if 
         * there is insufficient time remaining. 
         *  
         * Note: SI acquisition procedures using legacy implementation use both RR_GSM_SYS_INFO_TIMER and 
         * RR_GPRS_SYS_INFO_TIMER. Once all GSM SIs (which includes SI2ter) are received, 
         * RR_GSM_SYS_INFO_TIMER is stopped, and if SI13 is not yet received, RR_GPRS_SYS_INFO_TIMER is 
         * started. Therefore it is only necessary to check here if RR_GSM_SYS_INFO_TIMER is running to 
         * know whether an extension may be required to receive SI2ter. 
         * Newer implementations use only RR_GSM_SYS_INFO_TIMER. The time-out is set to cover both GSM SIs 
         * plus SI13. There is no need to extend the timer.
         */

        uint32 time_remaining_ms;

        uint32 max_time_to_si2ter = 0;

        if (tc < 8) /* TC range is 0..7 on BCCH (TC=8 used for PACCH sys info) */
        {
          /* this was the first reception of SI3 on the pending channel
           * ensure that we wait long enough to acquire SI2ter
           */
          max_time_to_si2ter = THREE_FULL_TC_CYCLES;
        ...
        else if (CACHED_SYS_INFO_TC == tc)
        {
          max_time_to_si2ter = FOUR_FULL_TC_CYCLES;

          /* Find the time remaining for RR_GSM_SYS_INFO_TIMER */
          time_remaining_ms = (uint32)rr_timer_get_time_remaining(RR_GSM_SYS_INFO_TIMER, gas_id);
        }

        if (max_time_to_si2ter > 0)
        {
