...
============================================================================*/
/* $Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_grr51_grr52_control.c#3 $ */
...
/*Handles common SGLTE specific CCO Failure actions if SGLTE is active */
static boolean grr_handle_sglte_cco_failure( const gas_id_t gas_id )
{
  if (rr_is_sglte_mode_active(gas_id))
  {
    /* First, switch L1 to GSM-only mode.  Once this is done
     then remove PS support from service domain then send
     a CCO failure indication to the originating RAT.  */
    rr_sglte_set_rat_priority_list_gsm_only(gas_id);
    rr_grr_send_imsg_switch_to_cs_only_gsm_only_req(rr_sglte_x2g_cco_failed_cb_fn, gas_id );
    return (TRUE);
  }
  else
  {
...
static void grr_handle_pseduo_lte_list_update_ind( rr_pseudo_lte_list_update_ind_T* rr_pseudo_lte_list_upd_ptr, const gas_id_t gas_id  )
{
  ...
  {
    MSG_GERAN_HIGH_0_G("[LTE Prio] LTE prioritization is in effect");
    l1_update_required = FALSE;
  }
...
boolean grr_is_ps_enable_in_progress(const gas_id_t gas_id)
{
  grr_control_data_t *grr_control_data_ptr = grr_control_get_data_ptr(gas_id);
  
  return (grr_control_data_ptr->state == GRR_CAMPED_ENABLING_PS_ACCESS);
} /* end grr_is_ps_enable_in_progress() */

boolean grr_is_ps_disable_in_progress(const gas_id_t gas_id)
{
  grr_control_data_t *grr_control_data_ptr = grr_control_get_data_ptr(gas_id);
  
  return (grr_control_data_ptr->state == GRR_CAMPED_DISABLING_PS_ACCESS);
} /* end grr_is_ps_disable_in_progress() */
...
#else
        MSG_GERAN_HIGH_3_G("BQbad GRR:%d CR:%d DSC:%d",
                           grr_control_data_ptr->state,
                           rr_resel_current_state(gas_id),
                           rr_gprs_return_gprs_dsc_counter_value(gas_id));
  ...
  while (grr_control_event != EV_NO_EVENT)
  {
    switch (grr_control_data_ptr->state)
    {
      ...
      case GRR_NULL:
      {
            ...
            switch (new_message->rr.header.rr_message_set)
            {
              case RR_GRR_IMSG:
              {
                switch (new_message->rr.header.imh.message_id)
                {
                  ...
                  case RR_GRR_IMSG_FREQ_LIST_UPDATE_IND:
                  {
        ...
        /* Idle mode being done after RACH failure OTA due to no resource      */
        /***********************************************************************/
        if ( ( grr_control_event == RR_EV_CAMPED_ON_CELL ) ||
             ( grr_control_event == EV_GRR51_GO_TO_PACKET_IDLE )||
             ( grr_control_event == RR_EV_CAMP_ON_CELL_FOR_RACH_FAIL_OTA ))
        {

          if (grr_control_event != RR_EV_CAMP_ON_CELL_FOR_RACH_FAIL_OTA)
          {
            /*Initialise the DSC*/
        ...
        if ( (grr_control_event == RR_EV_TRIGGER_CELL_SELECTION) ||
             (grr_control_event == RR_EV_TRIGGER_CELL_SELECTION_ACQ_DB_ONLY) )
        {
        ...
        */
                  ...
                  (grr_control_event == RR_EV_START_RPS_AFTER_DENIAL))
        {
          rr_send_rr_reselection_ind(gas_id);
      ...
      case GRR_ACQUIRE:
      {
        ...
        if ( grr_control_event == EV_INPUT_MESSAGE )
        {
          ...
          else if (message_set == MS_RR_RR)
          {
            switch (new_message->rr.header.rr_message_set)
            {
              ...
              {
                (void)rr_gprs_bsi_acquisition_control(EV_INPUT_MESSAGE, new_message, gas_id);              
                break;
              }
          ...
          {
            new_grr_control_event =
              rr_gprs_bsi_acquisition_control(grr_control_event, new_message, gas_id);
          }
        ...
        else if (grr_control_event == EV_T3174_TIMEOUT)
        {
          ...
          {
#ifdef FEATURE_SGLTE
            if (grr_handle_sglte_cco_failure(gas_id))
            {                        
              grr_control_data_ptr->state = GRR_CAMPED;
            }
            else
              ...
              switch (new_message->rr.header.rr_message_set)
              {
                case RR_GRR_IMSG:
                {
                  switch (new_message->rr.header.imh.message_id)
                  {
                    ...
                    {
                      MSG_GERAN_MED_0_G("Already changing mode so ignored");
                      break;
                    }
                ...
                case RR_L1_IDLE_OMSG:
                {
                  switch (new_message->rr.header.imh.message_id)
                  {
                    case RR_L1_IDLE_OMSG_START_IDLE_CNF:
                    {
                      ...
                      /*Individual priorities are valid and PLMN selection is done  */
                      if ((grr_control_data_ptr->notify_mm_plmn_select_cnf) && ( rr_individual_priorities_valid(gas_id)))
                      {
                        sys_plmn_id_s_type requested_PLMN_id = rr_PLMN_request_details_ptr->requested_PLMN_id;
                        sys_plmn_id_s_type dedicated_plmn_id = rr_dedicated_priority_info_get_plmn(gas_id);

                        /*Stored individual priorities shall be deleted by the mobile station when any of the following occurs:*/
                        /* a PLMN selection is performed and results in a change of PLMN and the new PLMN is not an equivalent PLMN, see 3GPP TS 23.122*/

                        if (!(sys_plmn_match(requested_PLMN_id, dedicated_plmn_id)) &&
                            !(rr_eplmn_list_equivalent_plmn(requested_PLMN_id, gas_id))
                           )
                        {
                        ...
                        {
                          MSG_GERAN_MED_0_G("PLMN is same and is equivalent, continue with individual priority");
                        }
                      }
#endif /*FEATURE_LTE*/
...
//                      }
      ...
      case GRR_CAMPED:
      {
              ...
              switch (new_message->rr.header.rr_message_set)
              {
                case RR_GRR_IMSG:
                {
                  switch (new_message->rr.header.imh.message_id)
                  {
                    ...
                    case RR_GRR_IMSG_IRAT_ALLOWED_UPDATED_IND:
                    {
                      rr_l1_update_wcdma_neighbor_list(gas_id);
                      rr_resel_send_imsg(
                      ...
                      break;
                    } /* RR_GRR_IMSG_IRAT_ALLOWED_UPDATED_IND */
                    ...
                    case RR_GRR_IMSG_FREQ_LIST_UPDATE_IND:
                    {
                      rr_update_hst_freqs_to_l1(gas_id);
                      break;
                    }
#endif /* FEATURE_GERAN_HST_OPT */
                    case RR_GRR_IMSG_PS_ACCESS_REQ:
                    ...
                    case RR_GRR_IMSG_GPRS_IA_IND:
                    {
                      ...
                      if (rr_resel_reselection_is_pending(gas_id))
                      {
                        // Call the supplied call-back function indicating the IA should be ignored
                        if (imsg->cb_fn != NULL)
                        {
                          (*imsg->cb_fn)(
                            imsg->params,    // params
                      ...
                      else
                      {
                        if (rr_mac_ps_access_blocked(gas_id))
                        {
                          ...
                          // Re-post the RR_IMSG_GPRS_IA_IND, which will be blocked until PS is enabled
                          rr_send_imsg_gprs_ia_ind(imsg->params, imsg->cb_fn, gas_id);
                        }
                        else
                        {
                          // Call the supplied call-back function to send the IA to GMAC
                          if (imsg->cb_fn != NULL)
                          {
                            (*imsg->cb_fn)(
                              imsg->params,    // params
                          ...
                          {
                            MSG_GERAN_ERROR_0_G("cb_fn=NULL");
                          }
                        }
                      }
                      break;
                    ...
                    case RR_GRR_IMSG_REVERT_SCELL_INFO_IND:
                    {
                      gprs_scell_info_t *scell_ptr = rr_gprs_get_scell_info(gas_id);

                      MSG_GERAN_HIGH_0_G("Attempting to restore Serving Cell Info with Broadcast measurements");
                      ...
                      else if (grr_examine_saved_mac_message(&saved_message_id, gas_id))
                      {
                        ...
                        switch (saved_message_id)
                        {
                          case MAC_GRR_UL_TBF_REQ:
                          {
                            ...
                            /* If UE is in multistandby mode and other sub is in dedicated connection, start reselection scan.
                               So UE camps on best cell before attempting RACH once TRM becomes available. */
                            else if (rr_ms_rps_required_before_connection(gas_id) && (FALSE == rr_x2g_cco_in_progress(gas_id)))
                            {
                              new_grr_control_event = RR_EV_START_RESELECTION_POWER_SCAN;

                              MSG_GERAN_HIGH_0_G("RPS due to UL_TBF_REQ when other sub in dedicated");
                            }
        ...
        /***************************************************/
        else if ((grr_control_event == EV_DSF_RESELECTION) || (grr_control_event == RR_EV_START_RESELECTION_POWER_SCAN) ||
                 (grr_control_event == RR_EV_START_RPS_AFTER_DENIAL))
        {
#if defined(FEATURE_WCDMA) || defined(FEATURE_GSM_TDS) || defined(FEATURE_LTE)
          if (rr_x2g_cco_in_progress(gas_id))
        ...
        {
          new_grr_control_event = rr_gprs_bsi_acquisition_control(EV_GPRS_SUSPENDED_FOR_SI_ACQUISITION, NULL, gas_id);
        }
      ...
      case GRR_CONNECTION_PENDING:
      {
        if ( grr_control_event == EV_INPUT_MESSAGE )
        {
          ...
          /**********************************************************************/
          if (( message_set == MS_RR_L1 ) && 
              (message_id == (byte) MPH_SURROUND_IRAT_MEAS_IND))
          {         
             rr_irat_process_surround_irat_meas_ind(&new_message->mph.surround_irat_meas_ind,gas_id);
          } 
#endif /* FEATURE_LTE || FEATURE_GSM_TDS */ 
        ...
        /*****************************************************************/
        /* EV_FORCE_CELL_RESELECTION,  RR_EV_START_RPS_AFTER_DENIAL              */
        /* EV_DSF_RESELECTION and RR_EV_START_RESELECTION_POWER_SCAN    */
      ...
      case GRR_CELL_RESELECTION:
      {
        switch (grr_control_event)
        {
            ...
            switch (message_set)
            {
              case MS_RR_L1:
              {
                switch (message_id)
                {
                  ...
                  {
                    rr_candidate_update_FCCH_SCH(&(new_message->mph_fcch_sch_decode_ind), gas_id);
                    break;
                  }
                    ...
                    switch (new_message->rr.resel.omsg.header.imh.message_id)
                    {
...
#ifdef FEATURE_QSH_EVENT_NOTIFY_TO_QSH 
                        rr_qsh_event_notify(RR_QSH_EVENT_RESEL_FAILURE, gas_id);
#endif //FEATURE_QSH_EVENT_NOTIFY_TO_QSH
                        break;
                      }
                      ...
                      case RR_RESEL_OMSG_FINISHED_IND:
                      {
                        if (new_message->rr.resel.omsg.finished_ind.status == RR_RESEL_STATUS_OUT_OF_SERVICE)
                        {
                          ...
                          {
                            output_event = relinquish_control_to_rr(RR_EV_TRIGGER_CELL_SELECTION_ACQ_DB_ONLY, gas_id);
                          }
                ...
                switch (message_id)
                {
                  case MID_TIMER_EXPIRY:
                  {
                    ...
                    if (timer_expiry->timer_id == RR_GSM_SYS_INFO_TIMER ||
                        timer_expiry->timer_id == RR_GPRS_SYS_INFO_TIMER ||
          ...
          case RR_EV_START_RPS_AFTER_DENIAL:
                    ...
                    switch (new_message->rr.header.imh.message_id)
                    {
                      case RR_GRR_IMSG_X2G_CCO_COMPLETE_IND:
                      {
                        ...
                        if (grr_verify_saved_mm_message(RR_PLMN_SELECT_REQ, gas_id))
                        {
                          ...
                          if (rr_examine_saved_message(&saved_message, gas_id) == EV_INPUT_MESSAGE)
                          {
                            boolean no_selection_rqrd = FALSE;
                            new_grr_control_event = handle_rr_plmn_select_req(
                              &saved_message->rr_plmn_select_req,
                              &no_selection_rqrd,
                              gas_id
                            );
                            /*This state can be reached when PS raching is aborted by high priority NAS request 
                              but while handling RR_PLMN_SELECT_REQ it may turn out that no selection is required 
                              and hence GRR may move to camp state without actually disabling MAC which is 
                              waiting for IA leading to crash in RLC */

                            if (no_selection_rqrd)
                            {
                              send_ps_imsg_no_ps_access_req(FALSE, gas_id);
                              grr_control_data_ptr->state = GRR_CAMPED_DISABLING_PS_ACCESS;                               
                            }
                            else
                            {
...
)
{
  ...
  /* A general rule - block IRAT Search search if NO SI2q is receivedactive */
  if ((SYS_SERVICE_SEARCH_IRAT_PLMN == rr_plmn_list_req->service_search) &&
      (FALSE == rr_gsm_only(gas_id)) &&
      (SI2Q_POSITION_NONE != scell_info_ptr->gsm.two_quater_position)&&
      (FALSE == rr_SI2q_MI_all_instances_received(rr_get_instances_ptr(SI2Q_INSTANCE,gas_id)))
     )
  {
    MSG_GERAN_HIGH_0_G("IRAT PLMN search aborted , all SI2q not received");
    rr_send_plmn_list_cnf(
      NULL,
      MM_AS_ABORTED,
      rr_plmn_list_req,
...
)
{
...
#endif /* FEATURE_WCDMA || FEATURE_GSM_TDS || FEATURE_LTE */
  {
    ...
    if (rr_event == RR_EV_NO_CELL_SELECTION_REQUIRED)
    {
      ...
      {
        if (no_selection_reqrd != NULL)
        {
          /*No action required as there is neither change in domain nor cell capability*/        
          *no_selection_reqrd = TRUE;
        }
...
{
  /* In most states, GRR processes RR_ABORT_REQ() by throsing saved MM message */
  /* Few transient states, it needs for state movement */
...
#if defined(FEATURE_DUAL_SIM) || defined(FEATURE_TRIPLE_SIM)
boolean grr_can_process_rr_procedure_ind(const gas_id_t gas_id)
{

/* In most states, GRR processes RR_PROCEDURE_IND()  */
...
boolean grr_can_process_rr_restore_scell_info(const gas_id_t gas_id)
{  
  boolean process = FALSE;
  grr_control_data_t *grr_control_data_ptr;
...
#ifdef FEATURE_QSH_MDUMP
QSH_MDUMP_FN_ATTR
void grr_mdump_grr_control(const gas_id_t gas_id)
{
  grr_control_data_t *grr_control_data_ptr = grr_control_get_data_ptr(gas_id);
  qsh_mdump_collect_high(grr_control_data_ptr, sizeof(grr_control_data_t));
}
