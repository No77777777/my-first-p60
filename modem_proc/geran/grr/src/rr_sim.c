...
*/
/* $Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_sim.c#1 $ */
...
)
{
  ...
  // The initial storage of the P-TMSI invalidates the Old P-TMSI
  memset(rr_sim_data_ptr->sim_data.old_ptmsi, 0xFF, sizeof(rr_sim_data_ptr->sim_data.old_ptmsi));   
  rr_sim_data_ptr->sim_data.old_ptmsi_valid = FALSE;
}
 ...
 * \param gas_id
 */
static void rr_siminfo_update_ptmsi(
  byte *old_ptmsi,
  byte *ptmsi,
  const sys_modem_as_id_e_type as_id
)
{
  ...
  RR_NULL_CHECK_RETURN_VOID(old_ptmsi);
  RR_NULL_CHECK_RETURN_VOID(ptmsi);
  ...
  memscpy(
    rr_sim_data_ptr->sim_data.old_ptmsi,           // dst
    sizeof(rr_sim_data_ptr->sim_data.old_ptmsi),   // dst_size
    old_ptmsi,                                     // src
    MM_SIM_PTMSI_SIZE                              // src_size
  );
  rr_sim_data_ptr->sim_data.old_ptmsi_valid = TRUE;

  memscpy(
    rr_sim_data_ptr->sim_data.ptmsi,           // dst
    sizeof(rr_sim_data_ptr->sim_data.ptmsi),   // dst_size
    ptmsi,                                     // src
    MM_SIM_PTMSI_SIZE                          // src_size
  );
  rr_sim_data_ptr->sim_data.ptmsi_valid = TRUE;
...
)
{
  ...
  rr_siminfo_update_ptmsi(msg_ptr->old_ptmsi, msg_ptr->ptmsi, as_id);
...
/*!
 * \brief Gets the Old P-TMSI from the SIM.
 *
 * \param ptmsi (out) - Updated with the P-TMSI (if available).
 * \param gas_id
 *
 * \return boolean - TRUE if available; FALSE otherwise.
 */
boolean rr_siminfo_get_old_ptmsi(
  byte *ptmsi,
  byte destsize,
  const gas_id_t gas_id
)
{
  ...
  if (rr_sim_data_ptr->sim_data.ptmsi_valid)
  {
    memscpy(ptmsi,destsize,rr_sim_data_ptr->sim_data.old_ptmsi,MM_SIM_PTMSI_SIZE);
