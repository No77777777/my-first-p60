...
============================================================================*/
/* $Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_cell_acq.c#2 $ */
...
typedef enum
{
  ...
  RR_CELL_ACQ_STATE_ACQUIRING_SI,
...
)
{
  ...
  if (rr_cell_acq_data_ptr->gprs_cell_info->gsm.cell_identity_valid_flag == TRUE)
  {
    cgi.ci = rr_cell_acq_data_ptr->gprs_cell_info->gsm.cell_identity;
    cgi.lai = rr_cell_acq_data_ptr->gprs_cell_info->gsm.location_area_identification;
    cgi_ptr = &cgi;
  }

  if ((!rr_fcell_is_cell_available(rr_cell_acq_data_ptr->gprs_cell_info->gsm.location_area_identification,
                                   rr_cell_acq_data_ptr->gprs_cell_info->gsm.cell_identity,
                                   gas_id)) ||
      (rr_is_cell_in_barred_cell_list(arfcn, rr_cell_acq_data_ptr->bsic_info.bsic, cgi_ptr, NULL, gas_id))
     )                             
  {
    if (status != NULL)
    {
  ...
  forbidden_lai = rr_is_forbidden_LAI(&rr_cell_acq_data_ptr->gprs_cell_info->gsm.location_area_identification, gas_id);
  if (forbidden_lai)
  {
    rr_mm_send_resel_fail_ind(&rr_cell_acq_data_ptr->gprs_cell_info->gsm.location_area_identification, gas_id);
  }
...
static boolean rr_cell_acq_cell_suitable_cb_fn(rr_candidate_cell_T *cand_cell_ptr, const gas_id_t gas_id)
{
  ...
  // The target cell may not be present in the candidate cell DB. Use the local target cell info instead.
  (void)cand_cell_ptr;
  ...
  rr_cell_acq_data_ptr->status = status;
...
static void rr_gprs_action_cell_reselection(const gas_id_t gas_id)
{
  ...
  if (rr_cell_acq_data_ptr->bsic_info.send_to_l1)
  {
    bsic = rr_cell_acq_data_ptr->bsic_info.bsic;
  }
...
/*!
 * \brief Returns a text string represent of a given state.
 *
 * \param state (in)
 *
...
static const char *rr_cell_acq_state_name(const rr_cell_acq_state_e state)
{
  ...
  switch (state)
  {
    ...
    case RR_CELL_ACQ_STATE_ACQUIRING_SI:
      s = "ACQUIRING_SI";
      break;
...
/*!
 * \brief Sends RR_SELECT_BCCH_IMSG_ACQUIRE_SI_REQ to start the SI acquisition.
 *
 * \param cell_info_ptr (in)
 * \param gas_id (in)
 */
static void rr_cell_acq_start_si_acquisition(gprs_scell_info_t *cell_info_ptr, const gas_id_t gas_id)
{
  ...
  uint16 wait_for_si_mask = MASK_SI_NONE;
  boolean wait_for_si13 = FALSE;

  /**
   * If 'fast SI acq' is required, only a subset of SIs are required. These are specified in 'notify_si_mask',
   * so RR-SELECT-BCCH module will indicate when this subset of SIs has been received. Then the RR-SELECT-BCCH
   * acquisition can be aborted and cell acquisition can complete.
   */
  if (rr_is_fast_si_acq_enabled(&wait_for_si13, gas_id))
  {
    ...
    cell_info_ptr->gsm.complete_BA_list.no_of_entries = 0;
    cell_info_ptr->gsm.complete_idle_BA_list_received = FALSE;
    cell_info_ptr->gsm.complete_BA_list.BA_IND        = INVALID_BA_IND;
    cell_info_ptr->gsm.SI2_BA_list.BA_IND             = INVALID_BA_IND;
    cell_info_ptr->gsm.SI2bis_BA_list.BA_IND          = INVALID_BA_IND;
    cell_info_ptr->gsm.SI2ter_BA_list.BA_IND          = INVALID_BA_IND;
  }

  if (cell_info_ptr->gsm.control_channel_description_valid &&
      cell_info_ptr->gsm.cell_identity_valid_flag)
  {
    /**
     ...
     */
    init_params.bs_ag_blks_res = cell_info_ptr->gsm.control_channel_description.BS_AG_BLKS_RES;
    init_params.bs_cc_chans = cell_info_ptr->gsm.control_channel_description.BS_CC_CHANS;
    init_params.bs_ccch_sdcch_comb = cell_info_ptr->gsm.control_channel_description.BS_CCCH_SDCCH_COMB;
    init_params.bs_pa_mfrms = cell_info_ptr->gsm.control_channel_description.BS_PA_MFRMS;
    init_params.imsi_valid = rr_get_imsi_mod_1000(&init_params.imsi_mod_1000, gas_id);
    init_params.lai = cell_info_ptr->gsm.location_area_identification;
    init_params.ci = cell_info_ptr->gsm.cell_identity;
    init_params.si_messages_on_bcch_ext = cell_info_ptr->bcch_sys_info.si_messages_on_bcch_ext;
    init_params.si_messages_not_broadcast = cell_info_ptr->bcch_sys_info.si_messages_not_broadcast;
  ...
  /**
   * If no SI mask has been setup, assume the full set.
   * Note: Other SIs such as SI2quater will be requested to GL1 by RR-SELECT-BCCH, but as they are not
   * included in this mask, they will not be waited-for.
   */
  if (wait_for_si_mask == MASK_SI_NONE)
  {
  ...
  max_allowed_time_ms = rr_si_calc_sys_info_acquisition_time(
    wait_for_si_mask & (~cell_info_ptr->bcch_sys_info.si_messages_not_broadcast), // si_required
    cell_info_ptr->bcch_sys_info.si_messages_on_bcch_ext                          // si_on_bcch_ext
  );
  ...
  rr_select_bcch_send_imsg_acquire_si_req(
    cell_info_ptr->gsm.BCCH_ARFCN,                       // arfcn
    cell_info_ptr->gsm.reselection_info.RXLEV_average,   // rxlev_average
    cell_info_ptr->gsm.BSIC,                             // bsic
    rr_cell_acq_cell_suitable_cb_fn,                     // cell_suitable_cb_fn
...
/*!
 * \brief Indicates if waiting for confirmation that PS has been blocked in MAC/RLC.
 *
 * \param gas_id (in)
 *
 * \return boolean - TRUE if waiting for MAC to indicate IDLE status, FALSE otherwise
 */
boolean rr_cell_acq_pending_mac_suspension(const gas_id_t gas_id)
{
  rr_cell_acq_data_t *rr_cell_acq_data_ptr = rr_cell_acq_get_data_ptr(gas_id);
...
)
{
  ...
  while (event_in != EV_NO_EVENT)
  {
    switch (rr_cell_acq_data_ptr->state)
    {
                  ...
                  case RR_CELL_ACQ_IMSG:
                  {
                    switch (message->rr.cell_acq.imsg.header.imh.message_id)
                    {
                      case RR_CELL_ACQ_IMSG_START_IND:
                      {
                        ...
                        rr_cell_acq_data_ptr->bsic_info               = message->rr.cell_acq.imsg.start_ind.bsic_info;
                          ...
                          if (ptr_surround_database->gsm.valid_data_flag)
                          {
                                ...
                                ))
                            {
                              ...
                              rr_cell_acq_data_ptr->gprs_cell_info->gsm.control_channel_description_valid =
                                ptr_surround_database->gsm.control_channel_description_valid;
                              rr_cell_acq_data_ptr->gprs_cell_info->gsm.control_channel_description =
                                ptr_surround_database->gsm.control_channel_description;
                              rr_cell_acq_data_ptr->gprs_cell_info->gsm.cell_identity_valid_flag =
                                ptr_surround_database->gsm.cell_id_valid;
                              rr_cell_acq_data_ptr->gprs_cell_info->gsm.cell_identity =
                                ptr_surround_database->gsm.cell_id;
                              rr_cell_acq_data_ptr->gprs_cell_info->gsm.location_area_identification =
                                ptr_surround_database->gsm.location_area_identification;
                              rr_cell_acq_data_ptr->gprs_cell_info->bcch_sys_info.si_messages_not_broadcast =
                                ptr_surround_database->si_not_broadcast;
                              rr_cell_acq_data_ptr->gprs_cell_info->bcch_sys_info.si_messages_on_bcch_ext =
                                ptr_surround_database->si_on_bcch_ext;
                ...
                switch (message_id)
                {
                  case MPH_RESELECTION_CNF:
                  {
                    ...
                    if (message_ptr->BCCH_found)
                    {
                      ...
                      /**
                       * If SI messages need to be acquired from BCCH, then start this process now.
                       * In the case of a reselection type where the SI messages were received on the PACCH, a
                       * RR_CELL_ACQ_IMSG_NACC_ACQ_STATUS_IND message is expected which gives the status of
                       * what messages were received (which should be enough to complete the reselection). If
                       * there is a short-fall in SI messages, then further SI acquisition off BCCH is done at
                       * that point.
                       */
                      if (rr_cell_acq_data_ptr->action != RR_CELL_ACQ_ACTION_RESELECT_BCCH_NACC_PCCO_PCCC)
                      {
                        rr_cell_acq_start_si_acquisition(
                          rr_cell_acq_data_ptr->gprs_cell_info,  // cell_info_ptr,
                          gas_id                                 // gas_id
                        );
                      }

                      rr_cell_acq_data_ptr->state = RR_CELL_ACQ_STATE_ACQUIRING_SI;
                    }
                    else
                    {
                  ...
                  case MPH_SELECT_SPECIFIC_BCCH_CNF:
                  {
                    ...
                    if (message_ptr->BCCH_found)
                    {
                      ...
                      if (rr_cell_acq_data_ptr->bsic_info.bsic.PLMN_colour_code != PLMN_colour_code ||
                          rr_cell_acq_data_ptr->bsic_info.bsic.BS_colour_code != BS_colour_code)
                      {
                        MSG_GERAN_HIGH_4_G("BSIC mismatch L1:(%d,%d) RR:(%d,%d)",
                                           PLMN_colour_code,
                                           BS_colour_code,
                                           rr_cell_acq_data_ptr->bsic_info.bsic.PLMN_colour_code,
                                           rr_cell_acq_data_ptr->bsic_info.bsic.BS_colour_code);

                        switch (rr_cell_acq_data_ptr->bsic_info.mismatch_action)
                        {
                          case RR_CELL_ACQ_BSIC_MISMATCH_ACTION_STOP:
                          {
                            rr_cell_acq_send_omsg_finished_ind(
                          ...
                          case RR_CELL_ACQ_BSIC_MISMATCH_ACTION_REQUIRE_SI3:
                          {
                            MSG_GERAN_HIGH_0_G("CCD invalidated - require SI3");
                            rr_cell_acq_data_ptr->gprs_cell_info->gsm.control_channel_description_valid = FALSE;
                            break;
                          }

                          case RR_CELL_ACQ_BSIC_MISMATCH_ACTION_NONE:
                          default:
                          {
                            ;  // no action
                          }
                      ...
                      {
                        // Store the BSIC returned from L1
      ...
      *
      *
      * In this state: CELL ACQ state machine is waiting for SI acquisition to complete.
      ...
      * Messages:
      * RR_CELL_ACQ_IMSG_ABORT_REQ
      * RR_CELL_ACQ_IMSG_NACC_ACQ_STATUS_IND
        ...
        switch (event_in)
        {
            ...
            switch (message_set)
            {
              case MS_RR_RR:
              {
                switch (message->rr.header.rr_message_set)
                {
                  ...
                  {
                    switch (message->rr.select_bcch.omsg.header.imh.message_id)
                    {
                      ...
                      {
                        rr_select_bcch_omsg_select_fail_ind_t *msg_ptr;
                        rr_cell_acq_status_e status;

                        msg_ptr = &message->rr.select_bcch.omsg.rr_select_bcch_omsg_select_fail_ind;

                        switch (msg_ptr->reason)
                        {
                          case RR_SELECT_BCCH_FAIL_REASON_TIMEOUT:
                          {
                            status = RR_CELL_ACQ_STATUS_FAILURE_BSI_ACQUISITION_FAILURE;
                            break;
                          }

                          default:
                          {
                            status = rr_cell_acq_data_ptr->status;
                          }
                        }

                        if (status == RR_CELL_ACQ_STATUS_UNDEFINED)
                        {
                          rr_cell_acq_send_omsg_abort_cnf(gas_id);
                        }
                        else
                  ...
                  case RR_CELL_ACQ_IMSG:
                  {
                    switch (message->rr.cell_acq.imsg.header.imh.message_id)
                    {
                      case RR_CELL_ACQ_IMSG_ABORT_REQ:
                      {
                        ...
                        // Record the status for after RR-SELECT-BCCH aborts
                        rr_cell_acq_data_ptr->status = RR_CELL_ACQ_STATUS_UNDEFINED;

                        // Abort RR-SELECT-BCCH and wait for the response
                        rr_select_bcch_send_imsg_abort_req(gas_id);
                        break;
                      }

                      case RR_CELL_ACQ_IMSG_NACC_ACQ_STATUS_IND:
                      {
                        rr_cell_acq_imsg_nacc_acq_status_ind_t *msg_ptr;

                        MSG_GERAN_HIGH_0_G("RR_CELL_ACQ_IMSG_NACC_ACQ_STATUS rxd");

                        msg_ptr = &message->rr.cell_acq.imsg.nacc_acq_status_ind;

                        if (msg_ptr->pncd_enough_to_camp)
                        {
                          rr_cell_acq_reset_parameters(gas_id);
              ...
              case MS_RR_L1:
              {
                switch (message->message_header.message_id)
                {
                  ...
                  {
                    mph_block_quality_ind_T *block_qual_ind = &message->mph_block_quality_ind;
 
                    new_event = rr_gprs_check_dsc_in_early_camp(
          ...
          {            
            ...
            if (rr_timer_is_running(RR_GSM_SYS_INFO_TIMER, gas_id))
            {
              ...
              {
                MSG_GERAN_HIGH_0_G("XCCH failure observed during Cell SI Acq:RR_GSM_SYS_INFO_TIMER forcefully expired");
              }
            }
            break;
          }
    ...
    if (rr_cell_acq_data_ptr->state != rr_cell_acq_data_ptr->old_state)
    {
      ...
      MSG_SPRINTF_2(MSG_SSID_DFLT,
                    ...
                    rr_cell_acq_state_name(rr_cell_acq_data_ptr->state));
#else
      MSG_GERAN_HIGH_2_G("rr_cell_acq_control(%d->%d)",
...
/*!
 * \brief Checks if RR-CELL-ACQ-CONTROL is in a suitable state to handle 
 *        RR_CELL_ACQ_IMSG_SURROUNDING_SI_RECEIVED_IND.
 *
 * \param gas_id
 *
 ...
 */
boolean rr_cell_acq_can_process_surr_si_received_ind(const gas_id_t gas_id)
{
  if (grr_is_ps_enable_in_progress(gas_id) ||
      grr_is_ps_disable_in_progress(gas_id))
  {
    return FALSE;
  }
...
#ifdef FEATURE_QSH_MDUMP
QSH_MDUMP_FN_ATTR
void grr_mdump_rr_cell_acq(const gas_id_t gas_id)
{
  rr_cell_acq_data_t *rr_cell_acq_data_ptr = rr_cell_acq_get_data_ptr(gas_id);
  qsh_mdump_collect_high(rr_cell_acq_data_ptr, sizeof(rr_cell_acq_data_t));
}
