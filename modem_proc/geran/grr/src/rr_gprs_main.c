...
*** TITLE
***
***  $Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_gprs_main.c#2 $
***
*** DESCRIPTION
***
...
)
{
  ...
  if ( message->message_header.message_id == (byte)MAC_GRR_MAC_STATUS )
  {
    ...
    if (mac_status == MAC_IDLE)
    {
      ...
      {
        //Send SCell Ncell info to enG Mode
        {
...
#ifdef FEATURE_GSM_CXM_L3_STATE
        /* TBF is released here. Report IDLE state to CxM unless there exists a dedicated connection */
        if(!rr_is_control_state(RR_DATA_TRANSFER, gas_id) &&
...
#ifdef FEATURE_GSM_CXM_L3_STATE
        /* Report CONNECTED_PS state to CxM as TBF is active */
        rr_cxm_state_send_l3_state_ind(CXM_TECH_L3_CONNECTED_PS, gas_id);
#endif /* FEATURE_GSM_CXM_L3_STATE */
      }
    }
