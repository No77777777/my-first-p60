...
*/
/* $Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_sys_info_ncell.c#1 $ */
...
)
{
  ...
  {
    /* Serving cell Si acquisition is suspended if Transfer is in progress*/  
    if (MAC_TRANSFER == rr_macinfo_get_mac_status(gas_id))
    {
      MSG_GERAN_HIGH_0_G("NCell SI Acq,Packet Txfer in progress: Suspend serv cell acquisition ");    
...
void rr_gprs_ncell_sys_info_timer_expiry(const gas_id_t gas_id)
{
  ...
  if (rr_gprs_find_first_ncell_index(&ncell_index, BCCH_FORMAT, gas_id))
  {
    ...
    if ( ncell->ncell_data.bcch.system_info_timer_count != UNINITIALISED_SYS_INFO_COUNT )
    {
      ...
      {
        MSG_GERAN_MED_1_G("Ncell SI timer expired for ARFCN %d",CONVERT_FROM_ARFCN_T(ncell->arfcn));
        ...
        /* Invalidate the cell_id, to avoid reading cache for SI3 and SI4 during MPH_GPRS_SURROUND_UPDATE_REQ*/
        ncell->ncell_data.bcch.gsm.cell_id_valid = FALSE;
