...
*/
/* $Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_shared_cell_selection.c#2 $ */
...
)
{
  ...
  if ((C1 > 0 || rr_allow_negative_c1(location_area_identification->plmn_id, gas_id)) && 
      rr_is_forbidden_LAI(location_area_identification, gas_id))
  {
    rr_mm_send_resel_fail_ind(location_area_identification, gas_id);
  }
  if (location_area_identification != NULL)
  {
    if (!rr_fcell_is_cell_available(*location_area_identification, cell_id, gas_id))
    {
      MSG_GERAN_HIGH_1_G("Cell (%d) is on EFS forbidden cell list",BCCH_ARFCN.num);
      return FALSE;
    }
  }
  else
  {
    MSG_GERAN_HIGH_0_G("location_area_identification is NULL");
  }
  ...
  {
    access_result_T  access_result_for_ec_call;
    MSG_GERAN_MED_0_G(" W2G service redirection (emergency call check is cell allows EC call)");

    access_result_for_ec_call = rr_check_access_class(
    ...
    if (access_result_for_ec_call == EMERGENCY_CALLS_ALLOWED || access_result_for_ec_call == ALL_CALLS_ALLOWED)
    {
      registration_not_possible = FALSE;
    }
    else
    {
      registration_not_possible = TRUE;
  ...
  {
    MSG_GERAN_HIGH_3_G("Cell_type: %d, C1: %d, reg_not_possible: %d",cell_type, C1, registration_not_possible);
...
)
{
  ...
  boolean                    forbidden_ind_lai           = FALSE;
  boolean                    PLMN_ok                     = TRUE;
  boolean                    emergency_call_allowed      = TRUE;
  boolean                    all_calls_allowed           = TRUE;
  boolean                    band_ok;
  ...
  /* sends LAI to NAS if its forbidden */
  forbidden_ind_lai = rr_is_forbidden_LAI(location_area_identification, gas_id);
  if((C1 > 0 || rr_allow_negative_c1(location_area_identification->plmn_id, gas_id)) && forbidden_ind_lai)
  {
    rr_mm_send_resel_fail_ind(location_area_identification, gas_id);
  }
  ...
  if (!check_acceptable)
  {
    ...
    {
      forbidden_lai = forbidden_ind_lai;
    }
  ...
  if (rr_is_limited_plmn_selection_mode(gas_id) && 
      (!rr_plmn_search_in_progress(gas_id)) && 
      (!rr_sel_is_search_cause_service_req_mm_full_rat_acq_db_scan(rr_PLMN_request_details_ptr))
     )
  {
    access_result_T  access_result;
  ...
  if (rr_is_automatic_plmn_selection_mode (gas_id) && 
      ...
      (! all_calls_allowed))
  {
    ...
    MSG_GERAN_HIGH_5_G("PLMN_ok: %d, band_ok: %d, auth_failed_cell_barred: %d, emergency_call_allowed: %d all_calls_allowed: %d",
             PLMN_ok,
             band_ok,
             auth_failed_cell_barred,
             emergency_call_allowed,
