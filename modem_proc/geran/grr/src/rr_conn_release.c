...
$Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_conn_release.c#2 $ $DateTime: 2020/01/27 04:29:28 $ $Author: pwbldsvc $
...
#ifdef FEATURE_QSH_EVENT_NOTIFY_TO_QSH 
      rr_qsh_event_notify(RR_QSH_EVENT_CON_REL_L2_RESET, gas_id);
#endif // FEATURE_QSH_EVENT_NOTIFY_TO_QSH
    ...
    if (start_IE != NULL)
    {
          ...
          ))
      {
        ...
        if (rr_is_ms_locked_to_channel(gas_id))
        {
          ...
          {
            MSG_GERAN_HIGH_1_G("Lock mode enabled: Network indicates redirection to %d, ignoring", rr_cell_select_indicator_ptr->type);
...
/**
* This function checks for the next action to be performed once the connection is released
* if its set to moving to other RAT then check for lock exchange allowed
*/
static boolean rr_perform_exchange_for_redirection(const gas_id_t gas_id)
{
#ifdef FEATURE_GSM_RX_TX_SPLIT
  rr_rat_e target_rat = RR_RAT_UNDEFINED;
  rr_chan_rel_cell_selection_indicator_t *rr_cell_select_indicator_ptr;
   
  rr_cell_select_indicator_ptr = rr_get_cell_select_indicator_ptr(gas_id);

  if (rr_cell_select_indicator_ptr->valid == TRUE)     
  {
    target_rat = rr_g2x_redir_map_redir_type_to_target_rat(rr_cell_select_indicator_ptr->type); 
           ...
           (rr_connection_release_get_g2l_blind_redir_after_csfb_flag(gas_id) &&
            rr_is_blind_redir_possible_for_csfb_after_release(gas_id)))
  {
    target_rat = RR_RAT_LTE;
  }
#endif /* FEATURE_LTE */
#ifdef FEATURE_GSM_TDS_PSEUDO_FAST_RETURN
  else if (rr_pseudo_fr_return_to_tds(gas_id) == TRUE)
  {
    target_rat = RR_RAT_UTRAN_TDD;
  }
#endif /*FEATURE_GSM_TDS_PSEUDO_FAST_RETURN*/

  if (target_rat != RR_RAT_UNDEFINED)
  {
    return rr_g2x_lock_exchange_allowed(RR_G2X_OPERATION_TYPE_REDIRECTION, target_rat, gas_id);
  }
#endif /*FEATURE_GSM_RX_TX_SPLIT */  
...
static void rr_rel_deactivate_channels(const gas_id_t gas_id)
{
  ...
  */
  ...
  /*Check for lock exchange if the connection is not getting released because of RLF as that triggers re-establishment */
  if (!(rr_rel_is_cause_radio_link_failure(gas_id)))
  {
    if (rr_perform_exchange_for_redirection(gas_id))
    {
...
//tlx
  ...
  /* Cancel G2L redirections if blocked by NAS */
  if (!rr_g2l_redir_allowed(gas_id))
  {
    rr_connection_release_reset_g2l_blind_redir_flag(gas_id);
    ...
    if(rr_cell_select_indicator_ptr->type == LTE_TARGET)
    {
      rr_cell_select_indicator_ptr->valid = FALSE;
    }
  }
...
#ifdef FEATURE_WCDMA
  /* Cancel G2W redirections if blocked by NAS */
  if (!rr_g2w_resel_redir_allowed(gas_id))
  {
    if(rr_cell_select_indicator_ptr->type == FDD_TARGET)
    {
      rr_cell_select_indicator_ptr->valid = FALSE;
    }
  }
    ...
    switch (rr_conn_release_data_ptr->release_state)
    {
                  ...
                  case RR_CE_IMSG:
                  {
                    switch (rr_rr_msg_ptr->ce.imsg.header.imh.message_id)
                    {
                      case RR_CE_IMSG_CS_ABORT_REQ:
                      {
                        ...
                        {
                          // This is used later to skip sending MPH_CHANNEL_RELEASE_REQ
                          rr_conn_release_data_ptr->l1_in_null = TRUE;
          ...
          */
               ...
               (message_id == (byte) DL_RELEASE_IND)))
          {
            ...
            if (rr_ded_l1_chan_is_connected(gas_id) &&
                (FALSE == rr_conn_release_data_ptr->l1_in_null))
            {
              // Release L1 (MPH_CHANNEL_RELEASE_REQ will be sent)
              rr_rel_deactivate_channels(gas_id);
            }
            ...
            {
              MSG_GERAN_HIGH_0_G("No ded chan in GL1 - skip release");
      ...
      case RR_REL_MAIN_SIGNALLING_DISCONNECTED:
      {
...
#ifdef FEATURE_GSM_CXM_L3_STATE
            /* Report IDLE state to CxM as the connection is no more active */
            rr_cxm_state_send_l3_state_ind(CXM_TECH_L3_IDLE, gas_id);            
#endif /* FEATURE_GSM_CXM_L3_STATE */
          }
        ...
        else if (release_event == EV_CHANNEL_DEACTIVATED)
        {
          ...
          /* reset bsi acqistion control to avoid sending sys info req during DTM channel release.*/
          ...
          if (rr_ce_conn_originated_on_other_rat(gas_id))
          {
            ...
            if (scell_info_ptr->gsm.cell_identity_valid_flag &&
                ...
                (rr_is_manual_plmn_selection_mode(gas_id) == FALSE))
            {
          ...
          if (new_rr_event == EV_NO_EVENT)
          {
            ...
            if (ce_rel_status != RR_CE_REL_STATUS_CAMPED)
            {
              ...
              /* If specific channel search is not going to be started, 
                           log CAMP_ATTEMPT_START here. */
              if((ce_rel_status == RR_CE_REL_STATUS_CAMP_ON_CELL) || 
                 (ce_rel_status == RR_CE_REL_STATUS_ACQUIRE_SI))
              {
                ARFCN_T arfcn = scell_info_ptr->gsm.BCCH_ARFCN;
                BSIC_T bsic = scell_info_ptr->gsm.BSIC;
...
#ifdef FEATURE_QSH_MDUMP
QSH_MDUMP_FN_ATTR
void grr_mdump_rr_conn_rel(const gas_id_t gas_id)
{
  release_info_t *rr_conn_rel_ptr = rr_conn_release_get_data_ptr(gas_id);
  qsh_mdump_collect_high(rr_conn_rel_ptr, sizeof(release_info_t));
}
