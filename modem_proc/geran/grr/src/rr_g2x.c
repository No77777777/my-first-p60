...
============================================================================*/
/* $Header: //components/rel/geran.mpss/7.4.0/grr/src/rr_g2x.c#2 $ */
...
#ifdef FEATURE_GSM_TDS
static int rr_tds_cell_cmp(
  rr_utran_tdd_cell_t *a, 
  rr_utran_tdd_cell_t *b
)
{
   if (a->uarfcn == b->uarfcn &&
       a->cpid   == b->cpid   &&
...
)
{
  if (a->rat == b->rat)
  {
    switch (a->rat)
    {
      ...
      {
        return rr_tds_cell_cmp(&a->data.utran_tdd, &b->data.utran_tdd);
      }
#endif
  ...
  while (entry != NULL)
  {
    switch (entry->cell.rat)
    {
      ...
      case RR_RAT_UTRAN_TDD:
      {
        MSG_5(MSG_SSID_DFLT,
              MSG_LEGACY_HIGH,
              "barred_cell_db.entries[%d]: TDSCDMA UARFCN %d, CPID %d, reason %d, time (ms): %d",
              i,
              entry->cell.data.utran_tdd.uarfcn,
              entry->cell.data.utran_tdd.cpid,
              entry->reason,
              rr_timer_get_time_remaining(entry->timer, gas_id));
        break;
      } 
...
)
{
  ...
  rr_qsh_event_notify(RR_QSH_EVENT_G2X_RESEL_FAILED, gas_id);
...
)
{
  ...
  if (lte_rrc_earfcn_is_supported(RR_GAS_ID_TO_AS_ID, earfcn, band_pref_lte))
  {
    rr_irat_frequency_t frequency;
  ...
  if (barred_frequency_db_updated)
  {
    ...
    if (grr_is_idle_camped(gas_id) || grr_tbf_est_in_progress(gas_id))
    {
      ...
      if (rr_timer_is_running(RR_PRIORITIZE_LTE_ABORT_TIMER, gas_id))
      {
        ...
        {
          MSG_GERAN_MED_0_G("[LTE Prio] Priritizing LTE frequencies");
...
/**
  @brief Determines whether the specified RAT is supported on the specified GAS ID
  by taking into account both the segment loading and the subscription capability
*/
...
#ifdef FEATURE_WCDMA
    case RR_RAT_WCDMA:
      /* verify that this subscription has support for WCDMA */
...
#ifdef FEATURE_LTE
    case RR_RAT_LTE:
      /* verify that this subscription has support for LTE */
      if (rr_ms_sub_capabilities_exclude_sys_mode(SYS_SYS_MODE_MASK_LTE, gas_id))
      {
        return(FALSE);
      }
      return(TRUE);
...
#ifdef FEATURE_GSM_TDS
    case RR_RAT_UTRAN_TDD:
      /* verify that this subscription has support for TDSCDMA */
...
boolean rr_g2x_allowed_by_gprs_activity(const rr_rat_e target_rat, const gas_id_t gas_id)
{
  ...
  if (geran_in_multi_standby_mode())
  {
    ...
    if (rr_ms_gprs_active_on_other_stack(gas_id))
    {
