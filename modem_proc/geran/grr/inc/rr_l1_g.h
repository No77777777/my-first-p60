...
$Header: //components/rel/geran.mpss/7.4.0/grr/inc/rr_l1_g.h#2 $    $DateTime: 2019/09/19 07:13:01 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
04/11/16   ap      CR1086132 FR30575: Geran Support for Feature L Plus L 
...
/*RR->L1 DTX indicator values*/
typedef enum
{
  RR_L1_DTX_OFF,
  RR_L1_DTX_ON,
  RR_L1_DTX_OVERRIDE
} rr_l1_dtx_indicator_T;
...
#define RR_MAX_CANDIDATE_FREQ_LIST_BPLMN_SRCH 200
...
typedef struct
{
  ...
  ARFCN_T arfcn[RR_MAX_CANDIDATE_FREQ_LIST_BPLMN_SRCH];
...
typedef struct
{
  ...
  boolean                 retry_required;
   gas_id_t               gas_id;
  rr_l1_band_bitmap_T     denied_bands;
...
/* TRUE if L1 is being started for an X2G HO procedure.*/
...
typedef enum
{
  GERAN_ACQ_PROCEDURE,  /* Power scan, BCCH list decode, etc. */
  GERAN_MMS_PROCEDURE,  /* MMS data */
  GERAN_SIB_PROCEDURE,  /* System Information */
  GERAN_LOCK_HOLD,      /* Lock Hold for CSFB etc. */
  GERAN_LOCK_RELEASE,   /* Lock Release for CSFB etc. */
  GERAN_CONN_PROCEDURE, /* Circuit-switched procedure */
  GERAN_GPRS_PROCEDURE  /* Packet-switched procedure */
} geran_procedure_t;
...
{
  IMH_T            message_header;
  geran_procedure_t    procedure_type;
  geran_priority_t     new_priority;
  boolean          new_procedure;
  sys_proc_type_e_type sys_proc_type;
  gas_id_t         gas_id;
...
to be able to send to GL1 - this is set to the max number of LTE(16) and TDS(9) frequencies 
that RR passes to GL1 */
#define MAX_RR_L1_PRIORITY_ENTRIES (25)
...
/******************************************************************************
 * MPH_IDLE_COLL_FALLBACK_IND primitive
 * FROM : LAYER 1
 * TO   : RR
 * DESCRIPTION :
 * L1 indicates the persistent page collision event indication to RR.
 ******************************************************************************/
typedef struct
{
  IMH_T                     message_header;
...
/******************************************************************************
 * MPH_ACQUISITION_STATE_IND
 * FROM : L1
 * TO   : RR
 * DESCRIPTION :
 * Layer 1 indicates when acquisition (FIND_BCCH) state changes
 ******************************************************************************/
...
{
  IMH_T                   message_header;
  rr_l1_acq_state_t       rr_l1_acq_state;
  geran_priority_t        current_priority;
  gas_id_t                gas_id;
} mph_acquisition_state_ind_T;
...
/******************************************************************************
 * MPH_PREEMPTION_IND primitive
 * FROM : LAYER 1
 * TO   : RR
 * DESCRIPTION :
 * Informs GRR that a pre-emption notification has been received
 ******************************************************************************/
typedef enum
{
  RR_L1_CS_PREEMPTION, /* CS connection pre-emption */
  RR_L1_PS_PREEMPTION,  /* PS connection pre-emption */
  RR_L1_PS_PDU_TUNEAWAY_OVERRUN
} rr_l1_preemption_t;

typedef struct
{
  IMH_T                   message_header;
  rr_l1_preemption_t      preemption_type;
  gas_id_t                gas_id;
} mph_preemption_ind_T;

/******************************************************************************
 * MPH_VOICE_SAMPLE_RATE_CHANGE_IND primitive
 * FROM : LAYER 1
 * TO   : RR
 * DESCRIPTION :
 * L1 Notifies RR using this primitive when Voice Sample Rate changes 
 * on an existing CS connection
 ******************************************************************************/
typedef struct
{
  IMH_T                message_header;
  ...
  gas_id_t             gas_id;
} mph_voice_sample_rate_change_ind_T;
...
typedef union
{
  ...
  mph_idle_coll_fallback_ind_T            idle_coll_fallback_ind;
#ifndef FEATURE_WCDMA_PLT
#ifdef FEATURE_GTOW_BACKGROUND_PLMN_SEARCH
  ...
  mph_preemption_ind_T                    preemption_ind;
  mph_voice_sample_rate_change_ind_T      voice_sample_rate_change_ind;
  mph_acquisition_state_ind_T             acq_state_ind;
} l1_rr_messages_u;
