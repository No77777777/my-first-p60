...
General Description
  ...
  target.
  Documentation: 80-V5295-2 "Serial Interface Control Document for GSM, GPRS,
                             and EGPRS"
...
$Header: //components/rel/geran.mpss/7.4.0/gdiag/src/gsmdiag.c#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
20/07/16   sp      CR1038907: Adding SubId as external header in log_alloc_ext
...
[80-B5525-1 last rev. F January 27, 2005  seems to have been superseded
 (it refers to RS-232) by 80-V5295-2]
...
PACK(void *) gsmdiag_log_alloc(gsmdiag_pkt_info_t *info_ptr)
{
  ...
  if (info_ptr != NULL)
  {
    ...
    if (info_ptr->dual_sim_active == TRUE)
    {
      ...
      uint32 active_sub_id = info_ptr->active_sub_id;

      /* allocate one more byte than required, using an offset packet ID */
      header_ptr = (byte *)log_alloc_ext(info_ptr->pkt_id+LOG_GSM_DUAL_SIM_CODE_OFFSET,
                                           info_ptr->pkt_size+1,
                                           1,
                                           &active_sub_id);
      if (header_ptr != NULL)
...
CMD_CODE (75 / 0x4B)    1     Value is set to 75
SUBSYS_ID               1     Value is set to 8 (GSM)
SUBSYS_CMD_CODE (0)     2     Value is set to 0
VOC_DSP_VER             2     Vocoder DSP version number; 16 bits range from 0 to 0xffff
MDSP_ROM_VER            2     DSP version number (first part) 16 bits
MDSP_RAM_VER            2     DSP version number (second part) 16 bits
IMSI                    9     International Mobile Subscriber Identity, up to 15 digits.
                              In the first byte, the lowest 3 bits are 001 and the fourth bit
                              indicates odd or even number.
                              0 – Even number
                              1 – Odd number
                              Starting from high nibble of the first byte, every nibble is a
                              digit coded by BCD coding. If the number of digits is even
                              then bits 5 to 8 of the last octet shall be filled with an end
                              mark coded as "1111".
IMEI                    9     International Mobile Station Equipment Identity; 15 digits.
                              The first byte indicates length, which is 8. In the second
                              byte, the lowest 3 bits are 010 and the fourth bit is 1, which
                              indicates an odd number.
                              Starting from high nibble of the second byte, every nibble is
                              a digit coded by BCD coding.
IMEI_SV                 10    International Mobile Station Equipment Identity Software
                              Version Number, 16 digits.
                              The first byte indicates length, which is 9. In the second
                              byte, the lowest 3 bits are 011 and the fourth bit is 0, which
                              indicates an even number.
                              Starting from high nibble of the second byte, every nibble is
                              a digit coded by BCD coding, except the high nibble of the
                              9th byte.
...
Description:
       ...
       from QXDM and sends the response back. Data for the responses
       comes from outside GSM

Field                Length Description
CMD_CODE (75)           1   Value is set to 75
SUBSYS_ID               1   Value is set to 8 (GSM)
SUBSYS_CMD_CODE (1)     2   Value is set to 1
IMEI                    9   First byte is length (0 to 15)
                            Next 8 bytes are packed BCD
IMSI                    9   First byte is length (0 to 15)
                            Next 8 bytes are packed BCD
LAI                     5   Location area ID
                            Format:
                            * Public Land Mobile Network Identity – 3 bytes
                            * Location Area Code – 2 bytes
CELL_ID                 2   Cell identity
CM_CALL_STATE           1   CM call state
                            * -1 – None
                            * 0 – Idle
                            * 1 – Originating call
                            * 2 – Waiting
                            * 3 – Incoming call
                            * 4 – Concurrent incoming/originating call
                            * 5 – Conversation
CM_OPERATIONAL_MODE     1   CM operational mode
                            * -1 – None
                            * 0 – Power off
                            * 1 – Factory Test mode
                            * 2 – Offline
                            ...
                            * 7 – Reset
                            * 8 – Network test
CM_SYSTEM_MODE          1   CM system mode
                            * -1 – None
                            * 0 – No service
                            * 1 – AMPS
                            * 2 – CDMA
...
/*===========================================================================
Function: gsmdiag_state_retrieval
Description:
...
CMD_CODE (75)           1   Value is set to 75
SUBSYS_ID               1   Value is set to 8 (GSM)
SUBSYS_CMD_CODE (2)     2   Value is set to 2
TMC_STATE               1   * 0 – Offline
                            * 1 – Online
...
CMD_CODE (75)           1   Value is set to 75
SUBSYS_ID               1   Value is set to 8 (GSM)
SUBSYS_CMD_CODE (3)     2   Value is set to 3
NUM_DIGITS              1   Number of digits; the DM shall set this field to be the number
                            of valid digits in the DIGITS field
DIGITS                  32  Dialed digits; ASCII 0 to 9, #, * only
RESERVED                1   TBD
...
Field               Length Description
CMD_CODE                1   Value is set to 75 (DIAG_SUBSYS_CMD_F)
SUBSYS_ID               1   Value is set to 8 (DIAG_SUBSYS_GSM)
SUBSYS_CMD_CODE         2   Value is set to 5
LOG_PACKET_TYPE         2   Value is the log packet type being requested
ZERO_STATS              1   Count values
                             0 – Return current count values
                             1 – Zero all counts and return values
ADDITIONAL_INFO         1   Additional information dependent on the packet type

LOG_PACKET_TYPE                       ADDITIONAL_INFO
0x5200 RLC-UL Abnormal Release Counts Not used
0x5201 RLC-UL Event Counts            Not used
0x5202 RLC-UL Statistics              Not used
0x520A RLC-DL Statistics              Not used
0x5216 GPRS LLC PDU Statistics        GLLC SAPI – one of 1, 3, 5, 7, 9, 11
0x5217 GPRS LLC Periodic Statistics   GLLC SAPI – one of 1, 3, 5, 7, 9, 11
0x5220 GPRS SNDCP PDU Statistics      NSAPI ADDRESS – 5 to 15
============================================================================*/
...
static const diagpkt_user_table_entry_type gsmdiag_tbl[] =
{
  ...
  #error code not present
#endif /* FEATURE_GSM_SIMULATE_GL1_RF */
  /* 3.2.11 GSM feature set request (subcmd_6) is not implemented */
};
