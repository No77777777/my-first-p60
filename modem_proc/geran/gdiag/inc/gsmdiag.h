...
$Header: //components/rel/geran.mpss/7.4.0/gdiag/inc/gsmdiag.h#1 $
...
/* This Table is from 80-V5295-2 GSM ICD section 3.2 */
...
/* GSM states defined in 80-V5295-2 Serial Interface Control Document
...
#ifdef FEATURE_DUAL_SIM
#define GERAN_EVENT_REPORT_PAYLOAD(_event_id, _length, _payload) \
  {  \
      uint32 diag_sub_id = (uint32)geran_get_diag_sub_id(gas_id);   \
      event_report_payload_ext(_event_id, (uint8)_length, (void*)_payload, 1, &diag_sub_id);   \
  }
#define GERAN_EVENT_REPORT(_event_id) \
  {  \
      uint32 diag_sub_id = (uint32)geran_get_diag_sub_id(gas_id);   \
      event_report_ext(_event_id, 1, &diag_sub_id);  \
  }
#else /* FEATURE_DUAL_SIM */
#define GERAN_EVENT_REPORT_PAYLOAD(_event_id, _length, _payload) \
  event_report_payload(_event_id, (uint8)_length, (void*)_payload)
#define GERAN_EVENT_REPORT(_event_id) \
    event_report(_event_id)
#endif  /* FEATURE_DUAL_SIM */
