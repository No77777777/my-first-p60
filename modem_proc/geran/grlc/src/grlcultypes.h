...
***
*** $Header: //components/rel/geran.mpss/7.4.0/grlc/src/grlcultypes.h#2 $
***
 ...
 */
 uint16                     psn_success_ind;
  ...
  /* Point to the next address to place the metric
  */
  uint8                      *metric_dump_ptr;

  /* Saved pointer to the callback main param structure
  */
  qsh_client_cb_params_s     grlc_ul_qsh_cb_param;

  qsh_client_metric_context_id_t  metric_context_id;
  boolean                    b_metrics_active;
#endif /* FEATURE_QSH_EVENT_METRIC */

#ifdef FEATURE_QSH_DUMP
  grlcul_qsh_dump_tag_data_s  *dump_tag_ptr;
...
#define SEND_REALLOC_REQ(gas_id, mptr, pdu_ptr, num_octets, cv_status)                                              \
  {                                                                                                                 \
    ...
    MSG_GRLC_MED_4_G(GRLC_F3_MSG_ENUM_858, "GRLU RM_UL_REALLOC_REQ prev_realloc_psn: old=%d, new=%d, pt=%d, rp=%d", \
      ul[gas_id].prev_realloc_psn,                                                                                  \
      (pdu_ptr)->psn,                                                                                               \
      (pdu_ptr)->peak_throughput,                                                                                   \
      (pdu_ptr)->radio_priority);                                                                                   \
          ...
          {                                                 \
            ...
            case RM_MCS_6:                                  \
              ...
              {                                             \
                ...
                ERR_GERAN_FATAL_3("ERLU Old PS %d, New PS %d, MCS %d",old_ps,new_ps,mcs); \
              }                                             \
              break;                                        \
            case RM_MCS_3:                                  \
            case RM_MCS_4:                                  \
...
#define IS_DYNAMIC_OPEN(gas_id)  ((ul[gas_id].cur_res.access_mode == RM_DYNAMIC_ALLOC || \
