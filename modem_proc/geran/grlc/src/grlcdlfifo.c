...
***
*** $Header: //components/rel/geran.mpss/7.4.0/grlc/src/grlcdlfifo.c#1 $
***
  ...
  */
  ...
  else
  {
    ...
    else
    {
      ...
      {
        MSG_GRLC_HIGH_3_G(GRLC_F3_MSG_ENUM_1002, "GRLD FIFO IN <before write> ctr=%u in_ind=%u, out_ind=%u", 
...
)
{
  ...
  else
  {
    if (geran_nv_data_pdu_debug_enabled(GERAN_DATA_PDU_DEBUG_GRLC_DL))
    {
      MSG_GRLC_HIGH_3_G(GRLC_F3_MSG_ENUM_1003, "GRLD FIFO OUT <before read> ctr=%u in_ind=%u, out_ind=%u", 
        grlc_dl_fifo[gas_id].ctr, grlc_dl_fifo[gas_id].in_ind, grlc_dl_fifo[gas_id].out_ind);
    }
  ...
  */
  ...
  else
  {
    ...
    */
    if (geran_nv_data_pdu_debug_enabled(GERAN_DATA_PDU_DEBUG_GRLC_DL))
    {
      MSG_GRLC_HIGH_3_G(GRLC_F3_MSG_ENUM_1005, "GRLD FIFO IN <before write> ctr=%u in_ind=%u, out_ind=%u", 
        grlc_dl_fifo[gas_id].ctr, grlc_dl_fifo[gas_id].in_ind, grlc_dl_fifo[gas_id].out_ind);
    }
...
)
{
  ...
  else
  {
    ...
    */
    if (geran_nv_data_pdu_debug_enabled(GERAN_DATA_PDU_DEBUG_GRLC_DL))
    {
      MSG_GRLC_HIGH_3_G(GRLC_F3_MSG_ENUM_1011, "GRLD FIFO OUT <before read> ctr=%u in_ind=%u, out_ind=%u", 
        grlc_dl_fifo[gas_id].ctr, grlc_dl_fifo[gas_id].in_ind, grlc_dl_fifo[gas_id].out_ind);
    }
...
===
===========================================================================*/
void show_bsn_and_fifo_vars (const gas_id_t gas_id)
{
  uint32 u;

  if (geran_nv_data_pdu_debug_enabled(GERAN_DATA_PDU_DEBUG_GRLC_DL) == FALSE)
  {
    return;
  }
  ...
  /* avoid compiler and lint warnings about unused variables when MSG is defined away to nothing */
  (void)u;
...
)
{
  ...
  */
  ...
  for (cnt=0; cnt<2; cnt++)
  {
    ...
    if (cnt == 0 && data_blk_1_ptr->dsm_payload_ptr)
    {
      ...
      MSG_GRLC_HIGH_2_G(GRLC_F3_MSG_ENUM_1020, "ERLD BSN=%u, len=%u",data_blk_1_ptr->bsn, len);
    }
    else if (cnt == 1 && data_blk_2_ptr->dsm_payload_ptr)
    {
      dsm_ptr = gprs_pdu_copy(data_blk_2_ptr->dsm_payload_ptr);
      len = (uint16) GPRS_PDU_LENGTH(dsm_ptr);
      (void) gprs_pdu_strip_head(&dsm_ptr, buf, len);
      MSG_GRLC_HIGH_2_G(GRLC_F3_MSG_ENUM_1021, "ERLD BSN=%u, len=%u", data_blk_2_ptr->bsn, len);
    }
