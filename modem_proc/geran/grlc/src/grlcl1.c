...
***
*** $Header: //components/rel/geran.mpss/7.4.0/grlc/src/grlcl1.c#2 $
***
...
===
===========================================================================*/
void e_grlc_dl_copy_pld_for_dump(uint16* hdr_ptr, uint8* pld1_ptr, uint8* pld2_ptr, const gas_id_t gas_id)
{
  /* Two static index variables, one for each SUB
  */
  static uint8 idx[NUM_GERAN_DATA_SPACES] = {0};

  /* Copy the header */
  (void) memscpy( (void*) dl[gas_id].dump_buff[idx[gas_id]].hdr, sizeof(dl[gas_id].dump_buff[idx[gas_id]].hdr), (void*) hdr_ptr, 6 ); /* 3 of uint16  */

  /* Copy the first DUMP_PLD_LEN octets of the DL blocks   */
  (void) memscpy( dl[gas_id].dump_buff[idx[gas_id]].pld1, sizeof(dl[gas_id].dump_buff[idx[gas_id]].pld1), pld1_ptr, DUMP_PLD_LEN );
  (void) memscpy( dl[gas_id].dump_buff[idx[gas_id]].pld2, sizeof(dl[gas_id].dump_buff[idx[gas_id]].pld2), pld2_ptr, DUMP_PLD_LEN );

  idx[gas_id] = (idx[gas_id] + 1) % (GRLC_DL_PL1_FIFO_SIZE + 1);
}
