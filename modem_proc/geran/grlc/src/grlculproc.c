...
***
*** $Header: //components/rel/geran.mpss/7.4.0/grlc/src/grlculproc.c#2 $
***
  ...
  */
  if(NULL != p_tx_arr[gas_id])
  {
    ...
    for (b=0; b < loop_cnt ; b++)
    {
      ...
      {
        MSG_GRLC_LOW_2_G(GRLC_F3_MSG_ENUM_522, "ERLU tx_arr[%u].dup_dsm_ptr not NULL 0x%x",
          b, TX_ARR(gas_id, b).blk.dup_dsm_ptr);
        GPRS_PDU_FREE(&TX_ARR(gas_id, b).blk.dup_dsm_ptr);
      }

      if (NULL != TX_ARR(gas_id, b).blk.dsm_ptr)
      {
        MSG_GRLC_LOW_2_G(GRLC_F3_MSG_ENUM_523, "ERLU tx_arr[%u].dsm_ptr not NULL 0x%x",
          b, TX_ARR(gas_id, b).blk.dsm_ptr);
...
)
{
  if (cur_ptr->realloc_action == RA_NULL)
  {
    ...
    */
    ...
    else if (IS_GE_MODULO_COMP(ul[gas_id].prev_realloc_psn, next_ptr->psn, 32768))
    {
...
/*===========================================================================
===
===  FUNCTION      GRLC_LL_TBF_ACT_IND()
===
===  DESCRIPTION
===
===    Send DL TBF activate indication or in case of UL TBF, contention 
===    resolution success ind to LLC to inform RLCMAC for activation of 
===    UL or DL TBF. This will be used for triggering logging event
===
===  PARAMS
===    
===
...
===
===========================================================================*/
void grlc_ll_tbf_act_ind (gas_id_t gas_id, tbf_dir_t direction)
{
  gprs_grr_dl_mes_t *llc_msg_ptr;
  dl_state_t        cur_dl_state;
  cur_dl_state = grlc_dl_get_state(gas_id);

  if (direction == TBF_DL && (ul[gas_id].state == US_ACK_XFER || ul[gas_id].state == US_UNACK_XFER))
  {
    return;
  }
  else if (direction == TBF_UL && (cur_dl_state == DS_RX_ACK || cur_dl_state == DS_RX_UNACK))
  {
    return;
  }
  ...
  {
    llc_msg_ptr -> prim   = GRR_TBF_ACT_IND;
    llc_msg_ptr -> gas_id = gas_id;
...
void reprocess( gas_id_t gas_id, grlc_reprocess_cause_t reprocess_cause )
{
  ...
  */
  ...
  UPD_UL_VSF(gas_id);
  GRLC_UL_PROCEDURE_UNLOCK(gas_id);
