...
***
*** $Header: //components/rel/geran.mpss/7.4.0/grlc/src/grlcdltst.c#1 $
***
...
===
===========================================================================*/
void e_dump_dl_pld(const gas_id_t gas_id)
{
  uint8     i, *ptr;
  uint16    *hdr_ptr;
  uint32    a, b, c;
  ...
  for( i = 0; i <= GRLC_DL_PL1_FIFO_SIZE; i++ )
  {
    ...
    MSG_GRLC_HIGH_3_G(GRLC_F3_MSG_ENUM_34, "ERLD dump hdr bsn=%d %08x %08x",
      (E_GET_RAW_DL_BSN1((uint8 *)&dl[gas_id].dump_buff[i].hdr[0])), a, b );
...
===
===========================================================================*/
void show_block(grlc_dl_data_block_t *blk_ptr, const gas_id_t gas_id)
{
