...
***
*** $Header: //components/rel/geran.mpss/7.4.0/grlc/src/grlcdl.c#5 $
***
...
**
** 28ms (6 GSM frames) added to each duration below, to account for the fact
** that GL1 starts T3192 earlier than the point at which the PDAN is sent.
** Note that when the duration zero, 28ms is added as well, to ensure any
** expiry will happen AFTER RRBP was sent
**-----------------------------------------------------------------------
*/
...
{
  qsh_dump_hdr_s hdr;
  grlcdl_qsh_dump_tag_data_s grlcdl_qsh_dump_tag_data;
  grlcdl_qsh_dump_tag_data_s grlcdl_qsh_dump_tag_data_2;
} grlcdl_qsh_tc_dump_collect_s;

/* Pointer declared as type of QSH data DL structure for parser */
grlcdl_qsh_dump_tag_data_s  *grlcdl_qsh_dump_tag_0_ptr;
grlcdl_qsh_dump_tag_data_s  *grlcdl_qsh_dump_tag_1_ptr;

#endif /* FEATURE_QSH_DUMP */

#ifdef FEATURE_QSH_EVENT_METRIC

#define GRLC_QSH_METRIC_DL_COUNT   2
#define GRLC_DL_QSH_NUM_TRACE_MSGS 50

void grlc_dl_qsh_metric_report (gas_id_t gas_id);

/* Array of metric buffers */
qsh_ext_metric_cfg_s grlcdl_qsh_metric_cfg_arr[GRLC_QSH_METRIC_DL_COUNT];

#endif /* FEATURE_QSH_EVENT_METRIC */

#ifdef FEATURE_QSH_MDUMP
QSH_MDUMP_FN_ATTR static void grlcdl_qsh_mdump_collect_data(qsh_client_cb_params_s *param);
...
static void grlc_dl_create_rx_array ( gas_id_t gas_id, uint16 rx_workspace )
{
  ...
  else
  {
    ...
    {
#ifdef FEATURE_QSH_MDUMP
      QSH_MDUMP_SET(QSH_MDUMP_CRASH_CAT_UNKNOWN, QSH_CLT_GRLCDL, QSH_CLT_GRLCDL)
#endif
...
/*===========================================================================
===
===  FUNCTION      GRLC_DL_QSH_FILL_DUMP_DATA()
===
===  DESCRIPTION
===
...
===
============================================================================*/
void grlc_dl_qsh_fill_dump_data (gas_id_t gas_id, grlcdl_qsh_dump_tag_data_s *ptr)
{
  ...
  */
  ptr->dump_dl_data.tbf_mode = dl[gas_id].tbf_mode;

  /* EGPRS Window Size */
  ptr->dump_dl_data.ws = dl[gas_id].ws;

  /* current state of rlc downlink state machine
  */
  ptr->dump_dl_data.state = dl[gas_id].state;
  ...
  */
  ptr->dump_dl_data.vr = dl[gas_id].vr;

  /*
  ** set during each start of DL TBF to know what physical
  ** array size FW is doing EDGE IR.
  */
  ptr->dump_dl_data.ir_oper_win_size = dl[gas_id].ir_oper_win_size;
  /*
  ...
  **
  */
  ptr->dump_dl_data.vr_max = dl[gas_id].vr_max;
  /*
  ** Set when bsn falls outside physical memory space required
  ** by FW for EDGE IR
  */
  ptr->dump_dl_data.ms_out_of_mem = dl[gas_id].ms_out_of_mem;
  ...
  */
  ptr->dump_dl_data.vq = dl[gas_id].vq;
  ...
  */
  ptr->dump_dl_data.ass_start = dl[gas_id].ass_start;
  ptr->dump_dl_data.ass_end = dl[gas_id].ass_end;
  ...
  ** acknack message.
  */
  ptr->dump_dl_data.ssn = dl[gas_id].ssn;
  ...
  */
  ptr->dump_dl_data.b_first_block_ind_sent = dl[gas_id].b_first_block_ind_sent;

  /* current channel coding scheme for gprs or egprs.
  */
  ptr->dump_dl_data.cs = dl[gas_id].cs;

  /* the last valid channel coding scheme
  */
  ptr->dump_dl_data.last_valid_channel_coding = dl[gas_id].last_valid_channel_coding;

  /* current downlink block sequence number.
  ** It is the last bsn that have been assembled into a pdu.
  */
  ptr->dump_dl_data.bsn = dl[gas_id].bsn;

  ptr->dump_dl_data.timer_t3190_expiry_cnt = dl[gas_id].timer_t3190_expiry_cnt;
  ptr->dump_dl_data.timer_t3192_expiry_cnt = dl[gas_id].timer_t3192_expiry_cnt;

  /* ENG MODE block counters*/

  ptr->dump_dl_data.eng_mode_rcvd_dl_block_cnt = dl[gas_id].eng_mode_rcvd_dl_block_cnt;
  ptr->dump_dl_data.eng_mode_rcvd_dup_dl_block_cnt = dl[gas_id].eng_mode_rcvd_dup_dl_block_cnt;

  /* Receive block counter. This is a free running counter initialised at
  ** grlc dl task start up.
  */
  ptr->dump_dl_data.rx_blk_cnt = dl[gas_id].rx_blk_cnt;
  ...
  */
  ptr->dump_dl_data.b_block_tuneaway = dl[gas_id].b_block_tuneaway;

  /* PDU & octet counts
  */
  ptr->dump_dl_data.ack_rx_pdu_cnt = dl_diag_stats_st[gas_id].ack_rx_pdu_cnt;
  ptr->dump_dl_data.ack_rx_oct_cnt = dl_diag_stats_st[gas_id].ack_rx_oct_cnt;
  ptr->dump_dl_data.unack_rx_pdu_cnt = dl_diag_stats_st[gas_id].unack_rx_pdu_cnt;
  ptr->dump_dl_data.unack_pdu_filled_cnt = dl_diag_stats_st[gas_id].unack_pdu_filled_cnt;
  ptr->dump_dl_data.unack_rx_oct_cnt = dl_diag_stats_st[gas_id].unack_rx_oct_cnt;
  ptr->dump_dl_data.unack_oct_filled_cnt = dl_diag_stats_st[gas_id].unack_oct_filled_cnt;
  ptr->dump_dl_data.ack_nack_req_cnt = dl_diag_stats_st[gas_id].ack_nack_req_cnt;

  /* General purpose downlink counters
  */
  ptr->dump_dl_data.dl_gen_purpose_count_a = dl_diag_stats_st[gas_id].dl_gen_purpose_count_a;

} /* grlc_dl_qsh_fill_dump_data */


/*===========================================================================
===
===  FUNCTION      GRLC_DL_QSH_GET_DUMP()
===
===  DESCRIPTION
===
...
===
===========================================================================*/
void grlc_dl_qsh_get_dump (qsh_client_cb_params_s *param)
{
  qsh_client_action_done_s  client_action_done;
  size_t total_size_written = 0, remaining_size = 0;
  gas_id_t gas_id = GERAN_DATA_SPACE_INDEX_1;
  grlcdl_qsh_dump_tag_data_s* dump_ptr = NULL;

  remaining_size = param->action_params.dump_collect.dump_iovec.size_bytes;
  dump_ptr = (grlcdl_qsh_dump_tag_data_s *)(param->action_params.dump_collect.dump_iovec.addr);

  /* Filling the dump data
  */
  if (QSH_DUMP_TAG_ENABLED(param->action_params.dump_collect.dump_tag_mask, GRLCDL_QSH_DUMP_TAG_DATA))
  {
    gas_id = check_gas_id(GERAN_DATA_SPACE_INDEX_1);
    
    if(remaining_size >= sizeof(grlcdl_qsh_dump_tag_data_s))
    {
      /* get header*/
      qsh_client_dump_tag_hdr_init(
        (qsh_dump_tag_hdr_s*)dump_ptr,
        (qsh_dump_tag_t)GRLCDL_QSH_DUMP_TAG_DATA,
        (size_t)sizeof(grlcdl_qsh_dump_tag_data_s)
      );
      
      grlc_dl_qsh_fill_dump_data(gas_id, (grlcdl_qsh_dump_tag_data_s*)dump_ptr);
      ++dump_ptr;
      remaining_size -= sizeof(grlcdl_qsh_dump_tag_data_s);
      total_size_written += sizeof(grlcdl_qsh_dump_tag_data_s);
    }
  }

  if (QSH_DUMP_TAG_ENABLED(param->action_params.dump_collect.dump_tag_mask, GRLCDL_QSH_DUMP_TAG_DATA_2))
  {
    ...
    if(remaining_size >= sizeof(grlcdl_qsh_dump_tag_data_s))
    {
      ...
      qsh_client_dump_tag_hdr_init(
        (qsh_dump_tag_hdr_s*)dump_ptr,
        (qsh_dump_tag_t)GRLCDL_QSH_DUMP_TAG_DATA_2,
        (size_t)sizeof(grlcdl_qsh_dump_tag_data_s)
      );
  ...
  */
  ...
  /* MSG_GERAN_HIGH_1_G ("GRLD qsh_cb written %u in SYNC mode", total_size_written); */

} /* grlc_dl_qsh_get_dump */
...
QSH_MDUMP_FN_ATTR
#endif
void grlc_dl_qsh_debug_callback (qsh_client_cb_params_s *param)
{
  gas_id_t gas_id = GERAN_DATA_SPACE_INDEX_1;
  ...
  if (param == NULL)
  {
    ...
    QSH_MDUMP_SET(QSH_MDUMP_CRASH_CAT_UNKNOWN, QSH_CLT_GRLCDL, QSH_CLT_GRLCDL)
#endif
    ERR_GERAN_FATAL_0("GRLD qsh_debug_cb NULL ptr");
  }
  ...
  */
  switch (param->action)
  {
    ...
    case QSH_ACTION_METRIC_TIMER_EXPIRY:
      ...
      {
        MSG_GRLC_HIGH_1(GRLC_F3_MSG_ENUM_864, "GRLD qsh_cb action = %u", param->action);
      }
      break;
    case QSH_ACTION_METRIC_CFG:
      ...
      */
      dl[gas_id].grlc_dl_qsh_cb_param = *param;

      if (geran_nv_qsh_debug_enabled())
      {
        MSG_GRLC_HIGH_2_G(GRLC_F3_MSG_ENUM_865, "GRLD qsh_cb METRIC_CFG action=%u subs_id=%u",
          param->action,
          param->action_params.metric_cfg.subs_id);
      }

      if (!RLC_DL_IS_QSH_SIG_SET(gas_id))
      {
        /* Set mask to service message in RLC task time
        */
        RLC_DL_SET_QSH_SIG(gas_id);
      }
      else
      {
        MSG_GRLC_ERROR_0_G(GRLC_F3_MSG_ENUM_866, "GRLD qsh_cb sig already set");
      }

      break;
    case QSH_ACTION_DUMP_COLLECT:
      /* get the dump data and comlpete dump action */
      grlc_dl_qsh_get_dump(param);

      break;

#ifdef FEATURE_QSH_MDUMP
    case QSH_ACTION_MDUMP:
      grlcdl_qsh_mdump_collect_data(param);
      break;
#endif

    default:
      MSG_GRLC_ERROR_1(GRLC_F3_MSG_ENUM_867, "GRLD qsh_cb action oor = %u", param->action);
      break;
  }
} /* grlc_dl_qsh_debug_callback */
...
FUNCTION  grlcdl_qsh_mdump_collect_active
...
===========================================================================*/
QSH_MDUMP_FN_ATTR void grlcdl_qsh_mdump_collect_active(gas_id_t gas_id)
{
  qsh_mdump_collect_high(&dl[gas_id], sizeof(grlc_dl_t));
  qsh_mdump_collect_high(&grlc_dl_fifo[gas_id], sizeof(grlc_dl_pl1_fifo_t));
  qsh_mdump_collect_high(&dl_diag_pdu_stat[gas_id], sizeof(grlc_diag_dl_pdu_stat_t));
  qsh_mdump_collect_high(&dl_diag_rx_stat[gas_id], sizeof(grlc_diag_dl_rx_stat_t));
  qsh_mdump_collect_high(&p_rx_arr[gas_id], sizeof(grlc_dl_data_block_t*));

  if (p_rx_arr[gas_id] != NULL)
  {
    if (dl[gas_id].tbf_mode == TBF_MODE_EGPRS)
    {
      qsh_mdump_collect_high(p_rx_arr[gas_id], (sizeof(grlc_dl_data_block_t) * E_RX_WORKSPACE));
    }
    else if (dl[gas_id].tbf_mode == TBF_MODE_GPRS)
    {
      qsh_mdump_collect_high(p_rx_arr[gas_id], (sizeof(grlc_dl_data_block_t) * RM_RLC_RX_WORKSPACE));
    }
  }
}
...
FUNCTION  grlcdl_qsh_mdump_collect_inactive
...
===========================================================================*/
QSH_MDUMP_FN_ATTR void grlcdl_qsh_mdump_collect_inactive(gas_id_t gas_id)
{
  qsh_mdump_collect_high(&dl[gas_id].state, sizeof(dl_state_t));
  qsh_mdump_collect_high(&dl[gas_id].tbf_mode, sizeof(tbf_mode_T));
  qsh_mdump_collect_high(&p_rx_arr[gas_id], sizeof(grlc_dl_data_block_t*));
...
QSH_MDUMP_FN_ATTR void grlcdl_qsh_mdump_collect_data(qsh_client_cb_params_s *param)
{
  gas_id_t gas_id;

  for (gas_id = 0; gas_id < NUM_GERAN_DATA_SPACES; gas_id++)
  {
    /* If invalid as_id is passed, consider each sub as crashing sub */
    if (dl[gas_id].state != DS_NULL)
    {
      grlcdl_qsh_mdump_collect_active(gas_id);
    }
    else
    {
      grlcdl_qsh_mdump_collect_inactive(gas_id);
    }
  }

  qsh_mdump_prioritize_client(QSH_CLT_GRLCDL, QSH_CLT_GRLCUL);
  qsh_mdump_prioritize_client(QSH_CLT_GRLCDL, QSH_CLT_GMAC);
  qsh_mdump_prioritize_client(QSH_CLT_GRLCDL, QSH_CLT_GL1);
  qsh_mdump_prioritize_client(QSH_CLT_GRLCDL, QSH_CLT_GRR);
  qsh_mdump_prioritize_client(QSH_CLT_GRLCDL, QSH_CLT_GLLC);
}
#endif /* FEATURE_QSH_MDUMP */
...
===    only if NV allow_ta_dl_unack allows the blocking 
===
===  DEPENDENCIES
===
===    xxx.
...
{
  if (!grlc_nv_allow_ta_dl_unack() && !dl[gas_id].b_block_tuneaway)
  {
        ...
        */
        ...
        else
        {
          ...
          if (mac_msg_ptr->msg.sig == MR_DL_ASS_IND)
          {
            ...
            /* Send DL_TBF_ACT_IND to LLC */
            grlc_ll_tbf_act_ind (gas_id, TBF_DL);
      ...
      */
      {
        ...
        else
        {
          if( grlc_dl_task_sigs[gas_id] & GRLC_DL_PL1_EPDAN_REQ_SIG )
          {
            e_ds_rx_unack_pending_release_state_pl1_epdan_req_event_handler(gas_id);
          }
          else
          {
...
#ifdef FEATURE_QSH_EVENT_METRIC
  dl[gas_id].metric_dump_ptr = NULL;
  dl[gas_id].metric_context_id = 0;
  dl[gas_id].b_metrics_active = FALSE;
#endif

#ifdef FEATURE_QSH_DUMP
  dl[gas_id].dump_tag_ptr = NULL;
  // grlcdl_qsh_dump_tag_data_s  *grlcdl_qsh_dump_tag_1_ptr;
#endif
...
/*===========================================================================
===
===  FUNCTION      GRLC_Dl_HANLDE_QSH_SIG()
===
===  DESCRIPTION
===
...
===========================================================================*/
__attribute__((section(".uncompressible.text")))
void grlc_dl_handle_qsh_sig (gas_id_t gas_id)
{
  qsh_client_action_done_s  action_done;

  /* Short-cut to call-back parameters
  */
  qsh_client_cb_params_s *cb_params_ptr = &dl[gas_id].grlc_dl_qsh_cb_param;
  ...
  */
  ...
  {
    if (dl[gas_id].b_metrics_active == FALSE)
    {
      /* Set up start address and metric conext id and set metric event to active
      */
      dl[gas_id].metric_dump_ptr = metric_config_params_ptr->start_addr;
      dl[gas_id].metric_context_id = metric_config_params_ptr->metric_context_id;
      dl[gas_id].b_metrics_active = TRUE;
      if (geran_nv_qsh_debug_enabled())
      {
        MSG_GRLC_HIGH_1_G(GRLC_F3_MSG_ENUM_990, "GRLD handle_qsh_sig ACTION_START active addr=0x%x",
          metric_config_params_ptr->start_addr);
      }
    }
    else
    {
      MSG_GRLC_ERROR_0_G(GRLC_F3_MSG_ENUM_991, "GRLD handle_qsh_sig b_metrics_active already set");
      RLC_DL_CLR_QSH_SIG(gas_id);
    }
  }
  else if (metric_config_params_ptr->action == QSH_METRIC_ACTION_STOP)
  {
    ...
    if (geran_nv_qsh_debug_enabled())
    {
      MSG_GRLC_HIGH_0_G(GRLC_F3_MSG_ENUM_992, "GRLD handle_qsh_sig ACTION_STOP inactive");
    }
  }
  else
  {
    MSG_GRLC_ERROR_1_G(GRLC_F3_MSG_ENUM_993, "GRLD unexpected ACTION %d", metric_config_params_ptr->action);
  }
...
} /* grlc_dl_handle_qsh_sig */

/*===========================================================================
===
===  FUNCTION      GRLC_DL_QSH_METRIC_REPORT
===
===  DESCRIPTION
===
...
void grlc_dl_qsh_metric_report (gas_id_t gas_id)
{
  ...
  grlcdl_qsh_metric_data_s      *metric_ptr;           /* ptr to QSH dump loaction for metric data */

  if(dl[gas_id].b_metrics_active == TRUE)
  {
    if (dl[gas_id].metric_dump_ptr != NULL)
    {
      metric_ptr = (grlcdl_qsh_metric_data_s*)dl[gas_id].metric_dump_ptr;

      /* Fill the metric data
      */
      metric_ptr->metrics_data_dl.timer_t3190_expiry_cnt = dl[gas_id].timer_t3190_expiry_cnt;
      metric_ptr->metrics_data_dl.timer_t3192_expiry_cnt = dl[gas_id].timer_t3192_expiry_cnt;

      metric_ptr->metrics_data_dl.ack_rx_pdu_cnt         = dl_diag_stats_st[gas_id].ack_rx_pdu_cnt;
      metric_ptr->metrics_data_dl.ack_rx_oct_cnt         = dl_diag_stats_st[gas_id].ack_rx_oct_cnt;
      metric_ptr->metrics_data_dl.unack_rx_pdu_cnt       = dl_diag_stats_st[gas_id].unack_rx_pdu_cnt;
      metric_ptr->metrics_data_dl.unack_pdu_filled_cnt   = dl_diag_stats_st[gas_id].unack_pdu_filled_cnt;
      metric_ptr->metrics_data_dl.unack_rx_oct_cnt       = dl_diag_stats_st[gas_id].unack_rx_oct_cnt;
      metric_ptr->metrics_data_dl.unack_oct_filled_cnt   = dl_diag_stats_st[gas_id].unack_oct_filled_cnt;
      metric_ptr->metrics_data_dl.ack_nack_req_cnt       = dl_diag_stats_st[gas_id].ack_nack_req_cnt;
    ...
    client_metric_done.metric_context_id =  dl[gas_id].metric_context_id;
    client_metric_done.metric_id = GRLCDL_QSH_METRIC_DATA;
    client_metric_done.client = QSH_CLT_GRLCDL;

      /* QSH retuns pointer to the next metric dump loaction
      */
      dl[gas_id].metric_dump_ptr = qsh_client_metric_log_done(&client_metric_done);
      if (dl[gas_id].metric_dump_ptr == NULL)
      {
        MSG_GRLC_ERROR_0_G(GRLC_F3_MSG_ENUM_994, "ERLD qsh metric_dump_ptr NULL on exit");
      }
    }
    else
    {
      MSG_GRLC_ERROR_0_G(GRLC_F3_MSG_ENUM_995, "ERLD qsh metric_dump_ptr NULL on entry");
    }
  }

} /* grlc_dl_qsh_metric_capture */

/*===========================================================================
===
===  FUNCTION      GRLC_DL_QSH_REGISTER()
===
===  DESCRIPTION
===
===    Register with QSH framework for GPRS RLC Generic Downlink Task
===
===  DEPENDENCIES
===
===    None.
...
===========================================================================*/
__attribute__((section(".uncompressible.text")))
void grlc_dl_qsh_register(void)
{
  qsh_client_reg_s client_reg;

  qsh_client_reg_init(&client_reg);

  qsh_client_metric_cfg_init(grlcdl_qsh_metric_cfg_arr, GRLC_QSH_METRIC_DL_COUNT);

  /* SUB1
  */
  grlcdl_qsh_metric_cfg_arr[0].id = GRLCDL_QSH_METRIC_DATA;
  grlcdl_qsh_metric_cfg_arr[0].sampling_period_ms = 0;
  grlcdl_qsh_metric_cfg_arr[0].subs_id = SYS_MODEM_AS_ID_1;
  grlcdl_qsh_metric_cfg_arr[0].fifo.element_size_bytes = sizeof(grlcdl_qsh_metric_data_s);
  grlcdl_qsh_metric_cfg_arr[0].fifo.element_count_total = GRLC_DL_QSH_NUM_TRACE_MSGS;

  /* SUB2
  */
  grlcdl_qsh_metric_cfg_arr[1].id = GRLCDL_QSH_METRIC_DATA;
  grlcdl_qsh_metric_cfg_arr[1].sampling_period_ms = 0;
  grlcdl_qsh_metric_cfg_arr[1].subs_id = SYS_MODEM_AS_ID_2;
  grlcdl_qsh_metric_cfg_arr[1].fifo.element_size_bytes = sizeof(grlcdl_qsh_metric_data_s);
  grlcdl_qsh_metric_cfg_arr[1].fifo.element_count_total = GRLC_DL_QSH_NUM_TRACE_MSGS;

  client_reg.metric_info.metric_cfg_arr_ptr = &grlcdl_qsh_metric_cfg_arr[0];
  client_reg.metric_info.metric_cfg_count = GRLC_QSH_METRIC_DL_COUNT;
  client_reg.dump_info.max_size_bytes = sizeof(grlcdl_qsh_tc_dump_collect_s) - sizeof(qsh_dump_hdr_s);

  client_reg.client = QSH_CLT_GRLCDL;
  client_reg.major_ver = GRLCDL_QSH_MAJOR_VER ;
  client_reg.minor_ver = GRLCDL_QSH_MINOR_VER;

  client_reg.client_cb_ptr = grlc_dl_qsh_debug_callback;
  ...
  client_reg.cb_action_support_mask |= QSH_ACTION_METRIC_CFG;
   ...
   MSG_GERAN_HIGH_2("client : 0x%x, support mask : %d", client_reg.client, client_reg.cb_action_support_mask);
  qsh_client_reg(&client_reg);

} /* grlc_dl_qsh_register */
...
/*===========================================================================
===
===  FUNCTION      GRLC_DL_GET_STATE()
===
===  DESCRIPTION
===
===  Get function for other modules to query current DL TBF state
===
===  DEPENDENCIES
===
===
...
===
===========================================================================*/
dl_state_t grlc_dl_get_state (gas_id_t gas_id)
{
  return dl[gas_id].state;
}
