...
void rex_context_init(void)
{
   ...
   ASSERT(0 == pthread_key_create(&rex_posix_tls_key, NULL));
}
...
)
{
   ...
   do
   {
      ...
      /* Create the OS TCB */
      p_os_tcb = malloc(sizeof(rex_os_tcb_type));
      if (NULL == p_os_tcb)
      {
...
void *rex_create_context_for_external_thread(rex_task_attributes_type *p_attr)
{
   ...
   do
   {
      ...
      p_tcb = (rex_tcb_type*)malloc(sizeof(rex_tcb_type));
      if (NULL == p_tcb)
      {
         break;
      }
...
int rex_destroy_context_for_external_thread(void)
{
   ...
   ASSERT(0 < pthread_equal(pthread_self(), thread_id));
