...
int rex_create_dpc_task(void)
{
   ...
   info_handle = rcinit_lookup("DPC_Task");
       
   if (!info_handle)
   {
      REX_ERR_FATAL("DPC_Task Task Info Not Found");
   }
   else
   {
      ...
      if ((prio > 255) || (stksz == 0))
      {
         REX_ERR_FATAL("Invalid Priority or Stack Size for DPC_Task");
      }
   }
   ...
   rex_task_attr_set_stack_size(&rex_attr, stksz);
   rex_task_attr_set_prio(&rex_attr, prio);
   rex_task_attr_set_entry_func(&rex_attr, rex_dpc_task, 0);
   rex_task_attr_set_task_name(&rex_attr, "DPC_Task");
