...
def processArgs ():
    ...
    parser.add_option("-y", "--Symbol", dest="symbol_name", help="Symbol Name, INPUT")
...
def PrintConfig(ModName, CONFIG, devcfgdatafilename, ConfigType, structSize, symbolName):
    ...
    if symbolName:
        dalprop_propsinfo_str = symbolName
    elif (ConfigType and (len(ConfigType) !=0)):
        dalprop_propsinfo_str = "DALPROP_PropsInfo_" + ConfigType
    else :
        dalprop_propsinfo_str = "DALPROP_PropsInfo"

    print "dalprop_propsinfo_str : ", dalprop_propsinfo_str
    ...
    CONFIG.write("\n#ifdef DEVCFG_BOOT\n")
    CONFIG.write("devcfgMetaDataType devcfg_meta_data = {{.fixedAddr={0},.structPtrTableSize=sizeof({1}),.propBinSize=sizeof({2}),.deviceListSize=sizeof({3})}} @ DEVCFG_META_SECTION_BASE;\n".format(fixedAddr,dalprop_structptrs_str,dalprop_propbin_str,driver_list_str))
    CONFIG.write("\n#endif\n")
...
def writeStructHeader (filename, ConfigType, symbolName):
    ...
    if symbolName:
        propsInfoSym = symbolName
    else:
        propsInfoSym = "DALPROP_PropsInfo_{}".format(ConfigType)
    structH.write("#define DEVCFG_PROPS_INFO {}".format(propsInfoSym))
    structH.close()
...
def main():
   ...
   if options.cfg_dir_filename != None:
      ...
      PrintConfig(ModName, CONFIG, options.devcfgdata_filename, options.config_type, options.size_of_struct, options.symbol_name)
      CONFIG.close()

   if options.structHeader:
      writeStructHeader(options.structHeader, options.config_type, options.symbol_name)
      return 0
