...
class Config:
    ...
    def parseImageCfgXML(self):
        ...
        for b in image_cfg_dom.getElementsByTagName("blob"):
            if b.getAttribute("type") == 'ram':
                loadtype = 0
            elif b.getAttribute("type") == 'xbl':
                loadtype = 1
            else:
                loadtype = b.getAttribute("type")
            self.blobs.append({"id":b.getAttribute("id"),"name":b.getAttribute("name"),"type":loadtype})
...
if __name__ == '__main__':
    ...
    for b in thisConfig.blobs:
        ...
        bloblist = bloblist + "\t{{.id=\"{}\",.soc_id={},.name=&{},.loadtype={}}},\n".format(b['id'], thisConfig.socHwVersion[b['id']], b['name'], b['type'])
    
    #print "bloblist: ", bloblist, bloblist[: -2]
    #print "soc_id: ", soc_id   

    f=open(sys.argv[2],"w")
    f.write(template.format(externList, bloblist[:-2], len(thisConfig.blobs)))
    f.close()
    exit(0)