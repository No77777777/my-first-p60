...
v66default=os.path.join(default_root,"8.2.08")
v56wlandefault=os.path.join(default_root,"8.0.13")
v67default=os.path.join(default_root,"8.3.04")
v68default=os.path.join(earlyaccess_default_root,"8.4.alpha2")

toolsver_whitelist = ["3.1", "5.0", "5.1", "6.2", "6.4", "7.0", "7.1", "7.2", "7.3", "7.4", "8.0", "8.1", "8.2", "8.3", "8.4"]
#------------------------------------------------------------------------------
...
def generate(env, target='staticv62'):
   ...
   #try to use config dir of source root first, else default to install directory
   env['CONFIG_DIR'] = "{0}/config/{1}".format(env.subst('$QURT_TOP'), target)
   if not os.path.exists(env['CONFIG_DIR']):
      env['CONFIG_DIR'] = "{0}/config".format(os.path.dirname(__file__))
   #default case, an explicit condition for target/config is only needed if you are not following the default convention
   match = re.search("\w+v(\d+)\w*",target)
   hexagon_hw_ver = int(match.group(1))
   if target=='mpqurt':
      hexagon_hw_ver=62
   elif target=='staticv62':
      env['CONFIG_DIR'] = env.subst('$QURT_TOP')+'/config/static'
   elif target=='mpsecure':
      env['TEST_OBJS'] = True # mpsecure is an internal target for advanced testing so we can force TEST_OBJS on for this target.
      hexagon_hw_ver=62
   elif target=='sdkv65':
   ...
   env['HEX_HW_VER'] = hexagon_hw_ver if hexagon_hw_ver >= 10 else hexagon_hw_ver*10
   if not os.path.exists(env.subst('$CONFIG_DIR')):
      print target
   ...
   if hexagon_hw_ver == 66:
      ...
      if target == 'ADSPv66S_2048':
        env.Replace(T32_MCD_ARCH = "v66s_2048")
      if target == 'ADSPv66M_2048':
        env.Replace(T32_MCD_ARCH = "v66m_2048")
   ...
   if hexagon_hw_ver == 67:
      env.Replace(T32_SYS_CPU = "HexagonV67")
      if target == 'modemv67':
        env.Replace(T32_MCD_ARCH = "v67h_3072")
      if target=='modemv67W_1536':
        env.Replace(T32_MCD_ARCH = "v67h_3072")
      if target == 'modemv67H_2560':
        env.Replace(T32_MCD_ARCH = "v67h_2560")
      if target == 'wlanv67_rom':
        env.Replace(T32_MCD_ARCH = "v67t")
   if hexagon_hw_ver == 68:
      #T32 does not support HexagonV68 yet
      env.Replace(T32_SYS_CPU = "HexagonV67")
      if target == 'nspv68':
        env.Replace(T32_MCD_ARCH = "v68n_1024")
   ...
   if 'sdkv' in target:
      env.Append(BUILD_CONFIG_FLAGS = ' -DCONFIG_DEBUG_FUSE_OVERRIDE')
   ...
   if (hex_tools_root):
      ...
      elif hexagon_hw_ver == 68:
         hex_tools_root = v68default
      else:
...
#variable used by testcases/Sconscript
   ...
   if os.name == 'posix':
      ...
      env.Replace(T32MCD = "{0}hexagon-mcd64.so".format(env.subst('$QDSP6ISS')))
   elif os.name == 'nt':
      env['ENV']['TMP'] = os.environ['TMP']
      env.Replace(QURT_MODEL = env.subst('$Q6_RTOS_ROOT')+"/"+env.subst('$TRGT')+"/debugger/Win/qurt_model.dll")
      env.Replace(T32MCD = "{0}/hexagon-mcd64.dll".format(env.subst('$QDSP6ISS')))
   ...
   env.Replace(CPPPATH = [env.subst('$CONFIG_DIR'), env.subst('$Q6_TOOLS_ROOT')+'/qc/bin', '$COMMON_INCLUDE',
                          "$API_DIR/qurt", env.subst('$QURT_TOP')+'/libs/qurt/include', env.subst('$QURT_TOP')+'/api/qurt/restricted',
   ...
   if(version_compare(hex_tools_ver, '8.4.0') > 0):
    env.Append(HEXAGON_WARN = ' -Wno-builtin-requires-header ')
   ...
   if target == 'ADSPv62MP' or target == 'computev65' or target == "sdkv65" or target == "ADSPv65MP_ISV":
      ...
      if(version_compare(hex_tools_ver, '8.2.04') > 0):
         env.Append(CFLAGS = ' -Wno-deprecated-hvx-length ')
   elif target == 'computev66' or target == 'computev66E_256' or target == 'computev66A_512' or target == "sdkv66" or target == "computev66_ISV":
