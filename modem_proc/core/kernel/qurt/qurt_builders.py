...
def generate(env):
   ...
   fill_template_generate(env)
   sim_cfg_file_generate(env)
   t32_script_generate(env)
   qurt_image_build_generate(env)
   return None
...
def cust_config_obj_generate(env):
   ...
   CustConfigObjBuilder = Builder(action = 'python ${{Q6_RTOS_ROOT}}/scripts/qurt_config.py update ${{USE_LLVM_OPT}} -T ${{Q6_TOOLS_ROOT}} -r {0}/.. -o $TARGET $SOURCE'.format(thispath),
...
#debugger stuff
def runDebugger(env, cmd, target):
  # Standard out is a tty.  We are being run interactively on a Linux(?) machine, Start a pty (pseudo terminal) and start the subprocess on that pty
  import pty
  import select
  import signal
  import sys
  ...
  try:
    while True:
        ...
        #  Wait up to 1 second for data to be available on (master)
          ...
          #  There might be residual content available at (master).  Only shut down after (master) has run empty.
...
def test_sim_generate(env):
  ...
  env.AddMethod(test_sim, 'TestSim')

def test_sim(env, targets, image, osamcfg, program, q6cfg, imgDump=None):
  sources = [image, osamcfg, program, q6cfg]
  if imgDump:
    sources.extend(imgDump)
  return env.TestSimulator(targets, sources)
...
def test_sim_builder(env, target, source):
    machine=''
...
def traptab_generator(source, target, env, for_signature):
   pclass_sub_cmd = ''
   for file in source[4:]:
      pclass_sub_cmd += '-pclass_file '
      pclass_sub_cmd += str(file) + ' '
   return ' '.join(['python ', str(source[0]),'-outtab', str(target[0]), '-inc', str(source[2]), '-itraps',str(source[3]), pclass_sub_cmd, env['CC'],env.subst('$CCFLAGS'),' '.join("-I%s" % w for w in env['CPPPATH']),'-E -x c', str(source[1])])
      ...
      for src in psource:
         ...
         if ext == '.c' or ext == '.cpp' or ext == '.s' or ext == '.S':
            ...
            if 'ISLAND' in control and control['ISLAND']:
               ...
               objsrcs += [len(nsources)]
               nsources += [env.CustConfigObj('$BUILD_DIR/cust_configbs.o', src)]
...
def lcs_generate (env):
    ...
    def LcsWrapper(env, target, source, **kw):
        ...
        #
        if not target:
            target = ['$BUILD_DIR/qurt_generated.lcs']
        if not source:
...
#-------------------------------------------------------------------------------
#builder and utilities to build config files generated to run hexagon simulator
def sim_cfg_file_generate(env):
  env['BUILDERS']['SimCfgFileBuilder'] = Builder(action = sim_cfg_file_builder)
  env.AddMethod(sim_cfg_file, 'SimCfgFile')
  
#helper fucntion called by user, specify out file and cosim or dll to go into file, and optional parameters as dictionary entry
def sim_cfg_file(env, outfile, contentDictList):
  filteredDict = {}
  for contentDict in contentDictList:
    if contentDict:
      if isinstance(contentDict, dict):
        filteredDict.update(contentDict)
      else:
        print "Ignoring cosim from {0}, parameter passed is incorrect type".format(env["TEST"])
  return env.SimCfgFileBuilder(target=outfile, source=list(filteredDict), CONTENT_DICT=filteredDict)

#builder called by scons method to make sure nodes get tracked
def sim_cfg_file_builder(env, target, source):
  f = open(str(target[0]),'w+')
  for src in list(env['CONTENT_DICT']):
    f.write("{0} {1}\n".format(env.subst(str(src)), env.subst(env['CONTENT_DICT'][str(src)])))
  f.close()
  AlwaysBuild(target)

#-------------------------------------------------------------------------------
def qurt_image_build_generate(env):
  env.AddMethod(qurt_image_build, 'QurtImageBuild')
  env['BUILDERS']['QurtImageBuildBuilder'] = Builder(generator = qurt_image_build_builder, emitter = qurt_image_build_emitter)
def qurt_image_build(env, target, source, restartableUserProcesses=[], extrafilesduetopoordesign=[], physAddr='0x1e100000'):
  return env.QurtImageBuildBuilder(target, source, RESTART=restartableUserProcesses, EXTRA=extrafilesduetopoordesign, PHYS_ADDR=physAddr)
def qurt_image_build_builder(env, target, source, for_signature):
  restartStr = ""
  if 'RESTART' in env:
    for file in env['RESTART']:
      restartStr += " -R {0}".format(file)
  extraStr = ""
  if 'EXTRA' in env:
    for file in env['EXTRA']:
      extraStr += " --file {0}".format(file)
  sourceStr = ""
  for file in source:
    sourceStr += " {0}".format(file)
  return "python $QURT_IMAGE_BUILD {0} {1} {2} -o {3} -p {4}".format(sourceStr, restartStr, extraStr, target[0], env['PHYS_ADDR'])
def qurt_image_build_emitter(env, target, source):
  if env['RESTART']:
    for process in env['RESTART']:
      env.Depends(target, process)
  if 'EXTRA' in env:
    for extra in env['EXTRA']:
      env.Depends(target, extra)
  for file in target:
    if str(file).endswith('.elf'):
      target.append('_reloc.'.join(str(file).rsplit('.',1)))
  return (target, source)

#-------------------------------------------------------------------------------
#builder and utilities to build config files generated to run hexagon simulator
def fill_template_generate(env):
  env.AddMethod(fill_template, 'FillTemplate')
  env['BUILDERS']['FillTemplateBuilder'] = Builder(action = fill_template_builder)

#helper fucntion called by user, specify out file and cosim or dll to go into file, and optional parameters as dictionary entry
def fill_template(env, outfile, template, replacements):
  return env.FillTemplateBuilder(outfile, template, REPLACEMENTS=replacements)

#builder called by scons method to make sure nodes get tracked
def fill_template_builder(env, target, source):
  templateFH = open(str(source[0]), "r")
  templateStr = templateFH.read()
  templateFH.close()

  f = open(str(target[0]),'w+')
  outStr = templateStr
  for oldStr in list(env['REPLACEMENTS']):
    outStr = outStr.replace(oldStr,env.subst(env['REPLACEMENTS'][oldStr]))
  f.write(outStr)
  f.close()
  AlwaysBuild(target)
...
#-------------------------------------------------------------------------------
#builder and utilities to build config files generated to run hexagon simulator
def t32_script_generate(env):
  env['BUILDERS']['T32Script'] = Builder(action = t32_script_builder)

def t32_script_builder(env, target, source):
  if os.name == 'posix':
    cmds = ["#!/bin/bash\n",
            env.subst("export PATH=$$PATH:${QDSP6QCBIN}")+"\n",
            "export RLM_LICENSE=27999@license-wan-sd1:27999@license-wan-aus1\n",
            "export Q6_L2CFG_BASE_PHYS_ADDR=0x{0:08X}\n".format(env['Q6_L2CFG_BASE_PHYS_ADDR']),
            "export CSR_BASE=0x{0:08X}\n".format(env['CSR_BASE']),
            "export T32SYS=/pkg/qct/software/hexagon/trace32/run/latest\n"
            "$T32SYS/bin/pc_linux64/t32mqdsp6-qt -c t32sim.cfg -s t32sim.cmm\n"]
  else:
    cmds = [env.subst("set PATH=%PATH%;${QDSP6QCBIN};")+"\n",
            "set T32SYS=C:\\t32\n" ,
            "set Q6_L2CFG_BASE_PHYS_ADDR=0x{0:08X}\n".format(env['Q6_L2CFG_BASE_PHYS_ADDR']),
            "set CSR_BASE=0x{0:08X}\n".format(env['CSR_BASE']),
            "%T32SYS%\\bin\\windows64\\t32mqdsp6.exe -c t32sim.cfg\n"]
