...
'''
    ...
    (4th argument should be a list of specs or a dictionary with tags as keys and specs as values)

    import imp
    qurt_island_py_file = '%s/qurt-island-link.py' % QURT_SCRIPT_DIRECTORY
    qurtisland = imp.load_source('qurtisland', qurt_island_py_file)
    spec_dict = {'island': [island_spec_file_name]}    
    qurtisland.create_lcs(output_file_name, template_file_name, None, spec_dict)
'''
...
def create_lcs(linker_script_name, template_script_name, full_file_names, *island_dict, **kw):
    '''
    Read in all of the named spec files and the template script.
    Write out the final linker script.
    island_dict is a dictionary of segment names as Keys and list of spec files as values.
    The segment names are added to the list of 'anchors' and matched with the anchors in the Linker template
    If full_file_names is a non-empty list, then basenames which are retrieved from the list of spec
    files in island_dict are expanded to match any full names in the list. If full_file_names is an 
    empty list or None, then the names in the spec_file_list are not modified at all.  Example:
    ...
    anchors.append('QURTOS_TEXT')
    anchors.append('QURTOS_DATA')
    anchors.append('QURTOS_RODATA')
    
    # If a list is passed, use default anchor 'island'
    if island_dict:
        if isinstance(island_dict[0], list):
            island_dict = {'island': island_dict[0]}
        elif isinstance(island_dict[0], dict):
            island_dict = island_dict[0]
        else:
            # To maintain backward compatibility for full builds
            # Input is a tuple of spec files, tag it to default island segment
            island_dict = {'island': list(island_dict)}
    else:
        island_dict = {'island': []} 
    ...
    for segment in island_dict.keys():
        ...
        for fname in island_dict[segment]:
            segment = segment.lower()
            spec_file_list.append(fname)
            f = open(fname, 'r')
            for specline in f:
                temp = specline.split()
                if len(temp):
                    if temp[0].startswith('#'):
                        ...
                        if temp[2] != '*':
                            ...
                            link_dict[segment + '_sections'].append(temp)
                        else:
                            # Add the entry to the '_specs' list object for the current segment.
                            link_dict[segment + '_specs'].append(temp)
                    else:
    ...
    script_file = ''
    check_qurtos_anchor_present = 0
    qurtos_anchor_present = 0

    while len(tokens) >= 2:
        script_file += tokens[0]
        if 'QURT_ISLAND_UKERNEL' in tokens[1]:
            # There's stuff going into the island
            # Keep code (.text sections) together to prevent potential HW
            # issues due to interspercing of code & data
            script_file += ' .text.ukernel.island : { *(*.text*.ukernel.island) }\n'
            #script_file += ' .rela.ukernel : { *(.rela.*.ukernel.*) }\n'
            script_file += ' .ukernel.island : { *(*.ukernel.island *.export.island) }\n'
            if 'island' not in island_dict.keys():
                check_qurtos_anchor_present = 1
            ...
            # Keep code (.text sections) together to prevent potential HW
            # issues due to interspercing of code & data
            script_file += ' .text.ukernel.main : { *(*.text*.ukernel.main *.text*.ukernel.island) }\n'
            # Add alignment in case the sections go into different segments
            script_file += '. = ALIGN(4K);\n'
        ...
        elif 'QURTOS_TEXT' in tokens[1]:
            script_file += ' *(.text.qurtos.island)\n'
            qurtos_anchor_present = 1
        elif 'QURTOS_RODATA' in tokens[1]:
            script_file += ' *(.rodata.qurtos.island)\n'
        elif 'QURTOS_DATA' in tokens[1]:
            ...
            current_segment = ''
            for segment in island_dict.keys():
               if segment.upper() in tokens[1].split('_')[0]:
                  current_segment = segment
            for nm in link_dict[current_segment + '_names']:
                ...
                script_file += '   %s.IS.%s.island : %s {\n' % (nm, current_segment, section_constraint)
                for i in link_dict[current_segment + '_sections']:
                    if i[2] == nm:
                        if i[0] == '*' and i[1] != '*':
                            # Single object file
                            script_file += '      %s(%s)\n' % (cpath(i[1]), nm)
            ...
            section_type = ''
            for segment in island_dict.keys():
                  if segment.upper() + '_TEXT' in tokens[1]:
                      if segment == 'island':
                            script_file += ' *(.text.qurtos.island)\n'
                      current_segment = segment
                      sections = ['.text .text.*']
                      section_type = 'TEXT'
                      break;
                  elif segment.upper() + '_RODATA' in tokens[1]:
                      if segment == 'island':
                            script_file += ' *(.rodata.qurtos.island)\n'
                      current_segment = segment
                      ...
                      section_type = 'RODATA'
                      break;
                  elif segment.upper() + '_DATA' in tokens[1]:
                      if segment == 'island':
                            script_file += ' *(.data.qurtos.island)\n'
                            script_file += ' *(.bss.qurtos.island)\n'
                      current_segment = segment
            ...
            script_file += ' /* START_' + current_segment.upper() + '_' + section_type + '*/\n'
            for sec in sections:
                for i in link_dict[current_segment + '_specs']:
                    if i[0] == '*' and i[1] != '*':
                        # Single object file
                        script_file += '      %s(%s)\n' % (cpath(i[1]),sec)
                    elif i[1] == '*' and i[0] != '*':
            ...
            script_file += ' /* END_'+ current_segment.upper() + '_' + section_type + '*/\n'
        tokens = tokens[2:]

    script_file += tokens[0]

    if qurtos_anchor_present != 1 and check_qurtos_anchor_present == 1:
        raise Exception('Missing QURTOS Anchors in LCS file. Please add /* QURTOS_TEXT */, /* QURTOS_RODATA */ and /* QURTOS_DATA */')
