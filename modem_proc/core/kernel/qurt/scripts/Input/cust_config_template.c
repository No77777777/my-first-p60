...
INITIALIZATION AND SEQUENCING REQUIREMENTS
        None.
             ...
             Copyright (c) 2013,2019,2022  by Qualcomm Technologies, Inc.  All Rights Reserved.
=============================================================================*/
...
#define CACHEALIGNED_64 __attribute__((aligned(64)))
...
#define KERNEL_MQ                                 \
   typeof(struct {                                     \
         unsigned char a[{:SIZEOF_MQ}]            \
         __attribute__((aligned({:ALIGNOF_MQ}))); \
   })
#define KERNEL_MQMSG                            \
    typeof(struct{                              \
          unsigned char a[{:SIZEOF_MQMSG}]    \
          __attribute__((aligned({:ALIGNOF_MQMSG}))); \
  })
...
CONFIG_SEC_VAR(" ",__attribute__((section(".data.qurtos.main"))), unsigned int, qurtos_island_stack_size, ROUNDUP(ACTUALQURTOSISLANDSTACKSIZE, 32));
...
CONFIG_SEC_VAR(" ",__attribute__((section(".data.qurtos.coldboot"))), struct objcache_config, qurtos_objcache_config, ((struct objcache_config){{:OBJCACHECONFIG}}));
...
#if defined(CONFIG_AUTOSTACK_V1) || defined(GEN_CONFIG_HEADER ) 
...
#ifdef CONFIG_ISLAND
CONFIG_SEC_VAR(" ",__attribute__((section(".data.qurtos.main"))),unsigned, qurtos_island_handle_count, {:QURTOSISLANDHANDLES});
#endif
CONFIG_SEC_VAR(" ",__attribute__((section(".data.qurtos.main"))),unsigned, qurtos_qdi_handle_count, {:QURTOSQDIHANDLES});
...
#if {:ITCMSIZE} > 0
CONFIG_RW_SPACE("Space for saving ITCM during Power Collapse/error handling",
                __attribute__((section(".data.qurtos"))),
                unsigned char [],
                qurt_itcm_err_dump,
                CACHEALIGNED_64,
                {ARRAY_INIT(ROUNDUP({:ITCMSIZE},32)) = 0xFF});
CONFIG_RW_VAR("Pointer to ITCM dump",
               unsigned char *,
               QURTK_itcm_err_dump_ptr,
               &qurt_itcm_err_dump[0]);
...
#define ITCM_SIZE ({:ITCMSIZE} > 0)? ((ROUNDUP({:ITCMSIZE},32))) : 0
CONFIG_RW_VAR("Size for ITCM dump",
              unsigned int,
              QURTK_itcm_dump_size,
              ITCM_SIZE);
...
CONFIG_SEC_VAR(" ",__attribute__((section(".data.qurtos.main"))),unsigned, qurtos_ssr_default, 0x018F0000);
...
CONFIG_SEC_VAR("Default CCR value",
               ...
               {:CACHEATTR});
CONFIG_SEC_VAR(" ",__attribute__((section(".data.qurtos.main"))),unsigned, qurtos_ccr_default, {:CACHEATTR});
...
CONFIG_SEC_VAR("Default USR value",
               ...
               {:USRATTR});
CONFIG_SEC_VAR(" ",__attribute__((section(".data.qurtos.main"))),unsigned, qurtos_usr_default, {:USRATTR});
...
CONFIG_SEC_VAR("L2 cache size configuration",
               ...
               {:L2SIZE});
CONFIG_RO_VAR("L2 cache size in KB",
               unsigned int,
               QURTK_l2cache_size_kb,
               {:L2SIZEKB});
 ...
 *          - QURTK_L2_partition                                            *
 ****************************************************************************/
#ifdef CONFIG_LOW_FOOTPRINT
   #define QURTK_SYSCFG_STATIC {:SYSCFGSTATIC}
   CONFIG_RO_VAR( "static configuration for SYSCFG set at build time in cust_cfg.xml",
                  unsigned int,
                  QURTK_syscfg_static,
                  QURTK_SYSCFG_STATIC );
                  
   #define QURTK_SYSCFG_STATIC_MASK {:SYSCFGSTATICMASK}
   CONFIG_RO_VAR( "static configuration for SYSCFG mask",
                  unsigned int,
                  QURTK_syscfg_static_mask,
                  QURTK_SYSCFG_STATIC_MASK );
#else
   CONFIG_SEC_VAR(" ",__attribute__((section(".data.early.ukernel.main"))),unsigned int, QURTK_syscfg_static, {:SYSCFGSTATIC});
   CONFIG_SEC_VAR(" ",__attribute__((section(".data.early.ukernel.main"))),unsigned int, QURTK_syscfg_static_mask, {:SYSCFGSTATICMASK});
#endif
...
CONFIG_SEC_VAR("",
               __attribute__((section(".start"))),
               unsigned int,
               const QURTK_chicken_bits_rgdr_pigeon,
               {:QURTKCHICKENBITSPIGEONRGDR});


CONFIG_SEC_VAR("",
               __attribute__((section(".start"))),
               unsigned int,
               const QURTK_chicken_bits_rgdr_goose,
               {:QURTKCHICKENBITSGOOSERGDR});
...
CONFIG_SEC_VAR(" ",__attribute__((section(".data.ukernel.main"))),unsigned int, QURTK_timer_freq_init,{:TIMERFREQINIT});
#else
CONFIG_RW_VAR(" ",unsigned int,qurtos_timer_freq_init,{:TIMERFREQINIT});
#endif
 ...
 */
#define NAMED_MMAP_INIT {{:NAMED_MEM_SECTIONS} -1}
...
/* This structure stores config info on callback framework
 * Each instance of the structure corresponds to one process
 */
#define CB_FWK_TABLE_INIT {{:CB_FWK_INFO_TABLE} -1}

typedef struct qurtos_cb_fwk_info
{
  int worker_thread_num;
  int default_prio;
  int queue_max_len;
  int stack_size;
}qurtos_cb_fwk_info_t;

CONFIG_SEC_VAR("Per-process callback framework info is stored here",
               __attribute__((section(".data.qurtos"))),
               qurtos_cb_fwk_info_t [],
               qurtos_cb_fwk_info_table,
               CB_FWK_TABLE_INIT);

CONFIG_SEC_VAR("Callback info data",
               __attribute__((section(".data.qurtos"))),
              unsigned int,
              QURTK_cbinfo_cfg,
              {:CBINFOCFG});
...
/* Define stack memory for fast interrupt callback */
    ...
    CONFIG_SEC_VAR(" ",__attribute__((section(".data"))), unsigned, qurtos_fastint_stack_size, sizeof(qurtos_fastint_stack_island[0]));
    CONFIG_SEC_VAR(" ",__attribute__((section(".data"))), void*, qurtos_fastint_stack_addr, &qurtos_fastint_stack_island[0]);
    CONFIG_RW_VAR("", unsigned, QURTK_fastint_stack_size, sizeof(qurtos_fastint_stack_island[0]));
    CONFIG_RW_VAR("", void *, QURTK_fastint_stack_addr, &qurtos_fastint_stack_island[0]);
  #else
    unsigned long long qurtos_fastint_stack_main[{:QURT_CONFIG_MAX_HTHREADS}][DIVUP({:FASTINTSTACKSIZE},8)];
    CONFIG_SEC_VAR(" ",__attribute__((section(".data"))), unsigned, qurtos_fastint_stack_size, sizeof(qurtos_fastint_stack_main[0]));
    CONFIG_SEC_VAR(" ",__attribute__((section(".data"))), void*, qurtos_fastint_stack_addr, &qurtos_fastint_stack_main[0]);
    CONFIG_RW_VAR("", unsigned, QURTK_fastint_stack_size, sizeof(qurtos_fastint_stack_main[0]));
    CONFIG_RW_VAR("", void *, QURTK_fastint_stack_addr, &qurtos_fastint_stack_main[0]);
  #endif
#endif
...
CONFIG_SEC_VAR(" ",__attribute__((section(".data.qurtos.coldboot"))), unsigned int,QURTOS_HEAP_SIZE, sizeof(qurtos_heap));
...
#ifdef CONFIG_ISLAND
CONFIG_SEC_VAR(" ",__attribute__((section(".data.qurtos.main"))), unsigned int,qurtos_island_heap_size, ROUNDUP({:KERNELHEAPSIZEISLAND},4));
#endif
CONFIG_SEC_VAR(" ",__attribute__((section(".data.qurtos.main"))), unsigned, qurtos_max_user_processes, {:MAXUSERPROCESSES});
CONFIG_SEC_VAR(" ",__attribute__((section(".data.export.main"))), unsigned int, QURTK_MAX_USER_PROCESSES, {:MAXUSERPROCESSES});
...
#define VTLB_CLIENT_COUNT (1+{:MAXUSERPROCESSES})
...
CONFIG_SEC_VAR(" ",__attribute__((section(".data.QURTK_vtlb_client_bitmask"))), unsigned, QURTK_vtlb_client_bitmask, (1u << VTLB_CLIENT_COUNT) - 1);
...
#define PAGETABLE_DEBUG_ENTRIES {:PAGETABLEDEBUGENTRIES}

#if (PAGETABLE_DEBUG_ENTRIES & (PAGETABLE_DEBUG_ENTRIES-1))
   #error code not present
#endif

CONFIG_RW_VAR("",
              unsigned ,
              QURTK_pagetable_debug_entries,
              PAGETABLE_DEBUG_ENTRIES);
              
CONFIG_ZI_SPACE("Array for VLTB Debug Buffer",
                __attribute__((section(".data"))),
                unsigned [PAGETABLE_DEBUG_ENTRIES],
                QURTK_pagetable_debug_buffer,
                CONFIG_DEFAULT);
       
#ifdef CONFIG_ISLAND
#define VTLB_OPS_REAPER_SIZE {:REAPERVTLBQSZ}
struct vtlb_op {
    unsigned int op;
    unsigned tlblo;
    unsigned tlbhi;
};
CONFIG_ZI_SPACE( "VTLB notifications to Island through reaper",
                 CONFIG_DEFAULT,
                 struct vtlb_op [IDIM({:REAPERVTLBQSZ})],
                 QURTK_vtlb_reaper_queue,
                 CONFIG_DEFAULT);
CONFIG_RW_VAR( "Queue size",
               unsigned,
               QURTK_vtlb_reaper_queue_sz,
               {:REAPERVTLBQSZ});
...
CONFIG_ZI_SPACE("Thread list",
                __attribute__((section(".bss.export"))),
                THREAD_LIST [IDIM({:MAXTHREADS})],
                QURTK_thread_list_pid,
                CONFIG_DEFAULT);
...
#ifdef GEN_CONFIG_HEADER
extern unsigned int QURTK_max_mq_count;
extern unsigned int QURTK_max_mq_depth;
extern unsigned int QURTK_max_mq_msg_sz;
extern unsigned int QURTK_mq_entries_array[];
extern unsigned int QURTK_mq_msg_pool[];
#elif CONFIG_SECURE_MQ 
CONFIG_RO_VAR("max mqs", 
              unsigned int,
              QURTK_max_mq_count,
              {:MQMAXCOUNT});

CONFIG_ZI_SPACE("MQ Memory",
                S(".data.ukernel.main",".data.QURTK.SCHEDULER",".bss"),
                KERNEL_MQ [IDIM({:MQMAXCOUNT})],
                QURTK_mq_entries_array,
                CONFIG_DEFAULT);
...
CONFIG_ZI_SPACE("MQ msg Memory",
                S(".data.ukernel.main",".data.QURTK.SCHEDULER",".bss"),
                KERNEL_MQMSG [IDIM({:MQMAXCOUNT}*{:MQMAXDEPTH})],
                QURTK_mq_msg_pool,
                CONFIG_DEFAULT);
...
typedef unsigned int QURTK_trace_mask_t;
CONFIG_SEC_VAR(" ",__attribute__((section(".data.qurtos.main"))), unsigned, qurtos_trace_buffer_size, TBSIZE);

CONFIG_SEC_VAR("",
               SECTION(".data", "QURTK.SCHEDULER"),
               QURTK_trace_mask_t,
               QURTK_config_trace_mask,
			   {:QURTKDEFAULTTRACEMASK});
...
/*
|| We will declare process_reaper_stack, process_reaper_priority
|| and process_reaper_tcb_partition here. qurtos_process_reaper_init() 
|| has been moved out of here to qurtos_process_reaper.c which uses 
|| these variables.
*/
CONFIG_ZI_SPACE("Process reaper stack",
                       __attribute__((section(".bss.user.config"))),
                       unsigned long long [IDIM(DIVUP({:PROCESS_REAPER_STACK_SIZE},8))],
                       qurtos_process_reaper_stack,
                       CONFIG_DEFAULT);

CONFIG_RW_VAR("Process reaper stack size",
                       unsigned int,
                       qurtos_process_reaper_stack_size,
                       sizeof(qurtos_process_reaper_stack));

CONFIG_RW_VAR("Process reaper priority",
                       unsigned int,
                       qurtos_process_reaper_priority,
                       {:PROCESS_REAPER_PRIORITY});

CONFIG_RW_VAR("Process reaper TCB partition",
                       unsigned int,
                       qurtos_process_reaper_tcb_partition,
...
CONFIG_SEC_VAR("", __attribute__((section(".data"))),
               unsigned int, const QURTK_ecc_word_paddr,
               {:ECCWORDADDR});

CONFIG_SEC_VAR("", __attribute__((section(".data"))),
               unsigned int, const QURTK_ecc_word_value,
               {:ECCWORDCONTENT});
...
CONFIG_RW_VAR("",
              ...
              {:MAXTHREADSINTCM});
CONFIG_SEC_VAR(" ",__attribute__((section(".data.qurtos.coldboot"))), unsigned, qurtos_max_threads_in_tcm, {:MAXTHREADSINTCM});
...
#ifdef CONFIG_ISLAND
CONFIG_SEC_VAR(" ",__attribute__((section(".data.export.main"))), unsigned int, QURTK_max_island_int, {:INTISLANDNUM});
#endif

#define INT_TCM_INIT {{:INTTCMDATA}}
CONFIG_RW_VAR("tcm interrupts",
              unsigned short [],
              QURTK_int_tcm,
              INT_TCM_INIT);

CONFIG_RW_VAR("Maximum TCM interrupts",
              unsigned int,
              QURTK_max_tcm_int,
              {:INTTCMNUM});

CONFIG_ZI_SPACE("",
                __attribute__((section(".bss.ukernel.QURTK_l2_tcm_interrupt_table.fast"))),
                ALIGNED_SPACE(16) [IDIM({:INTTCMNUM})],
                QURTK_l2_tcm_interrupt_table,
                CONFIG_DEFAULT);
...
CONFIG_SEC_VAR("The value DTM ATID",
               __attribute__((section(".data"))),
               unsigned int,
               QURTK_dtm_atid,
               {:DTMATIDVAL});
...
CONFIG_SEC_VAR("disabled=0, enabled=1",
              ...
              QURTK_int_privilege,
              {:INTPRIV});
CONFIG_SEC_VAR(" ",__attribute__((section(".data.qurtos.main"))), unsigned char, qurtos_int_privilege, (char)({:INTPRIV}!=0));
