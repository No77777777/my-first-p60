...
class Elf_File:
    ...
    def read_elf_info(self):
        ...
        for p in self.phdr:
            ...
            if padding:
                print 'Padding Program header at %X -> p_memsz increase from %X to %X' % (p.p_vaddr, p.p_memsz, p.p_memsz+padding)
            p.p_memsz += padding
            p.section_list = []
...
def parse_build_elements(cfg, els):
    ...
    class max_elf_tlb(generic_xml):
        defaults = [('value', 0x1000000)] # Default 16MB alignment
        def finalize(self):
    ...
    class section:
        ...
        def add_attr(self,k,v):
            ...
            if k == 'tlb_lock' and v == '1':
                ...
                # Minimum TLB size of an entry to consider for locking
                # Convert to pages
    ...
    def alloc_pages(self, size, align, proposed_cache_policy, range_idx=None):
        ...
        if range_idx != None:
            if range_idx < len(self.ranges):
                ...
                if r[1] >= size:
                    if (r[0] & (size - 1)): # Next physical address is not aligned to the size
                        raise AllocException(r[0])
                    r[0] += size
                    r[1] -= size
                    if r[3] is not None:
...
class QurtElfPatch:
    ...
    def write_tlb_reclaim(self, mask):
        shift = 0
        addr = self.QURTK_tlb_reclaim
        #1 bit per TLB entry
        if self.QURTK_tlb_reclaim:
        ...
        for a in self.all_ranges:
            ...
            while nsize:
                ...
                if best_map.locked or best_map.island:
                    ...
                    # The largest mapping size is defined by max_elf_tlb
                    tmp = self.max_elf_tlb.value/0x1000
                    if best_map.reloc:
    ...
    def get_gen_relocs(self):
        #
        #  Read the QURTK_gen_relocs[] array from the input ELF into a local
        #   attribute called image_gen_relocs.  Each element of this array
        #   is an (addr,data,adjust,addr_of_adjust) tuple.
        #

        if self.QURTK_gen_relocs == 0:
            self.QURTK_gen_relocs = 0xc1b2f8c8       # TBD -- this is a hack, remove it

        self.image_gen_relocs = []
        a,v,j = self.QURTK_gen_relocs,1,1
        while a and v:
            a,v,j = (a+8,)+self.unpack_from('<Ll',a)
            if v:
                d = self.unpack_from('<L',v)[0]
                self.image_gen_relocs.append((v,d,j,a-4))

    def get_phys_relocs(self):
        #
        #  Read the QURTK_phys_relocs[] array from the input ELF into a local
        #   attribute called image_phys_relocs.  Each element of this array
        #   is an (addr,data) tuple.
        #

        if self.QURTK_phys_relocs == 0:
            self.QURTK_phys_relocs = 0xc1b2f900       # TBD -- this is a hack, remove it

        self.image_phys_relocs = []
        a,v = self.QURTK_phys_relocs,1
        while a and v:
            a,v = a+4,self.unpack_from('<L',a)[0]
            if v:
                d = self.unpack_from('<L',v)[0]
                self.image_phys_relocs.append((v,d))

    def find_in_place_pools(self):
        #
        #  This routine finds image pools which can actually be placed at the
        #   location where their contents are located in the input ELF,
        #   rather than being placed at the end of the default physical pool.
        #
        #  First get the set of physpool names which are candidates
        #

        poolnames = list(set([t.physpool for t in self.opt_mappings if t.physpool and t.physpool != '*']))

        #
        #  Convert to the set of physpool class objects
        #

        poolset = [p for p in self.pools if p.name in poolnames]

        adjusted_pools = []

        for p in poolset:
            #
            #  Get the list of relocatable range indices in this pool
            #

            reloc_indices = [i for i,r in enumerate(p.ranges) if p.is_reloc(i)]

            #
            #  Only try to do our magic if there's only one relocatable range
            #

            if len(reloc_indices) != 1:
                continue

            i = reloc_indices[0]
            m = [t for t in self.opt_mappings if t.physpool == p.name]

            if len(m) == 0:
                continue

            #
            #  Get the range of addresses of the mappings as placed in the ELF
            #

            candidate_ppage_lo = min([t.ppage for t in m])
            candidate_ppage_hi = max([t.ppage+t.size-1 for t in m])
            region_size = p.ranges[i][1]

            #
            # Total range of addresses in the input extends beyond the pool region size,
            #  so this pool is not a candidate
            #

            if candidate_ppage_hi >= candidate_ppage_lo + region_size:
                continue

            #
            #  See if any other mappings point into the candidate region.
            #
            ...
            for t in self.opt_mappings:
                if t.ppage > candidate_ppage_lo+region_size-1:
                    continue
                if t.ppage+t.size-1 < candidate_ppage_lo:
                    continue
                ...
                if t.C < 0:
                    dontreclaim.append(t)
                    continue

                print "Found overlap in %s (0x%X, 0x%X) with section %s (0x%X, 0x%X) " \
                        % (p.name, candidate_ppage_lo, candidate_ppage_lo + region_size, t.sname, t.ppage, t.ppage + t.size)

                break
            else:
                #
                #  This pool is a candidate.  It's big enough, and no other
                #   mappings overlap with it.
                #
                #  The list dontcopy has the list of mappings where we can avoid copying.
                #  The list dontreclaim has the list of mappings where we shouldn't reclaim.
                #
                #  To make a "dontcopy" mapping so we don't copy it, just change its
                #   physpool attribute to None.  Also, grab the cache policy if it's present,
                #   and adjust the mapping accordingly.
                #
                #  To make a "dontreclaim" mapping so we don't reclaim it, just change its
                #   overlay attribute to 1.
                #
                adjusted_pools.append((p,i,candidate_ppage_lo))
                print p.name
                print 'LO %X - HI %X; SIZE %X' % (candidate_ppage_lo, candidate_ppage_hi, region_size)
                print p.ranges
                p.loaded_at_boot = 0
                for t in dontcopy:
                    print 'nocopy      %s' % str(t)
                    t.physpool = None
                    if p.ranges[i][3] is not None:
                        t.C = p.ranges[i][3]
                    p.loaded_at_boot += t.size
                for t in dontreclaim:
                    print 'noreclaim   %s' % str(t)
                    t.overlay = 1
        #
        # When we get here, the list adjusted_pools contains a list of the (pool,region_index)
        #  tuples that we were able to adjust.
        #
        # Sort them according to their starting address.
        #

        adjusted_pools.sort(key=lambda x: x[0].ranges[x[1]][0])

        self.get_phys_relocs()
        self.get_gen_relocs()

        #
        # Find out how to adjust effective_memsize in the boot image.
        #  Look for a general relocation directive which results in the
        #  value of effective_memsize.
        #

        em_address,em_adjust = 0,0

        for a,d,j,u in self.image_gen_relocs:
            if d+j == int(self.effective_memsize,0):
                print 'Effective memsize is at addr %X, %X adjusted by %X, change at %X' % (a,d,j,u)
                em_address,em_adjust = u,j

        #
        # For each pool we were able to adjust, go update QURTK_phys_relocs[] with the new
        #  offset from the base of the image.
        #

        for p,i,page in adjusted_pools:
            #
            #  We should find one entry in self.image_phys_reloc_data which matches (page).
            #   Update it with the new offset.
            #
            print 'Adjust pool %s, region index %d, phys offset %X, page %X' % (p.name, i, p.ranges[i][0], page)
            print ['%X->%X' % (a,d) for a,d in self.image_phys_relocs]
            for a,d in self.image_phys_relocs:
                if d == p.ranges[i][0]:
                    new_offset = page - (self.physaddr >> 12)
                    print 'Change from %X to %X' % (d, new_offset)
                    self[(a,4)] = ('<L', new_offset)
                    break
            else:
                raise Exception('Could not find and fix the QURTK_phys_relocs[] entry for %s' % p.name)
            #
            #  If the pool starts at offset self.effective_memsize from the beginning of the image,
            #   we should be able to reclaim that memory.
            #
            if (p.ranges[i][0] << 12) == int(self.effective_memsize,0):
                self.effective_memsize = str((p.ranges[i][0] + p.ranges[i][1]) << 12)
                em_adjust += p.ranges[i][1] << 12
                print 'Write new adjust value: %X <- %X' % (em_address, em_adjust)
                self[(em_address,4)] = ('<l', em_adjust)
    ...
    def move_pool_mappings(self):
        ...
        while cnt < len(w):
            ...
            except AllocException as e:
                # next physical address in the pool is not aligned to the size
                next_phys_addr = e.addr
                print 'WARNING: Below tlb entry size 0x%x not aligned to %s next available phys address 0x%x' \
                                                                % ((t.size << 12), p.name,(next_phys_addr << 12))
                print t

                #Find the TLB size that aligns with the next physical address
                cur_size = t.size
                while (next_phys_addr & (cur_size - 1)):
                    cur_size = cur_size/4  #Get the next possible TLB entry size

                if cur_size == 0: 
                    # No appropriate size found
                    raise Exception('Unable to find suitable physical address from %s' % (p.name))
                else:
                    # Found an appropriate size
                    # Split the mapping into multiple mappings.
                    # FIXME: Image Builder overmaps memory at times to get better TLB usage,
                    #        Check if the split entry is really a gap section in all_mappings
                    print 'Splitting entry into %d' % (t.size/cur_size)
                    tlb_list = []
                    total_size = cur_size

                    #Make copies and append to opt_mappings
                    while total_size < t.size:
                        tmp = copy(t)
                        tmp.size = cur_size 
                        tmp.vpage = t.vpage + total_size  # Not really required, we would replace anyway
                        tmp.ppage = t.ppage + total_size  
                        total_size += cur_size
                        tlb_list.append(tmp)

                    t.size = cur_size # Change size of the original to the new size
                    #Add to the list we are iterating over at the right location
                    w = w[:cnt+1] + tlb_list + w[cnt+1:]
                    #Add to the final list at the right position
                    for map_idx in range(len(self.opt_mappings)):
                        if self.opt_mappings[map_idx].vpage == t.vpage:
                            self.opt_mappings = self.opt_mappings[:map_idx+1] + tlb_list + self.opt_mappings[map_idx+1:]        
                        
                    #try to allocate again, it should succeed next time
                    continue
                    #t.reloc, t.ppage, t.C = p.alloc_pages(t.size, t.size, t.C, t.range_idx)
   ...
   # "This function prepares phys addr for uncompressed sections.
   # They will be used for TLB mappings for Q6 to convert virtual address to physical address"
                    ...
                    for sect in p.section_list:
                        ...
                        if hasattr(sect, 'clade_reg'):
                            ...
                            if re_result:
                                ...
                                print "clade region PA: 0x%x" %(p.p_paddr)
                                break
                            # Clade2 regions
    ...
    def clade_reg_writes(self):
        ...
        clade_struct=[# Clade registers
                      "clade_comp","clade_exception_high","clade_exception_low_small","clade_exception_low_large", "clade_base_paddr", "clade_base_paddr_high",
                      # Clade2 registers
                      "clade2_base_paddr", "clade2_base_paddr_high", "clade2_comp", "clade2_comp_len", "clade2_metadata_base", "clade2_metadata_len",
        ...
        print "clade_base_paddr value1: %x," % (self.clade_base_paddr.value)   #Printing 36bit clade_base_paddr value
        if self.clade_base_paddr.value != 0:
            self.clade_regs[clade_struct.index("clade_base_paddr")] = (self.clade_base_paddr.value) & 0xFFFFFFFF     #Extracting the lower 32bits of 36bits address
            if ((self.clade_base_paddr.value) >> 32) > 0xF:
                raise Exception("error: clade base addr more than 36 bits")
            self.clade_regs[clade_struct.index("clade_base_paddr_high")] = (self.clade_base_paddr.value) >> 32       #Extracting the upper 4 bits of 36bits address
            print "clade_base_paddr value: %x," % ((self.clade_base_paddr.value) & 0xFFFFFFFF)
            print "clade_base_paddr value_high: %x," % ((self.clade_base_paddr.value) >> 32)

        print "clade2_base_paddr value1: %x," % (self.clade2_base_paddr.value)  #Printing 36bit clade2_base_paddr value
        if self.clade2_base_paddr.value != 0:
            self.clade_regs[clade_struct.index("clade2_base_paddr")] = (self.clade2_base_paddr.value) & 0xFFFFFFFF    #Extracting the lower 32bits of 36bits address
            if ((self.clade2_base_paddr.value) >> 32) > 0xF:
                raise Exception("error: clade2 base addr more than 36 bits")
            self.clade_regs[clade_struct.index("clade2_base_paddr_high")] = (self.clade2_base_paddr.value) >> 32      #Extracting the upper 4 bits of 36bits address
            print "clade2_base_paddr value: %x," % ((self.clade2_base_paddr.value) & 0xFFFFFFFF)
            print "clade2_base_paddr value_high: %x," % ((self.clade2_base_paddr.value) >> 32)
    ...
    def phys_island_report(self):
        ...
        for p in pp:
            ...
            print '-' * 40
            loaded_at_boot = getattr(p,'loaded_at_boot',0)
            total_remaining_region -= loaded_at_boot
            print 'Total island space loaded at boot:      %4dK (%d bytes)' % (loaded_at_boot * 4, loaded_at_boot * 4096)
    ...
    def ramfs_populate(self):
        ...
        for x in self.ramfs:
            ...
            sz = getsize(x[0])
            f = open(x[0],'rb')
            paddr = self.physpool_space.find(sz,0x100000,extra_instructions=True).addr
            vaddr = self.vaddr_space.find_optimal_match(sz,match_addr=paddr,extra_instructions=True).addr
    ...
    def ProcessFile(self):
        ...
        if self.CONFIG_ADVANCED_MEMORY:
            ...
            if self.CONFIG_IN_PLACE_ISLAND:
                self.find_in_place_pools()
            ...
            for t in self.opt_mappings:
                ...
                if t.locked and t.reloc and t.overlay != 1 and t.C >= 0:
                    ...
                    t.ppage -= (self.phys_base >> 12)
                    print '%05X000 -> BASE  +  %05X000, length %05X000, perms X:%u W:%u R:%u U:%u G:%u C:%X%s %s' % (t.vpage, t.ppage, t.size,
                                                                                                             t.X, t.W, t.R, t.U, t.G, t.C,
                                                                                                             reclaim_msg, t.sname)
            ...
            for t in self.opt_mappings:
                if t.locked and not t.reloc and t.overlay != 1 and t.C >= 0:
                    reclaim = self.can_be_reclaimed(t)
                    reclaim_msg = ('',' unlock')[reclaim]
                    print '%05X000 -> Physical %05X000, length %05X000, perms X:%u W:%u R:%u U:%u G:%u C:%X%s %s' % (t.vpage, t.ppage, t.size,
                                                                                                             t.X, t.W, t.R, t.U, t.G, t.C,
                                                                                                             reclaim_msg, t.sname)
