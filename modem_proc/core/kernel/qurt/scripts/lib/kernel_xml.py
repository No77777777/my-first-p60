...
kernel_db = [
    ...
    ["reaper_vtlb_queue_size",
                           size_attr, "optional", "REAPERVTLBQSZ",         40     ],
    ["tcm_size",           size_attr, "optional", "TCMSIZE",               32     ],
    ["itcm_size",           size_attr, "optional", "ITCMSIZE",             0      ],
    ...
    ["page_table_debug_entries",    size_attr, "optional", "PAGETABLEDEBUGENTRIES", 512],
...
#MQ_el = Element("message_queue",
#                max_mq_count = (long_attr, "optional"),
#                max_mq_depth = (long_attr, "optional"),
#                max_mq_msg_size = (size_attr, "optional"))

MQmsg_el = Element("message", max_size = (size_attr, "required"))
MQ_el = Element("message_queue",
                MQmsg_el, 
                max_mq_count = (long_attr, "required"),
                max_mq_depth = (long_attr, "required"))
...
Kernel_el = Element("kernel", object_cache_el(), ramfs_scan_el(), Fastint_stack_el, Mprotect.element(), Tcmdump_el, Autostack_el, VTLB_el, MQ_el, MQmsg_el, STM_tracemask_el, *[Element(e[0], value=(e[1],e[2])) for e in kernel_db]) 
...
def collect_kernel_element(cfg):
    ...
    pgtable_debug_entires = int(cfg['PAGETABLEDEBUGENTRIES'], 16)
    if (pgtable_debug_entires & (pgtable_debug_entires -1)):
      raise Exception('Page table debug buffer entries are not multiple of 2... ')
    ...
    #collect mq_element
    if '-DCONFIG_SECURE_MQ' in cfg.build_flags:    
        try:
            mq_element = k.find_child("message_queue")
            max_number_mqs = int(mq_element.max_mq_count);
            cfg['MQMAXCOUNT'] = ('%d' % (max_number_mqs));
            max_depth_mqs = int (mq_element.max_mq_depth);
            cfg['MQMAXDEPTH'] = ('%d' % (max_depth_mqs));
            try:
                msg_element = mq_element.find_child("message")
                max_msg_size = int(msg_element.max_size);
                cfg['MQMAXMSGSZ'] = ('%d' % (max_msg_size));
            except AttributeError:
                raise Exception('MQ misconfiguration... provide maximum message size...')
        except AttributeError:
            #if MQ is enabled all above needs to be setup
            #raise Exception('MQ misconfiguration... provide all 3: max_number messages, max_depth of queue, max_msg size... ')
            #if MQ not setup, but config flag is enbled, do defaults equivalent to mailboxes:
            cfg['MQMAXCOUNT']='1'
            cfg['MQMAXDEPTH']='1'
            cfg['MQMAXMSGSZ']='8'
    ...
    if '-DCONFIG_AUTOSTACK_V1' in cfg.build_flags:
       try: 
