...
#          uint32_t clade_base_paddr_high;
#          uint32_t QURTK_clade2_base_paddr;
#          uint32_t QURTK_clade2_base_paddr_high;
...
def _build_eip_info(physaddr, q6cfg_records, f, infiles, island_moves, clade_registers, opt_mappings, mappings=None):
    ...
    if None not in clade_registers:
        eip_data += encode_eip_record(REGISTER, '<LLLLLLLLLLLLLLLLLLLLLLLLLL', *clade_registers)
...
class ElfInfoPatch:
    ...
    def writeback(self, *args):
        ...
        if len(data) > self.cfg.eip_size:
            raise Exception(" ELF Info Patch area overflow, size of EIP data=%d, configured size=%d, Cannot complete image build." % (len(data), self.cfg.eip_size))
        print 'EIP data size=%d, Configured size=%d' % (len(data), self.cfg.eip_size)
        if len(data) < self.cfg.eip_size:
