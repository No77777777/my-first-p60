...
class QurtElfPatch:
    ...
    def set_secure_program_boundaries(self):
        if self.QURTK_srmimage_start == 0 or self.QURTK_secure_start == 0:
            return None
        srm_first_ppage = QurtInfinity
        srm_last_ppage = -QurtInfinity
        secure_first_ppage = QurtInfinity
        secure_last_ppage = -QurtInfinity
        for s in self.alloc_sections_sorted:
            if hasattr(s,'boot_map_asid'):
                if s.sh_flags & s.SHF_MAP_CAN_BE_SKIPPED:
                    continue
                if hasattr(s,'isSRM'):
                    srm_first_ppage = min(srm_first_ppage, int(s.physaddr) >> 12)
                    srm_last_ppage = max(srm_last_ppage, (int(s.physaddr) + s.sh_size - 1) >> 12)
                else:
                    secure_first_ppage = min(secure_first_ppage, int(s.physaddr) >> 12)
                    secure_last_ppage = max(secure_last_ppage, (int(s.physaddr) + s.sh_size - 1) >> 12)
                
        if srm_first_ppage == QurtInfinity or secure_first_ppage == QurtInfinity:
            return None
        self[(self.QURTK_srmimage_start,4)] = ('<L', srm_first_ppage)
        self[(self.QURTK_srmimage_end,4)] = ('<L', srm_last_ppage+1)
        self[(self.QURTK_secure_start,4)] = ('<L', secure_first_ppage)
        self[(self.QURTK_secure_end,4)] = ('<L', secure_last_ppage+1)
        print "SRM (start,end) is (%X,%X)" % (srm_first_ppage, srm_last_ppage+1)
        print "Secure (start,end) is (%X,%X)" % (secure_first_ppage, secure_last_ppage+1)
    ...
    def optimize_mappings(self):
        ...
        for s in self.mappings:
            print '%08X -> %08X, size %08X %s G:%u RWXU:%u%u%u%u C:%X' % \
                    (s.sh_addr, int(s.physaddr), s.sh_size, s.name, s.G.value(), s.R.value(), s.W.value(), s.X.value(), s.U.value(), s.C.value())
        print '*' * 60
        for s in self.mappings:
            if s.locked.value():
    ...
    def write_output_file(self):
        ...
        output_location += len(pad_string)
    ...
    def main(self, argv):
        ...
        self.set_secure_program_boundaries()
