...
# MemoryCompare provides a memory breakdown of the libraries/object files provided through a spec list (-s) or
# provided individually (-o) and a comparison with input csv sheet. The output is a CSV sheet with the information
# Input CSV sheet should have the following columns for comparison to work - Symbol Name, Size in bytes, Category, Discipline
# Output CSV has the following columns
# Symbol Name,Size in bytes,Category,Discipline,Old Size,New Symbol,Library,Object File,Type,Diff,Diff in KB
def MemoryCompare(base_filename, db_filename):
    symbol_name_db = []
    symbol_name_base = []
    size_base = []
    category_db = []
    ...
    if os.path.isfile(base_filename):
        #Base reference used for getting symbol size comparison 
        input_base_ref_file = csv.DictReader(open(base_filename))
        input_base_ref_fields = input_base_ref_file.fieldnames
        if set(['Symbol Name', 'Size in bytes']).issubset(input_base_ref_fields):
            for row in input_base_ref_file:
                symbol_name_base.append(row["Symbol Name"].strip())
                size_base.append(int(row["Size in bytes"]))
                category_base.append(row["Category"])
                discipline_base.append(row["Discipline"])
        else:
            print >> sys.stderr, "Key Error - Invalid format for base csv file %s" % base_filename
    else:
        print >> sys.stderr, "File %s not found" % base_filename
    
    if os.path.isfile(db_filename):
        #Database used for getting symbol Classification 
        input_db_file = csv.DictReader(open(db_filename))
        input_db_fields = input_db_file.fieldnames
        if set(['Symbol Name', 'Category']).issubset(input_db_fields):
            for row in input_db_file:
                symbol_name_db.append(row["Symbol Name"].strip())
                category_db.append(row["Category"])
                discipline_db.append(row["Discipline"])
        else:
            print >> sys.stderr, "Key Error - Invalid format for db csv file %s" % db_filename
    else:
        print >> sys.stderr, "File %s not found" % db_filename
    
    # file that belongs to qurtos but doesnt have prefix qurtos_
    qurtos_whitelist   = ['obj_cache.c', 'memory.c', 'mmap.c', 'alloc.c', 'heap.c', 'mem_init.c', 'shmem.c', \
                           'mprotect.c', 'memheap_lite.c','strlcatK.c', 'strlenK.c', 'strlcpyK.c', 'strncmpK.c',\
                           'strtoulK.c', 'memory_mapping.c', 'shmem.c','hvx_mgr.c','hvx_mgr_asm.S','obj_cache.c']
    
    print "Generating Memory Breakdown in %s" % os.path.join(os.getcwd(), 'symbol_list.csv')
    with open('symbol_list.csv', 'wb') as writeFile:
        csv_report = csv.writer(writeFile, delimiter=',')
        csv_report.writerow(['Symbol Name','Size in bytes', 'Category', 'Discipline', 'Old Size', 'New Symbol', \
                                'Library', 'Object File', 'Type', 'Diff', 'Diff in KB'])
                    ...
                    if obj in objset:
                        for sym in defdict_obj:
                            if defdict_obj[sym] == obj:
                                ...
                                dis = "Uncategorized"
                                old_sz = 0
                                if sym[-56:] in symbol_name_base:
                                    new = "no"
                                    idx_base = symbol_name_base.index(sym[-56:])
                                    old_sz = size_base[idx_base]
                                    #Take Classification from the database if present
                                    if sym[-56:] in symbol_name_db:
                                        idx_db = symbol_name_db.index(sym[-56:])
                                        cat = category_db[idx_db]
                                        dis = discipline_db[idx_db]
                                    else:
                                      cat = category_base[idx_base]
                                      dis = discipline_base[idx_base]
                                else :    
                                   if sym[-56:] in symbol_name_db:
                                       idx_db = symbol_name_db.index(sym[-56:])
                                       cat = category_db[idx_db]
                                      ...
                                      cat = "Uncategorized"
                                      if os.path.basename(file) == "config.lib":
                                          dis = "Configuration"
                                      
                                      elif "libqurtkernel" in os.path.basename(file) or "crt" in os.path.basename(file):
                                          #import pdb; pdb.set_trace()
                                          filename = obj.split('_patched')
                                          if ( len ( filename) > 1):
                                              filename_str = filename[0] 
                                          else:
                                               filename_str = filename[0][0:-2]
                                                                                          
                                          dis = "kernel"
                                          if 'qurtos' in filename_str:
                                             dis = "qurtos"
                                          else:
                                              for x in qurtos_whitelist:
                                                 if filename_str in x.split('.')[0]:
                                                    dis = "qurtos"
                                                    break
                                      else:
                                        if os.path.basename(file) == "libstd.lib":
                                           dis = "libs\std"
                                        if os.path.basename(file) == "libtimer.a":
                                           dis = "libs\\timer"
                                        if "libqurt_main" in os.path.basename(file) or "libqurt_island" in os.path.basename(file) \
                                                                                         or "libqurt.a" in os.path.basename(file) :
                                           dis = "libs\qurt"
                                csv_report.writerow([sym[-56:],str(size_sym[sym]), cat, dis, old_sz, new, os.path.basename(file), \
                                                       obj, type_sym[sym], size_sym[sym] - old_sz, (size_sym[sym] - old_sz)/1024])
    if verbose:
        total=0
        all_sizes = size_arch
...
def parseArchive(archive_str, archive):
    ...
    for archline in archive_str.split("\n"):
        ...
        temp = temp.replace('(','/',1)
        temp = temp.replace(')','',1)
        # Check if current line is the object file string or not
        # This is to handle libraries which have symbols listed with arguments
        if os.path.dirname(temp) == archive:
            archline = os.path.basename(temp)
        objpattern = re.compile('^(.*\.[a-zA-Z]*):\s')
        # FIXME: Below pattern has to be modified for the 5th capture group to handle symbol names 
        # with arguments, currently it lists all or some portion of the arguments until we hit a
        # whitespace
        ...
        if objmatch:
            ...
            #Hack: Fix for one library whose pattern is different
            if "*UND*" in symmatch.group(2):
               sympattern = re.compile('(\S{8}){1}\s+(\S*)\s+?(\S+){1}\s+(\S{8}){1}\s+(\S+){1}\s*')
               symmatch = sympattern.match(archline)
                ...
                if ((symmatch.group(5) != symmatch.group(3)) and ("df" not in str(symmatch.group(2)))):
                    if ((symmatch.group(3) == "*UND*")):
                        ...
                        if archive:
                            ...
                            try:
                                int(symmatch.group(4),16)
                            except ValueError:
                                raise Exception("Unexpected format in %s, format: %s" % (archive, archline))
                            if (('.data' in current_section)or('.sdata' in current_section)):
                                ...
                                if 'start' in current_section:
                                 sym_type = "start"
                                elif 'init' in current_section:
                                 sym_type = "init"
                                elif 'fini' in current_section:
                                 sym_type = "fini"
                                elif 'ctors' in current_section:
                                 sym_type = "ctors"
                                elif 'dtors' in current_section:
                                 sym_type = "dtors"
                                else:
                                 sym_type = "unknown"
                        ...
                        type_sym[symmatch.group(5)] = sym_type
...
if __name__ == "__main__":
    ...
    Memory breakdown, Classification and Comparison example:
        %prog -b -s <spec_file> -e <objdump_executable> 
        %prog -b -B <base.csv> -D <database.csv> -s <spec_file> -e <objdump_executable> 
    """
    ...
    group = optparse.OptionGroup(parser, "Memory breakdown, Classification and Comparison Options")
    group.add_option('-b', '--breakdown', dest="breakdown", action="store_true", default=False, help="Generate CSV (symbol_list.csv) with Memory breakdown of libraries provided (with -o or -s)")
    group.add_option('-B', '--base', dest="base_filename", action="store", default="qurt_symbolinfo_base.csv", help="Optional: Location of CSV file with symbol information to compare against, default: qurt_symbolinfo_base.csv")
    group.add_option('-D', '--database', dest="database_filename", action="store", default="qurt_symbolinfo_db.csv", help="Optional: Location of CSV file with symbol classification, default: qurt_symbolinfo_db.csv")
    ...
    libqurt_path = []
    for s in libs:
        if "libqurt.a" in s or "libqurt_island.a" in s:
            libqurt_path = s
...
#        libfiles.add(config_lib_path)
    ...
    if breakdown:
        MemoryCompare(base_filename, database_filename)
        sys.exit(0)
