...
v55default=os.path.join(default_root,"5.1.06")
v55sim=os.path.join(default_root,"5.1.06")
v56default=os.path.join(default_root,"5.1.06")
v60default=os.path.join(default_root,"7.2.12")
v61default=os.path.join(default_root,"7.3.08")
v62default=os.path.join(default_root,"7.4.02")
v65default=os.path.join(default_root,"8.1.alpha2")
...
def build_flags(env):
   if os.path.exists(os.path.realpath(env.subst('${QURT_TOP}/kernel/scripts/build_flags.py'))):
...
def generate(env, target='simv5'):
   ...
   if target=='modemv5':
      ...
      env['CONFIG_DIR'] = env.subst('$QURT_TOP')+'/config/MBAv61'
      hexagon_hw_ver=61
   elif target=='MBAv62':
      env['CONFIG_DIR'] = env.subst('$QURT_TOP')+'/config/MBAv62'
      hexagon_hw_ver=62
   elif target=='MBAv65':
      env['CONFIG_DIR'] = env.subst('$QURT_TOP')+'/config/MBAv65'
      hexagon_hw_ver=65
   elif target=='simv55':
      env['CONFIG_DIR'] = env.subst('$QURT_TOP')+'/config/sim'
      hexagon_hw_ver=55
...
#variable used by testcases/Sconscript
   ...
   # Init FLAGS
   env.Replace(CCFLAGS   = "$HEXAGON_MACHINE $HEXAGONCC_WARN $HEXAGON_UNDEF $HEXAGONCC_OPT $HEXAGON_DBG $HEXAGON_LANIND $HEXAGON_STKPRO $BUILD_CCFLAGS $DEBUGFLAGS $CC_DFLAGS $CFLAGS $LLVMCFLAGS $PICCFLAGS $ERRATACFLAGS")
