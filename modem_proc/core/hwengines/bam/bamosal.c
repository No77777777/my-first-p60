...
===============================================================================
 ...
 $Header: //components/rel/core.mpss/10.0/hwengines/bam/bamosal.c#1 $

when       who     what, where, why
--------   ---     ------------------------------------------------------------
12/08/17   PC      Added support for Minidump
10/23/17   KS      Added bam_osal_busywait function
...
void * bam_osal_malloc(uint32 size)
{
    ...
    {
        ptr = md_ss_malloc(size);
		res = DAL_SUCCESS;
    }
    else
    {
        res = DALSYS_Malloc(size, &ptr);
    }
...
*/
uint32 bam_osal_check_if_md_path(void)
{
#ifndef BAM_MBA
    if(md_ss_check_if_in_minidump_mode())
    {
