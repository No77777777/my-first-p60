...
bam_status_type bam_core_dump(bam_handle bamhandle, bam_coredump_config_type *coredump_cfg)
{
     ...
     {
         bam_osal_debug(MSG_HIGH,"Corrupted user input  bamhandle:0x%X, coredump_cfg:0x%X\n", bamhandle, coredump_cfg);
         return BAM_FAILED;
     }
     if(NULL == coredump_cfg->user_buffer)
     {
         bam_osal_debug(MSG_HIGH,"Corrupted user Buffer  user_buffer:0x%X\n", coredump_cfg->user_buffer);
         return BAM_FAILED;
     }
     bam_osal_debug(MSG_HIGH,"Vailid BAM core dump config ptr   coredump_cfg:0x%X\n", coredump_cfg);
     ...
     {
         bam_osal_debug(MSG_HIGH, "bam_get_info: BAM return error \n");
         return BAM_FAILED;
     }
     bam = BAM_GET_DEV_FROM_HANDLE(bamhandle);
     ...
     {
         bam_osal_debug(MSG_HIGH, "bamcore_check: BAM is not enable\n");
         return BAM_FAILED;
     }
     ...
     {
         bam_osal_debug(MSG_HIGH, "Invalid Pipe Mask pipemask:0x%X\n", coredump_cfg->pipe_mask);
         return BAM_FAILED;
     }
     ...
     {
         bam_osal_debug(MSG_HIGH,"Invailid BAM core dump buffer size  bufsize:%d, buf_size:%d\n", bufsize, coredump_cfg->buf_size);
     ...
     if(tgt_cfg == NULL)
     {
         bam_osal_debug(MSG_HIGH,"bam_tgt_getcfg has returned NULL  tgt_cfg:0x%X, user_buffer:0x%X\n", tgt_cfg, bam->bam_cfg.bam_pa);
         return BAM_FAILED;
     }
