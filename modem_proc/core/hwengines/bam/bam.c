...
===============================================================================
 ...
 $Header: //components/rel/core.mpss/10.0/hwengines/bam/bam.c#5 $

when       who     what, where, why
--------   ---     ------------------------------------------------------------
01/09/20   KS      WA to support Multi descriptor use case with the changes of CR2077977
10/14/19   ML      Added changes to allow queuing a cmd desc with eot flag set for crypto bam
12/08/17   PC      Added support for Minidump
10/23/17   KS      EOT is being ignored for MSS CE BAM's descriptor queued on Producer pipe.
03/04/16   SS      Optimized code for secure config
...
#define BAM_MAX_NUM_ITER_EOT_CHECK 500
...
BAM_API_NON_PAGED void bam_pipe_eot_handler(pipe_dev *pipe, bam_result_type *pollresult)
{
          ...
          (pipe->irq_enabled || bamcore_pipe_nwd_done(pipe->bam->bam_cfg.bam_va,pipe->pipe_num)))
    {
        ...
        else
        {
			...
			if (pipe->flags & BAM_FLAG_CHECK_EOT && (pipe->desc_fifo[pipe->acked_offset / BAM_DESC_SIZE].desc_wait_for_eot == WAIT_FOR_EOT))
			{
			    do
				{
...
BAM_API_NON_PAGED bam_handle bam_init(bam_config_type *bam_cfg, bam_callback_type *bam_cb)
{
    ...
    // To check if its Minidump path
    if(BAM_OPT_SET(tgt_cfg->options,BAM_TGT_CFG_ENABLE_MD))
    {
        if(bam_osal_check_if_md_path())
		{
			bams = NULL;
		}
    }
...
bam_handle bam_pipe_init(bam_handle bamhandle, uint32 pipe_num, bam_pipe_config_type *pipe_cfg, bam_callback_type *pipe_cb)
{
    ...
    pipe_cfg->lock_group = bam_tgt_getpipelockgroup(bam->bam_cfg.bam_pa,pipe_num);
...
BAM_API_NON_PAGED bam_status_type bami_pipe_transfer_no_lock(pipe_dev* pipe, bam_paddr buf_pa, uint16 buf_size, uint16 xfer_opts, void* user_data)
{
    ...
    if((xfer_opts & BAM_IOVEC_FLAG_CMD && xfer_opts & BAM_IOVEC_FLAG_EOT) && (BAM_OPT_NOT_SET(pipe->bam->bam_tgt_cfg->options,BAM_TGT_CFG_CHECK_DESC_EOT)))
    {
...
BAM_API_NON_PAGED bam_status_type bam_pipe_transfer(bam_handle pipehandle, bam_paddr buf_pa, uint16 buf_size, uint16 xfer_opts, void *user_data)
{
	...
	{
		if( xfer_opts & BAM_IOVEC_FLAG_EOT)
             pipe->desc_fifo[pipe->desc_offset / BAM_DESC_SIZE].desc_wait_for_eot = WAIT_FOR_EOT;
        else
             pipe->desc_fifo[pipe->desc_offset / BAM_DESC_SIZE].desc_wait_for_eot = DONT_WAIT_FOR_EOT;
...
{
    uint32 producer_bam, consumer_bam;
    ...
    {
	uint32 consumer_evnt_offset, producer_evnt_offset;
