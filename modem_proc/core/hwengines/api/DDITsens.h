...
============================================================================*/
/* $Header: //components/rel/core.mpss/10.0/hwengines/api/DDITsens.h#1 $ */

#include "DalDevice.h"
#include "DALSys.h"
#include "DALDeviceId.h"

#define DALTSENS_INTERFACE_VERSION DALINTERFACE_VERSION(3,1)
...
typedef struct
{
   ...
   int32 nMinDeciDegC;  /**< Minimum threshold temperature in deci (tenths) degrees Celsius */
   int32 nMaxDeciDegC;  /**< Maximum threshold temperature in deci (tenths) degrees Celsius */
} TsensTempRangeType;

typedef struct
{
   int32 nDegC;      /**< Temperature in degrees Celsius */
   int32 nDeciDegC;  /**< Temperature in deci (tenths) degrees Celsius */
} TsensTempType;

typedef struct
{
   TsensTempType tempTriggered;    /**< Temperature value that triggered */
...
{
   DalDevice DalDevice;
   DALResult (*GetTemp)(uint32 ClientHandle,DalDeviceHandle *, uint32 uSensor, TsensTempType *);
   DALResult (*GetTempRange)(uint32 ClientHandle,DalDeviceHandle *, uint32 uSensor, TsensTempRangeType *);
   DALResult (*SetThreshold)(uint32 ClientHandle,DalDeviceHandle *, uint32 uSensor, TsensThresholdType, int32, DALSYSEventHandle);
   DALResult (*SetEnableThresholds)(uint32 ClientHandle,DalDeviceHandle *, DALBOOL);
   DALResult (*GetNumSensors)(uint32 ClientHandle,DalDeviceHandle *, uint32 *);
   DALResult (*CheckCalibration)(uint32 ClientHandle,DalDeviceHandle *, uint32 uSensor);
};
...
DalTsens_GetTemp(DalDeviceHandle *_h, uint32 uSensor, TsensTempType *pTemp)
{
   ...
   return ((DalTsensHandle *)_h)->pVtbl->GetTemp(DALDEVICE_LOCAL_CLIENTID, _h, uSensor, pTemp);
}
...
DalTsens_GetTempRange(DalDeviceHandle *_h, uint32 uSensor, TsensTempRangeType * pTsensTempRange)
{
   ...
   return ((DalTsensHandle *)_h)->pVtbl->GetTempRange(DALDEVICE_LOCAL_CLIENTID, _h, uSensor, pTsensTempRange);
}
...
*/
static __inline DALResult
DalTsens_SetThreshold(DalDeviceHandle *_h, uint32 uSensor, TsensThresholdType eThreshold, int32 nDegC, DALSYSEventHandle hEvent)
{
   return ((DalTsensHandle *)_h)->pVtbl->SetThreshold(DALDEVICE_LOCAL_CLIENTID, _h, uSensor, eThreshold, nDegC, hEvent);
}
...
{
   return ((DalTsensHandle *)_h)->pVtbl->SetEnableThresholds(DALDEVICE_LOCAL_CLIENTID, _h, bEnableThresholds);
}
...
{
   return ((DalTsensHandle *)_h)->pVtbl->GetNumSensors(DALDEVICE_LOCAL_CLIENTID, _h, pnNumSensors);
}
...
{
   return ((DalTsensHandle *)_h)->pVtbl->CheckCalibration(DALDEVICE_LOCAL_CLIENTID, _h, uSensor);
}
