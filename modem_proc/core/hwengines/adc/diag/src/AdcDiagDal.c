/*============================================================================
  ...
  $Header: //components/rel/core.mpss/10.0/hwengines/adc/diag/src/AdcDiagDal.c#1 $

                Copyright (c) 2009-2012, 2014-2015, 2018 Qualcomm Technologies, Inc.
                All Rights Reserved.
...
)
{
  ...
  result &= (int)(pItem->result.channel.nChannelIdx == pCompareVal->nChannelIdx);
  return result;
}
...
PACKED void* AdcDiagDAL_StartReadHandler( PACKED void *req_pkt, uint16 pkt_len )
{
  ...
  for(i = 0; i < pReqPkt->nChannelCount; i++ )
  {
    ...
    pRespPkt->results[i].result = (uint16)result;
...
PACKED void* AdcDiagDAL_RecalibrateHandler( PACKED void *req_pkt, uint16 pkt_len )
{
  ...
  for(i = 0; i<pReqPkt->nChannelCount; i++ )
  {
    ...
    pRespPkt->results[i].channel = pReqPkt->channels[i];
    pRespPkt->results[i].result = (uint16)DalAdc_RequestRecalibration(phAdcDev, &params);
