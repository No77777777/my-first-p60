...
/*============================================================================
  ...
  $Header: //components/rel/core.mpss/10.0/hwengines/adc/devices/vadc/src/DalVAdcFwk.c#1 $$DateTime: 2019/04/24 00:03:26 $$Author: pwbldsvc $
...
------------------------------------------------------------------------------*/
static uint32
VAdc_DalAdcDevice_Detach(uint32 uClientHandle, DalDeviceHandle *h)
{
   uint32 dwref;
...
static DALResult
VAdc_DalAdcDevice_Init(uint32 uClientHandle, DalDeviceHandle *h)
{
...
static DALResult
VAdc_DalAdcDevice_DeInit(uint32 uClientHandle, DalDeviceHandle *h)
{
   VAdcClientCtxt *pCtxt = h->pClientCtxt;
...
static DALResult
VAdc_DalAdcDevice_Close(uint32 uClientHandle, DalDeviceHandle *h)
{
   if (uClientHandle != DALDEVICE_LOCAL_CLIENTID)
   {
      return DAL_ERROR_DEVICE_ACCESS_DENIED;
