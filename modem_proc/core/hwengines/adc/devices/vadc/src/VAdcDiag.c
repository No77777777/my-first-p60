...
/*============================================================================
  ...
  $Header: //components/rel/core.mpss/10.0/hwengines/adc/devices/vadc/src/VAdcDiag.c#1 $$DateTime: 2019/04/24 00:03:26 $$Author: pwbldsvc $
  ...
  )
{
   ...
   if (currentData->eStatus == ADC_DEVICE_RESULT_VALID)
   {
   ...
   log_ptr->client_curr_info.nPhysical   = currentData->nPhysical;
   log_ptr->client_curr_info.nPercent    = currentData->uPercent;
   log_ptr->client_curr_info.nMicrovolts = currentData->uMicrovolts;
   log_ptr->client_curr_info.nRaw        = currentData->uCode;
