...
/*============================================================================
  ...
  $Header: //components/rel/core.mpss/10.0/hwengines/tsens/diag/src/TsensDiag.c#1 $$DateTime: 2019/04/24 00:03:26 $$Author: pwbldsvc $

  when        who  what, where, why
  ----------  ---  -----------------------------------------------------------
  2014-11-03  jjo  Initial revision.
...
static void* TsensDiagHandleGetTemp(void *pReqPkt, uint16 uPktLen)
{
   ...
   for (uIndex = 0; uIndex < uNumSensors; uIndex++)
   {
      ...
      if (result == DAL_SUCCESS)
      {
         pResult->nDeciDegc = tsensResult.nDeciDegC;
      ...
      else
      {
         switch (result)
         {
            ...
            case TSENS_ERROR_DEAD_SENSOR:
               pResult->uResult = TSENS_DIAG_ERROR_DEAD_SENSOR;
            default:
