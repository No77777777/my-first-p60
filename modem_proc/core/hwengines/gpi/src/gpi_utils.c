                           ...
                           U_LONG size)
{
   ...
   if (gpii != NULL)
   {
      gpi_log(GPI_DEBUG, 4, "nbr_avail_ring_elem - GPII-%d Chan %d size %d avail %d!!\n",
              gpii->id, chan, size, avail_elem);
   }
   else
   {
      gpi_log(GPI_DEBUG, 3, "nbr_avail_ring_elem - Queue %d size %d avail %d!!\n",
...
boolean en_q(gpi_work_q *q, gpi_ring_elem *elem)
{
   ...
   {
      gpi_log(GPI_WARNING, 0, "en_q - WARN - Q full!\n");
...
U_LONG get_ring_ptr(gpii_info *gpii, uint8 chan, GPI_RING_PTR_TYPE ptr_type)
{
   ...
   switch (ptr_type)
   {
      ...
      default:
         ptr = 0;
         gpi_log(GPI_ERROR, 1, "get_ring_ptr - ERR - default: %d\n", ptr_type);
   }
...
/**
 * @brief Return QUPv3 type
 *
 * @param[in,out ]     qup           QUP value
 *
 * @return       QUP type.
 *
 */
QUPV3_BLOCK_TYPE get_qup_type(uint32 qup)
{
   return (QUPV3_BLOCK_TYPE)((qup == QUP_SSC) ? MAX_NUM_QUP_BLOCKS - 1 : qup);
   }
