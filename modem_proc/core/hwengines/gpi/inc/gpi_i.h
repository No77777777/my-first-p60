...
--------   ---     ------------------------------------------------------------ 
03/09/18   ah      Hana/Poipu changes.
06/15/17   ah      Synced to commong GPI driver.
04/21/17   ah      Moved NUM_GPII definition to target specific files.
...
#define NUM_GPI_EVT_RING_ELEM        NUM_EVT_RING_ELEM
...
#define GPI_I3C_IBI_PREV_OWN__MASK                    0x1F
#define GPI_I3C_IBI_PREV_OWN__SHIFT                   0x0

#define GPI_I3C_IBI_CURR_OWN__MASK                    0x1F
#define GPI_I3C_IBI_CURR_OWN__SHIFT                   0x0
...
#define GPI_LOCAL_EVT_GLOB__MASK                     0xF000
#define GPI_LOCAL_EVT_GLOB__SHIFT                    0xC
...
{
   QUPV3_BLOCK_TYPE  type;
   gpi_sync_type   qup_mutex;
   boolean         active;
   U_LONG          gsi_pa;
   uint32          qup_irq;
   gpii_info     *gpii[NUM_GPII_PER_QUP];
   uint16          gpii_hw_ready_bmsk;
   uint16          evt_ring_bmsk;
...
#define VALID_QUP(qup)                                                                    \
                   ((qup >= QUP_0) && (qup < MAX_NUM_QUP_BLOCKS))

#define VALID_PROTOCOL(protocol)                                                          \
                   ((protocol >= GPI_SPI_PROTOCOL) && (protocol < GPI_MAX_PROTOCOL))
