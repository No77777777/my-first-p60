...
$Header: //components/rel/core.mpss/10.0/services/time/src/time.c#1 $
...
/*incremental circular counter for number of times time event logging is done*/ 
static uint32 time_event_counter = 0;
...
FUNCTION TIME_SYSTEM_TIME_UPDATE_NOTIFICATION_CB

DESCRIPTION
  This function is called whenever there is a system time
  update, i.e. time updaet on the TOD time base OR time 
  update on ATS_USER base (in case TOD is not available).
...
=============================================================================*/
static void time_system_time_update_notification_cb
(
  /* Data provided during the callback registeration */
  uint32  data,
  
  /* Delta in milliseconds b/w the time updates. */
  int64   delta_ms    
)
{
    time_svc_event_packet_t  time_event_packet;

    /* Update the time event packet */
    time_event_packet.counter = time_event_counter++;
    time_event_packet.tt = timetick_sclk64_get();
    time_event_packet.sum_offset = time_genoff_get_sum_offset(time_get_base);
    time_event_packet.delta = delta_ms;
    time_event_packet.time_base = time_get_base;

    /* Post the diag event */
    event_report_payload (EVENT_TIME_SVC_TIMESTAMP_DATA, 
                (uint8)sizeof(time_event_packet), 
                &time_event_packet);
...
void time_init( void )
{
  ...
  /* Initialize the time-services driver using */
...
=============================================================================*/
void time_find_time_get_base(uint32 data_arg, int64 data_delta)
{
...
void time_init_config( void )
{
  ...
  (void)time_genoff_register_cb(ATS_TOD, time_find_time_get_base, 0);
  
  /* Call once just at init to know if TOD is ready or not */
  time_find_time_get_base(0, 0x1234567890);
  
  /* Save history of below offsets */
  err = time_genoff_save_offsets_for_base(ATS_TOD);
  ATS_MSG_1(" ATS_TOD table err: %d", err);
  err = time_genoff_save_offsets_for_base(ATS_USER);
  ATS_MSG_1(" ATS_USER table err: %d", err);

  /* Register with time-services to get 
   * notifications on system time update
   * for posting event packet */
   time_register_cb_func(time_system_time_update_notification_cb, 0);
...
/*=============================================================================
FUNCTION time_system_time_event_logging

DESCRIPTION
   Do event reporting for the present system time
   if ATS_TOD is NULL then use ATS_USER 
...
SIDE EFFECTS
  None
=============================================================================*/
void  time_system_time_event_logging( void )
{

    time_svc_event_packet_t  time_event_packet= {0};

    time_event_packet.counter = time_event_counter++;
    
    /* Return in case there is no time update yet */
    if ( 0 == time_genoff_get_sum_offset(time_get_base) )
    {
        time_event_packet.time_base = (uint32)time_get_base;
        time_event_packet.tt = timetick_sclk64_get();
    ...
    /* We have some time on the current base, so
       update the provided struct */
    time_event_packet.time_base = time_get_base;
    time_event_packet.tt = timetick_sclk64_get();

    time_event_packet.sum_offset = time_genoff_get_sum_offset(time_get_base);

    if (time_genoff_get_tt_delta_ms(time_get_base,&(time_event_packet.delta))){;}
    /* Post the diag event */
    event_report_payload (EVENT_TIME_SVC_TIMESTAMP_DATA, 
           (uint8)sizeof(time_event_packet), 
           &time_event_packet);
