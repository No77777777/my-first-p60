...
/*===========================================================================
  ...
  $Header: //components/rel/core.mpss/10.0/services/time/src/time_diag.c#1 $
...
#define TIME_DIAG_TEST_CB_FUNC          37 /* send_data 0x4B 0x49 0x00 0x00 37 0x0 */
#define TIME_DIAG_TEST_GENOFF_CB_FUNC   38 /* send_data 0x4B 0x49 0x00 0x00 38 0x0 */
#define TIME_DIAG_TIMESTAMP_EVENT_PKT   39 /* send_data 0x4B 0x49 0x00 0x00 39 0x0 */
...
void time_diag_test_cb_func(uint32 data, int64 delta_ms){
	ATS_ERR("--------TIME_DIAG: time_diag_test_cb_func--------");
	ATS_ERR_1("data = %d",data);
}
...
)
{
  ...
  switch ( req->command )
  {
    ...
    case TIME_GET_MEMHEAP_DEBUG_INFO:
     ...
     case TIME_DIAG_TEST_CB_FUNC:
     {
	 	static uint32 time_cb_data = 1;
        time_register_cb_func(time_diag_test_cb_func,time_cb_data++);
        ATS_ERR_1("TIME_DIAG: time_register_cb_func registered : iter = %d", time_cb_data);
     }
     break;

     case TIME_DIAG_TEST_GENOFF_CB_FUNC:
     {
	 	static uint32 genoff_cb_data = 1;
		ats_error_type ret_val;
		ret_val = time_genoff_register_cb(ATS_1X,time_diag_test_cb_func,genoff_cb_data++);
        ATS_ERR_2("TIME_DIAG: time_genoff_register_cb registered : iter = %d; err = %d", genoff_cb_data, ret_val);
     }
     break;

     case TIME_DIAG_TIMESTAMP_EVENT_PKT:
     {
