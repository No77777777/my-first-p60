...
$Header: //components/rel/core.mpss/10.0/services/time/src/time_genoff.c#1 $

when       who     what, where, why
--------   ---     ------------------------------------------------------------
05/10/15   ab      Added changes for saving offset history in tables
...
/* Pointer to function which will pass time updates to client user PD */
time_genoff_client_update_func time_genoff_update_to_client = NULL;
...
/* Sum generic offset vs timetick table */
typedef struct
{
  uint64           tt;             /* timetick */
  uint64           sum_offset;     /* sum generic offset for the base */
}time_genoff_offset_table_row_type;

typedef struct
{
   /* Time Base whose offset history is in the table */
   time_bases_type                    base;
   /* Indicates how many rows are filled. If this is equal to table size,
      then it could mean table got wrapped. */
   uint32                             filled_rows;
   /* Index indicates next index in the array pointed by table to be filled */
   uint32                             index;
   /* This has the address to the buffer of MAX_TIME_GENOFF_OFFSET_TABLE_SIZE
      where offsets for a base are saved */
   time_genoff_offset_table_row_type *table;
}time_genoff_offset_table_type;

/* Size of each base table */
#define MAX_TIME_GENOFF_OFFSET_TABLE_SIZE 64

/* Max number of bases' tables that can be saved */
#define MAX_TIME_GENOFF_OFFSET_TABLES_ARRAY_SIZE 4

/* Array of offset table structures */
static time_genoff_offset_table_type time_genoff_offset_tables[MAX_TIME_GENOFF_OFFSET_TABLES_ARRAY_SIZE];

/* This indicates how many bases are registered
   for a time base offset history*/
static uint32 time_genoff_offset_table_allocated_cnt = 0;
...
/* Mutex for general genoff operations */
static time_osal_mutex_t             time_genoff_mutex;

static boolean                       time_genoff_inited = FALSE;
...
void time_genoff_init(  void  )
{
  ...
  memset(&time_genoff_mutex, 0x0, sizeof(time_osal_mutex_t));

  /* Init the time genoff mutex */
  TIME_OSAL_MUTEX_INIT(&time_genoff_mutex);
...
)
{
  ...
  /* Mark the update notification cb list pointer as NULL */
  time_genoff->cb_list_ptr = NULL;

  /* Update the number of registered cb functions as zero */
...
( 
  ...
  /* Delta is msecs b/w the time updates */
  int64 delta_ms
)
{
  ...
  /* Get the time genoff base in consideration */
  ptime_genoff = time_genoff_get_pointer((time_bases_type)data);
  ...
  /* Set delta_ms to zero , to avoid compiler warning */
  delta_ms = 0 * delta_ms ;
...
)
{
  ...
  {
    (void)time_genoff_register_cb(ATS_RTC,
                           (time_genoff_t2_cb_type)&time_genoff_cb_update_sum_gen_off, 
        (uint32)time_genoff->bases_type);
  }

  /* Initialize sum_generic_offset */
  time_genoff_cb_update_sum_gen_off ( (uint32)time_genoff->bases_type, 0 );
...
FUNCTION TIME_GENOFF_GET_SUM_OFFSET

DESCRIPTION
  Returns sum generic offset value of a given time base
...
=============================================================================*/
int64 time_genoff_get_sum_offset
( 
    time_bases_type  base  
)
{
  ...
  return ptime_genoff->sum_generic_offset;
...
RETURN VALUE
  boolean as return value which indicates offset is +ve or -ve
...
)
{
  ...
  if((temp_generic_offset >> 32) > TIME_GENOFF_OFFSET_MAX_VAL_UPPER_WORD)
  {
    ...
    /* Re-initialize sum_generic_offset in case total time was invalid */
    time_genoff_cb_update_sum_gen_off ( (uint32)ptime_genoff->bases_type, 0 );
...
FUNCTION TIME_GENOFF_SAVE_OFFSETS_FOR_BASE

DESCRIPTION
  Registers base whose offset history is to be saved

DEPENDENCIES
  None

RETURN VALUE
  Returns Success/Reason for failure as enum

SIDE EFFECTS
  None

=============================================================================*/
ats_error_type time_genoff_save_offsets_for_base(time_bases_type base)
{
   uint32 index;
   void  *tbl_ptr;

   /* Check if base is greater than max static base */
   if(base > ATS_MAX || base == ATS_RTC)
      return ATS_INVALID_BASE_NUM;

   /* Check if time_genoff_init is already called or not */
   if(FALSE == time_genoff_inited)
      return ATS_FAILURE;
   
   TIME_OSAL_MUTEX_LOCK(&time_genoff_mutex);
   
   /* Check if the input base is already registered for logging */
   for(index = 0; index < time_genoff_offset_table_allocated_cnt; index++)
      if(time_genoff_offset_tables[index].base == base)
      {
         TIME_OSAL_MUTEX_UNLOCK(&time_genoff_mutex);
         return ATS_SUCCESS;
      }

   /* If the tables array is exhausted */
   if(time_genoff_offset_table_allocated_cnt == MAX_TIME_GENOFF_OFFSET_TABLES_ARRAY_SIZE)
   {
      TIME_OSAL_MUTEX_UNLOCK(&time_genoff_mutex);
      return ATS_TABLE_EXHAUSTED;
   }
    
   /* Allocate memory from heap for new table */
   tbl_ptr = calloc(MAX_TIME_GENOFF_OFFSET_TABLE_SIZE, sizeof(time_genoff_offset_table_row_type));
   if(tbl_ptr == NULL)
   {
     TIME_OSAL_MUTEX_UNLOCK(&time_genoff_mutex);
   ...
   index = time_genoff_offset_table_allocated_cnt++;
   
   time_genoff_offset_tables[index].base        = base;
   time_genoff_offset_tables[index].filled_rows = 0;
   time_genoff_offset_tables[index].index       = 0;
   time_genoff_offset_tables[index].table       = tbl_ptr;

   /* Free the mutex */
   TIME_OSAL_MUTEX_UNLOCK(&time_genoff_mutex);
   
   return ATS_SUCCESS;
} /* time_genoff_save_offsets_for_base */
...
/* Delta b/w new & old generic offsets */
  ...
  uint64   tt;
  uint64   sum_offset_temp;
  uint32   index, tbl_index;
  uint32   base;

  /* Obtain current timetick */
  tt = timetick_sclk64_get();

  /* Log new offset if the base is registered for saving offset history */
  base = ptime_genoff->bases_type;
  for(index = 0; index < time_genoff_offset_table_allocated_cnt; index++)
  {
    if(time_genoff_offset_tables[index].base==base)
    {
      /* Copy the offset in TS format to temp uint64 variable */
      qw_equ((uint32 *)&sum_offset_temp, ptime_genoff->sum_generic_offset_ts);

      /* Update 63rd bit to 1 if the sum offset is -ve */
      if(ptime_genoff->sum_generic_offset_ts_is_negative)
         sum_offset_temp |= (((uint64)1)<<63);

      tbl_index = time_genoff_offset_tables[index].index;
      time_genoff_offset_tables[index].table[tbl_index].tt = tt;
      time_genoff_offset_tables[index].table[tbl_index].sum_offset = sum_offset_temp;   /* Updating calculated temp sum offset to the table */
      time_genoff_offset_tables[index].index = (time_genoff_offset_tables[index].index + 1) % MAX_TIME_GENOFF_OFFSET_TABLE_SIZE;

      /* Add filled rows by 1 if it didnt cross the max table size. This will indicate
         if the table is full or half filled, which will not be clear from above index. */
      if(time_genoff_offset_tables[index].filled_rows < MAX_TIME_GENOFF_OFFSET_TABLE_SIZE)
         time_genoff_offset_tables[index].filled_rows++;
         break;
    }
  }
  ...
  if ((delta_ms > TIME_GENOFF_REMOTE_UPDATE_MS) || 
    (delta_ms < ((-1)*TIME_GENOFF_REMOTE_UPDATE_MS)) )
    {
      ...
      time_genoff_set_log[time_genoff_set_log_cntr].timetick_now = tt;
...
void time_genoff_print_debug_stats( void )
{
 ...
 /* Print the last update information for TOD base */
  ...
  ATS_MSG_ERROR("TIME_DEBUG: Printing from time_genoff_offset_tables",0,0,0);
  for ( base = 0; base < time_genoff_offset_table_allocated_cnt; base++)
  {
    ATS_ERR_1("TIME_DEBUG: Printing offset history for base %d", time_genoff_offset_tables[base].base);

    for(index=0; index < time_genoff_offset_tables[base].index; index++)
         ATS_ERR_4("TIME_DIAG_DEBUG: index: %d --> tt:0x%x, sum offset=0x%.8x%.8x",
         index,
             time_genoff_offset_tables[base].table[index].tt,
             (uint32)(time_genoff_offset_tables[base].table[index].sum_offset>>32),
             (uint32)(time_genoff_offset_tables[base].table[index].sum_offset));
  }
#endif /* FEATURE_TIME_DEBUG */
...
)
{
  ...
  time_genoff_cb_struct_ptr                  cb_struct_ptr = NULL; 
  ...
  cb_struct_ptr = ptime_genoff->cb_list_ptr;  

  /* Release the mutex */
  TIME_OSAL_MUTEX_UNLOCK(ptime_genoff->mutex);

  /* Call callback functions */
  /* This loop will not be safe in case the dereg function is also enabled.
    * Since then there can be removal of elements from the list while the
    * loop is still executing and the callback are taken from one node.
    */
  ...
  /* Update the data to client PD's to process only if the bases are in the range */
  if ( ( time_genoff_update_to_client != NULL ) &&
         ( delta_ms != 0) )
  {
    /* Pass the updates to remote PD only to internal genoff 
    * bases or to external genoff bases which are created in
    * the remote PD's.
    */
    if (ptime_genoff->bases_type < ATS_MAX)
    {
      time_genoff_update_to_client(ptime_genoff,-1);
    }
    else if (ptime_genoff->bases_type > ATS_PRIVATE && ptime_genoff->remote_pd != TIME_GENOFF_DUMMY_PD_NUMBER) 
    {
       time_genoff_update_to_client(ptime_genoff,ptime_genoff->remote_pd);
    }
  }
...
)
{
  ...
  time_genoff_cb_struct_ptr    cb_struct_ptr = NULL;
  time_genoff_cb_struct_ptr    cb_struct_ptr_temp = NULL ;
  /* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  /* Free any allocated registered cb function */
  cb_struct_ptr = ptime_genoff->cb_list_ptr;
  while(cb_struct_ptr != NULL)
  {
    cb_struct_ptr_temp = cb_struct_ptr->next;
    free(cb_struct_ptr);
    cb_struct_ptr = cb_struct_ptr_temp;
  }
...
FUNCTION TIME_GENOFF_REGISTER_CLIENT_UPDATE_FUNC

DESCRIPTION
  For registering the function pointer used by Guest PD for passing time updates to the client user PD.
...
=============================================================================*/
void time_genoff_register_client_update_func
(
  time_genoff_client_update_func client_update_func_ptr
)
{
  time_genoff_update_to_client = client_update_func_ptr;
}


/*=============================================================================

FUNCTION TIME_GENOFF_GET_INIT_UPDATES

DESCRIPTION
  Provides the updated time genoff offsets to the user PD correspoding to the passed client_index.
...
=============================================================================*/
void time_genoff_get_init_updates
(
  int client_index
)
{
  ...
  for (index = 0 ; index < ATS_MAX  ; index++)
    {
      ptime_genoff = &ats_bases[index];
      if ( ptime_genoff->sum_generic_offset !=0 )
      {
        time_genoff_update_to_client(ptime_genoff, client_index);
      }
    }
}
  ...
  uint32 data )
{
   ...
   time_genoff_cb_struct_ptr cb_struct_ptr_local = NULL;
   time_genoff_cb_struct_ptr cb_struct_ptr_local_temp = NULL;
   ...
   if ( (ptime_genoff = time_genoff_get_pointer(base)) == NULL )
      return ATS_FAILURE;

   /* Allocate memory for the list element */
   if ( (cb_struct_ptr_local = malloc(sizeof(time_genoff_cb_struct_type))) == NULL )
      return ATS_MALLOC_FAIL;

   /* Memset this malloc'ed structure */
   memset(cb_struct_ptr_local, 0x0, sizeof(time_genoff_cb_struct_type));
   ...
   time_genoff_t2_cb_type cb_func )
{
   ...
   /* Pointers for the time update notification structure */
   time_genoff_cb_struct_ptr cb_struct_ptr = NULL;
   time_genoff_cb_struct_ptr update_struct_ptr_prev = NULL;
...
/*=============================================================================
FUNCTION time_genoff_get_tt_delta_ms

DESCRIPTION
  returns the timetick and delta in ms for the given base
...
SIDE EFFECTS
  None
=============================================================================*/
ats_error_type time_genoff_get_tt_delta_ms(
    time_bases_type base,
    uint64 * delta_msecs
)
{
    uint32 index,tbl_index,tbl_index_before;
    ...
    for(index = 0; index < time_genoff_offset_table_allocated_cnt; index++)
    {
        if(time_genoff_offset_tables[index].base == base)
        {
            /* Found a base with matching base
               Update the passed params and return */

             /* Get the latest updated index & the index before it */
             if (time_genoff_offset_tables[index].index == 0)
             {
                tbl_index = MAX_TIME_GENOFF_OFFSET_TABLE_SIZE - 1;
                tbl_index_before = tbl_index - 1;
             }
             else
             {
                tbl_index = time_genoff_offset_tables[index].index - 1;
                if (tbl_index == 0)
                    tbl_index_before = MAX_TIME_GENOFF_OFFSET_TABLE_SIZE - 1;
                else
                    tbl_index_before = tbl_index - 1;
             }
            
             *delta_msecs = time_genoff_offset_tables[index].table[tbl_index].sum_offset -
                        time_genoff_offset_tables[index].table[tbl_index_before].sum_offset;
