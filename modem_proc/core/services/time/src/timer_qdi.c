...
$Header: //components/rel/core.mpss/10.0/services/time/src/timer_qdi.c#1 $ 
...
GET_QUEUE_DATA:
   ...
   qurt_signal_wait_cancellable(&obj->timer_qdi_sig, TIMER_REMOTE_EXPIRY_ENQUEUED, QURT_SIGNAL_ATTR_WAIT_ANY, &sigs);
...
=============================================================================*/
void timer_qdi_remote_handler( timer_client_ptr_type clnt_timer , time_timetick_type run_time_ms)
{
   timer_qdi_queue_type *q_data = NULL;
   uint32           process_idx = clnt_timer->info.process_idx;
   uint32    q_item_count = 0;

   if(process_idx >= TIMER_MAX_CLIENTS)
      ERR_FATAL("ProcessIdx %d >= %d client_timer=0x%x", process_idx, TIMER_MAX_CLIENTS, clnt_timer);
   ...
   if(q_item_count >= TIMER_CLIENT_QUEUE_THRESHOLD)
	...
	TIMER_QDI_FREE();
	ATS_ERR_3("Queue thold reached, Timer=%x;sig=%x;func=%x",
				clnt_timer->timer_client_ptr,
				(uint32)clnt_timer->tcb,
				(uint32)clnt_timer->cbfn.func1);
	return;
   	}
   ...
   q_data->cb_info.timer   		= clnt_timer->timer_client_ptr;
   q_data->cb_info.cb_type 		= clnt_timer->info.callback_type;
   q_data->cb_info.sig_addr 	    = (uint32)clnt_timer->tcb;
   q_data->cb_info.func_addr	    = (uint32)clnt_timer->cbfn.func1;
   q_data->cb_info.data		    = (uint32)clnt_timer->data;   
   if (q_data->cb_info.cb_type == TIMER_FUNC2_CB_TYPE)
   	{
   	 q_data->cb_info.mask_data2 = (uint32)run_time_ms;
   	}
   else
   	{
     q_data->cb_info.mask_data2 = (uint32)clnt_timer->sigs;
   	}
...
static __attribute__((noinline)) timer_error_type timer_qdi_group_delete(int client_handle, timer_group_ptr pgroup, 
                                                                                  unsigned int process_idx)
{
   ...
   status = timer_drv_group_delete(&group, pgroup, process_idx);
...
static __attribute__((noinline)) timer_error_type timer_qdi_group_set_deferrable(int client_handle, timer_group_ptr pgroup, boolean condition,
                                                                                  unsigned int process_idx)
{
   timer_group_type group;
   int              result;
...
static __attribute__((noinline)) timer_error_type timer_qdi_group_enable(int client_handle, timer_group_ptr pgroup)
{
   timer_group_type group;
   int              result;
   timer_error_type status;
   ...
   status = timer_drv_group_enable(&group, pgroup);
...
static __attribute__((noinline)) timer_error_type timer_qdi_group_disable(int client_handle, timer_group_ptr pgroup)
{
   timer_group_type group;
   int              result;
   timer_error_type status;
   ...
   status = timer_drv_group_disable(&group, pgroup);
...
{
   time_timetick_type absolute_expiry_time;
   int                result;
   timer_non_defer_data temp;
   
   absolute_expiry_time = timer_drv_get_first_non_deferrable_timer_expiry(&temp);
   result = qurt_qdi_copy_to_user(client_handle, ptick, &absolute_expiry_time, sizeof(time_timetick_type));
...
static __attribute__((noinline)) timer_error_type timer_qdi_defer_match_interrupt_64(int client_handle, time_timetick_type *ptick)
{
...
static __attribute__((noinline)) timer_error_type timer_qdi_mark_interdependence_by_tid(int client_handle, qurt_thread_t tid1, qurt_thread_t tid2)
{
	timer_error_type status = FALSE;
    status = timer_drv_mark_interdependence_by_tid(tid1, tid2);
    return status;   
} /* timer_qdi_mark_interdependence_by_tid */

static __attribute__((noinline)) timer_error_type timer_qdi_mark_interdependence_by_name
(
      int             client_handle, 
	  char            *thread1_name, 
	  uint16          name_len1,
...
)
{
	...
	result = qurt_qdi_copy_from_user(client_handle, thread1, thread1_name, name_len1);
    if(result)
      return TE_INVALID_PARAMETERS;

    result = qurt_qdi_copy_from_user(client_handle, thread2, thread2_name, name_len2);
    if(result)
     return TE_INVALID_PARAMETERS;  
 
    status = timer_drv_mark_interdependence_by_name(thread1, thread2);
    return status;   
} /* timer_qdi_mark_interdependence_by_name */

static __attribute__((noinline)) void timer_qdi_undefer_match_interrupt(void)
{
   timer_drv_undefer_match_interrupt();
} /* timer_qdi_undefer_match_interrupt */

static __attribute__((noinline)) timer_error_type timer_qdi_def_priority
(
  int                      client_handle, 
...
)
{
   ...
   if(pgroup != NULL)
   {
      ...
      status = timer_drv_def_priority(&timer, ptimer, &group, pgroup, tcb, sigs, func, data, timer_prio, process_idx);
   }
   else
   {
       ATS_INTFREE();
...
static __attribute__((noinline)) timer_error_type timer_qdi_def_osal_priority
(
  int						client_handle, 
  timer_ptr_type			ptimer,
  timer_group_ptr			pgroup,
...
)
{
   ...
   if(pgroup != NULL)
   {
      ...
      status = timer_drv_def_osal_priority(&timer, ptimer, &group, pgroup, notify_type, sigs_func_addr, sigs_mask_data, timer_prio, process_idx);
   }
   else
   {
	   ATS_INTFREE();
...
static __attribute__((noinline)) timer_error_type timer_qdi_def2_priority(int client_handle, timer_ptr_type ptimer,
                                              timer_group_ptr pgroup, timer_priority_enum_type timer_prio, 
                                              unsigned int process_idx )
{
   int result;
   timer_error_type status;
   ...
   if(pgroup != NULL)
   {
      ...
      status = timer_drv_def2_priority(&timer, ptimer, &group, pgroup, timer_prio, process_idx);
   }
   else
   {
	   ATS_INTFREE();
...
static __attribute__((noinline)) timer_error_type timer_qdi_reg(int client_handle, timer_ptr_type ptimer, 
...
static __attribute__((noinline)) timer_error_type timer_qdi_set_64(int client_handle, timer_ptr_type ptimer, time_timetick_type time, 
...
static __attribute__((noinline)) timer_error_type timer_qdi_set_absolute(int client_handle, timer_ptr_type ptimer, time_timetick_type time, unsigned int process_idx)
{
   int result;
   timer_error_type status;
   timer_type timer;
...
static __attribute__((noinline)) timer_error_type timer_qdi_get_64(int client_handle, timer_ptr_type ptimer, timer_unit_type unit,
...
static __attribute__((noinline)) boolean timer_qdi_is_active(int client_handle, timer_ptr_type ptimer, unsigned int process_idx)
{
   int result;
   boolean is_active;
   timer_type timer;
...
static __attribute__((noinline)) timer_error_type timer_qdi_is_periodic(int client_handle, 
	                                          timer_ptr_type ptimer, 
	                                          boolean  *is_periodic,
	                                          ...
	                                          unsigned int process_idx)
{
   ...
   timer_type timer;
   time_timetick_type reload_period = 0;
   timer_error_type status;
   ...
   if (period != NULL)
   	{
   	   status = timer_drv_is_periodic_timer(&timer, ptimer, &periodic, &reload_period, process_idx);
       result = qurt_qdi_copy_to_user(client_handle, period, &reload_period, sizeof(time_timetick_type));
       if(result)
         return TE_INVALID_PARAMETERS;
   	}
   else
	status = timer_drv_is_periodic_timer(&timer, ptimer, &periodic, NULL, process_idx);
 

   result = qurt_qdi_copy_to_user(client_handle, is_periodic, &periodic, sizeof(boolean));
   if(result)
      return TE_INVALID_PARAMETERS;
...
static __attribute__((noinline)) timer_error_type timer_qdi_get_expiry_type(int client_handle, 
	                                          timer_ptr_type ptimer, 
	                                          timer_expiry_type *expiry_type,
	                                          unsigned int process_idx)
{
   int result;
   timer_expiry_type expiry = TIMER_RELATIVE_EXPIRY;
   timer_type timer;
   timer_error_type status;
   ...
   status = timer_drv_get_expiry_type(&timer, ptimer, &expiry, process_idx);

   result = qurt_qdi_copy_to_user(client_handle, expiry_type, &expiry, sizeof(timer_expiry_type));
   if(result)
    return TE_INVALID_PARAMETERS;
...
static __attribute__((noinline)) timer_error_type timer_qdi_get_group(int client_handle, 
	                                          timer_ptr_type ptimer, 
	                                          timer_group_ptr pgroup,
	                                          unsigned int process_idx)
{
   ...
   status = timer_drv_get_group(&timer, ptimer, &group, process_idx);
...
static __attribute__((noinline)) timer_error_type timer_qdi_expires_at_64(int client_handle, timer_ptr_type ptimer, 
                                                       unsigned int process_idx, time_timetick_type *ptick)
{
   int result;
   timer_type timer;
...
static __attribute__((noinline)) timer_error_type timer_qdi_get_start_64(int client_handle, timer_ptr_type ptimer, 
                                                      unsigned int process_idx, time_timetick_type *ptick)
{
   int result;
   timer_type timer;
...
static __attribute__((noinline)) timer_error_type timer_qdi_get_params(int client_handle, timer_ptr_type ptimer, 
                                                      timer_attrib_struct_type * timer_attrib_ptr, unsigned int process_idx)
{
   int result;
   timer_type timer;
   timer_error_type status;
   ...
   status = timer_drv_get_params(&timer, ptimer, &timer_attrib, process_idx);
   
   result = qurt_qdi_copy_to_user(client_handle, timer_attrib_ptr, &timer_attrib, sizeof(timer_attrib_struct_type));
   if(result)
   	{
   	  ATS_INTFREE();
      return TE_INVALID_PARAMETERS;
...
static __attribute__((noinline)) timer_error_type timer_qdi_set_params(int client_handle, timer_ptr_type ptimer, timer_attrib_set_type to_set,
                                                      timer_attrib_struct_type * timer_attrib_ptr, unsigned int process_idx)
{
   int result;
   timer_type timer;
   ...
   result = qurt_qdi_copy_from_user(client_handle, &timer_attrib, timer_attrib_ptr, sizeof(timer_attrib_struct_type));
   if(result)
   	{
   	  ATS_INTFREE();
      return TE_INVALID_PARAMETERS;   
   ...
   status = timer_drv_set_params(&timer, ptimer, to_set, &timer_attrib, process_idx);
...
static __attribute__((noinline)) timer_error_type timer_qdi_stop(int client_handle, timer_ptr_type ptimer, 
                                                timer_unit_type unit, time_timetick_type *ptick, 
                                                unsigned int process_idx)
{
   int result;
   timer_type timer;
   ...
   status = timer_drv_stop(&timer, ptimer, unit, &tick, process_idx);
...
static __attribute__((noinline)) timer_error_type timer_qdi_undef(int client_handle, timer_ptr_type ptimer, unsigned int process_idx)
{
   int result;
   timer_type timer;
   timer_error_type status;
...
static __attribute__((noinline)) timer_error_type timer_qdi_pause(int client_handle, timer_ptr_type ptimer, unsigned int process_idx)
{
   int result;
   timer_type timer;
   timer_error_type status;
...
static __attribute__((noinline)) timer_error_type timer_qdi_resume(int client_handle, timer_ptr_type ptimer, unsigned int process_idx)
{
   int result;
   timer_type timer;
   timer_error_type status;
...
static __attribute__((noinline)) timer_error_type timer_qdi_clr_osal_thread_timers(time_osal_thread_id thread_id)
{
   return timer_drv_clr_osal_thread_timers(thread_id);
} /* timer_qdi_clr_osal_thread_timers */

static __attribute__((noinline)) timer_error_type timer_qdi_clr_task_timers(rex_tcb_type * tcb_ptr)
{
   return timer_drv_clr_task_timers(tcb_ptr);
} /* timer_qdi_clr_task_timers */

static __attribute__((noinline)) timer_error_type timer_qdi_get_sclk_till_expiry_64(int client_handle, time_timetick_type *ptick)
{
...
static __attribute__((noinline)) timer_error_type timer_qdi_set_wakeup_64(time_timetick_type tick)
{
   return timer_drv_set_wakeup_64(tick);
} /* timer_qdi_set_wakeup_64 */

static __attribute__((noinline)) timer_error_type timer_qdi_register_for_non_defer_expiry_change(
	                       timer_notify_type		notify_type,
...
static __attribute__((noinline)) void timer_qdi_restart(void)
{
   timer_drv_restart();
} /* timer_qdi_restart */
                          ...
                          qurt_qdi_arg_t a9)
{
   ...
   switch(method)
   {
      ...
      case TIMER_QDI_GROUP_DELETE: 
         return timer_qdi_group_delete(client_handle, a1.ptr, process_idx);
      ...
      case TIMER_QDI_DEF_PRIORITY:  
         return timer_qdi_def_priority(client_handle, a1.ptr, a2.ptr, a3.ptr, a4.num, a5.ptr, a6.num, a7.num, process_idx);		 
      
      case TIMER_QDI_DEF_OSAL_PRIORITY:  
         return timer_qdi_def_osal_priority(client_handle, a1.ptr, a2.ptr, a3.num, a4.ptr, a5.num, a6.num, process_idx);
         
      case TIMER_QDI_DEF2_PRIORITY: 
         return timer_qdi_def2_priority(client_handle, a1.ptr, a2.ptr, a3.num, process_idx);
      ...
      case TIMER_QDI_IS_PERIODIC: 
         return timer_qdi_is_periodic(client_handle, a1.ptr, a2.ptr, a3.ptr, process_idx);

	  case TIMER_QDI_GET_EXPIRY_TYPE:
	  	return timer_qdi_get_expiry_type(client_handle, a1.ptr, a2.ptr, process_idx); 

	  case TIMER_QDI_GET_GROUP:
	  	return timer_qdi_get_group(client_handle, a1.ptr, a2.ptr, process_idx); 
	  ...
	  case TIMER_QDI_GET_PARAMS:
	  	return timer_qdi_get_params(client_handle, a1.ptr, a2.ptr, process_idx); 

	  case TIMER_QDI_SET_PARAMS:
	  	return timer_qdi_set_params(client_handle, a1.ptr, a2.num, a3.ptr, process_idx); 
         
      case TIMER_QDI_STOP: 
        return timer_qdi_stop(client_handle, a1.ptr, a2.num, a3.ptr, process_idx); 
      ...
      case TIMER_QDI_CLR_OSAL_THREAD_TIMERS: 
        return timer_qdi_clr_osal_thread_timers(a1.num); 

	  case TIMER_QDI_CLR_TASK_TIMERS:
	  	return timer_qdi_clr_task_timers(a1.ptr); 
      ...
      case TIMER_QDI_GET_CB_INFO:
    	 return timer_qdi_get_cb_info(client_handle, clntobj, a1.ptr);
	  
	  case TIMER_QDI_MARK_INTERDEPENDENCY_BY_TID:
	     if(process_idx != 0)
           return TE_FAIL;       
  	     return timer_qdi_mark_interdependence_by_tid(client_handle, a1.num, a2.num);
		 
	  case TIMER_QDI_MARK_INTERDEPENDENCY_BY_NAME:
	     if(process_idx != 0)
           return TE_FAIL;   
		 return timer_qdi_mark_interdependence_by_name(client_handle, a1.ptr, a2.num, a3.ptr, a4.num);
