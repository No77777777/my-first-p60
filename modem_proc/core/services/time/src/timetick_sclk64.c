...
$Header: //components/rel/core.mpss/10.0/services/time/src/timetick_sclk64.c#1 $
...
)
{
  ...
  {
    /* Temporarily commenting DalTimetick api since its causing an exception.
       Instaed using Qurt timetick api. */
	   
    //DalTimetick_GetTimetick64(hTimetickSclk64Handle, &xo_delta);
	xo_delta = qurt_sysclock_get_hw_ticks();
  }
