...
#                                                                              #
# $Header: //components/rel/core.mpss/10.0/services/utils/tools/t32/memheap2_heapwalkparser.pl#1 $
# $Author: pwbldsvc $
...
if (($TotalFreeMem ne 0) and ($usedMemory<$ignoreFragmentationFactor) ) {
   ...
   print $OUTPUTFILEHANDLE "    Heap is exhausted so fragmentation factor is ignored heap memory used % is : ".sprintf("%3.2f%%",$usedMemory)."\n";
}
