...
$Header: //components/rel/core.mpss/10.0/services/utils/src/queue.c#1 $ $DateTime: 2019/04/24 00:03:26 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
11/08/16   nk      Fix NULL pointer check in q_delete_ext
...
)
{
   ...
   #else
        if((q_ptr->cnt <=0) || 
          (NULL == q_delete_ptr->next_ptr) || 
          (NULL == q_delete_ptr->prev_ptr))
        { 
	      return FALSE; 
        }
        q_delete_ptr->prev_ptr->next_ptr = q_delete_ptr->next_ptr;
