...
Copyright (c) 2006-2015,2017-2018 by Qualcomm Technologies, Inc.  All Rights Reserved.
============================================================================ */
...
$Header: //components/rel/core.mpss/10.0/services/utils/src/amssheap.c#1 $ $DateTime: 2019/04/24 00:03:26 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ---------------------------------------------------------
07/20/17   nk      Fixed crash while opening heap analyzer window
04/07/17   nk      FR36389: Provided an API to get AMSS heap statistics
...
static inline void *hexagon_return_address(int level) {
 ...
 #define  MEM_HEAP_CALLER_ADDRESS() ((void *)hexagon_return_address(AMSS_HEAP_CALLER_LEVEL));
#else
...
#define AMSS_MEM_HEAP_PHYSPOOL "AMSS_MEM_HEAP_POOL"
qurt_mem_pool_t amss_mem_heap_pool;
#endif
...
void amss_heap_init(void *start_heap, uint32 size)
{
  ...
  amss_mem_heap.heap_used_bytes   = 0;
  ...
  heap_ptr_table[magic_num_index] = &amss_mem_heap;
  magic_num_index++;
...
#ifdef FEATURE_MEM_DEBUG
  snprintf(amss_mem_heap.heapName,sizeof(amss_mem_heap.heapName),"amss_mem_heap");
#endif
}
static void amss_heap_initialized(void)
{
   unsigned int mem_region = 0;
   ...
   heap_start = (void *)heap_header_ptr;
   heap_end = (void *)((char *)heap_header_ptr+heap_header_ptr->s.size);
   ...
   ret = qurt_mem_pool_attach(AMSS_MEM_HEAP_PHYSPOOL, &amss_mem_heap_pool);
   ASSERT( ret == QURT_EOK );

   ret = qurt_mem_pool_attr_get (amss_mem_heap_pool, &pool_attr);
   ASSERT(ret == QURT_EOK);

   amssheapsize = pool_attr.ranges[0].size;

   heap_start = heap_create_region(amssheapsize,amss_mem_heap_pool, &mem_region);
   if(NULL == heap_start)
   {
      ASSERT(0);
   }
   heap_end = (void *)((char *)heap_start + amssheapsize);
    ...
    amss_mem_heap.mem_region = mem_region;
#else
...
FUNCTION amssheap_get_sections_info

DESCRIPTION
  returns amss_mem_heap heap's section's address info and size

ARGUMENTS
  @param[in]  - memheap_section_info type blank structure.
  @param[out] - passed structure will be filled with required info.
...
===========================================================================*/
DLL_API_GLOBAL void amssheap_get_sections_info(memheap_section_info *heap_info)
{
	memheap_get_section_info(&amss_mem_heap, heap_info);
}