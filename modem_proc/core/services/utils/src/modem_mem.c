...
/*===========================================================================
  ...
  Copyright (c) 2009-2017,2020 Qualcomm Technologies Incorporated. All Rights Reserved
...
$Header: //components/rel/core.mpss/10.0/services/utils/src/modem_mem.c#2 $

when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
04/06/17   nk      Provided an API to get modem heap statistics
01/29/16   ps      Added callback for clients to free memory when high threshold reaches.
...
static volatile unsigned int modem_mem_heap_size;

#ifdef FEATURE_MEM_DEBUG
static char modem_heap_name[]="modem_mem_heap";
#endif

#define MODEM_MEM_HEAP_PHYSPOOL "MODEM_MEM_HEAP_POOL"
qurt_mem_pool_t modem_mem_heap_pool;
...
#define  MEM_HEAP_CALLER_ADDRESS() ((void *)hexagon_return_address(MODEM_HEAP_CALLER_LEVEL));
#else
...
static DalDeviceHandle *modem_mem_time_handle = NULL;

/* Array of alloc costs */
modem_mem_profile_t modem_alloc_log[MODEM_ALLOC_LOG_MAX];
volatile uint32 modem_alloc_log_idx  = 0;
static atomic_word_t modem_atomic_alloc_idx = ATOMIC_INIT(0);
#endif

modem_mem_high_cb_type modem_mem_high_cb = NULL;
unsigned long modem_mem_high_threshold = 0;
...
/*===========================================================================
  ...
  FUNCTION:  modem_mem_time_init

===========================================================================*/
/*!
    @brief
    Initialises timetick module for profiling allocations 

    @return
	None
*/
/*=========================================================================*/
static void modem_mem_time_init
(
) 
{
  DALSYS_InitMod(NULL);
  DalTimetick_Attach("SystemTimer", &modem_mem_time_handle);
  ASSERT(modem_mem_time_handle);

  /* Enable system timer */
  DalTimetick_Enable(modem_mem_time_handle, 1);
}

/*===========================================================================

  FUNCTION:  modem_mem_time_trace

===========================================================================*/
/*!
    @brief
    Populates the passed field with current timetick value
...
static inline void modem_mem_time_trace(time_timetick_type * ticks_ptr) 
{
  DalTimetick_GetTimetick64(modem_mem_time_handle, ticks_ptr);
}
#endif
...
static void modem_mem_init_heap(void)
{
  ...
  */
  int j;
  int ret;
  qurt_mem_pool_attr_t pool_attr;
  ...
  */
  ...
  ret = qurt_mem_pool_attach(MODEM_MEM_HEAP_PHYSPOOL, &modem_mem_heap_pool);
  ASSERT( ret == QURT_EOK );
  
  ret = qurt_mem_pool_attr_get (modem_mem_heap_pool, &pool_attr);
  ASSERT(ret == QURT_EOK);

  modem_mem_heap_size = pool_attr.ranges[0].size;

  mem_init_heap_alloc(&modem_mem_heap, modem_mem_heap_size,&modemheap_malloc_failed, modem_mem_heap_pool);
  ...
  for (j = 0; sections[j].start != 0x0; ++j) 
  {
  ...
  modem_mem_time_init();
   ...
   memheap_set_heap_name(&modem_mem_heap,&modem_heap_name[0],strlen(modem_heap_name));
#endif  
  return;
} /* modem_mem_init_heap() */
...
#ifdef FEATURE_HEAP_ALLOC_PROFILE
  time_timetick_type alloc_start = 0, alloc_end = 0;
  modem_mem_time_trace(&alloc_start);
#endif
...
#endif
  if((NULL !=modem_mem_high_cb) && (modem_mem_high_threshold < modem_mem_heap.heap_used_bytes))
  {
     modem_mem_high_cb(modem_mem_heap.heap_used_bytes);
  }

#ifdef FEATURE_HEAP_ALLOC_PROFILE
  modem_mem_time_trace(&alloc_end);
  unsigned int local_idx;
  local_idx = atomic_inc_return(&modem_atomic_alloc_idx)%MODEM_ALLOC_LOG_MAX;
  /* Please be aware that we could be off by a couple of indexes here since
     write to the global is no longer in an ordered fashion */
  modem_alloc_log_idx = local_idx;
  time_timetick_type diff = alloc_end - alloc_start;
  modem_alloc_log[local_idx].caller_ptr = MEM_HEAP_CALLER_ADDRESS();
  modem_alloc_log[local_idx].task_id    = qurt_thread_get_id();
  modem_alloc_log[local_idx].time_delta = (uint32)    
      ((diff >> MSB_WORD_BITS) & MSB_WORD)? MSB_WORD: diff;
#endif
...
#ifdef FEATURE_HEAP_ALLOC_PROFILE
  time_timetick_type calloc_start = 0, calloc_end = 0;
  modem_mem_time_trace(&calloc_start);
#endif
...
#ifdef FEATURE_HEAP_ALLOC_PROFILE
  modem_mem_time_trace(&calloc_end);
  unsigned int local_idx;
  local_idx = atomic_inc_return(&modem_atomic_alloc_idx)%MODEM_ALLOC_LOG_MAX;
  modem_alloc_log_idx = local_idx;
  time_timetick_type diff = calloc_end - calloc_start;
  modem_alloc_log[local_idx].caller_ptr = MEM_HEAP_CALLER_ADDRESS();
  modem_alloc_log[local_idx].task_id    = qurt_thread_get_id();   
  modem_alloc_log[local_idx].time_delta = (uint32)
       ((diff >> MSB_WORD_BITS) & MSB_WORD)? MSB_WORD: diff; 
...
/*===========================================================================
  ...
  FUNCTION:  modem_mem_register_high_usage_callback

===========================================================================*/
/*!
    @brief
    Sets the callback to be called if the total used memory on the heap goes above preset value

    @detail
    The callback function should do minimal processing and should not allocate any additional memory.
    The callback implementation should ideally just set a signal on the relevant task and return.
...
*/
/*=========================================================================*/
void modem_mem_register_high_usage_callback(modem_mem_high_cb_type cb_fn,size_t heap_usage)
{
   modem_mem_high_cb = cb_fn;
   modem_mem_high_threshold =(unsigned long)heap_usage;
}

/*===========================================================================

  FUNCTION:  modem_mem_deregister_high_usage_callback

===========================================================================*/
/*!
    @brief
    De registers the callback to be called if the total used memory on the heap goes above preset value
...
*/
/*=========================================================================*/
void modem_mem_deregister_high_usage_callback(void)
{
   modem_mem_high_cb = NULL;
}

#ifdef FEATURE_MEM_DEBUG
/*===========================================================================
 Function    : modemheap_get_task_stats
 Description :
     Returns the modem_mem_heap statistics for a thread 
	 with given memheap TLS index
===========================================================================*/
int modemheap_get_task_stats(uint32 thread_id, memheap_task_stats_type **stats)
{
  return memheap_get_task_stats(&modem_mem_heap, thread_id, stats);
}
#endif
