...
/*===========================================================================
  ...
  Copyright (c) 2009-2014,2016-2018 QUALCOMM Technologies Incorporated. All Rights Reserved
...
$Header: //components/rel/core.mpss/10.0/services/utils/src/dynamic_heap.c#1 $

when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
09/13/16   nk      Made Dynamic heap interface generic
...
unsigned int dynamic_heap_g_critical_section_init(void)
{
   if (0 ==  dyn_heap_critical_section_init)
   {
...
unsigned int dynamic_heap_g_init(dynamic_mem_client_id client_id, dynamic_mem_attribute attr, size_t size, mem_heap_type *heap_ptr,void **alloc_ptr)
{
   ...
   if(dynamic_heap_g_critical_section_init() != DYNAMIC_MEM_SUCCESS)
   {
      return DYNAMIC_MEM_FAIL;
   }

   /* Get mem share QMI client connection status */
   if(MEM_SHARE_SUCCESS != mem_share_get_qmi_poll_status())
   {
...
unsigned int dynamic_heap_g_querysize( dynamic_mem_client_id client_id, uint32_t* size)
{
   ...
   if(MEM_SHARE_SUCCESS != mem_share_get_qmi_poll_status())
   {
      MSG_ERROR("dynamic_heap_g_querysize: mem_share_client_init returned failure\n",0,0,0);
      return DYNAMIC_MEM_FAIL;
   }
   ...
   switch(client_id)
   {
      ...
      case DYNAMIC_MEM_CLIENT_ID_DIAG:
        if ( memheap_region[DYNAMIC_MEM_CLIENT_ID_FTM].xpu_in_use == 0){
           *xpu_partition = MBA_XPU_PARTITION_RECLAIM_MBA;
        }else{
...
static int release_heap_memory(dynamic_mem_client_id client_id,boolean doXpuUnlock)
{
   ...
   ret = qurt_mem_pool_remove_pages(memheap_region[client_id].mem_pool,
                              memheap_region[client_id].paddr >> 12,
                              memheap_region[client_id].num_bytes >> 12,
                              QURT_POOL_REMOVE_ALL_OR_NONE, NULL, NULL);
   if(QURT_EOK != ret)
   {
      MSG_ERROR("qurt_mem_pool_remove_pages returned error %d!!!\n",ret,0,0);
      return DYNAMIC_MEM_DEINIT_FAIL_KERNEL_ERROR;
   }
   if(doXpuUnlock)
   {
...
static int create_mem_region(dynamic_mem_client_id client_id, uint32 physAddr, uint32 numBytes, void **ptrMemBlock)
{
   /*if we have the memory pool with name DYNAMIC_MEM_PHYS_POOL already created 
   then attach to the existing pool else if attach fail then creating the pool for the first time*/
   if (QURT_EOK != qurt_mem_pool_attach(DYNAMIC_MEM_PHYS_POOL, &memheap_region[client_id].mem_pool))
   {
      ...
      else
      {
       ...
       /*install new page to the existing memory pool*/ 
      if (QURT_EOK != qurt_mem_pool_add_pages(memheap_region[client_id].mem_pool, (physAddr >> 12), (numBytes>>12)))
      {
        MSG_ERROR("qurt_mem_pool_add_pages failed pool = %d , physAddr= 0x%x, size = 0x%x", memheap_region[client_id].mem_pool, physAddr, numBytes);
      }
      else{
        MSG_HIGH("qurt_mem_pool_add_pages success pool = %d , physAddr= 0x%x, size = 0x%x", memheap_region[client_id].mem_pool, physAddr, numBytes);
      }
   }
   
   qurt_mem_region_attr_init (&memheap_region[client_id].mem_attribs);
   qurt_mem_region_attr_set_cache_mode(&memheap_region[client_id].mem_attribs,QURT_MEM_CACHE_WRITEBACK);
