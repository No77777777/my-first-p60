...
Copyright (c) 1997-2018 by QUALCOMM Technologies Incorporated.  All Rights Reserved.
============================================================================ */
...
$Header: //components/rel/core.mpss/10.0/services/utils/src/memheap.c#1 $ $DateTime: 2019/04/24 00:03:26 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ---------------------------------------------------------
03/30/17   nk      Added Support to log multiple callers
06/09/16   nk      Added check to avoid 16Byte Block creation in MBA image
07/21/16   nk      Added mechanism to retry for non-zero magic number from prng api 
...
07/24/15   ps      Add support for heap analyzer view in QXDM
...
#ifndef FEATURE_MBA_MEMHEAP_IMAGE
static void delete_heap_region(mem_heap_type *heap_ptr);
#endif
...
static inline void *hexagon_return_address(int level) {
  int return_addr;
 ...
 #define  MEM_HEAP_CALLER_ADDRESS(level) ((void *)hexagon_return_address(level));
#else
...
#ifdef FEATURE_MEM_DEBUG
  snprintf(heap_ptr->heapName,sizeof(heap_ptr->heapName),"heap_%d",heap_ptr->magic_num_index);
#endif
...
)
{
 ...
 {
   MEMHEAP_ASSERT(OSAL_SUCCESS == osal_delete_crit_sect((osal_crit_sect_t*)&(heap_ptr->memheap_crit_sect)));
   memset(&(heap_ptr->memheap_crit_sect),0,sizeof(osal_crit_sect_t));
#ifndef FEATURE_MBA_MEMHEAP_IMAGE
   if(heap_ptr->mem_region){
     delete_heap_region(heap_ptr);
   }
#endif
   memset(heap_ptr, 0, sizeof(mem_heap_type));
 }
...
)
{
  ...
  uint32 blockHeaderSize=sizeof(mem_block_header_type);
    ...
    mem_block_header_type *tempHeader=NULL;
#endif
#endif
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
...
#ifdef MEMHEAP_LOG_MULTIPLE_CALLERS 
	tempHeader = (mem_block_header_type *)((char *) answer - sizeof(mem_block_header_type));
	tempHeader = (mem_block_header_type *)((char *)tempHeader+FPOS(mem_block_header_type,multiple_caller_ptr));
	memset(tempHeader,0,(sizeof(mem_block_header_type)-FPOS(mem_block_header_type,multiple_caller_ptr))); /*Memset the caller pointers to 0 so that we dont display previously logged values for present block */
	tempHeader = (mem_block_header_type *)((char *)tempHeader-FPOS(mem_block_header_type,multiple_caller_ptr));
    if((tempHeader->header_debug_info.caller_ptr!=NULL) && (MEMHEAP_MAX_CALLER_LEVEL_WITH_LIMIT_5>=MEMHEAP_CALLER_2))
	{
        tempHeader->multiple_caller_ptr[0]=__builtin_return_address(MEMHEAP_CALLER_2);
		if((tempHeader->multiple_caller_ptr[0]!=NULL) && (MEMHEAP_MAX_CALLER_LEVEL_WITH_LIMIT_5>=MEMHEAP_CALLER_3))
		{
		    tempHeader->multiple_caller_ptr[1]=__builtin_return_address(MEMHEAP_CALLER_3);
			if((tempHeader->multiple_caller_ptr[1]!=NULL) && (MEMHEAP_MAX_CALLER_LEVEL_WITH_LIMIT_5>=MEMHEAP_CALLER_4))
			{
			   tempHeader->multiple_caller_ptr[2]=__builtin_return_address(MEMHEAP_CALLER_4);
			   if((tempHeader->multiple_caller_ptr[2]!=NULL) && (MEMHEAP_MAX_CALLER_LEVEL_WITH_LIMIT_5>=MEMHEAP_CALLER_5))
			   {
			      tempHeader->multiple_caller_ptr[3]=__builtin_return_address(MEMHEAP_CALLER_5);
			   } 
			} 
		} 	
	}	
...
)
{
   ...
   heap_ptr->heap_used_bytes += newBlock->forw_offset;
   #if defined FEATURE_MEM_DEBUG_ARRAY && defined FEATURE_QDSP6
...
===========================================================================*/
int mem_heap_set_int_lock( mem_heap_type *heap_ptr )
{
#ifdef FEATURE_QUBE
  NOTUSED(heap_ptr);
  return 0;
...
DESCRIPTION
  ...
  Returns 1 on success and 0 on failure.
===========================================================================*/
int mem_heap_set_task_lock( mem_heap_type *heap_ptr )
{
#ifdef FEATURE_QUBE
  NOTUSED(heap_ptr);
  return 0;
...
DESCRIPTION
  ...
  Returns 1 on success and 0 on failure.
===========================================================================*/
int mem_heap_set_crit_sect( mem_heap_type *heap_ptr )
{
#ifdef FEATURE_QUBE
  NOTUSED(heap_ptr);
  return 0;
...
DESCRIPTION
  ...
  Returns 1 on success and 0 on failure.
===========================================================================*/
int mem_heap_set_no_lock( mem_heap_type *heap_ptr )
{
...
===========================================================================*/
void mem_heap_get_freeblock_info(
  mem_heap_type *heap_ptr,
...
===========================================================================*/
unsigned int mem_heap_get_chunk_size( mem_heap_type *heap_ptr )
{
#ifdef FEATURE_QUBE  
  NOTUSED(heap_ptr);
#endif
...
static void delete_heap_region(mem_heap_type *heap_ptr)
{
   ...
   int ret;
   heap_region = (qurt_mem_region_t) heap_ptr->mem_region;

   qurt_mem_region_attr_get(heap_region, &attr);

   /* invalidate the cache */
   ret = qurt_mem_cache_clean((qurt_addr_t)heap_ptr->sections[0].start_addr, 
                             (qurt_size_t)attr.size,
...
/*===========================================================================
FUNCTION HEAP_CREATE_REGION

DESCRIPTION
  Create a heap region of requested size 
  returns the heap reqgion address if successful or  NULL if failure.
===========================================================================*/
...
void* heap_create_region(uint32 size, qurt_mem_pool_t mem_pool, qurt_mem_region_t *mem_region)
{
   ...
   ret = qurt_mem_region_create(&heap_region, size, mem_pool, &heap_region_attr);
...
)
{
   ...
   if((heap_ptr->magic_num) && (heap_ptr->magic_num == magic_num[heap_ptr->magic_num_index]))
   {
      /* heap is already initialized so just return */
      return;
   }

   heap_mem_ptr = heap_create_region(heap_mem_size, mem_pool, &mem_region);
   ...
   heap_ptr->mem_region = (unsigned int) mem_region;
   return;
...
===========================================================================*/
unsigned long mem_heap_get_current_heap_usages(mem_heap_type *heap_ptr)
{
   return heap_ptr->used_bytes;
}
...
ARGUMENTS
  None
===========================================================================*/
DLL_API_GLOBAL unsigned long mem_heap_get_current_in_use_blocks_count(mem_heap_type *heap_ptr)
{
  int i;
  ...
  return (heap_ptr->total_blocks - totalFreeBlocks);
}

/*===========================================================================

FUNCTION memheap_set_heap_name

DESCRIPTION
  sets the heap name for debugging purposes
...
===========================================================================*/
#ifdef FEATURE_MEM_DEBUG
int memheap_set_heap_name(mem_heap_type *heap_ptr, char *name, uint16 length)
{
   if((NULL == name) || (NULL == heap_ptr)) 
     return MEMHEAP_EARGS;

   memscpy(heap_ptr->heapName, sizeof(heap_ptr->heapName), name, length);
   return MEMHEAP_EOK;
}
#endif
/*===========================================================================
...
===========================================================================*/
DLL_API_GLOBAL int findLargestBlockSize(mem_heap_type *heap_ptr )
{
   binStruct *binToSearch;
   int i = MEMHEAP2_BIN_COUNT - 1;

   if(NULL == heap_ptr) return -1;
...
FUNCTION memheap_get_section_info

DESCRIPTION
  returns passed heap's section's address info and size
...
===========================================================================*/
void memheap_get_section_info(mem_heap_type *heap, memheap_section_info *heap_info)
{
	int i = 0;
	...
	BEGIN_CRITICAL_SECTION(heap);
	
	for(i = 0; i < MEM_HEAP_SECTIONS_MAX; i++)
     {
	    if(heap->sections[i].start_addr == NULL)
          break;
     
        heap_info->sections[i].start_addr = heap->sections[i].start_addr;
	    heap_info->sections[i].size       = heap->sections[i].size;
	 }

	END_CRITICAL_SECTION(heap);
}
#ifdef FEATURE_MEM_DEBUG
