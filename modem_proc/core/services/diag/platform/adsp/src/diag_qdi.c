...
Copyright (c) 2012-2018, 2020 Qualcomm Technologies Incorporated.  All Rights Reserved.
...
/*===========================================================================
 ...
 $Header: //components/rel/core.mpss/10.0/services/diag/platform/adsp/src/diag_qdi.c#2 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
06/19/18   ph      Configure buffering modes for user PD send buffers.
09/21/18   gn      Initialize diag master table queue and mask client list queue before diag registers with QDI
04/23/18   gn      Command registration enhancement
10/31/17   ph      Invoke signal to drain userPD data only in streaming mode.
09/18/17   sa      Make QDI call to drain userpd events.
06/19/17   ph      Tagging the data with dynamic diag IDs.
02/21/17   ph      Tag DIAG ID header to all the diag packets sent out.
05/01/17   sp      Initialize diag_main_q for events before Diag registration with QDI
03/06/17   ph      diagbuffer API changes and minor updates.
03/08/17   ph      Handle flow control case in event_drain.
02/07/17   tbg     Adjustment to internal diagID API call
12/13/16   gn      Changes to support command deregistration
11/08/16   tbg     Added qshrink4 GUID registration function for UserPDs
11/05/16   sa      Added support for DiagID registration
11/03/16   sa      Implemented seperate queue for UserPD events and enabled buffering
09/29/16   ph      Changing to diag drain_buf_type structure.
...
10/26/15   sr      Fixed the race condition while sending PD status control message
...
#include "diagdiag_v.h"/*for diag_ctrl_send_pd_status*/
...
extern void diagpkt_table_q_init(void);
...
extern uint8 diag_qshrink_guid_count;
extern boolean diag_qshrink4_register_entry(diag_qshrink_4_database_file_type *);
                           ...
                           int buflen )
{
    ...
    diag_userpd_struct_ptr = EVENT_Q_SEARCH(diag_pid_store);
      ...
      if ( pDiagEventData )
      {
        ...
        if ( memcpy_res == 0 )
        {
          ...
          pDiagEventData->version = DIAGBUF_VERSION_0;
          pDiagEventData->id = 0;
  ...
  case DIAG_DATA_TYPE_QSHRINK_4_DATABASE_INFO:
    {
      diag_qshrink_4_database_file_type db_entry;

      memset((char*)&db_entry,0,sizeof(diag_qshrink_4_database_file_type));

      /* Create local copy of proposed new DB list entry */
      memcpy_res = qurt_qdi_copy_from_user(client_handle,
        &(db_entry.guid),
           (((diag_data_qshrink_4_database_info *)buf)->guid),
        QSHRINK_4_DATABASE_GUID_LENGTH);
      if ( memcpy_res < 0 )
      {
        return 0;
      }
      ...
      memcpy_res = qurt_qdi_copy_from_user(client_handle,
        &(db_entry.file_name),
           (((diag_data_qshrink_4_database_info *)buf)->filename),
        MIN(strlen(((diag_data_qshrink_4_database_info *)buf)->filename),MAX_MSG_V4_DBASE_NAME_SIZE));
      if ( memcpy_res < 0 )
      {
        return 0;
      }
      ...
      memcpy_res = qurt_qdi_copy_from_user(client_handle,
        &(db_entry.pid),
           &(((diag_data_qshrink_4_database_info *)buf)->pid),
           sizeof(int));
      if ( memcpy_res < 0 )
      {
        return 0;
      ...
      db_entry.fd = -1;
      db_entry.file_len = 0;
      db_entry.updated = FALSE;

      if(diag_qshrink4_register_entry(&db_entry) == TRUE)
    diag_set_internal_sigs(DIAG_INT_UPDATE_PD_STATUS);
    }
    break;
                           ...
                           dword *out_buf_actual_bytes_ptr )
{
  ...
  case DIAG_IOCTL_GET_DELAYED_RSP_ID:
    ...
    case DIAG_IOCTL_SET_TX_MODE_PD:
    {
      diag_LSM_tx_mode_info mode_info;
      diag_LSM_tx_mode_info* pMode_info = &mode_info;
      copy_ret = qurt_qdi_copy_from_user(client_handle, pMode_info, in_buf_ptr, sizeof(diag_LSM_tx_mode_info));
      if( copy_ret < 0 )
      {
        fn_ret_val = FALSE;
        return fn_ret_val;
      }                                      
      fn_ret_val = diag_set_tx_mode_dcm(client_handle, mode_info, out_buf_ptr, (unsigned long)out_buf_max_bytes,(unsigned long*) &tot_out_buf_bytes);
    }
    break;
    case DIAG_IOCTL_GET_TX_MODE_PD:
    {
      diag_LSM_tx_mode_info mode_info;
      diag_LSM_tx_mode_info* pMode_info = &mode_info;
      copy_ret = qurt_qdi_copy_from_user(client_handle, pMode_info, in_buf_ptr, sizeof(*pMode_info));
      if( copy_ret < 0 )
      {
        fn_ret_val = FALSE;
        return fn_ret_val;
      }
      fn_ret_val = diag_get_tx_mode_dcm(client_handle, mode_info, out_buf_ptr, (unsigned long)out_buf_max_bytes,(unsigned long*) &tot_out_buf_bytes);
    }
    break;
  default:
    fn_ret_val = FALSE;
...
void diag_qdi_release( qurt_qdi_obj_t *qdiobj )
{
  ...
  ULogFront_RealTimePrintf(diag_err_log_handle,1,"diag_qdi_release : Calling diagbuf_mpd_cleanup with qdi_opner address = %p",qdi_opener); 
...
{
  diag_drain_buf_type *rootpd_buf_ptr = NULL;
  uint8 *diagbuf = NULL;
  diagbuf_mpd_type* userbuf_loc_ptr = NULL;

  ULogFront_RealTimePrintf(diag_err_log_handle, 0, "Entered in diag_qdi_get_diagbuf funtion");
  ...
  rootpd_buf_ptr = malloc(sizeof(diag_drain_buf_type));
  if(rootpd_buf_ptr == NULL)
  {
    return (-1);
  }

  memset(rootpd_buf_ptr, 0, sizeof(diag_drain_buf_type));
  rootpd_buf_ptr->buffer_type = DIAG_DIAGBUFFER_UPD_TYPE;

  userbuf_loc_ptr = diagbuf_mpd_init(client_handle, pid);

  /* Check the pointers to make sure data was allocated */
  if ( userbuf_loc_ptr == NULL )
  {
    free(rootpd_buf_ptr);
    diagbuf_mpd_deinit(client_handle, NULL);
    return (-1);
  }

  /* Copy the root PD pointer to the user PD structure */
  if ( qurt_qdi_copy_to_user(client_handle, userpd_buf_ptr, &userbuf_loc_ptr, sizeof(userbuf_loc_ptr)) < 0 )
  {
    free(rootpd_buf_ptr);
    diagbuf_mpd_deinit(client_handle, userbuf_loc_ptr);
    return (-1);
  }
  ...
  */
  ...
  rootpd_buf_ptr->buf_ptr = (void *)userbuf_loc_ptr;

  /*
    After successful create of the user PD diagbuf, add the new diagbuf to the
    root PDs list of buffers
  */
  ULogFront_RealTimePrintf(diag_err_log_handle,1,"diag_qdi_get_diagbuf : Adding user pd diagbuf to draining queue, address= %p",rootpd_buf_ptr); 
  diagbuf_add_to_drain_queue(rootpd_buf_ptr);

  /*
    In addition to adding it to the PD link list, add the node to
    the qdi obj for easy (only way) reference during PD clean up.
  */
  diag_qdi_obj_ptr->user_pd_node_ptr = userbuf_loc_ptr;
  ULogFront_RealTimePrintf(diag_err_log_handle,1,"diag_qdi_get_diagbuf : Updated new diag buf to qdi object i.e user_pd_node_ptr= %p",diag_qdi_obj_ptr->user_pd_node_ptr); 
...
/*===========================================================================
FUNCTION diag_qdi_clear_drain_timer

DESCRIPTION
  clear the drain timer on diagtask.
...
===========================================================================*/
int diag_qdi_clear_drain_timer( void )
{
  //Call the function to clear drain timer on Diag Task
  diag_clr_drain_timer();
  return (0);
}
/*===========================================================================
FUNCTION diag_qdi_set_event_stale

DESCRIPTION
  Call event stale to drain the events in the buffer.
...
===========================================================================*/
int diag_qdi_set_event_stale( int client_handle,diag_qdi_opener *diag_qdi_obj_ptr)
{
  
  diag_mpd_ptr_struct *diag_userpd_struct_ptr;
  int diag_pid=0;
  
  if((diag_qdi_obj_ptr!=NULL) && (diag_qdi_obj_ptr->user_pd_node_ptr!=NULL))
  {
    diag_pid= diag_qdi_obj_ptr->user_pd_node_ptr->pid;
    diag_userpd_struct_ptr=EVENT_Q_SEARCH(diag_pid);
    if(diag_userpd_struct_ptr)
    {
      event_stale(diag_userpd_struct_ptr->event_mpd_struct_ptr);
    }
	else
	{ 
	  event_stale(NULL);
...
===========================================================================*/
int diag_qdi_clear_event_stale( void)
{  
                            ...
                            qurt_qdi_arg_t a9 )
{
  ...
  switch ( method )
  {
    ...
    {
      int rv = -1;
	  ULogFront_RealTimePrintf(diag_err_log_handle,0,"Diag_qdi_invoke : Calling diag_qdi_get_diagbuf()");
  ...
  case DIAG_QDI_INIT_EVENT_QUEUE:
  {
    ...
    case DIAG_QDI_CLEAR_DRAIN_TIMER:
    {
      return (diag_qdi_clear_drain_timer());
    }
  case DIAG_QDI_REQUEST_DIAGID:
    ...
    return_val =osal_set_sigs(diag_task_tcb, DIAG_REQUEST_DIAGID_SIG, &return_sigs);
    ASSERT(OSAL_SUCCESS == return_val);
      return (rv);
    }
    case DIAG_QDI_SET_EVENT_STALE:
    {
      return (diag_qdi_set_event_stale(client_handle,me));
    }
     case DIAG_QDI_CLEAR_EVENT_STALE:
    {
      return (diag_qdi_clear_event_stale());
    }
    default:
    {
...
void diag_ctrl_send_pd_status(int pid, uint8 status)
{
  ...
  /*send a ctrl pkt to apps to notify it that a user-pd is up*/
  /* Compose DIAG_CTRL_MSG_FEATURE packet */
  ctrl_msg.cmd_type = DIAG_CTRL_PD_STATUS;
  ctrl_msg.data_len = sizeof(diag_ctrl_pd_status_type);
  ctrl_msg.data.ctrl_pd_status.version = DIAG_CTRL_PD_STATUS_VER;
  ctrl_msg.data.ctrl_pd_status.pid = pid;
  ctrl_msg.data.ctrl_pd_status.status = status;


    diagcomm_ctrl_send_mult_pkt(&ctrl_msg, DIAGCOMM_CTRL_MSG_HDR_SIZE + sizeof(diag_ctrl_pd_status_type),
