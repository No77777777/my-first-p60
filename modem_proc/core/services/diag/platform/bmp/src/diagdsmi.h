...
Copyright (c) 2009-2014, 2016 by QUALCOMM Technologies, Incorporated.  All Rights Reserved.
===========================================================================*/

/*===========================================================================
                            EDIT HISTORY FOR FILE

  $Header: //components/rel/core.mpss/10.0/services/diag/platform/bmp/src/diagdsmi.h#1 $

when        who    what, where, why
--------    ---    ----------------------------------------------------------
06/29/16    ph     Adjust the many mark for DSM_DIAG_SIO_TX_ITEM_POOL.
04/11/16    as     Reduced the SIO_TX count for MP builds only
...
#else
#define DSMI_DIAG_SIO_TX_ITEM_CNT                 1 /* Making the count one on slave processor, since it is using sockets and flat buffer */
#endif /* defined DIAG_MP */
#endif

#define DSMI_DIAG_SIO_TX_ITEM_DONT_EXCEED                  5
#define DSMI_DIAG_SIO_TX_ITEM_FEW_MARK                     ((DSMI_DIAG_SIO_TX_ITEM_CNT) - 10)

/*Since the send buffer can have a maximum of 16KB data to be copied to DSM, Ensure
  the many mark accommodates as many DSM items required for maximum size to take care of
  maximum length of chained DSMs  */
#define DSMI_DIAG_SIO_TX_ITEM_MANY_MARK                    ((DSMI_DIAG_SIO_TX_ITEM_CNT) - 5)
