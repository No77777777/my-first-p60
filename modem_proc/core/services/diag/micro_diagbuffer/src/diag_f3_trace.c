...
Copyright (c) 2004-2019 by QUALCOMM Technologies, Incorporated.  All Rights Reserved.
*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*/

/*==========================================================================

                        EDIT HISTORY FOR MODULEW
$Header: //components/rel/core.mpss/10.0/services/diag/micro_diagbuffer/src/diag_f3_trace.c#2 $

when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
08/22/18   kdey    Fixed frame mismatch issue
03/14/17   rs      Fixed the swapped arguments savevars and savetime in err_f3_save_msg()
...
===========================================================================*/
static void diag_f3_trace_set_up_msg_header(int nargs, char savevars, char savetime, char byte_sized, uint32 type, diag_f3_trace_hdr_type* hdr, uint8 version, uint8 qtimer)
{
  ...
  /*lint -restore */
  hdr->config |= (byte_sized | savevars | savetime | version | qtimer_flag);
...
SIDE EFFECTS
  None
===========================================================================*/
void err_f3_save_msg(diag_f3_trace_handle *h, const void * constant_data_ptr, msg_arg_type * args, uint8 nargs, void* msg, uint32 type, uint8 version, void* param, uint8 arg_bytes)
{
   /* 
   ...
   */
   ...
   else
   {
      ...
      {
         if ((qsr_hash_v4 >= (uint32)qsr_4_0_msg_aligned_start_address) && (qsr_hash_v4 < (uint32)qsr_4_0_msg_unaligned_start_address))
         {
         ...
         else
         {
            ...
            ULogFront_RealTimePrintf(diag_debug_ulog_handle,1,"err_f3_save_msg: Invalid hash value : qsr_hash_v4 : 0x%x",qsr_hash_v4); 
            return;
         }
   ...
   /* Proceed with saving msg to buffer */
   {
      ...
      diag_f3_trace_set_up_msg_header(nargs, savevars, savetime, (char)byte_sized, type, &record_hdr, version, qtimer_flag);
      ...
      switch(version)
      {
         case DIAGBUF_VERSION_1:
            ...
            /* Save record to the main F3 trace buffer*/
            diag_f3_trace_to_buffer(h, write_offset, (uint8*) &ext_params, sizeof(diag_f3_trace_ext_type));
            /*Increment the offset by size of the header */
          ...
          case DIAG_F3_TRACE_MSG_TIME:
          {
            ...
            if (msg)
            {
              ...
              else
              {
                ...
                {
                    qw_equ(tstamp, ((msg_v4_multi_client_store_type*)msg)->msg.hdr.ts);
                }
              }
            ...
            else
            {
              ...
              msg_get_time_ext(&tstamp,DIAGBUF_VERSION_0,NULL);
            }
