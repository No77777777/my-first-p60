...
$Header: //components/rel/core.mpss/10.0/services/diag/micro_diagbuffer/src/micro_diagbuffer_msg_api.c#1 $

when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
06/02/17   sa      Updated to VU_CORE_SERVICES_DIAG_DEFS_API.02.00.103
...
static uint32 msg_micro_rt_masks_dpm[1 + (MSG_SSID_DPM_LAST - MSG_SSID_DPM )];
static uint32 msg_micro_rt_masks_v2x[1 + (MSG_SSID_V2X_LAST - MSG_SSID_V2X )];
...
)
{
  ...
  err_f3_save_msg(&uimage_f3_trace, constant_data_ptr, args, nargs, msg, type, version, parameter,0);
...
===========================================================================*/
void _micro_msg_send_ext(const msg_const_type *const_blk, uint32 version, void *parameter)
{
  ...
  if (msg_enabled_mask != 0 && msg_service_init == TRUE)
  {
    ...
    {
      _micro_diagbuffer_debug_msg(uimage_diagbuffer, const_blk, 0, version, parameter);
      if (micro_alloc_count_msg == MAX_VALUE_UINT32)
      {
        MICRO_MSG_EXT(MSG_SSID_DIAG, MSG_LEGACY_LOW, 0, NULL, "Variable micro_alloc_count_msg has reached maximum value.");
      }
      else
      {
...
===========================================================================*/
void
_micro_msg_send_1_ext (const msg_const_type *const_blk, uint32 version, 
                        void *parameter, uint32 xx_arg1)
{
  ...
  if(msg_enabled_mask != 0 && msg_service_init == TRUE)
  {
    ...
    if (micro_msg_mask_enabled(&const_blk->desc))
    {
      _micro_diagbuffer_debug_msg(uimage_diagbuffer, const_blk, 1, version, parameter, xx_arg1);
      if ( micro_alloc_count_msg == MAX_VALUE_UINT32 )
      {
        MICRO_MSG_EXT(MSG_SSID_DIAG, MSG_LEGACY_LOW, 0, NULL, "Variable micro_alloc_count_msg has reached maximum value.");
      }
...
===========================================================================*/
void
_micro_msg_send_2_ext(const msg_const_type *const_blk, uint32 version, void *parameter,
                       uint32 xx_arg1, uint32 xx_arg2)
{
  ...
  if(msg_enabled_mask != 0 && msg_service_init == TRUE)
  {
    ...
    if (micro_msg_mask_enabled(&const_blk->desc))
    {
      _micro_diagbuffer_debug_msg(uimage_diagbuffer, const_blk, 2, version, parameter, xx_arg1, xx_arg2);
      if ( micro_alloc_count_msg == MAX_VALUE_UINT32 )
      {
        MICRO_MSG_EXT(MSG_SSID_DIAG, MSG_LEGACY_LOW, 0, NULL, "Variable micro_alloc_count_msg has reached maximum value.");
      }
...
===========================================================================*/
void
_micro_msg_send_3_ext (const msg_const_type *const_blk, uint32 version, void *parameter,
                       uint32 xx_arg1, uint32 xx_arg2, uint32 xx_arg3)
{
  ...
  if(msg_enabled_mask != 0 && msg_service_init == TRUE)
  {
    ...
    if (micro_msg_mask_enabled(&const_blk->desc))
    {
      _micro_diagbuffer_debug_msg(uimage_diagbuffer, const_blk, 3, version, parameter, xx_arg1, xx_arg2, xx_arg3);
      if ( micro_alloc_count_msg == MAX_VALUE_UINT32 )
      {
        MICRO_MSG_EXT(MSG_SSID_DIAG, MSG_LEGACY_LOW, 0, NULL, "Variable micro_alloc_count_msg has reached maximum value.");
      }
...
===========================================================================*/
void
_micro_msg_send_var_ext (const msg_const_type *const_blk, uint32 version, void *parameter, uint32 num_args, ...)
{
  int i = 0;
  ...
  if (micro_msg_mask_enabled(&const_blk->desc))
  {
    ...
    {
    case 4:
      _micro_diagbuffer_debug_msg(uimage_diagbuffer, const_blk, 4, version, parameter, args[0], args[1], args[2], args[3]);
      break;
    case 5:
      _micro_diagbuffer_debug_msg(uimage_diagbuffer, const_blk, 5, version, parameter, args[0], args[1], args[2], args[3], args[4]);
      break;
    case 6:
      _micro_diagbuffer_debug_msg(uimage_diagbuffer, const_blk, 6, version, parameter, args[0], args[1], args[2], args[3], args[4], args[5]);
      break;
    case 7:
      _micro_diagbuffer_debug_msg(uimage_diagbuffer, const_blk, 7, version, parameter, args[0], args[1], args[2], args[3], args[4], args[5], args[6]);
      break;
    case 8:
      _micro_diagbuffer_debug_msg(uimage_diagbuffer, const_blk, 8, version, parameter, args[0], args[1], args[2], args[3], args[4], args[5], args[6], args[7]);
      break;
    case 9:
      _micro_diagbuffer_debug_msg(uimage_diagbuffer, const_blk, 9, version, parameter, args[0], args[1], args[2], args[3], args[4], args[5], args[6], args[7], args[8]);
      break;
    }
