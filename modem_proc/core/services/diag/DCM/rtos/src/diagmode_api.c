...
$Header: //components/rel/core.mpss/10.0/services/diag/DCM/rtos/src/diagmode_api.c#1 $
...
uint8 diag_set_tx_mode(uint8 mode)
{
    ...
    set_mode = diag_set_tx_mode_stream(mode, DIAG_STREAM_1, diag_get_diagID());
...
uint8 diag_get_tx_mode()
{
    ...
    mode = diag_get_tx_mode_stream( DIAG_STREAM_1, diag_get_diagID() );
...
void diag_drain_tx_buffer()
{
  ...
  diag_drain_tx_buffer_stream(DIAG_STREAM_1, diag_get_diagID());
}
