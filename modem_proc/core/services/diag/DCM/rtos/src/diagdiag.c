...
$Header: //components/rel/core.mpss/10.0/services/diag/DCM/rtos/src/diagdiag.c#2 $
...
DESCRIPTION
  ...
  Afterward, we will OR in bits from a callback registered by the modem.
...
)
{
  ...
  byte * modem_features = NULL;
  int modem_feature_len = 0;
  int i;
  ...
  if (rsp)
  {
    ...
    if(modem_feature_mask_cb != NULL)
        modem_features = modem_feature_mask_cb(&modem_feature_len);
	...
	if(modem_features != NULL)
		for(i = 0; i < FEATURE_MASK_LENGTH && i < modem_feature_len; i += 1)
		{
			rsp->feature_mask[i] |= modem_features[i];
		}
  }
...
/*===========================================================================
FUNCTION DIAGDIAG_EXT_LISTENER_CMD_VERIFY

DESCRIPTION
  This function is responsible for validating a diag command used to modify 
  extended listeners.

PARAMETERS
  req_pkt - pointer to the start of the cmd request packet
  pkt_len - packet length of the cmd req packet

RETURN VALUE
  TRUE or FALSE indicating if the diag ext listener
  command is valid or not
===========================================================================*/
boolean diagdiag_ext_listener_cmd_verify(void * req_pkt, uint16 pkt_len)
{
  diag_log_event_ext_listener_req_type *req = NULL;
  uint32 mask_arr_size = 0;
  ...
  req = (diag_log_event_ext_listener_req_type *) req_pkt;

  mask_arr_size = req->num_codes * sizeof(req->codes[0]);

  /* Check the size of the listener command sent against expected size */
  if((pkt_len-FPOS(diag_log_event_ext_listener_req_type, codes)) != mask_arr_size)
  {
    return FALSE;
  }
...
DESCRIPTION The event has been added to listen The API diag_listener_get_subid is called
            from the listener. If a negative value is returned by the API indicating an
            error code then only event id, param and seq num are printed, if a positive
            value is returned then the sub ID is also printed along with event id, param
            and seq num.
============================================================================*/
void
diagdiag_event_listener (uint32 seq_num, const diag_event_type * event, void * param)
{
  int subid = diag_listener_get_subid(DIAG_EXT_EVENT, event);
  if(subid < 0)
  {
  ...
  else
  {
	MSG_4 (MSG_SSID_DIAG, MSG_LEGACY_HIGH,
           "Event listener 0x%04X param:0x%08X seq:%d subID:%d", 
            event->event_id, (uint32) param, seq_num, subid);
  }
}
...
DESCRIPTION The event has been added to listen The API diag_listener_get_subid is called
            from the listener. If a negative value is returned by the API indicating an
            error code then only event id and param are printed, if a positive value is
            returned then the sub ID is also printed along with event id and param.
============================================================================*/
void
diagdiag_event_listener_ext (const diag_event_type * event, void * param)
{
  ...
  {
	MSG_3 (MSG_SSID_DIAG, MSG_LEGACY_HIGH,
          "Extended Event listener 0x%04X param:0x%08X subID:%d",
           event->event_id, (uint32) param, subid);
  }
}
...
DESCRIPTION The log has been added to listen The API diag_listener_get_subid is called
            from the listener. If a negative value is returned by the API indicating an
            error code then only log code, param and seq num are printed, if a positive
            value is returned then the sub ID is also printed along with log code, param
            and seq num.
============================================================================*/
void
                       ...
                       void * param)
{
  ...
  int subid = diag_listener_get_subid(DIAG_EXT_LOG, log);
  if(subid < 0)
  {
	MSG_3 (MSG_SSID_DIAG, MSG_LEGACY_HIGH,
  ...
  {
	MSG_4 (MSG_SSID_DIAG, MSG_LEGACY_HIGH,
          "Log listener 0x%04X param:0x%08X seq:%d subID:%d", 
          log_get_code ((void *) log), (uint32) param, seq_num, subid);
  }
}
...
DESCRIPTION The log has been added to listen The API diag_listener_get_subid is called
            from the listener. If a negative value is returned by the API indicating an
            error code then only log code and param are printed, if a positive value is
            returned then the sub ID is also printed along with log code and param.
============================================================================*/
                       ...
                       void * param)
{
  ...
  {
	MSG_3 (MSG_SSID_DIAG, MSG_LEGACY_HIGH,
         "Extended Log listener 0x%04X param:0x%08X subID:%d",
         log_get_code ((void *) log), (uint32) param, subid);
  }
}
...
DESCRIPTION The F3 has been added to listen The API diag_listener_get_subid is called from
            the listener only for qshrink4 messages. If a negative value is returned by the
            API indicating an error code then cmd code, ssid, mask, param and seq num are
            printed, if a positive value is returned then the sub ID is also printed along 
            with cmd code, ssid, mask, param and seq num.
============================================================================*/
void
diagdiag_f3_listener (uint32 seq_num, const diag_msg_type * msg, void * param)
{
  ...
  if( msg )
  {
    ...
    else if (msg->cmd_code == DIAG_QSR4_EXT_MSG_TERSE_F)
    {
      ...
      {
		int subid = diag_listener_get_subid(DIAG_EXT_MSG, msg);
        if(subid < 0)
        {
          MSG_5(MSG_SSID_LEGACY, MSG_LEGACY_HIGH,
             "F3 listener cmd_code:%d ssid:%d ss_mask:%d param:0x%08X seq:%d",
        ...
        {
          MSG_6(MSG_SSID_LEGACY, MSG_LEGACY_HIGH,
              	 "F3 listener cmd_code:%d ssid:%d ss_mask:%d param:0x%08X seq:%d subid:%d",
              	 msg->cmd_code, msg_ssid, msg_mask, (uint32)param, seq_num, subid);
        }
      }
      else
      {
...
DESCRIPTION The F3 has been added to listen The API diag_listener_get_subid is called from
            the listener only for qshrink4 messages. If a negative value is returned by the
            API indicating an error code then cmd code, ssid, mask, and param are printed, 
            if a positive value is returned then the sub ID is also printed along with cmd
            code, ssid, mask, and param.
============================================================================*/
void
  ...
  if( msg )
  {
    ...
    else if (msg->cmd_code == DIAG_QSR4_EXT_MSG_TERSE_F)
    {
      ...
      if (msg_get_qsr4_table_entry(qsr_hash_v4, &msg_ssid, &msg_mask))
      {
        ...
        {
        MSG_5(MSG_SSID_LEGACY, MSG_LEGACY_HIGH,
              	"F3 listener cmd_code:%d ssid:%d ss_mask:%d param:0x%08X subid:%d",
              	 msg->cmd_code, msg_ssid, msg_mask, (uint32)param, subid);
        }
      }
      else
      {
...
diagdiag_listener_pkt (void * req_pkt, uint16 pkt_len)
{
  ...
  if( req == NULL || (pkt_len != sizeof(diag_log_event_listener_req_type)))
  {
...
diagdiag_listener_ext_pkt (PACK(void *) req_pkt, uint16 pkt_len)
{
  ...
  if( req == NULL || (pkt_len < sizeof(diag_log_event_ext_listener_req_type)) || ((diagdiag_ext_listener_cmd_verify(req_pkt, pkt_len) == FALSE)))
  {
	return diagpkt_err_rsp( DIAG_BAD_PARM_F, req_pkt, pkt_len );
  }
