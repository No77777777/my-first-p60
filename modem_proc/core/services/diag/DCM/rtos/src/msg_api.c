...
$Header: //components/rel/core.mpss/10.0/services/diag/DCM/rtos/src/msg_api.c#3 $

when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
01/04/19   kdey    Fixed bugs in the logic for removal of extended listeners for events/logs/msgs
08/22/18   kdey    Fixed frame mismatch issue
06/25/18   kdey    Fixed reordering of assembly instructions in diag_time_init
04/04/18   kdey    Fixed compiler errors
02/09/18   sa      Removed Unused APIs.
02/09/17   gn      Added handling for Hash values not falling under QSR region
08/30/17   sa      Fixed the compiler warnings
07/03/17   sa      Added support for STM for msg_v4_send_var
05/02/17   gn      Added counter which gets incremented when a Hash value not falling under QSR region is generated
03/23/17   sa      Check if listener list in initialised before accessing it.
13/03/17   rs      Potential memory corruption vulnerability in msg_sprintf_ext(). Added va_end before returing on error.
02/16/17   sa      Added mutex to protect diag_searchlist_search_all
...
02/24/16   as      Fixed issue with QShrink extended API pkts sent in STM mode
06/18/15   sa      Added support for time sync feature.
09/30/15   is      Add workaround for shadow packet DV issue on Hexagon compiler
08/12/15   gn      Added mutex protection for diag_f3_searchlist_ext().
...
#include "ULog_Diag.h"
#include "micro_msg_diag_service.h"  /* for diag_msg_composite_mask */
...
void qsr_v4_copy_args(uint8 dst[], uint8 src[], uint8 length);
...
extern boolean micro_get_msg_mask(uint16 curr_ssid, uint32 *rt_mask);


extern void diag_update_composite_msg_mask(int index, int start, int end);
/*===========================================================================
...
)
{
  ...
  err_f3_save_msg(&core_f3_trace,constant_data_ptr, args, nargs, msg, type, version, param, arg_bytes);
...
msg_get_time_ext(qword * ts,unsigned int version, void* parameter)
{
  ...
  /* Not overwriting the 10 least significant bits of the timestamp in cases
  ** where the WCDMA frame callback is not registered with Diag.
  */
  if(frame_number_set == TRUE)
  {
...
DESCRIPTION
  Returns whether the specific SSID and subsystem mask is enabled or not by checking for
  the preset mask, listener masks and extended listener masks.
...
boolean msg_status(uint16 ss_id, uint32 ss_mask) {
  #ifdef DIAG_UIMAGE_MODE
    uint32 rt_mask = 0;
    boolean ret_val = micro_get_msg_mask(ss_id,&rt_mask);
	if(ret_val) // If ret_val is TRUE then check if the rt_mask matches with ss_mask 
	{
	  if(rt_mask & ss_mask)
	  	return TRUE;
	  else
	...
	return ret_val; // If ret_val is FALSE return ret_val
  ...
  valid_ssid =
    ...
    if (stream_id) {
		    ...
		    status = msg_listeners_masks_enabled(ss_id,ss_mask,F3_LISTENER);
			status|= msg_listeners_masks_enabled(ss_id,ss_mask,F3_LISTENER_EXT);
    }
...
static byte msg_v3_stream_check(const msg_v3_const_hash * const_blk)
{
  ...
  if(const_blk != NULL)
  {
    ...
    if((!stream_id) || (diag_stm_enabled))
    {
      ...
      {
        if(diag_f3_listeners.initialized)
        {
  ...
  if(stream_id)
  {
   ...
   if (version == DIAGBUF_VERSION_1)
    ...
    else
    {
      ...
      {
        msg_get_time_ext(&(msg->hdr.ts),version,param);
      }
      if ((diag_wcdma_wl1_frame_cb != NULL) || (diag_wcdma_wl1_frame_cb_ext != NULL))
...
static void diag_stm_msg_hdr_prep(msg_hdr_type *msg_hdr, unsigned int num_args , uint32 version, uint32 id)
{
  ...
  msg_get_time_ext(&(msg_hdr->ts),version,param);
  if ((diag_wcdma_wl1_frame_cb != NULL) || (diag_wcdma_wl1_frame_cb_ext != NULL))
...
void msg_sprintf_ext(uint32 version, void *parameter, uint32 num_args, const msg_const_type *const_blk, ...)
{
  ...
  if (((version == DIAGBUF_VERSION_1) && (NULL == parameter)) || (version > DIAGBUF_MAX_VERSION_NUM))
  {
     return;
  }
  ...
  if(stream_id)
  {
    ...
    if (msg)
    {
      ...
      else
      {
        ...
        msg_get_time_ext(&(msg_temp->hdr.ts),DIAGBUF_VERSION_0,NULL);
        ...
        memscpy((void *)((char *)(msg_temp) + fmt_pos), alloc_len + sizeof(msg_desc_type) - fmt_pos,
			(void *)&(const_blk->desc), sizeof(msg_desc_type));
        ...
        memscpy((void *)((char *)(msg_temp) + fmt_pos), alloc_len + sizeof(msg_desc_type) - fmt_pos,
			(void *)(const_blk->msg_hash + filename_len + 1),  MIN(MSG_MAX_STRLEN, (int)strlen((char *)const_blk->msg_hash + filename_len + 1)));
...
/*lint -save -e{545} Suspicious use of & */
      msg_get_time_ext(&(msg->hdr.ts),DIAGBUF_VERSION_0,NULL);
/*lint -restore */
...
void msg_v3_send_level1 ( const msg_v3_const_type * const_blk)
{
 asm("{r31 = r4; jump msg_v3_send}");
}

void msg_v3_send_level2 ( const msg_v3_const_type * const_blk)
{
 asm("{call msg_v3_send_level1}");
}

void msg_v3_send_level3 ( const msg_v3_const_type * const_blk)
{
 asm("{call msg_v3_send_level2}");
}

void msg_v3_send_level4 ( const msg_v3_const_type * const_blk)
{
 asm("{call msg_v3_send_level3}");
}

void msg_v3_send_pcrelR0 ( const msg_v3_const_type * const_blk)
{
  asm("{r0 = memw(r31++#4)}");
  asm("{r4 = r31; call msg_v3_send_level4}");
}
#else
void msg_v3_send_pcrelR0 ( const msg_v3_const_type * const_blk)
{
  asm("{r0 = memw(r31++#4); jump msg_v3_send}");
}
#endif
#endif
...
#ifdef __HEXMSGABI_2_SUPPORTED__
#ifdef __FLUSH_HARDWARE_RAS__
void msg_v3_send_1_level1 ( const msg_v3_const_type * const_blk, uint32 xx_arg1)
{
 asm("{r31 = r4; jump msg_v3_send_1}");
}

void msg_v3_send_1_level2 ( const msg_v3_const_type * const_blk, uint32 xx_arg1)
{
 asm("{call msg_v3_send_1_level1}");
}

void msg_v3_send_1_level3 ( const msg_v3_const_type * const_blk, uint32 xx_arg1)
{
 asm("{call msg_v3_send_1_level2}");
}

void msg_v3_send_1_level4 ( const msg_v3_const_type * const_blk, uint32 xx_arg1)
{
 asm("{call msg_v3_send_1_level3}");
}

void msg_v3_send_1_pcrelR0 ( const msg_v3_const_type * const_blk, uint32 xx_arg1)
{
 asm("{r0 = memw(r31++#4)}");
 asm("{r4 = r31; call msg_v3_send_1_level4}");
}
#else
void msg_v3_send_1_pcrelR0 (const msg_v3_const_type * const_blk,uint32 xx_arg1)
{
  asm("{r0 = memw(r31++#4); jump msg_v3_send_1}");
}
#endif
#endif
...
#ifdef __HEXMSGABI_2_SUPPORTED__
#ifdef __FLUSH_HARDWARE_RAS__
void msg_v3_send_2_level1 ( const msg_v3_const_type * const_blk, uint32 xx_arg1, uint32 xx_arg2)
{
 asm("{r31 = r4; jump msg_v3_send_2}");
}

void msg_v3_send_2_level2 ( const msg_v3_const_type * const_blk, uint32 xx_arg1, uint32 xx_arg2)
{
 asm("{call msg_v3_send_2_level1}");
}

void msg_v3_send_2_level3 ( const msg_v3_const_type * const_blk, uint32 xx_arg1, uint32 xx_arg2)
{
 asm("{call msg_v3_send_2_level2}");
}

void msg_v3_send_2_level4 ( const msg_v3_const_type * const_blk, uint32 xx_arg1, uint32 xx_arg2)
{
 asm("{call msg_v3_send_2_level3}");
}

void msg_v3_send_2_pcrelR0 ( const msg_v3_const_type * const_blk, uint32 xx_arg1, uint32 xx_arg2)
{
 asm("{r0 = memw(r31++#4)}");
 asm("{r4 = r31; call msg_v3_send_2_level4}");
}
#else
void msg_v3_send_2_pcrelR0 (const msg_v3_const_type * const_blk, uint32 xx_arg1, uint32 xx_arg2)
{
  asm("{r0 = memw(r31++#4); jump msg_v3_send_2}");
}
#endif
#endif
...
#ifdef __HEXMSGABI_2_SUPPORTED__
#ifdef __FLUSH_HARDWARE_RAS__
void msg_v3_send_3_level1 ( const msg_v3_const_type * const_blk, uint32 xx_arg1,
        uint32 xx_arg2, uint32 xx_arg3)
{
 asm("{r31 = r4; jump msg_v3_send_3}");
}

void msg_v3_send_3_level2 ( const msg_v3_const_type * const_blk, uint32 xx_arg1,
        uint32 xx_arg2, uint32 xx_arg3)
{
 asm("{call msg_v3_send_3_level1}");
}

void msg_v3_send_3_level3 ( const msg_v3_const_type * const_blk, uint32 xx_arg1,
        uint32 xx_arg2, uint32 xx_arg3)
{
 asm("{call msg_v3_send_3_level2}");
}

void msg_v3_send_3_level4 ( const msg_v3_const_type * const_blk, uint32 xx_arg1,
        uint32 xx_arg2, uint32 xx_arg3)
{
 asm("{call msg_v3_send_3_level3}");
}

void msg_v3_send_3_pcrelR0 ( const msg_v3_const_type * const_blk, uint32 xx_arg1,
        uint32 xx_arg2, uint32 xx_arg3)
{
 asm("{r0 = memw(r31++#4)}");
 asm("{r4 = r31; call msg_v3_send_3_level4}");
}
...
void msg_v3_send_3_pcrelR0 (const msg_v3_const_type * const_blk, uint32 xx_arg1,
                            uint32 xx_arg2, uint32 xx_arg3)
{
  asm("{r0 = memw(r31++#4); jump msg_v3_send_3}");
}
#endif
#endif
...
===========================================================================*/
uint8 qsr_v4_check_bytes(uint8 arg_bytes, uint32 arg)
{
  int byte_sized = 0;
...
qsr_v4_get_address_table_index (const uint32 search_address)
{
  ...
  uint16 index = 0;
  uint16 msg_index = 0;
  /* sheethal start commented out for ULOG_RT_PRINTF_1 compilation errors
  
  if ( (search_address > qsr_4_0_msg_end_address) || (search_address < msg_v4_ss_uam_table[start_index].start_address)) 
  {
    
    ULOG_RT_PRINTF_1( diag_err_ulog_handle,"Search_address %d is out of bounds",search_address);
  }  sheethal end */
...
#ifdef __HEXMSGABI_2_SUPPORTED__
#ifdef __FLUSH_HARDWARE_RAS__
void msg_v4_send_level1 ( const msg_v4_const_type * xx_msg_const_ptr)
{
 asm("{r31 = r4; jump msg_v4_send}");
}

void msg_v4_send_level2 ( const msg_v4_const_type * xx_msg_const_ptr)
{
 asm("{call msg_v4_send_level1}");
}

void msg_v4_send_level3 ( const msg_v4_const_type * xx_msg_const_ptr)
{
 asm("{call msg_v4_send_level2}");
}

void msg_v4_send_level4 ( const msg_v4_const_type * xx_msg_const_ptr)
{
 asm("{call msg_v4_send_level3}");
}

void msg_v4_send_pcrelR0 ( const msg_v4_const_type * xx_msg_const_ptr)
{
  asm("{r0 = memw(r31++#4)}");
...
void msg_v4_send_ext (const msg_v4_const_type * xx_msg_const_ptr,uint32 version, void *parameter)
{
  ...
  if(msg_enabled_mask != 0)
  {
    ...
    else
    {
	  /* Message not part of QShrink  - Error to be investigated */
	  qsr_v4_incr_drop_count_f3();
	  ULogFront_RealTimePrintf(diag_debug_ulog_handle,1,"msg_v4_send_ext: Invalid hash value : qsr_hash_v4 : 0x%x",qsr_hash_v4);
      return;
    }
    stream_id = msg_v4_stream_check(xx_msg_index);
...
FUNCTION MSG_V4_SEND_EXT_v1

DESCRIPTION
  This function is a shim API which calls msg_v4_send_ext_v1.
  The version is hardcoded to 1. Reference to the parameter is passed.
...
{
	msg_v4_send_ext(xx_msg_const_ptr,1,&parameter);
} /* msg_v4_send_ext_v1 */
...
#ifdef __HEXMSGABI_2_SUPPORTED__
#ifdef __FLUSH_HARDWARE_RAS__
void msg_v4_send_1_level1 ( const msg_v4_const_type * xx_msg_const_ptr, uint32 xx_arg1)
{
 asm("{r31 = r4; jump msg_v4_send_1}");
}

void msg_v4_send_1_level2 ( const msg_v4_const_type * xx_msg_const_ptr, uint32 xx_arg1)
{
 asm("{call msg_v4_send_1_level1}");
}

void msg_v4_send_1_level3 ( const msg_v4_const_type * xx_msg_const_ptr, uint32 xx_arg1)
{
 asm("{call msg_v4_send_1_level2}");
}

void msg_v4_send_1_level4 ( const msg_v4_const_type * xx_msg_const_ptr, uint32 xx_arg1)
{
 asm("{call msg_v4_send_1_level3}");
}

void msg_v4_send_1_pcrelR0 ( const msg_v4_const_type * xx_msg_const_ptr, uint32 xx_arg1)
{
 asm("{r0 = memw(r31++#4)}");
...
void msg_v4_send_1_ext(const msg_v4_const_type *xx_msg_const_ptr, uint32 version, void *parameter, uint32 xx_arg1)
{
  ...
  if (msg_enabled_mask != 0)
  {
    ...
    else
    {
	  /* Message not part of QShrink  - Error to be investigated */
	  qsr_v4_incr_drop_count_f3();
	  ULogFront_RealTimePrintf(diag_debug_ulog_handle,1,"msg_v4_send_1_ext: Invalid hash value : qsr_hash_v4 : 0x%x",qsr_hash_v4);
      return;
    }
    stream_id = msg_v4_stream_check(xx_msg_index);
...
{
	msg_v4_send_1_ext(xx_msg_const_ptr, 1, &parameter, xx_arg1);
} /* msg_v4_send_ext_v1 */	
...
#ifdef __HEXMSGABI_2_SUPPORTED__
#ifdef __FLUSH_HARDWARE_RAS__
void msg_v4_send_2_level1 ( const msg_v4_const_type * xx_msg_const_ptr, uint32 xx_arg1, uint32 xx_arg2)
{
 asm("{r31 = r4; jump msg_v4_send_2}");
}

void msg_v4_send_2_level2 ( const msg_v4_const_type * xx_msg_const_ptr, uint32 xx_arg1, uint32 xx_arg2)
{
 asm("{call msg_v4_send_2_level1}");
}

void msg_v4_send_2_level3 ( const msg_v4_const_type * xx_msg_const_ptr, uint32 xx_arg1, uint32 xx_arg2)
{
 asm("{call msg_v4_send_2_level2}");
}

void msg_v4_send_2_level4 ( const msg_v4_const_type * xx_msg_const_ptr, uint32 xx_arg1, uint32 xx_arg2)
{
 asm("{call msg_v4_send_2_level3}");
}

void msg_v4_send_2_pcrelR0 ( const msg_v4_const_type * xx_msg_const_ptr, uint32 xx_arg1, uint32 xx_arg2)
{
 asm("{r0 = memw(r31++#4)}");
...
void msg_v4_send_2_ext (const msg_v4_const_type * xx_msg_const_ptr, uint32 version, void *parameter, uint32 xx_arg1, uint32 xx_arg2)
{
  ...
  if(msg_enabled_mask != 0)
  {
    ...
    else
    {
	  /* Message not part of QShrink  - Error to be investigated */
	  qsr_v4_incr_drop_count_f3();
	  ULogFront_RealTimePrintf(diag_debug_ulog_handle,1,"msg_v4_send_2_ext: Invalid hash value : qsr_hash_v4 : 0x%x",qsr_hash_v4);
      return;
    }
    stream_id = msg_v4_stream_check(xx_msg_index);
#ifdef FEATURE_DIAG_STM
...
===========================================================================*/
void msg_v4_send_2_ext_v1 (const msg_v4_const_type * xx_msg_const_ptr, uint32 parameter, uint32 xx_arg1, uint32 xx_arg2)
{
	msg_v4_send_2_ext(xx_msg_const_ptr, 1, &parameter, xx_arg1, xx_arg2);
} /* msg_v4_send_2_ext_v1 */	
...
#ifdef __HEXMSGABI_2_SUPPORTED__
#ifdef __FLUSH_HARDWARE_RAS__
void msg_v4_send_3_level1 ( const msg_v4_const_type * xx_msg_const_ptr, uint32 xx_arg1,
        uint32 xx_arg2, uint32 xx_arg3)
{
 asm("{r31 = r4; jump msg_v4_send_3}");
}

void msg_v4_send_3_level2 ( const msg_v4_const_type * xx_msg_const_ptr, uint32 xx_arg1,
        uint32 xx_arg2, uint32 xx_arg3)
{
 asm("{call msg_v4_send_3_level1}");
}

void msg_v4_send_3_level3 ( const msg_v4_const_type * xx_msg_const_ptr, uint32 xx_arg1,
        uint32 xx_arg2, uint32 xx_arg3)
{
 asm("{call msg_v4_send_3_level2}");
}

void msg_v4_send_3_level4 ( const msg_v4_const_type * xx_msg_const_ptr, uint32 xx_arg1,
        uint32 xx_arg2, uint32 xx_arg3)
{
     ...
     uint32 xx_arg2, uint32 xx_arg3)
{
  ...
  if (msg_enabled_mask != 0)
  {
    ...
    else
    {
	  /* Message not part of QShrink  - Error to be investigated */
	  qsr_v4_incr_drop_count_f3();
	  ULogFront_RealTimePrintf(diag_debug_ulog_handle,1,"msg_v4_send_3_ext: Invalid hash value : qsr_hash_v4 : 0x%x",qsr_hash_v4);
      return;
    }
    stream_id = msg_v4_stream_check(xx_msg_index);
#ifdef FEATURE_DIAG_STM
...
===========================================================================*/
void msg_v4_send_3_ext_v1 (const msg_v4_const_type * xx_msg_const_ptr, uint32 parameter, uint32 xx_arg1, uint32 xx_arg2, uint32 xx_arg3)
{
	msg_v4_send_3_ext(xx_msg_const_ptr, 1, &parameter, xx_arg1, xx_arg2, xx_arg3);
} /* msg_v4_send_3_ext_v1 */	
...
/*lint -restore */
    //XXX TODO: don't cast like this --rh
    msg_save_trace((const void *)qsr_hash_v4, f3_args, num_args, (void *)msg, DIAG_F3_TRACE_QSHRINK40, 0, NULL, arg_bytes);
...
msg_v4_send_var_ext(const msg_v4_const_type *xx_msg_const_ptr, uint32 version, void *parameter, uint32 num_args, ...)
{
  ...
  if(num_args > MAX_VAR_ARGS)
  {
    ERR_FATAL("msg_v4_send_var_ext:  num_args(%lu) > 9", num_args, 0, 0);
  }
  num_args = MIN(num_args, MAX_VAR_ARGS);
 ...
 /*lint -save -e{64,10,78,718,746,628,732,530} */
  ...
  if (msg_enabled_mask != 0)
  {
    ...
    else
    {
	  /* Message not part of QShrink  - Error to be investigated */
	  qsr_v4_incr_drop_count_f3();
	  ULogFront_RealTimePrintf(diag_debug_ulog_handle,1,"msg_v4_send_var_ext: Invalid hash value : qsr_hash_v4 : 0x%x",qsr_hash_v4);
      return;
    }
    stream_id = msg_v4_stream_check(xx_msg_index);
#ifdef FEATURE_DIAG_STM
...
/*lint -restore */
  //XXX TODO: don't cast like this --rh
    msg_save_trace((const void *)qsr_hash_v4, f3_args, num_args, (void *)msg, DIAG_F3_TRACE_QSHRINK40, (uint8)version, parameter, arg_bytes);
...
===========================================================================*/
void msg_v4_send_4_ext_v1 (const msg_v4_const_type * xx_msg_const_ptr, uint32 parameter, uint32 num_args,
						   uint32 xx_arg1, uint32 xx_arg2, uint32 xx_arg3, uint32 xx_arg4)
{
	msg_v4_send_var_ext(xx_msg_const_ptr, 1, &parameter, num_args, xx_arg1, xx_arg2, xx_arg3, xx_arg4);
} /* msg_v4_send_4_ext_v1 */

/*===========================================================================

FUNCTION MSG_V4_SEND_5_EXT_V1
...
===========================================================================*/
void msg_v4_send_5_ext_v1 (const msg_v4_const_type * xx_msg_const_ptr, uint32 parameter, uint32 num_args, 
						   uint32 xx_arg1, uint32 xx_arg2, uint32 xx_arg3, uint32 xx_arg4, uint32 xx_arg5)
{
	msg_v4_send_var_ext(xx_msg_const_ptr, 1, &parameter, num_args, xx_arg1, xx_arg2, xx_arg3, xx_arg4, xx_arg5);
} /* msg_v4_send_5_ext_v1 */

/*===========================================================================

FUNCTION MSG_V4_SEND_6_EXT_V1
...
===========================================================================*/
void msg_v4_send_6_ext_v1 (const msg_v4_const_type * xx_msg_const_ptr, uint32 parameter, uint32 num_args, uint32 xx_arg1, 
						   uint32 xx_arg2, uint32 xx_arg3, uint32 xx_arg4, uint32 xx_arg5, uint32 xx_arg6)
{
	msg_v4_send_var_ext(xx_msg_const_ptr, 1, &parameter, num_args, xx_arg1, xx_arg2, xx_arg3, xx_arg4, xx_arg5, xx_arg6);
} /* msg_v4_send_6_ext_v1 */

/*===========================================================================

FUNCTION MSG_V4_SEND_7_EXT_V1
...
===========================================================================*/
void msg_v4_send_7_ext_v1 (const msg_v4_const_type * xx_msg_const_ptr, uint32 parameter, uint32 num_args, uint32 xx_arg1, 
						   uint32 xx_arg2, uint32 xx_arg3, uint32 xx_arg4, uint32 xx_arg5, uint32 xx_arg6, uint32 xx_arg7)
{
	msg_v4_send_var_ext(xx_msg_const_ptr, 1, &parameter, num_args, xx_arg1, xx_arg2, xx_arg3, xx_arg4, xx_arg5, xx_arg6, xx_arg7);
} /* msg_v4_send_7_ext_v1 */

/*===========================================================================

FUNCTION MSG_V4_SEND_8_EXT_V1
...
===========================================================================*/
void msg_v4_send_8_ext_v1 (const msg_v4_const_type * xx_msg_const_ptr, uint32 parameter, uint32 num_args, uint32 xx_arg1, uint32 xx_arg2, 
						   uint32 xx_arg3, uint32 xx_arg4, uint32 xx_arg5, uint32 xx_arg6, uint32 xx_arg7, uint32 xx_arg8)
{
	msg_v4_send_var_ext(xx_msg_const_ptr, 1, &parameter, num_args, xx_arg1, xx_arg2, xx_arg3, xx_arg4, xx_arg5, xx_arg6, xx_arg7, xx_arg8);
} /* msg_v4_send_8_ext_v1 */

/*===========================================================================

FUNCTION MSG_V4_SEND_9_EXT_V1
...
===========================================================================*/
void msg_v4_send_9_ext_v1 (const msg_v4_const_type * xx_msg_const_ptr, uint32 parameter, uint32 num_args, uint32 xx_arg1, uint32 xx_arg2, 
						   uint32 xx_arg3, uint32 xx_arg4, uint32 xx_arg5, uint32 xx_arg6, uint32 xx_arg7, uint32 xx_arg8, uint32 xx_arg9)
{
	msg_v4_send_var_ext(xx_msg_const_ptr, 1, &parameter, num_args, xx_arg1, xx_arg2, xx_arg3, xx_arg4, xx_arg5, xx_arg6, xx_arg7, xx_arg8, xx_arg9);
} /* msg_v4_send_9_ext_v1 */
    ...
    if (success)
    {
      ...
      for(i = 0 ; !found && i < LISTENER_MSG_MASK_TBL_CNT; i++)
      {
        if( (ssid >= listener_msg_mask_tbl[i].ssid_first) && (ssid <= listener_msg_mask_tbl[i].ssid_last))
        {
          ...
          diag_update_composite_msg_mask(i,ssid - listener_msg_mask_tbl[i].ssid_first, ssid - listener_msg_mask_tbl[i].ssid_first );
        }
      }
...
for(f3_listener_key_index = 0; f3_listener_key_index < num_keys; f3_listener_key_index++)
{
  ...
  for(i = 0 ; !found && i < LISTENER_MSG_MASK_TBL_CNT; i++)
  {
    if( (keys[f3_listener_key_index].ssid >= listener_msg_mask_tbl[i].ssid_first) && (keys[f3_listener_key_index].ssid <= listener_msg_mask_tbl[i].ssid_last))
    {
      ...
      diag_update_composite_msg_mask(i,keys[f3_listener_key_index].ssid - listener_msg_mask_tbl[i].ssid_first,keys[f3_listener_key_index].ssid - listener_msg_mask_tbl[i].ssid_first);
    }
  }
   osal_exit_crit_sect(&diag_f3_listeners_ext.lock);
}
...
void diag_clean_f3_listener_ext (void *param)
{
  ...
  int i = 0;
  int ssid_index = 0;
  boolean found = FALSE; /* For finding the ssid node to update the extended listeners mask */
  ...
  if(diag_f3_listeners_ext.free_count > 0)
  {
    ...
    ssid_node = (f3_searchlist_ext_entry_type*)ordered_list_peek_front(&diag_f3_listeners_ext.list);
      ...
      {
        found = FALSE;
        /* Logic for obtaining the index of the ssid in the listener msg mask table */
        for(i = 0 ; !found && i < LISTENER_MSG_MASK_TBL_CNT; i++)
        {
    ...
    /* Clearing the mask of the SSID */
    listener_msg_mask_tbl[ssid_index].f3_listener_ext_mask_array[ssid_node->entry.ssid - listener_msg_mask_tbl[ssid_index].ssid_first] = 0;
        ...
        while(listener_node)
        {
          /* Updating the mask. If the node is to be deleted then its mask will be 0 in any case as updated in diag_remove_f3_listener_ext */
          listener_msg_mask_tbl[ssid_index].f3_listener_ext_mask_array[ssid_node->entry.ssid - listener_msg_mask_tbl[ssid_index].ssid_first] |= listener_node->ss_mask;
          diag_update_composite_msg_mask(ssid_index, ssid_node->entry.ssid - listener_msg_mask_tbl[ssid_index].ssid_first,
          ssid_node->entry.ssid - listener_msg_mask_tbl[ssid_index].ssid_first);
