...
$Header: //components/rel/core.mpss/10.0/services/diag/DCM/rtos/src/log_api.c#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
01/04/19   kdey    Fixed bugs in the logic for removal of extended listeners for events/logs/msgs
08/30/17   ph      Fix memory leak with extended log packets.
...
06/13/16   as      Fixed log_free call for STM use case
...
02/22/16   as      Fixed log_shorten call for STM use case
...
diag_searchlist_type log_listeners = {FALSE, {0},{{0}}, 0, 0, 0,0,0,NULL, NULL};
...
/*=========================================================================*/
DLL_API_GLOBAL void log_commit (void * ptr)
{
...
/*=========================================================================*/
DLL_API_GLOBAL void log_free (void * ptr)
{
...
void diag_clean_log_listener_ext(void *param)
{
  ...
  int i = 0;
  unsigned char log_listener_ext_mask_local[LOG_MASK_SIZE];
  ...
  osal_enter_crit_sect(&log_listeners_ext.lock);
  node = (log_searchlist_ext_entry_type *)list_peek_front((list_type *)&log_listeners_ext);
  if(node && log_listeners_ext.free_count > 0)
  {
  ...
  {
    node = (log_searchlist_ext_entry_type *)list_peek_front((list_type *)&log_listeners_ext);
	/* Clearing out the local copy of extended log listener masks, it will be updated for the nodes which are in the list */
    memset((void*)log_listener_ext_mask_local,0,LOG_MASK_SIZE);
	/* Loop for going through all the nodes in the list and OR-ing the masks of the nodes */
    while(node)
    {
      for(i = 0; i < LOG_MASK_SIZE; i++)
      {
        log_listener_ext_mask_local[i] |= node->entry.mask[i];
      }
      /*Check for all the matching listeners */
      node = (log_searchlist_ext_entry_type *)list_peek_next((list_type *)&log_listeners_ext, (list_link_type *)node);
    }
    /* Copying the masks from local array to global array */
    memscpy(log_listener_ext_mask,LOG_MASK_SIZE,log_listener_ext_mask_local,LOG_MASK_SIZE);
    /* Update composite mask */
    log_update_composite_mask(0, LOG_MASK_SIZE);
  }
  osal_exit_crit_sect(&log_listeners_ext.lock);
}
