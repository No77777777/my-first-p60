...
Copyright (c) 2011-2019, 2021, 2023-2024 Qualcomm Technologies, Inc.
All Rights Reserved.
...
$Header: //components/rel/core.mpss/10.0/services/diag/DCM/rtos/src/event_api.c#3 $
...
09/20/16  as      Fixed Klocwork warning regarding stm_event_id.reserved
...
05/28/14   ph      Fixed warnings on 9x35 Build.
...
void event_service_listener_ext (event_store_type *event);
...
diag_searchlist_type event_listeners = {FALSE, {0, 0, 0}, {{0}}, 0, 0, 0,0,0, NULL, NULL};
...
static void event_generate_stm (event_id_enum_type event_id, uint8 length, void *payload, unsigned int version, void *parameter)
{
  ...
  if (event_pkt)
  {
    ...
    /* set the reserved bit to zero; its value does
      not matter as it is only used for padding */
...
boolean event_report_ext(event_id_enum_type event_id, unsigned int version, void *parameter)
{
  ...
  {
  case DIAGBUF_VERSION_0:
        event_ctrl->version = DIAGBUF_VERSION_0;
        event_ctrl->id = 0;
    break;
  case DIAGBUF_VERSION_1:
        if(dci_ext_header_support == TRUE){
		id = (uint32 *)parameter;
        event_ctrl->version = DIAGBUF_VERSION_1;
        event_ctrl->id = *id;
		}
		else{
...
event_report_payload_ext(event_id_enum_type event_id, uint8 length, void *payload, unsigned int version, void *parameter)
{
  ...
  else
  {
    ...
    if ((stream_id & DIAG_STREAM_2))
{
      event_ctrl = (event_store_type *)event_q_alloc(event_id, length, DIAG_STREAM_2,NULL);
      if (event_ctrl)
  {
        event_ctrl->stream_id = DIAG_STREAM_2;
  switch (version)
...
FUNCTION EVENT_SERVICE_LISTENER_EXT
...
void event_service_listener_ext(event_store_type *event)
{
  if(event && (event_config & EVENT_CONFIG_EXT_LISTENER_MASK))
  {
             ...
             const diag_event_listener listener, void *param)
{
  ...
  if (entry)
  {
    ...
    if (success)
    {
      ...
      diag_update_composite_event_mask(event_id/8, event_id/8);
    }
    else
    {
                         ...
                         const diag_event_listener_ext listener, void *param)
{
  ...
  if(node)
  {
    ...
    while(count++ < num_events)
    {
      ...
      if(current_event <= EVENT_LAST_ID)
      {
        ...
        diag_update_composite_event_mask(current_event/8, current_event/8);
      }
      else
      {
        MSG_1(MSG_SSID_DIAG, MSG_LEGACY_HIGH, 
  ...
  /* This will make sure that even if only extended event listeners are added (no preset mask or single listener is added just extended listeners) the event_config
  variable is set. */
  event_control (
        (event_listeners_ext.count > 0) ? EVENT_CONFIG_EXT_LISTENER_MASK : 0, 
        EVENT_CONFIG_EXT_LISTENER_MASK);
    osal_exit_crit_sect(&event_listeners_ext.lock);
...
void diag_clean_event_listener_ext(void *param)
{
  ...
  int i = 0;
  boolean to_be_freed = FALSE;
  unsigned char diag_event_listener_ext_mask_local[EVENT_MASK_SIZE];
  if(!event_listeners_ext.initialized)
  {
    return ;
  }
  osal_enter_crit_sect(&event_listeners_ext.lock);
  node = (event_searchlist_ext_entry_type *)list_peek_front((list_type *)&event_listeners_ext);
  if(node && (event_listeners_ext.free_count > 0))
  {
    /* to_be_freed will only be set if some nodes are to be deleted */
    to_be_freed = TRUE;
  }	

  /* Loop for removing the nodes which are marked to be deleted */
  ...
  {
    node = (event_searchlist_ext_entry_type *)list_peek_front((list_type *)&event_listeners_ext);
	/* Clearing out the local listener extended mask array, should update it for the nodes which are in the list */
    memset((void*)diag_event_listener_ext_mask_local,0,EVENT_MASK_SIZE);
	/* Loop for going through all the nodes in the list and OR-ing the masks of the nodes */
    while(node)
    {
      for(i = 0; i < EVENT_MASK_SIZE; i++)
      {
        diag_event_listener_ext_mask_local[i] |= node->entry.mask[i];
      }
      /* Check for all the listener nodes */
      node = (event_searchlist_ext_entry_type *)list_peek_next((list_type *)&event_listeners_ext, (list_link_type *)node);
    }
	/* Copying the masks from local array to global array */
    memscpy(diag_event_listener_ext_mask,EVENT_MASK_SIZE,diag_event_listener_ext_mask_local,EVENT_MASK_SIZE);
    diag_update_composite_event_mask(0, EVENT_MASK_SIZE);
  }
  osal_exit_crit_sect(&event_listeners_ext.lock);
  return;
}
...
boolean event_status(event_id_enum_type event_id)
{
  ...
  if(event_config || event_config_sec)
  {
