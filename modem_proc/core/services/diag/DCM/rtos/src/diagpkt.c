...
$Header: //components/rel/core.mpss/10.0/services/diag/DCM/rtos/src/diagpkt.c#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
04/29/18   ph      Enable diag boot up logging.
09/25/18   vg      Add input length validation check for diagpkt allocation APIs
...
06/13/18   gn      Set DIAG_INT_CTRL_REG_SIG even when control channel is not connected 
                   (to handle standalone cases)
06/19/18   ph      Configure buffering modes for user PD send buffers.
05/25/18   gn      Send error responses to User PD commands received over DCI channel
05/25/18   gn      Changed the retry mechanism in diagpkt_commit_dcm to suspend current task 
                   for some interval instead of waiting on a signal
04/23/18   gn      Command registration enhancement                   
12/12/17   sa      Featurize the code to avoid checking for feature mask in standalone case.
...
10/09/17   ph      Correct the feature mask check while sending command responses.
09/25/17   ph      Ensure root diagID control packet is sent first before any userPD diagIDs.
09/08/17   ph      Handle command responses only after feature mask is received.
06/19/17   ph      Tagging the data with dynamic diag IDs.
02/21/17   ph      Tag DIAG ID header to all the diag packets sent out.
05/09/17   sa      Fixed the issue of DiagID exchange failure in rootPD.
...
03/03/17   sa      Added check to see if DCM table is initialised before deregistering the table
02/15/17   rs      Added API diagpkt_max_delayed_rsp_size() and diagpkt_total_delayed_bytes_queued() and exposed them public.
02/07/17   tbg     Send all unsent DiagID requests after feature mask received
12/20/16   sa      Added support for DiagID control packet
...
08/26/16   ph      Avoid sending double error response when registration is not found.
...
04/20/16   nk      Added Extended Packet support for DCI
...
10/07/14   sr      Fixed the issue with UserPD command registration
...
06/04/14   is      Resolve crash due to uninitialized diagpkt_master_table_q
...
#include "diagstub.h"
#include "qurt_atomic_ops.h" //MPD ToDo: remove feature flag?
...
/*This is used to store GUID for rootpd */
...
extern void *diag_rsp_malloc (uint32 num_bytes); /* from diag.c */
...
static diagpkt_master_table_type * diagpkt_master_table[DIAGPKT_MSTR_TBL_SIZE];
...
int DCM_table_Init_begin = FALSE;
...
/*To indicate whether Apps support extended packets on DCI*/
boolean dci_ext_header_support = FALSE;
...
/* Intermediate queue where the registration table are added in client's context.
   When diag task handles DIAG_INT_CTRL_REG_SIG signal, the entries are extracted
   from this queue and diagpkt_master_table is updated */
q_type diagpkt_tbl_reg_q;

typedef struct
{
  q_link_type link;
  const diagpkt_master_table_type* tbl_ptr;
}diagpkt_tbl_reg_q_type;

int diagpkt_tbl_reg_q_init_begin = FALSE;
int diagpkt_tbl_reg_q_initialized = FALSE;

/* Intermediate queue where the registration table are marked for dereg in client's context.
   When diag task handles DIAG_INT_CTRL_DEREG_SIG signal, the entries are extracted
   from this queue and diagpkt_master_table is updated */

q_type diagpkt_tbl_dereg_q;

typedef struct
{
  q_link_type link;
  const diagpkt_master_table_type* tbl_ptr;
}diagpkt_tbl_dereg_q_type;

int diagpkt_tbl_dereg_q_init_begin = FALSE;
int diagpkt_tbl_dereg_q_initialized = FALSE;
extern byte * log_mask[DIAG_MAX_STREAM_ID];
extern uint16 log_mask_offset_tbl[LOG_EQUIP_ID_LAST+1];
extern const uint16 log_last_item_tbl[];
...
DESCRIPTION
  This function copies the response from user PD, allocates memory in diag_rsp_heap,
  copies the user PD reponse to the packet allocated in diag_rsp_heap and commits
  the response.  
  
  If the allocation in diag_rsp_heap fails, retry mechanism will be initiated. 
  Thread will sleep for DIAG_RSP_ALLOC_RETRY_TIMER_LEN ms and keeps retrying the 
  allocation in diag_rsp_heap.

===========================================================================*/
int diagpkt_commit_dcm (int client_handle, PACKED byte * pkt)
{
  ...
  if (memcpy_res < 0)
    ...
    while ((item = (diagpkt_q_type *) diag_rsp_malloc (size)) == NULL)
    {
      ...
      /*-----------------------------------------------------------
        ...
        asserted.  Wait for flow control to allow more TX.
        ...
        Retry mechanism:
        Sleep for DIAG_RSP_ALLOC_RETRY_TIMER_LEN ms and retry allocation.
      -----------------------------------------------------------*/
      osal_thread_sleep(DIAG_RSP_ALLOC_RETRY_TIMER_LEN, T_MSEC, FALSE);

#ifdef FEATURE_DEBUG_DIAG_HEAP
      if( (get_event_q_cnt() == 0) && (get_rsp_q_cnt() == 0) && (diag_heap_used_bytes() !=0) )
      {
        ERR_FATAL("Diag Heap Corruption Used = %d",diag_heap_used_bytes(),0,0);
      }
#endif /* FEATURE_DEBUG_DIAG_HEAP */

    } /* end while not getting space */
...
boolean diagpkt_tbl_reg_dcm(int client_handle, const byte * tbl_ptr, unsigned int count, qurt_signal_t *signal, int qdi_handle)
{
  ...
  /* Make sure that diagpkt_master_table_q is initialized */
  while(!DCM_table_Initialized)
  {
    ...
    if(temp_params.client_id)
    {
      ...
      /*Set the LSM_obj for the second and subsequent commands from the first entry
        ...
        for the same entry*/
          ...
          local_tbl_entry->fwd_status = DIAG_CTRL_INFO_FWD_STATUS_NOT_SENT;
    }
    else
    {
...
DESCRIPTION
  Registers the LSM clients interested in masks into a list.
...
boolean diagpkt_mask_tbl_reg (uint32 client_id, qurt_signal_t *mask, int qdi_handle)
{
  ...
  /* Make sure that diagpkt_mask_client_list_q is initialized */
  while(!DCM_table_Initialized)
  {
    diagpkt_table_q_init();
  }
  ...
  if(NULL == mask_client_entry->mask_client.LSM_obj)
    MSG(MSG_SSID_DIAG,MSG_LEGACY_ERROR,"failed to connect to LSM trying to register for mask changes");
...
===========================================================================*/
void diagpkt_table_q_init()
{
  /* Try to initialize diagpkt_master_table_q atomically as this function can be called from multiple threads */ 
  if(!osal_atomic_compare_and_set((osal_atomic_word_t*)&DCM_table_Init_begin, FALSE, TRUE))
    return;
...
boolean diagpkt_get_delayed_rsp_id_dcm(int client_handle, byte* out_buf_ptr, unsigned long outbuf_max_bytes, unsigned long* pbytes_written)
{
  ...
  copy_ret = qurt_qdi_copy_to_user(client_handle, out_buf_ptr, &next_id, sizeof(next_id));
    *pbytes_written = sizeof(next_id);
    success = (copy_ret >= 0);
  }
  else
  {
      MSG(MSG_SSID_DIAG,MSG_LEGACY_ERROR,"LSM request for delayed response ID which is not a uint16");
  }
  return success;

} /* diagpkt_get_delayed_rsp_id_dcm */
       ...
       diagpkt_fwd_tbl_index++ )
  {
    ...
    if( (tbl_ptr != NULL)  && (tbl_ptr->count > 0 ) && (diagpkt_table_fwd_status[diagpkt_fwd_tbl_index] == DIAG_CTRL_INFO_FWD_STATUS_NOT_SENT) )
    {
      ...
      len_written = diagcomm_ctrl_send_mult_pkt( (void *)&reg_pkt, DIAGCOMM_CTRL_MSG_HDR_SIZE + sizeof(diag_ctrl_msg_reg_type),
                                                 ...
                                                 DIAG_CTRL_PKT_CORE_CMD_REG, DIAGCOMM_PORT_SIO, DIAGCOMM_PORT_1, FALSE, &is_sent );
    ...
    else
    {
      ...
      diagcomm_ctrl_send_mult_pkt( NULL, 0, NULL, 0, DIAG_CTRL_USER_PKT_ID_0, DIAGCOMM_PORT_SIO,
...
void diagpkt_fwd_cmd_dereg(void)
{
  ...
  for( i = 0; i < DIAGPKT_MSTR_TBL_SIZE; i++ )
  {
    ...
    {
    if(diagcomm_ctrl_channel_status(DIAGCOMM_PORT_SIO, DIAGCOMM_PORT_1) && diagcomm_connect_state())
    {
     ...
     len_written = diagcomm_ctrl_send_mult_pkt( &ctrl_pkt, DIAGCOMM_CTRL_MSG_HDR_SIZE + FPOS(diag_ctrl_msg_dereg_type, user_tbl),
                          (void *)(ctrl_pkt.data.ctrl_msg_dereg.user_tbl), userpkt_len, DIAG_CTRL_USER_PKT_ID_0, DIAGCOMM_PORT_SIO, DIAGCOMM_PORT_1, TRUE, &is_sent );
                          ...
                          diagcomm_enum_port_type ctrl_channel )
{
  ...
  while( (msg_pkt_ptr != NULL) && (data_pkt_len > 0) )
  {
    ...
    /*Change the parameters for RootPD */
        ULogFront_RealTimePrintf(diag_debug_ulog_handle, 1 , "diagpkt_process_ctrl_msg: NRT Mode with diag_real_time_mode = %d", diag_real_time_mode);
          ...
          event_ctrl_report_size(EVENT_RPT_PKT_SIZE_NRT, 1024, EVENT_RPT_PKT_LEN_SIZE_NRT);
      ...
      /*Update the UserPD parameters*/
        ...
        else
        {
          ...
          {
            diag_real_time_mode = FALSE;
          ULogFront_RealTimePrintf(diag_debug_ulog_handle, 1 , "diagpkt_process_ctrl_msg: RT Mode with diag_real_time_mode = %d", diag_real_time_mode);
            diagbuf_ctrl_tx_sleep_parameters(DIAG_TX_SLEEP_THRESHOLD_NRT, DIAG_TX_SLEEP_TIME_NRT);
            diagbuf_ctrl_commit_threshold(DIAGBUF_COMMIT_THRESHOLD_NRT);
            diag_ctrl_drain_timer_len(DIAG_DRAIN_TIMER_LEN_NRT);
            event_ctrl_stale_timer(EVENT_STALE_TIMER_LEN_NRT);
      ...
      case DIAG_CTRL_MSG_EQUIP_LOG_MASK:
      {
        ...
        {
          diag_early_log_enabled = FALSE;
          /* Switch to streaming mode */
          diag_set_tx_mode_stream(DIAG_TX_MODE_STREAMING, DIAG_STREAM_1, diag_get_diagID());
        }
        break;
    ...
    ULogFront_RealTimePrintf(diag_debug_ulog_handle, 1 , "diagpkt_process_ctrl_msg: Received APPS Feature mask = 0x%X", msg_pkt_ptr->data.ctrl_msg_feature.mask[0]);
...
#endif
        {
          ...
          diag_add_diagID_element(name,msg_v4_dbase_guid,FALSE,FALSE);
      ...
      if(strncmp(name, diagID_node->process_name, MAX_PNAME_LEN) == 0)
      {
        ...
        if(!diag_diagID_supported)
        {
          /* Modem is not dependent on diagID, set signal to notify Diag that
          there's reg tables to be fwded */
          (void)diag_set_internal_sigs( DIAG_INT_CTRL_REG_SIG );
        }
...
#endif
       if (DIAG_IS_INT_FEATURE_BIT_SET(diagcomm_io_ctrl_conn[DIAGCOMM_PORT_SIO][DIAGCOMM_PORT_1].feature_mask, F_DIAG_DCI_EXTENDED_HEADER_SUPPORT))
        {
           dci_ext_header_support = TRUE;
        }
        if (DIAG_IS_INT_FEATURE_BIT_SET(diagcomm_io_ctrl_conn[DIAGCOMM_PORT_SIO][DIAGCOMM_PORT_1].feature_mask, F_DIAG_TAGGING_HEADER_SUPPORT))
        {
          diag_tagging_support = TRUE;
          diag_tag_hdr_size = sizeof(diag_tag_hdr_type);
        }
       if (!DIAG_IS_INT_FEATURE_BIT_SET(diagcomm_io_ctrl_conn[DIAGCOMM_PORT_SIO][DIAGCOMM_PORT_1].feature_mask, F_DIAG_DCI_DISABLED))
        {
       /* Set signal to open DCI channels as Apps supports DCI (DCI_DISABLED bit is not set to 1 in feature mask) */
    ...
    case DIAG_CTRL_MSG_BUF_TX_MODE:
    {
      uint8 stream_id;
        ...
        MSG_1(MSG_SSID_DIAG, MSG_LEGACY_HIGH, "Mode control packet received, version %d ", msg_pkt_ptr->data.ctrl_msg_buf_mode.version);
        ...
        if( msg_pkt_ptr->data.ctrl_msg_buf_mode.version == DIAG_CTRL_MSG_BUF_TX_MODE_VER_1 )
    {
      ...
      {
            MSG(MSG_SSID_DIAG, MSG_LEGACY_HIGH, "Version 1 all streams");
          ...
          else
          {
            ...
            MSG_1(MSG_SSID_DIAG, MSG_LEGACY_HIGH, "Version 1 stream_id %d", stream_id );
            min_id = stream_id;
            max_id = stream_id;
          }
          ...
          for( id = min_id; id <= max_id; id++ )
          {
            ...
            while(diagid_ptr !=NULL)
            {
              ...
              MSG_3(MSG_SSID_DIAG, MSG_LEGACY_HIGH, "Setting mode %d for stream_id %d diagID %d", 
                        msg_pkt_ptr->data.ctrl_msg_buf_mode.mode, id, diag_id );
        ...
        else if(msg_pkt_ptr->data.ctrl_msg_buf_mode.version == DIAG_CTRL_MSG_BUF_TX_MODE_VER_2)
        {
          ...
          {
            MSG(MSG_SSID_DIAG, MSG_LEGACY_HIGH, "Version 2 all streams");
            min_id = DIAG_MIN_STREAM_ID;
            max_id = DIAG_MAX_STREAM_ID;
      }
      else
      {
            ...
            MSG_1(MSG_SSID_DIAG, MSG_LEGACY_HIGH, "Version 2 stream_id %d", stream_id );
            min_id = stream_id;
            max_id = stream_id;
          }
          for( id = min_id; id <= max_id; id++ )
          {
            MSG_3(MSG_SSID_DIAG, MSG_LEGACY_HIGH, "Setting mode %d for stream_id %d diagID %d", 
                        msg_pkt_ptr->data.ctrl_msg_buf_mode_v2.mode, id, msg_pkt_ptr->data.ctrl_msg_buf_mode_v2.diag_id );
    ...
    /* Reset the early logging flag since TX mode change is attempted */
    diag_early_log_enabled = FALSE;
      break;
    }
      ...
      case DIAG_CTRL_MSG_TIME_API:
      {
       ...
       case DIAG_CTRL_MSG_DIAGID:
       {
           if( msg_pkt_ptr->data.ctrl_msg_diagID.version<= DIAG_CTRL_MSG_DIAGID_VER)
           {
             ...
             if(!diag_diagID_set_tx_ptr(pname))
             {
               MSG_1(MSG_SSID_DIAG, MSG_LEGACY_ERROR, "diagpkt_process_ctrl_msg: Unable to set TX ptr for diagID %d",
                      msg_pkt_ptr->data.ctrl_msg_diagID.diagID);
             }
  ...
  unsigned int length)
{
  ...
  if (length > DIAG_MAX_DELAYED_RSP_SIZE) {
  ...
  if(item)
  {
      ...
      ULogFront_RealTimePrintf(diag_err_log_handle, 2, "1: cur_delayed_rsp_memory_used is %d Length is %d ", cur_delayed_rsp_memory_used, length );
...
diagpkt_alloc (diagpkt_cmd_code_type code, unsigned int length)
{
  ...
  if(length < sizeof(diagpkt_hdr_type))
  {
    ULogFront_RealTimePrintf(diag_cmd_ulog_handle, 2, "diagpkt_alloc : Invalid length request for response: %d cmd_code: %d", length, code);
	return NULL;
  }
              ...
              diagpkt_subsys_cmd_code_type code, unsigned int length)
{
  ...
  if(length < sizeof(diagpkt_subsys_hdr_type))
  {
    ULogFront_RealTimePrintf(diag_cmd_ulog_handle, 3, "diagpkt_subsys_alloc : Invalid length request for response: %d  subsys_id: %d subsys_cmdcode: %d", length, id, code);
	return NULL;
  }
...
FUNCTION diagpkt_next_delayed_rsp_id

DESCRIPTION
  Retrieves next delayed response id.

===========================================================================*/
uint16 diagpkt_next_delayed_rsp_id(uint16 rspid) //MPD ToDo: return value and param was uint32 in adsp
{
  uint16 temprspid;
              ...
              diagpkt_subsys_cmd_code_type code, unsigned int length)
{
  ...
  if(length < sizeof(diagpkt_subsys_hdr_type_v2))
  {
    ULogFront_RealTimePrintf(diag_cmd_ulog_handle, 3, "diagpkt_subsys_alloc_v2 : Invalid length request for response: %d susbsys_id: %d subsys_cmdcode: %d", length, id, code);
	return NULL;
  }
              ...
              unsigned int length)
{
  ...
  if(length < sizeof(diagpkt_subsys_hdr_type_v2))
  {
    ULogFront_RealTimePrintf(diag_cmd_ulog_handle, 4, "diagpkt_subsys_alloc_v2_delay : Invalid length request for response: %d subsys_id: %d subsys_cmdcode: %d delayed_rsp_id: %d", length, id, code, delayed_rsp_id_arg);
	return NULL;
  }
...
diagpkt_rsp_send (void)
{
  ...
  /* Check for feature mask and diagID only in case of multiprocessor slave configurations */
#if defined(DIAG_MP) && !defined(DIAG_MP_MASTER)
  ...
  {
    ULogFront_RealTimePrintf(diag_debug_ulog_handle, 0 , "diagpkt_rsp_send: Attempt to send response before feature mask OR diagID is received");
    return FALSE;
  }
#endif
      ...
      if (item)
      {
        ...
        else
        {
          ...
          if( item->rsp.cmd_tag )
          {
            ...
            /* If response is greater than or equal to 4 bytes, logging first four bytes */
            if (item->rsp.length >= 4)
            {
              ULogFront_RealTimePrintf(diag_cmd_ulog_handle, 5, "diagpkt_rsp_send: Sending response packet over DCI: Tag : %d, Packet : 0x%02X 0x%02X 0x%02X 0x%02X",\
                                       item->rsp.cmd_tag, item->rsp.pkt[0], item->rsp.pkt[1], item->rsp.pkt[2], item->rsp.pkt[3], item->rsp.pkt[4]);
            }
            /* If response is of less than 4 bytes, logging just the first byte (as the first byte is the command code) */
            else
            {
              ULogFront_RealTimePrintf(diag_cmd_ulog_handle, 2, "diagpkt_rsp_send: Sending response packet over DCI: Tag : %d, Packet : 0x%02X",\
                                       item->rsp.cmd_tag, item->rsp.pkt[0]);
            }
          }
          else
          {
...
diagpkt_delay_rsp_send (void)
{
  ...
  static diag_send_desc_type send =
    { NULL, NULL, DIAG_SEND_STATE_COMPLETE, TRUE, FALSE, TRUE, DIAGID_UNKNOWN_ID};
  ...
  if((diagcomm_io_ctrl_conn[DIAGCOMM_PORT_SIO][DIAGCOMM_PORT_1].feature_mask[0] == 0) ||
    ...
    (diag_diagID_supported) && ( diag_get_diagID() == DIAGID_UNKNOWN_ID)))
  {
    ULogFront_RealTimePrintf(diag_debug_ulog_handle, 0 , "diagpkt_delay_rsp_send: Attempt to send response before feature mask OR diagID is received");
    return FALSE;
  }
#endif
  ...
  while (!finished && status == DIAGBUF_READY_S)
  {
    if (item)
    {
      ...
      if (send.state == DIAG_SEND_STATE_COMPLETE)
      {
        ...
        osal_lock_mutex(&diag_delayed_response_mutex);
        cur_delayed_rsp_memory_used -=length;
        ULogFront_RealTimePrintf(diag_err_log_handle, 2, "2: cur_delayed_rsp_memory_used is %d Length is %d ", cur_delayed_rsp_memory_used, length );
        osal_unlock_mutex(&diag_delayed_response_mutex);
...
/* ==========================================================================
FUNCTION DIAGPKT_TBL_REG_Q_INIT

DESCRIPTION
  Initializes diagpkt_tbl_reg_q
============================================================================= */
void diagpkt_tbl_reg_q_init()
{
  /* Try to initialize diagpkt_tbl_reg_q_init_begin atomically as this function can be called from multiple threads */
  if(!osal_atomic_compare_and_set((osal_atomic_word_t*)&diagpkt_tbl_reg_q_init_begin, FALSE, TRUE))
    return;

  /* Initialize the queue */
  if(!q_init(&diagpkt_tbl_reg_q))
  {
    /* In case of queue init failure, reset diagpkt_tbl_reg_q_init_begin and return */
    diagpkt_tbl_reg_q_init_begin = FALSE;
    return;
  }

  /* Queue initialization successful */
  diagpkt_tbl_reg_q_initialized = TRUE;
}
...
{
  diagpkt_tbl_reg_q_type* diagpkt_tbl_reg_q_entry = NULL;
  ...
  /* Initialize the registration table queue */
  while(diagpkt_tbl_reg_q_initialized == FALSE)
  {
    diagpkt_tbl_reg_q_init();
  }

  /* Allocate memory for the queue entry */
  diagpkt_tbl_reg_q_entry = (diagpkt_tbl_reg_q_type*) malloc(sizeof(diagpkt_tbl_reg_q_type));  

  /* If the malloc fails, inject crash. Ideally malloc shouldn't fail as we are requesting for just 20 bytes */
  if(!diagpkt_tbl_reg_q_entry)
  {
    ERR_FATAL("Malloc failure in diagpkt_tbl_reg for client table : %p and size : %d", tbl_ptr, sizeof(diagpkt_tbl_reg_q_type), 0);
  }

  /* Update the queue entry with the table pointer */
  diagpkt_tbl_reg_q_entry->tbl_ptr = tbl_ptr;

  /* Add the item to the queue */
  q_put(&diagpkt_tbl_reg_q, q_link (diagpkt_tbl_reg_q_entry, &diagpkt_tbl_reg_q_entry->link));

  /* Set signal to update diagpkt_master_table and if multi proc then forward command registrations to apps */
  diag_set_internal_sigs(DIAG_INT_CTRL_REG_SIG);
}

/* ==========================================================================
FUNCTION DIAGPKT_UPDATE_MASTER_TABLE_REG

DESCRIPTION
  Extracts items from diagpkt_tbl_reg_q and appends to diagpkt_master_table
============================================================================= */

void diagpkt_update_master_table_reg()
{
  diagpkt_tbl_reg_q_type* diagpkt_tbl_reg_q_entry = NULL;

  /* Queue not initialized, nothing to update */
  if(diagpkt_tbl_reg_q_initialized == FALSE)
	  return;

  /* Extract all elements from the queue and update diagpkt_master_table */
  while(q_cnt(&diagpkt_tbl_reg_q) > 0)
  {
    diagpkt_tbl_reg_q_entry = (diagpkt_tbl_reg_q_type*) q_get(&diagpkt_tbl_reg_q);

    /* No more entries in the queue, exit the loop */
    if(!diagpkt_tbl_reg_q_entry)
      break;

    /* Update the diagpkt_master_table */
    for( ; diagpkt_master_tbl_index < DIAGPKT_MSTR_TBL_SIZE; diagpkt_master_tbl_index++ )
    {
      if( diagpkt_master_table[diagpkt_master_tbl_index] == NULL )
      {
        /* Always register the user table locally */
        diagpkt_master_table[diagpkt_master_tbl_index] = (diagpkt_master_table_type *)diagpkt_tbl_reg_q_entry->tbl_ptr;
        break;
      }
    }
    /* Free the queue element */
    free(diagpkt_tbl_reg_q_entry);
  }
}

/* ==========================================================================
FUNCTION DIAGPKT_TBL_DEREG_Q_INIT

DESCRIPTION
  Initializes diagpkt_tbl_dereg_q
============================================================================= */
void diagpkt_tbl_dereg_q_init()
{
  /* Try to initialize diagpkt_tbl_dereg_q_init_begin atomically as this function can be called from multiple threads */
  if(!osal_atomic_compare_and_set((osal_atomic_word_t*)&diagpkt_tbl_dereg_q_init_begin, FALSE, TRUE))
    return;

  /* Initialize the queue */
  if(!q_init(&diagpkt_tbl_dereg_q))
  {
    /* In case of queue init failure, reset diagpkt_tbl_reg_q_init_begin and return */
    diagpkt_tbl_dereg_q_init_begin = FALSE;
    return;
  }

  /* Queue initialization successful */
  diagpkt_tbl_dereg_q_initialized = TRUE;
}
...
{
  diagpkt_tbl_dereg_q_type* diagpkt_tbl_dereg_q_entry = NULL;
  
  if(tbl_ptr == NULL)
  {
    /* Special case where all commands have to be deregistered 
       An entry is created with tbl_ptr as NULL and added to the diagpkt_tbl_dereg_q.
       If tbl_ptr is NULL, all tables are marked for dereg in diagpkt_update_master_table_dereg */
  }
  else if(tbl_ptr->count == 0)
  {
    return;
  }

  /* Initialize the registration table queue */
  while(diagpkt_tbl_dereg_q_initialized == FALSE)
  {
    diagpkt_tbl_dereg_q_init();
  }

  /* Allocate memory for the queue entry */
  diagpkt_tbl_dereg_q_entry = (diagpkt_tbl_dereg_q_type*) malloc(sizeof(diagpkt_tbl_dereg_q_type));  

  /* If the malloc fails, just return */
  if(!diagpkt_tbl_dereg_q_entry)
  {
    return;
  }

  /* Update the queue entry with the table pointer */
  diagpkt_tbl_dereg_q_entry->tbl_ptr = tbl_ptr;

  /* Add the item to the queue */
  q_put(&diagpkt_tbl_dereg_q, q_link (diagpkt_tbl_dereg_q_entry, &diagpkt_tbl_dereg_q_entry->link));
...
/* ==========================================================================
FUNCTION DIAGPKT_UPDATE_MASTER_TABLE_DEREG

DESCRIPTION
  Extracts items from diagpkt_tbl_dereg_q and mark corresponding entry in 
  diagpkt_table_fwd_status as deregistered
============================================================================= */

void diagpkt_update_master_table_dereg()
{
  int i = 0;
  diagpkt_tbl_dereg_q_type* diagpkt_tbl_dereg_q_entry = NULL;
  ...
  /* Queue not initialized, nothing to update */
  if(diagpkt_tbl_dereg_q_initialized == FALSE)
    return;

  /* Extract all elements from the queue and update diagpkt_master_table */
  while(q_cnt(&diagpkt_tbl_dereg_q) > 0)
  {
    diagpkt_tbl_dereg_q_entry = (diagpkt_tbl_dereg_q_type*) q_get(&diagpkt_tbl_dereg_q);

    /* No more entries in the queue, exit the loop */
    if(!diagpkt_tbl_dereg_q_entry)
      break;
  
    tbl_ptr = (diagpkt_master_table_type *)diagpkt_tbl_dereg_q_entry->tbl_ptr;
    ...
    if(tbl_ptr == NULL)
    {
      ...
      /* Free the queue element */
      free(diagpkt_tbl_dereg_q_entry);	  
...
#define DIAG_CM_POWER_OFF 6
  ...
  if( !bFromDiagCmdQ)
  {
    switch(((byte *)req_pkt)[0])
    {
      ...
      {
        /* Request coming from .DIAGIN, parse the request, update the masks
        and return. There is no need for any response to be sent */
        msg_set_rt_mask_req_type *req = (msg_set_rt_mask_req_type *) req_pkt;
        uint32* mask =NULL;
        uint32 i =0;
        if(req->sub_cmd == MSG_EXT_SUBCMD_SET_RT_MASK )
        {
          mask = req->rt_mask;

          for (i = req->ssid_start; i <= req->ssid_end; i++)
          {
            (void) msg_set_rt_mask_adv ((uint16) i, *mask, DIAG_STREAM_1, diag_cur_preset_id);
      ...
      {
        DIAG_LOG_CONFIG_F_req_type *req_ptr = (DIAG_LOG_CONFIG_F_req_type *) req_pkt;
        log_config_op_req_type *req_op_ptr = (log_config_op_req_type *) req_ptr->op_data;
        byte *mask_rd_ptr = NULL;
        byte *mask_wr_ptr = NULL;
        uint16 i=0;
        unsigned int last_item, equip_id = 0;


        if(req_ptr->operation == LOG_CONFIG_SET_MASK_OP)
        {
          equip_id = req_op_ptr->set_mask.code_range.equip_id;
          if ( (equip_id < LOG_EQUIP_ID_MAX) && (equip_id < LOG_EQUIP_ID_LAST_DEFAULT))
          {
            /* Set the log mask */
      ...
      break;
      case DIAG_EVENT_MASK_SET_F:
      {
          event_mask_set_req_type *mask_set_req_ptr = (event_mask_set_req_type *) req_pkt;
          unsigned int mask_len = (unsigned int) pkt_len -
                          FPOS (event_mask_set_req_type, mask);

          event_mask_set_internal_handler( DIAG_OP_SET_EVENT_STREAM, DIAG_STREAM_1,
                                          mask_set_req_ptr->numbits, mask_len,
                                          (unsigned char *)mask_set_req_ptr->mask );
  ...
  if (!found && diag_dci_cmd_tag)
  {
     /* If the command is not registered in root pd and the command is received over DCI channel (Non zero diag_dci_cmd_tag),
        send error response as the command responses path is not enabled for User PD DCI */
     rsp_pkt = (void *) diagpkt_err_rsp( DIAG_BAD_CMD_F, req_pkt, pkt_len );
  ...
  /* multi-pd start */
    if ( DCM_table_Initialized && !found && !skip_master_table )
    {
        int local_count, k;
     ...
     command code. */
        ...
        /*If not found, error response wil be sent at the end of this function */
        if ( (found) && (lsm_mstr_tbl_entry->LSM_obj != NULL) )
        {
          if ( (pkt_len <= DIAG_MAX_RX_PKT_SIZ) )
          {
            ...
            */
            memcpy(g_rx_buffer, req_pkt, pkt_len);
            ...
            if( return_sigs & DIAG_CMD_READ_COMPLETE_SIG )
            {
              ...
              osal_reset_sigs(diag_task_tcb,DIAG_CMD_READ_COMPLETE_SIG,&return_sigs);
            }
            else if ( return_sigs & DIAG_CMD_READ_TOUT_SIG )
            {
              osal_reset_sigs(diag_task_tcb,DIAG_CMD_READ_TOUT_SIG,&return_sigs);
              MSG(MSG_SSID_DIAG, MSG_LEGACY_ERROR, "Client stuck in reading the command");
  ...
  if(reg_tbl_is_master == TRUE)
  {
    while (TRUE)  /* repeat till registration information is completely sent */
    {
      ...
      if( rsp != NULL )
      {
        ...
        for( ; table_index < DIAGPKT_MSTR_TBL_SIZE; table_index++ )
        {
...
boolean diag_client_maskchange(int mask_type)
{
  ...
  if( DCM_table_Initialized )
  {
    ...
    while( q_item != NULL )
    {
      /* Notify user pd clients to read new masks */
      qurt_signal_set(q_item->mask_client.LSM_obj, mask_type);
...
/*============================================================================
FUNCTION DIAGPKT_MAX_DELAYED_RSP_SIZE

DESCRIPTION
   This API is used to get a maximum size of a delayed response Diag packet.

PARAMETERS
  None

RETURNS
   Returns maximum delayed response packet size in bytes.

============================================================================*/
unsigned int diagpkt_max_delayed_rsp_size(void)
{
  return (DIAG_MAX_DELAYED_RSP_SIZE);
}

/*============================================================================
FUNCTION DIAGPKT_TOTAL_DELAYED_BYTES_QUEUED

DESCRIPTION
   This API is used to get total bytes queued for delayed diag transport.

PARAMETERS
  None

RETURNS
   Returns total number of bytes queued for delayed diag transport.

============================================================================*/
unsigned int diagpkt_total_delayed_bytes_queued(void)
{
  uint32 delayed_rsp_memory_used = 0;
    osal_lock_mutex(&diag_delayed_response_mutex);
  delayed_rsp_memory_used = cur_delayed_rsp_memory_used;
    osal_unlock_mutex(&diag_delayed_response_mutex);
  return (delayed_rsp_memory_used);
}
