...
Copyright (c) 2011-2013, 2015-2018 by QUALCOMM Technologies, Incorporated.
All Rights Reserved.
...
/*===========================================================================
 ...
 $Header: //components/rel/core.mpss/10.0/services/diag/DCM/common/src/diagcomm_io.c#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
07/27/18   gn      Featurize SIO and DSM code under DIAG_SIO_SUPPORT
07/10/18   ph      Set an inactivity timer so that UART votes for sleep.
...
12/06/16   rs      removed smdl reference and changed the diag_send_buf_threshold default value to
                   DIAG_MAX_SOCKETS_DATA_BLOCK_SIZE
12/02/16   rs      removed compilation error introduced in Diag code due to smdl references
10/17/16   rs      Removed unused variables/functions and featurized variables/functions
                   used only in standalone modem to avoid compilation warnings in diag.
...
07/23/14   rh      Changed variable that is checked to determine if SMDL TX pending
...
uint32 diag_send_buf_threshold = DIAG_MAX_SOCKETS_DATA_BLOCK_SIZE; /* used to buffer up data upto the threshold value */
uint32 diag_max_send_buf_len = DIAG_MAX_TX_PKT_SIZ; /*Maximum send size*/
extern volatile diag_tx_mode_type diag_tx_mode[DIAG_MAX_STREAM_ID];
...
DESCRIPTION
  This sets the signal to open COMM channels
      ...
      break;
        ...
        tx_param->params.len, conn->channel_type, conn->io_type, conn->port_num);
          ...
          if( num_write == tx_param->params.len )
           {
            ...
            ULogFront_RealTimePrintf(diag_debug_ulog_handle, 2, "Sent %d bytes on channel %d", num_write, conn->port_num);

            if(num_write > diag_send_buf_threshold)
            {
              ERR_FATAL("Attempt to send more bytes than AP accepts, num_bytes %d port_num %d", num_write, conn->port_num, 0 );
            }
               diagbuf_clear_index(conn, tx_param);
            if (0 != retry_count)
            {
              // printing logs for every successful re-try after QNOMEM has encountered
           ...
           {     		  
            ...
            if (0 == (total_retry_count % 15))
            {
              // printing logs after every 15 re-tries for total number of retries since bootup
		  ...
		  {	   
		     ...
		     ULogFront_RealTimePrintf(diag_debug_ulog_handle, 3 ,"Closing channel with socket handle %d channel %d port %d",conn->sock_handle, conn->channel_type, conn->port_num);		   
...
void diagcomm_io_enable_flow( diagcomm_io_conn_type * conn )
{
  switch( conn->io_type )
  {
    ...
    ULogFront_RealTimePrintf(diag_err_log_handle, 3, "1: diagcomm_io_enable_flow() to check and set signal diag_prev_tx_pending %d diag_resend_data %d allow_flow %d",
      ...
      if(conn->diag_prev_tx_pending)
      {
          ...
          {
          ULogFront_RealTimePrintf(diag_err_log_handle, 3, "1: diagcomm_io_enable_flow() Signal set. diag_prev_tx_pending %d diag_resend_data %d allow_flow %d",
          diagcomm_io_conn[DIAGCOMM_PORT_SIO][DIAGCOMM_PORT_1].diag_prev_tx_pending, diagcomm_io_conn[DIAGCOMM_PORT_SIO][DIAGCOMM_PORT_1].diag_resend_data,
          diagcomm_io_conn[DIAGCOMM_PORT_SIO][DIAGCOMM_PORT_1].allow_flow );
