...
Copyright (c) 2012-2013,2016 by QUALCOMM Technologies, Incorporated.
All Rights Reserved.
...
/*===========================================================================
 ...
 $Header: //components/rel/core.mpss/10.0/services/diag/DCM/common/src/diag_dci_auth.c#1 $
...
boolean diag_dci_open_query(void)
{
   ...
   if(efs_stat(DCI_DISABLE_NV_PATH, &temp_buf) == 0)    /* 0 -- file exists   (-1)-- file does not exist*/
   {
	  ...
	  {
	     ULogFront_RealTimePrintf(diag_err_log_handle, 2, "Error returned by efs_get(). efs_errno = %d result=%d ",efs_errno,result);
      }
   }
   uint32 status;
   int i =0;
