...
$Header: //components/rel/core.mpss/10.0/services/diag/DCM/common/src/event.c#3 $
...
09/21/17   gn      Fix event drop count calculation bug
09/18/17   sa      Fixed the event stall issue when event heap is full
...
03/27/17   ph      event_drain() updates to handle PD restart.
03/08/17   ph      Handle flow control case in event_drain.
12/27/16   sa      Fixed events not being allocated in DCI stream
...
03/16/15   sr      Set version to 0 in event_q_alloc
...
11/10/14   sa      Check the status of the DCI Channel while draining events on DCI.
...
extern boolean dci_ext_header_support;
...
unsigned int event_stale_timer_len = EVENT_TIMER_LEN; /* Set default value */
unsigned int event_report_pkt_size = EVENT_RPT_PKT_SIZE;
unsigned int event_report_pkt_len_size = EVENT_RPT_PKT_LEN_SIZE;
unsigned int event_send_max_num = EVENT_SEND_MAX;
...
extern void event_service_listener_ext (event_store_type *event_store);
#endif
...
extern uint8 (*diag_time_get)(qword);
...
event_cycle (event_struct * ptr)
{
  ...
  {
    /* Event cycle ptr is NULL, so we need to drain any event sitting in queues for all PD's. */	
	event_stale(NULL);
  }
}
...
event_init (void)
{
  ...
  return_val = osal_create_timer(&event_stale_timer, diag_task_tcb, DIAG_EVENT_TIMER_SIG);
  ASSERT(OSAL_SUCCESS == return_val);

  return_val = osal_create_timer(&event_stale_timer_sec, diag_task_tcb, DIAG_EVENT_TIMER_SEC_SIG);
  ASSERT(OSAL_SUCCESS == return_val);

  /* Timer is used to notify diag that events need to be drained, if INTS are locked */
  return_val = osal_create_timer(&event_notify_timer, diag_task_tcb, DIAG_EVENT_DRAIN_SIG);
  ASSERT(OSAL_SUCCESS == return_val);

   /* Timer is used to notify diag that events need to be drained, if INTS are locked */
   return_val = osal_create_timer(&event_notify_timer_sec, diag_task_tcb, DIAG_EVENT_DRAIN_SEC_SIG);
   ASSERT(OSAL_SUCCESS == return_val);
...
event_drop_handler (event_struct * drop_ptr, uint8 payload_length)
{
  ...
  if(drop_event)
  {
      ...
      else
    {
      ...
      drop_event->stream_id = DIAG_STREAM_1;
  ...
  */
  ...
  if(drop_event)
  {
    ...
    if(event)
    {
	  ...
	  drop_event->stream_id = DIAG_STREAM_2;
...
event_rpt_prep (event_struct* ptr)
{
  ...
  if(ptr->event_rpt_last)
  {
    ...
    if(event)
    {
      ...
      event_service_listener_ext (event);
      #endif
	...
	/* Recalculate these variables if the first event is freed and moved on to next event in the queue*/
	 if(event)
	 {
          event_curr_version = event->version;       
          event_curr_param = event->id; 
...
event_rpt_prep_sec (void)
{
  ...
  if(event_rpt_last_sec)
  {
    ...
    if(event)
    {
      if(dci_ext_header_support == TRUE){
    event_curr_version = event->version;
      event_curr_param = event->id;
    }
#ifndef FEATURE_WINCE
      ...
      else
      {
        ...
        if(event != event_rpt_last_sec)
        {
          ...
          if(dci_ext_header_support && event)
          {
            if((event->version != event_curr_version) || (event->id != event_curr_param))
            {
    ...
    while((event) && (last_event != TRUE) &&
          (num_events < event_send_max_num && pkt_len < event_report_pkt_len_size))
    {
      if(dci_ext_header_support && ((event->version != event_curr_version) || (event->id != event_curr_param)))
      {
      event_pkg_count_sec = num_events;
            return pkt_len;
      }
              ...
              (current_item->mpd_buf_ptr->diagbuf_mpd_mode==DIAG_PD_BUF_MODE_STREAMING)))
   {
     ...
     if(current_drain_ptr->event_rpt_last)
     {
       ...
       if(advance_to_next_queue==FALSE)
       {
        ...
        ext_rsp.length = event_rpt_prep (current_drain_ptr);
        ...
        }// end advance_to_next_queue=false
     }
     }

     while (diagcomm_status(DIAGCOMM_PORT_1) && frag && status == DIAGBUF_READY_S && (advance_to_next_queue==FALSE))
     {
     send.pkt = (void *) frag;
...
event_drain_sec (osal_sigs_t mask)
{
  ...
  if(finished)
  {
    ...
    if(event_rpt_last_sec)
    {
      ...
      if((event->version == 0) || !(dci_ext_header_support))
      {
      hdr.cmd_code = DIAG_EVENT_REPORT_F;
      ...
      else
      {
        ...
        ext_rsp.ext_hdr.proc_id = DIAG_MY_PROC_ID;
        ext_rsp.event_cmd_code = DIAG_EVENT_REPORT_F;
        ext_rsp.length = event_rpt_prep_sec();
  ...
  while (diagcomm_status(DIAGCOMM_PORT_2) && frag && status == DIAGBUF_READY_S && diagcomm_io_conn[DIAGCOMM_PORT_SIO][DIAGCOMM_PORT_2].allow_flow == TRUE)
  {
    send.pkt = (void *) frag;
    send.last = (void *) frag_last;
...
===========================================================================*/
void diag_clear_event_stale_timer(void)
{
  int return_val = 0;
...
event_control (uint8 config, uint8 config_mask)
{
  ...
  {
    /* Calling this with NULL will to drain events frm all pds.*/
    event_stale (NULL); 
...
void event_mask_ssm()//diag_event_mask_range,num_event_code_ssm
{
  ...
  diag_update_composite_event_mask(start, end);
                                    ...
                                    boolean send_now )
{
  ...
  else if( event_mask_mstr_ptr->status == DIAG_CTRL_MASK_VALID )
  {
    ...
    (void)diagcomm_ctrl_send_mult_pkt( (void *)&ctrl_msg,
                                       DIAGCOMM_CTRL_MSG_HDR_SIZE + sizeof(diag_ctrl_msg_event_mask_v3_type),
                                       (void *)&event_mask_mstr_ptr->mask[0],
                                       event_mask_mstr_ptr->mask_len, DIAG_CTRL_USER_PKT_ID_0,
                                       port_type, port_num, send_now, &is_sent );
  }
...
void diag_ctrl_update_event_mask( diag_ctrl_msg_type * ctrl_msg )
{
  ...
  ULogFront_RealTimePrintf(diag_debug_ulog_handle, 4 , "diag_ctrl_update_event_mask: update event mask- mask_len = %d, stream_id = %d, status = %d, config = %d", mask_len, stream_id, status, config);
...
void diag_update_composite_event_mask(int start, int end)
{
  ...
  {
    temp_mask  = diag_event_listener_mask[mask_index];
    temp_mask |= diag_event_listener_ext_mask[mask_index];
...
FUNCTION EVENT_CONFIG_MASK

DESCRIPTION
  This function enables or disables the mask for an event ID passed to the
  desired stream and preset ID.

PARAMETERS
  event_id - Specifies which event ID is being referenced. Should be one of
             the enumerated event IDs defined in event_defs.h
             -1 means entire mask.
...
WARNING
  Please update composite event mask if event_config_mask() is updated.
...
boolean event_config_mask (int event_id, boolean flag, byte stream_id, uint8 preset_id)
{
  ...
  uint8 min_id_idx = 0;
  uint8 max_id_idx = 0;
  uint8 min_preset_idx = 0;
  uint8 max_preset_idx = 0;

  /* This will update entire composite event mask range */
  if( stream_id == DIAG_STREAM_ALL )
  {
    min_id_idx = DIAG_MIN_STREAM_ID;
    max_id_idx = DIAG_MAX_STREAM_ID;
  }
  else
  {
    min_id_idx = DIAG_ID_TO_INDEX(stream_id);
    max_id_idx = DIAG_ID_TO_INDEX(stream_id);
  }
  if( preset_id == DIAG_PRESET_MASK_ALL )
  {
    min_preset_idx = DIAG_ID_TO_INDEX(DIAG_MIN_PRESET_ID);
    max_preset_idx = DIAG_ID_TO_INDEX(DIAG_MAX_PRESET_ID);
  }
  else
  {
    min_preset_idx = DIAG_ID_TO_INDEX(preset_id);
    max_preset_idx = DIAG_ID_TO_INDEX(preset_id);
  }
  if (event_id == -1)
  {
    start = 0;
  ...
  else if ((event_id >=0) &&(event_id <= EVENT_LAST_ID))
  {
  ...
  for(stream_index=min_id_idx;stream_index<= max_id_idx;stream_index++)
  {
    for( preset_index=min_preset_idx; preset_index <= max_preset_idx; preset_index++ )
    {
      for( index = start; index <= end; index++ )
      {
        if(stream_index == DIAG_ID_TO_INDEX(DIAG_STREAM_1))
        {
          if(flag)
          {
            diag_event_preset_mask[preset_index][index] |= mask;
          }
          else
          {
            diag_event_preset_mask[preset_index][index] &= ~mask;
        ...
        else
        {
          if(flag)
          {
            diag_event_dci_mask[preset_index][index] |= mask;
          }
          else
          {
            diag_event_dci_mask[preset_index][index] &= ~mask;
  ...
  return_status = diag_client_maskchange(DIAG_EVENTSVC_MASK_CHANGE);
  diag_update_composite_event_mask(start, end);
