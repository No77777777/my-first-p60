...
/*===========================================================================
 ...
 $Header: //components/rel/core.mpss/10.0/services/diag/DCM/common/src/diagcomm_sio.c#2 $ $DateTime: 2020/05/27 12:41:49 $ $Author: pwbldsvc $
...
uint32 diag_dsm_drop_count_sio_cmd = 0;
...
static dsm_item_type *diagcomm_sio_inbound_cmd_ptr = NULL;
static dsm_item_type *diagcomm_sio_outbound_cmd_ptr = NULL;
static dsm_item_type *diagcomm_sio_outbound_data_ptr = NULL;
...
DESCRIPTION
  Closes the current DCI connection, first Data channel then Command channel
  so that the diag_pfds get cleaned up in a proper order.
  4th entry of Diag_pfds is for DCI Command channel and 5th for DCI Data channel

===========================================================================*/
void
diagcomm_close_dci (void)
{
 ULogFront_RealTimePrintf(diag_debug_ulog_handle, 0, "diagcomm_close_dci: Closing DCI connections"); 
...
diagcomm_status (diagcomm_enum_port_type port)
{
  return (diagcomm_io_conn[DIAGCOMM_PORT_SIO][port].connected \
  	       && diagcomm_io_conn[DIAGCOMM_PORT_SIO][port].channel_connected); 
}
...
diagcomm_inbound (unsigned int *stream_len, diagcomm_enum_port_type port_num)
{
...
#if !defined(DIAG_MP) && defined(DIAG_SIO_SUPPORT)
  if (diagcomm_sio_inbound_cmd_ptr != NULL)
  {
  ...
  {
    diagcomm_sio_inbound_cmd_ptr = diagcomm_io_dequeue_rx_wmq( diagcomm_get_sio_conn(port_num) );

    if (diagcomm_sio_inbound_cmd_ptr != NULL)
    {
      ASSERT (diagcomm_sio_inbound_cmd_ptr->data_ptr != NULL);

      *stream_len = diagcomm_sio_inbound_cmd_ptr->used;

      return_ptr = diagcomm_sio_inbound_cmd_ptr;
      diagcomm_sio_inbound_cmd_ptr = NULL;
    }
  }
...
diagcomm_inbound_complete (void *ptr)
{
  ...
  if (diagcomm_sio_inbound_cmd_ptr != NULL &&
      ptr == diagcomm_sio_inbound_cmd_ptr->data_ptr)
  {
    (void) dsm_free_packet (&diagcomm_sio_inbound_cmd_ptr);
    diagcomm_sio_inbound_cmd_ptr = NULL;
  }
...
/*===========================================================================
FUNCTION DIAGCOMM_CPY_TO_DSM

DESCRIPTION
This function creates a DSM item and copies the data from the buffer pointer
being passed in.

PARAMETERS
  *buffer         :  buffer pointer from where the data will be copied
  len             :  length of data to be copied
  is_cmd_rsp_type :  set if the data being copied is a response
...
dsm_item_type * diagcomm_cpy_to_dsm(uint8 * buffer, uint16 len, boolean is_cmd_rsp_type)
{
   ...
   dest_size = (is_cmd_rsp_type) ? DSMI_DIAG_CMD_SIO_TX_ITEM_SIZ
   ...
   if( len > dest_size )
   {
     ...
     while (bytes_to_cpy > 0)
     {
        ...
        else
        {
          if (is_cmd_rsp_type)
          {
            /* if DSM_DIAG_CMD_SIO_TX_ITEM_POOL reaches zero */
            MSG(MSG_SSID_DIAG, MSG_LEGACY_ERROR, "Ran out of DSM items in DSM_DIAG_CMD_SIO_TX_ITEM_POOL");

            /* Free any other chained parts of this DSM packet */
            dsm_free_packet (&diagcomm_sio_inbound_cmd_ptr);
          }
          else
          {
            /* if DSM_DIAG_SIO_TX_ITEM_POOL reaches zero */
            MSG(MSG_SSID_DIAG, MSG_LEGACY_ERROR, "Ran out of DSM items in DSM_DIAG_SIO_TX_ITEM_POOL");
            ASSERT(0);
          }
        }
     }
   ...
   return (is_cmd_rsp_type) ? diagcomm_sio_outbound_cmd_ptr 
                            : diagcomm_sio_outbound_data_ptr;
}
...
diagcomm_outbound_alloc (boolean is_chain, boolean is_cmd_rsp_type)
{
  ...
  dsm_mempool_id_type pool_id = 0;
  unsigned int chain_size = 0;

  if (is_cmd_rsp_type)
  {
    pool_id = DSM_DIAG_CMD_SIO_TX_ITEM_POOL;
    tmp = diagcomm_sio_outbound_cmd_ptr;
  }
  else
  {
    pool_id = DSM_DIAG_SIO_TX_ITEM_POOL;
    tmp = diagcomm_sio_outbound_data_ptr;
  }
  ...
  if (item_ptr)
  {
    ...
    if (tmp == NULL)
    {
      if (is_cmd_rsp_type)
      {
        ...
        diagcomm_sio_outbound_cmd_ptr = item_ptr;
      }
      else
      {
        diagcomm_sio_outbound_data_ptr = item_ptr;
      }
    }
    else if (is_chain)
    {
...
#ifdef DIAG_DEBUG_NON_HDLC_PARSE
typedef PACK(struct)
{
    uint8 async;
    uint8 version;
    uint16 length;
}diag_debug_non_hdlc_type;

void diag_parse_non_hdlc(uint8* buf, int length)
{
    diag_debug_non_hdlc_type* header = NULL;
    uint8 *buf_local = (uint8 *)buf;
    uint32 len=0;

    if(buf == NULL)
        return;

    ULogFront_RealTimePrintf(diag_err_log_handle, 2, "Parse start for buffer 0x%x, length is 0x%x", buf_local, length  );

    while(len < length )
    {
        header= (diag_debug_non_hdlc_type *)buf_local;

        if((header->async !=0x7E) || (header->version !=0x1) ||(header->length ==0 ))
        {
            ULogFront_RealTimePrintf(diag_err_log_handle, 0, "Bad header at the start");
            ERR_FATAL("Bad header at the start 0x%x ",header,0,0);
        }

        buf_local= buf_local + sizeof(diag_debug_non_hdlc_type) + header->length;
        len+= sizeof(diag_debug_non_hdlc_type)+ header->length;

        if(buf_local[0] !=0x7E)
        {
            ULogFront_RealTimePrintf(diag_err_log_handle, 0, "No terminating character");
            ERR_FATAL("No terminating character %x ",buf_local,0,0);
        }
        buf_local++;
        len++;

    }
    ULogFront_RealTimePrintf(diag_err_log_handle, 0, "All data is proper");
}
#endif
              ...
              diag_tx_mode_type *tx_mode_ptr )
{
   ...
   else
   {
      ...
      /* If diagcomm_cpy_to_dsm returned NULL, we have run
        out of DSMs, so return without sending anything */
      if (tx_params.dsm_ptr == NULL)
      {
         return;
      }
#else
   ...
   if (is_cmd_rsp_type)
   {
     diagcomm_sio_outbound_cmd_ptr = NULL;
   }
   else
   {
     diagcomm_sio_outbound_data_ptr = NULL;
   }
#endif /* !defined(DIAG_MP) && defined(DIAG_SIO_SUPPORT) */
}
...
DESCRIPTION
   ...
   for DSM_DIAG_SIO_TX_ITEM_POOL is reached.(DSM_DIAG_SIO_TX_ITEM_POOL is
   used by sio, to receive data.)
