...
Copyright (c) 2001 - 2015, 2018 by QUALCOMM Technologies, Incorporated.  All Rights Reserved.
*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*/
...
$Header: //components/rel/core.mpss/10.0/services/diag/DCM/common/src/toolsdiag.c#1 $ $DateTime: 2019/04/24 00:03:26 $ $Author: pwbldsvc $


when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
01/29/18   kdey    Detached phChipInfo in diag_hw_partnum_version()
...
/* Store the registered function to call to get cdma info */
diag_get_cur_cdma_info_cb_type diag_get_cur_cdma_info_fp = NULL;
...
void * toolsdiag_verno (void * req_pkt, uint16 pkt_len)
{
  ...
  {   /* Format response packet with version, date and time. */
      ...
      if (diag_get_cur_cdma_info_fp != NULL) {
          (*diag_get_cur_cdma_info_fp)(&scm,&mob_cai_rev,&sci_p);
      }
#endif /* DIAG_IS2000 */
