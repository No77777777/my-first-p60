...
Copyright (c) 1990-2018 by QUALCOMM Technologies, Incorporated.  All Rights Reserved.
*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*/
...
$Header: //components/rel/core.mpss/10.0/services/diag/DCM/common/src/diaglog.c#1 $
...
NOTE:  removed both the arguments 'diag_mask_ssm_buffer_type* diag_log_codes_range'
       and 'uint32 size' of this function as we did it for event_mask_ssm() to
     avoid race condition in diag task and event_mask_ssm.

       Now, directly using the global variables 'diag_mask_ssm_buffer_type *diag_log_mask_range'
     and 'uint32 num_log_code_ssm' for same purpose
=================================================================================================*/
void log_mask_ssm()
{
  int i=0;
  ...
  diag_mask_ssm_buffer_type *temp_diag_log_mask_range = diag_log_mask_range;
  if ((temp_diag_log_mask_range == NULL) || (num_log_code_ssm == 0))
  {
    osal_unlock_mutex(&diag_log_mask_ssm_mutex);
  return;
  }

  for(i=0;i<num_log_code_ssm;i++)
  {
    if(temp_diag_log_mask_range->permission == check_perm_value)
   {
/* Get ID and item from the log code start*/
      code_start = (log_code_type) temp_diag_log_mask_range->start;
    code_end = (log_code_type) temp_diag_log_mask_range->end;
    ...
    temp_diag_log_mask_range++;
  }
   free (diag_log_mask_range);
  diag_log_mask_range = NULL;
  temp_diag_log_mask_range = NULL;
   num_log_code_ssm = 0;
  osal_unlock_mutex(&diag_log_mask_ssm_mutex);
  log_update_composite_mask(start, end);
  return;
...
{
  const diag_log_rsp_type *log = NULL;
  log_code_type code;
  ...
  if (desc)
  {
    ...
    if (state == DIAG_SEND_STATE_START)
    {
      ...
      else
      {
		...
		/* This is to make sure that in case of chained log packets, when this function is called for the first chained packet
the listener gets called before the control moves to diagbuf_drain_diag. Otherwise the subid field in the first chained
log packet will be getting cleared. In such a case when listener is called the subid will be zero.*/
		if(diagbuf_hdr1->version == DIAGBUF_VERSION_0)
		{
			log = (diag_log_rsp_type *) desc->pkt;
		}
		else if((diagbuf_hdr1->version == DIAGBUF_VERSION_1 ) && (diagbuf_hdr1->chain == DIAGBUF_FIRST_PKT_CHAIN))
		{
			addr = (byte*)diagbuf_hdr1;
			addr = addr + sizeof(diagbuf_header_type); //moving past the diagbuf header
			/*At this point we are at the 1st byte after the diagbuf header */
			addr = addr + sizeof(log_ext_hdr_type); //moving past the log_ext_hdr_type
			/*At this point we are at the 1st byte after the log_ext_hdr_type, i.e. 0xDE of 0xDEAD */
			addr = addr + sizeof(uint32); //moving past the DEAD pattern and 2 bytes of padding
			/*At this point we are at the 1st byte of the diagbuf header of the second chained extended log packet */
			addr = addr + sizeof(diagbuf_header_type); //moving past the diagbuf header of the second chained packet
			log = (diag_log_rsp_type *)addr;
		}
		else
		{
			log = NULL;
		...
		if(log) //if log is NULL then either it is a version 2 log packet or a second chained log packet of version 1
		{
      ...
      stream_check = (stream_id & (DIAG_STREAM_1 | DIAG_STREAM_2));}
      else{
      stream_check = (stream_id&DIAG_STREAM_1);}
...
===========================================================================*/
log_code_type log_set_equip_id (
  unsigned int code,
                                  ...
                                  boolean send_now )
{
      ...
      (log_mask_mstr_ptr->status == DIAG_CTRL_MASK_ALL_ENABLED) )
  {
    ...
    (void)diagcomm_ctrl_send_mult_pkt( (void *)&ctrl_log_mask,
    ...
    for( equip_id = 0; equip_id <= LOG_EQUIP_ID_MAX; equip_id++ )
    {
      if( update_all || (mask_status_ptr[equip_id] == TRUE) )
      {
        if( diag_log_last_item_mstr_tbl[equip_id] > 0 )
        {
          ...
          (void)diagcomm_ctrl_send_mult_pkt( (void *)&ctrl_log_mask,
                                             DIAGCOMM_CTRL_MSG_HDR_SIZE + sizeof(diag_ctrl_msg_equip_log_mask_v3_type),
                                             (void *)&log_mask_mstr_ptr->mask[equip_id][0],
                                             ctrl_log_mask.data.ctrl_msg_equip_log_mask_v3.log_mask_size,
...
void diag_ctrl_update_log_mask( diag_ctrl_msg_type * ctrl_msg )
{
  ...
  ULogFront_RealTimePrintf(diag_debug_ulog_handle, 4 , "diag_ctrl_update_log_mask: update log mask - mask_len = %d, stream_id = %d, status = %d, equip_id = %d", mask_len, stream_id, status, equip_id);
