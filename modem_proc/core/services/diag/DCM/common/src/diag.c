...
Copyright (c) 1992-2018, 2020 by QUALCOMM Technologies, Incorporated.
All Rights Reserved.
...
/*===========================================================================
 ...
 $Header: //components/rel/core.mpss/10.0/services/diag/DCM/common/src/diag.c#3 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
12/13/18   ph      Enable diag boot up logging.
11/28/18   vg      Supporting SubID in extended logs and msgs listener callback
09/18/18   kdey    Removed signaling diag task at island exit callback.
09/03/18   gn      Diag timers categorization into deferrable and non-deferrable
08/23/18   gn      Fixed the issue of sending error responses to DCI commands
08/06/18   ph      Query SSM everytime before opening or closing the DCI channels.
...
06/12/18   gn      Send error response in HDLC format when CRC decode failures are observed.
05/31/18   kdey    Supporting SubID in extended event listener callback
06/13/18   gn      Update master table in standalone configuration during DIAG_INT_CTRL_REG_SIG handling
05/25/18   gn      Read NV item value using diag_nv_read (non-blocking)
04/23/18   gn      Command registration enhancement
04/11/18   kdey    Added fix for graceful handling of scenario where mutexes are accessed before initialization
02/11/18   sa      Added check for f3 trace status in diag_services_enabled.
08/01/17   ph      Added new API diag_services_enabled().
11/29/17   kdey    Fixed bootup crash when diag_init_large_buffer() takes time to complete, preventing mutex initializations in diag_early_init() 
11/20/17   nk      Fix bootup crash when clients call diag_early_init() before the diag task is initialized
08/14/17   sa      Resend the data in diagbuf_send_ctrl buf when io_transmit fails.
08/16/17   ph      Remove sleep voting in data channel path.
09/28/17   sa      Handle NULL ptr dereference when diagID exchange is pending 
09/18/17   nk      Fixed event stalL issue when event heap gets full.
08/29/17   gn      Changes to handle remote socket close
08/01/17   gn      Added support to disable DCI when Apps doesnt open DCI channels
06/19/17   ph      Tagging the data with dynamic diag IDs.
07/12/17   sp      Fix issue with diag command response timeout due to CRC mismatch for huge inbound RX packet
05/08/17   ph      Enable and disable sleep vote in case of flow control.
...
03/20/17   is      Add a vote against sleep before calling diagpkt_rsp_send()
03/08/17   rs      removed the local arguments of log_mask_ssm() and used global shared variable to avoid race condition
                   between diag task and log_mask_ssm().
03/08/17   ph      Handle flow control case in event_drain.
02/21/17   ph      Tag DIAG ID header to all the diag packets sent out.
03/02/17   sp      Added retry mechanism to pet the watchdog to avoid infinite wait on qpoll
02/07/17   sa      DiagID support added
01/25/17   rs      Re-try sending control packets using bit mask and signal and by-passing the control heap usage.
01/23/17   sa      Remove micro_diag_uimage_tout_timer while exiting uimage
...
11/08/16   tbg     Relocated qshrink4 init to its own function
...
10/06/16   gn      Changes to allocate large memory by DHMS
08/26/16   sp      Initialize diagbuf_mpd_q before Diag registration with QDI. Also cleanup unused mutex
08/26/16   gn      Fixed issue of Masks not being cleared on Listener removal
08/26/16   gn      Fix for DCI_Override issue
06/30/16   nk      SMD References Removal
07/01/16   rs      Made diag_first_pkt variable gloabal so that it can be accessed by diag_unified_script.cmm for debugging.
                   Also, changed it's datatype from integer to boolean.
...
05/20/15   rh      Fixed compiler warning
...
01/05/15   is      Make drain timers deferable
...
08/01/14   rh      Moved confirmation of Diag initialization to after Diag inits.
07/08/14   is      Remove featurization around diagpkt_delayrspid_mutex_init()
...
08/01/13   sa      Updated setting of DIAG_SMDL_WRTIE_SIG only if items are pending
...
#include "rcevt.h"
#include "diagpkti.h"
#include "rcinit_rex.h" /* for rcinit_lookup_rextask() */
#include "time_svc.h" /* for time_get, time_event_system_time_update */
...
static boolean diag_srch_list_mutex_initialized = FALSE;
static boolean diag_idle_process_mutex_initialized= FALSE;
static boolean diag_internal_sig_mutex_initialized= FALSE;
static int diag_srch_list_mutex_init_begin = FALSE;
static int diag_idle_process_mutex_init_begin= FALSE;
static int diag_internal_sig_mutex_init_begin= FALSE;
...
ULogHandle diag_err_log_handle;
/* ULog handle for debug information */
ULogHandle diag_debug_ulog_handle;
/* ULog handle for command/response and signals */
ULogHandle diag_cmd_ulog_handle;
/* ULog handle for log draining and transmit */
ULogHandle diag_data_ulog_handle;
...
/* boot up log timer callback function that clears the boot up settings */
...
/* Timer used to clear all the F3/Log/Events masks if enabled during
   DIAG initialization */
osal_timer_t diag_early_log_timer;
...
extern uint8 diag_qshrink4_guid_count;
...
/* This timer is used to notify that we should start draining micro diagbuffer. The timer is used in island exit callback*/
#ifdef DIAG_UIMAGE_MODE
osal_timer_t diag_micro_diagbuffer_drain_notify_timer;
#endif
...
extern void diag_qdi_register(void); /* From diag_qdi.c */

extern void diag_qshrink4_init(void); 
...
extern void diag_send_buf_retry(diagcomm_io_channel_type ch_type, diagcomm_enum_port_type port_type, diag_tx_mode_type* tx_mode_ptr);
...
static void diag_process_input_file (diag_fs_type * fs,void *buf,
                  unsigned int size);
extern void diag_early_log_nv_config_create(void);
extern boolean diag_check_early_log_nv_enabled(void);
...
typedef struct
{
  ...
  unsigned int length;
  byte req[1];             /* sized by the 'length' field */
}
diag_req_cmd_type;
...
extern void diag_read_early_devcfg(void);
extern void diag_early_log_enable(uint8 config);
...
boolean diag_extract_pkt (diag_hdlc_decode_type * hdlc);
...
/* Boolean that holds whether NV item for early logging is enabled */
boolean diag_early_log_nv_enabled = FALSE;
...
osal_mutex_arg_t diag_idle_process_mutex;
 ...
 void diag_search_list_cs_init(void)
 {
    ...
    if(!osal_atomic_compare_and_set((osal_atomic_word_t*)&diag_srch_list_mutex_init_begin, FALSE, TRUE))
		return;
	...
	diag_srch_list_mutex_initialized = TRUE;
 }


void diag_idle_process_mutex_init(void)
{
    int return_val;

	if(!osal_atomic_compare_and_set((osal_atomic_word_t*)&diag_idle_process_mutex_init_begin, FALSE, TRUE))
		return;
	/*initialize the mutex used while acessing diag_idl_proc_tbl*/
	 ...
	 diag_idle_process_mutex_initialized= TRUE;
 ...
 void diag_int_sig_cs_init(void)
 {
    int return_val;
	  ...
	  if(!osal_atomic_compare_and_set((osal_atomic_word_t*)&diag_internal_sig_mutex_init_begin, FALSE, TRUE))
			return;
    diag_internal_sig_mutex.name = "MUTEX_DIAG_INT_SIG_CS";
    return_val = osal_init_mutex(&diag_internal_sig_mutex);
    ASSERT(OSAL_SUCCESS == return_val);
	  diag_internal_sig_mutex_initialized= TRUE; 
 }
...
typedef enum
{
  ...
  /*REmoving this as this part of switch is removed from the function*/
...
boolean diag_dci_is_enabled(void);
...
void diag_uimage_entry_exit_cb(uSleep_state_notification notification_state)
{
  ...
  {
    int return_val = 0;
    /*Setting the timer on whose expiry DIAG_MICRO_DIAGBUFFER_UIMAGE_SIG will be set. */
    return_val = osal_set_timer(&diag_micro_diagbuffer_drain_notify_timer,DIAG_MICRO_DIAGBUFFER_DRAIN_TIMER_LEN);
    ASSERT(OSAL_SUCCESS == return_val);
  }
  else
  {
...
diag_event_malloc (unsigned long num_bytes,event_struct *ptr)
{
  ...
  if(ptr!=NULL)
  {
    ...
    if (buf == NULL)
    {
      ...
      /* Drain the events since the heap is full */
      event_stale(NULL);
    }
    else
    {
...
diag_event_malloc_sec (unsigned long num_bytes)
{
  ...
  */
  ...
  if (buf == NULL)
  {
    ...
    MSG_1(MSG_SSID_DIAG, MSG_LEGACY_LOW,"diag_event_heap_malloc_sec allocation failed: %d",num_bytes);
    /* Drain the events since the heap is full */
    event_stale_sec();
  ...
  else
  {
    ...
    MSG_2(MSG_SSID_DIAG, MSG_LEGACY_HIGH,
    " <<DIAG MALLOC diag_event_malloc_success_cnt = %d,  event_heap used = %d ",
...
void diag_early_init(void)
{
  ...
  return_val = osal_create_timer_ex(&diag_mask_change_notify_timer, diag_task_tcb, diag_mask_notify_timer_cb, 0);
  ASSERT(OSAL_SUCCESS == return_val);
...
diag_init (void)
{
  ...
  /* Initialize diagpkt_master_table_q and diagpkt_mask_client_list_q */
  ...
  if(diag_task_tcb)
  {
      return_val = osal_create_diag_timer(&diag_cmd_read_tout_timer, diag_task_tcb, DIAG_CMD_READ_TOUT_SIG);
      ASSERT(OSAL_SUCCESS == return_val);

      return_val = osal_create_timer(&diag_tout_timer, diag_task_tcb, DIAG_TOUT_TIMER_SIG);
      ASSERT(OSAL_SUCCESS == return_val);

      /* Timer used to notify DIAG that TX is pending from a client. */
      return_val = osal_create_timer(&diag_tx_notify_timer, diag_task_tcb, DIAG_TX_SIG);
      ASSERT(OSAL_SUCCESS == return_val);

      /* Timer used to notify DIAG that TX is pending from a client. */
      return_val = osal_create_timer(&diag_drain_timer, diag_task_tcb, DIAG_DRAIN_TIMER_SIG);
      ASSERT(OSAL_SUCCESS == return_val);
      ...
      return_val = osal_create_timer(&diag_sio_tout_timer, diag_task_tcb, DIAG_SIO_TOUT_TIMER_SIG);
      ASSERT(OSAL_SUCCESS == return_val);

     #ifdef DIAG_UIMAGE_MODE
     return_val = osal_create_timer(&diag_micro_diagbuffer_drain_notify_timer, diag_task_tcb, DIAG_MICRO_DIAGBUFFER_UIMAGE_SIG);
     ASSERT(OSAL_SUCCESS == return_val);
     #endif
      ...
      return_val = osal_create_diag_timer_ex(&diag_early_log_timer, diag_task_tcb, diag_early_log_timer_cb, 0);
      ASSERT(OSAL_SUCCESS == return_val);
  ...
  */
...
#if !defined (FEATURE_WINCE) && defined(DIAG_SIO_SUPPORT)
  diagdsm_init();
#endif
  ...
  micro_diagbuffer_create( (micro_ULogHandle *)(&uimage_diagbuffer), "diag_uimag_buffer", uimage_buffer, &uimage_bufUsed, UIMAGE_DIAGBUFFER_SIZE, MICRO_DIAGBUFFER_LOCKABLE );
#endif
#ifdef FEATURE_WINCE
#error code not present
#endif
  ...
  /* Read the masks of DEVCFG and apply the masks */
  diag_read_early_devcfg();

#if defined(DIAG_FEATURE_EFS2)
  diag_early_log_nv_config_create();
  
  /* Check if boot up logging NV is enabled */
  diag_early_log_nv_enabled = diag_check_early_log_nv_enabled();
  if(diag_early_log_nv_enabled)
  {
    ULogFront_RealTimePrintf(diag_data_ulog_handle, 0, "Enabling diag early logging via NV ");
    diag_early_log_enable(DIAG_EARLY_LOG_CONFIG_SRC_NV);
  }
...
#else
   /* Set DIAG_COMM_OPEN_SIG to diag task to open communication channels */
   diagcomm_open();
   #endif
...
#if !defined  (FEATURE_WINCE)
   MSG (MSG_SSID_DIAG, MSG_LEGACY_HIGH, "DIAG task initialized");
#else
  #error code not present
#endif
...
diag_process_rx (void)
{
  ...
  if (FALSE == diag_first_pkt)
  {
    diag_tx_notify();
    diag_first_pkt = TRUE;
	ULogFront_RealTimePrintf(diag_debug_ulog_handle, 1, "diag_process_rx: diag_first_pkt (First packet received): %d", diag_first_pkt
);
  }
...
void diag_process_rx_dci (void)
{
    ...
    if(diag_get_rx_pkt_slave(&rx_pkt) == TRUE)
    {
      ...
      /* If command is greater than or equal to 4 bytes, logging first four bytes */
      if (actual_cmd_length >= 4)
      {
        ULogFront_RealTimePrintf(diag_cmd_ulog_handle, 5, "diag_process_rx_dci: DCI command received, Tag: %d, command : 0x%02X, 0x%02X, 0x%02X, 0x%02X",\
                                 diag_dci_cmd_tag, rx_dci->command[0], rx_dci->command[1], rx_dci->command[2], rx_dci->command[3]);
      }
      /* If command is of less than 4 bytes, logging just the first byte (as the first byte is the command code) */
      else
      {
        ULogFront_RealTimePrintf(diag_cmd_ulog_handle, 2, "diag_process_rx_dci: DCI command received, Tag: %d, command : 0x%02X", \
                                 diag_dci_cmd_tag, rx_dci->command[0]);
      }
...
{	
   /*Check if any transcation to resend the ctrl packet from diag_ctrl_send_buf failed. Send the data in send buffer now. */
   uint8 port_num;
   boolean is_sent;
   ...
   {
     if(diagcomm_io_ctrl_conn[DIAGCOMM_PORT_SMD][port_num].diag_prev_tx_pending==TRUE)
	 	 (void)diagcomm_ctrl_send_mult_pkt( NULL,0,NULL, 0, DIAG_CTRL_USER_PKT_ID_0, DIAGCOMM_PORT_SMD, (diagcomm_enum_port_type)port_num, TRUE, &is_sent );
   }
#else
	for( port_num=0; port_num < NUM_SIO_PORTS; port_num++ )
   {
     if(diagcomm_io_ctrl_conn[DIAGCOMM_PORT_SIO][port_num].diag_prev_tx_pending==TRUE)
	 	(void)diagcomm_ctrl_send_mult_pkt( NULL,0,NULL, 0, DIAG_CTRL_USER_PKT_ID_0, DIAGCOMM_PORT_SIO, (diagcomm_enum_port_type)port_num, TRUE, &is_sent );
   }
#endif

   /* If the previous data in send buffer is send, check if any feature bit is set and send the packet */
    ...
    if (DIAG_IS_CTRLMASK_BIT_SET(DIAG_CTRL_MSG_DEREG_BIT_POS))
    {
				  DIAG_CLEAR_CTRLMASK_BIT(DIAG_CTRL_MSG_DEREG_BIT_POS); 		  
#if defined(DIAG_MP) && !defined(DIAG_MP_MASTER)
    ...
    if (DIAG_IS_CTRLMASK_BIT_SET(DIAG_CTRL_PD_STATUS_BIT_POS))
    {
          int index = 0;
          int guid_count = diag_qshrink4_guid_count;

				  DIAG_CLEAR_CTRLMASK_BIT(DIAG_CTRL_PD_STATUS_BIT_POS); 					  

				  for (index = 0; index < guid_count; index++)
				  { 
				    if (diag_qshrink_database_file_list[index].updated == FALSE)
				    {
			          diag_ctrl_send_pd_status(diag_qshrink_database_file_list[index].pid, DIAG_CTRL_PD_STATUS_UP);
				      diag_qshrink_database_file_list[index].updated = TRUE;
				    }
				  }
		}
    if (DIAG_IS_CTRLMASK_BIT_SET(DIAG_CTRL_MSG_PERIPH_BUF_BIT_POS))
    {
     	     	  DIAG_CLEAR_CTRLMASK_BIT( DIAG_CTRL_MSG_PERIPH_BUF_BIT_POS);
#if defined(DIAG_MP) &&!defined(DIAG_MP_MASTER)
	  ...
	  {
	    /* If no data is pending in the ctrl send buffer, and no feature bit is also set, then send error */
...
void diag_handle_internal_sigs( void )
{
  ...
  /* Filtering out both the internal signals, since their occurrence is so frequent. */
  if ((sigs_internal != DIAG_INT_RESEND_DATA_SIG) && (sigs_internal != DIAG_INT_DRAIN_DSM_BUFFER_SIG)) 
    ULogFront_RealTimePrintf(diag_cmd_ulog_handle, 1, "diag_handle_internal_sigs: sigs_internal = 0x%X", sigs_internal);
  ...
  else if (sigs_internal &   DIAG_INT_RESEND_CTRL_SIG)
  {
    ...
    {
		ULogFront_RealTimePrintf(diag_data_ulog_handle, 3, "diag_handle_internal_sigs: DIAG_INT_RESEND_CTRL_SIG - diag_resend_data = %d, diag_prev_tx_pending = %d, allow_flow = %d",\
				 diagcomm_io_ctrl_conn[DIAGCOMM_PORT_SIO][DIAGCOMM_PORT_1].diag_resend_data, diagcomm_io_ctrl_conn[DIAGCOMM_PORT_SIO][DIAGCOMM_PORT_1].diag_prev_tx_pending, diagcomm_io_ctrl_conn[DIAGCOMM_PORT_SIO][DIAGCOMM_PORT_1].allow_flow);
        diag_send_buf_ctrl_retry();
  ...
  else if( sigs_internal & DIAG_INT_CTRL_REG_SIG )
  {
    ...
    /* Extracts items from diagpkt_tbl_reg_q and appends to diagpkt_master_table */
    diagpkt_update_master_table_reg();

#if defined(DIAG_MP) &&!defined(DIAG_MP_MASTER)
    /* Check if control is connected, flow is enabled on control channel, if diagID is supported make sure that 
    diagID is valid */
   ...
   else if( sigs_internal & DIAG_INT_CMD_DEREG_SIG )
  {
    ...
    /* Extracts items from diagpkt_tbl_dereg_q and mark corresponding entries in 
     diagpkt_table_fwd_status as deregistered*/
    diagpkt_update_master_table_dereg();
    ...
    {
        resend_sig_mask |=  DIAG_INT_CMD_DEREG_SIG;
    }
#endif
  }
  ...
  else if (sigs_internal & DIAG_INT_RESEND_DATA_SIG)
  {
      ...
      (diagcomm_io_conn[DIAGCOMM_PORT_SIO][DIAGCOMM_PORT_1].diag_resend_data == TRUE))
    {
      ...
      ULogFront_RealTimePrintf(diag_data_ulog_handle, 3, "diag_handle_internal_sigs: DIAG_INT_RESEND_DATA_SIG - diag_resend_data = %d, diag_prev_tx_pending = %d, allow_flow = %d",\
				 diagcomm_io_conn[DIAGCOMM_PORT_SIO][DIAGCOMM_PORT_1].diag_resend_data, diagcomm_io_conn[DIAGCOMM_PORT_SIO][DIAGCOMM_PORT_1].diag_prev_tx_pending, diagcomm_io_conn[DIAGCOMM_PORT_SIO][DIAGCOMM_PORT_1].allow_flow);	  
      ...
      (diagcomm_io_conn[DIAGCOMM_PORT_SIO][DIAGCOMM_PORT_2].diag_resend_data == TRUE))
    {
	    ...
	    ULogFront_RealTimePrintf(diag_data_ulog_handle, 3, "diag_handle_internal_sigs: DIAG_INT_RESEND_DATA_SIG - diag_resend_data = %d, diag_prev_tx_pending = %d, allow_flow = %d",\
				 diagcomm_io_conn[DIAGCOMM_PORT_SIO][DIAGCOMM_PORT_2].diag_resend_data, diagcomm_io_conn[DIAGCOMM_PORT_SIO][DIAGCOMM_PORT_2].diag_prev_tx_pending, diagcomm_io_conn[DIAGCOMM_PORT_SIO][DIAGCOMM_PORT_2].allow_flow);		
        
        /* Retry data in local DIAGID instance for stream2 */
        tx_mode_ptr = diag_get_tx_mode_ptr(DIAG_STREAM_2,diag_get_diagID());
        diag_send_buf_retry(DIAGCOMM_IO_DATA,DIAGCOMM_PORT_2, tx_mode_ptr);
  ...
  else if (sigs_internal & DIAG_INT_RESEND_CMD_SIG)
  {
    ...
    (diagcomm_io_cmd_conn[DIAGCOMM_PORT_SIO][DIAGCOMM_PORT_1].diag_resend_data == TRUE))
    {
	  ...
	  ULogFront_RealTimePrintf(diag_data_ulog_handle, 3, "diag_handle_internal_sigs: DIAG_INT_RESEND_CMD_SIG - diag_resend_data = %d, diag_prev_tx_pending = %d, allow_flow = %d",\
				 diagcomm_io_cmd_conn[DIAGCOMM_PORT_SIO][DIAGCOMM_PORT_1].diag_resend_data, diagcomm_io_cmd_conn[DIAGCOMM_PORT_SIO][DIAGCOMM_PORT_1].diag_prev_tx_pending, diagcomm_io_cmd_conn[DIAGCOMM_PORT_SIO][DIAGCOMM_PORT_1].allow_flow);		  
      ...
      (diagcomm_io_cmd_conn[DIAGCOMM_PORT_SIO][DIAGCOMM_PORT_2].diag_resend_data == TRUE))
    {
	  ...
	  ULogFront_RealTimePrintf(diag_data_ulog_handle, 3, "diag_handle_internal_sigs: DIAG_INT_RESEND_CMD_SIG - diag_resend_data = %d, diag_prev_tx_pending = %d, allow_flow = %d",\
				 diagcomm_io_cmd_conn[DIAGCOMM_PORT_SIO][DIAGCOMM_PORT_2].diag_resend_data, diagcomm_io_cmd_conn[DIAGCOMM_PORT_SIO][DIAGCOMM_PORT_2].diag_prev_tx_pending, diagcomm_io_cmd_conn[DIAGCOMM_PORT_SIO][DIAGCOMM_PORT_2].allow_flow);		  
      diag_send_buf_retry(DIAGCOMM_IO_CMD,DIAGCOMM_PORT_2, NULL);
  ...
  else if(sigs_internal & DIAG_INT_CLOSE_DCI_CHANNEL_SIG)
  {
    ...
    if (!is_dci_allowed)
    {
      MSG_1 (MSG_SSID_DIAG, MSG_LEGACY_HIGH, "Close the DCI channel %d", 0 );
      ULogFront_RealTimePrintf(diag_err_log_handle, 0, "Close the DCI channel when DIAG_INT_CLOSE_DCI_CHANNEL_SIG is set ");
      /*Close DCI stream*/
      diagcomm_close_dci();   
    }
    #endif
  ...
  else if( sigs_internal & DIAG_INT_DRAIN_DSM_BUFFER_SIG )
  {
    ...
    while(diagid_ptr !=NULL)
    {
      ...
      if(tx_mode_ptr)
      {  
    diag_drain_dsm_buffer( DIAG_STREAM_1, tx_mode_ptr );
  }
      diagid_ptr=diagid_ptr->next;
    }
  }
  else if( sigs_internal & DIAG_INT_DRAIN_DCI_DSM_BUFFER_SIG )
  {
    ...
    /*Drain local DIAGID traffic of stream2*/
    tx_mode_ptr = diag_get_tx_mode_ptr(DIAG_STREAM_2,diag_get_diagID());
    diag_drain_dsm_buffer( DIAG_STREAM_2, tx_mode_ptr );
  }
  else if (sigs_internal & DIAG_INT_UPDATE_PD_STATUS)
  {
    int index = 0;
    int guid_count = diag_qshrink4_guid_count;
    diag_clear_internal_sigs(DIAG_INT_UPDATE_PD_STATUS);
...
diag_handle_sigs (osal_sigs_t mask)
{
  ...
  osal_sigs_t busy_mask = mask |
      ...
      DIAG_EVENT_TIMER_SIG |DIAG_EVENT_TIMER_SEC_SIG | DIAG_EVENT_DRAIN_SEC_SIG | DIAG_TASK_STOP_SIG | DIAG_OPEN_DCI_CHANNEL_SIG | DIAG_RX_DELAY_RSP_SIG ;
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  while (!found)
  {
	...
	/* Filtering out printing signal mask value set by ULog handler command for retreiving ULogs, since it occurs too frequently */
    if(*(diag_req_inbound + 1) != DIAG_SUBSYS_ULOG) {
			ULogFront_RealTimePrintf(diag_cmd_ulog_handle, 1, "diag_handle_sigs: sigs = 0x%X", sigs);
	}
...
#if (defined(FEATURE_SSM))
            /* Query SSM to check whether DCI is allowed */
            is_dci_allowed = diag_dci_open_query();
            if (is_dci_allowed && diag_dci_is_enabled())
            {
              MSG_1(MSG_SSID_DIAG, MSG_LEGACY_HIGH, "Open the DCI channel %d", 1);
              ULogFront_RealTimePrintf(diag_err_log_handle, 0, "Opening the DCI channel when DIAG_CHANGE_CONN_SIG is set");
  ...
  diagcomm_smd_close( DIAGCOMM_PORT_2 );
      ...
      else if (sigs & DIAG_COMM_OPEN_SIG)
      {
        ...
        if(TRUE == diagcomm_io_socket_check_support(&diagcomm_io_ctrl_conn[DIAGCOMM_PORT_SIO][DIAGCOMM_PORT_1]))
        {
           ...
           if (is_dci_allowed && diag_dci_is_enabled())
           {
             MSG_1 (MSG_SSID_DIAG, MSG_LEGACY_HIGH, "Open the DCI channel %d", 2);
             ULogFront_RealTimePrintf(diag_err_log_handle, 0, "Open the DCI channel when DIAG_COMM_OPEN_SIG is set");
             diagcomm_cmd_open (DIAGCOMM_PORT_SIO, DIAGCOMM_PORT_2);
             diagcomm_sio_open (DIAGCOMM_PORT_2); /* Open the DCI stream */
           }
#endif
    ...
    {
      return_val = osal_reset_sigs(diag_task_tcb, DIAG_REQUEST_DIAGID_SIG, &return_sigs);
      ASSERT(OSAL_SUCCESS == return_val);
...
#else
      if ((DIAG_IS_INT_FEATURE_BIT_SET(diagcomm_io_ctrl_conn[DIAGCOMM_PORT_SIO][DIAGCOMM_PORT_1].feature_mask,F_DIAG_DIAGID_SUPPORT)))
#endif
      {
      ...
      else if((sigs & DIAG_RX_RSP_SIG)!= 0)
      {
        ...
        {
           status = diagcomm_cmd_status(DIAGCOMM_PORT_1);
        }
        else
        {
           status = diagcomm_status(DIAGCOMM_PORT_1);
     ...
     return_val = osal_reset_sigs(diag_task_tcb, DIAG_OPEN_DCI_CHANNEL_SIG, &return_sigs);
	   ...
	   if(is_dci_allowed && diag_dci_is_enabled())
	   {
          ULogFront_RealTimePrintf(diag_err_log_handle,0,"Open the DCI channels as DIAG_OPEN_DCI_CHANNEL_SIG is set");
       if (diagcomm_io_cmd_conn[DIAGCOMM_PORT_SIO][DIAGCOMM_PORT_2].connected == FALSE)
       {
            MSG(MSG_SSID_DIAG, MSG_LEGACY_HIGH, "Handling DIAG_OPEN_DCI_CHANNEL_SIG:Open the DCI command channel");
         diagcomm_cmd_open (DIAGCOMM_PORT_SIO, DIAGCOMM_PORT_2);
       }
     if (diagcomm_io_conn[DIAGCOMM_PORT_SIO][DIAGCOMM_PORT_2].connected == FALSE)
       {
            MSG(MSG_SSID_DIAG, MSG_LEGACY_HIGH, "Handling DIAG_OPEN_DCI_CHANNEL_SIG: Open the DCI data channel");
          diagcomm_sio_open (DIAGCOMM_PORT_2); 
         }
       }
#endif	   
      }
      ...
      else if (sigs & DIAG_DRAIN_TIMER_SIG)
      {
             ...
             (TRUE == diagcomm_io_conn[DIAGCOMM_PORT_SIO][DIAGCOMM_PORT_2].allow_flow))
        {
          ...
          status = diagbuf_drain(DIAG_TASK_STOP_SIG | mask | DIAG_RX_SIG | DIAG_RX_RSP_SIG | DIAG_OPEN_DCI_CHANNEL_SIG | DIAG_EVENT_DRAIN_SIG | DIAG_EVENT_DRAIN_SEC_SIG | DIAG_COMM_SIO_TX_FLUSH_SIG |DIAG_REQUEST_DIAGID_SIG |DIAG_CTRL_MSG_SIG);
      ...
      else if (sigs & DIAG_TX_SIG)
      {
        ...
        if( (TRUE == diagcomm_io_conn[DIAGCOMM_PORT_SIO][DIAGCOMM_PORT_1].allow_flow) &&
             (TRUE == diagcomm_io_conn[DIAGCOMM_PORT_SIO][DIAGCOMM_PORT_2].allow_flow))
        {
           status = diagbuf_drain (DIAG_TASK_STOP_SIG | mask | DIAG_RX_SIG | DIAG_RX_RSP_SIG | DIAG_OPEN_DCI_CHANNEL_SIG | DIAG_EVENT_DRAIN_SIG | DIAG_COMM_SIO_TX_FLUSH_SIG | DIAG_REQUEST_DIAGID_SIG |DIAG_CTRL_MSG_SIG);
      ...
      else if (sigs & DIAG_MICRO_DIAGBUFFER_UIMAGE_SIG)
      {
        ...
        return_val = osal_reset_sigs(diag_task_tcb, DIAG_MICRO_DIAGBUFFER_UIMAGE_SIG, &return_sigs);
        ASSERT(OSAL_SUCCESS == return_val);
...
#endif /* !defined(DIAG_MP) && defined(DIAG_SIO_SUPPORT) */

}       /* diag_get_rx_pkt_slave */
#endif

#if (((!defined(FEATURE_DIAG_SWITCH) && defined(DIAG_FRAMING)) || (defined(FEATURE_DIAG_SWITCH))) && defined(DIAG_SIO_SUPPORT))
/*===========================================================================
...
boolean diag_extract_pkt (diag_hdlc_decode_type * hdlc)
{
  ...
  if (hdlc && hdlc->src_ptr && hdlc->dest_ptr)
  {
    ...
    if (pkt_bnd == TRUE)
    {
      ...
      crc = crc_16_l_calc (hdlc->dest_ptr, ((hdlc->dest_idx - 1) * 8));
    ...
    hdlc->dest_idx -= DIAG_ASYNC_TAIL_SIZE;
      ...
      {
        MSG_2 (MSG_SSID_DIAG, MSG_LEGACY_ERROR, "Bad RX CRC - pkt_size 0x%x, crc 0x%x", hdlc->dest_idx, crc);
        pkt_bnd = FALSE;
        hdlc->dest_idx = 0; /* Start new packet */
        
        /* QPST needs a response in order to start its recovery algorithm. Send an error response if the
          packet is not in expected format. */
          
        /* Set DIAG_CX_COMM_S bit of diag_cx_state to allow the command responses to be sent to SIO layer */
        DIAG_SET_MASK (diag_cx_state, DIAG_CX_COMM_S);
        
        /* Send error response */
        rsp_pkt = (void*) diagpkt_err_rsp(DIAG_BAD_CMD_F, hdlc->src_ptr, hdlc->src_size);
        if(rsp_pkt)
        {
...
/*OCH- Initialization of the DCI Override Feature */
  ...
  is_dci_allowed = diag_dci_open_query();
  ULogFront_RealTimePrintf(diag_debug_ulog_handle, 1, "diag_task: is_dci_allowed = %d", is_dci_allowed);
...
diag_idle_processing_register (diag_idle_processing_ftype fp, void *param)
{
  ...
  if (fp)
  {
	   while(!diag_idle_process_mutex_initialized)
     {
      		diag_idle_process_mutex_init();
     } 
...
diag_sleep_vote (diag_sleep_vote_enum_type diag_svc, boolean vote)
{
  ...
  /* Only allow sleep voting for cmd/responses and not masks. */
  if (diag_svc != DIAG_SLEEP_DIAG)
    return;
...
uint32 diag_services_enabled(void)
{
  ...
  if( msg_enabled_mask & DIAG_MSG_STREAM_1)
  {
    DIAG_SET_MASK(mask, DIAG_SERVICE_MSG);
  }
  if(msg_enabled_mask & DIAG_MSG_STREAM_2)
  {
    DIAG_SET_MASK(mask, DIAG_SERVICE_MSG_DCI);
  }

  if( diag_log_config & DIAG_LOG_STREAM_1)
  {
    DIAG_SET_MASK(mask, DIAG_SERVICE_LOG);
  }

  if( diag_log_config & DIAG_LOG_STREAM_2)
  {
    DIAG_SET_MASK(mask, DIAG_SERVICE_LOG_DCI);
  }
  ...
  if(event_config_sec)
  {
    DIAG_SET_MASK(mask, DIAG_SERVICE_EVENT_DCI);
  }

  if(DIAG_CHECK_MASK(diag_cx_state, DIAG_CX_F3_LISTEN_S))
  {
    DIAG_SET_MASK(mask, DIAG_SERVICE_F3_LISTEN_S);
  }

  if(DIAG_CHECK_MASK(diag_cx_state, DIAG_CX_F3_LISTEN_EXT_S))
  {
    DIAG_SET_MASK(mask, DIAG_SERVICE_F3_LISTEN_EXT_S);
  }

  if(DIAG_CHECK_MASK(diag_cx_state, DIAG_CX_LOG_LISTEN_S))
  {
    DIAG_SET_MASK(mask, DIAG_SERVICE_LOG_LISTEN_S);
  }
  if(DIAG_CHECK_MASK(diag_cx_state, DIAG_CX_LOG_LISTEN_EXT_S))
  {
    DIAG_SET_MASK(mask, DIAG_SERVICE_LOG_LISTEN_EXT_S);
  }

  if(DIAG_CHECK_MASK(diag_cx_state, DIAG_CX_EVENT_LISTEN_S))
  {
    DIAG_SET_MASK(mask, DIAG_SERVICE_EVENT_LISTEN_S);
  }
  if(DIAG_CHECK_MASK(diag_cx_state, DIAG_CX_EVENT_LISTEN_EXT_S))
  {
    DIAG_SET_MASK(mask, DIAG_SERVICE_EVENT_LISTEN_EXT_S);
  }
                            ...
                            diag_searchlist_notify notify)
{
  ...
  if (list && list->initialized && !os_is_in_irq_mode ())
  {
    ...
    */
    ...
    /* Search for the entry having required key with remove_flag set as FALSE. */
  for (i = 0; i < count; i++)
  {
...
diag_searchlist_add (diag_searchlist_type *list, unsigned int key, void *goo)
{
  ...
  if (list && !os_is_in_irq_mode ())
  {
    if (!list->initialized)
    {
      ...
      while(!diag_srch_list_mutex_initialized)
      {
...
DESCRIPTION
  This function closes and deletes the specified file. However .DIAGIN file is
  not deleted if diag early logging is enabled.
...
diag_fs_terminate (diag_fs_type * fs)
{
  ...
  if(fs)
  {
    ...
    /*.DIAGIN file is referenced using the file descriptor "diag_fs_in".
    So, check for the file descriptor and early logging NV and delete the
    file if early logging is disabled.*/
    if ( (fs != &diag_fs_in) || ((!diag_early_log_nv_enabled)))
    {
...
dword diag_set_internal_sigs( dword sigs )
{
  ...
  while(!diag_internal_sig_mutex_initialized)
  {
  		diag_int_sig_cs_init();	
  }
...
{
  /* Trigger DiagID related events */
  diag_send_diagID_events(TRUE);

  /* Call time services sync function */
  time_system_time_event_logging();
}

/*===========================================================================

FUNCTION diag_dci_is_enabled

DESCRIPTION
  Checks if Diag received feature mask and whether DCI_DISABLED bit in feature mask bit is set or not

RETURN
  Returns TRUE if feature mask is received and DCI_DISABLED bit is not set to 1
...
boolean diag_dci_is_enabled(void)
{
  ...
  status = ((diagcomm_io_ctrl_conn[DIAGCOMM_PORT_SIO][DIAGCOMM_PORT_1].feature_mask[0] > 0) && \
	         !DIAG_IS_INT_FEATURE_BIT_SET(diagcomm_io_ctrl_conn[DIAGCOMM_PORT_SIO][DIAGCOMM_PORT_1].feature_mask, F_DIAG_DCI_DISABLED)) ? TRUE : FALSE;
  return status;
}
...
{
	byte *addr = NULL;
	event_store_type* event_ptr = NULL; 
	diag_cmd_ext_v1_type* log_ptr = NULL;
	msg_v4_multi_client_store_type* msg_ptr = NULL;
	int subid = DIAG_LISTENER_ERROR_INVALID;
	
	if(pkt == NULL)
	{
		return DIAG_LISTENER_ERROR_NULLPTR;
	}
	
	/* If the API is called from a task other than diag task, then DIAG_LISTENER_ERROR_OTHER_TASK error code will be returned*/
	if(osal_thread_self () != diag_task_tcb)
		return DIAG_LISTENER_ERROR_OTHER_TASK;
	
	switch(pkt_type)
	{
		case DIAG_EXT_EVENT:
		/* Extracting the event pointer from the pkt passed and de-referencing the sub ID field.  */
			addr = (byte*)pkt;
			addr = addr - FPOS(event_store_type,event_id.id); //moving to the beginning of the event_store_type structure
			event_ptr = (event_store_type*)addr;
			if(event_ptr->version != DIAG_EXT_API_VERSION_1)    // Return error if version is not DIAG_EXT_API_VERSION_1,i.e. it is not an extended packet
			{
				subid = DIAG_LISTENER_ERROR_NOTEXT;
			}
			else
			{
				subid = event_ptr->id;
			...
			break;
		case DIAG_EXT_LOG:
		/* The pointer passed actually points to the log field of diag_log_rsp_type structure which is in the second packet of the two chained packets. 
		   Moving to the first chained packet to access sub ID from the extended header. */
			addr = (byte*)pkt;
			addr = addr - FPOS(diag_log_rsp_type,log); //moving to the beginning of the diag_log_rsp_type struct
			addr = addr - sizeof(diagbuf_header_type); //moving over the diagbuf header
			addr = addr - sizeof(uint32); //moving over padding(2 bytes) and DEAD pattern(2 bytes) of 1st packet
			addr = addr - sizeof(diag_cmd_ext_v1_type); //moving to the beginning of diag_cmd_ext_v1_type struct
			log_ptr = (diag_cmd_ext_v1_type*)addr;
			if(log_ptr->version != DIAG_EXT_API_VERSION_1) // Return error if version is not DIAG_EXT_API_VERSION_1, i.e. it is not an extended packet
			{
				subid = DIAG_LISTENER_ERROR_NOTEXT;
			}
			else
			...
			break;
		case DIAG_EXT_MSG:
		/* Note: The sub ID will be extracted from a qshrink 4 type msg pointer. The pointer passed is of type diag_msg_type. Dereferencing it to ultimately get to the structure msg_v4_multi_client_store_type and access the sub ID field. */
			addr = (byte*)(((diag_msg_type*)pkt)->msg_store.qsr_v4_msg); // accessing the msg_v4_store_type pointer
			addr = addr - FPOS(msg_v4_multi_client_store_type,msg); // moving to the beginning of the msg_v4_multi_client_store_type structure
			msg_ptr = (msg_v4_multi_client_store_type*)addr;
			if(msg_ptr->version != DIAG_EXT_API_VERSION_1) // Return error if version is not DIAG_EXT_API_VERSION_1, i.e. it is not an extended packet
			{
				subid = DIAG_LISTENER_ERROR_NOTEXT;
			}
			else
			...
			break;
		default:
			subid = DIAG_LISTENER_ERROR_INVALID;
			break;
	}
	return subid;
}
