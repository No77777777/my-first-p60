...
/*===========================================================================
 ...
 $Header: //components/rel/core.mpss/10.0/services/diag/DCM/common/src/diagcomm_io_udp.c#1 $
...
12/10/15    gn     Resolved Klockwork errors
...
int diagcomm_io_udp_get_srv_addr (ipcr_name_t name, struct qsockaddr_ipcr *srv_addr, int timeout)
{
  ...
  {
    ipcr_ctrl_msg msg;
    unsigned int num_entries = DIAG_NUM_SRV_ADDR_ITEMS;
    rc = ipcr_find_name(ctrl_fd, &name, srv_addr, NULL, &num_entries, 0);
    if(rc > 0)
    { 
      for(num = 0; num < DIAG_NUM_SRV_ADDR_ITEMS; num++) { 
        MSG_2(MSG_SSID_DIAG, MSG_LEGACY_HIGH,"Got service with addr <%u,%u>\n",
                srv_addr[num].address.addr.port_addr.node_id,
                srv_addr[num].address.addr.port_addr.port_id);
		
        ULogFront_RealTimePrintf(diag_debug_ulog_handle, 2, "diagcomm_io_udp_get_srv_addr: Got service with node ID - %d, port ID - %d",\
	  	 	   srv_addr[num].address.addr.port_addr.node_id, srv_addr[num].address.addr.port_addr.port_id);		
      }
...
{
   int sock = -1;
   struct qsockaddr_ipcr srv_addr[DIAG_NUM_SRV_ADDR_ITEMS];
   int rc = 0;
...
DESCRIPTION
  Close the socket connection, change the Service and Instance IDs to default values
