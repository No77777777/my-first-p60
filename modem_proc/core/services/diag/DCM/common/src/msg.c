...
Copyright (c) 1992-2019 by QUALCOMM Technologies, Incorporated.  All Rights Reserved.
*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*/

/*==========================================================================

                           Edit History
  $Header: //components/rel/core.mpss/10.0/services/diag/DCM/common/src/msg.c#2 $
...
02/03/18   gn      Optimized QShrink 2 and 3 msg send functions.
02/26/18   ph      Update descriptor state correctly while sending qshrink 4 messages.
02/01/18   gn      Remove unused variable diag_msg_ext for qshrink 4 extended messages
01/25/18   gn      Added diag_pkt_len calculation for Qshrink2 msg api
...
05/05/16   is      Ensure extended Diag packets are put into the same send blocks
04/11/16   ph      Added support for QShrink 4.0
02/04/16   as      Featurized diag_msg_composite_mask with DIAG_UIMAGE_MODE
10/20/15   gn      Fixed bug in sending sprintf Extended messages
...
04/29/14   vk      Fixed build error caused by index variable defined but not
                   used.
04/01/14   rs      Added support for build mask centralization.
04/15/14   is      Fix for msgs not coming out after switching preset masks
04/01/14   rs      Added support for build mask centralization.
...
uint32 msg_v3_ss_num_table_entries QSR_MSG_V3_ATTR = 0;
msg_v3_ssid_table_type *msg_v3_ss_table QSR_MSG_V3_ATTR = NULL;
...
/* Statistics */
uint32 msg_total;    /* total number of messages logged */
...
boolean msg_set_rt_mask_adv (uint16 ssid, uint32 mask, byte stream_id, uint8 preset_id);
boolean msg_masks_enabled_check(byte stream_id);
...
#define MAX_QSR_MSG_SIZE 0x500
/* Intermediate buffer used to construct a msg expanded from diagbuf to be sent out via diagbuf_send_packet() */
uint8 diag_qsr_msg_buffer[MAX_QSR_MSG_SIZE];
uint16 diag_qsr_msg_buffer_index = 0;
...
msg_init (void)
{
...
#if defined (DIAG_UIMAGE_MODE) ||(defined(DIAG_MP) && !defined(DIAG_MP_MASTER))
  uint8 index = 0;
#endif
  ...
  (void)diagbuf_register_outbound_pkt_handler(DIAG_EXT_MSG_F,
...
uint32 msg_get_qsr4_hash(uint32 qsr_hash_v4)
{
  ...
  /* sheethal -compiled out compilation errors for ULOG_RT_PRINTF_1
  else
  {
	  ULOG_RT_PRINTF_1( diag_err_ulog_handle,"qsr_hash_v4 %d is out of bounds",qsr_hash_v4);
  } sheethal end */
return qsr_hash_v4;
}
...
FUNCTION MSG_UPDATE__v3_SS_Table
...
msg_qsr_ext_expand_and_send_pkt(diag_send_desc_type *desc, byte stream_id)
{
  ...
  if (desc->pkt)
  {
    ...
    else if (cmd_code == DIAG_QSR_EXT_MSG_TERSE_F)
    {
      ...
      /* Calculate length of Message packet */
    diag_pkt_len = FPOS(msg_v3_multi_client_store_type, msg) + sizeof(msg_hdr_type) + sizeof(msg_desc_type) + sizeof(msg_v3_store->msg.args[0]) * msg_v3_store->msg.hdr.num_args;

    /* Based on the QSR flag, add hash value length or string length(filename + format string) appropriately */
      if (msg_v3_store->msg.qsr_flag & QSR_BITFLAG_IS_HASH)
      {
        diag_pkt_len += sizeof(msg_v3_store->msg.qsr_const_data_ptr.v3_index->msg);
      }
      else if(!(msg_v3_store->msg.qsr_flag & QSR_BITFLAG_IS_HASH))
      {
        diag_pkt_len += strlen((char*)msg_v3_store->msg.qsr_const_data_ptr.msg_desc->msg_hash);
      }

      ULogFront_RealTimePrintf(diag_debug_ulog_handle, 1, "msg_qsr_ext_expand_and_send_pkt: msg_qsr_ext_expand_and_send_pkt is %d", msg_qsr_ext_expand_and_send_pkt);
...
msg_qsr_expand_and_send_pkt(diag_send_desc_type *desc, byte stream_id)
{
  ...
  if (desc)
  {
    ...
    /* 'desc' is not moved until the entire message is complete.  Therefore,
       it always points to the stored message, in terse format. */
    ...
    if (msg_store)
    {
      ...
      {
        /* Reset the index for new message */
        diag_qsr_msg_buffer_index = 0;

        /* Calculate the full message packet length.
           Packet length is calculated only while sending first part to reserve space for the entire message packet, need not be calculated for other parts */
        diag_pkt_len = sizeof(msg_hdr_type) + sizeof(msg_desc_type) + (sizeof(msg_store->args[0]) * msg_store->hdr.num_args);

        /* Based on the QSR flag add hash value length or string length(filename + format string) appropriately */
        if (msg_store->qsr_flag & QSR_BITFLAG_IS_HASH)
        {
          diag_pkt_len += sizeof(msg_store->qsr_const_data_ptr.v3_index->msg);
        }
        else if(!(msg_store->qsr_flag & QSR_BITFLAG_IS_HASH))
        {
          diag_pkt_len += strlen((char*)msg_store->qsr_const_data_ptr.msg_desc->msg_hash);
        }

        if (msg_store->qsr_flag & QSR_BITFLAG_IS_HASH)
        {
          /* Set the command code for QShrink 3 MSG. */
          msg_store->hdr.cmd_code = DIAG_QSR_EXT_MSG_TERSE_F;
        }
        else
        {
          /* Set the command code for QShrink 2 MSG */
          msg_store->hdr.cmd_code = DIAG_EXT_MSG_F;
        }

        /* Copy message header */
        memscpy((void*)(diag_qsr_msg_buffer + diag_qsr_msg_buffer_index), sizeof(msg_hdr_type), (void *)&msg_store->hdr, sizeof(msg_hdr_type));
        diag_qsr_msg_buffer_index += sizeof(msg_hdr_type);

        /* Copy message Descriptor */
        ldesc.ss_id = msg_ssid;
        ldesc.ss_mask = msg_mask;
        ldesc.line = msg_line;
        memscpy((void*)(diag_qsr_msg_buffer + diag_qsr_msg_buffer_index), sizeof(msg_desc_type), (void *)&ldesc, sizeof(msg_desc_type));
        diag_qsr_msg_buffer_index += sizeof(msg_desc_type);

        /* Copy Hash value if HASH flag is set */
        if (msg_store->qsr_flag & QSR_BITFLAG_IS_HASH)
        {
          memscpy((void*)(diag_qsr_msg_buffer + diag_qsr_msg_buffer_index), sizeof(msg_store->qsr_const_data_ptr.v3_index->msg),
                      (void *)&msg_store->qsr_const_data_ptr.v3_index->msg, sizeof(msg_store->qsr_const_data_ptr.v3_index->msg));
          diag_qsr_msg_buffer_index += sizeof(msg_store->qsr_const_data_ptr.v3_index->msg);
        }

        /* Copy Arguments */
        memscpy((void*)(diag_qsr_msg_buffer + diag_qsr_msg_buffer_index), sizeof(msg_store->args[0]) * msg_store->hdr.num_args,
                                               (void *)msg_store->args, sizeof(msg_store->args[0]) * msg_store->hdr.num_args);
        diag_qsr_msg_buffer_index += sizeof(msg_store->args[0]) * msg_store->hdr.num_args;

        /* Copy format string and file name if the message is not hashed */
        if (!(msg_store->qsr_flag & QSR_BITFLAG_IS_HASH))
        {
          /* saved format --> file_name : format string
             Sent format  --> format string + file_name */
          for (str_buf = (char *) msg_store->qsr_const_data_ptr.msg_desc->msg_hash; *str_buf != ':'; str_buf++);

          filename_len = str_buf - (char *)msg_store->qsr_const_data_ptr.msg_desc-> msg_hash;
          str_buf++;
          str_len = MIN(MSG_MAX_STRLEN, strlen(str_buf));

          /* Copy Format string */
          memscpy((void*)(diag_qsr_msg_buffer + diag_qsr_msg_buffer_index), str_len, (void*)str_buf, str_len);
          diag_qsr_msg_buffer_index += str_len;

          /* Copy NULL terminator */
          *(diag_qsr_msg_buffer + diag_qsr_msg_buffer_index++) = 0;

          filename_len = MIN(MSG_MAX_STRLEN, filename_len);

          /* Copy File name */
          memscpy((void*)(diag_qsr_msg_buffer + diag_qsr_msg_buffer_index), filename_len,
                  (void*)msg_store->qsr_const_data_ptr.msg_desc->msg_hash, filename_len);
          diag_qsr_msg_buffer_index += filename_len;
        ...
        /* Set up the send descriptor */
        send.pkt = (void*)diag_qsr_msg_buffer;
        send.last = (void *)(diag_qsr_msg_buffer + diag_qsr_msg_buffer_index - 1);
        send.state = DIAG_SEND_STATE_START;
        send.terminate = TRUE;
        send.diag_id = desc->diag_id;
...
void diag_switch_msg_preset_mask()
{
  ...
  diag_qsr_rt_mask = msg_stream_ssid_tbl_mask[DIAG_STREAM_1-1][diag_cur_preset_id-1] |
                                  ...
                                  boolean send_now )
{
    ...
    if( update_all || diagcomm_ctrl_msg_mask_status(port_type, port_num, stream_id, preset_id, i) )
    {
      ...
      if( ctrl_msg.data.ctrl_msg_f3_mask_v3.status == DIAG_CTRL_MASK_ALL_DISABLED )
      {
        ...
        (void)diagcomm_ctrl_send_mult_pkt( (void *)&ctrl_msg,
                                           ...
                                           NULL, 0, DIAG_CTRL_USER_PKT_ID_0,
      ...
      else if( ctrl_msg.data.ctrl_msg_f3_mask_v3.status == DIAG_CTRL_MASK_ALL_ENABLED )
      {
        ...
        if( stream_id == DIAG_STREAM_1 )
        {
          (void)diagcomm_ctrl_send_mult_pkt( (void *)&ctrl_msg,
                                         DIAGCOMM_CTRL_MSG_HDR_SIZE + sizeof(diag_ctrl_msg_f3_mask_v3_type),
                                         (void *)&diag_mstr_rt_preset_mask[preset_id-1][i][0],
                                         ctrl_msg.data.ctrl_msg_f3_mask_v3.msg_mask_size * sizeof(uint32),
                                         DIAG_CTRL_USER_PKT_ID_0, port_type, port_num, send_now, &is_sent );
        }
        #if defined(DIAG_STREAM_2_MASK_CHECK)
        else if( stream_id == DIAG_STREAM_2 )
        {
          (void)diagcomm_ctrl_send_mult_pkt( (void *)&ctrl_msg,
                                         DIAGCOMM_CTRL_MSG_HDR_SIZE + sizeof(diag_ctrl_msg_f3_mask_v3_type),
                                         (void *)&diag_mstr_rt_dci_mask[DIAG_DCI_MASK_1-1][i][0],
                                         ctrl_msg.data.ctrl_msg_f3_mask_v3.msg_mask_size * sizeof(uint32),
                                         DIAG_CTRL_USER_PKT_ID_0, port_type, port_num, send_now, &is_sent );
        }
        #endif
  ...
  diagcomm_ctrl_send_mult_pkt( NULL, 0, NULL, 0, DIAG_CTRL_USER_PKT_ID_0, port_type, port_num, send_now, &is_sent );
...
void diag_ctrl_update_msg_mask( diag_ctrl_msg_type * ctrl_msg )
{
  ...
  ULogFront_RealTimePrintf(diag_debug_ulog_handle, 5, "diag_ctrl_update_msg_mask: update msg mask - msg_mask_size = %d, ssid_first = %d, ssid_last = %d, stream_id = %d, status = %d", msg_mask_size, ssid_first, ssid_last, stream_id, status); 
...
void diag_ctrl_send_msg_ssid_range_ctrl_pkt(void)
{
     ...
     if(bt_msg_ssid_range_fwd_status == DIAG_CTRL_INFO_FWD_STATUS_NOT_SENT)
     {
          ...
          bytes_written = diagcomm_ctrl_send_mult_pkt( &ctrl_msg, DIAGCOMM_CTRL_MSG_HDR_SIZE + sizeof(diag_ctrl_bt_msg_ssid_range_type),
...
void diag_ctrl_send_msg_bt_mask_ctrl_pkt(void)
{
     ...
     for (i = 0; i < MSG_MASK_TBL_CNT; i++)
     {
          ...
          if((tbl_ptr != NULL) && (bt_msg_mask_fwd_status[i] == DIAG_CTRL_INFO_FWD_STATUS_NOT_SENT))
          {
                ...
                bytes_written = diagcomm_ctrl_send_mult_pkt( &ctrl_msg, DIAGCOMM_CTRL_MSG_HDR_SIZE + sizeof(diag_ctrl_bt_msg_mask_type),
                                                                tbl_ptr->bt_mask_array, userpkt_len, DIAG_CTRL_USER_PKT_ID_0, DIAGCOMM_PORT_SIO, DIAGCOMM_PORT_1, FALSE, &is_sent );
     ...
     /* No more msg mask tables to fwd; Send the current DSM item immediately and then quit */
     (void)diagcomm_ctrl_send_mult_pkt( NULL, 0, NULL, 0, DIAG_CTRL_USER_PKT_ID_0, DIAGCOMM_PORT_SIO,
...
void diag_update_composite_msg_mask(int index, int start, int end)
{
  ...
  for(mask_index = start; mask_index <= end; mask_index++)
  {
    ...
    for(stream_index = 0; stream_index < DIAG_MAX_STREAM_ID; stream_index++)
    {
      ...
      temp_mask |= listener_msg_mask_tbl[index].f3_listener_mask_array[mask_index];
      temp_mask |= listener_msg_mask_tbl[index].f3_listener_ext_mask_array[mask_index];
    }
