...
Copyright (c) 1999-2020 by QUALCOMM Technologies, Incorporated.
All Rights Reserved.
...
/*===========================================================================
 ...
 $Header: //components/rel/core.mpss/10.0/services/diag/DCM/common/src/diagbuf.c#3 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
02/24/19   gn      Resolved send buffer out of bound access
01/09/19   ph      Handle out of bounds access of send buffer.
04/23/18   ph      Separation of data and DCI send buffer macros.
01/02/19   ph      Fix the send buffer size check for DCI stream.
11/15/18   gn      Clear drain timer on reaching commit threshold.
10/28/18   kdey    Added featurized debug code for ring buffer corruption issue
04/29/18   ph      Enable diag boot up logging.
08/21/18   kdey    Adding fixes for double commit and pattern overrun check
07/27/18   gn      Featurize SIO and DSM code under DIAG_SIO_SUPPORT
06/29/18   gn      Fixed issue in send buffer wrap case.
06/19/18   ph      Configure buffering modes for user PD send buffers.
09/22/17   gn      Diag performance improvements and optimizations
05/15/18   ph      Featurize for standalone case in all diagbuf functions.
08/29/17   gn      Changes to handle remote socket close
08/14/17   ph      Fix the memory leak with event queue during PDR.
...
06/16/17   ph      Avoid sending packets larger than 16kb over DCI channel since Apps can't support it
...
03/03/17   sa      Fixed PD restart issue, moved PD check up.
02/21/17   ph      Tag DIAG ID header to all the diag packets sent out.
02/21/17   gn      Fixed extended logs drop issue when DCI and QXDM run concurrently
02/14/17   sa      Do not set is_in_use bit for PD in circular mode
01/25/17   rs      Re-try sending control packets using bit mask and signal and by-passing the control heap usage.
12/22/16   gn      Fixed log drop issue in NRT mode
12/20/16   tbg     Added qshtrace health commands
12/02/16   ph      Resolve KW errors in diag codebase.
11/23/16   nk      Fixed the issue of missing de-initilization of the event heap
09/29/16   ph      Diag and ULOG integration.
11/14/16   sp      Cleared buf_in_use just after the inner loop is exited.
11/08/16   rs      checking for diagcomm_cmd_status() instead of diagcomm_status() in diagbuf_flush_timeout()
                   before sending a response.
11/03/16   sa      Implemented seperate queue for UserPD events and enabled buffering
10/24/16   rs      Mainling debug support code via ULogs
08/26/16   sp      Initialize diagbuf_mpd_q before Diag registration with QDI. Also cleanup unused mutexes
08/26/16   gn      Fixed length miscalculation in Diagbuf_shorten
08/26/16   gn      Added check for diag_cx_state before setting TX signal
...
05/12/16   ph      Remove event_flush as it is redundant.
...
01/05/16   nk      Added check to pet dog in diagbuf_drain function.
...
12/02/15   as      Fixed issue that diagbuf drain only first part of the chained pkt before checking for signal
09/11/15   gn      Fixed issue of missing Subscription ID and Proc ID for Extended logs
08/26/15   ph      Buffering mode revamp to make it functional.
08/25/15   vk      Fixed unused variable, extraneous parentheses compiler warnings
08/16/15   vk      Changes to support micro_diagbuffer_drain
...
05/12/15   xy      Fixed issue that diagbuf drain only first part of the
                   chained pkt before checking for signal
04/08/15   ps      Diag over sockets and dsm removal
02/26/15   ps      Check flow control in inner loop in diagbuf_drain
...
DESCRIPTION
  ...
  diag_drain_buf_type queue and return the next buffer to check

PARAMETERS
 diag_drain_buf_type *buf - the UserPD diagbuf pointer.
...
\
   boolean rv = FALSE;\
   diag_drain_buf_type *q_cur_ptr = NULL;\
   diagbuf_mpd_type *upd_buf =NULL; \
\
   ...
   {\
      q_cur_ptr = (diag_drain_buf_type *)q_check(&diagbuf_mpd_q);\
\
      while (q_cur_ptr != NULL)\
      {\
        if(q_cur_ptr->buffer_type == DIAG_DIAGBUFFER_UPD_TYPE)\
        {\
         upd_buf = (diagbuf_mpd_type *)q_cur_ptr->buf_ptr; \
         if (DIAGBUF_USED(upd_buf->diagbuf_mpd_head, \
                          upd_buf->diagbuf_mpd_tail, \
                          upd_buf->diagbuf_size) > \
                          upd_buf->diagbuf_size) \
         {\
            rv = TRUE;\
            break;\
...
\
         q_cur_ptr = (diag_drain_buf_type *)q_next(&diagbuf_mpd_q, &q_cur_ptr->link);\
      }\
   }\
   (rv);\
})
...
extern boolean diag_diagID_supported;
extern boolean diag_early_log_enabled;
extern void diag_early_mask_cleanup(void);

/*===========================================================================
FUNCTION DIAGBUF_ARE_BUFFERS_EMPTY

DESCRIPTION
  This function checks all diagbuffers to see if any of them is
  empty.

RETURN VALUE
  TRUE  - If the GuestOS diagbuf, UserPD diagbuf or private diagbuffer
          is empty.
  FALSE - Otherwise

===========================================================================*/
boolean DIAGBUF_ARE_BUFFERS_EMPTY(void)
{
   boolean rv = TRUE;
   diag_drain_buf_type *q_cur_ptr = NULL;
   diagbuf_mpd_type* upd_buf = NULL;
   ...
   else
   {
      ...
      while (q_cur_ptr != NULL)
      {
         ...
         {
            upd_buf = (diagbuf_mpd_type *)q_cur_ptr->buf_ptr;
            if (!DIAGBUF_EMPTY(upd_buf->diagbuf_mpd_head, upd_buf->diagbuf_mpd_tail))
            {
                rv = FALSE;
                break;
            }
         ...
         {
            h = (diagbuffer_handle) q_cur_ptr->buf_ptr;
            if(!DIAGBUFFER_IS_EMPTY(h))
            {
                rv = FALSE;
                break;
            }
...
PARAMETERS
 diagbuf_mpd_type *current - the UserPD diagbuf pointer.
...
RETURN VALUE
  ...
  FALSE - If PD is a core PD, it cannot be deleted. Or if
          ...
          item in queue.
===========================================================================*/
boolean DIAGBUF_REMOVE_PD_FROM_PD_LINK_LIST(diagbuf_mpd_type* current)
{
  boolean rv = FALSE;
  diag_drain_buf_type *q_cur_ptr = NULL;
  diagbuf_mpd_type* upd_buf = NULL;
   ...
   else
   {
      ...
      while (q_cur_ptr != NULL)
      {
        if(q_cur_ptr->buffer_type == DIAG_DIAGBUFFER_UPD_TYPE)
        {
          ...
          if( upd_buf == current)
          {
            /*Matching PD found, delete it */
		  
	        ULogFront_RealTimePrintf(diag_err_log_handle,1,"DIAGBUF_REMOVE_PD_FROM_PD_LINK_LIST : Deleting user pd from diagbuf_mpd_q  pd address= %p", q_cur_ptr);
            rv = q_delete_ext(&diagbuf_mpd_q, &q_cur_ptr->link);
...
boolean event_pd_delete(diagbuf_mpd_type *mpd_buf_to_remove)
{
  ...
  if ( DIAGBUF_IS_PD_KILLED(mpd_buf_to_remove) )
  {
    ...
    ULogFront_RealTimePrintf(diag_err_log_handle, 1, "event_pd_delete : Deleting pd from diag_main_q pd address= %p", diagmpd_clean_ptr->link);
...
uint8 diagbuf_send_cmd[NUM_SMD_PORTS][DIAG_SEND_BUF_SIZE_CMD]; /*Buffer used to send out cmd rsp */
uint8 diagbuf_send_data_wpd[DIAG_SEND_BUF_SIZE_DATA_WLAN];
...
static uint32 diag_send_buf_data_index[NUM_SMD_PORTS] = {0, 0}; /*Current data buffer index*/
static uint32 diag_send_buf_cmd_index[NUM_SMD_PORTS] = {0, 0}; /*Current cmd buffer index*/
extern uint32 diag_send_buf_threshold; /*Threshold to buffer up data in send buffer*/
extern uint32 diag_max_send_buf_len; /*Maximum send size*/
...
void diag_non_hdlc_encode( diag_send_desc_type *src_desc, diag_hdlc_dest_type *enc, byte stream_id);
...
/*===========================================================================
FUNCTION DIAGBUF_RESET_DROP_COUNT_QSHTACE

DESCRIPTION
    This function resets the internal variables drop_count_f3 and
    alloc_count_f3.
===========================================================================*/
void diagbuf_reset_drop_count_qshtrace( void )
{
  drop_count_qshtrace = 0;
  alloc_count_qshtrace = 0;
  bCountReset = TRUE;
}
/*===========================================================================
FUNCTION DIAGBUF_GET_DROP_COUNT_QSHTRACE

DESCRIPTION
    This function returns the internal variable drop_count_f3.
===========================================================================*/
uint32 diagbuf_get_drop_count_qshtrace( void )
{
  return (drop_count_qshtrace);
}

/*===========================================================================
FUNCTION DIAGBUF_GET_ALLOC_COUNT_QSHTRACE

DESCRIPTION
    This function returns the internal variable alloc_count_f3.
===========================================================================*/
uint32 diagbuf_get_alloc_count_qshtrace( void )
{
  return (alloc_count_qshtrace);
}
...
retry:
  ...
  if ( ptr )
  {
    ...
    case DIAGBUF_ALLOC_REQUEST_MSG:
      ...
      break;
    case DIAGBUF_ALLOC_REQUEST_QSHTRACE:
      if ( alloc_count_qshtrace == MAX_VALUE_UINT32 )
      {
        MSG(MSG_SSID_DIAG, MSG_LEGACY_LOW, "Variable alloc_count_qshtrace has reached maximum value.");
      }
      else
      {
        alloc_count_qshtrace++;
      }
      break;
  ...
  else
  {
    ...
    case DIAGBUF_ALLOC_REQUEST_MSG:
      ...
      break;
    case DIAGBUF_ALLOC_REQUEST_QSHTRACE:
      if ( drop_count_qshtrace == MAX_VALUE_UINT32 )
      {
        MSG(MSG_SSID_DIAG, MSG_LEGACY_LOW, "Variable drop_count_qshtrace has reached maximum value.");
      }
      else
      {
        drop_count_qshtrace++;
  ...
  if ( ptr )
  {
    ...
    //This is just to verify whether we were able to write the DEAD pattern or whether it could not be written due to bad memory regions 
    diagbuf_check_overrun_pattern(pkt_hdr);
   ...
   )
{
  ...
  //If this condition is true then the client is trying to commit a packet which is already committed, ideally it should not be happenning.
  if(pkt_hdr->status==DIAGBUF_COMMIT_S)
  {
    ULogFront_RealTimePrintf(diag_debug_ulog_handle,1,"Client is trying to commit an already committed packet at address 0x%x",pkt_hdr);
    return;
  }
  ...
  if ((diagbuf_commit_size >= diagbuf_commit_threshold) || (DIAGBUF_FREE(diagbuf_head,diagbuf_tail,DIAGBUF_SIZE) < DIAG_MAX_TX_PKT_SIZ))
  {
...
void diagbuf_clear_index (diagcomm_io_conn_type * conn, diagcomm_io_tx_params_type * tx_param )
{
  ...
  stream_id = DIAG_INDEX_TO_ID(port_num);

  stream_index = DIAG_ID_TO_INDEX(stream_id);

#if !defined(DIAG_MP) && defined(DIAG_SIO_SUPPORT)
  ...
  else if (ch_type == DIAGCOMM_IO_DATA)
  {
    ...
    {
#if !defined(DIAG_MP) && defined(DIAG_SIO_SUPPORT)
      tx_mode_ptr = &diag_tx_mode[stream_index][0];
#else
      /* tx_mode_ptr is obtained from the diag_id field of tx_param */
      tx_mode_ptr = tx_param->params.tx_ptr;
    ...
    else
    {
     ...
     dsm_pkt_length = dsm_length_packet(tx_param->dsm_ptr);
      ASSERT(tx_mode_ptr->buf_used >= dsm_pkt_length);
#else
      /*Check to see if we have sent data less than used bytes in the buffer*/
      ASSERT(tx_mode_ptr->buf_used >= tx_param->params.len);
#endif
...
#if !defined(DIAG_MP) && defined(DIAG_SIO_SUPPORT)
      tx_mode_ptr->buf_tail += dsm_pkt_length + sizeof(diag_send_buf_header_type);
#else
      tx_mode_ptr->buf_tail += tx_param->params.len + sizeof(diag_send_buf_header_type);
#endif
...
#if !defined(DIAG_MP) && defined(DIAG_SIO_SUPPORT)
        tx_mode_ptr->buf_used -= (pad + dsm_pkt_length + sizeof(diag_send_buf_header_type));
#else
        tx_mode_ptr->buf_used -= (pad + tx_param->params.len + sizeof(diag_send_buf_header_type));
#endif
        buf_header = (diag_send_buf_header_type *)(tx_mode_ptr->buf_ptr + tx_mode_ptr->buf_tail);
...
void diag_get_sendbuf_params(diag_send_buf_type *send_buf, boolean is_cmd_rsp, uint8 diag_id)
{
  ...
  else
  {
    ...
    /*If early logging is enabled and if used bytes is greater than many mark, then
    invoke the callback that clear the masks so that the data is retained in the buffer.
    ( assuming mask updates are not received yet from APPS )*/
    if ( (diag_early_log_enabled) &&
        (send_buf->used >= tx_mode_ptr->buffered_many_bytes_mark) )
    {
...
#else
    l_tx_mode = &diag_tx_mode[stream_index][0];
#endif
...
/*Logic to check if we have to flush bufferred data*/
   ...
   if (desc == NULL) {
      ...
      diag_get_sendbuf_params(&send_buf, is_cmd_rsp_type, desc->diag_id);
      if (!is_cmd_rsp_type) {
   ...
   if (pre_send == TRUE) {
      ...
      if (diag_tag_hdr_size >= send_size) {
            MSG(MSG_SSID_DIAG, MSG_LEGACY_LOW, "Send buffer is Empty");
      } else {
   ...
   if ((!is_cmd_rsp_type) && (FALSE == diagcomm_io_conn[DIAGCOMM_PORT_SIO][DIAGCOMM_PORT_1].allow_flow)) {
    ULogFront_RealTimePrintf(diag_data_ulog_handle, 4, "diagbuf_send_pkt: allow_flow on data channel = %d, channel_type = %d, io_type = %d, port_num = %d",\
   ...
   if (!is_cmd_rsp_type) {
     if (((curr_block_len + pkt_len) >= send_buf.threshold) && (desc->state == DIAG_SEND_STATE_START))
     {
       /* Add a header here to mark the start of a data block;
         ...
         diag_add_block_header(&send_buf, pkt_len, desc->diag_id);
   ...
   if ((DIAG_IS_INT_FEATURE_BIT_SET(diagcomm_io_ctrl_conn[DIAGCOMM_PORT_SIO][DIAGCOMM_PORT_1].feature_mask, F_DIAG_HDLC_ENCODE_IN_APPS))\
          && (DIAG_INT_FEATURE_MASK_CHECK(F_DIAG_HDLC_ENCODE_IN_APPS))) {
      diag_non_hdlc_encode(desc, &enc, DIAG_STREAM_1);
   } else {
   ...
   {
      diag_update_sendbuf_params(&send_buf, is_cmd_rsp_type, desc->diag_id);
   }
   ...
   if (ch_type == DIAGCOMM_IO_DATA)
   {
     if (port_type == DIAGCOMM_PORT_1) 
     {
        ...
        else 
        {
          ...
          if(diag_drain_pool_check(DIAG_STREAM_1, tx_mode_ptr))
            {
     ...
     else if (port_type == DIAGCOMM_PORT_2)
     {
        ...
        diagcomm_send((void *)diagbuf_send_data_dci, diag_send_buf_data_index[port_type], FALSE, port_type, tx_mode_ptr);
     }
      }
...
diagbuf_status_enum_type diagbuf_send_pkt_single_proc_dci (diag_send_desc_type *desc)
{
   ...
   uint32 *diag_send_buf_index; /*Current buffer index*/
   diag_tx_mode_type *tx_mode_ptr = NULL;
   uint8 pad_len = 0;
   boolean diag_not_connected = FALSE;
   boolean is_cmd_rsp_type = FALSE;
   ...
   stream_index = DIAG_ID_TO_INDEX(DIAG_STREAM_2);
...
#if defined(DIAG_MP) && !defined(DIAG_MP_MASTER)
   tx_mode_ptr = diag_get_tx_mode_ptr(DIAG_STREAM_2, diag_get_diagID());
#else
    tx_mode_ptr = &diag_tx_mode[stream_index][0];
#endif
   ...
   if(is_cmd_rsp_type == TRUE)
   {
	 if (!(diagcomm_cmd_status(DIAGCOMM_PORT_2) && DIAG_CHECK_MASK(diag_cx_state, DIAG_CX_DCI_S)))
     {
        diag_not_connected = TRUE;
     }
   }
   ...
   if (diag_not_connected == TRUE) {
      ...
      if (outbound != NULL) {
         if (diag_send_buf_cmd_index[DIAGCOMM_PORT_2] > 0) {
            ...
            ASSERT(outbound_used < DIAG_SEND_BUF_SIZE_CMD);
            if (TRUE == diagcomm_io_cmd_conn[DIAGCOMM_PORT_SIO][DIAGCOMM_PORT_2].allow_flow) {
               diagcomm_send((void *)diagbuf_send_cmd[DIAGCOMM_PORT_2], outbound_used, TRUE, DIAGCOMM_PORT_2, tx_mode_ptr);
            }
         }
         if (diag_send_buf_data_index[DIAGCOMM_PORT_2] > 0) {
            outbound_used = diag_send_buf_data_index[DIAGCOMM_PORT_2];
            ASSERT(outbound_used < DIAG_SEND_BUF_SIZE_DATA);

            if (diagcomm_io_conn[DIAGCOMM_PORT_SIO][DIAGCOMM_PORT_2].allow_flow == TRUE) {
               diagcomm_send((void *)diagbuf_send_data_dci, outbound_used, FALSE, DIAGCOMM_PORT_2, tx_mode_ptr);
            }
         }
         outbound = NULL;
         item_ptr_dci = NULL;
      ...
      if ((!is_cmd_rsp_type) && (FALSE == diagcomm_io_conn[DIAGCOMM_PORT_SIO][DIAGCOMM_PORT_2].allow_flow)) {
         status = DIAGBUF_NOT_READY_S;
         return status;
      }
      ...
      while (status == DIAGBUF_READY_S && desc->state < DIAG_SEND_STATE_COMPLETE) {
         ...
         if (((pkt_len + outbound_used) >= diag_send_buf_threshold)
             && (TRUE == first_packet_dci) && (!is_cmd_rsp_type)) {
            ...
            if (item_ptr_dci) {
               if (is_cmd_rsp_type) {
                  diagcomm_send((void *)diagbuf_send_ptr, *diag_send_buf_index, TRUE, DIAGCOMM_PORT_2, tx_mode_ptr);
                  diagbuf_send_ptr = &diagbuf_send_cmd[DIAGCOMM_PORT_2][0];
                  diag_send_buf_index = &diag_send_buf_cmd_index[DIAGCOMM_PORT_2];
                  ...
                  diagcomm_send((void *)diagbuf_send_ptr, *diag_send_buf_index, FALSE, DIAGCOMM_PORT_2, tx_mode_ptr);
                  if (diag_send_buf_data_index[DIAGCOMM_PORT_2] != 0) {
                     //Sending previously accumulated data was not successful
                     status = DIAGBUF_NOT_READY_S;
                     return status;
         ...
         if (outbound) {
            diag_non_hdlc_encode(desc, &enc_dci, DIAG_STREAM_2);
         ...
         if (is_cmd_rsp_type) {
            diagcomm_send((void *)diagbuf_send_ptr, *diag_send_buf_index, desc->is_cmd_rsp_type, DIAGCOMM_PORT_2, tx_mode_ptr);
            outbound = NULL;
         }
...
diagbuf_status_enum_type diagbuf_send_pkt (diag_send_desc_type *desc, byte stream_id)
{
    ...
    if(!desc) {
      ...
      if(stream_id & DIAG_STREAM_2)
      {
            ...
            {
          ULogFront_RealTimePrintf(diag_data_ulog_handle, 4, "diagbuf_send_pkt: allow_flow = %d, channel_type = %d, io_type = %d, port_num = %d",\
            diagcomm_io_conn[DIAGCOMM_PORT_SIO][DIAGCOMM_PORT_2].allow_flow, diagcomm_io_conn[DIAGCOMM_PORT_SIO][DIAGCOMM_PORT_2].channel_type, \
            diagcomm_io_conn[DIAGCOMM_PORT_SIO][DIAGCOMM_PORT_2].io_type, diagcomm_io_conn[DIAGCOMM_PORT_SIO][DIAGCOMM_PORT_2].port_num);
               status = DIAGBUF_NOT_READY_S;
            }
        }
      return status;
  	...
  	if(diag_dci_pending && desc->last != send_last)
  	{
  	  ...
  	  ULogFront_RealTimePrintf(diag_debug_ulog_handle, 0, "diagbuf_send_pkt: Resetting diag_dci_pending flag");
  	}
  	...
  	{
  	  desc->state = DIAG_SEND_STATE_COMPLETE;
  	  ULogFront_RealTimePrintf(diag_debug_ulog_handle, 0, "diagbuf_send_pkt: Trying to resend same packet");
  	  return status;
  	}
    ...
    if ((stream_id & DIAG_STREAM_1) && !diag_dci_pending && desc->last != send_last)
    {
      ...
      /* Unable to send packet on Stream 1, mark desc->state as DIAG_SEND_STATE_START and return
      Eventhough Stream 2 is enabled, skip sending to Stream 2 when flow is controlled on Stream 1
      and retry for both streams together when flow is enabled on Stream 1*/
      if(status == DIAGBUF_NOT_READY_S)
      {
        desc->state = DIAG_SEND_STATE_START;
        ULogFront_RealTimePrintf(diag_debug_ulog_handle, 0, "diagbuf_send_pkt: status is DIAGBUF_NOT_READY_S");
        return status;
      }
    ...
    if(stream_id & DIAG_STREAM_2)
    {
      ...
      {
        ULogFront_RealTimePrintf(diag_debug_ulog_handle, 5, "diagbuf_send_pkt: allow_flow on command channel = %d, allow_flow on data channel = %d, channel_type = %d, io_type = %d, port_num = %d",\
                                 diagcomm_io_cmd_conn[DIAGCOMM_PORT_SIO][DIAGCOMM_PORT_2].allow_flow, diagcomm_io_conn[DIAGCOMM_PORT_SIO][DIAGCOMM_PORT_2].allow_flow, diagcomm_io_conn[DIAGCOMM_PORT_SIO][DIAGCOMM_PORT_2].channel_type,\
                                 diagcomm_io_conn[DIAGCOMM_PORT_SIO][DIAGCOMM_PORT_2].io_type, diagcomm_io_conn[DIAGCOMM_PORT_SIO][DIAGCOMM_PORT_2].port_num);
        status = DIAGBUF_NOT_READY_S;
		    ULogFront_RealTimePrintf(diag_debug_ulog_handle, 0, "diagbuf_send_pkt: status is DIAGBUF_NOT_READY_S due to flow control on DCI channel");
        return status;
      }
      ...
      if(status == DIAGBUF_NOT_READY_S)
      {
        ...
        if(desc->terminate == TRUE || (stream_id == DIAG_STREAM_2))
        {
          ...
          ULogFront_RealTimePrintf(diag_debug_ulog_handle,3,"Failed to send data DCI status : %d desc->state %d diag_dci_pending %d",status, desc->state, diag_dci_pending);
        }
        else
        {   
          ...
          /* Reset NHDLC state for Stream 1 */
       	  diag_non_hdlc_encode(desc, NULL, DIAG_STREAM_1);
          ULogFront_RealTimePrintf(diag_debug_ulog_handle,3,"Moved back TX params for QXDM : send_buf.head %d send_buf.used %d Block len %d",send_buf.head, send_buf.used, send_buf.curr_data_block_header->length);
        }
      }
    }
  return status;
...
void diag_non_hdlc_encode( diag_send_desc_type *src_desc, diag_hdlc_dest_type *enc, byte stream_id)
{
  ...
  static diag_enhc_encode_state diag_non_hdlc_state[DIAG_MAX_STREAM_ID] = {DIAG_STATE_START};
  static uint16 diag_non_hdlc_length[DIAG_MAX_STREAM_ID] = {0};
  static uint16 diag_non_hdlc_total_length[DIAG_MAX_STREAM_ID] = {0};
  static uint8 *diag_non_hdlc_length1[DIAG_MAX_STREAM_ID];
  static uint8 *diag_non_hdlc_length2[DIAG_MAX_STREAM_ID];
  
  int length_current_chunk = 0;
  uint8 stream_idx;

  if((stream_id != DIAG_STREAM_1) && (stream_id != DIAG_STREAM_2))
	  return;
  
  stream_idx = DIAG_ID_TO_INDEX(stream_id);

  if(enc == NULL)
  {
  	diag_non_hdlc_state[stream_idx] = DIAG_STATE_START;
	  diag_non_hdlc_total_length[stream_idx] = 0;
    ULogFront_RealTimePrintf(diag_debug_ulog_handle,2,"diag_non_hdlc_state : Reset non_hdlc_state for stream_idx %d",stream_idx);
	  return;
  }
  ...
  if ( dest && dest_last )
  {
    ...
    switch ( diag_non_hdlc_state[stream_idx] )
    {
    ...
    case DIAG_STATE_START:
      ...
      *dest++ = DIAG_CONSUMER_API_VER_1;
      if ( dest > dest_last )
      {
        diag_non_hdlc_state[stream_idx] = DIAG_STATE_WRITE_FIRST_BYTE_LEN;
        break;
    ...
    case DIAG_STATE_WRITE_FIRST_BYTE_LEN:
      ...
      {
        /* Memorize location of length field */
        diag_non_hdlc_length1[stream_idx] = dest++;
        diag_non_hdlc_length2[stream_idx] = dest++;

        if ( dest > dest_last )
        {
          diag_non_hdlc_state[stream_idx] = DIAG_STATE_WRITE_PACKET;
          break;
      ...
      {
        diag_non_hdlc_length1[stream_idx] = dest++;
        diag_non_hdlc_state[stream_idx] = DIAG_STATE_WRITE_SECOND_BYTE_LEN;
        break;
    ...
    case DIAG_STATE_WRITE_PACKET:
      ...
      write_packet:
        ...
        diag_non_hdlc_length[stream_idx] = src_last - src + 1;

        if ( diag_non_hdlc_length[stream_idx] > (dest_last - dest + 1) )
        {
          length_current_chunk = dest_last - dest + 1;
        }
        else
        {
          length_current_chunk = diag_non_hdlc_length[stream_idx];
        }
        memscpy(dest, (dest_last - dest + 1), src, length_current_chunk);
        dest += length_current_chunk;
        diag_non_hdlc_length[stream_idx] -= length_current_chunk;
        src_desc->pkt = src + length_current_chunk;
        diag_non_hdlc_total_length[stream_idx] += length_current_chunk;

        if ( dest > dest_last )
        {
          if ( diag_non_hdlc_length[stream_idx] == 0 )
          {

            diag_non_hdlc_state[stream_idx] = DIAG_STATE_END;
    ...
    case DIAG_STATE_END:
      {
        ...
        {
          /*Go update the total length */
          *diag_non_hdlc_length1[stream_idx] = *((uint8 *)&diag_non_hdlc_total_length[stream_idx]);
          *diag_non_hdlc_length2[stream_idx] = *((uint8 *)(&diag_non_hdlc_total_length[stream_idx]) + 1);
          *dest++ = DIAG_ASYNC_CONTROL_CHAR;
          diag_non_hdlc_total_length[stream_idx] = 0;
          diag_non_hdlc_state[stream_idx] = DIAG_STATE_START;
        }
...
inline micro_diagbuffer_drain_result diagbuf_drain_micro_diagbuffer( osal_sigs_t mask )
{
...
#ifdef DIAG_RING_BUFFER_DEBUG
#define NUM_ITERATIONS_DBG 100

typedef struct
{	
	uint8 pos;
	diagbuf_header_type hdr;	
	int current_tail;
	int current_tail_index;
	int iTail_temp;
	int current_head;
	diag_send_desc_type send_desc;
	diagbuf_status_enum_type diagbuf_status;
	boolean is_chained;
}dbg_diagbuf_drain_type;

dbg_diagbuf_drain_type dbg_diagbuf_drain[NUM_ITERATIONS_DBG];
uint32 dbg_diagbuf_drain_idx=0;
#endif

void dbg_diagbuf_func(uint8 pos,diagbuf_header_type hdr,int current_tail,
	int current_tail_index,int iTail_temp,int current_head,diag_send_desc_type send_desc,diagbuf_status_enum_type diagbuf_status,boolean is_chained)
{

  #ifdef DIAG_RING_BUFFER_DEBUG
	if(dbg_diagbuf_drain_idx >=NUM_ITERATIONS_DBG)
		dbg_diagbuf_drain_idx=0; 

	
	dbg_diagbuf_drain[dbg_diagbuf_drain_idx].hdr=hdr;
	dbg_diagbuf_drain[dbg_diagbuf_drain_idx].pos=pos;
	dbg_diagbuf_drain[dbg_diagbuf_drain_idx].current_tail_index=current_tail_index;
	dbg_diagbuf_drain[dbg_diagbuf_drain_idx].current_tail=current_tail;
	dbg_diagbuf_drain[dbg_diagbuf_drain_idx].iTail_temp=iTail_temp;
	dbg_diagbuf_drain[dbg_diagbuf_drain_idx].current_head=current_head;
	dbg_diagbuf_drain[dbg_diagbuf_drain_idx].send_desc=send_desc;
	dbg_diagbuf_drain[dbg_diagbuf_drain_idx].diagbuf_status=diagbuf_status;
	dbg_diagbuf_drain[dbg_diagbuf_drain_idx].is_chained=is_chained;
	
	dbg_diagbuf_drain_idx++;
  #endif
}
...
diagbuf_status_enum_type diagbuf_drain_diag( osal_sigs_t mask )
{
  ...
  diagbuf_header_type pkt_hdr = {0,0,0,0,0,0};    /* Store the header in local variable. Initialized the structure to suppress KW errors.*/
  uint8 *tmp_pkt_hdr;
  diagbuf_status_enum_type status = DIAGBUF_READY_S;
  ...
  static diag_drain_buf_type *current = NULL; /* It is used to keep track of which buffer
                                         to drain from.  If it is NULL, should
                                         drain from the guest OS buf.*/
  static diagbuf_mpd_type *curr_upd = NULL;
  ...
  uint32 bytes_sent_from_buff = 0;      /* Tracks number of bytes sent from buffer
                                           ...
                                           threshould check
                                        */
  uint32 bytes_drained = 0;    /* Bytes drained at current instant from diagbuffer */
  ...
  {
    status = DIAGBUF_NOT_READY_S;
  ULogFront_RealTimePrintf(diag_data_ulog_handle, 1, "diagbuf_drain: diag_cx_state = %d", diag_cx_state);
  }
  else if ( DIAGBUF_ARE_BUFFERS_EMPTY() )
  {
    status = DIAGBUF_EMPTY_S;
  ULogFront_RealTimePrintf(diag_data_ulog_handle, 0 , "diagbuf_drain: Buffers are empty.");
  }
  else
  {
    ...
    */
    while ( !DIAGBUF_ARE_BUFFERS_EMPTY() &&
            diagcomm_status(DIAGCOMM_PORT_1) &&
           ...
           (send.state == DIAG_SEND_STATE_COMPLETE && chained_extended_pkt == TRUE)) )
    {
      ...
      if ( current == NULL )
      {
        ...
        ULogFront_RealTimePrintf(diag_data_ulog_handle, 1, "diagbuf_drain_diag: Start - diagbuf used (core PD)= %d", DIAGBUF_USED(current_head, current_tail, current_buff_size));
    dbg_diagbuf_func(0,pkt_hdr,current_tail,current_tail_index,iTail_temp,current_head,send,status,chained_extended_pkt);
    if(current_tail% 4 != 0)
          ERR_FATAL("diagbuf_tail 0x%x is not 4 multiple", current_tail, 0, 0);		
      }
      else if (current!= NULL)
      {
        if( current->buffer_type == DIAG_DIAGBUFFER_UPD_TYPE )
        {
          curr_upd = (diagbuf_mpd_type *)current->buf_ptr;
          ASSERT(curr_upd != NULL);
          if(curr_upd->diagbuf_mpd_mode==DIAG_PD_BUF_MODE_STREAMING)
      {
            current_buf = curr_upd->diagbuf_mpd_buf;
            current_buff_size = curr_upd->diagbuf_size;
            current_tail = curr_upd->diagbuf_mpd_tail;
            current_head = curr_upd->diagbuf_mpd_head;
        iTail_temp = current_tail;
        dbg_diagbuf_func(1,pkt_hdr,current_tail,current_tail_index,iTail_temp,current_head,send,status,chained_extended_pkt);
		/* Set the is_in_use bit here if it is not set. This will cover 2 scenarios:
		    1. If the PD was in circular buffering when the next buffer "current" was found and 
		        the client has changed to streaming, we need to set this bit before we start draining from that PD buffer.
		    2. When we find the next buffer in the else case below, we should seT the in use bit for that PD here. */
    curr_upd->is_in_use =TRUE;
            ULogFront_RealTimePrintf(diag_data_ulog_handle, 1, "diagbuf_drain_diag: Start - diagbuf used (user PD)= %d", DIAGBUF_USED(current_head, current_tail, current_buff_size));
        if(current_tail% 4 != 0)
          ERR_FATAL("diagbuf_tail 0x%x is not 4 multiple", current_tail, 0, 0);
      }
      else
      ...
      while ( (status == DIAGBUF_READY_S) && (diagcomm_io_conn[DIAGCOMM_PORT_SIO][DIAGCOMM_PORT_1].allow_flow == TRUE ) &&
      (diagcomm_io_conn[DIAGCOMM_PORT_SIO][DIAGCOMM_PORT_2].allow_flow == TRUE) )
      {
        data_available_in_buff = TRUE;

        if((current == NULL) ||(current->buffer_type == DIAG_DIAGBUFFER_UPD_TYPE))
        {
        ...
        dbg_diagbuf_func(2,pkt_hdr,current_tail,current_tail_index,iTail_temp,current_head,send,status,chained_extended_pkt);
        ...
        else if ( status == DIAGBUF_READY_S )
        {
            ...
            dbg_diagbuf_func(3,pkt_hdr,current_tail,current_tail_index,iTail_temp,current_head,send,status,chained_extended_pkt);
            if ( pkt_hdr.status == DIAGBUF_COMMIT_S )
            {
              dbg_diagbuf_func(4,pkt_hdr,current_tail,current_tail_index,iTail_temp,current_head,send,status,chained_extended_pkt);
              ...
              */
              ...
              {
                  if ( bytes_sent_from_buff >= curr_upd->diagbuf_mpd_drain_threshold )
                {
                  /* Set the flag to indicate it's time to move onto the next buffer. */
                  advance_to_next_buff = TRUE;
                }
              ...
              pad += sizeof(uint16);
              iTail_temp = current_tail;
              dbg_diagbuf_func(5,pkt_hdr,current_tail,current_tail_index,iTail_temp,current_head,send,status,chained_extended_pkt);
              ...
              {
                  curr_upd->diagbuf_mpd_tail = current_tail;
              }
            ...
            {
              ULogFront_RealTimePrintf(diag_err_log_handle, 1, "diagbuf_drain - UNCOMMIT or UNUSED %d", pkt_hdr.status);

              /* If panic mode, treat an uncommitted packet as a hole. */
              tmp_pkt_hdr = &diagbuf_buf[current_tail_index];
              tmp_pkt_hdr += sizeof(diagbuf_header_type); // Move diagbuf header
              if((*tmp_pkt_hdr == DIAG_LOG_F) && (pkt_hdr.version == 1)) // version =1 for ext. packets
                tmp_pkt_hdr += sizeof(log_ext_header_type) + DIAGBUF_NEXT_ALIGNED_BYTE(sizeof(uint16)) + sizeof(diagbuf_header_type); // move ext header + moving 0xDEAD pattern + diagbuf header
              if((*tmp_pkt_hdr == DIAG_LOG_F) && (pkt_hdr.version == 0)) { //version = 0 if it's not an extended packet
                tmp_pkt_hdr += FPOS(diag_log_rsp_type, log) + FPOS(log_hdr_type, code); // offset of `log` in log_rsp_type header + offset of `code`(for log code) in log_hdr_type
                ULogFront_RealTimePrintf(diag_data_ulog_handle, 2, "diagbuf_drain_diag: Log packet is not committed - Log code = 0x%02X 0x%02X", *(tmp_pkt_hdr + 1), *(tmp_pkt_hdr));
              }
              status = DIAGBUF_NOT_READY_S;
			  //As there is an uncommitted packet in the current buffer, set the flag for switching to the next pd.			  
			  advance_to_next_buff = TRUE;
        ULogFront_RealTimePrintf(diag_debug_ulog_handle, 1, "Uncommitted packet at %p, so moving on to the next buffer",&current_buf[current_tail_index]);
            } // End of status == UNCOMMIT_S
            else if ( pkt_hdr.status == DIAGBUF_WRAP_S )
            {
              dbg_diagbuf_func(6,pkt_hdr,current_tail,current_tail_index,iTail_temp,current_head,send,status,chained_extended_pkt);
              iTail_temp = current_tail;
              current_tail_index = DIAGBUF_CALC_IDX(current_tail, current_buff_size);
              diagbuf_clear(current_tail_index, 1, current_buf);
              DIAGBUF_WRAP(iTail_temp, current_buff_size);
              dbg_diagbuf_func(7,pkt_hdr,current_tail,current_tail_index,iTail_temp,current_head,send,status,chained_extended_pkt);
              current_tail = iTail_temp;
              if ( current == NULL )
              {
                diagbuf_tail = current_tail;
             ...
             if(current_tail% 4 != 0)
	             ERR_FATAL("current_tail 0x%x is not 4 multiple", current_tail, 0, 0);			  
              continue;
            } //end of status == WRAP_S
            else if ( pkt_hdr.status == DIAGBUF_HOLE_S )
            {
              dbg_diagbuf_func(8,pkt_hdr,current_tail,current_tail_index,iTail_temp,current_head,send,status,chained_extended_pkt);
              iTail_temp = current_tail;
              ...
              dbg_diagbuf_func(9,pkt_hdr,current_tail,current_tail_index,iTail_temp,current_head,send,status,chained_extended_pkt);
              if ( DIAGBUF_USED(current_head, iTail_temp, current_buff_size) > current_buff_size )
              {
            ...
            else // condition for checking corrupted status value
            {
              ERR_FATAL("Status byte corrupt! - status = %d", pkt_hdr.status, 0, 0);
            }
        ...
        if ( status == DIAGBUF_READY_S && data_available_in_buff == TRUE )
        {
          ...
          dbg_diagbuf_func(10,pkt_hdr,current_tail,current_tail_index,iTail_temp,current_head,send,status,chained_extended_pkt);
          ...
          if ( send.state == DIAG_SEND_STATE_COMPLETE )
          {
            ...
            dbg_diagbuf_func(11,pkt_hdr,current_tail,current_tail_index,iTail_temp,current_head,send,status,chained_extended_pkt);
          ...
          DIAGBUF_MOVE(iTail_temp, (int)((uint32)send.pkt - (uint32)start_pos), current_buff_size);
            dbg_diagbuf_func(12,pkt_hdr,current_tail,current_tail_index,iTail_temp,current_head,send,status,chained_extended_pkt);
          if ( DIAGBUF_USED(current_head, iTail_temp, current_buff_size) > current_buff_size )
          {
          ...
          else
          {
            ...
            dbg_diagbuf_func(13,pkt_hdr,current_tail,current_tail_index,iTail_temp,current_head,send,status,chained_extended_pkt);
          ASSERT(current_tail < 2 * current_buff_size);
          ASSERT(current_tail_index < current_buff_size);
            ...
            dbg_diagbuf_func(14,pkt_hdr,current_tail,current_tail_index,iTail_temp,current_head,send,status,chained_extended_pkt);
            current_tail = iTail_temp;
            if ( current == NULL )
            {
              diagbuf_tail = current_tail;
            ...
            dbg_diagbuf_func(15,pkt_hdr,current_tail,current_tail_index,iTail_temp,current_head,send,status,chained_extended_pkt);
            ASSERT(current_tail < 2 * current_buff_size);
            ASSERT(current_tail_index < current_buff_size);

              if ( current && DIAGBUF_IS_PD_KILLED(curr_upd) )
            {
              osal_lock_mutex(&diagmpd_buf_list_state_mutex);
              /* Making PD empty allows Drain to move to next PD buffer in the linked list */
              DIAGBUF_ARTIFICALY_MAKE_PD_EMPTY(curr_upd);
        ...
        dbg_diagbuf_func(16,pkt_hdr,current_tail,current_tail_index,iTail_temp,current_head,send,status,chained_extended_pkt);
        if(current_tail% 4 != 0)
          ERR_FATAL("current_tail 0x%x is not 4 multiple", current_tail, 0, 0);
        /* If we are handling a pkt that has a chained pkt,
           do not advance to the next buffer. We should drain
           the second pkt first */
        ...
        if (chained_extended_pkt == FALSE)
        {
            ...
            if ( sigs & mask )
            {
                ULogFront_RealTimePrintf(diag_data_ulog_handle, 3, "diagbuf_drain_diag: Exiting from inner while loop: diag signal is blocking - mask = 0x%X, \
                          status = %d, chained_extended_pkt = %d, current = 0x%X", mask, status, current);
        ...
        {
          while ((current !=NULL) && ( current->buffer_type == DIAG_DIAGBUFFER_TYPE) )
          {
            handle = (diagbuffer_handle)current->buf_ptr;
            result = diagbuffer_drain(handle, &bytes_drained);

            bytes_sent_from_buff+= bytes_drained;

            if((bytes_drained == 0) || (!result) || (bytes_sent_from_buff >= DIAG_CORE_PD_DRAIN_THRESHOLD))
            {
              advance_to_next_buff = TRUE;
              break;
            }
      ...
      ULogFront_RealTimePrintf(diag_data_ulog_handle, 3, "diagbuf_drain_diag: Exit inner while loop: status = %d, allow flow(Port1) = %d, allow flow(Port2) = %d",\
                                            status,
                                            diagcomm_io_conn[DIAGCOMM_PORT_SIO][DIAGCOMM_PORT_1].allow_flow,
                                            diagcomm_io_conn[DIAGCOMM_PORT_SIO][DIAGCOMM_PORT_2].allow_flow);
      /* Get signals */
      return_val = osal_get_sigs(diag_task_tcb, &sigs);
      ASSERT(OSAL_SUCCESS == return_val);
      ...
      if (chained_extended_pkt ==FALSE)
      {
        ...
        if (sigs & mask)
        {
           ULogFront_RealTimePrintf(diag_data_ulog_handle, 3, "diagbuf_drain_diag: Exiting from outer while loop: diag signal is blocking - mask = 0x%X, \
        status = %d, current = 0x%X", mask, status, current);
          break; /* Exit loop.  Something more important has happened! */
        }
      }
    } /* End of outer While loop */
    if ( current == NULL )
      ULogFront_RealTimePrintf(diag_data_ulog_handle, 1, "diagbuf_drain_diag: End - diagbuf used (core PD)= %d", DIAGBUF_USED(current_head, diagbuf_tail, current_buff_size));
    else
      ULogFront_RealTimePrintf(diag_data_ulog_handle, 1, "diagbuf_drain_diag: End - diagbuf used (user PD)= %d", DIAGBUF_USED(current_head, diagbuf_tail, current_buff_size));

    ULogFront_RealTimePrintf(diag_data_ulog_handle, 6, "diagbuf_drain_diag: Exit outer while loop: diagbufs are empty = %d, diagcomm_status = %d, allow flow(Port1) = %d, allow flow(Port2) = %d, send state = %d, chained_extended_pkt = %d",\
    DIAGBUF_ARE_BUFFERS_EMPTY(), diagcomm_status(DIAGCOMM_PORT_1), diagcomm_io_conn[DIAGCOMM_PORT_SIO][DIAGCOMM_PORT_1].allow_flow,\
    diagcomm_io_conn[DIAGCOMM_PORT_SIO][DIAGCOMM_PORT_2].allow_flow, send.state, chained_extended_pkt);
  } /* end of ELSE */
  ...
  else if ( !DIAGBUF_ARE_BUFFERS_EMPTY() && diagcomm_status(DIAGCOMM_PORT_1) && (DIAG_CHECK_MASK(diag_cx_state, DIAG_CX_ANY_S)))
  {
...
void diagbuf_mpd_ctrl_commit_threshold( unsigned int val, uint8 diagID )
{
  /* Change the commit threshold for all the UserPD's */
   ...
   while (q_cur_ptr != NULL)
   {
      if(q_cur_ptr->buffer_type == DIAG_DIAGBUFFER_UPD_TYPE)
      {
        buffer = (diagbuf_mpd_type *)q_cur_ptr->buf_ptr;
      if(diagID == buffer->diagID )
      {
        buffer->diagbuf_mpd_commit_threshold = (buffer->diagbuf_size * val) / 100;
        break;
      }
      q_cur_ptr = (diag_drain_buf_type *)q_next(&diagbuf_mpd_q, &q_cur_ptr->link);
   }
...
/*===========================================================================
FUNCTION diagbuf_add_to_drain_queue

DESCRIPTION
  Adds the diag_drain_buf_type pointer to the linked list that keeps track of
  all diag buffers that are serviced and drained by diag task.

PARAMETERS
 diagbuf_mpd_type *buf - Pointer to add to queue.

RETURN VALUE
  None

===========================================================================*/
void diagbuf_add_to_drain_queue( diag_drain_buf_type *buf )
{
  if ( buf )
  {
...
/*===========================================================================
FUNCTION diagbuf_remove_from_drain_queue

DESCRIPTION
  Removes the diag_drain_buf_type pointer from the linked list that keeps track of
  all diag buffers that are serviced and drained by diag task. This function will not
  free memory associated with the diagbuf.

PARAMETERS
 diag_drain_buf_type *buf - Pointer to remove from queue

RETURN
 None

===========================================================================*/
void diagbuf_remove_from_drain_queue( diag_drain_buf_type *buf )
{
  if ( buf )
  {
...
/*===========================================================================
FUNCTION diagbuf_add_buffer_to_drain

DESCRIPTION
  Formulates the buffer information and adds the diagbuffer handle pointer
  to the linked list that keeps track of all diag buffers that are serviced and
  drained by diag task. It simply returns if the handle is already present in
  the list.

PARAMETERS
 diagbuffer_handle h - the diagbuffer handle

RETURN
 None

===========================================================================*/
void diagbuf_add_buffer_to_drain( diagbuffer_handle h )
{
  diag_drain_buf_type* dbuf = NULL;

  if (!diagbuf_is_buffer_in_queue(h))
  {
    dbuf = (diag_drain_buf_type *)malloc(sizeof(diag_drain_buf_type));

    if ( dbuf == NULL )
    {
      ULogFront_RealTimePrintf(diag_err_log_handle, 0, "diagbuf_add_buffer_to_drain: malloc failed.");
      return;
    }
    memset(dbuf, 0, sizeof(diag_drain_buf_type));
    dbuf->buffer_type = DIAG_DIAGBUFFER_TYPE;
    dbuf->buf_ptr = (diagbuffer_handle)h;
    diagbuf_add_to_drain_queue(dbuf);
    return;
  }
  else
  {
    /* diagbuffer_handle already present in drain queue */
    return;
  }
}

/*===========================================================================
FUNCTION diagbuf_remove_buffer_from_drain

DESCRIPTION
  Removes the diagbuffer handler from the linked list that keeps track of the
  all diag buffers that are serviced and drained by diag task. This function will
  not free memory associated with the diagbuf. It simply returns if the handle is
  not present in the list.

PARAMETERS
 diagbuffer_handle h - handle of the diagbuffer

RETURN
 None

===========================================================================*/
void diagbuf_remove_buffer_from_drain( diagbuffer_handle h )
{
  diag_drain_buf_type* drain_ptr= NULL;

  drain_ptr = diagbuf_is_buffer_in_queue(h);
  if(drain_ptr)
  {
    /* Pointer to handle is found, remove it */
    diagbuf_remove_from_drain_queue(drain_ptr);
  }
  else
  {
    /* handle not in the queue, do nothing */
    return;
  }
}
...
diag_drain_buf_type* diagbuf_is_buffer_in_queue(diagbuffer_handle h)
{
    ...
    while (q_cur_ptr != NULL)
    {
      if(q_cur_ptr->buffer_type == DIAG_DIAGBUFFER_TYPE)
      {
        buffer = (diagbuffer_h_type *)q_cur_ptr->buf_ptr;
        if (buffer == h)
        {
          break;
        }
