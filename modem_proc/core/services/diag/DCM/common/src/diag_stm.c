...
/*===========================================================================
 ...
 $Header: //components/rel/core.mpss/10.0/services/diag/DCM/common/src/diag_stm.c#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
02/06/18   sa      Removed Unused APIs.
08/30/17   ph      Fix memory leak with extended log packets.
07/19/17   sa      Fix  the issue of stm packet not being freed since packet header was not copied properly.
10/12/16   sa      Added qshtrace mode change
...
boolean diag_stm_initialized = FALSE; // Indicates if diag tracer has been initialized
...
uint8 diag_stm_control_mode(uint8 mode_info)
{
  ...
  if(diag_stm_initialized)
  {
    if (mode_info == DIAG_STM_ENABLE)
    {
	  ...
	  //Call the qshtrace API to check if QSHTRACE data path need to be switched.
...
void diag_stm_log_ext_handler(void* pkt_ptr)
{
  ...
  else if(diagbuf_hdr2->version == DIAGBUF_VERSION_2 )
  {
      ...
      if(diagbuf_hdr2->status == DIAGBUF_STM_BUFFER_S)
      {
        ...
        memscpy((void *)copy_ptr,name_length,
        ...
        case DIAGBUF_VERSION_2:
        {
            ...
            if (ptr)
            {
                ...
                else
                {
                  ...
                  /*Copy the ULOG name*/
                  strlcpy(ext_pkt_hdr->ulog_name, temp_buf, (name_len +1) );
...
void diag_stm_log_free(void *ptr)
{
  ...
  if(ptr)
  {
    ...
    if (pkt_hdr->chain == DIAGBUF_SECOND_PKT_CHAIN)
    {
      ...
      /* Only version 1 and 2 supported currently */
      ...
      {
        head_ptr -= sizeof(diag_cmd_ext_v2_type);
      }
