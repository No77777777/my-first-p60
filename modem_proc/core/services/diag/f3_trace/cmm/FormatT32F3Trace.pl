...
#
# $Header: //components/rel/core.mpss/10.0/services/diag/f3_trace/cmm/FormatT32F3Trace.pl#1 $
#
...
sub CompletePreviousItem
{
    ...
    #Extract the format specifiers from the format string
	my @formatspecifiers = $item{mformat} =~ /(%[a-z]+)\s+/g;
	
	#Array that containsformat specifiers which require special handling.
	@searchspecifiers = ("%f","%llu","%lld");
	
	$index = 0 ;
	foreach my $var1 (@formatspecifiers)
	{
		my $index1 = 0;
		foreach my $var2 (@searchspecifiers)
		{
			if($var2 eq $var1)
			{
				#Formulate 64-bit argument
				@{$item{margs}}[$index] =  (@{$item{margs}}[$index]) | (@{$item{margs}}[$index+1] << 32);
				#Remove the 2nd argument from argument array
				splice(@{$item{margs}},$index+1,1);
				if($var1 eq "%f")
				{
					#Logic to display %f in proper format
					@{$item{margs}}[$index] = unpack("d>",pack("q>",@{$item{margs}}[$index]));
				}
			}
			$index1++;
		}
