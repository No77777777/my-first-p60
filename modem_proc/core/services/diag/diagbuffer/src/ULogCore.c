...
$Header: //components/rel/core.mpss/10.0/services/diag/diagbuffer/src/ULogCore.c#1 $
============================================================================*/
 ...
 */
DLL_API_GLOBAL ULogResult ULogCore_LogCreate(ULogHandle * h, const char *  logName,
                               ...
                               ULOG_INTERFACE_TYPE interfaceType )
{
  ...
  log->param.high_mark = 80;
  log->param.low_mark =  10;
  log->param.tx_mode = DIAGBUFFER_MODE_CBUFFER;
  log->param.ts_type = DIAGBUFFER_TIMESTAMP_TYPE_QTIMER;
  log->param.mask_type = 0;
  log->param.tool_filter = 0;
 ...
 */
DLL_API_GLOBAL ULogResult ULogCore_HeaderSet(ULogHandle h, char * headerText)
{
  ULOG_TYPE * log = (ULOG_TYPE *)h;
                           ...
                           ULOG_INTERFACE_TYPE interfaceType )
{
  ...
  if ((log == NULL) ||
      ...
      (msgCount > DIAGBUFFER_MAX_MSG_SIZE)
     )
  {
    return(DAL_ERROR);
  }
...
ULogResult ULogCore_SetTimestampSrcFn(ULogHandle h, ULOG_ALT_TS_SRC altULogTimeStampFn)
{
  ...
  ((ULOG_TYPE*)h)->param.ts_type = DIAGBUFFER_TIMESTAMP_TYPE_CUSTOM;
	ULogInternal_LockRelease(((ULOG_TYPE*)h));

	return DAL_SUCCESS;
}
/**
 * <!-- diagbuffer_set_mask_type -->
 *
 * @brief Allows the mask type of a diagbuffer to be altered by the user
 *
 * This mask_type remains default if the user passes invalid mask_type
 * for the 2nd parameter
 *
 * @param Handle : A diagbuffer handle.
 * @param mask_type : mask_type to set to the diagbuffer.
 *
 * @return DAL_SUCCESS if the mask_type was correctly set
 *        Errors are defined in the DIAGBUFFER_ERRORS enum.
 */
diagbuffer_result diagbuffer_set_mask_type(diagbuffer_handle Handle, uint8 mask_type )
{
  ...
  if( mask_type <= DIAGBUFFER_MAX_MASK )
  {

    ULogInternal_LockGet(log);

    log->param.mask_type = mask_type;
...
/**
 * <!-- diagbuffer_set_tool_filter -->
 *
 * @brief This sets the flag indicating if the external tool/client has enabled this
 *        DiagBuffer to transmit packets. If the flag is not enabled, then attempts to
 *        drain this DiagBuffer will be rejected.
 *
 * The setting remains default(or previous mode ) if the user passes invalid flag
 * for the 2nd parameter
 *
 * @param Handle : A diagbuffer handle.
 * @param tool_filter : flag to set to the diagbuffer.
 *
 * @return DAL_SUCCESS if the tool_filter was correctly set
 *        Errors are defined in the DIAGBUFFER_ERRORS enum.
 */
diagbuffer_result diagbuffer_set_tool_filter(diagbuffer_handle Handle, uint8 tool_filter )
{
  diagbuffer_result result = DAL_ERROR;
  DIAGBUFFER_TYPE * log = (DIAGBUFFER_TYPE *)Handle;
  ...
  if( tool_filter <= DIAGBUFFER_TOOL_FILTER_MAX )
  {

    ULogInternal_LockGet(log);

    log->param.tool_filter = tool_filter;
 ...
 *        Errors are defined in the ULOG_ERRORS enum.
 */
DLL_API_GLOBAL ULogResult ULogCore_Disable(ULogHandle h)
{
  ULOG_TYPE * log = (ULOG_TYPE *)h;
  DALResult result = DAL_ERROR;
