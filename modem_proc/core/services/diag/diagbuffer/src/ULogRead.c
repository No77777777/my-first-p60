...
Copyright (c) 2009-2013,2017 Qualcomm Technologies Incorporated.
All Rights Reserved.
Qualcomm Confidential and Proprietary

$Header: //components/rel/core.mpss/10.0/services/diag/diagbuffer/src/ULogRead.c#1 $
============================================================================*/
...
/**
 * <!-- diagbuffer_get_format_type -->
 *
 * @brief Get the type of message format of the diagbuffer.
 *
 * This function is mainly to distinguish the msgFormat of the
 * message present in diagbuffer. i.e. if the msgformat is of
 * legacy diagbuffer type, it returns the messages is leagcy
 * type else it returns the messages is of diag type.
 *
 * @param format : Format of the messages being checked.
 *
 * @return format type of the message.
 */
uint32 diagbuffer_get_format_type(uint32 format)
{
  if( format > DIAGBUFFER_SUBTYPE_RAW32 )
  {
    return DIAGBUFFER_MSG_FORMAT_DIAG;
  }
  else
  {
    return DIAGBUFFER_MSG_FORMAT_LEGACY;
  }
}
                            ...
                            uint32 outputMessageLimit)
{
  ...
  uint32 msgLength;
  uint32 msgFormat=0;
  uint32 new_msgLength;
  uint32 new_msgformat;
  char * copyPtr;
  ...
  *outputCount = 4;
         ...
         overrunDetected == 0)                                                /* we couldn't read fast enough, the log moved under our feet.*/
  {
    ...
    core_memcpy(&msgdata, 4, &log->buffer[log->readerCore->read & log->bufSizeMask], 4);
    ...
    if (ReadIsValid(log))
    {
      ...
      {
        core_memcpy(&msgdata, 4, &log->buffer[log->readerCore->read & log->bufSizeMask], 4);
        new_msgLength = (msgdata & ULOG_LENGTH_MASK) >> ULOG_LENGTH_SHIFT;
        new_msgformat = (msgdata & ~ULOG_LENGTH_MASK);
        new_msgLength = ULOGINTERNAL_PADCOUNT(new_msgLength);

        if( (msgLength != new_msgLength) || (msgFormat != new_msgformat))
        {
          CORE_MEM_BARRIER();
          log->readerCore->read = log->writerCore->readWriter;
          overrunDetected = 1;
  ...
  // Write the status here. Lower 16 bits contain the usageData flag.
  statusValue = log->writerCore->usageData;

  // Write the format type to the upper 16bits so that caller
  // of this function can determine the type of data written into the output buffer
  // on whether the format type is legacy diagbuffer or new diagbuffer types.
  statusValue |= ((msgFormat<<16) & 0xFFFF0000 );
