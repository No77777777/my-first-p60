...
$Header: //components/rel/core.mpss/10.0/services/diag/diagbuffer/src/ULogDiagFormat.h#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
05/29/17   ph      Update the enum list ulogdiag subsys command codes.
============================================================================*/
...
typedef struct
{
  ...
  char response[DIAGBUFFER_MAX_MSG_RSP_SIZE];
...
typedef struct
{
  ...
  char headerData[DIAGBUFFER_MAX_MSG_RSP_SIZE];
...
typedef struct
{
  ulogdiag_msg_header header;
  signed int result;
}ulogdiag_cmd_rsp_type_setfilter;


diagbuffer_result diagbuffer_setup_header(ULogHandle h, void* buf, uint32 size, uint32* length);
