...
$Header: //components/rel/core.mpss/10.0/services/diag/diagbuffer/src/ULogInlines.h#2 $
============================================================================*/
...
#include "diagpkt.h" //To include DIAG_MAX_TX_PKT_SIZ macro

#ifdef __cplusplus
extern "C" {
#endif

/*For clients who need diag timestamp */
...
static ULOG_INLINE DALBOOL ULogInternal_FreeSpaceForNewMsg(ULOG_TYPE * log, unsigned int requestingSize){
  ...
  while (bufferNeeded > log->bufSize)
  {
    ...
    else
    {
      ...
      //times needed to fit the largest DIAGBUFFER_MAX_MSG_SIZE message.
      //There's no log msg smaller than 8 bytes so 
      //DIAGBUFFER_MAX_MSG_SIZE/8, so any attempt to make space more than
      //DIAGBUFFER_MAX_MSG_SIZE/8 times should cause a log reset.
      freeAttempts++;

      if ((freeAttempts>DIAGBUFFER_MAX_MSG_SIZE/8) ||
          (oldMsgSize > DIAGBUFFER_MAX_MSG_SIZE) ||
          (oldMsgSize < 8)){
        ULogCore_ResetLog((ULogHandle)log);
        bufferNeeded = requestingSize;
      }
...
static ULOG_INLINE DALBOOL ULogInternal_ReadyForMessage(ULOG_TYPE * log, unsigned int msgCount)
{
  ...
  if ((log->sharedHeader == NULL) ||
      ...
      (requestingSize > DIAGBUFFER_MAX_MSG_SIZE)
     )
  {
    return(FALSE);
  }
 ...
 */
static ULOG_INLINE int ULogInternal_FastFmtSizeTS(ULogHandle h, uint32 size, uint32 msg_format, uint32* start_idx){
...
#endif
  {
    ...
    if(start_idx)
    {
        *start_idx = log->writerCore->write;
    }
    // len/fmt word
    ...
    *wordPtr = msg_format;  //keep the size zero until the msg is completely written.
    ...
    if ( log->param.ts_type != DIAGBUFFER_TIMESTAMP_TYPE_DIAG)
    {
    //Timestamp
#ifdef ULOG_64BIT_TIME
...
/*Currently updating only for root PD. If this is a user PD, the corresponding
  function pointer to be used is diag_time_get_LSM */
      ...
      if(diag_time_get)
      {
        ...
        core_memcpy(&(log->buffer[log->writerCore->write & log->bufSizeMask]),sizeof(qword),diag_ts,sizeof(qword));
      }
      else
      {
        /*Set timestamp as 0 if DIAG timestamp is not initialized */
        core_memcpy(&(log->buffer[log->writerCore->write & log->bufSizeMask]),sizeof(qword),diag_ts,sizeof(qword));
      }
      log->writerCore->write+= sizeof(qword);
