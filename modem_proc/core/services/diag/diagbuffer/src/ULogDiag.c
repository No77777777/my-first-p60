...
$Header: //components/rel/core.mpss/10.0/services/diag/diagbuffer/src/ULogDiag.c#1 $
============================================================================*/
...
uint32 oneMsgBuffer[DIAGBUFFER_MAX_MSG_SIZE/4];
 ...
 *
 */
ULogResult ULogDiagFormatPayloadASCII(ULogHandle handle, unsigned long int *format_rsp, int buf_size, char * payload_buf, unsigned long int *bytes_written)
{
    char * stringStart;
...
/**
 * <!-- ULogDiagFormatPayloadDiag -->
 *
 * @brief Formats a payload suitable for a diag response or log packet with
 *        payload type as DIAG format. Non-HDLC header is not included with the
 *        output payload of this function. If non-hdlc header is also needed,
 *        then ULogDiagFormatPayload needs to be invoked with
 *        DIAGBUFFER_FORMAT_DIAG_NHDLC option.
 ...
 *
 */
ULogResult ULogDiagFormatPayloadDiag(ULogHandle handle, unsigned long int *format_rsp, int buf_size, char * payload_buf, unsigned long int *bytes_written)
{
    ULogResult rval = ULOG_ERR_INVALIDPARAMETER;
    ...
    uint8* copy_ptr;
    uint32 hdr_len=0;
    PACK (diag_log_rsp_type*) log_rsp = NULL;
    PACK (log_hdr_type*) log_ptr = NULL;
    PACK (LOG_DIAG_ULOG_type *)log_diag_ulog_ptr = NULL;
    uint32 statusBytes =0;
    uint16 msgFormat = 0;
    uint16 cur_msg_format = 0;


    boolean flag_updated = FALSE;
    uint8 header_start = FALSE;
    ...
    *bytes_written = 0;
    ...
    else
    {
      ...
      while (1)
      {
        *format_rsp = DIAGBUFFER_FORMAT_DIAG;
        ...
        if(ULOG_ERR_OVERRUN ==
          ULogCore_ReadEx (handle,
            ...
            1))
        {
          *format_rsp   = DIAGBUFFER_FORMAT_DIAG_OVERRUN_DETECTED;
        }

        // Read one message from the log
        if (readOutputLength > 4)
        {
          core_memcpy(&statusBytes, sizeof(statusBytes), &oneMsgBuffer[0], 4);

          /*take the upper 2 bytes where format is saved in ULogCore_ReadEx() routine */
          msgFormat = (statusBytes & 0xFFFF0000) >> 16;

          if(!flag_updated)
          {
            cur_msg_format = diagbuffer_get_format_type(msgFormat);
            flag_updated = TRUE;
          }
          else
          {
            if(cur_msg_format != diagbuffer_get_format_type(msgFormat))
            {
              /*Different format messages cannot be mixed together */
              rval = DAL_SUCCESS;
              break;
            }
          }
          ...
          if(!header_start)
          {
            copy_ptr = (uint8 *)payload_buf;
            if ( DAL_SUCCESS == diagbuffer_setup_header(handle, copy_ptr, buf_size, &hdr_len))
            {
              header_start = TRUE;
              copy_ptr = (uint8 *)payload_buf;
              copy_ptr+= hdr_len;

              if(cur_msg_format == DIAGBUFFER_MSG_FORMAT_LEGACY)
              {
                //Update diag_log_rsp_type
                log_rsp = (diag_log_rsp_type *)copy_ptr;
                log_rsp->cmd_code = DIAG_LOG_F;
                log_rsp->more = 0;
                copy_ptr+= LOG_DIAGPKT_OFFSET;

                //Update log_hdr_type
                log_ptr = (log_hdr_type *)copy_ptr;
                log_set_code( log_ptr, LOG_DIAG_ULOG_PAYLOAD );
                log_set_timestamp_ext((void *)log_ptr, 0, NULL);

                log_diag_ulog_ptr = (LOG_DIAG_ULOG_type *)copy_ptr;
                log_diag_ulog_ptr->output_format = ULOGDIAG_FORMAT_ASCII;

                total_len = hdr_len + LOG_DIAGPKT_OFFSET + FPOS(LOG_DIAG_ULOG_type, buf );

                outputMsgIndex+= total_len;
              }
              else
              {
                outputMsgIndex+= hdr_len;
              }
            }
          }
          // Convert it and put it in the output buffer.
          ...
          {
            // Now how full is our output buffer
            outputMsgIndex+= strlen(&stringStart[outputMsgIndex]);
            *bytes_written = outputMsgIndex;
            ...
            {
              rval = DAL_SUCCESS;
              if(log_diag_ulog_ptr && log_rsp && log_ptr)
              {
                log_diag_ulog_ptr->output_length = outputMsgIndex - total_len;
                log_rsp->len = FPOS(LOG_DIAG_ULOG_type, buf ) + log_diag_ulog_ptr->output_length;
                log_ptr->len = FPOS(LOG_DIAG_ULOG_type, buf ) + log_diag_ulog_ptr->output_length;
              }
              break; // break from while loop
            }
          }
...
/**
 * <!-- diagbuffer_setup_nhdlc_header -->
 *
 * @brief This function sets up the NON-HDLC header at the start of the pointer
 *        passed by the client. Below is the format of the header in general:
 *        0x7E | 0x01 | <Length > (2 bytes ) |
 * @param ptr:    ptr passed by the client to populate the content.
 * @param size:   size of the buffer.
 * @param length: length indicating the bytes written.
 *
 * @return DAL_SUCCESS if successful.
 *         Error are defined in the DIAGBUFFER_ERRORS enum.
 */
diagbuffer_result diagbuffer_setup_nhdlc_header(uint8* ptr, uint32 size, uint8* length)
{
  diag_nhdlc_type* hdr=NULL;
  ...
  if(size < sizeof(diag_nhdlc_type))
  {
    return result;
  }
  ...
  if(ptr)
  {
    hdr = (diag_nhdlc_type *)ptr;
    hdr->async = 0x7E;
    hdr->version = 0x01;
    hdr->length = 0;

    *length = sizeof(diag_nhdlc_type);
    result = DAL_SUCCESS;
  }
  return result;
}
 ...
 *
 * @param handle : Pointer to ULog handle to read from
 * @param format_req : Format of payload to generate
 *                      ULOGDIAG_FORMAT_BINARY -Output data copied in binary format.
                        ULOGDIAG_FORMAT_ASCII  -Output data copied as ASCII strings.
                        DIAGBUFFER_FORMAT_DIAG -Output data copied in DIAG format
                                                excluding non-HDLC header.(Only DIAG
                                                task can append the necessary headers and
                                                send the packet out of the device).
                        DIAGBUFFER_FORMAT_DIAG_NHDLC -Output data copied in DIAG format
                                                with non-HDLC header.(This format of data
                                                is recognized by the external tools ).
 * @param format_rsp : Format of payload generated
                       (will contain OVERRUN information if needed)
 * @param buf_size : size of payload_buf
 * @param payload_buf : pointer to payload buffer where output will be written
...
ULogResult ULogDiagFormatPayload(ULogHandle handle, int format_req, unsigned long int *format_rsp, int buf_size, char * payload_buf, unsigned long int *bytes_written)
{
  ...
  {
    rval = ULogDiagFormatPayloadASCII(handle, format_rsp, buf_size, payload_buf, bytes_written);
  }
  else if(format_req == DIAGBUFFER_FORMAT_DIAG)
  {
    rval = ULogDiagFormatPayloadDiag(handle, format_rsp, buf_size, payload_buf, bytes_written);
  }
  else if(format_req == DIAGBUFFER_FORMAT_DIAG_NHDLC)
  {
    uint8* out_buf = (uint8 *)&payload_buf[0];
    diag_nhdlc_type* hdr_ptr= (diag_nhdlc_type *)&payload_buf[0];

    diagbuffer_setup_nhdlc_header(out_buf, buf_size, &hdr_len);
    out_buf+= hdr_len;

    /*NHDLC header starts 0x7E 0x01 <2 bytes length> <Payload> 0x7E, so the
    output buffer reduces to (size - hdr_len - 1 )*/
    rval = ULogDiagFormatPayloadDiag(handle, format_rsp, (buf_size - hdr_len - 1 ), (char *) out_buf, bytes_written);
    hdr_ptr->length= *bytes_written;
    out_buf+= *bytes_written;
    /* Terminating 0x7E character */
    *out_buf = 0x7E;
  }
                                       ...
                                       word pkt_len )
{
  ...
  ulogdiag_cmd_type_setfilter * setfilterCmd;
  ulogdiag_cmd_rsp_type_setfilter * setfilterRsp;
    ...
    /*  Verify that the command code is valid */
    {
      switch (command_code)
      {
        ...
        case ULOGDIAG_PLUGIN:
          ...
          rsp = (void *)pluginRsp;
          break;
        /* This is for external tool to enable the flag that indicates the tool is ready to
        accept the data from the specified diagbuffer handle*/
        case ULOGDIAG_SET_TOOL_FILTER:
          setfilterCmd = (ulogdiag_cmd_type_setfilter *)req;
          setfilterRsp = (ulogdiag_cmd_rsp_type_setfilter *) diagpkt_subsys_alloc(DIAG_SUBSYS_ULOG,
                                            (uint16) hdr->subsysCmdCode,
                                            sizeof(ulogdiag_cmd_rsp_type_setfilter));
          if(setfilterRsp == NULL)
          {
            return(NULL);
          }
          setfilterRsp->result = diagbuffer_set_tool_filter(GetInternalHandle(setfilterCmd->handle), setfilterCmd->tool_filter);
          rsp = (void *)setfilterRsp;
          break;
