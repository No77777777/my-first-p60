...
Copyright (c) 2014-2017, 2020 by QUALCOMM Technologies, Incorporated. 
...
/*===========================================================================
 ...
 $Header: //components/rel/core.mpss/10.0/services/diag/LSM/common/src/diagbuf_mpd_common.c#2 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
09/29/17   sa      Initialise diagID value.
09/20/17   sa      Added variable for previous mpd mode.
...
diagbuf_mpd_type* diagbuf_mpd_init( int client_handle, int pid)
{
  ...
  rv->diagbuf_mpd_mode             = 0;
  rv->prev_diagbuf_mpd_mode        = 0;
...
void diagbuf_mpd_deinit( int client_handle, diagbuf_mpd_type *buf )
{
  ...
  if ( buf )
  {
	...
	ULogFront_RealTimePrintf(diag_err_log_handle, 0 ,"diagbuf_mpd_deinit : Calling qurt_qdi_user_free()"); 
...
void diagbuf_mpd_cleanup(diag_qdi_opener *diag_qdi_obj_ptr)
{
  if ( diag_qdi_obj_ptr )
  {
    ...
    ULogFront_RealTimePrintf(diag_err_log_handle, 2 , "diagbuf_mpd_cleanup : Entered in diagbuf_mpd_cleanup() diag_qdi_obj_ptr=%p, mpd_buf= %p", diag_qdi_obj_ptr, diag_qdi_obj_ptr->user_pd_node_ptr);
    ...
    if ( mpd_buf ) 
    {
      ...
      */
      diagbuf_mpd_deinit(diag_qdi_obj_ptr->qdi_handle, diag_qdi_obj_ptr->user_pd_node_ptr); 
  	  ...
  	  ULogFront_RealTimePrintf(diag_err_log_handle, 1, "diagbuf_mpd_cleanup : Value of user_pd_node_ptr before making it null =%p", diag_qdi_obj_ptr->user_pd_node_ptr);
      diag_qdi_obj_ptr->user_pd_node_ptr = NULL;
    }
  }
  else 
  	{
  	  ULogFront_RealTimePrintf(diag_err_log_handle, 0, "diagbuf_mpd_cleanup : diag_qdi_obj_ptr is Null");
  	}
