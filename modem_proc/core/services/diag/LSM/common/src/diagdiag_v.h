...
/*===========================================================================
  ...
  $Header: //components/rel/core.mpss/10.0/services/diag/LSM/common/src/diagdiag_v.h#4 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
12/26/18   nk      Updated DIAGDIAG_STM_CONTROL to correct value on MPSS
...
12/20/16   tbg     Added DiagID event payloads, and qshtrace health commands
11/08/16   tbg     Added support for v2 (delayed) Qshrink4 database commands
...
#include "diagtune.h" // For EVENT_MAX_PAYLOAD_LEN
...
/*
  DIAG_MAX_DELAYED_RSP_SIZE represents max. delayed response packet size and it should be:
  + DIAG_MAX_SOCKETS_DATA_BLOCK_SIZE - Max. socket data block size supported.
  - 256B (to accomodate  - an existing 5B N-HDLC hdr, 
                           4B tag hdr containing Diag ID,
                           and few more bytes to accomodate any meta/hdr info in future.)
						   
  MIN macro is used just to be on the safe side in case if the (DIAG_SEND_BUF_SIZE_CMD/2) ever be smaller than the DIAG_MAX_SOCKETS_DATA_BLOCK_SIZE size.						   
 */
#define DIAG_MAX_DELAYED_RSP_SIZE (MIN((DIAG_SEND_BUF_SIZE_CMD/2), (DIAG_MAX_SOCKETS_DATA_BLOCK_SIZE)) - 0x100)
...
typedef struct
{
  /* More number of entries here and hence has to be uint16 instead of uint32 */
  /* Index value will not be more than 4000 */
  uint16 index_to_v4_ssid_table;
...
/* Qshrink4 delayed task defines */
#define DIAG_QSHRINK_4_SLEEP_SIG     0x200000
#define QSHRINK_BUFFER_SIZE 16000
#define STRESS_QSHRINK_4_STACK_SIZE (STRESS_TEST_STACK_SIZE)

/* Maximum event payload for stress test */
#define MAX_STRESS_TEST_EVENT_PAYLOAD_LEN  (EVENT_MAX_PAYLOAD_LEN - FPOS(EVENT_STRESS_TEST_RANDOM_SIZE_type,payload))

/* Maximum log payload for stress test */
#define MAX_STRESS_TEST_LOG_PAYLOAD_LEN  (LOG_MAX_PAYLOAD_LEN - FPOS(LOG_STRESS_TEST_RANDOM_SIZE_type, payload))

/* Maximum extended log payload for stress test */
#define MAX_STRESS_TEST_LOG_EXT_PAYLOAD_LEN  (MAX_STRESS_TEST_LOG_PAYLOAD_LEN  - LOG_EXT_OFFSET_SIZE)
...
#define DIAGBUF_ALLOC_REQUEST_QSHTRACE  0x4
...
#define DIAGDIAG_GET_TIME_API_MODEM            (DIAGDIAG_SUBSYS_ID_MODEM_OFFSET + 0x0017) /* 0x0817 = 2071 */
...
#define DIAGDIAG_RESET_DROP_COUNT_QSHTRACE_MODEM (DIAGDIAG_SUBSYS_ID_MODEM_OFFSET + 0x0021) /* 0x821 = 2081 */
#define DIAGDIAG_GET_DROP_COUNT_QSHTRACE_MODEM	 (DIAGDIAG_SUBSYS_ID_MODEM_OFFSET + 0x0022) /* 0x822 = 2082 */
#define DIAGDIAG_GET_ALLOC_COUNT_QSHTRACE_MODEM	 (DIAGDIAG_SUBSYS_ID_MODEM_OFFSET + 0x0023) /* 0x823 = 2083 */

//DIAG query drain parameters
#define DIAGDIAG_QUERY_DRAIN_PARAM_MINMAX_MODEM       (0x081C) /* 0x81C = 2076 */  
#define DIAGDIAG_QUERY_DRAIN_PARAM_MODEM              (0x081D) /* 0x81D = 2077 */
#define DIAGDIAG_QUERY_MODE_MODEM                     (0x081E) /* 0x81E = 2078 */
#define DIAGDIAG_SET_DRAIN_PARAM_MODEM                (0x081F) /* 0x81F = 2079 */
#define DIAGDIAG_START_STRESS_TEST_ADV_MODEM_F        (DIAGDIAG_SUBSYS_ID_MODEM_OFFSET + 0x0020) /* 0x0820 = 2080 */
...
#define DIAG_DCM_CMD_REG_TEST_OFFSET           DIAG_DCM_CMD_REG_TEST_MODEM_OFFSET
...
#define DIAGDIAG_STM_CONTROL                   0x0214 /* 526 */ 
...
typedef enum {
 ...
 /* Shorter logs at 43... */
  ...
  DIAGDIAG_STRESS_TEST_LOG_RANDOM_SIZE = 112,
  /* Test cases for extended messages */
  DIAGDIAG_STRESS_TEST_MSG_EXT_V1 = 113,
  DIAGDIAG_STRESS_TEST_MSG_1_EXT_V1 = 114,
  DIAGDIAG_STRESS_TEST_MSG_2_EXT_V1 = 115,
  DIAGDIAG_STRESS_TEST_MSG_3_EXT_V1 = 116,
  DIAGDIAG_STRESS_TEST_MSG_4_EXT_V1 = 117,
  DIAGDIAG_STRESS_TEST_MSG_9_EXT_V1 = 118,
  DIAGDIAG_STRESS_TEST_ULOG = 119,
  /* Test cases for mtrace diagbuffer */  
  DIAGDIAG_STRESS_TEST_MTRACE = 127,
  DIAGDIAG_STRESS_TEST_MTRACE_3 = 128,
  DIAGDIAG_STRESS_TEST_MTRACE_9 = 129,
  DIAGDIAG_STRESS_TEST_MSG_64BIT =130,
  DIAGDIAG_STRESS_TEST_LOG_RANDOM_SIZE_EXT= 131,
  DIAGDIAG_STRESS_TEST_EVENT_RANDOM_SIZE = 132,
  DIAGDIAG_STRESS_TEST_EVENT_RANDOM_SIZE_EXT = 133,
  /* Macro with enum 134 is applicable only for system diagbuffer, not for DIAGBUFFER and Mtrace buffer */
  DIAGDIAG_STRESS_TEST_PKT_RANDOM = 134,
  DIAGDIAG_STRESS_TEST_LOG_MAX_SIZE = 135,
  DIAGDIAG_STRESS_TEST_LOG_EXT_MAX_SIZE = 136,
  DIAGDIAG_STRESS_TEST_LAST
...
typedef struct{
  ...
  uint8 buf_id;                             /* Buffer type in which data is allocated */
...
typedef struct {
  ...
  uint32 offset; 	
  uint32 length;
} diag_qshrink_4_delayed_task_info;

/* Stores the info of the task, tcb, status and stack */
typedef struct{
  diag_stress_test_status_enum_type status; /* refer to diag_stress_test_status_enum_type*/
  diag_qshrink_4_delayed_task_info task_info;        /* Contains info of the task */
  osal_tcb_t tcb;                         /* used to define a task */
  unsigned long long stack[STRESS_QSHRINK_4_STACK_SIZE];    /* stack needed by the task */
  int unique_id;                          /* Unique identifier to help differentiate tasks */
} diag_qshrink_4_delayed_tcb_stack_type;
...
typedef struct
{
  ...
  diag_qshrink_4_delayed_tcb_stack_type data; /* data block */
...
/* These  are the buffer IDs in which the diag packets
are allocated with the stress test commands */

/*DIAGDIAG_BUFFER_SYSTEM indicates the diag packets are allocated in
system diag buffer and uses the legacy or existing APIs */
#define DIAGDIAG_BUFFER_SYSTEM               0xFF

/*DIAGDIAG_BUFFER_PRIV_SYSTEM indicates the diag packets are allocated in
system diag buffer but uses the new diagbuffer macros/APIs. This is to test
backward compatibility of old functionality with new APIs */
#define DIAGDIAG_BUFFER_PRIV_SYSTEM          0x0

/*DIAGDIAG_BUFFER_CUSTOM indicates the diag packets are allocated in
custom diagbuffer(ULOG) and drained from them appropriately. Currently we
are allowing only two test diagbuffers namely "DIAG_STRESS_1" and "DIAG_STRESS_2".
These buffers remain created since diagbuffer(ULOG) doesn't support deleting the
buffer in a clean way.  */
#define DIAGDIAG_BUFFER_CUSTOM_1               0x1
#define DIAGDIAG_BUFFER_CUSTOM_2               0x2
#define DIAGDIAG_BUFFER_CUSTOM_MAX             0x4
...
typedef PACK(struct){
  ...
  int task_priority;     /* stores the priority of the current task */
  int req_iterations;    /* stores num of iterations requested */
  uint8 payload[1];      /* Total log size: randomly at run time */
}LOG_STRESS_TEST_RANDOM_SIZE_type;

typedef PACK(struct){
  int iteration;         /* stores which iteration */
  int task_priority;     /* stores the priority of the current task */
  int req_iterations;    /* stores num of iterations requested */
  uint8 payload[1];      /* Total event size: randomly at run time */
}EVENT_STRESS_TEST_RANDOM_SIZE_type;
...
typedef struct{
  ...
  int task_priority;     /* stores the priority of the current task */
  int req_iterations;    /* stores num of iterations requested */
  uint32 payload[MAX_STRESS_TEST_LOG_PAYLOAD_LEN/4];  /* Total log size: 12 + MAX_STRESS_TEST_LOG_PAYLOAD_LEN */
}LOG_STRESS_TEST_LOG_MAX_SIZE_type;
...
typedef struct{
  ...
  int task_priority;     /* stores the priority of the current task */
  int req_iterations;    /* stores num of iterations requested */
  uint32 payload[MAX_STRESS_TEST_LOG_EXT_PAYLOAD_LEN/4];  /* Total log size: 12 + MAX_STRESS_TEST_LOG_EXT_PAYLOAD_LEN */
}LOG_STRESS_TEST_LOG_EXT_MAX_SIZE_type;
...
typedef struct {
  ...
  uint8 version;                     /* Verison */
  uint8 buf_id;                      /* Unique diagbuffer ID*/
  uint8 rsvd[2];                     /* Reserved */
  diag_per_task_test_info test[1];   /* Place holder for per task info */
} DIAGDIAG_STRESS_TEST_ADV_req_type;
typedef DIAGDIAG_STRESS_TEST_ADV_req_type DIAGDIAG_STRESS_TEST_ADV_rsp_type;
...
typedef PACK(struct) {
  ...
  uint32 file_len;
  uint8 diagID; 
}diag_qshrink_4_file_info_v3_type;
...
typedef PACK(struct) {
  uint8 status;
  uint8 num_files;
  diag_qshrink_4_file_info_v3_type file_info[0];
} diag_qshrink_4_file_list_rsp_v3_type;
...
typedef PACK(union)
{
  ...
  diag_qshrink_4_file_list_rsp_v3_type file_list_v3_rsp;  
...
#define DIAG_QSHRINK_CMD_VERSION_2 2
#define DIAG_QSHRINK_CMD_VERSION_3 3
/*==========================================================================
...
PACKET   DIAGDIAG_SET_DRAIN_PARAM_MODEM

PURPOSE  Request sent from the DM to the DMSS to set the drain paramerters.

NOTE  The structure member "params" indicates the parameters that are configured 
      through this command. Below are the parameters that can be configured through
	  this command and the corresponding bit mask values that need to be set.
	  
      DIAGBUF_COMMIT_THRESHOLD    ->   0x0001
      DIAG_DRAIN_TIMER_LEN        ->   0x0002
      EVENT_STALE_TIMER_LEN       ->   0x0004
      EVENT_REPORT_PKT_LEN_SIZE   ->   0x0008
      EVENT_SEND_MAX_NUM          ->   0x0010
      EVENT_REPORT_PKT_SIZE       ->   0x0020
	  
	  For example, if the bit mask value is set to 0x0005 in command request then 
	  it indicates that we are configuring 	DIAGBUF_COMMIT_THRESHOLD and 
	  EVENT_STALE_TIMER_LEN through this command. In this case we need to pass 
	  only  DIAGBUF_COMMIT_THRESHOLD and EVENT_STALE_TIMER_LEN values in the 
	  request packet after bitmask field.
============================================================================*/
...
typedef PACK(struct) {
  ...
  uint16 bitmask;
  uint32 params[6];
}diagdiag_set_drain_param_req_type;
...
typedef PACK(struct) {
  ...
  uint16 bitmask;
  uint32 params[6];
}diagdiag_set_drain_param_rsp_type;


/*==========================================================================

PACKET   DIAGDIAG_QUERY_MODE_MODEM

PURPOSE  Request sent from the DM to the DMSS to query the diag mode(Real Time |Non-Real Time).
============================================================================*/

typedef PACK(struct) {
  diagpkt_subsys_header_type header;
  uint8 cmd_version;
} diagdiag_query_mode_req_type;
...
typedef PACK(struct) {
  ...
  uint8 status;
  uint8 mode;
} diagdiag_query_mode_rsp_type;


/*==========================================================================

PACKET   DIAGDIAG_QUERY_DRAIN_PARAM_MIN_MAX_MODEM

PURPOSE  Request sent from the DM to the DMSS to query the drain paramerters 
...
typedef PACK(struct) {
  ...
  uint8 cmd_version;
  uint8 mode;
} diagdiag_query_drain_param_minmax_req_type;
...
typedef PACK(struct) {
  ...
  uint8 reserved;
  uint32 diagbuf_size;
  uint32 diagbuf_commit_threshold_min;
  uint32 diagbuf_commit_threshold_max;
  uint32 drain_timer_max;
  uint32 drain_timer_min;
  uint32 event_stale_timer_max;
  uint32 event_stale_timer_min;
  uint32 event_queue_len_max;
  uint32 event_queue_len_min;
  uint32 max_event_send_max;
  uint32 max_event_send_min;
  uint32 event_rpt_size_max;
  uint32 event_rpt_size_min;
} diagdiag_query_drain_param_minmax_rsp_type;


/*==========================================================================

PACKET   DIAGDIAG_QUERY_DRAIN_PARAM_MODEM

PURPOSE  Request sent from the DM to the DMSS to query the drain paramerters
         that are presently set.
============================================================================*/

typedef PACK(struct) {
  diagpkt_subsys_header_type header;
  uint8 cmd_version;
}diagdiag_query_drain_param_req_type;
...
typedef PACK(struct) {
  ...
  uint8 status;
  uint16 reserved;
  uint32 diagbuf_commit_threshold;
  uint32 drain_timer;
  uint32 event_stale_timer;
  uint32 event_queue_len;
  uint32 max_event_send;
  uint32 event_rpt_size;
}diagdiag_query_drain_param_rsp_type;
...
void diag_ctrl_send_pd_status(int pid, uint8 status);
