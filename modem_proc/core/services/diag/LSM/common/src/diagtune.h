...
Copyright (c) 2001-2006,2009-2014, 2018 by QUALCOMM Technologies, Incorporated.
All Rights Reserved.
===========================================================================*/
...
$Header: //components/rel/core.mpss/10.0/services/diag/LSM/common/src/diagtune.h#2 $ $DateTime: 2020/01/20 06:30:52 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
11/20/18   vg      Enhancements to stress test framework
09/18/18   kdey    Removed signaling diag task at island exit callback.
11/26/18   vg      Added support for max size(15.5KB) log packet
11/20/18   vg      Enhancements to stress test framework
09/18/18   kdey    Removed signaling diag task at island exit callback.
...
/* Timer to drain data from micro diagbuffer. The length for the timer is kept to be zero. We want to be notified immediately when island exit
   happens and drain the micro diagbuffer. Interrupts are enabled only when island exit is complete, so the interrupt will be handled immediately 
   after island exit is complete and we should start draining the micro diagbuffer. This will make sure that there is no context switch from sleep
   to diag task before island stage 2 exit. */
#define DIAG_MICRO_DIAGBUFFER_DRAIN_TIMER_LEN 0
...
#define EVENT_MAX_PAYLOAD_LEN 255

#define LOG_DIAGPKT_OFFSET 4 /* FPOS(diag_log_rsp_type, log) */

/*Max payload data item length for logs */
#define LOG_MAX_PAYLOAD_LEN (DIAG_MAX_TX_PKT_SIZ - LOG_DIAGPKT_OFFSET)

/* Offset_size = V1 Header (8 ) + DEAD (2) + Pad (2) + Diagbuf Header (4) */
#define LOG_EXT_OFFSET_SIZE 16 
...
/* Time(in ms) for which lsm_rx_thread has to be suspended in diagpkt_commit_dcm before retrying allocation
   in case of memory allocation failure */
#define DIAG_RSP_ALLOC_RETRY_TIMER_LEN    200
