...
02/11/08   sa      Removed Unused Signals.
...
12/20/16   tbg     Added suppot for diagID
...
06/27/16   ph      Updated the send buffer pad length for NRT mode to avoid
                   16k blocks to be created.
...
08/16/15   vk      Grabbing unused signal for micro_diagbuffer_timer
...
#define DIAG_MICRO_DIAGBUFFER_UIMAGE_SIG 0x00000010 /* Since DIAG_MC_CMD_SIG is not used
...
#define DIAG_CMD_READ_COMPLETE_SIG  0x00400000

#define DIAG_SIO_TOUT_TIMER_SIG       0x00100000    /* This signal is a timer
                                                       signal for timeout if sio
                                                       callback is not called */

#define DIAG_DRAIN_TIMER_SIG          0x00200000    /* Signal to send TX traffic to SIO/SMD layer*/
...
#define DIAG_CMD_READ_TOUT_SIG       0x80000000
...
#define DIAG_INT_UPDATE_PD_STATUS       0x00000800    /* Signal to notify DIAG to send pd status to apps*/ 
...
#define DIAG_INT_SEND_BT_MASK_SIG          0x00200000    /* Signal to send BT mask to master diag */
...
#ifndef DIAG_SEND_BUF_SIZE_DATA_WLAN
#define DIAG_SEND_BUF_SIZE_DATA_WLAN  0x8000 /* Buffer size for F3s/logs/events sent from WLAN*/
#endif

#ifndef DIAG_SEND_BUF_SIZE_DATA_DCI
#define DIAG_SEND_BUF_SIZE_DATA_DCI 0x8000  /* Buffer size for F3s/logs/events sent over DCI*/
#endif

#define DIAG_SEND_BUF_SIZE_CMD ((DIAG_MAX_SOCKETS_DATA_BLOCK_SIZE) * 2) /* Buffer size for Commands*/
...
typedef struct
{
  ...
  uint8 stream_id : 5;      /* stores the stream ID; MSB (value = 16) is set to indicate
                               that the pkt is from User PD*/
