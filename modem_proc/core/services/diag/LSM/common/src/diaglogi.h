...
Copyright (c) 2007-2014, 2017-2018 by Qualcomm Technologies, Incorporated.  All Rights Reserved.
...
10/22/18   vg      Removed packed void * parameters
08/28/18   sp      Expose global variables to detect diag services enabled for F3/Logs/Events
...
03/08/17   rs      removed the arguments of log_mask_ssm()
...
extern log_ext_hdr_type log_ext_header_type;
...
#ifdef FEATURE_SSM
/* removed both the arguments 'diag_mask_ssm_buffer_type* diag_log_codes_range' and 'uint32 size' of this function as we did it for 
   event_mask_ssm() to avoid race condition in diag task and event_mask_ssm. 
   
   Now, directly using the global variables 'diag_mask_ssm_buffer_type *diag_log_mask_range' and 'uint32 num_log_code_ssm' for same purpose*/
void log_mask_ssm();
#endif
...
boolean log_mask_enabled( const byte *xx_mask_ptr, unsigned int xx_id, unsigned int xx_item );

log_code_type log_set_equip_id (unsigned int code, unsigned int equip_id);
