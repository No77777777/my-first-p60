...
Copyright (c) 2007-2019 by QUALCOMM Technologies, Incorporated. All Rights Reserved.
All Rights Reserved.
...
$Header: //components/rel/core.mpss/10.0/services/diag/LSM/qurt/src/Diag_LSM_Pkt.c#2 $
...
===========================================================================*/
DLL_API_GLOBAL diagpkt_cmd_code_type
...
===========================================================================*/
DLL_API_GLOBAL diagpkt_subsys_id_type
...
DLL_API_GLOBAL diagpkt_subsys_get_cmd_code (PACKED void *ptr)
{
...
===========================================================================*/
DLL_API_GLOBAL diagpkt_subsys_status_type 
...
SIDE EFFECTS
  None
===========================================================================*/
DLL_API_GLOBAL diagpkt_subsys_delayed_rsp_id_type 
...
diagpkt_alloc (diagpkt_cmd_code_type code, unsigned int length)
{
    ...
    diag_data* pdiag_data = NULL;
	 ...
	 if(length <  sizeof(diagpkt_hdr_type))
	  {
  		 ULogFront_RealTimePrintf(diag_debug_ulog_handle, 2, "diagpkt_alloc : Invalid length request for response: %d cmd_code: %d", length, code);
		 return NULL;
      }
...
===========================================================================*/
DLL_API_GLOBAL void diagpkt_delay_commit (PACKED void *pkt)
{
    if (pkt)
    {
              ...
              diagpkt_subsys_cmd_code_type code, unsigned int length)
{
  ...
  {
     ULogFront_RealTimePrintf(diag_debug_ulog_handle, 3, "diagpkt_subsys_alloc : Invalid length request for response: %d  subsys_id: %d subsys_cmdcode: %d", length, id, code);
	 return NULL;
  }
              ...
              diagpkt_subsys_cmd_code_type code, unsigned int length)
{
  diagpkt_subsys_hdr_type_v2 *hdr = NULL;
   ...
   {
       ULogFront_RealTimePrintf(diag_debug_ulog_handle, 3, "diagpkt_subsys_alloc_v2 : Invalid length request for response: %d susbsys_id: %d subsys_cmdcode: %d", length, id, code);
       return NULL;
		 ULogFront_RealTimePrintf(diag_debug_ulog_handle, 3, "diagpkt_subsys_alloc_v2 : Invalid length request for response: %d susbsys_id: %d subsys_cmdcode: %d", length, id, code);
		 return NULL;
   }
              ...
              unsigned int length)
{
  diagpkt_subsys_hdr_type_v2 *hdr = NULL;
   ...
   if(length < sizeof(diagpkt_subsys_hdr_type_v2))
   {
       ULogFront_RealTimePrintf(diag_debug_ulog_handle, 4, "diagpkt_subsys_alloc_v2_delay : Invalid length request for response: %d subsys_id: %d subsys_cmdcode: %d delayed_rsp_id: %d", length, id, code, delayed_rsp_id_arg);
	   return NULL;
   }
