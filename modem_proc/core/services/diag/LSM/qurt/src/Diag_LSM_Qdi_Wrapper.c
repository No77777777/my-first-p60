...
09/18/17   sa      Make QDI call to drain events in userpd heap.
...
inline int diag_lsm_clear_drain_timer( void )
{
  int return_val = 0;
  return_val = qurt_qdi_handle_invoke(diag_qdi_handle, DIAG_QDI_CLEAR_DRAIN_TIMER);
...
inline int diag_lsm_set_event_stale( void )
{
  int return_val = 0;
  return_val = qurt_qdi_handle_invoke(diag_qdi_handle, DIAG_QDI_SET_EVENT_STALE);

  return (return_val);
}

inline int diag_lsm_clear_event_stale_timer( void )
{
  int return_val = 0;
  return_val = qurt_qdi_handle_invoke(diag_qdi_handle, DIAG_QDI_CLEAR_EVENT_STALE);
