 ...
 * layer for QURT.
 * Copyright (C) 2011-2015, 2017-2018 QUALCOMM Technologies, Incorporated.  All Rights Reserved.
 *
 *
 ***********************************************************************/
...
/*===========================================================================
  ...
  $Header: //components/rel/core.mpss/10.0/services/osal/qurt/src/osal_qurt.c#1 $ $DateTime: 2019/04/24 00:03:26 $ $Author: pwbldsvc $

when         who   what, where, why
----------   ---   ---------------------------------------------------------
2018-09-18    kdey Removed signaling diag task at island exit callback.
2018-09-03    gn   Diag timers categorization into deferrable and non-deferrable
2018-06-25    kdey Fixed reordering of assembly instructions in diag_time_init
2018-05-25    gn   Changed the retry mechanism in diagpkt_commit_dcm to suspend current task 
                   for some interval instead of waiting on a signal
2018-01-11  kdey   Resolved compile errors by removing unused variables
2017-07-12    gn   OSAL APIs updated to use ATS timers instead of qurt timers
2015-05-11    vk   fixed race condition in osal_thread_self()
2015-03-05    vk   Fixed unused variable compilation warning for diag_timer_mutex
2014-06-03    ph   Resolve warnings
...
/* Called just once by RCINIT. It initializes the mutexes.
it can be used in future for other initializations */
void osal_qurt_init (void);

static int thread_count = 0;

#ifndef OSAL_CORE_PD
/* Common timer group for deferrable diag timers */
static osal_timer_group diag_deferrable_timer_group;
/* Common timer group for non-deferrable diag timers */
static osal_timer_group diag_non_deferrable_timer_group;
...
/*!
  ...
  @sideeffects None */
/*==========================================================================*/
DLL_API_GLOBAL void osal_qurt_init (void)
{
  ...
  if (!initialized)
  {
    ...
    /* Set diag_deferrable_timer_group property to be deferrable */
    timer_group_set_deferrable(&diag_deferrable_timer_group,TRUE);

    /* Set diag_non_deferrable_timer_group property to be non-deferrable */
    timer_group_set_deferrable(&diag_non_deferrable_timer_group,FALSE);
  }
...
/*!
@brief Creates a timer in the diag_deferrable_timer_group which is defferable.
...
int osal_create_timer(osal_timer_t *timer,  osal_tcb_t *tcb_ptr, osal_sigs_t sig)
{
    ...
    if (timer && tcb_ptr )
    {
        ...
        /*Passing diag_deferrable_timer_group to timer_def_osal creates
        deferrable timer */
        time_err= timer_def_osal(timer, &diag_deferrable_timer_group, TIMER_NATIVE_OS_SIGNAL_TYPE,
...
/*!
    @brief
       ...
       This timer will be created in the diag_deferrable_timer_group
       which is defferable.
                ...
                osal_timer_cb_type timer_cb_ptr, osal_timer_param_type cb_param)
{
    ...
    if (timer && timer_cb_ptr)
    {
        ...
        /*Passing diag_deferrable_timer_group to timer_def_osal creates
        deferrable timer */
        time_err= timer_def_osal(timer, &diag_deferrable_timer_group, TIMER_FUNC1_CB_TYPE,
...
/*!
  @brief
    Creates a timer in the diag_non_deferrable_timer_group,
                            ...
                            osal_sigs_t sig)
{
    ...
    if (timer && tcb_ptr )
    {
        ...
        /*Passing diag_non_deferrable_timer_group to timer_def_osal creates non-deferrable timer */
        time_err= timer_def_osal(timer, &diag_non_deferrable_timer_group,
...
/*!
  @brief
       ...
       This timer will be created in the diag_non_deferrable_timer_group, 
       which is made non-defferable.
               ...
               osal_timer_cb_type timer_cb_ptr, osal_timer_param_type cb_param)
{
    ...
    if (timer && timer_cb_ptr)
    {
        ...
        /*Passing diag_non_deferrable_timer_group to timer_def_osal creates non-deferrable timer */
        time_err= timer_def_osal(timer, &diag_non_deferrable_timer_group, TIMER_FUNC1_CB_TYPE,
                                    timer_cb_ptr, cb_param);
...
/*===========================================================================
  FUNCTION   OSAL_THREAD_SLEEP */

/*!
  @brief
       Suspends the current task for specified time.
       
  @param[in] timer_val      - Sleep timer value
  @param[in] timer_type     - Timer unit (sec, ms, us)
  @param[in] non_deferrable - TRUE  : Task will surely resumed at specified time
                              FALSE : If Proc goes into sleep at the time of
                              specified timeout, task will resume only after 
                              Proc wakesup
...
void osal_thread_sleep(uint32 timer_val, timer_unit_type timer_type, boolean non_deferrable)
{
  timer_sleep(timer_val, timer_type, non_deferrable);
}
