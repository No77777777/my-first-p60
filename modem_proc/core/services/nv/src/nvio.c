...
$Header: //components/rel/core.mpss/10.0/services/nv/src/nvio.c#1 $
...
)
{
  ...
  item_size = NV_ROAMING_LIST_HEADER_SIZE +
                 nv_max_size_of_roaming_list_ext(cmd_ext_ptr->context);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
      ...
      default:
        ...
        if (nvim_op_get_presence(cmd_ext_ptr->nvcmd->item)) {
          ...
          /* P60A16 for write only once by duyi 20260709 begin*/
          /*if MEID with value 0 ,return NV_NOTACTIVE_S*/
          if(cmd_ext_ptr->nvcmd->item == NV_MEID_I)
          {
            ...
            if (status == NV_DONE_S )
            {
              nvmeid = (nv_item_type*)(cmd_ext_ptr->nvcmd->data_ptr);
              if(qw_cmp(nvmeid->meid, meid_0) == 0)
              {
                MSG_HIGH ("nvio_read:NV_MEID_I = 0,return NV_NOTACTIVE_S",0,0,0);
                status = NV_NOTACTIVE_S;
              }
            }
          }
          /* P60A16 for wirte imei once by duyi 20260709 end*/
        }
...
{
  nv_stat_enum_type status;   /* Function return value */
  /*Delete for always enable NV writable (cp_founction 1)by duyi 20260709 begin*/
  #if 0
  dword local_esn = 0;            /* tmp buffer for esn */
  #endif
  /*Delete for always enable NV writable (cp_founction 1)by duyi 20260709 end*/
  ...
  #endif
  /*Delete for always enable NV writable (cp_founction 1)by duyi 2026709 end*/
...
)
{
  ...
  int i;
  #endif
  /*Delete for always enable NV writable (cp_founction 1)by yangjian 20260709 end*/
  /*Delete for always enable NV writable (cp_founction 1)by yangjian 20260709 begin*/
  #if 0    
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
      ...
      case NV_ESN_I:
        status = nvio_write_esn(cmd_ext_ptr);
        break;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
      ...
      case NV_MEID_I:
        status = nvio_write_meid(cmd_ext_ptr);
        break;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
//add for make sure NV_FACTORY_DATA write in NV (cp_founction 2)by duyi 20260709 Start
      case NV_FACTORY_DATA_3_I:
      case NV_FACTORY_DATA_2_I:
      /* All other items get generic treatment, if they have a valid entry */
        if (nvim_op_get_presence(cmd_ext_ptr->nvcmd->item)) {
          array_size = nvim_op_get_array_size(cmd_ext_ptr->nvcmd->item);
          if (array_size == 0) {
            status = nvio_write_item(cmd_ext_ptr->nvcmd->item,
        ...
        uint32 i;
        if(efs_sync_no_wait("/",&i)<0){
            ERR("nvw_write efs_sync_no_wait fail", 0, 0, 0);
        } else {
            ERR("nvw_write efs_sync_no_wait success", 0, 0, 0);
        }
        break;
        //add for make sure NV_FACTORY_DATA write in NV (cp_founction 2)by duyi 20260709 end 
      default:
