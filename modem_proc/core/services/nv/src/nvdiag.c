...
Copyright (c) 1990-2015, 2018 by QUALCOMM Technologies, Incorporated.  All Rights Reserved.
*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*/
...
$Header: //components/rel/core.mpss/10.0/services/nv/src/nvdiag.c#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
11/04/15   ph      1x Core dependency removal from NV.
11/14/14   sri     Fixed buffer overflow on read of NV# whose size is more than 128 bytes
...
)
{
  ...
  if (parameter_changed) {
    nv_1xcp_cb(NV_MC_CLEAR_REG_F, NULL, NULL);    /* clear sid_nid_list and registered flag, so   */
...
)
{
  ...
  /* P60A16 for duyi 20260709 begin */
  #if 0
  /*-----------------------------------------------------------------------
...
#endif
/*P60A16 by duyi 20260709 end */
/*P60A16 by duyi 20260709 begin */ 
/* The write is ok, let it through */
  ok_to_write = TRUE;
  if ((eItem == NV_MEID_I )&&(nv_1xcp_cb != NULL))
  {
    boolean nv_1xcp_status = FALSE;
    qword meid;         /* Mobile Equipment Identifier */
    qword meid_0;       /* MEID with value 0 */
    ...
    nv_item_type        ItemDatatemp;    /* temp Item Data */
    nv_stat_enum_type   nv_status;    /* nv status */
    /* initialize meid_0 to a value zero */
    qw_set(meid_0, 0L, 0L);   
    /* check the values we are attempting to write into the MEID */
    /* Writing MEID with a value 0 is not allowed */
    if (!(qw_cmp(pItemData->meid, meid_0) == 0)) 
    {
      ...
      {
        MSG_MED("NV_MC_COMPUTE_PESN_F Failed ", 0,0,0);
      }
    }
  }
  /* P60A16 for write imei only once by duyi 20260709 end */
  if (!ok_to_write) 
  {
    /*-----------------------------------------------------------------------
