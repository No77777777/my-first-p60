...
Copyright (c) 2005-2016,2018 by QUALCOMM Technologies, Incorporated.
All Rights Reserved.
...
$Header: //components/rel/core.mpss/10.0/services/nv/src/nvim.c#1 $ $DateTime: 2019/04/24 00:03:26 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ---------------------------------------------------------
11/27/18   ph      Ensure to initialize the nv_access_op_cs mutex only once.
05/05/16   ph      NV to pass context information in existing APIs.
03/03/16   ph      NV Changes to pass sub ID to NVRUIM callback.
...
#define NV_SSM_CALLER_CONV(x)  (x+1)
  ...
  factory commands.*/
#define NV_DEFAULT_WHITELIST "0|8000|0|"

/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
/*                                                                         */
/*                         DATA DEFINITIONS                                */
...
/* Flag to indicate mutex init has begun */
static int nv_cs_init_begin = FALSE;
...
static nv_validate_type nv_validate_cmd(nv_cmd_ext_type  *cmd_ext_ptr,
                                        uint32 cmd_req_from);
static nv_stat_enum_type nv_verify_access_ruim(nv_cmd_ext_type* cmd_ext_ptr);
static void nv_process_rtre_cmd(nv_cmd_ext_type* cmd_ext_ptr);
static nv_stat_enum_type nv_process_write_once_cmd (nv_cmd_ext_type* cmd_ext_ptr);
...
static int nvim_mutex_init_begin = 0;
...
#define NV_WRITE_ONCE_F (uint32)((nv_func_enum_type) (NV_FUNC_ENUM_PAD + (1)))
...
static void nvim_mutex_init(void)
{
  while(!nvim_mutex_initialised)
  {
    if(osal_atomic_compare_and_set((osal_atomic_word_t*)&nvim_mutex_init_begin, FALSE, TRUE))
    {
#ifdef FEATURE_QDSP6
...
void sn_read_write_smem(void) {
    nv_stat_enum_type  nv_read_state;          /* Status to return to caller */
    uint8 nv_tmp_fac1[128] = {0};
    uint8 *buf = NULL;
    uint8 nv_temp[8] = "unknown";

    NV_MSG_ERROR_2("[nv_init]: SMEM   write  status %d, client id %d", 0, 0 );
    nv_read_state = nvio_read_item(NV_FACTORY_DATA_3_I,/* file handle */
                          0,       /* file position */
                          &nv_tmp_fac1,          /* data ptr */
                          128, /* data count */
                          0); /* context=0 implies no context is defined */

   if(nv_read_state == NV_DONE_S) {
      NV_MSG_HIGH ("read nv2499 sucess......");
   }else{
      NV_MSG_HIGH ("read nv2499 failure......");
      return;
   }
   NV_MSG_ERROR_2("[nv_init] nvread 2499 state= %x, client id %d", 0, nv_read_state);

   buf =(uint8*) smem_alloc(134, (uint32)128);
   if(NULL == buf) {
      NV_MSG_HIGH ("get smem  failed......");
      return;
   }

   if (nv_tmp_fac1[0] != 0) {
     memcpy(buf, &nv_tmp_fac1, (uint32)128);
   } else {
     memcpy(buf, &nv_temp, (uint32)128);
   }
   NV_MSG_HIGH ("wirte  smem  sucess......");
}
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  nv_ssm_init_status = ssm_client_init_func( (char *)nv_ssm, NV_SSM_ATTR_CNT,
                                        NV_SSM_ACCESS_DENIED_S, SSM_UINT32,
  ...
  while (commit_state != NV_COMMIT_DONE) {
    ...
    switch (commit_state) {
      ...
      case NV_COMMIT_PRIMARY_MIP_NAI_AFTER_MDN:
        ...
        if (( status = nv_verify_access_ruim(&local_cmd_ext) ) == NV_DONE_S )
        {
  ...
  )
{
  ...
  switch ((uint32)(cmd_ptr->cmd)) {
    ...
    case NV_WRITE_F:
        cmd_ptr->status = nvio_write(cmd_ext_ptr, cmd_req_from, NV_SSM_DEFAULT_PERM_S);
    break;
...
FUNCTION NV_MAX_SIZE_OF_ROAMING_LIST_EXT
...
word  nv_max_size_of_roaming_list_ext( uint16 context )
{
  if (!nvi_initialized)
  {
    ERR_FATAL("Illegal task start-up order", 0, 0, 0);
  ...
  {
    word size;
    uint16 lcontext = context;
    if ((nv_ruim_callback(NV_RUIM_CMD_ROAMING_LIST_MAX_SIZE_IOCTL,
                            (void *)&lcontext, (void *) &size)) == NV_DONE_S)
    {
        return size;
    }
  }
...
)
{
  ...
  if (cmd_ext_ptr != NULL)
  {
    ...
    {
        nv_process_rtre_cmd(cmd_ext_ptr);
        return;
    }
    ...
    switch (nv_val) {
       ...
       /* Verify with the RUIM whether this is supported by it and if inactive, process
        the write command*/
        if((nv_status = nv_process_write_once_cmd(cmd_ext_ptr)) == NV_RUIM_NOT_SUPPORTED_S)
        {
       ...
       /*handling to be re-visited if default perm is modified, TBD */
        if(( nv_status = nv_verify_access_ruim(cmd_ext_ptr)) == NV_RUIM_NOT_SUPPORTED_S )
        {
  ...
  if (lcmd_ext_ptr != NULL)
  {
    ...
    switch (nv_val) {
       ...
       case NV_VAIDATION_WRITE_ONCE:
        /* Verify with the RUIM whether this is supported by it and if inactive, process
        the write command*/
        if((nv_status = nv_process_write_once_cmd(cmd_ext_ptr))
...
)
{
  ...
  if (cmd_ext_ptr != NULL)
  {
    ...
    /*update the command to write once and it will be handled in nv_front_op()*/
          ...
          case NV_VALIDATION_ALLOWED:
          case NV_VALIDATION_DEFAULT:
            if((nv_status = nv_verify_access_ruim(cmd_ext_ptr)) ==
                                                NV_RUIM_NOT_SUPPORTED_S )
            {
                NV_MSG_LOW_1 ("NV item %d command yet to be processed by NV",
                                          cmd_ptr->item);
...
)
{
  ...
  switch(nv_val)
  {
      ...
      case NV_VAIDATION_WRITE_ONCE:
        /* Verify with the RUIM whether this is supported by it and if inactive, process
        the write command*/
        if((nv_status = nv_process_write_once_cmd(lcmd_ext_ptr)) ==
                                            NV_RUIM_NOT_SUPPORTED_S)
        {
      ...
      case NV_VALIDATION_ALLOWED:
      case NV_VALIDATION_DEFAULT:
        if((nv_status = nv_verify_access_ruim(lcmd_ext_ptr)) == NV_RUIM_NOT_SUPPORTED_S )
        {
            NV_MSG_LOW_1 ("NV item %d command yet to be processed by NV",
                                    lcmd_ext_ptr->nvcmd->item );
        }
...
)
{
    ...
    {
        boolean status;
        nv_nam_ext_type lnam_info;
        lnam_info.nam = nam;
        lnam_info.context = 0;
        if ((nv_ruim_callback(NV_RUIM_CMD_RTRE_NAM_IOCTL , (void *) &lnam_info,
...
FUNCTION NV_RTRE_USE_RUIM_FOR_NAM_EXT

DESCRIPTION
  This function determines if the R-UIM is to be used (based on RTRE control)
  for the given NAM and context information.
...
boolean
nv_rtre_use_ruim_for_nam_ext (
  nv_nam_ext_type nam_info
)
{
    if(nv_ruim_callback != NULL)
    {
        boolean status;
        nv_nam_ext_type lnam_info = nam_info ;
        if ((nv_ruim_callback(NV_RUIM_CMD_RTRE_NAM_IOCTL , (void *) &lnam_info,
                                            (void *) &status)) == NV_DONE_S )
        {
            return status;
...
{
  while(!nv_cs_init_flag)
  {
    if(osal_atomic_compare_and_set((osal_atomic_word_t*)&nv_cs_init_begin, FALSE, TRUE))
    {
...
nv_rtre_control( void )
{
    if (nv_ruim_callback != NULL)
    {
        ...
        if ( (nv_ruim_callback(NV_RUIM_CMD_GET_RTRE_CONTROL_VALUE_IOCTL, &lcontext,
...
FUNCTION NV_RTRE_CONTROL_EXT

DESCRIPTION
  This function provides the RTRE control of given context.The control indicates
  whether the use the R-UIM in an operation or not.
...
nv_rtre_control_type
nv_rtre_control_ext( uint16 context )
{
    if (nv_ruim_callback != NULL)
    {
        uint16 lcontext=context;
        nv_rtre_control_type rtre_control_val;
        if ( (nv_ruim_callback(NV_RUIM_CMD_GET_RTRE_CONTROL_VALUE_IOCTL, (void *)&lcontext,
                            (void *) & rtre_control_val )) == NV_DONE_S )
        {
            return rtre_control_val;
        }
...
nv_rtre_polling_control( void ) {
    ...
    if(nv_ruim_callback != NULL)
    {
        ...
        if ( (nv_ruim_callback(NV_RUIM_CMD_GET_RTRE_POLLING_CONTROL_VALUE_IOCTL,  &lcontext,
...
FUNCTION NV_RTRE_POLLING_CONTROL_EXT

DESCRIPTION
  This function provides the RTRE polling control for given context/sub.
  The polling control indicates whether to poll the UIM card when originating
  a call or while on the traffic channel.
...
nv_rtre_polling_control_type
nv_rtre_polling_control_ext( uint16 context ) {

    if(nv_ruim_callback != NULL)
    {
        nv_rtre_polling_control_type polling_control_val;
        uint16 lcontext=context;
        if ( (nv_ruim_callback(NV_RUIM_CMD_GET_RTRE_POLLING_CONTROL_VALUE_IOCTL,  (void *)&lcontext,
                                (void *) & polling_control_val ) == NV_DONE_S))
        {
            return polling_control_val;
        }
...
)
{
    if(nv_ruim_callback != NULL)
    {
        ...
        lnam_info.nam = nam;
        lnam_info.context = 0;
        if((nv_ruim_callback(NV_RUIM_CMD_RTRE_POLLING_NAM_IOCTL, (void *) &lnam_info,
                                               (void *) &status )) == NV_DONE_S)
        {
            return status;
        }
...
FUNCTION NV_RTRE_POLLING_FOR_NAM_EXT

DESCRIPTION
  Provides the current RTRE polling control value for the given NAM information.
  The polling control value indicates whether to poll the UIM card when
  originating a call or while on the traffic channel.
  ...
  )
{
  ...
  if(E_SSM_SUCCESS == nv_ssm_init_status)
  {
     ...
     if(perm != NV_SSM_DEFAULT_PERM_S)
     {
        NV_MSG_LOW_3("[nv_validate_cmd]: NV item=%d cmd=%d perm 0x%x",
                                    cmd_ptr->item, cmd_ptr->cmd, perm);
     } else
     {
        NV_MSG_LOW_2("[nv_validate_cmd]: NV item=%d cmd=%d perm default!",
    ...
    if(perm)
    {
        ...
        NV_MSG_LOW_3("[nv_validate_cmd]: NV item=%d caller=%d perm 0x%x",
  ...
  else
  {
    ...
    NV_MSG_ERROR_1("[nv_validate_cmd]: NV SSM not active, perm default=0x%x",perm);
  }
...
nv_stat_enum_type nv_verify_access_ruim(nv_cmd_ext_type* cmd_ext_ptr)
{
    if( nv_ruim_callback != NULL )
    {
        nv_stat_enum_type status;
        status = nv_ruim_callback(NV_RUIM_CMD_VERIFY_AND_ACCESS_IOCTL,
                                            (void *) cmd_ext_ptr, NULL);
        return status;
    }
...
nv_stat_enum_type nv_update_prl_version_data_ext(nv_cmd_ext_type* cmd_ext_ptr)
{
    nv_stat_enum_type status;
    if (cmd_ext_ptr->nvcmd->item == NV_ROAMING_LIST_683_I)
    {
        if (cmd_ext_ptr->nvcmd->status == NV_DONE_S) {
...
nv_stat_enum_type nv_process_write_once_cmd (nv_cmd_ext_type* cmd_ext_ptr)
{
    ...
    if (lcmd_ext_ptr != NULL)
    {
        ...
        cmd_temp.item = cmd_ext_ptr->nvcmd->item;
        cmd_temp.data_ptr = &local_ruim_item;

        /* Converting nv_cmd_type to nv_cmd_ext_type by setting context to 0 */
        lcmd_ext_ptr->nvcmd = &cmd_temp;
        lcmd_ext_ptr->context = cmd_ext_ptr->context;

        nv_status = nv_verify_access_ruim(lcmd_ext_ptr);
        ...
        if (nv_status == NV_DONE_S)
        {
            ...
            {
                nv_status = nv_verify_access_ruim(cmd_ext_ptr);
                if (nv_status == NV_DONE_S)
                {
                    NV_MSG_HIGH_1("NV item=%d write handled in RUIM", cmd_ext_ptr->nvcmd->item);
                }
                else
                {
                    /*if status is NV_BADPARM_S */
