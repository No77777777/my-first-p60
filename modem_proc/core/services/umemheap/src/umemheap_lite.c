...
$Header: //components/rel/core.mpss/10.0/services/umemheap/src/umemheap_lite.c#1 $ $DateTime: 2019/04/24 00:03:26 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ---------------------------------------------------------
11/08/16   nk       Fixed Integer overflow in memheap_lite sanity check macro
07/05/15   ps       CR 865038 - Update all definitions to resolve conflicts
...
#define BOUNDARY_CHECK(theBlock, heap_ptr) ((theBlock >= ((umem_heap_type*)heap_ptr)->first_block) && (((char*)(theBlock)) < ((char*)(((umem_heap_type*)heap_ptr)->first_block) + ((umem_heap_type*)(heap_ptr))->total_bytes)))

#define FRD_OFFSET_CHECK(block, heap_ptr) (((((umem_block_header_type *)block)->forw_offset + (char *)block) > (char *)block)\
   && (((umem_block_header_type *)block)->forw_offset + (char *)block) <= ((((char*)((umem_heap_type*)heap_ptr)->first_block) + ((umem_heap_type*)heap_ptr)->total_bytes)))

//#define MEMHEAP_VERIFY_HEADER(block, heap_ptr) (((umem_block_header_type *)block)->header_guard == ((umem_heap_type*)heap_ptr)->block_header_guard ? TRUE : FALSE)
...
static umem_block_header_type *mem_find_free_block(
   umem_heap_type *heap_ptr,
...
static void mem_init_block_header(umem_block_header_type *, unsigned long, umem_heap_type *hep_ptr);
...
mem_heap_enter_crit_sect( void * ptr)
{
  ...
  qurt_rmutex_lock((qurt_mutex_t*) &(((umem_heap_type *)ptr)->memheap_crit_sect));
...
mem_heap_leave_crit_sect( void * ptr)
{
  ...
  qurt_rmutex_unlock((qurt_mutex_t*) &(((umem_heap_type *)ptr)->memheap_crit_sect));
  
 
} /* END mem_heap_leave_crit_sect */


static void memheap_copy_frd_offset_at_end(umem_block_header_type *mem_block, umem_heap_type *heap_ptr)
{
  ...
  if(mem_block->forw_offset != ukMinChunkSize){
...
===========================================================================*/
static umem_block_header_type *mem_get_next_block
(
   const umem_heap_type         *heap_ptr,
     /*  The heap the given block belongs to -- may NOT be NULL!
     */
   const umem_block_header_type *block_ptr
...
{
   uint16 *pblk = NULL;
   umem_block_header_type *nextBlkPtr = NULL;
   ...
   MEMHEAP_ASSERT(!((block_ptr->forw_offset)%ukMinChunkSize));   
  
   nextBlkPtr =  block_ptr->last_flag  ? heap_ptr->first_block
           : (umem_block_header_type *) ((char *) block_ptr + block_ptr->forw_offset);
   pblk = (uint16*)nextBlkPtr;
   if(nextBlkPtr->free_flag == ukBlockFree){
...
)
{
  ...
  unsigned long chunks;
    /* How many whole blocks of size ukMinChunkSize fit in the area of
  ...
  memset(heap_ptr, 0, sizeof(umem_heap_type));

  MEMHEAP_ASSERT(heap_mem_ptr);
  MEMHEAP_ASSERT(heap_mem_size);
  MEMHEAP_ASSERT(heap_mem_size >= (2*ukMinChunkSize-1));
  ...
  */
  ...
  chunks = (unsigned long) ((memory_end_ptr - memory_start_ptr) / ukMinChunkSize);

  heap_ptr->first_block            = (umem_block_header_type *) memory_start_ptr;
  ...
  umagic_num_index++;  
  mem_init_block_header(heap_ptr->first_block, chunks * ukMinChunkSize, heap_ptr);
  heap_ptr->first_block->last_flag = (char) ukLastBlock;
  heap_ptr->total_blocks           = 1;
  heap_ptr->max_used               = 0;
  heap_ptr->max_request            = 0;
  heap_ptr->used_bytes             = 0;
  heap_ptr->total_bytes            = chunks * ukMinChunkSize;  
...
===========================================================================*/
static void mem_init_block_header(
   umem_block_header_type *block_ptr,
...
)
{
  ...
  memset(p_temp, 0 , sizeof(umem_block_header_type));
  block_ptr->free_flag   = (char) ukBlockFree;
...
{  
  ...
  unsigned char bonusBytes;
    /* the computed number of unused bytes at the end of the allocated
       memory block.  Will always be < ukMinChunkSize */

  umem_block_header_type *freeBlock = NULL;
  ...
  uint16 * pblk = NULL;
  uint32 blockHeaderSize=sizeof(umem_block_header_type);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if( (0xFFFFFFFF - ( ukMinChunkSize + sizeof(umem_block_header_type)) ) 
  ...
  chunks = ((unsigned long) size + blockHeaderSize
            + ukMinChunkSize - 1) / ukMinChunkSize;
  actualSize = chunks * ukMinChunkSize;
  ...
  if (freeBlock)
  {
      ...
      if (freeBlock->forw_offset > actualSize)
      {
        ...
        umem_block_header_type *newBlock = (umem_block_header_type *)
      ...
      freeBlock->free_flag = ukBlockUsed;
...
{
  umem_block_header_type *theBlock;
  ...
  uint32          sizeBlockHeader=sizeof(umem_block_header_type);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  theBlock = (umem_block_header_type *)
                          ((char *) ptr - sizeBlockHeader);
  // now we have to make a guess that size of (umem_block_header_type) is 12 or 16
#if defined FEATURE_MEM_DEBUG && defined FEATURE_QDSP6
	if ( theBlock->header_guard != heap_ptr->block_header_guard )
	{
		theBlock=(umem_block_header_type *)((char *)theBlock+4);	
        sizeBlockHeader=sizeBlockHeader-4;
	}
#endif

  //check for block alignment to 16
  MEMHEAP_ASSERT((((uint32)theBlock)%ukMinChunkSize) == 0);
  ...
  MEMHEAP_ASSERT(theBlock->extra < ukMinChunkSize);
 ...
 //check for heap canary
                              ...
                              a multiple free of the same block is detected */
  {
    ...
    theBlock->free_flag = (char) ukBlockFree;
	...
	if(theBlock != heap_ptr->first_block)
    {
      ...
      if(!((temp->freeBlock_frdOff)%ukMinChunkSize)){
        ...
        if(validBlock){
           preFreeBlock =  (umem_block_header_type *)((char*)theBlock - temp->freeBlock_frdOff);
           ...
           if(preFreeBlock >= heap_ptr->first_block){
              if((preFreeBlock->forw_offset == temp->freeBlock_frdOff) 
                 ...
                 &&(preFreeBlock->free_flag  == ukBlockFree))
              {
    ...
    if(theBlock->last_flag != ukLastBlock){
      char *end_address=(char*)(heap_ptr->first_block) + heap_ptr->total_bytes;      
      if(((char*)theBlock + theBlock->forw_offset) < end_address)
      {
         umem_block_header_type *nextFreeBlock =
           (umem_block_header_type *)((char *)theBlock + theBlock->forw_offset);
         if(nextFreeBlock->free_flag == ukBlockFree)
         {
...
)
{
  ...
  umem_block_header_type *followingBlock;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if((heap_ptr->next_block)->free_flag == ukBlockFree){
