...
$Header: //components/rel/core.mpss/10.0/services/ssm/src/ssm_parser.c#1 $
...
/* Timeout for mode update */
#define SSM_MODE_UPDATE_TIMEOUT_PERIOD 250
...
void ssm_parse_perm_file(void)
{
  ...
  if ( ssm_is_smdl_ready( SSM_PROC_MODEM ) == FALSE )
    return;
  #else /* SSM_USES_SMDL */
  if ( ssm_is_glink_ready( SSM_PROC_MODEM ) == FALSE )
    return;
  #endif /* SSM_USES_SMDL */
  ...
  {
    timer_sleep( SSM_MODE_UPDATE_TIMEOUT_PERIOD, T_USEC, FALSE);
