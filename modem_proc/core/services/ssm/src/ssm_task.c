...
$Header: //components/rel/core.mpss/10.0/services/ssm/src/ssm_task.c#1 $
...
)
{
  ...
  for(;;)
  {
    ...
    /*-------------------------------------------------------------------------*/
    /*                                                                         */
    /*                        SSM_GLINK_READ_SIG SIGNAL                           */
    /*                                                                         */
    /*-------------------------------------------------------------------------*/

    if ( sigs & SSM_GLINK_READ_SIG )
    {
      #ifdef FEATURE_SSM_REX_API_PRESENT
      (void)rex_clr_sigs ( ssm_tcb_ptr, SSM_GLINK_READ_SIG );
      #else /* FEATURE_SSM_REX_API_PRESENT */
        qurt_anysignal_clear( &ssm_sig_qurt, SSM_GLINK_READ_SIG );
      #endif /* FEATURE_SSM_REX_API_PRESENT */
