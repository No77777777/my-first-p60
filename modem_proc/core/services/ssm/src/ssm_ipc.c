...
$Header: //components/rel/core.mpss/10.0/services/ssm/src/ssm_ipc.c#1 $

when       who     what, where, why
--------   ---     ------------------------------------------------------------
08/18/14   din     Message is sent to peripheral only if GLINK channel is ready.
...
)
{
  ...
  /* Check if destination proc smdl/glink connection is established */
  #ifdef SSM_USES_SMDL
  ...
  #else /* SSM_USES_SMDL */
  if (! ssm_is_glink_ready(dest_proc) )
  {
    return E_SSM_FAIL;
  }
  #endif /* SSM_USES_SMDL */
  ...
  /* Send the message to proc over glink */
  #ifdef SSM_USES_SMDL
  ...
  #else /* SSM_USES_SMDL */
  if ( E_SSM_SUCCESS != ssm_glink_handle_tx( dest_proc, temp_ipc, 
			                    len_temp_enc + SSM_ENC_IPC_HEADER_LENGTH) )
  { 
     SSM_MSG_ERROR_1 ( "Encode : GLINK trasnfer failed %d", 
		     len_temp_enc + SSM_ENC_IPC_HEADER_LENGTH);
...
)
{
  ...
  #else  /* SSM_USES_SMDL */
  if ( E_SSM_SUCCESS != ssm_glink_handle_tx( SSM_PROC_APPS, temp, 
			                    SSM_RAW_IPC_HEADER_LENGTH_APPS) )
  { 
     SSM_MSG_ERROR_1 ( "Encode : GLINK trasnfer failed %d", 
		     SSM_RAW_IPC_HEADER_LENGTH_APPS);
