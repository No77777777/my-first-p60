...
/*============================================================================
  ...
  $Header: //components/rel/core.mpss/10.0/buses/spi/src/driver/DalSpi.c#1 $$DateTime: 2019/04/24 00:03:26 $$Author: pwbldsvc $
...
===========================================================================*/
static DALResult SpiServeNextTransaction(DALSYSEventHandle hEvent, void *pInDevCtxt)
{
   ...
   do
   {
      ...
      {
         /* if the event has call back, DALSYS will call the callback function */
         SPIDEVICE_LOG(INFO, "Notify Transfer(%x) Complete,Device=0x%x", pPrevTransaction->u32TransferId, pDevCtxt->DevId);
         DALSYS_EventCtrl(pPrevTransaction->spiTransfer.dalSysEvent, DALSYS_EVENT_CTRL_TRIGGER);
      }
...
DALResult Spi_DeviceInit(SpiClientCtxt *pClientCtxt)
{
   ...
   DALSYS_memset(&config, 0x0, sizeof(config));
   ...
   {
      if (DALSYS_SyncCreate(DALSYS_SYNC_ATTR_RESOURCE, &(pDevCtxt->hDeviceSynchronization), &(pDevCtxt->DeviceSyncObject)) != DAL_SUCCESS)
      {
         SPIDEVICE_LOG(ERROR, "Spi_DeviceInit: Error in synchronize between client and device=0x%x", pDevCtxt->DevId);
         return DAL_ERROR;
      }

      if (DAL_SUCCESS != DALSYS_RegisterWorkLoop(0, 10, &(pDevCtxt->hStartTransferWorkLoop), NULL))
      {
         SPIDEVICE_LOG(ERROR, "Spi_DeviceInit:spi fail to create workloop, device=0x%x", pDevCtxt->DevId);
         return DAL_ERROR;
      }

      /* Create Asyc Object dedicated for workloop */
      if (DAL_SUCCESS != DALSYS_EventCreate(DALSYS_EVENT_ATTR_WORKLOOP_EVENT, &(pDevCtxt->hStartTransferEvent), NULL))
      {
         SPIDEVICE_LOG(ERROR, "Spi_DeviceInit:" "Error creating start workloop event, device=0x%x", pDevCtxt->DevId);
         return DAL_ERROR;
      }

      if (DAL_SUCCESS != DALSYS_AddEventToWorkLoop(pDevCtxt->hStartTransferWorkLoop, SpiServeNextTransaction,(void *)pDevCtxt, pDevCtxt->hStartTransferEvent, NULL))
      {
         SPIDEVICE_LOG(ERROR, "Spi_DeviceInit: unable to add event workloop, device=0x%x", pDevCtxt->DevId);
         return DAL_ERROR;
      }
 ...
 uint32 *pTransferId, uint32 olen7)
{
   ...
   {
      if (DALSYS_EventCreate(DALSYS_EVENT_ATTR_NORMAL, &(pSpiTransfer->dalSysEvent),NULL) != DAL_SUCCESS)
      {
         return DAL_ERROR;
      }
      bWaitEvent = TRUE;
