...
uint32 SpiDevice_IsrHook(void *pdata)
{
   ...
   for (;;)
   {
      ...
      write_req       = qup_operational & HAL_QUPE_OPSTATE_OUT_BLOCK_WRITE_REQ;
      read_req        = qup_operational & HAL_QUPE_OPSTATE_IN_BLOCK_READ_REQ;
      ...
      if (!output_service && !input_service && !write_req && !read_req) { break; }
      ...
      if (output_service && write_req) { write_block(ctxt); }
      if (input_service && read_req)   {  read_block(ctxt); }
   }
...
{
   //pdata = pdata;  // suppress compiler warning
   return 0;
}
