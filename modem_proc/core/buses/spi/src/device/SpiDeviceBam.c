...
$Header: //components/rel/core.mpss/10.0/buses/spi/src/device/SpiDeviceBam.c#1 $
...
{
   IBUSOS_Memcpy(&spiBam->mHalTransferParameters,
...
{
   IBUSOS_Memcpy(&spiBam->mHalDeviceParameters,
...
)
{
   ...
   do
   {
      if (SPIDEVICE_RES_SUCCESS != (res = IBUSOS_Malloc((void **)&spiBam,
      ...
      IBUSOS_Memcpy(&spiBam->bam_cfg, bam_cfg, sizeof(SpiDeviceBam_Config));
