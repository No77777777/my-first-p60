...
/*===========================================================================
  ...
  $Header: //components/rel/core.mpss/10.0/api/memory/dsm_item.h#1 $
...
typedef enum dsm_item_pkt_len_type_s
{
   ...
   DSM_PKT_LEN_NOT_SET = 0, /* PKT Len is set by client, library will not use or modify the field */
   DSM_PKT_LEN_SET_BY_PKT_LEN_APIS, /* Library will believe in the field and will not touch it*/
   DSM_PKT_LEN_SET_BY_CLIENT /* Library will not believe the field */
...
struct dsm_item_s
{
  q_link_type        link;	/* field for put/getting to/fromQs */
                            /*  link's prev_ptr field contains client data */
...
)
{
  ...
  item->pkt_bit_field.dsm_bit_field.dsm_q_use_pkt_len_bit = DSM_PKT_LEN_SET_BY_PKT_LEN_APIS;
}
...
)
{
    /* IMPORTANT : DSM_PKT_LEN field is user defined field and is not validated by DSM Library */
    pkt->dsm_pkt_len = 0;
    pkt->pkt_bit_field.dsm_bit_field.dsm_q_use_pkt_len_bit = DSM_PKT_LEN_NOT_SET;
}
static __inline uint32 dsm_item_update_pkt_len(
  dsm_item_type* pkt
)
{
    /* IMPORTANT : DSM_PKT_LEN field is user defined field and is not validated by DSM Library */
    pkt->dsm_pkt_len = dsm_length_packet(pkt);
    pkt->pkt_bit_field.dsm_bit_field.dsm_q_use_pkt_len_bit = DSM_PKT_LEN_SET_BY_PKT_LEN_APIS;
...
{
    item->pkt_bit_field.dsm_bit_field.q_priority_bit = priority;
}
...
{
    return  (dsm_item_header_priority_type)item->pkt_bit_field.dsm_bit_field.q_priority_bit;
}
...
/* NOTE: START LINK.PREV_PTR - This field can be used for storing client data
 or used as prev_ptr directly by client , depends on client usage */

static __inline void dsm_item_set_link_prev_ptr_data(
  dsm_item_type* item,
  uint32 data
)
{
   q_set_usr_data_s(&item->link, (void*)((int)data));
}

static __inline uint32 dsm_item_get_link_prev_ptr_data(
  dsm_item_type* item
)
{
    return  (uint32)(int)q_get_usr_data_s(&item->link);
}
/* NOTE: END LINK.PREV_PTR */


/* NOTE: START USR_DATA/TIME_STAMP - This field can be used for time_stamp or 
   any other client data , the usage depends on client */   
...
{
    *((uint32*)((uint32*)item)-1) = time_stamp;
}
...
{
   return  (uint32)(*((uint32*)((uint32*)item)-1));
}
    
static __inline void dsm_item_set_usr_data(
      dsm_item_type* item,
      uint32 usr_data
)
{
    *((uint32*)((uint32*)item)-1) = usr_data;
}
    
static __inline uint32 dsm_item_get_usr_data(
      const dsm_item_type* item
)
{
   return  (uint32)(*((uint32*)((uint32*)item)-1));
...
SIDE EFFECTS
  None
===========================================================================*/
#define dsm_new_data_buffer(pool_id) dsmi_new_data_buffer(pool_id,dsm_file,__LINE__)

static __inline void* dsmi_new_data_buffer
(
 dsm_mempool_id_type pool_id,
 const char * file,
 uint32 line
...
{
	void* ret_val = NULL;
	dsm_item_type *new_item = dsmi_new_buffer(pool_id,file,line);
	if (new_item)
	{
		ret_val = (void*)(new_item->data_ptr);
	}
	return ret_val;
}
...
DESCRIPTION
  This function will return an array of pointers to payload of new DSM_ITEMs.
  Can be only used with DSM_FREE_DATA_BUFFER() function
  Will not work with DSM_FREE_PACKET() function

  usage :
  void* data_buffer_array[20];
  dsm_new_data_buffers(DSM_DS_GNSS_5k_ITEM_POOL,20,data_buffer_array);
DEPENDENCIES
  None
...
SIDE EFFECTS
===========================================================================*/
#define dsm_new_data_buffers(pool_id,num_buffers, pdsm_array) \
		dsmi_new_data_buffers(pool_id, num_buffers, pdsm_array, dsm_file, __LINE__)
...
SIDE EFFECTS
===========================================================================*/
#define dsm_free_data_buffer(payload_ptr) \
		dsmi_free_data_buffer(payload_ptr, dsm_file, __LINE__)
