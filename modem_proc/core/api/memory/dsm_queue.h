...
/*===========================================================================
  ...
  $Header: //components/rel/core.mpss/10.0/api/memory/dsm_queue.h#1 $
...
typedef struct dsm_watermark_type_s
{
  ...
  wm_cb_type non_empty_cb_ptrs[2]; /* Array of non empty function pointers for future use */
  void * non_empty_cb_data[2]; /* Array of non empty function pointers for future use */
  ...
  uint32 current_cnt_prio_high; /* Current count of items with
                                       DSM_QUEUE_PRIORITY_HIGH */

  uint32 current_cnt_prio_highest; /* Current count of items with
                                         DSM_ENQ_HIGHEST_PRIORITY */
...
---------------------------------------------------------------------------*/
  ...
  /* #define DSM_NORMAL			0 */
  /*.................  SAME AS DSM_QUEUE_PRIORITY_NORMAL ....................*/
  ...
  /* Cannot comment as it is still in use */
  /*................. SAME AS  DSM_QUEUE_PRIORITY_HIGH..............*/
  ...
  /* Cannot comment as it is still in use */
  /*.................  SAME AS DSM_ENQ_HIGHEST_PRIORITY ................*/
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  typedef enum
  {
    ...
    DSM_WM_EMPTY,
    DSM_WM_NON_EMPTY, 
    DSM_WM_NON_EMPTY_PTRS_0, 
    DSM_WM_NON_EMPTY_PTRS_1, 
...
static __inline uint32 dsm_pkt_chain_link_update_pkt_len(
  dsm_item_type **chain_link,
  dsm_item_type *second
)
{
  ...
  q_chain_link_s_i((q_link_type*)*chain_link,(q_link_type*)second);
  *chain_link = second;
  return dsm_item_update_pkt_len(second);  
}
...
DESCRIPTION
  ...
  Usage below.
    ...
    total_pkt_len+=dsm_item_update_pkt_len(enq_pkts[0]);
    for (i = 1; i<5;i++){
       dsm_pkt_chain_link_update_pkt_len(&chain_tail_ptr,enq_pkt[i]);
       total_pkt_len+=dsm_item_get_pkt_len(enq_pkt[i]);
       #if 0
       total_pkt_len+=dsm_pkt_chain_link_update_pkt_len(&chain_tail_ptr,enq_pkt[i]); 
       #endif
    }
