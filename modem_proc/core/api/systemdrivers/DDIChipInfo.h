...
============================================================================
  $Header: //components/rel/core.mpss/10.0/api/systemdrivers/DDIChipInfo.h#2 $
  $DateTime: 2023/05/03 00:36:34 $
  $Author: pwbldsvc $
===========================================================================
*/
...
struct DalChipInfo
{
   ...
   DALResult (*GetSKU)(uint32 ClientHandle, DalDeviceHandle * _h, ChipInfoSKUType *pInfo);
...
#define DAL_ChipInfoDeviceDetach(hDalDevice)\
...
DalChipInfo_ChipVersion(void)
{
  ...
  DALResult                     eResult;
  static ChipInfoVersionType    nVersion = CHIPINFO_VERSION_UNKNOWN;

  if (nVersion == CHIPINFO_VERSION_UNKNOWN)
  {
    eResult =
    ...
    DAL_ChipInfoDeviceDetach(phChipInfo);
  }
...
DalChipInfo_ChipId(void)
{
  DalDeviceHandle              *phChipInfo = NULL;
  DALResult                     eResult;
  static ChipInfoIdType         neId = CHIPINFO_ID_UNKNOWN;

  if (neId == CHIPINFO_ID_UNKNOWN)
  {
    eResult =
      DAL_ChipInfoDeviceAttach(DALDEVICEID_CHIPINFO, &phChipInfo);
...
DalChipInfo_ChipFamily(void)
{
  DalDeviceHandle              *phChipInfo = NULL;
  DALResult                     eResult;
  static ChipInfoFamilyType     neFamily = CHIPINFO_FAMILY_UNKNOWN;

  if (neFamily == CHIPINFO_FAMILY_UNKNOWN)
  {
    eResult =
      DAL_ChipInfoDeviceAttach(DALDEVICEID_CHIPINFO, &phChipInfo);
...
DalChipInfo_FoundryId(void)
{
  DalDeviceHandle              *phChipInfo = NULL;
  DALResult                     eResult;
  static ChipInfoFoundryIdType  peId =CHIPINFO_FOUNDRYID_UNKNOWN;

  if (peId == CHIPINFO_FOUNDRYID_UNKNOWN)
  {
    eResult =
      DAL_ChipInfoDeviceAttach(DALDEVICEID_CHIPINFO, &phChipInfo);
...
/**
  ...
  @param[in,out]  _h         Dal Device Handle given by the caller.
  @param[out]     pInfo      Pointer to a caller-allocated buffer where 
                             SKU information will be stored
...
*/
static __inline DALResult
DalChipInfo_GetSKU(DalDeviceHandle * _h, ChipInfoSKUType *pInfo)
{
  return ((DalChipInfoHandle *)_h)->pVtbl->GetSKU(DALDEVICE_LOCAL_CLIENTID, _h, pInfo);
}
