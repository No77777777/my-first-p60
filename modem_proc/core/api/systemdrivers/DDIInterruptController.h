...
$Header: //components/rel/core.mpss/10.0/api/systemdrivers/DDIInterruptController.h#1 $
 ...
 */
#define DALINTCTRL_CONFIG_STACK_1K (1<<4)
#define DALINTCTRL_CONFIG_STACK_2K (2<<4)
#define DALINTCTRL_CONFIG_STACK_4K (4<<4)
#define DALINTCTRL_CONFIG_STACK_8K (8<<4)

/**
 * Macros to set the priority of a client-owned IST
 */
#define DALINTCTRL_CONFIG_PRIO_HIGHEST 7
#define DALINTCTRL_CONFIG_PRIO_MEDIAN 3
#define DALINTCTRL_CONFIG_PRIO_LOWEST 1
 ...
 *  priority and to tell the Interrupt Controller this interrupt
 *  is a micro image interrupt controller. its 1 bit set to
 *  value =1 for uInterrupt and zero otherwise.  Added delayed
 *  ack option, 1 == delayed ack where EndOfInterrupt must be
 *  called by client, 0 == IST ack.
*/
#define DALINTRCTRL_ENABLE_CONFIG_EX2(trigger,priority,uInterrupt,delayed_ack) \
                                          (trigger|(priority<<24)|(uInterrupt<<18)|(delayed_ack<<16))
...
struct DalInterruptController
{
   ...
   DALResult (*RegisterISR)(uint32 ClientHandle, DalDeviceHandle * _h, DALInterruptID  intrID,
                           const DALISR  isr, const DALISRCtx  ctx, uint32  bEnable);
   DALResult (*RegisterIST)(uint32 ClientHandle, DalDeviceHandle * _h, DALInterruptID  intrID,
                           const DALISR  isr, const DALISRCtx  ctx, uint32  bEnable,char* pISTName);
   DALResult (*RegisterEvent)(uint32 ClientHandle, DalDeviceHandle * _h, DALInterruptID  intrID,
                             const DALSYSEventHandle  hEvent, uint32  bEnable);
   DALResult (*Unregister)(uint32 ClientHandle, DalDeviceHandle * _h, DALInterruptID  intrID);
   DALResult (*InterruptDone)(uint32 ClientHandle, DalDeviceHandle * _h, DALInterruptID  intrID);
   DALResult (*InterruptEnable)(uint32 ClientHandle, DalDeviceHandle * _h, DALInterruptID  intrID);
   DALResult (*InterruptDisable)(uint32 ClientHandle, DalDeviceHandle * _h, DALInterruptID  intrID);
   DALResult (*InterruptTrigger)(uint32 ClientHandle, DalDeviceHandle * _h, DALInterruptID  intrID);
   DALResult (*InterruptClear)(uint32 ClientHandle, DalDeviceHandle * _h, DALInterruptID  intrID);
   DALResult (*InterruptStatus)(uint32 ClientHandle, DalDeviceHandle * _h, DALInterruptID  intrID);
   DALResult (*RegisterIRQHandler)(uint32 ClientHandle, DalDeviceHandle * _h, DALInterruptID  intrID,
                           const DALIRQ  irq, const DALIRQCtx  ctx, uint32  bEnable);
  DALResult (*SetInterruptTrigger)(uint32 ClientHandle, DalDeviceHandle * _h, DALInterruptID intrID, uint32 nTrigger);
  DALResult (*IsInterruptPending)(uint32 ClientHandle, DalDeviceHandle * _h, DALInterruptID intrID,void* bState,uint32 size);
  DALResult (*IsInterruptEnabled)(uint32 ClientHandle, DalDeviceHandle * _h, DALInterruptID intrID,void* bState,uint32 size);
  DALResult (*MapWakeupInterrupt)(uint32 ClientHandle, DalDeviceHandle * _h, DALInterruptID intrID,uint32 nWakeupIntID);
  DALResult (*IsAnyInterruptPending)(uint32 ClientHandle, DalDeviceHandle * _h, uint32* bState,uint32 size);
  DALResult (*Sleep)(uint32 ClientHandle, DalDeviceHandle * _h, InterruptControllerSleepType sleep);
  DALResult (*Wakeup)(uint32 ClientHandle, DalDeviceHandle * _h, InterruptControllerSleepType sleep);
  DALResult (*GetInterruptTrigger)(uint32 ClientHandle, DalDeviceHandle * _h, DALInterruptID  intrID,uint32* eTrigger, uint32 size);
  DALResult (*GetInterruptID)(uint32 ClientHandle, DalDeviceHandle * _h, const char *szIntrName, uint32* pnIntrID, uint32 size);
  DALResult (*LogState)(uint32 ClientHandle, DalDeviceHandle * _h,  void *pULog);
};
