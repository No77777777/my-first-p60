...
===========================================================================*/
/* $Header: //components/rel/core.mpss/10.0/api/debugtrace/tracer_config.h#1 $ */
...
/**
  @brief Add a table of SW events 

  Add a list of SW events to be managed by trace.
  The event id range added cannot conflict with an existing range.

  @param event_id_start [in]:  Id of the first event in the list of events 
  @param event_id_last  [in]:  Id of the last event in the list of events
  @param tag            [in]:  Pointer to a tag string for the event table.
  @param tag_len        [in]:  Length in bytes of the tag string
  @param pre_enable_mask[in]:  An array of uint32 values represnting the bitmask
                               of events to be pre-enabled if conditions are met.
  @param pre_enable_mask_len[in]: Length of the array above.

  @return TRACER_CMDRESP_...
 */

int tracer_add_event_tbl(int event_id_start,
                         int event_id_last,
                         char *tag,
                         int tag_len,
                         uint32 *pre_enable_mask,
                         int pre_enable_mask_len); 
