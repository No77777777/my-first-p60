...
=============================================================================*/
/*=============================================================================
  $Header: //components/rel/core.mpss/10.0/api/debugtrace/tracer.h#1 $
...
tracer_return_enum_t tracer_data(tracer_client_handle_t hClient,
                                 ...
                                 const char *pData,
                                 ...);
/** @ingroup tracer_h_data_start
  Send the start of a multi-part trace data packet to trace stream
  ...
  @param[in] uData_len  Number of bytes to be placed in the trace output
                        stream. 
  @param[in] pData      Starting address of the uData_len bytes of trace
                        data to be placed in the trace output stream. \n

  @detdesc
  The below three functions can be called to send a multi-part tracer data
  packet to the trace stream.
  @par
  trace_data_start() \n
  trace_data_payload() \n
  trace_data_finish() \n
  @par
  A multi-part tracer data packet send must start with tracer_data_start() call,
  and must end with tracer_data_finish() call. Between the start and end there 
  could be 0 or more trace_data_payload() calls. No other data must be
  sent on this tracer client handle before the packet is completely sent.
  @par
  The trace data is encapsulated based onthe mode selected when the
  client registered with tracer. 
  @par
  The trace data is typically a text string or a binary data block.
  @par
  Data may be silently discarded if Tracer is not initialized, Tracer
...
/*-------------------------------------------------------------------------*/
tracer_return_enum_t tracer_data_start(tracer_client_handle_t hClient,
                                       tracer_options_t options,
                                       uint32 uData_len,
                                       const char *pData);

/*-------------------------------------------------------------------------*/
/** @ingroup tracer_h_data_payload
    Send a payload chunk of a multi-part trace data packet to trace stream.
    This piece would be appended to what was sent before.
    
    @detdesc
    See tracer_data_start for details

*/
tracer_return_enum_t tracer_data_payload(tracer_client_handle_t hClient,
                                         tracer_options_t options,
                                         uint32 uData_len,
                                         const char *pData);
...
/*-------------------------------------------------------------------------*/
/** @ingroup tracer_h_data_finish
    Send the last part ofa a  multi-part trace data packet to trace stream
...
tracer_return_enum_t tracer_data_finish(tracer_client_handle_t hClient,
                                        tracer_options_t options,
                                        uint32 uData_len,
                                        const char *pData);
...
/*-------------------------------------------------------------------------*/
/** @ingroup tracer_h_header32_and_data

  @param[in] hClient    Client's handle obtained when registered.
  @param[in] options    Options for delivering trace data (timestamped and/or
                        guaranteed). See Section @xref{dox:tracerConstants}
                        for details.
  @param[in] header     A 32 bit payload speicific header that would be
                        put out on the trace stream before the rest of the
                        the data packet.

  @param[in] data_len   Number of bytes to be placed in the trace output
                        stream. 
  @param[in] pData      Starting address of the uData_len bytes of trace
                        data to be placed in the trace output stream. \n
*/

tracer_return_enum_t tracer_header32_and_data(tracer_client_handle_t hClient,
