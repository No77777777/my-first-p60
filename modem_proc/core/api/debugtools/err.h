...
$Header: //components/rel/core.mpss/10.0/api/debugtools/err.h#1 $
...
===========================================================================*/
CORE_WARN_DEPRECATED_MSG("Deprecated API","Migrate to new API err_cb_enable(err_cb_info_t * cb_info)")
...
/********************
 NOTE:
  Define FEATURE_FATAL_SECTION *globally* to enable this feature. Necessary
...
#ifdef FEATURE_MAP_ERR_FATAL_TO_EXCEPTION
/* Forcing an intentional precise exception to catch the locality of the fault.  
 * The change will replace ERR_FATAL with a dereference of 0xDEADDEAD, 
 * which is unaligned and guaranteed to generate an exception and put us on fatal path */
  static int err_var =0xDEADDEAD;
  #define ERR_FATAL(format, xx_arg1, xx_arg2, xx_arg3) \
  do { \
        err_var = *(int*)(err_var);\
  } while (0)
#elif FEATURE_GENERIC_ERR_FATAL_MSG
 #define ERR_FATAL(format, xx_arg1, xx_arg2, xx_arg3) \
  do { \
...
__attribute__((warn_unused_result))
#endif
CORE_WARN_DEPRECATED_MSG("Deprecated API","Migrate to new API err_cb_disable(err_cb_info_t * cb_info)")
