...
/**@ingroup driver_support_functions
  Invokes an asynchornous callback for a given process. 
  A driver residing in root process would call this API to launch a callback in
  a process described by the client_handle.
  Once the callback is invoked, the framework will queue the callback as per its 
  priority and subsequently execute it.
  Caller of this function will NOT be suspended during callback execution period.
  The API would return immidiately with a success/failure error code.
  ...
  @param  client_handle   Obtained from the current invocation function (Section 3.4.1).
  @param  cb_data         Pointer to callback data structure (refer to qurt_callback.h)
  @param  prio            Priority at which callback should be executed.
                          This paraemter is optional. If -1 is passed, callback frameowrk will
                          execute callback at the priority of the API caller.
  @return
  0 -- Callback was successfully communicated to the framework.
  Negative error code -- Callback could not be communicated to the framework.
 */
static __inline qurt_cb_result_t qurt_qdi_cb_invoke_async(int client_handle,
                                                          qurt_cb_data_t* cb_data, 
                                                          int prio)
{

    return (qurt_cb_result_t)qurt_qdi_handle_invoke(QDI_HANDLE_GENERIC, 
                                                    QDI_CALLBACK_CBDATA_ADD, 
                                                    client_handle, 
                                                    cb_data, 
                                                    prio);
}
...
/**@ingroup driver_support_functions
  Invokes a synchornous callback for a given process. 
  A driver residing in root process would call this API to launch a sync callback in
  a process described by the client_handle.
  Once the callback is invoked, the framework will queue the callback as per its 
  priority and subsequently execute it.
  Caller of this function WILL BE be suspended during callback execution period.
  If process in which callback is to be executed exits/terminates the caller will be
  worken up with error code QURT_CB_WAIT_CANCEL. (refer to qurt_callback.h)
  ...
  Negative error code -- Callback could not be communicated to the framework.
 */
static __inline qurt_cb_result_t qurt_qdi_cb_invoke_sync(int client_handle,
                                                         qurt_cb_data_t* cb_data,
                                                         int prio)
{
    return (qurt_cb_result_t)qurt_qdi_handle_invoke(QDI_HANDLE_GENERIC, 
                                                    QDI_CALLBACK_CBDATA_ADD_SYNC, 
                                                    client_handle, 
                                                    cb_data, 
                                                    prio);  
}
