...
$Header: //components/rel/core.mpss/10.0/api/services/timer.h#1 $
...
/* temp define for supporting older timer structures */
//#define TIMER_HANDLE_AS_STRUCTURE
//#define  TIMER_SUPPORT_OLD_TIMER_STRUCTURE
...
typedef enum
{
  ...
  TE_ALL_TIMER_NOT_UNDEEF,       /* Error returned if All timer belonging to a group are not undefined */
  TE_MAX = 0xFFFFFFFF
}
timer_error_type;

/* Timer expiry type classifier */
typedef enum timer_expiry_type_enum
{
  TIMER_RELATIVE_EXPIRY = 0x0,
  TIMER_ABSOLUTE_EXPIRY = 0x1
} timer_expiry_type;
...
typedef enum
{
  TIMER_NO_NOTIFY_TYPE = 0,           /* Use this if, no notification is required */
  TIMER_NATIVE_OS_SIGNAL_TYPE = 1,    /* In Qurt, required for qurt_anysignal_t object */
  TIMER_NATIVE_OS_SIGNAL2_TYPE = 2 ,  /* In Qurt, required for qurt_signal2_t object */
  TIMER_REX_TCB_SIGNAL_TYPE = 3,      /* For Rex only, rex_tcb_type is required */
  TIMER_FUNC1_CB_TYPE = 4,            /* timer_t1_cb_type type of function pointer is expected */
  TIMER_FUNC2_CB_TYPE = 5,            /* timer_t2_cb_type type of function pointer is expected */
  TIMER_SIGS_FUNC_DUAL_NOTIFY = 6,    /* when both sigs & func type of notification is passed */
  TIMER_DALSYS_EVENT_SIGNAL_TYPE = 7, /* For Dalsys Event objects, DALSYSEventHandle* obj addr is required */
...
{
  /**< Timers in the disabled timer group. */
//  timer_list_type                 list;       

  /**< Disabled flag for the timer group. */
  boolean                         disabled;   

  /**< Deferrable flag for the timer group. Default = FALSE. */
  boolean                         deferrable; 

  /**< Link to the next timer group. */
...
#else /* TIMER_SUPPORT_OLD_TIMER_STRUCTURE */

#ifdef TIMER_HANDLE_AS_STRUCTURE
typedef struct timer_group_struct
{
  unsigned int                    group_handle;
}
...
#else /* TIMER_HANDLE_AS_STRUCTURE */

/* Timer Group handle */
/* Note: Groups variables are to be created as Global variables. 
         Timer groups are a way of enabling and disabling groups of timers at the same time. 
...
#endif /* TIMER_HANDLE_AS_STRUCTURE */
...
typedef struct timer_struct
{
  ...
  /**< Handle to the internal client timer. */
...
/* Timer Data Type. This contains the handle which represents internal client timer object */
...
timer_state_enum_type    timer_state;   /* The current state which the timer is in */
...
/* Dalsys Macro's.
 * The passed arg 'h' should be pointer to DALSYSEventHandle variable.
 */
...
None.
*/
timer_error_type timer_group_delete
( 
...
@dependencies  
Not be called before allocating timer memory & if reusing a previously
defined timer, call timer_undef() before calling timer_def_priority.
...
@dependencies  
Not be called before allocating timer memory & if reusing a previously
defined timer, call timer_undef() before calling timer_def2_priority.
...
@dependencies  
The timer must be initialized with timer_reg() & if reusing a previously
defined timer, call timer_undef() before calling timer_def2.
*/
...
@param[in] timer Timer to set.
@param[in] group Group timer belongs to (NULL=the "NULL" timer group). 
@param[in] notify_type Refer to comments on timer_notify_type
@param[in] sigs_func_ptr   signal or function ptr depending on notify_type
