...
$Header: //components/rel/core.mpss/10.0/api/services/umemheap_lite.h#1 $ $DateTime: 2019/04/24 00:03:26 $ $Author: pwbldsvc $
...
/* Doxygen comments for this struct are in typedef struct umem_heap_struct */
struct umem_heap_struct;


/* Each heap can have its own function that is used to lock the heap and
** free the heap.  umem_lock_fnc_type and umem_free_fnc_type are the types
** of these functions.
*/
/**
  ...
  None.
*/
typedef void (*umem_lock_fnc_type)( void * ptr);
...
/**
  ...
  None.
*/
typedef void (*umem_free_fnc_type)( void * ptr);
...
/** @brief Header for each memory block allocated. The header describes the 
  ...
  host CPU.
*/
typedef struct umem_block_header_struct {
  uint16        header_guard;
...
typedef struct umem_heap_struct {
  uint32                         magic_num;
  umem_block_header_type         *first_block;   
  /**< First block in the heap. */
  umem_block_header_type         *next_block;    
  ...
  umem_lock_fnc_type              lock_fnc_ptr;
  umem_free_fnc_type              free_fnc_ptr;
...
enum {
  ukBlockFree = 1,/* needed some sapce for checksum in block header so reduced it to 1*/
  ukBlockUsed = 0,
  ukLastBlock = 1,/* needed some sapce for checksum in block header so reduced it to 1*/
  ukMinChunkSize = 16
};
/** @endcond */
