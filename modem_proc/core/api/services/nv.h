...
$Header: //components/rel/core.mpss/10.0/api/services/nv.h#1 $ $DateTime: 2019/04/24 00:03:26 $ $Author: pwbldsvc $
...
/**
  Returns the maximum number of bytes available for storage of a roaming
  list for a give context (per NAM). The value returned depends on the total 
  amount of NVRAM installed in the phone and the total number of NAMs defined.
...
/**
  Provides the current RTRE control value based on context. The control value 
  determines whether to use the R-UIM in an operation.
...
@sideeffects
  ...
  called and does not update if the RTRE control value changes.
*/
nv_rtre_control_type nv_rtre_control_ext( uint16 context );
...
/**
  Determines if the R-UIM is to be used (based on RTRE control) and if the NAM
  is the R-UIM NAM for the given nam information.

@param[in] nam information to be queried.
...
boolean nv_rtre_use_ruim_for_nam_ext (nv_nam_ext_type nam_info);
...
/**
  Provides the RTRE polling control value for a given context/sub. The polling 
  control indicates whether to poll the UIM card when originating a call or 
  while on the traffic channel.
...
nv_rtre_polling_control_type nv_rtre_polling_control_ext( uint16 context );
...
boolean nv_rtre_polling_for_nam_ext ( nv_nam_ext_type nam_info);
...
@param[in] : nv_cmd_ext_type pointer of the command
