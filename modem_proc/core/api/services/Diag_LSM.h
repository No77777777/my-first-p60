...
@ingroup diag_misc
  @file Diag_LSM.h 
  @brief
  Diagnostic Services API definitions in LSM layer

  This file contains APIs necessary to initialize Diagnostic services in User PD.
  This additional initialization is necessary to support Diagnostic services on
  multiple process-spaces.
  
Copyright (c) 2007-2010, 2014, 2016, 2018 by QUALCOMM Technologies, Incorporated.  All Rights Reserved.
All Rights Reserved.
...
$Header: //components/rel/core.mpss/10.0/api/services/Diag_LSM.h#1 $

when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
06/19/18   ph      Configure buffering modes for user PD send buffers.
05/12/16   sa      Circular Mode for UserPD buffer
07/29/15   is      Added define for LSM request buffer size
09/19/14   sr      Added F3 trace APIs
...
/*!
   Maximum LSM RX Packet Size 
*/
#define DIAG_MAX_LSM_RX_PKT_SIZ (0x1000)
...
#define DIAG_PD_BUF_MODE_THRESHOLD 2
#define DIAG_PD_BUF_MODE_LAST      2
...
@brief
   ...
   @param[in]
    address: starting address of the circular buffer that should be used to store the f3 messages.
...
@brief
   ...
   <ul>
      ...
      <li> TRUE  - De-initialization of Diag F3 Trace buffer succeeded
   </ul>
