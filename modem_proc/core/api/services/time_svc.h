...
/*=============================================================================
  Copyright (c) 2003-2017, 2008-2014 QUALCOMM Technologies Incorporated.
  All rights reserved.
...
$Header: //components/rel/core.mpss/10.0/api/services/time_svc.h#1 $
...
/** For returning error from the time API's.
*/
typedef enum
{
  TIME_SUCCESS,
...
/* Time Generic Offset 'type 2' callback function.
     Used for registration of callback function for time updates.
 */
typedef void (*time_cb_type)
(
...
/* Structue typedef for generating event packet on time updates.
   Also for providing time update information to DIAG */
typedef struct time_svc_event_packet_s {
   uint64 tt;         /* Timetick when this packet is generated */
   uint64 sum_offset; /* Sum genoff offset in msecs */
   uint64 delta;      /* Delta from the last update */
   uint32 counter;    /* circular counter increment every time we do the event logging */
   uint8  time_base;  /* If its ATS_USER or TOD */
   uint8  reserved_1;
   uint16 reserved_2;
}time_svc_event_packet_t;
...
@param[in] cb_func Callback function which is called when the system time changes.
@param[in] cb_data Data passed to the cb func.
...
/**
Send event with current system time and when it was last 
updated. Please refer to the structure time_svc_event_packet_t to
understand what details will be provided.

@return
None.
 
@dependencies  
Time-init should be done before this function is called.
*/
