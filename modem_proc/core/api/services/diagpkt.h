...
/*===========================================================================
  ...
  $Header: //components/rel/core.mpss/10.0/api/services/diagpkt.h#2 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
11/26/18   vg      Added support for max size(15.5KB) log packet
02/09/18   sa      Removed Unused APIs.
02/15/17   rs      Added APIs diagpkt_max_delayed_rsp_size() and diagpkt_max_delayed_rsp_size() and exposed them public
...
07/23/14   rh      Changes to allow for 16kB command packets
11/22/13   sr      Removed PACK from pointer references
06/04/13   rh      Changes to allow for 8k command packets
...
*/
#define DIAG_MAX_TX_PKT_SIZ (0x3E20) /* To support max size(15.5KB) logs */

/*!
   Maximum RX Packet Size 
*/
#define DIAG_MAX_RX_PKT_SIZ (0x4000)  /* To support long command packets */
...
@diagpkt_max_delayed_rsp_size
   ...
   @return 
   <ul>
   <li>   Returns maximum delayed response packet size in bytes.
   </ul>   
...
/*!
@diagpkt_total_delayed_bytes_queued
   ...
   @return 
   <ul>
   <li>   Returns total number of bytes queued for delayed diag transport.
   </ul>  
   @return 
