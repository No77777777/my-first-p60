...
$Header: //components/rel/core.mpss/10.0/api/power/npa_resource.h#2 $
==========================================================================*/
...
typedef struct npa_event
{
  ...
  /* Context data for meta events, used to store qdi server handle */
  void                      *meta_event_data;
...
/**
 * <!-- npa_create_meta_event -->
 *
 * @brief Register a callback that is invoked whenever the event list
 * attached to a resource changes. So, whenever - 
 ...
 * 4) Custom resource events are created or destroyed
 *
 * When invoked, the callback arguments will be 
 * (context, NPA_EVENT_META, npa_event *, sizeof(npa_event) ); 
 *
 * where the npa_event is the event that was added/destroyed/modified.
 *
 * @param resource_name : name of the resource that is to be used
 * @param event_name : name of this event
 * @param callback : callback to be invoked
 * @param context : opaque context to be passed back in callback
 *
 * @return The created npa_event_handle OR NULL
 */
#define npa_create_meta_event( resource_name, event_name, callback, context ) \
  npa_create_event_cb( resource_name, event_name,              \
                       NPA_TRIGGER_META_EVENT,                 \
                       callback, context )

/**
 * <!-- npa_destroy_meta_event -->
 *  
 * @brief Destroys a previously created meta event
 */
#define npa_destroy_meta_event npa_destroy_event

#define npa_destroy_event npa_destroy_event_handle
