 ...
 */
/*===========================================================================
Copyright (c) {2009-2013, 2016-2018, 2020} Qualcomm Technologies, Inc.
All Rights Reserved.
...
/*===========================================================================
  ...
  $Header: //components/rel/core.mpss/10.0/api/securemsm/crypto/secapi.h#7 $ 
  $DateTime: 2025/09/24 08:04:55 $ 
  $Author: pwbldsvc $
...
typedef enum
{
  ...
  SECAPI_ECIES               = 24, /**< Secapi interface for ecies profile. */
  SECAPI_SHA512              = 25, /**< Secapi interface for SHA512. **/
...
typedef enum
{
  ...
  SECAPI_PKX_ECIES_PROFILE_TYPE   = SECAPI_PKX_PARAM_START + 15,
   /**< PKX-specific parameter: ECIES Profile Info.  */
  SECAPI_PKX_ECIES_PARAM_TYPE      = SECAPI_PKX_PARAM_START + 16,
   /**< PKX-specific parameter: ECIES param Info.  */
  SECAPI_PKX_ECC_COMPRESSED_PUBLIC_KEY = SECAPI_PKX_PARAM_START + 17,
   /*ECIES compressed key only x coordinate is reuired for ECIES operation*/
  SECAPI_PKX_MAX_PARAM            = SECAPI_PKX_PARAM_START + 18,
...
*/
#define SECAPI_PKX_MAX_KEY_SIZE     1024
...
typedef enum 
{
  SECAPI_ECIES_PROFILE_X25519    = 0,  
  SECAPI_ECIES_PROFILE_P256      = 1, 
  SECAPI_ECIES_PROFILE_MAX  /**< Reserved.  */
} secapi_ecies_profile_type;

typedef enum 
{
  SECAPI_ECIES_AES_128 = 0,
  SECAPI_ECIES_CIPHER_MAX,
} secapi_ecies_cipher_t;
...
typedef enum 
{
  SECAPI_ECIES_CURVE_X25519 = 0,
  SECAPI_ECIES_CURVE_P256   = 1,
  SECAPI_ECIES_CURVE_MAX,
} secapi_ecies_curve_t;

/**
 * Digests provided by keymaster implementations.
 */
typedef enum {
  SECAPI_ECIES_DIGEST_SHA_2_256 = 0,
  SECAPI_ECIES_DIGEST_MAX,
} secapi_ecies_digest_t;
/*
 * Key derivation functions, mostly used in ECIES.
 */
typedef enum 
{
  /* HKDF defined in X9.63.1 with SHA256 */
  SECAPI_ECIES_KDF_X9_63 = 0,
  SECAPI_ECIES_KDF_MAX,
} secapi_ecies_kdf_t;

typedef struct secapi_ecies_params
{
  secapi_ecies_curve_t     ecies_curve;  /* ECC curve */
  secapi_ecies_kdf_t       kdf;          /* kdf */
  secapi_ecies_digest_t    digest;       /* digest hash function */
  secapi_ecies_cipher_t    cipher;       /* symetric algorithm (AES) scheme */
} secapi_ecies_params_type;
/**
...
typedef enum {
  RSA_OAEP_MGF1_HASH_ALGO_SHA1 = 0,
  RSA_OAEP_MGF1_HASH_ALGO_SHA256,
  RSA_OAEP_MGF1_HASH_ALGO_SHA384,
  RSA_OAEP_MGF1_HASH_ALGO_SHA512,
  RSA_OAEP_MGF1_HASH_ALGO_MAX
} secapi_pkx_rsa_oaep_mgf1_hash_algo_type;
...
typedef struct secapi_pkx_rsa_oaep_info_s
{
   ...
   secapi_pkx_rsa_oaep_mgf1_hash_algo_type mgf1_hash_algo;  /** Hash algo used in oaep for mgf1  */
...
typedef union {
    ...
    secapi_ecies_profile_type profile;
      /**< PKX ECIES profile type.*/
    secapi_ecies_params_type ecies_param;
      /**< PKX ECIES parameter type.*/
...
#define SECAPI_HSH_SHA384_DIGEST_SIZE         48 /**< SHA-384 digest size. */
#define SECAPI_HSH_SHA384_BLOCK_BYTE_LEN      128 /**< SHA-384 block length in bytes. */

#define SECAPI_HSH_SHA512_DIGEST_SIZE         64 /**< SHA-512 digest size. */
#define SECAPI_HSH_SHA512_BLOCK_BYTE_LEN      128 /**< SHA-512 block length in bytes. */
...
#define SECAPI_HSH_LARGEST_DIGEST_SIZE     SECAPI_HSH_SHA512_DIGEST_SIZE    /**< Maxmium hash digest size. */
#define SECAPI_HSH_LARGEST_BLOCK_BYTE_LEN  SECAPI_HSH_SHA512_BLOCK_BYTE_LEN /**< Maxmium hash block size */
...
typedef enum
{
  ...
  SECAPI_NON_CRYPTO_RANDOM     = 5   /**< Get pseudo random number from pool or from SW algorithm for non cryptographic purpose */
...
/**
  ...
  - SECAPI_SECURE_RANDOM -  Random data enhanced for maximum entropy (even 
    ...
    applications.
  - SECAPI_PKCS_RANDOM - Secure random data as above, but with no zero bytes 
    in the stream.  
  - SECAPI_NON_CRYPTO_RANDOM - Random data generated by HW or SW, suitable to 
    use for non-cryptographic operations which guarantees to return random number 
    always without blocking by calling into HW API's.
...
typedef enum
{
  ...
  SC_CID_SSGSCA,                 /* Generic Secure Channel                      */
...
/**
  Encrypts a message using the ECIES algorithm as per in section c3.4 of 
  3GPP spec from TS 33.501  

  @param[in] handle                 Handle to the crypto instance.
  @param[in] algo                   Algorithm.
  @param[in] msg                    Pointer to the plaintext data.
  @param[in] msg_len                plaintext data length.
  @param[in/Optional] ppAD          Shared information pointer [check limitation below].
  @param[in/Optional] pAD_len       Length of the shared information.
  @param[in,out] out                Pointer to the cryptogram  data.
  @param[in,out] out_len            pointer to length of the cryptogram data (ciphertext).
                                    Check the INFO section for minimum out_len required 

  @return
  E_SUCCESS     - Function executes successfully. \n
  E_NOT_ALLOWED - One or more ECIES parameters have not been set. \n
  E_IN_PROGRESS - CRYPTO-DSP interface was already busy. \n
  E_FAILURE     - Any other failures.

  @dependencies
  The complete implementation support only two profiles mentioned in 
   section c3.4 in 3GPP spec from TS 33.501 [i.e profile A and Profile B]

  @Limitation:
   ppAD and PPAD_len are OPTIONAL argument and kept for future use as 3GPP mentioned nothing about 
   how to use shared information.These two input argument will be ignored in encryption operation.

  @INFO:
  Cryptogram Format = ECC_KEY_SIZE + CIPHERTEXT + MAC
     NST curve      =   33byte     + msg_len    + 8byte
     X25519 curve   =   32byte     + msg_len    + 8byte
*/
secerrno_enum_type secapi_ecies_enc(
                 secapi_handle_type     handle,
                 secapi_algo_enum_type  algo,
                 uint8_t*               msg,
                 uint32_t               msg_len,
...
*/
secerrno_enum_type secapi_util_pkx_ecc_keygen(
                 uint8_t*                     priv_key,
                 size_t*                      priv_keylen,
                 uint8_t*                     peer_key,
                 size_t*                      peer_keylen,
...
/*
  ...
  @INFO:
*/
secerrno_enum_type secapi_util_pkx_ecc_dh(
                 uint8_t*                 shared,
                 size_t*                  shared_len,
                 uint8_t*                 peerkey,
                 size_t                   peerkey_len,
