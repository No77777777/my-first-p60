...
$Header: //components/rel/core.mpss/10.0/systemdrivers/InterruptController/src/qurt/kernel/uimage/uInterruptController.c#1 $
 ...
 */
#define UINT_IST_STACK_SIZE  2048
...
uInterruptController_AssignInterruptIndex(uint32 nInterruptID)
{
  ...
  if (uIntData.auIntLUT[nInterruptID] == UINT_NONE) 
  {
    for(index =0;index < MAX_NUMBER_OF_UINTS; index++)
    {
      if (uIntData.state[index].qurt_intr_id == UINT_NONE) 
      {
        uIntData.state[index].qurt_intr_id = nInterruptID;
        uIntData.auIntLUT[nInterruptID] = index;
        break;
      }   
    }
...
/*==========================================================================
   ...
   FUNCTION      uInterruptController_SetQURTTriggerConfig
...
==========================================================================*/
static int32 uInterruptController_SetQURTTriggerConfig
(
...
)
{
  ...
  switch (nTrigger)
  {
    case UINTERRUPTCONTROLLER_FALLING_EDGE_TRIGGER:
    case UINTERRUPTCONTROLLER_RISING_EDGE_TRIGGER:
      nDetectType = 1;
      nPolarity = 0;
      break;
  
    case UINTERRUPTCONTROLLER_LEVEL_HIGH_TRIGGER:
    case UINTERRUPTCONTROLLER_LEVEL_LOW_TRIGGER:
      nDetectType = 0;
      nPolarity = 0;
      break;
    default:
                            ...
                            uIRQ isr,uIRQCtx param, uint32 nFlags)
{
  ...
  if(UINTERRUPT_ERROR == uInterruptController_SetQURTTriggerConfig(uIntData.state[UINT_IDX(nInterruptID)].qurt_intr_id,trigger))
  {
    qurt_mutex_unlock(&uIntData.uIntLock);
    return UINTERRUPT_ERROR;
  }
