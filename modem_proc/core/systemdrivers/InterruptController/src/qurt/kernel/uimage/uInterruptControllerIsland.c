...
$Header: //components/rel/core.mpss/10.0/systemdrivers/InterruptController/src/qurt/kernel/uimage/uInterruptControllerIsland.c#1 $
...
/* 
 * Maximum number of unhandled interrupts
 */
#define UINT_MAX_UNHANDLED 10
...
)
{
  ...
  while(1)
  {
    ...
    while (1)
    {
      ...
      {
        uIntUnhandled++;
        if ( uIntUnhandled >= UINT_MAX_UNHANDLED )
        {
          ERR_FATAL("uInterruptController: Too many unhandled client ISRs! Last nInterruptID: %d, Total unhandled: %d",
                      nInterruptID, uIntUnhandled, 0);
        }
