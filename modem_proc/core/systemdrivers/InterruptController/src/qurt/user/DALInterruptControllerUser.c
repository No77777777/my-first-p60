...
$Header: //components/rel/core.mpss/10.0/systemdrivers/InterruptController/src/qurt/user/DALInterruptControllerUser.c#1 $ 
...
10/03/17   shm    Corrected condition in InterruptController_RegisterInterrupt to
                  return if ISR is already registered
...
static void ISTMain(void * ISTParam);
...
)
{
  ...
  while(1)
  {
   ...
   while (1)
   {
      ...
      /*
       * Get flag to determine whether this interrupt is a DSR 
       */
...
)
{
  ...
  if (nResult == QURT_EFATAL) 
  {
    ...
    pInterruptData->pInterruptState[nInterruptVector].pISTStack = NULL;
    return DAL_ERROR;
  }
...
{
  uint32 nInterruptPriority,nTrigger,nStackSize, nReqStackSize, nOldStackSize, nDsr = 0;
  InterruptControllerDevCtxt *pDevice;
  DALISR OldIsr;
   ...
   */
  if ( nInterruptPriority == 0 || nInterruptPriority >= IST_THREAD_PRIORITY_BASE)
  {
...
)
{
  ...
  (pInterruptData->pInterruptState[nInterruptVector].nThreadID == qurt_thread_get_id()))
  {
    ...
    nStatus = qurt_interrupt_register(nInterruptVector, 
                                      &pInterruptData->pInterruptState[nInterruptVector].ISTSignal, 
                                      INTERRUPT_SIGNAL_MASK);
    if (QURT_EOK != nStatus)
    {
      DALSYS_LogEvent(pDevice->DevId, DALSYS_LOGEVENT_FATAL_ERROR, 
