...
GPIOMgr_DeviceInit(uint32 ClientHandle, DalDeviceHandle* h)
{
       ...
       dCtxt->nMaxGpios = sProp.Val.dwVal;
        ...
        if ( eResult == DAL_SUCCESS && dCtxt->pState != NULL )
        {
          for ( nIter = 0; nIter < dCtxt->nMaxGpios; nIter++ ) 
          {
            ...
            dCtxt->pState[nIter].nDirConnID = (uint8)GPIOINT_SUMMARY_ID;
   ...
   if ( eResult == DAL_SUCCESS )
   {
       ...
       while ( !( dCtxt->pDirConnMap[dCtxt->nDirConnMapSize].nDirConnID     == 0 &&
                  ...
                  dCtxt->pDirConnMap[dCtxt->nDirConnMapSize].nDirectConnectBaseType == 0 ) )
       {
   ...
   eResult = DALSYS_GetPropertyValue(hGPIOMgrProps,
                                     "GPIOMGR_DIRCONN_BASE_MAP", 0,
                                     &sProp); 
   if ( eResult == DAL_SUCCESS )
   {
       dCtxt->pDirConnBaseMap = (GPIOMgrDirConnBaseMapType *)sProp.Val.pStruct;
       dCtxt->nDirConnBaseMapSize = 0;
       while ( !( dCtxt->pDirConnBaseMap[dCtxt->nDirConnBaseMapSize].nDirectConnectBaseType     == GPIOMGR_DIRECTCONNECT_NONE &&
                  dCtxt->pDirConnBaseMap[dCtxt->nDirConnBaseMapSize].nIndx   == 0  ) )
       {
         dCtxt->nDirConnBaseMapSize++;
       }
   }
...
/* =========================================================================
**  Function : GPIOMgr_GetDirConnBaseIdx
** =========================================================================*/
/**
...
static DALResult
GPIOMgr_GetDirConnBaseIdx( GPIOMgrDevCtxt *dCtxt, GPIOMgrDirectConnectBaseType nDirectConnectBaseType, uint32 *nIdx)
{
   uint32 i;
   GPIOMgrDirConnBaseMapType *pDirConnBaseMap = dCtxt->pDirConnBaseMap; 
   for ( i=0; i<dCtxt->nDirConnBaseMapSize ; i++)
   {
      if(pDirConnBaseMap->nDirectConnectBaseType == nDirectConnectBaseType)
      {
        *nIdx = pDirConnBaseMap->nIndx;
        return DAL_SUCCESS;
      }
      pDirConnBaseMap++;
   }
...
DALResult GPIOMgr_GetDirectConnectGPIO(uint32 ClientHandle, DalDeviceHandle * h, uint32 nGPIO, uint32* pDirConnID, uint32 *pDirectConnectBaseIdx)
{
    GPIOMgrDevCtxt *dCtxt = NULL;
    DALResult eResult = DAL_ERROR;
    uint32 nDirConn = 0;
    uint32 nCurrPD;
    uint32 nPDDevice;
    uint32 nDirConnID,nDirectConnectBaseIdx;
    ...
    if ( ( nGPIO < dCtxt->nMaxGpios ) )
    {
        ...
        {
            /*
             * If the interrupt is already reserved for this gpio
             */
            if ( ( (dCtxt->pDirConnMap[nDirConn].nGPIO == nGPIO) &&
               (dCtxt->pState[dCtxt->pDirConnMap[nDirConn].nGPIO].nFlags & GPIOMGR_FLAG_RESERVED) ) ||
               ( dCtxt->pDirConnMap[nDirConn].nGPIO == GPIOMGR_UNUSED_GPIO ) )
            {
                ...
                dCtxt->pState[nGPIO].nDirectConnectBaseType = (uint8)dCtxt->pDirConnMap[nDirConn].nDirectConnectBaseType;
                nDirConnID = (uint32)dCtxt->pDirConnMap[nDirConn].nDirConnID;
                if(GPIOMgr_GetDirConnBaseIdx( dCtxt, dCtxt->pDirConnMap[nDirConn].nDirectConnectBaseType, &nDirectConnectBaseIdx) == DAL_ERROR)
                {
                   return DAL_ERROR;
                }
                ...
                if ( eResult == DAL_SUCCESS && DalSys_CopyToUser(ClientHandle, pDirectConnectBaseIdx, &nDirectConnectBaseIdx, sizeof(nDirectConnectBaseIdx)) )
                {
                  eResult = DAL_ERROR_INVALID_ADDRESS;
                }
                else
...
DALResult GPIOMgr_ReleaseDirectConnectGPIO(uint32 ClientHandle, DalDeviceHandle * h, uint32 nGPIO, uint32 nInterruptID)
{
    GPIOMgrDevCtxt *dCtxt = NULL;
    DALResult eResult = DAL_ERROR;
    uint32 nDirConn = 0;
         ...
         ( dCtxt->pState != NULL ) )
    {
        for ( nDirConn = 0; nDirConn < dCtxt->nDirConnMapSize; nDirConn++ )
        {
            ...
            else if ( ( dCtxt->pDirConnMap[nDirConn].nGPIO == nGPIO ) &&
                 ( dCtxt->pDirConnMap[nDirConn].nInterruptID == nInterruptID ) )
            {
                dCtxt->pDirConnMap[nDirConn].nGPIO = GPIOMGR_UNUSED_GPIO;
                dCtxt->pState[nGPIO].nDirConnID = (uint8)GPIOINT_SUMMARY_ID;
