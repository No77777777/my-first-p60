...
struct GPIOMgrDevCtxt
{
  ...
  GPIOMgrDirConnBaseMapType *pDirConnBaseMap;
  uint32 nDirConnMapSize;
  uint32 nPDMapSize;
  uint32 nDirConnBaseMapSize;
};
...
DALResult GPIOMgr_GetDirectConnectGPIO(uint32, DalDeviceHandle*, uint32, uint32*, uint32*);
