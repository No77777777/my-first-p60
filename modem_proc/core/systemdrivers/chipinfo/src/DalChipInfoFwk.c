...
$Header: //components/rel/core.mpss/10.0/systemdrivers/chipinfo/src/DalChipInfoFwk.c#2 $
...
static DALResult
ChipInfo_DalChipInfo_GetSKU(uint32 ClientHandle, DalDeviceHandle *h, ChipInfoSKUType *pInfo)
{
  ChipInfoSKUType pInfo_copy;
  DALResult result;

  result = ChipInfo_GetSKU(&pInfo_copy);
  if (result != DAL_SUCCESS)
  {
    return result;
  }

  if (DalSys_CopyToUser(ClientHandle, pInfo, &pInfo_copy, sizeof(pInfo_copy)))
  {
    return DAL_ERROR_INVALID_ADDRESS;
  }
