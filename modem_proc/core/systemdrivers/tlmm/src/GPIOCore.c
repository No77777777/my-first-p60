...
/*========================================================================================================
  ...
  $Header: //components/rel/core.mpss/10.0/systemdrivers/tlmm/src/GPIOCore.c#2 $
...
ChipInfoFamilyType ChipFamily;

boolean bCoreInitialized = FALSE;
...
(
  uint32 nHWVersion , ChipInfoFamilyType eChipFamily
)
{
  if ( (eChipFamily == ChipFamily)  && (GPIO_MIN_TO_CHIPINFO(nHWVersion) <= ChipVersion) && (GPIO_MAX_TO_CHIPINFO(nHWVersion) >= ChipVersion) )
  {
    return TRUE;
  }
  return FALSE;
  ...
  if ( (nSlotIdx < GPIO_MAX_ARR_SIZE) && (eStatus != GPIO_ERROR) )
  {
    for ( nDescIdx = 0; nDescIdx < gnNumBlockDescs; nDescIdx++ )
    {
      if ( (Handle.DeviceId == GPIOBlockDescriptors[nDescIdx].DeviceId) &&
         ( (TRUE == GPIO_IsBlockDescSupported(GPIOBlockDescriptors[nDescIdx].nHWVersion ,GPIOBlockDescriptors[nDescIdx].eChipFamily))) )
      {
        break;
      }
    }
