...
TLMM_DalTlmm_GetGpioNumber(uint32 ClientHandle, DalDeviceHandle* h, DALGpioSignalType gpio_config, uint32 *  number) 
{
  ...
  nResult = TLMM_GetGpioNumber(((DalTlmmHandle *)h)->pClientCtxt, gpio_config, &nPinNumber);
  
  if (nResult == DAL_SUCCESS)
  {
    if ( DalSys_CopyToUser(ClientHandle, number, &nPinNumber, sizeof(uint32)))
    {
      return DAL_ERROR_INVALID_ADDRESS;
    }
  }
...
{
  DALGpioValueType GpioValue = DAL_GPIO_LOW_VALUE;
  DALResult nResult;
  ...
  nResult = TLMM_GpioIn(((DalTlmmHandle *)h)->pClientCtxt, gpio_config, &GpioValue);
  
  if ( DAL_SUCCESS == nResult )
  {
    if ( DalSys_CopyToUser(ClientHandle, value, &GpioValue, sizeof(DALGpioValueType)) )
    {
      return DAL_ERROR_INVALID_ADDRESS;
    }
  }
...
TLMM_DalTlmm_GetCurrentConfig(uint32 ClientHandle, DalDeviceHandle* h, uint32 gpio_number, DALGpioSignalType* gpio_config ) 
{
  ...
  nResult = TLMM_GetCurrentConfig( ((DalTlmmHandle *)h)->pClientCtxt, gpio_number, &nGpioConfig );
...
TLMM_DalTlmm_GetPinStatus(uint32 ClientHandle, DalDeviceHandle* h, uint32  gpio_number, DALGpioStatusType* status ) 
{
  ...
  nResult = TLMM_GetPinStatus( ((DalTlmmHandle *)h)->pClientCtxt, gpio_number, &nStatus );
...
TLMM_DalTlmm_GetInactiveConfig(uint32 ClientHandle, DalDeviceHandle* h, uint32 gpio_number, DALGpioSignalType* gpio_config ) 
{
  ...
  nResult = TLMM_GetInactiveConfig( ((DalTlmmHandle *)h)->pClientCtxt, gpio_number, &nGpioConfig );
...
TLMM_DalTlmm_GetOutput(uint32 ClientHandle, DalDeviceHandle* h, uint32 gpio_number, DALGpioValueType* value ) 
{
  ...
  nResult = TLMM_GetOutput( ((DalTlmmHandle *)h)->pClientCtxt, gpio_number, &nValue );
