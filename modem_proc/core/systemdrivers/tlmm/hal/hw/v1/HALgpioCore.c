...
$Header: //components/rel/core.mpss/10.0/systemdrivers/tlmm/hal/hw/v1/HALgpioCore.c#1 $
...
)
{
   ...
   */
  if ( inpdw(pBlock->nBaseAddr + nOffs + (pBlock->nPinRegWidth * nGPIO) + HAL_GPIO_CFG_REG_OFFS_BMSK) & HAL_GPIO_CFG_EGPIO_PRESENT_BMSK )
  {
    nRegVal |= HAL_GPIO_CFG_EGPIO_ENABLE_BMSK;
  }

  outpdw(pBlock->nBaseAddr + nOffs + (pBlock->nPinRegWidth * nGPIO) + HAL_GPIO_CFG_REG_OFFS_BMSK, nRegVal);
...
{
  return (inpdw(pBlock->nBaseAddr + nOffs + (pBlock->nPinRegWidth * nGPIO) + HAL_GPIO_IN_OUT_REG_OFFS_BMSK) & 0x1) ? GPIO_HIGH : GPIO_LOW;
...
{
  if ( ((inpdw(pBlock->nBaseAddr + nOffs +
...
{
  uint32 nRegVal = inpdw(pBlock->nBaseAddr + nOffs + (pBlock->nPinRegWidth * nGPIO) + HAL_GPIO_CFG_REG_OFFS_BMSK);
