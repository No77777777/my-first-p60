...
$Header: //components/rel/core.mpss/10.0/systemdrivers/tlmm/hal/hw/v1/HALgpioExt.c#1 $
...
{
  uint32 nGPIOCfg = inpdw(pBlock->nBaseAddr + nOffs + (pBlock->nPinRegWidth * nGPIO) + HAL_GPIO_LP_CFG_REG_OFFS_BMSK);
...
)
{
  ...
  outpdw(pBlock->nBaseAddr + nOffs + (pBlock->nPinRegWidth * nGPIO) + HAL_GPIO_LP_CFG_REG_OFFS_BMSK, nGPIOCfgVal);
}
