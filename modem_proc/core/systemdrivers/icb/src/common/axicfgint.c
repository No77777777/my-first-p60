 ...
 * 
 * $Header: //components/rel/core.mpss/10.0/systemdrivers/icb/src/common/axicfgint.c#1 $ 
...
ICBArb_Bus_Version get_noc_version(HAL_noc_InfoType * bus_info)
{
  return bus_info->noc_version;
}
...
static void _AxiCfgInt_QoSSetLocalQNOC4
(
  icb_master_def_type *pMaster,
  uint64_t             uTotalBW,
  uint64_t             uActiveBW,
  icb_qnoc4_master_defaults_type *pMasterQos
)
{
  uint32_t port;
  icb_qnoc4_master_defaults_type *qos = (icb_qnoc4_master_defaults_type *)pMasterQos;
  ...
  for( port = 0; port < pMaster->uNumPorts; port++ )      
  {
    ...
    HAL_noc_SetDefaultPriority( pMaster->pBus->pInfo, pMaster->pQosPortIds[port], qos->default_priority );
    /* Disable the limiter first.
       There's no guarantee that this can be atomically configured. */
    HAL_noc_EnableLimiter( pMaster->pBus->pInfo, pMaster->pQosPortIds[port], false );
    HAL_noc_SetLimiter( pMaster->pBus->pInfo, pMaster->pQosPortIds[port], &qos->limiter );
    HAL_noc_EnableLimiter( pMaster->pBus->pInfo, pMaster->pQosPortIds[port], qos->limiter_enabled );
    /* Disable the regulator first. 
          There's no guarantee that this can be atomically configured. */
    HAL_noc_SetRegulatorMode( pMaster->pBus->pInfo, pMaster->pQosPortIds[port], &regulator_mode );
    HAL_noc_SetRegulator( pMaster->pBus->pInfo, pMaster->pQosPortIds[port], &qos->regulator );
    HAL_noc_SetRegulatorMode( pMaster->pBus->pInfo, pMaster->pQosPortIds[port], &qos->regulator_mode );
    HAL_noc_EnableUrgencyForwarding( pMaster->pBus->pInfo, pMaster->pQosPortIds[port], qos->urgency_forwarding_enabled );      
  }
}

static void _AxiCfgInt_QoSSetLocalQNOC2
(
  icb_master_def_type *pMaster,
  uint64_t             uTotalBW,
  uint64_t             uActiveBW,
  icb_qnoc2_master_defaults_type *pMasterQos
)
{
  icb_qnoc2_master_defaults_type *pQos = (icb_qnoc2_master_defaults_type *)pMasterQos;
...
)
{
  ...
  switch( pMaster->pBus->busType )
  {
    ...
    case ICB_BUS_NOC:
    {
      if(get_noc_version(pMaster->pBus->pInfo) == NOC_VERSION_QNOC4)
      {
		icb_qnoc4_master_defaults_type *pMasterQos = (icb_qnoc4_master_defaults_type *)pQosPolicy;  
        _AxiCfgInt_QoSSetLocalQNOC4(pMaster, uTotalBW, uActiveBW, pMasterQos);
      }
      else 
      {
		icb_qnoc2_master_defaults_type *pMasterQos = (icb_qnoc2_master_defaults_type *)pQosPolicy;
        _AxiCfgInt_QoSSetLocalQNOC2(pMaster, uTotalBW, uActiveBW, pMasterQos);
      }
      break;
    }
...
)
{
  ...
  switch( pMaster->pBus->busType )
  {
    ...
    case ICB_BUS_NOC:
    {
      if(get_noc_version(pMaster->pBus->pInfo) == NOC_VERSION_QNOC4)
      {
        icb_qnoc4_master_defaults_type *pOverride = (icb_qnoc4_master_defaults_type *)pMaster->pQosPolicyOverride;
        memset(pOverride, 0, sizeof(icb_qnoc4_master_defaults_type));
        pOverride->default_priority = pQosPolicy->defaultPriority;
        pOverride->limiter.bandwidth  = pQosPolicy->limiter.bandwidth;
        pOverride->limiter.saturation  = pQosPolicy->limiter.saturation;
        pOverride->limiter_enabled     = pQosPolicy->limiterEnabled;
        pOverride->regulator.low_priority = pQosPolicy->regulator.low_priority;
        pOverride->regulator.high_priority    = pQosPolicy->regulator.high_priority;
        pOverride->regulator.bandwidth        = pQosPolicy->regulator.bandwidth;
        pOverride->regulator.saturation       = pQosPolicy->regulator.saturation;
        pOverride->regulator_mode.read        = pQosPolicy->regulatorMode.read;
        pOverride->regulator_mode.write       = pQosPolicy->regulatorMode.write;
        pOverride->urgency_forwarding_enabled = pQosPolicy->urgencyForwardingEnabled;
      }
      else
      {
        icb_qnoc2_master_defaults_type *pOverride = (icb_qnoc2_master_defaults_type *)pMaster->pQosPolicyOverride;
        memset(pOverride, 0, sizeof(icb_qnoc2_master_defaults_type));
        ...
        * Otherwise, just overwrite it. */
        if( NULL == pMaster->pQosPolicyOverride )
        {
          ...
          switch( pMaster->pBus->busType )
          {
            ...
            case ICB_BUS_NOC:
			  if(get_noc_version(pMaster->pBus->pInfo) == NOC_VERSION_QNOC4)
                size = sizeof(icb_qnoc4_master_defaults_type);
			  else
				size = sizeof(icb_qnoc2_master_defaults_type);
              break;
            default:
              return ret;
          }
