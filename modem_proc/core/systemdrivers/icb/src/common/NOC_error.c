...
$Header: //components/rel/core.mpss/10.0/systemdrivers/icb/src/common/NOC_error.c#1 $
...
void NOC_Error_Init( void )
{
   ...
   for(i=0; i < NOCERR_propdata->len; i++)
   {
      ...
      /* No power domain, initialize now if we aren't deferring. */
      else if( NOCERR_DEFER_NONE == NOCInfo[i].deferral_pref )
      {
