...
$Header: //components/rel/core.mpss/10.0/systemdrivers/icb/src/common/ABTimeout.c#1 $ 
...
2016/02/04  bc     Added register sharing safety for multiple PDs
...
typedef struct
{
    uint8_t  hw_version;              /**< ABT_HW_VERSION offset */
    uint8_t  inst_id;                 /**< ABT_INST_ID offset */
    uint8_t  num_slaves;              /**< ABT_NUM_SLAVES offset */
    uint8_t  timer_loadval;           /**< ABT_TIMER_LOADVAL offset */
    uint8_t  timer_mode;              /**< ABT_MODE offset */
    uint8_t  intr_status;             /**< ABT_INTR_STATUS offset */
    uint8_t  intr_clear;              /**< ABT_INTR_CLEAR offset */
    uint8_t  intr_enable;             /**< ABT_INTR_ENABLE offset */
    uint8_t  synd_valid;              /**< ABT_SYND_VALID offset */
    uint8_t  synd_clear;              /**< ABT_SYND_CLEAR offset */
    uint8_t  synd_id;                 /**< ABT_SYND_ID offset */
    uint8_t  synd_addr0;              /**< ABT_SYND_ADDR0 offset */
    uint8_t  synd_addr1;              /**< ABT_SYND_ADDR1 offset */
    uint8_t  synd_hready;             /**< ABT_SYND_HREADY offset */   
}ABT_hw_type;
...
*/ 
/*============================================================================*/
static ABT_hw_type* ABT_Get_HW_type(uint32_t version)
{
    switch(version)
    {
...
{
 uint8_t i, index, slave;
 uint32_t intr_status[MAX_NUM_INTR_REGS], synd_valid, hang_detected = 0;
 ABT_hw_type* hw;
     ...
     if(intr_status[index] & (1 << slave))
     {
        ...
        // Read syndrom registers
         ...
         if(synd_valid != 0)
         {
             ...
             /* Error sources listed in order of precidence.
             ** If we have found an error, we cannot service any afterwards. */
             break;
         }
     }
 }
 if(hang_detected > 0)
 {
     ...
     ABT_Platform_Report_Hang(syndrome.slave_name);
 }
 return arg;
}
...
{
   uint8_t i, index, slave;
   uint32_t reg_data[MAX_NUM_INTR_REGS];
   ABT_hw_type* hw;
