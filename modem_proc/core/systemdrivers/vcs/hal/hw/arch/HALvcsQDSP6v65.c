...
$Header: //components/rel/core.mpss/10.0/systemdrivers/vcs/hal/hw/arch/HALvcsQDSP6v65.c#1 $
 ...
 */
#define HAL_LDO_VREF_MIN_UV  335900
...
)
{
  ...
  // Config LDO voltage.
  HAL_VCS_HWIO_OUTF(QDSP6SS_LDO_VREF_SET, pLDO->nAddr, VREF_LDO, nData);
  
  // Trigger voltage update.
  HAL_VCS_HWIO_OUTF(QDSP6SS_LDO_VREF_CMD, pLDO->nAddr, LDO_VREF_SEL_UPDATE, 0x1);
...
)
{
  ...
  nData = HAL_VCS_HWIO_INF(QDSP6SS_LDO_VREF_SET, pLDO->nAddr, VREF_LDO);
...
)
{
  ...
  // Read QDSP6SS_CP_CLK_CTL register to make sure the previous step is complete.
  HAL_VCS_HWIO_INF(QDSP6SS_CP_CLK_CTL, pCoproc->nAddr, CLK_ENA);
