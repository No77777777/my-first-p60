...
/*  
  ...
  ==================================================================== 
  $Header: //components/rel/core.mpss/10.0/systemdrivers/vcs/src/RailImpls/VCSLDORailImpl.c#1 $
...
/**
  ...
  @param *pRail [in]  -- Pointer to LDO rail node.
...
)
{
      ...
      pImpl->pActiveConfig->eCornerMax >= eCornerLDO)
  {
     ...
     * Set the LDO voltage.
     */
    eResult = VCS_LDORailImplSetVoltage(pRail, nVoltageUV);
    if (eResult != DAL_SUCCESS)
    {
      return eResult;
    }
...
)
{
  ...
  eResult = VCS_LDORailImplSetVoltage(pRail, pEventData->voltage_uv);
  if (eResult == DAL_SUCCESS)
  {
    /*
...
)
{
  ...
  for (i = 0; i < pImpl->nNumCornerFuses; i++)
  {
    ...
    if (eCorner >= RAIL_VOLTAGE_LEVEL_MAX)
    {
