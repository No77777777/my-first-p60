...
/*
  ...
  $Header: //components/rel/core.mpss/10.0/systemdrivers/clock/src/ClockDriver.c#1 $
...
)
{
  ...
  if (nRefCountTotal == 0)
  {
    ...
    {
      eResult = Clock_ConfigSource(pDrvCtxt, pDomain->pSource, pNewConfig->pSourceFreqConfig);
      if (eResult != DAL_SUCCESS)
      {
        return DAL_ERROR_INTERNAL;
      }
...
)
{
  ...
  if (pPowerDomain->nReferenceCountTotal == 0)
  {
     ...
     */
    if (pPowerDomain->pRail)
    {
      VRegRequest.eVRegLevel = RAIL_VOLTAGE_LEVEL_MIN_SVS;
      VRegRequest.bSuppressible = CLOCK_FLAG_IS_SET(pPowerDomain, SUPPRESSIBLE);

      eResult = Clock_VoltageRequest(pPowerDomain->pRail,
...
)
{
  ...
  uint32               i;
   ...
   /*
    * Define remote resource - Added for Niocbar RPM based targets
    */
  for (i=0; i<pDrvCtxt->pBSP->nNumRemoteNodes; i++)
  { 
    npa_remote_define_resource(&(pDrvCtxt->pBSP->aRemoteNodes[i].Resource),pDrvCtxt->pBSP->aRemoteNodes[i].nInitState,NULL);
  }
