...
/*
  ...
  $Header: //components/rel/core.mpss/10.0/systemdrivers/clock/src/ClockSources.c#2 $
  $DateTime: 2019/12/01 23:55:01 $
  $Author: pwbldsvc $
  ...
  else
  {
    ...
    if (!bStub)
    {
      ...
      if (bNeedsCal && !CLOCK_FLAG_IS_SET(pSource, READ_ONLY))
      {
        ...
        {
          DALSYS_LogEvent(DALDEVICEID_CLOCK, DALSYS_LOGEVENT_FATAL_ERROR,
            "Failed to calibrate source[%s].", pSource->szName);
          return;
        }
      }
      ...
      if (pSource->nConfigMask & CLOCK_CONFIG_PLL_FSM_MODE_ENABLE)
      {
        ...
        {
          DALSYS_LogEvent(DALDEVICEID_CLOCK, DALSYS_LOGEVENT_FATAL_ERROR,
            "Unable to enable source[%s] vote", pSource->szName);
          return;
        }
      }
      else
      {
        ...
        {
          DALSYS_LogEvent(DALDEVICEID_CLOCK, DALSYS_LOGEVENT_FATAL_ERROR,
            "Unable to enable source[%s]", pSource->szName);
          return;
        }
      }
    }
