...
$Header: //components/rel/core.mpss/10.0/systemdrivers/clock/hal/hw/v1/HALclkLucidPLL.c#2 $
...
)
{
  ...
  boolean     bPLLEnabled = FALSE, bInStandBy = FALSE, bResult = FALSE;
  
  /*
  * Check if the PLL is in standby, which will require an update to be triggered.
  */
  if ( HAL_CLK_PLL_HWIO_IN(PLL_MODE, pSource) & HWIO_FMSK(PLL_MODE, PLL_RESET_N) )
  {
    bInStandBy = TRUE;
  }
  ...
  /*
   * If the PLL was in standby but needs to be set to a different frequency.
   * we need to trigger the update to latch the new frequency.
   */
  if ( (bInStandBy == TRUE) && (bPLLEnabled == FALSE) )
  {
    /*
     * Latch in the new configuration values.
     */
...
{
  /*
   * Lucid PLLs have a dedicated bit in PLL_MODE to indicate calibration.
   */
