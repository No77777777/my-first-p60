...
$Header: //components/rel/core.mpss/10.0/systemdrivers/hal/tlmmtile/src/HALtlmmtile.c#1 $
...
#define HALTLMMTILE_VALIDDSP( x )  ( x < gHalTlmm.nNumDirConnectBases )

#define HALTLMMTILE_VALIDDIRCONN( nDirConn,nDSPidx ) ( nDirConn <= gHalTlmm.DirConnIntrCfg[nDSPidx].nMax )
...
static uint32 HAL_tlmmtile_GetDirConnValues ( uint32 nDirConn, HAL_tlmmtile_FieldType nField, HalTlmmOutArgs* pArgs, uint32 nDSPidx)
{
  ...
  nOffset = gHalTlmm.DirConnIntrCfg[nDSPidx].nOffset * nDirConn;

  switch( nField )
  {
    case HAL_TLMMTILE_FIELD_DIRCONNINTRCFG_SEL:
      nShift = gHalTlmm.DirConnIntrCfg[nDSPidx].nDirConnIntrCfgSelShift;
      nBmsk = gHalTlmm.DirConnIntrCfg[nDSPidx].nDirConnIntrCfgSelBmsk;
      break;
    case HAL_TLMMTILE_FIELD_DIRCONNINTRCFG_POL:
      nShift = gHalTlmm.DirConnIntrCfg[nDSPidx].nDirConnIntrCfgPolShift;
      nBmsk = gHalTlmm.DirConnIntrCfg[nDSPidx].nDirConnIntrCfgPolBmsk;
      break;
    ...
    case HAL_TLMMTILE_REG_CFG:
      ...
      switch( nField )
      {
        ...
        case HAL_TLMMTILE_FIELD_CFG_HIHYS:
          ...
          break;
        case HAL_TLMMTILE_FIELD_CFG_EGPIO_PRESENT:
          nShift = gHalTlmm.GpioConfig.nGpioEgpioPresentShift;
          nBmsk = gHalTlmm.GpioConfig.nGpioEgpioPresentBmsk;
          break;
        case HAL_TLMMTILE_FIELD_CFG_EGPIO_ENABLE:
          nShift = gHalTlmm.GpioConfig.nGpioEgpioEnableShift;
          nBmsk = gHalTlmm.GpioConfig.nGpioEgpioEnableBmsk;
          break;
        case HAL_TLMMTILE_FIELD_ALL:
          nShift = 0;
          nBmsk = ~0;
...
uint32 HAL_tlmmtile_SetDirConn ( uint32 nDirConn, HAL_tlmmtile_FieldType nField, uint32 nVal, uint32 nDSPidx)
{
  HalTlmmOutArgs Args;
  uint32 nTile;

  if( !HALTLMMTILE_VALIDDSP( nDSPidx))
  {
    return HALTLMMTILE_ERR;
  }
  
  if( !HALTLMMTILE_VALIDDIRCONN( nDirConn, nDSPidx ) )
  {
    return HALTLMMTILE_ERR;
  }

  if ( HAL_tlmmtile_GetDirConnValues ( nDirConn, nField, &Args, nDSPidx ) == HALTLMMTILE_ERR )
  {
    return HALTLMMTILE_ERR;
  }
  ...
  {
    HALTLMMTILE_OUT( gHalTlmm.DirConnIntrCfg[nDSPidx].nBase[nTile],
                     Args.nOffset,
                     Args.nShift,
                     Args.nBmsk,
                     nVal );
...
uint32 HAL_tlmmtile_GetDirConn ( uint32 nDirConn, HAL_tlmmtile_FieldType nField , uint32 nDSPidx)
{
  HalTlmmOutArgs Args;
  ...
  return HALTLMMTILE_IN( gHalTlmm.DirConnIntrCfg[nDSPidx].nBase[0],
                         Args.nOffset,
                         Args.nShift,
                         Args.nBmsk );
