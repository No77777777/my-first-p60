...
$Header: //components/rel/core.mpss/10.0/systemdrivers/hal/tlmmtile/src/HALtlmmtileInternal.h#2 $
...
typedef struct
{
  ...
  uint32 nGpioEgpioPresentShift;
  uint32 nGpioEgpioPresentBmsk;
  uint32 nGpioEgpioEnableShift;
  uint32 nGpioEgpioEnableBmsk;
}
...
typedef struct
{
  ...
  uint32                        aTiles[5];
  uint32                        nPinRegWidth;
  uint32                        nNumDirConnectBases;
  uint32                        pHalTlmmTileDirConnArray[1][5];
  ChipInfoFamilyType            eChipFamily;
}
HalTlmmPlatformDescriptionType;
...
typedef struct
{
  ...
  HalTlmmGpioDirConnIntrCfgType DirConnIntrCfg[2];
