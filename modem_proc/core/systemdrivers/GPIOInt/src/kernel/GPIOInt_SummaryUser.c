...
$Header: //components/rel/core.mpss/10.0/systemdrivers/GPIOInt/src/kernel/GPIOInt_SummaryUser.c#1 $
...
{
  SummaryGpioCfgCtxtType *device = (SummaryGpioCfgCtxtType *)ISTParam; 
  uint32 nIdx = 0;
  uint32 nGpio = 0;
  int nQDIResult, nIndexSearchResult;


  while (1)
  {

    nQDIResult = qurt_qdi_handle_invoke(device->gpioint_qdi, GPIOINT_QDI_RECEIVE_SUMMARY_SIGNAL, qurt_process_get_id(), &nGpio);

    nIndexSearchResult = SummaryGpioInternal_FindIdxByGpio(nGpio, &nIdx);

    if ( nQDIResult != GPIOINT_QDI_SUCCESS || nIndexSearchResult != DAL_SUCCESS )
    {
      continue;
    }
     ...
     */
    //if(device->pStateTable[gpio].event)
    //{
      
      //DALSYS_EventCtrl(device->pStateTable[gpio].event, DALSYS_EVENT_CTRL_TRIGGER);
    //}
    //else 
    //{
      /*
       ...
       */
      device->pStateTable[nIdx].isr(device->pStateTable[nIdx].isr_param);
    //}
...
static 
DALResult 
GPIOIntSummaryInternal_SpawnServer(SummaryGpioCfgCtxtType *device)
{
  int nResult;
