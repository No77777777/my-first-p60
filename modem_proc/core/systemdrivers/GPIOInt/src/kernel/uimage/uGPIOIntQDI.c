...
$Header: //components/rel/core.mpss/10.0/systemdrivers/GPIOInt/src/kernel/uimage/uGPIOIntQDI.c#1 $
...
)
{
  ...
  return uGPIOIntKernel_SetMPMTrigger(nGPIO,(uGPIOIntTriggerType)nTrigger);
...
)
{
  ...
  nResult = uGPIOIntKernel_ClearGPIOIntConfig(nGPIO);
...
)
{
  ...
  nResult = uGPIOIntKernel_SetGPIOIntConfig((uint32)nGPIO,(uGPIOIntTriggerType)nTrigger,
...
)
{
  ...
  nResult = uGPIOIntKernel_SetDirectConnectGPIOMapping((uint32)nGPIO,(uint32)nQurtInterruptID);
  return nResult;
...
)
{
  ...
  nResult = uGPIOIntKernel_ClearDirectConnectGPIOMapping((uint32)nGPIO);
  return nResult;
...
FUNCTION    uGPIOIntQDI_MapMPMInterrupt

DESCRIPTION
  Update MPM interrupt id in ugpiointkerneldata structure
 
DEPENDENCIES
  None.

RETURN VALUE
  UGPIOINT_QDI_SUCCESS 
  UGPIOINT_QDI_ERROR if the gpio is invalid
 
SIDE EFFECTS
  None

===========================================================================*/
int uGPIOIntQDI_MapMPMInterrupt
(
  int hClientHandle,
  int nGPIO,
  int mpmid
)
{
  ...
  nResult = uGPIOIntKernel_MapMPMInterrupt((uint32)nGPIO, (uint32)mpmid);
  return nResult;
 
} /* End uGPIOIntQDI_MapMPMInterrupt */
                          ...
                          qurt_qdi_arg_t a9)
{
  switch (method) 
  {
    ...
    case UGPIOINT_QDI_MAP_UGPIOINT:
      return uGPIOIntQDI_MapMPMInterrupt(hClientHandle,a1.num,a2.num);
