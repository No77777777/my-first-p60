...
$Header: //components/rel/core.mpss/10.0/systemdrivers/GPIOInt/src/user/uimage/uGPIOIntUserIsland.c#1 $
  ...
  while(1)
  {
    ...
    while (1)
    {
      ...
      if (nSignalValue & uGPIOIntData.nGPIOIntRegistrationMask)
      {
        ...
        if (nStatus != QURT_EOK) 
        {
          ERR_FATAL("Cannot register GPIO Interrupt for GPIO %d, interrupt %d", gpio, uGPIOIntData.state[nGpioIdx].qurt_intr_id, 0);
      }
        qurt_mutex_unlock(&uGPIOIntData.uGPIOIntLock);
        continue;     
      }
