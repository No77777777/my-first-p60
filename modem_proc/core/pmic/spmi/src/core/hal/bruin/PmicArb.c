 ...
 * $Header: //components/rel/core.mpss/10.0/pmic/spmi/src/core/hal/bruin/PmicArb.c#1 $
 * $Change: 18983508 $ 
 * 
 *                              Edit History
 * Date     Description
...
#define MAX_TRANSACTION_LEN_DEBUG 1
...
spmi_dbg_cmd prev_spmi_dbg_cmd = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF};
                   ...
                   Spmi_AccessPriority priority, uint16 address)
{
    ...
    {
        SPMI_HWIO_OUT( HWIO_QDSS_AODBG_PMIC_ARB_PMIC_ARB_DEBUG_SPMI_CHNL_WDATAg_ADDR(base_addr,i), (uint32)(data[i]) );
    }
    return;    
}

/*Comment this out for Napali V1 */
/*
static void 
                   ...
                   uint8 regOffset, uint32 len)
{
    ...
    if(len != prev_spmi_dbg_cmd.byte_cnt || slaveId != prev_spmi_dbg_cmd.sid)
    {  
        SPMI_HWIO_OUT( HWIO_QDSS_AODBG_PMIC_ARB_PMIC_ARB_DEBUG_SPMI_CHNL_CMD1_ADDR( base_addr ),
                      (len       << HWIO_QDSS_AODBG_PMIC_ARB_PMIC_ARB_DEBUG_SPMI_CHNL_CMD1_BYTE_CNT_SHFT) |
                      (slaveId   << HWIO_QDSS_AODBG_PMIC_ARB_PMIC_ARB_DEBUG_SPMI_CHNL_CMD1_SID_SHFT) );
    }
    
    if(ppId != prev_spmi_dbg_cmd.ppId)
    {
        SPMI_HWIO_OUT( HWIO_QDSS_AODBG_PMIC_ARB_PMIC_ARB_DEBUG_SPMI_CHNL_CMD2_ADDR( base_addr ),
                      (ppId      << HWIO_QDSS_AODBG_PMIC_ARB_PMIC_ARB_DEBUG_SPMI_CHNL_CMD2_PPID_SHFT) );
    }
    
    if(regOffset != prev_spmi_dbg_cmd.address)
    {
        SPMI_HWIO_OUT( HWIO_QDSS_AODBG_PMIC_ARB_PMIC_ARB_DEBUG_SPMI_CHNL_CMD3_ADDR( base_addr ),
                      (regOffset << HWIO_QDSS_AODBG_PMIC_ARB_PMIC_ARB_DEBUG_SPMI_CHNL_CMD3_ADDRESS_SHFT) );
    }
    
    
    SPMI_HWIO_OUT( HWIO_QDSS_AODBG_PMIC_ARB_PMIC_ARB_DEBUG_SPMI_CHNL_CMD0_ADDR( base_addr ),
                  (cmd       << HWIO_QDSS_AODBG_PMIC_ARB_PMIC_ARB_DEBUG_SPMI_CHNL_CMD0_OPCODE_SHFT) |
                  (priority  << HWIO_QDSS_AODBG_PMIC_ARB_PMIC_ARB_DEBUG_SPMI_CHNL_CMD0_PRIORITY_SHFT) );
    
    prev_spmi_dbg_cmd.byte_cnt = len; 
    prev_spmi_dbg_cmd.sid      = slaveId;
    prev_spmi_dbg_cmd.ppId     = ppId;
    prev_spmi_dbg_cmd.address  = regOffset;
    prev_spmi_dbg_cmd.opcode   = cmd; 
    prev_spmi_dbg_cmd.priority = priority;
...
static void readSpmiDataDebug(uint8 *base_addr, uint8* data, uint32 bytesToRead)
{
    ...
    {
        data[i] = (uint8)( SPMI_HWIO_IN( HWIO_QDSS_AODBG_PMIC_ARB_PMIC_ARB_DEBUG_SPMI_CHNL_RDATAh_ADDR(base_addr, i) ) );
    }
    return;
}
...
static Spmi_Result waitForStatusDoneDebug(uint8 *base_addr)
{
    ...
    {
        SpmiOs_Wait( 1 );
        status = SPMI_HWIO_IN( HWIO_QDSS_AODBG_PMIC_ARB_PMIC_ARB_DEBUG_SPMI_CHNL_STATUS_ADDR( base_addr ) ); 
    }
    
    if(status & HWIO_QDSS_AODBG_PMIC_ARB_PMIC_ARB_DEBUG_SPMI_CHNL_STATUS_DROPPED_BMSK) 
    {
        result = SPMI_FAILURE_TRANSACTION_DROPPED;
    }
    else if(status & HWIO_QDSS_AODBG_PMIC_ARB_PMIC_ARB_DEBUG_SPMI_CHNL_STATUS_DONE_BMSK)
    {
        if(status & HWIO_QDSS_AODBG_PMIC_ARB_PMIC_ARB_DEBUG_SPMI_CHNL_STATUS_FAILURE_BMSK) 
        {
            result = SPMI_FAILURE_TRANSACTION_FAILED;
        }
        else if(status & HWIO_QDSS_AODBG_PMIC_ARB_PMIC_ARB_DEBUG_SPMI_CHNL_STATUS_DENIED_BMSK) 
        {
            result = SPMI_FAILURE_TRANSACTION_DENIED;
        }
    }
                              ...
                              uint32*             bytesTransacted )
{
    ...
    *bytesTransacted = 0;    
    ...
    while(dataLen > 0)
    {
        ...
        len = dataLen > MAX_TRANSACTION_LEN_DEBUG ? MAX_TRANSACTION_LEN_DEBUG : dataLen;
        
        //Check for previous done status here
        status = SPMI_HWIO_IN( HWIO_QDSS_AODBG_PMIC_ARB_PMIC_ARB_DEBUG_SPMI_CHNL_STATUS_ADDR( dbgBaseAddrs ) );
        
        if( !(status & HWIO_QDSS_AODBG_PMIC_ARB_PMIC_ARB_DEBUG_SPMI_CHNL_STATUS_DONE_BMSK) )
        {
            return SPMI_FAILURE_BUS_BUSY;
        }
