...
$Header: //components/rel/core.mpss/10.0/pmic/pm/application/bcl/src/pm_bcl_comp_app.c#1 $
...
===========================================================================*/
static pm_err_flag_type pmapp_vadc_btm_thr_config(uint32 voltage,
...
void* pmapp_vadc_btm_interrupt_cb(void *pArg, uint32 uIntrStatusMask);
                          ...
                          boolean irq_en)
{
  ...
  else if(bcl_status.is_supported==FALSE)
  {
    ...
    else if(bcl_status.is_vadc_periph==TRUE)
    {
        err_flag = pmapp_vadc_btm_thr_config(voltage, thr_type,
                                             cb_ptr, irq_en);
    }
    else if(bcl_status.is_vadc_periph==FALSE)
  {
...
{
  pmapp_bcl_cfg_info_type *bcl_data   = NULL;
  ...
  bcl_data = target_ptr->pmapp_bcl_info;

  if (bcl_data == NULL)
  {
    PM_LOG_MSG_INFO("Unable to get BCL COMP Data");
    return;
  }

  bcl_status.pmic_chip = bcl_data->pmic_chip;

    pm_vadc_data_type *vadc_ptr = pm_vadc_get_data(bcl_status.pmic_chip);
  ...
  pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
    if(vadc_ptr != NULL)
    {
        err_flag |= pm_vadc_register_isr(bcl_status.pmic_chip,
                                         PM_VADC_BTM_LOW1_THR,
                                 &pmapp_vadc_btm_interrupt_cb);

        err_flag |= pm_vadc_register_isr(bcl_status.pmic_chip,
                                         PM_VADC_BTM_HI1_THR,
                                 &pmapp_vadc_btm_interrupt_cb);

        bcl_status.is_vadc_periph = TRUE;

    }else if (bcl_comp_ptr != NULL)
  {
    ...
    bcl_status.is_vadc_periph = FALSE;
  }
  else
  {
    err_flag |= PM_ERR_FLAG_FEATURE_NOT_SUPPORTED;
  ...
  bcl_status.is_supported = (err_flag == PM_ERR_FLAG_SUCCESS)? TRUE : FALSE;
                                                  ...
                                                  boolean irq_en)
{
    pm_err_flag_type err_flag=PM_ERR_FLAG_SUCCESS;
    pm_vadc_data_type *vadc_ptr = pm_vadc_get_data(bcl_status.pmic_chip);
    pm_vadc_btm_thr_type thr_type=(pm_vadc_btm_thr_type)pmapp_bcl_get_thr(thr);

    if((vadc_ptr == NULL) ||
       (thr_type>=PM_VADC_BTM_THR_INVALID) ||
       (cb_ptr == NULL) ||
       (voltage < vadc_ptr->limits->min_mv) ||
       (voltage > vadc_ptr->limits->max_mv))
    {
        err_flag |= PM_ERR_FLAG_INVALID_PARAMETER;
        PM_LOG_MSG_ERROR(err_flag, "error = %d", err_flag);
        return err_flag;
    ...
    //Disable IRQ in-case its enabled to avoid triggering after
    //IRQ clear
    err_flag |= pm_vadc_irq_enable(bcl_status.pmic_chip, thr_type, FALSE);

    err_flag |= pm_vadc_set_thr_voltage(bcl_status.pmic_chip,
                                        thr_type, voltage);

    err_flag |= pm_vadc_irq_clear(bcl_status.pmic_chip, thr_type);
    ...
    err_flag |= pm_vadc_irq_enable(bcl_status.pmic_chip, thr_type, irq_en);
...
void* pmapp_vadc_btm_interrupt_cb(void *pArg, uint32 uIntrStatusMask)
{
    ...
    {
        case 0x02:
            pm_vadc_irq_enable(bcl_status.pmic_chip,
                               PM_VADC_BTM_LOW1_THR,
                               FALSE);
            bcl_status.low1_thr_cb(PMAPP_BCL_LOW1_THR, NULL, NULL, NULL);
            break;
    ...
    PM_LOG_MSG_INFO("interrupt received: mask = %d", uIntrStatusMask);
    return NULL;
}
