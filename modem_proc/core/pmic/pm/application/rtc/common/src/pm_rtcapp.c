...
$Header: //components/rel/core.mpss/10.0/pmic/pm/application/rtc/common/src/pm_rtcapp.c#1 $ 
...
pm_err_flag_type pm_get_rtc_display_mode(pm_rtc_display_type *mode_ptr)
{
    ...
    PM_LOG_MSG_ERROR(error, "error=%d", error);
    return error; 
}
        ...
        switch (cmd)
        {
            case PM_RTC_SET_CMD:
                ...
                else 
                {
                    ...
                    // start RTC
                    error = pm_rtc_start(0, &time_hal_format);
                }
                break;
...
pm_err_flag_type pmapp_rtc_set_time_adjust(uint8 time_adjust)
{
    pm_err_flag_type error = PM_ERR_FLAG__SUCCESS;
   ...
   else
   {
       ...
       error = pm_rtc_set_time_adjust(0, time_adjust_hal_format);
   }
...
pm_err_flag_type pmapp_rtc_get_time_adjust(uint8 *time_adjust_ptr)
{
    pm_err_flag_type error = PM_ERR_FLAG__SUCCESS;
    uint8               time_adjust_hal_format = 0;
    
    error = pm_rtc_get_time_adjust(0, &time_adjust_hal_format);
