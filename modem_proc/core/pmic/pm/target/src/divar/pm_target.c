...
$Header: //components/rel/core.mpss/10.0/pmic/pm/target/src/divar/pm_target.c#2 $ 
...
extern boolean  bPMICStub;
static char *pmic_props = "/modem/divar";
...
pm_target_pre_init(void)
{
	...
	pm_err_flag_type err_flag = PM_ERR_FLAG__SUCCESS;
	uint8 pmic_index = PM_MAX_NUM_PMICS;

	// Get Seco Jr PMIC Index
	if(PM_ERR_FLAG_SUCCESS == pm_is_pmic_supported(PMIC_IS_PMG1110, &pmic_index))
    {
	  /* we wish to uninitilize the Seco Jr as this branch does not supported
	   single Slave ID and there is no use case for it in modem Sw */
 
		err_flag |= pm_uninitialize_version_info(pmic_index);		
    }
	

    if(err_flag != PM_ERR_FLAG_SUCCESS)
    {
      PM_LOG_MSG_ERR_FATAL ("Sending wrong PMIC index for uninitilization");
    }	
