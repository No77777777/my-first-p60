...
$Header: //components/rel/core.mpss/10.0/pmic/pm/utils/diag/pm_diag_peek_poke.c#1 $
...
)
{
  ...
  if (pm_peek_poke_diag.peek_poke_type == PM_DIAG_CMD_POKE)
  {
    ...
    {
      err_flag = pm_comm_write_byte_array(slave_id, pm_peek_poke_diag.reg_addr, 
                                              1, &pm_peek_poke_diag.reg_data, 0);
    }
...
static void* pm_diag_peek_poke_log_start()
{
  ...
  if (pm_peek_poke_diag.req_start_ptr == NULL)
  {
      return pm_diag_peek_poke_error_response(PM_DIAG_LOGGING_START);
  }
