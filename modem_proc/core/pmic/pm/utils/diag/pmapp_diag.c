...
/*===========================================================================
  ...
  $Header: //components/rel/core.mpss/10.0/pmic/pm/utils/diag/pmapp_diag.c#1 $
...
{   
    ...
    if( req_ptr->device < num_pmic_devices && req_ptr->sid <= 1 && rsp_ptr != NULL )
        ...
        pm_get_slave_id(req_ptr->device, req_ptr->sid , &slave_id); 
            ...
            {
            if( pm_comm_read_byte_array(slave_id, req_ptr->reg, 1, &val, 0) == PM_ERR_FLAG__SUCCESS )
                {
                    rsp_ptr->val = val;
            ...
            {
            if( pm_comm_write_byte_array(slave_id, req_ptr->reg, 1, &req_ptr->val, 0) == PM_ERR_FLAG__SUCCESS )
                {
                    rsp_ptr->peek_poke = 0x0;
                }
                else
