...
$Header: //components/rel/core.mpss/10.0/pmic/pm/utils/diag/pm_diag_pwr_rails.c#1 $

when       who     what, where, why
--------   ---     -------------------------------------------------
04/10/19   kcs     Geting the pheripherals info from driver data insted of target data. 
...
/* =========================================================================
**  Function : pm_diag_pwr_rails_convert_mode
** =========================================================================*/
/**
  Converts pmic mode to diag command recognizable mode.

  @param   *diag_mode       [out]  - Diag recognizable mode value
  @param   mode             [in]   - Mode read from pmic.
...
*/
static void 
pm_diag_pwr_rails_convert_mode(pm_sw_mode_type mode, uint8 *diag_mode)
{
    switch(mode)
    {
        case PM_SW_MODE_LPM:
            *diag_mode = 0;
            break;
        case PM_SW_MODE_BYPASS:
            *diag_mode = 1;
            break;
        case PM_SW_MODE_AUTO:
            *diag_mode = 2;
            break;
        case PM_SW_MODE_RETENTION:
            *diag_mode = 3;
            break;
        case PM_SW_MODE_NPM:
            *diag_mode = 4;
...
)
{
  ...
  num_peripherals = pm_smps_get_num_peripherals(pmic_index);
  ...
  for(index = 0;index < num_peripherals;index++)
  {
    ...
    /*
     * Convert mode to diag readable mode.
     */
    pm_diag_pwr_rails_convert_mode(mode, &diag_mode);
    ...
    smps_log.mode_type = (uint8)diag_mode;
...
)
{
  ...
  num_peripherals = pm_ldo_get_num_peripherals(pmic_index);
...
{
  pm_sw_mode_type                   mode = PM_SW_MODE_INVALID;
  pm_sw_mode_type                   diag_mode = 0xff;
  boolean                           vreg_ok_status = FALSE;
  pm_on_off_type                    sw_enable_status = PM_INVALID;
  pm_model_type                     pmic_type = PMIC_IS_INVALID;
  pm_volt_level_type                volt = 0;
  ...
  num_peripherals = pm_vs_get_num_peripherals(pmic_index);
  ...
  for(index = 0;index < num_peripherals;index++)
  {
    ...
    vs_log.mode_type = (uint8)diag_mode;
...
static DALResult pm_diag_pwr_rails_init_logging(void)
{
  ...
  for (pmic_index = 0;pmic_index < pm_pwr_rails_diag.num_pmic_devices;pmic_index++)
  {
        ...
        num_of_smps = pm_smps_get_num_peripherals(pmic_index);
        num_of_ldo  = pm_ldo_get_num_peripherals(pmic_index);
        num_of_vs   = pm_vs_get_num_peripherals(pmic_index);
     
        pm_pwr_rails_diag.num_pwr_rails = 
        pm_pwr_rails_diag.num_pwr_rails + num_of_smps + num_of_ldo + num_of_vs;
  }
