...
$Header: //components/rel/core.mpss/10.0/pmic/pm/utils/diag/pm_diag_gpios.c#1 $
...
)
{
  ...
  for(gpio_index = 0;gpio_index < num_gpio;gpio_index++)
  {
    ...
    err_flag = pm_comm_read_byte_array(gpio_data_ptr->comm_ptr->slave_id, status_reg, 1, &status_reg_data, 0 );
    ...
    err_flag = pm_gpio_status_get(pmic_index, (pm_gpio_perph_index)gpio_index, &gpio_status);
