...
$Header: //components/rel/core.mpss/10.0/pmic/pm/npa/src/pm_npa_device_vs.c#1 $
...
npa_resource_state pm_npa_vs_kvps_update_fcn(npa_resource *resource, npa_client_handle client)
{
  ...
  if (req_type == NPA_CLIENT_VECTOR)
  {
    ...
    PM_LOG_MSG_DEBUG("ResName=%s ClientName=%s ActiveSet: enable = %d, uV = %d", 
            client->resource_name, client->name, kvps->sw_enable_value, kvps->uv_value);
            
    //Print final aggregated required set values   
     PM_LOG_MSG_DEBUG("ResName=%s ClientName=%s RequiredSet: enable = %d, uV = %d", 
            client->resource_name, client->name, reqd_kvps->sw_enable_value, reqd_kvps->uv_value);
