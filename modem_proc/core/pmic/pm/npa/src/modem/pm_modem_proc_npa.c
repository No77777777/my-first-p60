...
$Header: //components/rel/core.mpss/10.0/pmic/pm/npa/src/modem/pm_modem_proc_npa.c#1 $ 
...
pm_modem_proc_npa_init (void)
{
    ...
    pm_npa_resource_publish_type       *publishers = NULL;
    ...
    // Get the PAM info
    nodeRscArray = (pm_npa_node_resource_info*)pm_pam_data->pm_npa_pam_rsrcs; 
    node_num_ptr = (uint32*)pm_pam_data->num_of_pam_rsrcs; 
   ...
   publishers = (pm_npa_resource_publish_type*)pm_pam_data->pm_npa_pub_pam_rsrcs;

   if((publishers != NULL) &&
     (publishers->client_list != NULL) &&
     (publishers->num_clients > 0))
   {
      npa_remote_publish_resources(publishers->domain, publishers->num_clients, (const char**)publishers->client_list);
   }
