...
$Header: //components/rel/core.mpss/10.0/pmic/pm/npa/src/modem/pm_modem_proc_npa_device.c#2 $ 
...
FUNCTION pm_modem_proc_npa_register_resources
...
pm_modem_proc_npa_register_resources (void)
{
  ...
  pm_npa_pam_data_type *pm_pam_data = pm_target_info_pam_data();

  if (pm_pam_data == NULL)
  {
      PM_LOG_MSG_ERR_FATAL("Unable to find pm_pam_data");
  }

  /*
   * Get the PMIC remote node resources for LDO
   */
  pPmRemoteRsrc = (pm_npa_remote_resource_type*)pm_pam_data->pmic_npa_remote_ldo;
 ...
 /* Initialize all Remote Resources - LDO */
   ...
   */
  pPmRemoteRsrc = (pm_npa_remote_resource_type*)pm_pam_data->pmic_npa_remote_smps;
 ...
 /* Initialize all Remote Resources - CLK */
   ...
   */
  pPmRemoteRsrc = (pm_npa_remote_resource_type*)pm_pam_data->pmic_npa_remote_clk;
   ...
   */
  pPmRemoteRsrc = (pm_npa_remote_resource_type*)pm_pam_data->pmic_npa_remote_vs;
