...
$Header: //components/rel/core.mpss/10.0/pmic/pm/npa/src/pm_npa_device_clk_buff.c#1 $
...
pm_npa_clk_buff_kvps_update_fcn(npa_resource *resource, npa_client_handle client)
{
  ...
  if (req_type == NPA_CLIENT_VECTOR)
  {
    ...
    pm_sw_event(PMIC_CLK_BUF_AGG1, 3, 0, kvps->sw_enable_value); 
    ...
    if (req_type == NPA_CLIENT_VECTOR && active_client->type == NPA_CLIENT_VECTOR)
    {
      ...
      pm_sw_event(PMIC_CLK_BUF_AGG2, 3, 0, kvps->sw_enable_value);
    }

        active_client = active_client->next;
    }
    
   PM_LOG_MSG_DEBUG("ResName=%s ClientName=%s ActiveSet: enable = %d", 
                   client->resource_name, client->name, kvps->sw_enable_value);
    
   PM_LOG_MSG_DEBUG("ResName=%s ClientName=%s RequiredSet: enable = %d", 
                   client->resource_name, client->name, reqd_kvps->sw_enable_value);
