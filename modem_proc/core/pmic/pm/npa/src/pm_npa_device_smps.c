...
$Header: //components/rel/core.mpss/10.0/pmic/pm/npa/src/pm_npa_device_smps.c#1 $
...
npa_resource_state pm_npa_smps_kvps_update_fcn(npa_resource *resource, npa_client_handle client)
{
  ...
  PM_LOG_MSG_DEBUG("ResName=%s ClientName=%s ActiveSet: enable = %d, mode = %d, uV = %d", 
            client->resource_name, client->name, kvps->sw_enable_value, kvps->smps_sw_mode_value, 
            kvps->uv_value);
            
  //Print final aggregated Active set values
  PM_LOG_MSG_DEBUG("ResName=%s ClientName=%s RequiredSet: enable = %d, mode = %d, uV = %d", 
            client->resource_name, client->name, reqd_kvps->sw_enable_value, reqd_kvps->smps_sw_mode_value, 
            reqd_kvps->uv_value);
