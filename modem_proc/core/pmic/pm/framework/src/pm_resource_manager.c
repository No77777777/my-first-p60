...
$Header: //components/rel/core.mpss/10.0/pmic/pm/framework/src/pm_resource_manager.c#2 $
 ...
 */
static void
pm_resource_manager_create_pmic_resource(pm_comm_info_type *comm_ptr,
     ...
     //every  pmic chip has one or more slaves
      for(sid_index = 0; sid_index < num_slaves_per_pmic ; sid_index++)
      {
        ...
        for(peripheral_index = 0; peripheral_index < 256; peripheral_index++)
        {
          ...
          if(pm_target_info_read_periph_rev(comm_ptr, &peripheral_info) == PM_ERR_FLAG_SUCCESS)
          {
                                         ...
                                         uint8 pmic_index)
{
  switch((pm_hw_module_type)peripheral_info->peripheral_type)
  {
    ...
    case PM_HW_MODULE_ADC:
      if(peripheral_info->peripheral_subtype == PM_HW_MODULE_VADC_HM9_BTM_2)
      {
         pm_vadc_driver_init( comm_ptr, peripheral_info, pmic_index );
      }
      break;
