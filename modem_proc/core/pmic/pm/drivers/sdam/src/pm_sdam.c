...
$Header: //components/rel/core.mpss/10.0/pmic/pm/drivers/sdam/src/pm_sdam.c#1 $
...
pm_err_flag_type
pm_sdam_trig_seq ( uint8 pmic_chip, uint8 trig_arg, uint8 *trig_ret_val,
                   pm_sdam_trig_type trig_fnf_wait )
{
  pm_err_flag_type    err_flag   = PM_ERR_FLAG_SUCCESS;
  pm_sdam_data_type   *sdam_ptr  = pm_sdam_get_data(pmic_chip);
  uint8               sdam_index = 0;
  ...
  sdam_index = sdam_ptr->sdam_supported_periph->sdam_periph;
  ...
  /* Trigger new seq */
  err_flag |= pm_comm_write_byte(SLAVE_SDAM, REG_SDAM(trig_arg), trig_arg, 0);

  err_flag |= pm_comm_write_byte(SLAVE_SDAM, REG_SDAM(trig_busy_ret),
                                 (uint8)BUSY_INDICATOR, 0);

  err_flag |= pm_comm_write_byte(SLAVE_SDAM, REG_SDAM(trig_clr), 0x1, 0);

  err_flag |= pm_comm_write_byte(SLAVE_SDAM, REG_SDAM(trig_set), 0x1, 0);
...
pm_sdam_seq_done( pm_sdam_data_type *sdam_ptr, pm_register_address_type addr )
{
  ...
  do
  {
    err_flag |= pm_comm_read_byte(SLAVE_SDAM, addr, &data, 0);
...
pm_err_flag_type
pm_sdam_mem_write ( uint8 pmic_chip, uint8 start_addr,
                    uint32 num_bytes, uint8 *data_ptr )
{
  pm_err_flag_type    err_flag   = PM_ERR_FLAG_SUCCESS;
  pm_sdam_data_type   *sdam_ptr  = pm_sdam_get_data(pmic_chip);
  uint8               sdam_index = 0;
  ...
  /* Input Check */
  if(sdam_ptr == NULL || data_ptr == NULL || num_bytes == 0)
  {
    err_flag |= (sdam_ptr==NULL) ?
                PM_ERR_FLAG_FEATURE_NOT_SUPPORTED :
                PM_ERR_FLAG_INVALID_PARAMETER;
      ...
      ((num_bytes + (uint32)start_addr) > sdam_mem_size) )
  {
    ...
    PM_LOG_MSG_ERROR(err_flag, "addr offset = 0x%x, numbytes = %d",
...
pm_err_flag_type
pm_sdam_mem_read ( uint8 pmic_chip, uint8 start_addr,
                   uint32 num_bytes, uint8 *data_ptr )
{
  pm_err_flag_type    err_flag   = PM_ERR_FLAG_SUCCESS;
  pm_sdam_data_type   *sdam_ptr  = pm_sdam_get_data(pmic_chip);
