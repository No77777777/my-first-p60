...
$Header: //components/rel/core.mpss/10.0/pmic/pm/drivers/gpio/src/pm_gpio.c#1 $ 
...
pm_err_flag_type pm_gpio_config_bias_voltage(uint8 pmic_chip, pm_gpio_perph_index gpio, pm_gpio_volt_src_type voltage_source)
{
    ...
    errFlag = pm_gpio_set_voltage_source(pmic_chip, (uint8)gpio, voltage_source);
    if(errFlag != PM_ERR_FLAG__SUCCESS) return errFlag;

    errFlag = pm_gpio_set_output_buffer_configuration(pmic_chip, (uint8)gpio, PM_GPIO_OUT_BUFFER_CONFIG_CMOS);
    if(errFlag != PM_ERR_FLAG__SUCCESS) return errFlag;

    errFlag = pm_gpio_set_inversion_configuration(pmic_chip, (uint8)gpio, TRUE);
    if(errFlag != PM_ERR_FLAG__SUCCESS) return errFlag;

    errFlag = pm_gpio_set_output_buffer_drive_strength(pmic_chip, (uint8)gpio, PM_GPIO_OUT_BUFFER_HIGH);
    if(errFlag != PM_ERR_FLAG__SUCCESS) return errFlag;
    ...
    errFlag = pm_gpio_set_source_configuration(pmic_chip, (uint8)gpio, PM_GPIO_SOURCE_GND);
    if(errFlag != PM_ERR_FLAG__SUCCESS) return errFlag;
                                              ...
                                              pm_gpio_src_config_type source)
{
    ...
    pm_register_address_type dig_in_ctl = gpio_ptr->gpio_register->base_address + (pm_register_address_type)(gpio_ptr->gpio_register->peripheral_offset*(uint8)gpio) 
    ...
    errFlag = pm_gpio_set_current_source_pulls(pmic_chip, (uint8)gpio, i_source_pulls);
    if(errFlag != PM_ERR_FLAG__SUCCESS) return errFlag ;

    errFlag = pm_gpio_set_output_buffer_drive_strength(pmic_chip, (uint8)gpio, out_buffer_strength) ;
    if(errFlag != PM_ERR_FLAG__SUCCESS) return errFlag ;
    ...
    //no inversion
    errFlag = pm_gpio_set_inversion_configuration(pmic_chip, (uint8)gpio, FALSE);
     if(errFlag != PM_ERR_FLAG__SUCCESS) return errFlag;
                                                   ...
                                                   boolean  out_inversion)
{
    ...
    errFlag = pm_gpio_set_inversion_configuration(pmic_chip, (uint8)gpio, out_inversion);
    if(errFlag != PM_ERR_FLAG__SUCCESS) return errFlag;

    errFlag = pm_gpio_set_output_buffer_configuration(pmic_chip, (uint8)gpio, out_buffer_config);
    if(errFlag != PM_ERR_FLAG__SUCCESS) return errFlag;

    errFlag = pm_gpio_set_output_buffer_drive_strength(pmic_chip, (uint8)gpio, out_buffer_strength);
    if(errFlag != PM_ERR_FLAG__SUCCESS) return errFlag;

    errFlag = pm_gpio_set_source_configuration(pmic_chip, (uint8)gpio, source);
    if(errFlag != PM_ERR_FLAG__SUCCESS) return errFlag;
...
pm_err_flag_type pm_gpio_set_inversion_configuration(uint8 pmic_chip, pm_gpio_perph_index gpio, boolean  inversion)
{
    ...
    {
         pm_register_address_type dig_out_src_ctl = gpio_ptr->gpio_register->base_address + (pm_register_address_type)(gpio_ptr->gpio_register->peripheral_offset*(uint8)gpio) +
...
static pm_err_flag_type pm_gpio_status_get_v2(uint8 pmic_chip, pm_gpio_perph_index gpio , pm_gpio_status_type *gpio_status)
{
    ...
    pm_register_address_type dig_out_ctl =  base_address + gpio_ptr->gpio_register->dig_out_ctl;
    pm_register_address_type en_ctl =  base_address + gpio_ptr->gpio_register->en_ctl;
    pm_register_address_type dig_out_src_ctl = base_address + gpio_ptr->gpio_register->dig_out_src_ctl;
...
pm_err_flag_type pm_gpio_peripheral_info_get(uint8 pmic_chip, pm_gpio_perph_index gpio, peripheral_info_type *peripheralInfo)
{
    ...
    errFlag = pm_target_info_read_periph_rev(gpio_ptr->comm_ptr, peripheralInfo);
