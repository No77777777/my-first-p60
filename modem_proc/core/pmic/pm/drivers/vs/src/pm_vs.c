...
$Header: //components/rel/core.mpss/10.0/pmic/pm/drivers/vs/src/pm_vs.c#1 $ 
...
pm_err_flag_type pm_vs_vreg_ok_status(uint8 pmic_chip, uint8 perph_index, boolean *on_off)
{
    ...
    { 
        err_flag = pm_pwr_is_vreg_ready_alg(&(vs_ptr->pm_pwr_data), vs_ptr->comm_ptr, perph_index, on_off);
    }
    return err_flag;
}
