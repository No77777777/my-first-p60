...
$Header: //components/rel/core.mpss/10.0/pmic/pm/drivers/clk/src/pm_clk_driver.c#1 $
...
{   
    0x5000, 0x100, 0x044, 0x046, 0x048, 0x043, 0x008, 0x045
};

static pm_clk_sleep_register_info_type clk_sleep_reg = 
{   //SLPSRC_CTL1
    0x049 
};
  ...
  if ((peripheral_info->peripheral_subtype == PM_HW_MODULE_CLOCK_CALRC2) || 
      ...
      (peripheral_info->peripheral_subtype == PM_HW_MODULE_CLOCK_LFRC))
   {
    ...
    if (clk_ptr != NULL)
    {
        ...
        if((peripheral_info->peripheral_subtype == PM_HW_MODULE_CLOCK_RF_CLK) && (peripheral_info->digital_major_version >= 0x4))
        {
           clk_ptr->clk_hold_supported[clk_index] = TRUE;
        }
    }
}
