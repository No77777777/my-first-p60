...
$Header: //components/rel/core.mpss/10.0/pmic/pm/drivers/smps/src/pm_smps.c#1 $ 
...
pm_err_flag_type 
pm_smps_quiet_mode_mask(uint8 pmic_chip, uint8 periph_index, pm_quiet_mode_follow_type mode)
{
    pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
    pm_smps_data_type *smps_ptr = pm_smps_get_data(pmic_chip);
    //uint8 periph_type = 0;
    ...
    else if (mode >= PM_QUIET_MODE_FOLLOW__INVALID)
    {
        err_flag = PM_ERR_FLAG_INVALID_PARAMETER;
    }
    else
...
pm_err_flag_type pm_smps_npm_status(uint8 pmic_chip, uint8 periph_index, boolean *npm_status)
{
    pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
    pm_smps_data_type *smps_ptr = pm_smps_get_data(pmic_chip);
    ...
    {
        pm_register_address_type    base_addr = 0;
        pm_register_address_type    smps_status_reg = 0;
        pm_pwr_data_type            *pwr_data = &(smps_ptr->pm_pwr_data);
        pm_register_data_type       data = 0;
        ...
        smps_status_reg = (pm_register_address_type)(base_addr + pwr_data->pwr_reg_table->STATUS);
        err_flag = pm_comm_read_byte_mask(smps_ptr->comm_ptr->slave_id, smps_status_reg, 0xFF, &data, 0 ); 
