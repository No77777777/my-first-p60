...
$Header: //components/rel/core.mpss/10.0/mproc/glink/core/inc/glink_internal.h#2 $
...
typedef enum {
  ...
  GLINK_EVENT_CH_WAIT_PREV_INTENT_ACK
...
typedef struct _glink_channel_intents_type {
  ...
  * are available */
  ...
  /* Flag to indicate remote intent is requested */
  boolean                       rm_intent_req_sent;
  
  /* Size of requested intent that this channel wait on */
  size_t                        rm_intent_req_size;

  /* This is a debug flag indicates current wait is for remote ack sent previously */
  boolean                       dbg_prev_rm_intent_ack_wait;
