/*===========================================================================
  ...
  ---------------------------------------------------------------------------
  Copyright (c) 2012-2016, 2018, 2019-2021 Qualcomm Technologies, Inc.  All Rights Reserved.
...
$Header: //components/rel/core.mpss/10.0/mproc/ipc_router/src/ipc_router_qdi_driver.c#3 $
$DateTime: 2021/09/30 12:22:40 $
$Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
10/09/17   ss      Avoiding sending rx buf to user pd if client is closed
10/09/17   ss      Get the Poll FDs ref outside of the waiterlock
...
===========================================================================*/
typedef struct ipc_router_qdi_rx_buf_s ipc_router_qdi_rx_buf_t;
...
typedef enum {
  RX_BUF_TYPE_EVENT = 0,
  RX_BUF_TYPE_DATA  = 1,
} ipc_router_qdi_rx_buf_type;

struct ipc_router_qdi_rx_buf_s {
  ipc_router_qdi_rx_buf_type type;
  union {
    struct event_buf_s event;
    struct data_buf_s  data;
  } info;
  LINK(ipc_router_qdi_rx_buf_t, link);
};
...
struct ipc_router_qdi_handle_s 
{
  ...
  LIST(ipc_router_qdi_rx_buf_t, rx_q);
...
LIST(ipc_router_qdi_handle_t, g_active_handles);

LIST(ipc_router_qdi_waiter_t, g_waiter_list);
static ipc_router_os_sem waiter_list_lock;
static unsigned int waiter_next_uid = 1;

LIST(ipc_router_qdi_name_t, g_name_list);
...
static void cleanup_client_queues(ipc_router_user_ep_t *handle)
{
  ...
  ipc_router_address_type addr;
  ipc_router_qdi_rx_buf_t *rx_buf;
  ...
  /* Remove all data packets from RX queue */
  ipc_router_os_sem_lock(&dev->rx_q_lock);
  rx_buf = LIST_HEAD(dev->rx_q);
  while(rx_buf)
  {
    ipc_router_qdi_rx_buf_t *to_free = rx_buf;
    rx_buf = rx_buf->link.next;
    if(to_free->type == RX_BUF_TYPE_DATA)
    {
      struct data_buf_s *data_to_free = &to_free->info.data;
      if (data_to_free->user->fd == handle->fd)
      {
        LIST_REMOVE(dev->rx_q, to_free, link);
        ipc_router_packet_free(&data_to_free->pkt);
        ipc_router_os_free(to_free);
      }
    }
  }
...
{
  ipc_router_qdi_rx_buf_t *rx_buf = ipc_router_os_calloc(1, sizeof(*rx_buf));
  struct data_buf_s *data_buf;
  if(!rx_buf)
  {
    /* TODO Log error */
    if(pkt)
  ...
  LINK_INIT(rx_buf->link);
  rx_buf->type = RX_BUF_TYPE_DATA;
  data_buf = &rx_buf->info.data;
  data_buf->user = user;
  if(src_addr)
  {
    data_buf->src_addr = *src_addr;
  }
  else
  {
    data_buf->src_addr.processor_id = data_buf->src_addr.port_id = 0xffffffff;
  }
  data_buf->confirm_rx = (uint32)confirm_rx;
  data_buf->len = len;
  data_buf->pkt = pkt;

  ipc_router_os_sem_lock(&user->dev->rx_q_lock);
  LIST_ADD(user->dev->rx_q, rx_buf, link);
    ...
    ipc_router_event_info_type *info)
{
  ...
  if(!ctrl_port)
  {
    ...
    LINK_INIT(rx_buf->link);
    rx_buf->type = RX_BUF_TYPE_EVENT;
    event_buf = &rx_buf->info.event;
    event_buf->event = (uint32)event;
    event_buf->cookie= cookie;
    event_buf->info  = *info;

    /* Queue up the event. */
    ipc_router_os_sem_lock(&handle->rx_q_lock);
    LIST_ADD(handle->rx_q, rx_buf, link);
...
{
  ipc_router_qdi_rx_buf_t *rx_buf = NULL;
  struct event_buf_s *event_buf;
  int rc;
  ...
  ipc_router_os_sem_lock(&handle->rx_q_lock);
  LIST_FIND(handle->rx_q, rx_buf, link, rx_buf->type == RX_BUF_TYPE_EVENT);
  if(!rx_buf)
  {
    ipc_router_os_sem_unlock(&handle->rx_q_lock);
    return 0;
  ...
  rc = qurt_qdi_copy_to_user(client_handle, out_event, &event_buf->event, sizeof(uint32));
  if(rc)
  {
  ...
  rc = qurt_qdi_copy_to_user(client_handle, out_info, &event_buf->info, sizeof(ipc_router_event_info_type));
  if(rc)
  {
    rc = IPC_ROUTER_QDI_PERM_ERR;
    goto bail;
  ...
  rc = qurt_qdi_copy_to_user(client_handle, out_cookie, &event_buf->cookie, sizeof(void *));
  if(rc)
  {
    rc = IPC_ROUTER_QDI_PERM_ERR;
    goto bail;
  ...
  LIST_REMOVE(handle->rx_q, rx_buf, link);
  ipc_router_os_free(rx_buf);
...
{
  ipc_router_qdi_rx_buf_t *rx_buf = NULL;
  struct data_buf_s       *data_buf = NULL;
  int rc;
...
RETRY:
  ...
  if(!(flags & IPC_ROUTER_QDI_RDFLAG_DONTWAIT))
  {
    ...
    while(1)
    {
      ...
      ipc_router_os_sem_lock(&handle->rx_q_lock);
      LIST_FIND(handle->rx_q, rx_buf, link, rx_buf->type == RX_BUF_TYPE_DATA);
      ipc_router_os_sem_unlock(&handle->rx_q_lock);
      if(rx_buf || handle->active_receivers <= 0 || 
      ...
      if(rc == QURT_ECANCEL)
      {
        wait_rc = IPC_ROUTER_QDI_TERM_ERR;
        break;
      }
#else
      ...
      {
        wait_rc = IPC_ROUTER_QDI_TIMEOUT_ERR;
        break;
      }
    }
  ...
  rx_buf = LIST_HEAD(handle->rx_q);
  while(rx_buf)
  {
    ipc_router_qdi_rx_buf_t *check_rx_buf = rx_buf;
    ipc_router_user_ep_t    *rx_user = check_rx_buf->info.data.user;
    uint32                  need_flag = rx_user->ctrl_port ? EVENT_ALLOWED : DATA_ALLOWED;
    
    rx_buf = rx_buf->link.next;
    
    if ((check_rx_buf->type == RX_BUF_TYPE_DATA) &&
        !(rx_user->allowed_flag & need_flag))
    {
      LIST_REMOVE(handle->rx_q, check_rx_buf, link);
      ipc_router_packet_free(&check_rx_buf->info.data.pkt);
      ipc_router_os_free(check_rx_buf);
    }
  }
  ...
  {
    *out_len = data_buf->len;
    rc = IPC_ROUTER_STATUS_SUCCESS;
    goto bail;
  }

  if(data_buf->len > in_max_len && 0 == (flags & IPC_ROUTER_QDI_RDFLAG_TRUNC))
  {
  ...
  if(out_addr && qurt_qdi_copy_to_user(client_handle, out_addr, &data_buf->src_addr, sizeof(ipc_router_address_type)) != 0)
  {
    rc = IPC_ROUTER_QDI_PERM_ERR;
    goto bail;
  }

  if(cookie && qurt_qdi_copy_to_user(client_handle, cookie, &data_buf->user->cookie, sizeof(void *)) != 0)
  {
    rc = IPC_ROUTER_QDI_PERM_ERR;
    goto bail;
  }
  ...
  if(out_confirm_rx)
  {
    ...
    rc = qurt_qdi_copy_to_user(client_handle, out_confirm_rx, &data_buf->confirm_rx, sizeof(uint32));
    if(rc)
    {
      rc = IPC_ROUTER_QDI_PERM_ERR;
      goto bail;
  ...
  else if(data_buf->confirm_rx)
  {
    /* If user chooses not to be the one to send confirmation (Does not receive conf_rx),
     * we should send it on their behalf on read */
    ipc_router_core_send_conf(data_buf->user->handle, &data_buf->src_addr);
  }

  *out_len = ipc_router_packet_read(&data_buf->pkt, return_buf, data_buf->len > in_max_len ? in_max_len : data_buf->len);
  ...
  LIST_REMOVE(handle->rx_q, rx_buf, link);
  ipc_router_packet_free(&data_buf->pkt);
  ipc_router_os_free(rx_buf);
  rc = IPC_ROUTER_STATUS_SUCCESS;
...
/*===========================================================================
FUNCTION      ipc_router_qdi_rx_invocation

DESCRIPTION   Returns a buffer or event sent to this endpoint.

ARGUMENTS     client_handle - Handle identifying the client
              handle - The handle for this device.
              out_is_event - Flag to indicate RX packet is a event or data.
              event_info - RX event info is filled into this buffer.
              data_info - RX data info is filled into this buffer,
              out_cookie - the cookie associated with this endpoint
                       (Provided by the client on  open)
              flags - Read flags (See IPC_ROUTER_QDI_RDFLAG_* for more)

This function will never block if there are event or data buffers.

RETURN VALUE  0 on success, error code on failure.

SIDE EFFECTS  None
===========================================================================*/
static inline int ipc_router_qdi_rx_invocation
(
    int client_handle,
    ipc_router_qdi_handle_t *handle,
    uint32 *out_is_event,
    ipc_router_qdi_event_info_type *event_info,
    ipc_router_qdi_data_info_type  *data_info,
    void *out_cookie,
    uint32 flags
)
{
  ...
  if(!handle || !out_is_event || !event_info || !data_info || !out_cookie)
  {
    return IPC_ROUTER_QDI_PARAM_ERR;
  }
  ...
  is_event = (rx_buf->type == RX_BUF_TYPE_EVENT);

  rc = qurt_qdi_copy_to_user(client_handle, out_is_event, &is_event, sizeof(uint32));
  if(rc)
  {
    rc = IPC_ROUTER_QDI_PERM_ERR;
    goto bail;
  ...
  if (is_event)
  {
    struct event_buf_s *event_buf = &rx_buf->info.event;

    rc = qurt_qdi_lock_buffer(client_handle, event_info, sizeof(ipc_router_qdi_event_info_type), QDI_PERM_RW, (void **)&event_info);
    if(rc || !event_info)
    {
      rc = IPC_ROUTER_QDI_PERM_ERR;
      goto bail;
    ...
    event_info->event = event_buf->event;
    ipc_router_os_mem_copy(&event_info->info, sizeof(ipc_router_event_info_type), 
                           &event_buf->info, sizeof(ipc_router_event_info_type));
    ...
    LIST_REMOVE(handle->rx_q, rx_buf, link);
    ipc_router_os_free(rx_buf);
    rc = sizeof(ipc_router_qdi_event_info_type);
  }
  else
  {
    struct data_buf_s *data_buf = &rx_buf->info.data;
    void *return_buf = NULL;

    rc = qurt_qdi_lock_buffer(client_handle, data_info, sizeof(ipc_router_qdi_data_info_type), QDI_PERM_RW, (void **)&data_info);
    if(rc || !data_info)
    {
      rc = IPC_ROUTER_QDI_PERM_ERR;
      goto bail;
    ...
    /* check if the client related to buffer is allowed to receive data or not */
    if (!(data_buf->user->allowed_flag & DATA_ALLOWED))
    {
      data_info->len = 0;
      LIST_REMOVE(handle->rx_q, rx_buf, link);
      ipc_router_packet_free(&data_buf->pkt);
      ipc_router_os_free(rx_buf);
      rc = sizeof(ipc_router_qdi_data_info_type);
      goto bail;
    }
    ...
    if(flags & IPC_ROUTER_QDI_RDFLAG_MSGPEEK)
    {
      data_info->len = data_buf->len;
      rc = sizeof(ipc_router_qdi_data_info_type);
      goto bail;
    }
    
    if(data_buf->len > data_info->buf_max_len && 0 == (flags & IPC_ROUTER_QDI_RDFLAG_TRUNC))
    {
      rc = IPC_ROUTER_QDI_NO_SPACE_ERR;
      goto bail;
    }
    
    if(data_info->buf_max_len > 0 && data_info->buf != NULL)
    {
      rc = qurt_qdi_lock_buffer(client_handle, data_info->buf, data_info->buf_max_len, QDI_PERM_W, (void **)&return_buf);
      if(rc || !return_buf)
      {
        rc = IPC_ROUTER_QDI_PERM_ERR;
        goto bail;
    ...
    rc = qurt_qdi_copy_to_user(client_handle, out_cookie, &data_buf->user->cookie, sizeof(void *));
    if(rc)
    {
      rc = IPC_ROUTER_QDI_PERM_ERR;
      goto bail;
    ...
    data_info->confirm_rx = data_buf->confirm_rx;
    ipc_router_os_mem_copy(&data_info->src_addr, sizeof(ipc_router_address_type), 
                           &data_buf->src_addr, sizeof(ipc_router_address_type));
                           
    data_info->len = ipc_router_packet_read(&data_buf->pkt, return_buf, 
                          data_buf->len > data_info->buf_max_len ? data_info->buf_max_len : data_buf->len);
...
ARGUMENTS     client_handle - The calling client's handle
              handle - The device handle (Assumption is that this is also
                the first fd in pfd)
              poll_fds - Array of pollfd structures (See ipc_router_qdi_poll_fd)
...
{
  ipc_router_qdi_rx_buf_t *rx_buf, *rx_buf_to_free;
  ...
  while(rx_buf)
  {
    if (rx_buf->type == RX_BUF_TYPE_DATA)
    {
      ipc_router_packet_free(&rx_buf->info.data.pkt);
    }
    rx_buf_to_free = rx_buf;
    rx_buf = rx_buf->link.next;
    ipc_router_os_free(rx_buf_to_free);
  }
  ipc_router_os_sem_unlock(&handle->rx_q_lock);
...
)
{
  ...
  switch(method)
  {
    ...
    case IPC_ROUTER_QDI_RX:
      ret = ipc_router_qdi_rx_invocation(client_handle, handle, a1.ptr, a2.ptr, a3.ptr, a4.ptr, a5.num);
      break;
