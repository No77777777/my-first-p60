/*===========================================================================
  ...
  ---------------------------------------------------------------------------
  Copyright (c) 2007-2013,2015-2021 Qualcomm Technologies, Inc.  All Rights Reserved.
...
$Header: //components/rel/core.mpss/10.0/mproc/ipc_router/src/ipc_router_core.c#2 $
...
)
{
  ...
  ipc_router_client_type *src_client = NULL;
  ...
  /* Take reference to source client */
  src_client = ipc_router_find_client(&client->addr);
  if (!src_client)
  {
    return IPC_ROUTER_STATUS_INVALID_PARAM;
  }
  
  /* Check if caller has passed valid client */
  if (src_client != client)
  {
    ipc_router_unlock_client(src_client);
    return IPC_ROUTER_STATUS_INVALID_PARAM;
  }
