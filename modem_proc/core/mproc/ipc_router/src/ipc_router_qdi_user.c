...
$Header: //components/rel/core.mpss/10.0/mproc/ipc_router/src/ipc_router_qdi_user.c#2 $
$DateTime: 2019/11/19 06:35:38 $
$Author: pwbldsvc $
...
SIDE EFFECTS  None
===========================================================================*/
static void ipc_router_qdi_handle_event
(
...
)
{
  if(handle && handle->event_cb)
  {
    handle->event_cb((ipc_router_client_type *)handle, 
                        handle->event_cb_data, event_info->event, &event_info->info);
  }
  /* Now it is safe to free up the handle */
  if(event_info->event == IPC_ROUTER_EVENT_CLOSED)
  {
...
SIDE EFFECTS  None
===========================================================================*/
static void ipc_router_qdi_handle_data
(
...
)
{
  ...
  data_info->buf = ipc_router_os_malloc(data_info->len);
  if(!data_info->buf)
  {
    /* TODO: Log error */
    return;
  ...
  data_info->buf_max_len = data_info->len;
  
  rc = qurt_qdi_handle_invoke(ipc_router_qdi_device, IPC_ROUTER_QDI_RX, 
                &is_event, &event_info, data_info, &cookie, 0);
  if(rc <= 0)
  {
    ipc_router_os_free(data_info->buf);
    return;
  }
  ...
  {
    ipc_router_os_free(data_info->buf);
    ipc_router_qdi_handle_event(handle, &event_info);
    return;
  }
  
  if(handle && handle->rx_cb && data_info->len)
  {
    handle->rx_cb((ipc_router_client_type *)handle, handle->rx_cb_data, 
                   (ipc_router_packet_type *)data_info->buf, &data_info->src_addr, 
                   data_info->len, (boolean)data_info->confirm_rx);
...
/*===========================================================================
FUNCTION      ipc_router_qdi_handle_rx

DESCRIPTION   Handles a event and data signal
...
SIDE EFFECTS  None
===========================================================================*/
static void ipc_router_qdi_handle_rx(void)
{
  ipc_router_qdi_event_info_type event_info;
  ipc_router_qdi_data_info_type  data_info;
  ipc_router_qdi_handle *handle;
  ...
  while(1)
  {
    ...
    rc = qurt_qdi_handle_invoke(ipc_router_qdi_device, IPC_ROUTER_QDI_RX, 
                &is_event, &event_info, &data_info, &cookie,
    ...
    {
      ipc_router_qdi_handle_data(&data_info);
    }
  }
}
