...
$Header: //components/rel/core.mpss/10.0/power/npa/src/remote/npa_remote_qdi_drv.c#2 $
==============================================================================*/
#include <string.h>
#include <stddef.h>
...
typedef struct npa_qdi_event_object
{
  qurt_qdi_obj_t base;
  npa_event_handle event;
} npa_qdi_event_object;

typedef struct npa_qdi_eventq_elem
{
  npa_event_handle handle;
  unsigned int type;
  npa_event_data *data;
  struct npa_qdi_eventq_elem *next;
} npa_qdi_eventq_elem;

static struct {
  npa_qdi_eventq_elem *head;
  npa_qdi_eventq_elem *tail;
  qurt_mutex_t lock;
...
int npa_qdi_event_invoke( int client_handle, qurt_qdi_obj_t *ptr, int method,
                          qurt_qdi_arg_t a1, qurt_qdi_arg_t a2,
                          qurt_qdi_arg_t a3, qurt_qdi_arg_t a4,
                          qurt_qdi_arg_t a5, qurt_qdi_arg_t a6,
                          qurt_qdi_arg_t a7, qurt_qdi_arg_t a8,
...
void npa_qdi_event_release( npa_qdi_event_object *obj )
{
  npa_event_trigger_type trigger_type;
  trigger_type = 
    ( npa_event_trigger_type )( obj->event->trigger_type & NPA_TRIGGER_TYPE_MASK );

  /*call appropraite destroy event api based on event type */
  if( trigger_type < NPA_TRIGGER_CUSTOM_EVENT1 )  
  {	
    npa_destroy_event( obj->event );
  }
  else
  {
	npa_destroy_custom_event( obj->event );  
  }
  Core_Free( obj );
}
                           ...
                           qurt_qdi_arg_t a9 )
{
  ...
  case NPA_QDI_ISSUE_SCALAR_REQUEST_UNCONDITIONAL:
  { /* state, request_attr */
    obj->client->request_attr = 
      (unsigned int)a2.num | NPA_REQUEST_REMOTE_CLIENT;
    npa_issue_scalar_request_unconditional( obj->client, (npa_resource_state)a1.num );
    return 0;
  }
...
int npa_remote_qdi_wait_for_event( int client_handle, qurt_qdi_arg_t a1, 
                                   qurt_qdi_arg_t a2, qurt_qdi_arg_t a3 )
{
  int ret = 0;
  npa_qdi_eventq_elem *e;
  unsigned int new_signals = 0;
  
  while ( 1 )
  {
    qurt_mutex_lock( &npa_qdi_eventq.lock );
    
    qurt_signal_clear(&npa_qdi_eventq.signal, 0x1);
    
    e = npa_qdi_eventq.head;
    if ( e )
    {
      npa_qdi_eventq.head = e->next;
      if ( e->next == NULL )
      {
        npa_qdi_eventq.tail = NULL;
      }
    }
    qurt_mutex_unlock( &npa_qdi_eventq.lock );
    
    if ( e )
    {
      ret = qurt_qdi_copy_to_user( client_handle, a1.ptr, 
                                   &(e->handle), sizeof(npa_event_handle) );
      if ( ret < 0 ) break;
      
      ret = qurt_qdi_copy_to_user( client_handle, a2.ptr, 
                                   &(e->type), sizeof(unsigned int) );
      if ( ret < 0 ) break;
      
      ret = qurt_qdi_copy_to_user( client_handle, a3.ptr, 
                                   &(e->data), sizeof(npa_event_data) );
      break;
    }
    
    ret = qurt_signal_wait_cancellable( &npa_qdi_eventq.signal, 0x1, 
                                        QURT_SIGNAL_ATTR_WAIT_ANY, (unsigned int *)&new_signals );
    if ( ret == QURT_ECANCEL ) break;
  }

  // Core_Free does a NULL check already  
  Core_Free( e );
  return ret;
}

static void npa_qdi_handle_events( void *cb_context, unsigned int event_type,
                                   void *event_data, unsigned int data_size )
{
  npa_qdi_eventq_elem *e = (npa_qdi_eventq_elem *)Core_Malloc(sizeof(npa_qdi_eventq_elem));
  CORE_VERIFY_PTR( e );
  
  e->handle = (npa_event_handle)cb_context;
  e->type = event_type;
  e->data = (npa_event_data *)event_data;
  e->next   = NULL;
  
  qurt_mutex_lock( &npa_qdi_eventq.lock );

  if ( npa_qdi_eventq.tail == NULL )
  {
    npa_qdi_eventq.tail = npa_qdi_eventq.head = e;
  }
  else
  {
    npa_qdi_eventq.tail->next = e;
    npa_qdi_eventq.tail = e;
  }

  qurt_signal_set(&npa_qdi_eventq.signal, 0x1);
                         ...
                         qurt_qdi_arg_t a9 )
{
    ...
    if ( count != 0 )
    {
      while ( pubr )
      {
        if ( domain_id & pubr->remote_domains )
        {
		  ...
		  ULOG_RT_PRINTF_2( npa.log, 
                              "Length of node name %u"
                              "Node %s", namelen, pubr->local_link->name);
  ...
  case NPA_QDI_CREATE_EVENT_HANDLE:
  {
    npa_event_handle event;
    
    npa_qdi_event_object *obj = Core_Malloc( sizeof(npa_qdi_event_object) );
    CORE_VERIFY_PTR( obj );
    memset( obj, 0, sizeof(npa_qdi_event_object) );

    event =  npa_create_event_cb( a1.ptr, a2.ptr, a3.num, 
                                  npa_qdi_handle_events, a4.ptr );
      
    if ( event )
    { /* Associate the QDI object with the local event handle and return a
         handle to this object to caller. QURT will call the object's
       * invoke routine when the guest PD calls methods on the handle and the
       * release routine when the guest PD shuts down or restarts.
       */
      ...
      /* Set up */
      obj->base.invoke = (qurt_qdi_pfn_invoke_t) npa_qdi_event_invoke;
      obj->base.refcnt = QDI_REFCNT_INIT;
      obj->base.release = (qurt_qdi_pfn_release_t) npa_qdi_event_release;
      obj->event = event;
      ...
      /* Return handle to caller */
      err = qurt_qdi_copy_to_user( client_handle, a5.ptr, &qh, sizeof(qh) );
      if ( err >= 0 ) return 0;
      
      /* If unable to return handle to caller.. */
      npa_destroy_event( event );
    }
    
    Core_Free( obj );
    return -1;
  }
  case NPA_QDI_ANY_RESOURCE_EVENT:
  {
    return npa_remote_qdi_wait_for_event( client_handle, a1, a2, a3 );
  }
...
void npa_qdi_eventq_init ( void )
{
  npa_qdi_eventq.tail = npa_qdi_eventq.head = NULL;
  qurt_mutex_init( &npa_qdi_eventq.lock );
  qurt_signal_init( &npa_qdi_eventq.signal );
}
