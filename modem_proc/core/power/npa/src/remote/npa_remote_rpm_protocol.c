...
$Header: //components/rel/core.mpss/10.0/power/npa/src/remote/npa_remote_rpm_protocol.c#1 $
============================================================================*/
...
{
  unsigned msg_id = 0, agg_msg_id = 0;
  ...
  while ( resource )
  {
    if ( _npa_resource_has_attribute( resource, NPA_RESOURCE_VECTOR_STATE ) )
    {
      ...
      /* If there is a semi active set, update it */
    ...
    else
    {
      ...
      if ( NPA_RPM_SEMI_ACTIVE_SET != RPM_ACTIVE_SET )
      {
        ...
        npa_resource_state semiactive_agg = resource->active_state;
    ...
    /* Gather the last non-zero msg_id */
    if ( msg_id ) agg_msg_id = msg_id;
    
    resource = 
      (npa_resource *) npa_list_PopFront( &npa_rpm_deferred_nas_updates );
  }
  
  /* Barrier on the last non-zero msg_id, if available */
  if ( agg_msg_id )
  {
    rpm_churn_queue( agg_msg_id );
  }
                                            ...
                                            npa_resource_state state )
{
  ...
  if ( _npa_resource_has_attribute( resource, NPA_RESOURCE_VECTOR_STATE ) )
  {
    ...
    /* If there is a semi active set, update it. In doing this inline with NAS
       requests, we assume there will be no DLS between now and real sleep */
    if ( NPA_RPM_SEMI_ACTIVE_SET != RPM_ACTIVE_SET )
    {
      /* If a vector resource doesn't support SUPPRESSIBLE2 requests,
         semiactive_state_vector has already been set to state_vector */
   ...
   * In such a case, if we are dealing with an AS request, set msg_id to the
   * msg_id of the last Fire-and-Forget request on this resource. 
   */
  if ( msg_id == 0 && set == RPM_ACTIVE_SET )
  {
