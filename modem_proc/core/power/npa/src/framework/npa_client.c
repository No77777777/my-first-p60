...
$Header: //components/rel/core.mpss/10.0/power/npa/src/framework/npa_client.c#2 $
============================================================================*/
 ...
 * @return Handle to new client, NULL if client could not be created
 */
DLL_API_GLOBAL npa_client_handle
npa_create_sync_client_ex( const char       *resource_name,
                           const char       *client_name, 
                           npa_client_type   client_type,
                           unsigned int      client_value,
                           void             *client_ref )
 ...
 */
DLL_API_GLOBAL void npa_issue_scalar_request( npa_client_handle   client, 
                                              npa_resource_state  state )
{
  ...
  if ( client->resource->active_plugin->map_request_fcn )
  {
    ...
    ULOG_RT_PRINTF_7( client->log, "npa_issue_%s_request "
                      "(client: 0x%08x) (client_name: \"%s\") "
                      "(request: %u) (mapped: %u) "
                      "(resource_name: \"%s\") (request_attr: 0x%08x)",
                      client_type, 
                      client, client->name, 
                      state, NPA_INCOMING_REQUEST(client).state,
 ...
 * @param state : Requested resource state
 */
DLL_API_GLOBAL void npa_issue_scalar_request_unconditional( npa_client_handle   client, 
                                             npa_resource_state  state )
{
  char *client_type = "custom";
  CORE_VERIFY_PTR( client );
  ...
  client->request_attr |= NPA_REQUEST_UNCONDITIONAL;
  ...
  if ( client->resource->active_plugin->map_request_fcn )
  {
    NPA_INCOMING_REQUEST(client).state = 
      client->resource->active_plugin->map_request_fcn( client, state );
    ULOG_RT_PRINTF_7( client->log, "npa_issue_%s_request_unconditional "
                      "(client: 0x%08x) (client_name: \"%s\") "
                      "(request: %u) (mapped: %u) "
                      "(resource_name: \"%s\") (request_attr: 0x%08x)",
                      client_type, 
 ...
 */
DLL_API_GLOBAL void npa_issue_vector_request( npa_client_handle   client, 
 ...
 *  @param client : Handle to client being used
 */
DLL_API_GLOBAL void npa_complete_request( npa_client_handle client )
{
  CORE_VERIFY_PTR( client );
 ...
 */
DLL_API_GLOBAL void npa_destroy_client( npa_client_handle client )
{
  npa_resource *resource;
 ...
 * npa_set_request_attribute 
 */
DLL_API_GLOBAL void npa_set_request_attribute( npa_client_handle     client,
