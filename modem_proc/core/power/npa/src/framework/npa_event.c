...
$Header: //components/rel/core.mpss/10.0/power/npa/src/framework/npa_event.c#2 $
============================================================================*/
...
/**
 * <!-- npa_dispatch_meta_event_callbacks -->
 * @brief Dispatches callbacks registered for changes to the event list
 * or one of the events in the event list, of a resource
 */
static void
npa_dispatch_meta_event_callbacks( npa_resource *resource, 
                                   npa_event *trigger_event,
                                   unsigned int caller_lock )
{
  npa_event 	*event;

  if ( !caller_lock ) Core_MutexLock( resource->event_lock );

  CORE_VERIFY_PTR( resource->events.list );

  if ( trigger_event->trigger_type == NPA_TRIGGER_META_EVENT )
  {
  ...
  {
    event = resource->events.list->meta_events;
    while ( event )
    {
      if ( NPA_TRIGGER_META_EVENT == event->trigger_type ) 
      {
  ...
  if ( !caller_lock ) Core_MutexUnlock( resource->event_lock );
}
...
npa_event_action_set_watermark( struct npa_event_action *action )
{
  ...
  if ( watermark->lo <= watermark->hi ) 
    ...
    /* Inform any meta watchers of the new watermark */
    npa_dispatch_meta_event_callbacks( resource, event, FALSE );
    
    /* Now dispatch the actual events if need be */
...
npa_event_action_set_threshold( struct npa_event_action *action )
{
  ...
  if ( threshold->lo <= threshold->hi )
  {
    ...
    /* Inform meta watchers of the new threshold */
    npa_dispatch_meta_event_callbacks( resource, event, FALSE );

    /* Now dispatch the actual event, if need be */
    event_data.resource_name = resource->definition->name;
...
/** 
 * <!-- npa_add_event_to_resource -->
 */
static void npa_add_event_to_resource( npa_resource *resource,
                                       npa_event_handle event,
                                       unsigned int trigger_meta_event )
{
  npa_event **head_ptr;
  Core_MutexLock( resource->event_lock );
  
  head_ptr = ( event->trigger_type == NPA_TRIGGER_META_EVENT ) ?
               &(resource->events.list->meta_events) :
               &(resource->events.list->reg_events);
         
  event->next = *head_ptr;
  *head_ptr = event;
  
  if ( trigger_meta_event )
  {
    npa_dispatch_meta_event_callbacks( resource, event, TRUE );
  }
...
/**
 * <!-- npa_remove_event_from_resource -->
 */
int npa_remove_event_from_resource( npa_resource *resource,
                                    npa_event_handle event )
{
  npa_event **head_ptr;
  npa_event *evt, *prev;
  boolean trigger_meta_event = FALSE;
  npa_event_trigger_type trigger_type = 
    (npa_event_trigger_type)(event->trigger_type & NPA_TRIGGER_MAX_EVENT);
  ...
  if ( trigger_type == NPA_TRIGGER_RESOURCE_AVAILABLE )
  {
    head_ptr = &(resource->events.creation_events);
  }
  else if ( trigger_type == NPA_TRIGGER_META_EVENT )
  {
    head_ptr = &(resource->events.list->meta_events);
  }
  else
  {
    head_ptr = &(resource->events.list->reg_events);
    trigger_meta_event = TRUE;
  }
  ...
  if ( evt )
  {
    if ( trigger_meta_event )
    {
      npa_dispatch_meta_event_callbacks( resource, evt, TRUE );
  }

    Core_MutexUnlock( resource->event_lock );
    Core_PoolFree( &npa.event_pool, evt );
    return DAL_SUCCESS;
  }
...
npa_event_action_destroy( struct npa_event_action *action )
  ...
  return npa_remove_event_from_resource( resource, event );
}
                          ...
                          void                  *context )
{
  ...
  npa_event_action *initial_action = NULL;
  unsigned int      trigger_meta_event = 0;
  ...
  case NPA_TRIGGER_META_EVENT:
    {
      Core_AtomicInc( &event_list->meta_count );
      trigger_meta_event = 1;
  ...
  npa_add_event_to_resource( resource, event, trigger_meta_event );
 ...
 */
DLL_API_GLOBAL npa_event_handle npa_create_event_cb( const char   *resource_name, 
                                      const char   *event_name,
                                      unsigned int  trigger,
                                      npa_callback  callback,
                                      void         *context )
 ...
 */
DLL_API_GLOBAL void npa_destroy_event_handle( npa_event_handle event )
{
  npa_resource          *resource;
  ...
  if ( trigger_type > NPA_TRIGGER_RESERVED_EVENT_2 )
  { // then it must be a state event
                                 ...
                                 npa_event_type event_type, void *event_data )
{
  ...
  if ( event_list && event_list->cust_count > 0 )
  {
    ...
    while ( event )
    {
      /* If event is active and trigger type includes "event_type", dispatch */
      if ( !NPA_EVENT_IS_DESTROYED( event ) && event->trigger_type & event_type )
      {
        ULOG_RT_PRINTF_3( npa.log,
          "npa_dispatch_custom_event "
...
npa_custom_event_action_destroy( npa_event_action *action )
{
  ...
  if ( event->resource )
  {
    npa_remove_event_from_resource( event->resource, event );
  }
  else
  { // just free the event
 ...
 */
DLL_API_GLOBAL void npa_destroy_custom_event( npa_event_handle event )
{
