...
$Header: //components/rel/core.mpss/10.0/power/npa/src/framework/npa_resource.c#1 $
============================================================================*/
...
#ifdef NPA_GATHER_STATS
  if ( _npa_resource_has_attribute( resource, NPA_RESOURCE_GATHER_STATS ) )
  {
    /* update stats */
    if ( _npa_request_has_attribute( client, NPA_REQUEST_NEXT_AWAKE ) )
    {
      npa_calc_duration( start_time, CoreTimetick_Get64(), 
                         &resource->_internal->fork_duration );
    }
    else
    {
      npa_calc_duration( start_time, CoreTimetick_Get64(), 
