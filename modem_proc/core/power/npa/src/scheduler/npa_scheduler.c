...
$Header: //components/rel/core.mpss/10.0/power/npa/src/scheduler/npa_scheduler.c#2 $
============================================================================*/
...
#ifdef NPA_SCHEDULER_WAKEUPLPR_ADJUSTMENT
  #define NPA_WAKEUPLPR_ADJUSTMENT_CYCLES   0x4
#endif
...
#ifdef NPA_SCHEDULER_WAKEUPLPR_ADJUSTMENT
/**
 * <!-- npa_scheduler_adjust_wakeuplpr_backoff -->
 *
 * @brief Record the stats and adjust the npa wakeup Lpr backoff for every adjust cycles
 *
 * @param stats   : struct used to store npa wakeuplpr adjustments
 * @param early_by: value used to caluclate adjustment
  *
 * @return 
 */
static uint64 npa_scheduler_adjust_wakeuplpr_backoff(npa_scheduler_stats_generic *stats, int64 early_by)
{
  uint64 actualAvg        = 0;
  uint64 adjusted_backoff = 0;

  /* Max total adjustment allowed for adjsut cycle.
   * Adjusting npa wakeuplpr backoff value by max of 1920ticks,incase of huge early wakeups
   */
  adjusted_backoff = MIN(1920, early_by);
  
  stats->total += adjusted_backoff;
  ++stats->count;
  
  if (stats->count >= NPA_WAKEUPLPR_ADJUSTMENT_CYCLES)
  {
	actualAvg = stats->total/stats->count;

   //Reset stats min
   if (!stats->max)
    stats->min = INT_MAX;

   if(actualAvg > stats->max)
     stats->max = actualAvg;

   if(actualAvg < stats->min)
     stats->min = actualAvg;
...
void npa_run_scheduler_requests( npa_scheduler_action *action )
{
  ...
  { /* Running because of timeout or LPR wakeup */
	...
	{
      //Adding adjusted backoff LPR time to get actual scheduled time
	  late_by = npa_compare_times(start_time, (npa_scheduler.next_scheduler_time + npa_scheduler.adjust.lpr));
	}
    ...
    { /* event is early */
      ...
      { /* just ahead of on time */
        ...
        adjective = "EARLY";
#ifdef NPA_SCHEDULER_WAKEUPLPR_ADJUSTMENT
		if ((NPA_SCHEDULER_ACTION_LPR_WAKEUP == action->type) && (npa_scheduler.adjust.lpr > 0))
		{
	       uint64 adjust = npa_scheduler_adjust_wakeuplpr_backoff(&npa_scheduler.adjust.lpr_adjust_stats, late_by);	
	       npa_scheduler.adjust.lpr -= MIN(npa_scheduler.adjust.lpr, adjust);
        }
#endif
      }
    }
  ...
  {
    lookahead = npa_scheduler.config_ptr->min_reschedule_delta
                                      ...
                                      NPA_LOCK_LIST)))
  {
    ...
    max_handle_time = CoreTimetick_Get64() + lookahead;
   ...
   /* If too early for request, put back in list, else execute */
    ...
    {
      if (action->type == NPA_SCHEDULER_ACTION_LPR_WAKEUP &&
          request_ptr->start_time > max_handle_time)
      {
        /* Unblock timers before processing a request in the batch whose
           start_time is outside max_handle_time */
      ...
      npa_list_Remove(&npa_scheduler.implied_completes_on_wakeup_list, client);
    }
  } /* for (client...) */

  /* If still blocking timer, release it */
  if ( npa_scheduler.timer.deferred_match_count )
  {
    CORE_VERIFY( 0 == (--npa_scheduler.timer.deferred_match_count) );
    timer_undefer_match_interrupt();
  ...
  */
  ...
  { /* for all clients check if should fork or issue implied complete */
    ...
    { /* It is due at wakeup and forkable, so handle it */
      ...
      ULOG_RT_PRINTF_7( npa.log, "\tNPA Scheduled Request LPR "
                        "(client: 0x%08x) (client_name: \"%s\") "
                        "(new_request: %d) (client_startTime : 0x%016llx )"
  ...
  { /* put something on the list so things changed.
    ...
    { /* have a request in NAS so need to tell sleep about the time change */
      time_timetick_type nondef_timer_expiry = 0;

      /* account for scheduler latency, sleep wake time, and 
         runtime lpr adjustment */
      absolute_time -= ( npa_scheduler.config_ptr->sched_latency +
      ...
      nondef_timer_expiry = timer_get_first_non_deferrable_timer_expiry(NULL);
      if ( nondef_timer_expiry < absolute_time &&
           ( absolute_time - nondef_timer_expiry ) >
           ( npa_scheduler.forked_queue.sum_wake_work + 
             npa_scheduler.config_ptr->timer_queue_latency ) )
      {
        absolute_time = nondef_timer_expiry - 
  ...
  ULOG_RT_PRINTF_10( npa_scheduler.log, "DONE npa_run_scheduler_forks "
                     ...
                     (int32) timer_adjust,
                     tell_sleep, npa_rpm_num_deferred_nas_updates, behind_timer);
  ULOG_RT_PRINTF_3( npa_scheduler.log,
                    "       (num_forked: %d)"
...
static void npa_run_scheduler_notifies(void)
{
  ...
  { /* have something to do */
    ...
    { /* loop and decide if need to do implied complete (backout) request
      ...
      /* Remove from the implied_completes_on_wakeup list; we are either going
         to notify clients or rollback below */
      npa_list_Remove(&npa_scheduler.implied_completes_on_wakeup_list, client);

      if ( delta < 0 )
      { /* woke up early so complete the request and put back on request queue */
        ULOG_RT_PRINTF_6( npa_scheduler.log, "\tNeed Rollback "
                          "(client: 0x%08x) "
                          "(delta: 0x%x) "
...
void npa_scheduler_handle_implied_complete(npa_scheduler_action *action)
{
  ...
  { 
    Core_MutexLock(npa_scheduler.scheduler_work_lock);
    /* If we are here and /core/cpu/busy is now 0 and there are clients in
       the _now list, move them to the _planB list */
    void *cptr = npa_list_PopFront(&npa_scheduler.implied_completes_now_list);
    while (cptr)
    {
      npa_list_PushBack(&npa_scheduler.implied_completes_planB_list, cptr);
      cptr = npa_list_PopFront(&npa_scheduler.implied_completes_now_list);
    }
    Core_MutexUnlock(npa_scheduler.scheduler_work_lock);
  }
}
                                        ...
                                        unsigned int data_size)
{
  ...
  { /* Set up a callback to initialize the scheduler LPR after "/sleep/lpr"
    ...
    static const char *lpr_deps[] = { SLEEP_LPR_NODE_NAME };
                                                   ...
                                                   void *client_ref)
{
  ...
  { /* create schedule request data structure */
    ...
    client->resource->definition->attributes |= NPA_RESOURCE_GATHER_STATS;
  }
...
/**
 * <!-- npa_has_implied_complete_pending -->
 * 
 * @brief Check if the given client has an implied complete pending
 * at the time this API is called. Note that there is no locking and 
 * results cannot be relied upon if used concurrent to scheduler activity.
 *
 * @param client: Handle of the client with the scheduled request
 * 
 * @return Returns 1 if there is an implied complete pending on the client,
 * 0 if not.
 */
unsigned int npa_has_implied_complete_pending( npa_client_handle client )
{
  CORE_VERIFY_PTR( client );
  CORE_VERIFY_PTR( client->request_ptr );
  
  return (client->request_ptr->implied_state == NPA_IMPLIED_COMPLETE_PENDING);
}
