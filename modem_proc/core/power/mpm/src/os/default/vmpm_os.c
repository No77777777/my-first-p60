...
===============================================================================
$Header: //components/rel/core.mpss/10.0/power/mpm/src/os/default/vmpm_os.c#2 $
...
void vmpm_init( void )
{
  ...
  for( mpmPinNum = 0; mpmPinNum < vmpm_getMapTblSize(); mpmPinNum++ )
  {
    ...
    if( MPM_SUCCESS == vmpm_getGpioNum(mpmPinNum, &gpioNum) )
    {
	  ...
	  {
	    vmpm_LogPrintf( 3, "Success: MPM GPIO mapping Successfull "
                           "(Handle: 0x%x) (MPM Pin Num: %d) (GPIO ID: %u)",
                           s_mpmDalGpioHandle, 
                           mpmPinNum, 
                           gpioNum );
    ...
    else if( MPM_SUCCESS == vmpm_IsIrq(mpmPinNum) )
    {
	  ...
	  {
		vmpm_LogPrintf( 3, "Success: MPM IRQ mapping successfull "
                           "(Handle: 0x%x) (MPM Pin Num: %d) (IRQ Num: %u)",
                           s_mpmDalGpioHandle, 
                           mpmPinNum, 
                           vmpm_pinMapTbl[mpmPinNum].local_pin_num );
