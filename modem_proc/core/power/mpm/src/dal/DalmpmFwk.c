/*===========================================================================
  ...
  DEPENDENCIES: None
                ...
                Copyright (c) 2014-2021 QUALCOMM Technologies, Inc. (QTI).
                All Rights Reserved.
                Qualcomm Confidential and Proprietary

===============================================================================
$Header: //components/rel/core.mpss/10.0/power/mpm/src/dal/DalmpmFwk.c#2 $
...
static uint32 mpm_Dalmpm_Detach(uint32 ClientHandle, DalDeviceHandle* h)
{
  uint32 dwref = DALFW_Release((DALClientCtxt *)(h->pClientCtxt));
  return dwref;
}

static DALResult mpm_Dalmpm_Init(uint32 ClientHandle, DalDeviceHandle *h)
{  
...
static DALResult mpm_Dalmpm_DeInit(uint32 ClientHandle, DalDeviceHandle *h)
{
  return mpm_DeviceDeInit(h->pClientCtxt);
}

static DALResult mpm_Dalmpm_PowerEvent(uint32 ClientHandle, DalDeviceHandle *h, DalPowerCmd PowerCmd, 
                                       DalPowerDomain PowerDomain )
{
  return mpm_PowerEvent(h->pClientCtxt, PowerCmd, PowerDomain);
}

static DALResult mpm_Dalmpm_Open(uint32 ClientHandle, DalDeviceHandle* h, uint32 mode)
{
  return mpm_Open(h->pClientCtxt, mode);
}

static DALResult mpm_Dalmpm_Close(uint32 ClientHandle, DalDeviceHandle* h)
{
  return mpm_Close(h->pClientCtxt);
}

static DALResult mpm_Dalmpm_Info(uint32 ClientHandle, DalDeviceHandle *h,
                                 DalDeviceInfo   *info,
                                 uint32           infoSize)
{
...
static DALResult mpm_Dalmpm_SysRequest(uint32 ClientHandle, DalDeviceHandle *h,
                                       DalSysReq        ReqIdx,
                                       const void      *SrcBuf,
                                       uint32           SrcBufLen, 
                                       void            *DestBuf,
