                        ...
                        glink_channel_event_type  testevent)
{
  ...
  {
    //should hit this when we see if R_DISCONNECTED, we do a L_DISCONNECTED by calling
    // glink_close()
    uint32  error_code = DALSYS_EventCtrl( hClientStartEvent, DALSYS_EVENT_CTRL_TRIGGER );
  ...
  {
    // remote disconnected we close and try openning the 
...
/**
 * <!-- glink_rx_intent_req_cb -->
 *
 * @brief Queues intent before sending
 *
 * @param handle        : link structure
 * @param priv          : glink ctx
 * @param pkt_priv      : per packet priv context
 ...
 * 
 */
boolean glink_rx_intent_req_cb(glink_handle_type handle,
                                    const void *priv,
                                    size_t      req_size)
{
  glink_err_type  ret_code;
  ret_code = glink_queue_rx_intent(handle, "ut", req_size);
 ...
 * <!-- glink_tx_abort -->
 *
 * @brief Callback function registered for tx abort
 *
 * @param handle        : link structure
 * @param priv          : glink ctx
 ...
 * <!-- glink_event_reset -->
 *
 * @brief Reset a registered DAL event
 *
 * @param pEvent        : Event handle
 * 
 */
 ...
 * <!-- glink_event_wait -->
 *
 * @brief waits on a particular event
 *
 * @param pObj          : glink_config
                        ...
                        uint32              type)
{
  ...
  {
    DALSYS_EventCtrl(pObj->testevent->tx_callback, DALSYS_EVENT_CTRL_RESET);
    dalResult = DALSYS_EventWait(pObj->testevent->tx_callback);
 ...
 * <!-- glink_events_init -->
 *
 * @brief inits events and registers with DAL
 *
 * @param pEvents          : event pool obj in glink_ctx
 * @param pAttb          : attribute per event
 * 
 */
...
static DALResult __qmp_client_init( DALSYSEventHandle handle, void * param )
{
  ...
  glink_ctx.config.name 				= "rpm_requests";
  ...
  link_id.xport 		                            = glink_ctx.config.transport;
  link_id.remote_ss 		                        = glink_ctx.config.remote_ss;
  link_id.link_notifier 	                      = glink_link_state_cb;
  link_id.handle			                        = NULL;
  glink_ctx.link_id = link_id;
  /* Initialize glink ut event pool */
  if(glink_events_init(glink_ctx.testevent,event_attb)){
      return GLINK_STATUS_FAILURE;
  }

  /*Register link state callback*/
  ret_code = glink_register_link_state_cb(&link_id, &glink_ctx);
  if(GLINK_STATUS_SUCCESS != ret_code)
  {
  ...
  if(glink_event_wait(&glink_ctx, UT_GLINK_LINK_UP)){
      glink_deregister_link_state_cb(link_id.handle);
      glink_events_deinit(glink_ctx.testevent);
      return GLINK_STATUS_FAILURE;
  }
  ...
  /* Indicate that we're ready for messages. */
  npa_define_marker("/init/qmp_client");
 ...
 * <!-- qmp_client_init -->
 *
 * @brief spawn a worker thread and trigger init
 *
 * @param void : void
 * 
 */
...
#if 0
void send2aop_test(void)
{
  char send_this[0x60] = "{class: clock, res: qdss, val: 5}";
  glink_err_type ret_code = send2aop(send_this, 0x60);

  send_this[31] = '6';
  ret_code = send2aop(send_this, 0x60);

  send_this[31] = '4';
  ret_code = send2aop(send_this, 0x60);
  //ret_code = qmp_msg_client_close();
