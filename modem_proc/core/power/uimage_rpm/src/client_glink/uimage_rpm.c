...
=============================================================================
$Header: //components/rel/core.mpss/10.0/power/uimage_rpm/src/client_glink/uimage_rpm.c#1 $
...
{
    uint32 i;
    uint8  islandEntry = (bimc_freq_khz < 2) ? TRUE : FALSE;
     ...
     */
    uint32 msg_id_offset = (FALSE == islandEntry) ? 3 : 1;

    UIMAGE_RPM_LOG("set_clk_values (snoc:%d) (bimc:%d) (wait:%d) (entry:%d)",
                   snoc_freq_khz, bimc_freq_khz, wait_mode, islandEntry);
    ...
    {
        UIMAGE_RPM_LOG_DEBUG("clear_acks");

        while(TRUE == uImage_rpm_readAck(0, TRUE));

        UIMAGE_RPM_LOG_DEBUG("clear_acks_done");
    }
    ...
    switch(wait_mode)
    {
        ...
        case UIMAGE_RPM_BUSY_WAIT_MODE:
            UIMAGE_RPM_LOG_DEBUG("wait_for_acks (%d, %d)", msg_id_offset, msg_id_offset+1);
             ...
             */
            if(islandEntry==FALSE)
            {
                busywait(700);
            }

            UIMAGE_RPM_LOG_DEBUG("wait_for_acks_done");
            break;

        default:
            UIMAGE_RPM_ASSERT(0, "invalid wait mode: %d", wait_mode);
    }

    UIMAGE_RPM_LOG("set_clk_values_done");
    ...
    uSleepTrans_notifyComplete(islandEntry);
}
