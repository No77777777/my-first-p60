...
================================================================================
$Header: //components/rel/core.mpss/10.0/power/sleep2.0/src/os/qurt/sleep_os.c#4 $
...
volatile sleep_state g_sleepState = {0};
...
volatile uint8 g_sleepAllowLowPowerModes = 2;
#else
...
static void sleepOS_performLPMExit(void)
{
  ...
  g_sleepState.isRPMAssistedMode =sleepTarget_getRPMAssistedMode();
  ...
  g_sleepState.master_interrupt = qurt_system_vid_get();
...
void sleepOS_PMIPerformer(void)
{
   ...
   * to exit time.  This will be overwriten in the perform function below if uncacheable.
   * This is done here to address a HW issue in RSCc timestamp unit 4 where the bus de-iso
   * sequence can overwrite it before the PC time can be read. */ 
  sleepStats_putLprTimeData(CoreTimetick_Get64(), SLEEP_STATS_TIME_STM_EXIT_TYPE);
  ...
  else
  {
    ...
    if(0 != solverOutput->selected_synthLPRM->uncacheable.num_component_modes)
    {
      /* Full PC case */
      qurt_power_exit();
      md_ss_enable_abnormal_reset_debug_path();
    }
    else
    {
	  /* Check if we are in an RPM assisted mode */	
	  ...
	  /* Get all the profiling timestamp units for fully cacheable modes */
      sleepStats_readWakeupTimes(&g_wakeupTimestamps);
    }
  }
#else
