...
================================================================================
$Header: //components/rel/core.mpss/10.0/power/sleep2.0/src/lpr/cpu_vdd_lpr.c#3 $
$DateTime: 2024/06/06 02:57:54 $ 
==============================================================================*/
#include "comdef.h"
#include "CoreTime.h"
...
static void cpuVddLPR_powerCollapseEnter(rsc_low_power_mode_type coreMode)
{
  ...
  q6LPMConfig_setupExtPLLControl(TRUE);
  ...
  switch(coreMode)
  {
    ...
    {
      md_ss_disable_abnormal_reset_debug_path();     
...
static void cpuVddLPR_powerCollapseExit(void)
{
  ...
  q6LPMConfig_setupExtPLLControl(FALSE);
 ...
 *        mode with l2 & TCM non retention (flushed).
 *
 * @param wakeupTick: Time at which master is expected to wake up.
 */
void powerConfigLPR_fullPCEnter(uint64 wakeupTick)
{
  powerConfigLPR_Q6LPMEnter(RSC_MODE_PWRC_BHS, wakeupTick);
  return;
}

/**
 * powerConfigLPR_l2PCEnter
 *
 * @brief This function will be called while entering into the power collapse 
 ...
 * @param wakeupTick: Time at which master is expected to wake up.
 */
void powerConfigLPR_l2PCEnter(uint64 wakeupTick)
{
 ...
 *
 * @brief Enter function for power collapse with L2 & TCM nonretention
 */
void cpuVddLPR_fullPCEnter(uint64 wakeupTick)
{
  cpuVddLPR_powerCollapseEnter(RSC_MODE_PWRC_BHS);
  return;
}
 ...
 * @brief Enter function for power collapse with L2 & TCM nonretention
 */
void cpuVddLPR_l2PCEnter(uint64 wakeupTick)
{
                                 ...
                                 boolean               runAsCacheable)
{
  ...
  if(start->valid)
  {
    if(TRUE == runAsCacheable)
    {
      ...
      /* HW Workaround: Use the software timestamp because a software
       * triggered RSCc sequence can overwrite the up end timestamp.
       * Since this is captured in the PMI, which is highest priority, it should be
       * close enough */ 
      end = sleepStats_getLprTimeData(SLEEP_STATS_TIME_STM_EXIT_TYPE);
    }
    else 
    {
