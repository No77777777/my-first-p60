...
=================================================================================
$Header: //components/rel/core.mpss/10.0/power/sleep2.0/src/active/sleepActive.c#1 $
...
{
  uint32          queueState=0; //harshal to 0
  npa_query_type  qres;
#ifdef USLEEP_ISLAND_MODE_ENABLE
  ...
  if(signals & SLEEP_SIGNAL_RPM_QUEUE_CHANGE)
  {
    ...
    if(NULL != g_rpmSyncHandle)
    {
      queueState = rpm_is_mq_empty();
      sleepLog_printf(SLEEP_LOG_LEVEL_ATS, 2,"SLEEP_SIGNAL_RPM_QUEUE_CHANGE : lastRPMQueueState = %d, queueState = %d", lastRPMQueueState, queueState);
      ...
      if(lastRPMQueueState != queueState)
      {
        ...
        if(FALSE == queueState)
        {
          /* If there are messages in the RPM cmd queue, disable all RPM power modes */
