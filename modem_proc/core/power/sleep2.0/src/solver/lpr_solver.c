...
================================================================================
$Header: //components/rel/core.mpss/10.0/power/sleep2.0/src/solver/lpr_solver.c#1 $
                                                  ...
                                                  uint64 refDeadline)
{
   ...
   * First check if it's valid.  Pad by a few to account for SW logic overhead */
  if(refDeadline < (duration_thresh + 16))
  {
    return 0;
  }
                                          ...
                                          sleep_solver_output *output)
{
  ...
  if(SLEEP_ENABLED == synthRegistry_getStatus(REGISTRY_TYPE_LPR))
  {
    ...
    if(input->hard_deadlines.overall_min < (CoreTimetick_Get64() + input->soft_duration))
    {
    ...
    if(lookupElement != NULL)
    {
      ...
      /* Verify things haven't expired already */
      if(0 == output->threshold_deadline)
      {
        sleepLog_printf(SLEEP_LOG_LEVEL_INFO, (1 * 2),
                        " LPR Solver bypass - Threshold in past "
                        "(Hard Deadline: 0x%llx)",
                        ULOG64_DATA(input->hard_deadlines.overall_min));
      ...
      for(nMode = 0; nMode < SLEEP_LPR_NUM_SYNTHESIZED_MODES; nMode++)
      {
        ...
        /* Final check for deadline in past */
        if(input->hard_deadlines.overall_min <= CoreTimetick_Get64())
        {
          sleepLog_printf(SLEEP_LOG_LEVEL_INFO, (1 * 2),
                          " LPR Solver bypass - Final check, deadline in past "
                          "(Hard Deadline: 0x%llx)",
                          ULOG64_DATA(input->hard_deadlines.overall_min));
