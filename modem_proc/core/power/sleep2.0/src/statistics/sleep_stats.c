...
================================================================================
$Header: //components/rel/core.mpss/10.0/power/sleep2.0/src/statistics/sleep_stats.c#1 $
...
void sleepStats_trackPowerCollapseTime(uint8 enable)
{
  ...
  if(TRUE == enable)
  {
    ...
    g_sleepTotalPowerCollapseTime.last_enable      = CoreTimetick_Get64();
  }
  else
  {
 ...
 */
void sleepStats_getPowerCollapseTime(sleepStats_mode_stats *stats)
{
  CORE_VERIFY_PTR(stats);

  /* Copy the current data */
  Core_MutexLock(g_sleepTotalPowerCollapseTime.lock);
  memcpy(stats, &g_sleepTotalPowerCollapseTime.stats, sizeof(sleepStats_mode_stats));
  Core_MutexUnlock(g_sleepTotalPowerCollapseTime.lock);
