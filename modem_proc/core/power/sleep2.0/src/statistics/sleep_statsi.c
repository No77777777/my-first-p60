...
================================================================================
$Header: //components/rel/core.mpss/10.0/power/sleep2.0/src/statistics/sleep_statsi.c#2 $
...
extern volatile sleep_state g_sleepState;
...
sleepLog_printf(SLEEP_LOG_LEVEL_WAKEUPS, 4 + (2 * 2),
                  " Last RSCc power down: (Start: 0x%llx  V/O: %u %u) (End: 0x%llx  V/O: %u %u) ",
                  ULOG64_DATA(g_wakeupTimestamps.rsc_child[STATS_RSC_PROFILE_DN_START].timeStamp),
                  g_wakeupTimestamps.rsc_child[STATS_RSC_PROFILE_DN_START].valid,
                  g_wakeupTimestamps.rsc_child[STATS_RSC_PROFILE_DN_START].overflow,
                  ULOG64_DATA(g_wakeupTimestamps.rsc_child[STATS_RSC_PROFILE_DN_END].timeStamp),
                  g_wakeupTimestamps.rsc_child[STATS_RSC_PROFILE_DN_END].valid,
                  g_wakeupTimestamps.rsc_child[STATS_RSC_PROFILE_DN_END].overflow);
...
void sleepStats_init(void)
{
  ...
  CORE_VERIFY_PTR(g_sleepTotalPowerCollapseTime.lock  = Core_MutexCreate(CORE_MUTEXATTR_DEFAULT));
                                ...
                                int64 wakeupAdjust)
{
  ...
  CORE_LOG_VERIFY((((int64)(*currentBackoff) + wakeupDelta) > 0) &&
                  (wakeupDelta < US_TO_TICKS(0.75 * 1000 * 1000)),
...
void sleepStats_logWakeup(void)
{
  ...
  sleepStats_wakeup_reason       master_wakeupReason;
  char                          *wakeupReason;
  ...
  /* Get expected Qtimer wakeup timetick */
 ...
 if (FALSE == g_sleepState.isRPMAssistedMode)
  {
   /* Obtain time when control return to Sleep exit path */
   actualWakeup  = g_wakeupTimestamps.rsc_child[STATS_RSC_PROFILE_UP_START].timeStamp;
   ...
   if( actualWakeup > expectedWakeup )
    {
      master_wakeupReason = SLEEP_STATS_WAKEUP_SA_SCHEDULED;
      wakeupReason = "Timer";
    }
   else
    {
      master_wakeupReason = SLEEP_STATS_WAKEUP_SA_UNSCHEDULED;
      wakeupReason = "Rude";
    }
    }
 else
   ...
   master_wakeupReason = rpmStats->wakeup_reason;
   if(SLEEP_STATS_WAKEUP_RPM_SCHEDULED == master_wakeupReason)
    {
      ...
      actualWakeup = rpmStats->wakeup_ind;
      wakeupReason = "Timer";
    }
   else if(SLEEP_STATS_WAKEUP_RPM_UNSCHEDULED == master_wakeupReason)
    {
      /* If unscheduled wakeup, use the bringup ack which will always be valid */
      actualWakeup = rpmStats->bringup_ack;
      wakeupReason = "Rude";
     ...
     actualWakeup += SLEEP_FIXED_WAKEUP_OFFSET;
#endif
  }
  else 
  {
   ...
   CORE_VERIFY(actualWakeup > lastRPMWakeup);
   lastRPMWakeup = actualWakeup;
  }	  
  ...
  if(TRUE == g_sleepState.isRPMAssistedMode)
    {
  ...
  sleepLog_QDSSPrintf(SLEEP_LOG_LEVEL_INFO, SLEEP_WAKEUP_NUM_ARGS,
					  ...
					  wakeupReason, 
                      g_sleepState.master_interrupt,
                      ULOG64_DATA(actualWakeup),
                      ULOG64_DATA(expectedWakeup),
                      ULOG64_DATA(wakeupAdjust));  //do we want wake up reason?
 ...
 if(TRUE == g_sleepState.isRPMAssistedMode)
  ...
  { 
    /* Log when MCPM - sleep Race condition occured . */
    sleepLog_printf( SLEEP_LOG_LEVEL_PROFILING, 0,"Cacheable mode");
    return;
  }
 ...
 */
void sleepStats_updatePCTime(uint64 time, uint64 pcExitTimestamp)
{
  Core_MutexLock(g_sleepTotalPowerCollapseTime.lock);

  /* Update only if enabled and the last time enabled was before* 
   * the last power collapse exit */
  if((TRUE == g_sleepTotalPowerCollapseTime.tracking_enabled) &&
     (g_sleepTotalPowerCollapseTime.last_enable < pcExitTimestamp))
  {
    /* Update stats */
  ...
  Core_MutexUnlock(g_sleepTotalPowerCollapseTime.lock);
