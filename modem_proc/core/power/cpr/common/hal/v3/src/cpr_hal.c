 ...
 * $Header: //components/rel/core.mpss/10.0/power/cpr/common/hal/v3/src/cpr_hal.c#1 $
 * $Change: 18983508 $
 *
 */
#include "cpr_hal.h"
...
{
    uint32 targetOffset = (HWIO_CPR_TARGET1_0_m_ADDR( 0,  0 ) - HWIO_CPR_TARGET0_0_m_ADDR( 0,  0 )) * ro;

    CPR_LOG_TRACE( " Setting RO %u to %u", ro, quotient );

    CPR_HWIO_OUT_FIELD( HWIO_CPR_TARGET0_0_m_ADDR( hdl->controller->base,  mode ) + targetOffset,
                        HWIO_CPR_TARGET0_0_m_TARGET0,
                        quotient );
}
...
void cpr_hal_enable_cpr_ctrl_interaction( cpr_hal_handle* hdl, boolean enable )
{
    ...
    if (enable != val)
    {
        CPR_LOG_ERROR("cpr_hal_enable_cpr_ctrl_interaction(%d) != %d",
                enable, val);
    }
}

boolean cpr_hal_cpr_ctrl_interaction_enabled(void)
{
    return (HWIO_RPMH_CPRF_CPRFm_CTRL_INMI(MSS_CPRF_ID,
            HWIO_RPMH_CPRF_CPRFm_CTRL_CTRL_INTERACTION_EN_BMSK) >>
            HWIO_RPMH_CPRF_CPRFm_CTRL_CTRL_INTERACTION_EN_SHFT);
}
...
void cpr_hal_enable_cprf_interface( cpr_hal_handle* hdl, boolean enable )
{
    ...
    val = CPR_HWIO_IN_FIELD(
                (HWIO_CPR_MISC_REGISTER_ADDR(hdl->controller->base)),
...
uint32 cpr_hal_get_hw_ol_count(uint32 mode)
{
    return HWIO_RPMH_CPRF_CPRFm_VALUES_4_n_INMI2(MSS_CPRF_ID, mode,
            HWIO_RPMH_CPRF_CPRFm_VALUES_4_n_OL_COUNT_BMSK); 
}
...
void cpr_hal_set_cprf_defaults ( const char* name, uint8 cprf_id )
{
    ...
    uint32 CPRFm_STEP_SETTINGS = 0x28;
    //TIMEOUT[31:1]=0xf00 ENABLE[0]=1
