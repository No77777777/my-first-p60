...
#define CPR_MSS_LDO_NPA_RESOURCE        "/cpr/mssldo/uv"
...
static cpr_npa_mss_resource_type cpr_npa_mss_ldo_resource =
{
  ...
  /* Node */
  {
    "/node" CPR_MSS_LDO_NPA_RESOURCE,        /* Name           */
    &cpr_npa_driver_func,                    /* Driver fn      */
    NPA_NODE_DEFAULT,                        /* Attributes     */
    NULL,                                    /* User data      */
    0,                                       /* Num Dependency */
...
void cpr_image_enable_measurements(cpr_rail* rail, cpr_measurement_type type)
{
    ...
    if((type == CPR_MEASUREMENT_TYPE_AGING) && (rail->id == CPR_RAIL_MSS))
    {
        ...
        pmapp_pwr_vdd_mss_set_npm(TRUE); // will set mss to NPM mode
    }
}
...
void cpr_image_disable_measurements(cpr_rail* rail, cpr_measurement_type type)
{
    ...
    if((type == CPR_MEASUREMENT_TYPE_AGING) && (rail->id == CPR_RAIL_MSS))
    {
        ...
        pmapp_pwr_vdd_mss_set_npm(FALSE); // will set mss back to Default mode
    }
...
{
    if ((eRail != CPR_RAIL_MSS && eRail != CPR_RAIL_MSS_LDO) || eCorner > RAIL_VOLTAGE_LEVEL_MAX )
    {
...
DALResult CPR_GetSupportedCorners( cpr_domain_id eRail, rail_voltage_level **eCorners, uint32 *uCornerCount )
{
    ...
    else if (eRail == CPR_RAIL_MSS_LDO)
    {
...
{
    if ((eRail != CPR_RAIL_MSS && eRail != CPR_RAIL_MSS_LDO) || eCorner > RAIL_VOLTAGE_LEVEL_MAX || nVoltageUV == NULL)
    {
        return DAL_ERROR_INVALID_PARAMETER;
    }
...
{
    if ((eRail != CPR_RAIL_MSS && eRail != CPR_RAIL_MSS_LDO) || eCorner > RAIL_VOLTAGE_LEVEL_MAX || floorVoltageUV == NULL || ceilingVoltageUV == NULL)
    {
        return DAL_ERROR;
    }
...
{
    if ((eRail != CPR_RAIL_MSS && eRail != CPR_RAIL_MSS_LDO) || eCorner > RAIL_VOLTAGE_LEVEL_MAX || nSafeVoltageuV == NULL)
    {
        return DAL_ERROR_INVALID_PARAMETER;
    }
...
{
    if (eRail != CPR_RAIL_MSS && eRail != CPR_RAIL_MSS_LDO)
    {
        return DAL_ERROR_INVALID_PARAMETER;
    }
