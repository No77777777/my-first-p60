...
================================================================================
$Header: //components/rel/core.mpss/10.0/power/uSleep/src/os/qurt/ddr/uSleep_os_trans.c#1 $
...
/*==============================================================================
                                EXTERNAL VARIABLES
 =============================================================================*/
extern int g_uSleepOrigThreadPriority;
...
static void uSleepOS_beginEntryTransition(uint64 startTime)
{
  ...
  uSleep_setStateInternal(uSLEEP_INTERNAL_STATE_INIT_ENTRY);

  CORE_VERIFY_PTR(profileData);


  /* Set original thread priority */
  g_uSleepOrigThreadPriority = qurt_thread_get_priority(qurt_thread_get_id());
...
{
  rsc_profile_unit  dummyRSCTimes[RSC_PROFILE_NUMBER_OF_UNITS];

  /* Undefer normal operational timers that were deferred on the way into uSleep */
  timer_undefer_match_interrupt();

  /* Clear out any previous wakeup timestamps, skip PDC read for now since it's
     over an external bus and can cause delays */
  rsc_profile_get_unit_data(RSC_CHILD, dummyRSCTimes);
 // rsc_profile_get_unit_data(RSC_PARENT, dummyRSCTimes);
