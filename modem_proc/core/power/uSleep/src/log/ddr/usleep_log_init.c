...
================================================================================
$Header: //components/rel/core.mpss/10.0/power/uSleep/src/log/ddr/usleep_log_init.c#1 $
...
void uSleepLog_initialize(void)
{
  ...
  for(nLog = 0; nLog < USLEEP_LOG_TOTAL_NUMBER; nLog++)
  {
    ...
    if(USLEEP_LOG_IS_VALID_LEVEL(level) && 0 == g_uSleepLogHandles[level])
    {
      ...
      /* Create island logs */
      status = micro_diagbuffer_create(&g_uSleepLogHandles[level],
      ...
      CORE_VERIFY(MICRO_DIAGBUFFER_SUCCESS == status);
