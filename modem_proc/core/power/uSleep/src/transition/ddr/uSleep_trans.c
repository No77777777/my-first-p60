...
================================================================================
$Header: //components/rel/core.mpss/10.0/power/uSleep/src/transition/ddr/uSleep_trans.c#1 $
...
void uSleepTrans_completeTransitionToNormalMode(boolean shortCutBypass)
{
  ...
  CORE_LOG_VERIFY(((CoreTimetick_Get64() - transProfileData->exit_start_time) < US_TO_TICKS(50000)),
                  uSleepDDRLog_printf(USLEEP_DDR_LOG_LEVEL_TRANSITION,(1 * 2), "FATAL: VERY LARGE ISLAND EXIT LATENCY DETECTED (Exit start: 0x%llx)",
				  ULOG64_DATA(transProfileData->exit_start_time)));

  if(TRUE != shortCutBypass)
  {
