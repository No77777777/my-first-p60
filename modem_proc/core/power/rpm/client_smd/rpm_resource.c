...
static rpm_key_data_t *rpm_resource_add_key(rpm_resource_t *self, unsigned key)
{
    ...
    for(i = 0; i < old_num_keys; ++i)
    {
        ...
        if(key_data->key > key)
        {
            ...
            memsmove(&self->keys[i+1],(old_num_keys - i) * sizeof(rpm_key_data_t),
                            key_data, (old_num_keys - i) * sizeof(rpm_key_data_t));
...
static rpm_request_t *rpm_key_get_local_request(rpm_key_data_t *key_data, rpm_set_type set)
{
    switch(set)
    {
        ...
        case RPM_SEMI_ACTIVE_SET:
            return &key_data->semi_active_local;
...
static void rpm_key_invalidate(rpm_key_data_t *key_data, rpm_set_type set)
{
    ...
    //
    //  Don't mark this as dirty if its a semi_active set request since we don't
    //  need to update the RPM (as it doesn't know anything about semi-active). Just mark it invalid
    if(rpm->valid)
...
static void rpm_key_update(rpm_key_data_t *key_data, rpm_set_type set, unsigned length, void *data)
{
    ...
    else
    {
        ...
        if((local_size != bytearray_size(rpm_value->data)) ||
           (local_data && rpm_data && memcmp(local_data, rpm_data, local_size)))
        {
            key_data->dirty[set] = 1;
        }
        else
    ...
    for (i = 0; i < self->num_keys; i++)
    {
        if(self->dirty[set])
        {
            ...
            switch(set)
            {
                ...
                case RPM_SEMI_ACTIVE_SET:
                    num_bytes += (8 + bytearray_size(key_data->semi_active_local.data));
                    break;
...
void rpm_resource_update(rpm_resource_t *self, rpm_set_type set, kvp_t *request_data)
{
    ...
    if(!request_data)
    {
        ...
        for(i = 0, key_data = self->keys; i < self->num_keys; ++key_data, ++i)
        {
...
rpm_set_type get_alt_set(rpm_set_type set)
{
    rpm_set_type alt_set = RPM_NUM_SETS;
    if(RPM_SLEEP_SET == set)
    {
        alt_set = RPM_SEMI_ACTIVE_SET;
    }
    else if(RPM_SEMI_ACTIVE_SET == set)
    {
        alt_set = RPM_SLEEP_SET;
    }

    return alt_set;
}

bool rpm_key_compose_data(rpm_key_data_t *key_data, rpm_set_type set, kvp_t *message_data)
{
    rpm_set_type  alt_set = get_alt_set(set);
    rpm_request_t *local_value = rpm_key_get_local_request(key_data, set);
    rpm_request_t *rpm_value   = rpm_key_get_cached_request(key_data, set);
    ...
    key_data->dirty[set] = 0;
    if((alt_set != RPM_NUM_SETS) && (!key_data->dirty[alt_set]) && rpm_key_get_local_request(key_data, alt_set)->valid)
    {
        key_data->dirty[alt_set] = 1;
        return true;
    }
    return false;
}
...
bool rpm_resource_compose_message(rpm_resource_t *self, rpm_set_type set, kvp_t *message_data)
{
    ...
    if(self->dirty[set])
    {
        ...
        for(i = 0; i < self->num_keys; ++key_data, ++i)
        {
            ...
            if(key_data->dirty[set])
                alt_keys |= rpm_key_compose_data(key_data, set, message_data) ? 1 : 0;
        }
        ...
        // If there's an alternate set with new dirty keys, update
        if(alt_keys && (alt_set != RPM_NUM_SETS))
        {
          rpm_update_num_dirty(alt_set, self->dirty[alt_set], alt_keys);
          self->dirty[alt_set] |= alt_keys;
        }
...
rpm_key_data_t *rpm_resource_preallocate(rpm_resource_t *self, unsigned key, unsigned expected_length)
{
    ...
    RESERVE_BUFFER(semi_active_local);
