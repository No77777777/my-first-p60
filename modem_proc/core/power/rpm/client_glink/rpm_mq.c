...
void rpm_mq_retire_acks(rpm_mq_t *self)
{
    ...
    {
        msg = (rpm_msg_t *)inq_pop(self->completed);
    ...
    {
        msg = (rpm_msg_t *)inq_pop(self->in_flight);
...
void rpm_mq_completed(rpm_mq_t *self, unsigned msg_id, bool send_pending_msg)
{
    ...
    ULOG_RT_PRINTF_2(rpm_log, "rpm_mq_completed (msg_id: 0x%0.8x) (send_pending_msg: %d)", msg_id, send_pending_msg);
