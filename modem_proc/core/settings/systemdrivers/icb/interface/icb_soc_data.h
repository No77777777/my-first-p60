...
$Header: //components/rel/core.mpss/10.0/settings/systemdrivers/icb/interface/icb_soc_data.h#3 $

when         who     what, where, why
----------   ---     --------------------------------------------------------
2016/08/08   sds     Add support for multiple targets binary compatibility
...
typedef struct
{
  ...
  /* Bookkeeping */
  bool bApplied;
} icb_qnoc4_master_defaults_type;
...
}icb_qnoc2_master_defaults_type;
...
/* Target definition structures */
typedef struct
{
  void                  *pDest;
  void                  *pSource;
  uint32_t               uSize;
} icb_target_delta_type;

typedef struct
{
  uint32_t               uChipFamily;
  uint32_t               uChipVersion;
  bool                   bExactMatch;
  icb_target_delta_type *pDeltas;
  uint32_t               uNumDeltas;
} icb_target_type;
