...
/*===========================================================================
  ...
  $Header: //components/rel/core.mpss/10.0/storage/efs/src/fs_golden_copy.c#2 $ $DateTime: 2019/12/17 05:16:47 $ $Author: pwbldsvc $
...
int
fs_golden_copy_is_hdr2_valid (struct fs_golden_copy_image_header2 *fsg_hdr)
{
  FS_ASSERT (fsg_hdr != NULL);

  if (fsg_hdr->magic1 != FS_GOLDEN_COPY_IMAGE_HEADER_V2_MAGIC_1)
    return 0;
  if (fsg_hdr->magic2 != FS_GOLDEN_COPY_IMAGE_HEADER_V2_MAGIC_2)
    return 0;
  if (fsg_hdr->magic3 != FS_GOLDEN_COPY_IMAGE_HEADER_V2_MAGIC_3)
    return 0;
  if (fsg_hdr->magic4 != FS_GOLDEN_COPY_IMAGE_HEADER_V2_MAGIC_4)
    return 0;

  if (fsg_hdr->version != FS_GOLDEN_COPY_IMAGE_HEADER_V2_VERSION)
    return 0;
...
int
fs_golden_copy_create_hdr2 (struct fs_golden_copy_image_header2 *fsg_hdr)
{
  FS_ASSERT (fsg_hdr != NULL);

  memset (fsg_hdr, 0, sizeof(*fsg_hdr));

  fsg_hdr->magic1 = FS_GOLDEN_COPY_IMAGE_HEADER_V2_MAGIC_1;
  fsg_hdr->magic2 = FS_GOLDEN_COPY_IMAGE_HEADER_V2_MAGIC_2;
  fsg_hdr->magic3 = FS_GOLDEN_COPY_IMAGE_HEADER_V2_MAGIC_3;
  fsg_hdr->magic4 = FS_GOLDEN_COPY_IMAGE_HEADER_V2_MAGIC_4;

  fsg_hdr->version = FS_GOLDEN_COPY_IMAGE_HEADER_V2_VERSION;
