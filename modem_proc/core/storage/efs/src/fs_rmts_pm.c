...
/*===========================================================================
  ...
  $Header: //components/rel/core.mpss/10.0/storage/efs/src/fs_rmts_pm.c#2 $ $DateTime: 2019/12/17 05:16:47 $ $Author: pwbldsvc $

when         who   what, where, why
----------   ---   ---------------------------------------------------------
2019-11-24   vm    Add compressed FSG zip-tar support.
2019-11-24   vm    Fix new gcc-compiler warnings.
2018-01-19   rp    Provide bigger buffer to the security API
 ...
 * This buffer is also used to hold secboot_util_handle_type structure while
 * calling the security API to verify TAR-file. So making this buffer bigger
 * than the secboot_util_handle_type
 */
static uint8 shared_superblk_buf [FS_RMTS_SUPERBLOCK_SIZE * 2];
static struct fs_rmts_superblock *shared_superblk;
  ...
  static uint8 fs_rmts_pm_zip_heap [FS_RMTS_ZIP_COMPRESSION_MEMORY];
#else
  static uint8 fs_rmts_pm_zip_heap[1];
#endif
...
static void fs_rmts_pm_decrypt_fsg_buf (struct fs_rmts_pm *pm,
...
fs_rmts_pm_init (efs_device_t dev)
{
  ...
  pm->zip_heap = fs_rmts_pm_zip_heap;
  pm->zip_heap_size = sizeof (fs_rmts_pm_zip_heap);
...
fs_rmts_pm_compress_efs_copy (struct fs_rmts_pm *pm)
{
  ...
  if (pm->is_compression_enabled == 1)
  {
    ...
    zip_handle = fs_zip_open (FS_ZIP_MODE_COMPRESSION, pm->zip_heap,
                              pm->zip_heap_size);
...
fs_rmts_pm_decompress_efs_copy (struct fs_rmts_pm *pm)
{
  ...
  zip_handle = fs_zip_open (FS_ZIP_MODE_DECOMPRESSION, pm->zip_heap,
                            pm->zip_heap_size);
...
fs_rmts_pm_try_golden_copy (struct fs_rmts_pm *pm)
{
  ...
  {
    fsg_hdr = (struct fs_golden_copy_image_header2 *)buf;
    valid = fs_golden_copy_is_hdr2_valid (fsg_hdr);
    if (valid == 1)
    {
      fs_rmts_pm_decrypt_fsg_buf (pm, fsg_hdr);
      is_tar_present = 1;
      tar_offset = sizeof (*fsg_hdr);
      tar_size = fsg_hdr->image_size;
    }
  }
                            ...
                            enum fs_golden_copy_type golden_copy_type)
{
  ...
  default:
    FS_ERR_FATAL("[%d] ]unsupported golden copy type",
                (int)golden_copy_type, 0, 0);
    break;
  }
...
static void
fs_rmts_pm_encrypt_fsg_buf (struct fs_rmts_pm *pm,
                            struct fs_golden_copy_image_header2 *fsg_hdr)
{
  uint8 *src_addr, *dst_addr, *hash_addr, *buf;
  uint32 hdr_size, enc_size;
  uint64 start_time, end_time, enc_time_in_us;
  int result;

  FS_ASSERT (pm != NULL);
  FS_ASSERT (fsg_hdr != NULL);

  hdr_size = sizeof (*fsg_hdr);

  result = fs_golden_copy_is_hdr2_valid (fsg_hdr);
  FS_ASSERT (result == 1);

  buf = (uint8 *)fsg_hdr;
  src_addr = buf + hdr_size;
  dst_addr = src_addr;
  hash_addr = (uint8 *)fsg_hdr->image_encryption_hash;
  enc_size = fsg_hdr->image_size;

  start_time = fs_timetick_get_timetick ();

  authenticate_encrypt_memory(dst_addr, src_addr, enc_size, hash_addr);

  end_time = fs_timetick_get_timetick ();

  fsg_hdr->is_image_encrypted = 1;

  enc_time_in_us = fs_timetick_timetick_diff_to_us (end_time - start_time);

  FS_MSG_HIGH_2("[%d,%d] fsg enc time", (uint32)enc_time_in_us, enc_size);

  pm->debug_info.fsg_enc_time_in_us = (uint32)enc_time_in_us;
...
static void
fs_rmts_pm_decrypt_fsg_buf (struct fs_rmts_pm *pm,
                            struct fs_golden_copy_image_header2 *fsg_hdr)
{
  uint8 *src_addr, *dst_addr, *hash_addr, *buf;
  uint32 hdr_size, enc_size;
  uint64 start_time, end_time, dec_time_in_us;
  int result;
  ...
  dst_addr = src_addr;
  enc_size = fsg_hdr->image_size;
  hash_addr = fsg_hdr->image_encryption_hash;

  start_time = fs_timetick_get_timetick ();

  authenticate_decrypt_memory (dst_addr, src_addr, enc_size, hash_addr);

  end_time = fs_timetick_get_timetick ();

  dec_time_in_us = fs_timetick_timetick_diff_to_us (end_time - start_time);

  FS_MSG_HIGH_2("[%d,%d] fsg dec time", (uint32)dec_time_in_us, enc_size);

  pm->debug_info.fsg_dec_time_in_us = (uint32)dec_time_in_us;
...
fs_rmts_pm_write_to_fsg (fs_pm_t pm_data, uint8 *buf, uint32 buf_size,
                        uint32 partition_arg, int do_encryption)
{
  struct fs_rmts_pm *pm = (struct fs_rmts_pm *)pm_data;
  enum fs_rmts_device_errno result;
  ...
  if (do_encryption)
  {
    ...
    fs_rmts_pm_encrypt_fsg_buf (pm, fsg_hdr);
  }
...
void fs_rmts_pm_erase_efs (void)
{
  ...
  struct fs_rmts_superblock *sb;
  enum fs_rmts_device_errno result;
  uint8 original_is_curr_sync_full_sync, *shared_buf_addr;
