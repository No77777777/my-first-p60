...
/*===========================================================================
  ...
  $Header: //components/rel/core.mpss/10.0/storage/efs/src/fs_profile.c#2 $ $DateTime: 2021/12/06 03:42:39 $ $Author: pwbldsvc $

when         who   what, where, why
----------   ---   ---------------------------------------------------------
2021-11-22   ms    EFS logging initialization efs_logging_enabled flag 
...
fs_profile_init (void)
{
  ...
  /* EFS logging initialization if we set efs_logging_enabled to 1 then logging code will start execution, by default efs_logging_enabled is set to zero */
  int result;
  result  = efs_get( EFS_LOGGING_STATE_FILE, &efs_logging_enabled, sizeof(uint32));
  if(result == -1 && efs_errno == ENOENT)
  {
    efs_logging_enabled = 0;
    /* If the item file does not exist then we can create the golden copy.*/
    result = efs_put( EFS_LOGGING_STATE_FILE,&efs_logging_enabled,sizeof(uint32),O_CREAT|O_AUTODIR,0777);
  }
