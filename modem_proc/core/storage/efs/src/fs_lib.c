...
/*===========================================================================
  ...
  $Header: //components/rel/core.mpss/10.0/storage/efs/src/fs_lib.c#4 $ $DateTime: 2021/12/06 03:42:39 $ $Author: pwbldsvc $

when          who     what, where, why
--------      ---     ------------------------------------------------------
2021-01-22    ms      Added EFS logging under efs_logging_enabled for AUTO
2019-11-20    vm      Fix the mutex locking order between fs_lib and rest.
2017-06-02    rp      Remove err-fatal in efs_deltree and instead return error
    ...
    int mode)
{
  ...
  {
    FS_MSG_MED_0 ("efs_put_core start");
  }
  ...
  if (efs_logging_enabled == 1)
  {
    FS_MSG_MED_3 ("efs_put_core oflag %d, result %d, errno %d",oflag,result,efs_errno);
  }
#endif /* EFS_PROFILING_ENABLE */ 
  ...
  if (efs_logging_enabled == 1)
  {
    FS_MSG_MED_0 ("efs_put_core after efs_close");
  }
#endif /* EFS_PROFILING_ENABLE */ 
  ...
  if (efs_logging_enabled == 1)
  {
    FS_MSG_MED_1 ("efs_rename return result %d",result);
  }
#endif /* EFS_PROFILING_ENABLE */ 
cleanup:
  FS_GLOBAL_UNLOCK ();
...
struct efs_dir_iter_data_type efs_dir_iter_data[1];

static int efs_dir_iter_file_check_init_list (const char *path,
        struct fs_dir_iter_file_check_info *check_info,
        int is_white_list)
{
  int result, err_code, path_len;
  struct fs_stat sbuf;
  struct fs_string_iter str_iter;
  char *cur_str;
  struct fs_dir_iter_file_check_node *new_node, *prev_node;
  uint32 node_size;

  FS_ASSERT (path != NULL);
  FS_ASSERT (check_info != NULL);

  memset (check_info, 0, sizeof (*check_info));

  check_info->file_name = path;
  ...
  check_info->file_size = sbuf.st_size;
  check_info->file_buf = malloc (check_info->file_size + 1);
  if (check_info->file_buf == NULL)
  {
     FS_MSG_ERROR_1 ("[%d] malloc failed", check_info->file_size);
     errno = -ENOMEM;
     return ENOMEM;
  }

  result = efs_get (path, check_info->file_buf, check_info->file_size);
  if (result != (int)check_info->file_size)
  {
    err_code = efs_errno;
    FS_MSG_ERROR_3 ("[%d, %d, %d] efs_get failed",
      result, err_code, check_info->file_size);
    FS_ASSERT (result == (int)check_info->file_size);
    return -1;
  }

  node_size = sizeof (*new_node);
  check_info->list_head = prev_node = NULL;

  memset (&str_iter, 0, sizeof (str_iter));
  str_iter.str_buf = check_info->file_buf;
  str_iter.str_buf_len = check_info->file_size;
  str_iter.str_seperator1 = '\r';
  str_iter.str_seperator2 = '\n';
  str_iter.new_str_seperator = 0;

  if (is_white_list)
    cur_str = EFS_DIR_ITER_FSG_DIRECTORY;
  else
  ...
  while (cur_str != NULL)
  {
    ...
    path_len = strlen (cur_str);
    if (path_len <= 0)
    {
      FS_MSG_ERROR_1("[%d] invalid line in file", new_node->path_len);
      errno = -EINVAL;
      free (new_node);
      new_node = NULL;
      return EINVAL;
    }
    new_node->path_len = (uint32) path_len;

    if (check_info->list_head == NULL)
      check_info->list_head = new_node;

    if (prev_node != NULL)
      prev_node->next = new_node;

    prev_node = new_node;
    ++check_info->list_items_count;

    cur_str = fs_string_iter_get_next (&str_iter);
  }

  check_info->enforce_list = 1;

  return 0;
}

int efs_dir_iter_file_check_init (struct efs_dir_iter_data_type *iter)
{
  int result;

  FS_ASSERT (iter != NULL);

  result = efs_dir_iter_file_check_init_list (EFS_DIR_ITER_BLACK_LIST_FILE,
               &iter->black_list, 0);
  iter->dbg_info.black_list_entries_count = iter->black_list.list_items_count;
  if (result != 0)
    return result;

  result = efs_dir_iter_file_check_init_list (EFS_DIR_ITER_WHITE_LIST_FILE,
               &iter->white_list, 1);
  iter->dbg_info.white_list_entries_count = iter->white_list.list_items_count;
  return result;
}

int efs_dir_iter_file_check_in_list (const char *path, uint32 path_len,
         struct fs_dir_iter_file_check_info *check_info)
{
  struct fs_dir_iter_file_check_node *node;
  int result;

  FS_ASSERT (path != NULL);
  FS_ASSERT (check_info != NULL);

  for (node = check_info->list_head; node != NULL; node = node->next)
  {
...
int efs_dir_iter_file_check_is_ok_to_add (struct efs_dir_iter_data_type *iter,
             const char *path)
{
  uint32 path_len;
  int is_file_found = 0;
  ...
  if (iter->black_list.enforce_list)
  {
    is_file_found = efs_dir_iter_file_check_in_list (path, path_len,
                        &iter->black_list);
    if (is_file_found)
      return 0;
  }

  if (iter->white_list.enforce_list)
  {
    is_file_found = efs_dir_iter_file_check_in_list (path, path_len,
                        &iter->white_list);
    if (is_file_found)
...
efs_opendir_recursive (const char *dirname, int allow_device_specific_files,
                       int allow_secure_files)
{
  struct efs_dir_iter_data_type *iterator = NULL;
  size_t len;
  char *normalized_path = NULL, *pathbuf = NULL;
  EFSDIR *dir_iter;
  int result;
  ...
  if (efs_dir_iter_data[0].in_use != 0)
  {
    efs_errno = EMFILE;
    goto open_dir_error;
  }
  ...
  iterator = &efs_dir_iter_data[0];
  memset (iterator, 0, sizeof (*iterator));

  result = efs_dir_iter_file_check_init (iterator);
  if (result != 0)
  {
    iterator = NULL;
    goto open_dir_error;
  ...
  iterator->allow_device_specific_files = allow_device_specific_files;
  iterator->allow_secure_files = allow_secure_files;
...
efs_readdir_rec_check_entry (struct fs_dirent *dent,
                              struct efs_dir_iter_data_type *rec_dirp)
{
  struct fs_stat stat_buf;
  char *curr_path;
  char *parent_dir;
 ...
 **/
static int
efs_recursive_get_next_dir (struct efs_dir_iter_data_type *rec_dirp)
{
  int result, error;
...
static int
efs_is_subdir (const char *dir1, const char *dir2)
{
  int dir1len, dir2len, result, is_subdir;
  ...
  result = memcmp(dir1, dir2, dir1len);
  is_subdir = (result == 0) ? 1 : 0;
  return is_subdir;
}

/* returns the path to the current entry. */
const char *
efs_readdir_recursive (struct efs_dir_iter_data_type *rec_dirp)
{
  ...
  do
  {
    ...
    if (full_path)
    {
      ++rec_dirp->dbg_info.total_files_count;

      if (!rec_dirp->allow_device_specific_files)
      {
        result = efs_is_subdir(SAFE_DIR_NAME, full_path);
        if (result)
        {
          /* Skip Device Specific folders */
          ++rec_dirp->dbg_info.skipped_files_count;
          full_path = NULL;
          continue;
        }
      }
...
#ifndef FEATURE_EFS_COMPILE_WITHOUT_DIAG
      if (!rec_dirp->allow_secure_files)
      {
        result = fs_diag_access_check(full_path, DIAG_FS_CHECK_OPEN);
        if (!result)
        {
          /* Skip secure files */
          ++rec_dirp->dbg_info.skipped_files_count;
      ...
      result = efs_dir_iter_file_check_is_ok_to_add (rec_dirp, full_path);
      if (result != 1)
      {
        ++rec_dirp->dbg_info.skipped_files_count;
        full_path = NULL;
  ...
  if (full_path != NULL)
     ++rec_dirp->dbg_info.added_files_count;
...
int
efs_closedir_recursive (struct efs_dir_iter_data_type *rec_dirp)
{
  int32 err_count;
