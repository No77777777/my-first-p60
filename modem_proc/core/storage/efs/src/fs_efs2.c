 ...
 * Copyright (C) 2002-2016, 2019, 2020 QUALCOMM Technologies, Inc.
 *
 ***********************************************************************/
...
/*===========================================================================
  ...
  $Header: //components/rel/core.mpss/10.0/storage/efs/src/fs_efs2.c#4 $ $DateTime: 2021/07/13 13:42:30 $ $Author: pwbldsvc $


when          who     what, where, why
--------      ---     ------------------------------------------------------
2020-04-13    rp      Fix writing beyond inode cluster index array.
2020-02-07    eo      Fix to not assert on nlink count = 0 scenario
...
efs_unlink_helper (struct nameidata *ndp)
{
  ...
  int result, nlink_val;

  XACT_CHECK (mp);

  if (!S_ISLNK (vp->mode) && !S_ISITM (vp->mode)) {

    nlink_val = (int)vp->p.inode.nlink;
    if (nlink_val <= 0)
      goto db_cleanup;
       ...
       enum fs_golden_copy_type golden_copy_type)
{
  ...
  return mp->buf->make_golden_copy (mp->buf, golden_copy_type);
}
...
v_efs_write_to_fsg (struct fs_vnode *vp, uint8 *buf, uint32 buf_size,
                    uint32 partition, int do_encryption)
{
  struct fs_mount_efs *mp = (struct fs_mount_efs *) vp->mp;
