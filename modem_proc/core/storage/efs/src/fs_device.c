...
/*===========================================================================
  ...
  $Header: //components/rel/core.mpss/10.0/storage/efs/src/fs_device.c#3 $ $DateTime: 2021/12/08 22:53:27 $ $Author: pwbldsvc $
...
FS_ALIGN static byte verify_buffer[FS_MAX_PAGE_SIZE];
  ...
  static uint64 log_region_block_erase_count = 0;
  static uint64 data_region_block_erase_count = 0;
                      ...
                      enum fs_flashop_category category)
{
  ...
  {
    FS_MSG_MED_3 ("fs_device_write_page page %d, category %d, result %d",page,category,result);
  }
#endif /* EFS_PROFILING_ENABLE */ 
#ifdef EFS_READ_BACK_VERIFY_PAGE
    ...
    enum fs_flashop_category category)
{
  ...
  if (efs_logging_enabled == 1)
  {
    if (category == FS_FOP_LOG || category == FS_FOP_SUPER)
    {
       log_region_block_erase_count++;
       FS_MSG_MED_1 ("fs_device_erase_block log blocks erase count %ld",log_region_block_erase_count);
    }
    else if (category == FS_FOP_USER)
    {
      data_region_block_erase_count++;
      FS_MSG_MED_1 ("fs_device_erase_block data blocks erase count %ld",data_region_block_erase_count);
    }
    FS_MSG_MED_3 ("fs_device_erase_block block %d, category %d, result %d",block,category,result);
  }
#endif /* EFS_PROFILING_ENABLE */ 
