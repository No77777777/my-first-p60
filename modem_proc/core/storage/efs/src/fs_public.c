 ...
 * Copyright (C) 2002-2020,2021 QUALCOMM Technologies, Inc.
 *
...
/*===========================================================================
  ...
  $Header: //components/rel/core.mpss/10.0/storage/efs/src/fs_public.c#5 $ $DateTime: 2021/12/06 03:42:39 $ $Author: pwbldsvc $

when          who     what, where, why
--------      ---     ------------------------------------------------------
2021-11-22    ms      Added EFS logging under efs_logging_enabled for AUTO
2020-02-19    vm      Add EFS lock/unlock before calling FS Debug APIs. 
2019-11-24    vm      Improve EFS API debug framework part 2.
2019-11-24    vm      Fix the mutex locking order between fs_lib and rest.
2019-11-24    vm      Fix for fs diag read and write handler.
2019-11-24    vm      Add FS API debug info support
2019-11-24    vm      Add compressed FSG zip-tar support.
2017-04-05    rp      Fix order of acquiring mutex while golden copy restore
...
/* Defines MAX_EFS_WRITE_TASKS to 40. 
 * EFS_PROFILING_ENABLE is the top level flag that will only be compiled for AUTO */
#ifdef EFS_PROFILING_ENABLE
#define MAX_EFS_WRITE_TASKS 40
#endif /* EFS_PROFILING_ENABLE */ 
...
/* updateEFSCounter function will only be called if efs_logging_enabled is set to 1*/
#ifdef EFS_PROFILING_ENABLE
 static char task_name[50];
 static fs_os_tcb_type *tcb;
 uint64 efs_write_counters[MAX_EFS_WRITE_TASKS]={0};
 static char efs_write_task_names[MAX_EFS_WRITE_TASKS][50];
 int efs_write_index = 0;
 void updateEFSCounter()
  {
    int index;
    tcb = fs_os_self ();
    memset(task_name, 0, sizeof(task_name));
    if(tcb != NULL){
        fs_os_get_task_name(tcb, task_name, 50, NULL);
        if (efs_write_index >= MAX_EFS_WRITE_TASKS)
        {
          MSG_SPRINTF_1(MSG_SSID_EFS, MSG_LEGACY_HIGH, "EFS write request from task name %s ", task_name);
          return;
        }
        for(index=0;index<efs_write_index;index++)
          if (!strcmp(task_name, efs_write_task_names[index]))
            break;
        if (index == efs_write_index)
        {
          strlcpy(efs_write_task_names[efs_write_index], task_name, sizeof(efs_write_task_names[efs_write_index]));
          efs_write_index++;
        }
        efs_write_counters[index]++;
        MSG_SPRINTF_3(MSG_SSID_EFS, MSG_LEGACY_HIGH, "task name %s, index %d, write counter %ld ",task_name, index, efs_write_counters[index]);
    }
    else
...
/*lint -restore */
...
#ifdef EFS_PROFILING_ENABLE
  if (efs_logging_enabled == 1)
  {
      tcb = fs_os_self ();
      memset(task_name, 0, sizeof(task_name));  
      if(tcb != NULL){
          fs_os_get_task_name(tcb, task_name, 50, NULL); 
          MSG_SPRINTF_2(MSG_SSID_EFS, MSG_LEGACY_HIGH, "efs_open: path %s, task %s ",path, task_name);
      }
      else
          FS_ERR_FATAL ("null tcb !!!",0,0,0);
  }
  ...
  if (efs_logging_enabled == 1)
  {
    FS_MSG_MED_1 ("efs_open: result %d",result);
  }
#endif /* EFS_PROFILING_ENABLE */ 
...
error_case:
  ...
  if (efs_logging_enabled == 1)
  {
    FS_MSG_MED_1 ("efs_open: error %d",efs_errno);
  }
#endif /* EFS_PROFILING_ENABLE */ 
  return -1;
}
...
efs_read (int filedes, void *buf, fs_size_t nbyte)
{
  ...
  if (efs_logging_enabled == 1)
  {
    FS_MSG_MED_2 ("efs_read fd %d, result %d",filedes,result);
  }
#endif /* EFS_PROFILING_ENABLE */ 
...
efs_write (int filedes, const void *buf, fs_size_t nbyte)
{
  ...
  if (efs_logging_enabled == 1)
  {
    FS_MSG_MED_2 ("efs_write fd %d, result %d",filedes,result);
  }
#endif /* EFS_PROFILING_ENABLE */ 
  return result;
}
...
clean_up:
  ...
  if (efs_logging_enabled == 1)
  {
    FS_MSG_MED_2 ("efs_mkdir path %s, result %d",path,result);
  }
#endif /* EFS_PROFILING_ENABLE */ 
  if (result == 0)
    return 0;
...
efs_close (int filedes)
{
  ...
  if (efs_logging_enabled == 1)
  {
    FS_MSG_MED_2 ("efs_close fd %d, result %d",filedes,result);
  }
#endif /* EFS_PROFILING_ENABLE */ 
...
efs_lseek (int filedes, fs_off_t offset, int whence)
{
  ...
  if (efs_logging_enabled == 1)
  {
    FS_MSG_MED_2 ("efs_lseek fd %d, result %d",filedes,result);
  }
#endif /* EFS_PROFILING_ENABLE */ 
...
efs_truncate_helper (const char *path, uint32 length, uint32 babylon_id)
{
  ...
  if (efs_logging_enabled == 1)
  {
    FS_MSG_MED_2 ("efs_truncate_helper path %s, result %d",path,result);
  }
#endif /* EFS_PROFILING_ENABLE */ 
  return result;
}
...
efs_stat (const char *path, struct fs_stat *buf)
{
  ...
  if (efs_logging_enabled == 1)
  {
    FS_MSG_MED_2 ("efs_stat path %s, result %d",path,result);
  }
#endif /* EFS_PROFILING_ENABLE */ 
...
efs_fstat (int fd, struct fs_stat *buf)
{
  ...
  if (efs_logging_enabled == 1)
  {
    FS_MSG_MED_2 ("efs_fstat fd %d, result %d",fd,result);
  }
#endif /* EFS_PROFILING_ENABLE */ 
  return result;
}
...
error_case:
  ...
  if (efs_logging_enabled == 1)
  {
    FS_MSG_MED_2 ("efs_lstat path %s, result %d",path,result);
  }
#endif /* EFS_PROFILING_ENABLE */ 
...
error_case:
  ...
  if (efs_logging_enabled == 1)
  {
    FS_MSG_MED_2 ("efs_unlink path %s, result %d",path,result);
  }
#endif /* EFS_PROFILING_ENABLE */ 
...
error_case:
  ...
  if (efs_logging_enabled == 1)
  {
    FS_MSG_MED_2 ("efs_rmdir path %s, result %d",path,result);
  }
#endif /* EFS_PROFILING_ENABLE */ 
...
efs_chmod (const char *path, fs_mode_t mode)
{
  ...
  if (efs_logging_enabled == 1)
  {
    FS_MSG_MED_2 ("efs_chmod path %s, result %d",path,result);
  }
#endif /* EFS_PROFILING_ENABLE */ 
                      ...
                      int oflag, int mode)
{
  ...
  if (efs_logging_enabled == 1)
  {
    ...
    if (tcb != NULL){
        fs_os_get_task_name(tcb, task_name, 50, NULL);
        MSG_SPRINTF_2(MSG_SSID_EFS, MSG_LEGACY_HIGH, "efs_put: path %s, task %s ",path, task_name);
        updateEFSCounter();
    }
    else
        FS_ERR_FATAL ("null tcb !!!",0,0,0);
  ...
  if (efs_logging_enabled == 1)
  {
    FS_MSG_MED_1 ("efs_put: result %d",result);
  }
#endif /* EFS_PROFILING_ENABLE */ 
...
efs_pure_get (const char *path, void *data, fs_size_t length)
{
  ...
  if (efs_logging_enabled == 1) 
  {
      ...
      if (tcb != NULL){
          fs_os_get_task_name(tcb, task_name, 50, NULL);
          MSG_SPRINTF_2(MSG_SSID_EFS, MSG_LEGACY_HIGH, "efs_get: path %s, task %s ",path, task_name);
      }
      else
          FS_ERR_FATAL ("null tcb !!!",0,0,0);
  }
  ...
  if (efs_logging_enabled == 1)
  {
    FS_MSG_MED_1 ("efs_get: result %d",result);
  }
#endif /* EFS_PROFILING_ENABLE */ 
    ...
    enum fs_golden_copy_type golden_copy_type)
{
  ...
  if (result == 0)
    result = ndp->vp->vops->make_golden_copy (ndp->vp, golden_copy_type);
  ...
  fs_api_debug_add (FS_API_DEBUG_ID_EFS_MAKE_GOLDEN_COPY, result, -1,
  ...
  int allow_device_specific_files, int allow_secure_files)
{
  ...
  fs_api_debug_add (FS_API_DEBUG_ID_EFS_FILESYSTEM_IMAGE_OPEN,
      allow_device_specific_files, allow_secure_files, path, 0);
  ...
  switch (image_type)
  {
    ...
    {
#ifdef FEATURE_FS_ZIP_TAR
      uint8 *zip_heap_buf = NULL;
      uint32 zip_heap_buf_size = 0;

      result = efs_private_borrow_ram((void **)&zip_heap_buf,
            &zip_heap_buf_size);
      if (result != 0)
      {
        FS_MSG_HIGH_0("efs_private_borrow_ram failed");
        zip_heap_buf = NULL;
        zip_heap_buf_size = 0;
      }

      fs_img_handle.handle = fs_zip_tar_open (path,
            zip_heap_buf, zip_heap_buf_size,
            allow_device_specific_files, allow_secure_files);
#else
      efs_errno = EPERM;
...
efs_write_to_golden_copy (const char *path, uint8 *ram_addr, uint32 ram_size,
          enum fs_golden_copy_partition_type fsg_partition, int do_encryption);

int
efs_make_golden_copy_zip_tar (const char *path)
{
  int result, err_code;
  uint8 *buf = NULL, *buf_adj;
  uint32 buf_size = 0, hdr_offset = 0, buf_size_adj;
  uint32 zip_tar_size, remainder, full_image_size;
  void *zip_hdl = NULL;
  int allow_device_specific_files = 0, allow_secure_files = 1;
  struct fs_golden_copy_image_header2 *fsg_hdr = NULL;
  struct fs_golden_copy_image_header2_debug_info *fsg_dbg_info;
  struct fs_dir_iter_debug_info *iter_dbg_info;
  ...
  result = efs_get_buf_for_fsg (path, &buf, &buf_size, &hdr_offset);
  if (result != 0)
  {
    err_code = efs_errno;
    FS_MSG_ERROR_2("[%d, %d] error : get_buf_for_fsg failed",
                    result, err_code);
    goto End;
  }

  FS_MSG_HIGH_2 ("[%d, %d] got fsg ram", buf_size, hdr_offset);

  fsg_hdr = (struct fs_golden_copy_image_header2 *)(buf + hdr_offset);

  hdr_offset += sizeof (*fsg_hdr);
  buf_adj = buf + hdr_offset;
  buf_size_adj = buf_size - hdr_offset;

  result = fs_golden_copy_create_hdr2 (fsg_hdr);
  FS_ASSERT (result == 0);
  if (result != 0)
  {
    FS_MSG_ERROR_1("[%d] error : fs_golden_copy_create_hdr2 failed", result);
    goto End;
  }

#ifdef FEATURE_FS_ZIP_TAR
  zip_hdl = fs_zip_tar_open(path, NULL, 0,
              allow_device_specific_files, allow_secure_files);
#else
  efs_errno = EPERM;
  result = -1;
  ...
  if (zip_hdl == NULL)
  {
    FS_MSG_ERROR_0("error : fs_zip_tar_open failed");
    goto End;
  }

#ifdef FEATURE_FS_ZIP_TAR
  result = fs_zip_tar_create_read(zip_hdl, buf_adj, buf_size_adj);
#endif
  if (result < 0)
  {
    err_code = efs_errno;
    FS_MSG_ERROR_2("error [%d, %d] : fs_zip_tar_read failed",
                   result, err_code);
    goto End;
  }

  FS_MSG_HIGH_1 ("[%d] zip-tar created", result);

  zip_tar_size = (uint32) result;
  remainder = zip_tar_size % 512;
  zip_tar_size += (512 - remainder);

  full_image_size = hdr_offset + zip_tar_size;

  fsg_hdr->golden_copy_type = FS_GOLDEN_COPY_TYPE_ZIP_TAR;
  fsg_hdr->image_size = zip_tar_size;
  fsg_hdr->full_image_size = full_image_size;

  fsg_dbg_info = &fsg_hdr->debug_info;
  iter_dbg_info = &efs_dir_iter_data[0].dbg_info;
  fsg_dbg_info->black_list_entries_count =
     iter_dbg_info->black_list_entries_count;
  fsg_dbg_info->white_list_entries_count =
     iter_dbg_info->white_list_entries_count;
  fsg_dbg_info->total_files_count = iter_dbg_info->total_files_count;
  fsg_dbg_info->added_files_count = iter_dbg_info->added_files_count;
  fsg_dbg_info->skipped_files_count = iter_dbg_info->skipped_files_count;

  result = fs_golden_copy_is_hdr2_valid (fsg_hdr);
  FS_ASSERT (result == 1);

  FS_MSG_HIGH_2 ("[%d, %d] zip-tar size", zip_tar_size, full_image_size);

  FS_MSG_HIGH_2 ("[%d, %d] fsg file stats",
     fsg_dbg_info->black_list_entries_count,
     fsg_dbg_info->white_list_entries_count);

  FS_MSG_HIGH_3 ("[%d, %d, %d] fsg file statss",
     fsg_dbg_info->total_files_count,
     fsg_dbg_info->added_files_count,
     fsg_dbg_info->skipped_files_count);

  result = efs_write_to_golden_copy (path, buf, full_image_size,
                                    FS_GOLDEN_COPY_PARTITION_TYPE_DEFAULT, 1);

  FS_MSG_HIGH_1 ("[%d] fsg write complete", result);

End:
#ifdef FEATURE_FS_ZIP_TAR
  if (zip_hdl != NULL)
    (void) fs_zip_tar_close(zip_hdl);
#endif
                              ...
                              enum fs_golden_copy_type golden_copy_type)
{
  ...
  switch (golden_copy_type)
  {
    ...
    case FS_GOLDEN_COPY_TYPE_ZIP_TAR:
      break;
...
efs_golden_copy_signal_received (void)
{
  ...
  {
    case FS_GOLDEN_COPY_TYPE_FULL_SNAPSHOT:
      result = efs_make_golden_copy ("/", fs_golden_copy_info.fsg_type);
      break;
    ...
    case FS_GOLDEN_COPY_TYPE_ZIP_TAR:
      result = efs_make_golden_copy_zip_tar("/");
      break;
...
efs_golden_copy_restore (void)
{
  ...
  while (entry != NULL)
  {
    ...
    if (result == 1)
    {
      ...
      result = efs_write_file_helper (file_path,
               entry->file_data, entry->hdr.file_size);
      if (result != 0)
        goto End_1;
    }
    else
...
efs_auto_mkdir(const char * path)
{
  ...
  temp_path = nameidata_buf2.namebuf;
  strlcpy (temp_path, path, sizeof (nameidata_buf2.namebuf));
