 ...
 *
 *
 *  Copyright (C) 2002-2016,2018,2019  QUALCOMM Technologies, Incorporated.
 *  All Rights Reserved.
 *
 ...
 *====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*/
 ...
 /*===========================================================================
  ...
  $Header: //components/rel/core.mpss/10.0/storage/efs/src/fs_diag.c#3 $


when       who     what, where, why
--------   ---     ----------------------------------------------------------
11/24/19   vm      Add compressed FSG zip-tar support.
10/14/19   eo      fix signed and unsigned arithmetic diag read/write handler.
03/08/18   eo      Cleanup non public apis from EFS public headers
12/19/16   rp      Cleanup in recursive iteration of files during zip-tar
09/21/16   np      Added Security Feature in fs_diag_filesystem_image_open()
...
fs_diag_make_golden_copy_handler (VOID_PACKED_PTR req_ptr, uint16 pkt_len)
{
  ...
  if (fs_rmts_can_create_golden_copy() == 1)
  {
    result = efs_make_golden_copy (request_path,
                                  FS_GOLDEN_COPY_TYPE_FULL_SNAPSHOT);
    if (result == 0)
    {
      response->diag_errno = 0;
    }
...
fs_diag_filesystem_image_open (VOID_PACKED_PTR req_ptr, uint16 pkt_len)
{
  ...
  allow_device_specific_files = 0;
  allow_secure_files = 1;
