...
/*===========================================================================
  ...
  $Header: //components/rel/core.mpss/10.0/storage/fs_tar/src/fs_tar.c#3 $ $DateTime: 2019/12/17 05:16:47 $ $Author: pwbldsvc $

when         who   what, where, why
----------   ---   ---------------------------------------------------------
2018-03-05   rp    Fix enum mismatch E_SECBOOT_UTIL_SUCCESS
...
PACKED struct fs_tar_verify_info_type
{
  uint32 is_hw_fuse_blown;
  uint32 tar_image_offset;
  uint32 tar_image_size;

  secboot_util_fuse_info_type      fuse_info;
  secboot_util_verified_info_type  verified_info;

  unsigned int hw_signature_failed : 1;
  unsigned int sw_signature_failed : 1;
  unsigned int sw_signature_invalid_size : 1;

  secboot_util_error_type hw_key_secapi_res;
  secboot_util_error_type sw_key_secapi_res;

} PACKED_POST;
static struct fs_tar_verify_info_type fs_tar_verify_info;
...
{
  uint32 img_offset, img_size;
  boolean valid_signature;
  int valid_image;
  uint64 software_id;
  uint32 is_auth_enabled_fuse_blown, result;
  secboot_util_error_type secapi_res;

  secboot_util_fuse_info_type      *fuse_info = NULL;
  secboot_util_verified_info_type  *verified_info = NULL;
  secboot_util_handle_type         *sec_boot_handle = NULL;
  ...
  memset (&fs_tar_verify_info, 0, sizeof (fs_tar_verify_info));

  fuse_info = &fs_tar_verify_info.fuse_info;
  verified_info = &fs_tar_verify_info.verified_info;
  ...
  secapi_res = secboot_util_update_fuse_info (fuse_info);
  if (secapi_res != E_SECBOOT_UTIL_SUCCESS)
  {
    FS_ERR_FATAL("[%d] update_fuse_info failed", secapi_res, 0, 0);
  }
  ...
  *image_offset = 0;
  ...
  fs_tar_verify_info.is_hw_fuse_blown = 0;
  ...
  {
    fs_tar_verify_info.is_hw_fuse_blown = 1;
    fs_tar_verify_info.hw_signature_failed = 1;

    secapi_res = secboot_util_auth_img_verify
                      (
                      sec_boot_handle,
                      buf,
                      buf_size,
    ...
    fs_tar_verify_info.hw_key_secapi_res = secapi_res;
    FS_MSG_HIGH_1 ("[%d] Fuse blown. FSG check using h/w key", secapi_res);
    if(secapi_res == E_SECBOOT_UTIL_SUCCESS)
    {
      valid_signature = TRUE;
      fs_tar_verify_info.hw_signature_failed = 0;
    }
  ...
  if (valid_signature == FALSE)
  {
    ...
    fs_tar_verify_info.sw_signature_failed = 1;
    ...
    fs_tar_verify_info.sw_key_secapi_res = secapi_res;
    FS_MSG_HIGH_1 ("[%d] FSG check using s/w key", secapi_res);
    if(secapi_res == E_SECBOOT_UTIL_SUCCESS)
    {
      valid_signature = TRUE;
      fs_tar_verify_info.sw_signature_failed = 0;
    }

    fs_tar_verify_info.tar_image_offset = verified_info->img_offset;
    fs_tar_verify_info.tar_image_size = verified_info->img_len;
  }
  ...
  if (valid_signature == TRUE)
  {
    ...
    {
      fs_tar_verify_info.sw_signature_invalid_size = 1;
...
struct fs_tar_create_info
{
  ...
  struct efs_dir_iter_data_type *rec_iter; /* Recursive directory iterator */
...
struct fs_tar_create_info *
fs_tar_create_open (const char *dir_path, int allow_device_specific_files,
                    int allow_secure_files)
{
  struct efs_dir_iter_data_type *rec_iter;
  ...
  rec_iter = efs_opendir_recursive (dir_path, allow_device_specific_files,
                                    allow_secure_files);
  if (rec_iter == NULL)
    return NULL;
