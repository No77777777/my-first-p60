...
/*===========================================================================
  ...
  $Header: //components/rel/core.mpss/10.0/storage/flash_scrub/src/flash_scrub_boot_code.c#2 $ $DateTime: 2019/12/17 05:16:47 $ $Author: pwbldsvc $
...
scrub_boot_get_boot_block_count (FLASH_SCRUB_BOOT_HANDLE *handle)
{
  ...
  if (result != 0)
  {
    FLASH_SCRUB_MSG_ERR_1 ("[%d] boot partition does not exist.", result);
    return 0;
  }
...
scrub_boot_find_img (FLASH_SCRUB_BOOT_HANDLE *handle)
{
  ...
  partition_size = handle->state.parti_size_blocks;
...
scrub_boot_init_state (FLASH_SCRUB_BOOT_HANDLE *handle, uint32 scrub_count)
{
  ...
  state->parti_size_blocks = scrub_boot_get_boot_block_count (handle);
