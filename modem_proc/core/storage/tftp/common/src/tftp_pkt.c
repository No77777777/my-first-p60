...
/*===========================================================================
  ...
  $Header: //components/rel/core.mpss/10.0/storage/tftp/common/src/tftp_pkt.c#3 $ $DateTime: 2020/03/31 14:47:10 $ $Author: pwbldsvc $

when         who   what, where, why
----------   ---   ---------------------------------------------------------
2019-11-20   vm    Fix KW issue.
2018-04-17   as    Validate oack options arrary size and then add items.
                              ...
                              struct tftp_pkt_options_type *options)
{
  ...
  for ( i = 0; i < options->extended_options_count; ++i, ++option_idx)
  {
    ...
    /* The idx should be within the buffer. */
    if (idx >= TFTP_MAX_TFTP_PACKET_SIZE)
    {
      result = -1;
      goto End;
    }
    ...
    if (str_len >= remaining_buff_size || idx >= TFTP_MAX_TFTP_PACKET_SIZE)
    {
      goto End;
    }
