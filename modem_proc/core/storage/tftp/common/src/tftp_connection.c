...
/*===========================================================================
  ...
  $Header: //components/rel/core.mpss/10.0/storage/tftp/common/src/tftp_connection.c#2 $ $DateTime: 2019/12/17 05:16:47 $ $Author: pwbldsvc $

when         who   what, where, why
----------   ---   ---------------------------------------------------------
2019-11-20   vm    Bring in changes from target builds.
2018-11-20   vm    Add support for upstreamed qsockets.
2018-04-27   as    Increase the TFTP timeout from 1S to 2S.
2018-02-07   as    Add timeout support for socket-send.
          ...
          uint32 timeout_in_ms)
{
  ...
  sock_res = tftp_socket_set_tx_timeout (&handle->socket_handle,
                                         sock_timeout_ms);
  if (sock_res != TFTP_SOCKET_RESULT_SUCCESS)
  {
    err_num = tftp_socket_get_last_errno (&handle->socket_handle);
                            ...
                            uint32 *out_buffer_size)
{
  ...
  *out_buffer_size = 0;
  ...
  if (sock_res == TFTP_SOCKET_RESULT_SUCCESS)
  {
    /*todo : change below assert for exact size  */
                          ...
                          uint32 *out_buffer_size)
{
  ...
  *out_buffer_size = 0;
  ...
  switch (sock_res)
  {
    ...
    case TFTP_SOCKET_RESULT_RCVD_CTRL_PKT:
    {
      read_result = TFTP_CONNECTION_RESULT_RCVD_CTRL_PKT;
      ++handle->debug_info.ctrl_pkt_cnt;
      ++tftp_connection_info_inst.ctrl_pkt_cnt;
      TFTP_LOG_DBG ("recv ctrl-pkt : [%d / %d]",
                     handle->debug_info.ctrl_pkt_cnt,
                     tftp_connection_info_inst.ctrl_pkt_cnt);
      break;
    }
