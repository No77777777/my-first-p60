...
/*===========================================================================
  ...
  $Header: //components/rel/core.mpss/10.0/storage/tftp/common/src/tftp_protocol.c#2 $ $DateTime: 2019/12/17 05:16:47 $ $Author: pwbldsvc $

when         who   what, where, why
----------   ---   ---------------------------------------------------------
2019-11-20   vm    Address KW errors.
2019-11-20   vm    Add support for upstreamed qsockets.
...
tftp_protocol_map_con_res_to_prot_res (enum tftp_connection_result con_res)
{
  ...
  switch (con_res)
  {
    ...
    case TFTP_CONNECTION_RESULT_RCVD_CTRL_PKT:
      prot_res = TFTP_PROTOCOL_RCVD_CTRL_PKT;
      break;
...
tftp_protocol_send_pkt (struct tftp_protocol_info *proto)
{
  ...
  TFTP_ASSERT (proto->tx_pkt != NULL);
  if (proto->tx_pkt == NULL)
  {
    prot_res = TFTP_PROTOCOL_INVALID;
    goto End;
  }
...
tftp_protocol_recv_pkt (struct tftp_protocol_info *proto)
{
  ...
  if (proto->rx_pkt == NULL)
      return TFTP_PROTOCOL_INVALID;
                                    ...
                                    uint16 block_num_for_pkt)
{
       ...
       ++rx_pkt_count)
  {
    ...
    if ((proto_res != TFTP_PROTOCOL_SUCCESS) &&
        ...
        (proto_res != TFTP_PROTOCOL_RCVD_CTRL_PKT))
    {
      break;
    }
                                ...
                                const char *filename)
{
  ...
  TFTP_ASSERT (proto->tx_pkt != NULL);
  TFTP_ASSERT (proto->rx_pkt != NULL);
  if (proto->tx_pkt == NULL || proto->rx_pkt == NULL)
  {
    proto_res = TFTP_PROTOCOL_INVALID;
    goto End;
  }
                                 ...
                                 const char *error_msg)
{
  ...
  if (proto->tx_pkt == NULL)
  {
    TFTP_LOG_ERR ("Cant send error pkt: tx_pkt is null");
    return;
  }
