...
/*===========================================================================
  ...
  $Header: //components/rel/core.mpss/10.0/storage/tftp/os/src/tftp_socket_ipcr_modem.c#1 $ $DateTime: 2019/04/24 00:03:26 $ $Author: pwbldsvc $
...
enum tftp_socket_result_type
tftp_socket_set_tx_timeout (tftp_socket *socket_handle, uint32 timeout_msec)
{
  enum tftp_socket_result_type sock_res = TFTP_SOCKET_RESULT_FAILED;

  sock_res = qsetsockopt (socket_handle->sock_fd, QSOL_SOCKET, QSO_SNDTIMEO,
                          &timeout_msec, sizeof(uint32));
  return sock_res;
}
