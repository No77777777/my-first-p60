...
/*===========================================================================
  ...
  $Header: //components/rel/core.mpss/10.0/storage/tftp/client/src/tftp_client.c#2 $ $DateTime: 2019/12/17 05:16:47 $ $Author: pwbldsvc $

when         who   what, where, why
----------   ---   ---------------------------------------------------------
2019-11-20   vm    Fix file allocation handle logic.
2019-11-20   vm    Store TFTP packets in RAM for debug
2017-08-18   np    Improve the debug-logs to store TX/RX packets details
...
/*----------------------------------------------------------------------------
  tftp-client : Each client's handle instance.
----------------------------------------------------------------------------*/
...
struct tftp_client_info_type
{
  ...
  struct tftp_client_handle_type client_handles[TFTP_CLIENT_MAX_CLIENTS];
  uint32 client_handle_idx;
};
...
struct tftp_client_handle_type*
tftp_client_allocate_handle (void)
{
  struct tftp_client_handle_type *hdl = NULL;
  uint32 i, idx;
  ...
  for ( i = 0; i < TFTP_CLIENT_MAX_CLIENTS; ++i )
  {
    ...
    idx = tftp_client_info.client_handle_idx++;
    if (tftp_client_info.client_handle_idx == TFTP_CLIENT_MAX_CLIENTS)
    {
      tftp_client_info.client_handle_idx = 0;
    }

    hdl = &tftp_client_info.client_handles[idx];
    if (hdl->is_in_use == 0)
    {
      ...
      hdl->proto.tx_pkt = tftp_malloc (sizeof(struct tftp_pkt_type));
      hdl->proto.rx_pkt = tftp_malloc (sizeof(struct tftp_pkt_type));
...
void
tftp_client_free_handle (struct tftp_client_handle_type *hdl)
{
  ...
  tftp_client_lock ();
  TFTP_ASSERT (hdl->is_in_use == 1);
  hdl->is_in_use = 0;

  tftp_free (hdl->proto.tx_pkt);
  tftp_free (hdl->proto.rx_pkt);

  hdl->proto.tx_pkt = hdl->proto.rx_pkt = NULL;
...
tftp_client_copy_build_config (struct tftp_client_handle_type *client_handle)
{
  ...
  cli_config = &tftp_client_info.client_internal_cfg;
                 ...
                 void *cb_func_param, struct rfs_debug_info_type *dbg_info)
{
  ...
  --------------------------------------------------------------------------*/
  client_handle = tftp_client_allocate_handle ();
  if (client_handle == NULL)
  {
...
End:
  if (proto != NULL)
  {
    ...
    if ((debug_info != NULL) && (con_debug_info != NULL))
    {
      ...
      TFTP_LOG_DBG ("WRQ time stats : Total : [API, TX, RX] = [%u, %u, %u]",
                    (uint32) debug_info->total_api_time,
                    (uint32) con_debug_info->total_send_time_delta,
                    (uint32) con_debug_info->total_recv_time_delta);

      TFTP_LOG_DBG ("WRQ time stats : [Open, Req, Xfer] = [%u, %u, %u]",
                    (uint32) debug_info->sock_open_time,
                    (uint32) debug_info->req_time,
                    (uint32) debug_info->data_xfer_time);

      TFTP_LOG_DBG ("WRQ time stats : Tx [Min, Max] = [%u, %u]",
                    (uint32) con_debug_info->min_send_time_delta,
                    (uint32) con_debug_info->max_send_time_delta);

      TFTP_LOG_DBG ("WRQ time stats : Rx [Min, Max] = [%u, %u]",
                    (uint32) con_debug_info->min_recv_time_delta,
                    (uint32) con_debug_info->max_recv_time_delta);
  ...
  if (client_handle != NULL)
  {
    tftp_client_free_handle (client_handle);
    client_handle = NULL;
  }
...
tftp_unlink (const char *filename)
{
  ...
  options->options[idx].option_id = TFTP_PKT_OPTION_ID_UNLINK;
  options->options[idx].option_value = 0;
  options->options[idx].is_valid = 1;
  ++idx;
