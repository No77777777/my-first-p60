...
{
  uint32 ecc_read_buff, *tmp_ecc_read_buff;
  uint32 ecc_loc = 0;
  uint32 data_addr = 0;
  ...
  *status = FLASH_HAL_FAIL;
  ...
  /* We are doing a 3 point erased page check,
        1. Check the erased cw status for erased page/codeword bits
        2. Check the ECC bytes for 0xFF
        3. Check the udata spare bytes
       ...
       FLASH_ERASED_CW_DETECT_STATUS)
  {
    *status = FLASH_HAL_ERASED_PAGE;
    ...
    /* PLEASE NOTE THAT THE BELOW LOGIC IS TO COMPARE 10 BYTES OF ECC.
       THIS LOGIC NEEDS TO BE CHANGED IF THE ECC SIZE IS NOT 10 */
    ecc_loc = (page_unit_layout->udata_len_bytes != 4) ?
      (page_unit_layout->main_len_bytes + page_unit_layout->udata_len_bytes) :
      page_unit_layout->main_len_bytes;

    ecc_read_buff = ((uint32)HAL_QPIC_NANDC_NANDC_BUFFER_VADDR + ecc_loc);
    tmp_ecc_read_buff = (uint32 *) ecc_read_buff;

    /* The below logic covers first 8 bytes from ECC buffer */
    value1 = *tmp_ecc_read_buff;

    tmp_ecc_read_buff++;

    value2 = *tmp_ecc_read_buff;

    if ((value2 & value1) != 0xFFFFFFFF)
    {
      *status = FLASH_HAL_FAIL;
    }
    else
    {
      ...
      {
        /* Compare 4 bytes for 0xFFFFFFFF */
        tmp_ecc_read_buff++;
        ...
        {
          /* Compare 2 bytes for 0xFFFF */
          tmp_ecc_read_buff++;

          if ((*tmp_ecc_read_buff & 0xFFFF) != 0xFFFF)
          {
            *status = FLASH_HAL_FAIL;
          }
        }
        ...
        {
          /* Compare 1 bytes for 0xFF */
          tmp_ecc_read_buff++;

          if ((*tmp_ecc_read_buff & 0xFF) != 0xFF)
          {
            *status = FLASH_HAL_FAIL;
          }
        }
      ...
      {
        /* Compare remaining 2 bytes for 0xFFFF */
        tmp_ecc_read_buff++;
    ...
    if ((FLASH_HAL_FAIL != *status) &&
        (FLASH_HAL_HW_ECC_ON_MAIN_ONLY == priv_data->ecc_state))
    {
      data_addr = (uint32) ecc_read_buff;

      if (FLASH_HAL_HW_ECC_8BIT == priv_data->ecc_len_bits)
      {
        if (FLASH_HAL_8_BIT_WIDTH == priv_data->width)
        {
          data_addr += NAND_8BIT_ECC_SIZE_8;
          value1 = (*(uint8 *) data_addr) & 0xFF;
          if ((value1 & 0xFF) != 0xFF)
          {
            /* If all the above conditions are satisfied,
               and if the udata is not 0xFFFFFFF, we can ignore
               the ECC error from the NAND controller */
        ...
        {
          data_addr += NAND_8BIT_ECC_SIZE_16;
        }
      }
      else
      {
        data_addr += NAND_ECC_SIZE;
      }

      value1 = (*(uint16 *) data_addr) & 0xFFFF;
      data_addr += 2;
      value2 = (*(uint16 *) data_addr) & 0xFFFF;

      if ((value2 & value1) != 0xFFFF)
      {
        /* If all the above conditions are satisfied,
           and if the udata is not 0xFFFFFFF, we can ignore
           the ECC error from the NAND controller */
