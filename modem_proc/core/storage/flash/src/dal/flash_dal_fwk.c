 ...
 *   order.
 *
 *  $Header: //components/rel/core.mpss/10.0/storage/flash/src/dal/flash_dal_fwk.c#1 $ $DateTime: 2019/04/24 00:03:26 $ $Author: pwbldsvc $
 *
 *
...
static uint32 flash_fwk_detach(uint32 ClientHandle, DalDeviceHandle* pif);
static int flash_fwk_init(uint32 ClientHandle, DalDeviceHandle *pif);
static int flash_fwk_deinit(uint32 ClientHandle, DalDeviceHandle *pif);
static int flash_fwk_open(uint32 ClientHandle, DalDeviceHandle *pif, uint32 mode);
static int flash_fwk_close(uint32 ClientHandle, DalDeviceHandle *pif);
static int flash_fwk_get_daldevice_info(uint32 ClientHandle, DalDeviceHandle *pif,
 ...
 */
static int flash_fwk_handle_power_event(uint32 ClientHandle, DalDeviceHandle *pif,
 ...
 * of DAL framework.
 */
static int flash_fwk_handle_sys_request(uint32 ClientHandle, DalDeviceHandle *pif, DalSysReq req_idx,
