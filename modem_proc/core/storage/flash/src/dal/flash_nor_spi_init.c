 ...
 *   order.
 *
 *  $Header: //components/rel/core.mpss/10.0/storage/flash/src/dal/flash_nor_spi_init.c#1 $ $DateTime: 2019/04/24 00:03:26 $ $Author: pwbldsvc $
 *
 ...
 */
static int nor_spi_open_partition(flash_client_ctxt *client_ctxt,
  const unsigned char *partition_name)
{
  int    status = FLASH_DEVICE_DONE;

  FLASHLOG(4, ("nor_spi_open_partition called\n"));
  ...
  struct flash_nor_params *devices)
{
  ...
  dev_data->flags2 = devices->flags2;
...
{
  int result = FLASH_DEVICE_DONE;
  uint32 size_bytes = 0, temp_addr = 0;
  struct nor_spi_dev_pdata *nor_spi_pdata;
  ...
  temp_addr = FLASH_ALIGN_32(flash_mem_data->drv_mem_info.VirtualAddr);
  ...
  *devices)
{
  ...
  nor_spi_base_vtbl.open_partition = nor_spi_open_partition;
  dev_ctxt->dev_base_vtbl = &nor_spi_base_vtbl;
...
/*
 * Update the partition table to use 8 blocks(256KB) for SFS for 2MB SPI NOR
 */
void nor_spi_update_usr_parti_tbl(flash_client_ctxt *client_ctxt)
{
  /* For 2MegaByte SPI NOR, use 256KB for SFP partition which would equate
   * to 8 blocks of 32KB
   */	
  if ((client_ctxt->flash_dev_ctxt->dev_info.u.nor.device_size_MB == 2) &&
    (strncmp((const char *)nor_ram_table.part_entry[0].name, FLASH_NOR_PARTI_SFP, 
    FLASH_NOR_PARTI_NAME_LENGTH) == 0))
  {
    nor_ram_table.part_entry[0].img_size = 256; 	  
  }	  
}	
...
int nor_spi_probe(flash_client_ctxt *client_ctxt)
{
  ...
  flash_dev_ctxt *dev_ctxt = client_ctxt->flash_dev_ctxt;
  struct nor_dev_data *dev_data = &dev_ctxt->dev_info.u.nor;
