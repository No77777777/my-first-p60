 ...
 *   order.
 *
 *  $Header: //components/rel/core.mpss/10.0/storage/flash/src/dal/flash_nand_entry.c#1 $ $DateTime: 2019/04/24 00:03:26 $ $Author: pwbldsvc $
 *
 *
...
int nand_entry_device_probe(flash_client_ctxt *client_ctxt)
{
  ...
  else
  {
    /* Synchronize with other clients */
    DALSYS_SyncEnter(client_ctxt->flash_dev_ctxt->handle_dev_sync);
    flash_nand_clocks_init((uint32 *)client_ctxt);
    flash_nand_clocks_request();
