 ...
 *   order.
 *
 *  $Header: //components/rel/core.mpss/10.0/storage/flash/src/dal/flash_nor_partition.c#1 $ $DateTime: 2019/04/24 00:03:26 $ $Author: pwbldsvc $
 *
 *
  ...
  flash_nor_usr_partable_t parti1, flash_nor_partable_t parti2)
{
  ...
  for (i = 0; i < parti1->numparts; i++)
  {
    ...
    {
      (void)memcpy(parti2->part_entry[i].name, parti1->part_entry[i].name,
