...
/*===========================================================================
  ...
  $Header: //components/rel/core.mpss/10.0/securemsm/modem_sec/src/modem_rollback_version.c#2 $
  $DateTime: 2019/07/23 04:29:08 $
  $Author: pwbldsvc $


when       who      what, where, why
--------   ---      ------------------------------------
07/18/2019 mkallutl	    Updated Parameters in modem_rollback_blow_raw_fuse
  ...
  do
  {
    ...
    */    
      ...
      if(current_version < fuse_lsb_max)
      {
        ...
        version_to_blow[0] =  (0x2 << (current_version - 1))- 1;  
      }
      /* If current version is bigger or equals to max then we blow the max */
      else
      {
        version_to_blow[0] =  (0x2 << (fuse_lsb_max - 1))- 1;
      }
    ...
    */
    ...
    version_to_blow[0] = version_to_blow[0] << 
                          rollback_img_set->version_shift;
    
    /* Blow LSB Fuse */
    blow_success = modem_rollback_blow_raw_fuse(version_to_blow, 
...
)
{
  ...
  modem_img_table->current_version_num = (uint32)HWIO_MSS_RELAY_MSG_SHADOW5_IN;
  
  /* Get the version of the MBA Image from the RMB register */
  mba_img_table->current_version_num = (uint32)HWIO_MSS_RELAY_MSG_SHADOW1_IN;
