...
/*===========================================================================
  ...
  $Header: //components/rel/core.mpss/10.0/securemsm/inc/seccrypt_internal.h#1 $ 
...
/**
  Decrypts a message using the configured private ECIES key. 

  @param[in] handle                Handle to the crypto instance.
  @param[in] algo                  Algorithm.
  @param[in] cryptogram_ptr        Pointer to the cryptogram data.
  @param[in] cryptogram_len        Length of the cryptogram data buffer.
  @param[in] plaintext_ptr         Pointer to the plaintext data 
  @param[in,out] plaintext_len_ptr Pointer to the plaintext data length.
  @param[in,optional] aad          shared info pointers
  @param[out,optional] aad_len     pointer to shared info length. 
  ...
  none
  @Limittaion:
   As of now "shared information " is not implemented . they are reserbed for future use.
*/
secerrno_enum_type seccrypt_pkx_ecies_decrypt
(
seccrypt_handle_type          handle,
secapi_algo_enum_type         algo,
uint8*                        cryptogram_ptr,
uint32                        cryptogram_len,
uint8*                        plaintext_ptr,
uint32*                       plaintext_len_ptr,
uint8**                       aad,
uint32*                       aad_len
);
...
/**
  Encrypts a message using the configured public ECIES key. 

  @param[in] handle                 Handle to the crypto instance.
  @param[in] algo                   Algorithm.
  @param[in] plaintext_ptr          Pointer to the plaintext data
  @param[in] plaintext_len          plaintext data length.
  @param[in/out] cryptogram_ptr     Pointer to the cryptogram  data.
  @param[in/out] cryptogram_len_ptr Length of the cryptogram data 
  @param[in,optional] aad           shared info pointers
  @param[in,optional] aad_len       shared info length. 
  ...
  None.
   @Limittaion:
   As of now "shared information " i.e argument aad and aad_len is not implemented . they are reserved for future use
*/
secerrno_enum_type seccrypt_pkx_ecies_encrypt
(
seccrypt_handle_type          handle,
secapi_algo_enum_type         algo,
uint8*                        plaintext_ptr,
