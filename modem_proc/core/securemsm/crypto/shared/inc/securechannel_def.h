...
/*===========================================================================
  ...
  $Header: //components/rel/core.mpss/10.0/securemsm/crypto/shared/inc/securechannel_def.h#2 $ 
  $DateTime: 2024/07/24 10:53:11 $ 
  $Author: pwbldsvc $
...
typedef struct
{
  ...
  #else
  uint32 peer_sequence_num[SC_CID_COUNT];      /* Initial sequence number modem will use for sending messages */
  uint32 peer_local_sequence_num[SC_CID_COUNT];       /* sequence number used for checking reply attack */
  uint32 tz_sequence_num[SC_CID_COUNT]; /* Intial sequence number TZ will use for sending messages */
  uint32 tz_local_sequence_num[SC_CID_COUNT];         /* sequence number used for checking reply attack */
  #endif
