...
$Header: //components/rel/core.mpss/10.0/boot/qfprom/hw/core_2_0/qfprom_target.c#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
10/15/15   jz      Ported changes to support different fusemaps at runtime
11/03/14   jz      Ported various fixes
...
QFPROM_REGION_INFO *qfprom_region_ptr = NULL;
uint32 qfprom_region_size;
...
extern QFPROM_REGION_INFO qfprom_region_data[];
extern uint32 qfprom_region_data_size;
...
*
* @par Dependencies
*   qfprom_init_fusemap has been called to initialize qfprom_region_ptr
*
* @param:
...
uint32 qfprom_find_index(uint32 qfpromRegion)
{
    ...
    */
    for (index = 0; index < qfprom_region_size; index++)
    {
        if (qfprom_region_ptr[index].region == qfpromRegion)
        {
            return_value = index;
            break;
        }
...
)
{
    ...
    */
    for (index = 0; index < qfprom_region_size; index++)
    {
        row_index = qfprom_region_ptr[index].size;

        do
        {
            local_raw_row_address =  ( qfprom_region_ptr[index].raw_base_addr  + (row_index -1)*8 );
            local_corr_row_address =  ( qfprom_region_ptr[index].corr_base_addr + (row_index -1)*8 );
...
)
{
    ...
    for (index = 0; index < qfprom_region_size; index++)
    {
        if (qfprom_region_ptr[index].region == region)
        {
            ...
            **  1 - Disable  Read
            */
            if ((read_perm & qfprom_region_ptr[index].read_perm_mask) == 0x0)
            {
                status = TRUE;
            }
...
)
{
    ...
    for (index = 0; index < qfprom_region_size; index++)
    {
        if (qfprom_region_ptr[index].region == region)
        {
            ...
            **  1 - Disable  write
            */
            if ((write_perm & qfprom_region_ptr[index].write_perm_mask) == 0x0)
            {
                status = TRUE;
            }
 ...
 )
{
    ...
    */
    for (index = 0; index < qfprom_region_size; index++)
    {
        ...
        {
            region_start_row_addr = qfprom_region_ptr[index].raw_base_addr;
        }
        else if (addr_type == QFPROM_ADDR_SPACE_CORR)
        {
            region_start_row_addr = qfprom_region_ptr[index].corr_base_addr;
        }

        region_end_row_addr = ((region_start_row_addr) +
                                            ((qfprom_region_ptr[index].size) *8) );

        if (( address >=  region_start_row_addr) &&
            (address < region_end_row_addr ))
        {
            *region_type = qfprom_region_ptr[index].region;
            return_value = QFPROM_NO_ERR;
            break;
        }
    }
...
*
* @param:
*   uint32    region_type - name of the region.
...
)
{
    ...
    do
    {
        ...
        */
        for (index = 0; index < qfprom_region_size; index++)
        {
            if (qfprom_region_ptr[index].region == region_type)
            {
                if (qfprom_region_ptr[index].fec_type == QFPROM_FEC_63_56)
                {
		      ...
		      fec_en_lsb &= (1 << qfprom_region_ptr[index].qfprom_region_index);

                      *fec_status = QFPROM_IS_FEC_ENABLED(fec_en_lsb);
                    return_value = QFPROM_NO_ERR;
                }
                /*For regions that do no support FEC , no need for FEC action */
                else if (qfprom_region_ptr[index].fec_type == QFPROM_FEC_NONE)
                {
                    *fec_status = FALSE;
                    return_value = QFPROM_NO_ERR;
                }
  ...
  do
  {
    ...
    */
    for (index = 0; index < qfprom_region_size; index++)
    {
      ...
      {
         *region_index = qfprom_region_ptr[index].qfprom_region_index;
         return_value = QFPROM_NO_ERR;
         break;
      }
    }
...
)
{
  ...
  if (qfprom_region_ptr[qfprom_find_index(region_type)].read_allowed == TRUE)
    {
    /* Verify if the data was correctly written */
    if ( raw_data_to_write && read_row_data &&
         ( ((raw_data_to_write[0]) != (raw_data_to_write[0] & read_row_data[0])) ||
           ((raw_data_to_write[1]) != (raw_data_to_write[1] & read_row_data[1])) ) )
      {
      qfprom_err_status = QFPROM_WRITE_ERR;
      }
      }
...
{  
  ...
  if (qfprom_region_ptr)
  {
    return;
  }

  /* Initialize the qfprom_region_ptr and qfprom_region_size with target specific defines  */
  qfprom_region_ptr = (QFPROM_REGION_INFO *)qfprom_region_data;
  qfprom_region_size = qfprom_region_data_size;

  /* Call target specific function if update is needed */
  qfprom_update_fusemap(&fusemap_data);
...
uint32 qfprom_write_set_clock_dependent_settings( void )
{
    ...
    if(QFPROM_NO_ERR == ret_val)
    {
      ...
      */
      ...
      /*  Configue QFPROM_ACCEL (check .h for details) */
