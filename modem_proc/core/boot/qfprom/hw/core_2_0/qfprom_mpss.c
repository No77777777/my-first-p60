...
Copyright  2010-2015, 2017 by Qualcomm Technologies, Incorporated.  All Rights Reserved.
============================================================================*/
...
$Header: //components/rel/core.mpss/10.0/boot/qfprom/hw/core_2_0/qfprom_mpss.c#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
05/19/17   ds	   clock id change
03/26/14   kedara  Do not enable gcc_sec_ctrl_clk from qfprom_clock_enable
...
uint32 qfprom_clock_enable( void )
{
  ...
  do
  {
    ...
    {
      dwResult = DalClock_GetClockId(qfprom_clk_handle,"gcc_sec_ctrl_clk_src",
                                     &clock_Id);
      if (DAL_SUCCESS != dwResult)
      {
        break;
