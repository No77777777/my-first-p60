...
#
# Copyright (c) 2014,2016-2020 by Qualcomm Technologies, Incorporated.  All Rights Reserved.
...
sub read_records
{
    ...
    if ($array[$j] =~ /^(@.*)$/) {    
       ...
       $pool_item_count{hex($pool_val[2])} = $pool_val[3];
       if ($pool_val[1] eq "DUP") {
        $dup_pool=hex($pool_val[2]);
        }
       if ($pool_val[1] eq "SL") {
        $sl_pool=hex($pool_val[2]);
        }
       next;
    }
...
sub filestamp_update {
    ...
    if (!exists $pktptr_hash{$addr}) {
        ...
        my $child = $i->{"PKT_PTR"};
        my $sl_ref = 0;
        my $dup_pkt = $i->{"DUP_PTR"};
        
        if ($i->{"POOL"} == $dup_pool)
        {
          if (exists $addr_hash{$dup_pkt})
          {
            my $j = $addr_hash{$dup_pkt};
            if ( $j->{"POOL"} == $sl_pool){
              $sl_ref=$dup_pkt;
            }
          }          
        ...
        accumulate_stats ($i->{"POOL"}, $i->{"PKT_FILE"}, $i->{"PKT_LINE"}, $i->{"USED"}, $sl_ref);
...
sub propagate_fields {
    ...
    printf "ERROR: Referenced DSM item in allocated list below is freed/not found: 0x%08X\n", $addr;
    }
}
...
sub accumulate_stats
{
    ...
    if ($sl_ref !=0)    {
      $stats_hash {$pool}->{$file}->{$line} = [1, $used, [$sl_ref]];
    }
    else {
...
{
    my $i = 0;
    print "\n D S M MEMORY POOL USAGE SUMMARY \n";
    print "=" x 40;
    print "\nNote:  Items in a packet are updated with packet header's file stamp.\n";
    print "          All ALLOCATED DSM items are printed after summary.\n";
    print "-" x 80;
    ...
    printf("(MAX Items %d) : ",$pool_item_count{$pool});
    }
    printf("0x%08x",$pool);
    ...
    foreach my $file (keys %{$pool_hash}) {
        ...
        if ($Cnt ne 0){
          my %dupptr_hash=();
          foreach my $elem (@$SL_arr) {
          $dupptr_hash{$elem}=1;
          $total_dupptr_hash{$elem}=1;
          }
          $Cnt= keys %dupptr_hash;
          print "|| Total SL items held by DUP items: $Cnt \n";
          }
          else{  print " \n"; }
          $total_count += $$tuple[0];
          $total_used += $$tuple[1];
