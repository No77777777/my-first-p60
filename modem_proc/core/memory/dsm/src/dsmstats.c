...
/*===========================================================================
  ...
  $Header: //components/rel/core.mpss/10.0/memory/dsm/src/dsmstats.c#1 $
...
void dsmi_modem_get_statistics( dsm_modem_stats_info_s  *dsm_modem_stats_info)
{
	...
	dsm_modem_stats_info->config.sl_pool.augment_buffer_recycle_minimum_count =
                                   dsm_pool_stats_get_recycle_min_count(&dsm_ds_sl_item_pool);
    dsm_modem_stats_info->config.sl_pool.free_item_cnt_heap_trigger =
                                   dsm_pool_stats_get_ext_mem_start_alloc_free_item_trigger(&dsm_ds_sl_item_pool);
    dsm_modem_stats_info->config.sl_pool.max_heap_alloc_bytes =
                                   dsm_pool_stats_get_config_ext_mem_max_alloc_bytes(&dsm_ds_sl_item_pool);
    dsm_modem_stats_info->config.sl_pool.num_items_per_heap_alloc =
                                   dsm_pool_stats_get_augment_count(&dsm_ds_sl_item_pool);
    dsm_modem_stats_info->config.sl_pool.max_item_cnt =
                                   dsm_pool_stats_get_max_item_count(&dsm_ds_sl_item_pool);
...
#else
       dsm_modem_stats_info->stats.dup.min_free_cnt_cb_based =
				 dsm_dup_item_pool.stats_min_free_count;
       dsm_modem_stats_info->stats.dup.total_item_cnt_at_min_free_level_cb_based =
				 dsm_dup_item_pool.item_count;  
#endif
     }
...
#else
        dsm_modem_stats_info->stats.small.min_free_cnt_cb_based =
					 dsm_ds_small_item_pool.stats_min_free_count;
        dsm_modem_stats_info->stats.small.total_item_cnt_at_min_free_level_cb_based =
					 dsm_ds_small_item_pool.item_count;
#endif
      }
     
#ifdef FEATURE_DSM_SUPER_LARGE_ITEMS

    dsm_modem_stats_info->stats.sl.items_not_allocated =
                                 dsm_pool_stats_get_items_not_allocated(&dsm_ds_sl_item_pool);
    dsm_modem_stats_info->stats.sl.client_alloc_bytes =
                                dsm_pool_stats_get_client_alloc_bytes(&dsm_ds_sl_item_pool);
    dsm_modem_stats_info->stats.sl.heap_alloc_bytes =
                                dsm_pool_stats_get_ext_mem_alloc_bytes(&dsm_ds_sl_item_pool);
    dsm_modem_stats_info->stats.sl.max_heap_alloc_bytes =
                                dsm_pool_stats_get_ext_mem_max_alloc_bytes(&dsm_ds_sl_item_pool);
    dsm_modem_stats_info->stats.sl.available_free_cnt = DSM_POOL_FREE_CNT(&dsm_ds_sl_item_pool);

    dsm_modem_stats_info->stats.sl.item_alloc_fail_cnt = 0;
    dsm_modem_stats_info->stats.sl.heap_alloc_fail_cnt = 0;
    dsm_modem_stats_info->stats.sl.min_free_cnt_cb_based = DSM_MODEM_STATS_DEFAULT_RET_VALUE;
    dsm_modem_stats_info->stats.sl.total_item_cnt_at_min_free_level_cb_based = DSM_MODEM_STATS_DEFAULT_RET_VALUE;

    if (dsm_ds_sl_item_pool.pool_stats)      
    {     
      dsm_modem_stats_info->stats.sl.item_alloc_fail_cnt =
                        dsm_ds_sl_item_pool.pool_stats->stats_num_out_of_memory;
#ifdef FEATURE_DSM_AUGMENT  
      dsm_modem_stats_info->stats.sl.heap_alloc_fail_cnt =
                      dsm_ds_sl_item_pool.pool_stats->stats_ext_mem_alloc_fail_count;
      dsm_modem_stats_info->stats.sl.min_free_cnt_cb_based =
                      dsm_ds_sl_item_pool.pool_stats->stats_free_count_at_lowest_cb_level;
      dsm_modem_stats_info->stats.sl.total_item_cnt_at_min_free_level_cb_based =
                      dsm_ds_sl_item_pool.pool_stats->stats_item_count_at_lowest_cb_level;
#else
      dsm_modem_stats_info->stats.sl.min_free_cnt_cb_based =
				dsm_ds_sl_item_pool.stats_min_free_count;
      dsm_modem_stats_info->stats.sl.total_item_cnt_at_min_free_level_cb_based =
				dsm_ds_sl_item_pool.item_count;
#endif
    }
...
#else
       dsm_modem_stats_info->stats.large.min_free_cnt_cb_based =
				 dsm_ds_large_item_pool.stats_min_free_count;
       dsm_modem_stats_info->stats.large.total_item_cnt_at_min_free_level_cb_based =
				 dsm_ds_large_item_pool.item_count;
