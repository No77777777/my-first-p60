...
/*===========================================================================
  ...
  $Header: //components/rel/core.mpss/10.0/memory/dsm/src/dsm_init.c#3 $
...
#define DSMI_DS_SMALL_ITEM_LINK_LAYER_FEW    50
  ...
  #ifdef FEATURE_DSM_SUPER_LARGE_ITEMS
  /*---------------------------------------------------------------------------
    Size, Count, Few, many and do not exceed counts for DUP items 
  ---------------------------------------------------------------------------*/
    #ifndef DSMI_DS_SL_ITEM_SIZ
      #define DSMI_DS_SL_ITEM_SIZ               9216
    #endif
    #ifndef DSMI_DS_SL_ITEM_CNT
      #define DSMI_DS_SL_ITEM_CNT                 300
    #endif
    #ifndef DSMI_DS_SL_ITEM_DONT_EXCEED
    #define DSMI_DS_SL_ITEM_DONT_EXCEED           1 
	#endif
  #endif /* FEATURE_DSM_SUPER_LARGE_ITEMS */ 
...
#ifndef FEATURE_DSM_AUGMENT /* For off-target testing builds */
#undef DSMI_DS_SMALL_ITEM_CNT_FLC
#undef DSMI_DS_LARGE_ITEM_CNT_FLC
#undef DSMI_DS_SL_ITEM_CNT_FLC
#undef DSMI_DUP_ITEM_CNT_FLC
#endif
...
#else
#ifndef DSMI_DS_SMALL_ITEM_CNT_STATIC
#define DSMI_DS_SMALL_ITEM_CNT_STATIC DSMI_DS_SMALL_ITEM_CNT_FLC
#endif
#endif
...
#else
#ifndef DSMI_DS_LARGE_ITEM_CNT_STATIC
#define DSMI_DS_LARGE_ITEM_CNT_STATIC DSMI_DS_LARGE_ITEM_CNT_FLC
#endif
#endif
#endif /* FEATURE_DSM_LARGE_ITEMS */

#ifdef FEATURE_DSM_SUPER_LARGE_ITEMS
#ifndef DSMI_DS_SL_ITEM_AUGMENT_CNT
#define DSMI_DS_SL_ITEM_AUGMENT_CNT 25
#endif

#ifndef DSMI_DS_SL_ITEM_CNT_FLC
#ifndef DSMI_DS_SL_ITEM_CNT_STATIC
#define DSMI_DS_SL_ITEM_CNT_STATIC DSMI_DS_SL_ITEM_CNT
#endif
#define DSMI_DS_SL_ITEM_CNT_FLC (DSMI_DS_SL_ITEM_AUGMENT_CNT<<1)
#else
#ifndef DSMI_DS_SL_ITEM_CNT_STATIC
#define DSMI_DS_SL_ITEM_CNT_STATIC DSMI_DS_SL_ITEM_CNT_FLC
#endif
#endif
#endif /* FEATURE_DSM_SUPER_LARGE_ITEMS */
...
#else
#ifndef DSMI_DUP_ITEM_CNT_STATIC
#define DSMI_DUP_ITEM_CNT_STATIC DSMI_DUP_ITEM_CNT_FLC
#endif
#endif

#ifdef FEATURE_DSM_AUGMENT
#ifndef DSMI_COMMON_POOL_MEMORY 
#define DSMI_COMMON_POOL_MEMORY 0x100000
#endif
#ifndef DSMI_COMMON_POOL_MEMORY_RESERVED
  #define DSMI_COMMON_POOL_MEMORY_RESERVED 8
#endif
 static uint32 dsm_ds_common_pool_array[((DSMI_COMMON_POOL_MEMORY/4)+64)];
 static uint32 dsm_ds_common_pool_reserve_array[((DSMI_COMMON_POOL_MEMORY_RESERVED)/4)+64];
#endif
...
/*--------------------------------------------------------------------------
  Defining the static array that stores super large items.
--------------------------------------------------------------------------*/
#ifdef FEATURE_DSM_SUPER_LARGE_ITEMS
#define DSMI_DS_SL_ITEM_ARRAY_SIZ (DSMI_DS_SL_ITEM_CNT_STATIC * \
   (DSMI_DS_SL_ITEM_SIZ + DSM_ITEM_HEADER_SIZE + 31))
static uint32  dsm_ds_sl_item_array[DSMI_DS_SL_ITEM_ARRAY_SIZ/4];
dsm_pool_mgmt_table_type dsm_ds_sl_item_pool;
#endif /* #ifdef FEATURE_DSM_SUPER_LARGE_ITEMS */
...
#define DSM_EVENT_ENTRIES_BITS  10

#define DSM_EVENT_ENTRIES_MASK  ((1<<DSM_EVENT_ENTRIES_BITS)-1)
#define DSM_EVENT_ENTRIES_MAX   (1<<DSM_EVENT_ENTRIES_BITS) /**< No. of log entries */
...
typedef struct dsm_mem_event_debug_entry
{
...
dsm_mem_event_log_t dsm_debug_events[DSM_EVENT_ENTRIES_MAX];

unsigned int dsm_event_idx;
void dsm_debug_event_entry(dsm_mem_level_enum_type  level,uint32 count)
{

#ifndef TEST_FRAMEWORK
    static atomic_word_t dsm_event_atomic_idx = ATOMIC_INIT(0);
    unsigned int log_idx;
	log_idx = atomic_inc_return(&dsm_event_atomic_idx)& DSM_EVENT_ENTRIES_MASK ;
	dsm_event_idx = log_idx;
   	dsm_debug_events[dsm_event_idx].level = level;
	dsm_debug_events[dsm_event_idx].cb_count = count;
	dsm_debug_events[dsm_event_idx].time_ticks = get_qtimer_32();
	dsm_debug_events[dsm_event_idx].free_count = dsm_ds_sl_item_pool.free_count;
#endif	
}
/*---------------------------------------------------------------------------
...
#ifdef FEATURE_DSM_AUGMENT
void __attribute__((weak))dsm_use_reserved_memory(void)
{
  /* use it only for external builds
     */
  dsm_augment_add_common_pool_memory((uint8*)dsm_ds_common_pool_reserve_array,DSMI_COMMON_POOL_MEMORY_RESERVED);
};
#endif
...
static void dsm_rlc_calc_dsm_pool_count_triggers()
{
  ...
  dsmi_ds_large_item_rlc_few_mark = (2+ (((4 + rlc_rtt/2 + 10) * num_items_req_per_sec)/1000)) - 100;
#else
  dsmi_ds_large_item_rlc_few_mark = DSMI_DS_LARGE_ITEM_RLC_FEW_MARK;
#endif

#ifndef DSMI_DS_LARGE_ITEM_RLC_MANY_MARK
  dsmi_ds_large_item_rlc_many_mark =(3+ (((4 + rlc_rtt/2 + 10 + rlc_rtt) * num_items_req_per_sec)/1000))-300;
#else
...
void dsm_init(void)
{
 ...
 {
#ifdef FEATURE_DSM_AUGMENT
     /* The following line makes sure that dsm_ds_common_pool_array_reserved is not optimized by compiler */
     dsm_ds_common_pool_array[0]= dsm_ds_common_pool_reserve_array[0];
     dsm_augment_add_common_pool_memory((uint8*)dsm_ds_common_pool_array,DSMI_COMMON_POOL_MEMORY);
     dsm_use_reserved_memory();
#endif
    ...
    #ifdef FEATURE_DSM_SUPER_LARGE_ITEMS
    (void)dsm_init_pool_augment(DSM_DS_SL_ITEM_POOL,
                        (uint8*)dsm_ds_sl_item_array,
                        DSMI_DS_SL_ITEM_ARRAY_SIZ,
                        DSMI_DS_SL_ITEM_SIZ,
                        DSMI_DS_SL_ITEM_AUGMENT_CNT,
                        DSMI_DS_SL_ITEM_CNT-DSMI_DS_SL_ITEM_CNT_STATIC,
                        DSMI_DS_SL_ITEM_CNT_FLC);
    dsm_reg_mem_event_level(DSM_DS_SL_ITEM_POOL,
                            DSM_MEM_LEVEL_DNE,
                            DSMI_DS_SL_ITEM_DONT_EXCEED);   
    dsm_reg_mem_event_level(DSM_DS_SL_ITEM_POOL,
                            DSM_MEM_LEVEL_TRANSPORT_LAYER_FEW,
                            DSMI_DS_LARGE_ITEM_TRANSPORT_FEW);
    dsm_reg_mem_event_level(DSM_DS_SL_ITEM_POOL,
