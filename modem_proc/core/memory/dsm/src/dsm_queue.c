...
/*===========================================================================
  ...
  $Header: //components/rel/core.mpss/10.0/memory/dsm/src/dsm_queue.c#1 $
   ...
   while (chain_ptr !=NULL)
   {
      ...
      if (dsm_wm_get_debug_flag(wm_ptr,DSM_WM_DEBUG_VERIFY_MULTIPLE_ENQ_PKT_CHAIN_BIT2))
      {
        if ((chain_ptr->references == 0) ||
          (dsm_item_get_priority(chain_ptr) != dsm_item_get_priority(chain_head_ptr)) )
        {
                     ...
                     uint32 initial_count)
{
  ...
  if(initial_count == 0)
  {
     if ( wm_ptr->current_cnt > 0)
     {
       ...
       DSM_WM_CB(wm_ptr->non_empty_cb_ptrs[0],wm_ptr,wm_ptr->non_empty_cb_data[0]);
       DSM_WM_CB(wm_ptr->non_empty_cb_ptrs[1],wm_ptr,wm_ptr->non_empty_cb_data[1]);
     }          
  }    
...
)
{
  ...
  -------------------------------------------------------------------------*/
  pkt_len=dsm_item_update_pkt_len(*pkt_head_ptr);
    ...
    (wm_ptr->current_cnt < wm_ptr->dont_exceed_cnt)))
  {
    ...
    if (dsm_item_get_priority(*pkt_head_ptr) == DSM_QUEUE_PRIORITY_NORMAL)
    {
      /*-------------------------------------------------------- 
    ...
    {
      if(dsm_item_get_priority(*pkt_head_ptr) == DSM_QUEUE_PRIORITY_HIGH)
      {
        /*---------------------------------------------------
         ...
         wm_ptr->current_cnt_prio_high += pkt_len;      

      }  /* if (*pkt_head_ptr.........)*/
      else
      {
        if(dsm_item_get_priority(*pkt_head_ptr) == DSM_ENQ_HIGHEST_PRIORITY)
        {
         ...
         /*--------------------------------------------------------
           ...
           wm_ptr->current_cnt_prio_highest += pkt_len;
...
)
{
  ...
  dsm_item_set_pkt_len(*pkt_head_ptr,item_length);
...
DESCRIPTION
  ...
  Usage below.
    ...
    dsm_item_set_pkt_len(enq_pkts[0],20);
    total_pkt_len+=20;
    for (i = 1; i<5;i++){
...
)
{
        ...
        (wm_ptr->current_cnt < wm_ptr->dont_exceed_cnt)))
      {
        ...
        if (dsm_item_get_priority(*chain_head_ptr)== DSM_QUEUE_PRIORITY_NORMAL)
        {
          /*-------------------------------------------------------- 
               Put to queue at the tail.
          ----------------------------------------------------------*/
        ...
        {
          if(dsm_item_get_priority(*chain_head_ptr) == DSM_QUEUE_PRIORITY_HIGH)
          {
            /*---------------------------------------------------
                 The following functionality ensures that a message with dsm_highest
                 is not added to a queue before another message with dsm_highest 
             ...
             wm_ptr->current_cnt_prio_high += total_pkt_len_of_chain;
    
          }  /* if (*pkt_head_ptr.........)*/
          else
          {
            if(dsm_item_get_priority(*chain_head_ptr) == DSM_ENQ_HIGHEST_PRIORITY)
            {
             ...
             /*--------------------------------------------------------
               ...
               wm_ptr->current_cnt_prio_highest += total_pkt_len_of_chain;
      ...
      else
      {
        ...
        while (*chain_head_ptr !=NULL)
        {
          chain_iter =  (dsm_item_type*)q_chain_delink_s_i((q_link_type**)chain_head_ptr);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if((item_ptr = (dsm_item_type *)q_get_s_ll_i(wm_ptr->q_ptr)) != NULL)
  {
    ...
    /* Always believe the field in dequeue  as library ensures correct length is set during enqueue */
    ...
    if (wm_ptr->current_cnt_prio_highest)
    {
        wm_ptr->current_cnt_prio_highest -= item_length;
    }
    else if (wm_ptr->current_cnt_prio_high)
    {
        wm_ptr->current_cnt_prio_high -= item_length;
    }
    wm_ptr->current_cnt -= item_length;
      ...
      if (total_items > 0)
      {
        if (total_items <= num_pkts_to_dequeue)
        {
          *chain_head_ptr = (dsm_item_type *) wm_ptr->q_ptr->link.next_ptr;
          ...
          wm_ptr->current_cnt_prio_highest = 0;
          wm_ptr->current_cnt_prio_high = 0;
        }
        /* if number of packets to dequeue are less than the total queue count */
        else 
        {
          ...
          *chain_head_ptr = (dsm_item_type *) wm_ptr->q_ptr->link.next_ptr;
          ...
          if (wm_ptr->q_prio_insert_ptr != &wm_ptr->q_ptr->link)
          {
            if (dsm_item_get_priority((dsm_item_type*)(tail_dsm_ptr->link.next_ptr)) == DSM_QUEUE_PRIORITY_NORMAL )
            {
              /* This logic has to be changed , when other priorities are added to queue */
              wm_ptr->q_prio_insert_ptr = &wm_ptr->q_ptr->link;
            }
          }
          ...
          if ((wm_ptr->current_cnt_prio_high) || (wm_ptr->current_cnt_prio_highest))
          {
              if(item_length > wm_ptr->current_cnt_prio_highest)
              {
                  item_length2 = item_length-wm_ptr->current_cnt_prio_highest;
                  wm_ptr->current_cnt_prio_highest = 0;
                  if (item_length2 > wm_ptr->current_cnt_prio_high)
                  {
                      wm_ptr->current_cnt_prio_high = 0;
                  }
                  else
                  {
                    wm_ptr->current_cnt_prio_high-=item_length2;
                  }
              }
              else
              {
...
)
{
  ...
  dsmi_multiple_dequeue(wm_ptr,q_cnt_s(wm_ptr->q_ptr)+100,&chain_head_ptr,&deq_pkt_len,file,line);
  while (chain_head_ptr !=NULL)
  {
     delinked_item = (dsm_item_type*)q_chain_delink_s_i((q_link_type**)&chain_head_ptr);
     (void)dsmi_free_packet(&delinked_item,file,line); 
  }
} /* dsm_empty_queue() */
  ...
  void* client_data)
{
  ...
  {
    if (pkt_head_ptr->pkt_bit_field.dsm_bit_field.q_priority_bit == DSM_QUEUE_PRIORITY_NORMAL)
    {
...
)
{
  ...
  uint32 initial_count;          /* initial count of bytes in WM           */
  dsm_item_header_priority_type initial_priority;       /* initial priority of pkt_ptr */
  ...
  if(wm_ptr->q_ptr->cnt != 0)
  {
    ...
    while ((DSM_WM_ITERATOR_CONTINUE == iterator) && (pkt_ptr != NULL))
    {
      ...
      else /* modify or dequeue packet */
      {
         ...
         /*Always believe the field in dequeue as library sets it in enqueue */
         ...
         if(iter_req == DSM_WM_MODIFY_AND_DEQUEUE_PKT || iter_req == DSM_WM_MODIFY_PKT)
         {
            ...
            if (dsm_wm_iterator_modifier_cb)
            {
                ...
                initial_priority = dsm_item_get_priority(pkt_ptr);
         ...
         if ( iter_req == DSM_WM_MODIFY_AND_DEQUEUE_PKT || iter_req == DSM_WM_DEQUEUE_PKT)
         {
          ...
          if (dsm_item_get_priority(pkt_ptr) == DSM_ENQ_HIGHEST_PRIORITY)
          {
            wm_ptr->current_cnt_prio_highest -= item_length;
          }
          else if (dsm_item_get_priority(pkt_ptr) == DSM_QUEUE_PRIORITY_HIGH)
          {
            wm_ptr->current_cnt_prio_high -= item_length;
          }
          wm_ptr->current_cnt -= item_length;
...
)
{
  ...
  switch(dsm_wm_cb_type)
  {
    ...
    case DSM_WM_NON_EMPTY:
      ...
      /* change_val does not apply here */
      break;
   case DSM_WM_NON_EMPTY_PTRS_0:
      if (change_cb)
      {
        wm_ptr->non_empty_cb_ptrs[0]= cb;
      }
      if (change_cb_data)
      {
        wm_ptr->non_empty_cb_data[0]= cb_data;
      }   
      if(wm_ptr->current_cnt > 0)
      {
        DSM_WM_CB(wm_ptr->non_empty_cb_ptrs[0],wm_ptr,wm_ptr->non_empty_cb_data[0]);
      ...
      /* change_val does not apply here */
      break;
  case DSM_WM_NON_EMPTY_PTRS_1:
      if (change_cb)
      {
        wm_ptr->non_empty_cb_ptrs[1]= cb;
      }
      if (change_cb_data)
      {
        wm_ptr->non_empty_cb_data[1]= cb_data;
      }   
      if(wm_ptr->current_cnt > 0)
      {
        DSM_WM_CB(wm_ptr->non_empty_cb_ptrs[1],wm_ptr,wm_ptr->non_empty_cb_data[1]);
...
/*===========================================================================
FUNCTION DSM_QUEUE_CNT_FOR_PRIORITY_PKTS()

DESCRIPTION
 Returns the number of bytes on the watermark queue
  for packets with specific priority value.
...
PARAMETERS
 wm_ptr - Pointer to a watermark
 priority - priority of items, whose total count is returned

RETURN VALUE
 Number of bytes recorded for the items with input priority

SIDE EFFECTS
 None.
===========================================================================*/
uint32 dsm_queue_cnt_for_priority_pkts
(
  dsm_watermark_type *wm_ptr,
...
)
{
  ...
  DSMI_QUEUE_LOCK_WM(wm_ptr);
  switch (priority) {
  case DSM_QUEUE_PRIORITY_NORMAL:
    ret = wm_ptr->current_cnt - wm_ptr->current_cnt_prio_high -
       wm_ptr->current_cnt_prio_highest;
    break;
  case DSM_QUEUE_PRIORITY_HIGH:
    ret = wm_ptr->current_cnt_prio_high;
    break;
  case DSM_ENQ_HIGHEST_PRIORITY:
    ret = wm_ptr->current_cnt_prio_highest;
    break;
  }
  DSMI_QUEUE_UNLOCK_WM(wm_ptr);
  return ret;
