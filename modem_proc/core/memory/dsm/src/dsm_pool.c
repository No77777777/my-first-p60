...
/*===========================================================================
  ...
  $Header: //components/rel/core.mpss/10.0/memory/dsm/src/dsm_pool.c#1 $
...
#define DSMI_ADJUST_ITEM_SIZE(item_size)                         \
  ( ( item_size < (uint16)(~63) ) ? ((item_size + 63) & (~63)) : \
    ((uint16)(~63))\
  )
...
)
{
  ...
  /* Pad size is used to store time_stamp/usr_data for clients */
  /* Make sure there is atleast uint32 size pad size */
  ASSERT (table->pad_size >=4);
  ...
  */
  ...
  if (augment_items)
  {
    ...
    if (table->item_count < (augment_count>>1))
    {
    	static_augment_items+= table->item_count;
    	table->item_count=0;
    	aug_sbuffer = table->item_array;
    }
    else
    {
...
/*===========================================================================
FUNCTION DSM_INIT_POOL_DYNAMIC()

DESCRIPTION
  Initialize a single DSM Dynamic pool.
...
PARAMETERS
  pool_id - The identifier for the pool that should be initialized.
  item_size - The size of the items that this pool should provide
  item_count - Number of items of the size in the pool
  pool_trigger_free_cb - Client callback function called by DSM Library
     for the cleanup of buffers and releasing dynamic pool memory
     for resource crunch situations

RETURN VALUE
  Number of DSM items available in pool

SIDE EFFECTS
  None
===========================================================================*/
int dsmi_init_pool_dynamic
(
 dsm_mempool_id_type pool_id,
 uint16 item_size,
 uint32 item_count,
 pool_trigger_free_cb_type pool_trigger_free_cb,
 const char * file,
 uint32 line
)
{
  ...
  /* Item size must be evenly divisable by 32, round up */
  table->pool_item_size = DSMI_ADJUST_ITEM_SIZE(item_size);
  table->item_count_static = item_count;

  dsmi_init_pool_data(table,NULL,file,line);

  table->pad_size = (32 - ( ( sizeof(dsm_item_type) +
                                DSM_HEAD_COOKIE_LEN  +
                                DSM_TAIL_COOKIE_LEN ) & 31) ) & 31 ;
  DSM_LOCK_CREATE( &(table->lock) );
  dsmi_dynamic_pool_init(table);
  if (table->dynamic_pool !=NULL)
  {
    ((dsm_dynamic_pool_data_type*)table->dynamic_pool)->trigger_free_cb = pool_trigger_free_cb;
  }
  return 0;
}
...
/*===========================================================================
FUNCTION DSM_DYNAMIC_POOL_ALLOC_MEMORY()

DESCRIPTION
  This API is used to allocate memory for dynamic pool. Allocation also happens
  automatically when the first item is allocated.
...
The pool is locked when the above mentioned API's are called
===========================================================================*/
boolean dsm_dynamic_pool_alloc_memory(
  dsm_mempool_id_type pool_id
)
{
    dsm_pool_mgmt_table_type *table; /* temp table pointer */
    ...
    /* Get the table pointer to the pool structure */
    table = (dsm_pool_mgmt_table_type*)pool_id;
    if ((table->dynamic_pool !=NULL) && (table->item_count ==0))
    {
        ret = dsm_augment_alloc_dynamic_pool_memory(table);
    }
    return ret;
}

/*===========================================================================
FUNCTION DSM_DYNAMIC_POOL_FREE_MEMORY()

DESCRIPTION
  This API is used to free memory for dynamic pool. It does not matter if each individual
  dsm items are released.
...
RETURN VALUE
  True , if memory is released to heap and
   all clean up action from client is completed

SIDE EFFECTS
The pool is locked when the above mentioned API's are called
IMPORTANT: It does not matter if each individual
  dsm items are released.
===========================================================================*/
boolean dsm_dynamic_pool_free_memory
(
    dsm_mempool_id_type pool_id
)
{
    ...
    if ((table->dynamic_pool !=NULL) && (dynamic_pool->item_array[0]!=NULL))
    {
        if(!dynamic_pool->release_state)
        {
            dynamic_pool->release_state = TRUE;
        }
        if (dsmi_freeze_pool_allocation(table,FALSE))
        {
