...
$Header: //components/rel/core.mpss/10.0/debugtools/procinfo/src/procinfo_qdi.c#1 $ 
...
static int procinfo_qdi_open(int client_handle)
{
  ...
  strlcpy(dataobj->internal_name, pn.name, PROCINFO_NAME_MAX_LEN);
  dataobj->pid = pn.asid;
...
static int procinfo_qdi_get_name_internal(int client_handle, procinfo_qdi_device_p clntobj, uint32_t ntype, char *dest, size_t dest_len)
{
  ...
  {
    if (0 <= qurt_qdi_copy_to_user(client_handle, dest, src_type, PROCINFO_NAME_MAX_LEN))
    {
      ret = PROCINFO_QDI_SUCCESS;
    }
    else
...
/** =====================================================================
 * Function:
 *     procinfo_qdi_prepend_domain_internal
 *
 * Description:
 *     Prepends the DOMAIN name to the provided process name
 *
 * Parameters:
 *     name_type        : Value of name type from enum
 ...
 *    Refer to the enum PROCINFO_RESULT for a list of possible results
 * =====================================================================  */
static int procinfo_qdi_prepend_domain_internal(int client_handle, procinfo_qdi_device_p clntobj, uint32_t ntype, char *src)
{
  int ret = PROCINFO_QDI_FAILURE;
  
  procinfo_qdi_device_data_p dataobj;
  char *soc_domain_name = NULL;
  char *dest_type = NULL;
  char *name_to_append = NULL; 
  
  if (0 == strnlen(src, PROCINFO_NAME_MAX_LEN))
  {
    TMS_MSG_ERROR("Extending empty name"); 
    return ret;
  }
  ...
  name_to_append = calloc(sizeof(char), PROCINFO_NAME_MAX_LEN);
  if (NULL == name_to_append)
  {
    PROCINFO_QDI_MUTEX_UNLOCK();
    return ret;
  }

  /* Get SOC+DOMAIN name from Service Registry */
  soc_domain_name = servreg_utils_get_string(servreg_get_local_domain(), SOC+DOMAIN);
  if(SERVREG_NULL == soc_domain_name)
  {
    TMS_MSG_ERROR("Failed to get Service Registry SOC+DOMAIN");
    PROCINFO_QDI_MUTEX_UNLOCK();
    return ret;
  }
  ...
  if (TRUE != dataobj->islocal)
  {
    /* Copy from remote client */
    if (0 <= qurt_qdi_copy_from_user(client_handle, name_to_append, src, PROCINFO_NAME_MAX_LEN))
    {
      /* Explicitly set NULL terminator */
      name_to_append[PROCINFO_NAME_MAX_LEN - 1] = '\0';
      ret = PROCINFO_QDI_SUCCESS;
    }
    else
    {
  ...
  {
    /* Local copy */
    if (0 != strlcpy(name_to_append, src, PROCINFO_NAME_MAX_LEN))
    {
      ret = PROCINFO_QDI_SUCCESS;
    }
    else
  ...
  /* Create full SOC/DOMAIN/SUB_DOMAIN string with PD's desired name */
  strlcpy(dest_type, soc_domain_name, PROCINFO_NAME_MAX_LEN);
  strlcat(dest_type, "/", PROCINFO_NAME_MAX_LEN);
  strlcat(dest_type, name_to_append, PROCINFO_NAME_MAX_LEN);
...
static int procinfo_qdi_set_name_internal(int client_handle, procinfo_qdi_device_p clntobj, uint32_t ntype, const char *src)
{
  ...
  {  
    ...
    if (0 <= qurt_qdi_copy_from_user(client_handle, dest_type, src, PROCINFO_NAME_MAX_LEN))
    {
      /* Explicitly set NULL terminator */
      dest_type[PROCINFO_NAME_MAX_LEN - 1] = '\0';
      ret = PROCINFO_QDI_SUCCESS;
    }
    else
    {
  ...
  {
    /* Local copy */
    if (0 != strlcpy(dest_type, src, PROCINFO_NAME_MAX_LEN))
    {
      ret = PROCINFO_QDI_SUCCESS;
    }
    else
...
static int procinfo_qdi_op(int client_handle, procinfo_qdi_device_p clntobj, uint32_t optype, uint32_t ntype, char* buf, size_t buflen)
{
  ...
  switch (optype)
  {
    ...
    case PROCINFO_OP_SET_PREPEND_DOMAIN:
      {
        ret = procinfo_qdi_prepend_domain_internal(client_handle, clntobj, ntype, buf);
        break;
      }      
  }
