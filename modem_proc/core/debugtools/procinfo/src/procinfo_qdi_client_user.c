...
static unsigned int procinfo_qdi_init_flag = 0; /* Use for making re-entrant call to client_init */
...
DLL_API_GLOBAL PROCINFO_RESULT procinfo_qdi_invoke_op(uint32_t optype, uint32_t ntype, char *buf, size_t buflen)
{
  ...
  if ( buf == NULL || ntype >= PROCINFO_NAME_TYPES || optype >= PROCINFO_OP_TYPES )
    return PROCINFO_QDI_INVALID_PARAMETERS;
  
  ret = qurt_qdi_handle_invoke(procinfo_qdi_client_handle, PROCINFO_QDI_OP, optype, ntype, buf, buflen);

  /* Populate client internal copy */
  if ( PROCINFO_OP_SET_NAME == optype )
  {
   switch(ntype)
   {
     case PROCINFO_OEM_NAME_TYPE: 
     /* Populate OEM name */
       strlcpy(procinfo_client_entry.oem_name, buf, PROCINFO_NAME_MAX_LEN);
     break;

     case PROCINFO_DOMAIN_NAME_TYPE: 
     /* Populate Domain name */
       strlcpy(procinfo_client_entry.domain_name, buf, PROCINFO_NAME_MAX_LEN);
     break;

     case PROCINFO_INTERNAL_NAME_TYPE: 
     /* Set by default - do nothing */
     default:
     break;
    }
  }
  else if ( PROCINFO_OP_SET_PREPEND_DOMAIN == optype )
  {
   switch(ntype)
   {
     case PROCINFO_OEM_NAME_TYPE: 
      /* Populate updated OEM name */
       qurt_qdi_handle_invoke(procinfo_qdi_client_handle, PROCINFO_QDI_OP, PROCINFO_OP_GET_NAME, PROCINFO_OEM_NAME_TYPE, procinfo_client_entry.oem_name, PROCINFO_NAME_MAX_LEN);
     break;

     case PROCINFO_DOMAIN_NAME_TYPE: 
     /* Populate updated domain name */
       qurt_qdi_handle_invoke(procinfo_qdi_client_handle, PROCINFO_QDI_OP, PROCINFO_OP_GET_NAME, PROCINFO_DOMAIN_NAME_TYPE, procinfo_client_entry.domain_name, PROCINFO_NAME_MAX_LEN);
     break;
...
{
  if (qurt_atomic_compare_and_set(&procinfo_qdi_init_flag, FALSE, TRUE) )
  {
    ...
    if (0 <= procinfo_qdi_client_handle)
    {
      ...
      qurt_thread_context_get_pname(qurt_thread_get_id(), procinfo_client_entry.internal_name, PROCINFO_NAME_MAX_LEN);
    }
    else
    {
      ERR_FATAL("User procinfo_qdi_client_init failed", 0, 0, 0);
    }
  }
}