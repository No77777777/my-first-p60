...
$Header: //components/rel/core.mpss/10.0/debugtools/err/src/errlog.h#2 $
...
typedef struct
{
  ...
  err_crumb_trail_u                 crumb_trail_bmsk;
  unsigned int                      tid;
  unsigned int                      pd_id;
...
typedef struct {
  ...
  err_crumb_trail_u                 crumb_trail_bmsk;
  ERR_MUTEX_TYPE                    mutex;
  char                              aux_msg[ERR_LOG_MAX_MSG_LEN];
  unsigned int                      mutex_init;
  boolean                           reentrancy_flag;
