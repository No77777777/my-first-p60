...
$Header: //components/rel/core.mpss/10.0/debugtools/err/src/err_qdi.h#1 $ 
...
#define ERR_QDI_COMMUNICATE_TID            (4+QDI_PRIVATE) 
...
typedef struct err_qdi_internal_struct
{
  ...
  unsigned int            crash_tid;  /* Only updated if user Exception handler is launched */
  unsigned int            crash_fault_pc;
  unsigned int            crash_sp;
  unsigned int            crash_badva;
  unsigned int            crash_cause;
  unsigned int            crash_fp;
  unsigned int            crash_lr;
  unsigned int            crash_ssr;
  uint32                  pid;
