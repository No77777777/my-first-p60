...
$Header: //components/rel/core.mpss/10.0/debugtools/err/src/err_fatal.c#2 $
...
static inline void err_minimal_logging 
(
...
void err_fatal_lock(void)
{
  if(err_fatal_params.mutex_init==TRUE)
  {
      ...
      {
        err_fatal_params.reentrancy_flag = TRUE;
      }
    }
  else
  {
...
void err_raise_to_kernel(void)
{
  if ( qurt_island_get_status() == FALSE )
  {
    ...
    tms_get_timetick(&(coredump.err.end_time));
    /* Explicitly flushing. It is needed for some targets.*/
    ERR_FLUSH_ADDR(coredump.err.end_time);
  }
  else
    ERR_CRUMB_TRAIL_BMSK_ISLAND(ERR_CRUMB_TRAIL_BMSK_EMERGENCY_RECOVERY_PRE);
