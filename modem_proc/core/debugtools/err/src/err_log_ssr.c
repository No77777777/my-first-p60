...
$Header: //components/rel/core.mpss/10.0/debugtools/err/src/err_log_ssr.c#1 $
...
void err_log_ssr_init (void)
{
  ...
  /* SSR failure reason callback registration with error handling */
  ...
  if(ERRCB_E_SUCCESS != rv)
    TMS_MSG_ERROR_1("Registration of ssr failure reason cb failed, rv = %d", rv);
} /* err_log_ssr_init */
...
static void err_log_ssr_failure_reason(void)
{
  ...
  if (!err_sfr_locked)
  {
    ...
    if (sfr_buf_ptr && coredump.err.is_exception == TRUE)
    {
      strlcpy(sfr_buf_ptr, sfr_msg_ptr, ERR_SSR_REASON_SIZE_BYTES);
