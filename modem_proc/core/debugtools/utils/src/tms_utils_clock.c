...
/*
  ...
  $Header: //components/rel/core.mpss/10.0/debugtools/utils/src/tms_utils_clock.c#2 $
...
typedef struct
{
  ...
  npa_client_handle hNPA_VDD; // NPA client handle for Island vote

} tms_utils_vote_internal_t;

static volatile tms_utils_vote_internal_t tms_utils_vote_internal;
...
/** tms_utils_vote_vdd_cb
 * VDD resource availability callback.
 */
static void tms_utils_vote_vdd_cb( void *context, unsigned int event_type,
                                            void *data, unsigned int data_size)
{
  tms_utils_vote_internal.hNPA_VDD = 
    npa_create_sync_client( (const char *)data, "tmsVDDVote", 
                            NPA_CLIENT_REQUIRED);
  if (NULL == tms_utils_vote_internal.hNPA_VDD)
    TMS_MSG_ERROR("NPA client create for VDD failed.");
  else
    TMS_MSG_HIGH("NPA client create for VDD succeed.");
}

/** tms_utils_vote_init
 * Init api for voting related initialization.
 */
void tms_utils_vote_init( void )
{
  ...
  npa_resource_available_cb(SLEEP_CPUVDD_LPR_NODE_NAME, tms_utils_vote_vdd_cb, NULL);
}
 ...
 * VDD vote.
 */
static void tms_utils_vote_vdd( void )
{
  if(tms_utils_vote_internal.hNPA_VDD)
  {
    npa_issue_scalar_request(tms_utils_vote_internal.hNPA_VDD, SLEEP_CPUVDD_LPR_DISABLE_ALL);
    TMS_MSG_HIGH("VDD vote");
  }
  else
  {
    TMS_MSG_HIGH("VDD vote not done");
  }
}

/** tms_utils_unvote_vdd
 * VDD unvote.
 */
static void tms_utils_unvote_vdd( void )
{
  if(tms_utils_vote_internal.hNPA_VDD)
  {
    npa_complete_request(tms_utils_vote_internal.hNPA_VDD);
    TMS_MSG_HIGH("VDD unvote");
  }
  else
  {
    TMS_MSG_HIGH("VDD unvote not done");
  }
}

/** tms_utils_vote_resources
 * Single api for cpu & ddr & vdd vote.
 */
 ...
 * Single api for cpu & ddr & vdd unvote.
 */
