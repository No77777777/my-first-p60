...
/** =====================================================================
 * Function:
 *     servreg_qdi_invoke_deregister_remote_handle
 *
 * Description:
 *     Invoke the qdi method to de-register a client to a service in the user pd
 *
 * Parameters:
 *     sr_name          : user service name to be deregisterd against
 *
 * Returns:
 *    SERVREG_SUCCESS (0) or SERVREG_FAILURE(-1)
 * =====================================================================  */
SERVREG_RESULT servreg_qdi_invoke_deregister_remote_handle(SERVREG_NAME sr_name)
{
   int ret = -1;

   ret = qurt_qdi_handle_invoke(servnotif_qdi_client_handle, SERVREG_QDI_DEREGISTER_NEW_HANDLE, sr_name);
...
/** =====================================================================
 * Function:
 *     servreg_qdi_invoke_free_monitor_handle
 *
 * Description:
 *     Invoke the qdi method to free monitor handle of a client to a service in the user pd
 *
 * Parameters:
 *     sr_name          : user service name for which , want to free monitor
 *
 * Returns:
 *    SERVREG_SUCCESS (0) or SERVREG_FAILURE(-1)
 * =====================================================================  */
SERVREG_RESULT servreg_qdi_invoke_free_monitor_handle(SERVREG_NAME sr_name)
{
   int ret = -1;

   ret = qurt_qdi_handle_invoke(servnotif_qdi_client_handle, SERVREG_QDI_FREE_MONITOR_HANDLE, sr_name);
 ...
 *
 * Description:
 *     De-register client as a user listener because the service is not a local service.
 *
 * Parameters:
 *     service_name             : Service name that the client is interested in 
 *     sr_mon_handle            : Opaque handle to existing event
