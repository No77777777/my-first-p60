...
struct servreg_loc_internal_s
{
   ...
   qurt_mutex_t mutex; /* Generic mutex */
}; 
...
static servreg_dl_table_p servreg_dl_table_alloc(void)
{
   servreg_dl_table_p new_node = SERVREG_NULL;

   new_node = (servreg_dl_table_p)calloc(sizeof(servreg_dl_table_t), 1);

   if (SERVREG_NULL != new_node)
   {
      new_node->domain_list_signature = SERVREG_DL_SIGNATURE;
      new_node->domain_list_status = SERVREG_FAILURE;
   }
   else
   {
      ERR_FATAL( "servreg_dl_table_p calloc failed", 0, 0, 0);
   }
...
static void servreg_set_servreg_loc_availability(void)
{
   ...
   {
      if(0 < rcevt_getregistrants_handle(rcevt_handle))
      {
         /* Signal the listeners who have registered for servreg_loc availability */
         if(RCEVT_NULL == rcevt_signal_handle(rcevt_handle))
         {
...
SERVREG_DL_HANDLE servreg_alloc_domainlist_handle(void)
{
   ...
   qurt_pimutex_lock(&(servreg_loc_internal.mutex));

   /* Allocate new dl table */
   sr_dl_table = servreg_dl_table_alloc();
   ...
   qurt_pimutex_unlock(&(servreg_loc_internal.mutex));
   return dlhandle;
}
...
SERVREG_RESULT servreg_get_domainlist(SERVREG_NAME service_name, SERVREG_DL_HANDLE dlhandle)
{
  ...
  sr_dl_table->service_name = (SERVREG_NAME)calloc(sizeof(char), len1);
  ...
  servreg_loc_dl_resp = (qmi_servreg_loc_get_domain_list_resp_msg_v01*)calloc(sizeof(qmi_servreg_loc_get_domain_list_resp_msg_v01), 1);
  ...
  do
  {  
    SERVREG_TIME_TRACE_2(GET_DOMAINLIST_QMI_SEND, sr_dl_table, servreg_loc_internal.servreg_locator_client_handle);
    SERVREG_TRACE_TIME();
    ...
    {
      srdl_entry = (servreg_dl_entry_p)calloc(sizeof(servreg_dl_entry_t), 1);
      ...
      srdl_entry->domain_name = (SERVREG_NAME)calloc(sizeof(char), len2);
...
void servreg_record_pd_entry(SERVREG_NAME sr_pd_name)
{
   ...
   pd_entry = (servreg_pd_list_p)calloc(sizeof(servreg_pd_list_t), 1);
   ...
   if (SERVREG_NULL != sr_dl_table)
   {
      if(SERVREG_DL_SIGNATURE == sr_dl_table->domain_list_signature)
      {
         ...
         while(SERVREG_NULL != servreg_pd_list_curr)
         {
            ...
            sr_dl_entry = (servreg_dl_entry_p)calloc(sizeof(servreg_dl_entry_t), 1);
            ...
            if(SERVREG_NULL != sr_dl_entry)
            {
               ...
               sr_dl_entry->domain_name = (SERVREG_NAME)calloc(sizeof(char), len1);
               ...
               {
                  /* Free the space that was dynamically allocated for the service name */
                  free(sr_dl_table->service_name);
                  /* free domainlist table */
                  free(sr_dl_table);
   ...
   if(SERVREG_NULL != sr_dl_table)
   {
      if(SERVREG_DL_SIGNATURE == sr_dl_table->domain_list_signature)
      {
         while(SERVREG_NULL != sr_dl_table->domain_list_head)
         {
            ...
            free(srdl_entry->domain_name);
            /* free domainlist entry */
            free(srdl_entry);
            srdl_entry = SERVREG_NULL;
         }
...
{
   SERVREG_TRACE_2(LOCATOR_STATE_QUERY, qurt_thread_get_id(), servreg_loc_internal.servreg_loc_client_init_status);
...
void servreg_loc_send_service_list(void)
{
   ...
   uint32_t *service_count_total_p = SERVREG_NULL;
   uint32_t *instance_id_p = SERVREG_NULL;

   sr_local_domain = servreg_get_local_domain();

   /* Do heap allocation of the req message because the req packet is huge */
   servreg_loc_dl_req = (qmi_servreg_loc_register_service_list_req_msg_v01*)calloc(sizeof(qmi_servreg_loc_register_service_list_req_msg_v01), 1);
   ...
   if(SERVREG_NULL != servreg_loc_dl_req)
   {
      ...
      service_count_total_p = (uint32_t *)servreg_get_local_info(LOCAL_SERVICE_COUNT);
      if(SERVREG_NULL != service_count_total_p)
         service_count_total = *service_count_total_p;
      ...
      do
      {
         ...
         for(i = 0; i < service_list_len; i++)
         {
            ...
            instance_id_p = (uint32_t *)servreg_get_local_info(LOCAL_QMI_INSTANCE_ID);
            if(SERVREG_NULL != instance_id_p)
               servreg_loc_dl_req->service_list[i].instance_id = *instance_id_p;
...
void servreg_loc_send_pfr(char * pd_name, int pd_name_len, char * pfr, int pfr_len)
{
   ...
   servreg_loc_pfr_req = (qmi_servreg_loc_pfr_req_msg_v01*)calloc(sizeof(qmi_servreg_loc_pfr_req_msg_v01), 1);
   servreg_loc_pfr_resp = (qmi_servreg_loc_pfr_resp_msg_v01*)calloc(sizeof(qmi_servreg_loc_pfr_resp_msg_v01), 1);
   ...
   if(QMI_NO_ERR != qmi_client_return)
   {
      ...
      free(servreg_loc_pfr_resp);
   }
   
   /* Free request after sending QMI msg */
   free(servreg_loc_pfr_req);
}
 ...
 *     None
 * =====================================================================  */
static void servreg_loc_init_task(void *argv DLL_API_UNUSED)
{
   SERVREG_RESULT ret = SERVREG_FAILURE;
...
void servreg_locator_init(void)
{
   ...
   qurt_pimutex_init(&(servreg_loc_internal.mutex));
   ...
   /* Set name for procinfo */
   local_domain = servreg_get_local_domain();
   if (local_domain != SERVREG_NULL)
   {
      procinfo_qdi_invoke_op( PROCINFO_OP_SET_NAME, PROCINFO_DOMAIN_NAME_TYPE, local_domain, strnlen(local_domain, PROCINFO_NAME_MAX_LEN) );
   }
   ...
   /* Ensure no-timeout qmi connect is in it's own context */
   /* Qurt task to handle QMI re-try connection attempts to APSS Locator */
   (void)tms_utils_launch_qurt_task(SERVREG_QMI_LOC_CONNECT_TASK_NAME, TMS_TASK_STACK_2K, TMS_TASK_PRIO_DEFAULT, servreg_loc_init_task, NULL, TRUE);
