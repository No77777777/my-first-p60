...
struct servnotif_qdi_device_data_s
{
   ...
   SERVREG_QDI_NOTIF_HANDLE sr_qdi_notif_handle_dereg;
   SERVREG_NAME free_mon_handle_service_name;
   boolean notif_worker_started;
};
...
struct servnotif_qdi_device_s
{
   ...
   qurt_anysignal_t sync_mutex_obj;
   qurt_mutex_t sync_mutex;
...
static int servreg_qdi_deregister_handle(int client_handle, servnotif_qdi_device_p clientobj, SERVREG_NAME name);
static int servreg_qdi_free_monitor_handle(int client_handle, servnotif_qdi_device_p clientobj, SERVREG_NAME name);
...
extern SERVREG_RESULT servreg_set_state(SERVREG_MON_HANDLE sr_mon_handle, SERVREG_SERVICE_STATE state);
extern int32_t servreg_get_transaction_id_no_locks(SERVREG_MON_HANDLE sr_mon_handle);
...
static void servreg_qdi_release(qurt_qdi_obj_t* qdiobj)
{
   ...
   SERVREG_NAME process_name = SERVREG_NULL;
   ...
   if(SERVREG_SUCCESS == ret)
   {
      ...
      qurt_anysignal_destroy(&clientobj_del->sync_mutex_obj);
      qurt_mutex_destroy(&clientobj_del->sync_mutex);


      dataobj->servreg_qdi_notif_node_list_head = SERVREG_NULL;

      servnotif_cache_pd_data_node = (servnotif_cache_pd_data_p)calloc(sizeof(servnotif_cache_pd_data_t), 1);
      if(SERVREG_NULL != servnotif_cache_pd_data_node)
      {
         process_name = servreg_get_pd_name(dataobj->sr_mon_handle);
         if(SERVREG_NULL != process_name)
         {
            servnotif_cache_pd_data_node->sr_name = (SERVREG_NAME)calloc(sizeof(char), strnlen(process_name,SERVREG_NAME_MAX_LENGTH));
            if(SERVREG_NULL != servnotif_cache_pd_data_node->sr_name)
               strlcpy(servnotif_cache_pd_data_node->sr_name,process_name,SERVREG_NAME_MAX_LENGTH);
            free(process_name);
         }
...
static int servreg_qdi_open(int client_handle, SERVREG_NAME name, uint32_t name_len)
{
   ...
   clientobj = (servnotif_qdi_device_p)calloc(sizeof(servnotif_qdi_device_t), 1);
   ...
   qurt_anysignal_init(&clientobj->sync_mutex_obj);

   qurt_pimutex_init(&clientobj->sync_mutex);
...
/** =====================================================================
 * Function:
 *     servreg_send_early_pd_down_ind
 *
 * Description: 
 *     API to send early PD down  indications.
 *
 * Parameters:
 *     pid      : PID of that process which is down/crashed.
 *
 * Returns:
 *     SERVREG_RESULT
 * =====================================================================  */
SERVREG_RESULT servreg_send_early_pd_down_ind(uint32 pid)
{
  servnotif_qdi_device_p clientobj = servreg_qdi_root_internal.servreg_qdi_pd_head;
  servnotif_qdi_device_data_p dataobj = SERVREG_NULL;
  SERVREG_RESULT ret = SERVREG_FAILURE;
  SERVREG_QDI_NOTIF_HANDLE sr_qdi_notif_handle = SERVREG_NULL;
  SERVREG_MON_HANDLE sr_mon_handle = SERVREG_NULL;

  if ( FALSE == is_pd_dump_enabled() )
  {
    TMS_MSG_HIGH("PD dump disabled. Not sending EDOWN");
    return SERVREG_SUCCESS;
  }

  for (; SERVREG_NULL != clientobj; clientobj = clientobj->next)
  {
    dataobj = &(clientobj->servreg_user_pd_data);
    ...
    ret = servreg_set_state(dataobj->sr_mon_handle, SERVREG_SERVICE_STATE_EARLY_DOWN);
    if(SERVREG_SUCCESS == ret)
    {
      TMS_MSG_HIGH("User-PD state set to Early DOWN");
    }
    else
    {
      TMS_MSG_ERROR("Early DOWN Set state failed");
    }
    ...
    while(SERVREG_NULL != sr_qdi_notif_handle)
    {
      ...
      if(SERVREG_SUCCESS != servreg_set_state(sr_mon_handle, SERVREG_SERVICE_STATE_EARLY_DOWN))
      {
        TMS_MSG_ERROR_1("Set state failed 0x%x",sr_mon_handle);
      }
...
static int servreg_qdi_worker_wait(int client_handle, servnotif_qdi_device_p clientobj, uint32_t * remote_mask)
{
   ...
   dataobj = &(clientobj->servreg_user_pd_data);
   qurt_pimutex_lock(&servreg_qdi_root_internal.servnotif_qdi_mutex);
   dataobj->notif_worker_started = SERVREG_TRUE;
   qurt_pimutex_unlock(&servreg_qdi_root_internal.servnotif_qdi_mutex);
   /* Wait till you get notified of any service state change */
   if ( QURT_ECANCEL == qurt_signal_wait_cancellable(&dataobj->sig_t, SERVREG_QDI_REG_USER_MASK_VALUE | SERVREG_QDI_REG_ROOT_MASK_VALUE | SERVREG_QDI_DEREG_ROOT_MASK_VALUE | SERVREG_QDI_FREE_MON_HANDLE_MASK_VALUE | SERVREG_QDI_ACK_ROOT_MASK_VALUE, QURT_SIGNAL_ATTR_WAIT_ANY, (unsigned int *)&return_mask) )
   {
     qurt_pimutex_lock(&servreg_qdi_root_internal.servnotif_qdi_mutex);
     dataobj->notif_worker_started = SERVREG_FALSE;
     qurt_pimutex_unlock(&servreg_qdi_root_internal.servnotif_qdi_mutex);
     /* setting signal as well , in case some thread is waiting for it */
     qurt_anysignal_set(&clientobj->sync_mutex_obj, SERVREG_QDI_ROOT_TO_USER_PD_SYNC_MUTEX_SIG);
   }
...
static int servreg_qdi_get_ack_handle(int client_handle, servnotif_qdi_device_p clientobj, SERVREG_NAME remote_name, int32_t *remote_transaction_id)
{
   ...
   if(SERVREG_NULL != sr_qdi_notif_handle)
   {
      ...
      {
         sr_len = strnlen(sr_name,SERVREG_NAME_MAX_LENGTH);
         ret = qurt_qdi_copy_to_user(client_handle, remote_name, sr_name, sizeof(char) * sr_len);
         if (-1 != ret)
         {
            /* Send the transaction id of the remote handle to user space */
            transaction_id = servreg_get_transaction_id_no_locks(sr_mon_handle);
            ret = qurt_qdi_copy_to_user(client_handle, remote_transaction_id, &transaction_id, sizeof(uint32_t));
...
static int servreg_qdi_register_handle(int client_handle, servnotif_qdi_device_p clientobj, uint32_t * remote_addr, SERVREG_NAME name)
{
   ...
   if(SERVREG_NULL != sr_qdi_notif_handle)
   {
      ...
      if (-1 != ret)
      {
         ...
         sr_mon_handle = servreg_notif_get_mon_handle(sr_qdi_notif_handle);
         SERVREG_TRACE_2(QDI_REG_HNDL, sr_mon_handle, sr_qdi_notif_handle);

         /*since we know for sure that the client who is registering 
         with the user PD service in root PD have held 
         the sr_mon_handle mutex so to avoid the deadlock access name directly */
         sr_mon_node = (servreg_mon_node_p)(sr_mon_handle);
         if(SERVREG_NULL != sr_mon_node)
         {
            sr_name = sr_mon_node->service_name;
            if(SERVREG_NULL != sr_name)
            {
               sr_len = strnlen(sr_name,SERVREG_NAME_MAX_LENGTH);
   ...
   /*Signal Callee to Pick Up Return Value and Unlock sync_mutex  */
   qurt_anysignal_set(&clientobj->sync_mutex_obj, SERVREG_QDI_ROOT_TO_USER_PD_SYNC_MUTEX_SIG);
...
/** =====================================================================
 * Function:
 *     servreg_qdi_deregister_handle
 *
 * Description: 
 *     Function de-registers the root handle in the user pd
 *
 * Parameters:
 *     client_handle  : QDI handle which represents the client
 *                      which made this QDI request
 *     clientobj      : QDI object which represents the client
 *                      which made this QDI request.
 *     name           : name of the service sent to the user pd to deregister
 *
 * Returns:
 *     SERVREG_SUCCESS (0) or SERVREG_FAILURE(-1)
 * =====================================================================  */
static int servreg_qdi_deregister_handle(int client_handle, servnotif_qdi_device_p clientobj, SERVREG_NAME name)
{
   ...
   while(SERVREG_NULL != sr_qdi_notif_handle)
   {
     if(dataobj->sr_qdi_notif_handle_dereg == sr_qdi_notif_handle)
     {
        sr_mon_handle = servreg_notif_get_mon_handle(sr_qdi_notif_handle);
        SERVREG_TRACE_2(QDI_DEREG_HNDL, sr_mon_handle, sr_qdi_notif_handle);
        /*since we know for sure that the client who is de-registering 
        with the user PD service in root PD have held 
        the sr_mon_handle mutex so to avoid the deadlock access name directly */
        sr_mon_node = (servreg_mon_node_p)(sr_mon_handle);
        if(SERVREG_NULL != sr_mon_node)
        ...
        if(SERVREG_SUCCESS != servreg_delete_qdi_notif_node(sr_qdi_notif_handle, &(dataobj->servreg_qdi_notif_node_register_head)))
           TMS_MSG_HIGH_1("Failed to remove qdi_notif_node  0x%x", sr_qdi_notif_handle);
...
/** =====================================================================
 * Function:
 *     servreg_qdi_free_monitor_handle
 *
 * Description: 
 *     Function free the monitor handle in the user pd
 *
 * Parameters:
 *     client_handle  : QDI handle which represents the client
 *                      which made this QDI request
 *     clientobj      : QDI object which represents the client
 *                      which made this QDI request.
 *     name           : name of the service sent to the user pd to free monitor handle
 *
 * Returns:
 *     SERVREG_SUCCESS (0) or SERVREG_FAILURE(-1)
 * =====================================================================  */
static int servreg_qdi_free_monitor_handle(int client_handle, servnotif_qdi_device_p clientobj, SERVREG_NAME name)
{
   ...
   sr_len = strnlen(dataobj->free_mon_handle_service_name,SERVREG_NAME_MAX_LENGTH);

   ret = qurt_qdi_copy_to_user(client_handle, name, dataobj->free_mon_handle_service_name, sr_len);
   ...
   while(SERVREG_NULL != servnotif_cache_pd_data_node)
   {
      ...
      if(strncmp(servnotif_cache_pd_data_node->sr_name, sr_name, sr_len) == 0)
      {
         ...
         if(SERVREG_NULL == servnotif_cache_pd_data_node->servreg_qdi_notif_node_register_head)
         {
            ...
            servnotif_cache_pd_data_node->next = SERVREG_NULL;
            free(servnotif_cache_pd_data_node->sr_name);
            free(servnotif_cache_pd_data_node);
            servnotif_cache_pd_data_node = SERVREG_NULL;
         }
         break;
      }
...
SERVREG_RESULT servreg_register_user_listener(SERVREG_NAME service_name, SERVREG_MON_HANDLE sr_mon_handle, uint32_t * sr_curr_remote_state, int32_t * sr_remote_transaction_id)
{
   ...
   while(SERVREG_NULL != servreg_qdi_pd_node)
   {
      ...
      {
         free(pd_name);
         if ( SERVREG_TRUE == servreg_qdi_pd_node->servreg_user_pd_data.notif_worker_started )
         {
            qurt_pimutex_lock(&servreg_qdi_pd_node->sync_mutex); /*this mutex will be released once the servreg_qdi_notif_node_register_head info is captured in servreg_qdi_register_handle() */
            ...
            if(SERVREG_NULL != sr_qdi_notif_handle)
            {
               ...
               qurt_anysignal_clear(&servreg_qdi_pd_node->sync_mutex_obj, SERVREG_QDI_ROOT_TO_USER_PD_SYNC_MUTEX_SIG);
               ...
               *sr_remote_transaction_id = servreg_notif_get_transaction_id(sr_qdi_notif_handle);
               ...
               qurt_pimutex_unlock(&servreg_qdi_root_internal.servnotif_qdi_mutex);/*release main handle mutex so that if another PD registration can be performed*/

               qurt_anysignal_wait(&servreg_qdi_pd_node->sync_mutex_obj, SERVREG_QDI_ROOT_TO_USER_PD_SYNC_MUTEX_SIG);

               qurt_pimutex_unlock(&servreg_qdi_pd_node->sync_mutex);

               return SERVREG_SUCCESS;
            }
            else
            {
               TMS_MSG_ERROR_1("Could not get sr_qdi_notif_handle for %d", sr_mon_handle);
               qurt_pimutex_unlock(&servreg_qdi_pd_node->sync_mutex);
               qurt_pimutex_unlock(&servreg_qdi_root_internal.servnotif_qdi_mutex);
   ...
   if(SERVREG_NULL == servreg_qdi_pd_node || SERVREG_FALSE == servreg_qdi_pd_node->servreg_user_pd_data.notif_worker_started)
   {
      ...
      if(SERVREG_NULL != servnotif_cache_pd_data_node)
      {
         process_name = servreg_get_pd_name(sr_mon_handle);
         if(SERVREG_NULL != process_name)
         {
            servnotif_cache_pd_data_node->sr_name = (SERVREG_NAME)calloc(sizeof(char), strnlen(process_name,SERVREG_NAME_MAX_LENGTH));
            if(SERVREG_NULL != servnotif_cache_pd_data_node->sr_name)
         ...
         /* Now save the registration info in servnotif_cache_pd_data_head */
         sr_qdi_notif_handle = servreg_create_qdi_notif_node(sr_mon_handle, &(servnotif_cache_pd_data_node->servreg_qdi_notif_node_register_head));
...
/** =====================================================================
 * Function:
 *     servreg_deregister_user_listener
 *
 * Description:
 *     Service level de-registration. Root client de-registering for a service in the user pd
 *
 * Parameters:
 *     service_name             : Service name that the client is interested in 
 *     sr_mon_handle            : Opaque handle to existing event
 ...
 *     Refer to the enum SERVREG_RESULT for list of possible results
 * =====================================================================  */
SERVREG_RESULT servreg_deregister_user_listener(SERVREG_NAME service_name, SERVREG_MON_HANDLE sr_mon_handle)
{
   SERVREG_QDI_NOTIF_HANDLE sr_qdi_notif_handle = SERVREG_NULL;
   servnotif_qdi_device_p servreg_qdi_pd_node = SERVREG_NULL;
   SERVREG_NAME pd_name = SERVREG_NULL;
      ...
      if(SERVREG_NULL != pd_name && SERVREG_NULL != strstr(service_name , pd_name))
      {
         ...
         if ( SERVREG_TRUE == servreg_qdi_pd_node->servreg_user_pd_data.notif_worker_started )
         {
            ...
            if(SERVREG_NULL != sr_qdi_notif_handle)
            {
              ...
              servreg_qdi_pd_node->servreg_user_pd_data.sr_qdi_notif_handle_dereg = sr_qdi_notif_handle;
              qurt_anysignal_set(&servreg_qdi_pd_node->servreg_user_pd_data.sig_t, SERVREG_QDI_DEREG_ROOT_MASK_VALUE);
              qurt_pimutex_unlock(&servreg_qdi_root_internal.servnotif_qdi_mutex);
   ...
   /* If the PD is not up yet, remove the registration info in servnotif_cache_pd_data_head , if saved*/
   if(SERVREG_NULL == servreg_qdi_pd_node || SERVREG_FALSE == servreg_qdi_pd_node->servreg_user_pd_data.notif_worker_started)
   {
      SERVREG_TRACE_0(DEREG_NULL_QDI_PD_NODE);
      ...
      while(SERVREG_NULL != servnotif_cache_pd_data_node)
      {
         ...
         if(SERVREG_NULL != strstr(service_name , pd_name))
         {
            sr_qdi_notif_handle = servreg_get_qdi_notif_node(sr_mon_handle, &(servnotif_cache_pd_data_node->servreg_qdi_notif_node_register_head));

            if(SERVREG_NULL != sr_qdi_notif_handle)
            {
               if(SERVREG_SUCCESS == servreg_delete_qdi_notif_node(sr_qdi_notif_handle, &(servnotif_cache_pd_data_node->servreg_qdi_notif_node_register_head)))
               {
                  TMS_MSG_HIGH_1("Registration info removed for %d", sr_mon_handle);
                  qurt_pimutex_unlock(&servreg_qdi_root_internal.servnotif_qdi_mutex);
                  return SERVREG_SUCCESS;
               }
               else
               {
                  TMS_MSG_HIGH_1("Failed to remove registration info for %d", sr_mon_handle);
                  qurt_pimutex_unlock(&servreg_qdi_root_internal.servnotif_qdi_mutex);
                  return SERVREG_FAILURE;
               }
            }
   ...
   qurt_pimutex_unlock(&servreg_qdi_root_internal.servnotif_qdi_mutex);
   /* if we reach this point , it indicates deregister_listener was called before register_listener so return Failure*/
   return SERVREG_FAILURE;
}

/** =====================================================================
 * Function:
 *     servreg_free_user_handle
 *
 * Description:
 *     Service level de-registration. Root client freeing handle for a service in the user pd
 *
 * Parameters:
 *     service_name             : Service name that the client is interested in 
 *     sr_mon_handle            : Opaque handle to existing event
 ...
 *     Refer to the enum SERVREG_RESULT for list of possible results
 * =====================================================================  */
SERVREG_RESULT servreg_free_user_handle(SERVREG_NAME service_name, SERVREG_MON_HANDLE sr_mon_handle)
{
   servnotif_qdi_device_p servreg_qdi_pd_node = SERVREG_NULL;
   SERVREG_NAME pd_name = SERVREG_NULL;
   ...
   /* It must exist in servreg_qdi_pd_head, if it does not exist that means we did not make
      a connection to user PD yet so we don't have to do anything, simple return success */
   while(SERVREG_NULL != servreg_qdi_pd_node)
   {
      pd_name = servreg_get_pd_name(servreg_qdi_pd_node->servreg_user_pd_data.sr_mon_handle);
      ...
      if(SERVREG_NULL != pd_name && SERVREG_NULL != strstr(service_name , pd_name))
      {
         ...
         servreg_qdi_pd_node->servreg_user_pd_data.free_mon_handle_service_name = service_name;
         qurt_anysignal_set(&servreg_qdi_pd_node->servreg_user_pd_data.sig_t, SERVREG_QDI_FREE_MON_HANDLE_MASK_VALUE);
         break;
      }
                      ...
                      )
{
   ...
   switch (servnotif_qdi_method)
   {
      ...
      case QDI_CLOSE:
      {
         ...
         {
            TMS_MSG_ERROR("QDI close failed");
         }
      ...
      /* API, servreg_qdi_invoke_deregister_remote_handle */
      case SERVREG_QDI_DEREGISTER_NEW_HANDLE:
      {
         ret = servreg_qdi_deregister_handle(client_handle, clientobj, a1.ptr);
         break;
      }

      /* API, servreg_qdi_invoke_free_monitor_handle */
      case SERVREG_QDI_FREE_MONITOR_HANDLE:
      {
         ret = servreg_qdi_free_monitor_handle(client_handle, clientobj, a1.ptr);
         break;
      }
