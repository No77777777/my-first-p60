...
struct sr_qmi_s_proxy_s
{
   ...
   int32_t e_d_transaction_id; // cached transaction_id of the last/active transaction
...
extern void err_qdi_invoke_user_process_cbs_per_bucket(err_cb_bucket_t bucket, uint32 pid);
extern int32_t servreg_get_e_d_transaction_id(SERVREG_MON_HANDLE sr_mon_handle);
extern SERVREG_RESULT servreg_send_early_pd_down_ind(uint32 pid);
/** =====================================================================
 * Function:
 *     sr_qmi_s_proxy_alloc
 *
 * Description:
 *     Allocates QMI Proxy node
 *
 * Parameters:
 *     None
 *
 * Returns:
 *    Dynamically allocated QMI Proxy node
 * =====================================================================  */
static sr_qmi_s_proxy_p sr_qmi_s_proxy_alloc(void)
{
   sr_qmi_s_proxy_p new_node = SERVREG_NULL;
   new_node = (sr_qmi_s_proxy_p)calloc(sizeof(sr_qmi_s_proxy_t),1);
   
   if ( SERVREG_NULL != new_node ) {
     new_node->notif_signature = SERVREG_QMI_NOTIF_SERVER_SIGNATURE;
 ...
 *
 * Description:
 *     Initialization of the internal mutex
 *
 * Parameters:
 *     None
 *
...
{
   qurt_pimutex_init(&servreg_qmi_server_internal.mutex);
}
...
static sr_qmi_s_proxy_p sr_qmi_get_proxy(SERVREG_MON_HANDLE sr_mon_handle)
{
   ...
   qurt_pimutex_lock(&servreg_qmi_server_internal.mutex);
   ...
   qurt_pimutex_unlock(&servreg_qmi_server_internal.mutex);
...
/** =====================================================================
 * Function:
 *     servreg_get_service_curr_state_from_proxy
 *
 * Description:
 *     Get the current state of a service from proxy node
 *
 * Parameters:
 *     sr_qmi_s_proxy : proxy node of service
 *
 * Returns:
 *     SERVREG_SERVICE_STATE : Returns the state of the service.
 *                             Refer to the enum SERVREG_SERVICE_STATE for list of states
 * =====================================================================  */
SERVREG_SERVICE_STATE servreg_get_service_curr_state_from_proxy(sr_qmi_s_proxy_p sr_qmi_s_proxy)
{
   SERVREG_SERVICE_STATE ret = SERVREG_SERVICE_STATE_UNINIT;

   if(SERVREG_NULL == sr_qmi_s_proxy)
   {
      TMS_MSG_ERROR("sr_qmi_s_proxy is NULL");
      return ret;
   }

   /* Not taking lock here to minimize Race condition window.
      Race condition may happen between this API ( called in QMI client registration path)
      and servreg_send_qmi_ind_msg_state_change() ( called in set state path).*/
   //qurt_pimutex_lock(&servreg_qmi_server_internal.mutex);

   ret = (SERVREG_SERVICE_STATE)sr_qmi_s_proxy->curr_state;
...
static sr_qmi_s_proxy_p sr_qmi_s_listener_attach(SERVREG_MON_HANDLE sr_mon_handle, qmi_client_handle client_handle)
{
   ...
   if(SERVREG_NULL == sr_qmi_s_proxy)
   {
      sr_qmi_s_proxy = sr_qmi_s_proxy_alloc();
      ...
      if(SERVREG_NULL != sr_qmi_s_proxy)
      {
         ...
         /* qmi entry */
         sr_qmi_s_client = (sr_qmi_s_client_p)calloc(sizeof(sr_qmi_s_client_t),1);
            ...
            if(sr_qmi_s_proxy_curr == sr_qmi_s_proxy_del)
            {
               ...
               while(SERVREG_NULL != sr_qmi_s_client_curr)
               {
                  if(client_handle_del == sr_qmi_s_client_curr->client_handle)
                  {
                     ...
                     free(sr_qmi_s_client_curr);
                     sr_qmi_s_client_curr = SERVREG_NULL;
                     ret = SERVREG_SUCCESS;
               ...
               if(SERVREG_NULL == sr_qmi_s_proxy_del->qmi_client_list_head)
               {
                 ...
                 free(sr_qmi_s_proxy_del);
                 sr_qmi_s_proxy_del = SERVREG_NULL;
                 ret = SERVREG_SUCCESS;
               }
   ...
   while(SERVREG_NULL != sr_qmi_s_proxy)
   {
      ...
      if(sr_qmi_s_proxy->curr_state != new_curr_state)
      {
         ...
         if(SERVREG_NULL != service_name)
         {
            ...
            sr_qmi_s_proxy->e_d_transaction_id = servreg_get_e_d_transaction_id(sr_qmi_s_proxy->sr_mon_handle);

            /* Update the last_ack_id if it is greater that transaction_id .
               This case may arise  when service is in User PD and
               listener is present in ROOT PD or remote subsystem */
            if(sr_qmi_s_proxy->last_ack_id >= sr_qmi_s_proxy->transaction_id)
               sr_qmi_s_proxy->last_ack_id = sr_qmi_s_proxy->transaction_id - 1;
            ...
            while(SERVREG_NULL != sr_qmi_s_client)
            {
               ...
               if(sr_qmi_s_client->last_ack_id >= sr_qmi_s_client->transaction_id)
                  sr_qmi_s_client->last_ack_id = sr_qmi_s_client->transaction_id - 1;
                                          ...
                                          void           *service_cookie)
{
   ...
   SERVREG_NAME local_domain = SERVREG_NULL;
   ...
   if(NULL != req_handle)
   {
      if(SERVREG_TRUE == servreg_qmi_validate_connection_handle(connection_handle) && SERVREG_TRUE == client->connected)
      {
         ...
         SERVREG_TIME_TRACE_2(QMI_MSG, client->client_handle, msg_id);
         SERVREG_TRACE_TIME();
         switch(msg_id)
         {
            case SERVREG_NOTIF_REGISTER_LISTENER_REQ:
            {
               ...
               service_domain = servreg_utils_get_string(request->service_name,DOMAIN);
               local_domain = (SERVREG_NAME)servreg_get_local_info(LOCAL_DOMAIN);
               if(SERVREG_NULL != service_domain && SERVREG_NULL != local_domain)
               {
                  if(0 == strncmp(service_domain,local_domain,SERVREG_NAME_MAX_LENGTH))
                  {
                     /* Register Listener */
                     if(SERVREG_TRUE == request->enable)
                     {
                        ...
                        sr_qmi_s_proxy = sr_qmi_get_proxy_iff_listener(sr_mon_handle, client->client_handle, SERVREG_NULL);
                        if(SERVREG_NULL != sr_qmi_s_proxy)
                        {
                           ...
                           resp_send.curr_state = (qmi_servreg_notif_service_state_enum_type_v01)servreg_get_service_curr_state_from_proxy(sr_qmi_s_proxy);
                        ...
                        if(SERVREG_SUCCESS == ret)
                        {
                           ...
                           if(SERVREG_NULL != sr_qmi_s_proxy)
                           {
                              ...
                              resp_send.resp.result = QMI_RESULT_SUCCESS_V01;
                              resp_send.resp.error  = QMI_ERR_NONE_V01;
                              TMS_MSG_HIGH_2("QMI Listener from 0x%x added: 0x%x", client->client_handle, resp_send.curr_state);
                           }
                           else
                           {
               ...
               else // servreg_domain or local_domain are NULL
               {
                  resp_send.resp.result = QMI_RESULT_FAILURE_V01;
                  resp_send.resp.error  = QMI_ERR_INVALID_DATA_FORMAT_V01;
               }
            ...
            case SERVREG_NOTIF_RESTART_PD_REQ:
            {
                ...
                {
                    /* Call Pre STM error callbacks of USER PD before USER PD threads are frozen */
                    err_qdi_invoke_user_process_cbs_per_bucket(ERRCB_BUCKET_PRE_STM,asid);
                    /* To provide early indication of PD down*/
                    (void)servreg_send_early_pd_down_ind(asid);
                    /* Need to pass any non-zero number, if QuRT should call PD dump callback so passing asid */
                    qurt_qdi_handle_invoke(QDI_HANDLE_GENERIC, QDI_OS_PROCESS_KILL, asid, asid);
                }
            }
            break;
  ...
  else
  {
   ...
   SERVREG_TIME_TRACE_2(QMI_EXIT, connection_handle, msg_id);
   SERVREG_TRACE_TIME();
...
qmi_csi_cb_error servreg_notif_server_connect(qmi_client_handle client_handle, void *service_cookie, void **connection_handle)
{
   ...
   client = (sr_client_type_p)calloc(sizeof(sr_client_type_t), 1);
...
qmi_csi_service_handle servreg_qmi_server_init(qmi_csi_os_params *os_params)
{
   ...
   unsigned int ser_instance_id = 0;
   unsigned int *ser_instance_id_p = SERVREG_NULL;
   ...
   ser_instance_id_p = (unsigned int *)servreg_get_local_info(LOCAL_QMI_INSTANCE_ID);
   if(SERVREG_NULL != ser_instance_id_p)
      ser_instance_id = *ser_instance_id_p;
...
static SERVREG_RESULT sr_qmi_set_ack(sr_qmi_s_proxy_p sr_qmi_s_proxy, qmi_client_handle client_h, int32_t ack_id)
{
   sr_qmi_s_client_p curr_client_p = SERVREG_NULL;
   SERVREG_RESULT ret = SERVREG_FAILURE;
   int32_t transaction_id = 0;
   int32_t e_d_transaction_id = 0;
   ...
   if (SERVREG_NULL != sr_qmi_s_proxy)
   {
      ...
      e_d_transaction_id = sr_qmi_s_proxy->e_d_transaction_id;

      //validate Early down transaction
      if (e_d_transaction_id == ack_id)
      {
         SERVREG_TRACE_2(QMI_ED_TXID, e_d_transaction_id, ack_id);
         qurt_pimutex_unlock(&servreg_qmi_server_internal.mutex);
