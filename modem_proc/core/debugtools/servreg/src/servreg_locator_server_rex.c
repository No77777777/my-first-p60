...
/* Internal structure */
struct servreg_locator_server_internal_s
{
   qurt_mutex_t node_mutex;
   qurt_mutex_t dl_table_mutex;
};
struct servreg_locator_server_internal_s servreg_locator_server_internal;
...
static servreg_qmi_locator_server_node_p servreg_qmi_locator_server_node_alloc(void);
...
static servreg_dl_table_p servreg_dl_table_hashtab_get(servreg_hash_t hash)
{
   ...
   qurt_pimutex_lock(&(servreg_locator_server_internal.dl_table_mutex));

   sr_dl_table = servreg_dl_table_hash[hash];

   qurt_pimutex_unlock(&(servreg_locator_server_internal.dl_table_mutex));
...
/** =====================================================================
 * Function:
 *     servreg_qmi_locator_server_node_alloc
 *
 * Description:
 *     Allocates the servreg QMI locator server node
 *
 * Parameters:
 *     None
 *
 * Returns:
 *    Dynamically allocated  servreg QMI locator server node
 * =====================================================================  */
static servreg_qmi_locator_server_node_p servreg_qmi_locator_server_node_alloc(void)
{
   servreg_qmi_locator_server_node_p new_node = SERVREG_NULL;

   new_node = (servreg_qmi_locator_server_node_p)calloc(sizeof(servreg_qmi_locator_server_node_t), 1);

   if (SERVREG_NULL != new_node)
   {
      new_node->locator_signature = SERVREG_QMI_NOTIF_SERVER_SIGNATURE;
   }
   else
   {
      ERR_FATAL( "servreg_qmi_locator_server_node_p calloc failed", 0, 0, 0);
   }
...
static SERVREG_QMI_LOC_SERVER_HANDLE servreg_get_qmi_locator_server_node(qmi_client_handle client_handle)
{
   ...
   qurt_pimutex_lock(&(servreg_locator_server_internal.node_mutex));
   ...
   qurt_pimutex_unlock(&(servreg_locator_server_internal.node_mutex));
...
static SERVREG_QMI_LOC_SERVER_HANDLE servreg_create_qmi_locator_server_node(qmi_client_handle client_handle)
{
   ...
   {
      /* qmi node */
      sr_qmi_locator_server_node = servreg_qmi_locator_server_node_alloc();
      ...
      if(SERVREG_QMI_LOC_SERVER_SIGNATURE == sr_qmi_locator_server_node_del->locator_signature)
      {
         while(SERVREG_NULL != sr_qmi_locator_server_node_curr)
         {
            if(sr_qmi_locator_server_node_curr == sr_qmi_locator_server_node_del)
            {
               ...
               free(sr_qmi_locator_server_node_del);
               sr_qmi_locator_server_node_del = SERVREG_NULL;
               ret = SERVREG_SUCCESS;
               break;
            }
      ...
      if(SERVREG_TRUE == servreg_qmi_validate_connection_handle(connection_handle) && SERVREG_TRUE == client->connected)
      {
         switch(msg_id)
         {
            ...
            case SERVREG_LOC_GET_DOMAIN_LIST_REQ:
            {
               ...
               resp_send = (qmi_servreg_loc_get_domain_list_resp_msg_v01*)calloc(sizeof(qmi_servreg_loc_get_domain_list_resp_msg_v01), 1);
            ...
            case SERVREG_LOC_REGISTER_SERVICE_LIST_REQ:
            {
               ...
               for(i = 0; i < request->service_list_len; i++)
               {
                  ...
                  if(SERVREG_NULL != sr_dl_table && SERVREG_NULL != srdl_entry)
                  {
                     ...
                     srdl_entry->domain_name = (SERVREG_NAME)calloc(sizeof(char), len1);
         ...
         if(client_curr == (servreg_locator_client_type_p)connection_handle && (SERVREG_FALSE != client_curr->connected))
         {
            ...
            while(SERVREG_NULL != sr_qmi_locator_server_node_curr)
            {
               ...
               if(client_curr->client_handle == sr_qmi_locator_server_node_curr->client_handle)
               {
                  ...
                  free(sr_qmi_locator_server_node_curr);
                  sr_qmi_locator_server_node_curr = SERVREG_NULL;
                  break;
               }
               else
               {
...
qmi_csi_cb_error servreg_locator_server_connect(qmi_client_handle client_handle, void *service_cookie, void **connection_handle)
{
   ...
   client = (servreg_locator_client_type_p)calloc(sizeof(servreg_locator_client_type_t), 1);
...
void servreg_locator_server_init(void)
{
   ...
   static char req_stack[TMS_TASK_STACK_2K];

   qurt_pimutex_init(&(servreg_locator_server_internal.node_mutex));
   qurt_pimutex_init(&(servreg_locator_server_internal.dl_table_mutex));
   ...
   rex_task_attr_set_stack_size(&req_attr, TMS_TASK_STACK_2K);
   ...
   rex_task_attr_set_task_name(&req_attr, SERVREG_QMI_LOC_REQ_SERVER_TASK_NAME);
