...
typedef enum
{
  ...
  SRT_QMI_ED_TXID,        // [Early down transaction_id, rx'd transaction_id]
  ...
  SRT_REG_LOCAL,          // [servreg_relationship_type, servreg_mon_node_p]
  SRT_REG_REMOTE,         // [servreg_relationship_type, servreg_mon_node_p]
  SRT_REG_USER,           // [servreg_relationship_type, servreg_mon_node_p]
  SRT_REG_USER_IF,        // no data
  ...
  SRT_COMMON_DEREG,       // [servreg_mon_node_p, thread id]
  SRT_DEREG_LOCAL,        // [servreg_relationship_type, servreg_mon_node_p]
  SRT_DEREG_REMOTE,       // [servreg_relationship_type, servreg_mon_node_p]
  SRT_DEREG_USER,         // [servreg_relationship_type, servreg_mon_node_p]
  SRT_COMMON_DEREG_EXIT,  // [servreg_mon_node_p, thread id]
  SRT_FREE_LOCAL,         // [servreg_relationship_type, servreg_mon_node_p]
  SRT_FREE_REMOTE,        // [servreg_relationship_type, servreg_mon_node_p]
  SRT_FREE_USER,          // [servreg_relationship_type, servreg_mon_node_p]
  SRT_QMI_IND_PROCESS,    // [servreg_mon_node_p, transaction_id] 
  ...
  SRT_COMMON_ACK_LOCAL,   // [servreg_mon_node_p, transaction_id]
  SRT_COMMON_ACK_USER,    // [servreg_mon_node_p, transaction_id]
  SRT_ED_ACK,             // [servreg_mon_node_p, ACK id]
  SRT_ED_ACK2,            // [servreg_mon_node_p, ACK id]
  ...
  SRT_SET_STATE_E,        // [servreg_mon_node_p, SERVREG_SERVICE_STATE]
  SRT_SET_STATE_ED,        // [servreg_mon_node_p, SERVREG_SERVICE_STATE]
  SRT_QDI_SET_STATE,      // [servreg_mon_node_p, SERVREG_SERVICE_STATE]
  SRT_QDI_SET_STATE_E,    // [servreg_mon_node_p, SERVREG_SERVICE_STATE]
  ...
  SRT_REMOTE_REG_QMI_SEND,// [servreg_mon_node_p, qmi_client_handle]
  SRT_REMOTE_DEREG_QMI_SEND,// [servreg_mon_node_p, qmi_client_handle]
  SRT_REMOTE_ACK_QMI_SEND,// [servreg_mon_node_p, transaction_id]
  SRT_GET_DOMAINLIST_QMI_SEND, // [dlhandle, qmi_client_handle]
  ...
  SRT_ACK_TIMER_RELOAD,   // [ack_check_timer_status, 0]
  SRT_INSTANCE_UP,        // [QMI Instance ID, qmi_client_handle]
  SRT_FINISH_PENDING_REG, // [sr_remote_service_entry->mon_handle, 0]
  SRT_INVALID_STATE_TRANS,// [sr_mon_handle, (to)state]
  SRT_ACK_TIMER_RESUME,   // no data
  SRT_ACK_TIMER_STOPPED,  // no data
  SRT_FREE_CACHE_PD_DATA, // no data
  SRT_NULL_QDI_PD_NODE,   // no data
  SRT_DEREG_NULL_QDI_PD_NODE,// no data
  SRT_TIMESTAMP,          // no data
  SRT_ERROR,              // no data
  SRT_SUCCESS,            // no data
  SRT_FAIL,               // no data
  SRT_EMPTY_Q_NOTIF,      // no data
  SRT_INSTANCE_NOT_UP,    // [QMI Instance ID, 0]
  SRT_REG_SSR_E,          // [service_name,SERVREG_QMI_NOTIF_CLIENT_HANDLE]
  SRT_RETRY_SSR_E,        // [service_name,SERVREG_QMI_NOTIF_CLIENT_HANDLE]
  SRT_DEREG_SSR_E,        // [service_name,SERVREG_QMI_NOTIF_CLIENT_HANDLE]
  SRT_LOCATOR_STATE_QUERY,// [thread id, status]
  SRT_QMI_INSTANCE_SIG,   // no data
  SRT_RETRY_QMI_CONN,     // [sr_mon_handle, qmi_client_handle]
  SRT_RELEASE_QMI_CONN,   // [sr_mon_handle, qmi_client_handle]
  SRT_SSR_UP_EVENT,       // [SERVREG_QMI_NOTIF_CLIENT_HANDLE]
  SRT_SSR_UP_PROCESS,     // no data
  SRT_SET_QDI_NODE_STATE,  // [sr_mon_handle, (to)state]
  SRT_QDI_REG_HNDL,        //[sr_mon_handle,sr_qdi_notif_handle]
  SRT_QDI_DEREG_HNDL       //[sr_mon_handle,sr_qdi_notif_handle]
} srt_event_type;
...
do {                                                \
  ...
  uint32 timestamp32_0 = timestamp64 >> 32;         \
  uint32 timestamp32_1 = timestamp64 & 0xFFFFFFFF;  \
  void *srt[2] = {(void*)timestamp32_0, (void*)timestamp32_1};        \
  srt_time_event(SRT_TIMESTAMP, srt);                    \
} while(0)

#define SERVREG_TIME_TRACE_0(event)               \
do {                                         \
     srt_time_event(SRT_##event, NULL);           \
} while(0)

#define SERVREG_TIME_TRACE_2(event, srt0, srt1)          \
do {                                                \
  void *srt[2] = {(void*)srt0, (void*)srt1};        \
  srt_time_event(SRT_##event, srt);                      \
} while(0)
...
#define SERVREG_TIME_TRACE_0(event) do{}while(0)
#define SERVREG_TIME_TRACE_2(event, srt0, srt1) do{}while(0)
