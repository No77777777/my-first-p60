...
$Header: //components/rel/core.mpss/10.0/debugtools/sys_m/src/ssreq_svc_poll_client.c#2 $
...
)
{
   ...
   if (!ssreq_service_object)
   {
      ...
      TMS_MSG_ERROR("Failed to get SSREQ service object");
      return;
   }
   ...
   {
       ssreq_client_init_status = FALSE;
       TMS_MSG_ERROR_1("Failed to connect to SSREQ server instance:%d", ser_instance_id);
       return;
   }

   ssreq_client_init_status = TRUE;
   TMS_MSG_HIGH_1("Connected to SSREQ server instance:%d", ser_instance_id);
   return;   
}
...
*/
/*=========================================================================*/
void ssreq_svc_poll_init(void)
{
  (void)tms_utils_launch_qurt_task(SSREQ_SVC_POLL_TASK_NAME, TMS_TASK_STACK_4K, TMS_TASK_PRIO_SYS_M, ssreq_svc_poll_task, NULL, TRUE);
}
