/*===========================================================================
           ...
           S Y S _ M _ SMSM _ NON _ MPSS. C
...
$Header: //components/rel/core.mpss/10.0/debugtools/sys_m/src/sys_m_smsm_non_mpss.c#1 $
...
static void sys_m_smsm_task(void * unused);
...
void sys_m_smsm_init(void)
{
   ...
   (void)tms_utils_launch_qurt_task(SYS_M_SMSM_TASK_NAME, TMS_TASK_STACK_4K, TMS_TASK_PRIO_SYS_M, sys_m_smsm_task, NULL, TRUE);
}
...
static void sys_m_smsm_task(void * unused)
{
    ...
    if (return_val != SMP2P_STATUS_SUCCESS && return_val != SMP2P_STATUS_NOT_FOUND)
     ...
     if(sys_m_smp2p_cached_init_state)
     {
        ...
        {
            TMS_MSG_HIGH("ERR RDY SMP2P bit set successfully");   
        }
     }
