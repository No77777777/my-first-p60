...
/*=============================================================================
Edit History
$Header: //components/rel/core.mpss/10.0/debugtools/pd_mon/src/pd_elf.c#4 $
...
typedef struct 
{
   ...
   void * data1;
   void * data2;
} pd_elf_marshall_data_s;


volatile pd_debug_buf_s pd_debug_buf={0};

// helper prototype
int32 pd_elf_fs_write_all(int filedes, void *buf, uint32 nbyte);
...
static PD_ELF_STATUS pd_elf_header_write(int fd, Elf32_Ehdr* header_p, unsigned int * bytes_written)
{
   PD_ELF_STATUS rc = PD_ELF_STATUS_ERROR;

   if (header_p->e_ehsize == pd_elf_fs_write_all(fd, (void*)header_p, header_p->e_ehsize))
   {
      rc = PD_ELF_STATUS_SUCCESS;
      if ( bytes_written != NULL )
        *bytes_written += header_p->e_ehsize;
   }
...
PD_ELF_STATUS pd_elf_prgtab_alloc(Elf32_Ehdr* header_p, pd_elf_prgtab_p* prgtab_p, Elf32_Word type, char* name, Elf32_Word ntype, void* addr, Elf32_Word size, void* mapped_addr)
{
   PD_ELF_STATUS rc = PD_ELF_STATUS_ERROR;
   pd_elf_prgtab_p pt_p = malloc(sizeof(pd_elf_prgtab_t));

   TMS_MSG_HIGH_2("prgtab_alloc 0x%x 0x%x", addr, size);
...
PD_ELF_STATUS pd_elf_prgtab_alloc_note_segment(Elf32_Ehdr* header_p, pd_elf_prgtab_p* prgtab_p, Elf32_Word type, Elf32_Word size)
{
   PD_ELF_STATUS rc = PD_ELF_STATUS_ERROR;
   pd_elf_prgtab_p pt_p = malloc(sizeof(pd_elf_prgtab_t));

   TMS_MSG_HIGH_1("prgtab_alloc_note_segment 0x%x", size);
   ...
   if (NULL != pt_p)
   {
      ...
      if (type == PT_NOTE)
      {
         /* Note segment size would be the size provided as an argument to this function */
         pt_p->size = size;
...
/* Creates header for Note section at address specified by buffer */
PD_ELF_STATUS pd_elf_prgtab_populate_note_section_header(char * buffer, char* name, Elf32_Word ntype, Elf32_Word size)
{
  Elf32_Word name_len = strlen(name) + 1;

  ((Elf32_Nhdr*)buffer)->n_namesz = name_len;
  ((Elf32_Nhdr*)buffer)->n_descsz = size;
  ((Elf32_Nhdr*)buffer)->n_type = ntype;

  memsmove((char*)buffer + sizeof(Elf32_Nhdr), name_len, name, name_len);
...
PD_ELF_STATUS pd_elf_prgtab_prepare_header(Elf32_Ehdr* header_p, pd_elf_prgtab_p* prgtab_p, void ** prgtab_phdr, unsigned int prgtab_phdr_num )
{
   ...
   *prgtab_phdr = malloc( prgtab_phdr_num * sizeof(Elf32_Phdr) );
   ...
   TMS_MSG_HIGH_1("Program Header Allocated %d", prgtab_phdr_num);
...
PD_ELF_STATUS pd_elf_prgtab_write(int fd, Elf32_Ehdr* header_p, pd_elf_prgtab_p* prgtab_p, void ** prgtab_phdr, unsigned int * bytes_written)
{
   PD_ELF_STATUS rc = PD_ELF_STATUS_ERROR;
   pd_elf_prgtab_p pt_p;
   void **link,**temp;
   ...
   pd_debug_buf.prg = 1;
   pd_debug_buf.prg_addr = prgtab_phdr;

   if (sizeof(Elf32_Phdr) * header_p->e_phnum == pd_elf_fs_write_all(fd, *prgtab_phdr, sizeof(Elf32_Phdr) * header_p->e_phnum))
   {
     // SUCCESS
     if ( bytes_written != NULL )
       *bytes_written += sizeof(Elf32_Phdr) * header_p->e_phnum; 
   }
   else
   {
     return rc; // for ()
   ...
   for (pt_p = *prgtab_p; NULL != pt_p; pt_p = pt_p->next)
   {
      ...
      if (PD_ELF_LIST == pt_p->addr_type)
      {
         ...
         for (link = pt_p->addr; NULL != link; link = (void**)(*link))
         {
            ...
            if (size == pd_elf_fs_write_all(fd, (char*)link + sizeof(void*) +sizeof(unsigned int), size))
            {
               // SUCCESS
               if ( bytes_written != NULL )
                 *bytes_written += size; 
            ...
            {
               TMS_MSG_ERROR(" Error and Early exit, failed to write TCB data into unmapped region");
               return rc;
            }
         }
         ...
         /* Free list here since it is holding big blocks which can create problems 
            and it is already dumped so no use of holding it*/
      ...
      else
      {
         ...
         if (0 != pt_p->phdr->p_filesz)
         {
            pd_debug_buf.prg = 6;
            pd_debug_buf.prg_addr = pt_p->addr;
            /* Dont write Note segment.. Note segments are expected to have TCB data.. This will written later 
               using pd_elf_write_composite_note_segment().*/
            if ( pt_p->addr )
            {
              if (pt_p->mapped_addr == NULL)
                 write_addr = pt_p->addr;
              else
                 write_addr = pt_p->mapped_addr;

              if (pt_p->phdr->p_filesz == pd_elf_fs_write_all(fd, (void*)write_addr, pt_p->phdr->p_filesz))
              {
                 // SUCCESS
                 if ( bytes_written != NULL )
                   *bytes_written += pt_p->phdr->p_filesz; 
              }
              else
              {
                 TMS_MSG_ERROR(" Error and Early exit, failed to program data ( note sections and memblocks) ");
                 return rc;
              }
           }
         }
...
PD_ELF_STATUS pd_elf_sectab_write(int fd, pd_elf_sectab_p* sectab_p, unsigned * bytes_written)
{
   PD_ELF_STATUS rc = PD_ELF_STATUS_ERROR;
   pd_elf_sectab_p st_p;
   ...
   for (st_p = *sectab_p; NULL != st_p; st_p = st_p->next)
   {
      if (sizeof(Elf32_Shdr) == pd_elf_fs_write_all(fd, (void*)&st_p->shdr, sizeof(Elf32_Shdr)))
      {
         // SUCCESS
         if ( bytes_written != NULL )
           *bytes_written += sizeof(Elf32_Shdr);
      }
      else
      {
         break; // for ()
   ...
   if (NULL == st_p)
   {
      for (st_p = *sectab_p; NULL != st_p; st_p = st_p->next)
      {
         if (0 != st_p->shdr.sh_size)
         {
            if (st_p->shdr.sh_size == pd_elf_fs_write_all(fd, (void*)st_p->shdr.sh_addr, st_p->shdr.sh_size))
            {
               // SUCCESS
               if ( bytes_written != NULL )
                 *bytes_written += st_p->shdr.sh_size;
            }
            else
            {
               break; // for ()
...
static PD_ELF_STATUS pd_elf_sectab_free(pd_elf_sectab_p* sectab_p)
{
   PD_ELF_STATUS rc = PD_ELF_STATUS_ERROR;
...
PD_ELF_STATUS pd_elf_write_header_prgtab_internal(pd_elf_marshall_data_s *data)
{
   int fd;
   Elf32_Ehdr* header_p;
   pd_elf_prgtab_p* prgtab_p;
   void ** prgtab_phdr;
   unsigned int bytes_written = 0;
   uint64 start_tick, end_tick;

   fd          = data->fd;
   header_p    = (Elf32_Ehdr*)data->data0;
   prgtab_p    = (pd_elf_prgtab_p*)data->data1;
   prgtab_phdr = (void **)data->data2;

   free(data);

   // Can be enabled for debug
   //TMS_MSG_HIGH_3("pd_elf_write_header_prgtab_internal header_p 0x%x prgtab_p 0x%x prgtab_phdr 0x%x", header_p, prgtab_p, prgtab_phdr );
   ...
   if (PD_ELF_STATUS_SUCCESS != pd_elf_header_write(fd, header_p, &bytes_written))
   {
   ...
   if (PD_ELF_STATUS_SUCCESS != pd_elf_prgtab_write(fd, header_p, prgtab_p, prgtab_phdr, &bytes_written))
   {
   ...
   TMS_MSG_HIGH_2_64("Bytes Written(ELf Hdr,Prg hdr, Memblocks, Some Note sections) %llu, time(ms) taken for fs operation %llu", bytes_written, TMS_TIME_DIFF_IN_MSEC(end_tick, start_tick));

   return PD_ELF_STATUS_SUCCESS;
}

PD_ELF_STATUS pd_elf_write_header_prgtab(int fd, Elf32_Ehdr* header_p, pd_elf_prgtab_p* prgtab_p, void ** prgtab_phdr)
{
   pd_elf_marshall_data_s * data = calloc(1,  sizeof(pd_elf_marshall_data_s));
   ...
   data->fd    = fd;
   data->data0 = (void *)header_p;
   data->data1 = (void *)prgtab_p;
   data->data2 = (void *)prgtab_phdr;

   if (PD_ELF_STATUS_SUCCESS == pd_elf_write_header_prgtab_internal(data))
   {
      pd_elf_term_header_prgtab( header_p, prgtab_p, prgtab_phdr);
      return PD_ELF_STATUS_SUCCESS;
   }
   else
   {
...
PD_ELF_STATUS pd_elf_write_composite_note_segment_internal(pd_elf_marshall_data_s * data)
{
   int fd;
   void * prgcomposite_note_segment;
   unsigned int size;
   uint64 start_tick, end_tick;

   fd          = data->fd;
   prgcomposite_note_segment = (void*)data->data0;
   size        = (unsigned int)data->data1;

   free(data);

   // Can be enabled for debug
   //TMS_MSG_HIGH_3("pd_elf_write_composite_note_segment_internal fd %d prgcomposite_note_segment 0x%x size %d", fd, prgcomposite_note_segment, size );

   tms_get_timetick(&start_tick);

   if (size!= pd_elf_fs_write_all(fd, prgcomposite_note_segment, size))
   {
      TMS_MSG_ERROR( "Elf write composite note segment error");
      return PD_ELF_STATUS_ERROR;
   }

   tms_get_timetick(&end_tick);

   TMS_MSG_HIGH_2_64("Bytes Written(Composite Note Segment) %llu, time(ms) taken for fs operation %llu", size, TMS_TIME_DIFF_IN_MSEC(end_tick, start_tick));

   return PD_ELF_STATUS_SUCCESS;
}

PD_ELF_STATUS pd_elf_write_composite_note_segment(int fd, void * prgcomposite_note_segment, unsigned int size)
{
   pd_elf_marshall_data_s * data = calloc(1,  sizeof(pd_elf_marshall_data_s));
   ...
   data->fd    = fd;
   data->data0 = (void *)prgcomposite_note_segment;
   data->data1 = (void *)size;

   if (PD_ELF_STATUS_SUCCESS == pd_elf_write_composite_note_segment_internal(data))
   {
      free(prgcomposite_note_segment);
      return PD_ELF_STATUS_SUCCESS;
   }
   else
   {
...
PD_ELF_STATUS pd_elf_write_sectab_internal(pd_elf_marshall_data_s * data)
{
   int fd;
   pd_elf_sectab_p* sectab_p;
   uint64 bytes_written = 0;
   uint64 start_tick, end_tick;

   fd          = data->fd;
   sectab_p    = (pd_elf_sectab_p*)data->data0;

   free(data);

   // Can be enabled for debug
   //TMS_MSG_HIGH_3("pd_elf_write_sectab_internal  sectab_p 0x%x sig 0x%x mask 0x%x", sectab_p, sig, mask );
   ...
   if (PD_ELF_STATUS_SUCCESS != pd_elf_sectab_write(fd, sectab_p, (unsigned *)&bytes_written))
   {
   ...
   TMS_MSG_HIGH_2_64("Bytes Written(Sectab) %llu, time(ms) taken for fs operation %llu", bytes_written, TMS_TIME_DIFF_IN_MSEC(end_tick, start_tick));

   return PD_ELF_STATUS_SUCCESS;
}

PD_ELF_STATUS pd_elf_write_sectab(int fd, pd_elf_sectab_p* sectab_p)
{
   pd_elf_marshall_data_s * data = calloc(1,  sizeof(pd_elf_marshall_data_s));
   ...
   data->fd    = fd;
   data->data0 = (void *)sectab_p;

   if (PD_ELF_STATUS_SUCCESS == pd_elf_write_sectab_internal(data))
   {
      pd_elf_term_sectab(sectab_p);
      return PD_ELF_STATUS_SUCCESS;
   }
   else
   {
...
PD_ELF_STATUS pd_elf_term_header_prgtab(Elf32_Ehdr* header_p, pd_elf_prgtab_p* prgtab_p, void **prgtab_phdr)
{
   PD_ELF_STATUS rc = PD_ELF_STATUS_ERROR;
...
PD_ELF_STATUS pd_elf_term_sectab(pd_elf_sectab_p* sectab_p)
{
   return pd_elf_sectab_free(sectab_p);
}
// write-to-completion helper for "int32 fs_write (int filedes, const void *buf, uint32 nbyte);"
int32 pd_elf_fs_write_all(int filedes, void *buf, uint32 nbyte)
{
   ...
   {
      dog_force_kick();
      written = pd_dump_fs_al_write(filedes, out_p, pending);

      // check for error 
      if (written <= 0)
      {
         TMS_MSG_ERROR_2(" Error writing Block 0x%x fs_error_code %d", out_p, written);
         return written;
      }
