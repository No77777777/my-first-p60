...
/*=============================================================================
Edit History
$Header: //components/rel/core.mpss/10.0/debugtools/pd_mon/src/pd_mon.c#2 $
...
void pd_mon_servreg_notif_sync_cb( SERVREG_CB_PARAM stat_p, SERVREG_CB_PARAM DLL_API_UNUSED ignored );
...
*/
void pd_mon_qurt_onspawn_cb(int client_handle, void DLL_API_UNUSED *data_ptr)
{
...
PD_MON_HANDLE pd_mon_spawn(PD_MON_RESTART* restart_p, PD_MON_NAME program, ...)
{
   ...
   if (sizeof(pd_mon_internal.status)/sizeof(PD_MON_STATUS_T) > i)
   {
      ...
      // Check if we can obtain stack resource for the Program Monitor Context
         ...
         // Init the Program Monitor Context
         (void)tms_utils_launch_qurt_task(name, TMS_TASK_STACK_4K, TMS_TASK_PRIO_DEFAULT, PROGRAM_MONITOR, (void*)program, TRUE);
