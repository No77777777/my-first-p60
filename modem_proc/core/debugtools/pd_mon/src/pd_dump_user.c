...
/*=============================================================================
Edit History
$Header: //components/rel/core.mpss/10.0/debugtools/pd_mon/src/pd_dump_user.c#2 $
...
extern PD_DUMP_HANDLE pd_dump_register_client(struct qurt_debug_thread_info * addr, int region_len, unsigned int *size);
extern PD_DUMP_STATUS pd_dump_add_note_section_interface(int handle, char * name, char * desc, uint32 descsz, uint32 type );
extern PD_DUMP_HANDLE pd_dump_close_client(int handle);
...
static PD_DUMP_HANDLE handle = -1;
...
PD_DUMP_HANDLE pd_dump_init(void)
{
   ...
   ****/
   ...
   handle = pd_dump_register_client(pd_dump_magic_region_v1.addr, PD_DUMP_MAGIC_REGION_BYTES, &pd_dump_magic_region_v1.size);

   qurt_printf("%s, device name '%s', return %d\n", __func__, PD_DUMP_DEVICE, handle);

   return handle;
}

DLL_API_GLOBAL PD_DUMP_STATUS pd_dump_add_note_section(char * name, char * desc, uint32 descsz, uint32 type )
{
   return pd_dump_add_note_section_interface(handle, name, desc, descsz, type);
}
...
{
   PD_DUMP_STATUS ret = PD_DUMP_STATUS_ERROR;
   ...
   ret = 0 <= pd_dump_close_client(handle) ? PD_DUMP_STATUS_SUCCESS : PD_DUMP_STATUS_ERROR;
