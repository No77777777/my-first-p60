...
/*=============================================================================
Edit History
$Header: //components/rel/core.mpss/10.0/debugtools/api/tms_rcinit.h#3 $
...
struct rcinit_internal_s
{
...
#endif
   rcinit_internal_hs_t hs_table[RCINIT_TASK_MAX];                                // term, registered tasks, per group, handshakes ignored

   // the following instruments are used for indepth manual debugging

   rcinit_info_t*** policy_base;                                                 // base pointer loaded from index

   rcinit_info_t** group_base;                                                   // base pointer loaded from index

   // internal worker

   rcinit_info_t** worker_argv;                                                  // worker argv (coordinated by rcevt initfn)
   ...
   rcinit_autogen_internal_t * autogen_internal;
   
   uint64 group_start_time[RCINIT_GROUP_MAX+1];                                  // timestamp for each group start time and total end time
};


struct rcinit_internal_static_s
{
   // the following instruments are used for indepth manual debugging
   int tls_key;

   int tls_key_autogen_struct;

   unsigned long tls_available;
   ...
   //qurt_anysignal_t worker_sync_sig;                                             // allows sync of worker task
   
   //qurt_anysignal_t workloop_ack_sig;                                            // allows workloop to receive ack from worker task

   q_type q_of_rcinit_client_internal_q_data_p;                                  //Stores client_internal for all RCINIT clients, 
                                                                                 //Use v.v %s %y (rcinit_client_internal_q_data_s *)rcinit_internal_static.q_of_rcinit_client_internal_q_data_p.link.next_ptr
...
}rcinit_client_internal_q_data_s;
...
extern struct rcinit_internal_static_s rcinit_internal_static;                   // internal, TRACE32 v.v rcinit_internal_static
