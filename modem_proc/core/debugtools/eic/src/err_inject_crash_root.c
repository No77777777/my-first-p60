...
$Header: //components/rel/core.mpss/10.0/debugtools/eic/src/err_inject_crash_root.c#2 $
...
extern void dog_force_bark(void); 
...
{
  /* Halt other HW threads for Root process */
  TMS_MSG_HIGH("Entering STM and injecting DOG NMI.");
  err_enter_stm_mode();
  dog_force_nmi();
  while(1); /* To prevent Q6 from entering power collapse */
}
...
{
  TMS_MSG_HIGH("Injecting DOG multithreaded NMI");
  dog_force_nmi();
}
...
{
  TMS_MSG_HIGH("Injecting DOG BARK");
  dog_force_bark();
}