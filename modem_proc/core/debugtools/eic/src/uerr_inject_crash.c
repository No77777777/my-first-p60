...
$Header: //components/rel/core.mpss/10.0/debugtools/eic/src/uerr_inject_crash.c#4 $
...
static void uerr_inject_crash_wdog_bark(void)
{
  //dog_force_bark();
}

static void uerr_inject_crash_wdog_nmi(void)
{
  //dog_force_nmi();
}

static void uerr_inject_crash_wdog_nmi_multithreaded(void)
{
  //dog_force_nmi();
}
...
void uerr_inject_crash_execute(err_inject_crash_type crash_type) 
{
  ...
  {
    utimer_clr_64(&uImage_timer,UT_MSEC);
    switch(crash_type)
    {
    ...
    case ERR_INJECT_WDOG_TIMEOUT:
      ...
      break;
    case ERR_INJECT_WDOG_BARK:
      uerr_inject_crash_wdog_bark();
      break;
    case ERR_INJECT_WDOG_NMI_MULTITHREADED:
      uerr_inject_crash_wdog_nmi_multithreaded();
      break;
    case ERR_INJECT_WDOG_NMI:
      uerr_inject_crash_wdog_nmi();
      break;
