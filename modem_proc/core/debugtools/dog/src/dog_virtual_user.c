...
#define DOG_VIRTUAL_TASK_NAME               "dog_vir_task"
 ...
 *     None
 * =====================================================================  */
static void dog_virtual_user_task(void *argv DLL_API_UNUSED)
{
   uint32_t mask = 0;
   ...
   for (;;)
   {
      ...
      if(mask & DOG_VIR_SIGNAL_MASK)
      {
         ...
         TMS_MSG_LOW("Dog signaled user dog_hb");
      }
   }
}
...
void dog_qdi_init(void)
{
   ...
   char taskname[20];
   int len = 0;
   dog_disable_state_e root_dog_disable_state = dog_disable_state; //User PD state
   ...
   {
      len = strlen(domain) + 1;
      dog_vir_user_internal.qdi_client_handle = qurt_qdi_open(DOG_QDI_DEVICE, domain, len, &root_dog_disable_state);
   }
   ...
   (void)tms_utils_launch_qurt_task(taskname, TMS_TASK_STACK_2K, TMS_TASK_PRIO_DEFAULT, dog_virtual_user_task, NULL, TRUE);
 
   /* Inherit dog_disable_state from Root PD */
   dog_disable_state = root_dog_disable_state;
   TMS_MSG_HIGH_1("dog_disable_state from root: 0x%x", dog_disable_state);
   
   // backward compatibility
   if (dog_disable_user_startup_timer)
   {
     dog_set_bit(dog_disable_state, USERPD_RCINIT_GRACE_TIMER_DISABLE); 
   }
   
   if( ! dog_all_bits_set(dog_disable_state, USERPD_RCINIT_GRACE_TIMER_DISABLE) )
   {
