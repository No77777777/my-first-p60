...
#
#  Description:
#    Watchdog Heartbeat monitoring for Root PD
#
...
//#define DOG_VIR_TIMESTAMPS
#define DOG_HB_TASK_NAME        "dog_hb"
...
DOG_EXTERN_ATTR extern volatile dog_disable_state_e dog_disable_state;
...
static void dog_hb_task( void* unused_param );
...
/** =====================================================================
 * Function:
 *     dog_hb_virtual_mutex_init
 *
 * Description:
 *     Initialize virtual dog hb mutex
 *
 * Parameters:
 *     None
 *
 ...
 *     None
 * =====================================================================  */
static inline void dog_hb_virtual_mutex_init(void)
{
   if (!dog_hb_internal.virtual_mutex_inited)
   {
      qurt_pimutex_init(&dog_hb_internal.virtual_mutex);

      dog_hb_internal.virtual_mutex_inited = 1;
   }
}

/** =====================================================================
 * Function:
 *     dog_hb_virtual_mutex_lock
 *
 * Description:
 *     Lock the virtual dog hb mutex
 *
 * Parameters:
 *    None
 *
 ...
 *    None
 * =====================================================================  */
static inline void dog_hb_virtual_mutex_lock(void)
{
   if (dog_hb_internal.virtual_mutex_inited)
   {
      qurt_pimutex_lock(&dog_hb_internal.virtual_mutex);
   }
}

/** =====================================================================
 * Function:
 *     dog_hb_virtual_mutex_unlock
 *
 * Description:
 *     Unlock the virtual dog hb mutex
 *
 * Parameters:
 *     None
 *
 ...
 *     None
 * =====================================================================  */
static inline void dog_hb_virtual_mutex_unlock(void)
{
   if (dog_hb_internal.virtual_mutex_inited)
   {
      qurt_pimutex_unlock(&dog_hb_internal.virtual_mutex);
   }
}
...
*
* Description:
*     PINGS all virtual dogs and all clients registered in the first list 
*     in the queue, i.e sets the OS signal/mask the clients registered with
*
* Parameters:
*     none
*
...
static DOG_HB_STATUS dog_hb_ping_clients(void)
{
   ...
   dog_hb_device_p dog_hb_virtual_p = dog_hb_internal.virtual_dog_head;

   /* Processes (Virtual Dog) reporting for all User processes (Virtual DOGs) */
   dog_hb_virtual_mutex_lock();

   while (NULL != dog_hb_virtual_p)
   {
     if(NULL != dog_hb_virtual_p->sig_p)
     {
        /* Signal virtual dog task */
        DOG_HB_SIGNAL_SET_QURT(dog_hb_virtual_p->sig_p, dog_hb_virtual_p->sig_m);
        
        /* Increment Ping */
        qurt_atomic_inc(&dog_hb_virtual_p->ping);
#if defined(DOG_VIR_TIMESTAMPS)
        uint64 timestamp;
        tms_get_timetick(&timestamp);
        TMS_MSG_SPRINTF_HIGH_4("dog_hb_virtual_p ping: %s(0x%x), %d, 0x%x", dog_hb_virtual_p->name, (unsigned int)dog_hb_virtual_p, dog_hb_virtual_p->ping, (unsigned int)timestamp);
#endif
     }
     
     dog_hb_virtual_p = dog_hb_virtual_p->next;
   }
   ...
   if(NULL != dog_hb_q_entry)
   {
      for(device_p_curr = dog_hb_q_entry->q_ping_head; NULL != device_p_curr; device_p_curr = device_p_curr->next)
      {
         if(NULL != device_p_curr->sig_p)
         {
            ...
            /* Increment Ping */
            qurt_atomic_inc(&device_p_curr->ping);
         }
      }
   }
...
static DOG_HB_STATUS dog_hb_pong(dog_report_type handle)
{
   ...
   {
      /* Clear ping */
      qurt_atomic_set(&device_p->ping, 0);
...
*
* Description:
*     Dog HB checks the pong value of all the virtual dogs and registered 
*     clients in the first list in the queue
*
* Parameters:
*     none
*
...
static DOG_HB_STATUS dog_hb_pong_check(void)
{
   ...
   while (NULL != dog_hb_virtual_p)
   {
     /* Check Pongs from Virtual Dog */
     if(DOG_HB_VIRTUAL_PING_ERROR_CNT < dog_hb_virtual_p->ping)
     {
       char msg[TMS_UTILS_BUFFER_SIZE]; /* Allocated on stack. We're about to crash anyway. */ 
       rc = DOG_HB_STATUS_ERROR;
       
       (void)tms_utils_fmt(msg, TMS_UTILS_BUFFER_SIZE, "Process (Virtual Dog) starvation: %s, pings: %d", dog_hb_virtual_p->name, dog_hb_virtual_p->ping);
            
       ERR_FATAL_AUX_MSG("DOG_HB detects starvation of Process (Virtual Dog), check coredump.err.aux_msg", msg, TMS_UTILS_BUFFER_SIZE);
...
/** =====================================================================
* Function:
*     dog_hb_virtual_register
*
* Description:
*     Registers process tasks for watchdog hb monitoring
*
* Parameters:
*     process_name : Client process name
*     sig_p        : Client supplied Qurt signal information.
*     sig_m        : Client supplied mask information.
*
* Returns:
*     dog_report_type : Dog heartbeat handle returned back and should be greater than 0
* =====================================================================  */
dog_report_type dog_hb_virtual_register(char const* process_name, void* sig_p, unsigned int sig_m)
{
   dog_report_type rc = 0;
   dog_hb_device_p device_p = NULL;

   if(NULL == process_name || NULL == sig_p || 0 == sig_m)
   {
      ERR_FATAL("Invalid input for Virtual dog HB monitor",0,0,0);
      return 0;
   }
   
   /* Allocate new */
   device_p = (dog_hb_device_p)calloc(1, sizeof(dog_hb_device_t));
   ...
   /* Initialize new */
   strlcpy(device_p->name, process_name, DOG_HB_CLIENT_NAME_LEN);
   device_p->sig_t = DOG_HB_SIG_QURTSIGNAL;
   ...
   device_p->next = dog_hb_internal.virtual_dog_head;     // Save old head pointer as next for new pointer
   dog_hb_internal.virtual_dog_head = device_p;           // New node becomes head pointer
...
void dog_hb_init(void)
{
   ...
   dog_hb_internal.dog_hb_q = (q_type *)calloc(1, sizeof(q_type));
   ...
   if(NULL != dog_hb_internal.dog_hb_q)
   {
      ...
      for(i = 0; i < DOG_HB_LIST_CNT; i++)
      {
         dog_hb_q_entry = (dog_hb_queue_p)calloc(1, sizeof(dog_hb_queue_t));
   ...
   (void)tms_utils_launch_qurt_task(DOG_HB_TASK_NAME, TMS_TASK_STACK_4K, TMS_TASK_PRIO_DOG_HB, dog_hb_task, NULL, TRUE);
...
static void dog_hb_task( void* unused_param )
{
   ...
   if (dog_hb_disable)
   {
     dog_set_bit(dog_disable_state, HB_DISABLE);
   } 
   
   if( ! dog_all_bits_set(dog_disable_state, HB_DISABLE) )
   {
      ...
      for (;;)
      {
         ...
         if(q_mask & DOG_HB_SHUTDOWN_MASK)
         {
            ...
            dog_set_bit(dog_disable_state, HB_DISABLE); // DOG HB disabled
         ...
         if(q_mask & DOG_HB_DEVICE_MONITOR_MASK)
         {
            ...
            /* Pong to Root PD on behalf of dog_virtual_user_task. If We are able to pong from
               this point, it indicates that dog_virtual_user_task and dog_hb_task in user PD 
               are both functioning normally. */
            dog_virtual_report();
            TMS_MSG_LOW("Dog reported back to root");
         }
      } /* for(;;) */
   } /* if( ! dog_all_bits_set(dog_disable_state, HB_DISABLE) ) */
}
...
/** =====================================================================
* Function:
*     dog_hb_virtual_deregister
*
* Description:
*     Dynamically de-registers processes for watchdog hb services.
*
* Parameters:
*     ident : Handle that the watchdog uses to identify the process.
*
* Returns:
*     none
* =====================================================================  */
void dog_hb_virtual_deregister(dog_report_type ident)
{
   dog_hb_device_p device_p_del = NULL, dog_hb_virtual_p = NULL, device_p_prev = NULL;
   ...
   dog_hb_virtual_p = dog_hb_internal.virtual_dog_head;
   
   /* Iterate to find required Processes (Virtual Dog) in list */
   while (NULL != dog_hb_virtual_p)
   {
     if(device_p_del == dog_hb_virtual_p)
     {
       device_found = 1;
       break;
     }

     device_p_prev = dog_hb_virtual_p;
     
     dog_hb_virtual_p = dog_hb_virtual_p->next;
   }

   /* Advance by saving next node as head pointer or assign as next to previous node */
   if(device_found)
   {     
     if (NULL == device_p_prev)
     {
       dog_hb_internal.virtual_dog_head = dog_hb_virtual_p->next; 
     }
     else 
     {
       device_p_prev->next = dog_hb_virtual_p->next;
     }
     
     dog_hb_virtual_p->next = NULL;
     free(dog_hb_virtual_p);
   }
   else
   {
     TMS_MSG_ERROR_1("Failed to deregister virtual dog HB 0x%X", device_p_del);
   }
