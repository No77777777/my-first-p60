...
static int dog_qdi_open(int client_handle, char* name, uint32_t name_len, dog_disable_state_e* disable_status);
...
extern dog_report_type dog_hb_virtual_register(char const* process_name, void* sig_p, unsigned int sig_m);
extern void dog_hb_virtual_deregister(dog_report_type ident);
...
static void dog_virtual_grace_failureCb(timer_cb_data_type pn_data)
{
   ...
   /* CPU,DDR and VDD unvote for vote made in dog_virtual_start_grace_timer() */
...
static void dog_virtual_start_grace_timer(int client_handle, void *data_ptr)
{
   ...
   {
      /* Vote for CPU,DDR and VDD during PD bootup start. This will unvote in 
      ...
      if(NULL != dog_virtual_grace_timer_curr)
      {
         ...
         TMS_MSG_SPRINTF_HIGH_2("Started %s (ASID: %u) grace timer", dog_virtual_grace_timer_curr->pn.name, pn.asid);
      }
      else
      {
   ...
   {
      TMS_MSG_SPRINTF_HIGH_2("User PD %s (ASID: %u) not started by TMS/PD_MON", pn.name, pn.asid);
   }
   ...
   while(NULL != dog_vir_grace_curr)
   {
      if(asid == dog_vir_grace_curr->pn.asid)
      {
         if (dog_vir_grace_curr->dog_pd_grace_timer_set == TRUE)
         {
            ...
            TMS_MSG_SPRINTF_HIGH_2("Stopped %s (ASID: %u) grace timer", dog_vir_grace_curr->pn.name, dog_vir_grace_curr->pn.asid);
         }
 ...
 *     len            : name length
 *     disable_status : dog_disable_state passed on to User pd for debug 
 *
 * Returns:
...
static int dog_qdi_open(int client_handle, char* name, uint32_t name_len, dog_disable_state_e* disable_status)
{
   ...
   dog_disable_state_e disable_status_root = dog_disable_state;
   int ret = -1, ret_handle = -1;

   qurt_pimutex_lock(&dog_qdi_internal.dog_qdi_mutex);

   /* CPU,DDR and VDD unvote for PD bootup end, 
   ...
   if( ! dog_all_bits_set(dog_disable_state, USERPD_SPAWN_TIMER_DISABLE) )
   {
   ...
   /* Pass on the disable enum to User-Pd so it can inherit some disabled features */
   ret = qurt_qdi_copy_to_user(client_handle, disable_status, &disable_status_root, sizeof(dog_disable_state_e));
...
static int dog_qdi_close(dog_qdi_device_p clientobj)
{
   ...
   if( ! dog_all_bits_set(dog_disable_state, VIRTUAL_DISABLE) )
   {
      dataobj = &(clientobj->dog_user_pd_data);

      if ( dataobj->dog_vir_hb_handle != NULL )
      {
        dog_hb_virtual_deregister(dataobj->dog_vir_hb_handle);
...
static int dog_qdi_hb_register(dog_qdi_device_p clientobj)
{
   ...
   if( ! dog_all_bits_set(dog_disable_state, VIRTUAL_DISABLE) )
   {
      ...
      dataobj->dog_vir_hb_handle = dog_hb_virtual_register(dataobj->pn.name, &dataobj->sig_t, DOG_VIR_SIGNAL_MASK);
...
static int dog_qdi_pong(dog_qdi_device_p clientobj)
{
   ...
   if( ! dog_all_bits_set(dog_disable_state, VIRTUAL_DISABLE) )
   {
      ...
      TMS_MSG_SPRINTF_HIGH_2("dog_qdi_pong: %s, 0x%x", dataobj->name, (unsigned int)timestamp);
#endif
                      ...
                      )
{
   ...
   switch (dog_qdi_method)
   {
      ...
      case QDI_OPEN:
      {
         ret = dog_qdi_open(client_handle, a2.ptr, a3.num, a4.ptr);
         break;
      }
...
void dog_qdi_init(void)
{
   ...
   if (dog_virtual_disable)
   {
     dog_set_bit(dog_disable_state, VIRTUAL_DISABLE); 
   }

   if (dog_disable_pd_grace_timer)
   {
     dog_set_bit(dog_disable_state, USERPD_SPAWN_TIMER_DISABLE); 
   }
