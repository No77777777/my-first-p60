...
/*=============================================================================
Edit History
$Header: //components/rel/core.mpss/10.0/debugtools/dog/src/dog_hal_common.c#2 $
...
void HAL_dogAutokick(boolean enable_autokick)
{
   ...
   /* Do not enter if either of the bits are set */
   if( ! dog_any_bit_set(dog_disable_state, HW_WDOG_DISABLE | HW_KICK_DISABLE) )
   {
      DOG_LOCK();
...
void HAL_dogInit(void)
{
...
#if defined(DOG_SCALING_FACTOR) && DOG_SCALING_FACTOR < 1
   dog_set_bit(dog_disable_state, HW_WDOG_DISABLE); // DOG disabled on account of 0/invalid scaling factor
#endif

   // backward compatibility
   if (dog_hal_disable_for_bringup)
   {
     dog_set_bit(dog_disable_state, HW_WDOG_DISABLE);
   }
   
   if (dog_intentional_timeout_flag)
   {
     dog_set_bit(dog_disable_state, HW_KICK_DISABLE);
   }

   TMS_MSG_HIGH_1("dog_disable_state: 0x%x", dog_disable_state);
   ...
   /* Set ERR RDY SMP2P bit OR cache it so sys_m_smsm_task can set it once it comes up */
   int32 smp2p_retval = sys_m_smsm_apps_set_cached(SYS_M_SUBSYS2AP_SMSM_ERR_HDL_RDY);

   if(smp2p_retval != SYS_M_SMP2P_SUCCESS && smp2p_retval != SYS_M_SMP2P_NOT_READY)
   {
      TMS_MSG_ERROR("ERR RDY SMP2P bit set/cache did not succeed");
   }
