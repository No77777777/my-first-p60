...
/*=============================================================================
Edit History
$Header: //components/rel/core.mpss/10.0/debugtools/rcinit/src/rcinit_term.c#1 $
...
} rcinit_term_internal_t;

rcinit_term_internal_t rcinit_term_internal;
...
void rcinit_register_term_group(RCINIT_GROUP group, RCEVT_SIGEX_TYPE type, RCEVT_SIGEX sigex)
{
...
#if !defined(RCINIT_EXCLUDE_MSG_SWEVT)
      TMS_SHUTDOWN_MSG_SPRINTF_ERROR_3("Register by task %s # %lx not started from rcinit grp %x", task_name, task_hash, group);
#endif
   }
   ...
   {
      if (RCINIT_TASK_MAX == rcinit_internal_hs_list_add(rcinit_term_internal.handle->client_internal->hs_table, group, task_name))
      {
#if !defined(RCINIT_EXCLUDE_TRACER_SWEVT)
...
void rcinit_unregister_term_group(RCINIT_GROUP group, RCEVT_SIGEX_TYPE type, RCEVT_SIGEX sigex)
{
...
#if !defined(RCINIT_EXCLUDE_MSG_SWEVT)
      TMS_SHUTDOWN_MSG_SPRINTF_ERROR_3("Unreg by task %s # %lx not started from rcinit grp %x", task_name, task_hash, group);
#endif
   }
   ...
   if (RCINIT_GROUP_7 >= group)
   {
      ...
      #if 0
      if (RCINIT_TASK_MAX == rcinit_internal_hs_list_del(rcinit_term_internal.handle->client_internal->hs_table, group, task_name))
      {
          #if defined(RCINIT_TRACER_SWEVT)
            tracer_event_simple_vargs(RCINIT_SWE_TERM_TASK_RI, 2, group, task_hash);
          #endif

         TMS_MSG_HIGH_2("Unreg hs_del failed grp %x # %x", group, task_hash);
      }
      #endif
...
#endif
#if !defined(RCINIT_EXCLUDE_MSG_SWEVT)
         TMS_SHUTDOWN_MSG_HIGH_2("Unreg task # %x grp %x", group, task_hash);
#endif
      }
   }
}
...
#if !defined(RCINIT_EXCLUDE_MSG_SWEVT)
      TMS_SHUTDOWN_MSG_SPRINTF_ERROR_3("Register termfn %s # %lx not started from rcinit grp %x", task_name, task_hash, group);
#endif
   }
...
#if !defined(RCINIT_EXCLUDE_MSG_SWEVT)
      TMS_SHUTDOWN_MSG_SPRINTF_ERROR_3("Unregister termfn %s # %lx not started from rcinit grp %x", task_name, task_hash, group);
#endif
   }
...
void rcinit_handshake_term(void)
{
...
#if !defined(RCINIT_EXCLUDE_MSG_SWEVT)
      TMS_SHUTDOWN_MSG_SPRINTF_ERROR_3("HS by task %s # %lx not started from rcinit grp %x", task_name, task_hash, rcinit_term_internal.handle->client_internal->group_curr);
#endif
   }
   ...
   if (RCINIT_GROUP_7 >= rcinit_term_internal.handle->client_internal->group_curr &&                           // we accept a HS only in valid GROUP
       (rcinit_process_group_term_task == rcinit_term_internal.handle->client_internal->process_state ||       // we accept a HS during task term event delivery
        rcinit_process_group_term_blocking == rcinit_term_internal.handle->client_internal->process_state      // we accept a HS during blocking task term event ACK
        )
       )
   {
      // check for ignored handshake, and report that as an error (not fatal)

      if (RCINIT_TASK_MAX == rcinit_internal_hs_list_hs(rcinit_term_internal.handle->client_internal->hs_table, rcinit_term_internal.handle->client_internal->group_curr, task_name))
      {
#if !defined(RCINIT_EXCLUDE_TRACER_SWEVT)
         tracer_event_simple_vargs(RCINIT_SWE_TERM_TASK_HI, 2, rcinit_term_internal.handle->client_internal->group_curr, task_hash);
#endif
#if !defined(RCINIT_EXCLUDE_MSG_SWEVT)
         TMS_SHUTDOWN_MSG_ERROR_2("HS ignored task # %lx grp %x", task_hash, rcinit_term_internal.handle->client_internal->group_curr);
#endif
      }
      ...
      {
#if !defined(RCINIT_EXCLUDE_TRACER_SWEVT)
         tracer_event_simple_vargs(RCINIT_SWE_TERM_TASK_HS, 2, rcinit_term_internal.handle->client_internal->group_curr, task_hash);
#endif
#if !defined(RCINIT_EXCLUDE_MSG_SWEVT)
         TMS_SHUTDOWN_MSG_HIGH_2("HS task # %lx grp %x", task_hash, rcinit_term_internal.handle->client_internal->group_curr);
#endif
...
#endif
#if !defined(RCINIT_EXCLUDE_MSG_SWEVT)
      TMS_SHUTDOWN_MSG_SPRINTF_ERROR_3("HS discarded task %s # %lx grp %x", task_name, task_hash, rcinit_term_internal.handle->client_internal->group_curr);
#endif
   }
}
...
void rcinit_internal_unprocess_groups(void)
{
   ...
   do
   {
      ...
      rcinit_term_internal.handle->client_internal->group_curr = group;                                        // processing group
      rcinit_term_internal.handle->client_internal->process_state = rcinit_process_group_term_task;            // processing tasks

      // Group Process Group Timer Starts

      timetick = rcinit_term_tmo[rcinit_term_internal.handle->client_internal->group_curr];

      if (0 != timetick)
      {
#if !defined(RCINIT_EXCLUDE_MSG_SWEVT)
         TMS_SHUTDOWN_MSG_HIGH_2("Group %x timer set %d", rcinit_term_internal.handle->client_internal->group_curr, timetick);
#endif
      ...
      count = rcevt_getregistrants_name(rcinit_term_rcevt[rcinit_term_internal.handle->client_internal->group_curr]); // get registered count

#if !defined(RCINIT_EXCLUDE_MSG_SWEVT)
      TMS_SHUTDOWN_MSG_HIGH_2("Terminate begin registrants %d grp %x", count, rcinit_term_internal.handle->client_internal->group_curr);
#endif
      ...
      if (rcevt_signal_name(rcinit_term_rcevt[rcinit_term_internal.handle->client_internal->group_curr])) {;}  // signal registered

      if (count > 0)                                                             // block only if count is positive
      {
         // HS Accepting On (Still), Checked in rcinit_handshake_term

         rcinit_term_internal.handle->client_internal->process_state = rcinit_process_group_term_blocking;     // blocking for *all* handshake
         ...
         do
         {
            ...
            {
               RCINIT_GROUP bail_group = rcinit_term_internal.handle->client_internal->group_curr;
               ...
               rcinit_term_internal.handle->client_internal->process_state = rcinit_process_group_term_fn;     // processing functions
               rcinit_term_internal.handle->client_internal->group_curr = RCINIT_GROUP_0;                      // current group

#if !defined(RCINIT_EXCLUDE_MSG_SWEVT)
               TMS_SHUTDOWN_MSG_ERROR("Bailout timer termfn processing group");
#endif
               ...
               if (rcecb_signal_name_timetick(rcinit_term_rcecb[rcinit_term_internal.handle->client_internal->group_curr], NULL)) {;}          // signal registered functions of their event
               ...
               rcinit_term_internal.handle->client_internal->process_state = rcinit_process_none;
               rcinit_term_internal.handle->client_internal->group_curr = RCINIT_GROUP_NONE;                   // current group
               ...
               {
#if !defined(RCINIT_EXCLUDE_MSG_SWEVT)
                  TMS_SHUTDOWN_MSG_ERROR_1("Bailout timer expire group %x", bail_group);
#endif
               }
            ...
            if (recv & SIG_TMO)                                                  // TIMEOUT - GROUP BASED TIMEOUT
            {
               ...
               {
                  if (0 == rcinit_term_internal.handle->client_internal->hs_table[i].hs &&
                      rcinit_term_internal.handle->client_internal->group_curr == rcinit_term_internal.handle->client_internal->hs_table[i].group)
                  {
                     task_name = &(rcinit_term_internal.handle->client_internal->hs_table[i].task_name[0]);     // selects FIRST (potentially more)
...
#if !defined(RCINIT_EXCLUDE_TRACER_SWEVT)
               tracer_event_simple_vargs(RCINIT_SWE_TERM_TIMEO, 2, rcinit_term_internal.handle->client_internal->group_curr, 0);
#endif
#if !defined(RCINIT_EXCLUDE_MSG_SWEVT)
               TMS_SHUTDOWN_MSG_SPRINTF_ERROR_2("Task %s grp timer expire missing handshake %x", task_name, rcinit_term_internal.handle->client_internal->group_curr);
#endif
               break;                                                            // exit do/while
            }
...
#if !defined(RCINIT_EXCLUDE_MSG_SWEVT)
               TMS_SHUTDOWN_MSG_HIGH_3("HS cnt %d/%d grp %x", current, count, rcinit_term_internal.handle->client_internal->group_curr);
#endif
...
#if !defined(RCINIT_EXCLUDE_MSG_SWEVT)
      TMS_SHUTDOWN_MSG_HIGH_3("termfn cnt %d grp %x remaining timeout %d", rcecb_getregistrants_name(rcinit_term_rcecb[rcinit_term_internal.handle->client_internal->group_curr]), rcinit_term_internal.handle->client_internal->group_curr, timetick);
#endif
...
#if !defined(RCINIT_EXCLUDE_TRACER_SWEVT)
      tracer_event_simple(rcinit_swe_event_term[rcinit_term_internal.handle->client_internal->group_curr]);    // software event
#endif
#if !defined(RCINIT_EXCLUDE_MSG_SWEVT)
      TMS_SHUTDOWN_MSG_HIGH_1("Terminate end grp %x", rcinit_term_internal.handle->client_internal->group_curr);
#endif
...
void rcinit_term_init( rcinit_workloop_handle_t * handle)
{
  rcinit_term_internal.handle = handle;
  rcinit_internal_hs_list_init(handle->client_internal);
}
