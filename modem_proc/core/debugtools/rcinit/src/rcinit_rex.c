...
/*=============================================================================
Edit History
$Header: //components/rel/core.mpss/10.0/debugtools/rcinit/src/rcinit_rex.c#1 $
...
RCINIT_GROUP rcinit_lookup_group_rextask(rex_tcb_type* tid)
{
   ...
   rcinit_sequence_group = autogen_p->group_map[rcinit_internal_p->policy_curr];
...
void rcinit_internal_start_rextask(const rcinit_info_t* rcinit_p, void* entry)
{
   if (RCINIT_NULL != rcinit_p && RCINIT_NULL != rcinit_p->handle)
   {
      ...
      {
         if (NULL == (rcinit_p->handle->tid_p = (rcinit_tid_p)malloc(sizeof(rex_tcb_type))))
         {
