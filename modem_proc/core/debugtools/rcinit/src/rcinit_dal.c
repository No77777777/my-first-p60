...
/*=============================================================================
Edit History
$Header: //components/rel/core.mpss/10.0/debugtools/rcinit/src/rcinit_dal.c#1 $
...
void rcinit_internal_start_daltask(const rcinit_info_t* rcinit_p, void* entry)
{
   if (RCINIT_NULL != rcinit_p && RCINIT_NULL != rcinit_p->handle)
   {
      if (NULL == (rcinit_p->handle->tid_p = (rcinit_tid_p)malloc(sizeof(RCINIT_TASK_DALTASK_CTX))))
      {
