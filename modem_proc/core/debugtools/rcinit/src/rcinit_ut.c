...
/*=============================================================================
Edit History
$Header: //components/rel/core.mpss/10.0/debugtools/rcinit/src/rcinit_ut.c#1 $
...
void rcinit_ut_blacklist(unsigned long list_count, RCINIT_NAME list[])
{
   ...
   for (offset = 0; offset < rcinit_internal->autogen_internal->rcinit_internal_name_map_size - 2; offset++)
   {
      // Iterate list
      for (cnt = 0; cnt < list_count; cnt++)
      {
         // Check if dictionary entry on list
         if (0 == strncmp(list[cnt], rcinit_internal->autogen_internal->rcinit_internal_name_map[offset].name, strlen(rcinit_internal->autogen_internal->rcinit_internal_name_map[offset].name)))
         {
            const rcinit_info_t* rcinit_p = rcinit_internal->autogen_internal->rcinit_internal_name_map[offset].info_p;
