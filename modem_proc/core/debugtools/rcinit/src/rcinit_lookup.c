...
/*=============================================================================
Edit History
$Header: //components/rel/core.mpss/10.0/debugtools/rcinit/src/rcinit_lookup.c#1 $
...
RCINIT_BOOL rcinit_lookup_available(void)
{
   ...
   */
   ...
   if (RCINIT_TASK_TLS_AVAILABLE == rcinit_internal_static.tls_available) // TLS API available
   {
      return RCINIT_TRUE;
   }
...
int rcinit_internal_name_map_search(rcinit_autogen_internal_t * autogen_p, RCINIT_NAME const value, int len, int low, int high)
{
   if ( autogen_p == NULL )
      return -1;
   ...
   while (low <= high)
   {
      ...
      compare = rcinit_internal_nmecmp(autogen_p->name_map[mid].name, value, len); // compare array[mid] to value
...
{
   rcinit_autogen_internal_t * autogen_p;
   struct rcinit_internal_s * rcinit_internal_p = rcinit_internal_tls_get_specific(rcinit_internal_static.tls_key_autogen_struct);
   if ( rcinit_internal_p == NULL )
     return RCINIT_POLICY_NONE;
   
   autogen_p = rcinit_internal_p->autogen_internal;
   ...
   while (low <= high)
   {
      ...
      compare = rcinit_internal_nmecmp(autogen_p->policy_map[mid].name, value, len); // compare array[mid] to value
...
#endif
static RCINIT_INFO rcinit_lookup_helper(RCINIT_NAME const name, struct rcinit_internal_s * rcinit_internal_p)
{
  rcinit_autogen_internal_t * autogen_p;
  if ( rcinit_internal_p == NULL )
     return RCINIT_NULL;
  ...
  int offset = rcinit_internal_name_map_search(autogen_p, name, rcinit_internal_nmelen(name) + 1, 0, autogen_p->name_map_size - 2);

  if (-1 != offset)
  {
    return(RCINIT_INFO)(autogen_p->name_map[offset].info_p);
  }
...
RCINIT_INFO rcinit_lookup(RCINIT_NAME const name)
{
   ...
   struct rcinit_internal_s * rcinit_internal_p_copy = NULL;
   struct rcinit_internal_s * rcinit_internal_p = rcinit_internal_tls_get_specific(rcinit_internal_static.tls_key_autogen_struct);
   if ( rcinit_internal_p != NULL )
   {
     rcinit_internal_p_copy = rcinit_internal_p;
     info = rcinit_lookup_helper(name, rcinit_internal_p);
     if ( info != RCINIT_NULL )
       return info;
   }
   ...
   while( qdata != NULL )
   {
      ...
      if ( rcinit_internal_p_copy != rcinit_internal_p)
      {
        info = rcinit_lookup_helper(name, rcinit_internal_p);
        if ( info != RCINIT_NULL )
           return info;
...
RCINIT_INFO rcinit_lookup_self(void)
{
   ...
   {
      const rcinit_info_t* rc = (const rcinit_info_t*)rcinit_internal_tls_get_specific(rcinit_internal_static.tls_key);
...
RCINIT_POLICY rcinit_lookup_policy(RCINIT_NAME const name)
{
   ...
   offset = rcinit_internal_policy_map_search(name, rcinit_internal_nmelen(name) + 1, 0, autogen_p->policy_map_size - 2);

   if (-1 != offset)
   {
      return autogen_p->policy_map[offset].index;
   }
...
RCINIT_GROUP rcinit_lookup_group(RCINIT_NAME const name)
{
   ...
   rcinit_info_t*** rcinit_sequence_group;
   ...
   rcinit_sequence_group = autogen_p->group_map[rcinit_internal->policy_curr];

   while (RCINIT_NULL != *rcinit_sequence_group)
   {
      rcinit_info_t** rcinit_group = *rcinit_sequence_group;

      while (RCINIT_NULL != *rcinit_group)
      {
         rcinit_info_t* rcinit_p = *rcinit_group;
...
RCINIT_HEAPQSZ rcinit_lookup_heapqsz_self(RCINIT_HEAP_PTR heap_ptr)
{
   ...
   */
   ...
   if (rcinit_lookup_available()) // TLS API available
   {
      ...
      if (rcinit_internal_static.tls_key) // TLS API available
      {
