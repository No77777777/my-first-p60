...
#define QCEQ_ETM_PARAM_FLAG_TESTBUS 0x08
#define QCEQ_ETM_PARAM_FLAG_ALL     0x0F
   ...
   switch (param_id)
   {
      ...
      case QDSS_CTRL_ETM_PARAM_ID_DSP_FILTER:
         *pval = etm_param_val.filter;
         break;
      case QDSS_CTRL_ETM_PARAM_ID_DSP_TESTBUS:
         *pval = etm_param_val.testbus;
         break;
      default:
         *pval = 0;
...
{
   int nErr = QDSS_CONTROL_RVAL_SUCCESS;
   ...
   switch (param_id)
   {
      ...
      case QDSS_CTRL_ETM_PARAM_ID_DSP_FILTER:
         ...
         break;
      case QDSS_CTRL_ETM_PARAM_ID_DSP_TESTBUS:
         nErr = qurt_etm_testbus_set_config(val);
         if (QURT_ETM_SETUP_OK==nErr) {
            etm_param_val.testbus = val;
         }
         break;
      default:
         nErr= QDSS_CONTROL_RVAL_UNKNOWN_ERR;
   }
   return nErr;
}
                             ...
                             size_t resp_max_len)
{
   ...
   while (0 < strlen(*ppqry))
   {
      ...
      else if (!strcasecmp(name, "testbus"))
      {
         flag |= QCEQ_ETM_PARAM_FLAG_TESTBUS;
      }
   ...
   if (flag & QCEQ_ETM_PARAM_FLAG_TESTBUS)
   {
      snprintf(resp_buf, sizeof(resp_buf),
               " testbus 0x%lX", etm_param_val.testbus);
      strlcat(presp_str, resp_buf, resp_max_len);
   }
...
int qdss_ctrl_etm_set_config(const char *pset_str)
{
   ...
   while (0 < remaining_len)
   {
      ...
      if (qpu_rval ==  QDSS_PARSE_UTIL_RVAL_SUCCESS)
      {
         ...
         {
            param_id = QDSS_CTRL_ETM_PARAM_ID_DSP_TESTBUS;
         }
...
void qdss_ctrl_etm_init_config_def(void)
{
   ...
   //There is no QuRT API to get this. 0 should be interpreted as default
   etm_param_val.testbus  = 0; 
}
