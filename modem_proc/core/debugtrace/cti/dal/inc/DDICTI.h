...
struct DalCTI
{
    ...
    DALResult           (*Map)(uint32 ClientHandle, DalDeviceHandle *h,
                                    const char  *block,
                                    uint8       type,
                                    uint8       trigger,
                                    uint8       channel);
    DALResult           (*Unmap)(uint32 ClientHandle, DalDeviceHandle *h,
                                    const char  *block,
                                    uint8       type,
                                    uint8       trigger,
                                    uint8       channel);
    DALResult           (*Reset)(uint32 ClientHandle, DalDeviceHandle *h, const char *block);
    DALResult           (*Query)(uint32 ClientHandle, DalDeviceHandle *h,
                                    const char  *block,
                                    uint8       type,
                                    uint8       max_triggers,
                                    uint8       *length,
                                    uint32      *mappings);
   DALResult           (*Trigger)(uint32 ClientHandle, DalDeviceHandle *h,
                                    const char  *block,
                                uint8       channel,
                                uint8       trigger_type);
...
static __inline DALResult DALCTI_Map(DalDeviceHandle *h,
                                        ...
                                        uint8       channel)
{
    return ((struct DalCTIHandle *) h)->pVtbl->Map(DALDEVICE_LOCAL_CLIENTID, h,
                                                    block,
                                                    type,
                                                    trigger,
                                                    channel);
...
static __inline DALResult DALCTI_Unmap(DalDeviceHandle *h,
                                        ...
                                        uint8       channel)
{
    return ((struct DalCTIHandle *) h)->pVtbl->Unmap(DALDEVICE_LOCAL_CLIENTID, h,
                                                        block,
                                                        type,
                                                        trigger,
                                                        channel);
...
{
    return ((struct DalCTIHandle *) h)->pVtbl->Reset(DALDEVICE_LOCAL_CLIENTID, h, block);
}
...
static __inline DALResult DALCTI_Query(DalDeviceHandle *h,
                                        ...
                                        uint32 *mappings)
{
    return ((struct DalCTIHandle *) h)->pVtbl->Query(DALDEVICE_LOCAL_CLIENTID, h,
                                                        block,
                                                        type,
                                                        max_triggers,
                                                        length,
...
static __inline DALResult DALCTI_Trigger(DalDeviceHandle *h,
                                       ...
                                       uint8       trigger_type)
{
    return ((struct DalCTIHandle *) h)->pVtbl->Trigger(DALDEVICE_LOCAL_CLIENTID, h,
                                                    block,
                                                     channel,
                                                     trigger_type);
}
