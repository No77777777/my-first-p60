...
const DALREG_DriverInfo
DALTMC_WCSS_DriverInfo = { TMC_DeviceAttach, 0, NULL};
                            ...
                            DalDeviceHandle **phDalDevice)
{
   ...
   static TMCDrvCtxt drvCtxt_WCSS = {{TMC_DriverInit,
                                      TMC_DriverDeInit
                                },1,
                                sizeof(TMCDevCtxt)};
   static TMCClientCtxt clientCtxt_WCSS;

   const char *pszDev_WCSS="DALDEVICEID_TMC_WCSS";
   
   *phDalDevice = NULL;

   if (0==strncmp(pszArg,pszDev_WCSS,strlen(pszDev_WCSS))) {
      pdrvCtxt = &drvCtxt_WCSS;
      pclientCtxt = &clientCtxt_WCSS;
   }
   else {
      pdrvCtxt = &drvCtxt;
...
extern int qdss_control_set_stm(uint8 state);

/*
 *This helper function is called befer WCSS enters island mode 
 */
...
/*
 *This helper function is called after WCSS exits island mode 
 */
