...
$Header: //components/rel/core.mpss/10.0/wiredconnectivity/uart/uart_qupv3/dal/src/DalUartCxm.c#2 $
...
/*==================================================================================================
                                          LOCAL VARIABLES
==================================================================================================*/
const DalUartFcnTable dal_functions_uart_cxm =
{
   Uart_ClearBreak,
   Uart_Close,
...
static boolean configure_interrupts(UART_CONTEXT *uart_ctxt)
{
   ...
   // The UART IRQ is mandatory.  It can be caller-specified (passed into Uart_Init)
...
static boolean get_properties(UART_CONTEXT *uart_ctxt)
{
   ...
   {               
      uart_ctxt->prop_is_cxm = FALSE;                                    //on Rumi by default CXM UART used as debug UART          
   }
...
static boolean map_hardware(UART_CONTEXT *uart_ctxt)
{
   ...
   if (uart_ctxt->prop_gsbi_control)
   {
      ...
      uart_ctxt->gsbi_mapping = tal_os_phys_map(phys_addr, GSBI_MEM_SIZE);
...
DESCRIPTION:
   ...
   Uart_Init) since we are not using a flag and critical section to control
   access.
...
DESCRIPTION:
   The DAL client must ensure that they never call this function (via Uart_Open)
...
static boolean reg_init(UART_CONTEXT *uart_ctxt, UartOpenConfig *open_cfg)
{
   ...
   if (DalPlatformInfo_Platform() == DALPLATFORMINFO_TYPE_RUMI)            
   {               
      HALuart_SetCxmEnable(uart_hal, HAL_UART_CXM_BYPASS, 1);    //on Rumi by default CXM UART used as debug UART          
   }
...
static uint32 Uart_CxmTxDirectChar(void *h, uint8 character)
{
   ...
   tal_os_log(id, INFO, "+Uart_CxmTxDirectChar(0x%08p, 0x%02X)", h, character);
   ...
   if (uart_ctxt->prop_is_cxm)
   {
      ...
      {
         tal_os_log(id, ERROR, "Uart_CxmTxDirectChar Failed");
      }
      tal_os_sync_exit(uart_ctxt->register_sync);
   }
   else
   {
      tal_os_log(id, ERROR, "Uart_CxmTxDirectChar Failed Not Supported");
   }

   tal_os_log(id, INFO, "-Uart_CxmTxDirectChar(%d)", result);
   return result;
}
...
static uint32 Uart_GetCxmTxSticky(void *h, uint32 *tx_sticky, uint32 clear)
{
   ...
   tal_os_log(id, INFO, "+Uart_GetCxmTxSticky(0x%08p, 0x%08p, 0x%08X)", h, tx_sticky, clear);
   ...
   tal_os_log(id, INFO, "-Uart_GetCxmTxSticky(%d)", result);
   return result;
}
...
static uint32 Uart_SetCxm(void *h, uint32 enabled, uint32 sam)
{
   ...
   tal_os_log(id, INFO, "+Uart_SetCxm(0x%08p, %d, %x)", h, enabled, sam);
   ...
   tal_os_log(id, INFO, "-Uart_SetCxm(%d)", result);
   return result;
}
...
static uint32 Uart_SetCxmMsgIntrMask(void *h, uint32 intr_mask)
{
   ...
   tal_os_log(id, INFO, "+Uart_SetCxmMsgIntrMask(0x%08p, %d)", h, intr_mask);
   ...
   tal_os_log(id, INFO, "-Uart_SetCxmMsgIntrMask(%d)", result);
   return result;
}
...
static uint32 Uart_SetCxmType2Mode(void *h, uint32 enabled)
{
   ...
   tal_os_log(id, INFO, "+Uart_SetCxmType2Mode(0x%08p, %d)", h, enabled);
   ...
   tal_os_log(id, INFO, "-Uart_SetCxmType2Mode(%d)", result);
   return result;
}
