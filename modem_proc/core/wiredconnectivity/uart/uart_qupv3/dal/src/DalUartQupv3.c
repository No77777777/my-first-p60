...
$Header: //components/rel/core.mpss/10.0/wiredconnectivity/uart/uart_qupv3/dal/src/DalUartQupv3.c#1 $
...
#define RXLEV_MARGIN             32 // number of empty bytes remaining in RXFIFO when an RXLEV
                                    // interrupt fires (must be a multiple of 4)

#define TXLEV_MARGIN             16 // number of full bytes remaining in TXFIFO when a TXLEV
                                    // interrupt fires (must be a multiple of 4)

#define AUTORFR_MARGIN            8 // number of empty bytes remaining in RXFIFO when we
...
static boolean       allocate_buffers        (QUPV3_UART_CONTEXT *uart_ctxt);
static boolean       check_rx_error          (QUPV3_UART_CONTEXT *uart_ctxt);
static void          clock_disable           (QUPV3_UART_CONTEXT *uart_ctxt);
static boolean       clock_enable            (QUPV3_UART_CONTEXT *uart_ctxt);
static void          close_devices           (QUPV3_UART_CONTEXT *uart_ctxt);
static boolean       configure_interrupts    (QUPV3_UART_CONTEXT *uart_ctxt);
static boolean       create_objects          (QUPV3_UART_CONTEXT *uart_ctxt);
static void          deallocate_buffers      (QUPV3_UART_CONTEXT *uart_ctxt);
static void          deregister_isr_for_rx   (QUPV3_UART_CONTEXT *uart_ctxt);
static void          deregister_isr_for_uart (QUPV3_UART_CONTEXT *uart_ctxt);
static void          destroy_objects         (QUPV3_UART_CONTEXT *uart_ctxt);
static void          dump_regs               (QUPV3_UART_CONTEXT *uart_ctxt);
static boolean       get_properties          (QUPV3_UART_CONTEXT *uart_ctxt);
static void          get_property_dword      (void *id, const char *property_name,
                                              uint32 *property_value, uint32 default_value);
static void          gpio_disable            (QUPV3_UART_CONTEXT *uart_ctxt);
static boolean       gpio_enable             (QUPV3_UART_CONTEXT *uart_ctxt);
static void          halt_line_detection     (QUPV3_UART_CONTEXT *uart_ctxt, boolean skip_halt_edge);
static void          halt_rx_path            (QUPV3_UART_CONTEXT *uart_ctxt);
static void          halt_tx_path            (QUPV3_UART_CONTEXT *uart_ctxt);
static boolean       map_hardware            (QUPV3_UART_CONTEXT *uart_ctxt);
static void          mu_close                (QUPV3_UART_CONTEXT *uart_ctxt);
static void          mu_deinit               (QUPV3_UART_CONTEXT *uart_ctxt);
static QUPV3_UART_CONTEXT *mu_init                 (UartInitConfig *init_cfg);
static boolean       mu_open                 (QUPV3_UART_CONTEXT *uart_ctxt, UartOpenConfig *open_cfg);
static void          notify_events           (QUPV3_UART_CONTEXT *uart_ctxt, UartLineEventType events);
static void          notify_rxdata_available (QUPV3_UART_CONTEXT *uart_ctxt);
static void          notify_transmit_complete(QUPV3_UART_CONTEXT *uart_ctxt);
static boolean       open_devices            (QUPV3_UART_CONTEXT *uart_ctxt);
static uint32        power_025               (QUPV3_UART_CONTEXT *uart_ctxt);
static uint32        power_off               (QUPV3_UART_CONTEXT *uart_ctxt);
static uint32        power_on                (QUPV3_UART_CONTEXT *uart_ctxt);
static boolean       prepare_for_sleep       (QUPV3_UART_CONTEXT *uart_ctxt);
static void          read_rx_fifo            (QUPV3_UART_CONTEXT *uart_ctxt, uint32 num_bytes);
static void          receive_chunk           (QUPV3_UART_CONTEXT *uart_ctxt);
static boolean       reg_init                (QUPV3_UART_CONTEXT *uart_ctxt, UartOpenConfig *open_cfg);
static boolean       register_isr_for_rx     (QUPV3_UART_CONTEXT *uart_ctxt, RX_TRIGGER_TYPE rx_trigger);
static boolean       register_isr_for_uart   (QUPV3_UART_CONTEXT *uart_ctxt);
static boolean       resume_from_sleep       (QUPV3_UART_CONTEXT *uart_ctxt);
static void *        rx_dpc                  (void *context);
static void *        rx_isr                  (void *context);
static void          rx_transfer_complete    (QUPV3_UART_CONTEXT *uart_ctxt, boolean *rx_data,
                                                                       boolean *rx_error);
static boolean       rx_transfer_start       (QUPV3_UART_CONTEXT *uart_ctxt);
static void          service_rx_break_start  (QUPV3_UART_CONTEXT *uart_ctxt);
static void          service_rx_dma          (QUPV3_UART_CONTEXT *uart_ctxt);
static void          service_rx_falling      (QUPV3_UART_CONTEXT *uart_ctxt);
static void          service_rx_rising       (QUPV3_UART_CONTEXT *uart_ctxt);
static void          service_rx_stale        (QUPV3_UART_CONTEXT *uart_ctxt);
static void          service_rx_watermark    (QUPV3_UART_CONTEXT *uart_ctxt);
static void          service_tx_dma          (QUPV3_UART_CONTEXT *uart_ctxt);
static void          service_tx_ready        (QUPV3_UART_CONTEXT *uart_ctxt);
static void          service_tx_watermark    (QUPV3_UART_CONTEXT *uart_ctxt);
static boolean       set_baud_rate           (QUPV3_UART_CONTEXT *uart_ctxt, uint32 baud_rate);
static void          set_char_format         (QUPV3_UART_CONTEXT *uart_ctxt, UartCharFormat *char_format);
static boolean       set_input_freq          (QUPV3_UART_CONTEXT *uart_ctxt, uint32 input_freq);
static void          set_rts_control         (QUPV3_UART_CONTEXT *uart_ctxt,
                                              UartRtsControlType rts_control);
static void          start_break_detection   (QUPV3_UART_CONTEXT *uart_ctxt);
static boolean       transmit_chunk          (QUPV3_UART_CONTEXT *uart_ctxt);
static uint32        Uart_ClearBreak         (void *h);
static void          Uart_Close              (void *h);
static uint32        Uart_CxmTxDirectChar    (void *h, uint8 character);
static void          Uart_DeInit             (void *h);
static void *        uart_dpc                (void *context);
static uint32        Uart_DumpRegs           (void *h);
static uint32        Uart_GetCTS             (void *h, uint32 *cts_state);
static uint32        Uart_GetCxmTxSticky     (void *h, uint32 *tx_sticky, uint32 clear);
static uint32        Uart_GetStatus          (void *h, UartStatusType *status);
static void *        Uart_Init               (UartInitConfig *init_cfg);
static void *        uart_isr                (void *context);
static uint32        Uart_LoopbackSet        (void *h, uint32 enabled);
static uint32        Uart_Open               (void *h, UartOpenConfig *open_cfg);
static uint32        Uart_PowerCapabilities  (void *h, uint32 *power_mask);
static uint32        Uart_PowerGet           (void *h, UartPowerType *power_state);
static uint32        Uart_PowerSet           (void *h, UartPowerType power_state);
static uint32        Uart_PurgeRx            (void *h);
static uint32        Uart_PurgeTx            (void *h);
static uint32        Uart_Read               (void *h, uint8 *read_buffer, uint32 buffer_len,
                                                       uint32 *bytes_read, uint32 *overrun_events);
static uint32        Uart_RxActive           (void *h, uint32 *rx_active);
static uint32        Uart_SetBreak           (void *h);
static uint32        Uart_SetCharFormat      (void *h, UartCharFormat *char_format);
static uint32        Uart_SetCtsControl      (void *h, UartCtsControlType cts_control);
static uint32        Uart_SetCxm             (void *h, uint32 enabled, uint32 sam);
static uint32        Uart_SetCxmMsgIntrMask  (void *h, uint32 intr_mask);
static uint32        Uart_SetCxmType2Mode    (void *h, uint32 enabled);
static uint32        Uart_SetCxmRatIdMode    (void *h, uint32 rat_msg_type,
                                                       boolean enable_rat_msg_to_fifo,
                                                       boolean enable_tcm_resend,
                                                       uint32 sam, boolean enabled);
static uint32        Uart_SetRate            (void *h, uint32 baud_rate);
static uint32        Uart_SetRtsControl      (void *h, UartRtsControlType rts_control);
static uint32        Uart_TxComplete         (void *h, uint32 *tx_complete);
static uint32        Uart_TxSingleChar       (void *h, uint8 character);
static uint32        Uart_Write              (void *h, const uint8 *write_buffer, uint32 buffer_len,
                                                       uint32 *bytes_written);
static void          unmap_hardware          (QUPV3_UART_CONTEXT *uart_ctxt);
...
/*==================================================================================================
                                          LOCAL VARIABLES
==================================================================================================*/
const DalUartFcnTable dal_functions_uart_qupv3 =
{
   Uart_ClearBreak,
   Uart_Close,
...
static const CLOCK_SETTINGS baud_table_qupv3[] =
{
   // baud_rate, input_freq, divider
   {        300,    7372800,   24576 },
   {        600,    7372800,   12288 },
   ...
   {    2500000,   80000000,      32 },
   {    3000000,   96000000,      32 },
   {    3200000,  102400000,      32 },
   {    3500000,  112000000,      32 },
   {    3686400,  117964800,      32 },
...
static boolean map_hardware(QUPV3_UART_CONTEXT *uart_ctxt)
{
   ...
   uart_ctxt->uart_mapping = tal_os_phys_map(phys_addr, UART_MEM_SIZE);
...
static QUPV3_UART_CONTEXT *mu_init(UartInitConfig *init_cfg)
{
   QUPV3_UART_CONTEXT *uart_ctxt;
   HALUartType hal_type;
   HALUartObject *uart_hal;

   uart_ctxt = tal_os_malloc(sizeof(QUPV3_UART_CONTEXT));
   if (uart_ctxt == NULL) { goto error; }

   memset(uart_ctxt, 0, sizeof(QUPV3_UART_CONTEXT));

   if      (init_cfg->uart_type == UART_QUPV3)  { hal_type = HAL_UART_QUPV3; }
   else                                         { goto error; }
...
static boolean mu_open(QUPV3_UART_CONTEXT *uart_ctxt, UartOpenConfig *open_cfg)
{
   ...
   // The UART clocks must be enabled before we do anything that accesses UART registers.
...
static boolean reg_init(QUPV3_UART_CONTEXT *uart_ctxt, UartOpenConfig *open_cfg)
{
   ...
   // FIFO Sizes:
   //             On QUPv3 Top instance: SE instances 7 & 8 : 128 bytes each way
   //                                    Other instances    :  64 bytes each way
...
static void *rx_dpc(void *context)
{
   QUPV3_UART_CONTEXT *uart_ctxt = (QUPV3_UART_CONTEXT *)context;
   void *id = uart_ctxt->init_cfg.device_id;
   uint32 rx_events;
...
==================================================================================================*/
static void rx_transfer_complete(QUPV3_UART_CONTEXT *uart_ctxt, boolean *rx_data, boolean *rx_error)
{
   void *id = uart_ctxt->init_cfg.device_id;
   HALUartObject *uart_hal = &uart_ctxt->uart_hal;
   MSM_UART_RX_BUF *rx_head;
...
static void service_rx_stale(QUPV3_UART_CONTEXT *uart_ctxt)
{
   ...
   bytes_to_read = HALuart_GetRxDataSize(uart_hal);
   HALuart_ClearInterrupts(uart_hal, HAL_UART_RXSTALE_IMR_FLAG);
   tal_os_log(id, INFO, "Bytes_to_read = 0x%08X", bytes_to_read);

   rx_head = &uart_ctxt->rx_buf[uart_ctxt->rx_head];
   buffer_room = RX_BUF_SIZE - rx_head->write_offset;

   if (buffer_room < bytes_to_read)
   {
      rx_transfer_complete(uart_ctxt, &rx_data, &rx_error);
   }

   read_rx_fifo(uart_ctxt, bytes_to_read);

   HALuart_ClearInterrupts(uart_hal, HAL_UART_RXLEV_IMR_FLAG);
   rx_transfer_complete(uart_ctxt, &rx_data, &rx_error);
...
static void service_rx_watermark(QUPV3_UART_CONTEXT *uart_ctxt)
{
   ...
   // TODO: FIX the overriding of rx_data and rx_error after the sync exit
   if (buffer_room < bytes_to_copy)
   {
      rx_transfer_complete(uart_ctxt, &rx_data, &rx_error);
   }
   read_rx_fifo(uart_ctxt, bytes_to_copy);
   HALuart_ClearInterrupts(&uart_ctxt->uart_hal, HAL_UART_RXLEV_IMR_FLAG);
...
static void service_tx_ready(QUPV3_UART_CONTEXT *uart_ctxt)
{
   ...
   HALuart_ClearInterrupts(uart_hal, HAL_UART_TX_READY_IMR_FLAG);
...
static boolean set_baud_rate(QUPV3_UART_CONTEXT *uart_ctxt, uint32 baud_rate)
{
   ...
   for (entry = baud_table_qupv3; entry->baud_rate; entry++)
   {
...
==================================================================================================*/
static void set_rts_control(QUPV3_UART_CONTEXT *uart_ctxt, UartRtsControlType rts_control)
{
   HALUartObject *uart_hal = &uart_ctxt->uart_hal;
...
static void start_break_detection(QUPV3_UART_CONTEXT *uart_ctxt)
{
   ...
   {
      HALuart_ClearInterrupts(uart_hal, HAL_UART_RXBREAK_START_IMR_FLAG);
...
static boolean transmit_chunk(QUPV3_UART_CONTEXT *uart_ctxt)
{
   ...
   if (bytes_sent)
   {
      ...
      {
         tal_interrupt_sync_enter(uart_ctxt->interrupt_handle, &uart_ctxt->uart_interrupt);
		 HALuart_ClearInterrupts(&uart_ctxt->uart_hal, HAL_UART_TXLEV_IMR_FLAG);
...
static uint32 Uart_ClearBreak(void *h)
{
   QUPV3_UART_CONTEXT *uart_ctxt = (QUPV3_UART_CONTEXT *)h;
   void *id = uart_ctxt->init_cfg.device_id;
   HALUartObject *uart_hal = &uart_ctxt->uart_hal;

   tal_os_log(id, INFO, "+Uart_ClearBreak(0x%08p)", h);

   HALuart_StopBreak(uart_hal);

   tal_os_log(id, INFO, "-Uart_ClearBreak(UART_SUCCESS)");
   return UART_SUCCESS;
}
...
static void Uart_Close(void *h)
{
   ...
   tal_os_log(id, INFO, "+Uart_Close(0x%08p)", h);

   mu_close(uart_ctxt);

   tal_os_log(id, INFO, "-Uart_Close(void)");
}
...
static void Uart_DeInit(void *h)
{
   ...
   tal_os_log(id, INFO, "+Uart_DeInit(0x%08p)", h);

   mu_deinit(uart_ctxt);

   tal_os_log(id, INFO, "-Uart_DeInit(void)");
}
...
static uint32 Uart_DumpRegs(void *h)
{
   ...
   tal_os_log(id, INFO, "+Uart_DumpRegs(0x%08p)", h);
   ...
   tal_os_log(id, INFO, "-Uart_DumpRegs(UART_SUCCESS)");
   return UART_SUCCESS;
}
...
static uint32 Uart_GetCTS(void *h, uint32 *cts_state)
{
   ...
   tal_os_log(id, INFO, "+Uart_GetCTS(0x%08p, 0x%08p)", h, cts_state);
   ...
   *cts_state = !(HALuart_GetCTSStatus(uart_hal));

   tal_os_log(id, INFO, "-Uart_GetCTS(UART_SUCCESS)");
   return UART_SUCCESS;
}
...
static uint32 Uart_GetStatus(void *h, UartStatusType *status)
{
   ...
   tal_os_log(id, INFO, "+Uart_GetStatus(0x%08p, 0x%08p)", h, status);
   tal_os_sync_enter(uart_ctxt->rx_sync);
   ...
   *status |= uart_ctxt->rx_cached_status;  // get cached status (overrun/parity/framing)
   ...
   if ( !(HALuart_GetCTSStatus(uart_hal)) )
   {
   ...
   tal_os_sync_exit(uart_ctxt->rx_sync);
   tal_os_log(id, INFO, "-Uart_GetStatus(UART_SUCCESS)");
   return UART_SUCCESS;
}
...
static void *Uart_Init(UartInitConfig *init_cfg)
{
   ...
   tal_os_log(id, INFO, "+Uart_Init(0x%08p)", init_cfg);

   uart_ctxt = mu_init(init_cfg);

   tal_os_log(id, INFO, "-Uart_Init(0x%08p)", uart_ctxt);
   return uart_ctxt;
}
...
static uint32 Uart_LoopbackSet(void *h, uint32 enabled)
{
   ...
   tal_os_log(id, INFO, "+Uart_LoopbackSet(0x%08p, 0x%08X)", h, enabled);

   tal_os_sync_enter(uart_ctxt->register_sync);
   HALuart_LoopbackSet(uart_hal, enabled);
   tal_os_sync_exit(uart_ctxt->register_sync);

   tal_os_log(id, INFO, "-Uart_LoopbackSet(UART_SUCCESS)");
   return UART_SUCCESS;
}
...
static uint32 Uart_Open(void *h, UartOpenConfig *open_cfg)
{
   ...
   tal_os_log(id, INFO, "+Uart_Open(0x%08p, 0x%08p)", h, open_cfg);

   result = mu_open(uart_ctxt, open_cfg) ? UART_SUCCESS : UART_ERROR;

   tal_os_log(id, INFO, "-Uart_Open(%d)", result);
   return result;
}
...
static uint32 Uart_PowerCapabilities(void *h, uint32 *power_mask)
{
   ...
   tal_os_log(id, INFO, "+Uart_PowerCapabilities(0x%08p, 0x%08p)", h, power_mask);

   *power_mask = POWER_FULL | POWER_025 | POWER_OFF;

   tal_os_log(id, INFO, "-Uart_PowerCapabilities(UART_SUCCESS)");
   return(UART_SUCCESS);
}
...
static uint32 Uart_PowerGet(void *h, UartPowerType *power_state)
{
   ...
   tal_os_log(id, INFO, "+Uart_PowerGet(0x%08p, 0x%08p)", h, power_state);

   *power_state = uart_ctxt->power_state;

   tal_os_log(id, INFO, "-Uart_PowerGet(UART_SUCCESS)");
   return(UART_SUCCESS);
}
...
static uint32 Uart_PowerSet(void *h, UartPowerType power_state)
{
   ...
   tal_os_log(id, INFO, "+Uart_PowerSet(0x%08p, 0x%08X)", h, power_state);
   ...
   switch (power_state)  // requested power state
   {
      ...
      case POWER_075:
      default:
         tal_os_log(id, ERROR, "Uart_PowerSet: invalid power state");
         ret = UART_ERROR;
         break;
   }

   tal_os_log(id, INFO, "-Uart_PowerSet(%d)", ret);
   return(ret);
}
...
static uint32 Uart_PurgeRx(void *h)
{
   ...
   tal_os_log(id, INFO, "+Uart_PurgeRx(0x%08p)", h);
   ...
   tal_os_log(id, INFO, "-Uart_PurgeRx(UART_SUCCESS)");
   return UART_SUCCESS;
}
...
static uint32 Uart_PurgeTx(void *h)
{
   ...
   tal_os_log(id, INFO, "+Uart_PurgeTx(0x%08p)", h);

   halt_tx_path(uart_ctxt);
   uart_ctxt->tx_halt = FALSE;

   tal_os_log(id, INFO, "-Uart_PurgeTx(UART_SUCCESS)");
   return UART_SUCCESS;
}
                           ...
                           uint32 *bytes_read, uint32 *overrun_events)
{
   ...
   tal_os_log(id, INFO, "+Uart_Read(0x%08p, 0x%08p, 0x%08X, 0x%08p, 0x%08p)",
   ...
   *overrun_events = uart_ctxt->rx_overruns;  // report number of overrun events since last read
   ...
   tal_os_log(id, INFO, "-Uart_Read(UART_SUCCESS)");
   return UART_SUCCESS;
}
...
static uint32 Uart_RxActive(void *h, uint32 *rx_active)
{
   ...
   tal_os_log(id, INFO, "+Uart_RxActive(0x%08p, 0x%08p)", h, rx_active);

   // TODO: Implement proper way to find if RX is active.
   *rx_active = 0;

   tal_os_log(id, INFO, "-Uart_RxActive(UART_SUCCESS)");
   return UART_SUCCESS;
}
...
static uint32 Uart_SetBreak(void *h)
{
   ...
   tal_os_log(id, INFO, "+Uart_SetBreak(0x%08p)", h);

   HALuart_StartBreak(uart_hal);

   tal_os_log(id, INFO, "-Uart_SetBreak(UART_SUCCESS)");
   return UART_SUCCESS;
}
...
static uint32 Uart_SetCharFormat(void *h, UartCharFormat *char_format)
{
   ...
   tal_os_log(id, INFO, "+Uart_SetCharFormat(0x%08p, 0x%08p)", h, char_format);

   if ( !valid_char_format(char_format) ) { return UART_ERROR; }
   set_char_format(uart_ctxt, char_format);

   tal_os_log(id, INFO, "-Uart_SetCharFormat(UART_SUCCESS)");
   return UART_SUCCESS;
}
...
static uint32 Uart_SetCtsControl(void *h, UartCtsControlType cts_control)
{
   ...
   tal_os_log(id, INFO, "+Uart_SetCtsControl(0x%08p, 0x%08X)", h, cts_control);

   tal_os_sync_enter(uart_ctxt->register_sync);
   HALuart_SetOutboundFlowCtl(uart_hal, (cts_control == CTS_ENABLE));
   tal_os_sync_exit(uart_ctxt->register_sync);

   tal_os_log(id, INFO, "-Uart_SetCtsControl(UART_SUCCESS)");
   return UART_SUCCESS;
}
...
static uint32 Uart_SetRate(void *h, uint32 baud_rate)
{
   ...
   tal_os_log(id, INFO, "+Uart_SetRate(0x%08p, %d)", h, baud_rate);

   if ( !set_baud_rate(uart_ctxt, baud_rate) ) { return UART_ERROR; }

   tal_os_log(id, INFO, "-Uart_SetRate(UART_SUCCESS)");
   return UART_SUCCESS;
}
...
static uint32 Uart_SetRtsControl(void *h, UartRtsControlType rts_control)
{
   ...
   tal_os_log(id, INFO, "+Uart_SetRtsControl(0x%08p, %d)", h, rts_control);

   if ( !valid_rts_control(rts_control) ) { return UART_ERROR; }
   uart_ctxt->rts_control = rts_control;
   set_rts_control(uart_ctxt, uart_ctxt->rts_control);

   tal_os_log(id, INFO, "-Uart_SetRtsControl(UART_SUCCESS)");
   return UART_SUCCESS;
}
...
static uint32 Uart_TxComplete(void *h, uint32 *tx_complete)
{
   ...
   tal_os_log(id, INFO, "+Uart_TxComplete(0x%08p, 0x%08p)", h, tx_complete);

   *tx_complete = HALuart_GetStatus(uart_hal) & HAL_UART_STATUS_TX_EMPTY;

   tal_os_log(id, INFO, "-Uart_TxComplete(UART_SUCCESS)");
   return UART_SUCCESS;
}
...
static uint32 Uart_TxSingleChar(void *h, uint8 character)
{
   ...
   tal_os_log(id, INFO, "+Uart_TxSingleChar(0x%08p, 0x%02X)", h, character);
   ...
   tal_os_log(id, INFO, "-Uart_TxSingleChar(%d)", result);
   return result;
}
                     ...
                     uint32 *bytes_written)
{
   ...
   HALUartObject *uart_hal = &uart_ctxt->uart_hal;
   uint32 result = UART_ERROR;
   boolean transmit_ok = FALSE;
   TAL_OS_PHYS *tx_memobj = uart_ctxt->tx_memobj;
   uint32 num_bytes;

   tal_os_log(id, INFO, "+Uart_Write(0x%08p, 0x%08p, 0x%08X, 0x%08p)",
   ...
   *bytes_written = 0;
   ...
   if (buffer_len == 0)
   {
      tal_os_log(id, ERROR, "Uart_Write: No data to send");
   }
   else
   {
      tal_os_sync_enter(uart_ctxt->tx_sync);
      ...
      {
         tal_os_log(id, ERROR, "Uart_Write: TX transfer in progress");
      }
      else if (uart_ctxt->tx_halt || uart_ctxt->tx_wait)
      {
         tal_os_log(id, ERROR, "Uart_Write: TX halt/wait in progress");
      }
      else
      {
   ...
   tal_os_log(id, INFO, "-Uart_Write(%d)", result);
   return result;
}
