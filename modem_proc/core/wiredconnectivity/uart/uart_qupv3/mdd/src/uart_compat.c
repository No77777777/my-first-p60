...
/*============================================================================
  ...
  $Header: //components/rel/core.mpss/10.0/wiredconnectivity/uart/uart_qupv3/mdd/src/uart_compat.c#1 $
...
uart_sio_port_id_to_device_id( sio_port_id_type port_id )
{
   ...
   else if( SIO_PORT_UART_CXM == port_id )
   {
      ...
      if( bool_prop  && !(DalPlatformInfo_Platform() == DALPLATFORMINFO_TYPE_RUMI) )
      {
   ...
   switch( device_id )
   {
      case UART_DEVICE_1:
         ...
         if( bool_prop == TRUE )
         {
            ...
            {
               dal_device_id = DALDEVICEID_UARTCXM;
            }
