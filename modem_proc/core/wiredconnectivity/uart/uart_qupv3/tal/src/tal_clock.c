...
$Header: //components/rel/core.mpss/10.0/wiredconnectivity/uart/uart_qupv3/tal/src/tal_clock.c#1 $
...
TAL_RESULT tal_clock_disable(TAL_CLOCK_HANDLE handle)
{
   ...
   {
	  if(handle->ahb_m_clk_id != 0)
	  {
		 dal_result = DalClock_DisableClock(handle->daldev_clock, handle->ahb_m_clk_id);
		 if (dal_result != DAL_SUCCESS) { tal_result = TAL_ERROR; }
	  }
	  if(handle->ahb_s_clk_id != 0)
	  {
		 dal_result = DalClock_DisableClock(handle->daldev_clock, handle->ahb_s_clk_id);
		 if (dal_result != DAL_SUCCESS) { tal_result = TAL_ERROR; }
	  }
   }
   return tal_result;
...
TAL_RESULT tal_clock_enable(TAL_CLOCK_HANDLE handle)
{
   ...
   if ( handle->manage_pclk )
   {
	  if(handle->ahb_m_clk_id != 0)
	  {
		 result = DalClock_EnableClock(handle->daldev_clock, handle->ahb_m_clk_id);
		 if (result != DAL_SUCCESS) { goto error; }
	  }
	  if(handle->ahb_s_clk_id != 0)
	  {
		 result = DalClock_EnableClock(handle->daldev_clock, handle->ahb_s_clk_id);
		 if (result != DAL_SUCCESS) { goto error; }
	  }
   }
   return TAL_SUCCESS;
...
TAL_RESULT tal_clock_open(TAL_CLOCK_HANDLE *phandle, void *device_id)
{
   ...
   handle->ahb_m_clk_id = 0;
   handle->ahb_s_clk_id = 0;
   ...
   if (handle->manage_pclk)
   {
      ...
      result = get_property_string(handle->dal_id, property_handle, "AhbMClockName", &clock_name, NULL);

      // Look up the clock ID for the peripheral clock
      if (result == DAL_SUCCESS && clock_name)
      {
         result = DalClock_GetClockId(handle->daldev_clock, clock_name, &handle->ahb_m_clk_id);
         if (result != DAL_SUCCESS)
         {
            DALSYS_LogEvent(handle->dal_id, DALSYS_LOGEVENT_ERROR, "Periph M GetClockId(%s) = %d", clock_name, result);
            goto error;
         }
      }
       // Read the name of the peripheral clock
      result = get_property_string(handle->dal_id, property_handle, "AhbSClockName", &clock_name, NULL);

      // Look up the clock ID for the peripheral clock
      if (result == DAL_SUCCESS && clock_name)
      {
         result = DalClock_GetClockId(handle->daldev_clock, clock_name, &handle->ahb_s_clk_id);
         if (result != DAL_SUCCESS)
         {
            DALSYS_LogEvent(handle->dal_id, DALSYS_LOGEVENT_ERROR, "Periph S GetClockId(%s) = %d", clock_name, result);
            goto error;
         }
      }
   }
