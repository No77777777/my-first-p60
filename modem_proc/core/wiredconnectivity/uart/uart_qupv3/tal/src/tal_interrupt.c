...
$Header: //components/rel/core.mpss/10.0/wiredconnectivity/uart/uart_qupv3/tal/src/tal_interrupt.c#1 $
...
#define MAX_IRQS       5     // (1) UART (2) RX GPIO (3) CXM Msg (4) CXM Misalignment
...
TAL_RESULT tal_interrupt_open(TAL_INTERRUPT_HANDLE *phandle, void *device_id)
{
   ...
   if (!workloop_handle)
   {
      result = DALSYS_RegisterWorkLoopEx( "uart_intr_wl",
                                          0x1000,
                                          175, 
                                          ctxt->max_irqs, 
                                          &workloop_handle, 
