...
Copyright (c) 2014 - 2021 by Qualcomm Technologies, Incorporated.  All Rights Reserved.
...
$Header: //components/rel/rfc_habanero.mpss/1.11/target/divar/src/rfc_card_factory.cpp#2 $

when       who     what, where, why
--------------------------------------------------------------------------------
09/02/22   tej     FR69305 To add support for new error codes for QMI_DMS_SET_OPERATING_MODE
01/09/20   akm     Disable rfc memory reclaim
...
*/
rfm_init_status_type_e_type rfc_set_card (rf_hw_type rf_hw)
{
  static rfm_init_status_type_e_type rfcard_initalize_status = RFM_INIT_FAILURE_INVALID;
 ...
 // int   num_card_loaded, n;

  /* Check if the Card is set already */
    if(rfcard_initalize_status == RFM_INIT_FAILURE_NONE)
  {
    return RFM_INIT_FAILURE_NONE; /* return */
  }
#if 0  
  ...
  { 
    MSG_FATAL("WARNING! RFC HWID %lu has not been loaded for memory reclamation! This is only acceptable if the card does not belong to dynrec_section", rf_hw,0,0);
  }
  if ((n = mcfg_dynrec_reclaim(&num_card_loaded)) != 0)
  { 
    MSG_FATAL("WARNING! Dynrec_section cannot be reclaimed. Current HWID = %lu", rf_hw,0,0);
  }

#endif

  if( !rfc_factory(rf_hw, &vreg_mgr_p, &rfc_data) )
  {
    RF_MSG_1 ( RF_ERROR, "rfc_set_card: RFC ERROR Failed rfc_factory API HWID %lu",
               rf_hw );
  ...
  else
  {
    ...
    {
	  rfcard_initalize_status = RFM_INIT_FAILURE_NONE;
  ...
  return rfcard_initalize_status;
} /* rfc_set_card() */
