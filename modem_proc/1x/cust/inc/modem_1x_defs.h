...
$Header: //components/rel/1x.mpss/8.0/cust/inc/modem_1x_defs.h#2 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
05/18/21   vaa     Fix for compilation errors in MSIM disable build flavor
10/03/18   srk     Undefine TSYNC feature.
11/20/17   ab      1x2GTA diag cmd changes
06/21/16   ppr     Chgs to rem FEATURE_MODEM_VoLTE_TO_1X feature defn
04/14/16   srk     Enable optimized Tx Trigger.
02/05/16   pk      1x2GTA Feature changes
12/24/15   eye     Enable RX IQ vector verification on RUMI CRM.
11/09/15   agh     Support QSH framework to collect periodic/event metrics
11/05/15   eye     Enabled JACALA MCVS feature.
09/30/15   ab      Enable FL DRX on Tabasco
09/29/15   agh     Remove all references to MVS and cleanup WON feature
08/21/15   ab      Undef ASDIV feature for RUMI builds
24/08/15   sjo     Mainline the feature FEATURE_1X_CUST_VOICE_SO
08/16/15   srk     Revert previous changes.
08/14/15   srk     Feature cleanup.
06/16/15   eye     Enable feature flag FEATURE_MODEM_1X_TA_MCVS for TA
06/11/15   srk     Fix Atlas compilation errors in Full Stack build.
06/03/15   srk     Cleanup target level Atlas feature and enable RF support.
06/03/15   pk      Enable SW demback and QTA Feature for TA.2.0
05/21/15   agh     Enable feature flag FEATURE_1X_NSET_OPTIMIZATION
05/18/15   ab      Enable 1X QTA Feature
05/18/15   pk      Define FEATURE_ONEX_SUPPORTS_RF_MEAS_INTF_V2 for TA.2.0
05/15/15   jh      Undef FEATURE_MODEM_1X_SUPPORTS_RF for Atlas
05/15/15   ab      Define FEATURE_MODEM_1X_SW_DEMBACK for Thor
05/05/15   eye     Added new macro for VFR register usage.
04/21/15   srk     Unification of IDLE DIV algorithm for all CRAT modes.
04/21/15   srk     DRDSDS and BOLT REVISIT Feature cleanup
04/13/15   pk      Changes to support new RF Meas API for IRAT & QTA
04/13/15   ppr     Chgs to do reg during next 1x prot_act when 4G set to OFF
04/09/15   pap     Add changes to enable voice adapter by default.
04/02/15   srk     Add support for ATLAS modem.
03/19/15   agh     Changes to support Voice Adapter based Audio architecture
03/17/15   agh     Mainline feature flag FEATURE_LTE_RESEL_SPLMN_SUPPORT
...
**                      ATLAS
** ------------------------------------------------------------------------ */
#if defined( FEATURE_MODEM_1X_MPSS_AT_1_0 ) || \
    defined( FEATURE_MODEM_1X_MPSS_AT_1_1 ) || \
    defined( FEATURE_MODEM_1X_MPSS_AT_2_0 )

  /* REVISIT feature for ATLAS */
  #define FEATURE_MODEM_1X_ATLAS_REVISIT

  /* Leverage from THOR */
  #define FEATURE_MODEM_1X_THOR

#endif /* ATLAS modem feature */
...
** ------------------------------------------------------------------------ */
  ...
  #ifdef FEATURE_MODEM_1X_MPSS_TA_2_2
  #define FEATURE_MODEM_1X_JACALA_MCVS
  #endif /* FEATURE_MODEM_1X_MPSS_TA_2_2 */
 ...
 /* MCVS changes for TA */
  #define FEATURE_MODEM_1X_TA_MCVS
...
/* Optimization to add previous serving pilot as neighbor of current serving
 * pilot after handoff to handle incorrect network configurations of NLM */
#define FEATURE_1X_NSET_OPTIMIZATION

/* Feature flag to guard the changes made to support QSH framework by 1X_CP */
#define FEATURE_1XCP_QSH_SUPPORT

#define FEATURE_1X_TUNEAWAY_TEST

/* ------------------------------------------------------------------------
** RUMI related features
** ------------------------------------------------------------------------ */
#if defined( T_RUMI_EMULATION ) || defined( FEATURE_MODEM_1X_ATLAS_REVISIT )
  #define FEATURE_MODEM_1X_RUMI
#endif /* T_RUMI_EMULATION || FEATURE_MODEM_1X_ATLAS_REVISIT */

#ifdef FEATURE_MODEM_1X_RUMI

  /* Undefine RF as we do not want any RF calls to be made on a RUMI */
  #undef FEATURE_MODEM_1X_SUPPORTS_RF
