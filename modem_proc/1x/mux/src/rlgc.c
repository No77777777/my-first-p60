...
PUBLIC FUNCTIONS
  ...
  rpc_calculate_tx_rate_adj
  rpc_get_sch_scrm_rate
  rpc_set_scrm_allowed
...
$Header: //components/rel/1x.mpss/8.0/mux/src/rlgc.c#1 $ $DateTime: 2019/06/11 23:51:40 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
10/03/13   srk     Fix R-FCH gating issue for Full rate frames.
09/06/13   srk     Unification of T2P boost for RC3 and RC8.
09/06/13   srk     Do PCG gating in FW for R-FCH gating mode.
08/21/13   srk     Update the FCH2PICH gain ratio for RC3 DTX with boost.
...
08/02/12   srk     Moved RF_NON_LINEAR_BIAS_CORRECTION to rlgc_i.h
05/24/12   srk     Corrected T2P values for RC8.
...
04/14/11   jtm     Initialize extra_gain variables in rpc_get_sch_scrm_rate().
...
10/30/10   jtm     Moved featurization using FEATURE_IS2000_R_SCH inside
                   rpc_get_sch_scrm_rate() and rpc_set_scrm_allowed().
06/03/10   jtm     Corrected which ACS gain is used when R-ACKCH 1&2 gain
                   ratios are calculated.
04/29/10   trc     Add rpc_set_scrm_allowed() API to allow external client SCRM
                   inhibition, based upon arbitrary parameters (e.g. thermal)
04/12/10   jtm     Corrected use of multichannel gains for the fchackch. Use
                   the calculated value when R-SCH is assigned, 0 otherwise.
                   Cleaned up RC8 gain ratio processing.
02/04/10   jtm     Removed FEATURE_PLT. Typedef'd txc_rpc structure and
                   so it could be used in QTF testing.
02/02/10   jtm     Added sign extension calculation on the active cell size
                   gain adjust value received in the RCPM prior to adding it
                   to the R-ACKCH/R-PICH gain ratios.
...
06/12/09   jtm     Added check for null ptr in rpc_rcpm_cmd().
...
04/01/09   jtm     Removed include for rf1x.h
...
07/21/08   jtm     Fixed Klockwork Errors. Added RLGC_UPPER_BOUNDS_CHECK macro.
...
/* Nominal Attribute Gain Table for Rate Set 1 RC3/RC8 for R-FCH
 * and R-DCCH */
const rl_att_gain_type basic_rs1_rl_nom_att_gains[RLGC_MAX][ENC_NUM_FRAC_RATES]=
                 {
                   /* RC3 T2P values */
                   {
                     {  30, 0}, /* FULL 9600 bps */
...
/* Rate Set 1 Nominal Attribute Gain for R-ACKCH */
const rl_att_gain_type rackch_rs1_rl_nom_att_gains [NUM_RACK_CHAN_MOD_TYPES] =
  {
    {24, 0}, /* BPSK */
    {72, 0}  /* OOK  */
  };
...
int16 rpc_get_sch_pilot_ref_level(enc_sch_rate_type, cai_sch_coding_type);
...
#ifdef FEATURE_IS2000_1X_ADV
txc_linear_gain_ratio_type rpc_compute_ratios( int16 fch, int16 fch_ack,
                                                int16 sch, int16 sch_ack,
                                               byte chan_mask );
#endif /* FEATURE_IS2000_1X_ADV */
...
int2 rpc_get_fch2pich_gain_ratio(enc_rate_type, boolean);
...
)
{
       ...
       ((enc_fch_radio_config_type) cmd_ptr->tc.rev_fch.rc >= ENC_FCH_RC_3))
   {
      ...
      txc_rpc.fch_rate_set =
        (enc_rate_set_type) txc_get_rev_link_rate_set (cmd_ptr->tc.rev_fch.rc);

#ifdef FEATURE_IS2000_1X_ADV
      txc_rpc.fch_ack_on = FALSE;
      txc_rpc.sch_ack_on = FALSE;

      if( txc_rpc.fch_rc == CAI_RC_8 )
      {
        /* FCH ACK Channel may be used since FCH is RC8 */
        txc_rpc.fch_ack_on = TRUE;

        txc_rpc.fch_ack_multi_chan_adj_gain = 0;
        txc_rpc.sch_ack_multi_chan_adj_gain = 0;
      }
#endif /* FEATURE_IS2000_1X_ADV */
   }
       ...
       ((enc_fch_radio_config_type) cmd_ptr->tc.rev_dcch.rc >= ENC_FCH_RC_3))
   {
      ...
      txc_rpc.dcch_rate_set =
        (enc_rate_set_type) txc_get_rev_link_rate_set (cmd_ptr->tc.rev_dcch.rc);
...
)
{
  ...
  if (rlgc_ptr->fch_rlgc_op.mode == MCTRANS_RLGC_START)
  {
    ...
    txc_rpc.fch_rate_set =
        (enc_rate_set_type) txc_get_rev_link_rate_set (rlgc_ptr->fch_rlgc_op.rc);
  ...
  if (rlgc_ptr->dcch_rlgc_op.mode == MCTRANS_RLGC_START)
  {
    ...
    txc_rpc.dcch_rate_set =
        (enc_rate_set_type) txc_get_rev_link_rate_set (rlgc_ptr->dcch_rlgc_op.rc);
  }
  ...
  {
    txc_rpc.sch_on = TRUE;
    txc_rpc.sch_rate = (enc_sch_rate_type)((uint32) rlgc_ptr->sch_rlgc_op.sch_rate + 3);
    txc_rpc.sch_code_type = rlgc_ptr->sch_rlgc_op.sch_code_type;
    txc_rpc.sch_rc   = rlgc_ptr->sch_rlgc_op.rc;

    #ifdef FEATURE_IS2000_1X_ADV
    if( txc_rpc.sch_rc == CAI_RC_8 )
    {
      /* SCH ACK Channel may be used since SCH is RC8 */
      txc_rpc.sch_ack_on = TRUE;
    }
    #endif /* FEATURE_IS2000_1X_ADV */
  }
  ...
  if (txc_rpc.sch_on)
  {
       ...
       )
    {
      ...
      if ( txc_rpc.sch_rc == CAI_RC_8 )
      {
        /* Since SCH is on we must calculate the multi-channel gain
           adjustment for the ACK Channels. Both FCH_ACK and SCH_ACK
           can share the same multichannel adjustment.

           ACK channel is only RC 8 which is Rate Set 1 (9600)
        */
        txc_rpc.fch_ack_multi_chan_adj_gain = txc_rpc.sch_ack_multi_chan_adj_gain =
          rpc_get_multi_chan_adj_gain ( txc_rpc.sch_rate, ENC_RATE_9600 );
      }
#endif /* FEATURE_IS2000_1X_ADV */
    }
    else
...
{
  int16 tx_open_loop = 0;
  int16 tx_gain_adj  = 0;
  int16 tx_rate_adj  = 0;
  int16 tx_pwr_limit = 0;
  ...
  **------------------------------------------------------*/
  ...
  #ifdef FEATURE_MODEM_1X_SUPPORTS_RF
  tx_open_loop = rfm_1x_get_tx_open_loop(txc_get_tx_device());
  /* gain adjust value in 1/12 dB units */
  tx_gain_adj = rfm_1x_get_tx_gain_adjust(txc_get_tx_device());
  #endif /* FEATURE_MODEM_1X_SUPPORTS_RF */
  ...
  tx_pwr_limit = txc_get_max_tx_agc_dbm12();
...
{
  int16  sch2pich, fch2pich, dcch2pich, pch_adj_gain;
  int16  scrm_tx_extra_gain = 0, current_tx_extra_gain = 0;
  ...
  txc_rfm_1x_txc_pwr_limit_type    scrm_proposed_pwr_limit = {0};
  #ifdef FEATURE_MODEM_1X_SUPPORTS_RF
  txc_rfm_1x_txc_chan_config_type  scrm_proposed_chan_config = {0};
  #endif /* FEATURE_MODEM_1X_SUPPORTS_RF */

  /* If the external flag is set to inhibit SCRM, do so */
  if ( rlgc_inhibit_scrm )
  {
    M1X_MSG( MUX, LEGACY_HIGH,
      "SCRM inhibition in effect (%d rate requested)",
      curr_sch_rate);
    return( ENC_NUM_SCH_RATES );
  }

  /* compute current the extra gain for SCRM without the current config power limit */
  current_tx_extra_gain = ( tx_extra_gain - txc_get_max_tx_agc_dbm12() );
  ...
  if ((txc_rpc.dcch_on) && (txc_rpc.fch_on))
  {
    for (rate_index = (curr_sch_rate); rate_index >= ENC_SCH_FULL_RATE; rate_index--) //lint !e734 Loss of precision - 31 to 8 bits
    {
      ...
      */
      ...
      M1X_MSG( MUX, LEGACY_HIGH,
        "fch2pich %d sch2pich %d dcch2pich %d",
        fch2pich,
        sch2pich,
        dcch2pich);
      ...
      rfm_1x_get_tx_pwr_limit_adj( txc_get_tx_device(),
                                   &scrm_proposed_chan_config,
                                   &scrm_proposed_pwr_limit );
      #endif /* FEATURE_MODEM_1X_SUPPORTS_RF */
      ...
      M1X_MSG( MUX, LEGACY_HIGH,
        "SCRM rfpl %d,eg %d,plra %d",
        scrm_proposed_pwr_limit.tx_pwr_limit,
        scrm_tx_extra_gain,
        pilot_ref_lvl_adj);
      ...
      */
      ...
      {
        M1X_MSG( MUX, LEGACY_HIGH,
          "NEP,lower from %d rate, to %d rate ",
          curr_sch_rate,
          rate_index);
        txc_rpc.fch_multi_chan_adj_gain = 0;
        return(rate_index);
...
#endif /* FEATURE_IS2000_REL_A_SVD */
  {
    for (rate_index = (curr_sch_rate); rate_index >= ENC_SCH_FULL_RATE; rate_index--) //lint !e734 Loss of precision - 31 to 8 bits
    {
      ...
      */
      ...
      M1X_MSG( MUX, LEGACY_HIGH,
        "fch2pich %d sch2pich %d mulchgain %d",
        fch2pich,
        sch2pich,
        txc_rpc.fch_multi_chan_adj_gain);
  ...
  else if (txc_rpc.dcch_on)
  {
    for (rate_index = (curr_sch_rate); rate_index >= ENC_SCH_FULL_RATE; rate_index--) //lint !e734 Loss of precision - 31 to 8 bits
    {
      ...
      M1X_MSG( MUX, LEGACY_HIGH,
        "dcch2pich %d sch2pich %d mulchgain %d",
        dcch2pich,
        sch2pich,
        txc_rpc.dcch_multi_chan_adj_gain);
...
{
  /* We don't know how rapidly this API may be beaten upon by a client,
     so only update the flag & spit out a message upon change.  Also,
     sanitize rlgc_inhibit_scrm such that it is only 0 or 1, despite what
     the client passes as the zero / non-zero parameter. */
  if ( rlgc_inhibit_scrm != !is_scrm_allowed )
  {
    rlgc_inhibit_scrm = !is_scrm_allowed;
    M1X_MSG( MUX, LEGACY_HIGH,
      "SCRM inhibition flag changed = %d",
      rlgc_inhibit_scrm);
  }
} /* rpc_set_scrm_allowed() */
...
)
{
  ...
  int16  multi_chan_adj_gain;
   ...
   /* Retrieve the nominal attribute gain and the attribute adjustment gain */
  if ( txc_rpc.fch_rate_set == ENC_RATE_9600 )
  {
    /* Rate Set 1 */
    nom_att_gain = ( txc_rpc.fch_rc == CAI_RC_3 )?
                   basic_rs1_rl_nom_att_gains[RLGC_RC3][rate].nom_att_gain:
                   basic_rs1_rl_nom_att_gains[RLGC_RC8][rate].nom_att_gain;
  ...
  /* If 1/8th rate gating is on, then use the standard defined
     special FCH gains instead of normal FCH gains */
  if ( ( rate == ENC_8TH_RATE ) && ( txc_rpc.rev_fch_gating_on ) )
  {
    /* Update the standard defined nominal attribute gain */
    nom_att_gain = ( txc_rpc.fch_rate_set == ENC_RATE_9600 )?
                     gated_rs1_rl_nom_att_gains.nom_att_gain:
                     gated_rs2_rl_nom_att_gains.nom_att_gain;

    if ( tx_frame_rate == ENC_8TH_RATE )
    {
      /* Disable Device only DTX for this frame as it is already gated */
      txc_device_only_dtx_param.dtx_frame          = FALSE;

      /* The DTX mask needs to be set to follow 2 ON and 2 OFF gating
         pattern for the standard defined FCH gating to happen at FW */
      txc_device_only_dtx_param.dtx_mask           = DTX_MASK_ENABLED;

      /* When 1/8th rate gating is on, the FCH gains are already boosted by
         4.625 dB. Hence there is no need for additional 5 dB boost for DTX */
      txc_device_only_dtx_param.dtx_t2p_boost_dbq3 = 0;
    }
  }

  if ( tx_frame_rate == rate )
  {
    DTX_DBG_MSG( "1x DTX: Rate = %d, Nominal Gain = %d dB, DTX = %d, "
                 "DTX Mask = 0x%04X, FCH/PICH Gain Boost = %d dB",
                 rate,
                 nom_att_gain>>3,
                 txc_device_only_dtx_param.dtx_frame,
                 txc_device_only_dtx_param.dtx_mask,
                 txc_device_only_dtx_param.dtx_t2p_boost_dbq3>>3 );
  }
...
)
{
   ...
   if (txc_rpc.dcch_rate_set == ENC_RATE_9600)
   {
      /* Rate Set 1 */
      nom_att_gain =
           basic_rs1_rl_nom_att_gains[RLGC_RC3][ENC_FULL_RATE].nom_att_gain;
...
)
{
   ...
   if (rate >= ENC_SCH_2X_RATE)
   {
     uint8 index = (uint8)(RLGC_UPPER_BOUNDS_CHECK(
                            (uint32) rate - (uint32) ENC_SCH_2X_RATE, Max_Num_Of_High_Rates));
   ...
   else if (rate == ENC_SCH_FULL_RATE)
   {
      ...
      else
      {
        /* Rate Set 1 */
      nom_att_gain =
        ( txc_rpc.sch_rc == CAI_RC_3 )?
        basic_rs1_rl_nom_att_gains[RLGC_RC3][ENC_FULL_RATE].nom_att_gain:
        basic_rs1_rl_nom_att_gains[RLGC_RC8][ENC_FULL_RATE].nom_att_gain;
         att_adj_gain = basic_rs1_rl_att_adj_gains[ENC_FULL_RATE].nom_att_gain;
      }
   }
...
int2 rpc_get_fch_ack2pich_gain_ratio ( txc_active_cell_size_types acs )
{
  ...
  /* Calculate the ACKCH/PICH ratio. */

  /* Determine which gain adjust we use from the RCPM based on the number of
     cells in the active set.

     Note: This value is expressed as a 7bit sign extended value in .125dB
     units the range from -8dB to 8dB inclusive. We must perform sign
     extension on the value provided to us in the RCPM to obtain the
     proper 7bit signed gain.
  */
  if ( acs == TXC_ACS_IS_1 )
  {
    /* Chn adj gain for Rev Ack Chn (Num Cells =1) with sign extention */
    acs_gain_adj = ((txc_rcpm_data.rev_gain_adj_acs1 & 0x7F) ^ 0x40) - 0x40;
  }
  else
  {
    /* Chn adj gain for Rev Ack Chn (Num Cells >1) with sign extension */
    acs_gain_adj = ((txc_rcpm_data.rev_gain_adj_acs2plus & 0x7F) ^ 0x40) - 0x40;
  }

  /* Retrieve the nominal attribute gain and the attribute adjustment gain. */

  /* 2.1.3.10 Reverse Acknowledgment Channel -
     When acknowledging successful reception of a Forward Fundamental Channel
     frame, the Reverse Acknowledgment Channel shall use OOK (ON-OFF keying)
     modulation. When acknowledging successful reception of a Forward Supplemental
     Channel frame, the Acknowledgment Channel shall use BPSK modulation.
  */

  /* Determine multiple channel adjustment */
  multi_chan_adj_gain = ( (txc_rpc.sch_on == FALSE) ? 0 : txc_rpc.fch_ack_multi_chan_adj_gain);

  return (
           rackch_rs1_rl_nom_att_gains[RACK_CHAN_MOD_OOK].nom_att_gain
           + acs_gain_adj
           + txc_rpc.fch_ack_chan_adj_gain
           - multi_chan_adj_gain
         );

} /* rpc_get_fch_ack2pich_gain_ratio */
...
int2 rpc_get_sch_ack2pich_gain_ratio ( txc_active_cell_size_types acs )
{
  ...
  /* Determine which gain adjust we use from the RCPM based on the number of
     ...
     Note: This value is expressed as a 7bit sign extended value in .125dB
     units the range from -8B to 8dB inclusive. We must perform sign
     extension on the value provided to us in the RCPM to obtain the
     proper 7bit signed gain.
  */
  if ( acs == TXC_ACS_IS_1 )
  ...
  */
  ...
  return (
           rackch_rs1_rl_nom_att_gains[RACK_CHAN_MOD_BPSK].nom_att_gain
           +  acs_gain_adj
           +  txc_rpc.sch_ack_chan_adj_gain
           -  txc_rpc.sch_ack_multi_chan_adj_gain
         );

} /* rpc_get_sch_ack2pich_gain_ratio */
#endif /* FEATURE_IS2000_1X_ADV */
...
void rpc_init_att_adj_gain (void)
{
   ...
   for (rate = ENC_FULL_RATE; rate < ENC_NUM_FRAC_RATES; rate++)
   {
      basic_rs1_rl_att_adj_gains[rate] = att_adj_gain;
      basic_rs2_rl_att_adj_gains[rate] = att_adj_gain;
   }

   for (rate_index = 0; rate_index < Max_Num_Of_High_Rates; rate_index++)
   {
      high_rs1_rl_att_adj_gains[0][rate_index] = att_adj_gain;
      high_rs1_rl_att_adj_gains[1][rate_index] = att_adj_gain;
      high_rs2_rl_att_adj_gains[0][rate_index] = att_adj_gain;
      high_rs2_rl_att_adj_gains[1][rate_index] = att_adj_gain;
   }
} /* rpc_init_att_adj_gain */
...
DESCRIPTION
  Initializes Cahnnel Adjust Gains for FCH, DCCH, SCH0, SCH1, RACKCH
...
#ifdef FEATURE_IS2000_1X_ADV
  txc_rpc.fch_ack_chan_adj_gain = 0;
  txc_rpc.sch_ack_chan_adj_gain = 0;
#endif /* FEATURE_IS2000_1X_ADV */
...
#ifdef FEATURE_IS2000_1X_ADV
/*===========================================================================
FUNCTION RPC_COMPUTE_RATIOS

DESCRIPTION
  Computes the linear gain ratios for FCH, PCH, SCH, ACKCH.
  DCCH gain ratios are NOT calculated since it is not required for
  RC8 (1x Advanced)
...
RETURN VALUE
  Relative gains for FCH, PCH, SCH, DCCH, ACKCH.

SIDE EFFECTS
  None.
===========================================================================*/
txc_linear_gain_ratio_type rpc_compute_ratios
(
  int16 fch,
  int16 fch_ack,
  int16 sch,
...
{
  txc_linear_gain_ratio_type gain_ratios =
  {
    /* initialize all ratios to 0 */
    0,  /* FCH     */
    0,  /* FCH_ACK */
    0,  /* SCH     */
    0   /* SCH_ACK */
  ...
  if ( chan_mask & FCH_GAIN_M )
  {
    gain_ratios.fch_gain_ratio =
      (word) convert_dBq8_to_lin( ( ( fch << 4 ) + RF_NON_LINEAR_BIAS_CORRECTION ), 11, 16 );

    gain_ratios.fch_ack_gain_ratio =
      (word) convert_dBq8_to_lin( ( ( fch_ack << 4 ) + RF_NON_LINEAR_BIAS_CORRECTION ), 11, 16 );
  }

  if ( chan_mask & SCH_GAIN_M )
  {
    gain_ratios.sch_gain_ratio =
      (word) convert_dBq8_to_lin( ( ( sch << 4 ) + RF_NON_LINEAR_BIAS_CORRECTION ), 11, 16 );

    gain_ratios.sch_ack_gain_ratio =
      (word) convert_dBq8_to_lin( ( ( sch_ack << 4 ) + RF_NON_LINEAR_BIAS_CORRECTION ), 11, 16 );
  }
...
DESCRIPTION
  Computes the relative gains for FCH, PCH, SCH, DCCH, ACKCH.
  Resulting gains have 15 bit resolution to output to MDSP.
...
)
{
  ...
  #ifdef FEATURE_IS2000_1X_ADV
  if ( adj0_rec->rev_ackch_incl )
  {
    txc_rpc.fch_ack_chan_adj_gain = (int8) adj0_rec->rev_ackch_chan_adj_gain;
    txc_rpc.sch_ack_chan_adj_gain = (int8) adj0_rec->rev_ackch_chan_adj_gain;
  }
  #endif /* FEATURE_IS2000_1X_ADV */
...
)
{
   ...
   {
      M1X_MSG( MUX, LEGACY_ERROR,
        "Num of recs in PCNM > CAI_MAX_NUM_RPC_RECS" );
...
void rpc_mean_code_chan_output_power (void)
{
   ...
   enc_rate_type  rate = ENC_FULL_RATE;
   txc_gain_type  gain;

#ifdef FEATURE_IS2000_1X_ADV
  int16 fch_ack2pich_acs1 = 0,
        fch_ack2pich_acs2plus = 0,
        sch_ack2pich_acs1 = 0,
        sch_ack2pich_acs2plus = 0;
   ...
   if (txc_rpc.fch_on && txc_rpc.dcch_on)
   {
     for (rate = ENC_FULL_RATE; rate < ENC_NUM_FRAC_RATES; rate++)
     {
       if (txc_rpc.sch_on)
       {
         ...
         /* get DCCH to PCH gain ratio for R-DCCH  */
       ...
       else
       {
         ...
         */
        gain = rpc_compute_gains (fch2pich,0,rlgc_dcch_gain_ratio,
                                   FCH_GAIN | DCCH_GAIN, RPC_TOT_DIG_PWR_255 );
      ...
      for (rate = ENC_FULL_RATE; rate < ENC_NUM_FRAC_RATES; rate++)
      {
         if (txc_rpc.sch_on)
         {
            ...
            if( txc_rpc.fch_rc == CAI_RC_8 )
            {
              fch_ack2pich_acs1 = 0;
              fch_ack2pich_acs2plus = 0;
              if ( txc_rpc.fch_ack_on )
              {
                /* R-ACK channel for R-FCH
                   fch_ack_on is true when R-FCH RC = 8 and R-FCH is on.

                   Calculate gain for an active cell size of 1 and also
                   for and active cell size of 2 or more.
                */
                fch_ack2pich_acs1 = rpc_get_fch_ack2pich_gain_ratio( TXC_ACS_IS_1 );
                fch_ack2pich_acs2plus = rpc_get_fch_ack2pich_gain_ratio( TXC_ACS_IS_2_OR_MORE );
              }

              /* FCH is ON and FCH_ACK is ON, SCH is ON but inactive so no SCH_ACK
                Compute the linear ratios for both active cell size configurations,
                (<=1, 2 or more). Store both ratios.
              */
              ratio = rpc_compute_ratios( fch2pich, fch_ack2pich_acs1, 0, 0, FCH_GAIN );
              txc_set_pch_fch_sch_ratios ( rate, ratio, TXC_SCH_DTX, TXC_ACS_IS_1 );

              ratio = rpc_compute_ratios( fch2pich, fch_ack2pich_acs2plus, 0, 0, FCH_GAIN );
              txc_set_pch_fch_sch_ratios ( rate, ratio, TXC_SCH_DTX, TXC_ACS_IS_2_OR_MORE );
            }
            else
            #endif /* FEATURE_IS2000_1X_ADV */
            {
            ...
            */
            ...
            if( txc_rpc.fch_rc == CAI_RC_8 )
            {
              ...
              sch_ack2pich_acs1 = 0;
              sch_ack2pich_acs2plus = 0;
              if ( txc_rpc.sch_ack_on )
              {
                /* R-ACK channel for R-SCH
                   sch_ack_on is true when R-SCH is on and R-SCH RC = 8.

                   Calculate gain for an active cell size of 1 and also
                   for and active cell size of 2 or more.
                */
                sch_ack2pich_acs1 = rpc_get_sch_ack2pich_gain_ratio( TXC_ACS_IS_1 );
                sch_ack2pich_acs2plus = rpc_get_sch_ack2pich_gain_ratio( TXC_ACS_IS_2_OR_MORE );
              }

              /* FCH is ON and FCH_ACK is ON, SCH is ON and active so SCH_ACK is ON also
                 Compute the linear ratios for both active cell size configurations,
                 (<=1, 2 or more). Store both ratios.
              */
              ratio = rpc_compute_ratios( fch2pich, fch_ack2pich_acs1, rlgc_sch_gain_ratio,
                                          sch_ack2pich_acs1, FCH_GAIN | SCH_GAIN );
              txc_set_pch_fch_sch_ratios( rate, ratio, TXC_SCH_ACTIVE, TXC_ACS_IS_1 );

              ratio = rpc_compute_ratios( fch2pich, fch_ack2pich_acs2plus, rlgc_sch_gain_ratio,
                                          sch_ack2pich_acs2plus, FCH_GAIN | SCH_GAIN );
              txc_set_pch_fch_sch_ratios ( rate, ratio, TXC_SCH_DTX, TXC_ACS_IS_2_OR_MORE );
            }
            else
            #endif /* FEATURE_IS2000_1X_ADV */
         ...
         else
         {
            ...
            if( txc_rpc.fch_rc == CAI_RC_8 )
            {
              ...
              /* FCH is ON and FCH_ACK is ON, No SCH assigned so no SCH_ACK
                 Compute the linear ratios for both active cell size configurations,
                 (<=1, 2 or more). Store both ratios.
              */
              ratio = rpc_compute_ratios( fch2pich, fch_ack2pich_acs1,
                                          0, 0, FCH_GAIN );
              txc_set_pch_fch_sch_ratios ( rate, ratio, TXC_FCH_ONLY, TXC_ACS_IS_1 );


              ratio = rpc_compute_ratios( fch2pich, fch_ack2pich_acs2plus,
                                          0, 0, FCH_GAIN );
              txc_set_pch_fch_sch_ratios ( rate, ratio, TXC_FCH_ONLY, TXC_ACS_IS_2_OR_MORE );
            }
            else
            #endif /* FEATURE_IS2000_1X_ADV */
            {
...
)
{
  ...
  {
    uint8 index = (uint8)(RLGC_UPPER_BOUNDS_CHECK(
                           (uint32) sch_rate - (uint32) ENC_SCH_2X_RATE, Max_Num_Of_High_Rates));
    /*  reduce the enum rate down to within the bounds of
         the gains arrays */
    if (txc_rpc.sch_rc == CAI_RC_4)
    {
...
void rpc_calculate_multi_chan_adjustments ( void )
{
  ...
  if (txc_rpc.fch_rate_set == ENC_RATE_9600)
  {
    if( txc_rpc.fch_rc == CAI_RC_3 )
    {
      /* Rate Set 1 - RC3 FCH pilot reference level */
      fch_prl =
            basic_rs1_rl_nom_att_gains[RLGC_RC3][ENC_FULL_RATE].pilot_ref_lvl
          + basic_rs1_rl_att_adj_gains [ENC_FULL_RATE].pilot_ref_lvl;
    }
    else
    {
      /* Rate Set 1 - RC8 FCH pilot reference level */
      fch_prl =
            basic_rs1_rl_nom_att_gains[RLGC_RC8][ENC_FULL_RATE].pilot_ref_lvl
          + basic_rs1_rl_att_adj_gains [ENC_FULL_RATE].pilot_ref_lvl;
    }
  }
  else
  ...
  if (txc_rpc.dcch_rate_set == ENC_RATE_9600)
  {
    dcch_prl = basic_rs1_rl_nom_att_gains[RLGC_RC3][ENC_FULL_RATE].pilot_ref_lvl
              + basic_rs1_rl_att_adj_gains [ENC_FULL_RATE].pilot_ref_lvl;
  }
  else
  {
  ...
  {
    uint8 index = (uint8)(RLGC_UPPER_BOUNDS_CHECK(
                           (uint32) txc_rpc.sch_rate - (uint32) ENC_SCH_2X_RATE, Max_Num_Of_High_Rates));
    /*  reduce the enum rate down to within the bounds of
         the gains arrays */
    if (txc_rpc.sch_rc == CAI_RC_4)
    {
  ...
  else
  {
    ...
    {
      if( txc_rpc.sch_rc == CAI_RC_3 )
      {
        /* Rate Set 1 - RC3 SCH pilot reference level */
        sch_prl =
              basic_rs1_rl_nom_att_gains[RLGC_RC3][ENC_FULL_RATE].pilot_ref_lvl
            + basic_rs1_rl_att_adj_gains [ENC_FULL_RATE].pilot_ref_lvl;
      }
      ...
      {
        /* Rate Set 1 - RC8 SCH pilot reference level */
        sch_prl =
              basic_rs1_rl_nom_att_gains[RLGC_RC8][ENC_FULL_RATE].pilot_ref_lvl
            + basic_rs1_rl_att_adj_gains [ENC_FULL_RATE].pilot_ref_lvl;
      }
  ...
  #ifdef FEATURE_IS2000_1X_ADV
  if ( ( txc_rpc.fch_on ) && ( txc_rpc.fch_rc == CAI_RC_8 ) )
  {
    /* for fch ack ch */
    fch_ack_prl = rackch_rs1_rl_nom_att_gains[ RACK_CHAN_MOD_OOK ].pilot_ref_lvl;
  }

  if ( ( txc_rpc.sch_on ) && ( txc_rpc.sch_rc == CAI_RC_8 ) )
  {
    /* for sch ack ch */
    sch_ack_prl = rackch_rs1_rl_nom_att_gains[ RACK_CHAN_MOD_BPSK ].pilot_ref_lvl;
  }
  #endif /* FEATURE_IS2000_1X_ADV */
  ...
  #ifdef FEATURE_IS2000_1X_ADV
  max_pilot_reference_level = MAX( max_pilot_reference_level, fch_ack_prl );
  max_pilot_reference_level = MAX( max_pilot_reference_level, sch_ack_prl );
  #endif /* FEATURE_IS2000_1X_ADV */
  ...
  #ifdef FEATURE_IS2000_1X_ADV
  txc_rpc.fch_ack_multi_chan_adj_gain = max_pilot_reference_level - fch_ack_prl - sch_ack_prl;
  txc_rpc.sch_ack_multi_chan_adj_gain = max_pilot_reference_level - fch_ack_prl - sch_ack_prl;
  #endif /* FEATURE_IS2000_1X_ADV */
...
{
  if ((index >0) && (index < ((int) TXC_EACH_RATE_38400_5_V)))
  {
