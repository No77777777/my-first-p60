...
$Header: //components/rel/1x.mpss/8.0/mux/src/rxcrda.c#2 $ $DateTime: 2020/01/27 00:26:21 $ $Author: pwbldsvc $
...
12/13/11   srk     Feature clean up: Mainlined FEATURE_GPSONE_OVERRIDE_RDA,
                   FEATURE_RDA_CORRECTION, changed all FEATURE_IS2000_REL_A_*
                   to FEATURE_IS2000_REL_A
...
07/14/10   jtm     RC2 RDA optimized.
07/13/10   jtm     Change rxc_dcch_status to use the same type as FCH.
                   Fixed compiler warnings.
07/12/10   jtm     Modification to RC2 RDA using energy metrics for less
                   misclassification.
06/10/10   jtm     Eliminate dependency on log_dmss.h
05/10/10   jtm     Added API to update and retrieve F-FCH SER and FER stats.
                   Moved RC11 RDA retrieval from the MDSP into rxc_dec_rate_sel().
04/02/10   jtm     Replaced rlp_api.h with mdrrlp_api.h
02/04/10   jtm     Removed FEATURE_PLT.
10/19/09   jtm     Added support for RC11.
...
04/01/09   jtm     Removed log_codes.h include.
03/30/09   jtm     Eliminated implicit includes.
03/20/09   jtm     Cleaned up header file inclusions.
09/08/08   vlc     Use CDMA air interface enum values to remove dependency on
                   vocoder driver module.
...
rxcrda_counter_data_t rxcrda_data =
{
  0,  /*rxcrda_dtx_msg_enable*/
  0,  /*rxcrda_sch_dtx_fix_counter*/
  0   /*rxcrda_dcch_dtx_fix_counter*/
};
...
const rxc_frame_type rxcrda_convert_to_rxc_rates[6] =
{
  RXC_ERASURE_FRAME,  /*CAI_BLANK_RATE*/
  RXC_GOOD_FRAME,     /*CAI_EIGHTH_RATE */
  RXC_GOOD_FRAME,     /*CAI_QUARTER_RATE */
  RXC_GOOD_FRAME,     /*CAI_HALF_RATE*/
  RXC_GOOD_FRAME,     /*CAI_FULL_RATE*/
  RXC_DTX_FRAME       /*CAI_NULL_RATE*/
};

const cai_data_rate_type rxcrda_convert_to_cai_rates[3] =
{
...
typedef struct
{
  int32 rxcrda_dcch_quality_thresh;
  rxc_dcch_quality_type rxc_dcch_frame_quality;
  int rxcrda_norm_eng[4];
  ...
  ** the system frame time. */
  byte   pending_pc_acc_scaler;
  qword  pending_pc_acc_scaler_time;

  /* RDA logging variables */
  uint32 raw_rda_log_buffer[ RDA_LOG_BUFFER_SIZE/4 +1 ];
  rda_sub_record_header_type *rxc_rda_sub_record_hdptr;
  uint16 rxc_rda_log_buff_offset;
  byte rxc_rda_log_sch_rc;
} rxcrda_scalar_buffer_quality_t;

static rxcrda_scalar_buffer_quality_t rxcrda_static_data =
{
  0,                 /*rxcrda_dcch_quality_thresh*/
  RXC_POOR_CHANNEL,  /*rxc_dcch_frame_quality*/
  {0},               /*rxcrda_norm_eng*/
  1,                 /*pc_acc_scaler*/
  1,                 /*pending_pc_acc_scaler*/
  {0},               /*pending_pc_acc_scaler_time*/
  {0},               /*raw_rda_log_buffer*/
  NULL,              /*rxc_rda_sub_record_hdptr*/
  0,                 /*rxc_rda_log_buff_offset*/
  0                  /*rxc_rda_log_sch_rc*/
};

static log_puma_rda_information_type * rxc_rda_log_buffer =
  (log_puma_rda_information_type *) rxcrda_static_data.raw_rda_log_buffer;
...
void rxc_rda_frame_info_send_log(void)
{
  if (rxc_rda_log_buffer->hdr.len > 0)
  {
    ...
    (void) log_submit(rxc_rda_log_buffer);
...
uint8  *rxc_rda_log_frame_info_init( void )
{
  ...
  if (log_status(LOG_RDA_FRAME_INFO_2_C))
  {
    ...
    if (rxc_rda_log_buffer->hdr.len == 0)
    {
      ...
      (void) m1x_time_get(rxc_rda_log_buffer->hdr.ts);

      /*  Fill in beginning of record   */
      rxc_rda_log_buffer->srheader.num_sub_records = 0;

      /* initialize all the control variables */
      rxcrda_static_data.rxc_rda_sub_record_hdptr =
        &(rxc_rda_log_buffer->srheader);
      rxcrda_static_data.rxc_rda_log_buff_offset = 0;
    }
    else if (rxcrda_static_data.rxc_rda_sub_record_hdptr != NULL)
    {
      ...
      */
      ...
      rda_chan_rc_mask.chan_rc.frame_offset = rxc_common_data.rxc_frame_offset;
      ...
      if( rxc_so.for_fch.included )
      {
        ...
        /* RC fields are only 3bits wide. When we are in an RC11 call,
          it will be reported as 7 since 11 doesn't fit in 3bits. */
        rda_chan_rc_mask.chan_rc.fch_rc =
          ( ( rxc_so.for_fch.rc == CAI_RC_11 ) ? 7 : rxc_so.for_fch.rc );
      }
      ...
      if( rxc_frame_data.rxc_sch_frame_status != RXC_NO_FRAME )
      {
        ...
        /* RC fields are only 3bits wide. When we are in an RC11 call,
          it will be reported as 7 since 11 doesn't fit in 3bits. */
        rda_chan_rc_mask.chan_rc.sch_rc =
          ((rxcrda_static_data.rxc_rda_log_sch_rc == CAI_RC_11) ?
            7 : rxcrda_static_data.rxc_rda_log_sch_rc);
      }

      if((rda_chan_set_mask.mask !=
          rxcrda_static_data.rxc_rda_sub_record_hdptr->
            assigned_channels.mask) ||
         (rda_chan_rc_mask.mask !=
          rxcrda_static_data.rxc_rda_sub_record_hdptr->rc.mask))
      {
        /* We no longer have a valid sub-record, re-initialize the header pointer */
        rxcrda_static_data.rxc_rda_sub_record_hdptr =
          (rda_sub_record_header_type*)
          &(rxcrda_static_data.rxc_rda_sub_record_hdptr->
            reference_buf[rxcrda_static_data.rxc_rda_log_buff_offset]);
        rxcrda_static_data.rxc_rda_sub_record_hdptr->num_sub_records = 0;
        rxcrda_static_data.rxc_rda_log_buff_offset = 0;
      }

    }
    /* Need to init a new subrecord */
    if (rxcrda_static_data.rxc_rda_sub_record_hdptr != NULL)
    {
      if (rxcrda_static_data.rxc_rda_sub_record_hdptr->num_sub_records == 0)
      {
        /* reset the channel mask */
        rxcrda_static_data.rxc_rda_sub_record_hdptr->assigned_channels.mask =0;

        /* reset the channels RC */
        rxcrda_static_data.rxc_rda_sub_record_hdptr->rc.mask = 0;

        /********Initialize the channel assignment and RC for each channels ************/

        /* set the frame offset in the header sub-record */
        rxcrda_static_data.rxc_rda_sub_record_hdptr->rc.chan_rc.frame_offset =
        ...
        {
          rxcrda_static_data.rxc_rda_sub_record_hdptr->
            assigned_channels.individual_channels.fch = TRUE;

          /* RC fields are only 3bits wide. When we are in an RC11 call,
            it will be reported as 7 since 11 doesn't fit in 3bits. */
          rxcrda_static_data.rxc_rda_sub_record_hdptr->rc.chan_rc.fch_rc =
            ((rxc_so.for_fch.rc == CAI_RC_11) ? 7 : rxc_so.for_fch.rc);
        ...
        {
          rxcrda_static_data.rxc_rda_sub_record_hdptr->
            assigned_channels.individual_channels.dcch = TRUE;
          rxcrda_static_data.rxc_rda_sub_record_hdptr->rc.chan_rc.dcch_rc =
            rxc_so.for_dcch.rc;
        }
        /* check for SCH */
        if (rxc_frame_data.rxc_sch_frame_status!=RXC_NO_FRAME)
        {
          ...
          rxcrda_static_data.rxc_rda_sub_record_hdptr->
            assigned_channels.individual_channels.sch0 = TRUE;

          /* SCH RC */

          /* RC fields are only 3bits wide. When we are in an RC11 call,
            it will be reported as 7 since 11 doesn't fit in 3bits. */
          rxcrda_static_data.rxc_rda_sub_record_hdptr->rc.chan_rc.sch_rc =
            ((rxcrda_static_data.rxc_rda_log_sch_rc == CAI_RC_11) ?
              7 : rxcrda_static_data.rxc_rda_log_sch_rc);
        ...
        rxc_rda_log_buffer->hdr.len +=
          FPOS(rda_sub_record_header_type,reference_buf);
      }
      rda_sub_record_ptr =
        &rxcrda_static_data.rxc_rda_sub_record_hdptr->
          reference_buf[rxcrda_static_data.rxc_rda_log_buff_offset];
...
void rxc_rda_log_frame_info
(
  cai_data_rate_type fch_dec_rate,
  cai_data_rate_type dcch_dec_rate
)
#else
void rxc_rda_log_frame_info (cai_data_rate_type dec_rate)
#endif /* FEATURE_IS2000_REL_A_SVD */
{
  ...
  if (rda_sub_record_ptr != NULL)
  {
    ...
    (
      rxcrda_static_data.rxc_rda_sub_record_hdptr->
        assigned_channels.individual_channels.fch
    )
    {
      /* fill in the RDA data for FULL, Haft, Quater and Eight frame */
      for (index = 0; index < 4; index++)
      {
        fch_rda_sub_record = (fch_rda_sub_record_type *)
                               (rda_sub_record_ptr + total_byte);

        //lint !e732 Loss of sign
        fch_rda_sub_record->norm_ren_energy =
          rxcrda_static_data.rxcrda_norm_eng[index];
        fch_rda_sub_record->ser = rxc_fch_status.ser[index];
#ifdef FEATURE_IS2000_REL_A_SVD
        fch_rda_sub_record->decision = (uint8) fch_dec_rate;
#else
        fch_rda_sub_record->decision = (uint8) dec_rate;
#endif /* FEATURE_IS2000_REL_A_SVD */
      ...
      rxcrda_static_data.rxc_rda_sub_record_hdptr->num_sub_records += 4;

    }
    if
    (
     rxcrda_static_data.rxc_rda_sub_record_hdptr->
       assigned_channels.individual_channels.dcch
    )
    {
      /* fill in the DCCH RDA data for FULL frame */
      /* note that, the data for DCCH is FCH with Full frame */
      dcch_rda_sub_record = (dcch_rda_sub_record_type *)
                              (rda_sub_record_ptr + total_byte);

      dcch_rda_sub_record->quality_threshold =
        rxcrda_static_data.rxcrda_dcch_quality_thresh;//lint !e732 Loss of sign
      dcch_rda_sub_record->quastat.quality_decision =
        (uint8) rxcrda_static_data.rxc_dcch_frame_quality;

      dcch_rda_sub_record->pc_acc = muxmdsp_get_pc_acc();
      dcch_rda_sub_record->fch_nt_io  = (uint16) muxmdsp_get_fch_nt_io();

#ifdef FEATURE_IS2000_REL_A_SVD
      dcch_rda_sub_record->decision = (uint8) dcch_dec_rate;
      dcch_rda_sub_record->ser = rxc_dcch_status.ser[DEC_RATE_FULL-1];
      dcch_rda_sub_record->quastat.status =
        rxc_dcch_status.status[DEC_RATE_FULL-1];
      dcch_rda_sub_record->norm_ren_energy =
        rxc_dcch_status.energy[DEC_RATE_FULL-1];//lint !e732 Loss of sign
#else
      dcch_rda_sub_record->decision = (uint8) dec_rate;
      ...
      rxcrda_static_data.rxc_rda_sub_record_hdptr->num_sub_records += 1;
    }

    if
    (
     rxcrda_static_data.rxc_rda_sub_record_hdptr->
       assigned_channels.individual_channels.sch0
    )
    {
      /* fill in the SCH RDA data for FULL frame */
      sch_rda_sub_record = (sch_rda_sub_record_type *)
                             (rda_sub_record_ptr + total_byte);

      sch_rda_sub_record->iq_acc = muxmdsp_get_sch_iq_acc();
      sch_rda_sub_record->sch_nt_io  = (uint16) muxmdsp_get_sch_nt_io();

      if (rxc_sch.code_type == DEC_CONVOLUTIONAL) /* convolutional code */
      {
        sch_rda_sub_record->norm_ren_energy
                                = rxc_frame_data.rxc_sch_status.conv_sch_status.energy;//lint !e732 Loss of sign
        ...
        sch_rda_sub_record->code_info.dev_sch_conv.status
                                = rxc_frame_data.rxc_sch_status.conv_sch_status.status;
        sch_rda_sub_record->rate = (uint16) rxc_sch.sch_rate;
        sch_rda_sub_record->decision = (uint8) rxc_frame_data.rxc_sch_frame_status;
        sch_rda_sub_record->code_type = (uint8) rxc_sch.code_type;
      }
      else /* turbo code */
      {
        sch_rda_sub_record->norm_ren_energy
                                     = rxc_frame_data.rxc_sch_status.turbo_sch_status.energy;//lint !e732 Loss of sign
    ...
    rxcrda_static_data.rxc_rda_log_buff_offset += total_byte;
  }
  ...
  rxcrda_static_data.rxc_rda_log_sch_rc = (byte) rxc_sch.rc;
}
...
cai_data_rate_type rxc_voc_rate_sel
(
  word mux1
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch( mux1 )
  {
    ...
    case CAI_MUX1_FOR_DCCH_15_ID:
    {
      ...
      ret_rate = ( (rxc_frame_data.rxc_erase) ? CAI_NULL_RATE : CAI_BLANK_RATE );
      break;
    }
...
cai_data_rate_type rxc_mar_rate_sel
(
  word mux1
    /* mux category for frame */
)
...
===========================================================================*/
cai_data_rate_type rxc_fch_rda( deint_rc_type rc )
{
  ...
  eng = rxcrda_static_data.rxcrda_norm_eng;
  eng[FULL] = rxc_fch_status.energy[(uint32) DEC_RATE_FULL - 1];
  /* eng[HALF] = energy metric estimate for the half rate frame */
  /* output buffer * sqrt(2). approximtion sqrt(2) = 181/2^7. */
  eng[HALF] = (((rxc_fch_status.energy[(uint32) DEC_RATE_HALF - 1]) * 181) / 128 ); /* >>7 */
  /* eng[2] = energy metric estimate for the qtr rate frame from decoder */
  /* sqrt(2)^2. */
  eng[QUARTER] = ((rxc_fch_status.energy[(uint32) DEC_RATE_QTR - 1]) * 2);
  ...
  eng[EIGHTH] = (((rxc_fch_status.energy[(uint32) DEC_RATE_8TH - 1]) * 181) / 64 ); /* >>6 */

  /* Set CRC's for full, half, quarter and eighth rate */
  crc[FULL] = rxc_fch_status.status[(uint32) DEC_RATE_FULL - 1] & 0x01;
  crc[HALF] = rxc_fch_status.status[(uint32) DEC_RATE_HALF - 1] & 0x01;
  crc[QUARTER] = rxc_fch_status.status[(uint32) DEC_RATE_QTR - 1] & 0x01;
  crc[EIGHTH] = rxc_fch_status.status[(uint32) DEC_RATE_8TH - 1] & 0x01;
  ...
  yam[QUARTER] = (rxc_fch_status.status[(uint32) DEC_RATE_QTR - 1] & 0x02) >> 1;
  yam[EIGHTH] = (rxc_fch_status.status[(uint32) DEC_RATE_8TH - 1] & 0x02) >> 1;
  ...
  switch (rc)
  {
    case DEINT_RC11:
    {
      /* No need for rate determination here since it is done in FW
         for us already. Just break out of here to avoid hitting the
         default error. */
      break;
    }
    ...
    default:
    {
      ONEX_ERR_FATAL ("Illegal Radio Configuration %d", (int) rc, 0, 0);
    }
  }
...
cai_data_rate_type rxc_9600_dec_rate_sel (void)
{
...
/*--------------------------------------------------------------------*/
  if ( rxc_fch_status.status[(uint32) DEC_RATE_FULL - 1] & DEC_CRC1_M)
  {
    if ( rxc_fch_status.status[(uint32) DEC_RATE_HALF - 1] & DEC_CRC2_M )
    {
      if ( rxc_fch_status.ser[(uint32) DEC_RATE_FULL - 1] <=
             rxc_fch_status.ser[(uint32) DEC_RATE_HALF - 1] + 15 )
      {
        /* full rate */
        dec_rate = CAI_FULL_RATE;
      }
    ...
    {
      if ( rxc_fch_status.ser[(uint32) DEC_RATE_FULL - 1] <= 77 )
      {
        /* full rate */
        dec_rate = CAI_FULL_RATE;
      }
  ...
  else if ( (rxc_fch_status.status[(uint32) DEC_RATE_HALF - 1] & DEC_CRC2_M) &&
            (rxc_fch_status.ser[(uint32) DEC_RATE_HALF - 1] <=SER2_THRESH) )
  {
    if( !(rxc_fch_status.status[(uint32) DEC_RATE_QTR - 1] & DEC_GOOD4_M ) &&
        !(rxc_fch_status.status[(uint32) DEC_RATE_8TH - 1] & DEC_GOOD8_M ) )
    {
      /* half rate */
      dec_rate = CAI_HALF_RATE;
    }
    else if (rxc_fch_status.status[(uint32) DEC_RATE_8TH - 1] & DEC_GOOD8_M )
    {
      if( rxc_fch_status.ser[(uint32) DEC_RATE_HALF - 1] <=
            rxc_fch_status.ser[(uint32) DEC_RATE_8TH - 1] + DSER_THRESH )
      {
        /* half rate */
        dec_rate = CAI_HALF_RATE;
      }
    ...
    else if( rxc_fch_status.ser[(uint32) DEC_RATE_HALF - 1] <=
               rxc_fch_status.ser[(uint32) DEC_RATE_QTR - 1] + DSER_THRESH )
    {
      /* half rate */
      dec_rate = CAI_HALF_RATE;
    }
  ...
  else if ( (rxc_fch_status.status[(uint32) DEC_RATE_QTR - 1] & DEC_GOOD4_M) &&
            (rxc_fch_status.status[(uint32) DEC_RATE_8TH - 1] &DEC_GOOD8_M) )
  {
    if ( (rxc_fch_status.ser[(uint32) DEC_RATE_8TH - 1] <
          rxc_fch_status.ser[(uint32) DEC_RATE_QTR - 1] ) &&
         (rxc_fch_status.ser[(uint32) DEC_RATE_8TH - 1] <=SER8_THRESH ) )
    {
      dec_rate = CAI_EIGHTH_RATE;
    }
    else if ( (rxc_fch_status.ser[(uint32) DEC_RATE_QTR - 1] <
               rxc_fch_status.ser[(uint32) DEC_RATE_8TH - 1] ) &&
              (rxc_fch_status.ser[(uint32) DEC_RATE_QTR - 1] <=SER4_THRESH ) )
    {
      dec_rate = CAI_QUARTER_RATE;
    }
    else
  ...
  else if ( rxc_fch_status.ser[(uint32) DEC_RATE_FULL - 1] <= 71 )
  {
...
/* The values in this table are represented as floor((N x 16) +.5)

   where N =

  0.000000e+00  8.872663e-01    6.496809e-01    3.949123e-01
  1.107785e+00  0.000000e+00    5.780115e-01    5.512895e-01
  1.390970e+00  1.174640e+00    0.000000e+00    8.709004e-01
  1.979284e+00  1.402028e+00    1.064701e+00    0.000000e+00

   Rows ordered 1/8, 1/4, 1/2, 1
   Columns ordered same

*/
static const int16 rxc_rc2_AA_thresholds[4][4] =
{
  {  0, 14, 10,  6 },
  { 18,  0,  9,  9 },
  { 22, 19,  0, 14 },
...
/* Values in this table are rounded down to the nearest whole integer

  3.658770e+02  -4.092413e+01   -7.372786e+00   8.968534e+01
  1.712814e+02  4.971741e+02    2.608789e+02    1.671552e+02
  1.481074e+02  8.021680e+01    4.650165e+02    -9.487794e+01
  3.308518e+01  7.586125e+01    8.237440e+01    6.315427e+02

   Rows ordered 1/8, 1/4, 1/2, 1
   Columns ordered same

*/
static const int16 rxc_rc2_BB_thresholds[4][4] =
{
  { 365,  -40,   -7,   89 },
  { 171,  497,  260,  167 },
  { 148,   80,  465,  -94 },
...
cai_data_rate_type rxc_14400_dec_rate_sel (void)
{
  /* result of the rate decision algorithm (initialized to erasure) */
  cai_data_rate_type dec_rate = CAI_BLANK_RATE;
...
/*--------------------------------------------------------------------*/
  ...
  switch (crc_status & RXC_CRC)
  {
    case 1:
      ...
      */
      if (rxc_fch_status.status[(uint32) DEC_RATE_8TH - 1] & DEC_GOOD8_M)
      {
        if (rxc_fch_status.ser[(uint32) DEC_RATE_8TH - 1] < CRC8_GOOD_THRESH1 )
        {
            dec_rate = CAI_EIGHTH_RATE;
        }
      }
      ...
      {
        if (rxc_fch_status.ser[(uint32) DEC_RATE_8TH - 1] < CRC8_GOOD_THRESH0 )
        {
            dec_rate = CAI_EIGHTH_RATE;
        }
      }
    ...
    case 2:
      /* Only the quarter rate CRC checks */
        if (rxc_fch_status.ser[(uint32) DEC_RATE_QTR - 1] < CRC4_GOOD_THRESH )
        {
            dec_rate = CAI_QUARTER_RATE;
        }
        break;
    ...
    case 4:
      ...
      if (rxc_fch_status.ser[(uint32) DEC_RATE_HALF - 1] < CRC2_GOOD_THRESH )
      {
          dec_rate = CAI_HALF_RATE;
      }
      break;
    ...
    case 8:
      ...
      if (rxc_fch_status.ser[(uint32) DEC_RATE_FULL - 1] < CRC1_GOOD_THRESH )
      {
          dec_rate = CAI_FULL_RATE;
      }
      break;
    ...
    case 3:
      ...
      if (rxc_fch_status.ser[(uint32) DEC_RATE_QTR - 1] <
          rxc_fch_status.ser[(uint32) DEC_RATE_8TH - 1] )
      {
        if ( rxc_fch_status.ser[(uint32) DEC_RATE_QTR - 1] < CRC4_MAX_THRESH )
          dec_rate = CAI_QUARTER_RATE;
      }
      else
      {
        if ( rxc_fch_status.ser[(uint32) DEC_RATE_8TH - 1] < CRC8_MAX_THRESH )
          dec_rate = CAI_EIGHTH_RATE;
      }
      break;
    ...
    case 5:
      ...
      if (rxc_fch_status.ser[(uint32) DEC_RATE_HALF - 1] <
          rxc_fch_status.ser[(uint32) DEC_RATE_8TH - 1] )
      {
        if ( rxc_fch_status.ser[(uint32) DEC_RATE_HALF - 1] < CRC2_MAX_THRESH )
          dec_rate = CAI_HALF_RATE;
      }
      else
      {
    ...
    case 6:
      ...
      if (rxc_fch_status.ser[(uint32) DEC_RATE_QTR - 1] <
          rxc_fch_status.ser[(uint32) DEC_RATE_HALF - 1] )
      {
        if ( rxc_fch_status.ser[(uint32) DEC_RATE_QTR - 1] < CRC4_MAX_THRESH )
          dec_rate = CAI_QUARTER_RATE;
      }
    ...
    case 9:
      ...
      if (rxc_fch_status.ser[(uint32) DEC_RATE_FULL - 1] <
          rxc_fch_status.ser[(uint32) DEC_RATE_8TH - 1] )
      {
        if ( rxc_fch_status.ser[(uint32) DEC_RATE_FULL - 1] < CRC1_MAX_THRESH )
          dec_rate = CAI_FULL_RATE;
      }
      else
      {
    ...
    case 10:
      ...
      if (rxc_fch_status.ser[(uint32) DEC_RATE_QTR - 1] <
          rxc_fch_status.ser[(uint32) DEC_RATE_FULL - 1] )
      {
        if ( rxc_fch_status.ser[(uint32) DEC_RATE_QTR - 1] < CRC4_MAX_THRESH )
          dec_rate = CAI_QUARTER_RATE;
      }
    ...
    case 12:
      ...
      {
        if ( rxc_fch_status.ser[(uint32) DEC_RATE_HALF] < CRC2_MAX_THRESH )
          dec_rate = CAI_HALF_RATE;
      }
      break;
    ...
    default:
      M1X_MSG( MUX, LEGACY_ERROR,
        "Unhanded CRC status %d",
        (crc_status & RXC_CRC));
      break;
  }

  /* In case Yamamoto metrics could not determine the rate of the frame,
     try to use the energy metrics returned from the decoder to classify
     the frame.

     Loop through the energy metrics for each rate and compare against
     the calculated thresholds. If any pass the thresholds, save that
     rate for return (4 = full, 3 = half, 2 = qtr, 1 = eighth, 0 = blank.
  */
  if(dec_rate == CAI_BLANK_RATE)
  {
    /* Loop through each rate, starting with FULL (0). */
    for(i=0; i<=3; i++)
    {
      /* Check if rate *i* passed CRC */
      if (rxc_fch_status.status[i] & 0x01)
      {
        /* Used to temporarily store the dynamic energy thresholds
           for RC2 rate determination. */
        int32 rxc_rc2_energy_threshold = 0;

        /* Calculate the energy threshold to use to determine if the frame
           is truly an erasure. */
        for(j=0; j<=3; j++)
        {
          /* Only retain the maximum threshold value. We only need to
             verify that our energy metric for this rate exceeds the
             maximum value. */
          rxc_rc2_energy_threshold =
            MAX(
                  rxc_rc2_energy_threshold,
                  ( ( ( ( rxc_fch_status.energy[j] *
                          rxc_rc2_AA_thresholds[3-i][3-j]
                        ) + 8
                      ) / 16
                    ) + rxc_rc2_BB_thresholds[3-i][3-j]
                  )
              );
        }

        /* Check if the energy metric for this rate exceeds the threshold.
           If it does, save the rate for return. */
        if ( rxc_fch_status.energy[i] > rxc_rc2_energy_threshold )
        {
          dec_rate = (cai_data_rate_type) (CAI_FULL_RATE - i);
          break;
        }
      }
    }
...
===========================================================================*/
cai_data_rate_type rxc_evrc_dec_rate_sel( void )
{
  const word SER2_THRESH = 60;
  /* Threshold for determining half rate */
  const word SER4_THRESH = 60;
  ...
  rxc_fch_status.status[(uint32) DEC_RATE_QTR - 1] =
    ( rxc_fch_status.status[(uint32) DEC_RATE_QTR - 1] & (~DEC_GOOD4_M) );
  rxc_fch_status.ser[(uint32) DEC_RATE_QTR - 1] = 200;
  ...
  if( rxc_fch_status.status[(uint32) DEC_RATE_FULL - 1] & DEC_CRC1_M )
  {
    if( rxc_fch_status.status[(uint32) DEC_RATE_HALF - 1] & DEC_CRC2_M )
    {
      ...
      {
        if( ( rxc_fch_status.status[(uint32) DEC_RATE_8TH-1] & DEC_GOOD8_M ) &&
            ( rxc_fch_status.ser[(uint32) DEC_RATE_8TH-1] + 12 <
              rxc_fch_status.ser[(uint32) DEC_RATE_HALF - 1] ) )
        {
          /* Eigth rate */
          dec_rate = CAI_EIGHTH_RATE;
    ...
    else
    {
      ...
      {
        if( ( rxc_fch_status.status[(uint32) DEC_RATE_8TH-1] & DEC_GOOD8_M ) &&
            ( rxc_fch_status.ser[(uint32) DEC_RATE_8TH-1] + 23 <
              rxc_fch_status.ser[(uint32) DEC_RATE_FULL - 1] ) )
        {
          /* Eigth rate */
          dec_rate = CAI_EIGHTH_RATE;
           ...
           ( rxc_fch_status.ser[(uint32) DEC_RATE_HALF - 1] <=SER2_THRESH ) )
  {
    ...
    else if( rxc_fch_status.ser[(uint16) DEC_RATE_HALF - 1] <=
             rxc_fch_status.ser[(uint16) DEC_RATE_QTR - 1] + DSER_THRESH )
    {
      /* half rate */
      dec_rate = CAI_HALF_RATE;
    }
...
===========================================================================*/
cai_data_rate_type rxc_dec_rate_sel( deint_rc_type curr_rc )
{
  cai_data_rate_type dec_rate = CAI_BLANK_RATE;

  if( rxc_hho_data.rxc_gps_state == RXC_GPS_STATE_NONE )
  {
    switch( curr_rc )
    {
      case DEINT_RC1:
      ...
      case DEINT_RC5:
      {
        ...
        if( rxc_so.for_dcch.included )
        {
          dec_rate = rxc_dtx_det_dcch( (dec_rc_type) curr_rc );
        }
        else
        #endif /* !FEATURE_IS2000_REL_A_SVD */
        {
          dec_rate = rxc_fch_rda( curr_rc );
        }
        break;
      }
      ...
      case DEINT_RC11:
      {
        /* FCH rate determination for RC 11 is performed in MDSP. */
        dec_rate = muxmdsp_rd_rc11_fch_rda_info();
        break;
      }
      ...
      default:
      {
        ONEX_ERR_FATAL( "Illegal Radio Configuration: %d",
                   curr_rc, 0, 0 );
        return dec_rate;
      }
    }
  ...
  if( rxc_so.for_fch.included )
  {
    /* Update FER and SER counts */
    rxc_update_fer_ser_counts( rxc_fch_status, curr_rc, dec_rate );
  }
...
=========================================================================*/
cai_data_rate_type rxc_dtx_det_dcch( dec_rc_type rc )
{
  cai_data_rate_type rate = CAI_BLANK_RATE;
  int32 norm_pc_acc;
  int2 pc_acc = (int2) muxmdsp_get_pc_acc();
  #ifdef FEATURE_IS2000_REL_A_SVD
  uint16 ser = rxc_dcch_status.ser[DEC_RATE_FULL-1];
  int32  energy = rxc_dcch_status.energy[DEC_RATE_FULL-1];
  #else
  ...
  if( rxc_hho_data.rxc_gps_state == RXC_GPS_STATE_NONE )
  {
    ...
    if( rxcrda_static_data.pending_pc_acc_scaler !=
        rxcrda_static_data.pc_acc_scaler )
    {
      qword cur_frame_time = {0};

      m1x_time_get_20ms_frame_offset_time( cur_frame_time );
      if(
          qw_cmp( rxcrda_static_data.pending_pc_acc_scaler_time, cur_frame_time )
            <= 0
        )
      {
        /* If we are at or pass the time to update the pc_acc_scaler, do it now */
        rxcrda_static_data.pc_acc_scaler =
          rxcrda_static_data.pending_pc_acc_scaler;
      }
    }
    ...
    pc_acc = (pc_acc * rxcrda_static_data.pc_acc_scaler);
      //lint !e734 Loss of precision (assignment) (23 bits to 15 bits)

    norm_pc_acc = (128 * pc_acc * 45) / subchan_gain;

    /* If CRC passes */
    #ifdef FEATURE_IS2000_REL_A_SVD
    if( rxc_dcch_status.status[DEC_RATE_FULL-1] & 0x01 )
    #else
    ...
    if( rxcrda_data.rxcrda_dtx_msg_enable )
    {
      switch( (cai_data_rate_type) rate )
      {
        case CAI_FULL_RATE:
          M1X_MSG( MUX, LEGACY_HIGH,
            "DCCH FULL pcacc %d ser %d e %d",
            pc_acc,
            ser,
            energy);
          M1X_MSG( MUX, LEGACY_HIGH,
            "DCCH-SC gain %d, CRC %d",
            debug_values,
            crc_value );
          break;
        case CAI_BLANK_RATE:
          M1X_MSG( MUX, LEGACY_HIGH,
            "DCCH ERAS pcacc %d ser %d e %d",
            pc_acc,
            ser,
            energy);
          M1X_MSG( MUX, LEGACY_HIGH,
...
)
{
  ...
  ret_status =
    rxcrda_dtx_filter( frame_status, &rxcrda_data.rxcrda_sch_dtx_fix_counter);
  return ret_status;
} /* rxc_sch_dtx_filter */
...
)
{
  ...
  ret_status =
    rxcrda_dtx_filter( frame_status, &rxcrda_data.rxcrda_dcch_dtx_fix_counter);
  return ret_status;
} /* rxc_dcch_dtx_filter */
...
rxc_frame_type rxcrda_dtx_filter
(
 rxc_frame_type frame_status,
 int8 *counter
)
...
rxc_frame_type rxc_dtx_det_sch( dec_rc_type rc, boolean crc_pass )
{
  ...
  int16 IQAcc     = (int16) muxmdsp_get_sch_iq_acc();
  int16 RtNtIo    = (int16) muxmdsp_get_sch_rt_nt_io();
  int16 RtEcpIo   = (int16) muxmdsp_get_sch_wtd_pilot();
  ...
  else
  {
    ...
    switch( rc )
    {
      ...
      default:
        M1X_MSG( MUX, LEGACY_ERROR,
          "Invalid RC for SCH DTX, RC enum=%d",
          rc);
        pi_EbNt    = 0;
        pi_codeR   = 0;
        shift_NtIo = 0;
    ...
    switch ( rxc_sch.sch_rate )
    {
      ...
      case DEC_SCH_RATE_4X:
        shift_NtIo -= 2;
        ltu_crc = (rxc_sch.code_type == DEC_TURBO) ?
                    0 : (0x03 & rxc_frame_data.rxc_sch_status.conv_sch_status.ltu_crc);
        break;

      case DEC_SCH_RATE_8X:
        shift_NtIo -= 3;
        ltu_crc = (rxc_sch.code_type == DEC_TURBO) ?
                    0 : (0x0f & rxc_frame_data.rxc_sch_status.conv_sch_status.ltu_crc);
        break;

      case DEC_SCH_RATE_16X:
        shift_NtIo -= 4;
        ltu_crc = (rxc_sch.code_type == DEC_TURBO) ?
                    0 : (0xff & rxc_frame_data.rxc_sch_status.conv_sch_status.ltu_crc);
        break;
      ...
      default:
        ltu_crc = 0;
        M1X_MSG( MUX, LEGACY_ERROR,
          "Invalid rate for SCH DTX, rate enum=%d.",
          rxc_sch.sch_rate);
        break;
    }
    ...
    while ((schEcpNtInstLim > MAX_SQRT_INT32) ||
           (schEcpNtFiltLim > MAX_SQRT_INT32))
    {
      schEcpNtFiltLim >>= 4;
      schEcpNtInstLim >>= 4;
    }
    ...
    {
      M1X_MSG( MUX, LEGACY_HIGH,
        "--DTX: IQAcc %-10d, RtNtIo %-10d, RtEcpIo %-10d",
        IQAcc,
        RtNtIo,
        RtEcpIo);
      M1X_MSG( MUX, LEGACY_HIGH,
        "  DTX: setpt %-10d, EcpNtF %-10d, EcpNtI  %-10d",
        fpc_setpt,
        schEcpNtFilt,
        schEcpNtInst);
      M1X_MSG( MUX, LEGACY_HIGH,
        "  DTX: IQNt  %-10d, IQNtQ  %-10d, metL    %-10d",
        IQAccRtNtIo,
        IQAccRtNtIo_Q,
        metricL);
      M1X_MSG( MUX, LEGACY_HIGH,
        "  DTX: metR  %-10d, metRQ  %-10d, crc/dec %-02x",
        metricR,
        metricR_Q,
        (crc_pass<<4) + (uint32) frame);
    }
...
void rxcrda_dcch_init( void )
{
  ...
  //lint !e734 Loss of precision (assignment) (9 bits to 8 bits)
  fpc_dcch_min_pcb_setpt =
    ffpc_get_dcch_min_setpt_db() + ( 2 * ffpc_get_subchan_gain_db() );

  if( fpc_dcch_min_pcb_setpt > 132 )
  {
    temp = ( fpc_setpt_db_lin_133_255[fpc_dcch_min_pcb_setpt - 133] + 40 );
    temp = temp * temp;
    rxcrda_static_data.rxcrda_dcch_quality_thresh = (256 * temp) / 49;
  }
  else
  {
    temp = ( fpc_setpt_db_lin_0_132[fpc_dcch_min_pcb_setpt] + 46 );
    temp = temp * temp;
    rxcrda_static_data.rxcrda_dcch_quality_thresh = (256 * temp) / 2025;
  }
} /* rxcrda_dcch_init */
...
)
{
  ...
  int2 pc_acc = (int2) muxmdsp_get_pc_acc();
  int2 fch_nt_io = (int2) muxmdsp_get_fch_nt_io();
  ...
  */
  ...
  {
    M1X_MSG( MUX, LEGACY_HIGH,
      "fch_nt_io value is equal to ZERO!");
    rxcrda_static_data.rxc_dcch_frame_quality = RXC_POOR_CHANNEL;
    return rxcrda_static_data.rxc_dcch_frame_quality;
  }
  ...
  {
    rxcrda_static_data.rxc_dcch_frame_quality = RXC_GOOD_CHANNEL;
    rxcrda_static_data.rxcrda_dcch_quality_thresh = ffpc_get_dcch_hw_setpt();
  }
  ...
  else if( rda_decision == RXC_DTX_FRAME )
  {
    ...
    else if( curr_rc == CAI_RC_5 )
    {
      temp32bit = ((uint32)(((pc_acc * pc_acc)/14400) * 48114))/((uint2)fch_nt_io);
    }
    else  /* Invalid RC */
    {
      ONEX_ERR_FATAL( "Illegal DCCH Radio Configuration %d", curr_rc, 0, 0 );
    }

    if( temp32bit > (uint32)rxcrda_static_data.rxcrda_dcch_quality_thresh )
    {
      rxcrda_static_data.rxc_dcch_frame_quality = RXC_GOOD_CHANNEL;
    }
    else
  ...
  if( rxcrda_data.rxcrda_dtx_msg_enable )
  {
    switch( rxcrda_static_data.rxc_dcch_frame_quality )
    {
      case RXC_POOR_CHANNEL:
        M1X_MSG( MUX, LEGACY_HIGH,
          "DCCH POOR %d %d %d",
          rxcrda_static_data.rxcrda_dcch_quality_thresh,
          pc_acc,
          fch_nt_io );
        break;
...
)
{
  ...
  m1x_time_get_20ms_frame_offset_time(
    rxcrda_static_data.pending_pc_acc_scaler_time );
  qw_inc( rxcrda_static_data.pending_pc_acc_scaler_time, 2L );
  ...
  rxcrda_static_data.pending_pc_acc_scaler = 1 << ( (byte) gating_rate );

} /* rxcrda_update_gating_rate */
