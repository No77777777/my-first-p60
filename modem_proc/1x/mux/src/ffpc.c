...
$Header: //components/rel/1x.mpss/8.0/mux/src/ffpc.c#1 $ $DateTime: 2019/06/11 23:51:40 $ $Author: pwbldsvc $
...
04/21/16   srk     1x2GTA cleanup fixes.
...
11/24/15   srk     Freeze setpt. udpates during WTR Hopping.
...
11/09/13   bb      Changes to send FFPC config command to FW
...
12/06/12   vks     Ensure there is no negative index referece for array
                   when calculating ffpc_log_offset
08/02/12   srk     Created an API to return current FPC mode.
06/15/12   eye     Added else condition to update RC value, when FPC primary
...
09/02/11   jtm     Add RC11 entry for ffpc_iqacc_sch_scale_tab.
08/15/11   jtm     Klocwork Fixes.
08/04/11   jtm     Set the rliFPCMask based on the updated FPC mode.
...
12/09/10   jtm     Removed functions no longer used after removing obsolete
                   CP->Mux commands.
...
09/07/10   jtm     Print an error message if FFPC mode 1 is received for RC11.
09/03/10   jtm     Corrected parameter type of ffpc_convert_l3_ffpc_mode().
...
08/03/10   jtm     Update ffpc_config_fch_ratchet_setpt_freeze() to combine
                   RC11 and legacy switch statements.
07/23/10   jtm     Added support for 1xA FFPC modes.
07/06/10   jtm     Fix compiler warnings.
06/15/10   jtm     Added FFPC event reporting.
                   rxc_tx_in_traffic_on now exported through rxctraffic.h
06/10/10   jtm     Eliminate dependency on log_dmss.h
05/20/10   jtm     Replaced ffpc log types from log_dmss.h with equivalent
                   types from mux_logtypes.h.
...
09/30/09   jtm     Added new configurations for the FPC_SELECT field of
                   ARM register DEM1X_FWD_PWR_CTL_FRAME for 1xA FPC Modes.
...
11/20/08   vlc     In function ffpc_disable(), add check to avoid disabling FFPC
                   functionality in HW when demod clocks are already turned off.
...
08/15/08   jtm     Fixed Klockwork Errors.
08/14/08   jtm     Fixed ffpc_trans_cmd() to proper handle RC 1 and 2 cases
                   and notify searcher that FFPC is off.
07/01/08   vlc     Merged code changes from old tip archive.
04/17/08   vlc     Added check for NULL pointer in function ffpc_log_frame_init()
...
*/
 /* Accumulated buffer */
mux_log_ffwd_pctrl_type           ffpc_log_buf;
/* Ptr to sub-record  */
mux_log_ffwd_pctrl_subrecord_type *ffpc_log_sr_ptr = NULL;
...
FUNCTION FFPC_CONVERT_L3_FFPC_MODE

DESCRIPTION
  Converts the FFPC mode provided by L3 to the correct mode based on RC.
  This is necessary since 1x Advanced RCs share the same mode values in
  L3 but they mean something different.

  For 1x Advanced
  --------------
  FPC_MODE_PRI_800Hz            =  FPC_MODE_1XA_PRI_400Hz
  FPC_MODE_PRI_400Hz_SEC_400Hz  =  FPC_MODE_1XA_PRI_200Hz (to be removed)
  FPC_MODE_PRI_200Hz_SEC_600Hz  =  FPC_MODE_1XA_PRI_200Hz_SEC_200Hz
  FPC_MODE_PRI_EIB              =  FPC_MODE_1XA_PRI_200Hz
...
=========================================================================*/
fpc_mode_type ffpc_convert_l3_ffpc_mode
(
  cai_radio_config_type rc,
  fpc_mode_type l3_mode
)
{
  fpc_mode_type converted_mode = l3_mode;
  ...
  if( rc == CAI_RC_11 )
  {
    switch(l3_mode)
    {
      case FPC_MODE_PRI_800Hz:
        converted_mode = FPC_MODE_1XA_PRI_400Hz;
        break;

      case FPC_MODE_PRI_400Hz_SEC_400Hz: /*Left in for backward compatibility*/
        M1X_MSG( MUX, LEGACY_MED,
          "Received FFPC Mode 1 for RC11. Defaulting to 200Hz PC");
        converted_mode = FPC_MODE_1XA_PRI_200Hz;
        break;
      ...
      case FPC_MODE_PRI_200Hz_SEC_600Hz:
        converted_mode = FPC_MODE_1XA_PRI_200Hz_SEC_200Hz;
        break;

      default:
        M1X_MSG( MUX, LEGACY_ERROR,
          "Invalid FFPC mode (%d) for RC11",
          l3_mode);
    }
  }
...
int32 ffpc_get_current_fch_setpoint_dbq3( void )
{
  ...
  return( ffpc_fch_params.curr_setpt_db >> 17 );

} /* ffpc_get_current_fch_setpoint_dbq3 */
...
int32 ffpc_get_max_fch_setpoint_dbq3( void )
{
  ...
  return( ffpc_fch_params.max_setpt_db );

} /* ffpc_get_max_fch_setpoint_dbq3 */
...
)
{
  ...
  if (cmd_ptr->tci.fpc.included)
  {
    ...
    /* initialize FPC_MODE */
    ffpc_info.fpc_mode = ffpc_convert_l3_ffpc_mode(
      ((ffpc_info.fpc_pri_chan == FFPC_FCH) ? cmd_ptr->tci.for_fch.rc : cmd_ptr->tci.for_dcch.rc),
      (fpc_mode_type) cmd_ptr->tci.fpc.fpc_mode);
  }

  #ifdef FEATURE_IS2000_1X_ADV
  if((cmd_ptr->tci.rcp_init.fpc_mode.incl) && (cmd_ptr->tci.for_fch.rc == CAI_RC_11))
  {
   M1X_MSG( MUX, LEGACY_MED,
     "Using FPC mode from the RCPM (%d)",
     cmd_ptr->tci.rcp_init.fpc_mode.value);

   /* initialize FPC_MODE */
   ffpc_info.fpc_mode =
      ffpc_convert_l3_ffpc_mode( cmd_ptr->tci.for_fch.rc,
             (fpc_mode_type)cmd_ptr->tci.rcp_init.fpc_mode.value);
  }
  #endif /* FEATURE_IS2000_1X_ADV */
  ...
  if ((cmd_ptr->tci.fpc.included) && (cmd_ptr->tci.fpc.fch_olpc.included))
  {
    M1X_MSG( MUX, LEGACY_HIGH,
      "FCH: INIT %d Subchan Gain %d MIN %d  MAX %d  Target FER %d",
      ffpc_fch_params.init_setpt_db,
      ffpc_info.fpc_subchan_gain_db,
      ffpc_fch_params.min_setpt_db,
      ffpc_fch_params.max_setpt_db,
  ...
  {
    M1X_MSG( MUX, LEGACY_HIGH,
      "DCCH: INIT %d Subchan Gain %d MIN %d  MAX %d  Target FER %d",
      ffpc_dcch_params.init_setpt_db,
      ffpc_info.fpc_subchan_gain_db,
      ffpc_dcch_params.min_setpt_db,
      ffpc_dcch_params.max_setpt_db,
...
DESCRIPTION
  This function returns the current fpc mode.

DEPENDENCIES
  None.

RETURN VALUE
  The current FPC mode.
...
{
  return ffpc_info.fpc_mode;

} /* ffpc_get_current_fpc_mode */
...
SIDE EFFECTS
  None.
=========================================================================*/
void ffpc_update_fpc_mode
(
 boolean rc_invalid_for_ffpc,
 fpc_mode_type fpc_mode
)
{
  uint32 temp_event_payload;
  ...
  /* Update FPC_MODE in SRCH module. */
    ...
    else
    {
       ...
       txc_set_fpc_mode (fpc_mode);
    }
  }
  else
  {
    /* program FPC_MODE in HW */
    ffpc_program_fpc_mode (fpc_mode);
  ...
  #ifdef FEATURE_IS2000_1X_ADV
  muxmdsp_set_rliFPCMask(fpc_mode);
  #endif /* FEATURE_IS2000_1X_ADV */

  temp_event_payload = (uint32)fpc_mode;
  event_report_payload (EVENT_1X_FPC_MODE,
                       sizeof(temp_event_payload),
                       &temp_event_payload);
...
)
{
  ...
  {
    M1X_MSG( MUX, LEGACY_ERROR,
      "FPC_SUBCHAN_GAIN value out of range" );
  }
  else
  {
...
void ffpc_program_hw( void )
{
  ...
  muxmdsp_set_pc_filt_gain( 0x4000 );

  /* forward link Nt/Io filter gain */
  muxmdsp_set_nt_io_filt_gain( 0x0800 );
...
=========================================================================*/
void ffpc_set_setpt_offset_dbq4
(
  int16 offset_dbq4
)
{
  word hw_setpt;
  ffpc_chan_info_type  *ffpc_params;
  boolean setpt_offset_changed = FALSE;
     ...
     )
  {
    ...
    /* store new offset */
    if( ffpc_params->setpt_offset_dbq4 != offset_dbq4 )
    {
      setpt_offset_changed = TRUE;
    }
    ...
    if( setpt_offset_changed == TRUE )
    {
      M1X_MSG( MUX, LEGACY_MED,
        "SP dB: 0x%08x. SPA dB: 0x%08x. SPD lin: 0x%08x.",
        ffpc_params->curr_setpt_db,
        ffpc_params->setpt_adjusted_dbq20,
        hw_setpt);
    }
  ...
  {
    M1X_MSG( MUX, LEGACY_MED,
      "SP offset not allowed, FPC Pri Chan (%d)",
      ffpc_info.fpc_pri_chan);
  }
...
)
{
  if (channel == FFPC_FCH)
  {
    muxmdsp_set_eb_no_target_fch (setpt);
  ...
  {
    muxmdsp_set_eb_no_target_sch (setpt);
    /* cache hw setpt for logging */
...
DESCRIPTION
  ...
  NEW FOR 1X ADVANCED -->
  For R-RC8/F-RC11 FPC mode 2, SW must configure the FPC_SELECT field of
  ARM register DEM1X_FWD_PWR_CTL_FRAME to 0x2222.

  For R-RC8/F-RC11 FPC modes 0 and 1, SW must configure the FPC_SELECT field of
  ARM register DEM1X_FWD_PWR_CTL_FRAME to 0x0.
...
SIDE EFFECTS
  None.
=========================================================================*/
void ffpc_program_fpc_mode
(
 fpc_mode_type fpc_mode
)
{
  uint16 fpc_sel_value;

  switch(fpc_mode)
  {
    case FPC_MODE_PRI_400Hz_SEC_400Hz:
      fpc_sel_value = 0xAAAA;
      break;

    case FPC_MODE_PRI_200Hz_SEC_600Hz:
      fpc_sel_value = 0xDDDD;
      break;

    #ifdef FEATURE_IS2000_1X_ADV
    case FPC_MODE_1XA_PRI_200Hz:
      fpc_sel_value = 0x0000;
      break;

    case FPC_MODE_1XA_PRI_200Hz_SEC_200Hz:
      fpc_sel_value = 0x2222;
      break;
    #endif /* FEATURE_IS2000_1X_ADV */

    /* All other fpc modes default to FPC_SEL = 0x0000 */
    default:
      fpc_sel_value = 0x0000;
  }

  /* program channel source selection */
  muxmdsp_update_pwr_ctrl_fpc(TRUE, fpc_sel_value );
...
{
  /* Check and only disable FFPC once */
  if ( ffpc_fch_params.olpc_enabled ||
       ffpc_sch_params.olpc_enabled ||
       ffpc_dcch_params.olpc_enabled
     )
  { /* Disable FFPC in HW only in case of RC1 or RC2.  If disabled for other
   ...
   {
     muxmdsp_update_pwr_ctrl_fpc(FALSE,0);
   }
  ...
  #ifdef FEATURE_IS2000_1X_ADV
  ffpc_config_fch_ratchet_setpt_freeze(FALSE);
  #endif /* FEATURE_IS2000_1X_ADV */
...
)
{
  ...
  /* Skip FFPC set point update until TX is UP */
  if( ta_is_enabled )
  {
    M1X_MSG( MUX, LEGACY_HIGH,
       "1x2GTA: TA is ON, skipping FFPC set point update" );
    return;
  }
  ...
  #ifdef FEATURE_IS2000_1X_ADV
  /* Do not update the FCH FFPC setpoint when MDSP indicates that the
    setpoint is frozen.*/
  if( ( channel == FFPC_FCH ) && ( muxmdsp_fch_fpc_setpoint_is_frozen() ) )
  {
    return;
  }
  #endif /* FEATURE_IS2000_1X_ADV */
  ...
  */
  ...
  if( ffpc_params->olpc_enabled )
  {
    ...
    {
      if( !srch_mux_wtr_hop_in_progress() )
      {
      ...
      {
        M1X_MSG( MUX, LEGACY_HIGH,
           "Skip FFPC Set point updates during WTR HOPPING" );
      }
    }
...
)
{
   ...
   {
      M1X_MSG( MUX, LEGACY_ERROR,
        "Incorrect value for TARGET_FER %d specified.",
        fer_bin);
   }
...
===========================================================================*/
void ffpc_send_log( void )
{
  if (ffpc_log_buf.hdr.len != 0)
  {
    /* Will send log if submitted. */
    (void) log_submit(&ffpc_log_buf);
...
{
  mux_log_ffwd_pctrl_frame_type *frm_ptr = NULL;  /* Ptr to frame entry */
  mux_log_ded_fwd_chan_set_type chan_set_mask;
  int32 ffpc_log_offset = 0;

  /* Is logging enabled? */
  if (log_status(LOG_FFWD_PCTRL_C))
  {
    /* If we reached the logging threshold for FFPC, send it. */
    if (ffpc_log_buf.hdr.len >= MUX_LOG_FFPC_BUF_THRESHOLD) //lint !e413 FPOS
    {
      ffpc_send_log();
    }
    ...
    {
      ffpc_log_buf.hdr.len = (word) FPOS(mux_log_ffwd_pctrl_type, var_len_buf); //lint !e413 FPOS

      ffpc_log_buf.hdr.code = LOG_FFWD_PCTRL_C;

      /* First frame, log time stamp. */
      (void) m1x_time_get(ffpc_log_buf.hdr.ts);
      ...
      ffpc_log_buf.fpc_pri_chan = (byte) ffpc_info.fpc_pri_chan;
    ...
    else if ((ffpc_log_buf.fpc_mode != (byte)(FPC_MODE_PRI_EIB))
#ifdef FEATURE_IS2000_REL_A
     && (ffpc_info.fpc_mode != FPC_MODE_PRI_QIB)
     && (ffpc_info.fpc_mode != FPC_MODE_PRI_QIB_SEC_EIB)
#endif /* FEATURE_IS2000_REL_A */
    ...
    */
    if ((ffpc_log_buf.fpc_mode == (byte)(FPC_MODE_PRI_EIB))
#ifdef FEATURE_IS2000_REL_A
     || (ffpc_info.fpc_mode == FPC_MODE_PRI_QIB)
     || (ffpc_info.fpc_mode == FPC_MODE_PRI_QIB_SEC_EIB)
#endif /* FEATURE_IS2000_REL_A */
    ...
    if (ffpc_log_sr_ptr == NULL)
    {
      ...
      */
      ffpc_log_offset = (ffpc_log_buf.hdr.len -
                         (word) FPOS(mux_log_ffwd_pctrl_type, var_len_buf));
      if( ffpc_log_offset < 0)
      {
        /* ffpc log offset not expected to be negative, return NULL so that
           ffpc_log_frame will stop execution after calling this function */

          return (NULL);

      }

      ffpc_log_sr_ptr = (mux_log_ffwd_pctrl_subrecord_type *)
      ...
      ffpc_log_sr_ptr->len_per_frame = (byte) FPOS(mux_log_ffwd_pctrl_frame_type, setpts);
      ...
      if (log_fch)
      {
        ...
        ffpc_log_sr_ptr->len_per_frame +=
          FSIZ(mux_log_ffwd_pctrl_frame_type, setpts[0]);
      }
      ...
      if (log_sch)
      {
        ...
        */
        ffpc_log_sr_ptr->len_per_frame +=
          2 * FSIZ(mux_log_ffwd_pctrl_frame_type, setpts[0]);
      }
      ...
      ffpc_log_buf.hdr.len += (word) FPOS(mux_log_ffwd_pctrl_subrecord_type, var_len_buf); //lint !e413 FPOS
    ...
    */
    ...
    frm_ptr = (mux_log_ffwd_pctrl_frame_type *)
                 &ffpc_log_buf.var_len_buf.buf_ref[ffpc_log_offset]; //lint !e413 FPOS
  }
  else
  {
...
)
{
  ...
  mux_log_ffwd_pctrl_frame_type *frm_ptr;
    ...
    switch (ffpc_sch_params.fwd_rc)
    {
      ...
      case DEINT_RC11:
      {
         switch (sch_rate)
         {
            case DEC_SCH_RATE_1X:
            {
               walsh_gain = FFPC_RC11_1X_SCH_WALSH_GAIN;
               break;
            }
            case DEC_SCH_RATE_2X:
            {
               walsh_gain = FFPC_RC11_2X_SCH_WALSH_GAIN;
               break;
            }
            case DEC_SCH_RATE_4X:
            {
               walsh_gain = FFPC_RC11_4X_SCH_WALSH_GAIN;
               break;
            }
            case DEC_SCH_RATE_8X:
            {
               walsh_gain = FFPC_RC11_8X_SCH_WALSH_GAIN;
               break;
            }
            case DEC_SCH_RATE_16X:
            {
               walsh_gain = FFPC_RC11_16X_SCH_WALSH_GAIN;
               break;
            }
            case DEC_SCH_RATE_32X:
            {
               walsh_gain = FFPC_RC11_32X_SCH_WALSH_GAIN;
               break;
            }
            default:
            {
      ...
      {
         M1X_MSG( MUX, LEGACY_ERROR,
           "Incorrect RC for SCH");
      }
    }

    MODEM_1X_UNUSED( walsh_gain );
  }
...
SIDE EFFECTS
  None.
=========================================================================*/
uint16 ffpc_get_iqacc_scale( void )
{
  dec_sch_rate_type sch_rate;
  boolean           double_ptr;
  dec_rc_type       sch_rc;
  uint16  iq_acc_scale = 0;
  ...
  if (rxc_get_sch_info (&sch_rc, &sch_rate, &double_ptr))
  {
     ...
     #ifdef FEATURE_IS2000_1X_ADV
     if ( (sch_rc > DEC_RC5) && (sch_rc != DEC_RC11) )
     #else
     if ( sch_rc > DEC_RC5 )
     #endif /* FEATURE_IS2000_1X_ADV */
     {
...
)
{
  ...
  boolean rc_invalid_for_ffpc = FALSE;
  cai_radio_config_type pri_rc = CAI_INIT_RC;
  ffpc_chan_info_type *ffpc_chan_info_ptr = NULL;

  if ( cmd_ptr == NULL )
  {
    M1X_MSG( MUX, LEGACY_ERROR,
      "NULL FFPC_TRANS_CMD received");
    return;
  }
  ...
  M1X_MSG( MUX, LEGACY_MED,
   "Got FFPC_TRANS_CMD: FCH %d, DCCH %d, SCH %d",
   ffpc_ptr->fch_ffpc_op.mode,
   ffpc_ptr->dcch_ffpc_op.mode,
   ffpc_ptr->sch_ffpc_op.mode );

  /* Make sure that Secondary channel is set to F-SCH0 */
  if (ffpc_ptr->sch.fpc_sec_chan.included &&
     ffpc_ptr->sch.fpc_sec_chan.value != 0)
  {
    M1X_MSG( MUX, LEGACY_ERROR,
      "FPC_SEC_CHAN not set to F-SCH0");
  ...
  if (ffpc_ptr->fpc_pri_chan.included)
  {
    ...
    {
       ffpc_info.fpc_pri_chan = FFPC_FCH;
       pri_rc = ffpc_ptr->fch_ffpc_op.rc;
       M1X_MSG( MUX, LEGACY_HIGH,
         "FPC primary channel : FCH with RC = %d",
         pri_rc);
    }
    else
    {
       ffpc_info.fpc_pri_chan = FFPC_DCCH;
       pri_rc = ffpc_ptr->dcch_ffpc_op.rc;
       M1X_MSG( MUX, LEGACY_HIGH,
         "FPC primary channel : DCCH with RC = %d",
         pri_rc);
    }
  }
  else
  {
    ...
    {
        ffpc_chan_info_ptr = ffpc_get_param_ptr(ffpc_info.fpc_pri_chan);
        pri_rc = ffpc_chan_info_ptr->fwd_rc;
        M1X_MSG( MUX, LEGACY_HIGH,
          "FPC primary channel : %d with RC = %d",
          ffpc_info.fpc_pri_chan,
          ffpc_chan_info_ptr->fwd_rc);
    }
    else
    {
        M1X_MSG( MUX, LEGACY_ERROR,
          "Invalid FPC primary channel");
    }
  }
  ...
  {
    ffpc_info.fpc_mode = ffpc_convert_l3_ffpc_mode( pri_rc,
      (fpc_mode_type) ffpc_ptr->fpc_mode.value);
  }
...
#ifdef FEATURE_IS2000_1X_ADV
/*=========================================================================
FUNCTION FFPC_RCPM_TRANS_CMD

DESCRIPTION
  This function handles the FFPC portion of the MCTRANS_RCP_ACTION
  command from MC.
...
SIDE EFFECTS
  None.
=========================================================================*/
void ffpc_rcpm_trans_cmd
(
  mctrans_action_packet_type *cmd_ptr
)
{
  mctrans_rxc_rcp_action_type  *rcp_ptr;
  ...
  {
    M1X_MSG( MUX, LEGACY_ERROR,
      "NULL RCP_TRANS_CMD received");
    return;
  }

  rcp_ptr = &cmd_ptr->action.rxc_rcp;

  if( rcp_ptr->fpc_mode.incl )
  {
    M1X_MSG( MUX, LEGACY_HIGH,
      "New FFPC Mode received = %d",
      rcp_ptr->fpc_mode.value);

    if( rxc_so.for_fch.rc > CAI_RC_2 )
    {
      ffpc_info.fpc_mode =
              ffpc_convert_l3_ffpc_mode( rxc_so.for_fch.rc,
                                         (fpc_mode_type) rcp_ptr->fpc_mode.value );

      ffpc_update_fpc_mode( FALSE, ffpc_info.fpc_mode );

      if( ( ffpc_info.fpc_mode == FPC_MODE_PRI_EIB )
          #ifdef FEATURE_IS2000_REL_A
          || ( ffpc_info.fpc_mode == FPC_MODE_PRI_QIB )
          || ( ffpc_info.fpc_mode == FPC_MODE_PRI_QIB_SEC_EIB )
          #endif /* FEATURE_IS2000_REL_A */
    ...
    {
      /* RC is invalid for FFPC operation. Disable FFPC.*/
      ffpc_update_fpc_mode( TRUE, ffpc_info.fpc_mode );

      ffpc_disable();
      M1X_MSG( MUX, LEGACY_ERROR,
        "RC%d invalid for FFPC. Disabling FFPC.",
        rxc_so.for_fch.rc );
    }
            ...
            else
            {
               ...
               */
               ...
               ffpc_info.pri_sch_init_delta =
                    (uint16) ((pri_chan_params->curr_setpt_db >> 17) -
...
SIDE EFFECTS
  None.
=========================================================================*/
void ffpc_init_outer_loop_params
(
...
)
{
       ...
       (ffpc_params->incl_status & FFPC_OLPC_M))
   {
      ...
      */
      ...
      {
        case FFPC_FCH:
          rxc_ta_info.fch_incl_status = ffpc_params->incl_status;
          M1X_MSG( MUX, LEGACY_HIGH,
          "FCH include status %d",rxc_ta_info.fch_incl_status );
        break;

        case FFPC_DCCH:
          rxc_ta_info.dcch_incl_status = ffpc_params->incl_status;
          M1X_MSG( MUX, LEGACY_HIGH,
            "DCCH include status %d",rxc_ta_info.dcch_incl_status );
        break;

        case FFPC_SCH:
          rxc_ta_info.sch_incl_status = ffpc_params->incl_status;
          M1X_MSG( MUX, LEGACY_HIGH,
          "SCH include status %d",rxc_ta_info.sch_incl_status );
        break;

        default:
          M1X_MSG( MUX, LEGACY_HIGH,
          "Bad status %d",ffpc_params->incl_status );
      }
...
#ifdef FEATURE_IS2000_1X_ADV
/*=========================================================================
FUNCTION FFPC_CONFIG_FCH_RATCHET_SETPT_FREEZE

DESCRIPTION
  This function will initialize the FCH FFPC setpoint Freeze parameters.
  Software will program the mDSP with the number of consecutive UP commands
  (3 frames x PCGs carrying PC bits per frame based on PC mode). The mDSP
  will then use this limit to later notify software if it is to freeze
  FFPC setpoint updates.
...
SIDE EFFECTS
  The mDSP will be configured with the number of consecutive UP commands
  the MS can receive before the FFPC setpoint should be frozen.
=========================================================================*/
void ffpc_config_fch_ratchet_setpt_freeze( boolean enable )
{
  uint16 fch_fpc_freeze_limit = 0;
  uint8 pc_bits = 0;
  ...
  if( enable == TRUE )
  {
    ...
    fch_fpc_freeze_limit = FFPC_RATCHET_FREEZE_FRAMES * pc_bits;
  }

  muxmdsp_fch_fpc_setpoint_freeze_config(enable, fch_fpc_freeze_limit);
  event_report_payload (EVENT_1X_FPC_RATCHET,
                        sizeof(uint16), &fch_fpc_freeze_limit);

  M1X_MSG( MUX, LEGACY_MED,
    "FFPC Ratchet Mode: FPC Mode = %d, RC = %d, Limit = %d",
    ffpc_info.fpc_mode,
    ffpc_fch_params.fwd_rc,
    fch_fpc_freeze_limit);
} /* ffpc_config_fch_ratchet_setpt_freeze */
#endif /* FEATURE_IS2000_1X_ADV */
...
void ffpc_check_setpt_thresh( void )
{
   ...
   if (ffpc_sch_params.olpc_enabled && pri_chan_params->olpc_enabled &&
       ((ffpc_info.fpc_mode == FPC_MODE_PRI_800Hz)      ||
        #ifdef FEATURE_IS2000_1X_ADV
        (ffpc_info.fpc_mode == FPC_MODE_1XA_PRI_400Hz)  ||
        #endif /* FEATURE_IS2000_1X_ADV */
        (ffpc_info.fpc_mode == FPC_MODE_PRI_EIB)) &&
...
void ffpc_send_olrm( void )
{
  ...
  else
  {
    ...
    M1X_MSG( MUX, LEGACY_MED,
      "Send Outer Loop Report Message");
...
{
   #ifdef FEATURE_IS2000_1X_ADV
   if (((old_rc == DEINT_RC3) || (old_rc == DEINT_RC4) || (old_rc == DEINT_RC11)) &&
       ((new_rc == DEINT_RC3) || (new_rc == DEINT_RC4) || (new_rc == DEINT_RC11)))
   #else
