...
$Header: //components/rel/1x.mpss/8.0/mux/inc/rxccommon_i.h#1 $ $DateTime: 2019/06/11 23:51:40 $ $Author: pwbldsvc $
...
extern rex_crit_sect_type rxc_voc_crit_sect;
...
/* The following defines create sections where other code is not allowed to
  ...
  |-----------------------------------------------------------------------|
  | No   Yes  -   | Use RXC_ENTER_ATOMIC_SECTION()                        |
  |-----------------------------------------------------------------------|
  | Yes  No   No  | Use RXC_ENTER_ISR_SECTION()                           |
  |-----------------------------------------------------------------------|
  | Yes  No   Yes | Use RXC_ENTER_CRIT_SECTION()                          |
  |-----------------------------------------------------------------------|
  | Yes  Yes  No  | Use RXC_ENTER_ATOMIC_CRIT_SECTION()                   |
  |-----------------------------------------------------------------------|
  | Yes  Yes  Yes | Use RXC_ENTER_ATOMIC_ISR_SECTION()                    |
  \-----------------------------------------------------------------------/
...
#define RXC_ENTER_ATOMIC_CRIT_SECTION()     \
    rex_enter_crit_sect( &rxc_crit_sect );  \
    REX_DISABLE_PREMPTION()

#define RXC_LEAVE_ATOMIC_CRIT_SECTION()     \
    REX_ENABLE_PREMPTION();                 \
...
DESCRIPTION
  This function gets the value of rxc_mvs_so_change variable. To 
...
RETURN VALUE
  True if SO change, False if no SO change.

SIDE EFFECTS
  None

=========================================================================*/
extern boolean rxc_mvs_get_is_so_change( void );
...
=========================================================================*/
extern void rxc_mvs_set_is_so_change( boolean so_change );
