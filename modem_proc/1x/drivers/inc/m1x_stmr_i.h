...
--------   ---     ----------------------------------------------------------
11/29/16   srk     Added new events for paging and sync SW DEMBACK decode.
...
typedef enum
{
  ...
  M1X_STMR_PCG_15,
  M1X_STMR_DECODE_PCH,
  /* Enum ordering is important. Add any PCG related events
     above and add any PN Roll and Non-PCG related events below.
     Also please maintain the order of PCG events and only add
     new events below PCG 15. */
  M1X_STMR_PNROLL,
  M1X_STMR_DECODE_SYNCH,
  M1X_STMR_EVENT_MAX
} m1x_stmr_event_type;
...
(
  ...
  m1x_stmr_event_type  new_event,                           /* Event to trigger */
...
DESCRIPTION
  This function returns the combiner time for the given XO time

DEPENDENCIES
  None.

RETURN VALUE
  COMB count at the given XO
