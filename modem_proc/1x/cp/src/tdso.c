...
INITIALIZATION AND SEQUENCING REQUIREMENTS
  ...
  Copyright (c) 1998 - 2020 Qualcomm Technologies, Inc.
...
$Header: //components/rel/1x.mpss/8.0/cp/src/tdso.c#2 $ $DateTime: 2020/01/27 00:26:21 $ $Author: pwbldsvc $
...
10/30/17   ppr     FER error issue after HHO with WTR Hop for TDSO call fix
...
{
  /* For HHO with WTR Hop req case, don't initialize TDSO 
   * to retain the TDSO statistics otherwise, 
   * we are facing 100% FER error immediately after 
   * HHO with WTR hop req case */
  if ( mcctcho_is_hho_in_progress() &&
       mcc_is_hho_with_wtr_hopping_in_prog()
     )
  {
    M1X_MSG( DCP, LEGACY_MED,
             "HHO with WTR Hop in Prog, not initializing TDSO SO:%d",
             cfg->con[connect_info].so);
    return;
  }
...
void tdso_init_so_cfg(void)
{
  ...
  {
    ONEX_ERR_FATAL( "Registering SO %x Failed",  CAI_SO_TDSO, 0, 0);
  }

  if( snm_add_so( CAI_SO_FULL_TDSO, (snm_neg_par_type*)&ftdso_neg_par,
      &tdso_default_cfg ) == SNM_FAILURE )
  {
    ONEX_ERR_FATAL( "Registering SO %x Failed",  CAI_SO_FULL_TDSO, 0, 0);
  }
    ...
    ** -------------------------------------------------------------------*/
    for (i=0; i<frame_blk_ptr->num_frames; i++)
    {
      ...
      if (rx_frame_ptr->fr_rate >= TDSO_MAX_RATE)
      {
        ...
        ONEX_ERR_FATAL("Rate out of range %d", (int) rx_frame_ptr->fr_rate, 0, 0);
      }
