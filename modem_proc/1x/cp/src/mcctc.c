...
$Header: //components/rel/1x.mpss/8.0/cp/src/mcctc.c#2 $ $DateTime: 2020/01/27 00:26:21 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ------------------------------------------------------------
01/22/20   vaa     Implement QSH MDUMP framework
11/16/18   ppr     Don't update the mcc_exit rsn to SRCH_Restart while in 
                   TC_Rel state because of Prot_Deact cmd
01/19/18   ppr     F3 Reduction changes
11/17/17   ppr     Mainline GPP2_SUPPORT_ON_SEC_SUB feature
11/10/17   ppr     Chgs to allow Access handoff back to MT page rcvd PN 
                   irrespective of ESPM Access handoff fields
11/10/17   ppr     Fix to process Sys_Restart SRCH rpt only in TCI_W4_FRAMES 
                   Traffic state
11/10/17   sjo     Chgs to handle SYS_RESTART in state TCI_W4_FRAMES.
08/31/17   sjo     Chgs for CDMA_support on SUB2
08/02/17   sjo     FR 37336: Migration to MSGR VARIANT from Inst ID
08/29/16   sjo     Chgs to handle LTE_ABORT_1XCSFB_HO_REQ rxed in traffic state
...
03/24/16   ppr     WTR_HOP:Ignore UHDM if HHO is already in progress
...
10/20/15   ppr     More chgs to honor "MC_END_F with CAI_REL_PWDN end rsn"
09/30/15   ppr     Chgs to honor "MC_END_F with CAI_REL_PWDN end rsn" while 
                   call in traffic state
09/29/15   agh     Remove all references to MVS and cleanup WON feature
08/05/15   agh     Perform WTR hopping for ECAM with change in band class
...
05/14/15   agh     Optimization to add previous serving channel to NL rxed on
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch (msg_ptr->msg.gen.msg_type)
  {
      ...
      ** ------------------------------------- */
      ...
      switch (msg_ptr->msg.tc_ord.gen.order)
      {
        ...
        case CAI_RELEASE_ORD:
          ...
          ** ------------- */
          M1X_MSG( DCP, LEGACY_MED,
            "Rxed Release Order, tc_exit:%d", tc_exit );
    ...
    case CAI_MEID_UNIV_HO_DIR_MSG: /* MEID Universal Handoff Direction Message */
    {
      ...
      {
        /* Ignore UHDM msg even if HHO without WTR Hop is not in progress
         ...
         * there is one more UHDM msg with WTR Hopping required then it will 
         * cause crash in lower layers because HHO txns will be executed with 
         * latest updated WTR Hop req fields but WTR Hop not triggered because 
         * of earlier UHDM msg without WTR Hop required */
        ...
        {      
           ...
           *    (For WTR Hop req case, Pre_proc_done not updated but 
           *     WTR Hop req flag updated in validate_handoff_msg() ) */
          ...
          else
          {
            ...
            ** -------------------------------------------------------------  */
            if (mcc_continue_on_curr_sys(MEAS_BACK_IGNORED,
                   sd_ss_ind_cdma_opr_acpt_rej( mcc_asid, NULL))
                )
            {
              cdma.curr_state = CDMA_EXIT;
    ...
    case CAI_TC_SYS_PARM_MSG:
      ...
      ** ---------------------------------------------------------- */
      if (!mcc_continue_on_curr_sys(MEAS_BACK_IGNORED,
            sd_ss_ind_cdma_opr_itspm( mcc_asid, msg_ptr->msg.tc_sys.sid,
    ...
    case CAI_MOB_REG_MSG:
      ...
      ** ------------------------------------------------------- */
      if (!mcc_continue_on_curr_sys(MEAS_BACK_IGNORED,
              sd_ss_ind_cdma_opr_itspm( mcc_asid,
                                 msg_ptr->msg.reg.sid, 
                                        msg_ptr->msg.reg.nid, NULL))
         )
      {
        /* Go ahead and delete all CC instances now */
        mccccim_evt.del_all_cc.event_code = MCCCCIM_DELETE_ALL_CC;
  ...
  break;
    ...
    case CAI_TC_ALT_TECH_INFO_MSG:
      ...
      else if(sd_misc_is_lte_full_srv_allowed( mcc_asid ))
      {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch( msg_ptr->hdr.cmd )
  {
    ...
    case MC_MO_DBM_F:
      ...
      {
        ONEX_ERR_FATAL( "NULL DBM pointer", 0, 0, 0);
        return;
      }
    ...
    case MC_MVS_ENABLE_STATUS_F:
    {
      ...
      /* Defer sending TX cmds if TX is in inactive state */
      if (txc_is_curr_state_exit())
      {      
        tc_wtr_hop_defer_tx_cmd(&mcc_tx_buf);
      }
      else
      {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  ** --------------------------------------- */
  ...
  M1X_MSG( DCP, LEGACY_MED,
           "cdma.end_reason:%d, cdma.rel_orig:%d, tc_exit:%d",
           cdma.end_reason, cdma.rel_orig, tc_exit);
    ...
    ** -------------------------------------------------------------------- */
    ...
    else if ((sigs & MCC_RXTX_Q_SIG) != 0)
    {
      if ((msg_ptr = (mccrxtx_cmd_type*) q_get( &mcc_rxtx_q )) != NULL)
      {
        ...
        else if ( msg_ptr->hdr.command == MCCRXTX_HDM_ACK_CNF_F )
        {
          /* We got the confirmation from RxTx that L2 Ack sent to n/w
           * for Generic Handoff Direction Msg(HDM, EHDM, GHDM, ...) */        
          M1X_MSG( DCP, LEGACY_HIGH, 
              "WTR_Hop:UHDM TX ack sent cnf in REL, "
              "ack seq %d, TX cmd done %d, TA_hho_prog %d", 
              msg_ptr->hdm_ack_cnf.ack_seq, 
              mcc_wtr_hopping.tx_cmd_proc_done,
              cdma.tc_data_ta.hho_prog );        
      
          /* Check that this L2 ack is for the most recently processed UHDM */
          if ( mcc_wtr_uhdm_msg_ptr != NULL && 
               mcc_wtr_uhdm_msg_ptr->msg.gen_tc.hdr.msg_seq == msg_ptr->hdm_ack_cnf.ack_seq )
          {          
            (void) mcc_clr_callback_timer ( &mcc_hdm_tx_ack_timer, MCC_HDM_TX_ACK_TIMER_ID );
      
            if( mcc_is_hho_with_wtr_hopping_in_prog() == TRUE && 
                !mcc_wtr_hopping.tx_cmd_proc_done &&
                /* Honor Ack confirmation only for legacy WTR flow and 
                 * for TA flow hho_prog updated to HHO_MSG_BUFFERED */
                cdma.tc_data_ta.hho_prog == HHO_MSG_RCVD
              )
            {
              /* WTR hopping may need to be deferred till HHO action time */
              tc_start_or_defer_wtr_hopping();     
              mcc_wtr_hopping.tx_cmd_proc_done = TRUE;
            }
          } /* if (msg_ptr->hdm_ack_cnf.ack_seq  */
          else if ( mcc_wtr_uhdm_msg_ptr != NULL )
          {
            M1X_MSG( DCP, LEGACY_HIGH, 
                "WTR_Hop: Ack seq mismatch! UHDM msg seq %d, L2 ack seq %d", 
                mcc_wtr_uhdm_msg_ptr->msg.gen_tc.hdr.msg_seq,
                msg_ptr->hdm_ack_cnf.ack_seq );             
          }
        } /* else if ( msg_ptr->hdr.command == MCCRXTX_HDM_ACK_CNF_F ) */
    ...
    else if ((sigs & MCC_SRCH_Q_SIG) != 0)
    {
      ...
      {
        mcc_exit_name_type temp_tc_exit;
        /*--------------------------------------------------------*/
        ...
        temp_tc_exit = tc_srch_rpt( srch_ptr, tc_exit );

        /* Check if we are in TC_Rel substate due to Prot_deact from MMODE.
         * If yes then don't update the tx_exit rsn to avoid MMOC Sanity
         * time out from MMODE, waiting for Prot_Deact cnf from 1x */
        if( mcc_exit.mmoc_trans_id == PROT_TRANS_NONE )
        {
          tc_exit = temp_tc_exit;
        }
        else
        {
          M1X_MSG( DCP, LEGACY_HIGH,
                   "Prot_deact state, mcc_exit trans_id:%d, exit:%d, "
                   "end_rsn:%d, tc_exit:%d, temp_tc_exit:%d, srch_rpt:%d",
                   mcc_exit.mmoc_trans_id,
                   mcc_exit.exit,
                   cdma.end_reason,
                   tc_exit,
                   temp_tc_exit,
            ...
            )                                         
    {
      mcctcsup_proc_ta_cfs_defer_timer_expiry();
              ...
              mcc_timer_on_callback_q(MCC_TA_COMP_BUF_TIMER_ID)
            )                                         
    {
      mcctcsup_proc_ta_comp_buf_timer_expiry();
              ...
              mcc_timer_on_callback_q(MCC_HDM_TX_ACK_TIMER_ID)
            )                                         
    {
      /* Clear entry in callback timer queue */
      (void) mcc_clr_callback_timer ( &mcc_hdm_tx_ack_timer, MCC_HDM_TX_ACK_TIMER_ID );

        /* Timeout waiting for confirmation from RxTx that L2 Ack sent to n/w
         * for Generic Handoff Direction Msg(HDM, EHDM, GHDM, ...) */      
      M1X_MSG( DCP, LEGACY_HIGH, 
               "WTR_hop: Timeout for UHDM Ack Tx cnf in REL, "
               "TX cmd done %d, TA_hho_prog %d",
                mcc_wtr_hopping.tx_cmd_proc_done,
                cdma.tc_data_ta.hho_prog );

      if( mcc_is_hho_with_wtr_hopping_in_prog() == TRUE && 
          !mcc_wtr_hopping.tx_cmd_proc_done &&
          /* Honor Ack timer expiry only for legacy WTR flow and 
           * for TA flow hho_prog updated to HHO_MSG_BUFFERED */
          cdma.tc_data_ta.hho_prog == HHO_MSG_RCVD
        )
      {
            ...
            )                                        
    {
      ...
      /* Deferral time for WTR hopping over. Start Hopping procedures */
      M1X_MSG( DCP, LEGACY_MED, 
               "WTR_hop: Deferral timer expiry in tc release, TA_hho_prog:%d",
               cdma.tc_data_ta.hho_prog );
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /* Don't log QSH cmd as it flooding every 200ms */
  if(msg_ptr->hdr.cmd != MC_QSH_CB_CMD_F)
  {
    M1X_MSG( DCP, LEGACY_MED,
  ...
  switch (msg_ptr->hdr.cmd)
  {
    ...
    case MC_ORIGINATION_F:
    {
      ...
      ** ---------------------------------------- */
               ...
               (mcctctag_is_call_id_outstanding(msg_ptr->origination.call_id)))
      {
        ...
        ONEX_ERR_FATAL("Duplicated call id", 0, 0, 0);
      }
...
#endif /* FEATURE_MODEM_1X_SRVCC */
    {
      ...
      /* SRVCC mode */
      if( (srvcc_mode == TRUE) &&
          (msg_ptr->hdr.cmd == MC_SRVCC_CALL_END_F)
        )
    {
      ...
      /* CSFB mode */
               ...
               (msg_ptr->hdr.cmd == MC_CSFB_CALL_END_F)
             )
#endif /* FEATURE_MODEM_1X_SRVCC */
      {
      ...
      else
      {
        ...
        M1X_MSG( DCP, LEGACY_HIGH,
          "DEBUG: Rxed CSFB/SRVCC CALL_END in TC state");
        msg_ptr->hdr.status = MC_DONE_S;

#ifdef FEATURE_MODEM_1X_SRVCC
        /* Send report to CM confirming call end only for CSFB call
         * No need to send call_end_cnf for SRVCC call */
        if(srvcc_mode == FALSE)
#endif /* FEATURE_MODEM_1X_SRVCC */
        {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch (msg_ptr->msg.gen.msg_type)
  {
    ...
    case CAI_MEID_UNIV_HO_DIR_MSG: /* MEID Universal Handoff Direction Message */
      ...
      /* Ignore UHDM msg even if HHO without WTR Hop is not in progress because
       * we might have despatched the HHO txns but not executed yet
       * as action time not arrived yet when of use_time=1 and in b/w if there 
       * is one more UHDM msg with WTR Hopping required then it will cause crash
       * in lower layers because HHO txns will be executed with latest updated 
       * WTR Hop req fields but WTR Hop not triggered because of earlier UHDM 
       * msg without WTR Hop required */
  ...
  do
  {
    ...
    ** -------------------------------------------------------------------- */
    ...
    else if ((sigs & MCC_RXTX_Q_SIG) != 0)
    {
      if ((msg_ptr = (mccrxtx_cmd_type*) q_get( &mcc_rxtx_q )) != NULL)
      {
        ...
        else if ( msg_ptr->hdr.command == MCCRXTX_HDM_ACK_CNF_F )
        {
           ...
           * for Generic Handoff Direction Msg(HDM, EHDM, GHDM, ...) */        
          M1X_MSG( DCP, LEGACY_HIGH, 
              "WTR_Hop: TX ack sent cnf for UHDM with "
              "ack seq %d, TX cmd done %d, TA_hho_prog %d", 
              msg_ptr->hdm_ack_cnf.ack_seq, 
              mcc_wtr_hopping.tx_cmd_proc_done,
              cdma.tc_data_ta.hho_prog );        
            ...
            )                                         
    {
       ...
       * for Generic Handoff Direction Msg(HDM, EHDM, GHDM, ...) */      
      M1X_MSG( DCP, LEGACY_HIGH, 
               "WTR_hop: Timeout for Tx cnf for UHDM Ack, "
               "TX cmd done %d, TA_hho_prog %d",
                mcc_wtr_hopping.tx_cmd_proc_done,
                cdma.tc_data_ta.hho_prog );
            ...
            )                                        
    {
      ...
      /* Deferral time for WTR hopping over. Start Hopping procedures */
      M1X_MSG( DCP, LEGACY_MED, 
               "WTR_hop: Deferral timer expiry in tc_tc" );
...
void tc_handle_ccim_event( mccccim_tc_event_type * evt_ptr)
{
  ...
  switch (evt_ptr->gen.evt_code)
  {
    ...
    {
      M1X_MSG( DCP, LEGACY_MED,
               "Call end received from MCCCCIM for %d, rel_rsn:%d",
      ...
      else
      {
        ...
        else if(evt_ptr->call_end.rel_reason == MCCCCIM_LOW_BATTERY)
        {
          cdma.end_reason = CAI_REL_PWDN;
          M1X_MSG( DCP, LEGACY_MED,
            "Ending call due to low battery, tc_exit:%d", tc_exit);
        }
        else
        {
          cdma.end_reason = CAI_REL_NORMAL;
...
void tc_init (void)
{
  ...
  /* Indicates if WTR_HOPPPING is required while entering traffic state */
  ...
  /* Clear MT page in prog flag */
  cdma.mt_page_in_progress = FALSE;
    ...
    switch( sub_state )
    {
      ...
      case TCI_START:
      {
        ...
        if (cdma.chnasn.mode.am0.freq_incl)
        {
          ...
          /* WTR hopping may be required if band_class changes in ECAM */
          if ( (cdma.cdmach != cdma.chnasn.mode.am0.cdma_freq) ||
               (cdma.band_class != cdma.chnasn.mode.am0.band_class) )
          {
            if (srch_rx_check_wtr_hopping(
                         cdma.chnasn.mode.am0.band_class, 
                         cdma.chnasn.mode.am0.cdma_freq, 
                         SRCH_RX_ONEX_MC ) == SRCH_RX_BAND_HOP )
            {
              wtr_hop_required = TRUE;
              M1X_MSG( DCP, LEGACY_HIGH,
               "WTR_HOP: WTR Hopping req on entering traffic, band %d, chn %d",
               cdma.chnasn.mode.am0.band_class, 
               cdma.chnasn.mode.am0.cdma_freq );              
            }
          }          
        ...
        /* If WTR hopping is required then we need to shutdown TXC before 
         * tuning SRCH to new band/freq and so JUMP to hyperspace should 
        ...
        {
          M1X_MSG( DCP, LEGACY_MED,
               "WTR_Hop: Start WTR Hopping procedures: Sending TXC_EXIT_F" );                  
         ...
         * CDMA_F and TT_F cmds. Note that TC_F doesn't need to sent separately
         * in WTR hopping flow for ECAM case as it will be sent as part of 
         * legacy flow after RXC has been moved to TCI state and when TX needs 
         * to be turned on.
         * Although it might seem a more appropriate place for this block of 
         * code, don't try to move it just before TXC is turned on as we will
         * face issues in VSTMR and RXC will get stuck for VSTMR response while 
         * trying to move to TCI state.  
         */ 
        if(wtr_hop_required)
        {
          M1X_MSG( DCP, LEGACY_MED,
            "WTR_HOP: Re-enabling TX before sending TC_F in traffic init");        
      ...
      case TCI_W4_FRAMES:
      {
        int srch_q_cnt = 0, srch_bkup_q_cnt = 0;

        /*----------------------------------------------------------------*/

        while ((cdma.curr_state == CDMA_TCI) && (sub_state == TCI_W4_FRAMES))
        {
          ...
          ** ------------------------------------ */
          ...
          { 
            /* Process SRCH Sys_Restart rpt only in TCI_W4_FRAMES state and
             * if any other rpt rcvd then buffer them to procees later.
             * It can happen that FW may ask SRCH for a restart and 
             * SRCH inturn ask MC for the same. In that case, FW needs the 
             * restart to be triggered immediately, otherwise FW will crash 
             */
            if ( (srch_rpt_ptr = 
                 (mccsrch_rpt_type*) q_get(&mcc_srch_q )) != NULL )
            {                
              prev_state = cdma.curr_state; 
              ...
              {                
               (srch_rpt_ptr->sys_restart.trm_lock_not_avail == TRUE) ?
                ...
                tc_exit = MCC_SRCH_REQUEST_SYS_RESTART;
                mcc_exit.exit = tc_exit;
                cdma.curr_state = CDMA_EXIT;

                /* Keep track of the SRCH report for debugging purpose */
                mcc_enqueue_dbg_srch_cmd( srch_rpt_ptr->hdr.rpt, prev_state, 
                                    cdma.curr_state, MCC_NON_SLEEP, TC_INIT );

                /* Following fun call will put the node into the free queue */
                cmd_done( &srch_rpt_ptr->hdr.rpt_hdr );
                #ifdef FEATURE_1X_CP_MEM_OPT
                mc_free_queue_buf( srch_rpt_ptr, MCC_SRCH_Q );
                #endif /* FEATURE_1X_CP_MEM_OPT */
              } /* if( rpt == SRCH_REQUEST_SYS_RESTART_R ) */
              else
              {
                /* Enqueue the rcvd SRCH rpt to process once we move out 
                 * of TCI_W4_FRAMES state */
                q_put ( &mcc_srch_bkup_q, q_link ( srch_rpt_ptr, 
                                      &srch_rpt_ptr->hdr.rpt_hdr.link ) );
              }              
            } /* (srch_rpt_ptr != NULL) */
          } /* end else if ((sigs & MCC_SRCH_Q_SIG) != 0) */
          ...
          {
            ONEX_ERR_FATAL( "Invalid signal mask 0x%lx", sigs, 0,0 );
          }
        } /* while */

        /* Check if we have any buffered SRCH rpts to process once 
         * we move out of TCI_W4_FRAMES state. */
        srch_q_cnt      = q_cnt( &mcc_srch_q ); 
        srch_bkup_q_cnt = q_cnt( &mcc_srch_bkup_q );

        if ( srch_bkup_q_cnt > 0 )
        {
          M1X_MSG( DCP, LEGACY_HIGH,
            "TCI_W4_FRAMES state, srch_bkup_q_cnt:%d, srch_q_cnt:%d",
                   srch_bkup_q_cnt, srch_q_cnt );

          /* Now re-arrage buffered SRCH Rpts such that 
           * they are in order of rcvd time to process in next state */
          while ( (srch_rpt_ptr = 
                  (mccsrch_rpt_type*) q_get(&mcc_srch_q)) != NULL )
          {
            /* First move all the rpts from mcc_srch_q to mcc_srch_bkup_q */
            q_put ( &mcc_srch_bkup_q, q_link ( srch_rpt_ptr, 
                                  &srch_rpt_ptr->hdr.rpt_hdr.link ) );
          } /* while(srch_rpt_ptr != NULL) */

          /* Now all the SRCH rpts are in order of rcvd time 
           * in mcc_srch_bkup_q, so move all of them back 
           * to mcc_srch_q to process in next state */

          while ( (srch_rpt_ptr = 
                  (mccsrch_rpt_type*) q_get(&mcc_srch_bkup_q)) != NULL )
          {
            /* First move all the rpts from mcc_srch_q to mcc_srch_bkup_q */
            q_put ( &mcc_srch_q, q_link ( srch_rpt_ptr, 
                                  &srch_rpt_ptr->hdr.rpt_hdr.link ) );
          } /* while(srch_rpt_ptr != NULL) */
        } /* if(srch_bkup_q_cnt > 0) */
               ...
               (!bs_acked || !two_good_frames_received) )
        {
          ...
          ** ---------------------------------- */
          ...
          else if ((sigs & MCC_CALLBACK_TIMER_SIG) != 0)
          {
            ...
            {
              ONEX_ERR_FATAL("Unexpected timer expired", 0, 0, 0);
            }
          ...
          else if ((sigs & MCC_SRCH_Q_SIG) != 0)
          {
            if ((srch_rpt_ptr = (mccsrch_rpt_type*) q_get(&mcc_srch_q )) != NULL)
            {
              ...
              {
                M1X_MSG( DCP, LEGACY_HIGH,
                  "SRCH request sys restart in TCI_PREAMBLE state" );
                mcc_exit.exit = tc_exit;
                cdma.curr_state = CDMA_EXIT;
              }
              /* Keep track of the SRCH report for debugging purpose */
      ...
      default:
      {
        ONEX_ERR_FATAL("Invalid TCI substate %d", sub_state, 0, 0);
      }
    } /* end switch */
  } /* end while */
...
)
{
  ...
  if (new_state == CDMA_IDLE)
  {
    ...
    if (!mcc_continue_on_curr_sys(MEAS_BACK_ALLOWED,
          sd_ss_ind_cdma_opr_acc_term( mcc_asid, 
                                    SD_SS_ACC_REAS_TRAFFIC,
  ...
  if ( cur_bs_ptr->pilot_pn != PSEUDO_PILOT_PN )
  {
    ...
    ** ---------------------------------------------------------------------- */
    ...
    if ( (( cdma.mob_cai_rev >= P_REV_IS2000_REL_B ) 
         ...
         (exit_reason == MCC_RELEASE) && (cdma.end_reason == CAI_REL_NORMAL) &&
         tc_to_idle_possible
       )
    {
      /* -----------------------------------------------------------
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /* Clear the deferred TX cmds queue to ensure no stale cmds are sent to TX */
  ...
  (void) sd_ss_ind_cdma_opr_start_activity( mcc_asid,
                                            NULL, 
  ...
  while ( MAIN_STATE(cdma.curr_state) == CDMA_TC )
  {
    ...
    switch (cdma.curr_state)
    {
      ...
      default:
      {
        ONEX_ERR_FATAL( "Bad Traffic State %d", cdma.curr_state , 0, 0 );
      }
    } /* end of switch */
  } /* end of while */
   ...
   * 3. Return Type: Should always return SD_SS_ACT_CONTINUE 
   *    so we can ignore the return type */
  (void) sd_ss_ind_cdma_opr_stop_activity( mcc_asid, 
                                           NULL, 
                                           SD_SS_CDMA_ACT_MASK_IN_TRAFFIC);
  M1X_MSG( DCP, LEGACY_MED,
  ...
  if(cdma.entry_ptr->entry == MCC_CSFB)
  {
      ...
      )
    {
      ...
      mcc_enqueue_dbg_lte_rrc_cnf( MCC_CDMA_TC, 
  ...
  if (cdma.curr_state == CDMA_EXIT)
  {
    switch (mcc_exit.exit)
    {
      case MCC_LOCK:
        ...
        ** ------------------------------------------------------- */
        if (mcc_continue_on_curr_sys(MEAS_BACK_IGNORED,
            sd_ss_ind_cdma_opr_lock( mcc_asid, 
                                     NULL))
           )
        {
          (void) sd_ss_ind_cdma_opr_release( mcc_asid, NULL);
        }
        break;
      ...
      case MCC_NDSS_OFF:
        ...
        ** ------------------------------------------------------- */
        if (mcc_continue_on_curr_sys(MEAS_BACK_IGNORED,
            sd_ss_ind_cdma_opr_ndss_off( mcc_asid, NULL)))
        {
          (void) sd_ss_ind_cdma_opr_release( mcc_asid, NULL);
        }
        break;
      ...
      case MCC_RELEASE:
      {
        /* --------------------------------------
        ** Inform system determination of Release
        ** only if end_rsn is not the powerdown release
        ** This is req from MMODE side if the they send
        ** Call_end with CAI_REL_PWDN rsn scenarios
        ** -------------------------------------- */
      ...
      case MCC_SYSTEM_LOST:
        ...
        ** ------------------------------------------ */
        ...
        (void) sd_ss_ind_cdma_opr_sys_lost( mcc_asid, cdma.system_lost_reason, NULL );
