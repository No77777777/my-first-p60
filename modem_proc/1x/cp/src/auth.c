...
INITIALIZATION AND SEQUENCING REQUIREMENTS
  ...
  Copyright (c) 1995 - 2020 Qualcomm Technologies, Inc.
...
$Header: //components/rel/1x.mpss/8.0/cp/src/auth.c#2 $ $DateTime: 2020/01/27 00:26:21 $ $Author: pwbldsvc $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
                          ...
                          &nv_cmd_buf))
  {
    M1X_MSG( DCP, LEGACY_ERROR,
             "Bad Auth NV Read Status:%d, NV:%d, cdma_subs_avail:%d",
             (word) nv_cmd_buf.status,
             (int) item_code,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    M1X_MSG( DCP, LEGACY_ERROR,
      "Failed to write Auth NV item, Status:%d, NV:%d, cdma_subs_avail:%d",
             (word) nv_cmd_buf.status,
             (int) item_code,
             cdma.is_cdma_subs_avail );
  }
...
#endif /* FEATURE_UIM_RUIM */
    default:
      ONEX_ERR_FATAL( "Unknown command to AUTH %d",
...
===========================================================================*/
QSH_MDUMP_FN_ATTR
void mc_qsh_mdump_collect_auth_data()
{
  qsh_mdump_collect_high(&auth_mmgsdi_rpt_ptr,sizeof(auth_mmgsdi_rpt_ptr));
  qsh_mdump_collect_high(&auth_cnf,sizeof(auth_cnf));
  qsh_mdump_collect_high(&auth_mmgsdi_rpt_status,sizeof(auth_mmgsdi_rpt_status));
#if !defined(FEATURE_CSIM) && defined (FEATURE_UIM_RUIM)
  qsh_mdump_collect_high(&auth_uim_rsp_buf_ptr, sizeof(auth_uim_rsp_buf_ptr));
#endif /* !defined(FEATURE_CSIM) && defined (FEATURE_UIM_RUIM) */
}
