...
$Header: //components/rel/1x.mpss/8.0/cp/src/mccini.c#2 $ $DateTime: 2020/01/27 00:26:21 $ $Author: pwbldsvc $
...
07/22/16   ppr     Pilot ch acq timer expiry changes
...
06/10/15   ppr     CSFB:Chgs to send IRAT_Fail ind, Auto_deact ind first and
...
05/08/15   ppr     Send CSFB Orig fail rsp with CM_CALL_ORIG_ERR_REDIR rsn
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch( cmd_ptr->hdr.cmd )
  {
    ...
    case MC_CSFB_CALL_END_F:
    {
      ...
      M1X_MSG( DCP, LEGACY_MED, "CSFB DEBUG: Rxed CSFB_CALL_END in Init state");
  ...
  /* deactivate lower layers first */
       ...
       mc_msgr_send_msg( &ho_fail_rsp.hdr,
                      ...
                      sizeof(ho_fail_rsp) );
       mcc_enqueue_dbg_lte_rrc_cnf( MCC_UI_INI,
      ...
      if(cdma.entry_ptr->entry == MCC_LTE_1X_REDIR)
      {
       ...
       cmd_ptr->hdr.status = MC_DONE_S;
        ...
        M1X_MSG( DCP, LEGACY_HIGH,
          "CSFB DEBUG: Clearing the Call object for call id = %d",
          cmd_ptr->csfb_call_clear.call_id);
    ...
    {
      /* CALL_CLEAR_F is only meant to be sent in native 1X for race condition
       * scenarios where 1X-CP was redirected to native 1X before getting
       * CALL_CLEAR from CM after 1X-CP reported some call failure via
       * L-RRC->NAS->CM route */
      if ( cdma.entry_ptr->entry == MCC_LTE_1X_REDIR &&
           csfb_cdma.waiting_for_call_clear )
      {
        cm_call_id_type   call_id;  /* CSFB call_id */

        call_id = cmd_ptr->csfb_call_clear.call_id;
        M1X_MSG( DCP, LEGACY_HIGH,
                 "DEBUG:Rxed MC_CSFB_CALL_CLEAR_F in native 1X");
        ...
        if(call_id == cdma.call_id)
        {
          ...
          /* Mark that we don't need to wait for call_clear_f in CSFB mode */
          csfb_cdma.call_cleared_in_native_1x =  TRUE;
      ...
      } /* if (csfb_cdma.waiting_for_call_clear) */
      else
      {
        M1X_MSG( DCP, LEGACY_ERROR,
                 "Unexpected CALL_CLEAR_F in native 1x");
      }
      cmd_ptr->hdr.status = MC_DONE_S;
      break;
    } /* MC_CSF_CALL_CLEAR_F  */
   ...
   )
  {
    ...
    /* Wait for slew report from Searcher */
    {
          ...
          ** ----------------------------------------------------------- */
          if (!mcc_continue_on_curr_sys(MEAS_BACK_ALLOWED,
              sd_ss_ind_misc_meas_rprt( mcc_asid, 
                                        srch_ptr->sys_meas.meas_id, TRUE, 
                                        NULL )))
          {
                ...
                (mc_msgr_rcv_msg_type*) q_get( &mc_msgr_rcv.msg_q )) != NULL)
      {
        ...
        {
          mc_cdma_deactivate_protocol( mcc_asid,
                                       PROT_DEACT_REDIR_ABORT,
                                       PROT_TRANS_NONE );
          mcc_enqueue_dbg_msgr_cmd( msgr_cmd_ptr, prev_state, 
                                    next_state, FALSE, CDMA_JUMP_API );
        }
        else
        {
  ...
  if (next_state == CDMA_IDLE)
  {
    ...
    ** ------------------------------------------------------------------------*/
    ...
    if ( db_value.daylt != cur_bs_ptr->sy.daylt)
    {
      ...
      {
        ONEX_ERR_FATAL("Failed write item to NV %d", (int) mcc_temp_nv_buf.item, 0, 0);
      }
    }
      ...
      * ------------------------------------------------------------------- */
      if (!mcc_continue_on_curr_sys(MEAS_BACK_ALLOWED,
          sd_ss_ind_cdma_acq_schm(mcc_asid,
                                  cur_bs_ptr->sy.sid, cur_bs_ptr->sy.nid,
  ...
  while (next_state == CDMA_SYNC_MSG)  /*lint !e641 */
  {
      ...
      if ((msg_ptr = (mccrxtx_cmd_type*) q_get( &mcc_rxtx_q) ) != NULL)
      {
            ...
            (msg_ptr->msg.chn == CAIX_SC))
        {
          ...
          if (cdma.mob_cai_rev < msg_ptr->msg.msg.sync.min_p_rev)
          {
            ...
            ** ------------------------------------------------------------ */
            if (!mcc_continue_on_curr_sys(MEAS_BACK_ALLOWED,
              sd_ss_ind_cdma_acq_prot_mis( mcc_asid, NULL)))
            {
      ...
      if ((srch_ptr = (mccsrch_rpt_type*) q_get( &mcc_srch_q )) != NULL)
      {
        ...
        if (srch_ptr->hdr.rpt ==  SRCH_SYS_MEAS_R)
        {
          ...
          ** ----------------------------------------------------------- */
          if (!mcc_continue_on_curr_sys(MEAS_BACK_ALLOWED,
            sd_ss_ind_misc_meas_rprt( mcc_asid,
                                     srch_ptr->sys_meas.meas_id, TRUE, NULL)))
          {
            mcc_exit.exit = MCC_RESELECTION;
            next_state = CDMA_EXIT;
            event_report(EVENT_PREF_SYS_RESEL);
        ...
        else if ( srch_ptr->hdr.rpt == SRCH_REQUEST_SYS_RESTART_R )
        {
          ...
          ** ----------------------------------------------------------*/
          if ( !mcc_continue_on_curr_sys( MEAS_BACK_ALLOWED,
                  sd_ss_ind_cdma_acq_failed_unlock_rf( mcc_asid, NULL ) )
             )
          {
            M1X_MSG( DCP, LEGACY_HIGH,
    ...
    else if ((sigs & MCC_STATE_TIMER_SIG) != 0)
    {
      ...
      if ( !cdma2kdiag_get_switch( CDMA2KDIAG_SW_ENABLE_SOUNDER_MODE ) )
      {
        ...
        ** ------------------------------------------------------------- */
        if ( !mcc_continue_on_curr_sys( MEAS_BACK_ALLOWED,
                sd_ss_ind_cdma_acq_failed_sync( mcc_asid, NULL ) )
           )
        {
          mcc_exit.exit = MCC_ACQ_FAILED;
          next_state = CDMA_EXIT;
           ...
           (mc_msgr_rcv_msg_type*) q_get( &mc_msgr_rcv.msg_q )) != NULL)
      {
             ...
             (msgr_cmd_ptr->msg.hdr.id == ONEX_CP_LTE_ABORT_RESEL_REQ) )
        {
          mc_cdma_deactivate_protocol( mcc_asid,
                                       ...
                                       PROT_TRANS_NONE );
          mcc_enqueue_dbg_msgr_cmd( msgr_cmd_ptr, prev_state, 
                                    next_state, FALSE, CDMA_SYNC_MSG_API );
        }
        else
        {
          /*  Process the MSGR commands */
      ...
      if ((srch_ptr = (mccsrch_rpt_type*) q_get( &mcc_srch_q )) != NULL)
      {
        ...
        else if (srch_ptr->hdr.rpt == SRCH_LTE_1X_TT_FAIL_R)
        {
          ...
          /* ----------------------------------------------------------
            ...
            ** ----------------------------------------------------------*/
            if ( !mcc_continue_on_curr_sys( MEAS_BACK_ALLOWED,
                    sd_ss_ind_cdma_acq_failed_pilot( mcc_asid, NULL ) )
               )
            {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /* Set Pilot Channel acquisition time-out timer (T20m = 15-sec)
   * for keep track of expected SRCH response for SRCH_ACQ_F cmd */
  (void) rex_clr_timer( &mcc_state_timer );
  (void) rex_clr_sigs( MC_TCB_PTR, MCC_STATE_TIMER_SIG );
  (void) rex_set_timer( &mcc_state_timer, cai_tmo.t20m );
   
   M1X_MSG( DCP, LEGACY_MED,
              "Started Pilot Acq Timer:%d-ms", cai_tmo.t20m );
  ...
  while (next_state == CDMA_PIL_ACQ)  /*lint !e641 */
  {
           ...
           (mc_msgr_rcv_msg_type*) q_get( &mc_msgr_rcv.msg_q )) != NULL)
      {
             ...
             (msgr_cmd_ptr->msg.hdr.id == ONEX_CP_LTE_ABORT_RESEL_REQ) )
        {
          mc_cdma_deactivate_protocol( mcc_asid,
                                       ...
                                       PROT_TRANS_NONE );
          mcc_enqueue_dbg_msgr_cmd( msgr_cmd_ptr, prev_state,
		  	            next_state, FALSE, CDMA_PIL_ACQ_API );
        }
        else
        {
          /*  Process the MSGR commands */
    ...
    else if ((sigs & MCC_SRCH_Q_SIG) != 0)
    {
      ...
      if ((srch_ptr = (mccsrch_rpt_type*) q_get( &mcc_srch_q )) != NULL)
      {
        ...
        if (srch_ptr->hdr.rpt == SRCH_ACQ_R)
        {
          ...
          /* No need to explicitly clear the T20M mcc_state_timer and 
           * MCC_STATE_TIMER_SIG because we any way clear timer and sig 
           * once we are out of cdma_pil_acq() */
        }
        else if (srch_ptr->hdr.rpt == SRCH_ACQFAIL_R)
        {
          ...
          if ( !cdma2kdiag_get_switch( CDMA2KDIAG_SW_ENABLE_SOUNDER_MODE ) )
          {
            ...
            if(srch_ptr->acq_fail.trm_lock_not_avail == TRUE)
            {
              ...
              /* Restart the Pilot Channel acquisition time-out timer 
               * (T20m = 15-sec) to keep track of SRCH response for 
               * SRCH_ACQ_F cmd */
              (void) rex_clr_timer( &mcc_state_timer );
              (void) rex_clr_sigs( MC_TCB_PTR, MCC_STATE_TIMER_SIG );
              (void) rex_set_timer( &mcc_state_timer, cai_tmo.t20m );
              ...
              M1X_MSG( DCP, LEGACY_MED,
                 "Re-started Pilot Acq Timer:%d-ms for trm_lock_not_avail", 
                       cai_tmo.t20m );
    ...
    else if ((sigs & MCC_STATE_TIMER_SIG) != 0)
    {
       ...
       M1X_MSG( DCP, LEGACY_HIGH,
         "Sending the SRCH_ACQ_F with same values, Pilot Acq timer expiry" );
      ...
      M1X_MSG( DCP, LEGACY_MED,
       "Re-started Pilot Acq Timer:%d-ms for Pilot Acq timer expiry", 
               cai_tmo.t20m );
      ...
      if ( !cdma2kdiag_get_switch( CDMA2KDIAG_SW_ENABLE_SOUNDER_MODE ) )
      {
        ...
        ** --------------------------------------------------------- */
        M1X_MSG( DCP, LEGACY_HIGH,
          "CDMA pilot acquisition failed, Pilot Acq state timer expired" );
        /* ----------------------------------------------------------
        ** Inform system determination of pilot acquisition failure.
        ** Determine the next state to be processed based on returned
        ** SS-Action.
  ...
  while (MAIN_STATE( cdma.curr_state ) == CDMA_INIT)
  {
    ...
    switch (cdma.curr_state)
    {
      ...
      case CDMA_PIL_ACQ:
      {
        ...
        /* To be on Safe side, clear the Pilot Acq State timer */
        (void) rex_clr_timer( &mcc_state_timer );
        (void) rex_clr_sigs( MC_TCB_PTR,  MCC_STATE_TIMER_SIG );
      ...
      default:
      {
        ONEX_ERR_FATAL( "Bad init substate %d", cdma.curr_state, 0, 0 );
      }
    } /* end of switch */
  } /* end of while */
  ...
  * Hence the debug message have the API cdma_jump mentioned in it */
  ...
  while (next_state == CDMA_IDLE)
  {
    ...
    if ( (cmd_ptr = (mc_msg_type*)q_get(&mc_cmd_q)) != NULL)
    {
      ...
      else if(cmd_ptr->hdr.cmd ==  MC_CSFB_CALL_CLEAR_F)
      {
        /* CALL_CLEAR_F is only meant to be sent in native 1X for race condition
         * scenarios where 1X-CP was redirected to native 1X before getting
         * CALL_CLEAR from CM after 1X-CP reported some call failure via
             ...
             csfb_cdma.waiting_for_call_clear )
        {
          ...
          call_id = cmd_ptr->csfb_call_clear.call_id;
          M1X_MSG( DCP, LEGACY_HIGH,
                   "DEBUG:Rxed CALL_CLEAR_F while waiting for Auto_act");
        ...
        } /* if (Redir && waiting_for_call_clear) */
        else
        {
          M1X_MSG( DCP, LEGACY_ERROR,
                   "Unexpected CALL_CLEAR_F while waiting for Auto_act cnf");
        }
        cmd_ptr->hdr.status = MC_DONE_S;
              ...
              (mc_msgr_rcv_msg_type*) q_get( &mc_msgr_rcv.msg_q )) != NULL)
    {
         ...
         )
      {
        ...
        mcc_enqueue_dbg_msgr_cmd( msgr_cmd_ptr, old_state,
			          next_state, FALSE, MCC_RPT_AUTO_ACT_IND );
        break;
      }
      else
      {
