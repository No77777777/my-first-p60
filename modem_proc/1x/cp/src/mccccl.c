...
$Header: //components/rel/1x.mpss/8.0/cp/src/mccccl.c#2 $ $DateTime: 2020/01/27 00:26:21 $ $Author: pwbldsvc $
...
void append_reduced_active_set_log ( void )
{
  ...
  if ( (FPOS(LOG_REDUCED_ASET_INFO_C_type, reference_buf) + reduced_active_set_offset)
       > REDUCED_ACTIVE_SET_LOG_BUFFER_SIZE )
  {
    ONEX_ERR_FATAL("Exceeded reduced active set buffer",0,0,0);
  }
...
void mccccl_log_aset_info ( void )
{
  ...
  {
     ONEX_ERR_FATAL("Log exceeds buffer length", 0,0,0);
  }
