...
/* Ack seq of the message just transmitted by TX */
static byte rxtx_tx_ack_seq = 0;
  ...
  while( (wait_ptr = (rxtx_tx_wait_type*) q_get(&rxtx_tx_wait_q)) != NULL )
  {
    if(!wait_ptr->ack_req )
    {
      ...
      if( rxtx_noack_msgs[ wait_ptr->index].cmd_ptr != NULL )
      {
        ...
        #ifdef FEATURE_1X_CP_MEM_OPT
        /* Free the memory only if done_q_ptr is NULL. Otherwise the buffer will
         * be placed in done_q and will be freed by the task owning done_q */
        if (rxtx_noack_msgs[wait_ptr->index].cmd_ptr->hdr.cmd_hdr.done_q_ptr == NULL)
        {
          RXTX_FREE_QUEUE_BUF( rxtx_noack_msgs[wait_ptr->index].cmd_ptr,
                               rxtx_noack_msgs[wait_ptr->index].cmd_ptr->hdr.queue );
        }
        #endif
  ...
  while(new_state == RXTX_START_STATE )
  {
    ...
    if( (cmd_ptr= (rxtx_cmd_type*) q_get(&rxtx_mc_q)) != NULL )
    {
      switch(cmd_ptr->hdr.command )
      {
        ...
        default:
        {
          ONEX_ERR_FATAL("Invalid START cmd %d", (int) cmd_ptr->hdr.command,0,0 );
        }
      } /* switch */ /*lint !e788 */
...
/*lint -esym(550,status) */
  ...
  /* Temporary ptr required in case UHDM ack needs to be inserted at 
   * the head of TX wait queue for WTR hopping */
  rxtx_tx_wait_type *temp_rxtx_ptr;
  ...
  if (!rxtx_noack_msgs[index].active)
  {
    ...
    else
    {
      ...
      else
      {
        ...
        {          
          /* For WTR hopping, put the UHDM ack at the head of TX wait queue to
           * initiate the HHO as soon as possible. Till L2 ack for UHDM is not
           * transmitted, TX cannot be shutdown and TX cmd flow for WTR hopping
           * cannot be initiated. 
           * It does not matter if any pending messages do not reach BS due to 
           * this opt as we are going to change the channel anyway. 
           */
          if ( msg_ptr->hdr.cmd_hdr.sigs == RXTX_TX_HDM_ACK_SIG)
          {
            temp_rxtx_ptr = (rxtx_tx_wait_type*) q_check( &rxtx_tx_wait_q );
            if (temp_rxtx_ptr == NULL)
            {
              M1X_MSG( DCP, LEGACY_MED,
               "WTR_HOP: rxtx_tx_wait_q is empty, queuing HDM ack");              
              q_put(&rxtx_tx_wait_q, &wait_ptr->link );
            }
            else
            {
              M1X_MSG( DCP, LEGACY_HIGH,
               "WTR_HOP: rxtx_tx_wait_q is NOT empty, queuing HDM ack at top");             
              q_insert(&rxtx_tx_wait_q, &wait_ptr->link, &temp_rxtx_ptr->link);
            }
          } /* if ( msg_ptr->hdr.cmd_hdr.sigs == RXTX_TX_HDM_ACK_SIG) */
          else
          {
  ...
  if (msg_ptr->msg.msg.gen_tc.hdr.ack_req)
  {
       ...
       )
    {
      ...
      else
      {
        ...
        if (msg_ptr->msg.msg.ho.extra_parms)
        { 
          /* Check if WTR hopping is required. If yes, then RXTX needs to ack
           * MC when L2 ack for UHDM has been transmitted. */
          if (srch_rx_check_wtr_hopping(
                 (srch_rx_band_type)msg_ptr->msg.msg.ho.extra.band_class,
                 (srch_rx_chan_type)msg_ptr->msg.msg.ho.extra.cdma_freq, 
                 SRCH_RX_ONEX_MC ) 
                  == SRCH_RX_BAND_HOP)
          {
            if ( msg_ptr->msg.msg.ho.extra.band_class == cdma.band_class &&
                 msg_ptr->msg.msg.ho.extra.cdma_freq  == cdma.cdmach )
            {
                M1X_MSG( DCP, LEGACY_ERROR,
            ...
            {            
              M1X_MSG( DCP, LEGACY_MED,
                 "WTR_HOP:UHDM msg_seq:%d, rxtx_last_ack:%d, rxtx_next_ack:%d",
                      msg_ptr->msg.msg.gen_tc.hdr.msg_seq,
                      rxtx_last_ack,
                      rxtx_next_ack );  
              
              /* Check if next Acq_seq is for UHDM, if not then make it */
              while(rxtx_auto_ack[ rxtx_last_ack].ack_seq != 
                    msg_ptr->msg.msg.gen_tc.hdr.msg_seq)
              {
                M1X_MSG( DCP, LEGACY_MED,
                         "WTR_HOP:Last_Ack Seq:%d, Next_Acq Seq:%d",
                         rxtx_auto_ack[ rxtx_last_ack].ack_seq,
                ...
                {
                  /* Not expected to be here but safe side 
                   * break out of while loop */
                  M1X_MSG( DCP, LEGACY_ERROR,
                           "WTR_HOP:In to Infinite Loop, breaking loop" );
                  break;
                }
              } /* while() */
                     
              M1X_MSG( DCP, LEGACY_MED,
                       "WTR_HOP: Sending L2 Ack for HDM message with ack seq %d",
                       rxtx_auto_ack[ rxtx_last_ack].ack_seq );   
              /* We need Transmission cnf for HDM Ack so need to set these fields */
              ack_ptr->tc_msg.hdr.cmd_hdr.task_ptr = RXTX_TCB_PTR;
              ack_ptr->tc_msg.hdr.cmd_hdr.sigs = RXTX_TX_HDM_ACK_SIG;
              ack_ptr->tc_msg.hdr.cmd_hdr.done_q_ptr = NULL;          
            }
          }
        } /* if (msg_ptr->msg.msg.ho.extra_parms) */
...
void check_tx_wait_q( void )
{
  if( rxtx_tx_ptr != NULL )
  {
    ONEX_ERR_FATAL( "Bad tx_wait_q check", 0, 0, 0 );
  }
  else if( (rxtx_tx_ptr =
...
/*-   -   -   -   -   -   -   -   -   -   -   -   -   -   -   -   -   -   -*/
  ...
  switch( cmd_ptr->hdr.command )
  {
    ...
    default:
    {
      ONEX_ERR_FATAL ( "Invalid TC cmd %d", (int) cmd_ptr->hdr.command,0,0 );
    }
  } /* switch */ /*lint !e788 */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if( rxtx_tx_ptr == NULL )
  {
    ONEX_ERR_FATAL("rxtx_tx_ptr error",0,0,0 );
  }
  else
  {
    ...
    else
    {
      ...
      if( rxtx_noack_msgs[ rxtx_tx_ptr->index].cmd_ptr != NULL )
      {
        ...
        /* Ack seq of the message just transmitted by TX. Only meant to be 
         * used to track Ack for HDM messages */
        rxtx_tx_ack_seq = rxtx_noack_msgs[rxtx_tx_ptr->index].cmd_ptr->tc_msg.msg.gen_tc.hdr.ack_seq;

        M1X_MSG( DCP, LEGACY_MED,
         "Received TX cnf for msg not requiring ack, seq %d, sig 0x%x",
         rxtx_tx_ack_seq, rxtx_noack_msgs[rxtx_tx_ptr->index].cmd_ptr->hdr.cmd_hdr.sigs );
        ...
        /* Free the memory only if done_q_ptr is NULL. Otherwise the buffer will
         * be placed in done_q and will be freed by the task owning done_q */   
        if (rxtx_noack_msgs[rxtx_tx_ptr->index].cmd_ptr->hdr.cmd_hdr.done_q_ptr == NULL)
        {
          RXTX_FREE_QUEUE_BUF( rxtx_noack_msgs[rxtx_tx_ptr->index].cmd_ptr,
                               rxtx_noack_msgs[rxtx_tx_ptr->index].cmd_ptr->hdr.queue );
        }
        #endif
...
FUNCTION RXTX_SEND_MC_HDM_ACK_CNF

DESCRIPTION
  This task sends confirmation to MC when Ack for HDM message
...
void rxtx_send_mc_hdm_ack_cnf( void )
{
   /* Pointer to buffer to place cmd in */ 
  mccrxtx_cmd_type *msg_ptr;
  ...
  {  
    ...
    M1X_MSG( DCP, LEGACY_MED,
     "WTR_Hop: Sending TX cnf for HDM msg L2 ack to MC, ack seq %d",
     rxtx_tx_ack_seq );  
    
    /* Send HDM Ack confirmation to MC */
    msg_ptr->hdm_ack_cnf.hdr.command = MCCRXTX_HDM_ACK_CNF_F;
    msg_ptr->hdm_ack_cnf.ack_seq = rxtx_tx_ack_seq;
  
    #ifndef FEATURE_1X_CP_MEM_OPT
    msg_ptr->hdm_ack_cnf.hdr.cmd_hdr.done_q_ptr = &mcc_rxtx_free_q;
    #endif /* !FEATURE_1X_CP_MEM_OPT */
    
    msg_ptr->hdm_ack_cnf.hdr.cmd_hdr.task_ptr = NULL;
  
    /* Send Ack cnf and set signal for MC */
    mccrxtx_cmd( msg_ptr); 
  }
  
} /* rxtx_send_mc_hdm_ack_cnf */
    ...
    else
    {
      ...
      else
      {
        ...
        if( (sigs = rxtx_wait( RXTX_MC_Q_SIG | RXTX_RX_Q_SIG |
                    RXTX_FRAME_SIG | RXTX_TX_CMD_SIG | RXTX_TX_HDM_ACK_SIG ))
                   & RXTX_TX_CMD_SIG )
        {
        ...
        if( sigs & RXTX_TX_HDM_ACK_SIG)
        {
          /* This signal indicates L2 Ack for UHDM has been transmitted. RXTX
           * will inform MC of the same */
          (void)rex_clr_sigs(RXTX_TCB_PTR, RXTX_TX_HDM_ACK_SIG);
          rxtx_send_mc_hdm_ack_cnf();          
        }
      }
    }
...
)
{
  ...
  for( ;; )
  {
    switch( rxtx_state )
    {
      ...
      default:
      {
        ONEX_ERR_FATAL( "Invalid state %d",(int) rxtx_state, 0,0  );
      }
    }/* switch */
  }/* while */
...
)
{
  ...
  switch( queue_type )
  {
    ...
    default:
      ONEX_ERR_FATAL("Can't get here: Bad rxtx queue type %d", queue_type, 0, 0);
      break;
  }
...
/*===========================================================================
FUNCTION mc_qsh_mdump_collect_rxtx_data

DESCRIPTION
This function dumps the rxtx data to QSH after a crash
...
===========================================================================*/
QSH_MDUMP_FN_ATTR
void mc_qsh_mdump_collect_rxtx_data()
{
    qsh_mdump_collect_high(&rxtx_state,sizeof(rxtx_state));
}
