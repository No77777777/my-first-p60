...
$Header: //components/rel/1x.mpss/8.0/cp/src/mcctctag.c#2 $ $DateTime: 2020/01/27 00:26:21 $ $Author: pwbldsvc $
...
)
{
  ...
  for( i=0; i<MAX_TAG_ITEM_NUM; i++)
  {
    if (tag_mgr.tags[i].active && (tag_mgr.tags[i].tag_id == id ))
    {
      ...
      if (info == NULL)
      {
        ONEX_ERR_FATAL("info is Null Ptr", 0,0,0);
        /* return FALSE; */
      }
      else
      {
...
)
{
  ...
  else
  {
    ...
    if (info_ptr == NULL)
    {
      ONEX_ERR_FATAL("info_ptr is Null", 0,0,0);
      /* return FALSE; */
    }
    else
    {
    ...
    if (tag_id_ptr == NULL)
    {
      ONEX_ERR_FATAL("tag_id_ptr is Null", 0,0,0);
      /* return FALSE; */
    }
    else
    {
