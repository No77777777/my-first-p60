...
INITIALIZATION AND SEQUENCING REQUIREMENTS
  ...
  Copyright (c) 1995 - 2015 Qualcomm Technologies, Inc. 
...
$Header: //components/rel/1x.mpss/8.0/cp/src/srv.c#1 $ $DateTime: 2019/06/11 23:51:40 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
11/23/16   sjo     Enable 1x tune-away during MMS/SMS in non DDS sub
05/12/16   ppr     1x2GTA diag cmd changes
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  else
  {
    ...
    /* For SO-32(TDSO) we need to know when Serv Connect Msg is acked by BS
     * to send "Tune Away Ctrl_f" cmd to SRCH. It is needed if the call is in
     * non DDS sub with SO 6/14 also  */
    if( ( cdma.is_1x_on_non_dds_sub &&
      ...
      )
    {
      M1X_MSG( DCP, LEGACY_HIGH,
               "1xTA: Filling mcc_rxtx_ack_q for SCCM");
...
{
  M1X_MSG( DCP, LEGACY_MED,
           "WTR_HOP: Saved Service Conn Seq:%d",
           srv_saved_con_seq );
  return srv_saved_con_seq;
} /* mcc_wtr_hop_get_srv_con_seq()*/
