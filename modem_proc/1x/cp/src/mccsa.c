...
$Header: //components/rel/1x.mpss/8.0/cp/src/mccsa.c#2 $ $DateTime: 2020/01/27 00:26:21 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ------------------------------------------------------------
01/22/20   vaa     Implement QSH MDUMP framework
05/10/18   svu     Fixed back to back APHO on UE when it is on MT page rcvd PN
04/12/18   pga     Preserve cdma state to avoid second access attempt
...
11/10/17   ppr     Block AHO to MT page rcvd once UE got Ack for MT page rsp
...
04/01/16   ppr     Logging chgs
01/27/16   agh     Interface change in srch band/channel validation API
12/11/15   ppr     1xSRLTE:Corrected F3 logging
...
11/04/15   ppr     Corrected Reg Throttling Timer value calcuation
...
04/15/15   ppr     Debug code to find root cause for SRCH & CP out of sync
03/17/15   ppr     Chgs to honor "acquired_rf_resources" flag in
                   SRCH_ACCESS_READY_R rpt from SRCH
03/13/15   agh     Fixed a KW error in building access handoff list
        ...
        if (srch_rpt_ptr->pilots_meas.pilot_list[i].access_attempted)
        {
          ...
          ** ------------------------------------ */
                   ...
                   mccsa_pilot_rpt_ovhd_rec.prev_pilot)
          {
            if (pplt_idx < 0)
            {
              ONEX_ERR_FATAL("negative pplt_idx", 0, 0, 0);
            }
            else
            {
...
access_result_type mccsa_wait_for_srch_pilots_meas_rpt(void)
{
  ...
  if ( access_result == NOT_ABORT )
  {
    ...
    if(count >= 8)
    {
      ONEX_ERR_FATAL("MC and SRCH Out of Sync, count:%d", count, 0, 0);
    }
  ...
  if ( curr_state == CDMA_PAGE_RES )
    ...
    case MC_CDMA_PROT_DEACT_CMD_F:
      ...
      if( cdma.deregister_state == MCC_DEREGISTER_RSP_PENDING )
      {
        ...
        mcc_send_deregister_status(CM_DEREG_FAIL_PENDING_DEACT);
      }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (ack_found)
  {
    ...
    ** ------------------------------------ */
    ...
    (void) sd_ss_ind_cdma_opr_acc_term( mcc_asid,
    ...
    */
      ...
      )
    {
      ...
      if ((cm_ptr = cm_mc_rpt_get_buf_else_err_fatal()) != NULL)
      {
        ...
        cm_ptr->incoming_page.vfr = mcc_get_vfr();
        mcc_cm_mc_rpt( cm_ptr );
      }
        ...
        #endif /* FEATURE_IS2000_REL_A_AES */
        {
          ...
          ** -------------------------------------------------------- */
          ...
          if (!mcc_continue_on_curr_sys(MEAS_BACK_ALLOWED,
              sd_ss_ind_cdma_opr_reg_rej( mcc_asid, NULL)))
          {
            M1X_MSG( DCP, LEGACY_MED,
              "Exiting CDMA...");
            next_state = CDMA_EXIT;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  **---------------------------------------------------------------*/
  ...
  {
    ONEX_ERR_FATAL("PD message format error.", 0, 0, 0);
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if( timer_is_active(&mcc_pri_pch_hash_timer) )
  {
    M1X_MSG( DCP, LEGACY_HIGH,
             "Acc State: Valid OTA msg rcvd, Clearing Pri_PCH_Hash timer, "
             "PCH_Validation_done:%d, cdma.pagech:%d",
             mcc_get_idle_ho_pch_validation_done_flag(),
             cdma.pagech );
  } /* (mcc_pri_pch_hash_timer) */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
       ...
       )
    {
      ...
      ONEX_ERR_FATAL("Unknown channel type",0,0,0);
    }
  ...
  M1X_MSG( DCP, LEGACY_MED,
           "psist:%d, cancel:%d, cdma.accolc:%d",
           psist,
           cancel,
           cdma.accolc );
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ( access_aborted == NOT_ABORT )
  {
    if (!mccsa_layer2.access_in_progress)
    {
       ...
       * while MT Page is in progress and 
       * UE is not on the SRLTE MT page rcvd BS */
      else if(mcc_ue_not_on_mt_page_rcvd_bs() == TRUE)
      {
        M1X_MSG( DCP, LEGACY_MED,
                 "APHO allowed, since UE is not on MT page rcvd PN" );
        cdma.ho_state = CDMA_APHO;
        /* Also, note that the function mccsa_is_apho_allowed()    */
        /* increments the count of access Probe Handoffs performed */
        /* which is not correct in this state. Reset it, therefore */
    ...
    {
      ONEX_ERR_FATAL( "Two access attempts at once.", 0, 0, 0);
    }
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  ** -------------------------------------------------------*/
  ...
  mcc_tx_int_msg.page_resp.request_mode = sd_misc_get_cdma_request_mode( mcc_asid );
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  ** -------------------------------------------------------*/
  ...
  mcc_tx_int_msg.orig.request_mode = sd_misc_get_cdma_request_mode( mcc_asid );
  ...
  ** -------------------------------------------------------------- */
  ...
  if ( P_REV_IS_4_OR_GREATER )
  {
    ...
    mcc_tx_int_msg.orig.return_cause = sd_misc_get_cdma_return_cause( mcc_asid );
    M1X_MSG( DCP, LEGACY_HIGH,
      "Return Cause: %d",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  ** -------------------------------------------------------*/
  ...
  if (P_REV_IS_4_OR_GREATER)
  {
    ...
    mcc_tx_int_msg.reg.return_cause = sd_misc_get_cdma_return_cause( mcc_asid );
  }
...
)
{
  ...
  ** ------------------------------------------------- */
  ...
  switch ( cdma.sa_resp_ord.gen.order )
  {
    ...
    default:
      ONEX_ERR_FATAL ("Tried to respond with illegal order type %d",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  ** ---------------------------------------------------- */
  switch (cdma.sa_resp_msg)
  {
    ...
    default:
      ONEX_ERR_FATAL ("Tried to respond with unexpected message type %d",
...
static word mccsa_apho_done(word curr_state)
{
  ...
  */
    ...
    {
      ONEX_ERR_FATAL("APHO Done before updating Access Params??",0,0,0);
    }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  while (next_state == curr_state)
  {
    ...
    ** --------------------------------------------------------- */
        ...
        ** --------------------------------------------------- */
        if (rx_ptr->hdr.command == MCCRXTX_MSG_F)
        {
          ...
          if ((next_state == curr_state) && mccsa_init_aho)
          {
            ...
            {
              ONEX_ERR_FATAL("Unintialized mcc_srch_rpt", 0, 0, 0);
            }
    ...
    /* Process Primary PCH hash timer expiry immediately after
     * MCC_RXTX_Q_SIG so that, if we don't have any valid OTA msg
     * and if Primary PCH Hash timer expires then we can moved back to 
     * Primary PCH1 paging channel to collect SPM OTA msg and hash to
     * correct PCH if required. 
     ...
     * diff number of paging channels as nghbr_config=0 nghbrs */
              ...
              mcc_timer_on_callback_q( MCC_PRI_PCH_HASH_TIMER_ID ) 
            )
    {
      /* Clear the Timer */
      (void) mcc_clr_callback_timer( &mcc_pri_pch_hash_timer, 
      ...
      {
        /* Hash back to Primary Paging Channel PCH1 */
        next_state = mccidl_hash_to_primary_pch( curr_state );
        M1X_MSG( DCP, LEGACY_HIGH,
                 "Acc State: Pri PCH Hash Timer Expired, hashed to PCH1, "
                 "curr_state:0x%x, next_state:0x%x",
                 curr_state,
                 next_state );
      ...
      {
        M1X_MSG( DCP, LEGACY_ERROR,
         "Acc State:Ignoring invalid Pri PCH Hash Timer Expiry,cdma.pagech:%d",
                 cdma.pagech );
      }
    } /* MCC_PRI_PCH_HASH_TIMER_ID */
          ...
          ** -------------------------------------------------------------*/
          ...
          if ( (cdma.ho_state == CDMA_APHO) &&
               ( ( mccsa_is_apho_okay((word) srch_rpt_ptr->pc_meas.pilot,
                  ...
                  * while MT page is in progress even if it is disabled from 
                  * BS side */
                 || ((mcc_is_mt_page_in_prog() == TRUE) 
                     &&((cur_bs_ptr->pilot_pn !=(word) srch_rpt_ptr->pc_meas.pilot)
                        ||(cdma.cdmach  != (word) srch_rpt_ptr->pc_meas.freq) 
                        ||(cdma.band_class != srch_rpt_ptr->pc_meas.band_class))
                     &&(cdma.mt_page_rcvd_info.pilot_pn == (word) srch_rpt_ptr->pc_meas.pilot) 
                     &&(cdma.mt_page_rcvd_info.cdma_ch == (word) srch_rpt_ptr->pc_meas.freq) 
                     &&(cdma.mt_page_rcvd_info.band_class == srch_rpt_ptr->pc_meas.band_class)
                    )
               )
             )
          {
            M1X_MSG( DCP, LEGACY_MED,
              "Access Probe Handoff pending on MT page rcvd PN" );
  ...
  if ((curr_state == CDMA_REG) && (cdma.reg_type == CAI_POWERDOWN_REG)) /*lint !e641 */
  {
    ...
    else
    {
      next_state = CDMA_EXIT;
      mcc_exit.exit = MCC_POWERDOWN;
      ONEX_ERR_FATAL("Invalid registration type : %d",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  while (MAIN_STATE(new_state) == CDMA_SYS_ACC)
  {
    ...
    */
    ...
    switch (new_state)
    {
      ...
      default:
        ONEX_ERR_FATAL("Internal error bad state %d", new_state, 0, 0);
    } /* end of switch */
    ...
    /* -----------------------------------------------------------------
    ** When the deregister-f timer expires, cdma.curr_state will be 
    ** modified to exit state. That will cause to exit form the 
    ** registration attempt and without updating the new_state with 
    ** dma.curr_state, we end up doing registration attempt second time, 
    ** which is causing err_fatal with two access attempts in progress
    ** ----------------------------------------------------------------- */
    if ( (cdma.curr_state == CDMA_EXIT) && 
         (new_state != cdma.curr_state) )
    {
      M1X_MSG( DCP, LEGACY_HIGH,
               "Access: new_state:0x%x, cdma.curr_state:0x%x",
               new_state, 
               cdma.curr_state );
      new_state = cdma.curr_state;
    }
  ...
  ** ---------------------------------------------------------------- */
  ...
  {
    M1X_MSG( DCP, LEGACY_HIGH,
             "Exiting Acc State, Clearing Pri_PCH_Hash timer, "
             "PCH_Validation_done:%d, cdma.pagech:%d",
             mcc_get_idle_ho_pch_validation_done_flag(),
             cdma.pagech );
  } /* (mcc_pri_pch_hash_timer) */
  ...
  if (new_state == CDMA_EXIT)
  {
    ...
    mccsa_tx_stop();
    M1X_MSG( DCP, LEGACY_MED,
             "Tell TXC to go back to IDLE State, curr_state:0x%x, exit_rsn:%d",
             cdma.curr_state,
             mcc_exit.exit );
    ...
    ** -------------------------------------------------------------------- */
    switch (mcc_exit.exit)
    {
...
#endif /* FEATURE_PPP_CLEANUP_NON_SV_MODE */
      {
        ...
        ** --------------------------------------------------------------- */
        if (mcc_continue_on_curr_sys(MEAS_BACK_ALLOWED,
            ...
            mccsa_map_sa_state_to_sa_reason(cdma.curr_state),
      ...
      case MCC_ACCESS_FAILED_SYS_LOST:
      {
        ...
        sd_ss_acc_reas_e_type acc_rsn; /* To keep track of Access Reason */
        sd_ss_acc_term_e_type acc_term = SD_SS_ACC_TERM_ACC_FAIL; 
                                         /* Access termination reason */

        acc_rsn = mccsa_map_sa_state_to_sa_reason(cdma.curr_state);
        
        /* Check if the Sys Lost is due to fade time out happened 
         * while attempting registration */
        if(acc_rsn == SD_SS_ACC_REAS_REG)
        {
          uint32 reg_throt_count_max_in_ms; 
                   /* Variable to store Reg Throt time */
          uint32 temp_buf_time_in_ms; 
                   /* Temp Var to store Buffer time */
          int8 sci_s = mccidl_get_slot_cycle_index_s(); 
                            /* To store SCI being used */
          
          if(sci_s < 0)
          {
            M1X_MSG( DCP, LEGACY_MED,
             "SCI being used:%d, so using SCI=0 for reg throt calculation",
                     sci_s );
            /* If SCI being used <= 0 then use SCI=1 
             * for reg throt calculation */
            sci_s = 0;
          } /* if(sci_s <= 0) */         
          
          /* If it is Power-up Reg attempt then pass Power-up specific 
           * access termination and reason to MMODE such that MMODE can 
           * have the diff service loss recovery logic for power-up reg 
           * compared to normal reg attempt failure due to fade timeout */
          if(cdma.reg_type == CAI_POWERUP_REG)
          {
            /* Update Acc Term and Acc Rsn specific to power-up reg */
            acc_rsn = SD_SS_ACC_REAS_PWR_UP_REG;
            acc_term = SD_SS_ACC_TERM_FADE;
            
            /* For Power-up Reg, update reg_throt_count_max value to 
             * One SCI cycle */
            reg_throt_count_max_in_ms = (uint32) ((1 << sci_s) * 16 * 80);
          } /* if(reg_type == CAI_POWERUP_REG) */
          else
          {
            /* For Normal Reg, update reg_throt_count_max value to 
             * Two SCI cycle */
            reg_throt_count_max_in_ms = (uint32) (2 * (1 << sci_s) * 16 * 80);
          }
          
          /* Updage temp buffer time value to half of the SCI cycle value */
          temp_buf_time_in_ms = (uint32) ( ((1 << sci_s) * 16 * 80) / 2 );      
             
          /* Enable Reg Throttling such that 1x will release the chain by going 
           * to sleep and attempt next reg based reg_throt_count_max value 
           * subtracted by some buffer time to make sure next reg attempt 
           * (power-up = one wakeup cycle/normal reg = two wakeup cycles) 
           * if access attempt failed at start of slot boundary */           
           mcc_enable_reg_throttling(reg_throt_count_max_in_ms - 
                                             temp_buf_time_in_ms);             
        } /* if(acc_rsn == SD_SS_ACC_REAS_REG) */
        ...
        ** ------------------------------------------------------------ */
        if (mcc_continue_on_curr_sys(MEAS_BACK_ALLOWED,
            sd_ss_ind_cdma_opr_acc_term( mcc_asid, acc_rsn, acc_term, NULL)))
        {
            new_state = CDMA_IDLE;
        }
      ...
      case MCC_LOCK:
      {
        ...
        ** --------------------------------------------------------------- */
        if (mcc_continue_on_curr_sys(MEAS_BACK_ALLOWED,
            sd_ss_ind_cdma_opr_lock( mcc_asid, NULL)))
        {
          new_state = CDMA_IDLE;
        }
        if (cdma.curr_state == CDMA_ORIG) /*lint !e641 */
      ...
      case MCC_SYSTEM_LOST:
      {
        ...
        ** ------------------------------------------------------------- */
        if ( mcc_continue_on_curr_sys( MEAS_BACK_ALLOWED,
               sd_ss_ind_cdma_opr_sys_lost( mcc_asid, sys_lost_reason, NULL ) )
           )
        {
          new_state = CDMA_IDLE;
        }
...
/*===========================================================================
FUNCTION mc_qsh_mdump_collect_mccsa_data

DESCRIPTION
This function dumps the mccsa data to QSH after a crash
...
===========================================================================*/
QSH_MDUMP_FN_ATTR
void mc_qsh_mdump_collect_mccsa_data()
{
  qsh_mdump_collect_high(&mccsa_access_attempt_is_request,
                          sizeof(mccsa_access_attempt_is_request));
  qsh_mdump_collect_high(&mccsa_acct_all_mobiles_blocked_for_voice,
                          sizeof(mccsa_acct_all_mobiles_blocked_for_voice));
  qsh_mdump_collect_high(&mccsa_gen_auth_signature_in_progress,
                          sizeof(mccsa_gen_auth_signature_in_progress));
  qsh_mdump_collect_high(&mccsa_init_aho,sizeof(mccsa_init_aho));
  qsh_mdump_collect_high(&mccsa_layer2,sizeof(mccsa_layer2));
  qsh_mdump_collect_high(&mccsa_layer3_msg_pending,
                          sizeof(mccsa_layer3_msg_pending));
  qsh_mdump_collect_high(&mccsa_matched_msg,sizeof(mccsa_matched_msg));
  qsh_mdump_collect_high(&mccsa_num_apho_done,sizeof(mccsa_num_apho_done));
  qsh_mdump_collect_high(&mccsa_orig_or_page_ministate,
                          sizeof(mccsa_orig_or_page_ministate));
  qsh_mdump_collect_high(&mccsa_release_cause,sizeof(mccsa_release_cause));
  qsh_mdump_collect_high(&mccsa_apho_substate,sizeof(mccsa_apho_substate));
}
