...
$Header: //components/rel/1x.mpss/8.0/cp/src/mccsch.c#2 $ $DateTime: 2020/01/27 00:26:21 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
01/22/20   vaa     Implement QSH MDUMP framework
04/21/16   ppr     1X2GTA:Chgs to save rcvd ESCAM OTA msg.
03/24/16   ppr     1X2GTA:Chgs to deffer R/F-SCH Start txn if ESCAM assigned 
                   while HHO txns are pending due to TA
02/05/16   ppr     1x2GTA Feature changes
07/07/15   ppr     Reverting chgs to stop F-SCH when num_for_sch=0 in ESCAM
                    ...
                    (((uint32) packet->action.rxc_f_sch.sch_rate) < NUM_DEFINED_RATE))
                {
                  ...
                  if (packet->action.rxc_f_sch.num_ltu != 0)
                  {
                    ...
                    else
                    {
                      ...
                      ONEX_ERR_FATAL("Invalid RS", 0, 0, 0);
                    }
                  }
                  else
                  {
