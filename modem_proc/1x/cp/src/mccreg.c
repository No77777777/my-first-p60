...
INITIALIZATION AND SEQUENCING REQUIREMENTS
  ...
  Copyright (c) 1993 - 2020 Qualcomm Technologies, Inc.
...
$Header: //components/rel/1x.mpss/8.0/cp/src/mccreg.c#3 $ $DateTime: 2020/08/31 06:52:14 $ $Author: pwbldsvc $
...
05/23/16   ppr     Logging changes
11/04/15   ppr     Chgs to clear Reg Throttling Timer
10/05/15   ppr     Registration Throttling feature changes
09/15/15   sjo     Changes for proper processing of distance based registration
  ...
  while (sid_nid_q_cnt > 1)
  {
    ...
    ** -------------------------------------------- */
    ...
    for (i=0; i < sid_nid_q_cnt; i++)
    {
      if ( sid_nid_ptr == NULL )
      {
        ONEX_ERR_FATAL(" Null sid_nid_ptr, cnt=%d", sid_nid_q_cnt, 0, 0 );
      }
      else
      {
        sid = sid_nid_ptr->sid;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  timer_def2( &mccreg_throt_timer, NULL );
...
FUNCTION MCCREG_RELOAD_BOOTUP_NV_ITEMS

DESCRIPTION
  Reloads the NV items related to registration read while initial bootup.

DEPENDENCIES
  NV task muct be running
...
void mccreg_reload_bootup_nv_items( void )
{

  nv_stat_enum_type read_status;  /* NV read status */

  /* There is only one registration related NV item read while bootup */
  ...
  if (cdma.is_cdma_subs_avail)
  {
    ...
    ** --------------------------------------------------------------- */
    ...
    if ((read_status != NV_DONE_S) && (read_status != NV_NOTACTIVE_S))
    {
      ...
      {
        ONEX_ERR_FATAL("Bad NV read, Item %d, Cmd %d, Status:%d",
                  mccreg_nv_buf.item, 
                  mccreg_nv_buf.cmd, 
                  read_status);
    ...
    if ((read_status == NV_DONE_S) && (mccreg_nv_data.zone_list.sid != 0))
    {
      ...
      {
        ONEX_ERR_FATAL( "zone_free_q error", 0, 0, 0 );
      }
    }
  } /* if (cdma.is_cdma_subs_avail) */
  ...
  if (cdma.is_cdma_subs_avail)
  {
    ...
    ** --------------------------------------------------------------------- */
    ...
    if ((read_status == NV_DONE_S) && (mccreg_nv_data.sid_nid_list.sid != 0))
    {
      ...
      {
        ONEX_ERR_FATAL( "sid_nid_free_q error", 0, 0, 0 );
      }
    }
  } /* if (cdma.is_cdma_subs_avail) */
...
{
  boolean mmode_cap_sub = sd_misc_is_sub_multimode_capable( mcc_asid );
    /* Var to know mmode capable sub or not */
  ...
  ** ---------------------------------------------------------------- */
  ...
  M1X_MSG( DCP, LEGACY_MED,
           "Reg init, 1x_ASID:%d, mmode_cap_sub:%d, pwrup_opt_enabled:%d, skip_powerup_timer:%d,"
           "reg_status:%d, powering_up:%d, is_1x_on_non_dds_sub:%d",
           mcc_asid,
           mmode_cap_sub,
           cdma.pwrup_opt_enabled,
           cdma.skip_powerup_timer,
           mccreg_vars.reg_status,
           mccreg_vars.powering_up,
           cdma.is_1x_on_non_dds_sub );

  /* Skip the power-up timer if 
   * 1. Power-up optimization is enabled &&
   * 2. MMODE returns current Sub is MMODE capable or 
   *    VoLTE registration successful and 1x on non dds sub*/
  if ( ( cdma.pwrup_opt_enabled ) &&
       ( mmode_cap_sub || 
         (cdma.skip_powerup_timer && cdma.is_1x_on_non_dds_sub)
       )
     )
  {
    /* Set indicator that the powerup timer is no longer on */
    mccreg_vars.powerup_timer_on = FALSE;
    mccreg_vars.powering_up = FALSE;
  }
  else if ( (mccreg_vars.reg_status == MCCREG_REG_NEEDED) && 
            (mccreg_vars.powering_up)
          )
  {
...
/* This prevents all kinds of registration without updating the necessary
  ...
  */
  ...
  /* If device is configured in sniffer mode, then there should be no upward
   * communication with BS. Hence there should be no registration also */
  if( cdma.config_sniff_mode.sniff_mode_enabled == TRUE ) 
  {
    count++ ;
    if( count == 10 )
    { 
      /* This check is made to limit the f3 to come once every 10 times the
       * check is done */
      M1X_MSG( DCP, LEGACY_MED,
               "Sniff mode enabled. No Reg required");
      count = 0;
    }
    return MCCREG_NO_REG_REQUIRED;
  }
  ...
  #endif /* FEATURE_MODEM_1X_IRAT_LTO1X */
  {
    /* Check if we need to skip reg check due to Reg Throttling */
    if(mcc_check_if_reg_throt_timer_on() == TRUE)
    {
      /* Registration throttling is enabled so return NO_REG req */
      M1X_MSG( DCP, LEGACY_MED,
               "Reg Throt enabled, Skiping reg check" );      
      return MCCREG_NO_REG_REQUIRED;
    }
  ...
  ** -------------------------------------------------------------- */
  ...
  #endif /* FEATURE_MODEM_1X_IRAT_LTO1X */
  {
    if ( sd_misc_is_orig_mode( mcc_asid, 
                              SD_SS_ORIG_MODE_VAL_EMERG)
         ||
         ( mcc_passport_mode_is_jcdma() &&
           sd_misc_is_orig_mode( mcc_asid, SD_SS_ORIG_MODE_VAL_ORIG)
         )
       )
    {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /* If device is configured in sniffer mode, then there should be no uplink
   * communication with BS. Hence there should be no registration also */
  if( cdma.config_sniff_mode.sniff_mode_enabled == TRUE ) 
  {
    count++ ;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  ** ---------------------- */
  ...
  /* Reset skip_powerup_timer flag */
  cdma.skip_powerup_timer = FALSE;
  M1X_MSG( DCP, LEGACY_MED,
           "REG_COUNTs is set to 0 and skip_powerup_timer: %d ",
           cdma.skip_powerup_timer);
