...
05/03/18   ppr     Chgs to wait for ASET cmd response report from SRCH
04/13/18   ppr     Chgs to have Sniffer device SCI as DUT(MIN (MS_SCI, BS_SCI))
04/09/18   ppr     Chgs to clear Tr_Req timer
04/09/18   ppr     Chgs to try Force-Sleep & Rude-wakeup again if Trans Req 
                   timer expires while requesting for chain
09/12/17   ppr     API to get if 1x is waiting for RF resource or not.
11/28/17   ppr     Chgs to clear Tr_Req timer
11/17/17   ppr     Mainline GPP2_SUPPORT_ON_SEC_SUB feature
11/10/17   sjo     Recheck if reg needed or not only if the registration is 
                   not CAI_POWERDOWN_REG
11/10/17   sjo     Chgs to re-check if reg needed or not irrespective of the
                   change in state after idle handoff during rude wakeu-up.
11/10/17   ppr     Don't honor unlock events till SPM rcvd for Idle HO to 
                   same config ngbr and SPM not in stored ovhds
11/10/17   sjo     Chgs to avoid the check to see if the ovhds are current when
                   1X is about to send ACCESS_F for doing registration.
11/10/17   sjo     Chgs to add API for SRCH to query if an empty GPM was rxed.
11/10/17   ppr     Chgs to do Reg check after Force-sleep & Rude-wakeup 
                   before sending Access_f to SRCH
11/10/17   sjo     Chgs to retry rude wakeup for a maximum of 3 times until
                   srch is ready to acquire primary TX chain.
11/10/17   sjo     Chgs to query SRCH if a rude wakeup is required before 
                   sending SRCH_ACCESS_F.
11/10/17   sjo     changes to do sleep rude wakeup for multiple attempts 
                   for registration.
10/30/17   ppr     Remove Feature flag ONEX_TA_30_BRINGUP.
08/31/17   sjo     Chgs to add new failure rsn for MC_DEREGISTER_F cmd.
08/31/17   sjo     Chgs for CDMA_support on SUB2
08/29/17   sjo     Pass SCI 0 instead of 1 in the srch parms on receipt of 
                   SYS_RELEASE when the SCI agreed with BS is 0 itself.
06/05/17   ppr     Chgs not to call srch_get_stats_info() API while MC in 
                   Exit/Enter State
10/13/16   ppr     Chgs to allow Idle HO to strong PN for Fast RAHO scenarios 
                   while rx.config_msg_seq validation pending
08/29/16   sjo     Added LTE_ABORT_1XCSFB_HO_REQ to msgr dbg buffer.
                   Created new debug buffer for lte rrc responses
07/21/16   sjo     Added more commands to mc_dbg_cmd_q.
07/12/16   sjo     Cleaned some medium level f3s.
07/08/16   ppr     QSH:Enable event diagnostic data to QSH
07/07/16   ppr     Chgs to ingore MT pages to know n/w page rep pattern
06/18/16   sjo     Debug buffer enhancements. Added more confirmation rpts to
...
03/29/16   ppr     Backing out ignoring MT page if VoIWLAN is in progress
02/18/16   ppr     1X2GTA:Priority Inversion changes
02/11/16   bb      UE side enhancement chgs to fix same NGBR_CONFIG=0 PN's 
                   having diff Paging Channels configured in NL ovhd msg
02/11/16   agh     Honor band/channels in GSRM that require WTR hopping
02/05/16   ppr     1x2GTA Feature changes
02/05/16   sjo     Skip processing ESPM,SPM,ECL if waiting for ACCESS_READY_R
                   to avoid channel hashing and CP sending ASET_F to SRCH
01/27/16   agh     Interface change in srch band/channel validation API
12/23/15   sjo     Added new rude wakeup rsn to distinguish between rude wakeup
                   for MO/MT and reg access attempts.
...
10/27/15   ppr     WTR HOP:Chgs for ECAM wtih AM=1(Paging Channel Assignment)
10/23/15   sjo     Changes for power-up time optimization. FR-27607
10/16/15   agh     Ignore MT page if VoIWLAN is in progress
10/16/15   ppr     QTA:Chgs to keep track of pending Rude Wake-up and Aset cmd
10/05/15   ppr     Registration Throttling feature changes
...
08/07/15   ppr     Chgs to clear Trans Req Timer
07/15/15   agh     For Split overheads, honor system Release from SRCH only
                   if at least one OTA is received in current wakeup cycle
07/13/15   ppr     HHO WTR Hopping Changes
07/01/15   jh      Changes to support WTR hopping
06/29/15   ppr     Chgs to avoid sys lost due to BS insanity timer expiry
                   while in split ovhd collection
06/19/15   agh     Added Diag cmd to modify Ovhds Refresh time for testing
06/06/15   jh      Chg to temporarily following srlte path for all calls
06/01/15   sjo     Enhance debugging interface between MMOC and CP
06/05/15   ppr     Chgs to send calculated idle_pgslot while sending default
                   srch_params to SRCH for spilt overhead scenarios
04/02/15   agh     Changes to support Voice Adapter based Audio architecture
03/26/15   agh     1X-CP changes to support QTA feature
03/24/15   bb      Changes for not using default parameters during TC state
03/17/15   ppr     Chgs to honor "acquired_rf_resources" flag in
...
/* Keep track of the lte rrc cnf buffer */
static word lte_rrc_dbg_buf_index = 0;

/* Keep the count of total responses sent to lte rrc */
static word lte_rrc_total_rsp_sent = 0;
    ...
    if ( auth_cmd_ptr->hdr.status == AUTH_RECEIVED_S )
    {
      do
      {
        ...
        ** ---------------------------------- */
        ...
        {
          ONEX_ERR_FATAL( "Wait returned sig for empty Q!", 0, 0, 0 );
        }
      } while (!resync);
    }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  ** ----------------------------------------------- */
  ...
  switch (msg_type )
  {
    ...
    default:
    {
      ONEX_ERR_FATAL( "mc_get_msg bad msg_type %d", msg_type, 0 , 0 );
    }
  } /* end of switch */
...
void mcc_init_ovhd_info()
{
  ...
  ** -------------------------------------------------------------- */
  ...
  for (i=0; i< MC_MAX_BASES; i++)
  {
    ...
    bs_info[i].rx.config_msg_seq_val_pending = FALSE;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
            ...
            )
    {
      ...
      else
      {
             ...
             ( qw_lo( mcc_old_time ) < cdma.ovhd_refresh_time ) )
        {
          ...
          good_old = TRUE;
          M1X_MSG( DCP, LEGACY_MED,
                   "Stored ovhds not expired on PN:%d,, BC:%d, CH:%d,"
                   " refresh time:%d in 20 ms",
                   bs_info_id->pilot_pn, 
                   bs_info_id->band_class,
                   bs_info_id->cdma_ch,
                   cdma.ovhd_refresh_time );
        }
        #ifdef FEATURE_1X_OVHD_OPTIMIZATION
        ...
        else if ( mcc_is_opt_supported( VALIDATE_EXPIRED_OVHDS ) == TRUE )
        {
          ...
          M1X_MSG( DCP, LEGACY_MED,
                   "Refresh OVHDS - PN:%d, BC:%d, CH:%d",
                   bs_info[i].pilot_pn,
                   bs_info[i].band_class,
                   bs_info[i].cdma_ch );
  ...
  {
    M1X_MSG( DCP, LEGACY_MED,
             "Need to collect fresh ovhds on PN:%d, BC:%d, CH:%d,"
             " refresh time = %d in 20 ms",
             bs_info_id->pilot_pn, 
             bs_info_id->band_class,
             bs_info_id->cdma_ch,
...
)
{
  ...
  else
  {
    ...
    /* By default clear the config_msg_seq validation pending flag 
     * as we received an OTA on current BS */
    cur_bs_ptr->rx.config_msg_seq_val_pending = FALSE;
  }
} /* mcc_put_seq */
  ...
  switch( msg_type )
  {
    case CAI_SYS_PARM_MSG:
    {
      ...
      ** ------------------------------------------------------------------ */
      ...
      #ifdef FEATURE_1X_SPLIT_OVERHEAD
      if( mcc_get_idle_ho_spm_wait_block_sys_rel_flag() == TRUE )
      {
        /* Clear idle_ho_spm_wait_block_sys_rel_flag if set to TRUE */
        mcc_set_idle_ho_spm_wait_block_sys_rel_flag( FALSE );
        M1X_MSG( DCP, LEGACY_HIGH,
                 "SPM Rcvd, Cleared SPM_block_sys_rel_flag:%d, "
                 "Split_Ovhd_block_sys_rel_flag:%d",
                 mcc_get_idle_ho_spm_wait_block_sys_rel_flag(),
    ...
    default:
      ONEX_ERR_FATAL( "mc_put_msg bad msg_type %d", msg_type, 0, 0 );
      break;
...
{
  M1X_MSG( DCP, LEGACY_MED,
           "Clearing the Ovhds for PN:%d, BC:%d, CH:%d", 
           cur_bs_ptr->pilot_pn,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch (bcd_digit)
  {
    ...
    default:
      ONEX_ERR_FATAL( "Illegal digit code %d", bcd_digit, 0, 0 );
...
//       #endif /* FEATURE_HDR_HYBRID */
  ...
  if(match_found == PAGE_MATCH)
  {
    /* Ignore MT page if EFS Item to ignore MT pages is enabled 
     * to know the n/w Page Repetition Pattern */
    if(cdma.ignore_mt_pages)
    {
      match_found = NO_MATCH;
      /* Increment the Ignored page count */
      cdma.ignored_mt_page_cnt++;

      M1X_MSG( DCP, LEGACY_HIGH,
      "Ignoring MT page with SO:%d for Page Rep Pattern, "
      "ignored_mt_page_cnt:%d",
               page_info->service_option,
               cdma.ignored_mt_page_cnt );
    } /* if(ignore_mt_pages) */
    
    /* Ignore MT page if UI is not ready. This check is implemented to support
     * power up time optimization (FR 26707). As a part of this optimization, 
     * it can happen that UI many not be ready but modem will be up. In such 
     * situations, we need to reject the MT voice call received.
     */
    else if(cm_get_ui_ready_status() == FALSE)
    {
      M1X_MSG( DCP, LEGACY_HIGH,
               "Ignoring MT page with SO:%d as UI is not ready",
               page_info->service_option );
      match_found = NO_MATCH;
    }
     
    /* Ignore MT OTAPA page if NV#260-NV_OTAPA_ENABLED_I set to FALSE */
    else if( (cdma.otapa_enabled == FALSE) &&
    ...
    else if(cdma.is_special_srv_data_call)
    {
      M1X_MSG( DCP, LEGACY_HIGH,
    ...
    else if(cdma.deregister_state == MCC_DEREGISTER_RSP_PENDING)
    {
      M1X_MSG( DCP, LEGACY_HIGH,
        "MT page(SO-%d) ignored while deregister_f in progress",
        page_info->service_option);
      match_found = NO_MATCH;
    } /* if ( deregister_state == MCC_DEREGISTER_RSP_PENDING) */
...
)
{
  ...
  db_items_value_type dbi;
    /* DB items holder */
  boolean wtr_hop_required = FALSE;
  /* Indicates if WTR_HOPPPING is required */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  ** ------------------------------------------------------ */
  ...
  /* WTR hopping may be required if band_class changes in PC_ASSIGN msg */
  if( pc_assign_msg_ptr->mode.am1.freq_incl &&
      ( (cdma.cdmach != pc_assign_msg_ptr->mode.am1.cdma_freq) ||
        (cdma.band_class != pc_assign_msg_ptr->mode.am1.band_class)
      )
  ...
  {
    /* Check if WTR Hopping required
     * If yes then shutdownt Tx and do WTR Hopping */
    if( srch_rx_check_wtr_hopping(
            (srch_rx_band_type) pc_assign_msg_ptr->mode.am1.band_class,
            (srch_rx_chan_type) pc_assign_msg_ptr->mode.am1.cdma_freq,
            SRCH_RX_ONEX_MC )
            == SRCH_RX_BAND_HOP
      )
    {
      wtr_hop_required = TRUE;
      M1X_MSG( DCP, LEGACY_HIGH,
               "WTR_Hop req for pc_assign with BC:%d, CH:%d",
               pc_assign_msg_ptr->mode.am1.band_class,
               pc_assign_msg_ptr->mode.am1.cdma_freq );                 
      /* Send request to Transmit task to go to START mode */
      mcc_tx_buf.exit.hdr.command = TXC_EXIT_F;
      mcc_txc_cmd( &mcc_tx_buf );  
    } /* srch_rx_check_wtr_hopping() */
  } /* (freq_incl) */
  ...
  do
  {
    ...
    else if ((sigs & MCC_SRCH_Q_SIG) != 0) /* Searcher has returned! */
    {
      ...
      else
      {
        ...
        {
          boolean srch_rpt_proc_pending = FALSE;
            /* To know if we need to proc SRCH Rpt with priority */
          /*----------------------------------------------------------*/
         ...
         /* Wait for ASET mask to take effect */
         (void) mccidl_wait_for_srch_aset_update( new_state,
                                                  &srch_rpt_proc_pending );
    ...
    {
      ONEX_ERR_FATAL( "MCC_WAIT returned bad sig", 0, 0, 0);
    }
  }
  while (!got_searcher_report);

  /* Now that SRCH is tuned to new band/freq re-enable TX by sending
   * CDMA_F cmd.  
   */ 
  if(wtr_hop_required)
  {
    M1X_MSG( DCP, LEGACY_MED,
      "WTR_Hop: Re-enabling TX by sending TXC_CDMA_F, TXC_PC_F");        
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    ONEX_ERR_FATAL( "No free buffers on mc_auth_free_q", 0, 0, 0 );
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
#ifdef FEATURE_1X_CP_MEM_OPT
    ONEX_ERR_FATAL( "Memory allocation failed for MC_UIM_Q", 0, 0, 0 );
#else /* !FEATURE_1X_CP_MEM_OPT */
    ONEX_ERR_FATAL( "No free buffers on mc_uim_free_q", 0, 0, 0 );
#endif /* FEATURE_1X_CP_MEM_OPT */
  }
...
/* End of deep copy of MMGSDI Rpt if required */
  ...
  {
#ifdef FEATURE_1X_CP_MEM_OPT
    ONEX_ERR_FATAL( "Memory allocation failed for MC_MMGSDI_Q", 0, 0, 0 );
#else /* !FEATURE_1X_CP_MEM_OPT */
    ONEX_ERR_FATAL( "No free buffers on mc_mmgsdi_free_q", 0, 0, 0 );
#endif /* FEATURE_1X_CP_MEM_OPT */
  }
  ...
  switch ( req_typ )
  {
    ...
    case CAI_TERM_INFO_REQ:
    {
      ...
      else
      {
        ...
        switch (op_mode)
        {
          ...
          default:
            ONEX_ERR_FATAL( "Impossible op_mode value", 0, 0, 0);
            break;
    ...
    case CAI_OP_MODE_REQ:
    {
      ...
      else
      {
        ...
        {
          ONEX_ERR_FATAL( "Impossible band value", 0, 0, 0);
        }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  for (rec_num = 1; rec_num <= num_recs; rec_num++)
  {
    if ((cmd_ptr = cm_mc_rpt_get_buf_else_err_fatal()) == NULL)
    {
      ONEX_ERR_FATAL( "No buffers on cm_mc_rpt_free_q", 0, 0, 0 );
    }
    else
    {
...
#ifdef FEATURE_CDSMS
           cm_mt_dbm_msg_extn( (byte) CAI_SHORT_MSG_SERVICES,
     ...
     )
  {
    ...
    * across multple wakeup is supported and SPM or/ and ESPM is not rcvd */
       ...
       )
    {
      ...
      /* If SCI is 0, then we need to pass 0 itself to SRCH. Otherwise we might
       * miss some MT pages. If it is any value other than 0, pass the value 1.
       * If the SCI is 2 then also CP will pass SCI 1 to srch. This will help
       * in collecting the ovhds much faster. Also it guarantees that UE will
       * not miss any MT page. 
       */
      
      mcc_srch_buf.parm.slot_cycle_index      = MIN( 1, mccidl_get_slot_cycle_index_s() ) ;
  ...
  else /* not ftm mode */
  {
    ...
    if(IS_1X_TO_LTE_RESELECTION_ENABLED)
    {
      mcc_srch_buf.parm.atim_enabled   = cur_bs_ptr->csp.sp.atim_ind &&
        ...
        sd_misc_is_lte_full_srv_allowed( mcc_asid );
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
#ifdef FEATURE_1X_CP_MEM_OPT
    ONEX_ERR_FATAL( "Memory allocation failed for MC_DH_RPT_Q", 0, 0, 0 );
#else /* !FEATURE_1X_CP_MEM_OPT */
    ONEX_ERR_FATAL( "No free buffers on mc_dh_free_q", 0, 0, 0 );
#endif /* FEATURE_1X_CP_MEM_OPT */
  }
...
)
{
  ...
  switch (status_code)
  {
    ...
    case CM_DEREG_FAIL_PENDING_DEACT:
    case CM_DEREG_FAIL_ALREADY_DEACT:
    {
...
boolean mcc_sys_meas_cmd()
{
  ...
  (void) sd_ss_act_get_per_sub(&ss_act_buf, mcc_asid);
  ...
  for (i=0; sd_ss_meas_list_get_per_sub(i, &ss_sys.mode, &ss_band, 
                                        &ss_sys.chan, mcc_asid); i++)
  {
  ...
  if (mcc_srch_buf.hdr.status != SRCH_DONE_S)
  {
    ...
    for (i=0; (i < mcc_srch_buf.sys_meas.list_cnt) && (i < MAX_SYS_MEAS_LIST_SIZ); i++)
    {
      ...
      ** ------------------------- */
      ...
      {
        if (!sd_ss_meas_list_rprt_per_sub(i, (int2) NO_SYS_MEAS_RESULTS, mcc_asid))
        {
          M1X_MSG( DCP, LEGACY_ERROR,
    ...
    if (!mcc_continue_on_curr_sys(MEAS_BACK_ALLOWED,
        sd_ss_ind_misc_meas_rprt( mcc_asid,
                                 mcc_srch_buf.sys_meas.meas_id, 
                                 TRUE, NULL)))
    {
  ...
  for (i=0; i < rpt_ptr->sys_meas.list_cnt; i++)
  {
    ...
    ** ------------------------- */
    if (rpt_ptr->sys_meas.meas_list[i].member_type == MEAS_MEMBER_AMPS)
    {
      ...
      if (!sd_ss_meas_list_rprt_per_sub(i,
  ...
  switch (chan_assign_msg_ptr->assign_mode)
  {
    ...
    case CAI_PAGING_CHAN_ASSIGN:
      ...
      if (!chan_assign_msg_ptr->mode.am1.freq_incl)
      {
        if (mcc_continue_on_curr_sys(sys_meas_mode,
          sd_ss_ind_cdma_opr_cam_cdma( mcc_asid, 
      ...
      else
      {
           ...
           )
        {
          ...
          sd_ss_ind_cdma_opr_cam_cdma( mcc_asid,
            SD_SS_HO_CDMA_CAM_TO_CDMA_PAG, (sd_band_e_type) cdma.band_class, 
            chan_assign_msg_ptr->mode.am1.cdma_freq, NULL)))
        {
          chan_assign_accepted = TRUE;
        }
    ...
    case CAI_TRAFFIC_CHAN_ASSIGN:
      ...
      if (!chan_assign_msg_ptr->mode.am0.freq_incl)
      {
        if (mcc_continue_on_curr_sys(sys_meas_mode,
          sd_ss_ind_cdma_opr_cam_cdma( mcc_asid,
             SD_SS_HO_CDMA_CAM_TO_CDMA_TRF, (sd_band_e_type) cdma.band_class,
             cdma.cdmach, NULL)))
        {
          chan_assign_accepted = TRUE;
        }
    ...
    case CAI_ACQ_ANALOG_SYSTEM:
      if (sd_ss_ind_cdma_opr_cam_amps( mcc_asid, 
                                      SD_SS_HO_CDMA_CAM_TO_AMPS_SYS,
              (sd_cell_sys_e_type) chan_assign_msg_ptr->mode.am2.analog_sys, 0,
                                       SD_WILDCARD_SID, 
                                       NULL) == SD_SS_ACT_ACQ_AMPS)
      {
        chan_assign_accepted = TRUE;
      }
      break;
    ...
    case CAI_ANALOG_VCHAN_ASSIGN:
      ...
      if (sd_ss_ind_cdma_opr_cam_amps( mcc_asid, 
	  	...
	  	(sd_cell_sys_e_type) 0, chan_assign_msg_ptr->mode.am3.analog_chan,
...
)
{
  ...
  if (sd_ss_ind_cdma_opr_redir( mcc_asid, 
      redir_info->return_if_fail,
...
)
{
  ...
  {
    ONEX_ERR_FATAL("ddtm_ptr is null", 0, 0, 0);
  }
...
)
{
...
#endif /* FEATURE_MODEM_1X_IRAT_LTO1X */
  {
       ...
       )
    {
      ...
      {
        /* Found the SID, NID, We are registered OR Sniffer mode enabled
         * so use best slot_cycle. 
         * Standard for Rev 0, A, B, C Says - the mobile station shall
         * use a slot cycle index no greater than SLOT_CYCLE_INDEXs */

        M1X_MSG( DCP, LEGACY_HIGH,
                 "SID:%d,NID:%d FOUND OR Sniffer_Mode:%d,"
                 "Updating Oper_SCI min of MS_Pref_SCI:%d, BS_Pref_SCI:%d",
        ...
        sci_s = (int8) MIN( cdma.curr_sci_pref, 
                            cur_bs_ptr->csp.sp.max_slot_cycle_index );
      }
      else
      {
...
)
{
  ...
  if (group_cnt >= MAX_BAND_GROUPS)
  {
    /* There is no room in band class table and gp does not
       already exist in it
    */
    ONEX_ERR_FATAL("Band Class Table FULL",0,0,0);
    return FALSE;
  }
  else
  {
      ...
      if (i >= mcc_band_table[group_cnt].num_bands)
      {
        ...
        */
        ...
        else
        {
          ...
          ONEX_ERR_FATAL("Band class array full for this group",0,0,0);
        }
      } /* if */
    } /* for band_cnt */
  }
...
)
{
  ...
  sd_ss_band_pref_e_type band_pref = sd_misc_get_supp_band_pref( mcc_asid );
...
)
{
  ...
  if ( continue_proc )
  {
    ...
    /* Waiting for RF Resource Flag */
    cdma.waiting_for_rf_res = TRUE;
    ...
    /* Clear waiting for RF Resource Flag */
    cdma.waiting_for_rf_res = FALSE;
...
)
{
  ...
  byte rude_wakeup_retry_count = 0;
     /* Rude wakeup shall be attempted for a max of 3 times. */
  word prev_state = new_state;

  boolean srch_require_rude_wakeup = 
                  !srch_pr_rx_chain_is_using_tx_compatible_reason();
  /* Boolean to check if rude wakeup needs to be triggered again.
   * SRCH API returns TRUE if we are already have the primary chain
   * with ACCESS/TRAFFIC reason and don't need Force-sleep/Rude-wakeup
   * to enter into Access state.
   * SRCH API returns FALSE if we don't have the primary chain with 
   * ACCESS/TRAFFIC reason and MC has to do Force-sleep/Rude-wakeup
   * to enter into Access state.
   *
   * Note that the negated return value is stored in the boolean. 
   */
  ...
  M1X_MSG( DCP, LEGACY_MED,
    "DEBUG: rude_wakeup_with_tx_req_done:%d, sa_tc_state:0x%x, "
    "reg_trans_req_denial_cnt:%d, srch_req_rude_wkup:%d, wait_for_rf_res:%d",
    cdma.is_rude_wakeup_with_tx_req_done,
    sa_tc_state,
    cdma.reg_trans_req_denial_cnt,
           srch_require_rude_wakeup,
           cdma.waiting_for_rf_res );

  /* The sleep-rude wake shall be triggered on the following scenarios.
   * 1. cdma.is_rude_wakeup_with_tx_req_done is FALSE.
   *    If this varaible is set, then CP has already asked for rude wakeup
   *    with tx_resources_required set to TRUE. Then we need not do the 
   *    sleep-rude wakeup cycle again.
   * 
   * 2. If there was a transciever request timer expiry while attempting
   *    registration, then while coming back for the another attempt, we 
   *    should do sleep-rude wakeup irrespective of whether the 
   *    cdma.is_rude_wakeup_with_tx_req_done is set to TRUE during the 
   *    first registration attempt.
   * 
   * 3. srch_require_rude_wakeup is set to TRUE.
   *    FALSE - SRCH is in a position to get transceiver resources 
   *           without rude wakeup..
   *    TRUE - SRCH needs a rude wakeup.
   */
  if( (cdma.is_rude_wakeup_with_tx_req_done != TRUE)||
      ( ( sa_tc_state ==  CDMA_REG ) &&
        (cdma.reg_trans_req_denial_cnt <= MAX_REG_TRANS_REQ_DENIAL_CNT)
      )||
      ( srch_require_rude_wakeup == TRUE )
    )
  {
    /* CP needs to trigger forced sleep - rude wakeup cycle until SRCH
     * returns rude wakeup is not required. Currently the design is to
     * loop through for a maximum of 3 retires.
     */
    while( ( (srch_require_rude_wakeup = 
               !srch_pr_rx_chain_is_using_tx_compatible_reason() ) == TRUE )&&
           ( rude_wakeup_retry_count < 3 )
         )
    {
      M1X_MSG( DCP, LEGACY_HIGH,
               "srch_pr_rx_is_tx_comp_rsn:%d, rude_wakeup_retry_count:%d",
               srch_pr_rx_chain_is_using_tx_compatible_reason(),
               rude_wakeup_retry_count );

      /* Increment the rude wakeup count */
      rude_wakeup_retry_count ++ ;
      ...
      {
        /* Forced SRCH to Sleep due to QTA */
      ...
      #endif /* FEATURE_MODEM_1X_QTA */
      {
        ...
        mccidl_trigger_sleep(SRCH_FSLEEP_REGULAR);
      }
      ...
      while(MCC_BLOCKED_WAITING == cont_blocked_wait_srch)
      {
               ...
               )
        {
          ...
          M1X_MSG( DCP, LEGACY_HIGH,
                   "TransReq timer expired, rude_wakeup_retry_count:%d", 
                   rude_wakeup_retry_count );
        }
      } /* while() */

      /* Force sleep to SRCH failed */
      if(cont_blocked_wait_srch == MCC_BLOCKED_WAIT_FAILURE)
      {
        /* SRCH_REQUEST_SYS_RESTART_R: Check if we need to exit */
        if(new_state == CDMA_EXIT)
        {
          /* Clear waiting for RF Resource Flag */
          cdma.waiting_for_rf_res = FALSE;
         
          /* Safe side clear Trans Req Timer */
          (void) mcc_clr_timer_on_callback_q(MCC_TR_REQUEST_TIMER_ID);

          return new_state;
        }
        /* MCC_TR_REQUEST_TIMER_ID Expiry case */
        else 
        {
          /* TransReq timer expired so try again sending Force sleep and 
           * Rude-wakeup after sending SRCH_IDLE_F cmd to bring SRCH back
           * to Idle state */
          mcc_srch_buf.idle.hdr.cmd = SRCH_IDLE_F;
          mcc_srch_cmd(&mcc_srch_buf);
          
          /* Continue the while loop to send Force sleep and Rude-wakeup again */
          continue;
        }       
      } /* if(cont_blocked_wait_srch == MCC_BLOCKED_WAIT_FAILURE) */

      /* Trigger the Rude wakeup now. 
       * The wakeup reason are set as follows:
       * 1. MCC_RUDE_WAKEUP_MO_MT_ACC_RSN - if it is an MO/MT voice/data call.
       * 2. MCC_RUDE_WAKEUP_SMS_ACC_RSN   - SMS over access channel or
       *                                    MO/MT call with SO=6,14.
       * 3. MCC_RUDE_WAKEUP_REG_ACC_RSN   - otherwise. */

      if( ( (sa_tc_state == CDMA_ORIG) || 
            (sa_tc_state == CDMA_PAGE_RES)
          ) &&
          ( !CAI_IS_SO_SMS(cdma.so_req) )
        )
      {
        new_state = mccidl_blocked_rude_wakeup(new_state, 
                              MCC_RUDE_WAKEUP_MO_MT_ACC_RSN);
      }
      else if ( ( ( (sa_tc_state == CDMA_ORIG) || 
                    (sa_tc_state == CDMA_PAGE_RES)
                  ) &&
                  ( CAI_IS_SO_SMS(cdma.so_req) )
                ) ||
                (sa_tc_state == CDMA_MSG)                
              ) 
      {
        new_state = mccidl_blocked_rude_wakeup(new_state, 
                              MCC_RUDE_WAKEUP_SMS_ACC_RSN);
      }
      else
      {
        new_state = mccidl_blocked_rude_wakeup(new_state, 
                              MCC_RUDE_WAKEUP_REG_ACC_RSN);
      }
      ...
      {
        M1X_MSG( DCP, LEGACY_HIGH,
                 "SLTE: State changed, prev_state:0x%x, new_state:0x%x",
                  prev_state,
                  new_state );
        if(new_state == CDMA_EXIT)
        {
      ...
      /* Check if we are requesting trans res for REG. If so, re-check
       * if the registration is needed or not. It might happen that during 
       * Rude-wakeup we might have done Idle HO. For example, zone based 
       * reg might be not required if we did an idle handoff to an
       * already registered zone. Also here we need not check if there has
       * been a state change to IDLE_INIT. Because the handoff can happen
       * to a BS with all the overheads current and stored. In that case, 
       * there will not be any state change.

       * Also the check need to be done only if the reg_type is any other
       * than power-down registration. Power-down registration need to be
       * done anyways. Moreover if the mccidl_reg_check API is called, then
       * UE will end up either doing any other registration or no
       * registration at all. */

      if( (sa_tc_state == CDMA_REG)&&
          (cdma.reg_type != CAI_POWERDOWN_REG)
        )
      {    
        /* Do the reg check again if reg is really req or not. */
        cdma.reg_type = mccreg_idle_check();
        M1X_MSG( DCP, LEGACY_MED,
                 "Rechecking reg needed or not, reg_type = %d ",
                 cdma.reg_type );
        if(cdma.reg_type == MCCREG_NO_REG_REQUIRED)
        {
          /* Reset the flags to default values */
          mccidl_reset_reg_required();
          ...
          M1X_MSG( DCP, LEGACY_MED,
             "Registration not req, waiting_for_rf_res:%d, returning",
                   cdma.waiting_for_rf_res );
  ...
  if( ( (srch_require_rude_wakeup = 
          !srch_pr_rx_chain_is_using_tx_compatible_reason() ) == TRUE ) &&
        ( rude_wakeup_retry_count >= 3 )
    )
  {
    ONEX_ERR_FATAL( "Rude wakeup retries exceed limit. "
               "srch_require_rude_wakeup = %d, rude_wakeup_retry_count = %d",
               srch_require_rude_wakeup, rude_wakeup_retry_count, 0 ); 
  }
  ...
  {
    M1X_MSG( DCP, LEGACY_HIGH,
      "Updating transceiver request from:0x%x to:0x%x, wait_for_rf_res:%d",
      cdma.ho_sa_tc_state,
             sa_tc_state,
             cdma.waiting_for_rf_res );
  }
  ...
  {
    M1X_MSG( DCP, LEGACY_HIGH,
      "Requesting transceiver for:0x%x, wait_for_rf_res:%d",
             sa_tc_state,
             cdma.waiting_for_rf_res );
  }
...
void mcc_send_txc_early_jump_cmd (void)
{
  ...
  ** ----------------------------------------------------------------- */
  ...
  else
  {
    ...
    {
      ONEX_ERR_FATAL("Jump to hyperspace before updating Access Params??",0,0,0);
    }
  }
...
)
{
       ...
       chan_index++)
  {
    ...
    if ( srch_rx_check_wtr_hopping(band_chan.band, band_chan.chan_num, 
         SRCH_RX_ONEX_MC ) != SRCH_RX_BAND_REJECT )
    {
...
void mcc_ta_init (void)
{
  ...
  /* Initialize TA complete buffer timer */
  rex_def_timer_ex( &mcc_ta_comp_buf_timer, mcc_expired_timer_callback_func,
                    MCC_TA_COMP_BUF_TIMER_ID );                    
...
)
{
  ...
  /* Return FALSE if MC is in either Exit/Enter state */
  if( (MAIN_STATE(cdma.curr_state) == CDMA_ENTER) ||
      (MAIN_STATE(cdma.curr_state) == CDMA_EXIT)
    )
  {
    M1X_MSG( DCP, LEGACY_ERROR,
      "CDMA in Exit/Enter State:0x%x",
      cdma.curr_state );
    return FALSE;
  }
     ...
     )
  {
    ...
    if( dbg_item_ptr != NULL )
    {
      ...
      if (mmoc_cmd->hdr.cmd == MC_CDMA_PROT_GEN_CMD_F)
      {
        ...
        dbg_item_ptr->cmd.gen_cmd.info.asubs_id =
                                  mmoc_cmd->gen_cmd.info.asubs_id;
		dbg_item_ptr->cmd.gen_cmd.info.stk_id =
                                  mmoc_cmd->gen_cmd.info.stk_id;
...
)
{
  ...
  if( ( msgr_cmd->msg.hdr.id == ONEX_CP_LTE_REDIR_REQ )||
      ...
      ( msgr_cmd->msg.hdr.id == ONEX_CP_LTE_ABORT_1XCSFB_HO_REQ)
    )
  {  	
    /* Delete the oldest cmd in case the debug cmd queue is full */
    ...
    if( dbg_item_ptr != NULL )
    {
      ...
      switch(msgr_cmd->msg.hdr.id)
      {
        ...
        case ONEX_CP_LTE_ABORT_1XCSFB_HO_REQ:
          dbg_item_ptr->cmd.id = MC_ONEX_CP_LTE_ABORT_1XCSFB_HO_REQ;
          break;
        default:
...
DESCRIPTION
  This procedure will keep track whether the confirmation or the indication
  has been sent to lte rrc. This will help in debugging issues like whether
  msgr commands have been processed and the confirmation or indication has
  been sent back to lte rrc. These information will be present in the dumps.
...
SIDE EFFECTS
  None
===========================================================================*/
void mcc_enqueue_dbg_lte_rrc_cnf
(
    mc_api_name_type      api_name,
       /* API in which this cmd is processed */
    mc_lte_rrc_cnf_name_type lte_rrc_cnf_name
       /* lte rrc cnf/rsp which was sent */
)
{
  byte i;
    /* buffer index */

  if ( lte_rrc_dbg_buf_index >= NUM_MAX_LTE_RRC_RSPS )
  {
    lte_rrc_dbg_buf_index = lte_rrc_dbg_buf_index % NUM_MAX_LTE_RRC_RSPS;
  }

  i = lte_rrc_dbg_buf_index;

  /* Increment the index */
  lte_rrc_dbg_buf_index++;

  lte_rrc_cnf_dbg_buf[i].lte_rrc_cnf_name = lte_rrc_cnf_name;
  lte_rrc_cnf_dbg_buf[i].api_name         = api_name;
  lte_rrc_cnf_dbg_buf[i].cdma_time        = m1x_time_get_ms_native();
  lte_rrc_cnf_dbg_buf[i].ticks_time       = timetick_get();

  /* Keep a count of total SRCH-> MC reports in current power cycle */
  lte_rrc_total_rsp_sent ++;
  
} /* mcc_enqueue_dbg_lte_rrc_cnf */
...
{
  /* Return TRUE to force SRCH to ignore unlcok callback from TRM,
   * if any of the below falg is set to TRUE
   * 1. split_ovhd_block_sys_rel_flag OR
   * 2. idle_ho_spm_wait_block_sys_rel_flag */

  return ( cdma.split_ovhd_block_sys_rel_flag ||
           cdma.idle_ho_spm_wait_block_sys_rel_flag
         );
} /* mcc_get_split_ovhd_block_sys_rel_flag */
...
FUNCTION MCC_SET_IDLE_HO_SPM_WAIT_BLOCK_SYS_REL_FLAG

DESCRIPTION
   This API will set cdma global var flag, whether we are waiting for
   SPM OTA msg after Idle handoff and need to blcok Sys_Rel from SRCH side.
...
void mcc_set_idle_ho_spm_wait_block_sys_rel_flag
( 
  boolean flag 
)
{
  cdma.idle_ho_spm_wait_block_sys_rel_flag = flag;

  /* If flag is for FALSE then on defensive side, 
   * clear Primary PCH hash timer */
  if( flag == FALSE )
  {
    (void) mcc_clr_callback_timer( &mcc_pri_pch_hash_timer, 
                                   MCC_PRI_PCH_HASH_TIMER_ID );
  }
} /* mcc_set_idle_ho_spm_wait_block_sys_rel_flag() */

/*===========================================================================

FUNCTION MCC_GET_IDLE_HO_SPM_WAIT_BLOCK_SYS_REL_FLAG

DESCRIPTION
   This API will return cdma global var flag, whether we are waiting for
   SPM OTA msg after Idle handoff and need to blcok Sys_Rel from SRCH side.
...
RETURN VALUE
  TRUE - If we are waiting for SPM OTA msg after Idle Handoff and 
         need to blcok Sys_Rel rpt from SRCH side.
  FALSE - Otherwise.
...
boolean mcc_get_idle_ho_spm_wait_block_sys_rel_flag
( 
  void 
)
{
  return cdma.idle_ho_spm_wait_block_sys_rel_flag;
} /* mcc_get_idle_ho_spm_wait_block_sys_rel_flag() */

/*===========================================================================

FUNCTION MCC_SET_IDLE_HO_PCH_VALIDATION_DONE_FLAG

DESCRIPTION
   This API will set cdma global var flag, whether we received valid OTA
   after same config nghbr Idle handoff triggered.
...
void mcc_set_idle_ho_pch_validation_done_flag
( 
  boolean flag 
)
{
  cdma.idle_ho_pch_validation_done = flag;
} /* mcc_set_idle_ho_pch_validation_done_flag() */

/*===========================================================================

FUNCTION MCC_GET_IDLE_HO_PCH_VALIDATION_DONE_FLAG

DESCRIPTION
   This API will return cdma global var flag, whether we received valid OTA
   after same config nghbr Idle handoff triggered.
...
RETURN VALUE
  TRUE - If we received valid OTA after Idle handoff triggered.
  FALSE - Otherwise.
...
boolean mcc_get_idle_ho_pch_validation_done_flag
( 
  void 
)
{
  return cdma.idle_ho_pch_validation_done;
} /* mcc_get_idle_ho_pch_validation_done_flag() */

/*===========================================================================

FUNCTION MCC_CHECK_EMPTY_GPM_RXED

DESCRIPTION
   This function will return TRUE is MC has received an empty GPM. It will
   return TRUE if an empty GPM has been received and MC has not moved to
   any states other than IDLE and it has not even gone to SLEEP. SRCH
   will be querying this API.

DEPENDENCIES
  None.

RETURN VALUE
  TRUE - If an empty GPM was received and MC is still in idle state
         and has not gone to sleep. 
  FALSE - Otherwise
...
boolean mcc_check_empty_gpm_rxed( void )
{
  return cdma.empty_gpm_rxed;  
} /* mcc_check_empty_gpm_rxed */

/*===========================================================================

FUNCTION MCC_ENABLE_REG_THROTTLING

DESCRIPTION
  This API will start the Registration Throtting timer and 
  will set the Registration Throttling timer on flag

DEPENDENCIES
  This API is called when any registration attempt got failed 
  due to fade timeout
...
===========================================================================*/
void mcc_enable_reg_throttling
(  
  uint32 time_in_ms
)
{
  /* Start Reg Throttling timer and set Reg Throttling Timer on flag  */
  timer_reg( &mccreg_throt_timer, 
             (timer_t2_cb_type)mcc_proc_reg_throt_timer_expiry,
             (timer_cb_data_type)NULL, (uint32)time_in_ms, 0L );
  mccreg_vars.reg_throt_timer_on = TRUE;
  
  M1X_MSG( DCP, LEGACY_HIGH,
           "Reg Throt timer started with:%d-msec, reg_throt_timer_on:%d",
           (uint32)time_in_ms, 
           mccreg_vars.reg_throt_timer_on );
} /* mcc_enable_reg_throttling() */

/*===========================================================================

FUNCTION MCC_PROC_REG_THROT_TIMER_EXPIRY

DESCRIPTION
  This function will handle Registration Throttling timer expiry
  and resettig the Reg Throt timer on flag.
...
void mcc_proc_reg_throt_timer_expiry
(
  void
)
{
  ...
  M1X_MSG( DCP, LEGACY_MED,
           "Reg Throttling Timer expiry, reg_throt_timer_on:%d",
           mccreg_vars.reg_throt_timer_on );           
} /* mcc_proc_reg_throt_timer_expiry() */
/*===========================================================================

FUNCTION MCC_CHECK_IF_REG_THROT_TIMER_ON

DESCRIPTION
  This API will check if registration throttling is enabled or not

DEPENDENCIES
  None

RETURN VALUE
  TRUE - If Registration Throttling timer is on
  FALSE - Otherwise
...
===========================================================================*/
boolean mcc_check_if_reg_throt_timer_on
(
  void
)
{
  return mccreg_vars.reg_throt_timer_on;
} /* mcc_check_if_reg_throt_timer_on() */

/*===========================================================================

FUNCTION MCC_IS_MT_PAGE_IN_PROG

DESCRIPTION

  This API checks if MT page processing is in progress.

DEPENDENCIES
  None

RETURN VALUE
  TRUE - If MT Page is in progress.
  FALSE - Otherwise
...
===========================================================================*/
boolean mcc_is_mt_page_in_prog
(
  void
)
{
  return ( cdma.mt_page_in_progress );
} /* mcc_is_mt_page_in_prog() */

/*===========================================================================

FUNCTION MCC_UE_NOT_ON_MT_PAGE_RCVD_BS

DESCRIPTION

  This API checks whether UE is on the same BS,
  where we received MT page or not.

DEPENDENCIES
  None

RETURN VALUE
  TRUE - If UE is on MT Page rcvd BS.
  FALSE - Otherwise
...
===========================================================================*/
boolean mcc_ue_not_on_mt_page_rcvd_bs
(
  void
)
{
  ...
  /* Return TRUE if MT page in progress and 
   * UE is not on same BS where MT page is recieved */
  if( cdma.mt_page_in_progress == TRUE )
  {
    M1X_MSG( DCP, LEGACY_MED,
           "MT page, cur_bs_pn:%d, CDMA CH:%d, BC:%d",
           cur_bs_ptr->pilot_pn,
           cdma.cdmach,
           cdma.band_class );
    if( (cdma.mt_page_rcvd_info.pilot_pn != cur_bs_ptr->pilot_pn) ||
        (cdma.mt_page_rcvd_info.cdma_ch != cdma.cdmach) ||
        (cdma.mt_page_rcvd_info.band_class != cdma.band_class)
      )
    {
      status = TRUE;
    }
