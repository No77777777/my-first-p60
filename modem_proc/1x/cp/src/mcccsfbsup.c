...
$Header: //components/rel/1x.mpss/8.0/cp/src/mcccsfbsup.c#2 $ $DateTime: 2020/01/27 00:26:21 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ------------------------------------------------------------
01/22/20   vaa     Implement QSH MDUMP framework
11/17/17   ppr     Mainline GPP2_SUPPORT_ON_SEC_SUB feature
08/31/17   ppr     Log SID/NID from SIB8 params
08/31/17   sjo     Chgs for CDMA_support on SUB2
08/02/17   sjo     FR 37336: Migration to MSGR VARIANT from Inst ID
11/18/16   sjo     Chgs to treat LTE_NAS_IRAT_INVALID_STATE as hard failure if
                   the access attempt is for pwr down registration.
08/29/16   sjo     Chgs to handle LTE_ABORT_1XCSFB_HO_REQ if the cmd was rxed
                   in traffic state. Also added the cmd to msgr dbg buffer.
...
06/08/16   sjo     Ignore sending GCSNA ACK for the MT SMS rxed while waiting
                   for the response from NAS for the PARAM_REQ sent
06/07/16   sjo     Chgs to handle MC_UE_MODE_CHGD_CMD_F when UE in CSFB state.
05/27/16   srk     LLVM 8.x compilation error fixes
04/12/16   ppr     Chgs to ingnore invalid MOEUTRA with MT Page paging ch msg
02/05/16   ppr     1x2GTA Feature changes
01/27/16   agh     Interface change in srch band/channel validation API
01/25/16   ppr     CFSB:Chgs to send ONEX_CP_LTE_ABORT_1xCSFB_HO_RSP to RRC 
                   for all the retry attempts exhausted case
11/09/15   ppr     Chgs to abort tranmission of C2K Parms Req 
                   if C2K Params Resp received before NAS_UL_MSG_CNF
11/09/15   agh     Support QSH framework to collect periodic/event metrics
10/23/15   sjo     Changes to support power-up time optimization. FR-27607
10/16/15   agh     Ignore MT page if VoIWLAN is in progress
09/22/15   agh     Handle ATTACH_COMPLETE while ESR in progress for MO/MT call
07/17/15   sjo     Changes to ignore MT page while pwr dwn reg is progressing
06/22/15   ppr     Chgs to enable csfb_sib8 powerDownReg by default if that 
...

{
  ...
  switch (cmd_ptr->hdr.cmd)
  {
...
#endif /* FEATURE_MODEM_1X_SRVCC */
    {
      ...
      /* SRVCC mode */
      if( (srvcc_mode == TRUE) &&
          (cmd_ptr->hdr.cmd == MC_SRVCC_ORIGINATION_F)
        )
      {
        M1X_MSG( DCP, LEGACY_MED,
      ...
      /* CSFB mode */
               ...
               (cmd_ptr->hdr.cmd == MC_CSFB_ORIGINATION_F)
             )
#endif /* FEATURE_MODEM_1X_SRVCC */
      {
        M1X_MSG( DCP, LEGACY_MED,
...
#endif /* FEATURE_MODEM_1X_SRVCC */
    {
      ...
      if(call_id == cdma.call_id)
      {
        ...
        /* Reset waiting for call clear flag */
  ...
  #endif /* FEATURE_MODEM_1X_SRVCC */
    {
      ...
      /* SRVCC mode */
      if( (srvcc_mode == TRUE) &&
          (cmd_ptr->hdr.cmd == MC_SRVCC_CALL_END_F)
        )
      {
      ...
      /* CSFB mode */
               ...
               (cmd_ptr->hdr.cmd == MC_CSFB_CALL_END_F)
             )
#endif /* FEATURE_MODEM_1X_SRVCC */
      {
...
#ifdef FEATURE_MODEM_1X_SRVCC
        /* Only send CALL_END_CNF for CSFB_CALL_END cmd and
         * for SRVCC_CALL_END_F cmd no need to send CALL_END_CNF to CM */
        if(srvcc_mode == FALSE)
#endif /* FEATURE_MODEM_1X_SRVCC */
        {
    ...
    case MC_MO_DBM_F:
    {
      ...
      ** ------------------------------------------------------------*/
      ...
      if (sd_misc_is_cdma_locked( mcc_asid ))
      {
      ...
      ** ---------------------------------------------------------*/
      ...
      {
        ONEX_ERR_FATAL( "NULL SMS pointer", 0, 0, 0);
      }
          ...
          if ( trans_req_status == MCC_TRANSCEIVER_RESOURCE_LOST )
          {
            ...
            ** ------------------------------------------------------------- */
            if ( !mcc_continue_on_curr_sys( MEAS_BACK_ALLOWED,
                 sd_ss_ind_cdma_opr_sys_lost( mcc_asid, 
                                              cdma.system_lost_reason,
                                              NULL 
                                            )
                                         )
               )
...
void mcc_csfb_proc_irat_switch(mc_msgr_rcv_msg_type *msgr_ptr)
{
  ...
  switch ( msgr_ptr->msg.hdr.id )
  {
    ...
    case ONEX_CP_LTE_RESEL_REQ:
    {
      ...
      mcc_enqueue_dbg_msgr_cmd( msgr_ptr, old_state, cdma.curr_state,
                                FALSE, MCC_CSFB_PROC_IRAT_SWITCH );
          ...
          )
        {
          ...
          if(proc_irat_switch == FALSE)
          {
            if(msgr_ptr->msg.hdr.id == ONEX_CP_LTE_REDIR_REQ)
            {
              ...
              mcc_enqueue_dbg_lte_rrc_cnf( MCC_CSFB_PROC_IRAT_SWITCH, 
...
void mcc_csfb_process_sib8_msg(mc_msgr_rcv_msg_type *msgr_ptr)
{
  ...
  {
    M1X_MSG( DCP, LEGACY_MED,
      "DEBUG :SIB8 Pre-reg info is valid, 1x_ASID:%d, SID:%d, NID:%d",
             mcc_asid,
             mcc_csfb_sib8_msg.reg_params.sid,
             mcc_csfb_sib8_msg.reg_params.nid );
    /* Perform the Negative system check. */
    if(sd_misc_allowed_sid_nid( mcc_asid,
                                mcc_csfb_sib8_msg.reg_params.sid,
                                mcc_csfb_sib8_msg.reg_params.nid
                              )
      )
  ...
  if(!cdma.is_sib8_proc_deferred)
  {
     ...
     */
     ...
     sd_ss_ind_update_cdma_sib8_nbrs( mcc_asid, &mcc_sib8_nbr_list );
...
#endif /* FEATURE_MODEM_1X_IRAT_LTO1X_TUNNELED_SMS */
    default:
      ONEX_ERR_FATAL("Illegal msg_type = %d is filled.",
  ...
  ** -------------------------------------------------------------- */
  ...
  if ( P_REV_IS_4_OR_GREATER )
  {
    ...
    mcc_gcsna_int_msg.crkt_srv_msg.l3_msg.orig.return_cause = 
                                 sd_misc_get_cdma_return_cause( mcc_asid );
    M1X_MSG( DCP, LEGACY_MED,
...
void xlate_gcsna_int_msg(byte msg_type)
{
  ...
  if(msg_type != mcc_gcsna_int_msg.crkt_srv_msg.tlac_hdr.msg_type)
  {
    ...
    ONEX_ERR_FATAL("mcc_gcsna_int_msg is not filled according to msg_type = %d",
      msg_type, 0, 0);
  }
...
boolean mcc_csfb_set_sd_context(void)
{
  ...
  sd_action = sd_ss_ind_user_cdma_irat_lte( mcc_asid,
                                            SD_SS_REDIR_CDMA_LTE_REDIR,
                                            &(mc_irat_redir_rec),
                                            1,
                                            NULL );
  ...
  */
  ...
  if((sd_action != SD_SS_ACT_ACCEPT) ||
     ((sd_ss_ind_cdma_opr_acpt_rej( mcc_asid, NULL )) != SD_SS_ACT_ACQ_CDMA)
    )
  {
    M1X_MSG( DCP, LEGACY_ERROR,
...
void mcc_csfb_process_mo_from_eutra(lte_irat_tunnel_msg_container_s *mo_ho_buf_ptr)
{
  ...
  */
    ...
    )
  {
    ...
    { /* CSFB Mode */
      ...
      mcc_enqueue_dbg_lte_rrc_cnf( MCC_CSFB_PROCESS_MO_FROM_EUTRA,
  ...
  */
  ...
  if( mcc_gcsna_ext_msg_ptr->gen_msg_id.is_l3_msg 
                                      == MCC_CSFB_DUP_L3_MSG_RXED ||
      ( mcc_gcsna_ext_msg_ptr->gen_msg_id.msg_id == GCSNA_1X_CRKT_SRV_MSG &&
        mcc_gcsna_ext_msg_ptr->gen_msg_id.cdma_chn == GCSNA_CHN_PC &&
        mcc_gcsna_ext_msg_ptr->ckt_srv_msg.msg_type == CAI_GENERAL_PAGE_MSG
      )
    )
  {
    /* As there is no possibility of getting the duplicate Mobility from EUTRA 
     * this might be a NW bug as Invalid Mobilty from EUTRA case 
     * so send handoff failure ind to LTE-RRC, to indicate the call failure
     * to CM via RRC->NAS->CM. After indicating the error, expect CM to
     ...
     */
    M1X_MSG( DCP, LEGACY_HIGH,
        "DEBUG:Rxed Invalid/Dup MO-EUTRA, sending HO FAILURE to LTE-RRC");
    ...
    { /* CSFB Mode */
      ...
      mcc_enqueue_dbg_lte_rrc_cnf( MCC_CSFB_PROCESS_MO_FROM_EUTRA_1,
                                   MC_ONEX_CP_LTE_1xCSFB_HO_FAILED_RSP );
    } /* CSFB Mode */
    mcc_gcsna_ext_msg_ptr->gen_msg_id.is_l3_msg = MCC_CSFB_L3_MSG_NOT_RXED;
    return;
...
)
{
  ...
  if (msg_ptr == NULL)
  {
    ONEX_ERR_FATAL ("Memory allocation failed for MC_MCC_RXTX_Q", 0, 0, 0 );
  }
#else /* !FEATURE_1X_CP_MEM_OPT */
  ...
  if (msg_ptr == NULL)
  {
    ONEX_ERR_FATAL ("No buffers on mcc_rxtx_free_q", 0, 0, 0);
  }
#endif /* FEATURE_1X_CP_MEM_OPT */
  else
  {
      ...
      if ((srch_ptr = (mccsrch_rpt_type*) q_get( &mcc_srch_q )) != NULL)
      {
        ...
        else if (srch_ptr->hdr.rpt == SRCH_LTE_1X_TT_FAIL_R)
        {
          ...
          { /* CSFB Mode */
            ...
            mcc_enqueue_dbg_lte_rrc_cnf( MCC_CSFB_CDMA_DO_TIME_TRANSFER_API,
                                         MC_ONEX_CP_LTE_1xCSFB_HO_FAILED_RSP );
            
            /* Mark that UE is waiting for csfb_call_clear_f from CM */
            csfb_cdma.waiting_for_call_clear = TRUE;            
          } /* CSFB Mode */
    ...
    { /* CSFB Mode */
      ...
      */
      ...
      mc_msgr_send_msg( &ho_fail_rsp.hdr,
                        ONEX_CP_LTE_1xCSFB_HO_FAILED_RSP,
                        sizeof(ho_fail_rsp));
      mcc_enqueue_dbg_lte_rrc_cnf( MCC_CSFB_PROCESS_TUNNELED_UHDM,
                                   MC_ONEX_CP_LTE_1xCSFB_HO_FAILED_RSP );
    ...
    )
  {
    ...
    { /* CSFB Mode */
      ...
      mc_msgr_send_msg(&ho_fail_rsp.hdr,
                       ONEX_CP_LTE_1xCSFB_HO_FAILED_RSP,
                       sizeof(ho_fail_rsp));
      mcc_enqueue_dbg_lte_rrc_cnf( MCC_CSFB_PROCESS_TUNNELED_UHDM_1,
                                   MC_ONEX_CP_LTE_1xCSFB_HO_FAILED_RSP );
    } /* CSFB Mode */
    M1X_MSG( DCP, LEGACY_ERROR,
  ...
  if(! mcc_csfb_create_and_validate_ecam() )
  {
    ...
    */
    ...
    mc_msgr_send_msg( &ho_fail_rsp.hdr,
                      ONEX_CP_LTE_1xCSFB_HO_FAILED_RSP,
                      sizeof(ho_fail_rsp));
    mcc_enqueue_dbg_lte_rrc_cnf( MCC_CSFB_PROCESS_TUNNELED_ECAM,
                                 MC_ONEX_CP_LTE_1xCSFB_HO_FAILED_RSP );
    /* Mark that UE is waiting for csfb_call_clear_f from CM */
    csfb_cdma.waiting_for_call_clear = TRUE;    
          ...
          )
        {
          ...
          if(mcc_is_mo_eutra_rxed == TRUE)
          {
            ...
            { /* CSFB Mode */
              ...
              mcc_enqueue_dbg_lte_rrc_cnf( MCC_CSFB_PROCESS_ORD_MSG,
                                           MC_ONEX_CP_LTE_1xCSFB_HO_FAILED_RSP );
              /* Mark that UE is waiting for csfb_call_clear_f from CM */
              csfb_cdma.waiting_for_call_clear = TRUE;              
            } /* CSFB Mode */
  ...
  switch(mcc_gcsna_ext_msg_ptr->gen_msg_id.msg_id)
  {
    ...
    case GCSNA_1X_CRKT_SRV_MSG:
    {
      ...
      */
      ...
      /* If MS received an MT call page on GCSNA_CHN_PC while it is performing
       * power-down registration, ignore the MT page and go ahead with the 
	...
	{
	  M1X_MSG( DCP, LEGACY_HIGH,
            "DEBUG: PAGE_MSG rxed during Power Down Regn, ignoring it");
	  break;
        }
        ...
        case GCSNA_CHN_PC:
        {
          ...
          switch(mcc_gcsna_ext_msg_ptr->ckt_srv_msg.msg_type)
          {
            case CAI_GENERAL_PAGE_MSG:
            {
              ...
              /* Ignore MT voice call if UI is not ready. This check is 
               * implemented to support power up time optimization,
               * (FR 26707). As a part of this optimization, it can happen
               * that UI many not be ready but modem will be up. Then we
               * need to reject the MT voice call received. */
              if(cm_get_ui_ready_status() == FALSE)
              {
                M1X_MSG( DCP, LEGACY_HIGH,
                 "DEBUG: MT page received while UI is not ready, ignoring it");
                return;
              }

              /* 1X should ignore MT pages if VoIWLAN voice call is active */
              ...
              {                 
                M1X_MSG( DCP, LEGACY_HIGH, 
                  "Ignoring CSFB MT page, VoIWLAN call is in progress" );
                return;
              }
              ...
              if(csfb_cdma.call_status == CSFB_CALL_PENDING_NONE)
              {
                ...
                if ((cmd_ptr = cm_mc_rpt_get_buf_else_err_fatal()) != NULL)
                {
                  ...
                  cmd_ptr->csfb_mt_call.vfr = mcc_get_vfr();
...
)
{
  ...
  if( ( MAIN_STATE(cdma.curr_state) != CDMA_CSFB ) &&  
      ( 
        ...
        !(
           ...
           ( msgr_cmd_ptr->msg.hdr.id == ONEX_CP_LTE_ABORT_1XCSFB_HO_REQ )
         )
      )
    )
  {
    M1X_MSG( DCP, LEGACY_ERROR,
             "DEBUG:Ignoring unexpected msgr cmd:%d, cdma_entry = %d",
             msgr_cmd_ptr->msg.hdr.id,
             cdma_entry.entry  );
    mcc_enqueue_dbg_msgr_cmd( msgr_cmd_ptr, old_state, cdma.curr_state,
                              TRUE, MC_CSFB_PROC_MSGR_RX_1);
    return;
  }
...
#endif /* FEATURE_MODEM_1X_IRAT_LTO1X_TUNNELED_SMS */
              {
                ...
                { /* CSFB Mode */
                  ...
                  mc_msgr_send_msg( &ho_fail_rsp.hdr,
                                    ONEX_CP_LTE_1xCSFB_HO_FAILED_RSP,
                                    sizeof(ho_fail_rsp) );
                  mcc_enqueue_dbg_lte_rrc_cnf( MC_CSFB_PROC_MSGR_RX,
                                               MC_ONEX_CP_LTE_1xCSFB_HO_FAILED_RSP );
                  /* Mark that UE is waiting for csfb_call_clear_f from CM */
                  csfb_cdma.waiting_for_call_clear = TRUE;                     
                } /* CSFB Mode */
 ...
 /* If CONTEXT loss has happened, while sending out release order for call end
	...
	mcc_csfb_cntx_loss_ind = MCC_CSFB_CNTX_LOSS_IMSI_ATTACH;
      ...
      /* If NAS_EMM_ATTACH_COMPLETE_IND is received while MO/MT call is in 
       * progress, make sure we don't lose context of the call.
       * Instead just mark that we need to perform forced powerup reg later */
      else if (cdma.curr_state == CDMA_CSFB_ACCESS &&
               (mcc_csfb_access_reason == MCC_CSFB_ACCESS_PRM ||
                mcc_csfb_access_reason == MCC_CSFB_ACCESS_ORIG)
              )
      {
        M1X_MSG( DCP, LEGACY_MED,
          "DEBUG: Attach complete during MO/MT call, marked context loss");        
        mcc_csfb_cntx_loss_ind = MCC_CSFB_CNTX_LOSS_IMSI_ATTACH;
      }
      else
      {
...
void mcc_csfb_exit_reason_processing(void)
{
  ...
  /* Check if we reached here by performing power_down registration */
      ...
      else if (mcc_csfb_exit.exit_reason == MCC_CSFB_EXIT_RSN_BLIND_REDIR_FAIL)
      {
        M1X_MSG( DCP, LEGACY_HIGH,
          "DEBUG: Go to CSFB IDLE after blind Redir Failure to native 1X");
        /* Get back to Idle state */
        cdma.curr_state = CDMA_CSFB_IDLE;        
      }
      break;
    ...
    if(mcc_gcsna_last_ul_msg_seq == mcc_gcsna_ext_msg_ptr->gcsna_ack.ack_seq)
    {
               ...
               mcc_csfb_exit.exit_reason == MCC_CSFB_EXIT_RSN_LTE_ABORT_HO)
      {
        ...
        mc_msgr_send_msg(&lte_abort_1xcsfb_ho_rsp.hdr,
                         ONEX_CP_LTE_ABORT_1xCSFB_HO_RSP,
                         sizeof(onex_cp_lte_abort_1xcsfb_ho_rsp_msg_t));      
        mcc_enqueue_dbg_lte_rrc_cnf( MCC_CSFB_PROCESS_GCSNA_ACK,
                                     MC_ONEX_CP_LTE_ABORT_1xCSFB_HO_RSP );
      }
      else
      {
...
#endif /* FEATURE_MODEM_1X_IRAT_LTO1X_TUNNELED_SMS */
      {
        ...
        { /* CSFB Mode */
          ...
          mc_msgr_send_msg(&ho_fail_rsp.hdr,
                           ONEX_CP_LTE_1xCSFB_HO_FAILED_RSP,
                           sizeof(ho_fail_rsp));
          mcc_enqueue_dbg_lte_rrc_cnf( MC_CSFB_PROCESS_GCSNA_REJ,
                                       MC_ONEX_CP_LTE_1xCSFB_HO_FAILED_RSP );
          /* Mark that UE is waiting for csfb_call_clear_f from CM */
          csfb_cdma.waiting_for_call_clear = TRUE;          
        } /* CSFB Mode */
...
#endif /* FEATURE_MODEM_1X_IRAT_LTO1X_TUNNELED_SMS */
      {
        ...
        { /* CSFB Mode */
          ...
          mc_msgr_send_msg(&ho_fail_rsp.hdr,
                           ONEX_CP_LTE_1xCSFB_HO_FAILED_RSP,
                           sizeof(ho_fail_rsp));
          mcc_enqueue_dbg_lte_rrc_cnf( MC_CSFB_PROCESS_GCSNA_REJ_1,
                                       MC_ONEX_CP_LTE_1xCSFB_HO_FAILED_RSP );
          /* Mark that UE is waiting for csfb_call_clear_f from CM */
          csfb_cdma.waiting_for_call_clear = TRUE;
        } /* CSFB Mode */
...
)
{
  ...
  switch(msgr_cmd_ptr->msg.emm_failure_ind_type.cause_type)
  {
    ...
    {
      /* if the failure cause is LTE_NAS_IRAT_INVALID_STATE, then the msg 
       * whichever 1X was trying to send will not be sent at all even after
       * all the retrials as LTE is either inactive/not registered/...
       * deregistered/in deregister initiated state(due to detach) and NAS
       * receives 1X UL message. Treat it as a hard failure only if the 
       * access attempt is for power down registration. For other cases, the
       * retrial mechanism will be continuing. */
      ...
      {
        M1X_MSG( DCP, LEGACY_HIGH,
            "NAS returned failure_cause %d. Treating as Hard failure",
            msgr_cmd_ptr->msg.emm_failure_ind_type.cause_type );
        gcsna_msg_sent.num_remaining_txns = 0;
        gcsna_msg_sent.num_remaining_retries = 0;
        csfb_cdma.reg_retry_cnt = 0;
...
boolean mcc_csfb_send_handover_complete_ind(void)
{
   ...
   */
  sd_ss_ind_cdma_opr_ho_lte( mcc_asid, SD_SS_HO_LTE_TO_CDMA, 
                             handover_param_ptr, &act );
  ...
  {
    sd_ss_ind_cdma_acq_schm( mcc_asid,
    ...
    sd_ss_ind_cdma_opr_oh_info( mcc_asid,
                                handover_param_ptr->band,
    ...
    mmoc_cmd_prot_handover_ind( mcc_asid,
