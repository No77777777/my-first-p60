...
$Header: //components/rel/1x.mpss/8.0/cp/src/mcctcsup.c#2 $ $DateTime: 2020/01/27 00:26:21 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ------------------------------------------------------------
01/22/20   vaa     Implement QSH MDUMP framework
06/15/18   ppr     CFS_Ready req's while prev CFS is in pending state
...
11/07/17   ppr     Don't send TA_CNTL_F to SRCH to block TA if 1x is on DDS 
                   Sub with Advanced data call (F-FCH RC=11)
...
04/27/16   ppr     1X2GTA:Chgs to rej LTA for SO-33 RC-11 1x Adv data calls
04/21/16   ppr     1X2GTA:Chgs to exec R/F-SCH0 start txn with ESCAM msg if req
04/12/16   ppr     WTR_HOP:Chgs to buf CFS Req msg if CFS with WTR Hop in prog
04/07/16   ppr     1X2GTA:Chgs to ignore TA request if MC is waiting for 
                   CFS “FING_DONE_R” rpt to re-enable the MUX
...
03/02/16   sjo     Trigger TX_ON_OFF trans on receipt of SRCH_FING_DONE_R
03/08/16   ppr     1X2GTA:Chgs to reset CFS rel params properly while 
                   UHDM processing in progress
...
)
{
  ...
  {
    ONEX_ERR_FATAL("Unknown Privacy Preference", 0, 0, 0);
  }
  ...
  else
  {
    ...
    switch (msg_ptr->gen_tc.msg_type)
    {
      ...
      default:
        ...
        ONEX_ERR_FATAL( "Bad msg %d ", msg_ptr->gen_tc.msg_type, 0, 0 );
...
)
{
  ...
  */
  ...
  if (mcctc_ppsm_setpt_incl)
  {
    ...
    {
       ONEX_ERR_FATAL("Setpoint request to RXC returned failure code.",0,0,0);
    }
    else
    {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch (cmd_ptr->tc_msg.msg.gen.msg_type)
  {
    ...
    {
      M1X_MSG( DCP, LEGACY_HIGH,
            "1xTA:Rxed ack for SCCM, send TA CNTL to SRCH, 1X_on_non_dds = %d",
            cdma.is_1x_on_non_dds_sub );
      /* Send Tune Away allowed indication to SRCH if
       * 1. SO-32 (TDSO) or 
       *    SO 6/14 && 1X in non DDS sub 
       * 2. TA_CTRL_F not sent to SRCH yet and
       */
      if( (srchtc_is_ta_allowed() == FALSE) &&
           ( ( ( cdma.is_1x_on_non_dds_sub == TRUE ) &&
...
)
{
  ...
  boolean cfs_wtr_hop_in_prog;
  /* Temporary variable for storing value of CFS with WTR hop in progress */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch (rpt_ptr->hdr.rpt)
  {
    ...
    case RXC_TX_ON_R:
    {
      ...
      /* Need to store CFS WTR Hop in progress value in local variable since
       * the global variable will be reset in post process API */
      cfs_wtr_hop_in_prog = mcc_is_cfs_with_wtr_hopping_in_prog();
      ...
      /* If we are here while executing RETURN transaction for CFS with 
       * WTR Hopping, we have already executed the tx_on_off trans on
       * receipt of the SRCH_FING_DONE_R. Hence no need to execute here.*/
      if (!cfs_wtr_hop_in_prog && !txc_is_curr_state_exit())
      {
      ...
      else if( mcc_temp_cfs_req_msg_ptr != NULL )
      {
        M1X_MSG( DCP, LEGACY_HIGH,
                 "Processing buffered CFS Req msg" );
        mcctcho_process_cand_freq_srch_req_msg(mcc_temp_cfs_req_msg_ptr);
...
{
  rxtx_cmd_type *cmd_ptr;
  wtr_hop_blocked_wait_type wait_result;
  ...
  switch (rpt_ptr->hdr.rpt)
  {
    ...
    case SRCH_RIF_HHO_ABORT_R:
    {
      ...
      switch( rpt_ptr->hho_abort.hho_fail_reason )
      {
        ...
        default:
        {
          ONEX_ERR_FATAL( "Bad hho failure reason %d ",
    ...
    case SRCH_RIF_HHO_FAILURE_R:
    {
      ...
      wait_result = WTR_HOP_RPT_RECEIVED;
      
      if (mcc_wtr_hopping.rif_in_progress)
      {               
        wait_result = mcctcho_srch_wtr_hop_blocked_wait();
        if (wait_result == WTR_HOP_SYS_RESTART)
        {
          new_tc_exit = MCC_SRCH_REQUEST_SYS_RESTART;
        }
        else if (wait_result == WTR_HOP_RPT_RECEIVED)       
        {        
          mcctcho_config_txc_for_cfs_or_rif_with_wtr_hopping();
        }           
      } 
      /* For WTR Hopping RIF HHO, in case of timeout waiting for WTR_HOP_R
      * during blocked wait don't try to tune to serving frequency as it 
      * would lead to UE crash. Instead remain on target frequency and that 
      * will lead to call drop at worst */
      if (wait_result == WTR_HOP_RPT_RECEIVED)    
      {
    ...
    case SRCH_CFS_READY_R:
    {
      ...
      /* Update CFS state only if no previous CFS is in pending state */
      if( cdma.tc_data_ta.cfs_prog == CFS_NONE )
      {
    ...
    case SRCH_FING_DONE_R:
    {
      ...
      /* For CFS with WTR hopping, need to re-enable TX on getting this rpt */
      if (mcc_is_cfs_with_wtr_hopping_in_prog() && 
          !mcc_is_hho_with_wtr_hopping_in_prog() &&
          (mcc_get_cfs_return_trans_despatched() == TRUE) &&
          txc_is_curr_state_exit())
      {        
        M1X_MSG( DCP, LEGACY_HIGH,
          "CFS with WTR HOP in progress, start TX re-enable procedures" );
  
        /* Return transaction is completed.*/
        mcc_set_cfs_return_trans_despatched(FALSE);
  
        /* Configure TXC for CFS with WTR Hopping after CFS is done */
        mcctcho_config_txc_for_cfs_or_rif_with_wtr_hopping();    
                       
        /* Trigger tx_on_off transaction here. Otherwise even if TX is on,
         * SRCH will have no information of the TX being turned ON. This 
         * will result in failure in sending the Extended Pilot Strength
         * Measurement Reprot to BS. */
        mcctxns_begin_tx_on_off_trans( TRUE);
    ...
    case SRCH_SYS_MEAS_R:
    {
      ...
      ** ------------------------------------------------------------- */
      (void) sd_ss_ind_misc_meas_rprt ( mcc_asid,
                                        rpt_ptr->sys_meas.meas_id,
                                        FALSE, NULL);
      break;
    }
    ...
    {
      M1X_MSG( DCP, LEGACY_HIGH,
               "WTR_HOP:Rcvd SRCH_WTR_HOP_R, status %d", 
               rpt_ptr->wtr_hop_rpt.status );
      /* Check if Hopping was succesful at SRCH */
      if (rpt_ptr->wtr_hop_rpt.status == TRUE)
      {  
        /* Check if HHO with WTR Hopping is in progress or not */
        if(mcc_is_hho_with_wtr_hopping_in_prog() == TRUE)
        {
          M1X_MSG( DCP, LEGACY_HIGH,
            "WTR_HOP:Rcvd WTR Hopping Done Rpt from SRCH" );

          /* Then configure TXC for HHO with WTR Hopping Scenario */
          mcctcho_config_txc_for_hho_with_wtr_hopping();    
  
          /* Then proceed with legacy Hard Handoff execution */
          if( mcc_wtr_uhdm_msg_ptr != NULL )
          {
            mcctcho_process_generic_handoff_msg(mcc_wtr_uhdm_msg_ptr);
          }
          else
          {
    ...
    case SRCH_TUNE_AWAY_REQ_R:
    {
      ...
      {
        ONEX_ERR_FATAL("1xTA: Already in TA GAP", 0, 0, 0);
      }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  while ( (rpt_ptr = (mctrans_rpt_type*) q_get( &mctrans_rpt_q )) != NULL )
  {
    ...
    switch ( rpt_ptr->trans_type )
    {
      ...
      {
        /* We have already logged the event in the API
         * "do_successful_handoff_operations()" */
       M1X_MSG( DCP, LEGACY_MED,
   	   "Rxed trans done rpt for MCTRANS_HHO_BEGIN." );
      ...
      case MCTRANS_RETURN:
      {
        ...
        /* 1xTA: Check for CFS Progress */
        if(cdma.tc_data_ta.cfs_prog == CFS_IN_PROG)
        {
          /* Check we already got the SRCH_FING_DONE_R rpt from SRCH to 
           * re-enable the MUX or not for CFS with WTR Hop req case */
          if( mcc_is_cfs_with_wtr_hopping_in_prog() &&
              txc_is_curr_state_exit()
            )
          {
            /* MUX Txc is in exit state means we might not got 
             * SRCH_FING_DONE_R rpt from SRCH so don't reset the cfs_prog flag
             * because if we reset the cfs_prog flag we might honor TA request
             * and SRCH will TA state and will not send SRCH_FING_DONE_R rpt,
             * which will leave Txc in exit state and lead to stability issue
             * in MUX while processing TA cmds.
             * Anyway we will reset that flag once we re-enable the MUX while
             * SRCH_FING_DONE_R rpt */
            M1X_MSG( DCP, LEGACY_MED,
   	             "1xTA: CFS with WTR Hop, waiting for SRCH_FING_DONE_R rpt" );             
          }
          else
          {
      ...
      {
        /* Reset the CFS_NSET Trans in progress flag to allow 
         * processing future CFS req msgs */
        if(rpt_ptr->trans_type == MCTRANS_CFS_NSET)
        {
        ...
        else
        {
               ...
               i < restore_srv_cfg_info->cfg.neg_scr.num_con; i++)
          {
            ...
            if (mccccim_evt.create_cc.status != MCCCCIM_DONE)
            {
              /* Shouldn't be here */
              rej_parms.rej_code = CAI_REJ_CALL_ASSIGN_REJ;
              ONEX_ERR_FATAL("Failed to create CCI", 0, 0, 0);
            }
          } /* end for */
        ...
        else
        {
              ...
              i++)
          {
            if ((call_id[num_call_id++] = cm_call_id_allocate()) == CM_CALL_ID_INVALID)
            {
              /* Shouldn't be here */
              rej_parms.rej_code = CAI_REJ_CALL_ASSIGN_REJ;
              ONEX_ERR_FATAL("Failed to allocate call_id", 0, 0, 0);
            }
            else
            {
              mccccim_evt.create_cc.event_code = MCCCCIM_CREATE_CC;
...
)
{
  ...
  else
  {
    ...
    ONEX_ERR_FATAL("BAD CC_INFO", 0, 0, 0);
  }
      ...
      if (cc_info->conn[i].response_ind)
      {
        if (mcctctag_get_by_tag_id(cc_info->conn[i].tag, &tag_info))
        {
          ...
          if (mccccim_is_con_ref_exist(cc_info->conn[i].con_ref))
          {
            ...
            ONEX_ERR_FATAL("BAD CON_REF", 0, 0, 0);
          }
          else
          {
            /* Create CCI */
            ...
            if (mccccim_evt.create_cc.status != MCCCCIM_DONE)
            {
              ...
              ONEX_ERR_FATAL("Cannot create CCI", 0, 0, 0);
            }
          }
        }
        else /* Tag not found */
        {
          ...
          ONEX_ERR_FATAL("INVALID TAG", 0, 0, 0);
        }
      }
      else
      {
...
void tc_initiate_wtr_hopping (void)
{
  ...
  cdma.tc_data_ta.hho_prog = HHO_WTR_HOP_IN_PROG;

  M1X_MSG( DCP, LEGACY_MED,
       "WTR_Hop:Start WTR Hop proc: Sending TXC_EXIT_F, hho_prog:%d", 
           cdma.tc_data_ta.hho_prog );                  
  /* Send request to Transmit task to go to START mode */
  mcc_tx_buf.exit.hdr.command = TXC_EXIT_F;
  mcc_txc_cmd( &mcc_tx_buf );
  
  /* Once Tx task moved to START mode, indicate SRCH to tune
   * to new WTR device as part of HHO with WTR hopping */
  M1X_MSG( DCP, LEGACY_MED,
           "WTR_Hop: Sending SRCH_WTR_HOP_F" );             
...
{
  qword wtr_hop_current_time;   
  /* Temp variable to perform timing related calculations to decide 
   * if WTF_HOP_F needs to be deferred or not */    

  uint32 wtr_hop_timer_value;
  /* Timer value to defer WTR hoping procedures till action time arrival */

  boolean defer_wtr_hop = FALSE;
  /* Should WTR hopping be deferred till HHO action time */

  boolean hho_wtr_hop_timer_active = timer_is_active(&mcc_wtr_hop_buf_timer);
  /* HHO WTR hopping timer is active */
    
  /*-----------------------------------------------------------------------*/
   
  /* Get current time in 20 ms units */
  m1x_time_get_20ms_frame_offset_time( wtr_hop_current_time );

  M1X_MSG( DCP, LEGACY_MED,
     "hho_wtr_hop_timer_active:%d, curr_time:0x%x,0x%x, trans_time:0x%x,0x%x",
           hho_wtr_hop_timer_active,
           qw_hi(wtr_hop_current_time), 
           qw_lo(wtr_hop_current_time), 
           qw_hi(mcctcho_ho_data_ptr->abs_action_time), 
           qw_lo(mcctcho_ho_data_ptr->abs_action_time) );
               
  /* If current time < HHO action time, need to check further */
  if ( !hho_wtr_hop_timer_active &&
       (qw_cmp(wtr_hop_current_time, mcctcho_ho_data_ptr->abs_action_time) < 0)
     )
  {
    /* Calculate difference b/w curr time and HHO trans scheduled time */
    qw_sub( wtr_hop_current_time, mcctcho_ho_data_ptr->abs_action_time,
            wtr_hop_current_time );

    if ( ( qw_hi( wtr_hop_current_time ) == 0 ) &&
         ( qw_lo( wtr_hop_current_time ) > WTR_HOP_SRCH_TX_BUFFER_TIME ) )
    {
      /* 1xTA:This API is called multiple times for TA case
       * so to be safe side, clear the timer first */
      if ( mcc_timer_on_callback_q(MCC_WTR_HOP_BUF_TIMER_ID) )
      {
        (void) mcc_clr_callback_timer ( &mcc_wtr_hop_buf_timer, 
                                        MCC_WTR_HOP_BUF_TIMER_ID );
      }
    
      wtr_hop_timer_value = (qw_lo(wtr_hop_current_time) - WTR_HOP_SRCH_TX_BUFFER_TIME) * 20;
      /* Start timer to ensure WTR HOP procedure starts 
       * WTR_HOP_SRCH_TX_BUFFER_TIME before the HHO action time */
      (void) mcc_set_callback_timer( &mcc_wtr_hop_buf_timer,
                                      wtr_hop_timer_value,
                                      MCC_WTR_HOP_BUF_TIMER_ID );  
      defer_wtr_hop = TRUE;
      M1X_MSG( DCP, LEGACY_HIGH,
       "WTR_Hop: Start timer:%d ms, defer Hopping till HHO action time", 
       wtr_hop_timer_value );              
      
    } /* if ( ( qw_hi( wtr_hop_current_time ) == 0 ) && */
    else if (qw_hi( wtr_hop_current_time ) != 0)
    {
      M1X_MSG( DCP, LEGACY_ERROR,
       "WTR_Hop: Invalid time diff calculated, 0x%x, 0x%x", 
       qw_hi( wtr_hop_current_time ), qw_lo( wtr_hop_current_time ) ); 

    } /*  else if (qw_hi( wtr_hop_current_time ) != 0) */     
  } /*  if ( hho_wtr_hop_timer_active && curr_time<HHO action time) */
  
  if(hho_wtr_hop_timer_active)
  { 
    /* Defer WTR procedures till wtr hop timer expiry */
    defer_wtr_hop = TRUE;
  }
  else
  {
    /* WTR Hop timer is not active so clear the timer to avoid processing 
     * wtr hop timer expirey again in tc_tc/tc_rel */
    if ( mcc_timer_on_callback_q(MCC_WTR_HOP_BUF_TIMER_ID) )
    {
      (void) mcc_clr_callback_timer ( &mcc_wtr_hop_buf_timer, 
                                      MCC_WTR_HOP_BUF_TIMER_ID );
      M1X_MSG( DCP, LEGACY_MED,
               "1xTA:Cleared mcc_wtr_hop_buf_timer" );      
    }
  }
  

  if (!defer_wtr_hop &&
      /* 1xTA: Check if we need to defer UHDM processing */
      mcctcsup_ta_is_uhdm_msg_deferrred() == FALSE
     )
  {
    tc_initiate_wtr_hopping();
  }
...
{
  txc_cmd_type *tx_cmd_ptr;
  size_t copy_size;
  
  if ( ( tx_cmd_ptr = ( txc_cmd_type* ) modem_mem_alloc
                                ( sizeof( txc_cmd_type ), MODEM_MEM_CLIENT_1X ) ) != NULL)
  {
    M1X_MSG( DCP, LEGACY_HIGH,
     "WTR_Hop: TX is inactive, deferred sending TX cmd %d,", 
     tx_buf->hdr.command);
        
    copy_size = memscpy( tx_cmd_ptr, sizeof(txc_cmd_type),
                             tx_buf, sizeof(txc_cmd_type));
    if( copy_size != sizeof(txc_cmd_type) )
    {
      M1X_MSG( DCP, LEGACY_ERROR,
        "Truncated memscpy, bytes_copied:%d, src and dest_size:%d",
        copy_size,
        sizeof(txc_cmd_type));
    }         
    q_put( &mc_deferred_tx_cmd_q, &tx_cmd_ptr->hdr.cmd_hdr.link ); /* and queue it */
  }
  else
  {
    M1X_MSG( DCP, LEGACY_ERROR,
             "WTR_Hop: Unable to allocate memory for queuing deferred TX cmd");
  }                               
}/* tc_wtr_hop_defer_tx_cmd */
...
void tc_wtr_hop_send_deferred_tx_cmds (void)
{
  ...
  while((tx_cmd_ptr = (txc_cmd_type*) q_get( &mc_deferred_tx_cmd_q)) != NULL)
  {
    copy_size = memscpy( &mcc_tx_buf, sizeof(txc_cmd_type),
                             tx_cmd_ptr, sizeof(txc_cmd_type));
    if( copy_size != sizeof(txc_cmd_type) )
    {
      M1X_MSG( DCP, LEGACY_ERROR,
        "Truncated memscpy, bytes_copied:%d, src and dest_size:%d",
    ...
    M1X_MSG( DCP, LEGACY_HIGH,
         "WTR_Hop: Sending deferred TX cmd %d", 
         mcc_tx_buf.hdr.command);
    
    mcc_txc_cmd(&mcc_tx_buf);  
    
    modem_mem_free( tx_cmd_ptr, MODEM_MEM_CLIENT_1X );
  }
...
} /* tc_wtr_hop_clr_deferred_tx_cmds */
...
)
{
  ...
  for (i = 0; i < chn_assign->mode.am0.num_pilots && i < SRCH_ASET_MAX; i++)
  {
         ...
         !mcc_srch_buf.tc.aset[i].fch_info.active))
    {
      ...
      ONEX_ERR_FATAL("No forward channels",0,0,0);
    }
        ...
        i < restore_srv_cfg_info->cfg.neg_scr.num_con; i++)
    {
      ...
      else
      {
        ...
        if ((call_id_ptr[*num_call_id_ptr] = cm_call_id_allocate()) == CM_CALL_ID_INVALID)
        {
          ...
          ONEX_ERR_FATAL("No Call_ID", 0, 0, 0);
        }
        else
        {
...
)
{
  ...
  M1X_MSG( DCP, LEGACY_HIGH,
           "1xTA:Rxed TA_CNTL_F, curr_state:0x%x, SO:%d, "
           "Srch_ta_sup:%d, Srch_ta_allowed:%d, F-FCH RC:%d",
           cdma.curr_state,
           cdma.so_req,
           srchtc_is_ta_supported(),
           srchtc_is_ta_allowed(),
...
{
  /* Clear TA complete buffer timer */
  (void) mcc_clr_callback_timer( &mcc_ta_comp_buf_timer,
...
)
{
  ...
  /* Chech if UHDM msg buffered and need to process */
  mcctcsup_check_and_proc_buf_uhdm_msg();

  /* We need to process any pending CFS rpt only after 100-ms of TA Complete.
   * We need to start the timer only if any pending CFS to be processed */
  if( mcctcho_ho_data_ptr->cfs_in_progress &&
      cdma.tc_data_ta.cfs_prog == CFS_READY_RPT_BUFFERED
    )
  {
    /* Start TA complet buffer Timer */
    (void) mcc_set_callback_timer( &mcc_ta_comp_buf_timer,
                                   (uint32) ONEX2GTA_TA_COMP_BUF_TIME,
                                   MCC_TA_COMP_BUF_TIMER_ID );
    M1X_MSG( DCP, LEGACY_HIGH,
             "1xTA:Started TA comp buf timer:%d-msec",
             ONEX2GTA_TA_COMP_BUF_TIME ); 
  }
      ...
      if ((srch_ptr = (mccsrch_rpt_type*) q_get( &mcc_srch_q )) != NULL)
      {
        ...
        if(srch_ptr->hdr.rpt == SRCH_REQUEST_SYS_RESTART_R)
        {
          ...
          switch (cdma.curr_state)
          {
            ...
            default:
            {
              ONEX_ERR_FATAL( "1xTA:Bad Traffic State %d", cdma.curr_state , 0, 0 );
            }
          } /* end of switch */
...
boolean tc_check_if_data_ta_allowed (void)
{
 ...
 /*--------------------------------------------------------------------*/
   ...
   *  1. SO 32 call with Test mode disabled.
   *  2. If SO is that of SMS (6/14) and the call was not in a non DDS sub.
   *  3. Call in progress is not a data call OR
   *     Data call with Forward FCH RC 11.
   *  4. If any Active Transaction execution is in progress
   *  5. Ignore LTA in Release Sub-state
   *  6. HHO is in progress and
   *     HHO defer timer got expired to force HHO execution OR
   *     HHO exec timer active & not expired
   *  7. SHO defer timer got expired to force HHO execution
   *  8. If CFS Single search in progress OR
   *     If MC already returned TRUE to SRCH for honoring
   *     CFS_READY_R
   *  9. If CFS defer timer expired
   * 10. If WTR HOP/RIF is in progress for HHO and also if CFS in progress
   * 11. If TA reg time already past by some threshold time
   */

  /* Test SO check */
  if( cdma.so_req == CAI_SO_FULL_TDSO
      #ifdef FEATURE_1X_TUNEAWAY_TEST
      && !srchtc_ta_test_mode_enabled()
      #endif /* FEATURE_1X_TUNEAWAY_TEST */
    )
  {
    M1X_MSG( DCP, LEGACY_HIGH,
     "1xTA: Rej TA Req, SO 32 not supported" );
    ta_allowed = FALSE;
  }
  /* Check for SO and RC to enable/disable TA .
   *
   * 1. If the SO is a non data one, then allow TA only if SO is
   *    CAI_SO_FULL_TDSO or CAI_SO_SMS (6/14) in non dds sub.
   * 2. If RC used is RC11, then also reject TA.
   */
  else if( ( ( !CAI_IS_SO_DATA(cdma.so_req) ) &&
             ( cdma.so_req != CAI_SO_FULL_TDSO ) &&
             ( !CAI_IS_SO_SMS(cdma.so_req) ||
               !cdma.is_1x_on_non_dds_sub
             )
  ...
  {
    M1X_MSG( DCP, LEGACY_HIGH,
     "1xTA: Rej TA Req, ta_type:%d, SO:%d, F-FCH-RC:%d, R-FCH-RC:%d, 1X_on_dds = %d",
             cdma.tc_data_ta.ta_type,
             cdma.so_req,
             current_cfg->for_fch_rc,
  ...
  /* WTR HOP procedures in progress */
  else if( (cdma.tc_data_ta.hho_prog == HHO_WTR_HOP_IN_PROG) ||
...
void mcctcsup_proc_ta_hho_defer_timer_expiry(void)
{
  ...
  if( mcctcho_is_hho_in_progress() )
  {
    ...
    {
      /* Check if UHDM msg processing got deferred before TA start */
      if(cdma.tc_data_ta.hho_prog == HHO_MSG_BUFFERED)
      {
        mcctcsup_check_and_proc_buf_uhdm_msg();
      }
      else
...
{
  boolean defer_status = FALSE;
    /* To keep track if UHDM msg to be buffered  */  
  ...
  M1X_MSG( DCP, LEGACY_MED,
    "1xTA:ta_rem_time:%d, hho_defer_timer_expired:%d, lta_req_pend:%d",
          ta_rem_time, 
          cdma.tc_data_ta.is_hho_defer_timer_expired,
          lta_req_pend );
          
  /* Do not defer UHDM processing if
   * 1. TA is not allowed OR
   * 2. HHO defer timer got expired
   */
  if( !srchtc_is_ta_allowed() ||
      cdma.tc_data_ta.is_hho_defer_timer_expired
    )
  {
    /* Do not defer UHDM processing */
    return FALSE;
  }
  
  /* Defer UHDM msg for the below cases
   * 1. Not enough time before next TA to process UHDM msg OR
   *    Non TA:mctrans_get_tuneaway_remaining_time() will return 0xFFFFFFFF
   * 2. OR if LTA Req processing pending in mcc_srch_q
   */         
  if( ( ta_rem_time != ONEX2GTA_TUNEAWAY_NOT_REGISTERED &&
        ta_rem_time < ONEX2GTA_HHO_MIN_REQ_TIME
      ) ||
      (lta_req_pend == TRUE)
    )
  {
    M1X_MSG( DCP, LEGACY_HIGH,
             "1xTA:Defering UHDM msg processing");
    defer_status = TRUE;
  }
  
  return defer_status;
} /* mcctcsup_ta_is_uhdm_msg_deferrred() */
...
{
  boolean hho_wtr_hop_req = mcc_is_hho_with_wtr_hopping_in_prog();
  
  /*-----------------------------------------------------------------------*/
  
/* We will be calling this API for TA case for HHO with WTR hop req only
 * and other UHDM scenario will be taken by txns logic */

  M1X_MSG( DCP, LEGACY_MED,
           "1xTA:hho_prog state:%d, wtr_hop_req:%d", 
            cdma.tc_data_ta.hho_prog, hho_wtr_hop_req ); 
  ...
  {
    /* WTR hopping may need to be deferred till HHO action time */
    tc_start_or_defer_wtr_hopping();  
  } /* (hho_prog) */
} /* mcctcsup_check_and_proc_buf_uhdm_msg() */
...
void mcctcsup_proc_ta_comp_buf_timer_expiry(void)
{
  ...
  M1X_MSG( DCP, LEGACY_HIGH,
           "1xTA:TA buf_timer_exp, curr_state:0x%x, cfs_in_prog:%d, "
           "cfs_wtr_hop:%d, cfs_prog:%d",
           cdma.curr_state, 
           mcctcho_ho_data_ptr->cfs_in_progress,
           mcctcho_ho_data_ptr->cfs_wtr_hop_in_prog,
           cdma.tc_data_ta.cfs_prog );   
           
  /* Check if CFS rpt processing got deferred before TA start */
  mcctcsup_check_and_proc_buf_cfs_rpt();                               
} /* mcctcsup_proc_ta_comp_buf_timer_expiry() */
...
void mcctcsup_check_and_proc_buf_cfs_rpt(void)
{
    ...
    )
  {
    ...
    {
      /* Ignore CFS rpt if 
       * 1. It is periodic conditional CFS rpt OR
       * 2. Hard handoff in progress */
...
boolean mcctcsup_ta_is_cfs_rpt_deferrred(void)
{
  ...
  M1X_MSG( DCP, LEGACY_MED,
           "1xTA:CFS srch_type:%d, SRCH_TA_Allowed:%d, def_timer_exp:%d, "
           "cfs_prog:%d, srch_cfs_proc_status:%d",
           cand_freq_srch_parm.cur_srch_type,
           srchtc_is_ta_allowed(),
           cdma.tc_data_ta.is_cfs_defer_timer_expired,
           cdma.tc_data_ta.cfs_prog,
  ...
  {    
    if(mcc_is_cfs_with_wtr_hopping_in_prog())
    {
      ta_cfs_proc_time = ONEX2GTA_CFS_WTR_MIN_REQ_TIME;
    }
    else
    {
    ...
    /* Reject CFS Search request if 
     * 1. We don't have enough time to proc CFS rpt based on WTR hop req OR
     * 2. LTA TA Request is Pending
     */
    if( ( ta_rem_time != ONEX2GTA_TUNEAWAY_NOT_REGISTERED &&
      ...
      )
    {
      ...
      M1X_MSG( DCP, LEGACY_ERROR,
               "1xTA:Defering CFS rpt processing, cfs_prog:%d, "
               "ta_cfs_proc_time:%d, ta_rem_time:%d, lta_req_pend:%d",
               cdma.tc_data_ta.cfs_prog,
               ta_cfs_proc_time,
               ta_rem_time,
               lta_req_pend );
...
void mcctcsup_proc_ta_cfs_defer_timer_expiry(void)
{
  ...
  if( mcctcho_ho_data_ptr->cfs_in_progress )
  {
    ...
    {
      /* Check if UHDM msg processing got deferred before TA start */
      if(cdma.tc_data_ta.cfs_prog == CFS_READY_RPT_BUFFERED)
      {
        mcctcsup_check_and_proc_buf_cfs_rpt();
      }
      else
...
boolean mccsrch_check_if_cfs_rpt_honored ( void )
{
   ...
   * 1. Prev CFS is in Prgress OR
   * 2. If CFS Rpt to be deferred OR
   * 3. HHO is in progress
   */
  if( (cdma.tc_data_ta.cfs_prog != CFS_NONE) ||
      mcctcsup_ta_is_cfs_rpt_deferrred() ||
      mcctcho_ho_data_ptr->hard_handoff_in_progress
...
void mcctcsup_update_srch_cfs_proc_status(boolean status)
{
  ...
  M1X_MSG( DCP, LEGACY_MED,
            "1xTA:Updated srch_cfs_proc_status:%d, cfs_prog:%d",
...
void mcctcsup_cp_exit_processing (void)
{
  ...
  */
  ...
  /* Clear the deferred TX cmds queue */
  tc_wtr_hop_clr_deferred_tx_cmds();

  /* Free memory allocated for buffering UHDM for WTR hopping */
  if (mcc_wtr_uhdm_msg_ptr != NULL)
  {
  ...
  /* Free memory allocated for buffering UHDM for CFS with WTR hop progress */
  if (mcc_temp_uhdm_msg_ptr != NULL)
  {
    modem_mem_free( mcc_temp_uhdm_msg_ptr, MODEM_MEM_CLIENT_1X );
    mcc_temp_uhdm_msg_ptr = NULL;
  ...
  /* Free memory allocated for buffering CFS Req msg for CFS with WTR hop */
