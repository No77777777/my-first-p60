...
$Header: //components/rel/1x.mpss/8.0/cp/src/mccccim.c#2 $ $DateTime: 2020/01/27 00:26:21 $ $Author: pwbldsvc $
...
04/10/17   ppr     Chgs to forcefully send codec_info to MMODE immediately 
...
09/29/15   ppr     Chgs to honor "MC_END_F with CAI_REL_PWDN end rsn" while 
  ...
  ** ------------------------------------------------------- */
...
#endif /* FEATURE_1X_CP_MEM_OPT */
  {
    ...
    ** ------------------------------------------------------- */
    ...
    {
#ifdef FEATURE_1X_CP_MEM_OPT
      ONEX_ERR_FATAL("No free RXTX buffer and backup buffer", 0, 0, 0 );
#else /* !FEATURE_1X_CP_MEM_OPT */
      ONEX_ERR_FATAL( "No free buffers on rxtx_mc_free_q and backup buf",
            0, 0, 0 );
#endif /* FEATURE_1X_CP_MEM_OPT */
    }
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ((cmd_ptr = cm_mc_rpt_get_buf_else_err_fatal()) != NULL)
  {
    ...
    cmd_ptr->incoming_call.vfr = mcc_get_vfr();
  ...
  switch (msg_ptr->hdr.cmd)
  {
    case MC_END_F:
    {
      ...
      for (i = 0; i<msg_ptr->end.num_call_ids; i++)
      {
            ...
            != MCCCCIM_CALL_ID_NOT_FOUND)
        {
          ...
          else if(msg_ptr->end.end_reasons[i] == CAI_REL_PWDN)
          {
...
#endif /* FEATURE_MODEM_1X_SRVCC */
    {
        ...
        )
      {
        ...
        {
          if(msg_ptr->csfb_call_end.end_reason == CAI_REL_SRV_INACT)
          {
            mccccim_cc_info[cc_index].rel_del_reason = 
                                          MCCCCIM_USER_SRV_INACTIVE;
          }
          else if(msg_ptr->csfb_call_end.end_reason == CAI_REL_PWDN)
          {
            mccccim_cc_info[cc_index].rel_del_reason = MCCCCIM_LOW_BATTERY;
          }
          else
...
)
{
  ...
  {
    ONEX_ERR_FATAL("Internal error",0,0,0);
    return;
    /* return; */
  }
