...
/*===========================================================================
  ...
  $Header: //components/rel/1x.mpss/8.0/cp/src/mc.c#5 $ $DateTime: 2020/08/31 06:52:14 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ------------------------------------------------------------
08/31/20   pga     Skip Powerup timer only if 1x is on non dds sub
05/07/20   vaa     Remove F3s inside Mdump APIs
01/22/20   vaa     Implement QSH MDUMP framework
06/12/19   vaa     Fixed link error on Habanero
11/16/18   vaa     Changes to avoid powerup timer on VoLTE registration
02/22/18   sjo     Chgs to open MMGSDI sessions for both subs. 
02/14/18   sjo     Chgs to handle commands with wrong asubs_id .
01/30/18   sjo     Chgs to re-enable SO73 if 1X Advanced is set.
01/19/18   ppr     F3 Reduction changes
12/18/17   ppr     1x pwr-up optimization changes
07/05/17   ppr     API to get the CDMA release revision
...
08/29/16   sjo     Added LTE_ABORT_1XCSFB_HO_REQ to mc_dbg_msgr_q. 
...
06/21/16   ppr     Chgs to rem featurization part to do reg when 4G set to OFF
...
10/09/15   agh     Fixed a bug in enabling Ovhd caching feature by default
09/30/15   ppr     New EFS Item chgs for 1xSRLTE MT Page Acc HO feature
08/18/15   ppr     Chgs for not to wait for deact_done rpt from SRCH, if it
...
06/04/15   agh     Handle NAS_EMM_ATTACH_COMPLETE_IND in inactive state to do 
...
04/13/15   ppr     Chgs to do reg during next 1x prot_act when 4G set to OFF
...
/* Queue used as a temporary placeholder for cmds to be sent by MC to TX task 
 * while TX is in inactive state during WTR hopping. The cmds will be sent
 * to TX once it has been put back to traffic state */
q_type mc_deferred_tx_cmd_q;
...
LOCAL const efs_item_file_type mc_efs_item_file[MC_EFS_ITEM_FILE_MAX] =
{
    ...
    ,
    /* MC_EFS_1xSRLTE_MT_PAGE_ACC_HO_SUPPORTED - 73773 */
    {
      "/nv/item_files/cdma/1xcp/1xsrlte_mt_page_acc_ho_supported", 1
    }
    ,
    /* MC_EFS_IGNORE_MT_PAGE - 73870 */
    {
      "/nv/item_files/cdma/1xcp/ignore_mt_pages", 1
    }
    ,
    /* MC_EFS_CONFIGURE_SNIFFER_MODE - 73901 */
    {
      "/nv/item_files/cdma/1xcp/configure_sniffer_mode", 37
    }
    , 
    /* MC_EFS_1x_PWRUP_OPT_ENABLED - 74048 */
    {
      "/nv/item_files/cdma/1xcp/1x_pwrup_opt_enabled", 1
    }
    , 
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  (void) q_init(&mc_deferred_tx_cmd_q); 
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (mc_nv_cmd_buf.status != NV_DONE_S) 
  {
    M1X_MSG( DCP, LEGACY_ERROR,
             "Failed write item, Status:%d, NV:%d, cdma_subs_avail:%d",
             (word)mc_nv_cmd_buf.status,
             (int) item_code,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {                                        /* else fatal error  */
    M1X_MSG( DCP, LEGACY_ERROR,
             "Bad NV Read Status:%d, NV:%d, cdma_subs_avail:%d",
             (int) mc_nv_cmd_buf.status,
             (int) item_code,
             cdma.is_cdma_subs_avail );
  }
...
/*lint -save -e539 Indentation varies due to features defined */
   ...
   */
  mc_efs_get( MC_EFS_MMGSDI_FEATURES_STATUS_LIST, &mc_efs_data );
...
)
{
  ...
  #endif /* FEATURE_MODEM_1X_IRAT_LTO1X */
  {
    ...
    {
      ONEX_ERR_FATAL("Memory allocation failure!", 0, 0, 0);
    }
    else
    {
      /* ----------------------------------
      ...
      ** ---------------------------------- */
      (void) sd_ss_act_get_per_sub(ss_act_param_ptr, mcc_asid);
...
)
{
  ...
  {
    ONEX_ERR_FATAL("deact_transid_ptr is NULL", 0,0,0);
    /* return deact_reason; */
  }
    ...
    switch (cmd_ptr->hdr.cmd)
    {
      ...
      case MC_DEREGISTER_F:
      {
        ...
        if(cmd_ptr != NULL)
        {
          ...
          mcc_send_deregister_status(CM_DEREG_FAIL_ALREADY_DEACT);
        }
        else
        {
          M1X_MSG( DCP, LEGACY_ERROR,
    ...
    #endif /* FEATURE_MODEM_1X_IRAT_LTO1X */
    {
      ...
      switch (cdma_exit_info.exit)
      {
        ...
        case MCC_PPP_CLEANUP:
          ...
          deact_reason = mc_map_ss_action_to_deact_reason(MC_ONLINE_DIGITAL_STATE,
                                           sd_ss_act_get_per_sub(NULL, mcc_asid)
                                           );
          break;
        ...
        )
      {
        ...
        if(csfb_cdma.csfb_1x_transition == TRUE)
        {
           ...
           */
          M1X_MSG( DCP, LEGACY_MED,
           "CSFB DEBUG: sending MMOC AUTO DEACT IND, deact_rsn:%d, 1X_ASID:%d",
                   deact_reason, 
                   mcc_asid );
          mmoc_rpt_prot_auto_deactd_ind_subs( deact_reason,
          ...
          /* Mark that there is no need to wait for call_clear_f in CSFB mode
           * as call_clear has already been received in native 1x */
          if (csfb_cdma.call_cleared_in_native_1x)
          {
            mcc_csfb_exit.exit_reason = MCC_CSFB_EXIT_RSN_BLIND_REDIR_FAIL;
            csfb_cdma.call_cleared_in_native_1x = FALSE;
            M1X_MSG( DCP, LEGACY_MED,
             "CSFB DEBUG: Call cleared in native 1X..don't go to CSFB access");
          }
          else
          {
    ...
    if (new_sys_meas_req)
    {
      ...
      if (!mcc_sys_meas_cmd())
      {
        ...
        if ((deact_reason = mc_map_ss_action_to_deact_reason(MC_MEAS_DED_STATE,
            sd_ss_act_get_per_sub(NULL, mcc_asid))) == PROT_DEACT_MAX)
        {
...
FUNCTION MC_READ_EFS_ITEMS

DESCRIPTION
  This API will read all the required EFS Items.
  This API will be called either from
  1. mc_task_init() OR
  2. mcc_reload_bootup_nv_items()
...
===========================================================================*/
void mc_read_efs_items( void )
{
 
   M1X_MSG( DCP, LEGACY_MED,
            "Reading EFS Items on 1x_ASID:%d",
            mcc_asid );
  ...
  /* MC_EFS_SO73_COP0_SUPPORTED: Read the system capability 
   * for SO73 COP0 support */
  mccap_set_so73_op0_sup_cap();

  /* Read EFS for support for reg after page drop because of DDTM */
  mc_efs_get( MC_EFS_REG_AFTER_DDTM_PAGE_DROP_SUPPORTED, &mc_efs_data );
  ...
  mc_efs_get( MC_EFS_1XADVANCED_CAPABILITY_STATUS, &mc_efs_data );
  ...
  /* Re-enable SO73 here. This is to handle the following scenario.
   *
   * Consider 1X Advanced is disabled. While reading SO73 NV, if it is set to
   * FALSE, SO73 will get disabled. Later if 1X Advanced was enabled, then UE 
   * will not try to read SO73 NV. The expectation is that the SO73 should be
   * enabled by default. If the user has done a hotswap instead of hard reboot,
   * then the MC task re-initialization do not happen and the SO73 will not be
   * re-intialized. It will remain disabled. To handle such situation, SO73 
   * shall be re-enabled if 1X Advanced is enabled. */ 

  if( cdma.onexadv_status.status == MC_1XADVANCED_ENABLED )
  {
    (void)snm_enable_so( CAI_SO_VOICE_EVRC_NW );
    M1X_MSG( DCP, LEGACY_MED,
             "Enabled SO73. SNP_SO73_Enabled: %d",
  ...
  mc_efs_get( MC_EFS_1xCSFB_T_MAX_NO_C2K_PARAM, &mc_efs_data );
  ...
  mc_efs_get( MC_EFS_1xCSFB_T_MAX_NO_VOICE, &mc_efs_data );
  ...
  mc_efs_get( MC_EFS_1XCSFB_N_MAX_REG_RETRY_CNT, &mc_efs_data );
  ...
  mc_efs_get( MC_EFS_DISABLE_SO35_SO36, &mc_efs_data );
  ...
  mc_efs_get( MC_EFS_OTASP_MAX_UNLOCK_ATTEMPTS, &mc_efs_data );
  ...
  /* Read EFS for support for reg after page drop because of DDTM */
  mc_efs_get( MC_EFS_DISABLE_1X_TO_LTE_RESELECTION, &mc_efs_data );
  ...
  mc_efs_get( MC_EFS_CMAS_DUP_DET_SUPPORTED, &mc_efs_data );
  ...
  mc_efs_get( MC_EFS_DEREGISTER_TIMER_VAL, &mc_efs_data );
  ...
  mc_efs_get( MC_EFS_REJ_VOICE_PAGE_FOR_VOLTE, &mc_efs_data );
  ...
  mc_efs_get( MC_EFS_SPECIFIC_CALL_END_STATS_SUPPORTED, &mc_efs_data );
  ...
  mc_efs_get( MC_EFS_1xCSFB_CGI_FEATURE_SUPPORTED, &mc_efs_data );
  ...
  ** CSFB call is to be optimised */
  mc_efs_get( MC_EFS_1xCSFB_REDIR_CALL_SETUP_OPTIMISED, &mc_efs_data );
  ...
  mc_efs_get( MC_EFS_1XSRVCC_T_MAX_NO_UHDM, &mc_efs_data );
  ...
  mc_efs_get( MC_EFS_USE_DEF_VAL_FOR_PWRUP_NV_ERR, &mc_efs_data );
  ...
  mc_efs_get( MC_EFS_SPLIT_OVERHEAD_SUPPORTED, &mc_efs_data );
  ...
  mc_efs_get( MC_EFS_1X_OVHD_OPTIMIZATION, &mc_efs_data );
  ...
  mc_efs_get( MC_EFS_CUST_EVRC_VOICE_ENCRYPT_SO, &mc_efs_data );
  ...
  /* Add cust_voice_so to SO3 Alt SO List if required */
  mccso_update_cust_voice_so();

  M1X_MSG( DCP, LEGACY_MED,
           "DEBUG: Customer specific voice SO:%d, cdma.cust_so:%d",
  ...
  /* Read EFS if MT pages ignored to know the n/w Page Repetition Pattern */
  (void) mc_efs_get(MC_EFS_IGNORE_MT_PAGES, &mc_efs_data);
  cdma.ignore_mt_pages = mc_efs_data.ignore_mt_pages;
  M1X_MSG( DCP, LEGACY_MED,
           "DEBUG: Ignore MT pages: %d",
           cdma.ignore_mt_pages );

  /* Read EFS if device is configured in sniffer mode */
  (void) mc_efs_get(MC_EFS_CONFIGURE_SNIFFER_MODE, &mc_efs_data);
  cdma.config_sniff_mode.sniff_mode_enabled = mc_efs_data.config_sniff_mode.sniff_mode_enabled;
  cdma.config_sniff_mode.imsi_s_hi = mc_efs_data.config_sniff_mode.imsi_s_hi;
  cdma.config_sniff_mode.imsi_s_lo = mc_efs_data.config_sniff_mode.imsi_s_lo;

  M1X_MSG( DCP, LEGACY_MED,
           "SNIFF_DBG: Device in sniffer mode : %d, imsi_s_hi = 0x%x, imsi_s_lo = 0x%x",
           cdma.config_sniff_mode.sniff_mode_enabled,
           cdma.config_sniff_mode.imsi_s_hi,
           cdma.config_sniff_mode.imsi_s_lo );

  /* Read EFS if 1x Power-up optimization is enabled */
  (void) mc_efs_get(MC_EFS_1x_PWRUP_OPT_ENABLED, &mc_efs_data);
  cdma.pwrup_opt_enabled = mc_efs_data.pwrup_opt_enabled;
  M1X_MSG( DCP, LEGACY_MED,
           "DEBUG: Power-up Optimization Enabled:%d",
           cdma.pwrup_opt_enabled );


  /*************************************************/
  /********EFS ITEMS OWNED BY OTHER MODULES*********/
  /*************************************************/
  ...
  mc_efs_get( MC_EFS_JCDMA_SUPPORT, &mc_efs_data );
  cdma.jcdma_support.service = mc_efs_data.jcdma_support.service;
  M1X_MSG( DCP, LEGACY_MED,
           "jcdma_support:%d, 1x_ASID:%d",
  ...
  M1X_MSG( SRCH, LEGACY_MED,
           "mcc_csfb_sms_suported_over_s102:%d, 1x_ASID:%d",
           csfb_cdma.mcc_csfb_sms_suported_over_s102,
           mcc_asid );
  ...
  mc_efs_get( MC_C2K_RESEL_SPLMN_SUPPORT, &mc_efs_data );
  cdma.splmn_sup.c2k_resel_splmn_supp = mc_efs_data.c2k_resel_splmn_supp;
  M1X_MSG( DCP, LEGACY_MED,
           "c2k_resel_splmn_supp:%d, 1x_ASID:%d", 
           cdma.splmn_sup.c2k_resel_splmn_supp,
           mcc_asid );
...
void mc_init ( void )
{
  ...
  {
    ONEX_ERR_FATAL("nv_mc_init_fp failed to register a 1X-CP API",0,0,0);
  }
...
)
{
  ...
  if(bs_info == NULL)
  {
    ...
    {
      ONEX_ERR_FATAL("Memory allocation failed for bs_info[] array", 0, 0, 0);
      return;
    }
  }
  else
  ...
  if(mcctcho_ho_data_ptr == NULL)
  {
    ...
    {
      ONEX_ERR_FATAL("Memory allocation failed for mcctcho_ho_data_ptr", 0, 0, 0);
      return;
    }
  }
  else
  ...
  if(act_reason != PROT_ACT_FTM)
  {
    ...
    if( sync_id_info == NULL)
    {
      ...
      {
        ONEX_ERR_FATAL("Memory allocation failed for sync_id_info[] array", 0, 0, 0);
        return;
      }
    }
    else
    ...
    if(caii_gem_message_rec_ptr == NULL)
    {
      ...
      {
        ONEX_ERR_FATAL("Memory allocation failed for caii_gem_message_rec_ptr", 0, 0, 0);
        return;
      }
    }
    else
    ...
    if(restore_srv_cfg_info_ptr == NULL)
    {
      ...
      {
        ONEX_ERR_FATAL("Memory allocation failed for restore_srv_cfg_info_ptr", 0, 0, 0);
        return;
      }
    }
    else
    ...
    if(tdso_pre_sync_ptr == NULL)
    {
      ...
      {
        ONEX_ERR_FATAL("Memory allocation failed for tdso_pre_sync_ptr", 0, 0, 0);
        return;
      }
    }
    else
...
//      break;
    ...
    default:
      ONEX_ERR_FATAL("Unknown activate reason on CDMA protocol", 0, 0, 0 );
      break;
  ...
  {
    M1X_MSG( DCP, LEGACY_MED,
            "DEBUG: sending MMOC AUTO DEACT IND, deact_rsn:%d, 1X_ASID:%d",
            deact_reason, 
            mcc_asid);
    mmoc_rpt_prot_auto_deactd_ind_subs( deact_reason, 
                                        prot_state,
...
LOCAL void mc_csfb_prot_activate(void)
{
  ...
  if(mcc_csfb_exit.exit == MCC_CSFB_EXIT_CSFB_DEACT)
  {
    ...
    {
      ONEX_ERR_FATAL( "No buffers on cm_cmd_rpt_free_q", 0, 0, 0 );
    }
  }
  else
  {
  ...
  */
  switch ( ph_stat_ptr->chg_type )
  {
    ...
    case PROT_PH_STAT_CHG_SUBSC:
    {
      ...
      M1X_MSG( DCP, LEGACY_MED,
               "CDMA subs_avail:%d, Prot_subsc_chg:%d, Oprt_mode:%d, asid = %d",
               cdma.is_cdma_subs_avail,
      ...
      /* Update the global variable session_id according to the asid value */
      if( mcc_asid == SYS_MODEM_AS_ID_1 )
      {
        cdma.session_id = cdma.pri_sub_session_id;
      }
      else if( mcc_asid == SYS_MODEM_AS_ID_2 )
      {
        cdma.session_id = cdma.sec_sub_session_id;
      }

      M1X_MSG( DCP, LEGACY_MED,
               "cdma.session_id = %d, pri_sub_session_id = %d, sec_sub_session_id = %d ",
                cdma.session_id,
      ...
      */
      if (CHECK_PROTO(ph_stat_ptr->prot_subsc_chg, 
                      PROT_SUBSC_CHG_3GPP2
                     ))
      {
        M1X_MSG( DCP, LEGACY_MED,
          "Re-initialize boot-up NVs and EFS items for asub_id = %d",
          mcc_asid);
		
        /* Reload the NVs and EFS items read during initial boot-up */
  ...
  mmoc_rpt_ph_stat_chgd_cnf2( ph_stat_ptr->trans_id
                             ,mmoc_convert_sys_mode_asubsid_to_mmoc_mode(
                                   SYS_SYS_MODE_CDMA,
                                   mcc_asid )
                             #ifdef MMOC_API_PH_STAT_CNF_WITH_STATUS
                             ,req_status
      ...
      switch ( cmd_ptr->hdr.cmd )
      {
...
#ifdef FEATURE_MODEM_1X_SUPPORTS_RF
          /* inform RF driver the subscriber id. This is implemented
           * as a prt of FR:29153 */
          rfm_1x_set_subscriber_id(mcc_asid);
#endif /* FEATURE_MODEM_1X_SUPPORTS_RF */
        ...
        case MC_PROT_REDIR_CNF_CMD_F:
        {
          ...
          if ( mc_irat_switch == TRUE )
          {
            ...
            {
              mc_cdma_activate_protocol( mcc_asid,
                                         PROT_ACT_GWL_REDIR );
            ...
            else
            {
              ...
              if (mc_redir_req_msg.hdr.id == ONEX_CP_LTE_REDIR_REQ)
              {
                ...
                mcc_enqueue_dbg_lte_rrc_cnf( MC_PROCESS_CMD, 
              ...
              M1X_MSG( DCP, LEGACY_HIGH,
                       ...
                       "deact_rsn REDIR_ABORT, 1X_ASID:%d",
                       mcc_asid );
        ...
        case MC_MMGSDI_CLIENT_ID_REG_CNF_F:
        {
          ...
          /* mmgsdi_session_open_ext() is used to open 2 or more sessions 
             in a single shot. Here we pass the session mask. */
          uint32 session_type_mask = MMGSDI_1X_PROV_PRI_SESSION_MASK |
                                     MMGSDI_1X_PROV_SEC_SESSION_MASK ;

          if ( mmgsdi_session_open_ext( cdma.client_id,
                                        session_type_mask,
                                    #ifdef MMOC_API_PH_STAT_CNF_WITH_STATUS
                                    mc_mmgsdi_evt_cb,
                                    #else
    ...
    else if((mc_msgr_msg_ptr = (mc_msgr_rcv_msg_type*) q_check( &mc_msgr_rcv.msg_q )) != NULL)
    {
      ...
      switch ( mc_msgr_msg_ptr->msg.hdr.id )
      {
          ...
          else
          {
            ...
            if(mc_msgr_msg_ptr->msg.hdr.id == ONEX_CP_LTE_REDIR_REQ)
            {
              ...
              mc_msgr_send_msg( &mc_redir_failed_ind.hdr,
                                ONEX_CP_LTE_REDIR_FAILED_RSP,
                                sizeof(mc_redir_failed_ind) );
              mcc_enqueue_dbg_lte_rrc_cnf( MC_PROCESS_CMD_1, 
                                           MC_ONEX_CP_LTE_REDIR_FAILED_RSP );
        ...
        case ONEX_CP_LTE_ABORT_RESEL_REQ:
        {
          ...
          mcc_enqueue_dbg_msgr_cmd( mc_msgr_msg_ptr, old_state,
                                    cdma.curr_state, FALSE, MC_PROCESS_CMD_1 );
          break;
        }
        ...
        case ONEX_CP_LTE_ABORT_1XCSFB_HO_REQ:
        {
          ...
          mcc_enqueue_dbg_lte_rrc_cnf( MC_PROCESS_CMD, 
                                       MC_ONEX_CP_LTE_ABORT_1xCSFB_HO_RSP );
        ...
        {
          /* Received dom_sel_change_ind from MMODE and 
           * Check if the dom_sel_change_ind is for successful VoLTE registration 
           * where dom=IMS, call_type_mask has VOICE, system = LTE
           * Then perform power-up reg procedures such that MC will do
           * power-up reg during next 1x prot activation */          
          if( (mc_msgr_msg_ptr->msg.cm_dom_sel_chg_ind.data.domain == 
                                              SYS_CM_DOMAIN_SEL_DOMAIN_IMS) &&
              (mc_msgr_msg_ptr->msg.cm_dom_sel_chg_ind.data.call_type_mask & 
                                                  CM_CALL_TYPE_VOICE_MASK ) &&
              (mc_msgr_msg_ptr->msg.cm_dom_sel_chg_ind.data.system == 
                                                          SYS_SYS_MODE_LTE)
            )
          {
            /* Perform power-up reg procedures */
            mccreg_powerup_mode_online();
            /* Skip starting power up timer if VoLTE registration is successful */
            cdma.skip_powerup_timer = TRUE;
            M1X_MSG( DCP, LEGACY_HIGH,
                     "VoLTE Reg Success Ind from MMODE and skip_powerup_timer: %d, is_1x_on_non_dds_sub:%d, mmode_cap_sub:%d",
                     cdma.skip_powerup_timer,
                     cdma.is_1x_on_non_dds_sub,
                     sd_misc_is_sub_multimode_capable( mcc_asid ) );
          }          
...
mc_msg_type *mc_get_buf_else_err_fatal( void )
{
  ...
  if( mc_cmd_ptr == NULL )
  {
    ONEX_ERR_FATAL("Memory allocation failed for mc cmd",0,0,0);
    /* return NULL; */
  }
  else
  {
...
(
  ...
  mc_msg_type  *msg_ptr   
    /* Pointer to MC message buffer */
)
{
  cm_mc_sub_diff_action_e_type action_req;
    /* Local variable to hold the action retuned by CM API if the command
     * was received on wrong sub. */
     
  cm_mc_sub_diff_action_s_type cmd_info;
    /* Structure holding the command name and asubs_id information which will
     * be used to check the action required if a asub_is mismatch occurs */
  
  if(asubs_id == mcc_asid)
  {
  ...
  {
    /* If asubs_id passed is not matching with the mcc_asid, then 1X will
     * check with CM on how to handle the command. Actions to be taken 
     * based on the return value is as follows.
     * 1. MC_HANDLE_CMD     - Handle the command irrespective of the sub_id.
     * 2. MC_IGNORE_CMD     - Ignore the command. Neither process the cmd,
     *                        nor crash the device.
     * 3. MC_UNEXPECTED_CMD - Sub_id mismatch is unexpected. Crash the device.
     */
     
    cmd_info.cmd      = msg_ptr->hdr.cmd;
    cmd_info.asubs_id = mcc_asid; /* Pass the current 1x sub id */
    action_req = cm_mc_get_sub_diff_action( cmd_info );

    M1X_MSG( DCP, LEGACY_MED,
      "DEBUG: 1X_ASID %d mismatch with asub_id %d for cmd %d. Action_req = %d",
      mcc_asid,
      asubs_id,
      msg_ptr->hdr.cmd,
      action_req );
    ...
    switch(action_req)
    {
      case MC_HANDLE_CMD:
      {
        /* CM needs the cmd to be handled irrespective of the asubs_id value */
        switch (msg_ptr->hdr.cmd)
        {
          #ifdef FEATURE_DDTM_CNTL       /* Data Dedicated mode access prevention cmd */
          case MC_DDTM_CNTL_F:
      ...
      case MC_IGNORE_CMD:
      {
        /* CM is asking to ignore the command. Do not crash. CM may send some
         * commands on both the subs. The commands received on wrong sub shall
         * be ignored. Once it comes on right sub, it can be processed. The 
         * commands like MC_DEREGISTER_F is one of those. */
        M1X_MSG( DCP, LEGACY_MED,
                 "DEBUG: Ignoring the cmd %d rxed with asid = %d. 1X_ASID = %d",
                 msg_ptr->hdr.cmd,
                 asubs_id,
                 mcc_asid );

        /* Keep track of the MMOC cmd for debugging purpose */
        mcc_enqueue_dbg_mmoc_cmd( msg_ptr, cdma.curr_state, cdma.curr_state,
                                  TRUE, MC_CMD );
      ...
      case MC_UNEXPECTED_CMD:
      {
        /* The sub mismatch is not expected for this command. Hence UE shall
         * crash */
        ONEX_ERR_FATAL("mcc_asid %d mismatch with asubs_id %d rxed along with cmd = %d",
		mcc_asid, asubs_id, msg_ptr->hdr.cmd);
        break;
      }
      default:
      {
        ONEX_ERR_FATAL("Unexpected return value during 1X_ASID %d mismatch with asub_id %d.",
                  mcc_asid, asubs_id, 0);
      }
    }/* switch(action_req) */    
  }/* else */
  ...
  else
  {
    switch ( reason )
    {
      case MMOC_SANITY_ERR_DEACT_CMD_CNF:
        ...
        */
        ONEX_ERR_FATAL( "CDMA did not respond to  deactivate command", 0, 0, 0 );
        break;
      ...
      case MMOC_SANITY_ERR_PH_STAT_CMD_CNF:
        ...
        */
        ONEX_ERR_FATAL( "CDMA did not respond to phone status chgd command", 0, 0, 0 );
        break;
      ...
      case MMOC_SANITY_ERR_GENERIC_CMD_CNF:
        ...
        ** The CDMA protocol should respond within 15 seconds.
        */
        ONEX_ERR_FATAL( "CDMA did not respond to generic command", 0, 0, 0 );
        break;

      default:
       ONEX_ERR_FATAL( "Unknown reason", 0, 0, 0 );
       break;
...
)
{
  switch ( reason )
  {
    case MMOC_SANITY_ERR_DEACT_CMD_CNF:
      ...
      ** deactivating because of power down it is 30 seconds.
      */
      ONEX_ERR_FATAL( "AMPS did not respond to deactivate command",
                 0, 0, 0
                );
      break;
    ...
    case MMOC_SANITY_ERR_PH_STAT_CMD_CNF:
      ...
      */
      ONEX_ERR_FATAL( "AMPS did not respond to phone status chgd command",
                 0, 0, 0
                );
      break;
    ...
    case MMOC_SANITY_ERR_GENERIC_CMD_CNF:
      ...
      ** The AMPS protocol should respond within 15 seconds.
      */
      ONEX_ERR_FATAL( "AMPS did not respond to generic command",
                 0, 0, 0
                );
      break;
...
)
{
  ...
  onex_nv_efs_init( MC_EFS_CONF_FILE, 
                    (int) MC_OWNED_EFS_ITEM_FILE_MAX,
                    mc_efs_item_file, 
                    MCFG_FS_SUBID_0 );
...
SIDE EFFECTS
  onex_efs_write() is supposed to work efficiently to store
  small amount of data (FS_ITEM_MAX). This function will
  determine the type of file needed based on the length
  parameter, If the length is <= (FS_ITEM_MAX) bytes then this
  function will try to store this file efficiently on an
  item-file, but if the length exceeds (FS_ITEM_MAX) bytes then
  this function will store this data on a regular file and the
...
{
  int oflag = MCFG_FS_O_AUTODIR | MCFG_FS_O_RDWR | MCFG_FS_O_CREAT | MCFG_FS_O_TRUNC;
    /* special flags.
       MCFG_FS_O_AUTODIR : Auto create parent directories
       MCFG_FS_O_CREAT : Create the item if it does not exist
       MCFG_FS_O_RDWR : To specify that file will be used for reading and writing
  ...
  int ret_val;
    /* return value for onex_efs_write */
  ...
  {
    ONEX_ERR_FATAL("Invalid file (%d) to write",(int)efs_file, 0, 0);
  }

  /* It runs in MC task context */
  ret_val = onex_efs_write( mc_efs_item_file[efs_file].path,
                            data,
                            length,
                            oflag,
                            mode
...
{
  int ret_val;
    /* return value for onex_efs_read */
  ...
  if ( efs_file >= MC_EFS_ITEM_FILE_MAX )
  {
    ONEX_ERR_FATAL( "Invalid file (%d) to read", (int)efs_file, 0, 0 );
  }

  /* Pass specific MCFG SUB_ID if the EFS ITEMS OWNED by other modules 
   * MCFG_FS_SUBID_NONE = -1 <--> SYS_MODEM_AS_ID_NONE = -1
   * MCFG_FS_SUBID_0 = 0 <--> SYS_MODEM_AS_ID_1 = 0
   * MCFG_FS_SUBID_1 = 1 <--> SYS_MODEM_AS_ID_2 = 1 
   * MCFG_FS_SUBID_2 = 2 <--> SYS_MODEM_AS_ID_3 = 2
   */
   
  if(efs_file >= MC_OWNED_EFS_ITEM_FILE_MAX)
  {
    sys_modem_as_id_e_type onex_asid = mcc_get_asid();
      /* 1x ASID variable */
      
    mcfg_fs_sub_id_e_type sub_id = MCFG_FS_SUBID_0;
    ...
    /* EFS Items OWNED by other modules */
    /* It runs in MC task context */
    ret_val = onex_efs_read_per_sub( mc_efs_item_file[efs_file].path,
                                     data,
                                     length,
                                     sub_id
                                   );    
  ...
  {
    /* It runs in MC task context */
    ret_val = onex_efs_read( mc_efs_item_file[efs_file].path,
                             data,
                             length
                           );
  }
  ...
  if ( ret_val == FALSE )
  {
    ...
    defined( FEATURE_MODEM_1X_IRAT_LTO1X_TUNNELED_SMS_SO76 )
      ...
      case MC_EFS_IGNORE_MT_PAGES:
      {
        /* By default support for ignoring MT pages to know the 
         * n/w Page Repitition Pattern is disabled */
        data->ignore_mt_pages = 0;
        break;
      } /* MC_EFS_IGNORE_MT_PAGES */
        
      case MC_EFS_CONFIGURE_SNIFFER_MODE:
      { 
        /* By default the device is not configured in sniffer mode  */
        data->config_sniff_mode.sniff_mode_enabled = 0;
        break; 
      } /* MC_EFS_CONFIGURE_SNIFFER_MODE */

      case MC_EFS_1x_PWRUP_OPT_ENABLED:
      { 
        /* By default 1x Power-up optimization is disabled  */
        data->pwrup_opt_enabled = 0;
        break; 
      } /* MC_EFS_1x_PWRUP_OPT_ENABLED */
...
None.
===========================================================================*/
void mc_msgr_register_msgs( void )
{
  errno_enum_type  estatus;
  msgr_client_t*   mc_msgr_client_id_ptr = mc_get_msgr_client_id();
  ...
  M1X_MSG( DCP, LEGACY_HIGH,
    "CSFB DEBUG:Registering for msgr messages with 1x_ASID = %d",
    asid );

  estatus = msgr_register_variant( MSGR_ONEX_CP,
 ...
 /*=========================================================================
FUNCTION       MC_MSGR_DEREGISTER_MSGS
DESCRIPTION
This function de-registers a message that 1x is interested in
...
None.
===========================================================================*/
void mc_msgr_deregister_msgs( void )
{
  errno_enum_type  estatus;
  msgr_client_t*   mc_msgr_client_id_ptr = mc_get_msgr_client_id();
  sys_modem_as_id_e_type asid = mcc_get_asid();
  ...
  M1X_MSG( DCP, LEGACY_HIGH,
    "CSFB DEBUG:De-registering msgr messages with 1x_ASID = %d",
    asid );

  estatus = msgr_deregister_variant( MSGR_ONEX_CP,
                                     mc_msgr_client_id_ptr,
                                     ONEX_CP_LTE_REDIR_REQ,
                                     asid );
...
)
{
  ...
  msgr_init_hdr_variant(msg_hdr_ptr, MSGR_ONEX_CP, umid, asid );

  estatus = msgr_send( msg_hdr_ptr, size );

  ASSERT( estatus == E_SUCCESS );
  M1X_MSG( DCP, LEGACY_HIGH,
    "MC sent msg with umid=%d, 1x_ASID = %d",
    umid, asid );
  ...
  if( cdma_entry.entry == MCC_LTE_1X_REDIR )
  {
    if ( (deact_reason != PROT_DEACT_REDIR_ABORT) )
    {
      ...
      mc_msgr_send_msg( &mc_redir_failed_ind.hdr,
                        ONEX_CP_LTE_REDIR_FAILED_RSP,
                        sizeof(mc_redir_failed_ind) );
      mcc_enqueue_dbg_lte_rrc_cnf( MCC_CSFB_SEND_IRAT_FAIL_IND, 
                                   MC_ONEX_CP_LTE_REDIR_FAILED_RSP );
...
)
{
  ...
  switch ( queue_type )
  {
    ...
    default:
      ONEX_ERR_FATAL("Can't get here: Bad MC queue type %d", queue_type, 0, 0 );
      break;
  }
...
)
{
  if (ptr != NULL)
  {
    ...
    if (mc_queue_info[queue_type].allocated_buf_cnt > 0)
    {
      ...
      mc_queue_info[queue_type].allocated_buf_cnt--;
      M1X_MSG( DCP, LEGACY_MED,
        "[MC Mem opt] Freed 0x%x from queue %d",
        ptr,
        queue_type);
    }
    else
    {
      ONEX_ERR_FATAL("ptr: 0x%x. The queue %d is already empty", ptr, queue_type, 0);
    }
    /* Leave Critical Section */
    MC_LEAVE_CRIT_SECT();
  }
  ...
  {
    ONEX_ERR_FATAL("Pointer to free is NULL", 0, 0, 0);
  }
}/* mc_free_queue_buf */
...
uint32 mc_sec_ran_next(void)
{
  ...
  {
    ONEX_ERR_FATAL("Secure API failed due to %d",sec_ran_status,0,0);
  }
...
DESCRIPTION
  This function returns the status of CSFB call
...
FUNCTION MC_GET_CDMA_RELEASE_REVISION

DESCRIPTION
  This API returns release revision supported by CDMA.
  This is a public API and might be invoked from QMI to get 
  CDMA supported Release Revision.
...
RETURN VALUE
  ...
  4 - P_REV_IS95B
  6 - P_REV_IS2000
  7 - P_REV_IS2000_REL_A
  8 - P_REV_IS2000_REL_B
  9 - P_REV_IS2000_REL_C
  10 - P_REV_IS2000_REL_C_MI
  11 - P_REV_IS2000_REL_D

SIDE EFFECTS
  None.

===========================================================================*/
uint32 mc_get_cdma_release_revision
( 
  sys_modem_as_id_e_type asid 
)
{
  sys_modem_as_id_e_type cur_onex_asid = mcc_get_asid();
    /* 1x ASID variable */

  uint32 onex_rev = (uint32) P_REV_IS2000_REL_C;
    /* Max 1x Revision Supported */
  /*-----------------------------------------------------------------------*/

  if( asid != cur_onex_asid )
  {
    M1X_MSG( DCP, LEGACY_ERROR,
             "Queried with wrong 1x_ASID:%d, Current 1x_ASID:%d",
             asid,
             cur_onex_asid );
  }

  M1X_MSG( DCP, LEGACY_HIGH,
           "Returning 1X Rev:%d, Current 1x_ASID:%d, Passed 1x_ASID:%d",
           onex_rev,
           cur_onex_asid,
           asid );

  return onex_rev;
} /* mc_get_cdma_release_revision() */

#ifdef FEATURE_QSH_MDUMP
/*===========================================================================
FUNCTION mc_qsh_mdump_collect_cp_data

DESCRIPTION
This function dumps the 1x CP data to QSH after a crash
...
===========================================================================*/
QSH_MDUMP_FN_ATTR
void mc_qsh_mdump_collect_cp_data()
{
/*  M1X_MSG( DCP, LEGACY_HIGH,
           "DEBUG: Collecting CP minidump" ); */
  mc_qsh_mdump_collect_mccidl_data();
  mc_qsh_mdump_collect_mccsa_data();
  mc_qsh_mdump_collect_auth_data();
  mc_qsh_mdump_collect_snm_data();
  mc_qsh_mdump_collect_rxtx_data();

#ifdef FEATURE_MODEM_1X_IRAT_LTO1X
  qsh_mdump_collect_high(&mc_dbg_msgr_q,sizeof(mc_dbg_msgr_q));
#endif /* FEATURE_MODEM_1X_IRAT_LTO1X */
#ifdef FEATURE_CSIM
  qsh_mdump_collect_high(&mc_mmgsdi_q,sizeof(mc_mmgsdi_q));
#endif /* FEATURE_CSIM */

  qsh_mdump_collect_high(&cur_bs_ptr,sizeof(cur_bs_ptr));
  qsh_mdump_collect_high(&bs_info,sizeof(bs_info));
  qsh_mdump_collect_high(&mc_cmd_q,sizeof(mc_cmd_q));
  qsh_mdump_collect_high(&mc_dbg_cmd_q,sizeof(mc_dbg_cmd_q));
  qsh_mdump_collect_high(&cdma,sizeof(cdma));
  qsh_mdump_collect_high(&cdma_entry,sizeof(cdma_entry));
  qsh_mdump_collect_high(&mc_deferred_tx_cmd_q,sizeof(mc_deferred_tx_cmd_q));
  qsh_mdump_collect_high(&mcc_srch_q,sizeof(mcc_srch_q));
  qsh_mdump_collect_high(&mcc_rxtx_q,sizeof(mcc_rxtx_q));
  qsh_mdump_collect_high(&mcctcho_ho_data_ptr,sizeof(mcctcho_ho_data_ptr));
  qsh_mdump_collect_high(&mcc_mmgsdi_rpt_ptr,sizeof(mcc_mmgsdi_rpt_ptr));
  qsh_mdump_collect_high(&mcc_ds_sync_espm_prev_cb,sizeof(mcc_ds_sync_espm_prev_cb));
  qsh_mdump_collect_high(&mcc_temp_uhdm_msg_ptr,sizeof(mcc_temp_uhdm_msg_ptr));
  qsh_mdump_collect_high(&mcc_wtr_uhdm_msg_ptr,sizeof(mcc_wtr_uhdm_msg_ptr));
  qsh_mdump_collect_high(&mcc_temp_cfs_req_msg_ptr,sizeof(mcc_temp_cfs_req_msg_ptr));
  qsh_mdump_collect_high(&mc_tcb_ptr,sizeof(mc_tcb_ptr));
  qsh_mdump_collect_high(&mcc_exit,sizeof(mcc_exit));
  qsh_mdump_collect_high(&mcc_idle_hard_handoff,sizeof(mcc_idle_hard_handoff));
  qsh_mdump_collect_high(&mcc_is_mvs_acquired,sizeof(mcc_is_mvs_acquired));
  qsh_mdump_collect_high(&mcc_layer2,sizeof(mcc_layer2));
  qsh_mdump_collect_high(&mcc_mmgsdi_rpt_status,sizeof(mcc_mmgsdi_rpt_status));
  qsh_mdump_collect_high(&mcc_post_otasp_reg_pending,
                          sizeof(mcc_post_otasp_reg_pending));
  qsh_mdump_collect_high(&mccreg_vars,sizeof(mccreg_vars));
  qsh_mdump_collect_high(&reg_after_ddtm_page_drop_supported,
                          sizeof(reg_after_ddtm_page_drop_supported));
  qsh_mdump_collect_high(&mcc_old_bs_nghbr_info,sizeof(mcc_old_bs_nghbr_info));
  qsh_mdump_collect_high(&tc_exit,sizeof(tc_exit));
  qsh_mdump_collect_high(&mccsrch_dbg_buf,sizeof(mccsrch_dbg_buf));
  qsh_mdump_collect_high(&mmoc_cnf_dbg_buf,sizeof(mmoc_cnf_dbg_buf));
  qsh_mdump_collect_high(&lte_rrc_cnf_dbg_buf,sizeof(lte_rrc_cnf_dbg_buf));
  qsh_mdump_collect_high(&mc_queue_info,sizeof(mc_queue_info));
  qsh_mdump_collect_high(&rxtx_queue_info,sizeof(rxtx_queue_info));
  qsh_mdump_collect_high(&mcc_band_table,sizeof(mcc_band_table));
  qsh_mdump_collect_high(&mcc_is_config_chgd,sizeof(mcc_is_config_chgd));
  qsh_mdump_collect_high(&c2k_parms_msg,sizeof(c2k_parms_msg));
  qsh_mdump_collect_high(&mcc_srch_buf,sizeof(mcc_srch_buf));
  qsh_mdump_collect_high(&mcc_rx_buf,sizeof(mcc_rx_buf));
  qsh_mdump_collect_high(&mcc_tx_buf,sizeof(mcc_tx_buf));

/*  M1X_MSG( DCP, LEGACY_HIGH,
           "DEBUG: CP minidump collected successfully" ); */
}
