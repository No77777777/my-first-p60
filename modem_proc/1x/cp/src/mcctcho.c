...
$Header: //components/rel/1x.mpss/8.0/cp/src/mcctcho.c#2 $ $DateTime: 2020/01/27 00:26:21 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ------------------------------------------------------------
01/22/20   vaa     Implement QSH MDUMP framework
04/02/18   ppr     WTR_HOP: DCCH HHO fix
04/02/18   ppr     WTR_HOP: DCCH CFS fix
01/19/18   ppr     F3 Reduction changes
12/15/17   ppr     Fix for 100% FER issue with SO-2 RC1/1 loopback on IS-95 
11/17/17   ppr     Mainline GPP2_SUPPORT_ON_SEC_SUB feature
08/31/17   sjo     Chgs for CDMA_support on SUB2
07/22/16   sjo     Dont DTX frames while CFS is in progress.
07/08/16   ppr     QSH:Enable event diagnostic data to QSH
07/01/16   ppr     1x2GTA:Buffer UHDM msg if CFS without WTR Hop in prog
04/12/16   ppr     WTR_HOP:Chgs to buf CFS Req msg if CFS with WTR Hop in prog
03/28/16   sjo     Handle good frames Rxed not associated with HHO completion
...
01/18/16   ppr     WTR_HOP:Chgs to update HHO transactons action time with 
                   current time if transaction action time is alreay past.
11/03/15   agh     Fixed call drop and TX disable issues for inter band CFS
09/01/15   ppr     Chgs to delay HHO trans by 4-Frames if F_SCH_0 is active
08/07/15   ppr     WTR_HOP:Differ SCH assignment till Handoff is successful 
                   in case of HHO with FrameOffset change
07/31/15   agh     WTR Hopping changes for inter band CFS
07/28/15   ppr     WTR_HOP:Temp chgs to mark return_if_ho_fail to FALSE for 
                   HHO with WTR Hopping required scenarios
07/28/15   ppr     WTR HOP: Chgs to update SCR, NNSCR params to HHO ptr
                   if PREV_IN_USE < 6
07/27/15   sjo     Logged soft and hard handoff after receiving two good frames
07/17/15   ppr     WTR HOP: Fix for SCR included in UHDM case
07/13/15   ppr     HHO WTR Hopping Changes
06/16/15   jh      Chgs to check if the primary rf device supports new band
05/26/15   ppr     Chgs to delay HHO trans by 2-Frames if F_SCH_0 is active
...
void mcctcho_tc_init( void )
{
  ...
  mcctcho_ho_data_ptr->hho_wtr_hopping_req = FALSE;
  mcctcho_ho_data_ptr->cfs_wtr_hop_in_prog = FALSE;

  mcc_wtr_hopping.rif_in_progress = FALSE;
  mcc_wtr_hopping.tx_cmd_proc_done = FALSE;  

  mcctcho_ho_data_ptr->cfs_nset_trans_in_prog = FALSE;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  else
  {
    ...
    ** ----------------------------------------- */
    ...
    {
        ONEX_ERR_FATAL("Zero pilots not allowed in HOCM", 0, 0, 0);
    }
...
void stop_cf_srch (void)
{
  ...
  /* Don't reset CFS WTR HOP in progress flag here as we may need to perform
   * several cleanups after this point such as TXC re-enable */
...
)
{
  ...
  if ( num_pilots > 0 )
  {
    ...
    /* Additional time required for TX disable/enable procedures for 
     * WTR Hopping CFS needs to be added */
    ...
    {   
      if ( cfs_req->var.mod0.band_class == cdma.band_class &&
           cfs_req->var.mod0.cdma_freq  == cdma.cdmach )
      {
          M1X_MSG( DCP, LEGACY_ERROR,
           "SRCH returned WTR HOP without band/freq change, band %d, freq %d",
            cdma.band_class, cdma.cdmach );
      }
      else
      {        
        M1X_MSG( DCP, LEGACY_MED, 
                  "WTR_HOP: Updated Rev link off time for TX enable/disable" );
        off_times->max_off_time_rev += WTR_HOP_CFS_TX_EXTRA_OFF_TIME;
      }
    }
...
)
{
  ...
  rx_band_tune_type srch_bc_rtn_value;
    /* To track SRCH API return value (BC_CH Rej/Supoorted/WTR HOP req) */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  ** -------------------------------------------------------------------------- */
  ...
  {
    srch_bc_rtn_value = srch_rx_check_wtr_hopping(
                         (srch_rx_band_type)ho->extra.band_class,
                         (srch_rx_chan_type)ho->extra.cdma_freq, 
                         SRCH_RX_ONEX_MC );

    if( srch_bc_rtn_value == SRCH_RX_BAND_REJECT )
    {
      M1X_MSG( DCP, LEGACY_HIGH,
    ...
    else if ( srch_bc_rtn_value ==  SRCH_RX_BAND_HOP )
    {
      ...
      {
        M1X_MSG( DCP, LEGACY_HIGH,
          "WTR_HOP: WTR Hopping required for handoff, band %d, channel %d",
          ho->extra.band_class, ho->extra.cdma_freq );
  
        /* Mark that we need to perform WTR hopping */
        mcctcho_ho_data_ptr->hho_wtr_hopping_req = TRUE;
  
        /* Save band_class and freq in global struct to pass to SRCH later */
        mcc_wtr_hopping.band_class = ho->extra.band_class;
        mcc_wtr_hopping.cdma_freq  = ho->extra.cdma_freq;
  
        /* Start timer for TX cnf for transmitting UHDM L2 Ack */
        (void) mcc_set_callback_timer( &mcc_hdm_tx_ack_timer,
                                       HDM_L2_ACK_TX_CNF_TIMEOUT,
                                       MCC_HDM_TX_ACK_TIMER_ID );
      }     
    }    
  }/* ho->msg_type */
  ...
  ** -------------------------------------------------------------------------- */
  ...
  /* Validate nnscr:
   * Here, there is a need to evaluate the NNSCR even in scenarios 
   * when there may not be a SCR or NNSCR or both in a HO msg. 
   * Because a ch_ind change can cause scenarios that the MS can't
   * handle even if underlying SCR/NNSCR don't change. So, whenever
   * our PREV is >= 6, we'll evaluate the NNSCR (LPM fields within it
   * specifically) to find out if we should reject the msg.
   *  
   * Also, make sure to update ho msg ptr with proper SCR, NNSCR params 
   * for WTR Hopping Required case because we need to execture SCR action
   * on Tx as part of HO Transaction for WTR Hopping req scenario */
  if ( (P_REV_IS_6_OR_GREATER) ||
       (mcctcho_ho_data_ptr->hho_wtr_hopping_req == TRUE)
     )
  {
...
)
{
  ...
  ** ----------------------------------------------------------------------- */
  ...
  {
    M1X_MSG( DCP, LEGACY_HIGH,
             "Processing Hard Handoff, WTR Hop Req:%d, use_time:%d",
             mcctcho_ho_data_ptr->hho_wtr_hopping_req,
             ho->use_time );
  ...
  ** ----------------------------------------------------------------------- */
  ...
  /* For WTR HOP Required case don't mark pre_proc_done to TRUE because
   * we will update pre_proc_done to TRUE when we call
   * mcctcho_process_generic_handoff_msg() for second time, after we are
   * done with Tx and SRCH processing related to WTR Hopping */
  if (!mcctcho_ho_data_ptr->hho_wtr_hopping_req )
  {
...
)
{
  ...
  */
  ...
  /* Log both soft and hard handoffs here */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /* Call mcc_is_cfs_with_wtr_hopping_in_prog() to print
   * cfs_in_progress and cfs_wtr_hop_in_prog */
  ...
  {      
    ...
    if (mcc_temp_uhdm_msg_ptr == NULL)
    {
      ONEX_ERR_FATAL("Could not allocate memory for temp UHDM pointer", 0, 0, 0);
      return;
    }
  ...
  ** ----------------------------------------------------------------------- */
  ...
  M1X_MSG( DCP, LEGACY_MED,
           "Process HDM msg, WTR hop req:%d, hho_prog:%d, Pre proc done:%d",
           mcctcho_ho_data_ptr->hho_wtr_hopping_req,
           cdma.tc_data_ta.hho_prog,
           ho->pre_proc_done );

  /* Call mcctcho_default_handoff_msg_fields only if 
   * WTR HOP is not enabled OR TA case validating UHDM msg
   * Because for WTR Hopping case we will be entering
   * mcctcho_process_generic_handoff_msg() API twice
   * 1. First time to validate UHDM fields
   * 2. To dispacth HO transactions once Tx and SRCH rel processing done
   *    for WTR HOP required case
   * and when we enter for 2nd-time then WTR_HOP_Req is set to TRUE
   * to avoid calling this API Twice 
   *
   * 1xTA: We will call mcctcho_process_generic_handoff_msg() API twice
   * 1. First time to validate the UHDM fields and 
   *    to check if we need to defer processing UHDM msg
   * 2. Enough time to process buffered UHDM message before next TA */
  if ( !mcctcho_ho_data_ptr->hho_wtr_hopping_req &&
  ...
  if ( ! ho->pre_proc_done )
  {
    /* Validate UHDM only if WTR HOP is not enabled OR 
     * TA case validating UHDM msg when entered API first time
     * Because for WTR Hopping case we will be entering
     * mcctcho_process_generic_handoff_msg() API twice
     * 1. First time to validate UHDM fields
     * 2. To dispacth HO transactions once Tx and SRCH rel processing done
     *    for WTR HOP required case
     * and when we enter for 2nd-time then WTR_HOP_Req is set to TRUE
     * to avoid UHDM validation Twice 
     *
     * 1xTA: We will call mcctcho_process_generic_handoff_msg() API twice
     * 1. First time to validate the UHDM fields and 
     *    to check if we need to defer processing UHDM msg
       ...
       )
    {
      ...
      ** -------------------------------------------------------------- */
      if (ho->extra_parms)
      {
        ...
        #endif /* FEATURE_MODEM_1X_IRAT_LTO1X */
        {
          if (sd_ss_ind_cdma_opr_hdm_cdma( mcc_asid,
		  	       (sd_band_e_type)ho->extra.band_class,
      ...
      /* If WTR Hopping required OR 
       * 1xTA:Not enough time to process UHDM msg
       * then buffer the UHDM msg to use later.
       * We enter mcctcho_process_generic_handoff_msg() for 2nd-time
       *
       * To dispacth handoff related transactions, that is after Tx and SRCH
       * is done with WTR Hopping rel processing 
       */
      
      if ( mcctcho_ho_data_ptr->hho_wtr_hopping_req )
      {
        if (mcc_wtr_uhdm_msg_ptr != NULL)
        {
          modem_mem_free( mcc_wtr_uhdm_msg_ptr, MODEM_MEM_CLIENT_1X );
          mcc_wtr_uhdm_msg_ptr = NULL;
        }
        mcc_wtr_uhdm_msg_ptr = (mccrxtx_msg_type*) modem_mem_alloc(
                      sizeof(mccrxtx_msg_type), MODEM_MEM_CLIENT_1X );
        if (mcc_wtr_uhdm_msg_ptr == NULL)
        {
          ONEX_ERR_FATAL("Could not allocate memory for UHDM pointer", 0, 0, 0);
          return;
        }
        memset( mcc_wtr_uhdm_msg_ptr, 0, sizeof(mccrxtx_msg_type) );
        copy_size = memscpy( mcc_wtr_uhdm_msg_ptr, sizeof(mccrxtx_msg_type),
                             msg_ptr, sizeof(mccrxtx_msg_type) );
        if( copy_size != sizeof(mccrxtx_msg_type) )
        {
          M1X_MSG( DCP, LEGACY_ERROR,
        ...
        /* 1xTA: Update HHO progress state for TA allowed case only 
         * to differentiate HHO with WTR hop b/w legacy and TA flow.
         * If WTR hop not required then we will proceed with placing req HHO txns
         */
        if(srchtc_is_ta_allowed())
        {
          /* For TA allowed cases, don't wait for mcc_hdm_tx_ack_timer
           * expiry because it will zcc unnecessary delay in processing HHO.
           * So clear mcc_hdm_tx_ack_timer */
          (void) mcc_clr_callback_timer ( &mcc_hdm_tx_ack_timer, 
                                          MCC_HDM_TX_ACK_TIMER_ID ); 
                                          
          if(cdma.tc_data_ta.hho_prog == HHO_MSG_RCVD)
          {
            cdma.tc_data_ta.hho_prog = HHO_MSG_BUFFERED;
          }
          else
          {
            M1X_MSG( DCP, LEGACY_ERROR,
          ...
          /* check if WTR hopping may need to be deferred till 
           * HHO action time */
          tc_start_or_defer_wtr_hopping();        
        } /* TA Allowed */
             
        /* Set action_time_pending here. This helps in distinguishing if the     
         * good frames report was received due to completion of HHO     
         * procedures or not. It can happen that MUX reported bad frames earlier
         * and now it's seeing good frames. Then also MUX will send good frames
         * report. Otherwise a good frames report received while      
         * CP is waiting for the HHO deferral timer expiry or WTR hopping     
         * procedures to be completed will be mistaken as HHO completion. */ 

        action_time_pending = TRUE;
		       
        /* For WTR Hopping required case OR 1xTA: msg to be deferred, 
         * return from here to wait till
         * Tx and SRCH is done with WTR Hopping related processing and
         * we re-enter mcctcho_process_generic_handoff_msg() API for 2nd-time
         * to dispatch HO related transactions 
         * OR
         * 1xTA: We will call mcctcho_process_generic_handoff_msg() API 
         * second time when there is enough time before next TA
         * to process buffered UHDM message */
         
        return;
      }
    } /* if (!mcctcho_ho_data_ptr->hho_wtr_hopping_req ) */

    /* We will be here when we call mcctcho_process_generic_handoff_msg() API
     * for 2nd-time to dispacth Ho related transactions for
     * 1. WTR Hopping case OR
     * 2. 1xTA:UHDM msg proc got deferred and when there is enough time
     *    to execute HHO before next TA Gap
     *
     * Set pre_proc_done to TRUE so that HO transaction are dispatched as
     * per legacy implementation */
    if ( (mcctcho_ho_data_ptr->hho_wtr_hopping_req) ||
         (cdma.tc_data_ta.hho_prog == HHO_MSG_BUFFERED)
       )
    {
     ...
     * 1. HHO with WTR Hop case OR 
     * 2. HHO with 1x TA case */
    m1x_time_get_20ms_frame_offset_time(current_time);
    if(qw_cmp(mcctcho_ho_data_ptr->abs_action_time, current_time) <= 0)
    {
...
)
{
  ...
  */
  ...
  if (mcctcho_ho_data_ptr->hho_wtr_hopping_req)
  {
    mcc_wtr_hopping.band_class = cdma.band_class;
    mcc_wtr_hopping.cdma_freq = cdma.cdmach;
    mcc_wtr_hopping.rif_in_progress = TRUE;

    M1X_MSG( DCP, LEGACY_HIGH,
      "WTR_HOP:Failure for RIF HHO, "
      "return to serv band:%d, freq:%d, hho_prog:%d",
      cdma.band_class, cdma.cdmach, cdma.tc_data_ta.hho_prog );    

    M1X_MSG( DCP, LEGACY_MED,
         "WTR_HOP: Sending TXC_EXIT_F for RIF" );                  
...
)
{
  ...
  if ( mcctcho_ho_data_ptr->hard_handoff_in_progress )
  {
    ...
    (void) sd_ss_ind_cdma_opr_hdm_cdma ( mcc_asid,
...
)
{
  ...
  if ( ! cfs_req->fix.pre_proc_done )
  {
    ...
    /* Check if MCTRANS_CFS_NSET OR CFS with WTR Hopping required is in prog.
     * If yes then buffer the CFS Req msg to process it later after CFS is done
     * because if we process while CFS with WTR Hop in progress may to lead to
     * collission b/w two CFS with WTR Hop case and lead to crash 
     in lower layers */
     if( (mcctcho_ho_data_ptr->cfs_nset_trans_in_prog) ||
     ...
     { 
       ...
       /* Buffer the UHDM msg to process later 
        * after the CFS with WTR Hop is done */
       if (mcc_temp_cfs_req_msg_ptr != NULL)
       {
         modem_mem_free( mcc_temp_cfs_req_msg_ptr, MODEM_MEM_CLIENT_1X );
         mcc_temp_cfs_req_msg_ptr = NULL;
       }

       mcc_temp_cfs_req_msg_ptr = (caii_cfs_req_type*) modem_mem_alloc(
                     sizeof(caii_cfs_req_type), MODEM_MEM_CLIENT_1X );
       if (mcc_temp_cfs_req_msg_ptr == NULL)
       {
         ONEX_ERR_FATAL("Could not allocate memory for temp UHDM pointer", 0, 0, 0);
         return;
       }
       memset( mcc_temp_cfs_req_msg_ptr, 0, sizeof(caii_cfs_req_type) );
       copy_size = memscpy( mcc_temp_cfs_req_msg_ptr, sizeof(caii_cfs_req_type),
                            cfs_req, sizeof(caii_cfs_req_type) );
       if( copy_size != sizeof(caii_cfs_req_type) )
       {
         M1X_MSG( DCP, LEGACY_ERROR,
           "Truncated memscpy, bytes_copied:%d, src_size:%d, dest_size:%d",
           copy_size,
           sizeof(caii_cfs_req_type),
           sizeof(caii_cfs_req_type));
       }

       /* Send a Candidate Frequency Search Response Message */
       send_cand_freq_srch_resp_msg( cfs_req->fix.cfsrm_seq );
    
       M1X_MSG( DCP, LEGACY_HIGH,
         "WTR_HOP:Buffered CFS Req msg, cfs_nset_trans_in_prog:%d",
          mcctcho_ho_data_ptr->cfs_nset_trans_in_prog );

       return;      
     } /* CFS WTR HOP/NSET Txn in Prog */
...
{
  /* Do not honor SRCH_CFS_READY_R rpt 
   * if CFS with WTR Hopping required already in progress */
  if( mcc_is_cfs_with_wtr_hopping_in_prog() )
  {
    M1X_MSG( DCP, LEGACY_ERROR,
             "WTR_HOP:Ignoring CFS ready rpt as one already in progress" );
    return;
  }
...
/* At MUX level, DTX will be enabled if the CP variable 
  ...
  /* Check if WTR hopping is required for CFS measurements */
  if ( srch_rx_check_wtr_hopping(
       (srch_rx_band_type) cand_freq_srch_parm.cf_band_class,
       (srch_rx_chan_type) cand_freq_srch_parm.cf_cdma_freq, 
       SRCH_RX_ONEX_MC )
        == SRCH_RX_BAND_HOP
     )
  { 
    if ( cand_freq_srch_parm.cf_band_class == cdma.band_class &&
         cand_freq_srch_parm.cf_cdma_freq  == cdma.cdmach )
    {
        M1X_MSG( DCP, LEGACY_ERROR,
         "SRCH returned WTR HOP without band/freq change, band %d, freq %d",
    ...
    {  
      mcctcho_ho_data_ptr->cfs_wtr_hop_in_prog = TRUE;
      M1X_MSG( DCP, LEGACY_HIGH,
        "WTR_HOP: WTR Hopping required for CFS, band %d, channel %d",
        cand_freq_srch_parm.cf_band_class, cand_freq_srch_parm.cf_cdma_freq );   
    }      
  } /* if ( srch_rx_check_wtr_hopping */
...
===========================================================================*/
 void mcctcho_execute_cfs_meas(void)
 {
   ...
   boolean cfs_wtr_hop_req = mcc_is_cfs_with_wtr_hopping_in_prog();
   /* CFS: WTR Hopping required */
   /*--------------------------------------------------------------------*/

   M1X_MSG( DCP, LEGACY_MED,
     "CFS in Prog, srch_type:%d, Single_cfs_srch:%d, cfs_wtr_hop_req:%d, cfs_prog:%d",
            cand_freq_srch_parm.cur_srch_type,
            cdma.tc_data_ta.is_cfs_single_srch_in_prog,
            cfs_wtr_hop_req,
            cdma.tc_data_ta.cfs_prog );

   /* Check WTR Hop required for CFS measurements */
   if(cfs_wtr_hop_req)
   {
     /* 1xTA: Update CFS progress state */ 
     ...
     M1X_MSG( DCP, LEGACY_MED,
          "WTR_Hop: Start CFS WTR Hop proc's: Sending TXC_EXIT_F, cfs_prog:%d",
          cdma.tc_data_ta.cfs_prog );                  
     /* Send request to Transmit task to go to START mode */
     mcc_tx_buf.exit.hdr.command = TXC_EXIT_F;
     mcc_txc_cmd( &mcc_tx_buf );
...
FUNCTION MCC_IS_HHO_WITH_WTR_HOPPING_IN_PROG

DESCRIPTION
  This function checks if Hard Handoff with WTR hopping is in progress or not.

DEPENDENCIES
  None

RETURN VALUE
  TRUE if Hard Handoff with WTR hopping is in progress.

SIDE EFFECTS
  None

===========================================================================*/
boolean mcc_is_hho_with_wtr_hopping_in_prog(void)
{
  M1X_MSG( DCP, LEGACY_MED,
    "WTR_HOP: hard_handoff_in_progress:%d, hho_wtr_hopping_req:%d",
    mcctcho_ho_data_ptr->hard_handoff_in_progress,
    mcctcho_ho_data_ptr->hho_wtr_hopping_req );

  return mcctcho_ho_data_ptr->hho_wtr_hopping_req;
} /* mcc_is_hho_with_wtr_hopping_in_prog() */

/*===========================================================================

FUNCTION MCC_IS_CFS_WITH_WTR_HOPPING_IN_PROG

DESCRIPTION
  This function checks if CFS measurement with WTR hopping is in
  progress or not.
...
RETURN VALUE
  TRUE if CFS Measurement with WTR hopping is in progress.

SIDE EFFECTS
  None

===========================================================================*/
boolean mcc_is_cfs_with_wtr_hopping_in_prog(void)
{
  M1X_MSG( DCP, LEGACY_MED,
    "WTR_HOP: cfs_in_progress:%d, cfs_wtr_hopping_req:%d",
    mcctcho_ho_data_ptr->cfs_in_progress,
    mcctcho_ho_data_ptr->cfs_wtr_hop_in_prog );

  return mcctcho_ho_data_ptr->cfs_wtr_hop_in_prog ;
} /* mcc_is_cfs_with_wtr_hopping_in_prog() */
...
FUNCTION MCCTCHO_CREATE_ECAM_FROM_UHDM

DESCRIPTION
  This function creates an ECAM from the UHDM.
...
boolean mcctcho_create_ecam_from_uhdm 
(
  caii_ho_msg_type *ho, 
...
)
{
  ...
  M1X_MSG( DCP, LEGACY_HIGH,
           "WTR_HOP: Forming ECAM from UHDM,scr_inc:%d,nnscr_inc:%d,ch_ind:%d",
           ho->extra.scr_included,
           ho->extra.nnscr_included,
           ho->ch_ind );
  ...
  chn_assign->mode.am0.freq_incl = 1;
  chn_assign->mode.am0.default_config = CAI_DEF_CONFIG_USE_ASSGN_RC;
  chn_assign->mode.am0.num_pilots = MIN(SRCH_ASET_MAX, ho->num_pilots);
  chn_assign->mode.am0.granted_mode = CAI_GRANT_STR;
  chn_assign->mode.am0.cdma_freq = ho->extra.cdma_freq;
  chn_assign->mode.am0.band_class = ho->extra.band_class;
  chn_assign->mode.am0.frame_offset = ho->extra.frame_offset;
  chn_assign->mode.am0.encrypt_mode = ho->extra.encrypt_mode;
  
  /* Copy the pilot records */
  for (i = 0; i < chn_assign->mode.am0.num_pilots &&
      i < ARR_SIZE(chn_assign->mode.am0.pilot_rec); i++)
  {
    chn_assign->mode.am0.pilot_rec[i].add_pilot_rec_incl =
                                          ho->pilot[i].add_pilot_rec_incl;
    chn_assign->mode.am0.pilot_rec[i].pilot_rec_type =
      ho->pilot[i].pilot_rec_type;
    chn_assign->mode.am0.pilot_rec[i].add_pilot_rec =
      ho->pilot[i].add_pilot_rec;
    chn_assign->mode.am0.pilot_rec[i].pwr_comb_ind =
      ho->pilot[i].pwr_comb_ind;
    chn_assign->mode.am0.pilot_rec[i].code_chan_fch =
      ho->pilot[i].code_chan_fch;
    chn_assign->mode.am0.pilot_rec[i].qof_mask_id_fch =
      ho->pilot[i].qof_mask_id_fch;
    chn_assign->mode.am0.pilot_rec[i].code_chan_dcch =
      ho->pilot[i].code_chan_dcch;
    chn_assign->mode.am0.pilot_rec[i].qof_mask_id_dcch =
      ho->pilot[i].qof_mask_id_dcch;
    chn_assign->mode.am0.pilot_rec[i].pilot_pn =
      ho->pilot[i].pilot_pn;
  }
  
  if ( ho->extra.scr_included )
  {
    /* Check if DCCH included */
    if( ho->extra.cfg.dcch_cc_incl )
    {
      /* DCCH RC */
      chn_assign->mode.am0.for_rc = ho->extra.cfg.for_dcch_rc;
      chn_assign->mode.am0.rev_rc = ho->extra.cfg.rev_dcch_rc;
    }
    else
    {
      /* Default FCH RC */
      chn_assign->mode.am0.for_rc = ho->extra.cfg.for_fch_rc;
      chn_assign->mode.am0.rev_rc = ho->extra.cfg.rev_fch_rc;
    } /* FCH RC */
  
    M1X_MSG( DCP, LEGACY_HIGH,
             "WTR_HOP: FCH CC_Incl:%d,For_RC:%d,Rev_RC:%d, "
             "DCCH CC_Incl:%d,For_RC:%d,Rev_RC%d",
             ho->extra.cfg.fch_cc_incl,
             ho->extra.cfg.for_fch_rc,
             ho->extra.cfg.rev_fch_rc,
             ho->extra.cfg.dcch_cc_incl,
             ho->extra.cfg.for_dcch_rc,
             ho->extra.cfg.rev_dcch_rc );
  } /* if (ho->extra.scr_included) */

  /* We can have below ch_ind values (cai_i.h)
   * CAI_REFER_TO_EXT_CH_IND       = 0
   * RESERVED                      = 1,3,4
   * CAI_DED_CTL                   = 2 (DCCH)
   * CAI_FUND_CONT_REV_PLT         = 5 (FCH)
   * CAI_DED_CTL_CONT_REV_PLT      = 6 (DCCH)
   * CAI_FUND_DED_CTL_CONT_REV_PLT = 7 (FCH & DCCH) */
  if( ho->ch_ind )
  {
    chn_assign->mode.am0.ch_ind = ho->ch_ind;
  }
  else
  {
    /* Default make it FCH-only */
    chn_assign->mode.am0.ch_ind = CAI_CHIND_FCH_ONLY;
  }

  chn_assign->mode.am0.fpc_subchan_gain = ho->fpc_subchan_gain;
  /* The Standards specifies that rl_gain_adj should be 0. */
  chn_assign->mode.am0.rl_gain_adj = 0;
  
  M1X_MSG( DCP, LEGACY_HIGH,
           "WTR_HOP: Using FCH init_setpt:%d,DCCH init_setpt:%d,nnscr_inc:%d,"
           "fpc_incl:%d,fpc_olpc_fch_incl:%d,fpc_olpc_dcch_incl:%d",
           chn_assign->mode.am0.fpc_fch_init_setpt,
           chn_assign->mode.am0.fpc_dcch_init_setpt,
           ho->extra.nnscr_included,
           ho->extra.non_neg_cfg.fpc_incl,
           ho->extra.non_neg_cfg.fpc_olpc_fch_incl,
           ho->extra.non_neg_cfg.fpc_olpc_dcch_incl );
  
  if ( (ho->extra.nnscr_included) &&
       (ho->extra.non_neg_cfg.fpc_incl)
     )
  {
    /* FCH FPC Fields */
    if( ho->extra.non_neg_cfg.fpc_olpc_fch_incl )
    {
      M1X_MSG( DCP, LEGACY_HIGH,
               "WTR_HOP: FCH FPC Fer:%d,Min_setpt:%d,Max_setpt:%d",
               ho->extra.non_neg_cfg.fpc_fch_fer,
               ho->extra.non_neg_cfg.fpc_fch_min_setpt,
               ho->extra.non_neg_cfg.fpc_fch_max_setpt );
      chn_assign->mode.am0.fpc_fch_fer = ho->extra.non_neg_cfg.fpc_fch_fer;
    ...
    /* DCCH FPC Fields */
    if( ho->extra.non_neg_cfg.fpc_olpc_dcch_incl )
    {
      M1X_MSG( DCP, LEGACY_HIGH,
               "WTR_HOP: DCCH FPC Fer:%d,Min_setpt:%d,Max_setpt:%d",
               ho->extra.non_neg_cfg.fpc_dcch_fer,
               ho->extra.non_neg_cfg.fpc_dcch_min_setpt,
               ho->extra.non_neg_cfg.fpc_dcch_max_setpt );
      chn_assign->mode.am0.fpc_dcch_fer = ho->extra.non_neg_cfg.fpc_dcch_fer;
      chn_assign->mode.am0.fpc_dcch_min_setpt = 
                                    ho->extra.non_neg_cfg.fpc_dcch_min_setpt;
      chn_assign->mode.am0.fpc_dcch_max_setpt = 
                                    ho->extra.non_neg_cfg.fpc_dcch_max_setpt;
    } /* (fpc_olpc_dcch_incl) */
  } /* (nnscr_included && fpc_incl) */

  /* Primary channel is FCH */
  chn_assign->mode.am0.fpc_pri_chan = 0;
  chn_assign->mode.am0.rev_fch_gating_on = ho->rev_fch_gating_mode;
  chn_assign->mode.am0.rev_pwr_cntl_delay_incl = ho->rev_pwr_cntl_delay_incl;
  chn_assign->mode.am0.rev_pwr_cntl_delay = ho->rev_pwr_cntl_delay;


  chn_assign->mode.am0.plcm_type_incl = ho->plcm_type_incl;
  if (chn_assign->mode.am0.plcm_type_incl)
  {
    chn_assign->mode.am0.plcm_type = ho->plcm_type;
    if (chn_assign->mode.am0.plcm_type == CAI_PLCM_TYPE_BS_SPEC)
    {
      qw_set(chn_assign->mode.am0.plcm_39,
             qw_hi(ho->plcm_39),
             qw_lo(ho->plcm_39));
    }
  }
  ...
  if (mcc_check_if_traf_chan_assgn_is_okay(chn_assign) != 0)
  {
    M1X_MSG( DCP, LEGACY_ERROR,
  ...
  M1X_MSG( DCP, LEGACY_HIGH,
    "WTR_HOP: ECAM created successfully from UHDM ");

  return(TRUE);
} /* mcctcho_create_ecam_from_uhdm() */

/*===========================================================================

FUNCTION MCCTCHO_UPDATE_UHDM_FOR_WTR_HOPPING

DESCRIPTION
  This function updates saved HO message with SCR related fields.
...
===========================================================================*/
void mcctcho_update_uhdm_for_wtr_hopping( void )
{
   if (mcc_wtr_uhdm_msg_ptr != NULL)
   {
     /* mcctcho_ho_data_ptr->ho_msg.extra.cfg and
      * mcctcho_ho_data_ptr->ho_msg.extra.non_neg_cfg are guaranteed
      * to be updated (validate_handoff_msg) with the right scr/nnscr 
      * irrespective of whether one is included in the message or not. 
      *
      * For WTR Hopping req for Hard Handoff case, Set scr_included
      * and nnscr_included to make sure we execute MCTRANS_SCR_ACTION
      * on Tx as part HO Transaction to configure Tx with correct SCR
      * (Service Configuration Record) params after we do TXC_EXIT_F
      * as part of WTR Hopping req for HHO case */
     if (!mcc_wtr_uhdm_msg_ptr->msg.ho.extra.scr_included)
     {
       mcctcho_ho_data_ptr->ho_msg.extra.scr_included = TRUE;
       mcctcho_ho_data_ptr->ho_msg.extra.serv_con_seq = mcc_wtr_hop_get_srv_con_seq();

       mcc_wtr_uhdm_msg_ptr->msg.ho.extra.scr_included = TRUE;
       mcc_wtr_uhdm_msg_ptr->msg.ho.extra.serv_con_seq = mcc_wtr_hop_get_srv_con_seq();
       M1X_MSG( DCP, LEGACY_MED,
             "WTR_HOP:Updated UHDM with scr_included to TRUE, con seq %d",
              mcc_wtr_uhdm_msg_ptr->msg.ho.extra.serv_con_seq);
     }
     if (!mcc_wtr_uhdm_msg_ptr->msg.ho.extra.nnscr_included)
     {
       mcctcho_ho_data_ptr->ho_msg.extra.nnscr_included = TRUE;

       mcc_wtr_uhdm_msg_ptr->msg.ho.extra.nnscr_included = TRUE;
       M1X_MSG( DCP, LEGACY_MED,
             "WTR_HOP:Updated UHDM with nnscr_included to TRUE, con seq %d",
              mcc_wtr_uhdm_msg_ptr->msg.ho.extra.serv_con_seq);
     }
   } /* if (mcc_wtr_uhdm_msg_ptr != NULL)  */
} /* mcctcho_update_uhdm_for_wtr_hopping */

/*===========================================================================

FUNCTION MCCTCHO_CONFIG_TXC_FOR_HHO_WITH_WTR_HOPPING

DESCRIPTION
  This API will send required cmds to TXC to configure TXC
  for Hard Handoff with WTR Hopping required case.

DEPENDENCIES
  Hard handoff with WTR hopping required is in progress.
...
===========================================================================*/
void mcctcho_config_txc_for_hho_with_wtr_hopping( void )
{
  ...
  if(mcc_wtr_uhdm_msg_ptr == NULL)
  {
    ONEX_ERR_FATAL("WTR_HOP: UHMD msg ptr is NULL", 0, 0, 0);
    return;
  }
  ...
  /* Update UHDM with SCR fields if not already present */
  mcctcho_update_uhdm_for_wtr_hopping();
  ...
  if(mcctcho_create_ecam_from_uhdm(&mcc_wtr_uhdm_msg_ptr->msg.ho, &chn_assign ) == TRUE)
  {
    M1X_MSG( DCP, LEGACY_MED,
      "WTR_HOP:Command Transmit Task to go to Traffic channel");
    ...
    (void) rex_clr_sigs(MC_TCB_PTR, MCC_TXC_DONE_SIG);
    M1X_MSG( DCP, LEGACY_HIGH,
             "WTR_HOP:TC Timing Jump Completed");

    /* Send any deferred cmds to TX */
    tc_wtr_hop_send_deferred_tx_cmds();
  }
  else
  {
    M1X_MSG( DCP, LEGACY_ERROR,
        "WTR_HOP:Can't create ECAM from UHDM");
  }
} /* mcctcho_config_txc_for_hho_with_wtr_hopping() */
...
{
  const caii_srv_cfg_type  *cur_config = snm_get_current_config();
  /*-------------------------------------------------------------*/

  /* This means we received UHDM so Create the ECAM from the UHDM */
  M1X_MSG( DCP, LEGACY_HIGH,
           "WTR_HOP/TA: Forming ECAM fields required by TXC, "
           "FCH incl:%d,For_RC:%d,Rev_RC:%d, DCCH incl:%d,For_RC:%d,Rev_RC:%d",
           cur_config->fch_cc_incl,
           cur_config->for_fch_rc,
           cur_config->rev_fch_rc,
           cur_config->dcch_cc_incl,
           cur_config->for_dcch_rc,
           cur_config->rev_dcch_rc );
  ...
  if( cur_config->dcch_cc_incl )
  {
    /* DCCH */
    chn_assign->mode.am0.rev_rc = cur_config->rev_dcch_rc;
  }
  else
  {
    /* FCH */
    chn_assign->mode.am0.rev_rc = cur_config->rev_fch_rc;
  }
...
FUNCTION MCCTCHO_CONFIG_TXC_FOR_CFS_WITH_WTR_HOPPING

DESCRIPTION
    This API will send required cmds to TXC to re-enable TXC for:
    1. CFS with WTR Hopping while executing Return transaction
    2. HHO failure in case of RIF handoff with WTR hopping before
    executing the Return transaction.
  
  DEPENDENCIES
    CFS measurement with WTR hopping required is in progress and
    SRCH has already tuned back to the serving frequency or
    Handoff Failure has happened for inter band RIF HHO but Return
    transaction has yet not been started.
...
===========================================================================*/
void mcctcho_config_txc_for_cfs_or_rif_with_wtr_hopping( void )
{
  qword public_long_code_mask;
  size_t copy_size;  
  /* Copy the original ECAM in this variable and modify this variable
  ...
  if ( !mcc_wtr_hopping.rif_in_progress )
  {
    ...
    M1X_MSG( DCP, LEGACY_HIGH,
             "WTR_HOP:Sending SCR info to TXC based on curr cfg");
    ...
    M1X_MSG( DCP, LEGACY_HIGH,
           "WTR_HOP: Done updating SCR info of TXC with curr cfg");
...
} /* mcctcho_config_txc_for_cfs_or_rif_with_wtr_hopping() */

/*===========================================================================

FUNCTION MCCTCHO_SRCH_WTR_HOP_BLOCKED_WAIT

DESCRIPTION
  This function sends WTR_HOP_F to SRCH and does a blocked wait
  for WTR_HOP_R report. 
  It is required while returning to serving frequency in inter band RIF HHO.

DEPENDENCIES
  None

RETURN VALUE
  True if WTR_HOP_R was received within time limit, False otherwise.

SIDE EFFECTS
  None

===========================================================================*/
wtr_hop_blocked_wait_type mcctcho_srch_wtr_hop_blocked_wait (void)
{
  dword wait_mask, sigs; /* Signals returned by mcc_wait() call */
  mccsrch_rpt_type *rpt_ptr;
  wtr_hop_blocked_wait_type ret_val;
  boolean wait = TRUE;
  
  /* Send WTR_HOP_F to SRCH to tune back to serving frequency */
  mcc_srch_buf.wtr_hop.band = (srch_rx_band_type)mcc_wtr_hopping.band_class;
  mcc_srch_buf.wtr_hop.chan = (srch_rx_chan_type)mcc_wtr_hopping.cdma_freq;
  mcc_srch_buf.wtr_hop.hdr.cmd = SRCH_WTR_HOP_F;
  mcc_srch_cmd( &mcc_srch_buf );  

  /* Start timer to ensure we are not stuck here forever.
   * Re-using this timer since its usage here is mutually exclusive
   * from its usage while initiating the handoff transaction */
  (void) mcc_set_callback_timer( &mcc_wtr_hop_buf_timer, 
                                 WTR_HOP_RPT_BLOCKED_WAIT_TIME,
                                 MCC_WTR_HOP_BUF_TIMER_ID );  

  /* set the wait mask to be MCC_SRCH_Q_SIG | MCC_CMD_TIMER_SIG
  * so it will return either by a timer expiration, or by a
  * Searcher Report */
  wait_mask = MCC_SRCH_Q_SIG | MCC_CALLBACK_TIMER_SIG;
  ...
  while (wait)
  {
    ...
    */
    if ( (sigs & MCC_SRCH_Q_SIG) != 0 )
    {
      ...
      {
        if ( rpt_ptr->hdr.rpt == SRCH_WTR_HOP_R )
        {
          M1X_MSG( DCP, LEGACY_HIGH,
                 "WTR_HOP: Received WTR_HOP_R in blocked wait, status %d",
                 rpt_ptr->wtr_hop_rpt.status );

          ret_val = WTR_HOP_RPT_RECEIVED; 
          wait = FALSE;
        }
        else if ( rpt_ptr->hdr.rpt == SRCH_REQUEST_SYS_RESTART_R )
        {
          ...
          M1X_MSG( DCP, LEGACY_HIGH,
            "WTR_HOP: SRCH request sys restart, TRM lock not available: %d",
            rpt_ptr->sys_restart.trm_lock_not_avail );

          ret_val = WTR_HOP_SYS_RESTART; 
          wait = FALSE;
        }
        else
        {
          M1X_MSG( DCP, LEGACY_HIGH,
                   "WTR_HOP: Unexpected SRCH report %d waiting for WTR_HOP_R",
                    rpt_ptr->hdr.rpt );
        }        
        ...
        mcc_enqueue_dbg_srch_cmd( rpt_ptr->hdr.rpt, cdma.curr_state, 
                                  ...
                                  MCCTCHO_SRCH_WTR_HOP_BLOCKED_WAIT );
        ...
        mc_free_queue_buf( rpt_ptr, MCC_SRCH_Q );
              ...
              mcc_timer_on_callback_q(MCC_WTR_HOP_BUF_TIMER_ID)
            )                                        
    {
      M1X_MSG( DCP, LEGACY_HIGH,
             "WTR_HOP: Timeout waiting for WTR_HOP_F report for RIF HHO" );
      
      ret_val = WTR_HOP_TIMEOUT; 
      wait = FALSE;
  ...
  /* Clear timer while exiting blocked wait irrespective of succcess/failure */
