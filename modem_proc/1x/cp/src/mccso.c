...
$Header: //components/rel/1x.mpss/8.0/cp/src/mccso.c#1 $ $DateTime: 2019/06/11 23:51:40 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
08/31/17   ppr     Sub specific 1x EFS Item support chgs
09/29/15   agh     Remove all references to MVS and cleanup WON feature
07/22/15   ppr     Chgs to convert enc_rate_params dynamic mem to static mem
04/07/15   agh     Use COP rate macros for sending COP rate to Voice Adapter
...
{
  word cust_voice_so = mcc_get_cust_so();
    /* Customer Configured Voice SO */

  /*----------------------------------------------------------------------*/

  if ( cust_voice_so != CAI_SO_NULL )
  {
    /* If valid Customer Voice SO is configured then 
     * we will add customer voice so as alternate SO in SO3 neg params 
     * at last entry in mccso_initialize() API.
     * Check if cust voice so is already part to SO3 alt so list, if not present
     * then add it and add the cust voice SO to SNP as well */
    if ( ( mccso_so3_neg_par.alternate_so[MAX_ALT_SO - 1] == CAI_SO_NULL ) ||
         ( mccso_so3_neg_par.alternate_so[MAX_ALT_SO - 1] != cust_voice_so )
       )
    {
      /* Add cust voice SO to SO3 Alt SO list as last entry */
      mccso_so3_neg_par.alternate_so[MAX_ALT_SO - 1] = cust_voice_so;

      /* First Add/Enable Cust Voice SO to SNP */
      (void)snm_add_so( cust_voice_so, 
                        (snm_neg_par_type*)&mccso_cust_so_neg_par, 
                        &mccso_rs1_so_default_cfg );

      /* Update SO-3 neg_params with newly configured Cust Voice SO */
      (void)snm_add_so( CAI_SO_VOICE_EVRC, 
                        (snm_neg_par_type*)&mccso_so3_neg_par, 
                        &mccso_rs1_so_default_cfg );

      (void)snm_reg_connection_initializer( cust_voice_so,
                                            mccso_so_voice_con_initializer );

      M1X_MSG( DCP, LEGACY_MED,
               "Added cust_voice_so:%d to SO3 Alt_so[%d]:%d",
               cust_voice_so,
               (MAX_ALT_SO - 1),
               mccso_so3_neg_par.alternate_so[MAX_ALT_SO - 1] );
    } /* SO3 Alt_so_last_entry == NULL || != cust_voice_so */
  } /* if ( cust_voice_so != CAI_SO_NULL ) */ 
  else
  {
    /* Customer Voice SO is NULL,
     * so remove Customer Voice SO from SO-3 Alt_SO list */
    mccso_so3_neg_par.alternate_so[MAX_ALT_SO - 1] = CAI_SO_NULL;

    /* Update SO-3 neg_params with removed entry for Cust Voice SO */
    (void)snm_add_so( CAI_SO_VOICE_EVRC, 
                      (snm_neg_par_type*)&mccso_so3_neg_par, 
                      &mccso_rs1_so_default_cfg );
  }
