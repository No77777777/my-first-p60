...
$Header: //components/rel/1x.mpss/8.0/cp/src/mccqsh.c#3 $ $DateTime: 2020/05/07 06:56:41 $ $Author: pwbldsvc $
...
01/19/18   ppr     QSH:Just ignore metric timer expiry QSH cmd while inactive
01/19/18   agh     Don't log RF metrics with QSH if RSSI has default value
01/19/18   agh     Added Srch state to QSH metrics and made ECIO, RSSI signed
08/02/17   sjo     FR 37336: Migration to MSGR VARIANT from Inst ID
12/12/16   sjo     Fixed KW errors.
07/19/16   ppr     QSH:Generic Event changes
07/12/16   sjo     Avoid processing QSH cmd when 1X is inactive.
07/08/16   ppr     QSH:Enable event diagnostic data to QSH
02/23/16   agh     Log only valid values of TX power in traffic state
11/09/15   agh     Fixed units and default values of some metrics
11/09/15   agh     Fixed compilation errors in No-TDSCDMA-or-L1 strip builds
11/09/15   agh     Initial Revision
...
/*  M1X_MSG( DCP, LEGACY_HIGH,
    ...
    )
  {
    ...
    { 
      /* While in inactive state, only ignore METRIC_TIMER_EXPIRY QSH cmd
       * and process other QSH config cmds to configure Event, Metric rel
       * params */       
  ...
  #ifdef FEATURE_QSH_MDUMP
  else if( cb_params_ptr->action ==  QSH_ACTION_MDUMP)
  {
/*    M1X_MSG( DCP, LEGACY_HIGH,
           "DEBUG: action = MDUMP, onex qsh mdump collect called" ); */
    mc_qsh_mdump_collect();
  }
  ...
  { 
    ...
    case QSH_ACTION_CFG:
    {
      ...
      M1X_MSG( DCP, LEGACY_MED, 
               "QSH: Event id:%d, action:%d, sub_id:%d, 1x_ASID:%d",
               event_id, 
               action,
               sub_id,
               tmp_asid );
      ...
      if(tmp_asid == sub_id)
      {
        /* event_id should be within the max value ONEXL3_QSH_EVENT_MAX */
        if( event_id < ONEXL3_QSH_EVENT_MAX )
        {
        ...
        {
          M1X_MSG( DCP, LEGACY_ERROR, 
                   "QSH: event_id %d greater than max value %d",
                   event_id, ONEXL3_QSH_EVENT_MAX);
        }
      } /* if(tmp_asid == sub_id) */
      else
      {
        M1X_MSG( DCP, LEGACY_ERROR, 
                 "QSH: 1x_ASID mismatch, ignore QSH_ACTION_CFG cmd" );
      }
...
)
{
    ...
    )
  {
    ...
    else
    {
      ...
      M1X_MSG( DCP, LEGACY_MED, 
               "QSH: Sending QSH_Event_Id:%d, 1x_ASID:%d", 
               onex_event,
               event_notify_params.subs_id );
    }
    ...
    switch(onex_event)
    {
      ...
      case ONEXL3_QSH_EVENT_L1_TIMING_ERR:
      {
        QSH_LOG(QSH_CLT_ONEXL3, QSH_CAT_EVENT, QSH_MSG_TYPE_HIGH,
                "QSH:1x ONEXL3_QSH_EVENT_L1_TIMING_ERR, 1x_ASID:%d",
                event_notify_params.subs_id);
        break;
      ...
      {
        QSH_LOG(QSH_CLT_ONEXL3, QSH_CAT_EVENT, QSH_MSG_TYPE_HIGH,
                "QSH:1x ONEXL3_QSH_EVENT_L1_TUNE_FREQ_ERR, 1x_ASID:%d",
                event_notify_params.subs_id);
        break;
      ...
      {
        QSH_LOG(QSH_CLT_ONEXL3, QSH_CAT_EVENT, QSH_MSG_TYPE_HIGH,
                "QSH:1x ONEXL3_QSH_EVENT_L1_PAGE_MISS, 1x_ASID:%d",
                event_notify_params.subs_id);
        break;
      ...
      {
        QSH_LOG(QSH_CLT_ONEXL3, QSH_CAT_EVENT, QSH_MSG_TYPE_HIGH,
                "QSH:1x ONEXL3_QSH_EVENT_L1_ASDIV_SWITCH, 1x_ASID:%d",
                event_notify_params.subs_id);
        break;
      ...
      {
        QSH_LOG(QSH_CLT_ONEXL3, QSH_CAT_EVENT, QSH_MSG_TYPE_HIGH,
                "QSH:1x ONEXL3_QSH_EVENT_L1_EMPTY_QTA, 1x_ASID:%d",
                event_notify_params.subs_id);
        break;
      ...
      {
        QSH_LOG(QSH_CLT_ONEXL3, QSH_CAT_EVENT, QSH_MSG_TYPE_HIGH,
                "QSH:1x ONEXL3_QSH_EVENT_L1_TRM_LOCK_WAIT_TIMEOUT, 1x_ASID:%d",
                event_notify_params.subs_id);
        break;
      ...
      {
        QSH_LOG(QSH_CLT_ONEXL3, QSH_CAT_EVENT, QSH_MSG_TYPE_HIGH,
                "QSH:1x ONEXL3_QSH_EVENT_L2_PAGE_DEC_FAIL, 1x_ASID:%d",
                event_notify_params.subs_id);
        break;
      ...
      {
        QSH_LOG(QSH_CLT_ONEXL3, QSH_CAT_EVENT, QSH_MSG_TYPE_HIGH,
                "QSH:1x ONEXL3_QSH_EVENT_L2_HIGH_RXAGC_DELTA, 1x_ASID:%d",
                event_notify_params.subs_id);
        break;
      ...
      {
        QSH_LOG(QSH_CLT_ONEXL3, QSH_CAT_EVENT, QSH_MSG_TYPE_HIGH,
                "QSH:1x ONEXL3_QSH_EVENT_L3_OOS, 1x_ASID:%d",
                event_notify_params.subs_id);
        break;
      ...
      {
        QSH_LOG(QSH_CLT_ONEXL3, QSH_CAT_EVENT, QSH_MSG_TYPE_HIGH,
                "QSH:1x ONEXL3_QSH_EVENT_L3_RLF, 1x_ASID:%d",
                event_notify_params.subs_id);
        break;
      ...
      {
        QSH_LOG(QSH_CLT_ONEXL3, QSH_CAT_EVENT, QSH_MSG_TYPE_HIGH,
                "QSH:1x ONEXL3_QSH_EVENT_L3_ACCESS_FAIL, 1x_ASID:%d",
                event_notify_params.subs_id);
        break;
      ...
      {
        QSH_LOG(QSH_CLT_ONEXL3, QSH_CAT_EVENT, QSH_MSG_TYPE_HIGH,
                "QSH:1x ONEXL3_QSH_EVENT_L3_INTER_FREQ_HO_FAIL, 1x_ASID:%d",
                event_notify_params.subs_id);
        break;
      ...
      {
        QSH_LOG(QSH_CLT_ONEXL3, QSH_CAT_EVENT, QSH_MSG_TYPE_HIGH,
                "QSH:1x ONEXL3_QSH_EVENT_L3_ORIG_FAIL, 1x_ASID:%d",
                event_notify_params.subs_id);
        break;
      ...
      {
        QSH_LOG(QSH_CLT_ONEXL3, QSH_CAT_EVENT, QSH_MSG_TYPE_HIGH,
                "QSH:1x ONEXL3_QSH_EVENT_L3_L21X_REDIR_FAIL, 1x_ASID:%d",
                event_notify_params.subs_id);
        break;
      ...
      {
        QSH_LOG(QSH_CLT_ONEXL3, QSH_CAT_EVENT, QSH_MSG_TYPE_HIGH,
                "QSH:1x ONEXL3_QSH_EVENT_L3_L21X_RESEL_FAIL, 1x_ASID:%d",
                event_notify_params.subs_id);
        break;
      ...
      {
        QSH_LOG(QSH_CLT_ONEXL3, QSH_CAT_EVENT, QSH_MSG_TYPE_HIGH,
                "QSH:1x ONEXL3_QSH_EVENT_L3_1X2L_RESEL_FAIL, 1x_ASID:%d",
                event_notify_params.subs_id);
        break;
...
)
{
       ...
       metric_cfg->size_bytes >= sizeof(onexl3_qsh_metric_cell_info_s))
  {
    ...
    {
      M1X_MSG( DCP, LEGACY_HIGH, 
          "QSH: Cell info Metrics, pilot %d, reg_zone %d", 
    ...
    {
      M1X_MSG( DCP, LEGACY_MED, 
               "QSH: Cell info metrics not available, Curr_State:0x%x", 
               cdma.curr_state ); 
    }
  }
} /* mc_send_cell_info_metrics */
...
)
{
  onexl3_qsh_metric_rf_params_s *metrics_ptr;
   ...
   static byte count = 0;
   /* This variable is used to make the below print come once every 10
    * times this API is called. This is done to reduce the log count 
    */
   boolean log_qsh = FALSE;
     /* Boolean var whether to log RF metric F3 msg or not */
  
  qsh_client_metric_cfg_s *metric_cfg = 
                     &(mc_qsh_data.metric_cfg[ONEXL3_QSH_METRIC_RF_PARAMS]);
  ...
  {
    M1X_MSG( DCP, LEGACY_MED, 
             "QSH: RF metrics disabled, skip sending to QSH");
    return;
  }
  
  /* Increment and check the count variable to log QSH RF metric F3 msg */
  if(count++ >= 20)
  {
    count = 0;
    log_qsh = TRUE;
  ...
  {
    M1X_MSG( DCP, LEGACY_MED, 
        "QSH: Sending RF metrics, start addr: 0x%x, size %d, req size %d", 
             metrics_ptr, metric_cfg->size_bytes, 
             sizeof(onexl3_qsh_metric_rf_params_s));
  } /* if(log_qsh) */
    ...
    )
  {
    if(mcc_update_rf_params_metrics(metrics_ptr))
    {
      ...
      {
        M1X_MSG( DCP, LEGACY_HIGH, 
          "QSH: RF Metrics, rssi0:%d, rssi1:%d, rssi_comb:%d, "
          "ecio0:%d, ecio1:%d, ecio_comb:%d, tx_pwr:%d, l1_state:0x%x", 
                 metrics_ptr->rssi0, 
                 metrics_ptr->rssi1,
                 metrics_ptr->rssi_comb,
                 metrics_ptr->ecio0,
                 metrics_ptr->ecio1,
                 metrics_ptr->ecio_comb,
                 metrics_ptr->tx_pwr,
                 metrics_ptr->l1_state );  
      } /* if(log_qsh) */
    ...
    {
      M1X_MSG( DCP, LEGACY_MED, 
               "QSH: RF metrics not available, Curr_State:0x%x", 
               cdma.curr_state ); 
    }
  }
...
{   
  ...
  ret_value = srch_get_cdma_rssi_all_chains( &rssi_comb, &rssi0, &rssi1 );

  /* Don't log RF metrics with QSH if RSSI has default value */
  if (ret_value == 125 || rssi0 == 125)
  {
    return FALSE;
  }
...
)
{
  ...
  if (cur_asid == SYS_MODEM_AS_ID_NONE)
  {
    ...
    M1X_MSG( DCP,  LEGACY_HIGH, 
             "1X_ASID is None, defaulting to ASID 1");    
  }
  return cur_asid;
...
#ifdef FEATURE_QSH_MDUMP
/*===========================================================================
FUNCTION mc_collect_mdump
  
DESCRIPTION
  This function collects the mdump for mc
...
===========================================================================*/
QSH_MDUMP_FN_ATTR 
void mc_qsh_mdump_collect()
{
  /*If WRRC was active, collect configuration related globals*/
  /* if(rrcmcm_get_rrc_mode(wrrc_as_id) != RRC_MODE_INACTIVE)*/
  {
/*    M1X_MSG( DCP, LEGACY_HIGH,
           "DEBUG: calling mc_qsh_mdump_collect_cp_data()" ); */
    mc_qsh_mdump_collect_cp_data();
    mc_qsh_mdump_collect_mux_data();
    mc_qsh_mdump_collect_srch_data();
    mc_qsh_mdump_collect_m1x_stmr_data();
/*    M1X_MSG( DCP, LEGACY_HIGH,
           "DEBUG: returned from mc_qsh_mdump_collect_cp_data()" ); */
  }
}
#endif /* FEATURE_QSH_MDUMP */
