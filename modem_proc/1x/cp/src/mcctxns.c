...
GENERAL DESCRIPTION
  ...
  Copyright (c) 2000 - 2020 Qualcomm Technologies, Inc.
...
$Header: //components/rel/1x.mpss/8.0/cp/src/mcctxns.c#2 $ $DateTime: 2020/01/27 00:26:21 $ $Author: pwbldsvc $
...
)
{
  ...
  else 
  {
    ...
    /* Set boolean to keep track of */
    mcctcho_ho_data_ptr->cfs_nset_trans_in_prog = TRUE;
  }
  ...
  M1X_MSG( DCP, LEGACY_MED,
   "CFS trans despatched, trans_num %d, trans_type %d, cfs_nset_trans_prog:%d",
    trans_num,
...
)
{
  ...
  switch ( server_token )
  {
    ...
    {
      /* Inform RX task if 1X is in non DDS sub or not. This information is 
       * given by MMODE. The cm_is_dds_sub returns TRUE if 1X is in dds sub */
      cdma.is_1x_on_non_dds_sub = !cm_is_dds_sub( mcc_asid );
      packet->action.rxc_scr.is_1x_on_non_dds_sub = cdma.is_1x_on_non_dds_sub;
      ...
      for (i = 0; (i < config->num_con) && ( i< MAX_NO_OF_SO_RECS); i++)
      {
             ...
             config->con[i].sr_id) > MAX_SR_ID_VALUE)
        {
          ...
          ONEX_ERR_FATAL("con ref %d has illegal sr_id %d", config->con[i].con_ref, config->con[i].sr_id,0);
        }
    ...
    case MCTRANS_SERVER_TXC :
    {
      ...
      for (i = 0; (i < config->num_con) && (i < MAX_NO_OF_SO_RECS); i++)
      {
        ...
        {
          ONEX_ERR_FATAL("No fundicated phy channel associated with sr_id %d", config->con[i].sr_id,0,0);
        }
         ...
         )
      {
        ...
        ONEX_ERR_FATAL("No phy channel available for signaling",0,0,0);
      }
...
)
{
  ...
  switch ( server_token )
  {
    case MCTRANS_SERVER_RXC:
    {
      ...
      {
        ONEX_ERR_FATAL ("Wrong action type %d", (int) action,0,0);
      }
...
)
{
  ...
  {
    /* Set the boolean indicating the return transaction for CFS have begun */
    mcc_set_cfs_return_trans_despatched(TRUE);
...
)
{
  ...
  /* If WTR Hopping is required, don't add TXC action as TXC will not even
   * be active while executing this transaction. */
  if ( mcctcho_get_ho_data()->cfs_wtr_hop_in_prog )    
  {     
    M1X_MSG ( DCP, LEGACY_MED, 
             "WTR_HOP: CFS Meas trans, skipped adding TXC action");
  }
  else
  {
    mctrans_add_action_to_server(curr_trans, MCTRANS_SERVER_TXC, MCTRANS_TX_ON_OFF_ACTION);      
  } /* if ( mcctcho_get_ho_data */
...
)
{
  ...
  mctrans_despatch_info_type despatch_info =
  {
   ...
   /* If WTR Hopping is required, add TXC as the last server since TXC will
    * not even be active while executing this transaction. So TXC will not
    * be able to signal the next server resulting in transaction getting 
    * stuck if TXC is not the last server */
  if ( mcctcho_get_ho_data()->cfs_wtr_hop_in_prog )    
  {     
    server_order[0] = MCTRANS_SERVER_RXC;
    server_order[1] = MCTRANS_SERVER_SRCH;
