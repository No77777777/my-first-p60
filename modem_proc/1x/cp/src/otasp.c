...
INITIALIZATION AND SEQUENCING REQUIREMENTS
  ...
  Copyright (c) 1996 - 2022 Qualcomm Technologies, Inc.
...
$Header: //components/rel/1x.mpss/8.0/cp/src/otasp.c#3 $ $DateTime: 2022/06/21 07:06:23 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
06/21/22   vaa     Length check for potential stack overflow in 
                   process_ms_key_req
...
void send_cfg_resp_msg(void)
{
  ...
  {
      ONEX_ERR_FATAL( "Failed OTASP message translation",0,0,0);
  }
}  /* send_cfg_resp_msg */
#endif /* FEATURE_UIM_RUIM */
...
)
{
 ...
 /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
...
#endif /* FEATURE_UIM_RUN_TIME_ENABLE */
   {
     ...
     pos = 0;
     for (i = 0; (i < (ms_key_req_ptr->param_g_len/sizeof(dword)) && 
     ...
     pos = 0;
     for (i = 0; (i < (ms_key_req_ptr->param_p_len/sizeof(dword)) && 
   ...
   switch(otasp_dh_rpt->rpt_type) {
      ...
      case DH_EXP_R:
         ...
         else if (otasp_dh.state == OTASP_WAIT_FOR_A_KEY)
         {
            ...
            {
              ONEX_ERR_FATAL( "No free buffers on auth_free_q", 0, 0, 0 );
            }
...
)
{
 ...
 /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
...
#endif /* FEATURE_UIM_RUN_TIME_ENABLE */
   {
     ...
     pos = 0;
     for (i = 0; (i < (key_gen_req_ptr->bs_result_len/sizeof(dword)) && 
                   i < (OTASPI_PARAM_P/sizeof(dword))); i++)
     {
  ...
  if ((auth_cmd_ptr = (auth_cmd_type*) q_get( &auth_free_q )) != NULL)
  {
    ...
    if (auth_cmd_ptr->hdr.status != AUTH_RECEIVED_S)
    {
      ...
      ** ----------------------------------------------- */
      ...
      #endif /* MMOC_API_PH_STAT_CNF_WITH_STATUS */
      {
        ONEX_ERR_FATAL( "Bad status from AUTH %d", (int) auth_cmd_ptr->hdr.status,0,0);
      }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
...
#if !defined( FEATURE_UIM_RUIM ) || defined( FEATURE_UIM_RUN_TIME_ENABLE )
  {
    ...
    if (validn_result == PRL_VALID)
    {
      ...
      if (res != 0)
      {
        ONEX_ERR_FATAL( "EFS sync failed",0,0,0);
      }
    }
    M1X_MSG( DCP, LEGACY_MED,
...
*/
const prl_hdr_s_type  *prl_hdr_ptr = prl_hdr_nv_get_ptr( mcc_asid );
...
)
{
  ...
  /* mobile station supports OTAPA */
    ...
    else //it's OK
    {
        ...
        if (nam_locked_state)
        {
          ...
          if ((auth_cmd_ptr = (auth_cmd_type*) q_get( &auth_free_q )) != NULL)
          {
            ...
            if (auth_cmd_ptr->hdr.status != AUTH_RECEIVED_S)
            {
              ...
              ** ----------------------------------------------- */
              ONEX_ERR_FATAL( "Bad status from AUTH %d",
      ...
      {
        ONEX_ERR_FATAL( "invalid stop/start value: %d", otapa_req_ptr->start_stop,0,0);
      }
    }
