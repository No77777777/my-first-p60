...
$Header: //components/rel/1x.mpss/8.0/cp/src/mccdma.c#2 $ $DateTime: 2020/01/27 00:26:21 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ------------------------------------------------------------
01/22/20   vaa     Implement QSH MDUMP framework
02/22/18   sjo     Chgs to open MMGSDI sessions for both subs. 
01/19/18   ppr     F3 Reduction changes
01/16/18   ppr     API to get if 1x is waiting for RF resource or not.
12/18/17   ppr     1x pwr-up optimization changes
11/17/17   ppr     Mainline GPP2_SUPPORT_ON_SEC_SUB feature
11/10/17   svu     fixed race condition for SLEEP_COMPLETED_R
11/10/17   ppr     Don't honor unlock events till SPM rcvd for Idle HO to 
                   same config ngbr and SPM not in stored ovhds
11/10/17   sjo     Chgs to handle RAFAIL_R rxed in CDMA_EXIT while 1X is in QTA
11/10/17   sjo     Chgs to add API for SRCH to query if an empty GPM was rxed.
11/10/17   sjo     Changes to send SLEEP_F if CDMA_EXIT is because of reacq
                   failure and SRCH is having QTA enabled.
11/10/17   sjo     Process SYS_RELEASE_R rxed while waiting for SLEEP_COMP_R
                   during exit processing.
11/10/17   sjo     Do force sleep, rude wakeup while ACC->IDLE transition.
10/30/17   ppr     Remove Feature flag ONEX_TA_30_BRINGUP.
09/25/17   sjo     Expose an API to MMOC to query if 1X is active or not.
08/31/17   ppr     Avoid ERR_FATAL for NV Read/Write operations
08/31/17   ppr     Chgs to replace EFS#MC_EFS_MMGSDI_FEATURES_STATUS_LIST 
                   with MMGSDI API
08/31/17   ppr     Chgs to re-enable SO68, SO70, SO73 if their NV is enabled
                   while re-loading the subscription info
08/31/17   sjo     Chgs for CDMA_support on SUB2
08/29/17   sjo     Fixed compilation errors by featurizing QTA related changes
                   under FEATURE_MODEM_1X_QTA.
08/29/17   sjo     Handle SRCH_RELEASE_SYSTEM_R rxed in cdma_exit_init() API
08/29/17   sjo     Chgs to handle RAFAIL_R rxed in CDMA_EXIT while 1X is in QTA
08/29/17   sjo     Changes to send SLEEP_F if CDMA_EXIT is because of reacq
                   failure and SRCH is having QTA enabled.
08/29/17   sjo     Process SYS_RELEASE_R rxed while waiting for SLEEP_COMP_R
                   during exit processing.
08/29/17   sjo     Changes to wait for SLEEP_COMP_R if SRHC is in QTA while
                   doing exit processing
08/14/17   sjo     FR: 34995 chgs to enable sniffer device mode.
08/02/17   sjo     FR 37336: Migration to MSGR VARIANT from Inst ID
03/07/17   ppr     Default mcc_asid related changes
10/13/16   sjo     Expose API MCC_IS_CDMA_CALL_IN_PROGRESS to check call status
08/29/16   sjo     Created new debug buffer for lte rrc responses
07/07/16   ppr     Chgs to ingore MT pages to know n/w page rep pattern
07/07/16   ppr     Logging chgs
06/29/16   ppr     Chgs to add RatMask payload to Incoming_page call rpts to CM
05/30/16   ppr     Chgs to honor UE mode change and 1X_to_LTE_RESEL Abort reqs
05/23/16   ppr     F3 Reduction changes
04/12/16   ppr     WTR_HOP:Chgs to buf CFS Req msg if CFS with WTR Hop in prog
03/24/16   ppr     WTR_HOP:Chgs to handle HHO colliding with CFS with WTR HOP
02/18/16   ppr     1X2GTA:Priority Inversion changes
02/17/16   sjo     Chgs to honor the SRCH_FING_DONE_R properly while cfs
02/05/16   ppr     1x2GTA Feature changes
12/01/15   ppr     Chgs to reduce F3 logging 
11/09/15   agh     Support QSH framework to collect periodic/event metrics
...
08/24/15   ppr     Reg Throttling chgs for Transceiver request denial cases
07/23/15   agh     Defer WTR HOP procedures till HHO action time
07/13/15   ppr     HHO WTR Hopping Changes
07/01/15   ppr     Reverting back MC sending force sleep if SRCH is in QTA 
                   before CDMA_F in cdma_exit
06/01/15   sjo     Enhance debugging interface between MMOC and CP
05/22/15   agh     Handle race conditions involving Redirection received from  
                   n/w while 1X_CP is waiting for call_clear_f from CM
05/21/15   agh     Optimization to add previous serving channel to NL rxed on
                   current serving channel after handoff if not already present
03/26/15   agh     1X-CP changes to support QTA feature
04/20/15   agh     Add stub for onex_voice_call API in non-CDMA build flavors
04/16/15   agh     Rename ONEX_VOICE_CALL macro to avoid Audio module changes
04/15/15   agh     Make 1X API available to Audio for CDMA compiled-out flavors
04/15/15   agh     Use COP rate macros for sending COP rate to Voice Adapter
                   instead of directly sending absolute COP values
04/15/15   agh     Use default value for configuring VFR for PLs on which
                   Voice agent interface is not present
04/15/15   ppr     Mainling FEATURE_COEX_RPT_RAT_L3_STATE feature
...
#include "tmsi.h"
#include "dh_v.h"
#ifdef FEATURE_MODEM_1X_QTA
#include "srch_idle_ta.h"
#endif /* FEATURE_MODEM_1X_QTA */
...
rex_timer_type mcc_hdm_tx_ack_timer;
/* Timer for waiting for confirmation from TX that Ack has been
 * sent for HDM message */

rex_timer_type mcc_wtr_hop_buf_timer;
/* Timer to defer WTR HOP procedures till UE is within a buffer time 
 * of HHO action time */

/* Timer to wait on PCH2 for a valid OTA msg before switching back to 
 * Primary PCH1, for Idle handoff to nghbr_config=0 nghbr and 
 * SPM is not in stored ovhds scenario. 
 * This is required in case if n/w wrongly configures two nghbrs with 
 * diff number of paging channels as nghbr_config=0 nghbrs */
rex_timer_type  mcc_pri_pch_hash_timer;
...
/* 1X2G Tune Away complete buffer timer */
rex_timer_type mcc_ta_comp_buf_timer;
...
/* The variable to store the information if the SRCH_CF_NSET_R report has
 * been received and the return transactions have begun or not. This will
 * be used to properly honor the SRCH_FING_DONE_R received once the return
 * transaction was finished. */

boolean mcc_cfs_return_trans_despatched = FALSE;
...
#ifdef FEATURE_1X_NSET_OPTIMIZATION
...
#endif /* FEATURE_1X_NSET_OPTIMIZATION */
...
#if defined(FEATURE_1X_SPLIT_OVERHEAD) || defined(FEATURE_MODEM_1X_QTA)
...
#endif /* FEATURE_1X_SPLIT_OVERHEAD || FEATURE_MODEM_1X_QTA */
...
/* VFR value passed by mmode. Initial value is set to SYS_VFR_PRIMARY */
sys_vfr_e_type mcc_vfr = SYS_VFR_PRIMARY;
...
uint32 mcc_curr_sub_rat_mask = 0;

/* WTR Hopping related structures */
mcc_wtr_hopping_type mcc_wtr_hopping;

/* Need to buffer a copy of UHDM message for WTR hopping */
mccrxtx_msg_type *mcc_wtr_uhdm_msg_ptr = NULL;

/* Need to buffer a copy of UHDM msg if CFS with WTR Hop is in Progress */
mccrxtx_msg_type *mcc_temp_uhdm_msg_ptr = NULL;

/* Need to buffer a copy of CFS Req msg if CFS with WTR Hop is in Progress */
caii_cfs_req_type *mcc_temp_cfs_req_msg_ptr = NULL;
...
DESCRIPTION
   ...
   is supported or not. This could either be for G->1X Tuneaway feature or
   1X QTA to LTE feature.

DEPENDENCIES
  None.

RETURN VALUE
  TRUE if NV#73501 is set as'1' or QTA NV is set as '1'.
...
FUNCTION MCC_IS_SPLIT_OVHDS_FOR_G_TO_1X_SUP

DESCRIPTION
   This function returns whether the split overhead across multiple wakeups
   is supported or not for G->1X TA feature. This API is meant to be used by
   1X-SRCH for G->1X TA feature only.
...
boolean mcc_is_split_ovhds_for_g_to_1x_sup(void)
{
  return cdma.split_ovhd_supported;
}/* mcc_is_split_ovhds_for_g_to_1x_sup */

#endif /* FEATURE_1X_SPLIT_OVERHEAD */

/*===========================================================================

FUNCTION mcc_reload_bootup_nv_items()

DESCRIPTION
  This function reloads all the NVs and EFS items read during initial
  bootup.
...
void mcc_reload_bootup_nv_items( void )
	
{
  nv_stat_enum_type read_status;  /* NV read status */
     
  mccreg_reload_bootup_nv_items();
  ...
  /* NV_CDMA_SO68_ENABLED_I */
  
  /* Read whether 4GV_NB is enabled */
  mcc_nv_buf.item = NV_CDMA_SO68_ENABLED_I;
  read_status = mcc_nv_cmd( &mcc_nv_buf );
  if( ((read_status == NV_DONE_S) && (!mcc_nv_data.cdma_so68_enabled)) ||
      (read_status == NV_NOTACTIVE_S)
    )
  {
    (void)snm_disable_so( CAI_SO_VOICE_4GV_NB );
  }
  else if( (read_status == NV_DONE_S) &&
           (mcc_nv_data.cdma_so68_enabled)
         )
  {
    (void)snm_enable_so( CAI_SO_VOICE_4GV_NB );
  }
  M1X_MSG( DCP, LEGACY_MED,
           "Subs Info Reload, SO68 read_status:%d, cdma_so68_enabled:%d, "
           "SNP_SO68_Enabled:%d",
           read_status,
           mcc_nv_data.cdma_so68_enabled,
           snm_so_is_enabled(CAI_SO_VOICE_4GV_NB) );
  
  /* NV_CDMA_SO70_ENABLED_I */
  
  /* Read whether 4GV_WB is enabled */
  mcc_nv_buf.item = NV_CDMA_SO70_ENABLED_I;
  read_status = mcc_nv_cmd( &mcc_nv_buf );

  if( ((read_status == NV_DONE_S) && (!mcc_nv_data.cdma_so70_enabled)) ||
      (read_status == NV_NOTACTIVE_S)
    )
  {
    (void)snm_disable_so( CAI_SO_VOICE_4GV_WB );
           ...
           (mcc_nv_data.cdma_so70_enabled)
         )
  {
    (void)snm_enable_so( CAI_SO_VOICE_4GV_WB );
  }
  M1X_MSG( DCP, LEGACY_MED,
           "Subs Info Reload, SO70 read_status:%d, cdma_so70_enabled:%d, "
           "SNP_SO70_Enabled:%d",
           read_status,
           mcc_nv_data.cdma_so70_enabled,
           snm_so_is_enabled(CAI_SO_VOICE_4GV_WB) );
  
  /* NV_CDMA_SO73_ENABLED_I */
  ...
  {
    if( ((read_status == NV_DONE_S) && (!mcc_nv_data.cdma_so73_enabled)) ||
        (read_status == NV_NOTACTIVE_S)
      )
    {
      (void)snm_disable_so( CAI_SO_VOICE_EVRC_NW );
             ...
             (mcc_nv_data.cdma_so73_enabled)
           )
    {
      (void)snm_enable_so( CAI_SO_VOICE_EVRC_NW );
    }
  } /* cdma.onexadv_status.status */
  M1X_MSG( DCP, LEGACY_MED,
           "Subs Info Reload, SO73 read_status:%d, cdma_so73_enabled:%d, "
           "SNP_SO73_Enabled:%d",
           read_status,
           mcc_nv_data.cdma_so73_enabled,
           snm_so_is_enabled(CAI_SO_VOICE_EVRC_NW) );
...
)
{
  ...
  if( ( cmd_ptr->hdr.cmd == SRCH_SLEEP_F ) ||
      ( cmd_ptr->hdr.cmd == SRCH_CDMA_F )
    )
  {
    /* Reset the flag is_rude_wakeup_with_tx_req_done whenever CP sends 
     * SLEEP_F or CDMA_F to SRCH. This flag is set whenever CP asks for 
     * rude_wakeup with the tx_resources_required argument set to TRUE.
     * The requirement is that CP should not do another rude_wakeup
     * until CP sends SLEEP_F or CDMA_F to SRCH. Hence we reset it here */
     
    mcc_update_rude_wakeup_with_tx_req_done( FALSE );

    /* Reset the flag indicating if an empty GPM has been received */
    cdma.empty_gpm_rxed = FALSE;
  }

  if( cmd_ptr->hdr.cmd == SRCH_CDMA_F )
  {
    /* Clear SPM Sys Rel Flag, PCH Validation Flag, PCH hash timer */
    mcc_set_idle_ho_spm_wait_block_sys_rel_flag( FALSE );
    mcc_set_idle_ho_pch_validation_done_flag( FALSE );
    (void) mcc_clr_callback_timer( &mcc_pri_pch_hash_timer, 
                                   MCC_PRI_PCH_HASH_TIMER_ID );
  } /* if(cmd == SRCH_CDMA_F) */
...
)
{
  ...
  if (cmd_ptr->hdr.status != RXC_DONE_S)
  {
    ...
    {
      ONEX_ERR_FATAL("Bad status %d to RXC cmd %d", (int) cmd_ptr->hdr.status,
...
)
{
  ...
  if( cmd_ptr->hdr.status != TXC_DONE_S )
  {
    ...
    {
      ONEX_ERR_FATAL( "Bad status %d to TXC cmd %d", (int) cmd_ptr->hdr.status,
...
)
{
  ...
  {
    ONEX_ERR_FATAL( "Bad status %d to RXTX cmd %d", (int) cmd_ptr->hdr.status,
                (int) cmd_ptr->hdr.command,  0 );
  }
} /* mcc_rxtx_cmd */
...
)
{
  ...
  (void) rex_clr_sigs( MC_TCB_PTR,  MCC_CMD_SIG );
  nvi_ext.nvcmd = cmd_ptr;
  nvi_ext.context = mcc_asid;
  /* call the NV interface function to queue the command */
  nv_cmd_ext(&nvi_ext);

  M1X_MSG( DCP, LEGACY_MED,
           "1x_ASID:%d, mc->nv cmd:%d item:%d",
           mcc_asid,
           cmd_ptr->cmd,
           cmd_ptr->item );
...
void mcc_mmgsdi_read_prl( void )
{
  ...
  {
    ONEX_ERR_FATAL("Memory allocation failed for MMGSDI report", 0, 0, 0);
  }
  #endif /* FEATURE_1X_CP_MEM_OPT */
...
)
{
  ...
  {
    ONEX_ERR_FATAL("Could not allocate memory for splmn_info_ptr", 0, 0, 0);
  }
  ...
  (void) sd_ss_ind_cdma_update_splmn_list( mcc_asid, splmn_info_ptr );
...
)
{
  ...
  /* If device is configured in sniffer mode, then the imsi_s1 and imsi_s2
   * values would have already been derived from imsi_s values entered 
   * through NV */
  if( cdma.config_sniff_mode.sniff_mode_enabled == FALSE )
  {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  ** --------------------------------------------------------------- */
  ...
  {
    /* Device is configured to function as sniffer. Hence modify the 
     * imsi_s values to the one which user has input through NV# 73901.
     * Through the NV, user will input the imsi_s HI and LO values.
     * Deduce the imsi_s1 and imsi_s2 values from the imsi_s value 
     */
    qw_set( cdma.imsi_s, 
            cdma.config_sniff_mode.imsi_s_hi,
            cdma.config_sniff_mode.imsi_s_lo
          );

    /* Extract the imsi_s1 and imsi_s2 from 64 bit imsi_s which user has
     * input through NV #73901
     *
     * imsi_s1 is the lower 24 bits (digits 0 - 5) of imsi_s.
     *
     * imsi_s2 is of length 10 bits. The upper 2 bits is the bits 32 and 33
     * of imsi_s. The rest 8 bits (2 bytes) are the digits 6 and 7 of imsi_s
     */
    cdma.imsi_s1 = cdma.config_sniff_mode.imsi_s_lo & 0x00FFFFFFL ;
    cdma.imsi_s2 = ( cdma.config_sniff_mode.imsi_s_hi << 8 )|
                   ( ( cdma.config_sniff_mode.imsi_s_lo & 0xFF000000L ) >> 24);
    
    M1X_MSG( DCP, LEGACY_MED,
             "Device in sniffer mode. imsi_s_hi = 0x%x, imsi_s_lo = 0x%x. "
             "Derived IMSI values: imsi_s1 = 0x%x, imsi_s2 = 0x%x",
             cdma.config_sniff_mode.imsi_s_hi, 
             cdma.config_sniff_mode.imsi_s_lo,
             cdma.imsi_s1, 
             cdma.imsi_s2 );
...
)
{
  ...
  if (read_ok)
  {
      ...
      if ( status == NV_DONE_S)
      {
        ...
        if (qw_hi(mcc_nv_data.meid) != 0x0UL || qw_lo(mcc_nv_data.meid) != 0x0UL)
        {
          ...
          else
          {
            ...
            ONEX_ERR_FATAL("MEID hash can not be created, phone is non-operative",
                      0, 0, 0);
          }
        }
      ...
      if(cdma.scm != temp_scm)
      {
        ...
        if ( status != NV_DONE_S )
        {
          ...
          {
            ONEX_ERR_FATAL("NV_SCM_I write failed with status:%d", status, 0, 0);
  ...
  ** ------------------------------------------------------------ */
...
#endif /* FEATURE_UIM_RUN_TIME_ENABLE */
  {
    ...
    {
      ONEX_ERR_FATAL("Memeory allocation failed for uim access buffer", 0, 0, 0);
      return FALSE;
    }
#ifdef FEATURE_CSIM
    if( (mcc_mmgsdi_rpt_ptr = modem_mem_alloc
  ...
  M1X_MSG( DCP, LEGACY_MED,
    "DEBUG:Spec_call_end_rsns_supp:%d, Ignore_mt_pages:%d",
    cdma.specific_call_end_stats_supported,
    cdma.ignore_mt_pages );

  M1X_MSG( DCP, LEGACY_MED,
    "DEBUG:Device in sniffer mode: %d",
    cdma.config_sniff_mode.sniff_mode_enabled );
...
#ifdef FEATURE_1X_SPLIT_OVERHEAD
  M1X_MSG( DCP, LEGACY_MED,
    "DEBUG: split_ovhd_supported NV: %d",
    mcc_is_split_ovhds_for_g_to_1x_sup() );
#endif /* FEATURE_1X_SPLIT_OVERHEAD */
  M1X_MSG( DCP, LEGACY_MED,
  ...
  M1X_MSG( DCP, LEGACY_MED,
    "DEBUG: Rej voice page for volte call:%d, "
    "use_def_val_for_pwrup_nv_err:%d, pwrup_opt_enabled:%d",
    cdma.rej_voice_page_for_volte,
    cdma.use_def_val_for_pwrup_nv_err,
    cdma.pwrup_opt_enabled );
  ...
  if( read_ok )
  {
     ...
     */
    if( onex_efs_read( "nv/item_files/modem/uim/gstk/bip_otasp_concurrency",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  rex_def_timer_ex( &mcc_hdm_tx_ack_timer, mcc_expired_timer_callback_func, MCC_HDM_TX_ACK_TIMER_ID );

  rex_def_timer_ex( &mcc_wtr_hop_buf_timer, mcc_expired_timer_callback_func, MCC_WTR_HOP_BUF_TIMER_ID );  

  rex_def_timer_ex( &mcc_pri_pch_hash_timer, 
...
)
{
  ...
  boolean sleep_triggered = FALSE;
  boolean sleep_comp_rpt_rcvd = FALSE;
#endif /* FEATURE_MODEM_1X_QTA */
  ...
  {
#ifdef FEATURE_MODEM_1X_QTA
    /* If SRCH is in QTA, then start a 500ms timer and wait for
     * SRCH_SLEEP_COMPLETED_R. Proceed with the legacy code once
     * the report is received.
     * If SRCH is not in QTA, then proceed with legacy code.
     */
    if( srch_idle_ta_is_qta_enabled() )
    {
      /* If the CDMA_EXIT is because of Reacq failure, then send SLEEP_F with
       * reason as FSLEEP_QTA so that SRCH will be out of QTA for sure before 
       * sending CDMA_F. Otherwise it can happen that CP will wait for 
       * SLEEP_COMPLETED_R which will never come. */

      word rafail_retry_cnt = mcc_get_rafail_retry_cnt();
         /* Temp variable holding the rafail retry count left */

      if( rafail_retry_cnt <= 0 )
      {       
        M1X_MSG( DCP, LEGACY_MED,
          "Reacq failed. Send SLEEP_F with rsn FSLEEP_QTA. retry_cnt = %d",
          rafail_retry_cnt );

        /* Send SLEEP_F with reason as QTA */
        mccidl_trigger_sleep( SRCH_FSLEEP_QTA );
        sleep_triggered = TRUE;
      }

      /* Clear all the stale reports in mcc_srch_q and wait for
       * SLEEP_COMP_R if SRCH is in QTA.
       */
      while ((rpt_ptr = (mccsrch_rpt_type*) q_get(&mcc_srch_q)) != NULL)
      {
        if( rpt_ptr->hdr.rpt == SRCH_RELEASE_SYSTEM_R )
        {
          M1X_MSG( DCP, LEGACY_MED,
                 "Rxed SRCH_RELEASE_SYSTEM_R rpt while exiting CDMA" );
    
          if( sleep_triggered == FALSE )
          {
            M1X_MSG( DCP, LEGACY_MED,
                 "Trigger Sleep, fsleep_rsn:%d ",
                 rpt_ptr->sys_rel.fsleep_rsn );

            mccidl_trigger_sleep(rpt_ptr->sys_rel.fsleep_rsn);
            sleep_triggered = TRUE;          		  
          }
          else
          {
            M1X_MSG( DCP, LEGACY_MED,
              "Ignored SRCH_RELEASE_SYSTEM_R as SLEEP_F was already sent");			   
          }
        } /* if( rpt_ptr->hdr.rpt == SRCH_RELEASE_SYSTEM_R ) */

        else if( rpt_ptr->hdr.rpt == SRCH_RAFAIL_R )
        {
          /* Send forced sleep with reason as SRCH_FSLEEP_QTA */
          M1X_MSG( DCP, LEGACY_MED,
                 "Rxed SRCH_RAFAIL_R rpt while exiting CDMA." );

          if( sleep_triggered == FALSE )
          {       
            M1X_MSG( DCP, LEGACY_MED,
              "Send SLEEP_F with rsn FSLEEP_QTA.");
          ...
          {
            M1X_MSG( DCP, LEGACY_MED,
              "Ignored SRCH_RAFAIL_R as SLEEP_F was already sent");
          }
        }/* SRCH_RAFAIL_R */
        else if (rpt_ptr->hdr.rpt == SRCH_SLEEP_COMPLETED_R)
	       {
           M1X_MSG( DCP, LEGACY_MED,
                   "Received SLEEP_COMPLETED_R from srch");
            sleep_comp_rpt_rcvd = TRUE;
	       }
        else
        {
          M1X_MSG( DCP, LEGACY_MED,
        ...
        mcc_enqueue_dbg_srch_cmd( rpt_ptr->hdr.rpt, CDMA_EXIT, CDMA_EXIT, 
                                  MCC_NON_SLEEP, CDMA_EXIT_API );      
      ...
      if(!sleep_comp_rpt_rcvd)
      {
        /* Now all the pending SRCH reports are processed/ignored.
         * Start the 500 ms timer now */
        (void) mcc_clr_callback_timer(&mcc_tr_request_timer,
                                      MCC_TR_REQUEST_TIMER_ID);
        (void)mcc_set_callback_timer(&mcc_tr_request_timer,
                                     MC_WAIT_FOR_SLEEP_COMP_R_BUFFER_TIME,
                                     MCC_TR_REQUEST_TIMER_ID);
        M1X_MSG( DCP, LEGACY_MED,
          "Blocked wait for %dms for SLEEP_COMP_R during exit processing.",
          MC_WAIT_FOR_SLEEP_COMP_R_BUFFER_TIME);
        ...
        while(MCC_BLOCKED_WAITING == cont_blocked_wait_srch)
        {
          ...
          {
            if ( rpt_ptr->hdr.rpt == SRCH_SLEEP_COMPLETED_R )
            {
               cont_blocked_wait_srch = MCC_BLOCKED_WAIT_SUCCESS;
               M1X_MSG( DCP, LEGACY_MED,
                 "Rxed SLEEP_COMP_R from SRCH.");
               /* clear the timer */
               (void) mcc_clr_callback_timer(&mcc_tr_request_timer,
                                             MCC_TR_REQUEST_TIMER_ID);
            }
            else if( rpt_ptr->hdr.rpt == SRCH_RELEASE_SYSTEM_R )
            {
              M1X_MSG( DCP, LEGACY_MED,
               "Rxed SRCH_RELEASE_SYSTEM_R rpt while waiting for SLEEP_COMP_R");

              /* check if SLEEP was already triggered while we processed the
               * RELEASE_SYSTEM_R earlier while cleaning up the SRCH rpts. 
               * If so, we shouldn't send the SLEEP_F again. 
               */
              if( sleep_triggered == FALSE )
              {               
                M1X_MSG( DCP, LEGACY_MED,
            ...
            {
              M1X_MSG( DCP, LEGACY_MED,
                 "Rxed SRCH_RAFAIL_R rpt while waiting for SLEEP_COMP_R");
            ...
            {
              M1X_MSG( DCP, LEGACY_MED,
               "Ignoring SRCH RPT %d rxed while waiting for SLEEP_COMP_R.",
               rpt_ptr->hdr.rpt);
            }
            /* Keep track of the SRCH report for debugging purpose */
           mcc_enqueue_dbg_srch_cmd( rpt_ptr->hdr.rpt, CDMA_EXIT, CDMA_EXIT,
               ...
               )
        {
          ...
          (void) mcc_clr_callback_timer(&mcc_tr_request_timer,
                                    MCC_TR_REQUEST_TIMER_ID);
            ONEX_ERR_FATAL("MC didn't get SLEEP_COMP_R rpt from SRCH in state %d",
                      cdma.curr_state, 0, 0);
        }
       } /* while(MCC_BLOCKED_WAITING == cont_blocked_wait_srch) */
     } /*  if(!sleep_completed)*/
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /* Clear the srch queue only if 1X is not in QTA. If so, then don't clear
   * the queue here. It shall be cleared from API cdma_exit(). This will
   * help to handle the following race scenario.
   *
   * Consider 1X is moving to EXIT state due to PROT_GEN/PROT_DEACT cmd
   * while 1X is still in QTA. If a RAFAIL_R is received here, then 1X.CP
   * will not send any SLEEP_F cmd to SRCH as that report will be cleared
   * here. Thus SRCH will not send SLEEP_COMPLETED_R resulting in crash in MC 
  */
  if( !srch_idle_ta_is_qta_enabled() )
  {
    while ( (srch_ptr = (mccsrch_rpt_type*) q_get( &mcc_srch_q )) != NULL )
    {
      /* Process SRCH_RELEASE_SYSTEM_R. Otherwise CP may end up waiting for the
       * SLEEP_COMPLETED_R inside the API cdma_exit(). If this report was not
       * processed, the SLEEP_COMPLETED_R will never come and the device will 
       * crash. */
      if( srch_ptr->hdr.rpt == SRCH_RELEASE_SYSTEM_R )
      {
        M1X_MSG( DCP, LEGACY_MED,
                 "Rxed SRCH_RELEASE_SYSTEM_R while in CDMA_EXIT_INIT." );
  
        M1X_MSG( DCP, LEGACY_MED,
                 "Trigger Sleep, fsleep_rsn:%d ",
                  srch_ptr->sys_rel.fsleep_rsn );

        mccidl_trigger_sleep(srch_ptr->sys_rel.fsleep_rsn);
      }
  ...
  {
    /* Note that SRCH queue will be cleared at CDMA_EXIT API */
    M1X_MSG( DCP, LEGACY_MED,
               "QTA is enabled. Skip clearing SRCH queue." );
  }
  ...
  (void) mcc_clr_callback_timer ( &mcc_hdm_tx_ack_timer, MCC_HDM_TX_ACK_TIMER_ID );

  (void) mcc_clr_callback_timer ( &mcc_wtr_hop_buf_timer, MCC_WTR_HOP_BUF_TIMER_ID );
  ...
  /* Clear Reg Throttling Timer and Reset Reg Throttling Timer on flag  */
  (void) timer_clr( &mccreg_throt_timer, T_NONE );
  mccreg_vars.reg_throt_timer_on = FALSE;
      ...
      if ((cmd_ptr = (mc_msg_type*) q_get( &mc_cmd_q )) != NULL)
      {
        ...
        switch (cmd_ptr->hdr.cmd)
        {
          ...
          case MC_UE_MODE_CHGD_CMD_F:
          {
            ...
            M1X_MSG( DCP, LEGACY_MED,
             "UE Mode Cnf, Mode:%d, Substate:%d, Trans_id:%d, 1x_ASID:%d",
  ...
  */
  ...
  while (MAIN_STATE( cdma.curr_state ) != CDMA_EXIT)
  {
    ...
    switch (MAIN_STATE( cdma.curr_state ))
    {
      ...
      default:
      {
        ONEX_ERR_FATAL( "BAD CDMA state %d", cdma.curr_state, 0, 0 );
        break;
      } /* default */

    } /* switch */

    /* If the device is configured in sniffer mode, then it shouldn't do any
     * communication with BS. Hence it is required that the device should 
     * not move to either ACCESS or TRAFFIC state.
     */
    if( ( ( MAIN_STATE( cdma.curr_state ) == CDMA_SYS_ACC )||
          ( MAIN_STATE( cdma.curr_state ) == CDMA_TC )
        )&&
        ( cdma.config_sniff_mode.sniff_mode_enabled == TRUE )
      )
    {      
      M1X_MSG( DCP, LEGACY_HIGH,
         "Device configured in sniffer mode. Moving back to CDMA_IDLE");
      cdma.curr_state = CDMA_IDLE;
    }
    ...
    if (MAIN_STATE( cdma.curr_state ) == CDMA_EXIT)
    {
      ...
      ** --------------------------------------- */
      ...
      #ifdef FEATURE_MODEM_1X_IRAT_LTO1X
      /* Reset the waiting for csfb_call_clear_f flag */
      csfb_cdma.waiting_for_call_clear = FALSE;  
      ...
      if(cdma_entry.entry == MCC_CSFB)
      {
        ...
        if(csfb_cdma.csfb_1x_transition == TRUE)
        {
          ...
          mc_msgr_send_msg( &ho_fail_rsp.hdr,
                            ...
                            sizeof(ho_fail_rsp) );
          mcc_enqueue_dbg_lte_rrc_cnf( MCC_SUBTASK,
                                       MC_ONEX_CP_LTE_1xCSFB_HO_FAILED_RSP );
...
)
{
  ...
  /* Pass the active subscription id */
  cmd_ptr->hdr.asubs_id = mcc_asid; 
  ...
  {
    (void) sd_ss_ind_misc_orig_success( mcc_asid, NULL );
  }
...
)
{
         ...
         )
  {
    ...
    {
          if ( cnf_ptr->session_open_ext_cnf.session_info_ptr[i].session_type == 
                        MMGSDI_1X_PROV_PRI_SESSION )
      {
            cdma.pri_sub_session_id = cnf_ptr->session_open_ext_cnf.session_info_ptr[i].session_id;
        M1X_MSG( DCP, LEGACY_MED,
                     "mcmmgsdi_session_reg_cb(): CDMA Session Id for pri sub = %d",
                     cdma.pri_sub_session_id );
      }
          else if ( cnf_ptr->session_open_ext_cnf.session_info_ptr[i].session_type == 
                             MMGSDI_1X_PROV_SEC_SESSION )
          {
            cdma.sec_sub_session_id = cnf_ptr->session_open_ext_cnf.session_info_ptr[i].session_id;
            M1X_MSG( DCP, LEGACY_MED,
                     "mcmmgsdi_session_reg_cb(): CDMA Session Id for sec sub = %d",
                     cdma.sec_sub_session_id );
    }
        } /* for( i=0; ... */
  }
  else
  {
    M1X_MSG( DCP, LEGACY_ERROR,
          "mcmmgsdi_session_reg_cb():SessionID Reg Failed. status = %d, client_id = %d",
          mmgsdi_status, cdma.client_id );
    cdma.client_id = MC_INVALID_CDMA_CLIENT_ID;
        cdma.pri_sub_session_id = MC_INVALID_CDMA_SESSION_ID;
        cdma.sec_sub_session_id = MC_INVALID_CDMA_SESSION_ID;
  }
    } /* MMGSDI_SESSION_OPEN_EXT_CNF */
  ...
  )
{
  ...
  if (mmgsdi_status != MMGSDI_SUCCESS ||
      ...
      cnf_ptr == NULL)
  {
    ONEX_ERR_FATAL("MMGSDI failed registering MC. Status %d, Confirmation %d",
   ...
   mcc_mmgsdi_rpt_status = MMGSDI_SUCCESS;
    if( mcc_mmgsdi_rpt_ptr != NULL)
    {
        ...
        )
      {
         ...
         {
           ONEX_ERR_FATAL( "Memory allocation failed", 0, 0,0);
         }
...
)
{
  ...
  ** If 1x is activated, ignore the NV refresh*/
  ...
  if(mc_cmd_ptr == NULL)
  {
    ONEX_ERR_FATAL("Can't get MC Buffer",0,0,0);
  }
  else
  {
...
boolean mcc_is_1x_to_l_meas_allowed(void)
{
  ...
  ---------------------------------------------------------------------------*/
  rat_pri_list = sd_misc_get_main_rat_priority_list( mcc_asid );
...
)
{
  ...
  switch(cmd_id)
  {
    ...
    case ONEX_VOC_OPEN_CMD:
    {
      ...
      M1X_MSG( DCP, LEGACY_MED,
               "Received OPEN cmd from Voice Adapter with 1x_ASID %d",
               adapter_params->asid ); 
      ...
      for(i=0; i<NUM_ASID; i++)
      {
        if (vadapter_params_map[i].asid == SYS_MODEM_AS_ID_NONE)
        {
          ...
          M1X_MSG( DCP, LEGACY_HIGH,
                   "Voice Adapter params stored for 1x_ASID %d at index %d",
             ...
             )
          {
            ...
            M1X_MSG( DCP, LEGACY_HIGH,
                     "Voice Adapter params set for current 1x_ASID %d",
        ...
        {
          M1X_MSG( DCP, LEGACY_HIGH,
              "1x_ASID %d params already registered by Voice Adapter",
              adapter_params->asid );          
        } /* else if (vadapter_params_map[i].asid == adapter_params->asid) */
      }/* for(i=0; i<NUM_ASID; i++) */   
...
FUNCTION MCC_GET_VFR

DESCRIPTION

  This API returns the vfr value which was updated by MMODE through the API
  mcc_set_asid_vfr_info()
...
FUNCTION MCC_SET_CFS_RETURN_TRANS_DESPATCHED

DESCRIPTION

  This API sets the global variable mcc_cfs_return_trans_despatched.
  This API is meant to be invoked by 1X-CP only.
...
===========================================================================*/
void mcc_set_cfs_return_trans_despatched
(  
  boolean status
)
{
  mcc_cfs_return_trans_despatched = status ;
} /* mcc_set_cfs_return_trans_despatched */

/*===========================================================================

FUNCTION MCC_UPDATE_RUDE_WAKEUP_WITH_TX_REQ_DONE

DESCRIPTION
   This function will set the is_rude_wakeup_with_tx_req_done to the passed
   flag. This flag is used to keep track if a rude wakeup has been 
   performed with tx_resources_required set to TRUE. If it is TRUE, UE need
   not perform another sleep-rudewakeup cycle while requesting for transceiver
   resources. 
...
void mcc_update_rude_wakeup_with_tx_req_done( boolean flag )
{
  cdma.is_rude_wakeup_with_tx_req_done = flag;

  M1X_MSG( DCP, LEGACY_MED,
    "Updated cdma.is_rude_wakeup_with_tx_req_done = %d",
    cdma.is_rude_wakeup_with_tx_req_done );
}

/*===========================================================================

FUNCTION MCC_GET_CFS_RETURN_TRANS_DESPATCHED

DESCRIPTION

  This API returns the global variable mcc_cfs_return_trans_despatched.
  This API is meant to be invoked by 1X-CP only.
...
RETURN VALUE
  mcc_cfs_return_trans_despatched

SIDE EFFECTS
  None

===========================================================================*/
boolean mcc_get_cfs_return_trans_despatched
(  
  void
)
{
  M1X_MSG( DCP, LEGACY_MED,
      "returned mcc_cfs_return_trans_despatched = %d",
      mcc_cfs_return_trans_despatched);
  return mcc_cfs_return_trans_despatched;
} /* mcc_get_cfs_return_trans_despatched */
...
)
{
  ...
  /* The VSTMR_ID is mapped to the VFR as follows:
   * SYS_VFR_PRIMARY   -> VSTMR_RTC_1X_SUB0
   * SYS_VFR_SECONDARY -> VSTMR_RTC_1X_SUB1
   */
  switch ( mcc_vfr )
  {
    case SYS_VFR_PRIMARY:
    {
      /* this is corresponding to sub 0 */
      M1X_MSG( DCP, LEGACY_MED, 
               "Returning VSTMR_RTC_1X_SUB0 for 1x_ASID %d, VFR: SYS_VFR_PRIMARY",
               mcc_asid );
      return VSTMR_RTC_1X_SUB0;
    }
    case SYS_VFR_SECONDARY:
    {
      /* this is corresponding to sub 1 */
      M1X_MSG( DCP, LEGACY_MED, 
               "Returning VSTMR_RTC_1X_SUB1 for 1x_ASID %d, VFR: SYS_VFR_SECONDARY", 
               mcc_asid );
      return VSTMR_RTC_1X_SUB1;
    }
    default:
    {
      ONEX_ERR_FATAL( "VFR : %d, CM hasn't updated 1X with correct VFR",
                  mcc_vfr, 0, 0 );
    }
  }
} /* mcc_get_vstmr_id */
...
FUNCTION MCC_SET_ASID_VFR_INFO

DESCRIPTION
  This API will be called in MultiMode context to set ASID value and VFR
...
)
{
  ...
  {
    M1X_MSG( DCP, LEGACY_ERROR, 
             "Unexpected values for 1x_ASID %d, VFR %d",
             asid, vfr );
    return;
  }

  M1X_MSG( DCP, LEGACY_MED, 
            "Updated 1x_ASID %d to %d and VFR %d to %d."
            "pri_sub_session_id = %d, sec_sub_session_id = %d",
           mcc_asid, asid, 
           mcc_vfr, vfr,
           cdma.pri_sub_session_id,
           cdma.sec_sub_session_id);
...
if(asid != mcc_asid )
  {
    ...
    /* There has been a subscription change. De-register the messages with
     * the old AS_ID. Once it is completed, register with the new AS_ID.
     * Note that the global variable mcc_asid is updated only after the
     * deregistering is completed so that the proper AS_ID values will be
     * used by either APIs. 
     */

    mc_msgr_deregister_msgs();

    /* De-registering has been done. Now update the global variable with
     * the new AS_ID and re-register */
    mcc_asid = asid;
    mc_msgr_register_msgs();

    M1X_MSG( DCP, LEGACY_MED,
      "CSFB DEBUG: MSGR re-registering completed with 1x_ASID = %d",
      mcc_asid );    
#else /* !FEATURE_MODEM_1X_IRAT_LTO1X */

    /* There has been a change in the asid. Update the global variable 
     * to the new asid. */
    mcc_asid = asid;
  ...
  /* Update the VFR global variable */
  mcc_vfr = vfr;
  ...
  if (vadapter_params.asid != asid)
  {
    ...
    for(i=0; i<NUM_ASID; i++)
    {
      if (vadapter_params_map[i].asid == asid)
      {
        vadapter_params = vadapter_params_map[i];
        M1X_MSG( DCP, LEGACY_MED, 
                 "Updated Voice Adapter parameters for 1x_ASID %d",
                 asid);        
        break;
      }
    }    
...
FUNCTION MCC_IS_CDMA_CALL_IN_PROG

DESCRIPTION
  This API will retutn TRUE if 1X is having either MO or MT call in progress.
  Depending on the state where this API is called, it checks the respective
  varibales to determine if the call is in progress.

  This API is exposed to other tasks. Currently it's meant to be called by HDR

DEPENDENCIES
  None.

RETURN VALUE
  TRUE  - if MO/MT call is in progress
  FALSE - Otherwise.
...
===========================================================================*/
boolean mcc_is_cdma_call_in_prog
( 
  sys_modem_as_id_e_type asid
)
{
  /* Check if the call activity check is done on active sub */
  if ( asid != mcc_asid )
  {
    ONEX_ERR_FATAL("Call_in_prog check for wrong sub. mcc_asid %d, asid = %d",
              mcc_asid, asid, 0);
  }
  boolean call_in_progress = FALSE;
    /* boolean indicating if the call is in progress */
  ...
  switch( MAIN_STATE(cdma.curr_state) )
  {
    ...
    case CDMA_SYS_ACC:
    {
      if( ( cdma.ho_sa_tc_state == CDMA_ORIG )||
  ...
  M1X_MSG( DCP, LEGACY_MED, 
           "Returning call_in_progress = %d, state = 0x%x, call_orig = %d, "
           "is_srlte_incoming_call = %d, ho_sa_tc_state = 0x%x ",
           call_in_progress,
           cdma.curr_state,
           cdma.call_orig,
...
} /* mcc_is_cdma_call_in_prog */

/*===========================================================================

FUNCTION MC_IS_CDMA_ACTIVE

DESCRIPTION
  This API checks whether 1X is in active state or not and return the status.
  The API returns 

  TRUE  - if state other than CDMA_EXIT and CDMA_ENTER.
  FALSE - otheriwise.
...
===========================================================================*/
boolean mc_is_cdma_active( void )
{

  /* The following global variables will help to identify when the 
   * AUTO_DEACT_IND has been sent. Note that there is no instance 
   * where auto_deact_ind will be sent while 1X is in CDMA_ENTER */

  M1X_MSG( DCP, LEGACY_MED, 
           "DEBUG: cdma.curr_state = 0x%x, cdma_entry.entry = %d"
#ifdef FEATURE_MODEM_1X_IRAT_LTO1X
           ", csfb_cdma.csfb_1x_transition = %d, mcc_csfb_exit.exit = %d, "
           "mc_redir_failed_ind.cause = %d"
#endif /* FEATURE_MODEM_1X_IRAT_LTO1X */
           , cdma.curr_state 
           , cdma_entry.entry
#ifdef FEATURE_MODEM_1X_IRAT_LTO1X
           , csfb_cdma.csfb_1x_transition
           , mcc_csfb_exit.exit
           , mc_redir_failed_ind.cause
#endif /* FEATURE_MODEM_1X_IRAT_LTO1X */
         );
...
FUNCTION MC_IS_CDMA_IN_RF_WAIT_STATE

DESCRIPTION
  This API checks whether 1X is in waiting for RF Resource state or not.

DEPENDENCIES
  None.

RETURN VALUE
  TRUE  - if 1x is waiting for RF Resource.
  FALSE - otheriwise.
...
===========================================================================*/
boolean mc_is_cdma_in_rf_wait_state
( 
  void 
)
{
  M1X_MSG( DCP, LEGACY_MED, 
           "Returning waiting_for_rf_res:%d",
           cdma.waiting_for_rf_res );
  return cdma.waiting_for_rf_res;
} /* mc_is_cdma_in_rf_wait_state() */
