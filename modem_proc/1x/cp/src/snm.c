...
$Header: //components/rel/1x.mpss/8.0/cp/src/snm.c#2 $ $DateTime: 2020/01/27 00:26:21 $ $Author: pwbldsvc $
  ...
  if (snp.total_so < MAX_SUPPORTED_SO)
  {
    ...
    for (n = 0; n < snp.total_so; n++)
    {
      ...
      if (snp.so[n].service_option_num == so_number)
      {
        ...
        /* We might call this API twice for SO-3, to update SO-3 Alt SO List
         * with newly configured Customer Voice SO as part of SIM Swap 
         * scenario without Reboot case.
         * If that is the case then update SO-3 neg_par with latest one */
        if ( so_number == CAI_SO_VOICE_EVRC )
        {
          snp.so[n].negotiation_pars = *neg_par;
          status = SNM_SUCCESS;

          M1X_MSG( DCP, LEGACY_ERROR,
                   "Updated Neg_Params for SO:%d",
                   so_number);
        }
        else
        ...
        break;
      } /* if(service_option_num == so_number) */
    } /* for() */
      ...
      if (P_REV_IS_7_OR_GREATER)
      {
        if (snp.so[so_index].qos_blob_generator)
        {
               ...
               snp.so[so_index].qos_blob_generator(cfg_ptr->con[0].qos_parms)) > 0)
          {
            ...
            {
              ONEX_ERR_FATAL("Data corruption by QOS generator, len %d",
      ...
      /* we have function to evaluate it */
  {
    ...
    else
    {
      ...
      if (!suggest_alt && (qos_eval_status == SNM_ALT_CFG))
      {
        ...
        ONEX_ERR_FATAL("Configuration may have been corrupted", 0, 0, 0);
      }
...
)
{
  ...
  {
    /* Should never happen */
    ONEX_ERR_FATAL("Null cfg ptr", 0, 0, 0);
  }
  ...
  if ((suggest_alt) && (cfg_ptr != NULL))
  {
    ...
    {
      /* Should never happen */
      ONEX_ERR_FATAL("Null alt cfg ptr", 0, 0, 0);
    }
    else
    {
...
#define LPM_BOUND_CHECK(num_lpm_entries)                                      \
   ...
   {                                                                          \
     ...
     ONEX_ERR_FATAL("Default LPM exceeding storage limit %d", num_lpm_entries,0,0);\
   }
     ...
     )
  {
    ...
    ONEX_ERR_FATAL("For prev <= 6, default LPM can't have more than one service", 0,0,0);
  }
...
//  }
  ...
  {
    ONEX_ERR_FATAL("Log exceeds buffer", 0, 0, 0);
  }
...
===========================================================================*/
QSH_MDUMP_FN_ATTR
void mc_qsh_mdump_collect_snm_data()
{
  qsh_mdump_collect_high(&negotiation_info,sizeof(negotiation_info));
  qsh_mdump_collect_high(&local_srv_cfg,sizeof(local_srv_cfg));
}
