...
$Header: //components/rel/1x.mpss/8.0/cp/src/mccgcsna.c#2 $ $DateTime: 2020/01/27 00:26:21 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
01/22/20   vaa     Implement QSH MDUMP framework
09/14/18   ppr     Array bounnd check and remove calls to REL-A, B, C API’s 
08/02/17   sjo     FR 37336: Migration to MSGR VARIANT from Inst ID
08/29/16   sjo     Created new debug buffer for lte rrc responses
08/06/15   sjo     Fixed Klocwork issues.
...
/* Get the original Transaction ID */
   ...
   */
      ...
      {
        ONEX_ERR_FATAL("Unknow UL TXSSION MSG",0,0,0);
      }
...
void mcc_gcsna_retry(void)
{
  ...
  if(gcsna_msg_sent.num_remaining_retries > 0)
  {
    if( gcsna_msg_sent.is_msg_tx_rsp_rxed )
    {
      ...
      msgr_init_hdr_variant( &(gcsna_msg_sent.msg_tx.msg_hdr), 
                             MSGR_ONEX_CP, 
      ...
      M1X_MSG( DCP, LEGACY_HIGH,
               "DEBUG: sent msg to NAS with 1x_ASID=%d",
               asid );
    }
    else
    {
...
)
{
    ...
    else
    {
      ...
      switch (rec_type)
      {
        case CAI_DISPLAY_REC:
        {
          ...
          /* Range Check */
          if( (((caii_display_rec_type *) msg_ptr)->fix.num_char == 0 )||
              (((caii_display_rec_type *) msg_ptr)->fix.num_char > 
              (CAII_DISPLAY_REC_MAX)) )
          {
            M1X_MSG( DCP, LEGACY_ERROR,
                     "Unsupported num_char = %d",
                     ((caii_display_rec_type *) msg_ptr)->fix.num_char );
        ...
        case CAI_CALLED_REC:
        {
          ...
          /* Range Check */
          if( rec_len < sizeof(cai_called_fix_type) )
          {
            M1X_MSG( DCP, LEGACY_ERROR,
                     "rec_len = %d < Fixed part of Called Party Number fixed rec",
                     rec_len );
            status = CAIX_INV_LEN_S;
            return (status);
          }
          ...
          /* Range Check */
          if( ((caii_called_rec_type *) msg_ptr)->fix.num_char > 
              (CAII_CALLED_REC_MAX) )
          {
            M1X_MSG( DCP, LEGACY_ERROR,
                     "num_char = %d exceeding CAII_CALLED_REC_MAX %d",
                     ((caii_called_rec_type *) msg_ptr)->fix.num_char,
                     CAII_CALLED_REC_MAX );
            status = CAIX_INV_MSG_S;
            return (status);
           ...
           * ---------------------------------------------------------- */
          if ( ((caii_called_rec_type *) msg_ptr)->fix.num_char == 0 )
          {
            msg_ptr += FENDPOS(caii_called_rec_type,fix);
          }
          else
          {
        ...
        case CAI_CONNECT_REC:
        {
          ...
          /* Range Check */
          if( rec_len < sizeof(cai_calling_fix_type) )
          {
            M1X_MSG( DCP, LEGACY_ERROR,
                     "rec_len = %d < Fixed Calling Party Num fix rec part",
                     rec_len );
            status = CAIX_INV_LEN_S;
            return (status);
          }
          ...
          /* Range Check */
          if( ((caii_calling_rec_type *) msg_ptr)->fix.num_char > 
              (CAII_CALLING_REC_MAX) )
          {
            M1X_MSG( DCP, LEGACY_ERROR,
                     "num_char = %d exceeding CAII_CALLING_REC_MAX %d",
                     ((caii_calling_rec_type *) msg_ptr)->fix.num_char,
                     CAII_CALLING_REC_MAX );
            status = CAIX_INV_MSG_S;
            return (status);
           ...
           * ------------------------------- */
          if ( ((caii_calling_rec_type *) msg_ptr)->fix.num_char == 0 )
          {
            msg_ptr += FENDPOS(caii_calling_rec_type,fix);
          }
          else
          {
        ...
        case CAI_CONNECT_SUB_REC:
        {
           ...
           * Increment internal message pointer to next record
           * ------------------------------------------------- */
          if ( ((caii_subaddr_rec_type *) msg_ptr)->fix.num_char == 0 )
          {
            msg_ptr += FENDPOS(caii_subaddr_rec_type,fix);
          }
          else
          {
        ...
        case CAI_REDIR_NUM_REC:
        {
           ...
           * Increment internal message pointer to next record
           * ------------------------------------------------- */
          if ( ((caii_redir_rec_type *) msg_ptr)->fix.num_char == 0 )
          {
            msg_ptr += FENDPOS(caii_redir_rec_type,fix);
          }
          else
          {
        ...
        case CAI_PARA_ALERT_REC:
  {
        ...
        default:
        {
          ...
          M1X_MSG( DCP, LEGACY_ERROR,
                   "Invalid Rec_Type:%d, Prev_in_use:%d, num_recs:%d, Status:%d",
                   rec_type,
                   cdma.p_rev_in_use,
                   int_ptr->alert.num_recs,
                   status );
        ...
        )
     {
       ...
       { /* CSFB Mode */
         ...
         mc_msgr_send_msg(&ho_fail_rsp.hdr,
                          ...
                          sizeof(ho_fail_rsp));
         mcc_enqueue_dbg_lte_rrc_cnf( MCC_GCSNA_XLATE_L3_MSG,
