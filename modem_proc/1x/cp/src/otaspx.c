...
INITIALIZATION AND SEQUENCING REQUIREMENTS
  ...
  Copyright (c) 1996 - 2018 Qualcomm Technologies, Inc. 
...
$Header: //components/rel/1x.mpss/8.0/cp/src/otaspx.c#1 $ $DateTime: 2019/06/11 23:51:40 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
12/03/18   pga     Check for overflow in xlate_ext_key_gen_req
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if( OTASPI_PARAM_P >= int_ptr->key_gen_req.bs_result_len )
  {
  ...
  {
    status = OTASPX_INV_LEN_S;
    M1X_MSG( DCP, LEGACY_ERROR,
             "Unsupported bs_result_len: %d > OTASPI_PARAM_P: %d",
             int_ptr->key_gen_req.bs_result_len,
             OTASPI_PARAM_P );
    return (status);
  }
