...
$Header: //components/rel/1x.mpss/8.0/cp/src/db.c#2 $ $DateTime: 2020/01/27 00:26:21 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
01/22/20   vaa     Implement QSH MDUMP framework
11/17/17   ppr     Mainline GPP2_SUPPORT_ON_SEC_SUB feature
08/31/17   sjo     Chgs to update the db data regardless of the sub id passed
...
)
{
  switch(item) {
    ...
    default:
      ONEX_ERR_FATAL("Bad item parameter %d ", (int) item, 0,0);
...
DESCRIPTION
  Changes the value of the item specified. The tasks that need to know that
  the value of the item has changed are notified. This API is called by the 
  mmode.
...
(
  db_items_type        item,        /* Item to be changed                  */
  db_items_value_type  *value_ptr,   /* Value to change the item to         */
  sys_modem_as_id_e_type asubs_id   /* Active Subscription id              */

)
{
  if(asubs_id == mcc_get_asid())
  {
    db_put(item, value_ptr); 
  }
  else
  {
    M1X_MSG( DCP, LEGACY_ERROR,
             "Passed asubs_id = %d mismatch with current mcc_asid = %d",
             asubs_id, mcc_get_asid());
  }
} /* mcc_db_put */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  ** ---------------------------------------------------------- */
  ...
  db_dsds_status = FALSE;
      ...
      default:
        ONEX_ERR_FATAL("Bad item parameter %d ",index,0,0);
