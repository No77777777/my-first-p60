...
$Header: //components/rel/1x.mpss/8.0/cp/inc/mc_i.h#1 $ $DateTime: 2019/06/11 23:51:40 $ $Author: pwbldsvc $
...
11/19/15   agh     Changed limit of MC cmds for cancelling MMGDSI requests
...
06/12/15   eye     Added CSFB check for demod ch0 FW update.
...
extern q_type mc_deferred_tx_cmd_q;
...
/* NV Item - 73901. Fields required to configure device as sniffer */

typedef PACK (struct)
{
  uint8      version;
  boolean    sniff_mode_enabled;
  uint32     imsi_s_hi;
  uint32     imsi_s_lo;
  uint8      PN_count;
  uint16     PN_0;
...
typedef PACK (union)
{
  ...
  ** is optimised */
  ...
  /* NV#73773 - 1xSRLTE MT page access handoff feature supported */
  boolean                            srlte_mt_page_acc_ho_supported;

  /* NV#73870 - Ignore MT pages to know the Page Repetition Pattern */
  boolean                            ignore_mt_pages;

  /* NV#73901 - Fields required to configure device as sniffer */
  mc_config_sniff_type               config_sniff_mode;

  /* NV#74048 - Power-up Optimization Enabled */
...
typedef union
{
  ...
  */
  ...
  /* Indication from MMODE regarding Domain Selection Change */
  cm_domain_sel_domain_chg_ind_msg_type cm_dom_sel_chg_ind;
...
typedef struct
{
  ...
  boolean waiting_for_call_clear;
  /* To track if 1X_CP is waiting for call_clear_f after sending some 
   * failure to CM via L-RRC->NAS->CM route. This variable will only track
   * call_clear_f when the failure was sent in CSFB mode. For CSFB call/handover   
   * failures sent in native 1X this variable will not track if 1X_CP is 
   * waiting for call_clear_f since for those cases 1X_CP would anyway move to 
   * CSFB state and wait for call_clear_f and handle it when received.
   *
   * The aim of tracking call_clear_f through this variable is to handle race 
   * conditions where 1X_CP moves from CSFB mode to native 1X due to n/w 
   * redirection before CM is able to send call_clear_f in CSFB mode. In such 
   * cases 1X_CP needs to handle the call_clear_f rxed in native 1X and
   * clear call context accordingly */

  boolean call_cleared_in_native_1x;
  /* This flag indicates that 1X has already received call clear from CM 
   * in native 1X and there is no need to wait in CSFB_ACCESS_STATE for
   * call_clear_f since it may not come from CM. This flag is set to True
   * when call_clear_f is rxed in native 1X after 1X had already sent HO/call
   * failure to CM in CSFB mode */
}mcc_csfb_ms_type;
...
/* Enumeration of the commands passed through msgr queue */
typedef enum
{
  INVALID_MSGR_CMD,
  ...
  MC_ONEX_CP_LTE_ABORT_1XCSFB_HO_REQ
} msgr_cmd_name_type;

/* Structure holding the details of the command passed through msgr queue */
typedef struct
{
  /*Link */
...
/* 30 */  
  ...
  MCC_CSFB_ACCESS_STATE,
  MCC_CSFB_ACCESS_STATE_1,
  MCC_CSFB_ACCESS_STATE_2,
  MCC_CSFB_ACCESS_STATE_PROCESS,
  MCC_CSFB_ACCESS_STATE_PROCESS_1,
  MCC_CSFB_PROCESS_MO_FROM_EUTRA,
  MCC_CSFB_PROCESS_MO_FROM_EUTRA_1,

/* 40 */
  MCC_CSFB_CDMA_DO_TIME_TRANSFER_API,
  MCC_CSFB_PROCESS_TUNNELED_UHDM,
  MCC_CSFB_PROCESS_TUNNELED_UHDM_1,
  MCC_CSFB_PROCESS_TUNNELED_ECAM,
  MCC_CSFB_PROCESS_ORD_MSG,
  MC_CSFB_PROCESS_GCSNA_REJ,
  MC_CSFB_PROCESS_GCSNA_REJ_1,
  MCC_SUBTASK,
...
/* Information to be enqueued on mc_dbg_msgr_q which will keep track of all
     commands received through msgr queue */
typedef struct
{  
  msgr_cmd_type            cmd;
  word                     prev_state;
...
/* Enumeration of responses sent to LTE RRC */
typedef enum
{
  INVALID_RSP,
...
#define NUM_MAX_LTE_RRC_RSPS 10
/* Total number of responses sent to LTE RRC */
...
/* Defining a global array of structures */
lte_rrc_cnf_dbg_buf_type lte_rrc_cnf_dbg_buf[NUM_MAX_LTE_RRC_RSPS];
...
DESCRIPTION    This function deregisters the messages which 1X is interested
               in getting from message router.
...
===========================================================================*/
extern void mc_msgr_deregister_msgs(void);

/*===========================================================================

FUNCTION       MC_MSGR_REGISTER_MSGS

DESCRIPTION    This function registers the messages which 1X is interested
               in getting from message router.
...
===========================================================================*/
extern void mc_msgr_register_msgs(void);
