...
$Header: //components/rel/1x.mpss/8.0/cp/inc/mcctcho_i.h#1 $ $DateTime: 2019/06/11 23:51:40 $ $Author: pwbldsvc $
...
typedef struct
{
  ...
  boolean hho_wtr_hopping_req;
    /* Indicator whether WTF Hopping is required for Hard Handoff */
    
  boolean cfs_nset_trans_in_prog;
    /* Indicator of whether a MCTRANS_CFS_NSET transaction is in progress.
     * If TRUE then we will ignore the back to back CAI_CF_SRCH_REQ_MSG
     * to avoid Two CFS with WTR Hop required collission and crash in 
     * lower layers */
  ...
  /* Indicates WTR hop in progress for CFS. This flag is set only when 
   * CFS measurements are actually in progress */
...
FUNCTION MCCTCHO_CONFIG_TXC_FOR_CFS_OR_RIF_WITH_WTR_HOPPING
