...
$Header: //components/rel/1x.mpss/8.0/cp/inc/mccrxtx.h#1 $ $DateTime: 2019/06/11 23:51:40 $ $Author: pwbldsvc $
...
typedef enum
{
  ...
  MCCRXTX_HDM_ACK_CNF_F      /* Ack for HDM message has been sent out */
} mccrxtx_name_type;
...
/* Type for MCCRXTX_HDM_ACK_CNF_F command */
typedef struct
{
  mccrxtx_hdr_type hdr;      /* command header */
  byte ack_seq;              /* Ack seq of the Ack sent by TX */
} mccrxtx_hdm_ack_cnf_type;
...
typedef union
{
  ...
  mccrxtx_hdm_ack_cnf_type hdm_ack_cnf;
}mccrxtx_cmd_type;
