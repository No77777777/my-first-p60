...
DESCRIPTION
  ...
  Copyright (c) 1990 - 2018 Qualcomm Technologies, Inc. 
...
$Header: //components/rel/1x.mpss/8.0/cp/inc/mccdma_i.h#1 $ $DateTime: 2019/06/11 23:51:40 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ------------------------------------------------------------
11/16/18   vaa     Added skip_powerup_timer to cdma_ms_type.
05/03/18   ppr     Chgs to wait for ASET cmd response report from SRCH
04/12/18   ppr     Decrease Sleep Comp rpt buffer time to 325-msec
03/23/18   ppr     PCH hash for AHO's with wrong n/w side ngbhr_config=0 nghbrs
...
11/23/16   sjo     Enable 1x tune-away during MMS/SMS in non DDS sub
...
11/10/17   sjo     Added new rude wakeup rsn for DBM, MO/MT call with SO = 6/14
...
08/29/17   sjo     Changes to wait for SLEEP_COMP_R if SRCH is in QTA while
...
04/21/16   ppr     1X2GTA:Chgs to save rcvd ESCAM OTA msg.
...
02/18/16   ppr     1X2GTA:HHO/CFS Priority Inversion changes
...
01/05/16   sjo     1xSRLTE: Chgs to handle deact_cmd while waiting for CM cnf.
...
08/21/15   ppr     Increased MCC_MAX_DTMFS_OUT to 30
...
07/16/15   agh     If no valid OTA decoded in current wakeup cycle in QTA, 
                   next wakeup should be in LTA
...
/* ENUM to define the reason for Rude wakeup to SRCH */
typedef enum
{
  MCC_RSN_NONE,
  MCC_RUDE_WAKEUP_REG_ACC_RSN,
  MCC_RUDE_WAKEUP_SMS_ACC_RSN,   /* It can be an SMS sent over access channel
                                  * or an MO/MT call with SO = 6 or 14. */
  MCC_RUDE_WAKEUP_MO_MT_ACC_RSN,
  MCC_RUDE_WAKEUP_NON_ACC_RSN
}rude_wakeup_rsn_type;
...
typedef enum
{
  ...
  HHO_WTR_HOP_IN_PROG,        /* HHO WTR hopping proc's under execuation */
...
/* Type to store MT page rcvd BS specific info */
typedef struct
{
  word pilot_pn;   
    /* pilot pn offset */
...
typedef struct
{
  ...
  /* To keep track if 1x is in waiting for RF resource state */
  boolean waiting_for_rf_res;
...
/* Set/Reset ONLY by ITSPM, UHDM. Used ONLY in traffic state.
  ...
  ** --------------------------------------------------------------------------- */
  ...
  /* MMGSDI session id for primary asid - SUB0 (SYS_MODEM_AS_ID_1) */
  mmgsdi_session_id_type pri_sub_session_id;

  /* MMGSDI session id for secondary asid - SUB1 (SYS_MODEM_AS_ID_2) */
  mmgsdi_session_id_type sec_sub_session_id;
  ...
  boolean mt_page_in_progress; /* indicates if any MT call is in progress */
  boolean is_page_in_sv_mode;
  boolean is_srlte_incoming_call; /* if there is any SRLTE incoming call */
  
  /* Str to store MT page rcvd BS specific info */
  mt_page_rcvd_info_type mt_page_rcvd_info; 
...
/* Structure to store 1X overheads optimization related global variables */
  ...
  /* Indicates if SRCH should perform the next WAKEUP in QTA. This flag is 
   * only supposed to be used for forced sleep in QTA */
  boolean next_wakeup_in_qta;

  #endif /* FEATURE_1X_SPLIT_OVERHEAD */

  /* Indicates if we are waiting for SPM OTA msg and need to block Sys_Rel
   * from SRCH side after Idle handoff to same_config nghbr. 
   * If the flag is set to TRUE then we will return TRUE in 
   * mcc_get_split_ovhd_block_sys_rel_flag() API to ignore unlock 
   * callback from TRM in SRCH to collect SPM OTA msg. */
  boolean idle_ho_spm_wait_block_sys_rel_flag;

  /* Indicates if we received a valid OTA on PCH after same config nghbr
   * Idle handoff triggered.
   * Based on this flag we will decide whether to start 
   * Primary PCH Hash timer */
  boolean idle_ho_pch_validation_done;
  ...
  /* To keep track of the Rude wakeup reason */
  rude_wakeup_rsn_type rude_wakeup_rsn;

  /* Indicates if posting ASET cmd to SRCH is pending or not */
  boolean aset_cmd_pend_flag;
  
  /* Structure to store 1x Tune Away rel global variables 
   * during 1x data call in trafifc state */
  tc_data_ta_type tc_data_ta;
  
  /* This variable is used exclusively to see if CP had sent SRCH to sleep
   * as a part of processing the SRLTE MT page */
  boolean srlte_is_srch_in_sleep_state;

  /* This variable is used to exit the blocked wait for CM confirmations */
  boolean exit_blocked_wait;
  
  /* This variable is used to keep track if CP had sent a rude wakeup with
   * tx_resources_required field set to TRUE. */
  boolean is_rude_wakeup_with_tx_req_done;
  ...
  /* Flag to indicate if MT pages ignored to check the Page Rep Pattern */
  boolean ignore_mt_pages;
  
  /* Flag to indicate if Power-up optimization is enabled */
  boolean pwrup_opt_enabled;

  /* Var to store the count of ignored MT pages */
  uint32 ignored_mt_page_cnt;

  /* Flag to check if an empty GPM has been received while MC still in 
   * IDLE state and has not sent any SLEEP_F to SRCH. */
  boolean empty_gpm_rxed;

  /* fields required to set a device function as Sniffer */
  mc_config_sniff_type config_sniff_mode;

  /* indicator of whether the call is in non DDS sub */
  boolean is_1x_on_non_dds_sub;


  /* Indicator to skip powerup timer of 20s */
  boolean skip_powerup_timer;
...
typedef struct
{
  ...
  boolean config_msg_seq_val_pending;
  /* To keep track if Rx config msg sequence validation pending
   * to make sure overhead is current before next sleep after Fast RAHO.
   * Without this flag, we used to set config_msg_seq = INVALID_SEQ
   * after Fast RAHO and because of this mcc_is_ovhd_cfg_okay() API 
   * will return FALSE while SRCH checking for HO to strong PN and 
   * which blocked further HO's, lead to stuck with weaker
   * PN waiting for an OTA for config_msg_seq validation and in
   * between UE missed MT page because of parser reset, poor channel condns */
...
#define MC_WAIT_FOR_SLEEP_COMP_R_BUFFER_TIME 325
   /* Maximum time to wait for SLEEP_COMPLETED_R from SRCH if SRCH is in QTA
    * while doing exit processing - 500 ms. */
...
/* Time (msec) to monitor PCH2 for a valid OTA msg before switching back to 
 ...
 * 
 * We will start timer PCH hash timer once we receive SRCH_REACQ_R and
 * after sending Idle_f to SRCH for RAHO/Fast RAHO case OR 
 * For Normal Idle handoff we start PCH hash timer after sending 
 * RXC_IDLE_HO_F to Rx.
 * In worst scenario we might be 80-msec away from interested Slot bounday
 * and we might need 160-msec from that point to decode valid OTA msg */
#define MCC_PRI_PCH_HASH_TIMEOUT 240
...
/* Registration throttling timer */
extern timer_type mccreg_throt_timer;
...
extern rex_timer_type mcc_ta_comp_buf_timer;
...
extern boolean mcc_cfs_return_trans_despatched;
...
extern rex_timer_type  mcc_hdm_tx_ack_timer;
extern rex_timer_type  mcc_wtr_hop_buf_timer;
extern rex_timer_type  mcc_pri_pch_hash_timer;
...
typedef struct
{
  /* Band class to perform handoff to */
  byte band_class;
  /* Channel to perform handoff to */
  word cdma_freq;
  /* Track if TX cmds have been sent which are required for WTR hopping */
  boolean tx_cmd_proc_done;
  /* Indicates return to serving frequency for RIF HHO is in progress */
  boolean rif_in_progress; 
}mcc_wtr_hopping_type;

/* Type of values returned after blocked wait for WTR_HOP_R */
typedef enum
{
  WTR_HOP_RPT_RECEIVED, 
  /* WTR_HOP_R received within time */
  WTR_HOP_TIMEOUT,
  /* Timeout happened waiting for WTR_HOP_R report */
  WTR_HOP_SYS_RESTART
  /* System Restart rxed while waitinf for WTR-HOP_R */
} wtr_hop_blocked_wait_type;

extern mcc_wtr_hopping_type mcc_wtr_hopping;

/* Need to buffer a copy of UHDM message for WTR hopping */
extern mccrxtx_msg_type *mcc_wtr_uhdm_msg_ptr;
...
extern caii_cfs_req_type *mcc_temp_cfs_req_msg_ptr;

/* Value of the time in ms in which UE is required to get L2 ack transmission
 * cnf from RXTX for HDM message */
#define HDM_L2_ACK_TX_CNF_TIMEOUT   100

/* Number of 20 ms frames before HHO action time when WTR HOP to SRCH and 
 * TX disable/enable should be executed. This timer value should be enough
 * to finish SRCH WTR HOP and TX disable/enable procedures */
#define WTR_HOP_SRCH_TX_BUFFER_TIME  8  /* 160 ms */

/* Number of 20 ms frames for which TX is off additionally for each 
 * measurement in CFS requiring WTR hopping */
#define WTR_HOP_CFS_TX_EXTRA_OFF_TIME 7  /* 140 ms */

/* Maximum time in ms for which 1X_CP will do a blocked wait waiting for
 * WTR_HOP_R report from SRCH for inter band RIF HHO*/ 
#define WTR_HOP_RPT_BLOCKED_WAIT_TIME 100
...
/* 1X2GTA: Max CFS with WTR req proc time in msec */
#define ONEX2GTA_CFS_WTR_MIN_REQ_TIME 220
...
FUNCTION MCCIDL_RESET_REG_REQUIRED

DESCRIPTION
  This procedure resets the mccidl_reg_required flag.
...
extern void mccidl_reset_reg_required( void );
...
FUNCTION MCCIDL_HASH_TO_PRIMARY_PCH

DESCRIPTION
  This procedure trigger hashing back to Primary PCH1 paging channel.

DEPENDENCIES
  We should be on Non-Primary paging channel and Fast RAHO not in progress
...
===========================================================================*/
word mccidl_hash_to_primary_pch
(
  word new_state
...
FUNCTION MCCIDL_WAIT_FOR_SRCH_ASET_UPDATE

DESCRIPTION
  This API will wait for Paging Channel mask/ASET cmd to take effect 
  from SRCH side.

DEPENDENCIES
  MC should have sent ASET cmd to SRCH for new ASET/PCH.
...
===========================================================================*/
void mccidl_wait_for_srch_aset_update
(
  word new_state,
    /* Current state being processed */
  boolean *srch_rpt_proc_pending
...
===========================================================================*/
void mccidl_trigger_sleep(srch_forced_sleep_rsn_type forced_sleep_rsn);
...
void mcc_set_idle_ho_spm_wait_block_sys_rel_flag( boolean flag );
...
boolean mcc_get_idle_ho_spm_wait_block_sys_rel_flag( void );
...
void mcc_set_idle_ho_pch_validation_done_flag( boolean flag );
...
boolean mcc_get_idle_ho_pch_validation_done_flag( void );
...
FUNCTION MCC_UPDATE_RUDE_WAKEUP_PEND_RSN

DESCRIPTION
  This API will update the rude wakeup pending reason global variable.
...
===========================================================================*/
void mcc_update_rude_wakeup_pend_rsn
(
  rude_wakeup_rsn_type rude_wakeup_rsn
);
...
FUNCTION MCC_GET_RUDE_WAKEUP_PEND_RSN

DESCRIPTION
  This API will return the rude wakeup pending reason gloable variable value.

DEPENDENCIES
  None

RETURN VALUE
  ENUM with Rude Wakeup pending reason type

SIDE EFFECTS
  None

===========================================================================*/
rude_wakeup_rsn_type mcc_get_rude_wakeup_pend_rsn
(
  void
);
...
FUNCTION MCC_UPDATE_ASET_CMD_PEND_FLAG

DESCRIPTION
  This API will update the aset cmd pending gloabl variable.
...
===========================================================================*/
void mcc_update_aset_cmd_pend_flag
(
  boolean flag
);
...
FUNCTION MCC_GET_ASET_CMD_PEND_FLAG
...
RETURN VALUE
  TRUE - If posting ASET cmd to SRCH is pending
  FALSE - Otherwise
...
===========================================================================*/
boolean mcc_get_aset_cmd_pend_flag
(
  void
);
...
===========================================================================*/
word mccidl_blocked_rude_wakeup
(
  word new_state,
    /* Current state being processed */
  rude_wakeup_rsn_type rude_wakeup_rsn
    /* Rude wakup reason type */
);
...
FUNCTION MCC_GET_RAFAIL_RETRY_CNT

DESCRIPTION
  This function returns the number of retrials left for sending SLEEP_F as 
  SRCH has sent SRCH_RAFAIL_R.

DEPENDENCIES
  None.

RETURN VALUE
  number of retrials left.

SIDE EFFECTS
  None.

===========================================================================*/
extern word mcc_get_rafail_retry_cnt(void);
...
DESCRIPTION
  ...
  bootup. This API is called whenever there is a subscription change
...
extern void mcc_reload_bootup_nv_items( void );
...
FUNCTION MCC_WTR_HOP_GET_SRV_CON_SEQ

DESCRIPTION
  This API returns saved service connect msg connection sequence.

DEPENDENCIES
  None

RETURN VALUE
  Saved service connect msg connection sequence

SIDE EFFECTS
  None

===========================================================================*/
byte mcc_wtr_hop_get_srv_con_seq(void);
...
FUNCTION TC_INITIATE_WTR_HOPPING

DESCRIPTION
  This function performs the initial procedures of WTR hopping.
...
===========================================================================*/
void tc_initiate_wtr_hopping (void);

/*===========================================================================

FUNCTION TC_START_OR_DEFER_WTR_HOPPING

DESCRIPTION
  This function starts WTR hopping or defers it according to the HHO
  action_time.
...
===========================================================================*/
void tc_start_or_defer_wtr_hopping (void);
...
FUNCTION MCCTCSUP_TA_IS_UHDM_MSG_DEFERRRED

DESCRIPTION
  This API will check if we have enough time to process UHDM msg OR it should
...
RETURN VALUE
  TRUE - If UHDM msg to be deferred.
  FALSE - Otherwise
...
boolean mcctcsup_ta_is_uhdm_msg_deferrred(void);

/*===========================================================================

FUNCTION MCCTCSUP_CHECK_AND_PROC_BUF_UHDM_MSG

DESCRIPTION
  This API will check if any UHDM msg got buffered and it should be processed.
...
void mcctcsup_check_and_proc_buf_uhdm_msg(void);
...
FUNCTION MCCTCSUP_PROC_TA_COMP_BUF_TIMER_EXPIRY

DESCRIPTION
  This API will handle 1x Tune Away complete buffer timer expiry
  to process pending CFS rpt.
...
void mcctcsup_proc_ta_comp_buf_timer_expiry(void);
...
/*===========================================================================
FUNCTION TC_WTR_HOP_DEFER_TX_CMD

DESCRIPTION
  This API defers MC->TX cmds and stores them in a queue. The cmds that
  were scheduled to be sent to TX by MC while TX was in inactive state 
  due to WTR hopping are deferred.
...
===========================================================================*/
void tc_wtr_hop_defer_tx_cmd (txc_cmd_type *tx_buf);

/*===========================================================================

FUNCTION TC_WTR_HOP_SEND_DEFERRED_TX_CMDS

DESCRIPTION
  This API sends deferred MC->TX cmds to TX task. The deferred cmds are the ones
  that were scheduled to be sent to TX by MC while TX was in inactive state 
  due to WTR hopping.
  While TX is in inactive state the scheduled MC->TX cmds were actually not 
  sent to TX but were instead deferred and placed temporarily in a queue.
...
===========================================================================*/
void tc_wtr_hop_send_deferred_tx_cmds (void);

/*===========================================================================

FUNCTION TC_WTR_HOP_CLR_DEFERRED_TX_CMDS

DESCRIPTION
  This API clears the deferred MC->TX cmds. This should be called while 
  entering and exiting traffic state to ensure any stale TX commands are
  not processed.
...
===========================================================================*/
void tc_wtr_hop_clr_deferred_tx_cmds (void);
...
extern void mcc_enqueue_dbg_mmoc_cnf
(
	mc_api_name_type api_name,
      /* API in which this cmd is processed */
...
/*===========================================================================
FUNCTION MCC_ENQUEUE_DBG_LTE_RRC_CNF
...
SIDE EFFECTS
  None
===========================================================================*/
extern void mcc_enqueue_dbg_lte_rrc_cnf
(
    mc_api_name_type      api_name,
       /* API in which this cmd is processed */
...
===========================================================================*/
extern boolean mcc_get_cfs_return_trans_despatched
(  
  void
);
...
void mcc_proc_reg_throt_timer_expiry(void);
...
===========================================================================*/
boolean mcc_check_if_reg_throt_timer_on(void);
...
FUNCTION MCCIDL_QTA_FORCED_SLEEP_RUDE_WAKEUP

DESCRIPTION
  This API will send forced sleep to SRCH followed by blocked rude wakeup.
  This is required for QTA feature where the forced sleep - rude wakeup 
  mechanism is required for several scenarios.
...
===========================================================================*/
word mccidl_qta_forced_sleep_rude_wakeup
(
  word new_state,
    /* Current state being processed */
  rude_wakeup_rsn_type rude_wakeup_rsn
