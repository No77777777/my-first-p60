...
$Header: //components/rel/1x.mpss/8.0/cp/inc/mccsrch_i.h#1 $ $DateTime: 2019/06/11 23:51:40 $ $Author: pwbldsvc $
...
11/10/17   sjo     Chgs for srch_rude_wakeup with tx_resources_required param.
...
04/08/16   ppr     QTA:Chgs to clear mcc_srch_q before doing force sleep with 
                   SRCH_FSLEEP_QTA rsn
...
/* 40 */
  ...
  SRCH_WTR_HOP_R,               /* SRCH is done with tuning to new WTR device
                                 * as part of HHO with WTR hopping */
...
/* 50 */
  SRCH_IDLE_ASET_UPDATE_R,      /* Idle ASET Update Report */
...
/*--------------------------------------------------------------------------
           RELEASE SYSTEM REPORT FROM SEARCHER
--------------------------------------------------------------------------*/
typedef struct
{
  mccsrch_rpt_hdr_type  hdr;
  ...
  srch_forced_sleep_rsn_type   fsleep_rsn;
    /* Reasons for which SRCH can receive forced sleep report
     * from CP */

  boolean                      rude_wakeup;
    /* This rude_wakeup flag should be "OR"ed with MC decision 
     * to trigger rude wakeup. */
}mccsrch_sys_rel_rpt_type;

/*--------------------------------------------------------------------------
           HHO WTR HOPPING DONE REPORT FROM SEARCHER
--------------------------------------------------------------------------*/
typedef struct
{
  mccsrch_rpt_hdr_type  hdr;
...
/*--------------------------------------------------------------------------
          IDLE ASET UPDATE REPORT FROM SEARCHER
--------------------------------------------------------------------------*/
typedef struct
{
  mccsrch_rpt_hdr_type  hdr;
  ...
  boolean aset_updated;
    /* ASET Updated or not */
}mccsrch_idle_aset_update_rpt_type;
...
typedef union
{
  ...
  mccsrch_sys_rel_rpt_type   sys_rel;
  
  mccsrch_wtr_hop_rpt_type  wtr_hop_rpt;

  mccsrch_ta_req_rpt_type  ta_req_rpt;
  
  mccsrch_ta_comp_rpt_type  ta_comp_rpt;

  mccsrch_idle_aset_update_rpt_type idle_aset_udpate_rpt;
...
typedef enum
{
  ...
  NONSIG_SLEEP,                              /* 42 */
  MCCIDL_QTA_FORCED_SLEEP_RUDE_WAKEUP,       /* 43 */
  MCCIDL_FORCED_SLEEP_RUDE_WAKEUP,           /* 44 */
  CDMA_EXIT_API,                             /* 45 */
  MCCTCHO_SRCH_WTR_HOP_BLOCKED_WAIT,         /* 46 */
  TC_WAIT_FOR_DATA_TA_COMPLETE,              /* 47 */
  MCCIDL_START_SLEEP_API,                    /* 48 */
  MCCIDL_WAIT_FOR_SRCH_ASET_UPDATE,          /* 49 */
  SRCH_API_MAX                               /* 50 */
...
FUNCTION MCC_MT_PAGE_ACCESS_HO_ALLOWED

DESCRIPTION

  This API returns whether access handoff is allowed to specific BS info 
  while MT page is in progress.

DEPENDENCIES
  None

RETURN VALUE
  TRUE - If handoff allowed to specific BS info during MT page access.
  FALSE - Otherwise
...
===========================================================================*/
boolean mcc_mt_page_access_ho_allowed
(  
  word pilot_pn,   
    /* pilot pn offset */
  byte band_class, 
