...
$Header: //components/rel/1x.mpss/8.0/srch/inc/srch_sect.h#1 $
...
08/26/15   bb      Use right set of band/channel for sector management for
                   hoping scenarios
03/11/15   jh      Added new search list for access handoff
03/31/14   as      Access srch_ref_ptr via API
01/16/14   bb      Changes to search expensive neighbor pilots during QPCH offline
12/13/13   as      Updates to support BOLT QPCH Offline
...
10/20/11   ab      Moved "srch_sect_pilot_info_image_type" str declaration to
                   srch_diag.h in 1x/api/main/latest
07/26/11   cjb     Moved "srch_sect_cs" init/deinit to srch_task().
07/11/11   bb      Changes to use NSET window size for all active pilots
                   until active pilot slew value is lesser than ASET_SIZ/2
06/08/11   adw     Corrected function return type.
04/20/11   adw     Added sect mode set function.
03/24/11   adw     Added function to sort search results based on total lecio.
03/07/11   mca     Rename srch_set_enum_type to srch_sect_set_enum_type
11/01/10   jtm     Modem 1x SU API changes.
10/04/10   bb      Explicitly add O-Freq neighbors to TC Neighbor list
                   during Paging to Traffic state transition
09/22/10   bb      Added code changes for Neighbor Set triage feature
12/04/09   vks     Move HW_MAX_FING_CNT definition to srch_hw.h.
09/18/09   adw     Support dynamic allocation of sector pools.
...
typedef enum
{
  ...
  SRCH_SECT_LIST_STRONG_NS_AHO = 17,
  SRCH_SECT_LIST_LAST_SEEN_SC_STRONG = 18,
  SRCH_SECT_LIST_TC_CF_SRCH_N  = 19,
  SRCH_SECT_LIST_TC_STRONG_NS  = 20,
  SRCH_SECT_NUM_LISTS
...
/* Declare a critical section srch sect operations */
extern rex_crit_sect_type srch_sect_cs;
...
/* constant to indicate the size of the SRCH_SECT_LIST_TC_STR_NS sector list */
#define SRCH_SECT_LIST_TC_STRONG_NS_MAX_SIZE   2

#ifdef FEATURE_MODEM_1X_IRAT_LTO1X
#define SRCH_SECT_LIST_N_MAX_SIZE      ( SRCH_NSET_MAX + SRCH_ASET_MAX )
#else /* !FEATURE_MODEM_1X_IRAT_LTO1X */
#define SRCH_SECT_LIST_N_MAX_SIZE        SRCH_NSET_MAX
#endif /* FEATURE_MODEM_1X_IRAT_LTO1X */
...
FUNCTION       SRCH_SECT_DEINIT

DESCRIPTION    De-initializes the sect module.
...
=============================================================================*/
extern void srch_sect_deinit(void);
...
(
  ...
  srch_sect_set_enum_type set     /* The set where this sector came from */
);
...
FUNCTION       SRCH_SECT_TC_PRE_TUNE_ASET_UPDATE
...
=============================================================================*/
boolean srch_sect_tc_pre_tune_aset_update
(
  ...
  boolean *tune_required,        /*Flag to indicate whether tune is required or not */
  boolean hop_in_progress        /* flag to indicate that hoping is in progress */
 );

/*=============================================================================

FUNCTION       SRCH_SECT_TC_POST_TUNE_ASET_UPDATE
DESCRIPTION    This  function is executed after a TUNE is
               completed. Sector Structure is refreshed here.

DEPENDENCIES   srch_sect_tc_pre_tune_aset_update should have been
               called before
...
=============================================================================*/
void  srch_sect_tc_post_tune_aset_update
(
  srch_tc_aset_info_type *info,  /* Pointer to the Aset Info Type */
  boolean hho_check,             /* RIF-HHO in progress */
  boolean on_sf,                 /* Flag to indicate if on the Serving Freq */
  boolean use_nset_win_size      /* flag to indicate whether NSET WINDOW size or not */
);
...
=============================================================================*/
extern srch_sect_struct_type*  srch_sect_get_ref(void);
...
FUNCTION       SRCH_SECT_LIST_GET_NOBUILD
DESCRIPTION    Returns a pointer to the requested sector list along with the
               size of the sector list.  This call will not build the list,
               will return the explicit stored values.
...
=============================================================================*/
extern srch_sect_list_type srch_sect_list_get_nobuild
(
  srch_sect_list_name_type list,  /* List to return */
  uint32* size                    /* Filled with number of sect is list */
);
...
(
  ...
  uint32 mode,          /* search mode   */
  uint16 rtc_cx2        /* stop rtc value (applicable when mode == OFFLINE) */
);
...
FUNCTION       SRCH_SECT_LIST_SEARCH_RESULT_SORT
DESCRIPTION    Sorts a list by total linear ecio seen in the latest
               search report.
...
=============================================================================*/
extern void srch_sect_list_search_result_sort
(
  srch_sect_list_name_type list_id
);
...
FUNCTION       SRCH_SECT_UPD_STR_TC_NS_LIST

DESCRIPTION    This function is used to update the SRCH_SECT_LIST_TC_STR_NS
               sector list, if the passed neighbor pilot is stronger than
               neighbor pilots contained in this list.
...
=============================================================================*/
extern void srch_sect_upd_str_tc_ns_list
(
  srch_sect_list_name_type list,    /* List to add sector to */
  srch_sect_struct_type   *sect     /* neighbor/candidate sector */
);


/*=============================================================================

FUNCTION       SRCH_SECT_CHECK_SECTOR

DESCRIPTION    It checks whether a given sector is in a passed list or not.

DEPENDENCIES   None

RETURN VALUE   TRUE  - sector is present in list
               FALSE - sector is not present in the list

SIDE EFFECTS   None

=============================================================================*/
extern boolean srch_sect_check_sector(
  srch_sect_list_name_type list,    /* sector list to check */
  srch_sect_struct_type*   sector    /* sector pointer */
);

/*=============================================================================

FUNCTION       SRCH_SECT_TC_ADD_OFREQ_NBRS
DESCRIPTION    This function is used to add O-Freq neigbors to
               SRCH_SECT_LIST_TC_N immediately after it enters to the
               Traffic state
...
=============================================================================*/
extern void srch_sect_tc_add_ofreq_nbrs(void);

#ifdef FEATURE_QPCH_OFFTL_SUPPORTS_EXPENSIVE_NBHR_SRCH
/*=============================================================================

FUNCTION       SRCH_SECT_RESET_QPCH_OFFL_NBR_SRCH_STATE
DESCRIPTION    This function is used to reset qpch offline reacquisition
               state to initial state and it is called when searches
               exits out of qpch offline timeline
...
=============================================================================*/
extern void srch_sect_reset_qpch_offtl_nbr_srch_state(void);
/*=============================================================================

FUNCTION       SRCH_SECT_SET_NEXT_QPCH_OFFTL_NBR_SRCH_STATE
DESCRIPTION    This function is used to set next qpch offline reacquisition
               search state based on current state.
...
=============================================================================*/
extern void srch_sect_set_next_qpch_offtl_nbr_srch_state(void);
#endif /* FEATURE_QPCH_OFFTL_SUPPORTS_EXPENSIVE_NBHR_SRCH */

#ifdef FEATURE_MODEM_1X_IRAT_LTO1X
/*=============================================================================

FUNCTION       SRCH_SECT_SET_MODE

DESCRIPTION    Configures the sector module mode.
...
=============================================================================*/
void srch_sect_set_mode
(
  srch_sect_mode_type mode    /* sector module mode */
);
#endif /* FEATURE_MODEM_1X_IRAT_LTO1X */
