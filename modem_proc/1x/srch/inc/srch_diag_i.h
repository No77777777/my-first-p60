...
$Header: //components/rel/1x.mpss/8.0/srch/inc/srch_diag_i.h#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
11/20/17   ab      1x2XTA test mode changes.
11/20/17   ab     1x2GTA diag cmd changes
06/18/15   ab      Remove SET QTA Params/ Add Set CHAIN UNLOCK THRESH
05/18/15   ab      Added Set QTA Params
...
enum {
  ...
  SRCH_FORCE_CRASH            = 0xFF,
  #ifdef FEATURE_1X_TUNEAWAY_TEST
  SRCH_TUNE_AWAY_TEST         = 0xA0,
...
This interface provides a way to force a crash
--------------------------------------------------------------------------*/

DIAGPKT_SUBSYS_REQ_DEFINE(SRCH1X, SRCH_FORCE_CRASH)
  uint8  when;
DIAGPKT_REQ_END

DIAGPKT_SUBSYS_RSP_DEFINE(SRCH1X, SRCH_FORCE_CRASH)
DIAGPKT_RSP_END
...
Starts Tuneaway test from DIAG
--------------------------------------------------------------------------*/

DIAGPKT_SUBSYS_REQ_DEFINE(SRCH1X, SRCH_TUNE_AWAY_TEST)
	uint8  enable;       /* Enable/Disable */
	uint8  Period;       /* TuneAway Period */
	uint8  TuneAwayGap;  /* TuneAway Gap */
	uint8  client;	     /* Requestting Client */
	uint8  ta_type;	     /* Tuneaway Type */
DIAGPKT_REQ_END

DIAGPKT_SUBSYS_RSP_DEFINE(SRCH1X, SRCH_TUNE_AWAY_TEST)
DIAGPKT_RSP_END
#endif /* FEATURE_1X_TUNEAWAY_TEST */
