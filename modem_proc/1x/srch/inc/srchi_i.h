...
DESCRIPTION
  ...
  Copyright (c) 1991 - 2017 Qualcomm Technologies, Inc.
...
$Header: //components/rel/1x.mpss/8.0/srch/inc/srchi_i.h#1 $
...
02/29/16   bb      Changes to avoid abort tuning CFS channel if it is 
                   already started tuning
02/05/16   pk      1x2GTA Feature changes
11/09/15   ab      Move srch_get_cdma_rssi_all_chains() to srch_v.h
...
02/24/14   pk      Removed API srch_cdma_set_aborted_acq
01/16/14   pk      Added function in CDMA state to clear the unlock rf timer,
                   if it is currently active.
12/05/13   bph     FR3839 - IRATMan dynamic Tbackoff
09/18/13   vks     Register with m1x stmr for pn roll isr
08/23/13   vks     Enabling of combiner interrupts obsolete on bolt
...
08/29/12   ab      Added ACQ state MACRO
08/21/12   srk     Added a new CFS state to freeze RDDS parameters before CFS.
08/21/12   dkb     Added new srch state macros and extern srch print func
...
02/08/11   adw     Removed featurization around micro TT triage.
02/01/11   sst     Added state SRCH_LTO1X_STATE for LTE->1x TT, Meas, CSFB
01/28/11   adw     Remove featurization around initial micro tt triage.
01/26/11   vks     Remove unncessary rf header file includes.
12/09/10   mca     Added support for SRCH_API_TEST
12/02/10   adw     Added function for notifying TC state that RC has changed.
...
09/03/10   vks     Replace NO_PATH with ONEX_SRCH_NO_PATH to fix compiler
                   warning.
                   Replace NO_FINGER with ONEX_SRCH_NO_FINGER to fix compiler
                   warning.
...
/* TCB for SRCH task */
extern rex_tcb_type* srch_tcb_ptr;

/* RCInit version */
#define SRCH_TCB_PTR ( srch_tcb_ptr )
...
#define IS_INACTIVE_STATE(state)         (((state) == SRCH_INACTIVE_STATE) || \
                                          ((state) == SRCH_LTO1X_STATE) )

#define IS_CDMA_STATE(state)             ((state) == SRCH_CDMA_STATE)

#define IS_ACQ_STATE(state)              ((state) == SRCH_ACQ_STATE)

#define IS_SYNC_STATE(state)             ((state) == SRCH_SC_STATE)

#define IS_INIT_STATE(state)             (((state) == SRCH_CDMA_STATE) || \
                                          ((state) == SRCH_ACQ_STATE)  || \
                                          ((state) == SRCH_SC_STATE)   || \
                                          ((state) == SRCH_SLEW_STATE))

#define IS_IDLE_STATE(state)             (((state) == SRCH_PC_STATE) || \
                                          ((state) == SRCH_ZZ_STATE) || \
                                          ((state) == SRCH_ACCESS_STATE))
...
#define IS_LTO1X_STATE(state)            ((state) == SRCH_LTO1X_STATE)

#define IS_TUNEAWAY_STATE(state)         ((state) == SRCH_TA_STATE)
...
#define  ONEX_SRCH_NO_FINGER ((word)~0)
  /* Value used in a finger index to indicate none of the fingers. */

#define  ONEX_SRCH_NO_PATH  ((word)~0)
...
/* Roll-over value for the 42 bits of user long code state */
#define MAX_ULPN_ROLL_OVER_VALUE            0x3FFFFFFFFFFULL
...
typedef struct
{
  ...
  qword ficch_lcm;
    /* Long code mask for forward indicator control channels */

  boolean tt_info_valid;
    /* Indicates that time transfer information is valid */
  uint16  resel_pilot;
    /* Store the reselection pilot information for fast acquisation */
  uint32  resel_srch_wincen;
    /* Store the calculated window center for fast acquisation scenarios */
...
#define  OFFLINE_DONE() \
  ( rex_clr_sigs ( SRCH_TCB_PTR, SRCH_OFFLINE_SIG ) & SRCH_OFFLINE_SIG )
...
#ifdef SRCH_VERBOSE
  #define SRCH_DEBUG_MSG(...)                                       \
            M1X_MSG( SRCH, LEGACY_LOW, __VA_ARGS__ )
#else
  #define SRCH_DEBUG_MSG(...)
#endif
...
/*lint -save -e760 indentically defined in other systems.*/
...
#define SRCH_NULL_PAYLOAD_CHECK( payload )                                   \
  ...
  {                                                                          \
    M1X_MSG( SRCH, LEGACY_ERROR,                                             \
      "Null Payload not expected in transition function" );                  \
...
/* The following macro is used to report that no message is present.
   An error condition exists if the switch statement ever falls through to
   the default case with a message flag other than 0. */
#define SRCH_MSG_FLAG_CHK( msg_flag )
...
/* The following defines create sections where other code is not allowed to
  ...
  |-----------------------------------------------------------------------|
  | No   Yes  -   | Use SRCH_ENTER_ATOMIC_SECT()                          |
  |-----------------------------------------------------------------------|
  | Yes  No   No  | Use SRCH_ENTER_ISR_SECT()                             |
  |-----------------------------------------------------------------------|
  | Yes  No   Yes | Use SRCH_ENTER_CRIT_SECT()                            |
  |-----------------------------------------------------------------------|
  | Yes  Yes  No  | Use SRCH_ENTER_ATOMIC_CRIT_SECT()                     |
  |-----------------------------------------------------------------------|
  | Yes  Yes  Yes | Use SRCH_ENTER_ATOMIC_ISR_SECT()                      |
  \-----------------------------------------------------------------------/
...
/* Atomic operations */
#define SRCH_ENTER_ATOMIC_SECT()                      \
          REX_DISABLE_PREMPTION()

#define SRCH_LEAVE_ATOMIC_SECT()                      \
          REX_ENABLE_PREMPTION()

/* Mutual exclusion between ISRs or between ISR(s) and task(s) */
#define SRCH_ENTER_ISR_SECT()                         \
          REX_ISR_LOCK( &srch_crit_sect )

#define SRCH_LEAVE_ISR_SECT()                         \
          REX_ISR_UNLOCK( &srch_crit_sect )

/* Mutual exclusion between two normal tasks */
#define SRCH_ENTER_CRIT_SECT()                        \
          rex_enter_crit_sect( &srch_crit_sect )

#define SRCH_LEAVE_CRIT_SECT()                        \
          rex_leave_crit_sect( &srch_crit_sect )

/* Mutual exclusion between two normal tasks and time critical code */
#define SRCH_ENTER_ATOMIC_CRIT_SECT()                 \
          rex_enter_crit_sect( &srch_crit_sect );     \
          REX_DISABLE_PREMPTION()

#define SRCH_LEAVE_ATOMIC_CRIT_SECT()                 \
          REX_ENABLE_PREMPTION();                     \
          rex_leave_crit_sect( &srch_crit_sect )

/* Mutual exclusion between two ISRs or between ISR(s) and normal task(s)
   and time critical code */
#define SRCH_ENTER_ATOMIC_ISR_SECT()                  \
          REX_ISR_LOCK( &srch_crit_sect );            \
          REX_DISABLE_PREMPTION()

#define SRCH_LEAVE_ATOMIC_ISR_SECT()                  \
          REX_ENABLE_PREMPTION();                     \
          REX_ISR_UNLOCK( &srch_crit_sect )
...
extern rex_crit_sect_type srch_crit_sect;
...
typedef enum          /* Define values for the CFS substate */
{
  ...
  SRCHTC_CFS_START_TUNING, /* It's started tuning the channel now */
  SRCHTC_CFS_RDDS_STOP     /* In SF but need to freeze RDDS metrics now */
}
...

/*===========================================================================
FUNCTION      SRCHCOM_IS_TX_RESOURCE_REQUIRED

DESCRIPTION   This function is used to get whether 1x requires tx resource

DEPENDENCIES  None

RETURN VALUE  srchcom.tx_resource_required

SIDE EFFECTS  None
===========================================================================*/
boolean srchcom_is_tx_resource_required( void );
...
/*===========================================================================
FUNCTION       SRCH_CDMA_RESET_UNLOCK_RF_TIMER

DESCRIPTION    This function is used to clear the unlock rf timer, if it
               is currently active
...
SIDE EFFECTS   cdma_stm gets updated

===========================================================================*/
extern void srch_cdma_reset_unlock_rf_timer ( void );

/*===========================================================================
FUNCTION       srch_cdma_get_unlock_rf_requested

DESCRIPTION    This function is used to get whether rf lock needs to be
               released.

DEPENDENCIES   None

RETURN VALUE   cdma_stm.unlock_rf_requested

SIDE EFFECTS   None

===========================================================================*/
extern boolean srch_cdma_get_unlock_rf_requested ( void );
...
FUNCTION SRCH_TT_STATE

DESCRIPTION    This function is the state handler for the HDR TIME TRANSFER

               state.Upon receiving SRCH_TT_F in CDMA state, the TT state

         will be active. During the state, the forward timing and the

         ULPN will be setup so that SRCH is ready to jump into traffic
...
===========================================================================*/
extern srch_state_enum_type  srch_tt_state( void );
...
/*===========================================================================
FUNCTION       SRCHCD_ALG_INIT

DESCRIPTION    This function defines and reads the timers value from EFS for
               CDMA state of operations.
...
===========================================================================*/
boolean srch_slew_page_decode_sample_ignore_timer_running(void);
...
===========================================================================*/
extern boolean  srch_get_sid_nid_base_id
(
  word  *sid,
...
DEPENDENCIES   Relies on SRCH_DOG_RPT_SIG being raised often
      enough to avoid watchdog alarms, so should not be called
      until srch_init has run.
...
===========================================================================*/
extern void  srch_roll_isr ( m1x_stmr_event_type );
...
===========================================================================*/
extern void srch_roll_common_isr ( m1x_stmr_event_type );
...
===========================================================================*/
extern void srchpc_sys_reselect_init( void );
...
SIDE EFFECTS   Updates srchpc_rssi, srchpc_rssi_ecio, srchpc_rssi_low_cnt,
               srchpc_rssi_samp_time.
===========================================================================*/
extern boolean srchpc_sys_reselect_check( boolean send_rpt );
...
FUNCTION       INITIAL_MICRO_TIME_TRANSFER_TRIAGE

DESCRIPTION    This function assigns fingers to all peaks after getting the
               search result in TT state machine

DEPENDENCIES   All fingers need to be unassigned when this function is
               called.

RETURN VALUE   Number of possible paths in the search result

SIDE EFFECTS   None

===========================================================================*/
extern uint8 initial_micro_time_transfer_triage
(
  boolean run_triage /* run triage or just return the number of path */
);

#ifdef FEATURE_SRCH_VOIP_1X_HANDOFF
/*===========================================================================

FUNCTION SRCH_TT_START_HDRTO1X_TIME_TRANSFER

DESCRIPTION  Upon transition from CDMA to TT, This function will be called to
             get the timing information from HDR, such as fing. position, RTC diff.
       Then, it computes the 1x fing. position based on those information
...
===========================================================================*/
extern void srch_tt_start_hdrto1x_time_transfer
(
  srch_cmd_type *cmd_ptr  /* external command pointer */
);
#endif /* FEATURE_SRCH_VOIP_1X_HANDOFF */
...
SIDE EFFECTS  None
===========================================================================*/
extern sys_modem_as_id_e_type srch_get_asid( void );

/*===========================================================================
FUNCTION SRCH_SET_ASID

DESCRIPTION   This function fetches the ASID of 1X
...
SIDE EFFECTS  None
===========================================================================*/
extern void srch_set_asid( void );
#endif /* SRCHI_I_H */
