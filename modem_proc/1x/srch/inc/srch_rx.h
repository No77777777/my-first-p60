...
$Header: //components/rel/1x.mpss/8.0/srch/inc/srch_rx.h#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
11/29/17   ab      Inform TRM about wake up chain denial or grant
11/10/17   pk      FR 33958: TRM-U Enhancements and Optimizations
03/30/16   srk     Pass valid RxLM handle to FW on RX_STOP.
02/05/16   pk      1x2GTA Feature changes
01/27/16   pk      Interface change in srch band/channel validation API
10/27/15   ppr     WTR HOP:Chgs for ECAM wtih AM=1(Paging Channel Assignment)
10/20/15   pk      Always tune diveristy to same band/chan, primary is tuned to
09/09/15   pk      Ignoe Start QTA from TRM, if we have reserved for LTA
08/27/15   jh      Fixed logging and other issues, initial idle div cleanup
08/26/15   srk     Do not re-enable DR mode for G data traffic.
07/29/15   srk     Removed exporting static functions.
07/20/15   ab      Release DIV chain in IDLE during WTR hop
07/17/15   pk      QTA timeline optimization, do RF script building just after
                   QTA start call back from TRM
07/10/15   ab      WTR hopping changes
07/09/15   pk      Changes to support partial grant from TRM for IRAT
07/01/15   jh      Changes to support WTR hopping
06/16/15   jh      Add new function srch_rx_check_wtr_hopping()
06/10/15   jh      added function srch_rx_is_band_chan_supported_on_card
06/03/15   ab      Enable 1X Split Overheads for all MSIM combinations
05/25/15   sst     Make LCR chain specific
05/11/15   jh      RXTX split - FR 21976
04/29/15   srk     Re-enable QPCH in Hybrid Fallback mode.
04/29/15   srk     Unification of IDLE DIV algorithm for all CRAT modes.
04/29/15   sst     Add LCR algorithm for Idle Div
04/28/15   pap     Adding log packet for IDLE DIV related stats.
04/25/15   sst     Add LCR algorithm for Idle Div
04/25/15   sst     Add new PCH MER Log packet
04/21/15   srk     DRDSDS and BOLT REVISIT Feature cleanup
04/17/15   srk     IDLE TL rebucketing
03/26/15   pk      Changes to support new RF Meas API for L to 1x meas
...
/* This is emulating the rf device type provided by rfcom_device_enum_type
   diversity devices must be odd */
typedef enum
{
  /*---------- non-SV chains ----------*/
  SRCH_RF_TRANSCEIVER_0 = 100,
  SRCH_RF_RECEIVER_MIN = 100,
  SRCH_RF_RECEIVER_0 = 100,
  SRCH_RF_RECEIVER_1,
  SRCH_RF_RECEIVER_2,
  SRCH_RF_RECEIVER_MAX,
  SRCH_RF_TRANSMITTER_MIN = 200,
  SRCH_RF_TRANSMITTER_0 = 200,
  SRCH_RF_TRANSMITTER_1 = 202,
  SRCH_RF_TRANSMITTER_MAX,

  /*--------- DIV ---------*/
  SRCH_RF_RECEIVER_DIV,

  SRCH_RF_DEPRECATED = 300,
  /*------------ SV chains ------------*/
...
typedef enum
{
  /* Valid Responses */
  SRCH_RX_BAND_REJECT,
  SRCH_RX_BAND_CONTINUE,
  SRCH_RX_BAND_HOP,

  /* Invalid Responses */
  SRCH_RX_BAND_INVALID        /* Invalid Type             */
} rx_band_tune_type;
...
typedef enum
{
  /* Valid clients */
  SRCH_RX_ONEX_SRCH,
  SRCH_RX_ONEX_SRCH_TUNE,
  SRCH_RX_ONEX_MC,

  /* Invalid clinet */
  SRCH_RX_ONEX_INVALID
} rx_onex_client;
...
/* This structure holds all RF scripts building related Params for QTA */
typedef struct
{  
  /* Flag to keep track for our latest reservation with TRM is LTA/QTA */ 
  boolean reserve_trm_for_qta;

  /* Flag to check whether qta grant from TRM was ignored */
  boolean qta_grant_ignored;

  /* This structure is passed by TRM in QTA grant cb, consisting of required
   * source RF related PARAMS, later on we add 1x RF params to this structure
   * and pass it to RF for script building */
  rfm_meas_common_param_type* rf_meas_param;

  /* Device id of primary RF device, passed in QTA strat cb from TRM */
  rfm_device_enum_type pri_rf_dev;

  /* Device id of secondary RF device, passed in QTA strat cb from TRM */
  rfm_device_enum_type sec_rf_dev;
} srch_rx_qta_type;

#ifdef FEATURE_CRAT_MSIM_TEST_FRAMEWORK
/* This structure holds all info related to ELS  */
typedef struct
{
  /* Flag to keep track of ELS state */
  boolean                     trm_els_enabled;
  /* Flag to indicate if 1X has registered for ELS */
  boolean                     trm_els_registered;
  /* Flag to check the ELS DATA tech */
...
#define SRCH_RX_SETUP_MSG_CLT( rx_client, command )                     \
  {                                                                     \
    ...
    stm_state_machine_type *sm_ptr;                                     \
                                                                        \
    switch ( rx_client )                                                \
    {                                                                   \
      case CLIENT_1X_RX_PRI:                                            \
        sm_ptr = &SRCH_RX_SM_P;                                         \
        break;                                                          \
      case CLIENT_1X_RX_SEC:                                            \
        sm_ptr = &SRCH_RX_SM_S;                                         \
        break;                                                          \
      case CLIENT_1X_TX:                                                \
      default:                                                          \
...
FUNCTION      SRCH_RX_GET_PRIM_RX_DEVICE

DESCRIPTION   This function returns the Primary RF Rx device.
...
===========================================================================*/
extern rfm_device_enum_type srch_rx_get_prim_rx_device( void );
...
FUNCTION      SRCH_RX_HYBRID_FALLBACK_IS_NEEDED

DESCRIPTION   This function checks if Hybrid Fallback is needed. This is only
              needed when UE is configured in Dual Receive Mode and currently
              DR is enabled.

DEPENDENCIES  None

RETURN VALUE  TRUE:  If in DR mode and Hybrid Fallback conditions are seen.
              FALSE: Otherwise

SIDE EFFECTS  None

===========================================================================*/
extern boolean srch_rx_hybrid_fallback_is_needed
(
  boolean query_trm
);
...
/*===========================================================================
FUNCTION       SRCH_RX_GET_IRAT_DEV

DESCRIPTION    This function returns primary RF device for IRAT measurement
               allocated to band corresponding to EARFCN. If primary device
               is invalid, then RF does not support measurement to this EARFCN.

DEPENDENCIES   None.

RETURN VALUE   rfm_device_enum_type - RF device

SIDE EFFECTS   None.
===========================================================================*/
extern rfm_device_enum_type srch_rx_get_irat_dev
(
  lte_earfcn_t earfcn,
...
#ifdef FEATURE_CRAT_MSIM_TEST_FRAMEWORK
/*===========================================================================
FUNCTION      SRCH_RX_UPDATE_ELS_STATE

DESCRIPTION   This function updates the ELS state to TRM
...
SIDE EFFECTS  None
===========================================================================*/
extern void srch_rx_update_els_state
(
  trm_els_request_enum_type req_type,
...
FUNCTION       SRCH_RX_IS_DEVICE_CONN_CAPABLE

DESCRIPTION    This function determines if the primary RX device is
               connection capable

DEPENDENCIES   None.

RETURN VALUE   True if connection capable

SIDE EFFECTS   None.

===========================================================================*/
extern boolean srch_rx_is_device_conn_capable ( void );

/*===========================================================================
FUNCTION       SRCH_RX_CHAIN_IS_PR_RX_CHAIN_IS_USING_TX_COMPATIBLE_REASON

DESCRIPTION    This function determines if the primary Rx client is using
               Tx compatible reason

DEPENDENCIES   None.

RETURN VALUE   True if specified chain is Tx compatible, FALSE otherwise

SIDE EFFECTS   None.
===========================================================================*/
extern boolean srch_pr_rx_chain_is_using_tx_compatible_reason( void );

/*===========================================================================

FUNCTION       SRCH_RX_CHAIN_IS_RX

DESCRIPTION    This function determines if the specified chain is RX

DEPENDENCIES   None.

RETURN VALUE   True if specified chain is RX, FALSE otherwise

SIDE EFFECTS   None.

===========================================================================*/
extern boolean srch_rx_chain_is_rx
(
  rx_sm_client_enum_t  chain_name    /* chain */
);
...
FUNCTION       SRCH_RX_CHAIN_IS_TX

DESCRIPTION    This function determines if the specified chain is TX

DEPENDENCIES   None.

RETURN VALUE   True if specified chain is TX, FALSE otherwise

SIDE EFFECTS   None.

===========================================================================*/
extern boolean srch_rx_chain_is_tx
(
  rx_sm_client_enum_t  chain_name    /* chain */
);
...
/*===========================================================================
FUNCTION      SRCH_RX_CHECK_WTR_HOPPING

DESCRIPTION   This function checks if a retune is possible on the currently
              held RX and TX devices. If the client is SRCH_RX_ONEX_SRCH_TUNE,
              this API also updates global wtr hopping related variables.


DEPENDENCIES  None

RETURN VALUE  SRCH_RX_BAND_REJECT:   If retune is not possible on the current
                                     RX device
              SRCH_RX_BAND_HOP:      If a WTR hopping is required for doing a
                                     retune
              SRCH_RX_BAND_CONTINUE: If retune is allowed on the currently held
                                     devices.

SIDE EFFECTS  None
===========================================================================*/
extern rx_band_tune_type  srch_rx_check_wtr_hopping
(
  srch_rx_band_type band,
  srch_rx_chan_type chan,
  rx_onex_client    client

);

/*===========================================================================
FUNCTION      SRCH_RX_IS_WTR_HOPPING_NEEDED

DESCRIPTION   This function checks if a WTR hopping is needed

DEPENDENCIES  None

RETURN VALUE  TRUE : If a WTR hop is needed
              FALSE: Otherwise

SIDE EFFECTS  None
===========================================================================*/
extern boolean srch_rx_is_wtr_hopping_needed( void );

/*===========================================================================
FUNCTION      SRCH_RX_IS_TX_HOPPING_NEEDED

DESCRIPTION   This function checks if a Tx hopping is needed

DEPENDENCIES  None

RETURN VALUE  TRUE: If a Tx hop is needed
              FALSE: Otherwise

SIDE EFFECTS  None
===========================================================================*/
extern boolean srch_rx_is_tx_hopping_needed( void );
...
===========================================================================*/
extern uint16 srch_rx_get_rf_prep_warmup_time
(
  boolean going_to_sleep
);
...
===========================================================================*/
extern uint16 srch_rx_get_rf_exec_warmup_time
(
  boolean going_to_sleep
);
...
FUNCTION      SRCH_RX_DIVERSITY_OVHD_IS_ADDED

DESCRIPTION   This function returns if the diversity overhead was added to
              the RF exec time allocation.

DEPENDENCIES  None.

RETURN VALUE  TRUE:  If diversity overhead was added
              FALSE: Otherwise

SIDE EFFECTS  None.

===========================================================================*/
extern boolean srch_rx_diversity_ovhd_is_added( void );
...
(
  trm_qta_data_type *qta_param,
  boolean	    start_qta
);

/*===========================================================================
FUNCTION       SRCH_RX_UPDATE_TRM_ABOUT_1X2X_QTA_OPERATION

DESCRIPTION    This function calls trm_set_client_state() to Start/End QTA.

DEPENDENCIES   None.

RETURN VALUE   Whether QTA GAP was empty or not, 
               Valid  only if start_qta is FALSE. 
               TRUE for empty QTA GAP.

SIDE EFFECTS   None.
===========================================================================*/
extern boolean srch_rx_update_trm_for_1x2x_qta_operation
(
  trm_qta_data_type *qta_param,
  boolean           start_qta
);
...
/*===========================================================================
FUNCTION       SRCH_RX_UPDATE_PARAMS_FOR_1X2X_QTA

DESCRIPTION    This function updates trm_qta_data_type structure with params
               common to both Start/End QTA call.
...
SIDE EFFECTS   None.
===========================================================================*/
extern void srch_rx_update_params_for_1x2x_qta
(
  trm_qta_data_type *qta_param,
...
/*===========================================================================
FUNCTION       SRCH_RX_UPDATE_MEAS_PARAMS

DESCRIPTION    This function updates rfm_meas_common_param_type structure with
               1x realted params.

DEPENDENCIES   1x must own the RX chain.

RETURN VALUE   None.

SIDE EFFECTS   None.
===========================================================================*/
extern void srch_rx_update_meas_params
(
  srch_rx_qta_type* srch_rx_qta
);

/*===========================================================================
FUNCTION       SRCH_RX_MEAS_COMMON_SCRIPT_BUILD_SCRIPTS_V2

DESCRIPTION    This function calls RF API to trigger the setup and build
               procedure for RF script building.
...
SIDE EFFECTS   None.
===========================================================================*/
extern void srch_rx_meas_common_script_build_scripts_v2
(
  rfm_meas_common_param_type* setup_params
);

/*===========================================================================
FUNCTION       SRCH_RX_MEAS_COMMON_SCRIPT_ENTER

DESCRIPTION    This function calls RF API to trigger the enter procedure
               for RF script building.
...
SIDE EFFECTS   None.
===========================================================================*/
extern void srch_rx_meas_common_script_enter
(
  rfm_meas_common_param_type* enter_params
);

/*===========================================================================
FUNCTION       SRCH_RX_MEAS_COMMON_SCRIPT_EXIT

DESCRIPTION    This function calls RF API to trigger the exit procedure
               for RF script building.
...
SIDE EFFECTS   None.
===========================================================================*/
void srch_rx_meas_common_script_exit
(
...
RETURN VALUE   TRUE:  RxLM Handle is valid
               FALSE: Otherwise
...
typedef enum
{
  ...
  /* LCR feature */
  SRCH_DYN_RID_RX0_LCR_FAIL,
  SRCH_DYN_RID_RX0_LCR_PASS,
  SRCH_DYN_RID_RX1_LCR_FAIL,
  SRCH_DYN_RID_RX1_LCR_PASS
...
FUNCTION       SRCH_RX_UPDATE_LCR
DESCRIPTION    This function records when total energy equals "0" in
                a mask for LCR (Level Crossing Rate) Algorithm
...
=============================================================================*/
extern void srch_rx_update_lcr
(
  srch_sect_result_type *results,
  uint16                 pilot
);
...
extern void srch_rx_tx_wakeup( void );

extern void srch_rx_tx_sleep( void );
...
FUNCTION       SRCH_RX_UPDATE_TRM_CHAIN_STATE

DESCRIPTION    The function informs TRM for chain grant or chain denial
...
===========================================================================*/
extern void srch_rx_update_trm_chain_state( uint64 next_wakeup_identifier );

/*===========================================================================

FUNCTION       SRCH_RX_UPDATE_WAKEUP_CANCEL_EVENT

DESCRIPTION    The function updates for wakeup cancel event.
...
===========================================================================*/
extern void srch_rx_update_wakeup_cancel_event( uint16 wakeup_id );
...
===========================================================================*/
extern void srch_rx_change_dual_receive_mode( void );
