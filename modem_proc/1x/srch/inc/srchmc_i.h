...
$Header: //components/rel/1x.mpss/8.0/srch/inc/srchmc_i.h#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
08/30/16   ab      1x2GTA : 0x18CD log packet changes.
08/03/16   pk      Added integer value to each field of srch_cmd_code_type
...
typedef enum
{
  SRCH_CDMA_F = 0,      /* Enter CDMA mode */
  SRCH_ACQ_F = 1,       /* Acquire pilot PN */
  SRCH_SC_F = 2,        /* Begin sync channel operation */
  SRCH_SLEW_F = 3,      /* Slew timing to align with system time */
  SRCH_IDLE_F = 4,      /* Begin idle operation */
  SRCH_ACCESS_F = 5,    /* Begin access operation */
  SRCH_SLEEP_F = 6,     /* Put the system to sleep until next slot */
  SRCH_WAKEUP_F = 7,    /* Initiate a wakeup now */
  SRCH_TC_F = 8,        /* Begin traffic channel operation */

  /* The above commands all request a state change. */

  SRCH_CHAN_CONFIG_F = 9, /* Channel configuration update */
  SRCH_BC_INFO_F = 10,
  SRCH_PARM_F = 11,      /* Change search parameters */
  SRCH_MEAS_F = 12,      /* Return Pilot Strength Measurements */
  SRCH_FPC_MEAS_F = 13,  /* Return Forward Power Control Pilot Measurements */
  SRCH_IDLE_ASET_F = 14, /* Active set update for idle */
  SRCH_IDLE_NSET_F = 15, /* Neighbor set update for idle */
  SRCH_PC_ASET_F = 16,   /* Active set update for paging */
  SRCH_PC_NSET_F = 17,   /* Neighbor set update for paging */
  SRCH_SYS_MEAS_F = 18,  /* System measurement request */
  SRCH_LIST_SIZE_F = 19, /* srch_meas_list_size > 0 */
  SRCH_RESELECT_MEAS_F = 20,  /* Start PC-state system reselection monitoring */
  SRCH_MCC_IDLE_F = 21,        /* MCC's Idle state signal */
  SRCH_MCC_ACC_ENTRY_F = 22,   /* MCC's Access Entry state signal */
  SRCH_MCC_ACC_PROBE_F = 23,   /* MCC's Access (Probe) State signal */
  SRCH_IDLE_FIND_STRONG_PILOT_F = 24,  /* Find the best pilot to entery access on */
  SRCH_START_PPSM_F = 25,      /* Start PPSMM timer */
  SRCH_STOP_PPSM_F = 26,       /* Stop PPSMM timer */
  SRCH_PPSM_F = 27,            /* One time PPSMM reporting */
  SRCH_CF_NSET_F = 28,         /* Candidate Frequency Neighbor Infor. */
  SRCH_START_CFS_F = 29,       /* Start Candidate Freq search */
  SRCH_WTR_HOP_F = 30,         /* Start WTR Hopping */
  SRCH_STOP_CFS_F = 31,        /* Stop Candidate Freq search */
  SRCH_CFS_MEAS_F = 32,        /* Perform pilot measurement on CF */
  SRCH_START_RIF_HHO_F = 33,   /* Start IS95B Hard handoff */
  SRCH_RIF_HHO_SUCCESS_F = 34, /* A successful hard handoff */
  SRCH_RIF_HHO_FAILURE_F = 35, /* An unsuccessful hard handoff */
  SRCH_REACQ_ASET_F = 36,      /* Hard handoff failure, restore the SF */
  SRCH_PILOT_LIST_F = 37,      /* Pilot Reporting request */
  SRCH_TC_ASET_F = 38,         /* Active set update for traffic */
  SRCH_TC_UPDATE_SUP_F = 39,   /* Supplemental channel update */
  SRCH_TC_ADD_SCH_F = 40,      /* Keep until 3GDATA prototype is not needed */
  SRCH_TC_NSET_F = 41,   /* Neighbor set update for traffic */
  SRCH_PC_ASSIGN_F = 42, /* Search pilot list given in paging channel assign */
  SRCH_POWERDOWN_F = 43, /* MC wants to power down - perform housekeeping */
  SRCH_OFF_TX_F = 44,    /* TX is off - hold off autonomous PSMMs temporarily */
  SRCH_ON_TX_F = 45,     /* TX is on now - resume reporting autonomous PSMMs */
  SRCH_RAND_BITS_F = 46, /* MC wants a buffer for A-key seed */
  SRCH_HO_F = 47,        /* MC sends handoff information to Search */
  SRCH_SCRM_MEAS_F = 48, /* SCRM pilot measurements request */
  SRCH_TC_FIND_STRONG_PILOTS_F = 49, /* MC wants a list of pilots */
  SRCH_MC_GPS_VISIT_OKAY_F = 50,
  SRCH_MC_RESUME_CDMA_F = 51,

  SRCH_DEACTIVATE_F = 52,
  SRCH_AMPS_F = 53,
  SRCH_TT_F = 54,        /* begin do HDR to 1x timing transfer */
  SRCH_LTE_1X_TT_F = 55, /* begin to do LTE to 1x timing transfer
                       for reselection */
  SRCH_LTE_NSET_UPDATE_PARAMS_F = 56, /* To get the LTE neighbors info from 1xCP */

  SRCH_TUNE_AWAY_RESP_F = 57,    /* Start or Reject TuneAway Request */          
  SRCH_TUNE_AWAY_PROCEED_F = 58, /* Begin TuneAway */
  SRCH_TUNE_AWAY_CNTL_F = 59,    /* Allow or disallow TuneAway Requests  */    
  SRCH_TUNE_AWAY_INV_PRIO_F = 60, /* CP sends this cmd to Invert the chain priority*/

  SRCH_MAX_F = 61,     /* Tells number of commands. MUST be last enum item. */
...
/* Enumeration of the reasons for which SRCH can receive forced sleep report
 * from CP */
typedef enum
{
  SRCH_FSLEEP_REGULAR,
...
typedef struct
{
  ...
  /* CP sets this boolean if it wants srch to do the next wakeup in QTA */
  boolean   next_wakeup_in_qta;

  /* Forced Sleep reason received in Sleep cmd */  
  srch_forced_sleep_rsn_type forced_sleep_rsn;
...
/*--------------------------------------------------------------------------
                 START WTR HOP
This command tells the searcher to start the WTR HOP.
--------------------------------------------------------------------------*/
