...
$Header: //components/rel/1x.mpss/8.0/srch/inc/srch_genlog_i.h#1 $
...
10/28/15   eye     Added tune away profile data to ZZ log packet.
09/11/15   jh      Added 2nd chain enum to srch_genlog_dev_mode_type bitmask
04/28/15   pap     Adding new log packet for IDLE DIV stats.
...
{
  /* timestamp */
  time_type      start_ta_ts;        /* timestamp at the start of ta */

  /* timetick snapshots */
  uint64  ta_start_trm_tt;           /* QTA start from TRM */
  uint64  ta_stop_trm_tt;            /* QTA stop from TRM */
  uint64  ta_enter_script_start_tt;  /* Start TA-Enter script  */
  uint64  ta_enter_script_stop_tt;   /* Stop TA-Enter script */
  uint64  ta_build_script_start_tt;  /* Start TA-Enter RF script building */
  uint64  ta_build_script_stop_tt;   /* Stop TA-Enter RF script building */
  uint64  ta_exit_script_start_tt;   /* Start TA-Exit script */
  uint64  ta_exit_script_stop_tt;    /* Stop TA-Exit script */
  uint64  ta_fw_start_request_tt;    /* QTA start request to FW */
  uint64  ta_fw_start_response_tt;   /* QTA start response from FW */
  uint64  ta_fw_stop_request_tt;     /* QTA stop request to FW */
  uint64  ta_fw_stop_response_tt;    /* QTA stop response from FW */
  uint64  ta_is_qta;                 /* LTA : FALSE & QTA : TRUE */
  uint64  ta_source_tech;            /* Client which triggered QTA */

} srchzz_tl_prof_ta_type;
...
typedef struct
{
  ...
  srchzz_tl_prof_ta_type             ta;        /* tune away profile data */
...
typedef struct
{
  srch_genlog_sub_header_type  hdr;
  tl_prof_type                 data;
} srch_genlog_zz_prof_sub_v6;

#define SRCH_LOG_TNG_ZZ_PROF_VER 6
...
typedef struct
{
  ...
  uint32               ta_time_sclk;
  uint32               force_lta_mode; 
  
  /*SRCH */
  uint32               srch_ta_start_time;
  uint32               srch_ta_end_time;

  /* TX */
  uint32               tx_ta_start_sclk;
  uint32               r_fch_ta_comp_sclk;
  uint32               r_sch_ta_comp_sclk;

  /* RX */
  uint32               rx_ta_start_sclk;
