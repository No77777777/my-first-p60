...
$Header: //components/rel/1x.mpss/8.0/srch/protected/srchlog.h#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
01/23/12   srk     Feature Cleanup
11/01/10   jtm     Modem 1x SU API changes.
06/10/10   jtm     Eliminate dependency on log_dmss.h by defining Searcher
                   specific log types locally.
01/20/10   mca     Updated PACKED macro
04/01/09   adw     Categorized included header files and standardized format.
03/30/09   adw     Cleaned up includes.
...
/*---------------------------------------------------------------------------
              SPARSELY-SAMPLED AGC AND POWER CONTROL ENTRY TYPE
                            LOG_S_AGC_PCTL_C
---------------------------------------------------------------------------*/
#define SRCH_LOG_AGC_PCTL_CNT  100
...
} srch_log_srch_fing_status_type;
...
typedef PACK(struct) {
    ...
    srch_log_srch_fing_status_type status_mask; /* Finger status */

    byte  reserved1;
    byte  reserved2;

} srch_log_enh_fing_info_type;
...
} srch_log_ded_rev_chan_set_type;
...
} srch_log_ded_fwd_chan_set_type;
...
typedef PACK(struct)
{
  ...
  srch_log_fwd_ch_rc_type fwd_ch_rc;
                     /* Forward channel Radio Configurations */

  srch_log_ded_fwd_chan_set_type active_channels;
                     /* Active forward channel set */

  srch_log_mmse_corr_coefs_type corr_coefs;
  ...
  PACK(union) {
    PACK(struct) {
      srch_log_enh_fing_info_type fings[15];  /* Up to 15 fingers based on 'num_fingers'.
