...
$Header: //components/rel/1x.mpss/8.0/srch/src/common/srch_hw.c#2 $

when       who     what, where, why
--------   ---     ------------------------------------------------------
02/18/20   dmd     Store PSD in int8 instead of byte to avoid errors due 
                   to type casting
11/26/14   jh      remove qw.h include
04/17/14   cjb     Clean-up related to using 1x MSG macros for printing F3s
04/09/14   bph     KW issues
01/23/12   srk     Feature Cleanup
01/06/12   ppr     Mainlined AlwaysOn FEATURE_DH feature
10/31/11   jtm     Feature clean up.
08/30/11   vks     No need to check for mdsp clock before reading tx gain
                   adjust on q6 targets.
08/11/11   vks     Have critical section only around the register access.
...
03/04/10   vks     Removed call to RF macro for closed loop control as the
                   TX_SET_GAIN_ADJUST macro takes care of it.
11/07/09   vks     Removed obsolete srch_set_tx_rate_adj().
08/13/09   jtm     Removed deprecated clk.h include.
08/03/09   adw     Changes to replace deprecated BIO funcs with new DAL API.
                   The TX_SET_GAIN_ADJUST macro no longer checks software
                   switches to see if RPC is disabled.
07/22/09   adw     Removed unnecessary adie.h include.
...
02/25/09   adw     Merge from 1h08 branch, based on VU_MODEM_1X_SRCH.12.00.39
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /* REVISIT: NikeL */
  muxmdsp_set_tx_gain_adjust( tx_adj );
...
int8  srch_get_rx_agc ( void )
{

/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

  /* Get the calibrated AGC from CAGC section */
  return srch_rx_get_rx_agc( CLIENT_1X_RX_PRI );
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /* Get the calibrated AGC from CAGC section */
  return srch_rx_get_rx_agc( CLIENT_1X_RX_SEC );
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  for( i = 0;
       ( i < num_peaks ) &&
       ( i < SRCH_SRCH4_MAX_PEAKS ) &&
       ( srch_rand_buf_cnt < SRCH_MAX_RAND_BUF_SIZE - 1 );
       i++)
  {
    srch_rand_buf[srch_rand_buf_cnt++] = (uint8)(results->max_lecio[i] & 0xFF);
    srch_rand_buf[srch_rand_buf_cnt++] = (uint8)(results->max_pos[i] & 0xFF);
  }

  if (srch_rand_buf_cnt >= SRCH_MAX_RAND_BUF_SIZE)
  {
    (void)rex_set_sigs( DH_TCB_PTR, DH_SRCH_RAND_SIG );
    ...
    M1X_MSG( SRCH, LEGACY_HIGH,
      "Akey buf fill took %ld ms",
      srch_akey_delta_time );
