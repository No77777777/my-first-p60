...
$Header: //components/rel/1x.mpss/8.0/api/srch_srch4.h#1 $ $DateTime: 2019/06/11 23:51:40 $ $Author: pwbldsvc $
...
typedef struct
{
  ...
  struct
  {
    ...
    /* total linear ecio for the sector's individual pri srch */
    uint32                   total_rx0;

    /* total linear ecio for the sector's individual div srch */
    uint32                   total_rx1;

    /* max linear ecio for the sector's individual pri srch */
    uint32                   max_rx0;
    ...
    /* window'ed Level Cross Rate bitmask. "1" equals minimum energy */
    uint32                   lcr_rx0_max;
    uint8                    lcr_rx0_mask;
    uint8                    lcr_rx0_cnt;
    uint32                   lcr_rx1_max;
...
DESCRIPTION    This function converts the peak energies to linear ecio and
                does the total linear ecio calc with filtering.
               This function also records total energy for LCR
                (Level Crossing Rate) Algorithm
