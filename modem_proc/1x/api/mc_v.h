...
$Header: //components/rel/1x.mpss/8.0/api/mc_v.h#2 $ $DateTime: 2020/01/27 00:26:21 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ------------------------------------------------------------
01/22/20   pga     Implement QSH MDUMP framework
...
08/31/17   agh     DRVCC from WiFi to 1X/1XCSFB 
...
08/21/15   ppr     Moved MCC_MAX_DTMFS_OUT macro to mccdma_i.h 
...
enum
{
    ...
    MCC_HDM_TX_ACK_TIMER_ID   = 0x00100000,
    MCC_WTR_HOP_BUF_TIMER_ID  = 0x00200000,
    MCC_TA_HHO_DEFER_TIMER_ID = 0x00300000,
    MCC_TA_SHO_DEFER_TIMER_ID = 0x00400000,
    MCC_TA_CFS_DEFER_TIMER_ID = 0x00500000,
    MCC_TA_COMP_BUF_TIMER_ID = 0x00600000,
    MCC_TA_PRIO_INV_TIMER_ID = 0x00700000,
    MCC_PRI_PCH_HASH_TIMER_ID = 0x00800000,
};
...
/*-------------------------------------------------------------------------*/
/* Minidump crash information*/

#ifdef FEATURE_QSH_MDUMP
#define ONEX_QSH_MDUMP_SET \
do {\
qsh_mdump_set_crash_client(QSH_CLT_ONEXL3,QSH_CLT_ONEXL3); \
qsh_mdump_set_crash_subs_id(QSH_CLT_ONEXL3,SYS_MODEM_AS_ID_1); \
qsh_mdump_set_crash_rat(QSH_CLT_ONEXL3,SYS_SYS_MODE_CDMA);\
qsh_mdump_set_crash_category(QSH_CLT_ONEXL3,QSH_MDUMP_CRASH_CAT_UNKNOWN);\
}while(0)
#else 
#define ONEX_QSH_MDUMP_SET \
do {\
...
/* Error Fatals */
#define ONEX_ERR_FATAL(FMT, A, B, C) \
ONEX_QSH_MDUMP_SET; \
ERR_FATAL(FMT, A, B, C);
...
#define MCC_CHK_NULL_PTR(ptr) \
  if(ptr == NULL){ \
     ONEX_ERR_FATAL("Null mcc Ptr", 0, 0, 0); \
  }

#define MCC_CHK_MAX_INDEX_RANGE(var, max_val) \
  if((var) > (max_val)) { \
    ONEX_ERR_FATAL("Range Error %d %d", var, max_val, 0); \
  }

#define MCC_CHK_MIN_INDEX_RANGE(var, min_val) \
  if((var) < (min_val)) { \
    ONEX_ERR_FATAL("Range Error %d %d", var, min_val, 0); \
  }
...
===========================================================================*/
extern void mc_cdma_abort_redir
(
  sys_modem_as_id_e_type asubs_id
    /* Active Subscription */
);
...
===========================================================================*/
boolean mc_is_cdma_in_rf_wait_state( void );
