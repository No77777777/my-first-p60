...
$Header: //components/rel/1x.mpss/8.0/api/srch_lib_ext.h#1 $
...
typedef struct
{
  ...
  /* Enable and Thresholds for comparing Rx0 and Rx1 AGC/EcIo
     during traffic calls when diversity is enabled */
  const boolean            rx_div_agc_ecio_ck;
  const int16              rx_div_agc_dis_delta_db;
  const int16              rx_div_agc_en_delta_db;
  const int16              rx_div_ecio_dis_delta_q15;
  const int16              rx_div_ecio_en_delta_q15;
...
typedef struct
{
  ...
  const uint16 rf_prep_rf_time_us; /* Overridden if "use_rf_warmup_api" set */
  const uint16 rf_prep_rf_time_non_max_clk_offset_us;
  const uint16 rf_prep_rf_time_lte_offset_us;
  const uint16 rf_prep_clk_overhead_us;
  const uint16 rf_prep_clk_overhead_lte_offset_us;
  const uint16 rf_prep_fw_overhead_us;
  const uint16 rf_prep_fw_overhead_lte_offset_us;
  const uint16 rf_prep_sw_overhead_us;
  const uint16 rf_prep_sw_offtl_overhead_us;
  const uint16 rf_prep_sw_overhead_cgps_offset_us;
  const uint16 rf_prep_sw_overhead_dsds_offset_us;
  const uint16 rf_prep_sw_overhead_lte_offset_us;

  /* RF exec overhead times */
  const uint16 rf_exec_rf_time_us; /* Overridden if "use_rf_warmup_api" set */
  const uint16 rf_exec_rf_time_non_max_clk_offset_us;
  const uint16 rf_exec_rf_time_div_offset_us;
  ...
  const uint16 rf_exec_sw_overhead_dsds_offset_us;
  const uint16 rf_exec_sw_overhead_lte_offset_us;
