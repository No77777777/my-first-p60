...
$Header: //components/rel/1x.mpss/8.0/api/onex_srch_lte_msg.h#1 $
...
--------   ---     ----------------------------------------------------------
04/14/16   pk      Added new abort state ONEX_SRCH_MEAS_SAMPLE_CAPTURED
04/05/16   pk      Ensure measurement aborts are handled correctly, when we
                   have already received pilot meas cfg rsp from FW.
03/25/15   pk      Changes to support new RF Meas API for L to 1x meas
...
} onex_srch_irat_meas_rf_params_target_type;

typedef struct
{
  /*! Pointer to the common RF parameters. Memory for this structure is always 
      allocated by the source tech. Source tech initializes the structure and 
      then sets all of the source fields. */
  rfm_meas_common_param_type *common;
  
  /*! Source tech updates 1x related info */
  onex_srch_irat_meas_rf_params_target_type target;

  struct
  {
    boolean is_gap_aborted;
  } flags;
} onex_srch_irat_meas_rf_params_type;
...
/* ==================================================================== */
/* @deprecated For THOR 2.0 use the new 
 * onex_srch_irat_meas_rf_params_type field */
  /*! RF primary device for measurement */
  rfm_device_enum_type          rf_pri_dev;
...
/* ==================================================================== */
  ...
  /* X21x RF parameter structure */
  onex_srch_irat_meas_rf_params_type rf_param;
} onex_srch_lte_pilot_meas_req_msg_t;
