...
$Header: //components/rel/1x.mpss/8.0/diag/src/onex_non_sig.c#2 $ $DateTime: 2020/08/11 07:59:34 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ------------------------------------------------------------
08/11/20   dmd     Changes for software dipswitches cleanup, new NV ID-74201
08/02/17   pk      FR 37336: Migration to MSGR VARIANT from Inst ID
...
09/10/15   agh     Enable WTR hopping flow for inter-band HHO in non-sig mode
...
/* Need to store the parameters to put TX in traffic state form Init state */
onex_nonsig_cmd_type  onex_wtr_hop_rev_fch_cmd;

byte onex_wtr_hop_tx_frame_offset = 0;
...
)
{
  ...
  /* Enable/Disable Reverse Power Control based on the FTM param enable_rpc */
  {
    ...
    if (rpc_active)
    {
      ...
      /* Enable closed loop power control */
	  #ifndef FEATURE_GSM_PLT
      cdma2kdiag_clear_switch( CDMA2KDIAG_SW_DISABLE_RL_PWR_CTRL );
    ...
    else
    {
      ...
      cdma2kdiag_set_switch( CDMA2KDIAG_SW_DISABLE_RL_PWR_CTRL );
      ...
      while( !exit_loop )
      {
        switch( sub_state )
        {
          ...
          case TCI_3:
          {
            ...
            onex_wtr_hop_tx_frame_offset = cmd_ptr->demod_fch.param.frame_offset;
      ...
      while( !exit_loop )
      {
        switch( sub_state )
        {
          ...
          case TCI_3:
          {
            ...
            onex_wtr_hop_tx_frame_offset = cmd_ptr->demod_dcch.param.frame_offset;
        ...
        switch( sub_state )
        {
          case TCI_4:
          {
            ...
            */
            ...
            onex_wtr_hop_tx_frame_offset = cmd_ptr->mod_dcch.param.frame_offset;
...
)
{
  ...
  if( cmd_ptr != NULL )
  {
        ...
        ( onex_nonsig_params.ns_rev_curr_state == NS1X_REV_TRAFFIC ) )
    {
      ...
      /* Check for WTR hopping here. If WTR hopping is required shutdown
       * before asking SRCH to tune to new band/channel */
      if( srch_rx_check_wtr_hopping(
                    (srch_rx_band_type)cmd_ptr->fwd_hho.param.band_class,
                    ...
                    == SRCH_RX_BAND_HOP )
      {
        ...
        M1X_MSG_SSID( DIAG, LEGACY_HIGH,
             "1xNonSig: WTR_Hop - WTR hopping required, sending TXC_EXIT_F" );
      ...
      /* Renable TX after WTR hopping procedures have been finished */
      if( wtr_hop_required )
      {
        M1X_MSG_SSID( DIAG, LEGACY_HIGH,
           "1xNonSig: WTR_Hop - Re-enabling TX after handoff, stored Rev FCH cmd:%d",
           onex_wtr_hop_rev_fch_cmd.hdr.cmd );

        M1X_MSG_SSID( DIAG, LEGACY_HIGH,
          "1xNonSig: WTR_HOP - Send TXC_CDMA_F." );
        mcc_tx_buf.cdma.hdr.command = TXC_CDMA_F;
        mcc_txc_cmd( &mcc_tx_buf );

        M1X_MSG_SSID( DIAG, LEGACY_HIGH,
          "1xNonSig: WTR_HOP - Send TXC_PC_F." );
        mcc_tx_buf.pc.hdr.command = TXC_PC_F;
        mcc_txc_cmd( &mcc_tx_buf );

        M1X_MSG_SSID( DIAG, LEGACY_HIGH,
          "1xNonSig: WTR_HOP - Send TXC_FRAME_OFFSET_F.");
        mcc_tx_buf.hdr.command = TXC_FRAME_OFFSET_F;
        mcc_tx_buf.fo.frame_offset = onex_wtr_hop_tx_frame_offset;
        ...
        switch( onex_wtr_hop_rev_fch_cmd.hdr.cmd )
        {
          case NS1X_CMD_MOD_DCCH:
            response_status = nonsig_rev_dcch( &onex_wtr_hop_rev_fch_cmd );
            break;

          case NS1X_CMD_MOD_FCH_DCCH:
            response_status = nonsig_rev_fch_dcch( &onex_wtr_hop_rev_fch_cmd );
            break;

          case NS1X_CMD_MOD_FCH:
            response_status = nonsig_rev_fch( &onex_wtr_hop_rev_fch_cmd );
            break;

          default:
            M1X_MSG_SSID( DIAG, LEGACY_MED,
              "1xNonSig: WTR_Hop - Unknown cmd %d",
              onex_wtr_hop_rev_fch_cmd.hdr.cmd );
            response_status = NS1X_TEST_INVALID_CMD;
        }
        M1X_MSG_SSID( DIAG, LEGACY_HIGH,
             "1xNonSig: WTR_Hop - TX traffic  procedures done, rsp_status: %d",
              response_status );

        fwd_hho_status = response_status;
      }
      else
      {
...
)
{
  ...
  switch (cmd_ptr->hdr.cmd)
  {
    ...
    case NS1X_CMD_MOD_DCCH:
    {
      /* Keep a copy of REV FCH paramters which may be required for WTR hopping */
      memset(&onex_wtr_hop_rev_fch_cmd, 0, sizeof(onex_wtr_hop_rev_fch_cmd));

      copy_size = memscpy(&onex_wtr_hop_rev_fch_cmd, sizeof(onex_wtr_hop_rev_fch_cmd),
                          cmd_ptr, sizeof(onex_nonsig_cmd_type));
      if( copy_size != sizeof(onex_nonsig_cmd_type) )
      {
        M1X_MSG( DCP, LEGACY_ERROR,
          "Truncated memscpy, bytes_copied:%d, src_size:%d, dest_size:%d",
          copy_size,
          sizeof(onex_nonsig_cmd_type),
          sizeof(onex_wtr_hop_rev_fch_cmd));
      }
