...
$Header: //components/rel/1x.mpss/8.0/diag/src/cdma2kdiag.c#4 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
08/11/20   dmd     Reading EFS(74201) during boot-up only
08/11/20   ppr     Disable all dip_switch settings by default
...
#include "onex_nv_rd_wt.h"  /* For EFS */
#include "mcfg_fs.h"
...
DESCRIPTION    This function sets  default value for    cdma2kdiag switches
...
{
  /* By default disable all Dip-Switches */
  uint32 dip_sw_state = 0;

  return dip_sw_state;
				   

} /* cdma2kdiag_get_default_dipswitch_state */
...
===========================================================================*/
void srch_cdma2kdiag_dipswitch_read_nv_item()
{
  
  /* Return value from EFS interface */
  boolean status;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  /* Query the current state in efs */
  status = onex_efs_read( SRCH_EFS_DIAG_EFS_FILENAME, &dip_sw_state, sizeof( dip_sw_state ) );

  /* Check for an unsuccessful read */
  if ( status != TRUE )
  {
	/*Get default value*/
	dip_sw_state = cdma2kdiag_get_default_dipswitch_state();
	
    M1X_MSG_SSID( DIAG, LEGACY_ERROR,
                  "Failed to read EFS item Using default Present state %x ",
                  dip_sw_state);
  }

  M1X_MSG_SSID( DIAG, LEGACY_LOW,
                "Read EFS file EFS returned %d Present state %x ",
                status,
                dip_sw_state);
...
RETURN VALUE   Returns the state of the dipswitch. 
...
)
{
  ...
  is_dip_set = (dip_sw_state >> dip_sw) & 1 ;

  M1X_MSG_SSID( DIAG, LEGACY_LOW,
                "Got switch %d Present state %x dipswitch value %d",
                dip_sw,
                dip_sw_state,
                is_dip_set);
...
)
{
  ...
  /* New state of the dipswitch */
  uint32 dip_sw_new_state = 0;

  /* for shifting of bits*/
  uint32 dip_sw_bit = 1;

  /* Return value from EFS file interface */
  boolean status;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
   ...
   dip_sw_bit = dip_sw_bit << dip_sw; 
		
   /* Update the dipswitch mask state to set the new switch */
   dip_sw_new_state = dip_sw_state | dip_sw_bit;

   /* Set the specified dipswitch using the updated mask */
   status = onex_efs_write( SRCH_EFS_DIAG_EFS_FILENAME,
                            &dip_sw_new_state,
                            length,
                            oflag,
                            mode );
   ...
   {
     M1X_MSG_SSID( DIAG, LEGACY_ERROR,
        	       "Failed to set switch %d: EFS returned %d  State is %x",
                   dip_sw,
                   status,
                   dip_sw_state );
   }
   ...
   {
	 M1X_MSG_SSID( DIAG, LEGACY_LOW,
        	       "Set switch %d: EFS returned %d Old State was %x New State is %x",
       	 	       dip_sw,
       		       status,
       		       dip_sw_state,
        	       dip_sw_new_state );

	 dip_sw_state = dip_sw_new_state;
   }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
   ...
   {
     M1X_MSG_SSID( DIAG, LEGACY_ERROR,
	 	           "Failed to set switch %d: EFS returned %d Present State is %x",
                   dip_sw,
                   status,
                   dip_sw_state );
   }
