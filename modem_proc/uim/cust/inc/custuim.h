  ...
  $Header: //components/rel/uim.mpss/6.1.0/cust/inc/custuim.h#2 $
  ...
  08/23/16   vdc     Enable FEATURE_UIM_MULTI_PROFILE when ECALL & ERA GLONASS enabled
  06/20/16   ar      Define FEATURE_MMGSDI_GSM
  06/09/16   shr     Deprecate QMI SIM Internal
  04/25/16   sam     Enable feature UIM_PUP_LOGGING_PRINT_F3 under FEATURE_CUST_1
  04/20/16   sam     Support for enhanced subscription manager (TSDS feature)
  03/28/16   sam     Support for SAM card
  03/25/16   ssr     Removal of some features for IoE flavor
  03/11/16   ar      Remove FEATURE_UIMDIAG
  12/01/15   ll      Removing compile time disable polling feature flag 
  10/12/15   ll      Re-eable FEATURE_SIMLOCK after crypto and SFS avalibility  
  10/02/15   ll      Temporarily disable FEATURE_SIMLOCK for 9x55 bringup
  08/28/15   hh      Removed JCDMA related code
  08/17/15   ll      Enable HOTSWAP by default for CUST_1
  07/13/15   ll      Temporarily disable FEATURE_SIMLOCK on RUMI
  06/30/15   vv      Re-enable FEATURE_SIMLOCK on RUMI
  05/19/15   sam     Disable FEATURE_UIM_POWERUP_LOGGING if FEATURE_CUST_1 is enabled
  06/08/15   ks      Customize slot2 LDO settings if FEATURE_CUST_1 is enabled
  05/19/15   sam     Disable FEATURE_UIM_T_1_SUPPORT if FEATURE_CUST_1 is enabled
  05/15/15   stv     Add SIMLOCK feature to protect QMI LOC changes
  04/30/15   gm      Using FEATURE_CGPS_LOC_MIDDLEWARE to enable FEATURE_GSTK_QMI_LOC
  04/29/15   gm      Enable FEATURE_GSTK_QMI_LOC when QMI_LOC interface is present
  ...
  03/10/15   vv      Disable FEATURE_SIMLOCK on RUMI
  01/09/15   vv      Removed unused feature flags
  01/07/15   ll      Temp disabling FEATURE_POWERUP_LOGGING before optimization
  01/07/15   sam     Enabling FEATURE_POWERUP_LOGGING
  01/05/15   sam     Temporarily disabling FEATURE_POWERUP_LOGGING
  01/02/15   sam     Addintion of new feature FEATURE_POWERUP_LOGGING
  ...
  /* Customize slot2 LDO settings */
  /* Customize enable HOTSWAP by default */
  /* Customize ontime HOTSWAP on UIM2*/
  /* Disable T=1 support */
  /* Disable power up logging */
  #ifdef FEATURE_CUST_1
    #define FEATURE_CUSTOMIZE_SLOT2_LDO_SETTINGS
    #define FEATURE_ENABLE_HOTSWAP_DEFAULT
    #define FEATURE_UIM_ONETIME_HOTSWAP_UIM2
    #undef  FEATURE_UIM_T_1_SUPPORT
    #define UIM_PUP_LOGGING_PRINT_F3
  #endif
  ...
  /* MMGSDI Task support */
    ...
    /* Disable personalization when FEATURE_CUST_1 is defined
       or in case of RUMI target */
    #if defined(FEATURE_CUST_1)|| defined(T_RUMI_EMULATION)
      /* PERSONALIZATION Features */
      #undef FEATURE_SIMLOCK
    #endif /* !FEATURE_CUST_1 && !T_RUMI_EMULATION */
    ...
    #if defined(FEATURE_ECALL_APP) && defined (FEATURE_ECALL_HAS_ERA_GLONASS)
      #define FEATURE_UIM_MULTI_PROFILE
    #endif /* FEATURE_ECALL_APP && FEATURE_ECALL_HAS_ERA_GLONASS */
  #endif /* FEATURE_MMGSDI */

  #ifdef FEATURE_QMI_UIM_SECURE_CHANNEL_TEST
    #error "QMI UIM Secure channel is disabled"
 ...
 /*=========================================================================== */
  ...
  #if defined(FEATURE_CGPS) && defined(FEATURE_CGPS_LOC_MIDDLEWARE)
    #define FEATURE_GSTK_QMI_LOC
  #endif
