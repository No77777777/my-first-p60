...
Copyright (c) 2013 - 2017 QUALCOMM Technologies, Incorporated and its licensors.
...
$Header: //components/rel/uim.mpss/6.1.0/uimqmi/src/uimqmi_task.c#2 $ $DateTime: 2019/10/30 06:56:05 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
08/08/17   me      Do not un-register from MCS EVT in off-target
06/28/17   me      De-register for QMI task ready signals only once 
...
rex_tcb_type                   * uimqmi_tcb_ptr                = NULL;
...
)
{
  ...
  uimqmi_tcb_ptr = rcinit_lookup_rextask("uimqmi");
...
)
{
       ...
       (0x00000001 << (uint8) UIMQMI_TASK_TYPE_QMI_MODEM)) == FALSE)
  {
    ...
    /* RCEVT registration/de-reg. is done only on-target since off-target support is not present for QMI Modem task */
...
)
{
  ...
  for (;;)
  {
    ...
    {
      uimqmi_task_handle_qmi_framework_ready(&uimqmi_qmi_modem_ready_sig);
    }
