...
$Header: //components/rel/uim.mpss/6.1.0/uimqmi/src/qmi_cat_decoded.h#2 $$ $DateTime: 2019/10/30 06:56:05 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     -----------------------------------------------------------
07/11/17   vr      Fill indication token for failure envelope command
...
typedef struct
{
  ...
  PACKED struct PACKED_POST
  {
    uint32          cmd_ref_id;
    uint16          tr_len;
    uint8           terminal_response[QMI_CAT_RAW_PROACTIVE_CMD_MAX_LENGTH_V02];
  }                 term_resp;
  PACKED struct PACKED_POST
  {
    uint8                                    slot;
...
typedef struct
{
  ...
  PACKED struct PACKED_POST
  {
    uint16              env_cmd_type;
    uint16              env_cmd_len;
    uint8               envelope_data[QMI_CAT_RAW_PROACTIVE_CMD_MAX_LENGTH_V02];
  }                     envelope;
  PACKED struct PACKED_POST
  {
    uint8                                    slot;
...
typedef struct
{
  ...
  boolean                                    slot_present;
  boolean                                    battery_status_present;
  boolean                                    ind_token_present;
  ...
  PACKED struct PACKED_POST
  {
    ...
    uint8       additional_info_data[QMI_CAT_TR_ADDITIONAL_INFO_MAX_LENGTH_V02];
  }                                          term_resp;
  PACKED struct PACKED_POST
  {
    uint8           dcs;
...
typedef struct
{
  ...
  boolean                                    contactless_state_request_present;
  boolean                                    ind_token_present;
  boolean                                    ims_uri_truncated_present;
  ...
  {
    uint8             length;
    uint8             pdp_act_param[QMI_CAT_PDP_CONTEXT_ACT_MAX_LENGTH_V02];
  }                   pdp_context_act;
  PACKED struct PACKED_POST
  {
    uint8             length;
    uint8             eps_act_param[QMI_CAT_PDP_CONTEXT_ACT_MAX_LENGTH_V02];
  }                   eps_pdn_connect_act;
  PACKED struct PACKED_POST
  {
  ...
  {
    uint8                 length;
    uint8                 tpdu[QMI_CAT_SMS_TPDU_MAX_LENGTH_V02];
  }                       sms_tpdu;
  PACKED struct PACKED_POST
  {
    boolean                                   is_cdma_sms;
  ...
  {
    uint8                 length;
    uint8                 transaction_id[QMI_CAT_TX_ID_MAX_LENGTH_V02];
  }                       transaction_id;
  PACKED struct PACKED_POST
  {
    uint8                     ton;
  ...
  {
    uint8                   length;
    uint8                   cause[QMI_CAT_CAUSE_MAX_LENGTH_V02];
  }                         cause;
  PACKED struct PACKED_POST
  {
    uint8                   length;
    uint8                   ims_uri[QMI_CAT_IMS_URI_MAX_LENGTH_V02];
  }                         ims_uri;
  PACKED struct PACKED_POST
  {
    uint32                                    contactless_state_request;
  }                                           contactless_state_request;
  PACKED struct PACKED_POST
  {
    uint32                                   ind_token;
  }                                          ind_token;
...
typedef struct
{
  ...
  {
    uint8               tp_len;
    uint8               tp_bitmask[QMI_CAT_TERMINAL_PROFILE_MAX_LENGTH_V02];
  }                     custom_tp;
