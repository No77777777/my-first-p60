...
$Header: //components/rel/uim.mpss/6.1.0/uimqmi/src/qmi_uim_encryption.c#2 $$ $DateTime: 2019/10/30 06:56:05 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     -----------------------------------------------------------
07/04/17   dd      Remove feature FEATURE_UIM_TEST_FRAMEWORK
11/04/16   me      Combined multi-line ASSERT in one line
10/28/16   nr      Support remote and local configurations together
08/25/16   vdc     Fix compiler error when secure channel test is enabled
...
05/18/15   vdc     Remove feature FEATURE_SEC_HW_CRYPTO
03/10/15   vdc     Replace hard coded values with IDL defined values
02/16/15   ar      Pass large buffer to SECAPI for decryption
02/10/15   at      Featurize call to secapi_util_kdf
02/05/15   hh      Fix off target compilation error
...
)
{
  ...
  encrypted_data_ptr->data_len = input_data.data_len;
  ...
  encrypted_data_ptr->data_len += QMI_UIM_EXTRA_ENCRYPTION_BYTES;
  ...
  if (input_data.data_len > encrypted_data_ptr->data_len)
  {
    UIM_MSG_ERR_2("Invalid input len: 0x%x, output len: 0x%x",
                  input_data.data_len,
                  encrypted_data_ptr->data_len);
    return FALSE;
  }

  (void)memscpy(encrypted_data_ptr->data_ptr,
                encrypted_data_ptr->data_len,
                input_data.data_ptr,
                input_data.data_len);
#endif /* FEATURE_QMI_UIM_SECURE_CHANNEL_TEST */
...
)
{
  ...
  ASSERT(in_data_len > 0 && in_data_ptr && out_hmac_data_ptr && out_hmac_data_len_ptr);
...
)
{
  ...
  ASSERT(in_data_len > 0 && in_data_ptr && out_data_ptr && out_data_len_ptr && iv_ptr);
...
)
{
  ...
  ASSERT(in_pin_length > 0 && in_pin_value_ptr && out_pin_value_ptr && out_pin_length_ptr);
  ...
  if (output_pin_info.pin_len == 0 ||
      output_pin_info.pin_len > QMI_UIM_PIN_MAX_V01 ||
...
)
{
  ...
  ASSERT(in_data_len > 0 && in_data_ptr && out_data_ptr && out_data_ptr && out_data_len_ptr && iv_ptr);
