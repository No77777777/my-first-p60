...
Copyright (c) 2013 - 2016, 2020, 2022 QUALCOMM Technologies, Incorporated and its licensors.
...
$Header: //components/rel/uim.mpss/6.1.0/uimqmi/src/qmi_uim_remote.c#4 $ $DateTime: 2022/04/22 08:40:54 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
04/22/22   ykr     Fix to integer overflow issue in uim_remote_apdu handler
03/02/20   vdc     Fix to incorrect slot ID passed to check ongoing connection
07/05/16   ar      Invalidate the slot on receiving conn_unavail req
05/16/16   bcho    Deregister qmi uim remote service on task stop
04/11/16   lm      Fix KW Error: memset local struct variable
03/02/16   ks      Return error to client when card service is not found
03/01/16   vr      Initialize the client_register slot value to invalid
11/18/15   ks      Do not allow connect if already connected to other slot
08/11/15   hyo     usb uicc removal
07/15/15   at      Send error to client if already in Connection Available
...
/* Global to store all the QMI UIM remote service related variables */
static qmi_uim_remote_state_type * qmi_uim_remote_state_ptr = NULL;
...
{
  boolean is_service_enabled = FALSE;
  ...
  if (UIM_COMMON_EFS_SUCCESS == uim_common_efs_read(UIM_COMMON_EFS_UIMQMI_QMI_UIM_CONFIG_REMOTE_SERVICE,
                                                   ...
                                                   (uint8 *)&(is_service_enabled),
                                                    sizeof(is_service_enabled)))
  {
    qmi_uim_remote_state_ptr->service_enable = is_service_enabled;
  }
  else
  {
    qmi_uim_remote_state_ptr->service_enable = FALSE;
  }

  UIM_MSG_HIGH_1("QMI UIM remote service enabled 0x%x",
                      qmi_uim_remote_state_ptr->service_enable);
...
)
{
  ...
  if (qmi_uim_remote_state_ptr == NULL)
  {
    UIM_MSG_ERR_0("Cannot do remote service inti remote state ptr is NULL");
    return;
  }  
  if(qmi_uim_remote_state_ptr->service_enable  == FALSE)
  {
    return;
  }
...
/*===========================================================================
   FUNCTION:      QMI_UIM_REMOTE_CHECK_IF_CLIENT_IS_CONNECTED_WITH_OTHER_SLOT

   DESCRIPTION:
     This function checks the client table to see if this client is already in 
     connection with any other slot.
     Note that the connection is assumed to ongoing until the client disconnects
     by sending event request with the event as connection unavailable.
   ...
   SIDE EFFECTS:
     None
===========================================================================*/
static boolean qmi_uim_remote_check_if_client_is_connected_with_other_slot (
  qmi_uim_remote_slot_type   in_slot,
  qmi_client_handle          in_client_handle
)
{
  uint8   client_idx = 0;

  if (in_slot > QMI_UIM_REMOTE_SLOT_3)
  {
    return FALSE;
  }

  for (client_idx = 0;
       client_idx < QMI_UIM_REMOTE_MAX_NUM_CARD_CLIENT;
       client_idx++)
  {
    if (qmi_uim_remote_state_ptr->card_client_registry[client_idx].in_use           &&
        qmi_uim_remote_state_ptr->card_client_registry[client_idx].slot != in_slot  &&
        qmi_uim_remote_state_ptr->card_client_registry[client_idx].client_handle ==
          in_client_handle                                                          &&
        qmi_uim_remote_state_ptr->card_client_registry[client_idx].last_evt_data.evt >
          QMI_UIM_REMOTE_EVT_CONNECTION_UNAVAILABLE)
    {
      return TRUE;
    }
  ...
  return FALSE;
} /* qmi_uim_remote_check_if_client_is_connected_with_other_slot */


/*===========================================================================
   FUNCTION:      QMI_UIM_REMOTE_CHECK_ONGOING_CONNECTION

   DESCRIPTION:
     This function checks the client table for any ongoing connection. Note
     that the connection is assumed to ongoing until the client disconnects
     by sending event request with the event as connection unavailable.
   ...
   SIDE EFFECTS:
     None
===========================================================================*/
static boolean qmi_uim_remote_check_ongoing_connection (
  qmi_uim_remote_slot_type   in_slot,
  qmi_client_handle          in_client_handle
)
{
  ...
  return FALSE;
} /* qmi_uim_remote_check_ongoing_connection */
...
)
{
  ...
  for (card_client_index = 0;
       ...
       card_client_index++)
  {
    if (evt.slot <= QMI_UIM_REMOTE_SLOT_3                                            &&
        qmi_uim_remote_state_ptr->card_client_registry[card_client_index].in_use     &&
        qmi_uim_remote_state_ptr->card_client_registry[card_client_index].client_handle ==
          card_client_handle_ptr->client_handle)
    {
      /* If there was already a client whose connection is in progress, reject any event
         request from this client. Note that we dont store any event info for this client */
      if (qmi_uim_remote_check_ongoing_connection(evt.slot,
                                                  card_client_handle_ptr->client_handle))
      {
        UIM_MSG_ERR_1("Cannot proceed, another client connected on slot: 0x%x", evt.slot);
        resp.resp.error  = QMI_ERR_ACCESS_DENIED_V01;
        resp.resp.result = QMI_RESULT_FAILURE_V01;
        goto send_resp;
      }

      /* If the existing client is already connected with one slot, reject any event
         request from this client for a different slot. */
      if (qmi_uim_remote_check_if_client_is_connected_with_other_slot(evt.slot,
                                                  card_client_handle_ptr->client_handle))
      {
        UIM_MSG_ERR_1("Cannot proceed, client is already connected on another slot 0x%x", 
                       qmi_uim_remote_state_ptr->card_client_registry[card_client_index].slot);
        resp.resp.error  = QMI_ERR_ACCESS_DENIED_V01;
        resp.resp.result = QMI_RESULT_FAILURE_V01;
        goto send_resp;
...
)
{
  ...
  /* If there was already a client whose connection is in progress, reject any APDU
     request from this client. Note that we dont store any event info for this client */
  if (qmi_uim_remote_check_ongoing_connection(qmi_uim_remote_decode_slot(req_msg_ptr->slot),
                                              card_client_ptr->client_handle))
  {
    UIM_MSG_ERR_1("Cannot proceed, another client connected on slot: 0x%x", req_msg_ptr->slot);
    resp.resp.error  = QMI_ERR_ACCESS_DENIED_V01;
    resp.resp.result = QMI_RESULT_FAILURE_V01;
    goto send_resp;
  }
...
{
  if((cmd_ptr == NULL) || (struct_len_ptr == NULL) || (msg_id_ptr == NULL) ||
...
{
  if((cmd_ptr == NULL) || (struct_len_ptr == NULL) || (msg_id_ptr == NULL) ||
     (ind_c_struct_ptr == NULL) || (qmi_uim_remote_state_ptr == NULL))
  {
    return QMI_CSI_INTERNAL_ERR;
  }
...
)
{
  ...
  if((cmd_ptr == NULL) || (qmi_uim_remote_state_ptr == NULL))
  {
    return;
  }
...
)
{
  ...
  if((cmd_ptr == NULL) || (qmi_uim_remote_state_ptr == NULL) ||
     (cmd_ptr->cmd_data.reg_req.event_cb_ptr == NULL))
  {
    return;
  }
