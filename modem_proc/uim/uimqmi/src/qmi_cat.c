...
Copyright (c) 2005 - 2017, 2019 QUALCOMM Technologies, Inc (QTI) and its licensors.
...
/*===========================================================================
  ...
  $Header: //components/rel/uim.mpss/6.1.0/uimqmi/src/qmi_cat.c#2 $ $DateTime: 2019/10/30 06:56:05 $ $Author: pwbldsvc $

when        who    what, where, why
--------    ---    ----------------------------------------------------------
09/16/19    dt     Handling of GSTK_ACCESS_NONE
05/23/19    ks     Avoid double freeing tlv_ptr when decoding poll interval evt_dl 
11/07/17    dt     Correct the USSD mask to set full functionality
10/31/17    vr     Send immediate response for decoded envelope with rsp data
06/28/17    dt     Hold SEND SMS till CP ACK for SMS PP is received from WMS.
07/11/17    vr     Fill indication token for failure envelope response
06/07/17    dt     Adding SMS-PP support on requested slot.
05/25/17    dt     Removing QMI_CAT_MODE_CUSTOM_TP_DL_MAX_VALUE 
04/17/17    dt     Enable Data Card Config. if QMI CAT is not supported
03/14/17    dt     Corrected sending additional terminal response
11/04/16    me     Combined multi-line ASSERT in one line 
11/02/16    vr     Long IMS URI in CC envelope command
10/24/16    gs     Changes in TP based on RAT and App Type
09/06/16    vr     Cache the raw GET_INPUT command
08/29/16    vr     QMICAT registration with GSTK in qmi_cat_pre_task_startup
05/12/16    tkl    Remove reference to uim_get_service_object_v01
04/14/16    shr    Pass SMS PP payload info for 62xx/63xx status
04/12/16    tkl    Add indication support for TR and Envelope cmd
04/11/16    ar     Support for eUICC
02/05/16    vr     Validate event confirmation
02/04/16    tkl    Add client operation check against QMI CAT config mode
...
10/09/15    vr     Fix for STK not displayed with security activated
09/08/15    vr     Split QMI CAT SCWS
...
04/27/15    gm     Added support for class "n" command
04/16/15    yt     Add class "g" support
04/14/15    vr     Fix custom TP update failure
04/03/15    kk     Migrate to mcfg EFS wrappers
02/23/15    tkl    Add class r TP support to NV
02/23/15    hh     Added Class R support
02/18/15    hh     qmi_cati_read_configuration in pre_task_startup
...
#define CATI_PRM_TYPE_CONTACTLESS_STATE_CHANGED        (0X6B)
#define CATI_PRM_TYPE_GEOGRAPHICAL_LOCATION            (0X6D)
...
#define CATI_REPORT_STATUS_NONE                                (0x00000000)
...
#define QMI_CAT_GSTK_EVT_MASK_CLASSR_CONTACTLESS (GSTK_REG_CONTACTLESS_STATE_REQ_EVT | \
                                                  GSTK_REG_CONTACTLESS_STATE_CHANGED_EVT)

#define QMI_CAT_GSTK_EVT_MASK_CLASSN_DISPLAY_GEOGRAPHICAL_LOCATION (GSTK_REG_GEO_LOC_REQ_EVT)
...
static gstk_toolkit_cmd_reg_info_type  qmi_cati_complete_reg_list[] =
{
  ...
  { { GSTK_REG_CATEGORY_R_CMD, QMI_CAT_GSTK_EVT_MASK_CLASSR_CONTACTLESS}, GSTK_HANDLE_ALL_FUNC},
  { { GSTK_REG_CATEGORY_N_CMD, QMI_CAT_GSTK_EVT_MASK_CLASSN_DISPLAY_GEOGRAPHICAL_LOCATION}, GSTK_HANDLE_DISPLAY_FUNC_ONLY}
};
...
static gstk_profile_dl_support_ext_type qmi_cati_decoded_profile_value_list[] =
{
  ...
  /* TP for class r support are disabled by default */
  {1, GSTK_CONTACTLESS_STATE_REQ_EVT_SUPPORT,        (uint8)GSTK_APP_USIM},
  {1, GSTK_CONTACTLESS_STATE_CHANGED_SUPPORT,        (uint8)GSTK_APP_USIM},
  {1, GSTK_PROVIDE_LOCAL_INFO_BATTERY_STATE_SUPPORT, (uint8)GSTK_APP_USIM}
};
...
#define CATI_RAW_PROACTIVE_CMD_MASK ((QMI_CAT_EVT_REPORT_REQ_DISPLAY_TEXT_MASK_V02)       | \
                                     ...
                                     (QMI_CAT_EVT_REPORT_REQ_BEARER_INDEPENDENT_PROTOCOL_MASK_V02)           | \
                                     (QMI_CAT_DEC_EVT_REPORT_REQ_SCWS_EVENT_MASK_V02)           | \
                                     (QMI_CAT_EVT_REPORT_REQ_ACTIVATE_MASK_V02)           | \
                                     (QMI_CAT_EVT_REPORT_REQ_REFRESH_MASK_V02)            | \
                                     (QMI_CAT_EVT_REPORT_REQ_CONTACTLESS_SUPPORT_MASK_V02) |\
				     (QMI_CAT_EVT_REPORT_REQ_GEOGRAPHICAL_LOCATION_MASK_V02))

#define CATI_SETUP_EVT_LIST_MASK ((QMI_CAT_EVT_REPORT_REQ_SETUP_EVENT_USER_ACTIVITY_MASK_V02) | \
                            ...
                            (QMI_CAT_EVT_REPORT_REQ_SETUP_EVENT_BROWSER_TERMINATION_MASK_V02)   | \
                            (QMI_CAT_EVT_REPORT_REQ_SETUP_EVENT_HCI_CONN_MASK_V02)       | \
                            (QMI_CAT_EVT_REPORT_REQ_CONTACTLESS_SUPPORT_MASK_V02))

#define CATI_PROVIDE_LOCAL_INFO_MASK ((QMI_CAT_EVT_REPORT_REQ_PROVIDE_LOCAL_INFO_LANGUAGE_MASK_V02) | \
                            (QMI_CAT_EVT_REPORT_REQ_PROVIDE_LOCAL_INFO_TIME_MASK_V02)|\
			    (QMI_CAT_EVT_REPORT_REQ_PROVIDE_LOCAL_INFO_BATTERY_MASK_V02))

#define CATI_CACHED_EVT_MASK ((QMI_CAT_EVT_REPORT_REQ_DISPLAY_TEXT_MASK_V02)           | \
                              (QMI_CAT_EVT_REPORT_REQ_SETUP_MENU_MASK_V02)             | \
                              (QMI_CAT_EVT_REPORT_REQ_SETUP_IDLE_MODE_TEXT_MASK_V02)    | \
...
#define CATI_DECODED_PROACTIVE_CMD_MASK ((QMI_CAT_DEC_EVT_REPORT_REQ_DISPLAY_TEXT_MASK_V02)            | \
                                         (QMI_CAT_DEC_EVT_REPORT_REQ_GET_INKEY_MASK_V02)               | \
                                         (QMI_CAT_DEC_EVT_REPORT_REQ_GET_INPUT_MASK_V02)               | \
                                         (QMI_CAT_DEC_EVT_REPORT_REQ_SETUP_MENU_MASK_V02)              | \
                                         (QMI_CAT_DEC_EVT_REPORT_REQ_SELECT_ITEM_MASK_V02)             | \
                                         (QMI_CAT_DEC_EVT_REPORT_REQ_SEND_SMS_MASK_V02)                | \
                                         (QMI_CAT_EVT_REPORT_REQ_SETUP_EVENT_USER_ACTIVITY_MASK_V02)       | \
                                         (QMI_CAT_EVT_REPORT_REQ_SETUP_EVENT_IDLE_SCREEN_NOTIF_MASK_V02) | \
                                         (QMI_CAT_EVT_REPORT_REQ_SETUP_EVENT_LANGUAGE_SEL_NOTIF_MASK_V02) | \
                                         (QMI_CAT_EVT_REPORT_REQ_SETUP_IDLE_MODE_TEXT_MASK_V02)    | \
                                         ...
                                         (QMI_CAT_EVT_REPORT_REQ_ACTIVATE_MASK_V02)                | \
                                         (QMI_CAT_EVT_REPORT_REQ_SETUP_EVENT_HCI_CONN_MASK_V02)       | \
                                         (QMI_CAT_DEC_EVT_REPORT_REQ_BEARER_INDEPENDENT_PROTOCOL_STATUS_MASK_V02)         | \
                                         (QMI_CAT_EVT_REPORT_REQ_REFRESH_MASK_V02)                 | \
                                         (QMI_CAT_EVT_REPORT_REQ_CONTACTLESS_SUPPORT_MASK_V02)|\
                                         (QMI_CAT_EVT_REPORT_REQ_GEOGRAPHICAL_LOCATION_MASK_V02)|\
                                         (QMI_CAT_DEC_EVT_REPORT_REQ_PROVIDE_LOCAL_INFO_BATTERY_MASK_V02))

#define CATI_DECODED_CACHED_EVT_MASK ((QMI_CAT_DEC_EVT_REPORT_REQ_DISPLAY_TEXT_MASK_V02)               | \
                                      (QMI_CAT_DEC_EVT_REPORT_REQ_SETUP_MENU_MASK_V02)                 | \
                                      (QMI_CAT_EVT_REPORT_REQ_SETUP_IDLE_MODE_TEXT_MASK_V02)       | \
                                      (QMI_CAT_EVT_REPORT_REQ_SETUP_EVENT_USER_ACTIVITY_MASK_V02)          | \
                                      (QMI_CAT_EVT_REPORT_REQ_SETUP_EVENT_IDLE_SCREEN_NOTIF_MASK_V02)    | \
...
const static qmi_cat_decoded_cache_cmd_index_type  qmi_cat_decoded_cache_cmd_index[] =
{
  ...
  {GSTK_SETUP_EVT_CONTACTLESS_STATE_REQ_NOTIFY, CATI_CACHED_EVT_DECODED_SETUP_EVT_CONTACTLESS_S_REQ},
  {GSTK_DISPLAY_TEXT_REQ,                       CATI_CACHED_EVT_DECODED_DISPLAY_TEXT},
  /* removed SCWS intentionally not required and handling moved to qmi_cat_scws.c */
  {GSTK_PROVIDE_LANG_INFO_REQ,                  CATI_CACHED_EVT_DECODED_PROVIDE_LOCAL_INFO_LANG},
  {GSTK_PROVIDE_BATTERY_STATUS_REQ,             CATI_CACHED_EVT_DECODED_PROVIDE_LOCAL_INFO_BATTERY},
  {GSTK_GET_INPUT_REQ,                          CATI_CACHED_EVT_DECODED_GET_INPUT}
};
...
const static qmi_cat_stk_cmd_to_gstk_cmd_type qmi_cat_stk_cmd_to_gstk_cmd[] =
{
  ...
  {GSTK_CMD_STK_CONTACTLESS_STATE_CHANGED, GSTK_CONTACTLESS_STATE_CHANGED_REQ},
  {GSTK_CMD_STK_GEO_LOC_REQ,               GSTK_GEO_LOC_REQ}
};
...
===========================================================================*/
qmi_cati_global_type                  *qmi_cati_global_ptr = NULL;

qmi_cati_state_type                   *qmi_cat_state_ptr   = NULL;
  ...
  switch(src->hdr_cmd.command_id)
  {
    case GSTK_DISPLAY_TEXT_REQ:
      (void)memscpy(&dst->cmd.display_text_pro_cmd_req,
                    ...
                    sizeof(gstk_display_text_req_type));
      uimqmi_deep_copy((void**)&dst->cmd.display_text_pro_cmd_req.text_string.text,
                       (void*)src->cmd.display_text_pro_cmd_req.text_string.text,
                       src->cmd.display_text_pro_cmd_req.text_string.length);
      uimqmi_deep_copy((void**)&dst->cmd.display_text_pro_cmd_req.icon.data,
    ...
    case GSTK_GET_INKEY_REQ:
      (void)memscpy(&dst->cmd.get_inkey_pro_cmd_req,
                    ...
                    sizeof(gstk_get_inkey_req_type));
      uimqmi_deep_copy((void**)&dst->cmd.get_inkey_pro_cmd_req.text_string.text,
                       (void*)src->cmd.get_inkey_pro_cmd_req.text_string.text,
                       src->cmd.get_inkey_pro_cmd_req.text_string.length);
      uimqmi_deep_copy((void**)&dst->cmd.get_inkey_pro_cmd_req.icon.data,
    ...
    case GSTK_LAUNCH_BROWSER_REQ:
      (void)memscpy(&dst->cmd.launch_browser_pro_cmd_req,
                    ...
                    sizeof(gstk_launch_browser_req_type));
      uimqmi_deep_copy((void**)&dst->cmd.launch_browser_pro_cmd_req.url.url,
                       (void*)src->cmd.launch_browser_pro_cmd_req.url.url,
                       src->cmd.launch_browser_pro_cmd_req.url.length);
      uimqmi_deep_copy((void**)&dst->cmd.launch_browser_pro_cmd_req.bearer.bearer_list,
      ...
      uimqmi_deep_copy((void**)&dst->cmd.launch_browser_pro_cmd_req.gateway_proxy_text_string.text,
      ...
      uimqmi_deep_copy((void**)&dst->cmd.launch_browser_pro_cmd_req.user_confirm_alpha.text,
      ...
      uimqmi_deep_copy((void**)&dst->cmd.launch_browser_pro_cmd_req.user_confirm_icon.data,
    ...
    case GSTK_PLAY_TONE_REQ:
      (void)memscpy(&dst->cmd.play_tone_pro_cmd_req,
                    ...
                    sizeof(gstk_play_tone_req_type));
      uimqmi_deep_copy((void**)&dst->cmd.play_tone_pro_cmd_req.alpha.text,
                       (void*)src->cmd.play_tone_pro_cmd_req.alpha.text,
                       src->cmd.play_tone_pro_cmd_req.alpha.length);
      uimqmi_deep_copy((void**)&dst->cmd.play_tone_pro_cmd_req.icon.data,
    ...
    case GSTK_SELECT_ITEM_REQ:
      (void)memscpy(&dst->cmd.select_item_pro_cmd_req,
                    ...
                    sizeof(gstk_select_item_req_type));
      uimqmi_deep_copy((void**)&dst->cmd.select_item_pro_cmd_req.title.text,
                       (void*)src->cmd.select_item_pro_cmd_req.title.text,
                       src->cmd.select_item_pro_cmd_req.title.length);
      uimqmi_deep_copy((void**)&dst->cmd.select_item_pro_cmd_req.icon.data,
                       (void*)src->cmd.select_item_pro_cmd_req.icon.data,
                       src->cmd.select_item_pro_cmd_req.icon.size);
      uimqmi_deep_copy((void**)&dst->cmd.select_item_pro_cmd_req.next_action_indicator.next_action_list,
      ...
      {
        uimqmi_deep_copy((void**)&dst->cmd.select_item_pro_cmd_req.items[i].item_text,
      ...
      {
        uimqmi_deep_copy((void**)&dst->cmd.select_item_pro_cmd_req.icon_id_list.icon_list[i],
        ...
        {
          uimqmi_deep_copy((void**)&dst->cmd.select_item_pro_cmd_req.icon_id_list.icon_list[i]->data,
    ...
    case GSTK_SEND_SMS_REQ:
      (void)memscpy(&dst->cmd.send_sms_pro_cmd_req,
                    ...
                    sizeof(gstk_send_sms_req_type));
      uimqmi_deep_copy((void**)&dst->cmd.send_sms_pro_cmd_req.sms_tpdu.tpdu,
                       (void*)src->cmd.send_sms_pro_cmd_req.sms_tpdu.tpdu,
                       src->cmd.send_sms_pro_cmd_req.sms_tpdu.length);
      uimqmi_deep_copy((void**)&dst->cmd.send_sms_pro_cmd_req.alpha.text,
                       (void*)src->cmd.send_sms_pro_cmd_req.alpha.text,
                       src->cmd.send_sms_pro_cmd_req.alpha.length);
      uimqmi_deep_copy((void**)&dst->cmd.send_sms_pro_cmd_req.icon.data,
    ...
    case GSTK_SEND_SS_REQ:
      (void)memscpy(&dst->cmd.send_ss_pro_cmd_req,
                    ...
                    sizeof(gstk_send_ss_req_type));
      uimqmi_deep_copy((void**)&dst->cmd.send_ss_pro_cmd_req.alpha.text,
                       (void*)src->cmd.send_ss_pro_cmd_req.alpha.text,
                       src->cmd.send_ss_pro_cmd_req.alpha.length);
      uimqmi_deep_copy((void**)&dst->cmd.send_ss_pro_cmd_req.icon.data,
    ...
    case GSTK_SEND_USSD_REQ:
      (void)memscpy(&dst->cmd.send_ussd_pro_cmd_req,
                    ...
                    sizeof(gstk_send_ussd_req_type));
      uimqmi_deep_copy((void**)&dst->cmd.send_ussd_pro_cmd_req.ussd_string.text,
                       (void*)src->cmd.send_ussd_pro_cmd_req.ussd_string.text,
                       src->cmd.send_ussd_pro_cmd_req.ussd_string.length);
      uimqmi_deep_copy((void**)&dst->cmd.send_ussd_pro_cmd_req.alpha.text,
                       (void*)src->cmd.send_ussd_pro_cmd_req.alpha.text,
                       src->cmd.send_ussd_pro_cmd_req.alpha.length);
      uimqmi_deep_copy((void**)&dst->cmd.send_ussd_pro_cmd_req.icon.data,
    ...
    case GSTK_SETUP_CALL_REQ:
      (void)memscpy(&dst->cmd.setup_call_pro_cmd_req,
                    ...
                    sizeof(gstk_setup_call_req_type));
      uimqmi_deep_copy((void**)&dst->cmd.setup_call_pro_cmd_req.user_cnf_alpha.text,
                       (void*)src->cmd.setup_call_pro_cmd_req.user_cnf_alpha.text,
                       src->cmd.setup_call_pro_cmd_req.user_cnf_alpha.length);
      uimqmi_deep_copy((void**)&dst->cmd.setup_call_pro_cmd_req.setup_call_display_alpha.text,
      ...
      uimqmi_deep_copy((void**)&dst->cmd.setup_call_pro_cmd_req.user_cnf_icon.data,
                       (void*)src->cmd.setup_call_pro_cmd_req.user_cnf_icon.data,
                       src->cmd.setup_call_pro_cmd_req.user_cnf_icon.size);
      uimqmi_deep_copy((void**)&dst->cmd.setup_call_pro_cmd_req.setup_call_display_icon.data,
    ...
    case GSTK_SETUP_MENU_REQ:
      (void)memscpy(&dst->cmd.setup_menu_pro_cmd_req,
                    ...
                    sizeof(gstk_setup_menu_req_type));
      uimqmi_deep_copy((void**)&dst->cmd.setup_menu_pro_cmd_req.title.text,
                       (void*)src->cmd.setup_menu_pro_cmd_req.title.text,
                       src->cmd.setup_menu_pro_cmd_req.title.length);
      uimqmi_deep_copy((void**)&dst->cmd.setup_menu_pro_cmd_req.next_action_indicator.next_action_list,
      ...
      uimqmi_deep_copy((void**)&dst->cmd.setup_menu_pro_cmd_req.icon.data,
      ...
      {
        uimqmi_deep_copy((void**)&dst->cmd.setup_menu_pro_cmd_req.items[i].item_text,
      ...
      {
        uimqmi_deep_copy((void**)&dst->cmd.setup_menu_pro_cmd_req.icon_id_list.icon_list[i],
        ...
        {
          uimqmi_deep_copy((void**)&dst->cmd.setup_menu_pro_cmd_req.icon_id_list.icon_list[i]->data,
    ...
    case GSTK_SETUP_IDLE_TEXT_REQ:
      (void)memscpy(&dst->cmd.setup_idle_mode_txt_pro_cmd_req,
                    ...
                    sizeof(gstk_setup_idle_mode_text_req_type));
      uimqmi_deep_copy((void**)&dst->cmd.setup_idle_mode_txt_pro_cmd_req.text_string.text,
                       (void*)src->cmd.setup_idle_mode_txt_pro_cmd_req.text_string.text,
                       src->cmd.setup_idle_mode_txt_pro_cmd_req.text_string.length);
      uimqmi_deep_copy((void**)&dst->cmd.setup_idle_mode_txt_pro_cmd_req.icon.data,
    ...
    case GSTK_SEND_DTMF_REQ:
      (void)memscpy(&dst->cmd.send_dtmf_pro_cmd_req,
                    ...
                    sizeof(gstk_send_dtmf_req_type));
      uimqmi_deep_copy((void**)&dst->cmd.send_dtmf_pro_cmd_req.dtmf.dtmf,
                       (void*)src->cmd.send_dtmf_pro_cmd_req.dtmf.dtmf,
                       src->cmd.send_dtmf_pro_cmd_req.dtmf.length);
      uimqmi_deep_copy((void**)&dst->cmd.send_dtmf_pro_cmd_req.alpha.text,
                       (void*)src->cmd.send_dtmf_pro_cmd_req.alpha.text,
                       src->cmd.send_dtmf_pro_cmd_req.alpha.length);
      uimqmi_deep_copy((void**)&dst->cmd.send_dtmf_pro_cmd_req.icon.data,
    ...
    case GSTK_SETUP_EVT_CONTACTLESS_STATE_REQ_NOTIFY:
    ...
    case GSTK_OPEN_CH_REQ:
      ...
      uimqmi_deep_copy((void**)&dst->cmd.open_ch_pro_cmd_req.alpha.alphabet,
                       (void*)src->cmd.open_ch_pro_cmd_req.alpha.alphabet,
                       (src->cmd.open_ch_pro_cmd_req.alpha.length * 2));
      uimqmi_deep_copy((void**)&dst->cmd.open_ch_pro_cmd_req.icon.data,
                       (void*)src->cmd.open_ch_pro_cmd_req.icon.data,
                       src->cmd.open_ch_pro_cmd_req.icon.size);
      uimqmi_deep_copy((void**)&dst->cmd.open_ch_pro_cmd_req.access_point_name.alphabet,
                       (void*)src->cmd.open_ch_pro_cmd_req.access_point_name.alphabet,
                       (src->cmd.open_ch_pro_cmd_req.access_point_name.length * 2));
      uimqmi_deep_copy((void**)&dst->cmd.open_ch_pro_cmd_req.local_address.pkt_address.address,
      ...
      uimqmi_deep_copy((void**)&dst->cmd.open_ch_pro_cmd_req.text_string_user_login.alphabet,
      ...
      uimqmi_deep_copy((void**)&dst->cmd.open_ch_pro_cmd_req.text_string_user_passwd.alphabet,
      ...
      uimqmi_deep_copy((void**)&dst->cmd.open_ch_pro_cmd_req.data_dest_address.pkt_address.address,
    ...
    case GSTK_CLOSE_CH_REQ:
      (void)memscpy(&dst->cmd.close_ch_pro_cmd_req,
                    ...
                    sizeof(gstk_close_ch_req_type));
      /* alphabet is type uint16, byte size for copying is alphabet.length * 2 */
      uimqmi_deep_copy((void**)&dst->cmd.close_ch_pro_cmd_req.alpha.alphabet,
                       (void*)src->cmd.close_ch_pro_cmd_req.alpha.alphabet,
                       (src->cmd.close_ch_pro_cmd_req.alpha.length * 2));
      uimqmi_deep_copy((void**)&dst->cmd.close_ch_pro_cmd_req.icon.data,
    ...
    case GSTK_SEND_DATA_REQ:
      (void)memscpy(&dst->cmd.send_data_pro_cmd_req,
                    ...
                    sizeof(gstk_send_data_req_type));
      uimqmi_deep_copy((void**)&dst->cmd.send_data_pro_cmd_req.ch_data.data,
                       (void*)src->cmd.send_data_pro_cmd_req.ch_data.data,
                       src->cmd.send_data_pro_cmd_req.ch_data.data_len);
      /* alphabet is type uint16, byte size for copying is alphabet.length * 2 */
      uimqmi_deep_copy((void**)&dst->cmd.send_data_pro_cmd_req.alpha.alphabet,
                       (void*)src->cmd.send_data_pro_cmd_req.alpha.alphabet,
                       (src->cmd.send_data_pro_cmd_req.alpha.length * 2));
      uimqmi_deep_copy((void**)&dst->cmd.send_data_pro_cmd_req.icon.data,
    ...
    case GSTK_RECEIVE_DATA_REQ:
      (void)memscpy(&dst->cmd.receive_data_pro_cmd_req,
                    ...
                    sizeof(gstk_receive_data_req_type));
      /* alphabet is type uint16, byte size for copying is alphabet.length * 2 */
      uimqmi_deep_copy((void**)&dst->cmd.receive_data_pro_cmd_req.alpha.alphabet,
                       (void*)src->cmd.receive_data_pro_cmd_req.alpha.alphabet,
                       (src->cmd.receive_data_pro_cmd_req.alpha.length * 2));
      uimqmi_deep_copy((void**)&dst->cmd.receive_data_pro_cmd_req.icon.data,
    ...
    case GSTK_REFRESH_REQ:
      (void)memscpy(&dst->cmd.refresh_pro_cmd_req,
                    ...
                    sizeof(gstk_refresh_req_type));
      uimqmi_deep_copy((void**)&dst->cmd.refresh_pro_cmd_req.alpha.text,
                       (void*)src->cmd.refresh_pro_cmd_req.alpha.text,
                       src->cmd.refresh_pro_cmd_req.alpha.length);
      uimqmi_deep_copy((void**)&dst->cmd.refresh_pro_cmd_req.icon.data,
                       (void*)src->cmd.refresh_pro_cmd_req.icon.data,
                       src->cmd.refresh_pro_cmd_req.icon.size);
      break;

    case GSTK_CONTACTLESS_STATE_CHANGED_REQ:
      (void)memscpy(&dst->cmd.contactless_state_changed_cmd_req,
                    sizeof(gstk_contactless_state_changed_req_type),
                    &src->cmd.contactless_state_changed_cmd_req,
                    sizeof(gstk_contactless_state_changed_req_type));
      break;

    case GSTK_GEO_LOC_REQ:
      (void)memscpy(&dst->cmd.geo_loc_pro_cmd_req,
                    sizeof(gstk_geo_loc_req_type),
                    &src->cmd.geo_loc_pro_cmd_req,
                    sizeof(gstk_geo_loc_req_type));
      uimqmi_deep_copy((void**)&dst->cmd.geo_loc_pro_cmd_req.alpha.alphabet,
                       (void*)src->cmd.geo_loc_pro_cmd_req.alpha.alphabet,
                       (uint32)src->cmd.geo_loc_pro_cmd_req.alpha.length);
      uimqmi_deep_copy((void**)&dst->cmd.geo_loc_pro_cmd_req.icon.data,
                       (void*)src->cmd.geo_loc_pro_cmd_req.icon.data,
                       (uint32)src->cmd.geo_loc_pro_cmd_req.icon.size);
      break;
...
)
{
  ...
  switch(command->hdr_cmd.command_id)
  {
    ...
    case GSTK_REFRESH_REQ:
      ...
      break;
    case GSTK_GEO_LOC_REQ:
      QMI_CAT_FREE_IF_NOT_NULL(command->cmd.geo_loc_pro_cmd_req.alpha.alphabet);
      QMI_CAT_FREE_IF_NOT_NULL(command->cmd.geo_loc_pro_cmd_req.icon.data);
      break;
    default:
      UIM_MSG_ERR_1("QMI_CAT: Invalid proactive command: 0x%x",
  ...
  switch (src->hdr_cmd.command_id)
  {
    case GSTK_CC_IND_RSP:
      (void)memscpy(&dst->cmd.cc_rsp,
                    ...
                    &src->cmd.cc_rsp,
                    sizeof(gstk_cc_rsp_type));
      uimqmi_deep_copy((void**)&dst->cmd.cc_rsp.cc_req_action.alpha.text,
      ...
      {
        uimqmi_deep_copy((void**)&dst->cmd.cc_rsp.cc_req_action.gstk_address_string.ussd_string.text,
    ...
    case GSTK_SMS_PP_DOWNLOAD_IND_RSP:
      /*reset is_cp_ack_received flag incase if SMS is from AP not from WMS */
      if (gstk_io_ctrl(
          GSTK_IO_SLOT_ID_TO_MODEM_AS_ID,
          &src->hdr_cmd.sim_slot_id,
          &as_id) == GSTK_SUCCESS)
      {
      ...
      (void)memscpy(&dst->cmd.sms_pp_download_rsp,
                    ...
                    sizeof(gstk_sms_pp_download_rsp_type));
      uimqmi_deep_copy((void**)&dst->cmd.sms_pp_download_rsp.ack.ack,
    ...
    case GSTK_MO_SMS_CTRL_IND_RSP:
       (void)memscpy(&dst->cmd.mo_sms_cc_rsp,
                     ...
                     sizeof(gstk_mo_sms_ctrl_rsp_type));
       uimqmi_deep_copy((void**)&dst->cmd.mo_sms_cc_rsp.alpha.text,
  ...
  if(cat_message_ptr != NULL)
  {
    switch(cat_message_ptr->message_type)
    {
      ...
      case QMI_CAT_MESSAGE_SCWS_COMMAND:
        qmi_cati_free_scws_command(&(cat_message_ptr->data.scws_cmd.scws_cmd));
        break;
      default:
...
)
{
  ...
  else
  {
    ...
    qmi_cat_message_ptr->data.framework_cmd.cmd.cmd_hdr = *msg_hdr_ptr;
    qmi_cat_message_ptr->data.framework_cmd.cmd.sdu     = *sdu;
...
)
{
  ...
  switch (qmi_tr_cnf)
  {
    case CAT_RESPONSE_CMD_DISPLAY_TEXT_V02:
      *gstk_tr_cnf = GSTK_DISPLAY_TEXT_CNF;
      break;

    case CAT_RESPONSE_CMD_GET_INKEY_V02:
      *gstk_tr_cnf = GSTK_GET_INKEY_CNF;
      break;

    case CAT_RESPONSE_CMD_GET_INPUT_V02:
      *gstk_tr_cnf = GSTK_GET_INPUT_CNF;
      break;

    case CAT_RESPONSE_CMD_LAUNCH_BROWSER_V02:
      *gstk_tr_cnf = GSTK_LAUNCH_BROWSER_CNF;
      break;

    case CAT_RESPONSE_CMD_PLAY_TONE_V02:
      *gstk_tr_cnf = GSTK_PLAY_TONE_CNF;
      break;

    case CAT_RESPONSE_CMD_SELECT_ITEM_REQ_V02:
      *gstk_tr_cnf = GSTK_SELECT_ITEM_CNF;
      break;

    case CAT_RESPONSE_CMD_SETUP_MENU_V02:
      *gstk_tr_cnf = GSTK_SETUP_MENU_CNF;
      break;

    case CAT_RESPONSE_CMD_SETUP_IDLE_TEXT_V02:
      *gstk_tr_cnf = GSTK_SETUP_IDLE_TEXT_CNF;
      break;

    case CAT_RESPONSE_CMD_PROVIDE_LOCAL_LANG_INFO_V02:
      *gstk_tr_cnf= GSTK_PROVIDE_LANG_INFO_CNF;
      break;

    case CAT_RESPONSE_CMD_LANGUAGE_NOTIFICATION_V02:
      *gstk_tr_cnf = GSTK_LANG_NOTIFICATION_CNF;
      break;

    case CAT_RESPONSE_CMD_SETUP_EVENT_USER_ACTIVITY_V02:
    case CAT_RESPONSE_CMD_SETUP_EVENT_IDLE_SCREEN_NOTIFY_V02:
    case CAT_RESPONSE_CMD_SETUP_EVENT_LANGUAGE_SEL_NOTIFY_V02:
    case CAT_RESPONSE_CMD_SETUP_EVENT_HCI_CONNECTIVITY_V02:
    case CAT_RESPONSE_CMD_SETUP_EVENT_BROWSER_TERMINATION_V02:
    case CAT_RESPONSE_CMD_SETUP_EVENT_CONTACTLESS_STATE_REQUEST_V02:
      *gstk_tr_cnf = GSTK_SETUP_EVENT_LIST_CNF;
      break;

    case CAT_RESPONSE_CMD_ACTIVATE_V02:
      *gstk_tr_cnf = GSTK_ACTIVATE_CNF;
    break;

    case CAT_RESPONSE_CMD_SEND_SMS_V02:
      *gstk_tr_cnf = GSTK_SEND_SMS_CNF;
      break;

    case CAT_RESPONSE_CMD_SETUP_CALL_V02:
      *gstk_tr_cnf = GSTK_SETUP_CALL_CNF;
      break;

    case CAT_RESPONSE_CMD_SEND_DTMF_V02:
      *gstk_tr_cnf = GSTK_SEND_DTMF_CNF;
      break;

    case CAT_RESPONSE_CMD_SEND_SS_V02:
      *gstk_tr_cnf = GSTK_SEND_SS_CNF;
      break;

    case CAT_RESPONSE_CMD_SEND_USSD_V02:
      *gstk_tr_cnf = GSTK_SEND_USSD_CNF;
      break;

    case CAT_RESPONSE_CMD_CONTACTLESS_STATE_CHANGED_V02:
      *gstk_tr_cnf = GSTK_CONTACTLESS_STATE_CHANGED_CNF;
      break;

    case CAT_RESPONSE_CMD_PROVIDE_LOCAL_INFO_BATTERY_STATE_V02:
      *gstk_tr_cnf = GSTK_PROVIDE_BATT_STATUS_CNF;
      break;
...
)
{
  ...
  switch(qmi_rat)
  {
    case CAT_ACCESS_TECH_GSM_V02:
      *gstk_rat_ptr = GSTK_ACCESS_TECH_GSM;
      break;
    case CAT_ACCESS_TECH_UTRAN_V02:
      *gstk_rat_ptr = GSTK_ACCESS_TECH_UTRAN;
      break;
    case CAT_ACCESS_TECH_CDMA_V02:
      *gstk_rat_ptr = GSTK_ACCESS_TECH_CDMA;
      break;
    case CAT_ACCESS_TECH_LTE_V02:
      *gstk_rat_ptr = GSTK_ACCESS_TECH_LTE;
      break;
    case CAT_ACCESS_TECH_WLAN_V02:
      *gstk_rat_ptr = GSTK_ACCESS_TECH_WLAN;
      break;
    case CAT_ACCESS_NONE_V02:
      *gstk_rat_ptr = GSTK_ACCESS_NONE;
      break;
    default:
      UIM_MSG_LOW_1("Invalid qmi rat : 0x%x", qmi_rat);
      return QMI_ERR_INVALID_ARG;
  }
  return QMI_ERR_NONE;
...
)
{
  ...
  switch(qmi_cati_global_ptr->last_gstk_cmd[slot_index])
  {
    ...
    case GSTK_SETUP_EVENT_LIST_REQ:
       ...
       */
      if(is_tr &&
         ((qmi_cati_global_ptr->config_mode != CAT_CONFIG_MODE_DECODED_V02 &&
           qmi_cati_global_ptr->config_mode != CAT_CONFIG_MODE_CUSTOM_DECODED_V02) ||
...
/*===========================================================================
  ...
  SIDE EFFECTS
    None
===========================================================================*/
static cat_config_mode_enum_v02 qmi_cati_read_configuration
(
  void
)
{
  ...
  if (status == UIM_COMMON_EFS_SUCCESS)
  {
    switch(value)
    {
      ...
      case 1:
        ...
        return CAT_CONFIG_MODE_DISABLED_V02;
      case 2:
        return CAT_CONFIG_MODE_ANDROID_V02;
      case 3:
        return CAT_CONFIG_MODE_DECODED_V02;
      case 4:
        return CAT_CONFIG_MODE_DECODED_PULLONLY_V02;
      case 5:
        return CAT_CONFIG_MODE_CUSTOM_RAW_V02;
      case 6:
        return CAT_CONFIG_MODE_CUSTOM_DECODED_V02;
      default:
        break;
    }
  }
...
/*===========================================================================
  FUNCTION QMI_CAT_DISABLE_UNSUPPORTED_FEATURES()

  DESCRIPTION
    Update the unsupported features in TP, disable / enable feature
    settings in the default profile list according to EFS file settings
  ...
  SIDE EFFECTS
    None
===========================================================================*/
static void qmi_cat_disable_unsupported_features
(
...
)
{
  ...
  PACKED struct PACKED_POST
  {
    ...
    boolean support_class_r;
    boolean support_class_g;
    boolean rfu[16];
  ...
  for(i = 0; i < profile_value_list_length; i++)
  {
    switch(profile_value_list_ptr[i].profile_dl_bitmask)
    {
      ...
      case GSTK_USER_ACT_EVT_SUPPORT:
        ...
        break;
       case GSTK_CONTACTLESS_STATE_REQ_EVT_SUPPORT:
       case GSTK_CONTACTLESS_STATE_CHANGED_SUPPORT:
         if (!raw_unsupported_features.support_class_r)
         {
           profile_value_list_ptr[i].value = 0;
         }
         break;
      case GSTK_PROVIDE_LOCAL_INFO_BATTERY_STATE_SUPPORT:
        if (!raw_unsupported_features.support_class_g)
        {
          profile_value_list_ptr[i].value = 0;
        }
        break;
...
} /* qmi_cat_disable_unsupported_features */
...
)
{
  ...
  switch(qmi_cati_global_ptr->config_mode)
  {
    ...
    case CAT_CONFIG_MODE_CUSTOM_RAW_V02:
      return CATI_DISPLAY_ALPHA_AUTOMATIC_FALSE;
    ...
    case CAT_CONFIG_MODE_ANDROID_V02:
      return CATI_DISPLAY_ALPHA_AUTOMATIC_TRUE;
    default:
      break;
  }
...
)
{
  ...
  uint8                              i                                 = 0;
  uint8                              mask                              = 1;
  uint8                              custom_tp[QMI_CAT_TERMINAL_PROFILE_MAX_LENGTH_V02] = {0};
  gstk_profile_dl_support_ext_type  *custom_profile_ptr                = NULL;
  uim_common_efs_status_enum_type status            = UIM_COMMON_EFS_ERROR;

  ASSERT(profile_value_list_pptr && *profile_value_list_pptr == NULL && tp_count_ptr);
  file_len = sizeof(uint8) * QMI_CAT_TERMINAL_PROFILE_MAX_LENGTH_V02;
...
)
{
  ...
  ASSERT (custom_tp_ptr != NULL && custom_tp_len > 0);
...
)
{
  ...
  ASSERT (extra_info_ptr != NULL && general_result_ptr != NULL);
...
)
{
  ...
  switch (ev_mask)
  {
    case QMI_CAT_EVT_REPORT_REQ_DISPLAY_TEXT_MASK_V02:
      ret_val = CATI_PRM_TYPE_DISPLAY_TEXT;
      break;

    case QMI_CAT_EVT_REPORT_REQ_GET_INKEY_MASK_V02:
      ret_val = CATI_PRM_TYPE_GET_INKEY;
      break;

    case QMI_CAT_EVT_REPORT_REQ_GET_INPUT_MASK_V02:
      ret_val = CATI_PRM_TYPE_GET_INPUT;
      break;

    case QMI_CAT_EVT_REPORT_REQ_SETUP_MENU_MASK_V02:
      ret_val = CATI_PRM_TYPE_SETUP_MENU;
      break;

    case QMI_CAT_EVT_REPORT_REQ_SELECT_ITEM_MASK_V02:
      ret_val = CATI_PRM_TYPE_SELECT_ITEM;
      break;

    case QMI_CAT_EVT_REPORT_REQ_SETUP_IDLE_MODE_TEXT_MASK_V02:
      ret_val = CATI_PRM_TYPE_SETUP_IDLE_MODE_TEXT;
      break;

    case QMI_CAT_EVT_REPORT_REQ_LANGUAGE_NOTIF_MASK_V02:
      ret_val = CATI_PRM_TYPE_LANG_NOTIFICATION;
      break;

    case QMI_CAT_EVT_REPORT_REQ_PLAY_TONE_MASK_V02:
      ret_val = CATI_PRM_TYPE_PLAY_TONE_EVENT;
      break;

    case QMI_CAT_EVT_REPORT_REQ_SETUP_CALL_MASK_V02:
      ret_val = CATI_PRM_TYPE_SETUP_CALL_EVENT;
      break;

    case QMI_CAT_EVT_REPORT_REQ_SEND_DTMF_MASK_V02:
      ret_val = CATI_PRM_TYPE_SEND_DTMF_EVENT;
      break;

    case QMI_CAT_EVT_REPORT_REQ_LAUNCH_BROWSER_MASK_V02:
      ret_val = CATI_PRM_TYPE_LAUNCH_BROWSER_EVENT;
      break;

    case QMI_CAT_EVT_REPORT_REQ_SEND_SMS_MASK_V02:
      ret_val = CATI_PRM_TYPE_SEND_SMS_EVENT;
      break;

    case QMI_CAT_EVT_REPORT_REQ_SEND_SS_MASK_V02:
      ret_val = CATI_PRM_TYPE_SEND_SS_EVENT;
      break;

    case QMI_CAT_EVT_REPORT_REQ_SEND_USSD_MASK_V02:
      ret_val = CATI_PRM_TYPE_SEND_USSD_EVENT;
      break;

    case QMI_CAT_EVT_REPORT_REQ_PROVIDE_LOCAL_INFO_LANGUAGE_MASK_V02:
    case QMI_CAT_EVT_REPORT_REQ_PROVIDE_LOCAL_INFO_TIME_MASK_V02:
    case QMI_CAT_EVT_REPORT_REQ_PROVIDE_LOCAL_INFO_BATTERY_MASK_V02:
    ...
    case QMI_CAT_EVT_REPORT_REQ_BEARER_INDEPENDENT_PROTOCOL_MASK_V02:
      switch(data_evt)
      {
    ...
    case QMI_CAT_EVT_REPORT_REQ_ACTIVATE_MASK_V02:
      ret_val = CATI_PRM_TYPE_ACTIVATE;
      break;

    case QMI_CAT_EVT_REPORT_REQ_REFRESH_MASK_V02:
      ret_val = CATI_PRM_TYPE_REFRESH_ALPHA;
      break;

    case QMI_CAT_EVT_REPORT_REQ_CONTACTLESS_SUPPORT_MASK_V02:
      ret_val = CATI_PRM_TYPE_CONTACTLESS_STATE_CHANGED;
      break;

    case QMI_CAT_EVT_REPORT_REQ_GEOGRAPHICAL_LOCATION_MASK_V02:
      ret_val = CATI_PRM_TYPE_GEOGRAPHICAL_LOCATION;
      break;
...
/*===========================================================================
  FUNCTION QMI_CAT_SKIP_GSTK_REGISTRATION()

  DESCRIPTION
    Function checks whether to skip GSTK registration or not used only for
    offtarget.
  ...
  SIDE EFFECTS
    None
===========================================================================*/
static boolean qmi_cat_skip_gstk_registration
(
  void
)
{
...
#endif /* FEATURE_UIM_TEST_FRAMEWORK */
  return FALSE;
} /* qmi_cat_skip_gstk_registration */
...
)
{
  ...
  (void)mmgsdi_get_hw_slots(&qmi_cati_global_ptr->slot_count);
  ...
  if (qmi_cati_global_ptr->config_mode != CAT_CONFIG_MODE_DECODED_PULLONLY_V02)
  {
...
/*===========================================================================
  ...
  RETURN VALUE
    Boolean : TRUE  - when qmi_cat_mode is enabled
              FALSE - when qmi_cat_mode is disabled
...
)
{
  ...
  if (qmi_cati_global_ptr != NULL)
  {
    if (qmi_cati_global_ptr->config_mode == CAT_CONFIG_MODE_DISABLED_V02)
    {
      return FALSE;
    }
    return TRUE;
  ...
  for (card_index = 0; card_index < QMI_CAT_MAX_CARD_COUNT; card_index++)
  {
    ...
    qmi_cati_global_ptr->decoded_evt_reg_mask[card_index]   = CATI_REPORT_STATUS_NONE;
    ...
    {
      qmi_cati_global_ptr->bip_status_cache[card_index][cache_index] = CAT_BIP_STATUS_END_V02;
    }
...
)
{
  ...
  UIM_MSG_MED_0("CAT Services registered with QMI Framework");

  /* register for MPSS/QMI support interfaces logging */
  (void) qmi_si_register_id ( QMUX_SERVICE_CAT,
                              0, /* Service Instance */
                              CATI_BASE_VER_MAJOR,
                              CATI_BASE_VER_MINOR,
...
)
{
  ...
  ASSERT(cmd_buf_ptr && qmi_cat_state_ptr);
  ...
  if((cmd_buf_ptr->x_p->cl_sp->clid == 0) ||
     (cmd_buf_ptr->x_p->cl_sp->clid > UIMQMI_SVC_MAX_CLIDS))
  {
...
{
  ASSERT (slot_index_ptr != NULL && qmi_cati_global_ptr);
  ...
  *tp_count_ptr = 0;
  ...
  if (config_mode == CAT_CONFIG_MODE_ANDROID_V02)
  {
    * tp_count_ptr = sizeof(qmi_cati_android_profile_value_list) / sizeof(gstk_profile_dl_support_ext_type);
    qmi_cat_disable_unsupported_features(qmi_cati_android_profile_value_list, *tp_count_ptr);
    return qmi_cati_android_profile_value_list;
  }
  else if (config_mode == CAT_CONFIG_MODE_DECODED_V02 ||
           config_mode == CAT_CONFIG_MODE_DECODED_PULLONLY_V02)
  {
    * tp_count_ptr = sizeof(qmi_cati_decoded_profile_value_list) / sizeof(gstk_profile_dl_support_ext_type);
    qmi_cat_disable_unsupported_features(qmi_cati_decoded_profile_value_list, *tp_count_ptr);
    return qmi_cati_decoded_profile_value_list;
  }
  else if (config_mode == CAT_CONFIG_MODE_CUSTOM_DECODED_V02 ||
...
)
{
  ...
  if (qmi_cati_global_ptr->config_mode == CAT_CONFIG_MODE_ANDROID_V02 ||
      qmi_cati_global_ptr->config_mode == CAT_CONFIG_MODE_CUSTOM_RAW_V02)
  {
    ...
    qmi_cati_scws_cmd_reg(slot_index, qmi_cati_gstk_cmd_cb, qmi_cati_gstk_evt_cb);
  }

  /* Decoded mode: everything in decoded format */
  if (qmi_cati_global_ptr->config_mode == CAT_CONFIG_MODE_DECODED_V02 ||
      qmi_cati_global_ptr->config_mode == CAT_CONFIG_MODE_CUSTOM_DECODED_V02)
  {
    gstk_status = gstk_client_toolkit_cmd_reg(
                          qmi_cati_global_ptr->cat_gstk_client_id[slot_index],
...
)
{
  ...
  switch (req_ptr->data_format_type)
  {
    ...
    case GSTK_RAW_FORMAT:
      ...
      ASSERT(req_ptr->pro_cmd_reg_data.raw_req_rsp_ptr->payload.data_len <= QMI_CAT_RAW_PROACTIVE_CMD_MAX_LENGTH_V02 )
      ...
      switch(cati_gstk_command_id)
      {
        case GSTK_END_PROACTIVE_CMD_REQ:
          ...
          if (req_ptr->pro_cmd_reg_data.gstk_req_rsp_ptr->cmd.end_pro_cmd_req.end_from_card)
          {
            msg_ptr->data.end_proactive_cmd.proactive_session_end_type =
                  CAT_PROACTIVE_SESSION_END_TYPE_CARD_V02;
          }
          else
          {
            msg_ptr->data.end_proactive_cmd.proactive_session_end_type =
                  CAT_PROACTIVE_SESSION_END_TYPE_INTERNAL_V02;
          }
        ...
        case GSTK_OPEN_CH_CNF:
              ...
              GSTK_COMMAND_PERFORMED_SUCCESSFULLY)
          {
            ...
            msg_ptr->data.decoded_bip_status.ch_id
            ...
            msg_ptr->data.decoded_bip_status.slot_id
              = req_ptr->pro_cmd_reg_data.gstk_req_rsp_ptr->hdr_cmd.sim_slot_id;
            msg_ptr->data.decoded_bip_status.bip_status = CAT_BIP_STATUS_IN_PROGRESS_V02;
            qmi_cat_post_message(msg_ptr);
          }
          else
          {
        ...
        case GSTK_CLOSE_CH_CNF:
              ...
              GSTK_COMMAND_PERFORMED_SUCCESSFULLY)
          {
            ...
            /* Since GSTK may send back to back CLOSE_CH REQ/CNF before REQ is
               ...
               */
            msg_ptr->data.decoded_bip_status.ch_id
                 = (uint8)req_ptr->pro_cmd_reg_data.gstk_req_rsp_ptr->cmd.terminal_response.other_info.extra_param.close_ch_extra_param.ch_status.ch_id;
            msg_ptr->data.decoded_bip_status.slot_id
              = req_ptr->pro_cmd_reg_data.gstk_req_rsp_ptr->hdr_cmd.sim_slot_id;
            msg_ptr->data.decoded_bip_status.bip_status = CAT_BIP_STATUS_END_V02;
            qmi_cat_post_message(msg_ptr);
          }
          else
          {
...
)
{
  ...
  if (mask & QMI_CAT_EVT_REPORT_REQ_SETUP_MENU_MASK_V02)
  {
  ...
  if (mask & QMI_CAT_EVT_REPORT_REQ_DISPLAY_TEXT_MASK_V02)
  {
    qmi_cat_send_buffered_raw_proactive_cmd_by_index(card_index,
  ...
  if (mask & QMI_CAT_EVT_REPORT_REQ_SETUP_IDLE_MODE_TEXT_MASK_V02)
  {
  ...
  /* If there is a cached GET INPUT not sent yet,
     Queue this proactive command for DS task */
  if (mask & QMI_CAT_EVT_REPORT_REQ_GET_INPUT_MASK_V02)
  {
    qmi_cat_send_buffered_raw_proactive_cmd_by_index(card_index,
...
)
{
  ...
  /* SETUP MENU */
  if (mask & QMI_CAT_DEC_EVT_REPORT_REQ_SETUP_MENU_MASK_V02)
  {
    qmi_cat_send_buffered_decoded_proactive_cmd_by_index(
                              card_index,
  ...
  /* DISPLAY TEXT */
  if (mask & QMI_CAT_DEC_EVT_REPORT_REQ_DISPLAY_TEXT_MASK_V02)
  {
    qmi_cat_send_buffered_decoded_proactive_cmd_by_index(
                              card_index,
  ...
  if (mask & QMI_CAT_EVT_REPORT_REQ_SETUP_EVENT_USER_ACTIVITY_MASK_V02)
  {
    qmi_cat_send_buffered_decoded_proactive_cmd_by_index(
                              card_index,
  ...
  if (mask & QMI_CAT_EVT_REPORT_REQ_SETUP_EVENT_IDLE_SCREEN_NOTIF_MASK_V02)
  {
    qmi_cat_send_buffered_decoded_proactive_cmd_by_index(
                              card_index,
  ...
  if (mask & QMI_CAT_EVT_REPORT_REQ_SETUP_EVENT_LANGUAGE_SEL_NOTIF_MASK_V02)
  {
    qmi_cat_send_buffered_decoded_proactive_cmd_by_index(
                              card_index,
  ...
  if (mask & QMI_CAT_EVT_REPORT_REQ_SETUP_EVENT_HCI_CONN_MASK_V02)
  {
    qmi_cat_send_buffered_decoded_proactive_cmd_by_index(
                              card_index,
  ...
  if (mask & QMI_CAT_EVT_REPORT_REQ_SETUP_EVENT_BROWSER_TERMINATION_MASK_V02)
  {
    qmi_cat_send_buffered_decoded_proactive_cmd_by_index(
                              card_index,
  ...
  /* SETUP EVENT LIST: Contactless State Request */
  if (mask & QMI_CAT_EVT_REPORT_REQ_CONTACTLESS_SUPPORT_MASK_V02)
  {
    qmi_cat_send_buffered_decoded_proactive_cmd_by_index(
                              card_index,
                              CATI_CACHED_EVT_DECODED_SETUP_EVT_CONTACTLESS_S_REQ);
  }

  /* SCWS OPEN CHANNEL */
  if (mask & QMI_CAT_DEC_EVT_REPORT_REQ_SCWS_EVENT_MASK_V02)
  {
    qmi_cat_send_buffered_scws_cmd(card_index);
  }

  /* PLI Language*/
  if (mask & QMI_CAT_EVT_REPORT_REQ_PROVIDE_LOCAL_INFO_LANGUAGE_MASK_V02)
  {
    qmi_cat_send_buffered_decoded_proactive_cmd_by_index(
                              card_index,
  ...
  /* PLI Battery State */
  if (mask & QMI_CAT_EVT_REPORT_REQ_PROVIDE_LOCAL_INFO_BATTERY_MASK_V02)
  {
    qmi_cat_send_buffered_decoded_proactive_cmd_by_index(
                              card_index,
                              CATI_CACHED_EVT_DECODED_PROVIDE_LOCAL_INFO_BATTERY);
  }
  ...
  if (mask & QMI_CAT_EVT_REPORT_REQ_GET_INPUT_MASK_V02)
  {
    qmi_cat_send_buffered_decoded_proactive_cmd_by_index(
                              card_index,
                              CATI_CACHED_EVT_DECODED_GET_INPUT);
  }
...
)
{
  ...
  for (i=0; i < QMI_CATI_MAX_BIP_CHANNEL; i++)
  {
    ...
    {
      case CAT_BIP_STATUS_IN_PROGRESS_V02:
        msg_ptr = qmi_cat_create_message(QMI_CAT_MESSAGE_DECODED_BIP_STATUS);
        if(NULL == msg_ptr)
        {
          return;
        ...
        msg_ptr->data.decoded_bip_status.slot_id = (gstk_slot_id_enum_type)(GSTK_SLOT_1 + card_index);
        msg_ptr->data.decoded_bip_status.bip_status = CAT_BIP_STATUS_IN_PROGRESS_V02;
        msg_ptr->data.decoded_bip_status.ch_id = i + 1;
        qmi_cat_post_message(msg_ptr);
        break;

     case CAT_BIP_STATUS_END_V02:
     default:
       break;
    }
  }
...
)
{
  ...
  else if(event_info_ptr->mask & QMI_CAT_EVT_REPORT_REQ_END_PROACTIVE_SESSION_MASK_V02)
  {
  ...
  (void)uimqmi_send_indication(clid, CATI_CMD_VAL_SET_EVENT_REPORT, ind, QMUX_SERVICE_CAT);
...
/*===========================================================================
  FUNCTION  QMI_CATI_IS_EVENT_REPORT_SET()

  DESCRIPTION
    Utility function to determine if atleast one QMI CAT client has
    registered for proactive command processing

  PARAMETERS
    slot_index : Slot on which the proactivre command is received
  ...
  SIDE EFFECTS
    None
===========================================================================*/
static boolean qmi_cati_is_event_report_set
(
  uint8 slot_index
)
{
  ...
  /* Check if atleast one QMI CAT client registers
     for Proactive Commands. If so, TRUE is returned. */
  if((qmi_cati_global_ptr->evt_reg_mask[slot_index] != 0) ||
     (qmi_cati_global_ptr->decoded_evt_reg_mask[slot_index] != 0))
  {
    return TRUE;
  }

  return FALSE;
} /* qmi_cati_is_event_report_set */
...
)
{
  ...
  ** +1  Command Qualifier */
  ...
  /* For the general results '20', '21','26', '38', '39', '3A', '3C' and '3D', it is mandatory
  for the terminal to provide a specific cause value as additional information */

  switch (result)
  {
    case GSTK_COMMAND_PERFORMED_SUCCESSFULLY:
      STK_response.result_tag_length = 1;
      break;
    case GSTK_ME_CURRENTLY_UNABLE_TO_PROCESS_COMMAND:
    case GSTK_NETWORK_CURRENTLY_UNABLE_TO_PROCESS_COMMAND:
    case GSTK_LAUNCH_BROWSER_GENERIC_ERROR:
    case GSTK_MULTIPLE_CARD_COMMANDS_ERROR:
    case GSTK_INTERACTION_WITH_CC_BY_SIM_OR_MO_SM_CONTROL_BY_SIM_PROBLEM:
    case GSTK_BEARER_INDEPENDENT_PROTOCOL_ERROR:
      STK_response.result_tag_length = 2;
      break;
    default:
      STK_response.result_tag_length = 1;
      break;
  ...
  */
  ...
  term_resp.data_len = (STK_response.result_tag_length == 1) ?
...
)
{
  ...
  for (i = 0; i < QMI_CATI_MAX_BIP_CHANNEL; i++)
  {
    if (qmi_cati_global_ptr->bip_status_cache[slot_index][i] == CAT_BIP_STATUS_IN_PROGRESS_V02)
    {
      memset(&bip_status, 0, sizeof(qmi_cat_decoded_bip_status_type));
      bip_status.slot_id = slot_index;
      bip_status.ch_id = i + 1;
      bip_status.bip_status = CAT_BIP_STATUS_END_V02;
      ...
      for (j = 0; j < UIMQMI_SVC_MAX_CLIDS; j++)
      {
        ...
        if( (cl_sp->common.clid != QMI_SVC_CLID_UNUSED) &&
            (cl_sp->decoded_evt_report_mask[slot_index] & QMI_CAT_DEC_EVT_REPORT_REQ_BEARER_INDEPENDENT_PROTOCOL_STATUS_MASK_V02))
        {
      ...
      /* update BIP Status */
      qmi_cati_global_ptr->bip_status_cache[slot_index][i] = CAT_BIP_STATUS_END_V02;
    }
  }
...
)
{
  ...
  qmi_cati_global_ptr->user_cnf_required[card_index]             = FALSE;
  ...
  for(cache_index = 0; cache_index < CATI_MAX_DECODED_CACHED_EVTS; cache_index++)
  {
    ...
    {
      if( cache_index != CATI_CACHED_EVT_DECODED_SCWS_OPEN_CHANNEL)
      {
        qmi_cati_free_decoded_proactive_command(
  ...
  qmi_cat_cleanup_scws_globals(card_index);
...
)
{
  ...
  /* Check raw data len */
  if (cat_msg_ptr->data.raw_proactive_cmd.raw_data_len > QMI_CAT_RAW_PROACTIVE_CMD_MAX_LENGTH_V02)
  {
...
)
{
  ...
  /* Reset the user confirmation required flag */
  qmi_cati_global_ptr->user_cnf_required[slot_index] = FALSE;
        ...
        case GSTK_CMD_STK_SEND_SHORT_MESSAGE:
          qmi_cati_global_ptr->full_func_evt[slot_index] =
            (qmi_cati_global_ptr->full_func_evt_reg_mask[slot_index] &
             CAT_FULL_FUNC_EVT_REPORT_REQ_SEND_SMS_MASK_V02)? TRUE:FALSE;
          break;

        case GSTK_CMD_STK_SET_UP_CALL:
          qmi_cati_global_ptr->full_func_evt[slot_index] =
            (qmi_cati_global_ptr->full_func_evt_reg_mask[slot_index] &
             CAT_FULL_FUNC_EVT_REPORT_REQ_SETUP_CALL_MASK_V02)? TRUE:FALSE;
          break;

        case GSTK_CMD_STK_SEND_DTMF:
          qmi_cati_global_ptr->full_func_evt[slot_index] =
            (qmi_cati_global_ptr->full_func_evt_reg_mask[slot_index] &
             CAT_FULL_FUNC_EVT_REPORT_REQ_SEND_DTMF_MASK_V02)? TRUE:FALSE;
          break;

        case GSTK_CMD_STK_SEND_SS:
          qmi_cati_global_ptr->full_func_evt[slot_index] =
            (qmi_cati_global_ptr->full_func_evt_reg_mask[slot_index] &
             CAT_FULL_FUNC_EVT_REPORT_REQ_SEND_SS_MASK_V02)? TRUE:FALSE;
          break;

        case GSTK_CMD_STK_USSD:
          qmi_cati_global_ptr->full_func_evt[slot_index] =
            (qmi_cati_global_ptr->full_func_evt_reg_mask[slot_index] &
             CAT_FULL_FUNC_EVT_REPORT_REQ_SEND_USSD_MASK_V02)? TRUE:FALSE;
          break;
  ...
  if (CAT_CONFIG_MODE_ANDROID_V02    == qmi_cati_global_ptr->config_mode ||
      CAT_CONFIG_MODE_CUSTOM_RAW_V02 == qmi_cati_global_ptr->config_mode)
  {
  ...
  switch(type_of_command)
  {
    case GSTK_CMD_STK_DISPLAY_TEXT:
      cati_event.mask = QMI_CAT_EVT_REPORT_REQ_DISPLAY_TEXT_MASK_V02;

      /* Check if any client is registered for handling this event */
      if (!(qmi_cati_global_ptr->evt_reg_mask[slot_index] & QMI_CAT_EVT_REPORT_REQ_DISPLAY_TEXT_MASK_V02))
      {
    ...
    case GSTK_CMD_STK_GET_INKEY:
      cati_event.mask = QMI_CAT_EVT_REPORT_REQ_GET_INKEY_MASK_V02;
      break;

    case GSTK_CMD_STK_GET_INPUT:
      cati_event.mask = QMI_CAT_EVT_REPORT_REQ_GET_INPUT_MASK_V02;

      /* Check if any client is registered for handling this event */
      if (!(qmi_cati_global_ptr->evt_reg_mask[slot_index] & QMI_CAT_EVT_REPORT_REQ_GET_INPUT_MASK_V02))
      {
        /* If no client reg for this save it for future and send it from the cache
           Fall through if caching failed so that failure TR can be sent back to GSTK
         */
    ...
    case GSTK_CMD_STK_SET_UP_MENU:
      cati_event.mask = QMI_CAT_EVT_REPORT_REQ_SETUP_MENU_MASK_V02;

      /* Check if any client is registered for handling this event */
      if (!(qmi_cati_global_ptr->evt_reg_mask[slot_index] & QMI_CAT_EVT_REPORT_REQ_SETUP_MENU_MASK_V02))
      {
        /* If no client reg for this save it for future and send it from the cache
           Fall through if caching failed so that failure TR can be sent back to GSTK
         */
    ...
    case GSTK_CMD_STK_SELECT_ITEM:
      cati_event.mask = QMI_CAT_EVT_REPORT_REQ_SELECT_ITEM_MASK_V02;
      break;

    case GSTK_CMD_STK_SEND_SHORT_MESSAGE:
      cati_event.mask = QMI_CAT_EVT_REPORT_REQ_SEND_SMS_MASK_V02;
      break;

    case GSTK_CMD_STK_SET_UP_IDLE_MODE_TEXT:
      cati_event.mask = QMI_CAT_EVT_REPORT_REQ_SETUP_IDLE_MODE_TEXT_MASK_V02;

      /* Check if any client is registered for handling this event */
      if (!(qmi_cati_global_ptr->evt_reg_mask[slot_index] & QMI_CAT_EVT_REPORT_REQ_SETUP_IDLE_MODE_TEXT_MASK_V02))
      {
        /* If no client reg for this save it for future and send it from the cache
           Fall through if caching failed so that failure TR can be sent back to GSTK
         */
    ...
    case GSTK_CMD_STK_LANG_NOTIFICATION:
      cati_event.mask = QMI_CAT_EVT_REPORT_REQ_LANGUAGE_NOTIF_MASK_V02;
      break;

    case GSTK_CMD_STK_PLAY_TONE:
      cati_event.mask = QMI_CAT_EVT_REPORT_REQ_PLAY_TONE_MASK_V02;
      break;

    case GSTK_CMD_STK_SET_UP_CALL:
      cati_event.mask = QMI_CAT_EVT_REPORT_REQ_SETUP_CALL_MASK_V02;
      ...
      /* User confirmation alpha, user confirmation is required */
      if (cat_msg_ptr->data.raw_proactive_cmd.raw_hdr_tag == GSTK_TAG_USER_CONF_ALPHA)
      {
        qmi_cati_global_ptr->user_cnf_required[slot_index] = TRUE;
      }
      break;

    case GSTK_CMD_STK_SEND_DTMF:
      cati_event.mask = QMI_CAT_EVT_REPORT_REQ_SEND_DTMF_MASK_V02;
      break;

    case GSTK_CMD_STK_LAUNCH_BROWSER:
      cati_event.mask = QMI_CAT_EVT_REPORT_REQ_LAUNCH_BROWSER_MASK_V02;
      break;

    case GSTK_CMD_STK_SEND_SS:
      cati_event.mask = QMI_CAT_EVT_REPORT_REQ_SEND_SS_MASK_V02;
      break;

    case GSTK_CMD_STK_USSD:
      cati_event.mask = QMI_CAT_EVT_REPORT_REQ_SEND_USSD_MASK_V02;
      break;
    ...
    case GSTK_CMD_STK_OPEN_CHANNEL:
      cati_event.mask = QMI_CAT_EVT_REPORT_REQ_BEARER_INDEPENDENT_PROTOCOL_MASK_V02;
    ...
    case GSTK_CMD_STK_ACTIVATE:
      cati_event.mask = QMI_CAT_EVT_REPORT_REQ_ACTIVATE_MASK_V02;
      break;

    case GSTK_CMD_STK_REFRESH:
      cati_event.mask = QMI_CAT_EVT_REPORT_REQ_REFRESH_MASK_V02;
      break;

    case GSTK_CMD_STK_CONTACTLESS_STATE_CHANGED:
      cati_event.mask = QMI_CAT_EVT_REPORT_REQ_CONTACTLESS_SUPPORT_MASK_V02;
      break;

    case GSTK_CMD_STK_GEO_LOC_REQ:
      cati_event.mask = QMI_CAT_EVT_REPORT_REQ_GEOGRAPHICAL_LOCATION_MASK_V02;
      break;
      ...
      cat_msg_ptr->data.raw_proactive_cmd.cmd_ref_id)
  {
    ...
    if(qmi_cati_global_ptr->full_func_evt[slot_index])
    {
      ...
      /* Check if atleast one QMI CAT client has registered for Proactive
         Commands and Card State is valid to send out proactive commands to
         clients.
         If so, its assumed that QMI CAT clients do not have the capability
         to process the proactive command and hence
         GSTK_COMMAND_BEYOND_THE_ME_CAPABILITIES is sent out.
         Note: This makes the assumption that only one client registers for
         proactive commands, which is true in practice. */
      if((qmi_cati_global_ptr->card_state[slot_index] == QMI_CAT_CARD_STATE_PRESENT) &&
         qmi_cati_is_event_report_set(slot_index))
      {
...
)
{
  ...
  if (ind_token.is_valid &&
      QMI_ERR_NONE != qmi_cat_response_indication_token(&response,
                                                        ind_token.token,
  ...
  {
    errval = qmi_cat_response_raw_envelope(env_rsp_ptr, &response);
...
)
{
  ...
  {
    errval = qmi_cat_response_decoded_envelope(decoded_env_rsp_ptr,&response_ptr);
  }
  ...
  {
    /* Pass SMS PP payload info to client for 62xx/63xx status */
    if((decoded_env_rsp_ptr->hdr_cmd.command_id == GSTK_SMS_PP_DOWNLOAD_IND_RSP) &&
       (decoded_env_rsp_ptr->cmd.sms_pp_download_rsp.ack.length != 0))
    {
      (void)qmi_cat_response_decoded_envelope(decoded_env_rsp_ptr,&response_ptr);
    }
    errval = QMI_ERR_GENERAL;
  }
  else
...
)
{
  ...
  cati_event.mask = QMI_CAT_EVT_REPORT_REQ_END_PROACTIVE_SESSION_MASK_V02;
...
)
{
  ...
  uimqmi_send_indication(clid, cmd_id, response, QMUX_SERVICE_CAT);
...
)
{
  ...
  /* Reset user confirmation required flag */
  qmi_cati_global_ptr->user_cnf_required[slot_index] = FALSE;
  ...
  {
    case GSTK_DISPLAY_TEXT_REQ:
      event_id = QMI_CAT_DEC_EVT_REPORT_REQ_DISPLAY_TEXT_MASK_V02;
      terminal_response_cnf = GSTK_DISPLAY_TEXT_CNF;
      break;
    case GSTK_GET_INKEY_REQ:
      event_id = QMI_CAT_DEC_EVT_REPORT_REQ_GET_INKEY_MASK_V02;
      terminal_response_cnf = GSTK_GET_INKEY_CNF;
      break;
    case GSTK_GET_INPUT_REQ:
      event_id = QMI_CAT_DEC_EVT_REPORT_REQ_GET_INPUT_MASK_V02;
      terminal_response_cnf = GSTK_GET_INPUT_CNF;
      break;
    case GSTK_LAUNCH_BROWSER_REQ:
      event_id = QMI_CAT_EVT_REPORT_REQ_LAUNCH_BROWSER_MASK_V02;
      terminal_response_cnf = GSTK_LAUNCH_BROWSER_CNF;
      break;
    case GSTK_PLAY_TONE_REQ:
      event_id = QMI_CAT_EVT_REPORT_REQ_PLAY_TONE_MASK_V02;
      terminal_response_cnf = GSTK_PLAY_TONE_CNF;
      break;
    case GSTK_SELECT_ITEM_REQ:
      event_id = QMI_CAT_DEC_EVT_REPORT_REQ_SELECT_ITEM_MASK_V02;
      terminal_response_cnf = GSTK_SELECT_ITEM_CNF;
      break;
    case GSTK_SEND_SMS_REQ:
      event_id = QMI_CAT_DEC_EVT_REPORT_REQ_SEND_SMS_MASK_V02;
      if (qmi_cati_global_ptr->ref_command_id[slot_index] !=
      ...
      break;
    case GSTK_SEND_SS_REQ:
      event_id = QMI_CAT_EVT_REPORT_REQ_SEND_SS_MASK_V02;
      if (qmi_cati_global_ptr->ref_command_id[slot_index] !=
          decoded_cmd->hdr_cmd.cmd_detail_reference)
      {
        qmi_cati_global_ptr->full_func_evt[slot_index] =
      ...
      break;
    case GSTK_SEND_USSD_REQ:
      event_id = QMI_CAT_EVT_REPORT_REQ_SEND_USSD_MASK_V02;
      if (qmi_cati_global_ptr->ref_command_id[slot_index] !=
          decoded_cmd->hdr_cmd.cmd_detail_reference)
      {
        qmi_cati_global_ptr->full_func_evt[slot_index] =
      ...
      break;
    case GSTK_SETUP_CALL_REQ:
      event_id = QMI_CAT_EVT_REPORT_REQ_SETUP_CALL_MASK_V02;
      if (qmi_cati_global_ptr->ref_command_id[slot_index] !=
          decoded_cmd->hdr_cmd.cmd_detail_reference)
      {
        qmi_cati_global_ptr->full_func_evt[slot_index] =
      ...
      {
        /* User confirmation alpha, user confirmation is required */
        if (decoded_cmd->hdr_cmd.tag == GSTK_TAG_USER_CONF_ALPHA)
        {
          qmi_cati_global_ptr->user_cnf_required[slot_index] = TRUE;
        }
      }
      break;
    case GSTK_SETUP_MENU_REQ:
      event_id = QMI_CAT_DEC_EVT_REPORT_REQ_SETUP_MENU_MASK_V02;
      terminal_response_cnf = GSTK_SETUP_MENU_CNF;
      break;
    case GSTK_SETUP_IDLE_TEXT_REQ:
      event_id = QMI_CAT_EVT_REPORT_REQ_SETUP_IDLE_MODE_TEXT_MASK_V02;
      terminal_response_cnf = GSTK_SETUP_IDLE_TEXT_CNF;
      break;
    case GSTK_LANG_NOTIFICATION_REQ:
      event_id = QMI_CAT_EVT_REPORT_REQ_LANGUAGE_NOTIF_MASK_V02;
      terminal_response_cnf = GSTK_LANG_NOTIFICATION_CNF;
      break;
    case GSTK_SEND_DTMF_REQ:
      event_id = QMI_CAT_EVT_REPORT_REQ_SEND_DTMF_MASK_V02;
      if (qmi_cati_global_ptr->ref_command_id[slot_index] !=
          decoded_cmd->hdr_cmd.cmd_detail_reference)
      {
        qmi_cati_global_ptr->full_func_evt[slot_index] =
      ...
      break;
    case GSTK_SETUP_EVT_IDLE_SCREEN_NOTIFY:
      event_id = QMI_CAT_EVT_REPORT_REQ_SETUP_EVENT_IDLE_SCREEN_NOTIF_MASK_V02;
      /* No TR sent from UI */
      break;
    case GSTK_SETUP_EVT_USER_ACT_NOTIFY:
      event_id = QMI_CAT_EVT_REPORT_REQ_SETUP_EVENT_USER_ACTIVITY_MASK_V02;
      /* No TR sent from UI */
      break;
    case GSTK_SETUP_EVT_LANG_SEL_NOTIFY:
      event_id = QMI_CAT_EVT_REPORT_REQ_SETUP_EVENT_LANGUAGE_SEL_NOTIF_MASK_V02;
      /* No TR sent from UI */
      break;
    case GSTK_SETUP_EVT_HCI_CONNECT_NOTIFY:
      event_id = QMI_CAT_EVT_REPORT_REQ_SETUP_EVENT_HCI_CONN_MASK_V02;
      /* No TR sent from UI */
      break;
    case GSTK_SETUP_EVT_BROWSER_TERM_NOTIFY:
      event_id = QMI_CAT_EVT_REPORT_REQ_SETUP_EVENT_BROWSER_TERMINATION_MASK_V02;
      /* No TR sent from UI */
      break;
    case GSTK_SETUP_EVT_CONTACTLESS_STATE_REQ_NOTIFY:
      event_id = QMI_CAT_EVT_REPORT_REQ_CONTACTLESS_SUPPORT_MASK_V02;
      /* No TR sent from UI */
      break;
    case GSTK_PROVIDE_LANG_INFO_REQ:
      event_id = QMI_CAT_EVT_REPORT_REQ_PROVIDE_LOCAL_INFO_LANGUAGE_MASK_V02;
      terminal_response_cnf = GSTK_PROVIDE_LANG_INFO_CNF;
      /* No TR sent from UI */
      break;
    case GSTK_OPEN_CH_REQ:
      event_id = QMI_CAT_EVT_REPORT_REQ_BEARER_INDEPENDENT_PROTOCOL_MASK_V02;
    ...
    case GSTK_CLOSE_CH_REQ:
      ...
      break;
    case GSTK_ACTIVATE_REQ:
      event_id = QMI_CAT_EVT_REPORT_REQ_ACTIVATE_MASK_V02;
      terminal_response_cnf = GSTK_ACTIVATE_CNF;
      break;
    case GSTK_REFRESH_REQ:
      event_id = QMI_CAT_EVT_REPORT_REQ_REFRESH_MASK_V02;
      /* No TR sent from UI */
      break;
    case GSTK_CONTACTLESS_STATE_CHANGED_REQ:
      event_id = QMI_CAT_EVT_REPORT_REQ_CONTACTLESS_SUPPORT_MASK_V02;
      terminal_response_cnf = GSTK_CONTACTLESS_STATE_CHANGED_CNF;
      break;
    case GSTK_GEO_LOC_REQ:
      event_id = QMI_CAT_EVT_REPORT_REQ_GEOGRAPHICAL_LOCATION_MASK_V02;
      /* No TR sent from UI */
      break;
    case GSTK_PROVIDE_BATTERY_STATUS_REQ:
      event_id = QMI_CAT_EVT_REPORT_REQ_PROVIDE_LOCAL_INFO_BATTERY_MASK_V02;
      terminal_response_cnf = GSTK_PROVIDE_BATT_STATUS_CNF;
      break;
    default:
  ...
  if (!ind_sent)
  {
    ...
    if (!clone_status &&
        (qmi_cati_global_ptr->config_mode != CAT_CONFIG_MODE_DECODED_PULLONLY_V02) &&
        (qmi_cati_global_ptr->ref_command_id[slot_index] == decoded_cmd->hdr_cmd.cmd_detail_reference))
    {
      ...
      if (terminal_response_cnf != GSTK_CMD_ENUM_NOT_USE)
      {
        ...
        /* Check if atleast one QMI CAT client has registered for Proactive
           ...
           Note: This makes the assumption that only one client registers for
           proactive commands, which is true in practice. */
        if((general_result == GSTK_ME_CURRENTLY_UNABLE_TO_PROCESS_COMMAND) &&
...
)
{
  ...
  /* Retrieve the slot index */
  if (qmi_cati_convert_gstk_slot_id_to_qmi_slot_index(cat_msg_ptr->data.decoded_bip_status.slot_id,
                                                      &slot_index) != QMI_ERR_NONE)
  {
    UIM_MSG_ERR_1("process_decoded_bip_status: fail to covert slot_id, 0x%x",
                  cat_msg_ptr->data.decoded_bip_status.slot_id);
    return;
  }

  UIM_MSG_HIGH_2("ch_id 0x%x, qmi_cat card state 0x%x",
                  cat_msg_ptr->data.decoded_bip_status.ch_id,
                  qmi_cati_global_ptr->card_state[slot_index]);

  /* Get mask of the proactive command in order to send it to the right client */
  if ((cat_msg_ptr->data.decoded_bip_status.ch_id > 0) &&
      (cat_msg_ptr->data.decoded_bip_status.ch_id <= QMI_CATI_MAX_BIP_CHANNEL))
  {
    qmi_cati_global_ptr->bip_status_cache[slot_index][cat_msg_ptr->data.decoded_bip_status.ch_id - 1]
    ...
    for (i = 0; i < UIMQMI_SVC_MAX_CLIDS && !ind_sent; i++)
    {
          ...
          (cl_sp->decoded_evt_report_mask[slot_index] & QMI_CAT_DEC_EVT_REPORT_REQ_BEARER_INDEPENDENT_PROTOCOL_STATUS_MASK_V02))
      {
        ...
        bip_status.slot_id = cat_msg_ptr->data.decoded_bip_status.slot_id;
        bip_status.bip_status = cat_msg_ptr->data.decoded_bip_status.bip_status;
        bip_status.ch_id = cat_msg_ptr->data.decoded_bip_status.ch_id;

        qmi_cati_decoded_bip_status_event_report_ind(cl_sp->common.clid, &bip_status);
        ind_sent = TRUE;
      }
    }
  }
...
)
{
  ...
  ASSERT(cat_message_ptr && qmi_cat_state_ptr);
...
)
{
  ...
  {
    UIM_MSG_ERR_0("QMI CAT in disabled config mode");
    is_disabled = TRUE;
  }
  else if(!(cmd_type != CATI_CMD_VAL_WIDTH
...
{
  ASSERT(cat_msg_ptr && qmi_cati_global_ptr);
...
)
{
  ...
  if (QMI_ERR_NONE != uimqmi_mmgsdi_slot_to_slot_index(
  ...
  switch(cat_msg_ptr->message_type)
  {
    ...
    case QMI_CAT_MESSAGE_FRAMEWORK_CMD:
      uimqmi_process_framework_cmd(
        &cat_msg_ptr->data.framework_cmd.cmd,
        (qmi_common_svc_state_type*)qmi_cat_state_ptr,
        (qmi_common_client_state_type*)
          qmi_cat_state_ptr->client[cat_msg_ptr->data.framework_cmd.cmd.cmd_hdr.common_hdr.client_id - 1],
        qmi_cati_cmd_callbacks,
        sizeof(qmi_cati_cmd_callbacks) / sizeof(qmi_cati_cmd_callbacks[0]),
        qmi_cat_is_command_disabled);
      break;
...
)
{
  ...
  /* Check event mask against QMI CAT config mode */
  if ((qmi_cati_global_ptr->config_mode == CAT_CONFIG_MODE_DECODED_V02 &&
       tlv_data.raw_mask_present) ||
      /* SCWS events are available in decoded format only, and client may register
         it in Android config mode */
      (qmi_cati_global_ptr->config_mode == CAT_CONFIG_MODE_ANDROID_V02 &&
       tlv_data.decoded_mask_present &&
       (tlv_data.decoded_mask != QMI_CAT_DEC_EVT_REPORT_REQ_SCWS_EVENT_MASK_V02)))
  {
    errval = QMI_ERR_INVALID_OPERATION;
    goto send_result;
  }
  ...
  if (tlv_data.decoded_mask_present)
  {
    ...
    else
    {
      ...
      for(slot_index = 0; slot_index < qmi_cati_global_ptr->slot_count; slot_index++)
      {
        ...
        if (cat_client_sp->decoded_evt_report_mask[slot_index] & QMI_CAT_DEC_EVT_REPORT_REQ_BEARER_INDEPENDENT_PROTOCOL_STATUS_MASK_V02)
        {
          qmi_cat_send_buffered_bip_status(slot_index);
        }
      }
  ...
  if (tlv_data.full_func_mask_present)
  {
    ...
    for(slot_index = 0;slot_index < qmi_cati_global_ptr->slot_count; slot_index++)
    {
      ...
      if ((cat_client_sp->evt_report_mask[slot_index] & QMI_CAT_EVT_REPORT_REQ_SEND_SMS_MASK_V02) ||
          (cat_client_sp->decoded_evt_report_mask[slot_index] & QMI_CAT_DEC_EVT_REPORT_REQ_SEND_SMS_MASK_V02))
      {
        if((tlv_data.full_func_mask & CAT_FULL_FUNC_EVT_REPORT_REQ_SEND_SMS_MASK_V02) == 0)
        {
          disable_full_func_mask = disable_full_func_mask | GSTK_REG_SEND_SMS_EVT;
          new_full_func_common_mask = new_full_func_common_mask & (~CAT_FULL_FUNC_EVT_REPORT_REQ_SEND_SMS_MASK_V02);
        }
        else
        {
      ...
      {
        if ((tlv_data.full_func_mask & CAT_FULL_FUNC_EVT_REPORT_REQ_SEND_SMS_MASK_V02) ==
            CAT_FULL_FUNC_EVT_REPORT_REQ_SEND_SMS_MASK_V02)
        {
          error_full_func_mask = error_full_func_mask | CAT_FULL_FUNC_EVT_REPORT_REQ_SEND_SMS_MASK_V02;
        }
      }

      if (cat_client_sp->evt_report_mask[slot_index] & QMI_CAT_EVT_REPORT_REQ_SETUP_CALL_MASK_V02 ||
          cat_client_sp->decoded_evt_report_mask[slot_index] & QMI_CAT_EVT_REPORT_REQ_SETUP_CALL_MASK_V02)
      {
        if((tlv_data.full_func_mask & CAT_FULL_FUNC_EVT_REPORT_REQ_SETUP_CALL_MASK_V02) == 0)
        {
          disable_full_func_mask = disable_full_func_mask | GSTK_REG_SETUP_CALL_EVT;
          new_full_func_common_mask = new_full_func_common_mask & (~CAT_FULL_FUNC_EVT_REPORT_REQ_SETUP_CALL_MASK_V02);
        }
        else
        {
      ...
      {
        if((tlv_data.full_func_mask & CAT_FULL_FUNC_EVT_REPORT_REQ_SETUP_CALL_MASK_V02) ==
           CAT_FULL_FUNC_EVT_REPORT_REQ_SETUP_CALL_MASK_V02)
        {
          error_full_func_mask = error_full_func_mask | CAT_FULL_FUNC_EVT_REPORT_REQ_SETUP_CALL_MASK_V02;
        }
      }

      if (cat_client_sp->evt_report_mask[slot_index] & QMI_CAT_EVT_REPORT_REQ_SEND_SS_MASK_V02 ||
          cat_client_sp->decoded_evt_report_mask[slot_index] & QMI_CAT_EVT_REPORT_REQ_SEND_SS_MASK_V02)
      {
        if((tlv_data.full_func_mask & CAT_FULL_FUNC_EVT_REPORT_REQ_SEND_SS_MASK_V02) == 0)
        {
          disable_full_func_mask = disable_full_func_mask | GSTK_REG_SEND_SS_EVT;
          new_full_func_common_mask = new_full_func_common_mask & (~CAT_FULL_FUNC_EVT_REPORT_REQ_SEND_SS_MASK_V02);
        }
        else
        {
      ...
      {
        if((tlv_data.full_func_mask & CAT_FULL_FUNC_EVT_REPORT_REQ_SEND_SS_MASK_V02) ==
           CAT_FULL_FUNC_EVT_REPORT_REQ_SEND_SS_MASK_V02)
        {
          error_full_func_mask = error_full_func_mask | CAT_FULL_FUNC_EVT_REPORT_REQ_SEND_SS_MASK_V02;
        }
      }

      if (cat_client_sp->evt_report_mask[slot_index] & QMI_CAT_EVT_REPORT_REQ_SEND_DTMF_MASK_V02 ||
          cat_client_sp->decoded_evt_report_mask[slot_index] & QMI_CAT_EVT_REPORT_REQ_SEND_DTMF_MASK_V02)
      {
        if((tlv_data.full_func_mask & CAT_FULL_FUNC_EVT_REPORT_REQ_SEND_DTMF_MASK_V02) == 0)
        {
          disable_full_func_mask = disable_full_func_mask | GSTK_REG_SEND_DTMF_EVT;
          new_full_func_common_mask = new_full_func_common_mask & (~CAT_FULL_FUNC_EVT_REPORT_REQ_SEND_DTMF_MASK_V02);
        }
        else
        {
      ...
      {
        if((tlv_data.full_func_mask & CAT_FULL_FUNC_EVT_REPORT_REQ_SEND_DTMF_MASK_V02) ==
           CAT_FULL_FUNC_EVT_REPORT_REQ_SEND_DTMF_MASK_V02)
        {
          error_full_func_mask = error_full_func_mask | CAT_FULL_FUNC_EVT_REPORT_REQ_SEND_DTMF_MASK_V02;
        }
      }

      if (cat_client_sp->evt_report_mask[slot_index] & QMI_CAT_EVT_REPORT_REQ_SEND_USSD_MASK_V02 ||
          cat_client_sp->decoded_evt_report_mask[slot_index] & QMI_CAT_EVT_REPORT_REQ_SEND_USSD_MASK_V02)
      {
        if((tlv_data.full_func_mask & CAT_FULL_FUNC_EVT_REPORT_REQ_SEND_USSD_MASK_V02) == 0)
        {
          disable_full_func_mask = disable_full_func_mask | GSTK_REG_SEND_USSD_EVT;
          new_full_func_common_mask = new_full_func_common_mask & (~CAT_FULL_FUNC_EVT_REPORT_REQ_SEND_USSD_MASK_V02);
        }
        else
        {
      ...
      {
        if((tlv_data.full_func_mask & CAT_FULL_FUNC_EVT_REPORT_REQ_SEND_USSD_MASK_V02) ==
           CAT_FULL_FUNC_EVT_REPORT_REQ_SEND_USSD_MASK_V02)
        {
          error_full_func_mask = error_full_func_mask | CAT_FULL_FUNC_EVT_REPORT_REQ_SEND_USSD_MASK_V02;
        }
      }
      ...
      if (enable_full_func_mask != 0)
      {
        ...
        else if(qmi_cati_global_ptr->config_mode == CAT_CONFIG_MODE_DECODED_V02 ||
                qmi_cati_global_ptr->config_mode == CAT_CONFIG_MODE_CUSTOM_DECODED_V02)
        {
          gstk_status = gstk_client_toolkit_cmd_reg(
                          qmi_cati_global_ptr->cat_gstk_client_id[slot_index],
...
/*===========================================================================
  ...
  DESCRIPTION
    To get the service state of the QMI CAT
...
)
{
  ...
  if(!ind_token.is_valid && gstk_nv_get_me_feature_status(
  ...
  else if(ind_token.is_valid && gstk_nv_get_me_feature_status(
                                     GSTK_CFG_FEATURE_ME_REPORT_SW1_SW2) == FALSE)
  {
    qmi_cat_message_type * msg_ptr = NULL;
...
)
{
  ...
  if (tlv_data.envelope.env_cmd_type != CAT_ENVELOPE_CMD_TYPE_MENU_SELECTION_V02 &&
      tlv_data.envelope.env_cmd_type != CAT_ENVELOPE_CMD_TYPE_EVENT_DL_USER_ACTIVITY_V02 &&
      tlv_data.envelope.env_cmd_type != CAT_ENVELOPE_CMD_TYPE_EVENT_DL_IDLE_SCREEN_AVAIL_V02 &&
      tlv_data.envelope.env_cmd_type != CAT_ENVELOPE_CMD_TYPE_EVENT_DL_LANGUAGE_SELECTION_V02 &&
      tlv_data.envelope.env_cmd_type != CAT_ENVELOPE_CMD_TYPE_UNKNOWN_V02 &&
      tlv_data.envelope.env_cmd_type != CAT_ENVELOPE_CMD_TYPE_EVENT_DL_BROWSER_TERMINATION_V02 &&
      tlv_data.envelope.env_cmd_type != CAT_ENVELOPE_CMD_TYPE_SEND_CALL_CONTROL_V02 &&
      tlv_data.envelope.env_cmd_type != CAT_ENVELOPE_CMD_TYPE_HCI_CONNECTIVITY_V02 &&
      tlv_data.envelope.env_cmd_type != CAT_ENVELOPE_CMD_TYPE_SMS_PP_DATA_DL_V02 &&
      tlv_data.envelope.env_cmd_type != CAT_ENVELOPE_CMD_TYPE_EVENT_DL_MT_CALL_V02 &&
      tlv_data.envelope.env_cmd_type != CAT_ENVELOPE_CMD_TYPE_EVENT_DL_CALL_CONNECTED_V02 &&
      tlv_data.envelope.env_cmd_type != CAT_ENVELOPE_CMD_TYPE_EVENT_DL_CALL_DISCONNECTED_V02 &&
      tlv_data.envelope.env_cmd_type != CAT_ENVELOPE_CMD_TYPE_CONTACTLESS_STATE_REQUEST_V02 )
  {
    errval = QMI_ERR_INVALID_ARG;
    goto send_result;
  }
  else if( tlv_data.envelope.env_cmd_type == CAT_ENVELOPE_CMD_TYPE_SMS_PP_DATA_DL_V02 &&
           (qmi_cati_global_ptr->sms_pp_envelope_blocked))
  {
    errval = QMI_ERR_INVALID_OPERATION;
    goto send_result;
...
)
{
  ...
  for(slot_index = 0; slot_index < QMI_CAT_MAX_CARD_COUNT; slot_index ++)
  {
    ...
    cl_sp->decoded_evt_report_mask[slot_index] = CATI_REPORT_STATUS_NONE;
  }
...
)
{
  ...
  for (slot_index = 0; slot_index < qmi_cati_global_ptr->slot_count; slot_index++)
  {
    ...
    if (GSTK_SUCCESS != gstk_status)
    {
      ...
      proactive_cmd.pro_cmd_reg_data.gstk_req_rsp_ptr = NULL;
      goto send_result;
    }
    ...
    if (proactive_cmd.data_format_type == GSTK_GSTK_FORMAT)
    {
      if (proactive_cmd.pro_cmd_reg_data.gstk_req_rsp_ptr->hdr_cmd.command_id == GSTK_END_PROACTIVE_CMD_REQ)
      {
        ...
        {
          end_session_type = CAT_PROACTIVE_SESSION_END_TYPE_CARD_V02;
        }
        else
        {
          end_session_type = CAT_PROACTIVE_SESSION_END_TYPE_INTERNAL_V02;
        }
...
)
{
  ...
  {
    req_slot = (gstk_slot_id_enum_type)tlv_data_ptr->slot.slot;
    errval = qmi_cati_convert_gstk_slot_id_to_qmi_slot_index(
                            req_slot,
                            &slot_index);
    if (errval != QMI_ERR_NONE)
  ...
  /* Result will be returned via indication if indication token is set and indication is only
     for envelope command with response bytes */
  if (tlv_data_ptr->ind_token_present &&
      (tlv_data_ptr->env_hdr.env_cmd_type == CAT_DECODED_ENVELOPE_CMD_TYPE_SEND_CALL_CONTROL_V02 ||
       tlv_data_ptr->env_hdr.env_cmd_type == CAT_DECODED_ENVELOPE_CMD_TYPE_SMS_PP_DATA_DL_V02))
  {
    ind_token.is_valid = TRUE;
  ...
  {
    case CAT_DECODED_ENVELOPE_CMD_TYPE_MENU_SELECTION_V02:
    ...
    case CAT_DECODED_ENVELOPE_CMD_TYPE_EVENT_DL_LANGUAGE_SELECTION_V02:
      /* Check mandatory items */
    ...
    case CAT_DECODED_ENVELOPE_CMD_TYPE_EVENT_DL_USER_ACTIVITY_V02:
    ...
    case CAT_DECODED_ENVELOPE_CMD_TYPE_HCI_CONNECTIVITY_V02:
    ...
    case CAT_DECODED_ENVELOPE_CMD_TYPE_EVENT_DL_IDLE_SCREEN_AVAIL_V02:
    ...
    case CAT_DECODED_ENVELOPE_CMD_TYPE_SEND_CALL_CONTROL_V02:
      cc_ind_ptr =
      ...
      if((tlv_data_ptr->call_type_present &&
         (tlv_data_ptr->call_type.call_type == CAT_CALL_CONTROL_VOICE_V02))||
      ...
      {
        if ((tlv_data_ptr->address.length    > QMI_CAT_ADDRESS_MAX_LENGTH_V02) ||
            (tlv_data_ptr->subaddress.length > QMI_CAT_SUBADDRESS_MAX_LENGTH_V02) ||
            (!tlv_data_ptr->call_type_present &&
        ...
        (void)memscpy(cc_ind_ptr->gstk_address_string.voice.ccp2.ccp,
                ...
                (tlv_data_ptr->call_type.call_type == CAT_CALL_CONTROL_SS_V02))||
                ...
                && !tlv_data_ptr->ccp2_present))
      {
        ...
        (void)memscpy(cc_ind_ptr->gstk_address_string.ss_string.address,
               ...
               (tlv_data_ptr->call_type.call_type ==  CAT_CALL_CONTROL_USSD_V02))||
               (!tlv_data_ptr->call_type_present)) &&
      ...
      else if (!tlv_data_ptr->call_type_present && tlv_data_ptr->eps_pdn_conn_act_present)
      {
        ...
        (void)memscpy(cc_ind_ptr->gstk_address_string.eps_conn_act.eps_act_param,
                ...
                ((tlv_data_ptr->call_type_present &&
                 tlv_data_ptr->call_type.call_type == CAT_CALL_CONTROL_IMS_V02) ||
                (!tlv_data_ptr->call_type_present)))
      {
        cc_ind_ptr->call_enum_type = GSTK_IMS_CALL;

        cc_ind_ptr->gstk_address_string.ims_call.length = tlv_data_ptr->ims_uri.length;
        /* Copy IMS URI truncated flag only if the length is greater than or equal to
           GSTK_MAX_IMS_CALL_PARAM_SIZE */
        if (tlv_data_ptr->ims_uri.length >= GSTK_MAX_IMS_CALL_PARAM_SIZE)
        {
          cc_ind_ptr->gstk_address_string.ims_call.length = GSTK_MAX_IMS_CALL_PARAM_SIZE;
          if (tlv_data_ptr->ims_uri.length == GSTK_MAX_IMS_CALL_PARAM_SIZE)
          {
            if (tlv_data_ptr->ims_uri_truncated_present)
            {
              cc_ind_ptr->gstk_address_string.ims_call.ims_uri_truncated =
                tlv_data_ptr->ims_uri_truncated.ims_uri_truncated;
            }
          }
          else
          {
            cc_ind_ptr->gstk_address_string.ims_call.ims_uri_truncated = TRUE;
          }
        }
        ...
        (void)memscpy(cc_ind_ptr->gstk_address_string.ims_call.ims_uri,
                      ...
                      cc_ind_ptr->gstk_address_string.ims_call.length);  
      }
      else if (tlv_data_ptr->call_type_present &&
               (tlv_data_ptr->call_type.call_type == CAT_CALL_CONTROL_SMS_V02))
      {
    ...
    case CAT_DECODED_ENVELOPE_CMD_TYPE_BROWSER_TERMINATION_V02:
      /* Check mandatory items */
     ...
     case CAT_DECODED_ENVELOPE_CMD_TYPE_SMS_PP_DATA_DL_V02:
      ...
      if (!tlv_data_ptr->sms_tpdu_present ||
          (tlv_data_ptr->sms_tpdu.length > ((QMI_CAT_SMS_TPDU_MAX_LENGTH_V02))) ||
          !tlv_data_ptr->address_present  ||
          (tlv_data_ptr->address.length > QMI_CAT_ADDRESS_MAX_LENGTH_V02))
      {
        errval = QMI_ERR_MISSING_ARG;
        goto send_result;
      }
      ...
      gstk_status = gstk_slot_send_envelope_sms_pp_dl_command(req_slot,
                                                              (uint32)cb_userdata_ptr,
                                                         is_cdma_sms,
                                                         &sms_pp_dl_address,
                                                         &sms_tpdu,
    ...
    case CAT_DECODED_ENVELOPE_CMD_TYPE_EVENT_DL_MT_CALL_V02:
      /* Check mandatory items */
      ...
      /* validate TLVs */
      if ((tlv_data_ptr->transaction_id.length > ((QMI_CAT_TX_ID_MAX_LENGTH_V02))) ||
          (tlv_data_ptr->address_present &&
           (tlv_data_ptr->address.length > QMI_CAT_ADDRESS_MAX_LENGTH_V02)) ||
    ...
    case CAT_DECODED_ENVELOPE_CMD_TYPE_EVENT_DL_MT_CALL_CONNECTED_V02:
    case CAT_DECODED_ENVELOPE_CMD_TYPE_EVENT_DL_MO_CALL_CONNECTED_V02:
      /* Check mandatory items */
      if (!tlv_data_ptr->transaction_id_present ||
          (tlv_data_ptr->transaction_id.length > ((QMI_CAT_TX_ID_MAX_LENGTH_V02))))
      {
        errval = QMI_ERR_MISSING_ARG;
        goto send_result;
      }
      ...
      evt_list.evt_ind.call_connected_evt_ind.transaction_id.transaction_list
        = tlv_data_ptr->transaction_id.transaction_id;
      if (tlv_data_ptr->env_hdr.env_cmd_type ==CAT_DECODED_ENVELOPE_CMD_TYPE_EVENT_DL_MT_CALL_CONNECTED_V02)
      {
    ...
    case CAT_DECODED_ENVELOPE_CMD_TYPE_EVENT_DL_CALL_DISCONNECTED_NEAR_END_V02:
    case CAT_DECODED_ENVELOPE_CMD_TYPE_EVENT_DL_CALL_DISCONNECTED_FAR_END_V02:
      /* Check mandatory items */
      if (!tlv_data_ptr->transaction_id_present ||
          (tlv_data_ptr->transaction_id.length > ((QMI_CAT_TX_ID_MAX_LENGTH_V02))))
      {
      ...
      if(tlv_data_ptr->cause_present &&
         (tlv_data_ptr->cause.length <= QMI_CAT_CAUSE_MAX_LENGTH_V02))
      {
      ...
      if (tlv_data_ptr->env_hdr.env_cmd_type == CAT_DECODED_ENVELOPE_CMD_TYPE_EVENT_DL_CALL_DISCONNECTED_NEAR_END_V02)
      {
    ...
    case CAT_DECODED_ENVELOPE_CMD_TYPE_CONTACTLESS_STATE_REQUEST_V02:
      /* Check mandatory items */
      if (!tlv_data_ptr->contactless_state_request_present)
      {
        errval = QMI_ERR_MISSING_ARG;
        goto send_result;
      }

      evt_list.evt_type = GSTK_CONTACTLESS_STATE_REQ_EVT_IND;
      switch (tlv_data_ptr->contactless_state_request.contactless_state_request)
      {
        case 0x00:
          evt_list.evt_ind.contactless_state_req_evt_ind = GSTK_CONTACTLESS_STATE_DISABLE;
          break;
        case 0x01:
          evt_list.evt_ind.contactless_state_req_evt_ind = GSTK_CONTACTLESS_STATE_ENABLE;
          break;
        case 0x02:
          evt_list.evt_ind.contactless_state_req_evt_ind = GSTK_CONTACTLESS_STATE_REQUEST;
          break;
        default:
          UIM_MSG_ERR_1("qmi_cati_decoded_envelope_cmd: Invalid contactless_state_request 0x%x",
                        tlv_data_ptr->contactless_state_request.contactless_state_request);
          errval = QMI_ERR_INVALID_ARG;
          goto send_result;
      }
      ...
      if (gstk_status != GSTK_SUCCESS)
      {
        UIM_MSG_HIGH_1("send GSTK_CONTACTLESS_STATE_REQ_EVT_IND: gstk_status = status %d",
                       gstk_status);
        errval = qmi_cati_map_gstk_status_to_errval(gstk_status);
        goto send_result;
      }
  ...
  if (ind_token.is_valid && QMI_ERR_NONE == errval)
  {
    /* add indication token to the immediate response */
    errval = qmi_cat_response_indication_token(&response,
                                               ind_token.token,
...
)
{
  ...
  switch (tlv_data.term_resp.response_cmd)
  {
   ...
   case CAT_RESPONSE_CMD_PROVIDE_LOCAL_INFO_BATTERY_STATE_V02:
      if (tlv_data.battery_status_present)
      {
        extra_info.present = TRUE;
        extra_info.rsp_type = GSTK_PROVIDE_BATT_STATUS_CNF;
        switch (tlv_data.battery_status.status)
        {
          case 0:
            extra_info.extra_param.battery_status_info_extra_param = GSTK_BATTERY_ST_VERY_LOW;
            break;
          case 1:
            extra_info.extra_param.battery_status_info_extra_param = GSTK_BATTERY_ST_LOW;
            break;
          case 2:
            extra_info.extra_param.battery_status_info_extra_param = GSTK_BATTERY_ST_AVG;
            break;
          case 3:
            extra_info.extra_param.battery_status_info_extra_param = GSTK_BATTERY_ST_GOOD;
            break;
          case 4:
            extra_info.extra_param.battery_status_info_extra_param = GSTK_BATTERY_ST_FULL;
            break;
          case 5:
            extra_info.extra_param.battery_status_info_extra_param = GSTK_BATTERY_ST_NA;
            break;
          case 6:
          default:
            extra_info.extra_param.battery_status_info_extra_param = GSTK_BATTERY_ST_UNKNOWN;
            break;
        }
      }
      break;
...
)
{
  ...
  {
    /* Event confirmation need to be for user confirmation */
    if (qmi_cati_global_ptr->user_cnf_required[slot_index])
    {
...
)
{
  ...
  if(tlv_data.config_mode.config_mode == CAT_CONFIG_MODE_CUSTOM_DECODED_V02 ||
     tlv_data.config_mode.config_mode == CAT_CONFIG_MODE_CUSTOM_RAW_V02)
  {
    ...
    if (tlv_data.custom_tp.tp_len > QMI_CAT_TERMINAL_PROFILE_MAX_LENGTH_V02)
    {
...
)
{
  ...
  qmi_result_e_type            result   = QMI_RESULT_SUCCESS;
  uint8                        tp_len   = 0;
  uint8                        tp[QMI_CAT_TERMINAL_PROFILE_MAX_LENGTH_V02];

  ASSERT(sp && cmd_buf_p && cl_sp && sdu_in && qmi_cati_global_ptr);

  /* Add TLV to response */
  if(qmi_cati_global_ptr->config_mode == CAT_CONFIG_MODE_CUSTOM_RAW_V02 ||
     qmi_cati_global_ptr->config_mode == CAT_CONFIG_MODE_CUSTOM_DECODED_V02)
  {
