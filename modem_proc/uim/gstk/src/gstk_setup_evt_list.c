...
Copyright (c) 2003-2017, 2019 QUALCOMM Technologies, Inc (QTI) and its licensors.
...
$Header: //components/rel/uim.mpss/6.1.0/gstk/src/gstk_setup_evt_list.c#2 $
...
03/20/17   shr     Fix compilation errors due to FEATURE_LTE disablement
...
{
  if ((STK_response == NULL) || (setup_evt_list_cnf == NULL))
  {
    return;
  }
  ...
  if (gstk_shared_data.gstk_client_table[client_id-1].client_cb != NULL)
  {
    if (data_format == GSTK_RAW_FORMAT)
    {
      ...
      if (((*cli_rcvd_raw_cmd_flg_ptr) & cli_bitmask) == 0)
      {
        ...
        raw_cmd_fmt.payload.data_len = pro_cmd_ptr->length;
...
gstk_status_enum_type gstk_setup_evt_list_req (gstk_proactive_cmd_type* cmd_ptr)
{
  ...
  */
  ...
  if(!error_already_raised) {
    ...
    GSTK_UTIL_SELECTIVE_DUMP_BYTE_ARRAY("new_event_list_mask", new_event_list_mask, GSTK_MAX_EVENT_LIST_LEN);
    ...
    if (gstk_status == GSTK_SUCCESS ) {
      ...
      if ((new_event_list_mask[GSTK_USER_ACTIVITY] & 0x01) == 0x01){ /* user act */
        ...
        UIM_SELECTIVE_MSG_HIGH_0(UIM_LOG_TOOLKIT, "... Register User act");
      }
      else {
      ...
      if ((gstk_status == GSTK_SUCCESS) || (gstk_status == GSTK_CLIENT_NOT_REGISTERED)) {
        ...
        if ((new_event_list_mask[GSTK_IDLE_SCRN_AVAILABLE] & 0x01) == 0x01) { /* idle scrn */
          request_ptr->cmd.evt_notify_req.require_notification = TRUE;
          UIM_SELECTIVE_MSG_HIGH_0(UIM_LOG_TOOLKIT, "... Register idle");
        }
        else {
          request_ptr->cmd.evt_notify_req.require_notification = FALSE;
        }
        ...
        if ((gstk_status == GSTK_SUCCESS) || (gstk_status == GSTK_CLIENT_NOT_REGISTERED)) {
          ...
          if ((new_event_list_mask[GSTK_LANGUAGE_SELECTION] & 0x01) == 0x01) { /* lang sel */
            request_ptr->cmd.evt_notify_req.require_notification = TRUE;
            UIM_SELECTIVE_MSG_HIGH_0(UIM_LOG_TOOLKIT, "... Register Language Selection");
          }
          else {
            request_ptr->cmd.evt_notify_req.require_notification = FALSE;
          }
          ...
          if ((gstk_status == GSTK_SUCCESS) || (gstk_status == GSTK_CLIENT_NOT_REGISTERED)) {
            ...
            if ((new_event_list_mask[GSTK_BROWSER_TERMINATION] & 0x01) == 0x01) { /* browser term */
              request_ptr->cmd.evt_notify_req.require_notification = TRUE;
              UIM_SELECTIVE_MSG_HIGH_0(UIM_LOG_TOOLKIT, "... Register Browser Termination");
            }
            else {
              request_ptr->cmd.evt_notify_req.require_notification = FALSE;
            }
            ...
            if ((gstk_status == GSTK_SUCCESS) || (gstk_status == GSTK_CLIENT_NOT_REGISTERED)) {
              ...
              if ((new_event_list_mask[GSTK_DATA_AVAILABLE] & 0x01) == 0x01) { /* data avail */
                request_ptr->cmd.evt_notify_req.require_notification = TRUE;
                UIM_SELECTIVE_MSG_HIGH_0(UIM_LOG_TOOLKIT, "... Register Data Available");
              }
              else {
                request_ptr->cmd.evt_notify_req.require_notification = FALSE;
              }
              ...
              if ((gstk_status == GSTK_SUCCESS) || (gstk_status == GSTK_NO_CLIENT_REG_FOUND)) {
                /* reset gstk_status */
                gstk_status = GSTK_SUCCESS;
                ...
                if ((new_event_list_mask[GSTK_CHANNEL_STATUS] & 0x01) == 0x01) { /* data avail */
                  request_ptr->cmd.evt_notify_req.require_notification = TRUE;
                  UIM_SELECTIVE_MSG_HIGH_0(UIM_LOG_TOOLKIT, "... Register Channel Status");
                }
                else {
                  request_ptr->cmd.evt_notify_req.require_notification = FALSE;
                }
                ...
                if (nv_data.cat_version >= GSTK_CFG_CAT_VER6
                    &&
                    ((gstk_status == GSTK_SUCCESS) || (gstk_status == GSTK_NO_CLIENT_REG_FOUND)) )
                {
                  gstk_status = GSTK_SUCCESS;
                ...
                if ((gstk_status == GSTK_SUCCESS) || (gstk_status == GSTK_NO_CLIENT_REG_FOUND)) {
                  ...
                  if ((new_event_list_mask[GSTK_HCI_CONNECTIVITY] & 0x01) == 0x01) { /* hci conn */
                    request_ptr->cmd.evt_notify_req.require_notification = TRUE;
                    UIM_SELECTIVE_MSG_HIGH_0(UIM_LOG_TOOLKIT, "... Register HCI Connectivity");
                  }
                  else {
                    request_ptr->cmd.evt_notify_req.require_notification = FALSE;
                  }
                  ...
                  if ((gstk_status == GSTK_SUCCESS) || (gstk_status == GSTK_CLIENT_NOT_REGISTERED)) {
                    ...
                    /* Contactless State Request Event */
                    /* build Message to clients */
                    request_ptr->hdr_cmd.command_id = GSTK_SETUP_EVT_CONTACTLESS_STATE_REQ_NOTIFY;
                    if ((new_event_list_mask[GSTK_CONTACTLESS_STATE_REQ] & 0x01) == 0x01) { /* contactless state req */
                      request_ptr->cmd.evt_notify_req.require_notification = TRUE;
                      UIM_SELECTIVE_MSG_HIGH_0(UIM_LOG_TOOLKIT, "... Register Contactless State Req");
                    }
                    else {
                      request_ptr->cmd.evt_notify_req.require_notification = FALSE;
                    }
                    /* Send command to clients only if any updates were received for this bit */
                    if(new_event_list_mask[GSTK_CONTACTLESS_STATE_REQ] !=
                  ...
                  } /* gstk_success from contactless state req (de)registration */
                } /* HCI Connectivity */
              } /* data available */
  ...
  gstk_setup_evt_list_cnf_type* cnf_ptr)
{
  ...
  */
  ...
  if (uim_cmd_ptr != NULL ) {
    ...
    if (gstk_status == GSTK_SUCCESS) {
      if (uim_cmd_ptr != NULL) {
        ...
        /* set user_data */
        uim_cmd_ptr->hdr.user_data               = cnf_ptr->message_header.client_id;
        ...
        if (gstk_status == GSTK_SUCCESS)
        {
          GSTK_UTIL_SELECTIVE_DUMP_BYTE_ARRAY("Setup Event List",
     ...
     )
  {
    ...
    {
      UIM_MSG_HIGH_2("Location status already changed! Send env 0x%x 0x%x",
  ...
  const gstk_client_raw_term_resp_type *raw_cnf_ptr)
{
  ...
  cnf.message_header.user_data = raw_cnf_ptr->client_id;
