...
$Header: //components/rel/uim.mpss/6.1.0/gstk/src/gstk_s_no_sim.c#2 $

when         who     what, where, why
--------   ---     --------------------------------------------------------------------
03/27/17   dt      Add psm status to TP from MMGSDI
10/07/16   shr     GSTK/ESTK F3 reduction
09/23/16   gs      F3 reduction
...
06/05/15   gm      Reset gstk sub-state after card error
  ...
  gstk_cmd_type  * cmd_ptr )
{
  ...
  case GSTK_TIMER_EXP_CMD:
    if(cmd_ptr->timer_msg.message_header.command_id == GSTK_ATTACH_CTRL_TIMER)
    {
