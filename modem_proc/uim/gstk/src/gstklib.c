...
$Header: //components/rel/uim.mpss/6.1.0/gstk/src/gstklib.c#3 $ $DateTime: 2019/10/30 06:56:05 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
10/22/19   ks      Fix GSTK compilation errors
...
06/07/17   dt      Removing function gstk_send_envelope_sms_pp_dl_command
06/07/17   dt      Removing GSTK_MAX_TP_DATA_SIZE
06/01/17   gs      Don't depend on DS Sys Status for establishing BIP on non-DDS
05/17/17   vr      Handle invalid refresh command with no alpha
05/04/17   vr      Dummy TR for proactive command with length 0
03/27/17   dt      Add psm status to TP from MMGSDI
02/14/17   gs      Remove No Serivce check
11/02/16   vr      Long IMS URI in envelope commands
...
09/22/16   gs      Control the IoE related functionlity
08/09/16   vr      Queue the location status to front of the gstk queue
06/20/16   shr     Fix porting error
...
01/06/16   gs      Removed the logic of setting CDMA as rat when both LTE and 
                   CDMA are active
11/30/15   vr      GSTK CC retry to be enabled only after the busy from card
10/09/15   gm      Converting to unsigned variables.
08/07/15   vr      Validate input data in GSTK_IO_CTRL function
06/25/15   gm      L4 compiler warning
06/05/15   gm      Split ME and slot specific NV
05/27/15   vr      Cleanup fetch flags before queuing recovery start command
04/27/15   gm      Added support for class "n" command
04/24/15   vr      Check Ignore and send null and no alpha NVs for refresh
                   alpha tag parsing during parallel processing
04/22/15   vr      Fixed Check for refresh alpha tag for parallel processing
...
03/11/15   dy      Enabling Toolkit-initiated GBA TP bit (byte 25 bit 5)
03/11/15   vr      UT multiple SIM SAT support
11/27/15   dy      Added support for class "r" commands
01/22/15   dy      Reduce GSTK priv data size
01/14/15   gm      Update slot according to client_id
...
)
{
  ...
  if(task_cmd != NULL)
  {
    ...
    {
      /* set GSTK_CMD_Q_SIG and put on GSTK queue */
      if (evt_list->evt_type == GSTK_LOCATION_STATUS_EVT_IND ||
          evt_list->evt_type == GSTK_ACCESS_TECH_EVT_IND)
      {
...
)
{
  ...
  if ( task_cmd != NULL )
  {
    ...
    gstk_memcpy(&(task_cmd->cmd.mo_sms_ctrl_envelope_ind.mo_sms_ctrl_data),
               mo_sms_control_data_ptr,
               sizeof(task_cmd->cmd.mo_sms_ctrl_envelope_ind.mo_sms_ctrl_data),
               sizeof(task_cmd->cmd.mo_sms_ctrl_envelope_ind.mo_sms_ctrl_data),
               sizeof(gstk_mo_sms_ctrl_ind_ext_type));  

    task_cmd->cmd.mo_sms_ctrl_envelope_ind.mo_sms_requeue = FALSE;
    /* set GSTK_CMD_Q_SIG and put on GSTK queue */
    gstk_task_put_cmd_buf(task_cmd);
  }
  else
    ...
    const gstk_profile_dl_support_ext_type    *value_list_ptr )
{
  ...
  /* successfully obtained command buffer, build the terminal profile command */
  ...
  task_cmd_ptr->cmd.terminal_profile_cmd.data_ptr = (uint8*)GSTK_CALLOC(i);
  
  if(task_cmd_ptr->cmd.terminal_profile_cmd.data_ptr == NULL)
  {
    gstk_task_free_cmd_buf(task_cmd_ptr);
    return GSTK_MEMORY_ERROR;
  }
           ...
           {
    task_cmd_ptr->cmd.terminal_profile_cmd.data_ptr[j++] =
  ...
  gstk_cmd_enum_type  * expected_cmd_rsp)
{
  ...
  switch(request_cmd)
  {
    ...
    case GSTK_CONTACTLESS_STATE_CHANGED_REQ:
        *expected_cmd_rsp = GSTK_CONTACTLESS_STATE_CHANGED_CNF;
        break;
    case GSTK_GEO_LOC_REQ:
        *expected_cmd_rsp = GSTK_GEO_LOC_CNF;
        break;        
    default:
...
/*===========================================================================
FUNCTION gstk_uim_send_fetch_cmd_report

DESCRIPTION

  This function allows clients to put a fetch cmd report before passing
...
PARAMETERS
  ...
  sw1        : [Input] Status word 1
  sw1        : [Input] Status word 2
...
SEE ALSO
  None
===========================================================================*/
gstk_status_enum_type gstk_uim_send_fetch_cmd_report(
  uim_slot_type   slot,
  uint32          data_length,
...
)
{
  ...
  if (sw1 != SW1_END_FETCH && sw1 != SW1_NORMAL_END)
  {
    UIM_MSG_ERR_2("Fetch failed sw1: 0x%x, sw2: 0x%x ", sw1, sw2);
    gstk_status = GSTK_ERROR;
  }
  else if (data_ptr != NULL)
  {
    if (data_length > GSTK_MAX_DATA_SIZE ||
        data_length < GSTK_PROACTIVE_COMMAND_MINIMUM_LEN)
    {
    ...
    else
    {
      ...
      if (!gstk_nv_get_me_feature_status(GSTK_CFG_FEATURE_ME_DISABLE_PCMD_SELECTIVE_PARALLEL_PROCESSING))
      {
        ...
        if(data_ptr[1] == GSTK_2_BYTE_LENGTH_FIRST_VALUE)
        {
          cmd_bit++;
        }
        switch(data_ptr[cmd_bit])
        {
          ...
          case GSTK_CMD_STK_REFRESH:
            if(gstk_nv_get_me_feature_status(GSTK_CFG_FEATURE_ME_ESTK_IGNORE_ALPHA) == FALSE)
            {
              ...
              else
              {
                ...
                cmd_bit += 1 + 4 + 1;
                /* Check for alpha tag */
                while((cmd_bit < data_length) && ((data_ptr[cmd_bit] & 0x7F) !=  GSTK_ALPHA_IDENTIFIER_TAG))
                {
                  /* Not alpha tag */
                  if(cmd_bit < (data_length - 1))
                  {
                    /* Increment length bytes */
                    cmd_bit += gstk_find_length_of_length_value(&data_ptr[cmd_bit + 1]);

                    if(cmd_bit < data_length)
                    {
                      /* length + Tag */
                      cmd_bit += data_ptr[cmd_bit];
                    }
                  }
                  /* next tag */
                  cmd_bit++;
                ...
                {
                  if((cmd_bit < (data_length - 1)) && (data_ptr[cmd_bit + 1] != 0))
                  {
                    gstk_shared_data.ui_nw_proactive_session_slot_in_progress = (gstk_slot_id_enum_type)slot;
                  }
                }
  ...
  if(gstk_status != GSTK_SUCCESS)
  {
    ...
    /* Send TR only if FETCH success */
    if (sw1 == SW1_END_FETCH || sw1 == SW1_NORMAL_END)
    {
      gstk_status = gstk_send_dummy_tr_to_uim(data_ptr, data_length, slot);
    }
    
    /* TR not going to card, reset gstk_proactive_cmd_in_progress flag */ 
    if (gstk_status != GSTK_SUCCESS)
    {
      GSTK_SET_PROACTIVE_CMD_IN_PROGRESS_FLAG(
        gstk_instances_ptr[slot - UIM_SLOT_1]->gstk_proactive_cmd_in_progress, FALSE);
    }
  }
  ...
  return gstk_status;
} /* gstk_uim_send_fetch_cmd_report */
...
)
{
  ...
  {
      UIM_MSG_HIGH_3("Don't allow slot 0x%x to fetch (PCMD on slot 0x%x or fetch command on slot 0x%x) is in progress !",
...
)
{
  ...
  /* dump CCP data */
  GSTK_UTIL_SELECTIVE_DUMP_BYTE_ARRAY("CCP", ccp_ptr->ccp, ccp_ptr->length);
...
/*============================================================================
  ...
  DESCRIPTION:
    General interface to set/get GSTK data. The I/O operations currently
    supported are:
    0) GSTK_IO_GET_CLIENT_TABLE_NEW_CLIENT_CB: return event callback function
    ...
    1) GSTK_IO_GET_CLIENT_TABLE_SLOT_ID: get the slot ID that the client is
       associated with. param_in_ptr points to the client ID and param_out_ptr
       points to slot ID returned.
    2) GSTK_IO_GET_CLIENT_TABLE_USER_DATA: get user data of a client.
       param_in_ptr points to client ID and param_out_ptr points to the user
       data of uint32 type
    3) GSTK_IO_GET_CMD_RAW_APDU: get raw APUD of a proactive command referred
    ...
    4) GSTK_IO_FIND_CLIENT_WITH_REGISTERED_EVENT: call the function
    ...
    5) GSTK_IO_SET_CURR_SESSION_UI_RELATED: set the value of
       ...
       nothing is returned)
    6) GSTK_IO_CALL_CLIENT_EVT_CB: invoke gstk_util_call_client_evt_cb().
    ...
    7) GSTK_IO_MODEM_AS_ID_TO_SLOT_ID: convert modem active subscription ID to
       ...
       returned.
    8) GSTK_IO_SLOT_ID_TO_MODEM_AS_ID: convert GSTK slot ID to modem active
       ...
       returned.
    9) GSTK_IO_DS_SUBSCRIPTION_ID_TO_SLOT_ID: Convert DS subscription ID to
       slot ID.
    10)GSTK_IO_GET_CURR_ACCESS_TECH: read evt_cb_type of
       gstk_client_table. param_in_ptr points to client ID and param_out_ptr
       points to a variable of gstk_access_technology_type.
    11)GSTK_IO_SEND_MM_IDLE_EVENT: Send MM IDLE event from NAS to GSTK
    12)GSTK_IO_SEND_EMM_REGISTERED_EVENT: Send MM IDLE from LTE-NAS to GSTK
    13)GSTK_IO_SEND_MM_INDICATION: Send MM indication from NAS to GSTK
    14)GSTK_IO_SEND_NW_REJ_EVENT: Send NW Reject event from NAS to GSTK
    15)GSTK_IO_GET_CURR_LOC_STATUS: Get the location status for the slot
       from the GSTK cache
    16)GSTK_IO_GET_TERMINAL_PROFILE_DATA: retrieve the cached TP info
    17)GSTK_IO_MANAGE_BIP_OTASP_CONCURRENCY: Manage BIP/OTASP concurrency
    18)GSTK_IO_GET_DS_SYSTEM_RAT_INFO: Get preferred network and the RAT details
       obtained from DS for the slot
    19)GSTK_IO_GET_WLAN_AVAILABILITY: Get availability of WLAN network
    20)GSTK_IO_SIM_INITIATED_MODIFIED_CC_RESP: Copy the call control response
       sent to the client in case of SIM initiated call control response
       modified to another call type
    21)GSTK_IO_GET_ACTIVE_DATA_SUB_SLOT_ID: get the slot ID corresponding to the
       active DATA subscription
    22)GSTK_IO_SEND_WMS_MT_MESSAGE_ERROR_EVENT: Used to send MT message error on 
         receiving WMS_MSG_EVENT_MT_MESSAGE_ERROR event.
    23)GSTK_IO_SLOT_ID_TO_DS_SUBSCRIPTION_ID: Get the Active Data sub to corresponding
       slot id
    24)GSTK_IO_RESET_WMS_IS_CP_ACK_PENDING_FLAG: reset is_cp_ack_pending flag
    25)GSTK_IO_GET_WMS_IS_CP_ACK_PENDING_FLAG: read is_cp_ack_pending flag
...
)
{
  ...
  ds_profile_subs_etype                                   ds_sub_id            = DS_PROFILE_ACTIVE_SUBSCRIPTION_NONE;
  ...
  if((io_cmd != GSTK_IO_MODEM_AS_ID_TO_SLOT_ID) &&
     ...
     (io_cmd != GSTK_IO_SLOT_ID_TO_DS_SUBSCRIPTION_ID))
  {
  ...
  case GSTK_IO_GET_CLIENT_TABLE_NEW_CLIENT_CB:
    ...
    if(*client_id_ptr > GSTK_MAX_CLIENT || *client_id_ptr == 0)
    {
      UIM_MSG_ERR_1("gstk_io_ctrl(): client_id_ptr 0x%x, is invalid", *client_id_ptr);
      return GSTK_BAD_PARAM;
    }
  ...
  case GSTK_IO_GET_CMD_RAW_APDU:
    ...
    {
      UIM_MSG_ERR_1("gstk_io_ctrl(): cmd_details_ref_id 0x%x, is invalid", GSTK_CMD_REF_TABLE_INDEX(cmd_details_ref_id));
      return GSTK_BAD_PARAM;
    }
    ...
    raw_cmd_ptr->data_len = len;
    if(raw_cmd_ptr->data_len == 0)
    {
      UIM_MSG_ERR_1("gstk_io_ctrl(): raw_cmd.data_len 0x%x, is invalid", len);
      return GSTK_BAD_PARAM;
    }
        ...
        for (j = 0; j < GSTK_MMGSDI_SESS_INFO_SIZE; ++j)
        {
          ...
          for (k = 0; k < GSTK_AS_MAX_SESSION; k++)
          {
              ...
              gstk_shared_data.modem_as_info[i].sess_types[k])
            {
              ...
              if (GSTK_IS_VALID_SLOT_ID(slot))
              {
                /* To avoid F3 spike, prints only in case cross-mapping / unexpected 
                   configuration */
                if (!((as_id == SYS_MODEM_AS_ID_1 && slot == GSTK_SLOT_1) ||
                      (as_id == SYS_MODEM_AS_ID_2 && slot == GSTK_SLOT_2)))
                {
  ...
  case GSTK_IO_GET_CURR_LOC_STATUS:
    ...
    if (!GSTK_IS_VALID_SLOT_ID(slot))
    {
      UIM_MSG_ERR_1("GSTK IO: Location Status Invalid slot id 0x%x!", slot);
      return GSTK_ERROR;
    }
    GSTK_RETURN_ERROR_IF_NULL_PTR(gstk_instances_ptr[(uint8)slot - 1], GSTK_ERROR);
    (void)gstk_memcpy(param_out_ptr,
  ...
  case GSTK_IO_GET_WLAN_AVAILABILITY:
    if(gstk_shared_data.wlan_info_ptr != NULL)
    {
      (*(boolean *)param_out_ptr) = gstk_shared_data.wlan_info_ptr->is_wlan_available;
    }
    else
    {
      (*(boolean *)param_out_ptr) = FALSE;
  ...
  case GSTK_IO_SLOT_ID_TO_DS_SUBSCRIPTION_ID:
    if(gstk_shared_data.gstk_num_of_instances == 1)
    {
      ds_sub_id = DS_PROFILE_ACTIVE_SUBSCRIPTION_1;
      (void)gstk_memcpy(param_out_ptr,
 			&ds_sub_id,
 			sizeof(ds_sys_subscription_enum_type),
 			sizeof(ds_sys_subscription_enum_type),
 			sizeof(ds_sys_subscription_enum_type));
      break;
    }
        ...
        &as_id) == GSTK_SUCCESS)
    {
      switch(as_id)
      {
        ...
        case SYS_MODEM_AS_ID_2:
          ds_sub_id = DS_PROFILE_ACTIVE_SUBSCRIPTION_2;
          break;
        case SYS_MODEM_AS_ID_3:
          ds_sub_id = DS_PROFILE_ACTIVE_SUBSCRIPTION_3;
          break;
        default:
          UIM_MSG_ERR_1("Unknown AS_ID 0x%x!", ds_sub_id);
          return GSTK_ERROR;
      }
      (void)gstk_memcpy(param_out_ptr,
                        &ds_sub_id,
    ...
    break;
  case GSTK_IO_RESET_WMS_IS_CP_ACK_PENDING_FLAG: 
    as_id = (*(sys_modem_as_id_e_type *)(param_in_ptr));
    
    if (!GSTK_IS_VALID_AS_ID(as_id))
    {
      UIM_MSG_MED_1("Invalid as_id 0x%x", as_id);
      return GSTK_ERROR;
    }
    rex_enter_crit_sect(&gstk_shared_data.gstk_crit_sect);
    gstk_shared_data.is_cp_ack_pending[as_id] = FALSE;
    (*(boolean *)param_out_ptr) = gstk_shared_data.is_cp_ack_pending[as_id];
    rex_leave_crit_sect(&gstk_shared_data.gstk_crit_sect);
    break;
    
  case GSTK_IO_GET_WMS_IS_CP_ACK_PENDING_FLAG:
    as_id = (*(sys_modem_as_id_e_type *)(param_in_ptr));
  ...
  if(task_cmd != NULL)
  {
    ...
    switch (cc_ind_param->rat)
    {
      ...
      case GSTK_ACCESS_TECH_WLAN:
        ...
        break;
      case GSTK_ACCESS_NONE:
        /* In case of ME based Sups Services RAT can be passed on NONE */
        break;
      default:
    ...
    switch(cc_ind_param->call_enum_type)
    {
      ...
      case GSTK_IMS_CALL:
      {
        ...
        /* Copy IMS URI truncated flag only if the length is greater than or equal to
           GSTK_MAX_IMS_CALL_PARAM_SIZE */
        if (cc_ind_param->gstk_address_string.ims_call.length >= GSTK_MAX_IMS_CALL_PARAM_SIZE)
        {
          task_cmd->cmd.cc_envelope_ind.cc_data.gstk_address_string.ims_call.length =
            GSTK_MAX_IMS_CALL_PARAM_SIZE;
          if (cc_ind_param->gstk_address_string.ims_call.length == GSTK_MAX_IMS_CALL_PARAM_SIZE)
          {
            task_cmd->cmd.cc_envelope_ind.cc_data.gstk_address_string.ims_call.ims_uri_truncated =
              cc_ind_param->gstk_address_string.ims_call.ims_uri_truncated;
          }
          else
          {
            task_cmd->cmd.cc_envelope_ind.cc_data.gstk_address_string.ims_call.ims_uri_truncated = TRUE;
          }
        } 
      }
      break;
    ...
    task_cmd->cmd.cc_envelope_ind.cc_requeue        = FALSE;
    /* set GSTK_CMD_Q_SIG and put on GSTK queue */
    gstk_task_put_cmd_buf(task_cmd);
  }
  else
...
/*===========================================================================
FUNCTION GSTK_SLOT_SEND_ENVELOPE_GEO_LOC_REPORT_CMD

DESCRIPTION

  Clients can call this function to send a Geographical location report to a
  specific SIM/USIM slot

PARAMETERS
  slot:                   [Input] The SIM slot ID
  user_data:              [Input] User data that client wants GSTK to keep track of
  nmea_sentence:          [Input] Pointer indicating the nmea sentence
  gstk_geo_loc_report_cb: [Input] Function pointer to which GSTK will send
                             the card response data corresponding to the
...
SEE ALSO
  None
===========================================================================*/
gstk_status_enum_type gstk_slot_send_envelope_geo_loc_report_cmd (
  gstk_slot_id_enum_type          slot,
  uint32                          user_data,
  const gstk_nmea_sentence_type *nmea_sentence,
  gstk_evt_cb_funct_type          gstk_geo_loc_report_cb
)
{
  gstk_task_cmd_type      * task_cmd = NULL;

  UIM_MSG_HIGH_0("GSTK client geo_loc");
  ...
  /* validate call back function and Input Pointers */
  if (gstk_geo_loc_report_cb == NULL || nmea_sentence == NULL)
  {
    UIM_MSG_ERR_2("gstk_geo_loc_report_cb is 0x%x, nmea_sentence is 0x%x",
                  gstk_geo_loc_report_cb,
                  nmea_sentence);
    return GSTK_NULL_INPUT_PARAM;
  }
  ...
  if(task_cmd != NULL)
  { /* successfully obtained command buffer - build the envelope command */
    task_cmd->cmd.geo_loc_report.message_header.client_id     = gstk_instances_ptr[(uint32)slot-1]->client_id;
    task_cmd->cmd.geo_loc_report.message_header.command_group = GSTK_ENVELOPE_CMD;
    task_cmd->cmd.geo_loc_report.message_header.command_id    = (int)GSTK_GEO_LOC_REPORT_IND;
    task_cmd->cmd.geo_loc_report.message_header.user_data     = user_data;
    task_cmd->cmd.geo_loc_report.message_header.sim_slot_id   = slot;
    task_cmd->cmd.geo_loc_report.client_cb                    = gstk_geo_loc_report_cb;
    /* Copy nmea Data */
    task_cmd->cmd.geo_loc_report.nmea_sentence.length         = nmea_sentence->length;
    if(nmea_sentence->length > 0 && nmea_sentence->nmea_ptr != NULL)
    {
      task_cmd->cmd.geo_loc_report.nmea_sentence.nmea_ptr   = GSTK_CALLOC(nmea_sentence->length);
      if (task_cmd->cmd.geo_loc_report.nmea_sentence.nmea_ptr == NULL)
      {
        UIM_MSG_ERR_0("Not Able to Allocate nmea memory");
        gstk_task_free_cmd_buf(task_cmd);
        return GSTK_MEMORY_ERROR;
      }
   
      gstk_memcpy(task_cmd->cmd.geo_loc_report.nmea_sentence.nmea_ptr,
                   nmea_sentence->nmea_ptr,
                   nmea_sentence->length,
                   task_cmd->cmd.geo_loc_report.nmea_sentence.length,
                   nmea_sentence->length);
    }
    /* set GSTK_CMD_Q_SIG and put on GSTK queue */
    gstk_task_put_cmd_buf(task_cmd);
  ...
  {
    UIM_MSG_ERR_0("No more gstk buf");
    return GSTK_MEMORY_ERROR;
  }
