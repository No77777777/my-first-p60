...
Copyright (c) 2003-2017, 2019-2020  QUALCOMM Technologies, Inc (QTI) and its licensors.
...
$Header: //components/rel/uim.mpss/6.1.0/gstk/src/gstk_terminal_rsp.c#3 $
...
06/05/17   vr      Set GSTK client id for dummy TR
05/19/17   dt      Changing gstk_curr_inst_ptr to gstk_instances_ptr
05/04/17   vr      Dummy TR for proactive command with length 0
02/23/17   gs      TR F3 modification
01/20/17   shr     Add support for handling error Access Tech TR
...
04/23/16   gs      Common out the code in gstk_populate_terminal_response_cnf 
                   for IoE devices
03/22/16   gs      Send the failure TR to the Card instead of requeue
02/20/16   gs      Send the Error TR in case of proactive command not supported
...
06/05/15   gs      Update the SCWS flag based on transport Protocol 
05/06/15   gm      Fixed compiler warning
04/27/15   gm      Added support for class "n" command
04/22/15   vr      Send SW1 report for the clients for RAW TR
...
)
{
  ...
  UIM_MSG_HIGH_4("Terminal Reponse 0x%x, Result 0x%x, Additional Info Length 0x%x," 
  	         "Other_info is 0x%x", terminal_response_enum, general_result,
  	               additional_result->length, other_info->present);
  ...
  switch(terminal_response_enum) 
  {
    ...
    case GSTK_GEO_LOC_CNF:
		return gstk_populate_tr_additional_res(
			  &(command->cmd.gstk_geo_loc_cnf.result_additional_info), 
			    additional_result);
        break;
        ...
        if(gstk_status == GSTK_SUCCESS) {
          if (other_info->present) {
            ...
            if(other_info->extra_param.open_ch_extra_param.ch_status.is_scws_ch)
            {
              ...
              UIM_MSG_HIGH_2("other_info->extra_param.open_ch_extra_param.ch_status.is_scws_ch 0x%x" 
                             "command->cmd.open_ch_term_rsp_cnf.is_scws_ch 0x%x",
            ...
            if (other_info->extra_param.open_ch_extra_param.dns_addrs.num_addresses > 0)
            {
              ...
              if (command->cmd.open_ch_term_rsp_cnf.dns_addrs.num_addresses > 0)
              {
                ...
                for (i = 0; i < (uint32)command->cmd.open_ch_term_rsp_cnf.dns_addrs.num_addresses; i++)
                {
        ...
        if(gstk_status == GSTK_SUCCESS)
        {
          if (other_info->present)
          {
            ...
            (void)memscpy(command->cmd.get_ch_status_term_rsp_cnf.ch_status,
                    ...
                    sizeof(gstk_ch_status_type) *
                    command->cmd.get_ch_status_term_rsp_cnf.ch_cnt);
          }
        }
        else {
    ...
    case GSTK_CONTACTLESS_STATE_CHANGED_CNF:
        return gstk_populate_tr_additional_res(
			  &(command->cmd.contactless_state_changed_term_rsp_cnf.result_additional_info), 
			    additional_result);
        break;
    ...
    gstk_cmd_type* cmd )
{
    ...
    gstk_nv_items_data_type             nv_data;
    uint32                              ref_id      = GSTK_MAX_PROACTIVE_COMMAND;
    gstk_status_enum_type               gstk_status = GSTK_SUCCESS;
    ...
    switch( type_of_command ) 
    {
      ...
      case GSTK_SEND_SMS_CNF:
        ...
        break;
      case GSTK_GEO_LOC_CNF:
        gstk_status = gstk_util_compare_ref_id(cmd->gstk_geo_loc_cnf.cmd_details_ref_id);
        if(gstk_status != GSTK_SUCCESS)
        {
          break;
        }
        gstk_status = gstk_geo_loc_cnf(&(cmd->gstk_geo_loc_cnf));
        break;
      case GSTK_LAUNCH_BROWSER_CNF:
      ...
      case GSTK_ACTIVATE_CNF:
        ...
        break;
      case GSTK_CONTACTLESS_STATE_CHANGED_CNF:
        gstk_status = gstk_util_compare_ref_id(cmd->contactless_state_changed_term_rsp_cnf.cmd_details_ref_id);
        if(gstk_status != GSTK_SUCCESS)
        {
          break;
        }
        gstk_status = gstk_contactless_state_changed_cnf(&(cmd->contactless_state_changed_term_rsp_cnf));
        break;
      case GSTK_RUN_AT_CMDS_CNF:
      case GSTK_UNSUPPORTED_UNKNOWN_CMD_CNF:
      ...
      case GSTK_RAW_TERMIMAL_RESPONSE:
        ...
        if(gstk_status != GSTK_SUCCESS)
        {
          gstk_shared_data.gstk_client_table[cmd->client_raw_term_resp.client_id - 1].user_data_updated = TRUE;
...
void gstk_uim_terminal_response_report (uim_rpt_type *report)
{
    ...
    if (task_cmd != NULL)
    {
      ...
      gstk_status = gstk_util_convert_from_stk_cmd_type_to_gstk_cmd_type(
                                         gstk_instances_ptr[(uint8)(report->slot) - 1]->gstk_cmd_cache.cmd[5],
                                         gstk_instances_ptr[(uint8)(report->slot) - 1]->gstk_cmd_cache.cmd[6],
                                        (gstk_cmd_enum_type *)&task_cmd->cmd.general_cmd.message_header.command_id);
      if (gstk_status != GSTK_SUCCESS)
      {
        UIM_MSG_ERR_1("cached proactive cmd gave has invalid cmd_type 0x%x, defaulting to 0",
                      task_cmd->cmd.general_cmd.message_header.command_id);
      ...
      /* If proactive command is not pending, then check the other slots for pending proactive commands */
      if(!report->rpt.terminal_response.proactive_cmd_pending)
      {
  ...
  gstk_general_result_enum_type            command_result)
{
  ...
  if (gstk_status != GSTK_SUCCESS)
  {
    ...
    gstk_util_release_upper_layer_cmd_memory(
      (gstk_cmd_type*)((void*)unsupported_unknown_cnf));
    return GSTK_ERROR;
  }
  ...
  */
  ...
  /* Send Message to UIM */
  GSTK_UTIL_SELECTIVE_DUMP_BYTE_ARRAY("Unknown Unsupported command: TR",
                                      uim_cmd_ptr->terminal_response.data,
...
)
{
  ...
  /*
    in TR_REPORT handling, GSTK interprets user_data as client_id of the client
    to send SW1 SW2 status report to. Since TR timer expired, this means client
    did not respond, and hence we should not send SW1 SW2 to client. If user_data
    is set as 0, we attempt to send SW1 SW2 to whichever raw client is available.

    Instead we set the user_data to the client id of GSTK for this current slot.
    This will be fine since GSTK itself has no client_cb.
  */

  gstk_status = gstk_send_terminal_response(
                  gstk_curr_inst_ptr->client_id,
                  gstk_curr_inst_ptr->client_id,  /* use gstk_client_id as user_data */
                  ref_id,
...
/*===========================================================================
FUNCTION gstk_send_dummy_tr_to_uim

DESCRIPTION

  This function constructs and sends a dummy TR, when there is no valid
  proactive command form SIM CARD.

  This behavior of the SIM card is against specifications, but it was 
  observed on one commercial card, that was continuing sending 91XX 
  until a dummy TR was sent by the terminal

PARAMETERS
  data_ptr   : [Input] Pointer to the data buffer that contains the Proactive cmd.
  data_length: [Input] Length of the data
  slot       : [Input] Slot on which TR need to send.
...
SEE ALSO
  None
===========================================================================*/
gstk_status_enum_type gstk_send_dummy_tr_to_uim
(
...
{
  uim_cmd_type *uim_cmd_ptr = NULL;
  uint8         tr_data[]   = {0x1, 0x3, 0x0, 0x0, 0x0, 0x2, 0x2, 0x82, 0x81, 0x3, 0x1, 0x31};

  if (!GSTK_IS_VALID_SLOT_ID(slot) ||
      gstk_instances_ptr[slot - UIM_SLOT_1] == NULL ||
      data_ptr == NULL)
  {
    return GSTK_BAD_PARAM;
  }

  /* Check for command details TLV in the data_ptr
     proactive cmd TAG + proactive cmd len + cmd details TLV */
  if (data_length >= 2 + GSTK_COMMAND_DETAILS_TLV_SIZE &&
      data_ptr[0] == GSTK_PROACTIVE_SIM_COMMAND_TAG)
  {
    uint8   offset = (data_ptr[1] != GSTK_2_BYTE_LENGTH_FIRST_VALUE) ? 2 : 3;

    if (data_length >= (GSTK_COMMAND_DETAILS_TLV_SIZE + offset) &&
        sizeof(tr_data) >= GSTK_COMMAND_DETAILS_TLV_SIZE &&
        (data_ptr[offset]  & 0x7F) == GSTK_COMMAND_DETAILS_TAG && 
        data_ptr[offset + 1] == GSTK_COMMAND_DETAILS_LEN) 
    { 
      (void)gstk_memcpy(tr_data, data_ptr + offset,
                        GSTK_COMMAND_DETAILS_TLV_SIZE,
                        sizeof(tr_data),
                        data_length - offset); 
    } 
  }
  ...
  uim_cmd_ptr->hdr.command           = UIM_TERMINAL_RESPONSE_F;
  uim_cmd_ptr->hdr.cmd_hdr.task_ptr  = NULL;
  uim_cmd_ptr->hdr.cmd_hdr.sigs      = 0;
  uim_cmd_ptr->hdr.slot              = slot;
  ...
  uim_cmd_ptr->terminal_response.num_bytes  = sizeof(tr_data);
  ...
  gstk_instances_ptr[slot - UIM_SLOT_1]->tr_unique_ref_id = gstk_util_generate_ref_id(gstk_instances_ptr[slot - UIM_SLOT_1]->client_id);
  uim_cmd_ptr->hdr.user_data = gstk_instances_ptr[slot - UIM_SLOT_1]->tr_unique_ref_id;

  /* Set up log masking */
  gstk_logging_populate_masking_fn_ptr(uim_cmd_ptr);

  if (uim_cmd(uim_cmd_ptr) != UIM_SUCCESS)
  {
    gstk_send_cmd_to_uim_free_q(uim_cmd_ptr);
    return GSTK_ERROR;
  }
