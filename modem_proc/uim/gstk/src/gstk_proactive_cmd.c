...
$Header: //components/rel/uim.mpss/6.1.0/gstk/src/gstk_proactive_cmd.c#2 $
...
12/20/16   dt      Add support for rejecting Proactive cmds based on TP support
...
03/22/16   gs      Handling of Send DTMF command
03/22/16   gs      Handling of Unsupported Proactive commands
02/20/16   gs      Send TR when proactive command not supported
10/09/15   gm      Converting to unsigned variables.
06/25/15   gm      L4 compiler warning
06/05/15   gm      Split ME and slot specific NV
06/05/15   vr      Non UI and non NW proactive command parallel processing
...
02/25/15   gs      Reject the Proactive Command for Data Card case
...
/*===========================================================================
FUNCTION gstk_check_proactive_cmd_supported

DESCRIPTION
  This function parses the Command Details TLV and checks if the
  Proactive Command recevied was indicated to be supported in the
  Terminal Profile sent to the card.
  If the Proactive Command is not supported in Terminal Profile,
  the function sends an error Terminal Response.
...
RETURN VALUE
    boolean
    TRUE : Proactive command is supported in Terminal Profile
    FALSE: Proactive command is not supported in Terminal Profile
...
SEE ALSO
  None
===========================================================================*/
static boolean gstk_check_proactive_cmd_supported(
  gstk_proactive_cmd_type* cmd_ptr)
{
  gstk_status_enum_type     gstk_status          = GSTK_SUCCESS;
  uint32                    cmd_details_ref_id   = GSTK_MAX_PROACTIVE_COMMAND;
  uint8                     type_of_command      = 0;
  uint8                     command_qualifier    = 0;
  uint32                    profile_dl_bitmask_1 = 0;
  uint32                    profile_dl_bitmask_2 = 0;
  uint32                    profile_dl_bitmask_3 = 0;
  ...
  /* Process the Command Details TLV */
  ...
  {
    UIM_MSG_ERR_0("gstk_check_proactive_cmd_supported(): Failed to pre-process proactive command");
    /* If return value is GSTK_BAD_PARAM, then there is
       entry in the command table, so return without TR */
  ...
  /* Type of Command */
  type_of_command = 
    command_details_ref_table_ptr[GSTK_CMD_REF_TABLE_INDEX(cmd_details_ref_id)].command_details.type_of_command;

  /* Command Qualifier */
  command_qualifier =
    command_details_ref_table_ptr[GSTK_CMD_REF_TABLE_INDEX(cmd_details_ref_id)].command_details.qualifier;

  /* If command is End of Proacitve Session, no need of TP checks.
     For Open Channel, an error TR needs to have bearer information, hence
     return from here to allow parsing of OPEN CHANNEL command in
     gstk_open_ch_req() where the TP check is added */
  if((type_of_command == GSTK_CMD_STK_END_OF_PROACTIVE_SES) ||
     (type_of_command == GSTK_CMD_STK_OPEN_CHANNEL))
  {
    /* Free Command Table entry */
  ...
  switch(type_of_command)
  {
    case GSTK_CMD_STK_REFRESH:
      profile_dl_bitmask_1 = GSTK_REFRESH_SUPPORT;
      break;
    case GSTK_CMD_STK_MORE_TIME:
      profile_dl_bitmask_1 = GSTK_MORE_TIME_SUPPORT;
      break;
    case GSTK_CMD_STK_POLL_INTERVAL:
      profile_dl_bitmask_1 = GSTK_POLL_INTERVAL_SUPPORT;
      break;
    case GSTK_CMD_STK_POLLING_OFF:
      profile_dl_bitmask_1 = GSTK_POLLING_OFF_SUPPORT;
      break;
    case GSTK_CMD_STK_SET_UP_EVENT_LIST:
      profile_dl_bitmask_1 = GSTK_SET_UP_EVT_LIST_SUPPORT;
      break;
    case GSTK_CMD_STK_SET_UP_CALL:
      profile_dl_bitmask_1 = GSTK_SET_UP_CALL_SUPPORT;
      break;
    case GSTK_CMD_STK_SEND_SS:
      profile_dl_bitmask_1 = GSTK_SEND_SS_SUPPORT;
      break;
    case GSTK_CMD_STK_USSD:
      profile_dl_bitmask_1 = GSTK_SEND_USSD_SUPPORT;
      break;
    case GSTK_CMD_STK_SEND_SHORT_MESSAGE:
      profile_dl_bitmask_1 = GSTK_SEND_SHORT_MSG_SUPPORT;
      profile_dl_bitmask_2 = GSTK_CDMA_SEND_SMS_SUPPORT;
      break;
    case GSTK_CMD_STK_LAUNCH_BROWSER:
      profile_dl_bitmask_1 = GSTK_LAUNCH_BROWSER_SUPPORT;
      break;
    case GSTK_CMD_STK_PLAY_TONE:
      profile_dl_bitmask_1 = GSTK_PLAY_TONE_SUPPORT;
      break;
    case GSTK_CMD_STK_DISPLAY_TEXT:
      profile_dl_bitmask_1 = GSTK_DISPLAY_TEXT_SUPPORT;
      break;
    case GSTK_CMD_STK_GET_INKEY:
      profile_dl_bitmask_1 = GSTK_GET_INKEY_SUPPORT;
      break;
    case GSTK_CMD_STK_GET_INPUT:
      profile_dl_bitmask_1 = GSTK_GET_INPUT_SUPPORT;
      break;
    case GSTK_CMD_STK_SELECT_ITEM:
      profile_dl_bitmask_1 = GSTK_SELECT_ITEM_SUPPORT;
      break;
    case GSTK_CMD_STK_SET_UP_MENU:
      profile_dl_bitmask_1 = GSTK_SET_UP_MENU_SUPPORT;
      break;
    case GSTK_CMD_STK_PROVIDE_LOCAL_INFO:
      switch(command_qualifier)
      {
        case GSTK_LOCATION_INFO:
        case GSTK_IMEI:          
          profile_dl_bitmask_1 = GSTK_PROVIDE_LOCAL_INFO_LOCATION_INFO_SUPPORT;        
          break;
        case GSTK_NMR:
          profile_dl_bitmask_1 = GSTK_PROVIDE_LOCAL_INFO_NMR_SUPPORT;        
          profile_dl_bitmask_2 = GSTK_PLI_NMR_BCCH;        
          profile_dl_bitmask_3 = GSTK_PROVIDE_LOCAL_INFO_UTRAN_EUTRAN;  
          break;
        case GSTK_DATE_TIME_ZONE:
          profile_dl_bitmask_1 = GSTK_PROVIDE_LOCAL_INFO_TIME_SUPPORT;        
          break;
        case GSTK_LANGUAGE_SETTING:
          profile_dl_bitmask_1 = GSTK_PROVIDE_LOCAL_INFO_LANG_SUPPORT;        
          break;
        case GSTK_TIMING_ADVANCE:
          profile_dl_bitmask_1 = GSTK_PROVIDE_LOCAL_INFO_TIME_ADV_SUPPORT;       
          break;
        case GSTK_ACCESS_TECHNOLOGY:
          profile_dl_bitmask_1 = GSTK_PROVIDE_LOCAL_INFO_ACCESS_TECH_SUPPORT;        
          break;
        case GSTK_ESN:
          profile_dl_bitmask_1 = GSTK_PROVIDE_LOCAL_INFO_ESN_SUPPORT;        
          break;
        case GSTK_IMEISV:
          profile_dl_bitmask_1 = GSTK_PROVIDE_LOCAL_INFO_IMEISV_SUPPORT;        
          break;
        case GSTK_NETWORK_SEARCH_MODE:
          profile_dl_bitmask_1 = GSTK_PROVIDE_LOCAL_INFO_NETWORK_SEARCH_MODE_SUPPORT;        
          break;
        case GSTK_BATTERY_STATUS:
          profile_dl_bitmask_1 = GSTK_PROVIDE_LOCAL_INFO_BATTERY_STATE_SUPPORT;        
          break;
        case GSTK_MEID:
          profile_dl_bitmask_1 = GSTK_PROVIDE_LOCAL_INFO_MEID_SUPPORT;        
          break;      
        case GSTK_CSG_CELL:
          profile_dl_bitmask_1 = GSTK_PROVIDE_LOCAL_INFO_CSG_SUPPORT;        
          break;           
        default:
          break;
      }        
    ...
    case GSTK_CMD_STK_SET_UP_IDLE_MODE_TEXT:
      profile_dl_bitmask_1 = GSTK_SET_UP_IDLE_MODE_TEXT_SUPPORT;
      break;
    case GSTK_CMD_STK_SEND_DTMF:
      profile_dl_bitmask_1 = GSTK_SEND_DTMF_SUPPORT;
      break;
    case GSTK_CMD_STK_TIMER_MANAGEMENT:
      /* Check command qualifier bits 1 and 2 */
      switch(command_qualifier & 0x03)
      {
        /* Start and Stop */
        case 0x00:
        case 0x01:          
          profile_dl_bitmask_1 = GSTK_TIMER_START_STOP_SUPPORT;        
          break;
        /* Get current value */
        case 0x02:
          profile_dl_bitmask_1 = GSTK_TIMER_GET_CURR_VALUE_SUPPORT;        
          break;
        default:
          break;
      }    
    ...
    case GSTK_CMD_STK_LANG_NOTIFICATION:
      profile_dl_bitmask_1 = GSTK_LANGUAGE_NOTIFICATION_SUPPORT;
      break;
    case GSTK_CMD_STK_OPEN_CHANNEL:
      profile_dl_bitmask_1 = GSTK_OPEN_CH_SUPPORT;
      break;
    case GSTK_CMD_STK_CLOSE_CHANNEL:
      profile_dl_bitmask_1 = GSTK_CLOSE_CH_SUPPORT;
      break;
    case GSTK_CMD_STK_RECEIVE_DATA:
      profile_dl_bitmask_1 = GSTK_RECEIVE_DATA_SUPPORT;
      break;
    case GSTK_CMD_STK_SEND_DATA:
      profile_dl_bitmask_1 = GSTK_SEND_DATA_SUPPORT;
      break;
    case GSTK_CMD_STK_GET_CHANNEL_STATUS:
      profile_dl_bitmask_1 = GSTK_GET_CH_STATUS_SUPPORT;
      break;
    case GSTK_CMD_STK_ACTIVATE:
      profile_dl_bitmask_1 = GSTK_ACTIVATE_SUPPORT;
      break;
    case GSTK_CMD_STK_CONTACTLESS_STATE_CHANGED:
      profile_dl_bitmask_1 = GSTK_CONTACTLESS_STATE_CHANGED_SUPPORT;
      break;
    case GSTK_CMD_STK_GEO_LOC_REQ:
      profile_dl_bitmask_1 = GSTK_GEO_LOC_REQUEST_SUPPORT;
      break;
  ...
  /* If the Proactive Command is supported in Terminal Profile */
  if(gstk_is_bit_enabled_in_term_profile(profile_dl_bitmask_1) ||
     gstk_is_bit_enabled_in_term_profile(profile_dl_bitmask_2) ||
     gstk_is_bit_enabled_in_term_profile(profile_dl_bitmask_3))
  {
    /* Free Command Table entry */
    gstk_util_cmd_details_ref_table_free_slot(cmd_details_ref_id);
    return TRUE;
  ...
  gstk_build_unknown_unsupported_term_rsp(
    ...
    &additional_info);
  return FALSE;
} /* gstk_check_proactive_cmd_supported */
         ...
         uint32*                   cmd_ref_id_ptr)
{
 ...
 /* Command Detail Tag   */
  ...
  if ((GSTK_TLV_PARSING_CONTINUE != tag_parsing_result)
      ||((count + GSTK_COMMAND_DETAILS_LEN) > (length_of_all_tlvs + (int32)length_of_TLV_length_field))
  ...
  */
  ...
  if (((count + GSTK_DEVICE_IDENTITY_LEN) <
         (length_of_all_tlvs + (int32)length_of_TLV_length_field))&&
      gstk_valid_tag(cmd_ptr->data[count],GSTK_DEVICE_IDENTITY_TAG))
  {
    ...
    {
      count++;
      if (cmd_ptr->data[count] == (int)GSTK_UICC_SIM_DEVICE) 
      { /* from card */
  ...
  gstk_proactive_cmd_type *cmd_ptr )
{
  ...
  */
  ...
  /* Check if the Proactive Command is supported in Terminal Profile */
  if(!gstk_check_proactive_cmd_supported(cmd_ptr))
  {
    return GSTK_UNSUPPORTED_COMMAND;
  }
  ...
  */
  ...
  case GSTK_CMD_STK_REFRESH:
          ...
          gstk_curr_inst_ptr->slot_id) == TRUE)
    {
      ...
      /* Find attach control request pending in queue and copy the data to use
         it for sending response to NAS */
  ...
  case GSTK_CMD_STK_OPEN_CHANNEL:
    ...
    if (gstk_nv_get_feature_status(GSTK_CFG_FEATURE_SLOT_IMSI_M_ENH, gstk_curr_inst_ptr->slot_id) == TRUE)
    {
      ...
      /* Find the index in mmgsdi_sess_info[] corresponding to 1X Primary 
         or Secondary Session */
      for (i = 0; i < GSTK_MMGSDI_SESS_INFO_SIZE; ++i)
      {
        /* Retrieve 1X Session index corresponding to the current slot */
        if((gstk_shared_data.mmgsdi_sess_info[i].sess_type == MMGSDI_1X_PROV_PRI_SESSION &&
            gstk_shared_data.mmgsdi_sess_info[i].slot == gstk_curr_inst_ptr->slot_id) || 
      ...
      if (
          ...
          gstk_curr_inst_ptr->gstk_is_imsi_m_provisioned != TRUE &&
      ...
      else /* Either IMSI_M is already provisioned or ME not on CDMA-1x or normal service, proceed with OPEN CH */
      {
         UIM_MSG_HIGH_3("gstk_is_imsi_m_provisioned = 0x%x, rat = 0x%x, location_status = 0x%x",
                        gstk_curr_inst_ptr->gstk_is_imsi_m_provisioned,
...
#endif /* FEATURE_CDMA */    
    {
      UIM_MSG_HIGH_0("FEATURE_CDMA is not enabled or IMSI_M not enabled");
      gstk_status = gstk_open_ch_req(cmd_ptr);
    }
    break;
  ...
  case GSTK_CMD_STK_CONTACTLESS_STATE_CHANGED:
    /* Read the CAT version */
    if(gstk_nv_access_read(GSTK_NV_SLOT_CAT_VERSION,
                           gstk_curr_inst_ptr->slot_id,
                           &nv_data) == GSTK_SUCCESS)
    {
      if (nv_data.cat_version >= GSTK_CFG_CAT_VER9)
      {
        gstk_status = gstk_contactless_state_changed_req(cmd_ptr);
      }
      else
      {
        gstk_status =
  ...
  case GSTK_CMD_STK_GEO_LOC_REQ:
#ifndef FEATURE_GSTK_QMI_LOC
    UIM_MSG_ERR_0("QMI_LOC is not supported");
    gstk_status =
      gstk_unknown_unsupported_proactive_cmd_req(cmd_ptr,
                                                 GSTK_COMMAND_BEYOND_THE_ME_CAPABILITIES);
#else                                                  
                           ...
                           &nv_data) == GSTK_SUCCESS)
    {
      ...
      {
        gstk_status = gstk_geo_loc_req(cmd_ptr);
      }
      else
      {
        gstk_status =
  ...
  /* To make sure that TR is sent to the card in case of devices where 
     stubs get compiled when proactive commands are disabled */
    ...
    gstk_tag_enum_type       gstk_tag)
{
    ...
    {
      if(gstk_util_retry_envelope_cmd(cmd->message_header.sim_slot_id) != GSTK_SUCCESS)
      {
    ...
    const gstk_additional_info_type    *additional_result)
{
  ...
  memset(&dummy_extra_param, 0x00, sizeof(dummy_extra_param));

  /* Extra param is needed for OPEN CHANNEL */
  if(cnf_type == GSTK_OPEN_CH_CNF)
  {
    dummy_extra_param.present = TRUE;
    dummy_extra_param.rsp_type = cnf_type;
  }
  else
  {
