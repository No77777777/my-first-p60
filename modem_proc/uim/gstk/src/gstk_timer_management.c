...
$Header: //components/rel/uim.mpss/6.1.0/gstk/src/gstk_timer_management.c#2 $
...
05/19/15   bd      Fixed multi slot issue
  ...
  timer_management_terminal_response_type  * STK_response)
{
  ...
  gstk_util_release_upper_layer_cmd_memory(
    (gstk_cmd_type*)timer_manage_cnf);
} /* gstk_timer_management_cnf_cleanup */
  ...
  */
  ...
  if (uim_cmd_ptr != NULL ) {
    ...
    if ((gstk_status == GSTK_SUCCESS) && (uim_cmd_ptr != NULL)) {
      ...
      GSTK_UTIL_SELECTIVE_DUMP_BYTE_ARRAY("Timer Management: TR",
