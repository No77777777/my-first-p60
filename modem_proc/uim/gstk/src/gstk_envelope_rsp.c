...
$Header: //components/rel/uim.mpss/6.1.0/gstk/src/gstk_envelope_rsp.c#2 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
11/02/17   dt      Do not send channel status indication for closed channel
10/31/17   dt      End proactive session when MENU SELECTION command response is not 91XX
10/04/17   dt      Do not set cp_ack pending flag for SMS PP on 1x
03/27/17   shr     Fix compilation errors due to FEATURE_LTE disablement
10/05/16   shr     Send immediate Attach Control Response on SIM BUSY
09/27/16   vr      Check for proactive cmd if the envelope rsp is busy
...
05/11/16   gm      Combine envelope retry processing in envelope response
...
12/03/15   shr     Fix PDP context activation TLV parsing logic
11/30/15   vr      GSTK CC retry to be enabled only after the busy from card
11/18/15   gm      Updating call type if address field is not present
...
void gstk_uim_envelope_report (uim_rpt_type *report_ptr)
{
  ...
  UIM_MSG_HIGH_5("UIM env rsp, 0x%x, 0x%x, rpt Status 0x%x userdata 0x%x, unique_ref_id 0x%x",
                 report_ptr->sw1,
                 report_ptr->sw2,
  ...
  if(report_ptr->rpt_status == UIM_PASS)
  {
    ...
    { /* has data */
      UIM_MSG_LOW_1("Copy 0x%x bytes of envelope rsp.",
  ...
  {
    task_cmd_ptr->cmd.envelope_response.sw1 = (uint8)report_ptr->rpt.envelope.get_resp_sw1;
    task_cmd_ptr->cmd.envelope_response.sw2 = (uint8)report_ptr->rpt.envelope.get_resp_sw2;
  }
  else
  {
    task_cmd_ptr->cmd.envelope_response.sw1 = (uint8)report_ptr->sw1;
    task_cmd_ptr->cmd.envelope_response.sw2 = (uint8)report_ptr->sw2;
  }
  ...
  if (type_of_command == GSTK_SMS_PP_DOWNLOAD_IND_RSP)
  {
        ...
        SW1_PROBLEM   == report_ptr->rpt.envelope.get_resp_sw1)
    {
      task_cmd_ptr->cmd.envelope_response.sw1 =
        (uint8)report_ptr->rpt.envelope.get_resp_sw1;
      task_cmd_ptr->cmd.envelope_response.sw2 =
        (uint8)report_ptr->rpt.envelope.get_resp_sw2;
    }
  }
  ...
  case SW1_BUSY:
    ...
    /* Check if any proactive command is pending for fetch */
    uim_check_pending_proactive_command(report_ptr->slot);
    break;
  default:
    break;
  }
  ...
  gstk_cmd_enum_type type_of_command)
{
     ...
     nv_data.env_retry_ext.max_retries)
  {
    ...
    return GSTK_SUCCESS;
          ...
          (type_of_command == GSTK_CC_IND_RSP && gstk_shared_data.ui_nw_proactive_session_slot_in_progress != 0xFF)))
  {
  ...
  const gstk_cmd_type * cmd)
{
     ...
     (gstk_curr_inst_ptr->gstk_curr_envelope_info.gstk_env_state == GSTK_SUB_ST_PENDING_ENV_RSP_FROM_CARD))
  {
    ...
    {
      /* Immediately send a response to NAS indicating timer expiry
         when card indicates SIM BUSY - this ensures NAS is not
         delayed unecessarily whle waiting got Attach Control timer
         to expire */
      if((type_of_command == GSTK_LOC_INFO_ENVELOPE_RSP) &&
  ...
  const gstk_cmd_type * cmd)
{
  ...
  if(cmd->envelope_response.rsp_status == GSTK_ENVELOPE_CMD_SUCCESS)
  {
    gstk_curr_inst_ptr->pco_info.is_pco_sent = TRUE;
  }
...
#endif        
        break;
      case GSTK_CONTACTLESS_STATE_REQ_EVT_IND_RSP:
        /* build the command */
        response_ptr->hdr_cmd.command_id = type_of_command;
        response_ptr->hdr_cmd.cmd_detail_reference = 0; /* doesn't matter */
        response_ptr->cmd.envelope_general_rsp = cmd->envelope_response.rsp_status;
  ...
  const gstk_cmd_type * cmd)
{
   ...
   gstk_io_ctrl(GSTK_IO_SLOT_ID_TO_MODEM_AS_ID,
         &gstk_curr_inst_ptr->slot_id,
                &as_id);
   
   if(response_ptr->cmd.sms_pp_download_rsp.general_resp == GSTK_ENVELOPE_CMD_SUCCESS)
   {
        ...
        gstk_curr_inst_ptr->slot_id) == FALSE)
     {
       ...
       /* for SMS PP on CDMA, no need to wait for ACK REPORT from WMS. 
          Hence is_cp_ack_pending flag is not set */
       else if(!gstk_shared_data.envelope_cmd_ref_table[env_ref_id].is_cdma_sms_pp)
       {
       gstk_shared_data.is_cp_ack_pending[as_id] = TRUE;
       /* flag will be reset in QMICAT if SMS PP is from AP */
     }
   }
   }
  ...
  gstk_cmd_type * cmd)
{
  ...
  /* Need condition to compare env_ref_id here or code in "else" part will be executed */
  if(env_ref_id == gstk_curr_inst_ptr->gstk_curr_envelope_info.env_ref_id)
  {
    if(gstk_envelope_rsp_process_retry(cmd) == GSTK_SUCCESS)
    {
  ...
  else if(env_ref_id != gstk_curr_inst_ptr->gstk_curr_envelope_info.env_ref_id &&
          env_ref_id < GSTK_MAX_ENVELOPE_COMMAND)
  {
    ...
    if(nv_data.env_retry_ext.max_retries != 0)
    {
      /* Requeue the command if response is busy */
      if(response.cmd.cc_rsp.general_resp == GSTK_ENVELOPE_CMD_CARD_BUSY &&
         gstk_shared_data.envelope_cmd_ref_table[env_ref_id].gstk_cc_cache_ptr != NULL)
      {
        ...
        if(task_cmd != NULL)
        {
          task_cmd->cmd.cc_envelope_ind.message_header.client_id     =
          ...
          task_cmd->cmd.cc_envelope_ind.message_header.sim_slot_id   =
                                                             gstk_curr_inst_ptr->slot_id;
          task_cmd->cmd.cc_envelope_ind.message_header.user_data     = 
                           gstk_shared_data.envelope_cmd_ref_table[env_ref_id].user_data;
          task_cmd->cmd.cc_envelope_ind.client_cb                    =
                gstk_shared_data.envelope_cmd_ref_table[env_ref_id].env_cb.client_env_cb;
          task_cmd->cmd.cc_envelope_ind.slot                         = 
                                                             gstk_curr_inst_ptr->slot_id;
          task_cmd->cmd.cc_envelope_ind.slot_info_present            = TRUE;
          task_cmd->cmd.cc_envelope_ind.cc_requeue                   = TRUE;
          gstk_memcpy(&(task_cmd->cmd.cc_envelope_ind.cc_data),
                     gstk_shared_data.envelope_cmd_ref_table[env_ref_id].gstk_cc_cache_ptr,
                     sizeof(task_cmd->cmd.cc_envelope_ind.cc_data),
                     sizeof(task_cmd->cmd.cc_envelope_ind.cc_data),
          ...
          /* command requeued, clean up envelope table */
          gstk_util_env_cmd_ref_table_free_slot(env_ref_id);
          return GSTK_SUCCESS;
        }
      }
  ...
  /* If address data object is not present, then the terminal shall assume the Dialling
     number is not to be modified. So passing call_type same as what we received from client*/
  if(response.cmd.cc_rsp.cc_req_action.call_type == GSTK_NONE)
  {
    if(gstk_shared_data.envelope_cmd_ref_table[env_ref_id].gstk_cc_cache_ptr != NULL)
    {
      response.cmd.cc_rsp.cc_req_action.call_type =
        gstk_shared_data.envelope_cmd_ref_table[env_ref_id].gstk_cc_cache_ptr->call_enum_type;
    }
  }
  ...
  gstk_cmd_type * cmd)
{
          ...
          env_ref_id < GSTK_MAX_ENVELOPE_COMMAND)
  {
    ...
    if(nv_data.env_retry_ext.max_retries != 0)
    {
      /* Requeue the command if response is busy */
      if(response.cmd.cc_rsp.general_resp == GSTK_ENVELOPE_CMD_CARD_BUSY &&
         gstk_shared_data.envelope_cmd_ref_table[env_ref_id].mo_sms_cache_ptr != NULL)
      {
        gstk_task_cmd_type*     task_cmd = NULL;
        ...
        if(task_cmd != NULL)
        {
          task_cmd->cmd.mo_sms_ctrl_envelope_ind.message_header.command_group = 
                                                                       GSTK_ENVELOPE_CMD;
          task_cmd->cmd.mo_sms_ctrl_envelope_ind.message_header.command_id    = 
                                                               (int)GSTK_MO_SMS_CTRL_IND;
          task_cmd->cmd.mo_sms_ctrl_envelope_ind.message_header.user_data     =
                            gstk_shared_data.envelope_cmd_ref_table[env_ref_id].user_data;
          task_cmd->cmd.mo_sms_ctrl_envelope_ind.message_header.sim_slot_id   = 
                                                              gstk_curr_inst_ptr->slot_id;
          task_cmd->cmd.mo_sms_ctrl_envelope_ind.message_header.client_id     =
                                                            gstk_curr_inst_ptr->client_id;
          task_cmd->cmd.mo_sms_ctrl_envelope_ind.client_cb                    =
                 gstk_shared_data.envelope_cmd_ref_table[env_ref_id].env_cb.client_env_cb;
          ...
          task_cmd->cmd.mo_sms_ctrl_envelope_ind.mo_sms_requeue = TRUE;
...
/*===========================================================================
FUNCTION gstk_geo_loc_report_envelope_rsp

DESCRIPTION

  This function builds the geographical location report rsp and sends it to client
...
SEE ALSO
  None
===========================================================================*/
static gstk_status_enum_type gstk_geo_loc_report_envelope_rsp(
  const gstk_cmd_type * cmd)
{
   gstk_status_enum_type                       gstk_status = GSTK_SUCCESS;
   gstk_cmd_from_card_type                     response;
   ...
   /* build the command */
   response.hdr_cmd.command_id = GSTK_GEO_LOC_REPORT_RSP;
   response.hdr_cmd.cmd_detail_reference = 0; /* doens't matter */
   ...
   return gstk_status;
} /* gstk_geo_loc_report_envelope_rsp */
    ...
    gstk_cmd_type* cmd_ptr )
{
    ...
    /* Get the type of response */
    type_of_command = gstk_shared_data.envelope_cmd_ref_table[env_ref_id].expected_cmd_rsp_type;
	UIM_MSG_HIGH_1("Processing Envelope rsp Cmd is 0x%x", type_of_command);
    ...
    switch( type_of_command )
    {
      ...
      case GSTK_LOC_INFO_ENVELOPE_RSP:
        ...
        break;
      case GSTK_GEO_LOC_REPORT_RSP:
        gstk_status = gstk_geo_loc_report_envelope_rsp(cmd_ptr);
        break;
      case GSTK_PCO_INFO_ENVELOPE_RSP:
  ...
  const gstk_cmd_type * cmd)
{
    ...
    UIM_MSG_HIGH_1("Non Idle State Event DL Command 0x%x", type_of_command);
    switch( type_of_command )
    {
      case GSTK_MT_CALL_EVT_IND_RSP:
      case GSTK_CALL_CONNECTED_EVT_IND_RSP:
