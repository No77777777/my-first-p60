...
$Header: //components/rel/uim.mpss/6.1.0/gstk/src/gstk_open_ch.c#2 $
...
12/07/17   gs      Support for OTASP Call on Slot#2
...
03/02/16   gs      Send right error cause when the protocol type can't be 
                   handled
...
10/08/14   gs      Check for the Open Channel Capability before checking for 
                   Sub Ready
08/27/14   vr      Move gstk_shared_data and gstk_instance_data to global
...
)
{
  ...
  UIM_MSG_HIGH_0("IN GSTK_OPEN_CH_REQ");
  ...
  */
  ...
  if (more_tlv) 
  {
    ...
    if(gstk_status == GSTK_SUCCESS)
    {
      /* Check if Local Address TLV is present */
      if(request.cmd.open_ch_pro_cmd_req.local_address.is_tlv_present)
      {
        /* If comprehension bit is set, send Error TR */  
        if(gstk_is_tag_comprehension_req(cmd_data_ptr[curr_offset]))
        {
        ...
        /* Do not reject the open channel request when comprehension bit is not set */
        else
        {
          command_details_ref_table_ptr[GSTK_CMD_REF_TABLE_INDEX(cmd_details_ref_id)].partial_comprehension = TRUE;
        }
      }
      curr_offset = new_offset; 
    }
  ...
  if (more_tlv)
  {
    ...
    if(gstk_status != GSTK_SUCCESS)
    {
      ...
      gen_result = GSTK_COMMAND_DATA_NOT_UNDERSTOOD_BY_ME;
  ...
  if(!is_scws_open_ch)
  {
    ...
    /* If TP does not support OPEN CHANNEL, send error TR */
    if(!gstk_is_bit_enabled_in_term_profile(GSTK_OPEN_CH_SUPPORT))
    {
      error_tr = TRUE;
    }
    ...
    if((gstk_shared_data.wlan_info_ptr == NULL ||
        (gstk_shared_data.wlan_info_ptr != NULL &&
    ...
    {
      UIM_MSG_HIGH_3("Drop open ch, Subs Ready GW: 0x%x, Subs Ready 1X: 0x%x, Loc status=0x%x",
                     gstk_curr_inst_ptr->is_subs_ready.subs_ready_gw,
                     gstk_curr_inst_ptr->is_subs_ready.subs_ready_1x,
...
)
{
  ...
  if (open_ch_cnf_ptr->other_data_present) {
    ...
    if (open_ch_cnf_ptr->dns_addrs.num_addresses > 0)
    {
      ...
      STK_response.num_dns_addrs = (uint8)open_ch_cnf_ptr->dns_addrs.num_addresses;
      for (i = 0; i < (uint32)open_ch_cnf_ptr->dns_addrs.num_addresses; ++ i)
      {
  ...
  if (uim_cmd_ptr != NULL )
  {
    ...
    if (gstk_status == GSTK_SUCCESS)
    {
      if (uim_cmd_ptr != NULL) {
        ...
        if (gstk_status == GSTK_SUCCESS)
        {
          GSTK_UTIL_SELECTIVE_DUMP_BYTE_ARRAY("Open Channel TR",
