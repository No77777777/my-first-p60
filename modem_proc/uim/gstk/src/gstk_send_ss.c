...
$Header: //components/rel/uim.mpss/6.1.0/gstk/src/gstk_send_ss.c#2 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
12/09/16   dt      Reverted KW fixes
12/06/16   dt      KW fixes array out of bound in estk_gstk_init
...
07/11/16   vr      Fix memory leak when send SS modified to USSD
05/27/16   shr     F3 reduction: Malloc Failure, Memcpy, Null Ptr checks
04/23/16   gs      Memory reduction for IoE
11/12/15   vr      F3 Logging inprovements
10/14/15   shr     Fix source buffer length when copying Alpha text in
                   Setup Call Confirmation handling
  ...
  if (uim_cmd_ptr != NULL ) {
    ...
    if (gstk_status == GSTK_SUCCESS) {
      if (uim_cmd_ptr != NULL) {
        ...
        if (gstk_status == GSTK_SUCCESS)
        {
          GSTK_UTIL_SELECTIVE_DUMP_BYTE_ARRAY("SS TR",
                                              uim_cmd_ptr->terminal_response.data,
...
DESCRIPTION
  This function check whether the SS capabilities are enabled
