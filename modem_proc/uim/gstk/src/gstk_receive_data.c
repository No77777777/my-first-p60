...
Copyright (c) 2003-2010, 2012-2014, 2016, 2019 QUALCOMM Technologies, Inc (QTI) and 
...
$Header: //components/rel/uim.mpss/6.1.0/gstk/src/gstk_receive_data.c#2 $
  ...
  receive_data_terminal_response_type *STK_response_ptr)
{
  ...
  gstk_util_release_upper_layer_cmd_memory(
    (gstk_cmd_type*)receive_data_cnf_ptr);
...
)
{
  ...
  UIM_MSG_HIGH_0(" IN GSTK_RECEIVE_DATA_REQ ");
...
)
{
  ...
  UIM_MSG_HIGH_0("IN GSTK receive data cnf");
  ...
  if (uim_cmd_ptr != NULL )
  {
     ...
     if (gstk_status == GSTK_SUCCESS)
     {
       if (uim_cmd_ptr != NULL) {
          ...
          if (gstk_status == GSTK_SUCCESS)
          {
            GSTK_UTIL_SELECTIVE_DUMP_BYTE_ARRAY("Receive Data TR",
