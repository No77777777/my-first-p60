...
Copyright (c) 2004-2010, 2014, 2015 - 2016 QUALCOMM Technologies, Inc (QTI)
...
$Header: //components/rel/uim.mpss/6.1.0/gstk/src/gstk_lang_notification.c#2 $
...
{
  if((STK_response_ptr == NULL) || (lang_notification_cnf_ptr == NULL))
  {
    return;
  }
  ...
  gstk_util_release_upper_layer_cmd_memory(
    (gstk_cmd_type*)lang_notification_cnf_ptr);
} /* gstk_lang_notification_cnf_cleanup */
  ...
  if (uim_cmd_ptr != NULL ) {
    ...
    if (gstk_status == GSTK_SUCCESS) {
      if (uim_cmd_ptr != NULL) {
        ...
        if (gstk_status == GSTK_SUCCESS)
        {
          GSTK_UTIL_SELECTIVE_DUMP_BYTE_ARRAY("LangNotification TR",
