...
Copyright (c) 2003-2010, 2015 - 2016 QUALCOMM Technologies, Inc (QTI)
...
$Header: //components/rel/uim.mpss/6.1.0/gstk/src/gstk_launch_browser.c#2 $
  ...
  launch_browser_terminal_response_type  *STK_response_ptr)
{
  ...
  if((STK_response_ptr == NULL) || (launch_browser_cnf_ptr == NULL))
  {
    return;
  }
  ...
  gstk_util_release_upper_layer_cmd_memory(
    (gstk_cmd_type*)launch_browser_cnf_ptr);
  ...
  if (uim_cmd_ptr != NULL ) {
    ...
    if (gstk_status == GSTK_SUCCESS) {
      if (uim_cmd_ptr != NULL) {
        ...
        if (gstk_status == GSTK_SUCCESS)
        {
          GSTK_UTIL_SELECTIVE_DUMP_BYTE_ARRAY("Launch browser TR",
