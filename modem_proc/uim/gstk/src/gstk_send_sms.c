...
$Header: //components/rel/uim.mpss/6.1.0/gstk/src/gstk_send_sms.c#2 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
06/28/17   dt      Hold SEND SMS till CP ACK for SMS PP is received
02/13/17   gs      Don't check for No service before sending SMS
  ...
  send_sms_terminal_response_type * STK_response_ptr)
{
  ...
  gstk_util_release_upper_layer_cmd_memory(
    (gstk_cmd_type*)send_sms_cnf_ptr);
  ...
  gstk_proactive_cmd_type* command_ptr)
{
  ...
  */
  ...
  if(!error_already_raised)
  {
    /* Don't check for No Service as ESTK checks for same, 
       if it is trying on CS */
    ...
    {
      /* if feature is disabled, Start Timer for 2 min 45 sec as 
            max time for CP_ACK of SMS_PP for which we are holding SEND SMS is 40sec */
          ...
          gstk_curr_inst_ptr->slot_id) == FALSE))
      {
        gstk_util_start_client_response_timer(cmd_details_ref_id,
                                              GSTK_SEND_SMS_TERM_RSP_WAIT_TIME);
      }
      else
      {
        /* Start Timer */
  ...
  if (uim_cmd_ptr != NULL ) {
    ...
    if (gstk_status == GSTK_SUCCESS) {
      if (uim_cmd_ptr != NULL) {
        ...
        if (gstk_status == GSTK_SUCCESS)
        {
          GSTK_UTIL_SELECTIVE_DUMP_BYTE_ARRAY("SMS TR",
