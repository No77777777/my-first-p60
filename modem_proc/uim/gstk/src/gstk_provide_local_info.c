...
$Header: //components/rel/uim.mpss/6.1.0/gstk/src/gstk_provide_local_info.c#2 $

when       who     what, where, why
--------   ---     --------------------------------------------------------------------
09/18/17   shr     Send error TR for PLI (Time) if user time is not yet updated
07/28/17   dt      Correct the size provided in memscpy
11/07/16   shr     Migration to MSGR VARIANT from Inst ID
10/07/16   vr      Start TR timeout for the PLI commands for which TR is not immediate
...
09/15/15   vr      USAT pairing, notify IMEI or IMEISV success TR to MMGSDI
  ...
  provide_local_info_terminal_response_type  *STK_response_ptr)
{
  ...
  gstk_util_release_upper_layer_cmd_memory(
    (gstk_cmd_type*)provide_local_info_cnf_ptr);
} /* gstk_provide_local_info_cnf_cleanup */
  ...
  *need_to_send_immediate_tr_ptr = TRUE;
  ...
  switch (*nmr_utran_qualifier_ptr)
  {
    ...
    case GSTK_NMR_LTE_INTER_RAT_UTRAN_MEAS:
      ...
      /* Populate Variant in the LTE RRC message header */
      msgr_set_hdr_variant(&lte_nmr_req.msg_hdr, (uint8)as_id);
  ...
  gstk_proactive_cmd_type* command_ptr)
{
  ...
  */
    ...
    */
...
#else
      UIM_MSG_HIGH_0("Loc Info Type: GSTK_GSM_UMTS_LOCATION_INFO - GWL not supported");
      general_result = GSTK_COMMAND_BEYOND_THE_ME_CAPABILITIES;
    ...
    case GSTK_DATE_TIME_ZONE_INFO:
            ...
            if (gstk_curr_inst_ptr->tz_info[i].tz_avail == GSTK_TZ_AVAILABLE)
            {
              switch (gstk_curr_inst_ptr->gstk_next_curr_rat)
              {
                ...
                case GSTK_ACCESS_TECH_LTE:
                  local_info_ptr->info_data.time.zone = (uint8)gstk_curr_inst_ptr->tz_info[i].tz;
      ...
      */
      ...
      /* Only for CUST_1, if current user time is not in the valid
         ballpark, this means user time is not yet updated in the modem.
         Hence, send error TR */
  ...
  {
    /* Response will come later, start TR wait Timer */
  ...
  gstk_provide_local_info_cnf_type* provide_local_info_cnf_ptr)
{
  ...
  uint32                                      ref_id        = 0;
  gstk_nv_items_data_type                     nv_data;
  boolean                                     ext_earfcn_supported = FALSE;
  mmgsdi_session_id_type                      mmgsdi_sess_id       = 0;

  UIM_MSG_HIGH_0("IN GSTK provide local info cnf");
         ...
         (provide_local_info_cnf_ptr->local_info_rsp.info_data.nmr_utran.nmr_ptr))
      {
        ...
        case GSTK_NMR_LTE_INTER_FREQ_MEAS:
          /* Handle multiple NMR */
          {
            ...
            /* Get current slot's MMGSDI session id */
            for (i = 0; i < GSTK_MMGSDI_SESS_INFO_SIZE; i++)
            {
              if (gstk_shared_data.mmgsdi_sess_info[i].slot == gstk_curr_inst_ptr->slot_id &&
                  (gstk_shared_data.mmgsdi_sess_info[i].sess_type ==  MMGSDI_GW_PROV_PRI_SESSION ||
              ...
              {
                mmgsdi_sess_id = gstk_shared_data.mmgsdi_sess_info[i].sess_id;
                break;
              }
            }

            /* Determine if extended EARFCN is supported */
            if (gstk_curr_inst_ptr->card_protocol != MMGSDI_UICC)
            {
              UIM_MSG_HIGH_0("Card is not UICC - disable ext EARFCN support");
              ext_earfcn_supported = FALSE;
            }
            else if (mmgsdi_session_is_service_available_sync(mmgsdi_sess_id,
                                                              MMGSDI_USIM_SRV_EARFCN,
                                                              &ext_earfcn_supported) != MMGSDI_SUCCESS)
            {
              UIM_MSG_HIGH_0("mmgsdi_session_is_service_available_sync() failed, disable ext EARFCN support");
              ext_earfcn_supported = FALSE;
            }
            else if (!ext_earfcn_supported)
            {
              UIM_MSG_HIGH_0("USIM service table does not support ext EARFCN");
            }
                 ...
                 i++)
            {
              ...
              /*
                 Copy Freq + Meas report
                 May need to convert Freq to byte order if LTE does not change
                 the data to byte order

                 Support 4 bytes for frequency if USIM service table supports extended EARFCN support
                 Otherwise do not support 4 bytes for frequency - truncate to 2 bytes
              */
              if (!ext_earfcn_supported)
              {
                /*
                  Truncate the frequency copied to 2 bytes, and copy back into nmr_buf_ptr
                  for gstk_packer_nmr_utran_tlv() to work as is
                */
                gstk_memcpy(&freq,
                            &nmr_buf_ptr[offset],
                            4,
              ...
              if (ext_earfcn_supported)
              {
                STK_response.local_info_rsp.nmr_utran_info.nmr_utran_ptr[i].tag =
                  GSTK_EUTRAN_INTER_FREQ_NMR_RESULTS_TAG;
              }
              offset += rpt_len;
            }
          }
  ...
  if (uim_cmd_ptr != NULL ) {
    ...
    if (gstk_status == GSTK_SUCCESS) {
      if (uim_cmd_ptr != NULL) {
        ...
        if (gstk_status == GSTK_SUCCESS)
        {
          GSTK_UTIL_SELECTIVE_DUMP_BYTE_ARRAY("Provide Local Info TR",
                                              uim_cmd_ptr->terminal_response.data,
                                              uim_cmd_ptr->terminal_response.num_bytes);

          gstk_send_cmd_to_uim_server(uim_cmd_ptr); /* clear signal and call uim_cmd */

          if((provide_local_info_cnf_ptr->local_info_rsp.info_type == GSTK_IMEI_INFO ||
              provide_local_info_cnf_ptr->local_info_rsp.info_type == GSTK_IMEISV_INFO) &&
             provide_local_info_cnf_ptr->command_result == GSTK_COMMAND_PERFORMED_SUCCESSFULLY)
          {
            /* IMEI or IMEISV sent to the card notify MMGSDI */
            (void) mmgsdi_notify_imei_sent_to_card((mmgsdi_slot_id_enum_type)gstk_curr_inst_ptr->slot_id);
          }
