...
Copyright (c) 2003-2005, 2008-2020 QUALCOMM Technologies, Inc (QTI) and its licensors.
...
$Header: //components/rel/uim.mpss/6.1.0/gstk/src/gstk_s_idle.c#3 $ $Author: pwbldsvc $ $DateTime: 2020/01/08 01:46:28 $

when       who     what, where, why
--------   ---     --------------------------------------------------------------------
01/02/20   sch     Added support for log masking
06/06/19   ks      Clear User data before sending the TR report to client
02/07/19   gs      Featurize processing mm or mode change under FEATURE_CDMA
01/19/18   dt      Check for pending proactive command after recovery
...
07/05/16   vr      Validate RRC rat with curr next rat
...
06/09/15   vr      Update END_PROACTIVE_SESSION command header details
06/05/15   gm      Split ME and slot specific NV
06/05/15   vr      Process End proactive command using different varaiable
05/13/15   vr      Process End proactive command using different varaiable
...
03/09/15   gm      Fix compiler error
02/20/15   gm      Moved CC command cache to heap
02/06/15   gm      Retry call control after recovery
02/05/15   dy      Fix GSTK not blocking Access Tech if MM idle/Mode change
                   received before setup event list
02/04/15   bd      Do not update intermediate 3GPP2 information
01/08/15   vr      Retry envelope command after fetching the proactive command
11/21/14   gm      Restrict Cell ID length to two for GSM
  ...
  gstk_cmd_type* cmd_ptr)
{
  ...
  boolean                         send_loc_status   = FALSE;
  boolean                         send_acc_tech     = FALSE;
  ...
  switch(cmd_ptr->general_cmd.message_header.command_group )
  {
    case GSTK_TR_REPORT:
     ...
     /* cmd_ptr->general_cmd_data[2] is proactive_cmd_pending */
      ...
      {
        gstk_proactive_cmd_type end_proactive_cmd;
        /* build the end proactive session */
        end_proactive_cmd.message_header.command_group = GSTK_PROACTIVE_CMD;
        end_proactive_cmd.message_header.command_id    = GSTK_END_PROACTIVE_CMD_REQ; /* doesn't matter */
        end_proactive_cmd.message_header.sim_slot_id   = cmd_ptr->general_cmd.message_header.sim_slot_id;
        end_proactive_cmd.message_header.user_data     = cmd_ptr->general_cmd.message_header.user_data;
        end_proactive_cmd.message_header.client_id     = gstk_curr_inst_ptr->client_id;

        /* initialize to 0 */
        memset(end_proactive_cmd.data, 0x00, sizeof(end_proactive_cmd.data));
        end_proactive_cmd.data[0] = (uint8)GSTK_PROACTIVE_SIM_COMMAND_TAG;
        end_proactive_cmd.data[1] = 0x09;
        end_proactive_cmd.data[2] = (uint8)GSTK_COMMAND_DETAILS_TAG;
        end_proactive_cmd.data[3] = (uint8) GSTK_COMMAND_DETAILS_LEN;
        end_proactive_cmd.data[4] = 0;
        end_proactive_cmd.data[5] = (uint8)GSTK_CMD_STK_END_OF_PROACTIVE_SES;
        end_proactive_cmd.data[GSTK_GENERATED_END_SES_INDEX] =  (uint8)GSTK_GENERATED_END_SES;
        end_proactive_cmd.data[7] = (uint8)GSTK_DEVICE_IDENTITY_TAG;
        end_proactive_cmd.data[8] = (uint8)GSTK_DEVICE_IDENTITY_LEN;
        end_proactive_cmd.data[9] = (uint8)GSTK_UICC_SIM_DEVICE;
        end_proactive_cmd.data[10]= (uint8)GSTK_ME_DEVICE;
        /* length of the end proactive command data */
        end_proactive_cmd.length  = 0x0B;
        ...
        gstk_status = gstk_process_proactive_command(&end_proactive_cmd);
    ...
    case GSTK_RECOVERY_CMD:
      ...
      switch(cmd_ptr->general_cmd.message_header.command_id)
      {
          ...
          for(i = 0; i < GSTK_MAX_ENVELOPE_COMMAND; ++i)
          {
               ...
               !gstk_shared_data.envelope_cmd_ref_table[i].is_free )
            {
              if(gstk_shared_data.envelope_cmd_ref_table[i].expected_cmd_rsp_type == GSTK_CC_IND_RSP && 
                   gstk_shared_data.envelope_cmd_ref_table[i].gstk_cc_cache_ptr != NULL)
              {
               (void)gstk_slot_send_envelope_cc_command_ext(slot,
                         gstk_shared_data.envelope_cmd_ref_table[i].user_data,
                         gstk_shared_data.envelope_cmd_ref_table[i].gstk_cc_cache_ptr,
                         gstk_shared_data.envelope_cmd_ref_table[i].env_cb.client_env_cb);

                gstk_util_env_cmd_ref_table_free_slot(i);                   
                continue;                
              }
              if(gstk_shared_data.envelope_cmd_ref_table[i].expected_cmd_rsp_type == GSTK_MO_SMS_CTRL_IND_RSP && 
                   gstk_shared_data.envelope_cmd_ref_table[i].mo_sms_cache_ptr != NULL)
              {
                (void)gstk_slot_send_envelope_mo_sms_ctrl_command_ext(slot,
                         gstk_shared_data.envelope_cmd_ref_table[i].user_data,
                         gstk_shared_data.envelope_cmd_ref_table[i].mo_sms_cache_ptr,
                         gstk_shared_data.envelope_cmd_ref_table[i].env_cb.client_env_cb);
        ...
        case GSTK_RECOVERY_END_CMD:
          ...
          if((gstk_shared_data.fetch_allowed[(uint8)slot - 1]) &&
             (gstk_curr_inst_ptr->is_fetch_ignored))
          {
            /* call uimdrv API to check pending proactive command */
            (void)uim_check_pending_proactive_command((uim_slot_type)slot);
          }
          break;
        default:
          break;
    ...
    case GSTK_PROACTIVE_CMD:
      ...
      {
        if(gstk_util_retry_envelope_cmd(slot) != GSTK_SUCCESS)
        {
        ...
        */
        switch(location_status)
        {
          ...
          case GSTK_LIMITED_SERVICE:
            ...
            else if(((sys_radio_access_tech_e_type)cmd_ptr->mm_idle_state.rat == SYS_RAT_GSM_RADIO_ACCESS) &&
                     (temp_cell_id.cell_len > GSTK_MIN_CELL_ID_LEN))
            {
               temp_cell_id.cell_len = GSTK_MIN_CELL_ID_LEN;
               cmd_ptr->mm_idle_state.cell_id.cell_len = temp_cell_id.cell_len;
            }
        ...
        { /* has registered for this event */
          ...
          send_loc_status = TRUE;
        }
      }while(0);
      /* Send location status and access technolgy events */
      gstk_status = gstk_build_and_queue_location_status_access_tech_evt_dl(send_loc_status,
                                                                            send_acc_tech,
                                                                            gstk_curr_inst_ptr->gstk_curr_rat,
                                                                            gstk_curr_inst_ptr->gstk_curr_location_status);
      if (gstk_status == GSTK_SUCCESS && send_loc_status)
      {
    ...
    case GSTK_MODE_CHANGE_IND:
     ...
     do
     {
        /* FEATURE DISABLE CDMA LOCATION enabled, skip mode change indication */
        ...
        else
        {
          ...
          if(gstk_curr_inst_ptr->mm_idle_pri_ptr != NULL)
          {
            ...
            {
              UIM_MSG_HIGH_0("No 3GPP RAT Found, Free MM_IDLE cache and wait for next MM IDLE evt");
        ...
        /* Do not process further if MM IDLE is prioritized */
        if(process_mm_idle == TRUE)
        {
          break;
        }
            ...
            ((gstk_curr_inst_ptr->gstk_evt_list[GSTK_LOCATION_STATUS] & 0x01) == 0x01))
        {
          ...
          gstk_curr_inst_ptr->gstk_curr_location_status =
             gstk_curr_inst_ptr->gstk_next_curr_location_status;
          UIM_MSG_HIGH_0("Send Location Status");
          send_loc_status = TRUE;
        }
     } while(0);
    ...
    case GSTK_RRC_CELL_CHANGE_IND:
      ...
      */
      ...
      if(cmd_ptr->rrc_cell_change_ind.rrc_rat != gstk_curr_inst_ptr->nas_rat ||
         cmd_ptr->rrc_cell_change_ind.rrc_rat != gstk_curr_inst_ptr->gstk_next_curr_rat)
      {
        UIM_MSG_LOW_3("RRC RAT 0x%x does not match with NAS RAT 0x%x and next curr rat 0x%x - drop this",
                       cmd_ptr->rrc_cell_change_ind.rrc_rat,
                       gstk_curr_inst_ptr->nas_rat,
