...
$Header: //components/rel/uim.mpss/6.1.0/gstk/src/gstklib_rawcmd.c#1 $ $DateTime: 2019/10/30 06:56:05 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
10/24/16   gs      Changes in TP based on RAT and App Type
09/23/16   gs      Initial Version
  ...
  gstk_generic_data_type                  term_resp)
{
  ...
  gstk_term_resp_buff_ptr = (uint8 *)GSTK_CALLOC(term_resp.data_len);
  ...
  if ( task_cmd_ptr != NULL ) { /* successfully obtained command buffer */
    ...
    task_cmd_ptr->cmd.client_raw_term_resp.message_header.command_id    =
      (uint32)GSTK_RAW_TERMIMAL_RESPONSE;
  ...
  gstk_evt_cb_funct_type       gstk_raw_env_cb)
{
  ...
  if ( task_cmd_ptr != NULL )
  {
    ...
    task_cmd_ptr->cmd.raw_envelope_ind.env_len      = env_cmd.data_len;
    task_cmd_ptr->cmd.raw_envelope_ind.env_data_ptr = GSTK_CALLOC(env_cmd.data_len);
