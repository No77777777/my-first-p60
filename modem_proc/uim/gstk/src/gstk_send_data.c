...
Copyright (c) 2003-2010, 2012-2014, 2016, 2020 QUALCOMM Technologies, Inc (QTI) and its licensors.  
...
$Header: //components/rel/uim.mpss/6.1.0/gstk/src/gstk_send_data.c#3 $
  ...
  send_data_terminal_response_type *STK_response_ptr)
{
  ...
  gstk_util_release_upper_layer_cmd_memory(
    (gstk_cmd_type*)send_data_cnf_ptr);
...
)
{
  ...
  UIM_MSG_HIGH_0(" IN GSTK_SEND_DATA_REQ ");
  ...
  */
  ...
  slot_id = command_ptr->message_header.sim_slot_id;

  if(estk_is_keep_channel_open_set(slot_id))
  /* Check if a BIP channel is kept open. If it is open and SEND DATA is received instead of OPEN CHANNEL, 
     send error TR*/
  {
    gstk_set_additional_info(&additional_info, GSTK_CH_CLOSED);
  ...
  if (uim_cmd_ptr != NULL )
  {
    ...
    if (gstk_status == GSTK_SUCCESS)
    {
      if (uim_cmd_ptr != NULL) {
        ...
        if (gstk_status == GSTK_SUCCESS)
        {
          GSTK_UTIL_SELECTIVE_DUMP_BYTE_ARRAY("Send Data TR",
