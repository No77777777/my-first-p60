...
$Header: //components/rel/uim.mpss/6.1.0/gstk/src/gstk_setup_idle_txt.c#2 $
  ...
  setup_idle_mode_txt_terminal_response_type       * STK_response_ptr)
{
  ...
  gstk_util_release_upper_layer_cmd_memory(
    (gstk_cmd_type*)idle_text_cnf_ptr);
  ...
  gstk_proactive_cmd_type* command_ptr)
{
  ...
  UIM_MSG_HIGH_0(" IN GSTK_SETUP_IDLE_MODE_TXT_REQ ");
    ...
    if (gstk_status == GSTK_SUCCESS) {
      if (uim_cmd_ptr != NULL) {
                ...
                (size_t)(STK_response.data.device_id.device_tag_length + GSTK_TAG_LENGTH_LEN))
       {
        ...
        if (gstk_status == GSTK_SUCCESS)
        {
          GSTK_UTIL_SELECTIVE_DUMP_BYTE_ARRAY("Setup Idle Txt",
...
} /* gstk_release_setup_idle_text_memory */
