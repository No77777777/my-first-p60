...
Copyright (c) 2003-2018 QUALCOMM Technologies, Inc (QTI) and its licensors.
...
$Header: //components/rel/uim.mpss/6.1.0/gstk/src/gstk.h#4 $ $DateTime: 2020/02/19 02:48:57 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     --------------------------------------------------------------------
11/08/19   dt      Fixing offtarget memory leak issue
10/09/19   dt      Add macro GSTK_FREE
...
04/17/17   dt      Adding qmi_cat_enabled to gstk_shared_data
...
11/09/15   vr      Reset usr_activity and idle_scrn for raw envelope response evt dl
...
#define GSTK_PROACTIVE_COMMAND_MINIMUM_LEN  11
#define GSTK_COMMAND_DETAILS_TLV_SIZE       5
...
#define GSTK_GEO_LOC_REPORT_TAG                  0xDD
...
#define GSTK_EUTRAN_MAPPED_UTRAN_PS_NO_PARAM_LEN 0x02
#define GSTK_EUTRAN_MAPPED_UTRAN_PS_PARAM_LEN    0x06
#define GSTK_EUTRAN_MAPPED_UTRAN_PS_PARAM_EX_LEN 0x0A
...
#define GSTK_CONTACTLESS_STATE_REQ_LEN           0x01
#define GSTK_CONTACTLESS_FUNC_STATE_LEN          0x01
...
#define GSTK_MAX_EVENT_LIST_LEN         0x1C /* based on Standard, there are only 0x1D events
...
#define GSTK_SEND_SMS_HOLD_TIME                 40000
/* Timer for 2 min 45 sec as max time for CP_ACK of SMS_PP for which 
   we are holding SEND SMS i.e 40sec is added to 2min 5 secs */
#define GSTK_SEND_SMS_TERM_RSP_WAIT_TIME        (GSTK_TERM_RSP_WAIT_TIME + GSTK_SEND_SMS_HOLD_TIME)
/* For BIP OPEN CHANNEL and SEND DATA, extend the TR timer by 30 sec, to
   ...
   vice-versa */
#define GSTK_BIP_TERM_RSP_WAIT_TIME             185000 /* 3 Minutes and 5 seconds */
...
#define GSTK_TI_VALUE_MAP_TABLE_SIZE    0x08
...
#define GSTK_CLASS_R_BEGIN_INDEX   (GSTK_CLASS_M_TERM_INDEX + 1)
#define GSTK_CLASS_R_TERM_INDEX    (GSTK_CLASS_M_TERM_INDEX + 2)

#define GSTK_CLASS_N_BEGIN_INDEX   (GSTK_CLASS_R_TERM_INDEX + 1)
#define GSTK_CLASS_N_TERM_INDEX    (GSTK_CLASS_R_TERM_INDEX + 2)

#define GSTK_TOTAL_ELEMENTS_IN_CLASS_ARRAY (GSTK_CLASS_N_TERM_INDEX + 1)
...
#define GSTK_CATEGORY_R_ADJ_OFFSET    GSTK_CLASS_R_BEGIN_INDEX
#define GSTK_CATEGORY_N_ADJ_OFFSET    GSTK_CLASS_N_BEGIN_INDEX
...
#define GSTK_RETURN_IF_NULL_PTR(x)                              \
  ...
  #define GSTK_FREE(x)                                              \
          if((x) != NULL)                                       \
          {                                                     \
            modem_mem_free((void*)(x),MODEM_MEM_CLIENT_UIM);    \
            (x) = NULL;                                         \
          }
#endif
...
#define GSTK_IS_VALID_AS_ID(x)                                  \
        ((sys_modem_as_id_e_type)(x) > SYS_MODEM_AS_ID_NONE &&  \
        (sys_modem_as_id_e_type)(x) < SYS_MODEM_AS_ID_NO_CHANGE)
...
typedef enum
{
    ...
    GSTK_CONTACTLESS_STATE_REQ_TAG                            = 0x53,
    GSTK_CONTACTLESS_FUNC_STATE_TAG                           = 0x54,
...
/* Envelope commands enum values that are cached and compared before sending
   to the card to avoid duplicate envelopes */
typedef enum
{
  GSTK_LOC_STATUS_EVT_DL = 0,
  GSTK_MAX_CACHED_ENV_CMD_TO_CARD
} gstk_cached_env_cmd_to_card_enum_type;
...
typedef struct
{
   uint8                         tag;                   /* GSTK_NETWORK_MEASUREMENT_RESULTS_TAG */
   uint8                         length;                /* GSTK_NMR_LEN */
   uint8                         nmea[GSTK_NMEA_LEN];   /* based on 3G 24.008 starting at octet 2 */
}
gstk_nmea_tlv_type;
...
typedef enum
{
   ...
   GSTK_CONTACTLESS_STATE_REQ          = 0x16,
...
{
    uint8                         tag;
    uint8                         contactless_state_req_length;
    uint8                         contactless_state_req;
}
gstk_contactless_state_req_tlv_type;
...
typedef struct
{
    uint8                         tag;                          /* GSTK_CSG_CELL_PLMN_INFO_TAG */
    uint8                         plmn_info_len;                /* GSTK_CSG_CELL_PLMN_INFO_LEN */
    uint8                         plmn_info[GSTK_MAX_PLMN_LEN]; /* GSTK_CSG_CELL_PLMN_INFO */
}
gstk_csg_cell_plmn_info_tlv_type;
...
typedef gstk_generic_terminal_response_msg_type contactless_state_changed_terminal_response_type;
typedef gstk_generic_terminal_response_msg_type geo_loc_terminal_response_type;
...
typedef struct
{
   ...
   gstk_csg_cell_plmn_info_tlv_type         plmn_info;   
}
...
{
  gstk_envelope_event_dl_cmd_header_type      event_download_hdr;
  gstk_contactless_state_req_tlv_type         request;
}
gstk_envelope_contactless_state_req_evt_dl_command_type;
...
/* based on gstk_envelope_sms_pp_download_command_type's nmea sentence params */
typedef struct
{
   uint8                                    tag;                 /* GSTK_GEO_LOC_REPORT_TAG */
   uint8                                    length;
   gstk_device_identities_tag_tlv_type      device_id;
   gstk_nmea_tlv_type                       nmea;
}
gstk_envelope_geo_loc_report_command_type;
...
typedef struct
{
    ...
    gstk_cc_ind_type_ext                     *gstk_cc_cache_ptr;
    gstk_mo_sms_ctrl_ind_ext_type            *mo_sms_cache_ptr;
    gstk_evt_list_code_enum_type              evt_list_code;
    /* Is used to hold the status if SMS PP received is on 1x or not */
...
typedef struct
{
  ...
  /* Current WLAN IPv4 address */
  uint32                    wlan_ipv4_addr;
} gstk_wlan_info_type;
...
/*===========================================================================
  ...
  STRUCT: GSTK_TI_VALUE_MAP_TABLE_TYPE

  PURPOSE:
    The structure is used to hold information related to the TI value assigned
    for a call event.
...
{
  /* Call Event Trigger */
  gstk_call_evt_trigger_type  call_evt_trigger;
  /* Holds TI value if Call Event trigger is:
     - GSTK_CALL_TRIGGER_IMS_EXT
     Holds Call ID if Call Event trigger is:
     - GSTK_CALL_TRIGGER_CM_CDMA
     - GSTK_CALL_TRIGGER_CM_GW
     - GSTK_CALL_TRIGGER_IMS_INT */
  uint8                      orig_ti_value_call_id;
  /* Indicates if the table entry is in use */
  boolean                    in_use;
} gstk_ti_value_map_table_type;
...
typedef struct
{
  ...
  /* GSTK TI value mapping table */
  gstk_ti_value_map_table_type       gstk_ti_value_map_table[GSTK_TI_VALUE_MAP_TABLE_SIZE];
  gstk_pco_info_type                 pco_info;
  /* Cached envelope commands */
  gstk_generic_data_type             gstk_cached_env_cmds[GSTK_MAX_CACHED_ENV_CMD_TO_CARD];
#ifdef FEATURE_CDMA
...
typedef struct
{
  ...
  /* WLAN availability status as extracted from
     DS_SYS_EVENT_SYSTEM_STATUS_EX */
  gstk_wlan_info_type                  *wlan_info_ptr;
...
#endif /* FEATURE_CDMA */
  boolean                               qmi_cat_enabled;
  /* Is used to hold the status if CP_ACK for SMS PP is received or not per as_id. */
  boolean                               is_cp_ack_pending[GSTK_MAX_NUM_OF_INSTANCES]; 
}
...
SEE ALSO
  None
===========================================================================*/
extern void gstk_power_down ( void );
