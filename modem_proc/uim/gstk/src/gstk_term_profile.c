...
$Header: //components/rel/uim.mpss/6.1.0/gstk/src/gstk_term_profile.c#3 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
12/10/19   ks      Send End proactive session when cleaning up refresh reset
05/24/19   dt      Set default value for PLI lang bit in TP to 0
10/25/18   ks      Enable/Disable CSG TP bit
...
06/28/17   shr     Disable Call Event Envelope TP bits if GSTK_CFG_FEATURE_ME_
...
04/07/17   dt      Add psm status to TP from MMGSDI
01/30/17   dt      Fix contactless state handling 
12/23/16   dt      Fix in sending the CSG TP
12/20/16   dt      Add support for rejecting Proactive cmds based on TP support
12/20/16   gs      Support / Fix TP Capabilities
...
09/07/15   gs      Fixed the condition to check number of TP DL 
...
)
{
  ...
  uint8 gstk_default_tp[] =
  {
    ...
    0x10, /* Access tech */ 
    0x00,
    0x07, /* Display Text, Send DMTF, PLI (NMR) */
...
)
{
  ...
  gstk_profile_dl_support_ext_type default_tp[] =
  {
    ...
    {1, GSTK_CSG_CELL_SELECTION_SUPPORT,          (uint8)GSTK_APP_USIM},
    {1, GSTK_PROVIDE_LOCAL_INFO_CSG_SUPPORT,      (uint8)GSTK_APP_USIM}
  };
...
)
{
  ...
  if (term_profile_ptr == NULL || term_profile_ptr->terminal_profile_cmd.length == 0)
  {
    return GSTK_ERROR;
  }
  ...
  gstk_curr_inst_ptr->client_tp_ptr[idx]->data_ptr = GSTK_CALLOC(term_profile_ptr->terminal_profile_cmd.length);
  
  if (!gstk_curr_inst_ptr->client_tp_ptr[idx]->data_ptr)
  {
    gstk_free(gstk_curr_inst_ptr->client_tp_ptr[idx]);
    gstk_curr_inst_ptr->client_tp_ptr[idx] = NULL;
    return GSTK_MEMORY_ERROR;
  ...
  if(gstk_memcpy( gstk_curr_inst_ptr->client_tp_ptr[idx]->data_ptr,
                 term_profile_ptr->terminal_profile_cmd.data_ptr,
                 term_profile_ptr->terminal_profile_cmd.length,
                 gstk_curr_inst_ptr->client_tp_ptr[idx]->length,
                 term_profile_ptr->terminal_profile_cmd.length) <
     term_profile_ptr->terminal_profile_cmd.length)
  {
    gstk_free(gstk_curr_inst_ptr->client_tp_ptr[idx]->data_ptr);
    gstk_curr_inst_ptr->client_tp_ptr[idx]->data_ptr = NULL;
    gstk_free(gstk_curr_inst_ptr->client_tp_ptr[idx]);
    gstk_curr_inst_ptr->client_tp_ptr[idx] = NULL;
    return GSTK_ERROR;
  }
  ...
  boolean exit_from_psm_status)
{
  ...
  if (uim_cmd_ptr != NULL)
  {
    ...
    if (gstk_status == GSTK_SUCCESS)
    {
      ...
      /* dump TP contents*/
      GSTK_UTIL_SELECTIVE_DUMP_BYTE_ARRAY("gstk_terminal_profile_cache",
                                gstk_curr_inst_ptr->gstk_terminal_profile_cache,
...
)
{
  ...
  {
      app_type      = (gstk_app_enum_type)term_profile_ptr->data_ptr[i++];
      byte_offset   = term_profile_ptr->data_ptr[i++];
      bit_offset    = term_profile_ptr->data_ptr[i++];
      ...
      if (byte_offset < GSTK_TERMINAL_PROFILE_SIZE)
      {
        if (app_type == GSTK_APP_NONE)
        {
          ...
          case GSTK_SET_UP_CALL_SUPPORT:
              term_profile_ptr->data_ptr[i] &= 0x01;
              gstk_curr_inst_ptr->gstk_terminal_profile_cache[byte_offset] |=
                (uint8)(term_profile_ptr->data_ptr[i] << bit_shift);
              gstk_curr_inst_ptr->gstk_terminal_profile_cache[7] |=
                uint16touint8(term_profile_ptr->data_ptr[i] << 6);
              break;
          case GSTK_GET_INKEY_SUPPORT:
              ...
              gstk_curr_inst_ptr->gstk_terminal_profile_cache[byte_offset] |=
                (uint8)(term_profile_ptr->data_ptr[i] << bit_shift);
              gstk_curr_inst_ptr->gstk_terminal_profile_cache[7] |=
                uint16touint8(term_profile_ptr->data_ptr[i] << 3);
              break;
          ...
          case GSTK_MAX_SK_SUPPORT: /* 8 bits of info */
              term_profile_ptr->data_ptr[i] &= 0xFF;
              bit_shift -= 0x07;
              gstk_curr_inst_ptr->gstk_terminal_profile_cache[byte_offset] |=
                (uint8) (term_profile_ptr->data_ptr[i] << bit_shift);
              break;
          case GSTK_PROTOCOL_VERSION: /* 4 bits of info */
              term_profile_ptr->data_ptr[i] &= 0x0F;
              bit_shift -= 0x03;
              gstk_curr_inst_ptr->gstk_terminal_profile_cache[byte_offset] |=
                (uint8) (term_profile_ptr->data_ptr[i] << bit_shift);
              break;
          case GSTK_LAUNCH_BROWSER_SUPPORT:
              ...
              /* Disable if GSTK_FEATURE_ENABLE_LAUNCH_BROWSER is FALSE */
              if((gstk_status == GSTK_SUCCESS) && (!command_enable))
              {
                gstk_curr_inst_ptr->gstk_terminal_profile_cache[byte_offset] &= 
                       ~((uint8)(term_profile_ptr->data_ptr[i] << bit_shift));
        ...
        if (app_type == GSTK_APP_USIM && gstk_curr_inst_ptr->card_protocol == MMGSDI_UICC)
        {
          switch(location_mask)
          {
            ...
            case GSTK_BROWSER_STATUS_EVT_SUPPORT:
            case GSTK_HCI_CONNECTIVITY_EVT_SUPPORT:
            case GSTK_ALPHA_REFRESH_SUPPORT:
            case GSTK_EXT_LAUNCH_BROWSER_HTML_SUPPORT:
            ...
            case GSTK_COMMUNICATION_CONTROL_FOR_IMS_SUPPORT:
              if (gstk_nv_get_me_feature_status(
                   GSTK_CFG_FEATURE_ME_TP_IMS_ENABLE) == TRUE)
              { 
                term_profile_ptr->data_ptr[i] &= 0x01;
                gstk_curr_inst_ptr->gstk_terminal_profile_cache[byte_offset] |=
                    (uint8) (term_profile_ptr->data_ptr[i] << bit_shift);
            ...
            case GSTK_IMS_REGISTRATION_EVENT:
              if (gstk_nv_get_me_feature_status(
                  ...
                  gstk_nv_get_feature_2_status(
                    GSTK_CFG_FEATURE_2_SLOT_ENABLE_TP_IMS_REG_EVENT,
                    gstk_curr_inst_ptr->slot_id) == TRUE)
              { 
                term_profile_ptr->data_ptr[i] &= 0x01;
                gstk_curr_inst_ptr->gstk_terminal_profile_cache[byte_offset] |=
           ...
           case GSTK_ACTIVATE_SUPPORT:
              ...
              break;
            case GSTK_PROVIDE_LOCAL_INFO_CSG_SUPPORT:
...
/*===========================================================================
FUNCTION: gstk_is_bit_enabled_in_term_profile

DESCRIPTION:
  This function will return TRUE if the TP bit indicated is enabled
  in the terminal profile sent to card.

PARAMETERS:
  profile_dl_bitmask - bitmask indicating the TP byte and bit

DEPENDENCIES:
  The function should be called only after TP is sent to card

RETURN VALUE:
  TRUE  - TP bit is enabled
  FALSE - TP bit is disabled
...
SIDE EFFECTS:
  None
===========================================================================*/
boolean gstk_is_bit_enabled_in_term_profile(
...
)
{
  ...
  byte_offset = (uint8)((profile_dl_bitmask & 0xFF00) >> 0x0008);
  bit_offset  = (uint8)(profile_dl_bitmask & 0x00FF);

  /* Check for byte_offset validity */
  if (byte_offset == 0)
  {
    return FALSE;
  }
  /* Since byte offset indicates TP bytes from 1 onwards,
     decrement to get offset into TP array */
  else
  {
    byte_offset--;
  }
  ...
  if (byte_offset < GSTK_TERMINAL_PROFILE_SIZE)
  {
    /* If Terminal Profile bit is enable, return TRUE */
    if(gstk_curr_inst_ptr->gstk_terminal_profile_cache[byte_offset] & bit_offset)
    {
      return TRUE;
    }
  }
  return FALSE;
} /* gstk_is_bit_enabled_in_term_profile */
