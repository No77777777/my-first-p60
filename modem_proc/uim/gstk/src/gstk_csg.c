...
$Header: //components/rel/uim.mpss/6.1.0/gstk/src/gstk_csg.c#1 $ $DateTime: 2019/10/30 06:56:05 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
10/25/18   ks      Add support to disable sending csg capability bit in TP
...
)
{
  ...
  if(csg_info.length == 0)
  {
    ...
    length = length + (uint8)gstk_byte_offset_memcpy(csg_cell_tlv->csg_info,
...
{
#ifdef FEATURE_FEMTO_CSG
  UIM_MSG_HIGH_0("** Packing CSG PLMN INFO **");

  csg_cell_plmn_info_tlv->tag = GSTK_CSG_PLMN_INFO_TAG;
  csg_cell_plmn_info_tlv->plmn_info_len =
      GSTK_MAX_PLMN_LEN;
  if(gstk_memcpy(csg_cell_plmn_info_tlv->plmn_info,
                 csg_cell_plmn_info,
                 GSTK_MAX_PLMN_LEN,
                 GSTK_MAX_PLMN_LEN,
                 GSTK_MAX_PLMN_LEN) <
  ...
  return GSTK_SUCCESS;
#else
  (void) csg_cell_plmn_info;
  (void) csg_cell_plmn_info_tlv;
  return GSTK_UNSUPPORTED_COMMAND;
#endif
}
...
void gstk_csg_cm_ss_info_change(const cm_mm_msim_ss_info_s_type*  ss_info_ptr)
{
  ...
  if (gstk_io_ctrl (GSTK_IO_MODEM_AS_ID_TO_SLOT_ID,
                    ...
                    &slot) != GSTK_SUCCESS)
  {
    UIM_MSG_ERR_1("failed to slot for asubs_id 0x%x!", ss_info_ptr->asubs_id);
    return;
  }
...
)
{
  ...
  (void)gstk_memcpy(task_cmd->cmd.csg_change_ind.cmd.plmn_info,
                    csg_ind->cmd.plmn_info,
                    GSTK_MAX_PLMN_LEN,
                    GSTK_MAX_PLMN_LEN,
                    GSTK_MAX_PLMN_LEN);
...
)
{
     ...
     (csg_evt_cmd->cmd.csg_id != SYS_CSG_ID_INVALID))
  {
    ...
    {
      /* Pack CSG plmn info */
      gstk_status = gstk_packer_csg_cell_plmn_info_tlv(
        &(csg_evt_cmd->cmd.plmn_info[0]),
        &STK_envelope_cmd.plmn_info);
      if (gstk_status != GSTK_SUCCESS)
      {
        return gstk_status;
      }  
  ...
  */
     ...
     (csg_evt_cmd->cmd.csg_id != SYS_CSG_ID_INVALID))
  {
    ...
    if(STK_envelope_cmd.plmn_info.plmn_info_len > 0)
    {
       STK_envelope_cmd.event_download_hdr.length += STK_envelope_cmd.plmn_info.plmn_info_len + 2;
    }    
  }
  ...
  if(uim_cmd_ptr != NULL )
  {
       ...
       (STK_envelope_cmd.csg_id.csg_id_len > 0))
    {
      ...
      if(gstk_status == GSTK_SUCCESS &&
         STK_envelope_cmd.plmn_info.plmn_info_len > 0)
      {
        if(gstk_byte_offset_memcpy(uim_cmd_ptr->envelope.data,
                                   &STK_envelope_cmd.plmn_info,
                                   offset,
                                   (size_t)STK_envelope_cmd.plmn_info.plmn_info_len + GSTK_TAG_LENGTH_LEN,
                                   sizeof(uim_cmd_ptr->envelope.data),
                                   sizeof(STK_envelope_cmd.plmn_info)) <
           (size_t)(STK_envelope_cmd.plmn_info.plmn_info_len + GSTK_TAG_LENGTH_LEN))
        {
          UIM_MSG_ERR_0("gstk_csg_evt_ind(): gstk_byte_offset_memcpy failed");
          gstk_status = GSTK_MEMORY_ERROR;
        }
        offset = (int)(offset + STK_envelope_cmd.plmn_info.plmn_info_len
                 + GSTK_TAG_LENGTH_LEN);
      }      
    }
  ...
  for(i=0; i < srs_plmn_csg_info->length; i++)
  {
    if(srs_plmn_csg_info->info[i].csg_info.csg_id != SYS_CSG_ID_INVALID)
    {
      ...
      dest_plmn_csg_info->available_plmn_info[j].csg_info[k].len =
        (uint8)gstk_memcpy(dest_plmn_csg_info->available_plmn_info[j].csg_info[k].hnb_name,
...
===========================================================================*/
boolean gstk_check_csg_capability(
  gstk_slot_id_enum_type slot_id)
{
#ifdef FEATURE_FEMTO_CSG
  /* Check if the device is configured as single SIM. CSG is not supported on multi-SIM devices. */ 
    if((gstk_shared_data.gstk_num_of_instances == 1) &&
       (gstk_nv_get_feature_2_status(GSTK_CFG_FEATURE_2_SLOT_DISABLE_TP_CSG, slot_id) == FALSE))
    { 			
      return TRUE;
    }
    else
