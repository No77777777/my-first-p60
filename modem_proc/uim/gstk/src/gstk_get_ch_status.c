...
$Header: //components/rel/uim.mpss/6.1.0/gstk/src/gstk_get_ch_status.c#2 $
  ...
  get_ch_status_terminal_response_type *STK_response_ptr)
{
  ...
  gstk_util_release_upper_layer_cmd_memory(
    (gstk_cmd_type*)get_ch_status_cnf_ptr);
...
)
{
  ...
  UIM_MSG_HIGH_0("In GSTK get channel status cnf");
  ...
  */
  ...
  if (uim_cmd_ptr != NULL )
  {
    ...
    if ((gstk_status == GSTK_SUCCESS) && (uim_cmd_ptr != NULL))
    {
      ...
      if (gstk_status == GSTK_SUCCESS)
      {
        GSTK_UTIL_SELECTIVE_DUMP_BYTE_ARRAY("Get Channel Status TR",
