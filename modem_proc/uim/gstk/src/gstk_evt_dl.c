...
$Header: //components/rel/uim.mpss/6.1.0/gstk/src/gstk_evt_dl.c#2 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
10/31/17   dt      Do not send channel status indication for closed channel
03/15/17   dt      adding extra byte handling of length for IMS Reg evt
02/08/17   vr      Skip blocking limited srv location status
01/18/17   vr      Avoid sending duplicate LS/Access Tech Envelopes to card
10/14/16   vr      Rework on cm_transaction_id to gstk_id function
10/07/16   vr      Fill only index 0 in cm_transaction_id to gstk_id
10/07/16   shr     GSTK/ESTK F3 reduction
10/03/16   dd      Removed lint comments
09/23/16   gs      F3 reduction / cleanup
09/15/16   vr      From Device identity of IMS evt dl changed to network
...
03/22/16   gs      Event downloads moved to corresponding new file
...
PARAMETERS
  transaction_id: [Input/Output] GSTK transaction ID length is only 1
...
static void gstk_util_translate_cm_transaction_id_to_gstk_id(
  ...
  boolean                           ti_flag,
  uint8                             cm_transaction_id)
{
  if ((transaction_id == NULL) || (transaction_id->length != 1))
  {
    return;
  }

  transaction_id->transaction_list[0] = cm_transaction_id;
  /* remove the most significant bit of the nibble to get TI value to construct the
     byte as per ETSI TS 124 007 */
  transaction_id->transaction_list[0] &= 0x07;

  /* Shift the transaction ID up because bit 1-4 if RFU according to ETSI TS 124 007 */
  transaction_id->transaction_list[0] = uint16touint8(transaction_id->transaction_list[0] << 4);
  /* Set TI Flag */
  if (ti_flag)
  {
    transaction_id->transaction_list[0] |= 0x80;
  }
} /* gstk_util_translate_cm_transaction_id_to_gstk_id */
  ...
  switch (call_event)
  {
    ...
    switch (call_info_ptr->mode_info.info_type)
    {
      case CM_CALL_MODE_INFO_GW_CS:
        ...
        /* Update Call Event info */
        call_evt_info.call_evt_trigger = GSTK_CALL_TRIGGER_CM_GW;
        call_evt_info.call_id = call_info_ptr->call_id;
#endif /* FEATURE_GSM || FEATURE_WCDMA || FEATURE_LTE*/
        break;
      ...
      case CM_CALL_MODE_INFO_CDMA:
        ...
        /* Update Call Event info */
        call_evt_info.call_evt_trigger = GSTK_CALL_TRIGGER_CM_CDMA;
        call_evt_info.call_id = call_info_ptr->call_id;
        break;
      ...
      case CM_CALL_MODE_INFO_IP:
           ...
           (call_info_ptr->call_type == CM_CALL_TYPE_EMERGENCY))
        {
          ...
          call_evt_info.call_id = call_info_ptr->call_id;
          /* Set TI flag */
          call_evt_user_data.transaction_id.transaction_list[0] = 0x80;
        }
        break;
    ...
    if(task_cmd != NULL)
    {
      ...
      /* Copy Call Event info */
      task_cmd->cmd.call_connected_envelope_ind.call_evt_info.call_evt_trigger = call_evt_info.call_evt_trigger;
      task_cmd->cmd.call_connected_envelope_ind.call_evt_info.call_id = call_evt_info.call_id;
      ...
      if ( task_cmd != NULL )
      {
        ...
        case CM_CALL_MODE_INFO_GW_CS:
          ...
          gstk_util_translate_cm_transaction_id_to_gstk_id(
            ...
            /* Update Call Event info */
            task_cmd->cmd.call_disconnected_envelope_ind.call_evt_info.call_evt_trigger = GSTK_CALL_TRIGGER_CM_GW;
            task_cmd->cmd.call_disconnected_envelope_ind.call_evt_info.call_id = call_info_ptr->call_id;
#endif /* FEATURE_GSM || FEATURE_WCDMA || FEATURE_LTE*/
        ...
        case CM_CALL_MODE_INFO_CDMA:
          ...
          /* Update Call Event info */
          task_cmd->cmd.call_disconnected_envelope_ind.call_evt_info.call_evt_trigger = GSTK_CALL_TRIGGER_CM_CDMA;
          task_cmd->cmd.call_disconnected_envelope_ind.call_evt_info.call_id = call_info_ptr->call_id;
          break;
        ...
        case CM_CALL_MODE_INFO_IP:
          ...
          {
            /* Update Call Event info */
            task_cmd->cmd.call_disconnected_envelope_ind.call_evt_info.call_evt_trigger = GSTK_CALL_TRIGGER_IMS_INT;
            task_cmd->cmd.call_disconnected_envelope_ind.call_evt_info.call_id = call_info_ptr->call_id;

            if(ti_flag == TRUE)
            {
              /* Set TI flag*/
              task_cmd->cmd.call_disconnected_envelope_ind.call_disconnected_data.transaction_id.transaction_list[0] = 0x80;
            }
            else
            {
              /* Clear TI flag */
              task_cmd->cmd.call_disconnected_envelope_ind.call_disconnected_data.transaction_id.transaction_list[0] = 0x0;
            }
          }
          break;
          ...
          if(task_cmd != NULL)
          {
            ...
            switch (call_info_ptr->mode_info.info_type)
            {
              case CM_CALL_MODE_INFO_GW_CS:
                ...
                /* Update Call Event info */
                task_cmd->cmd.mt_evt_envelope_ind.call_evt_info.call_evt_trigger = GSTK_CALL_TRIGGER_CM_GW;
                task_cmd->cmd.mt_evt_envelope_ind.call_evt_info.call_id = call_info_ptr->call_id;
#endif /* FEATURE_GSM || FEATURE_WCDMA || FEATURE_LTE */
                break;
              case CM_CALL_MODE_INFO_CDMA:
                ...
                /* Update Call Event info */
                task_cmd->cmd.mt_evt_envelope_ind.call_evt_info.call_evt_trigger = GSTK_CALL_TRIGGER_CM_CDMA;
                task_cmd->cmd.mt_evt_envelope_ind.call_evt_info.call_id = call_info_ptr->call_id;
                break;
              case CM_CALL_MODE_INFO_IP:
                /* For VOLTE call */
                ...
                {
                  /* Update Call Event info */
                  task_cmd->cmd.mt_evt_envelope_ind.call_evt_info.call_evt_trigger = GSTK_CALL_TRIGGER_IMS_INT;
                  task_cmd->cmd.mt_evt_envelope_ind.call_evt_info.call_id = call_info_ptr->call_id;
                  /* Clear TI flag */
                  task_cmd->cmd.mt_evt_envelope_ind.mt_evt_data.transaction_id.transaction_list[0] = 0x0;
                }
                break;
              default:
                UIM_MSG_ERR_1("invalid call type 0x%x",
   ...
   */
   ...
   if (uim_cmd_ptr != NULL ) {
     ...
     if ((gstk_status == GSTK_SUCCESS) && (uim_cmd_ptr != NULL)) {
       ...
       if (gstk_status == GSTK_SUCCESS)
       {
         GSTK_UTIL_SELECTIVE_DUMP_BYTE_ARRAY("HCI Connectivity Env",
  ...
  uint32      ref_id)
{
  ...
  if(gstk_curr_inst_ptr->gstk_curr_envelope_info.gstk_env_state == GSTK_SUB_ST_PENDING_ENV_NONE)
  {
    ...
    else
    {
      ...
      gstk_curr_inst_ptr->gstk_curr_envelope_info.env_data_bytes.data_len      = len;
  ...
  const gstk_envelope_location_statue_ind_type    *location_status_evt_cmd)
{
  ...
  case GSTK_GSM_UMTS_LOCATION_INFO:
    ...
    if(gstk_curr_inst_ptr->gstk_curr_location_status == GSTK_NORMAL_SERVICE)
    {
      ...
      UIM_MSG_HIGH_7("Location status is 0x%x  cell id len 0x%x"
	  	             "MNC_MCC is 0x%x 0x%x 0x%x"
	  	             "LAC is 0x%x 0x%x",
                     gstk_curr_inst_ptr->gstk_curr_location_status, location_info.cell_id_len,
                     location_info.mcc_and_mnc[0], location_info.mcc_and_mnc[1],
                     location_info.mcc_and_mnc[2], location_info.loc_area_code[0],
                     location_info.loc_area_code[1]);

      UIM_MSG_HIGH_4("Envelope Cell Id [0][1]: 0x%x 0x%x"
			           "Envelope Cell Id [2][3]: 0x%x 0x%x",
  ...
  */
  ...
  if (uim_cmd_ptr != NULL ) {
    ...
    if ((gstk_status == GSTK_SUCCESS) && (uim_cmd_ptr != NULL)) {
      ...
      if (gstk_status == GSTK_SUCCESS)
      {
        GSTK_UTIL_SELECTIVE_DUMP_BYTE_ARRAY("Location status Env",
                                            &uim_cmd_ptr->envelope.data[uim_cmd_ptr->envelope.offset],
                                            uim_cmd_ptr->envelope.num_bytes);

        /* Compared the data with cached envelope data if it is not limited service, if same don't send to card */
        if(STK_envelope_cmd.location_st.location_status != GSTK_LIMITED_SERVICE &&
           gstk_curr_inst_ptr->gstk_cached_env_cmds[GSTK_LOC_STATUS_EVT_DL].data_len ==  uim_cmd_ptr->envelope.num_bytes &&
           gstk_curr_inst_ptr->gstk_cached_env_cmds[GSTK_LOC_STATUS_EVT_DL].data_buffer_ptr != NULL &&
           memcmp(&uim_cmd_ptr->envelope.data[uim_cmd_ptr->envelope.offset],
                  gstk_curr_inst_ptr->gstk_cached_env_cmds[GSTK_LOC_STATUS_EVT_DL].data_buffer_ptr,
                  uim_cmd_ptr->envelope.num_bytes) == 0)
        {
          UIM_MSG_HIGH_0("gstk_location_status_evt_ind(): env cmd not send, same as previous one");
          /* cleanup envelope reference table */
          gstk_util_env_cmd_ref_table_free_slot(GSTK_ENV_CMD_TABLE_INDEX(uim_cmd_ptr->hdr.user_data));
          gstk_send_cmd_to_uim_free_q(uim_cmd_ptr);
          uim_cmd_ptr = NULL;
        ...
        {
          /* Cache the envelope info */
          gstk_free(gstk_curr_inst_ptr->gstk_cached_env_cmds[GSTK_LOC_STATUS_EVT_DL].data_buffer_ptr);
          gstk_curr_inst_ptr->gstk_cached_env_cmds[GSTK_LOC_STATUS_EVT_DL].data_buffer_ptr = NULL;
          gstk_curr_inst_ptr->gstk_cached_env_cmds[GSTK_LOC_STATUS_EVT_DL].data_buffer_ptr =
                                                        GSTK_CALLOC(uim_cmd_ptr->envelope.num_bytes);
          if (gstk_curr_inst_ptr->gstk_cached_env_cmds[GSTK_LOC_STATUS_EVT_DL].data_buffer_ptr != NULL)
          {
            gstk_curr_inst_ptr->gstk_cached_env_cmds[GSTK_LOC_STATUS_EVT_DL].data_len = 
              gstk_memcpy(gstk_curr_inst_ptr->gstk_cached_env_cmds[GSTK_LOC_STATUS_EVT_DL].data_buffer_ptr,
                          &uim_cmd_ptr->envelope.data[uim_cmd_ptr->envelope.offset],
                          uim_cmd_ptr->envelope.num_bytes,
                          uim_cmd_ptr->envelope.num_bytes,
   ...
   */
   ...
   if (uim_cmd_ptr != NULL ) {
     ...
     if ((gstk_status == GSTK_SUCCESS) && (uim_cmd_ptr != NULL)) {
       ...
       if (gstk_status == GSTK_SUCCESS)
       {
         GSTK_UTIL_SELECTIVE_DUMP_BYTE_ARRAY("Data Avail Env",
                                             &uim_cmd_ptr->envelope.data[uim_cmd_ptr->envelope.offset],
                                             uim_cmd_ptr->envelope.num_bytes);
   ...
   */
   ...
   if (uim_cmd_ptr != NULL ) {
     ...
     if ((gstk_status == GSTK_SUCCESS) && (uim_cmd_ptr != NULL)) {
       ...
       if (gstk_status == GSTK_SUCCESS)
       {
         GSTK_UTIL_SELECTIVE_DUMP_BYTE_ARRAY("Channel Status Env",
                                             &uim_cmd_ptr->envelope.data[uim_cmd_ptr->envelope.offset],
                                             uim_cmd_ptr->envelope.num_bytes);
...
)
{
  ...
  if (uim_cmd_ptr != NULL)
  {
    ...
    if ((gstk_status == GSTK_SUCCESS) && (uim_cmd_ptr != NULL)) {
      ...
      GSTK_UTIL_SELECTIVE_DUMP_BYTE_ARRAY("NW REJ EVT",
                                          &uim_cmd_ptr->envelope.data[uim_cmd_ptr->envelope.offset],
                                          uim_cmd_ptr->envelope.num_bytes);
  ...
  const gstk_envelope_access_tech_ind_type    *access_tech_evt_cmd)
{
   ...
   */
   ...
   if ((gstk_status == GSTK_SUCCESS) && (uim_cmd_ptr != NULL))
   {
     ...
     if (gstk_status == GSTK_SUCCESS)
     {
       GSTK_UTIL_SELECTIVE_DUMP_BYTE_ARRAY("Access Technology evt",
                                           &uim_cmd_ptr->envelope.data[uim_cmd_ptr->envelope.offset],
                                           uim_cmd_ptr->envelope.num_bytes);
       if(gstk_curr_inst_ptr->gstk_curr_envelope_info.retry_info.retry_flag == TRUE)
       {
...
)
{
  ...
  */
  ...
  if(uim_cmd_ptr != NULL )
  {
    ...
    if (gstk_status == GSTK_SUCCESS)
    {
      GSTK_UTIL_SELECTIVE_DUMP_BYTE_ARRAY("IMS Registration evt",
                                          &uim_cmd_ptr->envelope.data[uim_cmd_ptr->envelope.offset],
                                          uim_cmd_ptr->envelope.num_bytes);
    }
  ...
  return gstk_status;
}/* gstk_ims_registration_evt_ind */


/*===========================================================================
FUNCTION   gstk_contactless_state_req_evt_ind

DESCRIPTION
  This function builds the envelope of Contactless State request event

PARAMETERS
  contactless_state_req_evt_cmd: [Input] The struct of Contactless State request
...
SEE ALSO
  None
===========================================================================*/
gstk_status_enum_type gstk_contactless_state_req_evt_ind(
  gstk_envelope_contactless_state_req_evt_ind_type *contactless_state_req_cmd)
{
  gstk_status_enum_type                                    gstk_status         = GSTK_SUCCESS;
  uim_cmd_type*                                            uim_cmd_ptr         = NULL;
  int                                                      offset              = 0;
  gstk_envelope_contactless_state_req_evt_dl_command_type  STK_envelope_cmd;

  if (contactless_state_req_cmd == NULL)
  {
    return GSTK_BAD_PARAM;
  }
  ...
  /* Pack Contactless State Request TLV */
  gstk_status = gstk_packer_contactless_state_req_tlv(
     contactless_state_req_cmd->request,
  ...
  /* total data length:  evt id data + 2
                       + device id data + 2 (2 is for the tag and length field)
                       + contactless state request data + 2
  */
  STK_envelope_cmd.event_download_hdr.length =
                             STK_envelope_cmd.event_download_hdr.evt_list.length +
                             STK_envelope_cmd.event_download_hdr.device_id.device_tag_length +
                             STK_envelope_cmd.request.contactless_state_req_length +
                             6;
  ...
  if (uim_cmd_ptr != NULL ) {
    ...
    if ((gstk_status == GSTK_SUCCESS) && (uim_cmd_ptr != NULL)) {
      ...
      /* set user_data */
      uim_cmd_ptr->hdr.user_data               = contactless_state_req_cmd->message_header.user_data;
      ...
      /* copy contactless state req */
      if(gstk_byte_offset_memcpy(uim_cmd_ptr->envelope.data,
                                 &(STK_envelope_cmd.request),
                                 offset,
                                 (size_t)(STK_envelope_cmd.request.contactless_state_req_length +
                                          GSTK_TAG_LENGTH_LEN),
                                 sizeof(uim_cmd_ptr->envelope.data)-offset,
                                 sizeof(STK_envelope_cmd.request)) <
              (size_t)(STK_envelope_cmd.request.contactless_state_req_length + GSTK_TAG_LENGTH_LEN))
      {
        gstk_status = GSTK_MEMORY_ERROR;
      }
      ...
      if (gstk_status == GSTK_SUCCESS)
      {
        GSTK_UTIL_SELECTIVE_DUMP_BYTE_ARRAY("Contactless State Req Env",
                                            &uim_cmd_ptr->envelope.data[uim_cmd_ptr->envelope.offset],
                                            uim_cmd_ptr->envelope.num_bytes);
