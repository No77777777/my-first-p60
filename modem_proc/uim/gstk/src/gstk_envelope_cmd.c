...
FUNCTIONS
  ...
  gstk_build_and_queue_location_status_access_tech_evt_dl
    This function builds and queues the location status and access technology
    event download envelopes
...
$Header: //components/rel/uim.mpss/6.1.0/gstk/src/gstk_envelope_cmd.c#3 $

when       who     what, where, why
--------   ---     --------------------------------------------------------------------
01/02/20   sch     Added support for log masking
09/16/19   dt      Handling of GSTK_ACCESS_NONE
09/04/18   ks      Validate event_list_type
10/10/17   dt      End proactive session when MENU SELECTION command response is not 91XX
10/04/17   dt      Do not set cp_ack pending flag for SMS PP on 1x
07/12/17   vr      Fix memory leak when sending raw envelope failure response
01/26/17   shr     Disable CDMA loc status in non-US regions for specific operators
03/23/17   gs      Move Geo Loc Envelope to respective file
03/01/17   dt      Featurize IMSI_M and cdma_loc_st_disabled references
11/16/16   vr      Send error envelope response if failed to send to UIMDRV
...
06/15/16   shr     GSTK/ESTK globals memory optimization
...
04/29/16   gm      Fix compiler warning
04/13/16   vr      Send Cached TR only if the delay TR timer is running
03/07/16   shr     Added support for PCO Envelope
03/22/16   gs      Call Control mechanism reverted back
03/22/16   gs      Envelope commands moved to corresponding new file
03/14/16   gs      CBS feature controlled
11/30/15   vr      GSTK CC retry to be enabled only after the busy from card
11/09/15   vr      Reset usr_acty and idl_scrn for raw envelope rsp evt dl
10/09/15   gm      Converting to unsigned variables.
09/09/15   hh      Added support for informing MMGSDI about BIP and SMS-PP
08/19/15   gs      F3 reduction for the envelope data
07/17/15   shr     Validate destination buffer size
07/07/15   shr     Fix potential buffer overflow while building Envelope/TR
06/30/15   gm      Added PLMN TLV for CSG cell selection event download
06/29/15   gm      L4 compiler warning
...
02/20/15   gm      Moved CC command cache to heap
...
#define GSTK_IMS_URI_TRUNCATED_TLV_SIZE 2
...
#ifdef FEATURE_CDMA
  gstk_util_check_us_mcc(loc_info_cmd->mcc_mnc);
#endif /* FEATURE_CDMA */
  ...
  if (uim_cmd_ptr != NULL ) {
    ...
    if ((gstk_status == GSTK_SUCCESS) && (uim_cmd_ptr != NULL)) {
        ...
        if (gstk_status == GSTK_SUCCESS)
        {
          ...
          GSTK_UTIL_SELECTIVE_DUMP_BYTE_ARRAY("Env: Location Info Data",
  ...
  gstk_raw_envelope_ind_type * raw_env_cmd_ptr)
{
  ...
  if (raw_env_cmd_ptr->env_data_ptr[0] == GSTK_EVENT_DOWNLOAD_TAG)
  {
    ...
    if(event_list_type >= GSTK_MAX_EVENT_LIST_LEN)
    {
      UIM_MSG_ERR_0("gstk_raw_envelope_ind(): Event List type out of bound.");
      return GSTK_BAD_PARAM;
    }

    /* If the event is a Call event, modify TI list if needed */
    if((event_list_type == GSTK_MT_CALL) ||
       (event_list_type == GSTK_CALL_CONNECTED) ||
       (event_list_type == GSTK_CALL_DISCONNECTED))
    {
      gstk_transaction_id_type data;
      gstk_transaction_id_type modified_data;
      gstk_call_evt_info_type  call_evt_info;
      uint8                    dest_offset = 0;
      uint8                    src_offset  = 0;

      memset(&data, 0x00, sizeof(data));
      memset(&modified_data, 0x00, sizeof(modified_data));
      memset(&call_evt_info, 0x00, sizeof(call_evt_info));

      /* Skip to the Transaction Identifier mandatory TLV */
      offset = offset + 5;
      ...
      /* Bitmask AND with 0x7F to account for Transaction Identifier tag with comprehension bit */
      if ((raw_env_cmd_ptr->env_data_ptr[offset] & 0x7F) != GSTK_TRANSACTION_IDENTIFIER_TAG)
      {
        UIM_MSG_ERR_0("gstk_raw_envelope_ind(): missing mandatory Transaction Identifier List");
        return GSTK_BAD_PARAM;
      }

      /* Skip to Length byte of TI TLV */
      offset++;
      if(offset >= raw_env_cmd_ptr->env_len || offset >= UIM_MAX_ENVELOPE_BYTES)
      {
        UIM_MSG_ERR_0("gstk_raw_envelope_ind(): offset out of bound.");
      ...
      /* Copy the length of the TI value */
      data.length = raw_env_cmd_ptr->env_data_ptr[offset];

      /* If valid TI value exists, check and modify the same if needed */
      if(data.length > 0)
      {
        /* Skip to the first value byte of the TI TLV */
        offset++;
        if(offset >= raw_env_cmd_ptr->env_len || offset >= UIM_MAX_ENVELOPE_BYTES)
        {
          UIM_MSG_ERR_0("gstk_raw_envelope_ind(): offset out of bound.");
        ...
        /* Get a pointer to the TI value bytes */
        data.transaction_list = &raw_env_cmd_ptr->env_data_ptr[offset];

        /* For Raw envelope, the trigger is considered to be 3rd party IMS */
        call_evt_info.call_evt_trigger = GSTK_CALL_TRIGGER_IMS_EXT;

        /* Update Transaction ID list if needed */
        gstk_status = gstk_util_update_transaction_identifier_list(
                        &data,
                        &modified_data,
                        event_list_type,
                        call_evt_info);
        ...
        if(gstk_status != GSTK_SUCCESS)
        {
          if(modified_data.transaction_list != NULL)
          {
            gstk_free(modified_data.transaction_list);
          }
          return GSTK_BAD_PARAM;
        }

        /* Updated TI value list cannot have length greater than original TI value list */
        if(modified_data.length > data.length)
        {
          UIM_MSG_ERR_1("gstk_raw_envelope_ind(): modified_data.length invalid: 0x%x",
        ...
        /* Update the length field in the TI TLV */
        raw_env_cmd_ptr->env_data_ptr[offset - 1] = modified_data.length;

        /* Copy the modified TI value into the Raw envelope command */
        (void)memscpy(&raw_env_cmd_ptr->env_data_ptr[offset],
                      data.length,
                      modified_data.transaction_list,
                      modified_data.length);

        /* Logic to move the bytes after the TI TLV in case modified
           TLV length is smaller */
        dest_offset = (uint8)offset + modified_data.length;
        src_offset = (uint8)offset + data.length;

        memsmove(&raw_env_cmd_ptr->env_data_ptr[dest_offset],
                 raw_env_cmd_ptr->env_len - dest_offset,
                 &raw_env_cmd_ptr->env_data_ptr[src_offset],
                 raw_env_cmd_ptr->env_len - src_offset);

        /* Update the total length of Raw envelope command */
        if(modified_data.length < data.length)
        {
          raw_env_cmd_ptr->env_len =
            raw_env_cmd_ptr->env_len - (data.length - modified_data.length);
        }

        /* Update the Length field of the Envelope TLV - if Length field is
           1 byte, update the 1 byte field*/
        if (raw_env_cmd_ptr->env_data_ptr[1] != GSTK_2_BYTE_LENGTH_FIRST_VALUE)
        {
          raw_env_cmd_ptr->env_data_ptr[1] =
            raw_env_cmd_ptr->env_data_ptr[1] - (data.length - modified_data.length);
        }
        /* If the Length field of the Envelope TLV is of 2 bytes, convert
           it to 1 byte if needed */
        else
        {
          uint8 old_data_len = 0;
          uint8 new_data_len = 0;

          /* Old Envelope TLV length */
          old_data_len = raw_env_cmd_ptr->env_data_ptr[2];
          /* New envelope TLV length */
          new_data_len = old_data_len - (data.length - modified_data.length);

          /* If new length is also to be represented in 2 bytes */
          if(new_data_len > GSTK_TLV_LENGTH_1_BYTE_OFFSET_LIMIT)
          {
             /* Update the Length field of the Envelope TLV */
            raw_env_cmd_ptr->env_data_ptr[2] = new_data_len;
          }
          /* If new length is to be represented in 1 byte */
          else
          {
             /* Update the Length field of the Envelope TLV */
            raw_env_cmd_ptr->env_data_ptr[1] = new_data_len;

            /* Logic to move the bytes after the Envelope length in case modified
               length field is smaller */
            memsmove(&raw_env_cmd_ptr->env_data_ptr[2],
                     raw_env_cmd_ptr->env_len - 2,
                     &raw_env_cmd_ptr->env_data_ptr[3],
                     raw_env_cmd_ptr->env_len - 3);

            /* Update the total length of Raw envelope command */
            raw_env_cmd_ptr->env_len =
              raw_env_cmd_ptr->env_len - 1;
          }
        }
        if(modified_data.transaction_list != NULL)
        {
  ...
  if (uim_cmd_ptr != NULL )
  {
    ...
    if ((gstk_status == GSTK_SUCCESS) && (uim_cmd_ptr != NULL))
    {
      ...
      if(gstk_memcpy(uim_cmd_ptr->envelope.data,
         ...
         raw_env_cmd_ptr->env_len)
      {
        UIM_MSG_ERR_0("gstk_raw_envelope_ind(): gstk_memcpy failed");
        gstk_status = GSTK_MEMORY_ERROR;
      }
   ...
   if (uim_cmd_ptr != NULL ) {
     ...
     if ((gstk_status == GSTK_SUCCESS) && (uim_cmd_ptr != NULL)) {
	  ...
	  offset = (int)(offset + STK_envelope_cmd.device_id.device_tag_length +
         ...
         GSTK_UTIL_SELECTIVE_DUMP_BYTE_ARRAY("Env: SMS PP Data",
                                             &uim_cmd_ptr->envelope.data[uim_cmd_ptr->envelope.offset], 
                                             uim_cmd_ptr->envelope.num_bytes);
...
)
{
  ...
  /* If RAT is WLAN or NONE, no location information*/
  if (rat == GSTK_ACCESS_TECH_WLAN || rat == GSTK_ACCESS_NONE)
  {
    UIM_MSG_HIGH_1("RAT is 0x%x, set location info to all 0xFF and cell_id_len to 0", rat);
      ...
      else
      {
                                     ...
                                     &temp_cell_info) == GSTK_SUCCESS)
        {
          ...
          if (gstk_memcpy(loc_info_rsp_ptr->cell_id,
              ...
              (size_t)temp_cell_info.cell_len)
          {
            return GSTK_MEMORY_ERROR;
          }
        }
          ...
          gstk_envelope_cc_ind_type* cc_cmd)
{
      ...
      cc_cmd->cc_data.call_enum_type == GSTK_IMS_CALL)
  {
    ...
    boolean                      ims_uri_truncated = FALSE;
    ...
    /* Copy the gsm umts location info in case of PDP, EPS_PDP and IMS CC envelope */
    ...
    {
      int32  location_status_size = 0;
      int32  ims_cc_size          = 0;

      /* No location TLV for WLAN or RAT as None */
      if (cc_cmd->cc_data.rat != GSTK_ACCESS_TECH_WLAN &&
          cc_cmd->cc_data.rat != GSTK_ACCESS_NONE)
      {
         (void)gstk_util_find_tlv_len(
           GSTK_LOCATION_INFORMATION_TAG,
           (gstk_cmd_to_uim_type*)&gen_location_info,
           &location_status_size,
           GSTK_ENVELOPE_CMD);
         location_status_size += 2;
      }

      /* Calculate the IMS CC envelope total length */
      ims_cc_size = offset + location_status_size + cc_cmd->cc_data.gstk_address_string.ims_call.length + 2;
      if (cc_cmd->cc_data.gstk_address_string.ims_call.length > GSTK_TLV_LENGTH_1_BYTE_OFFSET_LIMIT)
      {
        ims_cc_size++;
      }

      /* Total IMS is greater than MAX envelope bytes (255), remove the extra bytes by truncating
         the ims uri */
      if (ims_cc_size > UIM_MAX_ENVELOPE_BYTES)
      {
        /* Truncate extra bytes + IMS URI TRUNCATE TLV size */
        cc_cmd->cc_data.gstk_address_string.ims_call.length -=
          (ims_cc_size - UIM_MAX_ENVELOPE_BYTES) + GSTK_IMS_URI_TRUNCATED_TLV_SIZE;

        ims_uri_truncated = TRUE;
      }
    ...
    if (cc_cmd->cc_data.call_enum_type == GSTK_IMS_CALL &&
        (cc_cmd->cc_data.gstk_address_string.ims_call.ims_uri_truncated ||
         ims_uri_truncated))
    {
      uim_cmd_ptr->envelope.data[offset++] = GSTK_IMS_URI_TRUNCATED_TAG;
      uim_cmd_ptr->envelope.data[offset++] = 0;
    }
    ...
    GSTK_UTIL_SELECTIVE_DUMP_BYTE_ARRAY("Env: CC Data",
                                        &uim_cmd_ptr->envelope.data[uim_cmd_ptr->envelope.offset], 
                                        uim_cmd_ptr->envelope.num_bytes);

    /* Update env_cmd_retry only if CC command is requeued */
    if(cc_cmd->cc_requeue)
    {
      gstk_status = gstk_process_and_store_env_cmd_for_retry(
                     uim_cmd_ptr->envelope.data,
                     uim_cmd_ptr->envelope.num_bytes,
  ...
  else
  {
    ...
    switch(rat)
    {
      ...
      case GSTK_ACCESS_TECH_WLAN:
        ...
        break;
      case GSTK_ACCESS_NONE:
        UIM_MSG_HIGH_0("RAT type is NONE");
        break;
      default:
        UIM_MSG_ERR_0("Invalid RAT");
        gstk_cc_ind_cleanup(cc_cmd, &STK_envelope_cmd);
    ...
    if (uim_cmd_ptr != NULL )
    {
      ...
      if ((gstk_status == GSTK_SUCCESS) && (uim_cmd_ptr != NULL))
      {
        ...
        else if (rat != GSTK_ACCESS_TECH_WLAN && rat != GSTK_ACCESS_NONE)
        /* copy location info */
        /* tag */
        {
          if(offset < UIM_MAX_ENVELOPE_BYTES)
  ...
  rat = mo_sms_ctrl_cmd->mo_sms_ctrl_data.rat;
   ...
   if (uim_cmd_ptr != NULL ) {
     ...
     if ((gstk_status == GSTK_SUCCESS) && (uim_cmd_ptr != NULL)) {
       ...
       GSTK_UTIL_SELECTIVE_DUMP_BYTE_ARRAY("Env: MO SMS control Data",
                                           &uim_cmd_ptr->envelope.data[uim_cmd_ptr->envelope.offset], 
                                           uim_cmd_ptr->envelope.num_bytes);

       /* Update env_cmd_retry only if mo sms command is requeued */
       if(mo_sms_ctrl_cmd->mo_sms_requeue)
       {
         gstk_status = gstk_process_and_store_env_cmd_for_retry(
                        uim_cmd_ptr->envelope.data,
                        uim_cmd_ptr->envelope.num_bytes,
...
)
{
   ...
   */
   ...
   GSTK_UTIL_SELECTIVE_DUMP_BYTE_ARRAY("Env: Network Search Mode evt",
                                       &uim_cmd_ptr->envelope.data[uim_cmd_ptr->envelope.offset], 
                                       uim_cmd_ptr->envelope.num_bytes);
   ...
   */
   ...
   if (uim_cmd_ptr != NULL ) {
     ...
     if ((gstk_status == GSTK_SUCCESS) && (uim_cmd_ptr != NULL)) {
       ...
       if (gstk_status == GSTK_SUCCESS)
       {
         GSTK_UTIL_SELECTIVE_DUMP_BYTE_ARRAY("Env: Timer Expire Data",
                                             &uim_cmd_ptr->envelope.data[uim_cmd_ptr->envelope.offset], 
                                             uim_cmd_ptr->envelope.num_bytes);
  ...
  if (uim_cmd_ptr != NULL )
  {
    ...
    if ((gstk_status == GSTK_SUCCESS) && (uim_cmd_ptr != NULL))
    {
      ...
      if (gstk_status == GSTK_SUCCESS)
      {
        ...
        GSTK_UTIL_SELECTIVE_DUMP_BYTE_ARRAY("Env: PCO Info Data",
                                            &uim_cmd_ptr->envelope.data[uim_cmd_ptr->envelope.offset], 
                                            uim_cmd_ptr->envelope.num_bytes);
...
)
{
  ...
  switch( type_of_command )
  {
    ...
    case GSTK_RAW_ENVELOPE_IND:
      ...
      gstk_raw_envelope_ind_cleanup(&(cmd->raw_envelope_ind));
    ...
    case GSTK_SMS_PP_DOWNLOAD_IND:
      ...
      {
        /* store if received SMS PP is over 1x or not to check later while enabling is_cp_ack_pending flag */
        gstk_shared_data.envelope_cmd_ref_table[env_ref_id].is_cdma_sms_pp = 
          cmd->sms_pp_envelope_ind.is_cdma_sms_pp;
    ...
    case GSTK_MT_CALL_EVT_IND:
      ...
      /* Release any memory allocated within the command pointer */
      gstk_util_release_upper_layer_cmd_memory(cmd);
    ...
    case GSTK_MO_SMS_CTRL_IND:
      ...
      if(error_env_flg == FALSE)
      {
        /* Cache the MO SMS indication data */
        if(gstk_shared_data.envelope_cmd_ref_table[env_ref_id].mo_sms_cache_ptr == NULL)
        {
          gstk_shared_data.envelope_cmd_ref_table[env_ref_id].mo_sms_cache_ptr =
             GSTK_CALLOC(sizeof(gstk_mo_sms_ctrl_ind_ext_type));
        }
        if(gstk_shared_data.envelope_cmd_ref_table[env_ref_id].mo_sms_cache_ptr != NULL)
        {
          gstk_memcpy(gstk_shared_data.envelope_cmd_ref_table[env_ref_id].mo_sms_cache_ptr,
                     &(cmd->mo_sms_ctrl_envelope_ind.mo_sms_ctrl_data),
                     sizeof(gstk_mo_sms_ctrl_ind_ext_type),
                     sizeof(gstk_mo_sms_ctrl_ind_ext_type),
                     sizeof(cmd->mo_sms_ctrl_envelope_ind.mo_sms_ctrl_data));
        }
    ...
    case GSTK_CC_IND:
      ...
      {
         /* Pending Response and TR is received: Send the TR first so that we can process CC
            only if terminal_rsp_delay_timer is running */
      ...
      if(error_env_flg == FALSE)
      {
        if(gstk_shared_data.envelope_cmd_ref_table[env_ref_id].gstk_cc_cache_ptr == NULL)
        {
          gstk_shared_data.envelope_cmd_ref_table[env_ref_id].gstk_cc_cache_ptr =
             GSTK_CALLOC(sizeof(gstk_cc_ind_type_ext));
        }
        if(gstk_shared_data.envelope_cmd_ref_table[env_ref_id].gstk_cc_cache_ptr != NULL)
        {
          /* cache CC info */
          gstk_memcpy(gstk_shared_data.envelope_cmd_ref_table[env_ref_id].gstk_cc_cache_ptr,
                     &(cmd->cc_envelope_ind.cc_data),
                     sizeof(gstk_cc_ind_type_ext),
                     sizeof(gstk_cc_ind_type_ext),
        ...
        else
        {
          ...
          else
          {
            ...
            else
            {
              ...
              /* If there is a pending Loc Info Envelope retry or if the last
                 Loc Info Envelope sent does not match the current request,
                 queue a new Envelope */
              if ((gstk_util_check_and_clear_pending_loc_info_retry() == TRUE) ||
                  (memcmp(gstk_curr_inst_ptr->gstk_last_loc_info_sent,
                         gstk_curr_inst_ptr->attach_ctrl_p->mcc_mnc,
                          GSTK_MAX_PLMN_LEN) != 0))
              {
                UIM_MSG_HIGH_0("Pending Loc Info retry or PLMN does not match with previous Env");
    ...
    case GSTK_CONTACTLESS_STATE_REQ_EVT_IND:
      UIM_MSG_HIGH_1("Receive Contactless State Req event: 0x%x",
                     gstk_curr_inst_ptr->gstk_evt_list[GSTK_CONTACTLESS_STATE_REQ]);

      if(nv_data.cat_version >= GSTK_CFG_CAT_VER9)
      {
        if(error_env_flg == FALSE)
        {
          gstk_status = gstk_contactless_state_req_evt_ind(&(cmd->contactless_state_req_envelope_ind));
        }
        if((error_env_flg == TRUE) || (gstk_status != GSTK_SUCCESS))
        {
          gstk_status = gstk_build_error_envelope_report(cmd->contactless_state_req_envelope_ind.message_header.user_data,
                                                         cmd->contactless_state_req_envelope_ind.message_header.command_id,
                                                         GSTK_ENVELOPE_CMD_FAIL);
        }
      }
      else
      {
        UIM_MSG_HIGH_1("GSTK_CONTACTLESS_STATE_REQ_EVT_IND : CAT version is pre-VER9 - 0x%x",
                       nv_data.cat_version);
      }
      break;
    case GSTK_GEO_LOC_REPORT_IND:
      UIM_MSG_HIGH_0(" Rec'd GSTK_GEO_LOC_REPORT_IND");
      if(error_env_flg == FALSE)
      {
        gstk_status = gstk_geo_loc_report_ind(&(cmd->geo_loc_report));
      }
      if((error_env_flg == TRUE) || (gstk_status != GSTK_SUCCESS))
      {
        gstk_status = gstk_build_error_envelope_report(cmd->geo_loc_report.message_header.user_data,
                                                       cmd->geo_loc_report.message_header.command_id,
                                                       GSTK_ENVELOPE_CMD_FAIL);
      }
      break;
...
)
{
  ...
  /* timer info */
  task_cmd->cmd.access_tech_evt_envelope_ind.access_tech = (uint8)access_tech;
...
)
{
 ...
 /* Validate Slot ID */
  GSTK_RETURN_ERROR_IF_NULL_PTR(gstk_instances_ptr[(uint32)slot_id - 1], GSTK_ERROR);
  ...
  if((gstk_instances_ptr[(uint32)slot_id - 1]->pco_info.is_pco_sent == TRUE) &&
  ...
  gstk_instances_ptr[(uint32)slot_id - 1]->pco_info.is_pco_sent = FALSE;
...
void gstk_send_cached_terminal_rsp(unsigned long instance)
{
  ...
  if (uim_cmd_ptr != NULL )
  {
    ...
    if((gstk_status == GSTK_SUCCESS) && (uim_cmd_ptr != NULL))
    {
      ...
      /* Set up log masking */
      gstk_logging_populate_masking_fn_ptr(uim_cmd_ptr);
...
/*===========================================================================
FUNCTION gstk_build_and_queue_location_status_access_tech_evt_dl
...
PARAMETERS
  send_loc_status_evt : [Input] boolean to send the location status or not
  send_acc_tech_evt   : [Input] boolean to send the access technology or not
  rat                 : [Input] Access technology info
...
SEE ALSO
  None
===========================================================================*/
gstk_status_enum_type gstk_build_and_queue_location_status_access_tech_evt_dl(
  boolean                          send_loc_status_evt,
  boolean                          send_acc_tech_evt,
  gstk_access_technology_type      rat,
  gstk_location_status_enum_type   location_status
)
{
  ...
  /* Check VZW location status flag, the order of queuing, VZW feature is
     enabled then first location status then access technology. Otherwise,
     first access technology then location status. These envelope commands
     queued to front of the queue */
  if ((gstk_nv_get_feature_status(
                  GSTK_CFG_FEATURE_SLOT_PROP_LOC_STATUS,
                  gstk_curr_inst_ptr->slot_id) == TRUE))
  {
    if (send_acc_tech_evt)
    {
      gstk_status = gstk_build_access_technology_envelope(rat);
    }
    if (send_loc_status_evt)
    {
