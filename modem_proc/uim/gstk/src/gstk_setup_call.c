...
Copyright (c) 2003-2010, 2012-2017 QUALCOMM Technologies, Inc (QTI) and its licensors.
...
$Header: //components/rel/uim.mpss/6.1.0/gstk/src/gstk_setup_call.c#2 $
  ...
  boolean                                           has_ussd)
{
 ...
 /* free the text string byte pointer from STK_response_ptr and setup_call_cnf_ptr */
  ...
  gstk_util_release_upper_layer_cmd_memory(
    (gstk_cmd_type*)setup_call_cnf_ptr);
  ...
  if (uim_cmd_ptr != NULL ) {
    ...
    if (gstk_status == GSTK_SUCCESS) {
      if (uim_cmd_ptr != NULL) {
        ...
        if (gstk_status == GSTK_SUCCESS)
        {
          GSTK_UTIL_SELECTIVE_DUMP_BYTE_ARRAY("SET UP CALL TR",
