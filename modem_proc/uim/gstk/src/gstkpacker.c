...
Copyright (c) 2003-2017, 2024 QUALCOMM Technologies, Inc (QTI) and its licensors.
...
$Header: //components/rel/uim.mpss/6.1.0/gstk/src/gstkpacker.c#3 $

when       who     what, where, why
--------   ---     --------------------------------------------------------------------
09/24/24   ac      Support change in VZW MSISDN format
03/01/17   dt      Featurize IMSI_M and cdma_loc_st_disabled references
09/29/16   shr     Remove TON/NPI for PCO Envelope and from incoming MSISDN
...
04/29/16   gm      Removal of CSG feature for IOE devices
03/07/16   shr     Added support for PCO Envelope
12/24/15   gs      KW error fix
...
02/27/14   dy      Added support for class "r" commands
    ...
    */
    ...
    if (index  < GSTK_MAX_PROACTIVE_COMMAND) { /*ref_id within range */
        if (command_details_ref_table_ptr[index].free_slot != TRUE ) { /* this location is still used*/
          if ((uint32)command_details_ref_table_ptr[index].expected_cmd_rsp_type == command_rsp_type) {
       ...
       gstk_call_evt_info_type           call_evt_info)
{
  ...
  /* Check if the Call event is requested by the Card */
  if((gstk_curr_inst_ptr->gstk_evt_list[call_evt] & 0x01) != 0x01)
  {
    UIM_MSG_ERR_1("Evt is not set: 0x%x",call_evt);
  ...
  /* Copy the Transaction ID TLV */
  transaction_id_tlv->tag = (int)GSTK_TRANSACTION_IDENTIFIER_TAG;
  transaction_id_tlv->length = modified_data.length;
...
SEE ALSO
  None
===========================================================================*/
gstk_status_enum_type gstk_packer_nmea_sentence_tlv(
       const gstk_nmea_sentence_type            *data,
       gstk_nmea_tlv_type                       *nmea_tlv)
{
  UIM_MSG_HIGH_0("** Packing nmea sentence");

  if(data == NULL || nmea_tlv == NULL)
  {
     UIM_MSG_ERR_0("Data Err: NULL");
     return GSTK_BAD_PARAM;
  }

  nmea_tlv->tag = (int)GSTK_NMEA_SENTENCE_TAG;
  nmea_tlv->length = data->length;

  if(gstk_memcpy(nmea_tlv->nmea,
                 data->nmea_ptr,
                 data->length,
                 sizeof(nmea_tlv->nmea),
                 data->length) <
     data->length)
  {
    UIM_MSG_ERR_0("gstk_packer_nmea_sentence_tlv(): gstk_memcpy failed");
    return GSTK_MEMORY_ERROR;
  }
  ...
  switch(bearer_description->bearer_type) 
  {
    ...
    case GSTK_EUTRAN_EXT_PARAM:
      ...
      if (bearer_description_tlv->bearer_param)
      {
        ...
        if(bearer_description->bearer_params.eutran_ext_bearer.max_bitrate_ul > 0xFFFF)
        {
          val = 0xFFFF;
        }
        else
        ...
        if(bearer_description->bearer_params.eutran_ext_bearer.max_bitrate_dl > 0xFFFF)
        {
          val = 0xFFFF;
        }
        else
        ...
        bearer_description_tlv->bearer_param[4] =
          uint16touint8((val & 0xFF00) >> 8);
        if(bearer_description->bearer_params.eutran_ext_bearer.guaranteed_bitrate_ul > 0xFFFF)
        {
          val = 0xFFFF;
        }
        else
        ...
        bearer_description_tlv->bearer_param[6] =
          uint16touint8((val & 0xFF00) >> 8);
        if(bearer_description->bearer_params.eutran_ext_bearer.guaranteed_bitrate_dl > 0xFFFF)
        {
          val = 0xFFFF;
        }
        else
        ...
        bearer_description_tlv->bearer_param[16] = GSTK_PKT_DATA_IP;
      }
      else
      {
        gstk_status = GSTK_MEMORY_ERROR;
    ...
    case GSTK_EUTRAN_MAPPED_UTRAN_PS:
      ...
      if(bearer_description->bearer_params.eutran_mapped_utran_ps_bearer.qci <= 4)
      {
        switch(bearer_description->bearer_params.eutran_mapped_utran_ps_bearer.data_rate_format)
        {
          case GSTK_DATA_RATE_FORMAT_MIN_MAX_TYPE:
            bearer_param_len = GSTK_EUTRAN_MAPPED_UTRAN_PS_PARAM_LEN;
            break;
          case GSTK_DATA_RATE_FORMAT_MIN_MAX_EX_TYPE:
            bearer_param_len = GSTK_EUTRAN_MAPPED_UTRAN_PS_PARAM_EX_LEN;
            break;
          default:
            bearer_param_len = 2;
            break;
      ...
      if (bearer_description_tlv->bearer_param != NULL)
      {
        gstk_status = GSTK_SUCCESS;
        bearer_description_tlv->bearer_param[0] =
          bearer_description->bearer_params.eutran_mapped_utran_ps_bearer.qci;
        ...
        bearer_description_tlv->bearer_param[bearer_param_len - 1] = GSTK_PKT_DATA_IP;
      }
      else
      {
        gstk_status = GSTK_MEMORY_ERROR;
         ...
         gstk_buffer_size_tlv_type    *buffer_size_tlv)
{
  ...
  buffer_size_tlv->buffer_size[0] = (uint8)((buffer_size >> 8) & 0xFF);
         ...
         gstk_ch_data_tlv_type        *ch_data_tlv)
{
  ...
  ch_data_tlv->ch_data_length = uint32touint8(ch_data.data_len);
...
)
{
  ...
  if(dest_size < GSTK_DEVICE_IDENTITY_LEN)
  {
    UIM_MSG_ERR_1("gstk_packer_dev_id_card_tlv(): invalid destination size: 0x%x",
                  dest_size);
    return GSTK_MEMORY_ERROR;
  }
...
)
{
  ...
  if (location_info_orig_ptr->location_info_tech == GSTK_GSM_UMTS_LOCATION_INFO)
  {
    ...
    if(gstk_memcpy(
         ...
         location_info_orig_ptr->location_info.gsm_umts_loc_info.cell_id_len,    /*copy_size */
         new_dest_size,           /* dest max size */
         sizeof(location_info_orig_ptr->location_info.gsm_umts_loc_info.cell_id)) < /* src max size */
       location_info_orig_ptr->location_info.gsm_umts_loc_info.cell_id_len)
    {
      return GSTK_MEMORY_ERROR;
    }
...
)
{
  ...
  if(gstk_memcpy(
       ...
       pdp_context_act_param_orig_ptr->length, /*copy_size */
       int32touint32(dest_size),     /* dest max size */
       pdp_context_act_param_orig_ptr->length) < /* src max size */
     pdp_context_act_param_orig_ptr->length)
  {
    UIM_MSG_ERR_0("gstk_packer_pdp_context_act_param_card_tlv(): gstk_memcpy failed");
    return GSTK_MEMORY_ERROR;
  }
...
)
{
  ...
  if(gstk_memcpy(
       ...
       pdn_conn_act_param_orig_ptr->length, /*copy_size */
       int32touint32(dest_size),     /* dest max size */
       pdn_conn_act_param_orig_ptr->length) < /* src max size */
     pdn_conn_act_param_orig_ptr->length)
  {
    UIM_MSG_ERR_0("gstk_packer_pdn_conn_act_param_card_tlv(): gstk_memcpy failed");
    return GSTK_MEMORY_ERROR;
  }
...
)
{
  ...
  if(gstk_memcpy(
       ...
       ims_call_param_orig_ptr->length, /*copy_size */
       int32touint32(dest_size),     /* dest max size */
       ims_call_param_orig_ptr->length) < /* src max size */
     ims_call_param_orig_ptr->length)
  {
    UIM_MSG_ERR_0("gstk_packer_ims_call_param_card_tlv(): gstk_memcpy failed");
    return GSTK_MEMORY_ERROR;
  }
...
)
{
    ...
    if(dest_size < GSTK_COMMAND_DETAILS_LEN)
    {
      UIM_MSG_ERR_1("gstk_packer_cmd_details_card_tlv(): invalid destination size: 0x%x",
                    dest_size);
      return GSTK_MEMORY_ERROR;
    }
    ...
    */
    ...
    if (index < GSTK_MAX_PROACTIVE_COMMAND) { /*ref_id within range */
        if (command_details_ref_table_ptr[index].free_slot != TRUE ) { /* this location is still used*/
          if ((uint32)command_details_ref_table_ptr[index].expected_cmd_rsp_type == cmd_info.command_rsp_type) {
              /* command matches -> valid ref_id */
              if (command_details_ref_table_ptr[index].command_details.command_number !=
...
)
{
  ...
  if(dest_size < GSTK_GEN_RESULT_LEN + result_info.result_additional_info.length)
  {
    UIM_MSG_ERR_1("gstk_packer_result_card_tlv(): invalid destination size: 0x%x",
                  dest_size);
    return GSTK_MEMORY_ERROR;
  }
...
)
{
  ...
  if(dest_size < GSTK_NW_SEARCH_MODE_LEN)
  {
    UIM_MSG_ERR_1("gstk_packer_nw_search_mode_card_tlv(): invalid destination size: 0x%x",
                  dest_size);
    return GSTK_MEMORY_ERROR;
  }
...
)
{
  ...
  if(dest_size < GSTK_EVENT_LIST_LEN)
  {
    UIM_MSG_ERR_1("gstk_packer_evt_list_card_tlv(): invalid destination size: 0x%x",
                  dest_size);
    return GSTK_MEMORY_ERROR;
  }
...
)
{
  ...
  dns_addr_tlv->length = (uint8)dns_addr_ptr->length + 1; /* 1 extra byte for address type */
...
SEE ALSO
  None
===========================================================================*/
gstk_status_enum_type gstk_packer_contactless_state_req_tlv(
  gstk_contactless_state_req_enum_type  request,
  gstk_contactless_state_req_tlv_type  *contactless_state_req_tlv
)
{
  UIM_MSG_HIGH_0("** Packing Contactless State Request");

  if(contactless_state_req_tlv == NULL)
  {
    UIM_MSG_ERR_0("contactless_state_req_tlv is NULL");
    return GSTK_BAD_PARAM;
  }

  contactless_state_req_tlv->tag                           = (uint8)GSTK_CONTACTLESS_STATE_REQ_TAG;
  contactless_state_req_tlv->contactless_state_req_length  = GSTK_CONTACTLESS_STATE_REQ_LEN;
  contactless_state_req_tlv->contactless_state_req         = (uint8)request;
...
)
{
  ...
  if((msisdn_info_ptr->msisdn_length) > GSTK_MSISDN_LEN)
  {
    copy_len = GSTK_MSISDN_LEN;
  }
  else
  {
    copy_len = msisdn_info_ptr->msisdn_length;
  }
