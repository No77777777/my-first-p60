...
Copyright (c) 2003-2008, 2010, 2014 - 2016 QUALCOMM Technologies, Inc (QTI) and
...
$Header: //components/rel/uim.mpss/6.1.0/gstk/src/gstk_more_time.c#2 $
  ...
  more_time_terminal_response_type  *STK_response_ptr)
{
  ...
  gstk_util_release_upper_layer_cmd_memory(
    (gstk_cmd_type*)more_time_cnf_ptr);
} /* gstk_more_time_cnf_cleanup */
  ...
  if (uim_cmd_ptr != NULL ) {
    ...
    if (gstk_status == GSTK_SUCCESS) {
      if (uim_cmd_ptr != NULL) {
        ...
        if (gstk_status == GSTK_SUCCESS)
        {
          GSTK_UTIL_SELECTIVE_DUMP_BYTE_ARRAY("More Time TR",
