...
Copyright (c) 2001-2017, 2024 QUALCOMM Technologies, Inc (QTI) and its licensors.
...
$Header: //components/rel/uim.mpss/6.1.0/gstk/src/gstk_refresh.c#3 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
04/10/24   ac      Make file list TLV optional for buggy card
09/18/17   shr     Parse File List TLV only for appropriate modes
    ...
    refresh_terminal_response_type *STK_response)
{
  ...
  /* free refresh tlv */
  gstk_util_release_upper_layer_cmd_memory((gstk_cmd_type*)refresh_cnf);
...
gstk_status_enum_type gstk_refresh_req (gstk_proactive_cmd_type* command)
{
  ...
  */
                ...
                expected_tag == GSTK_ALPHA_IDENTIFIER_TAG)
            {
              ...
              if (gstk_status != GSTK_SUCCESS) 
              {
                if(comprehension_flag)
                {
                 /* Send Terminal Response to Card indicating
                     COMMAND_DATA_NOT_UNDERSTOOD_BY_ME */
                  UIM_MSG_ERR_0("REFRESH TLV parsing errors for GSTK_ALPHA_IDENTIFIER_TAG");
                  gstk_status = GSTK_INVALID_COMMAND;
                  break;
                }/* if(comprehension_flag)*/
              }
  ...
  /* Treat file list as optional for buggy cards if refresh mode is 3g session reset */
  if ((refresh_mode == GSTK_REFRESH_3G_SESSION_RESET) &&
       gstk_nv_get_feature_2_status(GSTK_CFG_FEATURE_2_SLOT_REFRESH_FILE_LIST_OPTIONAL,
                                    gstk_curr_inst_ptr->slot_id))
  {
    UIM_MSG_HIGH_0("File List is treated as optional for 3g session rest if card is buggy.");
    file_list_tag_needed = FALSE;
  }
...
)
{
  ...
  UIM_MSG_HIGH_0("IN GSTK REFRESH CNF ");
  ...
  if (uim_cmd_ptr != NULL )
  {
    ...
    if ((gstk_status == GSTK_SUCCESS) && (uim_cmd_ptr != NULL))
    {
      ...
      */
      ...
      if (gstk_status == GSTK_SUCCESS)
      {
        GSTK_UTIL_SELECTIVE_DUMP_BYTE_ARRAY("Refresh TR",
...
)
{
  ...
  UIM_MSG_HIGH_1("IN GSTK / MMGSDI REFRESH CNF mmgsdi_status=0x%x", mmgsdi_status);
  ...
  uint32                         cmd_details_ref_id)
{
  ...
  {
    /* num_of_files + refresh_files */
    refresh_files.data_len = file_list_ptr->length + 1;
    if((file_list_ptr->length == 0) || (file_list_ptr->file_paths == NULL))
    {
      UIM_MSG_ERR_0("File List is empty");
      return GSTK_ERROR;
    }
    refresh_files.data_ptr = (uint8*)GSTK_CALLOC(sizeof(uint8) * refresh_files.data_len);
    if(refresh_files.data_ptr == NULL)
    {
      return GSTK_MEMORY_ERROR;
    }
    refresh_files.data_ptr[0] = file_list_ptr->num_of_files;
    if(gstk_memcpy((void*)&refresh_files.data_ptr[1],
                   (void*)file_list_ptr->file_paths,
                   sizeof(uint8) * file_list_ptr->length,
                   sizeof(uint8) * ((int32touint32(refresh_files.data_len) - 1)),
                   sizeof(uint8) * file_list_ptr->length) <
       (sizeof(uint8) * file_list_ptr->length))
    {
  ...
  if(plmn_list_ptr->data_len > 0)
  {
    ...
    if(gstk_memcpy((void*)plmn_list.data_ptr,
                   ...
                   sizeof(uint8) * plmn_list.data_len,
                   sizeof(uint8) * plmn_list_ptr->data_len) <
       (sizeof(uint8) * plmn_list_ptr->data_len))
    {
      gstk_free(refresh_files.data_ptr);
