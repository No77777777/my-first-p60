...
Copyright (c) 2014 - 2020, 2022, 2024 QUALCOMM Technologies, Inc (QTI) and its licensors.
...
$Header: //components/rel/uim.mpss/6.1.0/gstk/src/gstk_nv.c#6 $$ $DateTime: 2024/04/15 00:23:49 $

when       who      what, where, why
--------   ---    --------------------------------------------------------------------
04/10/24   ac      Make file list TLV optional for buggy card
11/08/22   ac      Disables GSTK response timer for specific slot
03/18/20   ks      Reject 3GPP2-SMS if transport type is IMS
01/24/20   ks      Add NV GSTK_CFG_FEATURE_ME_ENABLE_QMI_LOC_NOTIFY_SUPPORT
10/25/18   ks      Add GSTK_CFG_FEATURE_2_SLOT_DISABLE_TP_CSG
12/28/17   dt      GSTK F3 reduction
09/25/17   shr     Add slot specific NV for IMS reg. event support in TP
09/22/17   shr     Revert support for BIP APN over-ride
09/04/17   dt      Assign data card and ignore alpha NVs to ME specific NV pointer
08/08/17   shr     Update Envelope Retry flag per slot
08/08/17   shr     Add support for correctly handling VPE in multi-SIM scenario
08/07/17   dt      Allow default values to be updated if NV 73760 is inactive
06/28/17   dt      Hold SEND SMS till CP ACK for SMS PP is received
04/25/17   dt      Enable Data Card Config. if QMI CAT is not supported
04/17/17   shr     Removed GSTK_CFG_FEATURE_2_SLOT_DISABLE_ENH_RAT_AND_LOC_EVT enablement
...
01/24/17   gs      Support of Dual Data Capability
10/07/16   shr     GSTK/ESTK F3 reduction
09/23/16   gs      F3 reduction / cleanup
07/13/16   vr      Added feature to Disable CDMA location
06/20/16   shr     Change default CAT version to 12
...
11/30/15   vr      Enabled GSTK_CFG_FEATURE_2_SLOT_DISABLE_ENH_RAT_AND_LOC_EVT for GCF
11/30/15   vr      Removed GSTK_CFG_FEATURE_SLOT_DISABLE_CALL_AND_SMS_CTRL_RETRY
09/24/15   shr     Resolve KW warnings
09/15/15   shr     Reduce F3 logging
07/07/15   shr     Fix incorect typecasting of NV index
06/30/15   vr      Update rat priority list with default if NV 72585 is incorrect
06/25/15   gm      L4 compiler warning
06/08/15   shr     Add support for updated UIM_COMMON_EFS_DEVICE value
...
04/28/15   vr      Ignore GSTK_CFG_FEATURE_ME_ESTK_QMI_SEND_NO_AND_NULL_ALPHA_IND for GCF
04/27/15   shr     Allow DS to chose 3GPP/3GPP2 profiles when APN cannot be determined
04/24/15   dy      Make NV cache read print less lines
04/02/15   vr      Non UI and non NW proactive command parallel processing
03/12/15   vr      Enable GSTK_CFG_FEATURE_SETUP_CALL_REL7 if cat version REL7 and above
...
/*===========================================================================
  STRUCT: GSTK_NV_CSIM_PRESENT_DATA_TYPE
==========================================================================*/
typedef struct
{
  boolean                       value;
...
gstk_nv_csim_present_data_type;
...
/*===========================================================================
  STRUCT: GSTK_NV_SMS_LINK_CTRL_TIMER_TYPE
==========================================================================*/
typedef struct
{
  uint8                         value;
...
gstk_nv_sms_link_ctrl_timer_type;
...
typedef struct
{
  ...
  gstk_nv_sms_link_ctrl_timer_type       sms_link_ctrl_timer;
  gstk_nv_feature_bmask_2_data_type      feature_bmask_2;
  gstk_nv_env_retry_ext_data_type        env_retry_ext;
  gstk_nv_csim_present_data_type         csim_present;
}
...
)
{
  ...
  UIM_SELECTIVE_MSG_HIGH_4(UIM_LOG_TOOLKIT,"******** GSTK NV: item_num: %d, is_item_nv: %d, cache_index: %d, nv_index: %d ********",
  ...
  {
    UIM_SELECTIVE_MSG_HIGH_2(UIM_LOG_TOOLKIT, "cache_state: 0x%x, cache_len: 0x%x, cache_data:",
  ...
  if(dump_cache_data && item_params_ptr->cache_data_ptr != NULL)
  {
    ...
    while (i < item_params_ptr->cache_data_len)
    {
      uint8 temp[9];

      memset(temp, 0, sizeof(temp));
      memscpy(temp, sizeof(temp), &byte_array_ptr[i],
              (i + 9 <= item_params_ptr->cache_data_len) ? 9 : item_params_ptr->cache_data_len - i);
        UIM_SELECTIVE_MSG_HIGH_9(UIM_LOG_TOOLKIT, "0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x",
...
)
{
  ...
  memset(&gstk_nv_cmd, 0x00, sizeof(gstk_nv_cmd));
  ...
  {
  UIM_MSG_ERR_1("In gstk_nv_item_read_write, NV Status: 0x%x",
           gstk_nv_cmd.status);
    return GSTK_ERROR;
  }
  return GSTK_SUCCESS;
...
)
{
  ...
  if(mcfg_gcf_nv_get_status() && !read_data.csim_present)
  {
    /* Disable proprietary behavior if GCF NV947 is set and CSIM is not present */
    ...
    /* Enable GSTK_CFG_FEATURE_SLOT_ESTK_BIP_IP_DEFAULT if
       GCF NV947 is set and CSIM is not present */
  ...
  {
    gstk_instances_ptr[cache_index]->gstk_curr_envelope_info.retry_info.retry_flag = TRUE;
  }
  ...
  else
  {
    ...
    gstk_instances_ptr[cache_index]->gstk_curr_envelope_info.retry_info.retry_flag =
      read_data.env_retry;
  }
  ...
  /* If QMI CAT is not supported or QMI CAT mode is disabled, enable DATA_CARD and ESTK_IGNORE_ALPHA config
     Featurize this code as in off-target GSTK Test case is configured to be Display client */
  if(gstk_shared_data.qmi_cat_enabled == FALSE)
  {  
    /* Set Data Card bits */
...
)
{
  ...
  if(is_item_read)
  {
    gstk_rat_enum_type rat_prio[GSTK_MAX_NUM_RATS] = {GSTK_RAT_LTE, GSTK_RAT_EHRPD, GSTK_RAT_HRPD,
                                                      GSTK_RAT_1X, GSTK_RAT_UMTS, GSTK_RAT_GSM };
    uint8  i = 0;
    uint8  j = 0;
    uint8  k = 0;
    ...
    /* Update missing RATs */
    for(i=0, j=0; i < GSTK_MAX_NUM_RATS && j < GSTK_MAX_NUM_RATS;)
    {
      if(rat_prio[i] ==  GSTK_RAT_INVALID)
      {
        i++;
      }
      else
      {
        /* Fill the RAT in the next available slot in the prio_list */
        if(gstk_nv_cache.slot_items_ptr[cache_index]->rat_prio.prio_list[j] == GSTK_RAT_INVALID)
        {
          gstk_nv_cache.slot_items_ptr[cache_index]->rat_prio.prio_list[j] = rat_prio[i];
          rat_prio[i] = GSTK_RAT_INVALID;
          i++;
          j++;
        }
      }

      if(i >= GSTK_MAX_NUM_RATS || j >= GSTK_MAX_NUM_RATS)
      {
        break;
      }
      ...
      {
        /* slot available in prio list get the next available default RAT */
        if(rat_prio[i] !=  GSTK_RAT_INVALID)
        {
          gstk_nv_cache.slot_items_ptr[cache_index]->rat_prio.prio_list[j] = rat_prio[i];
          rat_prio[i] = GSTK_RAT_INVALID;
...
)
{
  ...
  switch (item_type)
  {
    ...
    case GSTK_NV_EXT_SLOT_CSIM_PRESENT:
      item_params_ptr->item_num =
        (uint32)UIM_COMMON_EFS_MMGSDI_CSIM_PRESENT;
      item_params_ptr->cache_data_ptr =
        (void*)&gstk_nv_cache.slot_items_ptr[item_params_ptr->cache_index]->csim_present.value;
      item_params_ptr->cache_data_len =
        sizeof(gstk_nv_cache.slot_items_ptr[item_params_ptr->cache_index]->csim_present.value);
      item_params_ptr->cache_state_ptr =
        &gstk_nv_cache.slot_items_ptr[item_params_ptr->cache_index]->csim_present.state;
      break;
    ...
    case GSTK_NV_SLOT_SMS_LINK_CTRL_TIMER:
      item_params_ptr->item_num =
        (uint32)UIM_COMMON_EFS_GSTK_SLOT_SMS_LINK_CTRL_TIMER;
      item_params_ptr->cache_data_ptr =
        (void*)&gstk_nv_cache.slot_items_ptr[item_params_ptr->cache_index]->sms_link_ctrl_timer.value;
      item_params_ptr->cache_data_len =
        sizeof(gstk_nv_cache.slot_items_ptr[item_params_ptr->cache_index]->sms_link_ctrl_timer.value);
      item_params_ptr->cache_state_ptr =
        &gstk_nv_cache.slot_items_ptr[item_params_ptr->cache_index]->sms_link_ctrl_timer.state;
      break;
    default:
...
)
{
  ...
  switch (item_type)
  {
    ...
    case GSTK_NV_EXT_ME_QMI_CAT_MODE:
      /* Write default value (CAT_CONFIG_MODE_DECODED_V02) into cache if item
         not found in FS */
      if(is_item_read &&
         *item_params_ptr->cache_state_ptr == GSTK_NV_STATE_NOT_FOUND)
      {
        *(uint8*)item_params_ptr->cache_data_ptr = CAT_CONFIG_MODE_DECODED_V02;
      }
    ...
    case GSTK_NV_EXT_SLOT_CSIM_PRESENT:
      ...
      if(data_ptr != NULL)
      {
        data_ptr->csim_present = *(boolean*)item_params_ptr->cache_data_ptr;
      }
      break;
    ...
    case GSTK_NV_SLOT_SMS_LINK_CTRL_TIMER:
      if(is_item_read &&
         *item_params_ptr->cache_state_ptr == GSTK_NV_STATE_INIT)
      {
        data_ptr->sms_link_ctrl_timer = *(uint8*)item_params_ptr->cache_data_ptr;
      }
      else
      {
        gstk_status = GSTK_ERROR;
...
)
{
  ...
  uint32                     nv_item_count   = 0;
  gstk_nv_op_categ_enum_type op_categ = GSTK_NV_OP_CATEG_INIT;

  UIM_MSG_HIGH_2("gstk_nv_init_slot_cache, category: 0x%x, slot_id: 0x%x",
                 init_categ, slot_id);
...
)
{
  ...
  UIM_MSG_HIGH_3("gstk_nv_init_sub_cache, category: 0x%x, slot_id: 0x%x, sub_id: 0x%x",
                 init_categ, slot_id, sub_id);
...
)
{
  ...
  switch(mask_value)
  {
    ...
    case GSTK_CFG_FEATURE_SLOT_EHRPD_LOC_INFO:
      ...
      {
        ret = TRUE;
        UIM_MSG_HIGH_2("gstk_nv_get_feature_status, mask: 0x%x is enabled on slot id: 0x%x",
                        mask_value, slot_id);
      }
      break;
...
)
{
  ...
  switch(mask_value)
  {
    ...
    case GSTK_CFG_FEATURE_2_SLOT_ENABLE_BIP_ALWAYS_ON_APN_USE:
    case GSTK_CFG_FEATURE_2_SLOT_DISABLE_BIP_DEFAULT_DS_PROFILE_USE:
    case GSTK_CFG_FEATURE_2_SLOT_DISABLE_ENH_RAT_AND_LOC_EVT:
    case GSTK_CFG_FEATURE_2_SLOT_ENABLE_PCO_ENV:
    case GSTK_CFG_FEATURE_2_SLOT_DISABLE_CDMA_LOCATION_INFO:
    case GSTK_CFG_FEATURE_2_SLOT_DISABLE_NON_US_CDMA_LOCATION_INFO:
    case GSTK_CFG_FEATURE_2_SLOT_DISABLE_HOLDING_SEND_SMS_TILL_SMS_PP_ACK:
    case GSTK_CFG_FEATURE_2_SLOT_ENABLE_TP_IMS_REG_EVENT:
    case GSTK_CFG_FEATURE_2_SLOT_DISABLE_TP_CSG:
    case GSTK_CFG_FEATURE_2_SLOT_BLOCK_3GPP2_SMS_OVER_IMS:
    case GSTK_CFG_FEATURE_2_SLOT_REFRESH_FILE_LIST_OPTIONAL:
    case GSTK_CFG_FEATURE_2_SLOT_GSTK_CUST_ENH0:
      if((!GSTK_IS_VALID_SLOT_ID(slot_id)) ||
         (gstk_nv_cache.slot_items_ptr[(uint8)(slot_id - 1)] == NULL))
      {
        return FALSE;
      ...
      {
        ret = TRUE;
        UIM_MSG_HIGH_2("gstk_nv_get_feature_2_status, mask: 0x%x is enabled on slot id: 0x%x",
                        mask_value, slot_id);
      }
      break;
...
)
{
  ...
  switch(mask_value)
  {
    ...
    case GSTK_CFG_FEATURE_ME_DISABLE_DUAL_DATA:
    case GSTK_CFG_FEATURE_ME_ENABLE_QMI_LOC_NOTIFY_SUPPORT:
      if(gstk_nv_cache.me_items.me_spec_feature_bmask.value & mask_value)
      {
        ret = TRUE;
        UIM_MSG_HIGH_1("gstk_nv_get_me_feature_status, mask: 0x%x is enabled",
						 mask_value);
      }
      break;
    default:
