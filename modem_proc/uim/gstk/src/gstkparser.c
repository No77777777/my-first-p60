...
Copyright (c) 2003-2017, 2019, 2022 QUALCOMM Technologies, Inc (QTI) and its licensors.
...
$Header: //components/rel/uim.mpss/6.1.0/gstk/src/gstkparser.c#8 $
...
02/12/19   gs      Send error TR when the Local Address TLV is present
                   Add comprehension required check for local address TLV
03/22/17   gs      Handle Browsing status event
...
04/04/16   gm      Ignore "0xF" in the address string
03/02/16   gs      Return right reason for unsupported transport protocol
...
07/02/15   shr     Fix gstk_parser_is_orig_offset_valid for offset 0 and length 0
06/25/15   gm      L4 compiler warning
06/09/15   vr      Parsed the num of files in the refresh file list TLV
                                   ...
                                   gstk_string_type *text_string)
{
    ...
    switch(dcs_enum) {
    case GSTK_DCS_7_BIT:
      GSTK_UTIL_SELECTIVE_DUMP_BYTE_ARRAY(" == command_data", &command_data[new_count], text_length);
      ...
      if (text_length > 223) {
        ...
        new_text_length = int32touint32(text_length + 1 + additional_mod + additional_int);

        text_string->text = GSTK_CALLOC(
            new_text_length * sizeof(uint8));
        ...
        memset(text_string->text, 0x00, new_text_length);
        ...
        text_length =  gstk_util_unpack_7_bit_chars(
          ...
          text_string->text,
          uint32touint8(new_text_length - 1));
    ...
    GSTK_UTIL_SELECTIVE_DUMP_BYTE_ARRAY("TEXT", text_string->text, text_length);
  ...
  gstk_alphabet_type *text_string_ptr)
{
  ...
  GSTK_UTIL_SELECTIVE_DUMP_BYTE_ARRAY("Orig command_data", &command_data_ptr[new_count], orig_text_length);
  ...
  switch(dcs_enum)
  {
    ...
    case GSTK_DCS_8_BIT:
      ...
      GSTK_UTIL_SELECTIVE_DUMP_BYTE_ARRAY("command_data_ptr", &command_data_ptr[new_count], orig_text_length);
      ...
      switch(orig_cmd_data[count]) {
        ...
        case GSTK_NW_SEARCH_MODE:
          if(nv_data.cat_version >= GSTK_CFG_CAT_VER6) {
            ...
            UIM_SELECTIVE_MSG_HIGH_0(UIM_LOG_TOOLKIT, "Network Search mode");
          }
          else
          {
            if (tag_comprehension_req) {
        ...
        case GSTK_CSG_CELL_SELECTION:
          if(nv_data.cat_version >= GSTK_CFG_CAT_VER9) {
            ...
            UIM_SELECTIVE_MSG_HIGH_0(UIM_LOG_TOOLKIT, "CSG cell selection");
          }
          else
          {
            if (tag_comprehension_req) {
        ...
        case GSTK_IMS_REGISTRATION:
          ...
          UIM_SELECTIVE_MSG_HIGH_0(UIM_LOG_TOOLKIT, "IMS Registration");
          break;
        case GSTK_CONTACTLESS_STATE_REQ:
          evt_list_mask[GSTK_CONTACTLESS_STATE_REQ] = 0x01;
          UIM_SELECTIVE_MSG_HIGH_0(UIM_LOG_TOOLKIT, "Contactless State Request");
          break;        
        default:
          if (tag_comprehension_req) 
          {
            UIM_MSG_ERR_1("Unhandled Event, 0x%x", orig_cmd_data[count]);
          }
          else {
            /* update the cmd_table info to indicate there was a parsing issue */
            if (ref_id != NULL) {
    ...
    *more_tlvs = TRUE;
    ...
    if(tlv_length == 0) {
      ...
      if ((bcd_addr_length > GSTK_MAX_BCD_ADDRESS_LEN) || (bcd_addr_length < 1)) {
        ...
        GSTK_UTIL_SELECTIVE_DUMP_BYTE_ARRAY("Raw Data", &orig_cmd_data[count], bcd_addr_length);

        GSTK_UTIL_SELECTIVE_DUMP_BYTE_ARRAY("Decoded address",
                                            address->address,
                       ...
                       const uint32                    *ref_id)
{
    ...
    *more_tlvs = TRUE;
    ...
    if (subaddress->length > GSTK_MAX_SUBADDR_LEN) {
      ...
      GSTK_UTIL_SELECTIVE_DUMP_BYTE_ARRAY("Raw Data", &orig_cmd_data[count], subaddress->length);

      GSTK_UTIL_SELECTIVE_DUMP_BYTE_ARRAY("Decoded subaddress", subaddress->subaddress, subaddress->length);
                       ...
                       const uint32                   *ref_id)
{
    ...
    *more_tlvs = TRUE;
    ...
    if (ccp->length > GSTK_MAX_CCP_LEN) {
      ...
      GSTK_UTIL_SELECTIVE_DUMP_BYTE_ARRAY("Raw Data", &orig_cmd_data[count], ccp->length);

      GSTK_UTIL_SELECTIVE_DUMP_BYTE_ARRAY("Decoded ccp", ccp->ccp, ccp->length);
    ...
    *more_tlvs = TRUE;
    ...
    if (url->length > 0) { /* Card provides a url address */
      ...
      if (url->url != NULL) {
        ...
        GSTK_UTIL_SELECTIVE_DUMP_BYTE_ARRAY("Raw Data", &orig_cmd_data[count], url->length);

        GSTK_UTIL_SELECTIVE_DUMP_BYTE_ARRAY("Decoded url", url->url, url->length);
    ...
    *more_tlvs = TRUE;
    ...
    if (bearer->length > 0) { /* Card provides a bearer */
      ...
      if (bearer->bearer_list != NULL) {
        ...
        GSTK_UTIL_SELECTIVE_DUMP_BYTE_ARRAY("Raw Data", &orig_cmd_data[count], bearer->length);
        UIM_SELECTIVE_MSG_HIGH_0(UIM_LOG_TOOLKIT, "============ Decoded bearer");
        for (i = 0; i< bearer->length; i++) {
          switch(orig_cmd_data[count]){
          case GSTK_SMS:
            bearer->bearer_list[i] = GSTK_SMS;
            UIM_SELECTIVE_MSG_HIGH_2(UIM_LOG_TOOLKIT, "bearer[%d]: 0x%x, SMS", i, bearer->bearer_list[i]);
            break;
          case GSTK_CSD:
            bearer->bearer_list[i] = GSTK_CSD;
            UIM_SELECTIVE_MSG_HIGH_2(UIM_LOG_TOOLKIT, "bearer[%d]: 0x%x, CSD", i, bearer->bearer_list[i]);
            break;
          case GSTK_USSD_BEARER_CODE:
            bearer->bearer_list[i] = GSTK_USSD_BEARER_CODE;
            UIM_SELECTIVE_MSG_HIGH_2(UIM_LOG_TOOLKIT, "bearer[%d]: 0x%x, USSD",
                                     i, bearer->bearer_list[i]);
            break;
          case GSTK_GPRS:
            bearer->bearer_list[i] = GSTK_GPRS;
            UIM_SELECTIVE_MSG_HIGH_2(UIM_LOG_TOOLKIT, "bearer[%d]: 0x%x, GPRS",
                                     i, bearer->bearer_list[i]);
            break;
          case GSTK_RFU:
            bearer->bearer_list[i] = GSTK_RFU;
            UIM_SELECTIVE_MSG_HIGH_2(UIM_LOG_TOOLKIT, "bearer[%d]: 0x%x, RFU", i, bearer->bearer_list[i]);
            break;
          default:
            if (tag_comprehension_req) {
              bearer->bearer_list[i] = GSTK_RFU;
                       ...
                       const uint32                   *ref_id)
{
    ...
    *more_tlvs = TRUE;
    ...
    if (provisioning_file->length != 0) {
      ...
      UIM_SELECTIVE_MSG_HIGH_1(UIM_LOG_TOOLKIT, "===== # Files", provisioning_file->num_of_files);

      GSTK_UTIL_SELECTIVE_DUMP_BYTE_ARRAY("provisioning_file",
                       ...
                       const uint32          *ref_id_ptr)
{
    ...
    *is_cdma_sms = FALSE;
    ...
    GSTK_UTIL_SELECTIVE_DUMP_BYTE_ARRAY("Raw Data", &orig_cmd_data[count], tpdu->length);

    GSTK_UTIL_SELECTIVE_DUMP_BYTE_ARRAY("Decoded Data", tpdu->tpdu, tpdu->length);
...
)
{
    ...
    *more_tlvs = TRUE;
    ...
    if (orig_cmd_data[count+length_of_length_field-1] == 0) {
      ...
      GSTK_UTIL_SELECTIVE_DUMP_BYTE_ARRAY("text", item->item_text, item->length);
                       ...
                       const uint32                   *ref_id)
{
    ...
    *more_tlvs = TRUE;
    ...
    /* length filed = file list + num of files */
    file_list->length = orig_cmd_data[count + length_of_length_field - 1] - 1;

    /* position count to the file list count in the tlv */
    count = count + length_of_length_field;

    file_list->num_of_files = orig_cmd_data[count];

    /* position count to the file list in the tlv */
    count = count + 1;

    if (file_list->length != 0 && file_list->num_of_files != 0)
    {
      ...
      else 
      {
        ...
        GSTK_UTIL_SELECTIVE_DUMP_BYTE_ARRAY("file_list", file_list->file_paths, file_list->length);
      }
    }
                       ...
                       const uint32                   *ref_id)
{
    ...
    *more_tlvs = TRUE;
    ...
    if (dtmf->length > 0) {
      ...
      GSTK_UTIL_SELECTIVE_DUMP_BYTE_ARRAY("Raw Data", &orig_cmd_data[count], dtmf->length);
    }
  ...
  uint32                                   len_bearer)
{
  ...
  memset(eutran_mapped_utran_ps_bearer, 0, sizeof(gstk_eutran_mapped_utran_ps_param_type));

  switch(len_bearer)
  {
    case GSTK_EUTRAN_MAPPED_UTRAN_PS_NO_PARAM_LEN:
      eutran_mapped_utran_ps_bearer->qci = cmd_data[0];   
      eutran_mapped_utran_ps_bearer->pdp_type = (gstk_packet_data_protocol_enum_type)cmd_data[1];   
      break;
    case GSTK_EUTRAN_MAPPED_UTRAN_PS_PARAM_LEN:
      eutran_mapped_utran_ps_bearer->qci = cmd_data[0];
      ...
      eutran_mapped_utran_ps_bearer->pdp_type = (gstk_packet_data_protocol_enum_type)cmd_data[5];  
      break;
    case GSTK_EUTRAN_MAPPED_UTRAN_PS_PARAM_EX_LEN:    
      eutran_mapped_utran_ps_bearer->qci = cmd_data[0];
      eutran_mapped_utran_ps_bearer->max_bitrate_ul = cmd_data[1];
      eutran_mapped_utran_ps_bearer->max_bitrate_dl = cmd_data[2];
      eutran_mapped_utran_ps_bearer->guaranteed_bitrate_ul = cmd_data[3];
    ...
    *more_tlv = FALSE;
    ...
    case 0x0B:
      if (len_bearer == GSTK_EUTRAN_MAPPED_UTRAN_PS_PARAM_LEN  ||
          len_bearer == GSTK_EUTRAN_MAPPED_UTRAN_PS_PARAM_EX_LEN  ||
          len_bearer == 2) 
      {
                                    ...
                                    uint16             *alphabet_out)
{
  ...
  if((in_len > 0) && (ucs2_default_in != NULL))
  {
                                                       ...
                                                       uint16             *alphabet_out)
{
  ...
  if((in_len > 0) && (gsm_default_in != NULL))
  {
    if(alphabet_out == NULL)
    {
      for(i = 0;  i < in_len; i++)
    ...
    memset(alphabet_out, 0x00, sizeof(uint16)*int32touint32(in_len));
    gstk_convert_from_gsm_7bit_default(gsm_default_in, in_len-1, alphabet_out);
  }
  else
  {
    if (alphabet_out != NULL) {
    ...
    if (orig_alpha_len > 0) { /* Card provides a url address */
      ...
      case 0x80:
        ...
        if (alpha->alphabet != NULL) {
          ...
          GSTK_UTIL_SELECTIVE_DUMP_BYTE_ARRAY("Raw Data", &orig_cmd_data[count], orig_alpha_len);

          for (i = 0; i< alpha->length; i++) {
            UIM_SELECTIVE_MSG_HIGH_3(UIM_LOG_TOOLKIT, " Decoded alpha[%d]: 0x%x, %c",
                       ...
                       const uint32                  *ref_id)
{
    ...
    *more_tlv = FALSE;
    ...
    other_address->is_tlv_present = FALSE;

    /* Mark is_tlv_present to TRUE if tag value is 0x3E or 0xBE i.e. If comprehension bit is set or not set */
    if((orig_cmd_data[count] & 0x7F) == GSTK_OTHER_ADDRESS_TAG)
    {
      other_address->is_tlv_present = TRUE;
    }
    ...
    UIM_SELECTIVE_MSG_HIGH_1(UIM_LOG_TOOLKIT, "Other Addr Length 0x%x",
    ...
    {
      /* address type */
      UIM_SELECTIVE_MSG_HIGH_1(UIM_LOG_TOOLKIT, "Address Type 0x%x", orig_cmd_data[count]);
      switch (orig_cmd_data[count]) 
      {
        case 0x21:
      ...
      if (other_address->pkt_address.address != NULL) 
      {
        ...
        GSTK_UTIL_SELECTIVE_DUMP_BYTE_ARRAY("Raw Data",
                       ...
                       const uint32                         *ref_id)
{
    ...
    *more_tlv = FALSE;
    ...
    ch_data->data = GSTK_CALLOC(ch_data->data_len);
    ...
    /* copy the data */
    (void)memscpy(ch_data->data, ch_data->data_len, &orig_cmd_data[count], int32touint32(ch_data->data_len));
  ...
  gstk_ussd_string_type *text_string)
{
    ...
    text_length = text_string->length; /*  length before decoding */
    UIM_SELECTIVE_MSG_HIGH_2(UIM_LOG_TOOLKIT, "In gstk_decode_ussd_string_body() dcs = %x, text_length = %x",
                             dcs, text_length);
    ...
    case GSTK_DCS_7_BIT:
      ...
      if (text_length > 223) {
        ...
        new_text_length = int32touint32(text_length + additional_mod + additional_int);
...
/*===========================================================================
FUNCTION gstk_parser_contactless_func_state_tlv

DESCRIPTION
    This function parses the Contactless functionality state TLV and
    populate the gstk_string_type passed in the function call with the info
    in the TLV.
    It sets the current UE contactless functional state.
    It also returns the index slot for next TLV's tag if available.  If there is
    no more TLVs to be parsed, the more_tlv will be set to FALSE
...
PARAMETERS
  ...
  orig_offset: [Input] Offset in the original command data where the parsing
                       should begin
  state: [Input/Output] Pointer to gstk_contactless_func_state_enum_type for
                        indicating contactless functionality state
  next_tlv_offset: [Input/Output] Pointer to the index offset for the next tlv
                                  if applicable.
  more_tlvs: [Input/Output] Pointer to boolean to indicate whether there are more
                            TLV to be parse after exiting this function
...
SEE ALSO
  None
===========================================================================*/
gstk_status_enum_type gstk_parser_contactless_func_state_tlv(
    uint8                                  *orig_cmd_data,
    int32                                   cmd_data_len,
    int32                                   orig_offset,
    tags_enum_type                          tag,
    gstk_contactless_func_state_enum_type  *state,
    int32                                  *next_tlv_offset,
    boolean                                *more_tlvs,
    boolean                                 man_opt_specifier,
    const uint32                           *ref_id)
{
  ...
  UIM_MSG_HIGH_0("** Parsing Contactless Functionality State TLV");

  if((!state) || (!next_tlv_offset) || (!more_tlvs))
  {
    UIM_MSG_ERR_0("NULL PTR");
    return GSTK_BAD_PARAM;
  }
  ...
  *more_tlvs = FALSE;
  ...
  /* Valid Contactless State Functionality Tag*/
  count++;

  length_of_length_field = gstk_find_length_of_length_value(&orig_cmd_data[count]);

  /* By spec definition, this TLV has length 1 */
  if (length_of_length_field != 1 ||
      orig_cmd_data[count] != 1)
  {
    UIM_MSG_ERR_2("Invalid length! length_of_length_field = 0x%x and length = 0x%x",
                  length_of_length_field, orig_cmd_data[count]);
    return GSTK_INVALID_LENGTH;
  }

  /* Valid Length */
  count++;
  *state = (gstk_contactless_func_state_enum_type)(orig_cmd_data[count]);

  count++;
  /* next TLV offset */
  gstk_status = gstk_parser_get_next_tlv_offset(cmd_data_len, count, next_tlv_offset, more_tlvs);

  /* more_tlvs should be false, because this proactive command should only have 1 tlv */
  if (*more_tlvs)
  {
    UIM_MSG_ERR_0("Error: Unexpected TLVs detected for this proactive command");
    return GSTK_ERROR;
  }

  return GSTK_SUCCESS;
} /* gstk_parser_contactless_func_state_tlv */
...
/*===========================================================================
FUNCTION gstk_parser_geo_loc_para_tlv

DESCRIPTION
    This function parses the geographical location parameters TLV
    It also returns the index slot for next TLV's tag if available. If there is
    no more TLVs to be parsed, the more tlv will be set to FALSE
...
PARAMETERS
  ...
  orig_offset: [Input] Offset in the original command data where the parsing
                       should begin
  geo_loc_para: [Input/Output] Pointer to the geographical location parameter
  next_tlv_offset: [Input/Output] Pointer to the index offset for the next tlv
                                  if applicable.
  more_tlv: [Input/Output] Pointer to boolean to indicate whether there are more
                            TLV to be parse after exiting this function
...
SEE ALSO
  None
===========================================================================*/
gstk_status_enum_type gstk_parser_geo_loc_para_tlv(
                       uint8                                *orig_cmd_data,
                       int32                                 cmd_data_len,
                       int32                                 orig_offset,
                       gstk_geo_loc_para_type               *geo_loc_para,
                       int32                                *next_tlv_offset,
                       boolean                              *more_tlv,
                       boolean                               man_opt_specifier,
                       const uint32                         *ref_id)
{
    ...
    UIM_MSG_HIGH_0("** Parsing geographical location parameter");
    ...
    *more_tlv = FALSE;
    ...
    /* channel data TLV */
    tag_parsing_result = gstk_is_tag_valid(ref_id, orig_cmd_data[count], (uint8)GSTK_GEO_LOC_PARA_TAG,
                                           man_opt_specifier, orig_cmd_data, &count, cmd_data_len);

    if (!gstk_continue_parsing(GSTK_GEO_LOC_PARA_TAG, tag_parsing_result, man_opt_specifier, &gstk_status))
    {
      if (gstk_status == GSTK_SUCCESS) {
        /* Was optional tag, go through the parser_get_next_tlv_offset */
        gstk_status = gstk_parser_get_next_tlv_offset(cmd_data_len, count, next_tlv_offset, more_tlv);
    ...
    /* copy the data */
    gstk_memcpy(geo_loc_para, 
                &orig_cmd_data[count],
                sizeof(gstk_geo_loc_para_type),
                sizeof(gstk_geo_loc_para_type),
                sizeof(gstk_geo_loc_para_type));

    if(geo_loc_para->pref_max_rsp_time > 7 || geo_loc_para->pref_max_rsp_time < 2)
    {
      if(tag_comprehension_req == TRUE)
      {
        UIM_MSG_ERR_0("Invalid Preferred Maximum Response Time");      
        return GSTK_ERROR;
      }
    }
    
    if((geo_loc_para->horizontal_accuracy > 0x7F) && (geo_loc_para->horizontal_accuracy != 0x81))
    {
      if(tag_comprehension_req == TRUE)
      {
        UIM_MSG_ERR_0("Invalid horizontal accuracy");            
        return GSTK_ERROR;
      }
    }
        
    if((geo_loc_para->vertical_coordinate > 0x7F) && 
        (geo_loc_para->vertical_coordinate != 0x81) && 
        (geo_loc_para->vertical_coordinate != 0x80))
    {
      if(tag_comprehension_req == TRUE)
      {
        UIM_MSG_ERR_0("Invalid vertical coordinate");            
        return GSTK_ERROR;
      }
    }
    
    count += sizeof(gstk_geo_loc_para_type);

    /* next TLV offset */
    gstk_status = gstk_parser_get_next_tlv_offset(cmd_data_len, count, next_tlv_offset, more_tlv);

    return gstk_status;
} /* gstk_parser_geo_loc_para_tlv */
