...
gstk_util_is_env_cmd_ref_table_full
  This function returns if the envelope_cmd_ref_table is FULL
 ...
 gstk_util_is_valid_func_req
  ...
  gstk_util_check_slot_with_fetch_rejected
    ...
    uimdrv API to check for 91 xx on that slot.

  gstk_util_convert_from_uim_slot_to_mmgsdi_slot
    This function will convert the UIM slot type to MMGSDI slot type
...
Copyright (c) 2003-2022 QUALCOMM Technologies, Inc (QTI) and its licensors.
...
$Header: //components/rel/uim.mpss/6.1.0/gstk/src/gstkutil.c#8 $ $DateTime: 2022/11/08 04:26:47 $ $Author: pwbldsvc $

when         who     what, where, why
--------   ---     --------------------------------------------------------------------
11/08/22   ac      Disables GSTK response timer for specific slot
08/04/22   shp     Fix OOB issues in APN parsing
...
02/07/19   gs      Featurize processing mm or mode change events under FEATURE_CDMA
10/10/18   dt      clear envelope ref table along with retry env data
06/05/18   dt      Respond to the Attach Control request upon refresh
12/28/17   dt      Update the right slot id while sending envelope command during retry
12/28/17   dt      GSTK F3 reduction
12/20/17   gs      Support for OTASP Call on Slot#2
12/07/17   gs      Check for 1x availability in CM info for OTASP
06/01/17   gs      Don't depend on DS Sys Status for establishing BIP on non-DDS
05/05/17   dt      Removing unused function gstk_util_dereg_client_type
04/13/17   shr     Disable CDMA loc status in non-US regions for specific operators
03/14/17   dt      Featurize IMSI_M and cdma_loc_st_disabled references
03/10/17   gs      Extend the slot id while doing retry
01/20/17   vr      Compare cdma location info only if it is valid
01/18/17   vr      Avoid sending duplicate LS/Access Tech Envelopes to card
12/20/16   dt      Add mem free for Activate command
12/01/16   dt      KW fixes array out of bound in gstk_get_input.c
...
10/05/16   shr     Send immediate Attach Control Response on SIM BUSY
10/03/16   dd      Removed lint comments
09/26/16   shr     Ignore NULL RATs while checking for 1X only availability
09/19/16   gs      Featurization of Setup idle text
08/18/16   shr     Fix compilation error with CDMA disabled
08/09/16   vr      Queue the location status to front of the gstk queue
07/27/16   gs      Scenario based F3 reduction
07/20/16   shr     Do not de-queue Envelope if Env command ref. table is full
...
05/11/16   gm      Removal of CSG feature for IOE devices
04/14/16   shr     F3 reduction
05/11/16   gm      Keeping 3gpp2 code under FEATURE_CDMA
03/07/16   shr     Added support for PCO Envelope
03/07/16   gm      Retry call control when no proactive session on all slot
12/08/15   dd      F3 messages reduction 
11/30/15   vr      GSTK CC retry to be enabled only after the busy from card
11/12/15   vr      F3 Logging inprovements
11/09/15   vr      Reset usr_actvty and idl_scrn for raw envelope rsp evt dl
11/02/15   gm      Added support for refresh enforcement policy
10/09/15   gm      Converting to unsigned variables.
08/26/15   bd      Moved the logic to reset cache in respective event handlers
07/17/15   shr     Validate destination buffer size
07/02/15   shr     Validate destination buffer size
07/07/15   shr     Fix potential buffer overflow while building Envelope/TR
06/25/15   gm      L4 compiler warning
06/05/15   gm      Split ME and slot specific NV
06/04/15   vr      Fix 7 bit unpacked array out of bound access
05/27/15   vr      Cleanup fetch flags before queuing recovery or card error command
04/29/15   shr     Added support for ME generated TI values in call events
04/27/15   gm      Added support for class "n" command
04/24/15   dy      Make util dump print less lines
04/22/15   dy      Add support for 4 byte freq measurement for EUTRAN
04/22/15   vr      Don't send SW1 report for the clients which didn't send TR
04/08/15   vr      Check pending proactive cmd MMGSDI FETCH allowed is set
04/02/15   vr      Non UI and non NW proactive command parallel processing
02/27/15   dy      Added support for class "r" commands
02/24/15   gm      Moved CC command cache to heap
02/24/15   gm      Moved generic function out of FEATURE_CDMA
02/06/15   dy      Reduce GSTK priv data size
02/06/15   gm      Retry call control after recovery
01/14/15   gm      Properly clear global variable when received REFRESH
...
static uint8 gstk_util_get_ti_value_index_in_table(
  gstk_call_evt_info_type           call_evt_info,
  uint8                             orig_ti
);

static uint8 gstk_util_add_ti_value_index_in_table(
  gstk_call_evt_info_type           call_evt_info,
  uint8                             orig_ti
);

static void gstk_remove_ti_value_list_in_table(
  gstk_transaction_id_type *modified_data_ptr);
...
/*===========================================================================
FUNCTION gstk_util_is_env_cmd_ref_table_full
...
RETURN VALUE
  boolean TRUE  : envelope_cmd_ref_table is FULL
          FALSE : envelope_cmd_ref_table is not FULL
...
SEE ALSO
  None
===========================================================================*/
boolean gstk_util_is_env_cmd_ref_table_full(void)
{
  uint8 i;

  for (i = 0; i < (GSTK_MAX_ENVELOPE_COMMAND - 1); i++)
  {
    if (gstk_shared_data.envelope_cmd_ref_table[i].is_free)
    {
      return FALSE;
    }
  }
...
void gstk_util_env_cmd_ref_table_free_slot(uint32 ref_id) {
  ...
  if (ref_id < GSTK_MAX_ENVELOPE_COMMAND)
  {
    ...
    if(gstk_shared_data.envelope_cmd_ref_table[ref_id].gstk_cc_cache_ptr != NULL)
    {
      gstk_free(gstk_shared_data.envelope_cmd_ref_table[ref_id].gstk_cc_cache_ptr);
      gstk_shared_data.envelope_cmd_ref_table[ref_id].gstk_cc_cache_ptr = NULL;
    }
    if(gstk_shared_data.envelope_cmd_ref_table[ref_id].mo_sms_cache_ptr != NULL)
    {
      gstk_free(gstk_shared_data.envelope_cmd_ref_table[ref_id].mo_sms_cache_ptr);
      gstk_shared_data.envelope_cmd_ref_table[ref_id].mo_sms_cache_ptr = NULL;
    }
...
)
{
  ...
  switch(env_cmd_type)
  {
    ...
    case GSTK_IMS_REG_EVT_IND:
      ...
      break;
    case GSTK_CONTACTLESS_STATE_REQ_EVT_IND:
      if (nv_data.cat_version >= GSTK_CFG_CAT_VER9)
      {
        env_rsp_type = GSTK_CONTACTLESS_STATE_REQ_EVT_IND_RSP;
      }
      break;
    case GSTK_GEO_LOC_REPORT_IND:
      client_cb    = cmd_ptr->geo_loc_report.client_cb;
      env_rsp_type = GSTK_GEO_LOC_REPORT_RSP;
      break;
...
)
{
  ...
  /* Set up gstk log masking */
  gstk_logging_populate_masking_fn_ptr(cmd_ptr);
   ...
   uint32               *class_bitmask_ptr)
{
  ...
  UIM_SELECTIVE_MSG_HIGH_1(UIM_LOG_TOOLKIT, "In gstk_util_convert_cmd_type_to_reg_bitmask cmd_type 0x%x",
                           cmd_type);
  ...
  case GSTK_CONTACTLESS_STATE_CHANGED_REQ:
    *cmd_bitmask_ptr   = GSTK_REG_CONTACTLESS_STATE_CHANGED_EVT;
    *class_bitmask_ptr = GSTK_REG_CATEGORY_R_CMD;
    break;

  case GSTK_SETUP_EVT_CONTACTLESS_STATE_REQ_NOTIFY:
    *cmd_bitmask_ptr   = GSTK_REG_CONTACTLESS_STATE_REQ_EVT;
    *class_bitmask_ptr = GSTK_REG_CATEGORY_R_CMD;
    break;

  case GSTK_GEO_LOC_REQ:
    *cmd_bitmask_ptr   = GSTK_REG_GEO_LOC_REQ_EVT;
    *class_bitmask_ptr = GSTK_REG_CATEGORY_N_CMD;
    break;
...
)
{
  ...
  *client_id_ptr = GSTK_MAX_CLIENT + 1;
  ...
  UIM_SELECTIVE_MSG_HIGH_1(UIM_LOG_TOOLKIT,
                           "In gstk_util_find_client_with_registered_event, class_bitmask 0x%x",
                           class_bitmask);
  ...
  case GSTK_REG_CATEGORY_M_CMD:
    ...
    break;
  case GSTK_REG_CATEGORY_R_CMD:
    l_index      = GSTK_CLASS_R_BEGIN_INDEX;
    adj_offset   = GSTK_CATEGORY_R_ADJ_OFFSET;
    break;
  case GSTK_REG_CATEGORY_N_CMD:
    l_index      = GSTK_CLASS_N_BEGIN_INDEX;
    adj_offset   = GSTK_CATEGORY_N_ADJ_OFFSET;
    break;
  default:
  ...
  rex_timer_cnt_type msecs)
{
    if((gstk_nv_get_me_feature_status(GSTK_CFG_FEATURE_ME_GSTK_CUST_ENH0) == FALSE) &&
     (gstk_nv_get_feature_2_status(GSTK_CFG_FEATURE_2_SLOT_GSTK_CUST_ENH0,
      command_details_ref_table_ptr[GSTK_CMD_REF_TABLE_INDEX(ref_id)].sim_slot_id) == FALSE))   
  {
    ...
    gstk_cmd_from_card_type * message_p )
{
  ...
  UIM_SELECTIVE_MSG_HIGH_1(UIM_LOG_TOOLKIT,
                           "In gstk_util_release_card_cmd_memory(), command_id is 0x%x",
  ...
  case GSTK_GEO_LOC_REQ:
    if (message_p->cmd.geo_loc_pro_cmd_req.alpha.alphabet != NULL)
    {
      gstk_free(message_p->cmd.geo_loc_pro_cmd_req.alpha.alphabet);
      message_p->cmd.geo_loc_pro_cmd_req.alpha.alphabet = NULL;
    }
    if ((message_p->cmd.geo_loc_pro_cmd_req.icon.present) &&
         (message_p->cmd.geo_loc_pro_cmd_req.icon.data != NULL))
    {
      gstk_free(message_p->cmd.geo_loc_pro_cmd_req.icon.data);
      message_p->cmd.geo_loc_pro_cmd_req.icon.data = NULL;
    }
    memset(&(message_p->cmd.geo_loc_pro_cmd_req),
           0x00,
    ...
    gstk_cmd_type * message_p )
{
    ...
    case GSTK_OPEN_CH_CNF:
      ...
      {
        for (i = 0; i < (uint32)message_p->open_ch_term_rsp_cnf.dns_addrs.num_addresses; i++)
        {
    ...
    case GSTK_ACTIVATE_CNF:
      if (message_p->activate_term_rsp_cnf.result_additional_info.additional_info_ptr != NULL) {
        addi_info_ptr = &message_p->activate_term_rsp_cnf.result_additional_info;
      }
      break;
    ...
    case GSTK_SMS_PP_DOWNLOAD_IND:
      ...
      break;
    case GSTK_GEO_LOC_REPORT_IND:
      if (message_p->geo_loc_report.nmea_sentence.nmea_ptr != NULL)
      {
        gstk_free(message_p->geo_loc_report.nmea_sentence.nmea_ptr);
        message_p->geo_loc_report.nmea_sentence.nmea_ptr = NULL;
      }
      break;
    case GSTK_MT_CALL_EVT_IND:
  ...
  const gstk_evt_dl_ind_type  * evt_list)
{
  ...
  case GSTK_MT_CALL_EVT_IND:
    ...
    /* Call Event trigger */
    cmd->cmd.mt_evt_envelope_ind.call_evt_info.call_evt_trigger = GSTK_CALL_TRIGGER_IMS_EXT;
    /* address */
  ...
  case GSTK_CALL_CONNECTED_EVT_IND:
    ...
    /* Call Event trigger */
    cmd->cmd.call_connected_envelope_ind.call_evt_info.call_evt_trigger = GSTK_CALL_TRIGGER_IMS_EXT;
    /* Transaction ID*/
    UIM_MSG_HIGH_2("Transaction ID: len: 0x%x ptr %p",
    ...
    if(evt_list->evt_ind.call_connected_evt_ind.transaction_id.length > 0 &&
         evt_list->evt_ind.call_connected_evt_ind.transaction_id.transaction_list != NULL)
    {
      ...
      if (cmd->cmd.call_connected_envelope_ind.transaction_id.transaction_list != NULL)
      {
  ...
  case GSTK_CALL_DISCONNECTED_EVT_IND:
    ...
    /* Call Event trigger */
    cmd->cmd.call_disconnected_envelope_ind.call_evt_info.call_evt_trigger = GSTK_CALL_TRIGGER_IMS_EXT;
    /* Transaction ID*/
    UIM_MSG_HIGH_2("Transaction ID: len: 0x%x ptr %p",
  ...
  case GSTK_CONTACTLESS_STATE_REQ_EVT_IND:
    if ((inst_ptr->gstk_evt_list[GSTK_CONTACTLESS_STATE_REQ] & 0x01) == 0x01) {
      cmd->cmd.contactless_state_req_envelope_ind.message_header.command_group = GSTK_ENVELOPE_CMD;
      gstk_set_command_id(&cmd->cmd.contactless_state_req_envelope_ind.message_header,
                          GSTK_CONTACTLESS_STATE_REQ_EVT_IND);
      /* already save the user data in the gstk_shared_data.envelope_cmd_ref_table, set the user_data as the env_ref_id */
      cmd->cmd.contactless_state_req_envelope_ind.message_header.user_data = env_ref_id;
      cmd->cmd.contactless_state_req_envelope_ind.request =
        evt_list->evt_ind.contactless_state_req_evt_ind;

      UIM_MSG_HIGH_1("Contactless State Req: 0x%x",
                     cmd->cmd.contactless_state_req_envelope_ind.request);
    }
    else {
      UIM_MSG_ERR_0("Contactless State Req Evt is not set");
      return GSTK_UNSUPPORTED_COMMAND;
    }
    break;
...
PARAMETERS
  ...
  in_len:  [Input] Input data length
  out:     [Input/Output] Pointer to output data
  out_len: [Input] Output data buffer length
...
)
{
  ...
  for (j=0; j<i; j++)
  {
    ...
    {
      gstk_byte_memmove(&out[j+1], &out[j+2], (uint8)(i-(j+1))-1);
      out[i - 1] = 0x00;
      i--;
    }
  }
...
)
{
  ...
  for (j=0; j<i; j++)
  {
    if (out[j] == 0x0D && out[j+1] == 0x0D)
    {
      gstk_byte_memmove((uint8*)&out[j+1], (uint8*)&out[j+2], (uint8)(i-(j+1))-1);
      out[i - 1] = 0x00;
      i--;
    }
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    j = def[i];
    if (j < sizeof(ascii_to_gsmdefault_table)) 
    {
  ...
  gstk_cdma_loci_ext_cache_type* new_cache)
{
  ...
  if((old_cache->is_cdma_info_valid == FALSE && new_cache->is_cdma_info_valid) ||
     (old_cache->is_advance_info_valid == FALSE && new_cache->is_advance_info_valid) ||
     (old_cache->is_hdr_info_valid == FALSE && new_cache->is_hdr_info_valid) ||
     (old_cache->is_cdma_info_valid && new_cache->is_cdma_info_valid == FALSE) ||
     (old_cache->is_advance_info_valid && new_cache->is_advance_info_valid == FALSE) ||
     (old_cache->is_hdr_info_valid && new_cache->is_hdr_info_valid == FALSE))
  {
    return TRUE;
  }

  /* Compare only if the new cache info is valid */
  if(new_cache->is_cdma_info_valid)
  {
    /* SID info */
  ...
  if (gstk_nv_get_feature_status(GSTK_CFG_FEATURE_SLOT_EHRPD_LOC_INFO, gstk_curr_inst_ptr->slot_id) == TRUE)
  {
     ...
     if(new_cache->is_hdr_info_valid)
     {
...
SEE ALSO
  None
===========================================================================*/
gstk_status_enum_type gstk_util_find_tlv_len (
  tags_enum_type              tag,
...
)
{
  ...
  *length_ptr = 0;
  ...
  case GSTK_EUTRAN_INTER_FREQ_NMR_RESULTS_TAG:
  ...
  case GSTK_CONTACTLESS_STATE_REQ_TAG:
  case GSTK_CONTACTLESS_FUNC_STATE_TAG:
  ...
  case GSTK_GEO_LOC_PARA_TAG:
  case GSTK_CSG_PLMN_INFO_TAG:
    return GSTK_UNSUPPORTED_COMMAND;
  }
  return GSTK_ERROR;
...
PARAMETERS
  ...
  dest_size:         [Input] Size of the new buffer where the data
                     will be copied into
  cmd_group:         [Input] Command group type of caller of this function
...
PARAMETERS
  ...
  new_offset_ptr:       [Input/Output] pointer to which the new offset will
                        be returned
  new_data_buffer_size: [Input] Size of the new buffer
  cmd_group:            [Input] Command group type of caller of this function
...
)
{
  ...
  gstk_status = gstk_util_populate_tlv_value(
                              ...
                              new_data_buffer_size - new_offset,
                              cmd_group);
  if (gstk_status != GSTK_SUCCESS)
  {
...
PARAMETERS
  ...
  *uim_cmd_ptr        - [Input/Output] uim Cmd ptr
  uim_cmd_buffer_size - [Input] Size of the uim Cmd ptr buffer
...
SEE ALSO
  None
===========================================================================*/
gstk_status_enum_type gstk_util_retry_envelope_cmd(gstk_slot_id_enum_type slot_id)
{
  ...
  UIM_MSG_HIGH_1("In gstk_util_retry_envelope_cmd() on Slot 0x%x", slot_id);
  ...
  GSTK_RETURN_ERROR_IF_NULL_PTR(gstk_instances_ptr[slot_id-1], GSTK_ERROR);

  (void)gstk_nv_access_read(GSTK_NV_SLOT_TOOLKIT_ENV_RETRY_EXT_DATA,
                          slot_id, &nv_data);

  /* Clear timer before sending the envelope */
  (void)rex_clr_timer(&gstk_instances_ptr[slot_id-1]->gstk_curr_envelope_info.gstk_env_retry_timer);

  if(gstk_instances_ptr[slot_id-1]->gstk_curr_envelope_info.gstk_env_state !=
       GSTK_SUB_ST_PENDING_ENV_IN_QUEUE)
  {
    UIM_MSG_HIGH_0("Incorrect GSTK env state to retry envelope");
    return GSTK_ERROR;
  }
  if(!gstk_instances_ptr[slot_id-1]->gstk_curr_envelope_info.env_data_bytes.data_buffer_ptr ||
     gstk_instances_ptr[slot_id-1]->gstk_curr_envelope_info.env_data_bytes.data_len > UIM_MAX_ENVELOPE_BYTES)
  {
    UIM_MSG_HIGH_0("env retry buffer ptr NULL or data len out of range");
    return GSTK_ERROR;
  }
  if ((gstk_instances_ptr[slot_id-1]->gstk_curr_envelope_info.env_ref_id == GSTK_CMD_DETAILS_REF_TABLE_FULL) ||
      (gstk_instances_ptr[slot_id-1]->gstk_curr_envelope_info.env_ref_id >= GSTK_MAX_ENVELOPE_COMMAND))
  {
  ...
  if ((gstk_status == GSTK_SUCCESS) && (uim_cmd_ptr != NULL))
  {
   ...
   /* set user_data */
   uim_cmd_ptr->hdr.user_data      = gstk_instances_ptr[slot_id-1]->gstk_curr_envelope_info.env_ref_id;
   /* Assign Slot Id */
   uim_cmd_ptr->hdr.slot           = (uim_slot_type) slot_id;
   /* populate the envelope command info */
   uim_cmd_ptr->envelope.num_bytes =
     (uint8)gstk_instances_ptr[slot_id-1]->gstk_curr_envelope_info.env_data_bytes.data_len;

   copy_len = uim_cmd_ptr->envelope.num_bytes;

   if(gstk_instances_ptr[slot_id-1]->gstk_curr_envelope_info.env_data_bytes.data_buffer_ptr[0] == 0x00)
   {
     uim_cmd_ptr->envelope.offset = 1;
   ...
   if(gstk_memcpy(uim_cmd_ptr->envelope.data,
                  gstk_instances_ptr[slot_id-1]->gstk_curr_envelope_info.env_data_bytes.data_buffer_ptr,
                  copy_len,
                  UIM_MAX_ENVELOPE_BYTES,
                  copy_len) <
      (gstk_instances_ptr[slot_id-1]->gstk_curr_envelope_info.env_data_bytes.data_len))
   {
   ...
   GSTK_UTIL_SELECTIVE_DUMP_BYTE_ARRAY("Envelope Retry",
                             &uim_cmd_ptr->envelope.data[uim_cmd_ptr->envelope.offset],
                             uim_cmd_ptr->envelope.num_bytes);   
   gstk_send_cmd_to_uim_server(uim_cmd_ptr); /* clear signal and call uim_cmd */
   if(gstk_instances_ptr[slot_id-1]->gstk_curr_envelope_info.retry_info.retry_counter <
        nv_data.env_retry_ext.max_retries)
   {
     ...
     gstk_instances_ptr[slot_id-1]->gstk_curr_envelope_info.retry_info.retry_counter++;
   }
   gstk_instances_ptr[slot_id-1]->gstk_curr_envelope_info.gstk_env_state = GSTK_SUB_ST_PENDING_ENV_RSP_FROM_CARD;
  }/* if ((gstk_status == GSTK_SUCCESS) && (uim_cmd_ptr != NULL))  */
  else
  {
...
void gstk_util_clear_retry_envelope_data(gstk_slot_id_enum_type slot)
{
  ...
  gstk_instances_ptr[(uint8)slot - 1]->gstk_curr_envelope_info.env_ref_id     = GSTK_MAX_ENVELOPE_COMMAND;
  ...
  gstk_cmd_enum_type *cmd_type_ptr)
{
  ...
  case GSTK_CMD_STK_CONTACTLESS_STATE_CHANGED:
    *cmd_type_ptr = GSTK_CONTACTLESS_STATE_CHANGED_REQ;
    break;
...
)
{
  ...
  if(gstk_shared_data.gstk_client_table[client_id-1].user_data_updated == TRUE)
  {
    /* Assigning to ZERO makes sure that the stale information is not used in 
       subsequent commands when there is no TR from client */
    gstk_shared_data.gstk_client_table[client_id-1].user_data = 0;
    /* Reset the corresponding flag as well */
    gstk_shared_data.gstk_client_table[client_id-1].user_data_updated = FALSE;
  }
...
PARAMETERS
  log_enabled:     [Input] Indicates if the array should be logged
...
)
{
  ...
  while (i < array_size)
  {
    ...
    memscpy(temp, sizeof(temp), &byte_array_ptr[i],
            (i + 9 <= array_size) ? 9 : array_size - i);
...
)
{
  ...
  case GSTK_REG_CATEGORY_R_CMD:
    is_evt_in_class = gstk_util_is_evt_in_category(evt_reg_bitmask,
                                                   (uint64)GSTK_CATEGORY_R_EVT_BITMASK);
    break;
  case GSTK_REG_CATEGORY_N_CMD:
    is_evt_in_class = gstk_util_is_evt_in_category(evt_reg_bitmask,
                                                   (uint64)GSTK_CATEGORY_N_EVT_BITMASK);
    break;
  default:
  ...
  *rem_bitmask_ptr = 0x00;
  ...
  case GSTK_REG_CATEGORY_M_CMD:
    ...
    *rem_bitmask_ptr = evt_reg_bitmask & ~GSTK_CATEGORY_M_EVT_BITMASK;
    break;
  case GSTK_REG_CATEGORY_R_CMD:
    *bitmask_ptr = evt_reg_bitmask & GSTK_CATEGORY_R_EVT_BITMASK;
    *rem_bitmask_ptr = evt_reg_bitmask & ~GSTK_CATEGORY_R_EVT_BITMASK;
    break;
  case GSTK_REG_CATEGORY_N_CMD:
    *bitmask_ptr = evt_reg_bitmask & GSTK_CATEGORY_N_EVT_BITMASK;
    *rem_bitmask_ptr = evt_reg_bitmask & ~GSTK_CATEGORY_N_EVT_BITMASK;
    break;
  default:
    UIM_MSG_ERR_1("Invalid proactive command class 0x%x", proactive_cmd_class);
    return GSTK_ERROR;
  ...
  gstk_cmd_check_orig_enum_type         cmd_origin)
{
  ...
  UIM_MSG_HIGH_3("MM IDLE pointer 0x%x MODE CHANGE pointer 0x%x  cmd_origin 0x%x",
  ...
  {
    UIM_MSG_HIGH_1("MM IDLE PTR 0x%x or MODE CHANGE PTR is NULL",
                   gstk_curr_inst_ptr->mm_idle_pri_ptr);
  ...
  /* When VPE feature/Non US CDMA loc. disable feature is enabled and 3GPP location status is Normal Service
     or Limited Service, check if MCC belongs to US or not and disable CDMA
     location status accordingly */
  if(((gstk_nv_get_feature_status(GSTK_CFG_FEATURE_SLOT_PROP_LOC_STATUS,
                                 gstk_curr_inst_ptr->slot_id) == TRUE) ||
       (gstk_nv_get_feature_2_status(GSTK_CFG_FEATURE_2_SLOT_DISABLE_NON_US_CDMA_LOCATION_INFO,
                                 gstk_curr_inst_ptr->slot_id) == TRUE)) &&
     ((mm_idle_ls == GSTK_NORMAL_SERVICE) ||
      (mm_idle_ls == GSTK_LIMITED_SERVICE)))
  {
    gstk_util_check_us_mcc(gstk_curr_inst_ptr->mm_idle_pri_ptr->mm_idle_state.mcc_mnc);
  }

  UIM_MSG_HIGH_5("Lowest = Highest MM IDLE LS 0x%x Mode Change LS 0x%x MM LS 0x%x cdma_loc_st_disabled 0x%x, RAT 0x%x",
                 mm_idle_ls,
                 mode_change_ls,
                 gstk_curr_inst_ptr->mm_idle_pri_ptr->mm_idle_state.location_state,
                 gstk_curr_inst_ptr->cdma_loc_st_disabled,
                 gstk_curr_inst_ptr->mm_idle_pri_ptr->mm_idle_state.rat);

  /* If CDMA location status is disabled, process 3GPP location info */
...
GSTK_UTIL_PROCESS_MM_OR_MODE_CHANGE_RETURN:
  ...
  if(cmd_origin == GSTK_MM_IDLE_ORIG)
  {
    if(gstk_memcpy(&(task_cmd_ptr->cmd.mode_change_ind),
                   gstk_curr_inst_ptr->mode_change_cmd_ptr,
       ...
       sizeof(gstk_cmd_type))
    {
      UIM_MSG_ERR_0("gstk_util_process_mm_or_mode_change(): gstk_memcpy failed");
      gstk_task_free_cmd_buf(task_cmd_ptr);
      return FALSE;
    }
                                         ...
                                         gstk_tag_enum_type     tag)
{
  ...
  switch(tag)
  {
    ...
    case GSTK_TAG_POWER_DOWN:
      ...
      for (i = 0; i < GSTK_MAX_CACHED_ENV_CMD_TO_CARD; ++i)
      {
        gstk_free(gstk_curr_inst_ptr->gstk_cached_env_cmds[i].data_buffer_ptr);
        gstk_curr_inst_ptr->gstk_cached_env_cmds[i].data_buffer_ptr = NULL;
        gstk_curr_inst_ptr->gstk_cached_env_cmds[i].data_len = 0;
      }
  ...
  switch(tag)
  {
    ...
    case GSTK_TAG_REFRESH_2G_INIT:
      /* Clean-up the TI value table */
      gstk_util_clean_up_ti_value_table(gstk_curr_inst_ptr->slot_id);
...
#ifdef FEATURE_CDMA      
      gstk_curr_inst_ptr->gstk_is_imsi_m_provisioned   = FALSE;
      /* Clean up OTASP data */     
...
/*===========================================================================
FUNCTION gstk_util_check_us_mcc
...
===========================================================================*/
void gstk_util_check_us_mcc(const uint8* loc_info_ptr)
{
  /* Check if MCC belongs to US */
  if (loc_info_ptr[0] == 0x13 &&
      (((loc_info_ptr[1] & 0x0F) == 0x00) ||
       ((loc_info_ptr[1] & 0x0F) == 0x01) ||
       ((loc_info_ptr[1] & 0x0F) == 0x02)))
  {
    gstk_curr_inst_ptr->cdma_loc_st_disabled = FALSE;
  }
  else
...
PARAMETERS
  ds_sys_status_info    [input] : DS system status structure
  gstk_sys_status_info  [output]: GSTK version of DS sys status structure
...
extern gstk_status_enum_type gstk_util_map_ds_to_gstk_sys_status(
  ds_sys_system_status_info_type       *ds_sys_status_info,
  gstk_ds_sys_system_status_info_type  *gstk_sys_status_info
)
{
  if (!ds_sys_status_info || !gstk_sys_status_info)
  {
    UIM_MSG_ERR_0("gstk_util_map_ds_to_gstk_sys_status: NULL input ptr");
    return GSTK_ERROR;
  }

  gstk_sys_status_info->so_mask = ds_sys_status_info->so_mask;

  switch (ds_sys_status_info->rat_value)
  {
    case DS_SYS_RAT_EX_NULL_BEARER:
      gstk_sys_status_info->rat_value = GSTK_DS_SYS_RAT_EX_NULL_BEARER;
      break;
    case DS_SYS_RAT_EX_3GPP_WCDMA:
      gstk_sys_status_info->rat_value = GSTK_DS_SYS_RAT_EX_3GPP_WCDMA;
      break;
    case DS_SYS_RAT_EX_3GPP_GERAN:
      gstk_sys_status_info->rat_value = GSTK_DS_SYS_RAT_EX_3GPP_GERAN;
      break;
    case DS_SYS_RAT_EX_3GPP_LTE:
      gstk_sys_status_info->rat_value = GSTK_DS_SYS_RAT_EX_3GPP_LTE;
      break;
    case DS_SYS_RAT_EX_3GPP_TDSCDMA:
      gstk_sys_status_info->rat_value = GSTK_DS_SYS_RAT_EX_3GPP_TDSCDMA;
      break;
    case DS_SYS_RAT_EX_3GPP_WLAN:
      gstk_sys_status_info->rat_value = GSTK_DS_SYS_RAT_EX_3GPP_WLAN;
      break;
    case DS_SYS_RAT_EX_3GPP_MAX:
      gstk_sys_status_info->rat_value = GSTK_DS_SYS_RAT_EX_3GPP_MAX;
      break;
    case DS_SYS_RAT_EX_3GPP2_1X:
      gstk_sys_status_info->rat_value = GSTK_DS_SYS_RAT_EX_3GPP2_1X;
      break;
    case DS_SYS_RAT_EX_3GPP2_HRPD:
      gstk_sys_status_info->rat_value = GSTK_DS_SYS_RAT_EX_3GPP2_HRPD;
      break;
    case DS_SYS_RAT_EX_3GPP2_EHRPD:
      gstk_sys_status_info->rat_value = GSTK_DS_SYS_RAT_EX_3GPP2_EHRPD;
      break;
    case DS_SYS_RAT_EX_3GPP2_WLAN:
      gstk_sys_status_info->rat_value = GSTK_DS_SYS_RAT_EX_3GPP2_WLAN;
      break;
    case DS_SYS_RAT_EX_3GPP2_MAX:
      gstk_sys_status_info->rat_value = GSTK_DS_SYS_RAT_EX_3GPP2_MAX;
      break;
    case DS_SYS_RAT_EX_WLAN:
      gstk_sys_status_info->rat_value = GSTK_DS_SYS_RAT_EX_WLAN;
      break;
    case DS_SYS_RAT_EX_WLAN_MAX:
      gstk_sys_status_info->rat_value = GSTK_DS_SYS_RAT_EX_WLAN_MAX;
      break;
    case DS_SYS_RAT_MAX:
      gstk_sys_status_info->rat_value = GSTK_DS_SYS_RAT_MAX;
      break;
    default:
      return GSTK_ERROR;
  }

  switch (ds_sys_status_info->technology)
  {
    case DS_SYS_NETWORK_3GPP:
      gstk_sys_status_info->technology = GSTK_DS_SYS_NETWORK_3GPP;
      break;
    case DS_SYS_NETWORK_3GPP2:
      gstk_sys_status_info->technology = GSTK_DS_SYS_NETWORK_3GPP2;
      break;
    case DS_SYS_NETWORK_WLAN:
      gstk_sys_status_info->technology = GSTK_DS_SYS_NETWORK_WLAN;
      break;
    case DS_SYS_NETWORK_MAX:
      gstk_sys_status_info->technology = GSTK_DS_SYS_NETWORK_MAX;
      break;
    default:
      return GSTK_ERROR;
  }
...
void gstk_util_check_slot_with_fetch_rejected(void)
{
  ...
  for(slot_id = 0; slot_id < gstk_shared_data.gstk_num_of_instances; slot_id++)
  {
       ...
       gstk_shared_data.fetch_allowed[slot_id])
    {
      ...
      if(gstk_instances_ptr[slot_id]->gstk_curr_envelope_info.gstk_env_state == GSTK_SUB_ST_PENDING_ENV_IN_QUEUE &&
         gstk_shared_data.envelope_cmd_ref_table[gstk_instances_ptr[slot_id]->gstk_curr_envelope_info.env_ref_id].expected_cmd_rsp_type == GSTK_CC_IND_RSP)
      {
        if(gstk_util_retry_envelope_cmd(slot_id+1) != GSTK_SUCCESS)
        {
          /* build envelope response back to the client */
          (void) gstk_build_error_envelope_report(
            gstk_instances_ptr[slot_id]->gstk_curr_envelope_info.env_ref_id,
            int32touint32((int32)gstk_shared_data.envelope_cmd_ref_table[gstk_instances_ptr[slot_id]->gstk_curr_envelope_info.env_ref_id].expected_cmd_rsp_type),
            GSTK_ENVELOPE_CMD_FAIL);
          gstk_util_clear_retry_envelope_data(gstk_util_get_sim_slot_id(gstk_instances_ptr[slot_id]->client_id));
        }
      } 
    }
  }
...
/*===========================================================================
FUNCTION gstk_util_get_ti_value_index_in_table

DESCRIPTION

  This function compares the TI value mapping table entries with
  - Call ID passed in (when call trigger is GSTK_CALL_TRIGGER_CM_GW or
    GSTK_CALL_TRIGGER_IMS_INT)
  - TI value passed in (when call trigger is GSTK_CALL_TRIGGER_IMS_EXT)
  If a successful match is found, the corresponding TI value mapping table
  index is returned

PARAMETERS
  call_evt_info: [Input] Info about the call event
  orig_ti      : [Input] Original TI value (applicable for
                         GSTK_CALL_TRIGGER_IMS_EXT call trigger)
...
RETURN VALUE
  uint8
  - 0 to GSTK_TI_VALUE_MAP_TABLE_SIZE -1 on success
  - GSTK_TI_VALUE_MAP_TABLE_SIZE on error (no entry found)
...
)
{
  ...
  GSTK_RETURN_ERROR_IF_NULL_PTR(gstk_curr_inst_ptr, GSTK_BAD_PARAM);

  orig_ti = orig_ti & 0x7F;
  orig_ti = orig_ti >> 4;

  for(index = 0; index < GSTK_TI_VALUE_MAP_TABLE_SIZE; index++)
  {
    if(gstk_curr_inst_ptr->gstk_ti_value_map_table[index].in_use == TRUE)
    {
      if((call_evt_info.call_evt_trigger == GSTK_CALL_TRIGGER_CM_GW)||
         (call_evt_info.call_evt_trigger == GSTK_CALL_TRIGGER_IMS_INT))
      {
        if((call_evt_info.call_evt_trigger == gstk_curr_inst_ptr->gstk_ti_value_map_table[index].call_evt_trigger) &&
           (gstk_curr_inst_ptr->gstk_ti_value_map_table[index].orig_ti_value_call_id ==
            call_evt_info.call_id))
        {
          UIM_SELECTIVE_MSG_HIGH_1(UIM_LOG_TOOLKIT, "Found matching Call ID, TI table index: 0x%x",
                                   index);
          return index;
        }
      }
      else if(call_evt_info.call_evt_trigger == GSTK_CALL_TRIGGER_IMS_EXT)
      {
        if((call_evt_info.call_evt_trigger == gstk_curr_inst_ptr->gstk_ti_value_map_table[index].call_evt_trigger) &&
           (gstk_curr_inst_ptr->gstk_ti_value_map_table[index].orig_ti_value_call_id ==
            orig_ti))
        {
          UIM_SELECTIVE_MSG_HIGH_1(UIM_LOG_TOOLKIT, "Found matching TI value, TI table index: 0x%x",
                                   index);
          return index;
        }
      }
  ...
  return index;
} /* gstk_util_get_ti_value_index_in_table */

/*===========================================================================
FUNCTION gstk_util_add_ti_value_index_in_table

DESCRIPTION

  This function allocates an index in the TI value table to the passed in
  TI and stores the
  - Call ID passed in (when call trigger is GSTK_CALL_TRIGGER_CM_GW or
    GSTK_CALL_TRIGGER_IMS_INT)
  - TI value passed in (when call trigger is GSTK_CALL_TRIGGER_IMS_EXT)
  On successful allocation, the corresponding TI value mapping table
  index is returned.
...
)
{
  ...
  if(call_evt_info.call_evt_trigger == GSTK_CALL_TRIGGER_CM_GW)
  {
    if(gstk_curr_inst_ptr->gstk_ti_value_map_table[orig_ti].in_use == FALSE)
    {
      gstk_curr_inst_ptr->gstk_ti_value_map_table[orig_ti].in_use = TRUE;
      gstk_curr_inst_ptr->gstk_ti_value_map_table[orig_ti].orig_ti_value_call_id =
        call_evt_info.call_id;
      gstk_curr_inst_ptr->gstk_ti_value_map_table[orig_ti].call_evt_trigger =
        GSTK_CALL_TRIGGER_CM_GW;
      return orig_ti;
    }
  }
  else if(call_evt_info.call_evt_trigger == GSTK_CALL_TRIGGER_IMS_EXT)
  ...
  for(index = 0; index < GSTK_TI_VALUE_MAP_TABLE_SIZE; index++)
  {
    if(gstk_curr_inst_ptr->gstk_ti_value_map_table[index].in_use == FALSE)
    {
      if((call_evt_info.call_evt_trigger == GSTK_CALL_TRIGGER_CM_GW)||
         (call_evt_info.call_evt_trigger == GSTK_CALL_TRIGGER_IMS_INT))
      {
      ...
      gstk_curr_inst_ptr->gstk_ti_value_map_table[index].in_use = TRUE;
      gstk_curr_inst_ptr->gstk_ti_value_map_table[index].call_evt_trigger =
        call_evt_info.call_evt_trigger;
      return index;
    }
  }
  return index;
} /* gstk_util_add_ti_value_index_in_table */

/*===========================================================================
FUNCTION gstk_remove_ti_value_list_in_table

DESCRIPTION

  This function clears the TI value mapping table for the TI values passed in

PARAMETERS
  data_ptr: [Input] List of Transaction Identifiers
...
SEE ALSO
  None
===========================================================================*/
static void gstk_remove_ti_value_list_in_table(gstk_transaction_id_type *data_ptr)
{
  uint8 index = 0;
  uint8 table_index = 0;
  ...
  for(index = 0; index < data_ptr->length; index++)
  {
    table_index = data_ptr->transaction_list[index];
    table_index = table_index & 0x7F;
    table_index = table_index >> 4;

    if(table_index < GSTK_TI_VALUE_MAP_TABLE_SIZE)
    {
      UIM_MSG_HIGH_1("TI table index cleared: 0x%x", table_index);
      memset(&gstk_curr_inst_ptr->gstk_ti_value_map_table[table_index], 0x00,
             sizeof(gstk_curr_inst_ptr->gstk_ti_value_map_table[table_index]));
    }
  }
} /* gstk_remove_ti_value_list_in_table */

/*===========================================================================
FUNCTION gstk_util_clean_up_ti_value_table

DESCRIPTION

  This function cleans-up the TI value table corresponding to the
  slot requested
...
SEE ALSO
  None
===========================================================================*/
void gstk_util_clean_up_ti_value_table(gstk_slot_id_enum_type slot_id)
{
  if (!GSTK_IS_VALID_SLOT_ID(slot_id))
  {
    return;
  ...
  GSTK_RETURN_IF_NULL_PTR(gstk_instances_ptr[(uint8)slot_id - 1]);

  memset(gstk_instances_ptr[(uint8)slot_id - 1]->gstk_ti_value_map_table,
         0x00,
         sizeof(gstk_instances_ptr[(uint8)slot_id - 1]->gstk_ti_value_map_table));
} /* gstk_util_clean_up_ti_value_table */

/*===========================================================================
FUNCTION gstk_util_update_transaction_identifier_list

DESCRIPTION

  This function updates the passed in Transaction ID list:
  - If terminal supports TS 31.111 Release 12 or greater and the Call event
    is due to a GW call/IMS call, then Transaction ID value shall be an
    identifier generated by the function to uniquely identify the call.
    (per update to Transaction identifier definition in TS 31.111, Rel 12).
  - Else, the TI list passed in is returned without modification


PARAMETERS
  data_ptr         : [Input]        Indicates the transaction id list
  modified_data_ptr: [Input/Output] Pointer to which the modified
                                    transaction ID list can be populated
  call_evt         : [Input]        Event Download
  call_evt_info    : [Input]        Info about the call event
...
SIDE EFFECTS
  The function allocates memory for the Modified TI value list
  (modified_data_ptr->transaction_list) and the caller is expected
  to free this memory.

SEE ALSO
  None
===========================================================================*/
gstk_status_enum_type gstk_util_update_transaction_identifier_list(
  const gstk_transaction_id_type   *data_ptr,
  gstk_transaction_id_type         *modified_data_ptr,
  gstk_evt_list_code_enum_type      call_evt,
  gstk_call_evt_info_type           call_evt_info
)
{
  gstk_nv_items_data_type   nv_data;
  uint8                     length = 0;
  uint8                     ti_index = GSTK_TI_VALUE_MAP_TABLE_SIZE;
  ...
  if(modified_data_ptr == NULL)
  {
     UIM_MSG_ERR_0("Modified Data Err: NULL");
     return GSTK_BAD_PARAM;
  }

  if((call_evt != GSTK_MT_CALL) &&
     (call_evt != GSTK_CALL_CONNECTED) &&
     (call_evt != GSTK_CALL_DISCONNECTED))
  {
    UIM_MSG_ERR_1("Invalid Event Download: 0x%x",call_evt);
    return GSTK_BAD_PARAM;
  }

  UIM_MSG_HIGH_4("Event: 0x%x, Event Trigger: 0x%x, Call ID: 0x%x, Data Length: 0x%x",
                 call_evt,
                 call_evt_info.call_evt_trigger,
                 call_evt_info.call_id,
                 data_ptr->length);

  GSTK_UTIL_SELECTIVE_DUMP_BYTE_ARRAY("Orig. TI",
                            data_ptr->transaction_list,
                            data_ptr->length);

  memset(modified_data_ptr, 0x00, sizeof(gstk_transaction_id_type));
  ...
  {
    modified_data_ptr->length = 0;
    return GSTK_SUCCESS;
  }

  modified_data_ptr->transaction_list =
    GSTK_CALLOC(data_ptr->length);

  if(modified_data_ptr->transaction_list == NULL)
  {
   return GSTK_MEMORY_ERROR;
  }

  /* CLean-up TI value table if CAT version is less than 12 */
  if(nv_data.cat_version < GSTK_CFG_CAT_VER12)
  {
    gstk_util_clean_up_ti_value_table(gstk_curr_inst_ptr->slot_id);
  }

  /* If terminal does not supportsupport TS 31.111 Release 12 or greater or
     the Call event is not due to a GW call/IMS call, return the TI list
     passed in without modification */
  if((nv_data.cat_version < GSTK_CFG_CAT_VER12) ||
     (call_evt_info.call_evt_trigger < GSTK_CALL_TRIGGER_CM_GW) ||
     (call_evt_info.call_evt_trigger > GSTK_CALL_TRIGGER_IMS_EXT))
  {
    modified_data_ptr->length = data_ptr->length;

    /* Clear any TI flags set in case of QC based IMS call */
    if(call_evt_info.call_evt_trigger == GSTK_CALL_TRIGGER_IMS_INT)
    {
      memset(modified_data_ptr->transaction_list, 0x00, modified_data_ptr->length);
    }
    else
    {
      (void)memscpy(modified_data_ptr->transaction_list,
                    modified_data_ptr->length,
                    data_ptr->transaction_list,
                    data_ptr->length);
    }

    for(length = 0; length < modified_data_ptr->length; length++)
    {
      UIM_SELECTIVE_MSG_HIGH_2(UIM_LOG_TOOLKIT, "Modified TI[0x%x]=0x%x",
                               length, modified_data_ptr->transaction_list[length]);
    }

    return GSTK_SUCCESS;
  }

  for(length = 0; length < data_ptr->length; length++)
  {
    /* Check if the current call already has an entry in the
       TI value mapping table */
    ti_index = gstk_util_get_ti_value_index_in_table(call_evt_info,
                                                     data_ptr->transaction_list[length]);

     /* Add current call into TI value mapping table */
    if(ti_index == GSTK_TI_VALUE_MAP_TABLE_SIZE)
    {
      ti_index = gstk_util_add_ti_value_index_in_table(call_evt_info,
                                                       data_ptr->transaction_list[length]);
    }

    /* IF TI value mapping table is full, stop */
    if(ti_index == GSTK_TI_VALUE_MAP_TABLE_SIZE)
    {
      break;
    }

    UIM_SELECTIVE_MSG_HIGH_1(UIM_LOG_TOOLKIT, "TI table index chosen: 0x%x", ti_index);

    ti_index = ti_index << 4;
    ti_index = ti_index | (data_ptr->transaction_list[length] & 0x80);

    modified_data_ptr->transaction_list[modified_data_ptr->length] = ti_index;
    modified_data_ptr->length++;
  }
  ...
  /* Clear calls in the TI value mapping table
     since Call is disconnected */
  if(call_evt == GSTK_CALL_DISCONNECTED)
  {
    gstk_remove_ti_value_list_in_table(modified_data_ptr);
  }
  return GSTK_SUCCESS;
} /* gstk_util_update_transaction_identifier_list */

/*===========================================================================
FUNCTION gstk_util_is_only_1x_cellular_rat_present_for_otasp

DESCRIPTION
  Checks if 1X is the only Cellular RAT present for OTASP
...
SEE ALSO
  None
===========================================================================*/
boolean gstk_util_is_only_1x_cellular_rat_present_for_otasp(void)
{
  uint32  sys_index          = 0;
  ...
  /* 1X is checked in CM SS to confirm 1X CS capability for OTASP. Other RATs 
     are checked in CM SS and DS system status event to confirm no other DATA 
     RATs available for BIP */
  ...
  UIM_SELECTIVE_MSG_HIGH_1(UIM_LOG_TOOLKIT,"DS_other_rats_present:0x%x", other_rats_present);
  /* When DS doesn't have any non-1x RATs, check in the Mode change indication for RAT availability */
#ifdef FEATURE_CDMA  
  if(other_rats_present == FALSE &&
     gstk_curr_inst_ptr->mode_change_cmd_ptr != NULL)
  {
    for (sys_index = 0; sys_index < GSTK_CM_NUM_STACKS; ++sys_index)
    {
      if(gstk_curr_inst_ptr->mode_change_cmd_ptr->mode_change_ind.cm_stack[sys_index].loc_status 
                                                                               == GSTK_NORMAL_SERVICE)
      {
        switch (gstk_curr_inst_ptr->mode_change_cmd_ptr->mode_change_ind.cm_stack[sys_index].access_tech)
        {
          case GSTK_ACCESS_TECH_CDMA:
            is_1x_present = TRUE;
            break;
  ...
  UIM_SELECTIVE_MSG_HIGH_2(UIM_LOG_TOOLKIT,"CM_is_1x_present:0x%x, other_rats_present:0x%x", is_1x_present, other_rats_present);
#endif /* FEATURE_CDMA */
  ...
  return FALSE;
} /* gstk_util_is_only_1x_cellular_rat_present_for_otasp */

/*===========================================================================
FUNCTION gstk_util_check_and_clear_pending_loc_info_retry

DESCRIPTION
  Checks if a Location Info Retry is pending in queue and if so clears the same
...
SEE ALSO
  None
===========================================================================*/
boolean gstk_util_check_and_clear_pending_loc_info_retry(void)
{
  uint32 env_ref_id = 0;

  GSTK_RETURN_ERROR_IF_NULL_PTR(gstk_curr_inst_ptr, FALSE);
  env_ref_id = gstk_curr_inst_ptr->gstk_curr_envelope_info.env_ref_id;
  /* If Location Info is being retried and pending, clear the same */
  if((gstk_curr_inst_ptr->gstk_curr_envelope_info.gstk_env_state == GSTK_SUB_ST_PENDING_ENV_IN_QUEUE) &&
     (gstk_shared_data.envelope_cmd_ref_table[env_ref_id].expected_cmd_rsp_type ==
  ...
  return FALSE;
} /* gstk_util_check_and_clear_pending_loc_info_retry */
 
/*===========================================================================
FUNCTION GSTK_UTIL_FIND_ATTACH_CTRL_CMD
 
DESCRIPTION
  This function is called by q_linear_delete_new. This function will return TRUE only when the
  command id is that of a GSTK_LOCATION_INFORMATION_IND.
 
PARAMETER
  item_ptr  [Input]:  ptr to queue item
  param_ptr [Input]:  not used as of now. This will be populated as NULL
                      by q_linear_delete_new()
...
SEE ALSO
  None
===========================================================================*/
static int gstk_util_find_attach_ctrl_cmd(
   void *item_ptr,
   void *param_ptr
)
{
  ...
  if((item_ptr == NULL)||(gstk_curr_inst_ptr == NULL))
  {
    UIM_MSG_HIGH_0("item_ptr or gstk_curr_inst_ptr is NULL");
    return 0;
  }
  ...
  if(task_cmd_ptr->cmd.general_cmd.message_header.command_group == GSTK_ENVELOPE_CMD &&
     task_cmd_ptr->cmd.general_cmd.message_header.command_id == GSTK_LOCATION_INFORMATION_IND &&
     task_cmd_ptr->cmd.general_cmd.message_header.sim_slot_id == slot &&
     task_cmd_ptr->cmd.general_cmd.message_header.user_data == GSTK_MAX_PROACTIVE_COMMAND)
  {
    UIM_MSG_HIGH_0("GSTK_LOCATION_INFORMATION_IND found");
    return 1;
  }
  else
  {
...
} /* gstk_util_find_attach_ctrl_cmd */
 
/*===========================================================================
FUNCTION GSTK_UTIL_FIND_AND_COPY_ATTACH_CTRL_CMD
 
DESCRIPTION
  This function is to find and copy the attach control cmd in the queue.
...
SEE ALSO
  None
===========================================================================*/
void gstk_util_find_and_copy_attach_ctrl_cmd(void)
{
  gstk_task_cmd_type     *task_cmd_ptr   = NULL;
  ...
  rex_leave_crit_sect(&gstk_shared_data.gstk_crit_sect);
  /* Leaving gstk critical section */
  if(task_cmd_ptr != NULL && gstk_curr_inst_ptr != NULL)
  {
    gstk_free(gstk_curr_inst_ptr->attach_ctrl_p);
    gstk_curr_inst_ptr->attach_ctrl_p =
    ...
    {
      (void)rex_set_sigs(
            (rex_tcb_type *)task_cmd_ptr->cmd.attach_ctrl_ind.attach_ctrl.task_tcb,
            (rex_sigs_type)task_cmd_ptr->cmd.attach_ctrl_ind.attach_ctrl.sig_to_wait);
    }
    else
    {
      (void)gstk_memcpy(gstk_curr_inst_ptr->attach_ctrl_p,
                        &task_cmd_ptr->cmd.attach_ctrl_ind.attach_ctrl,
                        sizeof(gstk_attach_ctrl_type),
                        sizeof(gstk_attach_ctrl_type),
                        sizeof(gstk_attach_ctrl_type));
    }
  }
  else if(task_cmd_ptr != NULL && gstk_curr_inst_ptr == NULL)
  {
    (void)rex_set_sigs(
          (rex_tcb_type *)task_cmd_ptr->cmd.attach_ctrl_ind.attach_ctrl.task_tcb,
          (rex_sigs_type)task_cmd_ptr->cmd.attach_ctrl_ind.attach_ctrl.sig_to_wait);
  }
} /* gstk_util_find_and_copy_attach_ctrl_cmd */

/* ==========================================================================
   FUNCTION:      GSTK_UTIL_CONVERT_FROM_UIM_SLOT_TO_MMGSDI_SLOT
   ...
   SIDE EFFECTS:
     None
==========================================================================*/
gstk_status_enum_type gstk_util_convert_from_uim_slot_to_mmgsdi_slot(
