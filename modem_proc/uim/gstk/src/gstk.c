...
Copyright (c) 2003 - 2020 QUALCOMM Technologies, Inc (QTI) and its licensors.
...
$Header: //components/rel/uim.mpss/6.1.0/gstk/src/gstk.c#4 $ $DateTime: 2020/04/13 23:25:26 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
04/08/20   dt      Check index 0 to Register for PCO 
01/24/20   ks      Add support to check NFW lock status
06/06/19   ks      Clear User data before sending the TR report to client
09/03/18   sg      Move wlan_info global to heap
08/17/18   rps     Add support for mini dump phase 2
12/21/17   gs      Do not pet the dog after the task power down
12/07/17   gs      OTASP Call on Slot#2
10/09/17   dt      Do not process BIP over cellular when cellular bearer is null on index 0
09/19/17   shr     Add support for caching RR/RRC PLMN info when VPE is off
08/31/17   srm     Fixed compilation errors after non-5G *RAT disablement
08/08/17   shr     Add support for correctly handling VPE in multi-SIM scenario
08/01/17   me      De-register for GSTK task ready signals only once
05/25/17   dt      Removing GSTK_MAX_TP_DATA_SIZE 
05/23/17   shr     Register for MT Call CM events during GSTK task init
05/19/17   dt      Changing gstk_curr_inst_ptr to gstk_instances_ptr
05/05/17   dt      Not allowing mutiple Client registrations for same client on a slot
04/17/17   dt      Updating gstk_shared_data.qmi_cat_enabled
03/27/17   dt      Add psm status to TP from MMGSDI
03/14/17   dt      Featurize IMSI_M and cdma_loc_st_disabled references
01/20/17   vr      Reset local height cache value instead of global
01/18/17   vr      Avoid sending duplicate LS/Access Tech Envelopes to card
12/23/16   dt      Fix inconsistent failure while notifying the RRC message
11/07/16   shr     Migration to MSGR VARIANT from Inst ID
11/02/16   vr      Delay Pcmd processing until CM and QMI task ready
10/07/16   shr     GSTK/ESTK F3 reduction
10/03/16   dd      Removed lint comments
09/23/16   gs      F3 reduction / cleanup
09/16/16   gs      Fix compilation warning
...
07/11/16   vr      Added feature to Disable CDMA location
...
06/02/16   shr     F3 reduction: Dumping Arrays
06/02/16   shr     Fix issue with RRC registration
...
05/11/16   shr     F3 reduction
05/11/16   gm      Keeping 3gpp2 code under FEATURE_CDMA
03/07/16   shr     Added support for PCO Envelope
03/22/16   gm      Removal of support for WLAN events for IOE devices
02/29/16   gs      Removal of Dynamic TP Mechanism
12/10/15   lm      Remove test framework feature flag for dog_hb_rex include
...
10/26/15   vr      Fix clearing envelope retry timer before initializing it
...
07/02/15   shr     Added support for W+W
06/25/15   gm      L4 compiler warning
06/17/15   bd      Get active data subscription information from
                   CM_PH_EVENT_SUBSCRIPTION_PREF_INFO
06/05/15   gm      Split ME and slot specific NV
05/27/15   vr      Cleanup fetch flags before queuing hotswap command
05/25/15   vr      Don't skip CC cmd when CC retry is diabled and when
                   evt dl is getting retried
05/20/15   vr      Fix compiler warnings
05/11/15   vr      Change ESTK UT notify handling
...
04/08/15   vr      Check pending proactive cmd after CM/QMI ready
...
03/11/15   vr      UT multiple SIM SAT support
...
rex_tcb_type   * gstk_tcb_ptr = NULL;
...
void gstk_process_rrc_msgr_cmd()
{
  ...
  /* LTE RRC Variant, by default set to 0 (i.e. AS_ID_1) */
  uint8                       variant       = 0;
  ...
  {
    /* Retrieve the LTE RRC Variant */
    variant = MSGR_GET_VARIANT(&msg_ptr->rrc_msgr_cmd.msg_hdr);

    /* Clear the Variant before Message is processed further */
    MSGR_CLEAR_VARIANT(msg_ptr->rrc_msgr_cmd.msg_hdr.id);
      ...
      case LTE_RRC_SERVICE_IND:
            ...
            LTE_RRC_SVC_STATUS_AVAILABLE)
        {
         ...
         (void)gstk_memcpy(cell_info.lac,
          ...
          /* Retrieve AS_ID by converting the Variant provided by the LTE RRC */
          cell_info.as_id = (sys_modem_as_id_e_type)variant;
...
)
{
...
#ifdef FEATURE_CDMA
  if(gstk_nv_get_feature_2_status(
        GSTK_CFG_FEATURE_2_SLOT_DISABLE_CDMA_LOCATION_INFO,
        slot) == FALSE)
  {
  ...
  if (ss_info_ptr->stack_info[0].changed_fields & (CM_SS_CSG_INFO_CHANGED_MASK))
  {
    ...
    {
      UIM_MSG_HIGH_1("Unsupported CAT version: 0x%x",
                     nv_data.cat_version);
    }
  }
...
)
{
  ...
  case MMGSDI_SESSION_OPEN_EXT_CNF:
    ...
    {
       UIM_MSG_ERR_0("status is not success");		
    }
    break;
  ...
  case MMGSDI_READ_CNF:
        ...
        cnf_ptr->read_cnf.access.file.path_type.path_buf[2] == GSTK_CSIM_IMSI_M_PATH_BUF_3)
    {
      ...
      else
      {
        ...
        gstk_instances_ptr[(uint8)slot - 1]->gstk_is_imsi_m_provisioned = TRUE;
...
)
{
  ...
  {
    /* Clean up fetch flags */
    if(gstk_shared_data.fetch_is_in_progress == (uint8)slot)
    {
      gstk_shared_data.fetch_is_in_progress = 0;
    }
    if(gstk_instances_ptr[(uint8)slot - 1] != NULL)
    {
      gstk_instances_ptr[(uint8)slot - 1]->is_fetch_ignored = FALSE;
    }
  }
...
)
{
  ...
  switch (subs_evt)
  {
    case CM_PH_EVENT_SUBSCRIPTION_PREF_INFO:
      UIM_MSG_HIGH_4("gstk_cm_subs_evt_cb(): subs_id: 0x%x, sess_t_1x: 0x%x, sess_t_gw: 0x%x, is_default_data: 0x%x",
      ...
      /* Update active data subscription */
      if (subs_info_ptr->is_default_data)
      {
        gstk_shared_data.active_data_subs = subs_info_ptr->subscription_id;
      }
...
)
{
  ...
  UIM_MSG_HIGH_1("CDMA Location Status envelope timer expiry for slot %d",
                 slot_index);
...
)
{
  ...
  /* Send Location Info Envelope (with PLMN from RR) to card if required - the same
     PLMN info shall be used to construct the Envelope for all slots, since the 
     RR PLMN info is common for the device */
  for(slot_index = 0; slot_index < gstk_shared_data.gstk_num_of_instances; slot_index++)
  {
    if(gstk_instances_ptr[slot_index] == NULL)
    {
      continue;
    }
    
    UIM_MSG_HIGH_3("New PLMN from RR, slot: 0x%x, First Loc Info sent: 0x%x, TP response received 0x%x",
                   (slot_index + 1),
                   gstk_instances_ptr[slot_index]->is_first_loc_info_envelope_sent,
                   gstk_instances_ptr[slot_index]->is_tp_rsp_received);

    /* If first Loc Info Envelope has not yet been sent to card, then check
       if an Envelope can be sent */
    if (gstk_instances_ptr[slot_index]->is_first_loc_info_envelope_sent == FALSE)
    {
      /* If PLMN info is valid, check if send a Loc Info Envelope can
         be sent to card */
      if((memcmp(gstk_instances_ptr[slot_index]->gstk_last_loc_info_sent,
                cell_info_ptr->plmn.identity,
                GSTK_MAX_PLMN_LEN) != 0) &&
         (memcmp(cell_info_ptr->plmn.identity,
                ...
                GSTK_MAX_PLMN_LEN) != 0))
      {
        /* If VPE feature is enabled, and Terminal Profile transaction
           has been completed, send VPE envelope to the card */
        if (gstk_nv_get_feature_status(
              GSTK_CFG_FEATURE_SLOT_PROP_LOC_STATUS,
              (gstk_slot_id_enum_type)(slot_index + 1)) &&
            gstk_instances_ptr[slot_index]->is_tp_rsp_received)
        {
          (void)gstk_send_envelope_loc_info_command(
                  gstk_instances_ptr[slot_index]->client_id,
                  cell_info_ptr->plmn.identity,
                  0);
        }
        /* Copy the PLMN so that VPE can be sent after TP DL reponse is
           received from card while VPE feature is enabled */
        else
        {
          (void)gstk_memcpy(gstk_instances_ptr[slot_index]->gstk_last_loc_info_sent,
                            cell_info_ptr->plmn.identity,
                            GSTK_MAX_PLMN_LEN,
                            GSTK_MAX_PLMN_LEN,
                            GSTK_MAX_PLMN_LEN);
...
)
{
  ...
  if((event_name != DS_SYS_EVENT_SYSTEM_STATUS_EX) &&
     ...
     (event_name != DS_SYS_EVENT_WLAN_IP_ADDR_CHANGE) &&
     (event_name != DS_SYS_EVENT_APN_MSISDN_INFO_CHANGE))
  {
     return;
  }
  ...
  if(event_name == DS_SYS_EVENT_SYSTEM_STATUS_EX)
  {
    ...
    task_cmd->cmd.ds_sys_status_ex_ind.ds_sys_status_info.num_avail_sys =
      ((ds_sys_system_status_ex_type*)event_info_ptr)->num_avail_sys;

    for (i = 0;
         i < task_cmd->cmd.ds_sys_status_ex_ind.ds_sys_status_info.num_avail_sys;
         i++)
    {
      status =
        gstk_util_map_ds_to_gstk_sys_status(&(((ds_sys_system_status_ex_type*)event_info_ptr)->
                                              avail_sys[i]),
                                            &(task_cmd->
  ...
  else if(event_name == DS_SYS_EVENT_WLAN_IP_ADDR_CHANGE)
  {
    task_cmd->cmd.ds_wlan_ip_addr_change_ind.message_header.command_id =
      GSTK_DS_WLAN_IP_ADDR_CHANGE_IND;
    task_cmd->cmd.ds_wlan_ip_addr_change_ind.ds_wlan_ip_addr_info =
      *((ds_sys_wlan_ip_addr_type*)event_info_ptr);
  }
  else if(event_name == DS_SYS_EVENT_APN_MSISDN_INFO_CHANGE)
  {
    task_cmd->cmd.ds_msisdn_change_ind.subscription_id = subscription_id;
    task_cmd->cmd.ds_msisdn_change_ind.message_header.command_id =
      GSTK_DS_MSISDN_CHANGE_IND;
    task_cmd->cmd.ds_msisdn_change_ind.ds_msisdn_info_per_apn =
      *((ds_sys_msisdn_info_per_apn_type*)event_info_ptr);
  }
...
void gstk_init( void )
{
  ...
  gstk_tcb_ptr = rcinit_lookup_rextask("gstk");
  ...
  for (i = 0; i < GSTK_MAX_CLIENT; i++) {
    ...
    gstk_shared_data.gstk_client_table[i].user_data_updated = FALSE;
...
void gstk_init_post_task_startup( void )
{
  ...
  UIM_MSG_LOW_0(" gstk_init_post_task_startup");

  (void)mmgsdi_get_hw_slots(&slot_count);
  ...
  /* Allocate memory for wlan info */
  gstk_shared_data.wlan_info_ptr = GSTK_CALLOC(sizeof(gstk_wlan_info_type));
...
/* To avoid crashes on Multi SIM NV setting with lesser SIM build variant */
  ...
  for (i = 0; i < gstk_shared_data.gstk_num_of_instances; ++ i)
  {
    for (j = 0; j < GSTK_MAX_CLIENT;j++)
    {
      if (gstk_shared_data.gstk_client_table[j].is_free)
      {
        ...
        gstk_shared_data.gstk_client_table[j].user_data_updated = FALSE;
  ...
  for (i = 0; i < gstk_shared_data.gstk_num_of_instances; ++ i)
  {
    ...
    gstk_instances_ptr[i]->gstk_curr_envelope_info.env_ref_id = GSTK_MAX_ENVELOPE_COMMAND;
  ...
  /* Memset MM-GSTK location information table */
    ...
    for (j = 0; j < GSTK_MAX_CACHED_ENV_CMD_TO_CARD; ++j)
    {
      gstk_free(gstk_instances_ptr[i]->gstk_cached_env_cmds[j].data_buffer_ptr);
      gstk_instances_ptr[i]->gstk_cached_env_cmds[j].data_buffer_ptr = NULL;
      gstk_instances_ptr[i]->gstk_cached_env_cmds[j].data_len = 0;
    }
  }
  ...
  /* Init QMI CAT globals and queues and to register with GSTK,
     qmi cat pre initialize before gstk_nv_init_task */
  gstk_shared_data.qmi_cat_enabled =  qmi_cat_init_pre_startup();
...
LOCAL static void gstk_cm_init( void )
{
  ...
  {
    UIM_MSG_ERR_1("cm_client_init() returned bad status %d",
                  (int32)cm_init_return_status);
    return;
  }
  ...
  /* register for incoming call and call connected events */
  ...
  /* register for setup indication event */
  cm_reg_return_status = cm_mm_client_call_reg(
                    gstk_shared_data.gstk_client_id_from_cm,     /* pointer to the client struct */
                    gstk_cm_call_event_cb,      /* pointer to a cb function for call events */
                    CM_CLIENT_EVENT_REG,        /* event registration */
  ...
  if (cm_reg_return_status == CM_CLIENT_OK)
  {
    if (!cm_stats_set_modem_info_mask(NULL,
                                      ...
                                      SYS_GERAN_ENG_MODE_SCELL_INFO))
    {
      UIM_MSG_ERR_0( "Fail to set CM Stats modem GSM info mask" );
    }
  }
  else
  {
...
void gstk_wms_init( void )
{
  ...
  do
  {
    ...
    {
    UIM_MSG_ERR_1("Failed to init GSTK WMS client. (0x%x)", wms_client_err);
      return;
    }
...
LOCAL static void gstk_msgr_init( void )
{
  ...
  do {
    ...
    /* Register for LTE RRC messages per variant */
    for (index = 0;
         (index < gstk_shared_data.gstk_num_of_instances) &&
         (index < GSTK_MAX_NUM_OF_INSTANCES);
         index++)
    {
      e_status = msgr_register_variant(MSGR_UIM_STK,
                                       &gstk_shared_data.msgr_rrc_client,
                                       MSGR_ID_NONE,
                                       LTE_RRC_SERVICE_IND,
                                       index);
...
)
{
  ...
  ds_sys_msisdn_info_per_apn_type              msisdn_info_per_apn;
  ...
  GSTK_RETURN_ERROR_IF_NULL_PTR(gstk_instances_ptr[(uint32)slot - 1], GSTK_ERROR);

  memset(&pdn_params, 0, sizeof(ds_sys_ioctl_3gpp_lte_attach_pdn_params_type)); 
  memset(&msisdn_info_per_apn, 0, sizeof(ds_sys_msisdn_info_per_apn_type)); 

  /* Validate SUB ID */
  if(!((sub_id == DS_SYS_PRIMARY_SUBS) ||
  ...
  dss_ret_val = ds_sys_ioctl_ex(DS_SYS_TECH_3GPP,
             ...
             pdn_params.apn_string_len) != 0))
  {
    ...
    if(gstk_instances_ptr[(uint32)slot - 1]->pco_info.sub_id != DS_SYS_DEFAULT_SUBS)
    {
      ...
      (void)ds_sys_event_dereg_ex2(DS_SYS_TECH_ALL,
                                   DS_SYS_EVENT_APN_MSISDN_INFO_CHANGE,
                                   gstk_instances_ptr[(uint32)slot - 1]->pco_info.sub_id,
                                   gstk_ds_sys_evt_cb,
                                   (void*)gstk_instances_ptr[(uint32)slot - 1]->pco_info.apn_string,
                                   &dss_errno);
  ...
  if(gstk_memcpy(msisdn_info_per_apn.apn_name,
                 pdn_params.apn_string,
                 pdn_params.apn_string_len,
                 sizeof(msisdn_info_per_apn.apn_name),
                 sizeof(pdn_params.apn_string)) <
      pdn_params.apn_string_len)
  {
    return GSTK_ERROR;
  ...
  dss_ret_val = ds_sys_ioctl_ex(DS_SYS_TECH_ALL,
                                DS_SYS_IOCTL_GET_APN_MSISDN_INFO,
                                sub_id,
                                (void*)&msisdn_info_per_apn,
                                &dss_errno);

  if(0 > dss_ret_val)
  {
    UIM_MSG_ERR_2("DS_SYS_IOCTL_GET_APN_MSISDN_INFO, ret_val=0x%x, err=0x%x",
                  dss_ret_val, dss_errno);
    return GSTK_ERROR;
  }
  ...
  (void)gstk_build_pco_envelope(&msisdn_info_per_apn.msisdn_info,
                                slot);
  return GSTK_SUCCESS;
} /* gstk_ds_pco_reg */
...
static gstk_status_enum_type gstk_ds_sys_init(void)
{
  ...
  (void)gstk_ds_event_reg_wlan_ip_addr_change();
...
void gstk_task( dword dummy )
{
  ...
  stop_sig.signal = UIM_GSTK_TCB;
  stop_sig.mask = GSTK_TASK_STOP_SIG;

  gstk_group = rcinit_lookup_group_rextask(UIM_GSTK_TCB);
  rcinit_register_term_group(gstk_group, RCEVT_SIGEX_TYPE_SIGREX, &stop_sig);

  rcevt_siginfo_qmi_modem.signal = UIM_GSTK_TCB;
  rcevt_siginfo_qmi_modem.mask   = GSTK_RCEVT_QMI_MODEM_TASK_READY_SIG;
  mcs_rcevt_register_sigex_name("QMI_MODEM: READY", RCEVT_SIGEX_TYPE_SIGREX, &rcevt_siginfo_qmi_modem);

  rcevt_siginfo_cm.signal = UIM_GSTK_TCB;
  rcevt_siginfo_cm.mask   = GSTK_RCEVT_CM_TASK_READY_SIG;
  mcs_rcevt_register_sigex_name("cm:ready", RCEVT_SIGEX_TYPE_SIGREX, &rcevt_siginfo_cm);

#ifdef FEATURE_SEGMENT_LOADING
  if(gstk_pi_wcdma != NULL)
#endif /* FEATURE_SEGMENT_LOADING */
  {
    rcevt_siginfo_rrc.signal = UIM_GSTK_TCB;
    rcevt_siginfo_rrc.mask   = GSTK_RCEVT_RRC_TASK_READY_SIG;
    mcs_rcevt_register_sigex_name("rrc:ready", RCEVT_SIGEX_TYPE_SIGREX, &rcevt_siginfo_rrc);
  }
  ...
  {
#ifdef FEATURE_TDSCDMA
    rcevt_siginfo_tds_rrc.signal = UIM_GSTK_TCB;
    rcevt_siginfo_tds_rrc.mask   = GSTK_RCEVT_TDS_RRC_TASK_READY_SIG;
    mcs_rcevt_register_sigex_name("TDSRRC:ready", RCEVT_SIGEX_TYPE_SIGREX, &rcevt_siginfo_tds_rrc);
#endif /*FEATURE_TDSCDMA*/
  }
...
#error code not present
...
#endif /*FEATURE_TDSCDMA*/
                                              | ESTK_NOTIFY_QMI_LOC_SIG
  ;

  mcs_rcevt_signal_name("GSTK:READY");
  ...
  for ( ; ; )
  {
    ...
    else if (gstk_sigs & GSTK_RCEVT_QMI_MODEM_TASK_READY_SIG)
    {
      ...
      {
        mcs_rcevt_unregister_sigex_name("QMI_MODEM: READY", RCEVT_SIGEX_TYPE_SIGREX, &rcevt_siginfo_qmi_modem);
        /* Perform qmi_cat_init irrespective of whether FTM NV is set or not */
        qmi_cat_init_post_startup();
#ifdef FEATURE_GSTK_QMI_LOC
        /* Register with QMI_LOC for position related information */
        estk_qmi_loc_client_init();
#endif
    ...
    else if (gstk_sigs & GSTK_RCEVT_CM_TASK_READY_SIG)
    {
      ...
      if(!(gstk_shared_data.ready_tasks & (00000001 << (uint8)GSTK_CM_TASK)))
      {
        mcs_rcevt_unregister_sigex_name("cm:ready", RCEVT_SIGEX_TYPE_SIGREX, &rcevt_siginfo_cm);
        /* Register with CM */
        gstk_cm_init();
    ...
    else if (gstk_sigs & GSTK_RCEVT_RRC_TASK_READY_SIG)
    {
      ...
      {
        mcs_rcevt_unregister_sigex_name("rrc:ready", RCEVT_SIGEX_TYPE_SIGREX, &rcevt_siginfo_rrc);
        /* Register with RRC to receive cell change indications for
         * Provide Local Info - Location Information proactive command
         * Location Status - envelope cmd
         */
        ...
        if(gstk_pi_wcdma != NULL)
        {
          ...
          (void)IWCDMA_rrc_register_serving_cell_change_ind_cb_sub(gstk_pi_wcdma, SYS_MODEM_AS_ID_1, gstk_rrc_cell_change_ind_cb);
          (void)IWCDMA_rrc_register_serving_cell_change_ind_cb_sub(gstk_pi_wcdma, SYS_MODEM_AS_ID_2, gstk_rrc_cell_change_ind_cb);
#else
...
#else        
#ifdef FEATURE_DUAL_WCDMA
        (void)rrc_register_serving_cell_change_ind_cb_sub(SYS_MODEM_AS_ID_1, gstk_rrc_cell_change_ind_cb);
        (void)rrc_register_serving_cell_change_ind_cb_sub(SYS_MODEM_AS_ID_2, gstk_rrc_cell_change_ind_cb);
#else
    ...
    else if (gstk_sigs & GSTK_RCEVT_TDS_RRC_TASK_READY_SIG)
    {
      ...
      {
        mcs_rcevt_unregister_sigex_name("TDSRRC:ready", RCEVT_SIGEX_TYPE_SIGREX, &rcevt_siginfo_tds_rrc);
#ifdef FEATURE_SEGMENT_LOADING
        if(gstk_pi_tdscdma != NULL)
        {
    ...
    else if(gstk_sigs & ESTK_NOTIFY_QMI_LOC_SIG)
    {
      (void)rex_clr_sigs(UIM_GSTK_TCB, ESTK_NOTIFY_QMI_LOC_SIG);
#ifdef FEATURE_GSTK_QMI_LOC
      estk_qmi_loc_client_process_signal();
#endif
    }
  }
#if defined FEATURE_UIM_TEST_FRAMEWORK
...
SEE ALSO
  None
===========================================================================*/
static int gstk_compare_specific_cmds(
   void *item_ptr,
   void *param_ptr
)
{
  ...
  /* Skip proactive command processing until CM and QMI tasks are ready */
  if ((task_cmd_ptr->cmd.general_cmd.message_header.command_group == GSTK_PROACTIVE_CMD) &&
      !((gstk_shared_data.ready_tasks & (00000001 << (uint8) GSTK_CM_TASK)) &&
       (gstk_shared_data.ready_tasks & (00000001 << (uint8) GSTK_QMI_MODEM_TASK))))
  {
    UIM_MSG_HIGH_1("Skip proactive command processing until CM and QMI tasks ready : 0x%x!", gstk_shared_data.ready_tasks);
    return 0;
  }
  ...
  if(task_cmd_ptr->cmd.general_cmd.message_header.command_group == GSTK_ENVELOPE_CMD)
  {
    ...
    /* Do not process new Envelope commands if Envelope Command
       Ref. Table is full: processing the command would lead
       to Envelope not being sent to card (since ref. table has no
       storage space for the Envelope). The envelope will be processed
       once a Ref. Table entry becomes available when UIM driver responds
       back to a previously queued Envelope */
    if(gstk_util_is_env_cmd_ref_table_full())
    {
      return 0;
    }
    ...
    if (gstk_instances_ptr[sim_slot_id - 1]->gstk_curr_envelope_info.gstk_env_state != GSTK_SUB_ST_PENDING_ENV_NONE)
    {
      ...
      switch((gstk_cmd_enum_type)task_cmd_ptr->cmd.general_cmd.message_header.command_id)
      {
        ...
        case GSTK_PCO_INFORMATION_IND:
          return 0;
        case GSTK_CC_IND:
          if(task_cmd_ptr->cmd.cc_envelope_ind.cc_requeue)
          {
            return 0;
          }
        case GSTK_MO_SMS_CTRL_IND:
          if(task_cmd_ptr->cmd.mo_sms_ctrl_envelope_ind.mo_sms_requeue)
          {
            return 0;
          }
          break;
...
)
{
  ...
  if (gstk_io_ctrl(
        ...
        &slot) != GSTK_SUCCESS)
  {
    slot = GSTK_SLOT_ID_MAX;
  }

  UIM_MSG_HIGH_9("In gstk_send_cell_change_ind, RRC Plmn MCC: 0x%x 0x%x 0x%x Plmn MNC: 0x%x 0x%x 0x%x LAC: 0x%x 0x%x, Cell ID: 0x%x",
  ...
  /* Send Location Info Envelope (with PLMN from RRC) to card if required - the same
     PLMN info shall be used to construct the Envelope for all slots, since the 
     RR PLMN info is common for the device */
  for(slot_index = 0; slot_index < gstk_shared_data.gstk_num_of_instances; slot_index++)
  {
    ...
    UIM_MSG_HIGH_3("New PLMN from RRC, slot: 0x%x, First Loc Info sent: 0x%x, TP response received 0x%x",
                   (slot_index + 1),
                   gstk_instances_ptr[slot_index]->is_first_loc_info_envelope_sent,
                   gstk_instances_ptr[slot_index]->is_tp_rsp_received);
  ...
  /* Validate Slot before proceeding further */
  if (!GSTK_IS_VALID_SLOT_ID(slot))
  {
    return;
  }
  ...
  UIM_MSG_HIGH_4("Rat: 0x%x, Apps enabled: 0x%x, Card Protocol: 0x%x, First Loc Info Env Sent: 0x%x",
                 rat,
  ...
  else
  {
               ...
               GSTK_MAX_LAC_LEN) == 0))
    {
      if(memcmp(cell_id_info_p->cell_id,
                ...
                gstk_instances_ptr[slot - 1]->gstk_loci_table.cell_info.cell_len) == 0)
      {
      ...
      (void)memscpy(cell_id_info_p->cell_id,
             gstk_instances_ptr[slot - 1]->gstk_loci_table.cell_info.cell_len,
             gstk_instances_ptr[slot - 1]->gstk_loci_table.cell_info.cell_id,
             gstk_instances_ptr[slot - 1]->gstk_loci_table.cell_info.cell_len);
...
)
{
  ...
  (void)memscpy(cell_id_info_p->cell_id,
                ...
                gstk_instances_ptr[slot - 1]->rrc_loci_table[i].cell_info.cell_len);
                                      ...
                                      gstk_cdma_loci_ext_type loc_info)
{
  ...
  {
    UIM_MSG_HIGH_0("Invalid Advance loci");
...
} /* gstk_cache_advance_location_info */
...
)
{
  ...
  UIM_MSG_HIGH_0(" IN gstk_cm_ph_error_event_cb ");
  ...
  {
    UIM_MSG_ERR_0("Invalud data from CM");
    return;
  }
...
FUNCTION GSTK_MEM_ALLOC

DESCRIPTION
  GSTK calls GSTK_CALLOC to request memory from modem heap.

PARAMETERS
  size:  size of data to which memory is allocated.

DEPENDENCIES
  None.

RETURN VALUE
  A pointer to allocated heap memory.

SIDE EFFECTS
  A pointer is allocated.
===========================================================================*/

void * gstk_mem_alloc(size_t size)
{
  void *ptr = GSTK_CALLOC(size);

  if (ptr == NULL)
  {
    ERR_FATAL("Couldn't allocate memory. Heap exhausted.", 0, 0, 0);
  }
  return ptr;
} /* gstk_mem_alloc */
...
)
{
  ...
  { 
    ...
    {
      if ((gstk_shared_data.gstk_client_table[i].client_type ==
          cmd_ptr->client_type) && (gstk_shared_data.gstk_client_table[i].sim_slot_id ==
          cmd_ptr->message_header.sim_slot_id))
      {
        client_ret_status = GSTK_DUPLICATE_REGISTRATION;
        break;
      }
    }
  }
  /* Note : Client Id 0 is reserved for GSTK */
  if (client_ret_status != GSTK_DUPLICATE_REGISTRATION)
  {
    for (i = 1; i < GSTK_MAX_CLIENT; i++)
    {
      if (gstk_shared_data.gstk_client_table[i].is_free)
      {
        ...
        gstk_shared_data.gstk_client_table[i].client_type =
          cmd_ptr->client_type;
        /* user_data_updated is intentionally not updated as that 
           will be used only in use-cases when original user_data
           is over-written */
  ...
  gstk_tk_functionality_enum_type           func_type)
{
     ...
     !gstk_util_is_valid_data_format_req(format_type))
  {
    UIM_MSG_ERR_2("Invalid func type 0x%x,"
		          "data format type 0x%x", func_type, format_type);
    return GSTK_BAD_PARAM;
  }
...
)
{
  ...
  if((cmd_ptr->ds_sys_status_ex_ind.ds_sys_status_info.avail_sys[0].technology ==
    GSTK_DS_SYS_NETWORK_3GPP) ||
     (cmd_ptr->ds_sys_status_ex_ind.ds_sys_status_info.avail_sys[0].technology ==
  ...
  else if(cmd_ptr->ds_sys_status_ex_ind.ds_sys_status_info.avail_sys[0].technology ==
...
)
{
  ...
  (void)gstk_build_pco_envelope(
          &cmd_ptr->ds_msisdn_change_ind.ds_msisdn_info_per_apn.msisdn_info,
          slot);
...
)
{
  ...
  switch((gstk_cmd_enum_type)cmd_ptr->general_cmd.message_header.command_id)
  {
    ...
    case GSTK_DS_WLAN_IP_ADDR_CHANGE_IND:
      gstk_process_ds_wlan_ip_addr_change_ind(cmd_ptr);
      break;
...
)
{
  ...
  gstk_instances_ptr[(uint32)slot - 1]->subs_feature_mode =
    cmd_ptr->subs_change_ind.subs_feature_mode;
#if defined(FEATURE_GSTK_1X_ACTIVATION_PREFERRED) && defined(FEATURE_CDMA)
...
#endif /* FEATURE_CDMA && FEATURE_GSTK_1X_ACTIVATION_PREFERRED */
...
void gstk_task_free_cmd_buf ( gstk_task_cmd_type * cmd_ptr )
{
  ...
  switch(cmd_ptr->cmd.general_cmd.message_header.command_group)
  {
    ...
    case GSTK_TERMINAL_PROFILE_CMD:
      if(cmd_ptr->cmd.terminal_profile_cmd.data_ptr != NULL)
      { 
        gstk_free(cmd_ptr->cmd.terminal_profile_cmd.data_ptr);
        cmd_ptr->cmd.terminal_profile_cmd.data_ptr = NULL;
      }
      break;
    default:
      break;
...
void gstk_process_session_changed_evt(gstk_cmd_type *cmd_ptr)
{
  ...
  UIM_MSG_LOW_3("GW Ready 0x%x, 1x Ready 0x%x, Apps 0x%x",
  gstk_curr_inst_ptr->is_subs_ready.subs_ready_gw,
  gstk_curr_inst_ptr->is_subs_ready.subs_ready_1x,
  gstk_curr_inst_ptr->apps_enabled);
} /* gstk_process_session_changed_evt */


#ifdef FEATURE_QSH_MDUMP
/*===========================================================================
FUNCTION UIM_QSH_MDUMP_GSTK

DESCRIPTION
  This function is used to dump globasl from GSTK.
...
SEE ALSO
  None
===========================================================================*/
QSH_MDUMP_FN_ATTR void uim_qsh_mdump_gstk(void)
{
  qsh_mdump_collect_high(&gstk_curr_inst_ptr, sizeof(gstk_curr_inst_ptr));
  if(gstk_curr_inst_ptr != NULL)
  {
    qsh_mdump_collect_high(&gstk_curr_inst_ptr->slot_id, sizeof(gstk_curr_inst_ptr->slot_id));
  }

  qsh_mdump_collect_high(gstk_instances_ptr, sizeof(gstk_instances_ptr));
  if(gstk_instances_ptr[0] != NULL)
  {
    qsh_mdump_collect_high(&gstk_instances_ptr[0]->gstk_sub_state, sizeof(gstk_instances_ptr[0]->gstk_sub_state));
  }
  if(gstk_instances_ptr[1] != NULL)
  {
    qsh_mdump_collect_high(&gstk_instances_ptr[1]->gstk_sub_state, sizeof(gstk_instances_ptr[0]->gstk_sub_state));
  }
} /* uim_qsh_mdump_gstk */
#endif /* FEATURE_QSH_MDUMP */
