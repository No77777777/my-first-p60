...
Copyright (c) 2003-2016, 2022 QUALCOMM Technologies, Inc (QTI) and its licensors.
...
$Header: //components/rel/uim.mpss/6.1.0/gstk/src/gstk_get_inkey.c#3 $
  ...
  gstk_get_inkey_terminal_response_type   *STK_response_ptr)
{
  ...
  gstk_util_release_upper_layer_cmd_memory(
    (gstk_cmd_type*)get_inkey_cnf_ptr);
  ...
  gstk_proactive_cmd_type* command_ptr)
{
  ...
  */
  ...
  if(gstk_status != GSTK_SUCCESS)
  {
    ...
    UIM_MSG_HIGH_0("Send Error TR for Get Inkey");
  ...
  if (uim_cmd_ptr != NULL ) {
    ...
    if (gstk_status == GSTK_SUCCESS) {
      if (uim_cmd_ptr != NULL) {
        ...
        if (gstk_status == GSTK_SUCCESS)
        {
          GSTK_UTIL_SELECTIVE_DUMP_BYTE_ARRAY("Get Inkey TR",
