...
$Header: //components/rel/uim.mpss/6.1.0/gstk/src/gstk_envelope_menu_sel.c#1 $

when         who     what, where, why
--------   ---     --------------------------------------------------------------------
08/17/17   dt      End proactive session when MENU SELECTION command response is not 91XX
   ...
   */
   ...
   if (uim_cmd_ptr != NULL ) 
   {
     ...
     if ((gstk_status == GSTK_SUCCESS) && (uim_cmd_ptr != NULL)) 
     {
       ...
       if (gstk_status == GSTK_SUCCESS)
       {
         GSTK_UTIL_SELECTIVE_DUMP_BYTE_ARRAY("Env: Menu Sel",
