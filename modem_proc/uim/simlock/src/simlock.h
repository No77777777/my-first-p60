...
$Header: //components/rel/uim.mpss/6.1.0/simlock/src/simlock.h#4 $$ $DateTime: 2020/08/04 23:12:22 $
...
01/14/15   bcho    Define GID MIN LEN
...
/* Max number of digits of a valid IMEI */
#define SIMLOCK_IMEI_MAX                    15

/* Max number of digits of a valid MEID */
#define SIMLOCK_MEID_MAX                    14

#define SIMLOCK_CONFIG_ID_DEFAULT           0
...
typedef enum {
  ...
  SIMLOCK_FILE_3GPP_IMPI_1,
  SIMLOCK_FILE_3GPP_NS_SP_1,
  ...
  SIMLOCK_FILE_3GPP_IMPI_2,
  SIMLOCK_FILE_3GPP_NS_SP_2,
  ...
  SIMLOCK_FILE_3GPP_IMPI_3,
  SIMLOCK_FILE_3GPP_NS_SP_3,
  ...
  SIMLOCK_FILE_3GPP_IMPI_4,
  SIMLOCK_FILE_3GPP_NS_SP_4,
  ...
  /* File to store IMSI data that has acquired full service */
...
/* ----------------------------------------------------------------------------
   STRUCTURE:      simlock_meid_type

   DESCRIPTION:
     This structure contains the MEID code in ASCII
------------------------------------------------------------------------------*/
typedef struct {
  uint8    meid_len;
  char     meid_data[SIMLOCK_MEID_MAX];
} simlock_meid_type;
...
/* ----------------------------------------------------------------------------
   ...
   DESCRIPTION:
     ...
     mcc         : mobile country code digits
     mnc         : mobile network code digits
     ...
     ehplmn_mcc  : Equivalent home PLMN MCC
     ehplmn_mnc  : Equivalent home PLMN MNC
     iccid       : ICCID content
     impi        : IMPI content
-------------------------------------------------------------------------------*/
typedef struct {
  ...
  simlock_mcc_type        ehplmn_mcc;
  simlock_mnc_type        ehplmn_mnc;
  simlock_iccid_type      iccid;
  simlock_impi_type       impi;
} simlock_category_code_data_type;

/* ----------------------------------------------------------------------------
   STRUCTURE:      SIMLOCK_EHPLMN_DATA_TYPE

   DESCRIPTION:
     This structure contains the EHPLMN data

     num_ehplmns   : Number of equivalent home PLMN
     ehplmn_entry  : Equivalent home PLMN entry
-------------------------------------------------------------------------------*/
typedef struct {
  uint8                      num_ehplmns;
  simlock_nw_code_data_type  ehplmn_entry[SIMLOCK_EHPLMN_NUM_MAX];
} simlock_ehplmn_list_type;

/* ----------------------------------------------------------------------------
   STRUCTURE:      SIMLOCK_DECODED_SUBSCRIPTION_DATA_TYPE

   DESCRIPTION:
     This structure contains the subscription data for a category code
...
typedef struct {
  ...
  simlock_gid_type          gid2;
  simlock_spn_type          spn;
  simlock_ehplmn_list_type  ehplmn;
  simlock_iccid_type        iccid;
} simlock_decoded_subscription_data_type;
...
(
  simlock_get_rsu_mode_msg_resp_type      * mode_config_ptr
);
