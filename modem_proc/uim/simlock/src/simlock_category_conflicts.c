...
$Header: //components/rel/uim.mpss/6.1.0/simlock/src/simlock_category_conflicts.c#3 $$ $DateTime: 2019/12/27 15:53:22 $

when       who     what, where, why
--------   ---     -----------------------------------------------------------
12/28/19   sg      Added support for NS+SP category
09/13/19   vdc     SimLock solution for activation at Point Of Sale
10/24/17   av      Fix category conflict logic when lock category is SP_EHPLMN
...
)
{
  ...
  simlock_category_file_data_type  *enabled_category_data_ptr = NULL;;
  ...
  SIMLOCK_CHECK_AND_MEM_MALLOC(enabled_category_data_ptr,
                               sizeof(simlock_category_file_data_type));
  if(enabled_category_data_ptr == NULL)
  {
  ...
  {
    memset(enabled_category_data_ptr, 0x00,
              sizeof(simlock_category_file_data_type));
    ...
    /* Continue if the categories are same or enabled category is IMPI category
       (as we don't need to check for conflits for IMPI category) */
    ...
    if(simlock_status != SIMLOCK_SUCCESS)
    {
      ...
      simlock_category_free_code_data(enabled_category_data_ptr);
      SIMLOCK_MEM_FREE(enabled_category_data_ptr);
      return SIMLOCK_GENERIC_ERROR;
    }

    /* if autolock is on for other category,
       we should not accept code data for this category */
    if(enabled_category_data_ptr->category_header.auto_lock)
    {
    ...
    if(enabled_category_data_ptr->code_data_ptr == NULL)
    {
    ...
    for(code_cnt = 0; code_cnt < enabled_category_data_ptr->category_header.num_of_codes;
        code_cnt++)
    {
      match_found = simlock_util_check_if_codes_match(&enabled_category_data_ptr->code_data_ptr[code_cnt],
                                                      lock_code_data_ptr,
                                                      category,
                                                      lock_category);
      if(match_found)
...
)
{
  ...
  simlock_category_code_data_type    *ruim_code_data_temp_ptr = NULL;
  ...
  SIMLOCK_CHECK_AND_MEM_MALLOC(ruim_code_data_temp_ptr,
                               sizeof(simlock_category_code_data_type));
  if(ruim_code_data_temp_ptr == NULL)
  {
    return SIMLOCK_MEMORY_ERROR_HEAP_EXHAUSTED;
  }
      ...
      ruim_code_cnt++)
  {
    ...
    simlock_status = simlock_category_check_codes_in_categories(ruim_code_data_temp_ptr,
                                                                config_data_ptr,
  ...
  SIMLOCK_MEM_FREE(ruim_code_data_temp_ptr);
...
)
{
  ...
  simlock_category_code_data_type    *nw_type2_code_data_temp_ptr = NULL;
  ...
  SIMLOCK_CHECK_AND_MEM_MALLOC(nw_type2_code_data_temp_ptr,
                               sizeof(simlock_category_code_data_type));
  if(nw_type2_code_data_temp_ptr == NULL)
  {
    return SIMLOCK_MEMORY_ERROR_HEAP_EXHAUSTED;
  }
  ...
  for(nw_type2_code_cnt = 0; nw_type2_code_cnt < nw_type2_code_data_ptr->code_data.nw_type2_3gpp2_code_list.num_nw_type2_codes;
      nw_type2_code_cnt++)
  {
    memset(nw_type2_code_data_temp_ptr, 0x00, sizeof(simlock_category_code_data_type));

    simlock_memscpy(nw_type2_code_data_temp_ptr->msin.msin_data,
                    SIMLOCK_MSIN_MAX,
                    nw_type2_code_data_ptr->code_data.nw_type2_3gpp2_code_list.nw_type2_code_data[nw_type2_code_cnt].irm,
                    SIMLOCK_IRM_LEN);

    /* check if the code is valid in all the enabled 3GPP2 categories */
    simlock_status = simlock_category_check_codes_in_categories(nw_type2_code_data_temp_ptr,
                                                                config_data_ptr,
                                                                SIMLOCK_CATEGORY_3GPP2_NW_TYPE1,
                                                                SIMLOCK_CATEGORY_3GPP2_RUIM,
  ...
  SIMLOCK_MEM_FREE(nw_type2_code_data_temp_ptr);
...
)
{
  ...
  simlock_category_code_data_type    *nw_type1_code_data_temp_ptr = NULL;
  ...
  SIMLOCK_CHECK_AND_MEM_MALLOC(nw_type1_code_data_temp_ptr,
                               sizeof(simlock_category_code_data_type));
  if(nw_type1_code_data_temp_ptr == NULL)
  {
    return SIMLOCK_MEMORY_ERROR_HEAP_EXHAUSTED;
  }
  ...
  for(nw_type1_code_cnt = 0; nw_type1_code_cnt < nw_type1_code_data_ptr->code_data.nw_type1_3gpp2_code_list.num_nw_codes;
      nw_type1_code_cnt++)
  {
    memset(nw_type1_code_data_temp_ptr, 0x00, sizeof(simlock_category_code_data_type));

    simlock_memscpy(nw_type1_code_data_temp_ptr->mcc,
                    SIMLOCK_MCC_LEN,
                    nw_type1_code_data_ptr->code_data.nw_type1_3gpp2_code_list.nw_code_data[nw_type1_code_cnt].mcc,
                    SIMLOCK_MCC_LEN);

    nw_type1_code_data_temp_ptr->mnc =
    ...
    simlock_status = simlock_category_check_codes_in_categories(nw_type1_code_data_temp_ptr,
                                                                config_data_ptr,
                                                                SIMLOCK_CATEGORY_3GPP2_NW_TYPE2,
                                                                SIMLOCK_CATEGORY_3GPP2_RUIM,
                                                                SIMLOCK_CATEGORY_3GPP2_NW_TYPE1,
  ...
  SIMLOCK_MEM_FREE(nw_type1_code_data_temp_ptr);
...
)
{
  ...
  simlock_category_code_data_type    *sim_code_data_temp_ptr = NULL;
  ...
  SIMLOCK_CHECK_AND_MEM_MALLOC(sim_code_data_temp_ptr,
                               sizeof(simlock_category_code_data_type));
  if(sim_code_data_temp_ptr == NULL)
  {
    return SIMLOCK_MEMORY_ERROR_HEAP_EXHAUSTED;
  }
  ...
  SIMLOCK_MEM_FREE(sim_code_data_temp_ptr);
...
(
  const simlock_category_data_type  * spn_code_data_ptr,
  const simlock_config_data_type    * config_data_ptr,
  simlock_slot_enum_type              slot,
  uint8                               config_id
)
{
  ...
  simlock_category_code_data_type    *spn_code_data_temp_ptr = NULL;

  if((spn_code_data_ptr == NULL) ||
     (config_data_ptr == NULL))
  {
    return SIMLOCK_INCORRECT_PARAMS;
  }

  SIMLOCK_CHECK_AND_MEM_MALLOC(spn_code_data_temp_ptr,
                               sizeof(simlock_category_code_data_type));
  if(spn_code_data_temp_ptr == NULL)
  {
    return SIMLOCK_MEMORY_ERROR_HEAP_EXHAUSTED;
  }
  ...
  /* if atleast one mcc-mnc is present in all the enabled categories,
     then codes are good */
  for(spn_code_cnt = 0; spn_code_cnt < spn_code_data_ptr->code_data.spn_3gpp_code_list.num_spn_codes;
      spn_code_cnt++)
  {
    memset(spn_code_data_temp_ptr, 0x00, sizeof(simlock_category_code_data_type));

    simlock_memscpy(spn_code_data_temp_ptr->mcc,
                    SIMLOCK_MCC_LEN,
                    spn_code_data_ptr->code_data.spn_3gpp_code_list.spn_code_data[spn_code_cnt].nw_code.mcc,
                    SIMLOCK_MCC_LEN);

    spn_code_data_temp_ptr->mnc =
        spn_code_data_ptr->code_data.spn_3gpp_code_list.spn_code_data[spn_code_cnt].nw_code.mnc;

    /* check if the code is valid in all the enabled 3GPP categories */
    simlock_status = simlock_category_check_codes_in_categories(spn_code_data_temp_ptr,
                                                                config_data_ptr,
                                                                SIMLOCK_CATEGORY_3GPP_NW,
                                                                SIMLOCK_CATEGORY_3GPP_SIM,
  ...
  SIMLOCK_MEM_FREE(spn_code_data_temp_ptr);
...
/*===========================================================================
FUNCTION SIMLOCK_CATEGORY_CHECK_SP_EHPLMN_CODES_FOR_CONFLICTS

DESCRIPTION
  This function checks if the codes provided can be added for enabling
  the SP + EHPLMN category lock
...
SIDE EFFECTS
  NONE
===========================================================================*/
static simlock_result_enum_type simlock_category_check_sp_ehplmn_codes_for_conflicts
(
  const simlock_category_data_type  * sp_ehplmn_code_data_ptr,
  const simlock_config_data_type    * config_data_ptr,
  simlock_slot_enum_type              slot,
  uint8                               config_id
)
{
  simlock_result_enum_type            simlock_status               = SIMLOCK_SUCCESS;
  uint8                               sp_ehplmn_code_cnt           = 0;
  simlock_category_code_data_type    *sp_ehplmn_code_data_temp_ptr = NULL;

  if((sp_ehplmn_code_data_ptr == NULL) ||
     (config_data_ptr == NULL))
  {
    return SIMLOCK_INCORRECT_PARAMS;
  }

  SIMLOCK_CHECK_AND_MEM_MALLOC(sp_ehplmn_code_data_temp_ptr,
                               sizeof(simlock_category_code_data_type));
  if(sp_ehplmn_code_data_temp_ptr == NULL)
  {
    return SIMLOCK_MEMORY_ERROR_HEAP_EXHAUSTED;
  }
  ...
  /* If atleast one mcc-mnc is present in all the enabled categories,
     then codes are good */
  for(sp_ehplmn_code_cnt = 0; sp_ehplmn_code_cnt < sp_ehplmn_code_data_ptr->code_data.sp_ehplmn_3gpp_code_list.num_sp_ehplmn_codes;
      sp_ehplmn_code_cnt++)
  {
    memset(sp_ehplmn_code_data_temp_ptr, 0x00, sizeof(simlock_category_code_data_type));

    simlock_memscpy(sp_ehplmn_code_data_temp_ptr->mcc,
                    SIMLOCK_MCC_LEN,
                    sp_ehplmn_code_data_ptr->code_data.sp_ehplmn_3gpp_code_list.sp_ehplmn_code_data[sp_ehplmn_code_cnt].nw_code.mcc,
                    SIMLOCK_MCC_LEN);

    sp_ehplmn_code_data_temp_ptr->mnc =
       sp_ehplmn_code_data_ptr->code_data.sp_ehplmn_3gpp_code_list.sp_ehplmn_code_data[sp_ehplmn_code_cnt].nw_code.mnc;

    simlock_memscpy(sp_ehplmn_code_data_temp_ptr->ehplmn_mcc,
                    SIMLOCK_MCC_LEN,
                    sp_ehplmn_code_data_ptr->code_data.sp_ehplmn_3gpp_code_list.sp_ehplmn_code_data[sp_ehplmn_code_cnt].ehplmn_nw_code.mcc,
                    SIMLOCK_MCC_LEN);

    sp_ehplmn_code_data_temp_ptr->ehplmn_mnc =
       sp_ehplmn_code_data_ptr->code_data.sp_ehplmn_3gpp_code_list.sp_ehplmn_code_data[sp_ehplmn_code_cnt].ehplmn_nw_code.mnc;

    sp_ehplmn_code_data_temp_ptr->gid1 =
       sp_ehplmn_code_data_ptr->code_data.sp_ehplmn_3gpp_code_list.sp_ehplmn_code_data[sp_ehplmn_code_cnt].gid1;

    /* Check if the code is valid in all the enabled 3GPP categories */
    simlock_status = simlock_category_check_codes_in_categories(sp_ehplmn_code_data_temp_ptr,
                                                                config_data_ptr,
                                                                SIMLOCK_CATEGORY_3GPP_NW,
                                                                SIMLOCK_CATEGORY_3GPP_SIM,
  ...
  SIMLOCK_MEM_FREE(sp_ehplmn_code_data_temp_ptr);

  return simlock_status;
} /* simlock_category_check_sp_ehplmn_codes_for_conflicts */
...
(
  const simlock_category_data_type  * iccid_code_data_ptr,
  const simlock_config_data_type    * config_data_ptr,
  simlock_slot_enum_type              slot,
  uint8                               config_id
)
{
  ...
  simlock_category_code_data_type    *iccid_code_data_temp_ptr = NULL;

  if((iccid_code_data_ptr == NULL) ||
     (config_data_ptr == NULL))
  {
    return SIMLOCK_INCORRECT_PARAMS;
  }

  SIMLOCK_CHECK_AND_MEM_MALLOC(iccid_code_data_temp_ptr,
                               sizeof(simlock_category_code_data_type));
  if(iccid_code_data_temp_ptr == NULL)
  {
    return SIMLOCK_MEMORY_ERROR_HEAP_EXHAUSTED;
  }
  ...
  /* if atleast one mcc-mnc is present in all the enabled categories,
     then codes are good */
  for(iccid_code_cnt = 0; iccid_code_cnt < iccid_code_data_ptr->code_data.iccid_3gpp_code_list.num_iccid_codes;
      iccid_code_cnt++)
  {
    memset(iccid_code_data_temp_ptr, 0x00, sizeof(simlock_category_code_data_type));

    simlock_memscpy(iccid_code_data_temp_ptr->mcc,
                    SIMLOCK_MCC_LEN,
                    iccid_code_data_ptr->code_data.iccid_3gpp_code_list.iccid_code_data[iccid_code_cnt].nw_code.mcc,
                    SIMLOCK_MCC_LEN);

    iccid_code_data_temp_ptr->mnc =
        iccid_code_data_ptr->code_data.iccid_3gpp_code_list.iccid_code_data[iccid_code_cnt].nw_code.mnc;

    /* check if the code is valid in all the enabled 3GPP categories */
    simlock_status = simlock_category_check_codes_in_categories(iccid_code_data_temp_ptr,
                                                                config_data_ptr,
                                                                SIMLOCK_CATEGORY_3GPP_NW,
                                                                SIMLOCK_CATEGORY_3GPP_SIM,
  ...
  SIMLOCK_MEM_FREE(iccid_code_data_temp_ptr);
...
/*===========================================================================
FUNCTION SIMLOCK_CATEGORY_CHECK_NS_SP_CODES_FOR_CONFLICTS

DESCRIPTION
  This function checks if the codes provided can be added for enabling
  the NS + SP category lock
...
SIDE EFFECTS
  NONE
===========================================================================*/
static simlock_result_enum_type simlock_category_check_ns_sp_codes_for_conflicts
(
  const simlock_category_data_type  * ns_sp_code_data_ptr,
  const simlock_config_data_type    * config_data_ptr,
  simlock_slot_enum_type              slot
)
{
  simlock_result_enum_type           simlock_status           = SIMLOCK_SUCCESS;
  uint8                              ns_sp_code_cnt           = 0;
  simlock_category_code_data_type   *ns_sp_code_data_temp_ptr = NULL;

  if((ns_sp_code_data_ptr == NULL) ||
     (config_data_ptr == NULL))
  {
    return SIMLOCK_INCORRECT_PARAMS;
  }

  SIMLOCK_CHECK_AND_MEM_MALLOC(ns_sp_code_data_temp_ptr,
                               sizeof(simlock_category_code_data_type));
  if(ns_sp_code_data_temp_ptr == NULL)
  {
    return SIMLOCK_MEMORY_ERROR_HEAP_EXHAUSTED;
  }

  for(ns_sp_code_cnt = 0; ns_sp_code_cnt < ns_sp_code_data_ptr->code_data.ns_sp_3gpp_code_list.num_ns_sp_codes;
      ns_sp_code_cnt++)
  {
    memset(ns_sp_code_data_temp_ptr, 0x00, sizeof(simlock_category_code_data_type));

    simlock_memscpy(ns_sp_code_data_temp_ptr->mcc,
                    SIMLOCK_MCC_LEN,
                    ns_sp_code_data_ptr->code_data.ns_sp_3gpp_code_list.ns_sp_code_data[ns_sp_code_cnt].ns_code.nw_code.mcc,
                    SIMLOCK_MCC_LEN);

    ns_sp_code_data_temp_ptr->mnc =
       ns_sp_code_data_ptr->code_data.ns_sp_3gpp_code_list.ns_sp_code_data[ns_sp_code_cnt].ns_code.nw_code.mnc;

    ns_sp_code_data_temp_ptr->msin.msin_data[0] =
      ns_sp_code_data_ptr->code_data.ns_sp_3gpp_code_list.ns_sp_code_data[ns_sp_code_cnt].ns_code.imsi_digit6;

    ns_sp_code_data_temp_ptr->msin.msin_data[1] =
      ns_sp_code_data_ptr->code_data.ns_sp_3gpp_code_list.ns_sp_code_data[ns_sp_code_cnt].ns_code.imsi_digit7;

    ns_sp_code_data_temp_ptr->msin.msin_len = 0x02;

    ns_sp_code_data_temp_ptr->gid1 =
      ns_sp_code_data_ptr->code_data.ns_sp_3gpp_code_list.ns_sp_code_data[ns_sp_code_cnt].gid1;

    /* check if the code is valid in all the enabled 3GPP categories */
    simlock_status = simlock_category_check_codes_in_categories(ns_sp_code_data_temp_ptr,
                                                                config_data_ptr,
                                                                SIMLOCK_CATEGORY_3GPP_NW,
                                                                SIMLOCK_CATEGORY_3GPP_SIM,
  ...
  SIMLOCK_MEM_FREE(ns_sp_code_data_temp_ptr);

  return simlock_status;
} /* simlock_category_check_ns_sp_codes_for_conflicts */
...
)
{
  simlock_result_enum_type            simlock_status        = SIMLOCK_SUCCESS;
  uint8                               cp_code_cnt           = 0;
  simlock_category_code_data_type    *cp_code_data_temp_ptr = NULL;
  ...
  SIMLOCK_CHECK_AND_MEM_MALLOC(cp_code_data_temp_ptr,
                               sizeof(simlock_category_code_data_type));
  if(cp_code_data_temp_ptr == NULL)
  {
    return SIMLOCK_MEMORY_ERROR_HEAP_EXHAUSTED;
  }
  ...
  for(cp_code_cnt = 0; cp_code_cnt < cp_code_data_ptr->code_data.cp_3gpp_code_list.num_cp_codes;
      cp_code_cnt++)
  {
    memset(cp_code_data_temp_ptr, 0x00, sizeof(simlock_category_code_data_type));

    simlock_memscpy(cp_code_data_temp_ptr->mcc,
                    SIMLOCK_MCC_LEN,
                    cp_code_data_ptr->code_data.cp_3gpp_code_list.cp_code_data[cp_code_cnt].nw_code.mcc,
                    SIMLOCK_MCC_LEN);

    cp_code_data_temp_ptr->mnc =
           cp_code_data_ptr->code_data.cp_3gpp_code_list.cp_code_data[cp_code_cnt].nw_code.mnc;

    cp_code_data_temp_ptr->gid1 =
          cp_code_data_ptr->code_data.cp_3gpp_code_list.cp_code_data[cp_code_cnt].gid1;

    /* check if the code is valid in all the enabled 3GPP categories */
    simlock_status = simlock_category_check_codes_in_categories(cp_code_data_temp_ptr,
                                                                config_data_ptr,
                                                                SIMLOCK_CATEGORY_3GPP_NW,
                                                                SIMLOCK_CATEGORY_3GPP_SIM,
  ...
  SIMLOCK_MEM_FREE(cp_code_data_temp_ptr);
...
)
{
  simlock_result_enum_type            simlock_status        = SIMLOCK_SUCCESS;
  uint8                               sp_code_cnt           = 0;
  simlock_category_code_data_type    *sp_code_data_temp_ptr = NULL;
  ...
  SIMLOCK_CHECK_AND_MEM_MALLOC(sp_code_data_temp_ptr,
                               sizeof(simlock_category_code_data_type));
  if(sp_code_data_temp_ptr == NULL)
  {
    return SIMLOCK_MEMORY_ERROR_HEAP_EXHAUSTED;
  }
  ...
  for(sp_code_cnt = 0; sp_code_cnt < sp_code_data_ptr->code_data.sp_3gpp_code_list.num_sp_codes;
      sp_code_cnt++)
  {
    memset(sp_code_data_temp_ptr, 0x00, sizeof(simlock_category_code_data_type));

    simlock_memscpy(sp_code_data_temp_ptr->mcc,
                    SIMLOCK_MCC_LEN,
                    sp_code_data_ptr->code_data.sp_3gpp_code_list.sp_code_data[sp_code_cnt].nw_code.mcc,
                    SIMLOCK_MCC_LEN);

    sp_code_data_temp_ptr->mnc =
       sp_code_data_ptr->code_data.sp_3gpp_code_list.sp_code_data[sp_code_cnt].nw_code.mnc;

    /* check if the code is valid in all the enabled 3GPP categories */
    simlock_status = simlock_category_check_codes_in_categories(sp_code_data_temp_ptr,
                                                                config_data_ptr,
                                                                SIMLOCK_CATEGORY_3GPP_NW,
                                                                SIMLOCK_CATEGORY_3GPP_SIM,
  ...
  SIMLOCK_MEM_FREE(sp_code_data_temp_ptr);
...
)
{
  simlock_result_enum_type           simlock_status        = SIMLOCK_SUCCESS;
  uint8                              ns_code_cnt           = 0;
  simlock_category_code_data_type   *ns_code_data_temp_ptr = NULL;
  ...
  SIMLOCK_CHECK_AND_MEM_MALLOC(ns_code_data_temp_ptr,
                               sizeof(simlock_category_code_data_type));
  if(ns_code_data_temp_ptr == NULL)
  {
    return SIMLOCK_MEMORY_ERROR_HEAP_EXHAUSTED;
  }
  ...
  for(ns_code_cnt = 0; ns_code_cnt < ns_code_data_ptr->code_data.ns_3gpp_code_list.num_ns_codes;
      ns_code_cnt++)
  {
    memset(ns_code_data_temp_ptr, 0x00, sizeof(simlock_category_code_data_type));

    simlock_memscpy(ns_code_data_temp_ptr->mcc,
                    SIMLOCK_MCC_LEN,
                    ns_code_data_ptr->code_data.ns_3gpp_code_list.ns_code_data[ns_code_cnt].nw_code.mcc,
                    SIMLOCK_MCC_LEN);

    ns_code_data_temp_ptr->mnc =
       ns_code_data_ptr->code_data.ns_3gpp_code_list.ns_code_data[ns_code_cnt].nw_code.mnc;

    ns_code_data_temp_ptr->msin.msin_data[0] =
      ns_code_data_ptr->code_data.ns_3gpp_code_list.ns_code_data[ns_code_cnt].imsi_digit6;

    ns_code_data_temp_ptr->msin.msin_data[1] =
      ns_code_data_ptr->code_data.ns_3gpp_code_list.ns_code_data[ns_code_cnt].imsi_digit7;

    ns_code_data_temp_ptr->msin.msin_len = 0x02;

    /* check if the code is valid in all the enabled 3GPP categories */
    simlock_status = simlock_category_check_codes_in_categories(ns_code_data_temp_ptr,
                                                                config_data_ptr,
                                                                SIMLOCK_CATEGORY_3GPP_NW,
                                                                SIMLOCK_CATEGORY_3GPP_SIM,
  ...
  SIMLOCK_MEM_FREE(ns_code_data_temp_ptr);
...
)
{
  simlock_result_enum_type            simlock_status        = SIMLOCK_SUCCESS;
  uint8                               nw_code_cnt           = 0;
  simlock_category_code_data_type    *nw_code_data_temp_ptr = NULL;
  ...
  SIMLOCK_CHECK_AND_MEM_MALLOC(nw_code_data_temp_ptr,
                               sizeof(simlock_category_code_data_type));
  if(nw_code_data_temp_ptr == NULL)
  {
    return SIMLOCK_MEMORY_ERROR_HEAP_EXHAUSTED;
  }
  ...
  for(nw_code_cnt = 0; nw_code_cnt < nw_code_data_ptr->code_data.nw_3gpp_code_list.num_nw_codes;
      nw_code_cnt++)
  {
    memset(nw_code_data_temp_ptr, 0x00, sizeof(simlock_category_code_data_type));

    simlock_memscpy(nw_code_data_temp_ptr->mcc,
                    SIMLOCK_MCC_LEN,
                    nw_code_data_ptr->code_data.nw_3gpp_code_list.nw_code_data[nw_code_cnt].mcc,
                    SIMLOCK_MCC_LEN);

    nw_code_data_temp_ptr->mnc =
       nw_code_data_ptr->code_data.nw_3gpp_code_list.nw_code_data[nw_code_cnt].mnc;

    /* check if the code is valid in all the enabled 3GPP categories */
    simlock_status = simlock_category_check_codes_in_categories(nw_code_data_temp_ptr,
                                                                config_data_ptr,
                                                                SIMLOCK_CATEGORY_3GPP_NS,
                                                                SIMLOCK_CATEGORY_3GPP_SIM,
                                                                SIMLOCK_CATEGORY_3GPP_NW,
  ...
  SIMLOCK_MEM_FREE(nw_code_data_temp_ptr);
...
)
{
  ...
  switch(category_data_ptr->category_type)
  {
    ...
    case SIMLOCK_CATEGORY_3GPP_SP_EHPLMN:
      simlock_status = simlock_category_check_sp_ehplmn_codes_for_conflicts(category_data_ptr,
                                                                            config_data_ptr,
                                                                            slot,
                                                                            config_id);
      break;
    ...
    case SIMLOCK_CATEGORY_3GPP_ICCID:
     ...
     case SIMLOCK_CATEGORY_3GPP_NS_SP:
      simlock_status = simlock_category_check_ns_sp_codes_for_conflicts(category_data_ptr,
                                                                        config_data_ptr,
                                                                        slot);
      break;
