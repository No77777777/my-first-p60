...
$Header: //components/rel/uim.mpss/6.1.0/simlock/src/simlock_crypto.c#3 $$ $DateTime: 2019/12/26 17:21:00 $
...
09/19/15   vr      Merged the simlock crypto traces
...
)
{
#ifdef FEATURE_SEC_SFS
  typedef struct {
    uint8 salt_and_index[SIMLOCK_SALT_LEN + 4];
    uint8 hmac_digest1[SIMLOCK_HMAC_SHA256_LEN];
    uint8 hmac_digest2[SIMLOCK_HMAC_SHA256_LEN];
    uint8 hmac_out[SIMLOCK_HMAC_SHA256_LEN];
  } pbkdf2_algo_data_type;

#ifndef FEATURE_UIM_TEST_FRAMEWORK
  pbkdf2_algo_data_type     * pbkdf2_algo_data_ptr = NULL;
  ...
  SIMLOCK_MSG_MED_4("simlock_crypto_run_pbkdf2_algorithm, iteration_cnt: 0x%x, \
                     password_data_len: 0x%x, salt_data_len: 0x%x, key_data_len: 0x%x",
                     iterations, password_data_len, salt_data_len, key_data_len);
  ...
  SIMLOCK_CHECK_AND_MEM_MALLOC(pbkdf2_algo_data_ptr,
                               sizeof(pbkdf2_algo_data_type));
  if(pbkdf2_algo_data_ptr == NULL)
  {
    (void)secapi_delete(&hsh_handle);
    return SIMLOCK_GENERIC_ERROR;
  }
  ...
  simlock_memscpy(pbkdf2_algo_data_ptr->salt_and_index,
                  salt_data_len + 4,
                  salt_data,
                  salt_data_len);
  ...
  for(dk_block_cnt = 1; dk_block_cnt <= dk_block_num;
      dk_block_cnt++)
  {
    pbkdf2_algo_data_ptr->salt_and_index[salt_data_len + 0] = (uint8)((dk_block_cnt >> 24) & 0xFF);
    pbkdf2_algo_data_ptr->salt_and_index[salt_data_len + 1] = (uint8)((dk_block_cnt >> 16) & 0xFF);
    pbkdf2_algo_data_ptr->salt_and_index[salt_data_len + 2] = (uint8)((dk_block_cnt >> 8) & 0xFF);
    pbkdf2_algo_data_ptr->salt_and_index[salt_data_len + 3] = (uint8)(dk_block_cnt & 0xFF);

    result = secapi_hsh_create_hmac(hsh_handle,
                                    SECAPI_SHA256,
                                    pbkdf2_algo_data_ptr->salt_and_index,
                                    salt_data_len + 4,
                                    password_buf,
                                    password_data_len,
                                    pbkdf2_algo_data_ptr->hmac_digest1);
    if(result != E_SUCCESS)
    {
      SIMLOCK_MSG_ERR_1("failed to create hmac, result: 0x%x", result);
      SIMLOCK_MEM_FREE(pbkdf2_algo_data_ptr);
      (void)secapi_delete(&hsh_handle);
      return SIMLOCK_GENERIC_ERROR;
    }

    simlock_memscpy(pbkdf2_algo_data_ptr->hmac_out,
                    SIMLOCK_HMAC_SHA256_LEN,
                    pbkdf2_algo_data_ptr->hmac_digest1, SIMLOCK_HMAC_SHA256_LEN);
    for(iteration_cnt = 1; iteration_cnt < iterations; iteration_cnt++)
    {
      result = secapi_hsh_create_hmac(hsh_handle,
                                      ...
                                      password_buf,
                                      password_data_len,
                                      pbkdf2_algo_data_ptr->hmac_digest2);
      if(result != E_SUCCESS)
      {
      ...
      simlock_memscpy(pbkdf2_algo_data_ptr->hmac_digest1,
                      SIMLOCK_HMAC_SHA256_LEN,
                      pbkdf2_algo_data_ptr->hmac_digest2, SIMLOCK_HMAC_SHA256_LEN);
      for(digest_index = 0; digest_index < SIMLOCK_HMAC_SHA256_LEN; digest_index++)
      {
        pbkdf2_algo_data_ptr->hmac_out[digest_index] ^= pbkdf2_algo_data_ptr->hmac_digest1[digest_index];
      }
    }
    ...
    if(key_data_len < SIMLOCK_HMAC_SHA256_LEN)
    {
      simlock_memscpy(key_data, key_data_len,
                      pbkdf2_algo_data_ptr->hmac_out, key_data_len);
      key_data += key_data_len;
    }
    else
    {
      simlock_memscpy(key_data, key_data_len,
                      pbkdf2_algo_data_ptr->hmac_out, SIMLOCK_HMAC_SHA256_LEN);
...
)
{
  ...
  if(result != E_SUCCESS)
  {
    SIMLOCK_MEM_FREE(digest_info_ptr->data_ptr);
    return SIMLOCK_GENERIC_ERROR;
  }
