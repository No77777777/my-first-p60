...
Copyright (c) 2014 - 2015, 2017 - 2020 QUALCOMM Technologies, Inc (QTI) and its licensors.
...
$Header: //components/rel/uim.mpss/6.1.0/simlock/src/simlocklib.c#3 $$ $DateTime: 2020/08/04 23:12:22 $
...
06/16/15   stv     Perform efs sync for remote sfs command
...
)
{
  ...
  switch(msg_req_ptr->msg_type)
  {
    ...
    case SIMLOCK_SET_RSU_MODE_MSG:
      simlock_status = simlock_set_simlock_rsu_mode(&msg_req_ptr->message.rsu_mode_msg);
      break;

    case SIMLOCK_SET_RSU_DEVICE_KEY_MSG:
#ifdef FEATURE_SIMLOCK_RSU
      simlock_status = simlock_set_simlock_rsu_device_key(&msg_req_ptr->message.rsu_device_key_msg.rsu_device_key);
#else
    ...
    case SIMLOCK_SET_LOCK_CK_MSG:
      simlock_status = simlock_set_lock_ck(&msg_req_ptr->message.lock_ck_msg,
                                           &msg_resp_ptr->message.token_resp);
      break;
    ...
    case SIMLOCK_GET_STATUS_MSG:
      ...
      simlock_status = simlock_get_lock_status(&msg_req_ptr->message.get_status_msg,
                                               &msg_resp_ptr->message.get_status_resp);
      break;
    ...
    case SIMLOCK_GET_RSU_MODE_MSG:
      ...
      simlock_status = simlock_get_simlock_rsu_mode(&msg_resp_ptr->message.get_rsu_mode);
      break;
    ...
    case SIMLOCK_ADD_LOCK_CODES_CK_MSG:
      ...
      simlock_status = simlock_add_lock_codes_ck_request(&msg_req_ptr->message.add_lock_codes_ck_msg,
                                                         &msg_resp_ptr->message.add_lock_resp);
      break;
    ...
    case SIMLOCK_CHANGE_LIST_TYPE_MSG:
      simlock_status = simlock_switch_config(&msg_req_ptr->message.change_list_type_msg);
      break;
    ...
    case SIMLOCK_GET_TOKEN_MSG:
      ...
      simlock_status = simlock_get_token_request(&msg_resp_ptr->message.get_token_resp);
      break;

    case SIMLOCK_ACTIVATE_CONFIG_MSG:
      simlock_status = simlock_activate_config_request(&msg_req_ptr->message.activate_configuration);
      break;
