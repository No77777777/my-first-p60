...
Copyright (c) 2015 - 2017, 2019 QUALCOMM Technologies, Inc (QTI) and its licensors.
...
12/29/17   rps      Reduce frequency of updating TIME INFO file
...
09/23/15   stv      Update back up time(last power cycle source time) correctly
...
#if defined(FEATURE_SIMLOCK) && defined(FEATURE_SIMLOCK_RSU) && defined(FEATURE_SIMLOCK_RSU_TIME)
...
/* Minimum time difference (in seconds) between NITZ/GPS events that are
   considered as valid by SIMLOCK from time_info updating point of view.
   Currently set to 15 minutes. */
#define SIMLOCK_TIME_INFO_FRESHNESS_THRESHOLD   900
...
RETURN VALUE
  boolean: TRUE if global was updated
           FALSE if global was not updated

SIDE EFFECTS
  NONE
===========================================================================*/
static boolean simlock_time_update_time(
...
)
{
  ...
  if(simlock_time_global_time_info.data[source].valid &&
     (source == SIMLOCK_TIME_SOURCE_NITZ_PRI ||
      source == SIMLOCK_TIME_SOURCE_NITZ_SEC ||
      source == SIMLOCK_TIME_SOURCE_GPS))
  {
    simlock_time_type         current_source_time  = 0;

    current_source_time = pmic_time + simlock_time_global_time_info.data[source].offset + SIMLOCK_TIME_BASE;

    /* We consider and store NITZ/GPS time only if the delta between the time
       reported now and the time reported now and the time estimated based on
       the previous event is greater than a certain threshold.
       This avoids writing of SIMLOCK_FILE_TIME_INFO too frequently. */
    if(((source_time >= current_source_time) &&
         ((source_time - current_source_time) <= SIMLOCK_TIME_INFO_FRESHNESS_THRESHOLD)) ||
       ((current_source_time >= source_time) &&
        ((current_source_time - source_time) <= SIMLOCK_TIME_INFO_FRESHNESS_THRESHOLD)))
    {
      return FALSE;
    }
  }
...
)
{
  ...
  /* Update the time. We don't write it to file system, because we do it at 
     the end of this function, if needed. If there is nothing to update, we 
     can directly return */
  if(FALSE == simlock_time_update_time(source, source_time, FALSE))
  {
    return;
  }
...
)
{
  ...
  else if (simlock_time_global_time_info.last_pmic_time >= pmic_time)
  {
    ...
    if(status == SIMLOCK_SUCCESS)
    {
      (void) simlock_time_update_time(SIMLOCK_TIME_SOURCE_LAST_POWER_CYCLE, time.current_time + SIMLOCK_TIME_BASE, TRUE);
...
#endif /* FEATURE_SIMLOCK && FEATURE_SIMLOCK_RSU && FEATURE_SIMLOCK_RSU_TIME */
