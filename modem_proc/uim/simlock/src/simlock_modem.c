...
$Header: //components/rel/uim.mpss/6.1.0/simlock/src/simlock_modem.c#3 $$ $DateTime: 2020/08/04 23:12:22 $
...
05/16/19   vgd     Send unlock event on all slots when wrong ck is entered
...
04/11/18   bcho    Move RSU config check to SIMLOCK task context 
...
07/13/16   av      Blow simlock fuse when config is moved to remote storage
...
10/16/15   vv      Perform fs commit after init
10/14/15   vv      Use mcs wrappers for rcevt APIs
10/01/15   stv     Fix for compilation issue
...
06/19/15   tl      Fix compiler warnings
06/16/15   stv     Fix to perform efs sync operation for critical file updates
06/05/15   stv     Send event to MMGSDI in case of add ck/hck lock,relock msgs
...
02/28/15   vv      Include the "rcinit_rex.h" file
01/29/15   vv      Register for task termination if FEATURE_SIMLOCK is enabled
...
=============================================================================*/
/* SimLock command queue */
static q_type                    simlock_cmd_q;

/* Flag indicating when SimLock task has stopped */
boolean                          simlock_task_stopped;

/* Critical section responsible for protecting data in the SimLock task */
...
/* Max number of simlock clients interested in listening to simlock events. */
#define SIMLOCK_MAX_CLIENTS      10

/* SimLock event callback array containing callbacks for all the clients
   interested in the simlock events */
static simlock_evt_cb_type       simlock_client_evt_callbacks[SIMLOCK_MAX_CLIENTS] = {NULL};

/* CM related SimLock data */
static RCEVT_SIGEX_SIGREX        simlock_rcevt_siginfo_cm;
static boolean                   simlock_cm_task_ready_received = FALSE;

/* Pointer to the SimLock task tcb. Initialized with the task. */
rex_tcb_type                   * simlock_tcb_ptr = NULL;
...
/*===========================================================================
FUNCTION SIMLOCK_MODEM_ADD_CLIENT_EVT_REG_ENTRY

DESCRIPTION
  Function to find a valid free index in the client event registration array
  and add the client callback at that index.
...
CRITICAL SECTIONS:
  None.
===========================================================================*/
static simlock_result_enum_type simlock_modem_add_client_evt_reg_entry(
  simlock_evt_cb_type           evt_cb_ptr)
{
  ...
  for (client_index = 0; client_index < SIMLOCK_MAX_CLIENTS; client_index++)
  {
    if (simlock_client_evt_callbacks[client_index] == NULL)
    {
      simlock_client_evt_callbacks[client_index] = evt_cb_ptr;
...
} /* simlock_modem_add_client_evt_reg_entry */


/*===========================================================================
FUNCTION SIMLOCK_MODEM_REMOVE_CLIENT_EVT_REG_ENTRY

DESCRIPTION
  Function to remove the client callback entry from the index in client event
  registration array corresponding to the matching input callback.
...
CRITICAL SECTIONS:
  None.
===========================================================================*/
static simlock_result_enum_type simlock_modem_remove_client_evt_reg_entry(
  simlock_evt_cb_type           evt_cb_ptr)
{
  uint8                         client_index = 0;
  ...
  for (client_index = 0; client_index < SIMLOCK_MAX_CLIENTS; client_index++)
  {
    if (simlock_client_evt_callbacks[client_index] == evt_cb_ptr)
    {
      simlock_client_evt_callbacks[client_index] = NULL;
  ...
  return SIMLOCK_GENERIC_ERROR;
} /* simlock_modem_remove_client_evt_reg_entry */
...
{
  simlock_result_enum_type simlock_result = SIMLOCK_GENERIC_ERROR;
  ...
  switch(req_msg_ptr->operation)
  {
    case SIMLOCK_REGISTER:
      ...
      {
        simlock_result = simlock_modem_add_client_evt_reg_entry(
                           req_msg_ptr->cb_type.simlock_evt_cb);
      }
      else if(req_msg_ptr->request_type == SIMLOCK_REGISTRATION_REMOTE_SFS)
      {
        simlock_result = simlock_sfs_remote_register_remote_cb(req_msg_ptr->cb_type.simlock_remote_req_cb);
      }
      break;
    ...
    case SIMLOCK_DEREGISTER:
      if(req_msg_ptr->request_type == SIMLOCK_REGISTRATION_EVENT)
      {
        simlock_result = simlock_modem_remove_client_evt_reg_entry(
                           req_msg_ptr->cb_type.simlock_evt_cb);
      }
      else if(req_msg_ptr->request_type == SIMLOCK_REGISTRATION_REMOTE_SFS)
      {
        simlock_result = simlock_sfs_remote_deregister_remote_cb();
      }
      break;

    default:
      simlock_result = SIMLOCK_INCORRECT_PARAMS;
      break;
  }

  return simlock_result;
...
/* ==========================================================================
  FUNCTION:      SIMLOCK_SEND_EVENT_TO_ALL

  DESCRIPTION:
    This function sends events to clients who have registered for receiving
    SimLock events from SimLock task
  ...
  SIDE EFFECTS:
    None
==========================================================================*/
static void simlock_send_event_to_all(
  ...
  simlock_event_data_type    event_data)
{
  ...
  for (client_index = 0; client_index < SIMLOCK_MAX_CLIENTS; client_index++)
  {
    ...
    /* Execute callback */
    (simlock_client_evt_callbacks[client_index])(simlock_status, event_data);
  }
} /* simlock_send_event_to_all */
...
)
{
  ...
  /* Since slot TLV is optional for depersonalization request in qmi_uim,
     qmi_uim fills slot as SLOT_1(if slot TLV is not present) while sending
     unlock request to simlock. If slot policy is SAME_ON_ALL_SLOTS, incorrect
     CK is filled in unlock request then send unlock event to MMGSDI on all
     slots so that status change indication is sent to all clients. There can
     be a case where card is inserted only in slot 2 and depersonalization
     request is received without slot TLV, when we send unlock event to MMGSDI
     with slot as SLOT_1 then no status change indication is sent to clients as
     no card is inserted in SLOT_1, to avoid such cases we need to send unlock
     event to MMSGDI on all slots. When correct CK is entered, we would send
     status change indication after run algo, so we need to send unlock event
     on all slots only when wrong ck is entered */
      ...
      simlock_status == SIMLOCK_MAX_RETRIES_REACHED))
  {
    ...
    {
      event_data.message.unlock.slot = slot;
      simlock_send_event_to_all(simlock_status, event_data);
    }
  }
  else
  {
...
)
{
  ...
  simlock_send_event_to_all(SIMLOCK_SUCCESS, event_data);
...
)
{
  ...
  event_data.message.lock.num_retries_max   = num_retries_max;
  event_data.message.lock.curr_retries      = curr_retries;
  event_data.message.lock.slot     = slot;
  event_data.message.lock.category = category;

  simlock_send_event_to_all(status, event_data);
} /* simlock_send_lock_event */


/*===========================================================================
FUNCTION SIMLOCK_SEND_DELAY_TIMER_EVENT

DESCRIPTION
  This function creates a delay timer event.
...
SIDE EFFECTS
  NONE
===========================================================================*/
void simlock_send_delay_timer_event
(
  void
)
{
  ...
  event_data.evt_type = SIMLOCK_DELAY_TIMER_EVENT;
  ...
  switch(req_msg_ptr->msg_type)
  {
    case SIMLOCK_SET_LOCK_CK_MSG:
      if(status == SIMLOCK_SUCCESS &&
         req_msg_ptr->message.lock_ck_msg.config_id == SIMLOCK_CONFIG_ID_DEFAULT)
      {
        simlock_send_lock_event(status,
                                req_msg_ptr->message.lock_ck_msg.slot,
                                req_msg_ptr->message.lock_ck_msg.category.category_type,
                                0,
                                req_msg_ptr->message.lock_ck_msg.num_retries_max);
      }
      break;

    case SIMLOCK_SET_LOCK_HCK_MSG:
      if(status == SIMLOCK_SUCCESS &&
         req_msg_ptr->message.lock_hck_msg.config_id == SIMLOCK_CONFIG_ID_DEFAULT)
      {
        simlock_send_lock_event(status,
                                req_msg_ptr->message.lock_hck_msg.slot,
                                req_msg_ptr->message.lock_hck_msg.category.category_type,
                                0,
                                req_msg_ptr->message.lock_hck_msg.num_retries_max);
      }
      break;
    ...
    case SIMLOCK_ADD_LOCK_CODES_HCK_MSG:
      ...
      {
        simlock_send_lock_event(status,
                                req_msg_ptr->message.add_lock_codes_hck_msg.slot,
                                req_msg_ptr->message.add_lock_codes_hck_msg.category.category_type,
                                resp_data_ptr->message.add_lock_resp.curr_retries,
                                resp_data_ptr->message.add_lock_resp.num_retries_max);
      }
      break;

    case SIMLOCK_ADD_LOCK_CODES_CK_MSG:
      if(resp_data_ptr != NULL  &&
         req_msg_ptr->message.add_lock_codes_ck_msg.config_id == SIMLOCK_CONFIG_ID_DEFAULT)
      {
        simlock_send_lock_event(status,
                                req_msg_ptr->message.add_lock_codes_ck_msg.slot,
    ...
    case SIMLOCK_RELOCK_DEVICE_MSG:
      ...
      {
        simlock_send_lock_event(status,
                                req_msg_ptr->message.relock_msg.slot,
                                req_msg_ptr->message.relock_msg.category_type,
                                resp_data_ptr->message.relock_resp.curr_retries,
                                resp_data_ptr->message.relock_resp.num_retries_max);
      }
      break;
    ...
    case SIMLOCK_CHANGE_LIST_TYPE_MSG:
      /* Send lock event if configuration has changed successfully */
      if(status == SIMLOCK_SUCCESS)
      {
        /* Since MMGSDI does not look for category, slot, current retries and
           number of retries for successful lock event, fill dummy data and
           send lock event */
        simlock_send_lock_event(status,
                                req_msg_ptr->message.change_list_type_msg.slot,
...
static void simlock_handle_cmd_sig(void)
{
  ...
  simlock_rsu_message_response_data_type rsu_resp_data;
    ...
    switch(simlock_cmd_ptr->cmd_type)
    {
...
#ifdef FEATURE_SIMLOCK_RSU
        if(simlock_rsu_config_is_lock_allowed(&simlock_cmd_ptr->data.engine) == FALSE)
        {
          simlock_status = SIMLOCK_INVALID_LOCK_DATA;
          resp_data_ptr->msg_type = simlock_cmd_ptr->data.engine.msg_type;
        }
#endif /* FEATURE_SIMLOCK_RSU */
...
#if defined(FEATURE_SIMLOCK_RSU) && defined(FEATURE_SIMLOCK_RSU_TIME)
...
#endif /* FEATURE_SIMLOCK_RSU && FEATURE_SIMLOCK_RSU_TIME */
      ...
      case SIMLOCK_REGISTER_CMD:
        ...
        /* When the registration for remote SFS operation is successfully
           processed, we inform the registered clients. This causes the
           client MMGSDI to trigger simlock_run_algo request */
        if(simlock_status == SIMLOCK_SUCCESS &&
      ...
      case SIMLOCK_SET_REMOTE_SFS_CMD:
        ...
        if(simlock_status == SIMLOCK_SUCCESS)
        {
          /* Blow the simlock fuse if not blown already */
...
#if defined(FEATURE_SIMLOCK_RSU) && defined(FEATURE_SIMLOCK_QMI_LOC) && defined(FEATURE_SIMLOCK_RSU_TIME)
        simlock_time_handle_gps_timer_expiry_cmd();
#endif /* FEATURE_SIMLOCK_RSU && FEATURE_SIMLOCK_QMI_LOC && FEATURE_SIMLOCK_RSU_TIME */
        break;
      ...
      case SIMLOCK_RSU_CMD:
        ...
        /* Free memory for both request and response pointers, as applicable */
        if(simlock_cmd_ptr->data.rsu_msg.msg_type == SIMLOCK_RSU_SET_CONFIGURATION_MSG)
        {
          SIMLOCK_MEM_FREE(simlock_cmd_ptr->data.rsu_msg.message.rsu_data.data_ptr);
          SIMLOCK_MEM_FREE(rsu_resp_data.message.set_config_resp_data.data_ptr);
        }
        else if(simlock_cmd_ptr->data.rsu_msg.msg_type == SIMLOCK_RSU_GENERATE_HASH_MSG)
        {
          SIMLOCK_MEM_FREE(simlock_cmd_ptr->data.rsu_msg.message.rsu_data.data_ptr);
          SIMLOCK_MEM_FREE(rsu_resp_data.message.generate_hash_resp_data.data_ptr);
        }
        else if(simlock_cmd_ptr->data.rsu_msg.msg_type == SIMLOCK_RSU_GET_KEY_MSG)
        {
          SIMLOCK_MEM_FREE(rsu_resp_data.message.get_key_resp_data.data_ptr);
        }
        else if(simlock_cmd_ptr->data.rsu_msg.msg_type == SIMLOCK_RSU_BLOB_REQUEST_MSG)
        {
          SIMLOCK_MEM_FREE(rsu_resp_data.message.generate_simlock_blob_resp_data.data_ptr);
        }
#endif /* FEATURE_SIMLOCK_RSU */
        break;
      ...
      case SIMLOCK_DELAY_TIMER_EXPIRY_CMD:
#ifdef FEATURE_SIMLOCK_RSU
        /* Notify MMGSDI with SIMLOCK_DELAY_TIMER_EVENT
           to re-run SimLock Algo */
        simlock_send_delay_timer_event();
#endif /* FEATURE_SIMLOCK_RSU */
        break;
    }
...
)
{
  ...
  simlock_tcb_ptr = rcinit_lookup_rextask("simlock");
...
)
{
  ...
  SIMLOCK_MEM_FREE(init_req_ptr);

  /* perform fs commit as new directories
     and file might be created during init */
  simlock_file_commit_to_fs_if_needed();
#endif /* FEATURE_SEC_SFS */
...
#ifdef FEATURE_SIMLOCK_RSU
    /*-----------------------------------------------------------------------------
                          Process Signal - SIMLOCK_MMGSDI_EVT_SIG
    -----------------------------------------------------------------------------*/
    else if (sigs_rcvd & SIMLOCK_MMGSDI_EVT_SIG)
    {
      simlock_handle_mmgsdi_evt();
    } /* end if SIMLOCK_MMGSDI_RESPONSE_SIG */
#endif /* FEATURE_SIMLOCK_RSU */
