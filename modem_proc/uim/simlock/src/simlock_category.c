...
$Header: //components/rel/uim.mpss/6.1.0/simlock/src/simlock_category.c#4 $$ $DateTime: 2020/03/02 04:01:09 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------- 
03/02/20   vdc     Copy complete HCK data to header while build upgrade
...
08/27/15   stv     Fix memory leak while retriving lock data
...
#define SIMLOCK_CATEGORY_FILE_HEADER_SIZE_v1   97
...
/* SIMLock Old supported GID length */
#define SIMLOCK_MAX_GID_LEN_V2       5

/* SIMLock V4 supports 8 bytes GID */
#define SIMLOCK_MAX_GID_LEN_V4       8
...
typedef enum {
  ...
  SIMLOCK_CATEGORY_FILE_VERSION_4   = 4,
  SIMLOCK_CATEGORY_FILE_VERSION_5   = 5,
  SIMLOCK_CATEGORY_FILE_VERSION_6   = 6,
  SIMLOCK_CATEGORY_FILE_VERSION_7   = 7,
  SIMLOCK_CATEGORY_FILE_VERSION_1_1 = 8,
  SIMLOCK_CATEGORY_FILE_VERSION_3_1 = 9,
  SIMLOCK_CATEGORY_FILE_VERSION_4_1 = 10,
  SIMLOCK_CATEGORY_FILE_VERSION_5_1 = 11,
  SIMLOCK_CATEGORY_FILE_VERSION_6_1 = 12,
  SIMLOCK_CATEGORY_FILE_VERSION_7_1 = 13
...
typedef PACKED struct PACKED_POST
{
  ...
  uint8                      hck[SIMLOCK_HCK_LEN];
  uint8                      token_present;
  uint8                      token[SIMLOCK_TOKEN_LEN];
  uint8                      num_of_codes;
} simlock_header_data_v1_type;
...
typedef PACKED struct PACKED_POST
{
  ...
  uint8                      gid1_len;
  uint8                      gid1_data[SIMLOCK_MAX_GID_LEN_V4];
} simlock_code_data_v7_type;

typedef PACKED struct PACKED_POST
{
  uint8                      code_type;
  uint8                      impi_length;
  uint8                      impi[SIMLOCK_IMPI_MAX];
} simlock_code_data_v6_type;
...
typedef PACKED struct PACKED_POST
{
  ...
  uint8                      gid1_data[SIMLOCK_MAX_GID_LEN_V4];
  uint8                      gid2_len;
  uint8                      gid2_data[SIMLOCK_MAX_GID_LEN_V4];
  uint8                      ehplmn_mcc[SIMLOCK_MCC_LEN];
  uint8                      ehplmn_mnc_length;
  uint8                      ehplmn_mnc[SIMLOCK_MNC_MAX];
} simlock_code_data_v4_type;
...
typedef PACKED struct PACKED_POST
{
  ...
  uint8                      mnc[SIMLOCK_MNC_MAX];
  uint8                      gid1_len;
  uint8                      gid1_data[SIMLOCK_MAX_GID_LEN_V2];
  uint8                      gid2_len;
  uint8                      gid2_data[SIMLOCK_MAX_GID_LEN_V2];
...
} simlock_category_mapping[] =
{
  ...
  { SIMLOCK_CATEGORY_3GPP_SP_EHPLMN, 12 },
  { SIMLOCK_CATEGORY_3GPP_ICCID,     13 },
  { SIMLOCK_CATEGORY_3GPP_IMPI,      14 },
  { SIMLOCK_CATEGORY_3GPP_NS_SP,     15 }
};
...
/*===========================================================================
FUNCTION SIMLOCK_CATEGORY_INITIALIZE_CODE_DATA

DESCRIPTION
  This function initializes the code data values. The values will be
  overwritten for specific category in the caller functions.
...
SIDE EFFECTS
  NONE
===========================================================================*/
void simlock_category_initialize_code_data
(
  simlock_category_code_data_type      *sim_code_data_ptr
)
{
  ...
  memset(sim_code_data_ptr, 0x00, sizeof(simlock_category_code_data_type));

  sim_code_data_ptr->code_type = SIMLOCK_CODE_WHITELIST;
  memset(sim_code_data_ptr->mcc, 0x00, sizeof(sim_code_data_ptr->mcc));
  memset(&sim_code_data_ptr->mnc, 0x00, sizeof(sim_code_data_ptr->mnc));
  sim_code_data_ptr->mnc.mnc_len = 0;
  sim_code_data_ptr->msin.msin_len = 0;
  memset(sim_code_data_ptr->msin.msin_data, 0x7F, SIMLOCK_MSIN_MAX);
  memset(sim_code_data_ptr->gid1.gid_data,
         SIMLOCK_LOCK_CODE_WILD_DIGIT,
         sizeof(sim_code_data_ptr->gid1.gid_data));
  sim_code_data_ptr->gid1.gid_len = 0;
  memset(sim_code_data_ptr->gid2.gid_data,
         SIMLOCK_LOCK_CODE_WILD_DIGIT,
         sizeof(sim_code_data_ptr->gid2.gid_data));
  sim_code_data_ptr->gid2.gid_len = 0;
  sim_code_data_ptr->spn.spn_valid = FALSE;
  memset(sim_code_data_ptr->spn.spn_data, 0xFF, SIMLOCK_SPN_LEN);
  memset(sim_code_data_ptr->ehplmn_mcc, 0x00, sizeof(sim_code_data_ptr->ehplmn_mcc));
  memset(&sim_code_data_ptr->ehplmn_mnc, 0x00, sizeof(sim_code_data_ptr->ehplmn_mnc));
  sim_code_data_ptr->ehplmn_mnc.mnc_len = 0;
  memset(&sim_code_data_ptr->iccid, 0x00, sizeof(sim_code_data_ptr->iccid));
  sim_code_data_ptr->iccid.iccid_len = 0;
} /* simlock_category_initialize_code_data */
...
)
{
  ...
  simlock_header_data_type                   header_data_v0;
  simlock_header_data_v1_type                header_data;
  ...
  union{
    ...
    simlock_code_data_v4_type                code_data_v4;
    simlock_code_data_v5_type                code_data_v5;
    simlock_code_data_v6_type                code_data_v6;
    simlock_code_data_v7_type                code_data_v7;
  }                                          code_data;
  ...
  memset(&header_data_v0, 0x00, sizeof(simlock_header_data_type));
  memset(&code_data, 0x00, sizeof(code_data));
  ...
  simlock_memscpy((uint8 *)&header_data_v0,
                  sizeof(header_data_v0),
                  category_file_data_ptr, sizeof(header_data_v0));

  /* copy the header data from packed struct */
  file_version = SIMLOCK_CATEGORY_GET_VERSION(header_data_v0.category_type_and_version);
  if((file_version != SIMLOCK_CATEGORY_FILE_VERSION_0) &&
     ...
     (file_version != SIMLOCK_CATEGORY_FILE_VERSION_4) &&
     (file_version != SIMLOCK_CATEGORY_FILE_VERSION_5) &&
     (file_version != SIMLOCK_CATEGORY_FILE_VERSION_6) &&
     (file_version != SIMLOCK_CATEGORY_FILE_VERSION_7) &&
     (file_version != SIMLOCK_CATEGORY_FILE_VERSION_1_1) &&
     (file_version != SIMLOCK_CATEGORY_FILE_VERSION_3_1) &&
     (file_version != SIMLOCK_CATEGORY_FILE_VERSION_4_1) &&
     (file_version != SIMLOCK_CATEGORY_FILE_VERSION_5_1) &&
     (file_version != SIMLOCK_CATEGORY_FILE_VERSION_6_1) &&
     (file_version != SIMLOCK_CATEGORY_FILE_VERSION_7_1))
  {
  ...
  if(file_version >= SIMLOCK_CATEGORY_FILE_VERSION_1_1 &&
     file_version <= SIMLOCK_CATEGORY_FILE_VERSION_7_1)
  { 
    /* if the file size is less than the minimum expected size,
       then it is not valid */
    if(category_file_size < SIMLOCK_CATEGORY_FILE_HEADER_SIZE_v1)
    {
      SIMLOCK_MSG_ERR_1("category file size is less than minimum size, 0x%x",
                        category_file_size);
      SIMLOCK_MEM_FREE(category_file_data_ptr);
    ...
    else
    {
      ...
      out_category_data_ptr->category_header.token.token_present =
        header_data.token_present;

      simlock_memscpy(out_category_data_ptr->category_header.token.token_data,
                      sizeof(out_category_data_ptr->category_header.token.token_data),
                      header_data.token, SIMLOCK_TOKEN_LEN);
    }
  }
  else
  {
    header_data.category_type_and_version = header_data_v0.category_type_and_version;
    header_data.auto_lock = header_data_v0.auto_lock;
    header_data.blacklist = header_data_v0.blacklist;
    header_data.num_retries_max = header_data_v0.num_retries_max;
    header_data.curr_retries = header_data_v0.curr_retries;
    header_data.iteration_cnt = header_data_v0.iteration_cnt;
    header_data.num_of_codes = header_data_v0.num_of_codes;

    simlock_memscpy(header_data.salt, sizeof(header_data.salt),
                    header_data_v0.salt, SIMLOCK_SALT_LEN);

    simlock_memscpy(header_data.hck, sizeof(header_data.hck),
                    header_data_v0.hck, SIMLOCK_HCK_LEN);

    out_category_data_ptr->category_header.token.token_present = FALSE;

    memset(out_category_data_ptr->category_header.token.token_data, 0x00,
           sizeof(out_category_data_ptr->category_header.token.token_data));

    offset = sizeof(header_data_v0);
  }
  ...
  lock_data_size = category_file_size - offset;
  ...
  if(((file_version == SIMLOCK_CATEGORY_FILE_VERSION_0) && (lock_data_size != (num_of_codes * sizeof(code_data.code_data_v0)))) ||
     ((file_version == SIMLOCK_CATEGORY_FILE_VERSION_1 ||
       file_version == SIMLOCK_CATEGORY_FILE_VERSION_1_1) && (lock_data_size != (num_of_codes * sizeof(code_data.code_data_v1)))) ||
     ((file_version == SIMLOCK_CATEGORY_FILE_VERSION_2) && (lock_data_size != (num_of_codes * sizeof(code_data.code_data_v2)))) ||
     ((file_version == SIMLOCK_CATEGORY_FILE_VERSION_3 ||
       file_version == SIMLOCK_CATEGORY_FILE_VERSION_3_1) && (lock_data_size != (num_of_codes * sizeof(code_data.code_data_v3)))) ||
     ((file_version == SIMLOCK_CATEGORY_FILE_VERSION_4 ||
       file_version == SIMLOCK_CATEGORY_FILE_VERSION_4_1) && (lock_data_size != (num_of_codes * sizeof(code_data.code_data_v4)))) ||
     ((file_version == SIMLOCK_CATEGORY_FILE_VERSION_5 ||
       file_version == SIMLOCK_CATEGORY_FILE_VERSION_5_1) && (lock_data_size != (num_of_codes * sizeof(code_data.code_data_v5)))) ||
     ((file_version == SIMLOCK_CATEGORY_FILE_VERSION_6 ||
       file_version == SIMLOCK_CATEGORY_FILE_VERSION_6_1) && (lock_data_size != (num_of_codes * sizeof(code_data.code_data_v6)))) ||
     ((file_version == SIMLOCK_CATEGORY_FILE_VERSION_7 ||
       file_version == SIMLOCK_CATEGORY_FILE_VERSION_7_1) && (lock_data_size != (num_of_codes * sizeof(code_data.code_data_v7)))))
  {
  ...
  if(num_of_codes > 0)
  {
    ...
    {
      if(file_version == SIMLOCK_CATEGORY_FILE_VERSION_7 ||
         file_version == SIMLOCK_CATEGORY_FILE_VERSION_7_1)
      {
        if(category != SIMLOCK_CATEGORY_3GPP_NS_SP)
        {
          SIMLOCK_MEM_FREE(category_file_data_ptr);
          return SIMLOCK_GENERIC_ERROR;
        }
        ...
        simlock_memscpy((uint8 *)&code_data.code_data_v7,
                        sizeof(code_data.code_data_v7),
                        &category_file_data_ptr[offset],
                        sizeof(code_data.code_data_v7));

        if(code_data.code_data_v7.code_type != 0)
        {
        ...
        simlock_memscpy(out_category_data_ptr->code_data_ptr[code_index].mcc,
                        SIMLOCK_MCC_LEN,
                        code_data.code_data_v7.mcc,
                        SIMLOCK_MCC_LEN);

        out_category_data_ptr->code_data_ptr[code_index].mnc.mnc_len = code_data.code_data_v7.mnc_length;
        simlock_memscpy(out_category_data_ptr->code_data_ptr[code_index].mnc.mnc_data,
                        SIMLOCK_MNC_MAX,
                        code_data.code_data_v7.mnc,
                        SIMLOCK_MNC_MAX);

        out_category_data_ptr->code_data_ptr[code_index].msin.msin_len = code_data.code_data_v7.msin_length;
        simlock_memscpy(out_category_data_ptr->code_data_ptr[code_index].msin.msin_data,
                        SIMLOCK_MSIN_MAX,
                        code_data.code_data_v7.msin,
                        SIMLOCK_MSIN_MAX);

        out_category_data_ptr->code_data_ptr[code_index].gid1.gid_len = code_data.code_data_v7.gid1_len;
        simlock_memscpy(out_category_data_ptr->code_data_ptr[code_index].gid1.gid_data,
                        sizeof(out_category_data_ptr->code_data_ptr[code_index].gid1.gid_data),
                        code_data.code_data_v7.gid1_data,
                        sizeof(code_data.code_data_v7.gid1_data));

        offset += sizeof(code_data.code_data_v7);
      }

      else if(file_version == SIMLOCK_CATEGORY_FILE_VERSION_6 ||
              file_version == SIMLOCK_CATEGORY_FILE_VERSION_6_1)
      {
        if(category != SIMLOCK_CATEGORY_3GPP_IMPI)
        {
          SIMLOCK_MEM_FREE(category_file_data_ptr);
          return SIMLOCK_GENERIC_ERROR;
        }
        ...
        /* copy the category code data (raw data) to packed struct */
        simlock_memscpy((uint8 *)&code_data.code_data_v6,
                        sizeof(code_data.code_data_v6),
                        &category_file_data_ptr[offset],
                        sizeof(code_data.code_data_v6));

        if(code_data.code_data_v6.code_type != 0)
        {
          out_category_data_ptr->code_data_ptr[code_index].code_type = SIMLOCK_CODE_BLACKLIST;
        }
        else
        ...
        if(code_data.code_data_v6.impi_length > SIMLOCK_IMPI_MAX)
        {
          SIMLOCK_MEM_FREE(category_file_data_ptr);
          return SIMLOCK_GENERIC_ERROR;
        }

        out_category_data_ptr->code_data_ptr[code_index].impi.impi_len = code_data.code_data_v6.impi_length;

        simlock_memscpy(out_category_data_ptr->code_data_ptr[code_index].impi.impi_data,
                        SIMLOCK_IMPI_MAX,
                        code_data.code_data_v6.impi,
                        SIMLOCK_IMPI_MAX);

        offset += sizeof(code_data.code_data_v6);
      }
      else if(file_version == SIMLOCK_CATEGORY_FILE_VERSION_5 ||
              file_version == SIMLOCK_CATEGORY_FILE_VERSION_5_1)
      {
        ...
        /* copy the category code data (raw data) to packed struct */
        simlock_memscpy((uint8 *)&code_data.code_data_v5,
                        sizeof(code_data.code_data_v5),
                        &category_file_data_ptr[offset],
                        sizeof(code_data.code_data_v5));

        if(code_data.code_data_v5.code_type != 0)
        {
          out_category_data_ptr->code_data_ptr[code_index].code_type = SIMLOCK_CODE_BLACKLIST;
        }
        else
        ...
        simlock_memscpy(out_category_data_ptr->code_data_ptr[code_index].mcc,
                        SIMLOCK_MCC_LEN,
                        code_data.code_data_v5.mcc,
                        SIMLOCK_MCC_LEN);

        out_category_data_ptr->code_data_ptr[code_index].mnc.mnc_len = code_data.code_data_v5.mnc_length;

        simlock_memscpy(out_category_data_ptr->code_data_ptr[code_index].mnc.mnc_data,
                        SIMLOCK_MNC_MAX,
                        code_data.code_data_v5.mnc,
                        SIMLOCK_MNC_MAX);

        out_category_data_ptr->code_data_ptr[code_index].iccid.iccid_len = code_data.code_data_v5.iccid_length;

        simlock_memscpy(out_category_data_ptr->code_data_ptr[code_index].iccid.iccid_data,
                        SIMLOCK_ICCID_DIGITS_MAX,
                        code_data.code_data_v5.iccid,
                        SIMLOCK_ICCID_DIGITS_MAX);

        offset += sizeof(code_data.code_data_v5);
      }
      else if(file_version == SIMLOCK_CATEGORY_FILE_VERSION_4 ||
              file_version == SIMLOCK_CATEGORY_FILE_VERSION_4_1)
      {
        if(category != SIMLOCK_CATEGORY_3GPP_CP &&
           category != SIMLOCK_CATEGORY_3GPP_SP &&
           category != SIMLOCK_CATEGORY_3GPP_SP_EHPLMN)
        {
          SIMLOCK_MEM_FREE(category_file_data_ptr);
          return SIMLOCK_GENERIC_ERROR;
        }
        /* copy the category code data (raw data) to packed struct */
        simlock_memscpy((uint8 *)&code_data.code_data_v4,
                        sizeof(code_data.code_data_v4),
                        &category_file_data_ptr[offset],
                        sizeof(code_data.code_data_v4));

        if(code_data.code_data_v4.code_type != 0)
        {
          out_category_data_ptr->code_data_ptr[code_index].code_type = SIMLOCK_CODE_BLACKLIST;
        }
        else
        ...
        simlock_memscpy(out_category_data_ptr->code_data_ptr[code_index].mcc,
                        SIMLOCK_MCC_LEN,
                        code_data.code_data_v4.mcc,
                        SIMLOCK_MCC_LEN);

        out_category_data_ptr->code_data_ptr[code_index].mnc.mnc_len = code_data.code_data_v4.mnc_length;

        simlock_memscpy(out_category_data_ptr->code_data_ptr[code_index].mnc.mnc_data,
                        SIMLOCK_MNC_MAX,
                        code_data.code_data_v4.mnc,
                        SIMLOCK_MNC_MAX);

        out_category_data_ptr->code_data_ptr[code_index].gid1.gid_len = code_data.code_data_v4.gid1_len;

        simlock_memscpy(out_category_data_ptr->code_data_ptr[code_index].gid1.gid_data,
                        sizeof(out_category_data_ptr->code_data_ptr[code_index].gid1.gid_data),
                        code_data.code_data_v4.gid1_data,
                        sizeof(code_data.code_data_v4.gid1_data));

        out_category_data_ptr->code_data_ptr[code_index].gid2.gid_len = code_data.code_data_v4.gid2_len;

        simlock_memscpy(out_category_data_ptr->code_data_ptr[code_index].gid2.gid_data,
                        sizeof(out_category_data_ptr->code_data_ptr[code_index].gid2.gid_data),
                        code_data.code_data_v4.gid2_data,
                        sizeof(code_data.code_data_v4.gid2_data));

        simlock_memscpy(out_category_data_ptr->code_data_ptr[code_index].ehplmn_mcc,
                        SIMLOCK_MCC_LEN,
                        code_data.code_data_v4.ehplmn_mcc,
                        SIMLOCK_MCC_LEN);

        out_category_data_ptr->code_data_ptr[code_index].ehplmn_mnc.mnc_len = code_data.code_data_v4.ehplmn_mnc_length;

        simlock_memscpy(out_category_data_ptr->code_data_ptr[code_index].ehplmn_mnc.mnc_data,
                        SIMLOCK_MNC_MAX,
                        code_data.code_data_v4.ehplmn_mnc,
                        SIMLOCK_MNC_MAX);

        offset += sizeof(code_data.code_data_v4);
      }
              ...
              file_version == SIMLOCK_CATEGORY_FILE_VERSION_3_1)
      {
        ...
        /* copy the category code data (raw data) to packed struct */
        simlock_memscpy((uint8 *)&code_data.code_data_v3,
                        sizeof(code_data.code_data_v3),
                        &category_file_data_ptr[offset],
                        sizeof(code_data.code_data_v3));

        if(code_data.code_data_v3.code_type != 0)
        {
          out_category_data_ptr->code_data_ptr[code_index].code_type = SIMLOCK_CODE_BLACKLIST;
        }
        else
        ...
        simlock_memscpy(out_category_data_ptr->code_data_ptr[code_index].mcc,
                        SIMLOCK_MCC_LEN,
                        code_data.code_data_v3.mcc,
                        SIMLOCK_MCC_LEN);

        out_category_data_ptr->code_data_ptr[code_index].mnc.mnc_len = code_data.code_data_v3.mnc_length;

        simlock_memscpy(out_category_data_ptr->code_data_ptr[code_index].mnc.mnc_data,
                        SIMLOCK_MNC_MAX,
                        code_data.code_data_v3.mnc,
                        SIMLOCK_MNC_MAX);

        simlock_memscpy(out_category_data_ptr->code_data_ptr[code_index].spn.spn_data,
                        SIMLOCK_SPN_LEN,
                        code_data.code_data_v3.spn_data,
                        SIMLOCK_SPN_LEN);

        out_category_data_ptr->code_data_ptr[code_index].spn.spn_valid = TRUE;

        offset += sizeof(code_data.code_data_v3);
      }
      else if(file_version == SIMLOCK_CATEGORY_FILE_VERSION_2)
      {
        ...
        /* copy the category code data (raw data) to packed struct */
        simlock_memscpy((uint8 *)&code_data.code_data_v2,
                        sizeof(code_data.code_data_v2),
                        &category_file_data_ptr[offset],
                        sizeof(code_data.code_data_v2));

        if(code_data.code_data_v2.code_type != 0)
        {
          out_category_data_ptr->code_data_ptr[code_index].code_type = SIMLOCK_CODE_BLACKLIST;
        }
        else
        ...
        simlock_memscpy(out_category_data_ptr->code_data_ptr[code_index].mcc,
                        SIMLOCK_MCC_LEN,
                        code_data.code_data_v2.mcc,
                        SIMLOCK_MCC_LEN);

        out_category_data_ptr->code_data_ptr[code_index].mnc.mnc_len = code_data.code_data_v2.mnc_length;

        simlock_memscpy(out_category_data_ptr->code_data_ptr[code_index].mnc.mnc_data,
                        SIMLOCK_MNC_MAX,
                        code_data.code_data_v2.mnc,
                        SIMLOCK_MNC_MAX);

        out_category_data_ptr->code_data_ptr[code_index].gid1.gid_len = code_data.code_data_v2.gid1_len;

        simlock_memscpy(out_category_data_ptr->code_data_ptr[code_index].gid1.gid_data,
                        sizeof(out_category_data_ptr->code_data_ptr[code_index].gid1.gid_data),
                        code_data.code_data_v2.gid1_data,
                        sizeof(code_data.code_data_v2.gid1_data));

        out_category_data_ptr->code_data_ptr[code_index].gid2.gid_len = code_data.code_data_v2.gid2_len;

        simlock_memscpy(out_category_data_ptr->code_data_ptr[code_index].gid2.gid_data,
                        sizeof(out_category_data_ptr->code_data_ptr[code_index].gid2.gid_data),
                        code_data.code_data_v2.gid2_data,
                        sizeof(code_data.code_data_v2.gid2_data));

        offset += sizeof(code_data.code_data_v2);
      }
      else if(file_version == SIMLOCK_CATEGORY_FILE_VERSION_1 ||
              file_version == SIMLOCK_CATEGORY_FILE_VERSION_1_1)
      {
        /* copy the category code data (raw data) to packed struct */
        simlock_memscpy((uint8 *)&code_data.code_data_v1,
                        sizeof(code_data.code_data_v1),
                        &category_file_data_ptr[offset],
                        sizeof(code_data.code_data_v1));

        if(code_data.code_data_v1.code_type != 0)
        {
          out_category_data_ptr->code_data_ptr[code_index].code_type = SIMLOCK_CODE_BLACKLIST;
        }
        else
        ...
        simlock_memscpy(out_category_data_ptr->code_data_ptr[code_index].mcc,
                        SIMLOCK_MCC_LEN,
                        code_data.code_data_v1.mcc,
                        SIMLOCK_MCC_LEN);

        out_category_data_ptr->code_data_ptr[code_index].mnc.mnc_len = code_data.code_data_v1.mnc_length;

        simlock_memscpy(out_category_data_ptr->code_data_ptr[code_index].mnc.mnc_data,
                        SIMLOCK_MNC_MAX,
                        code_data.code_data_v1.mnc,
                        SIMLOCK_MNC_MAX);

        out_category_data_ptr->code_data_ptr[code_index].msin.msin_len = code_data.code_data_v1.msin_length;

        simlock_memscpy(out_category_data_ptr->code_data_ptr[code_index].msin.msin_data,
                        SIMLOCK_MSIN_MAX,
                        code_data.code_data_v1.msin,
                        SIMLOCK_MSIN_MAX);

        out_category_data_ptr->code_data_ptr[code_index].gid1.gid_data[0] =
          code_data.code_data_v1.gid1;
        ...
        out_category_data_ptr->code_data_ptr[code_index].gid2.gid_data[0] =
          code_data.code_data_v1.gid2;
        out_category_data_ptr->code_data_ptr[code_index].gid2.gid_len =
          SIMLOCK_MIN_GID_LEN;

        offset += sizeof(code_data.code_data_v1);
      }
      else if (file_version == SIMLOCK_CATEGORY_FILE_VERSION_0)
      {
        /* copy the category code data (raw data) to packed struct */
        simlock_memscpy((uint8 *)&code_data.code_data_v0,
                        sizeof(code_data.code_data_v0),
                        &category_file_data_ptr[offset],
                        sizeof(code_data.code_data_v0));
        ...
        simlock_memscpy(out_category_data_ptr->code_data_ptr[code_index].mcc,
                        SIMLOCK_MCC_LEN,
                        code_data.code_data_v0.mcc,
                        SIMLOCK_MCC_LEN);

        out_category_data_ptr->code_data_ptr[code_index].mnc.mnc_len = code_data.code_data_v0.mnc_length;

        simlock_memscpy(out_category_data_ptr->code_data_ptr[code_index].mnc.mnc_data,
                        SIMLOCK_MNC_MAX,
                        code_data.code_data_v0.mnc,
                        SIMLOCK_MNC_MAX);

        out_category_data_ptr->code_data_ptr[code_index].msin.msin_len = code_data.code_data_v0.msin_length;

        simlock_memscpy(out_category_data_ptr->code_data_ptr[code_index].msin.msin_data,
                        SIMLOCK_MSIN_MAX,
                        code_data.code_data_v0.msin,
                        SIMLOCK_MSIN_MAX);

        out_category_data_ptr->code_data_ptr[code_index].gid1.gid_data[0] =
          code_data.code_data_v0.gid1;
        out_category_data_ptr->code_data_ptr[code_index].gid1.gid_len =
          SIMLOCK_MIN_GID_LEN;
        out_category_data_ptr->code_data_ptr[code_index].gid2.gid_data[0] =
          code_data.code_data_v0.gid2;
        out_category_data_ptr->code_data_ptr[code_index].gid2.gid_len =
          SIMLOCK_MIN_GID_LEN;

        offset += sizeof(code_data.code_data_v0);
      }
    }
  }
...
)
{
  ...
  memset(&header_data, 0x00, sizeof(simlock_header_data_v1_type));
  ...
  {
    category_data_length = (sizeof(simlock_header_data_v1_type) +
          ...
          category_data_ptr->category_header.category_type == SIMLOCK_CATEGORY_3GPP_SP_EHPLMN)
  {
    category_data_length = (sizeof(simlock_header_data_v1_type) +
                            (category_data_ptr->category_header.num_of_codes * sizeof(simlock_code_data_v4_type)));
  }
  ...
  else if(category_data_ptr->category_header.category_type == SIMLOCK_CATEGORY_3GPP_IMPI)
  {
    category_data_length = (sizeof(simlock_header_data_v1_type) +
                            (category_data_ptr->category_header.num_of_codes * sizeof(simlock_code_data_v6_type)));
  }
  else if(category_data_ptr->category_header.category_type == SIMLOCK_CATEGORY_3GPP_NS_SP)
  {
    category_data_length = (sizeof(simlock_header_data_v1_type) +
                            (category_data_ptr->category_header.num_of_codes * sizeof(simlock_code_data_v7_type)));
  }
  else
  {
    category_data_length = (sizeof(simlock_header_data_v1_type) +
  ...
  if(category_data_length < SIMLOCK_CATEGORY_FILE_HEADER_SIZE_v1)
  {
  ...
  if(category_data_ptr->category_header.category_type == SIMLOCK_CATEGORY_3GPP_SPN)
  {
    header_data.category_type_and_version = SIMLOCK_CATEGORY_ADD_VERSION_INFO(category_index,
                                                                              SIMLOCK_CATEGORY_FILE_VERSION_3_1);
  }
  else if(category_data_ptr->category_header.category_type == SIMLOCK_CATEGORY_3GPP_CP ||
          category_data_ptr->category_header.category_type == SIMLOCK_CATEGORY_3GPP_SP ||
          category_data_ptr->category_header.category_type == SIMLOCK_CATEGORY_3GPP_SP_EHPLMN)
  {
    header_data.category_type_and_version = SIMLOCK_CATEGORY_ADD_VERSION_INFO(category_index,
                                                                              SIMLOCK_CATEGORY_FILE_VERSION_4_1);
  }
  else if(category_data_ptr->category_header.category_type == SIMLOCK_CATEGORY_3GPP_ICCID)
  {
    header_data.category_type_and_version = SIMLOCK_CATEGORY_ADD_VERSION_INFO(category_index,
                                                                              SIMLOCK_CATEGORY_FILE_VERSION_5_1);
  }
  else if(category_data_ptr->category_header.category_type == SIMLOCK_CATEGORY_3GPP_IMPI)
  {
    header_data.category_type_and_version = SIMLOCK_CATEGORY_ADD_VERSION_INFO(category_index,
                                                                              SIMLOCK_CATEGORY_FILE_VERSION_6_1);
  }
  else if(category_data_ptr->category_header.category_type == SIMLOCK_CATEGORY_3GPP_NS_SP)
  {
    header_data.category_type_and_version = SIMLOCK_CATEGORY_ADD_VERSION_INFO(category_index,
                                                                              SIMLOCK_CATEGORY_FILE_VERSION_7_1);
  }
  else
  {
    header_data.category_type_and_version = SIMLOCK_CATEGORY_ADD_VERSION_INFO(category_index,
                                                                              SIMLOCK_CATEGORY_FILE_VERSION_1_1);
  }
  ...
  header_data.token_present = category_data_ptr->category_header.token.token_present;

  simlock_memscpy(header_data.token,
                  SIMLOCK_TOKEN_LEN,
  ...
  if((buffer_index + sizeof(simlock_header_data_v1_type)) <= 
           category_data_length)
  {
    ...
    buffer_ptr[buffer_index++] = header_data.token_present;

    simlock_memscpy(&buffer_ptr[buffer_index],
                    (category_data_length - buffer_index),
                    header_data.token, SIMLOCK_TOKEN_LEN);

    buffer_index += SIMLOCK_TOKEN_LEN;
  ...
  /* sanity check the buffer_index. If it is not equal to header size,
     then it is an error */
  if(buffer_index != SIMLOCK_CATEGORY_FILE_HEADER_SIZE_v1)
  {
      ...
      code_cnt++)
  {
    ...
    if(category_data_ptr->category_header.category_type == SIMLOCK_CATEGORY_3GPP_SPN)
    {
      ...
      {
        code_data.code_data_v3.code_type = 0x01;
      }

      simlock_memscpy(code_data.code_data_v3.mcc,
                      SIMLOCK_MCC_LEN,
                      category_data_ptr->code_data_ptr[code_cnt].mcc,
                      SIMLOCK_MCC_LEN);

      code_data.code_data_v3.mnc_length = category_data_ptr->code_data_ptr[code_cnt].mnc.mnc_len;

      simlock_memscpy(code_data.code_data_v3.mnc,
                      SIMLOCK_MNC_MAX,
                      category_data_ptr->code_data_ptr[code_cnt].mnc.mnc_data,
                      SIMLOCK_MNC_MAX);

      simlock_memscpy(code_data.code_data_v3.spn_data,
                      SIMLOCK_SPN_LEN,
      ...
      simlock_memscpy(&buffer_ptr[buffer_index],
                      (category_data_length - buffer_index),
                      (uint8 *)&code_data.code_data_v3,
                      sizeof(code_data.code_data_v3));

      buffer_index += sizeof(code_data.code_data_v3);
    }
    else if(category_data_ptr->category_header.category_type == SIMLOCK_CATEGORY_3GPP_CP ||
            category_data_ptr->category_header.category_type == SIMLOCK_CATEGORY_3GPP_SP ||
            category_data_ptr->category_header.category_type == SIMLOCK_CATEGORY_3GPP_SP_EHPLMN)
    {
      if(category_data_ptr->code_data_ptr[code_cnt].code_type == SIMLOCK_CODE_BLACKLIST)
      {
        code_data.code_data_v4.code_type = 0x01;
      }

      simlock_memscpy(code_data.code_data_v4.mcc,
                      SIMLOCK_MCC_LEN,
                      category_data_ptr->code_data_ptr[code_cnt].mcc,
                      SIMLOCK_MCC_LEN);

      code_data.code_data_v4.mnc_length = category_data_ptr->code_data_ptr[code_cnt].mnc.mnc_len;

      simlock_memscpy(code_data.code_data_v4.mnc,
                      SIMLOCK_MNC_MAX,
                      category_data_ptr->code_data_ptr[code_cnt].mnc.mnc_data,
                      SIMLOCK_MNC_MAX);

      code_data.code_data_v4.gid1_len = category_data_ptr->code_data_ptr[code_cnt].gid1.gid_len;

      simlock_memscpy(code_data.code_data_v4.gid1_data,
                      sizeof(code_data.code_data_v4.gid1_data),
                      category_data_ptr->code_data_ptr[code_cnt].gid1.gid_data,
                      sizeof(category_data_ptr->code_data_ptr[code_cnt].gid1.gid_data));

      code_data.code_data_v4.gid2_len = category_data_ptr->code_data_ptr[code_cnt].gid2.gid_len;

      simlock_memscpy(code_data.code_data_v4.gid2_data,
                      sizeof(code_data.code_data_v4.gid2_data),
                      category_data_ptr->code_data_ptr[code_cnt].gid2.gid_data,
                      sizeof(category_data_ptr->code_data_ptr[code_cnt].gid2.gid_data));

      simlock_memscpy(code_data.code_data_v4.ehplmn_mcc,
                      SIMLOCK_MCC_LEN,
                      category_data_ptr->code_data_ptr[code_cnt].ehplmn_mcc,
                      SIMLOCK_MCC_LEN);

      code_data.code_data_v4.ehplmn_mnc_length = category_data_ptr->code_data_ptr[code_cnt].ehplmn_mnc.mnc_len;

      simlock_memscpy(code_data.code_data_v4.ehplmn_mnc,
                      SIMLOCK_MNC_MAX,
                      category_data_ptr->code_data_ptr[code_cnt].ehplmn_mnc.mnc_data,
                      SIMLOCK_MNC_MAX);

      /* copy the entire code data to the data buffer */
      simlock_memscpy(&buffer_ptr[buffer_index],
                      (category_data_length - buffer_index),
                      (uint8 *)&code_data.code_data_v4,
                      sizeof(code_data.code_data_v4));

      buffer_index += sizeof(code_data.code_data_v4);
    }
    else if(category_data_ptr->category_header.category_type == SIMLOCK_CATEGORY_3GPP_ICCID)
    {
      if(category_data_ptr->code_data_ptr[code_cnt].code_type == SIMLOCK_CODE_BLACKLIST)
      {
        code_data.code_data_v5.code_type = 0x01;
      }

      simlock_memscpy(code_data.code_data_v5.mcc,
                      SIMLOCK_MCC_LEN,
                      category_data_ptr->code_data_ptr[code_cnt].mcc,
                      SIMLOCK_MCC_LEN);

      code_data.code_data_v5.mnc_length = category_data_ptr->code_data_ptr[code_cnt].mnc.mnc_len;

      simlock_memscpy(code_data.code_data_v5.mnc,
                      SIMLOCK_MNC_MAX,
                      category_data_ptr->code_data_ptr[code_cnt].mnc.mnc_data,
                      SIMLOCK_MNC_MAX);

      code_data.code_data_v5.iccid_length = category_data_ptr->code_data_ptr[code_cnt].iccid.iccid_len;

      simlock_memscpy(code_data.code_data_v5.iccid,
                      SIMLOCK_ICCID_DIGITS_MAX,
      ...
      simlock_memscpy(&buffer_ptr[buffer_index],
                      (category_data_length - buffer_index),
                      (uint8 *)&code_data.code_data_v5,
                      sizeof(code_data.code_data_v5));

      buffer_index += sizeof(code_data.code_data_v5);
    }
    else if(category_data_ptr->category_header.category_type == SIMLOCK_CATEGORY_3GPP_IMPI)
    {
      if(category_data_ptr->code_data_ptr[code_cnt].code_type == SIMLOCK_CODE_BLACKLIST)
      {
        code_data.code_data_v6.code_type = 0x01;
      }

      code_data.code_data_v6.impi_length = category_data_ptr->code_data_ptr[code_cnt].impi.impi_len;

      simlock_memscpy(code_data.code_data_v6.impi,
                      sizeof(code_data.code_data_v6.impi),
                      category_data_ptr->code_data_ptr[code_cnt].impi.impi_data,
                      sizeof(category_data_ptr->code_data_ptr[code_cnt].impi.impi_data));

      /* copy the entire code data to the data buffer */
      simlock_memscpy(&buffer_ptr[buffer_index],
                      (category_data_length - buffer_index),
                      (uint8 *)&code_data.code_data_v6,
                      sizeof(code_data.code_data_v6));

      buffer_index += sizeof(code_data.code_data_v6);
    }
    else if(category_data_ptr->category_header.category_type == SIMLOCK_CATEGORY_3GPP_NS_SP)
    {
      if(category_data_ptr->code_data_ptr[code_cnt].code_type == SIMLOCK_CODE_BLACKLIST)
      {
        code_data.code_data_v7.code_type = 0x01;
      }

      simlock_memscpy(code_data.code_data_v7.mcc,
                      SIMLOCK_MCC_LEN,
                      category_data_ptr->code_data_ptr[code_cnt].mcc,
                      SIMLOCK_MCC_LEN);

      code_data.code_data_v7.mnc_length = category_data_ptr->code_data_ptr[code_cnt].mnc.mnc_len;
      simlock_memscpy(code_data.code_data_v7.mnc,
                      SIMLOCK_MNC_MAX,
                      category_data_ptr->code_data_ptr[code_cnt].mnc.mnc_data,
                      SIMLOCK_MNC_MAX);

      code_data.code_data_v7.msin_length = category_data_ptr->code_data_ptr[code_cnt].msin.msin_len;
      simlock_memscpy(code_data.code_data_v7.msin,
                      SIMLOCK_MSIN_MAX,
                      category_data_ptr->code_data_ptr[code_cnt].msin.msin_data,
                      SIMLOCK_MSIN_MAX);

      code_data.code_data_v7.gid1_len = category_data_ptr->code_data_ptr[code_cnt].gid1.gid_len;
      simlock_memscpy(code_data.code_data_v7.gid1_data,
                      sizeof(code_data.code_data_v7.gid1_data),
                      category_data_ptr->code_data_ptr[code_cnt].gid1.gid_data,
                      sizeof(code_data.code_data_v7.gid1_data));
      ...
      simlock_memscpy(&buffer_ptr[buffer_index],
                      (category_data_length - buffer_index),
                      (uint8 *)&code_data.code_data_v7,
                      sizeof(code_data.code_data_v7));

      buffer_index += sizeof(code_data.code_data_v7);
    }
    else
    {
      if(category_data_ptr->code_data_ptr[code_cnt].code_type == SIMLOCK_CODE_BLACKLIST)
      {
        code_data.code_data_v1.code_type = 0x01;
      }

      simlock_memscpy(code_data.code_data_v1.mcc,
                      SIMLOCK_MCC_LEN,
                      category_data_ptr->code_data_ptr[code_cnt].mcc,
                      SIMLOCK_MCC_LEN);

      code_data.code_data_v1.mnc_length = category_data_ptr->code_data_ptr[code_cnt].mnc.mnc_len;

      simlock_memscpy(code_data.code_data_v1.mnc,
                      SIMLOCK_MNC_MAX,
                      category_data_ptr->code_data_ptr[code_cnt].mnc.mnc_data,
                      SIMLOCK_MNC_MAX);

      code_data.code_data_v1.msin_length = category_data_ptr->code_data_ptr[code_cnt].msin.msin_len;

      simlock_memscpy(code_data.code_data_v1.msin,
                      SIMLOCK_MSIN_MAX,
                      category_data_ptr->code_data_ptr[code_cnt].msin.msin_data,
                      SIMLOCK_MSIN_MAX);

      code_data.code_data_v1.gid1 = category_data_ptr->code_data_ptr[code_cnt].gid1.gid_data[0];
      code_data.code_data_v1.gid2 = category_data_ptr->code_data_ptr[code_cnt].gid2.gid_data[0];

      /* copy the entire code data to the data buffer */
      simlock_memscpy(&buffer_ptr[buffer_index],
                      (category_data_length - buffer_index),
                      (uint8 *)&code_data.code_data_v1,
                      sizeof(code_data.code_data_v1));

      buffer_index += sizeof(code_data.code_data_v1);
    }      
  }
...
/*===========================================================================
FUNCTION SIMLOCK_CATEGORY_BUILD_HEADER_DATA

DESCRIPTION
  This function builds the header data for a category from the request
...
SIDE EFFECTS
  NONE
===========================================================================*/
simlock_result_enum_type simlock_category_build_header_data
(
...
)
{
  ...
  category_data_ptr->category_header.category_type = category_type;
  category_data_ptr->category_header.auto_lock = auto_lock;
  if(blacklist)
  {
  ...
  category_data_ptr->category_header.num_retries_max = num_retries_max;
  category_data_ptr->category_header.iteration_cnt = iteration_cnt;
  ...
  return SIMLOCK_SUCCESS;
} /* simlock_category_build_header_data */
...
(
  ...
  uint32                                      num_retries_max,
  uint32                                      iteration_cnt,
  const simlock_category_data_type         *  category_ptr,
  const simlock_salt_type                     salt,
  const simlock_hck_type                      hck,
  simlock_category_file_data_type          *  category_data_ptr
)
{
  ...
  /* build the header data from the request */
  if(simlock_category_build_header_data(category_ptr->category_type,
                                        auto_lock,
                                        blacklist,
                                        num_retries_max,
                                        iteration_cnt,
                                        salt,
                                        hck,
                                        category_data_ptr) != SIMLOCK_SUCCESS)
  {
    return SIMLOCK_GENERIC_ERROR;
  }

  /* build the code data from the request */
  switch(category_ptr->category_type)
  {
    case SIMLOCK_CATEGORY_3GPP_NW:
      category_data_ptr->category_header.num_of_codes =
        category_ptr->code_data.nw_3gpp_code_list.num_nw_codes;

      /* in case of autolock num of codes can be 0 */
      if(category_ptr->code_data.nw_3gpp_code_list.num_nw_codes != 0)
      {
        code_data_length = (category_ptr->code_data.nw_3gpp_code_list.num_nw_codes *
      ...
      for(code_cnt = 0; code_cnt < category_ptr->code_data.nw_3gpp_code_list.num_nw_codes;
          code_cnt++)
      {
        SIMLOCK_MSG_LOW_6("From req - mcc[0]: 0x%x, mcc[1]: 0x%x, mcc[2]: 0x%x, mnc[0]: 0x%x, mnc[1]: 0x%x, mnc[2]: 0x%x",
                          category_ptr->code_data.nw_3gpp_code_list.nw_code_data[code_cnt].mcc[0],
                          category_ptr->code_data.nw_3gpp_code_list.nw_code_data[code_cnt].mcc[1],
                          category_ptr->code_data.nw_3gpp_code_list.nw_code_data[code_cnt].mcc[2],
                          category_ptr->code_data.nw_3gpp_code_list.nw_code_data[code_cnt].mnc.mnc_data[0],
                          category_ptr->code_data.nw_3gpp_code_list.nw_code_data[code_cnt].mnc.mnc_data[1],
                          category_ptr->code_data.nw_3gpp_code_list.nw_code_data[code_cnt].mnc.mnc_data[2]);
        ...
        simlock_memscpy(category_data_ptr->code_data_ptr[code_cnt].mcc,
                        SIMLOCK_MCC_LEN,
                        category_ptr->code_data.nw_3gpp_code_list.nw_code_data[code_cnt].mcc,
                        SIMLOCK_MCC_LEN);

        category_data_ptr->code_data_ptr[code_cnt].mnc.mnc_len =
               category_ptr->code_data.nw_3gpp_code_list.nw_code_data[code_cnt].mnc.mnc_len;

        simlock_memscpy(category_data_ptr->code_data_ptr[code_cnt].mnc.mnc_data,
                        SIMLOCK_MNC_MAX,
                        category_ptr->code_data.nw_3gpp_code_list.nw_code_data[code_cnt].mnc.mnc_data,
                        category_ptr->code_data.nw_3gpp_code_list.nw_code_data[code_cnt].mnc.mnc_len);
        ...
        memset(category_data_ptr->code_data_ptr[code_cnt].ehplmn_mcc, 0xFF, SIMLOCK_MCC_LEN);
        category_data_ptr->code_data_ptr[code_cnt].ehplmn_mnc.mnc_len = 0;
        memset(category_data_ptr->code_data_ptr[code_cnt].ehplmn_mnc.mnc_data, 0xFF, SIMLOCK_MNC_MAX);
        ...
        category_data_ptr->code_data_ptr[code_cnt].impi.impi_len = 0;
        memset(category_data_ptr->code_data_ptr[code_cnt].impi.impi_data, 0xFF, SIMLOCK_IMPI_MAX);
      }
      break;

    case SIMLOCK_CATEGORY_3GPP_NS:
      category_data_ptr->category_header.num_of_codes =
        category_ptr->code_data.ns_3gpp_code_list.num_ns_codes;

      /* in case of autolock num of codes can be 0 */
      if(category_ptr->code_data.ns_3gpp_code_list.num_ns_codes != 0)
      {
        code_data_length = (category_ptr->code_data.ns_3gpp_code_list.num_ns_codes *
                            sizeof(simlock_category_code_data_type));
      ...
      for(code_cnt = 0; code_cnt < category_ptr->code_data.ns_3gpp_code_list.num_ns_codes;
          code_cnt++)
      {
        SIMLOCK_MSG_LOW_6("From req - mcc[0]: 0x%x, mcc[1]: 0x%x, mcc[2]: 0x%x, mnc[0]: 0x%x, mnc[1]: 0x%x, mnc[2]: 0x%x",
                          category_ptr->code_data.ns_3gpp_code_list.ns_code_data[code_cnt].nw_code.mcc[0],
                          category_ptr->code_data.ns_3gpp_code_list.ns_code_data[code_cnt].nw_code.mcc[1],
                          category_ptr->code_data.ns_3gpp_code_list.ns_code_data[code_cnt].nw_code.mcc[2],
                          category_ptr->code_data.ns_3gpp_code_list.ns_code_data[code_cnt].nw_code.mnc.mnc_data[0],
                          category_ptr->code_data.ns_3gpp_code_list.ns_code_data[code_cnt].nw_code.mnc.mnc_data[1],
                          category_ptr->code_data.ns_3gpp_code_list.ns_code_data[code_cnt].nw_code.mnc.mnc_data[2]);
        ...
        simlock_memscpy(category_data_ptr->code_data_ptr[code_cnt].mcc,
                        SIMLOCK_MCC_LEN,
                        category_ptr->code_data.ns_3gpp_code_list.ns_code_data[code_cnt].nw_code.mcc,
                        SIMLOCK_MCC_LEN);

        category_data_ptr->code_data_ptr[code_cnt].mnc.mnc_len =
             category_ptr->code_data.ns_3gpp_code_list.ns_code_data[code_cnt].nw_code.mnc.mnc_len;

        simlock_memscpy(category_data_ptr->code_data_ptr[code_cnt].mnc.mnc_data,
                        SIMLOCK_MNC_MAX,
                        category_ptr->code_data.ns_3gpp_code_list.ns_code_data[code_cnt].nw_code.mnc.mnc_data,
                        category_ptr->code_data.ns_3gpp_code_list.ns_code_data[code_cnt].nw_code.mnc.mnc_len);

        SIMLOCK_MSG_LOW_2("From req - d6: 0x%x, d7: 0x%x",
                          category_ptr->code_data.ns_3gpp_code_list.ns_code_data[code_cnt].imsi_digit6,
                          category_ptr->code_data.ns_3gpp_code_list.ns_code_data[code_cnt].imsi_digit7);
    ...
    case SIMLOCK_CATEGORY_3GPP_SP:
      category_data_ptr->category_header.num_of_codes =
        category_ptr->code_data.sp_3gpp_code_list.num_sp_codes;

      /* in case of autolock num of codes can be 0 */
      if(category_ptr->code_data.sp_3gpp_code_list.num_sp_codes != 0)
      {
        code_data_length = (category_ptr->code_data.sp_3gpp_code_list.num_sp_codes *
                            sizeof(simlock_category_code_data_type));
      ...
      for(code_cnt = 0; code_cnt < category_ptr->code_data.sp_3gpp_code_list.num_sp_codes;
          code_cnt++)
      {
        if(blacklist)
        {
        ...
        simlock_memscpy(category_data_ptr->code_data_ptr[code_cnt].mcc,
                        SIMLOCK_MCC_LEN,
                        category_ptr->code_data.sp_3gpp_code_list.sp_code_data[code_cnt].nw_code.mcc,
                        SIMLOCK_MCC_LEN);

        category_data_ptr->code_data_ptr[code_cnt].mnc.mnc_len =
             category_ptr->code_data.sp_3gpp_code_list.sp_code_data[code_cnt].nw_code.mnc.mnc_len;

        simlock_memscpy(category_data_ptr->code_data_ptr[code_cnt].mnc.mnc_data,
                        SIMLOCK_MNC_MAX,
                        category_ptr->code_data.sp_3gpp_code_list.sp_code_data[code_cnt].nw_code.mnc.mnc_data,
                        category_ptr->code_data.sp_3gpp_code_list.sp_code_data[code_cnt].nw_code.mnc.mnc_len);

        category_data_ptr->code_data_ptr[code_cnt].gid1.gid_len =
             category_ptr->code_data.sp_3gpp_code_list.sp_code_data[code_cnt].gid1.gid_len;

        simlock_memscpy(category_data_ptr->code_data_ptr[code_cnt].gid1.gid_data,
                        sizeof(category_data_ptr->code_data_ptr[code_cnt].gid1.gid_data),
                        category_ptr->code_data.sp_3gpp_code_list.sp_code_data[code_cnt].gid1.gid_data,
                        category_ptr->code_data.sp_3gpp_code_list.sp_code_data[code_cnt].gid1.gid_len);
    ...
    case SIMLOCK_CATEGORY_3GPP_CP:
      category_data_ptr->category_header.num_of_codes =
        category_ptr->code_data.cp_3gpp_code_list.num_cp_codes;

      /* in case of autolock, num of codes can be 0 */
      if(category_ptr->code_data.cp_3gpp_code_list.num_cp_codes != 0)
      {
        code_data_length = (category_ptr->code_data.cp_3gpp_code_list.num_cp_codes *
                            sizeof(simlock_category_code_data_type));
      ...
      for(code_cnt = 0; code_cnt < category_ptr->code_data.cp_3gpp_code_list.num_cp_codes;
          code_cnt++)
      {
        if(blacklist)
        {
        ...
        simlock_memscpy(category_data_ptr->code_data_ptr[code_cnt].mcc,
                        SIMLOCK_MCC_LEN,
                        category_ptr->code_data.cp_3gpp_code_list.cp_code_data[code_cnt].nw_code.mcc,
                        SIMLOCK_MCC_LEN);

        category_data_ptr->code_data_ptr[code_cnt].mnc.mnc_len =
             category_ptr->code_data.cp_3gpp_code_list.cp_code_data[code_cnt].nw_code.mnc.mnc_len;

        simlock_memscpy(category_data_ptr->code_data_ptr[code_cnt].mnc.mnc_data,
                        SIMLOCK_MNC_MAX,
                        category_ptr->code_data.cp_3gpp_code_list.cp_code_data[code_cnt].nw_code.mnc.mnc_data,
                        category_ptr->code_data.cp_3gpp_code_list.cp_code_data[code_cnt].nw_code.mnc.mnc_len);

        category_data_ptr->code_data_ptr[code_cnt].gid1.gid_len =
             category_ptr->code_data.cp_3gpp_code_list.cp_code_data[code_cnt].gid1.gid_len;

        simlock_memscpy(category_data_ptr->code_data_ptr[code_cnt].gid1.gid_data,
                        sizeof(category_data_ptr->code_data_ptr[code_cnt].gid1.gid_data),
                        category_ptr->code_data.cp_3gpp_code_list.cp_code_data[code_cnt].gid1.gid_data,
                        category_ptr->code_data.cp_3gpp_code_list.cp_code_data[code_cnt].gid1.gid_len);

        category_data_ptr->code_data_ptr[code_cnt].gid2.gid_len =
             category_ptr->code_data.cp_3gpp_code_list.cp_code_data[code_cnt].gid2.gid_len;

        simlock_memscpy(category_data_ptr->code_data_ptr[code_cnt].gid2.gid_data,
                        sizeof(category_data_ptr->code_data_ptr[code_cnt].gid2.gid_data),
                        category_ptr->code_data.cp_3gpp_code_list.cp_code_data[code_cnt].gid2.gid_data,
                        category_ptr->code_data.cp_3gpp_code_list.cp_code_data[code_cnt].gid2.gid_len);
    ...
    case SIMLOCK_CATEGORY_3GPP_SIM:
      category_data_ptr->category_header.num_of_codes =
        category_ptr->code_data.sim_3gpp_code_list.num_sim_codes;

      /* in case of autolock, num of codes can be 0 */
      if(category_ptr->code_data.sim_3gpp_code_list.num_sim_codes != 0)
      {
        code_data_length = (category_ptr->code_data.sim_3gpp_code_list.num_sim_codes *
                            sizeof(simlock_category_code_data_type));
      ...
      for(code_cnt = 0; code_cnt < category_ptr->code_data.sim_3gpp_code_list.num_sim_codes;
          code_cnt++)
      {
        if(blacklist)
        {
        ...
        simlock_memscpy(category_data_ptr->code_data_ptr[code_cnt].mcc,
                        SIMLOCK_MCC_LEN,
                        category_ptr->code_data.sim_3gpp_code_list.sim_code_data[code_cnt].nw_code.mcc,
                        SIMLOCK_MCC_LEN);

        category_data_ptr->code_data_ptr[code_cnt].mnc.mnc_len =
             category_ptr->code_data.sim_3gpp_code_list.sim_code_data[code_cnt].nw_code.mnc.mnc_len;

        simlock_memscpy(category_data_ptr->code_data_ptr[code_cnt].mnc.mnc_data,
                        SIMLOCK_MNC_MAX,
                        category_ptr->code_data.sim_3gpp_code_list.sim_code_data[code_cnt].nw_code.mnc.mnc_data,
                        category_ptr->code_data.sim_3gpp_code_list.sim_code_data[code_cnt].nw_code.mnc.mnc_len);

        category_data_ptr->code_data_ptr[code_cnt].msin.msin_len =
             category_ptr->code_data.sim_3gpp_code_list.sim_code_data[code_cnt].imsi_msin.msin_len;

        simlock_memscpy(category_data_ptr->code_data_ptr[code_cnt].msin.msin_data,
                        SIMLOCK_MSIN_MAX,
                        category_ptr->code_data.sim_3gpp_code_list.sim_code_data[code_cnt].imsi_msin.msin_data,
                        category_ptr->code_data.sim_3gpp_code_list.sim_code_data[code_cnt].imsi_msin.msin_len);
        memset(category_data_ptr->code_data_ptr[code_cnt].gid1.gid_data,
               SIMLOCK_LOCK_CODE_WILD_DIGIT,
               sizeof(category_data_ptr->code_data_ptr[code_cnt].gid1.gid_data));
    ...
    case SIMLOCK_CATEGORY_3GPP_SPN:
      category_data_ptr->category_header.num_of_codes =
        category_ptr->code_data.spn_3gpp_code_list.num_spn_codes;

      /* in case of autolock, num of codes can be 0 */
      if(category_ptr->code_data.spn_3gpp_code_list.num_spn_codes != 0)
      {
        code_data_length = (category_ptr->code_data.spn_3gpp_code_list.num_spn_codes *
                            sizeof(simlock_category_code_data_type));
      ...
      for(code_cnt = 0; code_cnt < category_ptr->code_data.spn_3gpp_code_list.num_spn_codes;
          code_cnt++)
      {
        if(blacklist)
        {
        ...
        simlock_memscpy(category_data_ptr->code_data_ptr[code_cnt].mcc,
                        SIMLOCK_MCC_LEN,
                        category_ptr->code_data.spn_3gpp_code_list.spn_code_data[code_cnt].nw_code.mcc,
                        SIMLOCK_MCC_LEN);

        category_data_ptr->code_data_ptr[code_cnt].mnc.mnc_len =
             category_ptr->code_data.spn_3gpp_code_list.spn_code_data[code_cnt].nw_code.mnc.mnc_len;

        simlock_memscpy(category_data_ptr->code_data_ptr[code_cnt].mnc.mnc_data,
                        SIMLOCK_MNC_MAX,
                        category_ptr->code_data.spn_3gpp_code_list.spn_code_data[code_cnt].nw_code.mnc.mnc_data,
                        category_ptr->code_data.spn_3gpp_code_list.spn_code_data[code_cnt].nw_code.mnc.mnc_len);

        simlock_memscpy(&category_data_ptr->code_data_ptr[code_cnt].spn,
                        sizeof(category_data_ptr->code_data_ptr[code_cnt].spn),
                        &category_ptr->code_data.spn_3gpp_code_list.spn_code_data[code_cnt].spn,
                        sizeof(category_ptr->code_data.spn_3gpp_code_list.spn_code_data[code_cnt].spn));
    ...
    case SIMLOCK_CATEGORY_3GPP_SP_EHPLMN:
      category_data_ptr->category_header.num_of_codes =
        category_ptr->code_data.sp_ehplmn_3gpp_code_list.num_sp_ehplmn_codes;

      /* in case of autolock num of codes can be 0 */
      if(category_ptr->code_data.sp_ehplmn_3gpp_code_list.num_sp_ehplmn_codes != 0)
      {
        code_data_length = (category_ptr->code_data.sp_ehplmn_3gpp_code_list.num_sp_ehplmn_codes *
                            sizeof(simlock_category_code_data_type));
      ...
      for(code_cnt = 0; code_cnt < category_ptr->code_data.sp_ehplmn_3gpp_code_list.num_sp_ehplmn_codes;
          code_cnt++)
      {
        if(blacklist)
        {
        ...
        simlock_memscpy(category_data_ptr->code_data_ptr[code_cnt].mcc,
                        SIMLOCK_MCC_LEN,
                        category_ptr->code_data.sp_ehplmn_3gpp_code_list.sp_ehplmn_code_data[code_cnt].nw_code.mcc,
                        SIMLOCK_MCC_LEN);

        category_data_ptr->code_data_ptr[code_cnt].mnc.mnc_len =
             category_ptr->code_data.sp_ehplmn_3gpp_code_list.sp_ehplmn_code_data[code_cnt].nw_code.mnc.mnc_len;

        simlock_memscpy(category_data_ptr->code_data_ptr[code_cnt].mnc.mnc_data,
                        SIMLOCK_MNC_MAX,
                        category_ptr->code_data.sp_ehplmn_3gpp_code_list.sp_ehplmn_code_data[code_cnt].nw_code.mnc.mnc_data,
                        category_ptr->code_data.sp_ehplmn_3gpp_code_list.sp_ehplmn_code_data[code_cnt].nw_code.mnc.mnc_len);

        category_data_ptr->code_data_ptr[code_cnt].gid1.gid_len =
             category_ptr->code_data.sp_ehplmn_3gpp_code_list.sp_ehplmn_code_data[code_cnt].gid1.gid_len;

        simlock_memscpy(category_data_ptr->code_data_ptr[code_cnt].gid1.gid_data,
                        sizeof(category_data_ptr->code_data_ptr[code_cnt].gid1.gid_data),
                        category_ptr->code_data.sp_ehplmn_3gpp_code_list.sp_ehplmn_code_data[code_cnt].gid1.gid_data,
                        category_ptr->code_data.sp_ehplmn_3gpp_code_list.sp_ehplmn_code_data[code_cnt].gid1.gid_len);

        simlock_memscpy(category_data_ptr->code_data_ptr[code_cnt].ehplmn_mcc,
                        SIMLOCK_MCC_LEN,
                        category_ptr->code_data.sp_ehplmn_3gpp_code_list.sp_ehplmn_code_data[code_cnt].ehplmn_nw_code.mcc,
                        SIMLOCK_MCC_LEN);

        category_data_ptr->code_data_ptr[code_cnt].ehplmn_mnc.mnc_len =
             category_ptr->code_data.sp_ehplmn_3gpp_code_list.sp_ehplmn_code_data[code_cnt].ehplmn_nw_code.mnc.mnc_len;

        simlock_memscpy(category_data_ptr->code_data_ptr[code_cnt].ehplmn_mnc.mnc_data,
                        SIMLOCK_MNC_MAX,
                        category_ptr->code_data.sp_ehplmn_3gpp_code_list.sp_ehplmn_code_data[code_cnt].ehplmn_nw_code.mnc.mnc_data,
                        category_ptr->code_data.sp_ehplmn_3gpp_code_list.sp_ehplmn_code_data[code_cnt].ehplmn_nw_code.mnc.mnc_len);
    ...
    case SIMLOCK_CATEGORY_3GPP_ICCID:
      category_data_ptr->category_header.num_of_codes =
        category_ptr->code_data.iccid_3gpp_code_list.num_iccid_codes;

      /* in case of autolock, num of codes can be 0 */
      if(category_ptr->code_data.iccid_3gpp_code_list.num_iccid_codes != 0)
      {
        code_data_length = (category_ptr->code_data.iccid_3gpp_code_list.num_iccid_codes *
                            sizeof(simlock_category_code_data_type));
      ...
      for(code_cnt = 0; code_cnt < category_ptr->code_data.iccid_3gpp_code_list.num_iccid_codes;
          code_cnt++)
      {
        if(blacklist)
        {
        ...
        simlock_memscpy(category_data_ptr->code_data_ptr[code_cnt].mcc,
                        SIMLOCK_MCC_LEN,
                        category_ptr->code_data.iccid_3gpp_code_list.iccid_code_data[code_cnt].nw_code.mcc,
                        SIMLOCK_MCC_LEN);

        category_data_ptr->code_data_ptr[code_cnt].mnc.mnc_len =
             category_ptr->code_data.iccid_3gpp_code_list.iccid_code_data[code_cnt].nw_code.mnc.mnc_len;

        simlock_memscpy(category_data_ptr->code_data_ptr[code_cnt].mnc.mnc_data,
                        SIMLOCK_MNC_MAX,
                        category_ptr->code_data.iccid_3gpp_code_list.iccid_code_data[code_cnt].nw_code.mnc.mnc_data,
                        category_ptr->code_data.iccid_3gpp_code_list.iccid_code_data[code_cnt].nw_code.mnc.mnc_len);

        category_data_ptr->code_data_ptr[code_cnt].iccid.iccid_len =
             category_ptr->code_data.iccid_3gpp_code_list.iccid_code_data[code_cnt].iccid.iccid_len;

        simlock_memscpy(&category_data_ptr->code_data_ptr[code_cnt].iccid.iccid_data,
                        SIMLOCK_ICCID_DIGITS_MAX,
                        category_ptr->code_data.iccid_3gpp_code_list.iccid_code_data[code_cnt].iccid.iccid_data,
                        category_ptr->code_data.iccid_3gpp_code_list.iccid_code_data[code_cnt].iccid.iccid_len);
      ...
      case SIMLOCK_CATEGORY_3GPP_NS_SP:
        category_data_ptr->category_header.num_of_codes =
        category_ptr->code_data.ns_sp_3gpp_code_list.num_ns_sp_codes;

      /* in case of autolock num of codes can be 0 */
      if(category_ptr->code_data.ns_sp_3gpp_code_list.num_ns_sp_codes != 0)
      {
        code_data_length = (category_ptr->code_data.ns_sp_3gpp_code_list.num_ns_sp_codes *
                            sizeof(simlock_category_code_data_type));
      ...
      for(code_cnt = 0; code_cnt < category_ptr->code_data.ns_sp_3gpp_code_list.num_ns_sp_codes;
          code_cnt++)
      {
        SIMLOCK_MSG_LOW_6("From req - mcc[0]: 0x%x, mcc[1]: 0x%x, mcc[2]: 0x%x, mnc[0]: 0x%x, mnc[1]: 0x%x, mnc[2]: 0x%x",
                          category_ptr->code_data.ns_sp_3gpp_code_list.ns_sp_code_data[code_cnt].ns_code.nw_code.mcc[0],
                          category_ptr->code_data.ns_sp_3gpp_code_list.ns_sp_code_data[code_cnt].ns_code.nw_code.mcc[1],
                          category_ptr->code_data.ns_sp_3gpp_code_list.ns_sp_code_data[code_cnt].ns_code.nw_code.mcc[2],
                          category_ptr->code_data.ns_sp_3gpp_code_list.ns_sp_code_data[code_cnt].ns_code.nw_code.mnc.mnc_data[0],
                          category_ptr->code_data.ns_sp_3gpp_code_list.ns_sp_code_data[code_cnt].ns_code.nw_code.mnc.mnc_data[1],
                          category_ptr->code_data.ns_sp_3gpp_code_list.ns_sp_code_data[code_cnt].ns_code.nw_code.mnc.mnc_data[2]);
        ...
        simlock_memscpy(category_data_ptr->code_data_ptr[code_cnt].mcc,
                        SIMLOCK_MCC_LEN,
                        category_ptr->code_data.ns_sp_3gpp_code_list.ns_sp_code_data[code_cnt].ns_code.nw_code.mcc,
                        SIMLOCK_MCC_LEN);

        category_data_ptr->code_data_ptr[code_cnt].mnc.mnc_len =
             category_ptr->code_data.ns_sp_3gpp_code_list.ns_sp_code_data[code_cnt].ns_code.nw_code.mnc.mnc_len;

        simlock_memscpy(category_data_ptr->code_data_ptr[code_cnt].mnc.mnc_data,
                        SIMLOCK_MNC_MAX,
                        category_ptr->code_data.ns_sp_3gpp_code_list.ns_sp_code_data[code_cnt].ns_code.nw_code.mnc.mnc_data,
                        category_ptr->code_data.ns_sp_3gpp_code_list.ns_sp_code_data[code_cnt].ns_code.nw_code.mnc.mnc_len);

        SIMLOCK_MSG_LOW_2("From req - d6: 0x%x, d7: 0x%x",
                          category_ptr->code_data.ns_sp_3gpp_code_list.ns_sp_code_data[code_cnt].ns_code.imsi_digit6,
                          category_ptr->code_data.ns_sp_3gpp_code_list.ns_sp_code_data[code_cnt].ns_code.imsi_digit7);
        ...
        category_data_ptr->code_data_ptr[code_cnt].gid1.gid_len =
             category_ptr->code_data.ns_sp_3gpp_code_list.ns_sp_code_data[code_cnt].gid1.gid_len;

        simlock_memscpy(category_data_ptr->code_data_ptr[code_cnt].gid1.gid_data,
                        sizeof(category_data_ptr->code_data_ptr[code_cnt].gid1.gid_data),
                        category_ptr->code_data.ns_sp_3gpp_code_list.ns_sp_code_data[code_cnt].gid1.gid_data,
                        category_ptr->code_data.ns_sp_3gpp_code_list.ns_sp_code_data[code_cnt].gid1.gid_len);
    ...
    case SIMLOCK_CATEGORY_3GPP2_NW_TYPE1:
      category_data_ptr->category_header.num_of_codes =
        category_ptr->code_data.nw_type1_3gpp2_code_list.num_nw_codes;

      /* in case of autolock, num of codes can be 0 */
      if(category_ptr->code_data.nw_type1_3gpp2_code_list.num_nw_codes != 0)
      {
        code_data_length = (category_ptr->code_data.nw_type1_3gpp2_code_list.num_nw_codes *
                            sizeof(simlock_category_code_data_type));
      ...
      for(code_cnt = 0;
          code_cnt < category_ptr->code_data.nw_type1_3gpp2_code_list.num_nw_codes;
          code_cnt++)
      {
        SIMLOCK_MSG_LOW_6("From req - mcc[0]: 0x%x, mcc[1]: 0x%x, mcc[2]: 0x%x, mnc[0]: 0x%x, mnc[1]: 0x%x, mnc[2]: 0x%x",
                          category_ptr->code_data.nw_type1_3gpp2_code_list.nw_code_data[code_cnt].mcc[0],
                          category_ptr->code_data.nw_type1_3gpp2_code_list.nw_code_data[code_cnt].mcc[1],
                          category_ptr->code_data.nw_type1_3gpp2_code_list.nw_code_data[code_cnt].mcc[2],
                          category_ptr->code_data.nw_type1_3gpp2_code_list.nw_code_data[code_cnt].mnc.mnc_data[0],
                          category_ptr->code_data.nw_type1_3gpp2_code_list.nw_code_data[code_cnt].mnc.mnc_data[1],
                          category_ptr->code_data.nw_type1_3gpp2_code_list.nw_code_data[code_cnt].mnc.mnc_data[2]);
        ...
        simlock_memscpy(category_data_ptr->code_data_ptr[code_cnt].mcc,
                        SIMLOCK_MCC_LEN,
                        category_ptr->code_data.nw_type1_3gpp2_code_list.nw_code_data[code_cnt].mcc,
                        SIMLOCK_MCC_LEN);

        category_data_ptr->code_data_ptr[code_cnt].mnc.mnc_len =
             category_ptr->code_data.nw_type1_3gpp2_code_list.nw_code_data[code_cnt].mnc.mnc_len;

        simlock_memscpy(category_data_ptr->code_data_ptr[code_cnt].mnc.mnc_data,
                        SIMLOCK_MNC_MAX,
                        category_ptr->code_data.nw_type1_3gpp2_code_list.nw_code_data[code_cnt].mnc.mnc_data,
                        category_ptr->code_data.nw_type1_3gpp2_code_list.nw_code_data[code_cnt].mnc.mnc_len);
    ...
    case SIMLOCK_CATEGORY_3GPP2_NW_TYPE2:
      category_data_ptr->category_header.num_of_codes =
        category_ptr->code_data.nw_type2_3gpp2_code_list.num_nw_type2_codes;

      /* in case of autolock, num of codes can be 0 */
      if(category_ptr->code_data.nw_type2_3gpp2_code_list.num_nw_type2_codes != 0)
      {
        code_data_length = (category_ptr->code_data.nw_type2_3gpp2_code_list.num_nw_type2_codes *
                            sizeof(simlock_category_code_data_type));
      ...
      for(code_cnt = 0;
          code_cnt < category_ptr->code_data.nw_type2_3gpp2_code_list.num_nw_type2_codes;
          code_cnt++)
      {
        if(blacklist)
        {
        ...
        SIMLOCK_MSG_LOW_4("From req - irm[0]: 0x%x, irm[1]: 0x%x, irm[2]: 0x%x, irm[3]: 0x%x",
                          category_ptr->code_data.nw_type2_3gpp2_code_list.nw_type2_code_data[code_cnt].irm[0],
                          category_ptr->code_data.nw_type2_3gpp2_code_list.nw_type2_code_data[code_cnt].irm[1],
                          category_ptr->code_data.nw_type2_3gpp2_code_list.nw_type2_code_data[code_cnt].irm[2],
                          category_ptr->code_data.nw_type2_3gpp2_code_list.nw_type2_code_data[code_cnt].irm[3]);

        simlock_memscpy(category_data_ptr->code_data_ptr[code_cnt].msin.msin_data,
                        SIMLOCK_MSIN_MAX,
                        category_ptr->code_data.nw_type2_3gpp2_code_list.nw_type2_code_data[code_cnt].irm,
                        SIMLOCK_IRM_LEN);
    ...
    case SIMLOCK_CATEGORY_3GPP2_RUIM:
      category_data_ptr->category_header.num_of_codes =
        (uint8)category_ptr->code_data.ruim_3gpp2_code_list.num_sim_codes;

      /* in case of autolock num of codes can be 0 */
      if(category_ptr->code_data.ruim_3gpp2_code_list.num_sim_codes != 0)
      {
        code_data_length = (category_ptr->code_data.ruim_3gpp2_code_list.num_sim_codes *
                            sizeof(simlock_category_code_data_type));
      ...
      for(code_cnt = 0;
          code_cnt < category_ptr->code_data.ruim_3gpp2_code_list.num_sim_codes;
          code_cnt++)
      {
        SIMLOCK_MSG_LOW_6("From req - mcc[0]: 0x%x, mcc[1]: 0x%x, mcc[2]: 0x%x, mnc[0]: 0x%x, mnc[1]: 0x%x, mnc[2]: 0x%x",
                          category_ptr->code_data.ruim_3gpp2_code_list.sim_code_data[code_cnt].nw_code.mcc[0],
                          category_ptr->code_data.ruim_3gpp2_code_list.sim_code_data[code_cnt].nw_code.mcc[1],
                          category_ptr->code_data.ruim_3gpp2_code_list.sim_code_data[code_cnt].nw_code.mcc[2],
                          category_ptr->code_data.ruim_3gpp2_code_list.sim_code_data[code_cnt].nw_code.mnc.mnc_data[0],
                          category_ptr->code_data.ruim_3gpp2_code_list.sim_code_data[code_cnt].nw_code.mnc.mnc_data[1],
                          category_ptr->code_data.ruim_3gpp2_code_list.sim_code_data[code_cnt].nw_code.mnc.mnc_data[2]);

        SIMLOCK_MSG_LOW_6("From req - msin[0]: 0x%x, msin[1]: 0x%x, msin[2]: 0x%x, msin[3]: 0x%x, msin[4]: 0x%x, msin[5]: 0x%x",
                          category_ptr->code_data.ruim_3gpp2_code_list.sim_code_data[code_cnt].imsi_msin.msin_data[0],
                          category_ptr->code_data.ruim_3gpp2_code_list.sim_code_data[code_cnt].imsi_msin.msin_data[1],
                          category_ptr->code_data.ruim_3gpp2_code_list.sim_code_data[code_cnt].imsi_msin.msin_data[2],
                          category_ptr->code_data.ruim_3gpp2_code_list.sim_code_data[code_cnt].imsi_msin.msin_data[3],
                          category_ptr->code_data.ruim_3gpp2_code_list.sim_code_data[code_cnt].imsi_msin.msin_data[4],
                          category_ptr->code_data.ruim_3gpp2_code_list.sim_code_data[code_cnt].imsi_msin.msin_data[5]);

        SIMLOCK_MSG_LOW_4("From req - msin[6]: 0x%x, msin[7]: 0x%x, msin[8]: 0x%x, msin[9]: 0x%x",
                          category_ptr->code_data.ruim_3gpp2_code_list.sim_code_data[code_cnt].imsi_msin.msin_data[6],
                          category_ptr->code_data.ruim_3gpp2_code_list.sim_code_data[code_cnt].imsi_msin.msin_data[7],
                          category_ptr->code_data.ruim_3gpp2_code_list.sim_code_data[code_cnt].imsi_msin.msin_data[8],
                          category_ptr->code_data.ruim_3gpp2_code_list.sim_code_data[code_cnt].imsi_msin.msin_data[9]);
        ...
        simlock_memscpy(category_data_ptr->code_data_ptr[code_cnt].mcc,
                        SIMLOCK_MCC_LEN,
                        category_ptr->code_data.ruim_3gpp2_code_list.sim_code_data[code_cnt].nw_code.mcc,
                        SIMLOCK_MCC_LEN);

        category_data_ptr->code_data_ptr[code_cnt].mnc.mnc_len =
             category_ptr->code_data.ruim_3gpp2_code_list.sim_code_data[code_cnt].nw_code.mnc.mnc_len;

        simlock_memscpy(category_data_ptr->code_data_ptr[code_cnt].mnc.mnc_data,
                        SIMLOCK_MNC_MAX,
                        category_ptr->code_data.ruim_3gpp2_code_list.sim_code_data[code_cnt].nw_code.mnc.mnc_data,
                        category_ptr->code_data.ruim_3gpp2_code_list.sim_code_data[code_cnt].nw_code.mnc.mnc_len);

        category_data_ptr->code_data_ptr[code_cnt].msin.msin_len =
             category_ptr->code_data.ruim_3gpp2_code_list.sim_code_data[code_cnt].imsi_msin.msin_len;

        simlock_memscpy(category_data_ptr->code_data_ptr[code_cnt].msin.msin_data,
                        SIMLOCK_MSIN_MAX,
                        category_ptr->code_data.ruim_3gpp2_code_list.sim_code_data[code_cnt].imsi_msin.msin_data,
                        category_ptr->code_data.ruim_3gpp2_code_list.sim_code_data[code_cnt].imsi_msin.msin_len);
...
)
{
  ...
  simlock_status = simlock_category_write_lock_data(slot, category_data_ptr, sfs_type,
...
/*===========================================================================
FUNCTION SIMLOCK_CATEGORY_COPY_EHPLMN_NW_CODE_TO_RESPONSE

DESCRIPTION
  This function copies the EHPLMN network code to the response
...
SIDE EFFECTS
  NONE
===========================================================================*/
static simlock_result_enum_type simlock_category_copy_ehplmn_nw_code_to_response
(
  simlock_nw_code_data_type                  * resp_nw_code_ptr,
  const simlock_category_code_data_type      * code_data_ptr
)
{
  ...
  simlock_memscpy(resp_nw_code_ptr->mcc,
                  SIMLOCK_MCC_LEN,
                  code_data_ptr->ehplmn_mcc,
                  SIMLOCK_MCC_LEN);

  if (code_data_ptr->ehplmn_mnc.mnc_len > SIMLOCK_MNC_MAX)
  {
    SIMLOCK_MSG_ERR_1("MNC data length too long. length: 0x%x",
                      code_data_ptr->ehplmn_mnc.mnc_len);
    return SIMLOCK_GENERIC_ERROR;
  }

  resp_nw_code_ptr->mnc.mnc_len = code_data_ptr->ehplmn_mnc.mnc_len;

  simlock_memscpy(resp_nw_code_ptr->mnc.mnc_data,
                  SIMLOCK_MNC_MAX,
                  code_data_ptr->ehplmn_mnc.mnc_data,
                  code_data_ptr->ehplmn_mnc.mnc_len);

  return SIMLOCK_SUCCESS;
} /* simlock_category_copy_ehplmn_nw_code_to_response */
          ...
          code_cnt++)
      {
        if(category_data_ptr->code_data_ptr[code_cnt].code_type == SIMLOCK_CODE_WHITELIST)
        {
          ...
          if((whitelist_cnt < resp_data_ptr->category_data_whitelist.num_of_lists) &&
             (whitelist_code_count < SIMLOCK_NUM_CODES_MAX))
          {
            resp_data_ptr->category_data_whitelist.whitelist_data_ptr[whitelist_cnt].category_type = SIMLOCK_CATEGORY_3GPP_SP_EHPLMN;

            simlock_status = simlock_category_copy_nw_code_to_response(&resp_data_ptr->category_data_whitelist.whitelist_data_ptr[whitelist_cnt].code_data.sp_ehplmn_3gpp_code_list.sp_ehplmn_code_data[whitelist_code_count].nw_code,
                                                                       &category_data_ptr->code_data_ptr[code_cnt]);
            if(simlock_status != SIMLOCK_SUCCESS)
            {
              break;
            ...
            resp_data_ptr->category_data_whitelist.whitelist_data_ptr[whitelist_cnt].code_data.sp_ehplmn_3gpp_code_list.sp_ehplmn_code_data[whitelist_code_count].gid1.gid_len =
                category_data_ptr->code_data_ptr[code_cnt].gid1.gid_len;

            simlock_memscpy(resp_data_ptr->category_data_whitelist.whitelist_data_ptr[whitelist_cnt].code_data.sp_ehplmn_3gpp_code_list.sp_ehplmn_code_data[whitelist_code_count].gid1.gid_data,
                            resp_data_ptr->category_data_whitelist.whitelist_data_ptr[whitelist_cnt].code_data.sp_ehplmn_3gpp_code_list.sp_ehplmn_code_data[whitelist_code_count].gid1.gid_len,
                            category_data_ptr->code_data_ptr[code_cnt].gid1.gid_data,
                            category_data_ptr->code_data_ptr[code_cnt].gid1.gid_len);

            simlock_status = simlock_category_copy_ehplmn_nw_code_to_response(&resp_data_ptr->category_data_whitelist.whitelist_data_ptr[whitelist_cnt].code_data.sp_ehplmn_3gpp_code_list.sp_ehplmn_code_data[whitelist_code_count].ehplmn_nw_code,
                                                                              &category_data_ptr->code_data_ptr[code_cnt]);
            if(simlock_status != SIMLOCK_SUCCESS)
            {
              break;
            ...
            resp_data_ptr->category_data_whitelist.whitelist_data_ptr[whitelist_cnt].code_data.sp_ehplmn_3gpp_code_list.num_sp_ehplmn_codes++;
            whitelist_code_count++;
          }
        }
        else if(category_data_ptr->code_data_ptr[code_cnt].code_type == SIMLOCK_CODE_BLACKLIST)
        {
          ...
          if((blacklist_cnt < resp_data_ptr->category_data_blacklist.num_of_lists) &&
             (blacklist_code_count < SIMLOCK_NUM_CODES_MAX))
          {
            resp_data_ptr->category_data_blacklist.blacklist_data_ptr[blacklist_cnt].category_type = SIMLOCK_CATEGORY_3GPP_SP_EHPLMN;

            simlock_status = simlock_category_copy_nw_code_to_response(&resp_data_ptr->category_data_blacklist.blacklist_data_ptr[blacklist_cnt].code_data.sp_ehplmn_3gpp_code_list.sp_ehplmn_code_data[blacklist_code_count].nw_code,
                                                                       &category_data_ptr->code_data_ptr[code_cnt]);
            if(simlock_status != SIMLOCK_SUCCESS)
            {
              break;
            ...
            resp_data_ptr->category_data_blacklist.blacklist_data_ptr[blacklist_cnt].code_data.sp_ehplmn_3gpp_code_list.sp_ehplmn_code_data[blacklist_code_count].gid1.gid_len =
                category_data_ptr->code_data_ptr[code_cnt].gid1.gid_len;

            simlock_memscpy(resp_data_ptr->category_data_blacklist.blacklist_data_ptr[blacklist_cnt].code_data.sp_ehplmn_3gpp_code_list.sp_ehplmn_code_data[blacklist_code_count].gid1.gid_data,
                            resp_data_ptr->category_data_blacklist.blacklist_data_ptr[blacklist_cnt].code_data.sp_ehplmn_3gpp_code_list.sp_ehplmn_code_data[blacklist_code_count].gid1.gid_len,
                            category_data_ptr->code_data_ptr[code_cnt].gid1.gid_data,
                            category_data_ptr->code_data_ptr[code_cnt].gid1.gid_len);

            simlock_status = simlock_category_copy_ehplmn_nw_code_to_response(&resp_data_ptr->category_data_blacklist.blacklist_data_ptr[blacklist_cnt].code_data.sp_ehplmn_3gpp_code_list.sp_ehplmn_code_data[blacklist_code_count].ehplmn_nw_code,
                                                                              &category_data_ptr->code_data_ptr[code_cnt]);
            if(simlock_status != SIMLOCK_SUCCESS)
            {
              break;
            ...
            resp_data_ptr->category_data_blacklist.blacklist_data_ptr[blacklist_cnt].code_data.sp_ehplmn_3gpp_code_list.num_sp_ehplmn_codes++;
            blacklist_code_count++;
          }
        }
      }
          ...
          code_cnt++)
      {
        if(category_data_ptr->code_data_ptr[code_cnt].code_type == SIMLOCK_CODE_WHITELIST)
        {
          ...
          if((whitelist_cnt < resp_data_ptr->category_data_whitelist.num_of_lists) &&
             (whitelist_code_count < SIMLOCK_NUM_CODES_MAX))
          {
            resp_data_ptr->category_data_whitelist.whitelist_data_ptr[whitelist_cnt].category_type = SIMLOCK_CATEGORY_3GPP_NS_SP;

            simlock_status = simlock_category_copy_nw_code_to_response(&resp_data_ptr->category_data_whitelist.whitelist_data_ptr[whitelist_cnt].code_data.ns_sp_3gpp_code_list.ns_sp_code_data[whitelist_code_count].ns_code.nw_code,
                                                                       &category_data_ptr->code_data_ptr[code_cnt]);
            if(simlock_status != SIMLOCK_SUCCESS)
            {
              break;
            ...
            if(category_data_ptr->code_data_ptr[code_cnt].msin.msin_len >= 0x02)
            {
              resp_data_ptr->category_data_whitelist.whitelist_data_ptr[whitelist_cnt].code_data.ns_sp_3gpp_code_list.ns_sp_code_data[whitelist_code_count].ns_code.imsi_digit6 =
                   category_data_ptr->code_data_ptr[code_cnt].msin.msin_data[0];

              resp_data_ptr->category_data_whitelist.whitelist_data_ptr[whitelist_cnt].code_data.ns_sp_3gpp_code_list.ns_sp_code_data[whitelist_code_count].ns_code.imsi_digit7 =
                   category_data_ptr->code_data_ptr[code_cnt].msin.msin_data[1];
            }

            resp_data_ptr->category_data_whitelist.whitelist_data_ptr[whitelist_cnt].code_data.ns_sp_3gpp_code_list.ns_sp_code_data[whitelist_code_count].gid1.gid_len =
                category_data_ptr->code_data_ptr[code_cnt].gid1.gid_len;

            simlock_memscpy(resp_data_ptr->category_data_whitelist.whitelist_data_ptr[whitelist_cnt].code_data.ns_sp_3gpp_code_list.ns_sp_code_data[whitelist_code_count].gid1.gid_data,
                            resp_data_ptr->category_data_whitelist.whitelist_data_ptr[whitelist_cnt].code_data.ns_sp_3gpp_code_list.ns_sp_code_data[whitelist_code_count].gid1.gid_len,
                            category_data_ptr->code_data_ptr[code_cnt].gid1.gid_data,
                            category_data_ptr->code_data_ptr[code_cnt].gid1.gid_len);

            resp_data_ptr->category_data_whitelist.whitelist_data_ptr[whitelist_cnt].code_data.ns_sp_3gpp_code_list.num_ns_sp_codes++;
            whitelist_code_count++;
          }
        }
        else if (category_data_ptr->code_data_ptr[code_cnt].code_type == SIMLOCK_CODE_BLACKLIST)
        {
          ...
          if((blacklist_cnt < resp_data_ptr->category_data_blacklist.num_of_lists) &&
             (blacklist_code_count < SIMLOCK_NUM_CODES_MAX))
          {
            resp_data_ptr->category_data_blacklist.blacklist_data_ptr[blacklist_cnt].category_type = SIMLOCK_CATEGORY_3GPP_NS_SP;

            simlock_status = simlock_category_copy_nw_code_to_response(&resp_data_ptr->category_data_blacklist.blacklist_data_ptr[blacklist_cnt].code_data.ns_sp_3gpp_code_list.ns_sp_code_data[blacklist_code_count].ns_code.nw_code,
                                                                       &category_data_ptr->code_data_ptr[code_cnt]);
            if(simlock_status != SIMLOCK_SUCCESS)
            {
              break;
            ...
            if(category_data_ptr->code_data_ptr[code_cnt].msin.msin_len >= 0x02)
            {
              resp_data_ptr->category_data_blacklist.blacklist_data_ptr[blacklist_cnt].code_data.ns_sp_3gpp_code_list.ns_sp_code_data[blacklist_code_count].ns_code.imsi_digit6 =
                   category_data_ptr->code_data_ptr[code_cnt].msin.msin_data[0];

              resp_data_ptr->category_data_blacklist.blacklist_data_ptr[blacklist_cnt].code_data.ns_sp_3gpp_code_list.ns_sp_code_data[blacklist_code_count].ns_code.imsi_digit7 =
                   category_data_ptr->code_data_ptr[code_cnt].msin.msin_data[1];
            }

            resp_data_ptr->category_data_blacklist.blacklist_data_ptr[blacklist_cnt].code_data.ns_sp_3gpp_code_list.ns_sp_code_data[blacklist_code_count].gid1.gid_len =
                category_data_ptr->code_data_ptr[code_cnt].gid1.gid_len;

            simlock_memscpy(resp_data_ptr->category_data_blacklist.blacklist_data_ptr[blacklist_cnt].code_data.ns_sp_3gpp_code_list.ns_sp_code_data[blacklist_code_count].gid1.gid_data,
                            resp_data_ptr->category_data_blacklist.blacklist_data_ptr[blacklist_cnt].code_data.ns_sp_3gpp_code_list.ns_sp_code_data[blacklist_code_count].gid1.gid_len,
                            category_data_ptr->code_data_ptr[code_cnt].gid1.gid_data,
                            category_data_ptr->code_data_ptr[code_cnt].gid1.gid_len);

            resp_data_ptr->category_data_blacklist.blacklist_data_ptr[blacklist_cnt].code_data.ns_sp_3gpp_code_list.num_ns_sp_codes++;
            blacklist_code_count++;
          }
        }
      }
...
/*===========================================================================
FUNCTION SIMLOCK_CATEGORY_COPY_EHPLMN_NW_CODE_FROM_REQUEST

DESCRIPTION
  This function copies the EHPLMN network code from the request data
...
SIDE EFFECTS
  NONE
===========================================================================*/
static simlock_result_enum_type simlock_category_copy_ehplmn_nw_code_from_request
(
  simlock_category_code_data_type      * code_data_ptr,
  const simlock_nw_code_data_type      * req_nw_code_ptr
)
{
  ...
  simlock_memscpy(code_data_ptr->ehplmn_mcc,
                  SIMLOCK_MCC_LEN,
                  req_nw_code_ptr->mcc,
                  SIMLOCK_MCC_LEN);
  ...
  code_data_ptr->ehplmn_mnc.mnc_len = req_nw_code_ptr->mnc.mnc_len;

  simlock_memscpy(code_data_ptr->ehplmn_mnc.mnc_data,
                  SIMLOCK_MNC_MAX,
                  req_nw_code_ptr->mnc.mnc_data,
                  req_nw_code_ptr->mnc.mnc_len);

  return SIMLOCK_SUCCESS;
} /* simlock_category_copy_ehplmn_nw_code_from_request */
  ...
  switch(new_code_data_ptr->category_type)
  {
    case SIMLOCK_CATEGORY_3GPP_NW:
          ...
          code_cnt++, appended_code_index++)
      {
        ...
        memset(appended_code_data_ptr[appended_code_index].ehplmn_mcc, 0xFF, SIMLOCK_MCC_LEN);
        appended_code_data_ptr[appended_code_index].ehplmn_mnc.mnc_len = 0;
        memset(appended_code_data_ptr[appended_code_index].ehplmn_mnc.mnc_data, 0xFF, SIMLOCK_MNC_MAX);
        ...
        appended_code_data_ptr[appended_code_index].impi.impi_len = 0;
        memset(appended_code_data_ptr[appended_code_index].impi.impi_data, 0xFF, SIMLOCK_IMPI_MAX);
      }
      break;
    ...
    case SIMLOCK_CATEGORY_3GPP_SP_EHPLMN:
      if(new_code_data_ptr->code_data.sp_ehplmn_3gpp_code_list.num_sp_ehplmn_codes == 0)
      {
        return SIMLOCK_GENERIC_ERROR;
      }
      total_codes_cnt = (category_data_ptr->category_header.num_of_codes +
                         new_code_data_ptr->code_data.sp_ehplmn_3gpp_code_list.num_sp_ehplmn_codes);
      appended_code_data_length = (total_codes_cnt *
                                   sizeof(simlock_category_code_data_type));
      ...
      /* add the new codes */
      for(code_cnt = 0, appended_code_index = category_data_ptr->category_header.num_of_codes;
          code_cnt < new_code_data_ptr->code_data.sp_ehplmn_3gpp_code_list.num_sp_ehplmn_codes &&
            appended_code_index < total_codes_cnt;
          code_cnt++, appended_code_index++)
      {
        if(blacklist)
        ...
        simlock_status = simlock_category_copy_nw_code_from_request(&appended_code_data_ptr[appended_code_index],
                                                                    &new_code_data_ptr->code_data.sp_ehplmn_3gpp_code_list.sp_ehplmn_code_data[code_cnt].nw_code);
        if(simlock_status != SIMLOCK_SUCCESS)
        {
          break;
        }
        ...
        simlock_status = simlock_category_copy_gid_code_from_request(&appended_code_data_ptr[appended_code_index].gid1,
                                                    &new_code_data_ptr->code_data.sp_ehplmn_3gpp_code_list.sp_ehplmn_code_data[code_cnt].gid1);
        ...
        simlock_status = simlock_category_copy_ehplmn_nw_code_from_request(&appended_code_data_ptr[appended_code_index],
                                                                           &new_code_data_ptr->code_data.sp_ehplmn_3gpp_code_list.sp_ehplmn_code_data[code_cnt].ehplmn_nw_code);
        if(simlock_status != SIMLOCK_SUCCESS)
        {
          break;
        }
    ...
    case SIMLOCK_CATEGORY_3GPP_ICCID:
      ...
      case SIMLOCK_CATEGORY_3GPP_NS_SP:
      if(new_code_data_ptr->code_data.ns_sp_3gpp_code_list.num_ns_sp_codes == 0)
      {
        return SIMLOCK_GENERIC_ERROR;
      }
      total_codes_cnt = (category_data_ptr->category_header.num_of_codes +
                         new_code_data_ptr->code_data.ns_sp_3gpp_code_list.num_ns_sp_codes);
      appended_code_data_length = (total_codes_cnt *
                                   sizeof(simlock_category_code_data_type));
      ...
      /* add the new codes */
      for(code_cnt = 0, appended_code_index = category_data_ptr->category_header.num_of_codes;
          code_cnt < new_code_data_ptr->code_data.ns_sp_3gpp_code_list.num_ns_sp_codes &&
            appended_code_index < total_codes_cnt;
          code_cnt++, appended_code_index++)
      {
        if(blacklist)
        ...
        simlock_status = simlock_category_copy_nw_code_from_request(&appended_code_data_ptr[appended_code_index],
                                                                    &new_code_data_ptr->code_data.ns_sp_3gpp_code_list.ns_sp_code_data[code_cnt].ns_code.nw_code);
        if(simlock_status != SIMLOCK_SUCCESS)
        {
          break;
        }
        ...
        /* copy d6 and d7 to msin_data[0] and msin_data[1] respectively */
        appended_code_data_ptr[appended_code_index].msin.msin_data[0] =
          new_code_data_ptr->code_data.ns_sp_3gpp_code_list.ns_sp_code_data[code_cnt].ns_code.imsi_digit6;

        appended_code_data_ptr[appended_code_index].msin.msin_data[1] =
          new_code_data_ptr->code_data.ns_sp_3gpp_code_list.ns_sp_code_data[code_cnt].ns_code.imsi_digit7;

        appended_code_data_ptr[appended_code_index].msin.msin_len = 0x02;

        simlock_status = simlock_category_copy_gid_code_from_request(&appended_code_data_ptr[appended_code_index].gid1,
                                                                     &new_code_data_ptr->code_data.ns_sp_3gpp_code_list.ns_sp_code_data[code_cnt].gid1);
