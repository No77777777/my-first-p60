...
05/26/15   stv      Using time uncertanity to validate time received
...
#if defined(FEATURE_SIMLOCK) && defined(FEATURE_SIMLOCK_RSU) && defined(FEATURE_SIMLOCK_RSU_TIME) && defined(FEATURE_SIMLOCK_QMI_LOC)
...
)
{
  ...
  {
    SIMLOCK_MEM_FREE(position_req_ptr);
    return SIMLOCK_MEMORY_ERROR_HEAP_EXHAUSTED;
  }
  ...
  if(QMI_NO_ERR != req_status)
  {
    ...
    SIMLOCK_MEM_FREE(position_resp_ptr);
  ...
  do
  {
    ...
    if(ind_msg_ptr->timeSrc_valid == FALSE ||
       (ind_msg_ptr->timeSrc != eQMI_LOC_TIME_SRC_TOW_DECODE_V02 &&
        ...
        ind_msg_ptr->timeSrc != eQMI_LOC_TIME_SRC_GAL_TOW_DECODE_V02))
    {
      ind_success_status = FALSE;
      break;
    }
...
#endif /* FEATURE_SIMLOCK && FEATURE_SIMLOCK_RSU && FEATURE_SIMLOCK_RSU_TIME && FEATURE_SIMLOCK_QMI_LOC */
