/*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*


            S I M   L O C K   C O N F I G   F U N C T I O N S


GENERAL DESCRIPTION

  This source file contains the simlock configuration functions.


EXTERNALIZED FUNCTIONS


INITIALIZATION AND SEQUENCING REQUIREMENTS


                        COPYRIGHT INFORMATION

Copyright (c) 2014 - 2017, 2019 QUALCOMM Technologies, Inc (QTI) and its licensors.
All Rights Reserved.  QUALCOMM Technologies Proprietary.
Export of this technology or software
is regulated by the U.S. Government. Diversion contrary to U.S. law prohibited.
*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*/

/* <EJECT> */
/*===========================================================================
                        EDIT HISTORY FOR MODULE

This section contains comments describing changes made to the module.
Notice that changes are listed in reverse chronological order.

$Header: //components/rel/uim.mpss/6.1.0/simlock/src/simlock_config.c#3 $$ $DateTime: 2019/12/27 11:28:17 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
12/27/19   cj      Make device dual SIM to single SIM via RSU BLOB
08/26/19   vdc     SimLock solution for activation at Point Of Sale
08/17/19   vdc     Create new SIMLOCK folder if secure boot is enabled
07/24/17   nr      Reduce stack consumption
07/13/17   nr      Remote SIM Unlock supported in common build
07/13/17   av      Enhanced simlock RSU solution
12/16/16   av      Dont error out if the config dir is present but file absent
11/03/16   bcho    Added support for ICCID simlock
10/28/16   nr      Support remote and local configurations together
10/06/16   ar      Added support for SP + EHPLMN simlock
08/10/16   av      Reduce F3 logging in simlock
08/03/16   vdc     Added support for SPN based SIMLOCK feature
07/16/16   av      Postpone creation of simlock config files
08/19/15   vv      Added support to retrieve retry attempts left for locking
03/19/15   at      Introducing new relock feature in SIMLock
12/17/14   vv      Relocate the simlock data files
11/14/14   hh      Fix compilation warnings
09/29/14   vv      Added support for the remote SFS
09/29/14   vv      Added support for emergency only mode
09/17/14   tl      Add support for perso and deperso secure
06/20/14   vv      Added support for diag protection
04/15/14   vv      Added support for quad SIM
04/14/14   vv      Added support for identifying conflicting codes
02/14/14   vv      Initial revision
===========================================================================*/

/*===========================================================================

                     INCLUDE FILES FOR MODULE

===========================================================================*/
#include "uim_variation.h"
#include "customer.h"
#include "IxErrno.h"
#include "secboot_util.h"

#ifdef FEATURE_SIMLOCK

#include "simlock.h"
#include "simlock_config.h"
#include "simlock_file.h"
#include "simlock_util.h"
#include "simlock_efs.h"
#include "simlock_fuse.h"
#include "simlock_rsu_key.h"
#include "simlock_modem.h"
#include "simlock_category.h"

/*===========================================================================

                        DATA DECLARATIONS

===========================================================================*/
#ifdef FEATURE_UIM_TEST_FRAMEWORK
#error code not present
#else
static char *safe_efs_dir           =    "safe";
static char *sfs_efs_dir            =    "safe/sfs";
static char *uim_efs_dir            =    "safe/sfs/uim";
#endif /* FEATURE_UIM_TEST_FRAMEWORK */

static char *simlock_sfs_dir           =    "/safe/sfs/uim/simlock";
static char *simlock_sfs_dir_F           =    "/safe/sfs/uim/simlockF";

/* Version 0 - Size: 46 bytes */
#define SIMLOCK_CONFIG_FILE_SIZE_V0      (2 + ((SIMLOCK_CATEGORY_COUNT_MAX - 5) * SIMLOCK_SLOT_NUM))

/* Version 1 - Stores device operation mode and sfs type in config file along with version 0 - Size: 48 bytes */
#define SIMLOCK_CONFIG_FILE_SIZE_V1      (SIMLOCK_CONFIG_FILE_SIZE_V0 + 2)

/* Version 2 - Number of categories increased to 12 - Size: 52 bytes */
#define SIMLOCK_CONFIG_FILE_SIZE_V2      (4 + ((SIMLOCK_CATEGORY_COUNT_MAX - 4) * SIMLOCK_SLOT_NUM))

/* Version 3 - Number of categories increased to 13 - Size: 56 bytes */
#define SIMLOCK_CONFIG_FILE_SIZE_V3      (4 + ((SIMLOCK_CATEGORY_COUNT_MAX - 3) * SIMLOCK_SLOT_NUM))

/* Version 4 - Number of categories increased to 14 - Size: 60 bytes */
#define SIMLOCK_CONFIG_FILE_SIZE_V4      (4 + ((SIMLOCK_CATEGORY_COUNT_MAX - 2) * SIMLOCK_SLOT_NUM))

/* Version 5 - Added:
   simlock mode config (RSU vs non-RSU): 1 byte
   IMEI info for all the slots - Size: 124 bytes
   So, version 5 is 125 bytes long */
#define SIMLOCK_CONFIG_FILE_SIZE_V5      (SIMLOCK_CONFIG_FILE_SIZE_V4 + 1 + ((1 + SIMLOCK_IMEI_MAX) * SIMLOCK_SLOT_NUM))

/* Version 6 - Number of categories increased to 15 - Size: 129 bytes 
   Slot and sub policy - 2 bytes
   device operation mode and sfs type - 2 bytes
   simlock mode config (RSU vs non-RSU): 1 byte
   IMEI info for all the slots - Size: 64 bytes
   15 categories - 15 * 4 = 60 bytes
*/
#define SIMLOCK_CONFIG_FILE_SIZE_V6      ((4 + ((SIMLOCK_CATEGORY_COUNT_MAX - 1) * SIMLOCK_SLOT_NUM)) + 1 + ((1 + SIMLOCK_IMEI_MAX) * SIMLOCK_SLOT_NUM))

/* Version 7 - Number of categories increased to 16 - Size: 133 bytes 
   Slot and sub policy - 2 bytes
   device operation mode and sfs type - 2 bytes
   simlock mode config (RSU vs non-RSU): 1 byte
   IMEI info for all the slots - Size: 64 bytes
   16 categories - 16 * 4 = 64 bytes
*/
#define SIMLOCK_CONFIG_FILE_SIZE_V7      ((4 + (SIMLOCK_CATEGORY_COUNT_MAX * SIMLOCK_SLOT_NUM)) + 1 + ((1 + SIMLOCK_IMEI_MAX) * SIMLOCK_SLOT_NUM))

/* Version 8 - Added:
   slot enable/disable configuration - 1 byte
*/
#define SIMLOCK_CONFIG_FILE_SIZE_V8      (SIMLOCK_CONFIG_FILE_SIZE_V7 + 1)

#define SIMLOCK_CONFIG_FILE_SIZE_V9      (SIMLOCK_CONFIG_FILE_SIZE_V8 + 33)

#define SIMLOCK_CONFIG_FILE_SIZE         SIMLOCK_CONFIG_FILE_SIZE_V9

#define SIMLOCK_CATEGORY_STATUS_MASK                     1
#define SIMLOCK_CATEGORY_CK_PRESENT_MASK                 2
#define SIMLOCK_CATEGORY_DISABLED_FILE_PRESENT_MASK      4


/*=============================================================================

                       FUNCTION PROTOTYPES

=============================================================================*/

/*===========================================================================
FUNCTION SIMLOCK_CONFIG_CONVERT_CATEGORY_STATUS_TO_BITMASK

DESCRIPTION
  This function converts category status to a bitmask format to be
  stored in the configuration file in EFS.

DEPENDENCIES
  NONE

LIMITATIONS
  NONE

RETURN VALUE
  simlock_result_enum_type

SIDE EFFECTS
  NONE
===========================================================================*/
static simlock_result_enum_type simlock_config_convert_category_status_to_bitmask
(
  simlock_category_status_type     category_status,
  uint8                          * bitmask_ptr
)
{
  if(bitmask_ptr == NULL)
  {
    return SIMLOCK_INCORRECT_PARAMS;
  }

  *bitmask_ptr = 0;

  if(category_status.status == SIMLOCK_ENABLED)
  {
    *bitmask_ptr |= SIMLOCK_CATEGORY_STATUS_MASK;
  }
  else if(category_status.status == SIMLOCK_DISABLED_FILE_PRESENT)
  {
    *bitmask_ptr |= SIMLOCK_CATEGORY_DISABLED_FILE_PRESENT_MASK;
  }

  /* Need to update ck_present mask seperately because of relock functionality
     where a valid CK could be present even if status is disabled  */
  if(category_status.ck_present == SIMLOCK_CK_PRESENT)
  {
    *bitmask_ptr |= SIMLOCK_CATEGORY_CK_PRESENT_MASK;
  }

  return SIMLOCK_SUCCESS;
} /* simlock_config_convert_category_status_to_bitmask */


/*===========================================================================
FUNCTION SIMLOCK_CONFIG_CONVERT_BITMASK_TO_CATEGORY_STATUS

DESCRIPTION
  This function converts the category status stored in the configuration file
  from EFS to category status.

DEPENDENCIES
  NONE

LIMITATIONS
  NONE

RETURN VALUE
  simlock_result_enum_type

SIDE EFFECTS
  NONE
===========================================================================*/
static simlock_result_enum_type simlock_config_convert_bitmask_to_category_status
(
  uint8                            bitmask,
  simlock_category_status_type   * category_status_ptr
)
{
  if(category_status_ptr == NULL)
  {
    return SIMLOCK_INCORRECT_PARAMS;
  }

  category_status_ptr->ck_present = SIMLOCK_CK_NOT_PRESENT;

  if(bitmask & SIMLOCK_CATEGORY_STATUS_MASK)
  {
    category_status_ptr->status = SIMLOCK_ENABLED;
  }
  else if(bitmask & SIMLOCK_CATEGORY_DISABLED_FILE_PRESENT_MASK)
  {
    category_status_ptr->status = SIMLOCK_DISABLED_FILE_PRESENT;
  }
  else
  {
    category_status_ptr->status = SIMLOCK_DISABLED_FILE_NOT_PRESENT;
  }

  /* Need to update ck_present mask seperately because of relock functionality
     where a valid CK could be present even if status is disabled  */
  if(bitmask & SIMLOCK_CATEGORY_CK_PRESENT_MASK)
  {
    category_status_ptr->ck_present = SIMLOCK_CK_PRESENT;
  }

  return SIMLOCK_SUCCESS;
} /* simlock_config_convert_bitmask_to_category_status */


/*===========================================================================
FUNCTION SIMLOCK_CONFIG_UPDATE_IMEI

DESCRIPTION
  This function updates the slot IMEI in the config file

DEPENDENCIES
  NONE

LIMITATIONS
  NONE

RETURN VALUE
  simlock_result_enum_type

SIDE EFFECTS
  NONE
===========================================================================*/
static simlock_result_enum_type simlock_config_update_imei
(
  simlock_config_data_type  * config_data_ptr
)
{
  simlock_imei_type           imei;
  uint8                       slot_index  = SIMLOCK_SLOT_COUNT_MAX;
  simlock_slot_enum_type      slot_id     = SIMLOCK_SLOT_1;

  SIMLOCK_MSG_MED_0("simlock_config_update_imei");

  if(config_data_ptr == NULL)
  {
    return SIMLOCK_INCORRECT_PARAMS;
  }

  for(slot_id = SIMLOCK_SLOT_1; slot_id < SIMLOCK_SLOT_NUM; slot_id++)
  {
    slot_index  = SIMLOCK_SLOT_COUNT_MAX;
    if(SIMLOCK_SUCCESS != simlock_util_get_slot_index(slot_id, &slot_index)||
       slot_index >= SIMLOCK_SLOT_COUNT_MAX)
    {
      return SIMLOCK_GENERIC_ERROR;
    }

    memset(&imei, 0x00, sizeof(imei));
    if(simlock_util_get_imei(slot_id, &imei) != SIMLOCK_SUCCESS ||
       imei.imei_len == 0)
    {
      /* Let's not return an error here and continue reading IMEI for other
         slots. Anyways, when an IMEI is not configured in the config file,
         a check against that IMEI index will fail the simlock operation (that
         relies on IMEI validation). So, it is safe to not return an error
         here */
      continue;
    }

    config_data_ptr->imei[slot_index].imei_len = imei.imei_len;
    simlock_memscpy(config_data_ptr->imei[slot_index].imei_data,
                    sizeof(config_data_ptr->imei[slot_index].imei_data),
                    imei.imei_data,
                    imei.imei_len);
  }

  return SIMLOCK_SUCCESS;
} /* simlock_config_update_imei */


/*===========================================================================
FUNCTION SIMLOCK_CONFIG_SET_POLICIES_AND_IMEI_AND_DEFAULT_STATUS

DESCRIPTION
  This function sets the configuration policies and the default status for
  all the categories on supported slots.
  The function also reads the IMEIs of all the slots into the config data.

DEPENDENCIES
  NONE

LIMITATIONS
  NONE

RETURN VALUE
  simlock_result_enum_type

SIDE EFFECTS
  NONE
===========================================================================*/
static simlock_result_enum_type simlock_config_set_policies_and_imei_and_default_status
(
  simlock_slot_policy_enum_type            slot_policy,
  simlock_subscription_policy_enum_type    sub_policy,
  simlock_config_data_type              *  config_data_ptr
)
{
  simlock_result_enum_type    simlock_status  = SIMLOCK_SUCCESS;
  simlock_slot_enum_type      slot            = SIMLOCK_SLOT_1;
  simlock_category_enum_type  category        = SIMLOCK_CATEGORY_3GPP_NW;
  uint8                       slot_index      = 0;
  uint8                       category_index  = 0;

  if(config_data_ptr == NULL)
  {
    return SIMLOCK_INCORRECT_PARAMS;
  }

  /* This function gets called when the config file is created for the very
     first time. That is a good time to also read the IMEIs from the device
     and store them in the config file that is getting created */
  if(SIMLOCK_SUCCESS != simlock_config_update_imei(config_data_ptr))
  {
    SIMLOCK_MSG_ERR_0("Failed to read IMEI from the device");
    return SIMLOCK_GENERIC_ERROR;
  }

  config_data_ptr->slot_policy = slot_policy;
  config_data_ptr->sub_policy  = sub_policy;

  for(slot = SIMLOCK_SLOT_1; slot < SIMLOCK_SLOT_NUM;
      slot++)
  {
    simlock_status = simlock_util_get_slot_index(slot, &slot_index);
    if((simlock_status != SIMLOCK_SUCCESS) ||
       (slot_index >= SIMLOCK_SLOT_COUNT_MAX))
    {
      return SIMLOCK_GENERIC_ERROR;
    }
    for(category = SIMLOCK_CATEGORY_3GPP_NW; category < SIMLOCK_CATEGORY_COUNT_MAX;
        category++)
    {
      simlock_status = simlock_util_get_category_index(category, &category_index);
      if((simlock_status != SIMLOCK_SUCCESS) ||
         (category_index >= SIMLOCK_CATEGORY_COUNT_MAX))
      {
        return SIMLOCK_GENERIC_ERROR;
      }
      config_data_ptr->category_status[slot_index][category_index].status = SIMLOCK_DISABLED_FILE_NOT_PRESENT;
    }
  }

  return SIMLOCK_SUCCESS;
} /* simlock_config_set_policies_and_imei_and_default_status */


/*===========================================================================
FUNCTION SIMLOCK_CONFIG_BUILD_DATA

DESCRIPTION
  This function builds the config data from the raw config data buffer

DEPENDENCIES
  NONE

LIMITATIONS
  NONE

RETURN VALUE
  simlock_result_enum_type

SIDE EFFECTS
  NONE
===========================================================================*/
static simlock_result_enum_type simlock_config_build_data
(
  uint16                       data_len,
  uint8                     *  data_ptr,
  simlock_config_data_type  *  config_data_ptr,
  simlock_sfs_enum_type        sfs_type
)
{
  simlock_result_enum_type    simlock_status  = SIMLOCK_SUCCESS;
  simlock_slot_enum_type      slot            = SIMLOCK_SLOT_1;
  simlock_category_enum_type  category        = SIMLOCK_CATEGORY_3GPP_NW;
  uint8                       slot_index      = 0;
  uint8                       category_index  = 0;
  uint8                       data_index      = 0;

  if((data_ptr == NULL) ||
     (data_len != SIMLOCK_CONFIG_FILE_SIZE_V0 &&
      data_len != SIMLOCK_CONFIG_FILE_SIZE_V1 &&
      data_len != SIMLOCK_CONFIG_FILE_SIZE_V2 &&
      data_len != SIMLOCK_CONFIG_FILE_SIZE_V3 &&
      data_len != SIMLOCK_CONFIG_FILE_SIZE_V4 &&
      data_len != SIMLOCK_CONFIG_FILE_SIZE_V5 &&
      data_len != SIMLOCK_CONFIG_FILE_SIZE_V6 &&
      data_len != SIMLOCK_CONFIG_FILE_SIZE_V7 &&
      data_len != SIMLOCK_CONFIG_FILE_SIZE_V8 &&
      data_len != SIMLOCK_CONFIG_FILE_SIZE_V9) ||
     (config_data_ptr == NULL))
  {
    SIMLOCK_MSG_ERR_0("input parameters error");
    return SIMLOCK_INCORRECT_PARAMS;
  }

  if(data_len == SIMLOCK_CONFIG_FILE_SIZE_V1 ||
     data_len == SIMLOCK_CONFIG_FILE_SIZE_V2 ||
     data_len == SIMLOCK_CONFIG_FILE_SIZE_V3 ||
     data_len == SIMLOCK_CONFIG_FILE_SIZE_V4 ||
     data_len == SIMLOCK_CONFIG_FILE_SIZE_V5 ||
     data_len == SIMLOCK_CONFIG_FILE_SIZE_V6 ||
     data_len == SIMLOCK_CONFIG_FILE_SIZE_V7 ||
     data_len == SIMLOCK_CONFIG_FILE_SIZE_V8 ||
     data_len == SIMLOCK_CONFIG_FILE_SIZE_V9)
  {
    config_data_ptr->sfs_type    = data_ptr[data_index++];
    config_data_ptr->device_mode = data_ptr[data_index++];
  }

  if((data_index + 2) <= data_len)
  {
    config_data_ptr->slot_policy = data_ptr[data_index++];
    config_data_ptr->sub_policy = data_ptr[data_index++];
  }

  if(data_len == SIMLOCK_CONFIG_FILE_SIZE_V5 ||
     data_len == SIMLOCK_CONFIG_FILE_SIZE_V6 ||
     data_len == SIMLOCK_CONFIG_FILE_SIZE_V7 ||
     data_len == SIMLOCK_CONFIG_FILE_SIZE_V8 ||
     data_len == SIMLOCK_CONFIG_FILE_SIZE_V9)
  {
    if(data_index + 1 <= data_len)
    {
      config_data_ptr->rsu_mode = data_ptr[data_index++];
    }
    else
    {
      return SIMLOCK_GENERIC_ERROR;
    }
  }

  if(data_len == SIMLOCK_CONFIG_FILE_SIZE_V8 ||
     data_len == SIMLOCK_CONFIG_FILE_SIZE_V9)
  {
    if(data_index + 1 > data_len)
    {
      return SIMLOCK_GENERIC_ERROR;
    }

    for (slot = SIMLOCK_SLOT_1; slot < SIMLOCK_SLOT_NUM; slot++)
    {
      simlock_status = simlock_util_get_slot_index(slot, &slot_index);
      if((simlock_status != SIMLOCK_SUCCESS) ||
         (slot_index >= SIMLOCK_SLOT_COUNT_MAX))
      {
        return SIMLOCK_GENERIC_ERROR;
      }

      if(!(data_ptr[data_index] & (0x01 << slot_index)))
      {
        config_data_ptr->disabled_slots.slot[slot_index] = TRUE;
      }
    }
    data_index++;
  }

  if(data_len == SIMLOCK_CONFIG_FILE_SIZE_V9)
  {
    config_data_ptr->nickname_len = data_ptr[data_index++];

    simlock_memscpy(config_data_ptr->nickname,
                    sizeof(config_data_ptr->nickname),
                    &data_ptr[data_index],
                    SIMLOCK_CONFIG_NICKNAME_LEN_MAX);

    data_index += SIMLOCK_CONFIG_NICKNAME_LEN_MAX;
  }

  for(slot = SIMLOCK_SLOT_1; slot < SIMLOCK_SLOT_NUM; slot++)
  {
    simlock_status = simlock_util_get_slot_index(slot, &slot_index);
    if((simlock_status != SIMLOCK_SUCCESS) ||
       (slot_index >= SIMLOCK_SLOT_COUNT_MAX))
    {
      return SIMLOCK_GENERIC_ERROR;
    }

    if(data_len == SIMLOCK_CONFIG_FILE_SIZE_V5 ||
       data_len == SIMLOCK_CONFIG_FILE_SIZE_V6 ||
       data_len == SIMLOCK_CONFIG_FILE_SIZE_V7 ||
       data_len == SIMLOCK_CONFIG_FILE_SIZE_V8 ||
       data_len == SIMLOCK_CONFIG_FILE_SIZE_V9)
    {
      /* IMEI support in the config file is introduced in V5. So, if the
         blob is a V5 blob, copy over the slot IMEI */
      if(data_index <= (data_len - 1 - SIMLOCK_IMEI_MAX))
      {
        config_data_ptr->imei[slot_index].imei_len = data_ptr[data_index++];
        if(config_data_ptr->imei[slot_index].imei_len > SIMLOCK_IMEI_MAX)
        {
          config_data_ptr->imei[slot_index].imei_len = 0;
          return SIMLOCK_GENERIC_ERROR;
        }

        if(config_data_ptr->imei[slot_index].imei_len)
        {
          simlock_memscpy(config_data_ptr->imei[slot_index].imei_data,
                          sizeof(config_data_ptr->imei[slot_index].imei_data),
                          &data_ptr[data_index],
                          SIMLOCK_IMEI_MAX);
        }

        /* Even though the actual IMEI in the RSU blob might be less than 15
           bytes, we copy 15 bytes and advance the data_index by 15 bytes
           because 15 bytes are dedicated to IMEI in the RSU blob */
        data_index += SIMLOCK_IMEI_MAX;
      }
      else
      {
        return SIMLOCK_GENERIC_ERROR;
      }
    }

    for(category = SIMLOCK_CATEGORY_3GPP_NW; category < SIMLOCK_CATEGORY_COUNT_MAX;
        category++)
    {
      simlock_status = simlock_util_get_category_index(category, &category_index);
      if((simlock_status != SIMLOCK_SUCCESS) ||
         (category_index >= SIMLOCK_CATEGORY_COUNT_MAX))
      {
        return SIMLOCK_GENERIC_ERROR;
      }

      /* V0 and V1 versions of the configuration don't have support for SPN category.
         V0, V1 and V2 don't support for SP_EHPLMN category.
         V0, V1, V2 and V3 don't have support for ICCID category.
         V0, V1, V2, V3, V4 and V5 don't have support for IMPI category.
         V0, V1, V2, V3, V4 V5 and V6 don't have support for NS_SP category. */
      if (((data_len == SIMLOCK_CONFIG_FILE_SIZE_V0 ||
            data_len == SIMLOCK_CONFIG_FILE_SIZE_V1) &&
          (category == SIMLOCK_CATEGORY_3GPP_SPN))
                             ||
          ((data_len == SIMLOCK_CONFIG_FILE_SIZE_V0 ||
            data_len == SIMLOCK_CONFIG_FILE_SIZE_V1 ||
            data_len == SIMLOCK_CONFIG_FILE_SIZE_V2) &&
          (category == SIMLOCK_CATEGORY_3GPP_SP_EHPLMN))
                             ||
          ((data_len == SIMLOCK_CONFIG_FILE_SIZE_V0 ||
            data_len == SIMLOCK_CONFIG_FILE_SIZE_V1 ||
            data_len == SIMLOCK_CONFIG_FILE_SIZE_V2 ||
            data_len == SIMLOCK_CONFIG_FILE_SIZE_V3) &&
          (category == SIMLOCK_CATEGORY_3GPP_ICCID))
                             ||
          ((data_len == SIMLOCK_CONFIG_FILE_SIZE_V0 ||
            data_len == SIMLOCK_CONFIG_FILE_SIZE_V1 ||
            data_len == SIMLOCK_CONFIG_FILE_SIZE_V2 ||
            data_len == SIMLOCK_CONFIG_FILE_SIZE_V3 ||
            data_len == SIMLOCK_CONFIG_FILE_SIZE_V4 ||
            data_len == SIMLOCK_CONFIG_FILE_SIZE_V5) &&
          (category == SIMLOCK_CATEGORY_3GPP_IMPI))
                             ||
          ((data_len == SIMLOCK_CONFIG_FILE_SIZE_V0 ||
            data_len == SIMLOCK_CONFIG_FILE_SIZE_V1 ||
            data_len == SIMLOCK_CONFIG_FILE_SIZE_V2 ||
            data_len == SIMLOCK_CONFIG_FILE_SIZE_V3 ||
            data_len == SIMLOCK_CONFIG_FILE_SIZE_V4 ||
            data_len == SIMLOCK_CONFIG_FILE_SIZE_V5 ||
            data_len == SIMLOCK_CONFIG_FILE_SIZE_V6) &&
          (category == SIMLOCK_CATEGORY_3GPP_NS_SP)))
      {
        config_data_ptr->category_status[slot_index][category_index].status = SIMLOCK_DISABLED_FILE_NOT_PRESENT;
        config_data_ptr->category_status[slot_index][category_index].ck_present = SIMLOCK_CK_NOT_PRESENT;
        continue;
      }

      if(data_index < data_len)
      {
        simlock_status = simlock_config_convert_bitmask_to_category_status(
                           data_ptr[data_index],
                           &config_data_ptr->category_status[slot_index][category_index]);
        if(simlock_status != SIMLOCK_SUCCESS)
        {
          return SIMLOCK_GENERIC_ERROR;
        }
        config_data_ptr->category_status[slot_index][category_index].sfs_type = sfs_type;
        data_index++;
      }
    }
  }

  if(data_index != data_len)
  {
    SIMLOCK_MSG_ERR_1("unexpected data buffer size, 0x%x",
                       data_index);
    return SIMLOCK_GENERIC_ERROR;
  }

  return SIMLOCK_SUCCESS;
} /* simlock_config_build_data */


/*===========================================================================
FUNCTION SIMLOCK_CONFIG_CREATE_DIR

DESCRIPTION
  This function creates the simlock directory in the EFS or SFS

DEPENDENCIES
  NONE

LIMITATIONS
  NONE

RETURN VALUE
  simlock_result_enum_type

SIDE EFFECTS
  NONE
===========================================================================*/
static simlock_result_enum_type simlock_config_create_dir
(
  simlock_dir_enum_type         dir_type
)
{
  char  * simlock_dir_ptr = NULL;
  uint32                    auth_enabled    = 0;

  switch (dir_type)
  {
    case SIMLOCK_DIR_CONFIG:
      if(E_SUCCESS == secboot_util_hw_is_auth_enabled(SECBOOT_UTIL_HW_MSS_CODE_SEGMENT, &auth_enabled) && auth_enabled)
      {
        simlock_dir_ptr = simlock_sfs_dir_F;
      }
      else
      {
        simlock_dir_ptr = simlock_sfs_dir;
      }
      break;

    case SIMLOCK_DIR_UIM:
      simlock_dir_ptr = uim_efs_dir;
      break;

    case SIMLOCK_DIR_SFS:
      simlock_dir_ptr = sfs_efs_dir;
      break;

    /* we don't create /safe directory.
       It is always expected to be present */
    case SIMLOCK_DIR_SAFE:
    default:
      break;
  }

  if(simlock_dir_ptr == NULL)
  {
    return SIMLOCK_GENERIC_ERROR;
  }

  /* config directory is created on the SFS */
  if(dir_type == SIMLOCK_DIR_CONFIG)
  {
    return simlock_file_mkdir(simlock_dir_ptr);
  }
  else
  {
    return simlock_file_mkdir_efs(simlock_dir_ptr);
  }
} /* simlock_config_create_dir */


/*===========================================================================
FUNCTION SIMLOCK_CONFIG_CREATE_SIMLOCK_DIR_IF_NEEDED

DESCRIPTION
  This function creates the necessary directories needed for a config
  file to be written/created successfully

DEPENDENCIES
  NONE

LIMITATIONS
  NONE

RETURN VALUE
  simlock_result_enum_type

SIDE EFFECTS
  NONE
===========================================================================*/
simlock_result_enum_type simlock_config_create_simlock_dir_if_needed
(
  void
)
{
  simlock_result_enum_type simlock_status = SIMLOCK_SUCCESS;

  simlock_status = simlock_config_check_dir(SIMLOCK_DIR_UIM);
  if(simlock_status != SIMLOCK_SUCCESS)
  {
    /* Check if the /safe/sfs dir exists in EFS */
    simlock_status = simlock_config_check_dir(SIMLOCK_DIR_SFS);
    if(simlock_status != SIMLOCK_SUCCESS)
    {
      /* if the /safe/sfs directory doesn't exist, create it */
      simlock_status = simlock_config_create_dir(SIMLOCK_DIR_SFS);
      if(simlock_status != SIMLOCK_SUCCESS)
      {
        SIMLOCK_MSG_ERR_1("simlock: create /safe/sfs dir failed, status: 0x%x",
                           simlock_status);
        return simlock_status;
      }
    }
    /* now create /safe/sfs/uim directory */
    simlock_status = simlock_config_create_dir(SIMLOCK_DIR_UIM);
    if(simlock_status != SIMLOCK_SUCCESS)
    {
#ifndef FEATURE_UIM_TEST_FRAMEWORK
      SIMLOCK_MSG_ERR_1("simlock: create /safe/sfs/uim dir failed, status: 0x%x",
                         simlock_status);
      return simlock_status;
#endif /* !FEATURE_UIM_TEST_FRAMEWORK */
    }
  }

  /* If /safe/sfs/uim dir existed already, the simlock config dir under it would
     also be present already... in that case, the following config dir creation
     also fails... however, we do not want to treat that as failure for the
     caller of this function as this function is supposed to create that dir
     if not already present. If the dir is present already, it results in a
     no-op */
  (void) simlock_config_create_dir(SIMLOCK_DIR_CONFIG);

  return SIMLOCK_SUCCESS;
} /* simlock_config_create_simlock_dir_if_needed */


/*===========================================================================
FUNCTION SIMLOCK_CONFIG_WRITE_DATA

DESCRIPTION
  This function writes the configuration data to the config file

DEPENDENCIES
  NONE

LIMITATIONS
  NONE

RETURN VALUE
  simlock_result_enum_type

SIDE EFFECTS
  NONE
===========================================================================*/
simlock_result_enum_type simlock_config_write_data
(
  simlock_config_data_type  * config_data_ptr,
  uint8                       config_id
)
{
  simlock_result_enum_type    simlock_status  = SIMLOCK_SUCCESS;
  simlock_slot_enum_type      slot            = SIMLOCK_SLOT_1;
  simlock_category_enum_type  category        = SIMLOCK_CATEGORY_3GPP_NW;
  uint8                       slot_index      = 0;
  uint8                       category_index  = 0;
  uint8                       data_buf_index  = 0;
  uint8                       data_buf[SIMLOCK_CONFIG_FILE_SIZE];
  uint8                       remote_data_buf[SIMLOCK_CONFIG_FILE_SIZE];

  if(config_data_ptr == NULL)
  {
    return SIMLOCK_INCORRECT_PARAMS;
  }

  memset(data_buf, 0x00, sizeof(data_buf));
  memset(remote_data_buf, 0x00, sizeof(remote_data_buf));

  if((data_buf_index + 6) < SIMLOCK_CONFIG_FILE_SIZE)
  {
    data_buf[data_buf_index++]  = config_data_ptr->sfs_type;
    data_buf[data_buf_index++]  = config_data_ptr->device_mode;
    data_buf[data_buf_index++]  = config_data_ptr->slot_policy;
    data_buf[data_buf_index++]  = config_data_ptr->sub_policy;
    data_buf[data_buf_index++]  = config_data_ptr->rsu_mode;

    for(slot = SIMLOCK_SLOT_1; slot < SIMLOCK_SLOT_NUM; slot++)
    {
      simlock_status = simlock_util_get_slot_index(slot, &slot_index);
      if((simlock_status != SIMLOCK_SUCCESS) ||
         (slot_index >= SIMLOCK_SLOT_COUNT_MAX))
      {
        return SIMLOCK_GENERIC_ERROR;
      }

      if(!config_data_ptr->disabled_slots.slot[slot_index])
      {
        data_buf[data_buf_index]  = data_buf[data_buf_index] | (0x01 << slot_index);
      }
    }
    data_buf_index++;

    data_buf[data_buf_index++] = config_data_ptr->nickname_len;

    simlock_memscpy(&data_buf[data_buf_index], SIMLOCK_CONFIG_FILE_SIZE - data_buf_index,
                    config_data_ptr->nickname, SIMLOCK_CONFIG_NICKNAME_LEN_MAX);
    
    data_buf_index += SIMLOCK_CONFIG_NICKNAME_LEN_MAX;

    /* Updating remote configuration file along with local configuration file.
       This is needed beacuse categories can be eanbled in both local and remote SFS
       at the same time */
    if (config_data_ptr->sfs_type == SIMLOCK_SFS_REMOTE)
    {
      simlock_memscpy(remote_data_buf, sizeof(remote_data_buf), data_buf, data_buf_index);
    }
  }

  for(slot = SIMLOCK_SLOT_1; slot < SIMLOCK_SLOT_NUM;
      slot++)
  {
    simlock_status = simlock_util_get_slot_index(slot, &slot_index);
    if((simlock_status != SIMLOCK_SUCCESS) ||
       (slot_index >= SIMLOCK_SLOT_COUNT_MAX))
    {
      return SIMLOCK_GENERIC_ERROR;
    }

    /* IMEI support in the config file is introduced in V5. So, if the
       blob is a V5 blob, copy over the slot IMEI */
    if(data_buf_index <= (SIMLOCK_CONFIG_FILE_SIZE - 1 - SIMLOCK_IMEI_MAX))
    {
      if(config_data_ptr->imei[slot_index].imei_len > SIMLOCK_IMEI_MAX)
      {
        return SIMLOCK_GENERIC_ERROR;
      }

      data_buf[data_buf_index++] = config_data_ptr->imei[slot_index].imei_len;

      if(config_data_ptr->imei[slot_index].imei_len)
      {
        simlock_memscpy(&data_buf[data_buf_index],
                        (SIMLOCK_CONFIG_FILE_SIZE - data_buf_index),
                        config_data_ptr->imei[slot_index].imei_data,
                        SIMLOCK_IMEI_MAX);
      }

      /* Even though the actual IMEI in the RSU blob might be less than 15
         bytes, we copy 15 bytes and advance the data_index by 15 bytes
         because 15 bytes are dedicated to IMEI in the RSU blob */
      data_buf_index += SIMLOCK_IMEI_MAX;
    }

    for(category = 0; category < SIMLOCK_CATEGORY_COUNT_MAX;
        category++)
    {
      simlock_status = simlock_util_get_category_index(category, &category_index);
      if((simlock_status != SIMLOCK_SUCCESS) ||
         (category_index >= SIMLOCK_CATEGORY_COUNT_MAX))
      {
        return SIMLOCK_GENERIC_ERROR;
      }
      if(data_buf_index < SIMLOCK_CONFIG_FILE_SIZE)
      {
        if(config_data_ptr->sfs_type == SIMLOCK_SFS_REMOTE &&
           config_data_ptr->category_status[slot_index][category_index].sfs_type == SIMLOCK_SFS_REMOTE)
        {
          simlock_status = simlock_config_convert_category_status_to_bitmask(
                                 config_data_ptr->category_status[slot_index][category_index],
                                 &remote_data_buf[data_buf_index]);
        }
        else
        {
          simlock_status = simlock_config_convert_category_status_to_bitmask(
                             config_data_ptr->category_status[slot_index][category_index],
                             &data_buf[data_buf_index]);
        }
        if(simlock_status != SIMLOCK_SUCCESS)
        {
          return SIMLOCK_GENERIC_ERROR;
        }

        data_buf_index++;
      }
    }
  }

  if(data_buf_index != SIMLOCK_CONFIG_FILE_SIZE)
  {
    SIMLOCK_MSG_ERR_1("unexpected data buffer size, 0x%x",
                       data_buf_index);
    return SIMLOCK_GENERIC_ERROR;
  }

  /* Updating remote configuration file along with local configuration file.
     This is needed beacuse categories can be eanbled in both local and remote SFS
     at the same time */
  if(config_data_ptr->sfs_type == SIMLOCK_SFS_REMOTE)
  {
    simlock_status = simlock_file_write(remote_data_buf,
                                        SIMLOCK_CONFIG_FILE_SIZE,
                                        SIMLOCK_FILE_CONFIG,
                                        SIMLOCK_SFS_REMOTE,
                                        SIMLOCK_CONFIG_ID_DEFAULT);
  }

  if(simlock_status == SIMLOCK_SUCCESS)
  {
    /* Check if config file is not present, delete inactive configurations */
    if(config_id == SIMLOCK_CONFIG_ID_DEFAULT &&
       SIMLOCK_CONFIG_FILE_ABSENT == simlock_config_check_config_file(SIMLOCK_CONFIG_ID_DEFAULT))
    {
      (void)simlock_delete_inactive_config();
    }

    /* Store local configuration file */
    simlock_status = simlock_config_create_simlock_dir_if_needed();
    if(simlock_status == SIMLOCK_SUCCESS)
    {
      simlock_status = simlock_file_write(data_buf,
                                          SIMLOCK_CONFIG_FILE_SIZE,
                                          SIMLOCK_FILE_CONFIG,
                                          SIMLOCK_SFS_LOCAL,
                                          config_id);
    }
  }

  return simlock_status;
} /* simlock_config_write_data */


/*===========================================================================
FUNCTION SIMLOCK_CONFIG_CHECK_CONFIG_FILE

DESCRIPTION
  This function checks if the config file is present and is of the expected
  size

DEPENDENCIES
  NONE

LIMITATIONS
  NONE

RETURN VALUE
  simlock_config_file_presence_enum_type

SIDE EFFECTS
  NONE
===========================================================================*/
simlock_config_file_presence_enum_type simlock_config_check_config_file
(
  uint8 config_id
)
{
  simlock_config_file_presence_enum_type config_file_presence     = SIMLOCK_CONFIG_FILE_ABSENT;
  simlock_result_enum_type               simlock_status           = SIMLOCK_SUCCESS;
  uint32                                 sfs_file_len             = 0;

  /* the config file size should be either
     SIMLOCK_CONFIG_FILE_SIZE_V0 (without device_mode and sfs type indicator bytes) or
     SIMLOCK_CONFIG_FILE_SIZE_V1 (with device_mode and sfs type indicator bytes) or
     SIMLOCK_CONFIG_FILE_SIZE_V2 (with device_mode and sfs type indicator bytes and 12 categories) or
     SIMLOCK_CONFIG_FILE_SIZE_V3 (with device_mode and sfs type indicator bytes and 13 categories) or
     SIMLOCK_CONFIG_FILE_SIZE_V4 (with device_mode and sfs type indicator bytes and 14 categories) or
     SIMLOCK_CONFIG_FILE_SIZE_V5 (with device_mode, sfs type indicator and build mode config bytes,
                                  IMEI of slots and 14 categories) or
     SIMLOCK_CONFIG_FILE_SIZE_V6 (with device_mode, sfs type indicator and build mode config bytes,
                                  IMEI of slots and 15 categories) or
     SIMLOCK_CONFIG_FILE_SIZE_V7 (with device_mode, sfs type indicator and build mode config bytes,
                                  IMEI of slots and 16 categories) or
     SIMLOCK_CONFIG_FILE_SIZE_V8 (with device_mode, sfs type indicator and build mode config bytes,
                                  IMEI of slots, 16 categories and slot enable/disable config byte) or 
     SIMLOCK_CONFIG_FILE_SIZE_V9 (with device_mode, sfs type indicator and build mode config bytes,
                                  IMEI of slots, 16 categories, slot enable/disable config byte and nickname)*/
  simlock_status = simlock_file_check(SIMLOCK_FILE_CONFIG, &sfs_file_len, config_id);

  if(simlock_status == SIMLOCK_SUCCESS)
  {
    if(sfs_file_len == SIMLOCK_CONFIG_FILE_SIZE_V0 ||
       sfs_file_len == SIMLOCK_CONFIG_FILE_SIZE_V1 ||
       sfs_file_len == SIMLOCK_CONFIG_FILE_SIZE_V2 ||
       sfs_file_len == SIMLOCK_CONFIG_FILE_SIZE_V3 ||
       sfs_file_len == SIMLOCK_CONFIG_FILE_SIZE_V4 ||
       sfs_file_len == SIMLOCK_CONFIG_FILE_SIZE_V5 ||
       sfs_file_len == SIMLOCK_CONFIG_FILE_SIZE_V6 ||
       sfs_file_len == SIMLOCK_CONFIG_FILE_SIZE_V7 ||
       sfs_file_len == SIMLOCK_CONFIG_FILE_SIZE_V8 ||
       sfs_file_len == SIMLOCK_CONFIG_FILE_SIZE_V9)
    {
      config_file_presence = SIMLOCK_CONFIG_FILE_PRESENT;
    }
    else
    {
      config_file_presence = SIMLOCK_CONFIG_FILE_CORRUPTED;
    }
  }

  return config_file_presence;
} /* simlock_config_check_config_file */


/*===========================================================================
FUNCTION SIMLOCK_CONFIG_READ_CONFIG_DATA_FROM_SFS

DESCRIPTION
  This function reads the configuration data from the requested SFS
  (LOCAL/REMOTE)

DEPENDENCIES
  NONE

LIMITATIONS
  NONE

RETURN VALUE
  simlock_result_enum_type

SIDE EFFECTS
  NONE
===========================================================================*/
static simlock_result_enum_type simlock_config_read_config_data_from_sfs
(
  simlock_sfs_enum_type       sfs_type,
  simlock_config_data_type  * config_data_ptr,
  uint8                       config_id
)
{
  simlock_result_enum_type  simlock_status      = SIMLOCK_SUCCESS;
  uint32                    data_len            = 0;
  uint8                   * data_buf_ptr        = NULL;

  if(config_data_ptr == NULL)
  {
    return SIMLOCK_INCORRECT_PARAMS;
  }

  do
  {
    simlock_status = simlock_file_read(&data_buf_ptr,
                                       &data_len,
                                       SIMLOCK_FILE_CONFIG,
                                       sfs_type,
                                       config_id);
    if(simlock_status != SIMLOCK_SUCCESS)
    {
      SIMLOCK_MSG_ERR_1("config file read failed, status: 0x%x", simlock_status);
      break;
    }

    if((data_len != SIMLOCK_CONFIG_FILE_SIZE_V0  &&
        data_len != SIMLOCK_CONFIG_FILE_SIZE_V1  &&
        data_len != SIMLOCK_CONFIG_FILE_SIZE_V2  &&
        data_len != SIMLOCK_CONFIG_FILE_SIZE_V3  &&
        data_len != SIMLOCK_CONFIG_FILE_SIZE_V4  &&
        data_len != SIMLOCK_CONFIG_FILE_SIZE_V5  &&
        data_len != SIMLOCK_CONFIG_FILE_SIZE_V6  &&
        data_len != SIMLOCK_CONFIG_FILE_SIZE_V7  &&
        data_len != SIMLOCK_CONFIG_FILE_SIZE_V8  &&
        data_len != SIMLOCK_CONFIG_FILE_SIZE_V9) ||
       (data_buf_ptr == NULL))
    {
      SIMLOCK_MSG_ERR_1("incorrect config file len: 0x%x", data_len);
      simlock_status = SIMLOCK_GENERIC_ERROR;
      break;
    }

    simlock_status = simlock_config_build_data((uint16)data_len,
                                                data_buf_ptr,
                                                config_data_ptr,
                                                sfs_type);
    if(simlock_status != SIMLOCK_SUCCESS)
    {
      SIMLOCK_MSG_ERR_1("config file build data failed, status: 0x%x", simlock_status);
      break;
    }
  }while (0);

  SIMLOCK_MEM_FREE(data_buf_ptr);
  return simlock_status;
} /* simlock_config_read_config_data_from_sfs */


/*===========================================================================
FUNCTION SIMLOCK_CONFIG_MERGE_REMOTE_AND_LOCAL_CONFIG_DATA

DESCRIPTION
  This function merge the remote and local configuration data together present
  in configuration file.

DEPENDENCIES
  NONE

LIMITATIONS
  NONE

RETURN VALUE
  simlock_result_enum_type

SIDE EFFECTS
  NONE
===========================================================================*/
static simlock_result_enum_type simlock_config_merge_remote_and_local_config_data
(
  const simlock_config_data_type          *remote_config_data_ptr,
  simlock_config_data_type                *local_config_data_ptr
)
{
  uint8                                   slot_index           = 0;
  uint8                                   category_index       = 0;
  simlock_category_enum_type              category             = SIMLOCK_CATEGORY_3GPP_NW;
  simlock_slot_enum_type                  slot                 = SIMLOCK_SLOT_COUNT_MAX;
  simlock_result_enum_type                simlock_status       = SIMLOCK_SUCCESS;

  if(remote_config_data_ptr == NULL ||
     local_config_data_ptr == NULL)
  {
    return SIMLOCK_INCORRECT_PARAMS;
  }

  /* Merge remote configuration into local configuration:
     1.Slot and Sub policies from local storage
     2.Emergency only is combined between local and remote storage
     3.If a category is enabled on both local and remote storage, return error
   */
  if (remote_config_data_ptr->device_mode == SIMLOCK_DEVICE_EMERGENCY_ONLY_MODE)
  {
    local_config_data_ptr->device_mode = SIMLOCK_DEVICE_EMERGENCY_ONLY_MODE;
  }

  for(slot = SIMLOCK_SLOT_1; slot < SIMLOCK_SLOT_NUM; slot++)
  {
    simlock_status = simlock_util_get_slot_index(slot, &slot_index);
    if((simlock_status != SIMLOCK_SUCCESS) ||
       (slot_index >= SIMLOCK_SLOT_COUNT_MAX))
    {
      return SIMLOCK_GENERIC_ERROR;
    }

    for(category = SIMLOCK_CATEGORY_3GPP_NW; category < SIMLOCK_CATEGORY_COUNT_MAX;
        category++)
    {
      simlock_status = simlock_util_get_category_index(category, &category_index);
      if((simlock_status != SIMLOCK_SUCCESS) ||
         (category_index >= SIMLOCK_CATEGORY_COUNT_MAX))
      {
        return SIMLOCK_GENERIC_ERROR;
      }

      if(remote_config_data_ptr->category_status[slot_index][category_index].status == SIMLOCK_ENABLED)
      {
        if(local_config_data_ptr->category_status[slot_index][category_index].status == SIMLOCK_ENABLED)
        {
          SIMLOCK_MSG_ERR_1("Category Enabled on both Remote and Local SFS 0x%x", category);
          return SIMLOCK_INVALID_STATE;
        }

        /* set the category information to identify whether SimLock is enabled
           and whether the CK is present. */
        local_config_data_ptr->category_status[slot_index][category_index] =
                        remote_config_data_ptr->category_status[slot_index][category_index];
      }
    }
  }
  return SIMLOCK_SUCCESS;
}/* simlock_config_merge_remote_and_local_config_data */


/*===========================================================================
FUNCTION SIMLOCK_DELETE_INACTIVE_CONFIG

DESCRIPTION
  This function deletes all inactive configurations.

DEPENDENCIES
  NONE

LIMITATIONS
  NONE

RETURN VALUE
  simlock_result_enum_type

SIDE EFFECTS
  NONE
===========================================================================*/
simlock_result_enum_type simlock_delete_inactive_config(
  void
)
{
  uint8                                   config_index         = 0;
  uint16                                  index                = 0;
  simlock_config_file_presence_enum_type  config_file_presence = SIMLOCK_CONFIG_FILE_PRESENT;
  simlock_config_data_type               *config_file_data_ptr = NULL;
  simlock_result_enum_type                simlock_status       = SIMLOCK_SUCCESS;
  simlock_slot_enum_type                  slot                 = SIMLOCK_SLOT_1;
  simlock_category_enum_type              category             = SIMLOCK_CATEGORY_3GPP_NW;
  uint8                                   slot_index           = 0;
  uint8                                   category_index       = 0;
  simlock_data_file_enum_type             category_file_enum   = SIMLOCK_FILE_3GPP_NW_1;

  for(index = 1; index <= SIMLOCK_NUM_INACTIVE_CONFIG_MAX && config_index < SIMLOCK_NUM_INACTIVE_CONFIG_MAX; index++)
  {
    config_file_presence = simlock_config_check_config_file((uint8)index);

    if(SIMLOCK_CONFIG_FILE_CORRUPTED == config_file_presence)
    {
      SIMLOCK_MSG_ERR_0("Config file or dir is corrupted");
      return SIMLOCK_GENERIC_ERROR;
    }

    if (SIMLOCK_CONFIG_FILE_PRESENT == config_file_presence)
    {
      /* read config file data */
      config_file_data_ptr = simlock_config_read_config_data((uint8)index);
      if(config_file_data_ptr == NULL)
      {
        return SIMLOCK_GENERIC_ERROR;
      }

      for(slot = SIMLOCK_SLOT_1; slot < SIMLOCK_SLOT_NUM; slot++)
      {
        simlock_status = simlock_util_get_slot_index(slot, &slot_index);
        if((simlock_status == SIMLOCK_SUCCESS) &&
           (slot_index < SIMLOCK_SLOT_COUNT_MAX))
        {
          for(category = SIMLOCK_CATEGORY_3GPP_NW; category <= SIMLOCK_CATEGORY_3GPP2_RUIM;
              category++)
          {
            simlock_status = simlock_util_get_category_index(category, &category_index);
            if((simlock_status == SIMLOCK_SUCCESS) &&
               (category_index < SIMLOCK_CATEGORY_COUNT_MAX))
            {
              /* check if a category is disabled. If yes, continue */
              if(!simlock_util_check_category_enabled(config_file_data_ptr, category, slot))
              {
                continue;
              }

              /* Delete inactive category file */
              category_file_enum = simlock_util_compose_category_file_enum(category, slot);

              if(category_file_enum == SIMLOCK_FILE_ENUM_INVALID)
              {
                continue;
              }

              simlock_status = simlock_delete_file(category_file_enum, (uint8)index);

              if(simlock_status != SIMLOCK_SUCCESS)
              {
                SIMLOCK_MSG_MED_2("delelte inactive category file failed, category: 0x%x slot: 0x%x",
                                  category, slot);
                continue;
              }
            }
          } /* end loop categories */
        }
      } /* end loop for slots */

      simlock_config_free_config_data(config_file_data_ptr);

      /* Delete inactive config file */
      simlock_status = simlock_delete_file(SIMLOCK_FILE_CONFIG, (uint8)index);

      if(simlock_status != SIMLOCK_SUCCESS)
      {
        SIMLOCK_MSG_ERR_0("delelte inactive config file failed");
        return simlock_status;
      }

      config_index++;
    }
  }
  return SIMLOCK_SUCCESS;
}/* simlock_delete_inactive_config */


/*===========================================================================
FUNCTION SIMLOCK_ACTIVATE_INACTIVE_CONFIG

DESCRIPTION
  This function activates categories from inactive configuration associated with
  given config id.

DEPENDENCIES
  NONE

LIMITATIONS
  NONE

RETURN VALUE
  simlock_result_enum_type

SIDE EFFECTS
  NONE
===========================================================================*/
simlock_result_enum_type simlock_activate_inactive_config(
  uint8 config_id
)
{
  simlock_config_file_presence_enum_type  config_file_presence   = SIMLOCK_CONFIG_FILE_PRESENT;
  simlock_config_data_type               *config_file_data_ptr   = NULL;
  simlock_result_enum_type                simlock_status         = SIMLOCK_SUCCESS;
  simlock_slot_enum_type                  slot                   = SIMLOCK_SLOT_1;
  simlock_category_enum_type              category               = SIMLOCK_CATEGORY_3GPP_NW;
  uint8                                   slot_index             = 0;
  uint8                                   category_index         = 0;
  simlock_data_file_enum_type             category_file_enum     = SIMLOCK_FILE_3GPP_NW_1;
  uint32                                  category_file_size     = 0;
  uint8                                  *category_file_data_ptr = NULL;
  simlock_category_file_data_type         category_file_data;

  if(config_id == SIMLOCK_CONFIG_ID_DEFAULT)
  {
    return SIMLOCK_INCORRECT_PARAMS;
  }

  config_file_presence = simlock_config_check_config_file(config_id);

  if(SIMLOCK_CONFIG_FILE_CORRUPTED == config_file_presence ||
     SIMLOCK_CONFIG_FILE_ABSENT == config_file_presence)
  {
    SIMLOCK_MSG_ERR_0("Config file or dir is corrupted");
    return SIMLOCK_GENERIC_ERROR;
  }

  /* read config file data */
  config_file_data_ptr = simlock_config_read_config_data(config_id);

  if(config_file_data_ptr == NULL)
  {
    return SIMLOCK_GENERIC_ERROR;
  }

  for(slot = SIMLOCK_SLOT_1; slot < SIMLOCK_SLOT_NUM; slot++)
  {
    simlock_status = simlock_util_get_slot_index(slot, &slot_index);
    if((simlock_status == SIMLOCK_SUCCESS) &&
       (slot_index < SIMLOCK_SLOT_COUNT_MAX))
    {
      for(category = SIMLOCK_CATEGORY_3GPP_NW; category <= SIMLOCK_CATEGORY_3GPP2_RUIM;
          category++)
      {
        simlock_status = simlock_util_get_category_index(category, &category_index);

        if((simlock_status == SIMLOCK_SUCCESS) &&
           (category_index < SIMLOCK_CATEGORY_COUNT_MAX))
        {
           /* check if a category is disabled. If yes, continue */
          if(!simlock_util_check_category_enabled(config_file_data_ptr, category, slot))
          {
            continue;
          }

          /* Make inactive category file active*/
          category_file_enum = simlock_util_compose_category_file_enum(category, slot);

          if(category_file_enum == SIMLOCK_FILE_ENUM_INVALID)
          {
            simlock_config_free_config_data(config_file_data_ptr);
            return SIMLOCK_GENERIC_ERROR;
          }

          simlock_status = simlock_file_read(&category_file_data_ptr,
                                             &category_file_size,
                                             category_file_enum,
                                             SIMLOCK_SFS_LOCAL,
                                             config_id);

          if(simlock_status != SIMLOCK_SUCCESS ||
             category_file_data_ptr == NULL)
          {
            SIMLOCK_MSG_ERR_1("read category file failed, status: 0x%x",
                              simlock_status);
            continue;
          }

          SIMLOCK_MSG_MED_1("category file size is, 0x%x",
                            category_file_size);

          memset(&category_file_data, 0x00,
                 sizeof(simlock_category_file_data_type));
          
          simlock_status = simlock_category_read_lock_data(category, slot,
                                                           &category_file_data,
                                                           SIMLOCK_SFS_LOCAL,
                                                           config_id);

          if(simlock_status != SIMLOCK_SUCCESS)
          {
            SIMLOCK_MEM_FREE(category_file_data_ptr);
            simlock_config_free_config_data(config_file_data_ptr);
            SIMLOCK_MSG_ERR_1("failed to read category data, status: 0x%x",
                              simlock_status);
            return simlock_status;
          }

          simlock_status = simlock_file_write(category_file_data_ptr,
                                              category_file_size,
                                              category_file_enum,
                                              SIMLOCK_SFS_LOCAL,
                                              SIMLOCK_CONFIG_ID_DEFAULT);

          if(simlock_status != SIMLOCK_SUCCESS)
          {
            SIMLOCK_MEM_FREE(category_file_data_ptr);
            simlock_category_free_code_data(&category_file_data);
            simlock_config_free_config_data(config_file_data_ptr);
            SIMLOCK_MSG_ERR_1("failed to write category file, status: 0x%x",
                              simlock_status);
            return simlock_status;
          }

          /* send event for activated category */
          simlock_send_lock_event(SIMLOCK_SUCCESS,
                                  slot,
                                  category,
                                  0,
                                  category_file_data.category_header.num_retries_max);

          simlock_category_free_code_data(&category_file_data);
          SIMLOCK_MEM_FREE(category_file_data_ptr);
        }
      } /* end loop categories */
    }
  } /* end loop for slots */

  /* Write config data file */
  config_file_data_ptr->nickname_len = 0;
  memset(config_file_data_ptr->nickname, 0x00, SIMLOCK_CONFIG_NICKNAME_LEN_MAX);

  simlock_status = simlock_config_write_data(config_file_data_ptr, SIMLOCK_CONFIG_ID_DEFAULT);

  if(simlock_status != SIMLOCK_SUCCESS)
  {
    SIMLOCK_MSG_ERR_0("write active config file failed");
  }
  else if(SIMLOCK_SUCCESS != simlock_fuse_blow_fuse())
  {
    ASSERT(0);
  }

  simlock_config_free_config_data(config_file_data_ptr);

  return simlock_status;
}/* simlock_activate_inactive_config */


/*===========================================================================
FUNCTION SIMLOCK_GET_INACTIVE_CONFIG

DESCRIPTION
  This function provides config id and nickname for all available inactive
  configurations.

DEPENDENCIES
  NONE

LIMITATIONS
  NONE

RETURN VALUE
  simlock_result_enum_type

SIDE EFFECTS
  NONE
===========================================================================*/
simlock_result_enum_type simlock_get_inactive_config(
  simlock_inactive_config_resp_type * inactive_config_ptr
)
{
  uint8                                   config_index         = 0;
  uint16                                  index                = 0;
  simlock_config_file_presence_enum_type  config_file_presence = SIMLOCK_CONFIG_FILE_PRESENT;
  simlock_config_data_type               *config_file_data_ptr = NULL;

  if(inactive_config_ptr == NULL)
  {
    return SIMLOCK_INCORRECT_PARAMS;
  }

  for(index = 1; index <= SIMLOCK_NUM_INACTIVE_CONFIG_MAX &&
                 config_index < SIMLOCK_NUM_INACTIVE_CONFIG_MAX; index++)
  {
    config_file_presence = simlock_config_check_config_file((uint8)index);

    if(SIMLOCK_CONFIG_FILE_CORRUPTED == config_file_presence)
    {
      SIMLOCK_MSG_ERR_0("Config file or dir is corrupted");
      return SIMLOCK_GENERIC_ERROR;
    }

    if (SIMLOCK_CONFIG_FILE_PRESENT == config_file_presence)
    {
      /* read config file data */
      config_file_data_ptr = simlock_config_read_config_data((uint8)index);
      if(config_file_data_ptr == NULL)
      {
        return SIMLOCK_GENERIC_ERROR;
      }
      else
      {
        inactive_config_ptr->inactive_config[config_index].config_id = (uint8)index;

        inactive_config_ptr->inactive_config[config_index].config_nickname.nickname_len = 
          config_file_data_ptr->nickname_len;
        simlock_memscpy(inactive_config_ptr->inactive_config[config_index].config_nickname.nickname,
                        sizeof(inactive_config_ptr->inactive_config[config_index].config_nickname),
                        config_file_data_ptr->nickname,
                        SIMLOCK_CONFIG_NICKNAME_LEN_MAX);
        inactive_config_ptr->count++;
      }

      simlock_config_free_config_data(config_file_data_ptr);

      config_index++;
    }
  }
  return SIMLOCK_SUCCESS;
}/* simlock_get_inactive_config */


/*===========================================================================
FUNCTION SIMLOCK_MAX_INACTIVE_CONFIG_CREATED

DESCRIPTION
  This function returns TRUE or FALSE based on count of inactve configurations.

DEPENDENCIES
  NONE

LIMITATIONS
  NONE

RETURN VALUE
  boolean
    TRUE    if max inactive config are already created.
    FALSE   if max inactive configs are not created yet.

SIDE EFFECTS
  NONE
===========================================================================*/
boolean simlock_max_inactive_config_created(
  void
)
{
  uint16                                  index                = 0;
  uint8                                   count                = 0;
  simlock_config_file_presence_enum_type  config_file_presence = SIMLOCK_CONFIG_FILE_PRESENT;

  for(index = 1; index <= SIMLOCK_NUM_INACTIVE_CONFIG_MAX; index++)
  {
    config_file_presence = simlock_config_check_config_file((uint8)index);

    if(SIMLOCK_CONFIG_FILE_CORRUPTED == config_file_presence)
    {
      SIMLOCK_MSG_ERR_0("Config file or dir is corrupted");
      return TRUE;
    }

    if (SIMLOCK_CONFIG_FILE_PRESENT == config_file_presence)
    {
      count++;
      if(count == SIMLOCK_NUM_INACTIVE_CONFIG_MAX)
      {
        return TRUE;
      }
    }
  }
  return FALSE;
}/* simlock_max_inactive_config_created */


/*===========================================================================
FUNCTION SIMLOCK_CONFIG_READ_CONFIG_DATA

DESCRIPTION
  This function allocates and reads the configuration data present in a file. If the file
  doesnt exist, and all the security conditions are verified, it returns
  default configuration

DEPENDENCIES
  NONE

LIMITATIONS
  NONE

RETURN VALUE
  simlock_config_data_type*

SIDE EFFECTS
  NONE
===========================================================================*/
simlock_config_data_type* simlock_config_read_config_data
(
  uint8 config_id
)
{
  simlock_result_enum_type                simlock_status       = SIMLOCK_SUCCESS;
  simlock_config_file_presence_enum_type  config_file_presence = SIMLOCK_CONFIG_FILE_PRESENT;
  simlock_config_data_type               *config_data_ptr      = NULL;
  simlock_config_data_type                remote_config_data;

  SIMLOCK_CHECK_AND_MEM_MALLOC(config_data_ptr, sizeof(simlock_config_data_type));
  if(config_data_ptr == NULL)
  {
    return NULL;
  }

  memset(&remote_config_data, 0x00, sizeof(simlock_config_data_type));

  do
  {
    /* Check if the config file already exists */
    config_file_presence = simlock_config_check_config_file(config_id);
    if(SIMLOCK_CONFIG_FILE_CORRUPTED == config_file_presence)
    {
      SIMLOCK_MSG_ERR_0("Config file or dir is corrupted");
      simlock_status = SIMLOCK_GENERIC_ERROR;
      break;
    }
    else if(SIMLOCK_CONFIG_FILE_ABSENT == config_file_presence)
    {
      /* Config file is not present. At this point, if the simlock fuse is not
         blown, that is the device was attempted to be locked so far, return
         default config_data_ptr... else return failure */
      if(simlock_fuse_is_fuse_blown())
      {
        SIMLOCK_MSG_ERR_0("Device is SimLock locked");
        simlock_status = SIMLOCK_GENERIC_ERROR;
        break;
      }

      simlock_status = simlock_config_set_policies_and_imei_and_default_status(
                         SIMLOCK_SLOT_POLICY_SAME_ON_ALL_SLOTS,
                         SIMLOCK_SUBSCRIPTION_POLICY_INDEPENDENT,
                         config_data_ptr);
      if(simlock_status != SIMLOCK_SUCCESS)
      {
        break;
      }

      simlock_status = simlock_config_update_sfs_type(SIMLOCK_SFS_LOCAL,
                                                      config_data_ptr);
      break;
    }

    /* Config file is present. Lets read config data from the file */
    simlock_status = simlock_config_read_config_data_from_sfs(SIMLOCK_SFS_LOCAL,
                                                              config_data_ptr,
                                                              config_id);
    if(simlock_status != SIMLOCK_SUCCESS)
    {
      break;
    }

    if(config_data_ptr->sfs_type == SIMLOCK_SFS_REMOTE)
    {
      /* in case remote SFS storage is being used,
         read the config file from the remote SFS */
      simlock_status = simlock_config_read_config_data_from_sfs(SIMLOCK_SFS_REMOTE,
                                                                &remote_config_data,
                                                                SIMLOCK_CONFIG_ID_DEFAULT);
      if (simlock_status == SIMLOCK_UNSUPPORTED)
      {
        /* Switch to local storage when TZ data lost(example eMMC replacement) and wouldn't
           recover the data. This situation would indicate by application to the modem with
           error code SIMLOCK_UNSUPPORTED. It will be problem for all other error codes */
        simlock_status = simlock_config_update_sfs_type(SIMLOCK_SFS_LOCAL,
                                                        config_data_ptr);
        if(simlock_status == SIMLOCK_SUCCESS)
        {
          simlock_status = simlock_config_write_data(config_data_ptr,
                                                     SIMLOCK_CONFIG_ID_DEFAULT);
        }
        break;
      }
      else if (simlock_status != SIMLOCK_SUCCESS)
      {
        /* Error reading remote SimLock configuration, even if it should be present */
        break;
      }

      simlock_status = simlock_config_merge_remote_and_local_config_data(&remote_config_data,
                                                                         config_data_ptr);
    }
  }while(0);

  if(simlock_status != SIMLOCK_SUCCESS)
  {
    simlock_config_free_config_data(config_data_ptr);
    return NULL;
  }

  return config_data_ptr;
} /* simlock_config_read_config_data */


/*===========================================================================
FUNCTION SIMLOCK_CONFIG_FREE_CONFIG_DATA

DESCRIPTION
  This function frees the memory allocated for config file data.

DEPENDENCIES
  NONE

LIMITATIONS
  NONE

RETURN VALUE
  NONE

SIDE EFFECTS
  NONE
===========================================================================*/
void simlock_config_free_config_data
(
  simlock_config_data_type * config_data_ptr
)
{
  SIMLOCK_MEM_FREE(config_data_ptr);
} /* simlock_config_free_config_data */


/*===========================================================================
FUNCTION SIMLOCK_CONFIG_UPDATE_AND_WRITE_POLICIES

DESCRIPTION
  This function writes the slot and subscription policy in the config file.
  This function is expected to be called before any SIMLock category is
  enabled.

DEPENDENCIES
  NONE

LIMITATIONS
  NONE

RETURN VALUE
  simlock_result_enum_type

SIDE EFFECTS
  NONE
===========================================================================*/
simlock_result_enum_type simlock_config_update_and_write_policies
(
  const simlock_set_config_policy_msg_req_type  * config_policy_ptr,
  simlock_config_data_type                      * config_data_ptr,
  uint8                                           config_id
)
{
  if((config_policy_ptr == NULL) ||
     (config_data_ptr == NULL))
  {
    return SIMLOCK_INCORRECT_PARAMS;
  }

  SIMLOCK_MSG_HIGH_2("slot_policy: 0x%x, sub_policy: 0x%x",
                     config_policy_ptr->slot_policy, config_policy_ptr->sub_policy);

  if(simlock_config_set_policies_and_imei_and_default_status(
       config_policy_ptr->slot_policy,
       config_policy_ptr->sub_policy,
       config_data_ptr) != SIMLOCK_SUCCESS)
  {
    return SIMLOCK_GENERIC_ERROR;
  }

  return simlock_config_write_data(config_data_ptr, config_id);
} /* simlock_config_update_and_write_policies */


/*===========================================================================
FUNCTION SIMLOCK_CONFIG_UPDATE_AND_WRITE_SLOT_CONFIG

DESCRIPTION
  This function writes the slot configuration  in the config file.

DEPENDENCIES
  NONE

LIMITATIONS
  NONE

RETURN VALUE
  simlock_result_enum_type

SIDE EFFECTS
  NONE
===========================================================================*/
simlock_result_enum_type simlock_config_update_and_write_slot_config
(
  const simlock_disabled_slot_data_type  * disabled_slots_ptr,
  simlock_config_data_type               * config_data_ptr
)
{
  if(disabled_slots_ptr == NULL ||
     config_data_ptr == NULL)
  {
    return SIMLOCK_INCORRECT_PARAMS;
  }

  SIMLOCK_MSG_HIGH_2("slot1_disabled: 0x%x, slot2_disabled: 0x%x",
                     disabled_slots_ptr->slot[0],
                     disabled_slots_ptr->slot[1]);

  config_data_ptr->disabled_slots = *disabled_slots_ptr;

  return simlock_config_write_data(config_data_ptr, SIMLOCK_CONFIG_ID_DEFAULT);
} /* simlock_config_update_and_write_slot_config */


/*===========================================================================
FUNCTION SIMLOCK_CONFIG_UPDATE_AND_WRITE_CATEGORY_STATUS

DESCRIPTION
  This function updates and writes the status of a category in the config
  file

DEPENDENCIES
  NONE

LIMITATIONS
  NONE

RETURN VALUE
  simlock_result_enum_type

SIDE EFFECTS
  NONE
===========================================================================*/
simlock_result_enum_type simlock_config_update_and_write_category_status
(
  uint8                          category_index,
  uint8                          slot_index,
  simlock_status_enum_type       status,
  simlock_ck_present_type        ck_present,
  simlock_config_data_type     * config_data_ptr,
  simlock_sfs_enum_type          sfs_type,
  uint8                          config_id
)
{
  simlock_slot_policy_enum_type  slot_policy  = SIMLOCK_SLOT_POLICY_SAME_ON_ALL_SLOTS;
  uint8                          other_index  = 0;

  SIMLOCK_MSG_MED_0("simlock_config_update_and_write_category_status");

   if(config_data_ptr == NULL)
  {
    return SIMLOCK_INCORRECT_PARAMS;
  }

  if(slot_index >= SIMLOCK_SLOT_COUNT_MAX)
  {
    SIMLOCK_MSG_ERR_1("invalid slot index, 0x%x", slot_index);
    return SIMLOCK_GENERIC_ERROR;
  }

  if(category_index >= SIMLOCK_CATEGORY_COUNT_MAX)
  {
    SIMLOCK_MSG_ERR_1("invalid category index, 0x%x",
                      category_index);
    return SIMLOCK_GENERIC_ERROR;
  }

  /* set the category information to identify whether SimLock is enabled
     and whether the CK is present. */
  config_data_ptr->category_status[slot_index][category_index].status     = status;
  config_data_ptr->category_status[slot_index][category_index].ck_present = ck_present;
  config_data_ptr->category_status[slot_index][category_index].sfs_type   = sfs_type;

  slot_policy = config_data_ptr->slot_policy;

  /* if the slot policy is same for all slots, then update the
     status of the category, corresponding to other slots as well */
  if(slot_policy == SIMLOCK_SLOT_POLICY_SAME_ON_ALL_SLOTS)
  {
    for (other_index = 0; other_index < SIMLOCK_SLOT_COUNT_MAX;
         other_index++)
    {
      if(other_index == slot_index)
      {
        continue;
      }

      /* set the category information to identify whether SimLock is enabled
         and whether the CK is present. */
      config_data_ptr->category_status[other_index][category_index].status     = status;
      config_data_ptr->category_status[other_index][category_index].ck_present = ck_present;
      config_data_ptr->category_status[other_index][category_index].sfs_type   = sfs_type;
    }
  }

  return simlock_config_write_data(config_data_ptr, config_id);
} /* simlock_config_update_and_write_category_status */


/*===========================================================================
FUNCTION SIMLOCK_CONFIG_CHECK_DIR

DESCRIPTION
  This function checks if the directory type requested exists in the EFS

DEPENDENCIES
  NONE

LIMITATIONS
  NONE

RETURN VALUE
  simlock_result_enum_type

SIDE EFFECTS
  NONE
===========================================================================*/
simlock_result_enum_type simlock_config_check_dir
(
  simlock_dir_enum_type         dir_type
)
{
  char  * simlock_dir_ptr = NULL;

  switch (dir_type)
  {
    case SIMLOCK_DIR_SAFE:
      simlock_dir_ptr = safe_efs_dir;
      break;

    case SIMLOCK_DIR_UIM:
      simlock_dir_ptr = uim_efs_dir;
      break;

    case SIMLOCK_DIR_SFS:
      simlock_dir_ptr = sfs_efs_dir;
      break;

    case SIMLOCK_DIR_CONFIG:
    default:
      break;
  }

  if(simlock_dir_ptr == NULL)
  {
    return SIMLOCK_GENERIC_ERROR;
  }

  return simlock_efs_name_test(simlock_dir_ptr);
} /* simlock_config_check_dir */


/*===========================================================================
FUNCTION SIMLOCK_CONFIG_UPDATE_AND_WRITE_DEVICE_MODE

DESCRIPTION
  This function updates and writes the device mode to the config
  file

DEPENDENCIES
  NONE

LIMITATIONS
  NONE

RETURN VALUE
  simlock_result_enum_type

SIDE EFFECTS
  NONE
===========================================================================*/
simlock_result_enum_type simlock_config_update_and_write_device_mode
(
  simlock_device_mode_enum_type    device_mode,
  simlock_config_data_type       * config_data_ptr
)
{
  if(config_data_ptr == NULL)
  {
    return SIMLOCK_INCORRECT_PARAMS;
  }

  SIMLOCK_MSG_MED_0("simlock_config_update_and_write_device_mode");

  /* set the device mode */
  config_data_ptr->device_mode = device_mode;

  return simlock_config_write_data(config_data_ptr, SIMLOCK_CONFIG_ID_DEFAULT);
} /* simlock_config_update_and_write_device_mode */


/*===========================================================================
FUNCTION SIMLOCK_CONFIG_UPDATE_SFS_TYPE

DESCRIPTION
  This function updates the SFS type in the config file data variable.

DEPENDENCIES
  NONE

LIMITATIONS
  NONE

RETURN VALUE
  simlock_result_enum_type

SIDE EFFECTS
  NONE
===========================================================================*/
simlock_result_enum_type simlock_config_update_sfs_type
(
  simlock_sfs_enum_type       update_sfs_type,
  simlock_config_data_type  * config_data_ptr
)
{
  SIMLOCK_MSG_MED_0("simlock_config_update_sfs_type");

  if(config_data_ptr == NULL)
  {
    return SIMLOCK_INCORRECT_PARAMS;
  }

  /* set the SFS type */
  config_data_ptr->sfs_type = update_sfs_type;

  return SIMLOCK_SUCCESS;
} /* simlock_config_update_sfs_type */


/*===========================================================================
FUNCTION SIMLOCK_CONFIG_READ_CATEGORY_SFS_TYPE

DESCRIPTION
  This function retrieves the type of SFS (LOCAL/REMOTE) being used for
  storing the simlock category data.

DEPENDENCIES
  NONE

LIMITATIONS
  NONE

RETURN VALUE
  simlock_sfs_enum_type

SIDE EFFECTS
  NONE
===========================================================================*/
simlock_sfs_enum_type simlock_config_read_category_sfs_type
(
  const simlock_config_data_type  *config_data_ptr,
  simlock_category_enum_type       category,
  simlock_slot_enum_type           slot
)
{
  uint8                            category_index    = 0;
  uint8                            slot_index        = 0;
  simlock_result_enum_type         simlock_status    = SIMLOCK_SUCCESS;

  if(config_data_ptr == NULL)
  {
    return SIMLOCK_SFS_LOCAL;
  }

  simlock_status = simlock_util_get_category_index(category, &category_index);
  if(simlock_status != SIMLOCK_SUCCESS)
  {
    return SIMLOCK_SFS_LOCAL;
  }

  simlock_status = simlock_util_get_slot_index(slot, &slot_index);
  if(simlock_status != SIMLOCK_SUCCESS)
  {
    return SIMLOCK_SFS_LOCAL;
  }

  /* Returning local SFS here. Its because by default category storage will be local SFS if a
     particular category is not enabled */
  if(config_data_ptr->category_status[slot_index][category_index].status != SIMLOCK_ENABLED)
  {
    return SIMLOCK_SFS_LOCAL;
  }

  return config_data_ptr->category_status[slot_index][category_index].sfs_type;
} /* simlock_config_read_category_sfs_type */

#endif /* FEATURE_SIMLOCK */

