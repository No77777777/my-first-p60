...
Copyright (c) 2014 - 2015, 2017 QUALCOMM Technologies, Inc (QTI) and its licensors.
...
$Header: //components/rel/uim.mpss/6.1.0/simlock/src/simlock_sfs.c#2 $$ $DateTime: 2019/10/30 06:56:05 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
07/13/17   nr      Remote SIM Unlock supported in common build
10/16/15   vv      Set the appropriate initial value for return status
...
)
{
  ...
  simlock_result_enum_type   write_file_status  = SIMLOCK_GENERIC_ERROR;
  int                        sfs_fd             = 0;
  int                        sfs_status         = 0;
...
)
{
  ...
  /* If the file already exists remove it */
  file_descriptor = sfs_open(sfs_file_name, O_RDONLY);
  if(file_descriptor)
  {
    (void)sfs_close(file_descriptor);

    if(sfs_rm(sfs_file_name) != E_SUCCESS)
    {
      SIMLOCK_MSG_ERR_0("sfs file deletion failed");
      simlock_status = SIMLOCK_GENERIC_ERROR;
    }
