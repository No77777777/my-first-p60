...
--------   ---   ----------------------------------------------------------- 
01/04/16   dt    correcting QXDM sim data collection strip pack issues
12/29/16   dt    Support for QXDM sim data collection logs
11/03/16   me    Fix compilation error
...
#define UIMDIAG_MAX_NUM_SLOTS                   3
...
/* ----------------------------------------------------------------------------
   STRUCTURE:      uimdiag_mmgsdi_clientid_type

   DESCRIPTION:     Request params for client id. Used for
                    close command, deact_prov_cmd, get_app_cap,
                    get_cphs_info_cmd, create_pkcs15_lookup_tbl,
                    session_get_all_pin_status,
                    jcdma_get_card_info_cmd
                    ...
                    rsp_session_refresh_pyld,
                    rsp_session_perso_data_pyld
   client_id_high: Higher 32 bits of client ID for session 
   client_id_low:  Lower  32 bits of client ID for session 
-------------------------------------------------------------------------------*/
typedef PACKED struct PACKED_POST
{  
  uint32                  client_id_high;
  uint32                  client_id_low;
}uimdiag_mmgsdi_clientid_type;
...
/* ----------------------------------------------------------------------------
   STRUCTURE:         uimdiag_mmgsdi_session_read_from_cache_cmd_type

   DESCRIPTION:       Used to build request to MMGSDI to read files from cache
   session_type       session_type
   ef_enum:           File enum to access SIM/USIM/RUIM Elementary Files.
   offset:            offset from where read has to be performed
   rec_num:           record number where the read should be performed
  ------------------------------------------------------------------------------*/
typedef PACKED struct PACKED_POST
{
  ...
  uint32                  offset;
  uint32                  rec_num;
}uimdiag_mmgsdi_session_read_from_cache_cmd_type;
...
/* ----------------------------------------------------------------------------
   STRUCTURE:        uimdiag_mmgsdi_get_atr_cmd_type
...
typedef PACKED struct
{
  ...
  uint32              client_id_low;
  uint32              card_slot;
}uimdiag_mmgsdi_get_atr_cmd_type;
...
/* ----------------------------------------------------------------------------
   STRUCTURE:      uimdiag_mmgsdi_rsp_session_read_from cache_pyld_type

   DESCRIPTION:    Response parameters for read
   more_data:             offset location from where the read should start
   data_len:              length of the data
   data_ptr:              data read from the card
-------------------------------------------------------------------------------*/
typedef PACKED struct PACKED_POST
{
  ...
  uint32                  more_data;
  uint32                  data_len;
  uint8                   data_ptr[UIMDIAG_MAX_DATA_BLOCK_LEN];
...
/* ----------------------------------------------------------------------------
   STRUCTURE:        uimdiag_mmgsdi_get_atr_rsp_type

   DESCRIPTION:      Response for UIMDIAG_MMGSDI_GET_ATR_CMD
...
}uimdiag_mmgsdi_get_atr_rsp_type;
...
/* ----------------------------------------------------------------------------
   STRUCTURE:        uimdiag_mmgsdi_app_info_ext_type

   DESCRIPTION:      App info used in get all available apps ext handling
-----------------------------------------------------------------------------*/
typedef PACKED struct PACKED_POST
{
  uint8        slot_id;
  uint8        pin1_status;
  uint8        pin1_replacement_status;
  uint8        pin1_num_of_retries;
  uint8        pin1_num_of_unblock_retries;
  uint8        pin1_num_of_retries_valid;
  uint8        pin1_num_of_unblock_retries_valid;
  uint8        pin2_status;
  uint8        pin2_num_of_retries;
  uint8        pin2_num_of_unblock_retries;
  uint8        pin2_num_of_retries_valid;
  uint8        pin2_num_of_unblock_retries_valid;
  uint8        upin_status;
  uint8        upin_num_of_retries;
  uint8        upin_num_of_unblock_retries;
  uint8        upin_num_of_retries_valid;
  uint8        upin_num_of_unblock_retries_valid;
  uint8        app_type;
  uint8        aid_len;
  uint8        app_id_ptr[UIM_AID_MAX_NAME_LENGTH];
  uint8        label_len;
  uint8        label_ptr[UIM_AID_MAX_NAME_LENGTH];
  uint8        dir_index;
  uint8        RFU[7];
}uimdiag_mmgsdi_app_info_ext_type;

/* ----------------------------------------------------------------------------
   STRUCTURE:        uimdiag_mmgsdi_get_all_available_apps_ext_rsp_type

   DESCRIPTION:      Response for get all available apps sync ext command
-----------------------------------------------------------------------------*/
typedef PACKED struct PACKED_POST
{
  uint8                                total_no_of_apps;
  uimdiag_mmgsdi_app_info_ext_type     get_all_available_app_info_ext[1];
...
/* ----------------------------------------------------------------------------
   STRUCTURE:        uimdiag_mmgsdi_hide_csim_cmd_type

   DESCRIPTION :     Hide CSIM from modem clients or not

   slot_id:          Slot
   hide_csim:        Boolean that indicates whether CSIM should be visible to the
                     modem or not
  ------------------------------------------------------------------------------*/
typedef struct
{
  uint32             slot_id;
  boolean            hide_csim;
} uimdiag_mmgsdi_hide_csim_cmd_type;

/* ----------------------------------------------------------------------------
   STRUCTURE:        uimdiag_mmgsdi_map_sessions_to_slots_cmd_type

   DESCRIPTION :     Mapping of sessions to slots

   client_id_low: The lower value of client ID
   client_id_high:The higher value of client ID
   sessions_to_slots_len: Length of sessions_to_slots field
...
typedef struct
{
  uint32                  client_id_low;
  uint32                  client_id_high;
  uint32                  sessions_to_slots_len;
  uint32                  sessions_to_slots[UIMDIAG_MAX_NUM_SLOTS];
} uimdiag_mmgsdi_map_sessions_to_slots_cmd_type;

/* ----------------------------------------------------------------------------
   STRUCTURE:          uimdiag_mmgsdi_naa_refresh_ext_cmd_type
   ...
   path:               Path
   enforcement_policy: Refresh enforcement policy value
  ------------------------------------------------------------------------------*/
typedef struct
{
  uint32                  client_id_high;
  ...
  uint32                  path_len;
  uint8                 * path;
  uint8                   enforcement_policy;
}uimdiag_mmgsdi_naa_refresh_ext_cmd_type;
...
DESCRIPTION
  This function will build a request to MMGSDI to read file from cache.
...
DESCRIPTION
   Command handler to get all available apps ext info.
...
DESCRIPTION
  Command handler to extract all information in order to perform GET ATR.
...
DESCRIPTION
  Command handler to extract all information in order indicate to MMGSDI that
  whether the CSIMs on the card should be visible to modem or not.
