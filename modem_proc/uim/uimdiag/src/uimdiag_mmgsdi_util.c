...
$Header: //components/rel/uim.mpss/6.1.0/uimdiag/src/uimdiag_mmgsdi_util.c#3 $$ $DateTime: 2023/03/05 23:08:44 $ $Author: pwbldsvc $

when       who   what, where, why
--------   ---   -----------------------------------------------------------
03/02/23   ghk   Fix overflow issue
...
11/12/15   stv   Fixed compilation error due to featurization of internal APIs
...
}  uimdiag_file_to_mmgsdi_table[] =
{
  ...
  {UIMDIAG_USIM_NASCONFIG,MMGSDI_USIM_NASCONFIG}
...
typedef struct
{
  ...
  uint8                      uimdiag_evt;
} uimdiag_mmgsdi_event_map;

/* Values for some of the existing MMGSDI events is modified. But from a
   UIMDIAG client perpective (or ICD perspective), the value corresponding
   to each of these MMGSDI events must remain same as it has always been.
   So, we introduced this mapping table instead of directly casting a
   MMGSDI event enum to a uimdiag event uint8 */
static const uimdiag_mmgsdi_event_map uimdiag_mmgsdi_event_map_table[] =
{
  {MMGSDI_CARD_INSERTED_EVT,                0x00},
  {MMGSDI_CARD_ERROR_EVT,                   0x01},
  {MMGSDI_CARD_REMOVED_EVT,                 0x02},
  {MMGSDI_SIM_BUSY_EVT,                     0x03},
  {MMGSDI_SAP_CONNECT_EVT,                  0x06},
  {MMGSDI_SAP_DISCONNECT_EVT,               0x07},
  {MMGSDI_SESSION_CLOSE_EVT,                0x09},
  {MMGSDI_TERMINAL_PROFILE_DL_EVT,          0x0B},
  {MMGSDI_SUBSCRIPTION_READY_EVT,           0x0C},
  {MMGSDI_SESSION_CHANGED_EVT,              0x0D},
  {MMGSDI_PIN1_EVT,                         0x0E},
  {MMGSDI_PIN2_EVT,                         0x0F},
  {MMGSDI_UNIVERSAL_PIN_EVT,                0x10},
  {MMGSDI_REFRESH_EVT,                      0x11},
  {MMGSDI_FDN_EVT,                          0x12},
  {MMGSDI_PERSO_EVT,                        0x13},
  {MMGSDI_SESSION_ILLEGAL_SUBSCRIPTION_EVT, 0x14},
  {MMGSDI_HIDDENKEY_EVT,                    0x15},
  {MMGSDI_OTA_COMMIT_RSP_EVT,               0x16},
  {MMGSDI_EONS_EVT,                         0x17},
  {MMGSDI_FILE_UPDATE_EVT,                  0x18},
  {MMGSDI_PROFILE_DATA_EVT,                 0x21},
  {MMGSDI_CARD_DETECTION_STARTED_EVT,       0x24},
  {MMGSDI_RECOVERY_COMPLETE_EVT,            0x25},
  {MMGSDI_PERSO_TEMPORARY_UNLOCK_EVT,       0x26},
  {MMGSDI_EONS_OPL_PNN_CACHE_READY_EVT,     0x86},
  {MMGSDI_ECC_EVT,                          0x87},
  {MMGSDI_SESSION_LEGAL_SUBSCRIPTION_EVT,   0x88}
};
    ...
    uint32                                               diag_req_len)
{
  ...
  uimdiag_offset = 0;
} /* uimdiag_mmgsdi_util_init_decode_params */
...
FUNCTION UIMDIAG_MMGSDI_UTIL_POPULATE_CLIENT_ID

DESCRIPTION
  This function parses the cmd_ptr and fills the client id params
...
SIDE EFFECTS
  ...
  this memory
===========================================================================*/
static uimdiag_return_enum_type uimdiag_mmgsdi_util_populate_client_id(
  uimdiag_mmgsdi_clientid_type            * cmd_ptr,
  const void                              * diag_req_ptr,
  uint32                                    diag_req_len)
{
  UIMDIAG_UTIL_RETURN_IF_NULL_2(cmd_ptr, diag_req_ptr);
  ...
  /* offset updated to end of client_id*/

  return uimdiag_decode_status;
}/* uimdiag_mmgsdi_util_populate_client_id */
...
/*===========================================================================
FUNCTION UIMDIAG_MMGSDI_UTIL_POPULATE_SESSION_READ_FROM_CACHE_CMD

DESCRIPTION
  This command is used to build a request to MMGSDI to read files from cache
...
SIDE EFFECTS
  ...
  this memory
===========================================================================*/
static uimdiag_return_enum_type uimdiag_mmgsdi_util_populate_session_read_from_cache_cmd(
  uimdiag_mmgsdi_session_read_from_cache_cmd_type      *cmd_ptr,
  const void                                           *diag_req_ptr,
  uint32                                                diag_req_len)
{
  UIMDIAG_UTIL_RETURN_IF_NULL_2(cmd_ptr, diag_req_ptr);
  ...
  return uimdiag_decode_status;
}/* uimdiag_mmgsdi_util_populate_session_read_from_cache_cmd */
...
/*===========================================================================
FUNCTION UIMDIAG_MMGSDI_UTIL_POPULATE_GET_ATR

DESCRIPTION
  This function parses cmd_ptr and is used to get all information
  related to UIMDIAG_MMGSDI_GET_ATR_CMD
...
SIDE EFFECTS
  None.
===========================================================================*/
static uimdiag_return_enum_type  uimdiag_mmgsdi_util_populate_get_atr(
    uimdiag_mmgsdi_get_atr_cmd_type   *cmd_ptr,
    const void                        *diag_req_ptr,
  uint32                                                 diag_req_len)
{
  UIMDIAG_UTIL_RETURN_IF_NULL_2(cmd_ptr, diag_req_ptr);
  ...
  return uimdiag_decode_status;
}/* uimdiag_mmgsdi_util_populate_get_atr */
...
FUNCTION UIMDIAG_MMGSDI_UTIL_POPULATE_HIDE_CSIM_CMD

DESCRIPTION
  This function parses cmd_ptr and fills the hide_csim cmd parametes
...
SIDE EFFECTS
  None.
===========================================================================*/
static uimdiag_return_enum_type uimdiag_mmgsdi_util_populate_hide_csim_cmd(
  uimdiag_mmgsdi_hide_csim_cmd_type                * cmd_ptr,
  const void                                       * diag_req_ptr,
  uint32                                             diag_req_len)
{
  UIMDIAG_UTIL_RETURN_IF_NULL_2(cmd_ptr, diag_req_ptr);
  ...
  /* hide_csim */
  uimdiag_mmgsdi_util_decode_uint8((void*)&cmd_ptr->hide_csim);

  return uimdiag_decode_status;
}/* uimdiag_mmgsdi_util_populate_hide_csim_cmd */


/*===========================================================================

FUNCTION UIMDIAG_MMGSDI_UTIL_POPULATE_MAP_SESSIONS_TO_SLOTS_CMD

DESCRIPTION
  This function parses cmd_ptr and fills the map_sessions_to_slots cmd params
...
SIDE EFFECTS
  None.
===========================================================================*/
static uimdiag_return_enum_type uimdiag_mmgsdi_util_populate_map_sessions_to_slots_cmd(
  uimdiag_mmgsdi_map_sessions_to_slots_cmd_type    * cmd_ptr,
  const void                                       * diag_req_ptr,
  uint32                                             diag_req_len)
{
  uint8 i = 0;
  ...
  /* Client ID lower 32 bits  */
  uimdiag_mmgsdi_util_decode_uint32((void*)&cmd_ptr->client_id_low);

  /* Client ID higher 32 bits  */
  uimdiag_mmgsdi_util_decode_uint32((void*)&cmd_ptr->client_id_high);

  /* offset updated to access Data length */
  uimdiag_mmgsdi_util_decode_uint32((void*)&cmd_ptr->sessions_to_slots_len);

  if(cmd_ptr->sessions_to_slots_len == 0)
  {
    UIMDIAG_MSG_ERROR_0("Map Sessions to Slots, Data Len is 0");
    return UIMDIAG_ERROR;
  }

  for(i = 0;
      i < cmd_ptr->sessions_to_slots_len &&
       i < sizeof(cmd_ptr->sessions_to_slots)/sizeof(cmd_ptr->sessions_to_slots[0]);
      i++)
  {
    uimdiag_mmgsdi_util_decode_uint32((void*)&cmd_ptr->sessions_to_slots[i]);
  }

  return uimdiag_decode_status;
}/* uimdiag_mmgsdi_util_populate_map_sessions_to_slots_cmd */


/*===========================================================================
FUNCTION UIMDIAG_MMGSDI_UTIL_POPULATE_NAA_REFRESH_EXT_CMD

DESCRIPTION
  This command is used to build a request to trigger for refresh with
  refresh enforcement policy support
...
SIDE EFFECTS
  ...
  this memory
===========================================================================*/
static uimdiag_return_enum_type  uimdiag_mmgsdi_util_populate_naa_refresh_ext_cmd(
  uimdiag_mmgsdi_naa_refresh_ext_cmd_type   * cmd_ptr,
  const void                                * diag_req_ptr,
  uint32                                      diag_req_len)
{
  UIMDIAG_UTIL_RETURN_IF_NULL_2(cmd_ptr, diag_req_ptr);
  ...
  uimdiag_mmgsdi_util_decode_uint8((void*)&cmd_ptr->enforcement_policy);

  /* offset updated to the end of the data structure */
  return uimdiag_decode_status;
}/* uimdiag_mmgsdi_util_populate_naa_refresh_ext_cmd */
#endif /* FEATURE_UIMDIAG_INTERNAL_API */
...
{
  uint8 i          = 0;
  uint8 table_size = (uint8)(sizeof(uimdiag_mmgsdi_event_map_table) /
                               sizeof(uimdiag_mmgsdi_event_map));

  if (uimdiag_mmgsdi_evt_ptr == NULL)
  {
  ...
  {
    if (uimdiag_mmgsdi_event_map_table[i].mmgsdi_evt == mmgsdi_evt)
    {
      *uimdiag_mmgsdi_evt_ptr = uimdiag_mmgsdi_event_map_table[i].uimdiag_evt;
      return UIMDIAG_SUCCESS;
    }
  }
  return UIMDIAG_ERROR;
} /* uimdiag_util_convert_from_mmgsdi_events_enum */
...
)
{
  ...
  switch(uimdiag_subsys_cmd)
  {
      ...
      uimdiag_status = uimdiag_mmgsdi_util_populate_client_id(
                       &req_ptr->request.client_id_and_evt_dereg,
                       (void*)diag_req_ptr, diag_req_len);
      break;

      case UIMDIAG_MMGSDI_SESSION_OPEN_CMD:
      uimdiag_status = uimdiag_mmgsdi_util_populate_session_open_cmd(
                       &req_ptr->request.session_open,
                       (void*)diag_req_ptr, diag_req_len);
      break;

      case UIMDIAG_MMGSDI_SESSION_CLOSE_CMD:
      uimdiag_status = uimdiag_mmgsdi_util_populate_session_id(
                       &req_ptr->request.session_close,
                       (void*)diag_req_ptr, diag_req_len);
      break;

      case UIMDIAG_MMGSDI_SESSION_ACT_OR_SW_PROV_CMD:
      uimdiag_status = uimdiag_mmgsdi_util_populate_session_act_or_sw_prov_cmd(
                     &req_ptr->request.session_act_or_sw_prov,
                     (void*)diag_req_ptr, diag_req_len);
      break;

      case UIMDIAG_MMGSDI_SESSION_GET_INFO_CMD:
      uimdiag_status = uimdiag_mmgsdi_util_populate_session_get_info_cmd(
                     &req_ptr->request.session_get_info,
                     (void*)diag_req_ptr, diag_req_len);
      break;

      case UIMDIAG_MMGSDI_SESSION_VERIFY_PIN_CMD:
      uimdiag_status = uimdiag_mmgsdi_util_populate_session_verify_pin_cmd(
                 &req_ptr->request.session_verify_pin,
                 (void*)diag_req_ptr, diag_req_len);
      break;

      case UIMDIAG_MMGSDI_SESSION_GET_FILE_ATTR_CMD:
      uimdiag_status = uimdiag_mmgsdi_util_populate_session_get_file_attr_cmd(
                &req_ptr->request.session_get_file_attr,
                (void*)diag_req_ptr, diag_req_len);
      break;

      case UIMDIAG_MMGSDI_SESSION_READ_RECORD_CMD:
      uimdiag_status = uimdiag_mmgsdi_util_populate_session_read_record_cmd(
                &req_ptr->request.session_read_record,
                (void*)diag_req_ptr, diag_req_len);
      break;

    case UIMDIAG_MMGSDI_SESSION_READ_FROM_CACHE_CMD:
      uimdiag_status = uimdiag_mmgsdi_util_populate_session_read_from_cache_cmd(
                       &req_ptr->request.session_read_from_cache,
                       (void*)diag_req_ptr, diag_req_len);
      break;

      case UIMDIAG_MMGSDI_SESSION_READ_TRANSPARENT_CMD:
      uimdiag_status = uimdiag_mmgsdi_util_populate_session_read_transparent_cmd(
                &req_ptr->request.session_read_transparent,
                (void*)diag_req_ptr, diag_req_len);
      break;

      case UIMDIAG_MMGSDI_SESSION_WRITE_RECORD_CMD:
      uimdiag_status = uimdiag_mmgsdi_util_populate_session_write_record_cmd(
                &req_ptr->request.session_write_record,
                (void*)diag_req_ptr, diag_req_len);
      break;

      case UIMDIAG_MMGSDI_SESSION_WRITE_TRANSPARENT_CMD:
      uimdiag_status = uimdiag_mmgsdi_util_populate_session_write_transparent_cmd(
      ...
      uimdiag_status = uimdiag_mmgsdi_util_populate_get_info_sync(
      ...
      uimdiag_status = uimdiag_mmgsdi_util_populate_card_pdown(
          &req_ptr->request.card_pdown,
          (void*)diag_req_ptr, diag_req_len);
      break;

      case UIMDIAG_MMGSDI_CARD_PUP_CMD:
      uimdiag_status = uimdiag_mmgsdi_util_populate_card_pup(
      ...
      uimdiag_status = uimdiag_mmgsdi_util_populate_activate_onchip_sim(
          &req_ptr->request.activate_onchip_sim,
          (void*)diag_req_ptr, diag_req_len);
      break;

      case UIMDIAG_MMGSDI_SEND_APDU_CMD:
      uimdiag_status = uimdiag_mmgsdi_util_populate_send_apdu(
          &req_ptr->request.send_apdu,
          (void*)diag_req_ptr, diag_req_len);
      break;

    case UIMDIAG_MMGSDI_GET_ATR_CMD:
      uimdiag_status = uimdiag_mmgsdi_util_populate_get_atr(
                       &req_ptr->request.get_atr,
                       (void*)diag_req_ptr, diag_req_len);
      break;

    case UIMDIAG_MMGSDI_GET_ALL_AVAIL_APPS_SYNC_EXT_CMD:
      uimdiag_status = uimdiag_mmgsdi_util_populate_client_id(
                       &req_ptr->request.get_all_available_apps_ext,
                       (void*)diag_req_ptr, diag_req_len);
      break;
      ...
      uimdiag_status = uimdiag_mmgsdi_util_populate_session_disable_pin_cmd(
                 &req_ptr->request.session_disable_pin,
                 (void*)diag_req_ptr, diag_req_len);
      break;

      case UIMDIAG_MMGSDI_SESSION_ENABLE_PIN_CMD:
      uimdiag_status = uimdiag_mmgsdi_util_populate_session_enable_pin_cmd(
                &req_ptr->request.session_enable_pin,
                (void*)diag_req_ptr, diag_req_len);
      break;

      case UIMDIAG_MMGSDI_SESSION_CHANGE_PIN_CMD:
      uimdiag_status = uimdiag_mmgsdi_util_populate_session_change_pin_cmd(
                &req_ptr->request.session_change_pin,
                (void*)diag_req_ptr, diag_req_len);
      break;

      case UIMDIAG_MMGSDI_SESSION_UNBLOCK_PIN_CMD:
      uimdiag_status = uimdiag_mmgsdi_util_populate_session_unblock_pin_cmd(
      ...
      uimdiag_status = uimdiag_mmgsdi_util_populate_app_init_complete(
      ...
      uimdiag_status = uimdiag_mmgsdi_util_populate_session_get_operator_name_cmd(
          &req_ptr->request.session_get_operator_name,
          (void*)diag_req_ptr, diag_req_len);
      break;

      case UIMDIAG_MMGSDI_HIDE_CSIM_CMD:
      uimdiag_status = uimdiag_mmgsdi_util_populate_hide_csim_cmd(
          &req_ptr->request.hide_csim,
          (void*)diag_req_ptr, diag_req_len);
      break;

      case UIMDIAG_MMGSDI_MAP_SESSIONS_TO_SLOTS_CMD:
      uimdiag_status = uimdiag_mmgsdi_util_populate_map_sessions_to_slots_cmd(
          &req_ptr->request.map_sessions_to_slots,
          (void*)diag_req_ptr, diag_req_len);
      break;

      case UIMDIAG_MMGSDI_NAA_REFRESH_EXT_CMD:
      uimdiag_status = uimdiag_mmgsdi_util_populate_naa_refresh_ext_cmd(
               &req_ptr->request.session_trigger_refresh_ext,
               (void*)diag_req_ptr, diag_req_len);
      break;
#endif /* FEATURE_UIMDIAG_INTERNAL_API */
  ...
  {
    UIMDIAG_MSG_ERROR_2("uimdiag_mmgsdi_parse_commands: Parsing error 0x%x,uimdiag_subsys_cmd 0x%x",
                         uimdiag_status,uimdiag_subsys_cmd);
  }
