...
$Header: //components/rel/uim.mpss/6.1.0/mmgsdi/src/mmgsdi_1x_upp.c#2 $

when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
09/28/16   me      Removed feature flag FEATURE_MMGSDI_3GPP2   
...
)
{
  ...
  if(mmgsdi_client_req_table_info_ptr[index]->uim_rsp_status == MMGSDI_UIM_RSP_NOT_RECEIVED)
  {
    ...
    MMGSDIUTIL_TMC_MEM_FREE(otasp_commit_rsp_ptr->rsp_data.data_ptr);
    MMGSDIUTIL_TMC_MEM_FREE(otasp_commit_rsp_ptr);
...
)
{
  ...
  if(cnf_status != MMGSDI_SUCCESS)
  {
    ...
    MMGSDIUTIL_TMC_MEM_FREE(mdn_data_ptr);
    return;
  }
...
)
{
  ...
  if (mmgsdi_status != MMGSDI_SUCCESS)
  {
    MMGSDIUTIL_TMC_MEM_FREE(mdn_data_sipupp_ptr);
    return mmgsdi_status;
  }
  ...
  if (mmgsdi_status != MMGSDI_SUCCESS)
  {
    MMGSDIUTIL_TMC_MEM_FREE(mdn_data_mipupp_ptr);
    return mmgsdi_status;
  }
