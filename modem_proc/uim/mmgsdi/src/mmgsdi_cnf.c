...
$Header: //components/rel/uim.mpss/6.1.0/mmgsdi/src/mmgsdi_cnf.c#4 $$ $DateTime: 2024/02/26 16:03:48 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
01/24/24   cj      Send NV refreh done after processing app deact and act 
04/10/20   cj      Sync QMI UIM with proper slot status upon task start up
...
07/11/17   ll      Return full apdu response for SAP for SAP non-implicit case
06/08/17   nr      Modify FCP of linear fixed EF to have maximum of 254 records
...
07/01/16   vdc     Send only one response for SELECT_AID request
...
04/21/16   vdc     Send confirmation for activate request in case of onchip
...
10/12/15   vdc     Added support for recovery start notification
10/05/15   sp      Reduce signed/unsigned conversions in UIM
09/10/15   hh      Add support for potential silent file change notification
...
/* =============================================================================
   ...
   SIDE EFFECTS:
     None
=============================================================================*/
static mmgsdi_return_enum_type mmgsdi_cnf_build_session_enable_service_data(
  mmgsdi_return_enum_type                  mmgsdi_status,
...
/* =============================================================================
   ...
   SIDE EFFECTS:
     None
=============================================================================*/
static mmgsdi_return_enum_type mmgsdi_cnf_build_session_disable_service_data(
  mmgsdi_return_enum_type                  mmgsdi_status,
...
/* =============================================================================
   ...
   SIDE EFFECTS:
     None
=============================================================================*/
static mmgsdi_return_enum_type mmgsdi_cnf_build_get_se13_plmn_names_data(
    ...
    if (add_info_ptr->select_rsp_requested)
    {
      ...
      if (add_info_ptr->rsp_data.data_len > 0)
      {
        ...
        mmgsdi_memscpy(select_aid_cnf_ptr->select_rsp.data_ptr,
                       ...
                       add_info_ptr->rsp_data.data_len);
      }
    }
  }
  ...
  switch(task_cmd_ptr->cmd.cmd.mmgsdi_cnf_ptr->mmgsdi_cnf.response_header.response_type)
  {
    ...
    case MMGSDI_GET_M2M_EID_REQ:
      build_status = mmgsdi_cnf_build_get_m2m_eid_data(
        ...
        mmgsdi_status, (mmgsdi_send_apdu_data_type*)add_data_ptr);
      break;
...
)
{
  ...
  if (build_cnf_status != MMGSDI_SUCCESS)
  {
    ...
    MMGSDIUTIL_TMC_MEM_FREE(increase_cnf_ptr->increased_data.data_ptr);
  }
...
)
{
  ...
  MMGSDIUTIL_RETURN_IF_NULL_5(extra_param_ptr,
                              data_read_ptr,
                              req_header_ptr,
  ...
  if (*mmgsdi_status_ptr == MMGSDI_SUCCESS)
  {
    ...
    /* Initialize  file type to MMGSDI_MAX_FILE_STRUCTURE_ENUM so that if file
       ...
       considered as MMGSDI_LINEAR_FIXED_FILE (0). */
    get_file_attr_cnf_ptr->file_attrib.file_type = MMGSDI_MAX_FILE_STRUCTURE_ENUM;

    if(protocol == MMGSDI_UICC)
    {
      /* Extract the important file attributes from the file header */
      *mmgsdi_status_ptr = mmgsdi_uicc_rsp_decode_file_header(req_header_ptr,
                                                              &extra_param_ptr->get_file_attr_data,
                                                              data_read_ptr->data_ptr,
                                                              data_read_ptr->data_len,
                                                              &get_file_attr_cnf_ptr->file_attrib);

      /* In some test card, the FCP given in SELECT respose was all 0xFF. Although
         the status sent to the client was success for SELECT, the actual parsing 
         failed. This resulted in network camping issues.
         To avoid this, MMGSDI sends error only in case of an invalid FCP template tag */
      if(*mmgsdi_status_ptr != MMGSDI_WRONG_FCP_TEMPLATE_TAG)
      {
    ...
    {
      /* Extract the important file attributes from the file header */
      (void)mmgsdi_icc_rsp_extract_file_attr(req_header_ptr->slot_id,
                                             data_read_ptr->data_ptr,
                                             data_read_ptr->data_len,
                                             &get_file_attr_cnf_ptr->file_attrib,
                                             FALSE);
    ...
    switch (get_file_attr_cnf_ptr->file_attrib.file_type)
    {
      case MMGSDI_LINEAR_FIXED_FILE:
        ...
        if(get_file_attr_cnf_ptr->file_attrib.file_info.linear_fixed_file.num_of_rec > MMGSDI_MAX_RECS_IN_LINEAR_FIXED_EF)
        {
          get_file_attr_cnf_ptr->file_attrib.file_info.linear_fixed_file.num_of_rec = MMGSDI_MAX_RECS_IN_LINEAR_FIXED_EF;
        }
        break;
    ...
    if (build_cnf_status == MMGSDI_SUCCESS)
    {
      /* If the file is FPLMN update actual size so that it will be used later
         while creating larger FPLMN cache */
      if(extra_param_ptr->get_file_attr_data.access.access_method == MMGSDI_EF_ENUM_ACCESS &&
         (extra_param_ptr->get_file_attr_data.access.file.file_enum == MMGSDI_USIM_FPLMN ||
          extra_param_ptr->get_file_attr_data.access.file.file_enum == MMGSDI_GSM_FPLMN))
      {
        mmgsdi_int_app_info_type     *app_info_ptr  = NULL;
        ...
        if((mmgsdi_status == MMGSDI_SUCCESS) && (app_info_ptr != NULL))
        {
          app_info_ptr->fplmn_actual_size_on_card = get_file_attr_cnf_ptr->file_attrib.file_size;
        }
      }
      ...
      (void) mmgsdi_cache_attr_write(req_header_ptr->session_id,
                                     ...
                                     security_access_ptr,
                                     get_file_attr_cnf_ptr->file_attrib.is_arr_skipped,
                                     get_file_attr_cnf_ptr->file_attrib.arr_file_id,
                                     get_file_attr_cnf_ptr->file_attrib.arr_record_no,
  ...
  const mmgsdi_isim_auth_data_type  *auth_rsp_data_ptr)
{
  ...
  if (mmgsdi_status == MMGSDI_SUCCESS)
  {
    ...
    if (auth_rsp_data_ptr->res.data_len > 0)
    {
      ...
      isim_auth_cnf_ptr->res.data_len = auth_rsp_data_ptr->res.data_len;
    ...
    if (auth_rsp_data_ptr->ck.data_len > 0)
    {
      MMGSDIUTIL_MEM_MALLOC_AND_VALIDATE(isim_auth_cnf_ptr->ck.data_ptr,
                                         auth_rsp_data_ptr->ck.data_len);
      if (isim_auth_cnf_ptr->ck.data_ptr == NULL)
      {
        /* Free the auth data */
        MMGSDIUTIL_TMC_MEM_FREE(isim_auth_cnf_ptr->res.data_ptr);
        return MMGSDI_MEMORY_ERROR_HEAP_EXHAUSTED;
      }
      isim_auth_cnf_ptr->ck.data_len = auth_rsp_data_ptr->ck.data_len;
    ...
    if (auth_rsp_data_ptr->ik.data_len > 0)
    {
      MMGSDIUTIL_MEM_MALLOC_AND_VALIDATE(isim_auth_cnf_ptr->ik.data_ptr,
                                         auth_rsp_data_ptr->ik.data_len);
      if (isim_auth_cnf_ptr->ik.data_ptr == NULL)
      {
        /* Free the auth data */
        MMGSDIUTIL_TMC_MEM_FREE(isim_auth_cnf_ptr->res.data_ptr);
        MMGSDIUTIL_TMC_MEM_FREE(isim_auth_cnf_ptr->ck.data_ptr);
        return MMGSDI_MEMORY_ERROR_HEAP_EXHAUSTED;
      }
      isim_auth_cnf_ptr->ik.data_len = auth_rsp_data_ptr->ik.data_len;
  ...
  mmgsdi_slot_id_enum_type                  slot_id)
{
  ...
  {
    MMGSDIUTIL_MEM_MALLOC_AND_VALIDATE(apdu_cnf_ptr->apdu_data.data_ptr, 0x02);
  ...
  if (apdu_cnf_ptr->implicit_get_rsp_sw1 == 0x61 &&
      uim_rsp_data_ptr->data_len > (MMGSDI_GEN_MAX_GET_RESPONSE_REQ_LEN + MMGSDI_GEN_SW_LEN)) 
  {
    /* If there are more than 0xFF data to return, return 0xFF as sw2 */
    apdu_cnf_ptr->implicit_get_rsp_sw2 = MMGSDI_GEN_MAX_GET_RESPONSE_REQ_LEN;
  }
  else
  {
    /* else, return the actual sw2 */
...
/* =============================================================================
   ...
   RETURN VALUE:
     boolean
     TRUE: if NV refresh is in progress, FALSE otherwise
   ...
   CRITICAL SECTIONS:
    None
=============================================================================*/
static boolean mmgsdi_trigger_mcfg_refresh_session(
...
)
{
  ...
  mmgsdi_return_enum_type          mmgsdi_status            = MMGSDI_ERROR;
  mmgsdi_slot_data_type           *slot_data_ptr            = NULL;
  mmgsdi_slot_id_enum_type         session_to_check_slot    = MMGSDI_SLOT_NONE;
  mmgsdi_int_app_info_type        *app_info_ptr             = NULL;
  ...
  if((mmgsdi_status != MMGSDI_SUCCESS) ||
     (session_to_check_slot != slot) ||
     (session_to_check_app_ptr == NULL) ||
     ((session_to_check_app_ptr->nv_refresh_in_progress == FALSE)))
  {
    mmgsdi_technology_enum_type session_tech      = MMGSDI_TECH_UNKNOWN;
    mmgsdi_attribute_enum_type  session_attribute = MMGSDI_ATTR_UNKNOWN;
    ...
    /* Maintain different ref id for GW and 1x sessions because MMGSDI may call
       MCFG API back to back without getting response of first. MMGSDI needs to
       handle both responses after comparing with respective ref id.
       If MMGSDI uses only one ref id global per slot, it will be incremented in second
       call and MMGSDI will not be able to handle response of first because of
       mismatching ref id. */
    app_info_ptr->nv_refresh_ref_count++;

    mmgsdi_status = mmgsdi_util_get_tech_and_attr_from_session_type(
                      session_info_ptr->session_type,
                      &session_tech,
                      &session_attribute);
    ...
    /* It is required to assign different ref count to 3GPP and 3GPP2 apps.
       Because when MCFG triggers MMGSDI to continue after refresh, Ref count
       can match with that of both apps if thier respective ref count is not
       defferentiated by MSB. */
    if(session_tech == MMGSDI_TECH_3GPP)
    {
      /* Set most significant bit to 0 for GW session */
      app_info_ptr->nv_refresh_ref_count = 0x7FFFFFFF & app_info_ptr->nv_refresh_ref_count;
    }
    else
    {
      /* Set most significant bit to 1 for 1x session */
      app_info_ptr->nv_refresh_ref_count = 0x80000000 | app_info_ptr->nv_refresh_ref_count;
    }
    mcfg_uim_data.reference_id = app_info_ptr->nv_refresh_ref_count;

    mcfg_refresh_status = mcfg_refresh_update_from_uim (&mcfg_uim_data);
    if(mcfg_refresh_status)
    {
      app_info_ptr->nv_refresh_in_progress = TRUE;
    }
  ...
  return app_info_ptr->nv_refresh_in_progress;
  ...
  const mmgsdi_callback_cnf_type *callback_cnf_ptr)
{
  ...
  switch(cnf)
  {
    ...
    case MMGSDI_SESSION_ACT_OR_SWITCH_PROV_REQ:
      ...
      {
        boolean nv_refresh_in_progress = FALSE;
        ...
        /* In the case of Onchip we send out session changed event and set provisioning app indicies 
           as part of the command processing. So only send response and skip publishing event */
        ...
        *prov_app_indices_ptr = app_index | MMGSDI_PROV_INDEX_VALIDITY_MASK;
        ...
        {
          nv_refresh_in_progress = mmgsdi_trigger_mcfg_refresh_session(slot,
                                   session_info_ptr,
                                   session_to_check,
                                   mcfg_refresh_index);
        }
...
)
{
  ...
  if (build_cnf_status != MMGSDI_SUCCESS)
  {
    ...
    MMGSDIUTIL_TMC_MEM_FREE(search_cnf_ptr->searched_record_nums.data_ptr);
