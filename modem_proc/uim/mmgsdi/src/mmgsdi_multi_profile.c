...
Copyright (c) 2014 - 2017, 2020 QUALCOMM Technologies, Inc (QTI) and its licensors.
...
$Header: //components/rel/uim.mpss/6.1.0/mmgsdi/src/mmgsdi_multi_profile.c#4 $

when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
08/31/20   sch     Allocate free index while sending SIN APDU
12/09/19   vgd     Fix memory leak and enable multi profile in offtarget
04/06/17   gm      Added support for extended APDU
03/14/17   nc      Better checks for return values 
01/10/17   vdc     Queue profile determination cmd only once during boot up
12/01/16   vdc     Added support for new version of multi profile card
11/14/16   vdc     Change parsing logic to get current active profile
...
07/23/15   vdc     Featurize registration of callback with ecall 
06/08/15   vdc     Remove memory free for command when not sent to card
02/10/15   vdc     Add more sanity checks while parsing multi profile EFS
02/05/15   vdc     Leave crit sect when MMGSDI task is stopped in ecall CB
...
static void mmgsdi_multi_profile_internal_cb(
...
)
{
  ...
  for(apdu_index = 0; apdu_index < num_apdu_count; apdu_index++)
  {
    ...
    uim_cmd_ptr->stream_iso7816_apdu.num_cmd_bytes = uint32touint16(apdu_data_ptr->data_len);
    uim_cmd_ptr->stream_iso7816_apdu.cmd_data = apdu_data_ptr->data_ptr;
    ...
    if(mmgsdi_profile_info_ptr->dedicated_channel_needed)
    {
      ...
      {
        /* Fill CLA byte with the APDU class byte ORed with dedicated channel id
           Eg: APDU Class byte: 0xA0 and dedicated channel is 0x01 then CLA byte is 0xA1 */
...
)
{
  ...
  if(mmgsdi_profile_info_ptr != NULL)
  {
    ...
    MMGSDIUTIL_TMC_MEM_FREE(mmgsdi_profile_info_ptr->get_sin_apdu.data_ptr);
...
)
{
  ...
  if(file_buffer_ptr + mmgsdi_profile_info_ptr->aid.data_len <=
       orig_file_buffer_ptr + multi_profile_file_size)
  {
    if(mmgsdi_profile_info_ptr->aid.data_len > 0)
    {
  ...
  else
  {
         ...
         orig_file_buffer_ptr + multi_profile_file_size)
    {
      ...
      file_buffer_ptr += MMGSDI_MULTI_PROFILE_CARD_MULTI_PROFILE_SUPPORT_LENGTH;
    }
    else
    {
      goto send_result;
    ...
    /* If multi profile support is present then parse Get SIN APDU and
       proprietary SIN data */
    if(mmgsdi_profile_info_ptr->check_multi_profile_support)
    {
      /* Parse Get SIN APDU length */
      if(file_buffer_ptr + MMGSDI_MULTI_PROFILE_CARD_MAX_APDU_LENGTH <=
           orig_file_buffer_ptr + multi_profile_file_size)
      {
        mmgsdi_profile_info_ptr->get_sin_apdu.data_len = *file_buffer_ptr;
        file_buffer_ptr += MMGSDI_MULTI_PROFILE_CARD_MAX_APDU_LENGTH;
      }
      else
      {
      ...
      /* Parse Get SIN APDU */
      if(file_buffer_ptr + mmgsdi_profile_info_ptr->get_sin_apdu.data_len <=
           orig_file_buffer_ptr + multi_profile_file_size)
      {
        if(mmgsdi_profile_info_ptr->get_sin_apdu.data_len > 0)
        {
          MMGSDIUTIL_MEM_MALLOC_AND_VALIDATE(
            mmgsdi_profile_info_ptr->get_sin_apdu.data_ptr,
            mmgsdi_profile_info_ptr->get_sin_apdu.data_len);

          if(mmgsdi_profile_info_ptr->get_sin_apdu.data_ptr == NULL)
          {
            goto send_result;
          }

          mmgsdi_memscpy(mmgsdi_profile_info_ptr->get_sin_apdu.data_ptr,
                         mmgsdi_profile_info_ptr->get_sin_apdu.data_len,
                         file_buffer_ptr,
                         mmgsdi_profile_info_ptr->get_sin_apdu.data_len);

          file_buffer_ptr += mmgsdi_profile_info_ptr->get_sin_apdu.data_len;
        }
      }
      else
      {
      ...
      /* SIN data */
      if(file_buffer_ptr + sizeof(mmgsdi_profile_info_ptr->sin_data) <=
           orig_file_buffer_ptr + multi_profile_file_size)
      {
        mmgsdi_memscpy(mmgsdi_profile_info_ptr->sin_data,
                       sizeof(mmgsdi_profile_info_ptr->sin_data),
                       file_buffer_ptr,
                       sizeof(mmgsdi_profile_info_ptr->sin_data));

        file_buffer_ptr += sizeof(mmgsdi_profile_info_ptr->sin_data);
      }
      else
      {
        goto send_result;
...
/*===========================================================================
FUNCTION MMGSDI_MULTI_PROFILE_VALIDATE_SIN

DESCRIPTION
  Retrieves Security Domain Provider Identification Number (SIN)from card and validates
  it with the configured SIN
...
SIDE EFFECTS
  None.
===========================================================================*/
static mmgsdi_return_enum_type mmgsdi_multi_profile_validate_sin(
...
)
{
  ...
  uim_slot_type              uim_slot       = UIM_SLOT_NONE;
  uint32                     index          = 0;
  mmgsdi_request_header_type req_hdr;

  MMGSDIUTIL_RETURN_IF_NULL(mmgsdi_profile_info_ptr);

  memset(&req_hdr, 0x00, sizeof(req_hdr));

  mmgsdi_status = mmgsdi_util_convert_uim_slot(slot_id,
                                               &uim_slot);
  if (mmgsdi_status != MMGSDI_SUCCESS)
  {
    return mmgsdi_status;
  ...
  if(mmgsdi_profile_info_ptr->get_sin_apdu.data_len > MMGSDI_MAX_CHANNEL_DATA_CMD_LEN &&
     uim_is_extended_apdu_supported(uim_slot) == FALSE)
  {
    return MMGSDI_ERROR;
  }
  ...
  uim_cmd_ptr->stream_iso7816_apdu.num_cmd_bytes =
    uint32touint16(mmgsdi_profile_info_ptr->get_sin_apdu.data_len);

  uim_cmd_ptr->stream_iso7816_apdu.cmd_data = mmgsdi_profile_info_ptr->get_sin_apdu.data_ptr;
  ...
  UIM_MSG_HIGH_0("Sending down Get SIN APDU command to UIM");

  mmgsdi_status = mmgsdi_send_cmd_to_uim_server_synch(uim_cmd_ptr);
  if(mmgsdi_status == MMGSDI_SUCCESS)
  {
    mmgsdi_task_enum_type       mmgsdi_task    = MMGSDI_TASK_MAX;
    mmgsdi_uim_report_rsp_type *uim_rsp_ptr    = NULL;
    mmgsdi_return_enum_type     report_status  = MMGSDI_SUCCESS;

    mmgsdi_task = mmgsdi_util_get_current_task();
    if (MMGSDIUTIL_IS_MMGSDI_TASK(mmgsdi_task)) 
    {
      uim_rsp_ptr = mmgsdi_internal_synch_uim_rpt_rsp[mmgsdi_task];
    }
    ...
    if(uim_rsp_ptr != NULL)
    {
      ...
      if(report_status != MMGSDI_SUCCESS ||
         uim_rsp_ptr->rsp_data.data_len != MMGSDI_MULTI_PROFILE_SIN_DATA_LENGTH + MMGSDI_GEN_SW_LEN ||
         memcmp(uim_rsp_ptr->rsp_data.data_ptr,
                mmgsdi_profile_info_ptr->sin_data,
                MMGSDI_MULTI_PROFILE_SIN_DATA_LENGTH) != 0)
      {
        mmgsdi_status = MMGSDI_ERROR;
      }
  ...
  /* Close session opened for eCASD AID */
  (void)mmgsdi_session_close(mmgsdi_profile_info_ptr->ecasd_session_id,
                             mmgsdi_multi_profile_internal_cb,
                             0);

  mmgsdi_util_free_client_request_table_index(index);
  return mmgsdi_status;
} /* mmgsdi_multi_profile_validate_sin */


/* ==========================================================================
FUNCTION MMGSDI_MULTI_PROFILE_INTERNAL_CB

DESCRIPTION
  This function, defined as mmgsdi_callback_type, will handle the internal
  commands of multi profile functionality
    ...
    case MMGSDI_SESSION_OPEN_REQ:
      ...
      if(cnf_ptr->session_open_cnf.response_header.mmgsdi_status == MMGSDI_SUCCESS &&
         ...
         (0 == memcmp(cnf_ptr->session_open_cnf.app_info.aid.data_ptr,
                 ...
                 mmgsdi_profile_info_ptr->aid.data_len)))
      {
        /* Assume number of profiles as 2 for now */
        mmgsdi_profile_info_ptr->num_profile = 0x02;
        mmgsdi_profile_info_ptr->ecasd_session_id  = cnf_ptr->session_open_cnf.session_id;
        mmgsdi_profile_info_ptr->dedicated_channel_id = cnf_ptr->session_open_cnf.channel_id;

        if(mmgsdi_profile_info_ptr->check_multi_profile_support)
        {
          mmgsdi_status = mmgsdi_multi_profile_validate_sin(cnf_ptr->session_open_cnf.slot_id);
          if(mmgsdi_status != MMGSDI_SUCCESS)
          {
            send_card_inserted = TRUE;
            mmgsdi_multi_profile_update_to_single_profile_info();
        ...
        else if(mmgsdi_profile_info_ptr->dedicated_channel_needed)
        {
          mmgsdi_profile_info_ptr->dedicated_channel_id = cnf_ptr->session_open_cnf.channel_id;
...
/*===========================================================================
FUNCTION MMGSDI_MULTI_PROFILE_SESSION_OPEN

DESCRIPTION
  Opens a non provisioning session on proprietary AID provided in
...
SIDE EFFECTS
  None.
===========================================================================*/
static mmgsdi_return_enum_type mmgsdi_multi_profile_session_open(
  void
)
{
  mmgsdi_return_enum_type       mmgsdi_status    = MMGSDI_SUCCESS;
  ...
  return mmgsdi_status;
} /* mmgsdi_multi_profile_session_open */
...
)
{
  ...
  if(mmgsdi_profile_info_ptr->dedicated_channel_needed)
  {
    mmgsdi_status = mmgsdi_multi_profile_session_open();
    if(mmgsdi_status != MMGSDI_SUCCESS)
    {
  ...
  else
  {
    ...
    if(mmgsdi_profile_info_ptr->reg_profile_detected == FALSE)
    {
      ...
      /* If send_card_inserted is FALSE then 0x2FC8 & 0x2FC9 EFs are present.
         To make sure that card is in regular profile, send switch
         profile command to move to regular profile */
      else if(send_card_inserted == FALSE)
      {
        mmgsdi_status = mmgsdi_set_sim_profile(mmgsdi_generic_data_ptr->client_id,
                                               slot_id,
                                               MMGSDI_PROFILE_1,
