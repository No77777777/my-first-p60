...
Copyright (c) 2013 - 2016, 2020 QUALCOMM Technologies, Inc (QTI) and its licensors.
...
$Header: //components/rel/uim.mpss/6.1.0/mmgsdi/src/mmgsdi_se13.c#3 $$ $DateTime: 2020/02/04 05:35:28 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
01/31/20   sg      Fix special character handling in SE 13
...
04/29/16   vdc     Added feature FEATURE_MMGSDI_SE13_FROM_HARDCODED_ONLY
04/27/16   sp      Fix low level compiler warnings
04/11/16   vdc     Restructure accessing SE13 additional table
01/22/16   bcho    Featurize use of non-gsma entries
10/08/15   av      Remove F3s causing continuous burst of diag prints
...
#define MMGSDI_SE13_SPL_CHAR_INDEX         (10)
...
do_cleanup:
    ...
    MMGSDIUTIL_TMC_MEM_FREE(mmgsdi_se13_file_info.plmn_list_ptr);
...
)
{
  ...
  /* Now get the name length & addresses
    ...
    {
      MMGSDIUTIL_TMC_MEM_FREE(dynamic_names_ptr);
      return MMGSDI_ERROR;
    }
    ...
    UIM_SELECTIVE_MSG_HIGH_0(UIM_LOG_EONS,
                             "PLMN Entry fetched from SE13 present in EFS");
  }
#endif /* FEATURE_MMGSDI_SE13_FROM_HARDCODED_ONLY */
  ...
  {
    MMGSDIUTIL_TMC_MEM_FREE(info_ptr->short_name_ptr);
    MMGSDIUTIL_TMC_MEM_FREE(dynamic_names_ptr);
    return MMGSDI_MEMORY_ERROR_HEAP_EXHAUSTED;
  }
...
)
{
  ...
  MMGSDIUTIL_TMC_MEM_FREE(extra_param_ptr->get_se13_plmn_names_data.plmn_id_list.plmn_list_ptr);
  ...
  mmgsdi_eons_network_info_data_type  * info_ptr)
{
  ...
  if(is_match_found == FALSE && is_mcc_mnc_found)
  {
    ...
    else
    {
      ...
      if(match_index > 0 &&
         (continue_search ||
          mmgsdi_nv_get_device_feature_status(MMGSDI_FEATURE_SE13_TABLE_LOOKUP_GET_FIRST_GSMA_MATCH) ==
             ...
             (mnc ==  mmgsdi_se13_table_get_mnc(index)))
          {
            if(mmgsdi_se13_table_is_gsma_entry(index))
            {
              ...
              if(is_single_gsma_entry)
              {
                ...
                if(mmgsdi_nv_get_device_feature_status(MMGSDI_FEATURE_SE13_TABLE_LOOKUP_GET_FIRST_GSMA_MATCH) ==
      ...
      else if(mmgsdi_nv_get_device_feature_status(MMGSDI_FEATURE_SE13_TABLE_LOOKUP_GET_FIRST_GSMA_MATCH) ==
              MMGSDI_FEATURE_ENABLED)
      {
  ...
  if(is_match_found)
  {
    ...
    /* Don't use non-gsma entry if feature use_non_gsma_entry is disabled */
    if((mmgsdi_nv_get_device_feature_status(MMGSDI_FEATURE_SE13_TABLE_LOOKUP_USE_NON_GSMA_ENTRY) ==
        MMGSDI_FEATURE_ENABLED) ||
        mmgsdi_se13_table_is_gsma_entry(index) != FALSE)
    {
...
)
{
  ...
  /* In SE 13 table, for MCC MNC 286 02 , actual plmn name is Vodafone Türkiye.Since 
     ü is not supported, so previously we used to manually convert ü to normal u. But 
     later due to GCF TC failure, we are again converting ASCII u to ü in GSM 7 bit encoding, 
     whose value is 0x7E, so that the plmn name displayed in UI will be Vodafone Türkiye */
  if((eons_data_ptr->data_len > MMGSDI_SE13_SPL_CHAR_INDEX) &&
     (eons_data_ptr->data_len == strlen(name)) &&
     (0 == memcmp(eons_data_ptr->data_ptr, name, eons_data_ptr->data_len)))
  {
    eons_data_ptr->data_ptr[MMGSDI_SE13_SPL_CHAR_INDEX] = 0x7E;
  }
...
)
{
  ...
  for(index = 0; index < num_se13_entries; index++)
  {
    ...
    MMGSDIUTIL_TMC_MEM_FREE(network_info_data.full_name_ptr);
    MMGSDIUTIL_TMC_MEM_FREE(network_info_data.short_name_ptr);
  }
