...
Copyright (c) 2014 - 2021 QUALCOMM Technologies, Inc (QTI) and its licensors.
...
$Header: //components/rel/uim.mpss/6.1.0/mmgsdi/src/mmgsdi_simlock.c#6 $$ $DateTime: 2021/09/16 02:56:13 $
...
03/06/19   vdc     Move SIMLOCK algo message type to heap
...
07/25/16   bcho    Drop older SIMLOCK RUN algo response if new request sent
...
01/16/16   kv      Check for mmgsdi task state before posting simlock evt
01/07/16   ar      Send events if app_state is ILLEGAL and SIM is locked
09/21/15   stv     Send perso evt with status in progress if perso fail due to emergency mode
09/09/15   vv      Set appropriate status/feature, during ready to perso state
09/09/15   vv      Query status in lock and unlock events
...
06/10/15   stv     Send only perso evt to QMI UIM, MMGSDI during unlocked to locked state
...
/* Array of SIMLOCK run algo request token for provisioning sessions */
static uint32 mmgsdi_simlock_token_array[MMGSDI_MAX_PROV_APPS] = {0};
  ...
  boolean                       read_from_card)
{
  ...
  if(mmgsdi_status != MMGSDI_SUCCESS)
  {
    ...
    /* If file is not present in cache, not required to read from card */
    if(read_from_card == FALSE)
    {
      return 0;
    }
    ...
    if((mmgsdi_status != MMGSDI_SUCCESS) ||
       ...
       (file_data_buf.data_len > buffer_size))
    {
      MMGSDIUTIL_TMC_MEM_FREE(file_data_buf.data_ptr);
      return 0;
    }

    data_len = file_data_buf.data_len;
    mmgsdi_memscpy(buffer_ptr, buffer_size, file_data_buf.data_ptr, file_data_buf.data_len);
    MMGSDIUTIL_TMC_MEM_FREE(file_data_buf.data_ptr);
  }
  else if(data_len > buffer_size)
  {
    UIM_MSG_MED_3("Read for file: 0x%x failed, data_len: 0x%x, buffer_size: 0x%x",
                  file, data_len, buffer_size);
    return 0;
  }
...
static mmgsdi_return_enum_type mmgsdi_simlock_build_algo_data(
  ...
  mmgsdi_session_id_type                 trigger_session_id
)
{
  mmgsdi_return_enum_type       mmgsdi_status        = MMGSDI_SUCCESS;
  ...
  mmgsdi_session_type_enum_type trigger_session_type = MMGSDI_MAX_SESSION_TYPE_ENUM;
  mmgsdi_file_enum_type         spn                  = MMGSDI_NO_FILE_ENUM;
  simlock_iccid_file_type       iccid;
  boolean                       read_gid1_from_card  = FALSE;
  boolean                       read_gid2_from_card  = FALSE;
  ...
  (void)mmgsdi_util_get_session_type(trigger_session_id, &trigger_session_type, NULL);
  ...
  for (slot = 0; slot < MMGSDI_MAX_NUM_SLOTS && slot < SIMLOCK_SLOT_COUNT_MAX; slot++)
  {
    ...
    {
      simlock_algo_data_ptr->card_data[slot].is_card_present = FALSE;
      continue;
    }

    /* Even though sim_detection_started variable has been set to TRUE; we
       can't use it here to determine as it may lead to card error/card
       inserted, both scenarios. */
    simlock_algo_data_ptr->card_data[slot].is_card_present = TRUE;
        ...
        app++)
    {
      ...
      switch (mmgsdi_data_slot_ptr[slot]->app_info_pptr[app]->app_data.app_type)
      {
        ...
        case MMGSDI_APP_USIM:
          simlock_algo_data_ptr->card_data[slot].is_gw_app_available = TRUE;
          break;
        case MMGSDI_APP_RUIM:
        case MMGSDI_APP_CSIM:
          simlock_algo_data_ptr->card_data[slot].is_1x_app_available = TRUE;
          break;
        default:
          continue;
      }

      if(mmgsdi_data_slot_ptr[slot]->app_info_pptr[app]->app_selected == FALSE)
      {
        continue;
      }
      ...
      {
        if(mmgsdi_simlock_check_category_status(slot_id, SIMLOCK_CATEGORY_3GPP_CP))
        {
          read_gid1_from_card = TRUE;
          read_gid2_from_card = TRUE;
        }

        if(mmgsdi_simlock_check_category_status(slot_id, SIMLOCK_CATEGORY_3GPP_SP) ||
           mmgsdi_simlock_check_category_status(slot_id, SIMLOCK_CATEGORY_3GPP_NS_SP))
        {
          read_gid1_from_card = TRUE;
        }
      }
      ...
      /* EF-SPN and EF-EHPLMN are only required for 3GPP subscriptions */
      if (simlock_algo_data_ptr->card_data[slot].subscription_data[sub].sub_type == SIMLOCK_SUBSCRIPTION_3GPP)
      {
        ...
        spn_read_len = mmgsdi_simlock_read_binary_file(spn,
                                                       ...
                                                       mmgsdi_simlock_check_category_status(slot_id,
        ...
        /* For EHPLMN lock category, the protocol should be UICC */
        if (mmgsdi_data_slot_ptr[slot]->protocol == MMGSDI_UICC)
        {
          /* Initialize the ehplmn data */
          memset(&simlock_algo_data_ptr->card_data[slot].subscription_data[sub].ehplmn, 0x00, sizeof(simlock_ehplmn_type));

          simlock_algo_data_ptr->card_data[slot].subscription_data[sub].ehplmn.ehplmn_len =
              mmgsdi_simlock_read_binary_file(MMGSDI_USIM_EHPLMN,
                                              session_type,
                                              0,
                                              sizeof(simlock_algo_data_ptr->card_data[slot].subscription_data[sub].ehplmn.ehplmn_data),
                                              simlock_algo_data_ptr->card_data[slot].subscription_data[sub].ehplmn.ehplmn_data,
                                              mmgsdi_simlock_check_category_status(slot_id,
      ...
      session_id_table_ptr->session_id_token[slot][sub].session_id = session_id;

      prov_session_index = mmgsdi_util_get_prov_index_from_session(session_type);

      if(prov_session_index < MMGSDI_MAX_PROV_APPS)
      {
        /* 1. If build algo data function is not invoked during illegal
              subscription/activation/deactivation of session, increment token for all sessions
           2. If build algo data function is invoked during illegal
              subscription/activation/deactivation of a session, increment token for that
           session only */
        if(trigger_session_type == MMGSDI_MAX_SESSION_TYPE_ENUM ||
           trigger_session_type == session_type)
        {
          mmgsdi_simlock_token_array[prov_session_index]++;
        }
        session_id_table_ptr->session_id_token[slot][sub].token = 
          mmgsdi_simlock_token_array[prov_session_index];
      }
  ...
  if(mmgsdi_task_state[MMGSDI_TASK_MAIN] == MMGSDI_TASK_ACTIVE)
  {
    ...
    else if (task_cmd_ptr->cmd.cmd.simlock_evt.event_data.evt_type == SIMLOCK_DEVICE_MODE_EVENT)
    {
      task_cmd_ptr->cmd.cmd.simlock_evt.event_data.message.device_mode.mode
            ...
            task_cmd_ptr->cmd.cmd.simlock_evt.event_data.evt_type == SIMLOCK_DELAY_TIMER_EVENT)
    {
      /* No event data */
    }
    else if(task_cmd_ptr->cmd.cmd.simlock_evt.event_data.evt_type == SIMLOCK_LOCK_EVENT)
    {
      ...
      task_cmd_ptr->cmd.cmd.simlock_evt.event_data.message.lock.num_retries_max
                        = event_data.message.lock.num_retries_max;
      task_cmd_ptr->cmd.cmd.simlock_evt.event_data.message.lock.curr_retries
                        = event_data.message.lock.curr_retries;
    }
...
)
{
  ...
  {
    case SIMLOCK_SUBSCRIPTION_INVALID_REASON_APP_REQUIRED_BUT_NOT_ACTIVE:
      /* As other app is present on card but not activated yet, current app is
         marked as INVALID. It is required to change perso status to IN PROGRESS
         because it may move to PERSO DONE after other app is activated. */
    ...
    case SIMLOCK_SUBSCRIPTION_INVALID_REASON_SLOT_DISABLED:
        /* As slot is disabled in SIMLOCK config, current app is
           marked as INVALID. It is required to change perso status to IN PROGRESS
           because SIMLOCK also queues powering down the slot. */
      *perso_status_ptr = MMGSDI_PERSO_STATUS_GENERIC_FAILURE;
      *feature_ptr      = MMGSDI_MAX_PERSO_FEATURE_ENUM;
      break;
...
)
{
  ...
  {
    app_info_ptr->perso_status = MMGSDI_PERSO_STATUS_IN_PROGRESS;
  }
  ...
  {
    boolean          is_client_internal = FALSE;
    ...
    /* If the client is MMGSDI or QMI UIM then don't send deactivation events.
       Send only perso locked event for these clients.*/
    if(MMGSDIUTIL_IS_MMGSDI_CLIENT(client_index) ||
       MMGSDIUTIL_IS_QMI_UIM_CLIENT(client_index))
    {
      is_client_internal = TRUE;
    }
...
)
{
  ...
  for (slot = 0; slot < MMGSDI_MAX_NUM_SLOTS && slot < SIMLOCK_SLOT_COUNT_MAX; slot++)
  {
    ...
    for (sub = 0; sub < run_algo_data_ptr->card_validity_data[slot].num_subscriptions; sub++)
    {
      ...
      if (simlock_rsp_ptr->user_data_ptr->session_id_token[slot][sub].session_id == MMGSDI_SESSION_ID_ZERO ||
          simlock_rsp_ptr->user_data_ptr->session_id_token[slot][sub].session_id == MMGSDI_INVALID_SESSION_ID)
      {
        continue;
      }

      mmgsdi_status = mmgsdi_util_get_prov_session_info(simlock_rsp_ptr->user_data_ptr->session_id_token[slot][sub].session_id,
                                                        &prov_session_index,
                                                        &slot_id,
                                                        &app_info_ptr);

      if (mmgsdi_status != MMGSDI_SUCCESS || app_info_ptr == NULL ||
          prov_session_index >= MMGSDI_MAX_PROV_APPS)
      {
        continue;
      }

      if(simlock_rsp_ptr->user_data_ptr->session_id_token[slot][sub].token !=
         mmgsdi_simlock_token_array[prov_session_index])
      {
        UIM_MSG_HIGH_1("SIMLOCK run algo response dropped for prov session index 0x%x",
                       prov_session_index);
        continue;
      }
      ...
      notify_info.slot_id = slot_id;
      notify_info.session_id = simlock_rsp_ptr->user_data_ptr->session_id_token[slot][sub].session_id;
      ...
      if (run_algo_data_ptr->card_validity_data[slot].subscription_data[sub].is_subscription_valid)
      {
        ...
        if (halt_subs)
        {
          ...
          app_info_ptr->perso_status = MMGSDI_PERSO_STATUS_WAIT_FOR_EXT_PERSO;
        ...
        /* For a particular application, when its app state is Ready for perso and perso status is
        in progress i.e simlock run algo has been triggered by that app only or perso status
        is locked, then only we move its perso status to done. In this way, we can ensure that for 
        those apps which haven't triggered simlock run algo, it does not move to post perso step */
        else if(app_info_ptr->app_state == MMGSDI_APP_STATE_READY_FOR_PERSO &&
                (app_info_ptr->perso_status == MMGSDI_PERSO_STATUS_IN_PROGRESS ||
                 app_info_ptr->perso_status == MMGSDI_PERSO_STATUS_LOCKED))
        {
      ...
      else
      {
        ...
        perso_feature = app_info_ptr->perso_info.feature;
        ...
        if(app_info_ptr->perso_status == MMGSDI_PERSO_STATUS_IN_PROGRESS)
        {
        ...
        if (app_info_ptr->app_state == MMGSDI_APP_STATE_READY_FOR_PERSO)
        {
          /* Send the perso event only if the SIM is locked
             with a CK. In case the device is in emergency only mode,
             sending the perso event with status as MMGSDI_PERSO_STATUS_IN_PROGRESS */
          ...
          app_info_ptr->perso_info.feature = perso_feature;
        ...
        /* If the app state is current in subscription ready or illegal and the
        ...
        {
          /* Update app perso feature only if it is set. As app perso status
             can not be updated to IN_PROGRESS after reaching SUB READY, perso
             feature should not reset to MAX ENUM */
          if(perso_feature != MMGSDI_MAX_PERSO_FEATURE_ENUM)
          {
            app_info_ptr->perso_info.feature = perso_feature;
          }
          ...
          mmgsdi_state_update_app_state_for_prov_app(
            simlock_rsp_ptr->user_data_ptr->session_id_token[slot][sub].session_id,
...
)
{
  ...
  if(simlock_evt_ptr->status == SIMLOCK_SUCCESS &&
     (simlock_evt_ptr->event_data.evt_type == SIMLOCK_LOCK_EVENT ||
          ...
          simlock_evt_ptr->status == SIMLOCK_MAX_RETRIES_REACHED)
  {
    ...
    uint32                     num_retries_max = 0;
    uint32                     curr_retries    = 0;

    if(simlock_evt_ptr->event_data.evt_type == SIMLOCK_UNLOCK_EVENT)
    {
      slot            = simlock_evt_ptr->event_data.message.unlock.slot;
      category        = simlock_evt_ptr->event_data.message.unlock.category;
      num_retries_max = simlock_evt_ptr->event_data.message.unlock.num_retries_max;
      curr_retries    = simlock_evt_ptr->event_data.message.unlock.curr_retries;
    }
    else if(simlock_evt_ptr->event_data.evt_type == SIMLOCK_LOCK_EVENT)
    {
      slot            = simlock_evt_ptr->event_data.message.lock.slot;
      category        = simlock_evt_ptr->event_data.message.lock.category;
      num_retries_max = simlock_evt_ptr->event_data.message.lock.num_retries_max;
      curr_retries    = simlock_evt_ptr->event_data.message.lock.curr_retries;
    }
    else
    {
      return;
    ...
    if (slot_data_ptr == NULL)
    {
      UIM_MSG_ERR_1("Invalid SIM Lock slot: 0x%x", slot);
      return;
    }
    ...
    for (i = 0; i < MMGSDI_MAX_EXT_APP_INFO; i++)
    {
      ...
      (void)mmgsdi_simlock_convert_category(category,
        &perso_feature);
          ...
          slot_data_ptr->app_info_pptr[i]->perso_info.feature == perso_feature)
      {
        ...
        {
          slot_data_ptr->app_info_pptr[i]->perso_status = MMGSDI_PERSO_STATUS_LOCKED;
        }
        else
        {
          slot_data_ptr->app_info_pptr[i]->perso_status = MMGSDI_PERSO_STATUS_BLOCKED;
        }

        if (num_retries_max == 0)
        {
          slot_data_ptr->app_info_pptr[i]->perso_info.num_retries = MMGSDI_SIMLOCK_MAX_RETRIES;
        }
        else if (curr_retries >= num_retries_max)
        {
        ...
        mmgsdi_evt_build_and_send_perso(
          ...
          slot_data_ptr->app_info_pptr[i]->perso_status,
...
)
{
  ...
  switch (simlock_evt_ptr->event_data.evt_type)
  {
    ...
    case SIMLOCK_DELAY_TIMER_EVENT:
...
DESCRIPTION
  ...
  list so algorithm result will be ignored. session_id indicates the session
  that causes the need to run SIMLOCK engine. An invalid session id is used in
  other cases i.e. SIMLOCK run algo is requested on reception of SIMLOCK events.
...
{
  mmgsdi_return_enum_type                mmgsdi_status           = MMGSDI_SUCCESS;
  mmgsdi_return_enum_type                app_info_status         = MMGSDI_SUCCESS;
  ...
  /* We can come here in 2 cases: one with a valid session id and one with a zero
  session id. When the above api is called with a zero session id, that means
  mmgsdi does not know for which app, simlock algo has to be run. In that case,
  the below api will return error. But we are not returning from here because 
  simlock algo has to be run in those cases also */
  app_info_status = mmgsdi_util_get_prov_session_info(session_id,
                                                      NULL,
                                                      NULL,
                                                      &app_info_ptr);
      ...
      ignore_result)
  {
    ...
    /* if app_info_status is not success, that means we came here for a zero
    session id. In that case, there is no corresponding app for zero session id
    and hence we don't update the perso status of app in this case */
    if(simlock_status == SIMLOCK_SUCCESS && app_info_status == MMGSDI_SUCCESS && app_info_ptr != NULL)
    {
      app_info_ptr->perso_status = MMGSDI_PERSO_STATUS_IN_PROGRESS;
    }
