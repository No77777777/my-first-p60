...
$Header: //components/rel/uim.mpss/6.1.0/mmgsdi/src/mmgsdisessionlib_3gpp2.c#1 $$ $DateTime: 2019/10/30 06:56:05 $ $Author: pwbldsvc $
...
)
{
  ...
  -----------------------------------------------*/
  /* randseed & response_cb_ptr checking */
  MMGSDIUTIL_RETURN_IF_OUT_OF_RANGE(randseed.data_len,1, (UIM_RAND_SEED +1));
  MMGSDIUTIL_RETURN_IF_NULL_2(randseed.data_ptr,
                              response_cb_ptr);

  if(a_key_p_rev <= MMGSDI_A_KEY_P_REV)
  {
    /* param_p , param_g checking */
    ...
    MMGSDIUTIL_RETURN_IF_NULL_2(param_p.data_ptr,
                                param_g.data_ptr);
  }
  else
  {
  ...
  if(a_key_p_rev <= MMGSDI_A_KEY_P_REV)
  {
    mmgsdi_memscpy(msg_ptr->param_p.data_ptr,
                   msg_ptr->param_p.data_len,
                   param_p.data_ptr,
                   param_p.data_len);

    mmgsdi_memscpy(msg_ptr->param_g.data_ptr,
                   msg_ptr->param_g.data_len,
                   param_g.data_ptr,
                   param_g.data_len);
  }
...
)
{
  ...
  /*---------------------------------------------
    ...
    4) bs_result            Data length shall be in range of 1 to UIM_PARAM_P
                            Data ptr shall not be NULL.
  -----------------------------------------------*/
  /* bs_result, response_cb_ptr checking */
  MMGSDIUTIL_RETURN_IF_OUT_OF_RANGE(bs_result.data_len,1, (UIM_PARAM_P + 1));
  MMGSDIUTIL_RETURN_IF_NULL_2(bs_result.data_ptr,
                              response_cb_ptr);
  ...
  /*------------------------------------------------------------------------
   - Allocating memory to Task Command Pointer
   -------------------------------------------------------------------------*/
  MMGSDIUTIL_MEM_MALLOC_AND_VALIDATE(task_cmd_ptr, sizeof(mmgsdi_task_cmd_type) + temp_mem);
  if (task_cmd_ptr == NULL)
  {
    return MMGSDI_MEMORY_ERROR_HEAP_EXHAUSTED;
  }
...
)
{
  ...
  /* If prl data is available in cache and is valid then return data in caller's context */
  mmgsdi_status = mmgsdi_caller_context_read_prl_and_notify_client(task_cmd_ptr);

  if(mmgsdi_status == MMGSDI_SUCCESS)
  {
    /* If Prl read was successful, Free the command pointer and return the status */
    MMGSDIUTIL_TMC_MEM_FREE(task_cmd_ptr);
    return mmgsdi_status;
  }
...
PARAMETERS:
  ...
  mmgsdi_session   : session for retrieving NVRUIM data
...
)
{
  ...
  nvruim_session = mmgsdi_util_map_mmgsdi_session_to_nvruim_session(mmgsdi_session);

  if (nvruim_session != NVRUIM_1X_PROV_PRI_SESSION &&
  ...
  switch(nvruim_data_ptr->req_type)
  {
    ...
    case MMGSDI_NVRUIM_GET_3GPD_CTRL_AND_ESN_USG_DATA_REQ:
         ...
         MMGSDI_NVRUIM_GET_3GPD_CTRL_AND_ESN_USG_DATA_REQ)
      {
        ...
        nvruim_3gpd_stat = uim_3gpd_support_per_sub(as_id);
    ...
    case MMGSDI_NVRUIM_GET_HRPD_CONTROL_REQ:
      an_hrpd_status = uim_an_hrpd_support_per_sub(as_id);
