...
Copyright (c) 2001-2015, 2020 QUALCOMM Technologies, Inc (QTI) and its licensors.
...
$Header: //components/rel/uim.mpss/6.1.0/mmgsdi/src/mmgsdi_efs.c#3 $$ $DateTime: 2020/03/04 22:50:23 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
03/03/20   tq      Added EFS error number in F3s
12/04/15   bcho    file size should not be greater than input buffer size
...
)
{
  ...
  {
    UIM_MSG_ERR_2("EFS File or Dir Not Found, efs_errno: 0x%x, context 0x%x", efs_errno, context);
...
)
{
  ...
  if(mcfg_fs_stat((const char *)file_handle_ptr, &file_stat,
         ...
         != MCFG_FS_STATUS_OK)
  {
    UIM_MSG_ERR_3("efs stat procedure failed:file_stat.st_size 0x%x, efs_errno: 0x%x, context 0x%x",
                   file_stat.st_size,
                   efs_errno,
                   context);
    return MMGSDI_ERROR;
  ...
  {
    UIM_MSG_ERR_2("efs open Procedure Failed efs_errno: 0x%x, context 0x%x",
                  efs_errno,
                  context);
    return MMGSDI_ERROR;
  }
  ...
  {
    UIM_MSG_ERR_2("efs read procedure failed, efs_errno: 0x%x, file_size 0x%x",
                  efs_errno,
                  file_size);
    return MMGSDI_ERROR;
  }
  ...
  *data_buffer_len_ptr = file_size;
  ...
  {
      UIM_MSG_ERR_2("efs close procedure failed, fd: 0x%x, efs_errno: 0x%x",
                    file_descriptor,
                    efs_errno);
  }
...
)
{
  ...
  {
    UIM_MSG_ERR_2("efs stat procedure failed, efs_errno: 0x%x, context 0x%x", efs_errno, context);
    return MMGSDI_ERROR;
  }
} /* gsdi_efs_name_test*/
...
)
{
  ...
  {
    UIM_MSG_ERR_1("efs mkdir procedure failed, efs_errno: 0x%x", efs_errno);
    return MMGSDI_ERROR;
  }
...
)
{
  ...
  if(file_descriptor < 0)
  {
    UIM_MSG_ERR_2("efs open Procedure Failed, efs_errno: 0x%x, context 0x%x", efs_errno, context);
    return MMGSDI_ERROR;
  }
  ...
  {
    UIM_MSG_ERR_2("efs write failed, efs_errno: 0x%x, bytes_to_write 0x%x", efs_errno, bytes_to_write);
  ...
  if(mcfg_fclose(file_descriptor, MCFG_FS_TYPE_EFS) != 0)
  {
    UIM_MSG_ERR_2("efs close procedure failed, efs_errno: 0x%x, file_descriptor 0x%x", efs_errno, file_descriptor);
  }
...
)
{
  ...
  if(mcfg_fs_delete(file_handle_ptr, MCFG_FS_TYPE_EFS,
         ...
         != MCFG_FS_STATUS_OK)
  {
    UIM_MSG_ERR_2("Failure to remove file, efs_errno: 0x%x, context 0x%x", efs_errno, context);
    return MMGSDI_ERROR;
  }
