...
$Header: //components/rel/uim.mpss/6.1.0/mmgsdi/src/mmgsdi_evt.c#3 $ $DateTime: 2020/02/12 00:43:00 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
02/12/20   vdc     Allow sending card error event when error cause is voltage mismatch
09/17/19   mm      Add EID to SimSlotStatus radio config HAL
06/03/19   kb      Support for card reset on refresh failure 
04/19/19   tq      Clear telecom attr cache before sending FDN event
...
10/20/15   vdc     Move event info type to heap while sending ECC event
...
07/14/15   vv      Fix the perso status in perso evt sent based on app state
06/11/15   lm      Remove redundant code in slot index validation
05/13/15   av      Fix compilation warnings
05/06/15   vdc     send card error if prev card error was due to SAP connect
  ...
  mmgsdi_event_data_type    *evt_info_ptr)
{
      ...
      (app_info_ptr->app_data.aid.data_len <= MMGSDI_MAX_AID_LEN))
  {
    ...
    evt_info_ptr->data.session_close.app_id.data_len = app_info_ptr->app_data.aid.data_len;
  ...
  mmgsdi_event_data_type *event_info_ptr)
{
  ...
  switch(event_info_ptr->evt)
  {
    case MMGSDI_SESSION_CLOSE_EVT:
      MMGSDIUTIL_TMC_MEM_FREE(event_info_ptr->data.session_close.app_id.data_ptr);
      return;
    case MMGSDI_FDN_EVT:
      MMGSDIUTIL_TMC_MEM_FREE(event_info_ptr->data.fdn.rec_data.data_ptr);
      return;
    ...
    case MMGSDI_FILE_UPDATE_EVT:
      MMGSDIUTIL_TMC_MEM_FREE(event_info_ptr->data.file_update.file_data.data_ptr);
      return;
    case MMGSDI_ECC_EVT:
      if(event_info_ptr->data.ecc.ecc_data)
      {
        ...
        {
          MMGSDIUTIL_TMC_MEM_FREE(event_info_ptr->data.ecc.ecc_data[i].data.data_ptr);
        }
        MMGSDIUTIL_TMC_MEM_FREE(event_info_ptr->data.ecc.ecc_data);
      }
      return;
  }
  ...
  mmgsdi_event_data_type *event_info_ptr)
{
  ...
  if((mmgsdi_status != MMGSDI_SUCCESS) || (index >= MMGSDI_MAX_CLIENT_INFO) ||
     mmgsdi_client_id_reg_table[index] == NULL)
  {
    return;
  }
  ...
  if (mmgsdi_client_id_reg_table[index]->is_client_evt_reg_cnf_sent == TRUE)
  {
    ...
    {
      /* Traces */
      UIM_MSG_HIGH_9("Sending event 0x%x to client - id_low = 0x%x, id_high = 0x%x, Callback = 0x%x, name = %c%c%c%c%c",
                     event_info_ptr->evt,
  ...
  mmgsdi_event_data_type   *event_info_ptr)
{
      ...
      (mmgsdi_client_id_reg_table[client_index]->session_info_ptr[session_index]->session_evt_cb_ptr != NULL))
  {
    ...
    if(session_ptr->is_session_open_cnf_sent == TRUE )
    {
      ...
      /* Traces */
      UIM_MSG_HIGH_9("Sending event 0x%x to client - id_low = 0x%x, session - id_low = 0x%x, cb = 0x%x, name = %c%c%c%c%c",
                     event_info_ptr->evt,
  ...
  uint32                              client_id_table_index)
{
  ...
  switch (slot_data_ptr->mmgsdi_state)
  {
    ...
    case MMGSDI_STATE_ENUM_MAX:
      /* Detection started event is sent to late registered client only
         if we are not in no card or inserted state */
      if (slot_data_ptr->sim_detection_started)
      {
        mmgsdi_evt_build_and_send_sim_detection_started(
          FALSE,
          mmgsdi_client_id_reg_table[client_id_table_index]->client_id,
          slot);
      }
  ...
  mmgsdi_session_id_type   session_id)
{
  ...
  switch(app_info_ptr->app_state)
  {
    ...
    case MMGSDI_APP_STATE_EXT_PERSO_FAILED:
      ...
      {
        app_info_ptr->perso_status = MMGSDI_PERSO_STATUS_GENERIC_FAILURE;
      }
  ...
  mmgsdi_slot_id_enum_type slot)
{
  ...
  /* Indicate if the card type is TELECOM or NON-TELECOM */
  evt_info_ptr->data.card_inserted.card_type = slot_data_ptr->card_type;
  ...
  {
    evt_info_ptr->data.card_inserted.eid_len = slot_data_ptr->eid_info.length;
	mmgsdi_memscpy(evt_info_ptr->data.card_inserted.eid,
	               sizeof(evt_info_ptr->data.card_inserted.eid),
  ...
  mmgsdi_card_err_info_enum_type prev_err_cause)
{
 ...
 /* --------------------------------------------------------------------------
    ...
    (4)Send CARD_ERROR event when SIM detection has started
    (5)Send CARD_ERROR event when previous card error was due to SAP connect
    (6)Send CARD_ERROR event when error is due to SAP connection
    (7)Send CARD_ERROR event when error is due to voltage mismatch
    ------------------------------------------------------------------------*/
  if(notify_all)
  {
    ...
    if(((prev_err_cause != MMGSDI_CARD_ERR_PWR_DN_CMD_NOTIFY) ||
       ...
       (slot_data_ptr->card_error_condition != MMGSDI_CARD_ERR_VOLT_MISMATCH) &&
  ...
  mmgsdi_slot_id_enum_type       slot)
{
  ...
  {
    if (mmgsdi_client_id_reg_table[i] == NULL ||
        (!notify_all && mmgsdi_client_id_reg_table[i]->client_id != client_id))
    {
      continue;
    }

    if (MMGSDIUTIL_IS_MMGSDI_CLIENT(i) ||
        MMGSDIUTIL_IS_QMI_UIM_CLIENT(i))
    {
      mmgsdi_evt_build_and_send_card_inserted(FALSE,
                        mmgsdi_client_id_reg_table[i]->client_id,
                        slot);
    }
    else
    {
      mmgsdi_evt_build_and_send_card_error(FALSE,
                              mmgsdi_client_id_reg_table[i]->client_id,
                              slot,
                              MMGSDI_CARD_ERR_UNKNOWN_ERROR);
    }
  ...
  const mmgsdi_int_app_info_type   *app_info_ptr)
{
  ...
  if (slot_data_ptr != NULL)
  {
    evt_info_ptr->data.subscription_ready.app_info.slot_type = slot_data_ptr->slot_type;
  }
  ...
  const mmgsdi_int_app_info_type *app_info_ptr)
{
  ...
  if (slot_data_ptr != NULL)
  {
    evt_info_ptr->data.session_changed.app_info.slot_type = slot_data_ptr->slot_type;
  }
  ...
  mmgsdi_fdn_evt_source_enum_type   fdn_source)
{
  ...
  mmgsdi_return_enum_type   mmgsdi_status = MMGSDI_SUCCESS;
  mmgsdi_slot_id_enum_type slot_id       = MMGSDI_MAX_SLOT_ID_ENUM;
  mmgsdi_file_enum_type     file_list[]   = {MMGSDI_TELECOM_ADN,
                                             MMGSDI_TELECOM_FDN, 
                                             MMGSDI_TELECOM_EXT1, 
                                             MMGSDI_TELECOM_EXT2};
  mmgsdi_path_type          file_path     = {0,{0}};
  ...
  /* We are clearing attr cache here becuase during FDN enbale and disable,
     ADN invalidate and rehabilitated so its attr got updated*/
  if(fdn_source == MMGSDI_FDN_SOURCE_USER)
  {
    uint8 index = 0;

    for(index = 0; index < sizeof(file_list)/sizeof(file_list[0]); index++)
    {
      ...
      (void)mmgsdi_cache_delete_attr_selective_items(mmgsdi_cache_common_attr_cache_ptr(slot_id),
                                                     ...
                                                     &file_list[index],
                                                     1);
      if( mmgsdi_file_get_path_from_file(file_list[index],
                                         &file_path) == MMGSDI_SUCCESS)
      {
        /* Clear the attr cache also by path because it may possible we have 
           cached attr of file by path and by enum both */
  ...
  const mmgsdi_int_app_info_type *app_info_ptr)
{
  ...
  if (slot_data_ptr != NULL)
  {
    evt_info_ptr->data.legal_session.app_info.slot_type = slot_data_ptr->slot_type;
  }
  ...
  mmgsdi_evt_session_notify_type  notify_type)
{
 ...
 /* Get full ECC data from cache */
  ...
  MMGSDIUTIL_TMC_MEM_FREE(ecc_cache_data.data_ptr);
  mmgsdi_evt_free_data(evt_info_ptr);
  MMGSDIUTIL_TMC_MEM_FREE(evt_info_ptr);
  MMGSDIUTIL_TMC_MEM_FREE(invalid_data_ptr);
  ...
  mmgsdi_slot_id_enum_type slot)
{
  ...
  /* populate the evt type and slot */
  evt_info_ptr->evt = MMGSDI_CARD_DETECTION_STARTED_EVT;
  evt_info_ptr->data.card_detection.slot = slot;
  ...
  mmgsdi_slot_id_enum_type slot)
{
  ...
  /* populate the evt type and slot */
  evt_info_ptr->evt = MMGSDI_RECOVERY_COMPLETE_EVT;
  evt_info_ptr->data.recovery_complete.slot = slot;
  ...
  mmgsdi_evt_free_data(evt_info_ptr);
  MMGSDIUTIL_TMC_MEM_FREE(evt_info_ptr);
} /* mmgsdi_evt_build_and_send_recovery_complete */
  ...
  mmgsdi_slot_id_enum_type                slot)
{
  ...
  /* populate the evt type */
  evt_info_ptr->evt                                              = MMGSDI_PURGE_TEMPORARY_IDENTITY_STATUS_EVT;
  evt_info_ptr->data.purge_temporary_identity.temporary_identity = temporary_identity;
  evt_info_ptr->data.purge_temporary_identity.status             = mmgsdi_status;
  ...
  mmgsdi_evt_free_data(evt_info_ptr);
  MMGSDIUTIL_TMC_MEM_FREE(evt_info_ptr);
} /* mmgsdi_evt_build_and_send_purge_temporary_identity_status */
...
} /* mmgsdi_evt_build_and_send_sap_disconnected */
