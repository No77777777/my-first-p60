...
$Header: //components/rel/uim.mpss/6.1.0/mmgsdi/src/mmgsdi_eons_opl_pnn.c#2 $$ $DateTime: 2019/10/30 06:56:05 $

when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
09/10/19   vdc     Do not stop OPL PNN caching if OPL record is already cached
03/01/19   tq      Skip get file attributes if EF-OPL/PNN are found in persistent cache   
10/17/18   vdc     Enter critical section before updating/deleting OPL/PNN
                   cache
05/24/17   bcho    Even if EF-OPL caching fails, continue EF-PNN caching
03/14/17   nc      Better checks for return values 
11/17/16   ar      Leave APP_DATA_CRITICAL_SECTION when sending cmd to UIMDRV
...
01/27/16   bcho    Provide encoding byte from EF-PNN record's additional info
...
08/21/15   vdc     Free memory for OPL/PNN read confirmation pointer
08/13/15   sp      merging mmgsdiutil_mem_free macros into one
05/26/15   ar      Cache PNN file if OPL service is Disabled
04/29/15   bcho    Support for App specific cache
03/05/15   ar      Avoid send READ to card for record more than total records
03/04/15   ar      Call mmgsdi_uim_u/icc_read_record for PNN based on protocol
...
)
{
     ...
     int_app_info_ptr != NULL)
  {
    ...
    {
      *opl_pnn_cache_pptr = &int_app_info_ptr->opl_cache;
    }
    else if(cache_type == MMGSDI_EONS_CACHE_PNN)
    {
      *opl_pnn_cache_pptr = &int_app_info_ptr->pnn_cache;
    }
    else
    {
  ...
  do
  {
    ...
    if (opl_cache_ptr->record_list_ptr != NULL)
    {
      ...
      for(i = 0; i < opl_cache_ptr->record_ef_info.valid_record_list.data_len; i++)
      {
        ...
        valid_pnn_rec = opl_cache_ptr->record_list_ptr[i].data_ptr[MMGSDI_EONS_PNN_RECORD_NUM_POS];
  ...
  const mmgsdi_cnf_type  * cnf_ptr)
{
  ...
  UIM_SELECTIVE_MSG_HIGH_3(UIM_LOG_EONS,
                           "pnn read record cb status: 0x%x cnf: 0x%x access: 0x%x",
  ...
  if(cnf_ptr->read_cnf.response_header.client_data != cache_ref_cnt)
  {
    UIM_SELECTIVE_MSG_ERR_2(UIM_LOG_EONS,
                            "cache ref counf 0x%x does not match with client data 0x%x",
                            cache_ref_cnt,
  ...
  const mmgsdi_cnf_type  * cnf_ptr)
{
  ...
  UIM_SELECTIVE_MSG_HIGH_2(UIM_LOG_EONS,
                           "opl read record cb status: 0x%x cnf: 0x%x",
                           status, cnf);
  ...
  {
    UIM_SELECTIVE_MSG_ERR_2(UIM_LOG_EONS,
                            "get next valid record number for file 0x%x failed status: 0x%x",
                            file_enum, mmgsdi_status);

    /* Even if EF-OPL caching fails, continue EF-PNN caching */
...
)
{
  ...
  UIM_SELECTIVE_MSG_HIGH_2(UIM_LOG_EONS,
                           "get file attr file enum: 0x%x session id: 0x%x",
...
)
{
  ...
  if(file_enum_opl == MMGSDI_USIM_OPL)
  {
    ...
    {
      UIM_SELECTIVE_MSG_ERR_1(UIM_LOG_EONS,
                              "get opl uicc search failed status : 0x%x",
                              mmgsdi_status);
      mmgsdi_eons_send_cache_ready_event(session_id);
      return mmgsdi_status;
    }
  ...
  else
  {
    ...
    {
      UIM_SELECTIVE_MSG_ERR_1(UIM_LOG_EONS,
                              "update valid record info failed status: 0x%x",
                              mmgsdi_status);
  ...
  do
  {
       ...
       opl_pnn_cache_ptr->record_list_ptr != NULL)
    {
         ...
         opl_pnn_cache_ptr->record_list_ptr[loop_cnt].data_ptr != NULL)
      {
        ...
        if(data_len > 0 && data_len  <= opl_pnn_cache_ptr->record_ef_info.rec_len)
        {
        ...
        temp_data_len = record_data_ptr->data_len;
    ...
    /* Requested OPL/PNN record is not present in cache.
    ...
    {
      /* Return error if request is from client context */
      if(mmgsdi_util_get_current_task() >= MMGSDI_NUM_TASKS ||
         mmgsdi_util_get_current_task() == MMGSDI_TASK_MAIN)
      {
        mmgsdi_status = MMGSDI_ERROR;
        break;
      }

      /* Setting the OPL-PNN pointer to NULL as we are leaving the critical section.
         So, the pointer is no more valid */
      opl_pnn_cache_ptr = NULL;

      /* Leave Critical section before sending command to UIMDRV */
      MMGSDIUTIL_LEAVE_CLIENT_APP_DATA_CRIT_SECT;
      ...
      /* Take Critical section again after comig out of wait */
      MMGSDIUTIL_ENTER_CLIENT_APP_DATA_CRIT_SECT;

      if (mmgsdi_eons_opl_pnn_get_cache_ptr(session_id, cache_type, &opl_pnn_cache_ptr) != MMGSDI_SUCCESS ||
          opl_pnn_cache_ptr == NULL ||
    ...
    /* If record is not valid then send 0xFF data in response */
    else
    {
...
/*===========================================================================
FUNCTION MMGSDI_EONS_OPL_PNN_DELETE

DESCRIPTION
  Function to delete OPL/PNN cache
...
SIDE EFFECTS
  None
===========================================================================*/
void mmgsdi_eons_opl_pnn_delete(
  mmgsdi_cache_record_ef_data_type  *opl_pnn_cache_ptr
)
{
  int32  loop_cnt = 0;

  if(opl_pnn_cache_ptr == NULL)
  {
    return;
  }
...
)
{
  ...
  if(opl_pnn_cache_ptr != NULL && mmgsdi_status == MMGSDI_SUCCESS)
  {
    mmgsdi_eons_opl_pnn_delete(opl_pnn_cache_ptr);
  }
  return mmgsdi_status;
...
)
{
  ...
  for (; loop_cnt <= last_valid_rec_pos; loop_cnt++)
  {
    ...
    {
      /* If record is already cached or not found, then proceed with caching of next
         record */
      if(opl_pnn_cache_ptr->record_list_ptr[loop_cnt].init != MMGSDI_CACHE_NOT_INIT &&
         opl_pnn_cache_ptr->record_list_ptr[loop_cnt].data_ptr != NULL)
      {
...
)
{
  ...
  if(mmgsdi_status == MMGSDI_SUCCESS && int_app_info_ptr != NULL)
  {
    ...
    opl_cache_ptr = &int_app_info_ptr->opl_cache;
    pnn_cache_ptr = &int_app_info_ptr->pnn_cache;
  ...
  mmgsdi_plmn_info_type                   * plmn_info_ptr)
{
     ...
     (pnn_record.additional_info_ptr != NULL))
  {
        ...
        encoding == MMGSDI_UCS2_ENCODING_TYPE_3)
    {
      ...
      mmgsdi_len_type data_len = pnn_record.additional_info_length;
      MMGSDIUTIL_MEM_MALLOC_AND_VALIDATE(
      ...
      if(plmn_info_ptr->plmn_additional_info.info_data.data_ptr != NULL)
      {
        ...
        mmgsdi_memscpy(plmn_info_ptr->plmn_additional_info.info_data.data_ptr,
                       ...
                       &(pnn_record.additional_info_ptr[MMGSDI_EONS_PNN_ADD_INFO_CODING_POS]),
                       pnn_record.additional_info_length);
      }
    }
    else
...
)
{
  ...
  opl_cache_ptr = &app_info_ptr->opl_cache;
  pnn_cache_ptr = &app_info_ptr->pnn_cache;
       ...
       opl_cache_ptr->init == MMGSDI_CACHE_NOT_FOUND)
    {
      if(is_match_found == FALSE)
      {
        ...
        else if(app_info_ptr->app_data.app_type == MMGSDI_APP_USIM)
        {
          ...
          MMGSDIUTIL_TMC_MEM_FREE(ehplmn_data.data_ptr);
        }
      }
    }
...
)
{
     ...
     opl_cache_ptr->record_list_ptr != NULL)
  {
        ...
        loop_cnt++)
    {
      ...
      /* If cache is not found, EONS algo run should not be interrupted because
	     operator name can be retrieved from other sources i.e. NITZ, CPHS-ONS and
		 SE13 table*/
	  if(opl_cache_ptr->record_list_ptr[loop_cnt].init == MMGSDI_CACHE_NOT_FOUND)
      {
        break;
      }
    }
