...
03/02/17   sk      Fix KW errors
12/23/16   bcho    EONS selective logging
12/19/16   av      Consider sys_mode when copying lac from SS event for 2nd stack
...
02/16/15   ar      Use mm_info RAT for NITZ when Mobility Mask is SET
...
SIDE EFFECTS
  None
===========================================================================*/
static uint32 mmgsdi_eons_compute_aligned_ss_plmn_name_len(
  mmgsdi_eons_ss_data_type *ss_info_ptr
)
{
  uint32 aligned_plmn_name_len = 0;
  uint32 var_len               = 0;
...
)
{
  ...
  UIM_SELECTIVE_MSG_HIGH_2(UIM_LOG_EONS, "Received csg id 0x%x, len: 0x%x",
                           csg_info_ptr->csg_id,
...
)
{
  ...
  */
  ...
  /* Initialize the CSG ID */
  nitz_info_ptr->csg_info.csg_id = MMGSDI_EONS_INVALID_CSG_ID;
  if (csg_info_ptr != NULL)
  {
  ...
  if(mm_ss_info_ptr->asubs_id == SYS_MODEM_AS_ID_1)
  {
...
#if CM_NO_STACKS > 1
    /* Following configurations are all supported where a 2nd stack related
       events is processed for the primary subscription:
       1. SGLTE (where only G can be present on second stack, in which case
          we copy over LAC from the CM SS event.
       2. SRLTE (where either G or LTE may be present on second stack. If it
          is LTE that is on second stack, we copy over the LTE TAC; If it is
          G that is on second stack, we copy over the LAC from CM SS event */
       ...
       MMGSDI_SRV_STATUS_CHANGED(mm_ss_info_ptr->stack_info[1].srv_status))
    {
      ...
      mmgsdi_eons_copy_ss_event_rplmn_lac_tac_data(
        sys_id_ptr,
        &(ss_info.sec_stack_ss.nitz_info),
        ss_info.sec_stack_ss.sys_mode,
        (ss_info.sec_stack_ss.sys_mode == SYS_SYS_MODE_LTE) ?
         mm_ss_info_ptr->stack_info[1].rat_mode_info.lte_mode_info.tac:
        sys_id_ptr->id.plmn_lac.lac);
