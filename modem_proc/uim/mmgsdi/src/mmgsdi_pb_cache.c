...
$Header: //components/rel/uim.mpss/4.0/mmgsdi/src/mmgsdi_pb_cache.c

when       who     what, where, why
--------   ---     ----------------------------------------------------------
05/07/18   rps     Retrieving attribute cache from caller context
12/13/16   bcho    Remove global phonebook cache if Telecom EF-ADN in FCN list
...
01/21/16   nr      Reducing heap memory by reducing no of allocations
...
)
{
  ...
  if(mmgsdi_nv_get_slot_feature_status(MMGSDI_FEATURE_SKIP_SEARCH_FOR_PB_FILES, nv_context) == MMGSDI_FEATURE_ENABLED)
  {
...
)
{
  ...
  MMGSDIUTIL_MEM_MALLOC_AND_VALIDATE(pb_record_node_ptr,
                                     sizeof(mmgsdi_pb_record_element_type) + cnf_ptr->read_cnf.read_data.data_len - sizeof(pb_record_node_ptr->buffer_first_byte));
  ...
  pb_record_node_ptr->record_data.data_len = cnf_ptr->read_cnf.read_data.data_len;

  pb_record_node_ptr->record_data.data_ptr = &pb_record_node_ptr->buffer_first_byte;
...
)
{
  ...
  while(pb_record_element_ptr != NULL)
  {
    ...
    MMGSDIUTIL_TMC_MEM_FREE(pb_record_element_ptr);
...
DESCRIPTION
  This function frees the Phonebook data for the phonebook that are refreshed
  currently.
  If Refresh FCN happens on Telecom ADN, free card session phonebook data as
  Telecom ADN is mapped by card with first ADN present in PBR.
  Note that EF-PBR is part of Phonebook DF which is under Telecom DF.
...
)
{
  ...
  num_files = refresh_files_ptr->num_file_paths;

  for(loop_cnt = 0; loop_cnt < num_files; loop_cnt++)
  {
    if(mmgsdi_file_is_ef_under_global_telecom_df(&refresh_files_ptr->file_path_ptr[loop_cnt]))
    {
      /* Use card slot session for accesses outside GSM/RUIM DF or USIM/CSIM ADF. */
      session_id = mmgsdi_generic_data_ptr->slot_session_id[slot_index];

      /* If Telecom ADN is in FCN list, remove phonebook cache for
         that card session. Telecom EF-ADN is internally mapped with
         first EF-ADN present in EF-PBR. Instead of identifying that particular
         EF-ADN in DF Phonebook, we remove whole global phonebook cache */
      if(refresh_files_ptr->file_path_ptr[loop_cnt].path_len == 3 &&
         refresh_files_ptr->file_path_ptr[loop_cnt].path_buf[2] == GSDI_ADN_EF_PATH_USED)
      {
        mmgsdi_pb_free_cache_data(slot, session_id, FALSE);
        continue;
      }
    }
    else
    {
      ...
      if(mmgsdi_util_get_prov_session_id_for_app(slot, aid_ptr, &session_id) != MMGSDI_SUCCESS)
      {
        continue;
      }
    }
    mmgsdi_pb_free_ef_cache (refresh_files_ptr->file_path_ptr[loop_cnt], session_id, slot);
  }
