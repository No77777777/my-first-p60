...
Copyright (c) 2004 - 2017, 2019 - 2020 QUALCOMM Technologies, Inc (QTI) and its licensors.
...
$Header: //components/rel/uim.mpss/6.1.0/mmgsdi/src/mmgsdilib.c#4 $$ $DateTime: 2020/02/17 23:22:31 $

when       who     what, where, whyf
--------   ---     ----------------------------------------------------------
...
06/07/19   vgd     Support to disable conversion of APDU to high level mmgsdi command 
06/16/17   bcho    Align short name pointer properly
...
04/19/17   nc      Expose public API that returns current CSIM hiding status  
...
12/09/16   bcho    Allow MMGSDI_SLOT_NONE in mmgsdi_map_sessions_to_slots()
...
11/24/15   vdc     Support of SAP in server mode on slot2
...
08/19/15   ks      Exposing mmgsdi API which returns number of hw slots enabled
...
05/13/15   yt      Support for mapping sessions to slots
...
02/16/15   vdc     Send REFRESH source as ECALL if profile switch is REG to EM
01/07/15   vdc     Closed pending comments for handling multi profile cards
01/06/15   at      Correcting the check of incoming SAP APDU data length
...
)
{
  ...
  MMGSDIUTIL_RETURN_IF_OUT_OF_RANGE(pup_option,
                                    ...
                                    MMGSDI_CARD_POWER_UP_NON_TELECOM);
  ...
  do
  {
    ...
    if(slot_data_ptr != NULL &&
       ...
       mmgsdi_nv_get_device_feature_status(MMGSDI_FEATURE_CARD_SLOT_CHECK) == MMGSDI_FEATURE_ENABLED)
    {
  ...
  if (pup_option == MMGSDI_CARD_POWER_UP_INITIAL_PUP ||
      pup_option == MMGSDI_CARD_POWER_UP_NON_TELECOM)
  {
    ...
    refresh_req.refresh.card_req.pup_option   = pup_option;
...
)
{
  ...
  UIM_MSG_HIGH_0("In mmgsdi_get_atr");
  ...
  msg_ptr = &task_cmd_ptr->cmd.cmd.get_atr_req;
    ...
    /* Populating mmgsdi_get_atr_req_type header. */
  ...
  msg_ptr->request_header.response_cb        = response_cb_ptr;
    ...
    mmgsdi_status = mmgsdi_caller_context_get_atr_status_and_notify_client(task_cmd_ptr);
...
PARAMS:
  ...
  default_channel_streaming: Allow APDU streaming on default channel
  response_cb_ptr:           Response callback.
  client_ref:                User Data returned upon completion of this cmd
...
static mmgsdi_return_enum_type mmgsdi_send_apdu_operation(
  ...
  boolean                            default_channel_streaming,
  mmgsdi_callback_type               response_cb_ptr,
  mmgsdi_client_data_type            client_ref,
  mmgsdi_cmd_enum_type               req_type)
{
  ...
  mmgsdi_slot_data_type             * slot_data_ptr     = NULL;
  mmgsdi_card_type_enum_type          card_type         = MMGSDI_CARD_TELECOM;
  mmgsdi_transport_protocol_enum_type transmission_mode = MMGSDI_T_0_PROTOCOL;
  ...
  if(slot_data_ptr != NULL)
  {
    card_type         = slot_data_ptr->card_type;
    transmission_mode = slot_data_ptr->transmission_mode;
  }
  MMGSDIUTIL_LEAVE_CLIENT_APP_DATA_CRIT_SECT;

  mmgsdi_status = mmgsdi_util_apdu_sanity_check(&apdu_data, transmission_mode);
  if (mmgsdi_status != MMGSDI_SUCCESS)
  {
    return mmgsdi_status;
  }
  ...
  /* If apdu conversion feature is enabled then process the
     following interindustry (Bit b8 set to 0) APDUs as MMGSDI request:
     1. APDUs streamed on channel 0:
          SELECT
          AUTHENTICATE
          UPDATE_BINARY
          UPDATE_RECORD
          READ_BINARY
          READ_RECORD
     2. APDUs streamed on all channels:
          MANAGE CHANNEL
     In case of failure and the APDU should not be rejected, continue as APDU streaming
   */
  if(mmgsdi_nv_get_device_feature_status(MMGSDI_FEATURE_DISABLE_APDU_CONVERSION_ON_CH_0) ==
        MMGSDI_FEATURE_DISABLED &&
     apdu_data.data_len >= UIM_CASE1_7816_APDU_SIZE && 
     (apdu_data.data_ptr[UIM_7816_APDU_CLASS_OFFSET] & 0x80) == 0)
  {
    /* For Non telecom card process the APDU as it is */
    if (card_type == MMGSDI_CARD_NON_TELECOM)
    {
      /* Mark failure to process APDU as it is */
      mmgsdi_status = MMGSDI_ERROR;
    }
    else if ((uim_instrns_enum_type)apdu_data.data_ptr[UIM_7816_APDU_INSTRN_OFFSET] ==
        MANAGE_CHANNEL)
    {
    ...
    else if (req_type == MMGSDI_SEND_APDU_EXT_REQ &&
             mmgsdi_util_get_channel_id_from_cla(
               apdu_data.data_ptr[UIM_7816_APDU_CLASS_OFFSET]) == MMGSDI_DEFAULT_CHANNEL)
    {
      mmgsdi_status = mmgsdi_crsm_default_channel_apdu_access(client_id,
                                                              card_slot,
                                                              apdu_option,
                                                              apdu_data,
                                                              client_ref,
      ...
      /* Do not proceed with APDU streaming if default_channel_streaming
         is FALSE. In such a case, APDUs are allowed on default channel only
         if they can be converted to high-level commands */
      if (mmgsdi_status != MMGSDI_SUCCESS && !default_channel_streaming)
      {
        reject_apdu = TRUE;
        UIM_MSG_MED_0("APDU streaming not allowed on default channel");
      }
    }
    else
    {
...
/* ============================================================================
FUNCTION MMGSDI_SET_APDU_BEHAVIOR

DESCRIPTION:
  This is the API set the modem behavior for warning status(62XX/63XX)
...
SIDE EFFECTS:
  None
  ========================================================================== */
mmgsdi_return_enum_type mmgsdi_set_apdu_behavior(
  mmgsdi_client_id_type        client_id,
  mmgsdi_slot_id_enum_type     card_slot,
...
{
  mmgsdi_task_cmd_type                      * task_cmd_ptr  = NULL;
  mmgsdi_set_apdu_behavior_req_type         * msg_ptr       = NULL;
  mmgsdi_return_enum_type                     mmgsdi_status = MMGSDI_ERROR;
  ...
  MMGSDIUTIL_RETURN_IF_OUT_OF_RANGE(channel_id, UIM_CHANNEL0, UIM_MAX_CHANNELS);
  MMGSDIUTIL_RETURN_IF_NULL(response_cb_ptr);
  ...
  /* --------------------------------------------------------------------------
    No need to Align pointers of the structures members
    ------------------------------------------------------------------------ */
  msg_ptr = &task_cmd_ptr->cmd.cmd.set_apdu_behavior_req;

  /*
   * Populate header.
   */
  task_cmd_ptr->cmd.cmd_enum                 = MMGSDI_SET_APDU_BHEAVIOR_REQ;
  msg_ptr->request_header.client_id          = client_id;
  msg_ptr->request_header.request_type       = MMGSDI_SET_APDU_BHEAVIOR_REQ;
  msg_ptr->request_header.orig_request_type  = MMGSDI_SET_APDU_BHEAVIOR_REQ;
  msg_ptr->request_header.slot_id            = card_slot;
  msg_ptr->request_header.client_data        = client_ref;
  msg_ptr->request_header.response_cb        = response_cb_ptr;
  msg_ptr->channel_id                        = channel_id;
  msg_ptr->apdu_behavior                     = apdu_behavior;
  ...
  UIM_MSG_HIGH_4("Queue of MMGSDI command: MMGSDI_SET_APDU_BHEAVIOR_REQ status 0x%x slot 0x%x channel_id 0x%x apdu_behavior 0x%x",
                  mmgsdi_status,
                  card_slot,
                  channel_id,
                  apdu_behavior);
...
)
{
  ...
  if (client_id == MMGSDI_CLIENT_ID_ZERO || (MMGSDI_SUCCESS == mmgsdi_util_is_valid_client_id(client_id)))
  {
...
{
  return mmgsdi_util_queue_client_dereg_req(client_id,
                                            MMGSDI_TASK_MAIN,
                                            response_cb_ptr,
                                            client_ref);
...
/*============================================================================
   FUNCTION:      MMGSDI_NOTIFY_RECOVERY_START

   DESCRIPTION:
     Notifies MMGSDI of start of recovery on a card in a slot.
     The API shall be called by UIM drivers before starting of recovery procedure
     on a card in a slot.
   ...
   SIDE EFFECTS:
     None
============================================================================*/
static mmgsdi_return_enum_type mmgsdi_notify_recovery_start(
  mmgsdi_slot_id_enum_type     slot_id
)
{
  mmgsdi_task_cmd_type                       * task_cmd_ptr   = NULL;
  mmgsdi_notify_recovery_start_req_type      * msg_ptr        = NULL;
  ...
  msg_ptr = &task_cmd_ptr->cmd.cmd.recovery_start_req;

  /*---------------------------------------------------------------------------
    Populating recovery_complete_req header
      1) Client ID
      2) Request type
      3) Slot ID
      4) Client Data
      ...
      6) Payload len and request len will be populated after the content
         has been populated
  ---------------------------------------------------------------------------*/
  task_cmd_ptr->cmd.cmd_enum                = MMGSDI_NOTIFY_RECOVERY_START_REQ;
  msg_ptr->request_header.client_id         = mmgsdi_get_global_client_id();
  msg_ptr->request_header.request_type      = MMGSDI_NOTIFY_RECOVERY_START_REQ;
  msg_ptr->request_header.orig_request_type = MMGSDI_NOTIFY_RECOVERY_START_REQ;
  ...
  /* --------------------------------------------------------------------------
     Queue recovery complete notification request to command queue for main task.
     ----------------------------------------------------------------------- */
  ...
  UIM_MSG_HIGH_2(
    "Queue of MMGSDI command: MMGSDI_NOTIFY_RECOVERY_START_REQ status: 0x%x, slot_id: 0x%x",
    mmgsdi_status, slot_id);

  return mmgsdi_status;
} /* mmgsdi_notify_recovery_start */
...
)
{
  ...
  mmgsdi_status = mmgsdi_util_convert_from_uim_slot(uim_slot_id, &slot_id);
  if(mmgsdi_status != MMGSDI_SUCCESS || slot_id == MMGSDI_SLOT_AUTOMATIC)
  {
    return MMGSDI_ERROR;
  }

  /* Queue recovery start to MMGSDI */
  (void)mmgsdi_notify_recovery_start(slot_id);
           ...
           mmgsdi_channel_info_ptr_table[channel_index]->channel_id, slot_id) == FALSE)
      {
        ...
        switch(channel_state)
        {
          ...
          case MMGSDI_CHANNEL_STATE_READY:
            ...
            switch (mmgsdi_channel_info_ptr_table[channel_index]->apdu_behavior)
            {
              case MMGSDI_APDU_BEHAVIOR_NO_GET_RESPONSE_FOR_WARNING:
                recovery_data_ptr->app_data[channel_id].apdu_behavior =
                    UIM_APDU_BEHAVIOR_NO_GET_RESPONSE_FOR_WARNING;
                break;

              case MMGSDI_APDU_BEHAVIOR_AUTOMATIC_GET_RESPONSE_FOR_WARNING:
                recovery_data_ptr->app_data[channel_id].apdu_behavior =
                    UIM_APDU_BEHAVIOR_AUTOMATIC_GET_RESPONSE_FOR_WARNING;
                break;
            }
            break;
      ...
      if(protocol == MMGSDI_UICC)
      {
           ...
           (app_info_ptr->universal_pin_ptr->pin_data.data_len != 0))
        {
          ...
          mmgsdi_memscpy(recovery_data_ptr->upin_data.chv_digits,
            ...
            app_info_ptr->universal_pin_ptr->pin_data.data_len);
         ...
         (app_info_ptr->pin1_ptr->pin_data.data_len != 0))
      {
        ...
        if(!found)
        {
          ...
          mmgsdi_memscpy(recovery_data_ptr->app_data[channel_id].pin1_data.chv_digits,
            ...
            app_info_ptr->pin1_ptr->pin_data.data_len);
        }
      }
...
)
{
  ...
  UIM_MSG_HIGH_3(
    "Queue of MMGSDI command: MMGSDI_NOTIFY_SIM_BUSY_REQ status: 0x%x, Slot: 0x%x, SIM Busy: 0x%x",
     mmgsdi_status, slot, sim_busy);
...
mmgsdi_remote_sim_test_params_type    rm_sim_test_param_record = {0};
  ...
  /* --------------------------------------------------------------------------
     Allocate memory for the MMGSDI Task Command
     ------------------------------------------------------------------------*/
  if(MMGSDI_SUCCESS != mmgsdi_util_convert_from_uim_slot(slot_id, &mmgsdi_slot_id))
  {
    /* In this case we ignore the error and link_est is processed in main task */
  }
  ...
  if(msg_ptr->link_est_data.iccid.length > 0)
  {
   ...
   /* Copy EID */
  if(link_est_data.eid_buf.num_bytes <= 
     MMGSDI_EID_LEN)
  {
    msg_ptr->link_est_data.eid_info.length = 
	  link_est_data.eid_buf.num_bytes;
  }
  else
  {
    msg_ptr->link_est_data.eid_info.length = 
  ...
  if(msg_ptr->link_est_data.eid_info.length > 0)
  {
    mmgsdi_memscpy(msg_ptr->link_est_data.eid_info.data,
	               sizeof(msg_ptr->link_est_data.eid_info.data),
				   link_est_data.eid_buf.data,
				   msg_ptr->link_est_data.eid_info.length);
  }

  /* Convert UIM card type to MMGSDI card type and copy */
  (void)mmgsdi_util_convert_from_uim_card_type(
          link_est_data.card_type,
          &msg_ptr->link_est_data.card_type);

  /* Convert UIM transmission mode to MMGSDI transmission mode and copy */
  (void)mmgsdi_util_convert_from_uim_transport_protocol_type(
          link_est_data.transmission_mode,
          &msg_ptr->link_est_data.transmission_mode);

  /* Convert UIM slot type to MMGSDI slot type and copy */
  (void)mmgsdi_util_convert_from_uim_slot_type(
          link_est_data.slot_type,
          &msg_ptr->link_est_data.slot_type);

  /* If test EFS file is enabled, update the slot type. This is needed
     because clients expect card type to be remote in such cases */
  if (mmgsdi_nv_get_remote_test_params(mmgsdi_slot_id,
                                      &rm_sim_test_param_record))
  {
    msg_ptr->link_est_data.slot_type            = MMGSDI_CARD_REMOTE;
  }
  ...
  mmgsdi_status = mmgsdi_cmd_ext(task_cmd_ptr, mmgsdi_task);
  UIM_MSG_HIGH_2("Queue of MMGSDI_NOTIFY_LINK_EST_REQ command, status: 0x%x, slot_id: 0x%x",
                 mmgsdi_status, slot_id);
  if (mmgsdi_status != MMGSDI_SUCCESS)
  {
    /* Free the pointer since the task_cmd_ptr has not been put onto the
...
{
  mmgsdi_task_cmd_type                   * task_cmd_ptr  = NULL;
  mmgsdi_notify_sim_pre_init_req_type    * msg_ptr       = NULL;
  mmgsdi_slot_id_enum_type                 slot_id       = MMGSDI_MAX_SLOT_ID_ENUM;
  mmgsdi_return_enum_type                  mmgsdi_status = MMGSDI_ERROR;
  mmgsdi_task_enum_type                    mmgsdi_task   = MMGSDI_TASK_MAX;

  /* Validate the slot */
  MMGSDIUTIL_RETURN_IF_OUT_OF_RANGE(uim_slot,
                                    UIM_SLOT_1,
                                    UIM_SLOT_3);

  /* Convert slot type from UIM Driver type to MMGSDI type */
  mmgsdi_status = mmgsdi_util_convert_from_uim_slot(uim_slot, &slot_id);
  if(mmgsdi_status != MMGSDI_SUCCESS)
  {
    return mmgsdi_status;
  }
  ...
  /* No need to Align pointers of the structures members */
  msg_ptr = &task_cmd_ptr->cmd.cmd.sim_pre_init_req;
  ...
  /*---------------------------------------------------------------------------
    Populating card_error_req header
      ...
      6) Payload len and request len will be populated after the content
         has been populated
  ---------------------------------------------------------------------------*/
  task_cmd_ptr->cmd.cmd_enum                = MMGSDI_NOTIFY_SIM_PRE_INIT_REQ;
  msg_ptr->request_header.client_id         = mmgsdi_get_global_client_id();
  msg_ptr->request_header.request_type      = MMGSDI_NOTIFY_SIM_PRE_INIT_REQ;
  msg_ptr->request_header.orig_request_type = MMGSDI_NOTIFY_SIM_PRE_INIT_REQ;
  msg_ptr->request_header.slot_id           = slot_id;
  msg_ptr->request_header.client_data       = 0;
  msg_ptr->request_header.response_cb       = mmgsdi_util_internal_cb;

  /* --------------------------------------------------------------------------
     Put on mmgsdi slot-specific command queue
     ----------------------------------------------------------------------- */
  mmgsdi_task = mmgsdi_util_get_task_from_slot(slot_id);
  if(mmgsdi_task >= MMGSDI_NUM_TASKS)
  {
  ...
  UIM_MSG_HIGH_2(
    "Queue of MMGSDI command: MMGSDI_NOTIFY_SIM_PRE_INIT_REQ status: 0x%x, uim_slot: 0x%x",
    mmgsdi_status, uim_slot);

  return mmgsdi_status;
} /* mmgsdi_notify_sim_pre_init */
...
/*============================================================================
   ...
   DESCRIPTION:
     Called by GSTK to inform MMGSDI that the IMEI(SV) has been sent to the
   ...
   RETURN VALUE:
    ...
    MMGSDI_SUCCESS:           Successfully sent the notification
    MMGSDI_ERROR:             Generic error.
...
)
{
  ...
  UIM_MSG_MED_1("In mmgsdi_notify_imei_sent_to_card, slot: 0x%x", slot_id);
  ...
  return MMGSDI_SUCCESS;
} /* mmgsdi_notify_imei_sent_to_card */
...
{
  mmgsdi_task_cmd_type                       * task_cmd_ptr   = NULL;
  mmgsdi_notify_recovery_complete_req_type   * msg_ptr        = NULL;
  mmgsdi_return_enum_type                      mmgsdi_status  = MMGSDI_SUCCESS;
  mmgsdi_slot_id_enum_type                     mmgsdi_slot_id = MMGSDI_SLOT_AUTOMATIC;

  mmgsdi_status = mmgsdi_util_convert_from_uim_slot(uim_slot_id, &mmgsdi_slot_id);
  if(mmgsdi_status != MMGSDI_SUCCESS)
  {
    return mmgsdi_status;
  }
  ...
  msg_ptr = &task_cmd_ptr->cmd.cmd.recovery_complete_req;
  ...
  /*---------------------------------------------------------------------------
    Populating recovery_complete_req header
      ...
      6) Payload len and request len will be populated after the content
         has been populated
  ---------------------------------------------------------------------------*/
  task_cmd_ptr->cmd.cmd_enum                = MMGSDI_NOTIFY_RECOVERY_COMPLETE_REQ;
  msg_ptr->request_header.client_id         = mmgsdi_get_global_client_id();
  msg_ptr->request_header.request_type      = MMGSDI_NOTIFY_RECOVERY_COMPLETE_REQ;
  msg_ptr->request_header.orig_request_type = MMGSDI_NOTIFY_RECOVERY_COMPLETE_REQ;
  msg_ptr->request_header.slot_id           = mmgsdi_slot_id;
  msg_ptr->request_header.client_data       = 0;
  msg_ptr->request_header.response_cb       = mmgsdi_util_internal_cb;
  ...
  UIM_MSG_HIGH_2(
    "Queue of MMGSDI command: MMGSDI_NOTIFY_RECOVERY_COMPLETE_REQ status: 0x%x, uim_slot_id: 0x%x",
    mmgsdi_status, uim_slot_id);

  return mmgsdi_status;
} /* mmgsdi_notify_recovery_complete */
...
)
{
  ...
  if (mmgsdi_util_is_slot_valid(mmgsdi_slot) == FALSE)
  {
    return MMGSDI_INCORRECT_PARAMS;
  }
...
)
{
  ...
  /* Return INCORRECT PARAM error when API is called with
     wrong name length and/or NULL pointer and without any name param */
...
{
  nv_items_enum_type        item_enum      = NV_ITEMS_ENUM_MAX;
  ...
  do
  {
    switch (nv_item_data_ptr->nv_item_enum)
    {
      ...
      case MMGSDI_NV_PREF_MODE_NOT_FROM_RUIM_I:
        if (mmgsdi_nv_get_device_feature_status(
              MMGSDI_FEATURE_PREF_MODE_NOT_FROM_RUIM) == MMGSDI_FEATURE_ENABLED)
        {
      ...
      case MMGSDI_NV_CDMA_DF_SELECT_BEFORE_TP_I :
        if (mmgsdi_nv_get_device_feature_status(MMGSDI_FEATURE_CDMA_DF_SELECT_BEFORE_TP) ==
      ...
      case MMGSDI_NV_EXPLICIT_SELECT_DFCDMA_BEFORE_TELECOM_ADN_READ_I:
        if (mmgsdi_nv_get_device_feature_status(MMGSDI_FEATURE_EXPLICIT_SELECT_DFCDMA_BEFORE_TELECOM_ADN_READ)
            == MMGSDI_FEATURE_ENABLED)
        {
      ...
      case MMGSDI_NV_SUPPORT_EXTENDED_FPLMN_I:
        if (mmgsdi_nv_get_device_feature_status(MMGSDI_FEATURE_SUPPORT_EXTENDED_FPLMN) ==
            MMGSDI_FEATURE_ENABLED)
        {
          nv_item_data_ptr->nv_item_data.support_extended_fplmn = TRUE;
        }
        else
        {
          nv_item_data_ptr->nv_item_data.support_extended_fplmn = FALSE;
        }
        mmgsdi_status = MMGSDI_SUCCESS;
        break;
    ...
    if (mmgsdi_status != MMGSDI_MAX_RETURN_ENUM)
    {
      break;
    }

    mmgsdi_status = MMGSDI_SUCCESS;
    if(MMGSDI_CACHE_INIT == mmgsdi_nv_get_nv_item_from_cache(
                              item_enum, &mmgsdi_nv_data, MMGSDI_NV_CONTEXT_PRIMARY) &&
       mmgsdi_nv_data.item_status == NV_DONE_S)
    {
      switch (item_enum)
      {
...
SIDE EFFECTS:
  None
===============================================================*/
boolean mmgsdi_set_qmi_uim_sig(void)
{
  ...
  return (task_state == MMGSDI_TASK_ACTIVE ? TRUE : FALSE);
...
SIDE EFFECTS:
  None
===============================================================*/
boolean mmgsdi_set_slot_mapping_sig(void)
{
  mmgsdi_task_state_enum_type     task_state = MMGSDI_TASK_INACTIVE;
...
)
{
  ...
  mmgsdi_status = mmgsdi_process_cancel_req(cancel_info, TRUE);
...
)
{
  ...
  if (command != STATUS)
  {
    /* Calculate file access and update the file path from MF if only file id
       is provided. */
...
)
{
  ...
  {
    UIM_MSG_ERR_0("SET SIM PROFILE API is only supported on slot1");
...
)
{
  mmgsdi_task_cmd_type                *task_cmd_ptr  = NULL;
  mmgsdi_return_enum_type              mmgsdi_status = MMGSDI_ERROR;
  mmgsdi_m2m_get_eid_req_type         *msg_ptr       = NULL;
  ...
  /* --------------------------------------------------------------------------
	  No need to Align pointers of the structures members
     ------------------------------------------------------------------------ */
  msg_ptr = &task_cmd_ptr->cmd.cmd.euicc_m2m_get_eid_req;
  /*
   * Populate header.
   */
...
)
{
  ...
  if(slot != MMGSDI_SLOT_1)
  {
    UIM_MSG_ERR_0("GET SIM PROFILE API is only supported on slot1");
    return MMGSDI_MULTI_PROFILE_NOT_SUPPORTED;
  }
...
/*===========================================================================
FUNCTION:       MMGSDI_MAP_SESSIONS_TO_SLOTS
=========================================================================== */
/**
  This function attempts to map the session types to the slots. Client can
  mention which slots the PRI, SEC, TER (and so forth) session types should
  be associated with. If MMGSDI_SLOT_NONE is mentioned by client for a
  session type, all the (GW and 1X) sessions for that session type are
  deactivated.
  Following are some use cases and behavior of the API in those cases:
  1. Assume all the PRI (GW and 1X) sessions are activated on SLOT_1,
            all the SEC (GW and 1X) sessions are activated on SLOT_2,
            all the TER (GW and 1X) sessions are activated on SLOT_3,
     Client then calls the API with PRI<->SLOT_2, SEC<->SLOT_1, TER<->SLOT_3.
     This will result in
     - deactivation of GW_PRI, 1X_PRI, GW_SEC and 1X_SEC.
     - activation of GW_PRI and 1X_PRI on SLOT_2
     - activation of GW_SEC and 1X_SEC on SLOT_1
     - GW_TER and 1X_TER sessions' app/slot association remains untouched.
  2. Assume all the PRI (GW and 1X) sessions are activated on SLOT_1,
            all the SEC (GW and 1X) sessions are activated on SLOT_2,
            all the TER (GW and 1X) sessions are activated on SLOT_3,
     Client then calls the API with PRI<->SLOT_2, SEC<->SLOT_1, TER<->SLOT_NONE.
     This will result in
     - deactivation of GW_PRI, 1X_PRI, GW_SEC and 1X_SEC.
     - activation of GW_PRI and 1X_PRI on SLOT_2
     - activation of GW_SEC and 1X_SEC on SLOT_1
     - GW_TER and 1X_TER sessions are deactivated.

  NOTE that this API cannot be used to map sessions of same type on different
  slots, e.g., GW_PRI is required to be associated with SLOT_1 but 1X_PRI is
  required to be associated with SLOT_2.


  @param[in]  client_id:                             Client Id of the caller
  @param[in]  mmgsdi_sessions_to_slots_mapping_type: Mapping mentioning slots
                                                     to be used to activate
                                                     PRI, SEC, TER etc sessions.
  @param[in]  response_cb_ptr:                       Response callback.
  @param[in]  client_ref:                            User Data returned upon
                                                     completion of this cmd.
  ...
  mmgsdi_return_enum_type:
    ...
    MMGSDI_SUCCESS:          The command structure was properly generated
                             and queued onto the MMGSDI Command Queue.
    MMGSDI_INCORRECT_PARAMS: If slot id mentioned for any session type in
                             mmgsdi_sessions_to_slots_mapping_type was
                             invalid, that is, it was MMGSDI_SLOT_AUTOMATIC
                             or MMGSDI_MAX_SLOT_ID_ENUM.
                             OR, if attempt was made to associate different
                             session types to same slot id.
    MMGSDI_CMD_QUEUE_FULL:   The command was not Queued to the MMGSDI Task
                             because the max number of commands are already
                             queued.
  @sideeffects
  None
*/
mmgsdi_return_enum_type  mmgsdi_map_sessions_to_slots(
  mmgsdi_client_id_type                  client_id,
  mmgsdi_sessions_to_slots_mapping_type  sessions_to_slots,
  mmgsdi_callback_type                   response_cb_ptr,
  mmgsdi_client_data_type                client_ref
)
{
  mmgsdi_task_cmd_type                      *task_cmd_ptr   = NULL;
  mmgsdi_map_sessions_to_slots_req_type     *msg_ptr        = NULL;
  mmgsdi_return_enum_type                    mmgsdi_status  = MMGSDI_ERROR;

  MMGSDIUTIL_RETURN_IF_NULL(response_cb_ptr);

  MMGSDIUTIL_RETURN_IF_EXCEEDS(sessions_to_slots.pri_slot, MMGSDI_SLOT_3);
  MMGSDIUTIL_RETURN_IF_EXCEEDS(sessions_to_slots.sec_slot, MMGSDI_SLOT_3);
  MMGSDIUTIL_RETURN_IF_EXCEEDS(sessions_to_slots.ter_slot, MMGSDI_SLOT_3);

  if ((sessions_to_slots.pri_slot != MMGSDI_SLOT_NONE &&
       (sessions_to_slots.pri_slot == sessions_to_slots.sec_slot ||
        sessions_to_slots.pri_slot == sessions_to_slots.ter_slot)) ||
      (sessions_to_slots.sec_slot != MMGSDI_SLOT_NONE &&
       sessions_to_slots.sec_slot == sessions_to_slots.ter_slot))
  {
  ...
  msg_ptr = &task_cmd_ptr->cmd.cmd.map_sessions_to_slots_req;

  /* Populating mmgsdi_map_sessions_to_slots header */
  task_cmd_ptr->cmd.cmd_enum                = MMGSDI_MAP_SESSIONS_TO_SLOTS_REQ;
  msg_ptr->request_header.request_type      = MMGSDI_MAP_SESSIONS_TO_SLOTS_REQ;
  msg_ptr->request_header.orig_request_type = MMGSDI_MAP_SESSIONS_TO_SLOTS_REQ;
  msg_ptr->request_header.client_id         = client_id;
  msg_ptr->request_header.session_id        = 0;
  msg_ptr->request_header.client_data       = client_ref;
  msg_ptr->request_header.response_cb       = response_cb_ptr;
  msg_ptr->mapping                          = sessions_to_slots;
  ...
  UIM_MSG_HIGH_1("Queue of MMGSDI command: MMGSDI_MAP_SESSIONS_TO_SLOTS_REQ status 0x%x",
                 mmgsdi_status);

  return mmgsdi_status;
} /* mmgsdi_map_sessions_to_slots */


/* ==========================================================================
FUNCTION:       MMGSDI_GET_HW_SLOTS
========================================================================== */
/**
  This function provides the number of physical slots enabled in the device.

  @param[out] num_slots_ptr:   Buffer to fill the number of slots
  ...
  @return
  MMGSDI_SUCCESS            : The call to the API is successful and
                              out parameter data is successfully populated
  MMGSDI_INCORRECT_PARAMS   : Buffer to store slot information is NULL

  @sideeffects
  None

============================================================================*/
mmgsdi_return_enum_type  mmgsdi_get_hw_slots (
  uint8                     *num_slots_ptr
)
{
  uint8                                      i = 0;
  ...
  MMGSDIUTIL_RETURN_IF_NULL(num_slots_ptr);
  ...
  *num_slots_ptr = 0;
  ...
  /* Ceil to the max num slots supported.
     This condition should never happen ideally */
  if(*num_slots_ptr > MMGSDI_MAX_NUM_SLOTS)
  {
    *num_slots_ptr = MMGSDI_MAX_NUM_SLOTS;
  }
  return MMGSDI_SUCCESS;
} /* mmgsdi_get_hw_slots */
...
)
{
  ...
  mmgsdi_task_cmd_type                 * task_cmd_ptr  = NULL;
  mmgsdi_qsh_action_event_cfg_req_type * msg_ptr       = NULL;
  mmgsdi_return_enum_type                mmgsdi_status = MMGSDI_ERROR;

  MMGSDIUTIL_RETURN_IF_NULL(qsh_cb_params_ptr);
  ...
  /* No need to Align pointers of the structures members */
  msg_ptr = &task_cmd_ptr->cmd.cmd.qsh_action_event_cfg_req;
  ...
  /*---------------------------------------------------------------------------
    Populating card_error_req header
      ...
      5) Payload len and request len will be populated after the content
         has been populated
  ---------------------------------------------------------------------------*/
  task_cmd_ptr->cmd.cmd_enum                = MMGSDI_QSH_ACTION_EVENT_CFG_REQ;
  msg_ptr->request_header.client_id         = mmgsdi_get_global_client_id();
  msg_ptr->request_header.request_type      = MMGSDI_QSH_ACTION_EVENT_CFG_REQ;
  msg_ptr->request_header.orig_request_type = MMGSDI_QSH_ACTION_EVENT_CFG_REQ;
  msg_ptr->request_header.client_data       = 0;
  msg_ptr->request_header.response_cb       = mmgsdi_util_internal_cb;

  mmgsdi_memscpy(&msg_ptr->cb_params, sizeof(msg_ptr->cb_params),
                 qsh_cb_params_ptr, sizeof(qsh_client_cb_params_s));

  /* --------------------------------------------------------------------------
     Always put QSH ACTION EVENT CFG REQ on mmgsdi command queue for the main task.
     This ensures it will be processed even if the slot specific task is stuck.
     ----------------------------------------------------------------------- */
  mmgsdi_status = mmgsdi_cmd_ext(task_cmd_ptr, MMGSDI_TASK_MAIN);
...
} /* mmgsdi_qsh_action_event_config */
...
{
  if (mmgsdi_is_test_mode_logging_enabled.secure_logging_enabled)
  {
    return TRUE;
  }
  switch(sub_id)
...
)
{
  ...
  if (mmgsdi_is_test_mode_logging_enabled.private_logging_enabled)
  {
    return TRUE;
  }
  ...
  return FALSE;
} /* mmgsdi_is_private_logging_enabled_for_slot */
  ...
  mmgsdi_len_type          req_data_len)
{
  ...
  if(atr_data_ptr == NULL || atr_data_ptr->data_ptr == NULL)
  {
    return MMGSDI_INCORRECT_PARAMS;
  }
  ...
  do
  {
    ...
    if ((client_id != MMGSDI_CLIENT_ID_ZERO) && 
        (mmgsdi_util_is_valid_client_id(client_id)!= MMGSDI_SUCCESS))
    {
      mmgsdi_status = MMGSDI_ERROR; 
      break;
    }
    ...
    atr_data_ptr->data_len = slot_data_ptr->card_atr.atr_length;
    
    mmgsdi_memscpy(atr_data_ptr->data_ptr,
                   req_data_len,
  ...
  mmgsdi_len_type          req_data_len)
{
  ...
  if(eid_data_ptr == NULL || eid_data_ptr->data_ptr == NULL)
  {
    return MMGSDI_INCORRECT_PARAMS;
  }
  ...
  do
  {
    ...
    if (slot_data_ptr->eid_info.length == 0)
    {
      mmgsdi_status = MMGSDI_ERROR;
      break;
    }

    eid_data_ptr->data_len = slot_data_ptr->eid_info.length;
    
    mmgsdi_memscpy(eid_data_ptr->data_ptr,
                   req_data_len,
                   slot_data_ptr->eid_info.data,
                   slot_data_ptr->eid_info.length);
