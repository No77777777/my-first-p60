...
GENERAL DESCRIPTION
  ...
  This source file contains the sessions management support in MMGSDI.
...
Copyright (c) 2004 - 2018 QUALCOMM Technologies, Inc (QTI) and its licensors.
...
$Header: //components/rel/uim.mpss/6.1.0/mmgsdi/src/mmgsdi_session.c#3 $$ $DateTime: 2020/10/18 15:27:05 $
...
07/03/17   gm      Don't send status word when recovery
...
08/01/16   nr      Don't reject activation during ecc call if auto_activation enabled
...
02/05/16   ar      Reset deactivation_in_progress flag during app_reset_auto
01/27/16   vdc     Fail on going refresh if deactivation is in progress
01/18/16   sp      Reduce F3 logging
01/16/16   kv      Handle exclusive non prov session close on a prov app
...
01/07/16   vdc     Pass req hdr slot id while processing select AID response
12/16/15   ar      Add utility to check MMGSDI task
11/24/15   ar      Handling of 2 de-activation req when queued for same session
11/23/15   av      Block SELECT on AID only if first SELECT failed with 6A82
11/12/15   vdc     Fix KW issue for AID check
10/29/15   nr      Don't set skip app flag if app de-activation is due to refresh
10/27/15   ar      Don't send cnf during flush if the rsp_status is PROCESSING
10/23/15   vdc     Send err cnf when app deactivates due to refresh app reset
...
10/01/15   bcho    Reject internal deactivate req if APP RESET AUTO is running
...
08/14/15   lxu     Error code for card not supporting select by partial AID
...
07/13/15   ar      Don't send status word in ICC case for SESSION_OPEN_WITH_MF
06/16/15   yt      Handle cancellation of sync commands during deactivation
06/15/15   tl      Fix potential buffer overflow in cdma AKA cnf
...
06/11/15   av      User safer string comparison function
06/10/15   av      Reset app_termination_skipped for app reactvtd via switching
05/27/15   ar      Handle sending double confirmation to client
05/15/15   lxu     Fix logical channel exhausted before CSIM activation
05/13/15   av      During deact, flush pending commands even for the ICC case
05/13/15   av      Fix compilation warnings
...
04/17/15   ar      Update num_connection when a session mapped to channel_index
...
12/18/14   bcho    NULL check added before dereferencing app info ptr
  ...
  const mmgsdi_cnf_type   *cnf_ptr)
{
  ...
  switch(cnf)
  {
    ...
    case MMGSDI_SESSION_CLOSE_REQ:
      /* Obtain the index */
      index = cnf_ptr->session_close_cnf.response_header.client_data;
  ...
  const mmgsdi_cnf_type   *cnf_ptr)
{
  ...
  if (extra_param_ptr != NULL)
  {
    ...
    memset(&msg, 0x00, sizeof(mmgsdi_session_act_or_switch_prov_req_type));
...
)
{
  ...
  uim_rpt.cmd_transacted          = TRUE;
  uim_rpt.user_data               = index;
        ...
        if(/* Update channel info for sessions of provisioning type */
           ...
           ((mmgsdi_channel_info_ptr_table[session_info_ptr->channel_info_index]->slot_id == MMGSDI_SLOT_3) &&
            ...
            (mmgsdi_channel_info_ptr_table[session_info_ptr->channel_info_index]->channel_id ==
              MMGSDI_DEFAULT_CHANNEL)))
        {
          /* Before updating Channel mapping check if we are in CSIM hide state, if so
              follow the logic below
              - if we are in csim hide state and
              - client is not mmgsdi or not qmi and
              - if the current session type is 1X session
              do not update the channel for this session.
              This is to ensure that modem clients do not get CSIM related events as CSIM is
              hidden from them */
          if (slot_data_ptr->csim_hiding_status == MMGSDI_CSIM_HIDDEN &&
              ...
              (mmgsdi_util_client_receives_hidden_csim_events(i) == FALSE))
          {
            UIM_MSG_HIGH_1("populate_session_info: CSIM hidden, so skipping channel mapping update for session_id=0x%x",
           ...
           mmgsdi_util_is_card_session(session_info_ptr->session_type))
  {
    if(mmgsdi_channel_info_ptr_table[session_info_ptr->channel_info_index]->num_connections == 0)
    {
      ...
      for (c_index = 0; c_index < MMGSDI_MAX_CLIENT_INFO; c_index++)
      {
        ...
        num_sessions = mmgsdi_client_id_reg_table[c_index]->num_session;
        for(s_index = 0; (num_sessions > 0) && (s_index < MMGSDI_MAX_SESSION_INFO);s_index++)
        {
          curr_session_ptr =
          ...
          num_sessions--;
          if((curr_session_ptr == session_info_ptr) ||
  ...
  do
  {
       ...
       (mmgsdi_channel_info_ptr_table[session_info_ptr->channel_info_index] == NULL))
    {
         ...
         (mmgsdi_util_is_non_prov_session(session_info_ptr->session_type))))
      {
        ...
        MMGSDIUTIL_TMC_MEM_FREE(session_info_ptr->refresh_data.refresh_files.file_list_ptr);
        MMGSDIUTIL_TMC_MEM_FREE(session_info_ptr->refresh_data.refresh_files.file_path_ptr);

        /* Free the memory allocated for file update registration */
        MMGSDIUTIL_TMC_MEM_FREE(session_info_ptr->file_data.file_list_ptr);
       ...
       if(mmgsdi_channel_info_ptr_table[session_info_ptr->channel_info_index]->app_info_ptr)
        ...
        else if(mmgsdi_channel_info_ptr_table[session_info_ptr->channel_info_index]->app_info_ptr != NULL)
        {
          ...
          mmgsdi_channel_info_ptr_table[session_info_ptr->channel_info_index]->app_info_ptr->pre_perso_files_cached =
            FALSE;
          mmgsdi_channel_info_ptr_table[session_info_ptr->channel_info_index]->app_info_ptr->first_loci_write_after_activation =
            FALSE;
...
)
{
        ...
        req_ptr->request_header.slot_id, &aid.aid, &channel_id, &channel_index))
  {
    ...
    if((mmgsdi_nv_get_device_feature_status(MMGSDI_FEATURE_SKIP_PROV_APP_TERMINATION) ==
  ...
  const mmgsdi_uim_report_rsp_type *uim_rsp_ptr)
{
      ...
      uim_rsp_ptr->rsp_data.data_len > offset)
  {
          ...
          == MMGSDI_ISIM_3G_AUTH_SUCCESS)
    {
           ...
           extra_param_ptr->isim_auth.auth_context == MMGSDI_AUTN_ISIM_HTTP_DIGEST_CONTEXT))
      {
        ...
        for (i = 0; i < max_resp_elements; i++)
        {
          if (uim_rsp_ptr->rsp_data.data_len <= offset)
          {
            for (j = 0; j < i; j++)
            {
              MMGSDIUTIL_TMC_MEM_FREE(temp_auth_rsp_buf[j].data_ptr);
          ...
          if(temp_auth_rsp_buf[i].data_len > 0 &&
             temp_auth_rsp_buf[i].data_len < (uim_rsp_ptr->rsp_data.data_len - offset))
          {
            MMGSDIUTIL_MEM_MALLOC_AND_VALIDATE(temp_auth_rsp_buf[i].data_ptr,
                                               temp_auth_rsp_buf[i].data_len);
          }
                ...
                extra_param_ptr->isim_auth.auth_context == MMGSDI_AUTN_ISIM_GBA_NAF_DERIVATION_CONTEXT))
      {
        ...
        if(temp_auth_rsp_buf[0].data_len > 0 &&
           temp_auth_rsp_buf[0].data_len < (uim_rsp_ptr->rsp_data.data_len - offset))
        {
          MMGSDIUTIL_MEM_MALLOC_AND_VALIDATE(temp_auth_rsp_buf[0].data_ptr,
                                             temp_auth_rsp_buf[0].data_len);
        }
  ...
  MMGSDIUTIL_TMC_MEM_FREE(temp_auth_rsp_buf[0].data_ptr);
  MMGSDIUTIL_TMC_MEM_FREE(temp_auth_rsp_buf[1].data_ptr);
  MMGSDIUTIL_TMC_MEM_FREE(temp_auth_rsp_buf[2].data_ptr);
  ...
  mmgsdi_client_data_type            client_ref)
{
  ...
  UIM_MSG_HIGH_3("MMGSDI_SESSION_BUILD_SELECT_AID, session_id 0x%x, current_req_type: 0x%x, orig_req_type: 0x%x",
                 session_id, curr_req_type, orig_req_type);
  ...
  mmgsdi_memscpy((void*)file_name.file.app_id.data_ptr,
                 ...
                 (void*)aid.data_ptr,
                 file_name.file.app_id.data_len);
...
/* ----------------------------------------------------------------------------
   FUNCTION:      MMGSDI_SESSION_RELEASE_BY_SESSION_TYPE

   DESCRIPTION:
     This function will release sessions specific to given session type that
     has been deactivated and closed.
   ...
   SIDE EFFECTS:
     None
-----------------------------------------------------------------------------*/
static mmgsdi_return_enum_type mmgsdi_session_release_by_session_type (
  mmgsdi_session_type_enum_type session_type,
  mmgsdi_cmd_enum_type          orig_request_type
)
{
  uint8                     client_cnt       = 0;
  uint16                    session_cnt      = 0;
  mmgsdi_len_type           num_sessions     = 0;
  mmgsdi_session_info_type *session_info_ptr = NULL;
  mmgsdi_return_enum_type   mmgsdi_status    = MMGSDI_SUCCESS;
  ...
  for(client_cnt = 0; client_cnt < MMGSDI_MAX_CLIENT_INFO; client_cnt++)
  {
    ...
    for(session_cnt = 0; (num_sessions > 0) && (session_cnt < MMGSDI_MAX_SESSION_INFO); session_cnt++)
    {
      ...
      /* Continue if the session type does not match */
      if(session_info_ptr->session_type != session_type)
      {
        continue;
      }
  ...
  return mmgsdi_status;
} /* mmgsdi_session_release_by_session_type */
...
)
{
  ...
  /* If original request type is switch/activate then close all the sessions
     specific to the requested session type */
  if(req_header_ptr->orig_request_type == MMGSDI_SESSION_ACT_OR_SWITCH_PROV_REQ)
  {
    mmgsdi_status = mmgsdi_session_release_by_session_type(session_info_ptr->session_type,
                                                           req_header_ptr->orig_request_type);
  }
  else
  {
  ...
  const mmgsdi_uim_report_rsp_type *uim_rsp_ptr)
{
  ...
  else if (req_header.orig_request_type == MMGSDI_SESSION_ACT_OR_SWITCH_PROV_REQ)
  {
    ...
    if(mmgsdi_nv_get_device_feature_status(MMGSDI_FEATURE_CHANGE_INST_CLASS) == MMGSDI_FEATURE_ENABLED)
    {
  ...
  const mmgsdi_uim_report_rsp_type *uim_rsp_ptr)
{
  ...
  {
    /* Release all sessions specific to given session type */
    mmgsdi_status = mmgsdi_session_release_by_session_type(session_info_ptr->session_type,
                                                           req_header.orig_request_type);
    if(mmgsdi_status != MMGSDI_SUCCESS)
    {
      UIM_MSG_ERR_1("Failed to cleanup sessions of session type: 0x%x",
                    session_info_ptr->session_type);
    }
  }
  else
  ...
  const mmgsdi_uim_report_rsp_type *uim_rsp_ptr )
{
  ...
  else if (req_header.orig_request_type == MMGSDI_SESSION_DEACTIVATE_REQ)
  {
    if(report_status == MMGSDI_SUCCESS)
    {
      ...
      if(slot_data_ptr->onchip_sim_data.state != MMGSDI_ONCHIP_SIM_INIT)
      {
        ...
        mmgsdi_util_prov_session_deactivate_cleanup_and_notify(req_header.session_id, TRUE);
      }
      else
      {
        report_status = mmgsdi_onchip_deactivate_session(channel_info_ptr->slot_id,
                                                         app_info_ptr);
        if(report_status == MMGSDI_SUCCESS)
        {
          MMGSDIUTIL_ENTER_CLIENT_APP_DATA_CRIT_SECT;
          /* Reset the prov app indices*/
  ...
  else if (req_header.orig_request_type == MMGSDI_SESSION_APP_RESET_DEACTIVATE_REQ)
  {
    ...
    if(channel_info_ptr->app_info_ptr != NULL)
    {
      ...
      channel_info_ptr->app_info_ptr->deactivation_in_progress = FALSE;
        ...
        (req_header.orig_request_type == MMGSDI_SESSION_OPEN_WITH_SELECT_RSP_REQ))
    {
         ...
         extra_param_ptr->open_data.aid.aid.data_len <= MMGSDI_MAX_AID_LEN)
      {
        mmgsdi_memscpy((void*)app_id_data.data_ptr,
          ...
          extra_param_ptr->open_data.aid.aid.data_len);
        ...
        if (app_pin_tbl_pptr != NULL)
        {
          for(i=0; i<MMGSDI_MAX_EXT_APP_INFO; i++)
          {
                ...
                (session_info_ptr->session_type == MMGSDI_1X_PROV_TER_SESSION))))
            {
              ...
              (void)mmgsdi_cache_create_app_cache_table(app_pin_tbl_pptr[i]);
            }
          }
        }
        /* --------------------------------------------------------------
               ...
               (card_channel_index != channel_index))
            {
                   ...
                   MMGSDI_DEFAULT_CHANNEL)))
              {
                ...
                if (slot_data_ptr->csim_hiding_status == MMGSDI_CSIM_HIDDEN &&
                    ...
                    (mmgsdi_util_client_receives_hidden_csim_events(i) == FALSE))
                {
                  UIM_MSG_HIGH_1("session_rsp_open: CSIM hidden, so skipping channel mapping update for session_id=0x%x",
                                 mmgsdi_client_id_reg_table[i]->session_info_ptr[j]->session_id);
                  continue;
                }
                ...
                mmgsdi_client_id_reg_table[i]->session_info_ptr[j]->channel_info_index = channel_index;
              }
            }
          } /* for j<MAX_SESSION_INFO */
        } /* for i<MAX_CLIENT_INFO */
...
)
{
 ...
 /* -----------------------------------------------------------
  ...
  while(num_sessions_requested)
  {
    ...
    do
    {
      ...
      /* Populate following entities in session record from req_ptr
         ...
         4. notify_init
         Get the next available id. Use least significant 32 bits to maintain legacy behavior. */
    ...
    if (mmgsdi_util_is_prov_session(session_ptr->session_type))
    {
      ...
      for(i = 0; i < MMGSDI_MAX_CLIENT_INFO; i++)
      {
        ...
        for(j = 0; (num_sessions > 0) && (j < MMGSDI_MAX_SESSION_INFO); j++)
        {
          ...
          num_sessions--;
          if(mmgsdi_client_id_reg_table[i]->session_info_ptr[j]->session_type == session_ptr->session_type)
          {
            slot_data_ptr = NULL;

            if((mmgsdi_client_id_reg_table[i]->session_info_ptr[j]->channel_info_index < MMGSDI_MAX_CHANNEL_INFO) &&
               (mmgsdi_channel_info_ptr_table[mmgsdi_client_id_reg_table[i]->session_info_ptr[j]->channel_info_index] != NULL))
            {
              /* Get the slot data ptr */
              slot_data_ptr = mmgsdi_util_get_slot_data_ptr(
                                mmgsdi_channel_info_ptr_table[mmgsdi_client_id_reg_table[i]->session_info_ptr[j]->channel_info_index]->slot_id);
              if(slot_data_ptr == NULL)
              {
                UIM_MSG_ERR_1("Invalid slot ptr mapped for session id=0x%x",
                              session_ptr->session_id);
                mmgsdi_status = MMGSDI_ERROR;
              }
              /* If CSIM is hidden and it is a 1X session, and this session_open request
                 was for a client other than MMGDSI and QMI, make sure not to associate
                 channel with this session_id so as to avoid sending them CSIM related
                 events */
              else if(!(slot_data_ptr->csim_hiding_status == MMGSDI_CSIM_HIDDEN &&
                        ...
                        (mmgsdi_util_client_receives_hidden_csim_events(client_index) == FALSE)))
              {
    ...
    else if (mmgsdi_util_is_card_session(session_ptr->session_type))
    {
      ...
      mmgsdi_status = mmgsdi_util_get_mmgsdi_session_on_ch_zero(card_session_slot,
                                                                NULL,
                                                                &temp_session_id);
  ...
  MMGSDIUTIL_TMC_MEM_FREE( session_cnf_ext_info.session_info_ptr);
...
)
{
  ...
  if(mmgsdi_util_is_non_prov_session(msg_ptr->session_type))
  {
    ...
    do
    {
        ...
        (msg_ptr->non_prov_app_data.exclusive_channel))
      {
        ...
        /* Adding fake status words, as if the transaction was performed with the card, 
           to differentiate from the case where there is no response from the card at all */
        status_word.sw1 = GSDI_SW1_COMMAND_NOT_ALLOWED;
        status_word.sw2 = GSDI_SW2_CNDTNS_OF_USE_NOT_SATISFIED;
      }
    }while(0);
    ...
    if(mmgsdi_status != MMGSDI_SUCCESS)
    {
      return mmgsdi_cnf_build_and_queue(mmgsdi_status,
                                        ...
                                        NULL,
                                        &session_cnf_info,
                                        status_word.sw1 != 0x0 ? TRUE : FALSE,
                                        status_word);
    }
  ...
  if(session_ptr == NULL)
  {
    ...
    /* Adding fake status words, as if the transaction was performed with the card, 
       to differentiate from the case where there is no response from the card at all */
    status_word.sw1 = GSDI_SW1_FUNCTIONS_IN_CLA_ERROR;
    status_word.sw2 = GSDI_SW2_LOGICAL_CHANNEL_NOT_SUPPORTED;                  
...
)
{
  ...
  if (mmgsdi_util_is_prov_session(session_info_ptr->session_type))
  {
    ...
    if(msg_ptr->request_header.orig_request_type == MMGSDI_SESSION_ACT_OR_SWITCH_PROV_REQ &&
       channel_info_ptr != NULL &&
  ...
  else if (mmgsdi_util_is_non_prov_session(session_info_ptr->session_type))
  {
    ...
    else if(channel_info_ptr->num_connections == 1)
    {
      ...
      {
        mmgsdi_util_free_client_request_table_index(client_dummy_index);
        MMGSDIUTIL_TMC_MEM_FREE(extra_param_dummy_ptr);
        return mmgsdi_status;
      }
...
)
{
  ...
  if(mmgsdi_refresh_info_ptr[slot_index] != NULL)
  {
    if(mmgsdi_refresh_info_ptr[slot_index]->mode == MMGSDI_REFRESH_APP_RESET_AUTO)
    {
      ...
      mmgsdi_status = mmgsdi_util_get_client_request_table_info(
                        mmgsdi_refresh_info_ptr[slot_index]->request_header.client_data,
                        &req_header,
                        &extra_param_ptr);
...
)
{
  ...
  UIM_MSG_MED_3("MCFG REFRESH status in nv - 0x%x nv_status 0x%x uim_select_default_usim_app 0x%x",
                 nv_refresh_enabled,
                 nv_status,
  ...
  /* MCFG NV refresh gets delayed if emergency call is in progress or device is
     ...
     Do not reject session activation while emergency call in progress if auto
     activation is enabled. This is required to avoid card in detected state for
     ever as there is no activation retry */
  if(nv_status == NV_DONE_S &&
     !mmgsdi_nv_data.item_value.uim_select_default_usim_app &&
     nv_refresh_enabled &&
     (mmgsdi_generic_data_ptr == NULL ||
      mmgsdi_generic_data_ptr->cm_call_info[prov_index].emergency_call_active ||
  ...
  nv_context = mmgsdi_util_get_efs_item_index_for_slot(msg_ptr->request_header.slot_id);
  ...
  {
    UIM_MSG_ERR_2("CDMALess NV is enabled for context 0x%x slot 0x%x; not activating",
                  nv_context, msg_ptr->request_header.slot_id);
    return MMGSDI_NOT_SUPPORTED;
  }
  ...
  switch(session_status)
  {
    case MMGSDI_SESSION_ID_ASSIGNED_AND_ACTIVE:
      /* If this is auto-activation, send ERROR as session
         is already activated by client */
      if (msg_ptr->is_auto_activation)
      {
        UIM_MSG_ERR_0("Session is already activated by client");
        return MMGSDI_ERROR;
      }
      ...
      else
      {
        ...
        if(mmgsdi_util_is_prov_session(session_ptr->session_type))
        {
          ...
          {
            UIM_MSG_ERR_0("Can not switch or activate onchip session that is active on a different slot");
            return MMGSDI_ERROR;
          }
    ...
    case MMGSDI_SESSION_ID_ASSIGNED_NOT_ACTIVE:
      /* 1. Session is NOT mapped with an channel entry in
            mmgsdi_channel_info_ptr_table[] in this case we can go ahead
            with session activation when onchip is activated */
      if (MMGSDI_ONCHIP_SIM_INIT == slot_data_ptr->onchip_sim_data.state)
      {
        return mmgsdi_onchip_activate_session(msg_ptr);
      }
...
/* ----------------------------------------------------------------------------
   FUNCTION:      MMGSDI_SESSION_CANCEL_SYNC_CMD

   DESCRIPTION:
     This function will signal the slot task to come out of wait for a sync
     command. Then the main task waits for acknowledgement of cancel signal.
...
-------------------------------------------------------------------------------*/
static void mmgsdi_session_cancel_sync_cmd(
...
)
{
  ...
  /* Sync commands sent by slot tasks can only be canceled from the main task
     as the deactivation is supposed to be processed in main task */
  if (mmgsdi_util_get_current_task() != MMGSDI_TASK_MAIN)
  {
    return;
  }
  ...
  /* Set boolean for slot task to trigger cancel ack sig */
  mmgsdi_need_sync_cmd_cancel_ack[slot_index] = TRUE;

  UIM_MSG_HIGH_1("canceling sync cmd in progress for task 0x%x", mmgsdi_task);

  /* Leave the task critical section to give the slot task a chance
     to respond with the CANCEL_ACK signal */
  MMGSDIUTIL_LEAVE_TASK_SYNC_CRIT_SECT;

  /* Wait for acknowledgement of sync command cancellation from slot task. */
  sigs_rcvd = mmgsdi_wait(MMGSDI_SYNC_CMD_CANCEL_ACK_SIG);

  MMGSDIUTIL_ENTER_TASK_SYNC_CRIT_SECT;

  if (sigs_rcvd & MMGSDI_SYNC_CMD_CANCEL_ACK_SIG)
  {
    UIM_MSG_HIGH_0("received MMGSDI_SYNC_CMD_CANCEL_ACK_SIG");
    (void)rex_clr_sigs(rex_self(), MMGSDI_SYNC_CMD_CANCEL_ACK_SIG);

    /* Set SYNC_CMD_CANCEL_ACK signal back to slot specific task. Slot specific
       task is waiting for this signal to make sure that main task gets chance to
       run first after cacncelling the command. It is possible that when slot
       task sets this signal for main task, any other signal has already set for
       slot specific task. Both tasks are in ready state and any of them get
       chance to run. */
    (void)mmgsdi_util_set_signal_for_task(mmgsdi_task, MMGSDI_SYNC_CMD_CANCEL_ACK_SIG);
  }
} /* mmgsdi_session_cancel_sync_cmd */
...
)
{
  ...
  boolean                           cancel_pending_commands = FALSE;
  mmgsdi_channel_info_type         *channel_info_ptr        = NULL;
  mmgsdi_session_info_type         *session_info_ptr        = NULL;
  ...
  mmgsdi_status = mmgsdi_util_get_session_app_info(
                    msg_ptr->request_header.session_id,
                    &slot_id, NULL, &app_info_ptr, NULL, &session_info_ptr, NULL);
  ...
  /* Cancelling pending commands when app deactivates due to refresh app reset */
  if(msg_ptr->request_header.orig_request_type == MMGSDI_SESSION_APP_RESET_DEACTIVATE_REQ)
  {
    cancel_pending_commands = TRUE;
  }

  UIM_MSG_MED_3("app_type: 0x%x, skip_app_termination: 0x%x, session_type: 0x%x",
                app_info_ptr->app_data.app_type,
                skip_app_termination,
                (session_info_ptr != NULL) ? session_info_ptr->session_type : MMGSDI_MAX_SESSION_TYPE_ENUM);
  ...
  /* For the session_type and slot_id on which this deactivation request
     is being processed, before proceeding with the logical deactivation,
     lets cancel all the pending commands.
     This includes the commands
     1. being processed in the slot task that are currently sitting in the
        client req table, possibly waiting for uimdrv response.
     2. sitting in the slot task command queue.
     NOTE that we could be here either because of a direct DEACTIVATION
     request or as part of an ACT_OR_SWITCH where a deactivation is required
     before proceeding with the activation. For the latter case we should
     make sure to not cancel the ACT_OR_SWITCH request(being processed in slot
     task) that would be sitting in the client req table.
     The index of this ACT_OR_SWITCH request in the client req table is stored
     in the client_data of the current deactivation request. Retrieve that
     client index and make sure we do not flush the command at that client index
     in client req table */
  if (cancel_pending_commands)
  {
    mmgsdi_session_info_type       *session_info_ptr = NULL;
       ...
       channel_info_ptr != NULL)
    {
      ...
      uint8                              skip_client_index       = 0xFF;
      uint8                              num_skip_client_indices = 0;
      mmgsdi_request_header_type         req_header;
      mmgsdi_client_req_extra_info_type *extra_param_ptr         = NULL;
      boolean                            sync_cmd_in_progress    = FALSE;
      ...
      /* Retrieve the original session switch/activate request */
      mmgsdi_status = mmgsdi_util_get_client_request_table_info(
                        msg_ptr->request_header.client_data,
                        &req_header,
                        &extra_param_ptr);
      if (mmgsdi_status == MMGSDI_SUCCESS &&
          req_header.orig_request_type == MMGSDI_SESSION_ACT_OR_SWITCH_PROV_REQ)
      {
        /* Do not flush command at this index in client req table */
        skip_client_index = (uint8)msg_ptr->request_header.client_data;
        num_skip_client_indices = 1;
      }
      mmgsdi_flush_commands_in_progress(mmgsdi_util_get_task_from_slot(channel_info_ptr->slot_id),
                                        cancel_info,
                                        &skip_client_index,
                                        num_skip_client_indices,
                                        &sync_cmd_in_progress);

      if (sync_cmd_in_progress)
      {
        mmgsdi_session_cancel_sync_cmd(mmgsdi_util_get_task_from_slot(channel_info_ptr->slot_id));
      }

      /* Ensure passing FALSE as the second param below. This will make sure
         we do not end up duplicating what was done as part of
         mmgsdi_flush_commands_in_progress above. If mmgsdi_process_cancel_req
         is called with TRUE, it will remove all the commands pending uimdrv
         response on ALL the mmgsdi tasks, which is not the intention here...
         We only want to cancel the commands pending uimdrv response on the
         mmgsdi slot task corresponding to the slot on which this deactivation
         is requested */
...
/* ----------------------------------------------------------------------------
  FUNCTION MMGSDI_SESSION_CONTINUE_DEACTIVATE_CB

  DESCRIPTION
    This function is defined as dummy callback for CONTINUE_DEACTIVATE_REQ
  ...
  SIDE EFFECTS
    None
-------------------------------------------------------------------------------*/
static void mmgsdi_session_continue_deactivate_cb(
  mmgsdi_return_enum_type  status,
  mmgsdi_cnf_enum_type     cnf,
  const mmgsdi_cnf_type   *cnf_ptr)
{
  ...
  UIM_MSG_MED_1("mmgsdi_session_continue_deactivate_cb status: 0x%x", status);
} /* mmgsdi_session_continue_deactivate_cb */


/* ----------------------------------------------------------------------------
  FUNCTION MMGSDI_SESSION_QUEUE_CONTINUE_DEACTIVATE_REQ

  DESCRIPTION
    This function builds and queues the CONTINUE_DEACTIVATE_REQ
  ...
  SIDE EFFECTS
    None
-------------------------------------------------------------------------------*/
mmgsdi_return_enum_type mmgsdi_session_queue_continue_deactivate_req(
  uint32             client_req_index)
{
  mmgsdi_return_enum_type                 mmgsdi_status   = MMGSDI_ERROR;
  mmgsdi_task_cmd_type                   *task_cmd_ptr    = NULL;
  mmgsdi_continue_deactivate_req_type    *deact_msg_ptr   = NULL;
  uint32                                  task_cmd_len    = 0;
  ...
  deact_msg_ptr = &task_cmd_ptr->cmd.cmd.continue_deact_req;

  /*---------------------------------------------------------------------------
    Populating mmgsdi_continue_deact_req_type header
  ---------------------------------------------------------------------------*/
  task_cmd_ptr->cmd.cmd_enum                      = MMGSDI_CONTINUE_DEACTIVATE_REQ;
  deact_msg_ptr->request_header.request_type      = MMGSDI_CONTINUE_DEACTIVATE_REQ;
  deact_msg_ptr->request_header.orig_request_type = MMGSDI_CONTINUE_DEACTIVATE_REQ;
  deact_msg_ptr->request_header.response_cb       = mmgsdi_session_continue_deactivate_cb;
  deact_msg_ptr->client_req_index                 = client_req_index;

  /* Queue the command */
  mmgsdi_status = mmgsdi_cmd(task_cmd_ptr);

  UIM_MSG_HIGH_1("Queue of MMGSDI_CONTINUE_DEACTIVATE_REQ, status: 0x%x",
                 mmgsdi_status);
  if (mmgsdi_status != MMGSDI_SUCCESS)
  {
    /* Free the pointer since the task_cmd_ptr has not been put
  ...
  return mmgsdi_status;
} /* mmgsdi_session_queue_continue_deactivate_req */


/* ----------------------------------------------------------------------------
   FUNCTION:      MMGSDI_SESSION_DEACTIVATION_PAUSE_TIMER_CB

   DESCRIPTION:
     This function is called up expiration of the deactivation pause timer and
     queues the CONTINUE_DEACTIVATE_REQ to MMGSDI
...
-------------------------------------------------------------------------------*/
static void mmgsdi_session_deactivation_pause_timer_cb(
  unsigned long   client_data)
{
  mmgsdi_return_enum_type                 mmgsdi_status   = MMGSDI_SUCCESS;

  UIM_MSG_HIGH_1("mmgsdi_session_deactivation_pause_timer_cb: client_data: 0x%x",
                 client_data);

  if (client_data >= MMGSDI_MAX_CLIENT_REQ_INFO)
  {
    return;
  }

  mmgsdi_status = mmgsdi_session_queue_continue_deactivate_req(client_data);

  if (mmgsdi_status != MMGSDI_SUCCESS)
  {
    UIM_MSG_ERR_1("error 0x%x queuing MMGSDI_CONTINUE_DEACTIVATE_REQ", mmgsdi_status);
  }
} /* mmgsdi_session_deactivation_pause_timer_cb */


/* ----------------------------------------------------------------------------
   FUNCTION:      MMGSDI_SESSION_PAUSE_DEACTIVATION_IF_NEEDED

   DESCRIPTION:
     This function will start the deactivation pause timer if there any
     commands in queue or in progress.
   ...
   RETURN VALUE:
     TRUE:  If the deactivation pause timer was started
     FALSE: Otherwise

   SIDE EFFECTS:

-------------------------------------------------------------------------------*/
boolean mmgsdi_session_pause_deactivation_if_needed(
  mmgsdi_session_id_type session_id)
{
  uint8                            num_cmds           = 0;
  uint8                            slot_index         = MMGSDI_MAX_NUM_SLOTS;
  ...
  mmgsdi_return_enum_type          mmgsdi_status      = MMGSDI_ERROR;
  mmgsdi_session_info_type        *session_info_ptr   = NULL;
  mmgsdi_attribute_enum_type       attr_type          = MMGSDI_ATTR_UNKNOWN;
  mmgsdi_technology_enum_type      tech_type          = MMGSDI_TECH_UNKNOWN;
  uim_common_efs_status_enum_type  efs_access_status  = UIM_COMMON_EFS_ERROR;
  uint32                           pause_interval     = 0;
  timer_attrib_struct_type         timer_attributes   = {0};

  mmgsdi_status = mmgsdi_util_get_session_app_info(session_id, &slot_id,
                                                   NULL, NULL, NULL,
                                                   &session_info_ptr, NULL);
  if (mmgsdi_status != MMGSDI_SUCCESS || session_info_ptr == NULL)
  {
  ...
  /* Client req index of the deactivate req is retrieved from the refresh info
     pointer so ensure that it is valid. */
  if (mmgsdi_refresh_info_ptr[slot_index] == NULL)
  {
    return FALSE;
  }

  /* Pause deactivation only for GW sessions with the assumption that 1x
     clients do not have any write operations during deactivation that would
     require pausing the deactivation */
  mmgsdi_status = mmgsdi_util_get_tech_and_attr_from_session_type(session_info_ptr->session_type,
                                                                  &tech_type,
                                                                  &attr_type);
  if (mmgsdi_status != MMGSDI_SUCCESS || tech_type != MMGSDI_TECH_3GPP)
  {
    return FALSE;
  }

  /* Return true to pause if a timer is already running for this session type.
     If the timer has expired, return FALSE to proceed with deactivation */
  if(timer_get_params(&mmgsdi_session_deactivation_pause_timer[attr_type],
                      &timer_attributes) == TE_SUCCESS)
  {
    if(timer_attributes.timer_state == TIMER_SET_FLAG)
    {
      return TRUE;
    }
    else if (timer_attributes.timer_state == TIMER_EXPIRED_FLAG)
    {
      return FALSE;
    }
  }
  ...
  /* Deactivation is paused when there are
     1. commands in the slot task or main task queues, or
     2. commands in progress for the current slot */
  num_cmds = mmgsdi_util_find_commands_in_queue_for_session_type(
               &mmgsdi_task_cmd_q[mmgsdi_task], session_info_ptr->session_type) +
             mmgsdi_util_find_commands_in_queue_for_session_type(
               &mmgsdi_high_pri_cmd_q[mmgsdi_task], session_info_ptr->session_type) +
             mmgsdi_util_find_commands_in_queue_for_session_type(
               &mmgsdi_task_cmd_q[MMGSDI_TASK_MAIN], session_info_ptr->session_type) +
             mmgsdi_util_find_commands_in_queue_for_session_type(
               &mmgsdi_high_pri_cmd_q[MMGSDI_TASK_MAIN], session_info_ptr->session_type);
  ...
  if (num_cmds == 0)
  {
    for (i = 0; i < MMGSDI_MAX_CLIENT_REQ_INFO; i++)
    {
      ...
      /* Do not count the ongoing deactivation and refresh as pending cmds */
      if ((mmgsdi_client_req_table_info_ptr[i]->session_id == session_id &&
           mmgsdi_client_req_table_info_ptr[i]->request_type == MMGSDI_SESSION_DEACTIVATE_REQ) ||
          (mmgsdi_client_req_table_info_ptr[i]->request_type == MMGSDI_REFRESH_REQ &&
           mmgsdi_client_req_table_info_ptr[i]->extra_param_ptr != NULL &&
           mmgsdi_client_req_table_info_ptr[i]->extra_param_ptr->refresh_data.mode == MMGSDI_REFRESH_APP_RESET_AUTO))
      {
        continue;
      }
      else
      {
        if (mmgsdi_util_get_session_type(mmgsdi_client_req_table_info_ptr[i]->session_id,
            ...
            cmd_session_type == session_info_ptr->session_type)
        {
          num_cmds++;
          break;
        }
  ...
  /* Use timer interval from EFS if available, otherwise use the default value
     set in MMGSDI_SESSION_DEACTIVATION_PAUSE_TIMER*/
  efs_access_status = uim_common_efs_read(
                        UIM_COMMON_EFS_MMGSDI_DEACTIVATION_TIMER,
                        UIM_COMMON_EFS_ITEM_FILE_TYPE,
                        UIM_COMMON_EFS_DEVICE,
                        (uint8 *)&pause_interval,
                        sizeof(pause_interval));

  if (efs_access_status == UIM_COMMON_EFS_SUCCESS && pause_interval == 0)
  {
    return FALSE;
  }
  else if (efs_access_status != UIM_COMMON_EFS_SUCCESS)
  {
    pause_interval = MMGSDI_SESSION_DEACTIVATION_PAUSE_TIMER;
  }

  /* The client_req_table index of the deactivation request is stored in
     the refresh_info_ptr for this slot. Pass that index to the timer so that
     it can be used to build the deactivation request upon timer expiry */
  rex_def_timer_ex(&mmgsdi_session_deactivation_pause_timer[attr_type],
                   mmgsdi_session_deactivation_pause_timer_cb,
                   (unsigned long)mmgsdi_refresh_info_ptr[slot_index]->request_header.client_data);

  UIM_MSG_HIGH_2("pausing session deactivation for 0x%x ms for session type 0x%x",
                 pause_interval, session_info_ptr->session_type);
  (void)rex_set_timer(&mmgsdi_session_deactivation_pause_timer[attr_type],
                      pause_interval);

  return TRUE;
} /* mmgsdi_session_pause_deactivation_if_needed */
...
)
{
  ...
  UIM_MSG_HIGH_3("MMGSDI_SESSION_PROC_DEACTIVATE, orig req 0x%x internal_deact: 0x%x, is_reason_mismatch: 0x%x",
                 msg_ptr->request_header.orig_request_type,
                 msg_ptr->internal_deact_req,
                 msg_ptr->is_reason_mismatch);
...
/* ----------------------------------------------------------------------------
   FUNCTION:      MMGSDI_SESSION_PROC_CONTINUE_DEACTIVATE

   DESCRIPTION:
     This function will process the continue deactivation request
...
-----------------------------------------------------------------------------*/
static mmgsdi_return_enum_type  mmgsdi_session_proc_continue_deactivate (
  mmgsdi_continue_deactivate_req_type      * msg_ptr
)
{
  mmgsdi_session_deact_req_type             deact_req;
  uint32                                    index = 0;
  mmgsdi_session_type_enum_type             session_type = MMGSDI_MAX_SESSION_TYPE_ENUM;
  mmgsdi_attribute_enum_type                attr_type    = MMGSDI_ATTR_UNKNOWN;
  mmgsdi_technology_enum_type               tech_type    = MMGSDI_TECH_UNKNOWN;
  timer_attrib_struct_type                  timer_attr   = {0};

  MMGSDIUTIL_RETURN_IF_NULL(msg_ptr);

  index = msg_ptr->client_req_index;
  ...
  memset(&deact_req, 0, sizeof(deact_req));

  /* Populate request header from the client_req_table entry. */
  deact_req.request_header.client_id = mmgsdi_client_req_table_info_ptr[index]->client_id;
  deact_req.request_header.session_id = mmgsdi_client_req_table_info_ptr[index]->session_id;
  deact_req.request_header.slot_id = mmgsdi_client_req_table_info_ptr[index]->slot_id;
  deact_req.request_header.response_cb = mmgsdi_internal_refresh_app_reset_auto_cb;
  deact_req.request_header.client_data = index;

  deact_req.request_header.request_type = MMGSDI_SESSION_DEACTIVATE_REQ;
  deact_req.request_header.orig_request_type = MMGSDI_SESSION_DEACTIVATE_REQ;

  if (mmgsdi_util_get_session_type(deact_req.request_header.session_id,
                                   &session_type, NULL) != MMGSDI_SUCCESS)
  {
    return MMGSDI_ERROR;
  }

  if ((mmgsdi_util_get_tech_and_attr_from_session_type(session_type,
                                                       &tech_type,
                                                       &attr_type) != MMGSDI_SUCCESS) ||
      tech_type != MMGSDI_TECH_3GPP)
  {
    return MMGSDI_ERROR;
  }

  /* Udefine the timer if not already done so. */
  timer_get_params(&mmgsdi_session_deactivation_pause_timer[attr_type],
                   &timer_attr);
  if(timer_attr.timer_state != TIMER_UNDEF_FLAG)
  {
    (void)rex_undef_timer(&mmgsdi_session_deactivation_pause_timer[attr_type]);
  }

  return mmgsdi_session_deactivate_start(&deact_req);
} /* mmgsdi_session_proc_continue_deactivate */
...
)
{
  ...
  switch (cmd)
  {
    ...
    case MMGSDI_CONTINUE_DEACTIVATE_REQ:
      mmgsdi_status = mmgsdi_session_proc_continue_deactivate(
                        &cmd_ptr->cmd.continue_deact_req);
      break;
  ...
  boolean                                           *mismatch_or_error_detected_ptr)
{
  ...
  *mismatch_or_error_detected_ptr = TRUE;
  ...
  if (mmgsdi_status == MMGSDI_SUCCESS)
  {
    ...
    /* De-activate pair session if there is mis-match and
       the incoming request is not auto-activation */
    if (mmgsdi_status == MMGSDI_SUCCESS && slot_id != msg_ptr->request_header.slot_id)
    {
      if (msg_ptr->is_auto_activation)
      {
        UIM_MSG_ERR_2("Pair session_type: 0x%x is active on other Slot: 0x%x",
                      pair_session_type, slot_id);
        return MMGSDI_ERROR;
      }
      mmgsdi_status = mmgsdi_session_deactivate_found_mismatch(msg_ptr,
                                                               pair_session_id,
  ...
  for (attr_iterator = MMGSDI_ATTR_PRIMARY; attr_iterator <= MMGSDI_ATTR_TERTIARY; attr_iterator++)
  {
    ...
    for (tech_iterator = MMGSDI_TECH_3GPP; tech_iterator <= MMGSDI_TECH_3GPP2; tech_iterator++)
    {
      ...
      {
        /* If auto-activation then no need to proceed as client already
           activated destination app on some other session */
        if (msg_ptr->is_auto_activation)
        {
          UIM_MSG_ERR_2("Other attr session_type: 0x%x is active on same Slot: 0x%x",
                        current_session_type, current_slot);
          return MMGSDI_ERROR;
        }
  ...
  /* If dest_app is active on some session and the current request is not
     auto-activation then de-activate the corresponding session first and
     then deactivate the dest_app session at the end */
...
/* ============================================================================
FUNCTION        MMGSDI_SESSION_QUEUE_CLOSE_REQ_TO_TASK

DESCRIPTION
  This function is used to queue the session close command to a particular
  slot task.
...
SIDE EFFECTS:
    None
=============================================================================== */
mmgsdi_return_enum_type  mmgsdi_session_queue_close_req_to_task (
  mmgsdi_session_id_type       session_id,
  boolean                      terminate,
  mmgsdi_task_enum_type        mmgsdi_task,
  mmgsdi_callback_type         response_cb_ptr,
...
)
{
  ...
  return mmgsdi_status;
}/* mmgsdi_session_queue_close_req_to_task */
