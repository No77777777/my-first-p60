...
$Header: //components/rel/uim.mpss/6.1.0/mmgsdi/src/mmgsdi_gen.c#3 $$ $DateTime: 2020/02/11 06:59:18 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
02/11/19   tq      Fix KW issue
...
01/07/16   sp      Correct sanity checks for prl read
...
08/05/15   av      Introduce MMGSDI event for indicating recovery completion
07/06/15   tl      Remove error status in case of clearing the GET RESPONSE cache
06/29/15   tl      Add backup to Get Response APDU in case not in cache
  ...
  uim_protocol_type        protocol)
{
  ...
  uim_cmd_ptr->hdr.user_data = MMGSDI_MAX_CLIENT_REQ_INFO;
  ...
  const mmgsdi_card_reset_req_type  *req_ptr)
{
  ...
  /* Populate the power up method option to UIM*/
  if (req_ptr->pup_option == MMGSDI_CARD_POWER_UP_NON_TELECOM)
  {
    uim_cmd_ptr->reset.pup_option = UIM_PUP_OPTION_NON_TELECOM;
  }
  else
  {
    uim_cmd_ptr->reset.pup_option = UIM_PUP_OPTION_TELECOM;
  }
  ...
  mmgsdi_client_data_type            client_ref)
{
  ...
  card_reset_ptr->pup_option  = pup_option;
...
)
{
  ...
  ** length checks to ensure we can protect against buffer overrun*/
  ...
  UIM_MSG_HIGH_2("Req. length = 0x%x, Get Resp. cache total length 0x%x",
  ...
  {
    MMGSDIUTIL_TMC_MEM_FREE(mmgsdi_apdu_get_rsp_table_ptr[tbl_index]->apdu.data_ptr);
...
)
{
  ...
  /* --------------------------------------------------------------------------
     ...
     ------------------------------------------------------------------------*/
  channel_id = mmgsdi_util_get_channel_id_from_cla(req_ptr->data.data_ptr[0]);
  ...
  report_ptr->rpt.stream_iso7816_apdu.num_data_bytes = uint32touint16(get_rsp.data_len);
  ...
  /* --------------------------------------------------------------------------
     ...
     ----------------------------------------------------------------------- */
  MMGSDIUTIL_TMC_MEM_FREE(report_ptr->rpt.stream_iso7816_apdu.data_ptr);
  MMGSDIUTIL_TMC_MEM_FREE(report_ptr);
...
)
{
  ...
  if (req_ptr->data.data_ptr[1] == GET_RESPONSE /* For UICC && ICC */ )
  {
   ...
   /* ---------------------------------------------------------------------------
     ...
     ------------------------------------------------------------------------ */
  if(mmgsdi_nv_get_device_feature_status(MMGSDI_FEATURE_ALLOW_SEND_APDU_ONLY) == MMGSDI_FEATURE_ENABLED
  ...
  mmgsdi_status = mmgsdi_uim_common_send_apdu(req_ptr);
...
)
{
  ...
  mmgsdi_onchip_sim_config_data_type  mmgsdi_onchip_data          = {0, NULL};
...
FUNCTION mmgsdi_gen_process_hide_csim_req

DESCRIPTION
    This function processes the request to hide/unhide CSIM from modem clients
    by sending fake indications to modem clients regarding DEACTIVATION and
    ACTIVATION of CSIMs on the card

DEPENDENCIES
    None

RETURN VALUE
    MMGSDI_SUCCESS: If CSIMs succesfully hidden/unhidden
    MMGSDI_ERROR:   Otherwise

SIDE EFFECTS
    None
===========================================================================*/
mmgsdi_return_enum_type mmgsdi_gen_process_hide_csim_req(
   const mmgsdi_hide_csim_req_type *req_ptr
)
{
  int32                         i                         = 0;
  int32                         j                         = 0;
  uint8                         channel_info_index_1x_pri = MMGSDI_MAX_CHANNEL_INFO;
  uint8                         channel_info_index_1x_sec = MMGSDI_MAX_CHANNEL_INFO;
  uint8                         channel_info_index_1x_ter = MMGSDI_MAX_CHANNEL_INFO;
  uint8                         client_channel_info_index = MMGSDI_MAX_CHANNEL_INFO;
  ...
  if (!mmgsdi_util_is_slot_valid(req_ptr->request_header.slot_id))
  {
    UIM_MSG_ERR_0("Invalid slot id in request");
    return MMGSDI_ERROR;
  }

  if (!mmgsdi_generic_data_ptr)
  {
    UIM_MSG_ERR_0("mmgsdi_generic_data_ptr NULL");
    return MMGSDI_ERROR;
  }
  ...
  if(slot_data_ptr == NULL)
  {
    UIM_MSG_ERR_0("slot_data_ptr NULL");
    return MMGSDI_ERROR;
  }

  UIM_MSG_MED_3("MMGSDI GEN HIDE CSIM CMD hide=0x%x; current hiding status=0x%x on slot=0x%x",
                req_ptr->hide_csim, slot_data_ptr->csim_hiding_status, req_ptr->request_header.slot_id);

  if (req_ptr->hide_csim)
  {
    /* Request is for hiding from modem clients all the CSIMs on the slot
       that are active. Lets loop over all the MMGSDI 1X sessions and for
       the MMGSDI 1X sessions belonging to this slot, hide the corresponding
       activated CSIMs from the MMGSDI clients other than MMGSDI and QMI_UIM. */
    mmgsdi_session_id_type     mmgsdi_1x_session_ids[] =
    ...
    for (i = 0; i < sizeof(mmgsdi_1x_session_ids)/sizeof(mmgsdi_1x_session_ids[i]); i++)
    {
      mmgsdi_slot_id_enum_type slot                    = MMGSDI_MAX_SLOT_ID_ENUM;

      if ((MMGSDI_SUCCESS == mmgsdi_util_get_session_app_info(mmgsdi_1x_session_ids[i], &slot,
                                                             NULL, NULL, NULL, NULL, NULL)) &&
          (req_ptr->request_header.slot_id == slot))
      {
        /* Lets send out session_deactivated to all the modem clients of MMGSDI.
           If there is no app activated for the MMGSDI session retrieved above,
           the following would simply be a no-op */
        mmgsdi_util_update_session_closed_cause(mmgsdi_1x_session_ids[i],
                                                MMGSDI_SESSION_CLOSED_CAUSE_CSIM_HIDDEN);

        /* Make sure to call this function only after updating the session_closed_cause
           to MMGSDI_SESSION_CLOSED_CAUSE_CSIM_HIDDEN. session_changed 'deactivated'
           events with such cause are NOT sent to MMGSDI and QMI_UIM */
        mmgsdi_util_prov_session_deactivate_cleanup_and_notify(mmgsdi_1x_session_ids[i],
                                                               FALSE);

        /* Remeber in MMGSDI global that CSIMs on the card are hidden from
           MMSGDI's modem clients */
        MMGSDIUTIL_ENTER_CLIENT_APP_DATA_CRIT_SECT;
        slot_data_ptr->csim_hiding_status = MMGSDI_CSIM_HIDDEN;
        MMGSDIUTIL_LEAVE_CLIENT_APP_DATA_CRIT_SECT;
      }
    }
  }
  else
  {
    ...
    /* Request is for unhiding the CSIMs on the requested slot for the 1x
       sessions of various modem clients of MMGSDI.
       Reassociate 1x session ids of all the modem clients
       with the channels on which the CSIM is active and send session_activated
       and sub_ready events to them for the active CSIMs */
    slot_data_ptr->csim_hiding_status = MMGSDI_CSIM_UNHIDING;

    /* This is the procedure we follow here:
       1. From the client reg table, find out the channel_info_index associated
          with MMGSDI_1X_PRI, MMGSDI_1X_SEC and MMGSDI_1X_TER sessions.
       2. Once we have each of those channel_info_indexes,
          Loop through the client reg table, and for every client (except
          MMGSDI and QMIUIM, which will already have a valid channel_info_index)
          having a 1X session open, associate 1X_PRI session with the
          channel_info_index retrieved above for MMGSDI_1X_PRI session. Likewise
          for the SEC and TER sessions of the client. */
      ...
      for(j = 0; (num_sessions > 0) && (j < MMGSDI_MAX_SESSION_INFO); j++)
      {
        ...
        if((mmgsdi_client_id_reg_table[i]->session_info_ptr[j]->session_type ==
           ...
           (mmgsdi_client_id_reg_table[i]->session_info_ptr[j]->session_id ==
            mmgsdi_generic_data_ptr->pri_1x_session_id) &&
           (mmgsdi_client_id_reg_table[i]->session_info_ptr[j]->channel_info_index < MMGSDI_MAX_CHANNEL_INFO) &&
           ...
           (mmgsdi_channel_info_ptr_table[mmgsdi_client_id_reg_table[i]->session_info_ptr[j]->channel_info_index]->slot_id ==
             req_ptr->request_header.slot_id) &&
           (channel_info_index_1x_pri == MMGSDI_MAX_CHANNEL_INFO))
        {
          /* This is MMGSDI's 1x PRI index in client reg table... retrieve the
             channel_id */
          channel_info_index_1x_pri = mmgsdi_client_id_reg_table[i]->session_info_ptr[j]->channel_info_index;
        }
        else if((mmgsdi_client_id_reg_table[i]->session_info_ptr[j]->session_type ==
                ...
                (mmgsdi_channel_info_ptr_table[mmgsdi_client_id_reg_table[i]->session_info_ptr[j]->channel_info_index]->slot_id ==
                  req_ptr->request_header.slot_id) &&
                channel_info_index_1x_sec == MMGSDI_MAX_CHANNEL_INFO)
        {
          /* This is MMGSDI's 1x SEC index in client reg table... retrieve the
             channel_id */
          channel_info_index_1x_sec = mmgsdi_client_id_reg_table[i]->session_info_ptr[j]->channel_info_index;
        }
        else if((mmgsdi_client_id_reg_table[i]->session_info_ptr[j]->session_type ==
                ...
                (mmgsdi_channel_info_ptr_table[mmgsdi_client_id_reg_table[i]->session_info_ptr[j]->channel_info_index]->slot_id ==
                  req_ptr->request_header.slot_id) &&
                channel_info_index_1x_ter == MMGSDI_MAX_CHANNEL_INFO)
        {
          /* This is MMGSDI's 1x SEC index in client reg table... retrieve the
             channel_id */
          channel_info_index_1x_ter = mmgsdi_client_id_reg_table[i]->session_info_ptr[j]->channel_info_index;
        }
        if (channel_info_index_1x_pri < MMGSDI_MAX_CHANNEL_INFO &&
            channel_info_index_1x_sec < MMGSDI_MAX_CHANNEL_INFO &&
            channel_info_index_1x_ter < MMGSDI_MAX_CHANNEL_INFO)
        {
          break;
    ...
    /* At this point we have retrieved the valid channel_info_index of
       MMGSDI_1X_PRI, MMGSDI_1X_SEC and MMGSDI_1X_TER sessions.
       As per the procedure indicated above, associate these valid
       channel_info_indexes with the 1x sessions of all the clients in
       the client reg table (except for MMGSDI and QMIUIM) */
    for(i=0; i<MMGSDI_MAX_CLIENT_INFO; i++)
    {
      if (mmgsdi_client_id_reg_table[i] == NULL)
      {
      ...
      for(j = 0; (num_sessions > 0) && (j < MMGSDI_MAX_SESSION_INFO); j++)
      {
        ...
        num_sessions--;
        client_channel_info_index = mmgsdi_client_id_reg_table[i]->session_info_ptr[j]->channel_info_index;
        if(client_channel_info_index == MMGSDI_MAX_CHANNEL_INFO)
        {
          /* For MMGSDI and QMIUIM, since the channel is never disassociated
             CSIM hiding process, client_channel_info_index would be a valid
             one already. Hence we won't enter here for re-association
             of channels for MMGSDI and QMIUIM */
          if(channel_info_index_1x_pri < MMGSDI_MAX_CHANNEL_INFO &&
             mmgsdi_client_id_reg_table[i]->session_info_ptr[j]->session_type ==
               MMGSDI_1X_PROV_PRI_SESSION &&
             mmgsdi_channel_info_ptr_table[channel_info_index_1x_pri] != NULL &&
             mmgsdi_channel_info_ptr_table[channel_info_index_1x_pri]->num_connections > 0)
          {
            UIM_MSG_HIGH_1("Channel mapping updated for session_id=0x%x",
                           mmgsdi_client_id_reg_table[i]->session_info_ptr[j]->session_id);

            mmgsdi_channel_info_ptr_table[channel_info_index_1x_pri]->num_connections++;
            mmgsdi_client_id_reg_table[i]->session_info_ptr[j]->channel_info_index =
             channel_info_index_1x_pri;

            /* Now that the channel has been reassociated with the 1x sessions, send
               out the session_changed 'activated' and other events that follow upto
               sub_ready. Before this function is called, make sure csim_hiding_status
               is MMGSDI_CSIM_UNHIDING so that MMSGDI knows which events to send out
               to the clients as part of CSIM unhiding */
            mmgsdi_evt_build_and_notify_session_evt_from_state(mmgsdi_client_id_reg_table[i]->session_info_ptr[j]->session_id);
          }
          else if(channel_info_index_1x_sec < MMGSDI_MAX_CHANNEL_INFO &&
             mmgsdi_client_id_reg_table[i]->session_info_ptr[j]->session_type ==
               MMGSDI_1X_PROV_SEC_SESSION &&
             mmgsdi_channel_info_ptr_table[channel_info_index_1x_sec] != NULL &&
             mmgsdi_channel_info_ptr_table[channel_info_index_1x_sec]->num_connections > 0)
          {
            UIM_MSG_HIGH_1("Channel mapping updated for session_id=0x%x",
                           mmgsdi_client_id_reg_table[i]->session_info_ptr[j]->session_id);

            mmgsdi_channel_info_ptr_table[channel_info_index_1x_sec]->num_connections++;
            mmgsdi_client_id_reg_table[i]->session_info_ptr[j]->channel_info_index =
             channel_info_index_1x_sec;
          ...
          else if(channel_info_index_1x_ter < MMGSDI_MAX_CHANNEL_INFO &&
             mmgsdi_client_id_reg_table[i]->session_info_ptr[j]->session_type ==
               MMGSDI_1X_PROV_TER_SESSION &&
             mmgsdi_channel_info_ptr_table[channel_info_index_1x_ter] != NULL &&
             mmgsdi_channel_info_ptr_table[channel_info_index_1x_ter]->num_connections > 0)
          {
            UIM_MSG_HIGH_1("Channel mapping updated for session_id=0x%x",
                           mmgsdi_client_id_reg_table[i]->session_info_ptr[j]->session_id);

            mmgsdi_channel_info_ptr_table[channel_info_index_1x_ter]->num_connections++;
            mmgsdi_client_id_reg_table[i]->session_info_ptr[j]->channel_info_index =
             channel_info_index_1x_ter;
  ...
  /* Lets build a cnf and send response to the client */
  ...
  return mmgsdi_cnf_build_and_queue(MMGSDI_SUCCESS,
                                    ...
                                    NULL, NULL,
                                    FALSE, status_word);
} /* mmgsdi_gen_process_hide_csim_req */


/*===========================================================================
FUNCTION MMGSDI_GEN_PROCESS_NOTIFY_RECOVERY_START

DESCRIPTION
    This function processes the notification from UIM drivers indicating that
    the recovery of a card in a slot has started. It resets the PLI-IMEI sent
    to card flag.
...
RETURN VALUE
    MMGSDI_SUCCESS: Event successfully sent to the clients
    MMGSDI_ERROR:   Otherwise

SIDE EFFECTS
    None
===========================================================================*/
mmgsdi_return_enum_type mmgsdi_gen_process_notify_recovery_start(
  const mmgsdi_notify_recovery_start_req_type *req_ptr
)
{
  mmgsdi_slot_data_type  *slot_data_ptr = NULL;
  ...
  /* Since recovery has started, reset PLI-IMEI sent to card */
  if(slot_data_ptr != NULL)
  {
  ...
  if(uim_common_qsh_is_event_enabled(USIM_QSH_EVENT_RECOVERY_TRIGGERED))
  {
    sys_modem_as_id_e_type  as_id = SYS_MODEM_AS_ID_1;

    as_id = mmgsdi_util_get_as_id_from_slot_id(req_ptr->request_header.slot_id);
    uim_common_qsh_event_notify(USIM_QSH_EVENT_RECOVERY_TRIGGERED, as_id);
  }

  return MMGSDI_SUCCESS;
} /* mmgsdi_gen_process_notify_recovery_start */


/*===========================================================================
FUNCTION MMGSDI_GEN_PROCESS_NOTIFY_RECOVERY_COMPLETE

DESCRIPTION
    This function processes the notification from UIM drivers indicating that
    the recovery of a card in a slot is successfully completed. It sends event
    to all its clients that have registered for receiving card based events.
...
SIDE EFFECTS
    None
===========================================================================*/
mmgsdi_return_enum_type mmgsdi_gen_process_notify_recovery_complete(
  const mmgsdi_notify_recovery_complete_req_type *req_ptr
)
{
  MMGSDIUTIL_RETURN_IF_NULL(req_ptr);
  ...
  /* Send out the event */
  mmgsdi_evt_build_and_send_recovery_complete(TRUE,
                                              0,
                                              req_ptr->request_header.slot_id);

  return MMGSDI_SUCCESS;
} /* mmgsdi_gen_process_notify_recovery_complete */


/*=============================================================================
  FUNCTION: MMGSDI_GEN_PROCESS_SET_APDU_BEHAVIOR

  DESCRIPTION:
    Function will set the APDU behavior of UIM DRIVER.
  ...
  SIDE EFFECTS
    NONE
=============================================================================*/
mmgsdi_return_enum_type mmgsdi_gen_process_set_apdu_behavior (
  const mmgsdi_set_apdu_behavior_req_type  * req_ptr
)
{
  mmgsdi_return_enum_type            mmgsdi_status   = MMGSDI_SUCCESS;
  uint8                              i               = 0;
  ...
  {
    if(mmgsdi_channel_info_ptr_table[i] != NULL &&
       mmgsdi_channel_info_ptr_table[i]->channel_id == req_ptr->channel_id &&
       mmgsdi_channel_info_ptr_table[i]->slot_id == req_ptr->request_header.slot_id &&
       (mmgsdi_channel_info_ptr_table[i]->channel_state == MMGSDI_CHANNEL_STATE_READY ||
        mmgsdi_channel_info_ptr_table[i]->channel_state == MMGSDI_CHANNEL_STATE_APDU_STREAMING_DONE) &&
       mmgsdi_channel_info_ptr_table[i]->app_info_ptr->proprietary_app)
    {
      MMGSDIUTIL_ENTER_CLIENT_APP_DATA_CRIT_SECT;    
      mmgsdi_channel_info_ptr_table[i]->apdu_behavior = req_ptr->apdu_behavior;
      MMGSDIUTIL_LEAVE_CLIENT_APP_DATA_CRIT_SECT;      
      break;
    }
  }
  ...
  if (i >= MMGSDI_MAX_CHANNEL_INFO)
  {
    return MMGSDI_INCORRECT_PARAMS;
  }

  mmgsdi_status = mmgsdi_uim_uicc_set_apdu_behavior(req_ptr);
    
  return mmgsdi_status;
} /* mmgsdi_gen_process_set_apdu_behavior */
