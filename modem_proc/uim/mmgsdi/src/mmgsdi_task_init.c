...
Copyright (c) 2012 - 2017, 2020 QUALCOMM Technologies, Inc (QTI) and its licensors.
...
$Header: //components/rel/uim.mpss/6.1.0/mmgsdi/src/mmgsdi_task_init.c#4 $$ $DateTime: 2020/08/05 01:31:21 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
06/22/20   sg      Enable secure and private logging in internal builds
...
12/16/16   ll      Add test mode logging enabled APIs for sensitive info control
...
08/31/15   at      Void obd_init API call's return value
...
===========================================================================*/
#define MMGSDI_NV_ACCESS_CONTROLLED_EFS_DIRECTORY_LIST_SIZE      1

/* MMGSDI TCB pointers for the main task along with the subsequent slot tasks.
   TCB pointers are initialized when their respective task is initalized. */
rex_tcb_type        *gsdi_main_tcb_ptr   = NULL;
rex_tcb_type        *gsdi_slot_1_tcb_ptr = NULL;
rex_tcb_type        *gsdi_slot_2_tcb_ptr = NULL;
rex_tcb_type        *gsdi_slot_3_tcb_ptr = NULL;

/* UIMDIAG TCB pointers to store the uimdiag task info.*/
rex_tcb_type        *uimdiag_tcb_ptr     = NULL;
...
SIDE EFFECTS
  None.
===========================================================================*/
static void mmgsdi_init_common (
...
)
{
  ...
  ** ----------------------------------------------------------------------*/
  ...
  /* Initialization for mmgsdi bootup logs */
  mmgsdi_log_pkt_init();
  ...
  if (mmgsdi_add_client_id_entry(&mmgsdi_generic_data_ptr->client_id,
                                 "GSDI", MMGSDI_CLIENT_NAME) == MMGSDI_SUCCESS)
  {
...
{
  gsdi_main_tcb_ptr = rcinit_lookup_rextask("gsdi");
...
{
  gsdi_slot_1_tcb_ptr = rcinit_lookup_rextask("mmgsdi_1");
...
{
  gsdi_slot_2_tcb_ptr = rcinit_lookup_rextask("mmgsdi_2");
...
{
  gsdi_slot_3_tcb_ptr = rcinit_lookup_rextask("mmgsdi_3");
...
)
{
  ...
  /* In case of internal builds FEATURE_ENABLE_SECURE_LOGGING_INTERNAL will be defined in scons and 
     we will directly enable secure/private logging, instead of reading the value from EFS */
#ifndef FEATURE_ENABLE_SECURE_LOGGING_INTERNAL
  /* Check EFS item secure_logging_enabled for the whole device */
  if(UIM_COMMON_EFS_SUCCESS != uim_common_efs_read(UIM_COMMON_EFS_MMGSDI_SECURE_LOGGING_ENABLED,
                                                   UIM_COMMON_EFS_ITEM_FILE_TYPE,
                                                   UIM_COMMON_EFS_DEVICE,
                                                   (uint8*)&(mmgsdi_is_test_mode_logging_enabled.secure_logging_enabled),
                                                   sizeof(mmgsdi_is_test_mode_logging_enabled.secure_logging_enabled)))
  {
    mmgsdi_is_test_mode_logging_enabled.secure_logging_enabled = FALSE;
  }

  /* Check EFS item private_logging_enabled for the whole device */
  if(UIM_COMMON_EFS_SUCCESS != uim_common_efs_read(UIM_COMMON_EFS_MMGSDI_PRIVATE_LOGGING_ENABLED,
                                                   UIM_COMMON_EFS_ITEM_FILE_TYPE,
                                                   UIM_COMMON_EFS_DEVICE,
                                                   (uint8*)&(mmgsdi_is_test_mode_logging_enabled.private_logging_enabled),
                                                   sizeof(mmgsdi_is_test_mode_logging_enabled.private_logging_enabled)))
  {
    mmgsdi_is_test_mode_logging_enabled.private_logging_enabled = FALSE;
  }
#else
  mmgsdi_is_test_mode_logging_enabled.secure_logging_enabled = TRUE;
  mmgsdi_is_test_mode_logging_enabled.private_logging_enabled = TRUE;
#endif /* FEATURE_ENABLE_SECURE_LOGGING_INTERNAL */
...
)
{
  ...
  ** ----------------------------------------------------------------------*/
  ...
  (void)mmgsdi_get_hw_slots(&mmgsdi_generic_data_ptr->num_slots);
  ...
  ** ----------------------------------------------------------------------*/
  ...
  /* Retrieve DIAG TCB pointer */
  uimdiag_tcb_ptr = rcinit_lookup_rextask("diag");
  ...
  /* To regsiter with QSH module */
  uim_common_qsh_init();

  /* To initialize the slot mapping module */
