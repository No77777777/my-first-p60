...
$Header: //components/rel/uim.mpss/6.1.0/mmgsdi/src/mmgsdi_cnf_3gpp2.c#1 $$ $DateTime: 2019/10/30 06:56:05 $ $Author: pwbldsvc $
...
)
{
  ...
  if (mmgsdi_status == MMGSDI_SUCCESS)
  {
    ...
    -------------------------------------------------------------------------*/
    if (compute_ip_auth_rsp_data_ptr->data_len > 0)
    {
  ...
  if (mmgsdi_status == MMGSDI_SUCCESS)
  {
   ...
   run_cave_cnf_ptr->run_cave_response_data =
       b_unpackd (run_cave_rsp_data_ptr->data_ptr, 0,
                  uint32touint16(run_cave_rsp_data_ptr->data_len*8));
  }
  else
  {
  ...
  if (mmgsdi_status == MMGSDI_SUCCESS)
  {
   ...
   if( generate_key_rsp_data_ptr->data_len > MMGSDI_KEY_SIZE)
   {
      MMGSDIUTIL_MEM_MALLOC_AND_VALIDATE(
                ...
                generate_key_rsp_data_ptr->data_len - MMGSDI_KEY_SIZE);
      ...
      mmgsdi_memscpy(generate_key_cnf_ptr->octet_data.data_ptr,
                     ...
                     generate_key_rsp_data_ptr->data_len - MMGSDI_KEY_SIZE);
      generate_key_cnf_ptr->octet_data.data_len = generate_key_rsp_data_ptr->data_len - MMGSDI_KEY_SIZE;
   }
   else
   {
...
)
{
  ...
  if (mmgsdi_status == MMGSDI_SUCCESS)
  {
    /* Response data contains
       RANDBS            4 bytes */
    MMGSDIUTIL_RETURN_IF_NULL_2(rsp_data_ptr, rsp_data_ptr->data_ptr);
...
{
  MMGSDIUTIL_RETURN_IF_NULL_4(otasp_cnf_ptr,
                              extra_param_ptr,
                              rsp_data_ptr,
