...
Copyright (c) 2012 - 2017 QUALCOMM Technologies, Inc (QTI) and its licensors.
...
$Header: //components/rel/uim.mpss/6.1.0/mmgsdi/src/mmgsdi_csg.c#2 $$ $DateTime: 2019/10/30 06:56:05 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
07/19/17   bcho    Remove memory leak in EF-ACSGL record update operation
...
06/11/15   tl      Add length checks to prevent invalid dereference
04/29/15   bcho    Support for App specific cache
02/25/15   tl      Remove ICC seek support
01/16/15   tkl     Fixed compilation error with FFEATURE_UIM_PERSISTENT_CACHE
12/19/14   bcho    Skip unused byte in CSG List TLV
...
)
{
  ...
  mmgsdi_status = mmgsdi_cache_read_len(session_id, &access, hnbn_rec,
                                        &hnbn_rec_data.data_len, NULL);
  if((mmgsdi_status != MMGSDI_SUCCESS) ||
     (hnbn_rec_data.data_len == 0))
  {
  ...
  MMGSDIUTIL_MEM_MALLOC_AND_VALIDATE(hnbn_rec_data.data_ptr, hnbn_rec_data.data_len);
  ...
  mmgsdi_status = mmgsdi_cache_read(session_id, &access, &hnbn_rec_data,
                                    0, hnbn_rec, NULL);
  if(mmgsdi_status != MMGSDI_SUCCESS)
  {
    UIM_SELECTIVE_MSG_ERR_2(UIM_LOG_EONS,
    ...
    MMGSDIUTIL_TMC_MEM_FREE(hnbn_rec_data.data_ptr);
    return mmgsdi_status;
  }
     ...
     (hnbn_name_data->data_len > 0))
  {
    ...
    MMGSDIUTIL_MEM_MALLOC_AND_VALIDATE(hnbn_name_data->data_ptr, hnbn_name_data->data_len);
    if(hnbn_name_data->data_ptr != NULL)
    {
      if( offset + hnbn_name_data->data_len
  ...
  mmgsdi_rec_num_type  * hnbn_rec_num)
{
  ...
  if(total_records > 0)
  {
    mmgsdi_status = mmgsdi_cache_read_len(session_id, &access, 1,
                                          &csgl_data.data_len, NULL);
    if((mmgsdi_status != MMGSDI_SUCCESS) ||
       (csgl_data.data_len == 0))
    {
    ...
    MMGSDIUTIL_MEM_MALLOC_AND_VALIDATE(csgl_data.data_ptr, csgl_data.data_len);
  ...
  for (rec_num = 1; rec_num <= total_records; rec_num++)
  {
    ...
    mmgsdi_status = mmgsdi_cache_read(session_id, &access, &csgl_data,
                                      0, rec_num, NULL);
    if (mmgsdi_status != MMGSDI_SUCCESS)
    {
      ...
      if(mmgsdi_eons_compare_sim_plmn_with_nw_plmn(plmn_id_from_tbl, plmn_id))
      {
        if(csg_id == mmgsdi_csg_plmn_table[index].csg_id)
        {
          ...
          *hnbn_rec_num = mmgsdi_csg_plmn_table[index].hnbn_record_num;
          MMGSDIUTIL_TMC_MEM_FREE(csgl_data.data_ptr);
          return MMGSDI_SUCCESS;
        }
      }
      index++;
...
)
{
  ...
  if((mmgsdi_util_get_efs_item_index(session_id, NULL, &efs_index) ==
        MMGSDI_SUCCESS) &&
      ...
      (mmgsdi_status == MMGSDI_SUCCESS))
  {
    ...
    if(eons_trigger == MMGSDI_EONS_TRIGGER_SS_EVENT)
    {
         ...
         (hnbn_data.data_len <= SYS_HNB_NAME_LEN))
      {
        rplmn_info_ptr->csg_info.hnb_name.length =
          uint32touint8(hnbn_data.data_len);
    ...
    if (plmn_name_list_ptr->plmn_info_ptr != NULL)
    {
      ...
      {
        MMGSDIUTIL_TMC_MEM_FREE(hnbn_data.data_ptr);
        return MMGSDI_ERROR;
      }
...
)
{
  ...
  for(index = 0; index <= highest_index; index++)
  {
    ...
    if(acsgl_buf_len + MMGSDI_CSG_INFO_TLV_SIZE <= MMGSDI_MAX_REC_LEN)
    {
      ...
      temp_csg_tlv[acsgl_buf_len++] = uint32touint8(mmgsdi_csg_plmn_table_ptr[index].csgt_record_num);
      temp_csg_tlv[acsgl_buf_len++] = uint32touint8(mmgsdi_csg_plmn_table_ptr[index].hnbn_record_num);
...
)
{
  ...
  mmgsdi_status = mmgsdi_cache_write(session_id, &access, rec_data, 0, 0,
                                     rec_num);
  if(mmgsdi_status != MMGSDI_SUCCESS)
  {
...
)
{
  ...
  MMGSDIUTIL_MEM_MALLOC_AND_VALIDATE(hnbn_data.data_ptr, hnbn_data.data_len);
  ...
  for (rec_num = 1; rec_num <= total_records; rec_num++)
  {
    mmgsdi_status = mmgsdi_cache_read(session_id, &access, &hnbn_data,
                                      0, rec_num, NULL);
    if (mmgsdi_status != MMGSDI_SUCCESS ||
        hnbn_data.data_ptr == NULL ||
...
)
{
  ...
  MMGSDIUTIL_MEM_MALLOC_AND_VALIDATE(write_req_ptr->data.data_ptr, write_data.data_len);
  ...
  mmgsdi_status = mmgsdi_uim_uicc_write_record(write_req_ptr, FALSE, NULL);
...
)
{
  ...
  MMGSDIUTIL_MEM_MALLOC_AND_VALIDATE(hnbn_tlv_data.data_ptr, hnbn_rec_len);
  if(hnbn_tlv_data.data_ptr == NULL)
  {
    return MMGSDI_ERROR;
  }

  hnbn_tlv_data.data_len = hnbn_rec_len;

  memset(hnbn_tlv_data.data_ptr, 0xFF, hnbn_rec_len);
  ...
  {
    hnbn_tlv_data.data_ptr[1] = uint32touint8(req_ptr->hnb_name.data_len);
  }
  else
  {
  ...
  MMGSDIUTIL_TMC_MEM_FREE(hnbn_tlv_data.data_ptr);
  return mmgsdi_status;
...
)
{
  ...
  MMGSDIUTIL_MEM_MALLOC_AND_VALIDATE(acsgl_data.data_ptr, acsgl_data.data_len);
...
)
{
  ...
  {
    mmgsdi_status = mmgsdi_cache_read_len(session_id, &access, rec_cnt,
                                          &rec_len, NULL );
    if(mmgsdi_status != MMGSDI_SUCCESS)
    {
  ...
  for(rec_cnt = 1; rec_cnt <= total_recs; rec_cnt++)
  {
       ...
       (empty_data_buf.data_ptr != NULL))
    {
          ...
          empty_rec_cnt++)
      {
        ...
        {
          (void)mmgsdi_cache_write(session_id, &file_access, empty_data_buf,
                                   0, 0, rec_cnt);
...
)
{
  ...
  UIM_MSG_HIGH_3("mmgsdi_session_update_acsgl_record: update_type 0x%x, csg_id 0x%x, hnb_name data len 0x%x",
                 update_type,
				 plmn_csg_id.csg_id,
				 hnb_name.data_len);
