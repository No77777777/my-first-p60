...
$Header: //components/rel/uim.mpss/6.1.0/mmgsdi/src/mmgsdi_eons_cdma.c#2 $$ $DateTime: 2019/10/30 06:56:05 $
...
06/11/15   tl      Fix potential invalid pointer dereference
...
)
{
  ...
  if(mmgsdi_status == MMGSDI_SUCCESS && cdma_spn_data.data_len > 0)
  {
    ...
    if(mmgsdi_status == MMGSDI_SUCCESS &&
       cdma_spn_data.data_ptr != NULL &&
       cdma_spn_data.data_len > MMGSDI_EONS_SPN_DISP_COND_POS)
    {
  ...
  if(cdma_spn_data.data_ptr != NULL &&
     ...
     cdma_spn_data.data_len >= (MMGSDI_EONS_CDMA_SPN_DATA_BUF_POS + MMGSDI_EONS_CDMA_SPN_DATA_LEN))
  {
  ...
  MMGSDIUTIL_TMC_MEM_FREE(cdma_spn_data.data_ptr);
