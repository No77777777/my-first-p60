...
Copyright (c) 2004 - 2021, 2023 - 2024 QUALCOMM Technologies, Inc (QTI) and its licensors.
...
$Header: //components/rel/uim.mpss/6.1.0/mmgsdi/src/mmgsdiutil.c#9 $$ $DateTime: 2024/02/26 16:03:48 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
02/04/24   cj      Hide CSIM from indication when CDMA_LESS NV is enabled
08/17/23   cj      Read EF content from card when purging
...
05/12/20   sch     Skipping card issuer data while parsing ATR
02/28/20   sg      Initialize channel index while allocating session id
10/29/19   sch     Fixing enum Compilation issue
09/16/19   sg      Fallback to file system/cache for missing files
08/23/19   bcho    MMGSDI to not reserve channel for app which is already active
06/03/19   kb      Support for card reset on refresh failure
05/29/19   vdc     Parse ATR with pre-issuing data & status indicator elements
02/22/19   vm      Clear common telecom cache during FULL FCN Refresh
01/29/19   kb      Add sessions deactivated event for QSH
12/27/18   sg      Added NV check for setting no. of available logical channels
11/20/18   vdc     Purge temporary identities in INIT+FCN refresh mode
05/07/18   rps     Retrieving attribute cache from caller context
01/15/18   nr      Check app type while allocating entry in channel_ptr_table for icc
11/10/17   ra      Removed F3 Message  
10/26/17   bcho    Copy pin info only for activated apps
10/26/17   tq      Loop through only for number of sessions while cleaning up
...
04/19/17   tq      Added additional service check for ePDG files
...
03/21/17   bcho    Allow first write on EF-LOCI with same data
03/20/17   bcho    Set MMGSDI_SYNC_CMD_CANCEL_SIG to cancel sync command
03/02/17   sk      Fixed KW errors
...
10/25/16   me      set tp state when gstk is disabled
...
10/07/16   me      Removed F3 messages
...
08/25/16   vdc     Dont update config to 1 profile when reg profile is detected
08/08/16   me      Added cache support for msisdn, call term efs
07/29/16   bcho    Drop older SIMLOCK RUN algo response if new request sent
07/29/16   ar      Handle auto-activation race cond by passing an extra param
07/26/16   ar      Manage channel open gracefully if EF-DIR not present
07/08/16   bcho    UIM integration with QSH events
07/13/16   tkl     Remove sprintf for client name
...
06/14/16   sp      Move onchip entry function to onchip file
06/14/16   av      Block select AID only for ARA-M AID
06/09/16   shr     Deprecate QMI SIM Internal
06/08/16   sp      Rename enum table entries for mmgsdi_uim_ef_item_category
...
05/19/16   sp      Send Power down command to drivers when onchip is activated
05/13/16   sp      Extend SIMLOCK featurization in mmgsdi_util_tmsi_plmn_check
...
05/04/16   av      Attempt CSIM/RUIM activation irrespective of NV 855 value
...
03/11/16   bcho    Override access for auth related GSM files
...
01/22/16   bcho    Don't delete attr cache if not re-cached during NAA Refresh
...
01/05/16   ar      Don't fail APP_RESET_AUTO refresh if error in GSDI init
12/28/15   ks      Retry read request for non spec compliant cards
12/21/15   sp      Return correct status while validating card session
12/19/15   vdc     Send SAP disconnect evt if card is powered up in SAP mode
12/17/15   ar      Add utility to check MMGSDI task
12/17/15   vdc     Move event data to heap to reduce stack size
12/10/15   bcho    Usage of mmgsdi_refresh_data_info global removed
12/01/15   vdc     Send error status for sap connection if not successful
11/20/15   sp      Reduce F3 logging
11/02/15   yt      Remove prov_app field from mmgsdi_app_info_type
10/26/15   av      Adding test MCCs in list which have default MNC as 3 digits
10/21/15   av      Don't allow reSELECT on AID that failed atleast once earlier
...
07/22/15   av      Enhance tlv validation in mmgsdi_util_validate_onchip_config
07/22/15   ar      Set is_status_same flag for set_service_status request
07/17/15   ar      Remove the extra debug loggings
07/08/15   hh      Protect mmgsdi_internal_synch_uim_rpt_rsp with critical section
06/26/15   av      Make channel app check more robust
06/18/15   ar      Don't free the cphs cache pointer on refresh INIT
...
06/11/15   av      Use safer string comparison function
06/05/15   nr      Removing dead code in finding app index for an application
05/20/15   lxu     Fix logical channel exhausted before CSIM activation
05/20/15   bcho    Free channel info ptr on card error/card pdown on ICC card
...
04/17/15   ar      Don't update the card error cause in RESET AUTO mode
...
03/25/15   av      Ability to process app deact while activation is in progress
03/25/15   kk      Subs prov EFS error handling
03/25/15   ar      Support for cross-mapping in multisim_auto_provisioning NV
03/18/15   yt      Support for finding 1X session on slot
02/20/15   vv      Remove perso related code
02/19/15   nr      Phonebook record read optimization
01/22/15   ar      Remove un-necessary protocol check in u/icc_send_apdu
01/21/15   ar      Return ERROR if no free index in mmgsdi_client_req_table
01/07/15   vdc     Closed pending comments for handling multi profile cards
12/22/14   yt      Conversion of APDU streaming on channel 0 to MMGSDI requests
12/18/14   bcho    Cleanup session info even if app is not selected yet
...
#define MMGSDI_BASE_ID                             0x1
#define MMGSDI_MAX_ID                              0xFFFFFFFF

#define MMGSDI_FILE_LIST_MAX_SIZE             500
  ...
  if (response_ptr != NULL)
  {
    ...
    switch(cmd_type)
    {
      ...
      case MMGSDI_SEARCH_REQ:
        MMGSDIUTIL_TMC_MEM_FREE(response_ptr->search_cnf.searched_record_nums.data_ptr);
        break;
      ...
      {
        MMGSDIUTIL_TMC_MEM_FREE(response_ptr->session_get_operator_name_cnf.spn.eons_data.data_ptr);
      ...
      case MMGSDI_SESSION_SELECT_AID_REQ:
        MMGSDIUTIL_TMC_MEM_FREE(response_ptr->session_select_aid_cnf.select_rsp.data_ptr);
        break;
...
)
{
  ...
  MMGSDIUTIL_RETURN_IF_EXCEEDS(index, MMGSDI_MAX_CLIENT_REQ_INFO - 1);
...
)
{
  ...
  /* If this index belongs to a sync command that was canceled, this entry
     will be freed after receiving a response from UIMDRV */
  ...
  {
    UIM_MSG_HIGH_1("Index 0x%x is for a canceled sync command", index);
    return;
  }
  ...
  case MMGSDI_WRITE_REQ:
    ...
    MMGSDIUTIL_TMC_MEM_FREE(
      client_data_ptr->extra_param_ptr->write_data.actual_data.data_ptr);
    break;
  case MMGSDI_INCREASE_REQ:
    MMGSDIUTIL_TMC_MEM_FREE(
  ...
  MMGSDIUTIL_TMC_MEM_FREE(client_data_ptr->extra_param_ptr);
...
)
{
  ...
  mmgsdi_service_enum_type prose_service_type   = MMGSDI_NONE;
  ...
  {
    if(!mmgsdi_util_check_est_available(session_id,
                                        mmgsdi_access_ptr->file.file_enum))
    {
  ...
  if(mmgsdi_access_ptr->file.file_enum == MMGSDI_CDMA_EPRL &&
     mmgsdi_nv_get_slot_feature_status(MMGSDI_FEATURE_BLOCK_RUIM_EPRL_ACCESS, nv_context) == MMGSDI_FEATURE_ENABLED)
  {
    return MMGSDI_NOT_SUPPORTED;
  }

  if((mmgsdi_access_ptr->file.file_enum >> 8) == MMGSDI_USIM_PROSE_DF_START &&
     mmgsdi_access_ptr->file.file_enum != MMGSDI_USIM_PROSE_PST)
  {
    /* For ProSe EFs first check if the ProSe service (n�101) is set
       in the EF-UST */
    prose_service_type = service_type;
    service_type = MMGSDI_USIM_SRV_PROSE;
  }
  ...
  else if((mmgsdi_access_ptr->file.file_enum == MMGSDI_USIM_EPDG_ID) ||
          (mmgsdi_access_ptr->file.file_enum == MMGSDI_USIM_EPDG_SELECTION))
  {
    mmgsdi_status = mmgsdi_uicc_chk_srv_available(session_id,
                                                  MMGSDI_CHK_SRV_FROM_CARD_OR_CACHE,
                                                  MMGSDI_USIM_SRV_EPDG_CONFIG_CONFIG,
                                                  &file_ok_in_svc_table);
    if(mmgsdi_status != MMGSDI_SUCCESS)
    {
      file_ok_in_svc_table = TRUE;
  ...
  else if((mmgsdi_access_ptr->file.file_enum == MMGSDI_USIM_EPDG_IDEM) ||
          (mmgsdi_access_ptr->file.file_enum == MMGSDI_USIM_EPDG_SELECTION_EM))
  {
    mmgsdi_status = mmgsdi_uicc_chk_srv_available(session_id,
                                                  MMGSDI_CHK_SRV_FROM_CARD_OR_CACHE,
                                                  MMGSDI_USIM_SRV_EM_EPDG_CONFIG_CONFIG,
                                                  &file_ok_in_svc_table);
    if(mmgsdi_status != MMGSDI_SUCCESS)
    {
      file_ok_in_svc_table = TRUE;
  ...
  else if((mmgsdi_access_ptr->file.file_enum >> 8) == MMGSDI_USIM_PROSE_DF_START &&
          mmgsdi_access_ptr->file.file_enum != MMGSDI_USIM_PROSE_PST)
  {
    mmgsdi_status = mmgsdi_uicc_chk_srv_available(session_id,
                                                  MMGSDI_CHK_SRV_FROM_CARD_OR_CACHE,
  ...
  )
{
  ...
  switch(mmgsdi_cmd_enum)
  {
    ...
    case MMGSDI_GENERATE_KEY_VPM_REQ:
      ...
      break;
    case MMGSDI_SET_APDU_BHEAVIOR_REQ:
      msg_ptr->uim_rpt_code = (int)UIM_SET_APDU_BEHAVIOR_R;
      break;
    case MMGSDI_SESSION_RUN_GSM_ALGO_REQ:
  ...
  if(mmgsdi_status != MMGSDI_SUCCESS)
  {
    ...
    MMGSDIUTIL_TMC_MEM_FREE(task_cmd_ptr);
    UIM_MSG_ERR_2("Queue of MMGSDI_UIM_REPORT_RSP failed for cmd: 0x%x with status 0x%x",
                  mmgsdi_cmd_enum, mmgsdi_status);
  }
...
)
{
  ...
  else if (report_ptr->rpt_type == UIM_AUTHENTICATE_ODD_INS_R)
  {
    ...
    if (report_ptr->rpt.autn_odd_ins.len > 0)
    {
         ...
         report_ptr->rpt.autn_odd_ins.data[0] == MMGSDI_MBMS_BER_TLV_TAG)
      {
        ...
        {
           (*msg_pptr)->rsp_data.data_len = (*msg_pptr)->rsp_data.data_len << 8;
  ...
  if(report_ptr->rpt_type == UIM_STREAM_ISO7816_APDU_R)
  {
    ...
    else
    {
      if (report_ptr->cmd_transacted)
      {
        ...
        rsp_data_len = (*msg_pptr)->rsp_data.data_len;
  ...
  /* Fill error when SAP connect fails */
  if(report_ptr->rpt_status == UIM_FAIL &&
     report_ptr->rpt_type == UIM_RESET_SWITCH_UIM_PASSIVE_R)
  {
    (*msg_pptr)->mmgsdi_error_status = MMGSDI_ERROR;
  }
...
)
{
  ...
  if (mmgsdi_status != MMGSDI_SUCCESS)
  {
    if (msg_ptr != NULL)
    {
      MMGSDIUTIL_TMC_MEM_FREE((msg_ptr->rsp_data.data_ptr));
    }
  ...
  task_cmd_ptr->cmd.cmd.uim_report_rsp.mmgsdi_error_status = msg_ptr->mmgsdi_error_status;
...
)
{
      ...
      index < MMGSDI_MAX_CLIENT_REQ_INFO)
  {
    ...
    /* If the index belongs to a sync cmd that was marked as RSP_NOT_RECEIVED,
       return without populating the mmgsdi_internal_synch_uim_rpt_rsp as
       MMGSDI will not be waiting for this response. */
    if (mmgsdi_client_req_table_info_ptr[index] != NULL &&
        mmgsdi_client_req_table_info_ptr[index]->sync_cmd &&
        mmgsdi_client_req_table_info_ptr[index]->uim_rsp_status == MMGSDI_UIM_RSP_NOT_RECEIVED &&
        (mmgsdi_util_get_slot_index(mmgsdi_client_req_table_info_ptr[index]->slot_id,
                                    &slot_index) == MMGSDI_SUCCESS))
    {
      /* This command has been cancelled and MMGSDI slot task has stopped
         listening to TASK_CMD_Q_SIG. To start listeing signal back we need to
         process response in mmgsdi_process_response fn, so queue
  ...
  /* Populate response buffer if task is active and cmd was not canceled by MMGSDI */
  if (!cmd_canceled)
  {
    ...
    else
    {
      ...
      /* Set sync UIM report signal */
      (void)mmgsdi_util_set_signal_for_task(mmgsdi_task,
...
)
{
  ...
  /* If a sync cmd was canceled, reject new commands that could be sent
     before returning to the main loop */
  if (mmgsdi_is_sync_cmd_canceled(mmgsdi_util_get_slot_from_task(mmgsdi_task)))
  {
    UIM_MSG_ERR_0("sync command canceled, not allowing further commands");
    MMGSDIUTIL_TMC_MEM_FREE(cmd_ptr);
    return MMGSDI_ERROR;
  }
  ...
  index = cmd_ptr->hdr.user_data;
  if (index < MMGSDI_MAX_CLIENT_REQ_INFO &&
      mmgsdi_client_req_table_info_ptr[index] != NULL)
  {
    mmgsdi_client_req_table_info_ptr[index]->sync_cmd = TRUE;
  }
  ...
  (void)rex_clr_sigs(rex_self(), MMGSDI_SYNC_CMD_CANCEL_SIG);
  ...
  */
  if ( sigs_rcvd & sigs & MMGSDI_SYNCH_UIM_REPORT_SIG ||
       sigs_rcvd & sigs & MMGSDI_SYNC_CMD_CANCEL_SIG)
  {
    ...
    /* Whenever Main task set this signal, Slot task has to send
       acknowledgement back as Main task is waiting for it. */
    if((sigs_rcvd & sigs & MMGSDI_SYNC_CMD_CANCEL_SIG) &&
       (mmgsdi_util_get_slot_index_from_task(mmgsdi_task, &slot_index) ==
          MMGSDI_SUCCESS))
    {
      /* Mark this task as one where sync command was canceled to prevent
         subsequent commands from being sent */
      mmgsdi_sync_cmd_canceled[slot_index] = TRUE;
    }

    if(sigs_rcvd & sigs & MMGSDI_SYNCH_UIM_REPORT_SIG)
    {
      /* Set UIM cmd_in_progress to FALSE as card response is received */
    ...
    /* If the client req table entry is marked as RSP_NOT_RECEIVED, it means
       that the command has been canceled by the main task. */
    if (mmgsdi_task != MMGSDI_TASK_MAIN &&
        index < MMGSDI_MAX_CLIENT_REQ_INFO &&
        mmgsdi_client_req_table_info_ptr[index] != NULL &&
        mmgsdi_client_req_table_info_ptr[index]->uim_rsp_status == MMGSDI_UIM_RSP_NOT_RECEIVED &&
...
)
{
  ...
  MMGSDIUTIL_RETURN_IF_EXCEEDS(client_index, MMGSDI_MAX_CLIENT_INFO - 1);
...
static void mmgsdi_util_align_ptr(
  ...
  void               **new_msg_pptr,
  uint32               start_offset,
  uint32               base_struct_size,
  uint32              *end_offset_ptr)
{
  uint32               aligned_var_size = 0;

  if (base_msg_ptr == NULL || end_offset_ptr == NULL || new_msg_pptr == NULL)
  {
    return;
  }
  ...
  mmgsdi_client_req_extra_info_type **extra_param_pptr)
{
  ...
  case MMGSDI_WRITE_REQ:
    ...
    data_len = ((mmgsdi_write_req_type*)cmd_ptr)->data.data_len;
  ...
  case MMGSDI_INCREASE_REQ:
    ...
    data_len = ((mmgsdi_increase_req_type*)cmd_ptr)->data.data_len;
  ...
  case MMGSDI_SEND_APDU_EXT_REQ:
    ...
    MMGSDIUTIL_MEM_MALLOC_AND_VALIDATE(
      (*extra_param_pptr)->send_apdu_data.data.data_ptr,
      data_len);
    if ((*extra_param_pptr)->send_apdu_data.data.data_ptr == NULL)
    {
      MMGSDIUTIL_TMC_MEM_FREE(*extra_param_pptr);
      return MMGSDI_MEMORY_ERROR_HEAP_EXHAUSTED;
    }
    mmgsdi_memscpy((*extra_param_pptr)->send_apdu_data.data.data_ptr,
                   data_len,
                   ((mmgsdi_send_apdu_req_type*)cmd_ptr)->data.data_ptr,
                   data_len);

    (*extra_param_pptr)->send_apdu_data.data.data_len =
      ((mmgsdi_send_apdu_req_type*)cmd_ptr)->data.data_len;
  ...
  case MMGSDI_SAP_SEND_APDU_REQ:
    ...
    data_len = ((mmgsdi_sap_send_apdu_req_type*)cmd_ptr)->data.data_len;
  ...
  case MMGSDI_PIN_OPERATION_REQ:
    ...
    if(((mmgsdi_pin_operation_req_type*)cmd_ptr)->pin_id != MMGSDI_HIDDENKEY)
    {
      ...
      if(((mmgsdi_pin_operation_req_type*)cmd_ptr)->pin_puk.data_len != 0)
      {
           ...
           (((mmgsdi_pin_operation_req_type*)cmd_ptr)->pin_op == MMGSDI_PIN_OP_DISABLE_AND_REPLACE))
        {
             ...
             (((mmgsdi_pin_operation_req_type*)cmd_ptr)->pin_puk.data_len <= MMGSDI_PIN_MAX_LEN))
          {
            ...
            if(mmgsdi_nv_get_device_feature_status(MMGSDI_FEATURE_VERIFY_PIN_AFTER_CHANGE_OR_ENABLE_OR_UNBLOCK) == MMGSDI_FEATURE_ENABLED)
            {
      ...
      /* Cache the user entered PIN data into extra parameter pointer from the request for verification
         of PIN after enabling or changing or unblocking it */
     else if((mmgsdi_nv_get_device_feature_status(MMGSDI_FEATURE_VERIFY_PIN_AFTER_CHANGE_OR_ENABLE_OR_UNBLOCK) == MMGSDI_FEATURE_ENABLED) &&
  ...
  case MMGSDI_PURGE_TEMPORARY_IDENTITY_REQ:
    MMGSDIUTIL_MEM_MALLOC_AND_VALIDATE(*extra_param_pptr,
      sizeof(mmgsdi_session_temporary_identity_extra_info_type));
    if ((*extra_param_pptr) == NULL)
    {
      return MMGSDI_MEMORY_ERROR_HEAP_EXHAUSTED;
    }
    (*extra_param_pptr)->temporary_identity.temporary_identity = ((mmgsdi_purge_temporary_identity_req_type*)cmd_ptr)->temporary_identity;
    break;
...
)
{
  ...
  else if(mmgsdi_channel_info_ptr_table[session_info_ptr->channel_info_index]->num_connections == 1)
  {
    ...
    if (!mmgsdi_channel_info_ptr_table[session_info_ptr->channel_info_index]->app_termination_skipped)
    {
      ...
      /* Don't remove the channel info from table if it points to app info for SIM, RUIM */
      if(mmgsdi_channel_info_ptr_table[session_info_ptr->channel_info_index]->app_info_ptr == NULL)
      {
...
)
{
  ...
  else if (mmgsdi_util_is_non_prov_session(session_info_ptr->session_type))
  {
    ...
    {
      mmgsdi_event_data_type        *event_notify_data_ptr  = NULL;

      UIM_MSG_HIGH_1("Closing non prov session and sending out Session close evt - 0x%x",
                     session_info_ptr->session_id);

      MMGSDIUTIL_MEM_MALLOC_AND_VALIDATE(event_notify_data_ptr, sizeof(mmgsdi_event_data_type));
      if (event_notify_data_ptr != NULL)
      {
        mmgsdi_status = mmgsdi_evt_build_session_close(
                          mmgsdi_channel_info_ptr_table[session_info_ptr->channel_info_index]->slot_id,
                          session_info_ptr->session_id,
                          mmgsdi_channel_info_ptr_table[session_info_ptr->channel_info_index]->app_info_ptr,
                          event_notify_data_ptr);
        if(mmgsdi_status == MMGSDI_SUCCESS)
        {
           UIM_MSG_HIGH_1("Non prov session closed cause 0x%x",
                          event_notify_data_ptr->data.session_close.cause);

           /* Notify the specific client only */
           session_info_ptr->session_evt_cb_ptr(event_notify_data_ptr);
        }

        mmgsdi_evt_free_data(event_notify_data_ptr);
        MMGSDIUTIL_TMC_MEM_FREE(event_notify_data_ptr);
      }
    }
    else
    {
...
)
{
  ...
  mmgsdi_status = mmgsdi_util_get_mmgsdi_session_on_ch_zero(
                    slot_id,
                    NULL,
                    &ch_zero_session_id);
    ...
    for (session_index = 0; (num_sessions > 0) && (session_index < MMGSDI_MAX_SESSION_INFO); session_index++)
    {
          ...
          (mmgsdi_channel_info_ptr_table[curr_session_ptr->channel_info_index]->app_info_ptr != NULL))
      {
              ...
              &app_info_ptr->app_data, FALSE) == MMGSDI_SUCCESS)
        {
          ...
          {
            /* In order to satisfy customer requirement, do not sent out session deactivated
               event to MMGSDI and to QMIUIM when this is triggered due to permanent
               disabling of CSIM */
            if ((mmgsdi_channel_info_ptr_table[curr_session_ptr->channel_info_index]->app_info_ptr->session_closed_cause ==
                  MMGSDI_SESSION_CLOSED_CAUSE_CSIM_HIDDEN) &&
                mmgsdi_util_client_receives_hidden_csim_events(int32touint32(client_index)))
            {
              UIM_MSG_HIGH_1("CSIM Permanently disabled. Skipping session close cleanup for 0x%x", curr_session_ptr->session_id);
              continue;
            }
  ...
  switch(cnf)
  {
    case MMGSDI_GET_ALL_PIN_STATUS_REQ:
         ...
         status != MMGSDI_CODE_PERM_BLOCKED)
      {
        ...
        (void)mmgsdi_util_get_session_type(cnf_ptr->response_header.session_id,
                                           &session_type, NULL);

        mmgsdi_status = mmgsdi_util_get_slot_index(slot_id, &slot_index);

        /* End any ongoing REFRESH other than APP_RESET_AUTO on the session.
           If refresh session is different then close it here as it will not be
           closed as part of deactivate req. For APP_RESET_AUTO, it's triggered
           as part of deactivation req. We will deactivate this session as part
           of the original deactivate req to send SUCCESS cnf to client */
        if (mmgsdi_status == MMGSDI_SUCCESS &&
            ...
            mmgsdi_refresh_info_ptr[slot_index]->node_ptr->session_type == session_type)
        {
          break;
        }
    ...
    case MMGSDI_SESSION_ACT_OR_SWITCH_PROV_REQ:
      ...
      if (cnf_ptr->response_header.client_data)
      {
        ...
        mmgsdi_data_type          app_1x_data    = {0, NULL};
        ...
        nv_context = mmgsdi_util_get_efs_item_index_for_slot(cnf_ptr->response_header.slot_id);
        ...
        if (mmgsdi_sub_sel_get_session_id_for_slot(cnf_ptr->response_header.slot_id,
                                                   MMGSDI_TECH_3GPP2,
                                                   &session_id) != MMGSDI_SUCCESS)
        {
          UIM_MSG_HIGH_1("slot 0x%x not mapped to any session; not activating",
              ...
              MMGSDI_FEATURE_CDMA_LESS, nv_context) == MMGSDI_FEATURE_ENABLED))
        {
          UIM_MSG_HIGH_2("CDMALess NV is enabled for 1x subscription context 0x%x slot 0x%x; not activating",
                          nv_context, cnf_ptr->response_header.slot_id);
          break;
        }
    ...
    case MMGSDI_ACTIVATE_ONCHIP_SIM_REQ:
   /* For card power down request as a part of ONCHIP activation, we use this internal callback */
    case MMGSDI_CARD_PDOWN_REQ:
    default:
      break;
  }
...
)
{
  ...
  if(slot_data_ptr != NULL)
  {
    ...
    slot_data_ptr->imei_sent_to_card             = FALSE;
    slot_data_ptr->is_euicc                      = FALSE;
    slot_data_ptr->protocol                      = MMGSDI_NO_PROTOCOL;
    slot_data_ptr->refresh_failure_count         = 0;
    slot_data_ptr->nv_ref_app_deact_count        = 0;

    /* Initialize TP state, based on GSTK support */
    mmgsdi_card_init_tp_state_init(slot_data_ptr);
    ...
    if(!reselect_app)
    {
      if(slot_data_ptr->app_info_pptr != NULL)
      {
        for(i =0; i < MMGSDI_MAX_EXT_APP_INFO; i++)
        {
          ...
          {
            /* Flush app specific cache data on card error. App cache on UICC
               card will be also flushed on app termination */
            (void)mmgsdi_cache_delete(slot_data_ptr->app_info_pptr[i], 0, NULL);
            (void)mmgsdi_cache_delete_attr_selective_items(
              &slot_data_ptr->app_info_pptr[i]->attr_cache_tbl,
              NULL, 0, NULL, 0);
            mmgsdi_csg_reset_eons_flags_in_app_info(slot_data_ptr->app_info_pptr[i]);
            mmgsdi_eons_opl_pnn_delete(&slot_data_ptr->app_info_pptr[i]->opl_cache);
            mmgsdi_eons_opl_pnn_delete(&slot_data_ptr->app_info_pptr[i]->pnn_cache);

            if(slot_data_ptr->app_info_pptr[i]->cphs_info_ptr != NULL)
            {
              MMGSDIUTIL_TMC_MEM_FREE(
                slot_data_ptr->app_info_pptr[i]->cphs_info_ptr->cphs_ons_ptr);
              MMGSDIUTIL_TMC_MEM_FREE(
                slot_data_ptr->app_info_pptr[i]->cphs_info_ptr->cphs_ons_short_ptr);
              MMGSDIUTIL_TMC_MEM_FREE(slot_data_ptr->app_info_pptr[i]->cphs_info_ptr);
            }
            /* Clear orig. cache info */
            if(slot_data_ptr->app_info_pptr[i]->srv_table_orig_cache.data_ptr != NULL)
            {
              MMGSDIUTIL_TMC_MEM_FREE(
                slot_data_ptr->app_info_pptr[i]->srv_table_orig_cache.data_ptr);
              slot_data_ptr->app_info_pptr[i]->srv_table_orig_cache.data_len = 0;
            }
...
)
{
  ...
  for (index = 0; index < MMGSDI_MAX_EXT_APP_INFO; index++)
  {
    ...
    if(!memcmp(slot_data_ptr->app_info_pptr[index]->app_data.aid.data_ptr,
               aid.aid.data_ptr, aid.aid.data_len))
    {
      /* AID equal */
      return index;
    }
...
/* ============================================================================
   ...
   SIDE EFFECTS:
     None
============================================================================*/
void mmgsdi_util_find_first_ruim_csim_app(
  mmgsdi_slot_id_enum_type  mmgsdi_slot,
...
/* ============================================================================
   ...
   SIDE EFFECTS:
     None
============================================================================*/
void mmgsdi_util_find_first_sim_usim_app(
  mmgsdi_slot_id_enum_type  mmgsdi_slot,
  ...
  mmgsdi_slot_id_enum_type slot)
{
  ...
  if (app_gw_found)
  {
    if (mmgsdi_sub_sel_get_session_id_for_slot(slot,
                                               MMGSDI_TECH_3GPP,
                                               &session_id) != MMGSDI_SUCCESS)
    {
      UIM_MSG_HIGH_1("slot 0x%x not mapped to any session; not activating",
  ...
  /* Get RTRE configuration and if RTRE is NV_ONLY then don't auto-activate
     CSIM/RUIM apps as it is NV based not Card based. If it is auto_prov then
     we do straight mapping. So context can be derived from Slot. */
  result = mmgsdi_nv_get_item(NV_RTRE_CONFIG_I, &mmgsdi_nv_data, nv_context);
  ...
  if (app_1x_found || app_gw_found)
  {
    ...
    (void)mmgsdi_session_activate_or_switch_provisioning(
            session_id,
            slot,
            activation_app_data,
            TRUE,
...
/* ==========================================================================
   ...
   PARAMS:
     sel_app_ptr: Pointer to application information
...
)
{
  ...
  mmgsdi_status = mmgsdi_file_ef_access_conditions_check(prov_app_info_ptr,
                                                         attr_cache_ptr,
                                                         cmd_type,
                                                         &security_access,
                                                         session_id);
...
)
{
  ...
  /* The maximum size of the list needs to be limited to
     prevent integer overflows */
  if((src_file_path_ptr == NULL) || (num_files == 0) ||
     (dst_file_path_pptr == NULL) || (num_files > MMGSDI_FILE_LIST_MAX_SIZE))
  {
  ...
  for(i = 0; i < num_files; i++)
  {
    ...
    path_len = src_file_path_ptr[i].path_len;
  ...
  uint8                          *app_index_ptr)
{
  ...
  if(aid_ptr == NULL || app_index_ptr == NULL)
  {
    return MMGSDI_ERROR;
  }
...
{
  uint32                  i             = MMGSDI_MAX_CLIENT_INFO;
  uint32                  j             = MMGSDI_MAX_SESSION_INFO;
...
/* ==========================================================================
   FUNCTION:      MMGSDI_UTIL_GET_MMGSDI_SESSION_ON_CH_ZERO
...
==========================================================================*/
mmgsdi_return_enum_type mmgsdi_util_get_mmgsdi_session_on_ch_zero(
  mmgsdi_slot_id_enum_type   slot_id,
...
)
{
  ...
  return MMGSDI_SUCCESS;
}/* mmgsdi_util_get_mmgsdi_session_on_ch_zero */
...
)
{
  ...
  for(j = 0; j < MMGSDI_MAX_CLIENT_INFO; j++)
  {
    ...
    num_sessions = mmgsdi_client_id_reg_table[j]->num_session;
    for(k = 0; (num_sessions > 0) && (k < MMGSDI_MAX_SESSION_INFO); k++)
    {
      temp_session_ptr =
      ...
      num_sessions--;
      if((temp_session_ptr->channel_info_index < MMGSDI_MAX_CHANNEL_INFO) &&
...
)
{
  ...
  num_sessions = mmgsdi_client_id_reg_table[i]->num_session;
    for(j = 0; (num_sessions > 0) && (j < MMGSDI_MAX_SESSION_INFO); j++)
    {
      ...
      num_sessions--;
      if(mmgsdi_client_id_reg_table[i]->session_info_ptr[j]->session_id == session_id)
      {
...
mmgsdi_return_enum_type mmgsdi_util_check_file_access_data(
  ...
  mmgsdi_protocol_enum_type      protocol)
{
  mmgsdi_uim_ef_item_category_enum_type EF_category      = MMGSDI_EF_CATRGORY_ROOT;
  ...
  if (file_access.access_method == MMGSDI_EF_ENUM_ACCESS)
  {
    ...
    if (mmgsdi_util_is_prov_session(session_info_ptr->session_type))
    {
      if(((MMGSDI_ICC == protocol) &&
          ((EF_category == MMGSDI_EF_CATRGORY_USIM) || (EF_category == MMGSDI_EF_CATRGORY_CSIM))) ||
         ((MMGSDI_UICC == protocol) &&
          ((EF_category == MMGSDI_EF_CATRGORY_GSM)  || (EF_category == MMGSDI_EF_CATRGORY_CDMA))))
      {
    ...
    {
      if((EF_category == MMGSDI_EF_CATRGORY_USIM) ||(EF_category == MMGSDI_EF_CATRGORY_CSIM))
      {
        mmgsdi_status = MMGSDI_INCORRECT_PARAMS;
      }
    }
...
/* ============================================================================
FUNCTION:        MMGSDI_UTIL_IS_CLIENT_ID
_VALID_IN_CMD
...
)
{
  ...
  if(mmgsdi_nv_get_device_feature_status(MMGSDI_FEATURE_TMSI_PLMN_CHECK) == MMGSDI_FEATURE_DISABLED)
  {
    return mmgsdi_status;
  }
  ...
  mmgsdi_int_app_info_type *sel_app_ptr)
{
  ...
  MMGSDIUTIL_TMC_MEM_FREE(sel_app_ptr->eons_info.spn.eons_data.data_ptr);
...
)
{
  ...
  switch(error_condition)
  {
    ...
    case UIM_CMD_TIMEOUT_ERR_S:
      mmgsdi_error_condition = MMGSDI_CARD_ERR_CMD_TIMEOUT;
      break;
    case UIM_ERR_S:
    default:
...
/* ==========================================================================
   FUNCTION:      MMGSDI_UTIL_CONVERT_FROM_UIM_SLOT_TYPE
   ...
   SIDE EFFECTS:
     None
==========================================================================*/
mmgsdi_return_enum_type mmgsdi_util_convert_from_uim_slot_type(
  uim_slot_connection_enum_type     uim_slot_type,
  mmgsdi_slot_enum_type           * mmgsdi_slot_type_ptr)
{
  MMGSDIUTIL_RETURN_IF_NULL(mmgsdi_slot_type_ptr);

  switch(uim_slot_type)
  {
    case UIM_CONNECTION_ISO:
      *mmgsdi_slot_type_ptr = MMGSDI_CARD_LOCAL;
      break;

    case UIM_CONNECTION_BTSAP:
      *mmgsdi_slot_type_ptr = MMGSDI_CARD_REMOTE;
      break;

    default:
      *mmgsdi_slot_type_ptr = MMGSDI_CARD_TYPE_NONE;
      return MMGSDI_ERROR;
  }
  return MMGSDI_SUCCESS;
}/* mmgsdi_util_convert_from_uim_slot_type */


/* ==========================================================================
   FUNCTION:      MMGSDI_UTIL_CONVERT_FROM_UIM_CARD_TYPE

   DESCRIPTION:
     This function converts the UIM card type to MMGSDI card type
   ...
   SIDE EFFECTS:
     None
==========================================================================*/
mmgsdi_return_enum_type mmgsdi_util_convert_from_uim_card_type(
  uim_card_type_enum           uim_card_type,
  mmgsdi_card_type_enum_type * mmgsdi_card_type_ptr)
{
  MMGSDIUTIL_RETURN_IF_NULL(mmgsdi_card_type_ptr);

  switch(uim_card_type)
  {
    case UIM_CARD_TELECOM:
      *mmgsdi_card_type_ptr = MMGSDI_CARD_TELECOM;
      break;

    case UIM_CARD_SAM:
      *mmgsdi_card_type_ptr = MMGSDI_CARD_NON_TELECOM;
      break;

    default:
      *mmgsdi_card_type_ptr = MMGSDI_CARD_NONE;
      return MMGSDI_ERROR;
  }

  return MMGSDI_SUCCESS;
}/* mmgsdi_util_convert_from_uim_card_type */


/* ==========================================================================
   FUNCTION:      MMGSDI_UTIL_CONVERT_FROM_UIM_TRANSPORT_PROTOCOL_TYPE

   DESCRIPTION:
     This function converts the UIM transmission mode to MMGSDI transmission
     mode.
   ...
   SIDE EFFECTS:
     None
==========================================================================*/
mmgsdi_return_enum_type mmgsdi_util_convert_from_uim_transport_protocol_type(
  uim_transport_protocol_type           uim_transmission_mode,
  mmgsdi_transport_protocol_enum_type * mmgsdi_transmission_mode_ptr)
{
  MMGSDIUTIL_RETURN_IF_NULL(mmgsdi_transmission_mode_ptr);

  switch(uim_transmission_mode)
  {
    case UIM_T_0_PROTOCOL:
      *mmgsdi_transmission_mode_ptr = MMGSDI_T_0_PROTOCOL;
      break;

    case UIM_T_1_PROTOCOL:
      *mmgsdi_transmission_mode_ptr = MMGSDI_T_1_PROTOCOL;
      break;

    default:
      *mmgsdi_transmission_mode_ptr = MMGSDI_NO_T_PROTOCOL;
      return MMGSDI_ERROR;
  }

  return MMGSDI_SUCCESS;
}/* mmgsdi_util_convert_from_uim_transport_protocol_type */
  ...
  switch (refresh_mode)
  {
    ...
    case MMGSDI_REFRESH_3G_SESSION_RESET:
      ...
      if (is_icc)
      {
        ...
        mmgsdi_card_init_app_tp_state_init(app_info_ptr);
      }
      ...
      {
        MMGSDIUTIL_TMC_MEM_FREE(
          app_info_ptr->cphs_info_ptr->cphs_ons_ptr);
        MMGSDIUTIL_TMC_MEM_FREE(
          app_info_ptr->cphs_info_ptr->cphs_ons_short_ptr);
        memset(app_info_ptr->cphs_info_ptr,
               0x00, sizeof(mmgsdi_cphs_int_info_type));
      }

      /* Clear orig. cache info */
      if(app_info_ptr->srv_table_orig_cache.data_ptr != NULL)
      {
        MMGSDIUTIL_TMC_MEM_FREE(
          app_info_ptr->srv_table_orig_cache.data_ptr);
        app_info_ptr->srv_table_orig_cache.data_len = 0;
      }
      ...
      switch (app_info_ptr->app_data.app_type)
      {
        case MMGSDI_APP_USIM:
          ...
          mmgsdi_csg_reset_eons_flags_in_app_info(app_info_ptr);
...
)
{
  ...
  while(index < hist_ptr->hist_char_length)
  {
    ...
    /* Pre-issuing data as per sec 8.1.1.2.6 of ISO/IEC 7816-4 */
    if ((index < hist_ptr->hist_char_length) &&
        (index < MMGSDI_MAX_HIST_CHAR_LEN) &&
        ((hist_ptr->hist_char_data[index] & 0xF0) == MMGSDI_PRE_ISSUING_DATA_MASK))
    {
      ...
      {
        /* Increment the index to skip Y Bytes to skip pre-issuing data element */
        index = index + length;
      }
    }
        ...
        (hist_ptr->hist_char_data[index] == MMGSDI_CARD_CAPABILITIES))
    {
          ...
          index < MMGSDI_MAX_HIST_CHAR_LEN)
      {
        ...
        /*  For cards specifying fewer channels than legacy behavior
            assume out of spec card and default to legacy behavior based on NV. */
        else if(slot_data_ptr->available_logical_channels < MMGSDI_LEGACY_LOGICAL_CHANNELS &&
                mmgsdi_nv_get_device_feature_status(MMGSDI_FEATURE_DEFAULT_LEGACY_LOGICAL_CHANNELS) == MMGSDI_FEATURE_ENABLED)
        {
          slot_data_ptr->available_logical_channels = MMGSDI_LEGACY_LOGICAL_CHANNELS;
        }
    ...
    /* Status indicator and card issuer data as per sec 8.1.1.3 and 8.1.1.2.5 of ISO/IEC 7816-4 respectively*/
    if ((index < hist_ptr->hist_char_length) &&
        (index < MMGSDI_MAX_HIST_CHAR_LEN) &&
        (((hist_ptr->hist_char_data[index] & 0xF0) == MMGSDI_STATUS_INDICATOR_MASK)||
        ((hist_ptr->hist_char_data[index] & 0xF0) == MMGSDI_CARD_ISSUER_DATA)))
    {
      /* Length of Bytes to skip */
      length = hist_ptr->hist_char_data[index] & mask_length_nibble;
      ...
      {
        /* Increment the index to skip Y Bytes to skip status indicator element */
        index = index + length;
      }
    }
...
/* ============================================================================
   ...
   SIDE EFFECTS:
     None
============================================================================*/
uint8 mmgsdi_util_get_channel_id_from_cla(
  uint8                     cla
)
{
  ...
  {
    return ((cla & MMGSDI_CLA_EXT_LOGICAL_CHANNEL_MASK) +
                      MMGSDI_FIRST_EXT_LOGICAL_CHANNEL);
  }

  return (cla & MMGSDI_CLA_STD_LOGICAL_CHANNEL_MASK);
...
)
{
  ...
  do
  {
    ...
    /* Generate session id. */
    session_ptr->session_id = mmgsdi_util_get_next_available_id();
    ...
    /* Initialise channel_info_index to 0x3c, so that this session 
       is treated as deactivated before it is activated by MMGSDI */
    session_ptr->channel_info_index = MMGSDI_MAX_CHANNEL_INFO;
  }while(0);
...
/* ==========================================================================
   FUNCTION:      MMGSDI_UTIL_GET_NEXT_AVAILABLE_ID

   DESCRIPTION:
     This function returns next available ID for using it as
     session ID or client ID
...
CRITICAL SECTIONS:
     ...
     to client ID reg. table and session info ptr.
==========================================================================*/
uint32 mmgsdi_util_get_next_available_id
(
  void
)
{
  uint32                  next_id           = MMGSDI_SESSION_ID_ZERO;
  boolean                 found             = FALSE;
  static uint32           next_available_id = MMGSDI_BASE_ID;
  static boolean          max_id_reached    = FALSE;
  uint32                  client_index      = 0;
  uint32                  session_index     = 0;
  mmgsdi_return_enum_type mmgsdi_status     = MMGSDI_ERROR;
  uint64                  client_id         = 0;
  ...
  /* Make sure session ID is non-zero and less than MMGSDI_INVALID_SESSION_ID */
  if (next_available_id < MMGSDI_BASE_ID)
  {
    next_available_id = MMGSDI_BASE_ID;
  }

  if (MMGSDI_MAX_ID == next_available_id)
  {
    next_available_id = MMGSDI_BASE_ID;
    max_id_reached    = TRUE;
  }

  /* Once we hit the max value and loop back to 0, need to check
     mmgsdi_client_id_reg_table to find a value that is not being used */
  if (max_id_reached)
  {
    do
    {
      found = TRUE;
      /* If MAX ID is reached, check the table if the id is
         already in use for client or session*/
      client_id = next_available_id;
      ...
      {
        next_available_id++;
        if (next_available_id == MMGSDI_MAX_ID ||
            next_available_id < MMGSDI_BASE_ID)
        {
          next_available_id = MMGSDI_BASE_ID;
        }
  ...
  /* Update session_id_max if needed */
  next_id = next_available_id;

  /* Increment mmgsdi_generic_data_ptr->session_id_next_available */
  next_available_id++;

  MMGSDIUTIL_LEAVE_CLIENT_APP_DATA_CRIT_SECT;

  return next_id;
}/* mmgsdi_util_get_next_available_id */
     ...
     (mmgsdi_refresh_info_ptr[slot_index]->request_header.slot_id == slot_id))
  {
    ...
    while(node_ptr)
    {
      if(node_ptr->session_id == session_id)
      {
        ...
        mmgsdi_status = mmgsdi_refresh_rsp_send_cnf(
                          &mmgsdi_refresh_info_ptr[slot_index]->request_header);
        break;
      }
...
)
{
  ...
  /* If device supports only one slot and CDMA LESS NV is enabled for primary
     subscription, don't reserve a channel */
  if(mmgsdi_generic_data_ptr->num_slots == 1 &&
     (mmgsdi_nv_get_slot_feature_status(
        MMGSDI_FEATURE_CDMA_LESS, MMGSDI_NV_CONTEXT_PRIMARY) == MMGSDI_FEATURE_ENABLED))
  {
    return FALSE;
  }

  /* If device supports two slots and CDMA LESS NV is enabled for both primary
     and secondary subscription, don't reserve a channel */
  if(mmgsdi_generic_data_ptr->num_slots == 2 &&
     (mmgsdi_nv_get_slot_feature_status(
        MMGSDI_FEATURE_CDMA_LESS, MMGSDI_NV_CONTEXT_PRIMARY) == MMGSDI_FEATURE_ENABLED) &&
     (mmgsdi_nv_get_slot_feature_status(
        MMGSDI_FEATURE_CDMA_LESS, MMGSDI_NV_CONTEXT_SECONDARY) == MMGSDI_FEATURE_ENABLED))
  {
    return FALSE;
  }
     ...
     (mmgsdi_util_get_csim_apps(slot_id)))
  {
    ...
    for(index = 0; index < MMGSDI_MAX_APP_INFO; index++)
    {
      ...
      /* If CSIM app termination is skipped during session deactivation, app
         state will be moved back to DETECTED but app_selected flag will remain
         TRUE. */
      if(slot_data_ptr->app_info_pptr[index]->app_data.app_type == MMGSDI_APP_CSIM &&
         (slot_data_ptr->app_info_pptr[index]->app_state > MMGSDI_APP_STATE_DETECTED ||
          slot_data_ptr->app_info_pptr[index]->app_selected))
      {
        return FALSE;
      }
    }
...
/*===========================================================================
  FUNCTION MMGSDI_UTIL_SET_SIGNAL_FOR_TASK

DESCRIPTION
  This function sets the signal for the provided MMGSDI task
...
SIDE EFFECTS
  None
============================================================================*/
mmgsdi_return_enum_type mmgsdi_util_set_signal_for_task(
  mmgsdi_task_enum_type mmgsdi_task,
  rex_sigs_type         sigs
)
{
  switch (mmgsdi_task)
  {
    case MMGSDI_TASK_MAIN:
      (void) rex_set_sigs(UIM_GSDI_TCB, sigs);
      break;

    case MMGSDI_TASK_SLOT1:
      (void) rex_set_sigs(UIM_GSDI_SLOT1_TCB, sigs);
      break;

    case MMGSDI_TASK_SLOT2:
      (void) rex_set_sigs(UIM_GSDI_SLOT2_TCB, sigs);
      break;

    case MMGSDI_TASK_SLOT3:
      (void) rex_set_sigs(UIM_GSDI_SLOT3_TCB, sigs);
      break;
  ...
  return MMGSDI_SUCCESS;
} /* mmgsdi_util_set_signal_for_task */


/*===========================================================================
  FUNCTION MMGSDI_UTIL_GET_SLOT_INDEX_FROM_TASK

DESCRIPTION
  This function returns index of the MMGSDI slot corresponding to the task
...
SIDE EFFECTS
  None
============================================================================*/
mmgsdi_return_enum_type mmgsdi_util_get_slot_index_from_task(
  mmgsdi_task_enum_type  mmgsdi_task,
...
)
{
  ...
  MMGSDIUTIL_RETURN_IF_NULL(slot_index_ptr);

  slot_id = mmgsdi_util_get_slot_from_task(mmgsdi_task);

  return mmgsdi_util_get_slot_index(slot_id, slot_index_ptr);
} /* mmgsdi_util_get_slot_index_from_task */
  ...
  switch(task_cmd_ptr->cmd.cmd_enum)
  {
    ...
    case MMGSDI_PURGE_TEMPORARY_IDENTITY_REQ:
      if(task_cmd_ptr->cmd.cmd_enum == MMGSDI_SESSION_DEACTIVATE_REQ &&
         mmgsdi_nv_get_device_feature_status(MMGSDI_FEATURE_SKIP_PROV_APP_TERMINATION) ==
           MMGSDI_FEATURE_ENABLED)
      {
        /* If skip_app_termination NV item is enabled, we only perform a logical
           delinking of the session with the app. We do not actually close the
           channel with the card. Since no card interaction is required in this
           case, this command can be processed in main task */
        *slot_id_ptr = MMGSDI_SLOT_AUTOMATIC;
        return MMGSDI_SUCCESS;
      }
      mmgsdi_status = mmgsdi_util_get_session_app_info(
    ...
    case MMGSDI_REFRESH_REQ:
      ...
      {
        /* The following refresh commands don't need to interact with the card and
           thus can be processed in the main task:
           - Registration for REFRESH
           - Deregistration for REFRESH
           - APP_RESET_AUTO type REFRESH, used to deactivate a session, when
             skip_app_termination is TRUE. */
        *slot_id_ptr = MMGSDI_SLOT_AUTOMATIC;
      }
      ...
      else
      {
        ...
        else if((mmgsdi_nv_get_device_feature_status(MMGSDI_FEATURE_SKIP_PROV_APP_TERMINATION) ==
                   MMGSDI_FEATURE_ENABLED) &&
                (MMGSDI_SUCCESS == mmgsdi_util_get_slot_index(*slot_id_ptr, &slot_index)) &&
                (mmgsdi_refresh_info_ptr[slot_index] != NULL) &&
                (mmgsdi_refresh_info_ptr[slot_index]->orig_mode == MMGSDI_REFRESH_APP_RESET_AUTO))
        {
          /* For APP_RESET_AUTO (used to deactivate a session), when
             skip_app_termination NV is enabled, the refresh requests
             such as OK_TO_PROCEED do not need to interact with the
             card and hence can be handled in main task. */
          *slot_id_ptr = MMGSDI_SLOT_AUTOMATIC;
        }
      }
      return MMGSDI_SUCCESS;
    ...
    case MMGSDI_SESSION_ACT_OR_SWITCH_PROV_REQ:
      session_id_status = mmgsdi_util_is_session_id_valid(
                                    task_cmd_ptr->cmd.cmd.session_act_or_switch_prov_req.request_header.session_id);
      if(session_id_status == MMGSDI_SESSION_ID_ASSIGNED_AND_ACTIVE)
      {
        mmgsdi_status = mmgsdi_util_get_session_app_info(
                          task_cmd_ptr->cmd.cmd.session_act_or_switch_prov_req.request_header.session_id,
      ...
      else if(mmgsdi_util_check_app_termination_skipped(task_cmd_ptr->cmd.cmd.session_act_or_switch_prov_req.request_header.slot_id,
                                                        &task_cmd_ptr->cmd.cmd.session_act_or_switch_prov_req.app_id_data,
                                                        NULL))
      {
        /* If the activation is being attempted on an app that was only logically
           deactivated earlier, that is, the channel was not closed as part of
           deactivation due to skip_app_termination being TRUE, if that channel is
           still available and linked to the app for which we have received this
           activation request, the request does not need any card interaction and
           hence can be processed in main task */
        *slot_id_ptr = MMGSDI_SLOT_AUTOMATIC;
      }
      else
      {
        *slot_id_ptr = task_cmd_ptr->cmd.cmd.session_act_or_switch_prov_req.request_header.slot_id;
      }
      return MMGSDI_SUCCESS;
  ...
  for(slot_id = MMGSDI_SLOT_1; slot_id < MMGSDI_SLOT_AUTOMATIC; slot_id++)
  {
        ...
        (card_num_apps_ptr != NULL))
    {
      ...
      for (app_count = 0; app_count < MMGSDI_MAX_APP_INFO; app_count++)
      {
        ...
        card_app_info_ptr[card_app_count].slot_type = slot_data_ptr->slot_type;

        /* Update PIN status for apps where app state is greater than
           DEACTIVATED i.e. app should be activated. When skip app termination is
           enabled, MMGSDI does not reset PIN related data in app info ptr
           because app is still active on card. So we need to check app state here. */
        if(slot_data_ptr->app_info_pptr[app_count]->app_state > MMGSDI_APP_STATE_DEACTIVATED &&
           slot_data_ptr->app_info_pptr[app_count]->pin1_ptr != NULL &&
        ...
        /* PIN 2 INFO */
        if(slot_data_ptr->app_info_pptr[app_count]->app_state > MMGSDI_APP_STATE_DEACTIVATED &&
           slot_data_ptr->app_info_pptr[app_count]->pin2.cached)
        {
        ...
        if(slot_data_ptr->app_info_pptr[app_count]->app_state > MMGSDI_APP_STATE_DEACTIVATED &&
           slot_data_ptr->app_info_pptr[app_count]->universal_pin_ptr != NULL &&
...
mmgsdi_return_enum_type mmgsdi_util_apdu_sanity_check(
  ...
  mmgsdi_transport_protocol_enum_type    transmission_mode
)
{
  uint32 apdu_data_len = 0;
  
  MMGSDIUTIL_RETURN_IF_NULL(apdu_data_ptr);

  if (apdu_data_ptr->data_len < MMGSDI_MIN_APDU_LEN)
  {
    return MMGSDI_INCORRECT_PARAMS;
  }
  ...
  if (apdu_data_ptr->data_len > UIM_CASE2_7816_APDU_SIZE &&
      apdu_data_ptr->data_len <= MMGSDI_MAX_CHANNEL_DATA_CMD_LEN)
  {
    if (apdu_data_ptr->data_ptr[UIM_CASE3_4_7816_LC_OFFSET] == 0)
    {
      apdu_data_len = apdu_data_ptr->data_ptr[UIM_CASE3_4_7816_LC_OFFSET+1] << 8;
      apdu_data_len |= apdu_data_ptr->data_ptr[UIM_CASE3_4_7816_LC_OFFSET+2];

      if((apdu_data_ptr->data_len != UIM_CASE3_4_7816_LC_OFFSET + 3) && 
         ((apdu_data_ptr->data_len - (UIM_CASE3_4_7816_LC_OFFSET + 3)) != apdu_data_len) &&
         ((apdu_data_ptr->data_len - (UIM_CASE3_4_7816_LC_OFFSET + 5)) != apdu_data_len))
      {
        UIM_MSG_ERR_0("Malformed APDU");
        return MMGSDI_INCORRECT_PARAMS;
      }      
    }
    else if ((apdu_data_ptr->data_ptr[UIM_CASE3_4_7816_LC_OFFSET] !=
  ...
  else if (apdu_data_ptr->data_len > MMGSDI_MAX_CHANNEL_DATA_CMD_LEN)
  {
    if (apdu_data_ptr->data_ptr[UIM_CASE3_4_7816_LC_OFFSET] != 0)
    {
      return MMGSDI_INCORRECT_PARAMS;
    ...
    apdu_data_len = apdu_data_ptr->data_ptr[UIM_CASE3_4_7816_LC_OFFSET+1] << 8;
    apdu_data_len |= apdu_data_ptr->data_ptr[UIM_CASE3_4_7816_LC_OFFSET+2];
    if(((apdu_data_ptr->data_len - (UIM_CASE3_4_7816_LC_OFFSET + 3)) != apdu_data_len) &&
       ((apdu_data_ptr->data_len - (UIM_CASE3_4_7816_LC_OFFSET + 5)) != apdu_data_len))
    {
      UIM_MSG_ERR_0("Malformed APDU");
      return MMGSDI_INCORRECT_PARAMS;
  ...
  /* Per ISO 7816-4 Section 5.1, return error for invalid CLA byte (0xFF) */
  if (apdu_data_ptr->data_ptr[UIM_7816_APDU_CLASS_OFFSET] == 0xFF)
  {
    return MMGSDI_INCORRECT_PARAMS;
  }

  /* Per ISO 7816-4 Section 5.1, return error for INS byte (0x6X or 0x9X)
     for telecom cards */
  if ((transmission_mode == MMGSDI_T_0_PROTOCOL) &&
...
)
{
  ...
  /* Return TRUE in following cases:
     1. MMGSDI_FEATURE_PURGE_TEMP_IDENTITIES is enabled and refresh mode is RESET
     2. MMGSDI_FEATURE_PURGE_TEMP_IDENTITIES_INIT_FCN_REFRESH_WITH_IMSI_CHANGE
        is enabled and refresh mode is INIT+FCN */
  if(mmgsdi_refresh_info_ptr[slot_index] != NULL &&
     ((MMGSDI_REFRESH_RESET   == mmgsdi_refresh_info_ptr[slot_index]->mode &&
       MMGSDI_FEATURE_ENABLED == mmgsdi_nv_get_slot_feature_status(
                                   MMGSDI_FEATURE_PURGE_TEMP_IDENTITIES,
                                   nv_context))
                             ||
      (MMGSDI_REFRESH_NAA_INIT_FCN == mmgsdi_refresh_info_ptr[slot_index]->mode &&
       MMGSDI_FEATURE_ENABLED == mmgsdi_nv_get_slot_feature_status(
...
/* =============================================================================
  FUNCTION:      MMGSDI_UTIL_FIND_PROV_SESSION_FOR_SLOT

  DESCRIPTION:
    This function finds an MMGSDI prov session ID active on the slot based on
    technology type
  ...
  SIDE EFFECTS:
    None
=============================================================================*/
mmgsdi_return_enum_type mmgsdi_util_find_prov_session_for_slot(
  mmgsdi_slot_id_enum_type           slot_id,
  mmgsdi_session_id_type           * session_id_ptr,
  mmgsdi_app_enum_type             * app_type_ptr,
  mmgsdi_technology_enum_type        technology_type
)
{
  mmgsdi_return_enum_type    mmgsdi_status          = MMGSDI_SUCCESS;
  mmgsdi_channel_info_type  *channel_info_ptr       = NULL;
  uint8                      i                      = 0;
  mmgsdi_session_id_type    *mmgsdi_session_ids_ptr = NULL;
  uint8                      session_ids_len        = 0;
  mmgsdi_session_id_type     session_ids_1x[]       = {mmgsdi_generic_data_ptr->pri_1x_session_id,
                                                       mmgsdi_generic_data_ptr->sec_1x_session_id,
                                                       mmgsdi_generic_data_ptr->ter_1x_session_id};
  mmgsdi_session_id_type     session_ids_gw[]       = {mmgsdi_generic_data_ptr->pri_gw_session_id,
                                                       mmgsdi_generic_data_ptr->sec_gw_session_id,
                                                       mmgsdi_generic_data_ptr->ter_gw_session_id};
  ...
  if (technology_type == MMGSDI_TECH_3GPP)
  {
    mmgsdi_session_ids_ptr = session_ids_gw;
    session_ids_len = sizeof(session_ids_gw) / sizeof(mmgsdi_session_id_type);
  }
  else
  {
    mmgsdi_session_ids_ptr = session_ids_1x;
    session_ids_len = sizeof(session_ids_1x) / sizeof(mmgsdi_session_id_type);
  }

  for (i = 0; i < session_ids_len; i++)
  {
    mmgsdi_status = mmgsdi_util_get_session_and_channel_info(
                        mmgsdi_session_ids_ptr[i],
                        NULL,
                        &channel_info_ptr);
    ...
    {
      if ((technology_type == MMGSDI_TECH_3GPP &&
           (channel_info_ptr->app_info_ptr->app_data.app_type == MMGSDI_APP_SIM ||
            channel_info_ptr->app_info_ptr->app_data.app_type == MMGSDI_APP_USIM)) ||
          (technology_type == MMGSDI_TECH_3GPP2 &&
           (channel_info_ptr->app_info_ptr->app_data.app_type == MMGSDI_APP_RUIM ||
            channel_info_ptr->app_info_ptr->app_data.app_type == MMGSDI_APP_CSIM)))
      {
        *session_id_ptr = mmgsdi_session_ids_ptr[i];
  ...
  return MMGSDI_ERROR;
} /* mmgsdi_util_find_prov_session_for_slot */
...
)
{
  ...
  if(mmgsdi_channel_info_ptr_table[channel_info_index]->app_info_ptr->app_data.app_type != MMGSDI_APP_SIM &&
     mmgsdi_channel_info_ptr_table[channel_info_index]->app_info_ptr->app_data.app_type != MMGSDI_APP_RUIM)
  {
    (void)mmgsdi_cache_delete(
            mmgsdi_channel_info_ptr_table[channel_info_index]->app_info_ptr,
            0,
            NULL);
    (void)mmgsdi_cache_delete_attr_selective_items(
            &mmgsdi_channel_info_ptr_table[channel_info_index]->app_info_ptr->attr_cache_tbl,
            NULL, 0, NULL, 0);
    mmgsdi_csg_reset_eons_flags_in_app_info(mmgsdi_channel_info_ptr_table[channel_info_index]->app_info_ptr);
    mmgsdi_eons_opl_pnn_delete(&mmgsdi_channel_info_ptr_table[channel_info_index]->app_info_ptr->opl_cache);
    mmgsdi_eons_opl_pnn_delete(&mmgsdi_channel_info_ptr_table[channel_info_index]->app_info_ptr->pnn_cache);

    if(mmgsdi_channel_info_ptr_table[channel_info_index]->app_info_ptr->cphs_info_ptr != NULL)
    {
      MMGSDIUTIL_TMC_MEM_FREE(
        mmgsdi_channel_info_ptr_table[channel_info_index]->app_info_ptr->cphs_info_ptr->cphs_ons_ptr);
      MMGSDIUTIL_TMC_MEM_FREE(
        mmgsdi_channel_info_ptr_table[channel_info_index]->app_info_ptr->cphs_info_ptr->cphs_ons_short_ptr);
      MMGSDIUTIL_TMC_MEM_FREE(mmgsdi_channel_info_ptr_table[channel_info_index]->app_info_ptr->cphs_info_ptr);
    }
  }

  /* Clear orig. cache info */
  if(mmgsdi_channel_info_ptr_table[channel_info_index]->app_info_ptr->srv_table_orig_cache.data_ptr != NULL)
  {
    MMGSDIUTIL_TMC_MEM_FREE(
      mmgsdi_channel_info_ptr_table[channel_info_index]->app_info_ptr->srv_table_orig_cache.data_ptr);
    mmgsdi_channel_info_ptr_table[channel_info_index]->app_info_ptr->srv_table_orig_cache.data_len = 0;
  }
  ...
  mmgsdi_channel_info_ptr_table[channel_info_index]->app_info_ptr->first_loci_write_after_activation = FALSE;
  ...
  /* Don't break channel to app mapping for SIM and RUIM app */
  if(mmgsdi_channel_info_ptr_table[channel_info_index]->app_info_ptr->app_data.app_type != MMGSDI_APP_SIM &&
     mmgsdi_channel_info_ptr_table[channel_info_index]->app_info_ptr->app_data.app_type != MMGSDI_APP_RUIM)
  {
    mmgsdi_channel_info_ptr_table[channel_info_index]->app_info_ptr = NULL;
...
DESCRIPTION
  ...
  termination was previously skipped on that channel or if it is ICC card.
...
)
{
  ...
  for (i = 0; i < MMGSDI_MAX_CHANNEL_INFO; i++)
  {
    /* Channel info ptr pointing to app present on ICC card should be freed
       here. We are not freeing it on session deactivation to retain "channel
       to app" mapping. */
    if (mmgsdi_channel_info_ptr_table[i] == NULL ||
...
/* =============================================================================
  ...
  DESCRIPTION:
    This function retrieves the app_info_ptr by matching the slot_id, channel_id
    and optionally the app_type in mmgsdi_channel_info_ptr_table.
    It takes app_type into account if app_type is one of the valid ICC SIM/RUIM
    prov app types. Otherwise, it ignores the app_type.
    It is recommended to pass app_type in case of ICC. See comments inline for
    mroe details.
  ...
  mmgsdi_int_app_info_type        ** app_info_pptr)
{
  ...
  for (i = 0; i < MMGSDI_MAX_CHANNEL_INFO; i++)
  {
    ...
    /* mmgsdi_channel_info_ptr_table at this index has a matching channel_id at
       this point. See if the valid prov app_type was provided as input, if so,
       we need to take that into account as well and ensure that the app_type
       also matches the app type at this index in mmgsdi_channel_info_ptr_table.
       In case of UICC, the caller should not have to pass an app_type
       explicitly as every prov app_type would have its own separate channel_id
       and the above slot_id and channel_id checks should be sufficient in
       locating the channel_info_ptr_table entry for the app.
       However, in case of ICC, SIM and RUIM would both have channel_id as zero.
       In that case, unless the caller passes an app_type, we would always end
       up picking the first entry matching this channel_id in channel_info_ptr_table
       which can lead to ambiguities. So, in case this function is called for ICC,
       client should also pass an app_type for a more accurate app data. */
    if ((app_type == MMGSDI_APP_SIM ||
         app_type == MMGSDI_APP_RUIM) &&
        mmgsdi_channel_info_ptr_table[i]->app_info_ptr->app_data.app_type != app_type)
    {
      /* Valid ICC app_type was provided but it did not match the app at this
         index. This is not an error condition as this index could have either
         of SIM or RUIM apps (because slot_id and channel_id could be same for
         both these apps)... so, continue looping through */
      continue;
    }
...
/* ============================================================================
FUNCTION MMGSDI_UTIL_IS_MMGSDI_PROV_SESSIONID

DESCRIPTION
  If the session id is a prov session id belonging to MMGSDI, this function
...
SIDE EFFECTS
  None
============================================================================*/
boolean mmgsdi_util_is_mmgsdi_prov_sessionid(
  mmgsdi_session_id_type session_id
)
{
  boolean is_mmgsdi_prov_session = FALSE;
  ...
  if (mmgsdi_generic_data_ptr &&
      (session_id == mmgsdi_generic_data_ptr->pri_gw_session_id ||
       ...
       session_id == mmgsdi_generic_data_ptr->ter_1x_session_id))
  {
    is_mmgsdi_prov_session = TRUE;
  }

  return is_mmgsdi_prov_session;
} /* mmgsdi_util_is_mmgsdi_prov_sessionid */
...
/* ============================================================================
FUNCTION MMGSDI_UTIL_CLIENT_RECEIVES_HIDDEN_CSIM_EVENTS

DESCRIPTION
  This function determines if the client has requested to receive events
  related to a hidden CSIM.
...
SIDE EFFECTS
  None
============================================================================*/
boolean mmgsdi_util_client_receives_hidden_csim_events(
  uint32            client_index
)
{
  if (client_index >= MMGSDI_MAX_CLIENT_INFO ||
  ...
  return mmgsdi_client_id_reg_table[client_index]->receive_hidden_csim_events;
} /* mmgsdi_util_client_receives_hidden_csim_events */


/* ============================================================================
FUNCTION MMGSDI_UTIL_FIND_COMMANDS_IN_QUEUE_FOR_SESSION_TYPE

DESCRIPTION
  This function finds the number of commands in the queue for the session type.
...
SIDE EFFECTS
  None
============================================================================*/
uint16 mmgsdi_util_find_commands_in_queue_for_session_type(
  q_type                        * queue_ptr,
  mmgsdi_session_type_enum_type   session_type
)
{
  q_link_type                    link;
  uint16                         num_cmds         = 0;
  mmgsdi_session_type_enum_type  cmd_session_type = MMGSDI_MAX_SESSION_TYPE_ENUM;
  ...
  while (command_ptr != NULL)
  {
    ...
    if((mmgsdi_util_get_session_type(command_ptr->cmd.cmd.request_header.session_id,
                                     &cmd_session_type, NULL) == MMGSDI_SUCCESS) &&
       cmd_session_type == session_type)
    {
      num_cmds++;
    }

    command_ptr = (mmgsdi_task_cmd_type*)q_next(queue_ptr, &link);
  }
...
/*===========================================================================
FUNCTION:      MMGSDI_UTIL_QUEUE_CLIENT_DEREG_REQ

DESCRIPTION
  This utility function builds the DEREG request and queues to the task.
...
===========================================================================*/
mmgsdi_return_enum_type mmgsdi_util_queue_client_dereg_req (
  mmgsdi_client_id_type      client_id,
  mmgsdi_task_enum_type      mmgsdi_task,
...
)
{
  ...
  return mmgsdi_status;
} /* mmgsdi_util_queue_client_dereg_req */


/*===========================================================================
  FUNCTION MMGSDI_UTIL_PURGE_TEMP_IDENTITIES
  ...
  SIDE EFFECTS:
    none
 ===========================================================================*/
mmgsdi_return_enum_type mmgsdi_util_purge_temp_identities(
  mmgsdi_slot_id_enum_type   slot_id,
  mmgsdi_session_id_type     session_id,
...
)
{
  ...
  mmgsdi_return_enum_type            mmgsdi_status    = MMGSDI_ERROR;
  mmgsdi_task_enum_type              mmgsdi_task      = MMGSDI_TASK_MAX;
  mmgsdi_sync_enum_type              synch_type       = MMGSDI_SYNC_PROCESS;
  ...
  switch(file_enum)
  {
    ...
    case MMGSDI_USIM_EPSNSC:
      mmgsdi_status = mmgsdi_cache_read_len(session_id,
                                            &access,
                                            1,
                                            &read_write_data.data_len,
                                            NULL);
      if (mmgsdi_status != MMGSDI_SUCCESS)
      {
        read_write_data.data_len = 0;
      }
      break;
    default:
      break;
  ...
  if(read_write_data.data_len == 0)
  {
    return MMGSDI_ERROR;
  }

  MMGSDIUTIL_MEM_MALLOC_AND_VALIDATE(read_write_data.data_ptr, read_write_data.data_len);
  if(read_write_data.data_ptr == NULL)
  {
    return MMGSDI_MEMORY_ERROR_HEAP_EXHAUSTED;
  }

  memset(read_write_data.data_ptr, 0x00, read_write_data.data_len);

  /* Only read those EFs from card which have few bytes left unchanged while purging. 
     For rest of the EFs, which are completely changed as part of purging, 
     the logic remains the same. They are read from cache for purging. */
  if(file_enum == MMGSDI_GSM_LOCI ||
     file_enum == MMGSDI_USIM_LOCI ||
     file_enum == MMGSDI_USIM_PSLOCI ||
     file_enum == MMGSDI_USIM_EPSLOCI)
  {
    ...
    switch(protocol)
    {
      ...
      case MMGSDI_UICC:
        ...
        break;
      default:
        /* If App is neither from UICC or ICC protocol */
          MMGSDIUTIL_TMC_MEM_FREE(read_req_ptr);
          mmgsdi_status = MMGSDI_ERROR;
    }

    if (read_cnf_ptr == NULL || (mmgsdi_status != MMGSDI_SUCCESS) ||
        (read_cnf_ptr->response_header.mmgsdi_status != MMGSDI_SUCCESS))
    {
      UIM_MSG_ERR_1("Read from card failed, mmgsdi_status 0x%x", mmgsdi_status);
      if(read_cnf_ptr != NULL)
      {
        UIM_MSG_ERR_1("Read from card failed, cnf status 0x%x",
  ...
  if (MMGSDI_SUCCESS != mmgsdi_cache_read(session_id,
                                          ...
                                          NULL))
  {
    /* At the point, the files should have already been cached.
  ...
  switch(file_enum)
  {
    ...
    case MMGSDI_USIM_EPSNSC:
      /* Set EF_EPSNSC bytes:
          Byte all - 0xFF
      */
      memset(read_write_data.data_ptr, 0xFF, read_write_data.data_len);
      break;
  ...
  if(!req_ptr)
  {
    MMGSDIUTIL_TMC_MEM_FREE(read_write_data.data_ptr);
    return MMGSDI_MEMORY_ERROR_HEAP_EXHAUSTED;
  }
  ...
  if(file_enum == MMGSDI_USIM_EPSNSC)
  {
    req_ptr->file_type = MMGSDI_LINEAR_FIXED_FILE;
    req_ptr->rec_num = 1;
    if(MMGSDI_UICC == protocol)
    {
      mmgsdi_status = mmgsdi_uim_uicc_write_record(req_ptr, TRUE, &cnf_ptr);
    }
    else
    {
      mmgsdi_status = MMGSDI_INCORRECT_PARAMS;
  ...
  UIM_MSG_MED_1("mmgsdi_util_purge_temp_identities: mmgsdi_status=0x%x",
                 mmgsdi_status);
  ...
  return mmgsdi_status;
}/* mmgsdi_util_purge_temp_identities */
...
/* =============================================================================
  FUNCTION:      MMGSDI_UTIL_GET_AS_ID_FROM_SLOT_ID

  DESCRIPTION:
    This function determines as id from slot id. If there is no session
    activated on slots, it returns SYS_MODEM_AS_ID_1 as default.
  ...
  SIDE EFFECTS:
    None
=============================================================================*/
sys_modem_as_id_e_type mmgsdi_util_get_as_id_from_slot_id
(
  mmgsdi_slot_id_enum_type slot_id
)
{
  mmgsdi_slot_id_enum_type session_slot_id = MMGSDI_SLOT_NONE;
  uint8                    index           = 0;
  ...
  {
    {mmgsdi_generic_data_ptr->pri_gw_session_id, SYS_MODEM_AS_ID_1},
    {mmgsdi_generic_data_ptr->pri_1x_session_id, SYS_MODEM_AS_ID_1},
    {mmgsdi_generic_data_ptr->sec_gw_session_id, SYS_MODEM_AS_ID_2},
    {mmgsdi_generic_data_ptr->sec_1x_session_id, SYS_MODEM_AS_ID_2},
    {mmgsdi_generic_data_ptr->ter_gw_session_id, SYS_MODEM_AS_ID_3},
    {mmgsdi_generic_data_ptr->ter_1x_session_id, SYS_MODEM_AS_ID_3}
  };

  for (index = 0; index < sizeof(table) / sizeof(table[0]); index++)
  {
    if((mmgsdi_util_get_session_app_info(table[index].session_id, &session_slot_id, NULL, NULL, NULL, NULL, NULL) == MMGSDI_SUCCESS) &&
       (session_slot_id == slot_id))
    {
      return table[index].as_id;
    }
  }

  return SYS_MODEM_AS_ID_1;
} /* mmgsdi_util_get_as_id_from_slot_id */


/* =============================================================================
  FUNCTION:      MMGSDI_UTIL_GET_SLOT_ID_FROM_AS_ID

  DESCRIPTION:
    This function gets the Slot ID corresponding to the AS ID passed in.
  ...
  SIDE EFFECTS:
    None
=============================================================================*/
mmgsdi_return_enum_type mmgsdi_util_get_slot_id_from_as_id
(
  sys_modem_as_id_e_type          as_id,
  mmgsdi_slot_id_enum_type        *slot_id_ptr
)
{
  ...
  mmgsdi_session_id_type          session_id_gw          = MMGSDI_INVALID_SESSION_ID;
  mmgsdi_session_id_type          session_id_1x          = MMGSDI_INVALID_SESSION_ID;
  ...
  if (mmgsdi_util_get_session_type_from_as_id(as_id,
                                              &mmgsdi_gw_session_type,
                                              &mmgsdi_1x_session_type)!= MMGSDI_SUCCESS)
  {
    return MMGSDI_ERROR;
  }
  
  if (mmgsdi_util_get_mmgsdi_session_id_from_type(mmgsdi_gw_session_type,
                                                  &session_id_gw) ==  MMGSDI_SUCCESS)
  {
    if(mmgsdi_util_get_prov_session_info(session_id_gw, NULL, slot_id_ptr, NULL) == MMGSDI_SUCCESS)
    {
      return MMGSDI_SUCCESS;
    }
  }

  if (mmgsdi_util_get_mmgsdi_session_id_from_type(mmgsdi_1x_session_type,
                                                  &session_id_1x) ==  MMGSDI_SUCCESS)
  {
    if(mmgsdi_util_get_prov_session_info(session_id_1x, NULL, slot_id_ptr, NULL) == MMGSDI_SUCCESS)
    {
      return MMGSDI_SUCCESS;
    }
  }

  *slot_id_ptr = MMGSDI_MAX_SLOT_ID_ENUM;
  return MMGSDI_ERROR;
} /* mmgsdi_util_get_slot_id_from_as_id */


/* =============================================================================
  FUNCTION:      MMGSDI_UTIL_GET_AS_ID_FROM_SESSION_TYPE

  DESCRIPTION:
    This routine return as_id from session type.
  ...
  SIDE EFFECTS:
    None
=============================================================================*/
mmgsdi_return_enum_type mmgsdi_util_get_as_id_from_session_type(
  mmgsdi_session_type_enum_type  session_type,
  sys_modem_as_id_e_type        *as_id_ptr
)
{
  MMGSDIUTIL_RETURN_IF_NULL(as_id_ptr);
  ...
  switch(session_type)
  {
    ...
    case MMGSDI_GW_PROV_SEC_SESSION:
    case MMGSDI_1X_PROV_SEC_SESSION:
      *as_id_ptr = SYS_MODEM_AS_ID_2;
      break;
    case MMGSDI_GW_PROV_TER_SESSION:
    case MMGSDI_1X_PROV_TER_SESSION:
      *as_id_ptr = SYS_MODEM_AS_ID_3;
      break;
    default:
      return MMGSDI_ERROR;
  }
  ...
  return MMGSDI_SUCCESS;
} /* mmgsdi_util_get_as_id_from_session_type */
