...
$Header: //components/rel/uim.mpss/6.1.0/mmgsdi/src/mmgsdi_external_simlock.c#1 $
  ...
  const mmgsdi_session_sub_ok_req_type *sub_ok_req_ptr)
{
  ...
  if ((req_status == MMGSDI_SUCCESS) && (int_app_info_ptr != NULL))
  {
    ...
    if (int_app_info_ptr->app_state == MMGSDI_APP_STATE_WAIT_FOR_SUB_OK)
    {
      ...
      int_app_info_ptr->perso_status           = MMGSDI_PERSO_STATUS_NONE;
      ...
      if(mmgsdi_status == MMGSDI_SUCCESS)
      {
        ...
        else
        {
          ...
          int_app_info_ptr->perso_status = MMGSDI_PERSO_STATUS_GENERIC_FAILURE;
        }
