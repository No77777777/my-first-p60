...
Copyright (c) 2004 - 2005, 2008 - 2017 QUALCOMM Technologies, Inc (QTI) and its licensors.
...
$Header: //components/rel/uim.mpss/6.1.0/mmgsdi/src/mmgsdi_gen_rsp.c#2 $$ $DateTime: 2019/10/30 06:56:05 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
08/07/17   nr      Send get_response directly for 62XX and 63XX status words
...
)
{
  ...
  /* --------------------------------------------------------------------------
     ...
     ------------------------------------------------------------------------*/
  channel_id = mmgsdi_util_get_channel_id_from_cla(extra_param_ptr->send_apdu_data.cla);
  ...
  if (uim_rsp_ptr->is_uim_success)
  {
    ...
    {
      get_rsp_sw_apdu.data_len = uim_rsp_ptr->rsp_data.data_len;
      ...
      mmgsdi_memscpy(get_rsp_sw_apdu.data_ptr,
                     get_rsp_sw_apdu.data_len,
    ...
    else if ((extra_param_ptr->send_apdu_data.ins != GET_RESPONSE) &&
               ...
               (mmgsdi_util_determine_protocol(req_header.slot_id) == MMGSDI_ICC))))
    {
          ...
          MMGSDI_SEND_APDU_RETURN_IMPLICIT_STATUS_WORDS)
      {
        /* ------------------------------------------------------------------------
          ...
          --------------------------------------------------------------------- */
        get_rsp_apdu.data_len = uim_rsp_ptr->rsp_data.data_len;
  ...
  MMGSDIUTIL_TMC_MEM_FREE(get_rsp_sw_apdu.data_ptr);
  return mmgsdi_status;
} /* mmgsdi_gen_rsp_send_apdu_rsp */
