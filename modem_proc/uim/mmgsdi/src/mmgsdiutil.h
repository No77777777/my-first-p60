...
$Header: //components/rel/uim.mpss/6.1.0/mmgsdi/src/mmgsdiutil.h#4 $$ $DateTime: 2020/09/10 02:38:24 $
...
12/24/15   sp      Remove structure mmgsdi_uim_file_enum_table_type
...
/* ===========================================================================
   ...
   DESCRIPTION: Will Free the TMC Memory and peform the necessary checks for
                NULL.  It will also assign the Pointer to NULL.  It will not
                Error or print out a message if the pointer is NULL
   ===========================================================================*/
...
{                                                                      \
  if (((p_var1) == NULL) || ((p_var2) == NULL) || ((p_var3) == NULL))  \
  {                                                                    \
    return MMGSDI_INCORRECT_PARAMS;                                    \
  }                                                                    \
}
...
{                                                                                    \
  if (((p_var1) == NULL) || ((p_var2) == NULL) || ((p_var3) == NULL) ||              \
      ((p_var4) == NULL) || ((p_var5) == NULL) || ((p_var6) == NULL))                \
  {                                                                                  \
    return MMGSDI_INCORRECT_PARAMS;                                                  \
  }                                                                                  \
}
...
/* ===========================================================================
   MACRO:       MMGSDIUTIL_SLOT_INDEX_VALID
   DESCRIPTION: This macro is used to validate the slot index.
   ===========================================================================*/
#define MMGSDIUTIL_SLOT_INDEX_VALID(x)                                   \
                                       (((x) == MMGSDI_SLOT_1_INDEX) ||  \
                                        ((x) == MMGSDI_SLOT_2_INDEX) ||  \
                                        ((x) == MMGSDI_SLOT_3_INDEX))

/* ===========================================================================
   MACRO:       MMGSDIUTIL_IS_QMI_UIM_CLIENT
   DESCRIPTION: Determines whether the client registered at index in
                client_id_reg_table is QMI_UIM or not
   INPUT:       index: client id reg table index
    ===========================================================================*/
#define MMGSDIUTIL_IS_QMI_UIM_CLIENT(index)                              \
  (strncmp(QMI_UIM_CLIENT_NAME,                                          \
           mmgsdi_client_id_reg_table[index]->client_name,               \
           sizeof(mmgsdi_client_id_reg_table[index]->client_name)) == 0)

/* ===========================================================================
   MACRO:       MMGSDIUTIL_IS_MMGSDI_CLIENT
   DESCRIPTION: Determines whether the client registered at index in
                client_id_reg_table is MMGSDI or not
   INPUT:       index: client id reg table index
    ===========================================================================*/
#define MMGSDIUTIL_IS_MMGSDI_CLIENT(index)                               \
  (strncmp(MMGSDI_CLIENT_NAME,                                           \
           mmgsdi_client_id_reg_table[index]->client_name,               \
           sizeof(mmgsdi_client_id_reg_table[index]->client_name)) == 0)
...
#define MMGSDIUTIL_IS_PBM_CLIENT(index)                                  \
  ...
  /* =============================================================================
     MACRO:       MMGSDIUTIL_IS_NVRUIM_CLIENT
     DESCRIPTION: Determines whether the client registered at index in
                  client_id_reg_table is NVRUIM or not
     INPUT:       index: client id reg table index
     ===========================================================================*/
#define MMGSDIUTIL_IS_NVRUIM_CLIENT(index)                               \
  (strncmp(NVRUIM_CLIENT_NAME,                                           \
           mmgsdi_client_id_reg_table[index]->client_name,               \
           sizeof(mmgsdi_client_id_reg_table[index]->client_name)) == 0)

/* ===========================================================================
   MACRO:       MMGSDIUTIL_IS_MMGSDI_TASK
   DESCRIPTION: Tells whether the task is MMGSDI task or not
   ===========================================================================*/
#define MMGSDIUTIL_IS_MMGSDI_TASK(req_task)                        \
  (req_task >= MMGSDI_TASK_MAIN && req_task < MMGSDI_NUM_TASKS)

/* ===========================================================================
   MACRO:       MMGSDIUTIL_IS_MMGSDI_SLOT_TASK
   DESCRIPTION: Tells whether the task is MMGSDI slot task or not
   ===========================================================================*/
#define MMGSDIUTIL_IS_MMGSDI_SLOT_TASK(req_task)                        \
  (req_task >= MMGSDI_TASK_SLOT1 && req_task < MMGSDI_NUM_TASKS)

/* ===========================================================================
   MACRO:       MMGSDIUTIL_IS_DIAG_TASK
   DESCRIPTION: Tells whether the task is UIMDIAG task or not
   ===========================================================================*/
#define MMGSDIUTIL_IS_DIAG_TASK()   (UIMDIAG_TCB == rex_self())

/* ===========================================================================
   MACRO:       MMGSDIUTIL_IS_MAIN_TASK
   DESCRIPTION: Tells whether the task is MMGSDI MAIN task or not
   ===========================================================================*/
#define MMGSDIUTIL_IS_MAIN_TASK()   (UIM_GSDI_TCB == rex_self())
...
SIDE EFFECTS
  None
===========================================================================*/
void mmgsdi_util_timed_sleep(unsigned long millisec);
...
/*===========================================================================
FUNCTION:      MMGSDI_UTIL_BUILD_CLIENT_DEREG_REQ
...
/* ==========================================================================
   FUNCTION:      MMGSDI_UTIL_CONVERT_FROM_UIM_TRANSMISSION_PROTOCOL_TYPE
...
mmgsdi_return_enum_type mmgsdi_util_convert_from_uim_transport_protocol_type(
  ...
  mmgsdi_transport_protocol_enum_type * mmgsdi_transmission_mode
);

/*===========================================================================

FUNCTION MMGSDI_UTIL_MAP_MMGSDI_SESSION_TO_NVRUIM_SESSION

DESCRIPTION
  Converts the passed MMGSDI session_type to NVRUIM session_type
...
===========================================================================*/
nvruim_session_enum_type mmgsdi_util_map_mmgsdi_session_to_nvruim_session
(
  mmgsdi_session_type_enum_type session_type
);
...
/*===========================================================================
FUNCTION:      MMGSDI_UTIL_CONVERT_EFS_CONTEXT_TO_CM_SUBID

DESCRIPTION
  This function converts the EFS index to CM Sub-id
...
===========================================================================*/
sys_modem_as_id_e_type mmgsdi_util_convert_efs_context_to_cm_subid(
  mmgsdi_nv_context_type nv_efs_index
);
