...
$Header: //components/rel/uim.mpss/6.1.0/mmgsdi/src/mmgsdi_eons_nitz.c#2 $$ $DateTime: 2019/10/30 06:56:05 $
...
11/24/15   bcho    Copy full/short name params from EFS
11/06/15   bcho    Reduction in size of rplmn_info.txt
09/10/15   vv      Added csg_id check while retrieving stored rplmn names
...
/* Size of tag and length part in rplmn name TLV; to get stored in EFS*/
#define MMGSDI_EONS_NAME_TAG_SIZE            1
#define MMGSDI_EONS_NAME_LEN_SIZE            1

/* Short and long name tag value; part of rplmn name TLV to get stored in EFS*/
#define MMGSDI_EONS_LONG_NAME_TAG         0x01 
#define MMGSDI_EONS_SHORT_NAME_TAG        0x02
...
/* ----------------------------------------------------------------------------
   STRUCTURE:      MMGSDI_NITZ_EFS_RPLMN_ENTRY_TYPE

   DESCRIPTION:
     The structure represents an EFS entry in the RPLMN info array. Each entry
     contains the PLMN ID, full name params, short name params and session bit
     mask. session_bit_mask determines which subscription this entry belongs to.
-------------------------------------------------------------------------------*/
...
typedef struct
{
  ...
  uint8                              session_bit_mask;
  mmgsdi_plmn_id_type                hplmn_id;
}mmgsdi_nitz_efs_rplmn_entry_type;
...
typedef struct
{
  uint8                            start_index;
  uint8                            plmn_info_index[MMGSDI_EONS_MAX_SUBS];
  mmgsdi_nitz_efs_rplmn_entry_type nitz_rplmn_entry[MMGSDI_MAX_PLMN_NAMES];
  uint16                           plmn_names_length;
...
static mmgsdi_nitz_rplmn_info_type *nitz_rplmn_info_ptr = NULL;
...
)
{
  ...
  if(nitz_rplmn_info_ptr->nitz_rplmn_entry_ptr[rplmn_index]->full_name.full_name_length > 0)
  {
    ...
    MMGSDIUTIL_TMC_MEM_FREE(nitz_info_ptr->long_name.plmn_name.eons_data.data_ptr);
    MMGSDIUTIL_TMC_MEM_FREE(plmn_info_ptr->plmn_long_name.plmn_name.eons_data.data_ptr);
  ...
  {
    /* Ensure data_ptr is NULL, before allocating */
    MMGSDIUTIL_TMC_MEM_FREE(nitz_info_ptr->short_name.plmn_name.eons_data.data_ptr);
    MMGSDIUTIL_TMC_MEM_FREE(plmn_info_ptr->plmn_short_name.plmn_name.eons_data.data_ptr);
...
)
{
  ...
  for(rplmn_index = 0; rplmn_index < MMGSDI_MAX_PLMN_NAMES; rplmn_index++)
  {
    ...
    {
      /* In case the stored name corresponds to a femto cell,
         do not copy it to the macro cell */
      if((nitz_rplmn_info_ptr->nitz_rplmn_entry_ptr[rplmn_index]->plmn_id.csg_id != MMGSDI_EONS_INVALID_CSG_ID) &&
         (nitz_info_ptr->rplmn_id.csg_id == MMGSDI_EONS_INVALID_CSG_ID))
      {
        UIM_MSG_MED_2("Index 0x%x has name stored for CSG ID 0x%x",rplmn_index,
                      nitz_rplmn_info_ptr->nitz_rplmn_entry_ptr[rplmn_index]->plmn_id.csg_id);
        continue;
      }

      /* In case the name is being retrieved for a femto cell,
         do not copy if the csg_ids are not matched */
      if((nitz_rplmn_info_ptr->nitz_rplmn_entry_ptr[rplmn_index]->plmn_id.csg_id != MMGSDI_EONS_INVALID_CSG_ID) &&
         (nitz_info_ptr->rplmn_id.csg_id != MMGSDI_EONS_INVALID_CSG_ID))
      {
        if(nitz_rplmn_info_ptr->nitz_rplmn_entry_ptr[rplmn_index]->plmn_id.csg_id !=
           nitz_info_ptr->rplmn_id.csg_id)
        {
          continue;
        }
      }
...
/*===========================================================================
FUNCTION MMGSDI_EONS_FREE_NITZ_RPLMN_INFO

DESCRIPTION
  Function to deep free nitz_rplmn_info_ptr
...
SIDE EFFECTS
  None
===========================================================================*/
void mmgsdi_eons_free_nitz_rplmn_info
(
  void
)
{
...
} /* mmgsdi_eons_free_nitz_rplmn_info */
...
)
{
  ...
  uint8                             rplmn_index             = 0;
  uint16                            dynamic_length          = 0;
  uint16                            dynamic_index           = 0;
  uint8                           * dynamic_data_ptr        = NULL;
  boolean                           is_name_present         = FALSE;

  if(MMGSDI_SUCCESS !=
         gsdi_fs_get_file_size((const char *)mmgsdi_efs_eons_rplmn_file,
  ...
  if(sizeof(mmgsdi_nitz_efs_rplmn_info_type) +
     nitz_efs_rplmn_info_ptr->plmn_names_length != rplmn_info_read_size)
  {
    MMGSDIUTIL_TMC_MEM_FREE(nitz_efs_rplmn_info_ptr);
    return MMGSDI_ERROR;
  }
  ...
  dynamic_length = nitz_efs_rplmn_info_ptr->plmn_names_length;
  dynamic_data_ptr = (uint8*)nitz_efs_rplmn_info_ptr + sizeof(mmgsdi_nitz_efs_rplmn_info_type);
  ...
  for(rplmn_index = 0; rplmn_index < MMGSDI_MAX_PLMN_NAMES; rplmn_index++)
  {
    if((nitz_efs_rplmn_info_ptr->nitz_rplmn_entry[rplmn_index].plmn_id.plmn_id_val[0] == 0xFF) &&
       (nitz_efs_rplmn_info_ptr->nitz_rplmn_entry[rplmn_index].plmn_id.plmn_id_val[1] == 0xFF) &&
       (nitz_efs_rplmn_info_ptr->nitz_rplmn_entry[rplmn_index].plmn_id.plmn_id_val[2] == 0xFF))
    {
      /* All bytes of PLMN ID at this index are 0xFF, means this and reamining
         entries are not valid. break the loop */
      break;
    }
    ...
    nitz_rplmn_info_ptr->nitz_rplmn_entry_ptr[rplmn_index]->plmn_id =
      nitz_efs_rplmn_info_ptr->nitz_rplmn_entry[rplmn_index].plmn_id;

    nitz_rplmn_info_ptr->nitz_rplmn_entry_ptr[rplmn_index]->lac =
      nitz_efs_rplmn_info_ptr->nitz_rplmn_entry[rplmn_index].lac;

    nitz_rplmn_info_ptr->nitz_rplmn_entry_ptr[rplmn_index]->full_name.full_name_params =
      nitz_efs_rplmn_info_ptr->nitz_rplmn_entry[rplmn_index].full_name_params;

    nitz_rplmn_info_ptr->nitz_rplmn_entry_ptr[rplmn_index]->short_name.short_name_params =
      nitz_efs_rplmn_info_ptr->nitz_rplmn_entry[rplmn_index].short_name_params;

    nitz_rplmn_info_ptr->nitz_rplmn_entry_ptr[rplmn_index]->session_bit_mask =
      nitz_efs_rplmn_info_ptr->nitz_rplmn_entry[rplmn_index].session_bit_mask;

    nitz_rplmn_info_ptr->nitz_rplmn_entry_ptr[rplmn_index]->hplmn_id =
      nitz_efs_rplmn_info_ptr->nitz_rplmn_entry[rplmn_index].hplmn_id;

    is_name_present = FALSE;

    /* Check if we have enough bytes in dynamic part of buffer */
    if(dynamic_index + MMGSDI_EONS_NAME_TAG_SIZE + MMGSDI_EONS_NAME_LEN_SIZE >
       dynamic_length)
    {
      mmgsdi_status = MMGSDI_ERROR;
      break;
    ...
    if(dynamic_data_ptr[dynamic_index] != MMGSDI_EONS_LONG_NAME_TAG)
    {
      mmgsdi_status = MMGSDI_ERROR;
      break;
    }

    dynamic_index = dynamic_index + MMGSDI_EONS_NAME_TAG_SIZE;
    nitz_rplmn_info_ptr->nitz_rplmn_entry_ptr[rplmn_index]->full_name.full_name_length =
      dynamic_data_ptr[dynamic_index];
    dynamic_index = dynamic_index + MMGSDI_EONS_NAME_LEN_SIZE;
    ...
    if(nitz_rplmn_info_ptr->nitz_rplmn_entry_ptr[rplmn_index]->full_name.full_name_length)
    {
      mmgsdi_memscpy(nitz_rplmn_info_ptr->nitz_rplmn_entry_ptr[rplmn_index]->full_name.full_name_chars,
                     MMGSDI_EONS_NITZ_PLMN_FULL_NAME_LENGTH,
                     &dynamic_data_ptr[dynamic_index],
    ...
    if(dynamic_data_ptr[dynamic_index] != MMGSDI_EONS_SHORT_NAME_TAG)
    {
      mmgsdi_status = MMGSDI_ERROR;
      break;
    }
    ...
    if(nitz_rplmn_info_ptr->nitz_rplmn_entry_ptr[rplmn_index]->short_name.short_name_length)
    {
      mmgsdi_memscpy(nitz_rplmn_info_ptr->nitz_rplmn_entry_ptr[rplmn_index]->short_name.short_name_chars,
                     MMGSDI_EONS_NITZ_PLMN_FULL_NAME_LENGTH,
                     &dynamic_data_ptr[dynamic_index],
    ...
    /* Either full name or short name should be present */
    if(is_name_present == FALSE)
    {
      mmgsdi_status = MMGSDI_ERROR;
      break;
    }
...
SIDE EFFECTS
  None
==========================================================================*/
mmgsdi_return_enum_type mmgsdi_eons_store_and_free_nitz_info (
  void
)
{
  mmgsdi_nitz_efs_rplmn_info_type * nitz_efs_rplmn_info_ptr      = NULL;
  ...
  /* Check if file to store NITZ info exists, or create it with the required
     path */
  ...
  if(is_rplmn_info_file_available == FALSE)
  {
    mmgsdi_eons_free_nitz_rplmn_info();
    return MMGSDI_ERROR;
  }

  /* Calculate the total dynamic length required to store name TLVs */
  while((rplmn_index < MMGSDI_MAX_PLMN_NAMES) &&
        (nitz_rplmn_info_ptr->nitz_rplmn_entry_ptr[rplmn_index] != NULL))
  {
    uint16 total_name_len = 0;
    
    total_name_len = nitz_rplmn_info_ptr->nitz_rplmn_entry_ptr[rplmn_index]->full_name.full_name_length +
                     nitz_rplmn_info_ptr->nitz_rplmn_entry_ptr[rplmn_index]->short_name.short_name_length;
    
    /* Return ERROR if both names are not present. */
    if (total_name_len == 0)
    {
      mmgsdi_eons_free_nitz_rplmn_info();
      return MMGSDI_ERROR;
    ...
    /* Add length for long name/short name's TLV and length bytes and name length */
    dynamic_length = dynamic_length + total_name_len +
                     MMGSDI_EONS_NAME_TAG_SIZE + MMGSDI_EONS_NAME_LEN_SIZE +
                     MMGSDI_EONS_NAME_TAG_SIZE + MMGSDI_EONS_NAME_LEN_SIZE;
    rplmn_index++;
  }
  ...
  rplmn_info_write_size = sizeof(mmgsdi_nitz_efs_rplmn_info_type) + dynamic_length;
  ...
  memset(nitz_efs_rplmn_info_ptr, 0xFF, rplmn_info_write_size);

  dynamic_data_ptr = (uint8*)nitz_efs_rplmn_info_ptr + sizeof(mmgsdi_nitz_efs_rplmn_info_type);

  /* Copy variable length */
  nitz_efs_rplmn_info_ptr->plmn_names_length = dynamic_length;
        ...
        (nitz_rplmn_info_ptr->nitz_rplmn_entry_ptr[rplmn_index] != NULL))
  {
    ...
    ASSERT(dynamic_index + MMGSDI_EONS_NAME_TAG_SIZE + MMGSDI_EONS_NAME_LEN_SIZE +
           nitz_rplmn_info_ptr->nitz_rplmn_entry_ptr[rplmn_index]->full_name.full_name_length <=
           dynamic_length);

    dynamic_data_ptr[dynamic_index] = MMGSDI_EONS_LONG_NAME_TAG;
    dynamic_index = dynamic_index + MMGSDI_EONS_NAME_TAG_SIZE;
    
    dynamic_data_ptr[dynamic_index] = nitz_rplmn_info_ptr->nitz_rplmn_entry_ptr[rplmn_index]->full_name.full_name_length;
    dynamic_index = dynamic_index + MMGSDI_EONS_NAME_LEN_SIZE;

    if(nitz_rplmn_info_ptr->nitz_rplmn_entry_ptr[rplmn_index]->full_name.full_name_length != 0)
    {
      mmgsdi_memscpy(&dynamic_data_ptr[dynamic_index],
                     dynamic_length - dynamic_index,
                     nitz_rplmn_info_ptr->nitz_rplmn_entry_ptr[rplmn_index]->full_name.full_name_chars,
                     nitz_rplmn_info_ptr->nitz_rplmn_entry_ptr[rplmn_index]->full_name.full_name_length);
      dynamic_index = dynamic_index +
                       nitz_rplmn_info_ptr->nitz_rplmn_entry_ptr[rplmn_index]->full_name.full_name_length;
    ...
    dynamic_data_ptr[dynamic_index] = MMGSDI_EONS_SHORT_NAME_TAG;
    dynamic_index = dynamic_index + MMGSDI_EONS_NAME_TAG_SIZE;
    
    dynamic_data_ptr[dynamic_index] = nitz_rplmn_info_ptr->nitz_rplmn_entry_ptr[rplmn_index]->short_name.short_name_length;
    dynamic_index = dynamic_index + MMGSDI_EONS_NAME_LEN_SIZE;
    ...
    MMGSDIUTIL_TMC_MEM_FREE(nitz_rplmn_info_ptr->nitz_rplmn_entry_ptr[rplmn_index]);
    rplmn_index++;
  }

  ASSERT(sizeof(mmgsdi_nitz_efs_rplmn_info_type) + dynamic_index ==
         rplmn_info_write_size);
  ...
  return mmgsdi_status;
}/* mmgsdi_eons_store_and_free_nitz_info */
