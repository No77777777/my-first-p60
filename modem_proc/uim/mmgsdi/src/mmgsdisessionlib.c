...
$Header: //components/rel/uim.mpss/6.1.0/mmgsdi/src/mmgsdisessionlib.c#2 $$ $DateTime: 2019/10/30 06:56:05 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     -----------------------------------------------------------
05/07/18   rps     Retrieving attribute cache from caller context
12/27/17   ra      Disable GSM authentication security restrictions for test card
06/13/17   sk      Removed F3 from mmgsdi_session_get_info_sync()
...
04/14/16   bcho    Code optimization for READ, WRITE and SERVICE enable/disable APIs
...
04/12/06   nr      IoE specific changes
...
08/31/15   ar      Remove trace for error while retreiving cliet_id
...
12/03/14   ar      Allow cache access using card_session in client context
...
{
  return mmgsdi_session_queue_close_req_to_task(session_id,
                                                terminate,
                                                MMGSDI_TASK_MAIN,
                                                response_cb_ptr,
                                                client_ref);
...
PARAMETERS
  ...
  boolean                    is_auto_activation: MMGSDI auto-activate or
                                                 client request
...
)
{
  ...
  mmgsdi_memscpy(msg_ptr->app_id_data.data_ptr,
                 ...
                 app_id_data.data_len);

  /* Populated whether the request is auto_activation or client requested */
  msg_ptr->is_auto_activation               = is_auto_activation;
...
)
{
  ...
  do
  {
    ...
    /* Also fetch the slot type*/
    slot_data_ptr = mmgsdi_util_get_slot_data_ptr(session_get_info_ptr->slot_id);
    if (slot_data_ptr != NULL)
    {
      session_get_info_ptr->slot_type = slot_data_ptr->slot_type;;
    }
  }while(0);
...
)
{
  ...
  if (file_type == MMGSDI_TRANSPARENT_FILE &&
      (!mmgsdi_file_transparent_is_access_range_valid(offset, request_length)))
  {
    return MMGSDI_INCORRECT_PARAMS;
  }
...
)
{
  ...
  if (file_type == MMGSDI_TRANSPARENT_FILE &&
      (write_data.data_len == 0 ||
       (!mmgsdi_file_transparent_is_access_range_valid(offset, write_data.data_len))))
  {
    return MMGSDI_INCORRECT_PARAMS;
  }
  ...
  if(mmgsdi_nv_get_device_feature_status(MMGSDI_FEATURE_BLOCK_WRITES_TO_REFRESH_FILES) ==
       MMGSDI_FEATURE_ENABLED &&
     mmgsdi_is_file_being_refreshed(session_id, file_access))
  {
...
)
{
  ...
  /* Try and get the item from cache first. If file attributes are not present in cache,
     we will queue the request to MMGSDI */
  mmgsdi_status = mmgsdi_caller_context_get_file_attr_cache_and_notify_client(task_cmd_ptr);

  if(mmgsdi_status == MMGSDI_SUCCESS)
  {
    /* File attributes are present in attribute cache.
       Free the command and return the status */
    MMGSDIUTIL_TMC_MEM_FREE(task_cmd_ptr);
    return mmgsdi_status;
  }
...
)
{
  ...
  /* Item was either read successfully from the cache or the cache is not
     init as of now. If cache is not init, change status to MMGSDI_ERROR.
     Free the command and return the status */
  if(mmgsdi_status == MMGSDI_SUCCESS)
  {
    MMGSDIUTIL_TMC_MEM_FREE(task_cmd_ptr);
...
)
{
  ...
  return MMGSDI_NOT_SUPPORTED;
}/* mmgsdi_session_jcdma_get_card_info */
...
)
{
  return mmgsdi_session_register_for_refresh_ok_to_proceed_voting(
           ...
           response_cb_ptr,
           client_ref);
}/*mmgsdi_session_register_for_refresh*/
...
)
{
  ...
  MMGSDIUTIL_TMC_MEM_FREE(
   ...
   return mmgsdi_status;
}/*mmgsdi_session_register_for_refresh_ok_to_proceed_voting*/
...
)
{
  ...
  return mmgsdi_status;
}/*mmgsdi_session_deregister_for_refresh*/
...
)
{
  ...
  /* Retrieve internal MMGSDI session id for the session type */
  ...
  return mmgsdi_status;
} /* mmgsdi_session_read_cache_file_size_by_session_type */
...
)
{
  ...
  if(read_data_ptr->data_len <= 0 ||
     ...
     rec_num > MMGSDI_MAX_REC_NUM)
  {
    UIM_MSG_ERR_2("Invalid Read Data Len: 0x%x or record num: 0x%x",
  ...
  do
  {
    ...
    if (mmgsdi_util_is_prov_session(session_type))
    {
      ...
      if((mmgsdi_status != MMGSDI_SUCCESS) || (prov_app_info_ptr == NULL))
      {
        mmgsdi_status = MMGSDI_ERROR;
        break;
      }
...
)
{
  ...
  mmgsdi_status = mmgsdi_session_read_cache_ext(session_id,
                                                  ...
                                                  rec_num);
  return mmgsdi_status;
} /* mmgsdi_session_read_cache_by_session_type */
...
)
{
  ...
  if(mmgsdi_nv_get_device_feature_status(MMGSDI_FEATURE_BLOCK_WRITES_TO_REFRESH_FILES)
       == MMGSDI_FEATURE_ENABLED &&
     mmgsdi_is_file_being_refreshed(session_id, access))
  {
    UIM_MSG_ERR_0("Request temporarily not allowed as requested file is being refreshed");
    return MMGSDI_ERROR;
  }
  ...
  do
  {
    ...
    /* check if write data is same as cache */
    if(mmgsdi_cache_is_write_data_same_as_cache(session_id,
                                                &access,
                                                write_data,
                                                offset,
                                                1))
    {
      /* no need to build cnf and queue as the callback will be discarded. */
...
)
{
  ...
  UIM_MSG_HIGH_2("Queue of MMGSDI command: MMGSDI_SESSION_GET_APP_CAPABILITIES_REQ status 0x%x, client ref: 0x%x",
                 mmgsdi_status, client_ref);
...
)
{
  ...
  else if((app_info.app_id_data.data_len > MMGSDI_MAX_APP_ID_LEN) ||
...
)
{
  ...
  MMGSDIUTIL_MEM_MALLOC_AND_VALIDATE(task_cmd_ptr,
     sizeof(mmgsdi_task_cmd_type) + temp_mem + sizeof(q_link_type));
...
/* ==========================================================================
FUNCTION        MMGSDI_SESSION_PURGE_TEMPORARY_IDENTITY

DESCRIPTION
  This function will purge the temorary identity stored in UICC and non-volatile file system.
...
SIDE EFFECTS
  None.
========================================================================== */
mmgsdi_return_enum_type  mmgsdi_session_purge_temporary_identity (
  mmgsdi_session_id_type                session_id,
...
)
{
  ...
  /*---------------------------------------------------------------------------
    ...
    1) Session ID
    2) Response CB function Check
    3) Check temporary_identity value
  ---------------------------------------------------------------------------*/
  mmgsdi_status = mmgsdi_util_is_session_id_valid_in_cmd(
                    session_id,
  ...
  msg_ptr->request_header.session_id        = session_id;
  msg_ptr->request_header.request_type      = MMGSDI_PURGE_TEMPORARY_IDENTITY_REQ;
  msg_ptr->request_header.orig_request_type = MMGSDI_PURGE_TEMPORARY_IDENTITY_REQ;
  msg_ptr->request_header.client_data       = client_ref;
  msg_ptr->request_header.response_cb       = response_cb_ptr;
  ...
  ---------------------------------------------------------------------------*/
  msg_ptr->temporary_identity_req_state = MMGSDI_PURGE_TEMPORARY_IDENTITY_SOURCE_REQ;

  /*---------------------------------------------------------------------------
    Populating mmgsdi_purge_temporary_identity_req_type content
  ---------------------------------------------------------------------------*/
  msg_ptr->temporary_identity = temporary_identity;
  ...
  UIM_MSG_HIGH_1("Queue of MMGSDI command: MMGSDI_PURGE_TEMPORARY_IDENTITY_REQ status 0x%x",
                 mmgsdi_status);

  return mmgsdi_status;
} /* mmgsdi_session_purge_temporary_identity */
...
PARAMETERS:
  session_id:    Session ID of the caller.
  mcc_value_ptr: Buffer to store MCC data.
  mnc_value_ptr: Buffer to store MNC data.
...
/* ===========================================================================
FUNCTION     MMGSDI_SESSION_IS_TEST_CARD_USED

DESCRIPTION
  This function reads the IMSI data based on session id and compares the MCC value in IMSI
  against set of reserved MCC values to figure out if it is a test IMSI.
  The content in the passed parameter is only modified when the return value is MMGSDI_SUCCESS.

PARAMETERS
  @param[in]   session_id       :  Session ID of the caller.
  @param[out]  is_test_card_ptr :  Indicates whether test card is used

DEPENDENCIES
  None

RETURN VALUE
  MMGSDI_SUCCESS         :   Successful determination of whether test card is used
  MMGSDI_INCORRECT_PARAMS:   The parameters supplied to the API are
                             not within appropriate ranges.
...
SIDE EFFECTS
  None
===========================================================================*/
mmgsdi_return_enum_type mmgsdi_session_is_test_card_used
(
  mmgsdi_session_id_type  session_id,
  boolean                *is_test_card_ptr
)
{
  mmgsdi_return_enum_type            mmgsdi_status                = MMGSDI_SUCCESS;
  mmgsdi_data_type                   imsi_data                    = {0, NULL};
  ...
  MMGSDIUTIL_RETURN_IF_NULL(is_test_card_ptr);
  ...
  if(mmgsdi_status != MMGSDI_SUCCESS)
  {
    UIM_MSG_ERR_1("Could not read IMSI file, status = 0x%x", mmgsdi_status);
    return mmgsdi_status;
  }

  mmgsdi_status = mmgsdi_util_convert_to_imsi_data_type(&imsi_data, &imsi);
  if(mmgsdi_status != MMGSDI_SUCCESS)
  {
    return mmgsdi_status;
  }

  *is_test_card_ptr = mmgsdi_card_init_is_imsi_test_imsi(imsi);

  return MMGSDI_SUCCESS;
}/* mmgsdi_session_is_test_card_used */
