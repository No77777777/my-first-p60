...
$Header: //components/rel/uim.mpss/6.1.0/mmgsdi/src/mmgsdi.h#12 $$ $DateTime: 2024/02/26 16:03:48 $
...
09/09/21   cj      Fix IMSI read failure issue due to incorrect IMSI FCP data
...
02/28/20   sch     Remove unused perso_ability in mmgsdi_int_app_info_type
...
04/14/17   nc      Expose public API that returns current CSIM hiding status  
...
10/25/16   me      Removed feature flag FEATURE_MMGSDI_3GPP2
...
03/10/16   nr      Adding ONCHIP support for KCGPRS files
...
10/21/15   av      Don't allow reSELECT on AID that failed at least once earlier
...
08/25/15   gm      Update the refresh file list
...
12/16/14   ar      Refresh timer increased to 150 for PS_DETACH & STOP_MODE req
12/10/14   vdc     Added support for handling multiple profiles enabled cards
11/20/14   nr      Remove mmgsdi_is_perso_completed () API
...
extern rex_tcb_type              * gsdi_main_tcb_ptr;
extern rex_tcb_type              * gsdi_slot_1_tcb_ptr;
extern rex_tcb_type              * gsdi_slot_2_tcb_ptr;
extern rex_tcb_type              * gsdi_slot_3_tcb_ptr;
extern rex_tcb_type              * uimdiag_tcb_ptr;

#define UIM_GSDI_TCB               gsdi_main_tcb_ptr
#define UIM_GSDI_SLOT1_TCB         gsdi_slot_1_tcb_ptr
#define UIM_GSDI_SLOT2_TCB         gsdi_slot_2_tcb_ptr
#define UIM_GSDI_SLOT3_TCB         gsdi_slot_3_tcb_ptr
#define UIMDIAG_TCB                uimdiag_tcb_ptr
...
#define MMGSDI_SYNC_CMD_CANCEL_SIG            0x00000100 /* Set by main task on slot task
                                                            for cancelling sync cmd */
...
#define MMGSDI_ECALL_STATUS_SIG               0x00100000
#define MMGSDI_SYNC_CMD_CANCEL_ACK_SIG        0x00200000 /* Set by slot task on main task
        after canceling sync cmd. Main task also sets it for slot specific task to
        acknowledge back */
#define MMGSDI_CLASH_WITH_ONCRPC_REPLY_SIG    0x00400000 /* RESERVED SIGNAL */
#define MMGSDI_MAIN_TASK_READY_SIG            0x00800000 /* Main task sets this to unblock
                                                            slot tasks when it is ready */
#define MMGSDI_GSTK_IMEI_SENT_SIG             0x01000000 /* GSTK sets this signal to
                                                            indicate IMEI has sent to card */
#define MMGSDI_SLOT_MAPPING_SIG               0x02000000
#define SLOT_MAPPING_RETRY_SIG                0x04000000
...
#define MMGSDI_SYNC_WAIT_SIG                  0x80000000 /* Signal to sync wait for
                                                            simlock cb in PSM mode */
...
/* (Number of slots in use * (Number of max commands that can be cancelled at a time.
   We don't cancel commands from main task + Number of commands that can be in progress in each slot task) +
   Number of commands that can be in progress in main task */
#define MMGSDI_MAX_CLIENT_REQ_INFO         (2 * (MMGSDI_HIGH_PRI_CMD_QUEUE_MAX_SIZE + MMGSDI_CMD_QUEUE_MAX_SIZE + 4) + 4)
...
#define MMGSDI_PRE_ISSUING_DATA_MASK 0x60
#define MMGSDI_CARD_CAPABILITIES     0x73
#define MMGSDI_STATUS_INDICATOR_MASK 0x80
#define MMGSDI_LOGICAL_CHAN_MAX      0x08
#define MMGSDI_CARD_ISSUER_DATA      0x50
...
#define MMGSDI_SESSION_DEACTIVATION_PAUSE_TIMER 200 /* Default duration for pausing
                                                       session deactivation */

#define MMGSDI_GSTK_IMEI_TIMER_VALUE         2000 /* In case of USAT pairing, 2
                                                     seconds timer to wait for
                                                     IMEI sent to card */
...
#define GSDI_FILE_DESCRIPTOR_BUGGY_TAG   0x86
...
#define GSDI_LIFE_CYCLE_INTEG_BUGGY_TAG  0x03
...
#define GSDI_FILE_SIZE_BUGGY_TAG         0xE4
...
#define GSDI_IMSI_PATH_USED              0x6F07
...
/* ----------------------------------------------------------------------------
** MAX Number of extended FPLMNs supported by MMGSDI cache is 20 for both UICC & ICC cards
** ---------------------------------------------------------------------------*/
#define MMGSDI_MAX_EXTENDED_FPLMN_COUNT  20
#define MMGSDI_EXTENDED_FPLMN_CACHE_LEN  (MMGSDI_MAX_EXTENDED_FPLMN_COUNT * MMGSDI_PLMN_ID_SIZE)
...
/* Name of MMGSDI client (to be added to client id reg table) */
#define MMGSDI_CLIENT_NAME                  "mmgsdi"

/* Max. Card Refresh failure count */
#define MMGSDI_MAX_CARD_REFRESH_FAILURE_COUNT 3
...
/* ----------------------------------------------------------------------------
   ...
   DESCRIPTION:
     Tags that are allowed when activating USIM or SIM onchip.
     When adding any tag pertaining to an EF to this enum, add it to the
     table mmgsdi_common_onchip_tag_to_ef_table as well.
-------------------------------------------------------------------------------*/
typedef enum
{
  ...
  MMGSDI_ACT_ONCHIP_MAX_ENUM_TAG
...
/* ----------------------------------------------------------------------------
   ENUM:      MMGSDI_UIM_EF_ITEM_CATEGORY_ENUM_TYPE

   DESCRIPTION:
     This enum type denotes the category which EF item belongs to
-------------------------------------------------------------------------------*/
typedef enum {
  MMGSDI_EF_CATRGORY_ROOT     = MMGSDI_EF_UNDER_MF_START,
  MMGSDI_EF_CATRGORY_CDMA     = MMGSDI_RUIM_EF_START,
  MMGSDI_EF_CATRGORY_GSM      = MMGSDI_GSM_EF_START,
  MMGSDI_EF_CATRGORY_TELECOM  = MMGSDI_TELECOM_EF_START,
  MMGSDI_EF_CATRGORY_USIM     = MMGSDI_USIM_EF_START,
  MMGSDI_EF_CATRGORY_DCS1800  = MMGSDI_DCS1800_EF_START,
  MMGSDI_EF_CATRGORY_HZI      = MMGSDI_HZI_EF_START,
  MMGSDI_EF_CATRGORY_BY_PATH  = MMGSDI_EF_PATH_START,
  MMGSDI_EF_CATRGORY_CSIM     = MMGSDI_CSIM_EF_START,
  MMGSDI_EF_CATRGORY_ISIM     = MMGSDI_ISIM_EF_START
...
/* ----------------------------------------------------------------------------
   ...
   DESCRIPTION:
     ...
     Note that SESSION_TYPE works only for prov session types.
-------------------------------------------------------------------------------*/
typedef enum {
  MMGSDI_CANCEL_USING_CLIENT_ID,
  MMGSDI_CANCEL_USING_SESSION_ID,
  MMGSDI_CANCEL_USING_SESSION_TYPE,
  MMGSDI_CANCEL_ALL
...
/* ----------------------------------------------------------------------------
   ...
   ENUM:      MMGSDI_PURGE_TEMPORARY_IDENTITY_STATE_ENUM_TYPE

   DESCRIPTION:
     This enum indicate the source of purge request
-------------------------------------------------------------------------------*/
typedef enum {
  MMGSDI_PURGE_TEMPORARY_IDENTITY_SOURCE_REQ,                   /**< Purge request from AP via QMI. */
  MMGSDI_PURGE_TEMPORARY_IDENTITY_SOURCE_CLIENT_RSP,            /**< Purge confirmation from NAS. */
}mmgsdi_purge_temporary_identity_state_enum_type;
...
/* ----------------------------------------------------------------------------
   ENUM:      MMGSDI_SYNC_ENUM_TYPE

   DESCRIPTION:
     This enum indicates how the request that needs interaction with card
     should be processed.
     MMGSDI_ASYNC_PROCESS   :   Request is queued to driver and response 
                                from driver is processed asynchronoulsy
     MMGSDI_SYNC_PROCESS    :   Request is queued to driver and response 
                                from driver is processed synchronoulsy
     MMGSDI_SYNC_FS_PROCESS :   Request is not queued to driver, data is
                                directly fetched from the file system.
-------------------------------------------------------------------------------*/
typedef enum {
  MMGSDI_ASYNC_PROCESS = 0,
  MMGSDI_SYNC_PROCESS,
  MMGSDI_SYNC_FS_PROCESS
} mmgsdi_sync_enum_type;
...
typedef struct {
  ...
  union {
    ...
    mmgsdi_record_cache_node_list_type  cache_node;
  }file_cache;
...
/* ----------------------------------------------------------------------------
  STRUCTURE:      MMGSDI_DECODED_ATTR_CACHE_TYPE

  DESCRIPTION:
    The structure contains the decoded file attribute information
    to be used for the Clients.

    access:                  File path of the file
    file_type:               Stores file structure
    file_status:             Status of the file
    cyclic_increase_allowed: cyclic_increase_allowed
    file_size:               File size of the file
    file_id:                 ID used in the SIM/USIM/RUIM/CSIM
    num_of_rec:              No.of records present in the file
    rec_len:                 Record length
    sfi:                     Short File Indicator for the file,if supported
-------------------------------------------------------------------------------*/
typedef struct {
  ...
  uint8                              sfi;
}mmgsdi_decoded_attr_cache_type;

/* ----------------------------------------------------------------------------
  STRUCTURE:      MMGSDI_ATTR_CACHE_ELEMENT_TYPE

  DESCRIPTION:
   The structure contains the raw file attribute data to be used for attr
   cache buidling.

   is_arr_skipped:          Contains if cached security attributes is default or
                            decoded attributes
   arr_file_id:             Contains ARR file for fetching security attributes
   arr_record_no:           Record no in ARR file
   access:                  File access information
   security_access:         Security related parameters of a file
   raw_file_attr_data:      Raw file attribute data returned by the card
   next_ptr:                Pointer to next attribute cache node
-------------------------------------------------------------------------------*/
...
/* ----------------------------------------------------------------------------
   STRUCTURE:      MMGSDI_ONCHIP_CONFIG_TLV_TYPE

   DESCRIPTION:
     The structure contains the pointer and length for one TLV of the ONCHIP configuration
     Only useful when active.
-------------------------------------------------------------------------------*/
...
typedef struct {
  ...
  /* onchip_config_data temporarily holds the config data, once the data is populated,
     the allocated memory is freed */
  mmgsdi_onchip_sim_config_data_type       onchip_config_data;
  mmgsdi_onchip_config_tlv_type            onchip_tlv[MMGSDI_ACT_ONCHIP_MAX_ENUM_TAG];
...
/* ----------------------------------------------------------------------------
   ...
   DESCRIPTION:
     ...
     perso_status:                      Indicates the personalization status.
                                        When perso status becomes MMGSDI_PERSO_STATUS_DONE app
                                        state would also change to MMGSDI_APP_STATE_PERSO_VERIFIED
     ...
     pending_purge_guti:                Indicate if purging of guti is pending
     srv_table_orig_cache:              Caches the original value of service table from the card
                                        prior to any modification by MMGSDI
     fplmn_actual_size_on_card          This field will be updated with the actual FPLMN size
                                        on the card
     nv_refresh_ref_count:              Reference count for NV refresh
-------------------------------------------------------------------------------*/
typedef struct{
  ...
  mmgsdi_prov_apps_cache_info_type           app_cache;
  mmgsdi_cache_record_ef_data_type           opl_cache;
  mmgsdi_cache_record_ef_data_type           pnn_cache;
  mmgsdi_cphs_int_info_type                 *cphs_info_ptr;
  mmgsdi_attr_cache_type                     attr_cache_tbl;
  boolean                                    pending_purge_guti;
  mmgsdi_data_type                           srv_table_orig_cache;
  boolean                                    first_loci_write_after_activation;
  mmgsdi_len_type                            fplmn_actual_size_on_card;
  uint32                                     nv_refresh_ref_count;
...
/* ----------------------------------------------------------------------------
   STRUCTURE:      MMGSDI_EID_INFO_TYPE

   DESCRIPTION:
     This structure contains the EID info as received from uimdrv.
     length:    Number of EID bytes
     data:      EID
-------------------------------------------------------------------------------*/
typedef struct
{
  uint8 length;
  uint8 data[MMGSDI_EID_LEN];
} mmgsdi_eid_info_type;
...
/* ----------------------------------------------------------------------------
   ...
   DESCRIPTION:
     ...
     sim_detection_started:
                          Indicates if uimdrv has started an attempt to detect the card but
                          hasn't yet sent the link_est or card_error for the slot.
     nv_refresh_reference_count:
                          Reference count used for synchornizing NV Refresh trigger
     csim_hiding_status:  Indicates if the CSIM is hidden or not
     slot_type:           Indicates type of card for this slot - remote or local
     imei_sent_to_card:   Indicates IMEI has sent to card in the TR for PLI-IMEI
                          proactive command
     ...
     card_type:           Indicates type of card - telecom, non_telecom
     transmission_mode:   Card transport protocol - T = 0, T = 1
...
typedef struct {
  ...
  mmgsdi_csim_hiding_status_enum_type   csim_hiding_status;
  mmgsdi_slot_enum_type                 slot_type;
  boolean                               imei_sent_to_card;
  boolean                               aram_aid_select_failed;
  mmgsdi_card_type_enum_type            card_type;
  mmgsdi_transport_protocol_enum_type   transmission_mode;
  boolean                               is_euicc;
  boolean                               is_eid_in_process;
  uint32                                refresh_failure_count;
  mmgsdi_eid_info_type                  eid_info;
  uint8                                 nv_ref_app_deact_count;
} mmgsdi_slot_data_type;

/* ----------------------------------------------------------------------------
   STRUCTURE:      MMGSDI_CM_CALL_INFO_TYPE

   DESCRIPTION:
     The structure contains all call related information from CM
-------------------------------------------------------------------------------*/
typedef struct {
  boolean                                 voice_call_active;
  boolean                                 data_call_active;
  boolean                                 emergency_call_active;
  cm_call_id_type                         event_call_id;
  cm_call_type_e_type                     event_call_type;
} mmgsdi_cm_call_info_type;
...
typedef struct {
  ...
  /* CM call related info */
  mmgsdi_cm_call_info_type                 cm_call_info[MMGSDI_MAX_PROV_APPS];
...
/* ----------------------------------------------------------------------------
   ...
   DESCRIPTION:
     ...
     iccid:             ICCID
     card_type:         Indicates type of card - telecom, non_telecom
     transmission_mode: Card transport protocol - T = 0 and T = 1
...
/* ----------------------------------------------------------------------------
   ...
   DESCRIPTION:
     ...
     timestamp:      Time stamp of the request when queued to MMGSDI
...
/* ----------------------------------------------------------------------------
   STRUCTURE:      MMGSDI_SET_APDU_BEHAVIOR_REQ_TYPE

   DESCRIPTION:
     This structure is used when generating a set APDU behavior request
     for the MMGSDI Task.
------------------------------------------------------------------------------*/
typedef struct {
  mmgsdi_request_header_type         request_header;
...
} mmgsdi_set_apdu_behavior_req_type;

/* ----------------------------------------------------------------------------
   STRUCTURE:      MMGSDI_M2M_GET_EID_REQ_TYPE

   DESCRIPTION:
     This structure is used when generating a GET_EID request
     for the MMGSDI Task.
------------------------------------------------------------------------------*/
typedef struct {
  mmgsdi_request_header_type         request_header;
} mmgsdi_m2m_get_eid_req_type;
...
typedef struct {
  ...
  boolean                     cmd_transacted;  /* whether the cmd was sent to
                                                  the card or not              */
  uint32                      rsp_data_index;  /* index into
...
/* ===========================================================================
   STRUCTURE:      MMGSDI_SIMLOCK_SESSION_ID_TOKEN_TYPE

   DESCRIPTION: This structure contains session id and token for which SIMLOCK
   run algo request is sent.
===========================================================================*/
typedef struct {
  mmgsdi_session_id_type   session_id;
  uint32                   token;
} mmgsdi_simlock_session_id_token_type;

/* ===========================================================================
   STRUCTURE:      MMGSDI_SIMLOCK_SESSION_ID_TABLE_TYPE

   DESCRIPTION:
     This structure contains list of session id and token to be sent as user
     data with SIMLOCK run algo request. In response, this data is used to check
     whether sessions are still active before using corresponding subscription
     status.
===========================================================================*/
typedef struct {
  mmgsdi_simlock_session_id_token_type   session_id_token[SIMLOCK_SLOT_COUNT_MAX][SIMLOCK_SUBS_COUNT_MAX];
...
/* ----------------------------------------------------------------------------
   ...
   DESCRIPTION:
     ...
     pup_option         : pup option from Card pup API
  -------------------------------------------------------------------------------*/
typedef struct{
...
/* ----------------------------------------------------------------------------
   STRUCTURE:      MMGSDI_NOTIFY_RECOVERY_START_REQ_TYPE

   DESCRIPTION:
     This structure is used when notifying to MMGSDI task that recovery has
     started on a slot.
------------------------------------------------------------------------------*/
typedef struct {
  mmgsdi_request_header_type  request_header;
} mmgsdi_notify_recovery_start_req_type;

/* ----------------------------------------------------------------------------
   STRUCTURE:      MMGSDI_NOTIFY_RECOVERY_COMPLETE_REQ_TYPE

   DESCRIPTION:
     This structure is used when notifying to MMGSDI task that recovery is
     complete on a slot.
------------------------------------------------------------------------------*/
typedef struct {
  mmgsdi_request_header_type  request_header;
} mmgsdi_notify_recovery_complete_req_type;
...
/* ----------------------------------------------------------------------------
   STRUCTURE:      MMGSDI_SESSION_TEMPORARY_IDENTITY_EXTRA_INFO_TYPE

   DESCRIPTION:
     The structure contains temporary identity
------------------------------------------------------------------------------*/
typedef struct
{
  mmgsdi_temporary_identity_enum_type    temporary_identity;
}mmgsdi_session_temporary_identity_extra_info_type;
...
typedef union
{
  ...
  mmgsdi_session_temporary_identity_extra_info_type       temporary_identity;
...
/* ----------------------------------------------------------------------------
   ...
   DESCRIPTION:
     ...
     receive_hidden_csim_events  : Client receives events for CSIM that is hidden otherwise
     slot_task_dereg_count       : Number of slot tasks where dereg needs to be processed
-------------------------------------------------------------------------------*/
typedef struct {
  mmgsdi_client_id_type      client_id;
  mmgsdi_client_name_type    client_task_name;
...
/* ----------------------------------------------------------------------------
   ...
   DESCRIPTION:
     ...
     is_auto_activation: Is the request is MMGSDI aut-activation
-------------------------------------------------------------------------------*/
typedef struct {
  mmgsdi_request_header_type      request_header;
  mmgsdi_slot_id_enum_type        card_slot;
  mmgsdi_static_data_type         app_id_data;
  boolean                         is_auto_activation;
...
/* --------------------------------------------------------------------------
   STRUCTURE:      MMGSDI_HIDE_CSIM_REQ_TYPE

   DESCRIPTION:
     This structure is used when preparing Permanently disabling CSIM request

     request_header  : Request Header information
     hide_csim       : Hide CSIM from modem clients or not
---------------------------------------------------------------------------*/
typedef struct {
  mmgsdi_request_header_type  request_header;
  boolean                     hide_csim;
} mmgsdi_hide_csim_req_type;
...
/* ----------------------------------------------------------------------------
   STRUCTURE:      MMGSDI_NOTIFY_SIM_PRE_INIT_REQ_TYPE

   DESCRIPTION:
     This structure is used when notifying a SIM pre-init request to MMGSDI Task.
     request_header :  Request Header information
-------------------------------------------------------------------------------*/
typedef struct {
  mmgsdi_request_header_type     request_header;
} mmgsdi_notify_sim_pre_init_req_type;

/* ----------------------------------------------------------------------------
   STRUCTURE:      MMGSDI_MAP_SESSIONS_TO_SLOTS_REQ_TYPE

   DESCRIPTION:
     This structure is used when requesting MMGSDI to update sessions-to-slots
     mapping
     request_header    : Request header information
     sessions_to_slots : Mapping of sessions to slots
  -------------------------------------------------------------------------------*/
typedef struct {
  mmgsdi_request_header_type                request_header;
  mmgsdi_sessions_to_slots_mapping_type     mapping;
} mmgsdi_map_sessions_to_slots_req_type;
...
/* ----------------------------------------------------------------------------
   STRUCTURE:      MMGSDI_CONTINUE_DEACTIVATE_REQ_TYPE

   DESCRIPTION:
     This structure is used when requesting MMGSDI to continue a previously
     paused deactivation request
     request_header    : Request header information
     client_req_index  : Index original deactivation req
  -------------------------------------------------------------------------------*/
typedef struct {
  mmgsdi_request_header_type                request_header;
  uint32                                    client_req_index;
} mmgsdi_continue_deactivate_req_type;
...
/* ----------------------------------------------------------------------------
   STRUCTURE:      MMGSDI_PURGE_TEMPORARY_IDENTITY_REQ_TYPE

   DESCRIPTION:
     This structure is used when request to MMGSDI task to purge the
     temporary identity
     request_header:                Request Header information
     temporary_identity_req_state:  Request state
     temporary_identity:            Temporary Identity
     client_status:                 Request result from client
------------------------------------------------------------------------------*/
typedef struct {
  mmgsdi_request_header_type                          request_header;
  mmgsdi_purge_temporary_identity_state_enum_type     temporary_identity_req_state;
  mmgsdi_temporary_identity_enum_type                 temporary_identity;
  boolean                                             client_status;
} mmgsdi_purge_temporary_identity_req_type;

/* ----------------------------------------------------------------------------
   STRUCTURE:      MMGSDI_QSH_ACTION_EVENT_CFG_REQ_TYPE

   DESCRIPTION:
     This structure is used when request to MMGSDI task to config QSH
     action events.
     request_header:                Request Header information
     cb_params                      QSH callback params
------------------------------------------------------------------------------*/
typedef struct {
  mmgsdi_request_header_type                          request_header;
  qsh_client_cb_params_s                              cb_params;
} mmgsdi_qsh_action_event_cfg_req_type;
...
typedef struct {
  ...
  union {
    ...
    mmgsdi_m2m_get_eid_req_type                     euicc_m2m_get_eid_req;
    /* Response */
    mmgsdi_uim_report_rsp_type                      uim_report_rsp;
    mmgsdi_client_id_reg_rsp_type                   client_id_reg_rsp;
    ...
    mmgsdi_notify_sim_pre_init_req_type             sim_pre_init_req;
    mmgsdi_hide_csim_req_type                       hide_csim_req;
    mmgsdi_map_sessions_to_slots_req_type           map_sessions_to_slots_req;
    mmgsdi_manage_channel_req_type                  manage_channel_req;
    mmgsdi_continue_deactivate_req_type             continue_deact_req;
    mmgsdi_notify_recovery_complete_req_type        recovery_complete_req;
    mmgsdi_start_silent_file_change_req_type        start_silent_file_change_req;
    mmgsdi_notify_recovery_start_req_type           recovery_start_req;
    mmgsdi_purge_temporary_identity_req_type        purge_temporary_identity_req;
    mmgsdi_qsh_action_event_cfg_req_type            qsh_action_event_cfg_req;
    mmgsdi_set_apdu_behavior_req_type               set_apdu_behavior_req;
...
/* ----------------------------------------------------------------------------
   STRUCTURE:      MMGSDI_CANCEL_INFO_REQUEST_TYPE

   DESCRIPTION:
     Data Structure to indicate specific request type to cancel if is_valid is
   set.
-------------------------------------------------------------------------------*/
...
typedef struct {
  boolean                    is_valid;
  mmgsdi_cmd_enum_type       request_type;
} mmgsdi_cancel_info_request_type;
...
typedef struct {
  ...
  mmgsdi_cancel_info_request_type   cancel_request_type;
...
/* ----------------------------------------------------------------------------
   ...
   DESCRIPTION:
      ...
      Test mode logging of all commands will occur if both items secure_logging
      and private_logging are set, or if the IMSI is validated as Test Imsi.
      On a commercial imsi, some EFs and commands in secure or privacy logging
      categories will be masked if that category's logging enabled boolean
      is set to FALSE.
-------------------------------------------------------------------------------*/
typedef struct {
  boolean                    secure_logging_enabled;
  boolean                    private_logging_enabled;
...
extern rex_timer_type                        mmgsdi_refresh_retry_timer[MMGSDI_MAX_NUM_SLOTS];
...
extern rex_timer_type                        mmgsdi_session_deactivation_pause_timer[MMGSDI_NUM_ATTRIBUTES];
...
extern boolean mmgsdi_sync_cmd_canceled[MMGSDI_MAX_NUM_SLOTS];

extern boolean mmgsdi_need_sync_cmd_cancel_ack[MMGSDI_MAX_NUM_SLOTS];
...
DESCRIPTION
  ...
  the queues are canceled, not the commands currently in client req table.
