...
Copyright (c) 2004 - 2021, 2023 QUALCOMM Technologies, Inc (QTI) and its licensors.
...
$Header: //components/rel/uim.mpss/6.1.0/mmgsdi/src/mmgsdi_uicc_rsp.c#8 $$ $DateTime: 2023/06/07 03:45:05 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
06/06/23   ghk     Added support for buggy card where FCP containing wrong file descriptor tag
09/09/21   cj      Fix IMSI read failure issue due to incorrect file size tag
06/05/20   sg      Verify pin after pin is unblocked for buggy card
02/28/20   tq      Add main task check before going to card
02/28/20   sg      Update default security access for invalid SC_DO tag
12/24/19   sg      Send correct status word send for VERIFY PIN command
09/16/19   sg      Fallback to file system/cache for missing files
06/19/19   bcho    Don't change PIN status to locked/blocked if it is disabled
...
11/28/18   tq      Remove read ARR print while handling response
05/07/18   rps     Retrieving attribute cache from caller context
01/29/18   rps     Correct file descriptor byte for buggy card
01/25/18   rps     Handle buggy card with no file size tag in FCP response
11/02/17   bcho    Initialize file type before parsing FCP response
10/31/17   ra      Populate file attr for EST file when card doesn't return FCP 
07/14/17   rps     Return failure if FCP has invalid FCP template tag
...
02/07/17   vdc     Send verify PIN command after enabling the PIN for buggy card
...
11/24/16   bcho    Use ALWAYS ALLOWED security condition if SC bytes dont match
...
04/19/16   vdc     Fill enum data when error resp is timeout for get file attr
04/06/16   ar      Remove duplicates from mmgsdi_util_print_file_information()
03/31/16   vdc     Do not switch protocol when multi profile AID is not present
03/18/16   nr      Added tag 0x03 for life cycle status TLV in FCP response
02/16/16   na      Reset PIN info corresponding to the non-prov app in ICC mode
...
06/19/15   av      Add length checks while parsing exp sec conditions in FCP
06/16/15   av      Fix error in porting previous check-in
06/15/15   av      Clean up mmgsdi routines used while parsing UICC select resp
...
03/06/15   vdc     Treat 91 XX response for switch profile command as success
02/25/15   tl      Remove ICC seek support
02/19/15   nr      Phonebook record read optimization
01/23/15   tl      Add file size tag workaround for select response
...
12/12/14   vdc     Fixed compilation warnings
...
12/02/14   yt      Get retries only for Verify/Unblock/Enable/Disable PIN op
...
/* Default file size of EF GID1/GID2 */
#define MMGSDI_USIM_GID_FILE_SIZE                0x01
...
/* ----------------------------------------------------------------------------
   FUNCTION:      MMGSDI_UICC_RSP_GET_FILE_DES

   DESCRIPTION:
     This function populate the UICC File Type information
...
{
  uint8 linear_file = 0x02;
  uint8 trans_file  = 0x01;
  uint8 cyclic_file = 0x06;
  uint8 DF_ADF      = 0x38;
  ...
  if((file_descriptor & 0x80) == 0)
  {
    ...
    else if((file_descriptor & 0x07) == trans_file)
    {
      decoded_data_ptr->file_type =  MMGSDI_TRANSPARENT_FILE;
    }
    else if((file_descriptor & 0x07) == cyclic_file)
    {
      decoded_data_ptr->file_type =  MMGSDI_CYCLIC_FILE;
    }
    else if((file_descriptor & 0x3F) == DF_ADF)
    {
...
static mmgsdi_return_enum_type mmgsdi_uicc_rsp_extract_exp_sec_attr(
  ...
  mmgsdi_file_security_enum_type      *key_ref_ptr,
  ...
  )
{
  ...
  MMGSDIUTIL_RETURN_IF_NULL_4(oper_ptr,
  ...
  {
    *key_ref_ptr = MMGSDI_SINGLE_ALLOWED;
  }
  oper_ptr->protection_method = *key_ref_ptr;
...
)
{
  ...
  mmgsdi_file_security_access_type *file_security_attr_ptr        = NULL;

  MMGSDIUTIL_RETURN_IF_NULL_2(sec_oper_ptr, attr_buffer_ptr);

  if(mmgsdi_file_get_security_attributes(mmgsdi_file_type,
                                         sec_oper_ptr,
                                         &file_security_attr_ptr) != MMGSDI_SUCCESS)
  {
    return MMGSDI_ERROR;
  }

  MMGSDIUTIL_RETURN_IF_NULL(file_security_attr_ptr);
          ...
          ( attr_buffer_ptr[offset] == GSDI_AM_DO_INST_TAG ) )
    {
      ...
      if ((am_do_value & SEC_INC_MSK) == SEC_INC_OPERATION)
      {
        if(security_pins.num_protection_pin > 0)
        {
          ...
          MMGSDIUTIL_TMC_MEM_FREE(file_security_attr_ptr->increase.protection_pin_ptr);
          MMGSDIUTIL_MEM_MALLOC_AND_VALIDATE(file_security_attr_ptr->increase.protection_pin_ptr,
                                             pin_length);
          if (file_security_attr_ptr->increase.protection_pin_ptr == NULL)
          {
            MMGSDIUTIL_TMC_MEM_FREE(security_pins.protection_pin_ptr);
            return MMGSDI_ERROR;
          }
          mmgsdi_status = mmgsdi_uicc_rsp_extract_exp_sec_attr(
                                    &file_security_attr_ptr->increase,
                                    &key_ref_value,
                                    &security_pins);
          if (mmgsdi_status != MMGSDI_SUCCESS)
          {
            ...
            file_security_attr_ptr->increase.protection_method = MMGSDI_NEVER_ALLOWED;
            file_security_attr_ptr->increase.num_protection_pin = 0;
            MMGSDIUTIL_TMC_MEM_FREE(file_security_attr_ptr->increase.protection_pin_ptr);
          }
          else
          {
            sec_oper_ptr->cyclic_file.increase_allowed = TRUE;
          }
        }
        else
        {
          file_security_attr_ptr->increase.protection_method = key_ref_value;
        }
      }
      ...
      else
      {
        ...
        if ((am_do_value & SEC_READ_MSK) == SEC_READ_OPERATION)
        {
          ...
          {
            pin_length = sizeof(mmgsdi_pin_enum_type)*int32touint32(security_pins.num_protection_pin);
            MMGSDIUTIL_TMC_MEM_FREE(file_security_attr_ptr->read.protection_pin_ptr);
            MMGSDIUTIL_MEM_MALLOC_AND_VALIDATE(
              file_security_attr_ptr->read.protection_pin_ptr,
              pin_length);
            if (file_security_attr_ptr->read.protection_pin_ptr == NULL)
            {
              MMGSDIUTIL_TMC_MEM_FREE(security_pins.protection_pin_ptr);
              return MMGSDI_ERROR;
            }
            mmgsdi_status = mmgsdi_uicc_rsp_extract_exp_sec_attr(
                                      &file_security_attr_ptr->read,
                                      &key_ref_value,
                                      &security_pins);
            if (mmgsdi_status != MMGSDI_SUCCESS)
            {
              ...
              file_security_attr_ptr->read.protection_method = MMGSDI_NEVER_ALLOWED;
              file_security_attr_ptr->read.num_protection_pin = 0;
              MMGSDIUTIL_TMC_MEM_FREE(file_security_attr_ptr->read.protection_pin_ptr);
            }
          }
          else
          {
            file_security_attr_ptr->read.protection_method = key_ref_value;
          }
        }
        ...
        if ((am_do_value & SEC_UPDATE_MSK) == SEC_UPDATE_OPERATION)
        {
          ...
          {
            pin_length = sizeof(mmgsdi_pin_enum_type)*int32touint32(security_pins.num_protection_pin);
            MMGSDIUTIL_TMC_MEM_FREE(file_security_attr_ptr->write.protection_pin_ptr);
            MMGSDIUTIL_MEM_MALLOC_AND_VALIDATE(file_security_attr_ptr->write.protection_pin_ptr,
                                               pin_length);
            if (file_security_attr_ptr->write.protection_pin_ptr == NULL)
            {
              MMGSDIUTIL_TMC_MEM_FREE(security_pins.protection_pin_ptr);
              return MMGSDI_ERROR;
            }
            mmgsdi_status = mmgsdi_uicc_rsp_extract_exp_sec_attr(
                                      &file_security_attr_ptr->write,
                                      &key_ref_value,
                                      &security_pins);
            if (mmgsdi_status != MMGSDI_SUCCESS)
            {
              ...
              file_security_attr_ptr->write.protection_method = MMGSDI_NEVER_ALLOWED;
              file_security_attr_ptr->write.num_protection_pin = 0;
              MMGSDIUTIL_TMC_MEM_FREE(file_security_attr_ptr->write.protection_pin_ptr);
            }
          }
          else
          {
            file_security_attr_ptr->write.protection_method = key_ref_value;
          }
        }
        ...
        if ((am_do_value & SEC_DEACT_MSK) == SEC_DEACT_OPERATION)
        {
          ...
          {
            pin_length = sizeof(mmgsdi_pin_enum_type)*int32touint32(security_pins.num_protection_pin);
            MMGSDIUTIL_TMC_MEM_FREE(file_security_attr_ptr->invalidate_deactivate.protection_pin_ptr);
            MMGSDIUTIL_MEM_MALLOC_AND_VALIDATE(file_security_attr_ptr->invalidate_deactivate.protection_pin_ptr,
                                               pin_length);
            if (file_security_attr_ptr->invalidate_deactivate.protection_pin_ptr == NULL)
            {
              MMGSDIUTIL_TMC_MEM_FREE(security_pins.protection_pin_ptr);
              return MMGSDI_ERROR;
            }
            mmgsdi_status = mmgsdi_uicc_rsp_extract_exp_sec_attr(
                                      &file_security_attr_ptr->invalidate_deactivate,
                                      &key_ref_value,
                                      &security_pins);
            if (mmgsdi_status != MMGSDI_SUCCESS)
            {
              ...
              file_security_attr_ptr->invalidate_deactivate.protection_method = MMGSDI_NEVER_ALLOWED;
              file_security_attr_ptr->invalidate_deactivate.num_protection_pin = 0;
              MMGSDIUTIL_TMC_MEM_FREE(file_security_attr_ptr->invalidate_deactivate.protection_pin_ptr);
            }
          }
          else
          {
            file_security_attr_ptr->invalidate_deactivate.protection_method = key_ref_value;
          }
        }
        ...
        if ((am_do_value & SEC_ACT_MSK) == SEC_ACT_OPERATION)
        {
          ...
          {
            pin_length = sizeof(mmgsdi_pin_enum_type)*int32touint32(security_pins.num_protection_pin);
            MMGSDIUTIL_TMC_MEM_FREE(file_security_attr_ptr->rehabilitate_activate.protection_pin_ptr);
            MMGSDIUTIL_MEM_MALLOC_AND_VALIDATE(file_security_attr_ptr->rehabilitate_activate.protection_pin_ptr,
                                               pin_length);
            if (file_security_attr_ptr->rehabilitate_activate.protection_pin_ptr == NULL)
            {
              MMGSDIUTIL_TMC_MEM_FREE(security_pins.protection_pin_ptr);
              return MMGSDI_ERROR;
            }
            mmgsdi_status = mmgsdi_uicc_rsp_extract_exp_sec_attr(
                                      &file_security_attr_ptr->rehabilitate_activate,
                                      &key_ref_value,
                                      &security_pins);
            if (mmgsdi_status != MMGSDI_SUCCESS)
            {
              ...
              file_security_attr_ptr->rehabilitate_activate.protection_method = MMGSDI_NEVER_ALLOWED;
              file_security_attr_ptr->rehabilitate_activate.num_protection_pin = 0;
              MMGSDIUTIL_TMC_MEM_FREE(file_security_attr_ptr->rehabilitate_activate.protection_pin_ptr);
            }
          }
          else
          {
            file_security_attr_ptr->rehabilitate_activate.protection_method = key_ref_value;
          }
        }
      }
    }
...
/* ----------------------------------------------------------------------------
   ...
   DESCRIPTION:
     ...
     If caller from mmgsdi context, sync read will be done for ARR record if not
     found in cache. If called from client context, it will not fetch ARR record
     from card.
...
-------------------------------------------------------------------------------*/
mmgsdi_return_enum_type mmgsdi_uicc_rsp_read_arr(
  mmgsdi_session_id_type                 session_id,
  const mmgsdi_access_type              *access_ptr,
  uint16                                 arr_file_id,
  uint8                                  ef_arr_record_number,
  mmgsdi_file_structure_enum_type        mmgsdi_file_type,
  mmgsdi_file_security_attributes_type  *security_access_ptr
)
{
  ...
  memset(&path_data, 0x00, sizeof(mmgsdi_path_type));

  MMGSDIUTIL_RETURN_IF_NULL_2(access_ptr, security_access_ptr);
  ...
  {
    mmgsdi_file = access_ptr->file.file_enum;
  }
  else if (access_ptr->access_method == MMGSDI_BY_PATH_ACCESS)
  {
    if(access_ptr->file.path_type.path_len > MMGSDI_MAX_PATH_LEN)
    {
      UIM_MSG_ERR_1("Invalid path length: 0x%x",
                    access_ptr->file.path_type.path_len);
      return MMGSDI_ERROR;
    }

    path_data = access_ptr->file.path_type;
       ...
       path_data.path_len > 1)
    {
      ...
      mmgsdi_memscpy(path_data.path_buf,
                     sizeof(path_data.path_buf),
                     &access_ptr->file.path_type.path_buf[1],
                     (access_ptr->file.path_type.path_len - 1) * sizeof(uint16));
      path_data.path_len = access_ptr->file.path_type.path_len - 1;
    }
  }

  /* Get the file id of arr */
  switch(arr_file_id & 0xFF00)
  {
    case 0x2F00:
      num_arr_file = 1;
      arr_files[0] = (uint16)MMGSDI_ARR_UNDER_MF;

      if(mmgsdi_nv_get_device_feature_status(MMGSDI_FEATURE_SPECIAL_USIM_ARR) == MMGSDI_FEATURE_ENABLED)
      {
        EF_category = MMGSDIFILE_GET_CATEGORY_FROM_FILE(mmgsdi_file);
        if (EF_category == MMGSDI_EF_CATRGORY_USIM)
        {
          num_arr_file = 2;
    ...
    case 0x6F00:
      EF_category = MMGSDIFILE_GET_CATEGORY_FROM_FILE(mmgsdi_file);
      arr_files[0] = (uint16)MMGSDI_ARR_UNDER_MF;
      if(EF_category == MMGSDI_EF_CATRGORY_BY_PATH)
      {
        uint8 i = 0;
        /* Max number of ARR files to be checked against */
        num_arr_file = path_data.path_len;
    ...
    case 0x4F00:
      ...
      arr_files[0] = (uint16)MMGSDI_ARR_UNDER_MF;
      arr_files[1] = (uint16)MMGSDI_ARR_FIRST_DF_LEVEL;
      arr_files[2] = arr_file_id;
      break;
    default:
      UIM_MSG_ERR_1("Wrong EF ARR ID 0x%x", arr_file_id);
      return MMGSDI_ERROR;
  }
  /* read the file*/
  while(num_arr_file)
  {
    ...
    else if(arr_files[num_arr_file-1] == MMGSDI_ARR_FIRST_DF_LEVEL)
    {
      ...
      if (EF_category == MMGSDI_EF_CATRGORY_USIM)
      {
        if(mmgsdi_nv_get_device_feature_status(MMGSDI_FEATURE_SPECIAL_USIM_ARR) == MMGSDI_FEATURE_ENABLED)
        {
          if((arr_file_id & 0xFF00) == 0x2F00)
          {
            ...
            arr_access.file.path_type.path_buf[2]= arr_file_id;
          }
          else
          {
      ...
      else if (EF_category == MMGSDI_EF_CATRGORY_CSIM)
      {
        arr_access.file.file_enum = MMGSDI_CSIM_ARR;
      }
      else if (EF_category == MMGSDI_EF_CATRGORY_TELECOM)
      {
        arr_access.file.file_enum = MMGSDI_TELECOM_ARR;
      }
      else if(EF_category == MMGSDI_EF_CATRGORY_ISIM)
      {
        arr_access.file.file_enum = MMGSDI_ISIM_ARR;
      }
      else if( EF_category == MMGSDI_EF_CATRGORY_BY_PATH)
      {
        ...
        switch(path_data.path_buf[0])
        {
          ...
          case GSDI_ADF_PATH_USED:
            ...
            if(num_arr_file > 2)
            {
              ...
              arr_access.file.path_type.path_len = num_arr_file + 1;
              arr_access.file.path_type.path_buf[0]= (uint16)MMGSDI_MF_LEVEL;
              ...
              arr_access.file.path_type.path_buf[i]= arr_file_id;
            }
    ...
    else if (arr_files[num_arr_file - 1] == arr_file_id) /* 4Fxx ARR EF case */
    {
      ...
      /* Now that we have the path to the EF, lets substitute the EF ID
         with the EF ARR ID in order to create a path to EF ARR in "this"
         directory */
      arr_access.file.path_type.path_buf[path_data.path_len - 1] = arr_file_id;
    }
    else
    {
      return MMGSDI_ERROR;
    ...
    /* Do not goto card if skip_card_access is true. This is in case
       when attributes is tried to be decoded from non-gsdi task or main task, i.e.
       in case of caller context, in which we cannot goto card to read
       ARR record. */
    if(!MMGSDIUTIL_IS_MMGSDI_SLOT_TASK(mmgsdi_task))
    {
      /* If record is not found then goto next level of ARR checking,
         else record is not INIT or some error, in that case return to 
         error or NOT_INIT, so that read can be send to card */
        ...
        (cnf_ptr->response_header.mmgsdi_status == MMGSDI_SUCCESS))
    {
      ...
      if (mmgsdi_cache_write(session_id,
                             &arr_access,
                             cnf_ptr->read_data,
                             0,
                             0,
                             ef_arr_record_number) != MMGSDI_SUCCESS)
      {
...
)
{
  ...
  byte                                  i                    = 0;
  mmgsdi_session_info_type             *session_info_ptr     = NULL;
  mmgsdi_file_security_attributes_type *operation_ptr        = NULL;

  MMGSDIUTIL_RETURN_IF_NULL_2(attr_buffer_ptr, access_ptr);
  ...
  arr_record_number = attr_buffer_ptr[rec_number_offset];
  ...
  if (skip_uicc_arr)
  {
    decoded_attr_ptr->is_arr_skipped = skip_uicc_arr;
    decoded_attr_ptr->arr_file_id = ((attr_buffer_ptr[0] & 0xFF) << 8) + (attr_buffer_ptr[1] & 0xFF);
    decoded_attr_ptr->arr_record_no = arr_record_number;
    UIM_MSG_HIGH_0("Skipping reading EF_ARR, assuming default security attributes");
    (void)mmgsdi_file_set_default_sec_attr_per_spec(&decoded_attr_ptr->file_info,
                                                    decoded_attr_ptr->file_type,
  ...
  mmgsdi_status = mmgsdi_uicc_rsp_read_arr(session_id,
                                           access_ptr,
                                           ((attr_buffer_ptr[0] & 0xFF) << 8) + (attr_buffer_ptr[1] & 0xFF),
                                           arr_record_number,
                                           decoded_attr_ptr->file_type,
                                           &decoded_attr_ptr->file_info);

  return mmgsdi_status;
} /* mmgsdi_uicc_rsp_decode_ref_exp_sec_attr */
...
static mmgsdi_return_enum_type mmgsdi_uicc_rsp_decode_cmp_cmd_sec_attr(
  uint8                     *num_sc_bytes_referenced_ptr,
  uint8                      num_of_sc_bytes,
...
)
{
  ...
  if(*num_sc_bytes_referenced_ptr < num_of_sc_bytes)
  {
    ...
    {
      /* If SC byte has other values, set ALWAYS ALLOWED protection method.
         Value 0x01 is reserved for the default SE, always available; for other
         values also we are using same. */
      command_sec_ptr->protection_method = MMGSDI_ALWAYS_ALLOWED;
      command_sec_ptr->num_protection_pin = 0;
    }
...
{
  mmgsdi_return_enum_type           mmgsdi_status           = MMGSDI_SUCCESS;
  uint8                             num_of_sc_bytes         = 0;
  uint8                             last_buffer_index       = 0;
  ...
  uint8                             sc_bytes[7];
  uint8                             num_sc_bytes_referenced = 0;
  uint8                             am_byte                 = 0;
  uint8                             i                       = 0;
  ...
  mmgsdi_file_attributes_type *file_attr_ptr)
{
  ...
  MMGSDIUTIL_TMC_MEM_FREE(file_security_access_ptr->increase.protection_pin_ptr);
  MMGSDIUTIL_TMC_MEM_FREE(file_security_access_ptr->read.protection_pin_ptr);
  MMGSDIUTIL_TMC_MEM_FREE(file_security_access_ptr->write.protection_pin_ptr);
...
/* ----------------------------------------------------------------------------
   ...
   DESCRIPTION:
     ...
     It skips decoding security attr when skip_decode_sec_attr is set to TRUE
   ...
   RETURN VALUE:
     ...
     MMGSDI_SUCCESS:                The command process is successful.
     MMGSDI_ERROR:                  The Input parameter is null.
     MMGSDI_WRONG_FCP_TEMPLATE_TAG: The FCP template tag is inconsistent.

   SIDE EFFECTS:

-------------------------------------------------------------------------------*/
mmgsdi_return_enum_type mmgsdi_uicc_rsp_decode_EF_header_data (
  mmgsdi_session_id_type                       session_id,
  const mmgsdi_access_type                   * access_ptr,
  boolean                                      skip_uicc_arr,
...
)
{
  ...
  MMGSDIUTIL_RETURN_IF_NULL_3(header_content_ptr, decoded_data_ptr, access_ptr);

  if(header_content_ptr == NULL || header_length < 1)
  {
    /* There are some commercial cards that do not return FCP data for
       SELECT operation on EF EST file. To support it, the file size is
       assumed as 1 byte which is a mandatory byte according to 3GPP TS 31.102 */
    if(access_ptr->access_method == MMGSDI_EF_ENUM_ACCESS &&
       access_ptr->file.file_enum == MMGSDI_USIM_EST)
    {
      decoded_data_ptr->file_type  = MMGSDI_TRANSPARENT_FILE;
      decoded_data_ptr->file_size  = 0x01;
      decoded_data_ptr->file_id[0] = 0x6F;
      decoded_data_ptr->file_id[1] = 0x56;

      (void)mmgsdi_file_set_default_sec_attr(decoded_data_ptr);

      return MMGSDI_SUCCESS;
    }

    return MMGSDI_WRONG_FCP_TEMPLATE_TAG;
  }
  ...
  /* FCP template length is determined by byte 2 or 3 of the data
     ...
     In order to determine if the FCP length is 1 or 2 bytes, lets
     locate the first mandatory tag parameter. According to 101 220,
     we have the length encoding in section 7.1.2:
     If byte2 is used for length, that byte contains => 00 to 7F.
     If byte3 is used for length, byte2 contains 81 and byte3 contains
     the actual length that is between 80 and FF. */
  if(header_length >= 3 &&
     header_content_ptr[1] <= 0x7F &&
     (header_content_ptr[2] == GSDI_FILE_DESCRIPTOR_TAG ||
      header_content_ptr[2] == GSDI_FILE_DESCRIPTOR_BUGGY_TAG ))
  {
    ...
    total_len = header_content_ptr[1] + 2;
          ...
          header_content_ptr[2] >= 0x80 &&
          (header_content_ptr[3] == GSDI_FILE_DESCRIPTOR_TAG ||
           header_content_ptr[3] == GSDI_FILE_DESCRIPTOR_BUGGY_TAG))
  {
  ...
  if(total_len > (file_descriptor_offset + 6) &&
     (header_content_ptr[file_descriptor_offset] == GSDI_FILE_DESCRIPTOR_TAG ||
      header_content_ptr[file_descriptor_offset] == GSDI_FILE_DESCRIPTOR_BUGGY_TAG) &&
     (header_content_ptr[file_descriptor_offset + 1] == 2 ||
      header_content_ptr[file_descriptor_offset + 1] == 5))
  {
    uint8 file_descriptor_byte = header_content_ptr[file_descriptor_offset + 2];

    /* For some buggy commercial card, file descriptor value byte has the MSB bit set.
       Current MSB i.e. b8 bit, is reserved for future use. So make b8 bit to zero,
       in order to successfully parse the file type. */
    if(access_ptr->access_method == MMGSDI_EF_ENUM_ACCESS &&
       access_ptr->file.file_enum == MMGSDI_USIM_EST)
    {
      file_descriptor_byte = file_descriptor_byte & 0x7F;
    }

    /* File descriptor byte */
    mmgsdi_status = mmgsdi_uicc_rsp_get_file_des(file_descriptor_byte,
                      decoded_data_ptr);
    /* The actual value of Data Coding Byte, that is,
       header_content_ptr[file_descriptor_offset+3] shall not be checked
       nor used by the terminal per 102 221 */
  ...
  /* Mandatory life cyclic flag. The tag GSDI_LIFE_CYCLE_INTEG_BUGGY_TAG(0x03) is added to support
     a commercial USIM card. This has 0x03 as tag instead of life cyclic status tag in 4th data
     object TLV. As this is a commercial card we allow the parsing of TLV to proceed further.*/
  if (total_len > (life_cycle_offset + 2) &&
      (header_content_ptr[life_cycle_offset] == GSDI_LIFE_CYCLE_INTEG_TAG ||
       header_content_ptr[life_cycle_offset] == GSDI_LIFE_CYCLE_INTEG_BUGGY_TAG) &&
  ...
  /* We do not need to decode security attributes when this function
     was called on the fly to return attributes from the caller context
     as decoded security parameters were already present in attributes cache.*/
  if(!skip_decode_sec_attr)
  {
            ...
            total_len > (security_attr_offset + header_content_ptr[security_attr_offset + 1] + 1))
    {
      ...
      mmgsdi_status = mmgsdi_uicc_rsp_decode_exp_sec_attr(
                              &header_content_ptr[security_attr_offset + 2],
                              header_content_ptr[security_attr_offset + 1],
                              decoded_data_ptr->file_type,
                              &decoded_data_ptr->file_info);
      if (mmgsdi_status != MMGSDI_SUCCESS)
      {
        mmgsdi_uicc_rsp_free_pin_protection_info(decoded_data_ptr);
      }
            ...
            total_len > (security_attr_offset + header_content_ptr[security_attr_offset + 1] + 1))
    {
      ...
      mmgsdi_status = mmgsdi_uicc_rsp_decode_ref_exp_sec_attr(session_id,
                                                              access_ptr,
                                                              skip_uicc_arr,
                                                              &header_content_ptr[security_attr_offset + 2],
                                                              header_content_ptr[security_attr_offset + 1],
  ...
  /* File size Tag (Mandatory)
     Use alternate tag in below cases:
     1) Certain commercial cards use alternate, non-spec complient values for the
        file size tag so also check for alternate fize size tags.
     2) In some commercial buggy card, file size tag is not present for EF_UST. 
        So check for optional total file size tag, and if found use that as file size.
     3) In some commercial buggy card, non-spec complient value for file size tag 
        present in FCP IMSI file so also check for buggy file size tag for IMSI file
        If total file size tag is not present return error */
  ...
  if(total_len > (file_size_offset + 3) &&
     (header_content_ptr[file_size_offset] == GSDI_FILE_SIZE_TAG ||
      ...
      (((access_ptr->access_method == MMGSDI_EF_ENUM_ACCESS &&
         access_ptr->file.file_enum == MMGSDI_USIM_IMSI) ||
        (access_ptr->access_method == MMGSDI_BY_PATH_ACCESS && 
         access_ptr->file.path_type.path_len == 0x03 && 
         access_ptr->file.path_type.path_buf[0] == GSDI_MF_PATH_USED &&
         access_ptr->file.path_type.path_buf[1] == GSDI_ADF_PATH_USED &&
         access_ptr->file.path_type.path_buf[2] == GSDI_IMSI_PATH_USED)) &&
       header_content_ptr[file_size_offset] == GSDI_FILE_SIZE_BUGGY_TAG) ||
      (decoded_data_ptr->file_type == MMGSDI_TRANSPARENT_FILE &&
       header_content_ptr[file_size_offset] == GSDI_TOTAL_FILE_SIZE_TAG)) &&
  ...
  /* It is possible that file size tag was not present, but we have already 
     read the total file size. In this case, the offset is the same as total file size */
  if(header_content_ptr[file_size_offset] == GSDI_TOTAL_FILE_SIZE_TAG)
  {
    total_file_size_offset = file_size_offset;
  }
  else /* Optional total file size tag */
  {
  ...
  return MMGSDI_SUCCESS;
} /* mmgsdi_uicc_rsp_decode_EF_header_data */
...
)
{
  ...
  (void)mmgsdi_util_get_channel_app(slot,
...
)
{
  ...
  return mmgsdi_status;
} /* mmgsdi_uicc_rsp_decode_pin_header_data */
...
-------------------------------------------------------------------------------*/
mmgsdi_return_enum_type mmgsdi_uicc_rsp_decode_file_header (
  const mmgsdi_request_header_type           * orig_req_header_ptr,
  const mmgsdi_get_file_info_extra_info_type * orig_extra_info_ptr,
  const uint8                                * header_content_ptr,
  int32                                        header_length,
...
)
{
  ...
  if(NULL == gsdi_format_attr_ptr ||
    orig_extra_info_ptr == NULL ||
    orig_req_header_ptr == NULL)
  {
    return MMGSDI_INCORRECT_PARAMS;
  }

  memset( gsdi_format_attr_ptr, 0x00, sizeof(mmgsdi_file_attributes_type) );

  /* Initialize  file type to MMGSDI_MAX_FILE_STRUCTURE_ENUM so that if file
     type is not set explicitly because of some parsing issue, it will not be
     considered as MMGSDI_LINEAR_FIXED_FILE (0) due to above memset. */
  gsdi_format_attr_ptr->file_type = MMGSDI_MAX_FILE_STRUCTURE_ENUM;
 ...
 /* Parse for PIN */
  ...
  mmgsdi_status = mmgsdi_uicc_rsp_decode_EF_header_data(orig_req_header_ptr->session_id,
                                                        &orig_extra_info_ptr->access,
                                                        orig_extra_info_ptr->skip_uicc_arr,
                                                        header_content_ptr,
                                                        header_length,
  ...
  /* There are some commercial cards that have an invalid FCP template
     for GID file, causing the length to be determined as 0, even if
     the file is present. In these cases, assume that the size of the
     GID file is 1 byte, as this is the most common size for the file. */
  if(orig_extra_info_ptr->access.access_method == MMGSDI_EF_ENUM_ACCESS &&
     (orig_extra_info_ptr->access.file.file_enum == MMGSDI_USIM_GID1 ||
      orig_extra_info_ptr->access.file.file_enum == MMGSDI_USIM_GID2) &&
     gsdi_format_attr_ptr->file_size == 0x00)
  {
    gsdi_format_attr_ptr->file_size = MMGSDI_USIM_GID_FILE_SIZE;
  }
...
)
{
  ...
  /* Parse the UIM response to Select request and provide content
     from File System if file system fallback is enabled for the file */
  report_status = mmgsdi_file_fallback_parse_uim_get_file_attr_rsp(modified_uim_rsp_ptr,
                                                             &req_header,
                                                             extra_param_ptr,
                                                             report_status);
  ...
  {
    UIM_MSG_ERR_2("Select failed status 0x%x, cmd_transacted 0x%x",
                  report_status,
  ...
  else if ((report_status == MMGSDI_UIM_CMD_TIMEOUT || report_status == MMGSDI_NOT_FOUND) &&
           req_header.orig_request_type == MMGSDI_GET_FILE_ATTR_REQ)
  {
    uint8                  slot_index              = MMGSDI_SLOT_1_INDEX;
    mmgsdi_slot_data_type *mmgsdi_slot_data_ptr    = NULL;
    boolean                fcn_refresh_in_progress = FALSE;

    mmgsdi_slot_data_ptr = mmgsdi_util_get_slot_data_ptr(req_header.slot_id);

    /* In some buggy card, in FCN refresh, file is made available after sending
       TR to card. So do not mark file not found if FCN refresh in refresh.
       Later when client try to read the file, the request will goto card and will
       not be blocked due to the cache state which would say MMGSDI_NOT_FOUND */
    if(mmgsdi_slot_data_ptr != NULL &&
       mmgsdi_util_get_slot_index(req_header.slot_id,
                                  &slot_index) == MMGSDI_SUCCESS &&
       mmgsdi_refresh_info_ptr[slot_index] != NULL &&
       mmgsdi_refresh_info_ptr[slot_index]->mode == MMGSDI_REFRESH_NAA_FCN &&
       mmgsdi_slot_data_ptr->refresh_state == MMGSDI_REFRESH_WAIT_FOR_FCN_COMPLETE_STATE)
    {
      fcn_refresh_in_progress = TRUE;
    }

    /* Update cache to indicate that file was not found on card */
    if(!fcn_refresh_in_progress)
    {
      (void)mmgsdi_cache_write(req_header.session_id,
                               &extra_param_ptr->get_file_attr_data.access,
                               modified_uim_rsp_ptr->rsp_data,
                               0,
  ...
  if (!synch)
  {
    mmgsdi_status = mmgsdi_cnf_build_and_queue(report_status,
                                               &req_header,
                                               extra_param_ptr,
                                               (void*)&(modified_uim_rsp_ptr->rsp_data),
                                               modified_uim_rsp_ptr->cmd_transacted,
  ...
  else
  {
    ...
    mmgsdi_status = mmgsdi_cnf_build_get_file_attr_data(cnf_ptr,
                                                        &report_status,
                                                        extra_param_ptr, 
                                                        &req_header, 
                                                        &(modified_uim_rsp_ptr->rsp_data));
  ...
  else if ( req_header.orig_request_type == MMGSDI_CARD_RESET_REQ)
  {
        ...
        req_header.slot_id))
    {
      ...
      if (mmgsdi_status == MMGSDI_SUCCESS)
      {
        ...
        /* In case of UICC card, Refresh event with REFRESH START with mode
           RESET is sent to all clients after receiving response of status command */
...
)
{
  ...
  switch(report_status)
  {
    ...
    case MMGSDI_CODE_BLOCKED:
      ...
      /* Dont set PIN status to blocked if it is disabled or MMGSDI/client is
         trying to get PIN status. */
      if (pin_info_ptr->status != MMGSDI_PIN_DISABLED ||
          orig_request_type != MMGSDI_GET_ALL_PIN_STATUS_REQ) 
      {
    ...
    case MMGSDI_CODE_PERM_BLOCKED:
      ...
      /* Dont set PIN status to perm blocked if it is disabled or MMGSDI/client
         is trying to get PIN status. */
      if (pin_info_ptr->status != MMGSDI_PIN_DISABLED ||
          orig_request_type != MMGSDI_GET_ALL_PIN_STATUS_REQ) 
      {
    ...
    case MMGSDI_ERROR_NO_EVENT_NEEDED:
      pin_info_ptr->valid_num_retries         = TRUE;
      pin_info_ptr->valid_num_unblock_retries = TRUE;
      if (mmgsdi_nv_get_device_feature_status(MMGSDI_FEATURE_MMGSDI_UNBLOCK_PIN_HANDLE_INVALID_SW) ==
  ...
  if (report_status != MMGSDI_SUCCESS)
  {
   ...
   MMGSDIUTIL_DEBUG_MSG_ERROR_3("UIM_FAIL sw1:%x sw2:%x status:0x%x",
    ...
    if (extra_param_ptr->pin_data.orig_pin_op != MMGSDI_PIN_OP_NONE)
    {
      ...
      /* For VERIFY_PIN command, after verifying PIN, GET_NUM_OF_UNBLOCK_RETRIES and
         GET_NUM_RETRIES is done. In order to send send status word of original VERIFY PIN command
         below condition check are requiered. */
...
)
{
  ...
  uint32                                index           = 0;
  mmgsdi_client_req_extra_info_type   * extra_param_ptr = NULL;
  mmgsdi_remote_sim_test_params_type    rm_sim_test_param_record = {0};
  ...
  /* If this response needs to be delayed to the client per the
     EFS file, we store the entire response & setup a timer. */
  if (mmgsdi_nv_get_remote_test_params(
        req_header.slot_id, &rm_sim_test_param_record) &&
      (rm_sim_test_param_record.delay_interval > 0))
  {
    UIM_MSG_HIGH_1("Timed sleep for interval: %d msecs",
                   rm_sim_test_param_record.delay_interval);

    MMGSDIUTIL_LEAVE_TASK_SYNC_CRIT_SECT;
    mmgsdi_util_timed_sleep((unsigned long)rm_sim_test_param_record.delay_interval);
