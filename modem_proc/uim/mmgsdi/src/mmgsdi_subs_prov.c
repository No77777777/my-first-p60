/*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*
             ...
             M M G S D I   S U B S   P R O V   F U N C T I O N S
...
$Header: //components/rel/uim.mpss/6.1.0/mmgsdi/src/mmgsdi_subs_prov.c#2 $
...
12/04/15   bcho    gsdi_efs_read_file() replaced gsdi_efs_read_file_with_len()
08/25/15   bcho    Support for CDMALess Device
08/20/15   ar      Initial revision
...
/*===========================================================================
FUNCTION MMGSDI_SUBS_PROV_READ_FILE

DESCRIPTION
  This function reads "subs_prov_info.txt" and populates subs provisioning global
...
SIDE EFFECTS
  None
============================================================================ */
static mmgsdi_return_enum_type mmgsdi_subs_prov_read_file(void)
{
  mmgsdi_return_enum_type mmgsdi_status = MMGSDI_SUCCESS;
  uint32                  file_length   = 0;

  if(mmgsdi_subs_prov_info.is_efs_content_valid)
  {
...
)
{
  ...
  (void)mmgsdi_subs_prov_read_file();
  ...
  if (mmgsdi_status != MMGSDI_SUCCESS)
  {
   ...
   /* If we fail to get iccid clear data in EFS for that slot */
    memset(&mmgsdi_subs_prov_info.subs_prov_data[slot_index],
           0x00, sizeof(mmgsdi_subs_prov_data_type));
    (void)mmgsdi_write_to_subs_prov_efs();
    return;
  }
  ...
  subs_prov_data_ptr->iccid.length = (uint8)iccid.data_len;
...
)
{
  ...
  if(MMGSDI_SUCCESS != mmgsdi_subs_prov_read_file())
  {
    return;
  }
...
)
{
      ...
      index++)
  {
    ...
    if(app_specific_info_ptr->app_data_valid)
    {
      ...
      {
        UIM_MSG_HIGH_2("CDMALess NV is enabled for context 0x%x slot 0x%x; not activating",
                       nv_context, slot_id);
        continue;
      }
...
)
{
  ...
  /* clear data in EFS for requested slot */
  memset(&mmgsdi_subs_prov_info.subs_prov_data[slot_index],
         0x00, sizeof(mmgsdi_subs_prov_data_type));
  (void)mmgsdi_write_to_subs_prov_efs();
