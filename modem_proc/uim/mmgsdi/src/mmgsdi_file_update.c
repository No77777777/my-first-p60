...
$Header: //components/rel/uim.mpss/6.1.0/mmgsdi/src/mmgsdi_file_update.c#2 $$ $DateTime: 2019/10/30 06:56:05 $ $Author: pwbldsvc $
    ...
    if (session_ptr->file_data.file_list_ptr[num_files].access_method == MMGSDI_BY_PATH_ACCESS)
    {
      if ((file_path.path_len == session_ptr->file_data.file_list_ptr[num_files].file.path_type.path_len) &&
          (!memcmp(&file_path.path_buf,
                   ...
                   (session_ptr->file_data.file_list_ptr[num_files].file.path_type.path_len * sizeof(uint16)))))
      {
        return TRUE;
      }
    }
...
)
{
  ...
  switch(evt_info_ptr->data.file_update.file_type)
  {
    ...
    case MMGSDI_CYCLIC_FILE:
      evt_info_ptr->data.file_update.file_index.rec_num = write_extra_ptr->rec_num;
      break;
    case MMGSDI_TRANSPARENT_FILE:
      evt_info_ptr->data.file_update.file_index.offset = write_extra_ptr->offset;
      break;
    default:
