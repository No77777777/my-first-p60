...
Copyright (c) 2001-2009, 2011-2016 QUALCOMM Technologies, Inc (QTI) and its licensors.
...
$Header: //components/rel/uim.mpss/6.1.0/mmgsdi/src/mmgsdi_cphs.c#2 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
07/01/16   ar      Remove extra logging
...
INPUT:
  session_type: To get application info pointer
  ...
  mmgsdi_cphs_int_info_type           **cphs_int_info_pptr)
{
  ...
  switch (session_type)
  {
    ...
    case MMGSDI_GW_PROV_TER_SESSION:
      mmgsdi_status = mmgsdi_util_get_mmgsdi_session_id_from_provisioning_type(
                        session_type,
                        &session_id);
      ...
      *cphs_int_info_pptr = int_app_info_ptr->cphs_info_ptr;
      break;
    default:
...
)
{
  ...
  /* --------------------------------------------------------------------------
     ...
     ------------------------------------------------------------------------*/
  while ( (iteration + 1) < data.data_len)
  {
  ...
  if(read_cphs_efs.read_ons)
  {
    ...
    if (mmgsdi_status == MMGSDI_SUCCESS )
    {
      if(mmgsdi_data_buf.data_len > 0 && mmgsdi_data_buf.data_ptr != NULL)
      {
        ...
        MMGSDIUTIL_TMC_MEM_FREE(cphs_int_info_ptr->cphs_ons_ptr);
  ...
  if(read_cphs_efs.read_ons_short)
  {
    ...
    if (mmgsdi_status == MMGSDI_SUCCESS )
    {
      if(mmgsdi_data_buf.data_len > 0 && mmgsdi_data_buf.data_ptr != NULL)
      {
        ...
        MMGSDIUTIL_TMC_MEM_FREE(cphs_int_info_ptr->cphs_ons_short_ptr);
