...
Copyright (c) 2017, 2020 QUALCOMM Technologies, Inc (QTI) and
...
$Header: //components/rel/uim.mpss/6.1.0/mmgsdi/src/mmgsdi_euicc_m2m.c#2 $$ $DateTime: 2020/04/06 01:43:58 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
03/27/20   vgd     Support get_eid for M2M cards on all targets
06/20/17   ku      Compilation error fix for M2M Cards
...
/* Minimum size of the response is 4, as it includes one
   byte for tag, one byte for length and status words */
...
/*===========================================================================
   FUNCTION:      MMGSDI_M2M_CREATE_EID_APDU
   ...
   SIDE EFFECTS:
     None
===========================================================================*/
static mmgsdi_return_enum_type mmgsdi_m2m_create_eid_apdu(
  ...
  const mmgsdi_cnf_type    *cnf_ptr)
{
  ...
  UIM_MSG_HIGH_3("mmgsdi_euicc_m2m_get_eid_cb: cnf = 0x%x, slot_id = 0x%x and status = 0x%x",
  ...
  do
  {
    if (cnf == MMGSDI_SESSION_OPEN_WITH_SELECT_RSP_CNF)
    {
      ...
      mmgsdi_status = mmgsdi_m2m_create_eid_apdu(&apdu_data.data_len,
                                                 &apdu_data.data_ptr,
  ...
  /* Queue the confirmation to QMI UIM In error as well as success case */
  ...
  mmgsdi_task_cmd_type               *task_cmd_ptr)
{
  ...
  uint8                               mmgsdi_ecasd_aid[]   = {0xA0, 0x00, 0x00,0x05, 
                                                              ...
                                                              0x00, 0x00, 0x02,0x00};
  mmgsdi_slot_data_type              *slot_data_ptr        = NULL;
  mmgsdi_m2m_get_eid_req_type        *eid_req_ptr          = NULL;

  MMGSDIUTIL_RETURN_IF_NULL(task_cmd_ptr);

  memset(&app_info, 0x00, sizeof(app_info));

  slot_data_ptr = mmgsdi_util_get_slot_data_ptr(task_cmd_ptr->cmd.cmd.euicc_m2m_get_eid_req.request_header.slot_id);
  ...
  eid_req_ptr = &task_cmd_ptr->cmd.cmd.euicc_m2m_get_eid_req;
...
)
{
  ...
  if (mmgsdi_status == MMGSDI_SUCCESS && data_read_ptr != NULL &&
