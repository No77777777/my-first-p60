...
$Header: //components/rel/uim.mpss/6.1.0/mmgsdi/src/mmgsdi_icc_rsp.c#4 $$ $DateTime: 2023/08/25 04:15:55 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
08/25/23   shp     Fallback to cache if EF_AD FCP is not proper
02/11/20   vdc     Sanity check for pin data
...
06/11/15   av      Clean up mmgsdi routines used while parsing ICC select resp
04/29/15   bcho    Support for App specific cache
04/06/15   ar      Remove duplicate code
02/25/15   tl      Remove ICC seek support
02/20/15   stv     Handling 0x92 0x40 status word in error or failure case
...
-------------------------------------------------------------------------------*/
static void mmgsdi_populate_icc_security_info(
...
)
{
  ...
  pin_len = (int32touint8(protection_ptr->num_protection_pin)
...
static void mmgsdi_populate_icc_sec_attr_from_uim_to_mmgsdi(
  uint8                              access_cond_byte1,
  uint8                              access_cond_byte2,
  uint8                              access_cond_byte3,
...
-------------------------------------------------------------------------------*/
mmgsdi_return_enum_type mmgsdi_icc_rsp_extract_file_attr (
  mmgsdi_slot_id_enum_type       slot,
...
)
{
  ...
  attr_ptr->file_size = ((mmgsdi_file_size_type)sim_header_attr[2] << 8) | 
                         (mmgsdi_file_size_type)sim_header_attr[3];
      ...
      switch(sim_header_attr[13])
      {
        case 0x03: /* CYCLIC FILE */
          ...
          if(!skip_decode_sec_attr)
          {
            ...
            mmgsdi_populate_icc_sec_attr_from_uim_to_mmgsdi(sim_header_attr[8],
                                                            sim_header_attr[9],
                                                            sim_header_attr[10],
...
} /* mmgsdi_icc_rsp_extract_file_attr */
...
)
{
  ...
  /* If the FCP data is FF and if the file is EF_AD then change report status to fallback
     to file fallback */
  if(report_status == MMGSDI_SUCCESS &&
     mmgsdi_nv_get_device_feature_status(MMGSDI_FEATURE_SIM_FILE_FALLBACK_TO_FS) == MMGSDI_FEATURE_ENABLED &&
     extra_param_ptr->get_file_attr_data.access.file.file_enum == MMGSDI_GSM_AD &&
     uim_rsp_ptr->rsp_data.data_len != 0 &&
     uim_rsp_ptr->rsp_data.data_ptr[0] == 0xFF)
  {
     report_status = MMGSDI_NOT_FOUND;
  }
      ...
      req_header.orig_request_type == MMGSDI_GET_FILE_ATTR_REQ)
  {
    ...
    UIM_MSG_HIGH_1("Marked cache to MMGSDI_CACHE_NOT_FOUND due to 0x%x", report_status);
  }
     ...
     req_header.orig_request_type == MMGSDI_GET_ALL_PIN_STATUS_REQ)
  {
    ...
    mmgsdi_icc_rsp_parse_pin(report_status,
                             pin_extra_info_ptr,
                             (mmgsdi_data_type*)&modified_uim_rsp_ptr->rsp_data,
                             sel_app_ptr,
                             req_header.slot_id);
