...
Copyright (c) 2004 - 2017, 2020 - 2021 QUALCOMM Technologies, Inc (QTI) and its licensors.
...
$Header: //components/rel/uim.mpss/6.1.0/mmgsdi/src/mmgsdi_uim_uicc.c#4 $$ $DateTime: 2021/07/28 22:02:47 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
07/29/21   skc     Fix null pointer dereference issue of internal_synch global
...
)
{
  ...
  if(aid_ptr != NULL)
  {
    mmgsdi_memscpy(uim_cmd_ptr->select.aid.data,
                   ...
                   aid_ptr->data_ptr,
                   aid_ptr->data_len);
    uim_cmd_ptr->select.aid.aid_length = aid_ptr->data_len;
  }
    ...
    if (sync_process)
    {
          ...
          mmgsdi_internal_synch_uim_rpt_rsp[mmgsdi_task] != NULL)
      {
        ...
        if(mmgsdi_status != MMGSDI_SUCCESS)
        {
          ...
          {
            MMGSDIUTIL_TMC_MEM_FREE(internal_synch_uim_rpt_rsp_ptr->rsp_data.data_ptr);
...
/* ----------------------------------------------------------------------------
   ...
   LIMITATIONS:
     For ICC protocol access, mmgsdi_uim_icc_write_record should be used
...
)
{
  ...
  uim_cmd_ptr->hdr.slot            = uim_slot;
  uim_cmd_ptr->hdr.channel         = uim_channel_id;
  uim_cmd_ptr->write_rec.rec_num   = uint32touint8(req_ptr->rec_num);
  ...
  uim_cmd_ptr->write_rec.len       = uint32touint16(mmgsdi_write_len);
  ...
  if (aid_ptr != NULL )
  {
    mmgsdi_memscpy(uim_cmd_ptr->write_rec.aid.data,
                   ...
                   aid_ptr->data_ptr,
                   aid_ptr->data_len);
    uim_cmd_ptr->write_rec.aid.aid_length = aid_ptr->data_len;
  }
    ...
    if (sync_process)
    {
          ...
          mmgsdi_internal_synch_uim_rpt_rsp[mmgsdi_task] != NULL)
      {
        ...
        /* It is possible that the global mmgsdi_internal_synch_uim_rpt_rsp
           is freed up already inside mmgsdi_common_rsp_write above.
           That would happen incase of MMGSDI a sending command to card in 
           sync manner inside the function mmgsdi_common_rsp_write.
           so, free it only if it is not freed already */
...
)
{
  ...
  mmgsdi_len_type                    mmgsdi_file_size = 0;
  ...
  if(req_ptr->offset > 0 &&
     mmgsdi_read_len == mmgsdi_file_size)
  {
    mmgsdi_offset = 0;
  }

  if(!mmgsdi_file_transparent_is_access_range_valid(mmgsdi_offset, mmgsdi_read_len))
  {
    return MMGSDI_INCORRECT_PARAMS;
  }
  ...
  uim_cmd_ptr->read_bin.offset     = uint32touint16(mmgsdi_offset);
  uim_cmd_ptr->read_bin.len        = uint32touint16(mmgsdi_read_len);
  ...
  if (aid_ptr != NULL)
  {
    ...
    uim_cmd_ptr->read_bin.aid.aid_length = aid_ptr->data_len;
  }
  ...
  else
  {
    ...
    if (sync_process)
    {
      ...
      {
        mmgsdi_internal_synch_uim_rpt_rsp[mmgsdi_task]->rsp_data_index = index;
        mmgsdi_status = mmgsdi_common_rsp_read(
          mmgsdi_internal_synch_uim_rpt_rsp[mmgsdi_task], cnf_pptr, TRUE, MMGSDI_UICC);
        /* It is possible that the global mmgsdi_internal_synch_uim_rpt_rsp
...
)
{
  ...
  uim_cmd_ptr->hdr.slot            = uim_slot;
  uim_cmd_ptr->hdr.channel         = uim_channel_id;
  uim_cmd_ptr->read_rec.rec_num    = uint32touint8(req_ptr->rec_num);
  uim_cmd_ptr->read_rec.recmode    = UIM_ABSOLUTE;
  uim_cmd_ptr->read_rec.rec_len    = uint32touint8(mmgsdi_read_len);
  ...
  if (aid_ptr != NULL)
  {
    mmgsdi_memscpy(uim_cmd_ptr->read_rec.aid.data,
                   ...
                   aid_ptr->data_ptr,
                   aid_ptr->data_len);
    uim_cmd_ptr->read_rec.aid.aid_length = aid_ptr->data_len;
  }
...
)
{
  ...
  if (mmgsdi_status != MMGSDI_SUCCESS)
  {
    MMGSDIUTIL_TMC_MEM_FREE(extra_param_ptr->increase_data.data.data_ptr);
    MMGSDIUTIL_TMC_MEM_FREE(extra_param_ptr);
    MMGSDIUTIL_TMC_MEM_FREE(uim_cmd_ptr);
    return mmgsdi_status;
  }
  ...
  /* Assign uim_cmd_ptr's data len and data buffer info */
  uim_cmd_ptr->inc.len       = uint32touint16(req_ptr->data.data_len);
  ...
  if (aid_ptr != NULL )
  {
    mmgsdi_memscpy(uim_cmd_ptr->inc.aid.data,
                   ...
                   aid_ptr->data_ptr,
                   aid_ptr->data_len);
    uim_cmd_ptr->inc.aid.aid_length = aid_ptr->data_len;
  }
       ...
       (app_info_ptr != NULL))
    {
          ...
          (aid_ptr->data_len <= UIM_AID_MAX_NAME_LENGTH))
      {
        mmgsdi_memscpy(uim_cmd_ptr->status.aid.data,
                       ...
                       aid_ptr->data_ptr,
                       aid_ptr->data_len);
        uim_cmd_ptr->status.aid.aid_length = aid_ptr->data_len;
      }
    }
  }
...
)
{
  ...
  /* ------------------------------------------------------------------------
    ...
    ----------------------------------------------------------------------*/
  uim_cmd_ptr->autn.aid.aid_length = aid_ptr->data_len;
  ...
  switch(req_ptr->auth_context)
  {
    ...
    case MMGSDI_AUTN_USIM_VGCS_VBS_CONTEXT:
      ...
      uim_cmd_ptr->autn.num_bytes = uint32touint8(req_ptr->auth_req.data_len);
    ...
    case MMGSDI_AUTN_USIM_GBA_BOOTSTRAPPING_CONTEXT:
      ...
      uim_cmd_ptr->autn.num_bytes = uint32touint8(req_ptr->auth_req.data_len + 1);
    ...
    case MMGSDI_AUTN_USIM_MBMS_MUK_DEL_CONTEXT:
      ...
      -----------------------------------------------------------------------*/
        uint32 ber_len             = req_ptr->auth_req.data_len + 1; /*1 for the Tag*/
        uint8  buff_offset         = 0;
        uint8  ber_data[4];
        ...
        uim_cmd_ptr->autn_odd_ins.len += uint32touint16(req_ptr->auth_req.data_len + 2);
    ...
    case MMGSDI_AUTN_USIM_MBMS_REC_DEL_CONTEXT:
      ...
      -----------------------------------------------------------------------*/
        uint32 total_ber_len        = req_ptr->auth_req.data_len +
        ...
        uim_cmd_ptr->autn_odd_ins.data[buff_offset++] = uint32touint8
    ...
    case MMGSDI_AUTN_ISIM_HTTP_DIGEST_CONTEXT:
      ...
      /* ------------------------------------------------------------------------
         ...
         ----------------------------------------------------------------------*/
      uim_cmd_ptr->isim_autn.aid.aid_length = aid_ptr->data_len;
      ...
      uim_cmd_ptr->isim_autn.num_bytes = uint32touint8(req_ptr->auth_req.data_len);
  ...
  mmgsdi_logging_populate_masking_fn_ptr(uim_cmd_ptr, MMGSDI_APP_USIM);
...
)
{
 ...
 ----------------------------------------------------*/
  ...
  uim_cmd_ptr->seek.rec_number             = uint32touint8(req_ptr->rec_num);
  ...
  if(aid_ptr != NULL)
  {
    mmgsdi_memscpy(uim_cmd_ptr->seek.aid.data,
                   ...
                   aid_ptr->data_ptr,
                   aid_ptr->data_len);
    uim_cmd_ptr->seek.aid.aid_length = aid_ptr->data_len;
  }
...
)
{
  ...
  if (aid_ptr != NULL)
  {
    if (req_type == MMGSDI_REHABILITATE_REQ)
    {
      ...
      uim_cmd_ptr->rehab.aid.aid_length = aid_ptr->data_len;
    }
    else
    {
      ...
      uim_cmd_ptr->invalidate.aid.aid_length = aid_ptr->data_len;
    }
  }
...
/*===========================================================================
FUNCTION:  MMGSDI_UIM_UICC_SET_APDU_BEHAVIOR

DESCRIPTION
     This function prepares the set APDU behavior command and send it to UIM
     for further processing
...
RETURN VALUE
     ...
     MMGSDI_INCORRECT_PARAMS: The parameters supplied to the API are not
                              within appropriate ranges.
===========================================================================*/
mmgsdi_return_enum_type mmgsdi_uim_uicc_set_apdu_behavior(
  const mmgsdi_set_apdu_behavior_req_type* req_ptr)
{
  ...
  */
   ...
   * Populate UIM information.
   */
  uim_cmd_ptr->hdr.command  = UIM_SET_APDU_BEHAVIOR_F;
  uim_cmd_ptr->hdr.protocol = UIM_UICC;
  uim_cmd_ptr->hdr.slot     = uim_slot;
  uim_cmd_ptr->hdr.channel  = (uim_channel_type)req_ptr->channel_id;
  ...
  /*
   * Set APDU behavior flag.
   */
  switch ( req_ptr->apdu_behavior )
  {
    case MMGSDI_APDU_BEHAVIOR_NO_GET_RESPONSE_FOR_WARNING:
      uim_cmd_ptr->set_apdu_behavior.apdu_behavior = UIM_APDU_BEHAVIOR_NO_GET_RESPONSE_FOR_WARNING;
      break;
      
    case MMGSDI_APDU_BEHAVIOR_AUTOMATIC_GET_RESPONSE_FOR_WARNING:
      uim_cmd_ptr->set_apdu_behavior.apdu_behavior = UIM_APDU_BEHAVIOR_AUTOMATIC_GET_RESPONSE_FOR_WARNING;
      break;
  ...
  UIM_MSG_HIGH_3("Sending down set APDU behavior command to UIM slot : 0x%x channel_id : 0x%x apdu_behavior : 0x%x",
                  uim_slot, 
                  req_ptr->channel_id, 
                  req_ptr->apdu_behavior);
  ...
  return mmgsdi_status;
} /* mmgsdi_uim_uicc_set_apdu_behavior */
