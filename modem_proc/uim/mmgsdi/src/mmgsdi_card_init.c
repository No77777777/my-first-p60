...
Copyright (c) 2011 - 2017, 2019 - 2021, 2024 QUALCOMM Technologies, Inc (QTI) and its licensors.
...
$Header: //components/rel/uim.mpss/6.1.0/mmgsdi/src/mmgsdi_card_init.c#13 $$ $DateTime: 2024/04/24 21:49:05 $

when       who     what, where, why
--------   ---     ------------------------------------------------------------
04/23/24   pg     Dont switch protocol to ICC if USIM app is present in EF-DIR
02/15/24   cj      Hide CSIM from indication when CDMA_LESS NV is enabled
12/03/18   tq      Making EF_PSLOCI and EF_FPLMN as optional file for buggy card
03/11/20   vgd     Do not switch protocol to ICC if toolkit is disabled
...
02/12/20   sch     Handle invalid length in EF_IMSI from the GSM DF
01/02/20   sch     Added support for log masking
12/28/19   sg      Added support for NS+SP category
12/09/19   tq      Read & cache GID1 and GID2 in all cases during initialization.
09/19/19   sg      Fallback to file system/cache for missing files
09/17/19   mm      Add EID to SimSlotStatus radio config HAL
07/12/19   tq      Send FDN event during UICC init
...
01/03/18   ra      Remove unwanted updating of session closed cause
                   while initializing EF-PRL cache data
09/28/17   me      Ignore cache attr read failure if persistance cache is active  
09/06/17   ra      Read SPN file during CSIM initialization
07/10/17   av      Support for auto-activation of apps for all customers
06/09/17   bcho    Delete cache if any before write it back with default data
04/24/17   nr      Support to store more FPLMNs in cache for MMGSDI_USIM_FPLMN
04/13/17   me      Support to disable GBA_U service
04/12/17   av      Fix featurization controlling app auto-activation
03/30/17   av      Update EF-AD for cards with incorrectly configured MNC len
03/30/17   tq      Corrected logging
03/06/17   me      Added check for invalid ef ehplmn length
...
01/05/17   vdc     Consider only ESN len nibble for calculation of ESN len
01/03/17   sk      Removed USIM spec version
12/28/16   ll      Add test mode logging enabled APIs for sensitive info control
12/20/16   vdc     Added support to handle incorrectly configured EF IMSI
11/25/16   vdc     Check for ESN data length from card and in device
11/13/16   bcho    Return different error if sync command cancelled
11/09/16   vdc     Compare ESN data from EF ESN from second byte
10/25/16   me      Removed all the function under FEATURE_GSTK and change the prototype
10/24/16   vdc     Added support for auto reg SMS when ESN is changed
10/06/16   ar      Added support for SP + EHPLMN simlock
09/28/16   bcho    Read EF-RUIMID again after PIN1 verification
09/14/16   tkl     Support PSM card detected mode
09/01/16   dd      UT changes for if FEATURE_CUST_1 enabled
08/30/16   bcho    Optimization of MMGSDI attribute cache access functions
08/18/16   bcho    Disable GSM KCGPRS SRV when EF-KCGPRS is not present on card
08/10/16   kv      Handle read failures for AUTH related EFs
...
05/10/16   ar      Clear E/PRL cache if the sync_cmd is cancelled
...
03/23/16   lm      Local memory allocation is freed without any conditional check
03/21/16   av      Validate actual IMSI data length
03/15/16   vr      SAM CARD support
02/29/16   bcho    Delete attr cache while deleting cache of EPSLOCI and EPSNSC
02/12/16   vdc     Halt init if EF is missing or not configured at init time
...
01/22/16   vdc     Added new API to provide MCC and MNC values
...
01/12/16   bcho    Make CSIM AD an optional EF as it is not being used
...
10/22/15   vv      Set the appropriate return status after reading PRL
...
08/26/15   ar      Moving mandatory flag for EF-KEYS from TRUE to NV dependent
08/25/15   av      Move feature flag specific comment inside the feature flag
...
07/09/15   yt      Perform ENS operations during session activated processing
07/07/15   vv      Read GID files based on SP or CP lock status
06/26/15   kk      Update efs context parameter in uim common efs API call
...
06/12/15   bcho    NULL pointer check updated
05/13/15   yt      Ignore all init errors for EPSLOCI and EPSNSC
04/30/15   nr      Enhancements in handling of GCF anite flag update
04/29/15   bcho    Support for App specific cache
04/27/15   av      Support for hiding CSIMs from modem clients
04/27/15   vdc     Added support for life cycle TLV's incorrect length
...
01/06/15   vdc     Delete EPSLOCI & EPSNSC cache when EPS MMI srv is disabled
...
/* Default Size of USIM EF KC/KCGPRS data in bytes */
#define MMGSDI_USIM_GSM_ACCESS_KEYS_LEN      9
...
static const uint8  
  mmgsdi_file_usim_keys_attr[]                   = {0x62,0x3D,
...
static const uint8
  mmgsdi_file_usim_keysps_attr[]                 = {0x62,0x3D,
                                                   /* File Descriptor: Transparent file */
                                                   0x82,0x02,0x01,0x21,
                                                   /* File Identifier */
                                                   0x83,0x02,0x6F,0x09,
...
/* GSM EF-KC raw attr info */
static const uint8  mmgsdi_file_gsm_kc_attr[]    = {0x00,0x00,0x00,0x09,0x6F,0x20,0x04,0x00,0x11,0x00,0x44,0x21,0x02,0x00,0x00};
  ...
  mmgsdi_key_ref_values_enum_type slot_pin2_key_ref)
{
  ...
  mmgsdi_card_init_app_tp_state_init(slot_data_ptr->app_info_pptr[app_index]);
...
)
{
  ...
  if (mmgsdi_card_init_is_toolkit_allowed(MMGSDI_SESSION_ID_ZERO, slot_id, MMGSDI_APP_USIM))
  {
    /* Do a Terminal Profile Download */
    mmgsdi_card_init_gstk_terminal_profile_download(MMGSDI_SESSION_ID_ZERO,
                                                    slot_id,
                                                    MMGSDI_APP_USIM);
  ...
  {
    /* Set tp_state to MMGSDI_TP_STATE_NOT_NEEDED as Toolkit is disabled. */
    ...
    if((mmgsdi_status == MMGSDI_SUCCESS) && (num_apps > 0))
    {
      ...
      if(mmgsdi_util_get_csim_apps(slot_id))
      {
        ...
        nvruim_set_uim_dir_present(NVRUIM_CDMA_SUBSCRIPTION_PRESENT, slot_id);
      }
#endif /* FEATURE_MMGSDI_3GPP2 */
...
)
{
  ...
  /* copy the slot type */
  slot_data_ptr->slot_type = link_est_data_ptr->slot_type;

  /* copy the transmission mode */
  slot_data_ptr->transmission_mode = link_est_data_ptr->transmission_mode;

  /* copy the slot type */
  slot_data_ptr->card_type         = link_est_data_ptr->card_type;
  ...
  {
    /* In case of non telecom card set protocol as UICC */
  ...
  /* Copy EID */
  if(link_est_data_ptr->eid_info.length > 0)
  {
    MMGSDIUTIL_ENTER_CLIENT_APP_DATA_CRIT_SECT;
    mmgsdi_memscpy(&slot_data_ptr->eid_info,
	               sizeof(slot_data_ptr->eid_info),
				   &link_est_data_ptr->eid_info,
				   sizeof(mmgsdi_eid_info_type));
    MMGSDIUTIL_LEAVE_CLIENT_APP_DATA_CRIT_SECT;
  }
  ...
  if(nv_refresh_enabled)
  {
    ...
    if(slot_data_ptr->nv_ref_app_deact_count == 0)
    {
      mcfg_refresh_status = mcfg_refresh_update_from_uim (&mcfg_uim_data);
  ...
  switch (link_est_data_ptr->protocol)
  {
    case MMGSDI_ICC:
      ...
      */
      ...
      if (mmgsdi_nv_get_slot_feature_status(MMGSDI_FEATURE_UIM_GSM_DCS_1800, nv_context) ==
    ...
    case MMGSDI_UICC:
      /* Read the Language Preference EF only for telecom cards */
      if(slot_data_ptr->card_type == MMGSDI_CARD_TELECOM)
      {
        (void) mmgsdi_card_init_cache_binary(card_session_id, MMGSDI_APP_USIM,
                                             slot_id, MMGSDI_ELP, 0, NULL);
      }
      ...
      /* TP is applicable only for telecom cards */
      if (slot_data_ptr->card_type == MMGSDI_CARD_TELECOM)
      {
  ...
  /* In case of non telecom card don't switch to ICC protocol. Also, if card
     is eUICC then no need to send RESET commnd to card as spec doesn't
     mandate a default profile to be present in card */
  if (slot_data_ptr->card_apps_available != 0x00 ||
      slot_data_ptr->card_type == MMGSDI_CARD_NON_TELECOM ||
      slot_data_ptr->is_euicc)
  {
    /* Valid applications were found or a non telecom card */
    mmgsdi_status = MMGSDI_SUCCESS;
  }
  else if(mmgsdi_card_init_is_toolkit_allowed(card_session_id, slot_id,
                                              MMGSDI_APP_USIM) == FALSE &&
                              link_est_data_ptr->protocol == MMGSDI_UICC &&
                                              fail_reason_card_error == FALSE &&
                                              mcfg_refresh_status == FALSE)
  {
    ...
    /* If mcfg_refresh_status is TRUE it means that MMGSDI is waiting for
       continue_after_nv_refresh request from MCFG. MMGSDI will
       read EF-DIR to check if any applications are available and
       decide if protocol need to be switched to ICC in
       continue_after_nv_refresh handling, so no need to switch protocol
       here. */
...
)
{
  ...
  /* For few buggy cards, EF-RUIMID can be PIN1 protected. It would be read
     after PIN1 verification again */
  if (mmgsdi_status != MMGSDI_SUCCESS && mmgsdi_status != MMGSDI_ACCESS_DENIED)
  {
    return mmgsdi_status;
  }
...
)
{
  ...
  if(app_type == MMGSDI_APP_USIM)
  {
    ...
    mmgsdi_status = mmgsdi_cache_init_usim_ecc(session_id);
    if(mmgsdi_status != MMGSDI_SUCCESS)
    {
      ...
      imsi_data.data_ptr[3] == MMGSDI_HZI_IMSI_THIRD_BYTE_ALT))
  {
       ...
       (hzi_data.data_len > 0))
    {
      ...
      else if (hzi_data.data_len >= 2)
      {
        ...
        mmgsdi_status = mmgsdi_uim_common_write_transparent(&write_req, TRUE, &cnf_ptr, protocol);
...
)
{
  ...
  /* Use MCFG API to read GCF NV 947. Update the GCF NV item, if value of the subscription is different. */
  if (mcfg_gcf_nv_get_status() != app_info_ptr->gprs_anite_mcc)
  {
...
/* ==========================================================================
   FUNCTION:      MMGSDI_CARD_INIT_IS_MNC_LEN_INCORRECTLY_CONFIGURED

   DESCRIPTION:
     Cards with certain IMSIs are known to have incorrectly configured MNC
     length in EF-AD as 2 bytes, but the actual MNC on that card is 3 btyes.
     This function is used to identify such cards (based on IMSI).
   ...
   SIDE EFFECTS:
     None
==========================================================================*/
static boolean mmgsdi_card_init_is_mnc_len_incorrectly_configured(
  mmgsdi_data_type   imsi_data
)
{
  uint32 mcc_mnc_value                         = 0;
  uint8  i                                     = 0;
  /* The following list is compiled from a customer case where cards with
     these MCC-MNCs were identified by the customer to be incorrectly
     configured */
  uint32 incorrectly_configured_mcc_mnc_list[] =
           {405840,
            405854, 405855, 405856, 405857, 405858, 405859,
            405860, 405861, 405862, 405862, 405864, 405865,
            405866, 405867, 405868, 405869,
            405870, 405871, 405872, 405873, 405874,
            334050};

  if(imsi_data.data_len < MMGSDI_IMSI_MNC_LAST_DIGIT_POS ||
     imsi_data.data_ptr == NULL)
  {
    /* Does not contain enough digits that are needed to decode MCC and MNC.
       Lets assume this is a good card, with correctly configured MNC len */
    return FALSE;
  }

  mcc_mnc_value  = (uint32)((imsi_data.data_ptr[MMGSDIUTIL_IMSI_DIGIT_1_OFFSET] >> 4) &
                MMGSDIUTIL_IMSI_LOW_DIGIT_MASK) * 100000;
  mcc_mnc_value += (uint32)((imsi_data.data_ptr[MMGSDIUTIL_IMSI_DIGIT_2_OFFSET]) &
                MMGSDIUTIL_IMSI_LOW_DIGIT_MASK) * 10000;
  mcc_mnc_value += (uint32)((imsi_data.data_ptr[MMGSDIUTIL_IMSI_DIGIT_2_OFFSET] >> 4) &
                MMGSDIUTIL_IMSI_LOW_DIGIT_MASK) * 1000;

  mcc_mnc_value += (uint32)((imsi_data.data_ptr[MMGSDIUTIL_IMSI_DIGIT_3_OFFSET]) &
                MMGSDIUTIL_IMSI_LOW_DIGIT_MASK) * 100;
  mcc_mnc_value += (uint32)((imsi_data.data_ptr[MMGSDIUTIL_IMSI_DIGIT_3_OFFSET] >> 4) &
                MMGSDIUTIL_IMSI_LOW_DIGIT_MASK) * 10;
  mcc_mnc_value += (uint32)((imsi_data.data_ptr[MMGSDIUTIL_IMSI_DIGIT_4_OFFSET]) &
                MMGSDIUTIL_IMSI_LOW_DIGIT_MASK);

  for (i = 0; i < sizeof(incorrectly_configured_mcc_mnc_list) / sizeof(incorrectly_configured_mcc_mnc_list[0]); i++)
  {
    if (mcc_mnc_value == incorrectly_configured_mcc_mnc_list[i])
    {
      return TRUE;
    }
  }
  ...
  return FALSE;
} /* mmgsdi_card_init_is_mnc_len_incorrectly_configured */
...
DESCRIPTION
  This function updates the fourth byte data of EF AD to 0x2 or 0x3 in cache
  when the EF AD only has 3 bytes data.
...
)
{
  ...
  boolean                            is_mnc_len_incorrectly_configured  = FALSE;
  ...
  mmgsdi_status = mmgsdi_util_read_cache_file(session_id,
                                              &ad_file_access,
                                              &ad_data);
  ...
  is_mnc_len_incorrectly_configured =
    mmgsdi_card_init_is_mnc_len_incorrectly_configured(imsi_data);
  ...
  if(is_mnc_len_incorrectly_configured)
  {
    num_of_mnc_digit = MMGSDIUTIL_MNC_3_DIGITS;
  }
  else
  {
    mmgsdi_status = mmgsdi_util_get_num_of_mnc_digits(imsi_data,
                                                      ad_data,
                                                      &num_of_mnc_digit);
  ...
  if (ad_data.data_len != new_ad_data.data_len)
  {
    ...
    memset(&security_access, 0x0, sizeof(mmgsdi_file_security_access_type));

    /* Sending of SELECT is mandatory here, because it is possible that file attributes
       can be returned from static table. So in that case, the mmgsdi_cache_attr_read will fail.
       But in-order to update the correct AD-LEN in attr, raw attributes in needed. So mandating
       select helps to get raw attributes in cache */
    ...
    {
      mmgsdi_decoded_attr_cache_type  *attr_cache_temp_ptr     = NULL;
      ...
      do
      {
        ...
        mmgsdi_status = mmgsdi_cache_update_file_attributes(session_id,
        ...
        (void)mmgsdi_cache_attr_write(session_id,
                                      &ad_file_access,
                                      &security_access,
                                      FALSE,
                                      0,
      ...
      MMGSDIUTIL_TMC_MEM_FREE(attr_cache_temp_ptr);
      MMGSDIUTIL_TMC_MEM_FREE(raw_file_attr_data.data_ptr);
    }
    MMGSDIUTIL_TMC_MEM_FREE(attr_cache_ptr);
    mmgsdi_file_free_security_info(&security_access);
  }
...
/*=============================================================================
FUNCTION:      MMGSDI_CARD_INIT_UPDATE_IMSI_DATA

DESCRIPTION
  This function updates the IMSI data in the cache with the data input to
  this function
...
SIDE EFFECTS
  NONE
=============================================================================*/
static mmgsdi_return_enum_type mmgsdi_card_init_update_imsi_data (
  mmgsdi_session_id_type          session_id,
  mmgsdi_data_type                update_data
)
{
  mmgsdi_int_app_info_type          *app_info_ptr     = NULL;
  mmgsdi_return_enum_type            mmgsdi_status    = MMGSDI_ERROR;
  ...
  if(update_data.data_ptr == NULL ||
     update_data.data_len != (MMGSDI_MAX_VALID_IMSI_LEN + 1))
  {
    return MMGSDI_ERROR;
  }

  memset(&file_access, 0x0, sizeof(file_access));
  memset(&security_access, 0x0, sizeof(mmgsdi_file_security_access_type));

  /* Get the app and slot related to the session */
  mmgsdi_status = mmgsdi_util_get_session_app_info(session_id,
                                                   NULL,
                                                   NULL,
                                                   &app_info_ptr,
  ...
  {
    file_access.file.file_enum = MMGSDI_GSM_IMSI;
  }
  else
  {
    file_access.file.file_enum = MMGSDI_USIM_IMSI;
  }
  ...
  if (mmgsdi_status != MMGSDI_SUCCESS)
  {
    ...
    /* We shoudln't have reached here since attributes should already have
       been cached by this time... return error */
    return mmgsdi_status;
  }

  /* Update EF IMSI size in attr cache (basically we want to reflect the new
     EF IMSI size in the attr cache correctly) 
     ...
     select helps to get raw attributes in cache */
  if (attr_cache_ptr != NULL &&
      attr_cache_ptr->file_size != update_data.data_len)
  {
    mmgsdi_decoded_attr_cache_type *attr_cache_temp_ptr = NULL;
    mmgsdi_get_file_attr_data_type  raw_file_attr_data;
  ...
  /* Delete already existing EF IMSI cache if any */
  mmgsdi_status = mmgsdi_cache_delete_item(session_id,
                                           1,
  ...
  if (mmgsdi_status == MMGSDI_SUCCESS)
  {
    /* Write the new data to cache as if it came from the card. */
    mmgsdi_status = mmgsdi_cache_write(session_id,
                                       &file_access,
                                       update_data,
                                       0,
  ...
  return mmgsdi_status;
} /* mmgsdi_card_init_update_imsi_data */
...
)
{
  ...
  write_data.data_ptr = data_ptr->data_ptr;
  
  /* Validate actual length of the data in EFimsi */
  if (data_ptr->data_len > (MMGSDI_MAX_VALID_IMSI_LEN + 1) ||
      data_ptr->data_ptr[0] == MMGSDI_BUGGY_IMSI_LEN)
  {
    /* Actual length of data in EFimsi is greater than the max IMSI size of 9
       bytes (as per the spec). Lets truncate the IMSI data. */

    write_data.data_len = MMGSDI_MAX_VALID_IMSI_LEN + 1;

    /* Update the IMSI length */
    if(data_ptr->data_ptr[0] == MMGSDI_BUGGY_IMSI_LEN)
    {
      write_data.data_ptr[0] = imsi_len;
    }

    status = mmgsdi_card_init_update_imsi_data(session_id, write_data);
  ...
  status = mmgsdi_util_convert_to_imsi_data_type(&write_data, &imsi);
  if(status != MMGSDI_SUCCESS)
  {
  ...
  (void)mmgsdi_card_init_update_ad_mnc_len(session_id, write_data);
  ...
  /* Updating is_test_mode_logging_enabled flag based on the EFS item settings and
     reserved set of MCC table */
  if (FALSE == mmgsdi_is_test_mode_logging_enabled.secure_logging_enabled ||
  	  FALSE == mmgsdi_is_test_mode_logging_enabled.private_logging_enabled)
  {
    mmgsdi_session_type_enum_type session_type;
...
/*===========================================================================
FUNCTION:      MMGSDI_CARD_INIT_DISABLE_SRV_IN_ST

DESCRIPTION
  This function is called to disable any service in GSM/USIM Service Table
...
SIDE EFFECTS
  None
===========================================================================*/
static mmgsdi_return_enum_type mmgsdi_card_init_disable_srv_in_st (
  mmgsdi_session_id_type   session_id,
...
)
{
  ...
  {
    if ((service_to_disable & 0xFF00) != MMGSDI_GSM_SRV)
    {
      return MMGSDI_ERROR;
    }

    access.file.file_enum = MMGSDI_GSM_SST;
    service = service_to_disable - MMGSDI_GSM_SRV;
    offset = service / 4;
    shift = (uint8)((service % 4) * 2);
  ...
  if(offset < data.data_len)
  {
    ...
    /* Update cache to indicate that service is not available */
    mmgsdi_status = mmgsdi_cache_write(session_id,
                                       &access,
                                       data,
                                       0,
  ...
  MMGSDIUTIL_TMC_MEM_FREE(data.data_ptr);
  return mmgsdi_status;
}/* mmgsdi_card_init_disable_srv_in_st */


/*===========================================================================
FUNCTION:      MMGSDI_CARD_INIT_DISABLE_SRV_DELETE_CACHE

DESCRIPTION
  This function is called to disable service when file is present and file
  length is not expected or invalid, and then delete the file content
  from the cache.
...
SIDE EFFECTS
  None
===========================================================================*/
static mmgsdi_return_enum_type mmgsdi_card_init_disable_srv_delete_cache
(
  mmgsdi_session_id_type        session_id,
...
)
{
  ...
  mmgsdi_status = mmgsdi_card_init_disable_srv_in_st(session_id,
                                                     service,
                                                     app_type);
  if (mmgsdi_status != MMGSDI_SUCCESS)
  {
  ...
  /* If service is updated successfully to cache then delete the EF data
     from cache */
  ...
  return MMGSDI_SUCCESS;
}/* mmgsdi_card_init_disable_srv_delete_cache */
...
)
{
  ...
  UIM_MSG_HIGH_1("mmgsdi_card_init_validate_epsnsc slot 0x%x", slot_id);
       ...
       data_ptr->data_len < MMGSDI_EF_EPSNSC_LEN))
  {
    ...
    return mmgsdi_card_init_disable_srv_delete_cache(
                                                   session_id,
                                                   MMGSDI_USIM_SRV_EPS_MMI,
                                                   MMGSDI_APP_USIM,
                                                   mmgsdi_file,
                                                   sizeof(mmgsdi_file) / sizeof(mmgsdi_file[0]));
  }
  return MMGSDI_SUCCESS;
...
)
{
  ...
  UIM_MSG_HIGH_1("mmgsdi_card_init_validate_epsloci slot 0x%x", slot_id);
...
/*=============================================================================
FUNCTION:      MMGSDI_CARD_INIT_PURGE_TEMPORARY_IDENTITY

DESCRIPTION
  This is the function for purging temporary identity during app init
  if purging is pending for particual session.
...
SIDE EFFECTS
  NONE
=============================================================================*/
static void mmgsdi_card_init_purge_temporary_identity (
...
{
  mmgsdi_purge_temporary_identity_req_type   purge_cmd_detail;
  mmgsdi_return_enum_type                    mmgsdi_status      = MMGSDI_SUCCESS;
  mmgsdi_int_app_info_type                 * prov_app_info_ptr  = NULL;

  memset(&purge_cmd_detail, 0x00, sizeof(purge_cmd_detail));
  ...
  if(mmgsdi_file == MMGSDI_USIM_EPSLOCI)
  {
    (void) mmgsdi_util_purge_temp_identities(slot,
                                             session_id,
                                             MMGSDI_USIM_EPSLOCI,
                                             MMGSDI_UICC);
  }
  else if(mmgsdi_file == MMGSDI_USIM_EPSNSC)
  {
    (void) mmgsdi_util_purge_temp_identities(slot,
                                             session_id,
                                             MMGSDI_USIM_EPSNSC,
    ...
    /* storing session info which we will extract again NAS send purge status */
    purge_cmd_detail.request_header.session_id = session_id;
    purge_cmd_detail.request_header.slot_id = slot;
    purge_cmd_detail.temporary_identity = MMGSDI_PURGE_TEMPORARY_IDENTITY_GUTI;

    mmgsdi_status = mmgsdi_send_purge_req_to_nas(&purge_cmd_detail);

    /* Sending event if status is not SUCCESS */
    if(mmgsdi_status != MMGSDI_SUCCESS)
    {
      ...
      mmgsdi_evt_build_and_send_purge_temporary_identity_status(session_id,
                                                                MMGSDI_PURGE_TEMPORARY_IDENTITY_GUTI,
                                                                MMGSDI_ERROR,
                                                                slot);
    }
  }
} /* mmgsdi_card_init_purge_temporary_identity */
...
)
{
  ...
  /* If the KC/KCGPRS read fails and service is enabled, disable the GSM access service n27 in UST */
  if (mmgsdi_status != MMGSDI_SUCCESS &&
      mmgsdi_status != MMGSDI_NOT_SUPPORTED &&
      !mmgsdi_is_sync_cmd_canceled(slot_id))
  {
    UIM_MSG_HIGH_1("mmgsdi_card_init_validate_kc_kcgprs - disable n27 SRV in UST on slot 0x%x",
                   slot_id);

    return mmgsdi_card_init_disable_srv_in_st(session_id,
                                              MMGSDI_USIM_SRV_GSM_ACCESS,
                                              MMGSDI_APP_USIM);
  }

  /* in case if the file is present and the file length is smaller 
     than valid length then disable the service in ST */
  if (mmgsdi_status == MMGSDI_SUCCESS && 
      data_ptr != NULL &&
      data_ptr->data_len < MMGSDI_USIM_GSM_ACCESS_KEYS_LEN)
  {
    mmgsdi_file_enum_type mmgsdi_file[2] = {MMGSDI_USIM_KCGPRS,
                                            MMGSDI_USIM_KC};

    return mmgsdi_card_init_disable_srv_delete_cache(session_id,
                                                     MMGSDI_USIM_SRV_GSM_ACCESS,
                                                     MMGSDI_APP_USIM,
                                                     mmgsdi_file,
                                                     sizeof(mmgsdi_file) / sizeof(mmgsdi_file[0]));
...
)
{
  ...
  /* If the GSM KCGPRS read fails and service is enabled, disable the service n38 in SST */
  if (mmgsdi_status != MMGSDI_SUCCESS &&
      mmgsdi_status != MMGSDI_NOT_SUPPORTED &&
      !mmgsdi_is_sync_cmd_canceled(slot_id))
  {
    UIM_MSG_HIGH_1("mmgsdi_card_init_validate_gsm_kcgprs - disable n38 SRV in SST on slot 0x%x",
                   slot_id);
    return mmgsdi_card_init_disable_srv_in_st(session_id,
                                              MMGSDI_GSM_SRV_GPRS,
                                              MMGSDI_APP_SIM);
  ...
  if (mmgsdi_status == MMGSDI_SUCCESS && 
      data_ptr != NULL &&
      data_ptr->data_len < MMGSDI_GSM_EF_KEYS_LEN)
  {
    mmgsdi_file_enum_type mmgsdi_file[2] = {MMGSDI_GSM_KCGPRS, MMGSDI_GSM_LOCIGPRS};
...
/*===========================================================================
FUNCTION:      MMGSDI_CARD_INIT_DISABLE_EHPLMN_SRV

DESCRIPTION
  This function is called to disable EF EHPLMN service when EF-EHPLMN length is
  not 28.
...
SIDE EFFECTS
  None
===========================================================================*/
static mmgsdi_return_enum_type mmgsdi_card_init_validate_ehplmn(
  mmgsdi_session_id_type  session_id,
  mmgsdi_return_enum_type mmgsdi_status,
  const mmgsdi_data_type *data_ptr
)
{
  mmgsdi_return_enum_type    result         = MMGSDI_SUCCESS;
  mmgsdi_slot_id_enum_type   slot_id        = MMGSDI_SLOT_NONE;
  boolean                    disable_ehplmn = FALSE;
  ...
  /* According to TS 31.102, the length of EF-EHPLMN must be 3n */
  if (data_ptr != NULL && (data_ptr->data_len % 3) != 0)
  {
    disable_ehplmn = TRUE;
  }

  /* If access to EF-EHPLMN failed, even if the service was enabled, we
     need to disable the service in the UST */
  if (mmgsdi_status != MMGSDI_SUCCESS &&
      mmgsdi_status != MMGSDI_NOT_SUPPORTED &&
      !mmgsdi_is_sync_cmd_canceled(slot_id))
  {
  ...
  if (disable_ehplmn)
  {
    mmgsdi_file_enum_type         mmgsdi_file[1] = {MMGSDI_USIM_EHPLMN};

    result = mmgsdi_card_init_disable_srv_delete_cache(
                                                   session_id,
                                                   MMGSDI_USIM_SRV_EHPLMN,
                                                   MMGSDI_APP_USIM,
                                                   mmgsdi_file,
                                                   sizeof(mmgsdi_file) / sizeof(mmgsdi_file[0]));
  }
  return result;
}/* mmgsdi_card_init_validate_ehplmn */


/*=============================================================================
FUNCTION:      MMGSDI_CARD_INIT_VALIDATE_USIM_UST

DESCRIPTION
  This is the validation function for updating USIM UST EF
...
SIDE EFFECTS
  NONE
=============================================================================*/
static mmgsdi_return_enum_type mmgsdi_card_init_validate_usim_ust(
  mmgsdi_session_id_type  session_id,
  mmgsdi_return_enum_type mmgsdi_status,
  const mmgsdi_data_type *data_ptr
)
{
  ...
  /* Check the mmgsdi nv slot pecific features for the gba_u service
     and set the service in UST table accordignaly.
   - if the gba_u service NV is enabled and the service in the card
     is enabled then disable the gba_u service. */
  if (mmgsdi_status == MMGSDI_SUCCESS &&
     (MMGSDI_FEATURE_ENABLED == mmgsdi_nv_get_slot_feature_status(
                                          MMGSDI_FEATURE_DISABLE_GBA_U, nv_context)))
  {
    return mmgsdi_card_init_disable_srv_in_st(session_id,
                                              MMGSDI_USIM_SRV_GBA,
                                              MMGSDI_APP_USIM);
  ...
  return MMGSDI_SUCCESS;
} /* mmgsdi_card_init_validate_usim_ust */
...
)
{
  ...
  for (i = 0; i < num_entry; i++)
  {
    ...
    /* Calling either mmgsdi_card_init_cache_binary or
       ...
       mmgsdi_card_init_select_and_validate or
    ...
    if (app_init_table_ptr[i].mandatory)
    {
      if (mmgsdi_status != MMGSDI_SUCCESS)
      {
        UIM_MSG_ERR_2("Init file 0x%x failed with mmgsdi status 0x%x. Halting initialization",
                      app_init_table_ptr[i].mmgsdi_file, mmgsdi_status);

        MMGSDIUTIL_TMC_MEM_FREE(returned_data.data_ptr);
        return MMGSDI_HALT_INIT_ERROR;
      }
    }
    else
    {
        ...
        /* do not reset status here because validation function may require the
           info */
               ...
               (app_init_table_ptr[i].mmgsdi_file == MMGSDI_USIM_EPSLOCI ||
                ...
                app_init_table_ptr[i].mmgsdi_file == MMGSDI_USIM_EHPLMN))
      {
        /* Ignore errors for these files to allow the validation function
           to disable the service or update the cache and continue initialization */
               ...
               && mmgsdi_nv_get_device_feature_status(MMGSDI_FEATURE_IGNORE_INIT_ERROR) == MMGSDI_FEATURE_DISABLED)
      {
        /* return halt init error to halt the initialization */
        MMGSDIUTIL_TMC_MEM_FREE(returned_data.data_ptr);
        return MMGSDI_HALT_INIT_ERROR;
      }
    }
    ...
    if(mmgsdi_status != MMGSDI_SUCCESS)
    {
      UIM_MSG_ERR_0("Halting initialization due to validation error");
      return MMGSDI_HALT_INIT_ERROR;
    }

    /* If purge guti request is pending then purge the EPSLOCI, EPSNSC and
        call  NAS API for purge */
    if(prov_app_info_ptr->pending_purge_guti)
    {
      mmgsdi_card_init_purge_temporary_identity(app_init_table_ptr[i].mmgsdi_file, app_type, session_id, slot);
    }
  }
  return MMGSDI_SUCCESS;
...
)
{
     ...
     read_cnf_ptr->response_header.mmgsdi_status == MMGSDI_SUCCESS)
  {
    ...
    /* If both ESNs match then update ESN change flag to "NO_CHANGE"
       ...
       Note: when the ESN is stored with the STORE ESN_ME command, the ESN length
       is coded in the least significant nibble, but b5 must be set to 1. Some
       non-compliant cards don't store the ESN length correctly, copying also
       the b5: for this reason, we need to consider only the least significant
       nibble during the comparison.*/
    if(esn_meid_info.esn_length == (read_cnf_ptr->read_data.data_ptr[0] & 0x0F) &&
...
)
{
  ...
  if ((mmgsdi_status == MMGSDI_SUCCESS) && data_buf.data_ptr)
  {
    ...
    nvruim_set_esn_usage(ruim_usage_type,
                         mmgsdi_util_map_mmgsdi_session_to_nvruim_session(session_info_ptr->session_type));
  }
...
)
{
  ...
  mmgsdi_nv_context_type        slot_context             = MMGSDI_NV_CONTEXT_INVALID;
  mmgsdi_session_type_enum_type session_type             = MMGSDI_MAX_SESSION_TYPE_ENUM;
  nvruim_session_enum_type      nvruim_session_type      = NVRUIM_1X_PROV_PRI_SESSION;
  mmgsdi_nv_context_type        session_context          = MMGSDI_NV_CONTEXT_INVALID;
  ...
  slot_context          = mmgsdi_util_get_efs_item_index_for_slot(slot);
  nvruim_session_type   = mmgsdi_util_map_mmgsdi_session_to_nvruim_session(session_type);

  if(mmgsdi_nv_get_slot_feature_status(MMGSDI_FEATURE_UIM_MISCONFIG_RUIM_N5_WORKAROUND, slot_context) == MMGSDI_FEATURE_ENABLED)
  {
    ...
    nvruim_init_fallback_to_cave(mmgsdi_allow_fallback_to_cave_flag, nvruim_session_type);
    nvruim_init_hrpd_capability(mmgsdi_hrpd_disabled_card_flag, nvruim_session_type);
  }
  ...
  if(mmgsdi_nv_get_slot_feature_status(MMGSDI_FEATURE_USE_3GPD_CREDENTIALS_FROM_NV, slot_context) == MMGSDI_FEATURE_ENABLED)
  {
  ...
  nvruim_init_non_3gpd_cdma_card(non_3gpd_cdma_card, nvruim_session_type);


  if (app_type == MMGSDI_APP_CSIM)
  {
    (void)mmgsdi_uicc_chk_srv_available(session_id,
                                        MMGSDI_CHK_SRV_FROM_CARD_OR_CACHE,
  ...
  mmgsdi_status = mmgsdi_util_get_efs_item_index(MMGSDI_INVALID_SESSION_ID,
                                                 &session_type, &session_context);

  uim_init_an_hrpd_preference(
    support_an_hrpd,
    mmgsdi_util_convert_efs_context_to_cm_subid(session_context));
  ...
  uim_init_3gpd_preference(support_sip, support_mip, nvruim_session_type);
  nvruim_data_3gpd_init_extensions_support(service_3gpd_extensions, nvruim_session_type);
  ...
  nvruim_init_wms_svc_items(support_bcsms,support_smscap, nvruim_session_type);
...
{
  uint32                                pr_list_size       = 0;
  uint32                                pr_list_2_size     = 0;
  ...
  mmgsdi_return_enum_type               mmgsdi_status      = MMGSDI_SUCCESS;
  mmgsdi_access_type                    access;
  mmgsdi_len_type                       max_prl_size       = 0;
  mmgsdi_len_type                       max_eprl_size      = 0;
  word                                  prl_version        = 0;
      ...
      else
      {
        ...
        else
        {
           ...
           if(prl_status == PRL_VALID)
           {
             ...
             {
               MMGSDIUTIL_TMC_MEM_FREE(mmgsdi_1x_prl[prl_index].prl_data_ptr->roaming_list_ptr);
             }
           ...
           MMGSDIUTIL_ENTER_CLIENT_APP_DATA_CRIT_SECT;
           mmgsdi_1x_prl[prl_index].prl_validated =
               (prl_status == PRL_INVALID_SD_NOT_READY) ? FALSE : TRUE;
           MMGSDIUTIL_LEAVE_CLIENT_APP_DATA_CRIT_SECT;

           if(!mmgsdi_1x_prl[prl_index].prl_validated)
           {
             return MMGSDI_ERROR;
           }
  ...
  {
    UIM_MSG_ERR_2("Init file PRL 0x%x has invalid list size 0x%x",
                    access.file.file_enum, pr_list_size); 
    return MMGSDI_ERROR;
  }
...
)
{
  ...
  if(mmgsdi_nv_get_slot_feature_status(MMGSDI_FEATURE_OPLMN_LIST_CONFIGURABLE_OTA, nv_context) ==
        MMGSDI_FEATURE_ENABLED)
  {
    uint32  file_size       = 0;
...
)
{
  ...
  boolean is_mandatory =
    (mmgsdi_nv_get_slot_feature_status(MMGSDI_FEATURE_ABSENT_MANDATORY_FILES_INIT,
  ...
  mmgsdi_cache_init_order_type usim_pup_init_table[] =
    /* file name,        mandatory, init func,               validation func */
    {
     ...
     {MMGSDI_USIM_RPLMNACT,  FALSE,        mmgsdi_card_init_cache_binary, NULL},
     ...
     {MMGSDI_USIM_EHPLMN,    FALSE,        mmgsdi_card_init_cache_binary,
      mmgsdi_card_init_validate_ehplmn                                        },
  ...
  mmgsdi_status = mmgsdi_uicc_get_capabilities(session_id, MMGSDI_APP_USIM);
  ...
  if(mmgsdi_status == MMGSDI_SUCCESS && app_info_ptr != NULL)
  {
    ...
    mmgsdi_evt_build_and_send_fdn(notify_type,
                                  (app_info_ptr->app_capabilities.fdn_state == MMGSDI_SRV_ENABLE) ? TRUE : FALSE,
...
)
{
  ...
  mmgsdi_status = mmgsdi_uim_common_write_transparent(req_ptr, TRUE, &cnf_ptr, MMGSDI_UICC);
...
)
{
  ...
  /* No need to read PRL/EPRL from card if it is already present in cache */
  access.access_method = MMGSDI_EF_ENUM_ACCESS;
  access.file.file_enum = file;
  ...
  if((mmgsdi_status == MMGSDI_SUCCESS && data_ptr->data_len > 0) ||
     (mmgsdi_status == MMGSDI_NOT_FOUND && cache_state == MMGSDI_CACHE_NOT_FOUND))
  {
    return mmgsdi_status;
  }
  ...
  if (mmgsdi_status != MMGSDI_SUCCESS)
  {
    ...
    /* No need to initialize PRL if error is because of sync cmd cancelled.
       Otherwise, in next activation PRL file status will be MMGSDI_CACHE_INIT
       and MMGSDI will not try caching the file again which is not expected.
       If error is because of some other reason, then reset EF cache to all
       zeros. This ensures that the EF cache is present when populating the
       PRL cache in mmgsdi_card_init_1x_read_prl_init() */
  ...
  /* Free the PRL cache if error is because of sync cmd cancelled.
     Otherwise, in next activation PRL file status will be MMGSDI_CACHE_INIT
     and MMGSDI will not try caching the file again which is not expected.
     If error is because of some other reason, then don't do anything.
     This ensures that the EF cache is present when populating the
     PRL cache in mmgsdi_card_init_1x_read_prl_init() */
  if (mmgsdi_status != MMGSDI_SUCCESS && mmgsdi_sync_cmd_canceled[slot_index])
  {
    (void)mmgsdi_cache_delete_item(session_id,
                                   1,
                                   &file,
...
)
{
  ...
  mmgsdi_cache_init_order_type csim_pup_init_post_esn_table[] =
    ...
    {{MMGSDI_CSIM_CDMA_DIST_BASED_REGN_IND, TRUE,  mmgsdi_card_init_cache_binary,        NULL},
     ...
     {MMGSDI_CSIM_MSISDN,                   FALSE, mmgsdi_card_init_cache_record,        NULL},
     {MMGSDI_CSIM_CALL_TERM_MODE_PREF,      FALSE, mmgsdi_card_init_cache_binary,        NULL},
     {MMGSDI_CSIM_TMSI,                     TRUE,  mmgsdi_card_init_cache_binary,        NULL},
     {MMGSDI_CSIM_SSCI,                     FALSE, mmgsdi_card_init_cache_binary,        NULL},
     {MMGSDI_CSIM_3GPD_3GPDOPM,             FALSE, mmgsdi_card_init_cache_binary,        NULL},
     {MMGSDI_CSIM_3GPD_MIPFLAGS,            FALSE, mmgsdi_card_init_cache_binary,        NULL},
     {MMGSDI_CSIM_3GPD_MIPSP,               FALSE, mmgsdi_card_init_cache_binary,        NULL},
     {MMGSDI_CSIM_3GPD_MIPUPP,              FALSE, mmgsdi_card_init_cache_binary,        NULL},
     {MMGSDI_CSIM_3GPD_DGC,                 FALSE, mmgsdi_card_init_cache_binary,        NULL},
     {MMGSDI_CSIM_3GPD_UPPEXT,              FALSE, mmgsdi_card_init_cache_binary,        NULL},
     {MMGSDI_CSIM_3GPD_SIPUPP,              FALSE, mmgsdi_card_init_cache_binary,        NULL}};
  

  UIM_MSG_HIGH_1("mmgsdi_card_init_csim_post_pin1_init, slot:0x%x", slot_id);

  (void)mmgsdi_uicc_get_capabilities(session_id, MMGSDI_APP_CSIM);
  ...
  if(mmgsdi_nv_get_slot_feature_status(MMGSDI_FEATURE_CSIM_UPP_UPDATE_AFTER_OTASP, nv_context) ==
     MMGSDI_FEATURE_ENABLED)
  {
...
)
{
  ...
  mmgsdi_init_cache_func_type eprl_init_cache_func =
    (mmgsdi_nv_get_slot_feature_status(MMGSDI_FEATURE_BLOCK_RUIM_EPRL_ACCESS,
       mmgsdi_util_get_efs_item_index_for_slot(slot_id)) == MMGSDI_FEATURE_DISABLED) ?
  ...
  mmgsdi_cache_init_order_type ruim_pup_init_post_esn_table[] =
    ...
    {{MMGSDI_CDMA_ACCOLC,              TRUE,  mmgsdi_card_init_cache_binary,         NULL},
     ...
     {MMGSDI_CDMA_MSISDN,              FALSE, mmgsdi_card_init_cache_record,         NULL},
     {MMGSDI_CDMA_CALL_TERM_MODE_PREF, FALSE, mmgsdi_card_init_cache_binary,         NULL},
     {MMGSDI_CDMA_TMSI,                TRUE,  mmgsdi_card_init_cache_binary,         NULL},
     {MMGSDI_CDMA_SCI,                 FALSE, mmgsdi_card_init_cache_binary,         NULL},
     {MMGSDI_CDMA_3GPD_3GPDOPM,        FALSE, mmgsdi_card_init_cache_binary,         NULL},
     {MMGSDI_CDMA_3GPD_MIPFLAGS,       FALSE, mmgsdi_card_init_cache_binary,         NULL},
     {MMGSDI_CDMA_3GPD_MIPSP,          FALSE, mmgsdi_card_init_cache_binary,         NULL},
     {MMGSDI_CDMA_3GPD_MIPUPP,         FALSE, mmgsdi_card_init_cache_binary,         NULL},
     {MMGSDI_CDMA_3GPD_DGC,            FALSE, mmgsdi_card_init_cache_binary,         NULL},
     {MMGSDI_CDMA_3GPD_SIPUPP,         FALSE, mmgsdi_card_init_cache_binary,         NULL}};
...
)
{
  ...
  UIM_MSG_HIGH_1("Sent Card status cmd : mmgsdi_status 0x%x", mmgsdi_status);
...
)
{
  ...
  switch(app_type)
  {
    ...
    case MMGSDI_APP_CSIM:
      ...
      {
        /* mmgsdi_card_init_csim_post_pin1_init returns UIM_INSTN_CHG_SUCCESS
           when caching fails which could also happen when a sync cmd is canceled.
           Return error without starting reset if the command was canceled. */
        if(mmgsdi_is_sync_cmd_canceled(slot_id) == FALSE &&
           mmgsdi_util_get_usim_apps(slot_id) == 0)
        {
...
{
  mmgsdi_return_enum_type  mmgsdi_status            = MMGSDI_SUCCESS;
  /* These variables are initialized with TRUE intentionally so that if OEM
     tends to use his own SIMLOCK solution, we still cache these files */
  boolean                  is_sp_lock_active        = TRUE;
  boolean                  is_ns_sp_lock_active     = TRUE;
  boolean                  is_cp_lock_active        = TRUE;
  boolean                  is_spn_lock_active       = TRUE;
  boolean                  is_sp_ehplmn_lock_active = TRUE;
  ...
  mmgsdi_cache_init_order_type usim_pre_perso_init_table[] =
    /* file name,        mandatory, init func,               validation func */
    {
     ...
     {MMGSDI_USIM_ACC,       is_mandatory, mmgsdi_card_init_cache_binary,
      mmgsdi_card_init_validate_acc_data                                      },
     {MMGSDI_USIM_UST,       FALSE,        mmgsdi_card_init_cache_binary,
      mmgsdi_card_init_validate_usim_ust},
  ...
  mmgsdi_cache_init_order_type usim_pre_perso_ehplmn_init_table[] =
    /* file name,        mandatory, init func,               validation func */
    {
  ...
  mmgsdi_cache_init_order_type ruim_pre_perso_init_table[] =
    /* file name,        mandatory, init func,               validation func */
    {
     ...
     {MMGSDI_CDMA_RUIM_ID,           TRUE,  mmgsdi_card_init_cache_binary, NULL}
    };
     ...
     (app_type == MMGSDI_APP_USIM))
  {
    ...
    is_ns_sp_lock_active = mmgsdi_simlock_check_category_status(slot_id, SIMLOCK_CATEGORY_3GPP_NS_SP);
    ...
    if (app_type == MMGSDI_APP_USIM)
    {
      is_sp_ehplmn_lock_active = mmgsdi_simlock_check_category_status(
                                            slot_id,
  ...
  switch(app_type)
  {
    ...
    case MMGSDI_APP_USIM:
      ...
      if(mmgsdi_status == MMGSDI_SUCCESS)
      {
        ...
        /* If SP_EHPLMN lock is active, EHPLMN file needed in SIMLOCK engine.
           Cache it before running SIMLOCK engine. If not present in card,
           no need to return error. SIMLOCK engine will take care of it. */
        if(is_sp_ehplmn_lock_active)
        {
          /* Ignore the return status here as EHPLMN file is not mandatory */
          (void)mmgsdi_card_init_app_cache(session_id,
                                           MMGSDI_APP_USIM,
                                           slot_id,
                                           usim_pre_perso_ehplmn_init_table,
                                           sizeof(usim_pre_perso_ehplmn_init_table)/
                                           sizeof(mmgsdi_cache_init_order_type));
        }
      }
      break;
...
)
{
  ...
  mmgsdi_nv_item_cache_type          mmgsdi_nv_data               = {0};
  boolean                            set_csim_present_nv          = FALSE;
  boolean                            get_csim_present_nv          = FALSE;
  uim_common_efs_status_enum_type    efs_access_status            = UIM_COMMON_EFS_ERROR;
  nv_stat_enum_type                  nv_status                    = NV_FAIL_S;
  uint8                              slot_index                   = MMGSDI_SLOT_1_INDEX;
  uim_common_efs_context_type        csim_context                 = UIM_COMMON_EFS_CONTEXT_MAX;
  ...
  {
    /* Activate the app only in case of telecom cards */
    if (slot_data_ptr->card_type == MMGSDI_CARD_TELECOM)
    {
  ...
  /* Even if RTRE configuration is per subscription and not per slot,
     we send RTRE configuration based on slot insertion as per legacy
     logic. This will not have any side effect in case of cross-mapping,
     as CM should anyway already have the correct value of the RTRE. */
  mmgsdi_init_rtre_config_per_context(mmgsdi_util_get_efs_item_index_for_slot(slot_id));

  /* Check if card contains CSIM */
  if (mmgsdi_util_get_csim_apps(slot_id) > 0)
  {
    set_csim_present_nv = TRUE;
  }

  /* Now set the internal GSTK CSIM PRESENT efs item, if needed */
  if (mmgsdi_util_get_slot_index(slot_id, &slot_index) != MMGSDI_SUCCESS)
  {
    return MMGSDI_ERROR;
  }
  csim_context = mmgsdi_util_get_common_efs_item_context(slot_index);
  efs_access_status = uim_common_efs_read(UIM_COMMON_EFS_MMGSDI_CSIM_PRESENT,
                                          UIM_COMMON_EFS_ITEM_FILE_TYPE,
                                          csim_context,
                                          &get_csim_present_nv,
                                          sizeof(get_csim_present_nv));
  if (efs_access_status != UIM_COMMON_EFS_SUCCESS ||
      get_csim_present_nv != set_csim_present_nv)
  {
    (void) uim_common_efs_write(UIM_COMMON_EFS_MMGSDI_CSIM_PRESENT,
                                UIM_COMMON_EFS_ITEM_FILE_TYPE,
                                csim_context,
                                (const char*) &set_csim_present_nv,
                                sizeof(set_csim_present_nv));
  }
...
)
{
  ...
  {
    mmgsdi_status = mmgsdi_set_ens_support_flag(
                                session_id,
                                slot_id,
                                int_app_info_ptr->app_data.app_type);

    /* Update GSTK to enable FETCH as ENS initialization has happened */
    ...
    if (mmgsdi_status != MMGSDI_SUCCESS &&
        mmgsdi_is_sync_cmd_canceled(slot_id))
    {
      /* Return without continuing the INTERNAL_PUP_INIT if the ENS operation
         failed due to cancellation of sync command which indicates that a
         deactivation is being processed in the main task. */
      return mmgsdi_status;
    }
  }
...
)
{
  ...
  else if(MMGSDI_TP_STATE_NOT_NEEDED == tp_state)
  {
    int_app_info_ptr->app_tp_state = MMGSDI_TP_STATE_NOT_NEEDED;
  }
  ...
  mmgsdi_status = mmgsdi_simlock_build_and_run_algo(FALSE, session_id);
#else
                         ...
                         sizeof(mmgsdi_halt_sub_setting)) != UIM_COMMON_EFS_SUCCESS)
  {
    ...
    int_app_info_ptr->perso_status = MMGSDI_PERSO_STATUS_DONE;
  }
  ...
  else
  {
    ...
    int_app_info_ptr->perso_status = MMGSDI_PERSO_STATUS_WAIT_FOR_EXT_PERSO;
  }
...
)
{
  ...
  /* If the command failed due to a sync command being canceled, stop
     further processing. The sync command would be canceled during session
     deactivation and we need to avoid triggering another deactivation because
     of the failure. */
  if(internal_pup_status != MMGSDI_SUCCESS &&
     mmgsdi_is_sync_cmd_canceled(slot_id))
  {
    return;
  }
  ...
  switch(internal_pup_status)
  {
    case MMGSDI_SUCCESS:
      ...
      if (int_app_info_ptr->app_state == MMGSDI_APP_STATE_READY)
      {
        ...
        if((mmgsdi_util_get_efs_item_index(session_id, NULL, &nv_context) ==
              MMGSDI_SUCCESS) &&
...
)
{
  ...
  memset(&file_cache, 0x00, sizeof(mmgsdi_data_type));
  ...
  /* Populate passed in data ptr if file is already in cache */
  mmgsdi_status = mmgsdi_cache_read_len(session_id,
                                        &access,
                                        0,
                                        &file_cache.data_len,
  ...
  if(mmgsdi_status == MMGSDI_SUCCESS)
  {
    if (file_cache.data_len > 0)
    {
      MMGSDIUTIL_MEM_MALLOC_AND_VALIDATE(file_cache.data_ptr, file_cache.data_len);
      if (file_cache.data_ptr == NULL)
      {
        /* Unable to allocate memory */
        return MMGSDI_MEMORY_ERROR_HEAP_EXHAUSTED;
      }
      ...
      if (mmgsdi_status == MMGSDI_SUCCESS)
      {
        if(data_ptr != NULL)
        {
          data_ptr->data_len = file_cache.data_len;
          data_ptr->data_ptr = file_cache.data_ptr;
        }
        else
        {
          MMGSDIUTIL_TMC_MEM_FREE(file_cache.data_ptr);
        }
        return MMGSDI_SUCCESS;
      }

      file_cache.data_len = 0;
      MMGSDIUTIL_TMC_MEM_FREE(file_cache.data_ptr);
      ...
      /* We don't need 'else' here because rest of the function will take
         care of handling other return statuses. */
     } /* read cache len > 0 */
      ...
      (cnf_ptr->response_header.mmgsdi_status == MMGSDI_SUCCESS))
  {
    if (data_ptr != NULL)
    {
      ...
      {
        MMGSDIUTIL_MEM_MALLOC_AND_VALIDATE(data_ptr->data_ptr,  data_ptr->data_len);
...
)
{
  ...
  /* Per the requirement, if the refresh reset is in progress do not reset
     CSIM hiding status */
  if(!((mmgsdi_refresh_info_ptr[slot_index] != NULL) &&
       (mmgsdi_refresh_info_ptr[slot_index]->request_header.slot_id == req_ptr->request_header.slot_id) &&
       (mmgsdi_refresh_info_ptr[slot_index]->mode == MMGSDI_REFRESH_RESET)))
  {
    /* Reset the csim hiding status to normal */
    MMGSDIUTIL_ENTER_CLIENT_APP_DATA_CRIT_SECT;
    slot_data_ptr->csim_hiding_status = MMGSDI_CSIM_UNHIDDEN;
    MMGSDIUTIL_LEAVE_CLIENT_APP_DATA_CRIT_SECT;
  }

  UIM_MSG_HIGH_5("Slot 0x%x: Slot Type: 0x%x Card Present: 0x%x, Slot State: 0x%x, card type: 0x%x",
                 req_ptr->request_header.slot_id,
                 req_ptr->link_est_data.slot_type,
                 req_ptr->link_est_data.card_present,
                 slot_data_ptr->mmgsdi_state,
                 req_ptr->link_est_data.card_type);
  ...
  mmgsdi_log_pkt_link_established(req_ptr);
      ...
      (slot_data_ptr->nv_refresh_in_progress == FALSE))
  {
    ...
    {
      mmgsdi_evt_build_and_send_sap_disconnected(req_ptr->request_header.slot_id);
      mmgsdi_sap_reset_sap_data(req_ptr->request_header.slot_id);
    }
    ...
    {
      mmgsdi_evt_build_and_send_non_telecom_card_inserted(
                                       TRUE,
...
)
{
       ...
       (mmgsdi_refresh_info_ptr[slot_index]->mode == MMGSDI_REFRESH_RESET)))
  {
    /* Reset the csim hiding status to normal */
    MMGSDIUTIL_ENTER_CLIENT_APP_DATA_CRIT_SECT;
    mmgsdi_data_slot_ptr[slot_index]->csim_hiding_status = MMGSDI_CSIM_UNHIDDEN;
    MMGSDIUTIL_LEAVE_CLIENT_APP_DATA_CRIT_SECT;
  }
...
/*===========================================================================
FUNCTION:      MMGSDI_CARD_INIT_PROCESS_SIM_PRE_INIT

DESCRIPTION:
  This function will handle the SIM pre initalization message originating from
  UIM drivers.

  The function saves the new status internally and sends a detection started
  event to all MMGSDI clients
...
SIDE EFFECTS:
  NONE
===========================================================================*/
mmgsdi_return_enum_type mmgsdi_card_init_process_sim_pre_init(
  const mmgsdi_notify_sim_pre_init_req_type *req_ptr
)
{
  mmgsdi_return_enum_type                mmgsdi_status = MMGSDI_SUCCESS;
  uint8                                  slot_index    = MMGSDI_SLOT_1_INDEX;
  ...
  /* If in PSM early events mode, early events (card inserted & session events)
     are already sent, no need to update sim detection status nor sends detection
     started event */
  if(mmgsdi_psm_get_status_early_init() != FALSE &&
     mmgsdi_psm_get_enabled_status() == MMGSDI_PSM_MODE_EARLY_EVTS)
  {
    return MMGSDI_SUCCESS;
  }

  /* If UIMDRV already notified us, nothing else needed */
  if (mmgsdi_data_slot_ptr[slot_index]->sim_detection_started)
  {
    return MMGSDI_SUCCESS;
  }

  /* Save the detection status, this will be used to inform any new
     MMGSDI clients registering late */
  mmgsdi_data_slot_ptr[slot_index]->sim_detection_started = TRUE;

  /* infor all clients via the detection started event */
  mmgsdi_evt_build_and_send_sim_detection_started(TRUE, 0, req_ptr->request_header.slot_id);

  return MMGSDI_SUCCESS;
} /* mmgsdi_card_init_process_sim_pre_init */
