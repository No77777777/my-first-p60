...
Copyright (c) 2007 - 2020, 2022 QUALCOMM Technologies, Inc (QTI) and its licensors.
...
$Header: //components/rel/uim.mpss/6.1.0/mmgsdi/src/mmgsdi_refresh.c#5 $$ $DateTime: 2022/02/07 06:38:42 $

when       who      what, where, why
--------   ---      ----------------------------------------------------------
02/07/22   ghk      Fix INIT + FULLFCN refresh failure in case of SFI disabled
...
03/13/19   sg       Send Refresh event to clients after processing status rsp
02/26/19   vm       Clear attr cache by path as well as by enum in FCN Refresh
...
08/27/18   bcho     MMGSDI should not include call stack files in FCN list
04/19/18   rps      Do not fail FCN refresh if file not found on card
07/13/17   bcho     MMGSDI to set OPLMNwACT attr cache before caching file data
06/20/17   me       Added cache support for USIM MSISDN EF's while refresh process evt 
04/20/17   nr       Add MMGSDI_USIM_EHPLMN to call stack file list
...
01/24/17   ar       Don't reset card if REFRESH_REG/DEREG request comes
11/23/16   nr       Do not reject write request when there is no ongoing refresh
11/10/16   me       Handle FCN refresh for EF-NASCONFIG and convert to INIT Refresh mode
10/10/16   bcho     Queue NVRUIM read requests to high priority command queue
10/06/16   ar       Added support for SP + EHPLMN simlock
09/30/16   bcho     Delete file cache after deactivation in Refresh App Reset
...
08/25/16   av       Send refresh success TR for an AID not found in EF-DIR
08/08/16   me       Added cache support for MSISDN EF's while refresh process evt 
08/03/16   vdc      Added support for SPN based SIMLOCK feature
07/13/16   tkl      Remove sprintf for client task name
...
06/08/16   tkl      Refresh of MMGSDI_TELECOM_ADN in UICC mode
...
04/29/16   bcho     Disable CHPS functionality for IOE LTE Only devices
04/29/16   vdc      Return error if provided session is invalid for refresh reg
...
03/15/16   tkl      Cancel read request between refresh stage start & deactivate
...
12/04/15   bcho     Param file_size type change in mmgsdi_get_builtin_plmn_list
...
09/14/15   at       Update slot type in session based events
09/14/15   bcho     Free RUIM/GSM cache during refresh if session is not active
09/03/15   bcho     Check app is prov type before finding call stack files
09/10/15   kv       Features_status_list is split into device and slot items
09/02/15   gm       Always read EF_ECC if present in FCN list
09/01/15   gm       Ignoring FCN list as origninal mode is INIT_FCN
08/31/15   yt       Remove length information from requests and confirmations
08/19/15   gm       Update the stack file list
...
07/23/15   ar       Don't allow write if REFRESH_INIT is in progress
06/26/15   av       Make channel app check more robust
06/15/15   lm       Remove redundant code in slot index validation
06/12/15   bcho     Close channel if session is activated on another slot
06/05/15   nr       Extending support for refresh retry to multi slot
05/26/15   nr       Fixing memoory leak during card powerdown and task stop
05/25/15   nr       Fix to send refresh event if clients vote false for NAA FCN
05/20/15   nr       Do not add App Reset if 3G session reset is in retry table
05/18/15   nr       Fix to distiguish reads on different apps in single slot
05/15/15   vdc      Check file path pointer for NULL after memory allocation
...
03/26/15   nr       Clearing PB cache during REFRESH FCN handling
...
01/21/15   bcho     Handle Refresh FCN of GSM OPL file in USIM mode
01/20/15   bcho     Replace IF with ELSE IF to remove unnecessary check
01/20/15   ar       Block USIM write if protocol is UICC and FCN has GSM file
01/17/15   ar       Check for Register ALL flag while copying FCN list
...
12/04/14   ar       Change mode to INIT if refresh list contains EF-ARR
...
/* Global to hold failed refresh requests per slot */
static mmgsdi_refresh_retry_table_entry_type *mmgsdi_refresh_retry_table[MMGSDI_MAX_NUM_SLOTS][MMGSDI_REFRESH_RETRY_TABLE_SIZE] = {{NULL}};

/* Global to hold the current index to start fetching refresh request
   from next time */
static uint8 mmgsdi_refresh_retry_table_index[MMGSDI_MAX_NUM_SLOTS] = {0};

/* USIM EF-OPLMNWACT File Attribute Value 
   This is a variable size file, make sure to change the length accordingly while writing
   raw attributes to the cache */
static uint8  mmgsdi_file_usim_oplmnwact_attr[] = {0x62, 0x3D,
                                                   ...
                                                   /* Update Access - MMGSDI_NEVER_ALLOWED */
                                                   0x80,0x01,0x02,0xA4,0x06,0x83,0x01,0x0A,0x95,0x01,0x08,
                                                   /* Activate Access - MMGSDI_NEVER_ALLOWED */
                                                   0x80,0x01,0x10,0xA4,0x06,0x83,0x01,0x0A,0x95,0x01,0x08,
                                                   /* Deactivate Access - MMGSDI_NEVER_ALLOWED */
                                                   0x80,0x01,0x08,0xA4,0x06,0x83,0x01,0x0A,0x95,0x01,0x08,
                                                   /* File Size */
                                                   0x80,0x02,0x01, 0xf4};

/* GSM EF-OPLMNWACT File Attributes Value
   Note that 3rd and 4th elements in the below array indicate length and since it is a variable sized file,
   before writing the raw attributes of this file to the cache, make sure to change accordingly */
static uint8
  mmgsdi_file_gsm_oplmnwact_attr[] = {0x00,0x00,0x01,0xF4,0x6F,0x61,0x04,0x00,0x14,0x00,0x44,0x21,0x02,0x00,0x00};
  ...
  const mmgsdi_cnf_type   *cnf_ptr)
{
  ...
  switch(cnf)
  {
    case MMGSDI_CARD_STATUS_REQ:
      ...
      /* In case there are read requests between deleted cache and deactivate,
         we cancel pending read request that is in the queue. Since cache is
         deleted, the request will send to card, this will add delays to app
         goes to sub_ready state. Also, read data may be changed during refresh
         and client will need to read it again anyways */
      cancel_info.cancel_type     = MMGSDI_CANCEL_USING_SESSION_TYPE;
      cancel_info.id.session_type = session_info_ptr->session_type;
      cancel_info.cancel_request_type.is_valid     = TRUE;
      cancel_info.cancel_request_type.request_type = MMGSDI_READ_REQ;

      (void)mmgsdi_process_cancel_req(cancel_info, FALSE);
    ...
    case MMGSDI_SESSION_APP_RESET_DEACTIVATE_REQ:
         ...
         session_info_ptr != NULL)
      {
        ...
        (void) mmgsdi_process_cancel_req(cancel_info, FALSE);
         ...
         /* Delete file attribute cache */
        (void)mmgsdi_cache_delete_prov_file_attr_cache(session_info_ptr->session_type,
                                                       NULL,
                                                       0,
                                                       NULL,
                                                       0);
      ...
      /* Delete any file cache if it gets populated after stage START and
         before deactivating the application */
      (void)mmgsdi_cache_delete_item(cnf_ptr->status_cnf.response_header.session_id,
                                     0,
                                     NULL,
                                     0,
                                     NULL);
...
/*===========================================================================
  ...
  SIDE EFFECTS
    None
===========================================================================*/
static void mmgsdi_refresh_reset_notify_init(
  uint8 slot_index
)
{
...
/*===========================================================================
  ...
  SIDE EFFECTS
    None
===========================================================================*/
static void mmgsdi_refresh_timer_expiry (
  unsigned long timer_count_slot
)
{
  mmgsdi_slot_id_enum_type      slot_id       = MMGSDI_MAX_SLOT_ID_ENUM;
  ...
  uint8                  slot_index)
{
  ...
  if(slot_ptr == NULL || !MMGSDIUTIL_SLOT_INDEX_VALID(slot_index))
  {
    return;
  }
  ...
  const mmgsdi_request_header_type          *request_hdr_ptr)
{
  ...
  while(node_ptr)
  {
       ...
       node_ptr->stage == extra_info_ptr->stage)
    {
      ...
      slot_data_ptr = mmgsdi_util_get_slot_data_ptr(request_hdr_ptr->slot_id);
      if (slot_data_ptr != NULL)
      {
        evt_info_ptr->data.refresh.app_info.slot_type = slot_data_ptr->slot_type;
      }
      ...
      if(node_ptr->refresh_files.file_path_ptr != NULL)
      {
        ...
        for (j = 0; j < node_ptr->refresh_files.num_file_paths; j++)
        {
          evt_info_ptr->data.refresh.refresh_files.file_path_len +=
            node_ptr->refresh_files.file_path_ptr[j].path_len * sizeof(uint16);
        }
  ...
  for(i = 0; i < MMGSDI_MAX_CLIENT_INFO; i++)
  {
    ...
    for(j = 0; (num_sessions > 0) && (j < MMGSDI_MAX_SESSION_INFO); j++)
    {
         ...
         mmgsdi_refresh_info_ptr[slot_index]->mode != MMGSDI_REFRESH_RESET_AUTO)
      {
        ...
        else if(evt_info_ptr->data.refresh.mode == MMGSDI_REFRESH_APP_RESET_AUTO)
        {
  ...
  mmgsdi_file_enum_type     file)
{
  switch(file)
  {
    ...
    case MMGSDI_USIM_ARR:  /* If EF-ARR got changed then all USIM/CSIM files
                           need to re-cache again as security conditions might
                           have changed for already cached files */
    ...
    case MMGSDI_CDMA_EPRL:
    case MMGSDI_CDMA_GID1:
    case MMGSDI_CDMA_GID2:
    case MMGSDI_CDMA_3GPD_DGC:
    case MMGSDI_CDMA_3GPD_UPPEXT:
    ...
    case MMGSDI_CSIM_3GPD_DGC:
    case MMGSDI_CSIM_3GPD_UPPEXT:
      return TRUE;
...
)
{
     ...
     mmgsdi_refresh_info_ptr[slot_index]->request_header.slot_id != slot)
  {
    ...
    UIM_MSG_ERR_0("Refresh: No Refresh is in progress on current slot");
    return;
  }
...
/*===========================================================================
  FUNCTION MMGSDI_REFRESH_HANDLE_REFRESH_START_FCN

  DESCRIPTION
    This function will handle the Refresh event STAGE START for FCN mode.
  ...
  SIDE EFFECTS
===========================================================================*/
static mmgsdi_return_enum_type mmgsdi_refresh_handle_refresh_start_fcn(
  const mmgsdi_event_data_type *event_ptr
)
{
  ...
  if (event_ptr->data.refresh.refresh_files.file_path_len > 0 &&
      event_ptr->data.refresh.refresh_files.file_path_ptr == NULL)
  {
    UIM_MSG_ERR_0("Refresh: file path ptr is NULL");
    return MMGSDI_ERROR;
  }

  mmgsdi_status =  mmgsdi_util_get_session_type(event_ptr->session_id,
                                                &session_type,
                                                NULL);
  if(mmgsdi_status != MMGSDI_SUCCESS)
  {
  ...
  /* if card session the no need to get prov app info ptr */
  if(mmgsdi_util_is_card_session(session_type))
  {
    mmgsdi_status = mmgsdi_util_get_session_app_info(
  ...
  if ((mmgsdi_status != MMGSDI_SUCCESS) || 
      (int_app_info_ptr == NULL && !mmgsdi_util_is_card_session(session_type)))
  {
  ...
  do
  {
    /* Allocate temporary buffer to account for number of files as first byte */
    temp_len = (event_ptr->data.refresh.refresh_files.file_path_len + 1) * sizeof(uint8);
    ...
    for (i = 0; i < (event_ptr->data.refresh.refresh_files.file_path_len - 1); i += 2)
    {
      if ((event_ptr->data.refresh.refresh_files.file_path_ptr[i] == MMGSDIUTIL_MF_HIGH_BYTE) &&
          (event_ptr->data.refresh.refresh_files.file_path_ptr[i + 1] == 0x00))
      {
        if (temp_path_ptr[0] == 255)
        {
    ...
    mmgsdi_memscpy(&temp_path_ptr[1],
                   temp_len - 1,
                   event_ptr->data.refresh.refresh_files.file_path_ptr,
                   event_ptr->data.refresh.refresh_files.file_path_len);
    ...
    for(i = 0; i < num_files; i++)
    {
      ...
      (void)mmgsdi_file_get_file_from_path(event_ptr->data.refresh.aid.app_type,
                                           mmgsdi_file_path_ptr[i],
                                           &file_enum);

      /* delete common attr under telecom df */
      if(mmgsdi_util_is_card_session(session_type))
      {
        if(mmgsdi_file_is_ef_under_global_telecom_df(&mmgsdi_file_path_ptr[i]))
        {
          /* Delete file attribute cache by path as well as by enum */
          (void)mmgsdi_cache_delete_attr_cache_by_path(mmgsdi_cache_common_attr_cache_ptr(slot_id),
                                                       mmgsdi_file_path_ptr[i]);
          if(file_enum != MMGSDI_NO_FILE_ENUM)
          {
            /* clear attribute cache item by enum */
            (void)mmgsdi_cache_delete_attr_selective_items(mmgsdi_cache_common_attr_cache_ptr(slot_id),
                                                           NULL,
                                                           0,
                                                           &file_enum,
                                                           1);
      ...
      /* Delete file attribute cache by path as well as by enum */
      (void)mmgsdi_cache_delete_attr_cache_by_path(&int_app_info_ptr->attr_cache_tbl,
                                                   mmgsdi_file_path_ptr[i]);
      if(file_enum == MMGSDI_NO_FILE_ENUM)
      {
        continue;
...
#ifdef FEATURE_UIM_PERSISTENT_CACHE
      /* delete refresh files in persistent cache from active profile */
      (void)mmgsdi_persistent_cache_delete_file(event_ptr->data.refresh.slot,
                                                file_enum);
#endif /* FEATURE_UIM_PERSISTENT_CACHE */

      /* Ignoring re-reading of files which is already cached during INIT,
         Files which is not cache during INIT will be cached when client request for it */
      if (event_ptr->data.refresh.orig_mode == MMGSDI_REFRESH_NAA_INIT_FCN ||
          event_ptr->data.refresh.orig_mode == MMGSDI_REFRESH_3G_SESSION_RESET)
      {
        /* MMGSDI will not delete below files during INIT */
        switch(file_enum)
        {
      ...
      EF_category = MMGSDIFILE_GET_CATEGORY_FROM_FILE(file_enum);
      ...
      /* If protocol is ICC and file in refresh list is in USIM/CSIM ADF
         ...
         don't try to cache the file. Exempt GSM SPN and GSM OPL files
         when app type is UICC; we have seperate handling for it */
      if(protocol == UIM_ICC &&
      ...
      {
        if(event_ptr->data.refresh.aid.app_type != MMGSDI_APP_USIM ||
           (file_enum != MMGSDI_GSM_SPN &&
            file_enum != MMGSDI_GSM_OPL))
        {
          continue;
        }
      ...
      /* clear attribute cache item */
      (void)mmgsdi_cache_delete_attr_selective_items(
                                              &int_app_info_ptr->attr_cache_tbl,
                                              NULL,
                                              0,
                                              &file_enum,
                                              1);
...
#ifdef FEATURE_SIMLOCK
        /* If SPN category is enabled, run SIMLOCK engine as SPN data
           have changed */
        if(mmgsdi_simlock_check_category_status(
             event_ptr->data.refresh.slot,
             SIMLOCK_CATEGORY_3GPP_SPN))
        {
          (void)mmgsdi_simlock_build_and_run_algo(FALSE, event_ptr->session_id);
        }
#endif /*FEATURE_SIMLOCK*/
      }
      else if(file_enum == MMGSDI_USIM_CPHSI ||
              file_enum == MMGSDI_GSM_CPHSI)
      {
      ...
      else if(file_enum == MMGSDI_USIM_CSP ||
              file_enum == MMGSDI_GSM_CSP)
      {
        mmgsdi_cphs_read_cphs_efs_info_type  read_cphs_efs;
      ...
      else if(file_enum == MMGSDI_USIM_ECC)
      {
        mmgsdi_status = mmgsdi_cache_init_usim_ecc(
                                    event_ptr->session_id);
      }
      else if(file_enum == MMGSDI_USIM_ONS||
              file_enum == MMGSDI_GSM_ONS)
      {
        mmgsdi_cphs_read_cphs_efs_info_type  read_cphs_efs;
        read_cphs_efs.read_cphsi     = FALSE;
        read_cphs_efs.read_csp       = FALSE;
        ...
        (void) mmgsdi_cphs_post_pin1_init(
          event_ptr->session_id,
          event_ptr->data.refresh.slot,
          event_ptr->data.refresh.aid.app_type,
          read_cphs_efs);
      ...
      else if(file_enum == MMGSDI_USIM_ONS_SHORT||
              file_enum == MMGSDI_GSM_ONS_SHORT)
      {
        mmgsdi_cphs_read_cphs_efs_info_type  read_cphs_efs;
        read_cphs_efs.read_cphsi     = FALSE;
        read_cphs_efs.read_csp       = FALSE;
      ...
      else if (file_enum == MMGSDI_USIM_HIDDENKEY ||
               file_enum == MMGSDI_CSIM_HIDDEN_KEY)
      {
        mmgsdi_status = mmgsdi_hidden_key_refresh_process_fcn(
                          event_ptr,
                          file_enum);
      }
      else if(file_enum == MMGSDI_GSM_SPN ||
              file_enum == MMGSDI_GSM_SPDI||
              file_enum == MMGSDI_GSM_OPL ||
              file_enum == MMGSDI_GSM_PNN ||
              file_enum == MMGSDI_USIM_SPN ||
              file_enum == MMGSDI_USIM_SPDI ||
              file_enum == MMGSDI_USIM_OPL ||
              file_enum == MMGSDI_USIM_PNN ||
              file_enum == MMGSDI_CDMA_HOME_SVC_PVDR_NAME ||
              file_enum == MMGSDI_CSIM_SPN)
      {
      ...
      else if (file_enum == MMGSDI_USIM_ACM ||
               file_enum == MMGSDI_GSM_ACM  ||
               file_enum == MMGSDI_USIM_EPSNSC ||
               file_enum == MMGSDI_CSIM_CDMA_ZONE_BASED_REGN_IND ||
               file_enum == MMGSDI_CDMA_ZONE_BASED_REGN_IND ||
               file_enum == MMGSDI_CDMA_MSISDN ||
               file_enum == MMGSDI_CSIM_MSISDN ||
               file_enum == MMGSDI_USIM_MSISDN)
      {
      ...
      else if (file_enum == MMGSDI_CSIM_ECC ||
               file_enum == MMGSDI_CDMA_ECC ||
               file_enum == MMGSDI_GSM_ECC)
      {
        mmgsdi_evt_session_notify_type   notify_type;
      ...
      else if(file_enum == MMGSDI_USIM_ACSGL ||
              file_enum == MMGSDI_USIM_CSGT  ||
              file_enum == MMGSDI_USIM_HNBN  ||
              file_enum == MMGSDI_USIM_OCSGL  ||
              file_enum == MMGSDI_USIM_OCSGT  ||
              file_enum == MMGSDI_USIM_OHNBN)
      {
        mmgsdi_nv_context_type efs_index = MMGSDI_NV_CONTEXT_INVALID;

        if((mmgsdi_util_get_efs_item_index(
              event_ptr->session_id, NULL, &efs_index) == MMGSDI_SUCCESS) &&
      ...
      else if ((((file_enum == MMGSDI_USIM_OPLMNWACT) &&
                 (event_ptr->data.refresh.aid.app_type == MMGSDI_APP_USIM)) ||
                ((file_enum == MMGSDI_GSM_OPLMNWACT) &&
                 (event_ptr->data.refresh.aid.app_type == MMGSDI_APP_SIM))) &&
                (mmgsdi_nv_get_slot_feature_status(MMGSDI_FEATURE_OPLMN_LIST_CONFIGURABLE_OTA,
                 mmgsdi_util_get_efs_item_index_for_slot(event_ptr->data.refresh.slot)) ==
                MMGSDI_FEATURE_ENABLED))
      {
           ...
           file_buffer_ptr != NULL)
        {
          ...
          mmgsdi_data_type                  raw_file_attr_data ;
          ...
          if(mmgsdi_status == MMGSDI_SUCCESS)
          {
            ...
            if(file_enum == MMGSDI_USIM_OPLMNWACT)
            {
              raw_file_attr_data.data_len = sizeof(mmgsdi_file_usim_oplmnwact_attr);
              raw_file_attr_data.data_ptr = (uint8 *)mmgsdi_file_usim_oplmnwact_attr;
            }
            else
            {
              raw_file_attr_data.data_len = sizeof(mmgsdi_file_gsm_oplmnwact_attr);
              raw_file_attr_data.data_ptr = (uint8 *)mmgsdi_file_gsm_oplmnwact_attr;
            }

            /* Update file size in the raw attributes */
            mmgsdi_status = mmgsdi_cache_update_file_attributes(event_ptr->session_id,
                                                                MMGSDI_CACHE_UPDATE_FILE_SIZE,
                                                                data.data_len,
                                                                &raw_file_attr_data);
            if(mmgsdi_status != MMGSDI_SUCCESS)
            {
            ...
            mmgsdi_status = mmgsdi_cache_attr_write(event_ptr->session_id,
                                                    &access,
                                                    security_access_ptr,
                                                    FALSE,
                                                    0,
      ...
      else if (file_enum == MMGSDI_TELECOM_ADN)
      {
      ...
      {
        /* Check this file is present in cache list or not */
        if(FALSE == mmgsdi_cache_check_file_is_in_cache_list_or_not(file_enum, session_type))
        {
          /* Since we are registering all files for refresh it is possible that
             we receive some files which are not in MMGSDI cache enum list.
             So we are just updating to success here eithout actually re-reading the same */
          mmgsdi_status = MMGSDI_SUCCESS;
          continue;
        }
        mmgsdi_status = mmgsdi_card_init_cache_binary(
...
#ifdef FEATURE_SIMLOCK
        /* If SP_EHPLMN category is enabled, run SIMLOCK engine as EHPLMN
           data have changed */
        if ((file_enum == MMGSDI_USIM_EHPLMN) &&
            (event_ptr->data.refresh.aid.app_type == MMGSDI_APP_USIM))
        {
          if(mmgsdi_simlock_check_category_status(
                 event_ptr->data.refresh.slot,
  ...
  MMGSDIUTIL_TMC_MEM_FREE(mmgsdi_file_path_ptr);

  return mmgsdi_status;
}/* mmgsdi_refresh_handle_refresh_start_fcn */
  ...
  switch(event_ptr->data.refresh.stage)
  {
    ...
    case MMGSDI_REFRESH_STAGE_START:
      ...
      if((mmgsdi_status == MMGSDI_SUCCESS) &&
         (session_info_ptr != NULL))
      {
        /* update persistent cache FCN case handled below */
      ...
      switch(event_ptr->data.refresh.mode)
      {
        ...
        case MMGSDI_REFRESH_3G_SESSION_RESET:
          ...
          while(node_ptr)
          {
            /* Now MMGSDI is register for card session so card session want to 
               participate  in every FCN refresh */
            if(node_ptr->client_id != mmgsdi_generic_data_ptr->client_id ||
               mmgsdi_util_is_card_session(node_ptr->session_type))
            {
        ...
        case MMGSDI_REFRESH_NAA_FCN:
        {
          ...
          mmgsdi_status = mmgsdi_refresh_handle_refresh_start_fcn(event_ptr);
...
/*===========================================================================
  FUNCTION mmgsdi_refresh_all_register

  DESCRIPTION:
    This function will let MMGSDI register with itself for all files
    for refresh fcn
  ...
  SIDE EFFECTS
    None
 ===========================================================================*/
mmgsdi_return_enum_type mmgsdi_refresh_all_register(
  mmgsdi_session_id_type session_id,
  ...
  boolean                vote_for_init)
{
  ...
  if (mmgsdi_status != MMGSDI_SUCCESS)
  {
    UIM_MSG_ERR_1("MMGSDI register all for refresh command Queuing Failed 0x%x",
                   mmgsdi_status);
  }

  return mmgsdi_status;
} /* mmgsdi_refresh_all_register */
...
)
{
  ...
  path_len = files_path_ptr->path_len;
...
/*===========================================================================
  FUNCTION MMGSDI_REFRESH_PREPARE_FILE_LIST

  DESCRIPTION:
    This function will copy the refresh files list to client specific file list
    according to the client refresh registration.
  ...
  SIDE EFFECTS:
    None
 ===========================================================================*/
static mmgsdi_return_enum_type mmgsdi_refresh_prepare_file_list(
  const mmgsdi_session_info_type               *session_ptr,
  ...
  uint32                                       *num_files_ptr
)
{
  boolean                       is_reg_by_path           = FALSE;
  uint8                         i                        = 0;
  uint8                         j                        = 0;
  uint32                        num_files_temp           = 0;
  uint32                        path_len                 = 0;
  uint32                        num_files                = 0;
  boolean                       is_prov                  = FALSE;
  mmgsdi_slot_id_enum_type      slot_id                  = MMGSDI_SLOT_NONE;
  ...
  MMGSDIUTIL_RETURN_IF_NULL_4(client_spc_refresh_files_ptr, session_ptr,
                              refresh_files_ptr, num_files_ptr);
  ...
  {
    mmgsdi_file_enum_type file = MMGSDI_NO_FILE_ENUM;
    ...
    for(i = 0; i < refresh_files_ptr->num_file_paths ; i++)
    {
      if(refresh_files_ptr->file_path_ptr[i].path_len == 0)
      {
        UIM_MSG_ERR_1("Refresh:Zero path_len at file_path_ptr[0x%x].path_len", i);
        return MMGSDI_ERROR;
      }

      file = mmgsdi_util_convert_path_to_enum(session_ptr->session_id,
                                              refresh_files_ptr->file_path_ptr[i]);

      /* check if the file is not a call stack file. 
         MMGSDI changes refresh mode either from FCN to INIT or from FCN
         to INIT+FCN. It changes the mode so that INIT procedure can be
         run for call stack files. Usually all MMGSDI clients read call stack
         files after SUBSCRIPTION READY event. So there is no need to share
         call stack FCN file list again in FCN mode of refresh. Same with
         3G Session Reset + FCN refresh mode.*/
      if(mmgsdi_refresh_is_call_stack_file(file) && is_prov)
      {
        continue;
      }

      /* Compare the path depending upon the sesion type */
      if ((mmgsdi_refresh_check_path_for_session(session_ptr,
                                                 &refresh_files_ptr->file_path_ptr[i]) == MMGSDI_SUCCESS) ||
          (file == MMGSDI_GSM_SPN) ||
          (file == MMGSDI_GSM_OPL))
      {
        num_files++;
  ...
  else if(!is_reg_by_path)
  {
    ...
    if(session_ptr->refresh_data.refresh_files.num_files > 0)
    {
      for(i = 0; i < refresh_files_ptr->num_files; i++)
      {
        ...
        {
          if(refresh_files_ptr->file_list_ptr[i] ==
             session_ptr->refresh_data.refresh_files.file_list_ptr[j])
          {
            num_files++;
            break;
          }
  ...
  else /* client has registered by file path */
  {
    ...
    if(session_ptr->refresh_data.refresh_files.num_file_paths > 0)
    {
      for(i = 0; i < refresh_files_ptr->num_file_paths; i++)
      {
        if(refresh_files_ptr->file_path_ptr[i].path_len == 0)
        {
          UIM_MSG_ERR_1("Zero path_len at file_path_ptr[0x%x].path_len", i);
          return MMGSDI_ERROR;
        }
        /* check if the file is not a stack file */
        ...
        {
          /* compare path length */
          if((refresh_files_ptr->file_path_ptr[i].path_len ==
             session_ptr->refresh_data.refresh_files.file_path_ptr[j].path_len) &&
             refresh_files_ptr->file_path_ptr[i].path_len <= MMGSDI_MAX_PATH_LEN)
          {
            path_len = refresh_files_ptr->file_path_ptr[i].path_len;
            /* compare path buffers */
            if(!memcmp(
                refresh_files_ptr->file_path_ptr[i].path_buf,
  ...
  if (session_ptr->refresh_data.reg_all_files)
  {
    ...
    for(i = 0; i < refresh_files_ptr->num_file_paths; i++)
    {
          ...
          (file == MMGSDI_GSM_OPL))
      {
        if(num_files_temp < num_files)
        {
          ...
          /* Copy file path */
          client_spc_refresh_files_ptr->file_path_ptr[num_files_temp].path_len =
            refresh_files_ptr->file_path_ptr[i].path_len;
          mmgsdi_memscpy(client_spc_refresh_files_ptr->file_path_ptr[num_files_temp].path_buf,
                         sizeof(client_spc_refresh_files_ptr->file_path_ptr[num_files_temp].path_buf),
                         refresh_files_ptr->file_path_ptr[i].path_buf,
                         path_len * sizeof(uint16));
          num_files_temp++;
        }
    ...
    client_spc_refresh_files_ptr->num_file_paths = num_files_temp;
  }
  else if(!is_reg_by_path)
  {
    /*Allocate memory for the file list*/
    ...
    for(i = 0; i < refresh_files_ptr->num_files; i++)
    {
      ...
      for(j = 0; j < session_ptr->refresh_data.refresh_files.num_files; j++)
      {
           ...
           session_ptr->refresh_data.refresh_files.file_list_ptr[j])
        {
          if( num_files_temp < num_files )
          {
            client_spc_refresh_files_ptr->file_list_ptr[num_files_temp] =
    ...
    client_spc_refresh_files_ptr->num_files       = num_files_temp;
  ...
  *num_files_ptr = num_files_temp;
  return MMGSDI_SUCCESS;
}/* mmgsdi_refresh_prepare_file_list */
...
)
{
  ...
  mmgsdi_status = mmgsdi_refresh_prepare_file_list(session_ptr,
                    &client_spc_refresh_files,
                    refresh_files_ptr,
                    &num_files);

  /* Proceed with adding the node only on success */
  if (mmgsdi_status != MMGSDI_SUCCESS || num_files == 0)
  {
    return mmgsdi_status;
  }
  ...
  MMGSDIUTIL_TMC_MEM_FREE(client_spc_refresh_files.file_list_ptr);
  ...
  uint8                                    slot_index)
{
  ...
  {
    UIM_MSG_ERR_1("Incorrect input parameters for slot_index, 0x%x", slot_index);
    return FALSE;
  }
  ...
  for(i = 0; i < MMGSDI_MAX_CLIENT_INFO; i++)
  {
    ...
    for(j = 0; (num_sessions > 0) && (j < MMGSDI_MAX_SESSION_INFO); j++)
    {
         ...
         (session_ptr->notify_init))
      {
        ...
        {
          UIM_MSG_HIGH_7("Refresh: init not completed yet for Client ID = 0x%x, Session ID = 0x%x, name = %c%c%c%c%c",
                         ...
                         (mmgsdi_client_id_reg_table[i]->client_task_name[0] != 0) ? mmgsdi_client_id_reg_table[i]->client_task_name[0] : ' ',
                         (mmgsdi_client_id_reg_table[i]->client_task_name[1] != 0) ? mmgsdi_client_id_reg_table[i]->client_task_name[1] : ' ',
                         (mmgsdi_client_id_reg_table[i]->client_task_name[2] != 0) ? mmgsdi_client_id_reg_table[i]->client_task_name[2] : ' ',
                         (mmgsdi_client_id_reg_table[i]->client_task_name[3] != 0) ? mmgsdi_client_id_reg_table[i]->client_task_name[3] : ' ',
                         (mmgsdi_client_id_reg_table[i]->client_task_name[4] != 0) ? mmgsdi_client_id_reg_table[i]->client_task_name[4] : ' ');
          return TRUE;
        }
      }
    }
  ...
  uint8                              slot_index)
{
  ...
  if(mmgsdi_session_pause_deactivation_if_needed(msg.request_header.session_id) == FALSE)
  {
...
/* ============================================================================
  FUNCTION:      MMGSDI_IS_FILE_BEING_REFRESHED

  DESCRIPTION:
    This function is a utility to find out if a file is present in the
    global mmgsdi_refresh_info_ptr (that contains the files that have been
    requested to be refreshed as part of FCN or INIT_FCN or 3G SESSION RESET
    refresh types).
    The idea is that if the file is present in the global for FCN or INIT_FCN
    or 3G SESSION RESET refresh modes, the WRITE on that file will be blocked.
...
=============================================================================*/
boolean mmgsdi_is_file_being_refreshed(
  mmgsdi_session_id_type    session_id,
...
)
{
  ...
  do
  {
    ...
    /* If the slot corresponding to the session and that of refresh req are
       different or In case no refresh is in progress, return */
    if(mmgsdi_refresh_info_ptr[slot_index] == NULL ||
       mmgsdi_refresh_info_ptr[slot_index]->mode == MMGSDI_REFRESH_MODE_ENUM_MAX)
    {
      break;
    }
    ...
    if(mmgsdi_refresh_info_ptr[slot_index]->mode == MMGSDI_REFRESH_NAA_INIT_FULL_FCN ||
    ...
    if(mmgsdi_refresh_info_ptr[slot_index]->refresh_data.refresh_files.num_file_paths == 0)
    {
      break;
    }
    ...
    {
      mmgsdi_file = file_access.file.file_enum;
    ...
    {
      (void)mmgsdi_file_get_file_from_path(
              mmgsdi_refresh_info_ptr[slot_index]->aid.app_type,
              file_access.file.path_type,
              &mmgsdi_file);
    ...
    /* At this point, file_path contains the file paths (either by conversion
       from BY_ENUM access OR by BY_PATH access) and mmgsdi_file will contain
       the file_enum */
    ...
    for(i = 0; i < mmgsdi_refresh_info_ptr[slot_index]->refresh_data.refresh_files.num_file_paths ; i++)
    {
      if((mmgsdi_refresh_info_ptr[slot_index]->refresh_data.refresh_files.file_path_ptr[i].path_len == 0) ||
         (file_path.path_len !=
          mmgsdi_refresh_info_ptr[slot_index]->refresh_data.refresh_files.file_path_ptr[i].path_len))
      {
        continue;
      }
      if(!memcmp(mmgsdi_refresh_info_ptr[slot_index]->refresh_data.refresh_files.file_path_ptr[i].path_buf,
                 file_path.path_buf,
    ...
    /* To handle out-of-spec card where card is in UICC mode but the FCN list provided
       contains GSM files. In that case we should block the corresponding
       USIM file write/increase. Handle this for specific files on case basis */
    if (is_present == FALSE &&
        app_info_ptr->app_data.app_type == MMGSDI_APP_USIM &&
        (mmgsdi_file == MMGSDI_USIM_FPLMN ||
         mmgsdi_file == MMGSDI_USIM_LOCI))
    {
      switch (mmgsdi_file)
      {
        case MMGSDI_USIM_FPLMN:
          ...
          break;
        case MMGSDI_USIM_LOCI:
          mmgsdi_file = MMGSDI_GSM_LOCI;
          break;
        default:
          break;
      }

      if (MMGSDI_SUCCESS == mmgsdi_file_get_path_from_file(mmgsdi_file, &file_path))
      {
        for(i = 0; i < mmgsdi_refresh_info_ptr[slot_index]->refresh_data.refresh_files.num_file_paths ; i++)
        {
          if((mmgsdi_refresh_info_ptr[slot_index]->refresh_data.refresh_files.file_path_ptr[i].path_len == 0) ||
  ...
  MMGSDIUTIL_LEAVE_CLIENT_APP_DATA_CRIT_SECT;
  return is_present;
} /* mmgsdi_is_file_being_refreshed */
  ...
  uint32                                 aid_index)
{
  ...
  {
    /* mmgsdi_util_get_mmgsdi_session_on_ch_zero can't be invoked here as it
       uses mmgsdi's session id to find channel index and then app info ptr.
       And after session deactivation channel index is not available. */
    /* If we reach here during APP_RESET_AUTO, app_type in req_ptr is a
       valid app type even for ICC case (even though the aid.data_len
       is zero). For other refresh modes, app_type in req_ptr may be
       MMGSDI_APP_NONE... which is fine as this function will simply
       ignore if app_type is passed as MMGSDI_APP_NONE */
    mmgsdi_status = mmgsdi_util_get_channel_app(req_header_ptr->slot_id,
                                                MMGSDI_DEFAULT_CHANNEL,
                                                req_ptr->aid.app_type,
                                                &sel_app_ptr);
  ...
  mmgsdi_refresh_mode_enum_type        mode)
{
  ...
  UIM_MSG_HIGH_1("FUNCTION mmgsdi_refresh_process_init for slot: 0x%x", slot_id);
...
/*===========================================================================
  ...
  DESCRIPTION:
    ...
    if MMGSDI_NV_PURGE_TEMP_IDENTITIES_SUPPORT_I is enabled for refresh mode
    RESET (or)
    if MMGSDI_NV_PURGE_TEMP_IDENTITIES_SUPPORT_INIT_FCN_I is enabled for 
    refresh mode INIT+FCN
...
)
{
  ...
  /* Do nothing if
     mmgsdi_generic_data_ptr is NULL (or)
     MMGSDI_FEATURE_PURGE_TEMP_IDENTITIES is disabled (or)
     MMGSDI_FEATURE_PURGE_TEMP_IDENTITIES_INIT_FCN_REFRESH_WITH_IMSI_CHANGE is disabled */
  nv_context = mmgsdi_util_get_efs_item_index_for_slot(refresh_slot_id);

  if(!mmgsdi_generic_data_ptr ||
     (MMGSDI_FEATURE_DISABLED ==
        mmgsdi_nv_get_slot_feature_status(MMGSDI_FEATURE_PURGE_TEMP_IDENTITIES, nv_context) &&
      MMGSDI_FEATURE_DISABLED ==
        mmgsdi_nv_get_slot_feature_status(MMGSDI_FEATURE_PURGE_TEMP_IDENTITIES_INIT_FCN_REFRESH_WITH_IMSI_CHANGE,
                                          nv_context)))
  {
    return;
  }
  ...
  for(i = 0; i < (sizeof(session_ids) / sizeof(session_ids[0])); i++)
  {
    ...
    {
      /* Purge EF_LOCI */
      (void) mmgsdi_util_purge_temp_identities(refresh_slot_id,
                                               session_ids[i],
                                               MMGSDI_GSM_LOCI,
                                               protocol);
    }
    else if(MMGSDI_UICC == protocol)
    {
      ...
      if((MMGSDI_SUCCESS == mmgsdi_uicc_chk_srv_available(session_ids[i],
...
/*=============================================================================
  ...
  DESCRIPTION:
    ...
    Purge XXLOCI files if MMGSDI_FEATURE_PURGE_TEMP_IDENTITIES is enabled
    and IMSI file is present in FCN list
  ...
  mmgsdi_client_req_extra_info_type   *extra_param_ptr)
{
  ...
  UIM_MSG_HIGH_1("FUNCTION mmgsdi_refresh_process_init_fcn for slot: 0x%x",
                 slot_id);
  ...
  if(MMGSDI_REFRESH_NAA_INIT_FCN == mmgsdi_refresh_info_ptr[slot_index]->mode)
  {
    /* File paths are converted to enums. So, checking for file list pointer
       to NULL is sufficient */
    if(refresh_files.file_list_ptr != NULL)
    {
      uint8 i = 0;

      for(i = 0; i < refresh_files.num_files &&
                 i < MMGSDI_MAX_FILE_CHANGE_NOTIFICATIONS;
          i++)
      {
        /* Purge temporary identities only if the file list contains IMSI file */
        if(refresh_files.file_list_ptr[i] == MMGSDI_USIM_IMSI ||
           refresh_files.file_list_ptr[i] == MMGSDI_GSM_IMSI)
        {
  ...
  mmgsdi_refresh_mode_enum_type        mode)
{
  ...
  if(mode == MMGSDI_REFRESH_3G_SESSION_RESET &&
     mmgsdi_nv_get_device_feature_status(MMGSDI_FEATURE_REFRESH_3G_SESSION_RESET_PER_ETSI_SPEC) ==
       MMGSDI_FEATURE_ENABLED)
  {
  ...
  if(need_to_send_fcn_flag == FALSE)
  {
    ...
    mmgsdi_refresh_info_ptr[slot_index]->refresh_data.refresh_files.num_files = 0;
    mmgsdi_refresh_info_ptr[slot_index]->refresh_data.refresh_files.num_file_paths = 0;
    MMGSDIUTIL_TMC_MEM_FREE(mmgsdi_refresh_info_ptr[slot_index]->refresh_data.refresh_files.file_list_ptr);
    MMGSDIUTIL_TMC_MEM_FREE(mmgsdi_refresh_info_ptr[slot_index]->refresh_data.refresh_files.file_path_ptr);
  }
  ...
  mmgsdi_slot_id_enum_type             slot_id)
{
  ...
  /* Purge XXLOCI files if needed */
  if(MMGSDI_REFRESH_RESET == mmgsdi_refresh_info_ptr[slot_index]->mode)
  {
    mmgsdi_refresh_check_and_purge_temp_identities(slot_id, slot_ptr->protocol);
  }
  ...
  mmgsdi_client_req_extra_info_type   *extra_param_ptr)
{
  ...
  /* Free the phonebook cache for the files that refreshing currently */
  ...
  /* if card protocol is ICC, we need to delete GSM and CDMA files from app cache if respective
        apps status is deactivated */
  if(slot_ptr != NULL &&
     slot_ptr->protocol == MMGSDI_ICC)
  {
    uint8   index     = 0;
    ...
    {
      if (slot_ptr->app_info_pptr[index] == NULL)
      {
        continue;
      }

      if (slot_ptr->app_info_pptr[index]->app_selected == FALSE &&
          (slot_ptr->app_info_pptr[index]->app_data.app_type == MMGSDI_APP_SIM ||
           slot_ptr->app_info_pptr[index]->app_data.app_type == MMGSDI_APP_RUIM))
      {
        mmgsdi_cache_delete(slot_ptr->app_info_pptr[index],
                            refresh_files.num_files,
                            refresh_files.file_list_ptr);
  ...
  mmgsdi_client_req_extra_info_type *extra_param_ptr)
{
     ...
     mmgsdi_refresh_info_ptr[slot_index]->mode == MMGSDI_REFRESH_RESET_AUTO)
  {
    if(req_ptr->request_header.client_id != mmgsdi_generic_data_ptr->client_id)
    {
      uint32  index  = MMGSDI_MAX_CLIENT_INFO;

      if((mmgsdi_util_get_client_id_index(req_ptr->request_header.client_id,
                                          &index) == MMGSDI_SUCCESS) &&
         (index < MMGSDI_MAX_CLIENT_INFO))
      {
        UIM_MSG_HIGH_7("Unexpected COMPLETE INIT response received from client id low = 0x%x high = 0x%x, name = %c%c%c%c%c",
                       (req_ptr->request_header.client_id & 0xFFFFFFFF),
                       (req_ptr->request_header.client_id >> 32 & 0xFFFFFFFF),
                       (mmgsdi_client_id_reg_table[index]->client_task_name[0] != 0) ?
                              mmgsdi_client_id_reg_table[index]->client_task_name[0] : ' ',
                       (mmgsdi_client_id_reg_table[index]->client_task_name[1] != 0) ?
                              mmgsdi_client_id_reg_table[index]->client_task_name[1] : ' ',
                       (mmgsdi_client_id_reg_table[index]->client_task_name[2] != 0) ?
                              mmgsdi_client_id_reg_table[index]->client_task_name[2] : ' ',
                       (mmgsdi_client_id_reg_table[index]->client_task_name[3] != 0) ?
                              mmgsdi_client_id_reg_table[index]->client_task_name[3] : ' ',
                       (mmgsdi_client_id_reg_table[index]->client_task_name[4] != 0) ?
                              mmgsdi_client_id_reg_table[index]->client_task_name[4] : ' ');
      }
  ...
  mmgsdi_client_req_extra_info_type * extra_param_ptr)
{
  ...
  if((mmgsdi_util_get_client_id_index(req_ptr->request_header.client_id,
                                      &client_index) == MMGSDI_SUCCESS) &&
     (client_index < MMGSDI_MAX_CLIENT_INFO))
  {
    UIM_MSG_HIGH_8("REFRESH COMPLETE Steering response received from client id_low = 0x%x, id_high = 0x%x, with status = 0x%x, name = %c%c%c%c%c",
                   (req_ptr->request_header.client_id & 0xFFFFFFFF),
                   (req_ptr->request_header.client_id >> 32 & 0xFFFFFFFF),
                   req_ptr->refresh.pass_fail,
                   (mmgsdi_client_id_reg_table[client_index]->client_task_name[0] != 0) ?
                          mmgsdi_client_id_reg_table[client_index]->client_task_name[0] : ' ',
                   (mmgsdi_client_id_reg_table[client_index]->client_task_name[1] != 0) ?
                          mmgsdi_client_id_reg_table[client_index]->client_task_name[1] : ' ',
                   (mmgsdi_client_id_reg_table[client_index]->client_task_name[2] != 0) ?
                          mmgsdi_client_id_reg_table[client_index]->client_task_name[2] : ' ',
                   (mmgsdi_client_id_reg_table[client_index]->client_task_name[3] != 0) ?
                          mmgsdi_client_id_reg_table[client_index]->client_task_name[3] : ' ',
                   (mmgsdi_client_id_reg_table[client_index]->client_task_name[4] != 0) ?
                          mmgsdi_client_id_reg_table[client_index]->client_task_name[4] : ' ');
  }
...
)
{
  ...
  {
    UIM_MSG_MED_1("Refresh Retry: NULL global data ptr in retry cb 0x%x", slot_index);
    return;
  }

  UIM_MSG_MED_1("Refresh Retry - mmgsdi_refresh_retry_table: 0x%x", slot_index);

  for(i = 0; i < MMGSDI_REFRESH_RETRY_TABLE_SIZE; i++)
  {
    next_entry = mmgsdi_refresh_retry_table[slot_index][i];

    while(next_entry != NULL)
    {
      UIM_MSG_MED_2("Refresh Retry type: 0x%x, for app: 0x%x",
...
/*===========================================================================
  ...
  SIDE EFFECTS
    None
 ===========================================================================*/
static void mmgsdi_refresh_retry_timer_expiry(
...
)
{
  ...
  if(!mmgsdi_util_is_slot_valid((mmgsdi_slot_id_enum_type)slot_id))
  {
    return;
  }
...
SIDE EFFECTS
 ...
 ===========================================================================*/
static void mmgsdi_refresh_retry_cb(
...
)
{
     ...
     (mmgsdi_status != MMGSDI_REFRESH_ADDITIONAL_FILES_READ))
  {
    ...
    rex_def_timer_ex(&mmgsdi_refresh_retry_timer[slot_index],
                     mmgsdi_refresh_retry_timer_expiry,
                     cnf_ptr->response_header.slot_id);

    (void)rex_set_timer(&mmgsdi_refresh_retry_timer[slot_index],
                        mmgsdi_refresh_retry_settings_ptr[slot_index]->retry_timer);
    return;
  }

  if(mmgsdi_refresh_retry_table_index[slot_index] >= MMGSDI_REFRESH_RETRY_TABLE_SIZE)
  {
    UIM_MSG_ERR_0("Refresh Retry retry index out of bounds.");
    mmgsdi_refresh_retry_table_index[slot_index] = 0;
  }

  i = mmgsdi_refresh_retry_table_index[slot_index];
  ...
  do
  {
    if(mmgsdi_refresh_retry_table[slot_index][i] != NULL)
    {
      mmgsdi_memscpy(&req.refresh.card_req,
                     sizeof(req.refresh.card_req),
                     &mmgsdi_refresh_retry_table[slot_index][i]->card_req,
                     sizeof(mmgsdi_refresh_card_req_type));

      req.refresh_req_type                        = MMGSDI_REFRESH_ORIG_REFRESH_REQ;
      table_entry                                 = mmgsdi_refresh_retry_table[slot_index][i];
      req.refresh.card_req.retry_info.retry_count = table_entry->card_req.retry_info.retry_count++;
      mmgsdi_refresh_retry_table[slot_index][i]   = mmgsdi_refresh_retry_table[slot_index][i]->next;
      req.refresh.card_req.source                 = MMGSDI_REFRESH_SOURCE_MMGSDI;
      ...
      {
        mmgsdi_refresh_retry_table_index[slot_index] = i;
      }
      return;
    }
    i++;
  ...
  }while (i != mmgsdi_refresh_retry_table_index[slot_index]);
...
static mmgsdi_return_enum_type mmgsdi_refresh_retry_get_index(
  mmgsdi_refresh_mode_enum_type refresh_mode,
  uint8                        *retry_index_ptr,
  uint8                         slot_index
)
{
  uint8 index           = 0;
  uint8 first_app_index = MMGSDI_REFRESH_RETRY_PROV_APPS;
  ...
  MMGSDIUTIL_RETURN_IF_NULL_2(retry_index_ptr, mmgsdi_refresh_info_ptr[slot_index]);
  ...
  switch(mmgsdi_refresh_info_ptr[slot_index]->aid.app_type)
  {
    case MMGSDI_APP_SIM:
    ...
    case MMGSDI_APP_CSIM:
      *retry_index_ptr += 3 * MMGSDI_REFRESH_RETRY_MODES;
      break;
    default:
      for(index = (MMGSDI_REFRESH_RETRY_PROV_APPS * MMGSDI_REFRESH_RETRY_MODES);
          index < MMGSDI_REFRESH_RETRY_TABLE_SIZE; index++)
      {
        if(mmgsdi_refresh_retry_table[slot_index][index] != NULL)
        {
          if(mmgsdi_util_cmp_aid(&mmgsdi_refresh_retry_table[slot_index][index]->card_req.aid,
                                 &mmgsdi_refresh_info_ptr[slot_index]->aid,
                                 FALSE) == MMGSDI_SUCCESS)
          {
              *retry_index_ptr = *retry_index_ptr + (index - (index % MMGSDI_REFRESH_RETRY_MODES));
              return MMGSDI_SUCCESS;
          }
          else
          {
            ...
            if(first_app_index == (index / MMGSDI_REFRESH_RETRY_MODES))
            {
              first_app_index++;
            } /* if index has reached the next app */
          } /* if the aids don't match */
        } /* mmgsdi_refresh_retry_table[index] != NULL*/
      } /* for */
...
)
{
  ...
  UIM_MSG_HIGH_1("Refresh Retry: Add Entry slot_index 0x%x", slot_index);
  ...
  refresh_mode = mmgsdi_refresh_info_ptr[slot_index]->orig_mode;

  if(refresh_mode == MMGSDI_REFRESH_RESET_AUTO ||
     refresh_mode == MMGSDI_REFRESH_APP_RESET_AUTO)
  {
    return MMGSDI_SUCCESS;
  }

  if(mmgsdi_refresh_info_ptr[slot_index]->retry_count >=
     mmgsdi_refresh_retry_settings_ptr[slot_index]->retry_limit)
  {
    UIM_MSG_HIGH_2("Refresh Retry: Not adding refresh to retry table. retry_count:0x%x retry_limit:0x%x",
                   mmgsdi_refresh_info_ptr[slot_index]->retry_count,
                   mmgsdi_refresh_retry_settings_ptr[slot_index]->retry_limit);
    return MMGSDI_ERROR;
  }
  ...
  if(mmgsdi_refresh_retry_table[slot_index][MMGSDI_REFRESH_RETRY_RESET] != NULL)
  {
  ...
  if(refresh_mode == MMGSDI_REFRESH_RESET)
  {
    ...
    new_table_entry_ptr->card_req.retry_info.retry_count  = mmgsdi_refresh_info_ptr[slot_index]->retry_count;
    ...
    mmgsdi_refresh_retry_table[slot_index][MMGSDI_REFRESH_RETRY_RESET] = new_table_entry_ptr;
  ...
  {
    if(mmgsdi_refresh_retry_table[slot_index][retry_index] != NULL)
    {
      UIM_MSG_ERR_0("Refresh Retry Steering of roaming already in retry table");
      return MMGSDI_ERROR;
    }
    mmgsdi_util_cal_align_size(sizeof(mmgsdi_refresh_retry_table_entry_type), &table_entry_size);
    MMGSDIUTIL_MEM_MALLOC_AND_VALIDATE(new_table_entry_ptr,
      (table_entry_size + mmgsdi_refresh_info_ptr[slot_index]->steering_mode_data.data_len));
    ...
    mmgsdi_refresh_retry_table[slot_index][retry_index] = new_table_entry_ptr;
    (void)rex_undef_timer(&mmgsdi_refresh_retry_timer[slot_index]);
  ...
  if(mmgsdi_nv_get_device_feature_status(MMGSDI_FEATURE_REFRESH_3G_SESSION_RESET_PER_ETSI_SPEC) ==
       MMGSDI_FEATURE_DISABLED)
  {
    ...
    if(refresh_mode == MMGSDI_REFRESH_3G_SESSION_RESET)
    {
      mmgsdi_util_cal_align_size(sizeof(mmgsdi_refresh_retry_table_entry_type), &table_entry_size);
      MMGSDIUTIL_MEM_MALLOC_AND_VALIDATE(new_table_entry_ptr,
        (table_entry_size + mmgsdi_refresh_info_ptr[slot_index]->refresh_retry_files.data_len));
      ...
      else
      {
        ...
        fcn_node_count = 1;
        table_entry_ptr    = mmgsdi_refresh_retry_table[slot_index][retry_index];
  ...
  mmgsdi_client_req_extra_info_type  *extra_param_ptr)
{
  ...
  if(!mmgsdi_refresh_info_ptr[slot_index]->refresh_data.vote_for_init)
  {
    ...
    if(mmgsdi_refresh_info_ptr[slot_index]->mode != MMGSDI_REFRESH_APP_RESET_AUTO)
    {
      if(mmgsdi_refresh_retry_settings_ptr[slot_index] != NULL)
      {
        mmgsdi_refresh_retry_print_table(slot_index);
  ...
  mmgsdi_client_req_extra_info_type *extra_param_ptr)
{
  ...
  if(!wait_for_rsp)
  {
    ...
    while(node_ptr)
    {  
      /* Now MMGSDI is register for card session but card session will
         participates only in fcn because it need to delete common attr cache files*/
      if(!(mmgsdi_refresh_info_ptr[slot_index]->mode == MMGSDI_REFRESH_NAA_INIT_FCN && 
          mmgsdi_util_is_card_session(node_ptr->session_type)))
      {
        node_ptr->rsp_received = FALSE;
...
)
{
  ...
  if(mmgsdi_refresh_retry_table[slot_index][fcn_index] != NULL)
  {
    /* Compare the current fcn refresh to the first
       fcn in the table by comparing their file lists */
    if((mmgsdi_refresh_retry_table[slot_index][fcn_index]->card_req.refresh_files.data_len ==
        mmgsdi_refresh_info_ptr[slot_index]->refresh_retry_files.data_len) &&
        (memcmp(mmgsdi_refresh_retry_table[slot_index][fcn_index]->card_req.refresh_files.data_ptr,
           mmgsdi_refresh_info_ptr[slot_index]->refresh_retry_files.data_ptr,
           mmgsdi_refresh_info_ptr[slot_index]->refresh_retry_files.data_len) == 0 ))
    {
      /* Clear the matching FCN from the retry table */
      *retries_ptr = mmgsdi_refresh_retry_table[slot_index][fcn_index]->card_req.retry_info.retry_count;
      clear_entry_ptr                       = mmgsdi_refresh_retry_table[slot_index][fcn_index];
      mmgsdi_refresh_retry_table[slot_index][fcn_index] = mmgsdi_refresh_retry_table[slot_index][fcn_index]->next;
      MMGSDIUTIL_TMC_MEM_FREE(clear_entry_ptr);
      return;
    }
    else
    {
      last_entry_ptr  = mmgsdi_refresh_retry_table[slot_index][fcn_index];
      clear_entry_ptr = mmgsdi_refresh_retry_table[slot_index][fcn_index]->next;
    }
  }
  ...
  UIM_MSG_HIGH_1("FCN refresh not currently in retry table 0x%x", slot_index);
...
)
{
  ...
  while(mmgsdi_refresh_retry_table[slot_index][retry_index] != NULL)
  {
    entry_ptr = mmgsdi_refresh_retry_table[slot_index][retry_index];
    mmgsdi_refresh_retry_table[slot_index][retry_index] = entry_ptr->next;
...
)
{
  ...
  for(i = 0; i < MMGSDI_REFRESH_RETRY_TABLE_SIZE; i++)
  {
    mmgsdi_refresh_retry_clear_table_index(slot_index, i);
  }
...
)
{
  ...
  UIM_MSG_HIGH_1("Refresh Retry: Consolidate table 0x%x", slot_index);
  MMGSDIUTIL_RETURN_IF_NULL_2(mmgsdi_refresh_info_ptr[slot_index],
                              mmgsdi_refresh_retry_settings_ptr[slot_index]);

  refresh_mode = mmgsdi_refresh_info_ptr[slot_index]->mode;

  /* If there is a refresh reset all other retry attempts can
     be removed as they will occur as part of the card reset */
  table_entry_ptr = mmgsdi_refresh_retry_table[slot_index][MMGSDI_REFRESH_RETRY_RESET];
  if(table_entry_ptr != NULL)
  {
    if(refresh_mode == MMGSDI_REFRESH_RESET || refresh_mode == MMGSDI_REFRESH_RESET_AUTO)
    {
      mmgsdi_refresh_info_ptr[slot_index]->retry_count = table_entry_ptr->card_req.retry_info.retry_count;
  ...
  if(refresh_mode == MMGSDI_REFRESH_STEERING_OF_ROAMING)
  {
    ...
    if(mmgsdi_refresh_retry_table[slot_index][retry_index] != NULL)
    {
      MMGSDIUTIL_TMC_MEM_FREE(mmgsdi_refresh_retry_table[slot_index][retry_index]);
    }
    return MMGSDI_SUCCESS;
  }
  ...
  if(refresh_mode == MMGSDI_REFRESH_3G_SESSION_RESET)
  {
    ...
    mmgsdi_refresh_retry_clear_table_index(slot_index, retry_index - 1);/*APP_RESET*/
    mmgsdi_refresh_retry_clear_table_index(slot_index, retry_index - 2);/*NAA_INIT_FULL_FCN*/
    mmgsdi_refresh_retry_clear_table_index(slot_index, retry_index - 3);/*NAA_FCN*/
    mmgsdi_refresh_retry_clear_table_index(slot_index, retry_index - 4);/*NAA_INIT_FCN*/
    mmgsdi_refresh_retry_clear_table_index(slot_index, retry_index - 5);/*NAA_INIT*/
    return MMGSDI_SUCCESS;
  }
  ...
  if(table_entry_ptr != NULL)
  {
    if(refresh_mode == MMGSDI_REFRESH_NAA_APP_RESET || refresh_mode == MMGSDI_REFRESH_APP_RESET_AUTO)
    {
      ...
      mmgsdi_refresh_info_ptr[slot_index]->retry_count = table_entry_ptr->card_req.retry_info.retry_count;
      mmgsdi_refresh_retry_clear_table_index(slot_index, retry_index);/*NAA_APP_RESET*/
    }
    return MMGSDI_SUCCESS;
  }
     ...
     refresh_mode == MMGSDI_REFRESH_APP_RESET_AUTO)
  {
    ...
    mmgsdi_refresh_retry_clear_table_index(slot_index, retry_index - 1);/*NAA_INIT_FULL_FCN*/
    mmgsdi_refresh_retry_clear_table_index(slot_index, retry_index - 2);/*NAA_FCN*/
    mmgsdi_refresh_retry_clear_table_index(slot_index, retry_index - 3);/*NAA_INIT_FCN*/
    mmgsdi_refresh_retry_clear_table_index(slot_index, retry_index - 4);/*NAA_INIT*/
    return MMGSDI_SUCCESS;
  }
  ...
  if(table_entry_ptr != NULL)
  {
    if(refresh_mode == MMGSDI_REFRESH_NAA_INIT_FULL_FCN)
    {
      /* Remove the matching retry entry from the table */
      mmgsdi_refresh_info_ptr[slot_index]->retry_count = table_entry_ptr->card_req.retry_info.retry_count;
      mmgsdi_refresh_retry_clear_table_index(slot_index, retry_index);
    }
    return MMGSDI_SUCCESS;
  }
 ...
 )
{
  ...
  if(req_ptr->refresh.card_req.aid.aid.data_len != 0 )
  {
    ...
    {
      UIM_MSG_ERR_0("Refresh: Refresh for app not in the EF-DIR");
      extra_param_ptr->refresh_data.stage = MMGSDI_REFRESH_STAGE_END_SUCCESS;
      slot_ptr->refresh_state             = MMGSDI_REFRESH_READY_STATE;
      return MMGSDI_REFRESH_SUCCESS_NO_AID_ACTIVE;
    }
  ...
  switch(req_ptr->refresh.card_req.refresh_mode)
  {
    ...
    case MMGSDI_REFRESH_NAA_FCN:
      ...
      break;
    case MMGSDI_REFRESH_STEERING_OF_ROAMING:
      mmgsdi_status = mmgsdi_refresh_process_steering(slot_ptr,
                                                      &req_ptr->refresh.card_req,
                                                      extra_param_ptr);
      break;
    default:
...
)
{
  ...
  if(mmgsdi_refresh_retry_table_index[slot_index] >= MMGSDI_REFRESH_RETRY_TABLE_SIZE)
  {
    UIM_MSG_ERR_1("Refresh Retry retry index out of bounds.0x%x", slot_index);
    mmgsdi_refresh_retry_table_index[slot_index] = 0;
  }
  ...
  do
  {
    ...
    if(i == MMGSDI_REFRESH_RETRY_TABLE_SIZE)
    {
      if(mmgsdi_refresh_retry_table_index[slot_index] == 0)
      {
  ...
  mmgsdi_refresh_req_type *req_ptr)
{
  ...
  /* When the MMGSDI state is not initialized and we receive REFRESH request
     other than reg/dereg (this generally happens when we try to switch the
     profile at the boot up) with mode as RESET then we skip the voting from
     the clients as we did not send the CARD_INSERTED event. We build the card
     reset command, initialize the globals and clean up the sessions. */
  ...
  switch (req_ptr->refresh_req_type)
  {
    case MMGSDI_REFRESH_ORIG_AUTO_REFRESH_REQ:
      ...
      {
        mmgsdi_refresh_retry_handle_req(slot_index);
        break;
      }
      ...
      switch(orig_state)
      {
       ...
       ---------------------------------------------------*/
        case MMGSDI_REFRESH_READY_STATE:
          ...
          /* if Refresh mode is INIT/INIT FCN/INIT FFCN and card protocol
             is ICC, we need to delete GSM and CDMA app cache if respective
             apps status is deactivated */
         if(mmgsdi_util_determine_protocol(req_ptr->request_header.slot_id) == MMGSDI_ICC &&
            ((req_ptr->refresh.card_req.refresh_mode == MMGSDI_REFRESH_NAA_INIT) ||
             (req_ptr->refresh.card_req.refresh_mode == MMGSDI_REFRESH_NAA_INIT_FCN) ||
             (req_ptr->refresh.card_req.refresh_mode == MMGSDI_REFRESH_NAA_INIT_FULL_FCN)))
          {
            uint8   loop_index = 0;
            boolean loop_done  = FALSE;
            ...
            if(slot_data_ptr != NULL)
            {
              for (loop_index = 0; loop_index < MMGSDI_MAX_PROV_APPS; loop_index++)
              {
                if (slot_data_ptr->app_info_pptr[loop_index] == NULL)
                {
                  continue;
                }

                if (slot_data_ptr->app_info_pptr[loop_index]->app_selected == FALSE &&
                    (slot_data_ptr->app_info_pptr[loop_index]->app_data.app_type == MMGSDI_APP_SIM ||
                     slot_data_ptr->app_info_pptr[loop_index]->app_data.app_type == MMGSDI_APP_RUIM))
                {
                  mmgsdi_cache_delete(slot_data_ptr->app_info_pptr[loop_index], 0, NULL);
                  (void)mmgsdi_cache_delete_attr_selective_items(
                          &slot_data_ptr->app_info_pptr[loop_index]->attr_cache_tbl,
                          NULL, 0, NULL, 0);
                  mmgsdi_csg_reset_eons_flags_in_app_info(slot_data_ptr->app_info_pptr[loop_index]);
                  mmgsdi_eons_opl_pnn_delete(&slot_data_ptr->app_info_pptr[loop_index]->opl_cache);
                  mmgsdi_eons_opl_pnn_delete(&slot_data_ptr->app_info_pptr[loop_index]->pnn_cache);

                  if(slot_data_ptr->app_info_pptr[loop_index]->cphs_info_ptr != NULL)
                  {
                    MMGSDIUTIL_TMC_MEM_FREE(
                      slot_data_ptr->app_info_pptr[loop_index]->cphs_info_ptr->cphs_ons_ptr);
                    MMGSDIUTIL_TMC_MEM_FREE(
                      slot_data_ptr->app_info_pptr[loop_index]->cphs_info_ptr->cphs_ons_short_ptr);
                    MMGSDIUTIL_TMC_MEM_FREE(slot_data_ptr->app_info_pptr[loop_index]->cphs_info_ptr);
                  }
                  /* Clear orig. cache info */
                  if(slot_data_ptr->app_info_pptr[loop_index]->srv_table_orig_cache.data_ptr != NULL)
                  {
                    MMGSDIUTIL_TMC_MEM_FREE(
                      slot_data_ptr->app_info_pptr[loop_index]->srv_table_orig_cache.data_ptr);
                    slot_data_ptr->app_info_pptr[loop_index]->srv_table_orig_cache.data_len = 0;
                  }  
          ...
          if((req_ptr->refresh.card_req.refresh_mode != MMGSDI_REFRESH_RESET) &&
             ...
             (mmgsdi_util_check_app_termination_skipped(req_ptr->request_header.slot_id,
                                                        &req_ptr->refresh.card_req.aid.aid,
                                                        &channel_id)))
          {
            if(req_ptr->refresh.card_req.refresh_mode == MMGSDI_REFRESH_NAA_FCN)
            {
              /* Return SUCCESS for FCN refresh after removing the file cache.
                 As there is no active session, it is not required to refresh
                 the files.*/
              extra_param_ptr->refresh_data.mode = req_ptr->refresh.card_req.refresh_mode;
              extra_param_ptr->refresh_data.stage = MMGSDI_REFRESH_STAGE_END_SUCCESS;
              extra_param_ptr->refresh_data.send_evt = FALSE;
              ...
              {
                mmgsdi_internal_refresh_file_list_type  refresh_files = {0};
                mmgsdi_int_app_info_type               *app_int_info_ptr  = NULL;
                uint32                                  aid_index     = 0;
                ...
                /* Extract file list from refresh request */
                if(mmgsdi_refresh_malloc_and_convert_file_path(
                     &req_ptr->refresh.card_req,
                     slot_index,
                     &refresh_files) != MMGSDI_SUCCESS)
                {
                  if(mmgsdi_refresh_info_ptr[slot_index])
                  {
                    mmgsdi_refresh_free_refresh_info_type(&mmgsdi_refresh_info_ptr[slot_index]);
                ...
                if(req_ptr->refresh.card_req.aid.aid.data_len != 0)
                {
                  if(mmgsdi_data_slot_ptr[slot_index]->app_info_pptr != NULL)
                  {
                    app_int_info_ptr = mmgsdi_data_slot_ptr[slot_index]->app_info_pptr[aid_index];
                  }
                }
                else
                {
                  mmgsdi_status = mmgsdi_util_get_channel_app(
                                    req_ptr->request_header.slot_id,
                                    MMGSDI_DEFAULT_CHANNEL,
                                    MMGSDI_APP_NONE,
                                    &app_int_info_ptr);
                ...
                if(app_int_info_ptr != NULL)
                {
                  mmgsdi_cache_delete(app_int_info_ptr,
                                      refresh_files.num_files,
                                      refresh_files.file_list_ptr);
                }
  ...
  /* Freeing temporary PB cache to avoid stale data after refresh */
  if(extra_param_ptr->refresh_data.stage == MMGSDI_REFRESH_STAGE_START &&
     extra_param_ptr->refresh_data.mode != MMGSDI_REFRESH_NAA_FCN)
  {
    mmgsdi_pb_free_cache_data(req_ptr->request_header.slot_id, MMGSDI_INVALID_SESSION_ID, TRUE);
  }
      ...
      else if(req_header.orig_request_type == MMGSDI_SESSION_ACT_OR_SWITCH_PROV_REQ)
      {
        ...
        else
        {
          ...
          else
          {
            ...
            msg.request_header.client_data  = (mmgsdi_client_data_type)index;
            ...
            if(mmgsdi_status == MMGSDI_SUCCESS)
            {
              ...
              if(cnf_ptr->response_header.slot_id != req_header.slot_id)
              {
                ...
                if((MMGSDI_SUCCESS == mmgsdi_status)            &&
                   ...
                   MMGSDIUTIL_SLOT_INDEX_VALID(slot_index_req)  &&
...
)
{
  ...
  {
    UIM_MSG_ERR_1("Could not read refresh retry settings 0x%x", slot_index);
    MMGSDIUTIL_TMC_MEM_FREE(mmgsdi_refresh_retry_settings_ptr[slot_index]);
    return MMGSDI_ERROR;
  }
...
)
{
  ...
  {
    while(mmgsdi_refresh_retry_table[slot_index][retry_index + i] != NULL)
    {
      entry_ptr = mmgsdi_refresh_retry_table[slot_index][retry_index + i];
      mmgsdi_refresh_retry_table[slot_index][retry_index + i] = entry_ptr->next;
      MMGSDIUTIL_TMC_MEM_FREE(entry_ptr);
    }
  }
...
)
{
  ...
  if(refresh_info_pptr && (*refresh_info_pptr))
  {
    ...
    while((*refresh_info_pptr)->node_ptr)
    {
      ...
      MMGSDIUTIL_TMC_MEM_FREE(node_ptr->refresh_files.file_list_ptr);
      MMGSDIUTIL_TMC_MEM_FREE(node_ptr->refresh_files.file_path_ptr);
    ...
    MMGSDIUTIL_TMC_MEM_FREE((*refresh_info_pptr)->steering_mode_data.data_ptr);

    /* Free refresh_retry_files */
    MMGSDIUTIL_TMC_MEM_FREE((*refresh_info_pptr)->refresh_retry_files.data_ptr);
