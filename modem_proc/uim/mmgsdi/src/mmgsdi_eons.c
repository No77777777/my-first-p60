...
Copyright (c) 2011 - 2017 QUALCOMM Technologies, Inc (QTI) and its licensors.
...
$Header: //components/rel/uim.mpss/6.1.0/mmgsdi/src/mmgsdi_eons.c#2 $$ $DateTime: 2019/10/30 06:56:05 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
05/19/17   nr      Creating a new efs item for override_opl_pnn_lookup_for_non_rplmn
12/28/16   bcho    Use EF-SPN buffer for the length read from card
12/09/16   bcho    Check EF-SPN length before using it
07/05/16   ar      Removing array for selective logging
06/24/16   ar      EONS selective logging
06/22/16   tkl     Stub out additional APIs
06/10/16   vdc     Reduce repeated F3 logging in EONS
05/24/16   bcho    EONS F3 reduction
05/20/16   vdc     Remove F3 messages for memory allocation failure
04/28/16   bcho    Disable CHPS functionality for IOE LTE Only devices
...
07/21/15   av      Remove unused function
04/29/15   bcho    Support for App specific cache
04/03/15   av      Migrate to uim_common EFS APIs
03/20/15   bcho    Copy additional name also in API cnf
  ...
  mmgsdi_plmn_id_type nw_plmn)
{
     ...
     ((sim_plmn.plmn_id_val[1] & 0x0F) == (nw_plmn.plmn_id_val[1] & 0x0F)))
  {
    ...
    if(mmgsdi_nv_get_device_feature_status(MMGSDI_FEATURE_NORTH_AMERICAN_PLMN_COMPARISON) ==
...
)
{
  ...
  MMGSDIUTIL_TMC_MEM_FREE(spdi_data.data_ptr);
...
)
{
  ...
  {
    MMGSDIUTIL_TMC_MEM_FREE(spn_data.data_ptr);
    return;
  }
  ...
  *display_byte_ptr = spn_data.data_ptr[MMGSDI_EONS_SPN_DISP_COND_POS];
  ...
  if (spn_data.data_len > MMGSDI_EONS_SPN_DATA_BUF_POS)
  {
    ...
    {
      /* SPN Buffer does not have unused bytes with 0xFF, valid buffer length is
         buffer length - 1 or 16, whichever is minimum */
      spn_ef_len = (uint8)MIN(spn_data.data_len - 1, MMGSDI_EONS_SPN_DATA_LEN);
    }
  }
  ...
  if(spn_ef_len)
  {
    ...
    MMGSDIUTIL_TMC_MEM_FREE(eons_info_ptr->spn.eons_data.data_ptr);
...
/*===========================================================================
FUNCTION MMGSDI_EONS_PRINT_PLMN_INFO_NAMES
...
)
{
  ...
  return TRUE;
}/* mmgsdi_eons_populate_get_operator_name_cnf */
  ...
  mmgsdi_plmn_info_type * plmn_info_ptr)
{
  ...
  MMGSDIUTIL_TMC_MEM_FREE(dst_plmn_name_ptr->plmn_name.eons_data.data_ptr);
...
)
{
  ...
  mmgsdi_feature_override_feature_status_enum_type  override_opl_pnn_lookup_for_rplmn = MMGSDI_OVERRIDE_FEATURE_NO_OVERRIDE;
  ...
  {
    allow_run_algo = mmgsdi_eons_opl_pnn_cache_ready(int_app_info_ptr);
  }

  override_opl_pnn_lookup_for_rplmn = mmgsdi_nv_get_override_opl_pnn_lookup_for_non_rplmn_feature_status(slot_id);

  /* Do not override device specifc feature if the override_opl_pnn_lookup_for_non_rplmn
     file is not present otherwise override with the value configured no matter what is
     there in the device_features_list EFS item.
  */
  if (override_opl_pnn_lookup_for_rplmn == MMGSDI_OVERRIDE_FEATURE_NO_OVERRIDE)
  {
    opl_pnn_lookup_for_rplmn =
      mmgsdi_nv_get_device_feature_status(MMGSDI_FEATURE_OPL_PNN_LOOKUP_FOR_NON_RPLMN);
  }
  else
  {
    opl_pnn_lookup_for_rplmn = (override_opl_pnn_lookup_for_rplmn == MMGSDI_OVERRIDE_FEATURE_ENABLED) ? MMGSDI_FEATURE_ENABLED : MMGSDI_FEATURE_DISABLED;
  }
  ...
  if (eons_trigger == MMGSDI_EONS_TRIGGER_SS_EVENT)
  {
    ...
    if(mmgsdi_status != MMGSDI_SUCCESS)
    {
      MMGSDIUTIL_TMC_MEM_FREE(plmn_id_list.plmn_list_ptr);
      return MMGSDI_ERROR;
    }
  }
...
)
{
      ...
      in_plmn_ptr->plmn_name.eons_data.data_ptr != NULL)
  {
    ...
    MMGSDIUTIL_TMC_MEM_FREE(out_plmn_ptr->plmn_name.eons_data.data_ptr);
     ...
     in_plmn_ptr->plmn_name.eons_data.data_ptr != NULL)
  {
    ...
    mmgsdi_memscpy(out_plmn_ptr->plmn_name.eons_data.data_ptr,
      out_plmn_ptr->plmn_name.eons_data.data_len,
