...
Copyright (c) 2004 - 2017, 2020 QUALCOMM Technologies, Inc (QTI) and its licensors.
...
$Header: //components/rel/uim.mpss/6.1.0/mmgsdi/src/mmgsdi_icc.c#4 $$ $DateTime: 2020/05/31 06:36:18 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
05/26/20   sch     Fix to voice call failure due to file absence of EF_phase
05/12/20   vgd     Set all_srv_init to True when device is in onchip ICC mode
03/14/17   nc      Better checks for return values 
10/10/16   me      Removed FEATURE_GSTK flags
06/09/16   ar      Send FDN event source along with FDN event
06/06/16   ar      Send FDN event if FDN status changed
05/13/16   tkl     Review of macros used by MMGSDI
05/16/16   ar      F3 reduction
03/21/16   bcho    Make function static which has scope in this file
...
)
{
 ...
 /*---------------------------------------------------------------------------
  ...
  if (mmgsdi_status != MMGSDI_SUCCESS)
  {
    UIM_MSG_ERR_0("Unable to get free client request table index");
...
)
{
  ...
  if ((mmgsdi_status != MMGSDI_SUCCESS) || (rehab_cnf_ptr == NULL))
  {
    ...
    MMGSDIUTIL_TMC_MEM_FREE(rehab_cnf_ptr);
    return MMGSDI_ERROR;
  }
...
)
{
  ...
  if (slot_data_ptr->sim_phase == MMGSDI_SIM_PHASE_NOT_INIT)
  {
    ...
    if (mmgsdi_status == MMGSDI_NOT_FOUND)
    {
      ...
      /* If mmgsdi feature absent_mandatory_files_init is enabled, file is not
         mandatory */
      if (mmgsdi_nv_get_slot_feature_status(MMGSDI_FEATURE_ABSENT_MANDATORY_FILES_INIT, nv_context) !=
  ...
  {
    /* Spec 11.11
       Under section 10.2.19
       If EFPhase is coded '00', it may be assumed by the ME that some Phase 2 features are supported by this
       SIM. However, the services n°3 (FDN) and/or n°5 (AoC) shall only be allocated and activated in SIMs of
       phase 2 or later with EFPhase being coded '02' or greater.If EFPhase is coded ‘03’ or greater, an ME supporting SIM Application Toolkit shall perform the PROFILE
       DOWNLOAD procedure
    ...
    */
    ...
    app_info_ptr->app_capabilities.bdn_state        = MMGSDI_SRV_NOT_INIT;
    app_info_ptr->app_capabilities.cc_state         = MMGSDI_SRV_NOT_INIT;
    app_info_ptr->app_capabilities.acl_state        = MMGSDI_SRV_NOT_INIT;
    app_info_ptr->app_capabilities.imsi_invalidated = FALSE;
    app_info_ptr->app_capabilities.all_srv_init     = TRUE;
    UIM_MSG_HIGH_4("App capabilities fdn_state = %x,bdn_state = %x,cc_state = %x,acl_state = %x",
                    app_info_ptr->app_capabilities.fdn_state,app_info_ptr->app_capabilities.bdn_state,
                    app_info_ptr->app_capabilities.cc_state,app_info_ptr->app_capabilities.acl_state);
    return MMGSDI_SUCCESS;
  }
...
/*===========================================================================
  ...
  SIDE EFFECTS:
     None
===========================================================================*/
static mmgsdi_return_enum_type mmgsdi_icc_onchip_populate_run_gsm_data (
