...
Copyright (c) 2001 - 2020 QUALCOMM Technologies, Inc (QTI) and its licensors.
...
$Header: //components/rel/uim.mpss/6.1.0/mmgsdi/src/mmgsdi.c#6 $$ $DateTime: 2020/10/18 15:27:05 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
09/11/20   sg      Consider call ID and previous call type during call switch
05/13/20   sch     Fix to disable RUIM+SIM app sim from UI at the first go
...
20/05/19   kb      Support for card reset on refresh failure 
02/25/19   vm      Clear common telecom cache during FCN Refresh
08/17/18   rps     Add support for mini dump phase 2
06/01/18   dd      Enable toolkit fetch for eUICC without USIMs
08/28/17   nc      Change card err report and notify order
08/01/17   me      De-register for QMI task ready signals only once
06/28/17   me      Remove limitation of LTE on primary subscription
05/30/17   me      Extended rat balancing support check for single device 
05/19/17   me      Use UIM common utility to determine LTE support
05/04/17   tq      SIM profile Query should return error if SIM is not present
05/04/17   bcho    Don't queue command to UIMDRV if last one is not responded
...
04/02/17   vdc     Release all sessions of a session type when select AID fails
...
03/09/17   nr      Marking SILENT_FILE_CHANGE as non session specific command
02/27/17   vdc     Remove commands for internal queue while deactivation
...
12/29/16   bcho    Acknowledge back to slot task for cancelling sync command
12/28/16   ll      Add test mode logging enabled APIs for sensitive info control
10/25/16   me      Removed all functions under the FEATURE_GSTK
...
10/05/16   me      Removed F3 for mmgsdi_present_in_preinit_file_access_list
10/03/16   dd      Removed lint comments
09/26/16   me      Added support for Byte 10,11,12 mmgsdi_set_ens_support_flag
09/22/16   gm      Added support for set APDU behavior
09/13/16   bcho    Reset subscription provisioning info on card error
09/02/16   vdc     Extend full and expired service policies to any slot
...
07/01/16   tkl     F3 reduction
06/22/16   tkl     Stub out additional APIs
06/14/16   sp      Remove command type parameter from validate_client_id function 
06/09/16   shr     Deprecate QMI SIM Internal
06/07/16   ar      Queue MMGSDI_CONTINUE_AFTER_NV_REFRESH req to internal cmd q
...
05/13/16   tkl     Get protocol after slot is ready in mmgsdi_process_cmd
...
05/02/16   gm      Removed "FEATURE_TABASCO_MODEM"
04/29/16   vdc     Removed logging for refresh registration
04/26/16   ar      Removing the mmgsdi_cnf_map_req_to_cnf_enum_type()
04/27/16   bcho    building bootup logs
...
04/07/16   vdc     Feature guard 1X refresh files with FEATURE_MMGSDI_3GPP2
04/06/16   sp      Add DSDS onchip support
04/01/16   vdc     Send act resp and session changed evt at same time
03/25/16   av      Clear ATR related globals at card error/link est
03/22/16   ssr     Removal of NVRUIM Task for IoE variant
03/18/16   tkl     Cancel read request between refresh stage start & deactivate
03/16/16   gm      Removed "FEATURE_TABASCO_MODEM"
03/14/16   ar      No need to clear activate request during cancelling commands
...
02/25/16   vdc     Determine number of profiles based on card response
02/22/16   gm      Deprecate mmgsdi_option_type
02/10/16   kv      Update EF-TST 3G byte based on modem capability
02/05/16   kv      Handle SEARCH request from nvruim in high pri queue
...
01/18/16   sp      Reduce F3 logging
01/17/16   kv      Handle sync cmd cancel for external cancel scenarios
12/19/15   vdc     Send SAP disconnect evt if card is powered up in SAP mode
12/18/15   ar      Send error cnf for all req if MMGSDI task ended
12/16/15   ar      Add utility to check MMGSDI task
...
11/26/15   vdc     Read RAT mode file only for USIM application
11/16/15   nr      Adding MMGSDI_TELECOM_ADN to refresh reg file list
11/10/15   yt      Remove cmd_rsp_cb_ptr from mmgsdi_client_id_reg_table_type
11/06/15   bcho    Reduction in size of rplmn_info.txt
11/02/15   stv     Rel 12 refresh enforcement policy support
10/27/15   ar      Don't send cnf during flush if the rsp_status is PROCESSING
10/27/15   vdc     Send profile switching evt when profile has switched in card
10/16/15   vv      Use mcs wrappers for rcevt APIs
10/16/15   sp      Move client_req_table_info index variables to unsigned
10/12/15   ar      Return generic error if the req is not file_access.
10/12/15   kv      Update session ID and client ID generation mechanism
10/12/15   vdc     Added support for USAT application pairing
10/05/15   sp      Reduce signed/unsigned conversions in UIM
10/07/15   yt      Handle client deregistration in slot task
09/24/15   vv      Check EF-TST length before setting the data
09/16/15   tkl     Make MDN related EF read before SIM Lock has been validated
09/11/15   at      Updating timed sleep API to use mmgsdi_wait
...
08/31/15   av      Pack imsi_m activation status into subscription_ready_evt
08/31/15   yt      Remove length information from requests and confirmations
08/31/15   bcho    Run SIMLOCK Algo in Onchip SIM
08/30/15   vr      Don't wait for TASK critical section for non MMGSDI sigs.
08/28/15   hh      Removed JCDMA related code
08/25/15   bcho    Remove unused parameter
08/19/15   gm      Update the refresh file list
...
07/22/15   yt      Support for receiving events for hidden CSIM
07/21/15   yt      Perform ENS operations during session activated processing
07/10/15   spp     Synchronize main task with slot tasks at boot up
07/08/15   hh      Check mmgsdi_internal_synch_uim_rpt_rsp for sync cmd response
06/25/15   hh      Return generic error for non-permission related reasons
06/24/15   kv      Change the default RTRE value to RUIM instead of NV
06/18/15   ar      Validate PRL at diff steps if SD task not ready
06/16/15   yt      Handle cancellation of sync commands during deactivation
06/15/15   lm      Remove redundant code in slot index validation
06/14/15   hh      Convert MANAGE CHANNEL APDU to MMGSDI requests
06/12/15   ar      Increasing the size of mmgsdi_client_req_table_info_ptr
06/11/15   ar      Check for uim_rsp_status before setting it to NOT_RECEIVED
...
05/27/15   ar      Handle sending double confirmation to client
05/25/15   vdc     Add timestamp for commands queued to MMGSDI
05/22/15   nr      Block search request when deactivation in progress
05/20/15   vdc     Fix compiler warnings
05/18/15   yt      Support for mapping sessions to slots
05/18/15   nr      Fix to distiguish reads on different apps in single slot
05/01/15   vdc     Enter crit sect before updating slot data global
04/29/15   hh      Differentiate CSG support for W and LTE for EF-TST
04/29/15   bcho    Support for App specific cache
04/29/15   hh      Differentiate CSG support for W and LTE for EF-TST
04/27/15   av      Process deactivation faster
04/25/15   av      Support for hiding CSIMs from modem clients
04/16/15   vdc     Send card error event when SIM detection has started
04/10/15   vv      Added support for the ProSe EFs
04/10/15   nr      Added support for 256 bands (LTE U related)
04/08/15   at      Support for SIM detected usecase
04/06/15   ar      Remove duplicate code
04/01/15   stv     Set current_active_profile only if there is a provision from card
03/25/15   av      Allow back to DEACT of different apps on same slot
...
02/12/15   hh      UE supports LTE only if at least one LTE band is enabled
01/19/15   bcho    Add debug print for client data
...
11/20/14   nr      Send TR to the card if initialization gets stuck in perso state
...
1. mmgsdi_cache_crit_sect           | Used to protect acceses to
                                    ...
                                    | c. mmgsdi_common_cache_slot3
                                    | d. app cache in app info
                                    |
                                    | File attributes cache globals:
                                    | a. mmgsdi_slot_common_cache_tbl_ptr
                                    | d. attr cache tbl ptr in app info
--------------------------------------------------------------------------------
...
/* Timer related to REFRESH retry for multi slot */
rex_timer_type                        mmgsdi_refresh_retry_timer[MMGSDI_MAX_NUM_SLOTS];

/* MMGSDI UIM Interface Sanity Timer */
rex_timer_type                        mmgsdi_uim_sanity_timer[MMGSDI_NUM_TASKS];

/* MMGSDI session deactivation pause timer for GW sessions */
rex_timer_type                        mmgsdi_session_deactivation_pause_timer[MMGSDI_NUM_ATTRIBUTES];
...
/* To identify that QMI UIM SIG has been received and MMGSDI's Main Task is
   processing it. */
boolean mmgsdi_process_qmi_uim_sig = FALSE;

/* To indicate that a sync command has been canceled for the corresponding slot.
   This flag is used to block further commands on the slot after cancellation. */
boolean mmgsdi_sync_cmd_canceled[MMGSDI_MAX_NUM_SLOTS] = {FALSE};

/* To indicate if there is a need for sync cmd cancel ack sig */
boolean mmgsdi_need_sync_cmd_cancel_ack[MMGSDI_MAX_NUM_SLOTS] = {FALSE};
  ...
  mmgsdi_task_cmd_type           * task_cmd_ptr)
{
  ...
  switch (task_cmd_ptr->cmd.cmd_enum)
  {
    ...
    case MMGSDI_MAP_SESSIONS_TO_SLOTS_REQ:
    case MMGSDI_SET_APDU_BHEAVIOR_REQ:        
    ...
    case MMGSDI_PURGE_TEMPORARY_IDENTITY_REQ:
    ...
    case MMGSDI_CONTINUE_DEACTIVATE_REQ:
      break;
  ...
  mmgsdi_cmd_enum_type       cmd)
{
  switch (cmd)
  {
    ...
    /* Note - There is no default case here and it is done on purpose since we
       need to handle all values for mmgsdi_cmd_enum_type.*/
      if((cmd == MMGSDI_SEND_APDU_REQ ||
          cmd == MMGSDI_SEND_APDU_EXT_REQ ||
          cmd == MMGSDI_MANAGE_CHANNEL_REQ ||
          cmd == MMGSDI_SET_APDU_BHEAVIOR_REQ)
         && mmgsdi_nv_get_device_feature_status(MMGSDI_FEATURE_ALLOW_SEND_APDU_ONLY) == MMGSDI_FEATURE_ENABLED)
      {
        UIM_MSG_HIGH_1("Proceed with APDU 0x%x command irrespective of card state",
                       cmd);
        return FALSE;
      }
      return TRUE;
  ...
  const mmgsdi_task_cmd_data_type *cmd_ptr)
{
  ...
  if(mmgsdi_nv_get_device_feature_status(MMGSDI_FEATURE_ALLOW_ACCESS_BEFORE_APP_READY) ==
  ...
  mmgsdi_return_enum_type  status)
{
  ...
  task_cmd_len = sizeof(mmgsdi_callback_cnf_type) + mem_size;
...
DESCRIPTION
  ...
  from the queue.
  If cancel request type from cancel_info is set, then only the request type
  commond is deleted from the queue.
...
)
{
  ...
  while (command_ptr != NULL)
  {
    ...
    /* Delete command in queue only either if the cancel_request_type is
     not set, or if it is set, the command request type is matching with
     cancel_info request type. */
    if (!cancel_info.cancel_request_type.is_valid ||
        cancel_info.cancel_request_type.request_type ==
                    command_ptr->cmd.cmd.read_req.request_header.request_type)
    {
      ...
      switch (cancel_info.cancel_type)
      {
        ...
        case MMGSDI_CANCEL_USING_SESSION_ID:
          ...
          break;
        case MMGSDI_CANCEL_USING_SESSION_TYPE:
          /* Don't remove the activate request during clean-up. Session_type is
             used as cancel_type only from deactivate_start() function and during
             de-activation of that session, we should not remove any activation
             req of that session from queue. */
          if(mmgsdi_util_is_prov_session(cancel_info.id.session_type) &&
             MMGSDI_SUCCESS ==
               mmgsdi_util_get_session_type(command_ptr->cmd.cmd.read_req.request_header.session_id,
                                            &session_type, NULL) &&
             cancel_info.id.session_type == session_type &&
             command_ptr->cmd.cmd_enum != MMGSDI_SESSION_ACT_OR_SWITCH_PROV_REQ)
          {
            remove_from_queue = TRUE;
          }
          break;
        case MMGSDI_CANCEL_ALL:
          /* Cancel all requests */
          remove_from_queue = TRUE;
          break;
        default:
    ...
    if (remove_from_queue)
    {
      ...
      /* If the MMGSDI task ended, then send the error cnf to clients for
         all the requests queued in MMGSDI request queues */
      else if (task_state == MMGSDI_TASK_ENDED)
      {
        mmgsdi_callback_cnf_type       *mmgsdi_callback_cnf_ptr  = NULL;

        /* Obtain memory for confirmation */
        MMGSDIUTIL_MEM_MALLOC_AND_VALIDATE(mmgsdi_callback_cnf_ptr,
                                           sizeof(mmgsdi_callback_cnf_type));
        if(mmgsdi_callback_cnf_ptr != NULL)
        {
          mmgsdi_callback_cnf_ptr->mmgsdi_cnf.response_header.response_type =
                             command_ptr->cmd.cmd.request_header.orig_request_type;

          /* Construct the mmgsdi cnf response header in order to notify the client */
          mmgsdi_callback_cnf_ptr->mmgsdi_cnf.response_header.slot_id             = command_ptr->cmd.cmd.request_header.slot_id;
          mmgsdi_callback_cnf_ptr->mmgsdi_cnf.response_header.client_data         = command_ptr->cmd.cmd.request_header.client_data;
          mmgsdi_callback_cnf_ptr->mmgsdi_cnf.response_header.session_id          = command_ptr->cmd.cmd.request_header.session_id;
          mmgsdi_callback_cnf_ptr->mmgsdi_cnf.response_header.client_id           = command_ptr->cmd.cmd.request_header.client_id;
          mmgsdi_callback_cnf_ptr->mmgsdi_cnf.response_header.status_word.present = FALSE;
          mmgsdi_callback_cnf_ptr->mmgsdi_cnf.response_header.mmgsdi_status       = MMGSDI_ERROR;

          /* Lets call the client callback with cnf information. There is no
             need to call mmgsdi_process_cnf() because we don't want to send
             event to other clients as MMGSDI has not processed this req and
             is just sending the error response to original client */
          if (command_ptr->cmd.cmd.request_header.response_cb != NULL)
          {
            (command_ptr->cmd.cmd.request_header.response_cb)(
                      mmgsdi_callback_cnf_ptr->mmgsdi_cnf.response_header.mmgsdi_status,
                      mmgsdi_callback_cnf_ptr->mmgsdi_cnf.response_header.response_type,
                      &mmgsdi_callback_cnf_ptr->mmgsdi_cnf);
          }
          MMGSDIUTIL_TMC_MEM_FREE(mmgsdi_callback_cnf_ptr);
        }
      }
...
DESCRIPTION
  ...
  If command is cancel req - all requests with the client id are removed,
  If command is session cancel - all requests with session id are removed.
  If command is session_type cancel - all requests with session type are
  removed.
...
)
{
  ...
  for(index_client_req = 0; index_client_req < MMGSDI_MAX_CLIENT_REQ_INFO; index_client_req++)
  {
    ...
    /* If cancel request type from cancel_info is set, then only the request type
       commond is deleted from the queue. */
    if (cancel_info.cancel_request_type.is_valid &&
        cancel_info.cancel_request_type.request_type != mmgsdi_client_req_table_info_ptr[index_client_req]->request_type)
    {
      continue;
    }
    ...
    switch (cancel_info.cancel_type)
    {
      ...
      case MMGSDI_CANCEL_USING_SESSION_TYPE:
        if(mmgsdi_util_is_prov_session(cancel_info.id.session_type) &&
           MMGSDI_SUCCESS ==
             mmgsdi_util_get_session_type(mmgsdi_client_req_table_info_ptr[index_client_req]->session_id,
                                          &session_type, NULL) &&
           cancel_info.id.session_type == session_type)
        {
          remove_req = TRUE;
    ...
    /* for sync commands do not send cnf directly, just make sure it comes out of wait */
    if (mmgsdi_client_req_table_info_ptr[index_client_req]->sync_cmd)
    {
      /* set the sync report sig to be handled in
         mmgsdi_uim_report_synch */
      uint8                       slot_index  = MMGSDI_MAX_NUM_SLOTS;
      mmgsdi_task_enum_type       mmgsdi_task = MMGSDI_TASK_MAX;

      mmgsdi_task = mmgsdi_client_req_table_info_ptr[index_client_req]->task_id;
      if (mmgsdi_task == MMGSDI_TASK_MAIN ||
          mmgsdi_task >= MMGSDI_NUM_TASKS)
      {
        continue;
      }

      if (mmgsdi_util_get_slot_index_from_task(mmgsdi_task, &slot_index) !=
            MMGSDI_SUCCESS)
      {
        continue;
      }
      ...
      {
        /* Signal the slot task to come out of wait for the sync command */
        switch (mmgsdi_task)
        {
          case MMGSDI_TASK_SLOT1:
            rex_set_sigs(UIM_GSDI_SLOT1_TCB, MMGSDI_SYNCH_UIM_REPORT_SIG);
            break;
          case MMGSDI_TASK_SLOT2:
            rex_set_sigs(UIM_GSDI_SLOT2_TCB, MMGSDI_SYNCH_UIM_REPORT_SIG);
            break;
          case MMGSDI_TASK_SLOT3:
            rex_set_sigs(UIM_GSDI_SLOT3_TCB, MMGSDI_SYNCH_UIM_REPORT_SIG);
            break;
          default:
            break;
        }
...
DESCRIPTION
  ...
  If cancel_cmd_in_progress is set to FALSE, only the requests waiting in
  the queues are canceled, not the commands currently awaiting response from
  uimdrv.
...
(
  mmgsdi_cancel_info_type   cancel_info,
  boolean                   cancel_cmd_in_progress
)
{
  uint8                   index_task             = 0;
  mmgsdi_return_enum_type internal_cmd_q_status  = MMGSDI_SUCCESS;
  mmgsdi_return_enum_type high_pri_cmd_q_status  = MMGSDI_SUCCESS;
  mmgsdi_return_enum_type task_cmd_q_status      = MMGSDI_SUCCESS;
  ...
  for (index_task = 0; index_task < MMGSDI_NUM_TASKS; index_task++)
  {
    ...
    /* Remove requests in internal, high priority & task command queues */
    internal_cmd_q_status = mmgsdi_remove_requests_in_queue_send_cnf(&mmgsdi_internal_cmd_q[index_task],
                                                                     cancel_info);
    high_pri_cmd_q_status = mmgsdi_remove_requests_in_queue_send_cnf(&mmgsdi_high_pri_cmd_q[index_task],
                                                                     cancel_info);
    task_cmd_q_status = mmgsdi_remove_requests_in_queue_send_cnf(&mmgsdi_task_cmd_q[index_task],
                                                                 cancel_info);

    MMGSDIUTIL_LEAVE_CLIENT_APP_DATA_CRIT_SECT;

    if (internal_cmd_q_status != MMGSDI_SUCCESS ||
        high_pri_cmd_q_status != MMGSDI_SUCCESS ||
        task_cmd_q_status     != MMGSDI_SUCCESS)
    {
      return MMGSDI_ERROR;
    }
  }

  /* Also, if directed so by the input flag, mark the command in progress with
     UIMDRV so that error response can be sent now, but client req table entry
     is cleaned upon UIMDRV's response */
  if (cancel_cmd_in_progress)
  {
    MMGSDIUTIL_ENTER_CLIENT_APP_DATA_CRIT_SECT;

    (void)mmgsdi_remove_requests_in_progress_send_cnf(cancel_info);
...
/*===========================================================================
FUNCTION MMGSDI_PROCESS_QSH_ACTION_EVENT_CONFIG

DESCRIPTION
  This fucntion processes QSH action event configuration request
...
SIDE EFFECTS
  None.
===========================================================================*/
static mmgsdi_return_enum_type mmgsdi_process_qsh_action_event_config
(
  mmgsdi_qsh_action_event_cfg_req_type * qsh_action_event_cfg_ptr
)
{
  MMGSDIUTIL_RETURN_IF_NULL(qsh_action_event_cfg_ptr);

  uim_common_qsh_process_action_event_config_params(&qsh_action_event_cfg_ptr->cb_params, FALSE);
  return MMGSDI_SUCCESS;
}/* mmgsdi_process_qsh_action_event_config */
...
)
{
  ...
  if(MMGSDI_TASK_MAIN == mmgsdi_util_get_current_task())
  {
    ...
    {
      mmgsdi_task_enum_type mmgsdi_task = mmgsdi_util_get_task_from_slot(slot_id);
  ...
  if (!proceed_processing)
  {
    ...
    /* If cond_processing is TRUE, it means card_state is INSERTED, MMGSDI
       received file access req but App_state is not READY. In that case, we
       need to return ACCESS_DENIED as file is not in preinit_access_list
       (proceed_processing is FALSE) else return GENERIC_FAILURE */
    if (cond_processing)
    {
  ...
  {
    case MMGSDI_READ_REQ:
      mmgsdi_status = mmgsdi_common_read(&task_cmd_ptr->cmd.cmd.read_req, protocol);
    ...
    case MMGSDI_SAP_SEND_APDU_REQ:
      mmgsdi_status = mmgsdi_sap_sap_send_apdu(&task_cmd_ptr->cmd.cmd.sap_send_apdu_req);
    ...
    case MMGSDI_FULL_SERVICE_REQ:
         ...
         app_info_ptr->service_status != SIMLOCK_SRV_STATUS_FULL)
      {
        ...
        app_info_ptr->service_status = SIMLOCK_SRV_STATUS_FULL;
#ifdef FEATURE_SIMLOCK
        mmgsdi_simlock_build_and_run_algo(FALSE, MMGSDI_SESSION_ID_ZERO);
#endif /* FEATURE_SIMLOCK */
      }
      break;
    ...
    case MMGSDI_NOTIFY_SIM_PRE_INIT_REQ:
      mmgsdi_status = mmgsdi_card_init_process_sim_pre_init(
                        &task_cmd_ptr->cmd.cmd.sim_pre_init_req);
      break;
  ...
  case MMGSDI_NOTIFY_SIM_BUSY_REQ:
    ...
    case MMGSDI_HIDE_CSIM_REQ:
      mmgsdi_status = mmgsdi_gen_process_hide_csim_req(
                        &task_cmd_ptr->cmd.cmd.hide_csim_req);
      break;

    case MMGSDI_MAP_SESSIONS_TO_SLOTS_REQ:
      mmgsdi_status = mmgsdi_sub_sel_process_map_sessions_to_slots_req(
                       &task_cmd_ptr->cmd.cmd.map_sessions_to_slots_req);
      break;
    ...
    case MMGSDI_NOTIFY_RECOVERY_START_REQ:
      mmgsdi_status = mmgsdi_gen_process_notify_recovery_start(
                        &task_cmd_ptr->cmd.cmd.recovery_start_req);
    break;

    case MMGSDI_NOTIFY_RECOVERY_COMPLETE_REQ:
      mmgsdi_status = mmgsdi_gen_process_notify_recovery_complete(
                        &task_cmd_ptr->cmd.cmd.recovery_complete_req);
    break;

    case MMGSDI_PURGE_TEMPORARY_IDENTITY_REQ:
      mmgsdi_status = mmgsdi_common_purge_temporary_identity(&task_cmd_ptr->cmd.cmd.purge_temporary_identity_req,
                                                             protocol);
      break;

    case MMGSDI_QSH_ACTION_EVENT_CFG_REQ:
      mmgsdi_status = mmgsdi_process_qsh_action_event_config(&task_cmd_ptr->cmd.cmd.qsh_action_event_cfg_req);
      break;
      
    case MMGSDI_SET_APDU_BHEAVIOR_REQ:
      mmgsdi_status = mmgsdi_gen_process_set_apdu_behavior(&task_cmd_ptr->cmd.cmd.set_apdu_behavior_req);
      break;     
    ...
    case MMGSDI_GET_M2M_EID_REQ:
      ...
      {
      /* It is possible that the command is re-queued,
         if a previous command of the same type is already being executed */
        return mmgsdi_status;
      }
      break;
...
-----------------------------------------------------------------------------*/
  ...
  {
    /* When UIMDRV responds for cancelled sync command, we need to set
       uim_rsp_status to INBOUND so that it can be freed. */
    if( mmgsdi_client_req_table_info_ptr[index]->sync_cmd)
    {
  ...
  {
    case MMGSDI_READ_REQ:
      mmgsdi_status = mmgsdi_common_rsp_read(&task_cmd_ptr->cmd.cmd.uim_report_rsp,
                                             NULL,
                                             FALSE,
                                             protocol);
      break; /* end case - MMGSDI_READ_REQ*/
    ...
    case MMGSDI_SAP_SEND_APDU_REQ:
      mmgsdi_status = mmgsdi_sap_rsp_sap_send_apdu(&task_cmd_ptr->cmd.cmd.uim_report_rsp);
    break; /* end case - MMGSDI_SAP_SEND_APDU_REQ*/
...
-----------------------------------------------------------------------------*/
  ...
  /* Obtain the index and check the request type */
  if(response_ptr->rsp_data_index >= MMGSDI_MAX_CLIENT_REQ_INFO ||
...
-----------------------------------------------------------------------------*/
  ...
  UIM_MSG_HIGH_1("MMGSDI_PROCESS_CNF, Client data: 0x%x",
                 cnf_ptr->mmgsdi_cnf.response_header.client_data);
...
)
{
  ...
  if(MMGSDI_TASK_MAIN == mmgsdi_util_get_current_task())
  {
    (void)mmgsdi_eons_store_and_free_nitz_info();
  }
...
)
{
  ...
  if(mmgsdi_util_convert_uim_slot(mmgsdi_util_get_slot_from_task(mmgsdi_task), &uim_slot) ==
      MMGSDI_SUCCESS &&
         uim_slot != UIM_SLOT_NONE &&
         uim_slot != UIM_SLOT_AUTOMATIC)
  {
    /*-----------------------------------------------------------------------------
      Vote off the UIM
      1 Stop Polling the SIM/USIM in the case we are still in a Traffic Channel
    -----------------------------------------------------------------------------*/
    (void)uim_power_control_ds(uim_slot, UIM_TC, FALSE, UIM_CALL_TYPE_NOT_APPLICABLE);
...
/* ==========================================================================
FUNCTION MMGSDI_FLUSH_COMMANDS_IN_PROGRESS

DESCRIPTION
  This function flushes the commands of task waiting for UIM response except
  for the commands occupying client req table indices listed in
  skip_client_indices_ptr.
  If a valid cancel_info is provided, commands matching only that cancel_info
  are flushed.
  If cancel request type from cancel_info is set, then only the request type
  commond is deleted from the queue.
...
SIDE EFFECTS
  NONE
==========================================================================*/
void mmgsdi_flush_commands_in_progress
(
  mmgsdi_task_enum_type       mmgsdi_task,
  mmgsdi_cancel_info_type     cancel_info,
  uint8                      *skip_client_indices_ptr,
  uint8                       skip_client_indices_len,
  boolean                    *sync_cmd_in_progress_ptr
)
{
  uint16                        index                 = 0;
  uint8                         i                     = 0;
  ...
  for(index = 0; index < MMGSDI_MAX_CLIENT_REQ_INFO; index++)
  {
    ...
    /* If cancel request type from cancel_info is set, then only the request type
       commond is deleted from the queue. */
    if (cancel_info.cancel_request_type.is_valid &&
        cancel_info.cancel_request_type.request_type != mmgsdi_client_req_table_info_ptr[index]->request_type)
    {
      continue;
    }

    if (skip_client_indices_ptr)
    {
      remove_req = TRUE;
      for(i = 0; i < skip_client_indices_len; i++)
      {
        /* See if flushing of command at this index needs to be skipped based on
           input parameters.
           If the index matches with one of the values in skip_client_indices_ptr
           do not flush command at that index. */
        if(skip_client_indices_ptr[i] == index)
        {
          remove_req = FALSE;
          break;
        }
    ...
    switch (cancel_info.cancel_type)
    {
      case MMGSDI_CANCEL_USING_CLIENT_ID:
        ...
        if (cancel_info.id.client_id ==
              mmgsdi_client_req_table_info_ptr[index]->client_id)
        {
          remove_req = TRUE;
        }
        break;
      ...
      case MMGSDI_CANCEL_USING_SESSION_TYPE:
        /* Don't remove the activate request during clean-up. Session_type is
           used as cancel_type only from deactivate_start() function and during
           de-activation of that session, we should not clear any in-progress
           activation request of that session. */
        if(mmgsdi_util_is_prov_session(cancel_info.id.session_type) &&
           MMGSDI_SUCCESS ==
             mmgsdi_util_get_session_type(mmgsdi_client_req_table_info_ptr[index]->session_id,
                                          &session_type, NULL) &&
           cancel_info.id.session_type == session_type &&
           mmgsdi_client_req_table_info_ptr[index]->orig_request_type != MMGSDI_SESSION_ACT_OR_SWITCH_PROV_REQ)
        {
          remove_req = TRUE;
        }
        break;
      case MMGSDI_CANCEL_ALL:
        /* Request is for flushing all requests for this task */
        remove_req = TRUE;
        break;
      default:
        return;
    ...
    /* Check if the response corresponding to the index is already on response
       ...
       For sync cmd, check mmgsdi_internal_synch_uim_rpt_rsp[mmgsdi_task] */
    rsp_found = FALSE;
    if (mmgsdi_client_req_table_info_ptr[index]->sync_cmd == FALSE)
    {
    ...
    else if (mmgsdi_internal_synch_uim_rpt_rsp[mmgsdi_task] &&
             mmgsdi_internal_synch_uim_rpt_rsp[mmgsdi_task]->request_header.request_type
    ...
    else if (sync_cmd_in_progress_ptr)
    {
      *sync_cmd_in_progress_ptr = TRUE;
    }
  }
} /* mmgsdi_flush_commands_in_progress */


/*===========================================================================
FUNCTION MMGSDI_HANDLE_GSTK_IMEI_SENT_SIG

DESCRIPTION
  This function, called in mmgsdi_task, is called to set IMEI sent to card flag to TRUE.
...
SIDE EFFECTS
  NONE
===========================================================================*/
static void mmgsdi_handle_gstk_imei_sent_sig
(
  void
)
{
  ...
  UIM_MSG_MED_0("MMGSDI: Got MMGSDI_GSTK_IMEI_SENT_SIG");

  (void) rex_clr_sigs(rex_self(), MMGSDI_GSTK_IMEI_SENT_SIG);

  mmgsdi_task = mmgsdi_util_get_current_task();
  if(mmgsdi_task >= MMGSDI_NUM_TASKS || mmgsdi_task == MMGSDI_TASK_MAIN)
  {
    return;
  }
...
} /* mmgsdi_handle_gstk_imei_sent_sig */
...
)
{
  ...
  cancel_info.cancel_type = MMGSDI_CANCEL_ALL;
  (void) mmgsdi_flush_commands_in_progress(mmgsdi_task, cancel_info,
                                           NULL, 0, NULL);
...
)
{
  ...
  {
    UIM_MSG_LOW_0("MMGSDI_TP_DOWNLOAD_COMPLETE_REQ found in queue");
    return 1;
  }
...
)
{
  ...
  /* Check early initialization is done for Slot1 in PSM mode */
...
/*===========================================================================
FUNCTION MMGSDI_HANDLE_SLOT_MAPPING_SIG
...
SIDE EFFECTS
  None
===========================================================================*/
static void mmgsdi_handle_slot_mapping_sig
(
  void
)
{
  /* Even though only the main gsdi task is expected to receive this signal,
     ...
     we should be able to clear it in order to avoid infinite sig-processing
     loops */
  (void)rex_clr_sigs(rex_self(), MMGSDI_SLOT_MAPPING_SIG);
  if(uim_slot_mapping_sig_handler() == FALSE)
  {
    (void)rex_set_sigs(UIM_GSDI_TCB, MMGSDI_SLOT_MAPPING_SIG);
  }
} /* mmgsdi_handle_slot_mapping_sig */
...
)
{
  ...
  for(slot_index = MMGSDI_SLOT_1_INDEX; slot_index < MMGSDI_MAX_NUM_SLOTS; slot_index++)
  {
    ...
    timer_get_params(&mmgsdi_refresh_retry_timer[slot_index], &timer_attrib_struct_data);
    if(timer_attrib_struct_data.tcb_sigs_obj != NULL &&
       timer_attrib_struct_data.tcb_sigs_obj == mmgsdi_current_tcb)
    {
      (void)rex_undef_timer(&mmgsdi_refresh_retry_timer[slot_index]);
    }
  }
...
)
{
  ...
  running_task = mmgsdi_util_get_current_task();
  if(!MMGSDIUTIL_IS_MMGSDI_TASK(running_task))
  {
    return;
  }
...
/* ============================================================================
  FUNCTION:      MMGSDI_CHECK_PENDING_CARD_RESET_ON_CALL_END

  DESCRIPTION:
    This function shall check if there is a Card Reset pending, 
    and if so Reset the card

  DEPENDENCIES:
    Function should be called only when No active voice/emergency calls are ongoing
  ...
  SIDE EFFECTS:
    None
=============================================================================*/
static void mmgsdi_check_pending_card_reset_on_call_end(
...
)
{
  ...
  /* Check if card reset on on refresh failure is supported */
  ...
  {
    /* Reset card is Refresh failure count has crossed the max. threshold */
...
} /* mmgsdi_check_pending_card_reset_on_call_end */
...
)
{
  ...
  UIM_MSG_HIGH_4("mmgsdi_cm_call_evt_cb, call_event:0x%x, call_type:0x%x, call_state:0x%x, call_id:0x%x",
                  call_event, call_info_ptr->call_type, call_info_ptr->call_state, call_info_ptr->call_id);
  ...
  switch (call_info_ptr->call_type)
  {
    case CM_CALL_TYPE_VOICE:
      mmgsdi_generic_data_ptr->cm_call_info[mmgsdi_gw_prov_index].voice_call_active = call_active;
      mmgsdi_generic_data_ptr->cm_call_info[mmgsdi_1x_prov_index].voice_call_active = call_active;
      break;

    case CM_CALL_TYPE_CS_DATA:
    case CM_CALL_TYPE_PS_DATA:
      mmgsdi_generic_data_ptr->cm_call_info[mmgsdi_gw_prov_index].data_call_active = call_active;
      mmgsdi_generic_data_ptr->cm_call_info[mmgsdi_1x_prov_index].data_call_active = call_active;
      break;

    case CM_CALL_TYPE_EMERGENCY:
      if(call_info_ptr->call_state == CM_CALL_STATE_ORIG)
      {
        mmgsdi_generic_data_ptr->cm_call_info[mmgsdi_gw_prov_index].emergency_call_active = TRUE;
        mmgsdi_generic_data_ptr->cm_call_info[mmgsdi_1x_prov_index].emergency_call_active = TRUE;
      }
      else if(call_info_ptr->call_state == CM_CALL_STATE_IDLE)
      {
        mmgsdi_generic_data_ptr->cm_call_info[mmgsdi_gw_prov_index].emergency_call_active = FALSE;
        mmgsdi_generic_data_ptr->cm_call_info[mmgsdi_1x_prov_index].emergency_call_active = FALSE;
      }
      break;
  ...
  /*There was one issue where emergency call started and CM send event to MMGSDI with	
    Emergency call start . Later this emergency call ended, but due to fallback 
    to CS in IMS side, emergency call got switched to voice call and CM send event to
    MMGSDI with voice call end. However, to MMGSDI, emergency call was still in progress
    as it didn't receive any event from CM with emergency call end. Thus, it rejected
    activation of sim card when card was inserted later after hotswap.

    In order to resolve the above issue, we will follow the following approach:
    1. If any call started (call state is CM_CALL_STATE_ORIG) and call type is either voice or emergency,
       store the call id and call type
    2. If any call ended(call event is CM_CALL_EVENT_END) and call id of the recently ended call matches
       with the call id of the earlier stored call, then set the call active flag to false, for the following
       cases

    There can be 4 possible cases, all of which are discussed below:
    Case 1: Voice call start, Emergency call start and Voice call end, need to reset emergency call active flag
    Case 2: Emergency call start, Voice call start and Emergency call end, need to reset voice call active flag
    Case 3: Voice call start, Emergency call start and Emergency call end, need to reset voice call active flag
    Case 4: Emergency call start, Voice call start and Voice call end, need to reset Emergency call active flag

    Note: Other call types like PS data call and CS data call will never have same call id as voice/emergency */

  if(call_event == CM_CALL_EVENT_END &&
     mmgsdi_generic_data_ptr->cm_call_info[mmgsdi_gw_prov_index].event_call_id == call_info_ptr->call_id &&
     mmgsdi_generic_data_ptr->cm_call_info[mmgsdi_1x_prov_index].event_call_id == call_info_ptr->call_id)
  {
    switch(mmgsdi_generic_data_ptr->cm_call_info[mmgsdi_gw_prov_index].event_call_type)
    {
      case CM_CALL_TYPE_VOICE:
        if(call_info_ptr->call_type == CM_CALL_TYPE_VOICE)
        {
          /* case 4 */
          UIM_MSG_HIGH_0("Emergency call started and voice call ended. Emergency call no longer active");
          mmgsdi_generic_data_ptr->cm_call_info[mmgsdi_gw_prov_index].emergency_call_active = FALSE;
          mmgsdi_generic_data_ptr->cm_call_info[mmgsdi_1x_prov_index].emergency_call_active = FALSE;
        }
        else
        {
          /* case 2 */
          UIM_MSG_HIGH_0("Emergency call started and emergency call ended. Voice call no longer active");
          mmgsdi_generic_data_ptr->cm_call_info[mmgsdi_gw_prov_index].voice_call_active = FALSE;
          mmgsdi_generic_data_ptr->cm_call_info[mmgsdi_1x_prov_index].voice_call_active = FALSE;
        }
        break;
      case CM_CALL_TYPE_EMERGENCY:
        if(call_info_ptr->call_type == CM_CALL_TYPE_VOICE)
        {
          /* case 1 */
          UIM_MSG_HIGH_0("Voice call started and Voice call ended. Emergency call no longer active");
          mmgsdi_generic_data_ptr->cm_call_info[mmgsdi_gw_prov_index].emergency_call_active = FALSE;
          mmgsdi_generic_data_ptr->cm_call_info[mmgsdi_1x_prov_index].emergency_call_active = FALSE;
        }
        else
        {
          /* case 3 */
          UIM_MSG_HIGH_0("Voice call started and emergency call ended. Voice call no longer active");
          mmgsdi_generic_data_ptr->cm_call_info[mmgsdi_gw_prov_index].voice_call_active = FALSE;
          mmgsdi_generic_data_ptr->cm_call_info[mmgsdi_1x_prov_index].voice_call_active = FALSE;
        }
        break;
    ...
    /* Now clear the call id and call type since the call has ended */
    memset(&mmgsdi_generic_data_ptr->cm_call_info[mmgsdi_gw_prov_index].event_call_id , 0x00,
           sizeof(mmgsdi_generic_data_ptr->cm_call_info[mmgsdi_gw_prov_index].event_call_id));

    memset(&mmgsdi_generic_data_ptr->cm_call_info[mmgsdi_1x_prov_index].event_call_id , 0x00,
           sizeof(mmgsdi_generic_data_ptr->cm_call_info[mmgsdi_1x_prov_index].event_call_id));

    memset(&mmgsdi_generic_data_ptr->cm_call_info[mmgsdi_gw_prov_index].event_call_type , 0x00,
           sizeof(mmgsdi_generic_data_ptr->cm_call_info[mmgsdi_gw_prov_index].event_call_type));

    memset(&mmgsdi_generic_data_ptr->cm_call_info[mmgsdi_1x_prov_index].event_call_type , 0x00,
           sizeof(mmgsdi_generic_data_ptr->cm_call_info[mmgsdi_1x_prov_index].event_call_type));
  }

  /* If the call starts and call type is either voice or emergency, store the current call id and call type
     sent in the event cb */
  if(call_info_ptr->call_state == CM_CALL_STATE_ORIG &&
      ...
      call_info_ptr->call_type == CM_CALL_TYPE_EMERGENCY))
  {
    mmgsdi_generic_data_ptr->cm_call_info[mmgsdi_gw_prov_index].event_call_id = call_info_ptr->call_id;
    mmgsdi_generic_data_ptr->cm_call_info[mmgsdi_1x_prov_index].event_call_id = call_info_ptr->call_id;
    mmgsdi_generic_data_ptr->cm_call_info[mmgsdi_gw_prov_index].event_call_type = call_info_ptr->call_type;
    mmgsdi_generic_data_ptr->cm_call_info[mmgsdi_1x_prov_index].event_call_type = call_info_ptr->call_type;
  }
  ...
  if (!call_active)
  {
    ...
    mmgsdi_status = mmgsdi_util_get_slot_id_from_as_id(call_info_ptr->asubs_id, &slot_id);

    if (mmgsdi_status == MMGSDI_SUCCESS)
    {
      /* If call is not active then check if there is a card reset pending */
      mmgsdi_check_pending_card_reset_on_call_end(slot_id);
    }
  }
  MMGSDIUTIL_LEAVE_CLIENT_APP_DATA_CRIT_SECT;
...
)
{
  ...
  switch (ph_event)
  {
    case CM_PH_EVENT_OPRT_MODE:
      ...
      {
        mmgsdi_generic_data_ptr->is_oprt_mode_online = TRUE;
      }
      else
      {
        mmgsdi_generic_data_ptr->is_oprt_mode_online = FALSE;
      }
      break;
...
/*===========================================================================
  FUNCTION: mmgsdi_nas_purge_temporary_identity_cnf_cb

  DESCRIPTION:
    Callback handler for purge temporary identity request confirmation
  ...
  SIDE EFFECTS:
    None
===========================================================================*/
static void mmgsdi_nas_purge_temporary_identity_cnf_cb
(
  uint32    user_data,
  boolean   status
)
{
  mmgsdi_task_cmd_type                             * task_cmd_ptr    = NULL;
  mmgsdi_purge_temporary_identity_req_type         * msg_ptr         = NULL;
  mmgsdi_return_enum_type                            mmgsdi_status   = MMGSDI_SUCCESS;
  mmgsdi_client_req_extra_info_type                * extra_param_ptr = NULL;
  uint32                                             index           = MMGSDI_MAX_CLIENT_REQ_INFO;

  index = user_data;

  UIM_MSG_HIGH_2("mmgsdi_nas_purge_temporary_identity_cnf_cb, status:0x%x, user_data:0x%x",
                 status, user_data);
  ...
  do
  {
    ...
    mmgsdi_util_align_mem( &task_cmd_ptr->cmd.cmd.purge_temporary_identity_req,
                           MMGSDI_PURGE_TEMPORARY_IDENTITY_REQ);

    msg_ptr  = &task_cmd_ptr->cmd.cmd.purge_temporary_identity_req;

    /*---------------------------------------------------------------------------
      Populating mmgsdi_purge_temporary_identity_req_type header
        1) Session ID
        2) Request TYPE
        3) Client Data Pointer
        4) Response CallBack
    ---------------------------------------------------------------------------*/
    task_cmd_ptr->cmd.cmd_enum = MMGSDI_PURGE_TEMPORARY_IDENTITY_REQ;
    ...
    /*---------------------------------------------------------------------------
      Populating mmgsdi_purge_temporary_identity_state_enum_type content
    ---------------------------------------------------------------------------*/
    msg_ptr->temporary_identity_req_state = MMGSDI_PURGE_TEMPORARY_IDENTITY_SOURCE_CLIENT_RSP;

    /*---------------------------------------------------------------------------
      Populating mmgsdi_purge_temporary_identity_req_type content
    ---------------------------------------------------------------------------*/
    msg_ptr->temporary_identity = extra_param_ptr->temporary_identity.temporary_identity;
    msg_ptr->client_status = status;
  ...
  mmgsdi_util_free_client_request_table_index(index);
  MMGSDIUTIL_LEAVE_CLIENT_APP_DATA_CRIT_SECT;
}/* mmgsdi_nas_purge_temporary_identity_cnf_cb */


/*===========================================================================
  FUNCTION: MMGSDI_SEND_PURGE_REQ_TO_NAS

  DESCRIPTION:
    This function is called to send purge request to NAS
  ...
  SIDE EFFECTS:
    None
===========================================================================*/
mmgsdi_return_enum_type mmgsdi_send_purge_req_to_nas
(
...
)
{
  ...
  if(nas_mmgsdi_purge_temporary_identity_req(session_type,
                                             index,
                                             mmgsdi_nas_purge_temporary_identity_cnf_cb) == FALSE)
  {
    mmgsdi_util_free_client_request_table_index(index);
    mmgsdi_status = MMGSDI_ERROR;
  }
...
void mmgsdi_cm_init (void)
{
  ...
  else
  {
    ...
    /* Register callbacks */
    (void) cm_mm_client_call_reg(mmgsdi_generic_data_ptr->mmgsdi_cm_id,
                                 mmgsdi_cm_call_evt_cb,
                                 CM_CLIENT_EVENT_REG,
                                 CM_CALL_EVENT_ORIG,
  ...
  for (i = 0; i < MMGSDI_MAX_CLIENT_INFO; i++)
  {
    if (mmgsdi_client_id_reg_table[i] == NULL)
    {
      ...
      /* Get the next available id.
         The most significant 32 bits are used here to maintain
         legacy behavior. */
      client_id = mmgsdi_util_get_next_available_id();
      client_id = client_id << MMGSDI_CLIENT_ID_SHIFT;
      mmgsdi_client_id_reg_table[i]->client_id = client_id;

      strlcpy(mmgsdi_client_id_reg_table[i]->client_task_name, client_task_name, MMGSDI_TASK_NAME_LEN);
...
)
{
  ...
  if(!(mmgsdi_generic_data_ptr->ready_tasks & (0x00000001 << (uint8) MMGSDI_CM_TYPE)))
  {
    ...
    mcs_rcevt_unregister_sigex_name("cm:ready",
    ...
    mmgsdi_init_rtre_config_per_context(MMGSDI_NV_CONTEXT_PRIMARY);
    mmgsdi_init_rtre_config_per_context(MMGSDI_NV_CONTEXT_SECONDARY);
  }
...
)
{
  ...
  if(!(mmgsdi_generic_data_ptr->ready_tasks & (0x00000001 << (uint8) MMGSDI_QMI_MODEM_TYPE)))
  {
    ...
    mcs_rcevt_unregister_sigex_name("QMI_MODEM: READY",
                                    RCEVT_SIGEX_TYPE_SIGREX,
...
/*===========================================================================
FUNCTION MMGSDI_ACK_SYNC_CMD_CANCELED_IF_NEEDED

DESCRIPTION
  This function acknowledges to the main task (sets SYNC_CMD_CANCEL_ACK_SIG)
  if a sync command was canceled for the current slot-specific task.
...
SIDE EFFECTS
  None.
===========================================================================*/
static void mmgsdi_ack_sync_cmd_canceled_if_needed
(
  mmgsdi_task_enum_type running_task
)
{
  uint8 slot_index = 0;

  if (mmgsdi_util_get_slot_index_from_task(running_task,
                                           &slot_index) != MMGSDI_SUCCESS)
  {
    return;
  }

  if (mmgsdi_sync_cmd_canceled[slot_index] == FALSE)
  {
    return;
  }

  if(mmgsdi_need_sync_cmd_cancel_ack[slot_index])
  {
    MMGSDIUTIL_ENTER_TASK_STATE_CRIT_SECT;
    if (mmgsdi_task_state[MMGSDI_TASK_MAIN] == MMGSDI_TASK_ACTIVE)
    {
      (void)rex_set_sigs(UIM_GSDI_TCB, MMGSDI_SYNC_CMD_CANCEL_ACK_SIG);
    }
    MMGSDIUTIL_LEAVE_TASK_STATE_CRIT_SECT;
    mmgsdi_need_sync_cmd_cancel_ack[slot_index] = FALSE;

    /* wait for MAIN TASK to complete its processing*/
    (void)rex_clr_sigs(rex_self(), MMGSDI_SYNC_CMD_CANCEL_ACK_SIG);

    MMGSDIUTIL_LEAVE_TASK_SYNC_CRIT_SECT;
    
    /* Wait for acknowledgement back from main task for sync command cancellation. */
    (void)mmgsdi_wait(MMGSDI_SYNC_CMD_CANCEL_ACK_SIG);

    MMGSDIUTIL_ENTER_TASK_SYNC_CRIT_SECT;
  }
  mmgsdi_sync_cmd_canceled[slot_index] = FALSE;
} /* mmgsdi_ack_sync_cmd_canceled_if_needed */


/*===========================================================================
FUNCTION MMGSDI_CONTINUE_DEACTIVATION_IF_NEEDED

DESCRIPTION
  This function resumes deactivation if the deactivation pause timer is
  running and no commands are in the queue.
...
SIDE EFFECTS
  None.
===========================================================================*/
static void mmgsdi_continue_deactivation_if_needed
(
  mmgsdi_task_enum_type         running_task
)
{
  uint32                        index            = 0;
  mmgsdi_attribute_enum_type    attr_type        = MMGSDI_ATTR_PRIMARY;
  uint16                        num_cmds         = 0;
  mmgsdi_session_type_enum_type session_type     = MMGSDI_MAX_SESSION_TYPE_ENUM;
  mmgsdi_slot_id_enum_type      slot_id          = MMGSDI_MAX_SLOT_ID_ENUM;
  mmgsdi_return_enum_type       mmgsdi_status    = MMGSDI_ERROR;
  timer_attrib_struct_type      timer_attributes = {0};
  mmgsdi_session_type_enum_type gw_session_types[] = {
                                  MMGSDI_GW_PROV_PRI_SESSION,
                                  MMGSDI_GW_PROV_SEC_SESSION,
                                  MMGSDI_GW_PROV_TER_SESSION};

  slot_id = mmgsdi_util_get_slot_from_task(running_task);
  if (slot_id == MMGSDI_MAX_SLOT_ID_ENUM)
  {
    return;
  }

  for (attr_type = MMGSDI_ATTR_PRIMARY; attr_type < MMGSDI_ATTR_UNKNOWN; attr_type++)
  {
    /* Nothing to do if the timer is in any other state than SET_FLAG (running) */
    timer_get_params(&mmgsdi_session_deactivation_pause_timer[attr_type],
                     &timer_attributes);
    if(timer_attributes.timer_state != TIMER_SET_FLAG)
    {
      continue;
    }

    /* Retrieve client index for the deactivation req from the timer data */
    index = timer_attributes.data;

    if (index >= MMGSDI_MAX_CLIENT_REQ_INFO ||
        mmgsdi_client_req_table_info_ptr[index] == NULL ||
        mmgsdi_client_req_table_info_ptr[index]->request_type != MMGSDI_SESSION_DEACTIVATE_REQ ||
        mmgsdi_client_req_table_info_ptr[index]->orig_request_type != MMGSDI_SESSION_DEACTIVATE_REQ ||
        mmgsdi_client_req_table_info_ptr[index]->slot_id != slot_id)
    {
      continue;
    }

    /* Ensure that the session type for deactivation request matches the type
       for the paused deactivation */
    if ((mmgsdi_util_get_session_type(mmgsdi_client_req_table_info_ptr[index]->session_id,
                                      &session_type, NULL) != MMGSDI_SUCCESS) ||
         session_type != gw_session_types[attr_type])
    {
      continue;
    }

    /* Do not resume deactivation if there are commands in the queues for
       this session type */
    num_cmds = mmgsdi_util_find_commands_in_queue_for_session_type(
                 &mmgsdi_task_cmd_q[running_task], session_type) +
               mmgsdi_util_find_commands_in_queue_for_session_type(
                 &mmgsdi_high_pri_cmd_q[running_task], session_type);
    ...
    if (num_cmds == 0)
    {
      ...
      for (i = 0; i < MMGSDI_MAX_CLIENT_REQ_INFO; i++)
      {
        if (mmgsdi_client_req_table_info_ptr[i] == NULL ||
            mmgsdi_client_req_table_info_ptr[i]->slot_id != slot_id ||
            i == index)
        {
          continue;
        }

        if (mmgsdi_util_get_session_type(mmgsdi_client_req_table_info_ptr[i]->session_id,
                                         &session_type, NULL) == MMGSDI_SUCCESS &&
            session_type == gw_session_types[attr_type])
        {
          UIM_MSG_HIGH_2("cmd 0x%x in progress for session type 0x%x",
                         mmgsdi_client_req_table_info_ptr[i]->request_type, session_type);
          num_cmds++;
          break;
        }
      }
    ...
    mmgsdi_status = mmgsdi_session_queue_continue_deactivate_req(index);

    if (mmgsdi_status != MMGSDI_SUCCESS)
    {
      UIM_MSG_ERR_1("error 0x%x queuing MMGSDI_CONTINUE_DEACTIVATE_REQ", mmgsdi_status);
    }
    else
    {
      /* Clear the timer so that it does not fire causing the queuing of
         another CONTINUE_DEACTIVATE_REQ. */
      (void)rex_clr_timer(&mmgsdi_session_deactivation_pause_timer[attr_type]);
      (void)rex_undef_timer(&mmgsdi_session_deactivation_pause_timer[attr_type]);
    }
  }
} /* mmgsdi_continue_deactivation_if_needed */
...
)
{
  ...
  if (running_task == MMGSDI_TASK_MAIN)
  {
    mcs_rcevt_signal_name("MM_GSDI: READY");
  }
  ...
  if (running_task == MMGSDI_TASK_MAIN)
  {
    ...
    /* Add GSDI thread id to QSH */
    qsh_add_thread_id(QSH_CLT_USIM);
  ...
  ** ----------------------------------------------------------------------*/
  for (;;)
  {
    ...
    {
      mmgsdi_handle_stop_sig(running_task);
...
#ifdef FEATURE_QSH_MDUMP
      /* Update MMGSDI slot specific thread id to QSH */
      qsh_add_thread_id(QSH_CLT_USIM);
#endif /* FEATURE_QSH_MDUMP */
    }
    /*-----------------------------------------------------------------------------
    ...
    /*-----------------------------------------------------------------------------
                          Process Signal - MMGSDI_SLOT_MAPPING_SIG
    -----------------------------------------------------------------------------*/
    else if ((sigs_rcvd & sigs & MMGSDI_SLOT_MAPPING_SIG) &&
             (running_task == MMGSDI_TASK_MAIN))
    {
      mmgsdi_handle_slot_mapping_sig();
    } /* end if MMGSDI_SLOT_MAPPING_SIG */
    ...
    else
    {
      /*---------------------------------------------------------------------------
                      ...
                      Following signals need task sync crit section
      ---------------------------------------------------------------------------*/
      MMGSDIUTIL_ENTER_TASK_SYNC_CRIT_SECT;
      ...
      ---------------------------------------------------------------------------*/
      if (sigs_rcvd & sigs & MMGSDI_TASK_RSP_Q_SIG)
      {
      ...
      /*---------------------------------------------------------------------------
                          Process Signal - MMGSDI_GSTK_IMEI_SENT_SIG
      ---------------------------------------------------------------------------*/
      else if(sigs_rcvd & sigs & MMGSDI_GSTK_IMEI_SENT_SIG)
      {
        /* Do not handle response if GSDI task is not active */
        if(mmgsdi_generic_data_ptr &&
           task_state == MMGSDI_TASK_ACTIVE)
      ...
      } /* end if MMGSDI_GSTK_IMEI_SENT_SIG */
      ...
      /* If a sync cmd was canceled for this task and control has returned
         to the main loop, signal to the main task that the command was
         canceled so the main task can resume processing the deactivation req */
      if (running_task != MMGSDI_TASK_MAIN)
      {
        mmgsdi_ack_sync_cmd_canceled_if_needed(running_task);
      }

      /* If deactivation was previously paused, resume it if the timer is
         still running and there are no commands in the queues */
      if (running_task != MMGSDI_TASK_MAIN)
      {
        mmgsdi_continue_deactivation_if_needed(running_task);
      }
      /*---------------------------------------------------------------------------
  ...
  const mmgsdi_card_inserted_evt_info_type *evt_ptr)
{
  ...
  /* Card inserted event is received. Update sim busy and detection started state
     to FALSE */
  slot_data_ptr->sim_busy              = FALSE;
  slot_data_ptr->sim_detection_started = FALSE;
      ...
      if(slot_data_ptr->app_info_pptr[i] != NULL)
      {
        for (j = 0; j < evt_ptr->num_aids_avail && j < MMGSDI_MAX_APP_INFO; j++)
        {
            ...
            (evt_ptr->aid_info[j].app_type == MMGSDI_APP_CSIM))
          {
            ...
            (void)mmgsdi_util_get_slot_index(evt_ptr->slot, &slot_index);

            nvruim_set_uim_dir_present(NVRUIM_CDMA_SUBSCRIPTION_PRESENT, evt_ptr->slot);
  ...
  mmgsdi_slot_id_enum_type slot_id)
{
  ...
  /* Card error event is received. Update sim busy and detection started state
     to FALSE. Also reset the ATR related globals. */
  slot_data_ptr->sim_busy              = FALSE;
  slot_data_ptr->sim_detection_started = FALSE;
  ...
  memset(&slot_data_ptr->eid_info, 0x00, sizeof(slot_data_ptr->eid_info));
  ...
  mmgsdi_perso_status_enum_type perso_status)
{
  ...
  if ((mmgsdi_status != MMGSDI_SUCCESS) || (int_app_info_ptr == NULL)
#ifdef FEATURE_SIMLOCK
       || int_app_info_ptr->perso_state != MMGSDI_SIMLOCK_VERIFIED
#endif /* FEATURE_SIMLOCK */
     )
  {
    /* Invalid session_id*/
...
void mmgsdi_evt_cb (const mmgsdi_event_data_type *event_ptr)
{
  ...
  /* Reset slot to MMGSDI_MAX_SLOT_ID_ENUM if it is not PERSO_EVENT as
     slot value used in PERSON EVENT handling below */
  if(event_ptr->evt != MMGSDI_PERSO_EVT)
   {
  ...
  switch(event_ptr->evt)
  {
    case MMGSDI_SESSION_CHANGED_EVT:
      ...
      if (event_ptr->data.session_changed.activated)
      {
        ...
        if(mmgsdi_nv_get_device_feature_status(MMGSDI_FEATURE_MULTISIM_AUTO_PROVISIONING)
           == MMGSDI_FEATURE_ENABLED)
        {
      ...
      else
      {
        ...
        if (event_ptr->data.session_changed.cause != MMGSDI_SESSION_CLOSED_CAUSE_PERSO_OPERATION_FAIL)
        {
          ...
          mmgsdi_status = mmgsdi_simlock_build_and_run_algo(FALSE, event_ptr->session_id);
        ...
        /* Reset the purge_temporary_identity_guti if session is deactivated */
      ...
      if (FALSE == mmgsdi_is_test_mode_logging_enabled.secure_logging_enabled  &&
    ...
    case MMGSDI_PERSO_EVT:
      ...
      if(onchip_state != MMGSDI_ONCHIP_SIM_INIT)
      {
        ...
        /* Sending TR to the card if initialization gets stuck in perso state */
        if(event_ptr->data.perso.status != MMGSDI_PERSO_STATUS_DONE)
        {
          /* End any ongoing REFRESH on the Session */
          (void)mmgsdi_card_init_end_refresh_in_progress(event_ptr->session_id, slot);
        }
      }
      break;
    ...
    case MMGSDI_CARD_INSERTED_EVT:
      ...
      /* Now that card inserted event has been sent out, enable FETCH and
         therefore potential proactive commands for non-ATT cards and eUICC cards without any USIM */
      if(!mmgsdi_nv_is_ens_enabled(slot) || 
         (event_ptr->data.card_inserted.is_euicc && mmgsdi_util_get_usim_apps(slot) == 0))
      {
      ...
      mmgsdi_init_rtre_config_per_context(mmgsdi_util_get_efs_item_index_for_slot(slot));

      if(uim_common_qsh_is_event_enabled(USIM_QSH_EVENT_CARD_INSERTED))
      {
        sys_modem_as_id_e_type  as_id = SYS_MODEM_AS_ID_1;

        as_id = mmgsdi_util_get_as_id_from_slot_id(slot);
        uim_common_qsh_event_notify(USIM_QSH_EVENT_CARD_INSERTED, as_id);
      }
      break;
    ...
    case MMGSDI_CARD_REMOVED_EVT:
      ...
      if(mmgsdi_status == MMGSDI_SUCCESS)
      {
           ...
           (mmgsdi_refresh_info_ptr[slot_index]->request_header.slot_id == slot))
        {
          ...
          (void)mmgsdi_refresh_rsp_send_cnf(
      ...
      if(uim_common_qsh_is_event_enabled(USIM_QSH_EVENT_CARD_ERROR))
      {
        sys_modem_as_id_e_type  as_id = SYS_MODEM_AS_ID_1;

        as_id = mmgsdi_util_get_as_id_from_slot_id(slot);
        uim_common_qsh_event_notify(USIM_QSH_EVENT_CARD_ERROR, as_id);
      }

      /* Delete slot specific subscription provisioning data if card is removed
         or device boots up without card */
      if(event_ptr->data.card_error.info == MMGSDI_CARD_ERR_NO_ATR_RCVD_AT_MAX_VOLT ||
         event_ptr->data.card_error.info == MMGSDI_CARD_ERR_NO_ATR_RCVD_AFTER_RESET ||
         event_ptr->data.card_error.info == MMGSDI_CARD_ERR_CARD_REMOVED)
      {
        mmgsdi_delete_efs_slot_subs_prov(slot);
      }
#ifdef FEATURE_UIM_MULTI_PROFILE
      if (slot == MMGSDI_SLOT_1)
      {
  ...
  mmgsdi_log_pkt_mmgsdi_event(event_ptr);
}/* mmgsdi_evt_cb*/
  ...
  mmgsdi_nv_context_type nv_item_context)
{
  ...
  if (nv_item_context < MMGSDI_NV_CONTEXT_PRIMARY ||
  ...
  result = mmgsdi_nv_get_item(NV_RTRE_CONFIG_I, &mmgsdi_nv_data, nv_item_context);

  if(result != NV_DONE_S)
  {
    nv_rtre_config->rtre_config = NV_RTRE_CONFIG_RUIM_ONLY;
    (void) mmgsdi_nv_put_item(NV_RTRE_CONFIG_I, nv_rtre_config, nv_item_context);

    /* Building Bootup Logs */
    if(nv_item_context == MMGSDI_NV_CONTEXT_PRIMARY)
    {
      mmgsdi_log_pkt_rtre_update(MMGSDI_ATTR_PRIMARY);
    }
    else if (nv_item_context == MMGSDI_NV_CONTEXT_SECONDARY)
    {
      mmgsdi_log_pkt_rtre_update(MMGSDI_ATTR_SECONDARY);
    }
  }
  else
  {
  ...
  if((mmgsdi_generic_data_ptr->ready_tasks & (00000001 << (uint8) MMGSDI_CM_TYPE)))
  {
    ...
    sub_id = mmgsdi_util_convert_efs_context_to_cm_subid(nv_item_context);
    if ( FALSE == cm_ph_cmd_change_rtre_config1(mmgsdi_cm_rtre_cb,
                                                NULL,
                                                mmgsdi_generic_data_ptr->mmgsdi_cm_id,
  ...
  else
  {
    ...
    mmgsdi_util_send_rtre_command_to_nv(nv_rtre_config, nv_item_context);
  }

  MMGSDIUTIL_TMC_MEM_FREE(nv_rtre_config);
#endif /* FEATURE_MMGSDI_3GPP2 && !FEATURE_UIM_TEST_FRAMEWORK */
} /* mmgsdi_init_rtre_config_per_context */
  ...
  mmgsdi_session_id_type       *session_id_ptr )
{
  ...
  *session_id_ptr = mmgsdi_util_get_next_available_id();
...
void mmgsdi_open_base_sessions(void)
{
  ...
  /* Allow MMGSDI to receive events for a CSIM hidden from other clients */
  mmgsdi_client_id_reg_table[index]->receive_hidden_csim_events = TRUE;
  ...
  if(mmgsdi_generic_data_ptr->pri_gw_session_id == 0)
  {
    ...
    (void)mmgsdi_refresh_all_register(mmgsdi_generic_data_ptr->pri_gw_session_id, TRUE, TRUE);
  }
  ...
  if(mmgsdi_generic_data_ptr->sec_gw_session_id == 0)
  {
    ...
    /*Register for Refresh*/
    (void)mmgsdi_refresh_all_register(mmgsdi_generic_data_ptr->sec_gw_session_id, TRUE, TRUE);
  }
  ...
  if(mmgsdi_generic_data_ptr->ter_gw_session_id == 0)
  {
    ...
    /*Register for Refresh*/
    (void)mmgsdi_refresh_all_register(mmgsdi_generic_data_ptr->ter_gw_session_id, TRUE, TRUE);
  }
  ...
  if(mmgsdi_generic_data_ptr->pri_1x_session_id == 0)
  {
    ...
    /*Register for Refresh*/
    (void)mmgsdi_refresh_all_register(mmgsdi_generic_data_ptr->pri_1x_session_id, FALSE, TRUE);
  }
  ...
  if(mmgsdi_generic_data_ptr->sec_1x_session_id == 0)
  {
    ...
    /*Register for Refresh*/
    (void)mmgsdi_refresh_all_register(mmgsdi_generic_data_ptr->sec_1x_session_id, FALSE, TRUE);
  }
  ...
  if(mmgsdi_generic_data_ptr->ter_1x_session_id == 0)
  {
    ...
    /*Register for Refresh*/
    (void)mmgsdi_refresh_all_register(mmgsdi_generic_data_ptr->ter_1x_session_id, FALSE, TRUE);
  }
  ...
  if(mmgsdi_generic_data_ptr->slot_session_id[MMGSDI_SLOT_1_INDEX] == 0)
  {
    ...
    /* Register for Refresh for card session because we need to delete common attr cache 
       during Refresh FCN */
      (void)mmgsdi_refresh_all_register(mmgsdi_generic_data_ptr->slot_session_id[MMGSDI_SLOT_1_INDEX], FALSE, FALSE);
  }
  ...
  if(mmgsdi_generic_data_ptr->slot_session_id[MMGSDI_SLOT_2_INDEX] == 0)
  {
    ...
    /* Register for Refresh for card session because we need to delete common attr cache 
       during Refresh FCN */
    (void)mmgsdi_refresh_all_register(mmgsdi_generic_data_ptr->slot_session_id[MMGSDI_SLOT_2_INDEX], FALSE, FALSE);
  }
  ...
  if(mmgsdi_generic_data_ptr->slot_session_id[MMGSDI_SLOT_3_INDEX] == 0)
  {
    ...
    /* Register for Refresh for card session because we need to delete common attr cache 
       during Refresh FCN */
    (void)mmgsdi_refresh_all_register(mmgsdi_generic_data_ptr->slot_session_id[MMGSDI_SLOT_3_INDEX], FALSE, FALSE);
  }
...
/* ============================================================================
FUNCTION MMGSDI_CLOSE_NON_PROV_SESSIONS

DESCRIPTION
  This utility function checks if the client has any non-prov sessions open
  and queues SESSION_CLOSE requests for those sessions.
...
============================================================================*/
static mmgsdi_return_enum_type mmgsdi_close_non_prov_sessions(
  int32                  client_index
)
{
  mmgsdi_return_enum_type mmgsdi_status = MMGSDI_ERROR;
  ...
  MMGSDIUTIL_RETURN_IF_NULL(mmgsdi_client_id_reg_table[client_index]);
  ...
  for (i = 0; (num_sessions > 0) && (i < MMGSDI_MAX_SESSION_INFO) ; i++)
  {
    ...
    /* Send a CLOSE request if a non-prov session is found */
    switch (mmgsdi_client_id_reg_table[client_index]->session_info_ptr[i]->session_type)
    {
    ...
    mmgsdi_status = mmgsdi_session_queue_close_req_to_task(
                      mmgsdi_client_id_reg_table[client_index]->session_info_ptr[i]->session_id,
                      TRUE,
                      mmgsdi_task,
  ...
  return MMGSDI_SUCCESS;
} /* mmgsdi_close_non_prov_sessions */


/*===========================================================================
FUNCTION:      MMGSDI_QUEUE_CLIENT_DEREG_REQ_TO_SLOT_TASKS

DESCRIPTION
  This utility function sends the DEREG request to the slot tasks.
...
===========================================================================*/
static mmgsdi_return_enum_type  mmgsdi_queue_client_dereg_req_to_slot_tasks (
  const mmgsdi_client_id_dereg_req_type * req_ptr,
  int32                                   client_index
)
{
  mmgsdi_task_enum_type   mmgsdi_task   = MMGSDI_TASK_SLOT1;
  mmgsdi_return_enum_type mmgsdi_status = MMGSDI_ERROR;

  MMGSDIUTIL_CHECK_CLIENT_APP_DATA_CRIT_SECT_ACCESS;

  MMGSDIUTIL_RETURN_IF_OUT_OF_RANGE(client_index, 0, MMGSDI_MAX_CLIENT_INFO - 1);
  MMGSDIUTIL_RETURN_IF_NULL_2(req_ptr, mmgsdi_client_id_reg_table[client_index]);

  for (mmgsdi_task = MMGSDI_TASK_SLOT1; mmgsdi_task < MMGSDI_NUM_TASKS; mmgsdi_task++)
  {
    mmgsdi_status = mmgsdi_util_queue_client_dereg_req(req_ptr->request_header.client_id,
                                                       mmgsdi_task,
                                                       req_ptr->request_header.response_cb,
                                                       req_ptr->request_header.client_data);
    if (mmgsdi_status == MMGSDI_SUCCESS)
    {
      mmgsdi_client_id_reg_table[client_index]->slot_task_dereg_count++;
    }
  }

  if (mmgsdi_client_id_reg_table[client_index]->slot_task_dereg_count == 0)
  {
    /* The request was not queued to any slot task. Return failure so that
       it can be processed further in the main task */
    return MMGSDI_ERROR;
  }

  return MMGSDI_SUCCESS;
} /* mmgsdi_queue_client_dereg_req_to_slot_tasks */
...
/*===========================================================================
   ...
   DESCRIPTION:
     ...
     In case of main task, if the client has non-prov sessions,
     SESSION CLOSE and dereg requests are queued to slot tasks to allow
...
)
{
  ...
  uint32                  client_req_index = 0;
  mmgsdi_task_enum_type   mmgsdi_task      = MMGSDI_TASK_MAX;
  boolean                 skip_response    = FALSE;
  ...
  do
  {
    ...
    /* If the dereg is being processed in slot task, simply decrement the
       counter which is initially set to the number of slot tasks. The last
       task to decrement the counter performs the deregistration. This ensures
       that the closing of non-prov sessions for that client has been processed
       in the slot tasks. */
    if (mmgsdi_task != MMGSDI_TASK_MAIN)
    {
      if (mmgsdi_client_id_reg_table[client_index]->slot_task_dereg_count == 0)
      {
        /* This should never happen because the initial counter corresponds to
           the number of tasks! */
        skip_response = TRUE;
        break;
      }
    ...
    if(client_has_non_prov_open && mmgsdi_task == MMGSDI_TASK_MAIN)
    {
      /* If the client has non-prov sessions, we need to close them before
         deregistering the client. Queueing the dereg req to slot tasks
         ensures that the deregistration happens after the non-prov
         sessions are closed. */
      mmgsdi_status = mmgsdi_close_non_prov_sessions(client_index);
      if (mmgsdi_status == MMGSDI_SUCCESS)
      {
        mmgsdi_status = mmgsdi_queue_client_dereg_req_to_slot_tasks(req_ptr,
                                                                    client_index);
  ...
  if (skip_response)
  {
    UIM_MSG_LOW_0("skipping response for CLIENT_ID_AND_EVT_DEREG req");
    return MMGSDI_SUCCESS;
  }
...
)
{
  ...
  switch(req_ptr->refresh_req_type)
  {
    case MMGSDI_REFRESH_ORIG_REFRESH_REQ:
      ...
      {
        var_len = req_ptr->refresh.card_req.plmn_list.data_len;
      }
      else
      {
        var_len = req_ptr->refresh.card_req.refresh_files.data_len;
      }
      break;
       ...
       req_ptr->refresh_req_type == MMGSDI_REFRESH_ORIG_PUP_REQ)
  {
    ...
    msg_ptr->refresh.card_req.pup_option =
      req_ptr->refresh.card_req.pup_option;
          ...
          req_ptr->refresh_req_type == MMGSDI_REFRESH_ORIG_DEREG_ALL_REQ )
  {
    ...
    msg_ptr->refresh.reg_req.vote_for_init =
      req_ptr->refresh.reg_req.vote_for_init;
    msg_ptr->refresh.reg_req.steering_of_roaming =
...
)
{
  ...
  mmgsdi_return_enum_type      mmgsdi_status  = MMGSDI_ERROR;
  qword                        time_ms        = {0, 0};
  uint32                       client_index   = MMGSDI_MAX_CLIENT_INFO;
  uint32                       session_index  = MMGSDI_MAX_SESSION_INFO;
  boolean                      is_nvruim_req  = FALSE;
  ...
  do
  {
    ...
    /* Add timestamp (in ms) to the command before queuing to the queue */
    time_get_ms(time_ms);
    task_cmd_ptr->cmd.cmd.request_header.timestamp = (((uint64)time_ms[1]) << 32) | ((uint64)time_ms[0]);
    ...
    switch (task_cmd_ptr->cmd.cmd_enum)
    {
      ...
      case MMGSDI_CNF:
        ...
        /* Set task response queue signal */
        mmgsdi_status = mmgsdi_util_set_signal_for_task(mmgsdi_task,
                                                        MMGSDI_TASK_RSP_Q_SIG);
        break;
      ...
      case MMGSDI_NOTIFY_RECOVERY_START_REQ:
        ...
        /* Set card status command signal */
        mmgsdi_status = mmgsdi_util_set_signal_for_task(mmgsdi_task,
                                                        MMGSDI_CARD_STATUS_CMD_Q_SIG);
        break;

      /* Continue_after_nv_refresh req is also a part of init happens
         before card_inserted and session_changed. So, it's better to queue
         it on internal queue to not starve an app activation */
      case MMGSDI_INTERNAL_PUP_INIT_REQ:
      case MMGSDI_CONTINUE_AFTER_NV_REFRESH_REQ:
        (void)q_link(task_cmd_ptr, &task_cmd_ptr->link);
        ...
        /* Set task command signal */
        mmgsdi_status = mmgsdi_util_set_signal_for_task(mmgsdi_task,
                                                        MMGSDI_TASK_CMD_Q_SIG);
        break;

      case MMGSDI_READ_REQ:
        /* If NVRUIM request is re-queued from MMGSDI main task to a slot specific task or a read of a
           file required to acquire service then place the request in a high
           priority queue else break and place command in regular queue 
           Usage of high priority queue is not strictly required when NVRUIM
           queues request to  the main task, as all operations are performed
           very quickly and without delays*/
        current_task = mmgsdi_util_get_current_task();
        is_nvruim_req =  MMGSDIUTIL_IS_MMGSDI_TASK(current_task) &&
                        (mmgsdi_util_get_client_and_session_index(
                           task_cmd_ptr->cmd.cmd.read_req.request_header.session_id,
                           &client_index,
                           &session_index) == MMGSDI_SUCCESS) &&
                        (client_index < MMGSDI_MAX_CLIENT_INFO) &&
                        (MMGSDIUTIL_IS_NVRUIM_CLIENT(client_index));
      ...
      case MMGSDI_SESSION_OPEN_WITH_MF_REQ:
      case MMGSDI_SEARCH_REQ:
        /* Process NVRUIM commands via high priority queue 
           For session open and client id reg requests, NVRUIM will provide
           client data to identify it. For rest of requests we need to check
           client name*/
        if(task_cmd_ptr->cmd.cmd_enum == MMGSDI_SEARCH_REQ ||
        ...
        else if((task_cmd_ptr->cmd.cmd.read_req.request_header.client_data &
...
)
{
  ...
  /* If Refresh modes are other than RESET and RESET_AUTO and no AID is indicated,
     then the terminal shall assume the REFRESH applies to the NAA application
     currently selected on the basic logical channel; ETSI 102 223 section 6.4.7 */
  if(req_ptr->refresh_req_type == MMGSDI_REFRESH_ORIG_REFRESH_REQ &&
     (req_ptr->refresh.card_req.refresh_mode != MMGSDI_REFRESH_RESET &&
      req_ptr->refresh.card_req.refresh_mode != MMGSDI_REFRESH_RESET_AUTO))
  {
    if(req_ptr->refresh.card_req.aid.aid.data_len == 0)
    {
      mmgsdi_status = mmgsdi_get_protocol(client_id,
                                          card_slot,
                                          &protocol_data);
      if(mmgsdi_status == MMGSDI_SUCCESS)
      {
        /* When there is REFRESH Request queued due to RUIM deactivation, APP type should NOT be updated from RUIM to SIM */
        if(protocol_data != MMGSDI_ICC || 
           (protocol_data == MMGSDI_ICC && req_ptr->refresh.card_req.aid.app_type == MMGSDI_APP_NONE))
        {
          ...
          if(mmgsdi_status == MMGSDI_SUCCESS)
          {
            ...
            mmgsdi_memscpy(&req_ptr->refresh.card_req.aid,
                           sizeof(req_ptr->refresh.card_req.aid),
...
/*===========================================================================
FUNCTION MMGSDI_GET_ME_CONFIG_PARAMS_FROM_NV
...
)
{
  ...
  if(refresh_files.file_path_ptr != NULL)
  {
    ...
    temp_len = (refresh_files.file_path_len + 1) * sizeof(uint8);
...
SIDE EFFECTS
  None
===========================================================================*/
mmgsdi_return_enum_type mmgsdi_set_ens_support_flag(
  mmgsdi_session_id_type     session_id,
  mmgsdi_slot_id_enum_type   slot,
...
)
{
  ...
  /* The file EF-RAT Mode is only present in USIM application */
  ...
  MMGSDIUTIL_MEM_MALLOC_AND_VALIDATE(req_ptr->data.data_ptr, spt_file_len);
  ...
  /*Set the 3G Flag only if feature flag are set,
    modem cap indicates gsm or wcdma available */
 ...
 if (num_of_rats > 1 && 
     (!((modem_cap & UIM_MODEM_CAP_FEATURE_LTE_M1_MASK) || (modem_cap & UIM_MODEM_CAP_FEATURE_LTE_NB1_MASK)))) 
  {
  	  ...
  	  (nv_item.nv_item_data.uicc_rat_balancing_support))
    {
      ...
      if((modem_cap & UIM_MODEM_CAP_FEATURE_LTE_MASK) && MMGSDI_APP_SIM == app_type)
      {
        req_ptr->data.data_ptr[4] = 0x01;
      }
    }
  ...
  if(modem_cap & UIM_MODEM_CAP_FEATURE_LTE_MASK)
  {
     ...
     */
    if((mmgsdi_util_get_efs_item_index(session_id, NULL, &sub_nv_context)   == MMGSDI_SUCCESS) &&
  ...
  {
    uint8                               nv_data_psm[8]              = {0};
    uint8                               nv_data_edrx[9]             = {0};
    boolean                             nv_data_psm_value           = FALSE;
    boolean                             nv_data_edrx_value          = FALSE;
    mmgsdi_iot_category_enum            mmgsdi_iot_category         = MMGSDI_IOT_UNKNWON;

    /* MTC support category, byte 10 in EF TST */
    mmgsdi_iot_category = mmgsdi_get_iot_category();
    if (mmgsdi_iot_category != MMGSDI_IOT_UNKNWON)
    {
      req_ptr->data.data_ptr[9] = (uint8)mmgsdi_iot_category;

      /* bytes 11, 12, 13 in EF TST are reserved for MTC features support */
	 
      /* MTC support feature, byte 11 in EF TST */
      req_ptr->data.data_ptr[10] = 
        (mmgsdi_is_iot_capability_supported(MMGSDI_IOT_NAS_SIGNALING_PRIORITY_SUPPORT)              ? 0x01 : 0) |
         /* bit 2 is RFU */
        (mmgsdi_is_iot_capability_supported(MMGSDI_IOT_EXTENDED_ACCESS_BARRING_SUPPORT)             ? 0x04 : 0) |
        (mmgsdi_is_iot_capability_supported(MMGSDI_IOT_TIMER_T3245_SUPPORT)                         ? 0x08 : 0) |
         /* bits 5 and 6 are RFU */ 
        (mmgsdi_is_iot_capability_supported(MMGSDI_IOT_SM_RETRY_WAIT_TIME_SUPPORT)                  ? 0x40 : 0) |
        (mmgsdi_is_iot_capability_supported(MMGSDI_IOT_DELAY_TOLERANT_ACCESS_SUPPORT)               ? 0x80 : 0);

      /* Check MMODE PSM NV 73769 is enabled */
      if (uim_common_efs_read(UIM_COMMON_EFS_NON_UIM_PSM_MODE,
      					  UIM_COMMON_EFS_ITEM_FILE_TYPE,
      					  UIM_COMMON_EFS_DEVICE,
      					  (uint8 *)nv_data_psm,
      					  sizeof(nv_data_psm)) == UIM_COMMON_EFS_SUCCESS)
      {
        nv_data_psm_value = nv_data_psm[1];
      }

      /* Check EDRX NV 73871 is enabled */
      if (uim_common_efs_read(UIM_COMMON_EFS_NON_UIM_EDRX_SUPPORT,
      					  UIM_COMMON_EFS_ITEM_FILE_TYPE,
      					  UIM_COMMON_EFS_DEVICE,
      					  (uint8 *)nv_data_edrx,
      					  sizeof(nv_data_edrx)) == UIM_COMMON_EFS_SUCCESS)
      {
        nv_data_edrx_value = nv_data_edrx[6];
      }

      /* MTC support feature, byte 12 in EF TST */
      req_ptr->data.data_ptr[11] = 
        ((mmgsdi_is_iot_capability_supported(MMGSDI_IOT_POWER_SAVING_MODE_PSM_SUPPORT ) && nv_data_psm_value == 1)                       ? 0x01 : 0) |
        (mmgsdi_is_iot_capability_supported(MMGSDI_IOT_COVERAGE_ENHANCEMENT_MODE_A_SUPPORT)                                              ? 0x02 : 0) |
        (mmgsdi_is_iot_capability_supported(MMGSDI_IOT_COVERAGE_ENHANCEMENT_MODE_B_SUPPORT)                                              ? 0x04 : 0) |
        ((mmgsdi_is_iot_capability_supported(MMGSDI_IOT_IDLE_MODE_EXTENDED_DISCONTINUOUS_RECEPTION_SUPPORT) && nv_data_edrx_value == 1)  ? 0x08 : 0) |
        (mmgsdi_is_iot_capability_supported(MMGSDI_IOT_CONNECTED_MODE_EXTENDED_DISCONTINUOUS_RECEPTION_SUPPORT)                          ? 0x10 : 0) |
        (mmgsdi_is_iot_capability_supported(MMGSDI_IOT_DEVICE_BASED_POSITIONING_SUPPORT)                                                 ? 0x20 : 0) |
        (mmgsdi_is_iot_capability_supported(MMGSDI_IOT_SINGLE_CELL_POINT_TO_MULTIPOINT_TRANSMISSION_SUPPORT)                             ? 0x40 : 0);
    }
  }

  mmgsdi_status = mmgsdi_uim_common_write_transparent(req_ptr, TRUE, &cnf_ptr, protocol);
...
)
{
  ...
  if((mmgsdi_util_get_efs_item_index(req_ptr->request_header.session_id, NULL, &nv_context) ==
        MMGSDI_SUCCESS) &&
...
)
{
  ...
  /* Since card err is notified first before rsp report is sent back to MMGSDI
     from UIMDRV during pdown req, we need to check if the request is to pdown
     and not cancel it to ensure that we do not send an error cnf in that case*/
  for(index = 0; index < MMGSDI_MAX_CLIENT_REQ_INFO; index++)
  {
    if(mmgsdi_client_req_table_info_ptr[index] == NULL ||
       mmgsdi_client_req_table_info_ptr[index]->slot_id != slot_id ||
       mmgsdi_client_req_table_info_ptr[index]->request_type == MMGSDI_CARD_PDOWN_REQ)
    {
      continue;
    }
...
/*=======================================================================
FUNCTION  MMGSDI_IS_SYNC_CMD_CANCELED

DESCRIPTION:
  This function is called to check whether a sync command was canceled
  for this slot
...
SIDE EFFECTS:
  None
=======================================================================*/
boolean mmgsdi_is_sync_cmd_canceled
(
  mmgsdi_slot_id_enum_type  slot_id
)
{
  ...
  if ((mmgsdi_util_get_slot_index(slot_id, &slot_index) == MMGSDI_SUCCESS) &&
      mmgsdi_sync_cmd_canceled[slot_index])
  {
    return TRUE;
  }

  return FALSE;
}/* mmgsdi_is_sync_cmd_canceled */
