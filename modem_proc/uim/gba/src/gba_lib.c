...
07/23/15   yt      Update gba_bootstrap API parameters
...
PARAMETERS
  ...
  bootstrap_ext_info_ptr: Pointer to extra info
...
gba_result_enum_type gba_bootstrap (
  ...
  const gba_bootstrap_ext_info_type  * bootstrap_ext_info_ptr,
...
)
{
  ...
  if(bootstrap_ext_info_ptr != NULL &&
     bootstrap_ext_info_ptr->apn_ptr != NULL &&
     strlen(bootstrap_ext_info_ptr->apn_ptr) >= GBA_MAX_APN_LEN)
  {
    return GBA_INCORRECT_PARAMS;
  }
  ...
  if(bootstrap_ext_info_ptr != NULL)
  {
    if(bootstrap_ext_info_ptr->apn_ptr != NULL)
    {
      task_cmd_ptr->gba_bootstrap.apn_info.apn_available = TRUE;
      strlcpy(task_cmd_ptr->gba_bootstrap.apn_info.apn,
              bootstrap_ext_info_ptr->apn_ptr,
              sizeof(task_cmd_ptr->gba_bootstrap.apn_info.apn));
    }

    if(bootstrap_ext_info_ptr->bsf_info_ptr != NULL)
    {
      (void)memscpy(&task_cmd_ptr->gba_bootstrap.bsf_info,
                    sizeof(task_cmd_ptr->gba_bootstrap.bsf_info),
                    bootstrap_ext_info_ptr->bsf_info_ptr,
                    sizeof(gba_bsf_info_type));
    }

    task_cmd_ptr->gba_bootstrap.evt_callback = bootstrap_ext_info_ptr->evt_callback;
    task_cmd_ptr->gba_bootstrap.evt_user_data = bootstrap_ext_info_ptr->evt_user_data;
  }
  ...
  GBA_MSG_MED_1("Queue of command: GBA bootstrap status 0x%x", gba_status);
...
)
{
  ...
  for(bsf_index = 0; bsf_index < GBA_MAX_NUM_BSF; bsf_index++)
  {
    /* If B-TID does not match then move to next BSF */
    if(btid_ptr->data_len <= GBA_MAX_BTID_LEN &&
       btid_ptr->data_len == identity_profile_ptr->bootstrap_info[bsf_index].btid.data_len &&
       (memcmp(btid_ptr->data, identity_profile_ptr->bootstrap_info[bsf_index].btid.data, btid_ptr->data_len) == 0))
    {
      /* Check whether the life time of Ks has expired  */
      if(gba_lifetime_is_ks_expired(identity_profile_ptr->bootstrap_info[bsf_index].lifetime) == FALSE)
      {
        result = TRUE;
      }
      ...
      if(life_time_ptr)
      {
        *life_time_ptr = identity_profile_ptr->bootstrap_info[bsf_index].lifetime;
      }
      break;
    }
  }
