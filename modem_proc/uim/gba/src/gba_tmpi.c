...
--------   ---     ----------------------------------------------------------
05/07/18   nr       Remote SIM Unlock support to switch configuration
  ...
  gba_impi_type                   * tmpi_ptr)
{
  ...
  tmpi_temp_ptr = uim_common_util_alloc_bin_to_base64string(*naf_ptr, GBA_TMPI_OCTETS_OF_NAF_FOR_TEMP);
...
)
{
  ...
  if(identity_profile_ptr->impi.data_len == impi_ptr->data_len &&
     ...
     (memcmp(identity_profile_ptr->impi.data,
             impi_ptr->data,
             impi_ptr->data_len) == 0) &&
     identity_profile_ptr->bootstrap_info[bsf_index].tmpi.data_len > 0 &&
     identity_profile_ptr->bootstrap_info[bsf_index].tmpi.data_len <= GBA_MAX_IMPI_LEN)
  {
    tmpi_ptr->data_len = identity_profile_ptr->bootstrap_info[bsf_index].tmpi.data_len;
    memscpy(tmpi_ptr->data,
            tmpi_ptr->data_len,
            identity_profile_ptr->bootstrap_info[bsf_index].tmpi.data,
            identity_profile_ptr->bootstrap_info[bsf_index].tmpi.data_len);
