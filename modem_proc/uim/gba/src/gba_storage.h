...
--------   ---     ----------------------------------------------------------
11/16/15   yt      Support for second BSF and key expiry event
...
=============================================================================*/
/* --------------------------------------------------------------------------
   STRUCTURE:      GBA_STORAGE_BOOTSTRAP_INFO_TYPE

   DESCRIPTION:
     Profile data including the BSF info
---------------------------------------------------------------------------*/
typedef struct {
  gba_impi_type                       tmpi;
  gba_rand_type                       rand;
...
} gba_storage_bootstrap_info_type;
...
/* ----------------------------------------------------------------------------
   ...
   DESCRIPTION:
     ...
     procedure. Array of profiles for 2 BSFs for a common AID and IMPI.
     Index 0 is for default BSF that is derived from IMSI/IMPI. Index 1 is
     for the custom BSF address provided by the client.
-------------------------------------------------------------------------------*/
typedef struct {
  gba_aid_type                        aid;
  gba_impi_type                       impi;
  gba_storage_bootstrap_info_type     bootstrap_info[GBA_MAX_NUM_BSF];
