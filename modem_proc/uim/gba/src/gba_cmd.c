...
#define GBA_CMD_QUEUE_MAX_SIZE            10
...
)
{
  ...
  if (!gba_task_stopped)
  {
    /* Queue command only if queue size is less than GBA_CMD_QUEUE_MAX_SIZE */
    if(q_cnt(&gba_cmd_q) < GBA_CMD_QUEUE_MAX_SIZE)
    {
  ...
  if(gba_status != GBA_SUCCESS)
  {
    GBA_MSG_ERR_1("Cannot queue GBA command: 0x%x", gba_status);
  }
