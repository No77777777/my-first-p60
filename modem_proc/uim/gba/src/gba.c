...
06/30/15   lm      Removed memleak.h include
06/16/15   vr      Add task ready signal and offtarget includes
...
)
{
  rex_sigs_type                       sigs         = GBA_CMD_SIG                    |
                                                     ...
                                                     GBA_TASK_STOP_SIG              |
                                                     GBA_KEY_VALIDITY_TIMER_EXPIRE_SIG;
  ...
  mcs_rcevt_signal_name("GBA:READY");
  ...
  for (;;)
  {
    ...
    /*-----------------------------------------------------------------------------
                          Process Signal - GBA_KEY_VALIDITY_TIMER_EXPIRE_SIG
    -----------------------------------------------------------------------------*/
    else if (sigs_rcvd & GBA_KEY_VALIDITY_TIMER_EXPIRE_SIG)
    {
      gba_handle_key_validity_timer_expire_sig();
    } /* end if GBA_KEY_VALIDITY_TIMER_EXPIRE_SIG */
