...
$Header: //components/rel/uim.mpss/6.1.0/gba/src/gba_nv.c#2 $ $DateTime: 2019/10/30 06:56:05 $ $Author: pwbldsvc $
...
typedef PACKED struct PACKED_POST {
  ...
  uint16                       bsf_port;
  gba_bsf_protocol_enum_type   protocol;
...
static gba_nv_info_type   gba_nv_info = {GBA_SLOT_NONE,         /* slot id for GBA NV read */
...
DESCRIPTION
  Function initialization of gba nv, will read all GAB related NV (slot-based),
  and cache them in a static structure.

DEPENDENCIES
  This function should be called before slot-based gba_nv_get_* APIs
...
)
{
  ...
  if (gba_nv_info.slot_id == slot_id)
  {
    return;
  }
  ...
  if ((uim_common_efs_read(UIM_COMMON_EFS_GBA_BOOTSTRAP_BSF_ADDRESS,
                           UIM_COMMON_EFS_ITEM_FILE_TYPE,
                           efs_context,
                           (uint8 *)gba_nv_info.bsf_address_ptr,
                           sizeof(gba_nv_bsf_address_type)) != UIM_COMMON_EFS_SUCCESS))
  {
  ...
  if ((uim_common_efs_read(UIM_COMMON_EFS_GBA_BOOTSTRAP_APN,
                           UIM_COMMON_EFS_ITEM_FILE_TYPE,
                           efs_context,
                           (uint8 *)gba_nv_info.apn_ptr,
                           sizeof(gba_nv_apn_type)) != UIM_COMMON_EFS_SUCCESS))
  {
    GBA_FREE(gba_nv_info.apn_ptr);
  }

  /* get GBA custom realm NV */
  (void)uim_common_efs_read(UIM_COMMON_EFS_GBA_BOOTSTRAP_CUSTOM_REALM,
                            UIM_COMMON_EFS_ITEM_FILE_TYPE,
                            efs_context,
                            (uint8 *)&gba_nv_info.custom_realm,
                            sizeof(gba_nv_custom_realm_enum_type));

  /* get GBA TMPI user agent NV */
  (void)uim_common_efs_read(UIM_COMMON_EFS_GBA_BOOTSTRAP_CUSTOM_USER_AGENT_TMPI,
                            UIM_COMMON_EFS_ITEM_FILE_TYPE,
                            efs_context,
                            (uint8 *)&gba_nv_info.user_agent_tmpi,
                            sizeof(boolean));

  /* get GBA IMEI header NV */
  (void)uim_common_efs_read(UIM_COMMON_EFS_GBA_BOOTSTRAP_HTTP_HEADER_IMEI,
                            UIM_COMMON_EFS_ITEM_FILE_TYPE,
                            efs_context,
                            (uint8 *)&nv_imei_header,
...
)
{
  ...
  if (gba_nv_info.slot_id == GBA_SLOT_NONE)
  {
    gba_nv_init(slot_id);
  }

  if (bsf_ptr == NULL ||
      slot_id == GBA_SLOT_NONE ||
...
RETURN VALUE
  gba_bsf_protocol_enum_type

SIDE EFFECTS
  None
===========================================================================*/
gba_bsf_protocol_enum_type gba_nv_get_bsf_protocol(
  gba_slot_type  slot_id
)
{
  if (gba_nv_info.slot_id == GBA_SLOT_NONE)
  ...
  if (gba_nv_info.bsf_address_ptr == NULL ||
      slot_id == GBA_SLOT_NONE)
  {
    return GBA_BSF_PROTOCOL_HTTP;
  }

  if (GBA_BSF_PROTOCOL_HTTP == gba_nv_info.bsf_address_ptr->protocol ||
      GBA_BSF_PROTOCOL_HTTPS == gba_nv_info.bsf_address_ptr->protocol)
  {
...
{
  if (slot_id == GBA_SLOT_NONE)
  {
    return GBA_NV_REALM_PER_SPEC;
  }
...
)
{
  ...
  if(imei_header_ptr == NULL ||
     slot_id == GBA_SLOT_NONE ||
     gba_nv_info.http_header_imei_ptr == NULL ||
     gba_nv_info.http_header_imei_ptr->imei_header_name[0] == 0x0)
  {
    return GBA_GENERIC_ERROR;
  }
