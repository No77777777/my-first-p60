...
$Header: //components/rel/uim.mpss/6.1.0/gba/src/gba_mmgsdi.c#2 $ $DateTime: 2019/10/30 06:56:05 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
01/04/17   me      Updated read CNF case to access by enum for EF IST 
12/27/16   me      Updated read EF IST by path to read by enum 
...
10/22/15   tkl     Allow GBA to proceed if fail to read EF-GBABP content
...
#define GBA_MMGSDI_MAX_PROV_SESSION             2

#define GBA_SW1_COMMAND_NOT_ALLOWED             0x69
#define GBA_SW2_CNDTNS_OF_USE_NOT_SATISFIED     0x85
...
)
{
  ...
  switch (cnf)
  {
    ...
    case MMGSDI_SESSION_CLOSE_CNF:
      ...
      GBA_MSG_MED_3("Session close cnf: status 0x%x, for session id 0x%x and app type 0x%x",status,
...
)
{
  ...
  /*
   ...
   4. Trigger CB
  */
  GBA_MSG_MED_0("GBA MMGSDI process USIM session open");

  session_info_query.query_type = MMGSDI_SESSION_INFO_QUERY_BY_ID;
  session_info_query.query_key.session_id =
          gba_mmgsdi_client_info.session_id[gba_mmgsdi_cmd_info.identity_profile.session_type];
  ...
  mmgsdi_status =
    mmgsdi_session_read_cache(gba_mmgsdi_client_info.session_id[gba_mmgsdi_cmd_info.identity_profile.session_type],
  ...
  if(ust_data[8] & 0x08)
  {
    gba_mmgsdi_cmd_info.identity_profile.mode = GBA_U;
  }
  ...
  mmgsdi_status =
    mmgsdi_session_read_cache_file_size(gba_mmgsdi_client_info.session_id[gba_mmgsdi_cmd_info.identity_profile.session_type],
  ...
  /* update global cmd info */
  gba_mmgsdi_cmd_info.identity_profile.prov_session_type
                                                 = gba_mmgsdi_cmd_info.identity_profile.session_type;
  gba_mmgsdi_cmd_info.identity_profile.slot_id   = session_get_info.slot_id;
  gba_mmgsdi_cmd_info.identity_profile.impi      = impi_data;

  /* update callback data */
  callback_data.cmd_type                         = GBA_RETRIEVE_IDENTITY_CMD;
  callback_data.data.identity                    = gba_mmgsdi_cmd_info.identity_profile;
...
/*===========================================================================
   FUNCTION:      GBA_MMGSDI_GET_PROV_SESSION_AND_SLOT_FOR_ISIM

   DESCRIPTION:
     This function get the slot id and provisioning session type resides
     on the same slot as ISIM session.
   ...
   SIDE EFFECTS:
     None
===========================================================================*/
static gba_result_enum_type gba_mmgsdi_get_prov_session_and_slot_for_isim(
  gba_session_type              session_type,
  gba_session_type            * provisioning_session_ptr,
  gba_slot_type               * gba_slot_id_ptr
)
{
  mmgsdi_slot_id_enum_type       isim_slot_id           = MMGSDI_MAX_SLOT_ID_ENUM;
  mmgsdi_session_type_enum_type  mmgsdi_prov_session[GBA_MMGSDI_MAX_PROV_SESSION]
                                                        = {MMGSDI_GW_PROV_PRI_SESSION,
                                                           MMGSDI_GW_PROV_SEC_SESSION};
  gba_session_type               gba_prov_session[GBA_MMGSDI_MAX_PROV_SESSION]
  ...
  if (NULL == provisioning_session_ptr ||
      NULL == gba_slot_id_ptr)
  {
    return GBA_INCORRECT_PARAMS;
  }
  ...
  switch (session_type)
  {
    case GBA_NON_PROV_SESSION_SLOT_1:
      isim_slot_id     = MMGSDI_SLOT_1;
      *gba_slot_id_ptr = GBA_SLOT_1;
      break;

    case GBA_NON_PROV_SESSION_SLOT_2:
      isim_slot_id     = MMGSDI_SLOT_2;
      *gba_slot_id_ptr = GBA_SLOT_2;
      break;
  ...
  if (isim_slot_id == MMGSDI_MAX_SLOT_ID_ENUM)
  {
    return GBA_INCORRECT_PARAMS;
  }
  ...
  for (i = 0; i < GBA_MMGSDI_MAX_PROV_SESSION; i++ )
  {
    session_info_query.query_key.session_type = mmgsdi_prov_session[i];

    mmgsdi_status = mmgsdi_session_get_info_sync(session_info_query, &session_get_info);
    if(mmgsdi_status == MMGSDI_SUCCESS &&
       session_get_info.slot_id == isim_slot_id)
    {
      *provisioning_session_ptr = gba_prov_session[i];
      return GBA_SUCCESS;
    }
  }

  return GBA_GENERIC_ERROR;
}/* gba_mmgsdi_get_prov_session_and_slot_for_isim */
...
)
{
  ...
  if (gba_mmgsdi_cmd_info.identity_profile.session_type == GBA_3GPP_PROV_SESSION_PRI ||
      gba_mmgsdi_cmd_info.identity_profile.session_type == GBA_3GPP_PROV_SESSION_SEC)
  {
    auth_req.data_len += gba_mmgsdi_cmd_info.identity_profile.impi.data_len;
    /* for length field */
    auth_req.data_len += 1;
  }
  ...
  if (gba_mmgsdi_cmd_info.identity_profile.session_type == GBA_3GPP_PROV_SESSION_PRI ||
      gba_mmgsdi_cmd_info.identity_profile.session_type == GBA_3GPP_PROV_SESSION_SEC)
  {
    auth_req.data_ptr[len_offset] = gba_mmgsdi_cmd_info.identity_profile.impi.data_len;
    len_offset++;
    memscpy(&(auth_req.data_ptr[len_offset]),
            (auth_req.data_len - len_offset),
            gba_mmgsdi_cmd_info.identity_profile.impi.data,
            gba_mmgsdi_cmd_info.identity_profile.impi.data_len);

    len_offset += gba_mmgsdi_cmd_info.identity_profile.impi.data_len;
  }

  /* perform NAF Derivation */
  if (gba_mmgsdi_cmd_info.identity_profile.session_type == GBA_NON_PROV_SESSION_SLOT_1 ||
      gba_mmgsdi_cmd_info.identity_profile.session_type == GBA_NON_PROV_SESSION_SLOT_2)
  {
  ...
  else if (gba_mmgsdi_cmd_info.identity_profile.session_type == GBA_3GPP_PROV_SESSION_PRI ||
           gba_mmgsdi_cmd_info.identity_profile.session_type == GBA_3GPP_PROV_SESSION_SEC)
  {
...
)
{
  ...
  */
  ...
  GBA_MSG_MED_0("GBA MMGSDI GBABP Read done");
  ...
  {
    /* invalid RAND length */
    GBA_MSG_ERR_0("GBA MMGSDI EF GBABP Invalid data");
    return GBA_GENERIC_ERROR;
  }
...
)
{
  ...
  callback_data.data.auth_status.session_type = gba_mmgsdi_cmd_info.identity_profile.session_type;
  ...
  if (rsp_data_ptr->status == MMGSDI_SUCCESS)
  {
    ...
    else if (gba_mmgsdi_cmd_info.auth_context == MMGSDI_AUTN_ISIM_GBA_NAF_DERIVATION_CONTEXT)
    {
      ...
      {
        memscpy(callback_data.data.ks_naf_status_data.ks_ext_naf,
                sizeof(callback_data.data.ks_naf_status_data.ks_ext_naf),
                rsp_data_ptr->cnf_data.isim_auth_cnf.res.data_ptr,
                rsp_data_ptr->cnf_data.isim_auth_cnf.res.data_len);
      }
      else
  ...
  else if (rsp_data_ptr->status == MMGSDI_AUTS_FAIL)
  {
    ...
    /* Do not remove log print */
    GBA_MSG_MED_0("GBA MMGSDI ISIM AUTH SYNC FAILURE");
  }
  /* This error occurs when GBA Bootstrapping was performed and EF GBABP indicates a vaild Ks,
     but the card does not find valid Ks for GBA NAF generation. */
  else if (rsp_data_ptr->status == MMGSDI_ACCESS_DENIED &&
           gba_mmgsdi_cmd_info.auth_context == MMGSDI_AUTN_ISIM_GBA_NAF_DERIVATION_CONTEXT &&
           rsp_data_ptr->cnf_data.usim_auth_cnf.response_header.status_word.sw1 == GBA_SW1_COMMAND_NOT_ALLOWED &&
           rsp_data_ptr->cnf_data.usim_auth_cnf.response_header.status_word.sw2 == GBA_SW2_CNDTNS_OF_USE_NOT_SATISFIED)
  {
    /* set cmd type and auth result to trigger a retry in gba_bootstrap */
    gba_status = GBA_SUCCESS;
    callback_data.cmd_type = GBA_GEN_KEY_CMD;
    callback_data.data.ks_naf_status_data.result = GBA_AUTH_KS_FAILURE;
  }
...
)
{
  ...
  {
    gba_mmgsdi_cmd_info.identity_profile.mode = GBA_ME;
  }
  else
  {
    gba_mmgsdi_cmd_info.identity_profile.mode = GBA_U;
...
)
{
  ...
  /* After successful read of EF IMPI, Parse EF IMPI and extract the IMPI
     Format of EF IMPI is, | 0x80(1) | Len of IMPI(1) | IMPI |
  ...
  /* Copy IMPI data */
  gba_mmgsdi_cmd_info.identity_profile.impi.data_len = impi_len;
  memscpy(gba_mmgsdi_cmd_info.identity_profile.impi.data,
          GBA_MAX_IMPI_LEN,
          &rsp_data_ptr->cnf_data.read_cnf.read_data.data_ptr[2],
          impi_len);

  /* Get provisioning session and slot id */
  if( GBA_SUCCESS !=
      gba_mmgsdi_get_prov_session_and_slot_for_isim(gba_mmgsdi_cmd_info.identity_profile.session_type,
                                                    &gba_mmgsdi_cmd_info.identity_profile.prov_session_type,
                                                    &gba_mmgsdi_cmd_info.identity_profile.slot_id))
  {
    return GBA_GENERIC_ERROR;
  }
...
)
{
  ...
  if (gba_mmgsdi_cmd_info.reference_id != (uint32)rsp_data_ptr->cnf_data.response_header.client_data)
  {
    ...
    GBA_MSG_MED_3("While processing response for 0x%x, ref id 0x%x did not match with current ref id 0x%x",
  ...
  switch (rsp_data_ptr->cnf)
  {
    case MMGSDI_CLIENT_ID_AND_EVT_REG_CNF:
      ...
      gba_status = gba_mmgsdi_get_card_info(gba_mmgsdi_cmd_info.identity_profile.aid,
                                            gba_mmgsdi_cmd_info.identity_profile.session_type);
      break;
    ...
    case MMGSDI_SESSION_OPEN_CNF:
      ...
      else
      {
        ...
        /* read ISIM EF IST */
        file_access.access_method  = MMGSDI_EF_ENUM_ACCESS;
        file_access.file.file_enum = MMGSDI_ISIM_IST;
    ...
    case MMGSDI_READ_CNF:
      if (rsp_data_ptr->cnf_data.read_cnf.access.access_method == MMGSDI_EF_ENUM_ACCESS &&
          rsp_data_ptr->cnf_data.read_cnf.access.file.file_enum == MMGSDI_ISIM_IST)
      {
        /* Do not remove log print */
        GBA_MSG_MED_0("GBA MMGSDI ISIM IST Read done");
      ...
      {
        /* Do not remove log print */
        GBA_MSG_MED_0("GBA MMGSDI ISIM IMPI Read done");
    ...
    case MMGSDI_ISIM_AUTH_CNF:
      /* Do not remove log print */
      GBA_MSG_MED_1("GBA MMGSDI ISIM AUTH status 0x%x", rsp_data_ptr->status);
      gba_status = gba_mmgsdi_process_isim_auth_rsp(rsp_data_ptr);
      break;

    case MMGSDI_USIM_AUTH_CNF:
      /* Do not remove log print */
      GBA_MSG_MED_1("GBA MMGSDI USIM AUTH status 0x%x", rsp_data_ptr->status);
...
)
{
    ...
    case MMGSDI_CARD_ERROR_EVT:
      if(event_data_ptr->mmgsdi_evt_data.data.card_error.slot == MMGSDI_SLOT_1)
      {
            ...
            gba_mmgsdi_client_info.session_id[GBA_NON_PROV_SESSION_SLOT_1])
        {
          ...
          gba_mmgsdi_client_info.session_id[gba_mmgsdi_cmd_info.identity_profile.session_type] = 0;
          memset(&gba_mmgsdi_cmd_info, 0x00, sizeof(gba_mmgsdi_cmd_info));
        }
        session_type = GBA_NON_PROV_SESSION_SLOT_1;
      }
      else if(event_data_ptr->mmgsdi_evt_data.data.card_error.slot == MMGSDI_SLOT_2)
      {
        if (gba_mmgsdi_cmd_info.identity_profile.session_type == GBA_NON_PROV_SESSION_SLOT_2 &&
  ...
  {
    /* Send event to registered clients to indicate that Ks is invalid */
    gba_evt_send_key_event_for_session(session_type, GBA_KEY_DISCARDED_EVT);

    /* Clear the timer(s) for this session since the key has been discarded. */
    gba_clear_key_validity_timer_for_session(session_type);

    /* Remove data from EFS storage */
...
)
{
  ...
  if(task_cmd_ptr != NULL)
  {
    GBA_MSG_MED_1("GBA_MMGSDI_CMD_SIG received, cmd_type 0x%x",
...
)
{
  ...
  gba_mmgsdi_cmd_info.identity_profile.aid               = aid;
  gba_mmgsdi_cmd_info.identity_profile.session_type      = session_type;
  gba_mmgsdi_cmd_info.call_back                          = call_back;
  gba_mmgsdi_cmd_info.reference_id                       = reference_id;

  GBA_MSG_MED_2("GBA MMGSDI Retrive identity request session type:0x%x, ref_id:0x%x",
...
)
{
      ...
      gba_mmgsdi_cmd_info.identity_profile.mode != GBA_U)
  {
    GBA_MSG_ERR_3("Reference id did not match 0x%x, 0x%x or invalid GBA mode 0x%x",
                  gba_mmgsdi_cmd_info.reference_id,
                  reference_id,
                  gba_mmgsdi_cmd_info.identity_profile.mode);
...
)
{
  ...
  gba_status = gba_util_get_session_index_and_mmgsdi_session_type(gba_mmgsdi_cmd_info.identity_profile.session_type,
                                                                  &session_index,
                                                                  NULL);
  if(gba_status != GBA_SUCCESS)
  {
...
)
{
      ...
      gba_mmgsdi_client_info.session_id[gba_mmgsdi_cmd_info.identity_profile.session_type])
  {
    ...
    mmgsdi_status = mmgsdi_session_close(gba_mmgsdi_client_info.session_id[gba_mmgsdi_cmd_info.identity_profile.session_type],
...
gba_result_enum_type gba_mmgsdi_generate_ks_ext_naf(
  ...
  boolean                       naf_derivation
)
{
  /* Do not remove log print */
  GBA_MSG_MED_1("GBA MMGSDI GENERATE KS NAF ref id: 0x%x", reference_id);
  ...
  if (gba_mmgsdi_cmd_info.identity_profile.mode == GBA_ME)
  {
    return GBA_GENERIC_ERROR;
  }
  ...
  gba_mmgsdi_cmd_info.identity_profile.impi.data_len = impi_ptr->data_len;
  memscpy(gba_mmgsdi_cmd_info.identity_profile.impi.data,
          GBA_MAX_IMPI_LEN,
          impi_ptr->data,
          impi_ptr->data_len);
  ...
  /* Reading of EF GBABP is not required while generating TMPI or if Ks is valid */
  if(naf_derivation)
  {
