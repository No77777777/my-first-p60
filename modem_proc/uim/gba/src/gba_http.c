...
Copyright (c) 2015 - 2016, 2018 - 2019 QUALCOMM Technologies, Inc (QTI) and its licensors.
...
$Header: //components/rel/uim.mpss/6.1.0/gba/src/gba_http.c#2 $ $DateTime: 2019/10/30 06:56:05 $ $Author: pwbldsvc $
...
04/26/16   tkl     Added profile_info len in profile APN check
04/13/16   tkl     Added sub id for network availabity check
...
01/15/16   tkl     Add default value for switch case
12/22/15   vdc     While creating DS session send profile config subsid
12/15/15   vdc     While opening DS session send subsid instead of session type
11/16/15   yt      Support for second BSF and key status events
10/20/15   tkl     Commit and end ds profile transaction after APN update
10/20/15   tkl     Fixed APN availability check
10/13/15   tkl     Fixed auts len check for sync failure
09/30/15   tkl     memset iface_info for ds_http_open_session
09/23/15   tkl     Support NV multi-slot settings
09/22/15   tkl     Update use of iface type DS_HTTP_IFACE_ANY
09/22/15   tkl     Remove duplicate APN check
09/22/15   tkl     GBA DSDS support
09/16/15   tkl     fix KW errors
08/13/15   tkl     Update IMEI header NV struct
08/10/15   tkl     Make Server header as optional in HTTP response
08/05/15   tkl     Added WLAN support
...
06/10/15   vdc     Do not queue boostrap cmd if DS session open has failed
05/27/15   tkl     Support next nonce in auth resp header
05/20/15   tkl     Return GBA_SERVER_ERROR for BSF failure
...
typedef struct
{
  ...
  gba_session_type                  prov_session_type;
  gba_slot_type                     slot_id;
...
/*===========================================================================
FUNCTION:      GBA_HTTP_CONVERT_GBA_PROV_SESSION_TO_DS_HTTP_PROFILE_SUBS

DESCRIPTION:
  GBA http utility to convert GBA session type to DS profie config subs id
...
SIDE EFFECTS:
  None
===========================================================================*/
static ds_profile_subs_etype gba_http_convert_gba_prov_session_to_ds_http_profile_subs(
  gba_session_type   gba_prov_session
)
{
  switch (gba_prov_session)
  {
    case GBA_3GPP_PROV_SESSION_PRI:
      return DS_PROFILE_ACTIVE_SUBSCRIPTION_1;

    case GBA_3GPP_PROV_SESSION_SEC:
      return DS_PROFILE_ACTIVE_SUBSCRIPTION_2;

    default:
      break;
  }

  return DS_PROFILE_ACTIVE_SUBSCRIPTION_NONE;
} /* gba_http_convert_gba_prov_session_to_ds_http_profile_subs */
...
)
{
  ...
  do
  {
    ...
    if(profile_info.len > 0 &&
       strncasecmp((char *)profile_info.buf,
  ...
  UIM_MSG_MED_1("Creating new profile as no profile has matched the APN: %s",
                 gba_http_global_data_ptr->apn_info.apn);
  ...
  /* Fill profile subscription id to support flex mapping cases */
  profile_cfg.subs_id = gba_http_convert_gba_prov_session_to_ds_http_profile_subs(
                          gba_http_global_data_ptr->prov_session_type);

  if(profile_cfg.subs_id == DS_PROFILE_ACTIVE_SUBSCRIPTION_NONE)
  {
    return GBA_GENERIC_ERROR;
  }
...
)
{
  ...
  switch (event_type)
  {
    ...
    default:
      gba_http_cmd_ptr->data.block_rsp_data.event_type = GBA_HTTP_BLOCK_UNSUPPORTED;
      break;
  }
...
/*===========================================================================
   FUNCTION:      GBA_HTTP_CONVERT_GBA_PROV_SESSION_TO_DS_HTTP_SUB

   DESCRIPTION:
     GBA http utility to convert GBA session type to DS http subscription type
   ...
   SIDE EFFECTS:
     None
===========================================================================*/
static ds_http_subscription_e_type gba_http_convert_gba_prov_session_to_ds_http_sub(
  gba_session_type              gba_prov_session
)
{
  switch (gba_prov_session)
...
} /* gba_http_convert_gba_prov_session_to_ds_http_sub */
...
)
{
     ...
     DS_HTTP_READY != gba_http_session_id.session_status)
  {
    ...
    GBA_MSG_MED_2("pending session open : session id 0x%x; http_status 0x%x",
  ...
  if(DS_HTTP_ERROR == gba_http_session_id.session_id)
  {
    ...
    iface_info.subs_id = gba_http_convert_gba_prov_session_to_ds_http_sub(
                                  gba_http_global_data_ptr->prov_session_type);
    ...
    if (!gba_http_global_data_ptr->apn_info.apn_available)
    {
      (void)gba_nv_get_apn(&gba_http_global_data_ptr->apn_info,
                           gba_http_global_data_ptr->slot_id);
    }
    ...
    if (ds_http_is_nw_srv_avail_for_subs_id(DS_HTTP_IFACE_ANY, iface_info.subs_id))
    {
      ...
      if(ds_http_is_nw_srv_avail_for_subs_id(DS_HTTP_IFACE_3GPP, iface_info.subs_id))
      {
    ...
    /* if client didn't specifiy an APN for connection, then
       do NW service check on WLAN as well */
             ...
             ds_http_is_nw_srv_avail_for_subs_id(DS_HTTP_IFACE_WLAN_LB, iface_info.subs_id))
    {
      iface_info.iface_type = DS_HTTP_IFACE_WLAN_LB;
    }

    if (iface_info.iface_type != DS_HTTP_IFACE_MAX &&
        iface_info.subs_id != DS_HTTP_DEFAULT_SUBS)
    {
    ...
    GBA_MSG_MED_4("session id from ds_http open session 0x%x, "
                  "iface_type 0x%x, profile id 0x%x, ds subs id 0x%x",
                  gba_http_session_id.session_id,
...
)
{
  ...
  uint32                      binary_nonce_len   = 0;
  uint16                      offset             = 0;
  uint32                      nonce_base64_len   = 0;

  /* parse the nonce for RAND and AUTN
     RFC 3310 S3.2, 31.102 7.1.2 */
  nonce_base64_len = strnlen(nonce_ptr, GBA_HTTP_HEADER_VALUE_MAX_LEN + 1);

  if(nonce_base64_len > GBA_HTTP_HEADER_VALUE_MAX_LEN)
  {
    return GBA_GENERIC_ERROR;
  }
  binary_nonce_ptr = uim_common_util_alloc_base64string_to_bin(nonce_ptr,
                                                               nonce_base64_len,
                                                               &binary_nonce_len);
  if(binary_nonce_ptr == NULL || 
     (binary_nonce_len < (GBA_RAND_LEN + GBA_AUTN_LEN)))
  {
    return GBA_GENERIC_ERROR;
  }
...
)
{
  ...
  *support_tmpi_ptr = FALSE;
  ...
  while(str_ptr != NULL)
  {
    ...
    if(str_ptr != NULL &&
       ((!gba_nv_get_tmpi_user_agent(gba_http_global_data_ptr->slot_id) &&
         strncasecmp(str_ptr,
...
)
{
  ...
  uint8                        i                    = 0;
  gba_http_header_value_type   str_buf              = {0x00};
  gba_bsf_protocol_enum_type   bsf_protocol         = GBA_BSF_PROTOCOL_HTTP;
      ...
      i++)
  {
    ...
    switch(gba_http_request_header_table[i].header_id)
    {
       case GBA_HTTP_USER_AGENT_HEADER:
         ...
         if (gba_nv_get_tmpi_user_agent(gba_http_global_data_ptr->slot_id))
         {
  ...
  if(gba_status == GBA_SUCCESS)
  {
    ...
    if (gba_bootstrap_get_non_default_bsf_protocol(&bsf_protocol) != GBA_SUCCESS)
    {
      bsf_protocol = gba_nv_get_bsf_protocol(gba_http_global_data_ptr->slot_id);
    }
    if (GBA_BSF_PROTOCOL_HTTPS == bsf_protocol)
    {
    ...
    if(GBA_SUCCESS == gba_nv_get_imei_http_header(imei_header_ptr, gba_http_global_data_ptr->slot_id) &&
...
)
{
  ...
  if(task_cmd_ptr != NULL)
  {
    GBA_MSG_MED_0("GBA_HTTP_CMD_SIG received");
...
)
{
  ...
  GBA_MSG_MED_0("gba_http_cleanup");
  return GBA_SUCCESS;
} /* gba_http_cleanup */
...
)
{
  ...
  gba_http_global_data_ptr->prov_session_type = prov_session_type;
  gba_http_global_data_ptr->slot_id           = slot_id;
...
)
{
  ...
  GBA_MSG_MED_2("session_id 0x%x; request id 0x%x",
                gba_http_session_id.session_id, gba_http_global_data_ptr->request_id);
...
)
{
  ...
  base64_auts_ptr = uim_common_util_alloc_bin_to_base64string((const uint8*)auts_ptr->data, auts_ptr->data_len);
