...
$Header: //components/rel/uim.mpss/6.1.0/lpa/src/lpa.c#4 $ $DateTime: 2020/02/06 03:23:18 $

when       who     what, where, why
--------   ---     -----------------------------------------------------------
02/06/20   vdc     Do NOT register for QMI_Modem task if it is already ready
03/08/19   bcho    Map v2.2 cards to v2.1 functionality
...
05/16/16   bcho    Deregister qmi uim http service on task stop
...
/* Global holding the SVN from the eUICC */
uint8                        lpa_svn[LPA_NUM_SLOTS][LPA_SVN_LEN]   = {{LPA_SVN_BYTE1_DEFAULT, LPA_SVN_BYTE2_DEFAULT, 0x00},
                                                                      {LPA_SVN_BYTE1_DEFAULT, LPA_SVN_BYTE2_DEFAULT, 0x00}};
...
)
{
  ...
  lpa_client_data[slot_id].cmd = cmd;
...
)
{
  ...
  /* Register with RCINIT for QMI_MODEM task ready only if it is not ready.
     If QMI_MODEM task is already ready then initialize QMI UIM HTTP service */
  if(rcevt_getcount_name("QMI_MODEM: READY") == 0)
  {
  ...
  for (;;)
  {
    ...
    /*-----------------------------------------------------------------------------
                          Process Signal - LPA_NOTIFICATION_CMD_SIG
    -----------------------------------------------------------------------------*/
    else if (sigs_rcvd & LPA_NOTIFICATION_CMD_SIG)
    {
      lpa_notification_handle_cmd_sig();
    } /* end if LPA_NOTIFICATION_CMD_SIG */
...
)
{
  ...
  LPA_MSG_HIGH_3("euicc_phase; slot_id=0x%x, lpa_svn[0]=0x%x, lpa_svn[1]=0x%x", slot_id, lpa_svn[slot_id][0], lpa_svn[slot_id][1]);

  switch(lpa_svn[slot_id][0])
  {
    case 0x01:
      if(lpa_svn[slot_id][1] == 0x00)
      {
        *phase_ptr = LPA_PHASE1_EUICC;
      }
      else if(lpa_svn[slot_id][1] == 0x01)
      {
        *phase_ptr = LPA_PHASE1_1_EUICC;
      }
      else
      {
    ...
    case 0x02:
      if(lpa_svn[slot_id][1] == 0x00)
      {
        *phase_ptr = LPA_PHASE2_EUICC;
      }
      else if(lpa_svn[slot_id][1] == 0x01 || lpa_svn[slot_id][1] == 0x02)
      {
        *phase_ptr = LPA_PHASE2_1_EUICC;
      }
      else
      {
        lpa_status = LPA_GENERIC_ERROR;
...
)
{
  ...
  memscpy(lpa_svn[slot_id],
          sizeof(lpa_svn[slot_id]),
          svn_ptr,
          svn_len);
...
)
{
  if(slot_id >= LPA_NUM_SLOTS ||
     svn_ptr == NULL ||
     svn_ptr_len != sizeof(lpa_svn[slot_id]))
  {
    return LPA_GENERIC_ERROR;
  }
  ...
  LPA_MSG_HIGH_2("lpa_svn[slot_id][0]=0x%x, lpa_svn[slot_id][1]=0x%x", lpa_svn[slot_id][0], lpa_svn[slot_id][1]);
  return LPA_SUCCESS;
} /* lpa_get_svn */
...
)
{
  ...
  *cmd_ptr = lpa_client_data[slot_id].cmd;
