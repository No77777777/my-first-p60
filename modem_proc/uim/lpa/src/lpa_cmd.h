...
$Header: //components/rel/uim.mpss/6.1.0/lpa/src/lpa_cmd.h#3 $ $DateTime: 2019/12/27 02:07:35 $
...
/* ----------------------------------------------------------------------------
   STRUCTURE:      LPA_CARD_MEMORY_RESET_REQ_TYPE

   DESCRIPTION:
     LPA eUICC memory reset request type

-----------------------------------------------------------------------------*/
typedef struct {
  lpa_card_memory_reset_option_type             option;
} lpa_card_memory_reset_req_type;

/* ----------------------------------------------------------------------------
   STRUCTURE:      LPA_SET_DEFAULT_SERVER_ADDRESS_REQ_TYPE

   DESCRIPTION:
     LPA configure server address request type

-----------------------------------------------------------------------------*/
typedef struct {
  lpa_data_type                                default_smdp_address;
} lpa_set_default_server_address_req_type;
...
typedef struct {
  ...
  union {
    ...
    lpa_card_memory_reset_req_type              card_memory_reset_req;
    lpa_set_default_server_address_req_type     set_default_server_address_req;
  } cmd;
} lpa_cmd_data_type;
