...
$Header: //components/rel/uim.mpss/6.1.0/lpa/src/lpa_util.c#2 $ $DateTime: 2019/10/30 06:56:05 $
...
#define LPA_UTIL_ADD_PROFILE_USING_DEFAULT_SMDP        "UseSMDP"
#define LPA_UTIL_ADD_PROFILE_USING_SMDS                "UseSMDS"
...
)
{
  ...
  switch(lpa_response_ptr->msg_type)
  {
    ...
    case LPA_GET_RAT_MSG:
      LPA_FREE(lpa_response_ptr->message.get_rat_resp.rules_auth_table.ppar_entry_ptr);
      break;
    default:
       break;
  }
...
)
{
  ...
  {
    LPA_MSG_ERR_0("lpa_util_parse_activation_code: generic error");
    return LPA_GENERIC_ERROR;
  }

  /* Find out which method out of the following should be used by LPA to
     perform the AddProfile:
     1. Using activation code string
     2. Using default SMDP+ on the eUICC
     3. Using SMDS method */
  if(raw_activation_code->data_len >= strlen(LPA_UTIL_ADD_PROFILE_USING_DEFAULT_SMDP) &&
     !memcmp(raw_activation_code->data_ptr,
             LPA_UTIL_ADD_PROFILE_USING_DEFAULT_SMDP,
             (uint8) strlen(LPA_UTIL_ADD_PROFILE_USING_DEFAULT_SMDP)))
  {
    *method_ptr = LPA_ADD_PROFILE_METHOD_DEFAULT_SMDP;
    return LPA_SUCCESS;
  }

  if(raw_activation_code->data_len >= strlen(LPA_UTIL_ADD_PROFILE_USING_SMDS) &&
     !memcmp(raw_activation_code->data_ptr,
             LPA_UTIL_ADD_PROFILE_USING_SMDS,
             (uint8) strlen(LPA_UTIL_ADD_PROFILE_USING_SMDS)))
  {
    *method_ptr = LPA_ADD_PROFILE_METHOD_SMDS;
    return LPA_SUCCESS;
  }

  *method_ptr = LPA_ADD_PROFILE_METHOD_ACTIVATION_CODE;
  ...
  while (offset < raw_activation_code->data_len &&
         raw_activation_code->data_ptr[offset] !=
           LPA_ACTIVATION_CODE_DELIMITER_VALUE &&
         smdp_offset < LPA_SERVER_ADDRESS_LEN_MAX)
  {
...
)
{
  ...
  (void) lpa_retrieve_euicc_phase(slot_id, &phase);

  if(phase == LPA_PHASE1_EUICC ||
     phase == LPA_PHASE1_1_EUICC)
  {
    device_info->device_capability.gsm_supported_release.version_len           =
      LPA_UTIL_RELEASE_VERSION_LEN_PHASE1;
    device_info->device_capability.utran_supported_release.version_len         =
      LPA_UTIL_RELEASE_VERSION_LEN_PHASE1;
    device_info->device_capability.cdma2000onex_supported_release.version_len  =
      LPA_UTIL_RELEASE_VERSION_LEN_PHASE1;
    device_info->device_capability.cdma2000hrpd_supported_release.version_len  =
      LPA_UTIL_RELEASE_VERSION_LEN_PHASE1;
    device_info->device_capability.cdma2000ehrpd_supported_release.version_len =
      LPA_UTIL_RELEASE_VERSION_LEN_PHASE1;
    device_info->device_capability.eutran_supported_release.version_len        =
      LPA_UTIL_RELEASE_VERSION_LEN_PHASE1;
    device_info->device_capability.contactless_supported_release.version_len   =
      LPA_UTIL_RELEASE_VERSION_LEN_PHASE1;
  }
  else
  {
    device_info->device_capability.gsm_supported_release.version_len           =
      LPA_UTIL_RELEASE_VERSION_LEN;
    device_info->device_capability.utran_supported_release.version_len         =
      LPA_UTIL_RELEASE_VERSION_LEN;
    device_info->device_capability.cdma2000onex_supported_release.version_len  =
      LPA_UTIL_RELEASE_VERSION_LEN;
  ...
  device_info->device_capability.gsm_supported_release.version[0]           =
    LPA_UTIL_GSM_SUPPORTED_RELEASE;                         /* Release 11 */
  device_info->device_capability.utran_supported_release.version[0]         =
    LPA_UTIL_UTRAN_SUPPORTED_RELEASE;                       /* Release 11 */
  device_info->device_capability.cdma2000onex_supported_release.version[0]  =
    LPA_UTIL_CDMA2000_ONEX_SUPPORTED_RELEASE;               /* Per 1x team  */
  device_info->device_capability.cdma2000hrpd_supported_release.version[0]  =
    LPA_UTIL_CDMA2000_HRPD_SUPPORTED_RELEASE;               /* Per 1x team */
  device_info->device_capability.cdma2000ehrpd_supported_release.version[0] =
    LPA_UTIL_CDMA2000_EHRPD_SUPPORTED_RELEASE;              /* Per 1x team */
  device_info->device_capability.eutran_supported_release.version[0]        =
    LPA_UTIL_EUTRAN_SUPPORTED_RELEASE;                      /* Release 11 */
  device_info->device_capability.contactless_supported_release.version[0]   =
