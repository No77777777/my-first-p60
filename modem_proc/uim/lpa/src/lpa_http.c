...
Copyright (c) 2016, 2018 - 2019 QUALCOMM Technologies, Inc (QTI) and its licensors.
...
$Header: //components/rel/uim.mpss/6.1.0/lpa/src/lpa_http.c#3 $ $DateTime: 2019/12/27 02:07:35 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
09/23/19   kb      Fixed String Conversion Issues 
04/22/19   vgd     Fill iface_type with 3GPP if 3GPP n/w service is available
...
06/14/16   ll      Setting cmd_type to QMI_HTTP or DS_HTTP respectively
06/14/16   ll      Check for session_id only for DS_HTTP
06/13/16   ll      Fix qmi_uim_http_transaction_ptr memset after initialized
05/18/16   av      Route HTTP request to QMI HTTP
...
#define LPA_HTTP_INIT_AUTH_PATH_PHASE_1                 "/gsma/rsp1/es9plus/initiateAuthentication"
#define LPA_HTTP_GET_BPP_PATH_PHASE_1                   "/gsma/rsp1/es9plus/getBoundProfilePackage"
#define LPA_HTTP_HANDLE_INSTALLATION_RESULT_PATH        "/gsma/rsp1/es9plus/handleProfileInstallationResult"
#define LPA_HTTP_INIT_AUTH_PATH                         "/gsma/rsp2/es9plus/initiateAuthentication"
#define LPA_HTTP_GET_BPP_PATH                           "/gsma/rsp2/es9plus/getBoundProfilePackage"
#define LPA_HTTP_CANCEL_SESSION_PATH                    "/gsma/rsp2/es9plus/cancelSession"
#define LPA_HTTP_AUTHENTICATE_CLIENT_PATH               "/gsma/rsp2/es9plus/authenticateClient"
#define LPA_HTTP_HANDLE_NOTIFICATION_PATH               "/gsma/rsp2/es9plus/handleNotification"

#define LPA_HTTP_HEADER_ADMIN_PROTOCOL_NAME             "X-Admin-Protocol"
#define LPA_HTTP_HEADER_ADMIN_PROTOCOL_VALUE_PHASE_1    "gsma/rsp1"
#define LPA_HTTP_HEADER_ADMIN_PROTOCOL_VALUE_PREFIX     "gsma/rsp/v"
#define LPA_HTTP_HEADER_USER_AGENT_NAME                 "User-Agent"
#define LPA_HTTP_HEADER_USER_AGENT_VALUE                "gsma-rsp-lpad"
...
typedef struct
{
  ...
  lpa_http_cmd_enum_type                orig_lpd_cmd;
...
)
{
  ...
  do
  {
    lpa_http_rsp_ptr->rsp_type                            = LPA_HTTP_QMI_HTTP_RSP;
...
)
{
  ...
  do
  {
    lpa_http_rsp_ptr->rsp_type                   = LPA_HTTP_RSP;
...
)
{
  ...
  if(lpa_http_global_data[slot_id].retry_count == 0)
  {
    /* This is the first time the command is being requested to be retried...
       lets set the timer */
    if(lpa_http_global_data[slot_id].orig_lpd_cmd == LPA_HTTP_GET_BPP_CMD)
    {
      lpa_timer_set(slot_id, LPA_TIMER_TYPE_GET_BPP_RETRY_TIMER);
    }
    else if(lpa_http_global_data[slot_id].orig_lpd_cmd == LPA_HTTP_HANDLE_INSTALLATION_RESULT_CMD)
    {
...
{
  lpa_http_resp_data_type              http_resp;
  uint32                 timer_sig = LPA_HTTP_RETRY_TIMER_EXPIRE_SIG_SLOT_1;

  memset(&http_resp, 0x00, sizeof(http_resp));
...
/*===========================================================================
FUNCTION LPA_HTTP_GET_ADMIN_PROTOCOL_VALUE

DESCRIPTION
  This function returns the value of X-Admin_Protocol HTTPS header
...
SIDE EFFECTS
  None
===========================================================================*/
static lpa_result_enum_type lpa_http_get_admin_protocol_value(
  lpa_slot_id_enum_type                  slot_id,
  char                                   **header_value_ptr
)
{
  uint8                                   svn[LPA_SVN_LEN]           = {1, 1, 0};
  uint8                                   j                          = 0;
  uint8                                   k                          = 0;
  uint8                                   svn_ascii[LPA_SVN_LEN + 2] = {0x31, 0x2E, 0x31, 0x2E, 0x30}; /* 1.1.0 */
  lpa_euicc_phase_enum_type               euicc_phase                = LPA_PHASE1_1_EUICC;
  uint8                                   header_value_ptr_len       = 0;
            
  if(slot_id >= LPA_NUM_SLOTS ||
     header_value_ptr == NULL) 
  {
    return LPA_GENERIC_ERROR;
  }
  ...
  {
    header_value_ptr_len = (uint8) (strlen(LPA_HTTP_HEADER_ADMIN_PROTOCOL_VALUE_PHASE_1) + 1);
    *header_value_ptr = (char*) LPA_MALLOC(header_value_ptr_len);
    if(*header_value_ptr == NULL) 
    {
      return LPA_ERROR_HEAP_EXHAUSTED;
    }
    memscpy(*header_value_ptr,
            header_value_ptr_len,
            LPA_HTTP_HEADER_ADMIN_PROTOCOL_VALUE_PHASE_1,
            strlen(LPA_HTTP_HEADER_ADMIN_PROTOCOL_VALUE_PHASE_1));

    return LPA_SUCCESS;
  }

  if(LPA_SUCCESS != lpa_get_svn(slot_id, LPA_SVN_LEN, &svn[0]))
  {
    return LPA_GENERIC_ERROR;
  }

  for(j = 0, k = 0; j < sizeof(svn) && k < sizeof(svn_ascii); j++, k++) 
  {
    svn_ascii[k++] = svn[j] + 0x30;
    if(k < sizeof(svn_ascii))
    {
      svn_ascii[k] = 0x2E; /* The dot '.' */
    }
  }

  header_value_ptr_len = (uint8) (strlen(LPA_HTTP_HEADER_ADMIN_PROTOCOL_VALUE_PREFIX) + sizeof(svn_ascii) + 1);
  *header_value_ptr = (char*) LPA_MALLOC(header_value_ptr_len);
  if(*header_value_ptr == NULL) 
  {
    return LPA_ERROR_HEAP_EXHAUSTED;
  ...
  memscpy(*header_value_ptr,
          header_value_ptr_len,
          LPA_HTTP_HEADER_ADMIN_PROTOCOL_VALUE_PREFIX,
          strlen(LPA_HTTP_HEADER_ADMIN_PROTOCOL_VALUE_PREFIX));
  memscpy((*header_value_ptr) + strlen(LPA_HTTP_HEADER_ADMIN_PROTOCOL_VALUE_PREFIX),
          header_value_ptr_len - strlen(LPA_HTTP_HEADER_ADMIN_PROTOCOL_VALUE_PREFIX),
          svn_ascii,
          sizeof(svn_ascii));

  return LPA_SUCCESS;
} /* lpa_http_get_admin_protocol_value */
...
SIDE EFFECTS
  None
===========================================================================*/
static lpa_result_enum_type lpa_http_qmi_http_post(
  lpa_slot_id_enum_type                  slot_id,
  qmi_uim_http_service_registry_id_type  qmi_http_service_id,
...
{
  char*                                   header_admin_protocol_value_ptr     = NULL;
  lpa_euicc_phase_enum_type               euicc_phase                         = LPA_PHASE1_1_EUICC;
  lpa_result_enum_type                    lpa_status                          = LPA_SUCCESS;
  ...
  if(LPA_SUCCESS != lpa_http_get_admin_protocol_value(slot_id,
                                                      &header_admin_protocol_value_ptr))
  {
    LPA_FREE(qmi_uim_http_transaction_ptr);
    return LPA_GENERIC_ERROR;
  }

  if(header_admin_protocol_value_ptr == NULL)
  {
    LPA_FREE(qmi_uim_http_transaction_ptr);
    return LPA_GENERIC_ERROR;
  }
     ...
     euicc_phase == LPA_PHASE1_1_EUICC) 
  {
    ...
    qmi_uim_http_transaction_ptr->header_ptr->headers[0].name_ptr = LPA_HTTP_HEADER_ADMIN_PROTOCOL_NAME;
    qmi_uim_http_transaction_ptr->header_ptr->headers[0].value_ptr = header_admin_protocol_value_ptr;
  }
  else
  {
    qmi_uim_http_transaction_ptr->header_ptr->num_headers = 2;

    qmi_uim_http_transaction_ptr->header_ptr->headers[0].name_ptr = LPA_HTTP_HEADER_USER_AGENT_NAME;
    qmi_uim_http_transaction_ptr->header_ptr->headers[0].value_ptr = LPA_HTTP_HEADER_USER_AGENT_VALUE;
    qmi_uim_http_transaction_ptr->header_ptr->headers[1].name_ptr = LPA_HTTP_HEADER_ADMIN_PROTOCOL_NAME;
    qmi_uim_http_transaction_ptr->header_ptr->headers[1].value_ptr = header_admin_protocol_value_ptr;
  }
  ...
  if(QMI_UIM_HTTP_SUCCESS != qmi_uim_http_send_transaction_request(
                               qmi_uim_http_transaction_ptr))
  {
    LPA_MSG_ERR_0("ES9plusHTTPSPost QMI HTTP - Failed to create HTTP Post request");
    lpa_status = LPA_ERROR_NETWORK;
  }

  LPA_FREE(header_admin_protocol_value_ptr);
...
)
{
  ...
  else
  {
    ...
    {
      /* Fill iface_type with DS_HTTP_IFACE_3GPP if 3GPP network service is
         available and in other cases(where ANY network service is available)
         fill it with DS_HTTP_IFACE_ANY. This is done as per DS request. */
      if(ds_http_is_nw_srv_avail(DS_HTTP_IFACE_3GPP))
      {
        iface_info.iface_type = DS_HTTP_IFACE_3GPP;
      }
      else
      {
...
)
{
  ...
  {
    https_prefix_len = (uint8) strlen(LPA_SMDP_ADDRESS_HTTPS_PREFIX);
  }

  temp_smdp_url_path_len = (uint16) strlen(smdp_url_path_ptr);
...
)
{
  ...
  (void) lpa_retrieve_euicc_phase(initiate_auth_req_ptr->slot_id, &euicc_phase);
  ...
  lpa_http_global_data[initiate_auth_req_ptr->slot_id].user_data_ptr =
    initiate_auth_req_ptr->user_data_ptr;
  ...
  lpa_status = lpa_http_build_smdp_url (
                 lpa_http_global_data[initiate_auth_req_ptr->slot_id].smdp_address,
                 LPA_HTTP_INIT_AUTH_PATH_PHASE_1,
                 &url_ptr);
  }
  else
  {
...
)
{
  ...
  lpa_http_global_data[get_bpp_req_ptr->slot_id].smdp_address =
    get_bpp_req_ptr->smdp_address;
  lpa_http_global_data[get_bpp_req_ptr->slot_id].retry_count = 0;
  lpa_http_global_data[get_bpp_req_ptr->slot_id].user_data_ptr =
    get_bpp_req_ptr->user_data_ptr;

  memset(&lpa_json_data, 0x00, sizeof(lpa_json_data));

  (void) lpa_retrieve_euicc_phase(get_bpp_req_ptr->slot_id, &euicc_phase);
  ...
  lpa_http_global_data[get_bpp_req_ptr->slot_id].orig_lpd_cmd = LPA_HTTP_GET_BPP_CMD;
  ...
  if(lpa_http_use_qmi_http())
  {
    if(lpa_http_global_data[get_bpp_req_ptr->slot_id].qmi_http_service_id ==
       QMI_UIM_HTTP_MAX_SERVICE_ID)
    {
      lpa_status = lpa_http_open_session(get_bpp_req_ptr->slot_id);
    }
    if(lpa_status != LPA_SUCCESS ||
...
)
{
  ...
  lpa_http_global_data[profile_installation_result_req_ptr->slot_id].callback = 
    profile_installation_result_req_ptr->lpd_callback;
  lpa_http_global_data[profile_installation_result_req_ptr->slot_id].smdp_address =
    profile_installation_result_req_ptr->smdp_address;
  lpa_http_global_data[profile_installation_result_req_ptr->slot_id].retry_count = 0;
  lpa_http_global_data[profile_installation_result_req_ptr->slot_id].user_data_ptr =
    profile_installation_result_req_ptr->user_data_ptr;

  memset(&lpa_json_data, 0x00, sizeof(lpa_json_data));

  lpa_status = lpa_http_build_smdp_url (
                 lpa_http_global_data[profile_installation_result_req_ptr->slot_id].smdp_address,
                 LPA_HTTP_HANDLE_INSTALLATION_RESULT_PATH,
                 &url_ptr);
  if(lpa_status != LPA_SUCCESS)
  {
    if(lpa_status != LPA_ERROR_HEAP_EXHAUSTED)
  ...
  lpa_http_global_data[profile_installation_result_req_ptr->slot_id].orig_lpd_cmd = LPA_HTTP_HANDLE_INSTALLATION_RESULT_CMD;
  ...
  if(lpa_http_use_qmi_http())
  {
    if(lpa_http_global_data[profile_installation_result_req_ptr->slot_id].qmi_http_service_id ==
       QMI_UIM_HTTP_MAX_SERVICE_ID)
    {
      lpa_status = lpa_http_open_session(profile_installation_result_req_ptr->slot_id);
    }
    if (lpa_status != LPA_SUCCESS ||
...
)
{
  ...
  if(rsp_data.error == DS_HTTP_READY &&
     lpa_http_global_data[slot_id].pending_post_data.data_ptr &&
     lpa_http_global_data[slot_id].orig_lpd_cmd == LPA_HTTP_INITIATE_AUTH_CMD)
  {
  ...
  if(rsp_data.error == DS_HTTP_READY &&
     lpa_http_global_data[slot_id].pending_post_data.data_ptr &&
     (lpa_http_global_data[slot_id].orig_lpd_cmd == LPA_HTTP_INITIATE_AUTH_CMD ||
      (lpa_http_global_data[slot_id].orig_lpd_cmd == LPA_HTTP_GET_BPP_CMD &&
       lpa_http_global_data[slot_id].retry_count <= LPA_HTTP_GET_BPP_MAX_RETRIES &&
       LPA_SUCCESS == lpa_timer_get(slot_id,
                                    LPA_TIMER_TYPE_GET_BPP_RETRY_TIMER,
                                    &remaining_time) &&
       remaining_time > 0) ||
      (lpa_http_global_data[slot_id].orig_lpd_cmd == LPA_HTTP_HANDLE_INSTALLATION_RESULT_CMD &&
  ...
  http_resp.http_status = rsp_data.http_status;
  ...
  if(lpa_http_global_data[slot_id].callback)
  {
    http_resp.data_ptr = lpa_http_global_data[slot_id].rsp_content.data_ptr;
    http_resp.data_len = lpa_http_global_data[slot_id].rsp_content.data_len;
    lpa_http_global_data[slot_id].callback(lpa_status,
                                           &http_resp,
                                           lpa_http_global_data[slot_id].user_data_ptr);
  }
...
)
{
  ...
  http_resp.data_ptr = rsp_data.content_ptr;
  http_resp.data_len = rsp_data.content_size;
...
DESCRIPTION
  This function is used to send Authenticate Client request to SMxx
...
)
{
  ...
  if(authenticate_client_req_ptr == NULL)
  {
    return LPA_GENERIC_ERROR;
  }

  lpa_http_global_data[authenticate_client_req_ptr->slot_id].callback = 
     authenticate_client_req_ptr->lpd_callback;
  lpa_http_global_data[authenticate_client_req_ptr->slot_id].smdp_address =
    authenticate_client_req_ptr->smdp_address;
  lpa_http_global_data[authenticate_client_req_ptr->slot_id].retry_count = 0;
  lpa_http_global_data[authenticate_client_req_ptr->slot_id].user_data_ptr =
     authenticate_client_req_ptr->user_data_ptr;

  memset(&lpa_json_data, 0x00, sizeof(lpa_json_data));

  lpa_status = lpa_http_build_smdp_url (
                 lpa_http_global_data[authenticate_client_req_ptr->slot_id].smdp_address,
                 LPA_HTTP_AUTHENTICATE_CLIENT_PATH,
                 &url_ptr);
  ...
  lpa_http_global_data[authenticate_client_req_ptr->slot_id].orig_lpd_cmd = LPA_HTTP_AUTHENTICATE_CLIENT_CMD;
  lpa_json_authenticate_client_encoder(&lpa_json_data, authenticate_client_req_ptr);

  /* Erite the encoded content for EFS file for testing purposes */
  (void) lpa_efs_write(LPA_FILE_JSON_ENCODED_AUTH_CLIENT,
                       lpa_json_data.data_len,
                       lpa_json_data.data_ptr,
                       authenticate_client_req_ptr->slot_id);

  if(lpa_http_use_qmi_http())
  {
    if(lpa_http_global_data[authenticate_client_req_ptr->slot_id].qmi_http_service_id ==
       QMI_UIM_HTTP_MAX_SERVICE_ID)
    {
      lpa_status = lpa_http_open_session(authenticate_client_req_ptr->slot_id);
    }
    if (lpa_status != LPA_SUCCESS ||
        lpa_http_global_data[authenticate_client_req_ptr->slot_id].qmi_http_service_id ==
        QMI_UIM_HTTP_SERVICE_ID_NOT_APPLICABLE      ||
        lpa_http_global_data[authenticate_client_req_ptr->slot_id].qmi_http_service_id >=
        QMI_UIM_HTTP_MAX_SERVICE_ID)
    {
  ...
  {
    /* Session must be open already as part of Init Auth procedure! */
    if(DS_HTTP_ERROR == lpa_http_global_data[authenticate_client_req_ptr->slot_id].session_id)
    {
      lpa_status = LPA_ERROR_NETWORK;
    }
    else
    {
      /* Copy the encoded data from above to the pending_post_data global that
         ...
         later if the network is back up before the retry timer expires, or it
         gets freed once the retry timer expires. */
      lpa_status = lpa_http_create_pending_post_data(authenticate_client_req_ptr->slot_id,
                                                     &lpa_json_data,
                                                    (char*)url_ptr);
      if(LPA_SUCCESS == lpa_status)
      {
        lpa_http_global_data[authenticate_client_req_ptr->slot_id].request_id = DS_HTTP_ERROR;
        lpa_status = lpa_http_post(
                       lpa_http_global_data[authenticate_client_req_ptr->slot_id].session_id,
                       (char*)url_ptr,
                       &lpa_json_data,
                       authenticate_client_req_ptr->slot_id,
                       &(lpa_http_global_data[authenticate_client_req_ptr->slot_id].request_id));
      }

      if(DS_HTTP_ERROR == lpa_http_global_data[authenticate_client_req_ptr->slot_id].request_id)
      {
        lpa_status = LPA_ERROR_NETWORK;
      }
      ...
      if(LPA_SUCCESS != lpa_status)
      {
        lpa_http_free_pending_post_data(authenticate_client_req_ptr->slot_id);
      }
    }
}
  ...
  return lpa_status;
} /* lpa_http_build_and_send_authenticate_client */
...
DESCRIPTION
  This function is used to send Cancel Session request to SMxx
...
lpa_result_enum_type lpa_http_build_and_send_cancel_session (
  lpa_http_cancel_session_server_req_type        *cancel_session_req_ptr
)
{
  lpa_data_type             lpa_json_data;
  char*                     url_ptr          = NULL;
  ...
  if(cancel_session_req_ptr == NULL)
  {
    return LPA_GENERIC_ERROR;
  }

  lpa_http_global_data[cancel_session_req_ptr->slot_id].callback = 
     cancel_session_req_ptr->lpd_callback;
  lpa_http_global_data[cancel_session_req_ptr->slot_id].smdp_address =
    cancel_session_req_ptr->smdp_address;
  lpa_http_global_data[cancel_session_req_ptr->slot_id].retry_count = 0;
  lpa_http_global_data[cancel_session_req_ptr->slot_id].user_data_ptr =
    cancel_session_req_ptr->user_data_ptr;

  memset(&lpa_json_data, 0x00, sizeof(lpa_json_data));

  lpa_status = lpa_http_build_smdp_url (
                 lpa_http_global_data[cancel_session_req_ptr->slot_id].smdp_address,
                 LPA_HTTP_CANCEL_SESSION_PATH,
                 &url_ptr);
  ...
  lpa_http_global_data[cancel_session_req_ptr->slot_id].orig_lpd_cmd = LPA_HTTP_CANCEL_SESSION_SERVER_CMD;
  lpa_json_cancel_session_encoder(&lpa_json_data, cancel_session_req_ptr);

  /* Erite the encoded content for EFS file for testing purposes */
  (void) lpa_efs_write(LPA_FILE_JSON_ENCODED_CANCEL_SESSION_SERVER,
                       lpa_json_data.data_len,
                       lpa_json_data.data_ptr,
                       cancel_session_req_ptr->slot_id);

  if(lpa_http_use_qmi_http())
  {
    if(lpa_http_global_data[cancel_session_req_ptr->slot_id].qmi_http_service_id ==
       QMI_UIM_HTTP_MAX_SERVICE_ID)
    {
      lpa_status = lpa_http_open_session(cancel_session_req_ptr->slot_id);
    }
    if (lpa_status != LPA_SUCCESS ||
        lpa_http_global_data[cancel_session_req_ptr->slot_id].qmi_http_service_id ==
        QMI_UIM_HTTP_SERVICE_ID_NOT_APPLICABLE      ||
        lpa_http_global_data[cancel_session_req_ptr->slot_id].qmi_http_service_id >=
        QMI_UIM_HTTP_MAX_SERVICE_ID)
    {
  ...
  {
    /* Session must be open already as part of Init Auth procedure! */
    if(DS_HTTP_ERROR == lpa_http_global_data[cancel_session_req_ptr->slot_id].session_id)
    {
      lpa_status = LPA_ERROR_NETWORK;
    }
    else
    {
      /* Copy the encoded data from above to the pending_post_data global that
         ...
         later if the network is back up before the retry timer expires, or it
         gets freed once the retry timer expires. */
      lpa_status = lpa_http_create_pending_post_data(cancel_session_req_ptr->slot_id,
                                                     &lpa_json_data,
                                                    (char*)url_ptr);
      if(LPA_SUCCESS == lpa_status)
      {
        lpa_http_global_data[cancel_session_req_ptr->slot_id].request_id = DS_HTTP_ERROR;
        lpa_status = lpa_http_post(
                       lpa_http_global_data[cancel_session_req_ptr->slot_id].session_id,
                       (char*)url_ptr,
                       &lpa_json_data,
                       cancel_session_req_ptr->slot_id,
                       &(lpa_http_global_data[cancel_session_req_ptr->slot_id].request_id));
      }

      if(DS_HTTP_ERROR == lpa_http_global_data[cancel_session_req_ptr->slot_id].request_id)
      {
        lpa_status = LPA_ERROR_NETWORK;
      }
      ...
      if(LPA_SUCCESS != lpa_status)
      {
        lpa_http_free_pending_post_data(cancel_session_req_ptr->slot_id);
      }
    }
}
  ...
  return lpa_status;
} /* lpa_http_build_and_send_cancel_session */
...
DESCRIPTION
  This function is used to send Handle Notification request to SMDP
...
)
{
  ...
  if(send_notification_req_ptr == NULL)
  {
    return LPA_GENERIC_ERROR;
  }

  lpa_http_global_data[send_notification_req_ptr->slot_id].callback = 
     send_notification_req_ptr->callback;
  lpa_http_global_data[send_notification_req_ptr->slot_id].smdp_address = 
     send_notification_req_ptr->notification.smdp_address;
  lpa_http_global_data[send_notification_req_ptr->slot_id].retry_count = 0;
  lpa_http_global_data[send_notification_req_ptr->slot_id].user_data_ptr =
    send_notification_req_ptr->user_data_ptr;

  memset(&lpa_json_data, 0x00, sizeof(lpa_json_data));

  lpa_status = lpa_http_build_smdp_url (
                 lpa_http_global_data[send_notification_req_ptr->slot_id].smdp_address,
                 LPA_HTTP_HANDLE_NOTIFICATION_PATH,
                 &url_ptr);
  ...
  lpa_http_global_data[send_notification_req_ptr->slot_id].orig_lpd_cmd = LPA_HTTP_HANDLE_NOTIFICATION_CMD;
  lpa_json_handle_notification_encoder(&lpa_json_data, send_notification_req_ptr);

  /* Erite the encoded content for EFS file for testing purposes */
  (void) lpa_efs_write(LPA_FILE_JSON_ENCODED_HANDLE_NOTIFICATION,
                       lpa_json_data.data_len,
                       lpa_json_data.data_ptr,
                       send_notification_req_ptr->slot_id);

  if(lpa_http_use_qmi_http())
  {
    if(lpa_http_global_data[send_notification_req_ptr->slot_id].qmi_http_service_id ==
       QMI_UIM_HTTP_MAX_SERVICE_ID)
    {
      lpa_status = lpa_http_open_session(send_notification_req_ptr->slot_id);
    }
    if (lpa_status != LPA_SUCCESS ||
        lpa_http_global_data[send_notification_req_ptr->slot_id].qmi_http_service_id ==
        QMI_UIM_HTTP_SERVICE_ID_NOT_APPLICABLE      ||
        lpa_http_global_data[send_notification_req_ptr->slot_id].qmi_http_service_id >=
        QMI_UIM_HTTP_MAX_SERVICE_ID)
    {
  ...
  {
    /* Session must be open already as part of Init Auth procedure! */
    if(DS_HTTP_ERROR == lpa_http_global_data[send_notification_req_ptr->slot_id].session_id)
    {
      lpa_status = LPA_ERROR_NETWORK;
    }
    else
    {
      lpa_http_global_data[send_notification_req_ptr->slot_id].request_id = DS_HTTP_ERROR;
      lpa_status = lpa_http_post(
                     lpa_http_global_data[send_notification_req_ptr->slot_id].session_id,
                     (char*)url_ptr,
                     &lpa_json_data,
                     send_notification_req_ptr->slot_id,
                     &(lpa_http_global_data[send_notification_req_ptr->slot_id].request_id));

      if(DS_HTTP_ERROR == lpa_http_global_data[send_notification_req_ptr->slot_id].request_id)
      {
        lpa_status = LPA_ERROR_NETWORK;
      }
    }
  ...
  return lpa_status;
} /* lpa_http_build_and_send_notification */
