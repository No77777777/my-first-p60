...
$Header: //components/rel/uim.mpss/6.1.0/pbm/src/pbmgsdi.c#2 $


when       who     what, where, why
--------   ---     ----------------------------------------------------------
07/27/19   tq      Notify fdn_status to clients
07/10/18   nr      Remove stale MCC value in case of card is absent in slot
01/19/18   tq      Clear network ecc cache when no card is present in any slot
04/19/17   bcho    Moving QCRIL ECC DB to PBM
12/02/16   nr      Adding NULL check before accessing adn_cache_ptr in HIDDENKEY_EVT
11/21/16   bcho    Read NV hardcoded ECC config on NV Refresh
10/24/16   nr      Send cache update event to clear phonebook global in QMI-PBM
...
02/26/16    gm     Handling pbm restart in app_init
...
01/28/16    na     Initializing sim_text with 0xFF
...
11/23/15    gs     Send session change information to upper layers
...
08/31/15    sp     Add validation for additional number identifier while writing ANR
08/26/15    lm     Free memory allocated for file path ptr in refresh event
...
06/23/15    nr     Validate BCD number and EXT data length
06/16/15    nr     Freeing mmgsdi_resp_ptr
...
02/13/15    nr     Clearing slot id in session de-activation handling properly
03/03/15    lm     Remove double semicolon at end of the line
02/25/15    ar     Remove mmgsdi_session_seek usage as it is deprecated
02/13/15    nr     Clearing slot id after getting session info in session de-activation handling
01/28/15    nr     Clearing slot id in session de-activation handling
...
)
{
  ...
  {
    UIM_MSG_ERR_1("Unexpected cmd_type 0x%x", mmgsdi_resp_ptr->cmd_type);
    pbm_cmd_free(mmgsdi_resp_ptr);
    (void) rex_set_sigs(PBM_TCB, PBM_CMD_COMPLETE_SIG);
    return;
  }
...
SIDE EFFECTS
===========================================================================*/
void pbm_mmgsdi_fdn_ext_read_cb(
  mmgsdi_return_enum_type status,
  mmgsdi_cnf_enum_type    data_type,
  const mmgsdi_cnf_type  *data_ptr
)
...
)
{
  ...
  pbm_session_cache_free(deact_session_type);
  ...
  if(deact_session_complement == PBM_SESSION_MAX ||
     !(pbm_if_queue_has_sess(deact_session_complement,q_id) ||
      ...
      (pbm_init_info.pbm_session_init_done[deact_session_complement])))
  {
...
/*===========================================================================
FUNCTION PBM_HANDLE_SUB_READY_EVT

DESCRIPTION
   function to handle MMGSDI SUBSCRIPTION READY EVT
...
DEPENDENCIES
  None
===========================================================================*/
static void pbm_handle_sub_ready_evt(
  mmgsdi_event_data_type *mmgsdi_event_ptr
)
{
  PB_CACHE                cache;
  ...
  mmgsdi_len_type         length            = 0;
  mmgsdi_access_type      file_access;
  pbm_session_enum_type   session_type = PBM_SESSION_DEFAULT;

  PBM_CHECK_PTR_RET(mmgsdi_event_ptr, VOID);

  session_type = pbm_mmgsdi_session_to_pbm_session(mmgsdi_event_ptr->session_id);

  if(MMGSDI_SUBSCRIPTION_READY_EVT != mmgsdi_event_ptr->evt)
  {
    return;
  }

  mmgsdi_session_id = mmgsdi_event_ptr->session_id;
  app_info          = mmgsdi_event_ptr->data.subscription_ready.app_info;
  ...
  /* Resetting the session_init_done for the provisioning and card session
     to avoid sending PBM_EVENT_SESSION_INIT_DONE twice */
  ...
  /* for slot id, app type and in_use_adn_pb */
  if(pbm_session_type_to_pb_category(session_type) != PBM_GPB )
  {
    pbm_session_info[session_type].app_type = app_info.app_data.app_type;
  }
  pbm_session_info[session_type].slot_id = (uint16) mmgsdi_event_ptr->data.subscription_ready.app_info.slot;
  ...
  if(mmgsdi_event_ptr->data.subscription_ready.app_info.slot == MMGSDI_SLOT_1)
  {
    pbm_session_info[PBM_SESSION_GPB_1].slot_id = (uint16) PBM_SLOT_1;

    /* assigning the value directly as constant boolean warnign comes for
       this */
  ...
  else if(mmgsdi_event_ptr->data.subscription_ready.app_info.slot == MMGSDI_SLOT_2)
  {
  ...
  else if(mmgsdi_event_ptr->data.subscription_ready.app_info.slot == MMGSDI_SLOT_3)
  {
  ...
  {
    UIM_MSG_ERR_1("invalid slot from MMGSDI 0x%x", mmgsdi_event_ptr->data.subscription_ready.app_info.slot);
  }
  rex_leave_crit_sect(&pbm_crit_sect);
  ...
  */
  ...
  switch(mmgsdi_event_ptr->data.subscription_ready.app_info.app_data.app_type)
  {
    case MMGSDI_APP_RUIM:
       file_access.file.file_enum = MMGSDI_CDMA_IMSI_M;
       length = MMGSDI_CDMA_IMSI_LEN;
       break;

    case MMGSDI_APP_SIM:
       file_access.file.file_enum = MMGSDI_GSM_IMSI;
       length = MMGSDI_IMSI_LEN;
       break;

    case MMGSDI_APP_CSIM:
       file_access.file.file_enum = MMGSDI_CSIM_IMSI_M;
       length = MMGSDI_CDMA_IMSI_LEN;
       break;
  ...
  (void)mmgsdi_session_read_transparent(
          mmgsdi_session_id,
          file_access,
          0,
          length,
          pbm_ecc_mmgsdi_read_cb,
          (mmgsdi_client_data_type)file_access.file.file_enum);
}/* pbm_handle_sub_ready_evt */
...
)
{
  ...
  boolean                         new_init     = FALSE;
   ...
   if(MMGSDI_CARD_INSERTED_EVT == mmgsdi_event_ptr->evt ||
      MMGSDI_SESSION_CHANGED_EVT == mmgsdi_event_ptr->evt ||
      MMGSDI_SUBSCRIPTION_READY_EVT == mmgsdi_event_ptr->evt)
  {
    new_init = TRUE;
  }

  /* except for MMGSDI_REFRESH_NAA_FCN all refresh modes will have
     MMGSDI_SUBSCRIPTION_READY_EVT followed */
   if(MMGSDI_REFRESH_EVT == mmgsdi_event_ptr->evt &&
        (mmgsdi_event_ptr->data.refresh.mode != MMGSDI_REFRESH_NAA_FCN &&
         mmgsdi_event_ptr->data.refresh.stage == MMGSDI_REFRESH_STAGE_START))
  {
    new_init = TRUE;
  }
  ...
  if(new_init)
  {
   ...
   switch (mmgsdi_event_ptr->evt)
   {
    case MMGSDI_CARD_INSERTED_EVT:
    {
      ...
      uint16                    slot1_mcc        = PBM_INVALID_MCC;
      uint16                    slot2_mcc        = PBM_INVALID_MCC;
      uint8                     loop_index       = 0;
      pbm_slot_type             other_slot       = PBM_SLOT_NONE;

      slot = PBM_MMGSDI_SLOT_TO_PBM_SLOT(mmgsdi_event_ptr->data.card_inserted.slot);
      ...
      (void)pbm_get_mcc_from_iccid(mmgsdi_event_ptr->data.card_inserted.iccid,
                                   mmgsdi_event_ptr->data.card_inserted.iccid_len,
                                   &mcc);

      pbm_ecc_update_mcc(PBM_MMGSDI_SLOT_TO_PBM_SLOT(mmgsdi_event_ptr->data.card_inserted.slot),
                         PBM_ECC_MCC_SOURCE_ICCID,
                         mcc);

      slot1_mcc = pbm_ecc_get_current_mcc(PBM_SLOT_1);
      slot2_mcc = pbm_ecc_get_current_mcc(PBM_SLOT_2);

      other_slot = (slot == PBM_SLOT_1) ? PBM_SLOT_2 : PBM_SLOT_1;

      /* compare MCC on both the slots and delete mcc on card removed slot
         if doesn't match */
      if(!pbm_card_status[other_slot - 1] &&
         slot1_mcc != slot2_mcc)
      {
        for(loop_index = 0; loop_index < PBM_ECC_MCC_SOURCE_MAX; loop_index++)
        {
          pbm_ecc_delete_mcc(other_slot, loop_index);
        }
      }
      ...
      for (index = 0; index < (int) MAX_UIM_APPS; index++ )
      {
            ...
            !(pbm_session_lpm_status & PBM_SESSIONID_TO_SESSION_MASK(index)))
        {
          ...
          /* need to set hardcoded eccs as if card is present, and LPM mode is
             not ON */
      ...
      for (index = 0; index < (int) MAX_UIM_APPS; index++ )
      {
        if((slot == (pbm_slot_type)pbm_session_info[index].slot_id ) &&
            !(pbm_session_lpm_status & PBM_SESSIONID_TO_SESSION_MASK(index)))
        {
          /* cache ready for hardcoded #s */
          PBM_SET_PB_CACHE_READY(&ecc_other_cache);
          break;
        }
      }

      /* Initialize card_session related cahces */
      pbm_session_cache_init(card_sess);
    }
    ...
    case MMGSDI_ECC_EVT:
    {
      ...
      UIM_MSG_HIGH_3("PBM received MMGSDI_ECC_EVT slot 0x%x for session 0x%x, num ecc 0x%x",
                      mmgsdi_event_ptr->data.ecc.slot,
                      session_type,
                      mmgsdi_event_ptr->data.ecc.ecc_count);
      ...
      /* Make sure we free the memory which was allocated in deep copy in the
         below function.*/
      pbm_update_sim_ecc_cache(session_type,&mmgsdi_event_ptr->data.ecc);
    }
    break;
    case MMGSDI_PIN1_EVT:
    {
      ...
      UIM_MSG_HIGH_3("PBM received MMGSDI_PIN1_EVT with status  0x%x for session 0x%x, for pin 0x%x",
                     mmgsdi_event_ptr->data.pin.pin_info.status,
                     session_type,
                     mmgsdi_event_ptr->data.pin.pin_info.pin_id);
      if( pbm_session_type_to_pb_category(session_type) != PBM_GPB  )
      {
        UIM_MSG_HIGH_2("PIN app_type 0x%x for slot 0x%x",
                       mmgsdi_event_ptr->data.pin.aid_type[0].app_type,
                       mmgsdi_event_ptr->data.pin.slot);
                       pbm_session_info[session_type].app_type = mmgsdi_event_ptr->data.pin.aid_type[0].app_type;
                       pbm_session_info[session_type].slot_id = mmgsdi_event_ptr->data.pin.slot;
      }


      PBM_VALIDATE_SESSION_RET(session_type, VOID);

      /* since PIN events would come only for PROV sessions, we need to take
         the appropriate action on the card session that belonged to this prov
         session */
      switch( mmgsdi_event_ptr->data.pin.slot )
      {
        case MMGSDI_SLOT_1 :
      ...
      for(l_i = (uint8)PBM_SESSION_GPB_1;l_i < (uint8)PBM_SESSION_MAX; l_i++)
      {
        ...
        {
          if(pbm_session_info[l_i].isActive &&
             pbm_session_info[l_i].slot_id == (uint16) PBM_MMGSDI_SLOT_TO_PBM_SLOT(mmgsdi_event_ptr->data.pin.slot))
          {
            found_another = TRUE;
          }
        }
            ...
            switch ( mmgsdi_event_ptr->data.pin.pin_info.status )
      {
        ...
        /* there can be 2 possibilities for event MMGSDI_PIN_ENABLED_VERIFIED
           ...
           2 - SIM doesnt need PIN verification and now PIN is enabled to
           prompt for verification in the next powerup for case 2 we dont need
           to take any action since not applicable for PBM */
        case MMGSDI_PIN_ENABLED_VERIFIED :
        {
          /* here we should just mark the caches as not ready. However
             subscription ready follows this and PBM init would be triggred
             from that */
          ...
          /* in case of enabled verified no need to check other  app's status
             for pin1_verified on same slot */
        ...
        {
          /* isActive flag tells us whether we have recieved this event during
             powerup (OR), this PIN is blocked after
          ...
          {
            /* in this case the session is already initialized, dont flush the
               caches , instead just mark the status as
               PBM_STATUS_READY_PUC_REQ when we recieve MMGSDI_PIN_UNBLOCKED
               event we can just mark them back to PBM_STATUS_READY */
            pbm_set_session_caches_status(session_type, PBM_STATUS_READY_PUC_REQ);
          }
          else
          {
            /* session is in initialization process. since anyway it is not
               completed we will flush all the caches and restart the
               initialization after we get MMGSDI_PIN_UNBLOCKED event */
            if(TRUE == pbm_session_info[session_type].isActive)
            {
              pbm_stop_fileinfo_internal(session_type);
            }
        ...
        case MMGSDI_PIN_UNBLOCKED :
        {
          if((TRUE == pbm_init_info.pbm_session_init_done[session_type]) &&
                     (TRUE == pbm_session_info[session_type].isActive))
          {
            /* in this case the session is already initialized, we can just
               mark them back to STATUS_READY */
            pbm_set_session_caches_status(session_type, PBM_STATUS_READY);
          }
          else if(TRUE == pbm_session_info[session_type].isActive)
          {
            /* session is in initialization process restart initialization */
            pbm_session_info[session_type].isActive = FALSE;
          ...
          if(!found_another)
          {
            ...
            else if(TRUE == pbm_session_info[card_session].isActive)
            {
              ...
              {
                UIM_MSG_HIGH_1("Already session init 0x%x",card_session);
          ...
          /* in case of pin unblocked no need to check other  app's status for
             pin1_verified on same slot */
          PBM_SESSION_ATOMIC_SET(pbm_pin1_verified, card_session);
        }
        break;
    ...
    case MMGSDI_PIN2_EVT:
    {
      ...
      uint8 l_i;
      pbm_session_enum_type card_session = PBM_SESSION_GPB_1;
      UIM_MSG_HIGH_3("PBM received MMGSDI_PIN2_EVT with status  0x%x for session 0x%x, for pin 0x%x",
                      mmgsdi_event_ptr->data.pin.pin_info.status,
                      session_type,
                      mmgsdi_event_ptr->data.pin.pin_info.pin_id);
      PBM_VALIDATE_SESSION_RET(session_type, VOID);
      ...
      /* since PIN events would come only for PROV sessions, we need to take
         the appropriate action on the card session that belonged to this prov
         session */
      if(mmgsdi_event_ptr->data.pin.slot == MMGSDI_SLOT_1)
      {
        card_session = PBM_SESSION_GPB_1;
      }
            else if(mmgsdi_event_ptr->data.pin.slot == MMGSDI_SLOT_2)
      {
        card_session = PBM_SESSION_GPB_2;
      }
#ifdef FEATURE_TRIPLE_SIM
            else if(mmgsdi_event_ptr->data.pin.slot == MMGSDI_SLOT_3)
      {
        card_session = PBM_SESSION_GPB_3;
      }
#endif
      for(l_i = (uint8)PBM_SESSION_GPB_1;l_i < (uint8)PBM_SESSION_MAX; l_i++)
      {
        ...
        if(pbm_session_type_to_pb_category((pbm_session_enum_type)l_i) == PBM_LPB && l_i != (uint8)session_type)
        {
          ...
          break;
          /* break to have the valid value for l_i */
        }
      }
      switch(mmgsdi_event_ptr->data.pin.pin_info.status)
      {
        ...
        case MMGSDI_PIN_DISABLED:
        {
          ...
          /* in case of enabled verified no need to check other  app's status
             for pin2_verified on same slot */
    ...
    case MMGSDI_CARD_REMOVED_EVT:
    {
      ...
      slot = PBM_MMGSDI_SLOT_TO_PBM_SLOT(mmgsdi_event_ptr->data.card_error.slot);
      PBM_VALIDATE_SLOT_RET(slot, VOID);

      pbm_card_status[slot - 1] = FALSE;
      UIM_MSG_HIGH_4("PBM received event: 0x%x for slot 0x%x, pbm_card_status 0x%x, ecc_sim_airplane nv 0x%x",
                      mmgsdi_event_ptr->evt, slot, pbm_card_status[slot - 1], ecc_sim_airplane);
      ...
      for (index = 0; index < (int) MAX_UIM_APPS; index++ )
      {
        ...
        if ( slot == (pbm_slot_type)pbm_session_info[index].slot_id )
        {
          ...
          pbm_session_cache_free((pbm_session_enum_type)index);

          rex_enter_crit_sect(&pbm_crit_sect);
          pbm_session_info[index].isActive = FALSE;
          if ( ecc_sim_airplane && (mmgsdi_event_ptr->evt == MMGSDI_CARD_ERROR_EVT) &&
              (mmgsdi_event_ptr->data.card_error.info == MMGSDI_CARD_ERR_PWR_DN_CMD_NOTIFY ))
          {
      ...
      /* Remove ICCID's MCC from cache */
      pbm_ecc_delete_mcc(
        PBM_MMGSDI_SLOT_TO_PBM_SLOT(mmgsdi_event_ptr->data.card_error.slot),
        PBM_ECC_MCC_SOURCE_ICCID);

      if (!card_present_on_any_slot &&
          (!ecc_sim_airplane || (mmgsdi_event_ptr->evt != MMGSDI_CARD_ERROR_EVT) ||
          (mmgsdi_event_ptr->data.card_error.info != MMGSDI_CARD_ERR_PWR_DN_CMD_NOTIFY)))
      {
      ...
      else if ( card_present_on_any_slot )
      {
        uint16        slot1_mcc        = PBM_INVALID_MCC;
        uint16        slot2_mcc        = PBM_INVALID_MCC;
        boolean       ecc_ind_required = FALSE;
        uint8         loop_index       = 0;
        ...
        /* compare MCC on both the slots and delete mcc on card removed slot
           if doesn't match */
        if(slot1_mcc != slot2_mcc)
        {
          for(loop_index = 0; loop_index < PBM_ECC_MCC_SOURCE_MAX; loop_index++)
          {
            pbm_ecc_delete_mcc(slot, loop_index);
            ecc_ind_required = TRUE;
          }
        }
        ...
        if (ecc_ind_required ||
            pbm_populate_ecc_other_if_pin1_status_change(pin1_prev_status) )
        {
          PBM_SET_PB_CACHE_READY(&ecc_other_cache);
        }
      }
    ...
    case MMGSDI_FDN_EVT:
    {
      pbm_session_enum_type   pbm_card_session = PBM_SESSION_DEFAULT;
      PB_CACHE               *cache_ptr        = NULL;

      UIM_MSG_HIGH_4("PBM received MMGSDI_FDN_EVT status 0x%x  for session 0x%x, rec num 0x%x evt_source 0x%x",
                      mmgsdi_event_ptr->data.fdn.enabled,
                      session_type,
                      mmgsdi_event_ptr->data.fdn.rec_num,
                      mmgsdi_event_ptr->data.fdn.evt_source);
      ...
      /* if the card type is ICC though PBM gets FDN rec changed event on
         prov session, PBM need to update the corresponding card session */
      if(USE_GSM_SUPPORT(session_type) || USE_CDMA_SUPPORT(session_type))
      {
        session_type = pbm_card_session;
      }

      cache_ptr = (PB_CACHE *) pbm_file_id_to_cache (session_type, PBM_FILE_FDN);
      if(cache_ptr == NULL)
      {
        break;
      }

      cache_ptr->pb_id.fdn_status = mmgsdi_event_ptr->data.fdn.enabled;

      if (mmgsdi_event_ptr->data.fdn.evt_source == MMGSDI_FDN_SOURCE_CARD_INIT ||
          mmgsdi_event_ptr->data.fdn.evt_source == MMGSDI_FDN_SOURCE_REFRESH)
      {
        break;
      }
      ...
      if(mmgsdi_event_ptr->data.fdn.rec_num != 0)
      {
        int                  buffer_index          = 0;
        ...
        rec_id = pbm_session_location_to_record_id(pb_id,mmgsdi_event_ptr->data.fdn.rec_num);

        /* if PBM recieves this event before the write cnf has come from gsdi
           for FDN write originated via PBM.In this case PBM dont need to do
           anything for this event. PBM will update its commited list and
           caches when write cnf event would come from GSDI */
        for(iterator = 0; iterator < PBM_ASYNC_BUFFERS; iterator++)
        {
          /* check all pbm buffers to see if the for rec_id write rsp cnf is
             awaited */
              ...
              && (pbm_compare_rec_ids(pbm_buffers[iterator].write->rec_id,rec_id) ))
          {
            ...
            UIM_MSG_HIGH_3("PBM waiting for write cnf for record 0x%x  session 0x%x, buffer index 0x%x",
                            rec_id, session_type, iterator);
          }
        }
        if(!pbm_waiting_write_cnf) /* PBM not waiting for write confirmation */
        {
          /* if the particular FDN cache is not yet ready, we cannot update the
             cache,otherwise we need to restart the initialization of this session */
          if (cache_ptr->status != PBM_STATUS_READY)
          {
            if (pbm_session_initializing & PBM_SESSIONID_TO_SESSION_MASK(session_type))
            {
              UIM_MSG_HIGH_1("PBM Already init session 0x%x.  Continue later.\n", session_type);
          ...
          else
          {
            ...
            pbm_buffers[buffer_index].record.index = mmgsdi_event_ptr->data.fdn.rec_num;
            if ( pbm_mmgsdi_process_sim_data(session_type,
                                             PBM_FILE_FDN,
                                             mmgsdi_event_ptr->data.fdn.rec_num,
                                             mmgsdi_event_ptr->data.fdn.rec_data.data_ptr,
                                             &pbm_buffers[buffer_index].record.data.num_text,
                                             &extension_rec_num))
            {
              pbm_addr_cache_s_type          *prev_rec_ptr  = NULL;
              pbm_addr_cache_s_type          *curr_rec_ptr  = NULL;
              ...
              /* for fdn cache since fdn cache could be ready for this back
                 door event, and some thread might be updating the fdn cache via
                 record write */
              prev_rec_ptr = cache_ptr->pb_cache_array[mmgsdi_event_ptr->data.fdn.rec_num];
              if (cache_ptr->pb_cache_array[mmgsdi_event_ptr->data.fdn.rec_num] != NULL)
              {
                if ( pbm_cache_delete(cache_ptr, cache_ptr->pb_cache_array[mmgsdi_event_ptr->data.fdn.rec_num]->record_id) != PBM_SUCCESS )
                {
                  rex_leave_crit_sect(&pbm_crit_sect);
                  break;
                }
              ...
              if (record_added)
              {
                curr_rec_ptr = cache_ptr->pb_cache_array[mmgsdi_event_ptr->data.fdn.rec_num];
              }
              ...
              if(record_added)
              {
                evt_data.rec_id = pbm_session_location_to_record_id(pb_id,mmgsdi_event_ptr->data.fdn.rec_num);
                if(!prev_rec_ptr && curr_rec_ptr)
                {
                  event = PBM_EVENT_REC_ADD;
                }
                else if(prev_rec_ptr && curr_rec_ptr)
                {
              ...
              else
              {
                UIM_MSG_ERR_2("Unable to add FDN rec to cache, index 0x%x session 0x%x",
                              mmgsdi_event_ptr->data.fdn.rec_num, session_type);
              }
            ...
            {
              if (extension_rec_num && cache_ptr->extension_cache &&
                  (cache_ptr->extension_cache->file_id != PBM_FILE_NONE))
              {
                if (pbm_send_read_request(session_type,
                                           cache_ptr->extension_cache->file_id,
                                           extension_rec_num,
                                           (int) buffer_index,
                ...
                UIM_MSG_ERR_1("Could not issue read request 0x%x", extension_rec_num);
              } else
              {
        ...
        PBM_MEM_FREEIF(mmgsdi_event_ptr->data.fdn.rec_data.data_ptr);
      }
      else if (mmgsdi_event_ptr->data.fdn.enabled)
      {
    ...
    {
      UIM_MSG_HIGH_3("PBM received MMGSDI_REFRESH_EVT for session 0x%x, mode 0x%x, stage 0x%x",
                     session_type,
                     mmgsdi_event_ptr->data.refresh.mode,
                     mmgsdi_event_ptr->data.refresh.stage);

      if (mmgsdi_event_ptr->data.refresh.mode == MMGSDI_REFRESH_NAA_FCN &&
           mmgsdi_event_ptr->data.refresh.stage == MMGSDI_REFRESH_STAGE_START)
      {
        /* Send refresh complete immediately if the session is already in PBM
           intilization queue for which REFRESH FCN is received here */
        if(pbm_if_queue_has_sess(session_type, pbm_session_info[session_type].slot_id - 1))
        {
          /* This session Initialization is due to refresh - Send refresh
             complete to MMGSDI */
        ...
        else
        {
          ...
          pbm_refresh_file_change_notification_cb(&(mmgsdi_event_ptr->data.refresh.refresh_files),
                                                   mmgsdi_event_ptr->session_id);
        }
      } else if (((mmgsdi_event_ptr->data.refresh.mode == MMGSDI_REFRESH_NAA_INIT_FCN) ||
                 (mmgsdi_event_ptr->data.refresh.mode == MMGSDI_REFRESH_3G_SESSION_RESET) ||
                 (mmgsdi_event_ptr->data.refresh.mode == MMGSDI_REFRESH_NAA_FCN) )&&
                 (mmgsdi_event_ptr->data.refresh.stage == MMGSDI_REFRESH_STAGE_WAIT_FOR_OK_TO_FCN))
      {
        /* Notify MMGSDI it's okay to proceed with refresh for the above modes
           of refresh */
        ret = mmgsdi_session_ok_to_refresh(mmgsdi_event_ptr->session_id,
                                           TRUE,
                                           pbm_mmgsdi_ok_to_refresh_cb,
                                           0);
      } else if ( ( (mmgsdi_event_ptr->data.refresh.mode == MMGSDI_REFRESH_NAA_APP_RESET)
                  || (mmgsdi_event_ptr->data.refresh.mode == MMGSDI_REFRESH_RESET) )
                  && (mmgsdi_event_ptr->data.refresh.stage == MMGSDI_REFRESH_STAGE_START))
      {
        /* reset the LPM status variable, since in case of reset refresh, we
           need to read ecc again */
        PBM_SESSION_RESET(pbm_session_lpm_status, session_type);
      }

      if ( ret != MMGSDI_SUCCESS )
      {
        UIM_MSG_HIGH_1(" PBM: mmgsdi_session_ok_to_refresh returned error  0x%x",ret);
      }

      /* Clean up deep copy of file path */
      PBM_MEM_FREEIF(mmgsdi_event_ptr->data.refresh.refresh_files.file_path_ptr);
    }
    break;

    case MMGSDI_SESSION_CHANGED_EVT:
    {
      mmgsdi_app_info_type    app_info = mmgsdi_event_ptr->data.session_changed.app_info;
      PB_CACHE               *ecc_cache = NULL;
      atomic_word_t           pin1_prev_status;

      pin1_prev_status.value = pbm_pin1_verified.value ;

      UIM_MSG_HIGH_4("PBM received MMGSDI_SESSION_CHANGED_EVT event on slot 0x%x session 0x%x app_type 0x%x activated Status 0x%x",
                     (uint16) mmgsdi_event_ptr->data.session_changed.app_info.slot,
                     session_type,
                     app_info.app_data.app_type,
                     mmgsdi_event_ptr->data.session_changed.activated);
      ...
      if(mmgsdi_event_ptr->data.session_changed.activated)
      {
        if( pbm_session_type_to_pb_category(session_type) != PBM_GPB )
        {
          pbm_session_info[session_type].app_type = app_info.app_data.app_type;
        ...
        pbm_session_info[session_type].slot_id = (uint16) mmgsdi_event_ptr->data.session_changed.app_info.slot;

        /* Initialize session related cahces*/
        pbm_session_cache_init(session_type);
      }
      else
      {
        pbm_ecc_mcc_soruce_type source                   = PBM_ECC_MCC_SOURCE_MAX;
        boolean                 card_present_on_any_slot = FALSE;

        if(session_type == PBM_SESSION_LPB_GW ||
           session_type == PBM_SESSION_LPB_GW_SECONDARY)
        {
          source       = PBM_ECC_MCC_SOURCE_IMSI;
        }
        else
        {
          source       = PBM_ECC_MCC_SOURCE_IMSI_M;
        }
        ...
        /* Delete IMSI based MCC from cache */
        pbm_ecc_delete_mcc(
          PBM_MMGSDI_SLOT_TO_PBM_SLOT(mmgsdi_event_ptr->data.session_changed.app_info.slot),
          source);
        ...
        /* pin1 cleared in the above de-activate handling and hence cache
           building is to be after that */
        (void)pbm_populate_ecc_other_if_pin1_status_change(pin1_prev_status);

        /* Skip ECC ready indication if an card error received before session
           de-activation */
        if(pbm_card_status[PBM_SLOT_1 - 1] ||
           pbm_card_status[PBM_SLOT_2 - 1] ||
           pbm_card_status[PBM_SLOT_3 - 1])
        {
          /* sending cache ready to clients - not checking for
             pin1_status_change has updated hardcoded list because this ready
             has to go irrespective of its update.Because network and card ecc
             are cleared out*/
    ...
    case MMGSDI_SUBSCRIPTION_READY_EVT:
      pbm_handle_sub_ready_evt(mmgsdi_event_ptr);
    break;
    ...
    case MMGSDI_HIDDENKEY_EVT:
    {
      ...
      /* check if the event has come after the ADN cache was ready we need to
         start the ADN read again check to see if the cache is ready*/
      UIM_MSG_HIGH_3("PBM received MMGSDI_HIDDENKEY_EVT event on session 0x%x pin status 0x%x with dir_index 0x%x",
                      session_type, mmgsdi_event_ptr->data.pin.pin_info.status, mmgsdi_event_ptr->data.pin.dir_index);
      rex_enter_crit_sect(&pbm_crit_sect);
      switch(mmgsdi_event_ptr->data.pin.pin_info.status)
      {
        case MMGSDI_PIN_ENABLED_VERIFIED:
        case MMGSDI_PIN_DISABLED:
          ...
          pbm_session_info[session_type].dir_index = mmgsdi_event_ptr->data.pin.dir_index;
          ...
          pbm_session_info[session_type].dir_index != 0)
      {
        ...
        /* merge_hidden_contacts functionality this funcion will also be called
           from the init flow if verify event comes in between the init flow */
        if(pbm_hidden_key_verified & PBM_SESSIONID_TO_SESSION_MASK(session_type))
        {
          /* this api NEEDS TO KNOW THE INDICES where the hidden cache has to
             be added to adn cache */
          if(pbm_merge_hidden_contacts(session_type) == PBM_SUCCESS)
          {
        ...
        else
        {
          /* this api NEEDS TO KNOW THE INDICES where the hidden cache has to
             be added to adn cache */
          if(pbm_delete_hidden_contacts_from_adn_cache(session_type) == PBM_SUCCESS)
          {
            /* to send event to clients that the cache has been updated */
...
)
{
  ...
  while ((cmd_ptr = pbm_cmd_q_get(&pbm_client_cmd_q)) != NULL)
  {
    switch (cmd_ptr->cmd_type)
    {
      ...
      default:
        UIM_MSG_ERR_1("Unknown command 0x%x", cmd_ptr->cmd_type);
          continue;
    }
    ...
    {
      sigs = pbm_wait(PBM_CMD_COMPLETE_SIG |
